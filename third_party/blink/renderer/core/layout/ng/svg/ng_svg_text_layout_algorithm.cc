...
NGSvgTextLayoutAlgorithm::NGSvgTextLayoutAlgorithm(NGInlineNode node, WritingMode writing_mode)
    : inline_node_(node)
    ,
...
PhysicalSize NGSvgTextLayoutAlgorithm::Layout(const String& ifc_text_content, NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
...
void NGSvgTextLayoutAlgorithm::SetFlags(const String& ifc_text_content, const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    if (inline_node_.IsBidiEnabled()) {
        std::sort(sorted_item_indexes.data(), sorted_item_indexes.data() + sorted_item_indexes.size(),
            [&](wtf_size_t a, wtf_size_t b) { return items[a]->StartOffset() < items[b]->StartOffset(); });
    }
    ...
    for (wtf_size_t i : sorted_item_indexes) {
        ...
        gfx::PointF offset(logical_offset.inline_offset, logical_offset.block_offset + ascent);
        if (!horizontal_)
        ...
        for (unsigned text_offset = item_string.NextCodePointOffset(0); text_offset < item_string.length();
...
void NGSvgTextLayoutAlgorithm::AdjustPositionsDxDy(const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
...
void NGSvgTextLayoutAlgorithm::ApplyTextLengthAttribute(const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
...
void NGSvgTextLayoutAlgorithm::ResolveTextLength(
    const NGFragmentItemsBuilder::ItemWithOffsetList& items, const SvgTextContentRange& range, Vector<wtf_size_t>& resolved_descendant_node_starts)
{
    ...
    const float text_length = element->textLength()->CurrentValue()->Value(SVGLengthContext(element)) * ScalingFactorAt(items, i);
    ...
    if (length_adjust == kSVGLengthAdjustSpacingAndGlyphs) {
        ...
        auto n = std::count_if(result_.begin() + i, result_.begin() + j_plus_1, [](const auto& info) { return !info.middle && !info.text_length_resolved; });
        // 2.4.3. Let n = n + number of resolved descendant nodes − 1.
        n += std::count_if(resolved_descendant_node_starts.begin(), resolved_descendant_node_starts.end(), [i, j_plus_1](const auto& start_index) {
...
void NGSvgTextLayoutAlgorithm::AdjustPositionsXY(const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
...
void NGSvgTextLayoutAlgorithm::ApplyAnchoring(const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    while (i < result_.size()) {
        auto* next_anchor = std::find_if(result_.begin() + i + 1, result_.end(), [](const auto& info) { return info.anchored_chunk; });
        wtf_size_t j = static_cast<wtf_size_t>(std::distance(result_.begin(), next_anchor) - 1);
...
void NGSvgTextLayoutAlgorithm::PositionOnPath(const NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
        ...
        if (range_index < ranges.size() && index >= ranges[range_index].start_index && index <= ranges[range_index].end_index) {
            ...
            if (!info.middle) {
                ...
                if (!path_mapper) {
                    ...
                    const float mid = ((horizontal_ ? *info.x : *info.y) + info.inline_size / 2) / scaling_factor + offset;
            ...
            if (in_path) {
                ...
                if (path_mapper) {
                    ...
                    // drawn character.
                    const auto rbegin = std::make_reverse_iterator(result_.begin() + index);
                    const auto rend = std::make_reverse_iterator(result_.begin());
                    const auto iter = std::find_if(rbegin, rend, [](const auto& info) { return !info.hidden && !info.middle; });
                    if (iter != rend) {
                        if (horizontal_) {
...
PhysicalSize NGSvgTextLayoutAlgorithm::WriteBackToFragmentItems(NGFragmentItemsBuilder::ItemWithOffsetList& items)
{
    ...
    for (const SvgPerCharacterInfo& info : result_) {
        ...
        item.item.ConvertToSvgText(std::move(data), PhysicalRect::EnclosingRect(unscaled_rect), info.hidden);
    ...
    DCHECK_EQ(base::ranges::find(items.begin() + 1, items.end(), NGFragmentItem::kLine, &NGFragmentItem::Type), items.end());
    return { LayoutUnit(unscaled_visual_rect.right()), LayoutUnit(unscaled_visual_rect.bottom()) };
}

float NGSvgTextLayoutAlgorithm::ScalingFactorAt(const NGFragmentItemsBuilder::ItemWithOffsetList& items, wtf_size_t addressable_index) const
{
    return items[result_[addressable_index].item_index]->SvgScalingFactor();
}

bool NGSvgTextLayoutAlgorithm::IsFirstCharacterInTextPath(wtf_size_t index) const
{
