...
void LayoutNGSVGText::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    NOT_DESTROYED();
...
{
    NOT_DESTROYED();
    return type == kLayoutObjectNGSVGText || LayoutNGBlockFlowMixin<LayoutSVGBlock>::IsOfType(type);
}
...
bool LayoutNGSVGText::IsChildAllowed(LayoutObject* child, const ComputedStyle&) const
{
    NOT_DESTROYED();
    return child->IsSVGInline() || (child->IsText() && SVGLayoutSupport::IsLayoutableTextNode(child));
}

void LayoutNGSVGText::AddChild(LayoutObject* child, LayoutObject* before_child)
{
    NOT_DESTROYED();
...
void LayoutNGSVGText::SubtreeStructureChanged(LayoutInvalidationReasonForTracing)
{
    NOT_DESTROYED();
...
void LayoutNGSVGText::AbsoluteQuads(Vector<gfx::QuadF>& quads, MapCoordinatesFlags mode) const
{
    NOT_DESTROYED();
...
bool LayoutNGSVGText::NodeAtPoint(HitTestResult& result, const HitTestLocation& hit_test_location, const PhysicalOffset& accumulated_offset, HitTestPhase phase)
{
    ...
    return LayoutNGBlockFlowMixin<LayoutSVGBlock>::NodeAtPoint(result, *local_location, accumulated_offset, phase);
}

PositionWithAffinity LayoutNGSVGText::PositionForPoint(const PhysicalOffset& point_in_contents) const
{
    NOT_DESTROYED();
