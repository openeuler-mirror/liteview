...
class CORE_EXPORT NGColumnLayoutAlgorithm : public NGLayoutAlgorithm<NGBlockNode, NGBoxFragmentBuilder, NGBlockBreakToken> {
public:
...
private:
    MinMaxSizesResult ComputeSpannersMinMaxSizes(const NGBlockNode& search_parent) const;
    ...
    const NGLayoutResult* LayoutRow(const NGBlockBreakToken* next_column_token, NGMarginStrut*);
    ...
    // columns.
    NGBreakStatus LayoutSpanner(NGBlockNode spanner_node, const NGBlockBreakToken* break_token, NGMarginStrut*);
    ...
    void AttemptToPositionListMarker(const NGPhysicalBoxFragment& child_fragment, LayoutUnit block_offset);
    ...
    void PropagateBaselineFromChild(const NGPhysicalBoxFragment& child, LayoutUnit block_offset);
    ...
    LayoutUnit CalculateBalancedColumnBlockSize(const LogicalSize& column_size, LayoutUnit row_offset, const NGBlockBreakToken* child_break_token);

    LayoutUnit CalculateBalancedColumnBlockSizeInternal(const LogicalSize& column_size, LayoutUnit row_offset, const NGBlockBreakToken* child_break_token);

    LayoutUnit ConstrainColumnBlockSize(LayoutUnit size, LayoutUnit row_offset) const;
    ...
    NGConstraintSpace CreateConstraintSpaceForBalancing(const LogicalSize& column_size) const;
    NGConstraintSpace CreateConstraintSpaceForSpanner(const NGBlockNode& spanner, LayoutUnit block_offset) const;
