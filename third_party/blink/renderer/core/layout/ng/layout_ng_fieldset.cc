...
void LayoutNGFieldset::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
    ...
    if (new_child->IsRenderedLegendCandidate() && !LayoutFieldset::FindInFlowLegend(*this)) {
...
void LayoutNGFieldset::UpdateAnonymousChildStyle(const LayoutObject*, ComputedStyle& child_style) const
{
    ...
    child_style.SetColumnRuleColor(StyleColor(LayoutObject::ResolveColor(StyleRef(), GetCSSPropertyColumnRuleColor())));
    ...
    child_style.SetNamedGridAreaColumnCount(StyleRef().NamedGridAreaColumnCount());
    child_style.SetNamedGridAreaRowCount(StyleRef().NamedGridAreaRowCount());
    child_style.SetImplicitNamedGridColumnLines(StyleRef().ImplicitNamedGridColumnLines());
    child_style.SetImplicitNamedGridRowLines(StyleRef().ImplicitNamedGridRowLines());
...
void LayoutNGFieldset::InvalidatePaint(const PaintInvalidatorContext& context) const
{
    ...
    if (legend_box && legend_box->ShouldCheckGeometryForPaintInvalidation()) {
        GetMutableForPainting().SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kGeometry);
    }
    LayoutNGBlockFlow::InvalidatePaint(context);
}

bool LayoutNGFieldset::BackgroundIsKnownToBeOpaqueInRect(const PhysicalRect& local_rect) const
{
