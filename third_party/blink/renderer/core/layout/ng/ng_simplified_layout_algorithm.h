...
class CORE_EXPORT NGSimplifiedLayoutAlgorithm : public NGLayoutAlgorithm<NGBlockNode, NGBoxFragmentBuilder, NGBlockBreakToken> {
public:
    NGSimplifiedLayoutAlgorithm(const NGLayoutAlgorithmParams&, const NGLayoutResult&, bool keep_old_size = false);
...
private:
    void AddChildFragment(
        const NGLink& old_fragment, const NGPhysicalFragment& new_fragment, const NGMarginStrut* margin_strut = nullptr, bool is_self_collapsing = false);
