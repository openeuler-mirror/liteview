...
NGFlexItemIterator::NGFlexItemIterator(const HeapVector<NGFlexLine>& flex_lines, const NGBlockBreakToken* break_token, bool is_column)
...
NGFlexItemIterator::Entry NGFlexItemIterator::NextItem(bool broke_before_row)
{
    ...
    if (break_token_) {
        ...
        if (child_token_idx_ < child_break_tokens.size()) {
            current_child_break_token = To<NGBlockBreakToken>(child_break_tokens[child_token_idx_++].Get());
...
NGFlexItem* NGFlexItemIterator::FindNextItem(const NGBlockBreakToken* item_break_token)
{
