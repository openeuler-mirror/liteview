...
class CORE_EXPORT NGFlexLayoutAlgorithm : public NGLayoutAlgorithm<NGBlockNode, NGBoxFragmentBuilder, NGBlockBreakToken> {
public:
    explicit NGFlexLayoutAlgorithm(
        const NGLayoutAlgorithmParams& params, DevtoolsFlexInfo* devtools = nullptr, const HashMap<wtf_size_t, LayoutUnit>* cross_size_adjustments = nullptr);
...
private:
    ...
    const NGLayoutResult* RelayoutAndBreakEarlierForFlex(const NGLayoutResult* previous_result);
    ...
    bool IsUsedFlexBasisDefinite(const NGBlockNode& child, Length* flex_basis) const;
    bool DoesItemCrossSizeComputeToAuto(const NGBlockNode& child) const;
    bool IsItemCrossAxisLengthDefinite(const NGBlockNode& child, const Length& length) const;
    ...
    LayoutUnit AdjustMainSizeForAspectRatioCrossAxisMinAndMax(
        const NGBlockNode& child, LayoutUnit main_size, const MinMaxSizes& cross_min_max, const NGBoxStrut& border_padding_in_child_writing_mode);

    bool IsColumnContainerMainSizeDefinite() const;
    bool IsContainerCrossSizeDefinite() const;

    NGConstraintSpace BuildSpaceForIntrinsicInlineSize(const NGBlockNode& flex_item) const;
    NGConstraintSpace BuildSpaceForFlexBasis(const NGBlockNode& flex_item) const;
    NGConstraintSpace BuildSpaceForIntrinsicBlockSize(const NGBlockNode& flex_item, absl::optional<LayoutUnit> override_inline_size = absl::nullopt) const;
    ...
    NGConstraintSpace BuildSpaceForLayout(const NGBlockNode& flex_item_node, LayoutUnit item_main_axis_final_size,
        absl::optional<LayoutUnit> override_inline_size = absl::nullopt, absl::optional<LayoutUnit> line_cross_size_for_stretch = absl::nullopt,
        absl::optional<LayoutUnit> block_offset_for_fragmentation = absl::nullopt, bool min_block_size_should_encompass_intrinsic_size = false) const;
    ...
    void ApplyFinalAlignmentAndReversals(HeapVector<NGFlexLine>* flex_line_outputs);
    NGLayoutResult::EStatus GiveItemsFinalPositionAndSize(HeapVector<NGFlexLine>* flex_line_outputs, Vector<EBreakBetween>* row_break_between_outputs);
    ...
    NGLayoutResult::EStatus PropagateFlexItemInfo(FlexItem* flex_item, wtf_size_t flex_line_idx, LogicalOffset offset, PhysicalSize fragment_size);
    ...
    MinMaxSizesResult ComputeItemContributions(const NGConstraintSpace& space, const FlexItem& item) const;
    ...
    void ConsumeRemainingFragmentainerSpace(
        LayoutUnit previously_consumed_block_size, NGFlexLine* flex_line, const NGFlexColumnBreakInfo* column_break_info = nullptr);
    ...
    NGBreakStatus BreakBeforeRowIfNeeded(const NGFlexLine& row, EBreakBetween row_break_between, wtf_size_t row_index, NGLayoutInputNode child,
    ...
    // more documentation.
    bool MovePastRowBreakPoint(NGBreakAppeal appeal_before, LayoutUnit fragmentainer_block_offset, LayoutUnit row_block_size, wtf_size_t row_index,
