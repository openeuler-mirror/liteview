...
public:
    ...
    void AccumulateItem(const NGBoxFragment& fragment, const LayoutUnit block_offset, bool is_first_line, bool is_last_line)
    {
        if (is_first_line) {
...
NGFlexLayoutAlgorithm::NGFlexLayoutAlgorithm(
    const NGLayoutAlgorithmParams& params, DevtoolsFlexInfo* layout_info_for_devtools, const HashMap<wtf_size_t, LayoutUnit>* cross_size_adjustments)
    ...
    , child_percentage_size_(CalculateChildPercentageSize(ConstraintSpace(), Node(), ChildAvailableSize()))
...
bool NGFlexLayoutAlgorithm::MainAxisIsInlineAxis(const NGBlockNode& child) const
{
    return child.Style().IsHorizontalWritingMode() == is_horizontal_flow_;
}

LayoutUnit NGFlexLayoutAlgorithm::MainAxisContentExtent(LayoutUnit sum_hypothetical_main_size) const
{
    if (is_column_) {
        ...
        return ComputeBlockSizeForFragment(
                   ConstraintSpace(), Style(), BorderPadding(), sum_hypothetical_main_size.ClampNegativeToZero() + border_scrollbar_padding, inline_size)
...
AxisEdge MainAxisStaticPositionEdge(const ComputedStyle& style, bool is_column)
{
    const StyleContentAlignmentData justify = FlexLayoutAlgorithm::ResolvedJustifyContent(style);
    const ContentPosition content_position = justify.GetPosition();
    bool is_reverse_flex = is_column ? style.ResolvedIsColumnReverseFlexDirection() : style.ResolvedIsRowReverseFlexDirection();
...
{
    ItemPosition alignment = FlexLayoutAlgorithm::AlignmentForChild(style, child_style);
...
void NGFlexLayoutAlgorithm::HandleOutOfFlowPositionedItems(HeapVector<Member<LayoutBox>>& oof_children)
{
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        ...
        if (should_process_block_end) {
            ...
            total_block_size_
                = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), total_intrinsic_block_size_, container_builder_.InlineSize());
        } else {
            LayoutUnit center = total_block_size_ / 2;
            should_process_block_center = center - previous_consumed_block_size <= FragmentainerCapacity(ConstraintSpace());
        }
    }
    ...
    for (LayoutBox* oof_child : oofs) {
        ...
        if (!IsResumingLayout(BreakToken())) {
            ...
            const PhysicalToLogical<Length> insets_in_flexbox_writing_mode(
                Style().GetWritingDirection(), child_style.Top(), child_style.Right(), child_style.Bottom(), child_style.Left());
            if (is_column_) {
                const ItemPosition normalized_alignment = FlexLayoutAlgorithm::AlignmentForChild(style, child_style);
                const ItemPosition default_justify_self_behavior = child.IsReplaced() ? ItemPosition::kStart : ItemPosition::kStretch;
                const ItemPosition normalized_justify = FlexLayoutAlgorithm::TranslateItemPosition(
                    style, child_style, child_style.ResolvedJustifySelf(default_justify_self_behavior).GetPosition());

                const bool are_cross_axis_insets_auto
                    = insets_in_flexbox_writing_mode.InlineStart().IsAuto() && insets_in_flexbox_writing_mode.InlineEnd().IsAuto();

                if (normalized_alignment != normalized_justify && are_cross_axis_insets_auto) {
                    UseCounter::Count(Node().GetDocument(), WebFeature::kFlexboxNewAbsPos);
                }
            }
            if (main_axis_edge != AxisEdge::kStart) {
                const bool are_main_axis_insets_auto = is_column_
                    ? insets_in_flexbox_writing_mode.BlockStart().IsAuto() && insets_in_flexbox_writing_mode.BlockEnd().IsAuto()
                    : insets_in_flexbox_writing_mode.InlineStart().IsAuto() && insets_in_flexbox_writing_mode.InlineEnd().IsAuto();
                if (are_main_axis_insets_auto) {
                    UseCounter::Count(Node().GetDocument(), WebFeature::kFlexboxAbsPosJustifyContent);
                }
            }
        }
...
bool NGFlexLayoutAlgorithm::DoesItemStretch(const NGBlockNode& child) const
{
    ...
    if (!is_horizontal_flow_ && (child_style.MarginLeft().IsAuto() || child_style.MarginRight().IsAuto()))
        return false;
    return FlexLayoutAlgorithm::AlignmentForChild(Style(), child_style) == ItemPosition::kStretch;
}

bool NGFlexLayoutAlgorithm::IsUsedFlexBasisDefinite(const NGBlockNode& child, Length* out_flex_basis = nullptr) const
{
...
bool NGFlexLayoutAlgorithm::IsItemCrossAxisLengthDefinite(const NGBlockNode& child, const Length& length) const
{
...
bool NGFlexLayoutAlgorithm::DoesItemCrossSizeComputeToAuto(const NGBlockNode& child) const
{
    const ComputedStyle& child_style = child.Style();
...
bool NGFlexLayoutAlgorithm::AspectRatioProvidesMainSize(const NGBlockNode& child) const
{
    const Length& cross_axis_length = is_horizontal_flow_ ? child.Style().Height() : child.Style().Width();
    return child.HasAspectRatio() && (IsItemCrossAxisLengthDefinite(child, cross_axis_length) || WillChildCrossSizeBeContainerCrossSize(child));
}

bool NGFlexLayoutAlgorithm::WillChildCrossSizeBeContainerCrossSize(const NGBlockNode& child) const
{
    return !algorithm_.IsMultiline() && is_cross_size_definite_ && DoesItemStretch(child);
}

NGConstraintSpace NGFlexLayoutAlgorithm::BuildSpaceForIntrinsicInlineSize(const NGBlockNode& child) const
{
...
NGConstraintSpace NGFlexLayoutAlgorithm::BuildSpaceForIntrinsicBlockSize(const NGBlockNode& flex_item, absl::optional<LayoutUnit> override_inline_size) const
{
    const ComputedStyle& child_style = flex_item.Style();
    NGConstraintSpaceBuilder space_builder(ConstraintSpace(), child_style.GetWritingDirection(),
        /* is_new_fc */ true);
...
NGConstraintSpace NGFlexLayoutAlgorithm::BuildSpaceForFlexBasis(const NGBlockNode& flex_item) const
{
    NGConstraintSpaceBuilder space_builder(ConstraintSpace(), flex_item.Style().GetWritingDirection(),
        /* is_new_fc */ true);
    SetOrthogonalFallbackInlineSizeIfNeeded(Style(), flex_item, &space_builder);
...
Length NGFlexLayoutAlgorithm::GetUsedFlexBasis(const NGBlockNode& child) const
{
    ...
    if (specified_flex_basis.IsAuto()) {
        if (specified_length_in_main_axis.IsAuto() && Style().IsDeprecatedWebkitBox()
            && (Style().BoxOrient() == EBoxOrient::kHorizontal || Style().BoxAlign() != EBoxAlignment::kStretch)) {
...
NGConstraintSpace NGFlexLayoutAlgorithm::BuildSpaceForLayout(const NGBlockNode& flex_item_node, LayoutUnit item_main_axis_final_size,
    absl::optional<LayoutUnit> override_inline_size, absl::optional<LayoutUnit> line_cross_size_for_stretch,
    absl::optional<LayoutUnit> block_offset_for_fragmentation, bool min_block_size_should_encompass_intrinsic_size) const
{
    ...
    if (is_column_) {
        ...
        if (!IsColumnContainerMainSizeDefinite() && !IsUsedFlexBasisDefinite(flex_item_node) && !AspectRatioProvidesMainSize(flex_item_node)) {
            space_builder.SetIsInitialBlockSizeIndefinite(true);
        }
    } else {
    ...
    if (!line_cross_size_for_stretch && DoesItemStretch(flex_item_node)) {
        ...
        space_builder.SetCacheSlot(NGCacheSlot::kMeasure);
    } else if (block_offset_for_fragmentation && ConstraintSpace().HasBlockFragmentation()) {
        if (min_block_size_should_encompass_intrinsic_size)
            space_builder.SetMinBlockSizeShouldEncompassIntrinsicSize();
        SetupSpaceBuilderForFragmentation(ConstraintSpace(), flex_item_node, *block_offset_for_fragmentation, &space_builder,
    ...
    if (Node().IsButton()) {
        space_builder.SetBaselineAlgorithmType(ConstraintSpace().BaselineAlgorithmType());
    }

    return space_builder.ToConstraintSpace();
}

void NGFlexLayoutAlgorithm::ConstructAndAppendFlexItems(Phase phase, HeapVector<Member<LayoutBox>>* oof_children)
{
    ...
    const StyleContentAlignmentData align_content = algorithm_.ResolvedAlignContent(Style());
    bool is_alignment_behavior_change_possible = !algorithm_.IsMultiline() && align_content.Distribution() != ContentDistributionType::kStretch
        && align_content.GetPosition() != ContentPosition::kBaseline;
    const LayoutUnit kAvailableFreeSpace(100);
    LayoutUnit line_offset = FlexLayoutAlgorithm::InitialContentPositionOffset(Style(), kAvailableFreeSpace, align_content,
        /* number_of_items */ 1,
        /* is_reversed */ false);
    ...
    for (NGBlockNode child = iterator.NextChild(); child; child = iterator.NextChild()) {
        ...
        if (phase == Phase::kColumnWrapIntrinsicSize) {
            ...
            NGBoxStrut child_margins = ComputeMarginsFor(space, child.Style(), ConstraintSpace());
        ...
        if (is_alignment_behavior_change_possible && all_items_match_container_alignment && phase == Phase::kLayout) {
            LayoutUnit item_offset = FlexItem::AlignmentOffset(kAvailableFreeSpace, FlexLayoutAlgorithm::AlignmentForChild(Style(), child_style), LayoutUnit(),
                /* is_wrap_reverse */ false, Style().IsDeprecatedWebkitBox());
        ...
        NGPhysicalBoxStrut physical_child_margins = ComputePhysicalMargins(flex_basis_space, child_style);

        NGBoxStrut border_padding_in_child_writing_mode = ComputeBorders(flex_basis_space, child) + ComputePadding(flex_basis_space, child_style);

        NGPhysicalBoxStrut physical_border_padding(border_padding_in_child_writing_mode.ConvertToPhysical(child_style.GetWritingDirection()));

        LayoutUnit main_axis_border_padding = is_horizontal_flow_ ? physical_border_padding.HorizontalSum() : physical_border_padding.VerticalSum();
        LayoutUnit cross_axis_border_padding = is_horizontal_flow_ ? physical_border_padding.VerticalSum() : physical_border_padding.HorizontalSum();
        ...
        auto MinMaxSizesFunc = [&](MinMaxSizesType type) -> MinMaxSizesResult {
            if (!min_max_sizes) {
                ...
                min_max_sizes = child.ComputeMinMaxSizes(child_writing_mode, type, child_space);
            }
            return *min_max_sizes;
        };

        MinMaxSizes min_max_sizes_in_main_axis_direction { main_axis_border_padding, LayoutUnit::Max() };
        MinMaxSizes min_max_sizes_in_cross_axis_direction { LayoutUnit(), LayoutUnit::Max() };
        const Length& max_property_in_main_axis = is_horizontal_flow_ ? child.Style().MaxWidth() : child.Style().MaxHeight();
        if (MainAxisIsInlineAxis(child)) {
            min_max_sizes_in_main_axis_direction.max_size
                = ResolveMaxInlineLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, MinMaxSizesFunc, max_property_in_main_axis);
            min_max_sizes_in_cross_axis_direction = ComputeMinMaxBlockSizes(flex_basis_space, child_style, border_padding_in_child_writing_mode);
        } else {
            min_max_sizes_in_main_axis_direction.max_size
                = ResolveMaxBlockLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, max_property_in_main_axis);
            min_max_sizes_in_cross_axis_direction = ComputeMinMaxInlineSizes(flex_basis_space, child, border_padding_in_child_writing_mode, MinMaxSizesFunc);
        }

        auto ComputeTransferredMainSize = [&]() -> LayoutUnit {
            DCHECK(!IsItemCrossAxisLengthDefinite(child, cross_axis_length) || !WillChildCrossSizeBeContainerCrossSize(child))
            ...
            if (MainAxisIsInlineAxis(child)) {
                cross_size = ResolveMainBlockLength(
                    flex_basis_space, child_style, border_padding_in_child_writing_mode, cross_axis_length_to_resolve, kIndefiniteSize);
            } else {
                if (cross_axis_length_to_resolve.IsFitContent() && flex_basis_space.AvailableSize().inline_size == kIndefiniteSize) {
                    ...
                    cross_size = ResolveMainInlineLength(
                        flex_basis_space, child_style, border_padding_in_child_writing_mode, MinMaxSizesFunc, cross_axis_length_to_resolve);
                }
            }

            DCHECK_GE(cross_size, LayoutUnit());
            cross_size = min_max_sizes_in_cross_axis_direction.ClampSizeToMinAndMax(cross_size);
            if (MainAxisIsInlineAxis(child)) {
                return InlineSizeFromAspectRatio(
                    border_padding_in_child_writing_mode, child.GetAspectRatio(), child_style.BoxSizingForAspectRatio(), cross_size);
            }
            return BlockSizeFromAspectRatio(border_padding_in_child_writing_mode, child.GetAspectRatio(), child_style.BoxSizingForAspectRatio(), cross_size);
        };

        const NGLayoutResult* layout_result = nullptr;
        auto IntrinsicBlockSizeFunc = [&](MinMaxSizesType type = MinMaxSizesType::kIntrinsic) -> LayoutUnit {
            if (type == MinMaxSizesType::kContent && child.HasAspectRatio() && !child.IsReplaced()) {
                ...
                DCHECK(!AspectRatioProvidesMainSize(child)) << "We only ever call IntrinsicBlockSizeFunc with kContent for "
                                                               "determing flex base size in case E. If "
                                                               "AspectRatioProvidesMainSize==true, we would have fallen into "
                                                               "case B, not case E.";
                DCHECK(!MainAxisIsInlineAxis(child)) << "We assume that the main axis is block axis in the call to "
                                                        "BlockSum() below.";
                return AdjustMainSizeForAspectRatioCrossAxisMinAndMax(
                    child, ComputeTransferredMainSize(), min_max_sizes_in_cross_axis_direction, border_padding_in_child_writing_mode);
            }
            if (!layout_result) {
                NGConstraintSpace child_space = BuildSpaceForIntrinsicBlockSize(child, max_content_contribution);
        ...
        if (!IsUsedFlexBasisDefinite(child, &flex_basis_length)) {
            ...
            if (AspectRatioProvidesMainSize(child)) {
                ...
                // written for case E.
                flex_base_border_box = MinMaxSizesFunc(MinMaxSizesType::kContent).sizes.max_size;
            } else {
                // Parts C, D, and E for what are usually column flex containers.
                flex_base_border_box = IntrinsicBlockSizeFunc(MinMaxSizesType::kContent);
            }
        } else {
            ...
            if (MainAxisIsInlineAxis(child)) {
                flex_base_border_box
                    = ResolveMainInlineLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, MinMaxSizesFunc, flex_basis_length);
            } else {
                // Flex container's main axis is in child's block direction. Child's
                // flex basis is in child's block direction.
                flex_base_border_box
                    = ResolveMainBlockLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, flex_basis_length, IntrinsicBlockSizeFunc);
                if (const NGTableNode* table_child = DynamicTo<NGTableNode>(&child)) {
                    ...
                    LayoutUnit caption_block_size
                        = table_child->ComputeCaptionBlockSize(BuildSpaceForIntrinsicBlockSize(*table_child, max_content_contribution));
        ...
        LayoutUnit flex_base_content_size = flex_base_border_box - main_axis_border_padding;

        const Length& min = is_horizontal_flow_ ? child.Style().MinWidth() : child.Style().MinHeight();
        if (algorithm_.ShouldApplyMinSizeAutoForChild(*child.GetLayoutBox())) {
            LayoutUnit content_size_suggestion;
            if (MainAxisIsInlineAxis(child)) {
                content_size_suggestion = MinMaxSizesFunc(MinMaxSizesType::kContent).sizes.min_size;
            } else {
            ...
            if (child.HasAspectRatio() && !MainAxisIsInlineAxis(child)) {
                content_size_suggestion = AdjustMainSizeForAspectRatioCrossAxisMinAndMax(
                    child, content_size_suggestion, min_max_sizes_in_cross_axis_direction, border_padding_in_child_writing_mode);
            }
            ...
            if (MainAxisIsInlineAxis(child)) {
                if (!specified_length_in_main_axis.IsAuto() && !InlineLengthUnresolvable(flex_basis_space, specified_length_in_main_axis)) {
                    ...
                    specified_size_suggestion = ResolveMainInlineLength(
                        flex_basis_space, child_style, border_padding_in_child_writing_mode, MinMaxSizesFunc, specified_length_in_main_axis);
                }
            } else if (!BlockLengthUnresolvable(flex_basis_space, specified_length_in_main_axis)) {
                specified_size_suggestion = ResolveMainBlockLength(
                    flex_basis_space, child_style, border_padding_in_child_writing_mode, specified_length_in_main_axis, IntrinsicBlockSizeFunc);
                DCHECK_NE(specified_size_suggestion, kIndefiniteSize);
            }

            LayoutUnit transferred_size_suggestion = LayoutUnit::Max();
            if (specified_size_suggestion == LayoutUnit::Max() && child.IsReplaced() && AspectRatioProvidesMainSize(child)) {
                transferred_size_suggestion = ComputeTransferredMainSize();
            }

            DCHECK(specified_size_suggestion == LayoutUnit::Max() || transferred_size_suggestion == LayoutUnit::Max());

            min_max_sizes_in_main_axis_direction.min_size
                = std::min({ specified_size_suggestion, content_size_suggestion, transferred_size_suggestion, min_max_sizes_in_main_axis_direction.max_size });
        } else if (MainAxisIsInlineAxis(child)) {
            min_max_sizes_in_main_axis_direction.min_size
                = ResolveMinInlineLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, MinMaxSizesFunc, min);
        } else {
            min_max_sizes_in_main_axis_direction.min_size = ResolveMinBlockLength(flex_basis_space, child_style, border_padding_in_child_writing_mode, min);
        }
        ...
        if (child.IsTable()) {
            if (MainAxisIsInlineAxis(child)) {
                min_max_sizes_in_main_axis_direction.Encompass(MinMaxSizesFunc(MinMaxSizesType::kContent).sizes.min_size);
            } else {
                min_max_sizes_in_main_axis_direction.Encompass(IntrinsicBlockSizeFunc());
            }
        }
        ...
        bool is_last_baseline = FlexLayoutAlgorithm::AlignmentForChild(Style(), child_style) == ItemPosition::kLastBaseline;
        ...
        algorithm_
            .emplace_back(nullptr, child.Style(), flex_base_content_size, min_max_sizes_in_main_axis_direction, min_max_sizes_in_cross_axis_direction,
                main_axis_border_padding, cross_axis_border_padding, physical_child_margins, scrollbars, baseline_writing_mode, baseline_group,
    ...
    if (phase == Phase::kLayout && is_alignment_behavior_change_possible && algorithm_.all_items_.size() > 0 && !all_items_match_container_alignment) {
        if (algorithm_.IsColumnFlow()) {
            if (all_items_have_non_auto_cross_sizes) {
                UseCounter::Count(node_.GetDocument(), WebFeature::kFlexboxAlignSingleLineDifference);
            }
...
LayoutUnit NGFlexLayoutAlgorithm::AdjustMainSizeForAspectRatioCrossAxisMinAndMax(
    const NGBlockNode& child, LayoutUnit main_axis_size, const MinMaxSizes& cross_min_max, const NGBoxStrut& border_padding_in_child_writing_mode)
{
    DCHECK(child.HasAspectRatio());
    auto transferred_min_max_func = MainAxisIsInlineAxis(child) ? ComputeTransferredMinMaxInlineSizes : ComputeTransferredMinMaxBlockSizes;
    auto min_max
        = transferred_min_max_func(child.GetAspectRatio(), cross_min_max, border_padding_in_child_writing_mode, child.Style().BoxSizingForAspectRatio());
...
const NGLayoutResult* NGFlexLayoutAlgorithm::RelayoutIgnoringChildScrollbarChanges()
{
    DCHECK(!ignore_child_scrollbar_changes_);
    DCHECK(!layout_info_for_devtools_);
    NGLayoutAlgorithmParams params(Node(), container_builder_.InitialFragmentGeometry(), ConstraintSpace(), BreakToken(), /* early_break */ nullptr);
    NGFlexLayoutAlgorithm algorithm(params);
    algorithm.ignore_child_scrollbar_changes_ = true;
    return algorithm.Layout();
}

const NGLayoutResult* NGFlexLayoutAlgorithm::RelayoutAndBreakEarlierForFlex(const NGLayoutResult* previous_result)
{
    DCHECK(previous_result->GetEarlyBreak());
    NGLayoutAlgorithmParams params(
        Node(), container_builder_.InitialFragmentGeometry(), ConstraintSpace(), BreakToken(), previous_result->GetEarlyBreak(), &column_early_breaks_);
    NGFlexLayoutAlgorithm algorithm_with_break(params);
    algorithm_with_break.ignore_child_scrollbar_changes_ = ignore_child_scrollbar_changes_;
...
const NGLayoutResult* NGFlexLayoutAlgorithm::LayoutInternal()
{
    ...
    absl::optional<PaintLayerScrollableArea::FreezeScrollbarsScope> freeze_scrollbars;
    ...
    if (IsResumingLayout(BreakToken())) {
        const NGFlexBreakTokenData* flex_data = To<NGFlexBreakTokenData>(BreakToken()->TokenData());
    ...
    total_block_size_ = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), total_intrinsic_block_size_, container_builder_.InlineSize());

    if (!IsResumingLayout(BreakToken())) {
        ApplyFinalAlignmentAndReversals(&flex_line_outputs);
        NGLayoutResult::EStatus status = GiveItemsFinalPositionAndSize(&flex_line_outputs, &row_break_between_outputs);
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        ...
        NGLayoutResult::EStatus status = GiveItemsFinalPositionAndSizeForFragmentation(&flex_line_outputs, &row_break_between_outputs, &broke_before_row);
        if (status != NGLayoutResult::kSuccess)
            return container_builder_.Abort(status);

        intrinsic_block_size_ = ClampIntrinsicBlockSize(
            ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), intrinsic_block_size_ + BorderScrollbarPadding().block_end);

        block_size = ComputeBlockSizeForFragment(
            ConstraintSpace(), Style(), BorderPadding(), previously_consumed_block_size + intrinsic_block_size_, container_builder_.InlineSize());
    } else {
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        NGBreakStatus break_status
            = FinishFragmentation(Node(), ConstraintSpace(), BorderPadding().block_end, FragmentainerSpaceLeft(ConstraintSpace()), &container_builder_);
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        container_builder_.SetBreakTokenData(MakeGarbageCollected<NGFlexBreakTokenData>(
    ...
    HeapVector<NGFlexLine>* flex_line_outputs, HeapVector<Member<LayoutBox>>* oof_children, bool is_computing_multiline_column_intrinsic_size)
{
    ...
    if (is_column_) {
        const bool is_column_reverse = Style().ResolvedIsColumnReverseFlexDirection();
        main_axis_start_offset = is_column_reverse ? LayoutUnit() : BorderScrollbarPadding().block_start;
        main_axis_end_offset = is_column_reverse ? LayoutUnit() : BorderScrollbarPadding().block_end;
    ...
    FlexLine* line;
    while ((line = algorithm_.ComputeNextFlexLine(container_builder_.InlineSize()))) {
        ...
        for (wtf_size_t i = 0; i < line->line_items_.size(); ++i) {
            ...
            NGConstraintSpace child_space = BuildSpaceForLayout(flex_item.ng_input_node_, flex_item.FlexedBorderBoxSize(), flex_item.max_content_contribution_);
            auto minimum_top = DeferredShapingMinimumTopScope::CreateDelta(Node(), cross_axis_offset);
            ...
            if (DoesItemStretch(flex_item.ng_input_node_) && flex_item.layout_result_) {
                ...
                NGBoxStrut border = ComputeBorders(child_space, flex_item.ng_input_node_);
                NGBoxStrut padding = ComputePadding(child_space, flex_item.ng_input_node_.Style());
                ...
                DCHECK((child_space.CacheSlot() == NGCacheSlot::kLayout) || !flex_item.layout_result_)
                ...
                flex_item.cross_axis_size_ = is_horizontal_flow_ ? flex_item.layout_result_->PhysicalFragment().Size().height
        ...
        line->ComputeLineItemsPosition(main_axis_start_offset, main_axis_end_offset, cross_axis_offset);
...
void NGFlexLayoutAlgorithm::CalculateTotalIntrinsicBlockSize(bool use_empty_line_block_size)
{
    ...
    total_intrinsic_block_size_ = ClampIntrinsicBlockSize(
        ConstraintSpace(), Node(), BreakToken(), BorderScrollbarPadding(), total_intrinsic_block_size_ + BorderScrollbarPadding().block_end);
}

void NGFlexLayoutAlgorithm::ApplyFinalAlignmentAndReversals(HeapVector<NGFlexLine>* flex_line_outputs)
{
    auto& line_contexts = algorithm_.FlexLines();
    const LayoutUnit cross_axis_start_edge = line_contexts.empty() ? LayoutUnit() : line_contexts[0].cross_axis_offset_;

    LayoutUnit final_content_main_size = (container_builder_.InlineSize() - BorderScrollbarPadding().InlineSum()).ClampNegativeToZero();
    LayoutUnit final_content_cross_size = (total_block_size_ - BorderScrollbarPadding().BlockSum()).ClampNegativeToZero();
    if (is_column_)
    ...
    if (Style().FlexWrap() == EFlexWrap::kWrapReverse) {
        ...
        algorithm_.FlipForWrapReverse(cross_axis_start_edge, final_content_cross_size, flex_line_outputs);
        flex_line_outputs->Reverse();
    }

    if (Style().ResolvedIsColumnReverseFlexDirection()) {
        algorithm_.LayoutColumnReverse(final_content_main_size, BorderScrollbarPadding().block_start);
    }
    if (Style().ResolvedIsColumnReverseFlexDirection() || Style().ResolvedIsRowReverseFlexDirection()) {
    ...
    HeapVector<NGFlexLine>* flex_line_outputs, Vector<EBreakBetween>* row_break_between_outputs)
{
    ...
    if (is_column_) {
        final_content_cross_size = (container_builder_.InlineSize() - BorderScrollbarPadding().InlineSum()).ClampNegativeToZero();
    } else {
        final_content_cross_size = (total_block_size_ - BorderScrollbarPadding().BlockSum()).ClampNegativeToZero();
    }

    bool should_propagate_row_break_values = ConstraintSpace().ShouldPropagateChildBreakValues();
    ...
    for (wtf_size_t flex_line_idx = 0; flex_line_idx < flex_line_outputs->size(); ++flex_line_idx) {
        ...
        for (wtf_size_t flex_item_idx = 0; flex_item_idx < line_output.line_items.size(); ++flex_item_idx) {
            ...
            if (DoesItemStretch(flex_item.ng_input_node)) {
                NGConstraintSpace child_space = BuildSpaceForLayout(flex_item.ng_input_node, flex_item.main_axis_final_size,
                    /* override_inline_size */ absl::nullopt, line_output.line_cross_size);
                auto minimum_top = DeferredShapingMinimumTopScope::CreateDelta(Node(), offset.block_offset);
            ...
            const auto& physical_fragment = To<NGPhysicalBoxFragment>(layout_result->PhysicalFragment());

            NGBoxFragment fragment(ConstraintSpace().GetWritingDirection(), physical_fragment);
            ...
            if (PropagateFlexItemInfo(item, flex_line_idx, offset, physical_fragment.Size()) == NGLayoutResult::kNeedsRelayoutWithNoChildScrollbarChanges) {
    ...
    if (Node().IsButton()) {
        ...
        DCHECK(!InvolvedInBlockFragmentation(container_builder_));
        container_builder_.SetBaselines(BorderScrollbarPadding().BlockSum() + final_content_cross_size);
    }
...
NGLayoutResult::EStatus NGFlexLayoutAlgorithm::GiveItemsFinalPositionAndSizeForFragmentation(
    HeapVector<NGFlexLine>* flex_line_outputs, Vector<EBreakBetween>* row_break_between_outputs, bool* broke_before_row)
{
    DCHECK(InvolvedInBlockFragmentation(container_builder_));
    ...
    NGFlexItemIterator item_iterator(*flex_line_outputs, BreakToken(), is_column_);
    ...
    if (is_column_) {
        column_break_info = HeapVector<NGFlexColumnBreakInfo>(flex_line_outputs->size());
    }
    ...
    for (auto entry = item_iterator.NextItem(*broke_before_row); NGFlexItem* flex_item = entry.flex_item; entry = item_iterator.NextItem(*broke_before_row)) {
        ...
        if (item_break_token && item_break_token->IsBreakBefore()) {
            if (item_break_token->IsForcedBreak()) {
                ...
                line_output.item_offset_adjustment += previously_consumed_block_size - total_row_block_offset;
            } else {
                LayoutUnit total_item_block_offset = offset.block_offset + line_output.item_offset_adjustment;
                individual_item_adjustment = (previously_consumed_block_size - total_item_block_offset).ClampNegativeToZero();
        ...
        if (IsResumingLayout(item_break_token)) {
            ...
            LayoutUnit offset_adjustment = previously_consumed_block_size - line_output.item_offset_adjustment;
        ...
        if (UNLIKELY(early_break_)) {
            ...
            if (IsEarlyBreakTarget(*early_break_, container_builder_, flex_item->ng_input_node)) {
                ...
                ConsumeRemainingFragmentainerSpace(previously_consumed_block_size, &line_output);
        ...
        absl::optional<LayoutUnit> line_cross_size_for_stretch
            = DoesItemStretch(flex_item->ng_input_node) ? absl::optional<LayoutUnit>(line_output.line_cross_size) : absl::nullopt;
        ...
        if (line_cross_size_for_stretch && !is_column_ && item_break_token) {
            ...
            updated_cross_size_for_stretch
                -= previously_consumed_block_size - (original_offset.block_offset + line_output.item_offset_adjustment) - item_break_token->ConsumedBlockSize();
        ...
        NGConstraintSpace child_space = BuildSpaceForLayout(flex_item->ng_input_node, flex_item->main_axis_final_size,
            /* override_inline_size */ absl::nullopt, line_cross_size_for_stretch, offset.block_offset, min_block_size_should_encompass_intrinsic_size);
        auto minimum_top = DeferredShapingMinimumTopScope::CreateDelta(Node(), offset.block_offset);
        const NGLayoutResult* layout_result = flex_item->ng_input_node.Layout(child_space, item_break_token, early_break_in_child);
        ...
        if (!early_break_ && ConstraintSpace().HasBlockFragmentation()) {
            ...
            if (!is_column_) {
                ...
                if (flex_item_idx == 0 && (!item_break_token || (item_break_token->IsBreakBefore() && *broke_before_row))) {
                    ...
                    NGBreakStatus row_break_status = BreakBeforeRowIfNeeded(line_output, (*row_break_between_outputs)[flex_line_idx], flex_line_idx,
                        flex_item->ng_input_node, row_container_separation, is_first_for_row);
            ...
            break_status = BreakBeforeChildIfNeeded(ConstraintSpace(), flex_item->ng_input_node, *layout_result,
                ConstraintSpace().FragmentainerOffset() + offset.block_offset, has_container_separation, &container_builder_, !is_column_,
        ...
        if (break_status == NGBreakStatus::kBrokeBefore) {
            ConsumeRemainingFragmentainerSpace(previously_consumed_block_size, &line_output, current_column_break_info);
            // For column flex containers, continue to the next column. For rows,
            // continue until we've processed all items in the current row.
            has_inflow_child_break_inside_line[flex_line_idx] = true;
            if (is_column_) {
        ...
        bool is_at_block_end = !physical_fragment.BreakToken() || physical_fragment.BreakToken()->IsAtBlockEnd();
        ...
        if (is_column_) {
            flex_item->total_remaining_block_size -= fragment.BlockSize();
            if (flex_item->total_remaining_block_size < LayoutUnit() && !physical_fragment.BreakToken()) {
            ...
            LayoutUnit line_block_end = line_output.LineCrossEnd() - previously_consumed_block_size;
            if (line_block_end <= fragmentainer_space && line_block_end >= LayoutUnit() && previously_consumed_block_size != LayoutUnit::Max()) {
        ...
        container_builder_.AddResult(*layout_result, offset,
            ...
            /* inline_container */ nullptr, current_column_break_info ? &current_column_break_info->break_after : nullptr);
        baseline_accumulator.AccumulateItem(fragment, offset.block_offset, is_first_line, is_last_line);
        if (last_item_in_line) {
            if (!has_inflow_child_break_inside_line[flex_line_idx])
            ...
            if (!physical_fragment.BreakToken() || line_output.has_seen_all_children) {
    ...
    if (needs_earlier_break_in_column || status == NGLayoutResult::kNeedsEarlierBreak)
    ...
    FlexItem* flex_item, wtf_size_t flex_line_idx, LogicalOffset offset, PhysicalSize fragment_size)
{
    ...
    if (UNLIKELY(layout_info_for_devtools_)) {
        ...
        PhysicalSize flexbox_size = ToPhysicalSize(logical_flexbox_size, ConstraintSpace().GetWritingMode());
        item_rect.offset = offset.ConvertToPhysical(ConstraintSpace().GetWritingDirection(), flexbox_size, item_rect.size);
...
void NGFlexLayoutAlgorithm::AdjustButtonBaseline(LayoutUnit final_content_cross_size)
{
    // See LayoutButton::BaselinePosition()
    if (!Node().HasLineIfEmpty() && !Node().ShouldApplyLayoutContainment() && !container_builder_.FirstBaseline()) {
        ...
        container_builder_.SetBaselines(BorderScrollbarPadding().block_start + final_content_cross_size);
        return;
    }
    ...
    if (!LayoutButton::ShouldCountWrongBaseline(*Node().GetLayoutBox(), Style(), parent ? parent->Style() : nullptr))
        return;

    // The button should have at most one child.
    const NGContainerFragmentBuilder::ChildrenVector& children = container_builder_.Children();
    if (children.size() < 1) {
        const LayoutBlock* layout_block = To<LayoutBlock>(Node().GetLayoutBox());
        absl::optional<LayoutUnit> baseline = layout_block->BaselineForEmptyLine(layout_block->IsHorizontalWritingMode() ? kHorizontalLine : kVerticalLine);
        if (container_builder_.FirstBaseline() != baseline) {
            UseCounter::Count(Node().GetDocument(), WebFeature::kWrongBaselineOfEmptyLineButton);
        }
        return;
    }
    ...
    NGBoxFragment fragment(space.GetWritingDirection(), To<NGPhysicalBoxFragment>(*child.fragment));
    absl::optional<LayoutUnit> child_baseline
        = space.BaselineAlgorithmType() == NGBaselineAlgorithmType::kDefault ? fragment.FirstBaseline() : fragment.LastBaseline();
    if (child_baseline)
        child_baseline = *child_baseline + child.offset.block_offset;
    if (container_builder_.FirstBaseline() != child_baseline) {
        UseCounter::Count(Node().GetDocument(), WebFeature::kWrongBaselineOfMultiLineButton);
        String text = Node().GetDOMNode()->textContent();
        if (ContainsNonWhitespace(Node().GetLayoutBox())) {
            UseCounter::Count(Node().GetDocument(), WebFeature::kWrongBaselineOfMultiLineButtonWithNonSpace);
        }
    }
}

MinMaxSizesResult NGFlexLayoutAlgorithm::ComputeItemContributions(const NGConstraintSpace& space, const FlexItem& item) const
{
    const NGBlockNode& child = item.ng_input_node_;
    const ComputedStyle& child_style = child.Style();
    MinMaxSizesResult item_contributions = child.ComputeMinMaxSizes(ConstraintSpace().GetWritingMode(), MinMaxSizesType::kContent, space);

    // This calculates the "preferred size" part of 9.9.3:
    // "... outer preferred size (its width/height as appropriate) if that is not
    // auto ..."
    const Length& preferred_main_axis_length = is_horizontal_flow_ ? child_style.Width() : child_style.Height();
    ...
    if (!is_preferred_main_axis_length_auto) {
        NGFragmentGeometry child_initial_geometry = CalculateInitialFragmentGeometry(space, child,
            /* break_token */ nullptr,
            /* is_intrinsic */ false);
        const LayoutUnit preferred_size
            = MainAxisIsInlineAxis(child) ? child_initial_geometry.border_box_size.inline_size : child_initial_geometry.border_box_size.block_size;
    ...
    if (algorithm_.IsMultiline()) {
        ...
        if (child_style.ResolvedFlexGrow(parent_style) == 0.f) {
            item_contributions.sizes.min_size = std::min(item_contributions.sizes.min_size, flex_base_size_border_box);
        }
        if (child_style.ResolvedFlexShrink(parent_style) == 0.f) {
            item_contributions.sizes.min_size = std::max(item_contributions.sizes.min_size, flex_base_size_border_box);
        }
    }
...
public:
    ...
    LayoutUnit ApplyLargestFlexFractionToItem(const ComputedStyle& child_style, LayoutUnit flex_base_content_size)
    {
        ...
        if (chosen_flex_fraction_ > 0.f) {
            ...
            DCHECK_GT(sum_factors_less_than_one_adjustment_, 0.f) << "If all the flex grow factors were == 0, then "
                                                                     "chosen_flex_fraction_ can't be positive";
            return LayoutUnit(numerator_ * (child_style.ResolvedFlexGrow(parent_style_) / (sum_factors_less_than_one_adjustment_ * denominator_float_part_)));
        }
        if (chosen_flex_fraction_ < 0.f) {
            return LayoutUnit(flex_base_content_size.MulDiv(numerator_, *denominator_pixels_part_)
                * (sum_factors_less_than_one_adjustment_ * child_style.ResolvedFlexShrink(parent_style_) / denominator_float_part_));
        }
        ...
        DCHECK(!denominator_pixels_part_.has_value()) << "We're not supposed to get here if the chosen flex "
    ...
    void UpdateLargestFlexFraction(const FlexItem& item, LayoutUnit item_contribution)
    {
        ...
        const float flex_shrink_factor = child_style.ResolvedFlexShrink(parent_style_);
        ...
        LayoutUnit difference = item_contribution - item.flex_base_content_size_ - item.main_axis_border_padding_;
        if (difference > LayoutUnit()) {
            ...
            if (desired_flex_fraction > chosen_flex_fraction_) {
                ...
                denominator_float_part_ = flex_grow_factor >= 1.f ? flex_grow_factor : 1 / flex_grow_factor;
            ...
            const float scaled_flex_shrink_factor = item.flex_base_content_size_ * flex_shrink_factor;
            if (scaled_flex_shrink_factor == 0.f) {
                // If the desired flex fraction is -infinity, it should never become the
                // chosen flex fraction.
                return;
            }
            const float desired_flex_fraction = difference / scaled_flex_shrink_factor;
            if (desired_flex_fraction > chosen_flex_fraction_) {
                chosen_flex_fraction_ = desired_flex_fraction;
                numerator_ = difference;
    ...
    void SetSumFactorsLessThanOneAdjustment()
    {
        if (chosen_flex_fraction_ > 0.f && sum_flex_grow_factors_ < 1.f) {
            DCHECK_GT(sum_flex_grow_factors_, 0.f) << "If all the flex grow factors were == 0, then "
...
MinMaxSizesResult NGFlexLayoutAlgorithm::ComputeMinMaxSizeOfMultilineColumnContainer()
{
    ...
    largest_inline_size_contributions.min_size = largest_min_content_contribution_;
    if (!flex_line_outputs.empty()) {
        largest_inline_size_contributions.max_size
            = flex_line_outputs.back().line_cross_size + flex_line_outputs.back().cross_axis_offset - flex_line_outputs.front().cross_axis_offset;
    }

    DCHECK_GE(largest_inline_size_contributions.min_size, 0);
    DCHECK_LE(largest_inline_size_contributions.min_size, largest_inline_size_contributions.max_size);
...
MinMaxSizesResult NGFlexLayoutAlgorithm::ComputeMinMaxSizeOfRowContainer()
{
    ...
    for (const FlexItem& item : algorithm_.all_items_) {
        ...
        const MinMaxSizesResult min_max_content_contributions = ComputeItemContributions(space, item);
        ...
        if (algorithm_.IsMultiline()) {
            ...
            min_content_largest_fraction.UpdateLargestFlexFraction(item, min_max_content_contributions.sizes.min_size);
        }
        max_content_largest_fraction.UpdateLargestFlexFraction(item, min_max_content_contributions.sizes.max_size);
    }
    ...
    for (const FlexItem& item : algorithm_.all_items_) {
        const ComputedStyle& child_style = item.style_;
        const LayoutUnit flex_base_size_border_box = item.flex_base_content_size_ + item.main_axis_border_padding_;
        MinMaxSizes item_final_contribution { flex_base_size_border_box, flex_base_size_border_box };
        if (!algorithm_.IsMultiline()) {
            item_final_contribution.min_size += min_content_largest_fraction.ApplyLargestFlexFractionToItem(child_style, item.flex_base_content_size_);
        }
        item_final_contribution.max_size += max_content_largest_fraction.ApplyLargestFlexFractionToItem(child_style, item.flex_base_content_size_);

        item_final_contribution.Constrain(item.min_max_main_sizes_.max_size + item.main_axis_border_padding_);
...
MinMaxSizesResult NGFlexLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
...
LayoutUnit NGFlexLayoutAlgorithm::FragmentainerSpaceAvailable(LayoutUnit block_offset) const
{
    return (FragmentainerSpaceLeft(ConstraintSpace()) - block_offset).ClampNegativeToZero();
}

void NGFlexLayoutAlgorithm::ConsumeRemainingFragmentainerSpace(
    LayoutUnit previously_consumed_block_size, NGFlexLine* flex_line, const NGFlexColumnBreakInfo* column_break_info)
{
    if (To<NGBlockBreakToken>(container_builder_.LastChildBreakToken())->IsForcedBreak()) {
        ...
        flex_line->item_offset_adjustment -= intrinsic_block_size + previously_consumed_block_size;
    }
...
NGBreakStatus NGFlexLayoutAlgorithm::BreakBeforeRowIfNeeded(
    const NGFlexLine& row, EBreakBetween row_break_between, wtf_size_t row_index, NGLayoutInputNode child, bool has_container_separation, bool is_first_for_row)
{
    DCHECK(!is_column_);
    DCHECK(InvolvedInBlockFragmentation(container_builder_));

    LayoutUnit fragmentainer_block_offset = ConstraintSpace().FragmentainerOffset() + row.cross_axis_offset;
    if (BreakToken())
        fragmentainer_block_offset -= BreakToken()->ConsumedBlockSize();

    if (has_container_separation) {
        if (IsForcedBreakValue(ConstraintSpace(), row_break_between)) {
            BreakBeforeChild(ConstraintSpace(), child, /* layout_result */ nullptr, fragmentainer_block_offset, kBreakAppealPerfect,
                /* is_forced_break */ true, &container_builder_, row.line_cross_size);
            return NGBreakStatus::kBrokeBefore;
        }
    }

    bool breakable_at_start_of_container = IsBreakableAtStartOfResumedContainer(ConstraintSpace(), container_builder_, is_first_for_row);
    NGBreakAppeal appeal_before = CalculateBreakAppealBefore(
        ConstraintSpace(), NGLayoutResult::EStatus::kSuccess, row_break_between, has_container_separation, breakable_at_start_of_container);
    ...
    if (!AttemptSoftBreak(
            ConstraintSpace(), child, /* layout_result */ nullptr, fragmentainer_block_offset, appeal_before, &container_builder_, row.line_cross_size))
        return NGBreakStatus::kNeedsEarlierBreak;

    return NGBreakStatus::kBrokeBefore;
}

bool NGFlexLayoutAlgorithm::MovePastRowBreakPoint(NGBreakAppeal appeal_before, LayoutUnit fragmentainer_block_offset, LayoutUnit row_block_size,
    wtf_size_t row_index, bool has_container_separation, bool breakable_at_start_of_container)
{
    ...
    LayoutUnit space_left = FragmentainerCapacity(ConstraintSpace()) - fragmentainer_block_offset;
...
#if DCHECK_IS_ON()
        bool refuse_break_before = space_left >= FragmentainerCapacity(ConstraintSpace());
    ...
    if ((has_container_separation || breakable_at_start_of_container)
        && (!container_builder_.HasEarlyBreak() || appeal_before >= container_builder_.EarlyBreak().BreakAppeal())) {
        container_builder_.SetEarlyBreak(MakeGarbageCollected<NGEarlyBreak>(row_index, appeal_before));
    }
...
void NGFlexLayoutAlgorithm::AddColumnEarlyBreak(NGEarlyBreak* breakpoint, wtf_size_t index)
{
    DCHECK(is_column_);
...
void NGFlexLayoutAlgorithm::AdjustOffsetForNextLine(HeapVector<NGFlexLine>* flex_line_outputs, wtf_size_t flex_line_idx, LayoutUnit item_expansion) const
{
...
const NGLayoutResult* NGFlexLayoutAlgorithm::RelayoutWithNewRowSizes()
{
    ...
    NGLayoutAlgorithmParams params(
        Node(), container_builder_.InitialFragmentGeometry(), ConstraintSpace(), BreakToken(), early_break_, additional_early_breaks_);
    NGFlexLayoutAlgorithm algorithm_with_row_cross_sizes(params, layout_info_for_devtools_.get(), &row_cross_size_updates_);
...
bool NGFlexLayoutAlgorithm::MinBlockSizeShouldEncompassIntrinsicSize(const NGFlexItem& item) const
{
    ...
    DCHECK_EQ(ConstraintSpace().GetWritingDirection().GetWritingMode(), item_style.GetWritingMode());

    if (is_column_) {
        bool can_shrink = item_style.ResolvedFlexShrink(Style()) != 0.f && !Style().LogicalHeight().IsAutoOrContentOrIntrinsic();
...
#if DCHECK_IS_ON()
void NGFlexLayoutAlgorithm::CheckFlexLines(HeapVector<NGFlexLine>& flex_line_outputs) const
{
