...
public:
    static const NGPhysicalBoxFragment* Create(NGBoxFragmentBuilder* builder, WritingMode block_or_line_writing_mode);
    ...
    static const NGPhysicalBoxFragment* CloneWithPostLayoutFragments(
        const NGPhysicalBoxFragment& other, const absl::optional<PhysicalRect> updated_layout_overflow = absl::nullopt);

    using PassKey = base::PassKey<NGPhysicalBoxFragment>;
    NGPhysicalBoxFragment(PassKey, NGBoxFragmentBuilder* builder, bool has_layout_overflow, const PhysicalRect& layout_overflow, bool has_borders,
        const NGPhysicalBoxStrut& borders, bool has_padding, const NGPhysicalBoxStrut& padding, const absl::optional<PhysicalRect>& inflow_bounds,
    ...
    NGPhysicalBoxFragment(PassKey, const NGPhysicalBoxFragment& other, bool has_layout_overflow, const PhysicalRect& layout_overflow);
    ...
    private:
        ...
        MutableChildrenForOutOfFlow(const NGLink* buffer, wtf_size_t num_children)
            : buffer_(const_cast<NGLink*>(buffer))
    ...
    {
        return ConstHasRareData() ? ComputeRareDataAddress()->table_collapsed_borders.get() : nullptr;
    }

    const NGTableFragmentData::CollapsedBordersGeometry* TableCollapsedBordersGeometry() const
    {
    ...
    {
        DCHECK(IsTableNGSection());
        return ConstHasRareData() ? &ComputeRareDataAddress()->table_section_row_offsets : nullptr;
    }
    ...
    PhysicalRect OverflowClipRect(
        const PhysicalOffset& location, const NGBlockBreakToken* incoming_break_token, OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const;
    ...
    {
        return static_cast<NGInkOverflow::Type>(bit_field_.get<InkOverflowTypeValue>());
    }
    bool IsInkOverflowComputed() const
    {
        return InkOverflowType() != NGInkOverflow::Type::kNotSet && InkOverflowType() != NGInkOverflow::Type::kInvalidated;
    }
    ...
    void AddSelfOutlineRects(const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows, Vector<PhysicalRect>* outline_rects,
    ...
    void AddOutlineRects(const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows, Vector<PhysicalRect>* outline_rects) const;
    ...
    {
        return IsFirstForNode() && !BreakToken();
    }
...
#if DCHECK_IS_ON()
    void CheckSameForSimplifiedLayout(const NGPhysicalBoxFragment&, bool check_same_block_size, bool check_no_fragmentation) const;
#endif
    ...
    public:
        ...
        base::span<NGLink> Children() const
        {
            ...
            return base::make_span(fragment_.children_, fragment_.const_num_children_);
        }
...
private:
    ...
    using InkOverflowTypeValue = HasLayoutOverflowFlag::DefineNextValue<uint8_t, NGInkOverflow::kTypeBits>;
    ...
    using ConstHasRareDataFlag = HasInflowBoundsFlag::DefineNextValue<bool, 1, WTF::BitFieldValueConstness::kConst>;
    ...
    {
        return bit_field_.get<HasBordersFlag>();
    }
    bool HasPadding() const
    {
        return bit_field_.get<HasPaddingFlag>();
    }
    bool HasInflowBounds() const
    {
        return bit_field_.get<HasInflowBoundsFlag>();
    }

    static size_t AdditionalByteSize(wtf_size_t num_fragment_items, wtf_size_t num_children, bool has_layout_overflow, bool has_borders, bool has_padding,
        bool has_inflow_bounds, bool has_rare_data);
    ...
    public:
        ...
        std::unique_ptr<NGTableFragmentData::CollapsedBordersGeometry> table_collapsed_borders_geometry;
    ...
    {
        DCHECK(HasItems() || HasLayoutOverflow() || HasBorders() || HasPadding() || HasInflowBounds() || ConstHasRareData());
        const NGLink* children_end = children_ + const_num_children_;
        return reinterpret_cast<const NGFragmentItems*>(base::bits::AlignUp(reinterpret_cast<const uint8_t*>(children_end), alignof(NGFragmentItems)));
    }

    const PhysicalRect* ComputeLayoutOverflowAddress() const
    {
        DCHECK(HasLayoutOverflow() || HasBorders() || HasPadding() || HasInflowBounds() || ConstHasRareData());
        ...
        return reinterpret_cast<const PhysicalRect*>(base::bits::AlignUp(uint8_t_items, alignof(PhysicalRect)));
    }

    const NGPhysicalBoxStrut* ComputeBordersAddress() const
    {
        DCHECK(HasBorders() || HasPadding() || HasInflowBounds() || ConstHasRareData());
        const PhysicalRect* address = ComputeLayoutOverflowAddress();
        const uint8_t* unaligned_border_address
            = HasLayoutOverflow() ? reinterpret_cast<const uint8_t*>(address + 1) : reinterpret_cast<const uint8_t*>(address);
        return reinterpret_cast<const NGPhysicalBoxStrut*>(base::bits::AlignUp(unaligned_border_address, alignof(NGPhysicalBoxStrut)));
    }
    ...
    const NGPhysicalBoxStrut* ComputePaddingAddress() const
    {
        ...
        const uint8_t* unaligned_address = HasBorders() ? reinterpret_cast<const uint8_t*>(address + 1) : reinterpret_cast<const uint8_t*>(address);
        return reinterpret_cast<const NGPhysicalBoxStrut*>(base::bits::AlignUp(unaligned_address, alignof(NGPhysicalBoxStrut)));
    }

    const PhysicalRect* ComputeInflowBoundsAddress() const
    {
        DCHECK(HasInflowBounds() || ConstHasRareData());
        NGPhysicalBoxStrut* address = const_cast<NGPhysicalBoxStrut*>(ComputePaddingAddress());
        const uint8_t* unaligned_address = HasPadding() ? reinterpret_cast<const uint8_t*>(address + 1) : reinterpret_cast<const uint8_t*>(address);
        return reinterpret_cast<const PhysicalRect*>(base::bits::AlignUp(unaligned_address, alignof(PhysicalRect)));
    }

    const RareData* ComputeRareDataAddress() const
    {
        DCHECK(ConstHasRareData());
        PhysicalRect* address = const_cast<PhysicalRect*>(ComputeInflowBoundsAddress());
        const uint8_t* unaligned_address = HasInflowBounds() ? reinterpret_cast<const uint8_t*>(address + 1) : reinterpret_cast<const uint8_t*>(address);
        return reinterpret_cast<const RareData*>(base::bits::AlignUp(unaligned_address, alignof(RareData)));
    }
    ...
    void AddOutlineRects(const PhysicalOffset& additional_offset, NGOutlineType include_block_overflows, bool inline_container_relative,
        Vector<PhysicalRect>* outline_rects) const;
    void AddOutlineRectsForInlineBox(
        PhysicalOffset additional_offset, NGOutlineType include_block_overflows, bool inline_container_relative, Vector<PhysicalRect>* outline_rects) const;
    ...
    PositionWithAffinity PositionForPointRespectingEditingBoundaries(const NGPhysicalBoxFragment& child, PhysicalOffset point_in_child) const;
...
template <> struct DowncastTraits<NGPhysicalBoxFragment> {
