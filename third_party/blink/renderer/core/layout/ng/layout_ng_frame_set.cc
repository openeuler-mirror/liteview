...
LayoutNGFrameSet::LayoutNGFrameSet(Element* element)
    : LayoutNGBlock(element)
{
...
bool LayoutNGFrameSet::IsChildAllowed(LayoutObject* child, const ComputedStyle&) const
{
    NOT_DESTROYED();
    return child->IsFrame() || child->IsLayoutNGFrameSet();
}

void LayoutNGFrameSet::AddChild(LayoutObject* new_child, LayoutObject* before_child)
{
...
CursorDirective LayoutNGFrameSet::GetCursor(const PhysicalOffset& point, ui::Cursor& cursor) const
{
    NOT_DESTROYED();
