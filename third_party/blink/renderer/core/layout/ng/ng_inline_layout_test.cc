...
public:
    ...
    {
        NGConstraintSpaceBuilder builder(block_flow->Style()->GetWritingMode(), block_flow->Style()->GetWritingDirection(),
            /* is_new_fc */ false);
...
TEST_F(NGInlineLayoutTest, BlockWithSingleTextNode)
{
    ...
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, node, /* break_token */ nullptr);
    const NGLayoutResult* result = NGBlockLayoutAlgorithm({ node, fragment_geometry, constraint_space }).Layout();
    EXPECT_TRUE(result);
...
TEST_F(NGInlineLayoutTest, BlockWithTextAndAtomicInline)
{
    ...
    NGFragmentGeometry fragment_geometry = CalculateInitialFragmentGeometry(constraint_space, node,
        /* break_token */ nullptr);
    const NGLayoutResult* result = NGBlockLayoutAlgorithm({ node, fragment_geometry, constraint_space }).Layout();
    EXPECT_TRUE(result);
