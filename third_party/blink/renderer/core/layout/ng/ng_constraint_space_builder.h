...
public:
    ...
    NGConstraintSpaceBuilder(
        const NGConstraintSpace& parent_space, WritingDirectionMode writing_direction, bool is_new_fc, bool adjust_inline_size_if_needed = true)
        : NGConstraintSpaceBuilder(parent_space.GetWritingMode(), writing_direction, is_new_fc,
    ...
    // unless we force it.
    NGConstraintSpaceBuilder(WritingMode parent_writing_mode, WritingDirectionMode writing_direction, bool is_new_fc,
    ...
    void SetTableCellBorders(const NGBoxStrut& table_cell_borders, WritingDirectionMode cell_writing_direction, WritingDirectionMode table_writing_direction)
    {
#if DCHECK_IS_ON()
    ...
    void SetTableCellAlignmentBaseline(const absl::optional<LayoutUnit>& table_cell_alignment_baseline)
    {
#if DCHECK_IS_ON()
    ...
    void SetIsTableCellWithEffectiveRowspan(bool has_effective_rowspan)
    {
        ...
        if (has_effective_rowspan) {
            space_.EnsureRareData()->SetIsTableCellWithEffectiveRowspan(has_effective_rowspan);
        }
    }
    ...
    void SetTableRowData(const NGTableConstraintSpaceData* table_data, wtf_size_t row_index)
    {
#if DCHECK_IS_ON()
    ...
    void SetTargetStretchBlockSizes(NGConstraintSpace::MathTargetStretchBlockSizes target_stretch_block_sizes)
    {
...
public:
    NGMinMaxConstraintSpaceBuilder(const NGConstraintSpace& parent_space, const ComputedStyle& parent_style, const NGLayoutInputNode& child, bool is_new_fc)
