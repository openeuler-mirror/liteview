...
enum NGFragmentationType { kFragmentNone, kFragmentPage, kFragmentColumn, kFragmentRegion };
...
public:
    ...
    enum NGPercentageStorage { kSameAsAvailable, kZero, kIndefinite, kRareDataPercentage };
    ...
    {
        switch (static_cast<NGPercentageStorage>(bitfields_.percentage_inline_storage)) {
        default:
            NOTREACHED();
            [[fallthrough]];
    ...
    {
        switch (static_cast<NGPercentageStorage>(bitfields_.percentage_block_storage)) {
        default:
            NOTREACHED();
            [[fallthrough]];
        case kSameAsAvailable:
    ...
    {
        switch (static_cast<NGPercentageStorage>(bitfields_.replaced_percentage_block_storage)) {
        case kSameAsAvailable:
            return available_size_.block_size;
        case kZero:
            return LayoutUnit();
    ...
    {
        return HasRareData() ? rare_data_->OverrideMinMaxBlockSizes() : absl::nullopt;
    }
    ...
    {
        return HasRareData() ? rare_data_->TargetStretchBlockSizes() : absl::nullopt;
    }
    ...
    {
        return HasRareData() ? rare_data_->TableCellAlignmentBaseline() : absl::nullopt;
    }
    ...
    {
        return HasRareData() ? rare_data_->IsTableCellWithEffectiveRowspan() : false;
    }
    ...
    {
        return bitfields_.is_table_cell;
    }
    ...
    {
        return static_cast<NGBaselineAlgorithmType>(bitfields_.baseline_algorithm_type);
    }
    ...
    {
        return HasRareData() ? static_cast<NGFragmentationType>(rare_data_->block_direction_fragmentation_type) : kFragmentNone;
    }
    ...
    {
        return HasRareData() ? rare_data_->OptimisticBfcBlockOffset() : absl::nullopt;
    }
    ...
    LayoutUnit ExpectedBfcBlockOffset() const
    {
        ...
        return ForcedBfcBlockOffset().value_or(OptimisticBfcBlockOffset().value_or(BfcOffset().block_offset));
    }
    ...
    void ReplaceTableRowData(const NGTableConstraintSpaceData& table_data, const wtf_size_t row_index)
    {
        DCHECK(HasRareData());
...
private:
    ...
    public:
        ...
        explicit RareData(const NGBfcOffset bfc_offset)
            ...
            , block_direction_fragmentation_type(static_cast<unsigned>(kFragmentNone))
        ...
        {
            if (data_union_type != other.data_union_type || is_line_clamp_context != other.is_line_clamp_context
                || is_pushed_by_floats != other.is_pushed_by_floats || is_restricted_block_size_table_cell != other.is_restricted_block_size_table_cell
                ...
                || min_break_appeal != other.min_break_appeal || propagate_child_break_values != other.propagate_child_break_values
                || should_repeat != other.should_repeat || is_inside_repeatable_content != other.is_inside_repeatable_content)
                return false;
        ...
        {
            if (page_name || fragmentainer_block_size != kIndefiniteSize || fragmentainer_offset || is_line_clamp_context || is_pushed_by_floats
                || is_restricted_block_size_table_cell || hide_table_cell_if_empty || block_direction_fragmentation_type != kFragmentNone
                || is_block_fragmentation_forced_off || requires_content_before_breaking || is_inside_balanced_columns || should_ignore_forced_breaks
                || is_in_column_bfc || min_break_appeal != kBreakAppealLastResort || propagate_child_break_values || is_at_fragmentainer_start || should_repeat
                || is_inside_repeatable_content)
                return false;
        ...
        NGMarginStrut MarginStrut() const
        {
            return GetDataUnionType() == DataUnionType::kBlockData ? block_data_.margin_strut : NGMarginStrut();
        }
        ...
        absl::optional<LayoutUnit> OptimisticBfcBlockOffset() const
        {
            return GetDataUnionType() == DataUnionType::kBlockData ? block_data_.optimistic_bfc_block_offset : absl::nullopt;
        }
        ...
        absl::optional<LayoutUnit> ForcedBfcBlockOffset() const
        {
            return GetDataUnionType() == DataUnionType::kBlockData ? block_data_.forced_bfc_block_offset : absl::nullopt;
        }
        ...
        absl::optional<int> LinesUntilClamp() const
        {
            return GetDataUnionType() == DataUnionType::kBlockData ? block_data_.lines_until_clamp : absl::nullopt;
        }
        ...
        NGBoxStrut TableCellBorders() const
        {
            return GetDataUnionType() == DataUnionType::kTableCellData ? table_cell_data_.table_cell_borders : NGBoxStrut();
        }
        ...
        absl::optional<LayoutUnit> TableCellAlignmentBaseline() const
        {
            return GetDataUnionType() == DataUnionType::kTableCellData ? table_cell_data_.table_cell_alignment_baseline : absl::nullopt;
        }
        ...
        bool IsTableCellHiddenForPaint() const
        {
            return GetDataUnionType() == DataUnionType::kTableCellData && table_cell_data_.is_hidden_for_paint;
        }
        ...
        bool IsTableCellWithEffectiveRowspan() const
        {
            return GetDataUnionType() == DataUnionType::kTableCellData && table_cell_data_.has_effective_rowspan;
        }
        ...
        void SetTableRowData(scoped_refptr<const NGTableConstraintSpaceData> table_data, wtf_size_t row_index)
        {
            EnsureTableRowData()->table_data = std::move(table_data);
            EnsureTableRowData()->row_index = row_index;
        }

        void SetTableSectionData(scoped_refptr<const NGTableConstraintSpaceData> table_data, wtf_size_t section_index)
        {
            EnsureTableSectionData()->table_data = std::move(table_data);
            EnsureTableSectionData()->section_index = section_index;
        }

        void ReplaceTableRowData(const NGTableConstraintSpaceData& table_data, wtf_size_t row_index)
        {
        ...
        absl::optional<MathTargetStretchBlockSizes> TargetStretchBlockSizes() const
        {
            return GetDataUnionType() == DataUnionType::kStretchData ? stretch_data_.target_stretch_block_sizes : absl::nullopt;
        }
        ...
        const NGGridLayoutTrackCollection* SubgriddedColumns() const
        {
            return GetDataUnionType() == DataUnionType::kSubgridData ? subgrid_data_.layout_data.columns.get() : nullptr;
        }

        void SetSubgriddedColumns(std::unique_ptr<NGGridLayoutTrackCollection> columns)
        {
        ...
        const NGGridLayoutTrackCollection* SubgriddedRows() const
        {
            return GetDataUnionType() == DataUnionType::kSubgridData ? subgrid_data_.layout_data.rows.get() : nullptr;
        }
    ...
    private:
        ...
        struct TableCellData {
            bool MaySkipLayout(const TableCellData& other) const
            {
                ...
                return table_cell_borders == other.table_cell_borders && table_cell_column_index == other.table_cell_column_index
                    && is_hidden_for_paint == other.is_hidden_for_paint && has_collapsed_borders == other.has_collapsed_borders
                    && has_effective_rowspan == other.has_effective_rowspan;
            }

            bool IsInitialForMaySkipLayout() const
            {
                return table_cell_borders == NGBoxStrut() && table_cell_column_index == kNotFound && !is_hidden_for_paint && !has_collapsed_borders
        ...
        struct SubgridData {
            ...
            {
                const bool other_has_same_column_geometry = (layout_data.columns && other.layout_data.columns)
                    ? *layout_data.Columns() == *other.layout_data.Columns()
                    : !layout_data.columns && !other.layout_data.columns;

                const bool other_has_same_row_geometry = (layout_data.rows && other.layout_data.rows) ? *layout_data.Rows() == *other.layout_data.Rows()
    ...
    public:
        ...
        explicit Bitfields(WritingDirectionMode writing_direction)
            ...
            , baseline_algorithm_type(static_cast<unsigned>(NGBaselineAlgorithmType::kDefault))
            , cache_slot(static_cast<unsigned>(NGCacheSlot::kLayout))
            , inline_auto_behavior(static_cast<unsigned>(NGAutoBehavior::kFitContent))
            , block_auto_behavior(static_cast<unsigned>(NGAutoBehavior::kFitContent))
        ...
        bool MaySkipLayout(const Bitfields& other) const
        {
            return adjoining_object_types == other.adjoining_object_types && writing_mode == other.writing_mode && direction == other.direction
                && is_table_cell == other.is_table_cell && is_anonymous == other.is_anonymous && is_new_formatting_context == other.is_new_formatting_context
...
inline std::ostream& operator<<(std::ostream& stream, const NGConstraintSpace& value)
{
