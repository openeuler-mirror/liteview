...
const NGBlockBreakToken* CreateBreakToken(NGLayoutInputNode node, const NGBreakTokenVector* child_break_tokens = nullptr, bool has_seen_all_children = false)
{
    NGBoxFragmentBuilder builder(node, &node.Style(), NGConstraintSpace(), WritingDirectionMode(WritingMode::kHorizontalTb, TextDirection::kLtr));
...
TEST_F(NGBlockChildIteratorTest, NullFirstChild)
{
    ...
    ASSERT_EQ(NGBlockChildIterator::Entry(nullptr, nullptr), iterator.NextChild());
}
...
TEST_F(NGBlockChildIteratorTest, BreakTokens)
{
    ...
    const NGBlockBreakToken* parent_token = CreateBreakToken(container, &child_break_tokens);

    NGBlockChildIterator iterator(node1, parent_token);
    ASSERT_EQ(NGBlockChildIterator::Entry(node1, child_token1), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node2, nullptr), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node3, nullptr), iterator.NextChild());
    ...
    ASSERT_EQ(NGBlockChildIterator::Entry(node1, child_token1), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node2, child_token2), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node3, nullptr), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node4, nullptr), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(nullptr, nullptr), iterator.NextChild());
    ...
    iterator = NGBlockChildIterator(node1, parent_token);
    ASSERT_EQ(NGBlockChildIterator::Entry(node2, child_token2), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node3, child_token3), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(node4, nullptr), iterator.NextChild());
    ASSERT_EQ(NGBlockChildIterator::Entry(nullptr, nullptr), iterator.NextChild());
...
TEST_F(NGBlockChildIteratorTest, SeenAllChildren)
{
    ...
    NGBreakTokenVector child_break_tokens;
    child_break_tokens.push_back(child_token1);
    const NGBlockBreakToken* parent_token = CreateBreakToken(container, &child_break_tokens, /* has_seen_all_children*/ true);
