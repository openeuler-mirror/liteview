...
bool IsInlineContainerForNode(const NGBlockNode& node, const LayoutObject* inline_container)
{
...
NGLogicalAnchorQuery::SetOptions AnchorQuerySetOptions(
    const NGPhysicalFragment& fragment, const NGLayoutInputNode& container, bool is_fragmentation_context_root)
{
...
void NGContainerFragmentBuilder::ReplaceChild(wtf_size_t index, const NGPhysicalFragment& new_child, const LogicalOffset offset)
{
...
void NGContainerFragmentBuilder::PropagateChildAnchors(const NGPhysicalFragment& child, const LogicalOffset& child_offset)
{
    ...
    if (child.IsBox()) {
        ...
        if (const AtomicString& anchor_name = child.Style().AnchorName(); !anchor_name.IsNull()) {
            DCHECK(RuntimeEnabledFeatures::CSSAnchorPositioningEnabled());
            options = AnchorQuerySetOptions(child, node_, IsBlockFragmentationContextRoot());
            EnsureAnchorQuery().Set(anchor_name, child, LogicalRect { child_offset, child.Size().ConvertToLogical(GetWritingMode()) }, *options);
        }
    }
    ...
    if (const NGPhysicalAnchorQuery* anchor_query = child.AnchorQuery()) {
        ...
        EnsureAnchorQuery().SetFromPhysical(*anchor_query, converter, child_offset, *options);
    }
}

// Propagate data in |child| to this fragment. The |child| will then be added as
// a child fragment or a child fragment item.
void NGContainerFragmentBuilder::PropagateChildData(const NGPhysicalFragment& child, LogicalOffset child_offset, LogicalOffset relative_offset,
    const NGInlineContainer<LogicalOffset>* inline_container, absl::optional<LayoutUnit> adjustment_for_oof_propagation)
{
    ...
    if (adjustment_for_oof_propagation && child.NeedsOOFPositionedInfoPropagation()) {
    ...
    if (!has_descendant_that_depends_on_percentage_block_size_) {
        ...
        if (child.IsCSSBox() && child_style.GetPosition() == EPosition::kRelative) {
            if (IsHorizontalWritingMode(Style().GetWritingMode())) {
                if (child_style.Top().IsPercentOrCalc() || child_style.Bottom().IsPercentOrCalc())
                    has_descendant_that_depends_on_percentage_block_size_ = true;
            } else {
                if (child_style.Left().IsPercentOrCalc() || child_style.Right().IsPercentOrCalc())
                    has_descendant_that_depends_on_percentage_block_size_ = true;
            }
        }
    }
    ...
    if (!has_floating_descendants_for_paint_) {
        if (child.IsFloating() || child.IsLegacyLayoutRoot() || (child.HasFloatingDescendantsForPaint() && !child.IsPaintedAtomically()))
    ...
    if (has_block_fragmentation_ && !child.IsFragmentainerBox() && !break_token_ && !should_add_break_tokens_manually_) {
        ...
        case NGPhysicalFragment::kFragmentLineBox:
            const auto* inline_break_token = To<NGInlineBreakToken>(child_break_token);
            if (inline_break_token) {
                ...
                if (UNLIKELY(inline_break_token->SubBreakTokenInParallelFlow())) {
                    ...
                    const auto* sub_break_token = inline_break_token->SubBreakTokenInParallelFlow();
...
void NGContainerFragmentBuilder::AddChildInternal(const NGPhysicalFragment* child, const LogicalOffset& child_offset)
{
...
void NGContainerFragmentBuilder::AddOutOfFlowChildCandidate(
    NGBlockNode child, const LogicalOffset& child_offset, NGLogicalStaticPosition::InlineEdge inline_edge, NGLogicalStaticPosition::BlockEdge block_edge)
{
    DCHECK(child);
    oof_positioned_candidates_.emplace_back(child, NGLogicalStaticPosition { child_offset, inline_edge, block_edge }, NGInlineContainer<LogicalOffset>());
}

void NGContainerFragmentBuilder::AddOutOfFlowChildCandidate(const NGLogicalOutOfFlowPositionedNode& candidate)
{
    oof_positioned_candidates_.emplace_back(candidate);
}

void NGContainerFragmentBuilder::AddOutOfFlowInlineChildCandidate(
    NGBlockNode child, const LogicalOffset& child_offset, TextDirection inline_container_direction)
{
    ...
    AddOutOfFlowChildCandidate(child, child_offset,
        IsLtr(inline_container_direction) ? NGLogicalStaticPosition::kInlineStart : NGLogicalStaticPosition::kInlineEnd, NGLogicalStaticPosition::kBlockStart);
}

void NGContainerFragmentBuilder::AddOutOfFlowFragmentainerDescendant(const NGLogicalOOFNodeForFragmentation& descendant)
{
    oof_positioned_fragmentainer_descendants_.push_back(descendant);
}

void NGContainerFragmentBuilder::AddOutOfFlowFragmentainerDescendant(const NGLogicalOutOfFlowPositionedNode& descendant)
{
...
void NGContainerFragmentBuilder::AddOutOfFlowDescendant(const NGLogicalOutOfFlowPositionedNode& descendant)
{
    oof_positioned_descendants_.push_back(descendant);
}

void NGContainerFragmentBuilder::SwapOutOfFlowPositionedCandidates(HeapVector<NGLogicalOutOfFlowPositionedNode>* candidates)
{
    DCHECK(candidates->empty());
    std::swap(oof_positioned_candidates_, *candidates);
}

void NGContainerFragmentBuilder::AddMulticolWithPendingOOFs(const NGBlockNode& multicol, NGMulticolWithPendingOOFs<LogicalOffset>* multicol_info)
{
...
void NGContainerFragmentBuilder::SwapMulticolsWithPendingOOFs(MulticolCollection* multicols_with_pending_oofs)
{
    DCHECK(multicols_with_pending_oofs->empty());
    std::swap(multicols_with_pending_oofs_, *multicols_with_pending_oofs);
}

void NGContainerFragmentBuilder::SwapOutOfFlowFragmentainerDescendants(HeapVector<NGLogicalOOFNodeForFragmentation>* descendants)
{
...
void NGContainerFragmentBuilder::TransferOutOfFlowCandidates(
    NGContainerFragmentBuilder* destination_builder, LogicalOffset additional_offset, const NGMulticolWithPendingOOFs<LogicalOffset>* multicol)
{
    for (auto& candidate : oof_positioned_candidates_) {
        ...
        if (multicol && multicol->fixedpos_containing_block.Fragment() && node.Style().GetPosition() == EPosition::kFixed) {
            ...
            destination_builder->AddOutOfFlowFragmentainerDescendant({ node, candidate.static_position, multicol->fixedpos_inline_container,
                multicol->fixedpos_containing_block, multicol->fixedpos_containing_block, multicol->fixedpos_inline_container });
            continue;
        }
...
void NGContainerFragmentBuilder::MoveOutOfFlowDescendantCandidatesToDescendants()
{
    ...
    for (auto& candidate : oof_positioned_descendants_) {
        ...
        if (!candidate.inline_container.container && IsInlineContainerForNode(candidate.Node(), layout_object_)) {
            candidate.inline_container = NGInlineContainer<LogicalOffset>(To<LayoutInline>(layout_object_),
                /* relative_offset */ LogicalOffset());
        }

        // Ensure that the inline_container is a continuation root.
        if (candidate.inline_container.container) {
            candidate.inline_container.container = To<LayoutInline>(candidate.inline_container.container->ContinuationRoot());
        }
    }
}

void NGContainerFragmentBuilder::PropagateOOFPositionedInfo(const NGPhysicalFragment& fragment, LogicalOffset offset, LogicalOffset relative_offset,
    LogicalOffset offset_adjustment, const NGInlineContainer<LogicalOffset>* inline_container, LayoutUnit containing_block_adjustment,
    const NGContainingBlock<LogicalOffset>* fixedpos_containing_block, const NGInlineContainer<LogicalOffset>* fixedpos_inline_container,
    LogicalOffset additional_fixedpos_offset)
{
    ...
    for (const auto& descendant : fragment.OutOfFlowPositionedDescendants()) {
        ...
        NGLogicalStaticPosition static_position = descendant.StaticPosition().ConvertToLogical(converter);
        ...
        if ((fixedpos_containing_block || additional_fixedpos_offset != LogicalOffset()) && node.Style().GetPosition() == EPosition::kFixed) {
            ...
            if (fixedpos_containing_block && (fixedpos_containing_block->Fragment() || node_.IsPaginatedRoot())) {
                ...
                AddOutOfFlowFragmentainerDescendant({ node, static_position, new_fixedpos_inline_container, *fixedpos_containing_block,
        ...
        DCHECK(!base::Contains(oof_positioned_candidates_, node, &NGLogicalOutOfFlowPositionedNode::Node));
        oof_positioned_candidates_.emplace_back(node, static_position, new_inline_container);
    }
    ...
    const NGPhysicalBoxFragment* box_fragment = DynamicTo<NGPhysicalBoxFragment>(&fragment);
    ...
    if (!oof_data->multicols_with_pending_oofs.empty()) {
        ...
        for (auto& multicol : multicols_with_pending_oofs) {
            ...
            NGInlineContainer<LogicalOffset> new_fixedpos_inline_container(multicol_info->fixedpos_inline_container.container, fixedpos_inline_relative_offset);
            const NGPhysicalFragment* fixedpos_containing_block_fragment = multicol_info->fixedpos_containing_block.Fragment();
            ...
            if (fixedpos_containing_block_fragment) {
                ...
                fixedpos_containing_block_rel_offset
                    = converter.ToLogical(multicol_info->fixedpos_containing_block.RelativeOffset(), fixedpos_containing_block_fragment->Size());
            ...
            AddMulticolWithPendingOOFs(NGBlockNode(multicol.key),
                MakeGarbageCollected<NGMulticolWithPendingOOFs<LogicalOffset>>(multicol_offset,
                    NGContainingBlock<LogicalOffset>(fixedpos_containing_block_offset, fixedpos_containing_block_rel_offset, fixedpos_containing_block_fragment,
                        is_inside_column_spanner, multicol_info->fixedpos_containing_block.RequiresContentBeforeBreaking()),
                    new_fixedpos_inline_container));
        }
    }

    PropagateOOFFragmentainerDescendants(fragment, offset, relative_offset, containing_block_adjustment, fixedpos_containing_block);
}

void NGContainerFragmentBuilder::PropagateOOFFragmentainerDescendants(const NGPhysicalFragment& fragment, LogicalOffset offset, LogicalOffset relative_offset,
    LayoutUnit containing_block_adjustment, const NGContainingBlock<LogicalOffset>* fixedpos_containing_block,
    HeapVector<NGLogicalOOFNodeForFragmentation>* out_list)
{
    ...
    auto& out_of_flow_fragmentainer_descendants = oof_data->oof_positioned_fragmentainer_descendants;
    wtf_size_t next_idx;
    for (wtf_size_t idx = 0; idx < out_of_flow_fragmentainer_descendants.size(); idx = next_idx) {
        next_idx = idx + 1;
        ...
        NGInlineContainer<LogicalOffset> new_inline_container(descendant.inline_container.container, inline_relative_offset);
        ...
        NGInlineContainer<LogicalOffset> new_fixedpos_inline_container(descendant.fixedpos_inline_container.container, fixedpos_inline_relative_offset);
        ...
        NGLogicalOOFNodeForFragmentation oof_node(descendant.Node(), static_position, new_inline_container,
            NGContainingBlock<LogicalOffset>(containing_block_offset, containing_block_rel_offset, containing_block_fragment, container_inside_column_spanner,
                descendant.containing_block.RequiresContentBeforeBreaking()),
            NGContainingBlock<LogicalOffset>(fixedpos_containing_block_offset, fixedpos_containing_block_rel_offset, fixedpos_containing_block_fragment,
                fixedpos_container_inside_column_spanner, descendant.fixedpos_containing_block.RequiresContentBeforeBreaking()),
            new_fixedpos_inline_container);
...
void NGContainerFragmentBuilder::AdjustFixedposContainerInfo(const NGPhysicalFragment* box_fragment, LogicalOffset relative_offset,
    NGInlineContainer<LogicalOffset>* fixedpos_inline_container, const NGPhysicalFragment** fixedpos_containing_block_fragment,
        ...
        if (current_inline_container && current_inline_container->container && current_inline_container->container->CanContainFixedPositionObjects()) {
            ...
            *fixedpos_containing_block_fragment = box_fragment;
            ...
            if (!fixedpos_inline_container->container && box_fragment->GetLayoutObject()->IsLayoutInline()) {
                *fixedpos_inline_container = NGInlineContainer<LogicalOffset>(To<LayoutInline>(box_fragment->GetLayoutObject()), relative_offset);
            } else {
                *fixedpos_containing_block_fragment = box_fragment;
            }
        } else if (fixedpos_inline_container->container) {
            // Ensure that the inline_container is a continuation root.
            fixedpos_inline_container->container = To<LayoutInline>(fixedpos_inline_container->container->ContinuationRoot());
...
const NGLayoutResult* NGContainerFragmentBuilder::Abort(NGLayoutResult::EStatus status)
{
    return MakeGarbageCollected<NGLayoutResult>(NGLayoutResult::NGContainerFragmentBuilderPassKey(), status, this);
}
...
String NGContainerFragmentBuilder::ToString() const
{
    ...
    for (auto& child : children_) {
        builder.Append(child.fragment->DumpFragmentTree(NGPhysicalFragment::DumpAll & ~NGPhysicalFragment::DumpHeaderText));
    }
