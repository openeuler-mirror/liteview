...
protected:
    ...
    void SetTRBL(LayoutUnit top, LayoutUnit right, LayoutUnit bottom, LayoutUnit left)
    {
        style_->SetTop(top == kAuto ? Length::Auto() : Length::Fixed(top.ToInt()));
        style_->SetRight(right == kAuto ? Length::Auto() : Length::Fixed(right.ToInt()));
        style_->SetBottom(bottom == kAuto ? Length::Auto() : Length::Fixed(bottom.ToInt()));
        style_->SetLeft(left == kAuto ? Length::Auto() : Length::Fixed(left.ToInt()));
    }
...
TEST_F(NGRelativeUtilsTest, HorizontalTB)
{
    ...
    offset = ComputeRelativeOffset(*style_, { WritingMode::kHorizontalTb, TextDirection::kLtr }, container_size_);
    ...
    // kRtl
    offset = ComputeRelativeOffset(*style_, { WritingMode::kHorizontalTb, TextDirection::kRtl }, container_size_);
...
TEST_F(NGRelativeUtilsTest, VerticalRightLeft)
{
    ...
    // kLtr
    offset = ComputeRelativeOffset(*style_, { WritingMode::kVerticalRl, TextDirection::kLtr }, container_size_);
    EXPECT_EQ(offset.inline_offset, kTop);
    EXPECT_EQ(offset.block_offset, kRight);

    // kRtl
    offset = ComputeRelativeOffset(*style_, { WritingMode::kVerticalRl, TextDirection::kRtl }, container_size_);
...
TEST_F(NGRelativeUtilsTest, VerticalLeftRight)
{
    ...
    // kLtr
    offset = ComputeRelativeOffset(*style_, { WritingMode::kVerticalLr, TextDirection::kLtr }, container_size_);
    EXPECT_EQ(offset.inline_offset, kTop);
    EXPECT_EQ(offset.block_offset, kLeft);

    // kRtl
    offset = ComputeRelativeOffset(*style_, { WritingMode::kVerticalLr, TextDirection::kRtl }, container_size_);
    EXPECT_EQ(offset.inline_offset, kBottom);
    EXPECT_EQ(offset.block_offset, kLeft);
