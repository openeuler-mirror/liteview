...
class NGOutOfFlowLayoutPartTest : public NGBaseLayoutAlgorithmTest, private ScopedLayoutNGBlockFragmentationForTest {
protected:
...
TEST_F(NGOutOfFlowLayoutPartTest, FixedInsideAbs)
{
    ...
    EXPECT_TRUE(result);
    EXPECT_EQ(result->PhysicalFragment().OutOfFlowPositionedDescendants().size(), 2u);
...
TEST_F(NGOutOfFlowLayoutPartTest, PositionedFragmentationWithNegativeTopPropertyAndNewEmptyColumn)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
TEST_F(NGOutOfFlowLayoutPartTest, PositionedFragmentationInFlowWithAddedColumns)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
// spanner.
TEST_F(NGOutOfFlowLayoutPartTest, PositionedFragmentationWithNewColumnsAndSpanners)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
// spanner.
TEST_F(NGOutOfFlowLayoutPartTest, PositionedFragmentationWithNewColumnsAfterSpanner)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
// fragments beyond the last fragmentainer in a context in the presence of a
// spanner.
TEST_F(NGOutOfFlowLayoutPartTest, AbsposFragWithSpannerAndNewColumnsAutoHeight)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
TEST_F(NGOutOfFlowLayoutPartTest, AbsposFragmentationPctResolutionWithOverflow)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
TEST_F(NGOutOfFlowLayoutPartTest, PositionedFragmentationWithBottomPropertyAndHeightAuto)
{
    SetBodyInnerHTML(
        R"HTML(
      <style>
...
{
    for (const NGPhysicalBoxFragment& fragmentation_root : multicol->PhysicalFragments()) {
...
TEST_F(NGOutOfFlowLayoutPartTest, FragmentainerBreakTokens)
{
    ...
    const NGPhysicalBoxFragment* multicol_fragment = multicol->GetPhysicalFragment(0);
...
TEST_F(NGOutOfFlowLayoutPartTest, RelayoutNestedMulticolWithOOF)
{
    ...
    auto GetInnerFragmentainer = [&inner]() -> const NGPhysicalBoxFragment* {
        ...
        if (inner->GetPhysicalFragment(0)->Children().size() != 1u)
            return nullptr;
        return To<NGPhysicalBoxFragment>(inner->GetPhysicalFragment(0)->Children()[0].fragment.Get());
    };
