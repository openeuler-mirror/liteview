...
NGGridLayoutAlgorithm::NGGridLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
    : NGLayoutAlgorithm(params)
{
    ...
    if (grid_available_size_.block_size == kIndefiniteSize && Node().ShouldApplyBlockSizeContainment()) {
        ...
        const LayoutUnit block_size
            = ComputeBlockSizeForFragment(ConstraintSpace(), Style(), BorderPadding(), *contain_intrinsic_block_size_, container_builder_.InlineSize());

        grid_available_size_.block_size = grid_min_available_size_.block_size = grid_max_available_size_.block_size
            = (block_size - BorderScrollbarPadding().BlockSum()).ClampNegativeToZero();
    }
    ...
    if (grid_available_size_.inline_size == kIndefiniteSize) {
        ...
        const MinMaxSizes sizes = ComputeMinMaxInlineSizes(
            ConstraintSpace(), Node(), container_builder_.BorderPadding(), [&border_scrollbar_padding](MinMaxSizesType) -> MinMaxSizesResult {
                ...
                // "intrinsic" size.
                return MinMaxSizesResult({ border_scrollbar_padding, border_scrollbar_padding },
    ...
    if (grid_available_size_.block_size == kIndefiniteSize) {
        ...
        const MinMaxSizes sizes = ComputeMinMaxBlockSizes(ConstraintSpace(), Style(), container_builder_.BorderPadding());
...
NGGridProperties InitializeGridProperties(const GridItems& grid_items, const WritingMode container_writing_mode)
{
    NGGridProperties grid_properties;

    for (const auto& grid_item : grid_items) {
        grid_properties.has_baseline_column |= grid_item.IsBaselineSpecifiedForDirection(kForColumns);
        grid_properties.has_baseline_row |= grid_item.IsBaselineSpecifiedForDirection(kForRows);
        grid_properties.has_orthogonal_item |= !grid_item.is_parallel_with_root_grid;
    }
    return grid_properties;
}

void CacheGridTrackSpanProperties(const NGGridLayoutTrackCollection& track_collection, GridItems* grid_items, NGGridProperties* grid_properties = nullptr)
{
    DCHECK(grid_items);
    ...
    if (grid_properties) {
        auto& track_properties = (track_direction == kForColumns) ? grid_properties->column_properties : grid_properties->row_properties;
        track_properties.Reset();

        const wtf_size_t range_count = track_collection.RangeCount();
        auto CacheGridProperty = [&](const TrackSpanProperties::PropertyId property) {
    ...
    auto CacheTrackSpanProperty = [&](GridItemData& grid_item, const wtf_size_t range_index, const TrackSpanProperties::PropertyId property) {
    ...
    for (auto& grid_item : *grid_items) {
        ...
        if (range_indices.begin == range_indices.end) {
            CacheTrackSpanProperty(grid_item, range_indices.begin, TrackSpanProperties::kHasFlexibleTrack);
            CacheTrackSpanProperty(grid_item, range_indices.begin, TrackSpanProperties::kHasIntrinsicTrack);
            CacheTrackSpanProperty(grid_item, range_indices.begin, TrackSpanProperties::kHasAutoMinimumTrack);
            CacheTrackSpanProperty(grid_item, range_indices.begin, TrackSpanProperties::kHasFixedMinimumTrack);
            CacheTrackSpanProperty(grid_item, range_indices.begin, TrackSpanProperties::kHasFixedMaximumTrack);
        } else {
    ...
    auto CompareGridItemsByStartLine = [track_direction](const Member<GridItemData>& lhs, const Member<GridItemData>& rhs) -> bool {
        return lhs->StartLine(track_direction) < rhs->StartLine(track_direction);
    };
    std::sort(grid_items_spanning_multiple_ranges.item_data.begin(), grid_items_spanning_multiple_ranges.item_data.end(), CompareGridItemsByStartLine);

    auto CacheTrackSpanPropertyForAllGridItems = [&](const TrackSpanProperties::PropertyId property) {
        ...
        for (auto& grid_item : grid_items_spanning_multiple_ranges) {
            ...
            while (current_range_index < range_count
                && (track_collection.RangeEndLine(current_range_index) <= grid_item.StartLine(track_direction)
                    || !track_collection.RangeHasTrackSpanProperty(current_range_index, property))) {
                ++current_range_index;
            }
    ...
    CacheTrackSpanPropertyForAllGridItems(TrackSpanProperties::kHasIntrinsicTrack);
    CacheTrackSpanPropertyForAllGridItems(TrackSpanProperties::kHasAutoMinimumTrack);
    CacheTrackSpanPropertyForAllGridItems(TrackSpanProperties::kHasFixedMinimumTrack);
    CacheTrackSpanPropertyForAllGridItems(TrackSpanProperties::kHasFixedMaximumTrack);
}
...
const NGLayoutResult* NGGridLayoutAlgorithm::LayoutInternal()
{
    ...
    auto grid_sizing_tree = IsResumingLayout(BreakToken()) ? BuildGridSizingTree() : BuildGridSizingTree(&oof_children);
    ...
    if (IsResumingLayout(BreakToken())) {
        ...
        ComputeGridGeometry(node.CachedPlacementData(), &grid_items, &layout_data, &intrinsic_block_size);
    }
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        ...
        if (IsResumingLayout(BreakToken())) {
            ...
            PlaceGridItems(grid_items, layout_data, &row_break_between, &grid_items_placement_data);
        }

        PlaceGridItemsForFragmentation(
            grid_items, row_break_between, &layout_data, &grid_items_placement_data, &row_offset_adjustments, &intrinsic_block_size, &consumed_grid_block_size);
    } else {
    ...
    const auto block_size = ComputeBlockSizeForFragment(constraint_space, Style(), border_padding, intrinsic_block_size, container_builder_.InlineSize());
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        auto status = FinishFragmentation(node, constraint_space, border_padding.block_end, FragmentainerSpaceLeft(constraint_space), &container_builder_);
    ...
    if (ConstraintSpace().HasBlockFragmentation()) {
        container_builder_.SetBreakTokenData(MakeGarbageCollected<NGGridBreakTokenData>(container_builder_.GetBreakTokenData(), layout_data,
            intrinsic_block_size, consumed_grid_block_size, grid_items_placement_data, row_offset_adjustments, row_break_between, oof_children));
    }

    // Copy grid layout data for use in computed style and devtools.
    container_builder_.TransferGridLayoutData(std::make_unique<NGGridLayoutData>(layout_data));

    NGOutOfFlowLayoutPart(node, constraint_space, &container_builder_).Run();
    return container_builder_.ToBoxFragment();
}

MinMaxSizesResult NGGridLayoutAlgorithm::ComputeMinMaxSizes(const MinMaxSizesFloatInput&)
{
    const auto& node = Node();
    ...
    if (!node.ShouldApplyInlineSizeContainment()) {
        grid_items = node.ConstructGridItems(placement_data, /* oof_children */ nullptr, &has_nested_subgrid);

        placement_data.column_start_offset = node.CachedPlacementData().column_start_offset;
        placement_data.row_start_offset = node.CachedPlacementData().row_start_offset;
    }

    NGGridLayoutData layout_data(LayoutTrackCollection(placement_data, kForColumns, &grid_items), LayoutTrackCollection(placement_data, kForRows, &grid_items));
    ...
    auto grid_properties = InitializeGridProperties(grid_items, Style().GetWritingMode());

    CacheGridTrackSpanProperties(*layout_data.Columns(), &grid_items, &grid_properties);
    CacheGridTrackSpanProperties(*layout_data.Rows(), &grid_items, &grid_properties);

    bool depends_on_block_constraints = false;
    auto ComputeTotalColumnSize = [&](const SizingConstraint sizing_constraint) -> LayoutUnit {
        ...
        ComputeUsedTrackSizes(layout_data, grid_properties, sizing_constraint, &grid_items, layout_data.Columns(), &needs_additional_pass);
        ...
        if (needs_additional_pass || HasBlockSizeDependentGridItem(grid_items)) {
            ...
            if (layout_data.Columns()->IsForSizing() && layout_data.Rows()->IsForSizing()) {
                ComputeUsedTrackSizes(layout_data, grid_properties, sizing_constraint, &grid_items, layout_data.Rows(), &needs_additional_pass);

                if (needs_additional_pass) {
                    InitializeTrackSizes(grid_properties, layout_data.Columns());
                    ComputeUsedTrackSizes(layout_data, grid_properties, sizing_constraint, &grid_items, layout_data.Columns());
                }
            }
        }
...
wtf_size_t NGGridLayoutAlgorithm::BuildGridSizingSubtree(NGGridSizingTree* sizing_tree, HeapVector<Member<LayoutBox>>* oof_children,
    const NGGridSizingData* parent_sizing_data, const NGGridLineResolver* parent_line_resolver, const GridItemData* subgrid_data_in_parent) const
{
    const auto& node = Node();
    const auto& style = node.Style();

    bool has_nested_subgrid;
    auto& sizing_data = sizing_tree->CreateSizingData(node, parent_sizing_data, subgrid_data_in_parent);
    {
        // Initialize this grid's placement data.
        auto placement_data = parent_line_resolver ? NGGridPlacementData(style, *parent_line_resolver) : NGGridPlacementData(style);

        if (subgrid_data_in_parent) {
            wtf_size_t column_span_size_in_parent = subgrid_data_in_parent->SpanSize(kForColumns);
            wtf_size_t row_span_size_in_parent = subgrid_data_in_parent->SpanSize(kForRows);
        ...
        if (!parent_sizing_data) {
            placement_data.column_auto_repetitions = ComputeAutomaticRepetitions(kForColumns);
            placement_data.row_auto_repetitions = ComputeAutomaticRepetitions(kForRows);
        }

        // Construct grid items that are not subgridded.
        sizing_data.grid_items = node.ConstructGridItems(placement_data, oof_children, &has_nested_subgrid);
    }
    ...
    if (has_nested_subgrid) {
        ...
        for (const auto& grid_item : sizing_data.grid_items) {
            ...
            const auto space = CreateConstraintSpace(grid_item, NGGridLayoutData(), { kIndefiniteSize, kIndefiniteSize }, NGCacheSlot::kLayout,
                /* opt_fixed_block_size */ absl::nullopt);
            const auto fragment_geometry = CalculateInitialFragmentGeometry(space, grid_item.node, /* break_token */ nullptr,
                /* is_intrinsic */ true);
            NGGridLayoutAlgorithm algorithm({ grid_item.node, fragment_geometry, space });

            sizing_data.subtree_size += algorithm.BuildGridSizingSubtree(sizing_tree, /* oof_children */ nullptr, &sizing_data, line_resolver, &grid_item);
        }
        node.AppendSubgriddedItems(&sizing_data.grid_items);
    }

    auto FinalizeTrackBuilderCollection = [&](NGGridBlockTrackCollection* track_collection) {
        const auto track_direction = track_collection->Direction();
        ...
        for (auto& grid_item : sizing_data.grid_items) {
            ...
            track_collection->EnsureTrackCoverage(item_span.StartLine(), item_span.IntegerSpan(), &range_indices.begin, &range_indices.end);
        }
        track_collection->FinalizeRanges();
    };

    sizing_data.column_builder_collection = std::make_unique<NGGridBlockTrackCollection>(style, node.CachedPlacementData(), kForColumns);
    FinalizeTrackBuilderCollection(sizing_data.column_builder_collection.get());

    sizing_data.row_builder_collection = std::make_unique<NGGridBlockTrackCollection>(style, node.CachedPlacementData(), kForRows);
    FinalizeTrackBuilderCollection(sizing_data.row_builder_collection.get());

    return sizing_data.subtree_size;
}

NGGridSizingTree NGGridLayoutAlgorithm::BuildGridSizingTree(HeapVector<Member<LayoutBox>>* oof_children) const
{
    ...
    if (ConstraintSpace().SubgriddedColumns() || ConstraintSpace().SubgriddedRows()) {
        const auto& node = Node();
        auto& sizing_data = sizing_tree.CreateSizingData(node, nullptr, nullptr);

        bool has_nested_subgrid;
        sizing_data.grid_items = node.ConstructGridItems(node.CachedPlacementData(), oof_children, &has_nested_subgrid);
        if (has_nested_subgrid)
            node.AppendSubgriddedItems(&sizing_data.grid_items);
        return sizing_tree;
    }
...
LayoutUnit NGGridLayoutAlgorithm::Baseline(
    const NGGridLayoutData& layout_data, const GridItemData& grid_item, const GridTrackSizingDirection track_direction) const
{
    ...
    if (track_direction == kForColumns) {
        return (grid_item.column_baseline_group == BaselineGroup::kMajor) ? layout_data.Columns()->MajorBaseline(grid_item.column_set_indices.begin)
                                                                          : layout_data.Columns()->MinorBaseline(grid_item.column_set_indices.end - 1);
    } else {
        return (grid_item.row_baseline_group == BaselineGroup::kMajor) ? layout_data.Rows()->MajorBaseline(grid_item.row_set_indices.begin)
                                                                       : layout_data.Rows()->MinorBaseline(grid_item.row_set_indices.end - 1);
    }
}
...
FirstSetGeometry ComputeFirstSetGeometry(const NGGridSizingTrackCollection& track_collection, const ComputedStyle& container_style, LayoutUnit available_size,
...
void NGGridLayoutAlgorithm::ComputeGridGeometry(
    const NGGridPlacementData& placement_data, GridItems* grid_items, NGGridLayoutData* layout_data, LayoutUnit* intrinsic_block_size)
{
    ...
    layout_data->columns = LayoutTrackCollection(placement_data, kForColumns, grid_items);
    ...
    if (const auto* subgridded_rows = constraint_space.SubgriddedRows()) {
        layout_data->rows = std::make_unique<NGGridLayoutTrackCollection>(
            *subgridded_rows, border_scrollbar_padding, ComputeMarginsForSelf(constraint_space, container_style));
    } else {
        row_builder_collection = std::make_unique<NGGridBlockTrackCollection>(container_style, placement_data, kForRows);
        BuildBlockTrackCollection(grid_items, row_builder_collection.get());

        const bool is_block_available_size_indefinite = grid_available_size_.block_size == kIndefiniteSize;

        layout_data->rows = std::make_unique<NGGridSizingTrackCollection>(*row_builder_collection, is_block_available_size_indefinite);
    }
    ...
    auto grid_properties = InitializeGridProperties(*grid_items, container_style.GetWritingMode());
    ...
    bool needs_additional_pass = false;
    ComputeUsedTrackSizes(*layout_data, grid_properties, SizingConstraint::kLayout, grid_items, layout_data->Columns(), &needs_additional_pass);
    ComputeUsedTrackSizes(*layout_data, grid_properties, SizingConstraint::kLayout, grid_items, layout_data->Rows(), &needs_additional_pass);
    ...
    if (contain_intrinsic_block_size_) {
        ...
        *intrinsic_block_size = layout_data->Rows()->ComputeSetSpanSize() + border_scrollbar_padding.BlockSum();
        ...
        if (grid_items->IsEmpty() && node.HasLineIfEmpty()) {
            *intrinsic_block_size = std::max(*intrinsic_block_size, border_scrollbar_padding.BlockSum() + node.EmptyLineBlockSize(BreakToken()));
        }

        *intrinsic_block_size = ClampIntrinsicBlockSize(constraint_space, node, BreakToken(), border_scrollbar_padding, *intrinsic_block_size);
    }

    if (layout_data->Rows()->IsForSizing() && grid_available_size_.block_size == kIndefiniteSize) {
        const auto block_size
            = ComputeBlockSizeForFragment(constraint_space, container_style, BorderPadding(), *intrinsic_block_size, container_builder_.InlineSize());
        ...
        needs_additional_pass
            |= (container_style.RowGap() && container_style.RowGap()->IsPercentOrCalc()) || grid_properties.IsDependentOnAvailableSize(kForRows);
        ...
        if (constraint_space.IsInitialBlockSizeIndefinite()) {
            needs_additional_pass |= ComputeBlockSizeForFragment(constraint_space, container_style, BorderPadding(),
        ...
        if (!needs_additional_pass && container_style.AlignContent() != ComputedStyleInitialValues::InitialAlignContent()) {
            auto& sizing_collection = To<NGGridSizingTrackCollection>(*layout_data->Rows());
            ...
            auto first_set_geometry
                = ComputeFirstSetGeometry(sizing_collection, container_style, grid_available_size_.block_size, border_scrollbar_padding.block_start);

            sizing_collection.CacheSetsGeometry(first_set_geometry.start_offset, first_set_geometry.gutter_size);
        }
    }
    ...
    if (needs_additional_pass) {
        ...
        InitializeTrackSizes(grid_properties, layout_data->Columns());
        ComputeUsedTrackSizes(*layout_data, grid_properties, SizingConstraint::kLayout, grid_items, layout_data->Columns());

        layout_data->rows = std::make_unique<NGGridSizingTrackCollection>(*row_builder_collection, /* is_available_size_indefinite */ false);
        CacheGridTrackSpanProperties(*layout_data->Rows(), grid_items, &grid_properties);

        InitializeTrackSizes(grid_properties, layout_data->Rows());
        ComputeUsedTrackSizes(*layout_data, grid_properties, SizingConstraint::kLayout, grid_items, layout_data->Rows());
    }

    // Calculate final alignment baselines for grid item layout.
    if (grid_properties.HasBaseline(kForColumns) && layout_data->Columns()->IsForSizing()) {
        CalculateAlignmentBaselines(*layout_data, SizingConstraint::kLayout, grid_items, To<NGGridSizingTrackCollection>(layout_data->Columns()));
    }

    if (grid_properties.HasBaseline(kForRows) && layout_data->Rows()->IsForSizing()) {
        CalculateAlignmentBaselines(*layout_data, SizingConstraint::kLayout, grid_items, To<NGGridSizingTrackCollection>(layout_data->Rows()));
    }
}

LayoutUnit NGGridLayoutAlgorithm::ComputeIntrinsicBlockSizeIgnoringChildren() const
{
    const auto& node = Node();
    ...
    layout_data.rows = LayoutTrackCollection(PlacementData(), kForRows, &grid_items);
    ...
    return layout_data.Rows()->ComputeSetSpanSize() + BorderScrollbarPadding().BlockSum();
}

namespace {

const NGLayoutResult* LayoutGridItemForMeasure(
    const GridItemData& grid_item, const NGConstraintSpace& constraint_space, const SizingConstraint sizing_constraint)
{
...
LayoutUnit NGGridLayoutAlgorithm::GetLogicalBaseline(const NGBoxFragment& baseline_fragment, const bool is_last_baseline) const
{
    const auto font_baseline = Style().GetFontBaseline();
    return is_last_baseline ? baseline_fragment.BlockSize() - baseline_fragment.LastBaselineOrSynthesize(font_baseline)
                            : baseline_fragment.FirstBaselineOrSynthesize(font_baseline);
}

LayoutUnit NGGridLayoutAlgorithm::GetSynthesizedLogicalBaseline(const LayoutUnit block_size, const bool is_flipped_lines, const bool is_last_baseline) const
{
    const auto font_baseline = Style().GetFontBaseline();
    const auto synthesized_baseline = NGBoxFragment::SynthesizedBaseline(font_baseline, is_flipped_lines, block_size);
    return is_last_baseline ? block_size - synthesized_baseline : synthesized_baseline;
}

LayoutUnit NGGridLayoutAlgorithm::ContributionSizeForGridItem(const NGGridLayoutData& layout_data, const SizingConstraint sizing_constraint,
    const GridTrackSizingDirection track_direction, const GridItemContributionType contribution_type, GridItemData* grid_item) const
{
    DCHECK(grid_item);
    ...
    const auto space = CreateConstraintSpaceForMeasure(*grid_item, layout_data, track_direction);
    ...
    auto CalculateBaselineShim = [&](const LayoutUnit baseline) -> void {
        const LayoutUnit track_baseline = Baseline(layout_data, *grid_item, track_direction);
        ...
        baseline_shim -= ComputeMarginsFor(space, item_style, grid_item->BaselineWritingDirection(track_direction)).block_start;
    };
    ...
    auto MinOrMaxContentSize = [&](bool is_min_size) -> LayoutUnit {
        ...
        const LayoutUnit content_size = is_min_size ? result.sizes.min_size : result.sizes.max_size;

        if (grid_item->IsBaselineAlignedForDirection(track_direction)) {
            CalculateBaselineShim(GetSynthesizedLogicalBaseline(content_size, grid_item->BaselineWritingDirection(track_direction).IsFlippedLines(),
                grid_item->IsLastBaselineSpecifiedForDirection(track_direction)));
        }
        return content_size + baseline_shim;
    };

    auto MinContentSize = [&]() -> LayoutUnit { return MinOrMaxContentSize(/* is_min_size */ true); };
    auto MaxContentSize = [&]() -> LayoutUnit { return MinOrMaxContentSize(/* is_min_size */ false); };
    ...
    auto BlockContributionSize = [&]() -> LayoutUnit {
        ...
        if (space.AvailableSize().inline_size == kIndefiniteSize) {
            ...
            const auto fallback_space = CreateConstraintSpaceForMeasure(*grid_item, layout_data, track_direction,
        ...
        NGBoxFragment baseline_fragment(grid_item->BaselineWritingDirection(track_direction), To<NGPhysicalBoxFragment>(result->PhysicalFragment()));

        if (grid_item->IsBaselineAlignedForDirection(track_direction)) {
            CalculateBaselineShim(GetLogicalBaseline(baseline_fragment, grid_item->IsLastBaselineSpecifiedForDirection(track_direction)));
        }

        return baseline_fragment.BlockSize() + baseline_shim;
    };

    const LayoutUnit margin_sum = is_for_columns ? margins.InlineSum() : margins.BlockSum();
    ...
    case GridItemContributionType::kForIntrinsicMinimums: {
        ...
        const Length& main_length = is_parallel_with_track_direction ? item_style.LogicalWidth() : item_style.LogicalHeight();
        const Length& min_length = is_parallel_with_track_direction ? item_style.LogicalMinWidth() : item_style.LogicalMinHeight();
        ...
        case Length::kCalculated: {
            const NGBoxStrut border_padding = ComputeBorders(space, node) + ComputePadding(space, item_style);
            ...
            if (!min_length.IsAuto() || item_style.IsScrollContainer() || !grid_item->IsSpanningAutoMinimumTrack(track_direction)
                ...
                if (is_parallel_with_track_direction) {
                    auto MinMaxSizesFunc
                        = [&](MinMaxSizesType type) -> MinMaxSizesResult { return node.ComputeMinMaxSizes(item_style.GetWritingMode(), type, space); };

                    contribution = ResolveMinInlineLength(space, item_style, border_padding, MinMaxSizesFunc, item_style.LogicalMinWidth());
                } else {
                    contribution = ResolveMinBlockLength(space, item_style, border_padding, item_style.LogicalMinHeight());
                }
                break;
            }
            ...
            const auto& track_collection = is_for_columns ? *layout_data.Columns() : *layout_data.Rows();

            auto spanned_tracks_definite_max_size = track_collection.ComputeSetSpanSize(set_indices.begin, set_indices.end);
...
NGGridPlacementData NGGridLayoutAlgorithm::PlacementData() const
{
    ...
    if (const auto* subgridded_columns = constraint_space.SubgriddedColumns()) {
        ...
        placement_data.subgridded_column_span_size = subgridded_columns->EndLineOfImplicitGrid();
    } else {
        placement_data.column_auto_repetitions = ComputeAutomaticRepetitions(kForColumns);
    }
    ...
    if (const auto* subgridded_rows = constraint_space.SubgriddedRows()) {
        // TODO(ethavar): Compute automatic repetitions for subgrid.
        placement_data.subgridded_row_span_size = subgridded_rows->EndLineOfImplicitGrid();
    } else {
        placement_data.row_auto_repetitions = ComputeAutomaticRepetitions(kForRows);
    }
...
std::unique_ptr<NGGridLayoutTrackCollection> NGGridLayoutAlgorithm::LayoutTrackCollection(
    const NGGridPlacementData& placement_data, const GridTrackSizingDirection track_direction, GridItems* grid_items) const
{
    const auto& container_style = Style();
    const auto& constraint_space = ConstraintSpace();

    const auto is_for_columns = track_direction == kForColumns;
    const auto* subgridded_tracks = is_for_columns ? constraint_space.SubgriddedColumns() : constraint_space.SubgriddedRows();
    if (subgridded_tracks) {
        return std::make_unique<NGGridLayoutTrackCollection>(
            *subgridded_tracks, BorderScrollbarPadding(), ComputeMarginsForSelf(constraint_space, container_style));
    }

    NGGridBlockTrackCollection track_builder_collection(container_style, placement_data, track_direction);
    BuildBlockTrackCollection(grid_items, &track_builder_collection);

    const bool is_available_size_indefinite = (is_for_columns ? grid_available_size_.inline_size : grid_available_size_.block_size) == kIndefiniteSize;

    return std::make_unique<NGGridSizingTrackCollection>(track_builder_collection, is_available_size_indefinite);
}

// https://drafts.csswg.org/css-grid-2/#auto-repeat
wtf_size_t NGGridLayoutAlgorithm::ComputeAutomaticRepetitions(const GridTrackSizingDirection track_direction) const
{
    const bool is_for_columns = track_direction == kForColumns;
    const auto& track_list = is_for_columns ? Style().GridTemplateColumns().TrackList() : Style().GridTemplateRows().TrackList();
    ...
    for (wtf_size_t repeater_index = 0; repeater_index < track_list.RepeaterCount(); ++repeater_index) {
        ...
        for (wtf_size_t i = 0; i < repeater_track_count; ++i) {
            ...
            if (track_size.HasFixedMinTrackBreadth()) {
                fixed_min_track_breadth = MinimumValueForLength(track_size.MinTrackBreadth().length(), available_size);
            }

            absl::optional<LayoutUnit> fixed_max_track_breadth;
            if (track_size.HasFixedMaxTrackBreadth()) {
                fixed_max_track_breadth = MinimumValueForLength(track_size.MaxTrackBreadth().length(), available_size);
            }
...
void NGGridLayoutAlgorithm::BuildBlockTrackCollection(GridItems* grid_items, NGGridBlockTrackCollection* track_collection) const
{
...
void NGGridLayoutAlgorithm::CalculateAlignmentBaselines(const NGGridLayoutData& layout_data, const SizingConstraint sizing_constraint, GridItems* grid_items,
    NGGridSizingTrackCollection* track_collection, bool* needs_additional_pass) const
{
    DCHECK(grid_items && track_collection);
    const auto track_direction = track_collection->Direction();
    ...
    auto UpdateBaseline = [&](const GridItemData& grid_item, LayoutUnit candidate_baseline) {
        // "If a box spans multiple shared alignment contexts, then it participates
        //  in first/last baseline alignment within its start-most/end-most shared
        //  alignment context along that axis"
        // https://www.w3.org/TR/css-align-3/#baseline-sharing-group
        if (grid_item.BaselineGroup(track_direction) == BaselineGroup::kMajor) {
            track_collection->SetMajorBaseline(grid_item.SetIndices(track_direction).begin, candidate_baseline);
        } else {
            track_collection->SetMinorBaseline(grid_item.SetIndices(track_direction).end - 1, candidate_baseline);
        }
    };
    ...
    for (auto& grid_item : *grid_items) {
        ...
        const auto space = CreateConstraintSpaceForLayout(grid_item, layout_data, &unused_grid_area);
        ...
        if (InlineLengthUnresolvable(space, item_style.LogicalWidth()) || InlineLengthUnresolvable(space, item_style.LogicalMinWidth())
        ...
        const NGBoxFragment baseline_fragment(baseline_writing_direction, To<NGPhysicalBoxFragment>(result->PhysicalFragment()));
        ...
        const auto margins = ComputeMarginsFor(space, item_style, baseline_writing_direction);
        const bool is_last_baseline = grid_item.IsLastBaselineSpecifiedForDirection(track_direction);

        LayoutUnit baseline = (is_last_baseline ? margins.block_end : margins.block_start) + GetLogicalBaseline(baseline_fragment, is_last_baseline);
        ...
        // a better solution.
        if (grid_item.IsBaselineAlignedForDirection(track_direction) || grid_item.node.IsReplaced()) {
...
void NGGridLayoutAlgorithm::InitializeTrackSizes(const NGGridProperties& grid_properties, NGGridLayoutTrackCollection* track_collection) const
{
    ...
    for (auto set_iterator = sizing_collection->GetSetIterator(); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
        ...
        if (track_size.IsFitContent()) {
            ...
            DCHECK(!track_size.FitContentTrackBreadth().HasPercentage() || available_size != kIndefiniteSize);

            LayoutUnit fit_content_argument = MinimumValueForLength(track_size.FitContentTrackBreadth().length(), available_size);
            current_set.fit_content_limit = fit_content_argument * current_set.track_count;
        }

        if (track_size.HasFixedMaxTrackBreadth()) {
            DCHECK(!track_size.MaxTrackBreadth().HasPercentage() || available_size != kIndefiniteSize);
            ...
            LayoutUnit fixed_max_breadth = MinimumValueForLength(track_size.MaxTrackBreadth().length(), available_size);
        ...
        if (track_size.HasFixedMinTrackBreadth()) {
            DCHECK(!track_size.MinTrackBreadth().HasPercentage() || available_size != kIndefiniteSize);

            // A fixed sizing function: Resolve to an absolute length and use that
            // size as the track’s initial base size.
            LayoutUnit fixed_min_breadth = MinimumValueForLength(track_size.MinTrackBreadth().length(), available_size);
    ...
    LayoutUnit start_border_scrollbar_padding = is_for_columns ? BorderScrollbarPadding().inline_start : BorderScrollbarPadding().block_start;
    ...
    if (grid_properties.IsSpanningOnlyDefiniteTracks(track_direction)) {
        ...
        auto first_set_geometry = ComputeFirstSetGeometry(*sizing_collection, Style(), available_size, start_border_scrollbar_padding);
        sizing_collection->CacheSetsGeometry(first_set_geometry.start_offset, first_set_geometry.gutter_size);
    } else {
        sizing_collection->InitializeSetsGeometry(start_border_scrollbar_padding, GutterSize(track_direction));
    }
}
...
Vector<BlockSizeDependentGridItem> BlockSizeDependentGridItems(const GridItems& grid_items, const NGGridSizingTrackCollection& track_collection)
{
...
bool MayChangeBlockSizeDependentGridItemContributions(
    const Vector<BlockSizeDependentGridItem>& dependent_items, const NGGridSizingTrackCollection& track_collection)
{
    DCHECK_EQ(track_collection.Direction(), kForRows);
...
void NGGridLayoutAlgorithm::ComputeUsedTrackSizes(const NGGridLayoutData& layout_data, const NGGridProperties& grid_properties,
    const SizingConstraint sizing_constraint, GridItems* grid_items, NGGridLayoutTrackCollection* track_collection, bool* needs_additional_pass) const
{
    DCHECK(grid_items && track_collection);
    ...
    if (!track_collection->IsForSizing() || grid_properties.IsSpanningOnlyDefiniteTracks(track_collection->Direction())) {
        return;
    }
    ...
    if (grid_properties.HasBaseline(track_direction)) {
        CalculateAlignmentBaselines(layout_data, sizing_constraint, grid_items, sizing_collection, needs_additional_pass);
    }

    // 2. Resolve intrinsic track sizing functions to absolute lengths.
    if (grid_properties.HasIntrinsicTrack(track_direction)) {
        ResolveIntrinsicTrackSizes(layout_data, sizing_constraint, sizing_collection, grid_items);
    }
    ...
    if (grid_properties.HasFlexibleTrack(track_direction)) {
        ExpandFlexibleTracks(layout_data, sizing_constraint, sizing_collection, grid_items);
    }
    ...
    const bool needs_to_check_block_size_dependent_grid_items = !is_for_columns && needs_additional_pass && !(*needs_additional_pass);

    Vector<BlockSizeDependentGridItem> block_size_dependent_grid_items;
    if (needs_to_check_block_size_dependent_grid_items) {
        block_size_dependent_grid_items = BlockSizeDependentGridItems(*grid_items, *sizing_collection);
    }

    auto first_set_geometry
        = ComputeFirstSetGeometry(*sizing_collection, Style(), is_for_columns ? grid_available_size_.inline_size : grid_available_size_.block_size,
            is_for_columns ? BorderScrollbarPadding().inline_start : BorderScrollbarPadding().block_start);

    sizing_collection->CacheSetsGeometry(first_set_geometry.start_offset, first_set_geometry.gutter_size);

    if (needs_to_check_block_size_dependent_grid_items) {
        *needs_additional_pass = MayChangeBlockSizeDependentGridItemContributions(block_size_dependent_grid_items, *sizing_collection);
    }
}
...
SetIterator GetSetIteratorForItem(const GridItemData& grid_item, NGGridSizingTrackCollection& track_collection)
{
...
LayoutUnit AffectedSizeForContribution(const NGGridSet& set, GridItemContributionType contribution_type)
{
    switch (contribution_type) {
    case GridItemContributionType::kForIntrinsicMinimums:
    case GridItemContributionType::kForContentBasedMinimums:
...
void GrowAffectedSizeByPlannedIncrease(NGGridSet& set, GridItemContributionType contribution_type)
{
...
bool IsContributionAppliedToSet(const NGGridSet& set, GridItemContributionType contribution_type)
{
    switch (contribution_type) {
    case GridItemContributionType::kForIntrinsicMinimums:
...
bool ShouldUsedSizeGrowBeyondLimit(const NGGridSet& set, GridItemContributionType contribution_type)
{
    switch (contribution_type) {
    case GridItemContributionType::kForIntrinsicMinimums:
    case GridItemContributionType::kForContentBasedMinimums:
...
LayoutUnit GrowthPotentialForSet(const NGGridSet& set, GridItemContributionType contribution_type,
...
template <> bool AreEqual<double>(double a, double b)
{
...
void DistributeExtraSpaceToSets(LayoutUnit extra_space, double flex_factor_sum, GridItemContributionType contribution_type, GridSetVector* sets_to_grow,
    GridSetVector* sets_to_grow_beyond_limit)
{
    ...
    using ShareRatioType = typename std::conditional<is_equal_distribution, wtf_size_t, double>::type;
    ...
    if (growable_track_count || IsDistributionForGrowthLimits(contribution_type)) {
        auto CompareSetsByGrowthPotential = [contribution_type](const NGGridSet* lhs, const NGGridSet* rhs) {
    ...
    auto ExtraSpaceShare = [&](const NGGridSet& set, LayoutUnit growth_potential) -> LayoutUnit {
    ...
    if (sets_to_grow_beyond_limit && extra_space) {
        ...
        auto BeyondLimitsGrowthPotential = [contribution_type](const NGGridSet& set) -> LayoutUnit {
...
void DistributeExtraSpaceToSetsEqually(
    LayoutUnit extra_space, GridItemContributionType contribution_type, GridSetVector* sets_to_grow, GridSetVector* sets_to_grow_beyond_limit = nullptr)
{
...
void DistributeExtraSpaceToWeightedSets(
    LayoutUnit extra_space, const double flex_factor_sum, GridItemContributionType contribution_type, GridSetVector* sets_to_grow)
{
...
void NGGridLayoutAlgorithm::IncreaseTrackSizesToAccommodateGridItems(GridItems::Iterator group_begin, GridItems::Iterator group_end,
    const NGGridLayoutData& layout_data, const bool is_group_spanning_flex_track, const SizingConstraint sizing_constraint,
    const GridItemContributionType contribution_type, NGGridSizingTrackCollection* track_collection) const
{
    DCHECK(track_collection);
    const auto track_direction = track_collection->Direction();
    ...
    while (group_begin != group_end) {
        ...
        LayoutUnit extra_space = ContributionSizeForGridItem(layout_data, sizing_constraint, track_direction, contribution_type, &grid_item);
    ...
    for (auto set_iterator = track_collection->GetSetIterator(); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
        GrowAffectedSizeByPlannedIncrease(set_iterator.CurrentSet(), contribution_type);
    }
}

// https://drafts.csswg.org/css-grid-2/#algo-content
void NGGridLayoutAlgorithm::ResolveIntrinsicTrackSizes(
    const NGGridLayoutData& layout_data, const SizingConstraint sizing_constraint, NGGridSizingTrackCollection* track_collection, GridItems* grid_items) const
{
    ...
    auto CompareGridItemsForIntrinsicTrackResolution = [track_direction](const Member<GridItemData>& lhs, const Member<GridItemData>& rhs) -> bool {
    ...
    std::sort(reordered_grid_items.item_data.begin(), reordered_grid_items.item_data.end(), CompareGridItemsForIntrinsicTrackResolution);
    ...
    while (current_group_begin != reordered_grid_items.end() && !current_group_begin->IsSpanningFlexibleTrack(track_direction)) {
        ...
        IncreaseTrackSizesToAccommodateGridItems(current_group_begin, current_group_end, layout_data,
    ...
    if (current_group_begin != reordered_grid_items.end()) {
        ...
        IncreaseTrackSizesToAccommodateGridItems(current_group_begin, reordered_grid_items.end(), layout_data,
...
void NGGridLayoutAlgorithm::MaximizeTracks(const SizingConstraint sizing_constraint, NGGridSizingTrackCollection* track_collection) const
{
...
void NGGridLayoutAlgorithm::StretchAutoTracks(const SizingConstraint sizing_constraint, NGGridSizingTrackCollection* track_collection) const
{
    const auto track_direction = track_collection->Direction();
    ...
    const NGGridLayoutData& layout_data, const SizingConstraint sizing_constraint, NGGridSizingTrackCollection* track_collection, GridItems* grid_items) const
{
    ...
    auto FindFrSize = [&](SetIterator set_iterator, LayoutUnit leftover_space) -> double {
        ClampedDouble flex_factor_sum = 0;
        ...
        while (!set_iterator.IsAtEnd()) {
            auto& set = set_iterator.CurrentSet();
            if (set.track_size.HasFlexMaxTrackBreadth() && !AreEqual<double>(set.FlexFactor(), 0)) {
        ...
        auto CompareSetsByBaseSizeFlexFactorRatio = [](NGGridSet* lhs, NGGridSet* rhs) -> bool {
    ...
    if (free_space != kIndefiniteSize) {
        ...
        for (auto& grid_item : *grid_items) {
            if (grid_item.IsSpanningFlexibleTrack(track_direction)) {
                double grid_item_fr_size = FindFrSize(GetSetIteratorForItem(grid_item, *track_collection),
                    ContributionSizeForGridItem(layout_data, sizing_constraint, track_direction, GridItemContributionType::kForMaxContentMaximums, &grid_item));
        ...
        for (auto set_iterator = track_collection->GetConstSetIterator(); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
            ...
            double set_flex_factor = base::ClampMax(set.FlexFactor(), set.track_count);
    ...
    for (auto set_iterator = track_collection->GetSetIterator(); !set_iterator.IsAtEnd(); set_iterator.MoveToNextSet()) {
        ...
        const LayoutUnit expanded_size = LayoutUnit::FromRawValue(fr_share + kDoubleEpsilon);
...
LayoutUnit NGGridLayoutAlgorithm::GutterSize(const GridTrackSizingDirection track_direction) const
{
    const bool is_for_columns = track_direction == kForColumns;
    ...
    LayoutUnit available_size = (is_for_columns ? grid_available_size_.inline_size : grid_available_size_.block_size).ClampIndefiniteToZero();
...
LayoutUnit NGGridLayoutAlgorithm::DetermineFreeSpace(SizingConstraint sizing_constraint, const NGGridSizingTrackCollection& track_collection) const
{
    const auto track_direction = track_collection.Direction();
...
void AlignmentOffsetForOutOfFlow(const AxisEdge inline_axis_edge, const AxisEdge block_axis_edge, const LogicalSize container_size,
    NGLogicalStaticPosition::InlineEdge* inline_edge, NGLogicalStaticPosition::BlockEdge* block_edge, LogicalOffset* offset)
{
...
const NGConstraintSpace NGGridLayoutAlgorithm::CreateConstraintSpace(const GridItemData& grid_item, const NGGridLayoutData& layout_data,
    const LogicalSize& containing_grid_area_size, NGCacheSlot cache_slot, absl::optional<LayoutUnit> opt_fixed_block_size,
    absl::optional<LayoutUnit> opt_fragment_relative_block_offset, bool opt_min_block_size_should_encompass_intrinsic_size) const
{
    const auto& container_constraint_space = ConstraintSpace();

    NGConstraintSpaceBuilder builder(container_constraint_space, grid_item.node.Style().GetWritingDirection(),
    ...
    if (opt_fixed_block_size) {
        builder.SetAvailableSize({ containing_grid_area_size.inline_size, *opt_fixed_block_size });
    ...
    if (layout_data.columns && layout_data.rows && grid_item.has_subgridded_columns) {
        const auto& range_indices = grid_item.is_parallel_with_root_grid ? grid_item.column_range_indices : grid_item.row_range_indices;

        const auto* track_collection = grid_item.is_parallel_with_root_grid ? layout_data.Columns() : layout_data.Rows();

        builder.SetSubgriddedColumns(
            std::make_unique<NGGridLayoutTrackCollection>(track_collection->CreateSubgridCollection(range_indices.begin, range_indices.end, kForColumns)));
    }

    if (layout_data.columns && layout_data.rows && grid_item.has_subgridded_rows) {
        const auto& range_indices = grid_item.is_parallel_with_root_grid ? grid_item.row_range_indices : grid_item.column_range_indices;

        const auto* track_collection = grid_item.is_parallel_with_root_grid ? layout_data.Rows() : layout_data.Columns();

        builder.SetSubgriddedRows(
            std::make_unique<NGGridLayoutTrackCollection>(track_collection->CreateSubgridCollection(range_indices.begin, range_indices.end, kForRows)));
    }

    if (container_constraint_space.HasBlockFragmentation() && opt_fragment_relative_block_offset) {
        if (opt_min_block_size_should_encompass_intrinsic_size)
            builder.SetMinBlockSizeShouldEncompassIntrinsicSize();

        SetupSpaceBuilderForFragmentation(container_constraint_space, grid_item.node, *opt_fragment_relative_block_offset, &builder, /* is_new_fc */ true,
            container_builder_.RequiresContentBeforeBreaking());
    }
    return builder.ToConstraintSpace();
}

const NGConstraintSpace NGGridLayoutAlgorithm::CreateConstraintSpaceForLayout(const GridItemData& grid_item, const NGGridLayoutData& layout_data,
    LogicalRect* containing_grid_area, absl::optional<LayoutUnit> opt_fragment_relative_block_offset,
    bool opt_min_block_size_should_encompass_intrinsic_size) const
{
    ComputeGridItemOffsetAndSize(grid_item, *layout_data.Columns(), &containing_grid_area->offset.inline_offset, &containing_grid_area->size.inline_size);

    ComputeGridItemOffsetAndSize(grid_item, *layout_data.Rows(), &containing_grid_area->offset.block_offset, &containing_grid_area->size.block_size);

    return CreateConstraintSpace(grid_item, layout_data, containing_grid_area->size, NGCacheSlot::kLayout,
        /* opt_fixed_block_size */ absl::nullopt, opt_fragment_relative_block_offset, opt_min_block_size_should_encompass_intrinsic_size);
}

const NGConstraintSpace NGGridLayoutAlgorithm::CreateConstraintSpaceForMeasure(const GridItemData& grid_item, const NGGridLayoutData& layout_data,
    const GridTrackSizingDirection track_direction, absl::optional<LayoutUnit> opt_fixed_block_size) const
{
    LogicalOffset unused_offset;
    LogicalSize containing_grid_area_size(kIndefiniteSize, kIndefiniteSize);

    if (track_direction == kForColumns) {
        ComputeGridItemOffsetAndSize(grid_item, *layout_data.Rows(), &unused_offset.block_offset, &containing_grid_area_size.block_size);
    } else {
        ComputeGridItemOffsetAndSize(grid_item, *layout_data.Columns(), &unused_offset.inline_offset, &containing_grid_area_size.inline_size);
    }
    return CreateConstraintSpace(grid_item, layout_data, containing_grid_area_size, NGCacheSlot::kMeasure, opt_fixed_block_size);
}
...
public:
    ...
    void Accumulate(const GridItemData& grid_item, const NGBoxFragment& fragment, const LayoutUnit block_offset)
    {
...
void NGGridLayoutAlgorithm::PlaceGridItems(const GridItems& grid_items, const NGGridLayoutData& layout_data, Vector<EBreakBetween>* out_row_break_between,
    Vector<GridItemPlacementData>* out_grid_items_placement_data)
{
    ...
    bool should_propagate_child_break_values = container_space.ShouldPropagateChildBreakValues();
    ...
    for (const auto& grid_item : grid_items) {
        ...
        const auto space = CreateConstraintSpaceForLayout(grid_item, layout_data, &containing_grid_area);

        const auto& item_style = grid_item.node.Style();
        const auto margins = ComputeMarginsFor(space, item_style, container_space);

        auto* result = grid_item.node.Layout(space);
        const auto& physical_fragment = To<NGPhysicalBoxFragment>(result->PhysicalFragment());
        NGBoxFragment fragment(container_writing_direction, physical_fragment);

        auto BaselineOffset = [&](const GridTrackSizingDirection track_direction, LayoutUnit size) -> LayoutUnit {
            if (!grid_item.IsBaselineAlignedForDirection(track_direction))
                return LayoutUnit();

            NGBoxFragment baseline_fragment(grid_item.BaselineWritingDirection(track_direction), physical_fragment);
            // The baseline offset is the difference between the grid item's baseline
            // and its track baseline.
            const LayoutUnit baseline_delta = Baseline(layout_data, grid_item, track_direction)
                - GetLogicalBaseline(baseline_fragment, grid_item.IsLastBaselineSpecifiedForDirection(track_direction));
            if (grid_item.BaselineGroup(track_direction) == BaselineGroup::kMajor)
        ...
        containing_grid_area.offset
            += LogicalOffset(AlignmentOffset(containing_grid_area.size.inline_size, fragment.InlineSize(), margins.inline_start, margins.inline_end,
                                 inline_baseline_offset, grid_item.InlineAxisAlignment(), grid_item.IsInlineAxisOverflowSafe()),
                AlignmentOffset(containing_grid_area.size.block_size, fragment.BlockSize(), margins.block_start, margins.block_end, block_baseline_offset,
                    grid_item.BlockAxisAlignment(), grid_item.IsBlockAxisOverflowSafe()));
        ...
        if (out_grid_items_placement_data) {
            ...
            container_builder_.AddResult(*result, containing_grid_area.offset, relative_offset);
...
public:
    ResultAndOffsets(const NGLayoutResult* result, LogicalOffset offset, LogicalOffset relative_offset)
        : result(result)
        , offset(offset)
...
void NGGridLayoutAlgorithm::PlaceGridItemsForFragmentation(const GridItems& grid_items, const Vector<EBreakBetween>& row_break_between,
    NGGridLayoutData* layout_data, Vector<GridItemPlacementData>* grid_items_placement_data, Vector<LayoutUnit>* row_offset_adjustments,
    LayoutUnit* intrinsic_block_size, LayoutUnit* consumed_grid_block_size)
{
    DCHECK(layout_data && grid_items_placement_data && row_offset_adjustments && intrinsic_block_size && consumed_grid_block_size);

    // TODO(ikilpatrick): Update |SetHasSeenAllChildren| and early exit if true.
    const auto container_writing_direction = ConstraintSpace().GetWritingDirection();
    ...
    auto MinBlockSizeShouldEncompassIntrinsicSize = [&](const GridItemData& grid_item, bool has_descendant_that_depends_on_percentage_block_size) -> bool {
        ...
        return !grid_item.IsSpanningFixedMinimumTrack(kForRows) || Style().LogicalHeight().IsAutoOrContentOrIntrinsic();
    };
    ...
    auto PlaceItems = [&]() {
        ...
        for (const auto& grid_item : grid_items) {
            ...
            const LayoutUnit fragment_relative_block_offset
                = IsResumingLayout(break_token) ? LayoutUnit() : item_placement_data.offset.block_offset - *consumed_grid_block_size;
            ...
            const auto space = CreateConstraintSpaceForLayout(
                grid_item, *layout_data, &grid_area, fragment_relative_block_offset, min_block_size_should_encompass_intrinsic_size);

            // Make the grid area relative to this fragment.
            const auto item_row_set_index = grid_item.SetIndices(kForRows).begin;
            grid_area.offset.block_offset += (*row_offset_adjustments)[item_row_set_index] - *consumed_grid_block_size;
            ...
            result_and_offsets.emplace_back(
                result, LogicalOffset(item_placement_data.offset.inline_offset, fragment_relative_block_offset), item_placement_data.relative_offset);
            ...
            if (result->Status() != NGLayoutResult::kSuccess) {
                ...
                out_of_fragmentainer_space_item_placement.push_back(&item_placement_data);
                continue;
            }

            const NGBoxFragment fragment(container_writing_direction, To<NGPhysicalBoxFragment>(result->PhysicalFragment()));
            baseline_accumulator.Accumulate(grid_item, fragment, fragment_relative_block_offset);
            ...
            if (row_has_container_separation && item_row_set_index < breakpoint_row_set_index) {
                ...
                const NGBreakAppeal appeal_before
                    = CalculateBreakAppealBefore(ConstraintSpace(), grid_item.node, *result, container_builder_, row_has_container_separation);
                if (!MovePastBreakpoint(ConstraintSpace(), grid_item.node, *result, fragment_relative_block_offset, appeal_before,
                        /* builder */ nullptr)) {
                    UpdateBreakpointRowSetIndex(item_row_set_index);

                    // We are choosing to add an early breakpoint at a row. Propagate our
                    // space shortage to the column balancer.
                    PropagateSpaceShortage(ConstraintSpace(), result, fragment_relative_block_offset, &container_builder_);
                    ...
                    if (IsAvoidBreakValue(ConstraintSpace(), break_between)) {
                        for (int index = item_row_set_index - 1; index >= 0; --index) {
                            ...
                            LayoutUnit offset = layout_data->Rows()->GetSetOffset(index) + (*row_offset_adjustments)[index] - *consumed_grid_block_size;
            ...
            if (min_block_size_should_encompass_intrinsic_size && item_row_set_index <= expansion_row_set_index && IsExpansionMakingProgress(item_row_set_index)
                && fragmentainer_space != kIndefiniteSize && grid_area.BlockEndOffset() <= fragmentainer_space) {
    ...
    auto ShiftBreakpointIntoNextFragmentainer = [&]() -> bool {
        ...
        const LayoutUnit fragment_relative_row_offset = row_offset - *consumed_grid_block_size;
    ...
    for (auto& result_and_offset : result_and_offsets) {
        container_builder_.AddResult(*result_and_offset.result, result_and_offset.offset, result_and_offset.relative_offset);
    }
...
void NGGridLayoutAlgorithm::PlaceOutOfFlowItems(const NGGridLayoutData& layout_data, const LayoutUnit block_size, HeapVector<Member<LayoutBox>>& oof_children)
{
    ...
    if (UNLIKELY(InvolvedInBlockFragmentation(container_builder_))) {
        should_process_block_end = !container_builder_.DidBreakSelf() && !container_builder_.HasChildBreakInside();
    }
    ...
    const LayoutUnit previous_consumed_block_size = BreakToken() ? BreakToken()->ConsumedBlockSize() : LayoutUnit();
    ...
    for (LayoutBox* oof_child : oofs) {
        ...
        AlignmentOffsetForOutOfFlow(
            out_of_flow_item.InlineAxisAlignment(), out_of_flow_item.BlockAxisAlignment(), containing_block_size, &inline_edge, &block_edge, &child_offset);
        ...
        if (should_process_block_end || child_offset.block_offset <= FragmentainerCapacity(ConstraintSpace())) {
...
Vector<std::div_t> ComputeTrackSizesInRange(
    const NGGridLayoutTrackCollection& track_collection, const wtf_size_t range_begin_set_index, const wtf_size_t range_set_count)
{
...
LayoutUnit ComputeTrackOffsetInRange(const NGGridLayoutTrackCollection& track_collection, const wtf_size_t range_begin_set_index,
...
LayoutUnit TrackOffset(const NGGridLayoutTrackCollection& track_collection, const wtf_size_t range_index, const wtf_size_t offset_in_range)
{
...
LayoutUnit TrackStartOffset(const NGGridLayoutTrackCollection& track_collection, const wtf_size_t range_index, const wtf_size_t offset_in_range)
{
...
LayoutUnit TrackEndOffset(const NGGridLayoutTrackCollection& track_collection, const wtf_size_t range_index, const wtf_size_t offset_in_range)
{
    if (!track_collection.RangeCount()) {
...
void ComputeOutOfFlowOffsetAndSize(const GridItemData& out_of_flow_item, const NGGridLayoutTrackCollection& track_collection, const NGBoxStrut& borders,
...
void NGGridLayoutAlgorithm::ComputeGridItemOffsetAndSize(
    const GridItemData& grid_item, const NGGridLayoutTrackCollection& track_collection, LayoutUnit* start_offset, LayoutUnit* size) const
{
    DCHECK(start_offset && size && !grid_item.IsOutOfFlow());

    const auto& set_indices = grid_item.SetIndices(track_collection.Direction());
    *start_offset = track_collection.GetSetOffset(set_indices.begin);
    *size = track_collection.ComputeSetSpanSize(set_indices.begin, set_indices.end);
...
// static
LogicalRect NGGridLayoutAlgorithm::ComputeOutOfFlowItemContainingRect(const NGGridPlacementData& placement_data, const NGGridLayoutData& layout_data,
    const ComputedStyle& grid_style, const NGBoxStrut& borders, const LogicalSize& border_box_size, GridItemData* out_of_flow_item)
{
