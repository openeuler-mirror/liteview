...
class NGGridLayoutAlgorithmTest : public NGBaseLayoutAlgorithmTest, private ScopedLayoutNGBlockFragmentationForTest, private ScopedLayoutNGSubgridForTest {
protected:
    ...
    void BuildGridItemsAndTrackCollections(NGGridLayoutAlgorithm& algorithm)
    {
        ...
        auto grid_items = node.ConstructGridItems(algorithm.PlacementData(),
            /* oof_children */ nullptr, &has_nested_subgrid);

        LayoutUnit unused_intrinsic_block_size;
        algorithm.ComputeGridGeometry(node.CachedPlacementData(), &grid_items, &layout_data_, &unused_intrinsic_block_size);
    ...
    const NGGridSizingTrackCollection& TrackCollection(GridTrackSizingDirection track_direction)
    {
    ...
    LayoutUnit BaseRowSizeForChild(const NGGridLayoutAlgorithm& algorithm, wtf_size_t index)
    {
        LayoutUnit offset, size;
        algorithm.ComputeGridItemOffsetAndSize(GridItem(index), *layout_data_.Rows(), &offset, &size);
        return size;
    }
    ...
    Vector<wtf_size_t> GridItemsWithColumnSpanProperty(const NGGridLayoutAlgorithm& algorithm, TrackSpanProperties::PropertyId property)
    {
    ...
    Vector<wtf_size_t> GridItemsWithRowSpanProperty(const NGGridLayoutAlgorithm& algorithm, TrackSpanProperties::PropertyId property)
    {
        Vector<wtf_size_t> results;
        for (wtf_size_t i = 0; i < GridItemCount(); ++i) {
    ...
    Vector<LayoutUnit> BaseSizes(NGGridLayoutAlgorithm& algorithm, GridTrackSizingDirection track_direction)
    {
    ...
    Vector<LayoutUnit> GrowthLimits(NGGridLayoutAlgorithm& algorithm, GridTrackSizingDirection track_direction)
    {
        const auto& collection = TrackCollection(track_direction);
    ...
    Persistent<GridItems::GridItemDataVector> cached_grid_items_ = MakeGarbageCollected<GridItems::GridItemDataVector>();
...
TEST_F(NGGridLayoutAlgorithmTest, NGGridLayoutAlgorithmRangesImplicitAutoColumns)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    #grid1 {
...
TEST_F(NGGridLayoutAlgorithmTest, NGGridLayoutAlgorithmDetermineGridItemsSpanningIntrinsicOrFlexTracks)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    #grid {
    ...
    Vector<wtf_size_t> actual_items = GridItemsWithColumnSpanProperty(algorithm, TrackSpanProperties::kHasIntrinsicTrack);
    ...
    actual_items = GridItemsWithColumnSpanProperty(algorithm, TrackSpanProperties::kHasFlexibleTrack);
    ...
    actual_items = GridItemsWithRowSpanProperty(algorithm, TrackSpanProperties::kHasIntrinsicTrack);
    EXPECT_EQ(expected_grid_items_spanning_intrinsic_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
        EXPECT_EQ(expected_grid_items_spanning_intrinsic_track[i], actual_items[i]);

    actual_items = GridItemsWithRowSpanProperty(algorithm, TrackSpanProperties::kHasFlexibleTrack);
    EXPECT_EQ(expected_grid_items_spanning_flex_track.size(), actual_items.size());
    for (wtf_size_t i = 0; i < actual_items.size(); ++i)
        EXPECT_EQ(expected_grid_items_spanning_flex_track[i], actual_items[i]);
}
...
TEST_F(NGGridLayoutAlgorithmTest, SubgridLineNameList)
{
    ...
    const Vector<NamedGridLine> row_named_lines = { NamedGridLine("a"), NamedGridLine("b"), NamedGridLine("c") };
...
TEST_F(NGGridLayoutAlgorithmTest, SubgridLineNameListWithRepeaters)
{
    ...
    const Vector<NamedGridLine> column_named_lines = { NamedGridLine("a"), NamedGridLine("b"), NamedGridLine("c"), NamedGridLine("d") };
    ...
    const Vector<NamedGridLine> row_named_lines = { NamedGridLine("a"), NamedGridLine("b", /* is_in_repeat */ true, /* is_first_repeat */ true),
        NamedGridLine("c", /* is_in_repeat */ true, /* is_first_repeat */ true), NamedGridLine("b", /* is_in_repeat */ true),
        NamedGridLine("c", /* is_in_repeat */ true), NamedGridLine("d") };
