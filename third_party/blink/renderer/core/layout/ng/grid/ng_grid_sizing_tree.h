...
struct CORE_EXPORT NGGridProperties {
    ...
    bool IsDependentOnAvailableSize(const GridTrackSizingDirection track_direction) const;
    bool IsSpanningOnlyDefiniteTracks(const GridTrackSizingDirection track_direction) const;
...
struct NGGridSizingData : public GarbageCollected<NGGridSizingData> {
    ...
    NGGridSizingData(const NGGridSizingData* parent_sizing_data, const GridItemData* subgrid_data_in_parent)
...
public:
    using GridSizingDataLookupMap = HeapHashMap<Member<const LayoutBox>, Member<NGGridSizingData>>;
    using GridSizingDataVector = HeapVector<Member<NGGridSizingData>, 16>;

    NGGridSizingData& CreateSizingData(const NGGridNode& grid, const NGGridSizingData* parent_sizing_data, const GridItemData* subgrid_data_in_parent);

    wtf_size_t Size() const
    {
        return sizing_data_.size();
    }
