...
struct CORE_EXPORT NGGridPlacementData {
    ...
    NGGridPlacementData(const ComputedStyle& grid_style, const NGGridLineResolver& parent_line_resolver)
    ...
    {
        return grid_item_positions == other.grid_item_positions && explicit_grid_column_count == other.explicit_grid_column_count
            && explicit_grid_row_count == other.explicit_grid_row_count && column_start_offset == other.column_start_offset
            && row_start_offset == other.row_start_offset;
    }

    bool HasStandaloneAxis(GridTrackSizingDirection track_direction) const
    {
        const wtf_size_t subgrid_span_size = (track_direction == kForColumns) ? subgridded_column_span_size : subgridded_row_span_size;
        return subgrid_span_size == kNotFound;
    }

    bool IsSubgriddedToParent() const
    {
        return subgridded_column_span_size != kNotFound || subgridded_row_span_size != kNotFound;
    }
...
public:
    ...
    NGGridLayoutData(std::unique_ptr<NGGridLayoutTrackCollection> columns, std::unique_ptr<NGGridLayoutTrackCollection> rows)
