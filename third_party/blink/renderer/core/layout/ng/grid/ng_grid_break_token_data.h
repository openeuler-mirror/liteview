...
struct NGGridBreakTokenData final : NGBlockBreakTokenData {
    NGGridBreakTokenData(const NGBlockBreakTokenData* break_token_data, const NGGridLayoutData& layout_data, LayoutUnit intrinsic_block_size,
        LayoutUnit consumed_grid_block_size, const Vector<GridItemPlacementData>& grid_items_placement_data, const Vector<LayoutUnit>& row_offset_adjustments,
        const Vector<EBreakBetween>& row_break_between, const HeapVector<Member<LayoutBox>>& oof_children)
...
template <> struct DowncastTraits<NGGridBreakTokenData> {
