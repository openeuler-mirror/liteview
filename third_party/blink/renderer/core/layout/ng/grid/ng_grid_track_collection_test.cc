...
#define EXPECT_SET(expected_size, expected_count, iterator)                                                                                                    \
    {                                                                                                                                                          \
        NGGridSet expected_set(expected_count, expected_size, /* is_content_box_size_definite */ false);                                                       \
...
protected:
    ...
    {
        return NGGridBlockTrackCollection(explicit_tracks, implicit_tracks, auto_repetitions);
    }
    ...
    NGGridSizingTrackCollection::SetIterator IteratorForRange(NGGridSizingTrackCollection& track_collection, wtf_size_t range_index)
    {
        wtf_size_t starting_set_index = track_collection.RangeBeginSetIndex(range_index);
        return track_collection.GetSetIterator(starting_set_index, starting_set_index + track_collection.RangeSetCount(range_index));
    }
};
...
TEST_F(NGGridTrackCollectionTest, TestNGGridBlockTrackCollection)
{
    ...
    NGGridBlockTrackCollection block_collection = CreateBlockTrackCollection(explicit_tracks, implicit_tracks,
        /* auto_repetitions */ 3);
...
TEST_F(NGGridTrackCollectionTest, TestNGGridSetTrackSizeNormalization)
{
    ...
    // minmax(1fr, 3fr)
    TestTrackSizeNormalization(
        GridTrackSize(GridLength(1.0), GridLength(3.0)), GridTrackSize(Length::Auto(), GridLength(3.0)), GridTrackSize(Length::Auto(), GridLength(3.0)));
    // fit-content(40%)
...
TEST_F(NGGridTrackCollectionTest, TestNGGridSizingTrackCollectionSetIterator)
{
    ...
    NGGridSizingTrackCollection track_collection(block_collection, /* is_content_box_size_defined */ false);
...
TEST_F(NGGridTrackCollectionTest, TestNGGridSizingTrackCollectionExplicitTracks)
{
...
TEST_F(NGGridTrackCollectionTest, TestNGGridSizingTrackCollectionImplicitTracks)
{
    // We'll use fixed size tracks to differentiate between them by int value.
    NGGridTrackList explicit_tracks, implicit_tracks;
...
TEST_F(NGGridTrackCollectionTest, TestNGGridSizingTrackCollectionIntrinsicAndFlexTracks)
{
    ...
    Vector<GridTrackSize, 1> track_sizes
        = { GridTrackSize(Length::MinContent()), GridTrackSize(GridLength(1.0)), GridTrackSize(Length::Fixed(2)), GridTrackSize(Length::Fixed(3)) };
