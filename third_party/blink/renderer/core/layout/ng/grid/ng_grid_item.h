...
struct CORE_EXPORT GridItemData : public GarbageCollected<GridItemData> {
    ...
    GridItemData(const NGBlockNode node, const ComputedStyle& root_grid_style, bool parent_must_consider_grid_items_for_column_sizing = false,
    ...
    {
        return is_inline_axis_overflow_safe_fallback.value_or(is_inline_axis_overflow_safe);
    }
    bool IsBlockAxisOverflowSafe() const
    {
        return is_block_axis_overflow_safe_fallback.value_or(is_block_axis_overflow_safe);
    }

    bool IsBaselineAlignedForDirection(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? (InlineAxisAlignment() == AxisEdge::kFirstBaseline || InlineAxisAlignment() == AxisEdge::kLastBaseline)
                                                : (BlockAxisAlignment() == AxisEdge::kFirstBaseline || BlockAxisAlignment() == AxisEdge::kLastBaseline);
    }

    bool IsBaselineSpecifiedForDirection(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? (inline_axis_alignment == AxisEdge::kFirstBaseline || inline_axis_alignment == AxisEdge::kLastBaseline)
                                                : (block_axis_alignment == AxisEdge::kFirstBaseline || block_axis_alignment == AxisEdge::kLastBaseline);
    }

    bool IsLastBaselineSpecifiedForDirection(GridTrackSizingDirection track_direction) const
    {
        return (track_direction == kForColumns) ? inline_axis_alignment == AxisEdge::kLastBaseline : block_axis_alignment == AxisEdge::kLastBaseline;
    }
    ...
    void Set(GridTrackSizingDirection track_direction, bool parent_must_consider_grid_items_for_sizing);

    bool IsGridContainingBlock() const
    {
        return node.IsContainingBlockNGGrid();
    }
...
public:
    ...
    template <bool is_const> class IteratorBase {
        STACK_ALLOCATED();
    ...
    Iterator end()
    {
        return { &item_data, item_data.size() };
    }
