...
ASSERT_SIZE(NGGridLayoutTrackCollection::Range, SameSizeAsNGGridLayoutTrackCollectionRange);

wtf_size_t NGGridTrackCollectionBase::RangeEndLine(wtf_size_t range_index) const
{
    return RangeStartLine(range_index) + RangeTrackCount(range_index);
}

wtf_size_t NGGridTrackCollectionBase::RangeIndexFromGridLine(wtf_size_t grid_line) const
{
...
NGGridBlockTrackCollection::NGGridBlockTrackCollection(
    const ComputedStyle& grid_style, const NGGridPlacementData& placement_data, GridTrackSizingDirection track_direction)
    : NGGridTrackCollectionBase(track_direction)
    , auto_repetitions_((track_direction == kForColumns) ? placement_data.column_auto_repetitions : placement_data.row_auto_repetitions)
    , start_offset_((track_direction == kForColumns) ? placement_data.column_start_offset : placement_data.row_start_offset)
    , track_indices_need_sort_(false)
    , explicit_tracks_((track_direction == kForColumns) ? grid_style.GridTemplateColumns().TrackList() : grid_style.GridTemplateRows().TrackList())
    , implicit_tracks_((track_direction == kForColumns) ? grid_style.GridAutoColumns().NGTrackList() : grid_style.GridAutoRows().NGTrackList())
{
    ...
    const wtf_size_t named_grid_area_end_line
        = start_offset_ + ((track_direction == kForColumns) ? grid_style.NamedGridAreaColumnCount() : grid_style.NamedGridAreaRowCount());
...
void NGGridBlockTrackCollection::FinalizeRanges()
{
    ...
    while (true) {
        ...
        while (start_line_index < start_lines_.size() && current_range_start_line >= start_lines_[start_line_index].grid_line) {
            ++start_line_index;
            ++open_items_or_repeaters;
        }

        // Identify ending tracks index.
        while (end_line_index < end_lines_.size() && current_range_start_line >= end_lines_[end_line_index].grid_line) {
            ++end_line_index;
            --open_items_or_repeaters;
        ...
        wtf_size_t next_start_line = (start_line_index < start_lines_.size()) ? start_lines_[start_line_index].grid_line : kNotFound;
        ...
        if (current_explicit_repeater_index != kNotFound) {
            ...
            range.repeater_offset = (current_range_start_line - current_explicit_grid_line) % explicit_tracks_.RepeatSize(current_explicit_repeater_index);
        } else {
            range.SetIsImplicit();
            if (implicit_tracks_.RepeaterCount() == 0u) {
                ...
                range.repeater_offset
                    = (current_range_start_line + implicit_repeater_size - current_explicit_grid_line % implicit_repeater_size) % implicit_repeater_size;
            }
        }
        ...
        if (start_line_index != 0) {
            ...
            for (wtf_size_t line_index = start_line_index - 1; start_lines_[line_index].grid_line == range.start_line; --line_index) {
                if (start_lines_[line_index].grid_item_range_index_to_cache) {
                    *start_lines_[line_index].grid_item_range_index_to_cache = ranges_.size();
                }
        ...
        for (wtf_size_t line_index = end_line_index; line_index < end_lines_.size() && end_lines_[line_index].grid_line == end_line; ++line_index) {
...
NGGridBlockTrackCollection::NGGridBlockTrackCollection(
    const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, wtf_size_t auto_repetitions)
...
wtf_size_t NGGridBlockTrackCollection::RangeStartLine(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].start_line;
}

wtf_size_t NGGridBlockTrackCollection::RangeTrackCount(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
...
NGGridSet::NGGridSet(wtf_size_t track_count, const GridTrackSize& track_definition, bool is_available_size_indefinite)
    : track_count(track_count)
    ...
    , fit_content_limit(kIndefiniteSize)
{
    if (track_size.IsFitContent()) {
        ...
        if (is_available_size_indefinite && track_size.FitContentTrackBreadth().length().IsPercent()) {
        ...
        bool is_unresolvable_percentage_min_function = is_available_size_indefinite && track_size.MinTrackBreadth().HasPercentage();

        GridLength normalized_min_track_sizing_function
            = (is_unresolvable_percentage_min_function || track_size.HasFlexMinTrackBreadth()) ? Length::Auto() : track_size.MinTrackBreadth();

        bool is_unresolvable_percentage_max_function = is_available_size_indefinite && track_size.MaxTrackBreadth().HasPercentage();

        GridLength normalized_max_track_sizing_function
            = (is_unresolvable_percentage_max_function || track_size.HasAutoMaxTrackBreadth()) ? Length::Auto() : track_size.MaxTrackBreadth();
...
NGGridLayoutTrackCollection::NGGridLayoutTrackCollection(
    const NGGridLayoutTrackCollection& other, const NGBoxStrut& subgrid_border_scrollbar_padding, const NGBoxStrut& subgrid_margins)
    : NGGridLayoutTrackCollection(other)
{
    const bool is_for_columns = Direction() == kForColumns;

    sets_geometry_start_offset_ += is_for_columns ? subgrid_margins.inline_start : subgrid_margins.block_start;
    start_extra_margin_
        = sets_geometry_start_offset_ + (is_for_columns ? subgrid_border_scrollbar_padding.inline_start : subgrid_border_scrollbar_padding.block_start);

    end_extra_margin_ += is_for_columns ? subgrid_margins.inline_end + subgrid_border_scrollbar_padding.inline_end
                                        : subgrid_margins.block_end + subgrid_border_scrollbar_padding.block_end;
}

bool NGGridLayoutTrackCollection::operator==(const NGGridLayoutTrackCollection& other) const
{
    return gutter_size_ == other.gutter_size_ && sets_geometry_start_offset_ == other.sets_geometry_start_offset_
        && start_extra_margin_ == other.start_extra_margin_ && end_extra_margin_ == other.end_extra_margin_ && ranges_ == other.ranges_
        && major_baselines_ == other.major_baselines_ && minor_baselines_ == other.minor_baselines_ && sets_geometry_ == other.sets_geometry_;
}

wtf_size_t NGGridLayoutTrackCollection::RangeStartLine(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].start_line;
}

wtf_size_t NGGridLayoutTrackCollection::RangeTrackCount(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].track_count;
}

wtf_size_t NGGridLayoutTrackCollection::RangeSetCount(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].set_count;
}

wtf_size_t NGGridLayoutTrackCollection::RangeBeginSetIndex(wtf_size_t range_index) const
{
    DCHECK_LT(range_index, ranges_.size());
    return ranges_[range_index].begin_set_index;
}

bool NGGridLayoutTrackCollection::RangeHasTrackSpanProperty(wtf_size_t range_index, TrackSpanProperties::PropertyId property_id) const
{
    DCHECK_LT(range_index, ranges_.size());
...
bool NGGridLayoutTrackCollection::IsGridLineWithinImplicitGrid(wtf_size_t grid_line) const
{
...
LayoutUnit NGGridLayoutTrackCollection::GetSetOffset(wtf_size_t set_index) const
{
    ...
    return sets_geometry_[set_index].offset + extra_margin - sets_geometry_start_offset_;
}

wtf_size_t NGGridLayoutTrackCollection::GetSetTrackCount(wtf_size_t set_index) const
{
    DCHECK_LT(set_index + 1, sets_geometry_.size());
    return sets_geometry_[set_index + 1].track_count;
}

LayoutUnit NGGridLayoutTrackCollection::MajorBaseline(wtf_size_t set_index) const
{
    DCHECK_LT(set_index, major_baselines_.size());
    return major_baselines_[set_index];
}

LayoutUnit NGGridLayoutTrackCollection::MinorBaseline(wtf_size_t set_index) const
{
    DCHECK_LT(set_index, minor_baselines_.size());
    return minor_baselines_[set_index];
}

void NGGridLayoutTrackCollection::AdjustSetOffsets(wtf_size_t set_index, LayoutUnit delta)
{
    DCHECK_LT(set_index, sets_geometry_.size());
...
LayoutUnit NGGridLayoutTrackCollection::ComputeSetSpanSize(wtf_size_t begin_set_index, wtf_size_t end_set_index) const
{
...
NGGridLayoutTrackCollection NGGridLayoutTrackCollection::CreateSubgridCollection(
    wtf_size_t begin_range_index, wtf_size_t end_range_index, GridTrackSizingDirection subgrid_track_direction) const
{
    ...
    subgrid_collection.ranges_.ReserveInitialCapacity(end_range_index + 1 - begin_range_index);
    ...
    for (wtf_size_t i = begin_set_index + 1; i <= end_set_index; ++i) {
        subgrid_collection.sets_geometry_.emplace_back(sets_geometry_[i].offset - first_set_offset, sets_geometry_[i].track_count);
    }
    ...
    subgrid_collection.sets_geometry_start_offset_ = subgrid_collection.start_extra_margin_ = start_extra_margin_;
    subgrid_collection.end_extra_margin_ = end_extra_margin_;

    return subgrid_collection;
}

NGGridSizingTrackCollection::NGGridSizingTrackCollection(const NGGridBlockTrackCollection& block_track_collection, bool is_available_size_indefinite)
    ...
    , non_collapsed_track_count_(0)
{
    for (const auto& block_track_range : block_track_collection.Ranges()) {
        AppendTrackRange(block_track_range, block_track_range.IsImplicit() ? block_track_collection.ImplicitTracks() : block_track_collection.ExplicitTracks(),
...
const NGGridSet& NGGridSizingTrackCollection::GetSetAt(wtf_size_t set_index) const
{
    DCHECK_LT(set_index, sets_.size());
    return sets_[set_index];
}

NGGridSizingTrackCollection::SetIterator NGGridSizingTrackCollection::GetSetIterator()
{
    return SetIterator(this, 0, sets_.size());
}

NGGridSizingTrackCollection::ConstSetIterator NGGridSizingTrackCollection::GetConstSetIterator() const
{
    return ConstSetIterator(this, 0, sets_.size());
}

NGGridSizingTrackCollection::SetIterator NGGridSizingTrackCollection::GetSetIterator(wtf_size_t begin_set_index, wtf_size_t end_set_index)
{
    return SetIterator(this, begin_set_index, end_set_index);
}

bool NGGridSizingTrackCollection::IsSpanningIndefiniteSet(wtf_size_t begin_set_index, wtf_size_t end_set_index) const
{
    ...
    const wtf_size_t last_indefinite_index = last_indefinite_indices_[end_set_index];
...
void NGGridSizingTrackCollection::InitializeSetsGeometry(LayoutUnit first_set_offset, LayoutUnit gutter_size)
{
...
void NGGridSizingTrackCollection::CacheSetsGeometry(LayoutUnit first_set_offset, LayoutUnit gutter_size)
{
...
void NGGridSizingTrackCollection::SetMajorBaseline(wtf_size_t set_index, LayoutUnit candidate_baseline)
{
    DCHECK_LT(set_index, major_baselines_.size());
...
void NGGridSizingTrackCollection::SetMinorBaseline(wtf_size_t set_index, LayoutUnit candidate_baseline)
{
    DCHECK_LT(set_index, minor_baselines_.size());
...
void NGGridSizingTrackCollection::AppendTrackRange(
    const NGGridBlockTrackCollection::Range& block_track_range, const NGGridTrackList& specified_track_list, bool is_available_size_indefinite)
{
    Range new_range;
    ...
    if (block_track_range.repeater_index == kNotFound) {
        ...
        non_collapsed_track_count_ += new_range.track_count;
        wtf_size_t current_repeater_size = specified_track_list.RepeatSize(block_track_range.repeater_index);
        ...
        new_range.set_count = std::min(current_repeater_size, new_range.track_count);
        ...
        wtf_size_t floor_set_track_count = new_range.track_count / current_repeater_size;
        ...
        wtf_size_t remaining_track_count = new_range.track_count % current_repeater_size;

        for (wtf_size_t i = 0; i < new_range.set_count; ++i) {
            wtf_size_t set_track_count = floor_set_track_count + ((i < remaining_track_count) ? 1 : 0);
            wtf_size_t set_repeater_offset = (block_track_range.repeater_offset + i) % current_repeater_size;
            const GridTrackSize& set_track_size = specified_track_list.RepeatTrackSize(block_track_range.repeater_index, set_repeater_offset);
            sets_.emplace_back(set_track_count, set_track_size, is_available_size_indefinite);
            ...
            if (set_track_size.HasPercentage()) {
                new_range.properties.SetProperty(TrackSpanProperties::kIsDependentOnAvailableSize);
            }
        }
    }
    ...
    for (wtf_size_t i = 0; i < new_range.set_count; ++i) {
        const auto& set_track_size = sets_[new_range.begin_set_index + i].track_size;
        ...
        if (set_track_size.HasFixedMinTrackBreadth()) {
            new_range.properties.SetProperty(TrackSpanProperties::kHasFixedMinimumTrack);
        }
        if (set_track_size.HasFixedMaxTrackBreadth()) {
            new_range.properties.SetProperty(TrackSpanProperties::kHasFixedMaximumTrack);
        }
        ...
        if (!set_track_size.HasFixedMinTrackBreadth() || !set_track_size.HasFixedMaxTrackBreadth()
            || (set_track_size.MinTrackBreadth().length() != set_track_size.MaxTrackBreadth().length())) {
            new_range.properties.SetProperty(TrackSpanProperties::kHasNonDefiniteTrack);
        }
    }
