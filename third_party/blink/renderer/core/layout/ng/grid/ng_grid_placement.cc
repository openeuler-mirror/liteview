...
NGGridPlacement::NGGridPlacement(const ComputedStyle& grid_style, const NGGridPlacementData& placement_data)
    ...
    ,
#endif
    column_auto_repeat_track_count_(grid_style.GridTemplateColumns().TrackList().AutoRepeatTrackCount() * placement_data.column_auto_repetitions)
    , row_auto_repeat_track_count_(grid_style.GridTemplateRows().TrackList().AutoRepeatTrackCount() * placement_data.row_auto_repetitions)
{
}

// https://drafts.csswg.org/css-grid/#auto-placement-algo
NGGridPlacementData NGGridPlacement::RunAutoPlacementAlgorithm(const GridItems& grid_items)
{
#if DCHECK_IS_ON()
...
bool NGGridPlacement::PlaceNonAutoGridItems(const GridItems& grid_items, PlacedGridItemsList* placed_items, PositionVector* positions_locked_to_major_axis,
    PositionVector* positions_not_locked_to_major_axis)
{
    ...
    for (const auto& grid_item : grid_items) {
        ...
        GridArea position;
        position.columns = placement_data_.line_resolver.ResolveGridPositionsFromStyle(
            item_style, kForColumns, column_auto_repeat_track_count_, placement_data_.IsSubgriddedToParent(), placement_data_.subgridded_column_span_size);
        DCHECK(!position.columns.IsTranslatedDefinite());

        position.rows = placement_data_.line_resolver.ResolveGridPositionsFromStyle(
            item_style, kForRows, row_auto_repeat_track_count_, placement_data_.IsSubgriddedToParent(), placement_data_.subgridded_row_span_size);
    ...
    placement_data_.explicit_grid_column_count
        = placement_data_.line_resolver.ExplicitGridColumnCount(column_auto_repeat_track_count_, placement_data_.subgridded_column_span_size);

    placement_data_.explicit_grid_row_count
        = placement_data_.line_resolver.ExplicitGridRowCount(row_auto_repeat_track_count_, placement_data_.subgridded_row_span_size);

    minor_max_end_line_ = (minor_direction_ == kForColumns) ? placement_data_.column_start_offset + placement_data_.explicit_grid_column_count
                                                            : placement_data_.row_start_offset + placement_data_.explicit_grid_row_count;
...
void NGGridPlacement::PlaceGridItemsLockedToMajorAxis(const PositionVector& positions_locked_to_major_axis, PlacedGridItemsList* placed_items)
{
    DCHECK(placed_items);
    ...
    HashMap<wtf_size_t, wtf_size_t, DefaultHash<wtf_size_t>::Hash, WTF::UnsignedWithZeroKeyHashTraits<wtf_size_t>> minor_cursors;
...
void NGGridPlacement::PlaceAutoMajorAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
...
void NGGridPlacement::PlaceAutoBothAxisGridItem(GridArea* position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
    DCHECK(position && placed_items && placement_cursor);
...
void NGGridPlacement::PlaceGridItemAtCursor(const GridArea& position, PlacedGridItemsList* placed_items, AutoPlacementCursor* placement_cursor) const
{
...
void NGGridPlacement::ClampGridItemsToFitSubgridArea(GridTrackSizingDirection track_direction)
{
...
wtf_size_t NGGridPlacement::AutoRepeatTrackCount(GridTrackSizingDirection track_direction) const
{
    return (track_direction == kForColumns) ? column_auto_repeat_track_count_ : row_auto_repeat_track_count_;
}

wtf_size_t NGGridPlacement::AutoRepetitions(GridTrackSizingDirection track_direction) const
{
    return (track_direction == kForColumns) ? placement_data_.column_auto_repetitions : placement_data_.row_auto_repetitions;
}

wtf_size_t NGGridPlacement::StartOffset(GridTrackSizingDirection track_direction) const
{
    return (track_direction == kForColumns) ? placement_data_.column_start_offset : placement_data_.row_start_offset;
}

wtf_size_t NGGridPlacement::SubgridSpanSize(GridTrackSizingDirection track_direction) const
{
    return (track_direction == kForColumns) ? placement_data_.subgridded_column_span_size : placement_data_.subgridded_row_span_size;
}
...
NGGridPlacement::PlacedGridItem::PlacedGridItem(const GridArea& position, GridTrackSizingDirection major_direction, GridTrackSizingDirection minor_direction)
...
NGGridPlacement::GridPosition NGGridPlacement::PlacedGridItem::EndOnPreviousMajorLine() const
{
...
void NGGridPlacement::AutoPlacementCursor::MoveToMajorLine(const wtf_size_t major_line)
{
    DCHECK_LE(current_position_.major_line, major_line);
    current_position_.major_line = major_line;
}

void NGGridPlacement::AutoPlacementCursor::MoveToMinorLine(const wtf_size_t minor_line)
{
...
void NGGridPlacement::AutoPlacementCursor::MoveToNextMajorLine(bool allow_minor_line_movement)
{
    ++current_position_.major_line;
...
void NGGridPlacement::AutoPlacementCursor::InsertPlacedItemAtCurrentPosition(const PlacedGridItem* new_placed_item)
{
...
// static
void NGGridPlacement::ResolveOutOfFlowItemGridLines(const NGGridLayoutTrackCollection& track_collection, const NGGridPlacementData& placement_data,
    const ComputedStyle& grid_style, const ComputedStyle& item_style, wtf_size_t* start_line, wtf_size_t* end_line)
{
    ...
    *end_line = kNotFound;
    ...
    const wtf_size_t auto_repeat_track_count = is_for_columns
        ? grid_style.GridTemplateColumns().TrackList().AutoRepeatTrackCount() * placement_data.column_auto_repetitions
        : grid_style.GridTemplateRows().TrackList().AutoRepeatTrackCount() * placement_data.row_auto_repetitions;

    const auto span = placement_data.line_resolver.ResolveGridPositionsFromStyle(item_style, track_direction, auto_repeat_track_count,
        placement_data.IsSubgriddedToParent(), is_for_columns ? placement_data.subgridded_column_span_size : placement_data.subgridded_row_span_size);

    if (span.IsIndefinite())
        return;

    const wtf_size_t start_offset = is_for_columns ? placement_data.column_start_offset : placement_data.row_start_offset;
