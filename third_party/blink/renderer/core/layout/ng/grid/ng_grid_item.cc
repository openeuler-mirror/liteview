...
AxisEdge AxisEdgeFromItemPosition(bool is_inline_axis, bool is_replaced, bool is_out_of_flow, const ComputedStyle& item_style,
    const ComputedStyle& root_grid_style, NGAutoBehavior* auto_behavior, bool* is_overflow_safe)
{
    DCHECK(auto_behavior && is_overflow_safe);

    const auto& alignment = is_inline_axis ? item_style.ResolvedJustifySelf(ItemPosition::kNormal, &root_grid_style)
                                           : item_style.ResolvedAlignSelf(ItemPosition::kNormal, &root_grid_style);
    ...
    *is_overflow_safe = alignment.Overflow() == OverflowAlignment::kSafe;
    ...
    if (item_style.MayHaveMargin() && !is_out_of_flow) {
        const bool is_start_auto
            = is_inline_axis ? item_style.MarginStartUsing(root_grid_style).IsAuto() : item_style.MarginBeforeUsing(root_grid_style).IsAuto();
        const bool is_end_auto = is_inline_axis ? item_style.MarginEndUsing(root_grid_style).IsAuto() : item_style.MarginAfterUsing(root_grid_style).IsAuto();
    ...
    case ItemPosition::kSelfEnd: {
        ...
        if (is_inline_axis) {
            return item_position == ItemPosition::kSelfStart ? logical.InlineStart() : logical.InlineEnd();
        }
        return item_position == ItemPosition::kSelfStart ? logical.BlockStart() : logical.BlockEnd();
    }
    ...
    case ItemPosition::kNormal:
        *auto_behavior = is_replaced ? NGAutoBehavior::kFitContent : NGAutoBehavior::kStretchImplicit;
        return AxisEdge::kStart;
...
GridItemData::GridItemData(const NGBlockNode node, const ComputedStyle& root_grid_style, bool parent_must_consider_grid_items_for_column_sizing,
    ...
    , must_consider_grid_items_for_row_sizing(false)
{
    ...
    inline_axis_alignment = AxisEdgeFromItemPosition(
        /* is_inline_axis */ true, is_replaced, is_out_of_flow, style, root_grid_style, &inline_auto_behavior, &is_overflow_safe);
    is_inline_axis_overflow_safe = is_overflow_safe;

    block_axis_alignment = AxisEdgeFromItemPosition(
        /* is_inline_axis */ false, is_replaced, is_out_of_flow, style, root_grid_style, &block_auto_behavior, &is_overflow_safe);
    ...
    if (parent_must_consider_grid_items_for_column_sizing) {
        is_considered_for_column_sizing = is_parallel_with_root_grid ? !has_subgridded_columns : !has_subgridded_rows;
        must_consider_grid_items_for_column_sizing = !is_considered_for_column_sizing;
    }

    if (parent_must_consider_grid_items_for_row_sizing) {
        is_considered_for_row_sizing = is_parallel_with_root_grid ? !has_subgridded_rows : !has_subgridded_columns;
...
void GridItemData::SetAlignmentFallback(GridTrackSizingDirection track_direction, bool has_synthesized_baseline)
{
    ...
    auto CanParticipateInBaselineAlignment = [&]() -> bool {
        ...
        if (has_synthesized_baseline && (IsSpanningIntrinsicTrack(track_direction) || IsSpanningFlexibleTrack(track_direction))) {
            ...
            if (is_parallel_to_baseline_axis) {
                return !item_style.LogicalHeight().IsPercentOrCalcOrStretch() && !item_style.LogicalMinHeight().IsPercentOrCalcOrStretch()
                    && !item_style.LogicalMaxHeight().IsPercentOrCalcOrStretch();
            } else {
                return !item_style.LogicalWidth().IsPercentOrCalcOrStretch() && !item_style.LogicalMinWidth().IsPercentOrCalcOrStretch()
    ...
    if (!CanParticipateInBaselineAlignment()) {
        ...
        if (track_direction == kForColumns) {
            inline_axis_alignment_fallback = baseline_group == BaselineGroup::kMajor ? AxisEdge::kStart : AxisEdge::kEnd;
            is_inline_axis_overflow_safe_fallback = true;
        } else {
            block_axis_alignment_fallback = baseline_group == BaselineGroup::kMajor ? AxisEdge::kStart : AxisEdge::kEnd;
...
void GridItemData::ComputeSetIndices(const NGGridLayoutTrackCollection& track_collection)
{
    DCHECK(!IsOutOfFlow());
...
void GridItemData::ComputeOutOfFlowItemPlacement(
    const NGGridLayoutTrackCollection& track_collection, const NGGridPlacementData& placement_data, const ComputedStyle& grid_style)
{
    DCHECK(IsOutOfFlow());

    const bool is_for_columns = track_collection.Direction() == kForColumns;

    auto& start_offset = is_for_columns ? column_placement.offset_in_range.begin : row_placement.offset_in_range.begin;
    auto& end_offset = is_for_columns ? column_placement.offset_in_range.end : row_placement.offset_in_range.end;

    if (IsGridContainingBlock()) {
        NGGridPlacement::ResolveOutOfFlowItemGridLines(track_collection, placement_data, grid_style, node.Style(), &start_offset, &end_offset);
    } else {
    ...
    if (start_offset != kNotFound) {
        if (!range_count) {
            ...
            start_range_index
                = (start_offset < track_collection.EndLineOfImplicitGrid()) ? track_collection.RangeIndexFromGridLine(start_offset) : range_count - 1;
    ...
    if (end_offset != kNotFound) {
        if (!range_count) {
            ...
            end_range_index = end_offset ? track_collection.RangeIndexFromGridLine(end_offset - 1) : 0;
