...
GridItems NGGridNode::ConstructGridItems(const NGGridPlacementData& placement_data, HeapVector<Member<LayoutBox>>* oof_children, bool* has_nested_subgrid) const
{
    DCHECK(has_nested_subgrid);

    return ConstructGridItems(
        placement_data, Style(), placement_data.HasStandaloneAxis(kForColumns), placement_data.HasStandaloneAxis(kForRows), oof_children, has_nested_subgrid);
}

GridItems NGGridNode::ConstructGridItems(const NGGridPlacementData& placement_data, const ComputedStyle& root_grid_style,
    bool must_consider_grid_items_for_column_sizing, bool must_consider_grid_items_for_row_sizing, HeapVector<Member<LayoutBox>>* oof_children,
    bool* has_nested_subgrid) const
{
    ...
    if (layout_grid->HasCachedPlacementData()) {
        ...
        if (placement_data.column_auto_repetitions != cached_placement_data->column_auto_repetitions
            || placement_data.row_auto_repetitions != cached_placement_data->row_auto_repetitions) {
...

    {
        ...
        for (auto child = FirstChild(); child; child = child.NextSibling()) {
            ...
            auto* grid_item = MakeGarbageCollected<GridItemData>(
                To<NGBlockNode>(child), root_grid_style, must_consider_grid_items_for_column_sizing, must_consider_grid_items_for_row_sizing);
        ...
        if (should_sort_grid_items_by_order_property) {
            ...
            auto CompareItemsByOrderProperty
                = [](const Member<GridItemData>& lhs, const Member<GridItemData>& rhs) { return lhs->node.Style().Order() < rhs->node.Style().Order(); };
            std::stable_sort(grid_items.item_data.begin(), grid_items.item_data.end(), CompareItemsByOrderProperty);
        }
    }

    const auto& grid_style = Style();
    const bool grid_is_parallel_with_root_grid = IsParallelWritingMode(root_grid_style.GetWritingMode(), grid_style.GetWritingMode());
    ...
    for (auto& grid_item : grid_items) {
        ...
        if (!grid_is_parallel_with_root_grid) {
            std::swap(grid_item.resolved_position.columns, grid_item.resolved_position.rows);
        }
    }
    return grid_items;
}
...
void NGGridNode::AppendSubgriddedItems(GridItems* grid_items) const
{
    ...
    for (wtf_size_t i = 0; i < grid_items->Size(); ++i) {
        ...
        auto subgridded_items = subgrid.ConstructGridItems(subgrid.CachedPlacementData(), root_grid_style,
            current_item.must_consider_grid_items_for_column_sizing, current_item.must_consider_grid_items_for_row_sizing);
