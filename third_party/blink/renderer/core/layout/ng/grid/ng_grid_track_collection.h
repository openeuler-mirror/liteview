...
class CORE_EXPORT NGGridBlockTrackCollection : public NGGridTrackCollectionBase {
public:
    ...
    NGGridBlockTrackCollection(const ComputedStyle& grid_style, const NGGridPlacementData& placement_data, GridTrackSizingDirection track_direction);
...
private:
    ...
    NGGridBlockTrackCollection(const NGGridTrackList& explicit_tracks, const NGGridTrackList& implicit_tracks, wtf_size_t auto_repetitions);
...
class CORE_EXPORT NGGridLayoutTrackCollection : public NGGridTrackCollectionBase {
...
public:
    ...
    bool RangeHasTrackSpanProperty(wtf_size_t range_index, TrackSpanProperties::PropertyId property_id) const;
    ...
    virtual bool IsSpanningIndefiniteSet(wtf_size_t begin_set_index, wtf_size_t end_set_index) const
    {
        return false;
    }
...
struct CORE_EXPORT NGGridSet {
    ...
    NGGridSet(wtf_size_t track_count, const GridTrackSize& track_definition, bool is_available_size_indefinite);
...
class CORE_EXPORT NGGridSizingTrackCollection : public NGGridLayoutTrackCollection {
    USING_FAST_MALLOC(NGGridSizingTrackCollection);

public:
    template <bool is_const> class CORE_EXPORT SetIteratorBase {
    public:
        using TrackCollectionPtr = typename std::conditional<is_const, const NGGridSizingTrackCollection*, NGGridSizingTrackCollection*>::type;
        using NGGridSetRef = typename std::conditional<is_const, const NGGridSet&, NGGridSet&>::type;
    ...
    NGGridSizingTrackCollection(const NGGridBlockTrackCollection& block_track_collection, bool is_available_size_indefinite);

    NGGridSizingTrackCollection(const NGGridSizingTrackCollection&) = delete;
    NGGridSizingTrackCollection& operator=(const NGGridSizingTrackCollection&) = delete;
    ...
    bool IsSpanningIndefiniteSet(wtf_size_t begin_set_index, wtf_size_t end_set_index) const override;
    ...
    void InitializeSetsGeometry(LayoutUnit first_set_offset, LayoutUnit gutter_size);
    ...
    void SetGutterSize(LayoutUnit gutter_size)
    {
...
template <> struct DowncastTraits<NGGridSizingTrackCollection> {
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGGridLayoutTrackCollection::Range)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::NGGridLayoutTrackCollection::SetGeometry)
