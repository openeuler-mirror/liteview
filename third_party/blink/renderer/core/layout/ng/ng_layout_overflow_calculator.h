...
public:
    static PhysicalRect RecalculateLayoutOverflowForFragment(const NGPhysicalBoxFragment&);

    NGLayoutOverflowCalculator(const NGBlockNode&, bool is_css_box, const NGPhysicalBoxStrut& borders, const NGPhysicalBoxStrut& scrollbar,
        const NGPhysicalBoxStrut& padding, PhysicalSize size, WritingDirectionMode);
    ...
    void AddChild(const NGPhysicalBoxFragment& child_fragment, PhysicalOffset offset)
    {
    ...
    void AddItems(const LayoutObject*, const NGFragmentItemsBuilder::ItemWithOffsetList&);
...
private:
    ...
    PhysicalRect LayoutOverflowForPropagation(const NGPhysicalBoxFragment& child_fragment);
