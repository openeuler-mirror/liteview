...
void NGBoxFragmentBuilder::AddBreakBeforeChild(NGLayoutInputNode child, absl::optional<NGBreakAppeal> appeal, bool is_forced_break)
{
        ...
        if (!last_inline_break_token_) {
            ...
            if (previous_break_token_) {
                ...
                if (child_tokens.size()) {
                    ...
                    // child.
                    last_inline_break_token_ = DynamicTo<NGInlineBreakToken>(child_tokens.back().Get());
                    if (last_inline_break_token_)
                        return;
                }
            }
            ...
            last_inline_break_token_ = NGInlineBreakToken::Create(*child_inline_node, /* style */ nullptr, /* item_index */ 0,
...
void NGBoxFragmentBuilder::AddResult(const NGLayoutResult& child_layout_result, const LogicalOffset offset, absl::optional<LogicalOffset> relative_offset,
    const NGInlineContainer<LogicalOffset>* inline_container, EBreakBetween* flex_column_break_after)
{
    ...
    if (!fragment.IsBox() && items_builder_) {
        if (const NGPhysicalLineBoxFragment* line = DynamicTo<NGPhysicalLineBoxFragment>(&fragment)) {
            if (UNLIKELY(line->IsBlockInInline() && has_block_fragmentation_)) {
                ...
                const NGLogicalLineItems& line_items = items_builder_->LogicalLineItems(*line);
    ...
    AddChild(fragment, offset, &end_margin_strut, child_layout_result.IsSelfCollapsing(), relative_offset, inline_container, adjustment_for_oof_propagation);
...
void NGBoxFragmentBuilder::AddChild(const NGPhysicalFragment& child, const LogicalOffset& child_offset, const NGMarginStrut* margin_strut,
    bool is_self_collapsing, absl::optional<LogicalOffset> relative_offset, const NGInlineContainer<LogicalOffset>* inline_container,
    absl::optional<LayoutUnit> adjustment_for_oof_propagation)
{
#if DCHECK_IS_ON()
    ...
    DCHECK(!disable_oof_descendants_propagation_ || !adjustment_for_oof_propagation);
#endif
    ...
    PropagateChildData(child, child_offset, *relative_offset, inline_container, adjustment_for_oof_propagation);
    AddChildInternal(&child, child_offset + *relative_offset);
}

void NGBoxFragmentBuilder::AddBreakToken(const NGBreakToken* token, bool is_in_parallel_flow)
{
    // If there's a pre-set break token, we shouldn't be here.
    DCHECK(!break_token_);
...
void NGBoxFragmentBuilder::AddOutOfFlowLegacyCandidate(NGBlockNode node, const NGLogicalStaticPosition& static_position, const LayoutInline* inline_container)
{
    if (inline_container)
...
void NGBoxFragmentBuilder::RemoveOldLegacyOOFFlexItem(const LayoutObject& object)
{
...
void NGBoxFragmentBuilder::PropagateSpaceShortage(absl::optional<LayoutUnit> space_shortage)
{
...
EBreakBetween NGBoxFragmentBuilder::JoinedBreakBetweenValue(EBreakBetween break_before) const
{
...
void NGBoxFragmentBuilder::PropagateBreakInfo(const NGLayoutResult& child_layout_result, LogicalOffset offset)
{
    DCHECK(has_block_fragmentation_);
    ...
    const auto* child_box_fragment = DynamicTo<NGPhysicalBoxFragment>(&child_layout_result.PhysicalFragment());
    ...
    if (IsResumingLayout(token)) {
        ...
        NGBreakAppeal appeal_inside = CalculateBreakAppealInside(ConstraintSpace(), child_layout_result);
    ...
    if (UNLIKELY(ConstraintSpace().IsInColumnBfc())) {
        if (const NGColumnSpannerPath* child_spanner_path = child_layout_result.ColumnSpannerPath()) {
            DCHECK(HasInflowChildBreakInside() || !child_layout_result.PhysicalFragment().IsBox());
            const auto* spanner_path = MakeGarbageCollected<NGColumnSpannerPath>(Node(), child_spanner_path);
...
void NGBoxFragmentBuilder::PropagateChildBreakValues(const NGLayoutResult& child_layout_result, EBreakBetween* flex_column_break_after)
{
    if (child_layout_result.Status() != NGLayoutResult::kSuccess)
        return;

    const auto& fragment = child_layout_result.PhysicalFragment();
    if (fragment.IsInline() || !fragment.IsCSSBox() || fragment.IsFloatingOrOutOfFlowPositioned())
        return;
...
const NGLayoutResult* NGBoxFragmentBuilder::ToBoxFragment(WritingMode block_or_line_writing_mode)
{
#if DCHECK_IS_ON()
    if (ItemsBuilder()) {
    ...
    if (UNLIKELY(box_type_ == NGPhysicalFragment::kNormalBox && node_ && node_.IsBlockInInline()))
        SetIsBlockInInline();
    ...
    if (UNLIKELY(has_block_fragmentation_ && node_)) {
        ...
        if (!NGPhysicalFragment::IsFragmentainerBoxType(box_type_)) {
            ...
            if ((To<NGBlockNode>(node_).GetOverflowClipAxes() & block_axis) || is_block_size_for_fragmentation_clamped_) {
    ...
    if (!has_floating_descendants_for_paint_ && items_builder_) {
        has_floating_descendants_for_paint_ = items_builder_->HasFloatingDescendantsForPaint();
    }

    const NGPhysicalBoxFragment* fragment = NGPhysicalBoxFragment::Create(this, block_or_line_writing_mode);
    fragment->CheckType();

    return MakeGarbageCollected<NGLayoutResult>(NGLayoutResult::NGBoxFragmentBuilderPassKey(), std::move(fragment), this);
}

LogicalOffset NGBoxFragmentBuilder::GetChildOffset(const LayoutObject* object) const
{
    DCHECK(object);
    ...
    for (const auto& child : children_) {
        ...
        if (child.fragment->IsLineBox()) {
            const auto& line_box_fragment = To<NGPhysicalLineBoxFragment>(*child.fragment);
            for (const auto& line_box_child : line_box_fragment.Children()) {
                if (line_box_child->GetLayoutObject() == object) {
                    return child.offset + line_box_child.Offset().ConvertToLogical(GetWritingDirection(), line_box_fragment.Size(), line_box_child->Size());
                }
            }
        }
    }
...
void NGBoxFragmentBuilder::AdjustFragmentainerDescendant(NGLogicalOOFNodeForFragmentation& descendant, bool only_fixedpos_containing_block)
{
    ...
    if (!only_fixedpos_containing_block && !descendant.containing_block.Fragment()) {
        ...
        descendant.containing_block.SetRequiresContentBeforeBreaking(RequiresContentBeforeBreaking());
    }
    ...
    if (!descendant.fixedpos_containing_block.Fragment() && (node_.IsFixedContainer() || descendant.fixedpos_inline_container.container)) {
        ...
        descendant.fixedpos_containing_block.SetRequiresContentBeforeBreaking(RequiresContentBeforeBreaking());
    }
}

LayoutUnit NGBoxFragmentBuilder::BlockOffsetAdjustmentForFragmentainer(LayoutUnit fragmentainer_consumed_block_size) const
{
...
void NGBoxFragmentBuilder::AdjustFixedposContainingBlockForFragmentainerDescendants()
{
