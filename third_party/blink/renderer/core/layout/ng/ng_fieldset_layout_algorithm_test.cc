...
class NGFieldsetLayoutAlgorithmTest : public NGBaseLayoutAlgorithmTest, private ScopedLayoutNGBlockFragmentationForTest {
protected:
...
TEST_F(NGFieldsetLayoutAlgorithmTest, FieldsetPaddingWithLegend)
{
    ...
    const NGPhysicalBoxFragment* fragment = NGBaseLayoutAlgorithmTest::RunFieldsetLayoutAlgorithm(node, space);
...
TEST_F(NGFieldsetLayoutAlgorithmTest, SimpleFragmentation)
{
    ...
    fragment = NGBaseLayoutAlgorithmTest::RunFieldsetLayoutAlgorithm(node, space, fragment->BreakToken());
    EXPECT_EQ(PhysicalSize(176, 200), fragment->Size());
    ASSERT_TRUE(fragment->BreakToken());
...
TEST_F(NGFieldsetLayoutAlgorithmTest, LegendAndContentFragmentationNegativeMargin)
{
    SetBodyInnerHTML(R"HTML(
      <style>
        #fieldset {
...
TEST_F(NGFieldsetLayoutAlgorithmTest, LegendBreakInsideAvoid)
{
    ...
    const NGPhysicalBoxFragment* fragment = NGBaseLayoutAlgorithmTest::RunBlockLayoutAlgorithm(node, space);
    ASSERT_TRUE(fragment->BreakToken());
  ...
  offset:unplaced size:1000x100
    ...
    fragment = NGBaseLayoutAlgorithmTest::RunBlockLayoutAlgorithm(node, space, fragment->BreakToken());
    ASSERT_FALSE(fragment->BreakToken());
