...
public:
    ...
    // account.
    virtual MinMaxSizesResult ComputeMinMaxSizes(const MinMaxSizesFloatInput&) = 0;
};
...
public:
    NGLayoutAlgorithmParams(NGBlockNode node, const NGFragmentGeometry& fragment_geometry, const NGConstraintSpace& space,
...
template <typename NGInputNodeType, typename NGBoxFragmentBuilderType, typename NGBreakTokenType>
class CORE_EXPORT NGLayoutAlgorithm : public NGLayoutAlgorithmOperations {
    STACK_ALLOCATED();

public:
    NGLayoutAlgorithm(NGInputNodeType node, scoped_refptr<const ComputedStyle> style, const NGConstraintSpace& space, TextDirection direction,
        const NGBreakTokenType* break_token)
        : node_(node)
        , break_token_(break_token)
        , container_builder_(node, style, space, { space.GetWritingMode(), direction })
    {
    }
    ...
    explicit NGLayoutAlgorithm(const NGLayoutAlgorithmParams& params)
        ...
        , container_builder_(params.node, &params.node.Style(), params.space, { params.space.GetWritingMode(), params.space.Direction() })
        , additional_early_breaks_(params.additional_early_breaks)
    {
        ...
        if (UNLIKELY(params.space.HasBlockFragmentation() || IsResumingLayout(params.break_token))) {
...
protected:
    ...
    const NGInputNodeType& Node() const
    {
        return node_;
    }

    const NGBreakTokenType* BreakToken() const
    {
        return break_token_;
    }
    ...
    const NGLayoutResult* RelayoutAndBreakEarlier(const NGEarlyBreak& breakpoint, const HeapVector<Member<NGEarlyBreak>>* additional_early_breaks = nullptr)
    {
        ...
        NGLayoutAlgorithmParams params(
            Node(), container_builder_.InitialFragmentGeometry(), ConstraintSpace(), BreakToken(), &breakpoint, additional_early_breaks);
        Algorithm algorithm_with_break(params);
        return RelayoutAndBreakEarlier(&algorithm_with_break);
    }

    template <typename Algorithm> const NGLayoutResult* RelayoutAndBreakEarlier(Algorithm* new_algorithm)
    {
    ...
    template <typename Algorithm> const NGLayoutResult* RelayoutWithoutFragmentation()
    {
        ...
        NGLayoutAlgorithmParams params(Node(), container_builder_.InitialFragmentGeometry(), new_space, BreakToken());
