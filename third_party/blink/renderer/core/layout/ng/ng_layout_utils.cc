...
inline bool InlineLengthMayChange(const ComputedStyle& style, const Length& length, LengthResolveType type, const NGConstraintSpace& new_space,
    const NGConstraintSpace& old_space, const NGLayoutResult& layout_result)
{
    DCHECK_EQ(new_space.InlineAutoBehavior(), old_space.InlineAutoBehavior());

    bool is_unspecified = (length.IsAuto() && type != LengthResolveType::kMinSize) || length.IsFitContent() || length.IsFillAvailable();

    // Percentage inline margins will affect the size if the size is unspecified
    // (auto and similar).
    if (is_unspecified && style.MayHaveMargin() && (style.MarginStart().IsPercentOrCalc() || style.MarginEnd().IsPercentOrCalc())
...
inline bool BlockLengthMayChange(const Length& length, const NGConstraintSpace& new_space, const NGConstraintSpace& old_space)
{
    DCHECK_EQ(new_space.BlockAutoBehavior(), old_space.BlockAutoBehavior());
    if (length.IsFillAvailable() || (length.IsAuto() && new_space.IsBlockAutoBehaviorStretch())) {
...
bool BlockSizeMayChange(const NGBlockNode& node, const NGConstraintSpace& new_space, const NGConstraintSpace& old_space, const NGLayoutResult& layout_result)
{
...
bool SizeMayChange(const NGBlockNode& node, const NGConstraintSpace& new_space, const NGConstraintSpace& old_space, const NGLayoutResult& layout_result)
{
    ...
    if (style.MayHavePadding() && new_space.PercentageResolutionInlineSize() != old_space.PercentageResolutionInlineSize()) {
        ...
        if (style.PaddingTop().IsPercentOrCalc() || style.PaddingRight().IsPercentOrCalc() || style.PaddingBottom().IsPercentOrCalc()
...
NGLayoutCacheStatus CalculateSizeBasedLayoutCacheStatusWithGeometry(const NGBlockNode& node, const NGFragmentGeometry& fragment_geometry,
    const NGLayoutResult& layout_result, const NGConstraintSpace& new_space, const NGConstraintSpace& old_space)
{
    const ComputedStyle& style = node.Style();
    const NGPhysicalBoxFragment& physical_fragment = To<NGPhysicalBoxFragment>(layout_result.PhysicalFragment());
    ...
    if (is_initial_block_size_indefinite) {
        ...
        // custom-layout.
        if (old_space.IsFixedBlockSize() || (old_space.IsBlockAutoBehaviorStretch() && style.LogicalHeight().IsAuto())) {
        ...
        block_size = ComputeBlockSizeForFragment(
            new_space, style, fragment_geometry.border + fragment_geometry.padding, intrinsic_block_size, fragment_geometry.border_box_size.inline_size);
    ...
    DCHECK(!node.IsTable());
    return is_block_size_equal ? NGLayoutCacheStatus::kHit : NGLayoutCacheStatus::kNeedsSimplifiedLayout;
}

bool IntrinsicSizeWillChange(const NGBlockNode& node, const NGBlockBreakToken* break_token, const NGLayoutResult& cached_layout_result,
    const NGConstraintSpace& new_space, absl::optional<NGFragmentGeometry>* fragment_geometry)
{
    const ComputedStyle& style = node.Style();
    ...
    LayoutUnit inline_size = NGFragment(style.GetWritingDirection(), cached_layout_result.PhysicalFragment()).InlineSize();
...
NGLayoutCacheStatus CalculateSizeBasedLayoutCacheStatus(const NGBlockNode& node, const NGBlockBreakToken* break_token,
    const NGLayoutResult& cached_layout_result, const NGConstraintSpace& new_space, absl::optional<NGFragmentGeometry>* fragment_geometry)
{
...
bool MaySkipLegacyLayout(const NGBlockNode& node, const NGLayoutResult& cached_layout_result, const NGConstraintSpace& new_space)
{
    DCHECK_EQ(cached_layout_result.Status(), NGLayoutResult::kSuccess);
...
bool MaySkipLayoutWithinBlockFormattingContext(const NGLayoutResult& cached_layout_result, const NGConstraintSpace& new_space,
    absl::optional<LayoutUnit>* bfc_block_offset, LayoutUnit* block_offset_delta, NGMarginStrut* end_margin_strut)
{
    DCHECK_EQ(cached_layout_result.Status(), NGLayoutResult::kSuccess);
    ...
    bool is_margin_strut_equal = old_space.MarginStrut() == new_space.MarginStrut();
    ...
    if (is_pushed_by_floats) {
        ...
        // this check.
        if (old_clearance_offset - old_space.BfcOffset().block_offset > new_clearance_offset - new_space.BfcOffset().block_offset) {
            return false;
        }
    }
    ...
    const auto& physical_fragment = To<NGPhysicalBoxFragment>(cached_layout_result.PhysicalFragment());
    ...
    if (cached_layout_result.IsSelfCollapsing()) {
        ...
        if (physical_fragment.HasAdjoiningObjectDescendants()) {
            ...
            if (old_expected < old_space.ExclusionSpace().ClearanceOffset(EClear::kBoth))
                return false;

            // Check if the new position intersects with any floats.
            if (new_expected < new_space.ExclusionSpace().ClearanceOffset(EClear::kBoth))
                return false;
        }
    ...
    if (is_pushed_by_floats || ancestor_has_clearance_past_adjoining_floats) {
        ...
        *bfc_block_offset = new_clearance_offset;
        ...
        // block-offset amount.
        *block_offset_delta = new_space.BfcOffset().block_offset - old_space.BfcOffset().block_offset;
        *bfc_block_offset = **bfc_block_offset + *block_offset_delta;
        ...
        LayoutUnit old_bfc_block_offset = old_space.BfcOffset().block_offset + old_space.MarginStrut().Sum();
        DCHECK_EQ(old_bfc_block_offset, **bfc_block_offset);
#endif

        LayoutUnit new_bfc_block_offset = new_space.BfcOffset().block_offset + new_space.MarginStrut().Sum();
