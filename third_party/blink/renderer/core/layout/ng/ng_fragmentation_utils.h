...
template <typename Property> bool IsAvoidBreakValue(const NGConstraintSpace&, Property);
...
{
    return builder.ConstraintSpace().HasBlockFragmentation() || IsResumingLayout(builder.PreviousBreakToken());
}
...
// breakpoint.
EBreakBetween CalculateBreakBetweenValue(NGLayoutInputNode child, const NGLayoutResult&, const NGBoxFragmentBuilder&);
...
NGBreakAppeal CalculateBreakAppealBefore(
    const NGConstraintSpace&, NGLayoutInputNode child, const NGLayoutResult&, const NGBoxFragmentBuilder&, bool has_container_separation);
NGBreakAppeal CalculateBreakAppealBefore(const NGConstraintSpace&, NGLayoutResult::EStatus layout_result_status, EBreakBetween break_between,
...
NGBreakAppeal CalculateBreakAppealInside(
    const NGConstraintSpace& space, const NGLayoutResult&, absl::optional<NGBreakAppeal> hypothetical_appeal = absl::nullopt);
...
inline LayoutUnit FragmentainerSpaceLeft(const NGConstraintSpace& space)
{
    if (!space.HasKnownFragmentainerBlockSize())
        return kIndefiniteSize;
    LayoutUnit available_space = FragmentainerCapacity(space) - space.FragmentainerOffset();
...
inline LayoutUnit UnclampedFragmentainerSpaceLeft(const NGConstraintSpace& space)
{
...
// ignored.
inline void AdjustMarginsForFragmentation(const NGBlockBreakToken* break_token, NGBoxStrut* box_strut)
{
    if (!break_token)
        return;
...
LogicalOffset GetFragmentainerProgression(const NGBoxFragmentBuilder&, NGFragmentationType);
...
void SetupSpaceBuilderForFragmentation(const NGConstraintSpace& parent_space, const NGLayoutInputNode& child, LayoutUnit fragmentainer_offset_delta,
    NGConstraintSpaceBuilder*, bool is_new_fc, bool requires_content_before_breaking);

// Set up a node's fragment builder for block fragmentation. To be done at the
// beginning of layout.
void SetupFragmentBuilderForFragmentation(
    const NGConstraintSpace&, const NGLayoutInputNode&, const NGBlockBreakToken* previous_break_token, NGBoxFragmentBuilder*);
...
NGBreakStatus FinishFragmentation(NGBlockNode node, const NGConstraintSpace&, LayoutUnit trailing_border_padding, LayoutUnit space_left, NGBoxFragmentBuilder*);

// Special rules apply for finishing fragmentation when building fragmentainers.
NGBreakStatus FinishFragmentationForFragmentainer(const NGConstraintSpace&, NGBoxFragmentBuilder*);
...
// the column level.
NGBreakStatus BreakBeforeChildIfNeeded(const NGConstraintSpace&, NGLayoutInputNode child, const NGLayoutResult&, LayoutUnit fragmentainer_block_offset,
...
void BreakBeforeChild(const NGConstraintSpace&, NGLayoutInputNode child, const NGLayoutResult*, LayoutUnit fragmentainer_block_offset,
    absl::optional<NGBreakAppeal> appeal, bool is_forced_break, NGBoxFragmentBuilder*, absl::optional<LayoutUnit> block_size_override = absl::nullopt,
...
// possible.
inline void PropagateUnbreakableBlockSize(LayoutUnit block_size, LayoutUnit fragmentainer_block_offset, NGBoxFragmentBuilder* builder)
{
...
// layout result.
void PropagateSpaceShortage(const NGConstraintSpace&, const NGLayoutResult*, LayoutUnit fragmentainer_block_offset, NGBoxFragmentBuilder*,
...
LayoutUnit CalculateSpaceShortage(
    const NGConstraintSpace&, const NGLayoutResult*, LayoutUnit fragmentainer_block_offset, absl::optional<LayoutUnit> block_size_override = absl::nullopt);
// Update |minimal_space_shortage| based on the current |space_shortage|.
void UpdateMinimalSpaceShortage(absl::optional<LayoutUnit> space_shortage, LayoutUnit* minimal_space_shortage);
...
bool MovePastBreakpoint(const NGConstraintSpace& space, NGLayoutInputNode child, const NGLayoutResult& layout_result, LayoutUnit fragmentainer_block_offset,
    NGBreakAppeal appeal_before, NGBoxFragmentBuilder* builder, bool is_row_item = false, NGFlexColumnBreakInfo* flex_column_break_info = nullptr);
...
// details on |flex_column_break_info|.
void UpdateEarlyBreakAtBlockChild(const NGConstraintSpace&, NGBlockNode child, const NGLayoutResult&, NGBreakAppeal appeal_before, NGBoxFragmentBuilder*,
...
bool AttemptSoftBreak(const NGConstraintSpace&, NGLayoutInputNode child, const NGLayoutResult*, LayoutUnit fragmentainer_block_offset,
    NGBreakAppeal appeal_before, NGBoxFragmentBuilder*, absl::optional<LayoutUnit> block_size_override = absl::nullopt,
    NGFlexColumnBreakInfo* flex_column_break_info = nullptr);
...
const NGEarlyBreak* EnterEarlyBreakInChild(const NGBlockNode& child, const NGEarlyBreak&);

// Return true if this is the child that we had previously determined to break
// before.
bool IsEarlyBreakTarget(const NGEarlyBreak&, const NGBoxFragmentBuilder&, const NGLayoutInputNode& child);
...
inline const NGColumnSpannerPath* FollowColumnSpannerPath(const NGColumnSpannerPath* path, const NGBlockNode& child)
{
    if (!path)
        return nullptr;
...
NGBoxFragmentBuilder CreateContainerBuilderForMulticol(
    const NGBlockNode& multicol, const NGConstraintSpace& space, const NGFragmentGeometry& fragment_geometry);
...
inline LayoutUnit AdjustedMarginAfterFinalChildFragment(const NGConstraintSpace& space, LayoutUnit block_offset, LayoutUnit block_end_margin)
{
...
PhysicalOffset OffsetInStitchedFragments(const NGPhysicalBoxFragment&, PhysicalSize* out_stitched_fragments_size = nullptr);
...
LayoutUnit BlockSizeForFragmentation(const NGLayoutResult&, WritingDirectionMode container_writing_direction);
