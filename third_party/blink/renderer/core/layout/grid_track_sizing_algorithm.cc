...
{
    DCHECK(IsGrowthLimitBiggerThanBaseSize());
    DCHECK(!growth_limit_cap_ || growth_limit_cap_.value() >= growth_limit_ || base_size_ >= growth_limit_cap_.value());
...
{
    growth_limit_ = growth_limit == kInfinity ? growth_limit : std::min(growth_limit, growth_limit_cap_.value_or(growth_limit));
    EnsureGrowthLimitIsBiggerThanBaseSize();
}
...
void GridTrack::GrowSizeDuringDistribution(LayoutUnit size_during_distribution)
{
    DCHECK_GE(size_during_distribution, 0);
    size_during_distribution_ += size_during_distribution;
}
...
template <typename F>
static void IterateGridItemsInTrackIndices(const Grid& grid, GridTrackSizingDirection direction, Vector<wtf_size_t>& track_indices, F callback)
{
#if DCHECK_IS_ON()
...
private:
    void LayoutGridItemForMinSizeComputation(LayoutBox&, bool override_size_has_changed) const override;
    void MaximizeTracks(Vector<GridTrack>&, absl::optional<LayoutUnit>& free_space) override;
    double FindUsedFlexFraction(
        Vector<wtf_size_t>& flexible_sized_tracks_index, GridTrackSizingDirection, absl::optional<LayoutUnit> free_space) const override;
    bool RecomputeUsedFlexFractionIfNeeded(double& flex_fraction, Vector<LayoutUnit>& increments, LayoutUnit& total_growth) const override;
...
private:
    ...
    LayoutUnit FreeSpaceForStretchAutoTracksStep() const override;
    LayoutUnit MinContentForChild(LayoutBox&) const override;
    LayoutUnit MinLogicalSizeForChild(LayoutBox&, const Length& child_min_size, LayoutUnit available_size) const override;
    bool IsComputingSizeContainment() const override
    {
        return false;
    }
...
bool GridTrackSizingAlgorithmStrategy::HasRelativeMarginOrPaddingForChild(const LayoutGrid& grid, const LayoutBox& child, GridTrackSizingDirection direction)
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(grid, child, kForColumns);
    if (direction == child_inline_direction) {
        return child.StyleRef().MarginStart().IsPercentOrCalc() || child.StyleRef().MarginEnd().IsPercentOrCalc()
            || child.StyleRef().PaddingStart().IsPercentOrCalc() || child.StyleRef().PaddingEnd().IsPercentOrCalc();
    }
    return child.StyleRef().MarginBefore().IsPercentOrCalc() || child.StyleRef().MarginAfter().IsPercentOrCalc()
        || child.StyleRef().PaddingBefore().IsPercentOrCalc() || child.StyleRef().PaddingAfter().IsPercentOrCalc();
}

bool GridTrackSizingAlgorithmStrategy::HasRelativeOrIntrinsicSizeForChild(const LayoutGrid& grid, const LayoutBox& child, GridTrackSizingDirection direction)
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(grid, child, kForColumns);
    if (direction == child_inline_direction) {
        return child.HasRelativeLogicalWidth() || !child.StyleRef().LogicalWidth().IsSpecified();
    }
    return child.HasRelativeLogicalHeight() || !child.StyleRef().LogicalHeight().IsSpecified();
}

bool GridTrackSizingAlgorithmStrategy::ShouldClearOverrideContainingBlockContentSizeForChild(
    const LayoutGrid& grid, const LayoutBox& child, GridTrackSizingDirection direction)
{
    return HasRelativeOrIntrinsicSizeForChild(grid, child, direction) || HasRelativeMarginOrPaddingForChild(grid, child, direction);
}

void GridTrackSizingAlgorithmStrategy::SetOverrideContainingBlockContentSizeForChild(LayoutBox& child, GridTrackSizingDirection direction, LayoutUnit size)
{
...
LayoutSize GridTrackSizingAlgorithm::EstimatedGridAreaBreadthForChild(const LayoutBox& child) const
{
    return { EstimatedGridAreaBreadthForChild(child, kForColumns), EstimatedGridAreaBreadthForChild(child, kForRows) };
}

LayoutUnit GridTrackSizingAlgorithm::EstimatedGridAreaBreadthForChild(const LayoutBox& child, GridTrackSizingDirection direction) const
{
    ...
    for (auto track_position : span) {
        ...
        const GridTrackSize& track_size = WasSetup() ? CalculateGridTrackSize(direction, track_position) : RawGridTrackSize(direction, track_position);
        GridLength max_track_size = track_size.MaxTrackBreadth();
        if (max_track_size.IsContentSized() || max_track_size.IsFlex() || IsRelativeGridLengthAsAuto(max_track_size, direction)) {
            grid_area_is_indefinite = true;
        } else {
            grid_area_size += ValueForLength(max_track_size.length(), available_size.value_or(LayoutUnit()));
        }
    }

    grid_area_size += layout_grid_->GuttersSize(*grid_, direction, span.StartLine(), span.IntegerSpan(), available_size);

    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*layout_grid_, child, kForColumns);
    if (grid_area_is_indefinite) {
        return direction == child_inline_direction ? std::max(child.PreferredLogicalWidths().max_size, grid_area_size) : LayoutUnit(-1);
    }
    return grid_area_size;
}

LayoutUnit GridTrackSizingAlgorithm::GridAreaBreadthForChild(const LayoutBox& child, GridTrackSizingDirection direction) const
{
    bool add_content_alignment_offset = direction == kForColumns && sizing_state_ == kRowSizingFirstIteration;
    if (direction == kForRows && (sizing_state_ == kColumnSizingFirstIteration || sizing_state_ == kColumnSizingSecondIteration)) {
    ...
    if (add_content_alignment_offset) {
        grid_area_breadth += (span.IntegerSpan() - 1) * layout_grid_->GridItemOffset(direction);
    }

    grid_area_breadth += layout_grid_->GuttersSize(*grid_, direction, span.StartLine(), span.IntegerSpan(), AvailableSpace(direction));

    return grid_area_breadth;
}

bool GridTrackSizingAlgorithm::IsIntrinsicSizedGridArea(const LayoutBox& child, GridAxis axis) const
{
    DCHECK(WasSetup());
    GridTrackSizingDirection direction = GridDirectionForAxis(axis);
    const GridSpan& span = grid_->GridItemSpan(child, direction);
    for (auto track_position : span) {
        const GridTrackSize& track_size = RawGridTrackSize(direction, track_position);
        ...
        if (track_size.IsContentSized() || track_size.IsFitContent() || track_size.MinTrackBreadth().IsFlex()
...
bool GridTrackSizingAlgorithmStrategy::UpdateOverrideContainingBlockContentSizeForChild(
    LayoutBox& child, GridTrackSizingDirection direction, absl::optional<LayoutUnit> override_size) const
{
    if (!override_size)
        override_size = algorithm_->GridAreaBreadthForChild(child, direction);
    if (GridLayoutUtils::OverrideContainingBlockContentSizeForChild(child, direction) == override_size.value())
        return false;

    SetOverrideContainingBlockContentSizeForChild(child, direction, override_size.value());
    return true;
}

LayoutUnit GridTrackSizingAlgorithmStrategy::LogicalHeightForChild(LayoutBox& child) const
{
    GridTrackSizingDirection child_block_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForRows);
    ...
    // resolution).
    if (ShouldClearOverrideContainingBlockContentSizeForChild(*GetLayoutGrid(), child, child_block_direction)) {
        SetOverrideContainingBlockContentSizeForChild(child, child_block_direction, LayoutUnit(-1));
        child.SetSelfNeedsLayoutForAvailableSpace(true);
    }

    child.LayoutIfNeeded();

    return child.LogicalHeight() + GridLayoutUtils::MarginLogicalHeightForChild(*GetLayoutGrid(), child)
        + algorithm_->BaselineOffsetForChild(child, GridAxisForDirection(Direction()));
}

DISABLE_CFI_PERF
LayoutUnit GridTrackSizingAlgorithmStrategy::MinContentForChild(LayoutBox& child) const
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForColumns);
    if (Direction() == child_inline_direction) {
        ...
        return child.PreferredLogicalWidths().min_size + GridLayoutUtils::MarginLogicalWidthForChild(*GetLayoutGrid(), child)
            + algorithm_->BaselineOffsetForChild(child, GridAxisForDirection(Direction()));
    }

    if (UpdateOverrideContainingBlockContentSizeForChild(child, child_inline_direction)) {
        child.SetSelfNeedsLayoutForAvailableSpace(true);
    }
...
DISABLE_CFI_PERF
LayoutUnit GridTrackSizingAlgorithmStrategy::MaxContentForChild(LayoutBox& child) const
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForColumns);
    if (Direction() == child_inline_direction) {
        // FIXME: It's unclear if we should return the intrinsic width or the
        ...
        if (child.NeedsPreferredWidthsRecalculation())
            child.SetIntrinsicLogicalWidthsDirty();
        return child.PreferredLogicalWidths().max_size + GridLayoutUtils::MarginLogicalWidthForChild(*GetLayoutGrid(), child)
            + algorithm_->BaselineOffsetForChild(child, GridAxisForDirection(Direction()));
    }
...
LayoutUnit GridTrackSizingAlgorithmStrategy::MinSizeForChild(LayoutBox& child) const
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForColumns);
    bool is_row_axis = Direction() == child_inline_direction;
    const Length& child_size = is_row_axis ? child.StyleRef().LogicalWidth() : child.StyleRef().LogicalHeight();
    if (!child_size.IsAuto() && !child_size.IsPercentOrCalc())
        return MinContentForChild(child);

    const Length& child_min_size = is_row_axis ? child.StyleRef().LogicalMinWidth() : child.StyleRef().LogicalMinHeight();
    auto overflow = is_row_axis ? child.StyleRef().OverflowInlineDirection() : child.StyleRef().OverflowBlockDirection();
    bool overflow_allows_auto = overflow == EOverflow::kVisible || overflow == EOverflow::kClip;
    LayoutUnit baseline_shim = algorithm_->BaselineOffsetForChild(child, GridAxisForDirection(Direction()));

    if (child_min_size.IsAuto() && overflow_allows_auto) {
        LayoutUnit min_size = MinContentForChild(child);
        const GridSpan& span = algorithm_->GetGrid().GridItemSpan(child, Direction());
        LayoutUnit max_breadth;
        const Vector<GridTrack>& all_tracks = algorithm_->Tracks(Direction());
        for (auto track_position : span) {
            const GridTrackSize& track_size = all_tracks[track_position].CachedTrackSize();
            if (!track_size.HasFixedMaxTrackBreadth())
                return min_size;
            max_breadth += ValueForLength(track_size.MaxTrackBreadth().length(), AvailableSpace().value_or(LayoutUnit()));
        }
        if (min_size > max_breadth) {
            LayoutUnit margin_and_border_and_padding = is_row_axis
                ? GridLayoutUtils::MarginLogicalWidthForChild(*GetLayoutGrid(), child) + child.BorderAndPaddingLogicalWidth()
                : GridLayoutUtils::MarginLogicalHeightForChild(*GetLayoutGrid(), child) + child.BorderAndPaddingLogicalHeight();
            min_size = std::max(max_breadth, margin_and_border_and_padding + baseline_shim);
        }
        return min_size;
    }

    LayoutUnit grid_area_size = algorithm_->GridAreaBreadthForChild(child, child_inline_direction);
    return MinLogicalSizeForChild(child, child_min_size, grid_area_size) + baseline_shim;
}

bool GridTrackSizingAlgorithm::CanParticipateInBaselineAlignment(const LayoutBox& child, GridAxis baseline_axis) const
{
    DCHECK(baseline_axis == kGridColumnAxis ? column_baseline_items_map_.Contains(&child) : row_baseline_items_map_.Contains(&child));
    ...
    bool is_parallel_to_baseline_axis = baseline_axis == kGridColumnAxis ? !GridLayoutUtils::IsOrthogonalChild(*layout_grid_, child)
    ...
    return is_parallel_to_baseline_axis ? !child.HasRelativeLogicalHeight() : !child.HasRelativeLogicalWidth() && !child.StyleRef().LogicalWidth().IsAuto();
}

bool GridTrackSizingAlgorithm::ParticipateInBaselineAlignment(const LayoutBox& child, GridAxis baseline_axis) const
{
...
void GridTrackSizingAlgorithm::UpdateBaselineAlignmentContext(const LayoutBox& child, GridAxis baseline_axis)
{
    DCHECK(WasSetup());
    DCHECK(CanParticipateInBaselineAlignment(child, baseline_axis));
    DCHECK(!child.NeedsLayout());

    ItemPosition align = layout_grid_->SelfAlignmentForChild(baseline_axis, child).GetPosition();
    const auto& span = grid_->GridItemSpan(child, GridDirectionForAxis(baseline_axis));
    baseline_alignment_.UpdateBaselineAlignmentContext(align, span.StartLine(), child, baseline_axis);
}

LayoutUnit GridTrackSizingAlgorithm::BaselineOffsetForChild(const LayoutBox& child, GridAxis baseline_axis) const
{
    ...
    ItemPosition align = layout_grid_->SelfAlignmentForChild(baseline_axis, child).GetPosition();
    const auto& span = grid_->GridItemSpan(child, GridDirectionForAxis(baseline_axis));
    return baseline_alignment_.BaselineOffsetForChild(align, span.StartLine(), child, baseline_axis);
}
...
void GridTrackSizingAlgorithm::CacheBaselineAlignedItem(const LayoutBox& item, GridAxis axis)
{
...
double GridTrackSizingAlgorithmStrategy::FindFrUnitSize(const GridSpan& tracks_span, LayoutUnit left_over_space) const
{
    return algorithm_->FindFrUnitSize(tracks_span, left_over_space);
}

void GridTrackSizingAlgorithmStrategy::DistributeSpaceToTracks(Vector<GridTrack*>& tracks, LayoutUnit& available_logical_space) const
{
    algorithm_->DistributeSpaceToTracks<kMaximizeTracks>(tracks, nullptr, available_logical_space);
}

LayoutUnit GridTrackSizingAlgorithmStrategy::MinLogicalSizeForChild(LayoutBox& child, const Length& child_min_size, LayoutUnit available_size) const
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForColumns);
    bool is_row_axis = Direction() == child_inline_direction;

    if (is_row_axis) {
        return child.ComputeLogicalWidthUsing(kMinSize, child_min_size, available_size, GetLayoutGrid())
            + GridLayoutUtils::MarginLogicalWidthForChild(*GetLayoutGrid(), child);
    }

    bool override_size_has_changed = UpdateOverrideContainingBlockContentSizeForChild(child, child_inline_direction, available_size);
    LayoutGridItemForMinSizeComputation(child, override_size_has_changed);

    return child.ComputeLogicalHeightUsing(kMinSize, child_min_size, child.IntrinsicLogicalHeight())
        + GridLayoutUtils::MarginLogicalHeightForChild(*GetLayoutGrid(), child);
}

LayoutUnit DefiniteSizeStrategy::MinLogicalSizeForChild(LayoutBox& child, const Length& child_min_size, LayoutUnit available_size) const
{
    GridTrackSizingDirection child_inline_direction = GridLayoutUtils::FlowAwareDirectionForChild(*GetLayoutGrid(), child, kForColumns);
    LayoutUnit indefinite_size = Direction() == child_inline_direction ? LayoutUnit() : LayoutUnit(-1);
    if (HasRelativeMarginOrPaddingForChild(*GetLayoutGrid(), child, Direction())
        || (Direction() != child_inline_direction && HasRelativeOrIntrinsicSizeForChild(*GetLayoutGrid(), child, Direction()))) {
        SetOverrideContainingBlockContentSizeForChild(child, Direction(), indefinite_size);
    }
    return GridTrackSizingAlgorithmStrategy::MinLogicalSizeForChild(child, child_min_size, available_size);
}

void DefiniteSizeStrategy::LayoutGridItemForMinSizeComputation(LayoutBox& child, bool override_size_has_changed) const
{
...
void DefiniteSizeStrategy::MaximizeTracks(Vector<GridTrack>& tracks, absl::optional<LayoutUnit>& free_space)
{
    ...
    for (wtf_size_t i = 0; i < tracks_size; ++i) {
        ...
        tracks_for_distribution[i]->SetPlannedSize(tracks_for_distribution[i]->BaseSize());
    }
...
double DefiniteSizeStrategy::FindUsedFlexFraction(
    Vector<wtf_size_t>& flexible_sized_tracks_index, GridTrackSizingDirection direction, absl::optional<LayoutUnit> free_space) const
{
    GridSpan all_tracks_span = GridSpan::TranslatedDefiniteGridSpan(0, algorithm_->Tracks(direction).size());
    DCHECK(free_space);
...
LayoutUnit DefiniteSizeStrategy::MinContentForChild(LayoutBox& child) const
{
    ...
    if (Direction() == child_inline_direction && child.NeedsLayout()
        && ShouldClearOverrideContainingBlockContentSizeForChild(*GetLayoutGrid(), child, child_inline_direction)) {
        SetOverrideContainingBlockContentSizeForChild(child, child_inline_direction, LayoutUnit());
    }

    return GridTrackSizingAlgorithmStrategy::MinContentForChild(child);
}

void IndefiniteSizeStrategy::LayoutGridItemForMinSizeComputation(LayoutBox& child, bool override_size_has_changed) const
{
...
void IndefiniteSizeStrategy::MaximizeTracks(Vector<GridTrack>& tracks, absl::optional<LayoutUnit>&)
{
...
double IndefiniteSizeStrategy::FindUsedFlexFraction(
    Vector<wtf_size_t>& flexible_sized_tracks_index, GridTrackSizingDirection direction, absl::optional<LayoutUnit>) const
{
    auto all_tracks = algorithm_->Tracks(direction);

    double flex_fraction = 0;
    for (const auto& track_index : flexible_sized_tracks_index) {
        flex_fraction = std::max(flex_fraction, NormalizedFlexFraction(all_tracks[track_index]));
    }
    ...
    IterateGridItemsInTrackIndices(grid, direction, flexible_sized_tracks_index, [&](LayoutBox* grid_item, const GridSpan& span) {
        // Removing gutters from the max-content contribution of the item,
        // so they are not taken into account in FindFrUnitSize().
        LayoutUnit left_over_space = MaxContentForChild(*grid_item)
            - GetLayoutGrid()->GuttersSize(algorithm_->GetGrid(), direction, span.StartLine(), span.IntegerSpan(), AvailableSpace());
        flex_fraction = std::max(flex_fraction, FindFrUnitSize(span, left_over_space));
    });

    return flex_fraction;
}

bool IndefiniteSizeStrategy::RecomputeUsedFlexFractionIfNeeded(double& flex_fraction, Vector<LayoutUnit>& increments, LayoutUnit& total_growth) const
{
    if (Direction() == kForColumns)
        return false;

    const LayoutGrid* layout_grid = GetLayoutGrid();
    LayoutUnit min_size = layout_grid->ComputeContentLogicalHeight(kMinSize, layout_grid->StyleRef().LogicalMinHeight(), LayoutUnit(-1));
    LayoutUnit max_size = layout_grid->ComputeContentLogicalHeight(kMaxSize, layout_grid->StyleRef().LogicalMaxHeight(), LayoutUnit(-1));
    ...
    const Grid& grid = algorithm_->GetGrid();
    free_space = std::max(free_space, min_size) - layout_grid->GuttersSize(grid, kForRows, 0, grid.NumTracks(kForRows), AvailableSpace());

    wtf_size_t number_of_tracks = algorithm_->Tracks(Direction()).size();
    flex_fraction = FindFrUnitSize(GridSpan::TranslatedDefiniteGridSpan(0, number_of_tracks), free_space);
    return true;
}
...
LayoutUnit IndefiniteSizeStrategy::FreeSpaceForStretchAutoTracksStep() const
{
    ...
    LayoutUnit min_size = GetLayoutGrid()->ComputeContentLogicalHeight(kMinSize, GetLayoutGrid()->StyleRef().LogicalMinHeight(), LayoutUnit(-1));
...
absl::optional<LayoutUnit> GridTrackSizingAlgorithm::FreeSpace(GridTrackSizingDirection direction) const
{
    return direction == kForRows ? free_space_rows_ : free_space_columns_;
}

absl::optional<LayoutUnit> GridTrackSizingAlgorithm::AvailableSpace(GridTrackSizingDirection direction) const
{
    return direction == kForRows ? available_space_rows_ : available_space_columns_;
}
...
void GridTrackSizingAlgorithm::SetAvailableSpace(GridTrackSizingDirection direction, absl::optional<LayoutUnit> available_space)
{
    if (direction == kForColumns)
...
Vector<GridTrack>& GridTrackSizingAlgorithm::Tracks(GridTrackSizingDirection direction)
{
    return direction == kForColumns ? columns_ : rows_;
}

const Vector<GridTrack>& GridTrackSizingAlgorithm::Tracks(GridTrackSizingDirection direction) const
{
    return direction == kForColumns ? columns_ : rows_;
}

void GridTrackSizingAlgorithm::SetFreeSpace(GridTrackSizingDirection direction, absl::optional<LayoutUnit> free_space)
{
    if (direction == kForColumns)
...
const GridTrackSize& GridTrackSizingAlgorithm::RawGridTrackSize(GridTrackSizingDirection direction, wtf_size_t translated_index) const
{
    ...
    const Vector<GridTrackSize, 1>& track_list_sizes = computed_grid_track_list.track_sizes.LegacyTrackList();
    const Vector<GridTrackSize, 1>& auto_repeat_track_sizes = computed_grid_track_list.auto_repeat_track_sizes;
    const Vector<GridTrackSize, 1>& auto_track_styles
        = is_row_axis ? grid_container_style.GridAutoColumns().LegacyTrackList() : grid_container_style.GridAutoRows().LegacyTrackList();
    wtf_size_t insertion_point = computed_grid_track_list.auto_repeat_insertion_point;
    ...
    // example).
    wtf_size_t explicit_tracks_count = track_list_sizes.size() + auto_repeat_tracks_count;

    int untranslated_index_as_int = static_cast<int>(translated_index - grid_->ExplicitGridStart(direction));
    wtf_size_t auto_track_styles_size = auto_track_styles.size();
    if (untranslated_index_as_int < 0) {
        int index = untranslated_index_as_int % static_cast<int>(auto_track_styles_size);
    ...
    wtf_size_t untranslated_index = static_cast<wtf_size_t>(untranslated_index_as_int);
    if (untranslated_index >= explicit_tracks_count) {
        return auto_track_styles[(untranslated_index - explicit_tracks_count) % auto_track_styles_size];
    }
    ...
    if (untranslated_index < (insertion_point + auto_repeat_tracks_count)) {
        ...
        return auto_repeat_track_sizes[auto_repeat_local_index % auto_repeat_track_sizes.size()];
    }
...
bool GridTrackSizingAlgorithm::IsRelativeSizedTrackAsAuto(const GridTrackSize& track_size, GridTrackSizingDirection direction) const
{
...
GridTrackSize GridTrackSizingAlgorithm::CalculateGridTrackSize(GridTrackSizingDirection direction, wtf_size_t translated_index) const
{
    DCHECK(WasSetup());
    // Collapse empty auto repeat tracks if auto-fit.
    if (grid_->HasAutoRepeatEmptyTracks(direction) && grid_->IsEmptyAutoRepeatTrack(direction, translated_index))
        return { Length::Fixed(), kLengthTrackSizing };

    const GridTrackSize& track_size = RawGridTrackSize(direction, translated_index);
    if (track_size.IsFitContent()) {
        return IsRelativeGridLengthAsAuto(track_size.FitContentTrackBreadth(), direction) ? GridTrackSize(Length::Auto(), Length::MaxContent()) : track_size;
    }
    ...
    if (IsRelativeSizedTrackAsAuto(track_size, direction)) {
        if (direction == kForRows) {
            ...
            if ((grid_->NumTracks(direction) != 1) || !min_track_breadth.IsLength() || !min_track_breadth.length().IsPercent()
                || (min_track_breadth.length().Percent() != 100.0f) || !max_track_breadth.IsLength() || !max_track_breadth.length().IsPercent()
                || (max_track_breadth.length().Percent() != 100.0f) || !layout_grid_->StyleRef().LogicalMaxHeight().IsNone()) {
                UseCounter::Count(layout_grid_->GetDocument(), WebFeature::kGridRowTrackPercentIndefiniteHeight);
            }
        }
...
LayoutUnit GridTrackSizingAlgorithm::InitialBaseSize(const GridTrackSize& track_size) const
{
    ...
    if (track_length.IsSpecified()) {
        ...
        return ValueForLength(track_length, AvailableSpace().value_or(LayoutUnit()));
    }

    DCHECK(track_length.IsMinContent() || track_length.IsAuto() || track_length.IsMaxContent());
    return LayoutUnit();
}

LayoutUnit GridTrackSizingAlgorithm::InitialGrowthLimit(const GridTrackSize& track_size, LayoutUnit base_size) const
{
...
void GridTrackSizingAlgorithm::InitializeTrackSizes()
{
    ...
    bool indefinite_height = direction_ == kForRows && !layout_grid_->CachedHasDefiniteLogicalHeight();
    ...
    for (wtf_size_t i = 0; i < num_tracks; ++i) {
        ...
        if (track_size.IsFitContent()) {
            track.SetGrowthLimitCap(ValueForLength(track_size.FitContentTrackBreadth().length(), AvailableSpace().value_or(LayoutUnit())));
        }
        ...
        if (!has_percent_sized_rows_indefinite_height_ && indefinite_height) {
            ...
            if (raw_track_size.MinTrackBreadth().HasPercentage() || raw_track_size.MaxTrackBreadth().HasPercentage())
                has_percent_sized_rows_indefinite_height_ = true;
        }
    }
}

void GridTrackSizingAlgorithm::SizeTrackToFitNonSpanningItem(const GridSpan& span, LayoutBox& grid_item, GridTrack& track)
{
    const wtf_size_t track_position = span.StartLine();
    const GridTrackSize& track_size = Tracks(direction_)[track_position].CachedTrackSize();

    if (track_size.HasMinContentMinTrackBreadth()) {
        track.SetBaseSize(std::max(track.BaseSize(), strategy_->MinContentForChild(grid_item)));
    } else if (track_size.HasMaxContentMinTrackBreadth()) {
        track.SetBaseSize(std::max(track.BaseSize(), strategy_->MaxContentForChild(grid_item)));
    } else if (track_size.HasAutoMinTrackBreadth()) {
        track.SetBaseSize(std::max(track.BaseSize(), strategy_->MinSizeForChild(grid_item)));
    }

    if (track_size.HasMinContentMaxTrackBreadth()) {
        track.SetGrowthLimit(std::max(track.GrowthLimit(), strategy_->MinContentForChild(grid_item)));
    } else if (track_size.HasMaxContentOrAutoMaxTrackBreadth()) {
        LayoutUnit growth_limit = strategy_->MaxContentForChild(grid_item);
        if (track_size.IsFitContent()) {
            growth_limit = std::min(growth_limit, ValueForLength(track_size.FitContentTrackBreadth().length(), AvailableSpace().value_or(LayoutUnit())));
        }
        track.SetGrowthLimit(std::max(track.GrowthLimit(), growth_limit));
    }
}

bool GridTrackSizingAlgorithm::SpanningItemCrossesFlexibleSizedTracks(const GridSpan& span) const
{
    const Vector<GridTrack>& track_list = Tracks(direction_);
    for (auto track_position : span) {
        const GridTrackSize& track_size = track_list[track_position].CachedTrackSize();
        if (track_size.MinTrackBreadth().IsFlex() || track_size.MaxTrackBreadth().IsFlex())
            return true;
    }
...
static bool ShouldProcessTrackForTrackSizeComputationPhase(TrackSizeComputationPhase phase, const GridTrackSize& track_size)
{
    switch (phase) {
    case kResolveIntrinsicMinimums:
...
static bool TrackShouldGrowBeyondGrowthLimitsForTrackSizeComputationPhase(TrackSizeComputationPhase phase, const GridTrackSize& track_size)
{
    switch (phase) {
    case kResolveIntrinsicMinimums:
    case kResolveContentBasedMinimums:
        return track_size.HasAutoOrMinContentMinTrackBreadthAndIntrinsicMaxTrackBreadth();
    case kResolveMaxContentMinimums:
        return track_size.HasMaxContentMinTrackBreadthAndMaxContentMaxTrackBreadth();
    case kResolveIntrinsicMaximums:
    case kResolveMaxContentMaximums:
        return true;
    case kMaximizeTracks:
...
static void MarkAsInfinitelyGrowableForTrackSizeComputationPhase(TrackSizeComputationPhase phase, GridTrack& track)
{
    ...
    case kResolveMaxContentMinimums:
        return;
    case kResolveIntrinsicMaximums:
        if (TrackSizeForTrackSizeComputationPhase(phase, track, kAllowInfinity) == kInfinity && track.PlannedSize() != kInfinity)
...
LayoutUnit GridTrackSizingAlgorithm::ItemSizeForTrackSizeComputationPhase(TrackSizeComputationPhase phase, LayoutBox& grid_item) const
{
    switch (phase) {
    case kResolveIntrinsicMinimums:
...
static bool SortByGridTrackGrowthPotential(const GridTrack* track1, const GridTrack* track2)
{
    ...
    bool track1_has_infinite_growth_potential_without_cap = track1->InfiniteGrowthPotential() && !track1->GrowthLimitCap();
    bool track2_has_infinite_growth_potential_without_cap = track2->InfiniteGrowthPotential() && !track2->GrowthLimitCap();

    if (track1_has_infinite_growth_potential_without_cap && track2_has_infinite_growth_potential_without_cap)
        return false;

    if (track1_has_infinite_growth_potential_without_cap || track2_has_infinite_growth_potential_without_cap)
        return track2_has_infinite_growth_potential_without_cap;

    LayoutUnit track1_limit = track1->GrowthLimitCap().value_or(track1->GrowthLimit());
    LayoutUnit track2_limit = track2->GrowthLimitCap().value_or(track2->GrowthLimit());
    return (track1_limit - track1->BaseSize()) < (track2_limit - track2->BaseSize());
}

static void ClampGrowthShareIfNeeded(TrackSizeComputationPhase phase, const GridTrack& track, LayoutUnit& growth_share)
{
    if (phase != kResolveMaxContentMaximums || !track.GrowthLimitCap())
        return;

    LayoutUnit distance_to_cap = track.GrowthLimitCap().value() - track.SizeDuringDistribution();
...
void GridTrackSizingAlgorithm::DistributeSpaceToTracks(
    Vector<GridTrack*>& tracks, Vector<GridTrack*>* grow_beyond_growth_limits_tracks, LayoutUnit& available_logical_space) const
{
    DCHECK_GE(available_logical_space, 0);

    for (auto* track : tracks) {
        track->SetSizeDuringDistribution(TrackSizeForTrackSizeComputationPhase(phase, *track, kForbidInfinity));
    }
    ...
    if (available_logical_space > 0) {
        ...
        for (wtf_size_t i = 0; i < tracks_size; ++i) {
            ...
            LayoutUnit available_logical_space_share = available_logical_space / (tracks_size - i);
            const LayoutUnit& track_breadth = TrackSizeForTrackSizeComputationPhase(phase, track, kForbidInfinity);
            LayoutUnit growth_share = track.InfiniteGrowthPotential() ? available_logical_space_share
                                                                      : std::min(available_logical_space_share, track.GrowthLimit() - track_breadth);
    ...
    if (available_logical_space > 0 && grow_beyond_growth_limits_tracks) {
        ...
        if (phase == kResolveMaxContentMaximums) {
            std::sort(grow_beyond_growth_limits_tracks->begin(), grow_beyond_growth_limits_tracks->end(), SortByGridTrackGrowthPotential);
        }

        wtf_size_t tracks_growing_above_max_breadth_size = grow_beyond_growth_limits_tracks->size();
        for (wtf_size_t i = 0; i < tracks_growing_above_max_breadth_size; ++i) {
            GridTrack* track = grow_beyond_growth_limits_tracks->at(i);
            LayoutUnit growth_share = available_logical_space / (tracks_growing_above_max_breadth_size - i);
            ClampGrowthShareIfNeeded(phase, *track, growth_share);
            DCHECK_GE(growth_share, 0) << "We must never shrink any grid track or "
                                          "else we can't guarantee we abide by our "
                                          "min-sizing function.";
    ...
    for (auto* track : tracks) {
        track->SetPlannedSize(
            track->PlannedSize() == kInfinity ? track->SizeDuringDistribution() : std::max(track->PlannedSize(), track->SizeDuringDistribution()));
    }
}

template <TrackSizeComputationPhase phase>
void GridTrackSizingAlgorithm::IncreaseSizesToAccommodateSpanningItems(
    const HeapVector<GridItemWithSpan>::iterator& grid_items_with_span_begin, const HeapVector<GridItemWithSpan>::iterator& grid_items_with_span_end)
{
    ...
    for (const auto& track_index : content_sized_tracks_index_) {
        ...
        track.SetPlannedSize(TrackSizeForTrackSizeComputationPhase(phase, track, kAllowInfinity));
    }

    Vector<GridTrack*> grow_beyond_growth_limits_tracks;
    Vector<GridTrack*> filtered_tracks;
    for (auto* it = grid_items_with_span_begin; it != grid_items_with_span_end; ++it) {
        ...
        for (auto track_position : item_span) {
            ...
            spanning_tracks_size += TrackSizeForTrackSizeComputationPhase(phase, track, kForbidInfinity);
            if (!ShouldProcessTrackForTrackSizeComputationPhase(phase, track_size))
                continue;

            filtered_tracks.push_back(&track);

            if (TrackShouldGrowBeyondGrowthLimitsForTrackSizeComputationPhase(phase, track_size))
        ...
        spanning_tracks_size += layout_grid_->GuttersSize(*grid_, direction_, item_span.StartLine(), item_span.IntegerSpan(), AvailableSpace());

        LayoutUnit extra_space = ItemSizeForTrackSizeComputationPhase(phase, grid_item_with_span.GridItem()) - spanning_tracks_size;
        extra_space = extra_space.ClampNegativeToZero();
        auto& tracks_to_grow_beyond_growth_limits = grow_beyond_growth_limits_tracks.empty() ? filtered_tracks : grow_beyond_growth_limits_tracks;
        DistributeSpaceToTracks<phase>(filtered_tracks, &tracks_to_grow_beyond_growth_limits, extra_space);
    }
...
void GridTrackSizingAlgorithm::ResolveIntrinsicTrackSizes()
{
    ...
    if (grid_->HasGridItems()) {
        IterateGridItemsInTrackIndices(*grid_, direction_, content_sized_tracks_index_, [&](LayoutBox* grid_item, const GridSpan& span) {
            if (span.IntegerSpan() == 1) {
                SizeTrackToFitNonSpanningItem(span, *grid_item, all_tracks[span.StartLine()]);
            } else if (!SpanningItemCrossesFlexibleSizedTracks(span)) {
                items_sorted_by_increasing_span.push_back(GridItemWithSpan(*grid_item, span));
            }
        });
        std::sort(items_sorted_by_increasing_span.begin(), items_sorted_by_increasing_span.end());
    }
    ...
    while (it != end) {
        ...
        IncreaseSizesToAccommodateSpanningItems<kResolveIntrinsicMinimums>(it, range_end);
        IncreaseSizesToAccommodateSpanningItems<kResolveContentBasedMinimums>(it, range_end);
        IncreaseSizesToAccommodateSpanningItems<kResolveMaxContentMinimums>(it, range_end);
        IncreaseSizesToAccommodateSpanningItems<kResolveIntrinsicMaximums>(it, range_end);
        IncreaseSizesToAccommodateSpanningItems<kResolveMaxContentMaximums>(it, range_end);
        it = range_end;
    }
...
void GridTrackSizingAlgorithm::ComputeGridContainerIntrinsicSizes()
{
    ...
    for (auto& track : all_tracks) {
        DCHECK(strategy_->IsComputingSizeContainment() || !track.InfiniteGrowthPotential());
        min_content_size_ += track.BaseSize();
        max_content_size_ += track.GrowthLimitIsInfinite() ? track.BaseSize() : track.GrowthLimit();
...
LayoutUnit GridTrackSizingAlgorithm::ComputeTrackBasedSize() const
{
    ...
    for (auto& track : all_tracks) {
        size += track.GrowthLimitIsInfinite() ? track.BaseSize() : track.GrowthLimit();
    }

    size += layout_grid_->GuttersSize(*grid_, direction_, 0, all_tracks.size(), AvailableSpace());

    return size;
}

double GridTrackSizingAlgorithm::FindFrUnitSize(const GridSpan& tracks_span, LayoutUnit left_over_space) const
{
    ...
    return ComputeFlexFactorUnitSize(all_tracks, flex_factor_sum, left_over_space, flexible_tracks_indexes);
}

double GridTrackSizingAlgorithm::ComputeFlexFactorUnitSize(const Vector<GridTrack>& tracks, double flex_factor_sum, LayoutUnit& left_over_space,
    const Vector<wtf_size_t, 8>& flexible_tracks_indexes, std::unique_ptr<TrackIndexSet> tracks_to_treat_as_inflexible) const
{
    // We want to avoid the effect of flex factors sum below 1 making the factor
    // unit size to grow exponentially.
    double hypothetical_factor_unit_size = left_over_space / std::max<double>(1, flex_factor_sum);
    ...
    std::unique_ptr<TrackIndexSet> additional_tracks_to_treat_as_inflexible = std::move(tracks_to_treat_as_inflexible);
    bool valid_flex_factor_unit = true;
    for (auto index : flexible_tracks_indexes) {
        if (additional_tracks_to_treat_as_inflexible && additional_tracks_to_treat_as_inflexible->Contains(index))
            continue;
        LayoutUnit base_size = tracks[index].BaseSize();
        double flex_factor = tracks[index].CachedTrackSize().MaxTrackBreadth().Flex();
        ...
        if (base_size > hypothetical_factor_unit_size * flex_factor) {
            ...
            if (!additional_tracks_to_treat_as_inflexible) {
                additional_tracks_to_treat_as_inflexible = std::make_unique<TrackIndexSet>();
            }
    ...
    if (!valid_flex_factor_unit) {
        return ComputeFlexFactorUnitSize(
            tracks, flex_factor_sum, left_over_space, flexible_tracks_indexes, std::move(additional_tracks_to_treat_as_inflexible));
    }
    return hypothetical_factor_unit_size;
}

void GridTrackSizingAlgorithm::ComputeFlexSizedTracksGrowth(double flex_fraction, Vector<LayoutUnit>& increments, LayoutUnit& total_growth) const
{
    ...
    for (wtf_size_t i = 0; i < num_flex_tracks; ++i) {
        ...
        LayoutUnit new_base_size = std::max(old_base_size, LayoutUnit(flex_fraction * track_size.MaxTrackBreadth().Flex()));
        increments[i] = new_base_size - old_base_size;
        total_growth += increments[i];
    }
}

void GridTrackSizingAlgorithm::StretchFlexibleTracks(absl::optional<LayoutUnit> free_space)
{
    if (flexible_sized_tracks_index_.empty())
        return;

    double flex_fraction = strategy_->FindUsedFlexFraction(flexible_sized_tracks_index_, direction_, free_space);
    ...
    if (strategy_->RecomputeUsedFlexFractionIfNeeded(flex_fraction, increments, total_growth)) {
...
void GridTrackSizingAlgorithm::StretchAutoTracks()
{
    ...
    if (auto_sized_tracks_for_stretch_index_.empty() || (free_space <= 0)
        || (layout_grid_->ContentAlignment(direction_).Distribution() != ContentDistributionType::kStretch))
        return;

    unsigned number_of_auto_sized_tracks = auto_sized_tracks_for_stretch_index_.size();
...
void GridTrackSizingAlgorithm::Setup(GridTrackSizingDirection direction, wtf_size_t num_tracks, absl::optional<LayoutUnit> available_space)
{
    DCHECK(needs_setup_);
    direction_ = direction;
    SetAvailableSpace(direction, available_space ? available_space.value().ClampNegativeToZero() : available_space);
    ...
    if (available_space) {
        LayoutUnit gutters_size = layout_grid_->GuttersSize(*grid_, direction, 0, grid_->NumTracks(direction), available_space);
...
void GridTrackSizingAlgorithm::ComputeBaselineAlignmentContext()
{
    ...
    BaselineItemsCache& baseline_items_cache = axis == kGridColumnAxis ? column_baseline_items_map_ : row_baseline_items_map_;
...
void GridTrackSizingAlgorithm::Run()
{
    ...
    if (FreeSpace(direction_)) {
        LayoutUnit updated_free_space = FreeSpace(direction_).value() - min_content_size_;
    ...
    // Step 3.
    strategy_->MaximizeTracks(Tracks(direction_), direction_ == kForColumns ? free_space_columns_ : free_space_rows_);
