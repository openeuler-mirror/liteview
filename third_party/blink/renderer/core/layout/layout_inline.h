...
public:
    ...
    {
        NOT_DESTROYED();
        return IsInLayoutNGInlineFormattingContext() ? &LineBoxList::Empty() : &line_boxes_;
    }
    ...
    {
        NOT_DESTROYED();
        return AlwaysCreateLineBoxes() ? FirstLineBox() : CulledInlineFirstLineBox();
    }
    ...
    void AddOutlineRectsForChildrenAndContinuations(Vector<PhysicalRect>&, const PhysicalOffset& additional_offset, NGOutlineType) const;
    void AddOutlineRectsForContinuations(Vector<PhysicalRect>&, const PhysicalOffset& additional_offset, NGOutlineType) const;
    ...
    LayoutRect LocalCaretRect(const InlineBox*, int, LayoutUnit* extra_width_to_end_of_line) const final;
    ...
    bool HitTestCulledInline(HitTestResult&, const HitTestLocation&, const PhysicalOffset& accumulated_offset, const NGInlineCursor* parent_cursor = nullptr);
    ...
    PhysicalRect VisualRectInDocument(VisualRectFlags = kDefaultVisualRectFlags) const override;
...
protected:
    ...
    PhysicalOffset OffsetFromContainerInternal(const LayoutObject*, bool ignore_scroll_offset) const final;
...
private:
    ...
    template <typename PhysicalRectCollector> void CollectCulledLineBoxRects(const PhysicalRectCollector&) const;

    // FlippedRectCollector should be like a function:
    // void (const LayoutRect&);
    template <typename FlippedRectCollector>
    void CollectCulledLineBoxRectsInFlippedBlocksDirection(const FlippedRectCollector&, const LayoutInline* container) const;

    void AddChildToContinuation(LayoutObject* new_child, LayoutObject* before_child);
    ...
    void MoveChildrenToIgnoringContinuation(LayoutInline* to, LayoutObject* start_child);
    ...
    void SplitFlow(LayoutObject* before_child, LayoutBlockFlow* new_block_box, LayoutObject* new_child, LayoutBoxModelObject* old_cont);

    // Create an anonymous block for block children of this inline.
    LayoutBlockFlow* CreateAnonymousContainerForBlockChildren(bool split_flow) const;
    ...
    LayoutUnit LineHeight(bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const final;
    LayoutUnit BaselinePosition(FontBaseline, bool first_line, LineDirectionMode, LinePositionMode = kPositionOnContainingLine) const final;
