...
public:
    ...
    DeferredShapingViewportScope& operator=(const DeferredShapingViewportScope&) = delete;
...
public:
    ...
    DeferredShapingMinimumTopScope(const NGLayoutInputNode input_node, LayoutUnit minimum_top)
    ...
    [[nodiscard]] static DeferredShapingMinimumTopScope CreateDelta(const NGLayoutInputNode input_node, LayoutUnit delta)
    {
        return DeferredShapingMinimumTopScope(input_node, DeferredShapingController::From(input_node).CurrentMinimumTop() + delta);
    }
    ...
    DeferredShapingMinimumTopScope(const DeferredShapingMinimumTopScope&) = delete;
    DeferredShapingMinimumTopScope& operator=(const DeferredShapingMinimumTopScope&) = delete;
...
public:
    explicit DeferredShapingDisallowScope(const LayoutView& view, bool disable = true)
    ...
    DeferredShapingDisallowScope& operator=(const DeferredShapingDisallowScope&) = delete;
...
#define DEFERRED_SHAPING_VLOG(verbose_level) LAZY_STREAM(VLOG_STREAM(verbose_level), ((verbose_level) <= ::logging::GetVlogLevel("deferred_shaping")))
