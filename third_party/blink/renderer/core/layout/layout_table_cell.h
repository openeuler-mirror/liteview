...
static const unsigned kUnsetColumnIndex = (1u << BITS_OF_ABSOLUTE_COLUMN_INDEX) - 1;
...
class CORE_EXPORT LayoutTableCell : public LayoutBlockFlow, public LayoutNGTableCellInterface {
public:
    ...
    Length StyleOrColLogicalWidth() const
    {
        ...
        if (!style_width.IsAuto())
            return style_width;
        if (LayoutTableCol* first_column = Table()->ColElementAtAbsoluteColumn(AbsoluteColumnIndex()).InnermostColOrColGroup())
    ...
    int LogicalHeightFromStyle() const
    {
        ...
        int style_logical_height = height.IsSpecified() ? ValueForLength(height, LayoutUnit()).ToInt() : 0;

        // In strict mode, box-sizing: content-box do the right thing and actually
        // add in the border and padding.
        // Call computedCSSPadding* directly to avoid including implicitPadding.
        if (!GetDocument().InQuirksMode() && StyleRef().BoxSizing() != EBoxSizing::kBorderBox) {
            style_logical_height += (ComputedCSSPaddingBefore() + ComputedCSSPaddingAfter()).Floor() + (BorderBefore() + BorderAfter()).Floor();
        }
    ...
    int LogicalHeightForRowSizing() const
    {
        ...
        // layout!
        int adjusted_logical_height = PixelSnappedLogicalHeight() - (IntrinsicPaddingBefore() + IntrinsicPaddingAfter());
    ...
    bool IsBaselineAligned() const
    {
        ...
        return va == EVerticalAlign::kBaseline || va == EVerticalAlign::kTextBottom || va == EVerticalAlign::kTextTop || va == EVerticalAlign::kSuper
    ...
    void ComputeIntrinsicPadding(int collapsed_height, int row_height, EVerticalAlign, SubtreeLayoutScope&);
    ...
    void ScrollbarsChanged(bool horizontal_scrollbar_changed, bool vertical_scrollbar_changed, ScrollbarChangeContext = kLayout) override;
    ...
    static LayoutTableCell* CreateAnonymous(Document*, scoped_refptr<ComputedStyle>, LegacyLayout);
    ...
    static bool CompareInDOMOrder(const LayoutTableCell* cell1, const LayoutTableCell* cell2)
    {
    ...
    {
        NOT_DESTROYED();
        return other && AbsoluteColumnIndex() + ColSpan() == other->AbsoluteColumnIndex() + other->ColSpan();
    }
    ...
    {
        NOT_DESTROYED();
        return other && RowIndex() + ResolvedRowSpan() == other->RowIndex() + other->ResolvedRowSpan();
    }
...
protected:
    ...
    {
        NOT_DESTROYED();
        return type == kLayoutObjectTableCell || type == kLayoutObjectTableCellLegacy || LayoutBlockFlow::IsOfType(type);
    }
...
private:
    ...
    void PaintBoxDecorationBackground(const PaintInfo&, const PhysicalOffset& paint_offset) const override;
    void PaintMask(const PaintInfo&, const PhysicalOffset& paint_offset) const override;

    OverflowClipAxes ComputeOverflowClipAxes() const override;

    using CollapsedBorderValuesMethod = const CollapsedBorderValue& (CollapsedBorderValues::*)() const;
    LogicalToPhysical<CollapsedBorderValuesMethod> CollapsedBorderValuesMethodsPhysical() const
    {
        NOT_DESTROYED();
        return LogicalToPhysical<CollapsedBorderValuesMethod>(
            // Collapsed border logical directions are in table's directions.
            TableStyle().GetWritingDirection(), &CollapsedBorderValues::StartBorder, &CollapsedBorderValues::EndBorder, &CollapsedBorderValues::BeforeBorder,
    ...
    PhysicalToLogical<bool> InnerHalfPixelAsOneLogical() const
    {
        ...
        return PhysicalToLogical<bool>(
            // Collapsed border logical directions are in table's directions.
            TableStyle().GetWritingDirection(), kInnerHalfPixelAsOneTop, kInnerHalfPixelAsOneRight, kInnerHalfPixelAsOneBottom, kInnerHalfPixelAsOneLeft);
    }

    unsigned CollapsedBorderHalfLeft(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(kInnerHalfPixelAsOneLeft ^ outer, CollapsedBorderValuesMethodsPhysical().Left());
    }
    unsigned CollapsedBorderHalfRight(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(kInnerHalfPixelAsOneRight ^ outer, CollapsedBorderValuesMethodsPhysical().Right());
    }
    unsigned CollapsedBorderHalfTop(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(kInnerHalfPixelAsOneTop ^ outer, CollapsedBorderValuesMethodsPhysical().Top());
    }
    unsigned CollapsedBorderHalfBottom(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(kInnerHalfPixelAsOneBottom ^ outer, CollapsedBorderValuesMethodsPhysical().Bottom());
    }
    ...
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(InnerHalfPixelAsOneLogical().Start() ^ outer, &CollapsedBorderValues::StartBorder);
    }
    unsigned CollapsedBorderHalfEnd(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(InnerHalfPixelAsOneLogical().End() ^ outer, &CollapsedBorderValues::EndBorder);
    }
    unsigned CollapsedBorderHalfBefore(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(InnerHalfPixelAsOneLogical().Before() ^ outer, &CollapsedBorderValues::BeforeBorder);
    }
    unsigned CollapsedBorderHalfAfter(bool outer) const
    {
        NOT_DESTROYED();
        return CollapsedBorderHalf(InnerHalfPixelAsOneLogical().After() ^ outer, &CollapsedBorderValues::AfterBorder);
    }

    unsigned CollapsedBorderHalf(bool half_pixel_as_one, CollapsedBorderValuesMethod m) const
    {
        NOT_DESTROYED();
        UpdateCollapsedBorderValues();
    ...
    {
        NOT_DESTROYED();
        return LogicalToPhysical<int>(StyleRef().GetWritingDirection(), 0, 0, intrinsic_padding_before_, intrinsic_padding_after_);
    }
    ...
    Length LogicalWidthFromColumns(LayoutTableCol* first_col_for_this_cell, const Length& width_from_style) const;
