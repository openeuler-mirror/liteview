...
class SVGExternalDocumentCache final : public GarbageCollected<SVGExternalDocumentCache>, public Supplement<Document> {
public:
...
private:
    HeapHashMap<WeakMember<Resource>, Member<SVGResourceDocumentContent>> entries_;
};

const char SVGExternalDocumentCache::kSupplementName[] = "SVGExternalDocumentCache";

SVGExternalDocumentCache* SVGExternalDocumentCache::From(Document& document)
{
    SVGExternalDocumentCache* cache = Supplement<Document>::From<SVGExternalDocumentCache>(document);
    if (!cache) {
...
SVGResourceDocumentContent* SVGExternalDocumentCache::Get(TextResource* resource)
{
    auto& entry = entries_.insert(resource, nullptr).stored_value->value;
    if (!entry) {
        entry = MakeGarbageCollected<SVGResourceDocumentContent>(resource, GetSupplementable()->GetExecutionContext());
    }
    return entry;
}
...
Document* CreateDocument(const TextResource* resource, ExecutionContext* execution_context)
{
    const ResourceResponse& response = resource->GetResponse();
    if (!MimeTypeAllowed(response))
        return nullptr;
    auto* document = XMLDocument::CreateSVG(DocumentInit::Create().WithURL(response.CurrentRequestUrl()).WithExecutionContext(execution_context));
...
SVGResourceDocumentContent* SVGResourceDocumentContent::Fetch(FetchParameters& params, Document& document, ResourceClient* client)
{
    ...
    TextResource* resource = TextResource::FetchSVGDocument(params, document.Fetcher(), client);
    if (!resource)
        return nullptr;
    auto* document_content = SVGExternalDocumentCache::From(document)->Get(resource);
