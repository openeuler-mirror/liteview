...
void SVGPatternElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == svg_names::kPatternTransformAttr) {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTransform, *pattern_transform_->CurrentValue()->CssValue());
        return;
    }
...
void SVGPatternElement::InvalidatePattern(LayoutInvalidationReasonForTracing reason)
{
...
void SVGPatternElement::InvalidateDependentPatterns()
{
    NotifyIncomingReferences([](SVGElement& element) {
        if (auto* pattern = DynamicTo<SVGPatternElement>(element)) {
            pattern->InvalidatePattern(layout_invalidation_reason::kSvgResourceInvalidated);
        }
    });
}

LayoutObject* SVGPatternElement::CreateLayoutObject(const ComputedStyle&, LegacyLayout)
{
