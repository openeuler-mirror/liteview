...
void SVGUseElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
...
void SVGUseElement::UpdateTargetReference()
{
    ...
    document_content_ = SVGResourceDocumentContent::Fetch(params, *context_document, this);
}
...
{
    SVGElement* instance_root = &To<SVGElement>(target_root.CloneWithChildren(CloneChildrenFlag::kClone));
...
LayoutObject* SVGUseElement::CreateLayoutObject(const ComputedStyle& style, LegacyLayout)
{
...
void SVGUseElement::NotifyFinished(Resource* resource)
{
    ...
    if (resource->ErrorOccurred() || !document_content_->GetDocument()) {
        ...
        GetDocument().GetTaskRunner(TaskType::kDOMManipulation)->PostTask(FROM_HERE, WTF::BindOnce(&SVGUseElement::DispatchPendingEvent, WrapPersistent(this)));
    }
}
