...
SVGTransformList::SVGTransformList(SVGTransformType transform_type, const String& value)
{
    ...
    bool success = WTF::VisitCharacters(value, [&](const auto* chars, unsigned length) {
...
CSSValue* CreateTransformCSSValue(const SVGTransform& transform)
{
    ...
    case CSSValueID::kMatrix:
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().A(), CSSPrimitiveValue::UnitType::kUserUnits));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().B(), CSSPrimitiveValue::UnitType::kUserUnits));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().C(), CSSPrimitiveValue::UnitType::kUserUnits));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().D(), CSSPrimitiveValue::UnitType::kUserUnits));
...
SVGTransformType ParseTransformType(const String& string)
{
    if (string.empty())
        return SVGTransformType::kUnknown;
    return WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return ParseAndSkipTransformType(chars, chars + length); });
}
...
SVGParsingError SVGTransformList::SetValueAsString(const String& value)
{
    ...
    SVGParsingError parse_error = WTF::VisitCharacters(value, [&](const auto* chars, unsigned length) { return ParseInternal(chars, chars + length); });
