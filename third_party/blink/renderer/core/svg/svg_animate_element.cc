...
String ComputeCSSPropertyValue(SVGElement* element, CSSPropertyID id)
{
    ...
    if (!style)
        return "";
    const CSSValue* value = CSSProperty::Get(id).CSSValueFromComputedStyle(*style, element->GetLayoutObject(), false);
...
{
    if ((animation_mode == kFromToAnimation && percentage > 0.5) || animation_mode == kToAnimation || percentage == 1) {
        return to;
    }
    return from;
}
...
bool SVGAnimateElement::CalculateFromAndToValues(const String& from_string, const String& to_string)
{
    DCHECK(targetElement());
...
bool SVGAnimateElement::CalculateFromAndByValues(const String& from_string, const String& by_string)
{
    DCHECK(targetElement());
