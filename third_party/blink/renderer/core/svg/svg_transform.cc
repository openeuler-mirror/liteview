...
SVGTransform::SVGTransform(SVGTransformType transform_type, float angle, const gfx::PointF& center, const AffineTransform& matrix)
    : transform_type_(transform_type)
    , angle_(angle)
    , center_(center)
    , matrix_(matrix)
...
String SVGTransform::ValueAsString() const
{
    ...
    case SVGTransformType::kRotate: {
        ...
        float cx = ClampTo<float>(cos_angle != 1 ? (matrix_.E() * (1 - cos_angle) - matrix_.F() * sin_angle) / (1 - cos_angle) / 2 : 0);
        float cy = ClampTo<float>(cos_angle != 1 ? (matrix_.E() * sin_angle / (1 - cos_angle) + matrix_.F()) / 2 : 0);
        if (cx || cy) {
