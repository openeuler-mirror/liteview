...
static bool ParseKeySplines(const String& string, Vector<gfx::CubicBezier>& result)
{
    ...
    if (string.empty())
        return true;
    bool parsed = WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return ParseKeySplinesInternal(chars, chars + length, result); });
    if (!parsed) {
        result.clear();
        return false;
    }
...
bool SVGAnimationElement::CalculateValuesAnimation()
{
    ...
    // 'values'.
    if (calc_mode != kCalcModePaced && !FastHasAttribute(svg_names::kKeyPointsAttr) && FastHasAttribute(svg_names::kKeyTimesAttr)
    ...
    // 'keyPoints' or 'values'.
    if (calc_mode == kCalcModeSpline) {
        if ((key_splines_.empty() || key_splines_.size() != values_.size() - 1) && key_splines_.size() != key_points_.size() - 1)
            return false;
    }
    // If 'keyPoints' is specified it should have the same amount of points as
    // 'keyTimes', and at least two points.
    if (FastHasAttribute(svg_names::kKeyPointsAttr) && (KeyTimes().size() < 2 || KeyTimes().size() != key_points_.size()))
        return false;
...
bool SVGAnimationElement::CheckAnimationParameters()
{
    ...
    // If 'keyPoints' is specified it should have the same amount of points as
    // 'keyTimes'.
    if (FastHasAttribute(svg_names::kKeyPointsAttr) && KeyTimes().size() != key_points_.size())
        return false;
    ...
    if (calc_mode == kCalcModeSpline) {
        ...
        if (key_splines_.empty() || (FastHasAttribute(svg_names::kKeyPointsAttr) && key_splines_.size() != key_points_.size() - 1)
            || (animation_mode == kValuesAnimation && key_splines_.size() != values_.size() - 1)
            || (FastHasAttribute(svg_names::kKeyTimesAttr) && key_splines_.size() != KeyTimes().size() - 1))
            return false;
    }

    if (animation_mode == kFromToAnimation || animation_mode == kFromByAnimation || animation_mode == kToAnimation || animation_mode == kByAnimation) {
