...
void SVGElement::SetAnimatedAttribute(const QualifiedName& attribute, SVGPropertyBase* value)
{
    ...
    const SvgAttributeChangedParams params(attribute, AttributeModificationReason::kDirectly);
    ForSelfAndInstances(this, [&params, &value](SVGElement* element) {
        if (SVGAnimatedPropertyBase* animated_property = element->PropertyFromAttribute(params.name)) {
...
bool SVGElement::HasTransform(ApplyMotionTransform apply_motion_transform) const
{
    return (GetLayoutObject() && GetLayoutObject()->HasTransform()) || (apply_motion_transform == kIncludeMotionTransform && HasSVGRareData());
}

AffineTransform SVGElement::CalculateTransform(ApplyMotionTransform apply_motion_transform) const
{
    const LayoutObject* layout_object = GetLayoutObject();

    AffineTransform matrix;
    if (layout_object && layout_object->HasTransform()) {
        matrix = TransformHelper::ComputeTransform(*layout_object, ComputedStyle::kIncludeTransformOrigin);
    }
...
CSSPropertyID SVGElement::CssPropertyIdForSVGAttributeName(const ExecutionContext* execution_context, const QualifiedName& attr_name)
{
    ...
    if (!property_name_to_id_map) {
        ...
        for (size_t i = 0; i < std::size(attr_names); i++) {
            CSSPropertyID property_id = CssPropertyID(execution_context, attr_names[i]->LocalName());
            DCHECK_GT(property_id, CSSPropertyID::kInvalid);
            property_name_to_id_map->Set(attr_names[i]->LocalName().Impl(), property_id);
        }
    }
...
void SVGElement::InvalidateRelativeLengthClients(SubtreeLayoutScope* layout_scope)
{
    if (!isConnected())
        return;
    ...
    if (LayoutObject* layout_object = GetLayoutObject()) {
        if (HasRelativeLengths() && layout_object->IsSVGResourceContainer()) {
            To<LayoutSVGResourceContainer>(layout_object)->InvalidateCacheAndMarkForLayout(layout_invalidation_reason::kSizeChanged, layout_scope);
        } else if (SelfHasRelativeLengths()) {
            layout_object->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kUnknown, kMarkContainerChain, layout_scope);
        }
    }
...
struct AnimatedPropertyTypeHashTraits : HashTraits<AnimatedPropertyType> {
    ...
    static AnimatedPropertyType EmptyValue()
    {
...
using AttributeToPropertyTypeMap
    = HashMap<QualifiedName, AnimatedPropertyType, DefaultHash<QualifiedName>::Hash, HashTraits<QualifiedName>, AnimatedPropertyTypeHashTraits>;
...
{
    AttributeToPropertyMap::const_iterator it = attribute_to_property_map_.Find<SVGAttributeHashTranslator>(attribute_name);
...
bool SVGElement::IsPresentationAttribute(const QualifiedName& name) const
{
    ...
    if (RuntimeEnabledFeatures::LangAttributeAwareSvgTextEnabled() && (name.Matches(xml_names::kLangAttr) || name == svg_names::kLangAttr)) {
        return true;
    }
    return CssPropertyIdForSVGAttributeName(GetExecutionContext(), name) > CSSPropertyID::kInvalid;
}
...
void SVGElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    ...
    if (property_id > CSSPropertyID::kInvalid) {
        ...
        if (RuntimeEnabledFeatures::LangAttributeAwareSvgTextEnabled() && !FastHasAttribute(xml_names::kLangAttr)) {
            MapLanguageAttributeToLocale(value, style);
        }
    }
}
...
void SVGElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    ...
    CSSPropertyID prop_id = SVGElement::CssPropertyIdForSVGAttributeName(GetExecutionContext(), attr_name);
...
void SVGElement::BaseValueChanged(const SVGAnimatedPropertyBase& animated_property)
{
...
void SVGElement::UpdateWebAnimatedAttributeOnBaseValChange(const QualifiedName& attribute)
{
    if (!HasSVGRareData())
        return;
...
void SVGElement::SynchronizeSVGAttribute(const QualifiedName& name) const
{
    ...
    if (name == AnyQName()) {
        for (SVGAnimatedPropertyBase* property : attribute_to_property_map_.Values()) {
...
void SVGElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
    ...
    for (SVGAnimatedPropertyBase* property : attribute_to_property_map_.Values()) {
        if (!property->HasPresentationAttributeMapping() || !property->IsAnimating())
            continue;
        CollectStyleForPresentationAttribute(property->AttributeName(), g_empty_atom, style);
    }
}

scoped_refptr<ComputedStyle> SVGElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    ...
    return GetDocument().GetStyleResolver().ResolveStyle(corresponding_element, style_recalc_context, style_request);
}
...
SVGElement::InstanceUpdateBlocker::InstanceUpdateBlocker(SVGElement* target_element)
...
void SVGElement::RebuildAllIncomingReferences()
{
    ...
    const SVGElementSet& incoming_references = SvgRareData()->IncomingReferences();

    // Iterate on a snapshot as |incoming_references| may be altered inside loop.
    HeapVector<Member<SVGElement>> incoming_references_snapshot(incoming_references);

    // Force rebuilding the |source_element| so it knows about this change.
    const SvgAttributeChangedParams params(svg_names::kHrefAttr, AttributeModificationReason::kDirectly);
