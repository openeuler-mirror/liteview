...
bool TargetCanHaveMotionTransform(const SVGElement& target)
{
    ...
    return IsA<SVGGElement>(target) || IsA<SVGDefsElement>(target) || IsA<SVGUseElement>(target) || IsA<SVGImageElement>(target)
        ...
        || IsA<SVGTextElement>(target) || IsA<SVGClipPathElement>(target) || IsA<SVGMaskElement>(target) || IsA<SVGAElement>(target)
...
static bool ParsePoint(const String& string, gfx::PointF& point)
{
    if (string.empty())
        return false;
    return WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return ParsePointInternal(chars, chars + length, point); });
}
...
bool SVGAnimateMotionElement::CalculateFromAndToValues(const String& from_string, const String& to_string)
{
...
bool SVGAnimateMotionElement::CalculateFromAndByValues(const String& from_string, const String& by_string)
{
...
void SVGAnimateMotionElement::CalculateAnimationValue(SMILAnimationValue& animation_value, float percentage, unsigned repeat_count) const
{
    ...
    if (GetAnimationMode() != kPathAnimation) {
        float animated_x = ComputeAnimatedNumber(parameters, percentage, repeat_count, from_point_.x(), to_point_.x(), to_point_at_end_of_duration_.x());
        float animated_y = ComputeAnimatedNumber(parameters, percentage, repeat_count, from_point_.y(), to_point_.y(), to_point_at_end_of_duration_.y());
    ...
    PointAndTangent position = animation_path_.PointAndNormalAtLength(position_on_path);

    // Handle accumulate="sum".
    if (repeat_count && parameters.is_cumulative) {
        gfx::PointF position_at_end_of_duration = animation_path_.PointAtLength(path_length);
