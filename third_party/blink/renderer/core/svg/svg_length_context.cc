...
static inline float DimensionForLengthMode(SVGLengthMode mode, const gfx::SizeF& viewport_size)
{
    switch (mode) {
...
static float ConvertValueFromPercentageToUserUnits(const SVGLength& value, float viewport_dimension)
{
    return CSSPrimitiveValue::ClampToCSSLengthRange(value.ScaleByPercentage(viewport_dimension));
}
...
static const ComputedStyle* RootElementStyle(const Node* context)
{
    ...
    const ComputedStyle* style = document_element && context != document_element ? document_element->GetComputedStyle() : document_style;
    if (!style)
...
static inline float DimensionForViewportUnit(const SVGElement* context, CSSPrimitiveValue::UnitType unit)
{
    ...
    case CSSPrimitiveValue::UnitType::kViewportWidth:
        return ViewportLengthPercent(viewport_size.width()) / style->EffectiveZoom();

    case CSSPrimitiveValue::UnitType::kViewportHeight:
        return ViewportLengthPercent(viewport_size.height()) / style->EffectiveZoom();
...
gfx::RectF SVGLengthContext::ResolveRectangle(const SVGElement* context, SVGUnitTypes::SVGUnitType type, const gfx::RectF& viewport, const SVGLength& x,
    const SVGLength& y, const SVGLength& width, const SVGLength& height)
{
    DCHECK_NE(SVGUnitTypes::kSvgUnitTypeUnknown, type);
    if (type != SVGUnitTypes::kSvgUnitTypeUserspaceonuse) {
        return gfx::RectF(ConvertValueFromPercentageToUserUnits(x, viewport.width()) + viewport.x(),
            ConvertValueFromPercentageToUserUnits(y, viewport.height()) + viewport.y(), ConvertValueFromPercentageToUserUnits(width, viewport.width()),
            ConvertValueFromPercentageToUserUnits(height, viewport.height()));
    }

    SVGLengthContext length_context(context);
    return gfx::RectF(x.Value(length_context), y.Value(length_context), width.Value(length_context), height.Value(length_context));
}

gfx::PointF SVGLengthContext::ResolvePoint(const SVGElement* context, SVGUnitTypes::SVGUnitType type, const SVGLength& x, const SVGLength& y)
{
    DCHECK_NE(SVGUnitTypes::kSvgUnitTypeUnknown, type);
    if (type == SVGUnitTypes::kSvgUnitTypeUserspaceonuse) {
        SVGLengthContext length_context(context);
...
gfx::Vector2dF SVGLengthContext::ResolveLengthPair(const Length& x_length, const Length& y_length, const ComputedStyle& style) const
{
    ...
    return gfx::Vector2dF(ValueForLength(x_length, zoom, viewport_size.width()), ValueForLength(y_length, zoom, viewport_size.height()));
}
...
float SVGLengthContext::ValueForLength(const UnzoomedLength& unzoomed_length, SVGLengthMode mode) const
{
...
float SVGLengthContext::ConvertValueToUserUnits(float value, SVGLengthMode mode, CSSPrimitiveValue::UnitType from_unit) const
{
    ...
    case CSSPrimitiveValue::UnitType::kEms:
        user_units = ConvertValueFromEMSToUserUnits(ComputedStyleForLengthResolving(context_), value);
        break;
    ...
    case CSSPrimitiveValue::UnitType::kRems:
        user_units = ConvertValueFromEMSToUserUnits(RootElementStyle(context_), value);
        break;
...
float SVGLengthContext::ConvertValueFromUserUnits(float value, SVGLengthMode mode, CSSPrimitiveValue::UnitType to_unit) const
{
    ...
    case CSSPrimitiveValue::UnitType::kEms:
        return ConvertValueFromUserUnitsToEMS(ComputedStyleForLengthResolving(context_), value);
    case CSSPrimitiveValue::UnitType::kExs:
...
float SVGLengthContext::ConvertValueFromUserUnitsToCHS(float value) const
{
    ...
    if (!font_data)
        return 0;
    float zero_width = font_data->GetFontMetrics().ZeroWidth() / style->EffectiveZoom();
    if (!zero_width)
        return 0;
...
float SVGLengthContext::ConvertValueFromCHSToUserUnits(float value) const
{
    ...
    if (!font_data)
        return 0;
    return value * font_data->GetFontMetrics().ZeroWidth() / style->EffectiveZoom();
}
...
float SVGLengthContext::ConvertValueFromUserUnitsToICS(float value) const
{
    ...
    if (!font_data)
        return 0;
    float ideographic_full_width = font_data->GetFontMetrics().IdeographicFullWidth().value_or(style->ComputedFontSize()) / style->EffectiveZoom();
...
float SVGLengthContext::ConvertValueFromICSToUserUnits(float value) const
{
    ...
    if (!font_data)
        return 0;
    return value * font_data->GetFontMetrics().IdeographicFullWidth().value_or(style->ComputedFontSize()) / style->EffectiveZoom();
}

float SVGLengthContext::ConvertValueFromUserUnitsToLHS(float value) const
{
    const ComputedStyle* style = ComputedStyleForLengthResolving(context_);
    return value / AdjustForAbsoluteZoom::AdjustFloat(style->ComputedLineHeight(), *style);
}
...
float SVGLengthContext::ConvertValueFromUserUnitsToEXS(float value) const
{
    ...
    float x_height = ceilf(font_data->GetFontMetrics().XHeight() / style->EffectiveZoom());
    if (!x_height)
        return 0;
...
float SVGLengthContext::ConvertValueFromEXSToUserUnits(float value) const
{
    ...
    // coords-units-03-b.svg, if this causes problems in real world cases maybe it
    // would be best to remove this.
    return value * ceilf(font_data->GetFontMetrics().XHeight() / style->EffectiveZoom());
}
...
float SVGLengthContext::ResolveValue(const CSSPrimitiveValue& primitive_value, SVGLengthMode mode) const
{
    const ComputedStyle* style = ComputedStyleForLengthResolving(context_);
    if (!style)
        return 0;
    ...
    DCHECK(context_);
    CSSToLengthConversionData conversion_data = CSSToLengthConversionData(style, style, root_style, context_->GetDocument().GetLayoutView(),
        CSSToLengthConversionData::ContainerSizes(context_->ParentOrShadowHostElement()), 1.0f);
