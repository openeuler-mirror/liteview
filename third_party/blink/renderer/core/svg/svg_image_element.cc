...
SVGImageElement::SVGImageElement(Document& document)
    ...
    , is_default_overridden_intrinsic_size_(
          GetExecutionContext() && !GetExecutionContext()->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kUnsizedMedia))
...
ScriptPromise SVGImageElement::decode(ScriptState* script_state, ExceptionState& exception_state)
{
    return GetImageLoader().Decode(script_state, exception_state);
}

void SVGImageElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
...
LayoutObject* SVGImageElement::CreateLayoutObject(const ComputedStyle&, LegacyLayout)
{
...
Node::InsertionNotificationRequest SVGImageElement::InsertedInto(ContainerNode& root_parent)
{
