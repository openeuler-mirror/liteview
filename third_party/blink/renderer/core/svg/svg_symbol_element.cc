...
void SVGSymbolElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    if (SVGFitToViewBox::IsKnownAttribute(params.name))
        InvalidateInstances();
}

LayoutObject* SVGSymbolElement::CreateLayoutObject(const ComputedStyle&, LegacyLayout)
{
