...
public:
    ...
    template <typename T> static gfx::RectF ResolveRectangle(const T* context, SVGUnitTypes::SVGUnitType type, const gfx::RectF& viewport)
    {
        return ResolveRectangle(context, type, viewport, *context->x()->CurrentValue(), *context->y()->CurrentValue(), *context->width()->CurrentValue(),
            *context->height()->CurrentValue());
    }

    static gfx::RectF ResolveRectangle(const SVGElement*, SVGUnitTypes::SVGUnitType, const gfx::RectF& viewport, const SVGLength& x, const SVGLength& y,
        const SVGLength& width, const SVGLength& height);
    static gfx::PointF ResolvePoint(const SVGElement*, SVGUnitTypes::SVGUnitType, const SVGLength& x, const SVGLength& y);
    static float ResolveLength(const SVGElement*, SVGUnitTypes::SVGUnitType, const SVGLength&);
    gfx::Vector2dF ResolveLengthPair(const Length& x_length, const Length& y_length, const ComputedStyle&) const;
    ...
    float ValueForLength(const UnzoomedLength&, SVGLengthMode = SVGLengthMode::kOther) const;
    float ValueForLength(const Length&, const ComputedStyle&, SVGLengthMode = SVGLengthMode::kOther) const;
