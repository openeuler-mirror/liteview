...
void SVGGradientElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == svg_names::kGradientTransformAttr) {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTransform, *gradient_transform_->CurrentValue()->CssValue());
        return;
    }
...
void SVGGradientElement::InvalidateGradient(LayoutInvalidationReasonForTracing reason)
{
...
void SVGGradientElement::InvalidateDependentGradients()
{
    NotifyIncomingReferences([](SVGElement& element) {
        if (auto* gradient = DynamicTo<SVGGradientElement>(element)) {
            gradient->InvalidateGradient(layout_invalidation_reason::kSvgResourceInvalidated);
        }
    });
}
