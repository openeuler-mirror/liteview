...
static AffineTransform MakeMapBetweenRects(const gfx::RectF& source, const gfx::RectF& dest)
{
...
static absl::optional<AffineTransform> ComputeViewportAdjustmentTransform(const SVGElement* element, const gfx::RectF& target_rect)
{
...
gfx::RectF FEImage::MapInputs(const gfx::RectF&) const
{
    ...
    if (const LayoutObject* layout_object = ReferencedLayoutObject()) {
        ...
        if (element_->HasRelativeLengths()) {
            auto viewport_transform = ComputeViewportAdjustmentTransform(element_, dest_rect);
...
WTF::TextStream& FEImage::ExternalRepresentation(WTF::TextStream& ts, int indent) const
{
    gfx::Size image_size;
    if (image_) {
...
static gfx::RectF IntersectWithFilterRegion(const Filter* filter, const gfx::RectF& rect)
{
    gfx::RectF filter_region = filter->FilterRegion();
    // Workaround for crbug.com/512453.
    if (filter_region.IsEmpty())
        return rect;
    return IntersectRects(rect, filter->MapLocalRectToAbsoluteRect(filter_region));
}

sk_sp<PaintFilter> FEImage::CreateImageFilterForLayoutObject(const LayoutObject& layout_object)
{
    ...
    AffineTransform transform;
    if (element_->HasRelativeLengths()) {
        auto viewport_transform = ComputeViewportAdjustmentTransform(element_, dst_rect);
        if (viewport_transform) {
            src_rect = viewport_transform->MapRect(src_rect);
    ...
    cc::PaintCanvas* canvas = paint_recorder.beginRecording(gfx::RectFToSkRect(crop_rect));
    ...
    return sk_make_sp<RecordPaintFilter>(paint_recorder.finishRecordingAsPicture(), gfx::RectFToSkRect(crop_rect));
}
