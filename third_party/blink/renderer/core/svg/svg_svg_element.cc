...
void SVGSVGElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    ...
    if (property == x_) {
        AddPropertyToPresentationAttributeStyle(style, property->CssPropertyId(), x_->CssValue());
    } else if (property == y_) {
        AddPropertyToPresentationAttributeStyle(style, property->CssPropertyId(), y_->CssValue());
    } else if (IsOutermostSVGSVGElement() && (property == width_ || property == height_)) {
        ...
        if (property == width_) {
            if (const CSSValue* width = width_->NonNegativeCssValue()) {
                AddPropertyToPresentationAttributeStyle(style, property->CssPropertyId(), *width);
            }
        } else if (property == height_) {
            if (const CSSValue* height = height_->NonNegativeCssValue()) {
                AddPropertyToPresentationAttributeStyle(style, property->CssPropertyId(), *height);
            }
        }
    } else {
    ...
    if (width_or_height_changed || attr_name == svg_names::kXAttr || attr_name == svg_names::kYAttr) {
        ...
        if (width_or_height_changed) {
            ...
            if (!layout_object || layout_object->IsSVGRoot()) {
                ...
                SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kSVGContainerSizeChange));
                if (layout_object)
...
// (because of IsEmpty()).
static bool IntersectsAllowingEmpty(const gfx::RectF& r1, const gfx::RectF& r2)
{
...
{
    return layout_object->IsSVGShape() || layout_object->IsSVGText() || layout_object->IsNGSVGText() || layout_object->IsSVGImage()
        || IsA<SVGUseElement>(*layout_object->GetNode());
}

bool SVGSVGElement::CheckIntersectionOrEnclosure(const SVGElement& element, const gfx::RectF& rect, GeometryMatchingMode mode) const
{
...
StaticNodeList* SVGSVGElement::CollectIntersectionOrEnclosureList(const gfx::RectF& rect, SVGElement* reference_element, GeometryMatchingMode mode) const
{
...
StaticNodeList* SVGSVGElement::getIntersectionList(SVGRectTearOff* rect, SVGElement* reference_element) const
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);

    return CollectIntersectionOrEnclosureList(rect->Target()->Rect(), reference_element, kCheckIntersection);
}

StaticNodeList* SVGSVGElement::getEnclosureList(SVGRectTearOff* rect, SVGElement* reference_element) const
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);

    return CollectIntersectionOrEnclosureList(rect->Target()->Rect(), reference_element, kCheckEnclosure);
}
...
bool SVGSVGElement::checkIntersection(SVGElement* element, SVGRectTearOff* rect) const
{
    ...
    return CheckIntersectionOrEnclosure(*element, rect->Target()->Rect(), kCheckIntersection);
}
...
bool SVGSVGElement::checkEnclosure(SVGElement* element, SVGRectTearOff* rect) const
{
    ...
    return CheckIntersectionOrEnclosure(*element, rect->Target()->Rect(), kCheckEnclosure);
}
...
AffineTransform SVGSVGElement::LocalCoordinateSpaceTransform(CTMScope mode) const
{
    ...
    if (!IsOutermostSVGSVGElement()) {
        ...
        if (LayoutObject* layout_object = GetLayoutObject()) {
            ...
            matrix.PreConcat(TransformationMatrix(To<LayoutSVGRoot>(layout_object)->LocalToBorderBoxTransform()));
...
LayoutObject* SVGSVGElement::CreateLayoutObject(const ComputedStyle&, LegacyLayout)
{
    if (IsOutermostSVGSVGElement())
...
{
    return GetLayoutObject() && GetLayoutObject()->IsSVGRoot() && To<LayoutSVGRoot>(GetLayoutObject())->IsEmbeddedThroughSVGImage();
}
...
gfx::SizeF SVGSVGElement::CurrentViewportSize() const
{
    ...
    if (layout_object->IsSVGRoot()) {
        ...
        return gfx::SizeF(content_size.Width() / zoom, content_size.Height() / zoom);
    }

    gfx::RectF viewport_rect = To<LayoutSVGViewportContainer>(GetLayoutObject())->Viewport();
...
void SVGSVGElement::SetupInitialView(const String& fragment_identifier, Element* anchor_node)
{
