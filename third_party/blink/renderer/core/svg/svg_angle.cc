...
SVGParsingError SVGAngle::SetValueAsString(const String& value)
{
    ...
    SVGParsingError error = WTF::VisitCharacters(
        value, [&](const auto* chars, unsigned length) { return ParseValue(chars, chars + length, value_in_specified_units, unit_type); });
