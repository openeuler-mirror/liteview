...
SpeculationRule* ParseSpeculationRule(JSONObject* input, const KURL& base_url)
{
    ...
    return MakeGarbageCollected<SpeculationRule>(std::move(urls), requires_anonymous_client_ip, target_hint);
}

} // namespace

// static
SpeculationRuleSet* SpeculationRuleSet::Parse(const String& source_text, const KURL& base_url, String* out_error)
{
    ...
    if (!parsed) {
        if (out_error) {
            *out_error = parse_error.type != JSONParseErrorType::kNoError ? parse_error.message : "Parsed JSON must be an object.";
        }
        return nullptr;
    }
    ...
    // each...
    parse_for_action("prefetch_with_subresources", result->prefetch_with_subresources_rules_, false);
