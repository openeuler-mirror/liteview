...
bool AcceptableReferrerPolicy(mojom::blink::SpeculationAction action, const Referrer& referrer)
{
...
{
    return "Ignored attempt to prefetch " + url.ElidedString() + " due to unacceptable referrer policy ("
...
void DocumentSpeculationRules::QueueUpdateSpeculationCandidates()
{
    ...
    execution_context->GetAgent()->event_loop()->EnqueueMicrotask(
        WTF::BindOnce(&DocumentSpeculationRules::UpdateSpeculationCandidates, WrapWeakPersistent(this)));
}
...
void DocumentSpeculationRules::UpdateSpeculationCandidates()
{
    ...
    network::mojom::ReferrerPolicy referrer_policy = execution_context->GetReferrerPolicy();
    String outgoing_referrer = execution_context->OutgoingReferrer();

    Vector<mojom::blink::SpeculationCandidatePtr> candidates;
    auto push_candidates = [&candidates, &referrer_policy, &outgoing_referrer, &execution_context](
                               mojom::blink::SpeculationAction action, const HeapVector<Member<SpeculationRule>>& rules) {
            ...
            for (const KURL& url : rule->urls()) {
                ...
                if (!AcceptableReferrerPolicy(action, referrer)) {
                    execution_context->AddConsoleMessage(
                        mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, MakeReferrerWarning(url, referrer));
                    continue;
                }

                auto referrer_ptr = mojom::blink::Referrer::New(KURL(referrer.referrer), referrer.referrer_policy);
                candidates.push_back(
                    mojom::blink::SpeculationCandidate::New(url, action, std::move(referrer_ptr), rule->requires_anonymous_client_ip_when_cross_origin(),
                        rule->target_browsing_context_name_hint().value_or(mojom::blink::SpeculationTargetHint::kNoHint)));
            }
        }
    };
    ...
    for (SpeculationRuleSet* rule_set : rule_sets_) {
        ...
        // speculation rules.
        if (RuntimeEnabledFeatures::SpeculationRulesPrefetchProxyEnabled(execution_context)) {
            push_candidates(mojom::blink::SpeculationAction::kPrefetch, rule_set->prefetch_rules());
        }
        ...
        if (RuntimeEnabledFeatures::SpeculationRulesPrefetchWithSubresourcesEnabled(execution_context)) {
            push_candidates(mojom::blink::SpeculationAction::kPrefetchWithSubresources, rule_set->prefetch_with_subresources_rules());
        }
        ...
        if (RuntimeEnabledFeatures::Prerender2Enabled(execution_context)) {
            push_candidates(mojom::blink::SpeculationAction::kPrerender, rule_set->prerender_rules());
