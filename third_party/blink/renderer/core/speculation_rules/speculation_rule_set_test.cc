...
{
    return SpeculationRuleSet::Parse(String::Format(R"({
        "prefetch": [{
          "source": "list",
...
{
    String parse_error;
    auto* rule_set = SpeculationRuleSet::Parse("[invalid]", KURL("https://example.com"), &parse_error);
...
{
    String parse_error;
    auto* rule_set = SpeculationRuleSet::Parse("42", KURL("https://example.com"), &parse_error);
    EXPECT_FALSE(rule_set);
...
{
    auto* rule_set = SpeculationRuleSet::Parse(R"({"prefetch": [)"
        ...
        KURL("https://example.com/"));
    ASSERT_TRUE(rule_set);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesListOfURLs("https://example.com/valid.html")));
}
...
template <typename F> void PropagateRulesToStubSpeculationHost(DummyPageHolder& page_holder, StubSpeculationHost& speculation_host, const F& functor)
{
    // A <script> with a case-insensitive type match should be propagated to the
    // browser via Mojo.
    // TODO(jbroman): Should we need to enable script? Should that be bypassed?
