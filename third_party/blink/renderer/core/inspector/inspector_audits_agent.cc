...
InspectorAuditsAgent::InspectorAuditsAgent(InspectorNetworkAgent* network_agent, InspectorIssueStorage* storage, InspectedFrames* inspected_frames)
    ...
    Maybe<protocol::Binary>* out_body, int* out_original_size, int* out_encoded_size)
{
    ...
    Response response = network_agent_->GetResponseBody(request_id, &body, &is_base64_encoded);
    ...
    if (!EncodeAsImage(base64_decoded_buffer.data(), base64_decoded_buffer.size(), encoding, quality.fromMaybe(kDefaultEncodeQuality), &encoded_image)) {
...
Response InspectorAuditsAgent::checkContrast(protocol::Maybe<bool> report_aaa)
{
    ...
    CheckContrastForDocument(main_window->document(), report_aaa.fromMaybe(false));
