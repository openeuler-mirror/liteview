...
InspectorEmulationAgent::InspectorEmulationAgent(WebLocalFrameImpl* web_local_frame_impl)
...
Response InspectorEmulationAgent::setTouchEmulationEnabled(bool enabled, protocol::Maybe<int> max_touch_points)
{
    Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
Response InspectorEmulationAgent::setEmulatedMedia(Maybe<String> media, Maybe<protocol::Array<protocol::Emulation::MediaFeature>> features)
{
    Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    if (features.isJust()) {
        ...
        if (forced_colors_value == "active") {
            if (!forced_colors_override_) {
                initial_system_color_info_state_ = WebThemeEngineHelper::GetNativeThemeEngine()->GetSystemColorInfo();
            }
            ...
            WebThemeEngineHelper::GetNativeThemeEngine()->OverrideForcedColorsTheme(is_dark_mode);
        } else if (forced_colors_value == "none") {
            if (!forced_colors_override_) {
                initial_system_color_info_state_ = WebThemeEngineHelper::GetNativeThemeEngine()->GetSystemColorInfo();
            }
            forced_colors_override_ = true;
            WebThemeEngineHelper::GetNativeThemeEngine()->SetForcedColors(ForcedColors::kNone);
        } else if (forced_colors_override_) {
            WebThemeEngineHelper::GetNativeThemeEngine()->ResetToSystemColors(initial_system_color_info_state_);
        }
...
Response InspectorEmulationAgent::setEmulatedVisionDeficiency(const String& type)
{
    Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
Response InspectorEmulationAgent::setAutoDarkModeOverride(Maybe<bool> enabled)
{
    ...
    if (enabled.isJust()) {
        ...
        GetWebViewImpl()->GetDevToolsEmulator()->SetAutoDarkModeOverride(enabled.fromJust());
    } else {
...
Response InspectorEmulationAgent::setVirtualTimePolicy(const String& policy, Maybe<double> virtual_time_budget_ms,
    protocol::Maybe<int> max_virtual_time_task_starvation_count, protocol::Maybe<double> initial_virtual_time, double* virtual_time_ticks_base_ms)
{
    ...
    if (protocol::Emulation::VirtualTimePolicyEnum::Advance == policy) {
        ...
        if (virtual_time_budget_ms.isJust()) {
            return Response::InvalidParams("Can only specify budget for non-Pause policy");
        }
        if (max_virtual_time_task_starvation_count.isJust()) {
            return Response::InvalidParams("Can only specify starvation count for non-Pause policy");
        }
    }
    ...
    virtual_time_task_starvation_count_.Set(max_virtual_time_task_starvation_count.fromMaybe(0));

    InnerEnable();

    VirtualTimeController* virtual_time_controller = web_local_frame_->View()->Scheduler()->GetVirtualTimeController();
    // This needs to happen before we apply virtual time.
    base::Time initial_time = initial_virtual_time.isJust() ? base::Time::FromDoubleT(initial_virtual_time.fromJust()) : base::Time();
    virtual_time_base_ticks_ = virtual_time_controller->EnableVirtualTime(initial_time);
    virtual_time_controller->SetVirtualTimePolicy(scheduler_policy);
    if (virtual_time_budget_ms.fromMaybe(0) > 0) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("renderer.scheduler", "VirtualTimeBudget", TRACE_ID_LOCAL(this), "budget", virtual_time_budget_ms.fromJust());
        const base::TimeDelta budget_amount = base::Milliseconds(virtual_time_budget_ms.fromJust());
    ...
    if (max_virtual_time_task_starvation_count.fromMaybe(0)) {
        virtual_time_controller->SetMaxVirtualTimeTaskStarvationCount(max_virtual_time_task_starvation_count.fromJust());
    }
...
{
    if (!accept_language_override_.Get().empty() && request.HttpHeaderField("Accept-Language").empty()) {
        request.SetHttpHeaderField("Accept-Language", AtomicString(network_utils::GenerateAcceptLanguageHeader(accept_language_override_.Get())));
    }
...
Response InspectorEmulationAgent::setNavigatorOverrides(const String& platform)
{
    Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
void InspectorEmulationAgent::VirtualTimeBudgetExpired()
{
    ...
    view->Scheduler()->GetVirtualTimeController()->SetVirtualTimePolicy(VirtualTimeController::VirtualTimePolicy::kPause);
...
Response InspectorEmulationAgent::setDefaultBackgroundColorOverride(Maybe<protocol::DOM::RGBA> color)
{
    Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
Response InspectorEmulationAgent::setDeviceMetricsOverride(int width, int height, double device_scale_factor, bool mobile, Maybe<double> scale,
...
Response InspectorEmulationAgent::setHardwareConcurrencyOverride(int hardware_concurrency)
{
    if (hardware_concurrency <= 0) {
        return Response::InvalidParams("HardwareConcurrency must be a positive number");
    }
    InnerEnable();
...
Response InspectorEmulationAgent::setUserAgentOverride(const String& user_agent, protocol::Maybe<String> accept_language, protocol::Maybe<String> platform,
    protocol::Maybe<protocol::Emulation::UserAgentMetadata> ua_metadata_override)
{
    ...
    if (ua_metadata_override.isJust()) {
        ...
        if (user_agent.empty()) {
            ...
            return Response::InvalidParams("Can't specify UserAgentMetadata but no UA string");
        }
        std::unique_ptr<protocol::Emulation::UserAgentMetadata> ua_metadata = ua_metadata_override.takeJust();
...
Response InspectorEmulationAgent::setLocaleOverride(protocol::Maybe<String> maybe_locale)
{
    ...
    if (locale_override_.Get().empty() && LocaleController::instance().has_locale_override()) {
        return Response::ServerError("Another locale override is already in effect");
    }
...
Response InspectorEmulationAgent::setTimezoneOverride(const String& timezone_id)
{
    if (timezone_id.empty()) {
        ...
        if (!timezone_override_) {
            return TimeZoneController::HasTimeZoneOverride() ? Response::ServerError("Timezone override is already in effect")
...
void InspectorEmulationAgent::ApplyUserAgentMetadataOverride(absl::optional<blink::UserAgentMetadata>* ua_metadata)
{
...
{
    if (!web_local_frame_) {
        return Response::ServerError("Operation is only supported for pages, not workers");
    }
    return Response::Success();
}
...
protocol::Response InspectorEmulationAgent::setDisabledImageTypes(std::unique_ptr<protocol::Array<protocol::Emulation::DisabledImageType>> disabled_types)
{
    ...
    for (protocol::Emulation::DisabledImageType type : *disabled_types) {
        if (DisabledImageTypeEnum::Avif == type || DisabledImageTypeEnum::Jxl == type || DisabledImageTypeEnum::Webp == type) {
