...
{
    PhysicalRect local_coords_text_box_rect = layout_object->FlipForWritingMode(text_box.local_rect);
    PhysicalRect absolute_coords_text_box_rect = layout_object->LocalToAbsoluteRect(local_coords_text_box_rect);
    LocalFrameView* local_frame_view = layout_object->GetFrameView();
...
Response InspectorDOMSnapshotAgent::getSnapshot(std::unique_ptr<protocol::Array<String>> style_filter, protocol::Maybe<bool> include_event_listeners,
    ...
    std::unique_ptr<protocol::Array<String>>* strings)
{
    ...
    include_blended_background_colors_ = include_blended_background_colors.fromMaybe(false);
...
void InspectorDOMSnapshotAgent::VisitNode(Node* node, int parent_index, InspectorContrast& contrast)
{
    ...
    if (element) {
        ...
        if (auto* input_element = DynamicTo<HTMLInputElement>(*element)) {
            ...
            if ((input_element->type() == input_type_names::kRadio) || (input_element->type() == input_type_names::kCheckbox)) {
        ...
        if (element->GetPseudoId()) {
            SetRare(nodes->getPseudoType(nullptr), index, InspectorDOMAgent::ProtocolPseudoElementType(element->GetPseudoId()));
...
{
    for (PseudoId pseudo_id : { kPseudoIdFirstLetter, kPseudoIdBefore, kPseudoIdAfter, kPseudoIdMarker }) {
...
int InspectorDOMSnapshotAgent::BuildLayoutTreeNode(LayoutObject* layout_object, Node* node, int node_index, InspectorContrast& contrast)
{
    ...
    String text = layout_object->IsText() ? To<LayoutText>(layout_object)->GetText() : String();
...
std::unique_ptr<protocol::Array<int>> InspectorDOMSnapshotAgent::BuildStylesForNode(Node* node)
{
    ...
    for (const auto* property : *css_property_filter_) {
        const CSSValue* value = property->CSSValueFromComputedStyle(*style, layout_object, /* allow_visited_style= */ true);
        if (!value) {
