...
void InspectorLogAgent::ConsoleMessageAdded(ConsoleMessage* message)
{
    ...
    if (message->Source() == mojom::blink::ConsoleMessageSource::kWorker && !message->WorkerId().empty())
        entry->setWorkerId(message->WorkerId());
    if (message->Source() == mojom::blink::ConsoleMessageSource::kNetwork && !message->RequestIdentifier().IsNull()) {
...
Response InspectorLogAgent::startViolationsReport(std::unique_ptr<protocol::Array<ViolationSetting>> settings)
{
    if (!enabled_.Get())
        return Response::ServerError("Log is not enabled");
    if (!performance_monitor_) {
        return Response::ServerError("Violations are not supported for this target");
    }
