...
private:
    ...
    void AppendPathCommandAndPoints(const char* command, const gfx::PointF points[], size_t length);

    std::unique_ptr<protocol::ListValue> path_;
};

void PathBuilder::AppendPathCommandAndPoints(const char* command, const gfx::PointF points[], size_t length)
{
...
void AppendStyleInfo(
    Node* node, protocol::DictionaryValue* element_info, const InspectorHighlightContrastInfo& node_contrast, const ContrastAlgorithm& contrast_algorithm)
{
    std::unique_ptr<protocol::DictionaryValue> computed_style = protocol::DictionaryValue::create();
    CSSComputedStyleDeclaration* style = MakeGarbageCollected<CSSComputedStyleDeclaration>(node, true);
    Vector<CSSPropertyID> properties;

    // For text nodes, we can show color & font properties.
    bool has_text_children = false;
    for (Node* child = node->firstChild(); !has_text_children && child; child = child->nextSibling()) {
...
std::unique_ptr<protocol::DictionaryValue> BuildTextNodeInfo(Text* text_node)
{
    ...
    if (!layout_object || !layout_object->IsText())
        return text_info;
    PhysicalRect bounding_box = To<LayoutText>(layout_object)->PhysicalVisualOverflowRect();
...
void AppendLineStyleConfig(const absl::optional<LineStyle>& line_style, std::unique_ptr<protocol::DictionaryValue>& parent_config, String line_name)
{
...
void AppendBoxStyleConfig(const absl::optional<BoxStyle>& box_style, std::unique_ptr<protocol::DictionaryValue>& parent_config, String box_name)
{
    ...
    float scale, LayoutUnit rtl_offset, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    ...
    wtf_size_t first_explicit_index = grid_interface->ExplicitGridStartForDirection(direction);
    ...
    for (wtf_size_t i = first_explicit_index; i < track_count; ++i) {
        ...
        LayoutUnit offset = GetPositionForTrackAt(layout_object, i, direction, positions);
        if (is_rtl && direction == kForColumns)
            offset += rtl_offset;
    ...
    float scale, LayoutUnit rtl_offset, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    ...
    wtf_size_t track_count = positions.size();
    LayoutUnit alt_axis_pos = GetPositionForLastTrack(layout_object, direction == kForRows ? kForColumns : kForRows, alt_axis_positions);
    if (is_rtl && direction == kForRows)
        alt_axis_pos += rtl_offset;
    ...
    size_t explicit_grid_end_track_count = grid_interface->ExplicitGridEndForDirection(direction);

    {
        LayoutUnit first_offset = GetPositionForFirstTrack(layout_object, direction, positions);
        if (is_rtl && direction == kForColumns)
...
{
    return layout_object.StyleRef().IsDisplayFlexibleBox() && layout_object.IsLayoutNGFlexibleBox();
}

bool IsLayoutNGFlexItem(const LayoutObject& layout_object)
{
    return !layout_object.GetNode()->IsDocumentNode() && IsLayoutNGFlexibleBox(*layout_object.Parent()) && To<LayoutBox>(layout_object).IsFlexItemIncludingNG();
}
...
std::unique_ptr<protocol::DictionaryValue> BuildAreaNamePaths(Node* node, float scale, const Vector<LayoutUnit>& rows, const Vector<LayoutUnit>& columns)
{
    ...
    for (const auto& item : grid_area_map) {
        ...
        LayoutUnit start_column = GetPositionForTrackAt(layout_object, area.columns.StartLine(), kForColumns, columns);
        LayoutUnit end_column = GetPositionForTrackAt(layout_object, area.columns.EndLine(), kForColumns, columns);
        LayoutUnit start_row = GetPositionForTrackAt(layout_object, area.rows.StartLine(), kForRows, rows);
        LayoutUnit end_row = GetPositionForTrackAt(layout_object, area.rows.EndLine(), kForRows, rows);

        // Only subtract the gap size if the end line isn't the last line in the
        // container.
        LayoutUnit row_gap_offset = area.rows.EndLine() == rows.size() - 1 ? LayoutUnit() : row_gap;
        LayoutUnit column_gap_offset = area.columns.EndLine() == columns.size() - 1 ? LayoutUnit() : column_gap;
        if (is_rtl)
            column_gap_offset *= -1;
    ...
    Node* node, GridTrackSizingDirection direction, float scale, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    ...
    auto process_grid_lines_map = [&](const NamedGridLinesMap& named_lines_map) {
        for (const auto& item : named_lines_map) {
            ...
            for (const wtf_size_t index : item.value) {
                LayoutUnit track = GetPositionForTrackAt(layout_object, index, direction, positions);
    ...
    const NamedGridLinesMap& explicit_lines_map
        = (direction == kForColumns) ? grid_container_style.GridTemplateColumns().named_grid_lines : grid_container_style.GridTemplateRows().named_grid_lines;

    const NamedGridLinesMap& implicit_lines_map
        = (direction == kForColumns) ? grid_container_style.ImplicitNamedGridColumnLines() : grid_container_style.ImplicitNamedGridRowLines();
...
Vector<String> GetAuthoredGridTrackSizes(const CSSValue* value, size_t auto_repeat_count)
{
    ...
    for (auto list_value : *value_list) {
        if (auto* grid_auto_repeat_value = DynamicTo<cssvalue::CSSGridAutoRepeatValue>(list_value.Get())) {
...
DevtoolsFlexInfo GetFlexLinesAndItems(LayoutBox* layout_box, bool is_horizontal, bool is_reverse)
{
    ...
    for (const auto& fragment : layout_box->PhysicalFragments()) {
        ...
        for (const auto& child : fragment.Children()) {
            ...
            LayoutUnit baseline = NGBoxFragment(layout_box->StyleRef().GetWritingDirection(), *To<NGPhysicalBoxFragment>(child_fragment))
...
std::unique_ptr<protocol::DictionaryValue> BuildFlexContainerInfo(
    Node* node, const InspectorFlexContainerHighlightConfig& flex_container_highlight_config, float scale)
{
    CSSComputedStyleDeclaration* style = MakeGarbageCollected<CSSComputedStyleDeclaration>(node, true);
    LocalFrameView* containing_view = node->GetDocument().View();
    LayoutObject* layout_object = node->GetLayoutObject();
    ...
    bool is_reverse = layout_object->StyleRef().ResolvedIsRowReverseFlexDirection() || layout_object->StyleRef().ResolvedIsColumnReverseFlexDirection();
...
std::unique_ptr<protocol::DictionaryValue> BuildFlexItemInfo(Node* node, const InspectorFlexItemHighlightConfig& flex_item_highlight_config, float scale)
{
    std::unique_ptr<protocol::DictionaryValue> flex_info = protocol::DictionaryValue::create();
...
std::unique_ptr<protocol::DictionaryValue> BuildGridInfo(Node* node, const InspectorGridHighlightConfig& grid_highlight_config, float scale, bool isPrimary)
{
    LocalFrameView* containing_view = node->GetDocument().View();
    LayoutObject* layout_object = node->GetLayoutObject();
    DCHECK(layout_object);
    ...
    grid_info->setString("writingMode", GetWritingMode(layout_object->StyleRef()));

    auto row_gap = grid_interface->GridGap(kForRows) + grid_interface->GridItemOffset(kForRows);
    auto column_gap = grid_interface->GridGap(kForColumns) + grid_interface->GridItemOffset(kForColumns);
    ...
    if (layout_object->IsLayoutNGGrid()) {
        ...
        rtl_offset = layout_box->LogicalWidth() - columns.back() - layout_box->BorderAndPaddingLogicalRight();
    }
    ...
    if (grid_highlight_config.show_track_sizes) {
        ...
        Vector<String> column_authored_values
            = GetAuthoredGridTrackSizes(FindCSSValue(CSSPropertyID::kGridTemplateColumns), grid_interface->AutoRepeatCountForDirection(kForColumns));
        Vector<String> row_authored_values
            = GetAuthoredGridTrackSizes(FindCSSValue(CSSPropertyID::kGridTemplateRows), grid_interface->AutoRepeatCountForDirection(kForRows));

        grid_info->setValue("columnTrackSizes", BuildGridTrackSizes(node, kForColumns, scale, column_gap, rtl_offset, columns, rows, &column_authored_values));
        grid_info->setValue("rowTrackSizes", BuildGridTrackSizes(node, kForRows, scale, row_gap, rtl_offset, rows, columns, &row_authored_values));
    }
    ...
    for (wtf_size_t i = 1; i < columns.size(); ++i) {
        ...
        LayoutUnit line_left = GetPositionForTrackAt(layout_object, i - 1, kForColumns, columns);
        if (!is_ltr) {
        ...
        // Column Gaps
        if (i != columns.size() - 1) {
            LayoutUnit gap_left = GetPositionForTrackAt(layout_object, i, kForColumns, columns);
            if (is_ltr)
                gap_left -= column_gap;
            else
                gap_left += rtl_offset;
    ...
    if (grid_highlight_config.show_positive_line_numbers) {
        grid_info->setValue("positiveRowLineNumberPositions", BuildGridPositiveLineNumberPositions(node, row_gap, kForRows, scale, rtl_offset, rows, columns));
        grid_info->setValue(
            "positiveColumnLineNumberPositions", BuildGridPositiveLineNumberPositions(node, column_gap, kForColumns, scale, rtl_offset, columns, rows));
    }

    // Negative Row and column Line positions
    if (grid_highlight_config.show_negative_line_numbers) {
        grid_info->setValue("negativeRowLineNumberPositions", BuildGridNegativeLineNumberPositions(node, row_gap, kForRows, scale, rtl_offset, rows, columns));
        grid_info->setValue(
            "negativeColumnLineNumberPositions", BuildGridNegativeLineNumberPositions(node, column_gap, kForColumns, scale, rtl_offset, columns, rows));
    }

    // Area names
    if (grid_highlight_config.show_area_names) {
        grid_info->setValue("areaNames", BuildAreaNamePaths(node, scale, rows, columns));
    }

    // line names
    if (grid_highlight_config.show_line_names) {
        grid_info->setValue("rowLineNameOffsets", BuildGridLineNames(node, kForRows, scale, rows, columns));
        grid_info->setValue("columnLineNameOffsets", BuildGridLineNames(node, kForColumns, scale, columns, rows));
    }
...
std::unique_ptr<protocol::DictionaryValue> BuildGridInfo(Node* node, const InspectorHighlightConfig& highlight_config, float scale, bool isPrimary)
{
    ...
    return BuildGridInfo(node, *(highlight_config.grid_highlight_config), scale, isPrimary);
}
...
{
    PhysicalRect absolute_coords_text_box_rect = layout_object->LocalToAbsoluteRect(layout_object->FlipForWritingMode(text_box.local_rect));
    LocalFrameView* local_frame_view = layout_object->GetFrameView();
    ...
    , color_format_(highlight_config.color_format)
{
    ...
    if (element_info_ && highlight_config.show_styles) {
        AppendStyleInfo(node, element_info_.get(), node_contrast, highlight_config.contrast_algorithm);
    }
...
void InspectorHighlight::AppendDistanceInfo(Node* node)
{
    ...
    for (unsigned i = 0; i < style->length(); ++i) {
        ...
        const CSSValue* value = style->GetPropertyCSSValue(CssPropertyID(node->GetExecutionContext(), name));
        if (!value)
            continue;
        if (value->IsColorValue()) {
            Color color = static_cast<const cssvalue::CSSColor*>(value)->Value();
...
void InspectorHighlight::VisitAndCollectDistanceInfo(Node* node)
{
    ...
    if (auto* element = DynamicTo<Element>(node)) {
        if (element->GetPseudoId()) {
            ...
            for (PseudoId pseudo_id : { kPseudoIdFirstLetter, kPseudoIdBefore, kPseudoIdAfter }) {
...
void InspectorHighlight::AppendNodeHighlight(Node* node, const InspectorHighlightConfig& highlight_config)
{
    ...
    if (highlight_config.flex_container_highlight_config) {
        ...
        if (IsLayoutNGFlexibleBox(*layout_object)) {
            flex_container_info_->pushValue(BuildFlexContainerInfo(node, *(highlight_config.flex_container_highlight_config), scale_));
        }
    }
    ...
    if (highlight_config.flex_item_highlight_config) {
        ...
        if (IsLayoutNGFlexItem(*layout_object)) {
            flex_item_info_->pushValue(BuildFlexItemInfo(node, *(highlight_config.flex_item_highlight_config), scale_));
        }
    }
...
bool InspectorHighlight::GetBoxModel(Node* node, std::unique_ptr<protocol::DOM::BoxModel>* model, bool use_absolute_zoom)
{
    ...
    *model = protocol::DOM::BoxModel::create()
                 ...
                 .setWidth(model_object ? AdjustForAbsoluteZoom::AdjustInt(model_object->PixelSnappedOffsetWidth(model_object->OffsetParent()), model_object)
                                        : bounding_box.width())
                 .setHeight(model_object ? AdjustForAbsoluteZoom::AdjustInt(model_object->PixelSnappedOffsetHeight(model_object->OffsetParent()), model_object)
                                         : bounding_box.height())
                 .build();
...
std::unique_ptr<protocol::DictionaryValue> InspectorGridHighlight(Node* node, const InspectorGridHighlightConfig& config)
{
    ...
    std::unique_ptr<protocol::DictionaryValue> grid_info = BuildGridInfo(node, config, scale, true);
    return grid_info;
}
    ...
    Node* node, const InspectorContainerQueryContainerHighlightConfig& container_query_container_highlight_config, float scale)
{
    ...
    bool include_descendants
        = container_query_container_highlight_config.descendant_border && !container_query_container_highlight_config.descendant_border->IsTransparent();
...
InspectorFlexContainerHighlightConfig InspectorHighlight::DefaultFlexContainerConfig()
{
    ...
    config.container_border = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.line_separator = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.item_separator = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.main_distributed_space = absl::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.cross_distributed_space = absl::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.row_gap_space = absl::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.column_gap_space = absl::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.cross_alignment = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    return config;
}
...
InspectorFlexItemHighlightConfig InspectorHighlight::DefaultFlexItemConfig()
{
    ...
    config.base_size_box = absl::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.base_size_border = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.flexibility_arrow = absl::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    return config;
}
