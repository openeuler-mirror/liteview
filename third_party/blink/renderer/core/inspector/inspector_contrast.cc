    ...
    cssvalue::CSSGradientValue* gradient, Vector<Color>& colors, bool& found_non_transparent_color, bool& found_opaque_color, const LayoutObject& layout_object)
{
    ...
    if (colors.empty()) {
        ...
        for (auto stop_color : stop_colors) {
            found_non_transparent_color = found_non_transparent_color || (stop_color.Alpha() != 0);
...
void InspectorContrast::CollectNodesAndBuildRTreeIfNeeded()
{
    ...
    DeferredShapingController::From(*document_)->ReshapeAllDeferred(ReshapeReason::kInspector);
    if (!layout_view->GetFrameView()->UpdateLifecycleToPrePaintClean(DocumentUpdateReason::kInspector)) {
        return;
    }
    ...
    rtree_.Build(
        elements_, [](const HeapVector<Member<Node>>& items, size_t index) { return ToPixelSnappedRect(GetNodeRect(items[static_cast<wtf_size_t>(index)])); },
        [](const HeapVector<Member<Node>>& items, size_t index) { return items[static_cast<wtf_size_t>(index)]; });
...
ContrastInfo InspectorContrast::GetContrast(Element* top_element)
{
    ...
    float contrast_ratio = color_utils::GetContrastRatio(bgcolors.at(0).Blend(text_color).ToSkColorDeprecated(), bgcolors.at(0).ToSkColorDeprecated());
...
std::vector<Node*> InspectorContrast::ElementsFromRect(const PhysicalRect& rect, Document& document)
{
    CollectNodesAndBuildRTreeIfNeeded();
...
bool InspectorContrast::GetColorsFromRect(PhysicalRect rect, Document& document, Element* top_element, Vector<Color>& colors, float* text_opacity)
{
    ...
    *text_opacity = 1.0f;
    ...
    for (auto e = elements_under_rect.begin(); !found_top_element && e != elements_under_rect.end(); ++e) {
        ...
        if (style->HasOpacity()) {
            background_color = background_color.CombineWithAlpha(background_color.Alpha() / 255 * style->Opacity());
