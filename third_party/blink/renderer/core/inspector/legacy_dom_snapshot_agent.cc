...
struct LegacyDOMSnapshotAgent::VectorStringHashTraits : public WTF::GenericHashTraits<Vector<String>> {
    ...
    {
        new (NotNullTag::kNotNull, &vec) Vector<String>(WTF::kHashTableDeletedValue);
    }
...
Response LegacyDOMSnapshotAgent::GetSnapshot(Document* document, std::unique_ptr<protocol::Array<String>> style_filter,
    ...
    std::unique_ptr<protocol::Array<protocol::DOMSnapshot::ComputedStyle>>* computed_styles)
{
    ...
    // Actual traversal.
    VisitNode(document, include_event_listeners.fromMaybe(false), include_user_agent_shadow_tree.fromMaybe(false));
...
int LegacyDOMSnapshotAgent::VisitNode(Node* node, bool include_event_listeners, bool include_user_agent_shadow_tree)
{
    ...
    if (element) {
        ...
        if (element->GetPseudoId()) {
            value->setPseudoType(InspectorDOMAgent::ProtocolPseudoElementType(element->GetPseudoId()));
        }
...
{
    if (!parent->GetPseudoElement(kPseudoIdFirstLetter) && !parent->GetPseudoElement(kPseudoIdBefore) && !parent->GetPseudoElement(kPseudoIdAfter)) {
        return nullptr;
    }
