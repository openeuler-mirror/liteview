...
bool InspectorAgentState::Deserialize(span<uint8_t> in, WTF::String* v)
{
    ...
    if (tokenizer.TokenTag() == CBORTokenTag::STRING8) {
        *v = WTF::String::FromUTF8(reinterpret_cast<const char*>(tokenizer.GetString8().data()), static_cast<size_t>(tokenizer.GetString8().size()));
        return true;
    }
    if (tokenizer.TokenTag() == CBORTokenTag::STRING16) {
        *v = WTF::String(reinterpret_cast<const UChar*>(tokenizer.GetString16WireRep().data()), tokenizer.GetString16WireRep().size() / 2);
        return true;
    }
    return false;
}
