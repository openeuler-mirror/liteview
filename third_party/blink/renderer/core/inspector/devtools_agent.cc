...
public:
    IOAgent(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, scoped_refptr<InspectorTaskRunner> inspector_task_runner,
        CrossThreadWeakPersistent<::blink::DevToolsAgent> agent, mojo::PendingReceiver<mojom::blink::DevToolsAgent> receiver)
        ...
        const WTF::String& session_id) override
    {
        ...
        inspector_task_runner_->AppendTask(CrossThreadBindOnce(&::blink::DevToolsAgent::AttachDevToolsSessionImpl, agent_, std::move(host),
            std::move(main_session), std::move(io_session), std::move(reattach_session_state), client_expects_binary_responses, client_is_trusted, session_id));
    }
    ...
    void ReportChildTargets(bool report, bool wait_for_debugger, base::OnceClosure callback) override
    {
        ...
        bool did_append_task = inspector_task_runner_->AppendTask(CrossThreadBindOnce(&blink::DevToolsAgent::ReportChildTargetsPostCallbackToIO, agent_, report,
            wait_for_debugger, CrossThreadBindOnce(std::move(split_callback.first))));
    ...
    mojo::PendingReceiver<mojom::blink::DevToolsAgent> receiver, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    io_agent_ = new IOAgent(io_task_runner_, inspector_task_runner_, WrapCrossThreadWeakPersistent(this), std::move(receiver));
}
    ...
    mojom::blink::DevToolsSessionStatePtr reattach_session_state, bool client_expects_binary_responses, bool client_is_trusted, const WTF::String& session_id)
{
    ...
    DevToolsSession* session = MakeGarbageCollected<DevToolsSession>(this, std::move(host), std::move(session_receiver), std::move(io_session_receiver),
        std::move(reattach_session_state), client_expects_binary_responses, client_is_trusted, session_id, inspector_task_runner_->isolate_task_runner());
...
std::unique_ptr<WorkerDevToolsParams> DevToolsAgent::WorkerThreadCreated(ExecutionContext* parent_context, WorkerThread* worker_thread, const KURL& url,
    const String& global_scope_name, const absl::optional<const blink::DedicatedWorkerToken>& token)
{
