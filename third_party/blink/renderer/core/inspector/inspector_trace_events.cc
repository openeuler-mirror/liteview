...
void InspectorTraceEvents::WillSendRequest(DocumentLoader* loader, const KURL& fetch_context_url, const ResourceRequest& request,
    const ResourceResponse& redirect_response, const ResourceLoaderOptions&, ResourceType, RenderBlockingBehavior render_blocking_behavior,
    base::TimeTicks timestamp)
{
    ...
    TRACE_EVENT_INSTANT_WITH_TIMESTAMP1(
        "devtools.timeline", "ResourceSendRequest", TRACE_EVENT_SCOPE_THREAD, timestamp, "data", [&](perfetto::TracedValue ctx) {
            inspector_send_request_event::Data(std::move(ctx), loader, request.InspectorId(), frame, request, render_blocking_behavior);
        });
}
...
void InspectorTraceEvents::DidReceiveData(uint64_t identifier, DocumentLoader* loader, const char* data, uint64_t encoded_data_length)
{
    LocalFrame* frame = loader ? loader->GetFrame() : nullptr;
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceReceivedData", inspector_receive_data_event::Data, loader, identifier, frame, encoded_data_length);
}

void InspectorTraceEvents::DidFinishLoading(uint64_t identifier, DocumentLoader* loader, base::TimeTicks finish_time, int64_t encoded_data_length,
...
void InspectorTraceEvents::Did(const probe::ExecuteScript&)
{
    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "UpdateCounters", TRACE_EVENT_SCOPE_THREAD, "data",
        [](perfetto::TracedValue context) { inspector_update_counters_event::Data(std::move(context)); });
}
...
void InspectorTraceEvents::PaintTiming(Document* document, const char* name, double timestamp)
{
    TRACE_EVENT_MARK_WITH_TIMESTAMP2("loading,rail,devtools.timeline", name, trace_event::ToTraceTimestamp(timestamp), "frame",
        ToTraceValue(document->GetFrame()), "data", [&](perfetto::TracedValue context) { GetNavigationTracingData(std::move(context), document); });
}

void InspectorTraceEvents::FrameStartedLoading(LocalFrame* frame)
{
    TRACE_EVENT_INSTANT1("devtools.timeline", "FrameStartedLoading", TRACE_EVENT_SCOPE_THREAD, "frame", ToTraceValue(frame));
}
...
void inspector_schedule_style_invalidation_tracking_event::RuleSetInvalidation(
    perfetto::TracedValue context, ContainerNode& root_node, const InvalidationSet& invalidation_set)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_style_invalidator_invalidate_event::SelectorPart(
    perfetto::TracedValue context, Element& element, const char* reason, const InvalidationSet& invalidation_set, const String& selector_part)
{
    auto dict = std::move(context).WriteDictionary();
    FillCommonPart(dict, element, reason);
    {
...
void inspector_send_request_event::Data(perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, LocalFrame* frame,
    const ResourceRequest& request, RenderBlockingBehavior render_blocking_behavior)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_paint_event::Data(perfetto::TracedValue context, Frame* frame, const LayoutObject* layout_object, const gfx::QuadF& quad, int layer_id)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_evaluate_script_event::Data(perfetto::TracedValue context, LocalFrame* frame, const String& url, const TextPosition& text_position)
{
    auto dict = std::move(context).WriteDictionary();
...
inspector_compile_script_event::V8ConsumeCacheResult::V8ConsumeCacheResult(int cache_size, bool rejected)
...
void inspector_compile_script_event::Data(perfetto::TracedValue context, const String& url, const TextPosition& text_position,
    absl::optional<V8ConsumeCacheResult> consume_cache_result, bool eager, bool streamed, ScriptStreamer::NotStreamingReason not_streaming_reason)
{
    auto dict = std::move(context).WriteDictionary();
    FillLocation(dict, url, text_position);
...
void inspector_event_dispatch_event::Data(perfetto::TracedValue context, const Event& event)
{
    auto dict = std::move(context).WriteDictionary();
