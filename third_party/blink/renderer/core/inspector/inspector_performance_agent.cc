...
#define INSTANCE_COUNTER_NAME(name) (isPlural(#name, sizeof(#name)) ? #name : #name "s"),
...
protocol::Response InspectorPerformanceAgent::enable(Maybe<String> optional_time_domain)
{
    ...
    if (enabled_.Get()) {
        if (!HasTimeDomain(time_domain)) {
            return Response::ServerError("Cannot change time domain while performance metrics collection is "
                                         "enabled.");
        }
...
{
    if (enabled_.Get()) {
        return Response::ServerError("Cannot set time domain while performance metrics collection"
                                     " is enabled.");
    }
...
Response InspectorPerformanceAgent::InnerSetTimeDomain(const String& time_domain)
{
    DCHECK(!enabled_.Get());
    ...
    if (time_domain == TimeDomain::ThreadTicks) {
        if (!base::ThreadTicks::IsSupported()) {
            return Response::ServerError("Thread time is not supported on this platform.");
        }
...
Response InspectorPerformanceAgent::getMetrics(std::unique_ptr<protocol::Array<protocol::Performance::Metric>>* out_result)
{
    if (!enabled_.Get()) {
