...
public:
    ...
    {
        ResetResult();
        platform()->RunForPeriod(GetWebContentCaptureClient()->GetTaskInitialDelay());
    }

    void RunNextContentCaptureTask()
    {
        ResetResult();
        platform()->RunForPeriod(GetContentCaptureTask()->GetTaskDelayForTesting().GetNextTaskDelay());
    }
...
private:
    ...
    void BuildNodesInfo(const Vector<String>& ids, Vector<Persistent<Node>>& nodes, Vector<cc::NodeInfo>& node_ids)
    {
        for (auto id : ids) {
            ...
            GetOrResetContentCaptureManager()->ScheduleTaskIfNeeded(*node);
            node_ids.push_back(cc::NodeInfo(DOMNodeIds::IdForNode(node), GetRect(layout_object)));
        }
    }
...
private:
    void SetupPage()
    {
      ...
      <div id='d1'></div>
        ...
        auto* child_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
