...
struct ReverseBeaconTimeoutSorter {
    bool operator()(const Member<PendingBeaconDispatcher::PendingBeacon>& lhs, const Member<PendingBeaconDispatcher::PendingBeacon>& rhs)
    {
...
// static
const char PendingBeaconDispatcher::kSupplementName[] = "PendingBeaconDispatcher";

// static
PendingBeaconDispatcher& PendingBeaconDispatcher::FromOrAttachTo(ExecutionContext& ec)
{
    PendingBeaconDispatcher* dispatcher = Supplement::From<PendingBeaconDispatcher>(ec);
    if (!dispatcher) {
        dispatcher = MakeGarbageCollected<PendingBeaconDispatcher>(ec, base::PassKey<PendingBeaconDispatcher>());
...
PendingBeaconDispatcher::PendingBeaconDispatcher(ExecutionContext& ec, base::PassKey<PendingBeaconDispatcher> key)
    : Supplement(ec)
    , ExecutionContextLifecycleObserver(&ec)
    , PageVisibilityObserver(DomWindow() ? DomWindow()->GetFrame()->GetPage() : nullptr)
    , remote_(&ec)
{
    auto task_runner = ec.GetTaskRunner(kTaskType);

    mojo::PendingReceiver<mojom::blink::PendingBeaconHost> host_receiver = remote_.BindNewPipeAndPassReceiver(task_runner);
    ec.GetBrowserInterfaceBroker().GetInterface(std::move(host_receiver));
}

void PendingBeaconDispatcher::CreateHostBeacon(
    PendingBeacon* pending_beacon, mojo::PendingReceiver<mojom::blink::PendingBeacon> receiver, const KURL& url, mojom::blink::BeaconMethod method)
{
...
void PendingBeaconDispatcher::ScheduleDispatchBeacons()
{
    ...
    std::sort(background_timeout_descending_beacons_.begin(), background_timeout_descending_beacons_.end(), ReverseBeaconTimeoutSorter());
...
void PendingBeaconDispatcher::ScheduleDispatchNextBundledBeacons()
{
    ...
    const auto delayed = background_timeout_descending_beacons_[start_index]->GetBackgroundTimeout() - previous_delayed_;
    ...
    task_handle_ = PostNonNestableDelayedCancellableTask(
        *task_runner, FROM_HERE, WTF::BindOnce(&PendingBeaconDispatcher::OnDispatchBeaconsAndRepeat, WrapWeakPersistent(this), start_index), delayed);
}

void PendingBeaconDispatcher::OnDispatchBeaconsAndRepeat(wtf_size_t start_index)
{
    DCHECK(start_index < background_timeout_descending_beacons_.size());

    // Dispatches all beacons within the same bundle.
    for (auto i = start_index; i < background_timeout_descending_beacons_.size(); i++) {
...
wtf_size_t PendingBeaconDispatcher::GetStartIndexForNextBundledBeacons() const
{
    ...
    //     - returned index = [0]
    const auto floor_timeout = background_timeout_descending_beacons_.back()->GetBackgroundTimeout();
    // Rounds the the nearest 100x ms.
    const auto ceiling_timeout = (floor_timeout + kBeaconTimeoutInterval).IntDiv(kBeaconTimeoutInterval) * kBeaconTimeoutInterval;
    // Locates the first element such that
    // element.backgroundTimeout >= ceiling_timeout is false.
    const auto* const it = base::ranges::lower_bound(background_timeout_descending_beacons_, ceiling_timeout, std::greater_equal(),
        [](const Member<PendingBeaconDispatcher::PendingBeacon>& b) { return b->GetBackgroundTimeout(); });
    // The element with `floor_timeout`, i.e. last element, guarantees the
    // existence of `it`.
    return base::checked_cast<wtf_size_t>(std::distance(background_timeout_descending_beacons_.begin(), it));
}
...
scoped_refptr<base::SingleThreadTaskRunner> PendingBeaconDispatcher::GetTaskRunner()
{
...
bool PendingBeaconDispatcher::HasPendingBeaconForTesting(PendingBeacon* pending_beacon) const
{
