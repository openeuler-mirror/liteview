...
void ChildFrameCompositingHelper::SetSurfaceId(const viz::SurfaceId& surface_id, bool capture_sequence_number_changed)
{
    ...
    cc::DeadlinePolicy deadline = capture_sequence_number_changed ? cc::DeadlinePolicy::UseInfiniteDeadline() : cc::DeadlinePolicy::UseDefaultDeadline();
    ...
    if (sad_bitmap) {
        ...
        if (layer_size.width() >= paint_width && layer_size.height() >= paint_height) {
            ...
            auto image = cc::PaintImageBuilder::WithDefault()
                             ...
                             .set_image(SkImage::MakeFromBitmap(*sad_bitmap), cc::PaintImage::GetNextContentId())
                             .TakePaintImage();
