...
public:
    using Callback = base::OnceCallback<void(const Vector<v8::Local<v8::Value>>&)>;

    PromiseAggregator(ScriptState* script_state, const Vector<v8::Local<v8::Value>>& values, Callback callback);
...
private:
    ...
    public:
        ...
        static ScriptFunction* New(ScriptState* script_state, PromiseAggregator* aggregator, wtf_size_t index, bool was_fulfilled)
        {
            return MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<OnSettled>(aggregator, index, was_fulfilled));
        }
...
PromiseAggregator::PromiseAggregator(ScriptState* script_state, const Vector<v8::Local<v8::Value>>& values, Callback callback)
    : results_(values.size())
    , callback_(std::move(callback))
{
    for (wtf_size_t i = 0; i < values.size(); ++i) {
        ...
        ScriptPromise::Cast(script_state, values[i])
            .Then(OnSettled::New(script_state, this, i, /*was_fulfilled=*/true), OnSettled::New(script_state, this, i, /*was_fulfilled=*/false));
    }
...
Vector<v8::Local<v8::Value>> V8FunctionExecutor::Execute(ScriptState* script_state)
{
...

    {
        if (V8ScriptRunner::CallFunction(
                function_.Get(isolate), ExecutionContext::From(script_state), receiver_.Get(isolate), args.size(), args.data(), isolate)
    ...
    v8::Local<v8::Value> argv[], mojom::blink::WantResultOption want_result_option, WebScriptExecutionCallback callback)
{
    ...
    PausableScriptExecutor* executor = MakeGarbageCollected<PausableScriptExecutor>(script_state, mojom::blink::UserActivationOption::kDoNotActivate,
        mojom::blink::LoadEventBlockingOption::kDoNotBlock, want_result_option, mojom::blink::PromiseResultOption::kDoNotWait, std::move(callback), base::NullCallback(),
...
void PausableScriptExecutor::CreateAndRun(ScriptState* script_state, Vector<WebScriptSource> sources, ExecuteScriptPolicy execute_script_policy,
    ...
    mojom::blink::PromiseResultOption promise_result_option, WebScriptExecutionCallback callback, WebScriptExecutionCallbackUnmodified raw_callback)
{
    auto* executor = MakeGarbageCollected<PausableScriptExecutor>(script_state, user_activation_option, blocking_option, want_result_option,
        promise_result_option, std::move(callback), std::move(raw_callback), MakeGarbageCollected<WebScriptExecutor>(std::move(sources), execute_script_policy));
...
PausableScriptExecutor::PausableScriptExecutor(ScriptState* script_state, mojom::blink::UserActivationOption user_activation_option,
    mojom::blink::LoadEventBlockingOption blocking_option, mojom::blink::WantResultOption want_result_option,
    mojom::blink::PromiseResultOption promise_result_option, WebScriptExecutionCallback callback, WebScriptExecutionCallbackUnmodified raw_callback, Executor* executor)
...
void PausableScriptExecutor::HandleResults(const Vector<v8::Local<v8::Value>>& results)
{
    // The script may have removed the frame, in which case ContextDestroyed()
    // will have handled the disposal/callback.
    if (!script_state_->ContextIsValid())
