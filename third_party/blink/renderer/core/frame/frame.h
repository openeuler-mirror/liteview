...
public:
    ...
    // frame root.
    Frame* Parent(FrameTreeBoundary frame_tree_boundary = FrameTreeBoundary::kIgnoreFence) const;

    // Returns the top-most frame in the hierarchy containing this frame.
    // When `frame_tree_boundary` is `kFenced`, does not traverse out of fenced
    // frame root nodes.
    Frame* Top(FrameTreeBoundary frame_tree_boundary = FrameTreeBoundary::kIgnoreFence);

    // Returns the first child frame.
    // When `frame_tree_boundary` is `kFenced`, skips over children that are
    // fenced frame roots.
    Frame* FirstChild(FrameTreeBoundary frame_tree_boundary = FrameTreeBoundary::kIgnoreFence) const;
    ...
    // fenced frame roots.
    Frame* NextSibling(FrameTreeBoundary frame_tree_boundary = FrameTreeBoundary::kIgnoreFence) const;
...
protected:
    ...
    void NotifyUserActivationInFrameTree(mojom::blink::UserActivationNotificationType notification_type);
    bool ConsumeTransientUserActivationInFrameTree();
    void ClearUserActivationInFrameTree();

    void RenderFallbackContent();
    void RenderFallbackContentWithResourceTiming(mojom::blink::ResourceTimingInfoPtr timing, const String& server_timing_values);
