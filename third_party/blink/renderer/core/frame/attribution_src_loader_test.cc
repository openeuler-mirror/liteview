...
public:
    ...
    {
        return std::make_unique<WebURLLoaderFactoryWithMock>(WebURLLoaderMockFactory::GetSingletonInstance());
    }
...
public:
    explicit MockDataHost(mojo::PendingReceiver<mojom::blink::AttributionDataHost> data_host)
    {
        receiver_.Bind(std::move(data_host));
        receiver_.set_disconnect_handler(base::BindOnce(&MockDataHost::OnDisconnect, base::Unretained(this)));
    }
...
private:
    ...
    void SourceDataAvailable(mojom::blink::AttributionSourceDataPtr data) override
    {
        source_data_.push_back(std::move(data));
    }

    void TriggerDataAvailable(mojom::blink::AttributionTriggerDataPtr data) override
    {
...
public:
    explicit MockAttributionHost(blink::AssociatedInterfaceProvider* provider)
    {
        provider->OverrideBinderForTesting(
            mojom::blink::ConversionHost::Name_, base::BindRepeating(&MockAttributionHost::BindReceiver, base::Unretained(this)));
    }
...
private:
    ...
    {
        receiver_.Bind(mojo::PendingAssociatedReceiver<mojom::blink::ConversionHost>(std::move(handle)));
        if (quit_)
            std::move(quit_).Run();
    }

    void RegisterDataHost(mojo::PendingReceiver<mojom::blink::AttributionDataHost> data_host) override
    {
        mock_data_host_ = std::make_unique<MockDataHost>(std::move(data_host));
    }

    void RegisterNavigationDataHost(
        mojo::PendingReceiver<mojom::blink::AttributionDataHost> data_host, const blink::AttributionSrcToken& attribution_src_token) override
    {
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        attribution_src_loader_ = MakeGarbageCollected<AttributionSrcLoader>(GetDocument().GetFrame());
    }

    void TearDown() override
    {
        GetFrame().GetRemoteNavigationAssociatedInterfaces()->OverrideBinderForTesting(
            mojom::blink::ConversionHost::Name_, base::BindRepeating([](mojo::ScopedInterfaceEndpointHandle) {}));
...
TEST_F(AttributionSrcLoaderTest, RegisterTriggerWithoutEligibleHeader)
{
    ...
    response.SetHttpHeaderField(http_names::kAttributionReportingRegisterTrigger, R"({"event_trigger_data":[{"trigger_data": "7"}]})");

    MockAttributionHost host(GetFrame().GetRemoteNavigationAssociatedInterfaces());
    EXPECT_TRUE(attribution_src_loader_->MaybeRegisterAttributionHeaders(request, response, resource));
...
TEST_F(AttributionSrcLoaderTest, RegisterTriggerWithTriggerHeader)
{
    ...
    ResourceRequest request(test_url);
    request.SetHttpHeaderField(http_names::kAttributionReportingEligible, "trigger");
    auto* resource = MakeGarbageCollected<MockResource>(test_url);
    ResourceResponse response(test_url);
    response.SetHttpStatusCode(200);
    response.SetHttpHeaderField(http_names::kAttributionReportingRegisterTrigger, R"({"event_trigger_data":[{"trigger_data": "7"}]})");
    ...
    MockAttributionHost host(GetFrame().GetRemoteNavigationAssociatedInterfaces());
    attribution_src_loader_->MaybeRegisterAttributionHeaders(request, response, resource);
    host.WaitUntilBoundAndFlush();
...
TEST_F(AttributionSrcLoaderTest, RegisterTriggerWithSourceTriggerHeader)
{
    ...
    ResourceRequest request(test_url);
    request.SetHttpHeaderField(http_names::kAttributionReportingEligible, "event-source, trigger");
    auto* resource = MakeGarbageCollected<MockResource>(test_url);
    ResourceResponse response(test_url);
    response.SetHttpStatusCode(200);
    response.SetHttpHeaderField(http_names::kAttributionReportingRegisterTrigger, R"({"event_trigger_data":[{"trigger_data": "7"}]})");
...
TEST_F(AttributionSrcLoaderTest, AttributionSrcRequestsIgnored)
{
    ...
    EXPECT_FALSE(attribution_src_loader_->MaybeRegisterAttributionHeaders(request, response, resource));
}
...
TEST_F(AttributionSrcLoaderTest, AttributionSrcRequestsInvalidEligibleHeaders)
{
    ...
    for (const char* header : header_values) {
        response.SetHttpHeaderField(http_names::kAttributionReportingRegisterTrigger, header);

        EXPECT_FALSE(attribution_src_loader_->MaybeRegisterAttributionHeaders(request, response, resource)) << header;
    }
}
...
TEST_F(AttributionSrcLoaderTest, AttributionSrcRequestStatusHistogram)
{
    ...
    // kRequested = 0.
    histograms.ExpectUniqueSample("Conversions.AttributionSrcRequestStatus", 0, 2);
...
TEST_F(AttributionSrcLoaderTest, EligibleHeader_Register)
{
    ...
    const AtomicString& eligible = client_->request_head().HttpHeaderField(http_names::kAttributionReportingEligible);
    EXPECT_EQ(eligible, "event-source, trigger");

    absl::optional<net::structured_headers::Dictionary> dict = net::structured_headers::ParseDictionary(eligible.Utf8());
    ASSERT_TRUE(dict);
