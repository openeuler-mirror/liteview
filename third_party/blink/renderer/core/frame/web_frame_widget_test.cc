...
public:
    ...
    MOCK_METHOD4_T(Run, void(mojom::InputEventResultState, const ui::LatencyInfo&, InputHandlerProxy::DidOverscrollParams*, absl::optional<cc::TouchAction>));

    WidgetBaseInputHandler::HandledEventCallback GetCallback()
    {
        return base::BindOnce(&MockHandledEventCallback::HandleCallback, base::Unretained(this));
    }

private:
    void HandleCallback(mojom::InputEventResultState ack_state, const ui::LatencyInfo& latency_info,
        std::unique_ptr<InputHandlerProxy::DidOverscrollParams> overscroll, absl::optional<cc::TouchAction> touch_action)
    {
...
public:
    ...
    MOCK_METHOD2(WillHandleGestureEvent, void(const WebGestureEvent& event, bool* suppress));
...
public:
    SimWebFrameWidget* CreateSimWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
        ...
        bool is_for_scalable_page, SimCompositor* compositor) override
    {
        return MakeGarbageCollected<MockWebFrameWidgetImpl>(compositor, pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host),
    ...
    void WillHandleGestureEvent(const blink::WebGestureEvent& event, bool* suppress)
    {
        if (event.GetType() == WebInputEvent::Type::kGestureScrollUpdate) {
            MockMainFrameWidget()->DidOverscroll(gfx::Vector2dF(event.data.scroll_update.delta_x, event.data.scroll_update.delta_y),
                gfx::Vector2dF(event.data.scroll_update.delta_x, event.data.scroll_update.delta_y), event.PositionInWidget(),
                gfx::Vector2dF(event.data.scroll_update.velocity_x, event.data.scroll_update.velocity_y));
            *suppress = true;
        }
    }
...
{
    ON_CALL(*MockMainFrameWidget(), WillHandleGestureEvent(_, _)).WillByDefault(testing::Invoke(this, &WebFrameWidgetImplSimTest::WillHandleGestureEvent));
    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).WillRepeatedly(::testing::Return(WebInputEventResult::kNotHandled));

    WebGestureEvent scroll(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    ...
    EXPECT_CALL(handled_event, Run(mojom::InputEventResultState::kConsumed, _, testing::Pointee(expected_overscroll), _)).Times(1);
...
public:
    ...
    void CompositeAndWaitForPresentation(base::TimeDelta swap_to_presentation)
    {
        ...
        static_cast<WebFrameWidgetImpl*>(MainFrame().FrameWidget())
            ->NotifySwapAndPresentationTimeForTesting({ base::BindOnce(
                ...
                base::BindOnce(
                    [](base::OnceClosure presentation_quit_closure, base::TimeTicks timestamp) {
                        DCHECK(!timestamp.is_null());
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, PresentationTimestampValid)
{
    ...
    EXPECT_THAT(histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.Valid"), testing::ElementsAre(base::Bucket(true, 1)));
    EXPECT_THAT(histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.DeltaFromSwapTime"), testing::ElementsAre(base::Bucket(2, 1)));
}
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, PresentationTimestampInvalid)
{
    ...
    EXPECT_THAT(histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.Valid"), testing::ElementsAre(base::Bucket(false, 1)));
    EXPECT_THAT(histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.DeltaFromSwapTime"), testing::IsEmpty());
}

TEST_F(NotifySwapTimesWebFrameWidgetTest, PresentationTimestampEarlierThanSwaptime)
{
    base::HistogramTester histograms;
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, NotifyOnSuccessfulPresentation)
{
    ...
    EXPECT_THAT(histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.Valid"), testing::ElementsAre(base::Bucket(true, 1)));
    const auto expected_sample = static_cast<base::HistogramBase::Sample>((swap_to_failed + failed_to_successful).InMilliseconds());
    EXPECT_THAT(
        histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.DeltaFromSwapTime"), testing::ElementsAre(base::Bucket(expected_sample, 1)));
}
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, ReportPresentationOnlyOnSuccessfulCommit)
{
    ...
    presentation_run_loop.Run();
    const auto expected_sample = static_cast<base::HistogramBase::Sample>(delta_from_swap_time.InMilliseconds());
    EXPECT_THAT(
        histograms.GetAllSamples("PageLoad.Internal.Renderer.PresentationTime.DeltaFromSwapTime"), testing::ElementsAre(base::Bucket(expected_sample, 1)));
}
...
TEST_F(WebFrameWidgetSimTest, DispatchBufferedTouchEvents)
{
    ...
    widget->StartDragging(WebDragData(), kDragOperationCopy, SkBitmap(), gfx::Vector2d(), gfx::Rect());
    widget->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(touch.Clone(), {}, {}, ui::LatencyInfo()), base::DoNothing());
...
public:
    ...
    SimWebFrameWidget* CreateSimWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
        ...
        bool is_for_scalable_page, SimCompositor* compositor) override
    {
        return MakeGarbageCollected<TestWebFrameWidget>(compositor, pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host),
            std::move(widget), std::move(task_runner), frame_sink_id, hidden, never_composited, is_for_child_local_root, is_for_nested_main_frame,
            is_for_scalable_page);
    }
