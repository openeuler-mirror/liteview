...
v8::MaybeLocal<v8::Value> DOMWindow::Wrap(ScriptState* script_state)
{
    ...
    // per spec.
    return frame->GetWindowProxy(script_state->World())->GlobalProxyIfNotDetached();
}
...
DOMWindow* DOMWindow::self() const
{
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessSelf, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageSelf);
...
DOMWindow* DOMWindow::top() const
{
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessTop, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageTop);
...
void DOMWindow::Close(LocalDOMWindow* incumbent_window)
{
    ...
    if (!page->OpenedByDOM() && GetFrame()->Client()->BackForwardLength() > 1 && !allow_scripts_to_close_windows) {
        active_document->domWindow()->GetFrameConsole()->AddMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, "Scripts may close only the windows that were opened by them."));
        return;
    }
...
void DOMWindow::blur()
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessBlur, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageBlur);
}
...
void DOMWindow::ReportCoopAccess(const char* property_name)
{
    ...
    while (it != coop_access_monitor_.end()) {
        ...
        if (it->endpoint_defined) {
            it->reporter->QueueAccessReport(it->report_type, property_name, std::move(source_location), std::move(it->reported_window_url));
                ...
                ReportingContext::From(accessing_main_frame.DomWindow())
                    ->QueueReport(MakeGarbageCollected<Report>(ReportType::kCoopAccessViolation, accessing_main_frame.GetDocument()->Url().GetString(),
                        MakeGarbageCollected<CoopAccessViolationReportBody>(
                            std::move(location), it->report_type, String(property_name), it->reported_window_url)));
            }
        }
    ...
    LocalDOMWindow* source, ExceptionState& exception_state)
{
    ...
    if (!unload_event_in_progress && source_frame && source_frame->GetPage() && source_frame->GetPage()->DispatchedPagehideAndStillHidden()) {
        ...
        // this behavior is ok.
        UMA_HISTOGRAM_ENUMERATION("BackForwardCache.SameSite.ActionAfterPagehide2", ActionAfterPagehide::kSentPostMessage);
    }
...
void DOMWindow::RecordWindowProxyAccessMetrics(WebFeature property_access, WebFeature property_access_from_other_page) const
{
    if (!GetFrame())
        return;
