...
public:
    ...
    AttributionFilterDataBuilder& AddFilter(String filter_name, Vector<String> filter_values)
    {
        filters_.filter_values.insert(std::move(filter_name), std::move(filter_values));
        return *this;
    }
...
TEST(AttributionResponseParsingTest, ParseAggregationKeys)
{
    const struct {
        ...
        { "One valid key", ParseJSON(R"({"key":"0x159"})"), true, { { "key", absl::MakeUint128(/*high=*/0, /*low=*/345) } } },
        { "Two valid keys", ParseJSON(R"({"key1":"0x159","key2":"0x50000000000000159"})"), true,
            {
                { "key1", absl::MakeUint128(/*high=*/0, /*low=*/345) },
                { "key2", absl::MakeUint128(/*high=*/5, /*low=*/345) },
            } },
        { "Second key invalid", ParseJSON(R"({"key1":"0x159","key2":""})"), false, {} },
    };
...
TEST(AttributionResponseParsingTest, ParseAggregationKeys_CheckSize)
{
    struct AttributionAggregatableSourceSizeTestCase {
        ...
        WTF::HashMap<String, absl::uint128> GetAggregationKeys() const
        {
            ...
            for (wtf_size_t i = 0u; i < key_count; ++i) {
                aggregation_keys.insert(GetKey(i), absl::MakeUint128(/*high=*/0, /*low=*/1));
            }
    ...
    private:
        String GetKey(wtf_size_t index) const
        {
            ...
            return String(std::string(key_size, 'A' + index % 26 + 32 * (index / 26)));
        }
    };

    const AttributionAggregatableSourceSizeTestCase kTestCases[] = {
        { "empty", true, 0, 0 },
        { "max_keys", true, blink::kMaxAttributionAggregationKeysPerSourceOrTrigger, 1 },
        { "too_many_keys", false, blink::kMaxAttributionAggregationKeysPerSourceOrTrigger + 1, 1 },
        { "max_key_size", true, 1, blink::kMaxBytesPerAttributionAggregationKeyId },
        { "excessive_key_size", false, 1, blink::kMaxBytesPerAttributionAggregationKeyId + 1 },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(test_case.valid, valid) << test_case.description;
        if (test_case.valid) {
            EXPECT_EQ(test_case.GetAggregationKeys(), actual) << test_case.description;
        }
    }
}
...
TEST(AttributionResponseParsingTest, ParseAttributionAggregatableTrigger)
{
    const struct {
        ...
        { "Missing source_keys field", ParseJSON(R"([{"key_piece":"0x400"}])"), false, {} },
        { "source_keys not an array", ParseJSON(R"([{"key_piece":"0x400","source_keys":"key"}])"), false, {} },
        { "source_keys element not a string", ParseJSON(R"([{"key_piece":"0x400","source_keys":[123]}])") },
        { "Missing key_piece field", ParseJSON(R"([{"source_keys":["key"]}])"), false, {} },
        { "Invalid key", ParseJSON(R"([{"key_piece":"0xG00","source_keys":["key"]}])"), false, {} },
        { "Valid trigger", ParseJSON(R"([{"key_piece":"0x400","source_keys":["key"]}])"), true,
            VectorBuilder<mojom::blink::AttributionAggregatableTriggerDataPtr>()
                .Add(mojom::blink::AttributionAggregatableTriggerData::New(absl::MakeUint128(/*high=*/0, /*low=*/1024),
         ...
         "not_filters": {"filter": ["value2"]}
            ...
            VectorBuilder<mojom::blink::AttributionAggregatableTriggerDataPtr>()
                .Add(mojom::blink::AttributionAggregatableTriggerData::New(absl::MakeUint128(/*high=*/0, /*low=*/1024),
                    /*source_keys=*/Vector<String> { "key" },
                    /*filters=*/
                    AttributionFilterDataBuilder().AddFilter("filter", Vector<String> { "value1" }).Build(),
                    /*not_filters=*/
                    AttributionFilterDataBuilder().AddFilter("filter", Vector<String> { "value2" }).Build()))
                .Build() },
        { "Two valid trigger data", ParseJSON(R"([{"key_piece":"0x400","source_keys":["key1"]},
           {"key_piece":"0xA80","source_keys":["key2"]}])"),
            ...
            VectorBuilder<mojom::blink::AttributionAggregatableTriggerDataPtr>()
                .Add(mojom::blink::AttributionAggregatableTriggerData::New(absl::MakeUint128(/*high=*/0, /*low=*/1024),
                    ...
                    /*filters=*/mojom::blink::AttributionFilterData::New(),
                    /*not_filters=*/mojom::blink::AttributionFilterData::New()))
                .Add(mojom::blink::AttributionAggregatableTriggerData::New(absl::MakeUint128(/*high=*/0, /*low=*/2688),
    ...
    for (const auto& test_case : kTestCases) {
        WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr> trigger_data;
        bool valid = ParseAttributionAggregatableTriggerData(test_case.json.get(), trigger_data);
        EXPECT_EQ(test_case.valid, valid) << test_case.description;
        if (test_case.valid) {
...
TEST(AttributionResponseParsingTest, ParseAttributionAggregatableTrigger_CheckSize)
{
    struct AttributionAggregatableTriggerSizeTestCase {
        ...
        WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr> GetTriggerData() const
        {
            if (!valid)
                return {};
            ...
            for (wtf_size_t i = 0u; i < trigger_data_count; ++i) {
                data.push_back(mojom::blink::AttributionAggregatableTriggerData::New(absl::MakeUint128(/*high=*/0, /*low=*/1),
    ...
    private:
        String GetKey() const
        {
            return String(std::string(key_size, 'A'));
        }
    };

    const AttributionAggregatableTriggerSizeTestCase kTestCases[] = {
        { "empty", true, 0, 0, 0 },
        { "max_trigger_data", true, blink::kMaxAttributionAggregatableTriggerDataPerTrigger, 0, 0 },
        { "too_many_trigger_data", false, blink::kMaxAttributionAggregatableTriggerDataPerTrigger + 1, 0, 0 },
        { "max_key_count", true, 1, blink::kMaxAttributionAggregationKeysPerSourceOrTrigger, 0 },
        { "too many keys", false, 1, blink::kMaxAttributionAggregationKeysPerSourceOrTrigger + 1, 0 },
        { "max_key_size", true, 1, 1, kMaxBytesPerAttributionAggregationKeyId },
        { "excessive_key_size", false, 1, 1, kMaxBytesPerAttributionAggregationKeyId + 1 },
    };

    for (const auto& test_case : kTestCases) {
        std::unique_ptr<JSONArray> json = test_case.GetHeader();
        WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr> trigger_data;
        bool valid = ParseAttributionAggregatableTriggerData(json.get(), trigger_data);

        EXPECT_EQ(test_case.valid, valid) << test_case.description;
        if (test_case.valid) {
            EXPECT_EQ(test_case.GetTriggerData(), trigger_data) << test_case.description;
        }
    }
}
...
TEST(AttributionResponseParsingTest, ParseAttributionAggregatableValues)
{
    const struct {
        ...
        { "Two valid values", ParseJSON(R"({"key1":123,"key2":456})"), true, { { "key1", 123 }, { "key2", 456 } } },
        { "Max valid value", ParseJSON(R"({"key":65536})"), true, { { "key", 65536 } } },
        { "Value out of range", ParseJSON(R"({"key":65537})"), false, {} },
    };

    for (const auto& test_case : kTestCases) {
        WTF::HashMap<String, uint32_t> values;
        bool valid = ParseAttributionAggregatableValues(test_case.json.get(), values);
        EXPECT_EQ(test_case.valid, valid) << test_case.description;
        if (test_case.valid)
...
TEST(AttributionResponseParsingTest, ParseAttributionAggregatableValues_CheckSize)
{
...
TEST(AttributionResponseParsingTest, ParseFilterData)
{
    ...
    const struct {
        ...
        },
        {
            ...
            AttributionFilterDataBuilder().AddFilter("a", { "b" }).AddFilter("c", { "e", "d" }).Build(),
        },
        {
            "not_dictionary",
...
{
    const auto reporting_origin = SecurityOrigin::CreateFromString("https://r.test");
    ...
    const struct {
        ...
        { "no_aggregatable_dedup_key", R"json({})json",
            mojom::blink::AttributionTriggerData::New(reporting_origin, WTF::Vector<mojom::blink::EventTriggerDataPtr>(),
                /*filters=*/AttributionFilterDataBuilder().Build(),
                /*not_filters=*/AttributionFilterDataBuilder().Build(), WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr>(),
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (test_case.expected) {
            EXPECT_EQ(test_case.expected->reporting_origin->ToUrlOrigin(), trigger_data.reporting_origin->ToUrlOrigin()) << test_case.description;

            EXPECT_EQ(test_case.expected->aggregatable_dedup_key, trigger_data.aggregatable_dedup_key) << test_case.description;
        }
    }
}
...
TEST(AttributionResponseParsingTest, ParseSourceRegistrationHeader)
{
    ...
    const struct {
        ...
        },
        {
            ...
            mojom::blink::AttributionSourceData::New(
                /*destination=*/SecurityOrigin::CreateFromString("https://d.test"),
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (test_case.expected) {
            EXPECT_EQ(test_case.expected->destination->ToUrlOrigin(), source_data.destination->ToUrlOrigin()) << test_case.description;

            EXPECT_EQ(test_case.expected->reporting_origin->ToUrlOrigin(), source_data.reporting_origin->ToUrlOrigin()) << test_case.description;

            EXPECT_EQ(test_case.expected->source_event_id, source_data.source_event_id) << test_case.description;

            EXPECT_EQ(test_case.expected->expiry, source_data.expiry) << test_case.description;

            EXPECT_EQ(test_case.expected->priority, source_data.priority) << test_case.description;

            EXPECT_EQ(test_case.expected->debug_key, source_data.debug_key) << test_case.description;

            EXPECT_EQ(test_case.expected->filter_data, source_data.filter_data) << test_case.description;

            // This field is not populated by `ParseSourceRegistrationHeader()`, but
            // check it for equality with the test case anyway.
            EXPECT_EQ(test_case.expected->aggregation_keys, source_data.aggregation_keys) << test_case.description;
        }
    }
}
...
TEST(AttributionResponseParsingTest, ParseDebugKey)
{
    ...
    EXPECT_EQ(ParseDebugKey("18446744073709551615"), mojom::blink::AttributionDebugKey::New(std::numeric_limits<uint64_t>::max()));
}
        ...
        },
        {
            ...
            VectorBuilder<mojom::blink::EventTriggerDataPtr>()
                .Add(mojom::blink::EventTriggerData::New(
                    ...
                    /*dedup_key=*/nullptr,
                    /*filters=*/
                    AttributionFilterDataBuilder().AddFilter("source_type", { "navigation" }).Build(),
                    /*not_filters=*/AttributionFilterDataBuilder().Build()))
                .Build(),
        },
        {
...
TEST(AttributionResponseParsingTest, FilterValuesHistogram)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        histograms.ExpectUniqueSample("Conversions.ValuesPerFilter", test_case.size, test_case.expected);
    }
}
...
TEST(AttributionResponseParsingTest, FiltersSizeHistogram)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        mojom::blink::AttributionFilterData filter_data;
        ParseAttributionFilterData(json.get(), filter_data);
        histograms.ExpectUniqueSample("Conversions.FiltersPerFilterData", test_case.size, test_case.expected);
    }
}
...
TEST(AttributionResponseParsingTest, SourceAggregationKeysHistogram)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        histograms.ExpectUniqueSample("Conversions.AggregatableKeysPerSource", test_case.size, test_case.expected);
    }
}
...
TEST(AttributionResponseParsingTest, AggregatableTriggerDataHistogram)
{
    ...
    for (const auto& test_case : kTestCases) {
        base::HistogramTester histograms;
        std::unique_ptr<JSONValue> json = make_aggregatable_trigger_with_trigger_data(test_case.size);
        WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr> trigger_data;
        ParseAttributionAggregatableTriggerData(json.get(), trigger_data);
        histograms.ExpectUniqueSample("Conversions.AggregatableTriggerDataLength", test_case.size, test_case.expected);
    }
}
