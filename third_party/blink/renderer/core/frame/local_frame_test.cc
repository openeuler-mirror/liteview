...
public:
    ...
    void Init(base::OnceClosure callback, blink::BrowserInterfaceBrokerProxy& provider)
    {
        callback_ = std::move(callback);
        provider.SetBinderForTesting(
            mojom::blink::TextInputHost::Name_, base::BindRepeating(&TestTextInputHostWaiter::BindTextInputHostReceiver, base::Unretained(this)));
    }
...
TEST_F(LocalFrameTest, IsLazyLoadingImageAllowedWithFeatureDisabled)
{
    ...
    auto page_holder = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr, nullptr, base::BindOnce(&EnableLazyLoadInSettings));
    EXPECT_EQ(LocalFrame::LazyLoadImageSetting::kDisabled, page_holder->GetFrame().GetLazyLoadImageSetting());
}

TEST_F(LocalFrameTest, IsLazyLoadingImageAllowedWithSettingDisabled)
{
    ScopedLazyImageLoadingForTest scoped_lazy_image_loading_for_test(false);
    auto page_holder = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr, nullptr, base::BindOnce(&DisableLazyLoadInSettings));
    EXPECT_EQ(LocalFrame::LazyLoadImageSetting::kDisabled, page_holder->GetFrame().GetLazyLoadImageSetting());
}
...
TEST_F(LocalFrameTest, IsLazyLoadingImageAllowedWithAutomaticDisabled)
{
    ...
    auto page_holder = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr, nullptr, base::BindOnce(&EnableLazyLoadInSettings));
    EXPECT_EQ(LocalFrame::LazyLoadImageSetting::kEnabledExplicit, page_holder->GetFrame().GetLazyLoadImageSetting());
}
