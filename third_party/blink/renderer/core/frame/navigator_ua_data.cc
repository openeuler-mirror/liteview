...
const HeapVector<Member<NavigatorUABrandVersion>>& NavigatorUAData::brands() const
{
    ...
    if (context) {
        ...
        if (UNLIKELY(IdentifiabilityStudySettings::Get()->ShouldSampleSurface(identifiable_surface))) {
...
ScriptPromise NavigatorUAData::getHighEntropyValues(ScriptState* script_state, Vector<String>& hints) const
{
    ...
    execution_context->GetTaskRunner(TaskType::kPermission)
        ->PostTask(FROM_HERE,
            WTF::BindOnce(
                [](ScriptPromiseResolver* resolver, UADataValues* values) { resolver->Resolve(values); }, WrapPersistent(resolver), WrapPersistent(values)));
