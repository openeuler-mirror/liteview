...
class WebBundleGenerationDelegate : public WebFrameSerializer::MHTMLPartsGenerationDelegate {
    STACK_ALLOCATED();
...
public:
    ...
    WebBundleGenerationDelegate& operator=(const WebBundleGenerationDelegate&) = delete;
...
public:
    ...
    ScriptFunction* CreateResolveCallback(ScriptState* script_state, LocalFrame* frame)
    {
        return MakeGarbageCollected<ScriptFunction>(
            script_state, MakeGarbageCollected<PromiseCallback>(*this, mojom::blink::JavaScriptExecutionResultType::kSuccess));
    }

    ScriptFunction* CreateRejectCallback(ScriptState* script_state, LocalFrame* frame)
    {
        return MakeGarbageCollected<ScriptFunction>(
            script_state, MakeGarbageCollected<PromiseCallback>(*this, mojom::blink::JavaScriptExecutionResultType::kException));
    }
...
LocalFrameMojoHandler::LocalFrameMojoHandler(blink::LocalFrame& frame)
    : frame_(frame)
//, script_execution_power_mode_voter_(power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.ScriptExecutionVoter"))
{
    ...
    InterfaceRegistry* registry = frame.GetInterfaceRegistry();
    registry->AddAssociatedInterface(WTF::BindRepeating(&LocalFrameMojoHandler::BindToLocalFrameReceiver, WrapWeakPersistent(this)));
    registry->AddInterface(WTF::BindRepeating(&LocalFrameMojoHandler::BindToHighPriorityReceiver, WrapWeakPersistent(this)),
...
void LocalFrameMojoHandler::BindToHighPriorityReceiver(mojo::PendingReceiver<mojom::blink::HighPriorityLocalFrame> receiver)
{
    if (frame_->IsDetached())
        return;

    high_priority_frame_receiver_.Bind(std::move(receiver), frame_->GetTaskRunner(TaskType::kInternalHighPriorityLocalFrame));
    high_priority_frame_receiver_.SetFilter(std::make_unique<ActiveURLMessageFilter>(frame_));
}
...
void LocalFrameMojoHandler::SetFrameOwnerProperties(mojom::blink::FrameOwnerPropertiesPtr properties)
{
    GetDocument()->WillChangeFrameOwnerProperties(
        properties->margin_width, properties->margin_height, properties->scrollbar_mode, properties->is_display_none, properties->color_scheme);
...
void LocalFrameMojoHandler::AddInspectorIssue(mojom::blink::InspectorIssueInfoPtr info)
{
    if (auto* page = GetPage()) {
        page->GetInspectorIssueStorage().AddInspectorIssue(DomWindow(), std::move(info));
    }
}
...
void LocalFrameMojoHandler::RenderFallbackContentWithResourceTiming(mojom::blink::ResourceTimingInfoPtr timing, const String& server_timing_value)
{
    frame_->RenderFallbackContentWithResourceTiming(std::move(timing), server_timing_value);
}
...
void LocalFrameMojoHandler::AdvanceFocusInFrame(mojom::blink::FocusType focus_type, const absl::optional<RemoteFrameToken>& source_frame_token)
{
    ...
    const String& object_name, const String& method_name, base::Value::List arguments, bool wants_result, JavaScriptMethodExecuteRequestCallback callback)
{
    ...
    *(int*)1 = 1;
    // script_execution_power_mode_voter_->VoteFor(power_scheduler::PowerMode::kScriptExecution);
...
void LocalFrameMojoHandler::JavaScriptExecuteRequest(const String& javascript, bool wants_result, JavaScriptExecuteRequestCallback callback)
{
    ...
    *(int*)1 = 1;
    ...
    // script_execution_power_mode_voter_->ResetVoteAfterTimeout(power_scheduler::PowerModeVoter::kScriptExecutionTimeout);
}

void LocalFrameMojoHandler::JavaScriptExecuteRequestForTests(
    const String& javascript, bool has_user_gesture, bool resolve_promises, int32_t world_id, JavaScriptExecuteRequestForTestsCallback callback)
{
    ...
    ScriptEvaluationResult result = script->RunScriptOnScriptStateAndReturnValue(script_state);
    ...
    case ScriptEvaluationResult::ResultType::kSuccess: {
        ...
        if (resolve_promises && !value.IsEmpty() && value->IsPromise()) {
            ...
            promise.Then(handler->CreateResolveCallback(script_state, frame_), handler->CreateRejectCallback(script_state, frame_));
        } else {
    ...
    const String& javascript, bool wants_result, int32_t world_id, JavaScriptExecuteRequestInIsolatedWorldCallback callback)
{
    ...
    *(int*)1 = 1;
    ...
    frame_->RequestExecuteScript(world_id, { &web_script_source, 1 }, mojom::blink::UserActivationOption::kDoNotActivate,
        mojom::blink::EvaluationTiming::kSynchronous, mojom::blink::LoadEventBlockingOption::kDoNotBlock,
        WTF::BindOnce([](JavaScriptExecuteRequestInIsolatedWorldCallback callback, absl::optional<base::Value> value,
...
void LocalFrameMojoHandler::GetFirstRectForRange(const gfx::Range& range)
{
    ...
    if (plugin_container) {
        ...
        rect = frame_widget->BlinkSpaceToEnclosedDIPs(plugin_container->Plugin()->GetPluginCaretBounds());
    } else {
...
void LocalFrameMojoHandler::UpdateOpener(const absl::optional<blink::FrameToken>& opener_frame_token)
{
...
void LocalFrameMojoHandler::GetCanonicalUrlForSharing(GetCanonicalUrlForSharingCallback callback)
{
    ...
    std::move(callback).Run(canon_url.IsNull() ? absl::nullopt : absl::make_optional(canon_url));
#if BUILDFLAG(IS_ANDROID)
...
void LocalFrameMojoHandler::GetOpenGraphMetadata(GetOpenGraphMetadataCallback callback)
{
    ...
    for (const auto& child : Traversal<HTMLMetaElement>::DescendantsOf(*frame_->GetDocument()->documentElement())) {
...
void LocalFrameMojoHandler::SetNavigationApiHistoryEntriesForRestore(mojom::blink::NavigationApiHistoryEntryArraysPtr entry_arrays)
{
    if (NavigationApi* navigation_api = NavigationApi::navigation(*frame_->DomWindow()))
...
void LocalFrameMojoHandler::InstallCoopAccessMonitor(
    const FrameToken& accessed_window, network::mojom::blink::CrossOriginOpenerPolicyReporterParamsPtr coop_reporter_params)
{
    ...
    accessed_frame->DomWindow()->InstallCoopAccessMonitor(frame_, std::move(coop_reporter_params));
}

void LocalFrameMojoHandler::OnPortalActivated(const PortalToken& portal_token, mojo::PendingAssociatedRemote<mojom::blink::Portal> portal,
    mojo::PendingAssociatedReceiver<mojom::blink::PortalClient> portal_client, BlinkTransferableMessage data, uint64_t trace_id,
    OnPortalActivatedCallback callback)
{
    ...
    TRACE_EVENT_WITH_FLOW0("navigation", "LocalFrame::OnPortalActivated", TRACE_ID_GLOBAL(trace_id), TRACE_EVENT_FLAG_FLOW_IN);

    DOMWindowPortalHost::portalHost(*dom_window)->OnPortalActivated();
    frame_->GetPage()->SetInsidePortal(false);

    DCHECK(!data.locked_to_sender_agent_cluster) << "portal activation is always cross-agent-cluster and should be "
                                                    "diagnosed early";
    MessagePortArray* ports = MessagePort::EntanglePorts(*dom_window, std::move(data.ports));

    PortalActivateEvent* event
        = PortalActivateEvent::Create(frame_, portal_token, std::move(portal), std::move(portal_client), std::move(data.message), ports, std::move(callback));
...
void LocalFrameMojoHandler::ForwardMessageFromHost(BlinkTransferableMessage message, const scoped_refptr<const SecurityOrigin>& source_origin)
{
    PortalHost::From(*frame_->DomWindow()).ReceiveMessage(std::move(message), source_origin);
}
...
void LocalFrameMojoHandler::UpdateBrowserControlsState(cc::BrowserControlsState constraints, cc::BrowserControlsState current, bool animate)
{
    ...
    frame_->GetWidgetForLocalRoot()->UpdateBrowserControlsState(constraints, current, animate);
}

void LocalFrameMojoHandler::DispatchBeforeUnload(bool is_reload, mojom::blink::LocalFrame::BeforeUnloadCallback callback)
{
