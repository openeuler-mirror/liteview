...
void Frame::NotifyUserActivationInFrameTree(mojom::blink::UserActivationNotificationType notification_type)
{
    // TODO(crbug.com/1262022): Remove this once we use MPArch as the underlying
    // fenced frames implementation, instead of ShadowDOM.
    for (Frame* node = this; node; node = node->Tree().Parent(FrameTreeBoundary::kFenced)) {
    ...
    if (local_frame && RuntimeEnabledFeatures::UserActivationSameOriginVisibilityEnabled()) {
        ...
        for (Frame *top = &Tree().Top(FrameTreeBoundary::kFenced), *node = top; node; node = node->Tree().TraverseNext(top, FrameTreeBoundary::kFenced)) {
...
bool Frame::ConsumeTransientUserActivationInFrameTree()
{
    ...
    for (Frame* node = &root; node; node = node->Tree().TraverseNext(&root, FrameTreeBoundary::kFenced))
...
void Frame::RenderFallbackContentWithResourceTiming(mojom::blink::ResourceTimingInfoPtr timing, const String& server_timing_value)
{
    auto* local_dom_window = To<LocalDOMWindow>(Parent()->DomWindow());
    DOMWindowPerformance::performance(*local_dom_window)
        ->AddResourceTimingWithUnparsedServerTiming(std::move(timing), server_timing_value, html_names::kObjectTag.LocalName(), local_dom_window);
...
bool Frame::IsInFencedFrameTree() const
{
    ...
    case blink::features::FencedFramesImplementationType::kShadowDOM:
        return Tree().Top(FrameTreeBoundary::kFenced) != Tree().Top(FrameTreeBoundary::kIgnoreFence);
    default:
        return false;
    }
}
...
bool Frame::IsFencedFrameRoot() const
{
    ...
    case blink::features::FencedFramesImplementationType::kShadowDOM:
        return IsInFencedFrameTree() && this == &Tree().Top(FrameTreeBoundary::kFenced);
    default:
        return false;
    }
}

absl::optional<mojom::blink::FencedFrameMode> Frame::GetFencedFrameMode() const
{
    DCHECK(!IsDetached());
    ...
    case blink::features::FencedFramesImplementationType::kShadowDOM:
        ...
        return Tree().Top(FrameTreeBoundary::kFenced).Owner()->GetFramePolicy().fenced_frame_mode;
    }
}
...
Frame* Frame::Parent(FrameTreeBoundary frame_tree_boundary) const
{
    ...
    if (frame_tree_boundary == FrameTreeBoundary::kFenced && Owner() && Owner()->GetFramePolicy().is_fenced
        && GetPage()->FencedFramesImplementationType().has_value()
        && GetPage()->FencedFramesImplementationType().value() == features::FencedFramesImplementationType::kShadowDOM) {
        return nullptr;
    }
...
bool Frame::FocusCrossesFencedBoundary()
{
    ...
    if (Frame* focused_frame = GetPage()->GetFocusController().FocusedFrame()) {
        ...
        return Tree().Top(FrameTreeBoundary::kFenced) != focused_frame->Tree().Top(FrameTreeBoundary::kFenced);
    }
...
bool Frame::AllowFocusWithoutUserActivation()
{
    ...
    case blink::features::FencedFramesImplementationType::kMPArch:
        ...
        // focus.
        return (!GetPage()->GetFocusController().IsActive() && !IsInFencedFrameTree()) || GetPage()->GetFocusController().IsFocused();
    case blink::features::FencedFramesImplementationType::kShadowDOM:
    ...
    mojo::PendingAssociatedReceiver<mojom::blink::RemoteFrame> remote_frame_receiver)
{
    ...
    if (auto* new_local_frame = DynamicTo<LocalFrame>(new_frame)) {
        ...
        if (owner) {
            ...
            TRACE_EVENT_INSTANT1("loading", "markAsMainFrame", TRACE_EVENT_SCOPE_THREAD, "frame", ::blink::ToTraceValue(new_local_frame));
        }
    }
