...
void RejectWithCode(ScriptPromiseResolver* resolver, DOMExceptionCode code, const String& message)
{
    ...
    ExceptionState exception_state(resolver->GetScriptState()->GetIsolate(), ExceptionState::kExecutionContext, "SelectorDirective", "createSelectorDirective");
...
void SelectorDirective::ResolvePromise() const
{
    ...
    if (!selected_range_) {
        RejectWithCode(matching_range_resolver_, DOMExceptionCode::kNotFoundError, "Could not find range matching the given selector");
        return;
    }
