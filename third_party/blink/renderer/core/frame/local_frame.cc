...
RemoteFrame* SourceFrameForOptionalToken(const absl::optional<RemoteFrameToken>& source_frame_token)
{
...
void SetViewportSegmentVariablesForRect(StyleEnvironmentVariables& vars, gfx::Rect segment_rect, unsigned first_dimension, unsigned second_dimension)
{
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentTop, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.y()));
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentRight, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.right()));
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentBottom, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.bottom()));
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentLeft, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.x()));
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentWidth, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.width()));
    vars.SetVariable(
        UADefinedTwoDimensionalVariable::kViewportSegmentHeight, first_dimension, second_dimension, StyleEnvironmentVariables::FormatPx(segment_rect.height()));
}
...
void LocalFrame::Navigate(FrameLoadRequest& request, WebFrameLoadType frame_load_type)
{
    ...
    if (request.ClientRedirectReason() != ClientNavigationReason::kNone)
        probe::FrameScheduledNavigation(this, request.GetResourceRequest().Url(), base::TimeDelta(), request.ClientRedirectReason());

    if (NavigationShouldReplaceCurrentHistoryEntry(request, frame_load_type))
        frame_load_type = WebFrameLoadType::kReplaceCurrentItem;

    const ClientNavigationReason client_redirect_reason = request.ClientRedirectReason();
...
bool LocalFrame::NavigationShouldReplaceCurrentHistoryEntry(const FrameLoadRequest& request, WebFrameLoadType frame_load_type)
{
    ...
    if (request.ClientRedirectReason() != ClientNavigationReason::kNone && !GetDocument()->LoadEventFinished() && !HasTransientUserActivation(this)
        && request.ClientRedirectReason() != ClientNavigationReason::kAnchorClick)
        return true;
    return frame_load_type == WebFrameLoadType::kStandard && ShouldMaintainTrivialSessionHistory();
...
bool LocalFrame::ShouldMaintainTrivialSessionHistory() const
{
    ...
    return GetPage()->InsidePortal() || GetDocument()->IsPrerendering() || IsInFencedFrameTree();
}
...
static String FrameDescription(const Frame& frame)
{
    ...
    return local_frame ? "with URL '" + local_frame->GetDocument()->Url().GetString() + "'"
...
void LocalFrame::HookBackForwardCacheEviction()
{
    ...
    static_cast<LocalWindowProxyManager*>(GetWindowProxyManager())->SetAbortScriptExecution([](v8::Isolate* isolate, v8::Local<v8::Context> context) {
        ...
        if (frame) {
            frame->EvictFromBackForwardCache(mojom::blink::RendererEvictionReason::kJavaScriptExecution);
        }
    });
}
...
void LocalFrame::DidChangeBackgroundColor(SkColor background_color, bool color_adjust)
{
    DCHECK(!Tree().Parent());
...
void LocalFrame::StartPrinting(const gfx::SizeF& page_size, const gfx::SizeF& original_page_size, float maximum_shrink_ratio)
{
...
void LocalFrame::SetPrinting(bool printing, const gfx::SizeF& page_size, const gfx::SizeF& original_page_size, float maximum_shrink_ratio)
{
    ...
    if (ShouldUsePrintingLayout()) {
        View()->ForceLayoutForPagination(page_size, original_page_size, maximum_shrink_ratio);
    } else {
...
bool LocalFrame::ShouldUsePrintingLayout() const
{
    ...
    return local_parent ? !local_parent->GetDocument()->Printing() : Client()->UsePrintingLayout();
}
...
gfx::SizeF LocalFrame::ResizePageRectsKeepingRatio(const gfx::SizeF& original_size, const gfx::SizeF& expected_size) const
{
    ...
    float result_width = floorf(is_horizontal ? expected_size.width() : expected_size.height());
...
void LocalFrame::SetPageAndTextZoomFactors(float page_zoom_factor, float text_zoom_factor)
{
    if (page_zoom_factor_ == page_zoom_factor && text_zoom_factor_ == text_zoom_factor)
        return;
    ...
    for (Frame* child = Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
        if (auto* child_local_frame = DynamicTo<LocalFrame>(child)) {
            child_local_frame->SetPageAndTextZoomFactors(page_zoom_factor_, text_zoom_factor_);
        }
    }
...
void LocalFrame::WindowSegmentsChanged(const WebVector<gfx::Rect>& window_segments)
{
...
void LocalFrame::UpdateViewportSegmentCSSEnvironmentVariables(const WebVector<gfx::Rect>& window_segments)
{
    ...
    StyleEnvironmentVariables& vars = StyleEnvironmentVariables::GetRootInstance();
    ...
    if (window_segments.size() >= 2) {
        ...
        unsigned x_index = 0;
        unsigned y_index = 0;
        SetViewportSegmentVariablesForRect(vars, window_segments[0], x_index, y_index);
        for (size_t i = 1; i < window_segments.size(); i++) {
            if (window_segments[i].y() == current_y_position) {
                x_index++;
                SetViewportSegmentVariablesForRect(vars, window_segments[i], x_index, y_index);
            } else {
...
device::mojom::blink::DevicePostureType LocalFrame::GetDevicePosture()
{
    ...
    // OutputDebugStringA("LocalFrame::GetDevicePosture not impl\n");
...
void LocalFrame::PortalStateChanged()
{
    ...
    if (IsOutermostMainFrame()) {
        ...
        intersection_state_.occlusion_state = mojom::blink::FrameOcclusionState::kUnknown;
    }
}
...
LocalFrame::LocalFrame(LocalFrameClient* client, Page& page, FrameOwner* owner, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type,
    const LocalFrameToken& frame_token, WindowAgentFactory* inheriting_agent_factory, InterfaceRegistry* interface_registry, const base::TickClock* clock)
    : Frame(client, page, owner, parent, previous_sibling, insert_type, frame_token, client->GetDevToolsFrameToken(),
        MakeGarbageCollected<LocalWindowProxyManager>(*this), inheriting_agent_factory)
    , frame_scheduler_(page.GetPageScheduler()->CreateFrameScheduler(this,
          /*TODO(crbug.com/1170350): Set for portals*/ IsInFencedFrameTree(),
          IsMainFrame() ? FrameScheduler::FrameType::kMainFrame : FrameScheduler::FrameType::kSubframe))
    , loader_(this)
...
bool LocalFrame::CanNavigate(const Frame& target_frame, const KURL& destination_url)
{
    ...
    const bool target_escapes_fenced_frame = IsInFencedFrameTree() && (Tree().Top(FrameTreeBoundary::kFenced) != Tree().Top(FrameTreeBoundary::kFenced));
    ...
    if (destination_url.ProtocolIsJavaScript()
        && (!GetSecurityContext()->GetSecurityOrigin()->CanAccess(target_frame.GetSecurityContext()->GetSecurityOrigin()) || target_escapes_fenced_frame)) {
        PrintNavigationErrorMessage(target_frame,
            target_escapes_fenced_frame ? "The frame attempting navigation must be in the same fenced "
    ...
    if (GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kNavigation)) {
        ...
        if (target_is_outermost_frame && target_frame != Tree().Top(FrameTreeBoundary::kIgnoreFence)
        ...
        if ((target_frame == Tree().Top(FrameTreeBoundary::kIgnoreFence)) && !target_frame.GetPage()->IsMainFrameFencedFrameRoot()) {
    ...
    if (target_frame == Tree().Top()) {
        ...
        if (!target_domain.empty() && !destination_domain.empty() && target_domain == destination_domain
            && (target_frame.GetSecurityContext()->GetSecurityOrigin()->Protocol() == destination_url.Protocol()
                || !base::FeatureList::IsEnabled(features::kBlockCrossOriginTopNavigationToDiffentScheme))) {
            return true;
        }
...
{
    DCHECK(GetSettings());
    if (!RuntimeEnabledFeatures::LazyImageLoadingEnabled() || !GetSettings()->GetLazyLoadEnabled()) {
...
void LocalFrame::SetOpener(Frame* opener_frame)
{
    ...
    if (web_frame && Opener() != opener_frame) {
        GetLocalFrameHostRemote().DidChangeOpener(
            opener_frame ? absl::optional<blink::LocalFrameToken>(opener_frame->GetFrameToken().GetAs<LocalFrameToken>()) : absl::nullopt);
    }
...
void LocalFrame::ForceSynchronousDocumentInstall(const AtomicString& mime_type, scoped_refptr<const SharedBuffer> data)
{
...
void LocalFrame::UpdateBackForwardCacheDisablingFeatures(uint64_t features_mask)
{
    GetBackForwardCacheControllerHostRemote().DidChangeBackForwardCacheDisablingFeatures(features_mask);
}
...
void LocalFrame::LoadJavaScriptURL(const KURL& url)
{
    ...
    // manipulations.
    if (SchemeRegistry::ShouldTreatURLSchemeAsNotAllowingJavascriptURLs(GetDocument()->Url().Protocol()))
        return;
    ...
    DomWindow()->GetScriptController().ExecuteJavaScriptURL(url, network::mojom::CSPDisposition::DO_NOT_CHECK, &DOMWrapperWorld::MainWorld());
}
...
void LocalFrame::RequestExecuteScript(int32_t world_id, base::span<const WebScriptSource> sources, mojom::blink::UserActivationOption user_gesture,
    mojom::blink::EvaluationTiming evaluation_timing, mojom::blink::LoadEventBlockingOption blocking_option, 
    ...
    BackForwardCacheAware back_forward_cache_aware, mojom::blink::WantResultOption want_result_option, mojom::blink::PromiseResultOption promise_behavior)
{
    ...
    script_sources.Append(sources.data(), base::checked_cast<wtf_size_t>(sources.size()));

    PausableScriptExecutor::CreateAndRun(ToScriptState(DomWindow(), *world), std::move(script_sources), execute_script_policy, user_gesture, evaluation_timing,
        blocking_option, want_result_option, promise_behavior, std::move(callback), std::move(raw_callback));
}
...
void LocalFrame::DidResume()
{
    ...
    base::UmaHistogramMicrosecondsTimes("DocumentEventTiming.ResumeDuration", resume_event_end - resume_event_start);
    // TODO(fmeawad): Move the following logic to the page once we have a
...
void LocalFrame::UpdateFaviconURL()
{
    ...
    for (const auto& icon_url : icon_urls) {
        urls.push_back(mojom::blink::FaviconURL::New(icon_url.icon_url_, icon_url.icon_type_, icon_url.sizes_));
    }
...
void LocalFrame::EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason)
{
    if (!GetPage()->GetPageScheduler()->IsInBackForwardCache())
        return;
...
{
    if (!RuntimeEnabledFeatures::WebAppWindowControlsOverlayEnabled(GetDocument()->GetExecutionContext())) {
        return;
    }
...
{
    mojom::blink::LocalFrameHost& ret = mojo_handler_->LocalFrameHostRemote();
    return ret;
}
...
void LocalFrame::MediaPlayerActionAtViewportPoint(const gfx::Point& viewport_position, const mojom::blink::MediaPlayerActionType type, bool enable)
{
    ...
    case mojom::blink::MediaPlayerActionType::kPictureInPicture:
        ...
        if (enable) {
            ...
            PictureInPictureController::From(node->GetDocument()).ExitPictureInPicture(To<HTMLVideoElement>(media_element), nullptr);
        }
...
void LocalFrame::AdvanceFocusForIME(mojom::blink::FocusType focus_type)
{
    ...
    Element* next_element = GetPage()->GetFocusController().NextFocusableElementForIME(element, focus_type);
    if (!next_element)
        return;
...
void LocalFrame::PostMessageEvent(
    const absl::optional<RemoteFrameToken>& source_frame_token, const String& source_origin, const String& target_origin, BlinkTransferableMessage message)
{
    ...
    message_event->initMessageEvent(
        "message", false, false, std::move(message.message), source_origin, "" /*lastEventId*/, window, ports, user_activation, message.delegated_capability);
...
bool LocalFrame::HasBlockingReasonsHelper(const mojom::blink::BackForwardCacheNotRestoredReasonsPtr& not_restored)
{
