...
// static
PendingPostBeacon* PendingPostBeacon::Create(ExecutionContext* ec, const String& target_url)
{
...
// static
PendingPostBeacon* PendingPostBeacon::Create(ExecutionContext* ec, const String& target_url, PendingBeaconOptions* options)
{
    return MakeGarbageCollected<PendingPostBeacon>(ec, target_url, options->backgroundTimeout(), options->timeout(), base::PassKey<PendingPostBeacon>());
}

PendingPostBeacon::PendingPostBeacon(
    ExecutionContext* context, const String& url, int32_t background_timeout, int32_t timeout, base::PassKey<PendingPostBeacon> key)
    : PendingBeacon(context, url, http_names::kPOST, background_timeout, timeout)
{
}

void PendingPostBeacon::setData(const V8UnionReadableStreamOrXMLHttpRequestBodyInit* data, ExceptionState& exception_state)
{
    using ContentType = V8UnionReadableStreamOrXMLHttpRequestBodyInit::ContentType;
    ...
    case ContentType::kArrayBuffer: {
        ...
        if (!base::CheckedNumeric<wtf_size_t>(array_buffer->ByteLength()).IsValid()) {
            exception_state.ThrowRangeError("The data provided to setData() exceeds the maximally "
    ...
    case ContentType::kArrayBufferView: {
        ...
        SetDataInternal(BeaconDOMArrayBufferView(array_buffer_view), exception_state);
        return;
    }
    ...
    case ContentType::kURLSearchParams: {
        SetDataInternal(BeaconURLSearchParams(data->GetAsURLSearchParams()), exception_state);
        return;
    }
    ...
    case ContentType::kReadableStream: {
        exception_state.ThrowTypeError("PendingPostBeacon cannot have a ReadableStream body.");
        return;
    }
    }
    NOTIMPLEMENTED();
