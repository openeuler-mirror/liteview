...
bool LocalFrameView::LayoutFromRootObject(LayoutObject& root)
{
    ...
    if (auto* locked_ancestor = DisplayLockUtilities::LockedAncestorPreventingLayout(root)) {
...
void LocalFrameView::PerformLayout()
{
    ...
    if (!in_subtree_layout) {
        ...
        if (body && body->GetLayoutObject()) {
            if (IsA<HTMLFrameSetElement>(*body)) {
                ...
                if (!first_layout_ && size_.Height() != GetLayoutSize().height() && body->GetLayoutObject()->EnclosingBox()->StretchesToViewport())
                    body->GetLayoutObject()->SetChildNeedsLayout();
            }
        }
        ...
        if (old_size != size_ && !first_layout_) {
            LayoutBox* root_layout_object = document->documentElement() ? document->documentElement()->GetLayoutBox() : nullptr;
            LayoutBox* body_layout_object = root_layout_object && document->body() ? document->body()->GetLayoutBox() : nullptr;
...

    {
        ...
        if (in_subtree_layout) {
            ...
            DeferredShapingController& ds_controller = GetLayoutView()->GetDeferredShapingController();
            bool default_allow_deferred_shaping = ds_controller.DefaultAllowDeferredShaping() && RuntimeEnabledFeatures::DeferredShapingEnabled()
                && !frame_->PagePopupOwner() && !auto_size_info_ && !GetScrollableArea()->HasPendingHistoryRestoreScrollOffset();
            if (!default_allow_deferred_shaping)
                ds_controller.DisallowDeferredShaping();
            using PassKey = base::PassKey<LocalFrameView>;
            ds_controller.SetAllowDeferredShaping(PassKey(), default_allow_deferred_shaping && !document->Printing());
    ...
    if (old_size != new_size) {
        MarkFixedPositionObjectsForLayout(old_size.width() != new_size.width(), old_size.height() != new_size.height());
    }
...
void LocalFrameView::UpdateLayout()
{
    ...
    ENTER_EMBEDDER_STATE(V8PerIsolateData::MainThreadIsolate(), &GetFrame(), BlinkState::LAYOUT);
    TRACE_EVENT_BEGIN0("blink,benchmark", "LocalFrameView::layout");
    if (UNLIKELY(RuntimeEnabledFeatures::BlinkRuntimeCallStatsEnabled())) {
        rcs_scope.emplace(RuntimeCallStats::From(V8PerIsolateData::MainThreadIsolate()), RuntimeCallStats::CounterId::kUpdateLayout);
    }
...
void LocalFrameView::DidFinishForcedLayout(DocumentUpdateReason reason)
{
    ...
    if (!forced_layout_stack_depth_ && base::TimeTicks::IsHighResolution()) {
        ...
        aggregator.RecordForcedLayoutSample(reason, forced_layout_start_time_, base::TimeTicks::Now());
    }
}
...
{
    base::AutoReset<bool> in_post_lifecycle_steps(&in_post_lifecycle_steps_, true);
...
void LocalFrameView::RunIntersectionObserverSteps()
{
    ...
    if (frame_->IsOutermostMainFrame()) {
        ...
        gfx::Rect main_frame_dimensions(ToRoundedSize(To<LayoutBox>(layout_object)->PhysicalLayoutOverflowRect().size));
    ...
    SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kIntersectionObservation);

    // Populating monotonic_time may be expensive, and may be unnecessary, so
    // allow it to be populated on demand.
    absl::optional<base::TimeTicks> monotonic_time;
    bool needs_occlusion_tracking = UpdateViewportIntersectionsForSubtree(0, monotonic_time);
...
void LocalFrameView::ForceUpdateViewportIntersections()
{
    ...
    UpdateLifecycleToPrePaintClean(DocumentUpdateReason::kIntersectionObservation);
    absl::optional<base::TimeTicks> monotonic_time;
    UpdateViewportIntersectionsForSubtree(IntersectionObservation::kImplicitRootObserversNeedUpdate | IntersectionObservation::kIgnoreDelay, monotonic_time);
}
...
void LocalFrameView::RemoveBackgroundAttachmentFixedObject(LayoutObject* object)
{
...
bool LocalFrameView::RequiresMainThreadScrollingForBackgroundAttachmentFixed() const
{
    ...
    const auto* object = To<LayoutBoxModelObject>(background_attachment_fixed_objects_.begin()->Get());
    ...
    if (IsA<LayoutView>(object) && object->GetBackgroundPaintLocation() == kBackgroundPaintInBorderBoxSpace)
        return false;
    return true;
}
...
void LocalFrameView::ViewportSizeChanged(bool width_changed, bool height_changed)
{
...
void LocalFrameView::MarkFixedPositionObjectsForLayout(bool width_changed, bool height_changed)
{
    ...
    for (const auto& layout_object : *fixed_position_objects_) {
        ...
        if (width_changed) {
            if (style.Width().IsFixed() && (style.Left().IsAuto() || style.Right().IsAuto())) {
                ...
                layout_object->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSizeChanged);
            }
        }
        if (height_changed) {
...
void LocalFrameView::InvalidateBackgroundAttachmentFixedDescendantsOnScroll(const LayoutObject& scrolled_object)
{
    for (const auto& layout_object : background_attachment_fixed_objects_) {
        if (scrolled_object != GetLayoutView() && !layout_object->IsDescendantOf(&scrolled_object))
            continue;
        ...
        if (layout_object == GetLayoutView() && !(GetLayoutView()->GetBackgroundPaintLocation() & kBackgroundPaintInContentsSpace))
            continue;
...
void LocalFrameView::ProcessUrlFragment(const KURL& url, bool same_document_navigation, bool should_scroll)
{
    ...
    if (anchor) {
        ...
        if (const auto* layout_view = GetLayoutView()) {
            layout_view->GetDeferredShapingController().ReshapeAllDeferred(ReshapeReason::kFragmentAnchor);
        }
    }
}
...
static inline void RemoveFloatingObjectsForSubtreeRoot(LayoutObject& root)
{
    ...
    if (LayoutBlock* cb = root.ContainingBlock()) {
        ...
        if ((cb->NormalChildNeedsLayout() || cb->SelfNeedsLayout()) && child_block_flow) {
...
static bool PrepareOrthogonalWritingModeRootForLayout(LayoutObject& root)
{
    ...
    if (!root.NeedsLayout() || root.IsOutOfFlowPositioned() || root.IsColumnSpanAll() || root.StyleRef().LogicalHeight().IsSpecified()
        || To<LayoutBox>(root).IsGridItem() || root.IsTablePart() || root.IsLayoutFlowThread())
        return false;
...
void LocalFrameView::MarkOrthogonalWritingModeRootsForLayout()
{
    for (auto& root : orthogonal_writing_mode_root_list_.Ordered()) {
        ...
        root->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kSizeChanged);
    }
}
...
bool LocalFrameView::RunPostLayoutIntersectionObserverSteps()
{
    ...
    ForAllNonThrottledLocalFrameViews([&needs_more_lifecycle_steps](LocalFrameView& frame_view) {
        ...
        needs_more_lifecycle_steps |= frame_view.Lifecycle().GetState() < DocumentLifecycle::kPrePaintClean;
    });

    return needs_more_lifecycle_steps;
}

void LocalFrameView::ComputePostLayoutIntersections(unsigned parent_flags, absl::optional<base::TimeTicks>& monotonic_time)
{
    if (ShouldThrottleRendering())
        return;
    ...
    if (auto* controller = GetFrame().GetDocument()->GetIntersectionObserverController()) {
        controller->ComputeIntersections(flags, EnsureUkmAggregator(), monotonic_time);
    }
...
void LocalFrameView::SetUseColorAdjustBackground(UseColorAdjustBackground use, bool color_scheme_changed)
{
    ...
    if (GetFrame().IsMainFrame() && ShouldUseColorAdjustBackground()) {
        ...
        GetFrame().DidChangeBackgroundColor(BaseBackgroundColor().ToSkColorDeprecated(), true /* color_adjust */);
    }
...
void LocalFrameView::InitializeRootScroller()
{
    ...
    DCHECK(frame_->GetDocument());
    page->GlobalRootScrollerController().InitializeViewportScrollCallback(*root_frame_viewport, *frame_->GetDocument());
}
...
Color LocalFrameView::DocumentBackgroundColor()
{
    ...
    if (background_source->StyleRef().ColorSchemeForced()) {
        ...
        doc_bg = Color::FromSkColor(EnsureDarkModeFilter().InvertColorIfNeeded(doc_bg.ToSkColorDeprecated(), DarkModeFilter::ElementRole::kBackground));
    }
    if (blend_with_base)
...
void LocalFrameView::WillBeRemovedFromFrame()
{
    ...
    if (Settings* settings = frame_->GetSettings()) {
        ...
        if (settings->GetSpatialNavigationEnabled()) {
            frame_->GetPage()->GetSpatialNavigationController().DidDetachFrameView(*this);
        }
    }
}
...
bool LocalFrameView::UpdateLifecycleToPrePaintClean(DocumentUpdateReason reason)
{
...
bool LocalFrameView::NotifyResizeObservers()
{
    ...
    if (min_depth != ResizeObserverController::kDepthBottom) {
        ...
        if (resize_controller->SkippedObservations() && !resize_controller->IsLoopLimitErrorDispatched()) {
            ...
            ErrorEvent* error = ErrorEvent::Create("ResizeObserver loop limit exceeded", CaptureSourceLocation(frame_->DomWindow()), nullptr);
...
bool LocalFrameView::UpdateLifecyclePhases(DocumentLifecycle::LifecycleState target_state, DocumentUpdateReason reason)
{
    ...
    if (UNLIKELY(frame_->GetDocument() && frame_->GetDocument()->Lifecycle().LifecyclePostponed())) {
        return false;
    }
    ...
    if (UNLIKELY(IsUpdatingLifecycle())) {
        NOTREACHED() << "LocalFrameView::updateLifecyclePhasesInternal() reentrance";
        return false;
    }
    ...
    DCHECK(target_state == DocumentLifecycle::kLayoutClean || target_state == DocumentLifecycle::kAccessibilityClean
        || target_state == DocumentLifecycle::kCompositingInputsClean || target_state == DocumentLifecycle::kPrePaintClean
    ...
    if (reason == DocumentUpdateReason::kHitTest) {
        LocalFrameUkmAggregator& aggregator = EnsureUkmAggregator();
        aggregator.RecordTimerSample(static_cast<size_t>(LocalFrameUkmAggregator::kHitTestDocumentUpdate), lifecycle_data_.start_time, base::TimeTicks::Now());
    }
...
void LocalFrameView::UpdateLifecyclePhasesInternal(DocumentLifecycle::LifecycleState target_state)
{
    ...
    while (true) {
        ...
        if (RuntimeEnabledFeatures::CSSTogglesEnabled() && should_run_css_toggle_steps) {
        ...
        if (RuntimeEnabledFeatures::CSSScrollTimelineEnabled() && should_run_scroll_timeline_steps) {
...

        {
            ...
            DCHECK(ShouldThrottleRendering() || !ExistingAXObjectCache() || Lifecycle().GetState() == DocumentLifecycle::kAccessibilityClean);
            if (!run_more_lifecycle_phases)
                return;
...
bool LocalFrameView::RunScrollTimelineSteps()
{
    ...
    ForAllNonThrottledLocalFrameViews([&re_run_lifecycles](LocalFrameView& frame_view) {
        bool timelines_valid = frame_view.GetFrame().GetDocument()->GetDocumentAnimations().ValidateTimelines();
...
bool LocalFrameView::RunCSSToggleSteps()
{
    ...
    ForAllNonThrottledLocalFrameViews(
        [&re_run_lifecycles](LocalFrameView& frame_view) { re_run_lifecycles |= frame_view.GetFrame().GetDocument()->SetNeedsStyleRecalcForToggles(); });
    return re_run_lifecycles;
}

bool LocalFrameView::RunDocumentTransitionSteps(DocumentLifecycle::LifecycleState target_state)
{
    ...
    auto* document_transition_supplement = DocumentTransitionSupplement::FromIfExists(*frame_->GetDocument());
...
bool LocalFrameView::RunCompositingInputsLifecyclePhase(DocumentLifecycle::LifecycleState target_state)
{
    ...
    SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kCompositingInputs);
...
bool LocalFrameView::RunPrePaintLifecyclePhase(DocumentLifecycle::LifecycleState target_state)
{
            ...
            if (LayoutView* layout_view = frame_view.GetLayoutView()) {
                if (auto* owner = frame_view.GetFrame().OwnerLayoutObject()) {
                    ...
                    if (!RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
                        ...
                        if (layout_view->Layer()->NeedsCullRectUpdate() || layout_view->Layer()->DescendantNeedsCullRectUpdate()) {
                            layout_view->Layer()->MarkCompositingContainerChainForNeedsCullRectUpdate();
                        }
                    }
                }
            }
    ...
    {
        SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kPrePaint);
        ...
        ForAllNonThrottledLocalFrameViews([](LocalFrameView& view) { view.frame_->UpdateFrameColorOverlayPrePaint(); });
...
void LocalFrameView::RunPaintLifecyclePhase(PaintBenchmarkMode benchmark_mode)
{
    ...
    bool needed_update;
    {
        ...
        if (paint_artifact_compositor_ && benchmark_mode == PaintBenchmarkMode::kForcePaintArtifactCompositorUpdate) {
            paint_artifact_compositor_->SetNeedsUpdate(PaintArtifactCompositorUpdateReason::kLocalFrameViewBenchmarking);
        }
    ...
    ForAllNonThrottledLocalFrameViews([this, &needed_update, &total_animations_count](LocalFrameView& frame_view) {
        ...
        Document& document = frame_view.GetLayoutView()->GetDocument();
        {
            ...
            ScriptForbiddenScope forbid_script;
            document.GetDocumentAnimations().UpdateAnimations(DocumentLifecycle::kPaintClean, paint_artifact_compositor_.get(), needed_update);
        }
...
bool LocalFrameView::RunAccessibilityLifecyclePhase(DocumentLifecycle::LifecycleState target_state)
{
    TRACE_EVENT0("blink,benchmark", "LocalFrameView::RunAccessibilityLifecyclePhase");

    SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kAccessibility);
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
        if (AXObjectCache* cache = frame_view.ExistingAXObjectCache()) {
            ...
            cache->ProcessDeferredAccessibilityEvents(*frame_view.GetFrame().GetDocument());
...
bool LocalFrameView::PaintTree(PaintBenchmarkMode benchmark_mode, PaintControllerCycleScope& cycle_scope)
{
    SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kPaint);
    ...
    bool debug_info_newly_enabled = UpdateLayerDebugInfoEnabled() && PaintDebugInfoEnabled();
    ...
    PaintController::ScopedBenchmarkMode scoped_benchmark(*paint_controller_, benchmark_mode);

    if (paint_controller_->ShouldForcePaintForBenchmark() || GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint()
        ...
        repainted = true;
        if (paint_artifact_compositor_) {
            paint_artifact_compositor_->SetNeedsFullUpdateAfterPaintIfNeeded(previous_chunks, paint_controller_->GetPaintArtifactShared());
        }
    }
...
void LocalFrameView::PushPaintArtifactToCompositor(bool repainted)
{
    ...
    if (!paint_artifact_compositor_) {
        paint_artifact_compositor_ = std::make_unique<PaintArtifactCompositor>(page->GetScrollingCoordinator()->GetWeakPtr());
        page->GetChromeClient().AttachRootLayer(paint_artifact_compositor_->RootLayer(), &GetFrame());
    }

    paint_artifact_compositor_->SetPrefersLCDText(!page->GetSettings().GetPreferCompositingToLCDTextEnabled());

    SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kCompositingCommit);
    ...
    if (!paint_artifact_compositor_->NeedsUpdate()) {
        if (repainted) {
            paint_artifact_compositor_->UpdateRepaintedLayers(paint_controller_->GetPaintArtifactShared());
    ...
    paint_artifact_compositor_->SetLayerDebugInfoEnabled(layer_debug_info_enabled_);
    ...
    if (const auto& viewport = page->GetVisualViewport(); GetFrame().IsMainFrame() && viewport.IsActiveViewport()) {
        viewport_properties.overscroll_elasticity_effect = viewport.GetOverscrollElasticityEffectNode();
    ...
    if (base::FeatureList::IsEnabled(::features::kScrollUnification)) {
        ForAllNonThrottledLocalFrameViews(
            [&scroll_translation_nodes](LocalFrameView& frame_view) { frame_view.GetUserScrollTranslationNodes(scroll_translation_nodes); });
    }

    WTF::Vector<std::unique_ptr<DocumentTransitionRequest>> document_transition_requests;
    ...
    paint_artifact_compositor_->Update(
        paint_controller_->GetPaintArtifactShared(), viewport_properties, scroll_translation_nodes, std::move(document_transition_requests));

    CreatePaintTimelineEvents();
}

void LocalFrameView::AppendDocumentTransitionRequests(WTF::Vector<std::unique_ptr<DocumentTransitionRequest>>& requests)
{
    DCHECK(frame_ && frame_->GetDocument());
    auto* document_transition_supplement = DocumentTransitionSupplement::FromIfExists(*frame_->GetDocument());
    if (!document_transition_supplement)
...
void LocalFrameView::UpdateStyleAndLayoutIfNeededRecursive()
{
    ...
    TRACE_EVENT0("blink,benchmark", "LocalFrameView::updateStyleAndLayoutIfNeededRecursive");
    ...
    if (GetFrame().LocalFrameRoot().View()->target_state_ == DocumentLifecycle::kPaintClean) {
...
void LocalFrameView::UpdateStyleAndLayout()
{
    ...
    VisualViewport& visual_viewport = frame_->GetPage()->GetVisualViewport();
    gfx::SizeF visual_viewport_size(visual_viewport.VisibleWidthCSSPx(), visual_viewport.VisibleHeightCSSPx());
    ...
    if (did_layout) {
        ...
        if (frame_->IsMainFrame()) {
            ...
            gfx::SizeF new_viewport_size(visual_viewport.VisibleWidthCSSPx(), visual_viewport.VisibleHeightCSSPx());
            visual_viewport_size_changed = (new_viewport_size != visual_viewport_size);
            DCHECK(!visual_viewport_size_changed || visual_viewport.IsActiveViewport());
        }
...
bool LocalFrameView::UpdateStyleAndLayoutInternal()
{
    ...
    if (NeedsLayout()) {
        SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kLayout);
        UpdateLayout();
        return true;
    }
    return false;
...
void LocalFrameView::ForceLayoutForPagination(const gfx::SizeF& page_size, const gfx::SizeF& original_page_size, float maximum_shrink_factor)
{
    ...
    if (LayoutView* layout_view = GetLayoutView()) {
        layout_view->SetPageSize({ LayoutUnit(page_size.width()), LayoutUnit(page_size.height()) });
        ...
        bool horizontal_writing_mode = layout_view->StyleRef().IsHorizontalWritingMode();
        PhysicalRect document_rect(layout_view->DocumentRect());
        LayoutUnit doc_logical_width = horizontal_writing_mode ? document_rect.Width() : document_rect.Height();
        float page_logical_width = horizontal_writing_mode ? page_size.width() : page_size.height();
        if (doc_logical_width > page_logical_width) {
            ...
            gfx::SizeF expected_page_size(std::min<float>(document_rect.Width().Round(), page_size.width() * maximum_shrink_factor),
                std::min<float>(document_rect.Height().Round(), page_size.height() * maximum_shrink_factor));
            gfx::SizeF max_page_size = frame_->ResizePageRectsKeepingRatio(original_page_size, expected_page_size);
            page_logical_width = horizontal_writing_mode ? max_page_size.width() : max_page_size.height();
            layout_view->SetPageSize({ LayoutUnit(max_page_size.width()), LayoutUnit(max_page_size.height()) });
            layout_view->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kPrintingChanged);
            frame_->GetDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kPrinting);

            WritingModeConverter converter(layout_view->StyleRef().GetWritingDirection(), PhysicalSize(layout_view->Size()));
            LogicalRect logical_rect = converter.ToLogical(layout_view->DocumentRect());
            LayoutUnit clipped_logical_left;
            if (!layout_view->StyleRef().IsLeftToRightDirection()) {
                clipped_logical_left = LayoutUnit(logical_rect.InlineEndOffset() - page_logical_width);
            }
            ...
            layout_view->AddLayoutOverflow(converter.ToPhysical(logical_rect).ToLayoutFlippedRect(layout_view->StyleRef(), PhysicalSize(layout_view->Size())));
            return;
        }
    }
...
gfx::Point LocalFrameView::ConvertToContainingEmbeddedContentView(const gfx::Point& local_point) const
{
    return ToRoundedPoint(ConvertToContainingEmbeddedContentView(PhysicalOffset(local_point)));
}
...
void LocalFrameView::PaintOutsideOfLifecycle(GraphicsContext& context, const PaintFlags paint_flags, const CullRect& cull_rect)
{
...

    {
        ...
        OverriddenOldCullRectScope force_old_cull_rect(*GetLayoutView()->Layer(), cull_rect);
        PaintControllerCycleScope cycle_scope(context.GetPaintController(), PaintDebugInfoEnabled());
        PaintFrame(context, paint_flags);
    }
...
void LocalFrameView::PaintForTest(const CullRect& cull_rect)
{
    ...
    if (GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint()) {
        PaintControllerCycleScope cycle_scope(paint_controller, PaintDebugInfoEnabled());
...
sk_sp<PaintRecord> LocalFrameView::GetPaintRecord() const
{
    ...
    return paint_controller_->GetPaintArtifact().GetPaintRecord(PropertyTreeState::Root());
}
...
void LocalFrameView::CollectAnnotatedRegions(LayoutObject& layout_object, Vector<AnnotatedRegionValue>& regions) const
{
...
bool LocalFrameView::UpdateViewportIntersectionsForSubtree(unsigned parent_flags, absl::optional<base::TimeTicks>& monotonic_time)
{
    ...
    if (!NeedsLayout() || IsDisplayLocked()) {
        ...
        if (IntersectionObserverController* controller = GetFrame().GetDocument()->GetIntersectionObserverController()) {
            needs_occlusion_tracking |= controller->ComputeIntersections(flags, EnsureUkmAggregator(), monotonic_time);
        }
        intersection_observation_state_ = kNotNeeded;
    }

    {
        SCOPED_UMA_AND_UKM_TIMER(EnsureUkmAggregator(), LocalFrameUkmAggregator::kUpdateViewportIntersection);
        UpdateViewportIntersection(flags, needs_occlusion_tracking);
    }

    for (Frame* child = frame_->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
        needs_occlusion_tracking |= child->View()->UpdateViewportIntersectionsForSubtree(flags, monotonic_time);
    }

    if (DocumentPortals* portals = DocumentPortals::Get(*frame_->GetDocument())) {
        for (PortalContents* portal : portals->GetPortals()) {
            if (Frame* frame = portal->GetFrame()) {
                needs_occlusion_tracking |= frame->View()->UpdateViewportIntersectionsForSubtree(flags, monotonic_time);
            }
        }
    }
...
void LocalFrameView::RenderThrottlingStatusChanged()
{
    ...
    if (!CanThrottleRendering()) {
        ...
        SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES("Blink.RenderThrottling.PaintUpdateTime");
...
void LocalFrameView::SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason reason)
{
    LocalFrameView* root = GetFrame().LocalFrameRoot().View();
...
{
    if (lifecycle_updates_throttled_ || IsSubtreeThrottled() || IsDisplayLocked()) {
        return true;
    }
...
MainThreadScrollingReasons LocalFrameView::MainThreadScrollingReasonsPerFrame() const
{
...
MainThreadScrollingReasons LocalFrameView::GetMainThreadScrollingReasons() const
{
    MainThreadScrollingReasons reasons = static_cast<MainThreadScrollingReasons>(0);
    ...
    DCHECK(!cc::MainThreadScrollingReason::HasNonCompositedScrollReasons(reasons));
    return reasons;
}
...
void LocalFrameView::MapLocalToRemoteMainFrame(TransformState& transform_state)
{
    ...
    if (frame_->IsOutermostMainFrame())
        return;
    transform_state.ApplyTransform(TransformationMatrix(GetFrame().RemoteMainFrameTransform()), TransformState::kAccumulateTransform);
}
...
void LocalFrameView::DidChangeMobileFriendliness(const blink::MobileFriendliness& mf)
{
...
LocalFrameUkmAggregator& LocalFrameView::EnsureUkmAggregator()
{
    ...
    if (!local_root->ukm_aggregator_) {
        local_root->ukm_aggregator_
            = base::MakeRefCounted<LocalFrameUkmAggregator>(local_root->frame_->GetDocument()->UkmSourceID(), local_root->frame_->GetDocument()->UkmRecorder());
        if (base::FeatureList::IsEnabled(features::kThrottleIntersectionObserverUMA)) {
...
bool LocalFrameView::UpdateLayerDebugInfoEnabled()
{
...
#else
    bool should_enable = cc::frame_viewer_instrumentation::IsTracingLayerTreeSnapshots() || WebTestSupport::IsRunningWebTest()
    ...
    if (should_enable != layer_debug_info_enabled_) {
        ...
        SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason::kLocalFrameViewUpdateLayerDebugInfo);
        return true;
    }
#endif
    return false;
...
void LocalFrameView::GetUserScrollTranslationNodes(Vector<const TransformPaintPropertyNode*>& scroll_translation_nodes)
{
...
void LocalFrameView::RunPaintBenchmark(int repeat_count, cc::PaintBenchmarkResult& result)
{
    ...
    if (paint_controller_) {
        result.painter_memory_usage += paint_controller_->ApproximateUnsharedMemoryUsage();
    }
    if (paint_artifact_compositor_) {
...
void LocalFrameView::UpdateAllPendingTransforms()
{
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
        if (frame_view.pending_transform_updates_) {
            for (const LayoutObject* object : *frame_view.pending_transform_updates_) {
