...
LocalFrameView* RemoteFrameView::ParentFrameView() const
{
    ...
    if (owner && (owner->OwnerType() == FrameOwnerElementType::kPortal || owner->OwnerType() == FrameOwnerElementType::kFencedframe)) {
...
bool RemoteFrameView::UpdateViewportIntersectionsForSubtree(unsigned parent_flags, absl::optional<base::TimeTicks>&)
{
...
gfx::Rect RemoteFrameView::ComputeCompositingRect() const
{
    ...
    TransformationMatrix matrix = local_root_transform_state.AccumulatedTransform().Inverse();
...
void RemoteFrameView::UpdateCompositingRect()
{
    ...
    if (auto frozen_size = owner_layout_object->FrozenFrameSize()) {
        compositing_rect_ = gfx::Rect(frozen_size->width.Ceil(), frozen_size->height.Ceil());
    } else {
...
void RemoteFrameView::UpdateCompositingScaleFactor()
{
    ...
    gfx::Transform local_root_transform = local_root_transform_state.AccumulatedTransform().ToTransform();
    absl::optional<gfx::Vector2dF> scale_components = gfx::TryComputeTransform2dScaleComponents(local_root_transform);
    if (!scale_components) {
    ...
    compositing_scale_factor_ = base::clamp(compositing_scale_factor_, kMinCompositingScaleFactor, kMaxCompositingScaleFactor);
...
void RemoteFrameView::PropagateFrameRects()
{
    ...
    if (auto* layout_object = GetLayoutEmbeddedContent()) {
        if (auto frozen_size = layout_object->FrozenFrameSize()) {
            frame_size = gfx::Size(frozen_size->width.Ceil(), frozen_size->height.Ceil());
        }
    }
