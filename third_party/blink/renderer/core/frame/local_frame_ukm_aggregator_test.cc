...
public:
    ...
    {
        aggregator_ = base::MakeRefCounted<LocalFrameUkmAggregator>(ukm::UkmRecorder::GetNewSourceID(), &recorder_);
...
TEST_F(LocalFrameUkmAggregatorTest, ForcedLayoutReasonsReportOnlyMetric)
{
    ...
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kScroll, LocalFrameUkmAggregator::kScrollDocumentUpdate, 24u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kHitTest, LocalFrameUkmAggregator::kHitTestDocumentUpdate, 25u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kJavaScript, LocalFrameUkmAggregator::kJavascriptDocumentUpdate, 26u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kBeginMainFrame, LocalFrameUkmAggregator::kCount, 27u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kTest, LocalFrameUkmAggregator::kCount, 28u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kUnknown, LocalFrameUkmAggregator::kCount, 29u);
}
...
protected:
    void ChooseNextFrameForTest()
    {
        LocalFrameRoot().GetFrame()->View()->EnsureUkmAggregator().ChooseNextFrameForTest();
    }
    ...
    void TestIntersectionObserverCounts(Document& document)
    {
        ...
        TestIntersectionObserverDelegate* internal_delegate
            = MakeGarbageCollected<TestIntersectionObserverDelegate>(document, LocalFrameUkmAggregator::kLazyLoadIntersectionObserver);
        IntersectionObserver* internal_observer = IntersectionObserver::Create(observer_init, *internal_delegate);
        ...
        TestIntersectionObserverDelegate* javascript_delegate
            = MakeGarbageCollected<TestIntersectionObserverDelegate>(document, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
        IntersectionObserver* javascript_observer = IntersectionObserver::Create(observer_init, *javascript_delegate);
        ...
        document.View()->EnsureUkmAggregator().RecordEndOfFrameMetrics(base::TimeTicks(), base::TimeTicks() + base::Microseconds(10), 0);
...
TEST_F(LocalFrameUkmAggregatorSimTest, EnsureUkmAggregator)
{
    ...
    auto* subframe_view = To<HTMLFrameOwnerElement>(GetDocument().getElementById("frame"))->contentDocument()->View();
...
TEST_F(LocalFrameUkmAggregatorSimTest, IntersectionObserverCountsInChildFrame)
{
    ...
    Compositor().BeginFrame();
    ChooseNextFrameForTest();
    TestIntersectionObserverCounts(*To<HTMLFrameOwnerElement>(GetDocument().getElementById("frame"))->contentDocument());
}
