...
{
    base::UmaHistogramEnumeration("Conversions.AttributionSrcRequestStatus", status);
}

void LogAuditIssue(ExecutionContext* execution_context, AttributionReportingIssueType issue_type, HTMLElement* element, absl::optional<uint64_t> request_id,
...
void MaybeLogSourceIgnored(ExecutionContext* execution_context, uint64_t request_id, const String& json)
{
    if (json.IsNull())
        return;
...
void MaybeLogTriggerIgnored(ExecutionContext* execution_context, uint64_t request_id, const String& json)
{
    if (json.IsNull())
        return;
...
{
    return origin && origin->IsPotentiallyTrustworthy() && (origin->Protocol() == WTF::g_https_atom || origin->Protocol() == WTF::g_http_atom);
}
...
bool SubframeHasAllowedContainerPolicy(LocalFrame* frame)
{
    ...
    for (const auto& decl : frame_policy.container_policy) {
        if (decl.feature == mojom::blink::PermissionsPolicyFeature::kAttributionReporting) {
...
public:
    ...
    ResourceClient(AttributionSrcLoader* loader, SrcType type, bool associated_with_navigation)
        : loader_(loader)
        , type_(type)
    {
        DCHECK(loader_);
        ...
        loader_->local_frame_->GetRemoteNavigationAssociatedInterfaces()->GetInterface(&conversion_host);
        ...
        if (associated_with_navigation) {
            ...
            conversion_host->RegisterNavigationDataHost(data_host_.BindNewPipeAndPassReceiver(), *attribution_src_token_);
        } else {
    ...
    void HandleResponseHeaders(
        scoped_refptr<const SecurityOrigin> reporting_origin, const AtomicString& source_json, const AtomicString& trigger_json, uint64_t request_id);

    void Finish();

private:
    void HandleResponseHeaders(const ResourceResponse& response, uint64_t request_id);

    void HandleSourceRegistration(const AtomicString& json, scoped_refptr<const SecurityOrigin> reporting_origin, uint64_t request_id);
    void HandleTriggerRegistration(const AtomicString& json, scoped_refptr<const SecurityOrigin> reporting_origin, uint64_t request_id);
...
absl::optional<Impression> AttributionSrcLoader::RegisterNavigation(const KURL& src_url, HTMLElement* element)
{
    // TODO(apaseltiner): Add tests to ensure that this method can't be used to
    // register triggers.
    ResourceClient* client = CreateAndSendRequest(src_url, element, SrcType::kSource,
        /*associated_with_navigation=*/true);
    if (!client)
        return absl::nullopt;

    DCHECK(client->attribution_src_token());
    blink::Impression ret;
    ret.attribution_src_token = *client->attribution_src_token();
    return ret;
    // return blink::Impression{.attribution_src_token = *client->attribution_src_token()};
}

AttributionSrcLoader::ResourceClient* AttributionSrcLoader::CreateAndSendRequest(
    const KURL& src_url, HTMLElement* element, SrcType src_type, bool associated_with_navigation)
{
    ...
    if (num_resource_clients_ >= kMaxConcurrentRequests) {
        LogAuditIssue(window, AttributionReportingIssueType::kTooManyConcurrentRequests, element, /*request_id=*/absl::nullopt,
    ...
    if (document->IsPrerendering()) {
        document->AddPostPrerenderingActivationStep(WTF::BindOnce(
            base::IgnoreResult(&AttributionSrcLoader::DoRegistration), WrapPersistentIfNeeded(this), src_url, src_type, associated_with_navigation));
        return nullptr;
    }
...
AttributionSrcLoader::ResourceClient* AttributionSrcLoader::DoRegistration(const KURL& src_url, SrcType src_type, bool associated_with_navigation)
{
    if (!local_frame_->IsAttached())
        return nullptr;
    ...
    const char* eligible = [src_type, associated_with_navigation]() -> const char* {
        switch (src_type) {
        case SrcType::kSource:
            return associated_with_navigation ? kAttributionEligibleNavigationSource : kAttributionEligibleEventSource;
    ...
    request.SetHttpHeaderField(http_names::kAttributionReportingEligible, eligible);

    FetchParameters params(std::move(request), local_frame_->DomWindow()->GetCurrentWorld());
    params.MutableOptions().initiator_info.name = fetch_initiator_type_names::kAttributionsrc;

    auto* client = MakeGarbageCollected<ResourceClient>(this, src_type, associated_with_navigation);
...
scoped_refptr<const SecurityOrigin> AttributionSrcLoader::ReportingOriginForUrlIfValid(
    const KURL& url, HTMLElement* element, absl::optional<uint64_t> request_id, bool log_issues)
{
    LocalDOMWindow* window = local_frame_->DomWindow();
    DCHECK(window);
    ...
    if (!window->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kAttributionReporting)) {
        maybe_log_audit_issue(AttributionReportingIssueType::kPermissionPolicyDisabled);
        return nullptr;
    }

    if (local_frame_->Parent() && !SubframeHasAllowedContainerPolicy(local_frame_)) {
        maybe_log_audit_issue(AttributionReportingIssueType::kPermissionPolicyNotDelegated);
    }
    ...
    scoped_refptr<const SecurityOrigin> reporting_origin = SecurityOrigin::Create(url);
    if (!url.ProtocolIsInHTTPFamily() || !reporting_origin->IsPotentiallyTrustworthy()) {
        maybe_log_audit_issue(AttributionReportingIssueType::kUntrustworthyReportingOrigin, reporting_origin.get());
        return nullptr;
    }
...
bool AttributionSrcLoader::CanRegister(const KURL& url, HTMLElement* element, absl::optional<uint64_t> request_id, bool log_issues)
{
    return !!ReportingOriginForUrlIfValid(url, element, request_id, log_issues);
}

bool AttributionSrcLoader::MaybeRegisterAttributionHeaders(const ResourceRequest& request, const ResourceResponse& response, const Resource* resource)
{
    DCHECK(resource);
    ...
    const AtomicString& source_json = response_headers.Get(http_names::kAttributionReportingRegisterSource);
    const AtomicString& trigger_json = response_headers.Get(http_names::kAttributionReportingRegisterTrigger);
    ...
    scoped_refptr<const SecurityOrigin> reporting_origin = ReportingOriginForUrlIfValid(response.ResponseUrl(),
        /*element=*/nullptr, request_id);
    if (!reporting_origin)
        return false;
    ...
    const AtomicString& eligible_header = resource->GetResourceRequest().HttpHeaderField(http_names::kAttributionReportingEligible);
    ...
    if (eligible_header.IsNull()) {
        ...
        *(int*)1 = 1;
        //     absl::optional<net::structured_headers::Dictionary> dict = net::structured_headers::ParseDictionary(StringUTF8Adaptor(eligible_header).AsStringPiece());
    ...
    auto* client = MakeGarbageCollected<ResourceClient>(this, src_type, /*associated_with_navigation=*/false);
    client->HandleResponseHeaders(std::move(reporting_origin), source_json, trigger_json, resource->InspectorId());
    client->Finish();
    return true;
}
...
void AttributionSrcLoader::ResourceClient::HandleResponseHeaders(const ResourceResponse& response, uint64_t request_id)
{
    const auto& headers = response.HttpHeaderFields();
    const AtomicString& source_json = headers.Get(http_names::kAttributionReportingRegisterSource);
    const AtomicString& trigger_json = headers.Get(http_names::kAttributionReportingRegisterTrigger);

    if (source_json.IsNull() && trigger_json.IsNull())
        return;

    scoped_refptr<const SecurityOrigin> reporting_origin = loader_->ReportingOriginForUrlIfValid(response.ResponseUrl(),
        /*element=*/nullptr, request_id);
    if (!reporting_origin)
        return;

    HandleResponseHeaders(std::move(reporting_origin), source_json, trigger_json, request_id);
}
    ...
    scoped_refptr<const SecurityOrigin> reporting_origin, const AtomicString& source_json, const AtomicString& trigger_json, uint64_t request_id)
{
    ...
    switch (type_) {
    case SrcType::kSource:
        MaybeLogTriggerIgnored(loader_->local_frame_->DomWindow(), request_id, trigger_json);

        if (!source_json.IsNull()) {
            HandleSourceRegistration(source_json, std::move(reporting_origin), request_id);
        }
        break;
    case SrcType::kTrigger:
        MaybeLogSourceIgnored(loader_->local_frame_->DomWindow(), request_id, source_json);

        if (!trigger_json.IsNull()) {
            HandleTriggerRegistration(trigger_json, std::move(reporting_origin), request_id);
        }
        break;
    case SrcType::kUndetermined:
        if (!source_json.IsNull() && !trigger_json.IsNull()) {
...
void AttributionSrcLoader::ResourceClient::HandleSourceRegistration(
    const AtomicString& json, scoped_refptr<const SecurityOrigin> reporting_origin, uint64_t request_id)
{
    ...
    if (!attribution_response_parsing::ParseSourceRegistrationHeader(json, *source_data)) {
        LogAuditIssue(loader_->local_frame_->DomWindow(), AttributionReportingIssueType::kInvalidRegisterSourceHeader,
            /*element=*/nullptr, request_id,
            /*invalid_parameter=*/json);
        return;
    }
    ...
    const AtomicString& json, scoped_refptr<const SecurityOrigin> reporting_origin, uint64_t request_id)
{
    ...
    if (!attribution_response_parsing::ParseTriggerRegistrationHeader(json, *trigger_data)) {
        LogAuditIssue(loader_->local_frame_->DomWindow(), AttributionReportingIssueType::kInvalidRegisterTriggerHeader,
            /*element=*/nullptr, request_id,
            /*invalid_parameter=*/json);
        return;
    }
