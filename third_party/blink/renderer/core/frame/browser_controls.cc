...
ScrollOffset BrowserControls::ScrollBy(ScrollOffset pending_delta)
{
    ...
    if ((permitted_state_ == cc::BrowserControlsState::kShown && pending_delta.y() > 0)
        || (permitted_state_ == cc::BrowserControlsState::kHidden && pending_delta.y() < 0))
        return pending_delta;
    ...
    float new_content_offset = baseline_content_offset - accumulated_scroll_delta_;
    ...
    new_content_offset = ClampTo(new_content_offset, base_on_top_controls ? TopMinHeight() : BottomMinHeight(), height);
    ...
    float min_ratio = base_on_top_controls ? TopMinShownRatio() : BottomMinShownRatio();
    float normalized_shown_ratio = (ClampTo(shown_ratio, min_ratio, 1.f) - min_ratio) / (1.f - min_ratio);
    ...
    SetShownRatio(TopMinShownRatio() + normalized_shown_ratio * (1.f - TopMinShownRatio()),
        BottomMinShownRatio() + normalized_shown_ratio * (1.f - BottomMinShownRatio()));
    ...
    ScrollOffset applied_delta(0, base_on_top_controls ? (old_top_offset - new_content_offset) / page_scale : 0);
    return pending_delta - applied_delta;
}

void BrowserControls::ScrollEnd()
{
    if ((top_shown_ratio_ == TopMinShownRatio() || top_shown_ratio_ == 1) && (bottom_shown_ratio_ == BottomMinShownRatio() || bottom_shown_ratio_ == 1)) {
        return;
    }
    ...
    float normalized_top_ratio = (top_shown_ratio_ - TopMinShownRatio()) / (1.f - TopMinShownRatio());
    if (normalized_top_ratio >= 1.f - kTopControlsHideThreshold) {
        // If we're showing so much that the hide threshold won't trigger, show.
        UpdateConstraintsAndState(permitted_state_, cc::BrowserControlsState::kShown);
        ...
        UpdateConstraintsAndState(permitted_state_, cc::BrowserControlsState::kHidden);
    } else {
        NOTREACHED();
    }
}
