...
public:
    explicit TestLocationReportBody(const String& source_file = g_empty_string, absl::optional<uint32_t> line_number = absl::nullopt,
...
{
    const unsigned empty_hash = TestLocationReportBody("", absl::nullopt, absl::nullopt).MatchId();
    for (const auto& input : kLocationReportBodyInputs) {
