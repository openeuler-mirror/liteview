...
public:
    ...
    void InitializeWithDesktopSettings(void (*override_settings_func)(WebSettings*) = nullptr)
    {
...
TEST_P(VisualViewportTest, TestResizeAfterVerticalScroll)
{
    ...
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(TransformationMatrix::MakeScale(2), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(0, -300), visual_viewport.GetScrollTranslationNode()->Translation2D());
        ...
        EXPECT_EQ(expected_projection,
            GeometryMapper::SourceToDestinationProjection(*visual_viewport.GetScrollTranslationNode(), TransformPaintPropertyNode::Root()).Matrix());
    }
    ...
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(TransformationMatrix::MakeScale(4), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(0, -75), visual_viewport.GetScrollTranslationNode()->Translation2D());
...
TEST_P(VisualViewportTest, TestResizeAfterHorizontalScroll)
{
    ...
    // Verify the paint property nodes and GeometryMapper cache.
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(TransformationMatrix::MakeScale(2), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(-150, 0), visual_viewport.GetScrollTranslationNode()->Translation2D());
        auto expected_projection = TransformationMatrix::MakeScale(2);
...
TEST_P(VisualViewportTest, TestWebViewResizeCausesViewportConstrainedLayout)
{
    ...
    LayoutObject* navbar = GetFrame()->GetDocument()->getElementById("navbar")->GetLayoutObject();
...
public:
    MOCK_METHOD2(UpdateContextMenuDataForTesting, void(const ContextMenuData&, const absl::optional<gfx::Point>&));
...
{
    WebViewImpl* web_view_impl = helper_.InitializeWithSettings(&ConfigureAndroidCompositing);
...
TEST_P(VisualViewportTest, SlowScrollAfterImplScroll)
{
    ...
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(300, 200), gfx::Vector2dF(0, 0), 2, false, 0, 0, cc::BrowserControlsState::kBoth });
    ...
    gsb.data.scroll_begin.delta_hint_units = ui::ScrollGranularity::kScrollByPrecisePixel;
...
{
    WebViewImpl* web_view_impl = helper_.InitializeWithSettings(&ConfigureAndroidNonCompositing);
...
public:
    ...
    void ScrollIntoView(const WebString& element_name, bool is_for_scroll_sequence)
    {
        WebDocument web_doc = WebView().MainFrameImpl()->GetDocument();
        Element* bottom_element = web_doc.GetElementById(element_name);
        auto scroll_params = ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(),
            mojom::blink::ScrollType::kProgrammatic,
            /*make_visible_in_visual_viewport=*/true, mojom::blink::ScrollBehavior::kInstant, is_for_scroll_sequence);
        WebView().GetPage()->GetVisualViewport().ScrollIntoView(bottom_element->BoundingBox(), scroll_params);
    }
};

TEST_F(VisualViewportScrollIntoViewTest, ScrollingToFixedWithScrollSequenceAnimationShort)
{
    VisualViewport& visual_viewport = WebView().GetPage()->GetVisualViewport();
...
TEST_F(VisualViewportScrollIntoViewTest, ScrollingToFixedWithoutScrollSequenceAnimationShort)
{
    VisualViewport& visual_viewport = WebView().GetPage()->GetVisualViewport();
    EXPECT_EQ(0.f, visual_viewport.GetScrollOffset().y());
...
TEST_P(VisualViewportTest, DeviceEmulation)
{
    ...
    EXPECT_EQ(TransformationMatrix::MakeTranslation(-params.viewport_offset.x(), -params.viewport_offset.y()),
    ...
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ASSERT_TRUE(visual_viewport.GetDeviceEmulationTransformNode());
    EXPECT_EQ(TransformationMatrix::MakeScale(1.5f), visual_viewport.GetDeviceEmulationTransformNode()->Matrix());
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
...
TEST_P(VisualViewportTest, PaintScrollbar)
{
    ...
    auto check_scrollbar = [](const cc::Layer* scrollbar, float scale) {
        ...
        EXPECT_EQ(cc::ScrollbarOrientation::VERTICAL, static_cast<const cc::ScrollbarLayerBase*>(scrollbar)->orientation());
...
TEST_F(VisualViewportSimTest, UsedColorSchemeFromRootElement)
{
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, visual_viewport.UsedColorScheme());
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, visual_viewport.UsedColorScheme());
}
...
TEST_F(VisualViewportSimTest, PreferredOverlayScrollbarColorTheme)
{
    ...
    const VisualViewport& visual_viewport = WebView().GetPage()->GetVisualViewport();
    EXPECT_EQ(ScrollbarOverlayColorTheme::kScrollbarOverlayColorThemeLight, visual_viewport.GetScrollbarOverlayColorTheme());

    color_scheme_helper.SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kLight);
    Compositor().BeginFrame();
    EXPECT_EQ(ScrollbarOverlayColorTheme::kScrollbarOverlayColorThemeDark, visual_viewport.GetScrollbarOverlayColorTheme());
}
