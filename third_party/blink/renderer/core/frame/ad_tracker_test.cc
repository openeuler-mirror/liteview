...
protected:
    ...
    {
        ad_tracker_->WillExecuteScript(GetExecutionContext(), v8::Isolate::GetCurrent()->GetCurrentContext(), String(script_url), script_id);
    }
...
TEST_F(AdTrackerSimTest, InlineAdScriptRunningInNonAdContext)
{
    ...
    EXPECT_TRUE(To<LocalFrame>(GetDocument().GetFrame()->Tree().ScopedChild("ad_sibling"))->IsFrameCreatedByAdScript());
}
