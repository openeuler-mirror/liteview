...
BASE_FEATURE(kAsyncStackAdTagging, "AsyncStackAdTagging", base::FEATURE_ENABLED_BY_DEFAULT);
} // namespace features
...
AdTracker::AdTracker(LocalFrame* local_root)
    ...
    , async_stack_enabled_(base::FeatureList::IsEnabled(features::kAsyncStackAdTagging))
{
...
void AdTracker::Will(const probe::CallFunction& probe)
{
    ...
    if (!resource_name.IsEmpty()) {
        v8::MaybeLocal<v8::String> resource_name_string = resource_name->ToString(ToIsolate(local_root_)->GetCurrentContext());
...
bool AdTracker::IsAdScriptInStack(StackType stack_type, absl::optional<AdScriptIdentifier>* out_ad_script)
{
