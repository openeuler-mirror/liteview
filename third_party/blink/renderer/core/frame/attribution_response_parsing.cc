...
bool ParseAttributionAggregationKey(const JSONValue* value, absl::uint128* out)
{
    if (!value)
        return false;
    ...
    if (key_piece.length() < 3 || key_piece.length() > 34 || !key_piece.StartsWith("0x", kTextCaseASCIIInsensitive)) {
        return false;
    }
    ...
    if (key_piece.length() <= 18) {
        ...
        if (!ok)
            return false;
        high_bits = key_piece.Substring(2, key_piece.length() - 18).HexToUInt64Strict(&ok);
        if (!ok)
            return false;
    }
...
mojom::blink::AttributionTriggerDedupKeyPtr ParseDedupKey(const String& string)
{
    bool is_valid = false;
    uint64_t value = string.ToUInt64Strict(&is_valid);
    return is_valid ? mojom::blink::AttributionTriggerDedupKey::New(value) : nullptr;
}

} // namespace

bool ParseAttributionFilterData(const JSONValue* value, mojom::blink::AttributionFilterData& filter_data)
{
    if (!value)
        return true;
    ...
    static_assert(kMaxValuesPerAttributionFilter < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.ValuesPerFilter");

    static_assert(kMaxAttributionFiltersPerSource < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.FiltersPerFilterData");
    ...
    for (wtf_size_t i = 0; i < num_filters; ++i) {
        ...
        if (entry.first.CharactersSizeInBytes() > kMaxBytesPerAttributionFilterString) {
            return false;
        }
        ...
        for (wtf_size_t j = 0; j < num_values; ++j) {
            ...
            if (value_str.CharactersSizeInBytes() > kMaxBytesPerAttributionFilterString) {
                return false;
            }
...
bool ParseAggregationKeys(const JSONValue* json, WTF::HashMap<String, absl::uint128>& aggregation_keys)
{
    ...
    static_assert(
        kMaxAttributionAggregationKeysPerSourceOrTrigger < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.AggregatableKeysPerSource");
    ...
    base::UmaHistogramCounts100("Conversions.AggregatableKeysPerSource", num_keys);
    ...
    for (wtf_size_t i = 0; i < num_keys; ++i) {
        ...
        if (key_id.CharactersSizeInBytes() > kMaxBytesPerAttributionAggregationKeyId) {
            return false;
        }
...
bool ParseSourceRegistrationHeader(const String& json_string, mojom::blink::AttributionSourceData& source_data)
{
    ...
    if (!object->GetString("destination", &destination_string))
        return false;
    scoped_refptr<const SecurityOrigin> destination = SecurityOrigin::CreateFromString(destination_string);
    ...
    if (!ParseAttributionFilterData(object->Get("filter_data"), *source_data.filter_data)) {
        return false;
    }
    ...
    if (!ParseAggregationKeys(object->Get("aggregation_keys"), source_data.aggregation_keys)) {
        return false;
    }
...
bool ParseEventTriggerData(const JSONValue* json, WTF::Vector<mojom::blink::EventTriggerDataPtr>& event_trigger_data)
{
    if (!json)
        return true;
    ...
    for (wtf_size_t i = 0; i < array_value->size(); ++i) {
        ...
        mojom::blink::EventTriggerDataPtr event_trigger = mojom::blink::EventTriggerData::New();
        ...
        if (!ParseAttributionFilterData(object_val->Get("filters"), *event_trigger->filters)) {
            return false;
        }

        event_trigger->not_filters = mojom::blink::AttributionFilterData::New();
        if (!ParseAttributionFilterData(object_val->Get("not_filters"), *event_trigger->not_filters)) {
            return false;
        }
...
bool ParseAttributionAggregatableTriggerData(const JSONValue* json, WTF::Vector<mojom::blink::AttributionAggregatableTriggerDataPtr>& trigger_data)
{
    if (!json)
        return true;
    ...
    static_assert(kMaxAttributionAggregatableTriggerDataPerTrigger < kExclusiveMaxHistogramValue,
    ...
    base::UmaHistogramCounts100("Conversions.AggregatableTriggerDataLength", num_trigger_data);
    ...
    for (wtf_size_t i = 0; i < num_trigger_data; ++i) {
        ...
        if (!ParseAttributionAggregationKey(object->Get("key_piece"), &data->key_piece)) {
            return false;
        }

        JSONArray* source_keys_val = object->GetArray("source_keys");
        if (!source_keys_val || source_keys_val->size() > kMaxAttributionAggregationKeysPerSourceOrTrigger) {
            return false;
        }
        ...
        for (wtf_size_t j = 0; j < num_source_keys; ++j) {
            ...
            String source_key;
            if (!source_key_val->AsString(&source_key) || source_key.CharactersSizeInBytes() > kMaxBytesPerAttributionAggregationKeyId) {
                return false;
            }
        ...
        if (!ParseAttributionFilterData(object->Get("not_filters"), *data->not_filters)) {
            return false;
        }
...
bool ParseAttributionAggregatableValues(const JSONValue* json, WTF::HashMap<String, uint32_t>& values)
{
    if (!json)
        return true;

    const auto* object = JSONObject::Cast(json);
    if (!object || object->size() > kMaxAttributionAggregationKeysPerSourceOrTrigger) {
        return false;
    }
    ...
    for (wtf_size_t i = 0; i < num_values; ++i) {
        ...
        int key_value;
        if (!value->AsInteger(&key_value) || key_value <= 0 || key_value > kMaxAttributionAggregatableValue) {
            return false;
        }
...
bool ParseTriggerRegistrationHeader(const String& json_string, mojom::blink::AttributionTriggerData& trigger_data)
{
    std::unique_ptr<JSONValue> json = ParseJSON(json_string);
    if (!json)
        return false;
    ...
    if (!ParseEventTriggerData(object->Get("event_trigger_data"), trigger_data.event_triggers)) {
        return false;
    }

    trigger_data.filters = mojom::blink::AttributionFilterData::New();

    if (!ParseAttributionFilterData(object->Get("filters"), *trigger_data.filters)) {
        return false;
    }

    trigger_data.not_filters = mojom::blink::AttributionFilterData::New();

    if (!ParseAttributionFilterData(object->Get("not_filters"), *trigger_data.not_filters)) {
        return false;
    }

    if (!ParseAttributionAggregatableTriggerData(object->Get("aggregatable_trigger_data"), trigger_data.aggregatable_trigger_data)) {
        return false;
    }

    if (!ParseAttributionAggregatableValues(object->Get("aggregatable_values"), trigger_data.aggregatable_values)) {
        return false;
    }
