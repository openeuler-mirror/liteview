...
constexpr base::TimeDelta kDefaultPendingBeaconMaxBackgroundTimeout = base::Minutes(30); // 30 minutes

// Returns a max possible background timeout for every pending beacon.
base::TimeDelta GetMaxBackgroundTimeout()
{
    return base::Milliseconds(GetFieldTrialParamByFeatureAsInt(features::kPendingBeaconAPI, "PendingBeaconMaxBackgroundTimeoutInMs",
        base::checked_cast<int32_t>(kDefaultPendingBeaconMaxBackgroundTimeout.InMilliseconds())));
}

} // namespace

PendingBeacon::PendingBeacon(ExecutionContext* ec, const String& url, const String& method, int32_t background_timeout, int32_t timeout)
    ...
    , timeout_timer_(GetTaskRunner(), this, &PendingBeacon::TimeoutTimerFired)
{
    ...
    mojo::PendingReceiver<mojom::blink::PendingBeacon> beacon_receiver = remote_.BindNewPipeAndPassReceiver(GetTaskRunner());
    KURL host_url = ec_->CompleteURL(url);

    PendingBeaconDispatcher& dispatcher = PendingBeaconDispatcher::FromOrAttachTo(*ec_);
    dispatcher.CreateHostBeacon(this, std::move(beacon_receiver), host_url, host_method);
...
void PendingBeacon::SetDataInternal(const BeaconData& data, ExceptionState& exception_state)
{
    ...
    scoped_refptr<network::ResourceRequestBody> request_body = GetRequestBodyForWebURLRequest(WrappedResourceRequest(request));
    ...
    if (request_body->elements()->size() > 1) {
        exception_state.ThrowRangeError("PendingBeacon only supports single part data.");
        return;
    }

    AtomicString content_type = request.HttpContentType();
    remote_->SetRequestData(std::move(request_body), content_type.IsNull() ? "" : content_type);
}

base::TimeDelta PendingBeacon::GetBackgroundTimeout() const
{
    const auto max_background_timeout = GetMaxBackgroundTimeout();
    return (background_timeout_.is_negative() || background_timeout_ > max_background_timeout) ? max_background_timeout : background_timeout_;
}
...
{
    return GetExecutionContext()->GetTaskRunner(PendingBeaconDispatcher::kTaskType);
}
