...
void ReportingContext::SendToReportingAPI(Report* report, const String& endpoint) const
{
    ...
    if (type == ReportType::kCSPViolation) {
        ...
        GetReportingService()->QueueCspViolationReport(url, endpoint, body->documentURL() ? body->documentURL() : "", body->referrer(), body->blockedURL(),
            ...
            body->sample(), body->disposition() ? body->disposition() : "", body->statusCode(), line_number, column_number);
        ...
        GetReportingService()->QueuePermissionsPolicyViolationReport(
            url, body->featureId(), body->disposition(), body->message(), body->sourceFile(), line_number, column_number);
    } else if (type == ReportType::kIntervention) {
