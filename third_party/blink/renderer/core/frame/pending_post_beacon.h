...
public:
    static PendingPostBeacon* Create(ExecutionContext* context, const String& target_url);

    static PendingPostBeacon* Create(ExecutionContext* context, const String& target_url, PendingBeaconOptions* options);

    explicit PendingPostBeacon(ExecutionContext* context, const String& url, int32_t background_timeout, int32_t timeout, base::PassKey<PendingPostBeacon> key);

    void setData(const V8UnionReadableStreamOrXMLHttpRequestBodyInit* data, ExceptionState& exception_state);
};
