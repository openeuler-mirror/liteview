...
void RemoteFrameOwner::AddResourceTiming(const ResourceTimingInfo& info)
{
    ...
    mojom::blink::ResourceTimingInfoPtr resource_timing
        = Performance::GenerateResourceTiming(*frame->Tree().Parent()->GetSecurityContext()->GetSecurityOrigin(), info, *frame->DomWindow());
    frame->GetLocalFrameHostRemote().ForwardResourceTimingToParent(std::move(resource_timing));
}
