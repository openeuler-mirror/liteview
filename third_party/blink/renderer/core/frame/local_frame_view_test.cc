...
TEST_F(LocalFrameViewTest, SetPaintInvalidationDuringUpdateAllLifecyclePhases)
{
    ...
    GetDocument().getElementById("a")->setAttribute(html_names::kStyleAttr, "color: green");
...
TEST_F(LocalFrameViewTest, SetPaintInvalidationOutOfUpdateAllLifecyclePhases)
{
    SetBodyInnerHTML("<div id='a' style='color: blue'>A</div>");
    GetAnimationMockChromeClient().has_scheduled_animation_ = false;
    GetDocument().getElementById("a")->GetLayoutObject()->SetShouldDoFullPaintInvalidation();
    EXPECT_TRUE(GetAnimationMockChromeClient().has_scheduled_animation_);
    GetAnimationMockChromeClient().has_scheduled_animation_ = false;
    UpdateAllLifecyclePhasesForTest();
    GetDocument().getElementById("a")->GetLayoutObject()->SetShouldDoFullPaintInvalidation();
...
TEST_F(LocalFrameViewTest, CanHaveScrollbarsIfScrollingAttrEqualsNoChanged)
{
    ...
    ChildDocument().WillChangeFrameOwnerProperties(0, 0, mojom::blink::ScrollbarMode::kAlwaysOn, false, mojom::blink::ColorScheme::kLight);
...
TEST_F(LocalFrameViewSimTest, FragmentNavChangesFocusWhileRenderingBlocked)
{
    ...
    //  navigation.
    auto* anchor = To<HTMLAnchorElement>(GetDocument().getElementById("anchorlink"));
    anchor->click();
    ...
    EXPECT_EQ(GetDocument().getElementById("bottom"), GetDocument().ActiveElement()) << "Active element wasn't changed after load completed.";
...
TEST_F(LocalFrameViewSimTest, NestedCrossOriginPaintEligibility)
{
    ...
    auto* outer_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("outer"));
    ...
    auto* inner_frame_element = To<HTMLIFrameElement>(outer_frame_document->getElementById("inner"));
...
TEST_F(LocalFrameViewTest, StartOfLifecycleTaskRunsOnFullLifecycle)
{
    ...
    frame_view->EnqueueStartOfLifecycleTask(base::BindOnce(&TestCallback::Increment, base::Unretained(&callback)));
