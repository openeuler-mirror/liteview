...
void FrameView::UpdateViewportIntersection(unsigned flags, bool needs_occlusion_tracking)
{
    ...
    bool should_compute_occlusion = needs_occlusion_tracking && occlusion_state == mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded
    ...
    if (!owner_layout_object || owner_layout_object->ContentSize().IsEmpty() || (flags & IntersectionObservation::kAncestorFrameIsDetachedFromLayout)
        ...
        unsigned geometry_flags = IntersectionGeometry::kShouldUseReplacedContentRect;
        if (should_compute_occlusion)
            geometry_flags |= IntersectionGeometry::kShouldComputeVisibility;

        IntersectionGeometry geometry(
            nullptr, *owner_element, {} /* root_margin */, { IntersectionObserver::kMinimumThreshold }, {} /* target_margin */, geometry_flags);
        PhysicalRect new_rect_in_parent = geometry.IntersectionRect();
        if (new_rect_in_parent.size != rect_in_parent_.size
            || ((new_rect_in_parent.X() - rect_in_parent_.X()).Abs() + (new_rect_in_parent.Y() - rect_in_parent_.Y()).Abs()
        ...
        TransformationMatrix matrix = parent_frame_to_iframe_content_transform.AccumulatedTransform().Inverse();
        ...
        if (!geometry.UnclippedIntersectionRect().IsEmpty()) {
            mainframe_intersection_rect
                = PhysicalRect::EnclosingRect(matrix.ProjectQuad(gfx::QuadF(gfx::RectF(geometry.UnclippedIntersectionRect()))).BoundingBox());
    ...
    } else if (occlusion_state == mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded) {
    ...
    gfx::Transform main_frame_gfx_transform = main_frame_transform_matrix.ToTransform();
    ...
    SetViewportIntersection(mojom::blink::ViewportIntersectionState(viewport_intersection, mainframe_intersection, gfx::Rect(), occlusion_state,
        frame.GetOutermostMainFrameSize(), frame.GetOutermostMainFrameScrollPosition(), main_frame_gfx_transform));
    ...
    bool has_flag = RuntimeEnabledFeatures::ThrottleDisplayNoneAndVisibilityHiddenCrossOriginIframesEnabled();

    bool should_throttle
        = has_flag ? (is_display_none || (zero_viewport_intersection && !has_zero_area)) : (!is_display_none && zero_viewport_intersection && !has_zero_area);
...
void FrameView::UpdateFrameVisibility(bool intersects_viewport)
{
    ...
    if (LifecycleUpdatesThrottled())
        return;
    if (IsVisible()) {
        frame_visibility = intersects_viewport ? blink::mojom::FrameVisibility::kRenderedInViewport : blink::mojom::FrameVisibility::kRenderedOutOfViewport;
    } else {
...
{
    if (event_timestamp - rect_in_parent_stable_since_ < base::Milliseconds(mojom::blink::kMinScreenRectStableTimeMs)) {
        return false;
    }
