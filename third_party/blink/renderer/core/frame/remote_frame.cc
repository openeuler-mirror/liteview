...
RemoteFrame::RemoteFrame(RemoteFrameClient* client, Page& page, FrameOwner* owner, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type,
    ...
    : Frame(client, page, owner, parent, previous_sibling, insert_type, frame_token, devtools_frame_token,
        MakeGarbageCollected<RemoteWindowProxyManager>(*this), inheriting_agent_factory)
    ,
...
void RemoteFrame::Navigate(FrameLoadRequest& frame_request, WebFrameLoadType frame_load_type)
{
    ...
    absl::optional<LocalFrameToken> initiator_frame_token = base::OptionalFromPtr(frame_request.GetInitiatorFrameToken());
    ...
    if (window->GetFrame()) {
        ...
        if (frame_request.ClientRedirectReason() != ClientNavigationReason::kNone) {
            probe::FrameRequestedNavigation(window->GetFrame(), this, url, frame_request.ClientRedirectReason(), kNavigationPolicyCurrentTab);
        }

        if (!initiator_frame_token) {
            initiator_frame_token = window->GetFrame()->GetLocalFrameToken();
            initiator_policy_container_keep_alive_handle = window->GetPolicyContainer()->IssueKeepAliveHandle();
        }
    }
    ...
    params->initiator_policy_container_keep_alive_handle = std::move(initiator_policy_container_keep_alive_handle);
...
bool RemoteFrame::NavigationShouldReplaceCurrentHistoryEntry(WebFrameLoadType frame_load_type) const
{
    ...
    return (frame_load_type == WebFrameLoadType::kStandard && GetPage()->InsidePortal()) || IsInFencedFrameTree();
}
...
void RemoteFrame::RenderFallbackContentWithResourceTiming(mojom::blink::ResourceTimingInfoPtr timing, const String& server_timing_value)
{
    Frame::RenderFallbackContentWithResourceTiming(std::move(timing), server_timing_value);
}
...
void RemoteFrame::AddResourceTimingFromChild(mojom::blink::ResourceTimingInfoPtr timing)
{
    ...
    DOMWindowPerformance::performance(*owner_element->GetDocument().domWindow())
        ->AddResourceTiming(std::move(timing), owner_element->localName(), owner_element->GetDocument().GetExecutionContext());
}
    ...
    scoped_refptr<const SecurityOrigin> source_security_origin, scoped_refptr<const SecurityOrigin> target_security_origin)
{
    ...
    String source_origin = source_security_origin ? source_security_origin->ToString() : g_empty_string;
    String target_origin = target_security_origin ? target_security_origin->ToString() : g_empty_string;

    GetRemoteFrameHostRemote().RouteMessageEvent(source_token, source_origin, target_origin, std::move(transferable_message));
}
...
void RemoteFrame::UpdateOpener(const absl::optional<blink::FrameToken>& opener_frame_token)
{
...
bool RemoteFrame::IsIgnoredForHitTest() const
{
    ...
    return owner->OwnerType() == FrameOwnerElementType::kPortal || !visible_to_hit_testing_;
}
...
bool RemoteFrame::SynchronizeVisualProperties(bool propagate)
{
    ...
    bool synchronized_props_changed = !sent_visual_properties_ || sent_visual_properties_->auto_resize_enabled != pending_visual_properties_.auto_resize_enabled
        ...
        || sent_visual_properties_->root_widget_window_segments != pending_visual_properties_.root_widget_window_segments
...
void RemoteFrame::DidChangeRootWindowSegments(const std::vector<gfx::Rect>& root_widget_window_segments)
{
    pending_visual_properties_.root_widget_window_segments = std::move(root_widget_window_segments);
    SynchronizeVisualProperties();
}
...
void RemoteFrame::CreateRemoteChild(const RemoteFrameToken& token, const absl::optional<FrameToken>& opener_frame_token,
    mojom::blink::TreeScopeType tree_scope_type, mojom::blink::FrameReplicationStatePtr replication_state, const base::UnguessableToken& devtools_frame_token,
    mojom::blink::RemoteFrameInterfacesFromBrowserPtr remote_frame_interfaces)
{
    Client()->CreateRemoteChild(
        token, opener_frame_token, tree_scope_type, std::move(replication_state), devtools_frame_token, std::move(remote_frame_interfaces));
}
