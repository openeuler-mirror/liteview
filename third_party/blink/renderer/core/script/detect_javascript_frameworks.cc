...
inline void CheckIdMatches(Document& document, int& loading_behavior_flag, bool& has_nextjs_id)
{
...
inline void CheckAttributeMatches(const Element& element, int& loading_behavior_flag)
{
    DEFINE_STATIC_LOCAL(QualifiedName, ng_version, (g_null_atom, "ng-version", g_null_atom));
    DEFINE_STATIC_LOCAL(QualifiedName, data_reactroot, (g_null_atom, "data-reactroot", g_null_atom));
...
inline void CheckPropertyMatches(
    Element& element, DOMDataStore& dom_data_store, v8::Local<v8::Context> context, v8::Isolate* isolate, int& loading_behavior_flag)
{
...
inline void CheckGlobalPropertyMatches(v8::Local<v8::Context> context, v8::Isolate* isolate, int& loading_behavior_flag, bool& has_nextjs_id)
{
...
void DidObserveLoadingBehaviors(Document& document, int loading_behavior_flag)
{
    ...
    static constexpr LoadingBehaviorFlag flags[]
        = { kLoadingBehaviorAngularFrameworkUsed, kLoadingBehaviorGatsbyFrameworkUsed, kLoadingBehaviorNextJSFrameworkUsed, kLoadingBehaviorNextJSFrameworkUsed,
              kLoadingBehaviorNuxtJSFrameworkUsed, kLoadingBehaviorPreactFrameworkUsed, kLoadingBehaviorReactFrameworkUsed, kLoadingBehaviorSapperFrameworkUsed,
...
void TraverseTreeForFrameworks(Document& document, v8::Local<v8::Context> context)
{
    v8::Isolate* isolate = context->GetIsolate();
    v8::TryCatch try_catch(isolate);
    ...
    for (Element& element : ElementTraversal::InclusiveDescendantsOf(*document.documentElement())) {
        ...
        CheckPropertyMatches(element, dom_data_store, context, isolate, loading_behavior_flag);
    }
    CheckIdMatches(document, loading_behavior_flag, has_nextjs_id);
    CheckGlobalPropertyMatches(context, isolate, loading_behavior_flag, has_nextjs_id);
...
void DetectJavascriptFrameworksOnLoad(Document& document)
{
    ...
    if (!document.GetFrame() || !document.GetFrame()->IsMainFrame() || document.GetFrame()->IsInFencedFrameTree() || !document.Url().ProtocolIsInHTTPFamily()
