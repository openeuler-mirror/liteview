...
{
    tree_linker_registry_->Fetch(url, module_type, fetch_client_settings_object_fetcher, context_type, destination, options, this, custom_fetch_type, client);
}
...
KURL ModulatorImplBase::ResolveModuleSpecifier(const String& specifier, const KURL& base_url, String* failure_reason)
{
    ...
    if (GetImportMap()) {
        ...
        mapped_url = GetImportMap()->Resolve(parsed_specifier, base_url, &import_map_debug_message);
...
void ModulatorImplBase::ResolveDynamically(const ModuleRequest& module_request, const ReferrerScriptInfo& referrer_info, ScriptPromiseResolver* resolver)
{
    String reason;
