...
{
    base::test::ScopedFeatureList feature_list(features::kForceDeferScriptIntervention);
    ...
    NotifyScriptReady(pending_script1);
    EXPECT_CALL(*pending_script1, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(1); }));
    auto* delayer = MakeGarbageCollected<ScriptRunnerDelayer>(script_runner_, ScriptRunner::DelayReason::kForceDefer);
    delayer->Activate();
