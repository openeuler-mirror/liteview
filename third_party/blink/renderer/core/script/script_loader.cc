...
{
    return base::FeatureList::IsEnabled(features::kForceDeferScriptIntervention) && IsEligibleCommon(element_document);
}
...
bool IsEligibleForDelay(const Resource& resource, const Document& element_document, const ScriptElementBase& element)
{
    ...
    static const bool main_frame_only = features::kDelayAsyncScriptExecutionMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document.IsInOutermostMainFrame())
        return false;

    static const base::TimeDelta feature_limit = features::kDelayAsyncScriptExecutionFeatureLimitParam.Get();
    if (!feature_limit.is_zero() && element_document.GetStartTime().Elapsed() > feature_limit) {
        return false;
    }

    static const features::DelayAsyncScriptTarget delay_async_script_target = features::kDelayAsyncScriptTargetParam.Get();
    ...
    case features::DelayAsyncScriptTarget::kCrossSiteWithAllowListReportOnly:
        if (IsSameSite(resource.Url(), element_document))
            return false;
        DEFINE_STATIC_LOCAL(UrlMatcher, url_matcher, (UrlMatcher(features::kDelayAsyncScriptAllowList.Get())));
...
bool IsEligibleForLowPriorityScriptLoading(const Document& element_document, const ScriptElementBase& element, const KURL& url)
{
    ...
    // Most LCP elements are provided by the main frame, and delaying subframe's
    // resources seems not to improve LCP.
    static const bool main_frame_only = features::kLowPriorityScriptLoadingMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document.IsInOutermostMainFrame())
        return false;

    static const base::TimeDelta feature_limit = features::kLowPriorityScriptLoadingFeatureLimitParam.Get();
    if (!feature_limit.is_zero() && element_document.GetStartTime().Elapsed() > feature_limit) {
        return false;
    }

    static const bool cross_site_only = features::kLowPriorityScriptLoadingCrossSiteOnlyParam.Get();
...
bool ShouldBlockSyncScriptForDocumentPolicy(const ScriptElementBase* element, ScriptLoader::ScriptTypeAtPrepare script_type, bool parser_inserted)
{
    if (element->GetExecutionContext()->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kSyncScript)) {
        return false;
    }

    // Non-classic scripts don't block parsing.
    if (script_type == ScriptLoader::ScriptTypeAtPrepare::kModule || script_type == ScriptLoader::ScriptTypeAtPrepare::kImportMap
        || script_type == ScriptLoader::ScriptTypeAtPrepare::kSpeculationRules || script_type == ScriptLoader::ScriptTypeAtPrepare::kWebBundle
        || !parser_inserted)
        return false;
...
PendingScript* ScriptLoader::PrepareScript(ParserBlockingInlineOption parser_blocking_inline_option, const TextPosition& script_start_position)
{
    ...
    //--
//     std::string src;
//     src = "ScriptLoader::PrepareScript:";
//     src += element_->SourceAttributeValue().Utf8();
//     src += "\n";
//     OutputDebugStringA(src.c_str());
    //+++
    ...
    if (ShouldBlockSyncScriptForDocumentPolicy(element_.Get(), GetScriptType(), parser_inserted_)) {
        context_window->ReportDocumentPolicyViolation(mojom::blink::DocumentPolicyFeature::kSyncScript, mojom::blink::PolicyDisposition::kEnforce,
    ...
    if (GetScriptType() == ScriptTypeAtPrepare::kImportMap) {
        Modulator* modulator = Modulator::From(ToScriptStateForMainWorld(context_window->GetFrame()));
        ...
        case Modulator::AcquiringImportMapsState::kMultipleImportMaps:
            ...
            element_document.AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kError,
                    aquiring_state == Modulator::AcquiringImportMapsState::kAfterModuleScriptLoad ? "An import map is added after module script load was "
                                                                                                    "triggered."
    ...
    if (element_->HasSourceAttribute()) {
        ...
        case ScriptTypeAtPrepare::kImportMap:
            ...
            element_document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kError, "External import maps are not yet supported."));
            element_document.GetTaskRunner(TaskType::kDOMManipulation)
                ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptElementBase::DispatchErrorEvent, WrapPersistent(element_.Get())));
            return nullptr;
        ...
        case ScriptTypeAtPrepare::kClassic: {
            ...
            ClassicPendingScript* pending_script = ClassicPendingScript::Fetch(url, element_document, options, cross_origin, encoding, element_, defer);
    ...
    if (!element_->HasSourceAttribute()) {
        ...
        case ScriptTypeAtPrepare::kWebBundle: {
            ...
            if (absl::holds_alternative<ScriptWebBundleError>(script_web_bundle_or_error)) {
                ...
                if (error.GetType() == ScriptWebBundleError::Type::kSystemError) {
                    ...
                    ScriptState* script_state = ToScriptStateForMainWorld(To<LocalDOMWindow>(element_->GetExecutionContext())->GetFrame());
        ...
        case ScriptTypeAtPrepare::kSpeculationRules: {
            ...
            String parse_error;
            if (auto* rule_set = SpeculationRuleSet::Parse(source_text, base_url, &parse_error)) {
            ...
            if (!parse_error.IsNull()) {
                auto* console_message = MakeGarbageCollected<ConsoleMessage>(
                    mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "While parsing speculation rules: " + parse_error);
                console_message->SetNodes(element_document.GetFrame(), { element_->GetDOMNodeId() });
        ...
        case ScriptTypeAtPrepare::kClassic: {
            ...
            prepared_pending_script_ = ClassicPendingScript::CreateInline(element_, position, source_url, base_url, source_text, script_location_type, options);
        ...
        case ScriptTypeAtPrepare::kModule: {
            ...
            ModuleScriptCreationParams params(
                source_url, base_url, ScriptSourceLocationType::kInline, ModuleType::kJavaScript, ParkableString(source_text.Impl()), nullptr);
            ...
            prepared_pending_script_ = MakeGarbageCollected<ModulePendingScript>(element_, module_tree_client, is_external_script_);
            break;
        }
        }
    }
    ...
    case ScriptSchedulingType::kForceInOrder:
...

        {
            ...
            if (is_eligible_for_delay && script_scheduling_type == ScriptSchedulingType::kAsync) {
                ...
                if (delay_async_script_target == features::DelayAsyncScriptTarget::kCrossSiteWithAllowList
                    || delay_async_script_target == features::DelayAsyncScriptTarget::kCrossSiteWithAllowListReportOnly) {
                    UseCounter::Count(element_document.TopDocument(), WebFeature::kAutomaticLazyEmbeds);
                }
