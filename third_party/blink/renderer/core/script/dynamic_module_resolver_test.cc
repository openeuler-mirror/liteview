...
private:
    ...
    void FetchTree(const KURL& url, ModuleType module_type, ResourceFetcher*, mojom::blink::RequestContextType, network::mojom::RequestDestination,
        const ScriptFetchOptions&, ModuleScriptCustomFetchType custom_fetch_type, ModuleTreeClient* client) final
    {
...
public:
    ...
    ScriptValue Call(ScriptState* script_state, ScriptValue value) override
    {
        ...
        captured_value_ = ToCoreString(exported_value->ToString(context).ToLocalChecked());
...
class DynamicModuleResolverTestNotReached final : public ScriptFunction::Callable {
public:
...
TEST_F(DynamicModuleResolverTest, ResolveSuccess)
{
    ...
    auto* promise_resolver = MakeGarbageCollected<ScriptPromiseResolver>(scope.GetScriptState());
    ScriptPromise promise = promise_resolver->Promise();

    auto* capture = MakeGarbageCollected<CaptureExportedStringFunction>("foo");
    promise.Then(MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), capture),
        MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<DynamicModuleResolverTestNotReached>()));

    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), Vector<ImportAssertion>());
...
TEST_F(DynamicModuleResolverTest, ResolveJSONModuleSuccess)
{
    ...
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    Vector<ImportAssertion> import_assertions { ImportAssertion("type", "json", TextPosition::MinimumPosition()) };
    ModuleRequest module_request("./dependency.json", TextPosition::MinimumPosition(), import_assertions);
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
...
TEST_F(DynamicModuleResolverTest, ResolveSpecifierFailure)
{
    ...
    promise.Then(MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), MakeGarbageCollected<DynamicModuleResolverTestNotReached>()),
        MakeGarbageCollected<ScriptFunction>(scope.GetScriptState(), capture));

    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    ModuleRequest module_request("invalid-specifier", TextPosition::MinimumPosition(), Vector<ImportAssertion>());
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
...
TEST_F(DynamicModuleResolverTest, ResolveModuleTypeFailure)
{
    ...
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    Vector<ImportAssertion> import_assertions { ImportAssertion("type", "notARealType", TextPosition::MinimumPosition()) };
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), import_assertions);
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
