...
public:
    DynamicImportTreeClient(const KURL& url, Modulator* modulator, ScriptPromiseResolver* promise_resolver)
        : url_(url)
...
public:
    ModuleResolutionSuccessCallback(ScriptPromiseResolver* promise_resolver, ModuleScript* module_script)
...
public:
    explicit ModuleResolutionFailureCallback(ScriptPromiseResolver* promise_resolver)
        : ModuleResolutionCallback(promise_resolver)
    {
    }
...
void DynamicImportTreeClient::NotifyModuleTreeLoadFinished(ModuleScript* module_script)
{
    ...
    case ScriptEvaluationResult::ResultType::kSuccess: {
        ...
        auto* callback_success
            = MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ModuleResolutionSuccessCallback>(promise_resolver_, module_script));
        auto* callback_failure = MakeGarbageCollected<ScriptFunction>(script_state, MakeGarbageCollected<ModuleResolutionFailureCallback>(promise_resolver_));
...
void DynamicModuleResolver::ResolveDynamically(
    const ModuleRequest& module_request, const ReferrerScriptInfo& referrer_info, ScriptPromiseResolver* promise_resolver)
{
    ...
    KURL url = modulator_->ResolveModuleSpecifier(module_request.specifier, base_url);
    ...
    // Priority Hints.
    ScriptFetchOptions options(referrer_info.Nonce(), IntegrityMetadataSet(), String(), referrer_info.ParserState(), referrer_info.CredentialsMode(),
        referrer_info.GetReferrerPolicy(), mojom::blink::FetchPriorityHint::kAuto, RenderBlockingBehavior::kNonBlocking);
    ...
    modulator_->FetchTree(url, module_type, execution_context->Fetcher(), mojom::blink::RequestContextType::SCRIPT, network::mojom::RequestDestination::kScript,
        options, ModuleScriptCustomFetchType::kNone, tree_client);
