    ...
    CrossOriginAttributeValue cross_origin, const WTF::TextEncoding& encoding, ScriptElementBase* element, FetchParameters::DeferOption defer)
{
    ...
    ClassicPendingScript* pending_script = MakeGarbageCollected<ClassicPendingScript>(
        element, TextPosition::MinimumPosition(), KURL(), KURL(), String(), ScriptSourceLocationType::kExternalFile, options, true /* is_external */);
    ...
    ScriptResource::Fetch(params, element_document.Fetcher(), pending_script, ScriptResource::kAllowStreaming);
...
{
    ClassicPendingScript* pending_script = MakeGarbageCollected<ClassicPendingScript>(
        element, starting_position, source_url, base_url, source_text, source_location_type, options, false /* is_external */);
    pending_script->CheckState();
    return pending_script;
}
...
ClassicPendingScript::ClassicPendingScript(ScriptElementBase* element, const TextPosition& starting_position, const KURL& source_url_for_inline_script,
    ...
    const ScriptFetchOptions& options, bool is_external)
...
void ClassicPendingScript::RecordThirdPartyRequestWithCookieIfNeeded(const ResourceResponse& response) const
{
    ...
    if (!top_frame_origin || top_frame_origin->RegistrableDomain() != doc_origin->RegistrableDomain()) {
        return;
    }
...
bool ClassicPendingScript::IsEligibleForLowPriorityAsyncScriptExecution() const
{
    ...
    static const bool main_frame_only = features::kLowPriorityAsyncScriptExecutionMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document->IsInOutermostMainFrame())
        return false;

    static const base::TimeDelta feature_limit = features::kLowPriorityAsyncScriptExecutionFeatureLimitParam.Get();
    ...
    static const bool cross_site_only = features::kLowPriorityAsyncScriptExecutionCrossSiteOnlyParam.Get();
...
void ClassicPendingScript::NotifyFinished(Resource* resource)
{
    ...
    if (!options_.GetIntegrityMetadata().empty() || resource->IsLinkPreload()) {
        integrity_failure = resource->IntegrityDisposition() != ResourceIntegrityDisposition::kPassed;
    }
