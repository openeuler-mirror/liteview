    ...
    const ModuleScriptCreationParams& original_params, Modulator* modulator, const ScriptFetchOptions& options, const TextPosition& start_position)
{
    ...
    ExceptionState exception_state(isolate, ExceptionState::kExecutionContext, "JSModuleScript", "Create");

    ModuleRecordProduceCacheData* produce_cache_data = nullptr;

    v8::Local<v8::Module> result
        = ModuleRecord::Compile(script_state, params, options, start_position, exception_state, modulator->GetV8CacheOptions(), &produce_cache_data);
    ...
    for (const auto& requested : ModuleRecord::ModuleRequests(script_state, result)) {
        ...
        bool module_specifier_is_valid = script->ResolveModuleSpecifier(requested.specifier, &failure_reason).IsValid();
        ...
        if (!module_specifier_is_valid || module_type == ModuleType::kInvalid) {
            ...
            if (!module_specifier_is_valid) {
                error_message = "Failed to resolve module specifier \"" + requested.specifier + "\". " + failure_reason;
            } else {
                error_message = "\"" + requested.GetModuleTypeString() + "\" is not a valid module type.";
            }
