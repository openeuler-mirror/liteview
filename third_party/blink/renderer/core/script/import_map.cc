...
String NormalizeSpecifierKey(const String& key_string, const KURL& base_url, ConsoleLogger& logger)
{
...
// the console |logger|.
ImportMap* ImportMap::Parse(const String& input, const KURL& base_url, ConsoleLogger& logger, absl::optional<ImportMapError>* error_to_rethrow)
{
    ...
    if (parsed_map->Get("imports")) {
        ...
        // baseURL.</spec>
        sorted_and_normalized_imports = SortAndNormalizeSpecifierMap(imports, base_url, logger);
    }
    ...
    if (parsed_map->Get("scopes")) {
        ...
        for (wtf_size_t i = 0; i < scopes->size(); ++i) {
            ...
            // baseURL.</spec>
            sorted_and_normalized_scopes.push_back(std::make_pair(prefix_url.GetString(), SortAndNormalizeSpecifierMap(specifier_map, base_url, logger)));
        }
        ...
        std::sort(sorted_and_normalized_scopes.begin(), sorted_and_normalized_scopes.end(),
            [](const ScopeEntryType& a, const ScopeEntryType& b) { return CodeUnitCompareLessThan(b.first, a.first); });
    }
    ...
    return MakeGarbageCollected<ImportMap>(std::move(sorted_and_normalized_imports), std::move(sorted_and_normalized_scopes));
}
...
ImportMap::SpecifierMap ImportMap::SortAndNormalizeSpecifierMap(const JSONObject* imports, const KURL& base_url, ConsoleLogger& logger)
{
    ...
    for (wtf_size_t i = 0; i < imports->size(); ++i) {
        ...
        const String normalized_specifier_key = NormalizeSpecifierKey(entry.first, base_url, logger);
...
absl::optional<ImportMap::MatchResult> ImportMap::MatchPrefix(const ParsedSpecifier& parsed_specifier, const SpecifierMap& specifier_map) const
{
...
absl::optional<KURL> ImportMap::Resolve(const ParsedSpecifier& parsed_specifier, const KURL& base_url, String* debug_message) const
{
    ...
    for (const auto& entry : scopes_) {
        ...
        if (entry.first == base_url.GetString() || (entry.first.EndsWith("/") && base_url.GetString().StartsWith(entry.first))) {
            ...
            absl::optional<KURL> scope_match = ResolveImportsMatch(parsed_specifier, entry.second, debug_message);
...
// <specdef href="https://wicg.github.io/import-maps/#resolve-an-imports-match">
absl::optional<KURL> ImportMap::ResolveImportsMatch(const ParsedSpecifier& parsed_specifier, const SpecifierMap& specifier_map, String* debug_message) const
{
    DCHECK(debug_message);
    const String key = parsed_specifier.GetImportMapKeyString();
...
static void SpecifierMapToString(StringBuilder& builder, const ImportMap::SpecifierMap& specifier_map)
{
    builder.Append("{");
