...
void ModuleRecordResolverImpl::RegisterModuleScript(const ModuleScript* module_script)
{
    ...
    DVLOG(1) << "ModuleRecordResolverImpl::RegisterModuleScript(url=" << module_script->BaseUrl().GetString() << ", hash=" << BoxedV8ModuleHash::GetHash(record)
             << ")";
...
void ModuleRecordResolverImpl::UnregisterModuleScript(const ModuleScript* module_script)
{
    ...
    v8::Isolate* isolate = modulator_->GetScriptState()->GetIsolate();
    BoxedV8Module* record = MakeGarbageCollected<BoxedV8Module>(isolate, module);
    DVLOG(1) << "ModuleRecordResolverImpl::UnregisterModuleScript(url=" << module_script->BaseUrl().GetString()
...
v8::Local<v8::Module> ModuleRecordResolverImpl::Resolve(const ModuleRequest& module_request, v8::Local<v8::Module> referrer, ExceptionState& exception_state)
{
    ...
    DVLOG(1) << "ModuleRecordResolverImpl::resolve(specifier=\"" << module_request.specifier
             << ", referrer.hash=" << BoxedV8ModuleHash::GetHash(MakeGarbageCollected<BoxedV8Module>(isolate, referrer)) << ")";
