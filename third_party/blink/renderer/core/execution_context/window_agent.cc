...
WindowAgent::WindowAgent(scheduler::WebAgentGroupScheduler& agent_group_scheduler)
    : blink::Agent(agent_group_scheduler.Isolate(), base::UnguessableToken::Create(), CreateMicroTaskQueue(agent_group_scheduler.Isolate()))
    , agent_group_scheduler_(&agent_group_scheduler)
{
    agent_group_scheduler_->AsAgentGroupScheduler().AddAgent(this);
}

WindowAgent::WindowAgent(scheduler::WebAgentGroupScheduler& agent_group_scheduler, bool is_origin_agent_cluster, bool origin_agent_cluster_left_as_default)
    : blink::Agent(agent_group_scheduler.Isolate(), base::UnguessableToken::Create(), CreateMicroTaskQueue(agent_group_scheduler.Isolate()),
