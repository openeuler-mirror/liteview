...
WindowAgentFactory::WindowAgentFactory(scheduler::WebAgentGroupScheduler& agent_group_scheduler)
    ...
    bool has_potential_universal_access_privilege, const SecurityOrigin* origin, bool is_origin_agent_cluster, bool origin_agent_cluster_left_as_default)
{
    ...
    if (inserted.is_new_entry) {
        inserted.stored_value->value = MakeGarbageCollected<WindowAgent>(agent_group_scheduler_, is_origin_agent_cluster, origin_agent_cluster_left_as_default);
    }
    return inserted.stored_value->value;
}
...
// static
unsigned WindowAgentFactory::SchemeAndRegistrableDomainHash::GetHash(const SchemeAndRegistrableDomain& value)
{
    return WTF::HashInts(StringHash::GetHash(value.scheme), StringHash::GetHash(value.registrable_domain));
}

// static
bool WindowAgentFactory::SchemeAndRegistrableDomainHash::Equal(const SchemeAndRegistrableDomain& x, const SchemeAndRegistrableDomain& y)
{
    return x.scheme == y.scheme && x.registrable_domain == y.registrable_domain;
}

// static
bool WindowAgentFactory::SchemeAndRegistrableDomainTraits::IsEmptyValue(const SchemeAndRegistrableDomain& value)
{
    return HashTraits<String>::IsEmptyValue(value.scheme);
}

// static
bool WindowAgentFactory::SchemeAndRegistrableDomainTraits::IsDeletedValue(const SchemeAndRegistrableDomain& value)
{
    return HashTraits<String>::IsDeletedValue(value.scheme);
}

// static
void WindowAgentFactory::SchemeAndRegistrableDomainTraits::ConstructDeletedValue(SchemeAndRegistrableDomain& slot, bool zero_value)
{
