...
public:
    MetaParams() = default;
    MetaParams(FullscreenRequestType request_type, const FullscreenOptions* options)
...
using ElementMetaParamsMap = HeapHashMap<WeakMember<Element>, Member<const MetaParams>>;
...
void SetFullscreenFlag(Element& element, FullscreenRequestType request_type, const FullscreenOptions* options)
{
    FullscreenParamsMap().insert(&element, MakeGarbageCollected<MetaParams>(request_type, options));
}
...
void GoFullscreen(Element& element, FullscreenRequestType request_type, const FullscreenOptions* options)
{
    ...
    if (RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(document.GetExecutionContext())) {
        Element::HideAllPopupsUntil(
            nullptr, document, HidePopupFocusBehavior::kNone, HidePopupForcingLevel::kHideImmediately, HidePopupIndependence::kHideUnrelated);
    }
...
bool AllowedToRequestFullscreen(Document& document)
{
    ...
    if (RuntimeEnabledFeatures::CapabilityDelegationFullscreenRequestEnabled(document.GetExecutionContext())
    ...
    if (RuntimeEnabledFeatures::WindowPlacementFullscreenOnScreensChangeEnabled(document.GetExecutionContext())
...
bool FullscreenElementReady(const Element& element, ReportOptions report_on_failure)
{
...
ScriptPromise Fullscreen::RequestFullscreen(
    Element& pending, const FullscreenOptions* options, FullscreenRequestType request_type, ScriptState* script_state, ExceptionState* exception_state)
{
    ...
    if (!for_cross_process_descendant && !RequestFullscreenConditionsMet(pending, document))
        error = true;
    ...
    if (!error) {
        ...
        ContinueRequestFullscreen(document, pending, request_type, options, resolver, true /* error */);
    }
...
void Fullscreen::DidResolveEnterFullscreenRequest(Document& document, bool granted)
{
    ...
    for (const Member<PendingRequest>& request : requests) {
        ContinueRequestFullscreen(document, *request->element(), request->type(), request->options(), request->resolver(), !granted);
    }
}

void Fullscreen::ContinueRequestFullscreen(
    Document& document, Element& pending, FullscreenRequestType request_type, const FullscreenOptions* options, ScriptPromiseResolver* resolver, bool error)
{
    ...
    if (pending.GetDocument() != document || !FullscreenElementReady(pending, ReportOptions::kDoNotReport))
        error = true;
    ...
    if (error) {
        ...
        if (resolver && resolver->GetScriptState()->ContextIsValid()) {
            ...
            resolver->Reject(V8ThrowException::CreateTypeError(resolver->GetScriptState()->GetIsolate(), "fullscreen error"));
        }
        return;
    }
...
ScriptPromise Fullscreen::ExitFullscreen(Document& doc, ScriptState* script_state, ExceptionState* exception_state, bool ua_originated)
{
...
void Fullscreen::ContinueExitFullscreen(Document* doc, ScriptPromiseResolver* resolver, bool resize)
{
...
bool Fullscreen::FullscreenEnabled(Document& document)
{
    ...
    return AllowedToUseFullscreen(document, ReportOptions::kDoNotReport) && FullscreenIsSupported(document);
}
...
Fullscreen::PendingRequest::PendingRequest(Element* element, FullscreenRequestType type, const FullscreenOptions* options, ScriptPromiseResolver* resolver)
    : element_(element)
    , type_(type)
    , options_(options)
    , resolver_(resolver)
