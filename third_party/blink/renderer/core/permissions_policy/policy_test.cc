...
public:
    void SetUp() override
    {
        ...
        auto permissions_policy = PermissionsPolicy::CreateFromParentPolicy(nullptr, {}, origin->ToUrlOrigin());
        auto header = PermissionsPolicyParser::ParseHeader("fullscreen *; payment 'self'; midi 'none'; camera 'self' "
                                                           "https://example.com https://example.net",
            "gyroscope=(self \"https://*.example.com\" \"https://example.net\")", origin.get(), dummy_logger_, dummy_logger_);
    ...
    PolicyParserMessageBuffer dummy_logger_ = PolicyParserMessageBuffer("", true /* discard_message */);
...
TEST_P(DOMFeaturePolicyTest, TestAllowsFeature)
{
    ...
    EXPECT_EQ(HasWildcardSubdomainsInPermissionsPolicy(), GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginASubdomain));
...
TEST_P(DOMFeaturePolicyTest, TestGetAllowList)
{
    ...
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "gyroscope"),
        UnorderedElementsAre(kSelfOrigin, kOriginB, HasWildcardSubdomainsInPermissionsPolicy() ? "https://*.example.com" : "https://%2A.example.com"));
}
...
TEST_P(IFramePolicyTest, TestCombinedPolicyOnOriginA)
{
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginA), dummy_logger_);
...
TEST_P(IFramePolicyTest, TestCombinedPolicyOnOriginASubdomain)
{
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginASubdomain), dummy_logger_);
    ...
    EXPECT_EQ(HasWildcardSubdomainsInPermissionsPolicy(), allowed_features.Contains("gyroscope"));
    // "sync-xhr" is still implicitly allowed on all origins.
    EXPECT_TRUE(allowed_features.Contains("sync-xhr"));
    // This feature does not exist, so should not be advertised as allowed.
    EXPECT_FALSE(allowed_features.Contains("badfeature"));
...
TEST_P(IFramePolicyTest, TestCombinedPolicyOnOriginB)
{
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginB), dummy_logger_);
...
TEST_P(IFramePolicyTest, TestCombinedPolicyOnOriginBSubdomain)
{
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginBSubdomain), dummy_logger_);
