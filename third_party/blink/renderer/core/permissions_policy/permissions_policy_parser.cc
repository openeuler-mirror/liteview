...
private:
    ...
    absl::optional<mojom::blink::PermissionsPolicyFeature> ParseFeatureName(const String& feature_name);
...
{
    if (src_origin_) {
        if (!execution_context_ || !ParsedFeaturePolicies::From(*execution_context_).Observed(feature)) {
            UMA_HISTOGRAM_ENUMERATION("Blink.UseCounter.FeaturePolicy.Allow", feature);
        }
    } else {
...
void ParsingContext::RecordAllowlistTypeUsage(size_t origin_count)
{
    ...
    if (origin_count == 0) {
        ...
        if (allowlist_includes_origin_) {
            if (allowlist_includes_star_ || allowlist_includes_none_ || allowlist_includes_src_ || allowlist_includes_self_)
...
{
    for (const FeaturePolicyAllowlistType allowlist_type : allowlist_types_used_) {
        if (src_origin_) {
            UMA_HISTOGRAM_ENUMERATION("Blink.UseCounter.FeaturePolicy.AttributeAllowlistType", allowlist_type);
        } else {
            UMA_HISTOGRAM_ENUMERATION("Blink.UseCounter.FeaturePolicy.HeaderAllowlistType", allowlist_type);
        }
    }
}

absl::optional<mojom::blink::PermissionsPolicyFeature> ParsingContext::ParseFeatureName(const String& feature_name)
{
    ...
    if (feature == mojom::blink::PermissionsPolicyFeature::kUnload) {
        if (execution_context_ && execution_context_->IsWindow() && RuntimeEnabledFeatures::PermissionsPolicyUnloadEnabled(execution_context_)) {
        ...
        for (const String& origin_string : origin_strings) {
            ...
            // when parsing an iframe allow attribute.
                ...
                if (!src_origin_->IsOpaque()) {
                    origin_with_possible_wildcards = OriginWithPossibleWildcards(src_origin_->ToUrlOrigin(),
                        /*has_subdomain_wildcard=*/false);
                } else {
            ...
            // result in an error message.
                ...
                origin_with_possible_wildcards = OriginWithPossibleWildcards::Parse(origin_string.Utf8(), type);
...
ParsedPermissionsPolicy ParsingContext::ParsePolicyFromNode(const PermissionsPolicyParser::Node& root)
{
    ...
    for (const PermissionsPolicyParser::Declaration& declaration_node : root.declarations) {
        absl::optional<ParsedPermissionsPolicyDeclaration> parsed_feature = ParseFeature(declaration_node, root.type);
        if (parsed_feature) {
...
{
    if (policy.length() > MAX_LENGTH_PARSE || policy.length() == 0)
        return {};

    //OutputDebugStringA(" ParsingContext::ParsePermissionsPolicyToIR not impl");
    return {};
...
{
    ParsedPermissionsPolicy permissions_policy = ParsingContext(permissions_policy_logger, origin, nullptr, GetDefaultFeatureNameMap(), execution_context)
                                                     .ParsePermissionsPolicy(permissions_policy_header);
    ParsedPermissionsPolicy feature_policy
        = ParsingContext(feature_policy_logger, origin, nullptr, GetDefaultFeatureNameMap(), execution_context).ParseFeaturePolicy(feature_policy_header);
    ...
    for (const auto& policy_declaration : feature_policy) {
        if (!observer.FeatureObserved(policy_declaration.feature)) {
            ...
            overlap_features.push_back(GetNameForFeature(policy_declaration.feature).Ascii().c_str());
        }
    }
...
{
    return ParsingContext(logger, self_origin, src_origin, GetDefaultFeatureNameMap(), execution_context).ParseFeaturePolicy(policy);
}
...
{
    return ParsingContext(logger, origin, /*src_origin=*/nullptr, GetDefaultFeatureNameMap(), execution_context).ParsePolicyFromNode(policy);
}
