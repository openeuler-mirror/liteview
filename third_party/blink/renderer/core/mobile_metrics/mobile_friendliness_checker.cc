...
static constexpr base::TimeDelta kTimeBudgetForBadTapTarget = base::Milliseconds(5);
...
static constexpr base::TimeDelta kTimeBudgetForTapTargetExtraction = base::Milliseconds(4);
...
MobileFriendlinessChecker::MobileFriendlinessChecker(LocalFrameView& frame_view)
    ...
    , last_evaluated_(base::TimeTicks::Now() - kEvaluationInterval - base::Seconds(5))
{
}
...
class FenwickTree {
public:
    explicit FenwickTree(wtf_size_t n)
        : tree(n + 1)
    {
    }
...
bool IsTapTargetCandidate(Node* node)
{
    ...
    } else if (auto* element = DynamicTo<HTMLElement>(node); element && element->WillRespondToMouseClickEvents()) {
        return true;
    }
...
bool ShouldSkipSubtree(const LayoutObject* object)
{
    ...
    if (const LayoutBox* box = DynamicTo<LayoutBox>(object)) {
        ...
        if ((rect.Width() == LayoutUnit() && style.OverflowX() != EOverflow::kVisible)
            || (rect.Height() == LayoutUnit() && style.OverflowY() != EOverflow::kVisible)) {
            return true;
        }
    }
    return object->IsElementContinuation() || style.Visibility() != EVisibility::kVisible || !style.IsContentVisibilityVisible();
}
...
void UnionAllChildren(const LayoutObject* parent, gfx::RectF& rect)
{
    ...
    while (obj) {
        ...
        if (obj->GetNode() && obj->GetNode()->getRootNode(&options)->IsInUserAgentShadowRoot()) {
...
bool AddElement(const LayoutObject* object, WTF::HashSet<Member<const LayoutObject>>* tap_targets, int finger_radius, Vector<int>& x_positions,
    Vector<std::pair<int, EdgeOrCenter>>& vertices)
{
    ...
    if (auto* element = DynamicTo<HTMLElement>(object->GetNode())) {
        ...
        if (!rect.IsEmpty() && !isnan(rect.x()) && !isnan(rect.y()) && !isnan(rect.right()) && !isnan(rect.bottom())) {
...
int ExtractAndCountAllTapTargets(const LocalFrameView& frame_view, int finger_radius, Vector<int>& x_positions, const base::TimeTicks& started,
    Vector<std::pair<int, EdgeOrCenter>>& vertices)
{
    LayoutObject* const root = frame_view.GetFrame().GetDocument()->GetLayoutView();
    ...
    for (const LayoutObject *forward = root, *backward = root; forward && backward;) {
        ...
        blink::GetRootNodeOptions options;
        if (forward->GetNode() != nullptr && forward->GetNode()->getRootNode(&options)->IsInUserAgentShadowRoot()) {
            ...
            if (!AddElement(forward, &tap_targets, finger_radius, x_positions, vertices)) {
                break;
            }
        ...
        if (backward->GetNode() != nullptr && backward->GetNode()->getRootNode(&options)->IsInUserAgentShadowRoot()) {
            // Ignore shadow elements that may contain overlapping tap targets.
            backward = backward->PreviousInPostOrderBeforeChildren(nullptr);
        } else if (ShouldSkipSubtree(backward)) {
            backward = backward->PreviousInPostOrderBeforeChildren(nullptr);
        } else {
            if (!AddElement(backward, &tap_targets, finger_radius, x_positions, vertices)) {
                break;
            }
    ...
    base::UmaHistogramBoolean("Blink.MobileMetrics.BadTapTargetRatioExtractionSucceed", success_in_time);
...
void CompressKeyWithVector(const Vector<int>& positions, Vector<std::pair<int, EdgeOrCenter>>& vertices)
{
    ...
    for (auto& it : vertices) {
        ...
        case EdgeOrCenter::kEndEdge: {
            vertex.v.edge.left.index
                = static_cast<wtf_size_t>(std::distance(positions.begin(), std::lower_bound(positions.begin(), positions.end(), vertex.v.edge.left.position)));
            vertex.v.edge.right.index
                = static_cast<wtf_size_t>(std::distance(positions.begin(), std::lower_bound(positions.begin(), positions.end(), vertex.v.edge.right.position)));
            break;
        }
        case EdgeOrCenter::kCenter: {
            vertex.v.center.index
                = static_cast<wtf_size_t>(std::distance(positions.begin(), std::lower_bound(positions.begin(), positions.end(), vertex.v.center.position)));
            break;
        }
        }
    }
...
int CountBadTapTargets(wtf_size_t rightmost_position, const Vector<std::pair<int, EdgeOrCenter>>& vertices, const base::TimeTicks& started)
{
...
{
    DCHECK(frame_view_->GetFrame().IsOutermostMainFrame());
    SCOPED_BLINK_UMA_HISTOGRAM_TIMER_HIGHRES("Blink.MobileMetrics.BadTapTargetsRatioTime");
    base::TimeTicks started = base::TimeTicks::Now();
    constexpr float kOneDipInMm = 0.15875;

    const int finger_radius = std::floor((3 / kOneDipInMm) / initial_scale_); // 3mm in logical pixel.
    ...
    for (const Frame* frame = &frame_view_->GetFrame(); frame; frame = frame->Tree().TraverseNext()) {
        ...
        // targets.
        const int got_tap_targets = ExtractAndCountAllTapTargets(*view, finger_radius, x_positions, started, vertices);
    ...
    x_positions.erase(std::unique(x_positions.begin(), x_positions.end()), x_positions.end());
    CompressKeyWithVector(x_positions, vertices);
    if (IsTimeBudgetExpired(started))
        return kTimeBudgetExceeded;

    // Reorder vertices by y dimension for sweeping full page from top to bottom.
    std::sort(vertices.begin(), vertices.end(), [](const std::pair<int, EdgeOrCenter>& a, const std::pair<int, EdgeOrCenter>& b) {
        // Ordering with kStart < kCenter < kEnd.
        return std::tie(a.first, a.second.type) < std::tie(b.first, b.second.type);
    });
    if (IsTimeBudgetExpired(started))
        return kTimeBudgetExceeded;

    // Sweep x-compressed y-ordered vertices to detect bad tap targets.
    const int bad_tap_targets = CountBadTapTargets(x_positions.size(), vertices, started);
...
{
    MobileFriendliness liness;
    liness.viewport_device_width = viewport_device_width_;
    liness.viewport_initial_scale_x10 = viewport_initial_scale_x10_;
    liness.viewport_hardcoded_width = viewport_hardcoded_width_;
    liness.allow_user_zoom = allow_user_zoom_;
    liness.small_text_ratio = area_sizes_.SmallTextRatio();
    liness.text_content_outside_viewport_percentage = area_sizes_.TextContentsOutsideViewportPercentage(
        // Use SizeF when computing the area to avoid integer overflow.
        gfx::SizeF(frame_view_->GetPage()->GetVisualViewport().Size()).GetArea());
    liness.bad_tap_targets_ratio = ComputeBadTapTargetsRatio();
    frame_view_->DidChangeMobileFriendliness(liness);
