...
TEST(WebDragDataTest, items)
{
    ...
    // User visible snapshot file.
    {
        ...
        data_object->Add(File::CreateForFileSystemFile("name", metadata, File::kIsUserVisible));
    }
    ...
    // Not user visible snapshot file.
    {
        ...
        data_object->Add(File::CreateForFileSystemFile("name", metadata, File::kIsNotUserVisible));
    }
    ...
    EXPECT_EQ("filesystem:http://example.com/isolated/hash/visible-non-native-file", items[4].file_system_url);
    EXPECT_EQ(1234, items[4].file_system_file_size);

    EXPECT_EQ(WebDragData::Item::kStorageTypeFileSystemFile, items[5].storage_type);
    EXPECT_EQ("filesystem:http://example.com/isolated/hash/not-visible-non-native-file", items[5].file_system_url);
