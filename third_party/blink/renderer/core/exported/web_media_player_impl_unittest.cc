...
class MockMediaObserver : public media::MediaObserver, public base::SupportsWeakPtr<MockMediaObserver> {
public:
...
public:
    ...
    MOCK_METHOD5(AddAudioTrack, WebMediaPlayer::TrackId(const WebString&, WebMediaPlayerClient::AudioTrackKind, const WebString&, const WebString&, bool));
    MOCK_METHOD1(RemoveAudioTrack, void(WebMediaPlayer::TrackId));
    MOCK_METHOD5(AddVideoTrack, WebMediaPlayer::TrackId(const WebString&, WebMediaPlayerClient::VideoTrackKind, const WebString&, const WebString&, bool));
    ...
    MOCK_METHOD5(DidMediaMetadataChange, void(bool, bool, media::AudioCodec, media::VideoCodec, media::MediaContentType));
...
protected:
    void InitializeWebMediaPlayerImplInternal(std::unique_ptr<media::Demuxer> demuxer_override)
    {
        ...
        factory_selector->AddBaseFactory(media::RendererType::kDefault,
            std::make_unique<media::DefaultRendererFactory>(
                media_log.get(), decoder_factory_.get(), media::DefaultRendererFactory::GetGpuFactoriesCB(), player_id));
        factory_selector->StartRequestRemotePlayStateCB(base::DoNothing());
#else
        factory_selector->AddBaseFactory(media::RendererType::kDefault,
            std::make_unique<media::DefaultRendererFactory>(
                media_log.get(), decoder_factory_.get(), media::DefaultRendererFactory::GetGpuFactoriesCB(), player_id, nullptr));
#endif
        ...
        media::MediaMetricsProvider::Create(media::MediaMetricsProvider::BrowsingMode::kNormal, media::MediaMetricsProvider::FrameStatus::kNotTopFrame,
            ...
            base::BindRepeating(&WebMediaPlayerImplTest::GetRecordAggregateWatchTimeCallback, base::Unretained(this)),
            base::BindRepeating(&WebMediaPlayerImplTest::IsShuttingDown, base::Unretained(this)), provider.BindNewPipeAndPassReceiver());
        ...
        wmpi_ = std::make_unique<WebMediaPlayerImpl>(GetWebLocalFrame(), &client_, &encrypted_client_, &delegate_, std::move(factory_selector),
            ...
            media_thread_.task_runner(), media_thread_.task_runner(), media_thread_.task_runner(), media_thread_.task_runner(),
            base::BindRepeating(&WebMediaPlayerImplTest::OnAdjustAllocatedMemory, base::Unretained(this)), nullptr, media::RequestRoutingTokenCallback(),
            mock_observer_.AsWeakPtr(), false, false, provider.Unbind(),
            base::BindOnce(&WebMediaPlayerImplTest::CreateMockSurfaceLayerBridge, base::Unretained(this)), viz::TestContextProvider::Create(),
    ...
    media::MediaMetricsProvider::RecordAggregateWatchTimeCallback GetRecordAggregateWatchTimeCallback()
    {
    ...
    void Load(std::string data_file, LoadType load_type = LoadType::kFullyBuffered)
    {
        ...
        response.SetHttpHeaderField(WebString::FromUTF8("Content-Length"), WebString::FromUTF8(is_streaming ? "-1" : base::NumberToString(data->data_size())));
        response.SetExpectedContentLength(is_streaming ? -1 : data->data_size());
        response.SetHttpStatusCode(200);
        client->DidReceiveResponse(response);

        // Copy over the file data.
        client->DidReceiveData(reinterpret_cast<const char*>(data->data()), static_cast<int>(data->data_size()));
    ...
    void OnCdmCreated(base::RepeatingClosure quit_closure, WebContentDecryptionModule* cdm, const std::string& error_message)
    {
    ...
    void CreateCdm()
    {
        ...
        WebContentDecryptionModuleImpl::Create(
            &mock_cdm_factory_, test_origin, cdm_config, base::BindOnce(&WebMediaPlayerImplTest::OnCdmCreated, base::Unretained(this), run_loop.QuitClosure()));
        run_loop.Run();
    ...
    viz::LocalSurfaceId local_surface_id_ = viz::LocalSurfaceId(11, base::UnguessableToken::Deserialize(0x111111, 0));
...
TEST_F(WebMediaPlayerImplTest, BackgroundIdlePauseTimerDependsOnAudio)
{
    ...
    EXPECT_CALL(client_, PausePlayback(WebMediaPlayerClient::PauseReason::kSuspendedPlayerIdleTimeout));
    FireIdlePauseTimer();
    base::RunLoop().RunUntilIdle();
}
...
TEST_F(WebMediaPlayerImplTest, MemDumpReporting)
{
    ...
    base::trace_event::MemoryDumpRequestArgs args
        = { 1 /* dump_guid*/, base::trace_event::MemoryDumpType::EXPLICITLY_TRIGGERED, base::trace_event::MemoryDumpLevelOfDetail::DETAILED };
    ...
    auto on_memory_dump_done = base::BindLambdaForTesting([&](bool success, uint64_t dump_guid, std::unique_ptr<base::trace_event::ProcessMemoryDump> pmd) {
        ...
        if (args.level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::DETAILED) {
...
public:
    ...
    void SetUp() override
    {
        ...
        SetLoadType(is_media_source ? WebMediaPlayer::kLoadTypeMediaSource : WebMediaPlayer::kLoadTypeURL);
    ...
    {
        return WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideoMs / base::Time::kMillisecondsPerSecond;
    }
...
TEST_P(WebMediaPlayerImplBackgroundBehaviorTest, AudioVideo)
{
    ...
    EXPECT_EQ((!IsBackgroundVideoPlaybackEnabled() || (IsMediaSuspendOn() && IsResumeBackgroundVideoEnabled())) && !(IsPictureInPictureOn() && IsAndroid()),
...
INSTANTIATE_TEST_SUITE_P(BackgroundBehaviorTestInstances, WebMediaPlayerImplBackgroundBehaviorTest,
    ::testing::Combine(::testing::Bool(),
        ::testing::Values(WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideoMs / base::Time::kMillisecondsPerSecond - 1, 300),
        ::testing::Values(WebMediaPlayerImpl::kMaxKeyframeDistanceToDisableBackgroundVideoMs / base::Time::kMillisecondsPerSecond - 1, 100), ::testing::Bool(),
