...
class ComputedAccessibleNodePromiseResolver::RequestAnimationFrameCallback final : public FrameCallback {
public:
    explicit RequestAnimationFrameCallback(ComputedAccessibleNodePromiseResolver* resolver)
        : resolver_(resolver)
    {
    }
...
ComputedAccessibleNodePromiseResolver::ComputedAccessibleNodePromiseResolver(ScriptState* script_state, Document& document, AXID ax_id)
    : ax_id_(ax_id)
...
ComputedAccessibleNodePromiseResolver::ComputedAccessibleNodePromiseResolver(ScriptState* script_state, Element& element)
    : element_(element)
    , resolver_(MakeGarbageCollected<ScriptPromiseResolver>(script_state))
    , ax_context_(std::make_unique<AXContext>(element.GetDocument(), ui::kAXModeComplete))
{
}
...
void ComputedAccessibleNodePromiseResolver::EnsureUpToDate()
{
    ...
    continue_callback_request_id_ = ax_context_->GetDocument()->RequestAnimationFrame(callback);
}
...
void ComputedAccessibleNodePromiseResolver::UpdateTreeAndResolve()
{
    ...
    WebLocalFrameClient* client = WebLocalFrameImpl::FromFrame(local_frame)->Client();
    ...
    ax_context_->GetDocument()->View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kAccessibility);
    ...
    ComputedAccessibleNode* accessible_node = ax_context_->GetDocument()->GetOrCreateComputedAccessibleNode(ax_id);
...
ScriptPromise ComputedAccessibleNode::ensureUpToDate(ScriptState* script_state)
{
    if (!GetDocument())
        return ScriptPromise(); // Empty promise.
    auto* resolver = MakeGarbageCollected<ComputedAccessibleNodePromiseResolver>(script_state, *GetDocument(), ax_id_);
...
absl::optional<bool> ComputedAccessibleNode::GetBoolAttribute(WebAOMBoolAttribute attr) const
{
    bool value;
...
absl::optional<int32_t> ComputedAccessibleNode::GetIntAttribute(WebAOMIntAttribute attr) const
{
    int32_t value;
...
absl::optional<float> ComputedAccessibleNode::GetFloatAttribute(WebAOMFloatAttribute attr) const
{
    float value;
...
const String ComputedAccessibleNode::GetStringAttribute(WebAOMStringAttribute attr) const
{
    WebString out;
