...
const AtomicString& AccessibleNode::GetProperty(AOMStringProperty property) const
{
...
// static
AccessibleNode* AccessibleNode::GetProperty(Element* element, AOMRelationProperty property)
{
    if (!element)
        return nullptr;
...
// static
AccessibleNodeList* AccessibleNode::GetProperty(Element* element, AOMRelationListProperty property)
{
    if (!element)
        return nullptr;
...
// static
bool AccessibleNode::GetProperty(Element* element, AOMRelationListProperty property, HeapVector<Member<Element>>& targets)
{
...
template <typename P, typename T> static absl::optional<T> FindPropertyValue(P property, const Vector<std::pair<P, T>>& properties)
{
...
absl::optional<bool> AccessibleNode::GetProperty(AOMBooleanProperty property) const
{
    return FindPropertyValue(property, boolean_properties_);
}

// static
absl::optional<int32_t> AccessibleNode::GetProperty(Element* element, AOMIntProperty property)
{
    if (!element || !element->ExistingAccessibleNode())
        return absl::nullopt;
    return FindPropertyValue(property, element->ExistingAccessibleNode()->int_properties_);
}

// static
absl::optional<uint32_t> AccessibleNode::GetProperty(Element* element, AOMUIntProperty property)
{
    if (!element || !element->ExistingAccessibleNode())
        return absl::nullopt;
    return FindPropertyValue(property, element->ExistingAccessibleNode()->uint_properties_);
}

// static
absl::optional<float> AccessibleNode::GetProperty(Element* element, AOMFloatProperty property)
{
    if (!element || !element->ExistingAccessibleNode())
        return absl::nullopt;
    return FindPropertyValue(property, element->ExistingAccessibleNode()->float_properties_);
}
...
// static
const AtomicString& AccessibleNode::GetElementOrInternalsARIAAttribute(Element& element, const QualifiedName& attribute, bool is_token_attr)
{
    const AtomicString& attr_value = element.FastGetAttribute(attribute);
    if ((attr_value != g_null_atom) && (!is_token_attr || !IsUndefinedAttrValue(attr_value))) {
        return attr_value;
    }
...
// static
const AtomicString& AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMStringProperty property)
{
    if (!element)
        return g_null_atom;
    ...
    const AtomicString& attr_value = GetElementOrInternalsARIAAttribute(*element, attribute, is_token_attr);
...
// static
Element* AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMRelationProperty property)
{
    if (!element)
        return nullptr;
    QualifiedName attribute = GetCorrespondingARIAAttribute(property);
...
// static
bool AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMRelationListProperty property, HeapVector<Member<Element>>& targets)
{
    if (!element)
        return false;
    QualifiedName attribute = GetCorrespondingARIAAttribute(property);
    String value = GetElementOrInternalsARIAAttribute(*element, attribute).GetString();
    if (value.empty() && property == AOMRelationListProperty::kLabeledBy) {
        value = GetElementOrInternalsARIAAttribute(*element, html_names::kAriaLabeledbyAttr).GetString();
    }
    if (value.empty())
        return false;
...
// static
bool AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMBooleanProperty property, bool& is_null)
{
    is_null = true;
    if (!element)
        return false;
    ...
    QualifiedName attribute = GetCorrespondingARIAAttribute(property);
    AtomicString attr_value = GetElementOrInternalsARIAAttribute(*element, attribute);
    is_null = IsUndefinedAttrValue(attr_value);
    return !is_null && !EqualIgnoringASCIICase(attr_value, "false");
}

// static
float AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMFloatProperty property, bool& is_null)
{
    is_null = true;
    if (!element)
        return 0.0;
...
// static
uint32_t AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMUIntProperty property, bool& is_null)
{
    is_null = true;
    if (!element)
        return 0;
...
// static
int32_t AccessibleNode::GetPropertyOrARIAAttribute(Element* element, AOMIntProperty property, bool& is_null)
{
    is_null = true;
    if (!element)
        return 0;
...
{
    SetRelationProperty(AOMRelationProperty::kActiveDescendant, active_descendant);
...
void AccessibleNode::setBrailleRoleDescription(const AtomicString& braille_role_description)
{
    SetStringProperty(AOMStringProperty::kAriaBrailleRoleDescription, braille_role_description);
...
void AccessibleNode::appendChild(AccessibleNode* child, ExceptionState& exception_state)
{
    if (child == this) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "An AccessibleNode cannot be a child of itself");
        return;
    }

    if (child->element()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "An AccessibleNode associated with an Element cannot be a child.");
        return;
    }

    if (child->parent_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Reparenting is not supported yet.");
        return;
    }
    child->document_ = GetAncestorDocument();
    if (!child->document_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "AccessibleNode must have an ancestor that is attached to a document.");
        return;
    }
    child->parent_ = this;

    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Trying to access an AccessibleNode in a detached window.");
        return;
    }

    if (!GetExecutionContext()->GetSecurityOrigin()->CanAccess(child->GetExecutionContext()->GetSecurityOrigin())) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Trying to access an AccessibleNode from a different origin.");
        return;
    }
...
void AccessibleNode::removeChild(AccessibleNode* old_child, ExceptionState& exception_state)
{
    if (old_child->parent_ != this) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Node to remove is not a child of this node.");
        return;
    }
    auto* ix = base::ranges::find(children_, old_child, &Member<AccessibleNode>::Get);
    if (ix == children_.end()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Node to remove is not a child of this node.");
        return;
    }
...
void AccessibleNode::SetStringProperty(AOMStringProperty property, const AtomicString& value)
{
    for (auto& item : string_properties_) {
        if (item.first == property) {
            item.second = value;
...
void AccessibleNode::SetRelationProperty(AOMRelationProperty property, AccessibleNode* value)
{
    for (auto& item : relation_properties_) {
        if (item.first == property) {
            item.second = value;
...
void AccessibleNode::SetRelationListProperty(AOMRelationListProperty property, AccessibleNodeList* value)
{
    for (auto& item : relation_list_properties_) {
        if (item.first == property) {
            if (item.second)
...
template <typename P, typename T> static void SetProperty(P property, absl::optional<T> value, Vector<std::pair<P, T>>& properties)
{
...
void AccessibleNode::SetBooleanProperty(AOMBooleanProperty property, absl::optional<bool> value)
{
    SetProperty(property, value, boolean_properties_);
}

void AccessibleNode::SetIntProperty(AOMIntProperty property, absl::optional<int32_t> value)
{
    SetProperty(property, value, int_properties_);
}

void AccessibleNode::SetUIntProperty(AOMUIntProperty property, absl::optional<uint32_t> value)
{
    SetProperty(property, value, uint_properties_);
}

void AccessibleNode::SetFloatProperty(AOMFloatProperty property, absl::optional<float> value)
{
...
void AccessibleNode::NotifyAttributeChanged(const blink::QualifiedName& attribute)
{
