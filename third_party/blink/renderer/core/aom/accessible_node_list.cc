...
// static
AccessibleNodeList* AccessibleNodeList::Create(const HeapVector<Member<AccessibleNode>>& nodes)
{
...
void AccessibleNodeList::AddOwner(AOMRelationListProperty property, AccessibleNode* node)
{
    owners_.push_back(std::make_pair(property, node));
}

void AccessibleNodeList::RemoveOwner(AOMRelationListProperty property, AccessibleNode* node)
{
...
IndexedPropertySetterResult AccessibleNodeList::AnonymousIndexedSetter(unsigned index, AccessibleNode* node, ExceptionState& state)
{
    if (!node) {
        remove(index);
