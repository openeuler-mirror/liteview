...
public:
    ...
    // device-pixel boxes.
    static gfx::SizeF ComputeZoomAdjustedBox(ResizeObserverBoxOptions box_option, LayoutObject* layout_object, const ComputedStyle& style);

    // Compute a scaled and pixel snapped device pixel content box for svg
    // bounding boxes.
    static gfx::SizeF ComputeSnappedDevicePixelContentBox(LayoutSize box_size, LayoutObject* layout_object, const ComputedStyle& style);

    static DOMRectReadOnly* ZoomAdjustedLayoutRect(LayoutRect content_rect, const ComputedStyle& style);
};
