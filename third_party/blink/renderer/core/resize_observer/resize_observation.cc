...
bool ResizeObservation::ObservationSizeOutOfSync()
{
    ...
    if (UNLIKELY(target_ && DisplayLockUtilities::IsInLockedSubtreeCrossingFrames(*target_))) {
        return false;
    }
...
LayoutSize ResizeObservation::ComputeTargetSize() const
{
    if (target_) {
        if (LayoutObject* layout_object = target_->GetLayoutObject()) {
            ...
            if (auto* svg_graphics_element = DynamicTo<SVGGraphicsElement>(target_.Get())) {
                ...
                case ResizeObserverBoxOptions::kDevicePixelContentBox: {
                    ...
                    LayoutSize snapped_device_pixel_content_box_size
                        = LayoutSize(ResizeObserverUtilities::ComputeSnappedDevicePixelContentBox(bounding_box_size, layout_object, style));
            ...
            return LayoutSize(ResizeObserverUtilities::ComputeZoomAdjustedBox(observed_box_, layout_object, style));
        }
    }
    return LayoutSize();
}
