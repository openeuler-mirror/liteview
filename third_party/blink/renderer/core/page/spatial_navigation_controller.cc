...
bool SpatialNavigationController::HandleEnterKeyboardEvent(KeyboardEvent* event)
{
    ...
    if (event->type() == event_type_names::kKeydown) {
        ...
        if (RuntimeEnabledFeatures::FocuslessSpatialNavigationEnabled() && enter_key_down_seen_ && enter_key_press_seen_) {
            interest_element->Focus(FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kSpatialNavigation, nullptr));
            // We need enter to activate links, etc. The click should be after the
            // focus in case the site transfers focus upon clicking.
            interest_element->DispatchSimulatedClick(event, SimulatedClickCreationScope::kFromAccessibility);
        }
    }
...
void SpatialNavigationController::DidDetachFrameView(const LocalFrameView& view)
{
...
Node* SpatialNavigationController::StartingNode()
{
    if (RuntimeEnabledFeatures::FocuslessSpatialNavigationEnabled()) {
        if (interest_element_ && IsValidCandidate(interest_element_)) {
            ...
            if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(interest_element_.Get()))
...
void SpatialNavigationController::MoveInterestTo(Node* next_node)
{
    ...
    if (RuntimeEnabledFeatures::FocuslessSpatialNavigationEnabled()) {
        if (interest_element_) {
            ...
            interest_element_->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kPseudoClass));
        }
        ...
        if (interest_element_) {
            ...
            scroll_into_view_util::ScrollRectToVisible(*layout_object, element->BoundingBoxForScrollIntoView(), ScrollAlignment::CreateScrollIntoViewParams());
        }
    ...
    element->Focus(FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kSpatialNavigation, nullptr));
...
void SpatialNavigationController::OnSpatialNavigationSettingChanged()
{
    ...
    if (!page_->MainFrame())
        return;
    LocalFrame* frame = DynamicTo<LocalFrame>(page_->GetFocusController().FocusedOrMainFrame());
    if (frame && frame->GetDocument() && IsValidCandidate(frame->GetDocument()->FocusedElement())) {
...
void SpatialNavigationController::FocusedNodeChanged(Document* document)
{
    ...
    if (document->FocusedElement() && interest_element_ != document->FocusedElement()) {
