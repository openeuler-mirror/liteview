...
public:
    ...
    virtual void ComputePageRectsWithPageSize(const gfx::SizeF& page_size_in_pixels);

    // These are only valid after page rects are computed.
    wtf_size_t PageCount() const
    {
        return page_rects_.size();
    }
    const gfx::Rect& PageRect(wtf_size_t page_number) const
    {
        return page_rects_[page_number];
    }
    const Vector<gfx::Rect>& PageRects() const
    {
        return page_rects_;
    }
    ...
    static String PageProperty(LocalFrame*, const char* property_name, uint32_t page_number);
    static bool IsPageBoxVisible(LocalFrame*, uint32_t page_number);
    static String PageSizeAndMarginsInPixels(
        LocalFrame*, uint32_t page_number, int width, int height, int margin_top, int margin_right, int margin_bottom, int margin_left);
...
private:
    void ComputePageRectsWithPageSizeInternal(const gfx::SizeF& page_size_in_pixels);
