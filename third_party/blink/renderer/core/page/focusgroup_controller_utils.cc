...
{
    return direction == FocusgroupDirection::kForwardHorizontal || direction == FocusgroupDirection::kForwardVertical;
}

bool FocusgroupControllerUtils::IsDirectionBackward(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardHorizontal || direction == FocusgroupDirection::kBackwardVertical;
}

bool FocusgroupControllerUtils::IsDirectionHorizontal(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardHorizontal || direction == FocusgroupDirection::kForwardHorizontal;
}

bool FocusgroupControllerUtils::IsDirectionVertical(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardVertical || direction == FocusgroupDirection::kForwardVertical;
}

bool FocusgroupControllerUtils::IsAxisSupported(FocusgroupFlags flags, FocusgroupDirection direction)
{
    return ((flags & FocusgroupFlags::kHorizontal) && IsDirectionHorizontal(direction))
        || ((flags & FocusgroupFlags::kVertical) && IsDirectionVertical(direction));
}

bool FocusgroupControllerUtils::WrapsInDirection(FocusgroupFlags flags, FocusgroupDirection direction)
{
    return ((flags & FocusgroupFlags::kWrapHorizontally) && IsDirectionHorizontal(direction))
        || ((flags & FocusgroupFlags::kWrapVertically) && IsDirectionVertical(direction));
}
    ...
    for (Element* ancestor = FlatTreeTraversal::ParentElement(*element); ancestor; ancestor = FlatTreeTraversal::ParentElement(*ancestor)) {
        ...
        if (ancestor_flags != FocusgroupFlags::kNone) {
            ...
            case FocusgroupType::kGrid:
                ...
                if (ancestor_flags & FocusgroupFlags::kGrid && IsA<LayoutNGTable>(ancestor->GetLayoutObject())) {
                    return ancestor;
                }
                break;
...
{
    if (IsA<LayoutNGTable>(root->GetLayoutObject()) && root->GetFocusgroupFlags() & FocusgroupFlags::kGrid) {
