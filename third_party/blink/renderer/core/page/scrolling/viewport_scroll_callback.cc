...
ViewportScrollCallback::ViewportScrollCallback(
    BrowserControls* browser_controls, OverscrollController* overscroll_controller, RootFrameViewport& root_frame_viewport)
...
bool ViewportScrollCallback::ShouldScrollBrowserControls(const ScrollOffset& delta, ui::ScrollGranularity granularity) const
{
    if (granularity != ui::ScrollGranularity::kScrollByPixel && granularity != ui::ScrollGranularity::kScrollByPrecisePixel)
        return false;
...
ScrollResult ViewportScrollCallback::PerformNativeScroll(ScrollState& state)
{
    ...
    ScrollResult result = root_frame_viewport_->UserScroll(granularity, delta, ScrollableArea::ScrollCallback());
    ...
    state.ConsumeDeltaNative(delta.x() - result.unused_scroll_delta_x, delta.y() - result.unused_scroll_delta_y);
