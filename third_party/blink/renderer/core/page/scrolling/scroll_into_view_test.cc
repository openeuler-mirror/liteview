...
TEST_F(ScrollIntoViewTest, ScrollPaddingOnDocumentElWhenDocumentElDefinesViewport)
{
    v8::HandleScope HandleScope(WebView().GetPage()->GetAgentGroupScheduler().Isolate());
    WebView().MainFrameViewWidget()->Resize(gfx::Size(300, 300));
    SimRequest request("https://example.com/test.html", "text/html");
...
TEST_F(ScrollIntoViewTest, SmoothUserScrollNotAbortedByProgrammaticScrolls)
{
    ...
    scroll_into_view_util::ScrollRectToVisible(*content->GetLayoutObject(), content->BoundingBoxForScrollIntoView(),
        ScrollAlignment::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::TopAlways(), mojom::blink::ScrollType::kUser, false,
...
TEST_F(ScrollIntoViewTest, OriginCrossingUseCounter)
{
    ...
    Element* xorigin_child_frame = GetDocument().getElementById("xoriginChildFrame");
