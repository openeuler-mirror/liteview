...
#define EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(actual) EXPECT_EQ(cc::MainThreadScrollingReason::kNotScrollingOnMain, actual)
...
public:
    ...
    {
        const auto* scroll = GetFrame()->View()->GetLayoutView()->FirstFragment().PaintProperties()->Scroll();
...
TEST_P(MainThreadScrollingReasonsTest, BackgroundAttachmentFixedShouldTriggerMainThreadScroll)
{
    ...
    EXPECT_MAIN_THREAD_SCROLLING_REASON(
        cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, inner_scroll_node->GetMainThreadScrollingReasons());
    ...
    EXPECT_MAIN_THREAD_SCROLLING_REASON(cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, GetMainThreadScrollingReasons(inner_scroll_layer));
    ...
    EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(outer_scroll_node->GetMainThreadScrollingReasons());
    ...
    EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(GetMainThreadScrollingReasons(outer_scroll_layer));
    ...
    EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(inner_scroll_node->GetMainThreadScrollingReasons());
    ASSERT_EQ(inner_scroll_layer, CcLayerByCcElementId(root_layer, inner_scroll_node->GetCompositorElementId()));
    ASSERT_TRUE(IsScrollable(inner_scroll_layer));
    EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(GetMainThreadScrollingReasons(inner_scroll_layer));
    ...
    element->setAttribute("style", "background-image: url('white-1x1.png'); background-attachment: fixed;", ASSERT_NO_EXCEPTION);
    ...
    ASSERT_EQ(outer_scroll_node, outer_layout_view->FirstFragment().PaintProperties()->Scroll());
    EXPECT_MAIN_THREAD_SCROLLING_REASON(
        cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, outer_scroll_node->GetMainThreadScrollingReasons());
    ASSERT_EQ(outer_scroll_layer, CcLayerByCcElementId(root_layer, outer_scroll_node->GetCompositorElementId()));
    ASSERT_TRUE(IsScrollable(outer_scroll_layer));
    EXPECT_MAIN_THREAD_SCROLLING_REASON(cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, GetMainThreadScrollingReasons(outer_scroll_layer));
}
...
TEST_P(MainThreadScrollingReasonsTest, RecalculateMainThreadScrollingReasonsUponResize)
{
    ...
    EXPECT_MAIN_THREAD_SCROLLING_REASON(cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, GetViewMainThreadScrollingReasons());
...
protected:
    ...
    void TestNonCompositedReasons(const AtomicString& style_class, const uint32_t reason)
    {
        GetWebView()->GetSettings()->SetPreferCompositingToLCDTextEnabled(false);
        ...
        EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(scrollable_area->GetNonCompositedMainThreadScrollingReasons());

        container->classList().Add(style_class);
        ForceFullCompositingUpdate();

        ASSERT_TRUE(scrollable_area);
        EXPECT_MAIN_THREAD_SCROLLING_REASON(reason, scrollable_area->GetNonCompositedMainThreadScrollingReasons());
        ...
        EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(scrollable_area2->GetNonCompositedMainThreadScrollingReasons());

        LocalFrameView* frame_view = GetFrame()->View();
        ASSERT_TRUE(frame_view);
        EXPECT_NO_MAIN_THREAD_SCROLLING_REASON(frame_view->GetMainThreadScrollingReasons());
...
{
    TestNonCompositedReasons("background-not-opaque", cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText);
}

TEST_P(NonCompositedMainThreadScrollingReasonsTest, CantPaintScrollingBackgroundTest)
{
    TestNonCompositedReasons("cant-paint-scrolling-background", cc::MainThreadScrollingReason::kCantPaintScrollingBackgroundAndLCDText);
}
...
{
    TestNonCompositedReasons("inset-box-shadow", cc::MainThreadScrollingReason::kCantPaintScrollingBackgroundAndLCDText);
}
...
TEST_P(NonCompositedMainThreadScrollingReasonsTest, ForcedComositingWithLCDRelatedReasons)
{
    ...
    container->setAttribute("class", "composited transparent", ASSERT_NO_EXCEPTION);
    ForceFullCompositingUpdate();
    ...
    container2->setAttribute("class", "composited border-radius", ASSERT_NO_EXCEPTION);
    ForceFullCompositingUpdate();
    PaintLayerScrollableArea* scrollable_area2 = GetScrollableArea(*container2);
    ASSERT_TRUE(scrollable_area2);
