...
#define EXPECT_WHEEL_BUCKET(index, count) histogram_tester->ExpectBucketCount("Renderer4.MainThreadWheelScrollReason2", index, count);

#define EXPECT_TOUCH_BUCKET(index, count) histogram_tester->ExpectBucketCount("Renderer4.MainThreadGestureScrollReason2", index, count);

#define EXPECT_WHEEL_TOTAL(count) histogram_tester->ExpectTotalCount("Renderer4.MainThreadWheelScrollReason2", count);

#define EXPECT_TOUCH_TOTAL(count) histogram_tester->ExpectTotalCount("Renderer4.MainThreadGestureScrollReason2", count);
...
TEST_F(ScrollMetricsTest, TouchAndWheelGeneralTest)
{
    ...
    if (base::FeatureList::IsEnabled(::features::kScrollUnification)) {
        ...
        EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kFailedHitTest), 1);
        EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNoScrollingLayer), 1);
        ...
        //
        EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNonFastScrollableRegion), 1);
        EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText), 1);
        EXPECT_TOUCH_BUCKET(cc::MainThreadScrollingReason::kScrollingOnMainForAnyReason, 2);
    ...
    if (base::FeatureList::IsEnabled(::features::kScrollUnification)) {
        // cc reports the below reasons because #box is not composited.
        EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kFailedHitTest), 1);
        EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNoScrollingLayer), 1);
        ...
        //   kScrollingOnMainForAnyReason (again)
        //
        EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNonFastScrollableRegion), 1);
        EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText), 1);
        EXPECT_WHEEL_BUCKET(cc::MainThreadScrollingReason::kScrollingOnMainForAnyReason, 2);
...
TEST_F(ScrollMetricsTest, CompositedScrollableAreaTest)
{
    ...
    Compositor().BeginFrame();
    Scroll(box, WebGestureDevice::kTouchpad);
    EXPECT_FALSE(To<LayoutBox>(box->GetLayoutObject())->GetScrollableArea()->GetNonCompositedMainThreadScrollingReasons());
