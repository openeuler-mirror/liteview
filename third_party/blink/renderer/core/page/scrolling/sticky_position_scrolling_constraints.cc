...
PhysicalOffset StickyPositionScrollingConstraints::AncestorStickyBoxOffset() const
{
    ...
    auto* constraints = nearest_sticky_layer_shifting_sticky_box->GetLayoutObject().StickyConstraints();
    DCHECK(constraints);
...
PhysicalOffset StickyPositionScrollingConstraints::AncestorContainingBlockOffset() const
{
    if (!nearest_sticky_layer_shifting_containing_block)
        return PhysicalOffset();
    auto* constraints = nearest_sticky_layer_shifting_containing_block->GetLayoutObject().StickyConstraints();
    DCHECK(constraints);
