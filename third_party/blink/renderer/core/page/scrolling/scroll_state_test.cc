...
ScrollState* CreateScrollState(double delta_x, double delta_y, bool beginning, bool ending)
{
...
TEST_F(ScrollStateTest, ConsumeDeltaNative)
{
    ...
    scroll_state->ConsumeDeltaNative(scroll_state->deltaX(), scroll_state->deltaY());
    EXPECT_TRUE(scroll_state->DeltaConsumedForScrollSequence());
...
TEST_F(ScrollStateTest, CurrentNativeScrollingElement)
{
    ...
    auto* element = MakeGarbageCollected<Element>(QualifiedName::Null(), Document::CreateForTest());
