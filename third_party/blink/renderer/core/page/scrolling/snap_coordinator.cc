...
LayoutBox* FindSnapContainer(const LayoutBox& origin_box)
{
    ...
    while (box && !box->IsScrollContainer() && !IsA<LayoutView>(box) && box->GetNode() != document_element) {
...
void SnapCoordinator::AddSnapContainer(LayoutBox& snap_container)
{
    ...
    if (snap_areas_to_reassign.size()) {
        ancestor_snap_container->GetScrollableArea()->SetSnapContainerDataNeedsUpdate(true);
    }
}
...
void SnapCoordinator::SnapContainerDidChange(LayoutBox& snap_container)
{
    ...
    // by the LayoutView.
    if (snap_container.GetNode() == snap_container.GetDocument().documentElement())
        return;

    PaintLayerScrollableArea* scrollable_area = snap_container.GetScrollableArea();
...
void SnapCoordinator::SnapAreaDidChange(LayoutBox& snap_area, cc::ScrollSnapAlign scroll_snap_align)
{
    LayoutBox* old_container = snap_area.SnapContainer();
    if (scroll_snap_align.alignment_inline == cc::SnapAlignment::kNone && scroll_snap_align.alignment_block == cc::SnapAlignment::kNone) {
...
void SnapCoordinator::ResnapAllContainersIfNeeded()
{
    for (const auto& container : snap_containers_) {
        ...
        // crbug.com/866127
        if (!did_first_resnap_all_containers_ && scrollable_area->GetScrollOffset() != initial_offset) {
            UseCounter::Count(container->GetDocument(), WebFeature::kScrollSnapCausesScrollOnInitialLayout);
        }
    }
...
cc::SnapAreaData SnapCoordinator::CalculateSnapAreaData(const LayoutBox& snap_area, const LayoutBox& snap_container)
{
    const ComputedStyle* container_style = snap_container.Style();
    ...
    area_rect = snap_area.LocalToAncestorRect(area_rect, &snap_container, kTraverseDocumentBoundaries | kIgnoreScrollOffset);
    ...
    snap_area_data.element_id = CompositorElementIdFromDOMNodeId(DOMNodeIds::IdForNode(snap_area.GetNode()));
...
void SnapCoordinator::ShowSnapDataFor(const LayoutBox* snap_container)
{
    if (!snap_container)
        return;
    ScrollableArea* scrollable_area = ScrollableArea::GetForScrolling(snap_container);
    const auto* optional_data = scrollable_area ? scrollable_area->GetSnapContainerData() : nullptr;
    if (optional_data)
