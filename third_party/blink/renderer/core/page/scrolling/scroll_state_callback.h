...
protected:
    explicit ScrollStateCallback(enum NativeScrollBehavior native_scroll_behavior = NativeScrollBehavior::kDisableNativeScroll)
...
public:
    static ScrollStateCallbackV8Impl* Create(V8ScrollStateCallback* callback, const String& native_scroll_behavior)
    {
        DCHECK(callback);
        return MakeGarbageCollected<ScrollStateCallbackV8Impl>(callback, ParseNativeScrollBehavior(native_scroll_behavior));
    }

    explicit ScrollStateCallbackV8Impl(V8ScrollStateCallback* callback, enum NativeScrollBehavior native_scroll_behavior)
...
private:
    static enum NativeScrollBehavior ParseNativeScrollBehavior(const String& native_scroll_behavior);
