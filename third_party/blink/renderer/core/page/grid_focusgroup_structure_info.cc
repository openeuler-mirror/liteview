...
Element* AutomaticGridFocusgroupStructureInfo::PreviousCellInRow(const Element* cell_element)
{
    ...
    auto* cell = ToInterface<LayoutNGTableCellInterface>(cell_element->GetLayoutObject());
    if (!cell)
        return nullptr;
...
Element* AutomaticGridFocusgroupStructureInfo::PreviousCellInColumn(const Element* cell_element)
{
    ...
    auto* previous_row_element = DynamicTo<Element>(previous_row->ToLayoutObject()->GetNode());
...
Element* AutomaticGridFocusgroupStructureInfo::NextCellInColumn(const Element* cell_element)
{
    ...
    auto* next_row_element = DynamicTo<Element>(next_row->ToLayoutObject()->GetNode());
    if (!next_row_element)
        return nullptr;
...
Element* AutomaticGridFocusgroupStructureInfo::PreviousRow(Element* row_element)
{
    ...
    auto* row = ToInterface<LayoutNGTableRowInterface>(row_element->GetLayoutObject());
    if (!row)
        return nullptr;
...
LayoutNGTableRowInterface* AutomaticGridFocusgroupStructureInfo::PreviousRow(LayoutNGTableRowInterface* current_row)
{
    ...
    while (!previous_row || !previous_row->FirstCellInterface()) {
        ...
        auto* previous_section = Table()->PreviousSectionInterface(current_section, kSkipEmptySections);
        if (!previous_section)
            return nullptr;
...
LayoutNGTableRowInterface* AutomaticGridFocusgroupStructureInfo::NextRow(LayoutNGTableRowInterface* current_row)
{
    auto* current_section = current_row->SectionInterface();
    ...
    while (!next_row || !next_row->FirstCellInterface()) {
        ...
        auto* next_section = Table()->NextSectionInterface(current_section, kSkipEmptySections);
        if (!next_section)
            return nullptr;
...
LayoutNGTableCellInterface* AutomaticGridFocusgroupStructureInfo::TableCellAtIndexInRowRecursive(
    unsigned index, LayoutNGTableRowInterface* row, absl::optional<unsigned> expected_rowspan)
{
    if (!row)
        return nullptr;
    ...
    if (auto* table_row = DynamicTo<HTMLTableRowElement>(row->ToLayoutObject()->GetNode())) {
        ...
        if (table_cell)
            cell = ToInterface<LayoutNGTableCellInterface>(table_cell->GetLayoutObject());
    }
