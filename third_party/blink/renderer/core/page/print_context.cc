...
void PrintContext::ComputePageRects(const gfx::SizeF& print_size)
{
    ...
    gfx::SizeF page_size = frame_->ResizePageRectsKeepingRatio(print_size, gfx::SizeF(document_rect.size));
    ComputePageRectsWithPageSizeInternal(page_size);
}

void PrintContext::ComputePageRectsWithPageSize(const gfx::SizeF& page_size_in_pixels)
{
    page_rects_.clear();
    ComputePageRectsWithPageSizeInternal(page_size_in_pixels);
}

void PrintContext::ComputePageRectsWithPageSizeInternal(const gfx::SizeF& page_size_in_pixels)
{
    if (!IsFrameValid())
        return;
    ...
    int doc_logical_height = is_horizontal ? snapped_doc_rect.height() : snapped_doc_rect.width();
    ...
    unsigned page_count = ceilf(static_cast<float>(doc_logical_height) / page_logical_height);
    for (unsigned i = 0; i < page_count; ++i) {
        int page_logical_top = block_direction_end > block_direction_start ? block_direction_start + i * page_logical_height
                                                                           : block_direction_start - (i + 1) * page_logical_height;

        int page_logical_left = inline_direction_end > inline_direction_start ? inline_direction_start : inline_direction_start - page_logical_width;
...
void PrintContext::BeginPrintMode(float width, float height)
{
    ...
    gfx::SizeF min_layout_size
        = frame_->ResizePageRectsKeepingRatio(original_page_size, gfx::SizeF(width * kPrintingMinimumShrinkFactor, height * kPrintingMinimumShrinkFactor));

    // This changes layout, so callers need to make sure that they don't paint to
    // screen while in printing mode.
    frame_->StartPrinting(min_layout_size, original_page_size, kPrintingMaximumShrinkFactor / kPrintingMinimumShrinkFactor);
}
...
int PrintContext::PageNumberForElement(Element* element, const gfx::SizeF& page_size_in_pixels)
{
    ...
    scaled_page_size.Scale(frame->View()->LayoutViewport()->ContentsSize().width() / page_rect.width());
...
void PrintContext::OutputLinkedDestinations(GraphicsContext& context, const gfx::Rect& page_rect)
{
...
// static
String PrintContext::PageProperty(LocalFrame* frame, const char* property_name, uint32_t page_number)
{
    ...
    scoped_refptr<const ComputedStyle> style = document->StyleForPage(page_number);
    ...
    if (!strcmp(property_name, "font-family")) {
        return ComputedStyleUtils::ValueForFontFamily(style->GetFontDescription().Family())->CssText();
    }
...
String PrintContext::PageSizeAndMarginsInPixels(
    LocalFrame* frame, uint32_t page_number, int width, int height, int margin_top, int margin_right, int margin_bottom, int margin_left)
{
    ...
    return "(" + String::Number(floor(description.size.width())) + ", " + String::Number(floor(description.size.height())) + ") "
        + String::Number(description.margin_top) + ' ' + String::Number(description.margin_right) + ' ' + String::Number(description.margin_bottom) + ' '
...
int PrintContext::NumberOfPages(LocalFrame* frame, const gfx::SizeF& page_size_in_pixels)
{
    ...
    scaled_page_size.Scale(layout_view->PageLogicalHeight() / (is_horizontal ? page_rect.height() : page_rect.width()));
    print_context->ComputePageRectsWithPageSize(scaled_page_size);
