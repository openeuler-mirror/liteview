...
DocumentsVector GetAllDocuments(Frame* main_frame, OnlyThrottledOrNot which_documents)
{
    DocumentsVector documents;
    for (Frame* frame = main_frame; frame; frame = frame->Tree().TraverseNext()) {
        if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
            Document* document = local_frame->GetDocument();
            if (which_documents == kAllDocuments || !document->View() || !document->View()->CanThrottleRendering())
...
void PageAnimator::ServiceScriptedAnimations(base::TimeTicks monotonic_animation_start_time)
{
    ...
    DocumentsVector documents = GetAllDocuments(page_->MainFrame(), kAllDocuments);
...
void PageAnimator::ReportFrameAnimations(cc::AnimationHost* animation_host)
{
    if (animation_host) {
        ...
        animation_host->SetHasSharedElementTransition(has_shared_element_transition_);
    }
...
void PageAnimator::SetHasSharedElementTransition(bool has_shared_element_transition)
{
...
void PageAnimator::UpdateLifecycleToPrePaintClean(LocalFrame& root_frame, DocumentUpdateReason reason)
{
    LocalFrameView* view = root_frame.View();
    base::AutoReset<bool> servicing(&updating_layout_and_style_for_painting_, true);
