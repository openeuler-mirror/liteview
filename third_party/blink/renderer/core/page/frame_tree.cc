...
Frame* FrameTree::FindFrameForNavigationInternal(const AtomicString& name, const KURL& url, FrameLoadRequest* request) const
{
    if (EqualIgnoringASCIICase(name, "_current")) {
        UseCounter::Count(blink::DynamicTo<blink::LocalFrame>(this_frame_.Get())->GetDocument(), WebFeature::kTargetCurrent);
    }
    ...
    if (EqualIgnoringASCIICase(name, "_unfencedTop")) {
        ...
        if (this_frame_.Get()->GetFencedFrameMode() == mojom::blink::FencedFrameMode::kOpaqueAds && request != nullptr) {
    ...
    if (EqualIgnoringASCIICase(name, "_parent")) {
        return Parent(FrameTreeBoundary::kFenced) ? Parent(FrameTreeBoundary::kFenced) : this_frame_.Get();
    }
    ...
    for (Frame* frame = this_frame_; frame; frame = frame->Tree().TraverseNext(this_frame_, FrameTreeBoundary::kFenced)) {
        if (frame->Tree().GetName() == name && To<LocalFrame>(this_frame_.Get())->CanNavigate(*frame, url)) {
            return frame;
        }
    }
    ...
    for (Frame *top = &this_frame_->Tree().Top(FrameTreeBoundary::kFenced), *frame = top; frame;
        ...
        if (frame->Tree().GetName() == name && !frame->Tree().IsDescendantOf(this_frame_.Get())
    ...
    for (const Page* other_page : page->RelatedPages()) {
        if (other_page == page || other_page->IsClosing())
            continue;
        for (Frame* frame = other_page->MainFrame(); frame; frame = frame->Tree().TraverseNext(nullptr, FrameTreeBoundary::kFenced)) {
            if (frame->Tree().GetName() == name && To<LocalFrame>(this_frame_.Get())->CanNavigate(*frame, url)) {
                return frame;
            }
        }
    ...
    // cluster access.
    DCHECK(!named_frame || local_frame->DomWindow()->GetSecurityOrigin()->IsGrantedCrossAgentClusterAccess());
    return named_frame;
}
...
DISABLE_CFI_PERF
Frame* FrameTree::TraverseNext(const Frame* stay_within, FrameTreeBoundary frame_tree_boundary) const
{
    ...
    while (!sibling && (!stay_within || frame->Tree().Parent(frame_tree_boundary) != stay_within)) {
