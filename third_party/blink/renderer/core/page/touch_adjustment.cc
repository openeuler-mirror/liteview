...
bool SnapTo(const SubtargetGeometry& geom, const gfx::Point& touch_point, const gfx::Rect& touch_area, gfx::Point& adjusted_point)
{
...
bool FindNodeWithLowestDistanceMetric(Node*& target_node, gfx::Point& target_point, gfx::Rect& target_area, const gfx::Point& touch_hotspot,
    const gfx::Rect& touch_area, SubtargetGeometryList& subtargets, DistanceFunction distance_function)
{
    target_node = nullptr;
    ...
    if (target_node) {
        target_area = target_node->GetDocument().View()->ConvertToRootFrame(target_area);
    }
...
bool FindBestClickableCandidate(
    Node*& target_node, gfx::Point& target_point, const gfx::Point& touch_hotspot, const gfx::Rect& touch_area, const HeapVector<Member<Node>>& nodes)
{
    gfx::Rect target_area;
    touch_adjustment::SubtargetGeometryList subtargets;
    touch_adjustment::CompileSubtargetList(nodes, subtargets, touch_adjustment::NodeRespondsToTapGesture, touch_adjustment::AppendBasicSubtargetsForNode);
    return touch_adjustment::FindNodeWithLowestDistanceMetric(
        target_node, target_point, target_area, touch_hotspot, touch_area, subtargets, touch_adjustment::HybridDistanceFunction);
}

bool FindBestContextMenuCandidate(
    Node*& target_node, gfx::Point& target_point, const gfx::Point& touch_hotspot, const gfx::Rect& touch_area, const HeapVector<Member<Node>>& nodes)
{
    gfx::Rect target_area;
    touch_adjustment::SubtargetGeometryList subtargets;
    touch_adjustment::CompileSubtargetList(nodes, subtargets, touch_adjustment::ProvidesContextMenuItems, touch_adjustment::AppendContextSubtargetsForNode);
    return touch_adjustment::FindNodeWithLowestDistanceMetric(
        target_node, target_point, target_area, touch_hotspot, touch_area, subtargets, touch_adjustment::HybridDistanceFunction);
}

LayoutSize GetHitTestRectForAdjustment(LocalFrame& frame, const LayoutSize& touch_area)
{
    ...
    const LayoutSize max_size_in_dip(touch_adjustment::kMaxAdjustmentSizeDip, touch_adjustment::kMaxAdjustmentSizeDip);

    const LayoutSize min_size_in_dip(touch_adjustment::kMinAdjustmentSizeDip, touch_adjustment::kMinAdjustmentSizeDip);
