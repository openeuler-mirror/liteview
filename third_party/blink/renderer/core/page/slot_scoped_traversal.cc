...
namespace {
Element* NextSkippingChildrenOfShadowHost(const Element& start, const Element& scope)
{
    ...
    for (const Element* current = &start; current != scope; current = current->parentElement()) {
...
Element* PreviousSkippingChildrenOfShadowHost(const Element& start, const Element& scope)
{
    DCHECK(scope.AssignedSlot());
...
{
    if (Element* nearest_inclusive_ancestor_assigned_to_slot = SlotScopedTraversal::NearestInclusiveAncestorAssignedToSlot(current))
        return nearest_inclusive_ancestor_assigned_to_slot->AssignedSlot();
    return nullptr;
}

Element* SlotScopedTraversal::NearestInclusiveAncestorAssignedToSlot(const Element& current)
{
...
{
    Element* nearest_inclusive_ancestor_assigned_to_slot = SlotScopedTraversal::NearestInclusiveAncestorAssignedToSlot(current);
    DCHECK(nearest_inclusive_ancestor_assigned_to_slot);
    // Search within children of an element which is assigned to a slot.
    if (Element* next = NextSkippingChildrenOfShadowHost(current, *nearest_inclusive_ancestor_assigned_to_slot))
        return next;

    // Seek to the next element assigned to the same slot.
    HTMLSlotElement* slot = nearest_inclusive_ancestor_assigned_to_slot->AssignedSlot();
    DCHECK(slot);
    const HeapVector<Member<Node>>& assigned_nodes = slot->AssignedNodes();
    wtf_size_t current_index = assigned_nodes.Find(*nearest_inclusive_ancestor_assigned_to_slot);
    DCHECK_NE(current_index, kNotFound);
    for (++current_index; current_index < assigned_nodes.size(); ++current_index) {
...
Element* SlotScopedTraversal::Previous(const Element& current)
{
    ...
    if (current != nearest_inclusive_ancestor_assigned_to_slot) {
        // Search within children of an element which is assigned to a slot.
        Element* previous = PreviousSkippingChildrenOfShadowHost(current, *nearest_inclusive_ancestor_assigned_to_slot);
        DCHECK(previous);
        return previous;
    }

    // Seek to the previous element assigned to the same slot.
    const HeapVector<Member<Node>>& assigned_nodes = nearest_inclusive_ancestor_assigned_to_slot->AssignedSlot()->AssignedNodes();
    wtf_size_t current_index = assigned_nodes.ReverseFind(*nearest_inclusive_ancestor_assigned_to_slot);
    DCHECK_NE(current_index, kNotFound);
...
{
    const HeapVector<Member<Node>>& assigned_nodes = slot.AssignedNodes();
    for (auto assigned_node = assigned_nodes.rbegin(); assigned_node != assigned_nodes.rend(); ++assigned_node) {
        auto* element = DynamicTo<Element>(assigned_node->Get());
        if (!element)
            continue;
        return LastWithinOrSelfSkippingChildrenOfShadowHost(*element);
