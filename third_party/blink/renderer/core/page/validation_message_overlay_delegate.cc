...
void ValidationMessageOverlayDelegate::CreatePage(const FrameOverlay& overlay)
{
    ...
    page_ = Page::CreateNonOrdinary(*chrome_client_, main_page_->GetPageScheduler()->GetAgentGroupScheduler());
    ...
    auto* frame = MakeGarbageCollected<LocalFrame>(MakeGarbageCollected<EmptyLocalFrameClient>(), *page_, nullptr, nullptr, nullptr,
        FrameInsertType::kInsertInConstructor, LocalFrameToken(), nullptr, nullptr);
    ...
    if (WebTestSupport::IsRunningWebTest()) {
        ...
        GetElementById("icon").SetInlineStyleProperty(CSSPropertyID::kTransition, "none");
...
void ValidationMessageOverlayDelegate::AdjustBubblePosition(const gfx::Rect& view_rect)
{
    ...
    if (show_bottom_arrow) {
        GetElementById("outer-arrow-bottom").SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        GetElementById("inner-arrow-bottom").SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        ...
        GetElementById("outer-arrow-top").SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        GetElementById("inner-arrow-top").SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
...
void ValidationMessageOverlayDelegate::StartToHide()
{
    ...
    if (!page_)
        return;
    GetElementById("container").classList().replace("shown-fully", "hiding", ASSERT_NO_EXCEPTION);
}
