...
public:
    ...
    void UpdateContextMenuDataForTesting(const ContextMenuData& data, const absl::optional<gfx::Point>& host_context_menu_location) override
    {
...
class ContextMenuControllerTest : public testing::Test, public ::testing::WithParamInterface<bool> {
public:
    ContextMenuControllerTest()
    {
        bool penetrating_image_selection_enabled = GetParam();
        feature_list_.InitWithFeatureState(features::kEnablePenetratingImageSelection, penetrating_image_selection_enabled);
    }
...
TEST_P(ContextMenuControllerTest, HitTestVideoChildElements)
{
    ...
    Persistent<HTMLVideoElement> video = MakeGarbageCollected<HTMLVideoElement>(*GetDocument());
    video->SetSrc(video_url);
    video->setAttribute(html_names::kStyleAttr, "position: absolute; left: 0; top: 0; width: 200px; height: 200px");
    GetDocument()->body()->AppendChild(video);
    test::RunPendingTasks();
    SetReadyState(video.Get(), HTMLMediaElement::kHaveMetadata);
    test::RunPendingTasks();
...
TEST_P(ContextMenuControllerTest, ShowNonLocatedContextMenuEvent)
{
    ...
    LocalMainFrame()->LocalRootFrameWidget()->ShowContextMenu(ui::mojom::MenuSourceType::TOUCH_HANDLE, middle_point);
    ...
    LocalMainFrame()->LocalRootFrameWidget()->ShowContextMenu(ui::mojom::MenuSourceType::TOUCH_HANDLE, gfx::Point(rect->right() / 2, rect->bottom() / 2));
...
TEST_P(ContextMenuControllerTest, ValidateNonLocatedContextMenuOnLargeImageElement)
{
...
{
    if (base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageHitTestStandardImageSelectionExperiment)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
// the image.
TEST_P(ContextMenuControllerTest, ContextMenuImageHitTestContextMenuListenerAboveImageBlocking)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageHitTestContextMenuListenerOnImageBlocking)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageHitTestStandardContextMenuListenerAncestorNonBlocking)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageHitTestContextMenuListenerAncestorBlocking)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
// conducted.
TEST_P(ContextMenuControllerTest, ContextMenuImageRetrievalCachedImageNotFound)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageRetrievalAfterCachedImageReset)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, ContextMenuImageRetrievalCachedImageCrossFrame)
{
    if (!base::FeatureList::IsEnabled(features::kEnablePenetratingImageSelection)) {
        return;
    }
...
TEST_P(ContextMenuControllerTest, KeyboardTriggeredContextMenuPreservesSelection)
{
    ContextMenuAllowedScope context_menu_allowed_scope;
...
TEST_P(ContextMenuControllerTest, CheckRendererIdFromContextMenuOnTextField)
{
    ...
    // input_field_type
    std::vector<std::tuple<AtomicString, bool, bool, mojom::ContextMenuDataInputFieldType>> expectations = { // Input Text Field
        { "name", true, true, mojom::ContextMenuDataInputFieldType::kPlainText },
        // Text Area Field
        { "address", true, true, mojom::ContextMenuDataInputFieldType::kPlainText },
        // Non form element
        { "one", false, false, mojom::ContextMenuDataInputFieldType::kNone },
        // Formless Input field
        { "two", false, true, mojom::ContextMenuDataInputFieldType::kPlainText },
        // Formless text area field
        { "three", false, true, mojom::ContextMenuDataInputFieldType::kPlainText }
    };

    for (const auto& expectation : expectations) {
        auto [field_id, is_form_renderer_id_present, is_field_renderer_id_present, input_field_type] = expectation;
        Element* form_element = document->getElementById(field_id);
        EXPECT_TRUE(ShowContextMenuForElement(form_element, kMenuSourceMouse));
        ContextMenuData context_menu_data = GetWebFrameClient().GetContextMenuData();
        EXPECT_EQ(context_menu_data.form_renderer_id.has_value(), is_form_renderer_id_present);
        EXPECT_EQ(context_menu_data.field_renderer_id.has_value(), is_field_renderer_id_present);
...
class ContextMenuControllerRemoteParentFrameTest : public testing::Test, public ::testing::WithParamInterface<bool> {
public:
    ...
    {
        child_frame_->LocalRootFrameWidget()->ShowContextMenu(ui::mojom::MenuSourceType::MOUSE, point);
        base::RunLoop().RunUntilIdle();
    }
...
INSTANTIATE_TEST_SUITE_P(, ContextMenuControllerRemoteParentFrameTest, ::testing::Bool());
...
TEST_P(ContextMenuControllerRemoteParentFrameTest, ShowContextMenuInChild)
{
    ...
    const absl::optional<gfx::Point>& host_context_menu_location = child_web_frame_client().host_context_menu_location();
