...
Page* Page::CreateNonOrdinary(ChromeClient& chrome_client, scheduler::WebAgentGroupScheduler& agent_group_scheduler)
{
    return MakeGarbageCollected<Page>(base::PassKey<Page>(), chrome_client, agent_group_scheduler,
        /*is_ordinary=*/false);
}

Page* Page::CreateOrdinary(ChromeClient& chrome_client, Page* opener, scheduler::WebAgentGroupScheduler& agent_group_scheduler)
{
    Page* page = MakeGarbageCollected<Page>(base::PassKey<Page>(), chrome_client, agent_group_scheduler,
        /*is_ordinary=*/true);
...
Page::Page(base::PassKey<Page>, ChromeClient& chrome_client, scheduler::WebAgentGroupScheduler& agent_group_scheduler, bool is_ordinary)
    : SettingsDelegate(std::make_unique<Settings>())
    , main_frame_(nullptr)
    , fenced_frames_impl_(features::IsFencedFramesEnabled()
              ? absl::optional<features::FencedFramesImplementationType>(features::kFencedFramesImplementationTypeParam.Get())
              : absl::nullopt)
    ...
    , overscroll_controller_(MakeGarbageCollected<OverscrollController>(GetVisualViewport(), GetChromeClient()))
    ...
    , web_text_autosizer_page_info_({ 0, 0, 1.f })
{
    ...
    page_scheduler_ = agent_group_scheduler.AsAgentGroupScheduler().CreatePageScheduler(this);
...
{
    for (const Page* page : AllPages()) {
        if (auto* svg_image_chrome_client = DynamicTo<SVGImageChromeClient>(page->GetChromeClient()))
...
void Page::SetVisibilityState(mojom::blink::PageVisibilityState visibility_state, bool is_initial_state)
{
    ...
    page_visibility_observer_set_.ForEachObserver([](PageVisibilityObserver* observer) { observer->PageVisibilityChanged(); });
...
void Page::SettingsChanged(ChangeType change_type)
{
    ...
    case ChangeType::kImageLoading:
        for (Frame* frame = MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
                local_frame->GetDocument()->Fetcher()->SetImagesEnabled(GetSettings().GetImagesEnabled());
    ...
    case ChangeType::kAccessibilityState:
        if (!MainFrame() || !MainFrame()->IsLocalFrame())
            break;
        DeprecatedLocalMainFrame()->GetDocument()->AXObjectCacheOwner().ClearAXObjectCache();
        break;
    ...
    case ChangeType::kSpatialNavigation:
        if (spatial_navigation_controller_ || GetSettings().GetSpatialNavigationEnabled()) {
...
void Page::WillBeDestroyed()
{
    ...
    page_visibility_observer_set_.ForEachObserver([](PageVisibilityObserver* observer) { observer->ObserverSetWillBeCleared(); });
...
void Page::ReportIntervention(const String& text)
{
    if (LocalFrame* local_frame = DeprecatedLocalMainFrame()) {
        auto* message = MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, text,
