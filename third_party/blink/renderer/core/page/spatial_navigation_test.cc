...
TEST_F(SpatialNavigationTest, HasRemoteFrame)
{
    ...
    Element* iframe = webview->MainFrameImpl()->GetFrame()->GetDocument()->getElementById("iframe");
...
class SpatialNavigationWithFocuslessModeTest : public SpatialNavigationTest, public ::testing::WithParamInterface<bool> {
public:
    SpatialNavigationWithFocuslessModeTest()
        : use_focusless_mode_(GetParam())
    {
    }
...
INSTANTIATE_TEST_SUITE_P(All, SpatialNavigationWithFocuslessModeTest, ::testing::Bool());
...
public:
    FocuslessSpatialNavigationSimTest()
        : use_focusless_mode_(true)
    {
    }
    ...
    {
        WebKeyboardEvent event { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
        event.dom_key = dom_key;
        WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(event, ui::LatencyInfo()));
...
TEST_F(FocuslessSpatialNavigationSimTest, OpenSelectPopup)
{
    ...
    SpatialNavigationController& spat_nav_controller = GetDocument().GetPage()->GetSpatialNavigationController();
