...
OffscreenCanvas::OffscreenCanvas(ExecutionContext* context, const gfx::Size& size)
    : CanvasRenderingContextHost(CanvasRenderingContextHost::HostType::kOffscreenCanvasHost)
...
OffscreenCanvas* OffscreenCanvas::Create(ExecutionContext* context, unsigned width, unsigned height)
{
    UMA_HISTOGRAM_BOOLEAN("Blink.OffscreenCanvas.NewOffscreenCanvas", true);
    return MakeGarbageCollected<OffscreenCanvas>(context, gfx::Size(ClampTo<int>(width), ClampTo<int>(height)));
}
...
void OffscreenCanvas::Commit(scoped_refptr<CanvasResource>&& canvas_resource, const SkIRect& damage_rect)
{
    ...
    GetOrCreateResourceDispatcher()->DispatchFrameSync(std::move(canvas_resource), commit_start_time, current_frame_damage_rect_,
        !RenderingContext()->IsOriginTopLeft() /* needs_vertical_flip */, IsOpaque());
...
ScriptPromise OffscreenCanvas::convertToBlob(ScriptState* script_state, const ImageEncodeOptions* options, ExceptionState& exception_state)
{
...
scoped_refptr<Image> OffscreenCanvas::GetSourceImageForCanvas(SourceImageStatus* status, const gfx::SizeF& size, const AlphaDisposition alpha_disposition)
{
    if (!context_) {
    ...
    ScriptState* script_state, absl::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options, ExceptionState& exception_state)
{
    ...
    return ImageBitmapSource::FulfillImageBitmap(
        script_state, IsPaintable() ? MakeGarbageCollected<ImageBitmap>(this, crop_rect, options) : nullptr, exception_state);
}
...
CanvasRenderingContext* OffscreenCanvas::GetCanvasRenderingContext(
    ExecutionContext* execution_context, const String& id, const CanvasContextCreationAttributesCore& attributes)
{
    ...
    CanvasRenderingContext::CanvasRenderingAPI rendering_api = CanvasRenderingContext::RenderingAPIFromId(id, execution_context);
...
CanvasResourceDispatcher* OffscreenCanvas::GetOrCreateResourceDispatcher()
{
    ...
    if (!frame_dispatcher_) {
        ...
        frame_dispatcher_ = std::make_unique<CanvasResourceDispatcher>(
            this, std::move(agent_group_scheduler_compositor_task_runner), client_id_, sink_id_, placeholder_canvas_id_, size_);
...
CanvasResourceProvider* OffscreenCanvas::GetOrCreateResourceProvider()
{
    ...
    const bool can_use_gpu = SharedGpuContext::IsGpuCompositingEnabled()
        && (IsWebGL() || IsWebGPU()
            || (IsRenderingContext2D() && RuntimeEnabledFeatures::Accelerated2dCanvasEnabled() && !context_->CreationAttributes().will_read_frequently));
    const bool composited_mode = IsWebGPU() || (IsWebGL() && RuntimeEnabledFeatures::WebGLImageChromiumEnabled())
    ...
    if (can_use_gpu) {
        provider = CanvasResourceProvider::CreateSharedImageProvider(resource_info, filter_quality, CanvasResourceProvider::ShouldInitialize::kCallClear,
            SharedGpuContext::ContextProviderWrapper(), RasterMode::kGPU, false /*is_origin_top_left*/, shared_image_usage_flags);
        ...
        provider = CanvasResourceProvider::CreateSharedImageProvider(resource_info, filter_quality, CanvasResourceProvider::ShouldInitialize::kCallClear,
            SharedGpuContext::ContextProviderWrapper(), RasterMode::kCPU, false /*is_origin_top_left*/, shared_image_usage_flags);
    }
    ...
    if (!provider && HasPlaceholderCanvas()) {
        ...
        provider = CanvasResourceProvider::CreateSharedBitmapProvider(
            resource_info, filter_quality, CanvasResourceProvider::ShouldInitialize::kCallClear, std::move(dispatcher_weakptr));
    }

    if (!provider) {
        // If any of the above Create was able to create a valid provider, a
        // BitmapProvider will be created here.
        provider = CanvasResourceProvider::CreateBitmapProvider(resource_info, filter_quality, CanvasResourceProvider::ShouldInitialize::kCallClear);
    }
...
bool OffscreenCanvas::PushFrame(scoped_refptr<CanvasResource>&& canvas_resource, const SkIRect& damage_rect)
{
    ...
    GetOrCreateResourceDispatcher()->DispatchFrame(std::move(canvas_resource), commit_start_time, current_frame_damage_rect_,
        !RenderingContext()->IsOriginTopLeft() /* needs_vertical_flip */, IsOpaque());
    current_frame_damage_rect_ = SkIRect::MakeEmpty();
    return true;
}
