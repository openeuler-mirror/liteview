...
class ComputedStyle : public ComputedStyleBase, public RefCounted<ComputedStyle> {
...
protected:
    ...
    PositionFallbackStyleCache& EnsurePositionFallbackStyleCache(unsigned ensure_size) const;
...
public:
    ...
    StyleSelfAlignmentData ResolvedAlignItems(ItemPosition normal_value_behaviour) const;
    StyleSelfAlignmentData ResolvedAlignSelf(ItemPosition normal_value_behaviour, const ComputedStyle* parent_style = nullptr) const;
    StyleContentAlignmentData ResolvedAlignContent(const StyleContentAlignmentData& normal_behaviour) const;
    StyleSelfAlignmentData ResolvedJustifyItems(ItemPosition normal_value_behaviour) const;
    StyleSelfAlignmentData ResolvedJustifySelf(ItemPosition normal_value_behaviour, const ComputedStyle* parent_style = nullptr) const;
    ...
    CORE_EXPORT void InheritFrom(const ComputedStyle& inherit_parent, IsAtShadowBoundary = kNotAtShadowBoundary);
    ...
    void SetStyleType(PseudoId style_type)
    {
        SetStyleTypeInternal(style_type);
    }
    ...
    const ComputedStyle* AddCachedPseudoElementStyle(scoped_refptr<const ComputedStyle>, PseudoId, const AtomicString&) const;
    ...
    const ComputedStyle* AddCachedPositionFallbackStyle(scoped_refptr<const ComputedStyle>, unsigned index) const;
    ...
    {
        return base::ValuesEquivalent(BackdropFilterInternal(), o.BackdropFilterInternal());
    }
    ...
    {
        if (BorderTopStyle() == EBorderStyle::kNone || BorderTopStyle() == EBorderStyle::kHidden)
            return LayoutUnit(0);
        return BorderTopWidthInternal();
    }
    void SetBorderTopWidth(float v)
    {
        SetBorderTopWidthInternal(LayoutUnit(v));
    }

    // border-bottom-width
    LayoutUnit BorderBottomWidth() const
    {
        if (BorderBottomStyle() == EBorderStyle::kNone || BorderBottomStyle() == EBorderStyle::kHidden)
            return LayoutUnit(0);
    ...
    {
        if (BorderLeftStyle() == EBorderStyle::kNone || BorderLeftStyle() == EBorderStyle::kHidden)
            return LayoutUnit(0);
    ...
    {
        if (BorderRightStyle() == EBorderStyle::kNone || BorderRightStyle() == EBorderStyle::kHidden)
            return LayoutUnit(0);
    ...
    {
        SetBoxOrdinalGroupInternal(std::min(std::numeric_limits<unsigned>::max() - 1, og));
    }
    ...
    bool OpacityChangedStackingContext(const ComputedStyle& other) const
    {
        ...
        if (IsStackingContextWithoutContainment() == other.IsStackingContextWithoutContainment() || HasOpacity() == other.HasOpacity()) {
    ...
    bool OutlineVisuallyEqual(const ComputedStyle& other) const
    {
        if (OutlineStyle() == EBorderStyle::kNone && other.OutlineStyle() == EBorderStyle::kNone)
            return true;
        return OutlineWidthInternal() == other.OutlineWidthInternal() && OutlineColor() == other.OutlineColor() && OutlineStyle() == other.OutlineStyle()
            && OutlineOffset() == other.OutlineOffset() && OutlineStyleIsAuto() == other.OutlineStyleIsAuto();
    }
    ...
    {
        return OutlineStyleIsAuto() ? NGOutlineType::kIncludeBlockVisualOverflow : NGOutlineType::kDontIncludeBlockVisualOverflow;
    }

    // -webkit-perspective-origin-x
    const Length& PerspectiveOriginX() const
    {
        return PerspectiveOrigin().X();
    }
    ...
    {
        return GetTransformOrigin().X();
    }
    void SetTransformOriginX(const Length& v)
    {
        SetTransformOrigin(TransformOrigin(v, TransformOriginY(), TransformOriginZ()));
    }

    // -webkit-transform-origin-y
    const Length& TransformOriginY() const
    {
        return GetTransformOrigin().Y();
    }
    void SetTransformOriginY(const Length& v)
    {
        SetTransformOrigin(TransformOrigin(TransformOriginX(), v, TransformOriginZ()));
    }

    // -webkit-transform-origin-z
    float TransformOriginZ() const
    {
        return GetTransformOrigin().Z();
    }
    ...
    {
        return HasPseudoElementStyle(kPseudoIdScrollbar) && ScrollbarWidth() == EScrollbarWidth::kAuto;
    }
    ...
    CORE_EXPORT void SetFont(const Font& font)
    {
    ...
    CORE_EXPORT void SetVariableData(const AtomicString&, scoped_refptr<CSSVariableData>, bool is_inherited_property);
    CORE_EXPORT void SetVariableValue(const AtomicString&, const CSSValue*, bool is_inherited_property);
    ...
    {
        return !ColumnRuleColorInternal().Resolve(GetCurrentColor(), UsedColorScheme()).Alpha();
    }
    ...
    {
        if (IsDeprecatedWebkitBox()) {
            return BoxOrient() == EBoxOrient::kVertical && BoxDirection() == EBoxDirection::kReverse;
        }
    ...
    bool ResolvedIsRowFlexDirection() const
    {
        ...
        return FlexDirection() == EFlexDirection::kRow || FlexDirection() == EFlexDirection::kRowReverse;
    }
    ...
    {
        return MarginBeforeUsing(*this);
    }
    const Length& MarginAfter() const
    {
        return MarginAfterUsing(*this);
    }
    const Length& MarginStart() const
    {
        return MarginStartUsing(*this);
    }
    const Length& MarginEnd() const
    {
        return MarginEndUsing(*this);
    }
    ...
    {
        return MarginTop() == other.MarginTop() && MarginLeft() == other.MarginLeft() && MarginRight() == other.MarginRight()
    ...
    {
        return BorderValue(BorderLeftStyle(), BorderLeftColor(), BorderLeftWidthInternal());
    }
    const BorderValue BorderRight() const
    {
        return BorderValue(BorderRightStyle(), BorderRightColor(), BorderRightWidthInternal());
    }
    const BorderValue BorderTop() const
    {
        return BorderValue(BorderTopStyle(), BorderTopColor(), BorderTopWidthInternal());
    }
    const BorderValue BorderBottom() const
    {
        return BorderValue(BorderBottomStyle(), BorderBottomColor(), BorderBottomWidthInternal());
    }
    ...
    {
        return BorderBeforeUsing(*this);
    }
    BorderValue BorderAfter() const
    {
        return BorderAfterUsing(*this);
    }
    BorderValue BorderStart() const
    {
        return BorderStartUsing(*this);
    }
    BorderValue BorderEnd() const
    {
        return BorderEndUsing(*this);
    }
    ...
    {
        return HasBorder() || HasBorderFill();
    }
    ...
    {
        return BorderTopLeftRadius() == o.BorderTopLeftRadius() && BorderTopRightRadius() == o.BorderTopRightRadius()
            && BorderBottomLeftRadius() == o.BorderBottomLeftRadius() && BorderBottomRightRadius() == o.BorderBottomRightRadius();
    }

    bool BorderLeftEquals(const ComputedStyle& o) const
    {
        return BorderLeftWidthInternal() == o.BorderLeftWidthInternal() && BorderLeftStyle() == o.BorderLeftStyle()
            && ResolvedColor(BorderLeftColor()) == o.ResolvedColor(o.BorderLeftColor());
    }
    bool BorderLeftEquals(const BorderValue& o) const
    {
        return BorderLeftWidthInternal() == o.Width() && BorderLeftStyle() == o.Style() && BorderLeftColor() == o.GetColor();
    }

    bool BorderLeftVisuallyEqual(const ComputedStyle& o) const
    {
        if (BorderLeftStyle() == EBorderStyle::kNone && o.BorderLeftStyle() == EBorderStyle::kNone)
            return true;
        if (BorderLeftStyle() == EBorderStyle::kHidden && o.BorderLeftStyle() == EBorderStyle::kHidden)
            return true;
    ...
    {
        return BorderRightWidthInternal() == o.BorderRightWidthInternal() && BorderRightStyle() == o.BorderRightStyle()
            && ResolvedColor(BorderRightColor()) == o.ResolvedColor(o.BorderRightColor());
    }
    bool BorderRightEquals(const BorderValue& o) const
    {
        return BorderRightWidthInternal() == o.Width() && BorderRightStyle() == o.Style() && BorderRightColor() == o.GetColor();
    }

    bool BorderRightVisuallyEqual(const ComputedStyle& o) const
    {
        if (BorderRightStyle() == EBorderStyle::kNone && o.BorderRightStyle() == EBorderStyle::kNone)
            return true;
        if (BorderRightStyle() == EBorderStyle::kHidden && o.BorderRightStyle() == EBorderStyle::kHidden)
            return true;
    ...
    {
        if (BorderTopStyle() == EBorderStyle::kNone && o.BorderTopStyle() == EBorderStyle::kNone)
            return true;
        if (BorderTopStyle() == EBorderStyle::kHidden && o.BorderTopStyle() == EBorderStyle::kHidden)
            return true;
    ...
    {
        return BorderTopWidthInternal() == o.BorderTopWidthInternal() && BorderTopStyle() == o.BorderTopStyle()
            && ResolvedColor(BorderTopColor()) == o.ResolvedColor(o.BorderTopColor());
    }
    bool BorderTopEquals(const BorderValue& o) const
    {
        return BorderTopWidthInternal() == o.Width() && BorderTopStyle() == o.Style() && BorderTopColor() == o.GetColor();
    }

    bool BorderBottomVisuallyEqual(const ComputedStyle& o) const
    {
        if (BorderBottomStyle() == EBorderStyle::kNone && o.BorderBottomStyle() == EBorderStyle::kNone)
            return true;
        if (BorderBottomStyle() == EBorderStyle::kHidden && o.BorderBottomStyle() == EBorderStyle::kHidden)
            return true;
    ...
    {
        return BorderBottomWidthInternal() == o.BorderBottomWidthInternal() && BorderBottomStyle() == o.BorderBottomStyle()
            && ResolvedColor(BorderBottomColor()) == o.ResolvedColor(o.BorderBottomColor());
    }
    bool BorderBottomEquals(const BorderValue& o) const
    {
        return BorderBottomWidthInternal() == o.Width() && BorderBottomStyle() == o.Style() && BorderBottomColor() == o.GetColor();
    }

    bool BorderEquals(const ComputedStyle& o) const
    {
        return BorderLeftEquals(o) && BorderRightEquals(o) && BorderTopEquals(o) && BorderBottomEquals(o) && BorderImage() == o.BorderImage();
    }

    bool BorderVisuallyEqual(const ComputedStyle& o) const
    {
        return BorderLeftVisuallyEqual(o) && BorderRightVisuallyEqual(o) && BorderTopVisuallyEqual(o) && BorderBottomVisuallyEqual(o)
    ...
    CORE_EXPORT void AdjustDiffForBackgroundVisuallyEqual(const ComputedStyle& o, StyleDifference& diff) const;
    ...
    {
        SetBorderRadius(LengthSize(Length::Fixed(s.width()), Length::Fixed(s.height())));
    }
    ...
    {
        return FloatingInternal() != EFloat::kNone;
    }
    EFloat UnresolvedFloating() const
    {
        return FloatingInternal();
    }
    ...
    {
        return GetPosition() == EPosition::kAbsolute || GetPosition() == EPosition::kFixed;
    }
    ...
    EPosition GetPosition() const
    {
        ...
        if (PositionInternal() == EPosition::kSticky && (Display() == EDisplay::kTableColumnGroup || Display() == EDisplay::kTableColumn)) {
    ...
    bool HasClear() const
    {
        return ClearInternal() != EClear::kNone;
    }
    ...
    {
        return (Contain() & kContainsStyle) || IsInlineOrBlockSizeContainer() || !IsContentVisibilityVisible();
    }
    bool ContainsLayout() const
    {
        return (Contain() & kContainsLayout) || IsInlineOrBlockSizeContainer() || !IsContentVisibilityVisible();
    }
    bool ContainsSize() const
    {
        return ((Contain() & kContainsSize) == kContainsSize) || IsSizeContainer() || SkipsContents();
    }
    bool ContainsInlineSize() const
    {
        return (Contain() & kContainsInlineSize) || IsInlineSizeContainer() || SkipsContents();
    }
    bool ContainsBlockSize() const
    {
        return (Contain() & kContainsBlockSize) || IsBlockSizeContainer() || SkipsContents();
    }
    ...
    {
        return ContentVisibility() == EContentVisibility::kVisible && ToggleVisibility().IsNull();
    }
    ...
    CORE_EXPORT void AddCursor(StyleImage*, bool hot_spot_specified, const gfx::Point& hot_spot = gfx::Point());
    ...
    EResize Resize(const ComputedStyle& cb_style) const
    {
        ...
        switch (value) {
        case EResize::kBlock:
            return cb_style.IsHorizontalWritingMode() ? EResize::kVertical : EResize::kHorizontal;
        case EResize::kInline:
            return cb_style.IsHorizontalWritingMode() ? EResize::kHorizontal : EResize::kVertical;
        default:
            return value;
        }
    }
    ...
    {
        return !IsInert() && !(UserSelectInternal() == EUserSelect::kNone && UserModifyInternal() == EUserModify::kReadOnly);
    }
    ...
    bool IsFocusable() const
    {
        ...
        return !IsEnsuredInDisplayNone() && Display() != EDisplay::kContents && !IsInert() && Visibility() == EVisibility::kVisible;
    }

    // Text decoration utility functions.
    bool TextDecorationVisualOverflowEqual(const ComputedStyle& o) const;
    void ApplyTextDecorations(const Color& parent_text_decoration_color, bool override_existing_colors);
    void ClearAppliedTextDecorations();
    void RestoreParentTextDecorations(const ComputedStyle& parent_style);
    CORE_EXPORT const Vector<AppliedTextDecoration>& AppliedTextDecorations() const;
    ...
    {
        return OverflowX() == EOverflow::kAuto || OverflowX() == EOverflow::kOverlay;
    }

    bool HasAutoVerticalScroll() const
    {
        return OverflowY() == EOverflow::kAuto || OverflowY() == EOverflow::kOverlay;
    }
    ...
    bool IsFixedTableLayout() const
    {
        ...
        return TableLayout() == ETableLayout::kFixed && (LogicalWidth().IsSpecified() || LogicalWidth().IsMinContent() || LogicalWidth().IsFitContent());
    }
    ...
    {
        return HasGroupingPropertyForUsedTransformStyle3D() ? ETransformStyle3D::kFlat : TransformStyle3D();
    }
    ...
    void ApplyTransform(TransformationMatrix&, const LayoutSize& border_box_data_size, ApplyTransformOperations, ApplyTransformOrigin, ApplyMotionPath,
        ApplyIndependentTransformProperties) const;
    void ApplyTransform(TransformationMatrix&, const gfx::RectF& bounding_box, ApplyTransformOperations, ApplyTransformOrigin, ApplyMotionPath,
        ApplyIndependentTransformProperties) const;
    ...
    CORE_EXPORT void UpdateIsStackingContextWithoutContainment(bool is_document_element, bool is_in_top_layer, bool is_svg_stacking);
    ...
    static bool IsNot(EWhiteSpace a, EWhiteSpace b)
    {
        return !Is(a, b);
    }
    ...
    static bool CollapseWhiteSpace(EWhiteSpace ws)
    {
        ...
        return IsNot(ws, EWhiteSpace::kPre | EWhiteSpace::kPreWrap | EWhiteSpace::kBreakSpaces);
    }
    ...
    {
        return Is(WhiteSpace(), EWhiteSpace::kPreWrap | EWhiteSpace::kBreakSpaces) || GetLineBreak() == LineBreak::kAfterWhiteSpace;
    }
    ...
    {
        return (WordBreak() == EWordBreak::kBreakWord || OverflowWrap() != EOverflowWrap::kNormal)
    ...
    void GetBorderEdgeInfo(BorderEdge edges[], PhysicalBoxSides sides_to_include = PhysicalBoxSides()) const;
    ...
    CORE_EXPORT Color VisitedDependentColor(const CSSProperty& color_property, bool* is_current_color = nullptr) const;
    ...
    {
        return HasEffectiveAppearance() && (EffectiveAppearance() == kCheckboxPart || EffectiveAppearance() == kRadioPart);
    }
    ...
    {
        return DarkColorScheme() ? mojom::blink::ColorScheme::kDark : mojom::blink::ColorScheme::kLight;
    }
    ...
    {
        return Display() == EDisplay::kListItem && ListStyleImage() && !ListStyleImage()->ErrorOccurred();
    }
...
private:
    ...
    static bool IsDisplayBlockContainer(EDisplay display)
    {
        return display == EDisplay::kBlock || display == EDisplay::kListItem || display == EDisplay::kInlineBlock || display == EDisplay::kFlowRoot
            || display == EDisplay::kTableCell || display == EDisplay::kTableCaption;
    }
    ...
    {
        return display == EDisplay::kInlineBlock || display == EDisplay::kWebkitInlineBox || display == EDisplay::kInlineFlex
            || display == EDisplay::kInlineTable || display == EDisplay::kInlineGrid || display == EDisplay::kMath || display == EDisplay::kInlineLayoutCustom;
    }
    ...
    {
        return ColorIsCurrentColorInternal();
    }
    ...
    {
        return OutlineColorInternal();
    }
    const StyleColor& StopColor() const
    {
        return StopColorInternal();
    }
    ...
    bool InternalVisitedBorderLeftColorHasNotChanged(const ComputedStyle& other) const
    {
        return (InternalVisitedBorderLeftColor() == other.InternalVisitedBorderLeftColor() || !BorderLeftWidth());
    }
    const StyleColor& InternalVisitedBorderRightColor() const
    {
        return InternalVisitedBorderRightColorInternal();
    }
    bool InternalVisitedBorderRightColorHasNotChanged(const ComputedStyle& other) const
    {
        return (InternalVisitedBorderRightColor() == other.InternalVisitedBorderRightColor() || !BorderRightWidth());
    }
    const StyleColor& InternalVisitedBorderBottomColor() const
    {
        return InternalVisitedBorderBottomColorInternal();
    }
    bool InternalVisitedBorderBottomColorHasNotChanged(const ComputedStyle& other) const
    {
        return (InternalVisitedBorderBottomColor() == other.InternalVisitedBorderBottomColor() || !BorderBottomWidth());
    }
    const StyleColor& InternalVisitedBorderTopColor() const
    {
        return InternalVisitedBorderTopColorInternal();
    }
    bool InternalVisitedBorderTopColorHasNotChanged(const ComputedStyle& other) const
    {
        return (InternalVisitedBorderTopColor() == other.InternalVisitedBorderTopColor() || !BorderTopWidth());
    }
    const StyleColor& InternalVisitedOutlineColor() const
    {
        return InternalVisitedOutlineColorInternal();
    }
    bool InternalVisitedOutlineColorHasNotChanged(const ComputedStyle& other) const
    {
        return (InternalVisitedOutlineColor() == other.InternalVisitedOutlineColor() || !OutlineWidth());
    }
    ...
    void ApplyMotionPathTransform(float origin_x, float origin_y, const gfx::RectF& bounding_box, TransformationMatrix&) const;

    bool ScrollAnchorDisablingPropertyChanged(const ComputedStyle& other, const StyleDifference&) const;
    ...
    void AdjustDiffForNeedsPaintInvalidation(const ComputedStyle& other, StyleDifference&, const Document&) const;
    ...
    CORE_EXPORT void UpdatePropertySpecificDifferences(const ComputedStyle& other, StyleDifference&) const;
    ...
    Color GetInternalForcedVisitedCurrentColor(bool* is_current_color = nullptr) const;
    ...
    Color ResolvedColor(const StyleColor& color, bool* is_current_color = nullptr) const;
    ...
    PhysicalToLogical<BorderValue> PhysicalBorderToLogical(const ComputedStyle& other) const
    {
        return PhysicalToLogical<BorderValue>(other.GetWritingDirection(), BorderTop(), BorderRight(), BorderBottom(), BorderLeft());
    }

    PhysicalToLogical<LayoutUnit> PhysicalBorderWidthToLogical() const
    {
        return PhysicalToLogical<LayoutUnit>(GetWritingDirection(), BorderTopWidth(), BorderRightWidth(), BorderBottomWidth(), BorderLeftWidth());
    }
    ...
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesRespectsTransformAnimation);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsTransform);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesRespectsScaleAnimation);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesRespectsRotateAnimation);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesRespectsTranslateAnimation);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsOpacity);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsFilter);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsBackdropFilter);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsInlineTransform);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsBackfaceVisibility);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsWillChange);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsUsedStylePreserve3D);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsOverflow);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsContainsPaint);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, UpdatePropertySpecificDifferencesHasAlpha);
    ...
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, InitialAndInheritedAndNonInheritedVariableNames);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, GetVariableNamesWithInitialData_Invalidation);
    FRIEND_TEST_ALL_PREFIXES(StyleCascadeTest, ForcedVisitedBackgroundColor);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, TextDecorationEqualDoesNotRequireRecomputeInkOverflow);
    FRIEND_TEST_ALL_PREFIXES(ComputedStyleTest, TextDecorationNotEqualRequiresRecomputeInkOverflow);
...
inline void ComputedStyle::SetHasPseudoElementStyle(PseudoId pseudo)
{
    ...
    SetPseudoBitsInternal(PseudoBitsInternal() | 1 << (pseudo - kFirstPublicPseudoId));
}
