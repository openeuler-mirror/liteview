...
TEST_F(ComputedStyleTest, ClipPathEqual)
{
    ...
    scoped_refptr<ShapeClipPathOperation> path1 = ShapeClipPathOperation::Create(shape);
    scoped_refptr<ShapeClipPathOperation> path2 = ShapeClipPathOperation::Create(shape);
    scoped_refptr<ComputedStyle> style1 = CreateComputedStyle();
    scoped_refptr<ComputedStyle> style2 = CreateComputedStyle();
...
TEST_F(ComputedStyleTest, UpdatePropertySpecificDifferencesCompositingReasonsTransform)
{
    ...
    operations.Operations().push_back(ScaleTransformOperation::Create(1, 1, TransformOperation::kScale));
...
#define TEST_ANIMATION_FLAG(flag, inherited)                                                                                                                   \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(ComputedStyle::Difference::inherited, ComputedStyle::ComputeDifference(style.get(), other.get()));                                           \
...
#define TEST_ANIMATION_FLAG_NO_DIFF(flag)                                                                                                                      \
    do {                                                                                                                                                       \
        ...
        style->Set##flag(true);                                                                                                                                \
        EXPECT_TRUE(style->flag());                                                                                                                            \
        EXPECT_EQ(ComputedStyle::Difference::kEqual, ComputedStyle::ComputeDifference(style.get(), other.get()));                                              \
        auto diff = style->VisualInvalidationDiff(*document, *other);                                                                                          \
...
TEST_F(ComputedStyleTest, CustomPropertiesEqual_Values)
{
    ...
    css_test_helpers::RegisterProperty(dummy->GetDocument(), "--x", "<length>", "0px", false);
...
{
    auto dummy = std::make_unique<DummyPageHolder>(gfx::Size(0, 0));
    css_test_helpers::RegisterProperty(dummy->GetDocument(), "--x", "<length>", "0px", true);
    ...
    EXPECT_EQ(ComputedStyle::Difference::kIndependentInherited, ComputedStyle::ComputeDifference(old_style.get(), new_style.get()));
...
TEST_F(ComputedStyleTest, CustomPropertiesInheritance_StyleRecalc)
{
    ...
    EXPECT_EQ(ComputedStyle::Difference::kInherited, ComputedStyle::ComputeDifference(old_style.get(), new_style.get()));
...
{
    std::unique_ptr<DummyPageHolder> dummy_page_holder = std::make_unique<DummyPageHolder>(gfx::Size(0, 0), nullptr);
    Document& document = dummy_page_holder->GetDocument();
    scoped_refptr<const ComputedStyle> initial = document.GetStyleResolver().InitialStyleForElement();
    ...
    StyleResolverState state(document, *document.documentElement(), nullptr /* StyleRecalcContext */, StyleRequest(initial.get()));
...
TEST_F(ComputedStyleTest, ApplyInternalLightDarkColor)
{
    ...
    auto* color_declaration = ParseDeclarationBlock("color:-internal-light-dark(black, white)", CSSParserMode::kUASheetMode);
...
TEST_F(ComputedStyleTest, ApplyInternalLightDarkBackgroundImage)
{
    ...
    auto* bgimage_declaration = ParseDeclarationBlock("background-image:-internal-light-dark(none, url(dummy.png))", kUASheetMode);
    auto* dark_declaration = ParseDeclarationBlock("color-scheme:dark");
    auto* light_declaration = ParseDeclarationBlock("color-scheme:light");
...
TEST_F(ComputedStyleTest, StrokeWidthZoomAndCalc)
{
    ...
    auto* computed_value
        = To<Longhand>(GetCSSPropertyStrokeWidth()).CSSValueFromComputedStyleInternal(*style, nullptr /* layout_object */, false /* allow_visited_style */);
...
TEST_F(ComputedStyleTest, InitialVariableNames)
{
    ...
    style->SetInitialData(StyleInitialData::Create(*Document::CreateForTest(), *registry));
...
TEST_F(ComputedStyleTest, BorderWidthZoom)
{
    ...
    for (const auto& test : tests) {
        for (const auto* property : { &GetCSSPropertyBorderLeftWidth(), &GetCSSPropertyOutlineWidth(), &GetCSSPropertyColumnRuleWidth() }) {
            const Longhand& longhand = To<Longhand>(*property);
            longhand.ApplyValue(state, *test.css_value);
            auto* computed_value = longhand.CSSValueFromComputedStyleInternal(*style, nullptr /* layout_object */, false /* allow_visited_style */);
...
TEST_F(ComputedStyleTest, TextDecorationEqualDoesNotRequireRecomputeInkOverflow)
{
    ...
    style->SetTextDecorationThickness(TextDecorationThickness(Length(5, Length::Type::kFixed)));
...
TEST_F(ComputedStyleTest, TextDecorationNotEqualRequiresRecomputeInkOverflow)
{
    ...
    other = ComputedStyle::Clone(*style);
    other->SetTextDecorationThickness(TextDecorationThickness(Length(3, Length::Type::kFixed)));
    state.SetStyle(other);
    StyleAdjuster::AdjustComputedStyle(state, nullptr /* element */);
...
TEST_F(ComputedStyleTest, ClonedStyleAnimationsAreIndependent)
{
    ...
    cloned_style_animations.DelayList().push_back(CSSAnimationData::InitialDelay());
...
#define TEST_STYLE_VALUE_NO_DIFF(field_name)                                                                                                                   \
    {                                                                                                                                                          \
        scoped_refptr<ComputedStyle> style1 = CreateComputedStyle();                                                                                           \
        scoped_refptr<ComputedStyle> style2 = CreateComputedStyle();                                                                                           \
        style1->Set##field_name(ComputedStyleInitialValues::Initial##field_name());                                                                            \
        style2->Set##field_name(ComputedStyleInitialValues::Initial##field_name());                                                                            \
...
TEST_F(ComputedStyleTest, ShouldApplyAnyContainment)
{
    ...
    std::vector display_types = { CSSValueID::kInline, CSSValueID::kBlock, CSSValueID::kListItem, CSSValueID::kInlineBlock, CSSValueID::kTable,
        ...
        CSSValueID::kFlowRoot, CSSValueID::kNone };
    ...
    for (auto contain : { CSSValueID::kNone, CSSValueID::kLayout, CSSValueID::kPaint, CSSValueID::kSize, CSSValueID::kStyle }) {
        html->SetInlineStyleProperty(CSSPropertyID::kContain, getValueName(contain));
        ...
        for (auto html_display : display_types) {
            ...
            for (auto body_display : display_types) {
                ...
                EXPECT_EQ(html->GetLayoutObject()->ShouldApplyAnyContainment(), html->GetLayoutObject()->StyleRef().ShouldApplyAnyContainment(*html))
                    << "html contain:" << getValueName(contain) << " display:" << getValueName(html_display);
                ...
                EXPECT_EQ(body->GetLayoutObject()->ShouldApplyAnyContainment(), body->GetLayoutObject()->StyleRef().ShouldApplyAnyContainment(*body))
                    << "body contain:" << getValueName(contain) << " display:" << getValueName(body_display);
            }
        }
    }
}
