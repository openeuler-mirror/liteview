...
CSSValue* ComputedCSSValueBuilder::Build(CSSValue* value) const
{
    ...
    if (auto* image_crossfade = DynamicTo<cssvalue::CSSCrossfadeValue>(value)) {
        return MakeGarbageCollected<cssvalue::CSSCrossfadeValue>(
            CrossfadeArgument(&image_crossfade->From()), CrossfadeArgument(&image_crossfade->To()), &image_crossfade->Percentage());
    }
...
CSSValue* StylePendingImage::ComputedCSSValue(const ComputedStyle& style, bool allow_visited_style) const
{
