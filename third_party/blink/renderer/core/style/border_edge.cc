...
BorderEdge::BorderEdge(float edge_width, const Color& edge_color, EBorderStyle edge_style, bool edge_is_present)
    : color(edge_color)
...
{
    return style > static_cast<unsigned>(EBorderStyle::kHidden) && color.Alpha() > 0;
}
...
{
    if (!is_present || color.HasAlpha() || style == static_cast<unsigned>(EBorderStyle::kHidden))
        return false;

    if (style == static_cast<unsigned>(EBorderStyle::kDotted) || style == static_cast<unsigned>(EBorderStyle::kDashed))
        return false;
...
float BorderEdge::GetDoubleBorderStripeWidth(DoubleBorderStripe stripe) const
{
    ...
    return roundf(stripe == kDoubleBorderStripeOuter ? UsedWidth() / 3 : (UsedWidth() * 2) / 3);
}
