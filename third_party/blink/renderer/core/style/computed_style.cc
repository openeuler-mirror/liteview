...
struct SameSizeAsComputedStyle : public SameSizeAsComputedStyleBase, public RefCounted<SameSizeAsComputedStyle> {
...
PseudoElementStyleCache& ComputedStyle::EnsurePseudoElementStyleCache() const
{
    if (!cached_data_ || !cached_data_->pseudo_element_styles_) {
        EnsureCachedData().pseudo_element_styles_ = std::make_unique<PseudoElementStyleCache>();
    }
...
const ComputedStyle* ComputedStyle::AddCachedPositionFallbackStyle(scoped_refptr<const ComputedStyle> style, unsigned index) const
{
    EnsurePositionFallbackStyleCache(index + 1)[index] = std::move(style);
    return (*cached_data_->position_fallback_styles_)[index].get();
}

const ComputedStyle* ComputedStyle::GetCachedPositionFallbackStyle(unsigned index) const
{
    if (!cached_data_ || !cached_data_->position_fallback_styles_ || index >= cached_data_->position_fallback_styles_->size())
        return nullptr;
    return (*cached_data_->position_fallback_styles_)[index].get();
}

PositionFallbackStyleCache& ComputedStyle::EnsurePositionFallbackStyleCache(unsigned ensure_size) const
{
    if (!cached_data_ || !cached_data_->position_fallback_styles_) {
        EnsureCachedData().position_fallback_styles_ = std::make_unique<PositionFallbackStyleCache>();
    }
...
static bool DiffAffectsContainerQueries(const ComputedStyle& old_style, const ComputedStyle& new_style)
{
    ...
    if ((old_style.ContainerName() != new_style.ContainerName()) || (old_style.ContainerType() != new_style.ContainerType())) {
        return true;
    }
...
{
    if ((old_style.ScrollTimelineName() != new_style.ScrollTimelineName()) || (old_style.ScrollTimelineAxis() != new_style.ScrollTimelineAxis())) {
        return true;
    }
    if ((old_style.ViewTimelineName() != new_style.ViewTimelineName()) || (old_style.ViewTimelineAxis() != new_style.ViewTimelineAxis())
...
static bool AffectsScrollAnimations(const ComputedStyle* old_style, const ComputedStyle* new_style)
{
    if (old_style && !(old_style->ScrollTimelineName().empty() && old_style->ViewTimelineName().empty())) {
        return true;
    }
    if (new_style && !(new_style->ScrollTimelineName().empty() && new_style->ViewTimelineName().empty())) {
        return true;
    }
    return false;
}
...
bool ComputedStyle::NeedsReattachLayoutTree(const Element& element, const ComputedStyle* old_style, const ComputedStyle* new_style)
{
    ...
    if (UNLIKELY(IsA<HTMLLegendElement>(element)
            && (old_style->IsFloating() != new_style->IsFloating() || old_style->HasOutOfFlowPosition() != new_style->HasOutOfFlowPosition())))
        return true;
    ...
    if (RuntimeEnabledFeatures::LayoutNGEnabled() && new_style->HasTextCombine()
        && old_style->IsHorizontalWritingMode() != new_style->IsHorizontalWritingMode()) {
...
ComputedStyle::Difference ComputedStyle::ComputeDifferenceIgnoringInheritedFirstLineStyle(const ComputedStyle& old_style, const ComputedStyle& new_style)
{
    ...
    if (DiffAffectsScrollAnimations(old_style, new_style))
        return Difference::kSiblingDescendantAffecting;
    if (old_style.Display() != new_style.Display() && old_style.BlockifiesChildren() != new_style.BlockifiesChildren())
    ...
    if (new_style.HasAnyPseudoElementStyles() || old_style.HasAnyPseudoElementStyles())
        return Difference::kPseudoElementStyle;
    if (old_style.Display() != new_style.Display() && (new_style.Display() == EDisplay::kListItem || old_style.Display() == EDisplay::kListItem))
        return Difference::kPseudoElementStyle;
    return Difference::kNonInherited;
}

void ComputedStyle::PropagateIndependentInheritedProperties(const ComputedStyle& parent_style)
{
    ComputedStyleBase::PropagateIndependentInheritedProperties(parent_style);
    if (!HasVariableReference() && !HasVariableDeclaration() && (InheritedVariables() != parent_style.InheritedVariables())) {
        MutableInheritedVariablesInternal() = parent_style.InheritedVariablesInternal();
    }
}

StyleSelfAlignmentData ResolvedSelfAlignment(const StyleSelfAlignmentData& value, ItemPosition normal_value_behavior)
{
...
StyleSelfAlignmentData ComputedStyle::ResolvedAlignItems(ItemPosition normal_value_behaviour) const
{
...
StyleSelfAlignmentData ComputedStyle::ResolvedAlignSelf(ItemPosition normal_value_behaviour, const ComputedStyle* parent_style) const
{
    // We will return the behaviour of 'normal' value if needed, which is specific
    // of each layout model.
...
StyleSelfAlignmentData ComputedStyle::ResolvedJustifyItems(ItemPosition normal_value_behaviour) const
{
    // We will return the behaviour of 'normal' value if needed, which is specific
    // of each layout model.
...
StyleSelfAlignmentData ComputedStyle::ResolvedJustifySelf(ItemPosition normal_value_behaviour, const ComputedStyle* parent_style) const
{
    // We will return the behaviour of 'normal' value if needed, which is specific
    // of each layout model.
...
void ComputedStyle::InheritFrom(const ComputedStyle& inherit_parent, IsAtShadowBoundary is_at_shadow_boundary)
{
...
const ComputedStyle* ComputedStyle::AddCachedPseudoElementStyle(
    scoped_refptr<const ComputedStyle> pseudo, PseudoId pseudo_id, const AtomicString& pseudo_argument) const
{
    DCHECK(pseudo);
...
const ComputedStyle* ComputedStyle::ReplaceCachedPseudoElementStyle(
    scoped_refptr<const ComputedStyle> pseudo_style, PseudoId pseudo_id, const AtomicString& pseudo_argument) const
{
...
void ComputedStyle::SetHasCustomHighlightName(const AtomicString& custom_highlight_name)
{
    std::unique_ptr<HashSet<AtomicString>>& names_set = MutableCustomHighlightNamesInternal();
    if (!names_set)
...
StyleDifference ComputedStyle::VisualInvalidationDiff(const Document& document, const ComputedStyle& other) const
{
    ...
    if (!diff.NeedsFullLayout() && GetPosition() != EPosition::kStatic && !OffsetEqual(other)) {
...
bool ComputedStyle::ScrollAnchorDisablingPropertyChanged(const ComputedStyle& other, const StyleDifference& diff) const
{
...
bool ComputedStyle::DiffNeedsFullLayoutAndPaintInvalidation(const ComputedStyle& other) const
{
    ...
    if (IsDisplayTableType(Display())) {
        if (ComputedStyleBase::DiffNeedsFullLayoutAndPaintInvalidationDisplayTableType(*this, other))
            return true;
        ...
        if (ComputedStyleBase::DiffNeedsFullLayoutAndPaintInvalidationDisplayListItem(*this, other))
            return true;
    }
...
bool ComputedStyle::DiffNeedsFullLayout(const Document& document, const ComputedStyle& other) const
{
...
void ComputedStyle::AdjustDiffForNeedsPaintInvalidation(const ComputedStyle& other, StyleDifference& diff, const Document& document) const
{
    if (ComputedStyleBase::DiffNeedsPaintInvalidation(*this, other) || !BorderVisuallyEqual(other) || !RadiiEqual(other))
        diff.SetNeedsPaintInvalidation();
...
void ComputedStyle::AdjustDiffForBackgroundVisuallyEqual(const ComputedStyle& other, StyleDifference& diff) const
{
    ...
    if (InternalVisitedBackgroundColor() != other.InternalVisitedBackgroundColor()) {
        diff.SetBackgroundColorChanged();
    }
...
bool ComputedStyle::DiffNeedsVisualRectUpdate(const ComputedStyle& other) const
{
...
void ComputedStyle::UpdatePropertySpecificDifferences(const ComputedStyle& other, StyleDifference& diff) const
{
    ...
    if (ComputedStyleBase::UpdatePropertySpecificDifferencesTransform(*this, other)) {
        diff.SetTransformPropertyChanged();
    }

    if (ComputedStyleBase::UpdatePropertySpecificDifferencesOtherTransform(*this, other)) {
    ...
    if (ComputedStyleBase::UpdatePropertySpecificDifferencesNeedsRecomputeVisualOverflow(*this, other))
        diff.SetNeedsRecomputeVisualOverflow();

    if (!diff.NeedsPaintInvalidation() && ComputedStyleBase::UpdatePropertySpecificDifferencesTextDecorationOrColor(*this, other)) {
    ...
    if (HasCurrentTransformAnimation() != other.HasCurrentTransformAnimation() || HasCurrentScaleAnimation() != other.HasCurrentScaleAnimation()
        || HasCurrentRotateAnimation() != other.HasCurrentRotateAnimation() || HasCurrentTranslateAnimation() != other.HasCurrentTranslateAnimation()
        || HasCurrentOpacityAnimation() != other.HasCurrentOpacityAnimation() || HasCurrentFilterAnimation() != other.HasCurrentFilterAnimation()
        || HasCurrentBackdropFilterAnimation() != other.HasCurrentBackdropFilterAnimation() || SubtreeWillChangeContents() != other.SubtreeWillChangeContents()
        || WillChangeScrollPosition() != other.WillChangeScrollPosition() || WillChangeProperties() != other.WillChangeProperties()
        || BackfaceVisibility() != other.BackfaceVisibility() || UsedTransformStyle3D() != other.UsedTransformStyle3D()
        || ContainsPaint() != other.ContainsPaint() || IsOverflowVisibleAlongBothAxes() != other.IsOverflowVisibleAlongBothAxes()
        || !BackdropFilterDataEquivalent(other) || PotentialCompositingReasonsFor3DTransformChanged(other)) {
        diff.SetCompositingReasonsChanged();
    }

    // TODO(crbug.com/1246826): Remove CompositablePaintAnimationChanged.
    if (RuntimeEnabledFeatures::CompositeBGColorAnimationEnabled()
        && (HasCurrentBackgroundColorAnimation() != other.HasCurrentBackgroundColorAnimation() || other.CompositablePaintAnimationChanged())) {
...
void ComputedStyle::AddCursor(StyleImage* image, bool hot_spot_specified, const gfx::Point& hot_spot)
{
...
void ComputedStyle::UpdateIsStackingContextWithoutContainment(bool is_document_element, bool is_in_top_layer, bool is_svg_stacking)
{
    ...
    if (is_document_element || is_in_top_layer || is_svg_stacking || StyleType() == kPseudoIdBackdrop || HasTransformRelatedProperty()
        || HasStackingGroupingProperty(BoxReflect()) || GetPosition() == EPosition::kFixed || GetPosition() == EPosition::kSticky
...
void ComputedStyle::ApplyTransform(TransformationMatrix& result, const LayoutSize& border_box_size, ApplyTransformOperations apply_operations,
...
{
    ApplyTransform(result, gfx::RectF(gfx::SizeF(border_box_size)), apply_operations, apply_origin, apply_motion_path, apply_independent_transform_properties);
}

void ComputedStyle::ApplyTransform(TransformationMatrix& result, const gfx::RectF& bounding_box, ApplyTransformOperations apply_operations,
    ApplyTransformOrigin apply_origin, ApplyMotionPath apply_motion_path, ApplyIndependentTransformProperties apply_independent_transform_properties) const
{
    if (!HasOffset())
    ...
    if (apply_transform_origin ||
        ...
        origin_x = FloatValueForLength(TransformOriginX(), box_size.width()) + bounding_box.x();
        origin_y = FloatValueForLength(TransformOriginY(), box_size.height()) + bounding_box.y();
        if (apply_transform_origin) {
...
void ComputedStyle::ApplyMotionPathTransform(float origin_x, float origin_y, const gfx::RectF& bounding_box, TransformationMatrix& transform) const
{
    ...
    if (path->GetType() == BasicShape::kStyleRayType) {
        ...
        path_position.tangent_in_degrees = ClampTo<float, float>(To<StyleRay>(*path).Angle() - 90);
        ...
        float float_distance = FloatValueForLength(distance, path_length * zoom) / zoom;
        ...
        path_position = motion_path.GetPath().PointAndNormalAtLength(computed_distance);
    ...
    transform.Translate(path_position.point.x() - anchor_point.x() + origin_shift_x, path_position.point.y() - anchor_point.y() + origin_shift_y);
...
bool ComputedStyle::SetEffectiveZoom(float f)
{
    ...
    base::UmaHistogramSparse("Blink.EffectiveZoom", base::clamp<float>(clamped_effective_zoom * 100, 0, 400));
    return true;
}
...
const AtomicString& ComputedStyle::HyphenString() const
{
    ...
    DEFINE_STATIC_LOCAL(AtomicString, hyphen_minus_string, (&kHyphenMinusCharacter, 1));
...
void ComputedStyle::ApplyTextTransform(String* text, UChar previous_character) const
{
...
const AtomicString& ComputedStyle::TextEmphasisMarkString() const
{
    ...
    case TextEmphasisMark::kDot: {
        ...
        DEFINE_STATIC_LOCAL(AtomicString, open_dot_string, (&kWhiteBulletCharacter, 1));
        return GetTextEmphasisFill() == TextEmphasisFill::kFilled ? filled_dot_string : open_dot_string;
    }
    case TextEmphasisMark::kCircle: {
        DEFINE_STATIC_LOCAL(AtomicString, filled_circle_string, (&kBlackCircleCharacter, 1));
        DEFINE_STATIC_LOCAL(AtomicString, open_circle_string, (&kWhiteCircleCharacter, 1));
        return GetTextEmphasisFill() == TextEmphasisFill::kFilled ? filled_circle_string : open_circle_string;
    }
    case TextEmphasisMark::kDoubleCircle: {
        DEFINE_STATIC_LOCAL(AtomicString, filled_double_circle_string, (&kFisheyeCharacter, 1));
        DEFINE_STATIC_LOCAL(AtomicString, open_double_circle_string, (&kBullseyeCharacter, 1));
        return GetTextEmphasisFill() == TextEmphasisFill::kFilled ? filled_double_circle_string : open_double_circle_string;
    }
    case TextEmphasisMark::kTriangle: {
        DEFINE_STATIC_LOCAL(AtomicString, filled_triangle_string, (&kBlackUpPointingTriangleCharacter, 1));
        DEFINE_STATIC_LOCAL(AtomicString, open_triangle_string, (&kWhiteUpPointingTriangleCharacter, 1));
        return GetTextEmphasisFill() == TextEmphasisFill::kFilled ? filled_triangle_string : open_triangle_string;
    }
    case TextEmphasisMark::kSesame: {
        DEFINE_STATIC_LOCAL(AtomicString, filled_sesame_string, (&kSesameDotCharacter, 1));
        DEFINE_STATIC_LOCAL(AtomicString, open_sesame_string, (&kWhiteSesameDotCharacter, 1));
...
bool ComputedStyle::TextDecorationVisualOverflowEqual(const ComputedStyle& o) const
{
    const Vector<AppliedTextDecoration>& applied_with_this = AppliedTextDecorations();
    const Vector<AppliedTextDecoration>& applied_with_other = o.AppliedTextDecorations();
...
const Vector<AppliedTextDecoration>& ComputedStyle::AppliedTextDecorations() const
{
    if (HasSimpleUnderlineInternal()) {
        DEFINE_STATIC_LOCAL(Vector<AppliedTextDecoration>, underline,
            (1,
                AppliedTextDecoration(TextDecorationLine::kUnderline, ETextDecorationStyle::kSolid, VisitedDependentColor(GetCSSPropertyTextDecorationColor()),
                    TextDecorationThickness(), Length::Auto())));
        // Since we only have one of these in memory, just update the color before
        // returning.
        underline.at(0).SetColor(VisitedDependentColor(GetCSSPropertyTextDecorationColor()));
        return underline;
    }
    if (!AppliedTextDecorationsInternal()) {
...
bool ComputedStyle::IsAppliedTextDecorationsSame(const ComputedStyle& other) const
{
    if (HasSimpleUnderlineInternal() != other.HasSimpleUnderlineInternal())
        return false;
    if (AppliedTextDecorationsInternal().get() == other.AppliedTextDecorationsInternal().get())
        return true;
    ...
    const Vector<AppliedTextDecoration>& other_decorations = other.AppliedTextDecorations();
...
{
    return InheritedVariables() || NonInheritedVariables() || HasInitialVariables(InitialData().get());
}
...
StyleInheritedVariables& ComputedStyle::MutableInheritedVariables()
{
    ...
    scoped_refptr<StyleInheritedVariables>& variables = MutableInheritedVariablesInternal();
    if (!variables)
...
StyleNonInheritedVariables& ComputedStyle::MutableNonInheritedVariables()
{
    ...
    std::unique_ptr<StyleNonInheritedVariables>& variables = MutableNonInheritedVariablesInternal();
    if (!variables)
...
void ComputedStyle::SetVariableData(const AtomicString& name, scoped_refptr<CSSVariableData> value, bool is_inherited_property)
{
...
void ComputedStyle::SetVariableValue(const AtomicString& name, const CSSValue* value, bool is_inherited_property)
{
    if (is_inherited_property)
...
static CSSVariableData* GetInitialVariableData(const AtomicString& name, const StyleInitialData* initial_data)
{
    if (!initial_data)
        return nullptr;
...
static const CSSValue* GetInitialVariableValue(const AtomicString& name, const StyleInitialData* initial_data)
{
    if (!initial_data)
        return nullptr;
...
bool ComputedStyle::HasIdenticalAscentDescentAndLineGap(const ComputedStyle& other) const
{
    const SimpleFontData* font_data = GetFont().PrimaryFont();
    const SimpleFontData* other_font_data = other.GetFont().PrimaryFont();
    return font_data && other_font_data && font_data->GetFontMetrics().HasIdenticalAscentDescentAndLineGap(other_font_data->GetFontMetrics());
}
...
{
    scoped_refptr<AppliedTextDecorationList>& list = MutableAppliedTextDecorationsInternal();
...
void ComputedStyle::ApplyTextDecorations(const Color& parent_text_decoration_color, bool override_existing_colors)
{
    if (GetTextDecorationLine() == TextDecorationLine::kNone && !HasSimpleUnderlineInternal() && !AppliedTextDecorationsInternal())
        return;

    // If there are any color changes or decorations set by this element, stop
    // using m_hasSimpleUnderline.
    Color current_text_decoration_color = VisitedDependentColor(GetCSSPropertyTextDecorationColor());
    if (HasSimpleUnderlineInternal()
        && (GetTextDecorationLine() != TextDecorationLine::kNone || current_text_decoration_color != parent_text_decoration_color)) {
        SetHasSimpleUnderlineInternal(false);
        AddAppliedTextDecoration(AppliedTextDecoration(
            TextDecorationLine::kUnderline, ETextDecorationStyle::kSolid, parent_text_decoration_color, TextDecorationThickness(), Length::Auto()));
    }
    ...
    bool is_simple_underline = decoration_lines == TextDecorationLine::kUnderline && decoration_style == ETextDecorationStyle::kSolid
        && TextDecorationColor().IsCurrentColor() && TextUnderlineOffset().IsAuto() && GetTextDecorationThickness().IsAuto();
    ...
    AddAppliedTextDecoration(
        AppliedTextDecoration(decoration_lines, decoration_style, current_text_decoration_color, GetTextDecorationThickness(), TextUnderlineOffset()));
}
...
void ComputedStyle::RestoreParentTextDecorations(const ComputedStyle& parent_style)
{
    SetHasSimpleUnderlineInternal(parent_style.HasSimpleUnderlineInternal());
    if (AppliedTextDecorationsInternal() != parent_style.AppliedTextDecorationsInternal()) {
        SetAppliedTextDecorationsInternal(scoped_refptr<AppliedTextDecorationList>(parent_style.AppliedTextDecorationsInternal()));
    }
}
...
void ComputedStyle::ClearMultiCol()
{
    ...
    SetColumnRuleWidthInternal(LayoutUnit(ComputedStyleInitialValues::InitialColumnRuleWidth()));
    SetColumnRuleColorInternal(ComputedStyleInitialValues::InitialColumnRuleColor());
    SetInternalVisitedColumnRuleColorInternal(ComputedStyleInitialValues::InitialInternalVisitedColumnRuleColor());
    SetColumnCountInternal(ComputedStyleInitialValues::InitialColumnCount());
    SetHasAutoColumnCountInternal(ComputedStyleInitialValues::InitialHasAutoColumnCount());
    SetHasAutoColumnWidthInternal(ComputedStyleInitialValues::InitialHasAutoColumnWidth());
    ResetColumnFill();
    ResetColumnSpan();
}
...
StyleColor ComputedStyle::DecorationColorIncludingFallback(bool visited_link) const
{
    ...
    if (TextStrokeWidth()) {
        ...
        if (!text_stroke_style_color.IsCurrentColor() && text_stroke_style_color.Resolve(Color(), UsedColorScheme()).Alpha()) {
...
Color ComputedStyle::VisitedDependentColor(const CSSProperty& color_property, bool* is_current_color) const
{
    DCHECK(!color_property.IsVisited());

    Color unvisited_color = To<Longhand>(color_property).ColorIncludingFallback(false, *this, is_current_color);
    ...
    Color visited_color = To<Longhand>(*visited_property).ColorIncludingFallback(true, *this, is_current_color);
    ...
    return Color(visited_color.Red(), visited_color.Green(), visited_color.Blue(), unvisited_color.Alpha());
}

Color ComputedStyle::ResolvedColor(const StyleColor& color, bool* is_current_color) const
{
    bool visited_link = (InsideLink() == EInsideLink::kInsideVisitedLink);
    Color current_color = visited_link ? GetInternalVisitedCurrentColor() : GetCurrentColor();
...
{
    return InForcedColorsMode() && ForcedColorAdjust() == EForcedColorAdjust::kAuto && !unforced_color.IsSystemColorIncludingDeprecated();
}
...
bool ComputedStyle::StrokeDashArrayDataEquivalent(const ComputedStyle& other) const
{
...
{
    return LayoutRectOutsets(NinePieceImage::ComputeOutset(image.Outset().Top(), BorderTopWidth().ToInt()),
        NinePieceImage::ComputeOutset(image.Outset().Right(), BorderRightWidth().ToInt()),
        NinePieceImage::ComputeOutset(image.Outset().Bottom(), BorderBottomWidth().ToInt()),
        NinePieceImage::ComputeOutset(image.Outset().Left(), BorderLeftWidth().ToInt()));
}
...
LayoutRectOutsets ComputedStyle::BoxDecorationOutsets() const
{
    ...
    if (const ShadowList* box_shadow = BoxShadow()) {
        outsets = EnclosingLayoutRectOutsets(box_shadow->RectOutsetsIncludingOriginal());
    }
...
void ComputedStyle::GetBorderEdgeInfo(BorderEdge edges[], PhysicalBoxSides sides_to_include) const
{
...
Color ComputedStyle::GetInternalVisitedCurrentColor(bool* is_current_color) const
{
...
Color ComputedStyle::GetInternalForcedCurrentColor(bool* is_current_color) const
{
    ...
    return InternalForcedColor().Resolve(Color(), UsedColorScheme(), is_current_color,
        /* is_forced_color */ true);
}

Color ComputedStyle::GetInternalForcedVisitedCurrentColor(bool* is_current_color) const
{
    ...
    return InternalForcedVisitedColor().Resolve(Color(), UsedColorScheme(), is_current_color,
        /* is_forced_color */ true);
}
...
static unsigned PaintOrderSequence(EPaintOrderType first, EPaintOrderType second, EPaintOrderType third)
{
...
bool ComputedStyle::ShouldApplyAnyContainment(const Element& element) const
{
    ...
    if ((ContainsInlineSize() || ContainsBlockSize())
        && (!IsDisplayTableType() || Display() == EDisplay::kTableCaption || ShouldUseContentDataForElement(GetContentData()))) {
        return true;
    }
    return (ContainsLayout() || ContainsPaint())
        && (!IsDisplayTableType() || IsDisplayTableBox() || Display() == EDisplay::kTableCell || Display() == EDisplay::kTableCaption);
}

bool ComputedStyle::CanMatchSizeContainerQueries(const Element& element) const
{
    return RuntimeEnabledFeatures::LayoutNGEnabled() && IsContainerForSizeContainerQueries() && !element.ShouldForceLegacyLayout()
        && (RuntimeEnabledFeatures::LayoutNGPrintingEnabled() || !element.GetDocument().Printing())
        && (!element.IsSVGElement() || To<SVGElement>(element).IsOutermostSVGSVGElement());
}
...
void ComputedStyle::SetUsedColorScheme(ColorSchemeFlags flags, mojom::blink::PreferredColorScheme preferred_color_scheme, bool force_dark)
{
