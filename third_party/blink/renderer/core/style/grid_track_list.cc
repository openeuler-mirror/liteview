...
NGGridTrackRepeater::NGGridTrackRepeater(wtf_size_t repeat_index, wtf_size_t repeat_size, wtf_size_t repeat_count, RepeatType repeat_type)
...
wtf_size_t NGGridTrackList::RepeatCount(const wtf_size_t index, const wtf_size_t auto_value) const
{
...
NGGridTrackRepeater::RepeatType NGGridTrackList::RepeatType(const wtf_size_t index) const
{
    DCHECK_LT(index, RepeaterCount());
    return repeaters_[index].repeat_type;
}

const GridTrackSize& NGGridTrackList::RepeatTrackSize(const wtf_size_t index, const wtf_size_t n) const
{
    DCHECK_LT(index, RepeaterCount());
...
bool NGGridTrackList::AddRepeater(const Vector<GridTrackSize, 1>& repeater_track_sizes, NGGridTrackRepeater::RepeatType repeat_type, wtf_size_t repeat_count)
{
    ...
    repeaters_.emplace_back(repeater_track_sizes_.size(), repeat_size, repeat_count, repeat_type);
