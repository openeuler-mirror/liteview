...
MojoResult MojoHandle::writeMessage(const V8BufferSource* buffer, const HeapVector<Member<MojoHandle>>& handles)
{
    ...
    auto message = mojo::Message(base::make_span(static_cast<const uint8_t*>(bytes), num_bytes), base::make_span(scoped_handles));
...
MojoWriteDataResult* MojoHandle::writeData(const V8BufferSource* buffer, const MojoWriteDataOptions* options_dict)
{
    ...
    MojoResult result
        = checked_num_bytes.AssignIfValid(&num_bytes) ? MojoWriteData(handle_.get().value(), elements, &num_bytes, &options) : MOJO_RESULT_INVALID_ARGUMENT;
    result_dict->setResult(result);
...
MojoReadDataResult* MojoHandle::readData(const V8BufferSource* buffer, const MojoReadDataOptions* options_dict) const
{
    ...
    uint32_t num_bytes;
    MojoResult result
        = checked_num_bytes.AssignIfValid(&num_bytes) ? MojoReadData(handle_.get().value(), &options, elements, &num_bytes) : MOJO_RESULT_INVALID_ARGUMENT;
    result_dict->setResult(result);
    result_dict->setNumBytes(result == MOJO_RESULT_OK ? num_bytes : 0);
    return result_dict;
}
