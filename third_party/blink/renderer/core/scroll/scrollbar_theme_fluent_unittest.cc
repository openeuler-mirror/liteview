...
protected:
    FRIEND_TEST_ALL_PREFIXES(ScrollbarThemeFluentTest, VerticalScrollbarPartsSizes);
    FRIEND_TEST_ALL_PREFIXES(ScrollbarThemeFluentTest, HorizontalScrollbarPartsSizes);
};

class ScrollbarThemeFluentTest : public ::testing::Test, public ::testing::WithParamInterface<float> {
protected:
    void SetUp() override
    {
        ScrollbarThemeSettings::SetFluentScrollbarsEnabled(true);
        mock_scrollable_area_ = MakeGarbageCollected<NiceMock<MockScrollableArea>>();
...
TEST_P(ScrollbarThemeFluentTest, VerticalScrollbarPartsSizes)
{
    ...
    Scrollbar* vetical_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area_, kVerticalScrollbar, &theme);
    vetical_scrollbar->SetFrameRect(
        gfx::Rect(kOffsetFromViewport, 0, theme.ScrollbarThickness(ScaleFromDIP(), EScrollbarWidth::kAuto), kScrollbarLengthForTests));

    EXPECT_EQ(ThumbThickness() + 2 * ThumbOffset(), TrackThickness());

    const gfx::Rect track_rect = theme.TrackRect(*vetical_scrollbar);
    EXPECT_EQ(track_rect, gfx::Rect(kOffsetFromViewport, ButtonLength(), TrackThickness(), TrackLength()));

    const gfx::Rect thumb_rect = theme.ThumbRect(*vetical_scrollbar);
    EXPECT_EQ(thumb_rect, gfx::Rect(kOffsetFromViewport + ThumbOffset(), ButtonLength(), ThumbThickness(), kThumbLengthForTests));
...
{
    ScrollbarThemeFluentMock theme;
    Scrollbar* horizontal_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area_, kHorizontalScrollbar, &theme);
    horizontal_scrollbar->SetFrameRect(
        gfx::Rect(0, kOffsetFromViewport, kScrollbarLengthForTests, theme.ScrollbarThickness(ScaleFromDIP(), EScrollbarWidth::kAuto)));

    EXPECT_EQ(ThumbThickness() + 2 * ThumbOffset(), TrackThickness());

    const gfx::Rect track_rect = theme.TrackRect(*horizontal_scrollbar);
    EXPECT_EQ(track_rect, gfx::Rect(ButtonLength(), kOffsetFromViewport, TrackLength(), TrackThickness()));

    const gfx::Rect thumb_rect = theme.ThumbRect(*horizontal_scrollbar);
    EXPECT_EQ(thumb_rect, gfx::Rect(ButtonLength(), kOffsetFromViewport + ThumbOffset(), kThumbLengthForTests, ThumbThickness()));
