...
namespace {
const double kScrollAnimationDuration = (::features::IsImpulseScrollAnimationEnabled() ? 1.5 : 0.5);
}
...
TEST_P(FractionalScrollSimTest, NoRepaintOnScrollFromSubpixel)
{
    ...
    Compositor().BeginFrame();
    EXPECT_FALSE(GetRasterInvalidationTracking(*GetDocument().View(), 2)->HasInvalidations());
...
TEST_P(ScrollAnimatorSimTest, TestRootFrameLayoutViewportUserScrollCallBack)
{
    ...
    GetDocument().View()->GetScrollableArea()->UserScroll(
        ui::ScrollGranularity::kScrollByLine, ScrollOffset(100, 300), ScrollableArea::ScrollCallback(base::BindLambdaForTesting([&]() { finished = true; })));
...
TEST_P(ScrollAnimatorSimTest, TestRootFrameBothViewportsUserScrollCallBack)
{
    ...
    bool finished = false;
    GetDocument().View()->GetScrollableArea()->UserScroll(
        ui::ScrollGranularity::kScrollByLine, ScrollOffset(0, 1000), ScrollableArea::ScrollCallback(base::BindLambdaForTesting([&]() { finished = true; })));
    // Sync time with ScrollAnimator.
    Compositor().ResetLastFrameTime();
...
TEST_P(ScrollAnimatorSimTest, TestDivUserScrollCallBack)
{
    ...
    scrollable_area->UserScroll(
        ui::ScrollGranularity::kScrollByLine, ScrollOffset(0, 100), ScrollableArea::ScrollCallback(base::BindLambdaForTesting([&]() { finished = true; })));
    // Sync time with ScrollAnimator.
    Compositor().ResetLastFrameTime();
...
TEST_P(ScrollAnimatorSimTest, TestUserScrollCallBackAnimatorDisabled)
{
    ...
    bool finished = false;
    GetDocument().View()->GetScrollableArea()->UserScroll(
        ui::ScrollGranularity::kScrollByLine, ScrollOffset(0, 300), ScrollableArea::ScrollCallback(base::BindLambdaForTesting([&]() { finished = true; })));
    // Sync time with ScrollAnimator.
    Compositor().ResetLastFrameTime();
