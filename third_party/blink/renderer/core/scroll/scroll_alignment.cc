...
// static
ScrollOffset ScrollAlignment::GetScrollOffsetToExpose(const PhysicalRect& scroll_snapport_rect, const PhysicalRect& expose_rect,
    const mojom::blink::ScrollAlignment& align_x, const mojom::blink::ScrollAlignment& align_y, const ScrollOffset& current_scroll_offset)
{
    ...
    PhysicalRect expose_rect_x(expose_rect.X(), non_zero_visible_rect.Y(), expose_rect.Width(), non_zero_visible_rect.Height());
    ...
    if (scroll_x == mojom::blink::ScrollAlignment::Behavior::kClosestEdge) {
        ...
        if ((expose_rect.Right() > non_zero_visible_rect.Right() && expose_rect.Width() < non_zero_visible_rect.Width())
            || (expose_rect.Right() < non_zero_visible_rect.Right() && expose_rect.Width() > non_zero_visible_rect.Width())) {
    ...
    PhysicalRect expose_rect_y(non_zero_visible_rect.X(), expose_rect.Y(), non_zero_visible_rect.Width(), expose_rect.Height());
    ...
    if (scroll_y == mojom::blink::ScrollAlignment::Behavior::kClosestEdge) {
        ...
        if ((expose_rect.Bottom() > non_zero_visible_rect.Bottom() && expose_rect.Height() < non_zero_visible_rect.Height())
            || (expose_rect.Bottom() < non_zero_visible_rect.Bottom() && expose_rect.Height() > non_zero_visible_rect.Height())) {
    ...
    if (scroll_x == mojom::blink::ScrollAlignment::Behavior::kNoScroll) {
        ...
        x = ((expose_rect.X() + expose_rect.Right() - (non_zero_visible_rect.X() + non_zero_visible_rect.Right())) / 2).ToFloat();
    } else {
    ...
    if (scroll_y == mojom::blink::ScrollAlignment::Behavior::kNoScroll) {
        ...
        y = ((expose_rect.Y() + expose_rect.Bottom() - (non_zero_visible_rect.Y() + non_zero_visible_rect.Bottom())) / 2).ToFloat();
    } else {
...
// static
mojom::blink::ScrollIntoViewParamsPtr ScrollAlignment::CreateScrollIntoViewParams(const mojom::blink::ScrollAlignment& align_x,
    const mojom::blink::ScrollAlignment& align_y, mojom::blink::ScrollType scroll_type, bool make_visible_in_visual_viewport,
    mojom::blink::ScrollBehavior scroll_behavior, bool is_for_scroll_sequence, bool cross_origin_boundaries)
{
...
namespace {
mojom::blink::ScrollAlignment AlignmentFromOptions(const ScrollIntoViewOptions& options, ScrollOrientation axis, const ComputedStyle& computed_style)
{
    ...
    String alignment = ((axis == kHorizontalScroll && is_horizontal_writing_mode) || (axis == kVerticalScroll && !is_horizontal_writing_mode))
    ...
    if (alignment == "start") {
        if (axis == kHorizontalScroll) {
            ...
            case WritingMode::kHorizontalTb:
                return is_ltr ? ScrollAlignment::LeftAlways() : ScrollAlignment::RightAlways();
            ...
            case WritingMode::kSidewaysRl:
            case WritingMode::kVerticalLr:
                return is_ltr ? ScrollAlignment::TopAlways() : ScrollAlignment::BottomAlways();
            case WritingMode::kSidewaysLr:
                return is_ltr ? ScrollAlignment::BottomAlways() : ScrollAlignment::TopAlways();
            default:
                NOTREACHED();
                return ScrollAlignment::TopAlways();
            }
    ...
    if (alignment == "end") {
        if (axis == kHorizontalScroll) {
            switch (writing_mode) {
            case WritingMode::kHorizontalTb:
                return is_ltr ? ScrollAlignment::RightAlways() : ScrollAlignment::LeftAlways();
            case WritingMode::kVerticalRl:
            case WritingMode::kSidewaysRl:
                return ScrollAlignment::LeftAlways();
            case WritingMode::kVerticalLr:
...
// static
mojom::blink::ScrollIntoViewParamsPtr ScrollAlignment::CreateScrollIntoViewParams(const ScrollIntoViewOptions& options, const ComputedStyle& computed_style)
{
    mojom::blink::ScrollBehavior behavior = (options.behavior() == "smooth") ? mojom::blink::ScrollBehavior::kSmooth : mojom::blink::ScrollBehavior::kAuto;

    auto align_x = AlignmentFromOptions(options, kHorizontalScroll, computed_style);
    auto align_y = AlignmentFromOptions(options, kVerticalScroll, computed_style);

    mojom::blink::ScrollIntoViewParamsPtr params = ScrollAlignment::CreateScrollIntoViewParams(align_x, align_y);
