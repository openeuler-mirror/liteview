...
// the root.
absl::optional<LayoutBox*> GetScrollParent(const LayoutBox& box, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
...
absl::optional<PhysicalRect> PerformBubblingScrollIntoView(
    const LayoutBox& box, const PhysicalRect& absolute_rect, mojom::blink::ScrollIntoViewParamsPtr& params, bool from_remote_frame)
{
    ...
    while (current_box) {
        ...
        if (current_box->IsScrollContainer() && !IsA<LayoutView>(current_box)) {
            ...
            area_to_scroll
                = params->make_visible_in_visual_viewport ? current_box->GetFrameView()->GetScrollableArea() : current_box->GetFrameView()->LayoutViewport();
        }

        if (area_to_scroll) {
            ScrollOffset scroll_before = area_to_scroll->GetScrollOffset();
            DCHECK(area_to_scroll->GetSmoothScrollSequencer());
            wtf_size_t num_scroll_sequences = area_to_scroll->GetSmoothScrollSequencer()->GetCount();

            absolute_rect_to_scroll = area_to_scroll->ScrollIntoView(absolute_rect_to_scroll, params);
            ...
            bool scroll_changed = params->is_for_scroll_sequence ? area_to_scroll->GetSmoothScrollSequencer()->GetCount() != num_scroll_sequences
        ...
        if (is_fixed_to_frame && params->make_visible_in_visual_viewport) {
            ...
            if (current_box->GetFrame()->IsMainFrame() && visual_viewport.IsActiveViewport()) {
                absolute_rect_to_scroll = current_box->GetFrame()->GetPage()->GetVisualViewport().ScrollIntoView(absolute_rect_to_scroll, params);
            }
        ...
        absl::optional<LayoutBox*> next_box_opt = GetScrollParent(*current_box, params);
        if (!next_box_opt)
            return absl::nullopt;
    ...
    const LayoutObject& layout_object, const PhysicalRect& absolute_rect, mojom::blink::ScrollIntoViewParamsPtr params, bool from_remote_frame)
{
    ...
    absl::optional<PhysicalRect> updated_absolute_rect = PerformBubblingScrollIntoView(*enclosing_box, absolute_rect, params, from_remote_frame);
