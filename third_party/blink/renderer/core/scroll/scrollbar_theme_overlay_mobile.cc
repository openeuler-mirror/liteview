...
static const WebThemeEngine::ScrollbarStyle& ScrollbarStyle()
{
    ...
    DEFINE_STATIC_LOCAL(WebThemeEngine::ScrollbarStyle, style, (WebThemeEngine::ScrollbarStyle { 3, 4, 3, 4, 0x80808080 }));
    if (!initialized) {
...
ScrollbarThemeOverlayMobile& ScrollbarThemeOverlayMobile::GetInstance()
{
    ...
    DEFINE_STATIC_LOCAL(ScrollbarThemeOverlayMobile, theme,
        (ScrollbarStyle().thumb_thickness, ScrollbarStyle().scrollbar_margin, ScrollbarStyle().thumb_thickness_thin, ScrollbarStyle().scrollbar_margin_thin,
            Color::FromSkColor(ScrollbarStyle().color)));
    return theme;
}

ScrollbarThemeOverlayMobile::ScrollbarThemeOverlayMobile(
    int thumb_thickness_default, int scrollbar_margin_default, int thumb_thickness_thin, int scrollbar_margin_thin, Color color)
    : ScrollbarThemeOverlay(thumb_thickness_default, scrollbar_margin_default, thumb_thickness_thin, scrollbar_margin_thin)
    , color_(color)
{
}
