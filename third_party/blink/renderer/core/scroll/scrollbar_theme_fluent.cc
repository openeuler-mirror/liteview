...
ScrollbarThemeFluent::ScrollbarThemeFluent()
{
    ...
    scrollbar_button_length_
        = WebTestSupport::IsRunningWebTest() ? kScrollbarTrackThicknessForWebTests : theme_engine->GetSize(WebThemeEngine::kPartScrollbarUpArrow).height();
    scrollbar_thumb_thickness_
        = WebTestSupport::IsRunningWebTest() ? kScrollbarThumbThicknessForWebTests : theme_engine->GetSize(WebThemeEngine::kPartScrollbarVerticalThumb).width();
    scrollbar_track_thickness_
        = WebTestSupport::IsRunningWebTest() ? kScrollbarTrackThicknessForWebTests : theme_engine->GetSize(WebThemeEngine::kPartScrollbarVerticalTrack).width();
}

int ScrollbarThemeFluent::ScrollbarThickness(float scale_from_dip, EScrollbarWidth scrollbar_width)
{
    ...
    const int scrollbar_thickness = static_cast<int>(scrollbar_track_thickness_ * scale_from_dip);
    return (scrollbar_thickness - ThumbThickness(scale_from_dip)) % 2 != 0 ? scrollbar_thickness + 1 : scrollbar_thickness;
}
...
gfx::Size ScrollbarThemeFluent::ButtonSize(const Scrollbar& scrollbar) const
{
    ...
    if (scrollbar.Orientation() == kVerticalScrollbar) {
        ...
        const int button_height_unclamped = scrollbar_button_length_ * scrollbar.ScaleFromDIP();
        const int button_height = scrollbar.Height() < 2 * button_height_unclamped ? base::ClampFloor(scrollbar.Height() / 2.0f) : button_height_unclamped;
        return gfx::Size(button_width, button_height);
    } else {
        const int button_height = scrollbar.Height();
        const int button_width_unclamped = scrollbar_button_length_ * scrollbar.ScaleFromDIP();
        const int button_width = scrollbar.Width() < 2 * button_width_unclamped ? base::ClampFloor(scrollbar.Width() / 2.0f) : button_width_unclamped;
        return gfx::Size(button_width, button_height);
    }
}
