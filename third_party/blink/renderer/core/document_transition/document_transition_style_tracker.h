...
class DocumentTransitionStyleTracker : public GarbageCollected<DocumentTransitionStyleTracker> {
public:
    // Properties that transition on container elements.
    struct ContainerProperties {
        ContainerProperties() = default;
        ContainerProperties(const LayoutSize& size, const TransformationMatrix& matrix)
            : border_box_size_in_css_space(size)
            , snapshot_matrix(matrix)
        {
        }

        bool operator==(const ContainerProperties& other) const
        {
            return border_box_size_in_css_space == other.border_box_size_in_css_space && snapshot_matrix == other.snapshot_matrix;
        }
        bool operator!=(const ContainerProperties& other) const
        {
            return !(*this == other);
    ...
    void UpdateRootIndexAndSnapshotId(DocumentTransitionSharedElementId&, viz::SharedElementResourceId&) const;

    void UpdateElementIndicesAndSnapshotId(Element*, DocumentTransitionSharedElementId&, viz::SharedElementResourceId&) const;
    ...
    PseudoElement* CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& document_transition_tag);
    ...
    PaintPropertyChangeType UpdateEffect(Element* element, EffectPaintPropertyNode::State state, const EffectPaintPropertyNodeOrAlias& current_effect);
    PaintPropertyChangeType UpdateRootEffect(EffectPaintPropertyNode::State state, const EffectPaintPropertyNodeOrAlias& current_effect);
...
private:
    ...
    bool FlattenAndVerifyElements(VectorOf<Element>&, VectorOf<AtomicString>&, absl::optional<RootData>&);
    ...
    HeapHashMap<Member<Element>, HashSet<std::pair<AtomicString, int>>> pending_shared_element_tags_;
