...
bool DocumentTransitionPseudoElementBase::CanGeneratePseudoElement(PseudoId pseudo_id) const
{
    switch (GetPseudoId()) {
    ...
    case kPseudoIdPageTransitionImageWrapper:
        return pseudo_id == kPseudoIdPageTransitionOutgoingImage || pseudo_id == kPseudoIdPageTransitionIncomingImage;
...
scoped_refptr<ComputedStyle> DocumentTransitionPseudoElementBase::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    Element* parent = ParentOrShadowHostElement();
    auto style_request = StyleRequest(GetPseudoId(), parent->GetComputedStyle(), document_transition_tag());
