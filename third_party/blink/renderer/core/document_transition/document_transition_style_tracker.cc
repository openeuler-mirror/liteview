...
const char* kElementSetModificationError = "The element set cannot be modified at this transition state.";
const char* kContainmentNotSatisfied = "Dropping element from transition. Shared element must contain paint or "
                                       "layout";
const char* kDuplicateTagBaseError = "Unexpected duplicate page transition tag: ";
...
absl::optional<String> GetSnapshotViewportOffsetTransform(const gfx::Vector2d& offset, float device_pixel_ratio)
{
    ...
    // pixels.
    gfx::Vector2dF css_offset = gfx::ScaleVector2d(offset, 1.f / device_pixel_ratio);
    ...
    return String::Format("transform: translate(%.3fpx, %.3fpx);", css_offset.x(), css_offset.y());
}

absl::optional<String> ComputeInsetDifference(PhysicalRect reference_rect, const LayoutRect& target_rect, float device_pixel_ratio)
{
    ...
    float right_offset = (reference_layout_rect.MaxX() - target_rect.MaxX()).ToFloat();
    float bottom_offset = (reference_layout_rect.MaxY() - target_rect.MaxY()).ToFloat();
    float left_offset = (target_rect.X() - reference_layout_rect.X()).ToFloat();

    return String::Format("inset(%.3fpx %.3fpx %.3fpx %.3fpx);", top_offset, right_offset, bottom_offset, left_offset);
}
...
PhysicalRect ComputeVisualOverflowRect(LayoutBox* box)
{
    ...
    PhysicalRect result;
    for (auto* child = box->Layer()->FirstChild(); child; child = child->NextSibling()) {
...
class DocumentTransitionStyleTracker::ImageWrapperPseudoElement : public DocumentTransitionPseudoElementBase {
public:
    ImageWrapperPseudoElement(
        Element* parent, PseudoId pseudo_id, const AtomicString& document_transition_tag, const DocumentTransitionStyleTracker* style_tracker)
        : DocumentTransitionPseudoElementBase(parent, pseudo_id, document_transition_tag, style_tracker)
    {
    }
...
private:
    ...
    {
        if (!DocumentTransitionPseudoElementBase::CanGeneratePseudoElement(pseudo_id)) {
            return false;
        }
        viz::SharedElementResourceId snapshot_id;
        if (pseudo_id == kPseudoIdPageTransitionOutgoingImage) {
            if (style_tracker_->old_root_data_ && style_tracker_->old_root_data_->tags.Contains(document_transition_tag())) {
                snapshot_id = style_tracker_->old_root_data_->snapshot_id;
                DCHECK(snapshot_id.IsValid());
            } else if (auto it = style_tracker_->element_data_map_.find(document_transition_tag()); it != style_tracker_->element_data_map_.end()) {
                ...
                DCHECK(style_tracker_->new_root_data_->tags.Contains(document_transition_tag()));
            }
        } else {
            if (style_tracker_->new_root_data_ && style_tracker_->new_root_data_->tags.Contains(document_transition_tag())) {
                ...
                DCHECK(style_tracker_->old_root_data_->tags.Contains(document_transition_tag()));
            }
        }
...
DocumentTransitionStyleTracker::DocumentTransitionStyleTracker(Document& document)
    : document_(document)
{
}
...
void DocumentTransitionStyleTracker::AddConsoleError(String message, Vector<DOMNodeId> related_nodes)
{
...
void DocumentTransitionStyleTracker::AddSharedElement(Element* element, const AtomicString& tag)
{
    DCHECK(element);
    if (state_ == State::kCapturing || state_ == State::kStarted) {
        AddConsoleError(kElementSetModificationError, { DOMNodeIds::IdForNode(element) });
        return;
    }

    // Insert an empty hash set for the element if it doesn't exist, or get it if
    // it does.
    auto& value = pending_shared_element_tags_.insert(element, HashSet<std::pair<AtomicString, int>>()).stored_value->value;
...
void DocumentTransitionStyleTracker::AddSharedElementsFromCSS()
{
    ...
    // after that point.
    auto scope = document_->GetDisplayLockDocumentState().GetScopedForceActivatableLocks();

    // We need our paint layers, and z-order lists which is done during
    // compositing inputs update.
    document_->View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kDocumentTransition);

    AddSharedElementsFromCSSRecursive(document_->GetLayoutView()->PaintingLayer());
}

void DocumentTransitionStyleTracker::AddSharedElementsFromCSSRecursive(PaintLayer* root)
{
    ...
    if (root_style.PageTransitionTag()) {
        ...
        AddSharedElement(DynamicTo<Element>(root_object.GetNode()), root_style.PageTransitionTag());
    }
...
bool DocumentTransitionStyleTracker::FlattenAndVerifyElements(
    VectorOf<Element>& elements, VectorOf<AtomicString>& transition_tags, absl::optional<RootData>& root_data)
{
    ...
    for (auto& [element, tags] : pending_shared_element_tags_) {
        ...
        for (auto& tag_pair : tags) {
            if (is_root) {
                ...
                flat_list.push_back(MakeGarbageCollected<FlatData>(element, tag_pair.first, tag_pair.second));
            }
        }
    }
    ...
    auto have_root_tag = [&root_data](const AtomicString& tag) { return root_data && root_data->tags.Contains(tag); };
...
bool DocumentTransitionStyleTracker::Capture()
{
    ...
    bool success = FlattenAndVerifyElements(elements, transition_tags, old_root_data_);
    if (!success)
        return false;
    ...
    HeapHashMap<Member<Element>, viz::SharedElementResourceId> element_snapshot_ids;
    ...
    for (wtf_size_t i = 0; i < transition_tags.size(); ++i) {
        ...
        auto& snapshot_id = element_snapshot_ids.insert(element, viz::SharedElementResourceId()).stored_value->value;
...
void DocumentTransitionStyleTracker::CaptureResolved()
{
    ...
    for (auto& entry : element_data_map_) {
        ...
        if (!element_data->container_properties.empty()) {
            element_data->cached_container_properties = element_data->container_properties.back();
        }
        element_data->cached_visual_overflow_rect_in_layout_space = element_data->visual_overflow_rect_in_layout_space;
        element_data->effect_node = nullptr;
    }
    root_effect_node_ = nullptr;
}

VectorOf<Element> DocumentTransitionStyleTracker::GetTransitioningElements() const
{
...
bool DocumentTransitionStyleTracker::Start()
{
    ...
    VectorOf<AtomicString> transition_tags;
    VectorOf<Element> elements;
    bool success = FlattenAndVerifyElements(elements, transition_tags, new_root_data_);
    if (!success)
        return false;
    ...
    int next_index = element_data_map_.size() + OldRootDataTagSize() + NewRootDataTagSize();
    ...
    if (!found_new_tags && new_root_data_) {
        ...
        for (const auto& new_tag : new_root_data_->tags) {
            ...
            if (!old_root_data_->tags.Contains(new_tag) && !element_data_map_.Contains(new_tag)) {
                found_new_tags = true;
                break;
            }
        }
...
void DocumentTransitionStyleTracker::UpdateElementIndicesAndSnapshotId(
    Element* element, DocumentTransitionSharedElementId& index, viz::SharedElementResourceId& resource_id) const
{
    DCHECK(element);
...
auto DocumentTransitionStyleTracker::GetCurrentRootData() const -> absl::optional<RootData>
{
    return HasLiveNewContent() ? new_root_data_ : old_root_data_;
}

void DocumentTransitionStyleTracker::UpdateRootIndexAndSnapshotId(DocumentTransitionSharedElementId& index, viz::SharedElementResourceId& resource_id) const
{
...
PseudoElement* DocumentTransitionStyleTracker::CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& document_transition_tag)
{
    ...
    case kPseudoIdPageTransitionContainer:
        return MakeGarbageCollected<DocumentTransitionPseudoElementBase>(parent, pseudo_id, document_transition_tag, this);
    case kPseudoIdPageTransitionImageWrapper:
        return MakeGarbageCollected<ImageWrapperPseudoElement>(parent, pseudo_id, document_transition_tag, this);
    case kPseudoIdPageTransitionOutgoingImage: {
        ...
        auto* pseudo_element = MakeGarbageCollected<DocumentTransitionContentElement>(parent, pseudo_id, document_transition_tag, snapshot_id,
...
void DocumentTransitionStyleTracker::RunPostPrePaintSteps()
{
    ...
    for (auto& entry : element_data_map_) {
        ...
        if (!layout_object || (!layout_object->ShouldApplyPaintContainment() && !layout_object->ShouldApplyLayoutContainment())) {
            element_data->target_element = nullptr;
            ...
            // its snapshot.
            auto& live_snapshot_id = HasLiveNewContent() ? element_data->new_snapshot_id : element_data->old_snapshot_id;
        ...
        const float device_pixel_ratio = document_->documentElement()->GetLayoutObject()->StyleRef().EffectiveZoom();
        TransformationMatrix snapshot_matrix = layout_object->LocalToAbsoluteTransform();

        gfx::Vector2d snapshot_to_fixed_offset = -GetSnapshotViewportRect().OffsetFromOrigin();
        snapshot_matrix.PostTranslate(snapshot_to_fixed_offset.x(), snapshot_to_fixed_offset.y());
        ...
        auto* resize_observer_entry = MakeGarbageCollected<ResizeObserverEntry>(element_data->target_element);
        auto entry_size = resize_observer_entry->borderBoxSize()[0];
        LayoutSize border_box_size_in_css_space = layout_object->IsHorizontalWritingMode()
            ? LayoutSize(LayoutUnit(entry_size->inlineSize()), LayoutUnit(entry_size->blockSize()))
            : LayoutSize(LayoutUnit(entry_size->blockSize()), LayoutUnit(entry_size->inlineSize()));
        if (float effective_zoom = layout_object->StyleRef().EffectiveZoom();
            std::abs(effective_zoom - device_pixel_ratio) >= std::numeric_limits<float>::epsilon()) {
        ...
        ContainerProperties container_properties(border_box_size_in_css_space, snapshot_matrix);
        if (!element_data->container_properties.empty() && element_data->container_properties.back() == container_properties
        ...
        } else if (element_data->container_properties.back() != container_properties) {
        ...
        PseudoId live_content_element = HasLiveNewContent() ? kPseudoIdPageTransitionIncomingImage : kPseudoIdPageTransitionOutgoingImage;
        if (auto* pseudo_element = document_->documentElement()->GetNestedPseudoElement(live_content_element, entry.key)) {
            ...
            static_cast<DocumentTransitionContentElement*>(pseudo_element)->SetIntrinsicSize(size);
        }
...
bool DocumentTransitionStyleTracker::HasActiveAnimations() const
{
    ...
    DocumentTransitionUtils::ForEachTransitionPseudo(*document_, accumulate_pseudo);
    return has_animations;
}

PaintPropertyChangeType DocumentTransitionStyleTracker::UpdateEffect(
    Element* element, EffectPaintPropertyNode::State state, const EffectPaintPropertyNodeOrAlias& current_effect)
{
    for (auto& entry : element_data_map_) {
        auto& element_data = entry.value;
        ...
        if (!element_data->effect_node) {
            element_data->effect_node = EffectPaintPropertyNode::Create(current_effect, std::move(state));
#if DCHECK_IS_ON()
...
PaintPropertyChangeType DocumentTransitionStyleTracker::UpdateRootEffect(
    EffectPaintPropertyNode::State state, const EffectPaintPropertyNodeOrAlias& current_effect)
{
    if (!root_effect_node_) {
        root_effect_node_ = EffectPaintPropertyNode::Create(current_effect, std::move(state));
#if DCHECK_IS_ON()
...
EffectPaintPropertyNode* DocumentTransitionStyleTracker::GetEffect(Element* element) const
{
    for (auto& entry : element_data_map_) {
        auto& element_data = entry.value;
        if (element_data->target_element != element)
...
void DocumentTransitionStyleTracker::VerifySharedElements()
{
    for (auto& entry : element_data_map_) {
        ...
        if (object) {
            if (object->ShouldApplyPaintContainment() || object->ShouldApplyLayoutContainment()) {
                continue;
            }

            AddConsoleError(kContainmentNotSatisfied, { DOMNodeIds::IdForNode(active_element) });
        }
...
StyleRequest::RulesToInclude DocumentTransitionStyleTracker::StyleRulesToInclude() const
{
    switch (state_) {
    case State::kIdle:
    case State::kCapturing:
...
gfx::Vector2d DocumentTransitionStyleTracker::GetRootSnapshotPaintOffset() const
{
...
void DocumentTransitionStyleTracker::InvalidateStyle()
{
    ...
    originating_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kDocumentTransition));

    auto invalidate_style = [](PseudoElement* pseudo_element) {
        pseudo_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kDocumentTransition));
    };
    DocumentTransitionUtils::ForEachTransitionPseudo(*document_, invalidate_style);
    ...
    document_->GetDisplayLockDocumentState().NotifySharedElementPseudoTreeChanged();
}
...
const String& DocumentTransitionStyleTracker::UAStyleSheet()
{
    ...
    // Use the document element's effective zoom, since that's what the parent
    // effective zoom would be.
    float device_pixel_ratio = document_->documentElement()->GetLayoutObject()->StyleRef().EffectiveZoom();
    ...
    absl::optional<String> snapshot_viewport_offset = GetSnapshotViewportOffsetTransform(GetSnapshotViewportRect().OffsetFromOrigin(), device_pixel_ratio);
    ...
    for (auto& root_tag : AllRootTags()) {
        ...
        bool tag_is_old_root = old_root_data_ && old_root_data_->tags.Contains(root_tag);
        if (tag_is_old_root && element_data_map_.Contains(root_tag)) {
            DCHECK(element_data_map_.find(root_tag)->value->new_snapshot_id.IsValid());
            continue;
        }
        ...
        bool tag_is_new_root = new_root_data_ && new_root_data_->tags.Contains(root_tag);
    ...
    for (auto& entry : element_data_map_) {
        ...
        const bool tag_is_old_root = old_root_data_ && old_root_data_->tags.Contains(document_transition_tag);
        const bool tag_is_new_root = new_root_data_ && new_root_data_->tags.Contains(document_transition_tag);
        ...
        if (!tag_is_new_root) {
            ...
            // element.
            builder.AddContainerStyles(document_transition_tag, element_data->container_properties.back(), element_data->container_writing_mode);

            // Incoming inset also only makes sense if the tag is a new shared element
            // (not a new root).
            absl::optional<String> incoming_inset = ComputeInsetDifference(element_data->visual_overflow_rect_in_layout_space,
                LayoutRect(LayoutPoint(), element_data->container_properties.back().border_box_size_in_css_space), device_pixel_ratio);
            if (incoming_inset) {
                builder.AddIncomingObjectViewBox(document_transition_tag, *incoming_inset);
            }
        }
        ...
        // an old root).
        if (!tag_is_old_root) {
            absl::optional<String> outgoing_inset = ComputeInsetDifference(element_data->cached_visual_overflow_rect_in_layout_space,
                LayoutRect(LayoutPoint(), element_data->cached_container_properties.border_box_size_in_css_space), device_pixel_ratio);
            if (outgoing_inset) {
                builder.AddOutgoingObjectViewBox(document_transition_tag, *outgoing_inset);
            }
        }
        ...
        if (add_animations) {
            ...
            if (element_data->old_snapshot_id.IsValid() && (element_data->new_snapshot_id.IsValid() || tag_is_new_root)) {
                builder.AddAnimationAndBlending(document_transition_tag, element_data->cached_container_properties);
                ...
                // the effective zoom.
                layout_view_size.Scale(1 / document_->GetLayoutView()->StyleRef().EffectiveZoom());
                builder.AddAnimationAndBlending(document_transition_tag, ContainerProperties(layout_view_size, TransformationMatrix()));
            }
        }
    }
...
void DocumentTransitionStyleTracker::ElementData::Trace(Visitor* visitor) const
{
...
LayoutSize DocumentTransitionStyleTracker::ElementData::GetIntrinsicSize(bool use_cached_data)
{
    LayoutSize box_size
        = use_cached_data ? cached_visual_overflow_rect_in_layout_space.size.ToLayoutSize() : visual_overflow_rect_in_layout_space.size.ToLayoutSize();
    return box_size;
}
