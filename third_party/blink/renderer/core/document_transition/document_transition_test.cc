...
class DocumentTransitionTest : public testing::Test, public PaintTestConfigurations, private ScopedDocumentTransitionForTest {
public:
    ...
    void SetUp() override
    {
        ...
        web_view_helper_->Initialize(nullptr, nullptr, &ConfigureCompositingWebView);
    ...
    void UpdateAllLifecyclePhasesAndFinishDirectives()
    {
        ...
        for (auto& callback : LayerTreeHost()->TakeDocumentTransitionCallbacksForTesting()) {
    ...
    {
        auto* transition = DocumentTransitionSupplement::EnsureDocumentTransition(GetDocument());
    ...
    bool ShouldCompositeForDocumentTransition(Element* e)
    {
        ...
        auto* transition = DocumentTransitionSupplement::EnsureDocumentTransition(GetDocument());
        return layout_object && transition && transition->NeedsSharedElementEffectNode(*layout_object);
    }

    void ValidatePseudoElementTree(const Vector<WTF::AtomicString>& document_transition_tags, bool has_incoming_image)
    {
        auto* transition_pseudo = GetDocument().documentElement()->GetPseudoElement(kPseudoIdPageTransition);
        ...
        for (const auto& document_transition_tag : document_transition_tags) {
            ...
            auto* image_wrapper_pseudo = container_pseudo->GetPseudoElement(kPseudoIdPageTransitionImageWrapper, document_transition_tag);

            auto* outgoing_image = image_wrapper_pseudo->GetPseudoElement(kPseudoIdPageTransitionOutgoingImage, document_transition_tag);
            ASSERT_TRUE(outgoing_image);
            EXPECT_TRUE(outgoing_image->GetComputedStyle());
            EXPECT_TRUE(outgoing_image->GetLayoutObject());

            auto* incoming_image = image_wrapper_pseudo->GetPseudoElement(kPseudoIdPageTransitionIncomingImage, document_transition_tag);
...
TEST_P(DocumentTransitionTest, LayoutShift)
{
    ...
    auto* document_transition_callback = V8DocumentTransitionCallback::Create(funcs.ExpectCall());

    ScriptPromiseTester start_tester(script_state, transition->start(script_state, document_transition_callback, exception_state));
    ...
    auto* transition_pseudo = GetDocument().documentElement()->GetPseudoElement(kPseudoIdPageTransition);
    ASSERT_TRUE(transition_pseudo);
    auto* container_pseudo = transition_pseudo->GetPseudoElement(kPseudoIdPageTransitionContainer, "shared");
    ASSERT_TRUE(container_pseudo);
    ...
    auto* target = To<LayoutBox>(GetDocument().getElementById("target")->GetLayoutObject());
...
{
    auto* first_transition = DocumentTransitionSupplement::EnsureDocumentTransition(GetDocument());
    auto* second_transition = DocumentTransitionSupplement::EnsureDocumentTransition(GetDocument());
...
TEST_P(DocumentTransitionTest, TransitionPreparePromiseResolves)
{
    ...
    ScriptPromiseTester promise_tester(script_state, transition->start(script_state, document_transition_callback, exception_state));
...
TEST_P(DocumentTransitionTest, AdditionalStartAbortsTransition)
{
    ...
    MockFunctionScope funcs(script_state);
    auto* document_transition_callback = V8DocumentTransitionCallback::Create(funcs.ExpectNoCall());

    ScriptPromiseTester first_promise_tester(script_state, transition->start(script_state, document_transition_callback, exception_state));
    EXPECT_EQ(GetState(transition), State::kCapturing);

    EXPECT_FALSE(exception_state.HadException());
    transition->start(script_state, document_transition_callback, exception_state);
...
TEST_P(DocumentTransitionTest, StartSharedElementsWantToBeComposited)
{
    ...
    public:
        Data(Document& document, DocumentTransition* transition, ScriptState* script_state, ExceptionState& exception_state, Element* e1, Element* e2)
            : document(document)
    ...
    auto start_setup_lambda = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
        ...
        data->document.getElementById("e1")->setAttribute(html_names::kStyleAttr, "");
        data->document.getElementById("e3")->setAttribute(html_names::kStyleAttr, "");
        data->e1->setAttribute(html_names::kStyleAttr, "page-transition-tag: e1");
        data->e2->setAttribute(html_names::kStyleAttr, "page-transition-tag: e2");
    };
    auto start_setup_callback = v8::Function::New(v8_scope.GetContext(), start_setup_lambda, v8::External::New(v8_scope.GetIsolate(), &data)).ToLocalChecked();

    transition->start(script_state, V8DocumentTransitionCallback::Create(start_setup_callback), exception_state);
...
TEST_P(DocumentTransitionTest, DocumentTransitionPseudoTree)
{
    ...
    public:
        Data(DocumentTransition* transition, ScriptState* script_state, ExceptionState& exception_state, Document& document)
            : transition(transition)
            , script_state(script_state)
            , exception_state(exception_state)
            , document(document)
    ...
    const Vector<AtomicString> document_transition_tags = { "root", "e1", "e2", "e3" };
    ...
    FinishTransition();
    UpdateAllLifecyclePhasesAndFinishDirectives();
    EXPECT_FALSE(GetDocument().documentElement()->GetPseudoElement(kPseudoIdPageTransition));
}
...
TEST_P(DocumentTransitionTest, InspectorStyleResolver)
{
    ...
    TestCase test_cases[] = { { kPseudoIdPageTransition, false, "::page-transition { background-color: red; }" },
        { kPseudoIdPageTransitionContainer, true, "::page-transition-container(foo) { background-color: blue; }" },
        { kPseudoIdPageTransitionImageWrapper, true, "::page-transition-image-wrapper(foo) { background-color: lightblue; }" },
        { kPseudoIdPageTransitionIncomingImage, true, "::page-transition-incoming-image(foo) { background-color: black; }" },
        { kPseudoIdPageTransitionOutgoingImage, true, "::page-transition-outgoing-image(foo) { background-color: grey; }" } };

    for (const auto& test_case : test_cases) {
        InspectorStyleResolver resolver(GetDocument().documentElement(), test_case.pseudo_id, test_case.uses_tags ? "foo" : g_null_atom);
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(matched_rules_for_pseudo->document_transition_tag, test_case.uses_tags ? "foo" : g_null_atom);
