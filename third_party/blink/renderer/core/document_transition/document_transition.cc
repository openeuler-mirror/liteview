...
const char kAbortedFromCallback[] = "Aborted due to failure in DocumentTransitionCallback";
const char kAbortedFromCallbackTimeout[] = "Aborted due to timeout in DocumentTransitionCallback";
const char kAbortedFromInvalidConfigAtStart[] = "Start failed: invalid element configuration";
...
DocumentTransition::PostCaptureResolved::PostCaptureResolved(DocumentTransition* transition, bool success, Document* document)
...
ScriptValue DocumentTransition::PostCaptureResolved::Call(ScriptState* script_state, ScriptValue value)
{
    if (transition_)
    ...
    if (!success_) {
        ...
        std::unique_ptr<SourceLocation> location = CaptureSourceLocation(isolate, message, document_->GetExecutionContext());
        ErrorEvent* error = ErrorEvent::Create(ToCoreStringWithNullCheck(message->Get()), std::move(location), value, &DOMWrapperWorld::MainWorld());
        document_->domWindow()->DispatchErrorEvent(error, SanitizeScriptErrors::kSanitize);
    }
    return ScriptValue();
}
...
bool DocumentTransition::StartNewTransition()
{
    ...
    style_tracker_ = MakeGarbageCollected<DocumentTransitionStyleTracker>(*document_);
    return true;
}

ScriptPromise DocumentTransition::start(ScriptState* script_state, ExceptionState& exception_state)
{
    return start(script_state, nullptr, exception_state);
}

ScriptPromise DocumentTransition::start(ScriptState* script_state, V8DocumentTransitionCallback* callback, ExceptionState& exception_state)
{
...
ScriptPromise DocumentTransition::prepare(ScriptState* script_state, ExceptionState& exception_state)
{
    return prepare(script_state, nullptr, exception_state);
}

ScriptPromise DocumentTransition::prepare(ScriptState* script_state, V8DocumentTransitionCallback* callback, ExceptionState& exception_state)
{
    bool success = InitiateTransition(script_state, callback, exception_state);
...
bool DocumentTransition::InitiateTransition(ScriptState* script_state, V8DocumentTransitionCallback* callback, ExceptionState& exception_state)
{
    switch (state_) {
    case State::kIdle:
        if (!document_ || !document_->View()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The document must be connected to a window.");
            return false;
        }
        if (!style_tracker_) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Transition is aborted.");
            return false;
        }
        break;
    ...
    case State::kStarted:
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Transition aborted, invalid captureAndHold call");
        return false;
    }
    ...
    if (!capture_succeeded) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Capture failed: invalid element configuration.");
        ResetTransitionState();
        return false;
    }
    ...
    finished_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    prepare_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver>(script_state);

    state_ = State::kCapturing;
    pending_request_ = DocumentTransitionRequest::CreateCapture(document_tag_, style_tracker_->CapturedTagCount(), style_tracker_->TakeCaptureResourceIds(),
        ConvertToBaseOnceCallback(
            CrossThreadBindOnce(&DocumentTransition::NotifyCaptureFinished, WrapCrossThreadWeakPersistent(this), last_prepare_sequence_id_)));
    ...
    if (document_->GetFrame()->IsLocalRoot()) {
        document_->GetPage()->GetChromeClient().StopDeferringCommits(*document_->GetFrame(), cc::PaintHoldingCommitTrigger::kDocumentTransition);
    }
...
{
    return finished_promise_resolver_ ? finished_promise_resolver_->Promise() : ScriptPromise();
}
...
void DocumentTransition::NotifyHasChangesToCommit()
{
    ...
    document_->View()->SetPaintArtifactCompositorNeedsUpdate(PaintArtifactCompositorUpdateReason::kDocumentTransitionNotifyChanges);
}
...
void DocumentTransition::NotifyCaptureFinished(uint32_t sequence_id)
{
    ...
    post_capture_success_callable_ = MakeGarbageCollected<PostCaptureResolved>(this, true, document_);
    post_capture_reject_callable_ = MakeGarbageCollected<PostCaptureResolved>(this, false, document_);

    ScriptState::Scope scope(start_script_state_);
    result.ToChecked().Then(MakeGarbageCollected<ScriptFunction>(start_script_state_, post_capture_success_callable_),
        MakeGarbageCollected<ScriptFunction>(start_script_state_, post_capture_reject_callable_));
...
void DocumentTransition::NotifyPostCaptureCallbackResolved(bool success)
{
    ...
    pending_request_ = DocumentTransitionRequest::CreateAnimateRenderer(document_tag_);
    NotifyHasChangesToCommit();
...
std::unique_ptr<DocumentTransitionRequest> DocumentTransition::TakePendingRequest()
{
    return std::move(pending_request_);
}

bool DocumentTransition::NeedsSharedElementEffectNode(const LayoutObject& object) const
{
...
bool DocumentTransition::IsRepresentedViaPseudoElements(const LayoutObject& object) const
{
    if (!style_tracker_)
        return false;
...
PaintPropertyChangeType DocumentTransition::UpdateEffect(const LayoutObject& object, const EffectPaintPropertyNodeOrAlias& current_effect,
    const ClipPaintPropertyNodeOrAlias* current_clip, const TransformPaintPropertyNodeOrAlias* current_transform)
{
    ...
    state.direct_compositing_reasons = CompositingReason::kDocumentTransitionSharedElement;
    state.local_transform_space = current_transform;
    state.output_clip = current_clip;
    state.document_transition_shared_element_id = DocumentTransitionSharedElementId(document_tag_);
    state.compositor_element_id = CompositorElementIdFromUniqueObjectId(object.UniqueId(), CompositorElementIdNamespace::kSharedElementTransition);
    auto* element = DynamicTo<Element>(object.GetNode());
    if (!element) {
        // The only non-element participant is the layout view.
        DCHECK(object.IsLayoutView());

        style_tracker_->UpdateRootIndexAndSnapshotId(state.document_transition_shared_element_id, state.shared_element_resource_id);
        DCHECK(state.document_transition_shared_element_id.valid() || !style_tracker_->IsRootTransitioning());
        return style_tracker_->UpdateRootEffect(std::move(state), current_effect);
    }

    style_tracker_->UpdateElementIndicesAndSnapshotId(element, state.document_transition_shared_element_id, state.shared_element_resource_id);
    return style_tracker_->UpdateEffect(element, std::move(state), current_effect);
}

EffectPaintPropertyNode* DocumentTransition::GetEffect(const LayoutObject& object) const
{
    DCHECK(NeedsSharedElementEffectNode(object));
...
{
    DCHECK(document_->Lifecycle().GetState() >= DocumentLifecycle::LifecycleState::kPrePaintClean);
    ...
    if (style_tracker_) {
        ...
        DCHECK(state_ != State::kStarted || finished_promise_resolver_ || disable_end_transition_);
        if (state_ == State::kStarted && !style_tracker_->HasActiveAnimations() && finished_promise_resolver_) {
...
PseudoElement* DocumentTransition::CreatePseudoElement(Element* parent, PseudoId pseudo_id, const AtomicString& document_transition_tag)
{
    DCHECK(style_tracker_);

    return style_tracker_->CreatePseudoElement(parent, pseudo_id, document_transition_tag);
}
...
void DocumentTransition::PauseRendering()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("blink", "DocumentTransition::PauseRendering", this);
    const base::TimeDelta kTimeout = [this]() {
        if (auto* settings = document_->GetFrame()->GetContentSettingsClient(); settings && settings->IncreaseSharedElementTransitionCallbackTimeout()) {
    ...
    document_->GetTaskRunner(TaskType::kInternalFrameLifecycleControl)
        ->PostDelayedTask(
            FROM_HERE, WTF::BindOnce(&DocumentTransition::OnRenderingPausedTimeout, WrapWeakPersistent(this), last_prepare_sequence_id_), kTimeout);
}
...
void DocumentTransition::ResumeRendering()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("blink", "DocumentTransition::PauseRendering", this);
...
void DocumentTransition::ResetScriptState(const char* abort_message)
{
    ...
    if (start_script_state_ && start_script_state_->ContextIsValid()) {
        auto finalize = [this, &abort_message](ScriptPromiseResolver* resolver) {
            ...
            ScriptState::Scope scope(start_script_state_);
            if (abort_message) {
                resolver->Reject(V8ThrowDOMException::CreateOrDie(resolver->GetScriptState()->GetIsolate(), DOMExceptionCode::kAbortError, abort_message));
            } else {
                resolver->Detach();
            }
        };
