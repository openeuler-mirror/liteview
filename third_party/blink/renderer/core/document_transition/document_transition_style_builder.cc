...
void DocumentTransitionStyleBuilder::AddSelector(const String& name, const String& tag)
{
...
void DocumentTransitionStyleBuilder::AddRules(const String& selector, const String& tag, const String& rules)
{
...
void DocumentTransitionStyleBuilder::AddAnimationAndBlending(const String& tag, const ContainerProperties& source_properties)
{
...
String DocumentTransitionStyleBuilder::AddKeyframes(const String& tag, const ContainerProperties& source_properties)
{
    ...
    builder_.AppendFormat(
        ...
        ComputedStyleUtils::ValueForTransformationMatrix(source_properties.snapshot_matrix, 1, false)->CssText().Utf8().c_str(),
        source_properties.border_box_size_in_css_space.Width().ToFloat(), source_properties.border_box_size_in_css_space.Height().ToFloat());
    return keyframe_name;
}

void DocumentTransitionStyleBuilder::AddIncomingObjectViewBox(const String& tag, const String& value)
{
    AddObjectViewBox(kIncomingImageTagName, tag, value);
}

void DocumentTransitionStyleBuilder::AddOutgoingObjectViewBox(const String& tag, const String& value)
{
    AddObjectViewBox(kOutgoingImageTagName, tag, value);
}

void DocumentTransitionStyleBuilder::AddObjectViewBox(const String& selector, const String& tag, const String& value)
{
    StringBuilder rule_builder;
...
void DocumentTransitionStyleBuilder::AddContainerStyles(const String& tag, const String& rules)
{
    AddRules(kContainerTagName, tag, rules);
}

void DocumentTransitionStyleBuilder::AddContainerStyles(const String& tag, const ContainerProperties& properties, WritingMode writing_mode)
{
    ...
    rule_builder.AppendFormat(
        ...
        properties.border_box_size_in_css_space.Width().ToFloat(), properties.border_box_size_in_css_space.Height().ToFloat(),
        ComputedStyleUtils::ValueForTransformationMatrix(properties.snapshot_matrix, 1, false)->CssText().Utf8().c_str(), writing_mode_stream.str().c_str());
