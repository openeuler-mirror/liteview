...
// static
const char DocumentTransitionSupplement::kSupplementName[] = "DocumentTransition";

// static
DocumentTransitionSupplement* DocumentTransitionSupplement::FromIfExists(const Document& document)
{
    return Supplement<Document>::From<DocumentTransitionSupplement>(document);
}

// static
DocumentTransitionSupplement* DocumentTransitionSupplement::From(Document& document)
{
    auto* supplement = Supplement<Document>::From<DocumentTransitionSupplement>(document);
    if (!supplement) {
...
// static
DocumentTransition* DocumentTransitionSupplement::EnsureDocumentTransition(Document& document)
{
...
// static
DocumentTransition* DocumentTransitionSupplement::createDocumentTransition(Document& document, ExceptionState& exception_state)
{
    auto* transition = EnsureDocumentTransition(document);

    if (!transition->StartNewTransition()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Transition already in progress");
        return nullptr;
    }
