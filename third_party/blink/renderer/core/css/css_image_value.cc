...
CSSImageValue::CSSImageValue(
    const AtomicString& raw_value, const KURL& url, const Referrer& referrer, OriginClean origin_clean, bool is_ad_related, StyleImage* image)
    ...
    const Document& document, FetchParameters::ImageRequestBehavior image_request_behavior, CrossOriginAttributeValue cross_origin) const
{
    ...
    bool is_lazily_loaded = image_request_behavior == FetchParameters::ImageRequestBehavior::kDeferImageLoad &&
...
StyleImage* CSSImageValue::CacheImage(
    const Document& document, FetchParameters::ImageRequestBehavior image_request_behavior, CrossOriginAttributeValue cross_origin)
{
    if (!cached_image_) {
        ...
        cached_image_ = document.GetStyleEngine().CacheStyleImage(params, origin_clean_, is_ad_related_);
    }
...
void CSSImageValue::RestoreCachedResourceIfNeeded(const Document& document) const
{
    ...
    cached_content->EmulateLoadStartedForInspector(
        document.Fetcher(), KURL(absolute_url_), initiator_name_.empty() ? fetch_initiator_type_names::kCSS : initiator_name_);
}
