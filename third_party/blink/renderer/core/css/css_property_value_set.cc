...
ImmutableCSSPropertyValueSet* ImmutableCSSPropertyValueSet::Create(const CSSPropertyValue* properties, unsigned count, CSSParserMode css_parser_mode)
{
    DCHECK_LE(count, static_cast<unsigned>(kMaxArraySize));
    return MakeGarbageCollected<ImmutableCSSPropertyValueSet>(
        AdditionalBytesForImmutableCSSPropertyValueSetWithPropertyCount(count), properties, count, css_parser_mode);
}
...
ImmutableCSSPropertyValueSet* CSSPropertyValueSet::ImmutableCopyIfNeeded() const
{
    ...
    return ImmutableCSSPropertyValueSet::Create(mutable_this->property_vector_.data(), mutable_this->property_vector_.size(), CssParserMode());
}
    ...
    : CSSPropertyValueSet(kHTMLStandardMode)
{
    ...
    for (unsigned i = 0; i < length; ++i) {
        ...
        if (!may_have_logical_properties_) {
            ...
            may_have_logical_properties_ = prop.IsInLogicalPropertyGroup() && prop.IsSurrogate();
        }
    }
}

ImmutableCSSPropertyValueSet::ImmutableCSSPropertyValueSet(const CSSPropertyValue* properties, unsigned length, CSSParserMode css_parser_mode)
    : CSSPropertyValueSet(css_parser_mode, length)
{
    CSSPropertyValueMetadata* metadata_array = const_cast<CSSPropertyValueMetadata*>(MetadataArray());
    Member<const CSSValue>* value_array = const_cast<Member<const CSSValue>*>(ValueArray());
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetProperty(
    CSSPropertyID unresolved_property, const String& value, bool important, SecureContextMode secure_context_mode, StyleSheetContents* context_style_sheet)
{
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetProperty(const AtomicString& custom_property_name, const String& value, bool important,
    SecureContextMode secure_context_mode, StyleSheetContents* context_style_sheet, bool is_animation_tainted)
{
    if (value.empty()) {
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetProperty(const CSSPropertyValue& property, CSSPropertyValue* slot)
{
    CSSPropertyValue* to_replace = slot ? slot : FindCSSPropertyWithName(property.Name());
    if (to_replace) {
        if (may_have_logical_properties_) {
            const CSSProperty& prop = CSSProperty::Get(property.Id());
            if (prop.IsInLogicalPropertyGroup()) {
                DCHECK(property_vector_.Contains(*to_replace));
                int to_replace_index = static_cast<int>(to_replace - property_vector_.begin());
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetProperty(CSSPropertyID property_id, CSSValueID identifier, bool important)
{
    CSSPropertyName name(property_id);
    return SetProperty(CSSPropertyValue(name, *CSSIdentifierValue::Create(identifier), important));
}
...
void MutableCSSPropertyValueSet::MergeAndOverrideOnConflict(const CSSPropertyValueSet* other)
{
    ...
    for (unsigned n = 0; n < size; ++n) {
        ...
        if (old) {
            SetProperty(CSSPropertyValue(to_merge.PropertyMetadata(), to_merge.Value()), old);
        } else {
            property_vector_.push_back(CSSPropertyValue(to_merge.PropertyMetadata(), to_merge.Value()));
        }
    }
}
...
{
    int found_property_index = name.IsCustomProperty() ? FindPropertyIndex(name.ToAtomicString()) : FindPropertyIndex(name.Id());
    if (found_property_index == -1)
        return nullptr;
...
MutableCSSPropertyValueSet* CSSPropertyValueSet::CopyPropertiesInSet(const Vector<const CSSProperty*>& properties) const
{
    ...
    return MakeGarbageCollected<MutableCSSPropertyValueSet>(list.data(), list.size());
}
...
template <typename T> int MutableCSSPropertyValueSet::FindPropertyIndex(const T& property) const
{
    ...
    const CSSPropertyValue* it = std::find_if(
        begin, end, [property, id](const CSSPropertyValue& css_property) -> bool { return IsPropertyMatch(css_property.Metadata(), id, property); });
