...
public:
    ...
    CSSToLengthConversionData ConversionData(absl::optional<float> css_zoom = absl::nullopt, absl::optional<float> data_zoom = absl::nullopt)
    {
        ...
        return CSSToLengthConversionData(div->GetComputedStyle(), GetDocument().body()->GetComputedStyle(), root->GetComputedStyle(),
            GetDocument().GetLayoutView(), CSSToLengthConversionData::ContainerSizes(), data_zoom.value_or(div->GetComputedStyle()->EffectiveZoom()));
    }

    float Convert(const CSSToLengthConversionData& data, String value)
    {
        auto* primitive_value = DynamicTo<CSSPrimitiveValue>(css_test_helpers::ParseValue(GetDocument(), "<length>", value));
