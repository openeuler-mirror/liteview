...
{
    return RuntimeEnabledFeatures::CSSOverflowForReplacedElementsEnabled() ? UncompressResourceAsASCIIString(IDR_UASTYLE_OVERFLOW_REPLACED_CSS) : "";
}
    ...
    : media_controls_style_sheet_loader_(nullptr)
{
    ...
    String default_rules
        = UncompressResourceAsASCIIString(IDR_UASTYLE_HTML_CSS) + OverflowForReplacedElementRules() + LayoutTheme::GetTheme().ExtraDefaultStyleSheet();
...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetsForElement(const Element& element)
{
    ...
    if (element.IsSVGElement() && !svg_style_sheet_) {
        svg_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_SVG_CSS) + OverflowForSVGRules());
    ...
    if (element.namespaceURI() == mathml_names::kNamespaceURI && !mathml_style_sheet_) {
        mathml_style_sheet_ = ParseUASheet(RuntimeEnabledFeatures::MathMLCoreEnabled() ? UncompressResourceAsASCIIString(IDR_UASTYLE_MATHML_CSS)
    ...
    if (!popup_style_sheet_ && element.HasPopupAttribute()) {
        ...
        DCHECK(RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(element.GetDocument().GetExecutionContext()));
        popup_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_POPUP_CSS));
    ...
    if (!selectmenu_style_sheet_ && IsA<HTMLSelectMenuElement>(element)) {
        ...
        selectmenu_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_SELECTMENU_CSS));
...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetForXrOverlay()
{
    ...
    webxr_overlay_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_WEBXR_OVERLAY_CSS));
    AddRulesToDefaultStyleSheets(webxr_overlay_style_sheet_, NamespaceType::kHTML);
    return true;
}
...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetForForcedColors()
{
    ...
    String forced_colors_rules
        = RuntimeEnabledFeatures::ForcedColorsEnabled() ? UncompressResourceAsASCIIString(IDR_UASTYLE_THEME_FORCED_COLORS_CSS) : String();
    ...
    if (media_controls_style_sheet_) {
        default_forced_color_style_->AddRulesFromSheet(MediaControlsStyleSheet(), ForcedColorsEval());
    }
    if (webxr_overlay_style_sheet_) {
        default_forced_color_style_->AddRulesFromSheet(webxr_overlay_style_sheet_, ForcedColorsEval());
    }
