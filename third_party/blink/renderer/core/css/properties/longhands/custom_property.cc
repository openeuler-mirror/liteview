...
void CustomProperty::ApplyInitial(StyleResolverState& state) const
{
    ...
    state.Style()->SetVariableData(name_, initial_variable_data, is_inherited_property);
...
void CustomProperty::ApplyValue(StyleResolverState& state, const CSSValue& value) const
{
    ...
    if (registration_) {
        ...
        const CSSValue* registered_value = ParseSingleValue(range, *context, local_context);
        if (!registered_value) {
        ...
        data = StyleBuilderConverter::ConvertRegisteredPropertyVariableData(*registered_value, data->IsAnimationTainted());

        state.Style()->SetVariableData(name_, data, is_inherited_property);
        state.Style()->SetVariableValue(name_, registered_value, is_inherited_property);
    }
}

const CSSValue* CustomProperty::ParseSingleValue(CSSParserTokenRange& range, const CSSParserContext& context, const CSSParserLocalContext& local_context) const
{
...
const CSSValue* CustomProperty::CSSValueFromComputedStyleInternal(const ComputedStyle& style, const LayoutObject*, bool allow_visited_style) const
{
    if (registration_) {
    ...
    return MakeGarbageCollected<CSSCustomPropertyDeclaration>(data, /* parser_context */ nullptr);
}

const CSSValue* CustomProperty::ParseUntyped(CSSParserTokenRange range, const CSSParserContext& context, const CSSParserLocalContext& local_context) const
{
    // TODO(crbug.com/661854): Pass through the original string when we have it.
    return CSSVariableParser::ParseDeclarationValue({ range, StringView() }, local_context.IsAnimationTainted(), context);
}

const CSSValue* CustomProperty::ParseTyped(CSSParserTokenRange range, const CSSParserContext& context, const CSSParserLocalContext& local_context) const
{
    if (!registration_)
        return ParseUntyped(range, context, local_context);
    return registration_->Syntax().Parse(range, context, local_context.IsAnimationTainted());
}
