...
public:
    ...
    {
        return property.CSSValueFromComputedStyle(GetComputedStyle(), nullptr /* layout_object */, false /* allow_visited_style */);
    }

    const CSSValue* ParseValue(const Longhand& property, const String& value, const CSSParserLocalContext& local_context)
    {
...
TEST_F(CustomPropertyTest, ParseSingleValueAnimationTainted)
{
    ...
    EXPECT_TRUE(To<CSSCustomPropertyDeclaration>(value1)->Value().IsAnimationTainted());
    EXPECT_FALSE(To<CSSCustomPropertyDeclaration>(value2)->Value().IsAnimationTainted());
}
...
TEST_F(CustomPropertyTest, ParseSingleValueUntyped)
{
    RegisterProperty(GetDocument(), "--x", "<length>", "0px", false);
    CustomProperty property("--x", GetDocument());
    const CSSValue* value = ParseValue(property, "maroon", CSSParserLocalContext().WithVariableMode(VariableMode::kUntyped));
    ASSERT_TRUE(value->IsCustomPropertyDeclaration());
...
TEST_F(CustomPropertyTest, ParseSingleValueValidatedUntyped)
{
    RegisterProperty(GetDocument(), "--x", "<length>", "0px", false);
    CustomProperty property("--x", GetDocument());
    auto local_context = CSSParserLocalContext().WithVariableMode(VariableMode::kValidatedUntyped);
