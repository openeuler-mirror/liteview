...
const CSSProperty& CSSDirectionAwareResolver::ResolveInlineStart(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
...
const CSSProperty& CSSDirectionAwareResolver::ResolveInlineEnd(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (direction == TextDirection::kLtr) {
        if (IsHorizontalWritingMode(writing_mode))
            return group.GetProperty(kRightSide);
...
const CSSProperty& CSSDirectionAwareResolver::ResolveBlockStart(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (IsHorizontalWritingMode(writing_mode))
        return group.GetProperty(kTopSide);
...
const CSSProperty& CSSDirectionAwareResolver::ResolveBlockEnd(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (IsHorizontalWritingMode(writing_mode))
        return group.GetProperty(kBottomSide);
    if (IsFlippedLinesWritingMode(writing_mode))
...
const CSSProperty& CSSDirectionAwareResolver::ResolveInline(TextDirection, WritingMode writing_mode, const PhysicalMapping<2>& group)
{
    if (IsHorizontalWritingMode(writing_mode))
...
const CSSProperty& CSSDirectionAwareResolver::ResolveBlock(TextDirection, WritingMode writing_mode, const PhysicalMapping<2>& group)
{
    if (IsHorizontalWritingMode(writing_mode))
        return group.GetProperty(kPhysicalAxisY);
    return group.GetProperty(kPhysicalAxisX);
...
const CSSProperty& CSSDirectionAwareResolver::ResolveStartStart(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (direction == TextDirection::kLtr) {
        if (IsHorizontalWritingMode(writing_mode) || IsFlippedLinesWritingMode(writing_mode))
...
const CSSProperty& CSSDirectionAwareResolver::ResolveStartEnd(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (direction == TextDirection::kLtr) {
        if (IsHorizontalWritingMode(writing_mode))
            return group.GetProperty(kTopRightCorner);
...
const CSSProperty& CSSDirectionAwareResolver::ResolveEndStart(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (direction == TextDirection::kLtr) {
        if (IsHorizontalWritingMode(writing_mode))
            return group.GetProperty(kBottomLeftCorner);
...
const CSSProperty& CSSDirectionAwareResolver::ResolveEndEnd(TextDirection direction, WritingMode writing_mode, const PhysicalMapping<4>& group)
{
    if (direction == TextDirection::kLtr) {
        if (IsHorizontalWritingMode(writing_mode) || IsFlippedLinesWritingMode(writing_mode))
            return group.GetProperty(kBottomRightCorner);
