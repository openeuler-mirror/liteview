...
public:
    ...
    scoped_refptr<ComputedStyle> ComputedStyleWithValue(const CSSProperty& property, const CSSValue& value)
    {
        ...
        StyleBuilder::ApplyProperty(property, state, ScopedCSSValue(value, &GetDocument()));
...
{
    scoped_refptr<ComputedStyle> initial_style = GetDocument().GetStyleResolver().CreateComputedStyle();
    ...
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        const CSSValue* initial_value = property.CSSValueFromComputedStyle(*initial_style, nullptr /* layout_object */, false /* allow_visited_style */);
...
{
    EXPECT_EQ(&GetCSSPropertyWidth(), GetCSSPropertyInlineSize().SurrogateFor(TextDirection::kLtr, WritingMode::kHorizontalTb));
    EXPECT_EQ(&GetCSSPropertyHeight(), GetCSSPropertyInlineSize().SurrogateFor(TextDirection::kLtr, WritingMode::kVerticalRl));
    EXPECT_EQ(&GetCSSPropertyWritingMode(), GetCSSPropertyWebkitWritingMode().SurrogateFor(TextDirection::kLtr, WritingMode::kHorizontalTb));
    EXPECT_FALSE(GetCSSPropertyWidth().SurrogateFor(TextDirection::kLtr, WritingMode::kHorizontalTb));
}
