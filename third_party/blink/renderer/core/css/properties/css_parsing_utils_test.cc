...
double ConsumeAngleValue(String target, double min, double max)
{
    auto tokens = CSSTokenizer(target).TokenizeToEOF();
    CSSParserTokenRange range(tokens);
    return ConsumeAngle(range, *MakeContext(), absl::nullopt, min, max)->ComputeDegrees();
}
...
{
    auto ConsumeColorForTest = [](String css_text, css_parsing_utils::AllowedColorKeywords allowed_keywords) {
    ...
    for (auto& expectation : expectations) {
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, AllowedColorKeywords::kAllowSystemColor), expectation.allowed_expectation);
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, AllowedColorKeywords::kNoSystemColor), expectation.not_allowed_expectation);
    }
}
