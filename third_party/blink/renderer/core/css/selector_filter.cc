...
inline void CollectElementIdentifierHashes(const Element& element, Vector<unsigned, 4>& identifier_hashes)
{
    identifier_hashes.push_back(element.LocalNameForSelectorMatching().Impl()->ExistingHash() * kTagNameSalt);
    if (element.HasID()) {
        identifier_hashes.push_back(element.IdForStyleResolution().Impl()->ExistingHash() * kIdSalt);
    }
    ...
    if (element.IsStyledElement() && element.HasClass()) {
        ...
        for (wtf_size_t i = 0; i < count; ++i) {
            identifier_hashes.push_back(class_names[i].Impl()->ExistingHash() * kClassSalt);
        }
    }
...
inline void CollectDescendantSelectorIdentifierHashes(const CSSSelector& selector, unsigned*& hash)
{
    ...
    case CSSSelector::kTag:
        if (selector.TagQName().LocalName() != CSSSelector::UniversalSelectorAtom()) {
            (*hash++) = selector.TagQName().LocalName().Impl()->ExistingHash() * kTagNameSalt;
        }
        break;
...
void SelectorFilter::PushParentStackFrame(Element& parent)
{
    ...
    DCHECK(parent_stack_.empty() || parent_stack_.back().element == FlatTreeTraversal::ParentElement(parent));
...
void SelectorFilter::CollectIdentifierHashes(const CSSSelector& selector, unsigned* identifier_hashes, unsigned maximum_identifier_count)
{
