...
protected:
    ...
    CSSPropertyValueSet(CSSParserMode css_parser_mode, unsigned immutable_array_size)
...
public:
    ImmutableCSSPropertyValueSet(const CSSPropertyValue*, unsigned count, CSSParserMode);

    static ImmutableCSSPropertyValueSet* Create(const CSSPropertyValue* properties, unsigned count, CSSParserMode);
...
inline const CSSPropertyValueMetadata* ImmutableCSSPropertyValueSet::MetadataArray() const
{
    ...
    return reinterpret_cast<const CSSPropertyValueMetadata*>(
        base::bits::AlignUp(reinterpret_cast<const uint8_t*>(ValueArray() + array_size_), alignof(CSSPropertyValueMetadata)));
}
...
public:
    ...
    SetResult SetProperty(
        CSSPropertyID unresolved_property, const String& value, bool important, SecureContextMode, StyleSheetContents* context_style_sheet = nullptr);
    SetResult SetProperty(const AtomicString& custom_property_name, const String& value, bool important, SecureContextMode,
    ...
    SetResult SetProperty(CSSPropertyID, CSSValueID identifier, bool important = false);
    SetResult SetProperty(const CSSPropertyValue&, CSSPropertyValue* slot = nullptr);
