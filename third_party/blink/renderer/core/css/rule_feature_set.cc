...
bool SupportsInvalidationWithSelectorList(CSSSelector::PseudoType pseudo)
{
    return pseudo == CSSSelector::kPseudoAny || pseudo == CSSSelector::kPseudoCue || pseudo == CSSSelector::kPseudoHost
        ...
        || pseudo == CSSSelector::kPseudoSlotted || pseudo == CSSSelector::kPseudoWhere;
}
...
void ExtractInvalidationSets(InvalidationSet* invalidation_set, DescendantInvalidationSet*& descendants, SiblingInvalidationSet*& siblings)
{
...
InvalidationSet& RuleFeatureSet::EnsureMutableInvalidationSet(InvalidationType type, PositionType position, scoped_refptr<InvalidationSet>& invalidation_set)
{
    if (!invalidation_set) {
    ...
    if (invalidation_set->IsSelfInvalidationSet() && type == InvalidationType::kInvalidateDescendants && position == kSubject) {
...
InvalidationSet& RuleFeatureSet::EnsureInvalidationSet(InvalidationSetMap& map, const AtomicString& key, InvalidationType type, PositionType position)
{
...
InvalidationSet& RuleFeatureSet::EnsureInvalidationSet(
    PseudoTypeInvalidationSetMap& map, CSSSelector::PseudoType key, InvalidationType type, PositionType position)
{
    scoped_refptr<InvalidationSet>& invalidation_set = map.insert(key, nullptr).stored_value->value;
    return EnsureMutableInvalidationSet(type, position, invalidation_set);
}

void RuleFeatureSet::MergeInvalidationSet(InvalidationSetMap& map, const AtomicString& key, scoped_refptr<InvalidationSet> invalidation_set)
{
    ...
    if (!slot) {
        ...
        EnsureMutableInvalidationSet(invalidation_set->GetType(), invalidation_set->IsSelfInvalidationSet() ? kSubject : kAncestor, slot)
            .Combine(*invalidation_set);
    }
}

void RuleFeatureSet::MergeInvalidationSet(PseudoTypeInvalidationSetMap& map, CSSSelector::PseudoType key, scoped_refptr<InvalidationSet> invalidation_set)
{
    DCHECK(invalidation_set);
    scoped_refptr<InvalidationSet>& slot = map.insert(key, nullptr).stored_value->value;
    if (!slot) {
...
{
    return metadata_ == other.metadata_ && InvalidationSetMapsEqual<AtomicString>(class_invalidation_sets_, other.class_invalidation_sets_)
        && base::ValuesEquivalent(names_with_self_invalidation_, other.names_with_self_invalidation_)
        && InvalidationSetMapsEqual<AtomicString>(id_invalidation_sets_, other.id_invalidation_sets_)
        && InvalidationSetMapsEqual<AtomicString>(attribute_invalidation_sets_, other.attribute_invalidation_sets_)
        && InvalidationSetMapsEqual<CSSSelector::PseudoType>(pseudo_invalidation_sets_, other.pseudo_invalidation_sets_)
        && base::ValuesEquivalent(universal_sibling_invalidation_set_, other.universal_sibling_invalidation_set_)
        && base::ValuesEquivalent(nth_invalidation_set_, other.nth_invalidation_set_)
        && base::ValuesEquivalent(universal_sibling_invalidation_set_, other.universal_sibling_invalidation_set_)
        && base::ValuesEquivalent(type_rule_invalidation_set_, other.type_rule_invalidation_set_)
        && media_query_result_flags_ == other.media_query_result_flags_ && classes_in_has_argument_ == other.classes_in_has_argument_
        && attributes_in_has_argument_ == other.attributes_in_has_argument_ && ids_in_has_argument_ == other.ids_in_has_argument_
        && tag_names_in_has_argument_ == other.tag_names_in_has_argument_ && universal_in_has_argument_ == other.universal_in_has_argument_
        && not_pseudo_in_has_argument_ == other.not_pseudo_in_has_argument_ && pseudos_in_has_argument_ == other.pseudos_in_has_argument_;
}

ALWAYS_INLINE InvalidationSet& RuleFeatureSet::EnsureClassInvalidationSet(const AtomicString& class_name, InvalidationType type, PositionType position)
{
    CHECK(!class_name.empty());
    return EnsureInvalidationSet(class_invalidation_sets_, class_name, type, position);
}

ALWAYS_INLINE InvalidationSet& RuleFeatureSet::EnsureAttributeInvalidationSet(const AtomicString& attribute_name, InvalidationType type, PositionType position)
{
    CHECK(!attribute_name.empty());
    return EnsureInvalidationSet(attribute_invalidation_sets_, attribute_name, type, position);
}

ALWAYS_INLINE InvalidationSet& RuleFeatureSet::EnsureIdInvalidationSet(const AtomicString& id, InvalidationType type, PositionType position)
{
    CHECK(!id.empty());
    return EnsureInvalidationSet(id_invalidation_sets_, id, type, position);
}

ALWAYS_INLINE InvalidationSet& RuleFeatureSet::EnsurePseudoInvalidationSet(CSSSelector::PseudoType pseudo_type, InvalidationType type, PositionType position)
{
    CHECK_NE(pseudo_type, CSSSelector::kPseudoUnknown);
    return EnsureInvalidationSet(pseudo_invalidation_sets_, pseudo_type, type, position);
}

void RuleFeatureSet::UpdateFeaturesFromCombinator(CSSSelector::RelationType combinator, const CSSSelector* last_compound_in_adjacent_chain,
    InvalidationSetFeatures& last_compound_in_adjacent_chain_features, InvalidationSetFeatures*& sibling_features, InvalidationSetFeatures& descendant_features,
        ...
        if (!sibling_features) {
            ...
            if (last_compound_in_adjacent_chain) {
                ExtractInvalidationSetFeaturesFromCompound(
                    *last_compound_in_adjacent_chain, last_compound_in_adjacent_chain_features, kAncestor, for_logical_combination_in_has);
...
void RuleFeatureSet::UpdateFeaturesFromStyleScope(const StyleScope& style_scope, InvalidationSetFeatures& descendant_features)
{
    for (const StyleScope* scope = &style_scope; scope; scope = scope->Parent()) {
        for (const CSSSelector* selector = scope->From().First(); selector; selector = CSSSelectorList::Next(*selector)) {
...
bool RuleFeatureSet::InsertIntoSelfInvalidationBloomFilter(const AtomicString& value, int salt)
{
    if (!base::FeatureList::IsEnabled(blink::features::kInvalidationSetClassBloomFilter)) {
        return false;
    }
...
InvalidationSet* RuleFeatureSet::InvalidationSetForSimpleSelector(const CSSSelector& selector, InvalidationType type, PositionType position)
{
    if (selector.Match() == CSSSelector::kClass) {
        if (type == InvalidationType::kInvalidateDescendants && position == kSubject && InsertIntoSelfInvalidationBloomFilter(selector.Value(), kClassSalt)) {
    ...
    if (selector.IsAttributeSelector()) {
        return &EnsureAttributeInvalidationSet(selector.Attribute().LocalName(), type, position);
    }
    if (selector.Match() == CSSSelector::kId) {
        if (type == InvalidationType::kInvalidateDescendants && position == kSubject && InsertIntoSelfInvalidationBloomFilter(selector.Value(), kIdSalt)) {
    ...
    if (selector.Match() == CSSSelector::kPseudoClass) {
        ...
        case CSSSelector::kPseudoSelectorFragmentAnchor:
        case CSSSelector::kPseudoToggle:
            return &EnsurePseudoInvalidationSet(selector.GetPseudoType(), type, position);
        case CSSSelector::kPseudoFirstOfType:
        case CSSSelector::kPseudoLastOfType:
        case CSSSelector::kPseudoOnlyOfType:
        case CSSSelector::kPseudoNthChild:
        ...
        case CSSSelector::kPseudoHas:
            return position == kAncestor ? &EnsurePseudoInvalidationSet(selector.GetPseudoType(), type, position) : nullptr;
        case CSSSelector::kPseudoPart:
        default:
            break;
        }
...
void RuleFeatureSet::UpdateInvalidationSets(const CSSSelector& selector, const StyleScope* style_scope)
{
    InvalidationSetFeatures features;
    FeatureInvalidationType feature_invalidation_type
        = UpdateInvalidationSetsForComplex(selector, style_scope, features, kSubject, CSSSelector::kPseudoUnknown);
...
// (e.g. for :not() or :has()).
RuleFeatureSet::FeatureInvalidationType RuleFeatureSet::UpdateInvalidationSetsForComplex(
    const CSSSelector& complex, const StyleScope* style_scope, InvalidationSetFeatures& features, PositionType position, CSSSelector::PseudoType pseudo_type)
{
    ...
    // Step 2.
    const CSSSelector* next_compound = last_in_compound ? last_in_compound->TagHistory() : &complex;
...
void RuleFeatureSet::UpdateRuleSetInvalidation(const InvalidationSetFeatures& features)
{
    if (features.has_features_for_rule_set_invalidation)
        return;
    if (features.invalidation_flags.WholeSubtreeInvalid() || (!features.invalidation_flags.InvalidateCustomPseudo() && features.tag_names.empty())) {
...
void RuleFeatureSet::ExtractInvalidationSetFeaturesFromSelectorList(
    const CSSSelector& simple_selector, InvalidationSetFeatures& features, PositionType position)
{
    ...
    for (; sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
        ...
        if (UpdateInvalidationSetsForComplex(*sub_selector, nullptr /* style_scope */, complex_features, position, pseudo_type)
            == kRequiresSubtreeInvalidation) {
            features.invalidation_flags.SetWholeSubtreeInvalid(true);
            continue;
        }
        all_sub_selectors_have_features_for_ruleset_invalidation &= complex_features.has_features_for_rule_set_invalidation;
    ...
    if (pseudo_type != CSSSelector::kPseudoNot) {
        ...
        features.has_features_for_rule_set_invalidation |= all_sub_selectors_have_features_for_ruleset_invalidation;
    }
}
...
const CSSSelector* RuleFeatureSet::ExtractInvalidationSetFeaturesFromCompound(
    const CSSSelector& compound, InvalidationSetFeatures& features, PositionType position, bool for_logical_combination_in_has)
{
    ...
    for (;; simple_selector = simple_selector->TagHistory()) {
        ...
        if (InvalidationSet* invalidation_set = InvalidationSetForSimpleSelector(*simple_selector, InvalidationType::kInvalidateDescendants, position)) {
        ...
        ExtractInvalidationSetFeaturesFromSelectorList(*simple_selector, features, position);
        ...
        if (simple_selector->GetPseudoType() == CSSSelector::kPseudoHas && !for_logical_combination_in_has) {
            ...
            AddFeaturesToInvalidationSetsForHasPseudoClass(*simple_selector, &compound, nullptr, features);
        }

        if (!simple_selector->TagHistory() || simple_selector->Relation() != CSSSelector::kSubSelector) {
            return simple_selector;
        }
    }
}

void RuleFeatureSet::CollectValuesInHasArgument(const CSSSelector& has_pseudo_class)
{
...
void RuleFeatureSet::AddFeaturesToInvalidationSetsForHasPseudoClass(const CSSSelector& pseudo_has, const CSSSelector* compound_containing_has,
    InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    for (const CSSSelector* relative = pseudo_has.SelectorList()->First(); relative; relative = CSSSelectorList::Next(*relative)) {
        for (const CSSSelector* simple = relative; simple->GetPseudoType() != CSSSelector::kPseudoRelativeAnchor; simple = simple->TagHistory()) {
...
const CSSSelector* RuleFeatureSet::SkipAddingAndGetLastInCompoundForLogicalCombinationInHas(const CSSSelector* compound_in_logical_combination,
...
const CSSSelector* RuleFeatureSet::AddFeaturesAndGetLastInCompoundForLogicalCombinationInHas(const CSSSelector* compound_in_logical_combination,
    const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features,
    CSSSelector::RelationType previous_combinator, AddFeaturesMethodForLogicalCombinationInHas add_features_method)
{
    ...
    for (; simple; simple = simple->TagHistory()) {
        ...
        case CSSSelector::kPseudoNot:
            ...
            break;
        default:
            AddFeaturesToInvalidationSetsForSimpleSelector(*simple, *compound_in_logical_combination, sibling_features, descendant_features);
            break;
        }
...
struct AddFeaturesToInvalidationSetsForLogicalCombinationInHasContext {
    ...
    AddFeaturesToInvalidationSetsForLogicalCombinationInHasContext(const CSSSelector* compound_in_logical_combination,
        ...
        RuleFeatureSet::AddFeaturesMethodForLogicalCombinationInHas add_features_method)
    {
        ...
        bool add_features_for_compound_immediately_follows_adjacent_relation
            = add_features_method == RuleFeatureSet::kForCompoundImmediatelyFollowsAdjacentRelation;
...
void RuleFeatureSet::AddFeaturesToInvalidationSetsForLogicalCombinationInHas(const CSSSelector& logical_combination, const CSSSelector* compound_containing_has,
    ...
    AddFeaturesMethodForLogicalCombinationInHas add_features_method)
{
    ...
    for (const CSSSelector* complex = logical_combination.SelectorList()->First(); complex; complex = CSSSelectorList::Next(*complex)) {
        ...
        while (compound_in_logical_combination) {
            ...
            if (context.needs_skip_adding_features) {
                last_in_compound = SkipAddingAndGetLastInCompoundForLogicalCombinationInHas(
                    compound_in_logical_combination, compound_containing_has, sibling_features, descendant_features, previous_combinator, add_features_method);
            } else {
                last_in_compound = AddFeaturesAndGetLastInCompoundForLogicalCombinationInHas(
                    compound_in_logical_combination, compound_containing_has, sibling_features, descendant_features, previous_combinator, add_features_method);
            }
            ...
            if (context.needs_update_features) {
                UpdateFeaturesFromCombinatorForLogicalCombinationInHas(
                    ...
                    context.last_compound_in_adjacent_chain, last_compound_in_adjacent_chain_features, sibling_features, descendant_features);
            }
...
void RuleFeatureSet::UpdateFeaturesFromCombinatorForLogicalCombinationInHas(CSSSelector::RelationType combinator,
...
void RuleFeatureSet::AddValuesInComplexSelectorInsideIsWhereNot(const CSSSelectorList* selector_list)
{
    DCHECK(selector_list);
    for (const CSSSelector* complex = selector_list->First(); complex; complex = CSSSelectorList::Next(*complex)) {
        DCHECK(complex);

        for (const CSSSelector* simple = complex; simple; simple = simple->TagHistory()) {
            AddValueOfSimpleSelectorInHasArgument(*simple);
        }
    }
}

bool RuleFeatureSet::AddValueOfSimpleSelectorInHasArgument(const CSSSelector& selector)
{
    if (selector.Match() == CSSSelector::kClass) {
...
void RuleFeatureSet::AddFeaturesToInvalidationSet(InvalidationSet& invalidation_set, const InvalidationSetFeatures& features)
{
...
void RuleFeatureSet::AddFeaturesToInvalidationSetsForSelectorList(
    const CSSSelector& simple_selector, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    for (const CSSSelector* sub_selector = simple_selector.SelectorList()->First(); sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
        AutoRestoreMaxDirectAdjacentSelectors restore_max(sibling_features);
        AutoRestoreDescendantFeaturesDepth restore_depth(&descendant_features);
        AutoRestoreTreeBoundaryCrossingFlag restore_tree_boundary(descendant_features);
        AutoRestoreInsertionPointCrossingFlag restore_insertion_point(descendant_features);
        ...
        AddFeaturesToInvalidationSets(*sub_selector, sibling_features, descendant_features);
...
void RuleFeatureSet::AddFeaturesToInvalidationSetsForStyleScope(const StyleScope& style_scope, InvalidationSetFeatures& descendant_features)
{
    auto add_features = [this](const CSSSelectorList& selector_list, InvalidationSetFeatures& features) {
        for (const CSSSelector* selector = selector_list.First(); selector; selector = CSSSelectorList::Next(*selector)) {
            AddFeaturesToInvalidationSets(*selector, nullptr /* sibling_features */, features);
        }
    };
...
void RuleFeatureSet::AddFeaturesToInvalidationSetsForSimpleSelector(
    const CSSSelector& simple_selector, const CSSSelector& compound, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    if (pseudo_type == CSSSelector::kPseudoHas) {
        CollectValuesInHasArgument(simple_selector);
        AddFeaturesToInvalidationSetsForHasPseudoClass(simple_selector, &compound, sibling_features, descendant_features);
    }

    if (InvalidationSet* invalidation_set = InvalidationSetForSimpleSelector(
            simple_selector, sibling_features ? InvalidationType::kInvalidateSiblings : InvalidationType::kInvalidateDescendants, kAncestor)) {
        if (!sibling_features) {
            if (invalidation_set == nth_invalidation_set_) {
                // TODO(futhark): We can extract the features from the current compound
                // to optimize this.
                invalidation_set->SetWholeSubtreeInvalid();
                AddFeaturesToInvalidationSet(To<SiblingInvalidationSet>(invalidation_set)->EnsureSiblingDescendants(), descendant_features);
                return;
            } else {
        ...
        sibling_invalidation_set->UpdateMaxDirectAdjacentSelectors(sibling_features->max_direct_adjacent_selectors);
        ...
        if (sibling_features == &descendant_features) {
            ...
            AddFeaturesToInvalidationSet(sibling_invalidation_set->EnsureSiblingDescendants(), descendant_features);
        }
        return;
    }
    ...
    AddFeaturesToInvalidationSetsForSelectorList(simple_selector, sibling_features, descendant_features);
}

const CSSSelector* RuleFeatureSet::AddFeaturesToInvalidationSetsForCompoundSelector(
    const CSSSelector& compound, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    bool compound_has_features_for_rule_set_invalidation = false;
    const CSSSelector* simple_selector = &compound;
    for (; simple_selector; simple_selector = simple_selector->TagHistory()) {
        base::AutoReset<bool> reset_has_features(&descendant_features.has_features_for_rule_set_invalidation, false);
        AddFeaturesToInvalidationSetsForSimpleSelector(*simple_selector, compound, sibling_features, descendant_features);
        if (descendant_features.has_features_for_rule_set_invalidation)
            compound_has_features_for_rule_set_invalidation = true;
...
void RuleFeatureSet::AddFeaturesToInvalidationSets(
    const CSSSelector& selector, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    while (compound) {
        const CSSSelector* last_in_compound = AddFeaturesToInvalidationSetsForCompoundSelector(*compound, sibling_features, descendant_features);
...
RuleFeatureSet::SelectorPreMatch RuleFeatureSet::CollectFeaturesFromSelector(const CSSSelector& selector, const StyleScope* style_scope)
{
    FeatureMetadata metadata;
    const unsigned max_direct_adjacent_selectors = 0;
    if (CollectMetadataFromSelector(selector, max_direct_adjacent_selectors, metadata) == kSelectorNeverMatches) {
...
RuleFeatureSet::SelectorPreMatch RuleFeatureSet::CollectMetadataFromSelector(
    const CSSSelector& selector, unsigned max_direct_adjacent_selectors, RuleFeatureSet::FeatureMetadata& metadata)
{
    ...
    for (const CSSSelector* current = &selector; current; current = current->TagHistory()) {
        ...
        case CSSSelector::kPseudoHostContext:
            if (!found_host_pseudo && relation == CSSSelector::kSubSelector)
                return kSelectorNeverMatches;
            if (!current->IsLastInTagHistory() && current->TagHistory()->Match() != CSSSelector::kPseudoElement
        ...
        } else if (max_direct_adjacent_selectors && ((relation != CSSSelector::kSubSelector) || current->IsLastInTagHistory())) {
...
{
    return uses_first_line_rules == other.uses_first_line_rules && uses_window_inactive_selector == other.uses_window_inactive_selector
        && needs_full_recalc_for_rule_set_invalidation == other.needs_full_recalc_for_rule_set_invalidation
        && max_direct_adjacent_selectors == other.max_direct_adjacent_selectors && invalidates_parts == other.invalidates_parts;
}
...
void RuleFeatureSet::Merge(const RuleFeatureSet& other)
{
    ...
    if (base::FeatureList::IsEnabled(blink::features::kInvalidationSetClassBloomFilter) && other.names_with_self_invalidation_) {
        if (names_with_self_invalidation_ == nullptr) {
            names_with_self_invalidation_ = std::make_unique<WTF::BloomFilter<14>>();
        }
    ...
    if (other.universal_sibling_invalidation_set_) {
        EnsureUniversalSiblingInvalidationSet().Combine(*other.universal_sibling_invalidation_set_);
    }
...
void RuleFeatureSet::CollectInvalidationSetsForClass(InvalidationLists& invalidation_lists, Element& element, const AtomicString& class_name) const
{
    if (base::FeatureList::IsEnabled(blink::features::kInvalidationSetClassBloomFilter)) {
        // Implicit self-invalidation sets for all classes (with Bloom filter
        // rejection); see comment on class_invalidation_sets_.
        if (names_with_self_invalidation_ && names_with_self_invalidation_->MayContain(class_name.Impl()->ExistingHash() * kClassSalt)) {
            invalidation_lists.descendants.push_back(InvalidationSet::SelfInvalidationSet());
        }
    }

    InvalidationSetMap::const_iterator it = class_invalidation_sets_.find(class_name);
...
void RuleFeatureSet::CollectInvalidationSetsForId(InvalidationLists& invalidation_lists, Element& element, const AtomicString& id) const
{
    if (names_with_self_invalidation_ && names_with_self_invalidation_->MayContain(id.Impl()->ExistingHash() * kIdSalt)) {
        invalidation_lists.descendants.push_back(InvalidationSet::SelfInvalidationSet());
    }
...
void RuleFeatureSet::CollectInvalidationSetsForAttribute(InvalidationLists& invalidation_lists, Element& element, const QualifiedName& attribute_name) const
{
    InvalidationSetMap::const_iterator it = attribute_invalidation_sets_.find(attribute_name.LocalName());
...
void RuleFeatureSet::CollectInvalidationSetsForPseudoClass(InvalidationLists& invalidation_lists, Element& element, CSSSelector::PseudoType pseudo) const
{
    PseudoTypeInvalidationSetMap::const_iterator it = pseudo_invalidation_sets_.find(pseudo);
...
void RuleFeatureSet::CollectUniversalSiblingInvalidationSet(InvalidationLists& invalidation_lists, unsigned min_direct_adjacent) const
{
    if (universal_sibling_invalidation_set_ && universal_sibling_invalidation_set_->MaxDirectAdjacentSelectors() >= min_direct_adjacent)
...
void RuleFeatureSet::CollectNthInvalidationSet(InvalidationLists& invalidation_lists) const
{
...
void RuleFeatureSet::CollectPartInvalidationSet(InvalidationLists& invalidation_lists) const
{
...
void RuleFeatureSet::CollectTypeRuleInvalidationSet(InvalidationLists& invalidation_lists, ContainerNode& root_node) const
{
    if (type_rule_invalidation_set_) {
        invalidation_lists.descendants.push_back(type_rule_invalidation_set_);
        TRACE_SCHEDULE_STYLE_INVALIDATION(root_node, *type_rule_invalidation_set_, RuleSetInvalidation);
    }
}
    ...
    const InvalidationSetFeatures& sibling_features, const InvalidationSetFeatures& descendant_features)
{
    ...
    universal_set.UpdateMaxDirectAdjacentSelectors(sibling_features.max_direct_adjacent_selectors);

    if (&sibling_features == &descendant_features) {
        universal_set.SetInvalidatesSelf();
    } else {
        AddFeaturesToInvalidationSet(universal_set.EnsureSiblingDescendants(), descendant_features);
    }
}

bool RuleFeatureSet::NeedsHasInvalidationForClass(const AtomicString& class_name) const
{
    return classes_in_has_argument_.Contains(class_name);
}

bool RuleFeatureSet::NeedsHasInvalidationForAttribute(const QualifiedName& attribute_name) const
{
...
bool RuleFeatureSet::NeedsHasInvalidationForTagName(const AtomicString& tag_name) const
{
    return universal_in_has_argument_ || tag_names_in_has_argument_.Contains(tag_name);
}

bool RuleFeatureSet::NeedsHasInvalidationForInsertedOrRemovedElement(Element& element) const
{
...
void RuleFeatureSet::InvalidationSetFeatures::Merge(const InvalidationSetFeatures& other)
{
...
void RuleFeatureSet::InvalidationSetFeatures::NarrowToFeatures(const InvalidationSetFeatures& other)
{
    unsigned size = Size();
...
String RuleFeatureSet::ToString() const
{
    ...
    add_invalidation_sets("*", universal_sibling_invalidation_set_.get(), kUniversal);
