...
static bool ShouldMatchHoverOrActive(const SelectorChecker::SelectorCheckingContext& context)
{
    ...
    while (selector->Relation() == CSSSelector::kSubSelector && selector->TagHistory()) {
...
bool SelectorChecker::Match(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    if (MatchSelector(context, result) != kSelectorMatches)
        return false;
    if (RuntimeEnabledFeatures::CSSScopeEnabled() && !CheckInStyleScope(context, result)) {
        return false;
    }
    return true;
}
...
SelectorChecker::MatchStatus SelectorChecker::MatchForSubSelector(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    if (context.in_rightmost_compound && dynamic_pseudo != kPseudoIdNone && dynamic_pseudo != kPseudoIdSelection && !next_context.has_scrollbar_pseudo) {
        return kSelectorFailsCompletely;
    }
...
static bool AttributeValueMatches(
    const Attribute& attribute_item, CSSSelector::MatchType match, const AtomicString& selector_value, TextCaseSensitivity case_sensitivity)
{
    ...
    case CSSSelector::kAttributeList: {
        ...
        while (true) {
            wtf_size_t found_pos = value.Find(selector_value, start_search_at, case_sensitivity);
...
static bool AnyAttributeMatches(Element& element, CSSSelector::MatchType match, const CSSSelector& selector)
{
    ...
    TextCaseSensitivity case_sensitivity
        = (selector.AttributeMatch() == CSSSelector::AttributeMatchType::kCaseInsensitive) ? kTextCaseASCIIInsensitive : kTextCaseSensitive;
    ...
    for (const auto& attribute_item : attributes) {
        ...
        // flag is set or not.
        bool legacy_case_insensitive = IsA<HTMLDocument>(element.GetDocument()) && !selector.IsCaseSensitiveAttribute();

        // If case-insensitive, re-check, and count if result differs.
        // See http://code.google.com/p/chromium/issues/detail?id=327060
        if (legacy_case_insensitive && AttributeValueMatches(attribute_item, match, selector_value, kTextCaseASCIIInsensitive)) {
            ...
            UseCounter::Count(element.GetDocument(), WebFeature::kCaseInsensitiveAttrSelectorMatch);
            return true;
        }
        if (selector_attr.NamespaceURI() != g_star_atom)
            return false;
...
bool SelectorChecker::CheckOne(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    if (context.scope && context.scope->OwnerShadowHost() == element
        && (!selector.IsHostPseudoClass() && selector.GetPseudoType() != CSSSelector::kPseudoScope && !context.treat_shadow_host_as_normal_scope
...
bool SelectorChecker::CheckPseudoNot(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    for (sub_context.selector = selector.SelectorList()->First(); sub_context.selector; sub_context.selector = CSSSelectorList::Next(*sub_context.selector)) {
        MatchResult sub_result;
    ...
    HeapVector<Member<Element>>& has_argument_leftmost_compound_matches, CheckPseudoHasCacheScope::Context& cache_scope_context)
{
    ...
    case CSSSelector::kRelativeDescendant:
        return CacheMatchedElementsAndReturnMatchedResultForIndirectRelation(
            has_anchor_element, has_argument_leftmost_compound_matches, cache_scope_context, TraverseToParent);
    case CSSSelector::kRelativeChild:
...
{
    CheckPseudoHasCacheScope check_pseudo_has_cache_scope(&context.element->GetDocument());
...
bool SelectorChecker::CheckPseudoClass(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    case CSSSelector::kPseudoAutofillPreviewed: {
        auto* html_form_element = DynamicTo<HTMLFormControlElement>(&element);
        return html_form_element && html_form_element->GetAutofillState() == WebAutofillState::kPreviewed;
    }
    ...
    case CSSSelector::kPseudoPopupOpeningOrOpen:
        if (!RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(element.GetDocument().GetExecutionContext())) {
        ...
        if (element.HasPopupAttribute()) {
            return element.GetPopupData()->visibilityState() != PopupVisibilityState::kHidden;
        }
        return false;
...
bool SelectorChecker::CheckPseudoElement(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    case CSSSelector::kPseudoPageTransitionIncomingImage: {
        ...
        return selector.Argument() == CSSSelector::UniversalSelectorAtom() || selector.Argument() == pseudo_argument_;
    }
...
bool SelectorChecker::CheckPseudoScope(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    if (RuntimeEnabledFeatures::CSSScopeEnabled() && context.style_scope) {
        ...
        const Activations& activations = EnsureActivations(context, *context.style_scope);
...
// static
bool SelectorChecker::MatchesSpatialNavigationInterestPseudoClass(const Element& element)
{
    ...
    Element* interested_element = element.GetDocument().GetPage()->GetSpatialNavigationController().GetInterestedElement();
    return interested_element && *interested_element == element;
}

void SelectorChecker::StyleScopeActivation::Trace(blink::Visitor* visitor) const
{
    visitor->Trace(root);
}

const SelectorChecker::Activations& SelectorChecker::EnsureActivations(const SelectorCheckingContext& context, const StyleScope& style_scope) const
{
    DCHECK(context.style_scope_frame);
    ...
    // default activation.
    const Activations* outer_activations = style_scope.Parent()
        ? &EnsureActivations(context, *style_scope.Parent())
        : MakeGarbageCollected<Activations>(1, StyleScopeActivation { nullptr /* scope */, std::numeric_limits<unsigned>::max(), false });
    ...
    if (entry.is_new_entry) {
        activations = CalculateActivations(context.style_scope_frame->element_, style_scope, *outer_activations);
    }
...
const SelectorChecker::Activations* SelectorChecker::CalculateActivations(
    Element& element, const StyleScope& style_scope, const Activations& outer_activations) const
{
    ...
    if (outer_activations.front().root != &element) {
        ...
        if (Element* parent = element.parentElement()) {
            parent_activations = CalculateActivations(*parent, style_scope, outer_activations);
        }
    }
    ...
    if (parent_activations) {
        for (const StyleScopeActivation& activation : *parent_activations) {
            if (!activation.limit) {
                activations->push_back(StyleScopeActivation { activation.root, activation.proximity + 1, false });
            }
        }
    }
...
bool SelectorChecker::MatchesWithScope(Element& element, const CSSSelectorList& selector_list, Element* scope) const
{
    SelectorCheckingContext context(&element);
    context.scope = scope;
    for (context.selector = selector_list.First(); context.selector; context.selector = CSSSelectorList::Next(*context.selector)) {
        SelectorChecker::MatchResult ignore_result;
        if (MatchSelector(context, ignore_result) == SelectorChecker::kSelectorMatches) {
            return true;
        }
    }
    return false;
...
bool SelectorChecker::CheckInStyleScope(const SelectorCheckingContext& context, MatchResult& result) const
{
    ...
    for (; local_context.element; local_context.element = ParentElement(local_context)) {
