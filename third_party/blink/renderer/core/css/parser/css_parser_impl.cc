...
StyleRuleBase::LayerName ConsumeCascadeLayerName(CSSParserTokenRange& range)
{
    ...
    while (!range.AtEnd() && range.Peek().GetType() == kIdentToken) {
        ...
        const bool has_next_part = range.Peek().GetType() == kDelimiterToken && range.Peek().Delimiter() == '.' && range.Peek(1).GetType() == kIdentToken;
        if (!has_next_part)
            break;
        range.Consume();
    }
...
MutableCSSPropertyValueSet::SetResult CSSParserImpl::ParseValue(
    MutableCSSPropertyValueSet* declaration, CSSPropertyID unresolved_property, const String& string, bool important, const CSSParserContext* context)
{
    ...
    parser.ConsumeDeclarationValue(tokenized_value, unresolved_property, important, rule_type);
...
MutableCSSPropertyValueSet::SetResult CSSParserImpl::ParseVariableValue(MutableCSSPropertyValueSet* declaration, const AtomicString& property_name,
    const String& value, bool important, const CSSParserContext* context, bool is_animation_tainted)
{
    STACK_UNINITIALIZED CSSParserImpl parser(context);
    CSSTokenizer tokenizer(value);
    CSSParserTokenStream stream(tokenizer);
    CSSTokenizedValue tokenized_value = ConsumeValue(stream);
    parser.ConsumeVariableValue(tokenized_value, property_name, important, is_animation_tainted);
    if (parser.parsed_properties_.empty()) {
        return MutableCSSPropertyValueSet::kParseError;
    } else {
        return declaration->AddParsedProperties(parser.parsed_properties_);
...
static ImmutableCSSPropertyValueSet* CreateCSSPropertyValueSet(HeapVector<CSSPropertyValue, 64>& parsed_properties, CSSParserMode mode)
{
    ...
    ImmutableCSSPropertyValueSet* result = ImmutableCSSPropertyValueSet::Create(results.data() + unused_entries, results.size() - unused_entries, mode);
...
StyleRuleBase* CSSParserImpl::ParseRule(const String& string, const CSSParserContext* context, StyleSheetContents* style_sheet, AllowedRulesType allowed_rules)
{
...
ParseSheetResult CSSParserImpl::ParseStyleSheet(const String& string, const CSSParserContext* context, StyleSheetContents* style_sheet,
    CSSDeferPropertyParsing defer_property_parsing, bool allow_import_rules, std::unique_ptr<CachedCSSTokenizer> cached_tokenizer)
{
    absl::optional<LocalFrameUkmAggregator::ScopedUkmHierarchicalTimer> timer;
    if (context->GetDocument() && context->GetDocument()->View()) {
        timer.emplace(context->GetDocument()->View()->EnsureUkmAggregator().GetScopedTimer(static_cast<size_t>(LocalFrameUkmAggregator::kParseStyleSheet)));
    }
    ...
    bool first_rule_valid = parser.ConsumeRuleList(stream, kTopLevelRuleList, [&style_sheet, &result, allow_import_rules, context](StyleRuleBase* rule) {
    ...
    TRACE_EVENT_END2("blink,blink_style", "CSSParserImpl::parseStyleSheet", "tokenCount", tokenizer->TokenCount(), "length", string.length());
    return result;
}

// static
CSSSelectorList CSSParserImpl::ParsePageSelector(CSSParserTokenRange range, StyleSheetContents* style_sheet, const CSSParserContext& context)
{
    ...
    if (!type_selector.IsNull() && pseudo.IsNull()) {
        selector.reset(arena.New<CSSParserSelector>(QualifiedName(g_null_atom, type_selector, g_star_atom)));
    } else {
        ...
        if (!type_selector.IsNull()) {
            selector->PrependTagSelector(arena, QualifiedName(g_null_atom, type_selector, g_star_atom));
        }
    }
...
std::unique_ptr<Vector<double>> CSSParserImpl::ParseKeyframeKeyList(const String& key_list)
{
    ...
    const String& string, const CSSParserContext* context, StyleSheetContents* style_sheet, CSSParserObserver& observer)
{
    ...
    CSSTokenizer tokenizer(string);
    CSSParserTokenStream stream(tokenizer);
    bool first_rule_valid = parser.ConsumeRuleList(stream, kTopLevelRuleList, [&style_sheet](StyleRuleBase* rule) {
        if (rule->IsCharsetRule())
            return;
        style_sheet->ParserAppendRule(rule);
    });
...
static CSSParserImpl::AllowedRulesType ComputeNewAllowedRules(CSSParserImpl::AllowedRulesType allowed_rules, StyleRuleBase* rule)
{
    if (!rule || allowed_rules == CSSParserImpl::kKeyframeRules || allowed_rules == CSSParserImpl::kFontFeatureRules
        || allowed_rules == CSSParserImpl::kTryRules || allowed_rules == CSSParserImpl::kNoRules)
...
template <typename T> bool CSSParserImpl::ConsumeRuleList(CSSParserTokenStream& stream, RuleListType rule_list_type, const T callback)
{
...
StyleRuleBase* CSSParserImpl::ConsumeAtRule(CSSParserTokenStream& stream, AllowedRulesType allowed_rules)
{
    ...
    if (allowed_rules == kKeyframeRules || allowed_rules == kFontFeatureRules || allowed_rules == kNoRules) {
...
StyleRuleBase* CSSParserImpl::ConsumeQualifiedRule(CSSParserTokenStream& stream, AllowedRulesType allowed_rules)
{
    ...
    if (allowed_rules == kKeyframeRules) {
        ...
        CSSParserTokenStream::BlockGuard guard(stream);
        StyleRuleKeyframe* keyframe_style_rule = ConsumeKeyframeStyleRule(prelude, prelude_offset, stream);
    ...
    if (allowed_rules == kFontFeatureRules) {
        ...
        return MakeGarbageCollected<StyleRuleFontFace>(CreateCSSPropertyValueSet(parsed_properties_, kCSSFontFaceRuleMode));
    }
...
StyleRuleImport* CSSParserImpl::ConsumeImportRule(const AtomicString& uri, CSSParserTokenStream& stream)
{
    ...
    return MakeGarbageCollected<StyleRuleImport>(uri, std::move(layer), MediaQueryParser::ParseMediaQuerySet(prelude, context_->GetExecutionContext()),
...
StyleRuleNamespace* CSSParserImpl::ConsumeNamespaceRule(CSSParserTokenStream& stream)
{
    ...
    if (prelude.Peek().GetType() == kIdentToken)
        namespace_prefix = prelude.ConsumeIncludingWhitespace().Value().ToAtomicString();
...
StyleRuleMedia* CSSParserImpl::ConsumeMediaRule(CSSParserTokenStream& stream)
{
    ...
    ConsumeRuleList(stream, kRegularRuleList, [&rules](StyleRuleBase* rule) { rules.push_back(rule); });
...
StyleRuleSupports* CSSParserImpl::ConsumeSupportsRule(CSSParserTokenStream& stream)
{
    wtf_size_t prelude_offset_start = stream.LookAheadOffset();
    ...
    return MakeGarbageCollected<StyleRuleSupports>(prelude_serialized, supported == CSSSupportsParser::Result::kSupported, rules);
}
...
StyleRuleKeyframes* CSSParserImpl::ConsumeKeyframesRule(bool webkit_prefixed, CSSParserTokenStream& stream)
{
    ...
    ConsumeRuleList(
        stream, kKeyframesRuleList, [keyframe_rule](StyleRuleBase* keyframe) { keyframe_rule->ParserAppendKeyframe(To<StyleRuleKeyframe>(keyframe)); });
...
StyleRulePage* CSSParserImpl::ConsumePageRule(CSSParserTokenStream& stream)
{
    ...
    CSSSelectorList selector_list = ParsePageSelector(prelude, style_sheet_, *context_);
    ...
    return MakeGarbageCollected<StyleRulePage>(std::move(selector_list), CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}
...
StyleRuleProperty* CSSParserImpl::ConsumePropertyRule(CSSParserTokenStream& stream)
{
    ...
    return MakeGarbageCollected<StyleRuleProperty>(name, CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}
...
StyleRuleCounterStyle* CSSParserImpl::ConsumeCounterStyleRule(CSSParserTokenStream& stream)
{
    ...
    AtomicString name = css_parsing_utils::ConsumeCounterStyleNameInPrelude(prelude, *GetContext());
    if (!name)
        return nullptr;
    ...
    return MakeGarbageCollected<StyleRuleCounterStyle>(name, CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}
...
StyleRuleFontPaletteValues* CSSParserImpl::ConsumeFontPaletteValuesRule(CSSParserTokenStream& stream)
{
    ...
    return MakeGarbageCollected<StyleRuleFontPaletteValues>(name, CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}
...
StyleRuleContainer* CSSParserImpl::ConsumeContainerRule(CSSParserTokenStream& stream)
{
    wtf_size_t prelude_offset_start = stream.LookAheadOffset();
    CSSParserTokenRange prelude = ConsumeAtRulePrelude(stream);
    wtf_size_t prelude_offset_end = stream.LookAheadOffset();
    ...
    AtomicString name;
    if (prelude.Peek().GetType() == kIdentToken) {
        auto* ident = DynamicTo<CSSCustomIdentValue>(css_parsing_utils::ConsumeSingleContainerName(prelude, *context_));
        if (ident)
            name = ident->Value();
    }
...
StyleRulePositionFallback* CSSParserImpl::ConsumePositionFallbackRule(CSSParserTokenStream& stream)
{
    wtf_size_t prelude_offset_start = stream.LookAheadOffset();
    CSSParserTokenRange prelude = ConsumeAtRulePrelude(stream);
    wtf_size_t prelude_offset_end = stream.LookAheadOffset();
    ...
    if (observer_) {
        observer_->StartRuleHeader(StyleRule::kPositionFallback, prelude_offset_start);
        observer_->EndRuleHeader(prelude_offset_end);
        observer_->StartRuleBody(stream.Offset());
    }

    auto* position_fallback_rule = MakeGarbageCollected<StyleRulePositionFallback>(AtomicString(name));
    ConsumeRuleList(stream, kPositionFallbackRuleList,
        [position_fallback_rule](StyleRuleBase* try_rule) { position_fallback_rule->ParserAppendTryRule(To<StyleRuleTry>(try_rule)); });
...
StyleRuleTry* CSSParserImpl::ConsumeTryRule(CSSParserTokenStream& stream)
{
    ...
    return MakeGarbageCollected<StyleRuleTry>(CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}

StyleRuleKeyframe* CSSParserImpl::ConsumeKeyframeStyleRule(const CSSParserTokenRange prelude, const RangeOffset& prelude_offset, CSSParserTokenStream& block)
{
    ...
    return MakeGarbageCollected<StyleRuleKeyframe>(std::move(key_list), CreateCSSPropertyValueSet(parsed_properties_, kCSSKeyframeRuleMode));
}
...
StyleRule* CSSParserImpl::ConsumeStyleRule(CSSParserTokenStream& stream)
{
    ...
    CSSSelectorVector selector_vector = CSSSelectorParser::ConsumeSelector(stream, context_, style_sheet_, observer_, arena_);
    ...
    if (!observer_ && lazy_state_) {
        DCHECK(style_sheet_);
        return StyleRule::Create(selector_vector, MakeGarbageCollected<CSSLazyPropertyParserImpl>(stream.Offset() - 1, lazy_state_));
    }
    ConsumeDeclarationList(stream, StyleRule::kStyle);

    return StyleRule::Create(selector_vector, CreateCSSPropertyValueSet(parsed_properties_, context_->Mode()));
}

void CSSParserImpl::ConsumeDeclarationList(CSSParserTokenStream& stream, StyleRule::RuleType rule_type)
{
    DCHECK(parsed_properties_.empty());

    bool is_observer_rule_type = rule_type == StyleRule::kStyle || rule_type == StyleRule::kProperty || rule_type == StyleRule::kContainer
        || rule_type == StyleRule::kCounterStyle || rule_type == StyleRule::kFontPaletteValues || rule_type == StyleRule::kKeyframe
        || rule_type == StyleRule::kScope || rule_type == StyleRule::kTry;
...
void CSSParserImpl::ConsumeDeclaration(CSSParserTokenStream& stream, StyleRule::RuleType rule_type)
{
    ...
    if (rule_type == StyleRule::kFontFace || rule_type == StyleRule::kFontPaletteValues || rule_type == StyleRule::kProperty
        ...
        unresolved_property = lhs.ParseAsUnresolvedCSSPropertyID(context_->GetExecutionContext(), context_->Mode());
    }

    // @rules other than FontFace still handled with legacy code.
    if (important && (rule_type == StyleRule::kKeyframe || rule_type == StyleRule::kTry)) {
        return;
    }
    ...
    if (unresolved_property == CSSPropertyID::kVariable) {
        ...
        ConsumeVariableValue(tokenized_value, variable_name, important, is_animation_tainted);
    } else if (unresolved_property != CSSPropertyID::kInvalid) {
        ConsumeDeclarationValue(tokenized_value, unresolved_property, important, rule_type);
    }

    if (observer_ && (rule_type == StyleRule::kStyle || rule_type == StyleRule::kKeyframe)) {
...
void CSSParserImpl::ConsumeVariableValue(const CSSTokenizedValue& tokenized_value, const AtomicString& variable_name, bool important, bool is_animation_tainted)
{
    if (CSSValue* value = CSSVariableParser::ParseDeclarationIncludingCSSWide(tokenized_value, is_animation_tainted, *context_)) {
...
void CSSParserImpl::ConsumeDeclarationValue(
    const CSSTokenizedValue& tokenized_value, CSSPropertyID unresolved_property, bool important, StyleRule::RuleType rule_type)
{
    CSSPropertyParser::ParseValue(unresolved_property, important, tokenized_value.range, context_, parsed_properties_, rule_type);
}
...
std::unique_ptr<Vector<double>> CSSParserImpl::ConsumeKeyframeKeyList(CSSParserTokenRange range)
{
    ...
    while (true) {
        ...
        else if (token.GetType() == kIdentToken && EqualIgnoringASCIICase(token.Value(), "from"))
            result->push_back(0);
        else if (token.GetType() == kIdentToken && EqualIgnoringASCIICase(token.Value(), "to"))
            result->push_back(1);
        else
...
const MediaQuerySet* CSSParserImpl::CachedMediaQuerySet(String prelude_string, CSSParserTokenRange prelude)
{
    Member<const MediaQuerySet>& media = media_query_cache_.insert(prelude_string, nullptr).stored_value->value;
    if (!media) {
        media = MediaQueryParser::ParseMediaQuerySet(prelude, context_->GetExecutionContext());
    }
    DCHECK(media);
    return media.Get();
}
