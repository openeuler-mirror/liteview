...
CSSValue* CSSVariableParser::ParseDeclarationIncludingCSSWide(
    const CSSTokenizedValue& tokenized_value, bool is_animation_tainted, const CSSParserContext& context)
{
    ...
    const CSSTokenizedValue& tokenized_value, bool is_animation_tainted, const CSSParserContext& context)
{
    ...
    if (!IsValidVariable(tokenized_value.range, has_references))
        return nullptr;
    return MakeGarbageCollected<CSSCustomPropertyDeclaration>(CSSVariableData::Create(tokenized_value, is_animation_tainted, has_references), &context);
}

CSSVariableReferenceValue* CSSVariableParser::ParseVariableReferenceValue(CSSParserTokenRange range, const CSSParserContext& context, bool is_animation_tainted)
{
    if (range.AtEnd())
        return nullptr;
    ...
    if (ParseCSSWideValue(range))
        return nullptr;
    return MakeGarbageCollected<CSSVariableReferenceValue>(CSSVariableData::Create({ range, StringView() }, is_animation_tainted, has_references), context);
}
