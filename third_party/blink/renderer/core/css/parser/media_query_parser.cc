...
public:
    ...
    bool IsAllowedWithoutValue(const String& feature, const ExecutionContext* execution_context) const override
    {
        ...
        return feature == media_feature_names::kMonochromeMediaFeature || feature == media_feature_names::kColorMediaFeature
            ...
            || feature == media_feature_names::kColorGamutMediaFeature || feature == media_feature_names::kImmersiveMediaFeature
            || feature == media_feature_names::kPrefersColorSchemeMediaFeature || feature == media_feature_names::kPrefersContrastMediaFeature
            ...
            || (feature == media_feature_names::kHorizontalViewportSegmentsMediaFeature && RuntimeEnabledFeatures::CSSFoldablesEnabled())
            || (feature == media_feature_names::kVerticalViewportSegmentsMediaFeature && RuntimeEnabledFeatures::CSSFoldablesEnabled())
            || (feature == media_feature_names::kDevicePostureMediaFeature && RuntimeEnabledFeatures::DevicePostureEnabled());
    }
...
MediaQuerySet* MediaQueryParser::ParseMediaQuerySet(const String& query_string, const ExecutionContext* execution_context)
{
    return ParseMediaQuerySet(CSSParserTokenRange(CSSTokenizer(query_string).TokenizeToEOF()), execution_context);
}

MediaQuerySet* MediaQueryParser::ParseMediaQuerySet(CSSParserTokenRange range, const ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaQuerySetParser, kHTMLStandardMode, execution_context).ParseImpl(range);
}

MediaQuerySet* MediaQueryParser::ParseMediaQuerySetInMode(CSSParserTokenRange range, CSSParserMode mode, const ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaQuerySetParser, mode, execution_context).ParseImpl(range);
}

MediaQuerySet* MediaQueryParser::ParseMediaCondition(CSSParserTokenRange range, const ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaConditionParser, kHTMLStandardMode, execution_context).ParseImpl(range);
}

MediaQueryParser::MediaQueryParser(ParserType parser_type, CSSParserMode mode, const ExecutionContext* execution_context, SyntaxLevel syntax_level)
...
CSSParserTokenRange ConsumeUntilComparisonOrColon(CSSParserTokenRange& range)
{
    ...
    while (!range.AtEnd()) {
        ...
        if ((token.GetType() == kDelimiterToken && IsComparisonDelimiter(token.Delimiter())) || token.GetType() == kColonToken) {
            break;
        }
        range.ConsumeComponentValue();
    }
...
String MediaQueryParser::ConsumeAllowedName(CSSParserTokenRange& range, const FeatureSet& feature_set)
{
    if (range.Peek().GetType() != kIdentToken)
        return g_null_atom;
...
String MediaQueryParser::ConsumeUnprefixedName(CSSParserTokenRange& range, const FeatureSet& feature_set)
{
...
const MediaQueryExpNode* MediaQueryParser::ParseNameValueComparison(
    CSSParserTokenRange lhs, MediaQueryOperator op, CSSParserTokenRange rhs, NameAffinity name_affinity, const FeatureSet& feature_set)
{
...
const MediaQueryExpNode* MediaQueryParser::ConsumeFeature(CSSParserTokenRange& range, const FeatureSet& feature_set)
{
    ...
    if (range.AtEnd()) {
        ...
        if (feature_name.IsNull() || !segment1.AtEnd() || !feature_set.IsAllowedWithoutValue(feature_name, execution_context_)) {
            return nullptr;
        }
    ...
    if (range.AtEnd()) {
        ...
        if (const MediaQueryExpNode* node = ParseNameValueComparison(segment1, op1, segment2, NameAffinity::kLeft, feature_set)) {
            return node;
        }

        // Otherwise: <mf-value> <mf-comparison> <mf-name>
        return ParseNameValueComparison(segment1, op1, segment2, NameAffinity::kRight, feature_set);
    }
    ...
    auto left_value = MediaQueryExpValue::Consume(feature_name, segment1, fake_context_);
    if (!left_value || !segment1.AtEnd())
        return nullptr;

    CSSParserTokenRange& segment3 = range;
    auto right_value = MediaQueryExpValue::Consume(feature_name, segment3, fake_context_);
    if (!right_value || !segment3.AtEnd())
        return nullptr;

    return MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(feature_name, MediaQueryExpBounds(MediaQueryExpComparison(*left_value, op1), MediaQueryExpComparison(*right_value, op2))));
}

const MediaQueryExpNode* MediaQueryParser::ConsumeCondition(CSSParserTokenRange& range, ConditionMode mode)
{
    // <media-not>
    ...
    } else if (result && AtIdent(range.Peek(), "or") && mode == ConditionMode::kNormal && IsMediaQueries4SyntaxEnabled()) {
...
const MediaQueryExpNode* MediaQueryParser::ConsumeInParens(CSSParserTokenRange& range)
{
    ...
    if (range.Peek().GetType() == kLeftParenthesisToken) {
        ...
        // ( <media-feature> )
        const MediaQueryExpNode* feature = ConsumeFeature(block, MediaQueryFeatureSet(mode_));
...
const MediaQueryExpNode* MediaQueryParser::ConsumeGeneralEnclosed(CSSParserTokenRange& range)
{
...
MediaQuerySet* MediaQueryParser::ConsumeSingleCondition(CSSParserTokenRange range)
{
...
MediaQuery* MediaQueryParser::ConsumeQuery(CSSParserTokenRange& range)
{
    ...
    if (!type.IsNull()) {
        ...
        if (const MediaQueryExpNode* node = ConsumeCondition(range, ConditionMode::kWithoutOr)) {
...
bool MediaQueryParser::IsNotKeywordEnabled() const
{
    ...
    return (parser_type_ == kMediaConditionParser) || IsMediaQueries4SyntaxEnabled();
}
