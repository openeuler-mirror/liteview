...
SizesAttributeParser::SizesAttributeParser(MediaValues* media_values, const String& attribute, const ExecutionContext* execution_context)
    ...
    , length_was_set_(false)
{
    ...
    is_valid_ = Parse(CSSParserTokenRange(CSSTokenizer(attribute).TokenizeToEOF()));
}
...
bool SizesAttributeParser::Parse(CSSParserTokenRange range)
{
    ...
    while (!range.AtEnd()) {
        ...
        float length;
        if (!CalculateLengthInPixels(range.MakeSubRange(length_token_start, length_token_end), length))
            continue;
        MediaQuerySet* media_condition
            = MediaQueryParser::ParseMediaCondition(range.MakeSubRange(media_condition_start, length_token_start), execution_context_);
