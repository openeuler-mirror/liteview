...
static void RecordUsageAndDeprecationsOneSelector(const CSSSelector* selector, const CSSParserContext* context);
...
// static
CSSSelectorVector CSSSelectorParser::ParseSelector(CSSParserTokenRange range, const CSSParserContext* context, StyleSheetContents* style_sheet, Arena& arena)
{
...
CSSSelectorVector CSSSelectorParser::ConsumeSelector(
    CSSParserTokenStream& stream, const CSSParserContext* context, StyleSheetContents* style_sheet, CSSParserObserver* observer, Arena& arena)
{
    CSSSelectorParser parser(context, style_sheet, arena);
    stream.ConsumeWhitespace();
    CSSSelectorVector result = parser.ConsumeComplexSelectorList(stream, observer);
    parser.RecordUsageAndDeprecations(result);
    return result;
}
...
absl::optional<CSSSelectorList> CSSSelectorParser::ParseScopeBoundary(
    CSSParserTokenRange range, const CSSParserContext* context, StyleSheetContents* style_sheet)
{
    Arena arena;
    CSSSelectorParser parser(context, style_sheet, arena);
    ...
    range.ConsumeWhitespace();
    absl::optional<CSSSelectorList> result = parser.ConsumeForgivingComplexSelectorList(range);
    DCHECK(result);
    if (!range.AtEnd())
        return absl::nullopt;
    for (const CSSSelector* current = result->First(); current; current = current->TagHistory()) {
...
CSSSelectorParser::CSSSelectorParser(const CSSParserContext* context, StyleSheetContents* style_sheet, Arena& arena)
    : context_(context)
...
CSSSelectorVector CSSSelectorParser::ConsumeComplexSelectorList(CSSParserTokenRange& range)
{
...
CSSSelectorVector CSSSelectorParser::ConsumeComplexSelectorList(CSSParserTokenStream& stream, CSSParserObserver* observer)
{
    CSSSelectorVector selector_list;

    while (true) {
        const wtf_size_t selector_offset_start = stream.LookAheadOffset();
        CSSParserTokenRange complex_selector = stream.ConsumeUntilPeekedTypeIs<kLeftBraceToken, kCommaToken>();
        const wtf_size_t selector_offset_end = stream.LookAheadOffset();

        if (stream.UncheckedAtEnd())
            return {};

        ArenaUniquePtr<CSSParserSelector> selector = ConsumeComplexSelector(complex_selector);
...
public:
    explicit CSSAtSupportsDropInvalidWhileForgivingParsingCounter(const CSSParserContext* context)
        : context_(context)
    {
    }
...
absl::optional<CSSSelectorList> CSSSelectorParser::ConsumeForgivingNestedSelectorList(CSSParserTokenRange& range)
{
    if (inside_compound_pseudo_)
...
absl::optional<CSSSelectorList> CSSSelectorParser::ConsumeForgivingComplexSelectorList(CSSParserTokenRange& range)
{
    if (RuntimeEnabledFeatures::CSSAtSupportsAlwaysNonForgivingParsingEnabled() && in_supports_parsing_) {
    ...
    CSSSelectorVector selector_list;
    CSSAtSupportsDropInvalidWhileForgivingParsingCounter at_supports_drop_invalid_counter(context_);

    while (!range.AtEnd()) {
        base::AutoReset<bool> reset_failure(&failed_parsing_, false);
        CSSParserTokenRange argument = ConsumeNestedArgument(range);
        ArenaUniquePtr<CSSParserSelector> selector = ConsumeComplexSelector(argument);
...
absl::optional<CSSSelectorList> CSSSelectorParser::ConsumeForgivingCompoundSelectorList(CSSParserTokenRange& range)
{
    if (RuntimeEnabledFeatures::CSSAtSupportsAlwaysNonForgivingParsingEnabled() && in_supports_parsing_) {
    ...
    while (!range.AtEnd()) {
        base::AutoReset<bool> reset_failure(&failed_parsing_, false);
        CSSParserTokenRange argument = ConsumeNestedArgument(range);
        ArenaUniquePtr<CSSParserSelector> selector = ConsumeCompoundSelector(argument);
...
absl::optional<CSSSelectorList> CSSSelectorParser::ConsumeForgivingRelativeSelectorList(CSSParserTokenRange& range)
{
    if (RuntimeEnabledFeatures::CSSAtSupportsAlwaysNonForgivingParsingEnabled() && in_supports_parsing_) {
    ...
    while (!range.AtEnd()) {
        base::AutoReset<bool> reset_failure(&failed_parsing_, false);
        CSSParserTokenRange argument = ConsumeNestedArgument(range);
        ArenaUniquePtr<CSSParserSelector> selector = ConsumeRelativeSelector(argument);
        if (selector && !failed_parsing_ && argument.AtEnd())
            selector_list.push_back(std::move(selector));
        else if (in_supports_parsing_)
            at_supports_drop_invalid_counter.Count();
    ...
    if (inside_compound_pseudo_ || restricting_pseudo_element_ != CSSSelector::kPseudoUnknown || selector_list.empty()) {
        if (in_supports_parsing_)
            at_supports_drop_invalid_counter.Count();
...
unsigned ExtractCompoundFlags(const CSSParserSelector& simple_selector, CSSParserMode parser_mode)
{
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeRelativeSelector(CSSParserTokenRange& range)
{
    ...
    return ConsumePartialComplexSelector(range, combinator, std::move(selector), previous_compound_flags);
}

ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeComplexSelector(CSSParserTokenRange& range)
{
    ArenaUniquePtr<CSSParserSelector> selector = ConsumeCompoundSelector(range);
    if (!selector)
        return nullptr;
    ...
    for (CSSParserSelector* simple = selector.get(); simple && !previous_compound_flags; simple = simple->TagHistory())
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumePartialComplexSelector(
    CSSParserTokenRange& range, CSSSelector::RelationType& combinator, ArenaUniquePtr<CSSParserSelector> selector, unsigned& previous_compound_flags)
{
    do {
        ArenaUniquePtr<CSSParserSelector> next_selector = ConsumeCompoundSelector(range);
        if (!next_selector)
            return combinator == CSSSelector::kDescendant ? std::move(selector) : nullptr;
...
CSSSelector::PseudoType CSSSelectorParser::ParsePseudoType(const AtomicString& name, bool has_arguments, const Document* document)
{
    ...
    if (!RuntimeEnabledFeatures::WebKitScrollbarStylingEnabled()) {
        ...
        if (pseudo_type == CSSSelector::kPseudoResizer || pseudo_type == CSSSelector::kPseudoScrollbar || pseudo_type == CSSSelector::kPseudoScrollbarCorner
            || pseudo_type == CSSSelector::kPseudoScrollbarButton || pseudo_type == CSSSelector::kPseudoScrollbarThumb
            || pseudo_type == CSSSelector::kPseudoScrollbarTrack || pseudo_type == CSSSelector::kPseudoScrollbarTrackPiece) {
...
// static
PseudoId CSSSelectorParser::ParsePseudoElement(const String& selector_string, const Node* parent)
{
    ...
    // with specs.
    if (!range.AtEnd() && number_of_colons <= 2 && (range.Peek().GetType() == kIdentToken || range.Peek().GetType() == kFunctionToken)) {
        CSSParserToken selector_name_token = range.Consume();
        PseudoId pseudo_id = CSSSelector::GetPseudoId(ParsePseudoType(
            selector_name_token.Value().ToAtomicString(), selector_name_token.GetType() == kFunctionToken, parent ? &parent->GetDocument() : nullptr));

        if (PseudoElement::IsWebExposed(pseudo_id, parent)
            && ((PseudoElementHasArguments(pseudo_id) && range.Peek(0).GetType() == kIdentToken && range.Peek(1).GetType() == kRightParenthesisToken
...
// static
AtomicString CSSSelectorParser::ParsePseudoElementArgument(const String& selector_string)
{
    CSSTokenizer tokenizer(selector_string);
    const auto tokens = tokenizer.TokenizeToEOF();
    CSSParserTokenRange range(tokens);
    ...
    // TODO(crbug.com/1197620): allowing 0 or 1 preceding colons is not aligned
    // with specs.
    if (number_of_colons > 2 || range.Peek(0).GetType() != kFunctionToken || range.Peek(1).GetType() != kIdentToken
        || range.Peek(2).GetType() != kRightParenthesisToken || range.Peek(3).GetType() != kEOFToken) {
        return g_null_atom;
    }
...
bool IsSimpleSelectorValidAfterPseudoElement(const CSSParserSelector& simple_selector, CSSSelector::PseudoType compound_pseudo_element)
{
    switch (compound_pseudo_element) {
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeCompoundSelector(CSSParserTokenRange& range)
{
    ...
    while (ArenaUniquePtr<CSSParserSelector> simple_selector = ConsumeSimpleSelector(range)) {
        if (simple_selector->Match() == CSSSelector::kPseudoElement)
            restricting_pseudo_element_ = simple_selector->GetPseudoType();

        if (compound_selector) {
            compound_selector = AddSimpleSelectorToCompound(arena_, std::move(compound_selector), std::move(simple_selector));
        } else {
    ...
    base::AutoReset<bool> ignore_namespace(
        &ignore_default_namespace_, ignore_default_namespace_ || (resist_default_namespace_ && !has_q_name && AtEndIgnoringWhitespace(range)));
    ...
    if (!compound_selector) {
        ...
        return ArenaUniquePtr<CSSParserSelector>(arena_.New<CSSParserSelector>(QualifiedName(namespace_prefix, element_name, namespace_uri)));
    }
    ...
    PrependTypeSelectorIfNeeded(namespace_prefix, has_q_name, element_name, compound_selector.get());
    return SplitCompoundAtImplicitShadowCrossingCombinator(std::move(compound_selector));
}

ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeSimpleSelector(CSSParserTokenRange& range)
{
    const CSSParserToken& token = range.Peek();
    ...
    // video::-webkit-media-text-track-region-container.scrolling
    if (!selector || (context_->Mode() != kUASheetMode && !IsSimpleSelectorValidAfterPseudoElement(*selector.get(), restricting_pseudo_element_))) {
        failed_parsing_ = true;
    }
    return selector;
}

bool CSSSelectorParser::ConsumeName(CSSParserTokenRange& range, AtomicString& name, AtomicString& namespace_prefix)
{
    name = g_null_atom;
    namespace_prefix = g_null_atom;
    ...
    } else if (range.Peek(1).GetType() == kDelimiterToken && range.Peek(1).Delimiter() == '*') {
        range.Consume();
        range.Consume();
        name = CSSSelector::UniversalSelectorAtom();
    } else {
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeId(CSSParserTokenRange& range)
{
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeClass(CSSParserTokenRange& range)
{
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumeAttribute(CSSParserTokenRange& range)
{
    ...
    QualifiedName qualified_name
        = namespace_prefix.IsNull() ? QualifiedName(g_null_atom, attribute_name, g_null_atom) : QualifiedName(namespace_prefix, attribute_name, namespace_uri);

    ArenaUniquePtr<CSSParserSelector> selector(arena_.New<CSSParserSelector>());

    if (block.AtEnd()) {
        selector->SetAttribute(qualified_name, CSSSelector::AttributeMatchType::kCaseSensitive);
...
ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::ConsumePseudo(CSSParserTokenRange& range)
{
    ...
    case CSSSelector::kPseudoIs: {
        ...
        absl::optional<CSSSelectorList> selector_list = ConsumeForgivingNestedSelectorList(block);
        if (!selector_list || !block.AtEnd())
            return nullptr;
        selector->SetSelectorList(std::make_unique<CSSSelectorList>(std::move(*selector_list)));
        return selector;
    }
    case CSSSelector::kPseudoWhere: {
        DisallowPseudoElementsScope scope(this);
    ...
    case CSSSelector::kPseudoHas: {
        ...
        absl::optional<CSSSelectorList> selector_list = ConsumeForgivingRelativeSelectorList(block);
        if (!selector_list || !block.AtEnd())
            return nullptr;
        selector->SetSelectorList(std::make_unique<CSSSelectorList>(std::move(*selector_list)));
    ...
    case CSSSelector::kPseudoSlotted: {
        ...
        ArenaUniquePtr<CSSParserSelector> inner_selector = ConsumeCompoundSelector(block);
        block.ConsumeWhitespace();
    ...
    case CSSSelector::kPseudoToggle: {
        ...
        if (name.GetType() != kIdentToken || !css_parsing_utils::IsCustomIdent(name.Id())) {
            return nullptr;
        }
        ...
        if (!block.AtEnd()) {
            ...
            case kIdentToken:
                if (!css_parsing_utils::IsCustomIdent(value_token.Id()))
                    return nullptr;
                value = std::make_unique<State>(value_token.Value().ToAtomicString());
                break;
            case kNumberToken:
                if (value_token.GetNumericValueType() != kIntegerValueType || value_token.NumericValue() < 0) {
                    return nullptr;
                }
...
bool CSSSelectorParser::ConsumeANPlusB(CSSParserTokenRange& range, std::pair<int, int>& result)
{
    ...
    if (token.GetType() == kDelimiterToken && token.Delimiter() == '+' && range.Peek().GetType() == kIdentToken) {
        result.first = 1;
...
void CSSSelectorParser::PrependTypeSelectorIfNeeded(
    const AtomicString& namespace_prefix, bool has_q_name, const AtomicString& element_name, CSSParserSelector* compound_selector)
{
    if (!has_q_name && DefaultNamespace() == g_star_atom && !compound_selector->NeedsImplicitShadowCombinatorForMatching())
        return;
    ...
    if (is_host_pseudo && !has_q_name && namespace_prefix.IsNull())
        return;
    if (tag != AnyQName() || is_host_pseudo || compound_selector->NeedsImplicitShadowCombinatorForMatching()) {
        compound_selector->PrependTagSelector(
            arena_, tag, determined_prefix == g_null_atom && determined_element_name == CSSSelector::UniversalSelectorAtom() && !is_host_pseudo);
    }
}

ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::AddSimpleSelectorToCompound(
    Arena& arena, ArenaUniquePtr<CSSParserSelector> compound_selector, ArenaUniquePtr<CSSParserSelector> simple_selector)
{
    compound_selector->AppendTagHistory(CSSSelector::kSubSelector, std::move(simple_selector));
    return compound_selector;
}

ArenaUniquePtr<CSSParserSelector> CSSSelectorParser::SplitCompoundAtImplicitShadowCrossingCombinator(ArenaUniquePtr<CSSParserSelector> compound_selector)
{
    ...
    while (split_after->TagHistory() && !split_after->TagHistory()->NeedsImplicitShadowCombinatorForMatching())
        split_after = split_after->TagHistory();

    if (!split_after || !split_after->TagHistory())
        return compound_selector;

    ArenaUniquePtr<CSSParserSelector> remaining = split_after->ReleaseTagHistory();
    CSSSelector::RelationType relation = remaining->GetImplicitShadowCombinatorForMatching();
    ...
    remaining = SplitCompoundAtImplicitShadowCrossingCombinator(std::move(remaining));
...
static void RecordUsageAndDeprecationsOneSelector(const CSSSelector* selector, const CSSParserContext* context)
{
    ...
    if (selector->SelectorList()) {
        for (const CSSSelector* current = selector->SelectorList()->First(); current; current = current->TagHistory()) {
            RecordUsageAndDeprecationsOneSelector(current, context);
        }
    }
}

void CSSSelectorParser::RecordUsageAndDeprecations(const CSSSelectorVector& selector_vector)
{
    ...
    for (const ArenaUniquePtr<CSSParserSelector>& selector : selector_vector) {
        for (const CSSParserSelector* current = selector.get(); current; current = current->TagHistory()) {
            RecordUsageAndDeprecationsOneSelector(current->GetSelector(), context_);
        }
    }
}

bool CSSSelectorParser::ContainsUnknownWebkitPseudoElements(const CSSSelector& complex_selector)
{
    for (const CSSSelector* current = &complex_selector; current; current = current->TagHistory()) {
