...
// static
std::unique_ptr<CachedCSSTokenizer> CSSTokenizer::CreateCachedTokenizer(const String& input)
{
    ...
    wtf_size_t reserved_size = (tokenizer.input_.length() - tokenizer.Offset()) / kEstimatedCharactersPerToken;
    ...
    return std::make_unique<CachedCSSTokenizer>(input, std::move(tokens), std::move(offsets), std::move(tokenizer.string_pool_));
}

std::unique_ptr<CachedCSSTokenizer> CachedCSSTokenizer::DuplicateForTesting() const
{
    return std::make_unique<CachedCSSTokenizer>(input_.RangeAt(0, input_.length()).ToString(), tokens_, offsets_, string_pool_);
}
...
Vector<CSSParserToken, 32> CSSTokenizer::TokenizeToEOF()
{
    ...
    tokens.ReserveInitialCapacity((input_.length() - Offset()) / kEstimatedCharactersPerToken);
...
{
    if (input_.PeekWithoutReplacement(0) == '+' && (IsASCIIHexDigit(input_.PeekWithoutReplacement(1)) || input_.PeekWithoutReplacement(1) == '?')) {
        input_.Advance();
