...
public:
    ...
    {
        const auto* context = MakeGarbageCollected<CSSParserContext>(GetDocument());
        Vector<CSSParserToken, 32> tokens = CSSTokenizer(feature_query).TokenizeToEOF();
        CSSParserTokenRange range(tokens);
        const MediaQueryExpNode* node = ContainerQueryParser(*context).ConsumeFeatureQuery(range, TestFeatureSet());
        if (!node || !range.AtEnd())
            return g_null_atom;
