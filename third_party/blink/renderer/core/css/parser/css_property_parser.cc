...
CSSPropertyParser::CSSPropertyParser(const CSSParserTokenRange& range, const CSSParserContext* context, HeapVector<CSSPropertyValue, 64>* parsed_properties)
    : range_(range)
    , context_(context)
...
bool CSSPropertyParser::ParseValue(CSSPropertyID unresolved_property, bool important, const CSSParserTokenRange& range, const CSSParserContext* context,
    HeapVector<CSSPropertyValue, 64>& parsed_properties, StyleRule::RuleType rule_type)
{
    ...
    if (rule_type == StyleRule::kFontFace) {
        ...
        parse_success = parser.ParseValueStart(unresolved_property, rule_type, important);
    }
...
const CSSValue* CSSPropertyParser::ParseSingleValue(CSSPropertyID property, const CSSParserTokenRange& range, const CSSParserContext* context)
{
    DCHECK(context);
    ...
    const CSSValue* value = ParseLonghand(property, CSSPropertyID::kInvalid, *parser.context_, parser.range_);
    if (!value || !parser.range_.AtEnd())
        return nullptr;
    return value;
}

bool CSSPropertyParser::ParseValueStart(CSSPropertyID unresolved_property, StyleRule::RuleType rule_type, bool important)
{
    ...
    if (is_shorthand) {
        ...
        if (To<Shorthand>(property).ParseShorthand(important, range_, *context_, local_context, *parsed_properties_))
            return true;
    } else {
        if (const CSSValue* parsed_value = ParseLonghand(unresolved_property, CSSPropertyID::kInvalid, *context_, range_)) {
            if (range_.AtEnd()) {
    ...
    if (CSSVariableParser::ContainsValidVariableReferences(original_range)) {
        ...
        auto* variable
            = MakeGarbageCollected<CSSVariableReferenceValue>(CSSVariableData::Create({ original_range, StringView() }, is_animation_tainted, true), *context_);
...
static inline bool IsExposedInMode(const ExecutionContext* execution_context, const CSSProperty& property, CSSParserMode mode)
{
...
CSSPropertyID UnresolvedCSSPropertyID(const ExecutionContext* execution_context, const String& string)
{
    return WTF::VisitCharacters(
        string, [&](const auto* chars, unsigned length) { return UnresolvedCSSPropertyID(execution_context, chars, length, kHTMLStandardMode); });
}

CSSPropertyID UnresolvedCSSPropertyID(const ExecutionContext* execution_context, StringView string, CSSParserMode mode)
{
    return WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return UnresolvedCSSPropertyID(execution_context, chars, length, mode); });
}
...
bool CSSPropertyParser::ConsumeCSSWideKeyword(CSSPropertyID unresolved_property, bool important, StyleRule::RuleType rule_type)
{
...
bool CSSPropertyParser::ParseFontFaceDescriptor(CSSPropertyID resolved_property)
{
    ...
    if (id == AtRuleDescriptorID::Invalid)
        return false;
    CSSValue* parsed_value = AtRuleDescriptorParser::ParseFontFaceDescriptor(id, range_, *context_);
    if (!parsed_value)
        return false;
