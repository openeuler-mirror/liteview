...
CSSValue* ConsumeFontVariantList(CSSParserTokenRange& range)
{
    ...
    do {
        ...
        CSSIdentifierValue* font_variant = css_parsing_utils::ConsumeFontVariantCSS21(range);
        if (font_variant)
...
{
    return css_parsing_utils::ConsumeIdent<CSSValueID::kAuto, CSSValueID::kBlock, CSSValueID::kSwap, CSSValueID::kFallback, CSSValueID::kOptional>(range);
}
...
CSSValue* ConsumeFontFaceSrcURI(CSSParserTokenRange& range, const CSSParserContext& context, bool& tech_format_unsupported)
{
    tech_format_unsupported = false;

    String url = css_parsing_utils::ConsumeUrlAsStringView(range, context).ToString();
    if (url.IsNull())
        return nullptr;
    CSSFontFaceSrcValue* uri_value(CSSFontFaceSrcValue::Create(url, context.CompleteURL(url), context.GetReferrer(), context.JavascriptWorld(),
    ...
    if (!range.AtEnd() && range.Peek().GetType() != CSSParserTokenType::kCommaToken
        && (range.Peek().GetType() != CSSParserTokenType::kFunctionToken
            || (range.Peek().FunctionId() != CSSValueID::kFormat
                && (range.Peek().FunctionId() != CSSValueID::kTech || !RuntimeEnabledFeatures::CSSFontFaceSrcTechParsingEnabled()))))
        return nullptr;
    ...
    if (range.Peek().FunctionId() == CSSValueID::kFormat) {
        ...
        if (peek_type == kIdentToken) {
            CSSIdentifierValue* font_format = css_parsing_utils::ConsumeFontFormatIdent(format_args);
            if (!font_format)
                return nullptr;
    ...
    if (RuntimeEnabledFeatures::CSSFontFaceSrcTechParsingEnabled() && range.Peek().FunctionId() == CSSValueID::kTech) {
        ...
        do {
            CSSIdentifierValue* technology_value = css_parsing_utils::ConsumeFontTechIdent(tech_args);
            if (!technology_value) {
                return nullptr;
            }
            tech_format_unsupported |= !css_parsing_utils::IsSupportedKeywordTech(technology_value->GetValueID());
            if (!tech_args.AtEnd() && tech_args.Peek().GetType() != CSSParserTokenType::kCommaToken)
                return nullptr;
            if (!tech_format_unsupported) {
...
CSSValue* ConsumeFontFaceSrcLocal(CSSParserTokenRange& range, const CSSParserContext& context)
{
    ...
    if (args.Peek().GetType() == kStringToken) {
        ...
        return CSSFontFaceSrcValue::CreateLocal(
            arg.Value().ToString(), context.JavascriptWorld(), context.IsOriginClean() ? OriginClean::kTrue : OriginClean::kFalse, context.IsAdRelated());
    }
    if (args.Peek().GetType() == kIdentToken) {
        ...
        if (family_name.empty())
            return nullptr;
        return CSSFontFaceSrcValue::CreateLocal(
            family_name, context.JavascriptWorld(), context.IsOriginClean() ? OriginClean::kTrue : OriginClean::kFalse, context.IsAdRelated());
    }
    return nullptr;
}

CSSValueList* ConsumeFontFaceSrc(CSSParserTokenRange& range, const CSSParserContext& context)
{
    CSSValueList* values = CSSValueList::CreateCommaSeparated();
    ...
    do {
        ...
        if (token.FunctionId() == CSSValueID::kLocal) {
            ...
            parsed_value = ConsumeFontFaceSrcURI(range, context, tech_format_unsupported);
        }
...
CSSValue* ConsumeDescriptor(StyleRule::RuleType rule_type, AtRuleDescriptorID id, const CSSTokenizedValue& tokenized_value, const CSSParserContext& context)
{
...
CSSValue* ConsumeFontMetricOverride(CSSParserTokenRange& range, const CSSParserContext& context)
{
    if (CSSIdentifierValue* normal = css_parsing_utils::ConsumeIdent<CSSValueID::kNormal>(range)) {
        return normal;
    }
...
CSSValue* AtRuleDescriptorParser::ParseFontFaceDescriptor(AtRuleDescriptorID id, CSSParserTokenRange& range, const CSSParserContext& context)
{
    CSSValue* parsed_value = nullptr;
    range.ConsumeWhitespace();
    switch (id) {
...
CSSValue* AtRuleDescriptorParser::ParseFontFaceDescriptor(AtRuleDescriptorID id, const String& string, const CSSParserContext& context)
{
...
CSSValue* AtRuleDescriptorParser::ParseFontFaceDeclaration(CSSParserTokenRange& range, const CSSParserContext& context)
{
...
CSSValue* AtRuleDescriptorParser::ParseAtPropertyDescriptor(AtRuleDescriptorID id, const CSSTokenizedValue& tokenized_value, const CSSParserContext& context)
{
    CSSValue* parsed_value = nullptr;
    CSSParserTokenRange range = tokenized_value.range;
    switch (id) {
...
bool AtRuleDescriptorParser::ParseAtRule(StyleRule::RuleType rule_type, AtRuleDescriptorID id, const CSSTokenizedValue& tokenized_value,
