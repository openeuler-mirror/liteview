...
static bool IsValidPropertyValueForStyleRule(CSSPropertyID property_id, const String& value)
{
    ...
    return CSSPropertyParser::ParseValue(
        property_id, false, range, StrictCSSParserContext(SecureContextMode::kSecureContext), parsed_properties, StyleRule::RuleType::kStyle);
}
...
TEST(CSSPropertyParserTest, ClipPathEllipse)
{
    ...
    auto* context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kSecureContext, CSSParserContext::kLiveProfile, doc);
...
TEST(CSSPropertyParserTest, ScrollCustomizationPropertySingleValue)
{
    ...
    const CSSValue* value
        = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-down", StrictCSSParserContext(SecureContextMode::kSecureContext));
    const auto* list = To<CSSValueList>(value);
    EXPECT_EQ(1U, list->length());
    EXPECT_EQ(CSSValueID::kPanDown, To<CSSIdentifierValue>(list->Item(0U)).GetValueID());
}
...
TEST(CSSPropertyParserTest, ScrollCustomizationPropertyTwoValuesCombined)
{
    ScopedScrollCustomizationForTest scoped_feature(true);
    const CSSValue* value
        = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-left pan-y", StrictCSSParserContext(SecureContextMode::kSecureContext));
    const auto* list = To<CSSValueList>(value);
    EXPECT_EQ(2U, list->length());
    EXPECT_EQ(CSSValueID::kPanLeft, To<CSSIdentifierValue>(list->Item(0U)).GetValueID());
    EXPECT_EQ(CSSValueID::kPanY, To<CSSIdentifierValue>(list->Item(1U)).GetValueID());
}
...
TEST(CSSPropertyParserTest, ScrollCustomizationPropertyInvalidEntries)
{
    ...
    ScopedScrollCustomizationForTest scoped_feature(true);
    const CSSValue* value
        = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-left pan-right", StrictCSSParserContext(SecureContextMode::kSecureContext));
    EXPECT_FALSE(value);
    value = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-up pan-down", StrictCSSParserContext(SecureContextMode::kSecureContext));
    EXPECT_FALSE(value);
    value = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-x pan-left", StrictCSSParserContext(SecureContextMode::kSecureContext));
    EXPECT_FALSE(value);
    value = CSSParser::ParseSingleValue(CSSPropertyID::kScrollCustomization, "pan-x pan-x", StrictCSSParserContext(SecureContextMode::kSecureContext));
    EXPECT_FALSE(value);
}
...
TEST(CSSPropertyParserTest, PaintUseCount)
{
    ...
    dummy_page_holder->GetFrame().Loader().CommitNavigation(
        WebNavigationParams::CreateWithHTMLBufferForTesting(SharedBuffer::Create(), KURL("https://example.com")), nullptr /* extra_data */);
    Document& document = dummy_page_holder->GetDocument();
    Page::InsertOrdinaryPageForTesting(&dummy_page_holder->GetPage());
...
TEST(CSSPropertyParserTest, InternalLightDarkAuthor)
{
    ...
    ASSERT_FALSE(CSSParser::ParseSingleValue(CSSPropertyID::kColor, "-internal-light-dark(#000000, #ffffff)", context));
    ASSERT_FALSE(CSSParser::ParseSingleValue(CSSPropertyID::kColor, "-internal-light-dark(red, green)", context));
    ASSERT_FALSE(CSSParser::ParseSingleValue(CSSPropertyID::kBackgroundImage, "-internal-light-dark(url(light.png), url(dark.png))", context));
}
...
{
    auto* ua_context = MakeGarbageCollected<CSSParserContext>(kUASheetMode, SecureContextMode::kInsecureContext);
    const CSSValue* value = CSSParser::ParseSingleValue(CSSPropertyID::kColor, "-internal-light-dark(red,#aaa)", ua_context);
    ASSERT_TRUE(value);
...
bool ParseCSSValue(CSSPropertyID property_id, const String& value, const CSSParserContext* context)
{
    ...
    const CSSParserTokenRange range(tokens);
    HeapVector<CSSPropertyValue, 64> parsed_properties;
    return CSSPropertyParser::ParseValue(property_id, false, range, context, parsed_properties, StyleRule::RuleType::kStyle);
}
...
TEST(CSSPropertyParserTest, ParseRevert)
{
    ...
    const CSSValue* value = CSSPropertyParser::ParseSingleValue(CSSPropertyID::kMarginLeft, CSSParserTokenRange(tokens), context);
    ASSERT_TRUE(value);
