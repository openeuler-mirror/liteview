...
double CSSTokenizerInputStream::GetDouble(unsigned start, unsigned end) const
{
    ...
    if (start < end) {
        if (string_->Is8Bit())
            result = CharactersToDouble(string_->Characters8() + offset_ + start, end - start, &is_result_ok);
        else
            result = CharactersToDouble(string_->Characters16() + offset_ + start, end - start, &is_result_ok);
    }
