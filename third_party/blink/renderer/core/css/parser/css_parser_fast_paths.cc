...
static CSSValue* ParseSimpleLengthValue(CSSPropertyID property_id, const String& string, CSSParserMode css_parser_mode)
{
    DCHECK(!string.empty());
    ...
    const bool parsed_simple_length
        = WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return ParseSimpleLength(chars, length, unit, number); });
...
{
    if (length > 3 && (characters[length - 3] | 0x20) == 'd' && (characters[length - 2] | 0x20) == 'e' && (characters[length - 1] | 0x20) == 'g') {
        length -= 3;
        unit = CSSPrimitiveValue::UnitType::kDegrees;
    } else if (length > 4 && (characters[length - 4] | 0x20) == 'g' && (characters[length - 3] | 0x20) == 'r' && (characters[length - 2] | 0x20) == 'a'
        && (characters[length - 1] | 0x20) == 'd') { // Note: 'grad' must be checked before 'rad'.
        length -= 4;
        unit = CSSPrimitiveValue::UnitType::kGradians;
    } else if (length > 3 && (characters[length - 3] | 0x20) == 'r' && (characters[length - 2] | 0x20) == 'a' && (characters[length - 1] | 0x20) == 'd') {
        length -= 3;
        unit = CSSPrimitiveValue::UnitType::kRadians;
    } else if (length > 4 && (characters[length - 4] | 0x20) == 't' && (characters[length - 3] | 0x20) == 'u' && (characters[length - 2] | 0x20) == 'r'
...
template <typename CharacterType> static int FindLengthOfValidDouble(const CharacterType* string, const CharacterType* end)
{
...
template <typename CharacterType>
static bool ContainsCharAtPos(const CharacterType* string, const CharacterType* end, int pos, char ch, bool also_accept_whitespace)
{
    DCHECK_GE(pos, 0);
...
template <typename CharacterType> static int ParseDouble(const CharacterType* string, const CharacterType* end, double& value)
{
...
// no decimal part.
template <typename CharacterType>
static bool ParseFloatWithMaxValue(const CharacterType*& string, const CharacterType* end, int max_value, double& value, bool& negative)
{
    value = 0.0;
...
template <typename CharacterType>
static bool SkipToTerminator(const CharacterType*& string, const CharacterType* end, const char terminator, TerminatorStatus& terminator_status)
{
    const CharacterType* current = string;
...
template <typename CharacterType>
static bool ParseColorNumberOrPercentage(const CharacterType*& string, const CharacterType* end, const char terminator, TerminatorStatus& terminator_status,
    CSSPrimitiveValue::UnitType& expect, int& value)
{
    const CharacterType* current = string;
    double local_value;
    bool negative = false;
    if (!ParseFloatWithMaxValue<CharacterType>(current, end, 255, local_value, negative))
        return false;
    if (current == end)
        return false;
...
// 0.0..1.0.
template <typename CharacterType>
static bool ParsePercentage(const CharacterType*& string, const CharacterType* end, const char terminator, TerminatorStatus& terminator_status, double& value)
{
    const CharacterType* current = string;
    bool negative = false;
    if (!ParseFloatWithMaxValue<CharacterType>(current, end, 100, value, negative)) {
        return false;
    }
...
template <typename CharacterType> static inline bool IsTenthAlpha(const CharacterType* string, const wtf_size_t length)
{
    // "0.X"
...
template <typename CharacterType> static inline bool ParseAlphaValue(const CharacterType*& string, const CharacterType* end, const char terminator, int& value)
{
...
template <typename CharacterType> static inline bool MightBeRGBOrRGBA(const CharacterType* characters, unsigned length)
{
    if (length < 5)
        return false;
    return IsASCIIAlphaCaselessEqual(characters[0], 'r') && IsASCIIAlphaCaselessEqual(characters[1], 'g') && IsASCIIAlphaCaselessEqual(characters[2], 'b')
        && (characters[3] == '(' || (IsASCIIAlphaCaselessEqual(characters[3], 'a') && characters[4] == '('));
}

template <typename CharacterType> static inline bool MightBeHSLOrHSLA(const CharacterType* characters, unsigned length)
{
    if (length < 5)
        return false;
    return IsASCIIAlphaCaselessEqual(characters[0], 'h') && IsASCIIAlphaCaselessEqual(characters[1], 's') && IsASCIIAlphaCaselessEqual(characters[2], 'l')
        && (characters[3] == '(' || (IsASCIIAlphaCaselessEqual(characters[3], 'a') && characters[4] == '('));
}

template <typename CharacterType> static bool FastParseColorInternal(Color& color, const CharacterType* characters, unsigned length, bool quirks_mode)
{
    ...
    if (MightBeHSLOrHSLA(characters, length)) {
        ...
        double hue;
        if (!ParseSimpleAngle(current, static_cast<unsigned>(hue_end - current), hue_unit, hue)) {
            return false;
        }
        ...
        if (should_have_alpha) {
            ...
            if (current != end)
                return false;
            color = Color::FromHSLA(hue, saturation, lightness, alpha * (1.0 / 255.0));
        } else {
            if (current != end)
                return false;
...
static CSSValue* ParseColor(CSSPropertyID property_id, const String& string, CSSParserMode parser_mode)
{
    ...
    bool parse_result
        = WTF::VisitCharacters(string, [&](const auto* chars, unsigned length) { return FastParseColorInternal(color, chars, length, quirks_mode); });
    if (!parse_result)
        return nullptr;
    return cssvalue::CSSColor::Create(color);
}

CSSValue* CSSParserFastPaths::ParseColor(const String& string, CSSParserMode parser_mode)
{
...
bool CSSParserFastPaths::IsValidKeywordPropertyAndValue(CSSPropertyID property_id, CSSValueID value_id, CSSParserMode parser_mode)
{
    ...
    case CSSPropertyID::kBackgroundRepeatY:
        return value_id == CSSValueID::kRepeat || value_id == CSSValueID::kNoRepeat;
    ...
    case CSSPropertyID::kClear:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kLeft || value_id == CSSValueID::kRight || value_id == CSSValueID::kBoth
            || (RuntimeEnabledFeatures::CSSLogicalEnabled() && (value_id == CSSValueID::kInlineStart || value_id == CSSValueID::kInlineEnd));
    ...
    case CSSPropertyID::kResize:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kBoth || value_id == CSSValueID::kHorizontal || value_id == CSSValueID::kVertical
            || (RuntimeEnabledFeatures::CSSLogicalEnabled() && (value_id == CSSValueID::kBlock || value_id == CSSValueID::kInline))
    ...
    case CSSPropertyID::kTextTransform:
        return (value_id >= CSSValueID::kCapitalize && value_id <= CSSValueID::kLowercase) || value_id == CSSValueID::kNone
            || (RuntimeEnabledFeatures::CSSMathVariantEnabled() && value_id == CSSValueID::kMathAuto);
    ...
    case CSSPropertyID::kAppearance:
        return (value_id >= CSSValueID::kCheckbox && value_id <= CSSValueID::kTextarea) || value_id == CSSValueID::kNone || value_id == CSSValueID::kAuto;
    ...
    case CSSPropertyID::kMixBlendMode:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kMultiply || value_id == CSSValueID::kScreen || value_id == CSSValueID::kOverlay
            ...
            || value_id == CSSValueID::kLuminosity || (RuntimeEnabledFeatures::CSSMixBlendModePlusLighterEnabled() && value_id == CSSValueID::kPlusLighter);
    ...
    case CSSPropertyID::kTransformBox:
        return value_id == CSSValueID::kFillBox || value_id == CSSValueID::kViewBox;
    ...
    case CSSPropertyID::kWhiteSpace:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kPre || value_id == CSSValueID::kPreWrap || value_id == CSSValueID::kPreLine
            || value_id == CSSValueID::kNowrap || value_id == CSSValueID::kBreakSpaces;
...
static CSSValue* ParseKeywordValue(CSSPropertyID property_id, const String& string, CSSParserMode parser_mode)
{
    DCHECK(!string.empty());

    if (!CSSParserFastPaths::IsKeywordPropertyID(property_id)) {
        // All properties accept CSS-wide keywords.
        if (!EqualIgnoringASCIICase(string, "initial") && !EqualIgnoringASCIICase(string, "inherit") && !EqualIgnoringASCIICase(string, "unset")
            && !EqualIgnoringASCIICase(string, "revert") && !EqualIgnoringASCIICase(string, "revert-layer"))
            return nullptr;
    ...
    if (CSSParserFastPaths::IsValidKeywordPropertyAndValue(property_id, value_id, parser_mode))
        return CSSIdentifierValue::Create(value_id);
    return nullptr;
}

template <typename CharType>
static bool ParseTransformTranslateArguments(CharType*& pos, CharType* end, unsigned expected_count, CSSFunctionValue* transform_value)
{
...
template <typename CharType> static bool ParseTransformRotateArgument(CharType*& pos, CharType* end, CSSFunctionValue* transform_value)
{
...
template <typename CharType>
static bool ParseTransformNumberArguments(CharType*& pos, CharType* end, unsigned expected_count, CSSFunctionValue* transform_value)
{
    while (expected_count) {
        wtf_size_t delimiter = WTF::Find(pos, static_cast<wtf_size_t>(end - pos), expected_count == 1 ? ')' : ',');
        if (delimiter == kNotFound)
        ...
        unsigned argument_length = static_cast<unsigned>(delimiter);
        bool ok;
        double number = CSSValueClampingUtils::ClampDouble(CharactersToDouble(pos, argument_length, &ok));
        if (!ok)
            return false;
...
template <typename CharType> static CSSFunctionValue* ParseSimpleTransformValue(CharType*& pos, CharType* end)
{
    ...
    const bool is_translate = ToASCIILower(pos[0]) == 't' && ToASCIILower(pos[1]) == 'r' && ToASCIILower(pos[2]) == 'a' && ToASCIILower(pos[3]) == 'n'
        && ToASCIILower(pos[4]) == 's' && ToASCIILower(pos[5]) == 'l' && ToASCIILower(pos[6]) == 'a' && ToASCIILower(pos[7]) == 't'
    ...
    const bool is_matrix3d = ToASCIILower(pos[0]) == 'm' && ToASCIILower(pos[1]) == 'a' && ToASCIILower(pos[2]) == 't' && ToASCIILower(pos[3]) == 'r'
        && ToASCIILower(pos[4]) == 'i' && ToASCIILower(pos[5]) == 'x' && pos[6] == '3' && ToASCIILower(pos[7]) == 'd' && pos[8] == '(';
    ...
    const bool is_scale3d = ToASCIILower(pos[0]) == 's' && ToASCIILower(pos[1]) == 'c' && ToASCIILower(pos[2]) == 'a' && ToASCIILower(pos[3]) == 'l'
        && ToASCIILower(pos[4]) == 'e' && pos[5] == '3' && ToASCIILower(pos[6]) == 'd' && pos[7] == '(';
    ...
    const bool is_rotate = ToASCIILower(pos[0]) == 'r' && ToASCIILower(pos[1]) == 'o' && ToASCIILower(pos[2]) == 't' && ToASCIILower(pos[3]) == 'a'
        && ToASCIILower(pos[4]) == 't' && ToASCIILower(pos[5]) == 'e' && pos[6] == '(';

    if (is_rotate) {
        pos += 7;
        CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kRotate);
...
template <typename CharType> static bool TransformCanLikelyUseFastPath(const CharType* chars, unsigned length)
{
...
static CSSValue* ParseSimpleTransform(CSSPropertyID property_id, const String& string)
{
    DCHECK(!string.empty());
    ...
    return WTF::VisitCharacters(string, [&](const auto* pos, unsigned length) -> CSSValueList* {
...
CSSValue* CSSParserFastPaths::MaybeParseValue(CSSPropertyID property_id, const String& string, CSSParserMode parser_mode)
{
