...
public:
    CachedCSSTokenizer(const String& input, Vector<CSSParserToken> tokens, Vector<wtf_size_t> offsets, Vector<String> string_pool)
        : input_(input)
...
public:
    ...
    static std::unique_ptr<CachedCSSTokenizer> CreateCachedTokenizer(const String& input);
...
public:
    ...
    wtf_size_t PreviousOffset() const
    {
        return tokenizer_ ? tokenizer_->PreviousOffset() : cached_tokenizer_->PreviousOffset();
    }
    StringView StringRangeAt(wtf_size_t start, wtf_size_t length) const
    {
        return tokenizer_ ? tokenizer_->StringRangeAt(start, length) : cached_tokenizer_->StringRangeAt(start, length);
    }
    CSSParserToken TokenizeSingle()
    {
        return tokenizer_ ? tokenizer_->TokenizeSingle() : cached_tokenizer_->TokenizeSingle();
    }
    CSSParserToken TokenizeSingleWithComments()
    {
        return tokenizer_ ? tokenizer_->TokenizeSingleWithComments() : cached_tokenizer_->TokenizeSingleWithComments();
    }
    wtf_size_t TokenCount()
    {
        return tokenizer_ ? tokenizer_->TokenCount() : cached_tokenizer_->TokenCount();
    }
