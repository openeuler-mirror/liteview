...
public:
    ...
    bool IsPositionFallbackRule() const
    {
        return GetType() == kPositionFallback;
    }
    bool IsTryRule() const
    {
        return GetType() == kTry;
    }
    ...
    CSSRule* CreateCSSOMWrapper(wtf_size_t position_hint = std::numeric_limits<wtf_size_t>::max(), CSSStyleSheet* parent_sheet = nullptr) const;
    CSSRule* CreateCSSOMWrapper(wtf_size_t position_hint, CSSRule* parent_rule) const;
...
private:
    CSSRule* CreateCSSOMWrapper(wtf_size_t position_hint, CSSStyleSheet* parent_sheet, CSSRule* parent_rule) const;
...
public:
    ...
    static StyleRule* Create(CSSSelectorVector& selector_vector, CSSPropertyValueSet* properties)
    {
        size_t flattened_size = CSSSelectorList::FlattenedSize(selector_vector);
        return MakeGarbageCollected<StyleRule>(
            AdditionalBytesForSelectors(flattened_size), base::PassKey<StyleRule>(), selector_vector, flattened_size, properties);
    }
    static StyleRule* Create(CSSSelectorVector& selector_vector, CSSLazyPropertyParser* lazy_property_parser)
    {
        size_t flattened_size = CSSSelectorList::FlattenedSize(selector_vector);
        return MakeGarbageCollected<StyleRule>(
            AdditionalBytesForSelectors(flattened_size), base::PassKey<StyleRule>(), selector_vector, flattened_size, lazy_property_parser);
    }

    // Creates a StyleRule with the selectors changed (used by setSelectorText()).
    static StyleRule* Create(CSSSelectorVector& selector_vector, StyleRule&& other)
    {
        size_t flattened_size = CSSSelectorList::FlattenedSize(selector_vector);
        return MakeGarbageCollected<StyleRule>(
            AdditionalBytesForSelectors(flattened_size), base::PassKey<StyleRule>(), selector_vector, flattened_size, std::move(other));
    }
    ...
    StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, CSSPropertyValueSet*);
    StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, CSSLazyPropertyParser*);
    StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, StyleRule&&);
...
public:
    StyleRuleScope(const StyleScope&, HeapVector<Member<StyleRuleBase>>& adopt_rules);
...
public:
    StyleRuleLayerBlock(LayerName&& name, HeapVector<Member<StyleRuleBase>>& adopt_rule);
...
protected:
    ...
    StyleRuleCondition(RuleType, const String& condition_text, HeapVector<Member<StyleRuleBase>>& adopt_rule);
...
public:
    StyleRuleMedia(const MediaQuerySet*, HeapVector<Member<StyleRuleBase>>& adopt_rules);
...
public:
    StyleRuleSupports(const String& condition_text, bool condition_is_supported, HeapVector<Member<StyleRuleBase>>& adopt_rules);
...
public:
    StyleRuleContainer(ContainerQuery&, HeapVector<Member<StyleRuleBase>>& adopt_rules);
