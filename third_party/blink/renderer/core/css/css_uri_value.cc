...
CSSURIValue::CSSURIValue(const AtomicString& relative_url, const AtomicString& absolute_url)
    : CSSValue(kURIClass)
...
{
    return is_local_ || EqualIgnoringFragmentIdentifier(AbsoluteUrl(), document.Url());
}
...
CSSURIValue* CSSURIValue::ValueWithURLMadeAbsolute(const KURL& base_url, const WTF::TextEncoding& charset) const
{
    if (!charset.IsValid()) {
        return MakeGarbageCollected<CSSURIValue>(AtomicString(KURL(base_url, relative_url_).GetString()));
    }
    return MakeGarbageCollected<CSSURIValue>(AtomicString(KURL(base_url, relative_url_, charset).GetString()));
}
