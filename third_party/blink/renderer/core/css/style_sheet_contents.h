...
public:
    ...
    ParseSheetResult ParseString(const String&, bool allow_import_rules = true, std::unique_ptr<CachedCSSTokenizer> tokenizer = nullptr);
    ...
    void FindFontFaceRules(HeapVector<Member<const StyleRuleFontFace>>& font_face_rules);

    void SetHasViewportRule()
    {
        has_viewport_rule_ = true;
    }
    bool HasViewportRule() const
    {
        return has_viewport_rule_;
    }
    ...
    wtf_size_t ReplaceRuleIfExists(const StyleRuleBase* old_rule, StyleRuleBase* new_rule, wtf_size_t position_hint);
    ...
    {
        return referenced_from_resource_;
    }
