...
PageRuleCollector::PageRuleCollector(const ComputedStyle* root_element_style, uint32_t page_index, const AtomicString& page_name, MatchResult& match_result)
...
void PageRuleCollector::MatchPageRules(RuleSet* rules, const CascadeLayerMap* layer_map)
{
    if (!rules)
        return;
