...
static unsigned DetermineLinkMatchType(const AddRuleFlags add_rule_flags, const CSSSelector& selector)
{
...
RuleData::RuleData(StyleRule* rule, unsigned selector_index, unsigned position, unsigned extra_specificity, AddRuleFlags add_rule_flags)
    : rule_(rule)
...
void RuleData::ComputeBloomFilterHashes()
{
    ...
    marker_ = 0;
#endif
    SelectorFilter::CollectIdentifierHashes(Selector(), descendant_selector_identifier_hashes_, kMaximumIdentifierCount);
}
...
static void ExtractSelectorValues(const CSSSelector* selector, AtomicString& id, AtomicString& class_name, AtomicString& attr_name, AtomicString& attr_value,
    bool& is_exact_attr, AtomicString& custom_pseudo_element_name, AtomicString& tag_name, AtomicString& part_name, CSSSelector::PseudoType& pseudo_type)
{
    is_exact_attr = false;
        ...
        case CSSSelector::kPseudoWhere: {
            ...
            if (selector_list->HasOneSelector()) {
                ExtractSelectorValues(
                    selector_list->First(), id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, pseudo_type);
            }
        } break;
        default:
            break;
    ...
    CSSSelector::PseudoType& pseudo_type)
{
    ...
    for (; it && it->Relation() == CSSSelector::kSubSelector; it = it->TagHistory()) {
        ExtractSelectorValues(it, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, pseudo_type);
    }
    if (it) {
        ExtractSelectorValues(it, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, pseudo_type);
    }
...
bool RuleSet::FindBestRuleSetAndAdd(const CSSSelector& component, const RuleData& rule_data)
{
    AtomicString id;
    ...
    const CSSSelector* it = ExtractBestSelectorValues(
        component, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, pseudo_type);
    ...
    case CSSSelector::kPseudoFileSelectorButton:
        if (it->FollowsPart()) {
            ...
            const auto& name = pseudo_type == CSSSelector::kPseudoFileSelectorButton ? shadow_element_names::kPseudoFileUploadButton
    ...
    const CascadeLayer* cascade_layer, const StyleScope* style_scope)
{
    ...
    RuleData rule_data(rule, selector_index, rule_count_, extra_specificity, add_rule_flags);
    ++rule_count_;
    if (features_.CollectFeaturesFromSelector(rule_data.Selector(), style_scope) == RuleFeatureSet::kSelectorNeverMatches)
        return;
    ...
    if (rule_data.LinkMatchType() == CSSSelector::kMatchLink) {
        RuleData visited_dependent(rule, rule_data.SelectorIndex(), rule_data.GetPosition(), extra_specificity, add_rule_flags | kRuleIsVisitedDependent);
...
void RuleSet::AddRuleToLayerIntervals(const CascadeLayer* cascade_layer, unsigned position)
{
    ...
    const CascadeLayer* last_interval_layer = layer_intervals_.empty() ? implicit_outer_layer_.Get() : layer_intervals_.back().value.Get();
    if (!cascade_layer)
...
void RuleSet::AddChildRules(const HeapVector<Member<StyleRuleBase>>& rules, const MediaQueryEvaluator& medium, AddRuleFlags add_rule_flags,
    ...
    for (unsigned i = 0; i < rules.size(); ++i) {
        ...
        if (auto* style_rule = DynamicTo<StyleRule>(rule)) {
            ...
            if (MatchMediaForAddRules(medium, media_rule->MediaQueries())) {
                AddChildRules(media_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope);
            }
        ...
        } else if (auto* position_fallback_rule = DynamicTo<StyleRulePositionFallback>(rule)) {
            ...
            if (supports_rule->ConditionIsSupported()) {
                AddChildRules(supports_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope);
            }
            ...
            AddChildRules(container_rule->ChildRules(), medium, add_rule_flags, inner_container_query, cascade_layer, style_scope);
            ...
            AddChildRules(layer_block_rule->ChildRules(), medium, add_rule_flags, container_query, sub_layer, style_scope);
            ...
            AddChildRules(scope_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, inner_style_scope);
        }
    }
}
...
void RuleSet::AddRulesFromSheet(StyleSheetContents* sheet, const MediaQueryEvaluator& medium, AddRuleFlags add_rule_flags, CascadeLayer* cascade_layer)
{
    ...
    for (unsigned i = 0; i < import_rules.size(); ++i) {
        ...
        if (import_rule->GetStyleSheet()) {
            AddRulesFromSheet(import_rule->GetStyleSheet(), medium, add_rule_flags, import_layer);
        }
    }

    AddChildRules(sheet->ChildRules(), medium, add_rule_flags, nullptr /* container_query */, cascade_layer, nullptr);
}

void RuleSet::AddStyleRule(StyleRule* rule, AddRuleFlags add_rule_flags)
{
    for (wtf_size_t selector_index = 0; selector_index != kNotFound; selector_index = rule->IndexOfNextSelectorAfter(selector_index)) {
        AddRule(rule, selector_index, add_rule_flags, nullptr /* container_query */, nullptr /* cascade_layer */, nullptr /* scope */);
    }
}
...
{
    RuleMap::Extent& rules = buckets.insert(key, RuleMap::Extent()).stored_value->value;
    if (rules.length == 0)
...
void RuleMap::Compact()
{
    ...
    for (wtf_size_t i = 0; i < backing.size();) {
        ...
        wtf_size_t correct_pos = counts[rule_data.GetBucketNumber()] + rule_data.GetOrderInBucket();
        if (i == correct_pos) {
            ++i;
        } else {
            using std::swap;
...
static wtf_size_t GetMinimumRulesetSizeForSubstringMatcher()
{
    ...
    return base::FeatureList::IsEnabled(blink::features::kSubstringSetTreeForAttributeBuckets) ? 50 : std::numeric_limits<wtf_size_t>::max();
}
...
bool RuleSet::CanIgnoreEntireList(base::span<const RuleData> list, const AtomicString& key, const AtomicString& value) const
{
    ...
    if (list.size() < GetMinimumRulesetSizeForSubstringMatcher()) {
        ...
        DCHECK_EQ(attr_substring_matchers_.find(key), attr_substring_matchers_.end());
        return false;
    }
    ...
    for (const auto& [/*AtomicString*/ attr,
        ...
        for (const RuleData& rule : ruleset) {
            ...
            ExtractBestSelectorValues(
                rule.Selector(), id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, pseudo_type);
