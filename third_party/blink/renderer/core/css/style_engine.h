...
public:
    ...
    void AddedCustomElementDefaultStyles(const HeapVector<Member<CSSStyleSheet>>& default_styles);
    ...
    bool UsesRemUnits() const
    {
        return uses_rem_units_;
    }
    void SetUsesRemUnit(bool uses_rem_units)
    {
        uses_rem_units_ = uses_rem_units;
    }
    bool UpdateRemUnits(const ComputedStyle* old_root_style, const ComputedStyle* new_root_style);
    ...
    void ComputeFont(Element& element, ComputedStyle* font_style, const CSSPropertyValueSet& font_properties);
    ...
    void ScheduleInvalidationsForRuleSets(TreeScope&, const HeapHashSet<Member<RuleSet>>&, InvalidationScope = kInvalidateCurrentScope);
    void ScheduleCustomElementInvalidations(HashSet<AtomicString> tag_names);
    void ScheduleInvalidationsForHasPseudoAffectedByInsertion(Element* parent, Node* node_before_change, Element& element);
    void ScheduleInvalidationsForHasPseudoAffectedByRemoval(Element* parent, Node* node_before_change, Element& element);
    ...
    void ApplyVisionDeficiencyStyle(scoped_refptr<ComputedStyle> layout_view_style);
    ...
    void RebuildLayoutTree(RebuildTransitionPseudoTree rebuild_transition_pseudo_tree);
    ...
    StyleFetchedImage* CacheStyleImage(FetchParameters& params, OriginClean origin_clean, bool is_ad_related)
    {
        return style_image_cache_.CacheStyleImage(GetDocument(), params, origin_clean, is_ad_related);
    }
...
private:
    ...
    void ScheduleRuleSetInvalidationsForElement(Element&, const HeapHashSet<Member<RuleSet>>&);
    void ScheduleTypeRuleSetInvalidations(ContainerNode&, const HeapHashSet<Member<RuleSet>>&);
    ...
    HeapHashMap<WeakMember<StyleSheetContents>, AtomicString> sheet_to_text_cache_;
...
{
    return RuntimeEnabledFeatures::LayoutNGPrintingEnabled() && RuntimeEnabledFeatures::LayoutNGFlexFragmentationEnabled()
        && RuntimeEnabledFeatures::LayoutNGGridFragmentationEnabled() && RuntimeEnabledFeatures::LayoutNGTableFragmentationEnabled();
}
