...
TEST(SelectorQueryTest, NotMatchingPseudoElement)
{
    ...
    std::unique_ptr<SelectorQuery> query = SelectorQuery::Adopt(std::move(selector_list));
...
TEST(SelectorQueryTest, FastPathScoped)
{
    ...
    ShadowRoot& shadowRoot = scope->AttachShadowRootInternal(ShadowRootType::kOpen);
    // Make the inside the shadow root be identical to that of the outer document.
    shadowRoot.appendChild(&document->documentElement()->CloneWithChildren(CloneChildrenFlag::kClone));
    static const struct QueryTest kTestCases[] = {
...
TEST(SelectorQueryTest, DisconnectedTreeScope)
{
    ...
    ShadowRoot& shadowRoot = host->AttachShadowRootInternal(ShadowRootType::kOpen);
