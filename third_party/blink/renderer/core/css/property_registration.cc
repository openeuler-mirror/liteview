...
PropertyRegistration::PropertyRegistration(const AtomicString& name, const CSSSyntaxDefinition& syntax, bool inherits, const CSSValue* initial)
    : syntax_(syntax)
    , inherits_(inherits)
    , initial_(initial)
...
static bool ComputationallyIndependent(const CSSValue& value)
{
    ...
    if (auto* variable_reference_value = DynamicTo<CSSVariableReferenceValue>(value)) {
...
static absl::optional<CSSSyntaxDefinition> ConvertSyntax(const CSSValue& value)
{
...
static scoped_refptr<CSSVariableData> ConvertInitialVariableData(const CSSValue* value)
{
    if (!value)
        return nullptr;
...
PropertyRegistration* PropertyRegistration::MaybeCreateForDeclaredProperty(Document& document, const AtomicString& name, StyleRuleProperty& rule)
{
    ...
    scoped_refptr<CSSVariableData> initial_variable_data = ConvertInitialVariableData(initial_value);
    ...
    if (initial_variable_data) {
        ...
        initial = syntax->Parse(initial_variable_data->TokenRange(), *parser_context, is_animation_tainted);
        if (!initial)
            return nullptr;
    ...
    return MakeGarbageCollected<PropertyRegistration>(name, *syntax, inherits, initial);
}
...
void PropertyRegistration::registerProperty(ExecutionContext* execution_context, const PropertyDefinition* property_definition, ExceptionState& exception_state)
{
    ...
    absl::optional<CSSSyntaxDefinition> syntax_definition = CSSSyntaxStringParser(property_definition->syntax()).Parse();
    ...
    if (property_definition->hasInitialValue()) {
        ...
        initial = syntax_definition->Parse(CSSParserTokenRange(tokens), *parser_context, is_animation_tainted);
        if (!initial) {
