...
public:
    CSSImageValue(const AtomicString& raw_value, const KURL&, const Referrer&, OriginClean origin_clean, bool is_ad_related, StyleImage* image = nullptr);
    ~CSSImageValue();
    ...
    {
        return MakeGarbageCollected<CSSImageValue>(absolute_url_, KURL(absolute_url_), Referrer(), origin_clean_, is_ad_related_, cached_image_.Get());
    }

    CSSImageValue* Clone() const
    {
        return MakeGarbageCollected<CSSImageValue>(relative_url_, KURL(absolute_url_), Referrer(), origin_clean_, is_ad_related_, cached_image_.Get());
    }
