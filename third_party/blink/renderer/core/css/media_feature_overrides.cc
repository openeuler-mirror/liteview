...
absl::optional<ColorSpaceGamut> ConvertColorGamut(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
...
absl::optional<mojom::blink::PreferredColorScheme> ConvertPreferredColorScheme(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
...
absl::optional<mojom::blink::PreferredContrast> ConvertPreferredContrast(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
...
absl::optional<bool> ConvertPrefersReducedMotion(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
...
absl::optional<bool> ConvertPrefersReducedData(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
    return value.Id() == CSSValueID::kReduce;
...
absl::optional<ForcedColors> ConvertForcedColors(const MediaQueryExpValue& value)
{
    if (!value.IsValid())
        return absl::nullopt;
...
void MediaFeatureOverrides::SetOverride(const AtomicString& feature, const String& value_string)
{
    ...
    const CSSParserContext* fake_context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
    ...
    // removed.
    MediaQueryExpBounds bounds = MediaQueryExp::Create(feature, range, *fake_context).Bounds();
