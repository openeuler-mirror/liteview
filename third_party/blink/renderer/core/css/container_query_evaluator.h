...
public:
    ...
    static Element* FindContainer(Element* starting_element, const ContainerSelector&);
    static bool EvalAndAdd(const Element& matching_element, const StyleRecalcContext&, const ContainerQuery&, ContainerSelectorCache&, MatchResult&);
    ...
    // unchanged otherwise).
    Change SizeContainerChanged(Document&, Element& container, PhysicalSize, PhysicalAxes contained_axes);
    ...
    void MarkFontDirtyIfNeeded(const ComputedStyle& old_style, const ComputedStyle& new_style);
...
private:
    ...
    void SetData(Document&, Element& container, PhysicalSize, PhysicalAxes contained_axes);
