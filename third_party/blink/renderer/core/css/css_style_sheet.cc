...
unsigned CSSStyleSheet::insertRule(const String& rule_string, unsigned index, ExceptionState& exception_state)
{
    ...
    StyleRuleBase* rule = CSSParser::ParseRule(context, contents_.Get(), rule_string);
...
ScriptPromise CSSStyleSheet::replace(ScriptState* script_state, const String& text, ExceptionState& exception_state)
{
    if (!IsConstructed()) {
