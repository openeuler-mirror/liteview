...
const CSSSelector* CheckPseudoHasArgumentContext::GetCurrentRelationAndNextCompound(const CSSSelector* compound_selector, CSSSelector::RelationType& relation)
{
    DCHECK(compound_selector);
    for (const CSSSelector* simple_selector = compound_selector; simple_selector; simple_selector = simple_selector->TagHistory()) {
...
CheckPseudoHasArgumentContext::CheckPseudoHasArgumentContext(const CSSSelector* selector)
    : has_argument_(selector)
{
    ...
    for (selector = GetCurrentRelationAndNextCompound(selector, relation); selector; selector = GetCurrentRelationAndNextCompound(selector, relation)) {
        switch (relation) {
...
Element* CheckPseudoHasArgumentTraversalIterator::LastWithin(Element* element)
{
    ...
    for (Element* descendant = ElementTraversal::LastChild(*element); descendant; descendant = ElementTraversal::LastChild(*descendant)) {
