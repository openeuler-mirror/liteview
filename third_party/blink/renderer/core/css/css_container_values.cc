...
CSSContainerValues::CSSContainerValues(Document& document, Element& container, absl::optional<double> width, absl::optional<double> height)
    : MediaValuesDynamic(document.GetFrame())
    , element_(&container)
    , width_(width)
    , height_(height)
    , writing_mode_(container.ComputedStyleRef().GetWritingMode())
    , font_sizes_(CSSToLengthConversionData::FontSizes(container.GetComputedStyle(), document.documentElement()->GetComputedStyle()).Unzoomed())
...
{
    return AdjustForAbsoluteZoom::AdjustFloat(element_->ComputedStyleRef().ComputedLineHeight(), element_->ComputedStyleRef());
}
