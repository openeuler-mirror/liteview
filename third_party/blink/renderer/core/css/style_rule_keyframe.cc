...
StyleRuleKeyframe::StyleRuleKeyframe(std::unique_ptr<Vector<double>> keys, CSSPropertyValueSet* properties)
...
bool StyleRuleKeyframe::SetKeyText(const String& key_text)
{
    ...
    std::unique_ptr<Vector<double>> keys = CSSParser::ParseKeyframeKeyList(key_text);
    if (!keys || keys->empty())
        return false;
