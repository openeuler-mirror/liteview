...
absl::optional<Member<Element>> FirstFlatTreeAncestorForChildDirty(ContainerNode& parent)
{
...
void StyleRecalcRoot::SubtreeModified(ContainerNode& parent)
{
    ...
    for (Element* ancestor = opt_ancestor.value(); ancestor; ancestor = ancestor->GetStyleRecalcParent()) {
