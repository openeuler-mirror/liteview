...
StyleRuleFontPaletteValues::StyleRuleFontPaletteValues(const AtomicString& name, CSSPropertyValueSet* properties)
    ...
    , base_palette_(properties->GetPropertyCSSValue(CSSPropertyID::kBasePalette))
    , override_colors_(properties->GetPropertyCSSValue(CSSPropertyID::kOverrideColors))
{
    DCHECK(properties);
}
...
FontPalette::BasePaletteValue StyleRuleFontPaletteValues::GetBasePaletteIndex() const
{
    ...
    if (auto* base_palette_identifier = DynamicTo<CSSIdentifierValue>(*base_palette_)) {
    ...
    const CSSPrimitiveValue& palette_primitive = To<CSSPrimitiveValue>(*base_palette_);
...
Vector<FontPalette::FontPaletteOverride> StyleRuleFontPaletteValues::GetOverrideColorsAsVector() const
{
    ...
    auto ConvertToSkColor = [](const CSSValuePair& override_pair) -> SkColor {
        if (override_pair.Second().IsIdentifierValue()) {
            ...
            return static_cast<SkColor>(StyleColor::ColorFromKeyword(color_identifier.GetValueID(), mojom::blink::ColorScheme::kLight).Rgb());
        }
    ...
    for (auto& item : overrides_list) {
        ...
        FontPalette::FontPaletteOverride palette_override { palette_index.GetIntValue(), override_color };
