...
{
    RuleSet& rule_set = style_sheet_->Contents()->EnsureRuleSet(MediaQueryEvaluator(document_->GetFrame()), kRuleHasNoSpecialState);
...
void RegisterProperty(Document& document, const String& name, const String& syntax, const absl::optional<String>& initial_value, bool is_inherited)
{
    DummyExceptionStateForTesting exception_state;
...
void DeclareProperty(Document& document, const String& name, const String& syntax, const absl::optional<String>& initial_value, bool is_inherited)
{
...
scoped_refptr<CSSVariableData> CreateVariableData(String s)
{
    ...
    return CSSVariableData::Create({ range, StringView(s) }, is_animation_tainted, needs_variable_resolution);
}
