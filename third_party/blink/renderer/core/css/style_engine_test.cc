...
protected:
    ...
    enum RuleSetInvalidation { kRuleSetInvalidationsScheduled, kRuleSetInvalidationFullRecalc };
    RuleSetInvalidation ScheduleInvalidationsForRules(TreeScope&, const String& css_text);
    ...
    String GetListMarkerText(LayoutObject* list_item)
    {
        ...
        if (auto* legacy_marker = DynamicTo<LayoutListMarker>(marker)) {
            ...
            return counter_style.GetPrefix() + legacy_marker->GetText() + counter_style.GetSuffix();
        }
...
class StyleEngineContainerQueryTest : public StyleEngineTest, private ScopedCSSContainerQueriesForTest, private ScopedLayoutNGForTest {
public:
...
StyleEngineTest::RuleSetInvalidation StyleEngineTest::ScheduleInvalidationsForRules(TreeScope& tree_scope, const String& css_text)
{
    ...
    RuleSet& rule_set = sheet->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasDocumentSecurityOrigin);
...
TEST_F(StyleEngineTest, AnalyzedInject)
{
    ...
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ BoldWeightValue(), BoldWeightValue() }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ NormalSlopeValue(), NormalSlopeValue() }));
    ...
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ BoldWeightValue(), BoldWeightValue() }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ ItalicSlopeValue(), ItalicSlopeValue() }));

    auto* style_element = MakeGarbageCollected<HTMLStyleElement>(GetDocument(), CreateElementFlags());
    ...
    EXPECT_TRUE(font_face);
    capabilities = font_face->GetFontSelectionCapabilities();
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ NormalWeightValue(), NormalWeightValue() }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ ItalicSlopeValue(), ItalicSlopeValue() }));
    ...
    ASSERT_FALSE(GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation")));
    ...
    StyleRuleKeyframes* keyframes = GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation"));
    ASSERT_TRUE(keyframes);
    EXPECT_EQ(1u, keyframes->Keyframes().size());

    style_element = MakeGarbageCollected<HTMLStyleElement>(GetDocument(), CreateElementFlags());
    ...
    // and to).
    keyframes = GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation"));
    ASSERT_TRUE(keyframes);
...
{
    auto* element = MakeGarbageCollected<HTMLStyleElement>(GetDocument(), CreateElementFlags());
    ...
    element = MakeGarbageCollected<HTMLStyleElement>(GetDocument(), CreateElementFlags());
...
TEST_F(StyleEngineTest, RuleSetInvalidationTypeSelectors)
{
    ...
    EXPECT_EQ(kRuleSetInvalidationsScheduled, ScheduleInvalidationsForRules(GetDocument(), "span { background: green}"));
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);

    before_count = after_count;
    EXPECT_EQ(kRuleSetInvalidationsScheduled, ScheduleInvalidationsForRules(GetDocument(), "body div { background: green}"));
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(2u, after_count - before_count);

    EXPECT_EQ(kRuleSetInvalidationFullRecalc, ScheduleInvalidationsForRules(GetDocument(), "div * { background: green}"));
    UpdateAllLifecyclePhases();

    before_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(kRuleSetInvalidationsScheduled, ScheduleInvalidationsForRules(GetDocument(), "#i b { background: green}"));
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);
}
...
TEST_F(StyleEngineTest, RuleSetInvalidationCustomPseudo)
{
    ...
    unsigned before_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(ScheduleInvalidationsForRules(GetDocument(), "::-webkit-progress-bar { background: green }"), kRuleSetInvalidationsScheduled);
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
...
TEST_F(StyleEngineTest, RuleSetInvalidationHost)
{
    ...
    ShadowRoot& shadow_root = host->AttachShadowRootInternal(ShadowRootType::kOpen);

    shadow_root.setInnerHTML("<div></div><div></div><div></div>");
    UpdateAllLifecyclePhases();

    unsigned before_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host(#nohost), #nohost { background: green}"), kRuleSetInvalidationsScheduled);
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(0u, after_count - before_count);

    before_count = after_count;
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host(#host) { background: green}"), kRuleSetInvalidationsScheduled);
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host(div) { background: green}"), kRuleSetInvalidationsScheduled);

    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host(*) { background: green}"), kRuleSetInvalidationFullRecalc);
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host(*) :hover { background: green}"), kRuleSetInvalidationFullRecalc);
}
...
TEST_F(StyleEngineTest, RuleSetInvalidationSlotted)
{
    ...
    unsigned before_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, "::slotted(.s1) { background: green}"), kRuleSetInvalidationsScheduled);
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(4u, after_count - before_count);

    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, "::slotted(*) { background: green}"), kRuleSetInvalidationFullRecalc);
}
...
TEST_F(StyleEngineTest, RuleSetInvalidationHostContext)
{
    ...
    unsigned before_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host-context(.nomatch) .a { background: green}"), kRuleSetInvalidationsScheduled);
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);

    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host-context(:hover) { background: green}"), kRuleSetInvalidationFullRecalc);
    EXPECT_EQ(ScheduleInvalidationsForRules(shadow_root, ":host-context(#host) { background: green}"), kRuleSetInvalidationFullRecalc);
}
...
TEST_F(StyleEngineTest, MediaQueriesChangePrefersContrastOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride("prefers-contrast", "no-preference");
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride("prefers-contrast", "custom");
...
TEST_F(StyleEngineTest, MediaQueriesColorSchemeOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride("prefers-color-scheme", "dark");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, MediaQueriesReducedMotionOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride("prefers-reduced-motion", "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, GetComputedStyleOutsideFlatTreeCrash)
{
    ...
    GetDocument().getElementById("host")->AttachShadowRootInternal(ShadowRootType::kOpen);
    UpdateAllLifecyclePhases();
    GetDocument().body()->EnsureComputedStyle();
    GetDocument().getElementById("non-slotted")->SetInlineStyleProperty(CSSPropertyID::kColor, "blue");
    UpdateAllLifecyclePhases();
}
...
TEST_F(StyleEngineTest, EnsureCustomComputedStyle)
{
    ...
    // IsEnsuredInDisplayNone==true
    for (Node* node = progress; node; node = FlatTreeTraversal::Next(*node, progress)) {
        ASSERT_TRUE(!node->GetComputedStyle() || !node->ComputedStyleRef().IsEnsuredInDisplayNone() || !node->GetLayoutObject());
    }
}
...
TEST_F(StyleEngineTest, ColorSchemeBaseBackgroundChange)
{
    ...
    color_scheme_helper.SetForcedColors(GetDocument(), ForcedColors::kActive);
    UpdateAllLifecyclePhases();
    Color system_background_color = LayoutTheme::GetTheme().SystemColor(CSSValueID::kCanvas, mojom::blink::ColorScheme::kLight);
...
TEST_F(StyleEngineTest, MoveSlottedOutsideFlatTree)
{
    ...
    ShadowRoot& shadow_root = host1->AttachShadowRootInternal(ShadowRootType::kOpen);
...
TEST_F(StyleEngineTest, CSSMatchMediaUnknownUseCounter)
{
    ...
    {
        MediaQueryList* mql = GetDocument().domWindow()->matchMedia("(min-width: 0px)");
        ASSERT_TRUE(mql);
        mql->media();
    ...
    {
        MediaQueryList* mql = GetDocument().domWindow()->matchMedia("(width: 100px) or (unknown)");
        ASSERT_TRUE(mql);
        mql->media();
    ...
    {
        MediaQueryList* mql = GetDocument().domWindow()->matchMedia("(unknown: 0px)");
        ASSERT_TRUE(mql);
        mql->media();
    ...
    {
        MediaQueryList* mql = GetDocument().domWindow()->matchMedia("not print and (width: 100px) and (unknown)");
        ASSERT_TRUE(mql);
        mql->media();
        EXPECT_TRUE(IsUseCounted(WebFeature::kCSSMatchMediaUnknown));
        ClearUseCounter(WebFeature::kCSSMatchMediaUnknown);
...
TEST_F(StyleEngineTest, CSSMediaListUnknownUseCounter)
{
...

    {
      ...
      <style media="(min-width: 0px)"></style>
        ...
        auto* style = DynamicTo<HTMLStyleElement>(GetDocument().QuerySelector("style"));
        ASSERT_TRUE(style);
...
TEST_F(StyleEngineSimTest, OwnerColorScheme)
{
    ...
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById("frame"));
...
TEST_F(StyleEngineSimTest, OwnerColorSchemeBaseBackground)
{
    ...
    auto* dark_document = To<HTMLIFrameElement>(GetDocument().getElementById("dark-frame"))->contentDocument();
    auto* light_document = To<HTMLIFrameElement>(GetDocument().getElementById("light-frame"))->contentDocument();
    ...
    GetDocument().documentElement()->setAttribute(blink::html_names::kClassAttr, "dark");
...
TEST_F(StyleEngineContainerQueryTest, UpdateStyleAndLayoutTreeForContainer)
{
    ...
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container1, LogicalSize(200, 100), kLogicalAxisBoth);

    // The first span.affected child and #container2
    EXPECT_EQ(2u, GetStyleEngine().StyleForElementCount() - start_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container2, LogicalSize(200, 100), kLogicalAxisBoth);
...
TEST_F(StyleEngineContainerQueryTest, ContainerQueriesContainmentNotApplying)
{
    ...
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(200, 100), kLogicalAxisBoth);
...
TEST_F(StyleEngineContainerQueryTest, MarkStyleDirtyFromContainerRecalc)
{
    ...
    scoped_refptr<const ComputedStyle> old_inner_style = inner_editor->GetComputedStyle();
...
{
    Vector<String> units = { "vi", "vb", "svi", "svb", "svw", "svh", "svmin", "svmax", "lvi", "lvb", "lvw", "lvh", "lvmin", "lvmax", "dvi", "dvb", "dvw", "dvh",
        "dvmin", "dvmax" };
...
{
    Vector<String> declarations = { "container-type:inline-size", "container-name:foo", "container:inline-size" };
...
TEST_F(StyleEngineTest, AudioUAStyleNameSpace)
{
    ...
    Element* audio = GetDocument().createElementNS("http://dummyns", "audio", ASSERT_NO_EXCEPTION);
...
TEST_F(StyleEngineTest, CascadeLayersInOriginsAndTreeScopes)
{
    ...
    const CascadeLayer& document_outer_layer = To<HTMLStyleElement>(GetDocument().QuerySelector("style"))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    ...
    const CascadeLayer& shadow_outer_layer = To<HTMLStyleElement>(shadow->QuerySelector("style"))->sheet()->Contents()->GetRuleSet().CascadeLayers();
...
TEST_F(StyleEngineTest, CascadeLayersFromMultipleSheets)
{
    ...
    const CascadeLayer& sheet1_outer_layer = To<HTMLStyleElement>(GetDocument().getElementById("sheet1"))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    ...
    const CascadeLayer& sheet2_outer_layer = To<HTMLStyleElement>(GetDocument().getElementById("sheet2"))->sheet()->Contents()->GetRuleSet().CascadeLayers();
...
TEST_F(StyleEngineSimTest, UserFontFaceOverrideWithCascadeLayers)
{
    ...
    ahem_resource.Complete(test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2"))->CopyAs<Vector<char>>());
...
TEST_F(StyleEngineTest, ChangeRenderingForHTMLSelect_DetachParent)
{
    ...
    GetStyleEngine().ChangeRenderingForHTMLSelect(To<HTMLSelectElement>(*GetDocument().getElementById("select")));
    EXPECT_FALSE(GetParentForDetachedSubtree());
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantClassChange)
{
    ...
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div4")->setAttribute(html_names::kClassAttr, "c");
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div4")->setAttribute(html_names::kClassAttr, "b");
    UpdateAllLifecyclePhases();
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantAttributeChange)
{
    ...
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div4")->setAttribute(QualifiedName("", "c", ""), "C");
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div4")->setAttribute(QualifiedName("", "b", ""), "B");
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantInsertionRemoval)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div3")->RemoveChild(GetDocument().getElementById("div5"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div4")->RemoveChild(GetDocument().getElementById("div6"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationUniversalInArgument)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div2")->RemoveChild(GetDocument().getElementById("div3"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationInsertionRemovalWithPseudoInHas)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div2")->RemoveChild(GetDocument().getElementById("div4"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationLinkInHas)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("div1")->RemoveChild(GetDocument().getElementById("anchor1"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationCheckFiltering)
{
    ...
    //    - .a:has(.b)
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("child")->setAttribute(html_names::kClassAttr, "d");
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ...
    //    - .e:has(.f.h) .j
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById("child")->setAttribute(html_names::kClassAttr, "b");
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    EXPECT_EQ(4U, element_count);
}
...
{
    return String("Using container queries or units with printing, or in combination with "
...
TEST_F(StyleEngineTest, UsesCachedTokenizer)
{
    ...
    GetDocument().GetScriptableDocumentParser()->AddCSSTokenizer(".foo{}", CSSTokenizer::CreateCachedTokenizer(".foo{} .bar{}"));
    ...
    // (.foo and .bar).
    auto& style_element = To<HTMLStyleElement>(*GetDocument().getElementById("style"));
