...
static wtf_size_t ReplaceRuleIfExistsInternal(const StyleRuleBase* old_rule, StyleRuleBase* new_rule, HeapVector<Member<StyleRuleBase>>& child_rules)
{
    for (wtf_size_t i = 0; i < child_rules.size(); ++i) {
        ...
        if (IsA<StyleRuleGroup>(rule)) {
            if (ReplaceRuleIfExistsInternal(old_rule, new_rule, To<StyleRuleGroup>(rule)->ChildRules()) != std::numeric_limits<wtf_size_t>::max()) {
...
wtf_size_t StyleSheetContents::ReplaceRuleIfExists(const StyleRuleBase* old_rule, StyleRuleBase* new_rule, wtf_size_t position_hint)
{
...
void StyleSheetContents::ParseAuthorStyleSheet(const CSSStyleSheetResource* cached_style_sheet)
{
    ...
    CSSParser::ParseSheet(context, this, sheet_text, CSSDeferPropertyParsing::kYes, true, sheet_text.IsNull() ? nullptr : cached_style_sheet->TakeTokenizer());
}

ParseSheetResult StyleSheetContents::ParseString(const String& sheet_text, bool allow_import_rules, std::unique_ptr<CachedCSSTokenizer> tokenizer)
{
    const auto* context = MakeGarbageCollected<CSSParserContext>(ParserContext(), this);
    return CSSParser::ParseSheet(context, this, sheet_text, CSSDeferPropertyParsing::kNo, allow_import_rules, std::move(tokenizer));
}
...
RuleSet& StyleSheetContents::EnsureRuleSet(const MediaQueryEvaluator& medium, AddRuleFlags add_rule_flags)
{
...
static void FindFontFaceRulesFromRules(const HeapVector<Member<StyleRuleBase>>& rules, HeapVector<Member<const StyleRuleFontFace>>& font_face_rules)
{
    for (unsigned i = 0; i < rules.size(); ++i) {
...
void StyleSheetContents::FindFontFaceRules(HeapVector<Member<const StyleRuleFontFace>>& font_face_rules)
{
    for (unsigned i = 0; i < import_rules_.size(); ++i) {
