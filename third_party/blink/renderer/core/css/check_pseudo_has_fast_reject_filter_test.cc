...
protected:
    ...
    template <unsigned length> void AddElementIdentifierHashes(CheckPseudoHasFastRejectFilter& filter, const ElementInfo (&element_info_list)[length])
    {
        for (unsigned i = 0; i < length; i++) {
            NonThrowableExceptionState no_exceptions;
            Element* element = GetDocument().CreateElementForBinding(element_info_list[i].tag_name, nullptr, no_exceptions);
            element->setAttribute(html_names::kIdAttr, element_info_list[i].id);
            element->setAttribute(html_names::kClassAttr, element_info_list[i].class_names);
            element->setAttribute(element_info_list[i].attribute_name, element_info_list[i].attribute_value);
...
TEST_F(CheckPseudoHasFastRejectFilterTest, CheckFastReject)
{
    ...
    AddElementIdentifierHashes(filter,
