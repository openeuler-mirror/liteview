...
bool EvaluateAndAddContainerQueries(const Element& matching_element, const ContainerQuery& container_query, const StyleRecalcContext& style_recalc_context,
    ContainerSelectorCache& container_selector_cache, MatchResult& result)
{
    for (const ContainerQuery* current = &container_query; current; current = current->Parent()) {
        if (!ContainerQueryEvaluator::EvalAndAdd(matching_element, style_recalc_context, *current, container_selector_cache, result)) {
            return false;
        }
    }
...
public:
    ...
    const T* Seek(unsigned rule_position)
    {
        ...
        while (iter_ != intervals_.end() && iter_->start_position <= rule_position) {
            ++iter_;
        }
...
public:
    CascadeLayerSeeker(const ContainerNode* scope, Element* vtt_originating_element, const CSSStyleSheet* style_sheet, const RuleSet* rule_set)
...
private:
    static const CascadeLayerMap* FindLayerMap(const ContainerNode* scope, Element* vtt_originating_element, const CSSStyleSheet* style_sheet)
    {
        // VTT embedded style is not in any layer.
        if (vtt_originating_element)
            return nullptr;
        if (scope) {
            return scope->ContainingTreeScope().GetScopedStyleResolver()->GetCascadeLayerMap();
        }
...
using SelectorStatisticsRuleMap = HashMap<const RuleData*, CumulativeRulePerfData>;
...
void AggregateRulePerfData(const HeapVector<RulePerfDataPerRequest>& rules_statistics)
{
...
ElementRuleCollector::ElementRuleCollector(const ElementResolveContext& context, const StyleRecalcContext& style_recalc_context, const SelectorFilter& filter,
    MatchResult& result, ComputedStyle* style, EInsideLink inside_link)
    : context_(context)
    , style_recalc_context_(style_recalc_context)
    , selector_filter_(filter)
    , style_(style)
    , mode_(SelectorChecker::kResolvingStyle)
    , can_use_fast_reject_(selector_filter_.ParentStackIsConsistent(context.ParentNode()))
    ...
    , result_(result)
{
    if (!g_selector_stats_tracing_enabled) {
        g_selector_stats_tracing_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("blink.debug"));
    }
}
...
void ElementRuleCollector::AddElementStyleProperties(const CSSPropertyValueSet* property_set, bool is_cacheable, bool is_inline_style)
{
    if (!property_set)
        return;
    auto link_match_type = static_cast<unsigned>(CSSSelector::kMatchAll);
    result_.AddMatchedProperties(property_set,
        AddMatchedPropertiesOptions::Builder().SetLinkMatchType(AdjustLinkMatchType(inside_link_, link_match_type)).SetIsInlineStyle(is_inline_style).Build());
    if (!is_cacheable)
...
static bool RulesApplicableInCurrentTreeScope(const Element* element, const ContainerNode* scoping_node)
{
    ...
    return !scoping_node || element->ContainingTreeScope() == scoping_node->ContainingTreeScope();
}

template <bool perf_trace_enabled>
void ElementRuleCollector::CollectMatchingRulesForListInternal(base::span<const RuleData> rules, const MatchRequest& match_request, const RuleSet* rule_set,
    const CSSStyleSheet* style_sheet, int style_sheet_index, const SelectorChecker& checker, PartRequest* part_request)
{
    ...
    CascadeLayerSeeker layer_seeker(context.scope, context.vtt_originating_element, style_sheet, rule_set);
    ...
    for (const RuleData& rule_data : rules) {
        ...
        if (can_use_fast_reject_ && selector_filter_.FastRejectSelector<RuleData::kMaximumIdentifierCount>(rule_data.DescendantSelectorIdentifierHashes())) {
            fast_rejected++;
            if (perf_trace_enabled)
        ...
        context.is_inside_visited_link = rule_data.LinkMatchType() == CSSSelector::kMatchVisited;
        DCHECK(!context.is_inside_visited_link || inside_link_ != EInsideLink::kNotInsideLink);
        ...
        if (container_query) {
            ...
            if (pseudo_style_request_.pseudo_id != kPseudoIdNone || result.dynamic_pseudo == kPseudoIdNone) {
                if (!EvaluateAndAddContainerQueries(context_.GetElement(), *container_query, style_recalc_context_, container_selector_cache_, result_)) {
        ...
        DidMatchRule(&rule_data, layer_order, container_query, result.proximity, result, style_sheet, style_sheet_index);
    }
...
void ElementRuleCollector::CollectMatchingRulesForList(base::span<const RuleData> rules, const MatchRequest& match_request, const RuleSet* rule_set,
    const CSSStyleSheet* style_sheet, int style_sheet_index, const SelectorChecker& checker, PartRequest* part_request)
{
    ...
    if (!*g_selector_stats_tracing_enabled) {
        CollectMatchingRulesForListInternal<false>(rules, match_request, rule_set, style_sheet, style_sheet_index, checker, part_request);
    } else {
        CollectMatchingRulesForListInternal<true>(rules, match_request, rule_set, style_sheet, style_sheet_index, checker, part_request);
    }
}
...
void ElementRuleCollector::CollectMatchingRules(const MatchRequest& match_request)
{
    ...
    SelectorChecker checker(style_.get(), nullptr, pseudo_style_request_, mode_, matching_ua_rules_);
    ...
    if (!pseudo_id.empty()) {
        ...
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->UAShadowPseudoElementRules(pseudo_id), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    ...
    if (element.IsVTTElement()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->CuePseudoRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    ...
    if (element.HasID()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(bundle.rule_set->IdRules(element.IdForStyleResolution()), match_request, bundle.rule_set, bundle.style_sheet,
    ...
    if (element.IsStyledElement() && element.HasClass()) {
        for (wtf_size_t i = 0; i < element.ClassNames().size(); ++i) {
            for (const auto bundle : match_request.AllRuleSets()) {
                CollectMatchingRulesForList(bundle.rule_set->ClassRules(element.ClassNames()[i]), match_request, bundle.rule_set, bundle.style_sheet,
                    bundle.style_sheet_index, checker);
            }
        }
    }
            ...
            for (const auto bundle : match_request.AllRuleSets()) {
                if (bundle.rule_set->HasAnyAttrRules()) {
                    ...
                    if (!list.empty() && !bundle.rule_set->CanIgnoreEntireList(list, lower_name, attributes[attr_idx].Value())) {
                        CollectMatchingRulesForList(
                            bundle.rule_set->AttrRules(lower_name), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
                    }
                }
            }
    ...
    if (element.IsLink()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->LinkPseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    if (inside_link_ != EInsideLink::kNotInsideLink) {
        ...
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->VisitedDependentRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    if (SelectorChecker::MatchesFocusPseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->FocusPseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    if (SelectorChecker::MatchesSelectorFragmentAnchorPseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->SelectorFragmentAnchorRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    if (SelectorChecker::MatchesFocusVisiblePseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(
                bundle.rule_set->FocusVisiblePseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
        }
    }
    if (SelectorChecker::MatchesSpatialNavigationInterestPseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            CollectMatchingRulesForList(bundle.rule_set->SpatialNavigationInterestPseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet,
                bundle.style_sheet_index, checker);
        }
    }
    ...
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList(
            bundle.rule_set->TagRules(element_name), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
    }
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList(bundle.rule_set->UniversalRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
    }
}
...
void ElementRuleCollector::CollectMatchingShadowHostRules(const MatchRequest& match_request)
{
    ...
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList(bundle.rule_set->ShadowHostRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
    }
}
...
void ElementRuleCollector::CollectMatchingSlottedRules(const MatchRequest& match_request)
{
    ...
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList(
            bundle.rule_set->SlottedPseudoElementRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker);
    }
}
...
void ElementRuleCollector::CollectMatchingPartPseudoRules(const MatchRequest& match_request, PartNames& part_names, bool for_shadow_pseudo)
{
    ...
    SelectorChecker checker(style_.get(), &part_names, pseudo_style_request_, mode_, matching_ua_rules_);

    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList(
            bundle.rule_set->PartPseudoRules(), match_request, bundle.rule_set, bundle.style_sheet, bundle.style_sheet_index, checker, &request);
    }
}

template <class CSSRuleCollection> CSSRule* ElementRuleCollector::FindStyleRule(CSSRuleCollection* css_rules, StyleRule* style_rule)
{
    if (!css_rules)
        return nullptr;
...
void ElementRuleCollector::AppendCSSOMWrapperForRule(CSSStyleSheet* parent_style_sheet, const RuleData* rule_data, wtf_size_t position)
{
...
void ElementRuleCollector::SortAndTransferMatchedRules(bool is_vtt_embedded_style)
{
    ...
    if (mode_ == SelectorChecker::kCollectingCSSRules) {
        for (unsigned i = 0; i < matched_rules_.size(); ++i) {
            AppendCSSOMWrapperForRule(const_cast<CSSStyleSheet*>(matched_rules_[i].ParentStyleSheet()), matched_rules_[i].GetRuleData(), i);
        }
        return;
    }
    ...
    for (unsigned i = 0; i < matched_rules_.size(); i++) {
        ...
        result_.AddMatchedProperties(&rule_data->Rule()->Properties(),
            AddMatchedPropertiesOptions::Builder()
                .SetLinkMatchType(AdjustLinkMatchType(inside_link_, rule_data->LinkMatchType()))
                .SetValidPropertyFilter(rule_data->GetValidPropertyFilter(matching_ua_rules_))
...
void ElementRuleCollector::DidMatchRule(const RuleData* rule_data, unsigned layer_order, const ContainerQuery* container_query, unsigned proximity,
    const SelectorChecker::MatchResult& result, const CSSStyleSheet* style_sheet, int style_sheet_index)
{
    ...
    if (dynamic_pseudo != kPseudoIdNone && pseudo_style_request_.pseudo_id == kPseudoIdNone) {
        ...
        if (!style_ || dynamic_pseudo > kLastTrackedPublicPseudoId)
            return;
        if ((dynamic_pseudo == kPseudoIdBefore || dynamic_pseudo == kPseudoIdAfter) && !rule_data->Rule()->Properties().HasProperty(CSSPropertyID::kContent))
            return;
        ...
        if (IsHighlightPseudoElement(dynamic_pseudo)) {
            ...
            if (CSSSelector::GetPseudoId(selector.GetPseudoType()) == dynamic_pseudo) {
                ...
                universal = next->IsLastInTagHistory() && CSSSelector::GetPseudoId(next->GetPseudoType()) == dynamic_pseudo
            ...
            if (dynamic_pseudo == kPseudoIdHighlight) {
                ...
                style_->SetHasCustomHighlightName(AtomicString(result.custom_highlight_name));
            }
        } else if (dynamic_pseudo == kPseudoIdFirstLine && container_query) {
            style_->SetFirstLineDependsOnSizeContainerQueries(true);
        }
    } else {
        matched_rules_.push_back(MatchedRule(rule_data, layer_order, proximity, style_sheet_index, style_sheet));
    }
}
...
inline bool ElementRuleCollector::CompareRules(const MatchedRule& matched_rule1, const MatchedRule& matched_rule2)
{
...
void ElementRuleCollector::AddMatchedRulesToTracker(StyleRuleUsageTracker* tracker) const
{
    for (auto matched_rule : matched_rules_) {
        tracker->Track(matched_rule.ParentStyleSheet(), matched_rule.GetRuleData()->Rule());
    }
}
