...
scoped_refptr<SimpleFontData> LocalFontFaceSource::CreateLoadingFallbackFontData(const FontDescription& font_description)
{
    ...
    scoped_refptr<CSSCustomFontData> css_font_data = CSSCustomFontData::Create(this, CSSCustomFontData::kVisibleFallback);
    return SimpleFontData::Create(temporary_font->PlatformData(), css_font_data);
}

scoped_refptr<SimpleFontData> LocalFontFaceSource::CreateFontData(const FontDescription& font_description, const FontSelectionCapabilities&)
{
    if (!IsValid()) {
    ...
    if (IsValid() && IsLoading()) {
        scoped_refptr<SimpleFontData> fallback_font_data = CreateLoadingFallbackFontData(font_description);
        ReportFontLookup(font_description, fallback_font_data.get(), true /* is_loading_fallback */);
    ...
    scoped_refptr<SimpleFontData> font_data = FontCache::Get().GetFontData(unstyled_description, font_name_, AlternateFontName::kLocalUniqueFace);
...
void LocalFontFaceSource::ReportFontLookup(const FontDescription& font_description, SimpleFontData* font_data, bool is_loading_fallback)
{
