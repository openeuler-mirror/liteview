...
public:
    ...
    {
        return has_rare_data_ ? data_.rare_data_->bits_.has_.contains_pseudo_ : false;
    }
    bool ContainsComplexLogicalCombinationsInsideHasPseudoClass() const
    {
        return has_rare_data_ ? data_.rare_data_->bits_.has_.contains_complex_logical_combinations_ : false;
    }
    ...
    void SetToggle(const AtomicString& name, std::unique_ptr<ToggleRoot::State>&& value);
    ...
    {
        return static_cast<RelationType>(relation_);
    }
    ...
    {
        return static_cast<MatchType>(match_);
    }
    ...
    {
        return is_last_in_selector_list_;
    }
    ...
    {
        return is_last_in_tag_history_;
    }
    void SetLastInTagHistory(bool is_last)
    {
        is_last_in_tag_history_ = is_last;
    }
...
private:
    ...
    struct RareData : public RefCounted<RareData> {
        ...
        union {
            ...
            struct {
                AttributeMatchType attribute_match_; // used for attribute selector (with value)
        ...
        std::unique_ptr<CSSSelectorList> selector_list_; // Used for :-webkit-any and :not
        std::unique_ptr<Vector<AtomicString>> part_names_; // Used for ::part() selectors.
    ...
    , data_(DataUnion::kConstructUninitialized)
{
    if (o.match_ == kTag) {
        ...
        new (&data_.rare_data_) scoped_refptr<RareData>(std::move(o.data_.rare_data_));
    } else {
...
{
    return pseudo_type_ == kPseudoHover || pseudo_type_ == kPseudoActive || pseudo_type_ == kPseudoFocus || pseudo_type_ == kPseudoDrag
        || pseudo_type_ == kPseudoFocusWithin || pseudo_type_ == kPseudoFocusVisible;
}
