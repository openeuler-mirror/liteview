...
ActiveSheetsChange CompareActiveStyleSheets(
    const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets, HeapHashSet<Member<RuleSet>>& changed_rule_sets)
{
    ...
    merged_sorted.AppendRange(old_style_sheets.begin() + index, old_style_sheets.end());
    merged_sorted.AppendRange(new_style_sheets.begin() + index, new_style_sheets.end());
    ...
    while (merged_iterator != merged_sorted.end()) {
        ...
        if (merged_iterator == merged_sorted.end() || (*merged_iterator).first != sheet1.first) {
