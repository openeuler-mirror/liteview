...
CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSStyleSheet* parent_sheet) const
{
    return CreateCSSOMWrapper(position_hint, parent_sheet, nullptr);
}

CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSRule* parent_rule) const
{
...
CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSStyleSheet* parent_sheet, CSSRule* parent_rule) const
{
    ...
    case kCounterStyle:
        ...
        break;
    case kPositionFallback:
        rule = MakeGarbageCollected<CSSPositionFallbackRule>(To<StyleRulePositionFallback>(self), parent_sheet);
        break;
    case kTry:
    case kKeyframe:
    case kCharset:
...
StyleRule::StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, CSSPropertyValueSet* properties)
    : StyleRuleBase(kStyle)
    , properties_(properties)
{
    CSSSelectorList::AdoptSelectorVector(selector_vector, SelectorArray(), flattened_size);
}

StyleRule::StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, CSSLazyPropertyParser* lazy_property_parser)
    : StyleRuleBase(kStyle)
...
StyleRule::StyleRule(base::PassKey<StyleRule>, CSSSelectorVector& selector_vector, size_t flattened_size, StyleRule&& other)
    : StyleRuleBase(kStyle)
...
StyleRulePage::StyleRulePage(CSSSelectorList selector_list, CSSPropertyValueSet* properties)
    : StyleRuleBase(kPage)
    , properties_(properties)
...
StyleRuleScope::StyleRuleScope(const StyleScope& style_scope, HeapVector<Member<StyleRuleBase>>& adopt_rules)
    : StyleRuleGroup(kScope, adopt_rules)
...
void StyleRuleScope::SetPreludeText(const ExecutionContext* execution_context, String value)
{
...
StyleRuleGroup::StyleRuleGroup(RuleType type, HeapVector<Member<StyleRuleBase>>& adopt_rule)
    : StyleRuleBase(type)
{
...
StyleRuleLayerBlock::StyleRuleLayerBlock(LayerName&& name, HeapVector<Member<StyleRuleBase>>& adopt_rules)
    : StyleRuleGroup(kLayerBlock, adopt_rules)
    , name_(std::move(name))
{
}
...
StyleRuleCondition::StyleRuleCondition(RuleType type, HeapVector<Member<StyleRuleBase>>& adopt_rules)
    : StyleRuleGroup(type, adopt_rules)
{
}

StyleRuleCondition::StyleRuleCondition(RuleType type, const String& condition_text, HeapVector<Member<StyleRuleBase>>& adopt_rules)
    : StyleRuleGroup(type, adopt_rules)
...
StyleRuleMedia::StyleRuleMedia(const MediaQuerySet* media, HeapVector<Member<StyleRuleBase>>& adopt_rules)
...
StyleRuleSupports::StyleRuleSupports(const String& condition_text, bool condition_is_supported, HeapVector<Member<StyleRuleBase>>& adopt_rules)
...
StyleRuleContainer::StyleRuleContainer(ContainerQuery& container_query, HeapVector<Member<StyleRuleBase>>& adopt_rules)
