...
const static NameToPseudoStruct kPseudoTypeWithoutArgumentsMap[] = {
    ...
    { "-internal-spatial-navigation-interest", CSSSelector::kPseudoSpatialNavigationInterest },
...
const static NameToPseudoStruct kPseudoTypeWithArgumentsMap[] = {
    ...
    { "page-transition-image-wrapper", CSSSelector::kPseudoPageTransitionImageWrapper },
    { "page-transition-incoming-image", CSSSelector::kPseudoPageTransitionIncomingImage },
    { "page-transition-outgoing-image", CSSSelector::kPseudoPageTransitionOutgoingImage },
...
CSSSelector::PseudoType CSSSelector::NameToPseudoType(const AtomicString& name, bool has_arguments, const Document* document)
{
    ...
    if (match->type == CSSSelector::kPseudoAutofill && !RuntimeEnabledFeatures::CSSPseudoAutofillEnabled())
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoDir && !RuntimeEnabledFeatures::CSSPseudoDirEnabled())
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoFocusVisible && !RuntimeEnabledFeatures::CSSFocusVisibleEnabled())
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoPaused && !RuntimeEnabledFeatures::CSSPseudoPlayingPausedEnabled())
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoPictureInPicture && !RuntimeEnabledFeatures::CSSPictureInPictureEnabled())
        return CSSSelector::kPseudoUnknown;
    ...
    // stylesheets.
    bool popup_attribute_enabled = !document || RuntimeEnabledFeatures::HTMLPopupAttributeEnabled(document->GetExecutionContext());
    if (match->type == CSSSelector::kPseudoOpen && !popup_attribute_enabled)
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoPopupOpeningOrOpen && !popup_attribute_enabled)
        return CSSSelector::kPseudoUnknown;

    if (match->type == CSSSelector::kPseudoHighlight && !RuntimeEnabledFeatures::HighlightAPIEnabled()) {
        return CSSSelector::kPseudoUnknown;
    }

    if ((match->type == CSSSelector::kPseudoSpellingError || match->type == CSSSelector::kPseudoGrammarError)
        && !RuntimeEnabledFeatures::CSSSpellingGrammarErrorsEnabled()) {
        return CSSSelector::kPseudoUnknown;
    }

    if (match->type == CSSSelector::kPseudoHas && !RuntimeEnabledFeatures::CSSPseudoHasEnabled()) {
        return CSSSelector::kPseudoUnknown;
    }

    if (match->type == CSSSelector::kPseudoToggle && !RuntimeEnabledFeatures::CSSTogglesEnabled()) {
        return CSSSelector::kPseudoUnknown;
    }
...
const CSSSelector* CSSSelector::SerializeCompound(StringBuilder& builder) const
{
    ...
    for (const CSSSelector* simple_selector = this; simple_selector; simple_selector = simple_selector->TagHistory()) {
        if (simple_selector->match_ == kId) {
            ...
            builder.Append('.');
            SerializeIdentifier(simple_selector->SerializingValue(), builder);
        } else if (simple_selector->match_ == kPseudoClass || simple_selector->match_ == kPagePseudoClass) {
            ...
            builder.Append('[');
            SerializeNamespacePrefixIfNeeded(simple_selector->Attribute().Prefix(), g_star_atom, builder, simple_selector->IsAttributeSelector());
            ...
            if (simple_selector->match_ != kAttributeSet) {
                ...
                if (simple_selector->AttributeMatch() == AttributeMatchType::kCaseInsensitive) {
                    builder.Append(" i");
                } else if (simple_selector->AttributeMatch() == AttributeMatchType::kCaseSensitiveAlways) {
        ...
        if (simple_selector->SelectorList()) {
            builder.Append('(');
            const CSSSelector* first_sub_selector = simple_selector->SelectorList()->First();
...
{
    String result;
    for (const CSSSelector* compound = this; compound; compound = compound->TagHistory()) {
...
void CSSSelector::SetAttribute(const QualifiedName& value, AttributeMatchType match_type)
{
    CreateRareData();
    data_.rare_data_->attribute_ = value;
    data_.rare_data_->bits_.attr_.attribute_match_ = match_type;
    data_.rare_data_->bits_.attr_.is_case_sensitive_attribute_ = HTMLDocument::IsCaseSensitiveAttribute(value);
}
...
void CSSSelector::SetSelectorList(std::unique_ptr<CSSSelectorList> selector_list)
{
    CreateRareData();
    data_.rare_data_->selector_list_ = std::move(selector_list);
}

void CSSSelector::SetToggle(const AtomicString& name, std::unique_ptr<ToggleRoot::State>&& value)
{
    CreateRareData();
...
bool CSSSelector::IsTreeAbidingPseudoElement() const
{
    return Match() == CSSSelector::kPseudoElement
        && (GetPseudoType() == kPseudoBefore || GetPseudoType() == kPseudoAfter || GetPseudoType() == kPseudoMarker || GetPseudoType() == kPseudoPlaceholder
...
template <typename Functor> static bool ForAnyInTagHistory(const Functor& functor, const CSSSelector& selector)
{
...
bool CSSSelector::RareData::MatchNth(unsigned unsigned_count)
{
    ...
    if (UNLIKELY(unsigned_count > static_cast<unsigned>(max_value) || NthAValue() > max_value || NthAValue() < min_value || NthBValue() > max_value
            || NthBValue() < min_value))
        return false;
...
void CSSSelector::SetPartNames(std::unique_ptr<Vector<AtomicString>> part_names)
{
    CreateRareData();
