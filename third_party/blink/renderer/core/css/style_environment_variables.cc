...
const AtomicString StyleEnvironmentVariables::GetVariableName(UADefinedVariable variable, const FeatureContext* feature_context)
{
    ...
    case UADefinedVariable::kTitlebarAreaX:
        DCHECK(RuntimeEnabledFeatures::WebAppWindowControlsOverlayEnabled(feature_context));
...
// static
scoped_refptr<StyleEnvironmentVariables> StyleEnvironmentVariables::Create(StyleEnvironmentVariables& parent)
{
...
void StyleEnvironmentVariables::SetVariable(const AtomicString& name, const String& value)
{
    ...
    scoped_refptr<CSSVariableData> variable_data = CSSVariableData::Create(
        CSSTokenizedValue { CSSParserTokenRange { tokens }, StringView { value } }, false /* is_animation_tainted */, false /* needs_variable_resolution */);
...
void StyleEnvironmentVariables::SetVariable(UADefinedTwoDimensionalVariable variable, unsigned first_dimension, unsigned second_dimension, const String& value)
{
    SetVariable(GetVariableName(variable, GetFeatureContext()), first_dimension, second_dimension, value);
}
...
void StyleEnvironmentVariables::RemoveVariable(UADefinedTwoDimensionalVariable variable)
{
    const AtomicString name = GetVariableName(variable, GetFeatureContext());
    RemoveVariable(name);
}
...
void StyleEnvironmentVariables::BindToParent(StyleEnvironmentVariables& parent)
{
...
void StyleEnvironmentVariables::ParentInvalidatedVariable(const AtomicString& name)
{
    ...
    // locally.
    if (data_.find(name) == data_.end() && two_dimension_data_.find(name) == two_dimension_data_.end()) {
        InvalidateVariable(name);
    }
}
