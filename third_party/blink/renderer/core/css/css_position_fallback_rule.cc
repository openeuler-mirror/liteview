...
StyleRulePositionFallback::StyleRulePositionFallback(const AtomicString& name)
    : StyleRuleBase(kPositionFallback)
    , name_(name)
{
}

StyleRulePositionFallback::StyleRulePositionFallback(const StyleRulePositionFallback&) = default;
...
CSSPositionFallbackRule::CSSPositionFallbackRule(StyleRulePositionFallback* position_fallback_rule, CSSStyleSheet* parent)
    : CSSRule(parent)
    , position_fallback_rule_(position_fallback_rule)
    , rule_list_cssom_wrapper_(MakeGarbageCollected<LiveCSSRuleList<CSSPositionFallbackRule>>(this))
{
...
void CSSPositionFallbackRule::CreateChildRuleWrappers()
{
    ...
    child_rule_cssom_wrappers_.reserve(position_fallback_rule_->TryRules().size());
    for (StyleRuleTry* try_rule : position_fallback_rule_->TryRules()) {
        child_rule_cssom_wrappers_.push_back(MakeGarbageCollected<CSSTryRule>(try_rule, this));
    }
}
