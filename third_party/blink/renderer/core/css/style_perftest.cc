...
static std::unique_ptr<DummyPageHolder> LoadDumpedPage(const base::Value::Dict& dict, perf_test::PerfResultReporter& reporter)
{
    ...
    document.body()->setInnerHTML(WTF::String(*dict.FindString("html")), ASSERT_NO_EXCEPTION);
    ...
    const bool pre_tokenize = base::CommandLine::ForCurrentProcess()->HasSwitch("pre-tokenize");
    ...
    for (const base::Value& sheet_json : *dict.FindList("stylesheets")) {
        ...
        if (pre_tokenize) {
            tokenizers.push_back(CSSTokenizer::CreateCachedTokenizer(WTF::String(*sheet_dict.FindString("text"))));
    ...
    for (const base::Value& sheet_json : *dict.FindList("stylesheets")) {
        ...
        for (int i = 0; i < parse_iterations; ++i) {
            sheet->ParseString(WTF::String(*sheet_dict.FindString("text")),
                /*allow_import_rules=*/true, std::move(tokenizers[tokenizer_idx++]));
        }
...
static void MeasureStyleForDumpedPage(const char* filename, const char* label)
{
    ...
    {
        scoped_refptr<SharedBuffer> serialized = test::ReadFromFile(test::StylePerfTestDataPath(filename));
        absl::optional<base::Value> json = base::JSONReader::Read(base::StringPiece(serialized->Data(), serialized->size()));
        if (!json.has_value()) {
            char msg[256];
    ...
    reporter.AddResult("GCAllocated", (gc_allocated_bytes - orig_gc_allocated_bytes) / 1024);
    reporter.RegisterImportantMetric("PartitionAllocated", "kB");
    reporter.AddResult("PartitionAllocated", (partition_allocated_bytes - orig_partition_allocated_bytes) / 1024);
}
