...
private:
    ...
    MatchedRule(const RuleData* rule_data, unsigned layer_order, unsigned proximity, unsigned style_sheet_index, const CSSStyleSheet* parent_style_sheet)
        ...
        , parent_style_sheet_(parent_style_sheet)
    {
        ...
        position_ = (static_cast<uint64_t>(style_sheet_index) << kBitsForPositionInRuleData) + rule_data_->GetPosition();
    }
    ...
    {
        return GetRuleData()->Specificity();
    }
    unsigned LayerOrder() const
    {
        return layer_order_;
    }
    ...
    const CSSStyleSheet* ParentStyleSheet() const
    {
...
public:
    ElementRuleCollector(const ElementResolveContext&, const StyleRecalcContext&, const SelectorFilter&, MatchResult&, ComputedStyle*, EInsideLink);
    ...
    void AddElementStyleProperties(const CSSPropertyValueSet*, bool is_cacheable = true, bool is_inline_style = false);
...
private:
    ...
    void CollectMatchingRulesForListInternal(
        base::span<const RuleData>, const MatchRequest&, const RuleSet*, const CSSStyleSheet*, int, const SelectorChecker&, PartRequest* = nullptr);

    void CollectMatchingRulesForList(
        base::span<const RuleData>, const MatchRequest&, const RuleSet*, const CSSStyleSheet*, int, const SelectorChecker&, PartRequest* = nullptr);

    bool Match(SelectorChecker&, const SelectorChecker::SelectorCheckingContext&, MatchResult&);
    void DidMatchRule(const RuleData*, unsigned layer_order, const ContainerQuery*, unsigned proximity, const SelectorChecker::MatchResult&,
        const CSSStyleSheet* style_sheet, int style_sheet_index);

    template <class CSSRuleCollection> CSSRule* FindStyleRule(CSSRuleCollection*, StyleRule*);
...
private:
    static inline bool CompareRules(const MatchedRule& matched_rule1, const MatchedRule& matched_rule2);

    const ElementResolveContext& context_;
    StyleRecalcContext style_recalc_context_;
    const SelectorFilter& selector_filter_;
    scoped_refptr<ComputedStyle> style_; // FIXME: This can be mutated during matching!
