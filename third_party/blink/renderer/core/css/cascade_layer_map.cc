...
using CanonicalLayerMap = HeapHashMap<Member<const CascadeLayer>, Member<const CascadeLayer>>;

using LayerOrderMap = HeapHashMap<Member<const CascadeLayer>, unsigned>;

void AddLayers(CascadeLayer* canonical_layer, const CascadeLayer& layer_from_sheet, CanonicalLayerMap& canonical_layer_map)
{
    DCHECK_EQ(canonical_layer->GetName(), layer_from_sheet.GetName());
    canonical_layer_map.insert(&layer_from_sheet, canonical_layer);
    for (const auto& sub_layer_from_sheet : layer_from_sheet.GetDirectSubLayers()) {
        StyleRuleBase::LayerName sub_layer_name({ sub_layer_from_sheet->GetName() });
        CascadeLayer* canonical_sub_layer = canonical_layer->GetOrAddSubLayer(sub_layer_name);
...
CascadeLayerMap::CascadeLayerMap(const ActiveStyleSheetVector& sheets)
{
    ...
    for (const auto& sheet : sheets) {
        ...
        if (rule_set && rule_set->HasCascadeLayers()) {
            AddLayers(canonical_root_layer, rule_set->CascadeLayers(), canonical_layer_map);
        }
    }
