...
bool NodeLayoutUpgrade::ShouldUpgrade()
{
    ...
    bool maybe_affected_by_layout = style_engine.StyleMaybeAffectedByLayout(node_);
    ...
    for (const Node* ancestor = &node_; ancestor; ancestor = LayoutTreeBuilderTraversal::Parent(*ancestor)) {
