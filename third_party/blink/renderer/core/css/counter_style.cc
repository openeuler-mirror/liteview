...
{
    const CounterStyle* disc = CounterStyleMap::GetUACounterStyleMap()->FindCounterStyleAcrossScopes("disc");
    DCHECK(disc);
    return *disc;
}
...
String CJKIdeoGraphicAlgorithm(unsigned number, const UChar table[21], CJKStyle cjk_style)
{
    ...
    const unsigned kGroupLength = 9; // 4 digits, 3 digit markers, group marker of size 2.
...
{
    static const UChar kSimpleChineseInformalTable[21] = { kChinese, 0x4E07, 0x0000, 0x4EBF, 0x0000, 0x4E07, 0x4EBF, 0x5341, 0x767E, 0x5343, 0x96F6, 0x4E00,
        0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
{
    static const UChar kSimpleChineseFormalTable[21] = { kChinese, 0x4E07, 0x0000, 0x4EBF, 0x0000, 0x4E07, 0x4EBF, 0x62FE, 0x4F70, 0x4EDF, 0x96F6, 0x58F9,
        0x8D30, 0x53C1, 0x8086, 0x4F0D, 0x9646, 0x67D2, 0x634C, 0x7396, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kSimpleChineseFormalTable, kFormal);
}

String TradChineseInformalAlgorithm(unsigned value)
{
    static const UChar kTraditionalChineseInformalTable[21] = { kChinese, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x5341, 0x767E, 0x5343, 0x96F6,
...
{
    static const UChar kTraditionalChineseFormalTable[21] = { kChinese, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x62FE, 0x4F70, 0x4EDF, 0x96F6, 0x58F9,
        0x8CB3, 0x53C3, 0x8086, 0x4F0D, 0x9678, 0x67D2, 0x634C, 0x7396, 0x0000 };
...
{
    static const UChar kKoreanHangulFormalTable[21] = { kKorean, 0xB9CC, 0x0000, 0xC5B5, 0x0000, 0xC870, 0x0000, 0xC2ED, 0xBC31, 0xCC9C, 0xC601, 0xC77C, 0xC774,
        0xC0BC, 0xC0AC, 0xC624, 0xC721, 0xCE60, 0xD314, 0xAD6C, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kKoreanHangulFormalTable, kFormal);
}

String KoreanHanjaInformalAlgorithm(unsigned value)
{
    static const UChar kKoreanHanjaInformalTable[21] = { kKorean, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x5341, 0x767E, 0x5343, 0x96F6, 0x4E00,
        0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
{
    static const UChar kKoreanHanjaFormalTable[21] = { kKorean, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x62FE, 0x767E, 0x4EDF, 0x96F6, 0x58F9, 0x8CB3,
        0x53C3, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
String HebrewAlgorithmUnder1000(unsigned number)
{
    ...
    if (number == 15 || number == 16) {
        ...
        if (unsigned tens = number / 10) {
            static const UChar kHebrewTens[9] = { 1497, 1499, 1500, 1502, 1504, 1505, 1506, 1508, 1510 };
...
String EthiopicNumericAlgorithm(unsigned value)
{
    ...
    static const UChar units[9] = { 0x1369, 0x136A, 0x136B, 0x136C, 0x136D, 0x136E, 0x136F, 0x1370, 0x1371 };
    // Ethiopic characters for 10, 20, ..., 90
    static const UChar tens[9] = { 0x1372, 0x1373, 0x1374, 0x1375, 0x1376, 0x1377, 0x1378, 0x1379, 0x137A };
    if (!value)
        return String();
    if (value < 10u)
...
{
    DEFINE_STATIC_LOCAL(Persistent<CounterStyle>, decimal, (CounterStyleMap::GetUACounterStyleMap()->FindCounterStyleAcrossScopes("decimal")));
    DCHECK(decimal);
    return *decimal;
}
    ...
    , style_rule_version_(rule.GetVersion())
{
    ...
    if (HasSymbols(system_)) {
        if (system_ == CounterStyleSystem::kAdditive) {
            for (const CSSValue* symbol : To<CSSValueList>(*rule.GetAdditiveSymbols())) {
