...
bool NameMatches(const ComputedStyle& style, const ContainerSelector& container_selector)
{
...
bool Matches(const ComputedStyle& style, const ContainerSelector& container_selector)
{
    return NameMatches(style, container_selector) && TypeMatches(style, container_selector);
}

Element* CachedContainer(Element* starting_element, const ContainerSelector& container_selector, ContainerSelectorCache& container_selector_cache)
{
    ContainerSelectorCache::AddResult add_result = container_selector_cache.insert(container_selector, nullptr);

    if (add_result.is_new_entry) {
        add_result.stored_value->value = ContainerQueryEvaluator::FindContainer(starting_element, container_selector);
    }
...
// static
Element* ContainerQueryEvaluator::FindContainer(Element* starting_element, const ContainerSelector& container_selector)
{
    // TODO(crbug.com/1213888): Cache results.
    for (Element* element = starting_element; element; element = element->ParentOrShadowHostElement()) {
        if (const ComputedStyle* style = element->GetComputedStyle()) {
            if (style->StyleType() == kPseudoIdNone && Matches(*style, container_selector)) {
                return element;
            }
        }
    }
...
bool ContainerQueryEvaluator::EvalAndAdd(const Element& matching_element, const StyleRecalcContext& context, const ContainerQuery& query,
    ContainerSelectorCache& container_selector_cache, MatchResult& match_result)
{
    ...
    Element* starting_element = selects_size ? context.container : matching_element.ParentOrShadowHostElement();
    Element* container = CachedContainer(starting_element, query.Selector(), container_selector_cache);
    if (!container)
        return false;
    ...
    if (!evaluator) {
        ...
        evaluator->SetData(container->GetDocument(), *container, PhysicalSize(), kPhysicalAxisNone);
    }
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::SizeContainerChanged(
    Document& document, Element& container, PhysicalSize size, PhysicalAxes contained_axes)
{
...
void ContainerQueryEvaluator::SetData(Document& document, Element& container, PhysicalSize size, PhysicalAxes contained_axes)
{
    size_ = size;
    ...
    if ((supported_axes & PhysicalAxes(kPhysicalAxisHorizontal)) != PhysicalAxes(kPhysicalAxisNone)) {
        width = size.width.ToDouble();
    }

    if ((supported_axes & PhysicalAxes(kPhysicalAxisVertical)) != PhysicalAxes(kPhysicalAxisNone)) {
        height = size.height.ToDouble();
    }

    auto* query_values = MakeGarbageCollected<CSSContainerValues>(document, container, width, height);
...
void ContainerQueryEvaluator::UpdateValuesIfNeeded(Document& document, Element& container, StyleRecalcChange change)
{
    if (!media_query_evaluator_)
        return;
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(document, container, existing_values.Width(), existing_values.Height());
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

void ContainerQueryEvaluator::MarkFontDirtyIfNeeded(const ComputedStyle& old_style, const ComputedStyle& new_style)
{
