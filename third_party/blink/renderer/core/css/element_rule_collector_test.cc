...
public:
    ...
    absl::optional<unsigned> Match(Element* element, const String& selector, const ContainerNode* scope = nullptr)
    {
        ElementResolveContext context(*element);
        SelectorFilter filter;
        MatchResult result;
        auto style = GetDocument().GetStyleResolver().CreateComputedStyle();
        ElementRuleCollector collector(context, StyleRecalcContext(), filter, result, style.get(), InsideLink(element));

        String rule = selector + " { color: green }";
        auto* style_rule = DynamicTo<StyleRule>(css_test_helpers::ParseRule(GetDocument(), rule));
        if (!style_rule)
            return absl::nullopt;
...
TEST_F(ElementRuleCollectorTest, LinkMatchTypeHostContext)
{
    ...
    ShadowRoot& visited_root = visited_host->AttachShadowRootInternal(ShadowRootType::kOpen);
    ShadowRoot& unvisited_root = unvisited_host->AttachShadowRootInternal(ShadowRootType::kOpen);
...
TEST_F(ElementRuleCollectorTest, MatchesNonUniversalHighlights)
{
    String markup = "<html xmlns='http://www.w3.org/1999/xhtml'><body class='foo'>"
                    ...
                    "</body></html>";
    scoped_refptr<SharedBuffer> data = SharedBuffer::Create(markup.Utf8().data(), markup.length());
    ...
    auto run = [&](Element& element, String selector, absl::optional<AtomicString> defaultNamespace) {
        ...
        RuleSet& rules = sheet->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasNoSpecialState);
        auto* rule = To<StyleRule>(CSSParser::ParseRule(sheet->ParserContext(), sheet, selector + " { color: green }"));
        rules.AddStyleRule(rule, kRuleHasNoSpecialState);

        MatchResult result;
        auto style = GetDocument().GetStyleResolver().CreateComputedStyle();
        ElementResolveContext context { element };
        ElementRuleCollector collector(context, StyleRecalcContext(), SelectorFilter(), result, style.get(), EInsideLink::kNotInsideLink);
