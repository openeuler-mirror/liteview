...
TEST_P(CSSPaintValueTest, ReportingCompositedUMA)
{
    ...
    ON_CALL(*mock_generator, Paint(_, _, _)).WillByDefault(Return(PaintGeneratedImage::Create(nullptr, target_size)));
    ...
    if (RuntimeEnabledFeatures::OffMainThreadCSSPaintEnabled()) {
        ...
        histogram_tester.ExpectUniqueSample("Blink.CSSPaintValue.PaintOffThread", true, 1u);
    } else {
...
TEST_P(CSSPaintValueTest, ReportingNonCompositedUMA)
{
    ...
    ON_CALL(*mock_generator, IsImageGeneratorReady()).WillByDefault(Return(true));
    EXPECT_CALL(*mock_generator, Paint(_, _, _)).WillRepeatedly(Return(PaintGeneratedImage::Create(nullptr, target_size)));
    // The paint worklet is not composited, and falls back to the main thread
    // paint.
    ASSERT_TRUE(paint_value->GetImage(*target, GetDocument(), *style, target_size));
    if (RuntimeEnabledFeatures::OffMainThreadCSSPaintEnabled()) {
        histogram_tester.ExpectTotalCount("Blink.CSSPaintValue.PaintOffThread", 1u);
        histogram_tester.ExpectUniqueSample("Blink.CSSPaintValue.PaintOffThread", false, 1u);
    } else {
        histogram_tester.ExpectTotalCount("Blink.CSSPaintValue.PaintOffThread", 0u);
    }
...
TEST_P(CSSPaintValueTest, PrintingMustFallbackToMainThread)
{
    ...
    EXPECT_CALL(*mock_generator, Paint(_, _, _)).Times(1).WillOnce(Return(PaintGeneratedImage::Create(nullptr, target_size)));
