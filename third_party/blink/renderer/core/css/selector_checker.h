...
public:
    ...
    inline SelectorChecker(ComputedStyle* element_style, PartNames* part_names, const StyleRequest& style_request, const Mode& mode, const bool& is_ua_rule)
...
private:
    ...
    const Activations& EnsureActivations(const SelectorCheckingContext&, const StyleScope&) const;
    const Activations* CalculateActivations(Element&, const StyleScope&, const Activations& outer_activations) const;
...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::SelectorChecker::StyleScopeActivation)
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::SelectorChecker::StyleScopeFrame)
