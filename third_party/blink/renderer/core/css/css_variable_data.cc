...
template <typename CharacterType> static void UpdateTokens(const CSSParserTokenRange& range, const String& backing_string, CSSParserToken* result)
{
    const CharacterType* current_offset = backing_string.GetCharacters<CharacterType>();
    ...
    DCHECK(current_offset == backing_string.GetCharacters<CharacterType>() + backing_string.length());
}
...
{
    return token.GetType() == kDimensionToken && token.GetUnitType() == CSSPrimitiveValue::UnitType::kRems;
}
...
{
    return std::equal(Tokens().begin(), Tokens().end(), other.Tokens().begin(), other.Tokens().end());
}
...
template <typename CharacterType> bool IsSubspan(base::span<const CharacterType> inner, base::span<const CharacterType> outer)
{
...
bool TokenValueIsBacked(const CSSParserToken& token, const String& backing_string)
{
    StringView value = token.Value();
    if (value.Is8Bit() != backing_string.Is8Bit())
        return false;
    return value.Is8Bit() ? IsSubspan(value.Span8(), backing_string.Span8()) : IsSubspan(value.Span16(), backing_string.Span16());
}
...
void CSSVariableData::VerifyStringBacking() const
{
    for (const CSSParserToken& token : Tokens()) {
        DCHECK(!token.HasStringBacking() || TokenValueIsBacked(token, backing_string_)) << "Token value is not backed: " << token.Value().ToString();
    }
}

#endif // EXPENSIVE_DCHECKS_ARE_ON()

CSSVariableData::CSSVariableData(const CSSTokenizedValue& tokenized_value, bool is_animation_tainted, bool needs_variable_resolution)
...
const CSSValue* CSSVariableData::ParseForSyntax(const CSSSyntaxDefinition& syntax, SecureContextMode secure_context_mode) const
{
    ...
    return syntax.Parse(TokenRange(), *StrictCSSParserContext(secure_context_mode), is_animation_tainted_);
}
