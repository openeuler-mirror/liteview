...
class ContainerQueryEvaluatorTest : public PageTestBase, private ScopedCSSContainerQueriesForTest, private ScopedLayoutNGForTest {
public:
    ...
    {
        scoped_refptr<ComputedStyle> style = GetDocument().GetStyleResolver().InitialStyleForElement();
        ...
        evaluator->SizeContainerChanged(GetDocument(), ContainerElement(), PhysicalSize(LayoutUnit(width), LayoutUnit(height)), contained_axes);
    ...
    bool Eval(String query, String custom_property_name, String custom_property_value)
    {
        ...
        CSSCustomPropertyDeclaration* value = CSSVariableParser::ParseDeclarationValue(tokenized_value, false, *context);
        DCHECK(value);

        scoped_refptr<ComputedStyle> style = GetDocument().GetStyleResolver().InitialStyleForElement();
        style->SetVariableData(AtomicString(custom_property_name), &value->Value(), false);
        ContainerElement().SetComputedStyle(style);

        auto* evaluator = MakeGarbageCollected<ContainerQueryEvaluator>();
        evaluator->SizeContainerChanged(GetDocument(), ContainerElement(), PhysicalSize(LayoutUnit(100), LayoutUnit(100)), PhysicalAxes { kPhysicalAxisNone });
    ...
    Change SizeContainerChanged(ContainerQueryEvaluator* evaluator, PhysicalSize size, unsigned container_type, PhysicalAxes axes)
    {
        ...
        style->SetContainerType(container_type);
        ContainerElement().SetComputedStyle(style);
        return evaluator->SizeContainerChanged(GetDocument(), ContainerElement(), size, axes);
    }
...
TEST_F(ContainerQueryEvaluatorTest, StyleContainerChanged)
{
    ...
    auto* evaluator = MakeGarbageCollected<ContainerQueryEvaluator>();
    EXPECT_EQ(Change::kNone, evaluator->SizeContainerChanged(GetDocument(), container_element, size_100, horizontal));
    ...
    // match.
    style->SetVariableData("--no", css_test_helpers::CreateVariableData("match"), inherited);
    EXPECT_EQ(Change::kNone, evaluator->StyleContainerChanged());
    EXPECT_EQ(3u, GetResults(evaluator).size());

    // Set --foo: bar. Should trigger change.
    style->SetVariableData("--foo", css_test_helpers::CreateVariableData("bar"), inherited);
    ...
    eval_and_add_all();
    style->SetVariableData("--bar", css_test_helpers::CreateVariableData("foo"), inherited);
    EXPECT_EQ(Change::kNearestContainer, evaluator->StyleContainerChanged());
    EXPECT_EQ(0u, GetResults(evaluator).size());
}
...
TEST_F(ContainerQueryEvaluatorTest, FindContainer)
{
    ...
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("style(--foo: bar)")->Selector()), inner);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("(width > 100px) and style(--foo: bar)")->Selector()), inner_size);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer style(--foo: bar)")->Selector()), outer);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer (width > 100px) and style(--foo: bar)")->Selector()), outer_size);
}
