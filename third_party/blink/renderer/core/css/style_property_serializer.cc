...
StylePropertySerializer::PropertyValueForSerializer StylePropertySerializer::CSSPropertyValueSetForSerializer::PropertyAt(unsigned index) const
{
    ...
    CSSPropertyID property_id = static_cast<CSSPropertyID>(index + kIntFirstCSSProperty);
...
String StylePropertySerializer::SerializeShorthand(CSSPropertyID property_id) const
{
    ...
    case CSSPropertyID::kToggle: {
        const CSSValue* toggle_root = property_set_.GetPropertyCSSValue(GetCSSPropertyToggleRoot());
        DCHECK(toggle_root);
        const CSSValue* toggle_trigger = property_set_.GetPropertyCSSValue(GetCSSPropertyToggleTrigger());
...
String StylePropertySerializer::ScrollTimelineValue() const
{
    ...
    const CSSValue* axis = property_set_.GetPropertyCSSValue(GetCSSPropertyScrollTimelineAxis());
    const CSSValue* name = property_set_.GetPropertyCSSValue(GetCSSPropertyScrollTimelineName());
...
CSSValue* ViewTimelineValueItem(wtf_size_t index, const CSSValueList& name_list, const CSSValueList& axis_list)
{
    ...
    if (!(IsA<CSSIdentifierValue>(axis) && To<CSSIdentifierValue>(axis).GetValueID() == CSSValueID::kBlock)) {
        list->Append(axis);
    }
...
String StylePropertySerializer::ViewTimelineValue() const
{
    ...
    const CSSValueList& name_list = To<CSSValueList>(*property_set_.GetPropertyCSSValue(GetCSSPropertyViewTimelineName()));
    const CSSValueList& axis_list = To<CSSValueList>(*property_set_.GetPropertyCSSValue(GetCSSPropertyViewTimelineAxis()));
...
String StylePropertySerializer::FontValue() const
{
    ...
    auto* ligatures_identifier_value = DynamicTo<CSSIdentifierValue>(ligatures_value);
    if ((ligatures_identifier_value && ligatures_identifier_value->GetValueID() != CSSValueID::kNormal) || ligatures_value->IsValueList())
        return g_empty_string;

    auto* numeric_identifier_value = DynamicTo<CSSIdentifierValue>(numeric_value);
    if ((numeric_identifier_value && numeric_identifier_value->GetValueID() != CSSValueID::kNormal) || numeric_value->IsValueList())
        return g_empty_string;

    auto* east_asian_identifier_value = DynamicTo<CSSIdentifierValue>(east_asian_value);
    if ((east_asian_identifier_value && east_asian_identifier_value->GetValueID() != CSSValueID::kNormal) || east_asian_value->IsValueList())
        return g_empty_string;
...
String StylePropertySerializer::TextDecorationValue() const
{
    ...
    for (unsigned i = 0; i < shorthand.length(); ++i) {
        const CSSValue* value = property_set_.GetPropertyCSSValue(*shorthand.properties()[i]);
    ...
    for (wtf_size_t layer = 0; layer < num_layers; layer++) {
        ...
        for (unsigned property_index = 0; property_index < size; property_index++) {
            ...
            if (property->IDEquals(CSSPropertyID::kBackgroundRepeatX) || property->IDEquals(CSSPropertyID::kWebkitMaskRepeatX)) {
                DCHECK(shorthand.properties()[property_index + 1]->IDEquals(CSSPropertyID::kBackgroundRepeatY)
                    || shorthand.properties()[property_index + 1]->IDEquals(CSSPropertyID::kWebkitMaskRepeatY));
                auto* value_list = DynamicTo<CSSValueList>(values[property_index + 1].Get());
                const CSSValue& y_value = value_list ? value_list->Item(layer) : *values[property_index + 1];
                ...
                if (!IsA<CSSIdentifierValue>(value) || !IsA<CSSIdentifierValue>(y_value))
                    continue;
                ...
                } else if (x_id == CSSValueID::kRepeat && y_id == CSSValueID::kNoRepeat) {
                    use_repeat_x_shorthand = true;
                    property = shorthand.properties()[++property_index];
                } else if (x_id == CSSValueID::kNoRepeat && y_id == CSSValueID::kRepeat) {
            ...
            if (property->IDEquals(CSSPropertyID::kAnimationTimeline)) {
                if (auto* ident = DynamicTo<CSSIdentifierValue>(value)) {
                    if (ident->GetValueID() == CSSAnimationData::InitialTimeline().GetKeyword()) {
            ...
            if (!is_initial_value) {
                if (property->IDEquals(CSSPropertyID::kBackgroundSize) || property->IDEquals(CSSPropertyID::kWebkitMaskSize)) {
                ...
                if (property->IDEquals(CSSPropertyID::kBackgroundPositionY) || property->IDEquals(CSSPropertyID::kWebkitMaskPositionY)) {
...
String NamedGridAreaTextForPosition(const NamedGridAreaMap& grid_area_map, wtf_size_t row, wtf_size_t column)
{
...
String StylePropertySerializer::GetShorthandValueForGrid(const StylePropertyShorthand& shorthand) const
{
    ...
    const CSSValue* auto_flow_values = property_set_.GetPropertyCSSValue(*shorthand.properties()[3]);
    const CSSValue* auto_row_values = property_set_.GetPropertyCSSValue(*shorthand.properties()[4]);
    const CSSValue* auto_column_values = property_set_.GetPropertyCSSValue(*shorthand.properties()[5]);

    // 1- <'grid-template'>
    if (IsA<CSSIdentifierValue>(auto_flow_values) && To<CSSIdentifierValue>(auto_flow_values)->GetValueID() == CSSValueID::kRow
        && IsA<CSSIdentifierValue>(auto_row_values) && To<CSSIdentifierValue>(auto_row_values)->GetValueID() == CSSValueID::kAuto
        && IsA<CSSIdentifierValue>(auto_column_values) && To<CSSIdentifierValue>(auto_column_values)->GetValueID() == CSSValueID::kAuto) {
    ...
    const CSSValue* template_area_value = property_set_.GetPropertyCSSValue(*shorthand.properties()[2]);
    if (*template_area_value != *(To<Longhand>(GetCSSPropertyGridTemplateAreas()).InitialValue()))
        return String();
    ...
    const CSSValueList* auto_flow_value_list = DynamicTo<CSSValueList>(auto_flow_values);

    // We cannot represent a grid shorthand if we have both template row and
    // template column values along with auto-flow.
    if (*template_row_values != *(To<Longhand>(GetCSSPropertyGridTemplateRows()).InitialValue())
        && *template_column_values != *(To<Longhand>(GetCSSPropertyGridTemplateColumns()).InitialValue())) {
        return String();
    }
...
String StylePropertySerializer::GetShorthandValueForGridTemplate(const StylePropertyShorthand& shorthand) const
{
    ...
    // 1- 'none' case.
    if (IsA<CSSIdentifierValue>(template_row_values) && To<CSSIdentifierValue>(template_row_values)->GetValueID() == CSSValueID::kNone
        && IsA<CSSIdentifierValue>(template_column_values) && To<CSSIdentifierValue>(template_column_values)->GetValueID() == CSSValueID::kNone) {
        return "none";
    }

    const auto* template_row_value_list = DynamicTo<CSSValueList>(template_row_values);
    StringBuilder result;

    // 2- <grid-template-rows> / <grid-template-columns>
    if (!template_row_value_list
        || (IsA<CSSIdentifierValue>(template_area_values) && To<CSSIdentifierValue>(template_area_values)->GetValueID() == CSSValueID::kNone)) {
        result.Append(template_row_values->CssText());
        result.Append(" / ");
        result.Append(template_column_values->CssText());
        return result.ReleaseString();
    ...
    // [ / <track-list> ]?
    if (template_row_value_list->length() == 1 && IsA<CSSIdentifierValue>(template_row_value_list->Item(0))
        && To<CSSIdentifierValue>(template_row_value_list->Item(0)).GetValueID() == CSSValueID::kAuto) {
        ...
        const auto* template_areas = DynamicTo<cssvalue::CSSGridTemplateAreasValue>(template_area_values);
    ...
    if (!(IsA<CSSIdentifierValue>(template_column_values) && To<CSSIdentifierValue>(template_column_values)->GetValueID() == CSSValueID::kNone)) {
        result.Append(" / ");
        result.Append(template_column_values->CssText());
    }
    return result.ReleaseString();
...
static void AppendBackgroundRepeatValue(StringBuilder& builder, const CSSValue& repeat_xcss_value, const CSSValue& repeat_ycss_value)
{
    // FIXME: Ensure initial values do not appear in CSS_VALUE_LISTS.
    DEFINE_STATIC_LOCAL(const Persistent<CSSIdentifierValue>, initial_repeat_value, (CSSIdentifierValue::Create(CSSValueID::kRepeat)));
    const CSSIdentifierValue& repeat_x = repeat_xcss_value.IsInitialValue() ? *initial_repeat_value : To<CSSIdentifierValue>(repeat_xcss_value);
    const CSSIdentifierValue& repeat_y = repeat_ycss_value.IsInitialValue() ? *initial_repeat_value : To<CSSIdentifierValue>(repeat_ycss_value);
    ...
    } else if (repeat_x_value_id == CSSValueID::kNoRepeat && repeat_y_value_id == CSSValueID::kRepeat) {
        builder.Append("repeat-y");
    } else if (repeat_x_value_id == CSSValueID::kRepeat && repeat_y_value_id == CSSValueID::kNoRepeat) {
...
{
    const CSSValue& repeat_x = *property_set_.GetPropertyCSSValue(GetCSSPropertyBackgroundRepeatX());
    const CSSValue& repeat_y = *property_set_.GetPropertyCSSValue(GetCSSPropertyBackgroundRepeatY());
    ...
    for (size_t i = 0; i < shorthand_length; ++i) {
        ...
        const CSSValue& x_value = repeat_x_list ? repeat_x_list->Item(i % repeat_x_list->length()) : repeat_x;
        const CSSValue& y_value = repeat_y_list ? repeat_y_list->Item(i % repeat_y_list->length()) : repeat_y;
...
bool StylePropertySerializer::IsValidToggleShorthand(const CSSValue* toggle_root, const CSSValue* toggle_trigger)
{
    if (const auto* toggle_root_ident = DynamicTo<CSSIdentifierValue>(toggle_root)) {
        DCHECK_EQ(toggle_root_ident->GetValueID(), CSSValueID::kNone);
        if (const auto* toggle_trigger_ident = DynamicTo<CSSIdentifierValue>(toggle_trigger)) {
    ...
    for (wtf_size_t i = 0; i < length; ++i) {
        ...
        const auto& toggle_trigger_item = To<CSSValueList>(toggle_trigger_list->Item(i));
        if (toggle_trigger_item.length() > 1u) {
            DCHECK_EQ(toggle_trigger_item.length(), 2u);
            return false;
        }
        if (!base::ValuesEquivalent(&toggle_root_item.Item(0), &toggle_trigger_item.Item(0))) {
            return false;
        }
    }
    return true;
