...
protected:
    ...
    void TestArgumentContext(const String& selector_text, CSSSelector::RelationType expected_leftmost_relation, int expected_adjacent_distance_limit,
        int expected_depth_limit, CheckPseudoHasArgumentTraversalScope expected_traversal_scope) const
    {
    ...
    void TestTraversalIteratorForEmptyRange(Document* document, const char* has_anchor_element_id, const char* selector_text) const
    {
        Element* has_anchor_element = document->getElementById(has_anchor_element_id);
        ...
        unsigned i = 0;
        CSSSelectorList selector_list = css_test_helpers::ParseSelectorList(selector_text);
        CheckPseudoHasArgumentContext argument_context(selector_list.First()->SelectorList()->First());
        for (CheckPseudoHasArgumentTraversalIterator iterator(*has_anchor_element, argument_context); !iterator.AtEnd(); ++iterator, ++i) {
            AtomicString current_element_id = iterator.CurrentElement() ? iterator.CurrentElement()->GetIdAttribute() : "nullptr";
    ...
    CheckPseudoHasArgumentTraversalType GetTraversalType(const char* selector_text) const
    {
        CSSSelectorList selector_list = css_test_helpers::ParseSelectorList(selector_text);
    ...
    void TestTraversalIteratorSteps(
        Document* document, const char* has_anchor_element_id, const char* selector_text, const ExpectedTraversalStep (&expected_traversal_steps)[length]) const
    {
        Element* has_anchor_element = document->getElementById(has_anchor_element_id);
        if (!has_anchor_element) {
            ADD_FAILURE() << "Failed : test iterator on #" << has_anchor_element_id << " (Cannot find element)";
        ...
        for (; i < length; i++) {
            ...
            EXPECT_NE(document->getElementById(expected_traversal_steps[i].element_id), nullptr);
        }
    }
};
