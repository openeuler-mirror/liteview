...
const CSSValue* ConsumeSingleType(const CSSSyntaxComponent& syntax, CSSParserTokenRange& range, const CSSParserContext& context)
{
    switch (syntax.GetType()) {
    case CSSSyntaxType::kIdent:
        if (range.Peek().GetType() == kIdentToken && range.Peek().Value() == syntax.GetString()) {
    ...
    case CSSSyntaxType::kPercentage:
        return css_parsing_utils::ConsumePercent(range, context, CSSPrimitiveValue::ValueRange::kAll);
    case CSSSyntaxType::kLengthPercentage: {
        CSSParserContext::ParserModeOverridingScope scope(context, kHTMLStandardMode);
        return css_parsing_utils::ConsumeLengthOrPercent(
            range, context, CSSPrimitiveValue::ValueRange::kAll, css_parsing_utils::UnitlessQuirk::kForbid, kCSSAnchorQueryTypesAll);
    }
    ...
    case CSSSyntaxType::kAngle:
        return css_parsing_utils::ConsumeAngle(range, context, absl::optional<WebFeature>());
...
const CSSValue* ConsumeSyntaxComponent(const CSSSyntaxComponent& syntax, CSSParserTokenRange range, const CSSParserContext& context)
{
...
const CSSValue* CSSSyntaxDefinition::Parse(CSSParserTokenRange range, const CSSParserContext& context, bool is_animation_tainted) const
{
    if (IsUniversal()) {
        // The 'default' keyword is reserved despite not being a CSS-wide keyword.
        // TODO(https://crbug.com/1344170): This code may be unneeded.
        if (CouldConsumeReservedKeyword(range))
            return nullptr;
        return CSSVariableParser::ParseVariableReferenceValue(range, context, is_animation_tainted);
    }
    range.ConsumeWhitespace();
