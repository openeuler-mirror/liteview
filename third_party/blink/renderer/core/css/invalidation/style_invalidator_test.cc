...
TEST_F(StyleInvalidatorTest, SkipDisplayNone)
{
    ...
    PendingInvalidations pending;
    {
        ...
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById("root"));
    }
...
TEST_F(StyleInvalidatorTest, SkipDisplayNoneClearPendingNth)
{
    ...
    PendingInvalidations pending;
    {
        ...
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById("none"));
    }
    {
        InvalidationLists lists;
        scoped_refptr<InvalidationSet> set = DescendantInvalidationSet::Create();
        set->AddClass("a");
        lists.descendants.push_back(set);
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById("descendant"));
    }
    ...
    EXPECT_TRUE(GetDocument().getElementById("descendant")->ChildNeedsStyleRecalc());
}
