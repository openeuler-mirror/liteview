...
CSSStyleValue* CreateStyleValueWithoutProperty(const CSSValue& value)
{
    ...
    if (auto* custom_prop_declaration = DynamicTo<CSSCustomPropertyDeclaration>(value)) {
    ...
    CSSPropertyID property_id, const AtomicString& custom_property_name, const String& css_text, const CSSParserContext* parser_context)
{
    ...
    if (property_id != CSSPropertyID::kVariable
        && CSSPropertyParser::ParseValue(property_id, false, range, parser_context, parsed_properties, StyleRule::RuleType::kStyle)) {
    ...
    if ((property_id == CSSPropertyID::kVariable && !tokens.empty()) || CSSVariableParser::ContainsValidVariableReferences(range)) {
        const auto variable_data
            = CSSVariableData::Create({ range, StringView(css_text) }, false /* is_animation_tainted */, false /* needs variable resolution */);
