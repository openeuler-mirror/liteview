...
unsigned int ComputedStylePropertyMap::size() const
{
    ...
    return CSSComputedStyleDeclaration::ComputableProperties(StyledNode()->GetExecutionContext()).size()
        + ComputedStyleCSSValueMapping::GetVariables(*style, document.GetPropertyRegistry()).size();
}
...
const ComputedStyle* ComputedStylePropertyMap::UpdateStyle() const
{
    ...
    const ComputedStyle* style = node->EnsureComputedStyle(node->IsPseudoElement() ? kPseudoIdNone : pseudo_id_);
    node = StyledNode();
...
const CSSValue* ComputedStylePropertyMap::GetCustomProperty(const AtomicString& property_name) const
{
    ...
    return ref.GetProperty().CSSValueFromComputedStyle(*style, nullptr /* layout_object */, false /* allow_visited_style */);
}
...
void ComputedStylePropertyMap::ForEachProperty(IterationFunction visitor)
{
    ...
    for (const CSSProperty* property : CSSComputedStyleDeclaration::ComputableProperties(StyledNode()->GetExecutionContext())) {
        DCHECK(property);
        DCHECK(!property->IDEquals(CSSPropertyID::kVariable));
        const CSSValue* value = property->CSSValueFromComputedStyle(*style, nullptr /* layout_object */, false);
        if (value)
    ...
    for (const auto& name_value : ComputedStyleCSSValueMapping::GetVariables(*style, registry)) {
...
String ComputedStylePropertyMap::SerializationForShorthand(const CSSProperty& property) const
{
    ...
    if (const CSSValue* value = property.CSSValueFromComputedStyle(*style, nullptr /* layout_object */, false)) {
