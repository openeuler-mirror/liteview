...
bool CanCombineNodes(const CSSMathExpressionNode& root, const CSSMathExpressionNode& node)
{
    ...
    if (node.IsNestedCalc())
        return false;
    return CanonicalOperator(To<CSSMathExpressionOperation>(root).OperatorType()) == CanonicalOperator(To<CSSMathExpressionOperation>(node).OperatorType());
}
...
CSSNumericValue* CalcToNumericValue(const CSSMathExpressionNode& root)
{
    ...
    // CSSMathMax.
    if (const auto& node = To<CSSMathExpressionOperation>(root); (node.IsMinOrMax() || node.IsClamp())) {
...
CSSNumericValue* CSSNumericValue::parse(const ExecutionContext* execution_context, const String& css_text, ExceptionState& exception_state)
{
    ...
    case kFunctionToken:
        if (range.Peek().FunctionId() == CSSValueID::kCalc || range.Peek().FunctionId() == CSSValueID::kWebkitCalc
            || range.Peek().FunctionId() == CSSValueID::kMin || range.Peek().FunctionId() == CSSValueID::kMax
            ...
            CSSMathExpressionNode* expression = CSSMathExpressionNode::ParseMathFunction(
                CSSValueID::kCalc, range, *MakeGarbageCollected<CSSParserContext>(*execution_context), kCSSAnchorQueryTypesNone);
            if (expression)
...
CSSNumericValue* CSSNumericValue::FromCSSValue(const CSSPrimitiveValue& value)
{
    if (value.IsCalculated()) {
        return CalcToNumericValue(*To<CSSMathFunctionValue>(value).ExpressionNode());
    }
...
{
    auto values = CSSNumberishesToNumericValues(numberishes);
    std::transform(values.begin(), values.end(), values.begin(), [](CSSNumericValue* v) { return v->Negate(); });
    PrependValueForArithmetic<kSumType>(values, this);
