...
V8UnionCSSColorValueOrCSSStyleValue* CSSColorValue::parse(const ExecutionContext* execution_context, const String& css_text, ExceptionState& exception_state)
{
    ...
    const CSSValue* parsed_value = css_parsing_utils::ConsumeColor(range, *MakeGarbageCollected<CSSParserContext>(*execution_context));
    ...
    if (parsed_value->IsColorValue()) {
        ...
        case CSSColorType::kRGB:
            return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(
                CreateCSSRGBByNumbers(result->Value().Red(), result->Value().Green(), result->Value().Blue(), result->Value().Alpha()));
    ...
    const char* value_name = getValueName(To<CSSIdentifierValue>(parsed_value)->GetValueID());
    if (const NamedColor* named_color = FindColor(value_name, static_cast<wtf_size_t>(strlen(value_name)))) {
        ...
        return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(CreateCSSRGBByNumbers(color.Red(), color.Green(), color.Blue(), color.Alpha()));
    }

    return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(CSSKeywordValue::Create(css_text));
}
