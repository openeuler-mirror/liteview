...
public:
    ...
    {
        WTF::HashTraits<AtomicString>::ConstructDeletedValue(name_, false /* zero_value */);
    }
    explicit ContainerSelector(AtomicString name)
        : name_(std::move(name))
    {
    }
    ...
    {
        return physical_axes_ != kPhysicalAxisNone || logical_axes_ != kLogicalAxisNone;
    }
...
template <> struct DefaultHash<blink::ContainerSelector> {
    struct Hash {
        STATIC_ONLY(Hash);
        ...
        static bool Equal(const blink::ContainerSelector& a, const blink::ContainerSelector& b)
        {
            return a == b;
        }

        static const bool safe_to_compare_to_empty_or_deleted = DefaultHash<AtomicString>::Hash::safe_to_compare_to_empty_or_deleted;
    };
};
