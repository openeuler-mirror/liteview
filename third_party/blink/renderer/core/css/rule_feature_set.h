...
public:
    ...
    {
        return metadata_.uses_first_line_rules;
    }
    ...
    {
        return not_pseudo_in_has_argument_ || universal_in_has_argument_ || !tag_names_in_has_argument_.empty() || NeedsHasInvalidationForClassChange()
    ...
    {
        return id_invalidation_sets_.size() > 0;
    }
    bool InvalidatesParts() const
    {
        return metadata_.invalidates_parts;
    }
...
private:
    ...
    InvalidationSet* InvalidationSetForSimpleSelector(const CSSSelector&, InvalidationType, PositionType);
    ...
    using PseudoTypeInvalidationSetMap
        = HashMap<CSSSelector::PseudoType, scoped_refptr<InvalidationSet>, WTF::IntHash<unsigned>, WTF::UnsignedWithZeroKeyHashTraits<unsigned>>;
    ...
    struct FeatureMetadata {
        ...
        bool operator!=(const FeatureMetadata& o) const
        {
            return !(*this == o);
        }
    ...
    InvalidationSet& EnsureClassInvalidationSet(const AtomicString& class_name, InvalidationType, PositionType);
    InvalidationSet& EnsureAttributeInvalidationSet(const AtomicString& attribute_name, InvalidationType, PositionType);
    InvalidationSet& EnsureIdInvalidationSet(const AtomicString& id, InvalidationType, PositionType);
    InvalidationSet& EnsurePseudoInvalidationSet(CSSSelector::PseudoType, InvalidationType, PositionType);
    ...
    FeatureInvalidationType UpdateInvalidationSetsForComplex(
        const CSSSelector&, const StyleScope*, InvalidationSetFeatures&, PositionType, CSSSelector::PseudoType);
    ...
    const CSSSelector* ExtractInvalidationSetFeaturesFromCompound(
        const CSSSelector&, InvalidationSetFeatures&, PositionType, bool for_logical_combination_in_has);
    void ExtractInvalidationSetFeaturesFromSelectorList(const CSSSelector&, InvalidationSetFeatures&, PositionType);
    void UpdateFeaturesFromCombinator(CSSSelector::RelationType, const CSSSelector* last_compound_selector_in_adjacent_chain,
        ...
        InvalidationSetFeatures& descendant_features, bool for_logical_combination_in_has);
    ...
    void AddFeaturesToInvalidationSets(const CSSSelector&, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features);
    const CSSSelector* AddFeaturesToInvalidationSetsForCompoundSelector(
        const CSSSelector&, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features);
    ...
    static InvalidationSet& EnsureMutableInvalidationSet(InvalidationType type, PositionType position, scoped_refptr<InvalidationSet>& invalidation_set);

    static InvalidationSet& EnsureInvalidationSet(InvalidationSetMap&, const AtomicString& key, InvalidationType, PositionType);
    static InvalidationSet& EnsureInvalidationSet(PseudoTypeInvalidationSetMap&, CSSSelector::PseudoType key, InvalidationType, PositionType);
    ...
    void MergeInvalidationSet(InvalidationSetMap&, const AtomicString& key, scoped_refptr<InvalidationSet>);
    void MergeInvalidationSet(PseudoTypeInvalidationSetMap&, CSSSelector::PseudoType key, scoped_refptr<InvalidationSet>);
