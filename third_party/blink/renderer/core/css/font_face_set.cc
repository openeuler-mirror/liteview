...
ScriptPromise FontFaceSet::load(ScriptState* script_state, const String& font_string, const String& text)
{
    if (!InActiveContext())
    ...
    if (!ResolveFontStyle(font_string, font)) {
        ...
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kSyntaxError, "Could not resolve '" + font_string + "' as a font."));
        return promise;
    }
...
bool FontFaceSet::check(const String& font_string, const String& text, ExceptionState& exception_state)
{
    ...
    if (has_loaded_faces)
        return true;
    for (const FontFamily* f = &font.GetFontDescription().Family(); f; f = f->Next()) {
        if (font_selector->IsPlatformFamilyMatchAvailable(font.GetFontDescription(), *f))
            return true;
    }
    return false;
}
