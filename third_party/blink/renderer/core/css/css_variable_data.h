...
public:
    ...
    static scoped_refptr<CSSVariableData> Create(const CSSTokenizedValue& tokenized_value, bool is_animation_tainted, bool needs_variable_resolution)
    {
        void* buf = AllocateSpaceIncludingCSSParserTokens(tokenized_value.range.size());
        return base::AdoptRef(new (buf) CSSVariableData(tokenized_value, is_animation_tainted, needs_variable_resolution));
    }

    CSSParserTokenRange TokenRange() const
    {
        return CSSParserTokenRange { base::span<CSSParserToken>(TokenInternalPtr(), num_tokens_) };
    }
    ...
    const AtomicString& BackingString() const
    {
        return backing_string_;
    }
...
private:
    ...
    CSSVariableData(const CSSTokenizedValue&, bool is_animation_tainted, bool needs_variable_resolution);
    ...
    {
        const size_t bytes_needed = sizeof(CSSVariableData) + num_tokens * sizeof(CSSParserToken);
        return WTF::Partitions::FastMalloc(bytes_needed, WTF::GetStringWithTypeName<CSSVariableData>());
    }

    CSSParserToken* TokenInternalPtr() const
    {
        return const_cast<CSSParserToken*>(reinterpret_cast<const CSSParserToken*>(this + 1));
    }
