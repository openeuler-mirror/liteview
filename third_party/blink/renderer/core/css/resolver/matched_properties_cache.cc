...
static unsigned ComputeMatchedPropertiesHash(const MatchResult& result)
{
    ...
    return StringHasher::HashMemory(vector.data(), sizeof(MatchedProperties) * vector.size());
}

void CachedMatchedProperties::Set(const ComputedStyle& style, const ComputedStyle& parent_style, const MatchedPropertiesVector& properties)
{
...
bool CachedMatchedProperties::DependenciesEqual(const StyleResolverState& state)
{
    if (!state.ParentStyle())
        return false;
    if ((parent_computed_style->IsEnsuredInDisplayNone() || computed_style->IsEnsuredOutsideFlatTree()) && !state.ParentStyle()->IsEnsuredInDisplayNone()
    ...
    if (parent_computed_style->GetWritingMode() != state.ParentStyle()->GetWritingMode()) {
        return false;
    }
    if (parent_computed_style->Direction() != state.ParentStyle()->Direction())
        return false;
    if (parent_computed_style->UsedColorScheme() != state.ParentStyle()->UsedColorScheme()) {
        return false;
    }
    if (computed_style->HasVariableReferenceFromNonInheritedProperty()) {
        if (parent_computed_style->InheritedVariables() != state.ParentStyle()->InheritedVariables()) {
            return false;
        }
    }
...
const CachedMatchedProperties* MatchedPropertiesCache::Find(const Key& key, const StyleResolverState& style_resolver_state)
{
    DCHECK(key.IsValid());
    ...
    if (*cache_item != key.result_.GetMatchedProperties())
        return nullptr;
    if (cache_item->computed_style->InsideLink() != style_resolver_state.Style()->InsideLink())
        return nullptr;
...
bool CachedMatchedProperties::operator==(const MatchedPropertiesVector& properties)
{
    ...
    for (wtf_size_t i = 0; i < properties.size(); ++i) {
        if (properties[i].properties != matched_properties[i])
            return false;
        if (properties[i].types_.link_match_type != matched_properties_types[i].link_match_type)
            return false;
        if (properties[i].types_.tree_order != matched_properties_types[i].tree_order)
            return false;
        if (properties[i].types_.layer_order != matched_properties_types[i].layer_order)
            return false;
        if (properties[i].types_.valid_property_filter != matched_properties_types[i].valid_property_filter)
            return false;
        if (properties[i].types_.is_inline_style != matched_properties_types[i].is_inline_style)
            return false;
    }
    return true;
}
...
bool MatchedPropertiesCache::IsStyleCacheable(const ComputedStyle& style)
{
    ...
    if (style.HasPseudoElementStyle(kPseudoIdHighlight) || style.StyleType() == kPseudoIdHighlight)
        return false;
    return true;
}
...
bool MatchedPropertiesCache::IsCacheable(const StyleResolverState& state)
{
    ...
    if (IsAtShadowBoundary(&state.GetElement()) && style.UserModify() != parent_style.UserModify()) {
        return false;
    }
...
void MatchedPropertiesCache::Trace(Visitor* visitor) const
{
    ...
    visitor->RegisterWeakCallbackMethod<MatchedPropertiesCache, &MatchedPropertiesCache::RemoveCachedMatchedPropertiesWithDeadEntries>(this);
}

void MatchedPropertiesCache::RemoveCachedMatchedPropertiesWithDeadEntries(const LivenessBroker& info)
{
    ...
    for (const auto& entry_pair : cache_) {
        ...
        if (!entry_pair.value)
            continue;
        for (const auto& matched_properties : entry_pair.value->matched_properties) {
