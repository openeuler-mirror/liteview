...
public:
    ...
    const CachedMatchedProperties* Find(const TestKey& key, const ComputedStyle& style, const ComputedStyle& parent_style)
    {
        StyleResolverState state(document_, *document_.body(), nullptr /* StyleRecalcContext */, StyleRequest(&parent_style));
...
TEST_F(MatchedPropertiesCacheTest, AllowedKeyValues)
{
    ...
    EXPECT_TRUE(TestKey("left:0", deleted - 1, GetDocument()).InnerKey().IsValid());
