...
TEST_F(StyleBuilderTest, WritingModeChangeDirtiesFont)
{
    ...
    for (const CSSProperty* property : properties) {
        for (const CSSValue* value : values) {
            ...
            StyleResolverState state(GetDocument(), *GetDocument().body(), nullptr /* StyleRecalcContext */, StyleRequest(parent_style.get()));
...
TEST_F(StyleBuilderTest, GridTemplateAreasApplyOrder)
{
    ...
    scoped_refptr<ComputedStyle> parent_style = GetDocument().GetStyleResolver().CreateComputedStyle();
    StyleResolverState state(GetDocument(), *GetDocument().body(), nullptr /* StyleRecalcContext */, StyleRequest(parent_style.get()));
    ...
    StyleBuilder::ApplyProperty(grid_template_areas, state, ScopedCSSValue(*grid_template_areas_value, nullptr));
    StyleBuilder::ApplyProperty(grid_template_columns, state, ScopedCSSValue(*grid_template_columns_value, nullptr));
    StyleBuilder::ApplyProperty(grid_template_rows, state, ScopedCSSValue(*grid_template_rows_value, nullptr));
