...
void ExpandCascade(const MatchedProperties& matched_properties, const Document& document, wtf_size_t matched_properties_index, Callback&& callback)
{
    ...
    for (wtf_size_t property_idx = 0; property_idx < size; ++property_idx) {
        ...
        CascadePriority priority(matched_properties.types_.origin, metadata.important_, matched_properties.types_.tree_order,
            matched_properties.types_.is_inline_style, matched_properties.types_.layer_order,
        ...
        if (id == CSSPropertyID::kVariable) {
            ...
            if (!filter.Rejects(custom)) {
                callback(priority, custom, CSSPropertyName(reference.Name().ToAtomicString()), reference.Value(), matched_properties.types_.tree_order);
            }
            ...
            for (int i = kIntFirstCSSProperty; i <= kIntLastCSSProperty; ++i) {
                ...
                if (!filter.Rejects(property)) {
                    callback(priority, property, CSSPropertyName(expanded_id), reference.Value(), matched_properties.types_.tree_order);
                }
            }
        } else {
            const CSSProperty& property = CSSProperty::Get(id);
            if (!filter.Rejects(property)) {
                callback(priority, property, CSSPropertyName(id), reference.Value(), matched_properties.types_.tree_order);
            }
            if (expand_visited) {
                const CSSProperty* visited_property = property.GetVisitedProperty();
                if (visited_property && !filter.Rejects(*visited_property)) {
                    callback(priority, *visited_property, visited_property->GetCSSPropertyName(), reference.Value(), matched_properties.types_.tree_order);
                }
            }
        }
    }
