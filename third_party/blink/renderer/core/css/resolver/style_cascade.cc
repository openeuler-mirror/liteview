...
const CSSValue* Parse(const CSSProperty& property, CSSParserTokenRange range, const CSSParserContext* context)
{
    return CSSPropertyParser::ParseSingleValue(property.PropertyID(), range, context);
}
...
void StyleCascade::Apply(CascadeFilter filter)
{
    ...
    if ((state_.Style()->InsideLink() != EInsideLink::kInsideVisitedLink && (resolver.AuthorFlags() & CSSProperty::kHighlightColors))
        || (state_.Style()->InsideLink() == EInsideLink::kInsideVisitedLink && (resolver.AuthorFlags() & CSSProperty::kVisitedHighlightColors))) {
...
void StyleCascade::AnalyzeMatchResult()
{
    ...
    for (const MatchedProperties& properties : match_result_.GetMatchedProperties()) {
        ExpandCascade(properties, GetDocument(), index++,
            [this](CascadePriority cascade_priority, const CSSProperty& css_property, const CSSPropertyName& name, const CSSValue& css_value [[maybe_unused]],
...
void StyleCascade::ApplyCascadeAffecting(CascadeResolver& resolver)
{
    ...
    if (depends_on_cascade_affecting_property_) {
        if (direction != state_.Style()->Direction() || writing_mode != state_.Style()->GetWritingMode()) {
            Reanalyze();
        }
    }
}
...
void StyleCascade::LookupAndApplyDeclaration(const CSSProperty& property, CascadePriority* priority, CascadeResolver& resolver)
{
    ...
    *priority = CascadePriority(*priority, resolver.generation_);
    ...
    StyleBuilder::ApplyProperty(property, state_, ScopedCSSValue(*value, tree_scope));
}
...
bool StyleCascade::TokenSequence::AppendTokens(base::span<const CSSParserToken> tokens, wtf_size_t limit)
{
...
bool StyleCascade::TokenSequence::Append(const TokenSequence& sequence, wtf_size_t limit)
{
...
bool StyleCascade::TokenSequence::Append(CSSVariableData* data, wtf_size_t limit)
{
...
scoped_refptr<CSSVariableData> StyleCascade::TokenSequence::BuildVariableData()
{
    // TODO(crbug.com/661854): We have no original_text_ in this case,
    // even though we should.
    return CSSVariableData::Create(
        CSSTokenizedValue { CSSParserTokenRange { tokens_ }, StringView {} }, is_animation_tainted_, /*needs_variable_resolution=*/false);
}
...
const CSSValue* StyleCascade::ResolveCustomProperty(const CSSProperty& property, const CSSCustomPropertyDeclaration& decl, CascadeResolver& resolver)
{
    DCHECK(!property.IsSurrogate());
    ...
    return MakeGarbageCollected<CSSCustomPropertyDeclaration>(data, decl.ParserContext());
}

const CSSValue* StyleCascade::ResolveVariableReference(const CSSProperty& property, const CSSVariableReferenceValue& value, CascadeResolver& resolver)
{
    DCHECK(!property.IsSurrogate());
    DCHECK(!resolver.IsLocked(property));
    CascadeResolver::AutoLock lock(property, resolver);
    ...
    if (ResolveTokensInto(CSSParserTokenRange { data->Tokens() }, resolver, sequence)) {
    ...
    const CSSProperty& property, const cssvalue::CSSPendingSubstitutionValue& value, CascadeResolver& resolver)
{
    ...
    if (!is_cached) {
        ...
        if (!ResolveTokensInto(CSSParserTokenRange { shorthand_data->Tokens() }, resolver, sequence))
            return cssvalue::CSSUnsetValue::Create();
        ...
        if (!CSSPropertyParser::ParseValue(
                shorthand_property_id, important, sequence.TokenRange(), shorthand_value->ParserContext(), parsed_properties, StyleRule::RuleType::kStyle)) {
            return cssvalue::CSSUnsetValue::Create();
        }
...
const CSSValue* StyleCascade::ResolveRevertLayer(
    const CSSProperty& property, const CSSValue& value, CascadePriority priority, CascadeOrigin& origin, CascadeResolver& resolver)
{
...
scoped_refptr<CSSVariableData> StyleCascade::ResolveVariableData(CSSVariableData* data, CascadeResolver& resolver)
{
...
bool StyleCascade::ResolveTokensInto(CSSParserTokenRange range, CascadeResolver& resolver, TokenSequence& out)
{
    bool success = true;
    while (!range.AtEnd()) {
...
bool StyleCascade::ResolveVarInto(CSSParserTokenRange range, CascadeResolver& resolver, TokenSequence& out)
{
...
bool StyleCascade::ResolveEnvInto(CSSParserTokenRange range, CascadeResolver& resolver, TokenSequence& out)
{
    AtomicString variable_name = ConsumeVariableName(range);
    DCHECK(range.AtEnd() || (range.Peek().GetType() == kCommaToken) || (range.Peek().GetType() == kNumberToken));
...
const CSSParserContext* StyleCascade::GetParserContext(const CSSVariableReferenceValue& value)
{
...
bool StyleCascade::ValidateFallback(const CustomProperty& property, CSSParserTokenRange range) const
{
#if DCHECK_IS_ON()
...
const CSSProperty& StyleCascade::ResolveSurrogate(const CSSProperty& property)
{
    ...
    const CSSProperty* original = property.SurrogateFor(state_.Style()->Direction(), state_.Style()->GetWritingMode());
    DCHECK(original);
    return *original;
}
