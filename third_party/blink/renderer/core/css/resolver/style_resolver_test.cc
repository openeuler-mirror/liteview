...
protected:
    scoped_refptr<ComputedStyle> StyleForId(AtomicString id)
    {
        ...
        auto style = GetStyleEngine().GetStyleResolver().ResolveStyle(element, StyleRecalcContext());
        DCHECK(style);
        return style;
    }
    ...
    String ComputedValue(String name, const ComputedStyle& style)
    {
        ...
        DCHECK(ref.IsValid());
        return ref.GetProperty().CSSValueFromComputedStyle(style, nullptr, false)->CssText();
    }
...
class StyleResolverTestCQ : public StyleResolverTest, public ScopedCSSContainerQueriesForTest, public ScopedLayoutNGForTest {
protected:
    StyleResolverTestCQ()
...
TEST_F(StyleResolverTest, AnimationBaseComputedStyle)
{
    ...
    EXPECT_EQ(10, resolver.ResolveStyle(div, StyleRecalcContext(), style_request)->FontSize());
    ASSERT_TRUE(style1->GetBaseComputedStyle());
    EXPECT_EQ(20, style1->GetBaseComputedStyle()->FontSize());
...
TEST_F(StyleResolverTest, NoFetchForAtPage)
{
    ...
    scoped_refptr<const ComputedStyle> page_style = GetDocument().GetStyleResolver().StyleForPage(0, "");
...
TEST_F(StyleResolverTest, NoFetchForHighlightPseudoElements)
{
    ...
    for (const auto* pseudo_style : { target_text_style.get(), selection_style.get() }) {
...
TEST_F(StyleResolverTest, CSSMarkerPseudoElement)
{
    ...
    for (unsigned i = 0; i < lis->length(); ++i) {
        ...
        PseudoElement* before = li->QuerySelector("b")->GetPseudoElement(kPseudoIdBefore);
...
TEST_F(StyleResolverTest, EnsureComputedStyleSlotFallback)
{
    ...
    ShadowRoot& shadow_root = GetDocument().getElementById("host")->AttachShadowRootInternal(ShadowRootType::kOpen);
...
TEST_F(StyleResolverTest, ComputeValueStandardProperty)
{
    ...
    MutableCSSPropertyValueSet::SetResult result = set->SetProperty(property_id, "var(--color)", false, SecureContextMode::kInsecureContext,
...
TEST_F(StyleResolverTest, ComputeValueCustomProperty)
{
    ...
    const CSSValue* parsed_value = css_test_helpers::ParseLonghand(GetDocument(), CustomProperty(custom_property_name, GetDocument()), "blue");
    ASSERT_TRUE(parsed_value);
...
TEST_F(StyleResolverTest, TreeScopedReferences)
{
    ...
    ShadowRoot& inner_root = inner_host->AttachShadowRootInternal(ShadowRootType::kOpen);
...

    {
        ...
        ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, state.Style(), EInsideLink::kNotInsideLink);
        GetDocument().GetStyleEngine().GetStyleResolver().MatchAllRules(state, collector, false /* include_smil_properties */);
        ...
        // :host { font-family: myfont }
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[1].types_.tree_order), root.GetTreeScope());
        EXPECT_EQ(properties[1].types_.origin, CascadeOrigin::kAuthor);

        // #host { animation-name: anim }
        EXPECT_EQ(properties[2].types_.origin, CascadeOrigin::kAuthor);
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[2].types_.tree_order), host->GetTreeScope());
    }

    {
        ...
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[0].types_.tree_order), inner_root.GetTreeScope());
...
TEST_F(StyleResolverTest, InheritStyleImagesFromDisplayContents)
{
    ...
    EXPECT_FALSE(style->MaskImage()->IsPendingImage()) << "-webkit-mask-image is fetched";
}
...
TEST_F(StyleResolverTest, TextShadowInHighlightPseudoNotCounted1)
{
    ...
    scoped_refptr<ComputedStyle> selection_style = GetDocument().GetStyleResolver().ResolveStyle(target, StyleRecalcContext(), pseudo_style_request);
...
TEST_F(StyleResolverTest, CascadeLayersInDifferentSheets)
{
    ...
    EXPECT_TRUE(properties[3].properties->HasProperty(CSSPropertyID::kFontFamily));
...
{
    GetDocument().documentElement()->setInnerHTMLWithDeclarativeShadowDOMForTesting(R"HTML(
    <style>
      @layer foo {
        #host { color: green; }
      }
    ...
    StyleResolverState state(GetDocument(), *GetDocument().getElementById("host"));
    SelectorFilter filter;
    MatchResult match_result;
    ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, state.Style(), EInsideLink::kNotInsideLink);
    MatchAllRules(state, collector);
    ...
    EXPECT_EQ(0u, properties[1].types_.layer_order);
    EXPECT_EQ(properties[1].types_.origin, CascadeOrigin::kAuthor);
    EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[1].types_.tree_order), GetDocument().getElementById("host")->GetShadowRoot());

    // @layer foo { #host { color: green } }
    EXPECT_TRUE(properties[2].properties->HasProperty(CSSPropertyID::kColor));
    EXPECT_EQ(0u, properties[2].types_.layer_order);
    EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[2].types_.tree_order), &GetDocument());
}
...
TEST_F(StyleResolverTest, CascadeLayersAfterModifyingAnotherSheet)
{
    ...
    GetDocument().getElementById("addrule")->appendChild(GetDocument().createTextNode("target { font-size: 10px; }"));

    UpdateAllLifecyclePhasesForTest();

    ASSERT_TRUE(GetDocument().GetScopedStyleResolver()->GetCascadeLayerMap());

    StyleResolverState state(GetDocument(), *GetDocument().QuerySelector("target"));
    SelectorFilter filter;
    MatchResult match_result;
    ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, state.Style(), EInsideLink::kNotInsideLink);
    MatchAllRules(state, collector);
...
TEST_F(StyleResolverTest, CascadeLayersAddLayersWithImportantDeclarations)
{
    ...
    GetDocument().getElementById("addrule")->appendChild(GetDocument().createTextNode("@layer { target { font-size: 20px !important; } }"
...
{
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, "contain:size; display:inline-table; content:url(img);");
...
TEST_F(StyleResolverTestCQ, StyleRulesForElementContainerQuery)
{
    ...
    auto* rule_list = resolver.StyleRulesForElement(target, StyleResolver::kAuthorCSSRules | StyleResolver::kCrossOriginCSSRules);
...
TEST_F(StyleResolverTestCQ, ContainerUnitContext)
{
    ...
    scoped_refptr<ComputedStyle> style = ComputedStyle::Clone(div->ComputedStyleRef());
