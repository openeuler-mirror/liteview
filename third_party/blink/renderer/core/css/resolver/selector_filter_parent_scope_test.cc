    ...
    SelectorFilterParentScope html_scope(*GetDocument().documentElement());
    {
        ...
        SelectorFilterParentScope::EnsureParentStackIsPushed();
        {
            ...
            CSSSelectorVector selector_vector
                = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext), nullptr,
            ...
            for (const CSSSelector* selector = selectors.First(); selector; selector = CSSSelectorList::Next(*selector)) {
                ...
                filter.CollectIdentifierHashes(*selector, selector_hashes, max_identifier_hashes);
                EXPECT_NE(selector_hashes[0], 0u);
                EXPECT_FALSE(filter.FastRejectSelector<max_identifier_hashes>(selector_hashes));
            }
        }
    }
}
...
TEST_F(SelectorFilterParentScopeTest, RootScope)
{
    ...
    Arena arena;
    CSSSelectorVector selector_vector = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext),
