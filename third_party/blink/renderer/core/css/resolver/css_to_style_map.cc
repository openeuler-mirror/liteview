...
void CSSToStyleMap::MapFillImage(StyleResolverState& state, FillLayer* layer, const CSSValue& value)
{
    ...
    CSSPropertyID property = layer->GetType() == EFillLayerType::kBackground ? CSSPropertyID::kBackgroundImage : CSSPropertyID::kWebkitMaskImage;
...
StyleTimeline CSSToStyleMap::MapAnimationTimeline(const CSSValue& value)
{
    ...
    if (auto* custom_ident = DynamicTo<CSSCustomIdentValue>(value)) {
        return StyleTimeline(StyleName(custom_ident->Value(), StyleName::Type::kCustomIdent));
    }
    if (auto* string_value = DynamicTo<CSSStringValue>(value)) {
        return StyleTimeline(StyleName(AtomicString(string_value->Value()), StyleName::Type::kString));
    }
...
void CSSToStyleMap::MapNinePieceImageSlice(StyleResolverState&, const CSSValue& value, NinePieceImage& image)
{
    ...
    box.bottom_ = ConvertBorderImageSliceSide(To<CSSPrimitiveValue>(*slices.Bottom()));
    box.left_ = ConvertBorderImageSliceSide(To<CSSPrimitiveValue>(*slices.Left()));
    box.right_ = ConvertBorderImageSliceSide(To<CSSPrimitiveValue>(*slices.Right()));
