...
public:
    ...
    static constexpr uint32_t kOriginImportanceMask = 0xF << kOriginImportanceOffset; // of high_bits_
    ...
    CascadePriority(CascadeOrigin origin, bool important, uint16_t tree_order, bool is_inline_style, uint16_t layer_order, uint32_t position)
    ...
    {
        uint64_t important_xor = (((~static_cast<uint64_t>(high_bits_) >> kImportantBit) & 1) - 1);
        uint32_t high_important_xor = important_xor & (kOriginImportanceMask | kTreeOrderMask);
        ...
        uint64_t low_bits_without_importance_position_and_generation = (low_bits_ ^ low_important_xor) & ~(kGenerationMask | kPositionMask);
        return CascadePriority(low_bits_without_importance_position_and_generation, high_bits_without_importance);
    }
