...
TouchAction AdjustTouchActionForElement(TouchAction touch_action, const ComputedStyle& style, const ComputedStyle& parent_style, Element* element)
{
    ...
    if (element && element == element->GetDocument().FirstBodyElement()) {
        ...
        if (parent_style.IsOverflowVisibleAlongBothAxes()) {
            if (!parent_style.ShouldApplyAnyContainment(*document_element) && !style.ShouldApplyAnyContainment(*element)) {
                scrolls_overflow = false;
            }
        }
    }
    bool is_child_document = element && element == document_element && element->GetDocument().LocalOwner();
...
void AdjustStyleForSvgElement(const SVGElement& element, ComputedStyle& style)
{
    ...
    style.SetTextDecorationStyle(ETextDecorationStyle::kSolid); // crbug.com/1246719
...
void AdjustAnchorQueryStyles(ComputedStyle& style)
{
    ...
    // elements.
    if (style.GetPosition() != EPosition::kAbsolute && style.GetPosition() != EPosition::kFixed) {
...
static bool StopPropagateTextDecorations(const ComputedStyle& style, const Element* element)
{
    return style.Display() == EDisplay::kInlineTable || style.Display() == EDisplay::kInlineBlock || style.Display() == EDisplay::kWebkitInlineBox
        || IsAtMediaUAShadowBoundary(element) || style.IsFloating() || style.HasOutOfFlowPosition() || IsOutermostSVGElement(element)
...
{
    return element && (IsA<HTMLFontElement>(element) || IsA<HTMLAnchorElement>(element));
}
...
{
    return (element && element->IsInTopLayer()) || style.StyleType() == kPseudoIdBackdrop;
}
...
static void AdjustStyleForMarker(ComputedStyle& style, const ComputedStyle& parent_style, const Element& parent_element)
{
    if (style.StyleType() != kPseudoIdMarker)
        return;

    bool is_inside
        = parent_style.ListStylePosition() == EListStylePosition::kInside || (IsA<HTMLLIElement>(parent_element) && !parent_style.IsInsideListElement());

    if (is_inside) {
        Document& document = parent_element.GetDocument();
        auto margins = ListMarker::InlineMarginsForInside(document, style, parent_style);
...
static void AdjustStyleForHTMLElement(ComputedStyle& style, HTMLElement& element)
{
    ...
    if (IsA<HTMLTableElement>(element)) {
        ...
        // back to the default.
        if (style.GetTextAlign() == ETextAlign::kWebkitLeft || style.GetTextAlign() == ETextAlign::kWebkitCenter
    ...
    if (IsA<HTMLFencedFrameElement>(element)) {
        ...
        style.SetEffectiveZoom(element.GetDocument().GetStyleResolver().InitialZoom());
    ...
    if (IsA<HTMLLegendElement>(element) && style.Display() != EDisplay::kContents) {
    ...
    if (IsA<HTMLTextAreaElement>(element)) {
        ...
        style.SetOverflowX(style.OverflowX() == EOverflow::kVisible ? EOverflow::kAuto : style.OverflowX());
        style.SetOverflowY(style.OverflowY() == EOverflow::kVisible ? EOverflow::kAuto : style.OverflowY());
        if (style.Display() == EDisplay::kContents)
            style.SetDisplay(EDisplay::kNone);
        return;
    }
    ...
    if (auto* html_plugin_element = DynamicTo<HTMLPlugInElement>(element)) {
        style.SetRequiresAcceleratedCompositingForExternalReasons(html_plugin_element->ShouldAccelerate());
        if (style.Display() == EDisplay::kContents)
            style.SetDisplay(EDisplay::kNone);
        return;
    }
...
{
    DCHECK(style.OverflowX() != EOverflow::kVisible || style.OverflowY() != EOverflow::kVisible);

    bool overflow_is_clip_or_visible = IsOverflowClipOrVisible(style.OverflowY()) && IsOverflowClipOrVisible(style.OverflowX());
    ...
    if (element && !element->IsPseudoElement() && (style.OverflowX() == EOverflow::kClip || style.OverflowY() == EOverflow::kClip)) {
        UseCounter::Count(element->GetDocument(), WebFeature::kOverflowClipAlongEitherAxis);
    }
}

static void AdjustStyleForDisplay(ComputedStyle& style, const ComputedStyle& layout_parent_style, const Element* element, Document* document)
{
    ...
    if (style.Display() == EDisplay::kInline && style.StyleType() == kPseudoIdNone && style.GetWritingMode() != layout_parent_style.GetWritingMode())
        style.SetDisplay(EDisplay::kInlineBlock);
    ...
    if (style.Display() == EDisplay::kTableColumn || style.Display() == EDisplay::kTableColumnGroup || style.Display() == EDisplay::kTableFooterGroup
        || style.Display() == EDisplay::kTableHeaderGroup || style.Display() == EDisplay::kTableRow || style.Display() == EDisplay::kTableRowGroup) {
...
bool StyleAdjuster::IsEditableElement(Element* element, const ComputedStyle& style)
{
...
bool StyleAdjuster::IsPasswordFieldWithUnrevealedPassword(Element* element)
{
    if (!element)
        return false;
    if (auto* input = DynamicTo<HTMLInputElement>(element)) {
        return (input->type() == input_type_names::kPassword) && !input->ShouldRevealPassword();
    }
    return false;
}

void StyleAdjuster::AdjustEffectiveTouchAction(ComputedStyle& style, const ComputedStyle& parent_style, Element* element, bool is_svg_root)
{
    TouchAction inherited_action = parent_style.GetEffectiveTouchAction();

    bool is_replaced_canvas = element && IsA<HTMLCanvasElement>(element) && element->GetExecutionContext()
        && element->GetExecutionContext()->CanExecuteScripts(kNotAboutToExecuteScript);
    bool is_non_replaced_inline_elements
        = style.IsDisplayInlineType() && !(style.IsDisplayReplacedType() || is_svg_root || IsA<HTMLImageElement>(element) || is_replaced_canvas);
    bool is_table_row_or_column = style.IsDisplayTableRowOrColumnType();
    bool is_layout_object_needed = element && element->LayoutObjectIsNeeded(style);
    ...
    // iframe element.
    inherited_action = AdjustTouchActionForElement(inherited_action, style, parent_style, element);
    ...
    if (::features::IsSwipeToMoveCursorEnabled() && IsEditableElement(element, style)) {
        element_touch_action &= ~TouchAction::kInternalPanXScrolls;
    }
    ...
    if (base::FeatureList::IsEnabled(blink::features::kStylusWritingToInput) && RuntimeEnabledFeatures::StylusHandwritingEnabled()
        && (element_touch_action & TouchAction::kPan) == TouchAction::kPan && IsEditableElement(element, style)
        && !IsPasswordFieldWithUnrevealedPassword(element)) {
        element_touch_action &= ~TouchAction::kInternalNotWritable;
    }

    // Apply the adjusted parent effective touch actions.
    style.SetEffectiveTouchAction((element_touch_action & inherited_action) | enforced_by_policy);
    ...
    if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(element)) {
        ...
        if (content_frame) {
            content_frame->SetInheritedEffectiveTouchAction(style.GetEffectiveTouchAction());
        }
    }
}
...
{
    if (!style.InForcedColorsMode() || style.ForcedColorAdjust() != EForcedColorAdjust::kAuto)
        return;
...
void StyleAdjuster::AdjustComputedStyle(StyleResolverState& state, Element* element)
{
    ...
    if (html_element && (style.Display() != EDisplay::kNone || element->LayoutObjectIsNeeded(style))) {
        AdjustStyleForHTMLElement(style, *html_element);
    }

    auto* svg_element = DynamicTo<SVGElement>(element);

    bool is_mathml_element = RuntimeEnabledFeatures::MathMLCoreEnabled() && IsA<MathMLElement>(element);
    ...
    if (style.Display() != EDisplay::kNone) {
        ...
        if (IsInTopLayer(element, style) && !is_document_element) {
            if (style.GetPosition() == EPosition::kStatic || style.GetPosition() == EPosition::kRelative) {
                style.SetPosition(EPosition::kAbsolute);
            }
            if (style.Display() == EDisplay::kContents) {
        ...
        if (style.Display() != EDisplay::kContents && (style.HasOutOfFlowPosition() || style.IsFloating()))
            style.SetDisplay(EquivalentBlockDisplay(style.Display()));
        ...
        // values.
        if ((!element || !is_mathml_element) && style.IsDisplayMathBox(style.Display())) {
            DCHECK(RuntimeEnabledFeatures::MathMLCoreEnabled());
            style.SetDisplay(style.Display() == EDisplay::kBlockMath ? EDisplay::kBlock : EDisplay::kInline);
        }
        ...
        AdjustStyleForDisplay(style, layout_parent_style, element, element ? &element->GetDocument() : nullptr);
        ...
        if (layout_parent_style.IsDisplayLayoutCustomBox()) {
            style.SetDisplayLayoutCustomParentName(layout_parent_style.DisplayLayoutCustomName());
        }
    ...
    if (style.GetPosition() == EPosition::kStatic && !LayoutParentStyleForcesZIndexToCreateStackingContext(layout_parent_style)) {
    ...
    if (style.OverflowX() != EOverflow::kVisible || style.OverflowY() != EOverflow::kVisible)
    ...
    if (style.Display() != EDisplay::kContents) {
        style.ApplyTextDecorations(parent_style.VisitedDependentColor(GetCSSPropertyTextDecorationColor()), OverridesTextDecorationColors(element));
    }
    ...
    if (svg_element) {
        ...
        if (style.Display() == EDisplay::kContents
            && (is_svg_root
                || (!IsA<SVGSVGElement>(element) && !IsA<SVGGElement>(element) && !IsA<SVGUseElement>(element) && !IsA<SVGTSpanElement>(element)))) {
        ...
        if ((IsA<SVGForeignObjectElement>(*element) || IsA<SVGTextElement>(*element)) && style.IsDisplayInlineType())
            style.SetDisplay(EDisplay::kBlock);
    ...
    if (parent_style.JustifyItemsPositionType() == ItemPositionType::kLegacy && style.JustifyItemsPosition() == ItemPosition::kLegacy) {
    ...
    if (!HasFullNGFragmentationSupport()) {
        ...
        if (style.SpecifiesColumns() || (element && element->GetDocument().Printing()))
