...
bool IsUsingContainerRelativeUnits(const CSSValue& value)
{
    ...
    return image_generator_value && image_generator_value->IsUsingContainerRelativeUnits();
}
...
public:
    ...
    StyleImageLoader(Document& document, ComputedStyle& style, const PreCachedContainerSizes& pre_cached_container_sizes, float device_scale_factor)
        : document_(document)
        , style_(style)
...
{
    const ContainerSizes& container_sizes = IsUsingContainerRelativeUnits(value) ? pre_cached_container_sizes_.Get() : ContainerSizes();

    if (auto* image_value = DynamicTo<CSSImageValue>(value)) {
        return image_value->CacheImage(document_, image_request_behavior, cross_origin);
    }

    if (auto* paint_value = DynamicTo<CSSPaintValue>(value)) {
        auto* image = MakeGarbageCollected<StyleGeneratedImage>(*paint_value, container_sizes);
    ...
    if (auto* crossfade_value = DynamicTo<cssvalue::CSSCrossfadeValue>(value)) {
        return MakeGarbageCollected<StyleCrossfadeImage>(
            *crossfade_value, CrossfadeArgument(crossfade_value->From(), cross_origin), CrossfadeArgument(crossfade_value->To(), cross_origin));
    }
    ...
    if (auto* image_set_value = DynamicTo<CSSImageSetValue>(value)) {
        return image_set_value->CacheImage(document_, device_scale_factor_, image_request_behavior, cross_origin);
    }
...
ElementStyleResources::ElementStyleResources(Element& element, float device_scale_factor, PseudoElement* pseudo_element)
    : element_(element)
    , device_scale_factor_(device_scale_factor)
...
SVGResource* ElementStyleResources::GetSVGResourceFromValue(CSSPropertyID property, const cssvalue::CSSURIValue& value)
{
    if (value.IsLocal(element_.GetDocument())) {
        ...
        AtomicString decoded_fragment(DecodeURLEscapeSequences(value.FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
...
void ElementStyleResources::LoadPendingSVGResources(ComputedStyle& style)
{
    ...
    for (CSSPropertyID property : pending_svg_resource_properties_) {
        ...
        case CSSPropertyID::kBackdropFilter:
            LoadResourcesForFilter(style.MutableBackdropFilter().Operations(), document);
            break;
...
void ElementStyleResources::LoadPendingImages(ComputedStyle& style)
{
    ...
    StyleImageLoader loader(element_.GetDocument(), style, pre_cached_container_sizes_, device_scale_factor_);
    for (CSSPropertyID property : pending_image_properties_) {
        switch (property) {
        case CSSPropertyID::kBackgroundImage: {
            for (FillLayer* background_layer = &style.AccessBackgroundLayers(); background_layer; background_layer = background_layer->Next()) {
                if (auto* pending_value = PendingCssValue(background_layer->GetImage())) {
                    ...
                    if (new_image && new_image->IsLazyloadPossiblyDeferred()) {
                        LazyImageHelper::StartMonitoring(pseudo_element_ ? pseudo_element_ : &element_);
                    }
        ...
        case CSSPropertyID::kContent: {
            for (ContentData* content_data = const_cast<ContentData*>(style.GetContentData()); content_data; content_data = content_data->Next()) {
        ...
        case CSSPropertyID::kWebkitMaskImage: {
            for (FillLayer* mask_layer = &style.AccessMaskLayers(); mask_layer; mask_layer = mask_layer->Next()) {
                if (auto* pending_value = PendingCssValue(mask_layer->GetImage())) {
                    mask_layer->SetImage(loader.Load(*pending_value, FetchParameters::ImageRequestBehavior::kNone, kCrossOriginAttributeAnonymous));
                }
            }
            break;
        }
...
void ElementStyleResources::LoadPendingResources(ComputedStyle& computed_style)
{
