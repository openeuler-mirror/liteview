...
const CascadePriority* CascadeMap::FindRevertLayer(const CSSPropertyName& name, CascadePriority revert_from) const
{
    auto find_revert_layer = [this](const CascadeMap::CascadePriorityList& list, CascadePriority revert_from) -> const CascadePriority* {
        for (auto iter = list.Begin(backing_vector_); iter != list.End(backing_vector_); ++iter) {
...
void CascadeMap::Add(const CSSPropertyName& name, CascadePriority priority)
{
    ...
    if (name.IsCustomProperty()) {
        ...
        static_assert(static_cast<int>(kLastHighPriorityCSSProperty) < 64, "CascadeMap supports at most 63 high-priority properties");
