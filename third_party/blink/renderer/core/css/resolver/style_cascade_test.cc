...
public:
    ...
    void Add(String block, CascadeOrigin origin = CascadeOrigin::kAuthor, unsigned link_match_type = CSSSelector::kMatchAll, bool is_inline_style = false)
    {
        CSSParserMode mode = origin == CascadeOrigin::kUserAgent ? kUASheetMode : kHTMLStandardMode;
        Add(ParseDeclarationBlock(block, mode), origin, link_match_type, is_inline_style);
    }
    ...
    void Add(const CSSPropertyValueSet* set, CascadeOrigin origin = CascadeOrigin::kAuthor, unsigned link_match_type = CSSSelector::kMatchAll,
        bool is_inline_style = false)
    {
        ...
        cascade_.MutableMatchResult().AddMatchedProperties(
            set, AddMatchedPropertiesOptions::Builder().SetLinkMatchType(link_match_type).SetIsInlineStyle(is_inline_style).Build());
    }
    ...
    String ComputedValue(String name) const
    {
        ...
        const CSSValue* value = ref.GetProperty().CSSValueFromComputedStyle(*state_.Style(), layout_object, allow_visited_style);
...
private:
    ...
    void FinishOrigin()
    {
        ...
        case CascadeOrigin::kAuthor:
            cascade_.MutableMatchResult().FinishAddingAuthorRulesForTreeScope(GetDocument());
    ...
    {
        CSSAnimations::CalculateTransitionUpdate(state_.AnimationUpdate(), state_.GetElement(), *state_.Style());
        CSSAnimations::CalculateAnimationUpdate(
            state_.AnimationUpdate(), state_.GetElement(), state_.GetElement(), *state_.Style(), state_.ParentStyle(), &GetDocument().GetStyleResolver());
    }
...
public:
    CSSStyleSheet* CreateSheet(const String& css_text)
    {
        ...
        sheet->Contents()->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasNoSpecialState);
        return sheet;
    }
    ...
    const MutableCSSPropertyValueSet* AnimationTaintedSet(AtomicString name, String value)
    {
        CSSParserMode mode = kHTMLStandardMode;
        auto* set = MakeGarbageCollected<MutableCSSPropertyValueSet>(mode);
        set->SetProperty(name, value, /* important */ false, SecureContextMode::kSecureContext,
...
TEST_F(StyleCascadeTest, AnimatedVisitedImportantOverride)
{
    ...
    cascade.Add(ParseDeclarationBlock("background-color:red !important"), CascadeOrigin::kAuthor, CSSSelector::kMatchVisited);
    cascade.Add("animation-name:test");
    cascade.Add("animation-duration:10s");
    cascade.Add("animation-timing-function:linear");
    cascade.Add("animation-delay:-5s");
...
TEST_F(StyleCascadeTest, VarInInternalVisitedColorShorthand)
{
    ...
    cascade.Add("outline:medium solid var(--x)", CascadeOrigin::kAuthor, CSSSelector::kMatchVisited);
    cascade.Add("outline-color:green", CascadeOrigin::kAuthor, CSSSelector::kMatchLink);
...
{
    Vector<String> properties = { "background-attachment", "background-blend-mode", "background-clip", "background-image", "background-origin",
...
TEST_F(StyleCascadeTest, MarkHasReferenceLonghand)
{
    ...
    EXPECT_TRUE(cascade.State().StyleRef().HasVariableReferenceFromNonInheritedProperty());
}
...
TEST_F(StyleCascadeTest, NoMarkHasReferenceForInherited)
{
    ...
    EXPECT_FALSE(cascade.State().StyleRef().HasVariableReferenceFromNonInheritedProperty());
}
