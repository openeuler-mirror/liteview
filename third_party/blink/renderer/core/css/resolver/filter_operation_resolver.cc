...
double FilterOperationResolver::ResolveNumericArgumentForFunction(const CSSFunctionValue& filter)
{
    ...
    for (auto& curr_value : To<CSSValueList>(in_value)) {
        ...
        case CSSValueID::kHueRotate: {
            operations.Operations().push_back(
                MakeGarbageCollected<BasicColorMatrixFilterOperation>(ResolveNumericArgumentForFunction(*filter_value), operation_type));
            break;
        }
        case CSSValueID::kInvert:
        case CSSValueID::kBrightness:
        ...
        case CSSValueID::kOpacity: {
            operations.Operations().push_back(
                MakeGarbageCollected<BasicComponentTransferFilterOperation>(ResolveNumericArgumentForFunction(*filter_value), operation_type));
            break;
        }
        case CSSValueID::kBlur: {
...
FilterOperations FilterOperationResolver::CreateOffscreenFilterOperations(const CSSValue& in_value, const Font& font)
{
    ...
    CSSToLengthConversionData conversion_data(
        nullptr /* element_style */, nullptr /* parent_style */, WritingMode::kHorizontalTb, font_sizes, viewport_size, container_sizes, 1 /* zoom */);
