...
public:
    ...
    scoped_refptr<ComputedStyle> ResolveStyle(Element*, const StyleRecalcContext&, const StyleRequest& = StyleRequest());
    ...
    scoped_refptr<const ComputedStyle> StyleForPage(uint32_t page_index, const AtomicString& page_name);
    ...
    scoped_refptr<ComputedStyle> CreateAnonymousStyleWithDisplay(const ComputedStyle& parent_style, EDisplay);

    // Create ComputedStyle for anonymous wrappers between text boxes and
    // display:contents elements.
    scoped_refptr<ComputedStyle> CreateInheritedDisplayContentsStyleIfNeeded(const ComputedStyle& parent_style, const ComputedStyle& layout_parent_style);
    ...
    StyleRuleKeyframes* FindKeyframesRule(const Element*, const Element* animating_element, const AtomicString& animation_name);
    ...
    RuleIndexList* PseudoCSSRulesForElement(Element*, PseudoId, const AtomicString& document_transition_tag, unsigned rules_to_include = kAllCSSRules);
    ...
    scoped_refptr<ComputedStyle> StyleForInterpolations(Element& element, ActiveInterpolationsMap& animations);
    ...
    scoped_refptr<const ComputedStyle> ResolvePositionFallbackStyle(Element&, unsigned index);
...
private:
    void InitStyleAndApplyInheritance(Element& element, const StyleRequest&, StyleResolverState& state);
    void ApplyInheritance(Element& element, const StyleRequest& style_request, StyleResolverState& state);
    ...
    void CollectPseudoRulesForElement(const Element&, ElementRuleCollector&, PseudoId, const AtomicString& document_transition_tag, unsigned rules_to_include);
    ...
    void MatchAuthorRules(const Element&, ScopedStyleResolver*, ElementRuleCollector&);
    ...
    public:
        ...
        CacheSuccess(bool is_inherited_cache_hit, bool is_non_inherited_cache_hit, MatchedPropertiesCache::Key key,
            const CachedMatchedProperties* cached_matched_properties)
    ...
    void MaybeAddToMatchedPropertiesCache(StyleResolverState&, const CacheSuccess&, const MatchResult&);

    void CascadeAndApplyMatchedProperties(StyleResolverState&, StyleCascade& cascade);
