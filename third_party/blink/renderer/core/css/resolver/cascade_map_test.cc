...
{
    return CascadePriority(CascadeOrigin::kUserAgent, false, 0, false, 0, position);
}
...
TEST(CascadeMapTest, ResetHighPrio)
{
    CascadeMap map;
    EXPECT_FALSE(map.HighPriorityBits());
    map.Add(CSSPropertyName(CSSPropertyID::kFontSize), CascadePriority(CascadeOrigin::kAuthor));
