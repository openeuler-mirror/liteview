...
void MatchResult::AddMatchedProperties(const CSSPropertyValueSet* properties, const AddMatchedPropertiesOptions& options)
{
    ...
    new_properties.types_.valid_property_filter = static_cast<std::underlying_type_t<ValidPropertyFilter>>(options.GetValidPropertyFilter());
    new_properties.types_.layer_order = ClampTo<uint16_t>(options.GetLayerOrder());
...
void MatchResult::FinishAddingAuthorRulesForTreeScope(const TreeScope& tree_scope)
{
