    ...
    , owner_color_scheme_(mojom::blink::ColorScheme::kLight)
{
    if (document.GetFrame()) {
        ...
        force_dark_mode_enabled_ = document.GetSettings()->GetForceDarkModeEnabled();
        UpdateColorSchemeMetrics();
    }

    forced_colors_ = WebThemeEngineHelper::GetNativeThemeEngine()->GetForcedColors();
...
void StyleEngine::AddedCustomElementDefaultStyles(const HeapVector<Member<CSSStyleSheet>>& default_styles)
{
    if (!RuntimeEnabledFeatures::CustomElementDefaultStyleEnabled() || GetDocument().IsDetached())
        return;
...
void StyleEngine::ComputeFont(Element& element, ComputedStyle* font_style, const CSSPropertyValueSet& font_properties)
{
    UpdateActiveStyle();
...
RuleSet* StyleEngine::RuleSetForSheet(CSSStyleSheet& sheet)
{
    ...
    if (document_->GetExecutionContext()->GetSecurityOrigin()->CanRequest(sheet.BaseURL())) {
        add_rule_flags = kRuleHasDocumentSecurityOrigin;
    }
    return &sheet.Contents()->EnsureRuleSet(*media_query_evaluator_, add_rule_flags);
}
...
void StyleEngine::ClearResolvers()
{
    ...
    if (resolver_) {
        TRACE_EVENT1("blink", "StyleEngine::clearResolver", "frame", ToTraceValue(GetDocument().GetFrame()));
        resolver_->Dispose();
        resolver_.Clear();
    }
}
...
void SetNeedsStyleRecalcForViewportUnits(TreeScope& tree_scope, unsigned dirty_flags)
{
    for (Element* element = ElementTraversal::FirstWithin(tree_scope.RootNode()); element; element = ElementTraversal::NextIncludingPseudo(*element)) {
        ...
        if (style && (style->ViewportUnitFlags() & dirty_flags)) {
            element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kViewportUnits));
        }
    }
}
...
void StyleEngine::InvalidateElementAffectedByHas(Element& element, bool for_element_affected_by_pseudo_in_has)
{
    ...
    if (element.AffectedBySubjectHas()) {
        ...
        element.SetNeedsStyleRecalc(StyleChangeType::kLocalStyleChange, StyleChangeReasonForTracing::Create(blink::style_change_reason::kStyleInvalidator));
    }

    if (element.AffectedByNonSubjectHas()) {
        InvalidationLists invalidation_lists;
        GetRuleFeatureSet().CollectInvalidationSetsForPseudoClass(invalidation_lists, element, CSSSelector::kPseudoHas);
...
public:
    ...
    static PseudoHasInvalidationTraversalContext ForAttributeOrPseudoStateChange(Element& changed_element)
    {
        ...
        Element* parent = traverse_ancestors ? changed_element.parentElement() : nullptr;
        ...
        return PseudoHasInvalidationTraversalContext(previous_sibling ? previous_sibling : parent, traverse_ancestors);
    }

    // Create :has() invalidation traversal context for element or subtree
    // insertion.
    static PseudoHasInvalidationTraversalContext ForInsertion(Element* parent, Element* previous_sibling)
    {
        bool traverse_ancestors = parent ? parent->AncestorsOrAncestorSiblingsAffectedByHas() : false;
    ...
    static PseudoHasInvalidationTraversalContext ForRemoval(Element* parent, Element* previous_sibling, Element& removed_element)
    {
    ...
    {
        return PseudoHasInvalidationTraversalContext(&parent, parent.AncestorsOrAncestorSiblingsAffectedByHas());
    }

private:
    PseudoHasInvalidationTraversalContext(Element* first_element, bool traverse_to_parent_of_first_element)
...
void StyleEngine::ClassChangedForElement(const SpaceSplitString& changed_classes, Element& element)
{
    ...
    if (RuntimeEnabledFeatures::CSSPseudoHasEnabled() && features.NeedsHasInvalidationForClassChange() && PossiblyAffectingHasState(element)) {
    ...
    unsigned changed_size = changed_classes.size();
    for (unsigned i = 0; i < changed_size; ++i) {
        features.CollectInvalidationSetsForClass(invalidation_lists, element, changed_classes[i]);
    }
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}
...
void StyleEngine::ClassChangedForElement(const SpaceSplitString& old_classes, const SpaceSplitString& new_classes, Element& element)
{
    ...
    bool possibly_affecting_has_state
        = RuntimeEnabledFeatures::CSSPseudoHasEnabled() && features.NeedsHasInvalidationForClassChange() && PossiblyAffectingHasState(element);
    ...
    for (unsigned i = 0; i < new_classes.size(); ++i) {
        ...
        if (!found) {
            if (LIKELY(needs_schedule_invalidation)) {
                features.CollectInvalidationSetsForClass(invalidation_lists, element, new_classes[i]);
            }
    ...
    for (unsigned i = 0; i < old_classes.size(); ++i) {
        ...
        // Class was removed.
        if (LIKELY(needs_schedule_invalidation)) {
            features.CollectInvalidationSetsForClass(invalidation_lists, element, old_classes[i]);
        }
        if (UNLIKELY(possibly_affecting_has_state)) {
...
void StyleEngine::AttributeChangedForElement(const QualifiedName& attribute_name, Element& element)
{
    ...
    if (RuntimeEnabledFeatures::CSSPseudoHasEnabled() && features.NeedsHasInvalidationForAttributeChange() && PossiblyAffectingHasState(element)) {
    ...
    InvalidationLists invalidation_lists;
    features.CollectInvalidationSetsForAttribute(invalidation_lists, element, attribute_name);
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);

    if (!element.NeedsStyleRecalc() && HasAttributeDependentGeneratedContent(element)) {
...
void StyleEngine::IdChangedForElement(const AtomicString& old_id, const AtomicString& new_id, Element& element)
{
    ...
    if (RuntimeEnabledFeatures::CSSPseudoHasEnabled() && features.NeedsHasInvalidationForIdChange() && PossiblyAffectingHasState(element)) {
        if ((!old_id.empty() && features.NeedsHasInvalidationForId(old_id)) || (!new_id.empty() && features.NeedsHasInvalidationForId(new_id))) {
            InvalidateChangedElementAffectedByLogicalCombinationsInHas(element, /* for_element_affected_by_pseudo_in_has */ false);
            InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForAttributeOrPseudoStateChange(element));
        }
    }
    ...
    CSSSelector::PseudoType pseudo_type, Element& element, bool invalidate_descendants_or_siblings, bool invalidate_ancestors_or_siblings)
{
    ...
    if (invalidate_ancestors_or_siblings && RuntimeEnabledFeatures::CSSPseudoHasEnabled() && features.NeedsHasInvalidationForPseudoStateChange()
    ...
    InvalidationLists invalidation_lists;
    features.CollectInvalidationSetsForPseudoClass(invalidation_lists, element, pseudo_type);
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}
...
void StyleEngine::ScheduleSiblingInvalidationsForElement(Element& element, ContainerNode& scheduling_parent, unsigned min_direct_adjacent)
{
    ...
    if (element.HasID()) {
        features.CollectSiblingInvalidationSetForId(invalidation_lists, element, element.IdForStyleResolution(), min_direct_adjacent);
    }

    if (element.HasClass()) {
        const SpaceSplitString& class_names = element.ClassNames();
        for (wtf_size_t i = 0; i < class_names.size(); i++) {
            features.CollectSiblingInvalidationSetForClass(invalidation_lists, element, class_names[i], min_direct_adjacent);
        }
    }

    for (const Attribute& attribute : element.Attributes()) {
        features.CollectSiblingInvalidationSetForAttribute(invalidation_lists, element, attribute.GetName(), min_direct_adjacent);
    }
...
void StyleEngine::ScheduleRuleSetInvalidationsForElement(Element& element, const HeapHashSet<Member<RuleSet>>& rule_sets)
{
    AtomicString id;
    ...
    for (const auto& rule_set : rule_sets) {
        if (!id.IsNull()) {
            rule_set->Features().CollectInvalidationSetsForId(invalidation_lists, element, id);
        }
        if (class_names) {
            wtf_size_t class_name_count = class_names->size();
            for (wtf_size_t i = 0; i < class_name_count; i++) {
                rule_set->Features().CollectInvalidationSetsForClass(invalidation_lists, element, (*class_names)[i]);
            }
        }
        for (const Attribute& attribute : element.Attributes()) {
            rule_set->Features().CollectInvalidationSetsForAttribute(invalidation_lists, element, attribute.GetName());
        }
    }
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}

void StyleEngine::ScheduleTypeRuleSetInvalidations(ContainerNode& node, const HeapHashSet<Member<RuleSet>>& rule_sets)
{
    InvalidationLists invalidation_lists;
    for (const auto& rule_set : rule_sets) {
        rule_set->Features().CollectTypeRuleInvalidationSet(invalidation_lists, node);
    }
    DCHECK(invalidation_lists.siblings.empty());
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, node);
    ...
    for (auto& invalidation_set : invalidation_lists.descendants) {
        if (invalidation_set->InvalidatesTagName(host)) {
            host.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kStyleSheetChange));
            return;
        }
    }
}
...
void StyleEngine::ScheduleInvalidationsForHasPseudoAffectedByInsertion(Element* parent, Node* node_before_change, Element& inserted_element)
{
    ...
    bool needs_has_invalidation_for_inserted_subtree = parent->ChildrenAffectedByDirectAdjacentRules();

    if (!needs_has_invalidation_for_inserted_subtree && features.NeedsHasInvalidationForInsertedOrRemovedElement(inserted_element)) {
    ...
    if (descendants_possibly_affecting_has_state) {
        ...
        for (Element& element : ElementTraversal::DescendantsOf(inserted_element)) {
            ...
            if (!needs_has_invalidation_for_inserted_subtree && features.NeedsHasInvalidationForInsertedOrRemovedElement(element)) {
                needs_has_invalidation_for_inserted_subtree = true;
            }
        }
    }
    ...
    if (needs_has_invalidation_for_inserted_subtree) {
        InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForInsertion(parent, previous_sibling));
        return;
    }

    if (features.NeedsHasInvalidationForPseudoStateChange()) {
        InvalidateAncestorsOrSiblingsAffectedByHas(
            PseudoHasInvalidationTraversalContext::ForInsertion(parent, previous_sibling).SetForElementAffectedByPseudoInHas());
    }
}

void StyleEngine::ScheduleInvalidationsForHasPseudoAffectedByRemoval(Element* parent, Node* node_before_change, Element& removed_element)
{
    if (!RuntimeEnabledFeatures::CSSPseudoHasEnabled() || !parent)
        return;
    ...
    if (parent->ChildrenAffectedByDirectAdjacentRules()) {
        InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForRemoval(parent, previous_sibling, removed_element));
        return;
    }
    ...
    if (features.NeedsHasInvalidationForPseudoStateChange()) {
        InvalidateAncestorsOrSiblingsAffectedByHas(
            PseudoHasInvalidationTraversalContext::ForRemoval(parent, previous_sibling, removed_element).SetForElementAffectedByPseudoInHas());
    }
}
...
void StyleEngine::InvalidateSlottedElements(HTMLSlotElement& slot)
{
    for (auto& node : slot.FlattenedAssignedNodes()) {
        if (node->IsElementNode()) {
            node->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kStyleSheetChange));
        }
    }
}
...
void StyleEngine::ScheduleInvalidationsForRuleSets(TreeScope& tree_scope, const HeapHashSet<Member<RuleSet>>& rule_sets, InvalidationScope invalidation_scope)
{
#if DCHECK_IS_ON()
    ...
    while (element) {
        ...
        if (invalidation_scope == kInvalidateAllScopes) {
            if (ShadowRoot* shadow_root = element->GetShadowRoot()) {
                ScheduleInvalidationsForRuleSets(*shadow_root, rule_sets, kInvalidateAllScopes);
            }
        }

        if (element->GetStyleChangeType() < kSubtreeStyleChange && element->GetComputedStyle()) {
...
void StyleEngine::EnsureUAStyleForTransitionPseudos()
{
    ...
    // sheet.
    auto* document_transition = DocumentTransitionSupplement::From(GetDocument())->GetTransition();
    auto* style_sheet_contents = CSSDefaultStyleSheets::ParseUASheet(document_transition->UAStyleSheet());
    ua_document_transition_style_ = MakeGarbageCollected<RuleSet>();
    ua_document_transition_style_->AddRulesFromSheet(style_sheet_contents, CSSDefaultStyleSheets::ScreenEval());
}
    ...
    TreeScope& tree_scope, const HeapHashSet<Member<RuleSet>>& changed_rule_sets, unsigned changed_rule_flags, InvalidationScope invalidation_scope)
{
    ...
    if (changed_rule_flags & kFullRecalcRules) {
        invalidation_root.SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kActiveStylesheetsUpdate));
        return;
    }

    ScheduleInvalidationsForRuleSets(tree_scope, changed_rule_sets, invalidation_scope);
}
...
void StyleEngine::ApplyUserRuleSetChanges(const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets)
{
    ...
    ActiveSheetsChange change = CompareActiveStyleSheets(old_style_sheets, new_style_sheets, changed_rule_sets);
    ...
    if (changed_rule_flags & kKeyframesRules) {
        ...
        for (auto* it = new_style_sheets.begin(); it != new_style_sheets.end(); it++) {
            DCHECK(it->second);
...
void StyleEngine::ApplyRuleSetChanges(TreeScope& tree_scope, const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets)
{
    ...
    if (tree_scope.RootNode().IsDocumentNode()) {
        ...
        if ((changed_rule_flags & kFontFaceRules) || (changed_rule_flags & kFontPaletteValuesRules) || has_rebuilt_font_face_cache) {
            GetFontSelector()->FontFaceInvalidated(FontInvalidationReason::kGeneralInvalidation);
        }
    }
...
void StyleEngine::ApplyVisionDeficiencyStyle(scoped_refptr<ComputedStyle> layout_view_style)
{
...
bool StyleEngine::StyleMaybeAffectedByLayout(const Node& node)
{
    ...
    return StyleAffectedByLayout() || ComputedStyle::IsNullOrEnsured(node.GetComputedStyle());
}

bool StyleEngine::UpdateRemUnits(const ComputedStyle* old_root_style, const ComputedStyle* new_root_style)
{
    if (!new_root_style || !UsesRemUnits())
        return false;
    if (!old_root_style || old_root_style->SpecifiedFontSize() != new_root_style->SpecifiedFontSize()) {
...
void StyleEngine::AddFontPaletteValuesRules(const RuleSet& rule_set)
{
    ...
    for (auto& rule : font_palette_values_rules) {
        ...
        font_palette_values_rule_map_.Set(std::make_pair(rule->GetName(), String(rule->GetFontFamilyAsString()).FoldCase()), rule);
    }
}
...
{
    if (!environment_variables_) {
        environment_variables_ = DocumentStyleEnvironmentVariables::Create(StyleEnvironmentVariables::GetRootInstance(), *document_);
    }
...
void StyleEngine::RecalcStyleForNonLayoutNGContainerDescendants(Element& container)
{
...
void StyleEngine::UpdateStyleAndLayoutTreeForContainer(Element& container, const LogicalSize& logical_size, LogicalAxes contained_axes)
{
    ...
    ContainerQueryEvaluator::Change query_change = evaluator->SizeContainerChanged(GetDocument(), container, physical_size, physical_axes);
    ...
    GetDocument().GetLayoutView()->UpdateMarkersAndCountersAfterStyleChange(container.GetLayoutObject());
}
...
void StyleEngine::RecalcStyle(StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    ...
    StyleRecalcChange sibling_change = root_element.RecalcStyle(change, style_recalc_context);

    if (sibling_change.RecalcSiblingDescendants()) {
        root_element.RecalcSubsequentSiblingStyles(change.Combine(sibling_change), style_recalc_context);
    }
...
void StyleEngine::RecalcTransitionPseudoStyle()
{
    ...
    document_->documentElement()->RecalcTransitionPseudoTreeStyle(document_transition_tags_);
}
...
void StyleEngine::RebuildLayoutTree(RebuildTransitionPseudoTree rebuild_transition_pseudo_tree)
{
    bool propagate_to_root = false;
    {
        ...
        if (rebuild_transition_pseudo_tree == RebuildTransitionPseudoTree::kYes) {
            document_->documentElement()->RebuildTransitionPseudoLayoutTree(document_transition_tags_);
        }
        layout_tree_rebuild_root_.Clear();
...
void StyleEngine::ReattachContainerSubtree(Element& container)
{
    ...
    DCHECK(DynamicTo<HTMLFieldSetElement>(container) || container.ShouldForceLegacyLayout());
...
void StyleEngine::UpdateColorScheme()
{
    ...
    if (const auto* overrides = GetDocument().GetPage()->GetMediaFeatureOverrides()) {
        if (absl::optional<ForcedColors> forced_color_override = overrides->GetForcedColors()) {
            forced_colors_ = forced_color_override.value();
        }
        if (absl::optional<mojom::blink::PreferredColorScheme> preferred_color_scheme_override = overrides->GetPreferredColorScheme()) {
...
{
    forced_background_color_ = LayoutTheme::GetTheme().SystemColor(CSSValueID::kCanvas, mojom::blink::ColorScheme::kLight);
}
...
void StyleEngine::UpdateViewportStyle()
{
    ...
    if (ComputedStyle::ComputeDifference(viewport_style.get(), GetDocument().GetLayoutView()->Style()) != ComputedStyle::Difference::kEqual) {
...
{
    return NeedsActiveStyleUpdate() || IsViewportStyleDirty() || viewport_unit_dirty_flags_;
}
...
void StyleEngine::ChangeRenderingForHTMLSelect(HTMLSelectElement& select)
{
    ...
    select.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kControl));
}
...
void StyleEngine::ReportUseOfLegacyLayoutWithContainerQueries()
{
    ...
    ConsoleMessage* console_message
        = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
            String::Format("Using container queries or units with printing, or in combination "
