...
bool RemoteFontFaceSource::NeedsInterventionToAlignWithLCPGoal() const
{
    ...
    if (!base::FeatureList::IsEnabled(features::kAlignFontDisplayAutoTimeoutWithLCPGoal)) {
        return false;
    }
...
RemoteFontFaceSource::DisplayPeriod RemoteFontFaceSource::ComputeFontDisplayAutoPeriod() const
{
    ...
    if (NeedsInterventionToAlignWithLCPGoal()) {
        ...
        Mode mode = features::kAlignFontDisplayAutoTimeoutWithLCPGoalModeParam.Get();
...
RemoteFontFaceSource::RemoteFontFaceSource(
    ...
    display_(GetFontDisplayWithDocumentPolicyCheck(display, font_selector, ReportOptions::kDoNotReport))
...
void RemoteFontFaceSource::NotifyFinished(Resource* resource)
{
    ...
    if (!resource->IsLinkPreload() || resource->IntegrityDisposition() != ResourceIntegrityDisposition::kFailed) {
...
void RemoteFontFaceSource::SetDisplay(FontDisplay display)
{
    ...
    if (IsLoaded())
        return;
    display_ = GetFontDisplayWithDocumentPolicyCheck(display, font_selector_, ReportOptions::kReportOnFailure);
    UpdatePeriod();
}
...
FontDisplay RemoteFontFaceSource::GetFontDisplayWithDocumentPolicyCheck(
    FontDisplay display, const FontSelector* font_selector, ReportOptions report_option) const
{
    ExecutionContext* context = font_selector->GetExecutionContext();
    if (display != FontDisplay::kFallback && display != FontDisplay::kOptional && context && context->IsWindow()
        && !context->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kFontDisplay, report_option)) {
    ...
    const FontDescription& font_description, const FontSelectionCapabilities& font_selection_capabilities)
{
    ...
    return SimpleFontData::Create(
        custom_font_data_->GetFontPlatformData(font_description.EffectiveFontSize(), font_description.AdjustedSpecifiedSize(),
            ...
            font_selection_capabilities, font_description.FontOpticalSizing(), font_description.TextRendering(), font_description.Orientation(),
            font_description.VariationSettings(), font_description.GetFontPalette()),
        CustomFontData::Create());
}

scoped_refptr<SimpleFontData> RemoteFontFaceSource::CreateLoadingFallbackFontData(const FontDescription& font_description)
{
    // This temporary font is not retained and should not be returned.
    FontCachePurgePreventer font_cache_purge_preventer;
    scoped_refptr<SimpleFontData> temporary_font = FontCache::Get().GetLastResortFallbackFont(font_description, kDoNotRetain);
    if (!temporary_font) {
        NOTREACHED();
        return nullptr;
    }
    scoped_refptr<CSSCustomFontData> css_font_data
        = CSSCustomFontData::Create(this, period_ == kBlockPeriod ? CSSCustomFontData::kInvisibleFallback : CSSCustomFontData::kVisibleFallback);
...
void RemoteFontFaceSource::BeginLoadIfNeeded()
{
    ...
    if (!execution_context->IsContextThread()) {
        ...
        PostCrossThreadTask(*task_runner_, FROM_HERE, CrossThreadBindOnce(&RemoteFontFaceSource::BeginLoadIfNeeded, WrapCrossThreadPersistent(this)));
        return;
    }
#endif
