...
protected:
    CSSStyleSheet* CreateSheet(const String& css_text = String())
    {
        ...
        contents->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasDocumentSecurityOrigin);
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_Mutated)
{
    ...
    sheet2->Contents()->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()), kRuleHasDocumentSecurityOrigin);
...
TEST_F(ApplyRulesetsTest, AddUniversalRuleToDocument)
{
    ...
    EXPECT_EQ(kSubtreeStyleChange, GetDocument().documentElement()->GetStyleChangeType());
}
