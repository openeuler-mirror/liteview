...
{
    for (auto* container = &element; container && container != stay_within; container = container->ParentOrShadowHostElement()) {
...
StyleRecalcContext StyleRecalcContext::ForPartRules(Element& host) const
{
    ...
    if (Element* host_container = ClosestInclusiveAncestorContainer(host, container)) {
