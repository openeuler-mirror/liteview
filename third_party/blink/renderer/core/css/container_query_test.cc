...
class ContainerQueryTest : public PageTestBase, private ScopedCSSContainerQueriesForTest, private ScopedLayoutNGForTest {
public:
    ContainerQueryTest()
    ...
    absl::optional<MediaQueryExpNode::FeatureFlags> FeatureFlagsFrom(String query_string)
    {
...
TEST_F(ContainerQueryTest, PreludeParsing)
{
    ...
    EXPECT_EQ("not (width)", SerializeCondition(ParseAtContainer("@container somename not (width) {}")));
...
TEST_F(ContainerQueryTest, OldStyleForTransitions)
{
    ...
    // Simulate a style and layout pass with multiple rounds of style recalc.
    {
        ...
        GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(120, -1), kLogicalAxisInline);
        EXPECT_EQ("15px", ComputedValueString(target, "height"));
        EXPECT_EQ(0u, GetAnimationsCount(target));

        // Should transition between [10px, 30px]. (Intermediate round).
        GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(130, -1), kLogicalAxisInline);
...
TEST_F(ContainerQueryTest, CQDependentContentVisibilityHidden)
{
    ...
    EXPECT_TRUE(locker->firstChild()->GetComputedStyle()) << "The #locker element does not get content-visibility:hidden on the "
...
TEST_F(ContainerQueryTest, NoContainerQueryEvaluatorWhenDisabled)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(GetDocument().getElementById("container")->GetContainerQueryEvaluator());
}
...
TEST_F(ContainerQueryTest, NoStyleQueryWhenDisabled)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    const ComputedStyle& style = GetDocument().getElementById("span")->ComputedStyleRef();
    EXPECT_EQ(style.Display(), EDisplay::kInline) << "style() should not match when disabled";
    EXPECT_EQ(style.VerticalAlign(), EVerticalAlign::kBaseline) << "style() should be unknown when disabled";
}
