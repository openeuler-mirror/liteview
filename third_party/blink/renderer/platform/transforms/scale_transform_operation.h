...
public:
    static scoped_refptr<ScaleTransformOperation> Create(double sx, double sy, OperationType type)
    {
        return base::AdoptRef(new ScaleTransformOperation(sx, sy, 1, type));
    }

    static scoped_refptr<ScaleTransformOperation> Create(double sx, double sy, double sz, OperationType type)
    {
    ...
    void Apply(TransformationMatrix& transform, const gfx::SizeF&) const override
    {
