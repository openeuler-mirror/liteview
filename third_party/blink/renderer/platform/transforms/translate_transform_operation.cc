...
Length AddLengths(const Length& lhs, const Length& rhs)
{
    ...
    PixelsAndPercent result = PixelsAndPercent(lhs_pap.pixels + rhs_pap.pixels, lhs_pap.percent + rhs_pap.percent);
    if (result.percent == 0)
...
scoped_refptr<TransformOperation> TranslateTransformOperation::Accumulate(const TransformOperation& other)
{
    ...
    return TranslateTransformOperation::Create(new_x, new_y, new_z, GetTypeForTranslate(new_x, new_y, new_z));
}

scoped_refptr<TransformOperation> TranslateTransformOperation::Blend(const TransformOperation* from, double progress, bool blend_to_identity)
{
    DCHECK(!from || CanBlendWith(*from));

    const Length zero_length = Length::Fixed(0);
    if (blend_to_identity) {
        return TranslateTransformOperation::Create(zero_length.Blend(x_, progress, Length::ValueRange::kAll),
...
scoped_refptr<TranslateTransformOperation> TranslateTransformOperation::ZoomTranslate(double factor)
{
