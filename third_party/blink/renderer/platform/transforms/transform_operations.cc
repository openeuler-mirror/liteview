...
namespace {
using ApplyCallback
    = base::RepeatingCallback<scoped_refptr<TransformOperation>(const scoped_refptr<TransformOperation>& from, const scoped_refptr<TransformOperation>& to)>;
    ...
    ApplyCallback apply_cb, const TransformOperations& from, const TransformOperations& to, wtf_size_t matching_prefix_length, bool* success)
{
    ...
    for (wtf_size_t i = 0; i < matching_prefix_length; i++) {
        scoped_refptr<TransformOperation> from_operation = (i < from_size) ? from.Operations()[i].get() : nullptr;
        scoped_refptr<TransformOperation> to_operation = (i < to_size) ? to.Operations()[i].get() : nullptr;

        scoped_refptr<TransformOperation> result_operation = apply_cb.Run(from_operation, to_operation);
...
void TransformOperations::ApplyRemaining(const gfx::SizeF& border_box_size, wtf_size_t start, TransformationMatrix& t) const
{
...
scoped_refptr<TransformOperation> TransformOperations::BlendRemainingByUsingMatrixInterpolation(
    const TransformOperations& from, wtf_size_t matching_prefix_length, double progress) const
{
    ...
    if (BoxSizeDependencies(matching_prefix_length) || from.BoxSizeDependencies(matching_prefix_length)) {
        return InterpolatedTransformOperation::Create(from, *this, matching_prefix_length, progress);
    }
...
TransformOperations TransformOperations::Blend(const TransformOperations& from, double progress) const
{
    ...
    TransformOperations result
        = ApplyFunctionToMatchingPrefix(WTF::BindRepeating(
                                            [](double progress, const scoped_refptr<TransformOperation>& from, const scoped_refptr<TransformOperation>& to) {
                                                ...
                                                return to ? to->Blend(from.get(), progress) : (from ? from->Blend(nullptr, progress, true) : nullptr);
                                            },
                                            progress),
            from, *this, matching_prefix_length, &success);
    if (success && matching_prefix_length < max_path_length) {
        scoped_refptr<TransformOperation> matrix_op = BlendRemainingByUsingMatrixInterpolation(from, matching_prefix_length, progress);
        if (matrix_op)
...
TransformOperations TransformOperations::Accumulate(const TransformOperations& to) const
{
    ...
    TransformOperations result
        = ApplyFunctionToMatchingPrefix(WTF::BindRepeating([](const scoped_refptr<TransformOperation>& from, const scoped_refptr<TransformOperation>& to) {
              if (to && from)
    ...
    if (success && matching_prefix_length < max_path_length) {
        ...
        scoped_refptr<TransformOperation> from_matrix = Matrix3DTransformOperation::Create(from_transform);
        scoped_refptr<TransformOperation> to_matrix = Matrix3DTransformOperation::Create(to_transform);
        scoped_refptr<TransformOperation> matrix_op = from_matrix->Accumulate(*to_matrix);
    ...
    const gfx::BoxF& box, const TransformOperations& from, const double& min_progress, const double& max_progress, gfx::BoxF* bounds) const
{
    ...
    *bounds = box;
    for (int i = size - 1; i >= 0; i--) {
        scoped_refptr<TransformOperation> from_operation = (i < from_size) ? from.Operations()[i] : nullptr;
        scoped_refptr<TransformOperation> to_operation = (i < to_size) ? Operations()[i] : nullptr;
        ...
        if (from_operation && to_operation && !from_operation->CanBlendWith(*to_operation.get()))
            return false;
        ...
        case TransformOperation::kPerspective: {
            ...
            if (!to_operation) {
                from_transform = from_operation->Blend(to_operation.get(), 1 - min_progress, false);
                to_transform = from_operation->Blend(to_operation.get(), 1 - max_progress, false);
            } else {
                from_transform = to_operation->Blend(from_operation.get(), min_progress, false);
                to_transform = to_operation->Blend(from_operation.get(), max_progress, false);
            }
        ...
        case TransformOperation::kRotateZ: {
            ...
            if (from_operation) {
                from_rotation = static_cast<const RotateTransformOperation*>(from_operation.get());
                if (from_rotation->Axis().IsZero())
                    from_rotation = nullptr;
            }

            if (to_operation) {
                to_rotation = static_cast<const RotateTransformOperation*>(to_operation.get());
            ...
            if (!from_rotation) {
                identity_rotation
                    = RotateTransformOperation::Create(axis.x(), axis.y(), axis.z(), 0, from_operation ? from_operation->GetType() : to_operation->GetType());
