...
scoped_refptr<TransformOperation> MatrixTransformOperation::Accumulate(const TransformOperation& other_op)
{
    ...
    if (!other.matrix_.Decompose2D(from_decomp) || !matrix_.Decompose2D(to_decomp)) {
        return nullptr;
    }
...
scoped_refptr<TransformOperation> MatrixTransformOperation::Blend(const TransformOperation* from, double progress, bool blend_to_identity)
{
...
scoped_refptr<TransformOperation> MatrixTransformOperation::Zoom(double factor)
{
