...
HTTPHeaderSet ExtractCorsExposedHeaderNamesList(network::mojom::CredentialsMode credentials_mode, const ResourceResponse& response)
{
    ...
    if (credentials_mode != network::mojom::CredentialsMode::kInclude && header_set.find("*") != header_set.end()) {
        header_set.clear();
...
bool IsCorsSafelistedResponseHeader(const String& name)
{
    ...
    return allowed_cross_origin_response_headers.find(name.Ascii()) != allowed_cross_origin_response_headers.end();
}
