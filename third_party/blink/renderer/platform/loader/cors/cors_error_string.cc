...
String GetErrorString(const network::CorsErrorStatus& status, const KURL& initial_request_url, const KURL& last_request_url, const SecurityOrigin& origin,
    ResourceType resource_type, const AtomicString& initiator_name)
{
    ...
    case CorsError::kPreflightAllowOriginMismatch:
        Append(builder, { "The 'Access-Control-Allow-Origin' header has a value '", hint, "' that is not equal to the supplied origin." });
        if (initiator_name == fetch_initiator_type_names::kFetch)
            builder.Append(kNoCorsInformation);
        break;
    ...
    case CorsError::kPreflightInvalidAllowPrivateNetwork:
        Append(builder,
            { "The 'Access-Control-Allow-Private-Network' header in the "
              ...
              "the `",
                ShortAddressSpace(status.target_address_space), "` address space had a value of '", hint, "',  not 'true'." });
        break;
