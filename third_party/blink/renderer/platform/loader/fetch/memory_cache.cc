...
static const base::TimeDelta kCMinDelayBeforeLiveDecodedPrune = base::Seconds(1);
...
void MemoryCache::PruneResources(PruneStrategy strategy)
{
    ...
    size_t target_size = static_cast<size_t>(size_limit * kCTargetPrunePercentage);
    ...
    for (const auto& resource_map_iter : resource_maps_) {
        for (const auto& resource_iter : *resource_map_iter.value) {
            ...
            if (resource->IsLoaded() && resource->DecodedSize()) {
                ...
                if (strategy == kAutomaticPrune && prune_frame_time_stamp_.since_origin() < delay_before_live_decoded_prune_)
                    continue;
                resource->Prune();
...
void MemoryCache::Prune()
{
    ...
    if (prune_pending_) {
        ...
        if (current_time - prune_time_stamp_ >= kCMaxPruneDeferralDelay) {
            ...
            // Defer.
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&MemoryCache::PruneNow, base::Unretained(this), kAutomaticPrune));
            prune_pending_ = true;
        }
    }
}
