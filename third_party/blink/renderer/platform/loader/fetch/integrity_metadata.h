...
enum class ResourceIntegrityDisposition : uint8_t { kNotChecked = 0, kFailed, kPassed };
...
template <> struct DefaultHash<blink::IntegrityAlgorithm> {
    STATIC_ONLY(DefaultHash);
    typedef IntHash<blink::IntegrityAlgorithm> Hash;
};

template <> struct HashTraits<blink::IntegrityAlgorithm> : UnsignedWithZeroKeyHashTraits<blink::IntegrityAlgorithm> {
