    ...
    ResourceLoadSchedulerClient* client, ThrottleOption option, ResourceLoadPriority priority, int intra_priority, ResourceLoadScheduler::ClientId* id)
{
    *id = GenerateClientId();
    ...
    // operation is active.
    if (0 == pending_batch_operations_ || !RuntimeEnabledFeatures::BatchFetchRequestsEnabled()) {
        MaybeRun();
    }
}
...
void ResourceLoadScheduler::SetOutstandingLimitForTesting(size_t tight_limit, size_t normal_limit)
{
...
bool ResourceLoadScheduler::IsPendingRequestEffectivelyEmpty(ThrottleOption option)
{
    for (const auto& client : pending_requests_[option]) {
        ...
        if (pending_request_map_.find(client.client_id) != pending_request_map_.end()) {
            return false;
        }
    }
...
bool ResourceLoadScheduler::GetNextPendingRequest(ClientId* id)
{
    ...
    bool has_runnable_stoppable_request = stoppable_it != stoppable_queue.end()
        && (!IsClientDelayable(ThrottleOption::kStoppable)
            || IsRunningThrottleableRequestsLessThanOutStandingLimit(GetOutstandingLimit(stoppable_it->priority)));
    ...
    bool has_runnable_throttleable_request = throttleable_it != throttleable_queue.end()
        && (!IsClientDelayable(ThrottleOption::kThrottleable)
            || IsRunningThrottleableRequestsLessThanOutStandingLimit(GetOutstandingLimit(throttleable_it->priority)));
...
void ResourceLoadScheduler::MaybeRun()
{
    ...
    if (policy_ == ThrottlingPolicy::kTight) {
        http_rtt_ = http_rtt_for_testing_ ? http_rtt_for_testing_ : GetNetworkStateNotifier().HttpRtt();
    }
...
void ResourceLoadScheduler::Run(ResourceLoadScheduler::ClientId id, ResourceLoadSchedulerClient* client, bool throttleable)
{
    ...
    case ThrottlingPolicy::kTight:
        if (priority < ResourceLoadPriority::kHigh) {
            if (CanRequestForMultiplexedConnectionsInTight()) {
                policy_limit = static_cast<size_t>(features::kMaxNumOfThrottleableRequestsInTightMode.Get());
            } else {
...
bool ResourceLoadScheduler::IsRunningThrottleableRequestsLessThanOutStandingLimit(size_t out_standing_limit)
{
    if (CanRequestForMultiplexedConnectionsInTight()) {
        DCHECK_EQ(policy_, ThrottlingPolicy::kTight);
        return (running_throttleable_requests_.size() - in_flight_on_multiplexed_connections_ * features::kCostReductionOfMultiplexedRequests.Get())
            < out_standing_limit;
    }
...
void ResourceLoadScheduler::SetConnectionInfo(ClientId id, net::HttpResponseInfo::ConnectionInfo connection_info)
{
...
bool ResourceLoadScheduler::CanRequestForMultiplexedConnectionsInTight() const
{
    ...
    return base::FeatureList::IsEnabled(features::kDelayLowPriorityRequestsAccordingToNetworkState) && policy_ == ThrottlingPolicy::kTight && http_rtt_
