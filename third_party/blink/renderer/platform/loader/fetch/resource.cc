...
const char* const kHeaderPrefixesToIgnoreAfterRevalidation[] = { "content-", "x-content-", "x-webkit-" };
...
static inline bool ShouldUpdateHeaderAfterRevalidation(const AtomicString& header)
{
    ...
    for (size_t i = 0; i < std::size(kHeaderPrefixesToIgnoreAfterRevalidation); i++) {
        if (header.StartsWithIgnoringASCIICase(kHeaderPrefixesToIgnoreAfterRevalidation[i]))
            return false;
    }
    return true;
}
...
void Resource::CheckResourceIntegrity()
{
    ...
    if (SubresourceIntegrity::CheckSubresourceIntegrity(IntegrityMetadata(), data, data_length, Url(), *this, integrity_report_info_)) {
        integrity_disposition_ = ResourceIntegrityDisposition::kPassed;
    } else {
        integrity_disposition_ = ResourceIntegrityDisposition::kFailed;
    }
...
static base::TimeDelta CurrentAge(const ResourceResponse& response, base::Time response_timestamp)
{
    // RFC2616 13.2.3
...
static base::TimeDelta FreshnessLifetime(const ResourceResponse& response, base::Time response_timestamp)
{
...
static bool CanUseResponse(const ResourceResponse& response, bool allow_stale, base::Time response_timestamp)
{
    if (response.IsNull())
        return false;
...
Resource::MatchStatus Resource::CanReuse(const FetchParameters& params) const
{
    ...
    if (current_request.AllowStoredCredentials() != new_request.AllowStoredCredentials()) {
...
bool Resource::CanReuseRedirectChain() const
{
    for (auto& redirect : redirect_chain_) {
        if (!CanUseResponse(redirect.redirect_response_, false /*allow_stale*/, response_timestamp_))
            return false;
...
{
    return !CanUseResponse(GetResponse(), allow_stale, response_timestamp_) || GetResourceRequest().CacheControlContainsNoCache()
        || GetResourceRequest().CacheControlContainsNoStore();
}

static bool ShouldRevalidateStaleResponse(const ResourceResponse& response, base::Time response_timestamp)
{
    base::TimeDelta staleness = response.CacheControlStaleWhileRevalidate();
    if (staleness.is_zero())
        return false;

    return CurrentAge(response, response_timestamp) > FreshnessLifetime(response, response_timestamp);
}
...
bool Resource::ShouldRevalidateStaleResponse() const
{
    for (auto& redirect : redirect_chain_) {
        ...
        if (blink::ShouldRevalidateStaleResponse(redirect.redirect_response_, response_timestamp_)) {
            return true;
        }
    }

    return blink::ShouldRevalidateStaleResponse(GetResponse(), response_timestamp_);
}
...
static const char* InitiatorTypeNameToString(const AtomicString& initiator_type_name)
{
    ...
    static_assert(fetch_initiator_type_names::kNamesCount == 18, "New FetchInitiatorTypeNames should be handled correctly here.");
