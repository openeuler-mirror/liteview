...
// Document.
class PLATFORM_EXPORT ResourceFetcher : public GarbageCollected<ResourceFetcher> {
...
public:
    ...
    public:
        ...
        virtual std::unique_ptr<WebURLLoader> CreateURLLoader(const ResourceRequest&, const ResourceLoaderOptions&,
    ...
    std::unique_ptr<WebURLLoader> CreateURLLoader(const ResourceRequestHead&, const ResourceLoaderOptions&);
    ...
    static void AddPriorityObserverForTesting(const KURL&, base::OnceCallback<void(int)>);
    ...
    void HandleLoaderFinish(Resource*, base::TimeTicks finish_time, LoaderFinishType, uint32_t inflight_keepalive_bytes, bool should_report_corb_blocking);
    ...
    absl::optional<base::UnguessableToken> GetSubresourceBundleToken(const KURL& url) const;
    ...
    void EmulateLoadStartedForInspector(
        Resource*, const KURL&, mojom::blink::RequestContextType, network::mojom::RequestDestination, const AtomicString& initiator_name);
    ...
    ResourceLoadPriority ComputeLoadPriorityForTesting(ResourceType type, const ResourceRequestHead& request,
        ...
        FetchParameters::SpeculativePreloadType speculative_preload_type, RenderBlockingBehavior render_blocking_behavior, bool is_link_preload)
    {
        return ComputeLoadPriority(type, request, visibility_statue, defer_option, speculative_preload_type, render_blocking_behavior, is_link_preload);
    }
...
private:
    ...
    scoped_refptr<const SecurityOrigin> GetSourceOrigin(const ResourceLoaderOptions&) const;
    ...
    ResourceLoadPriority ComputeLoadPriority(ResourceType, const ResourceRequestHead&, ResourcePriority::VisibilityStatus,
        ...
        RenderBlockingBehavior = RenderBlockingBehavior::kNonBlocking, bool is_link_preload = false);

    // |virtual_time_pauser| is an output parameter. PrepareRequest may
    // create a new WebScopedVirtualTimePauser and set it to
    // |virtual_time_pauser|.
    absl::optional<ResourceRequestBlockedReason> PrepareRequest(FetchParameters&, const ResourceFactory&, WebScopedVirtualTimePauser& virtual_time_pauser);
    ...
    enum class RevalidationPolicy { kUse, kRevalidate, kReload, kLoad, kMaxValue = kLoad };
    ...
    void UpdateMemoryCacheStats(
        Resource*, RevalidationPolicy, const FetchParameters&, const ResourceFactory&, bool is_static_data, bool same_top_frame_site_resource_cached) const;
    ...
    void PopulateAndAddResourceTimingInfo(Resource* resource, scoped_refptr<ResourceTimingInfo> info, base::TimeTicks response_end);
    ...
    using ResourceTimingInfoMap = HeapHashMap<Member<Resource>, scoped_refptr<ResourceTimingInfo>>;
