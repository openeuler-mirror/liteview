...
using ProcessCodeCacheRequestCallback = base::RepeatingCallback<void(WebCodeCacheLoader::FetchCodeCacheCallback)>;
...
public:
    ...
    void FetchFromCodeCache(blink::mojom::CodeCacheType cache_type, const WebURL& url, WebCodeCacheLoader::FetchCodeCacheCallback callback) override
    {
        process_request_.Run(std::move(callback));
    }

    void ClearCodeCacheEntry(blink::mojom::CodeCacheType cache_type, const WebURL& url) override
    {
    }
...
public:
    ...
    void LoadSynchronously(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<WebURLRequestExtraData> url_request_extra_data,
        ...
        absl::optional<WebURLError>&, WebData&, int64_t& encoded_data_length, int64_t& encoded_body_length, WebBlobInfo& downloaded_blob,
    ...
    void LoadAsynchronously(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<WebURLRequestExtraData> url_request_extra_data,
        bool no_mime_sniffing, std::unique_ptr<blink::ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper, WebURLLoaderClient*) override
    {
    }
...
public:
    DeferTestLoaderFactory(WebLoaderFreezeMode* const freeze_mode_ptr, ProcessCodeCacheRequestCallback process_code_cache_request_callback)
        : freeze_mode_ptr_(freeze_mode_ptr)
        , process_code_cache_request_callback_(process_code_cache_request_callback)
    {
    }
    ...
    {
        return std::make_unique<TestCodeCacheLoader>(process_code_cache_request_callback_);
    }
...
public:
    ...
    {
        SetCodeCacheProcessFunction(base::BindRepeating(&ResourceLoaderDefersLoadingTest::SaveCodeCacheCallback, base::Unretained(this)));
    }

    void SaveCodeCacheCallback(WebCodeCacheLoader::FetchCodeCacheCallback callback)
    {
    ...
    {
        return MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(MakeGarbageCollected<TestResourceFetcherProperties>()->MakeDetachable(),
            MakeGarbageCollected<MockFetchContext>(), base::MakeRefCounted<scheduler::FakeTaskRunner>(), base::MakeRefCounted<scheduler::FakeTaskRunner>(),
            MakeGarbageCollected<DeferTestLoaderFactory>(&freeze_mode_, process_code_cache_request_callback_),
...
{
    SetCodeCacheProcessFunction(base::BindRepeating([](WebCodeCacheLoader::FetchCodeCacheCallback callback) { std::move(callback).Run(base::Time(), {}); }));
