...
scoped_refptr<CachedMetadata> CachedMetadata::CreateFromSerializedData(Vector<uint8_t> data)
{
    ...
    if (*reinterpret_cast<const uint32_t*>(data.data()) != CachedMetadataHandler::kSingleEntry) {
        return nullptr;
    }
...
CachedMetadata::CachedMetadata(Vector<uint8_t> data)
{
    ...
    CHECK_EQ(*reinterpret_cast<const uint32_t*>(data.data()), CachedMetadataHandler::kSingleEntry);

    vector_ = std::move(data);
}

CachedMetadata::CachedMetadata(uint32_t data_type_id, const uint8_t* data, wtf_size_t size)
{
