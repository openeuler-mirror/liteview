...
{
    UMA_HISTOGRAM_ENUMERATION("Memory.Renderer.BlinkCachedMetadataGetResult", state);
}
...
{
    if (!cached_metadata_) {
        RecordState(cached_metadata_discarded_ ? StateOnGet::kWasDiscarded : StateOnGet::kWasNeverPresent);
        return nullptr;
    }
...
void ScriptCachedMetadataHandler::CommitToPersistentStorage(CodeCacheHost* code_cache_host)
{
    if (cached_metadata_) {
        ...
        sender_->Send(code_cache_host, serialized_data.data(), serialized_data.size());
    } else {
...
void ScriptCachedMetadataHandlerWithHashing::SetSerializedCachedMetadata(mojo_base::BigBuffer data)
{
    ...
    if (*reinterpret_cast<const uint32_t*>(data.data()) != CachedMetadataHandler::kSingleEntryWithHash) {
        return;
    }
    ...
    cached_metadata_ = CachedMetadata::CreateFromSerializedData(data.data() + kHashingHeaderSize, data.size() - kHashingHeaderSize);
}
...
void ScriptCachedMetadataHandlerWithHashing::CommitToPersistentStorage(CodeCacheHost* code_cache_host)
{
    ...
    Sender()->Send(code_cache_host, serialized_data.data(), serialized_data.size());
}
...
Vector<uint8_t> ScriptCachedMetadataHandlerWithHashing::GetSerializedCachedMetadata() const
{
    ...
    if (cached_metadata_ && hash_state_ == kChecked) {
        ...
        serialized_data.Append(data.data(), base::checked_cast<wtf_size_t>(data.size()));
    }
