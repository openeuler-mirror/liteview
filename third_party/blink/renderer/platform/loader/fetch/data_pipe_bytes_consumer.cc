...
BytesConsumer::Result DataPipeBytesConsumer::BeginRead(const char** buffer, size_t* available)
{
    ...
    *available = 0;
    ...
    MojoResult rv = data_pipe_->BeginReadData(reinterpret_cast<const void**>(buffer), &pipe_available, MOJO_READ_DATA_FLAG_NONE);
...
{
    return state_ == InternalState::kReadable || state_ == InternalState::kWaiting;
}
