...
TEST_F(ResourceTest, RevalidationSucceededUpdateHeaders)
{
    ...
    EXPECT_EQ("keep-alive value", resource->GetResponse().HttpHeaderField("keep-alive"));
    EXPECT_EQ("expires value", resource->GetResponse().HttpHeaderField("expires"));
    EXPECT_EQ("last-modified value", resource->GetResponse().HttpHeaderField("last-modified"));
    EXPECT_EQ("proxy-authenticate value", resource->GetResponse().HttpHeaderField("proxy-authenticate"));
    EXPECT_EQ("proxy-authenticate value", resource->GetResponse().HttpHeaderField("proxy-authenticate"));
    EXPECT_EQ("proxy-connection value", resource->GetResponse().HttpHeaderField("proxy-connection"));
    EXPECT_EQ("custom value", resource->GetResponse().HttpHeaderField("x-custom"));
...
TEST_F(ResourceTest, RedirectDuringRevalidation)
{
    ...
    redirect_response.SetHttpHeaderField("location", AtomicString(redirect_target_url.GetString()));
...
TEST_F(ResourceTest, StaleWhileRevalidateCacheControlWithRedirect)
{
    ...
    redirect_response.SetHttpHeaderField(http_names::kCacheControl, "max-age=0, stale-while-revalidate=40");
