...
public:
    ...
    {
        return "DelegatingBytesConsumer";
    }
    ...
    void OnStateChange() override
    {
        ...
        if (loader_->IsAborted() || loader_->IsSuspended() || state_ == State::kCancelled) {
            return;
        }
        while (state_ == State::kLoading) {
            ...
            if (result == Result::kOk) {
                if (lookahead_bytes_ < available) {
                    loader_->DidReceiveData(base::make_span(buffer + lookahead_bytes_, available - lookahead_bytes_));
...
void ResponseBodyLoader::DidReceiveDecodedData(const String& data, std::unique_ptr<Resource::DecodedDataInfo> info)
{
    if (aborted_)
        return;
...
void ResponseBodyLoader::DidBufferLoadWhileInBackForwardCache(size_t num_bytes)
{
    if (!back_forward_cache_loader_helper_)
        return;
    back_forward_cache_loader_helper_->DidBufferLoadWhileInBackForwardCache(num_bytes);
}
...
void ResponseBodyLoader::OnStateChange()
{
    ...
    while (!aborted_ && (!IsSuspended() || IsSuspendedForBackForwardCache())) {
        ...
        if (result == BytesConsumer::Result::kOk) {
            TRACE_EVENT1("blink", "ResponseBodyLoader::OnStateChange", "available", available);
