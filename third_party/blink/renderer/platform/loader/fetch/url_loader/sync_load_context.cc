...
private:
    void Start(base::TimeDelta timeout)
    {
        ...
        if (timeout_timer_) {
            ...
            timeout_timer_->Start(FROM_HERE, timeout, context_, &SyncLoadContext::OnTimeout);
        }
    }
...
void SyncLoadContext::StartAsyncWithWaitableEvent(std::unique_ptr<network::ResourceRequest> request,
    ...
    base::TimeDelta timeout, mojo::PendingRemote<mojom::BlobRegistry> download_to_blob_registry, const WebVector<WebString>& cors_exempt_header_list,
    std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper)
{
    scoped_refptr<SyncLoadContext> context(new SyncLoadContext(request.get(), std::move(pending_url_loader_factory), response, context_for_redirect,
        redirect_or_response_event, abort_event, timeout, std::move(download_to_blob_registry), loading_task_runner));
...
SyncLoadContext::SyncLoadContext(network::ResourceRequest* request, std::unique_ptr<network::PendingSharedURLLoaderFactory> url_loader_factory,
    SyncLoadResponse* response, SyncLoadContext** context_for_redirect, base::WaitableEvent* redirect_or_response_event, base::WaitableEvent* abort_event,
    base::TimeDelta timeout, mojo::PendingRemote<mojom::BlobRegistry> download_to_blob_registry, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : response_(response)
    ...
    const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, std::vector<std::string>* removed_headers)
{
    ...
    if (removed_headers) {
        ...
        FindClientHintsToRemove(nullptr /* permissions_policy */, redirect_info.new_url, removed_headers);
    }
...
void SyncLoadContext::OnReceivedResponse(network::mojom::URLResponseHeadPtr head, base::TimeTicks response_arrival_at_renderer)
{
    DCHECK(!Completed());
    response_->head = std::move(head);
}

void SyncLoadContext::OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body)
{
    if (mode_ == Mode::kBlob) {
        ...
        download_to_blob_registry_->RegisterFromStream(response_->head->mime_type, "", std::max<int64_t>(0, response_->head->content_length), std::move(body),
            mojo::NullAssociatedRemote(), base::BindOnce(&SyncLoadContext::OnFinishCreatingBlob, base::Unretained(this)));
        return;
    }
...
void SyncLoadContext::OnBodyReadable(MojoResult, const mojo::HandleSignalsState&)
{
    ...
    MojoResult result = body_handle_->BeginReadData(&buffer, &read_bytes, MOJO_READ_DATA_FLAG_NONE);
