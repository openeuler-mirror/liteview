...
public:
    ...
    virtual void HandleMessage(WebResourceRequestSender* resource_request_sender) = 0;
    virtual bool IsCompletionMessage() const = 0;
};

class MojoURLLoaderClient::DeferredOnReceiveResponse final : public DeferredMessage {
public:
    explicit DeferredOnReceiveResponse(network::mojom::URLResponseHeadPtr response_head, base::TimeTicks response_arrival)
        : response_head_(std::move(response_head))
        , response_arrival_(response_arrival)
    {
    }

    void HandleMessage(WebResourceRequestSender* resource_request_sender) override
    {
        resource_request_sender->OnReceivedResponse(std::move(response_head_), response_arrival_);
    }
...
public:
    ...
    void HandleMessage(WebResourceRequestSender* resource_request_sender) override
    {
        resource_request_sender->OnReceivedRedirect(redirect_info_, std::move(response_head_), task_runner_);
    }
    bool IsCompletionMessage() const override
    {
        return false;
...
class MojoURLLoaderClient::DeferredOnReceiveCachedMetadata final : public DeferredMessage {
public:
...
class MojoURLLoaderClient::DeferredOnStartLoadingResponseBody final : public DeferredMessage {
public:
    explicit DeferredOnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body)
        : body_(std::move(body))
    {
    }
...
public:
    ...
    {
        DCHECK(draining_);
        SCOPED_CRASH_KEY_NUMBER("OnDataAvailable", "buffered_body_size", buffered_body_.size());
        SCOPED_CRASH_KEY_NUMBER("OnDataAvailable", "data_bytes", num_bytes);
        SCOPED_CRASH_KEY_STRING256("OnDataAvailable", "last_loaded_url", owner_->last_loaded_url().GetString().Utf8());
        ...
        if (owner_->freeze_mode() == WebLoaderFreezeMode::kBufferIncoming) {
            ...
            if (!owner_->CanContinueBufferingWhileInBackForwardCache()) {
                owner_->EvictFromBackForwardCache(blink::mojom::RendererEvictionReason::kNetworkExceedsBufferLimit);
                return;
            }
        }
        buffered_body_.emplace(static_cast<const char*>(data), static_cast<const char*>(data) + num_bytes);
...
private:
    void WriteBufferedBody(MojoResult)
    {
        ...
        while (!buffered_body_.empty()) {
            ...
            uint32_t bytes_sent = base::saturated_cast<uint32_t>(current_chunk.size() - offset_in_current_chunk_);
            MojoResult result = writable_->WriteData(current_chunk.data() + offset_in_current_chunk_, &bytes_sent, MOJO_WRITE_DATA_FLAG_NONE);
            switch (result) {
            case MOJO_RESULT_OK:
                break;
...
MojoURLLoaderClient::MojoURLLoaderClient(WebResourceRequestSender* resource_request_sender, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    bool bypass_redirect_checks, const GURL& request_url, WebBackForwardCacheLoaderHelper back_forward_cache_loader_helper)
...
void MojoURLLoaderClient::Freeze(WebLoaderFreezeMode mode)
{
    ...
    } else if (mode == WebLoaderFreezeMode::kBufferIncoming && !has_received_complete_ && !back_forward_cache_eviction_timer_.IsRunning()) {
...
void MojoURLLoaderClient::OnReceiveResponse(
    network::mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, absl::optional<mojo_base::BigBuffer> cached_metadata)
{
    ...
    if (NeedsStoringMessage()) {
        StoreAndDispatch(std::make_unique<DeferredOnReceiveResponse>(std::move(response_head), response_arrival_timing));
    } else {
        resource_request_sender_->OnReceivedResponse(std::move(response_head), response_arrival_timing);
    }
    ...
    if (cached_metadata) {
        if (NeedsStoringMessage()) {
            StoreAndDispatch(std::make_unique<DeferredOnReceiveCachedMetadata>(std::move(*cached_metadata)));
        } else {
            resource_request_sender_->OnReceivedCachedMetadata(std::move(*cached_metadata));
        }
    ...
    if (freeze_mode_ != WebLoaderFreezeMode::kBufferIncoming) {
        ...
        StoreAndDispatch(std::make_unique<DeferredOnStartLoadingResponseBody>(std::move(body)));
        return;
    }
    ...
    StoreAndDispatch(std::make_unique<DeferredOnStartLoadingResponseBody>(std::move(new_body_consumer)));
}

BackForwardCacheLoaderHelper* MojoURLLoaderClient::GetBackForwardCacheLoaderHelper()
{
    return back_forward_cache_loader_helper_.GetBackForwardCacheLoaderHelper();
}

void MojoURLLoaderClient::EvictFromBackForwardCache(blink::mojom::RendererEvictionReason reason)
{
...
void MojoURLLoaderClient::DidBufferLoadWhileInBackForwardCache(size_t num_bytes)
{
    ...
    if (!back_forward_cache_loader_helper)
        return;
    back_forward_cache_loader_helper->DidBufferLoadWhileInBackForwardCache(num_bytes);
}
...
{
    EvictFromBackForwardCache(blink::mojom::RendererEvictionReason::kNetworkRequestTimeout);
}
...
void MojoURLLoaderClient::OnReceiveRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr response_head)
{
    ...
    if (NeedsStoringMessage()) {
        StoreAndDispatch(std::make_unique<DeferredOnReceiveRedirect>(redirect_info, std::move(response_head), task_runner_));
    } else {
        resource_request_sender_->OnReceivedRedirect(redirect_info, std::move(response_head), task_runner_);
    }
}
...
{
    return freeze_mode_ != WebLoaderFreezeMode::kNone || deferred_messages_.size() > 0 || accumulated_transfer_size_diff_during_deferred_ > 0;
}
