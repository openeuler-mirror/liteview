...
public:
    CachedMetadataSenderImpl(const ResourceResponse&, blink::mojom::CodeCacheType);
...
CachedMetadataSenderImpl::CachedMetadataSenderImpl(const ResourceResponse& response, blink::mojom::CodeCacheType code_cache_type)
    ...
    , code_cache_type_(code_cache_type)
{
    ...
    DCHECK(response.CacheStorageCacheName().IsNull() || code_cache_type_ == blink::mojom::CodeCacheType::kWebAssembly);
    DCHECK(!response.WasFetchedViaServiceWorker() || response.IsServiceWorkerPassThrough() || code_cache_type_ == blink::mojom::CodeCacheType::kWebAssembly);
}

void CachedMetadataSenderImpl::Send(CodeCacheHost* code_cache_host, const uint8_t* data, size_t size)
{
    if (!code_cache_host)
        return;
    ...
    code_cache_host->get()->DidGenerateCacheableMetadata(
        code_cache_type_, GURL(response_url_), response_time_, mojo_base::BigBuffer(base::make_span(data, size)));
}
...
void ServiceWorkerCachedMetadataSender::Send(CodeCacheHost* code_cache_host, const uint8_t* data, size_t size)
{
    if (!code_cache_host)
        return;
    code_cache_host->get()->DidGenerateCacheableMetadataInCacheStorage(GURL(response_url_), response_time_, mojo_base::BigBuffer(base::make_span(data, size)),
...
void CachedMetadataSender::SendToCodeCacheHost(CodeCacheHost* code_cache_host, mojom::blink::CodeCacheType code_cache_type, WTF::String url,
    base::Time response_time, scoped_refptr<const SecurityOrigin> origin, const String& cache_storage_name, const uint8_t* data, size_t size)
{
    if (!code_cache_host)
        return;
    if (cache_storage_name.IsNull()) {
        code_cache_host->get()->DidGenerateCacheableMetadata(
            code_cache_type, GURL(url.Utf8()), response_time, mojo_base::BigBuffer(base::make_span(data, size)));
    } else {
        code_cache_host->get()->DidGenerateCacheableMetadataInCacheStorage(
            GURL(url.Utf8()), response_time, mojo_base::BigBuffer(base::make_span(data, size)), WebSecurityOrigin(origin), cache_storage_name.Utf8());
    }
}

// static
std::unique_ptr<CachedMetadataSender> CachedMetadataSender::Create(
    const ResourceResponse& response, blink::mojom::CodeCacheType code_cache_type, scoped_refptr<const SecurityOrigin> requestor_origin)
{
