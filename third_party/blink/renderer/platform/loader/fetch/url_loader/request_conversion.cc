...
void PopulateResourceRequestBody(const EncodedFormData& src, network::ResourceRequestBody* dest)
{
    for (const auto& element : src.Elements()) {
        ...
        case FormDataElement::kEncodedBlob: {
            ...
            mojo::Remote<mojom::blink::Blob> blob_remote(element.optional_blob_data_handle_->CloneBlobRemote());
...
void PopulateResourceRequest(const ResourceRequestHead& src, ResourceRequestBody src_body, network::ResourceRequest* dest)
{
    ...
    if (src.GetWebBundleTokenParams().has_value()) {
        *(int*)1 = 1;
        //         dest->web_bundle_token_params = absl::make_optional(network::ResourceRequest::WebBundleTokenParams(
        //             GURL(src.GetWebBundleTokenParams()->bundle_url), src.GetWebBundleTokenParams()->token, ToCrossVariantMojoType(src.GetWebBundleTokenParams()->CloneHandle())));
    }
