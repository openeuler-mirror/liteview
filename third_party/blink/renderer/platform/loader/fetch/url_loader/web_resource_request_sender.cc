...
void WebResourceRequestSender::SendSync(std::unique_ptr<network::ResourceRequest> request, const net::NetworkTrafficAnnotationTag& traffic_annotation,
    uint32_t loader_options, SyncLoadResponse* response, scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
    WebVector<std::unique_ptr<URLLoaderThrottle>> throttles, base::TimeDelta timeout, const WebVector<WebString>& cors_exempt_header_list,
    base::WaitableEvent* terminate_sync_load_event, mojo::PendingRemote<mojom::BlobRegistry> download_to_blob_registry, scoped_refptr<WebRequestPeer> peer,
    std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = base::ThreadPool::CreateSingleThreadTaskRunner({ base::TaskPriority::USER_BLOCKING });
    ...
    while (context_for_redirect) {
        ...
        bool follow_redirect = peer->OnReceivedRedirect(*response->redirect_info, response->head.Clone(), nullptr /* removed_headers */);
        redirect_or_response_event.Reset();
        if (follow_redirect) {
            task_runner->PostTask(FROM_HERE, base::BindOnce(&SyncLoadContext::FollowRedirect, base::Unretained(context_for_redirect)));
        } else {
...
int WebResourceRequestSender::SendAsync(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner,
    const net::NetworkTrafficAnnotationTag& traffic_annotation, uint32_t loader_options, const WebVector<WebString>& cors_exempt_header_list,
    scoped_refptr<WebRequestPeer> peer, scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
    ...
    WebBackForwardCacheLoaderHelper back_forward_cache_loader_helper)
{
    ...
    request_info_ = std::make_unique<PendingRequestInfo>(std::move(peer), request->destination, request->url, std::move(resource_load_info_notifier_wrapper));

    request_info_->resource_load_info_notifier_wrapper->NotifyResourceLoadInitiated(
        request_id, request->url, request->method, request->referrer, request_info_->request_destination, request->priority);

    auto client = std::make_unique<MojoURLLoaderClient>(
        this, loading_task_runner, url_loader_factory->BypassRedirectChecks(), request->url, back_forward_cache_loader_helper);

    std::vector<std::string> std_cors_exempt_header_list(cors_exempt_header_list.size());
    std::transform(
        cors_exempt_header_list.begin(), cors_exempt_header_list.end(), std_cors_exempt_header_list.begin(), [](const WebString& h) { return h.Latin1(); });
    std::unique_ptr<ThrottlingURLLoader> url_loader
        = ThrottlingURLLoader::CreateLoaderAndStart(std::move(url_loader_factory), throttles.ReleaseVector(), request_id, loader_options, request.get(),
            client.get(), traffic_annotation, std::move(loading_task_runner), absl::make_optional(std_cors_exempt_header_list));
...
void WebResourceRequestSender::Cancel(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
void WebResourceRequestSender::DidChangePriority(net::RequestPriority new_priority, int intra_priority_value)
{
    if (!request_info_) {
        DLOG(ERROR) << "unknown request";
        return;
...
void WebResourceRequestSender::DeletePendingRequest(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    if (!request_info_)
        return;
...
WebResourceRequestSender::PendingRequestInfo::PendingRequestInfo(scoped_refptr<WebRequestPeer> peer, network::mojom::RequestDestination request_destination,
    const GURL& request_url, std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper)
    : peer(std::move(peer))
...
void WebResourceRequestSender::FollowPendingRedirect(PendingRequestInfo* request_info)
{
    if (request_info->has_pending_redirect && request_info->should_follow_redirect) {
        ...
        if (request_info->redirect_requires_loader_restart) {
            ...
            std::transform(request_info_->removed_headers.begin(), request_info_->removed_headers.end(), removed_headers.begin(),
                [](const WebString& h) { return h.Ascii(); });
            request_info->url_loader->FollowRedirect(removed_headers, {} /* modified_headers */, {} /* modified_cors_exempt_headers */);
        }
    }
}

void WebResourceRequestSender::OnTransferSizeUpdated(int32_t transfer_size_diff)
{
...
void WebResourceRequestSender::OnUploadProgress(int64_t position, int64_t size)
{
    if (!request_info_)
        return;
...
void WebResourceRequestSender::OnReceivedResponse(network::mojom::URLResponseHeadPtr response_head, base::TimeTicks response_arrival)
{
    ...
    if (!remote_response_start.is_null()) {
        UMA_HISTOGRAM_TIMES("Blink.ResourceRequest.ResponseDelay", request_info_->local_response_start - remote_response_start);
    }
    request_info_->load_timing_info = response_head->load_timing;
    if (delegate_) {
        scoped_refptr<WebRequestPeer> new_peer
            = delegate_->OnReceivedResponse(std::move(request_info_->peer), WebString::FromUTF8(response_head->mime_type), KURL(request_info_->url));
        DCHECK(new_peer);
        request_info_->peer = std::move(new_peer);
    }

    request_info_->peer->OnReceivedResponse(response_head.Clone(), response_arrival);
    if (!request_info_)
        return;
...
void WebResourceRequestSender::OnReceivedCachedMetadata(mojo_base::BigBuffer data)
{
    if (!request_info_)
        return;
...
void WebResourceRequestSender::OnReceivedRedirect(
    const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr response_head, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    if (!request_info_->url_loader && request_info_->should_follow_redirect) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE,
            base::BindOnce(&WebResourceRequestSender::OnReceivedRedirect, weak_factory_.GetWeakPtr(), redirect_info, std::move(response_head), task_runner));
        return;
    }

    request_info_->local_response_start = base::TimeTicks::Now();
    request_info_->remote_request_start = response_head->load_timing.request_start;
    request_info_->redirect_requires_loader_restart = RedirectRequiresLoaderRestart(request_info_->response_url, redirect_info.new_url);

    base::TimeTicks remote_response_start = ToLocalURLResponseHead(*request_info_, *response_head);
    if (!remote_response_start.is_null()) {
        UMA_HISTOGRAM_TIMES("Blink.ResourceRequest.RedirectDelay", request_info_->local_response_start - remote_response_start);
    }
    std::vector<std::string> removed_headers;
    if (request_info_->peer->OnReceivedRedirect(redirect_info, response_head.Clone(), &removed_headers)) {
        ...
        std::transform(removed_headers.begin(), removed_headers.end(), vector.begin(), [](const std::string& h) { return WebString::FromASCII(h); });
...
void WebResourceRequestSender::OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body)
{
    TRACE_EVENT0("loading", "WebResourceRequestSender::OnStartLoadingResponseBody");
...
void WebResourceRequestSender::OnRequestComplete(const network::URLLoaderCompletionStatus& status)
{
    ...
    if (status.completion_time.is_null()) {
        ...
        renderer_status.completion_time
            = std::min(status.completion_time - request_info_->remote_request_start + request_info_->load_timing_info.request_start, base::TimeTicks::Now());
    }

    const net::LoadTimingInfo& timing_info = request_info_->load_timing_info;
    if (!timing_info.request_start.is_null()) {
        UMA_HISTOGRAM_TIMES("Blink.ResourceRequest.StartDelay", timing_info.request_start - request_info_->local_request_start);
    }
    if (!renderer_status.completion_time.is_null()) {
        UMA_HISTOGRAM_TIMES("Blink.ResourceRequest.CompletionDelay", base::TimeTicks::Now() - renderer_status.completion_time);
    }
...
base::TimeTicks WebResourceRequestSender::ToLocalURLResponseHead(const PendingRequestInfo& request_info, network::mojom::URLResponseHead& response_head) const
{
