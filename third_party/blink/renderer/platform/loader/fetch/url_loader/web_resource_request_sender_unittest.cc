...
static constexpr char kTestPageHeaders[] = "HTTP/1.1 200 OK\nContent-Type:text/html\n\n";
static constexpr char kTestPageMimeType[] = "text/html";
static constexpr char kTestPageCharset[] = "";
static constexpr char kTestPageContents[] = "<html><head><title>Google</title></head><body><h1>Google</h1></body></"
                                            "html>";
...
std::string ReadOneChunk(mojo::ScopedDataPipeConsumerHandle* handle)
{
    ...
    MojoResult result = (*handle)->ReadData(buffer, &read_bytes, MOJO_READ_DATA_FLAG_NONE);
...
class TestResourceRequestSenderDelegate : public WebResourceRequestSenderDelegate {
public:
    TestResourceRequestSenderDelegate() = default;
    TestResourceRequestSenderDelegate(const TestResourceRequestSenderDelegate&) = delete;
    TestResourceRequestSenderDelegate& operator=(const TestResourceRequestSenderDelegate&) = delete;
    ...
    scoped_refptr<WebRequestPeer> OnReceivedResponse(scoped_refptr<WebRequestPeer> current_peer, const WebString& mime_type, const WebURL& url) override
    {
    ...
    public:
        ...
        bool OnReceivedRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, std::vector<std::string>*) override
        {
            return false;
        }
        void OnReceivedResponse(network::mojom::URLResponseHeadPtr head, base::TimeTicks response_arrival) override
        {
...
public:
    ...
    bool OnReceivedRedirect(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, std::vector<std::string>* removed_headers) override
    {
    ...
    void OnReceivedResponse(network::mojom::URLResponseHeadPtr head, base::TimeTicks response_arrival) override
    {
        ...
        if (cancel_on_receive_response_) {
            resource_request_sender_->Cancel(scheduler::GetSingleThreadTaskRunnerForTesting());
        }
    }
    void OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body) override
    {
...
class WebResourceRequestSenderTest : public testing::Test, public network::mojom::URLLoaderFactory {
public:
    ...
    void CallOnReceiveResponse(network::mojom::URLLoaderClient* client, mojo::ScopedDataPipeConsumerHandle body)
    {
    ...
    void StartAsync(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<WebRequestPeer> peer)
    {
        sender()->SendAsync(std::move(request), scheduler::GetSingleThreadTaskRunnerForTesting(), TRAFFIC_ANNOTATION_FOR_TESTS, false, WebVector<WebString>(),
            std::move(peer), base::MakeRefCounted<network::WeakWrapperSharedURLLoaderFactory>(this), std::vector<std::unique_ptr<URLLoaderThrottle>>(),
    ...
    public:
        ...
        WebResourceRequestSenderDelegate* GetResourceRequestSenderDelegate() override
        {
            return delegate_;
        }
...
protected:
    ...
    ScopedTestingPlatformSupport<TestPlatform, WebResourceRequestSenderDelegate*> platform_;
...
TEST_F(WebResourceRequestSenderTest, DelegateTest)
{
    ...
    mock_peer_ = base::MakeRefCounted<MockRequestPeer>(resource_request_sender_.get());
    ...
    auto result = producer_handle->WriteData(kTestPageContents, &size, MOJO_WRITE_DATA_FLAG_NONE);
...
public:
    void PerformTest(network::mojom::URLResponseHeadPtr response_head)
    {
        ...
        mojo::Remote<network::mojom::URLLoaderClient> client(std::move(loader_and_clients_[0].second));
        loader_and_clients_.clear();
        client->OnReceiveResponse(std::move(response_head), mojo::ScopedDataPipeConsumerHandle(), absl::nullopt);
    }
...
private:
    network::mojom::URLResponseHeadPtr response_info_ = network::mojom::URLResponseHead::New();
};
...
TEST_F(TimeConversionTest, DISABLED_ProperlyInitialized)
{
    ...
    EXPECT_EQ(base::TimeTicks(), response_info().load_timing.connect_timing.domain_lookup_start);
    EXPECT_LE(request_start, response_info().load_timing.connect_timing.connect_start);
}
...
public:
    void PerformTest(base::TimeTicks remote_request_start, base::TimeTicks completion_time, base::TimeDelta delay)
    {
        ...
        client->OnReceiveResponse(std::move(response_head), std::move(consumer_handle), absl::nullopt);
