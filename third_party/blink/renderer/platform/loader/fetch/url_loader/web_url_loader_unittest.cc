...
public:
    ...
    void SendSync(std::unique_ptr<network::ResourceRequest> request, const net::NetworkTrafficAnnotationTag& traffic_annotation, uint32_t loader_options,
        ...
        base::TimeDelta timeout, const WebVector<WebString>& cors_exempt_header_list, base::WaitableEvent* terminate_sync_load_event,
        mojo::PendingRemote<mojom::BlobRegistry> download_to_blob_registry, scoped_refptr<WebRequestPeer> peer,
    ...
    int SendAsync(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner,
    ...
    void Cancel(scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
    ...
    WebLoaderFreezeMode freeze_mode() const
    {
        return freeze_mode_;
    }
...
public:
    TestWebURLLoaderClient()
        : loader_(new WebURLLoader(
            ...
            scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(scheduler::GetSingleThreadTaskRunnerForTesting()),
            scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(scheduler::GetSingleThreadTaskRunnerForTesting()),
    ...
    bool WillFollowRedirect(const WebURL& new_url, const net::SiteForCookies& new_site_for_cookies, const WebString& new_referrer,
        ...
        bool& report_raw_headers, std::vector<std::string>*, bool insecure_scheme_was_upgraded) override
    {
        EXPECT_TRUE(loader_);
    ...
    void DidStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle response_body) override
    {
    ...
    void DidFinishLoading(base::TimeTicks finishTime, int64_t totalEncodedDataLength, int64_t totalEncodedBodyLength, int64_t totalDecodedBodyLength,
        bool should_report_corb_blocking, absl::optional<bool> pervasive_payload_requested) override
    {
        EXPECT_TRUE(loader_);
    ...
    void DidFail(const WebURLError& error, base::TimeTicks finishTime, int64_t totalEncodedDataLength, int64_t totalEncodedBodyLength,
    ...
    const absl::optional<WebURLError>& error() const
    {
        return error_;
    }
...
public:
    WebURLLoaderTest()
        : client_(std::make_unique<TestWebURLLoaderClient>())
    {
    ...
    void DoReceiveRedirect()
    {
        ...
        peer()->OnReceivedRedirect(redirect_info, network::mojom::URLResponseHead::New(), &removed_headers);
    ...
    void DoReceiveHTTPSRedirect()
    {
        ...
        redirect_info.new_site_for_cookies = net::SiteForCookies::FromUrl(GURL(kTestHTTPSURL));
        peer()->OnReceivedRedirect(redirect_info, network::mojom::URLResponseHead::New(), nullptr);
        EXPECT_TRUE(client()->did_receive_redirect());
    }
...
TEST_F(WebURLLoaderTest, SSLInfo)
{
    ...
    base::StringPiece cert0_der = net::x509_util::CryptoBufferAsStringPiece(certs[0]->cert_buffer());
    base::StringPiece cert1_der = net::x509_util::CryptoBufferAsStringPiece(certs[1]->cert_buffer());
    ...
    const absl::optional<net::SSLInfo>& got_ssl_info = web_url_response.ToResourceResponse().GetSSLInfo();
...
TEST_F(WebURLLoaderTest, SyncLengths)
{
    ...
    client()->loader()->LoadSynchronously(std::move(request), /*url_request_extra_data=*/nullptr,
        /*pass_response_pipe_to_client=*/false, /*no_mime_sniffing=*/false, base::TimeDelta(), nullptr, response, error, data, encoded_data_length,
