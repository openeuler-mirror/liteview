...
public:
    ...
    void OnReceivedRedirect(
        const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr head, scoped_refptr<base::SingleThreadTaskRunner> task_runner) override
    {
    ...
    void OnReceivedResponse(network::mojom::URLResponseHeadPtr head, base::TimeTicks response_arrival_time) override
    {
        EXPECT_FALSE(context_->cancelled);
    ...
    void OnRequestComplete(const network::URLLoaderCompletionStatus& status) override
    {
        if (context_->cancelled)
            return;
        EXPECT_TRUE(context_->received_response);
...
std::string GetRequestPeerContextBody(MockWebResourceRequestSender::Context* context)
{
...
protected:
        ...
        : resource_request_sender_(new MockWebResourceRequestSender())
    {
        ...
        client_ = std::make_unique<MojoURLLoaderClient>(resource_request_sender_.get(), loading_task_runner, url_loader_factory->BypassRedirectChecks(),
            request->url, WebBackForwardCacheLoaderHelper(MakeGarbageCollected<TestBackForwardCacheLoaderHelper>()));
...
{
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), mojo::ScopedDataPipeConsumerHandle(), absl::nullopt);
...
TEST_P(WebMojoURLLoaderClientTest, ResponseBody)
{
    ...
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), std::move(data_pipe_consumer), absl::nullopt);
    ...
    uint32_t size = 5;
    MojoResult result = data_pipe_producer->WriteData("hello", &size, MOJO_WRITE_DATA_FLAG_NONE);
...
TEST_P(WebMojoURLLoaderClientTest, DeferWithResponseBody)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, data_pipe_producer->WriteData(msg1.data(), &size, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_P(WebMojoURLLoaderClientTest, DeferredAndDeferredWithBackForwardCacheTransitions)
{
    ...
    url_loader_client_->OnReceiveResponse(network::mojom::URLResponseHead::New(), std::move(consumer_handle), absl::nullopt);
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(context_->received_response);
    EXPECT_FALSE(context_->complete);
    EXPECT_EQ("", GetRequestPeerContextBody(context_));
    ...
    std::string msg1 = "he";
    uint32_t size = static_cast<uint32_t>(msg1.size());
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(msg1.data(), &size, MOJO_WRITE_DATA_FLAG_NONE));
    EXPECT_EQ(msg1.size(), size);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ("", GetRequestPeerContextBody(context_));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(msg2.data(), &size, MOJO_WRITE_DATA_FLAG_NONE));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(msg3.data(), &size, MOJO_WRITE_DATA_FLAG_NONE));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(msg4.data(), &size, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_P(WebMojoURLLoaderClientTest, DeferredWithBackForwardCacheLongResponseBody)
{
    ...
    while (bytes_remaining > 0) {
        ...
        MojoResult result = producer_handle->WriteData(body.c_str() + start_position, &bytes_sent, MOJO_WRITE_DATA_FLAG_NONE);
