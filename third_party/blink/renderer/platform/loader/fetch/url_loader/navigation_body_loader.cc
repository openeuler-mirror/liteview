...
struct DataChunk {
    ...
    {
        decoded_data = other.decoded_data;
        has_seen_end_of_data = other.has_seen_end_of_data;
        has_error = other.has_error;
        encoded_data.swap(((DataChunk*)(&other))->encoded_data);
        encoded_data_size = other.encoded_data_size;
        encoding_data = other.encoding_data;
    }
...
private:
    ...
    void AddChunk(const String& decoded_data, const char* encoded_data, size_t size, bool has_error)
    {
        DCHECK(reader_task_runner_->RunsTasksInCurrentSequence());
        ...
        bool post_task;
        {
            ...
            DataChunk chunk;
            chunk.decoded_data = decoded_data;
            chunk.has_seen_end_of_data = has_seen_end_of_data_;
            chunk.has_error = has_error;
            chunk.encoded_data = std::move(encoded_data_copy);
            chunk.encoded_data_size = size;
            chunk.encoding_data = decoder_->GetEncodingData();
            data_chunks_.push_back(chunk);
        }
        if (post_task) {
            PostCrossThreadTask(*main_thread_task_runner_, FROM_HERE, CrossThreadBindOnce(&NavigationBodyLoader::ProcessOffThreadData, body_loader_));
        }
...
void NavigationBodyLoader::OnReceiveResponse(
    network::mojom::URLResponseHeadPtr head, mojo::ScopedDataPipeConsumerHandle body, absl::optional<mojo_base::BigBuffer> cached_metadata)
{
    // This has already happened in the browser process.
    NOTREACHED();
}
...
void NavigationBodyLoader::StartLoadingBodyInBackground(std::unique_ptr<BodyTextDecoder> decoder, bool should_keep_encoded_data)
{
    ...
    off_thread_body_reader_.reset(new OffThreadBodyReader(std::move(response_body_), std::move(decoder), weak_factory_.GetWeakPtr(), task_runner_,
        worker_pool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING }), should_keep_encoded_data));
}
...
void NavigationBodyLoader::ProcessOffThreadData()
{
    ...
    auto chunks = off_thread_body_reader_->TakeData(max_data_to_process_per_task_);
    auto weak_self = weak_factory_.GetWeakPtr();
    for (const auto& chunk : chunks) {
        client_->DecodedBodyDataReceived(chunk.decoded_data, chunk.encoding_data, base::make_span(chunk.encoded_data.get(), chunk.encoded_data_size));
        if (!weak_self)
            return;
...
void NavigationBodyLoader::NotifyCompletionIfAppropriate()
{
    ...
    client_ = nullptr;
    client->BodyLoadingFinished(status_.completion_time, status_.encoded_data_length, status_.encoded_body_length, status_.decoded_body_length,
...
void WebNavigationBodyLoader::FillNavigationParamsResponseAndBodyLoader(mojom::CommonNavigationParamsPtr common_params,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, std::unique_ptr<ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper,
    bool is_main_frame, WebNavigationParams* navigation_params)
{
    ...
    resource_load_info_notifier_wrapper->NotifyResourceLoadInitiated(request_id, GURL(url),
        ...
        common_params->request_destination, is_main_frame ? net::HIGHEST : net::LOWEST);
    ...
    for (size_t i = 0; i < redirect_count; ++i) {
        ...
        WebURLLoader::PopulateURLResponse(url, *redirect_response, &redirect.redirect_response, response_head->ssl_info.has_value(), request_id);
    ...
    WebURLLoader::PopulateURLResponse(url, *response_head, &navigation_params->response, response_head->ssl_info.has_value(), request_id);
    if (url.ProtocolIsData())
