...
ResourceLoadPriority AdjustPriorityWithPriorityHintAndRenderBlocking(ResourceLoadPriority priority_so_far, ResourceType type,
    const ResourceRequestHead& resource_request, FetchParameters::DeferOption defer_option, RenderBlockingBehavior render_blocking_behavior,
    bool is_link_preload)
{
    mojom::blink::FetchPriorityHint fetch_priority_hint = resource_request.GetFetchPriorityHint();
...
// static
void ResourceFetcher::AddPriorityObserverForTesting(const KURL& resource_url, base::OnceCallback<void(int)> callback)
{
    PriorityObservers()->Set(MemoryCache::RemoveFragmentIdentifierIfNeeded(resource_url), std::move(callback));
}
...
ResourceLoadPriority ResourceFetcher::ComputeLoadPriority(ResourceType type, const ResourceRequestHead& resource_request,
    ...
    RenderBlockingBehavior render_blocking_behavior, bool is_link_preload)
{
    ...
    priority = AdjustPriorityWithPriorityHintAndRenderBlocking(priority, type, resource_request, defer_option, render_blocking_behavior, is_link_preload);

    if (properties_->IsSubframeDeprioritizationEnabled()) {
        if (properties_->IsOutermostMainFrame()) {
            UMA_HISTOGRAM_ENUMERATION("LowPriorityIframes.MainFrameRequestPriority", priority, static_cast<int>(ResourceLoadPriority::kHighest) + 1);
        } else {
            UMA_HISTOGRAM_ENUMERATION("LowPriorityIframes.IframeRequestPriority", priority, static_cast<int>(ResourceLoadPriority::kHighest) + 1);
...
bool ResourceFetcher::ResourceNeedsLoad(Resource* resource, const FetchParameters& params, RevalidationPolicy policy)
{
    ...
    if (resource->GetType() == ResourceType::kImage
        && (ShouldDeferImageLoad(resource->Url()) || params.GetImageRequestBehavior() == FetchParameters::ImageRequestBehavior::kDeferImageLoad)) {
        return false;
    }
    ...
    Resource* resource, const ResourceRequest& request, bool is_static_data, RenderBlockingBehavior render_blocking_behavior)
{
    ...
    if (resource->EncodedSize() > 0) {
        resource_load_observer_->DidReceiveData(request.InspectorId(), base::make_span(static_cast<const char*>(nullptr), resource->EncodedSize()));
    }

    resource_load_observer_->DidFinishLoading(request.InspectorId(), base::TimeTicks(), 0, resource->GetResponse().DecodedBodyLength(), false);

    if (!is_static_data) {
        // Resources loaded from memory cache should be reported the first time
        // they're used.
        scoped_refptr<ResourceTimingInfo> info = ResourceTimingInfo::Create(
            resource->Options().initiator_info.name, base::TimeTicks::Now(), request.GetRequestContext(), request.GetRequestDestination(), request.GetMode());
        ...
        info->SetInitialURL(resource->GetResourceRequest().GetRedirectInfo().has_value() ? resource->GetResourceRequest().GetRedirectInfo()->original_url
...
Resource* ResourceFetcher::CreateResourceForStaticData(const FetchParameters& params, const ResourceFactory& factory)
{
    ...
    if (url.ProtocolIsData()) {
        int result;
        std::tie(result, response, data) = network_utils::ParseDataURL(url, params.GetResourceRequest().HttpMethod());
        if (result != net::OK) {
            return nullptr;
        }
...
void ResourceFetcher::UpdateMemoryCacheStats(Resource* resource, RevalidationPolicy policy, const FetchParameters& params, const ResourceFactory& factory,
    bool is_static_data, bool same_top_frame_site_resource_cached) const
{
    if (is_static_data)
        return;
...
absl::optional<ResourceRequestBlockedReason> ResourceFetcher::PrepareRequest(
    FetchParameters& params, const ResourceFactory& factory, WebScopedVirtualTimePauser& virtual_time_pauser)
{
    ...
    if (resource_request.GetWebBundleTokenParams()) {
        ...
        if (resource_request.Url().Protocol() == "uuid-in-package" && resource_request.GetWebBundleTokenParams()) {
            ...
            // checks.
            bundle_url_for_uuid_resources = MemoryCache::RemoveFragmentIdentifierIfNeeded(resource_request.GetWebBundleTokenParams()->bundle_url);
        }
    }
    ...
    ReportingDisposition reporting_disposition
        = params.IsSpeculativePreload() || params.IsStaleRevalidation() ? ReportingDisposition::kSuppressReporting : ReportingDisposition::kReport;
    ...
    const absl::optional<ResourceRequest::RedirectInfo>& redirect_info = resource_request.GetRedirectInfo();
    ...
    Context().PopulateResourceRequest(resource_type, params.GetResourceWidth(), resource_request, options);
    ...
    if (!resource_request.PriorityHasBeenSet()) {
        computed_load_priority = ComputeLoadPriority(resource_type, params.GetResourceRequest(), ResourcePriority::kNotVisible, params.Defer(),
            params.GetSpeculativePreloadType(), params.GetRenderBlockingBehavior(), params.IsLinkPreload());
    }
    ...
    if (resource_request.GetRequestContext() == mojom::blink::RequestContextType::UNSPECIFIED) {
        resource_request.SetRequestContext(DetermineRequestContext(resource_type, kImageNotImageSet));
        resource_request.SetRequestDestination(DetermineRequestDestination(resource_type));
    }
    ...
    absl::optional<ResourceRequestBlockedReason> blocked_reason = Context().CanRequest(resource_type, resource_request,
        MemoryCache::RemoveFragmentIdentifierIfNeeded(bundle_url_for_uuid_resources.IsValid() ? bundle_url_for_uuid_resources : params.Url()), options,
        reporting_disposition, resource_request.GetRedirectInfo());

    if (Context().CalculateIfAdSubresource(resource_request, absl::nullopt /* alias_url */, resource_type, options.initiator_info))
    ...
    if (resource_request.GetCredentialsMode() == network::mojom::CredentialsMode::kOmit) {
...
void ResourceFetcher::AttachWebBundleTokenIfNeeded(ResourceRequest& resource_request) const
{
    SubresourceWebBundle* bundle = GetMatchingBundle(resource_request.Url());
    if (!bundle)
        return;
    resource_request.SetWebBundleTokenParams(ResourceRequestHead::WebBundleTokenParams(bundle->GetBundleUrl(), bundle->WebBundleToken(), mojo::NullRemote()));
...
Resource* ResourceFetcher::RequestResource(FetchParameters& params, const ResourceFactory& factory, ResourceClient* client)
{
    ...
    absl::optional<ResourceRequestBlockedReason> blocked_reason = PrepareRequest(params, factory, pauser);
    if (blocked_reason) {
    ...
    if (!is_stale_revalidation && !resource) {
        ...
        if (resource) {
            ...
            if (base::FeatureList::IsEnabled(features::kScopeMemoryCachePerContext) && !in_cached_resources_map) {
                resource = nullptr;
            } else {
                resource = MemoryCache::Get()->ResourceForURL(params.Url(), GetCacheIdentifier(params.Url()));
            }
            if (resource) {
    ...
    UpdateMemoryCacheStats(resource, policy, params, factory, is_static_data, same_top_frame_site_resource_cached);
...
void ResourceFetcher::InitializeRevalidation(ResourceRequest& revalidating_request, Resource* resource)
{
    ...
    if (!last_modified.empty() || !e_tag.empty()) {
        ...
        if (revalidating_request.GetCacheMode() == mojom::blink::FetchCacheMode::kValidateCache) {
            revalidating_request.SetHttpHeaderField(http_names::kCacheControl, "max-age=0");
        }
    }
...
std::unique_ptr<WebURLLoader> ResourceFetcher::CreateURLLoader(const ResourceRequestHead& request, const ResourceLoaderOptions& options)
{
    ...
    return loader_factory_->CreateURLLoader(ResourceRequest(request), options, freezable_task_runner_,
        request.GetKeepalive() ? Thread::Current()->GetDeprecatedTaskRunner() : unfreezable_task_runner_,
...
{
    const String cache_identifier = GetCacheIdentifier(params.GetResourceRequest().Url());
    if (!base::FeatureList::IsEnabled(blink::features::kScopeMemoryCachePerContext)) {
...
void ResourceFetcher::StorePerformanceTimingInitiatorInformation(Resource* resource, RenderBlockingBehavior render_blocking_behavior)
{
    ...
    scoped_refptr<ResourceTimingInfo> info = ResourceTimingInfo::Create(fetch_initiator, base::TimeTicks::Now(),
        resource->GetResourceRequest().GetRequestContext(), resource->GetResourceRequest().GetRequestDestination(), resource->GetResourceRequest().GetMode());
...
void ResourceFetcher::PrintPreloadMismatch(Resource* resource, Resource::MatchStatus status)
{
    ...
    TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::PrintPreloadMismatch", "data",
        CreateTracedValueForUnusedPreload(resource->Url(), status, resource->GetResourceRequest().GetDevToolsId().value_or(String())));
}
    ...
    ResourceType type, const FetchParameters& fetch_params, const Resource& existing_resource, bool is_static_data) const
{
    ...
    if (request.GetCacheMode() == mojom::blink::FetchCacheMode::kValidateCache
        || existing_resource.MustRevalidateDueToCacheHeaders(request.AllowsStaleResponse()) || request.CacheControlContainsNoCache()) {
...
void ResourceFetcher::ScheduleWarnUnusedPreloads()
{
    ...
    unused_preloads_timer_ = PostDelayedCancellableTask(
        *freezable_task_runner_, FROM_HERE, WTF::BindOnce(&ResourceFetcher::WarnUnusedPreloads, WrapWeakPersistent(this)), kUnusedPreloadTimeout);
}
...
void ResourceFetcher::WarnUnusedPreloads()
{
    for (const auto& pair : preloads_) {
        ...
        if (!resource || !resource->IsLinkPreload() || !resource->IsUnusedPreload())
            continue;
        String message = "The resource " + resource->Url().GetString() + " was preloaded " + "using link preload but not used within a few seconds from the "
            + "window's load event. Please make sure it has an appropriate `as` " + "value and it is preloaded intentionally.";
        ...
        TRACE_EVENT1("blink,blink.resource", "ResourceFetcher::WarnUnusedPreloads", "data",
            CreateTracedValueForUnusedPreload(resource->Url(), Resource::MatchStatus::kOk, resource->GetResourceRequest().GetDevToolsId().value_or(String())));
    }
...
void ResourceFetcher::HandleLoaderFinish(
    Resource* resource, base::TimeTicks response_end, LoaderFinishType type, uint32_t inflight_keepalive_bytes, bool should_report_corb_blocking)
{
    DCHECK(resource);
    ...
    if (scoped_refptr<ResourceTimingInfo> info = resource_timing_info_map_.Take(resource)) {
    ...
    if (resource->GetResponse().GetType() == network::mojom::FetchResponseType::kOpaque && resource->GetResponse().HasRangeRequested()
        && !resource->GetResourceRequest().HttpHeaderFields().Contains(net::HttpRequestHeaders::kRange)) {
    ...
    if (resource_load_observer_) {
        ...
        resource_load_observer_->DidFinishLoading(
            resource->InspectorId(), response_end, encoded_data_length, resource->GetResponse().DecodedBodyLength(), should_report_corb_blocking);
    }
}
...
{
    DCHECK(resource->GetType() == ResourceType::kFont || resource->GetType() == ResourceType::kImage);
    ...
    Resource* resource, ResourceRequestBody request_body, ImageLoadBlockingPolicy policy, RenderBlockingBehavior render_blocking_behavior)
{
...

    {
        ...
        loader = MakeGarbageCollected<ResourceLoader>(this, scheduler_, resource, std::move(request_body), size);
...
{
    if (properties_->GetControllerServiceWorkerMode() != mojom::ControllerServiceWorkerMode::kNoController) {
...
absl::optional<base::UnguessableToken> ResourceFetcher::GetSubresourceBundleToken(const KURL& url) const
{
    SubresourceWebBundle* bundle = GetMatchingBundle(url);
    if (!bundle)
        return absl::nullopt;
...
absl::optional<KURL> ResourceFetcher::GetSubresourceBundleSourceUrl(const KURL& url) const
{
    SubresourceWebBundle* bundle = GetMatchingBundle(url);
    if (!bundle)
        return absl::nullopt;
...
void ResourceFetcher::EmulateLoadStartedForInspector(Resource* resource, const KURL& url, mojom::blink::RequestContextType request_context,
...
void ResourceFetcher::PopulateAndAddResourceTimingInfo(Resource* resource, scoped_refptr<ResourceTimingInfo> info, base::TimeTicks response_end)
{
