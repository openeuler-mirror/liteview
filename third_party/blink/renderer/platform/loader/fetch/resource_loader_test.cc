...
const char kCnameAliasHadAliasesHistogram[] = "SubresourceFilter.CnameAlias.Renderer.HadAliases";
const char kCnameAliasIsInvalidCountHistogram[] = "SubresourceFilter.CnameAlias.Renderer.InvalidCount";
const char kCnameAliasIsRedundantCountHistogram[] = "SubresourceFilter.CnameAlias.Renderer.RedundantCount";
const char kCnameAliasListLengthHistogram[] = "SubresourceFilter.CnameAlias.Renderer.ListLength";
const char kCnameAliasWasAdTaggedHistogram[] = "SubresourceFilter.CnameAlias.Renderer.WasAdTaggedBasedOnAlias";
const char kCnameAliasWasBlockedHistogram[] = "SubresourceFilter.CnameAlias.Renderer.WasBlockedBasedOnAlias";
...
protected:
    ...
    class NoopLoaderFactory final : public ResourceFetcher::LoaderFactory {
        std::unique_ptr<WebURLLoader> CreateURLLoader(const ResourceRequest& request, const ResourceLoaderOptions& options,
...
TEST_F(ResourceLoaderTest, LoadDataURL_DefersAsyncAndStream)
{
    ...
    const char* buffer;
    size_t available;
    BytesConsumer::Result result = raw_resource_client->body()->BeginRead(&buffer, &available);
...
protected:
    ...
    {
        const scoped_refptr<const SecurityOrigin> origin = SecurityOrigin::Create(foo_url_);

        auto* properties = MakeGarbageCollected<TestResourceFetcherProperties>(origin);
        FetchContext* context = MakeGarbageCollected<MockFetchContext>();
        auto* fetcher = MakeResourceFetcher(properties, context);
        ...
        FetchParameters fetch_parameters = FetchParameters::CreateForTest(std::move(request));
...
TEST_F(ResourceLoaderIsolatedCodeCacheTest, SyntheticResponseFromServiceWorker)
{
    ResourceResponse response(foo_url_);
    response.SetHttpStatusCode(200);
...
TEST_F(ResourceLoaderIsolatedCodeCacheTest, PassThroughResponseFromServiceWorker)
{
    ResourceResponse response(foo_url_);
    response.SetHttpStatusCode(200);
    response.SetWasFetchedViaServiceWorker(true);
...
TEST_F(ResourceLoaderIsolatedCodeCacheTest, DifferentUrlResponseFromServiceWorker)
{
    ResourceResponse response(foo_url_);
    response.SetHttpStatusCode(200);
    response.SetWasFetchedViaServiceWorker(true);
...
protected:
    ...
    {
        histogram_tester()->ExpectUniqueSample(kCnameAliasHadAliasesHistogram, info.has_aliases, 1);

        if (info.has_aliases) {
            histogram_tester()->ExpectUniqueSample(kCnameAliasWasAdTaggedHistogram, info.was_ad_tagged_based_on_alias, 1);
            histogram_tester()->ExpectUniqueSample(kCnameAliasWasBlockedHistogram, info.was_blocked_based_on_alias, 1);
            histogram_tester()->ExpectUniqueSample(kCnameAliasListLengthHistogram, info.list_length, 1);
            histogram_tester()->ExpectUniqueSample(kCnameAliasIsInvalidCountHistogram, info.invalid_count, 1);
            histogram_tester()->ExpectUniqueSample(kCnameAliasIsRedundantCountHistogram, info.redundant_count, 1);
        }
    }
...
public:
    ...
    void SetUp() override
    {
        std::string pervasive_payloads_params = "1,http://127.0.0.1:4353/pervasive.js,"
                                                "2478392C652868C0AAF0316A28284610DBDACF02D66A00B39F3BA75D887F4829";
        feature_list_.InitWithFeaturesAndParameters(
            { { network::features::kPervasivePayloadsList, { { "pervasive-payloads", pervasive_payloads_params } } },
                { network::features::kCacheTransparency, {} }, { net::features::kSplitCacheByNetworkIsolationKey, {} } },
            { /* disabled_features */ });
...
TEST_F(ResourceLoaderCacheTransparencyTest, PervasivePayloadRequested)
{
    ...
    // Check UKM recording
    auto entries = GetUkmRecorder()->GetEntriesByName(ukm::builders::Network_CacheTransparency::kEntryName);
    ASSERT_EQ(1u, entries.size());
    const ukm::mojom::UkmEntry* entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(entry, ukm::builders::Network_CacheTransparency::kFoundPervasivePayloadName, true);
}
...
TEST_F(ResourceLoaderCacheTransparencyTest, PervasivePayloadNotRequested)
{
    ...
    ASSERT_EQ(1u, entries.size());
    const ukm::mojom::UkmEntry* entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(entry, ukm::builders::Network_CacheTransparency::kFoundPervasivePayloadName, false);
}
