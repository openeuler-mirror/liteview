...
bool SubresourceIntegrity::CheckSubresourceIntegrity(
    const IntegrityMetadataSet& metadata_set, const char* content, size_t size, const KURL& resource_url, const Resource& resource, ReportInfo& report_info)
{
    ...
    return CheckSubresourceIntegrityImpl(metadata_set, content, size, resource_url, report_info);
}

bool SubresourceIntegrity::CheckSubresourceIntegrity(
    const String& integrity_metadata, IntegrityFeatures features, const char* content, size_t size, const KURL& resource_url, ReportInfo& report_info)
{
    if (integrity_metadata.empty())
        return true;

    IntegrityMetadataSet metadata_set;
    IntegrityParseResult integrity_parse_result = ParseIntegrityAttribute(integrity_metadata, features, metadata_set, &report_info);
...
bool SubresourceIntegrity::CheckSubresourceIntegrityImpl(
    const IntegrityMetadataSet& metadata_set, const char* content, size_t size, const KURL& resource_url, ReportInfo& report_info)
{
    ...
    for (const IntegrityMetadata& metadata : metadata_set) {
        if (metadata.Algorithm() == max_algorithm && CheckSubresourceIntegrityDigest(metadata, content, size)) {
    ...
    DigestValue digest;
    if (ComputeDigest(IntegrityAlgorithmToHashAlgorithm(max_algorithm), content, size, digest)) {
        ...
        report_info.AddConsoleErrorMessage("Failed to find a valid digest in the 'integrity' attribute for "
                                           "resource '"
            + resource_url.ElidedString() + "' with computed " + IntegrityAlgorithmToString(max_algorithm) + " integrity '" + Base64Encode(digest)
...
bool SubresourceIntegrity::CheckSubresourceIntegrityDigest(const IntegrityMetadata& metadata, const char* content, size_t size)
{
    ...
    converted_hash_vector.Append(reinterpret_cast<uint8_t*>(hash_vector.data()), hash_vector.size());
...
SubresourceIntegrity::IntegrityParseResult SubresourceIntegrity::ParseIntegrityAttribute(
    const WTF::String& attribute, IntegrityFeatures features, IntegrityMetadataSet& metadata_set)
{
