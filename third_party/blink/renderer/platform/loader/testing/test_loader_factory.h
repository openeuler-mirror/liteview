...
public:
    ...
    explicit TestLoaderFactory(WebURLLoaderMockFactory* mock_factory)
        : url_loader_factory_(std::make_unique<WebURLLoaderFactoryWithMock>(mock_factory))
    {
    }
        ...
        WebBackForwardCacheLoaderHelper back_forward_cache_loader_helper) override
    {
        ...
        return url_loader_factory_->CreateURLLoader(wrapped,
            scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(std::move(freezable_task_runner)),
            scheduler::WebResourceLoadingTaskRunnerHandle::CreateUnprioritized(std::move(unfreezable_task_runner)),
