...
template <bool is_enum, typename T> struct EnumOrGenericHashTraits;
template <typename T> struct HashTraits;

namespace {
template <typename T, bool = IsTraceable<T>::value> struct ClearMemoryAtomicallyIfNeeded {
    static void Clear(T* slot)
    {
        memset(static_cast<void*>(slot), 0, sizeof(T));
    }
};
template <typename T> struct ClearMemoryAtomicallyIfNeeded<T, true> {
    static void Clear(T* slot)
    {
        AtomicMemzero<sizeof(T), alignof(T)>(slot);
    }
};
} // namespace
...
template <typename T> struct GenericHashTraits : GenericHashTraitsBase<std::is_integral<T>::value, T> {
    ...
    static IteratorConstReferenceType GetToReferenceConstConversion(IteratorConstGetType x)
    {
        return *x;
    }

    template <typename IncomingValueType> static void Store(IncomingValueType&& value, T& storage)
    {
...
template <typename T> struct EnumOrGenericHashTraits<false, T> : GenericHashTraits<T> { };

// Default traits for an enum type.  0 is very popular, and -1 is also popular.
// So we use -128 and -127.
template <typename T> struct EnumOrGenericHashTraits<true, T> : GenericHashTraits<T> {
    static const bool kEmptyValueIsZero = false;
    static T EmptyValue()
    {
    ...
    static bool IsDeletedValue(T value)
    {
        return value == static_cast<T>(-127);
    }
};

template <typename T> struct HashTraits : EnumOrGenericHashTraits<std::is_enum<T>::value, T> { };
...
// all values as keys.
template <typename T> struct HashTraits<IntegralWithAllKeys<T>> : SimpleClassHashTraits<IntegralWithAllKeys<T>> { };

template <typename P> struct HashTraits<scoped_refptr<P>> : SimpleClassHashTraits<scoped_refptr<P>> {
...
template <typename T> struct HashTraits<std::unique_ptr<T>> : SimpleClassHashTraits<std::unique_ptr<T>> {
...
{
    return HashTraitsEmptyValueChecker<Traits, Traits::kHasIsEmptyValueFunction>::IsEmptyValue(value);
}
...
struct PairHashTraits : GenericHashTraits<std::pair<typename FirstTraitsArg::TraitType, typename SecondTraitsArg::TraitType>> {
    ...
    static const bool kEmptyValueIsZero = FirstTraits::kEmptyValueIsZero && SecondTraits::kEmptyValueIsZero;
    static EmptyValueType EmptyValue()
    {
        return std::make_pair(FirstTraits::EmptyValue(), SecondTraits::EmptyValue());
    }

    static const bool kHasIsEmptyValueFunction = FirstTraits::kHasIsEmptyValueFunction || SecondTraits::kHasIsEmptyValueFunction;
    ...
    static void ConstructDeletedValue(TraitType& slot, bool zero_value)
    {
        ...
        if (zero_value) {
            ClearMemoryAtomicallyIfNeeded<typename SecondTraits::TraitType>::Clear(&slot.second);
        }
    }
...
struct KeyValuePairHashTraits : GenericHashTraits<KeyValuePair<typename KeyTraitsArg::TraitType, typename ValueTraitsArg::TraitType>> {
    ...
    static const bool kEmptyValueIsZero = KeyTraits::kEmptyValueIsZero && ValueTraits::kEmptyValueIsZero;
    static EmptyValueType EmptyValue()
    {
    ...
    static void ConstructDeletedValue(TraitType& slot, bool zero_value)
    {
        ...
        if (zero_value) {
            ClearMemoryAtomicallyIfNeeded<typename ValueTraits::TraitType>::Clear(&slot.value);
        }
    }
    static bool IsDeletedValue(const TraitType& value)
    {
    ...
    static constexpr bool kCanTraceConcurrently
        = KeyTraitsArg::kCanTraceConcurrently && (ValueTraitsArg::kCanTraceConcurrently || !IsTraceable<typename ValueTraitsArg::TraitType>::value);
};
...
template <typename T> struct NullableHashTraits : public HashTraits<T> {
    static const bool kEmptyValueIsZero = false;
    static T EmptyValue()
    {
