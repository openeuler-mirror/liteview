...
public:
    ...
    template <typename VisitorDispathcer, typename A = Allocator> std::enable_if_t<A::kIsGarbageCollected> Trace(VisitorDispathcer visitor) const
    {
...
private:
    ...
    template <typename A = Allocator> std::enable_if_t<A::kIsGarbageCollected> ProcessCustomWeakness(const typename A::LivenessBroker& broker)
    {
        auto it = begin();
        while (it != end()) {
...
template <typename VectorBackedLinkedListType> class VectorBackedLinkedListIterator {
    ...
    using Node = typename VectorBackedLinkedListConstIterator<VectorBackedLinkedListType>::Node;
...
private:
    ...
    {
        return const_cast<PointerType>(iterator_.Get());
    }
