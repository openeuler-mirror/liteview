...
public:
    ...
    static StringImpl* CreateStatic(const char* string, wtf_size_t length, wtf_size_t hash);
    ...
    static scoped_refptr<StringImpl> Create(const LChar*, wtf_size_t length, ASCIIStringAttributes ascii_attributes);
    static scoped_refptr<StringImpl> Create8BitIfPossible(const UChar*, wtf_size_t length);
    template <wtf_size_t inlineCapacity> static scoped_refptr<StringImpl> Create8BitIfPossible(const Vector<UChar, inlineCapacity>& vector)
    {
        return Create8BitIfPossible(vector.data(), vector.size());
    }

    ALWAYS_INLINE static scoped_refptr<StringImpl> Create(const char* s, wtf_size_t length)
    {
    ...
    void SetHash(wtf_size_t hash) const
    {
        ...
        // hash function.
        DCHECK(hash
            == (Is8Bit() ? StringHasher::ComputeHashAndMaskTop8Bits(Characters8(), length_)
...
template <> ALWAYS_INLINE size_t StringImpl::AllocationSize<LChar>(wtf_size_t length)
{
    ...
    size_t ret = base::CheckAdd(sizeof(StringImpl), length).ValueOrDie();
#ifdef _DEBUG
    ret += 10; // weolars: c_str
#endif
    return ret;
}
...
template <> struct DefaultHash<scoped_refptr<StringImpl>> {
    typedef StringHash Hash;
};
