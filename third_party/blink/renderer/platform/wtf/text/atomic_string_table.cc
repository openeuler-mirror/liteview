...
class UCharBuffer {
public:
    UCharBuffer(const UChar* chars, unsigned len, AtomicStringUCharEncoding encoding)
        : characters_(chars)
        , length_(len)
...
public:
    HashAndUTF8Characters(const char* chars_start, const char* chars_end)
        ...
        , hash_(unicode::CalculateStringHashAndLengthFromUTF8MaskingTop8Bits(chars_start, chars_end, length_, utf16_length_))
    {
    }
...
struct HashAndUTF8CharactersTranslator {
    ...
    static bool Equal(StringImpl* const& string, const HashAndUTF8Characters& buffer)
    {
        ...
        if (buffer.utf16_length() != buffer.length()) {
            if (string->Is8Bit()) {
                ...
                return unicode::EqualLatin1WithUTF8(characters8, characters8 + string->length(), buffer.characters(), buffer.characters() + buffer.length());
            }
            const UChar* characters16 = string->Characters16();
            return unicode::EqualUTF16WithUTF8(characters16, characters16 + string->length(), buffer.characters(), buffer.characters() + buffer.length());
        }
    ...
    static void Translate(StringImpl*& location, const HashAndUTF8Characters& buffer, unsigned hash)
    {
        ...
        if (!is_all_ascii) {
            ...
            if (unicode::ConvertUTF8ToUTF16(&source, source + buffer.length(), &target, target + buffer.utf16_length(), &is_all_ascii)
...
struct StringViewLookupTranslator {
    static unsigned GetHash(const StringView& buf)
    {
        ...
        if (buf.Is8Bit()) {
            return StringHasher::ComputeHashAndMaskTop8Bits(buf.Characters8(), buf.length());
        } else {
            return StringHasher::ComputeHashAndMaskTop8Bits(buf.Characters16(), buf.length());
        }
    }
...
public:
        ...
        : impl_(impl)
    {
        ...
        if (impl_->Is8Bit()) {
            hash_ = StringHasher::ComputeHashAndMaskTop8Bits<LChar, ToASCIILowerUChar<LChar>>(impl_->Characters8(), impl_->length());
        } else {
            hash_ = StringHasher::ComputeHashAndMaskTop8Bits<UChar, ToASCIILowerUChar<UChar>>(impl_->Characters16(), impl_->length());
        }
    }
...
private:
    template <typename CharType> static UChar ToASCIILowerUChar(CharType ch)
    {
