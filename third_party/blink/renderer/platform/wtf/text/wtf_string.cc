...
String::String(const char* characters, unsigned length)
    : impl_(characters ? StringImpl::Create(reinterpret_cast<const LChar*>(characters), length) : nullptr)
{
}
...
String String::FromUTF8(const LChar* string_start, size_t string_length)
{
    ...
    unsigned utf16_length = static_cast<wtf_size_t>(buffer_current - buffer_start);
...
void String::WriteIntoTrace(perfetto::TracedValue context) const
{
    ...
    StringUTF8Adaptor adaptor(Is8Bit() ? StringView(Characters8(), length()) : StringView(Characters16(), length()));
