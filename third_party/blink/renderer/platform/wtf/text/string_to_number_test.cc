...
#define EXPECT_VALID(string, options, expectedValue)                                                                                                           \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        int value = CharactersToInt(reinterpret_cast<const LChar*>(string), std::strlen(string), options, &ok);                                                \
        EXPECT_TRUE(ok);                                                                                                                                       \
...
#define EXPECT_INVALID(string, options)                                                                                                                        \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        CharactersToInt(reinterpret_cast<const LChar*>(string), std::strlen(string), options, &ok);                                                            \
        EXPECT_FALSE(ok);                                                                                                                                      \
    } while (false)
...
#define EXPECT_VALID(string, options, expectedValue)                                                                                                           \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        unsigned value = CharactersToUInt(reinterpret_cast<const LChar*>(string), std::strlen(string), options, &ok);                                          \
        EXPECT_TRUE(ok);                                                                                                                                       \
        EXPECT_EQ(value, expectedValue);                                                                                                                       \
    } while (false)
...
#define EXPECT_INVALID(string, options)                                                                                                                        \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        CharactersToUInt(reinterpret_cast<const LChar*>(string), std::strlen(string), options, &ok);                                                           \
        EXPECT_FALSE(ok);                                                                                                                                      \
    } while (false)
...
#define EXPECT_VALID(string, expectedValue)                                                                                                                    \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        unsigned value = HexCharactersToUInt(reinterpret_cast<const LChar*>(string), std::strlen(string), NumberParsingOptions::Strict(), &ok);                \
        EXPECT_TRUE(ok);                                                                                                                                       \
        EXPECT_EQ(value, expectedValue);                                                                                                                       \
    } while (false)
...
#define EXPECT_INVALID(string)                                                                                                                                 \
    do {                                                                                                                                                       \
        bool ok;                                                                                                                                               \
        HexCharactersToUInt(reinterpret_cast<const LChar*>(string), std::strlen(string), NumberParsingOptions::Strict(), &ok);                                 \
        EXPECT_FALSE(ok);                                                                                                                                      \
    } while (false)
...
NumberParsingResult ParseUInt(const char* str, unsigned* value)
{
    ...
    *value = CharactersToUInt(reinterpret_cast<const LChar*>(str), std::strlen(str), NumberParsingOptions::Strict(), &result);
    return result;
}
...
{
    bool ok;
    double value = CharactersToDouble(reinterpret_cast<const LChar*>(str), std::strlen(str), &ok);
    EXPECT_TRUE(ok) << "\"" << str << "\"";
...
{
    bool ok;
    CharactersToDouble(reinterpret_cast<const LChar*>(str), std::strlen(str), &ok);
    EXPECT_FALSE(ok) << "\"" << str << "\"";
}
...
{
    size_t parsed;
    CharactersToDouble(reinterpret_cast<const LChar*>(str), std::strlen(str), parsed);
    return parsed;
}
...
{
    bool ok;
    float value = CharactersToFloat(reinterpret_cast<const LChar*>(str), std::strlen(str), &ok);
    EXPECT_TRUE(ok) << "\"" << str << "\"";
    EXPECT_EQ(expected_value, value);
}
...
{
    size_t parsed;
    CharactersToFloat(reinterpret_cast<const LChar*>(str), std::strlen(str), parsed);
    return parsed;
}
