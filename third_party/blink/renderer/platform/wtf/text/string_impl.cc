...
#ifdef _DEBUG
    StringImpl* string = static_cast<StringImpl*>(Partitions::BufferMalloc(AllocationSize<LChar>(length + 1), "WTF::StringImpl"));
    memset(string, 0, sizeof(StringImpl) + length + 1);
    string->debug_stringA = (char*)(string + 1);
#else
...
#ifdef _DEBUG
    StringImpl* string = static_cast<StringImpl*>(Partitions::BufferMalloc(AllocationSize<UChar>(length + 1), "WTF::StringImpl"));
    memset(string, 0, sizeof(StringImpl) + (length + 1) * sizeof(UChar));
    string->debug_stringA = (char*)(string + 1);
#else
...
StringImpl* StringImpl::CreateStatic(const char* string, wtf_size_t length, wtf_size_t hash)
{
#if DCHECK_IS_ON()
...
scoped_refptr<StringImpl> StringImpl::Create(const UChar* characters, wtf_size_t length)
{
...
scoped_refptr<StringImpl> StringImpl::Create(const LChar* characters, wtf_size_t length)
{
    if (!characters || !length)
        return empty_;
...
scoped_refptr<StringImpl> StringImpl::Create(const LChar* characters, wtf_size_t length, ASCIIStringAttributes ascii_attributes)
{
...
scoped_refptr<StringImpl> StringImpl::Create8BitIfPossible(const UChar* characters, wtf_size_t length)
{
    if (!characters || !length)
        return empty_;
...
{
    if (Is8Bit())
        return String::Make16BitFrom8BitSource(Characters8(), length_).ReleaseImpl();
    return this;
}
...
int CodeUnitCompareIgnoringASCIICase(const StringImpl* string1, const StringImpl* string2)
{
    ...
    return VisitCharacters(*string2, [string1](const auto* chars, wtf_size_t length) { return CodeUnitCompareIgnoringASCIICase(string1, chars, length); });
}
