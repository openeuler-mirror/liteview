...
inline double ParseDouble(const LChar* string, size_t length, size_t& parsed_length)
{
    int int_parsed_length = 0;
    double d = internal::GetDoubleConverter().StringToDouble(reinterpret_cast<const char*>(string), base::saturated_cast<int>(length), &int_parsed_length);
    parsed_length = int_parsed_length;
    return d;
}

inline double ParseDouble(const UChar* string, size_t length, size_t& parsed_length)
{
