...
struct CustomHashTraitsForInt : public HashTraits<int> {
    ...
    static void ConstructDeletedValue(int& slot, bool)
    {
        slot = INT_MIN;
    }
    static bool IsDeletedValue(const int& value)
    {
...
template <> struct DefaultHash<EmptyString> {
    struct Hash {
...
struct ComplicatedHashTraits : GenericHashTraits<Complicated> {
    static const bool kEmptyValueIsZero = false;
    static const Complicated EmptyValue()
    {
...
{
    using Set = LinkedHashSet<Complicated, ComplicatedHashTraits, ComplicatedHashFunctions>;
    Set set;
