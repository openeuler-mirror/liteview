...
template <typename T> struct IntHash {
    ...
    {
        return HashInt(static_cast<typename IntTypes<sizeof(T)>::UnsignedType>(key));
    }
...
template <typename T> struct FloatHash {
    ...
    static unsigned GetHash(T key)
    {
        return HashInt(base::bit_cast<Bits>(key));
    }
    static bool Equal(T a, T b)
    {
...
#pragma warning(pop)
    ...
    static bool Equal(std::nullptr_t, T* b)
    {
        return !b;
    }
    static bool Equal(T* a, std::nullptr_t)
    {
        return !a;
    }
    static const bool safe_to_compare_to_empty_or_deleted = true;
...
template <typename T> struct RefPtrHash : PtrHash<T> {
    ...
    static bool Equal(T* a, const scoped_refptr<T>& b)
    {
        return a == b;
    }
    static bool Equal(const scoped_refptr<T>& a, T* b)
    {
        return a == b;
    }
};

template <typename T> struct UniquePtrHash : PtrHash<T> {
    using PtrHash<T>::GetHash;
...
// yourself.
template <typename T, bool isIntegral> struct DefaultHashImpl;

template <typename T> struct DefaultHashImpl<T, true> {
    using Hash = IntHash<T>;
};

// Canonical implementation of DefaultHash.
template <typename T> struct DefaultHash : DefaultHashImpl<T, std::is_integral<T>::value || std::is_enum<T>::value> { };
...
template <typename T> struct DefaultHash<scoped_refptr<T>> {
...
template <typename T, typename U, bool areBothIntegral> struct PairHashImpl {
    static unsigned GetHash(const std::pair<T, U>& p)
    {
        return HashInts(DefaultHash<T>::Hash::GetHash(p.first), DefaultHash<U>::Hash::GetHash(p.second));
    }
    ...
    static const bool safe_to_compare_to_empty_or_deleted
        = DefaultHash<T>::Hash::safe_to_compare_to_empty_or_deleted && DefaultHash<U>::Hash::safe_to_compare_to_empty_or_deleted;
};

// Special version for pairs of integrals:
template <typename T, typename U> struct PairHashImpl<T, U, true> {
    static unsigned GetHash(const std::pair<T, U>& p)
    {
    ...
    static const bool safe_to_compare_to_empty_or_deleted = PairHashImpl<T, U, false>::safe_to_compare_to_empty_or_deleted;
};

// Combined version:
template <typename T, typename U> struct PairHash : PairHashImpl<T, U, std::is_integral<T>::value && std::is_integral<U>::value> { };
...
template <typename T> struct IntegralWithAllKeys {
    IntegralWithAllKeys()
        : IntegralWithAllKeys(0, ValueType::kEmpty)
    {
    }
...
// including 0 and max.
template <typename T> struct IntegralWithAllKeysHash {
    static unsigned GetHash(const IntegralWithAllKeys<T>& key)
    {
        return key.Hash();
    }
    static bool Equal(const IntegralWithAllKeys<T>& a, const IntegralWithAllKeys<T>& b)
    {
        return a == b;
    }
    static const bool safe_to_compare_to_empty_or_deleted = true;
...
template <typename T> struct DefaultHash<IntegralWithAllKeys<T>> {
