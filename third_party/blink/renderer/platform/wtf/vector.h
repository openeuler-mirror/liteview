...
template <typename T, typename Allocator> struct VectorTypeOperations {
    ...
    static void Move(T* const src, T* const src_end, T* const dst, VectorOperationOrigin origin)
    {
        ...
        } else if constexpr (Allocator::kIsGarbageCollected && IsTraceableInCollectionTrait<VectorTraits<T>>::value) {
    ...
    template <typename U> static void UninitializedCopy(const U* src, const U* src_end, T* dst, VectorOperationOrigin origin)
    {
        if (!LIKELY(dst && src))
            return;
...
private:
    static constexpr bool NeedsToClearUnusedSlots()
    {
        ...
        return Allocator::kIsGarbageCollected && (IsTraceableInCollectionTrait<VectorTraits<T>>::value || VectorTraits<T>::kNeedsDestruction);
    }
...
public:
    ...
    void SwapVectorBuffer(VectorBuffer<T, inlineCapacity, Allocator>& other, OffsetRange this_hole, OffsetRange other_hole, VectorOperationOrigin this_origin)
    {
        ...
        static_assert(VectorTraits<T>::kCanSwapUsingCopyOrMove, "Cannot swap using copy or move.");
...
template <typename T, wtf_size_t inlineCapacity, bool isGced> struct VectorNeedsDestructor {
    static constexpr bool value = true;
};

// For garbage collection, Vector does not require destruction when there's no
// inline capacity.
template <typename T> struct VectorNeedsDestructor<T, 0, true> {
...
template <typename T, wtf_size_t inlineCapacity> struct VectorNeedsDestructor<T, inlineCapacity, true> {
...
//       public ConditionalDestructor<Vector2<T, INLINE_CAPACITY, Allocator>, VectorNeedsDestructor<T, INLINE_CAPACITY, Allocator::kIsGarbageCollected>::value> {
//     USE_ALLOCATOR(Vector2, Allocator);
// 
// public:
//     ~Vector2()
//     {
...
class Vector
    ...
    , VectorNeedsDestructor<T, INLINE_CAPACITY, Allocator::kIsGarbageCollected>::value> {
...
public:
    ...
    template <typename Collection,
        ...
        // Vector(3).
        typename = typename std::enable_if<std::is_class<Collection>::value>::type>
    explicit Vector(const Collection& collection)
        : Vector()
    {
        assign(collection);
    }
    // Replaces the vector with items copied from a collection.
    template <typename Collection> void assign(const Collection&);
    ...
    template <typename A = Allocator> std::enable_if_t<!A::kIsGarbageCollected> Fill(const T&, wtf_size_t);
    template <typename A = Allocator> std::enable_if_t<!A::kIsGarbageCollected> Fill(const T& val)
    {
        Fill(val, size());
    }
    ...
    void Finalize()
    {
        ...
        if (LIKELY(size_) && !(Allocator::kIsGarbageCollected && this->HasOutOfLineBuffer())) {
...
// static
template <typename T, wtf_size_t inlineCapacity, typename Allocator> constexpr void Vector<T, inlineCapacity, Allocator>::CheckTypeConstraints()
{
    ...
    static_assert(Allocator::kIsGarbageCollected || !IsDisallowNew<T>::value || !IsTraceable<T>::value,
    ...
    : Base(size)
{
    ...
    ANNOTATE_NEW_BUFFER(begin(), capacity(), size);
    size_ = size;
    TypeOperations::UninitializedFill(begin(), end(), val, VectorOperationOrigin::kConstruction);
}
    ...
    : Base(other.capacity())
{
    ...
    size_ = other.size();
    TypeOperations::UninitializedCopy(other.begin(), other.end(), begin(), VectorOperationOrigin::kConstruction);
}
...
Vector<T, inlineCapacity, Allocator>& Vector<T, inlineCapacity, Allocator>::operator=(const Vector<T, inlineCapacity, Allocator>& other)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, other.size());
    TypeOperations::Copy(other.begin(), other.begin() + size(), begin(), VectorOperationOrigin::kRegularModification);
    TypeOperations::UninitializedCopy(other.begin() + size(), other.end(), end(), VectorOperationOrigin::kRegularModification);
    size_ = other.size();
...
{
    static_assert(!std::is_same<Vector<T, inlineCapacity, Allocator>, Collection>::value, "This method is for copying from a collection of a different type.");
    ...
    : Base(base::checked_cast<wtf_size_t>(elements.size()))
{
    ...
    TypeOperations::UninitializedCopy(elements.begin(), elements.end(), begin(), VectorOperationOrigin::kConstruction);
}
...
Vector<T, inlineCapacity, Allocator>& Vector<T, inlineCapacity, Allocator>::operator=(std::initializer_list<T> elements)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, input_size);
    TypeOperations::Copy(elements.begin(), elements.begin() + size_, begin(), VectorOperationOrigin::kRegularModification);
    TypeOperations::UninitializedCopy(elements.begin() + size_, elements.end(), end(), VectorOperationOrigin::kRegularModification);
    size_ = input_size;
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator>
template <typename A>
std::enable_if_t<!A::kIsGarbageCollected> Vector<T, inlineCapacity, Allocator>::Fill(const T& val, wtf_size_t new_size)
{
    if (size() > new_size) {
        Shrink(new_size);
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, new_size);
    std::fill(begin(), end(), val);
    TypeOperations::UninitializedFill(end(), begin() + new_size, val, VectorOperationOrigin::kRegularModification);
    size_ = new_size;
}
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator> inline void Vector<T, inlineCapacity, Allocator>::resize(wtf_size_t size)
{
    if (size <= size_) {
        ...
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size);
    } else {
        if (size > capacity())
            ExpandCapacity(size);
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size);
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator> void Vector<T, inlineCapacity, Allocator>::reserve(wtf_size_t new_capacity)
{
    ...
    if constexpr (Allocator::kIsGarbageCollected) {
        ...
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), old_capacity, size_, old_capacity);
...
ALWAYS_INLINE void Vector<T, inlineCapacity, Allocator>::push_back(U&& val)
{
    ...
    if (LIKELY(size() != capacity())) {
        MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size_ + 1);
        ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(end(), std::forward<U>(val));
        ++size_;
        return;
    }
...
ALWAYS_INLINE T& Vector<T, inlineCapacity, Allocator>::emplace_back(Args&&... args)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size_ + 1);
    T* t = ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(end(), std::forward<Args>(args)...);
    ++size_;
    return *t;
}
...
NOINLINE void Vector<T, inlineCapacity, Allocator>::AppendSlowCase(U&& val)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size_ + 1);
    ConstructTraits<T, VectorTraits<T>, Allocator>::ConstructAndNotifyElement(end(), std::forward<U>(*ptr));
    ++size_;
}
...
inline void Vector<T, inlineCapacity, Allocator>::insert(wtf_size_t position, U&& val)
{
    ...
    TypeOperations::MoveOverlapping(spot, end(), spot + 1, VectorOperationOrigin::kRegularModification);
...
void Vector<T, inlineCapacity, Allocator>::insert(wtf_size_t position, const U* data, wtf_size_t data_size)
{
    ...
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, new_size);
    T* spot = begin() + position;
    TypeOperations::MoveOverlapping(spot, end(), spot + data_size, VectorOperationOrigin::kRegularModification);
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator>
template <typename U>
void Vector<T, inlineCapacity, Allocator>::InsertAt(T* position, const U* data, wtf_size_t data_size)
{
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator> inline void Vector<T, inlineCapacity, Allocator>::EraseAt(wtf_size_t position)
{
    ...
    T* spot = begin() + position;
    spot->~T();
    TypeOperations::MoveOverlapping(spot + 1, end(), spot, VectorOperationOrigin::kRegularModification);
...
inline void Vector<T, inlineCapacity, Allocator>::EraseAt(wtf_size_t position, wtf_size_t length)
{
    ...
    TypeOperations::MoveOverlapping(end_spot, end(), begin_spot, VectorOperationOrigin::kRegularModification);
    ClearUnusedSlots(end() - length, end());
    MARKING_AWARE_ANNOTATE_CHANGE_SIZE(Allocator, begin(), capacity(), size_, size_ - length);
    size_ -= length;
}
...
std::enable_if_t<A::kIsGarbageCollected> Vector<T, inlineCapacity, Allocator>::Trace(VisitorDispatcher visitor) const
{
    ...
    if (Base::IsOutOfLineBuffer(buffer)) {
        ...
        if (!VectorTraits<T>::kCanTraceConcurrently) {
            if (Allocator::DeferTraceToMutatorThreadIfConcurrent(
                    visitor, buffer, internal::DeferredTraceImpl<Allocator, VisitorDispatcher, T, inlineCapacity>, inlineCapacity * sizeof(T))) {
                return;
            }
        }
...
template <typename T, wtf_size_t inlineCapacity, typename Allocator> void Vector<T, inlineCapacity, Allocator>::ReallocateBuffer(wtf_size_t new_capacity)
{
...
#endif
        Base::ResetBufferPointer();
        TypeOperations::Move(old_begin, old_end, begin(), VectorOperationOrigin::kRegularModification);
    ...
    // white.
    TypeOperations::Move(begin(), end(), temp_buffer.Buffer(), VectorOperationOrigin::kConstruction);
...
// for std::vector
template <typename T> struct is_trivially_copy_constructible<WTF::Vector<T>> : std::false_type { };
#endif
