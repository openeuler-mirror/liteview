...
// and its size is different from the value which APIs tells us.
...
#elif defined(__GLIBC__) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FREEBSD) || BUILDFLAG(IS_FUCHSIA)
    ...
    printf("Stack frame size estimation not supported on this platform.");
    *(int*)1 = 1;
    return 0;
#else
...
void* GetStackStart()
{
#if defined(__GLIBC__) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FREEBSD) || BUILDFLAG(IS_FUCHSIA)
    pthread_attr_t attr;
    int error;
#if BUILDFLAG(IS_FREEBSD)
    pthread_attr_init(&attr);
...
#endif
#elif defined(OS_OHOS)
    printf("Unsupported getStackStart on this platform");
    *(int*)1 = 1;
    return nullptr;
#else
