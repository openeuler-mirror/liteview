...
// }
template <typename T> struct DowncastTraits {
...
auto IsDowncastAllowedHelper(const Base& from, base::internal::priority_tag<1>) -> decltype(DowncastTraits<Derived>::AllowFrom(from))
{
...
// Though redundant with the return type inferred by `auto`, the trailing return
// type is needed for SFINAE.
template <typename Derived, typename Base> auto IsDowncastAllowedHelper(const Base& from, base::internal::priority_tag<0>) -> decltype(Derived::IsClassOf(from))
{
...
{
    return internal::IsDowncastAllowedHelper<Derived>(from, base::internal::priority_tag<1>());
}
