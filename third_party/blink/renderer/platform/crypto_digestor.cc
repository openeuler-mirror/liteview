...
bool ComputeDigest(HashAlgorithm algorithm, const char* digestable, size_t length, DigestValue& digest_result)
{
#if 0 // ndef OPENSSL_HEADER_CRYPTO_H
    unsigned hash = StringHasher::ComputeHash((const LChar*)digestable, length);
    if (kHashAlgorithmSha256 == algorithm) {
        digest_result.resize(32);
        sprintf((char*)digest_result.data(), "ComputeDigest_%x\n", hash);
    } else
        CHECK(false);
