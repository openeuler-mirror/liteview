...
struct VideoCaptureImpl::BufferContext : public base::RefCountedThreadSafe<BufferContext> {
public:
    ...
    static void DestroyTextureOnMediaThread(media::GpuVideoAcceleratorFactories* gpu_factories, gpu::Mailbox mailbox, gpu::SyncToken release_sync_token)
    {
        if (!mailbox.IsZero()) {
...
private:
    ...
    void InitializeFromMailbox(media::mojom::blink::MailboxBufferHandleSetPtr mailbox_handles)
    {
        DCHECK_EQ(media::VideoFrame::kMaxPlanes, mailbox_handles->mailbox_holder.size());
    ...
    virtual ~BufferContext()
    {
        ...
        for (size_t plane = 0; plane < media::VideoFrame::kMaxPlanes; ++plane) {
            ...
            media_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(
                    &BufferContext::DestroyTextureOnMediaThread, gpu_factories_, gmb_resources_->mailboxes[plane], gmb_resources_->release_sync_token));
        }
    }
...
VideoCaptureImpl::VideoFrameBufferPreparer::VideoFrameBufferPreparer(VideoCaptureImpl& video_capture_impl, media::mojom::blink::ReadyBufferPtr ready_buffer)
...
const media::mojom::blink::VideoFrameInfoPtr& VideoCaptureImpl::VideoFrameBufferPreparer::frame_info() const
{
    return frame_info_;
}

scoped_refptr<media::VideoFrame> VideoCaptureImpl::VideoFrameBufferPreparer::frame() const
{
    return frame_;
}

scoped_refptr<VideoCaptureImpl::BufferContext> VideoCaptureImpl::VideoFrameBufferPreparer::buffer_context() const
{
...
bool VideoCaptureImpl::VideoFrameBufferPreparer::Initialize()
{
    ...
    case VideoFrameBufferHandleType::kUnsafeShmemRegion:
        ...
        if (frame_info_->strides) {
            CHECK(IsYuvPlanar(frame_info_->pixel_format) && (media::VideoFrame::NumPlanes(frame_info_->pixel_format) == 3))
            ...
            uint8_t* u_data = y_data
                + (media::VideoFrame::Rows(media::VideoFrame::kYPlane, frame_info_->pixel_format, frame_info_->coded_size.height())
                    * frame_info_->strides->stride_by_plane[0]);
            uint8_t* v_data = u_data
                + (media::VideoFrame::Rows(media::VideoFrame::kUPlane, frame_info_->pixel_format, frame_info_->coded_size.height())
                    * frame_info_->strides->stride_by_plane[1]);
            frame_ = media::VideoFrame::WrapExternalYuvData(frame_info_->pixel_format, gfx::Size(frame_info_->coded_size), gfx::Rect(frame_info_->visible_rect),
                frame_info_->visible_rect.size(), frame_info_->strides->stride_by_plane[0], frame_info_->strides->stride_by_plane[1],
                frame_info_->strides->stride_by_plane[2], y_data, u_data, v_data, frame_info_->timestamp);
        } else {
            frame_ = media::VideoFrame::WrapExternalData(frame_info_->pixel_format, gfx::Size(frame_info_->coded_size), gfx::Rect(frame_info_->visible_rect),
                frame_info_->visible_rect.size(), const_cast<uint8_t*>(buffer_context_->data()), buffer_context_->data_size(), frame_info_->timestamp);
        }
        break;
    case VideoFrameBufferHandleType::kReadOnlyShmemRegion:
    ...
    case VideoFrameBufferHandleType::kMailboxHandles: {
        ...
        CHECK_EQ(media::VideoFrame::kMaxPlanes, buffer_context_->mailbox_holders().size());
        for (int i = 0; i < media::VideoFrame::kMaxPlanes; i++) {
            mailbox_holder_array[i] = buffer_context_->mailbox_holders()[i];
        }
        frame_ = media::VideoFrame::WrapNativeTextures(frame_info_->pixel_format, mailbox_holder_array, media::VideoFrame::ReleaseMailboxCB(),
            gfx::Size(frame_info_->coded_size), gfx::Rect(frame_info_->visible_rect), frame_info_->visible_rect.size(), frame_info_->timestamp);
        break;
    }
    case VideoFrameBufferHandleType::kGpuMemoryBufferHandle: {
#if BUILDFLAG(IS_MAC)
        ...
        // used by both hardware and software paths.
        // https://crbug.com/1125879
        if (!video_capture_impl_.gpu_factories_ || !video_capture_impl_.media_task_runner_) {
            frame_ = media::VideoFrame::WrapUnacceleratedIOSurface(
                buffer_context_->TakeGpuMemoryBufferHandle(), gfx::Rect(frame_info_->visible_rect), frame_info_->timestamp);
            break;
        }
#endif
#if BUILDFLAG(IS_WIN)
        ...
        if (frame_info_->is_premapped && !buffer_context_->data()) {
            ...
            buffer_context_->InitializeFromUnsafeShmemRegion(std::move(gmb_handle.region));
        ...
        // GPU backed frames.
        if (!video_capture_impl_.gpu_factories_ || !video_capture_impl_.media_task_runner_ || video_capture_impl_.gmb_not_supported_) {
        ...
        if (!buffer_context_->GetGpuMemoryBuffer()) {
            ...
            auto gmb = video_capture_impl_.gpu_memory_buffer_support_->CreateGpuMemoryBufferImplFromHandle(buffer_context_->TakeGpuMemoryBufferHandle(),
                gfx::Size(frame_info_->coded_size), gfx_format, gfx::BufferUsage::SCANOUT_VEA_CPU_READ, base::DoNothing(),
                video_capture_impl_.gpu_factories_->GpuMemoryBufferManager(), video_capture_impl_.pool_);
        ...
        uint8_t* premapped_data = frame_info_->is_premapped ? const_cast<uint8_t*>(buffer_context_->data()) : nullptr;

        // Clone the GpuMemoryBuffer and wrap it in a VideoFrame.
        gpu_memory_buffer_ = video_capture_impl_.gpu_memory_buffer_support_->CreateGpuMemoryBufferImplFromHandle(std::move(buffer_handle),
            buffer_context_->GetGpuMemoryBuffer()->GetSize(), buffer_context_->GetGpuMemoryBuffer()->GetFormat(), gfx::BufferUsage::SCANOUT_VEA_CPU_READ,
            base::DoNothing(), video_capture_impl_.gpu_factories_->GpuMemoryBufferManager(), video_capture_impl_.pool_,
            base::span<uint8_t>(premapped_data, buffer_context_->data_size()));
...
bool VideoCaptureImpl::VideoFrameBufferPreparer::BindVideoFrameOnMediaThread(media::GpuVideoAcceleratorFactories* gpu_factories)
{
    ...
    const gpu::Capabilities* context_capabilites = gpu_factories->ContextCapabilities();
    ...
    const auto output_format = buffer_context_->gpu_factories()->VideoFrameOutputFormat(frame_info_->pixel_format);
    DCHECK(output_format == media::GpuVideoAcceleratorFactories::OutputFormat::NV12_SINGLE_GMB
        || output_format == media::GpuVideoAcceleratorFactories::OutputFormat::NV12_DUAL_GMB);
    ...
    unsigned texture_target = buffer_context_->gpu_factories()->ImageTextureTarget(gpu_memory_buffer_->GetFormat());
    ...
    for (size_t plane = 0; plane < planes.size(); ++plane) {
        if (should_recreate_shared_image || buffer_context_->gmb_resources()->mailboxes[plane].IsZero()) {
            buffer_context_->gmb_resources()->mailboxes[plane]
                = sii->CreateSharedImage(gpu_memory_buffer_.get(), buffer_context_->gpu_factories()->GpuMemoryBufferManager(), planes[plane],
                    *(frame_info_->color_space), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
        } else {
            sii->UpdateSharedImage(buffer_context_->gmb_resources()->release_sync_token, buffer_context_->gmb_resources()->mailboxes[plane]);
        }
    }
    ...
    for (size_t plane = 0; plane < planes.size(); ++plane) {
        ...
        mailbox_holder_array[plane] = gpu::MailboxHolder(buffer_context_->gmb_resources()->mailboxes[plane], sync_token, texture_target);
    }

    const auto gmb_size = gpu_memory_buffer_->GetSize();
    frame_ = media::VideoFrame::WrapExternalGpuMemoryBuffer(gfx::Rect(frame_info_->visible_rect), gmb_size, std::move(gpu_memory_buffer_), mailbox_holder_array,
        base::BindOnce(&BufferContext::MailboxHolderReleased, buffer_context_), frame_info_->timestamp);
    if (!frame_) {
...
{
    DCHECK(frame_);
    frame_->AddDestructionObserver(base::BindOnce(&VideoCaptureImpl::DidFinishConsumingFrame,
        media::BindToCurrentLoop(base::BindOnce(
            &VideoCaptureImpl::OnAllClientsFinishedConsumingFrame, video_capture_impl_.weak_factory_.GetWeakPtr(), buffer_id_, buffer_context_))));
    if (frame_info_->color_space.has_value() && frame_info_->color_space->IsValid()) {
...
VideoCaptureImpl::VideoCaptureImpl(
    media::VideoCaptureSessionId session_id, scoped_refptr<base::SequencedTaskRunner> main_task_runner, BrowserInterfaceBrokerProxy* browser_interface_broker)
...
void VideoCaptureImpl::StartCapture(int client_id, const media::VideoCaptureParams& params, const VideoCaptureStateUpdateCB& state_update_cb,
    const VideoCaptureDeliverFrameCB& deliver_frame_cb, const VideoCaptureCropVersionCB& crop_version_cb)
{
...
void VideoCaptureImpl::OnStateChanged(media::mojom::blink::VideoCaptureResultPtr result)
{
    ...
    if (result->which() == media::mojom::blink::VideoCaptureResult::Tag::kErrorCode) {
        ...
        clients_.clear();
        RecordStartOutcomeUMA(start_timedout_ ? VideoCaptureStartOutcome::kTimedout : VideoCaptureStartOutcome::kFailed);
        return;
    }
...
void VideoCaptureImpl::OnNewBuffer(int32_t buffer_id, media::mojom::blink::VideoBufferHandlePtr buffer_handle)
{
    ...
    const bool inserted = client_buffers_.emplace(buffer_id, new BufferContext(std::move(buffer_handle), media_task_runner_)).second;
    DCHECK(inserted);
}

void VideoCaptureImpl::OnBufferReady(media::mojom::blink::ReadyBufferPtr buffer, Vector<media::mojom::blink::ReadyBufferPtr> scaled_buffers)
{
    ...
    if (!consume_buffer) {
        ...
        for (auto& scaled_buffer : scaled_buffers) {
            GetVideoCaptureHost()->ReleaseBuffer(device_id_, scaled_buffer->buffer_id, DefaultFeedback());
        }
        return;
    }
    ...
    // frames.
    auto frame_preparer = std::make_unique<VideoFrameBufferPreparer>(*this, std::move(buffer));
    ...
    for (media::mojom::blink::ReadyBufferPtr& scaled_buffer : scaled_buffers) {
        auto scaled_frame_preparer = std::make_unique<VideoFrameBufferPreparer>(*this, std::move(scaled_buffer));
    ...
    if (!init_successful) {
        ...
        GetVideoCaptureHost()->ReleaseBuffer(device_id_, frame_preparer->buffer_id(), DefaultFeedback());
        for (auto& scaled_frame_preparer : scaled_frame_preparers) {
            GetVideoCaptureHost()->ReleaseBuffer(device_id_, scaled_frame_preparer->buffer_id(), DefaultFeedback());
        }
        return;
    }
    ...
    if (need_binding_on_media_thread) {
        media_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&VideoCaptureImpl::BindVideoFramesOnMediaThread, gpu_factories_, std::move(frame_preparer), std::move(scaled_frame_preparers),
                media::BindToCurrentLoop(base::BindOnce(&VideoCaptureImpl::OnVideoFrameReady, weak_factory_.GetWeakPtr(), reference_time)),
                base::BindPostTask(main_task_runner_, base::BindOnce(&VideoCaptureImpl::OnGpuContextLost, weak_factory_.GetWeakPtr()))));
        return;
    }
    OnVideoFrameReady(reference_time, std::move(frame_preparer), std::move(scaled_frame_preparers));
}

// static
void VideoCaptureImpl::BindVideoFramesOnMediaThread(media::GpuVideoAcceleratorFactories* gpu_factories,
    std::unique_ptr<VideoFrameBufferPreparer> frame_preparer, std::vector<std::unique_ptr<VideoFrameBufferPreparer>> scaled_frame_preparers,
    base::OnceCallback<void(std::unique_ptr<VideoFrameBufferPreparer>, std::vector<std::unique_ptr<VideoFrameBufferPreparer>>)> on_frame_ready_callback,
    base::OnceCallback<void()> on_gpu_context_lost)
{
    ...
    for (auto& scaled_frame_preparer : scaled_frame_preparers) {
        if (!scaled_frame_preparer->IsVideoFrameBound()) {
            bind_failed |= !scaled_frame_preparer->BindVideoFrameOnMediaThread(gpu_factories);
        }
    }
    if (bind_failed) {
    ...
    std::move(on_frame_ready_callback).Run(std::move(frame_preparer), std::move(scaled_frame_preparers));
}

void VideoCaptureImpl::OnVideoFrameReady(base::TimeTicks reference_time, std::unique_ptr<VideoFrameBufferPreparer> frame_preparer,
    std::vector<std::unique_ptr<VideoFrameBufferPreparer>> scaled_frame_preparers)
{
    DCHECK_CALLED_ON_VALID_THREAD(io_thread_checker_);
    ...
    for (const auto& client : clients_) {
        client.second.deliver_frame_cb.Run(frame_preparer->frame(), scaled_video_frames, reference_time);
    }
}
