...
public:
    ...
    VideoCaptureImplTest()
        : video_capture_impl_(new VideoCaptureImpl(session_id_, base::ThreadTaskRunnerHandle::Get(), &GetEmptyBrowserInterfaceBroker()))
    {
...
protected:
    ...
    MOCK_METHOD3(OnFrameReady, void(scoped_refptr<media::VideoFrame>, std::vector<scoped_refptr<media::VideoFrame>>, base::TimeTicks));
    ...
    void SimulateBufferReceived(BufferDescription buffer_description, std::vector<BufferDescription> scaled_buffer_descriptions = {})
    {
        ...
        for (const auto& scaled_buffer_description : scaled_buffer_descriptions) {
            scaled_ready_buffers.push_back(scaled_buffer_description.ToReadyBuffer(now));
        }
        video_capture_impl_->OnBufferReady(buffer_description.ToReadyBuffer(now), std::move(scaled_ready_buffers));
    }
...
TEST_F(VideoCaptureImplTest, BufferReceived_GpuMemoryBufferHandle)
{
    ...
    EXPECT_CALL(*this, OnStateUpdate(blink::VIDEO_CAPTURE_STATE_STOPPED));
    EXPECT_CALL(*this, OnFrameReady(_, _, _))
        .WillOnce(Invoke([&](scoped_refptr<media::VideoFrame> f, std::vector<scoped_refptr<media::VideoFrame>>, base::TimeTicks t) {
...
TEST_F(VideoCaptureImplTest, ScaledBufferReceived_SoftwareOnly)
{
    ...
    const size_t large_frame_size = media::VideoFrame::AllocationSize(media::PIXEL_FORMAT_I420, params_large_.requested_format.frame_size);
    base::UnsafeSharedMemoryRegion large_region = base::UnsafeSharedMemoryRegion::Create(large_frame_size);
    ASSERT_TRUE(large_region.IsValid());

    const size_t small_frame_size = media::VideoFrame::AllocationSize(media::PIXEL_FORMAT_I420, params_small_.requested_format.frame_size);
    base::UnsafeSharedMemoryRegion small_region = base::UnsafeSharedMemoryRegion::Create(small_frame_size);
    ...
    EXPECT_CALL(*this, OnStateUpdate(blink::VIDEO_CAPTURE_STATE_STOPPED));
    EXPECT_CALL(*this, OnFrameReady(_, _, _))
        .WillOnce(Invoke([&](scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, base::TimeTicks timestamp) {
            ready_frame = frame;
    ...
    EXPECT_CALL(mock_video_capture_host_, DoStart(_, session_id_, params_large_));
    EXPECT_CALL(mock_video_capture_host_, Stop(_));
    EXPECT_CALL(mock_video_capture_host_, ReleaseBuffer(_, kLargeBufferId, _)).Times(0);
    EXPECT_CALL(mock_video_capture_host_, ReleaseBuffer(_, kSmallBufferId, _)).Times(0);

    StartCapture(0, params_large_);
    SimulateOnBufferCreated(kLargeBufferId, large_region);
    SimulateOnBufferCreated(kSmallBufferId, small_region);
    SimulateBufferReceived(BufferDescription(kLargeBufferId, params_large_.requested_format.frame_size),
        { BufferDescription(kSmallBufferId, params_small_.requested_format.frame_size) });
    StopCapture(0);
...
TEST_F(VideoCaptureImplTest, ScaledBufferReceived_SoftwareAndHardware)
{
    ...
    const size_t large_frame_size = media::VideoFrame::AllocationSize(media::PIXEL_FORMAT_NV12, params_large_.requested_format.frame_size);
    base::UnsafeSharedMemoryRegion large_region = base::UnsafeSharedMemoryRegion::Create(large_frame_size);
    ASSERT_TRUE(large_region.IsValid());
    ...
    EXPECT_CALL(mock_video_capture_host_, DoStart(_, session_id_, params_large_));
    EXPECT_CALL(mock_video_capture_host_, Stop(_));
    EXPECT_CALL(mock_video_capture_host_, ReleaseBuffer(_, kLargeSoftwareBufferId, _)).Times(0);
    EXPECT_CALL(mock_video_capture_host_, ReleaseBuffer(_, kSmallHardwareBufferId, _)).Times(0);
    ...
    auto start_and_receive_buffers = [&]() {
        ...
        SimulateGpuMemoryBufferCreated(kSmallHardwareBufferId, std::move(gmb_handle));
        // Receive buffers.
        SimulateBufferReceived(BufferDescription(kLargeSoftwareBufferId, params_large_.requested_format.frame_size, media::PIXEL_FORMAT_NV12),
            { BufferDescription(kSmallHardwareBufferId, params_small_.requested_format.frame_size, media::PIXEL_FORMAT_NV12) });
    };
    ...
    testing_io_thread.Start();
    testing_io_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting(start_and_receive_buffers));

    frame_ready_event.Wait();
    EXPECT_TRUE(ready_frame);
    EXPECT_EQ(ready_scaled_frames.size(), 1u);

    testing_io_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting(stop_and_destroy_buffers));
    testing_io_thread.Stop();
}
