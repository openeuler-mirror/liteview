...
public:
    ...
    scoped_refptr<FontFallbackList> Get(const FontDescription& font_description) LOCKS_EXCLUDED(lock_);
...
private:
    // FontSelectorClient
    void FontsNeedUpdate(FontSelector*, FontInvalidationReason) override LOCKS_EXCLUDED(lock_);
    ...
    template <typename Predicate> void InvalidateInternal(Predicate predicate) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    Member<FontSelector> font_selector_;
    mutable LockForParallelTextShaping lock_;
    HashMap<FontDescription, scoped_refptr<FontFallbackList>> fallback_list_for_description_ GUARDED_BY(lock_);
};
