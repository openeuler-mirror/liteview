...
public:
    FontFallbackIterator(const FontDescription&, scoped_refptr<FontFallbackList>, FontFallbackPriority);
...
private:
    bool RangeSetContributesForHint(const Vector<UChar32>& hint_list, const FontDataForRangeSet*);
    ...
    scoped_refptr<FontDataForRangeSet> UniqueOrNext(scoped_refptr<FontDataForRangeSet> candidate, const Vector<UChar32>& hint_list);

    scoped_refptr<SimpleFontData> FallbackPriorityFont(UChar32 hint);
    scoped_refptr<SimpleFontData> UniqueSystemFontForHintList(const Vector<UChar32>& hint_list);
