...
const unsigned kMaxHarfBuzzRecords = 20;
...
absl::optional<float> OpenTypeMathSupport::MathConstant(const HarfBuzzFace* harfbuzz_face, MathConstants constant)
{
...
absl::optional<float> OpenTypeMathSupport::MathItalicCorrection(const HarfBuzzFace* harfbuzz_face, Glyph glyph)
{
    if (!harfbuzz_face)
        return absl::nullopt;
    ...
    return absl::optional<float>(HarfBuzzUnitsToFloat(hb_ot_math_get_glyph_italics_correction(font, glyph)));
}
    ...
    const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis)
{
    ...
    auto converter = base::BindRepeating([](hb_ot_math_glyph_variant_t record) -> OpenTypeMathStretchData::GlyphVariantRecord { return record.glyph; });
    ...
    const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis, float* italic_correction)
{
    ...
    auto getter = base::BindOnce([](hb_font_t* font, hb_codepoint_t glyph, hb_direction_t direction, unsigned int start_offset, unsigned int* parts_count,
    ...
    auto converter = base::BindRepeating([](hb_ot_math_glyph_part_t record) -> OpenTypeMathStretchData::GlyphPartRecord {
    ...
    Vector<OpenTypeMathStretchData::GlyphPartRecord> parts = GetHarfBuzzMathRecord(
        harfbuzz_face, base_glyph, stretch_axis, std::move(getter), std::move(converter), absl::optional<OpenTypeMathStretchData::GlyphPartRecord>());
