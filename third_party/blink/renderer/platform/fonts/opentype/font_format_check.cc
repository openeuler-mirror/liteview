...
FontFormatCheck::COLRVersion determineCOLRVersion(const FontFormatCheck::TableTagsVector& table_tags, const hb_face_t* face)
{
    ...
    if (table_tags.size() && table_tags.Contains(kCOLRTag) && table_tags.Contains(HB_TAG('C', 'P', 'A', 'L'))) {
        ...
        const char* colr_data = hb_blob_get_data(table_blob.get(), &required_bytes_count);
        if (required_bytes_count < 2u)
            return FontFormatCheck::COLRVersion::kNoCOLR;

        uint16_t colr_version = base::NetToHost16(*reinterpret_cast<const uint16_t*>(colr_data));
