...
scoped_refptr<FontFallbackList> GetOrCreateFontFallbackList(const FontDescription& font_description, FontSelector* font_selector)
{
...
{
    if (!font_fallback_list_ || !font_fallback_list_->IsValid() || !font_fallback_list_->HasFontFallbackMap()) {
...
void Font::RevalidateFontFallbackList() const
{
    ...
    font_fallback_list_ = font_fallback_list_->GetFontFallbackMap().Get(font_description_);
}

FontFallbackList* Font::EnsureFontFallbackList() const
{
    if (!font_fallback_list_ || !font_fallback_list_->HasFontFallbackMap()) {
        font_fallback_list_ = GetOrCreateFontFallbackList(font_description_, nullptr);
    }
...
bool Font::operator==(const Font& other) const
{
    ...
    FontSelector* first = font_fallback_list_ && font_fallback_list_->HasFontFallbackMap() ? font_fallback_list_->GetFontSelector() : nullptr;
    FontSelector* second
        = other.font_fallback_list_ && other.font_fallback_list_->HasFontFallbackMap() ? other.font_fallback_list_->GetFontSelector() : nullptr;
...
void Font::DrawText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point, float device_scale_factor, const cc::PaintFlags& flags,
    DrawType draw_type) const
{
    DrawText(canvas, run_info, point, device_scale_factor, cc::kInvalidNodeId, flags, draw_type);
}

void Font::DrawText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point, float device_scale_factor, cc::NodeId node_id,
    const cc::PaintFlags& flags, DrawType draw_type) const
{
    ...
    ShapeResultBloberizer::FillGlyphs bloberizer(GetFontDescription(), device_scale_factor > 1.0f, run_info, buffer,
...
void Font::DrawText(cc::PaintCanvas* canvas, const NGTextFragmentPaintInfo& text_info, const gfx::PointF& point, float device_scale_factor, cc::NodeId node_id,
    const cc::PaintFlags& flags, DrawType draw_type) const
{
    // Don't draw anything while we are using custom fonts that are in the process
    // of loading.
    ...
    ShapeResultBloberizer::FillGlyphsNG bloberizer(GetFontDescription(), device_scale_factor > 1.0f, text_info.text, text_info.from, text_info.to,
        text_info.shape_result, draw_type == Font::DrawType::kGlyphsOnly ? ShapeResultBloberizer::Type::kNormal : ShapeResultBloberizer::Type::kEmitText);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point, node_id);
}

bool Font::DrawBidiText(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const gfx::PointF& point,
    CustomFontNotReadyAction custom_font_not_ready_action, float device_scale_factor, const cc::PaintFlags& flags, DrawType draw_type) const
{
    // Don't draw anything while we are using custom fonts that are in the process
    ...
    bidi_resolver.SetStatus(BidiStatus(run.Direction(), run.DirectionalOverride()));
    ...
    CachingWordShaper word_shaper(*this);
    while (bidi_run) {
        TextRun subrun = run.SubRun(bidi_run->Start(), bidi_run->Stop() - bidi_run->Start());
        ...
        // crbug.com/1055652.
        STACK_UNINITIALIZED ShapeResultBloberizer::FillGlyphs bloberizer(GetFontDescription(), device_scale_factor > 1.0f, subrun_info, buffer,
            draw_type == Font::DrawType::kGlyphsOnly ? ShapeResultBloberizer::Type::kNormal : ShapeResultBloberizer::Type::kEmitText);
...
void Font::DrawEmphasisMarks(cc::PaintCanvas* canvas, const TextRunPaintInfo& run_info, const AtomicString& mark, const gfx::PointF& point,
    float device_scale_factor, const cc::PaintFlags& flags) const
{
    if (ShouldSkipDrawing())
        return;
    ...
    ShapeResultBuffer buffer;
    word_shaper.FillResultBuffer(run_info, &buffer);
    ShapeResultBloberizer::FillTextEmphasisGlyphs bloberizer(GetFontDescription(), device_scale_factor > 1.0f, run_info, buffer, emphasis_glyph_data);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point);
}

void Font::DrawEmphasisMarks(cc::PaintCanvas* canvas, const NGTextFragmentPaintInfo& text_info, const AtomicString& mark, const gfx::PointF& point,
    float device_scale_factor, const cc::PaintFlags& flags) const
{
    if (ShouldSkipDrawing())
        return;
    ...
    ShapeResultBloberizer::FillTextEmphasisGlyphsNG bloberizer(
        GetFontDescription(), device_scale_factor > 1.0f, text_info.text, text_info.from, text_info.to, text_info.shape_result, emphasis_glyph_data);
    DrawBlobs(canvas, flags, bloberizer.Blobs(), point);
}
...
float Font::Width(const TextRun& run, HashSet<const SimpleFontData*>* fallback_fonts, gfx::RectF* glyph_bounds) const
{
    FontCachePurgePreventer purge_preventer;
...
void Font::GetTextIntercepts(const TextRunPaintInfo& run_info, float device_scale_factor, const cc::PaintFlags& flags, const std::tuple<float, float>& bounds,
    Vector<TextIntercept>& intercepts) const
{
    ...
    ShapeResultBloberizer::FillGlyphs bloberizer(
        GetFontDescription(), device_scale_factor > 1.0f, run_info, buffer, ShapeResultBloberizer::Type::kTextIntercepts);

    GetTextInterceptsInternal(bloberizer.Blobs(), flags, bounds, intercepts);
}

void Font::GetTextIntercepts(const NGTextFragmentPaintInfo& text_info, float device_scale_factor, const cc::PaintFlags& flags,
    const std::tuple<float, float>& bounds, Vector<TextIntercept>& intercepts) const
{
    if (ShouldSkipDrawing())
        return;
...
void Font::ExpandRangeToIncludePartialGlyphs(const TextRun& text_run, int* from, int* to) const
{
