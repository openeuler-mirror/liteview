...
ThreadSpecific<std::unique_ptr<FontGlobalContext>>& GetThreadSpecificFontGlobalContextPool()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<std::unique_ptr<FontGlobalContext>>, thread_specific_pool, ());
...
{
    std::unique_ptr<HarfBuzzFontCache>& global_context_harfbuzz_font_cache = Get().harfbuzz_font_cache_;
