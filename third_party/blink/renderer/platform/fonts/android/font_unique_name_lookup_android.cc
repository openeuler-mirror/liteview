...
void FontUniqueNameLookupAndroid::PrepareFontUniqueNameLookup(NotifyFontUniqueNameLookupReady callback)
{
    ...
    firmware_font_lookup_service_->GetUniqueNameLookupTable(
        base::BindOnce(&FontUniqueNameLookupAndroid::ReceiveReadOnlySharedMemoryRegion, base::Unretained(this)));
}
...
void FontUniqueNameLookupAndroid::Init()
{
    ...
    if (base::FeatureList::IsEnabled(features::kPrefetchAndroidFonts) && RuntimeEnabledFeatures::AndroidDownloadableFontsMatchingEnabled()) {
        EnsureServiceConnected();
...
{
    absl::optional<FontTableMatcher::MatchResult> match_result = font_table_matcher_->MatchName(font_unique_name.Utf8().c_str());
    if (!match_result)
        return nullptr;
    return SkTypeface::MakeFromFile(match_result->font_path.c_str(), match_result->ttc_index);
}
...
bool FontUniqueNameLookupAndroid::RequestedNameInQueryableFonts(const String& font_unique_name)
{
    ...
    return queryable_fonts_ && queryable_fonts_->Contains(String::FromUTF8(IcuFoldCase(font_unique_name.Utf8()).c_str()));
}
...
sk_sp<SkTypeface> FontUniqueNameLookupAndroid::MatchUniqueNameFromDownloadableFonts(const String& font_unique_name)
{
    ...
    base::File font_file;
    String case_folded_unique_font_name = String::FromUTF8(IcuFoldCase(font_unique_name.Utf8()).c_str());
