...
FontFallbackIterator::FontFallbackIterator(
    const FontDescription& description, scoped_refptr<FontFallbackList> fallback_list, FontFallbackPriority font_fallback_priority)
...
{
    for (auto* it = tracked_loading_range_sets_.begin(); it != tracked_loading_range_sets_.end(); ++it) {
...
bool FontFallbackIterator::RangeSetContributesForHint(const Vector<UChar32>& hint_list, const FontDataForRangeSet* segmented_face)
{
    for (auto* it = hint_list.begin(); it != hint_list.end(); ++it) {
        if (segmented_face->Contains(*it)) {
            ...
            // load them.
            if (!segmented_face->IsPendingCustomFont() || segmented_face->IsPendingDataUrlCustomFont() || !AlreadyLoadingRangeForHintChar(*it))
                return true;
        }
    }
    return false;
...
scoped_refptr<FontDataForRangeSet> FontFallbackIterator::UniqueOrNext(scoped_refptr<FontDataForRangeSet> candidate, const Vector<UChar32>& hint_list)
{
...
scoped_refptr<FontDataForRangeSet> FontFallbackIterator::Next(const Vector<UChar32>& hint_list)
{
    ...
    if (fallback_stage_ == kFallbackPriorityFonts) {
        ...
        scoped_refptr<FontDataForRangeSet> fallback_priority_font_range = base::AdoptRef(new FontDataForRangeSet(FallbackPriorityFont(hint_list[0])));
    ...
    if (fallback_stage_ == kSystemFonts) {
        ...
        if (system_font) {
            ...
            return UniqueOrNext(base::AdoptRef(new FontDataForRangeSet(system_font)), hint_list);
        }
        ...
        scoped_refptr<SimpleFontData> last_resort = font_cache.GetLastResortFallbackFont(font_description_).get();

        if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
            font_selector->ReportLastResortFallbackFontLookup(font_description_, last_resort.get());
        }

        return UniqueOrNext(base::AdoptRef(new FontDataForRangeSetFromCache(last_resort)), hint_list);
    }
    ...
    if (!font_data->IsSegmented()) {
        ...
        if (!font_data->IsLoading()) {
            scoped_refptr<SimpleFontData> non_segmented = const_cast<SimpleFontData*>(To<SimpleFontData>(font_data));
            ...
            return UniqueOrNext(base::AdoptRef(new FontDataForRangeSet(non_segmented)), hint_list);
        }
        return Next(hint_list);
    }
    ...
    scoped_refptr<FontDataForRangeSet> current_segmented_face = segmented->FaceAt(segmented_face_index_);
...
scoped_refptr<SimpleFontData> FontFallbackIterator::FallbackPriorityFont(UChar32 hint)
{
    scoped_refptr<SimpleFontData> font_data = FontCache::Get().FallbackFontForCharacter(
        font_description_, hint, font_fallback_list_->PrimarySimpleFontData(font_description_), font_fallback_priority_);

    if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
        font_selector->ReportFontLookupByFallbackCharacter(hint, font_fallback_priority_, font_description_, font_data.get());
    }
    return font_data;
}
...
scoped_refptr<SimpleFontData> FontFallbackIterator::UniqueSystemFontForHintList(const Vector<UChar32>& hint_list)
{
    ...
    if (FontSelector* font_selector = font_fallback_list_->GetFontSelector()) {
        font_selector->ReportFontLookupByFallbackCharacter(hint, FontFallbackPriority::kText, font_description_, font_data.get());
    }
    return font_data;
}
