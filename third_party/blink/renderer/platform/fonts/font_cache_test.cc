...
TEST_F(FontCacheTest, getLastResortFallbackFont)
{
    ...
    for (FontDescription::GenericFamilyType family_type :
        ...
        scoped_refptr<SimpleFontData> font_data = font_cache.GetLastResortFallbackFont(font_description, kRetain);
...
TEST_F(FontCacheTest, NoFallbackForPrivateUseArea)
{
    ...
    for (FontDescription::GenericFamilyType family_type : { FontDescription::kStandardFamily, FontDescription::kWebkitBodyFamily }) {
        ...
        for (UChar32 character : { 0xE000, 0xE401, 0xE402, 0xE403, 0xF8FF, 0xF0000, 0xFAAAA, 0x100000, 0x10AAAA }) {
            scoped_refptr<SimpleFontData> font_data = font_cache.FallbackFontForCharacter(font_description, character, nullptr);
        ...
        for (auto info : {
...

            {
                scoped_refptr<SimpleFontData> font_data
                    = font_cache.FallbackFontForCharacter(font_description, character, nullptr, FontFallbackPriority::kEmojiEmoji);
            ...
            {
                scoped_refptr<SimpleFontData> font_data
                    = font_cache.FallbackFontForCharacter(font_description, character, nullptr, FontFallbackPriority::kEmojiText);
...
TEST_F(FontCacheTest, getLargerThanMaxUnsignedFont)
{
    ...
    scoped_refptr<blink::SimpleFontData> font_data = font_cache.GetFontData(font_description, AtomicString());
