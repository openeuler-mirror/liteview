...
struct IdentifiableTokenKeyHash {
    ...
    static unsigned GetHash(const IdentifiableTokenKey& key)
    {
        ...
        return hasher.GetHash(key.token.ToUkmMetricValue()) ^ hasher.GetHash((key.is_deleted_value << 1) + key.is_empty_value);
    }
    static bool Equal(const IdentifiableTokenKey& a, const IdentifiableTokenKey& b)
    {
        return a == b;
    }
...
public:
    ...
    FontMatchingMetrics(bool top_level, ukm::UkmRecorder* ukm_recorder, ukm::SourceId source_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    // Create a FontMatchingMetrics objects for a worker.
    FontMatchingMetrics(ukm::UkmRecorder* ukm_recorder, ukm::SourceId source_id, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    void ReportFontLookupByUniqueOrFamilyName(const AtomicString& name, const FontDescription& font_description, SimpleFontData* resulting_font_data);
    ...
    void ReportFontLookupByUniqueNameOnly(
        const AtomicString& name, const FontDescription& font_description, SimpleFontData* resulting_font_data, bool is_loading_fallback = false);
    ...
    void ReportLastResortFallbackFontLookup(const FontDescription& font_description, SimpleFontData* resulting_font_data);
...
private:
    ...
    using TokenToTokenHashMap = HashMap<IdentifiableTokenKey, IdentifiableToken, IdentifiableTokenKeyHash, IdentifiableTokenKeyHashTraits>;
    ...
    void InsertFontHashIntoMap(IdentifiableTokenKey input_key, SimpleFontData* font_data, TokenToTokenHashMap& hash_map);
    ...
    IdentifiableToken GetPostScriptNameTokenForFontData(SimpleFontData* font_data);
