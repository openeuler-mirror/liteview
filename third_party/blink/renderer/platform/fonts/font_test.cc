...
{
    return CreateTestFont(family_name, font_path, size, /* ligatures */ nullptr,
...
public:
    Vector<int> GetExpandedRange(const String& text, bool ltr, int from, int to)
    {
        ...
        Font font = CreateTestFont("roboto", test::PlatformTestDataPath("third_party/Roboto/roboto-regular.woff2"), 100, &ligatures);

        TextRun text_run(text, /* xpos */ 0, /* expansion */ 0, TextRun::kAllowTrailingExpansion | TextRun::kForbidLeadingExpansion,
...
TEST_F(FontTest, FonteMetricsCapHeight)
{
    const auto cap_height_of = [](const char* font_path, float size) {
        Font font = CreateTestFont("test", test::PlatformTestDataPath(font_path), size);
...
{
    Font font = CreateTestFont("Ahem", test::PlatformTestDataPath("Ahem.woff"), 16);
...
{
    Font font = CreateTestFont("M PLUS 1p", blink::test::BlinkWebTestsFontsTestDataPath("mplus-1p-regular.woff"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
...
{
    Font font = CreateTestFont("CSSHWOrientationTest", blink::test::BlinkWebTestsFontsTestDataPath("adobe-fonts/CSSHWOrientationTest.otf"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    EXPECT_TRUE(font_data->GetFontMetrics().IdeographicFullWidth().has_value());
...
{
    Font font = CreateVerticalUprightTestFont("M PLUS 1p", blink::test::BlinkWebTestsFontsTestDataPath("mplus-1p-regular.woff"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    ...
    EXPECT_EQ(*font_data->GetFontMetrics().IdeographicFullWidth(), font_data->GetFontMetrics().Height());
}

// A Japanese font, with the "water" glyph, with the `vmtx` table.
TEST_F(FontTest, IdeographicFullWidthUprightCjkVmtx)
{
    Font font = CreateVerticalUprightTestFont("CSSHWOrientationTest", blink::test::BlinkWebTestsFontsTestDataPath("adobe-fonts/CSSHWOrientationTest.otf"), 16);
    const SimpleFontData* font_data = font.PrimaryFont();
    ASSERT_TRUE(font_data);
    ASSERT_TRUE(font_data->GetFontMetrics().IdeographicFullWidth().has_value());
    EXPECT_EQ(*font_data->GetFontMetrics().IdeographicFullWidth(), 16);
...
TEST_F(FontTest, TextIntercepts)
{
    ...
    font.GetTextIntercepts(text_run_paint_info, device_scale_factor, default_paint, below_baseline_bounds, text_intercepts);
    ...
    font.GetTextIntercepts(text_run_paint_info, device_scale_factor, default_paint, above_baseline_bounds, text_intercepts);
...
{
    Font font = CreateTestFont("Ahem", test::PlatformTestDataPath("Ahem.woff"), 0);
    TabSize tab_size(8);
