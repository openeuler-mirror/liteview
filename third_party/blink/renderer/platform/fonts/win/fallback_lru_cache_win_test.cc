...
sk_sp<SkTypeface> fallbackForLocale(String locale, UChar32 codepoint)
{
    ...
    return sk_sp<SkTypeface>(font_mgr->matchFamilyStyleCharacter(kFontFamilyNameArial, SkFontStyle(), &locale_char, 1, codepoint));
}

void fillCacheWithDummies(blink::FallbackLruCache& lru_cache, const char* format_string, size_t count)
{
    for (size_t i = 0; i < count; ++i) {
        blink::TypefaceVector dummy_typefaces;
        dummy_typefaces.push_back(SkTypeface::MakeFromName(kFontFamilyNameArial, SkFontStyle()));
...
TEST(FallbackLruCacheTest, KeepChineseWhenFetched)
{
    ...
    fallback_typefaces_zh.push_back(fallbackForLocale(kHanSimplifiedLocale, kFirstCJKIdeograph));
    ...
    fillCacheWithDummies(lru_cache, "dummy_locale_2nd_%zu", kLruCacheTestSize - 1);
