...
String makeCacheKey(FontDescription::GenericFamilyType generic_family, String bcp47_language_tag, FontFallbackPriority fallback_priority)
{
    StringBuilder cache_key;
    cache_key.Append(bcp47_language_tag);
    cache_key.AppendNumber(static_cast<std::underlying_type<FontDescription::GenericFamilyType>::type>(generic_family));
    cache_key.AppendNumber(static_cast<std::underlying_type<FontFallbackPriority>::type>(fallback_priority));
    return cache_key.ToString();
}

void getFallbackFamilyAndStyle(SkTypeface* typeface, String* fallback_family, SkFontStyle* fallback_style)
{
    SkString family;
...
void FallbackFamilyStyleCache::Put(
    FontDescription::GenericFamilyType generic_family, String bcp47_language_tag, FontFallbackPriority fallback_priority, SkTypeface* typeface)
{
    String cache_key = makeCacheKey(generic_family, bcp47_language_tag, fallback_priority);
...
void FallbackFamilyStyleCache::Get(FontDescription::GenericFamilyType generic_family, String bcp47_language_tag, FontFallbackPriority fallback_priority,
    UChar32 character, String* fallback_family, SkFontStyle* fallback_style)
{
    auto it = recent_fallback_fonts_.Get(makeCacheKey(generic_family, bcp47_language_tag, fallback_priority));
    if (it == recent_fallback_fonts_.end())
        return;
