...
inline UScriptCode getScriptForOpenType(UChar32 ch, UErrorCode* status)
{
    ...
    if (UNLIKELY(U_FAILURE(*status)))
        return script;
    if (UNLIKELY(script == USCRIPT_KATAKANA || script == USCRIPT_KATAKANA_OR_HIRAGANA)) {
...
ScriptRunIterator::ScriptRunIterator(const UChar* text, wtf_size_t length, const ScriptData* data)
    : text_(text)
    , length_(length)
