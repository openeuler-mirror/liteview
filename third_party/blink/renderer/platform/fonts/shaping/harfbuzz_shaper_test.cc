...
struct ShapeResultRunData {
    ...
    static Vector<ShapeResultRunData> Get(const scoped_refptr<ShapeResult>& result)
    {
...
protected:
    ...
    void SelectDevanagariFont()
    {
        ...
        // Linux
        devanagari_family.AppendFamily("Lohit Devanagari", FontFamily::Type::kFamilyName);
        // Mac
        devanagari_family.AppendFamily("ITF Devanagari", FontFamily::Type::kFamilyName);
    ...
    Font CreateAhem(float size)
    {
        ...
        return blink::test::CreateTestFont("Ahem", blink::test::PlatformTestDataPath("Ahem.woff"), size, &ligatures);
    }

    Font CreateNotoColorEmoji()
    {
        return blink::test::CreateTestFont("NotoColorEmoji", blink::test::BlinkRootDir() + "/web_tests/third_party/NotoColorEmoji/NotoColorEmoji.ttf", 12);
    }

    scoped_refptr<ShapeResult> SplitRun(scoped_refptr<ShapeResult> shape_result, unsigned offset)
    {
    ...
    {
        scoped_refptr<ShapeResult> result = ShapeResult::Create(&font, 2, 8, direction);
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsUnicodeVariants)
{
    ...
    for (auto& test : testlist) {
        ...
        ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(0, start_index, num_glyphs, script)) << test.name;
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsDevanagariCommon)
{
    ...
    for (unsigned i = 0; i < TestInfo(result)->NumberOfRunsForTesting(); ++i) {
        ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(i, start_index, num_glyphs, script));
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsDevanagariCommonLatinCommon)
{
    ...
    for (unsigned i = 0; i < TestInfo(result)->NumberOfRunsForTesting(); ++i) {
        ...
        EXPECT_EQ(script, finished_devanagari ? HB_SCRIPT_LATIN : HB_SCRIPT_DEVANAGARI);
    }
}
...
TEST_F(HarfBuzzShaperTest, ResolveCandidateRunsArabicThaiHanLatin)
{
    ...
    ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(1, start_index, num_glyphs, script));
    EXPECT_EQ(3u, start_index);
    EXPECT_EQ(1u, num_glyphs);
    EXPECT_EQ(HB_SCRIPT_THAI, script);

    ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(2, start_index, num_glyphs, script));
    EXPECT_EQ(4u, start_index);
    EXPECT_EQ(1u, num_glyphs);
    EXPECT_EQ(HB_SCRIPT_HAN, script);

    ASSERT_TRUE(TestInfo(result)->RunInfoForTesting(3, start_index, num_glyphs, script));
...
TEST_F(HarfBuzzShaperTest, ShapeLatinSegment)
{
    ...
    ASSERT_TRUE(TestInfo(first)->RunInfoForTesting(0, start_index, num_characters, num_glyphs, script));
    EXPECT_EQ(0u, start_index);
    EXPECT_EQ(6u, num_characters);
    ASSERT_TRUE(TestInfo(second)->RunInfoForTesting(0, start_index, num_characters, num_glyphs, script));
    EXPECT_EQ(6u, start_index);
    EXPECT_EQ(5u, num_characters);
    ASSERT_TRUE(TestInfo(third)->RunInfoForTesting(0, start_index, num_characters, num_glyphs, script));
...
TEST_F(HarfBuzzShaperTest, ShapeTabulationCharacters)
{
    ...
    scoped_refptr<ShapeResult> result = ShapeResult::CreateForTabulationCharacters(&font, TextDirection::kLtr, TabSize(8), 0.f, 0, length);
...
TEST_F(HarfBuzzShaperTest, ShapeVerticalUpright)
{
    ...
    scoped_refptr<ShapeResult> result2 = shaper.Shape(&font, direction, 3, string.length());

    scoped_refptr<ShapeResult> composite_result = ShapeResult::Create(&font, 0, 0, direction);
...
TEST_F(HarfBuzzShaperTest, RangeShapeSmallCaps)
{
    ...
    HarfBuzzShaper shaper(string);
    scoped_refptr<ShapeResult> result = shaper.Shape(&font, TextDirection::kLtr, 2, 3);
...
TEST_P(OffsetForPositionTest, Data)
{
    ...
    scoped_refptr<ShapeResult> result = SplitRun(shaper.Shape(&ahem, TextDirection::kLtr), 2);
...
TEST_F(HarfBuzzShaperTest, EmojiZWJSequence)
{
    ...
    HarfBuzzShaper shaper(String(emoji_zwj_sequence, std::size(emoji_zwj_sequence)));
    scoped_refptr<ShapeResult> result = shaper.Shape(&font, direction);
}
...
TEST_P(ShapeResultCopyRangeTest, Split)
{
    ...
    // Split the result.
    scoped_refptr<ShapeResult> result1 = ShapeResult::Create(&font, 0, 0, direction);
    ...
    scoped_refptr<ShapeResult> result2 = ShapeResult::Create(&font, 0, 0, direction);
...
TEST_P(ShapeResultCopyRangeTest, ShapeRange)
{
    ...
    // Shape each range.
    scoped_refptr<ShapeResult> result1 = shaper.Shape(&font, direction, 0, test_data.break_point);
    EXPECT_EQ(test_data.break_point, result1->NumCharacters());
    scoped_refptr<ShapeResult> result2 = shaper.Shape(&font, direction, test_data.break_point, string.length());
    EXPECT_EQ(string.length() - test_data.break_point, result2->NumCharacters());
...
TEST_P(ShapeParameterTest, ShapeResultCopyRangeAcrossRuns)
{
    ...
    scoped_refptr<ShapeResult> target = ShapeResult::Create(&font, 0, 0, direction);
...
TEST_F(HarfBuzzShaperTest, ShapeResultCopyRangeSegmentGlyphBoundingBox)
{
    ...
    scoped_refptr<ShapeResult> result2 = shaper.Shape(&font, direction, 6, string.length());
...
TEST_F(HarfBuzzShaperTest, SafeToBreakLatinCommonLigatures)
{
    ...
    Font testFont
        = blink::test::CreateTestFont("MEgalopolis", blink::test::PlatformTestDataPath("third_party/MEgalopolis/MEgalopolisExtra.woff"), 16, &ligatures);
    ...
    scoped_refptr<ShapeResult> copied_result = ShapeResult::Create(&testFont, 0, 0, TextDirection::kLtr);
...
TEST_F(HarfBuzzShaperTest, SafeToBreakLatinDiscretionaryLigatures)
{
    ...
    scoped_refptr<ShapeResult> referenceResult = refShaper.Shape(&testFont, TextDirection::kLtr);
...
{
    TextDirection direction = GetParam();
    scoped_refptr<ShapeResult> result = ShapeResult::Create(&font, 0, 2, direction);
...
TEST_P(ShapeParameterTest, ShapeResultViewMissingRun)
{
    ...
    // 6 and 7 are missing but NumCharacters() should be 4.
    scoped_refptr<ShapeResultView> view = ShapeResultView::Create(result.get(), 5, 9);
...
static bool KerningIsHappening(const FontDescription& font_description, TextDirection direction, const String& str)
{
    ...
    scoped_refptr<ShapeResult> result_no_kern = shaper.Shape(&font_no_kern, direction);
...
TEST_F(HarfBuzzShaperTest, ShapeHorizontalWithSubpixelPositionWithoutKerningIsNotRounded)
{
...
TEST_F(HarfBuzzShaperTest, EmojiPercentage)
{
    ...
    scoped_refptr<ShapeResult> result = shaper.Shape(&emoji_font, TextDirection::kLtr);
