...
public:
    RunInfoPart(scoped_refptr<const ShapeResult::RunInfo> run, ShapeResult::RunInfo::GlyphDataRange range, unsigned start_index, unsigned offset,
        unsigned num_characters, float width)
        : run_(run)
        , range_(range)
    ...
    template <bool has_non_zero_glyph_offsets> ShapeResult::RunInfo::GlyphOffsetArray::iterator<has_non_zero_glyph_offsets> GetGlyphOffsets() const
    {
        return ShapeResult::RunInfo::GlyphOffsetArray::iterator<has_non_zero_glyph_offsets>(range_);
    }
    ...
    ShapeResult::RunInfo::GlyphDataRange FindGlyphDataRange(unsigned start_character_index, unsigned end_character_index) const
    {
        return GetGlyphDataRange().FindGlyphDataRange(IsRtl(), start_character_index, end_character_index);
    }
    ...
    void ExpandRangeToIncludePartialGlyphs(unsigned offset, unsigned* from, unsigned* to) const
    {
        DCHECK_GE(offset + start_index_, offset_);
        unsigned part_offset = offset + start_index_ - offset_;
        run_->ExpandRangeToIncludePartialGlyphs(part_offset, reinterpret_cast<int*>(from), reinterpret_cast<int*>(to));
    }
    ...
    static absl::optional<std::pair<unsigned, unsigned>> ComputeStartEnd(const RunType& run, const ShapeResultType& result, const Segment& segment)
    {
        if (!run.GetRunInfo())
            return absl::nullopt;
...
private:
    ...
    template <typename ShapeResultType> void PopulateFromShpaeResult(const ShapeResultType& result)
    {
    ...
    {
        return static_cast<unsigned>(std::count_if(result.RunsOrParts().begin(), result.RunsOrParts().end(),
...
{
    ShapeResult* new_result = new ShapeResult(primary_font_, start_index_ + char_index_offset_, num_characters_, Direction());
    new_result->runs_.reserve(num_parts_);
    for (const auto& part : RunsOrParts()) {
        auto new_run = ShapeResult::RunInfo::Create(part.run_->font_data_.get(), part.run_->direction_, part.run_->canvas_rotation_, part.run_->script_,
            part.start_index_, part.NumGlyphs(), part.num_characters_);
...
ShapeResultView::RunInfoPart* ShapeResultView::PopulateRunInfoParts(const ShapeResultType& other, const Segment& segment, RunInfoPart* part)
{
    ...
    for (const auto& run_or_part : other.RunsOrParts()) {
        ...
        if (part_start >= segment.start_index && part_end <= segment.end_index) {
            ...
            part_width = std::accumulate(range.begin, range.end, 0.0f, [](float sum, auto& glyph) { return sum + glyph.advance; });
        }
        ...
        new (part) RunInfoPart(run->GetRunInfo(), range, part_start_index, part_offset, part_characters, part_width);
        ++part;
...
ShapeResultView::RunInfoPart* ShapeResultView::PopulateRunInfoParts(const Segment& segment, RunInfoPart* part)
{
    if (segment.result) {
        DCHECK(!segment.view);
...
{
    return { reinterpret_cast<const ShapeResultView::RunInfoPart*>(parts_), num_parts_ };
}

// static
constexpr size_t ShapeResultView::ByteSize(wtf_size_t num_parts)
{
    static_assert(sizeof(ShapeResultView) % alignof(RunInfoPart) == 0, "We have RunInfoPart as flexible array in ShapeResultView");
    return sizeof(ShapeResultView) + sizeof(RunInfoPart) * num_parts;
}

scoped_refptr<ShapeResultView> ShapeResultView::Create(base::span<const Segment> segments)
{
    DCHECK(!segments.empty());
    InitData data;
    data.Populate(segments);

    void* const buffer = ::WTF::Partitions::FastMalloc(ByteSize(data.num_parts), ::WTF::GetStringWithTypeName<ShapeResultView>());
...
scoped_refptr<ShapeResultView> ShapeResultView::Create(const ShapeResult* result, unsigned start_index, unsigned end_index)
{
    const Segment segments[] = { { result, start_index, end_index } };
    return Create(segments);
}

scoped_refptr<ShapeResultView> ShapeResultView::Create(const ShapeResultView* result, unsigned start_index, unsigned end_index)
{
    const Segment segments[] = { { result, start_index, end_index } };
    return Create(segments);
}

scoped_refptr<ShapeResultView> ShapeResultView::Create(const ShapeResult* result)
{
    ...
    RunInfoPart* const part = out->PopulateRunInfoParts(segment, out->Parts().data());
    CHECK_EQ(part, out->Parts().data() + out->num_parts_);
    return base::AdoptRef(out);
}
...
void ShapeResultView::GetRunFontData(Vector<ShapeResult::RunFontData>* font_data) const
{
    for (const auto& part : RunsOrParts()) {
...
void ShapeResultView::FallbackFonts(HashSet<const SimpleFontData*>* fallback) const
{
    DCHECK(fallback);
