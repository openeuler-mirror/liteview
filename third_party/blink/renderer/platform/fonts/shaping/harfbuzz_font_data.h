...
public:
    ...
    void UpdateFallbackMetricsAndScale(const FontPlatformData& platform_data, HarfBuzzFace::VerticalLayoutCallbacks vertical_layout)
    {
        ...
        if (UNLIKELY(vertical_layout == HarfBuzzFace::kPrepareForVerticalLayout)) {
            FontMetrics::AscentDescentWithHacks(ascent, descent, dummy_ascent_inflation, dummy_descent_inflation, platform_data, font_);
    ...
    scoped_refptr<OpenTypeVerticalData> VerticalData()
    {
        if (!vertical_data_) {
            ...
            vertical_data_ = OpenTypeVerticalData::CreateUnscaled(font_.refTypeface());
        }
...
private:
    explicit HarfBuzzFontData(hb_font_t* hb_font)
        : unscaled_font_(hb_font)
    {
    }
};
