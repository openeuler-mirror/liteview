...
unsigned CharactersInShapeResult(const Vector<scoped_refptr<const ShapeResult>, 64>& results)
{
...
Vector<CharacterRange> ShapeResultBuffer::IndividualCharacterRanges(TextDirection direction, float total_width) const
{
...
void ShapeResultBuffer::AddRunInfoAdvances(const ShapeResult::RunInfo& run_info, double offset, Vector<double>& advances)
{
    ...
    for (unsigned glyph_id = 0; glyph_id < num_glyphs; glyph_id++) {
        ...
        unsigned next_gid = run_info.IsRtl() ? num_glyphs - glyph_id - 2 : glyph_id + 1;
        const HarfBuzzRunGlyphData& glyph = run_info.glyph_data_[gid];

        unsigned char_id = glyph.character_index;
        unsigned next_char_id = (glyph_id + 1 == num_glyphs) ? num_chars : run_info.glyph_data_[next_gid].character_index;
...
Vector<double> ShapeResultBuffer::IndividualCharacterAdvances(const StringView& text, TextDirection direction, float total_width) const
{
    ...
    for (const scoped_refptr<const ShapeResult>& result : results_) {
        ...
        result->EnsureGraphemes(StringView(text, character_offset, result->NumCharacters()));
...
void ShapeResultBuffer::ExpandRangeToIncludePartialGlyphs(int* from, int* to) const
{
    int offset = 0;
    for (unsigned j = 0; j < results_.size(); j++) {
        const scoped_refptr<const ShapeResult> result = results_[j];
...
GlyphData ShapeResultBuffer::EmphasisMarkGlyphData(const FontDescription& font_description) const
{
    for (const auto& result : results_) {
        for (const auto& run : result->runs_) {
            ...
            return GlyphData(run->glyph_data_[0].glyph, run->font_data_->EmphasisMarkFontData(font_description).get(), run->CanvasRotation());
        }
    }
