...
scoped_refptr<ShapeResult> HarfBuzzShaperCallback(void* untyped_context, unsigned start, unsigned end)
{
    HarfBuzzShaperCallbackContext* context = static_cast<HarfBuzzShaperCallbackContext*>(untyped_context);
    return context->shaper->Shape(context->font, context->direction, start, end);
}

scoped_refptr<const ShapeResultView> ShapeLine(ShapingLineBreaker* breaker, unsigned start_offset, LayoutUnit available_space, unsigned* break_offset)
{
    ShapingLineBreaker::Result result;
    scoped_refptr<const ShapeResultView> shape_result = breaker->ShapeLine(start_offset, available_space, &result);
...
TEST_F(ShapingLineBreakerTest, ShapeLineLatin)
{
    ...
    scoped_refptr<const ShapeResult> first4 = shaper.Shape(&font, direction, 0, 22);
    ASSERT_LT(first4->SnappedWidth(), result->SnappedWidth());

    // "Test run with"
    scoped_refptr<const ShapeResult> first3 = shaper.Shape(&font, direction, 0, 13);
    ASSERT_LT(first3->SnappedWidth(), first4->SnappedWidth());

    // "Test run"
    scoped_refptr<const ShapeResult> first2 = shaper.Shape(&font, direction, 0, 8);
    ASSERT_LT(first2->SnappedWidth(), first3->SnappedWidth());

    // "Test"
    scoped_refptr<const ShapeResult> first1 = shaper.Shape(&font, direction, 0, 4);
    ASSERT_LT(first1->SnappedWidth(), first2->SnappedWidth());

    HarfBuzzShaperCallbackContext context { &shaper, &font, result->Direction() };
    ShapingLineBreaker breaker(result, &break_iterator, nullptr, HarfBuzzShaperCallback, &context);
...
TEST_F(ShapingLineBreakerTest, ShapeLineLatinMultiLine)
{
    ...
    scoped_refptr<const ShapeResult> mid_third = shaper.Shape(&font, direction, 0, 16);
...
TEST_F(ShapingLineBreakerTest, ShapeLineLatinBreakAll)
{
    ...
    LazyLineBreakIterator break_iterator(string, "en-US", LineBreakType::kBreakAll);
    TextDirection direction = TextDirection::kLtr;

    HarfBuzzShaper shaper(string);
    scoped_refptr<const ShapeResult> result = shaper.Shape(&font, direction);
    scoped_refptr<const ShapeResult> midpoint = shaper.Shape(&font, direction, 0, 16);
...
{
    UChar mixed_string[] = { 0x628, 0x20, 0x64A, 0x629, 0x20, 0xE20, 0x65E5, 0x62, 0 };
    LazyLineBreakIterator break_iterator(mixed_string, "ar_AE", LineBreakType::kNormal);
    TextDirection direction = TextDirection::kRtl;

    HarfBuzzShaper shaper(mixed_string);
    scoped_refptr<const ShapeResult> result = shaper.Shape(&font, direction);
    scoped_refptr<const ShapeResult> words[] = { shaper.Shape(&font, direction, 0, 1), shaper.Shape(&font, direction, 2, 4),
        shaper.Shape(&font, direction, 5, 6), shaper.Shape(&font, direction, 6, 7), shaper.Shape(&font, direction, 7, 8) };
    auto* const longest_word = std::max_element(std::begin(words), std::end(words),
        [](const scoped_refptr<const ShapeResult>& a, const scoped_refptr<const ShapeResult>& b) { return a->SnappedWidth() < b->SnappedWidth(); });
...
TEST_F(ShapingLineBreakerTest, ShapeLineRangeEndMidWord)
{
    ...
    HarfBuzzShaper shaper(string);
    scoped_refptr<const ShapeResult> result = shaper.Shape(&font, direction, 0, 2);
...
TEST_F(ShapingLineBreakerTest, ShapeLineWithLucidaFont)
{
    ...
    HarfBuzzShaper shaper(string);
    scoped_refptr<const ShapeResult> result = shaper.Shape(&font, direction, 0, 35);
    scoped_refptr<const ShapeResult> segment1 = shaper.Shape(&font, direction, 13, 31);
    scoped_refptr<const ShapeResult> segment2 = shaper.Shape(&font, direction, 13, 32);
...
INSTANTIATE_TEST_SUITE_P(ShapingLineBreakerTest, BreakOpportunityTest,
    testing::Values(BreakOpportunityTestData { u"x y z", { 1, 3, 5 } }, BreakOpportunityTestData { u"y\xADz", { 2, 3 }, { 3 } },
...
TEST_P(BreakOpportunityTest, Next)
{
    ...
    HarfBuzzShaper shaper(string);
    scoped_refptr<const ShapeResult> result = shaper.Shape(&font, TextDirection::kLtr);
