...
protected:
    ...
    Font CreateMathFont(const String& name, float size = 1000)
    {
        ...
        return blink::test::CreateTestFont("MathTestFont", blink::test::BlinkWebTestsFontsTestDataPath(String("math/") + name), size, &ligatures);
    }
...
TEST_F(StretchyOperatorShaperTest, GlyphVariants)
{
    ...
    StretchyOperatorShaper vertical_shaper(kLeftBraceCodePoint, OpenTypeMathStretchData::StretchAxis::Vertical);
    StretchyOperatorShaper horizontal_shaper(kOverBraceCodePoint, OpenTypeMathStretchData::StretchAxis::Horizontal);
    ...
    for (unsigned i = 0; i < size_count; i++) {
        for (unsigned j = 1; j <= subdivision; j++) {
            ...
            {
                scoped_refptr<ShapeResult> result = horizontal_shaper.Shape(&math, target_size);
            ...
            {
                scoped_refptr<ShapeResult> result = vertical_shaper.Shape(&math, target_size);
                EXPECT_EQ(TestInfo(result)->NumberOfRunsForTesting(), 1u);
                EXPECT_EQ(TestInfo(result)->RunInfoForTesting(0).NumGlyphs(), 1u);
