...
TEST_F(ShapeResultViewTest, ExpandRangeToIncludePartialGlyphsWithCombiningCharacter)
{
    ...
    scoped_refptr<const ShapeResultView> view = ShapeResultView::Create(result.get(), result->StartIndex(), result->EndIndex());
    unsigned from = 0;
...
{
    String string = To16Bit("Test run with multiple words and breaking opportunities.", 56);
...
TEST_F(ShapeResultViewTest, ArabicSingleView)
{
    ...
    scoped_refptr<const ShapeResult> first_wortd_reference = first_reference_shaper.Shape(&font, direction);
...
TEST_F(ShapeResultViewTest, PreviousSafeToBreak)
{
    ...
    scoped_refptr<ShapeResult> shape_result = shaper.Shape(&font, direction, 51, 131, range);

    unsigned start_offset = 59;
    unsigned end_offset = 118;
    scoped_refptr<const ShapeResultView> result_view = ShapeResultView::Create(shape_result.get(), start_offset, end_offset);
...
TEST_F(ShapeResultViewTest, LatinMultiRun)
{
    ...
    scoped_refptr<const ShapeResult> reference_result = reference_shaper.Shape(&font, direction);
    ...
    scoped_refptr<ShapeResult> composite_copy = ShapeResult::Create(&font, 0, 0, direction);
...
TEST_F(ShapeResultViewTest, MixedScriptsCompositeView)
{
    ...
    ShapeResultView::Segment segments[] = { { result_a.get(), 0, 22 }, { result_b.get(), 0, 7 } };
    auto composite_view = ShapeResultView::Create(segments);
...
TEST_F(ShapeResultViewTest, TrimEndOfView)
{
    ...
    scoped_refptr<const ShapeResultView> view1 = ShapeResultView::Create(result.get(), 5, 20);
    EXPECT_EQ(view1->NumCharacters(), 15u);
    EXPECT_EQ(view1->NumGlyphs(), 15u);

    // Trim the last character from the view.
    scoped_refptr<const ShapeResultView> view2 = ShapeResultView::Create(view1.get(), 5, 19);
...
TEST_F(ShapeResultViewTest, MarkerAndTrailingSpace)
{
    ...
    scoped_refptr<const ShapeResult> result = ShapeResult::CreateForSpaces(&font, direction, 1, 2, symbol_width);

    ShapeResultView::Segment segments[] = { { result.get(), 1, 2 } };
    auto shape_result_view = ShapeResultView::Create(segments);
    scoped_refptr<ShapeResult> shape_result = shape_result_view->CreateShapeResult();
