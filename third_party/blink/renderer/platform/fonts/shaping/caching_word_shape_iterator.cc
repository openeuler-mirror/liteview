...
scoped_refptr<const ShapeResult> CachingWordShapeIterator::ShapeWordWithoutSpacing(const TextRun& word_run, const Font* font)
{
    ...
    scoped_refptr<const ShapeResult> shape_result = shaper.Shape(font, word_run.Direction());
    if (!shape_result)
        return nullptr;
...
scoped_refptr<const ShapeResult> CachingWordShapeIterator::ShapeWord(const TextRun& word_run, const Font* font)
{
    scoped_refptr<const ShapeResult> result = ShapeWordWithoutSpacing(word_run, font);
    if (LIKELY(!spacing_.HasSpacing()))
        return result;

    scoped_refptr<const ShapeResult> spacing_result = result->ApplySpacingToCopy(spacing_, word_run);
