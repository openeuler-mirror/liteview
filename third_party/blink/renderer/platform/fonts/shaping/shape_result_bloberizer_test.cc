...
static scoped_refptr<SimpleFontData> CreateTestSimpleFontData(bool force_rotation = false)
{
    FontPlatformData platform_data(SkTypeface::MakeDefault(), std::string(), 10, false, false, TextRenderingMode::kAutoTextRendering,
        force_rotation ? FontOrientation::kVerticalUpright : FontOrientation::kHorizontal);
...
void CheckBlobBuffer(const ShapeResultBloberizer::BlobBuffer& blob_buffer, const std::vector<const ExpectedBlob>& expected_blobs)
{
...
{
    Font font;
    ShapeResultBloberizer bloberizer(font.GetFontDescription(), 1, ShapeResultBloberizer::Type::kNormal);
...
TEST_F(ShapeResultBloberizerTest, StoresGlyphsOffsets)
{
    ...
    // 2 pending glyphs
    ShapeResultBloberizerTestInfo::Add(bloberizer, 42, font1.get(), CanvasRotationInVertical::kRegular, 10, 0);
    ShapeResultBloberizerTestInfo::Add(bloberizer, 43, font1.get(), CanvasRotationInVertical::kRegular, 15, 1);

    EXPECT_EQ(ShapeResultBloberizerTestInfo::PendingRunFontData(bloberizer), font1.get());
    EXPECT_FALSE(ShapeResultBloberizerTestInfo::HasPendingRunVerticalOffsets(bloberizer));
    {
    ...
    ShapeResultBloberizerTestInfo::Add(bloberizer, 44, font2.get(), CanvasRotationInVertical::kRegular, 12, 0);

    EXPECT_EQ(ShapeResultBloberizerTestInfo::PendingRunFontData(bloberizer), font2.get());
    EXPECT_FALSE(ShapeResultBloberizerTestInfo::HasPendingRunVerticalOffsets(bloberizer));
    {
        const auto& glyphs = ShapeResultBloberizerTestInfo::PendingRunGlyphs(bloberizer);
...
TEST_F(ShapeResultBloberizerTest, StoresGlyphsVerticalOffsets)
{
    ...
    // 2 pending glyphs
    ShapeResultBloberizerTestInfo::Add(bloberizer, 42, font1.get(), CanvasRotationInVertical::kRegular, gfx::Vector2dF(10, 0), 0);
    ShapeResultBloberizerTestInfo::Add(bloberizer, 43, font1.get(), CanvasRotationInVertical::kRegular, gfx::Vector2dF(15, 0), 1);
    ...
    // one more glyph, different font => pending run flush
    ShapeResultBloberizerTestInfo::Add(bloberizer, 44, font2.get(), CanvasRotationInVertical::kRegular, gfx::Vector2dF(12, 2), 2);
...
TEST_F(ShapeResultBloberizerTest, MixedBlobRotation)
{
    ...
    for (const auto& op : append_ops) {
        ShapeResultBloberizerTestInfo::Add(bloberizer, 42, test_font.get(), op.canvas_rotation, gfx::Vector2dF(), 0);
...
TEST_F(ShapeResultBloberizerTest, CommonAccentLeftToRightFillGlyphBuffer)
{
    ...
    ShapeResultBloberizer::FillGlyphs bloberizer(font.GetFontDescription(), false, run_info, buffer, ShapeResultBloberizer::Type::kEmitText);
    ...
    ShapeResultBloberizer::FillGlyphs reference_bloberizer(
        reference_font.GetFontDescription(), false, run_info, reference_buffer, ShapeResultBloberizer::Type::kEmitText);
...
TEST_F(ShapeResultBloberizerTest, CommonAccentRightToLeftFillGlyphBufferNG)
{
    ...
    scoped_refptr<ShapeResultView> result_view = ShapeResultView::Create(result.get());
    NGTextFragmentPaintInfo text_info { StringView(string), 1, string.length(), result_view.get() };
    ShapeResultBloberizer::FillGlyphsNG bloberizer_ng(
        font.GetFontDescription(), false, text_info.text, text_info.from, text_info.to, text_info.shape_result, ShapeResultBloberizer::Type::kEmitText);
...
TEST_F(ShapeResultBloberizerTest, FourByteUtf8CodepointsNG)
{
    ...
    scoped_refptr<ShapeResultView> result_view = ShapeResultView::Create(result.get());
    NGTextFragmentPaintInfo text_info { StringView(string), 0, string.length(), result_view.get() };
    ShapeResultBloberizer::FillGlyphsNG bloberizer_ng(
        font.GetFontDescription(), false, text_info.text, text_info.from, text_info.to, text_info.shape_result, ShapeResultBloberizer::Type::kEmitText);
...
TEST_F(ShapeResultBloberizerTest, LatinMultRunNG)
{
    ...
    scoped_refptr<ShapeResult> result = ShapeResult::Create(&font, 0, 0, direction);
    shaper_a.Shape(&font, direction)->CopyRange(0u, range_a.length(), result.get());
    shaper_b.Shape(&font2, direction)->CopyRange(0u, range_b.length(), result.get());
    shaper_c.Shape(&font, direction)->CopyRange(0u, range_c.length(), result.get());
    shaper_d.Shape(&font2, direction)->CopyRange(0u, range_d.length(), result.get());
