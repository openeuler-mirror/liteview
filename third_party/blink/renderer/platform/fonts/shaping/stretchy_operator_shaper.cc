...
absl::optional<OpenTypeMathStretchData::AssemblyParameters> GetAssemblyParameters(
    const HarfBuzzFace* harfbuzz_face, Glyph base_glyph, OpenTypeMathStretchData::StretchAxis stretch_axis, float target_size, float* italic_correction)
{
    ...
    return absl::optional<OpenTypeMathStretchData::AssemblyParameters>({ connector_overlap, repetition_count, glyph_count, stretch_size, std::move(parts) });
}

} // namespace

scoped_refptr<ShapeResult> StretchyOperatorShaper::Shape(const Font* font, float target_size, Metrics* metrics) const
{
    ...
    scoped_refptr<ShapeResult> shape_result_for_glyph_assembly = ShapeResult::CreateForStretchyMathOperator(font, direction, stretch_axis_, std::move(*params));
    if (metrics) {
