...
ShapingLineBreaker::ShapingLineBreaker(scoped_refptr<const ShapeResult> result, const LazyLineBreakIterator* break_iterator, const Hyphenation* hyphenation,
    ShapeCallback shape_callback, void* shape_callback_context)
...
ShapingLineBreaker::BreakOpportunity ShapingLineBreaker::PreviousBreakOpportunity(unsigned offset, unsigned start) const
{
    ...
    if (UNLIKELY(!IsSoftHyphenEnabled())) {
        for (;; offset--) {
            ...
            if (offset <= start || offset >= text.length() || text[offset - 1] != kSoftHyphenCharacter) {
...
scoped_refptr<const ShapeResultView> ShapingLineBreaker::ShapeLine(
    unsigned start, LayoutUnit available_space, unsigned options, ShapingLineBreaker::Result* result_out)
{
    ...
    DCHECK_GE(FlipRtl(LayoutUnit::FromFloatCeil(end_position - start_position), direction), LayoutUnit(0));
    unsigned candidate_break = result_->CachedOffsetForPosition(end_position) + range_start;

    unsigned first_safe = (options & kDontReshapeStart) ? start : result_->CachedNextSafeToBreakOffset(start);
...
#endif
            result_out->is_hyphenated = false;
            return ShapeResultView::Create(result_.get(), start, result_out->break_offset);
        }
    }
    ...
    if (first_safe != start) {
        if (first_safe >= break_opportunity.offset) {
            ...
            return ShapeResultView::Create(Shape(start, break_opportunity.offset).get());
        }
        float first_safe_position = result_->CachedPositionForOffset(first_safe - range_start);
        LayoutUnit original_width = LayoutUnit::FromFloatCeil(FlipRtl(first_safe_position - start_position, direction));
    ...
    if (reshape_line_end) {
        ...
        while (true) {
            ...
            float safe_position = result_->CachedPositionForOffset(last_safe - range_start);
            line_end_result = Shape(last_safe, break_opportunity.offset);
    ...
    DCHECK_EQ(break_opportunity.offset - start,
        (line_start_result ? line_start_result->NumCharacters() : 0) + (last_safe > first_safe ? last_safe - first_safe : 0)
...
scoped_refptr<const ShapeResultView> ShapingLineBreaker::ShapeToEnd(unsigned start, unsigned first_safe, unsigned range_start, unsigned range_end)
{
    DCHECK(result_);
    ...
    ShapeResultView::Segment segments[2] = { { line_start.get(), 0, std::numeric_limits<unsigned>::max() }, { result_.get(), first_safe, range_end } };
