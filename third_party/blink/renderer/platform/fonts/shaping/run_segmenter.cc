...
RunSegmenter::RunSegmenter(const UChar* buffer, unsigned buffer_size, FontOrientation run_orientation)
    : buffer_size_(buffer_size)
    , candidate_range_(NullRange())
    , script_run_iterator_(std::make_unique<ScriptRunIterator>(buffer, buffer_size))
    , orientation_iterator_(
          run_orientation == FontOrientation::kVerticalMixed ? std::make_unique<OrientationIterator>(buffer, buffer_size, run_orientation) : nullptr)
    , symbols_iterator_(std::make_unique<SymbolsIterator>(buffer, buffer_size))
    , last_split_(0)
    , script_run_iterator_position_(0)
    , orientation_iterator_position_(run_orientation == FontOrientation::kVerticalMixed ? 0 : buffer_size_)
...
void RunSegmenter::ConsumeIteratorPastLastSplit(std::unique_ptr<Iterator>& iterator, unsigned* iterator_position, SegmentationCategory* segmentation_category)
{
...
bool RunSegmenter::Consume(RunSegmenterRange* next_range)
{
    ...
    unsigned positions[] = { script_run_iterator_position_, orientation_iterator_position_, symbols_iterator_position_ };
