...
public:
    ...
    {
        return { 0, 0, USCRIPT_INVALID_CODE, OrientationIterator::kOrientationKeep, FontFallbackPriority::kText };
    }

private:
    template <class Iterator, typename SegmentationCategory>
    void ConsumeIteratorPastLastSplit(std::unique_ptr<Iterator>& iterator, unsigned* iterator_position, SegmentationCategory* segmentation_category);
