...
protected:
    void SetUp() override
    {
        ...
        arabic_font
            = blink::test::CreateTestFont("Noto", blink::test::PlatformTestDataPath("third_party/Noto/NotoNaskhArabic-regular.woff2"), 12.0, &ligatures);
    }
    ...
    {
        return new ShapeResult(direction == TextDirection::kLtr ? &font : &arabic_font, 0, 0, direction);
    }
...
TEST_F(ShapeResultTest, CopyRangeArabicMultiRun)
{
    ...
    // with multiple runs.
    scoped_refptr<ShapeResult> result = ShapeResult::Create(&arabic_font, 0, 0, direction);
