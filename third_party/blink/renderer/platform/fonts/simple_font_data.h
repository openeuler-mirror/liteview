...
public:
    ...
    static scoped_refptr<SimpleFontData> Create(
        const FontPlatformData& platform_data, scoped_refptr<CustomFontData> custom_data = nullptr, bool subpixel_ascent_descent = false)
    {
        return base::AdoptRef(new SimpleFontData(platform_data, std::move(custom_data), subpixel_ascent_descent));
    }
    ...
    scoped_refptr<SimpleFontData> SmallCapsFontData(const FontDescription&) const LOCKS_EXCLUDED(derived_font_data_lock_);
    scoped_refptr<SimpleFontData> EmphasisMarkFontData(const FontDescription&) const LOCKS_EXCLUDED(derived_font_data_lock_);
    scoped_refptr<SimpleFontData> MetricsOverriddenFontData(const FontMetricsOverride&) const;
    ...
    CustomFontData* GetCustomFontData() const
    {
        return custom_font_data_.get();
    }
...
private:
    SimpleFontData(const FontPlatformData&, scoped_refptr<CustomFontData> custom_data, bool subpixel_ascent_descent = false,
    ...
    scoped_refptr<SimpleFontData> CreateScaledFontData(const FontDescription&, float scale_factor) const;
    ...
    mutable std::unique_ptr<DerivedFontData> derived_font_data_ GUARDED_BY(derived_font_data_lock_);
...
ALWAYS_INLINE gfx::RectF SimpleFontData::BoundsForGlyph(Glyph glyph) const
{
    ...
    if (glyph_to_bounds_map_) {
        if (absl::optional<gfx::RectF> glyph_bounds = glyph_to_bounds_map_->MetricsForGlyph(glyph)) {
            return *glyph_bounds;
        }
    }
