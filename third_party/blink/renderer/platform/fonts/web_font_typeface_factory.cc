...
bool WebFontTypefaceFactory::CreateTypeface(sk_sp<SkData> sk_data, sk_sp<SkTypeface>& typeface)
{
    CHECK(!typeface);
    ...
    if (format_check.IsVariableFont() && format_check.IsColrCpalColorFontV0()) {
        typeface = FontManagerForColrV0Variations()->makeFromStream(std::move(stream));
        if (typeface)
    ...
    if (format_check.IsVariableFont()) {
        ...
        if (typeface) {
            ...
            ReportInstantiationResult(InstantiationResult::kErrorInstantiatingVariableFont);
        }
        return typeface.get();
    }
...
{
#if  0 // BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC)
...
{
#if BUILDFLAG(IS_WIN)
    if (DWriteVersionSupportsVariations() && blink::DWriteRasterizerSupport::IsDWriteFactory2Available())
        return DefaultFontManager();
#endif
    return FreeTypeFontManager();
}
