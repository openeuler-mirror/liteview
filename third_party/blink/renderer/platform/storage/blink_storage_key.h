...
public:
    ...
    BlinkStorageKey(scoped_refptr<const SecurityOrigin> origin, const BlinkSchemefulSite& top_level_site);
...
private:
    BlinkStorageKey(scoped_refptr<const SecurityOrigin> origin, const base::UnguessableToken* nonce);
    ...
    mojom::blink::AncestorChainBit ancestor_chain_bit_ { mojom::blink::AncestorChainBit::kSameSite };
    // Stores the value `ancestor_chain_bit_` would have had if
    // `kThirdPartyStoragePartitioning` were enabled. This isn't used in
    // serialization or comparison.
    // TODO(crbug.com/1159586): Remove when no longer needed.
    mojom::blink::AncestorChainBit ancestor_chain_bit_if_third_party_enabled_ { mojom::blink::AncestorChainBit::kSameSite };
};
