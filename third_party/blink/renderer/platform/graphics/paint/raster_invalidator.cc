...
const PaintChunk& RasterInvalidator::GetOldChunk(wtf_size_t index) const
{
    ...
    const auto& old_chunk = old_paint_artifact_->PaintChunks()[old_chunk_info.index_in_paint_artifact];
#if DCHECK_IS_ON()
...
static bool ApproximatelyEqual(const SkMatrix& a, const SkMatrix& b)
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if ((i == 0 || i == 4 || i == 9) && difference > (std::abs(a[i]) + std::abs(b[i])) * kTolerance)
            return false;
    }
    return true;
}
...
PaintInvalidationReason RasterInvalidator::ChunkPropertiesChanged(const PaintChunk& new_chunk, const PaintChunk& old_chunk,
    const PaintChunkInfo& new_chunk_info, const PaintChunkInfo& old_chunk_info, const PropertyTreeState& layer_state) const
{
    ...
    if (!ApproximatelyEqual(new_chunk_info.chunk_to_layer_transform, old_chunk_info.chunk_to_layer_transform))
        return PaintInvalidationReason::kPaintProperty;
...
void RasterInvalidator::GenerateRasterInvalidations(RasterInvalidationFunction function, const PaintChunkSubset& new_chunks, bool layer_offset_or_state_changed,
    bool layer_effect_changed, Vector<PaintChunkInfo>& new_chunks_info)
{
    ...
    for (auto it = new_chunks.begin(); it != new_chunks.end(); ++it) {
        ...
        if (matched_old_index == kNotFound) {
            ...
            AddRasterInvalidation(function, new_chunk_info.bounds_in_layer, new_chunk.id.client_id,
        ...
        if (!layer_offset_or_state_changed && reason == PaintInvalidationReason::kNone && new_chunk.is_moved_from_cached_subsequence
            && !new_chunk.properties.GetPropertyTreeState().Changed(PaintPropertyChangeType::kChangedOnlySimpleValues, layer_state_)) {
            ...
            if (reason == PaintInvalidationReason::kNone) {
                if (layer_effect_changed) {
                    ...
                    reason = ChunkPropertiesChanged(new_chunk, old_chunk, new_chunk_info, old_chunk_info, layer_state_);
                }
            }
            ...
            if (IsFullPaintInvalidationReason(reason)) {
                ...
                AddRasterInvalidation(function, old_chunk_info.bounds_in_layer, new_chunk.id.client_id, reason, kClientIsNew);
                if (old_chunk_info.bounds_in_layer != new_chunk_info.bounds_in_layer) {
                    AddRasterInvalidation(function, new_chunk_info.bounds_in_layer, new_chunk.id.client_id, reason, kClientIsNew);
                }
                ...
                if (reason == PaintInvalidationReason::kIncremental) {
                    IncrementallyInvalidateChunk(function, old_chunk_info, new_chunk_info, new_chunk.id.client_id);
                }
                ...
                if (&new_chunks.GetPaintArtifact() != old_paint_artifact_ && !new_chunk.is_moved_from_cached_subsequence) {
                    DisplayItemRasterInvalidator(
                        *this, function, old_paint_artifact_->DisplayItemsInChunk(old_chunk_info.index_in_paint_artifact), it.DisplayItems(), mapper)
                        .Generate();
                }
            }
        }
    ...
    for (wtf_size_t i = 0; i < old_paint_chunks_info_.size(); ++i) {
        ...
        AddRasterInvalidation(function, old_paint_chunks_info_[i].bounds_in_layer, old_chunk.id.client_id, reason, kClientIsOld);
    }
}

void RasterInvalidator::IncrementallyInvalidateChunk(
    RasterInvalidationFunction function, const PaintChunkInfo& old_chunk_info, const PaintChunkInfo& new_chunk_info, DisplayItemClientId client_id)
{
    ...
    for (SkRegion::Iterator it(diff); !it.done(); it.next()) {
        AddRasterInvalidation(function, gfx::SkIRectToRect(it.rect()), client_id, PaintInvalidationReason::kIncremental, kClientIsNew);
    }
}
...
void RasterInvalidator::Generate(RasterInvalidationFunction raster_invalidation_function, const PaintChunkSubset& new_chunks,
    const gfx::Vector2dF& layer_offset, const gfx::Size& layer_bounds, const PropertyTreeState& layer_state, DisplayItemClientId layer_client_id)
{
    ...
    if (layer_bounds_was_empty || layer_bounds_.IsEmpty()) {
        ...
        if (!layer_bounds.IsEmpty() && !new_chunks.IsEmpty()) {
            AddRasterInvalidation(raster_invalidation_function, gfx::Rect(layer_bounds), layer_client_id ? layer_client_id : new_chunks.begin()->id.client_id,
                PaintInvalidationReason::kFullLayer, kClientIsNew);
        }
    } else {
        GenerateRasterInvalidations(raster_invalidation_function, new_chunks, layer_offset_or_state_changed, layer_effect_changed, new_chunks_info);
    }
...
void RasterInvalidator::SetOldPaintArtifact(scoped_refptr<const PaintArtifact> old_paint_artifact)
{
