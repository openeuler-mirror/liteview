...
public:
    ...
    {
        return paint_chunker_.WillForceNewChunk();
    }
    ...
    void RecordHitTestData(const DisplayItemClient&, const gfx::Rect&, TouchAction, bool);

    void RecordRegionCaptureData(const DisplayItemClient& client, const RegionCaptureCropId& crop_id, const gfx::Rect& rect);

    void RecordScrollHitTestData(const DisplayItemClient&, DisplayItem::Type, const TransformPaintPropertyNode* scroll_translation, const gfx::Rect&);

    void RecordSelection(absl::optional<PaintedSelectionBound> start, absl::optional<PaintedSelectionBound> end);
    ...
    public:
        ScopedBenchmarkMode(PaintController& paint_controller, PaintBenchmarkMode mode)
    ...
    PaintBenchmarkMode GetBenchmarkMode() const
    {
...
private:
    ...
    void StartCycle(HeapVector<Member<const DisplayItemClient>>& clients_to_validate, bool record_debug_info);
    ...
    WeakPersistent<HeapVector<Member<const DisplayItemClient>>> clients_to_validate_ = nullptr;
...
public:
    ...
    {
        clients_to_validate_ = MakeGarbageCollected<HeapVector<Member<const DisplayItemClient>>>();
    }
    explicit PaintControllerCycleScope(PaintController& controller, bool record_debug_info)
