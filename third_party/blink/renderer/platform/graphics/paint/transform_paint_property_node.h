...
class PLATFORM_EXPORT TransformPaintPropertyNodeOrAlias : public PaintPropertyNode<TransformPaintPropertyNodeOrAlias, TransformPaintPropertyNode> {
public:
...
class TransformPaintPropertyNodeAlias : public TransformPaintPropertyNodeOrAlias {
public:
    static scoped_refptr<TransformPaintPropertyNodeAlias> Create(const TransformPaintPropertyNodeOrAlias& parent)
    {
        return base::AdoptRef(new TransformPaintPropertyNodeAlias(parent));
    }

    PaintPropertyChangeType SetParent(const TransformPaintPropertyNodeOrAlias& parent)
    {
...
private:
    explicit TransformPaintPropertyNodeAlias(const TransformPaintPropertyNodeOrAlias& parent)
        : TransformPaintPropertyNodeOrAlias(parent, kParentAlias)
    {
    }
};

class PLATFORM_EXPORT TransformPaintPropertyNode : public TransformPaintPropertyNodeOrAlias {
public:
    ...
    public:
        ...
        // NOLINTNEXTLINE(google-explicit-constructor)
        TransformAndOrigin(const TransformationMatrix& matrix, const gfx::Point3F& origin = gfx::Point3F())
        {
        ...
        bool TransformEquals(const TransformAndOrigin& other) const
        {
            return translation_2d_ == other.translation_2d_
                && ((!matrix_and_origin_ && !other.matrix_and_origin_)
                    || (matrix_and_origin_ && other.matrix_and_origin_ && matrix_and_origin_->matrix == other.matrix_and_origin_->matrix));
        }
    ...
    public:
        ...
        AnchorScrollContainersData(scoped_refptr<const TransformPaintPropertyNode> inner_most_scroll_container,
            scoped_refptr<const TransformPaintPropertyNode> outer_most_scroll_container, gfx::Vector2d accumulated_scroll_origin)
        ...
        {
            return inner_most_scroll_container == other.inner_most_scroll_container && outer_most_scroll_container == other.outer_most_scroll_container
    ...
    public:
        ...
        scoped_refptr<const TransformPaintPropertyNode> scroll_translation_for_fixed;
    ...
    static scoped_refptr<TransformPaintPropertyNode> Create(const TransformPaintPropertyNodeOrAlias& parent, State&& state)
    {
        return base::AdoptRef(new TransformPaintPropertyNode(&parent, std::move(state)));
    }
    ...
    bool IsIdentity() const
    {
        return state_.transform_and_origin.IsIdentity();
    }
    ...
    bool FlattensInheritedTransformSameAsParent() const
    {
        if (IsRoot())
            return true;
        return state_.flags.flattens_inherited_transform == Parent()->Unalias().state_.flags.flattens_inherited_transform;
    }
    ...
    {
        return state_.flags.is_for_svg_child;
    }

    std::unique_ptr<JSONObject> ToJSON() const;

private:
    friend class PaintPropertyNode<TransformPaintPropertyNodeOrAlias, TransformPaintPropertyNode>;

    TransformPaintPropertyNode(const TransformPaintPropertyNodeOrAlias* parent, State&& state)
        : TransformPaintPropertyNodeOrAlias(parent)
        , state_(std::move(state))
    {
        Validate();
    }
