...
PaintPropertyChangeType EffectPaintPropertyNode::State::ComputeChange(const State& other, const AnimationState& animation_state) const
{
    if (local_transform_space != other.local_transform_space || output_clip != other.output_clip || blend_mode != other.blend_mode
        || document_transition_shared_element_id != other.document_transition_shared_element_id
        || shared_element_resource_id != other.shared_element_resource_id) {
        return PaintPropertyChangeType::kChangedOnlyValues;
    }
    bool opacity_changed = opacity != other.opacity;
    bool opacity_change_is_simple = opacity_changed &&
        ...
        ((opacity != 1.f && other.opacity != 1.f)
            || ((direct_compositing_reasons & CompositingReason::kActiveOpacityAnimation)
                && (other.direct_compositing_reasons & CompositingReason::kActiveOpacityAnimation)));
...
const EffectPaintPropertyNode& EffectPaintPropertyNode::Root()
{
    DEFINE_STATIC_REF(EffectPaintPropertyNode, root,
        base::AdoptRef(new EffectPaintPropertyNode(nullptr, State { &TransformPaintPropertyNode::Root(), &ClipPaintPropertyNode::Root() })));
    return *root;
}
