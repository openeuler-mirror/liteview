...
std::unique_ptr<JSONArray> DisplayItemList::DisplayItemsAsJSON(
    const PaintArtifact& paint_artifact, wtf_size_t first_item_index, const DisplayItemRange& display_items, JsonFlags flags)
{
    auto json_array = std::make_unique<JSONArray>();
    wtf_size_t i = first_item_index;
    DCHECK(!(flags & kCompact) || !(flags & kShowPaintRecords)) << "kCompact and kShowPaintRecords are exclusive";
    for (auto& item : display_items) {
        if (flags & kCompact) {
            ...
            item.PropertiesAsJSON(*json, paint_artifact, flags & kClientKnownToBeAlive);
