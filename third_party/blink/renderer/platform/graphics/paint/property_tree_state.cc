...
bool ClipChainHasCompositedTransformTo(const ClipPaintPropertyNode& node, const ClipPaintPropertyNode& ancestor, const TransformPaintPropertyNode& transform)
{
    ...
    for (const auto* n = &node; n != &ancestor; n = n->UnaliasedParent()) {
        if (composited_ancestor != n->LocalTransformSpace().Unalias().NearestDirectlyCompositedAncestor())
            return true;
    }
    return false;
}
...
{
    DEFINE_STATIC_LOCAL(const PropertyTreeState, root, (TransformPaintPropertyNode::Root(), ClipPaintPropertyNode::Root(), EffectPaintPropertyNode::Root()));
    return root;
}

bool PropertyTreeStateOrAlias::Changed(PaintPropertyChangeType change, const PropertyTreeState& relative_to) const
{
...
absl::optional<PropertyTreeState> PropertyTreeState::CanUpcastWith(const PropertyTreeState& guest) const
{
    ...
    if (&Transform() == &guest.Transform()) {
        ...
        if (Transform().NearestDirectlyCompositedAncestor() != guest.Transform().NearestDirectlyCompositedAncestor())
            return absl::nullopt;
    ...
    if (&Clip() == &guest.Clip()) {
        ...
        if (ClipChainHasCompositedTransformTo(Clip(), *upcast_clip, *upcast_transform)
            || ClipChainHasCompositedTransformTo(guest.Clip(), *upcast_clip, *upcast_transform)) {
            return absl::nullopt;
        }
    }
...
std::ostream& operator<<(std::ostream& os, const PropertyTreeStateOrAlias& state)
{
