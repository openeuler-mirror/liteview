...
public:
    static void DrawNothing(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type)
    {
    ...
    static void DrawRect(GraphicsContext& context, const DisplayItemClient& client, DisplayItem::Type type, const gfx::Rect& bounds)
    {
        if (DrawingRecorder::UseCachedDrawingIfPossible(context, client, type))
            return;
...
protected:
    ...
    void InitRootChunk()
    {
        InitRootChunk(GetPaintController());
    }
    ...
    void InvalidateAll()
    {
        paint_controller_->InvalidateAllForTesting();
    }
