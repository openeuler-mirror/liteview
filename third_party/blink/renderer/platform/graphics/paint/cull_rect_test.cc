...
protected:
    ...
    void ApplyTransforms(CullRect& cull_rect, const TransformPaintPropertyNode& source, const TransformPaintPropertyNode& destination,
        const absl::optional<CullRect>& old_cull_rect)
    {
        ...
        cull_rect.ApplyPaintProperties(PropertyTreeState::Root(), source_state, destination_state, old_cull_rect);
    }

    bool ApplyScrollTranslation(CullRect& cull_rect, const TransformPaintPropertyNode& t)
    {
        return cull_rect.ApplyScrollTranslation(t, t);
    }

    bool ChangedEnough(const gfx::Rect& old_rect, const gfx::Rect& new_rect, const absl::optional<gfx::Rect>& bounds = absl::nullopt)
    {
...
TEST_F(CullRectTest, ApplyNonCompositedScrollTranslationPartialScrollingContents)
{
...
TEST_F(CullRectTest, SingleScrollWholeScrollingContents)
{
    auto t1 = CreateTransform(t0(), MakeTranslationMatrix(1, 2));
    PropertyTreeState state1(*t1, c0(), e0());
    auto ref_scroll_translation_state = CreateCompositedScrollTranslationState(state1, -10, -15, gfx::Rect(20, 10, 40, 50), gfx::Size(2000, 2000));
    auto scroll_translation_state = ref_scroll_translation_state.GetPropertyTreeState();

    // Same as ApplyScrollTranslationWholeScrollingContents.
    CullRect cull_rect1(gfx::Rect(0, 0, 50, 100));
    cull_rect1.ApplyPaintProperties(state1, state1, scroll_translation_state, absl::nullopt);
    ...
    // Should ignore old_cull_rect.
    cull_rect2.ApplyPaintProperties(state1, state1, scroll_translation_state, old_cull_rect);
    EXPECT_EQ(cull_rect1, cull_rect2);

    CullRect cull_rect3 = CullRect::Infinite();
    cull_rect3.ApplyPaintProperties(state1, state1, scroll_translation_state, absl::nullopt);
...
TEST_F(CullRectTest, SingleScrollPartialScrollingContents)
{
    ...
    auto ref_scroll_translation_state = CreateCompositedScrollTranslationState(state1, -3000, -5000, gfx::Rect(20, 10, 40, 50), gfx::Size(8000, 8000));
    auto scroll_translation_state = ref_scroll_translation_state.GetPropertyTreeState();
    ...
    cull_rect3.ApplyPaintProperties(state1, state1, scroll_translation_state, old_cull_rect);
    EXPECT_EQ(cull_rect1, cull_rect3);

    CullRect cull_rect4 = CullRect::Infinite();
    cull_rect4.ApplyPaintProperties(state1, state1, scroll_translation_state, absl::nullopt);
...
TEST_F(CullRectTest, SmallScrollContentsAfterBigScrollContents)
{
    ...
    auto ref_scroll_translation_state1 = CreateCompositedScrollTranslationState(state1, -10, -15, gfx::Rect(20, 10, 40, 50), gfx::Size(8000, 8000));
    auto scroll_translation_state1 = ref_scroll_translation_state1.GetPropertyTreeState();

    auto t2 = CreateTransform(scroll_translation_state1.Transform(), MakeTranslationMatrix(2000, 3000));
    PropertyTreeState state2(*t2, scroll_translation_state1.Clip(), e0());

    auto ref_scroll_translation_state2 = CreateCompositedScrollTranslationState(state2, -10, -15, gfx::Rect(30, 20, 100, 200), gfx::Size(200, 400));
    auto scroll_translation_state2 = ref_scroll_translation_state2.GetPropertyTreeState();

    CullRect cull_rect1(gfx::Rect(0, 0, 50, 100));
    cull_rect1.ApplyPaintProperties(state1, state1, scroll_translation_state2, absl::nullopt);
    ...
    CullRect cull_rect2(gfx::Rect(0, 0, 50, 100));
    // Should ignore old_cull_rect.
    cull_rect2.ApplyPaintProperties(state1, state1, scroll_translation_state2, old_cull_rect);
    EXPECT_EQ(cull_rect1, cull_rect2);
}
...
TEST_F(CullRectTest, BigScrollContentsAfterSmallScrollContents)
{
    ...
    auto ref_scroll_translation_state1 = CreateCompositedScrollTranslationState(state1, -10, -15, gfx::Rect(30, 20, 100, 200), gfx::Size(200, 400));
    auto scroll_translation_state1 = ref_scroll_translation_state1.GetPropertyTreeState();

    auto t2 = CreateTransform(scroll_translation_state1.Transform(), MakeTranslationMatrix(10, 20));
    PropertyTreeState state2(*t2, scroll_translation_state1.Clip(), e0());

    auto ref_scroll_translation_state2 = CreateCompositedScrollTranslationState(state2, -3000, -5000, gfx::Rect(20, 10, 50, 100), gfx::Size(10000, 20000));
    auto scroll_translation_state2 = ref_scroll_translation_state2.GetPropertyTreeState();
    ...
    // Use the new cull rect if it changed enough.
    cull_rect3.ApplyPaintProperties(state1, state1, scroll_translation_state2, old_cull_rect);
    EXPECT_EQ(cull_rect1, cull_rect3);
}
...
TEST_F(CullRectTest, ClipAndCompositedScrollAndClip)
{
    ...
    auto t2 = CreateTransform(scroll_translation, TransformationMatrix(), gfx::Point3F(), CompositingReason::kWillChangeTransform);

    // c2a is out of view, but in the expansion area of the composited scroll.
    CullRect cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(scroll_translation, *c2a, e0()), absl::nullopt);
    EXPECT_EQ(gfx::Rect(0, 300, 100, 100), cull_rect.Rect());
    // Composited case. The cull rect should be expanded.
    cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(*t2, *c2a, e0()), absl::nullopt);
    EXPECT_EQ(gfx::Rect(-4000, -3700, 8100, 8100), cull_rect.Rect());

    // Using c2a with old cull rect.
    cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(scroll_translation, *c2a, e0()), CullRect(gfx::Rect(0, 310, 100, 100)));
    ...
    // Composited case. The cull rect should be expanded.
    cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(*t2, *c2a, e0()), CullRect(gfx::Rect(-3900, -3700, 8100, 8100)));
    // The new cull rect touches the left edge of the clipped expanded scrolling
    // contents bounds, so the old cull rect is not used.
    EXPECT_EQ(gfx::Rect(-4000, -3700, 8100, 8100), cull_rect.Rect());
    ...
    cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(scroll_translation, *c2b, e0()), absl::nullopt);
    EXPECT_EQ(gfx::Rect(), cull_rect.Rect());
    // Composited case. The cull rect should be still empty.
    cull_rect = CullRect::Infinite();
    cull_rect.ApplyPaintProperties(root, root, PropertyTreeState(*t2, *c2b, e0()), absl::nullopt);
    EXPECT_EQ(gfx::Rect(), cull_rect.Rect());
}
