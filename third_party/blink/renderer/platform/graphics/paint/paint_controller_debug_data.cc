...
public:
    PaintArtifactAsJSON(const PaintArtifact& artifact, const Vector<SubsequenceMarkers>& subsequences, DisplayItemList::JsonFlags flags)
        : artifact_(artifact)
    ...
    String ToString()
    {
        return ChunksAsJSONArrayRecursive(0, artifact_.PaintChunks().size())->ToPrettyJSONString();
    }
...
std::unique_ptr<JSONArray> PaintController::PaintArtifactAsJSON::ChunksAsJSONArrayRecursive(wtf_size_t start_chunk_index, wtf_size_t end_chunk_index)
{
    ...
    while (next_subsequence_ != subsequences_.end() && next_subsequence_->start_chunk_index < end_chunk_index) {
        ...
        if (chunk_index < subsequence.start_chunk_index) {
            artifact_.AppendChunksAsJSON(chunk_index, subsequence.start_chunk_index, *array, flags_);
        }
...
void PaintController::ShowDebugDataInternal(DisplayItemList::JsonFlags flags) const
{
    ...
    LOG(INFO) << "current paint artifact: "
              << (current_paint_artifact_ ? PaintArtifactAsJSON(*current_paint_artifact_, current_subsequences_.tree, current_list_flags).ToString().Utf8()
                                          : "null");

    LOG(INFO) << "new paint artifact: "
              << (new_paint_artifact_ ? PaintArtifactAsJSON(*new_paint_artifact_, new_subsequences_.tree,
