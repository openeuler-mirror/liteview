...
static sk_sp<PaintRecord> CreateRectRecord(const gfx::RectF& record_bounds)
{
    ...
    cc::PaintCanvas* canvas = recorder.beginRecording(record_bounds.width(), record_bounds.height());
    canvas->drawRect(gfx::RectFToSkRect(record_bounds), cc::PaintFlags());
    return recorder.finishRecordingAsPicture();
}

static sk_sp<PaintRecord> CreateRectRecordWithTranslate(const gfx::RectF& record_bounds, float dx, float dy)
{
    PaintRecorder recorder;
    cc::PaintCanvas* canvas = recorder.beginRecording(record_bounds.width(), record_bounds.height());
    canvas->save();
...
{
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, gfx::Rect(), nullptr, client_->VisualRectOutsetForRasterEffects());
    EXPECT_FALSE(item.DrawsContent());
}

TEST_F(DrawingDisplayItemTest, EmptyPaintRecord)
{
    DrawingDisplayItem item(client_->Id(), DisplayItem::Type::kDocumentBackground, gfx::Rect(), sk_make_sp<PaintRecord>(), RasterEffectOutset::kNone);
    EXPECT_FALSE(item.DrawsContent());
}
...
TEST_F(DrawingDisplayItemTest, EqualsForUnderInvalidation)
{
    ...
    DrawingDisplayItem empty_item(client_->Id(), DisplayItem::kDocumentBackground, gfx::Rect(), nullptr, client_->VisualRectOutsetForRasterEffects());
...
TEST_F(DrawingDisplayItemTest, OpaqueRectForDrawRRectUniform)
{
    ...
    for (float r = kRadiusStep; r < kSize / 2; r += kRadiusStep) {
        PaintRecorder recorder;
        recorder.beginRecording(kSize, kSize)->drawRRect(SkRRect::MakeRectXY(SkRect::MakeWH(kSize, kSize), r, r), flags);
...
{
    auto image = cc::PaintImageBuilder::WithDefault().set_paint_record(sk_make_sp<PaintRecord>(), gfx::Rect(), 0).set_id(1).TakePaintImage();
    PaintRecorder recorder;
    recorder.beginRecording(100, 100)->drawImageRect(image, SkRect::MakeEmpty(), SkRect::MakeEmpty(), SkCanvas::kFast_SrcRectConstraint);
