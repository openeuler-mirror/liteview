...
public:
    ...
    bool MightOverlapForCompositing(const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2)
    {
...
void GeometryMapperTest::CheckLocalToAncestorVisualRect()
{
    ...
    LocalToAncestorVisualRectInternal(local_state, ancestor_state, actual_visual_rect);
    EXPECT_CLIP_RECT_EQ(expected_visual_rect, actual_visual_rect);

    actual_visual_rect = FloatClipRect(input_rect);
    LocalToAncestorVisualRectForCompositingOverlap(local_state, ancestor_state, actual_visual_rect);
    EXPECT_CLIP_RECT_EQ(
...
{
    FloatClipRect actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(local_state, ancestor_state);
...
void GeometryMapperTest::CheckSourceToDestinationRect()
{
    ...
    GeometryMapper::SourceToDestinationRect(local_state.Transform(), ancestor_state.Transform(), actual_transformed_rect);
    EXPECT_EQ(expected_transformed_rect, actual_transformed_rect);
}

void GeometryMapperTest::CheckSourceToDestinationProjection()
{
    const auto& actual_transform_to_ancestor = GeometryMapper::SourceToDestinationProjection(local_state.Transform(), ancestor_state.Transform());
    if (expected_transform) {
        EXPECT_EQ(*expected_transform, actual_transform_to_ancestor.Matrix());
    } else {
        EXPECT_EQ(expected_translation_2d, actual_transform_to_ancestor.Translation2D());
    }
}
...
void GeometryMapperTest::CheckCachedClip()
{
    ...
    if (&ancestor_state.Clip() == &local_clip
        || (&ancestor_state.Clip() == local_clip.Parent() && &ancestor_state.Transform() == &local_clip.LocalTransformSpace())) {
...
TEST_P(GeometryMapperTest, RotationAndScaleTransformWithTransformOrigin)
{
    expected_transform = MakeRotationMatrix(45);
    expected_transform->Scale(2, 2);
    auto transform = CreateTransform(t0(), *expected_transform, gfx::Point3F(50, 50, 0));
    local_state.SetTransform(*transform);
...
TEST_P(GeometryMapperTest, NestedTransformsFlattening)
{
    ...
    auto transform2 = TransformPaintPropertyNode::Create(*transform1, std::move(inverse_state));
    local_state.SetTransform(*transform2);
...
TEST_P(GeometryMapperTest, SimpleClipOverlayScrollbars)
{
    ...
    FloatClipRect actual_visual_rect(input_rect);
    GeometryMapper::LocalToAncestorVisualRect(local_state, ancestor_state, actual_visual_rect, kExcludeOverlayScrollbarSizeForHitTesting);
    ...
    // a different result.
    actual_visual_rect = FloatClipRect(input_rect);
    GeometryMapper::LocalToAncestorVisualRect(local_state, ancestor_state, actual_visual_rect, kIgnoreOverlayScrollbarSize);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 50, 50)), actual_visual_rect);

    FloatClipRect actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(local_state, ancestor_state, kExcludeOverlayScrollbarSizeForHitTesting);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(10, 10, 45, 43)), actual_clip_rect);

    // Check that not passing kExcludeOverlayScrollbarSizeForHitTesting gives
    // a different result.
    actual_clip_rect = GeometryMapper::LocalToAncestorClipRect(local_state, ancestor_state, kIgnoreOverlayScrollbarSize);
...
TEST_P(GeometryMapperTest, SimpleClipInclusiveIntersect)
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(local_state, ancestor_state, actual_clip_rect, kIgnoreOverlayScrollbarSize, kInclusiveIntersect);
    EXPECT_CLIP_RECT_EQ(FloatClipRect(gfx::RectF(60, 10, 0, 10)), actual_clip_rect);

    // Check that not passing kExcludeOverlayScrollbarSizeForHitTesting gives
    // a different result.
    actual_clip_rect.SetRect(gfx::RectF(60, 10, 10, 10));
    GeometryMapper::LocalToAncestorVisualRect(local_state, ancestor_state, actual_clip_rect, kIgnoreOverlayScrollbarSize, kNonInclusiveIntersect);
...
TEST_P(GeometryMapperTest, SimpleClipPlusOpacity)
{
    ...
    FloatClipRect actual_clip_rect(gfx::RectF(60, 10, 10, 10));
    auto intersects = GeometryMapper::LocalToAncestorVisualRect(local_state, ancestor_state, actual_clip_rect);
...
TEST_P(GeometryMapperTest, ExpandVisualRectWithClipBeforeSticky)
{
    ...
    auto transform = CreateTransform(t0(), *expected_transform, gfx::Point3F(), CompositingReason::kStickyPosition);
    auto clip = CreateClip(c0(), *transform, FloatRoundedRect(10, 10, 50, 50));
    local_state.SetClip(*clip);
    local_state.SetTransform(*transform);
...
TEST_P(GeometryMapperTest, TwoClipsWithTransformBetween)
{
    ...
    expected_transform = MakeRotationMatrix(45);
    auto transform = CreateTransform(t0(), *expected_transform);
    auto clip2 = CreateClip(*clip1, *transform, FloatRoundedRect(10, 10, 200, 200));
    local_state.SetClip(*clip2);
    local_state.SetTransform(*transform);
...
TEST_P(GeometryMapperTest, ExpandVisualRectForFixed)
{
    ...
    auto scroll_state
        = CreateCompositedScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -100, -200, gfx::Rect(0, 0, 800, 600), gfx::Size(2400, 1800));

    auto fixed_transform = CreateFixedPositionTranslation(*viewport, 200, 200, scroll_state.Transform());
    ...
    expected_transformed_rect = gfx::RectF(gfx::PointAtOffsetFromOrigin(descendant_offset), child_of_fixed_size);
    expected_visual_rect = FloatClipRect(expected_transformed_rect);
    expected_visual_rect_expanded_for_compositing = FloatClipRect(gfx::RectF(150, 50, 1700, 1300));
    ...
    expected_transform = MakeTranslationMatrix(descendant_offset.x(), descendant_offset.y());
    expected_visual_rect.ClearIsTight();
...
TEST_P(GeometryMapperTest, FilterWithClipsAndTransforms)
{
    ...
    auto effect = CreateFilterEffect(e0(), *transform_above_effect, clip_above_effect.get(), filters);
    ...
    local_state = PropertyTreeState(*transform_below_effect, *clip_below_effect, *effect);
    ...
    expected_translation_2d = transform_above_effect->Translation2D() + transform_below_effect->Translation2D();
    expected_transformed_rect = input_rect;
    expected_transformed_rect.Offset(expected_translation_2d);
...
TEST_P(GeometryMapperTest, FilterWithClipsAndTransformsWithAlias)
{
    ...
    CompositorFilterOperations filters;
    filters.AppendBlurFilter(20);
    auto real_effect = CreateFilterEffect(e0(), *transform_above_effect, clip_above_effect.get(), filters);
    ...
    local_state = PropertyTreeStateOrAlias(*transform_below_effect, *clip_below_effect, *effect);
...
{
    auto clip1 = CreateClip(c0(), t0(), FloatRoundedRect(10, 10, 200, 200));
    auto effect = CreateAnimatingFilterEffect(e0(), CompositorFilterOperations(), clip1.get());
...
TEST_P(GeometryMapperTest, MightOverlapFixed)
{
    ...
    // which is (100, 200, 1700, 1300) in the scrolling space.
    {
        ...
        CheckOverlap(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(100, 200, 1700, 1300), scroll_state1.GetPropertyTreeState());
    }
    ...
    if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
        {
            SCOPED_TRACE("fixed_state and scroll_state1");
            auto scroll_state2
                = CreateScrollTranslationState(scroll_state1.GetPropertyTreeState(), -2345, -678, gfx::Rect(20, 10, 200, 100), gfx::Size(3000, 2000));
            ...
            EXPECT_FALSE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(1, 2, 3, 4), scroll_state2.GetPropertyTreeState()));
        }
        {
            SCOPED_TRACE("fixed_state and scroll_state1");
            auto scroll_state3
                = CreateScrollTranslationState(scroll_state1.GetPropertyTreeState(), -234, -567, gfx::Rect(0, 300, 500, 500), gfx::Size(1000, 2000));
            EXPECT_TRUE(MightOverlapForCompositing(gfx::RectF(0, 0, 100, 100), fixed_state, gfx::RectF(1, 2, 3, 4), scroll_state3.GetPropertyTreeState()));
        }
    }
}
...
TEST_P(GeometryMapperTest, MightOverlapScroll)
{
    ...
    auto viewport = CreateTransform(t0(), TransformationMatrix());
    auto scroll_state1
        = CreateScrollTranslationState(PropertyTreeState(*viewport, c0(), e0()), -1234, -567, gfx::Rect(10, 20, 100, 200), gfx::Size(2400, 1800));
    auto scroll_state2 = CreateScrollTranslationState(scroll_state1.GetPropertyTreeState(), -2345, -678, gfx::Rect(20, 10, 200, 100), gfx::Size(3000, 2000));

    auto transform_outside = Create2DTranslation(*viewport, 100, 200);
    PropertyTreeState state_outside(*transform_outside, c0(), e0());

    auto transform_under_scroll1 = Create2DTranslation(scroll_state1.Transform(), 34, 56);
    PropertyTreeState state_under_scroll1(*transform_under_scroll1, scroll_state1.Clip(), scroll_state1.Effect());

    auto transform_under_scroll2 = Create2DTranslation(scroll_state2.Transform(), 45, 67);
    PropertyTreeState state_under_scroll2(*transform_under_scroll2, scroll_state2.Clip(), scroll_state2.Effect());
    ...
    gfx::RectF outer_container_rect1_in_state_outside(-90, -180, 100, 200);
    {
        ...
        CheckOverlap(gfx::RectF(1, 2, 3, 4), scroll_state1.GetPropertyTreeState(), outer_container_rect1_in_state_outside, state_outside);
    }
    {
        SCOPED_TRACE("state_under_scroll1 and state_outside");
        CheckOverlap(gfx::RectF(1, 2, 3, 4), state_under_scroll1, outer_container_rect1_in_state_outside, state_outside);
    }
    {
        SCOPED_TRACE("scroll_state2 and state_outside");
        CheckOverlap(gfx::RectF(3, 4, 5, 6), scroll_state2.GetPropertyTreeState(), outer_container_rect1_in_state_outside, state_outside);
    }
    {
        SCOPED_TRACE("state_under_scroll2 and state_outside");
        CheckOverlap(gfx::RectF(3, 4, 5, 6), state_under_scroll2, outer_container_rect1_in_state_outside, state_outside);
    }
    ...
    // scroll_state2.
    {
        ...
        CheckOverlap(gfx::RectF(5, 6, 7, 8), scroll_state2.GetPropertyTreeState(), gfx::RectF(20, 10, 200, 100), scroll_state1.GetPropertyTreeState());
    }
    {
        SCOPED_TRACE("state_under_scroll2 and scroll_state1");
        CheckOverlap(gfx::RectF(5, 6, 7, 8), state_under_scroll2, gfx::RectF(20, 10, 200, 100), scroll_state1.GetPropertyTreeState());
    }
    {
        SCOPED_TRACE("state_under_scroll2 and state_under_scroll1");
        CheckOverlap(gfx::RectF(7, 8, 9, 10), state_under_scroll2, gfx::RectF(-14, -46, 200, 100), state_under_scroll1);
    }
}
