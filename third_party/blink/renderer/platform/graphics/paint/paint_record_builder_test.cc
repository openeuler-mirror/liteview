...
TEST_F(PaintRecordBuilderTest, TransientPaintController)
{
    ...
    EXPECT_THAT(context.GetPaintController().GetDisplayItemList(), ElementsAre(IsSameId(client.Id(), kBackgroundType), IsSameId(client.Id(), kForegroundType)));
    EXPECT_FALSE(ClientCacheIsValid(context.GetPaintController(), client));
}
...
{
    FakeDisplayItemClient& client = *MakeGarbageCollected<FakeDisplayItemClient>("client");
    auto* builder = MakeGarbageCollected<PaintRecordBuilder>(GetPaintController());
    auto& context = builder->Context();
    MockPaintCanvas canvas;
    cc::PaintFlags flags;
    {
    ...
    EXPECT_THAT(GetPaintController().GetDisplayItemList(), ElementsAre(IsSameId(client.Id(), kBackgroundType), IsSameId(client.Id(), kForegroundType)));

    {
        PaintControllerCycleScopeForTest cycle_scope(GetPaintController());
        InitRootChunk();
        EXPECT_TRUE(DrawingRecorder::UseCachedDrawingIfPossible(context, client, kBackgroundType));
        EXPECT_TRUE(DrawingRecorder::UseCachedDrawingIfPossible(context, client, kForegroundType));
        EXPECT_CALL(canvas, drawPicture(_)).Times(1);
        builder->EndRecording(canvas);
    }
