...
void PaintChunker::StartMarkingClientsForValidation(HeapVector<Member<const DisplayItemClient>>& clients_to_validate)
{
#if DCHECK_IS_ON()
    DCHECK(IsInInitialState());
#endif
...
bool PaintChunker::EnsureCurrentChunk(const PaintChunk::Id& id, const DisplayItemClient& client)
{
    ...
    if (WillForceNewChunk() || current_properties_ != chunks_->back().properties) {
        if (!next_chunk_id_) {
...
bool PaintChunker::IncrementDisplayItemIndex(const DisplayItemClient& client, const DisplayItem& item)
{
    ...
    if (item.IsDrawing() && item.DrawsContent()) {
        ...
        Color item_color = Color::FromSkColor(To<DrawingDisplayItem>(item).BackgroundColor(item_area));
...
bool PaintChunker::AddHitTestDataToCurrentChunk(
    const PaintChunk::Id& id, const DisplayItemClient& client, const gfx::Rect& rect, TouchAction touch_action, bool blocking_wheel)
{
    bool created_new_chunk = EnsureCurrentChunk(id, client);
    auto& chunk = chunks_->back();
    chunk.bounds.Union(rect);
    if (touch_action != TouchAction::kAuto) {
        chunk.EnsureHitTestData().touch_action_rects.push_back(TouchActionRect { rect, touch_action });
    }
    if (blocking_wheel)
...
void PaintChunker::AddSelectionToCurrentChunk(absl::optional<PaintedSelectionBound> start, absl::optional<PaintedSelectionBound> end)
{
...
void PaintChunker::CreateScrollHitTestChunk(
    const PaintChunk::Id& id, const DisplayItemClient& client, const TransformPaintPropertyNode* scroll_translation, const gfx::Rect& rect)
{
#if DCHECK_IS_ON()
    if (id.type == DisplayItem::Type::kResizerScrollHitTest || id.type == DisplayItem::Type::kPluginScrollHitTest
    ...
    if (id.type == DisplayItem::Type::kScrollbarHitTest) {
        hit_test_data.touch_action_rects.push_back(TouchActionRect { rect, TouchAction::kNone });
    }
    SetWillForceNewChunk(true);
}

void PaintChunker::ProcessBackgroundColorCandidate(const PaintChunk::Id& id, const DisplayItemClient& client, Color color, float area)
{
    if (color == Color::kTransparent)
        return;

    bool created_new_chunk = EnsureCurrentChunk(id, client);
    float min_background_area = kMinBackgroundColorCoverageRatio * chunks_->back().bounds.width() * chunks_->back().bounds.height();
    if (created_new_chunk || area >= candidate_background_area_ || area >= min_background_area) {
        candidate_background_color_ = candidate_background_area_ >= min_background_area ? candidate_background_color_.Blend(color) : color;
