...
protected:
    ScrollbarDisplayItemTest()
        : scroll_state_(CreateScrollTranslationState(PropertyTreeState::Root(), 0, 0, gfx::Rect(0, 0, 100, 100), gfx::Size(1000, 1000)))
    {
    }
    ...
    CompositorElementId ScrollbarElementId(const cc::Scrollbar& scrollbar)
    {
        return CompositorElementIdFromUniqueObjectId(13579,
            scrollbar.Orientation() == cc::ScrollbarOrientation::HORIZONTAL ? CompositorElementIdNamespace::kHorizontalScrollbar
...
TEST_F(ScrollbarDisplayItemTest, HorizontalSolidColorScrollbar)
{
    ...
    ScrollbarDisplayItem display_item(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar, scrollbar_rect, &scroll_state_.Transform(), element_id,
    ...
    EXPECT_EQ(cc::ScrollbarOrientation::HORIZONTAL, scrollbar_layer->orientation());
...
TEST_F(ScrollbarDisplayItemTest, PaintedScrollbarOverlayNinePatch)
{
    ...
    ScrollbarDisplayItem display_item(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar, scrollbar_rect, &scroll_state_.Transform(), element_id,
        client.VisualRectOutsetForRasterEffects());
    auto layer = display_item.CreateOrReuseLayer(nullptr);
    ASSERT_EQ(cc::ScrollbarLayerBase::kPaintedOverlay, layer->GetScrollbarLayerType());
    EXPECT_TRUE(layer->HitTestable());
...
TEST_F(ScrollbarDisplayItemTest, CreateOrReuseLayer)
{
    ...
    ScrollbarDisplayItem display_item1a(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar1, scrollbar_rect, &scroll_state_.Transform(), element_id,
        client.VisualRectOutsetForRasterEffects());
    auto layer1 = display_item1a.CreateOrReuseLayer(nullptr);

    ScrollbarDisplayItem display_item1b(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar1, scrollbar_rect, &scroll_state_.Transform(), element_id,
        client.VisualRectOutsetForRasterEffects());
    ...
    ScrollbarDisplayItem display_item2(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar2, scrollbar_rect, &scroll_state_.Transform(), element_id,
        client.VisualRectOutsetForRasterEffects());
    ...
    ScrollbarDisplayItem display_item1c(client.Id(), DisplayItem::kScrollbarHorizontal, scrollbar1, scrollbar_rect, &scroll_state_.Transform(), element_id,
        client.VisualRectOutsetForRasterEffects());
    // Should reuse layer for a different display item and the same scrollbar.
