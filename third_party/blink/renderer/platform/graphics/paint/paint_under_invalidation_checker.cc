...
void PaintUnderInvalidationChecker::ShowItemError(const char* reason, const DisplayItem& new_item, const DisplayItem* old_item) const
{
    ...
    if (old_item) {
        LOG(ERROR) << "Old display item: " << old_item->AsDebugString(*paint_controller_.current_paint_artifact_);
    }
    ...
    const char* reason, DisplayItemClientId client_id, const PaintChunk* new_chunk, const PaintChunk* old_chunk)
{
    ...
    if (old_chunk) {
        LOG(ERROR) << "Old paint chunk: " << old_chunk->ToString(*paint_controller_.current_paint_artifact_);
    }
#if DCHECK_IS_ON()
    paint_controller_.ShowDebugData();
#else
...
const Vector<PaintChunk>& PaintUnderInvalidationChecker::OldPaintChunks() const
{
    return paint_controller_.current_paint_artifact_->PaintChunks();
}

const Vector<PaintChunk>& PaintUnderInvalidationChecker::NewPaintChunks() const
{
