...
public:
    ...
    PaintChunkSubset(scoped_refptr<const PaintArtifact> paint_artifact, wtf_size_t chunk_index)
    ...
    PaintChunkSubset(scoped_refptr<const PaintArtifact> paint_artifact)
        : PaintChunkSubset(paint_artifact, 0, paint_artifact->PaintChunks().size())
    {
    }

    // A subset defined by a range of segments. |end_segment_index| is not
    // inclusive.
    PaintChunkSubset(scoped_refptr<const PaintArtifact> paint_artifact, wtf_size_t begin_index, wtf_size_t end_index)
        : paint_artifact_(std::move(paint_artifact))
    ...
    public:
        ...
        {
            DCHECK_LE(subset_or_chunk_index_ + offset, subset_->end().subset_or_chunk_index_);
    ...
    private:
        ...
        Iterator(const PaintChunkSubset& subset, wtf_size_t subset_or_chunk_index)
            : subset_(&subset)
            , subset_or_chunk_index_(subset_or_chunk_index)
        {
        }
    ...
    Iterator end() const
    {
        return Iterator(*this, UsesSubsetIndices() ? subset_indices_.size() : end_index_);
    }

    bool IsEmpty() const
    {
        return UsesSubsetIndices() ? subset_indices_.empty() : begin_index_ == end_index_;
    }

    wtf_size_t size() const
    {
        return UsesSubsetIndices() ? subset_indices_.size() : end_index_ - begin_index_;
    }
    ...
    void SetPaintArtifact(scoped_refptr<const PaintArtifact> paint_artifact)
    {
        ...
        DCHECK_EQ(paint_artifact->PaintChunks().size(), paint_artifact_->PaintChunks().size());
        paint_artifact_ = std::move(paint_artifact);
    }
...
private:
    bool UsesSubsetIndices() const
    {
        return begin_index_ == kNotFound;
    }
