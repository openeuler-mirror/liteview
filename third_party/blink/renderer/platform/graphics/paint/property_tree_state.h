...
public:
    ...
    {
        return transform_ && clip_ && effect_;
    }
    ...
    bool Changed(PaintPropertyChangeType change, const PropertyTreeState& relative_to) const;
    ...
    {
        return transform_ == other.transform_ && clip_ == other.clip_ && effect_ == other.effect_;
    }
    bool operator!=(const PropertyTreeStateOrAlias& other) const
    {
        return !(*this == other);
...
public:
    PropertyTreeState(const TransformPaintPropertyNode& transform, const ClipPaintPropertyNode& clip, const EffectPaintPropertyNode& effect)
    ...
    // This is used as the initial value of uninitialized PropertyTreeState.
    // Access to the nodes are not allowed.
    static PropertyTreeState Uninitialized()
    {
    ...
    absl::optional<PropertyTreeState> CanUpcastWith(const PropertyTreeState& guest) const;
...
PLATFORM_EXPORT inline PropertyTreeState PropertyTreeStateOrAlias::Unalias() const
{
    return PropertyTreeState(Transform().Unalias(), Clip().Unalias(), Effect().Unalias());
}

PLATFORM_EXPORT std::ostream& operator<<(std::ostream&, const PropertyTreeStateOrAlias&);
