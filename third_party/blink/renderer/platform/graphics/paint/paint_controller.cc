...
PaintController::PaintController(Usage usage)
    : usage_(usage)
    , current_paint_artifact_(usage == kMultiplePaints ? base::MakeRefCounted<PaintArtifact>() : nullptr)
...
void PaintController::ReserveCapacity()
{
    ...
    if (current_paint_artifact_) {
        if (current_paint_artifact_->GetDisplayItemList().size()) {
            display_item_list_capacity = current_paint_artifact_->GetDisplayItemList().size();
        }
        new_paint_artifact_->PaintChunks().reserve(current_paint_artifact_->PaintChunks().size());
        new_subsequences_.tree.reserve(current_subsequences_.tree.size());
        new_subsequences_.map.ReserveCapacityForSize(current_subsequences_.map.size());
    }
...
void PaintController::RecordHitTestData(const DisplayItemClient& client, const gfx::Rect& rect, TouchAction touch_action, bool blocking_wheel)
{
    ...
    if (paint_chunker_.AddHitTestDataToCurrentChunk(id, client, rect, touch_action, blocking_wheel)) {
...
void PaintController::RecordScrollHitTestData(
    const DisplayItemClient& client, DisplayItem::Type type, const TransformPaintPropertyNode* scroll_translation, const gfx::Rect& rect)
{
...
void PaintController::RecordSelection(absl::optional<PaintedSelectionBound> start, absl::optional<PaintedSelectionBound> end)
{
...
bool PaintController::UseCachedItemIfPossible(const DisplayItemClient& client, DisplayItem::Type type)
{
    ...
    auto cached_item = FindCachedItem(client, DisplayItem::Id(client.Id(), type, current_fragment_));
    ...
    DisplayItem& new_item = new_paint_artifact_->GetDisplayItemList().AppendByMoving(current_paint_artifact_->GetDisplayItemList()[cached_item]);
...
bool PaintController::UseCachedSubsequenceIfPossible(const DisplayItemClient& client)
{
    ...
    wtf_size_t start_item_index = current_paint_artifact_->PaintChunks()[markers.start_chunk_index].begin_index;
    wtf_size_t end_item_index = current_paint_artifact_->PaintChunks()[markers.end_chunk_index - 1].end_index;
    if (end_item_index > start_item_index && current_paint_artifact_->GetDisplayItemList()[start_item_index].IsTombstone()) {
...
wtf_size_t PaintController::GetSubsequenceIndex(DisplayItemClientId client_id) const
{
...
const PaintController::SubsequenceMarkers* PaintController::GetSubsequenceMarkers(DisplayItemClientId client_id) const
{
...
void PaintController::CheckNewItem(DisplayItem& display_item)
{
    ...
    if (display_item.IsCacheable()) {
        ...
        auto index = FindItemFromIdIndexMap(display_item.GetId(), new_display_item_id_index_map_, new_display_item_list);
        if (index != kNotFound) {
            ShowDebugData();
            NOTREACHED() << "DisplayItem " << display_item.AsDebugString(*new_paint_artifact_).Utf8() << " has duplicated id with previous "
                         << new_display_item_list[index].AsDebugString(*new_paint_artifact_).Utf8() << " (index=" << index << ")";
        }
        AddToIdIndexMap(display_item.GetId(), new_display_item_list.size() - 1, new_display_item_id_index_map_);
    }
#endif
...
void PaintController::CheckNewChunkId(const PaintChunk::Id& id)
{
    ...
    if (it != new_paint_chunk_id_index_map_.end()) {
        ShowDebugData();
        NOTREACHED() << "New paint chunk id " << id.ToString(*new_paint_artifact_) << " is already used by a previous chuck "
                     << new_paint_artifact_->PaintChunks()[it->value].ToString(*new_paint_artifact_);
    }
#endif
}
...
{
    if (next_item_to_match_ < current_paint_artifact_->GetDisplayItemList().size()) {
        ...
        // new item.
        const DisplayItem& item = current_paint_artifact_->GetDisplayItemList()[next_item_to_match_];
    ...
    wtf_size_t found_index = FindItemFromIdIndexMap(id, out_of_order_item_id_index_map_, current_paint_artifact_->GetDisplayItemList());
...
{
    for (auto i = next_item_to_index_; i < current_paint_artifact_->GetDisplayItemList().size(); ++i) {
    ...
    const DisplayItemClient& client, wtf_size_t subsequence_index, wtf_size_t start_chunk_index, wtf_size_t end_chunk_index)
{
    ...
    DCHECK_EQ(client.Id(), current_subsequences_.tree[subsequence_index].client_id);
    ...
    for (auto chunk_index = start_chunk_index; chunk_index < end_chunk_index; ++chunk_index) {
        ...
        if (record_debug_info_) {
            ...
            new_paint_artifact_->RecordDebugInfo(new_chunk.id.client_id, current_paint_artifact_->ClientDebugName(new_chunk.id.client_id),
    ...
    new_display_item_list.AppendSubsequenceByMoving(
        current_paint_artifact_->GetDisplayItemList(), current_chunks[start_chunk_index].begin_index, current_chunks[end_chunk_index - 1].end_index);
    ...
    for (auto& item : new_display_item_list.ItemsInRange(new_item_start_index, new_display_item_list.size())) {
        ...
        if (record_debug_info_) {
            new_paint_artifact_->RecordDebugInfo(
                item.ClientId(), current_paint_artifact_->ClientDebugName(item.ClientId()), current_paint_artifact_->ClientOwnerNodeId(item.ClientId()));
        }
#if DCHECK_IS_ON()
        CheckNewItem(item);
#endif
    ...
    for (wtf_size_t i = subsequence_index + 1; i < current_subsequences_.tree.size(); i++) {
        auto& markers = current_subsequences_.tree[i];
        if (markers.start_chunk_index >= end_chunk_index)
            break;
        DCHECK(!new_subsequences_.map.Contains(markers.client_id))
            << "Multiple subsequences for client: " << current_paint_artifact_->ClientDebugName(markers.client_id);
...
void PaintController::CommitNewDisplayItems()
{
    TRACE_EVENT2("blink,benchmark", "PaintController::commitNewDisplayItems", "current_display_list_size",
        current_paint_artifact_ ? current_paint_artifact_->GetDisplayItemList().size() : 0, "num_non_cached_new_items",
...
void PaintController::StartCycle(HeapVector<Member<const DisplayItemClient>>& clients_to_validate, bool record_debug_info)
{
...
size_t PaintController::ApproximateUnsharedMemoryUsage() const
{
    ...
    memory_usage += new_subsequences_.map.Capacity() * sizeof(decltype(new_subsequences_.map)::value_type);
...
PaintUnderInvalidationChecker& PaintController::EnsureUnderInvalidationChecker()
{
    ...
    if (!under_invalidation_checker_) {
        under_invalidation_checker_ = std::make_unique<PaintUnderInvalidationChecker>(*this);
    }
