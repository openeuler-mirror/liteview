...
public:
    explicit PaintChunker(Vector<PaintChunk>& chunks)
    {
        ResetChunks(&chunks);
    }
    ...
    void StartMarkingClientsForValidation(HeapVector<Member<const DisplayItemClient>>& clients_to_validate);
    ...
    bool AddHitTestDataToCurrentChunk(const PaintChunk::Id&, const DisplayItemClient&, const gfx::Rect&, TouchAction, bool blocking_wheel);
    void CreateScrollHitTestChunk(const PaintChunk::Id&, const DisplayItemClient&, const TransformPaintPropertyNode* scroll_translation, const gfx::Rect&);
    ...
    // The id will be used when we need to create a new current chunk.
    // Otherwise it's ignored. Returns true if a new chunk is added.
    void AddSelectionToCurrentChunk(absl::optional<PaintedSelectionBound> start, absl::optional<PaintedSelectionBound> end);
...
private:
    ...
    void ProcessBackgroundColorCandidate(const PaintChunk::Id&, const DisplayItemClient&, Color color, float area);
    ...
    PropertyTreeStateOrAlias current_properties_ = PropertyTreeState::Uninitialized();
