...
sk_sp<PaintRecord> PaintArtifact::GetPaintRecord(const PropertyTreeState& replay_state) const
{
    return PaintChunksToCcLayer::Convert(PaintChunkSubset(this), replay_state, gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
        ->ReleaseAsRecord();
}
...
void PaintArtifact::AppendChunksAsJSON(wtf_size_t start_chunk_index, wtf_size_t end_chunk_index, JSONArray& json_array, unsigned flags) const
{
    ...
    for (auto i = start_chunk_index; i < end_chunk_index; ++i) {
        ...
        json_object->SetArray("displayItems", DisplayItemList::DisplayItemsAsJSON(*this, chunk.begin_index, DisplayItemsInChunk(i), flags));
#endif
