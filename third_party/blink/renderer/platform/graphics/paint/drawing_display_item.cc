...
static SkBitmap RecordToBitmap(sk_sp<const PaintRecord> record, const gfx::Rect& bounds)
{
    SkBitmap bitmap;
...
static bool BitmapsEqual(sk_sp<const PaintRecord> record1, sk_sp<const PaintRecord> record2, const gfx::Rect& bounds)
{
...
SkColor DrawingDisplayItem::BackgroundColor(float& area) const
{
    ...
    if (GetType() != DisplayItem::kBoxDecorationBackground && GetType() != DisplayItem::kDocumentBackground && GetType() != DisplayItem::kDocumentRootBackdrop
        && GetType() != DisplayItem::kCaret && GetType() != DisplayItem::kScrollCorner)
    ...
    for (cc::PaintOpBuffer::Iterator it(record_.get()); it; ++it) {
        ...
        if (flags.getLooper() || flags.getShader() || flags.getStyle() != cc::PaintFlags::kFill_Style) {
            continue;
        }
        SkRect item_rect;
        switch (op.GetType()) {
...
gfx::Rect DrawingDisplayItem::CalculateRectKnownToBeOpaqueForRecord(const PaintRecord* record) const
{
    ...
    for (cc::PaintOpBuffer::Iterator it(record); it; ++it) {
        ...
        if (op.GetType() == cc::PaintOpType::DrawRecord) {
            op_opaque_rect = CalculateRectKnownToBeOpaqueForRecord(static_cast<const cc::DrawRecordOp&>(op).record.get());
        } else {
            if (!op.IsPaintOpWithFlags())
                continue;
            ...
            if (flags.getStyle() != cc::PaintFlags::kFill_Style || flags.getLooper()
                || (flags.getBlendMode() != SkBlendMode::kSrc && flags.getBlendMode() != SkBlendMode::kSrcOver) || flags.getMaskFilter()
                || flags.getColorFilter() || flags.getImageFilter() || flags.getAlpha() != SK_AlphaOPAQUE
...
gfx::Rect DrawingDisplayItem::TightenVisualRect(const gfx::Rect& visual_rect, sk_sp<const PaintRecord>& record)
{
    ...
    if (flags.getStyle() != cc::PaintFlags::kFill_Style || flags.getLooper() || flags.getMaskFilter() || flags.getImageFilter() || flags.getShader()) {
        return visual_rect;
    }
...
bool DrawingDisplayItem::IsSolidColor() const
{
    ...
    switch (op.GetType()) {
    case cc::PaintOpType::DrawRect:
        solid_color_rect = gfx::SkRectToRectF(static_cast<const cc::DrawRectOp&>(op).rect);
        break;
    case cc::PaintOpType::DrawIRect:
        solid_color_rect = gfx::RectF(gfx::SkIRectToRect(static_cast<const cc::DrawIRectOp&>(op).rect));
        break;
    default:
        return false;
    }
