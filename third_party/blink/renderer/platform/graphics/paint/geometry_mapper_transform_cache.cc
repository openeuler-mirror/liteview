...
void GeometryMapperTransformCache::Update(const TransformPaintPropertyNode& node)
{
    ...
    has_sticky_ = node.RequiresCompositingForStickyPosition() || parent.has_sticky_;

    is_backface_hidden_ = node.IsBackfaceHiddenInternal(parent.is_backface_hidden_);

    nearest_scroll_translation_ = node.ScrollNode() ? &node : parent.nearest_scroll_translation_;

    nearest_directly_composited_ancestor_ = node.HasDirectCompositingReasons() ? &node : parent.nearest_directly_composited_ancestor_;
