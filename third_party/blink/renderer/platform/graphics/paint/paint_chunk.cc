...
template <typename T> bool PointerValueEquals(const std::unique_ptr<T>& a, const std::unique_ptr<T>& b)
{
    if (!a && !b) {
        return true;
    }
...
{
    return size() == other.size() && id == other.id && properties == other.properties && bounds == other.bounds && drawable_bounds == other.drawable_bounds
        && raster_effect_outset == other.raster_effect_outset && PointerValueEquals(hit_test_data, other.hit_test_data)
        && PointerValueEquals(region_capture_data, other.region_capture_data) && effectively_invisible == other.effectively_invisible;
...
{
    StringBuilder sb;
    sb.AppendFormat("PaintChunk(begin=%u, end=%u, id=%s cacheable=%d props=(%s) bounds=%s "
                    "rect_known_to_be_opaque=%s effectively_invisible=%d drawscontent=%d",
        c.begin_index, c.end_index, id_string.Utf8().c_str(), c.is_cacheable, c.properties.ToString().Utf8().c_str(), c.bounds.ToString().c_str(),
        c.rect_known_to_be_opaque.ToString().c_str(), c.effectively_invisible, c.DrawsContent());
    if (c.hit_test_data) {
