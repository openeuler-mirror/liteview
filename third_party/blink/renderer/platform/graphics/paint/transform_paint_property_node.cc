...
TransformPaintPropertyNode::TransformAndOrigin::TransformAndOrigin(const AffineTransform& transform)
{
    if (transform.IsIdentityOrTranslation()) {
        translation_2d_ = gfx::Vector2dF(transform.E(), transform.F());
    } else {
        matrix_and_origin_ = std::make_unique<MatrixAndOrigin>(TransformationMatrix(transform), gfx::Point3F());
    }
}

TransformationMatrix TransformPaintPropertyNode::TransformAndOrigin::SlowMatrix() const
{
    return matrix_and_origin_ ? matrix_and_origin_->matrix : TransformationMatrix::MakeTranslation(translation_2d_.x(), translation_2d_.y());
}
...
PaintPropertyChangeType TransformPaintPropertyNode::State::ComputeTransformChange(const TransformAndOrigin& other, const AnimationState& animation_state) const
{
    ...
    if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled() && direct_compositing_reasons & CompositingReason::kStickyPosition) {
    ...
    if (matrix_changed && !transform_and_origin.ChangePreserves2dAxisAlignment(other)) {
...
PaintPropertyChangeType TransformPaintPropertyNode::State::ComputeChange(const State& other, const AnimationState& animation_state) const
{
    ...
    DCHECK_EQ(flags.is_frame_paint_offset_translation, other.flags.is_frame_paint_offset_translation);

    // Changes other than compositing reason and the transform are not simple.
    if (flags.flattens_inherited_transform != other.flags.flattens_inherited_transform || flags.in_subtree_of_page_scale != other.flags.in_subtree_of_page_scale
        || flags.animation_is_axis_aligned != other.flags.animation_is_axis_aligned
        || flags.delegates_to_parent_for_backface != other.flags.delegates_to_parent_for_backface
        || flags.is_frame_paint_offset_translation != other.flags.is_frame_paint_offset_translation || flags.is_for_svg_child != other.flags.is_for_svg_child
        || backface_visibility != other.backface_visibility || rendering_context_id != other.rendering_context_id
        ...
        || !base::ValuesEquivalent(anchor_scroll_containers_data, other.anchor_scroll_containers_data)
...
const TransformPaintPropertyNode& TransformPaintPropertyNode::Root()
{
    DEFINE_STATIC_REF(TransformPaintPropertyNode, root,
        base::AdoptRef(new TransformPaintPropertyNode(nullptr,
            State { gfx::Vector2dF(), &ScrollPaintPropertyNode::Root(), nullptr,
                State::Flags { false /* flattens_inherited_transform */, false /* in_subtree_of_page_scale */ } })));
    return *root;
}
