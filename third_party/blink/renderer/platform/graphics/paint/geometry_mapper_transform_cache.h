...
public:
    ...
    {
        DCHECK(screen_transform_updated_);
        return LIKELY(!screen_transform_) || screen_transform_->projection_from_screen_is_valid;
    }
    ...
    {
        DCHECK(screen_transform_updated_);
        return UNLIKELY(screen_transform_) ? screen_transform_->has_animation : has_animation_to_plane_root();
    }
    ...
    {
        return UNLIKELY(plane_root_transform_) ? plane_root_transform_->plane_root : root_of_2d_translation();
    }
    bool has_animation_to_plane_root() const
    {
        return UNLIKELY(plane_root_transform_) && plane_root_transform_->has_animation;
    }
...
private:
    ...
    const TransformPaintPropertyNode* nearest_directly_composited_ancestor_ = nullptr;
