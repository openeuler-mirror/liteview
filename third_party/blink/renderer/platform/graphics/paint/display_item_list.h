...
private:
    ...
    template <typename BaseIterator, typename ItemType> class IteratorWrapper : public std::iterator<std::forward_iterator_tag, ItemType> {
        DISALLOW_NEW();
...
public:
    ...
    static std::unique_ptr<JSONArray> DisplayItemsAsJSON(
        const PaintArtifact&, wtf_size_t first_item_index, const Range<const_iterator>& display_items, JsonFlags);
#endif // DCHECK_IS_ON()
