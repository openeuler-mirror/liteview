...
// scrolling.
void ExpandFixedVisualRectInScroller(const TransformPaintPropertyNode& scroll_translation, gfx::RectF& rect)
{
    ...
    // container rect size).
    gfx::SizeF expansion(scroll_translation.ScrollNode()->ContentsRect().size() - scroll_translation.ScrollNode()->ContainerRect().size());
...
// max scroll offsets.
void ExpandFixedBoundsInScroller(const TransformPaintPropertyNode* local, const TransformPaintPropertyNode* ancestor, FloatClipRect& rect_to_map)
{
...
GeometryMapper::Translation2DOrMatrix GeometryMapper::SourceToDestinationProjection(
    const TransformPaintPropertyNode& source, const TransformPaintPropertyNode& destination)
{
...
GeometryMapper::Translation2DOrMatrix GeometryMapper::SourceToDestinationProjectionInternal(
    const TransformPaintPropertyNode& source, const TransformPaintPropertyNode& destination, ExtraProjectionResult& extra_result, bool& success)
{
    ...
    if (source_cache.root_of_2d_translation() == destination_cache.root_of_2d_translation()) {
        // We always use full matrix for animating transforms.
        return Translation2DOrMatrix(source_cache.to_2d_translation_root() - destination_cache.to_2d_translation_root());
    }
...
bool GeometryMapper::LocalToAncestorVisualRect(const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state, FloatClipRect& mapping_rect,
    OverlayScrollbarClipBehavior clip_behavior, InclusiveIntersectOrNot inclusive_behavior)
{
    return LocalToAncestorVisualRectInternal<ForCompositingOverlap::kNo>(local_state, ancestor_state, mapping_rect, clip_behavior, inclusive_behavior);
}
...
bool GeometryMapper::LocalToAncestorVisualRectInternal(const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state,
    FloatClipRect& rect_to_map, OverlayScrollbarClipBehavior clip_behavior, InclusiveIntersectOrNot inclusive_behavior)
{
    ...
    if (&local_state.Effect() != &ancestor_state.Effect() && &local_state.Clip() != &ancestor_state.Clip()) {
        return SlowLocalToAncestorVisualRectWithEffects<for_compositing_overlap>(local_state, ancestor_state, rect_to_map, clip_behavior, inclusive_behavior);
    }

    ExtraProjectionResult extra_result;
    bool success = false;
    const auto& translation_2d_or_matrix = SourceToDestinationProjectionInternal(local_state.Transform(), ancestor_state.Transform(), extra_result, success);
    if (!success) {
    ...
    if (for_compositing_overlap == ForCompositingOverlap::kYes && (extra_result.has_animation || extra_result.has_sticky)) {
        ...
        if (for_compositing_overlap == ForCompositingOverlap::kYes && !RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled() && extra_result.has_fixed) {
            ExpandFixedBoundsInScroller(&local_state.Transform(), &ancestor_state.Transform(), rect_to_map);
    ...
    FloatClipRect clip_rect = LocalToAncestorClipRectInternal<for_compositing_overlap>(
        local_state.Clip(), ancestor_state.Clip(), ancestor_state.Transform(), clip_behavior, inclusive_behavior);
...
template <GeometryMapper::ForCompositingOverlap for_compositing_overlap>
bool GeometryMapper::SlowLocalToAncestorVisualRectWithEffects(const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state,
    FloatClipRect& rect_to_map, OverlayScrollbarClipBehavior clip_behavior, InclusiveIntersectOrNot inclusive_behavior)
{
    ...
    while (filter_clip != ancestor_filter_clip) {
        ...
        if (for_compositing_overlap == ForCompositingOverlap::kYes && filter->HasActiveFilterAnimation()) {
            ...
            bool intersects = LocalToAncestorVisualRectInternal<for_compositing_overlap>(last_state, new_state, rect_to_map, clip_behavior, inclusive_behavior);
            if (!intersects) {
                rect_to_map = FloatClipRect(gfx::RectF());
                return false;
            }
    ...
    return LocalToAncestorVisualRectInternal<for_compositing_overlap>(last_state, ancestor_state, rect_to_map, clip_behavior, inclusive_behavior);
}
    ...
    const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state, OverlayScrollbarClipBehavior clip_behavior)
{
    ...
    auto result = LocalToAncestorClipRectInternal<ForCompositingOverlap::kNo>(
        local_clip, ancestor_clip, ancestor_state.Transform(), clip_behavior, kNonInclusiveIntersect);
...
static FloatClipRect GetClipRect(const ClipPaintPropertyNode& clip_node, OverlayScrollbarClipBehavior clip_behavior)
{
    ...
    FloatClipRect clip_rect = UNLIKELY(clip_behavior == kExcludeOverlayScrollbarSizeForHitTesting) ? clip_node.LayoutClipRectExcludingOverlayScrollbars()
...
FloatClipRect GeometryMapper::LocalToAncestorClipRectInternal(const ClipPaintPropertyNode& descendant_clip, const ClipPaintPropertyNode& ancestor_clip,
    const TransformPaintPropertyNode& ancestor_transform, OverlayScrollbarClipBehavior clip_behavior, InclusiveIntersectOrNot inclusive_behavior)
{
    ...
    for (auto* const node : base::Reversed(intermediate_nodes)) {
        ...
        const auto& translation_2d_or_matrix
            = SourceToDestinationProjectionInternal(node->LocalTransformSpace().Unalias(), ancestor_transform, extra_result, success);
        if (!success)
        ...
        if (inclusive_behavior == kInclusiveIntersect) {
            ...
            node->GetClipCache().SetCachedClip(
                GeometryMapperClipCache::ClipCacheEntry { clip_and_transform, clip, extra_result.has_animation, extra_result.has_sticky });
        }
    }
    // Clips that are inclusive intersected or expanded for animation are not
    // cached at present.
    DCHECK(inclusive_behavior == kInclusiveIntersect || for_compositing_overlap == ForCompositingOverlap::kYes
    ...
    const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2)
{
    ...
    PropertyTreeState common_ancestor(state1.Transform().LowestCommonAncestor(state2.Transform()).Unalias(),
        state1.Clip().LowestCommonAncestor(state2.Clip()).Unalias(), state1.Effect().LowestCommonAncestor(state2.Effect()).Unalias());
    // Move the common clip up if some effect nodes have OutputClip escaping the
    // common clip.
    if (const auto* clip_a = HighestOutputClipBetween(common_ancestor.Effect(), state1.Effect())) {
        common_ancestor.SetClip(clip_a->LowestCommonAncestor(common_ancestor.Clip()).Unalias());
    }
    if (const auto* clip_b = HighestOutputClipBetween(common_ancestor.Effect(), state2.Effect())) {
        common_ancestor.SetClip(clip_b->LowestCommonAncestor(common_ancestor.Clip()).Unalias());
    }
    ...
    const gfx::RectF& rect1, const PropertyTreeState& state1, const gfx::RectF& rect2, const PropertyTreeState& state2)
{
    ...
    const auto* fixed_scroll_translation1 = state1.Transform().ScrollTranslationForFixed();
    const auto* fixed_scroll_translation2 = state2.Transform().ScrollTranslationForFixed();
    if (fixed_scroll_translation1 == scroll_translation2 && &state1.Clip() == scroll_translation2->ScrollNode()->OverflowClipNode()) {
        ExpandFixedVisualRectInScroller(*fixed_scroll_translation1, new_rect1);
    } else if (scroll_translation1 == fixed_scroll_translation2 && &state2.Clip() == scroll_translation1->ScrollNode()->OverflowClipNode()) {
        ExpandFixedVisualRectInScroller(*fixed_scroll_translation2, new_rect2);
    } else {
        const auto& transform_lca = state1.Transform().LowestCommonAncestor(state2.Transform()).Unalias();
        const auto& scroll_translation_lca = transform_lca.NearestScrollTranslationNode();
        ...
        auto adjust_rect_and_state = [&scroll_translation_lca, &between_fixed_and_non_fixed](const TransformPaintPropertyNode* scroll_translation,
                                         const TransformPaintPropertyNode* other_fixed_scroll_translation, gfx::RectF& rect, PropertyTreeState& state) {
            if (scroll_translation == &scroll_translation_lca)
                return;

            auto* parent = scroll_translation->UnaliasedParent();
            DCHECK(parent);
            for (auto* next = &parent->NearestScrollTranslationNode(); next != &scroll_translation_lca; next = &parent->NearestScrollTranslationNode()) {
        ...
        adjust_rect_and_state(scroll_translation1, fixed_scroll_translation2, new_rect1, new_state1);
        if (between_fixed_and_non_fixed) {
            ExpandFixedVisualRectInScroller(*fixed_scroll_translation2, new_rect2);
        } else {
            adjust_rect_and_state(scroll_translation2, fixed_scroll_translation1, new_rect2, new_state2);
            if (between_fixed_and_non_fixed)
                ExpandFixedVisualRectInScroller(*fixed_scroll_translation1, new_rect1);
        }
    }

    return MightOverlapForCompositingInternal(new_rect1, new_state1, new_rect2, new_state2);
}
...
const ClipPaintPropertyNode* GeometryMapper::HighestOutputClipBetween(const EffectPaintPropertyNode& ancestor, const EffectPaintPropertyNode& descendant)
{
    const ClipPaintPropertyNode* result = nullptr;
    for (const auto* effect = &descendant; effect != &ancestor; effect = effect->UnaliasedParent()) {
    ...
    const gfx::RectF& local_rect, const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state)
{
    ...
    GeometryMapper::LocalToAncestorVisualRectInternal<ForCompositingOverlap::kYes>(
        local_state, ancestor_state, visual_rect, kIgnoreOverlayScrollbarSize, kNonInclusiveIntersect);
...
{
    return GeometryMapper::LocalToAncestorVisualRectInternal<ForCompositingOverlap::kNo>(
        local_state, ancestor_state, mapping_rect, kIgnoreOverlayScrollbarSize, kNonInclusiveIntersect);
}

bool GeometryMapper::LocalToAncestorVisualRectInternalForCompositingOverlapForTesting(
    const PropertyTreeState& local_state, const PropertyTreeState& ancestor_state, FloatClipRect& mapping_rect)
{
    return GeometryMapper::LocalToAncestorVisualRectInternal<ForCompositingOverlap::kYes>(
        local_state, ancestor_state, mapping_rect, kIgnoreOverlayScrollbarSize, kNonInclusiveIntersect);
}
