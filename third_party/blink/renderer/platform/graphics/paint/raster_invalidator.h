...
public:
    using RasterInvalidationFunction = base::RepeatingCallback<void(const gfx::Rect&)>;
    ...
    // paint artifact.
    void Generate(RasterInvalidationFunction, const PaintChunkSubset&, const gfx::Vector2dF& layer_offset, const gfx::Size& layer_bounds,
        const PropertyTreeState& layer_state, DisplayItemClientId layer_client_id = kInvalidDisplayItemClientId);
...
private:
    ...
    void GenerateRasterInvalidations(RasterInvalidationFunction, const PaintChunkSubset&, bool layer_offset_or_state_changed, bool layer_effect_changed,
    ...
    ALWAYS_INLINE void IncrementallyInvalidateChunk(
        RasterInvalidationFunction, const PaintChunkInfo& old_chunk_info, const PaintChunkInfo& new_chunk_info, DisplayItemClientId);
    ...
    void AddRasterInvalidation(
        RasterInvalidationFunction function, const gfx::Rect& rect, DisplayItemClientId client_id, PaintInvalidationReason reason, ClientIsOldOrNew old_or_new)
    {
        if (rect.IsEmpty())
            return;
        function.Run(rect);
    ...
    ALWAYS_INLINE PaintInvalidationReason ChunkPropertiesChanged(const PaintChunk& new_chunk, const PaintChunk& old_chunk, const PaintChunkInfo& new_chunk_info,
        const PaintChunkInfo& old_chunk_info, const PropertyTreeState& layer_state) const;
