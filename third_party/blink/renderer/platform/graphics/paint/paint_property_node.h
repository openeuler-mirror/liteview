...
PLATFORM_EXPORT const ClipPaintPropertyNodeOrAlias& LowestCommonAncestorInternal(const ClipPaintPropertyNodeOrAlias&, const ClipPaintPropertyNodeOrAlias&);
PLATFORM_EXPORT const EffectPaintPropertyNodeOrAlias& LowestCommonAncestorInternal(
    const EffectPaintPropertyNodeOrAlias&, const EffectPaintPropertyNodeOrAlias&);
PLATFORM_EXPORT const ScrollPaintPropertyNode& LowestCommonAncestorInternal(const ScrollPaintPropertyNode&, const ScrollPaintPropertyNode&);
PLATFORM_EXPORT const TransformPaintPropertyNodeOrAlias& LowestCommonAncestorInternal(
    const TransformPaintPropertyNodeOrAlias&, const TransformPaintPropertyNodeOrAlias&);

template <typename NodeTypeOrAlias, typename NodeType> struct PaintPropertyNodeRefCountedTraits {
...
class PaintPropertyNode : public RefCounted<NodeTypeOrAlias, PaintPropertyNodeRefCountedTraits<NodeTypeOrAlias, NodeType>> {
...
public:
    ...
    {
        for (auto* n = this; n && n->changed_sequence_number_ != sequence_number; n = n->Parent())
...
protected:
    explicit PaintPropertyNode(const NodeTypeOrAlias* parent)
        : changed_(parent ? PaintPropertyChangeType::kNodeAddedOrRemoved : PaintPropertyChangeType::kUnchanged)
        , parent_(parent)
    {
    }
    ...
    PaintPropertyChangeType SetParent(const NodeTypeOrAlias& parent)
    {
        ...
        parent_ = &parent;
        if (IsParentAlias()) {
            static_cast<NodeTypeOrAlias*>(this)->AddChanged(PaintPropertyChangeType::kChangedOnlyValues);
        } else {
            static_cast<NodeType*>(this)->AddChanged(PaintPropertyChangeType::kChangedOnlyValues);
        }
...
template <typename NodeType> class PropertyTreePrinter {
    STACK_ALLOCATED();
...
private:
    void BuildTreeString(StringBuilder& string_builder, const NodeType& node, unsigned indent)
    {
...
template <typename NodeTypeOrAlias, typename NodeType> String PaintPropertyNode<NodeTypeOrAlias, NodeType>::ToTreeString() const
{
    return PropertyTreePrinter<NodeTypeOrAlias>().PathAsString(*static_cast<const NodeTypeOrAlias*>(this));
}

#endif // DCHECK_IS_ON()

template <typename NodeTypeOrAlias, typename NodeType> std::ostream& operator<<(std::ostream& os, const PaintPropertyNode<NodeTypeOrAlias, NodeType>& node)
{
