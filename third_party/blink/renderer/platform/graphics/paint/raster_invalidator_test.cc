...
class RasterInvalidatorTest : public testing::Test, public PaintTestConfigurations {
public:
...
static gfx::Rect ChunkRectToLayer(const gfx::Rect& rect, const gfx::Vector2dF& layer_offset, const MapFunction& mapper = base::DoNothing())
{
    auto r = rect;
    mapper.Run(r);
...
static bool CheckChunkInvalidation(const RasterInvalidationInfo& info, const PaintChunkSubset& chunks, wtf_size_t index, PaintInvalidationReason reason,
    const gfx::Vector2dF& layer_offset, const absl::optional<gfx::Rect>& chunk_rect = absl::nullopt, const MapFunction& mapper = base::DoNothing())
{
...
{
    return CheckChunkInvalidation(arg, chunks, index, reason, layer_offset, absl::nullopt, mapper);
}
...
TEST_P(RasterInvalidatorTest, ImplicitFullLayerInvalidation)
{
    ...
    invalidator_.Generate(base::DoNothing(), chunks, kDefaultLayerOffset, kDefaultLayerBounds, DefaultPropertyTreeState());
...
TEST_P(RasterInvalidatorTest, LayerBounds)
{
    ...
    invalidator_.Generate(base::DoNothing(), chunks, new_layer_offset, kDefaultLayerBounds, DefaultPropertyTreeState());
    ...
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty),
            ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -new_layer_offset, base::DoNothing())));
    FinishCycle(chunks);
}
...
TEST_P(RasterInvalidatorTest, LayerOffsetChangeWithCachedSubsequence)
{
    ...
    invalidator_.Generate(base::DoNothing(), new_chunks, new_layer_offset, kDefaultLayerBounds, DefaultPropertyTreeState());
    // Change of layer origin causes change of chunk0's transform to layer.
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty),
            ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -new_layer_offset, base::DoNothing())));
...
TEST_P(RasterInvalidatorTest, LayerStateChangeWithCachedSubsequence)
{
    ...
    invalidator_.Generate(base::DoNothing(), new_chunks, kDefaultLayerOffset, kDefaultLayerBounds, new_layer_state);
    // Change of layer state causes change of chunk0's transform to layer.
    auto old_mapper = [](gfx::Rect& r) { r.Offset(100, 50); };
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -kDefaultLayerOffset, base::BindRepeating(old_mapper)),
...
TEST_P(RasterInvalidatorTest, ReorderChunks)
{
    ...
    invalidator_.Generate(base::DoNothing(), new_chunks, kDefaultLayerOffset, kDefaultLayerBounds, DefaultPropertyTreeState());
    // Invalidated new chunk 2's old (as chunks[{0, 1]) and new
    // (as new_chunks[{0, 2]) bounds.
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 1, PaintInvalidationReason::kChunkReordered),
            ChunkInvalidation(new_chunks, 2, PaintInvalidationReason::kChunkReordered)));
...
TEST_P(RasterInvalidatorTest, ReorderChunkSubsequences)
{
    ...
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 1, PaintInvalidationReason::kChunkReordered),
            ChunkInvalidation(new_chunks, 3, PaintInvalidationReason::kChunkReordered),
            ChunkInvalidation(new_chunks, 4, PaintInvalidationReason::kChunkReordered)));
    FinishCycle(new_chunks);
}
...
TEST_P(RasterInvalidatorTest, ClipPropertyChangeRounded)
{
    ...
    invalidator_.Generate(base::DoNothing(), chunks, kDefaultLayerOffset, kDefaultLayerBounds, layer_state);
    FinishCycle(chunks);
    ...
    invalidator_.SetTracksRasterInvalidations(true);
    invalidator_.Generate(base::DoNothing(), new_chunks1, kDefaultLayerOffset, kDefaultLayerBounds, layer_state);
...
TEST_P(RasterInvalidatorTest, ClipPropertyChangeWithOutsetForRasterEffects)
{
    ...
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -kDefaultLayerOffset, base::BindRepeating(mapper))));
    invalidator_.SetTracksRasterInvalidations(false);
    FinishCycle(chunks);
}
...
TEST_P(RasterInvalidatorTest, TransformPropertyChange)
{
    ...
    transform0->Update(*new_layer_transform, TransformPaintPropertyNode::State { transform0->Translation2D() });
    ...
    transform0->Update(layer_state.Transform(), TransformPaintPropertyNode::State { transform0->Translation2D() });
    ...
    transform0->Update(layer_state.Transform(), TransformPaintPropertyNode::State { transform0->Translation2D() + gfx::Vector2dF(20, 30) });
    transform1->Update(*transform0, TransformPaintPropertyNode::State { transform1->Translation2D() + gfx::Vector2dF(-20, -30) });

    invalidator_.Generate(base::DoNothing(), chunks, kDefaultLayerOffset, kDefaultLayerBounds, layer_state);
    auto mapper0 = [](gfx::Rect& r) { r.Offset(10, 20); };
    auto mapper1 = [](gfx::Rect& r) { r.Offset(30, 50); };
    EXPECT_THAT(TrackedRasterInvalidations(),
        ElementsAre(ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -kDefaultLayerOffset, base::BindRepeating(mapper0)),
            ChunkInvalidation(chunks, 0, PaintInvalidationReason::kPaintProperty, -kDefaultLayerOffset, base::BindRepeating(mapper1))));
    invalidator_.SetTracksRasterInvalidations(false);
    FinishCycle(chunks);
}
...
TEST_P(RasterInvalidatorTest, TransformPropertyTinyChange)
{
    ...
    chunk_transform->Update(layer_state.Transform(), TransformPaintPropertyNode::State { matrix_with_tiny_change(chunk_transform->SlowMatrix()) });
