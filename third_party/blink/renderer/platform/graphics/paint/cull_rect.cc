...
// and transform.
int LocalPixelDistanceToExpand(const TransformPaintPropertyNode& root_transform, const TransformPaintPropertyNode& local_transform)
{
...
bool CullRect::ApplyScrollTranslation(const TransformPaintPropertyNode& root_transform, const TransformPaintPropertyNode& scroll_translation)
{
    ...
    if (scroll->ContainerRect().width() >= contents_rect.width() && scroll->ContainerRect().height() >= contents_rect.height())
        return false;
...
bool CullRect::ApplyPaintProperties(
    const PropertyTreeState& root, const PropertyTreeState& source, const PropertyTreeState& destination, const absl::optional<CullRect>& old_cull_rect)
{
    ...
    if (!abnormal_hierarchy) {
        for (const auto* c = &destination.Clip(); c != &source.Clip(); c = c->UnaliasedParent()) {
            DCHECK(c);
            if (c == &root.Clip()) {
                abnormal_hierarchy = true;
                break;
    ...
    if (abnormal_hierarchy) {
        ...
        *this = Infinite();
        return root != destination && ApplyPaintProperties(root, root, destination, old_cull_rect);
    }
    ...
    for (const auto* scroll_translation : base::Reversed(scroll_translations)) {
        ...
        while (clip_it != clips.rend() && &(*clip_it)->LocalTransformSpace() != scroll_translation->Parent()) {
            clip_it++;
        }
        ...
        if (updated_last_clip) {
            if (!ApplyPaintPropertiesWithoutExpansion(PropertyTreeState(*last_transform, *last_clip, effect_root),
                    PropertyTreeState(*scroll_translation->UnaliasedParent(), *updated_last_clip, effect_root))) {
                return false;
            }
    ...
    if (last_transform != &destination.Transform() && destination.Transform().RequiresCullRectExpansion()) {
        ...
        int pixel_distance_to_expand = LocalPixelDistanceToExpand(root.Transform(), destination.Transform());
    ...
    if (expanded && old_cull_rect && !ChangedEnough(*old_cull_rect, expansion_bounds))
        rect_ = old_cull_rect->Rect();

    return expanded;
}

bool CullRect::ChangedEnough(const CullRect& old_cull_rect, const absl::optional<gfx::Rect>& expansion_bounds) const
{
    ...
    if (rect_.x() == expansion_bounds->x() && old_cull_rect.Rect().x() != expansion_bounds->x())
        return true;
    if (rect_.y() == expansion_bounds->y() && old_cull_rect.Rect().y() != expansion_bounds->y())
        return true;
    if (rect_.right() == expansion_bounds->right() && old_cull_rect.Rect().right() != expansion_bounds->right())
        return true;
    if (rect_.bottom() == expansion_bounds->bottom() && old_cull_rect.Rect().bottom() != expansion_bounds->bottom())
        return true;

    return false;
}

bool CullRect::HasScrolledEnough(const gfx::Vector2dF& delta, const TransformPaintPropertyNode& scroll_translation)
{
    if (!RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled())
        return !delta.IsZero();

    if (!scroll_translation.ScrollNode() || !scroll_translation.HasDirectCompositingReasons()) {
        return !delta.IsZero();
    }
    if (std::abs(delta.x()) < kChangedEnoughMinimumDistance && std::abs(delta.y()) < kChangedEnoughMinimumDistance) {
        return false;
    }
