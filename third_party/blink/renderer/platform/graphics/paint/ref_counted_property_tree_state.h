...
public:
    explicit RefCountedPropertyTreeStateOrAlias(const PropertyTreeStateOrAlias& property_tree_state)
    ...
    RefCountedPropertyTreeStateOrAlias& operator=(const PropertyTreeStateOrAlias& property_tree_state)
    {
    ...
    {
        return GetPropertyTreeState().ToString();
    }
#if DCHECK_IS_ON()
    // Dumps the tree from this state up to the root as a string.
    String ToTreeString() const
    {
        return GetPropertyTreeState().ToTreeString();
    }
#endif
...
class PLATFORM_EXPORT RefCountedPropertyTreeState : public RefCountedPropertyTreeStateOrAlias {
    USING_FAST_MALLOC(RefCountedPropertyTreeState);

public:
    explicit RefCountedPropertyTreeState(const PropertyTreeState& property_tree_state)
        : RefCountedPropertyTreeStateOrAlias(property_tree_state)
    {
    }

    RefCountedPropertyTreeState& operator=(const PropertyTreeState& property_tree_state)
    {
...
inline bool operator==(const RefCountedPropertyTreeStateOrAlias& a, const RefCountedPropertyTreeStateOrAlias& b)
{
    return &a.Transform() == &b.Transform() && &a.Clip() == &b.Clip() && &a.Effect() == &b.Effect();
}
...
inline bool operator==(const RefCountedPropertyTreeStateOrAlias& a, const PropertyTreeStateOrAlias& b)
{
    return &a.Transform() == &b.Transform() && &a.Clip() == &b.Clip() && &a.Effect() == &b.Effect();
}
...
inline bool operator==(const PropertyTreeStateOrAlias& a, const RefCountedPropertyTreeStateOrAlias& b)
{
    return b == a;
}
