...
public:
    ...
    using RestoreMatrixClipStackCb = base::RepeatingCallback<void(cc::PaintCanvas*)>;
    ...
    static std::unique_ptr<CanvasResourceProvider> CreateBitmapProvider(
        const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize initialize_provider);

    static std::unique_ptr<CanvasResourceProvider> CreateSharedBitmapProvider(
        const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize initialize_provider, base::WeakPtr<CanvasResourceDispatcher>);
    ...
    static std::unique_ptr<CanvasResourceProvider> CreateSharedImageProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        ShouldInitialize initialize_provider, base::WeakPtr<WebGraphicsContext3DProviderWrapper>, RasterMode raster_mode, bool is_origin_top_left,
        uint32_t shared_image_usage_flags);

    static std::unique_ptr<CanvasResourceProvider> CreateWebGPUImageProvider(
        const SkImageInfo& info, bool is_origin_top_left, uint32_t shared_image_usage_flags = 0);
    ...
    virtual scoped_refptr<StaticBitmapImage> Snapshot(const ImageOrientation& = ImageOrientationEnum::kDefault) = 0;
    ...
    virtual gpu::Mailbox GetBackingMailboxForOverwrite(MailboxSyncMode sync_mode)
    {
        NOTREACHED();
        return gpu::Mailbox();
    }
    virtual GLenum GetBackingTextureTarget() const
    {
    ...
    size_t TotalPinnedImageBytes() const
    {
        return total_pinned_image_bytes_;
    }
...
protected:
    ...
    CanvasResourceProvider(const ResourceProviderType&, const SkImageInfo&, cc::PaintFlags::FilterQuality, bool is_origin_top_left,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper>, base::WeakPtr<CanvasResourceDispatcher>);
    ...
    void RasterRecordOOP(sk_sp<cc::PaintRecord> last_recording, bool needs_clear, gpu::Mailbox mailbox, bool preserve_recording);
...
ALWAYS_INLINE void CanvasResourceProvider::FlushIfRecordingLimitExceeded()
{
    ...
    if (IsPrinting() && clear_frame_)
        return;
    if (recorder_ && ((TotalOpBytesUsed() > kMaxRecordedOpBytes) || total_pinned_image_bytes_ > max_pinned_image_bytes_)) {
        FlushCanvas();
    }
}
