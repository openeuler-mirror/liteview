...
public:
    MockCanvasResourceDispatcher(unsigned placeholder_id)
        : CanvasResourceDispatcher(/*client=*/nullptr, base::ThreadTaskRunnerHandle::Get(), kClientId, kSinkId, placeholder_id,
...
void OffscreenCanvasPlaceholderTest::SetUp()
{
    ...
    resource_provider_ = CanvasResourceProvider::CreateSharedBitmapProvider(SkImageInfo::MakeN32Premul(kWidth, kHeight), cc::PaintFlags::FilterQuality::kLow,
        CanvasResourceProvider::ShouldInitialize::kCallClear, dispatcher_->GetWeakPtr());
}
...
{
    scoped_refptr<CanvasResource> resource = resource_provider_->ProduceCanvasResource();
