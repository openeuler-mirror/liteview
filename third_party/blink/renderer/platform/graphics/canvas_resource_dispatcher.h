...
public:
    ...
    CanvasResourceDispatcher(CanvasResourceDispatcherClient*, scoped_refptr<base::SingleThreadTaskRunner> agent_group_scheduler_compositor_task_runner,
        uint32_t client_id, uint32_t sink_id, int placeholder_canvas_id, const gfx::Size&);
    ...
    void OnBeginFrame(const viz::BeginFrameArgs&, const WTF::HashMap<uint32_t, viz::FrameTimingDetails>&) final;
    ...
    void DidAllocateSharedBitmap(base::ReadOnlySharedMemoryRegion region, const gpu::Mailbox& id);
...
private:
    ...
    using ResourceMap = HashMap<viz::ResourceId, std::unique_ptr<FrameResource>, ResourceIdHash>;
