...
void DrawIcon(cc::PaintCanvas* canvas, const cc::PaintFlags& flags, float x, float y, const SkSamplingOptions& sampling, float scale_factor)
{
    // Note that |icon_image| will be a 0x0 image when running
    // blink_platform_unittests.
    DEFINE_STATIC_REF(Image, icon_image, (Image::LoadPlatformResource(IDR_PLACEHOLDER_ICON)));
    ...
    canvas->drawImageRect(icon_image->PaintImageForCurrentFrame(), SkRect::MakeWH(icon_image->width(), icon_image->height()),
        SkRect::MakeXYWH(x, y, scale_factor * kIconWidth, scale_factor * kIconHeight), sampling, &flags, SkCanvas::kFast_SrcRectConstraint);
}

void DrawCenteredIcon(cc::PaintCanvas* canvas, const cc::PaintFlags& flags, const gfx::RectF& dest_rect, const SkSamplingOptions& sampling, float scale_factor)
{
    DrawIcon(canvas, flags, dest_rect.x() + (dest_rect.width() - scale_factor * kIconWidth) / 2.0f,
        dest_rect.y() + (dest_rect.height() - scale_factor * kIconHeight) / 2.0f, sampling, scale_factor);
}
...
String FormatOriginalResourceSizeBytes(int64_t bytes)
{
    ...
    static constexpr int kUnitsResourceIds[] = { IDS_UNITS_KIBIBYTES, IDS_UNITS_MEBIBYTES, IDS_UNITS_GIBIBYTES, IDS_UNITS_TEBIBYTES, IDS_UNITS_PEBIBYTES };
    ...
    for (; units < kUnitsResourceIds + (std::size(kUnitsResourceIds) - 1) && bytes >= denomenator * 1000; ++units, denomenator *= 1024) {
    }
    ...
    if (bytes < denomenator) {
        ...
        // fractional digit.
        numeric_string = String::Number(static_cast<double>(bytes) / static_cast<double>(denomenator), 2);
    } else {
    ...
    return locale.QueryString(*units, locale.ConvertToLocalizedNumber(numeric_string));
}
...
public:
    static scoped_refptr<SharedFont> GetOrCreateInstance(float scale_factor)
    {
        ...
        scoped_refptr<SharedFont> shared_font = base::MakeRefCounted<SharedFont>(scale_factor);
...
// static
PlaceholderImage::SharedFont* PlaceholderImage::SharedFont::g_instance_ = nullptr;

PlaceholderImage::PlaceholderImage(ImageObserver* observer, const gfx::Size& size, int64_t original_resource_size)
    : Image(observer)
    , size_(size)
    , text_(original_resource_size <= 0 ? String() : FormatOriginalResourceSizeBytes(original_resource_size))
...
{
    auto builder = CreatePaintImageBuilder().set_completion_state(PaintImage::CompletionState::DONE);

    const gfx::Rect dest_rect(size_);
    if (paint_record_for_current_frame_) {
        return builder.set_paint_record(paint_record_for_current_frame_, dest_rect, paint_record_content_id_).TakePaintImage();
    }

    PaintRecorder paint_recorder;
    Draw(paint_recorder.beginRecording(gfx::RectToSkRect(dest_rect)), cc::PaintFlags(), gfx::RectF(dest_rect), gfx::RectF(dest_rect), ImageDrawOptions());
...
void PlaceholderImage::SetIconAndTextScaleFactor(float icon_and_text_scale_factor)
{
...
void PlaceholderImage::Draw(
    cc::PaintCanvas* canvas, const cc::PaintFlags& base_flags, const gfx::RectF& dest_rect, const gfx::RectF& src_rect, const ImageDrawOptions& draw_options)
{
    if (!src_rect.Intersects(gfx::RectF(0.0f, 0.0f, static_cast<float>(size_.width()), static_cast<float>(size_.height())))) {
        return;
    }
    ...
    if (dest_rect.width() < icon_and_text_scale_factor_ * (kIconWidth + 2 * kFeaturePaddingX)
        || dest_rect.height() < icon_and_text_scale_factor_ * (kIconHeight + 2 * kIconPaddingY)) {
        return;
    }

    if (text_.empty()) {
        DrawCenteredIcon(canvas, base_flags, dest_rect, draw_options.sampling_options, icon_and_text_scale_factor_);
        return;
    }
    ...
    const float icon_and_text_width
        = cached_text_width_.value() + icon_and_text_scale_factor_ * (kIconWidth + 2 * kFeaturePaddingX + kPaddingBetweenIconAndText);
    ...
    const float feature_x = dest_rect.x() + (dest_rect.width() - icon_and_text_width) / 2.0f;
    const float feature_y = dest_rect.y() + (dest_rect.height() - icon_and_text_scale_factor_ * (kIconHeight + 2 * kIconPaddingY)) / 2.0f;

    float icon_x, text_x;
    if (Locale::DefaultLocale().IsRTL()) {
        icon_x = feature_x + cached_text_width_.value() + icon_and_text_scale_factor_ * (kFeaturePaddingX + kPaddingBetweenIconAndText);
        ...
        text_x = feature_x + icon_and_text_scale_factor_ * (kFeaturePaddingX + kIconWidth + kPaddingBetweenIconAndText);
    }

    DrawIcon(canvas, base_flags, icon_x, feature_y + icon_and_text_scale_factor_ * kIconPaddingY, draw_options.sampling_options, icon_and_text_scale_factor_);

    flags.setColor(SkColorSetARGB(0xAB, 0, 0, 0));
    shared_font_->font().DrawBidiText(canvas, TextRunPaintInfo(TextRun(text_)),
        gfx::PointF(text_x, feature_y + icon_and_text_scale_factor_ * (kTextPaddingY + kFontSize)), Font::kUseFallbackIfFontNotReady, 1.0f, flags);
}

void PlaceholderImage::DrawPattern(GraphicsContext& context, const cc::PaintFlags& base_flags, const gfx::RectF& dest_rect, const ImageTilingInfo& tiling_info,
    const ImageDrawOptions& draw_options)
{
    ...
    Draw(context.Canvas(), base_flags, dest_rect, tiling_info.image_rect, draw_options);
}
...
Image::SizeAvailability PlaceholderImage::SetData(scoped_refptr<SharedBuffer>, bool)
{
