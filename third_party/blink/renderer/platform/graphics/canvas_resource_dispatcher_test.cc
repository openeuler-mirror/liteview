...
public:
    MockCanvasResourceDispatcher()
        : CanvasResourceDispatcher(nullptr /* client */, base::ThreadTaskRunnerHandle::Get(), kClientId, kSinkId, 0 /* placeholder_canvas_id* */,
...
public:
    ...
    {
        scoped_refptr<CanvasResource> canvas_resource = resource_provider_->ProduceCanvasResource();
...
protected:
    ...
    void CreateCanvasResourceDispatcher()
    {
        ...
        resource_provider_ = CanvasResourceProvider::CreateSharedBitmapProvider(SkImageInfo::MakeN32Premul(kWidth, kHeight),
            cc::PaintFlags::FilterQuality::kLow, CanvasResourceProvider::ShouldInitialize::kCallClear, dispatcher_->GetWeakPtr());
    }
...
TEST_P(CanvasResourceDispatcherTest, DispatchFrame)
{
    ...
    auto canvas_resource = CanvasResourceSharedBitmap::Create(
        SkImageInfo::MakeN32Premul(GetSize().width(), GetSize().height()), nullptr /* provider */, cc::PaintFlags::FilterQuality::kLow);
    ...
    EXPECT_CALL(mock_embedded_frame_sink_provider.mock_compositor_frame_sink(), SubmitCompositorFrame_(_))
        .WillOnce(::testing::WithArg<0>(::testing::Invoke([context_alpha](const viz::CompositorFrame* frame) {
            ...
            EXPECT_THAT(texture_quad->vertex_opacity, ::testing::ElementsAre(1.f, 1.f, 1.f, 1.f));
