...
public:
    ...
    void SubmitCompositorFrame(
        const viz::LocalSurfaceId& id, viz::CompositorFrame frame, absl::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time) override
    {
        last_submitted_compositor_frame_ = std::move(frame);
        DoSubmitCompositorFrame(id, &last_submitted_compositor_frame_);
    }
    void SubmitCompositorFrameSync(const viz::LocalSurfaceId& id, viz::CompositorFrame frame, absl::optional<viz::HitTestRegionList> hit_test_region_list,
    ...
    MOCK_METHOD2(DidAllocateSharedBitmap, void(base::ReadOnlySharedMemoryRegion region, const gpu::Mailbox& id));
...
public:
    ...
    {
        blink::VideoFrameResourceProvider::Initialize(context_provider, shared_bitmap_reporter);
    }
    ...
    MOCK_METHOD2(Initialize, void(viz::RasterContextProvider*, viz::SharedBitmapReporter*));
...
public:
    ...
    {
        resource_provider_ = new StrictMock<MockVideoFrameResourceProvider>(context_provider_.get(), nullptr);
        submitter_
            = std::make_unique<VideoFrameSubmitter>(base::DoNothing(), reporting_cb, base::WrapUnique<MockVideoFrameResourceProvider>(resource_provider_));
        ...
        auto surface_id = viz::SurfaceId(viz::FrameSinkId(1, 1), viz::LocalSurfaceId(11, base::UnguessableToken::Deserialize(0x111111, 0)));
    ...
    {
        submitter_->OnReceivedContextProvider(use_gpu_compositing, std::move(context_provider));
    }
...
TEST_F(VideoFrameSubmitterTest, StopUsingProviderSubmitsFrameAndStopsRendering)
{
    EXPECT_CALL(*sink_, SetNeedsBeginFrame(true));
    submitter_->StartRendering();
    task_environment_.RunUntilIdle();
...
TEST_F(VideoFrameSubmitterTest, RecreateCompositorFrameSinkAfterContextLostSoftwareCompositing)
{
    MockEmbeddedFrameSinkProvider mock_embedded_frame_sink_provider;
    mojo::ReceiverSet<mojom::blink::EmbeddedFrameSinkProvider> embedded_frame_sink_provider_receivers;
    auto override = mock_embedded_frame_sink_provider.CreateScopedOverrideMojoInterface(embedded_frame_sink_provider_receivers);
...
TEST_F(VideoFrameSubmitterTest, PreferredInterval)
{
    ...
    EXPECT_EQ(sink_->last_submitted_compositor_frame().metadata.preferred_frame_interval, video_frame_provider_->preferred_interval);
}
