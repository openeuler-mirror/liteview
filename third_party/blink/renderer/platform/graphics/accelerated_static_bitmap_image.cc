...
// static
scoped_refptr<AcceleratedStaticBitmapImage> AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token,
...
{
    return base::AdoptRef(new AcceleratedStaticBitmapImage(mailbox, sync_token, shared_image_texture_id, sk_image_info, texture_target, is_origin_top_left,
        supports_display_compositing, is_overlay_candidate, ImageOrientationEnum::kDefault, std::move(context_provider_wrapper), context_thread_ref,
        std::move(context_task_runner), std::move(release_callback)));
}

AcceleratedStaticBitmapImage::AcceleratedStaticBitmapImage(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token, GLuint shared_image_texture_id,
    const SkImageInfo& sk_image_info, GLenum texture_target, bool is_origin_top_left, bool supports_display_compositing, bool is_overlay_candidate,
    const ImageOrientation& orientation, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    ...
    bool unpack_premultiply_alpha, bool unpack_flip_y, const gfx::Point& dest_point, const gfx::Rect& source_sub_rectangle)
{
    ...
    GLuint source_texture_id = dest_gl->CreateAndTexStorage2DSharedImageCHROMIUM(mailbox_.name);
    dest_gl->BeginSharedImageAccessDirectCHROMIUM(source_texture_id, GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);
    dest_gl->CopySubTextureCHROMIUM(source_texture_id, 0, dest_target, dest_texture_id, dest_level, dest_point.x(), dest_point.y(), source_sub_rectangle.x(),
        source_sub_rectangle.y(), source_sub_rectangle.width(), source_sub_rectangle.height(), unpack_flip_y,
...
bool AcceleratedStaticBitmapImage::CopyToResourceProvider(CanvasResourceProvider* resource_provider)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    const auto& dst_mailbox = resource_provider->GetBackingMailboxForOverwrite(MailboxSyncMode::kOrderingBarrier);
    if (dst_mailbox.IsZero())
        return false;

    const GLenum dst_target = resource_provider->GetBackingTextureTarget();
    const bool unpack_flip_y = IsOriginTopLeft() != resource_provider->IsOriginTopLeft();
    ...
    ri->CopySubTexture(mailbox_, dst_mailbox, dst_target, 0, 0, 0, 0, Size().width(), Size().height(), unpack_flip_y, unpack_premultiply_alpha);
    // We need to update the texture holder's sync token to ensure that when this
    // mailbox is recycled or deleted, it is done after the copy operation above.
    gpu::SyncToken sync_token;
...
gpu::MailboxHolder AcceleratedStaticBitmapImage::GetMailboxHolder() const
{
    ...
    return gpu::MailboxHolder(mailbox_, mailbox_ref_->sync_token(), texture_target_);
}
...
scoped_refptr<StaticBitmapImage> AcceleratedStaticBitmapImage::ConvertToColorSpace(sk_sp<SkColorSpace> color_space, SkColorType color_type)
{
    SkImageInfo image_info = PaintImageForCurrentFrame().GetSkImageInfo();
    DCHECK(color_space);
    DCHECK(color_type == kRGBA_F16_SkColorType || color_type == kRGBA_8888_SkColorType || color_type == image_info.colorType());

    if (!ContextProviderWrapper())
        return nullptr;

    if (SkColorSpace::Equals(color_space.get(), image_info.colorSpace()) && color_type == image_info.colorType()) {
        return this;
    }
    image_info = image_info.makeColorSpace(color_space).makeColorType(color_type).makeWH(Size().width(), Size().height());

    auto usage_flags = ContextProviderWrapper()->ContextProvider()->SharedImageInterface()->UsageForMailbox(mailbox_);
    auto provider = CanvasResourceProvider::CreateSharedImageProvider(image_info, cc::PaintFlags::FilterQuality::kLow,
        CanvasResourceProvider::ShouldInitialize::kNo, ContextProviderWrapper(), RasterMode::kGPU, IsOriginTopLeft(), usage_flags);
    if (!provider) {
        return nullptr;
    }
    ...
    provider->Canvas()->drawImage(PaintImageForCurrentFrame(), 0, 0, SkSamplingOptions(), &paint);
