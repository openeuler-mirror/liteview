...
GpuMemoryBufferImageCopy::GpuMemoryBufferImageCopy(gpu::gles2::GLES2Interface* gl, gpu::SharedImageInterface* sii)
    : gl_(gl)
    , sii_(sii)
{
}
...
bool GpuMemoryBufferImageCopy::EnsureDestImage(const gfx::Size& size)
{
    ...
    if (dest_image_size_ != size || !gpu_memory_buffer_) {
        ...
        gpu_memory_buffer_
            = gpu_memory_buffer_manager->CreateGpuMemoryBuffer(size, gfx::BufferFormat::RGBA_8888, gfx::BufferUsage::SCANOUT, gpu::kNullSurfaceHandle, nullptr);
        if (!gpu_memory_buffer_)
            return false;

        dest_image_size_ = size;

        dest_mailbox_ = sii_->CreateSharedImage(gpu_memory_buffer_.get(), gpu_memory_buffer_manager, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin,
...
std::pair<gfx::GpuMemoryBuffer*, gpu::SyncToken> GpuMemoryBufferImageCopy::CopyImage(Image* image)
{
    if (!image)
        return {};
    ...
    GLuint dest_texture_id = gl_->CreateAndTexStorage2DSharedImageCHROMIUM(dest_mailbox_.name);
    gl_->BeginSharedImageAccessDirectCHROMIUM(dest_texture_id, GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
    ...
    GLuint source_texture_id = gl_->CreateAndTexStorage2DSharedImageCHROMIUM(source_mailbox_holder.mailbox.name);
    gl_->BeginSharedImageAccessDirectCHROMIUM(source_texture_id, GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);

    gl_->CopySubTextureCHROMIUM(source_texture_id, 0, GL_TEXTURE_2D, dest_texture_id, 0, 0, 0, 0, 0, size.width(), size.height(), false, false, false);
