...
static void ReleaseFrameResources(
    base::WeakPtr<CanvasResourceProvider> resource_provider, scoped_refptr<CanvasResource>&& resource, const gpu::SyncToken& sync_token, bool lost_resource)
{
    if (!resource)
        return;
...
bool CanvasResource::PrepareTransferableResource(
    viz::TransferableResource* out_resource, CanvasResource::ReleaseCallback* out_callback, MailboxSyncMode sync_mode)
{
    DCHECK(IsValid());
...
bool CanvasResource::PrepareAcceleratedTransferableResource(viz::TransferableResource* out_resource, MailboxSyncMode sync_mode)
{
    ...
    *out_resource = viz::TransferableResource::MakeGpu(mailbox, GLFilter(), TextureTarget(), GetSyncToken(), Size(), GetResourceFormat(), IsOverlayCandidate());
...
bool CanvasResource::PrepareUnacceleratedTransferableResource(viz::TransferableResource* out_resource)
{
    TRACE_EVENT0("blink", "CanvasResource::PrepareUnacceleratedTransferableResource");
    ...
    *out_resource = viz::TransferableResource::MakeSoftware(mailbox, Size(), viz::RGBA_8888);
...
{
    return filter_quality_ == cc::PaintFlags::FilterQuality::kNone ? GL_NEAREST : GL_LINEAR;
}
...
CanvasResourceSharedBitmap::CanvasResourceSharedBitmap(
    const SkImageInfo& info, base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality)
    ...
    , size_(info.width(), info.height())
{
    ...
    base::MappedReadOnlyRegion shm = viz::bitmap_allocation::AllocateSharedBitmap(Size(), viz::RGBA_8888);
...
scoped_refptr<StaticBitmapImage> CanvasResourceSharedBitmap::Bitmap()
{
    ...
    SkPixmap pixmap(image_info, shared_mapping_.memory(), image_info.minRowBytes());
    AddRef();
    sk_sp<SkImage> sk_image = SkImage::MakeFromRaster(
        pixmap, [](const void*, SkImage::ReleaseContext resource_to_unref) { static_cast<CanvasResourceSharedBitmap*>(resource_to_unref)->Release(); }, this);
...
scoped_refptr<CanvasResourceSharedBitmap> CanvasResourceSharedBitmap::Create(
    const SkImageInfo& info, base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality)
{
    auto resource = AdoptRef(new CanvasResourceSharedBitmap(info, std::move(provider), filter_quality));
...
const gpu::Mailbox& CanvasResourceSharedBitmap::GetOrCreateGpuMailbox(MailboxSyncMode sync_mode)
{
...
{
    SkImageInfo image_info = SkImageInfo::Make(SkISize::Make(Size().width(), Size().height()), GetSkColorInfo());
    bool read_pixels_successful = image->readPixels(image_info, shared_mapping_.memory(), image_info.minRowBytes(), 0, 0);
...
CanvasResourceSharedImage::CanvasResourceSharedImage(
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, const SkColorInfo& info)
...
CanvasResourceRasterSharedImage::CanvasResourceRasterSharedImage(const SkImageInfo& info,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceProvider> provider,
    cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left, bool is_accelerated, uint32_t shared_image_usage_flags)
    : CanvasResourceSharedImage(std::move(provider), filter_quality, info.colorInfo())
    ...
    is_overlay_candidate_(shared_image_usage_flags & (gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_WEBGPU))
    ,
#else
    is_overlay_candidate_(shared_image_usage_flags & gpu::SHARED_IMAGE_USAGE_SCANOUT)
    ,
#endif
    supports_display_compositing_(shared_image_usage_flags & gpu::SHARED_IMAGE_USAGE_DISPLAY_READ)
    , texture_target_(is_overlay_candidate_
              ? gpu::GetBufferTextureTarget(gfx::BufferUsage::SCANOUT, GetBufferFormat(), context_provider_wrapper_->ContextProvider()->GetCapabilities())
              : GL_TEXTURE_2D)
    , use_oop_rasterization_(is_accelerated && context_provider_wrapper_->ContextProvider()->GetCapabilities().supports_oop_raster)
{
    auto* gpu_memory_buffer_manager = Platform::Current()->GetGpuMemoryBufferManager();
    if (!is_accelerated_) {
        ...
        gpu_memory_buffer_ = gpu_memory_buffer_manager->CreateGpuMemoryBuffer(
            Size(), GetBufferFormat(), gfx::BufferUsage::SCANOUT_CPU_READ_WRITE, gpu::kNullSurfaceHandle, nullptr);
    ...
    if (use_oop_rasterization_) {
        ...
        // trials.
        shared_image_usage_flags = shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION
            | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT;
    } else {
        shared_image_usage_flags = shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT;
    }

    GrSurfaceOrigin surface_origin = is_origin_top_left_ ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin;
    ...
    if (gpu_memory_buffer_) {
        shared_image_mailbox = shared_image_interface->CreateSharedImage(
            gpu_memory_buffer_.get(), gpu_memory_buffer_manager, GetColorSpace(), surface_origin, surface_alpha_type, shared_image_usage_flags);
    } else {
        shared_image_mailbox = shared_image_interface->CreateSharedImage(
            GetResourceFormat(), Size(), GetColorSpace(), surface_origin, surface_alpha_type, shared_image_usage_flags, gpu::kNullSurfaceHandle);
    }
    ...
    owning_thread_data().texture_id_for_read_access = raster_interface->CreateAndConsumeForGpuRaster(shared_image_mailbox);
...
scoped_refptr<CanvasResourceRasterSharedImage> CanvasResourceRasterSharedImage::Create(const SkImageInfo& info,
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceProvider> provider,
    cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left, bool is_accelerated, uint32_t shared_image_usage_flags)
{
    TRACE_EVENT0("blink", "CanvasResourceRasterSharedImage::Create");
    auto resource = base::AdoptRef(new CanvasResourceRasterSharedImage(
        info, std::move(context_provider_wrapper), std::move(provider), filter_quality, is_origin_top_left, is_accelerated, shared_image_usage_flags));
    return resource->IsValid() ? resource : nullptr;
}
...
{
    RasterInterface()->BeginSharedImageAccessDirectCHROMIUM(GetTextureIdForReadAccess(), GL_SHARED_IMAGE_ACCESS_MODE_READ_CHROMIUM);
}

void CanvasResourceRasterSharedImage::EndReadAccess()
{
    RasterInterface()->EndSharedImageAccessDirectCHROMIUM(GetTextureIdForReadAccess());
}
...
void CanvasResourceRasterSharedImage::TearDown()
{
    ...
    if (ContextProviderWrapper()) {
        ...
        if (raster_interface && shared_image_interface) {
            ...
            shared_image_interface->DestroySharedImage(shared_image_sync_token, mailbox());
        }
        if (raster_interface) {
...
void CanvasResourceRasterSharedImage::OnBitmapImageDestroyed(
    scoped_refptr<CanvasResourceRasterSharedImage> resource, bool has_read_ref_on_texture, const gpu::SyncToken& sync_token, bool is_lost)
{
    ...
    ReleaseFrameResources(std::move(weak_provider), std::move(resource), sync_token, is_lost);
}
...
scoped_refptr<StaticBitmapImage> CanvasResourceRasterSharedImage::Bitmap()
{
    ...
    if (!is_accelerated_) {
        ...
        SkPixmap pixmap(CreateSkImageInfo(), gpu_memory_buffer_->memory(0), gpu_memory_buffer_->stride(0));
    ...
    auto release_callback = base::BindOnce(&OnBitmapImageDestroyed, scoped_refptr<CanvasResourceRasterSharedImage>(this), has_read_ref_on_texture);
    ...
    image = AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(mailbox(), GetSyncToken(), texture_id_for_image, image_info, texture_target_,
        is_origin_top_left_, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_, std::move(release_callback),
...
void CanvasResourceRasterSharedImage::CopyRenderingResultsToGpuMemoryBuffer(const sk_sp<SkImage>& image)
{
    DCHECK(!is_cross_thread());
    ...
    auto surface = SkSurface::MakeRasterDirect(CreateSkImageInfo(), gpu_memory_buffer_->memory(0), gpu_memory_buffer_->stride(0));
...
const gpu::Mailbox& CanvasResourceRasterSharedImage::GetOrCreateGpuMailbox(MailboxSyncMode sync_mode)
{
    if (!is_cross_thread()) {
...
const gpu::SyncToken CanvasResourceRasterSharedImage::GetSyncToken()
{
    ...
    if (owning_thread_data().mailbox_sync_mode == kVerifiedSyncToken && !owning_thread_data().sync_token.verified_flush()) {
...
base::WeakPtr<WebGraphicsContext3DProviderWrapper> CanvasResourceRasterSharedImage::ContextProviderWrapper() const
{
    DCHECK(!is_cross_thread());
    return context_provider_wrapper_;
}

void CanvasResourceRasterSharedImage::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, size_t bytes_per_pixel) const
{
    *(int*)1 = 1;
    //     if (!IsValid())
    //         return;
    //
    //     std::string dump_name = base::StringPrintf("canvas/ResourceProvider/CanvasResource/0x%" PRIXPTR, reinterpret_cast<uintptr_t>(this));
...
//==============================================================================
scoped_refptr<ExternalCanvasResource> ExternalCanvasResource::Create(const gpu::Mailbox& mailbox, viz::ReleaseCallback release_callback,
    gpu::SyncToken sync_token, const SkImageInfo& info, GLenum texture_target, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left, bool is_overlay_candidate)
{
    TRACE_EVENT0("blink", "ExternalCanvasResource::Create");
    auto resource = AdoptRef(new ExternalCanvasResource(mailbox, std::move(release_callback), sync_token, info, texture_target,
        std::move(context_provider_wrapper), std::move(provider), filter_quality, is_origin_top_left, is_overlay_candidate));
    return resource->IsValid() ? resource : nullptr;
}
...
scoped_refptr<StaticBitmapImage> ExternalCanvasResource::Bitmap()
{
    ...
    return AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(mailbox_, GetSyncToken(), /*shared_image_texture_id=*/0u, CreateSkImageInfo(), texture_target_,
        is_origin_top_left_, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_, std::move(release_callback),
...
const gpu::Mailbox& ExternalCanvasResource::GetOrCreateGpuMailbox(MailboxSyncMode sync_mode)
{
...
ExternalCanvasResource::ExternalCanvasResource(const gpu::Mailbox& mailbox, viz::ReleaseCallback out_callback, gpu::SyncToken sync_token,
    const SkImageInfo& info, GLenum texture_target, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    base::WeakPtr<CanvasResourceProvider> provider, cc::PaintFlags::FilterQuality filter_quality, bool is_origin_top_left, bool is_overlay_candidate)
    : CanvasResource(std::move(provider), filter_quality, info.colorInfo())
    , context_provider_wrapper_(std::move(context_provider_wrapper))
    , size_(info.width(), info.height())
...
scoped_refptr<StaticBitmapImage> CanvasResourceSwapChain::Bitmap()
{
    ...
    return AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(back_buffer_mailbox_, GetSyncToken(), shared_texture_id, image_info, GL_TEXTURE_2D,
        true /*is_origin_top_left*/, context_provider_wrapper_, owning_thread_ref_, owning_thread_task_runner_, std::move(release_callback),
...
const gpu::Mailbox& CanvasResourceSwapChain::GetOrCreateGpuMailbox(MailboxSyncMode sync_mode)
{
    DCHECK_EQ(sync_mode, kVerifiedSyncToken);
...
void CanvasResourceSwapChain::PresentSwapChain()
{
    ...
    raster_interface->CopySubTexture(front_buffer_mailbox_, back_buffer_mailbox_, GL_TEXTURE_2D, 0, 0, 0, 0, size_.width(), size_.height(),
...
CanvasResourceSwapChain::CanvasResourceSwapChain(const SkImageInfo& info, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper,
    ...
    , context_provider_wrapper_(std::move(context_provider_wrapper))
    , size_(info.width(), info.height())
    , use_oop_rasterization_(context_provider_wrapper_->ContextProvider()->GetCapabilities().supports_oop_raster)
{
    if (!context_provider_wrapper_)
        return;

    uint32_t usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_GLES2 | gpu::SHARED_IMAGE_USAGE_GLES2_FRAMEBUFFER_HINT
        | gpu::SHARED_IMAGE_USAGE_SCANOUT;

    if (use_oop_rasterization_) {
        usage = usage | gpu::SHARED_IMAGE_USAGE_RASTER | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
    }

    auto* sii = context_provider_wrapper_->ContextProvider()->SharedImageInterface();
    DCHECK(sii);
    gpu::SharedImageInterface::SwapChainMailboxes mailboxes
        = sii->CreateSwapChain(GetResourceFormat(), Size(), GetColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage);
