...
WebGPUTextureAlphaClearer::WebGPUTextureAlphaClearer(scoped_refptr<DawnControlClientHolder> dawn_control_client, WGPUDevice device, WGPUTextureFormat format)
    ...
    , format_(format)
{
    ...
    WGPUShaderModuleDescriptor shader_module_desc = { .nextInChain = &wgsl_desc.chain };
    WGPUShaderModule shader_module = procs.deviceCreateShaderModule(device_, &shader_module_desc);
    ...
    alpha_to_one_pipeline_ = procs.deviceCreateRenderPipeline(device_, &pipeline_desc);
...
bool WebGPUTextureAlphaClearer::IsCompatible(WGPUDevice device, WGPUTextureFormat format) const
{
...
void WebGPUTextureAlphaClearer::ClearAlpha(WGPUTexture texture)
{
    ...
    WGPUCommandEncoder command_encoder = procs.deviceCreateCommandEncoder(device_, &command_encoder_desc);
    ...
    WGPURenderPassEncoder pass = procs.commandEncoderBeginRenderPass(command_encoder, &render_pass_desc);
    ...
    WGPUCommandBuffer command_buffer = procs.commandEncoderFinish(command_encoder, nullptr);
