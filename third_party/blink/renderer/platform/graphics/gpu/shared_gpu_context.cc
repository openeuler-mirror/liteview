...
static void CreateContextProviderOnMainThread(bool only_if_gpu_compositing, bool* gpu_compositing_disabled,
    std::unique_ptr<WebGraphicsContext3DProviderWrapper>* wrapper, base::WaitableEvent* waitable_event)
{
    DCHECK(IsMainThread());
...
void SharedGpuContext::CreateContextProviderIfNeeded(bool only_if_gpu_compositing)
{
    ...
    if (context_provider_factory_) {
        ...
        auto context_provider = context_provider_factory_.Run(&is_gpu_compositing_disabled_);
        if (context_provider) {
        ...
        PostCrossThreadTask(*task_runner, FROM_HERE,
            CrossThreadBindOnce(&CreateContextProviderOnMainThread, only_if_gpu_compositing, CrossThreadUnretained(&is_gpu_compositing_disabled_),
                CrossThreadUnretained(&context_provider_wrapper_), CrossThreadUnretained(&waitable_event)));
        waitable_event.Wait();
        if (context_provider_wrapper_ && !context_provider_wrapper_->ContextProvider()->BindToCurrentThread())
            context_provider_wrapper_ = nullptr;
    }
}
