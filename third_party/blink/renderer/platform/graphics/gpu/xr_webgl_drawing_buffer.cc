...
XRWebGLDrawingBuffer::ColorBuffer::ColorBuffer(
    base::WeakPtr<XRWebGLDrawingBuffer> drawing_buffer, const gfx::Size& size, const gpu::Mailbox& mailbox, GLuint texture_id)
...
XRWebGLDrawingBuffer::ColorBuffer::~ColorBuffer()
{
    ...
    auto* sii = drawing_buffer->drawing_buffer_->ContextProvider()->SharedImageInterface();
...
bool XRWebGLDrawingBuffer::Initialize(const gfx::Size& size, bool use_multisampling)
{
    ...
    if (use_multisampling) {
        ...
        if (extensions_util->SupportsExtension("GL_EXT_multisampled_render_to_texture")) {
...
void XRWebGLDrawingBuffer::UseSharedBuffer(const gpu::MailboxHolder& buffer_mailbox_holder)
{
    gpu::gles2::GLES2Interface* gl = drawing_buffer_->ContextGL();
    ...
    DVLOG(3) << __func__ << ": mailbox=" << buffer_mailbox_holder.mailbox.ToDebugString() << ", SyncToken=" << buffer_mailbox_holder.sync_token.ToDebugString();
    ...
    shared_buffer_texture_id_ = gl->CreateAndTexStorage2DSharedImageCHROMIUM(buffer_mailbox_holder.mailbox.name);
...
scoped_refptr<XRWebGLDrawingBuffer::ColorBuffer> XRWebGLDrawingBuffer::CreateColorBuffer()
{
    ...
    gpu::Mailbox mailbox = sii->CreateSharedImage(
        alpha_ ? viz::RGBA_8888 : viz::RGBX_8888, size_, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage, gpu::kNullSurfaceHandle);
    ...
    // buffer.
    GLuint texture_id = gl->CreateAndTexStorage2DSharedImageCHROMIUM(mailbox.name);

    DrawingBuffer::Client* client = drawing_buffer_->client();
    client->DrawingBufferClientRestoreTexture2DBinding();

    return base::MakeRefCounted<ColorBuffer>(weak_factory_.GetWeakPtr(), size_, mailbox, texture_id);
}
...
scoped_refptr<StaticBitmapImage> XRWebGLDrawingBuffer::TransferToStaticBitmapImage()
{
    ...
    return AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(buffer->mailbox, buffer->produce_sync_token,
