...
public:
    ...
    void AssociateMailbox(GLuint, GLuint, GLuint, GLuint, GLuint, gpu::webgpu::MailboxFlags, const GLbyte*) override
    {
...
public:
    WebGPUSwapBufferProviderForTests(bool* alive, FakeProviderClient* client, WGPUDevice device, scoped_refptr<DawnControlClientHolder> dawn_control_client,
        WGPUTextureUsage usage, WGPUTextureFormat format)
        : WebGPUSwapBufferProvider(client, dawn_control_client, device, usage, format)
        , alive_(alive)
        , client_(client)
    {
...
protected:
    ...
    void SetUp() override
    {
        ...
        dawn_control_client_ = base::MakeRefCounted<DawnControlClientHolder>(std::move(provider), base::ThreadTaskRunnerHandle::Get());

        provider_ = base::MakeRefCounted<WebGPUSwapBufferProviderForTests>(&provider_alive_, &client_, fake_device_, dawn_control_client_, kUsage, kFormat);
    }
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyDestructionCompleteAfterAllResourceReleased)
{
    ...
    gpu::webgpu::ReservedTexture reservation1 = { reinterpret_cast<WGPUTexture>(&resource1), 1, 1, 1, 1 };
    viz::ReleaseCallback release_callback1;

    viz::TransferableResource resource2;
    gpu::webgpu::ReservedTexture reservation2 = { reinterpret_cast<WGPUTexture>(&resource2), 2, 2, 1, 1 };
    viz::ReleaseCallback release_callback2;

    viz::TransferableResource resource3;
    gpu::webgpu::ReservedTexture reservation3 = { reinterpret_cast<WGPUTexture>(&resource3), 3, 3, 1, 1 };
    viz::ReleaseCallback release_callback3;

    // Produce resources.
    EXPECT_CALL(*webgpu_, ReserveTexture(fake_device_, _)).WillOnce(Return(reservation1));
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource1, &release_callback1));

    EXPECT_CALL(*webgpu_, ReserveTexture(fake_device_, _)).WillOnce(Return(reservation2));
    provider_->GetNewTexture(kSize);
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource2, &release_callback2));

    EXPECT_CALL(*webgpu_, ReserveTexture(fake_device_, _)).WillOnce(Return(reservation3));
    provider_->GetNewTexture(kSize);
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyResizingProperlyAffectsResources)
{
    ...
    gpu::webgpu::ReservedTexture reservation = { reinterpret_cast<WGPUTexture>(&resource), 1, 1, 1, 1 };
    viz::ReleaseCallback release_callback;

    // Produce one resource of size kSize.
    EXPECT_CALL(*webgpu_, ReserveTexture(fake_device_, _)).WillOnce(Return(reservation));
    provider_->GetNewTexture(kSize);
...
TEST_F(WebGPUSwapBufferProviderTest, VerifyInsertAndWaitSyncTokenCorrectly)
{
    ...
    EXPECT_TRUE(provider_->PrepareTransferableResource(nullptr, &resource, &release_callback));
    EXPECT_EQ(webgpu_->most_recent_generated_token, resource.mailbox_holder.sync_token);
...
TEST_F(WebGPUSwapBufferProviderTest, ReserveTextureDescriptorForReflection)
{
    ...
    EXPECT_CALL(*webgpu_, ReserveTexture(fake_device_, _)).WillOnce(Invoke([&](WGPUDevice device, const WGPUTextureDescriptor* desc) -> auto {
