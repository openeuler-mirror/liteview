    ...
    const scoped_refptr<StaticBitmapImage>& source, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper)
{
    ...
    auto image_info = paint_image.GetSkImageInfo().makeWH(source->Size().width(), source->Size().height());
    ...
    auto provider
        = CanvasResourceProvider::CreateSharedImageProvider(image_info, cc::PaintFlags::FilterQuality::kLow, CanvasResourceProvider::ShouldInitialize::kNo,
            context_provider_wrapper, RasterMode::kGPU, source->IsOriginTopLeft(), gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT);
    ...
    cc::SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* out_resource, viz::ReleaseCallback* out_release_callback)
{
    ...
    const ImageOrientation origin = image_->IsOriginTopLeft() ? ImageOrientationEnum::kOriginTopLeft : ImageOrientationEnum::kOriginBottomLeft;
    ...
    if (gpu_compositing) {
        ...
        uint32_t filter = filter_quality_ == cc::PaintFlags::FilterQuality::kNone ? GL_NEAREST : GL_LINEAR;
        ...
        bool is_overlay_candidate = sii->UsageForMailbox(mailbox_holder.mailbox) & gpu::SHARED_IMAGE_USAGE_SCANOUT;

        SkColorType color_type = image_for_compositor->GetSkColorInfo().colorType();
        *out_resource = viz::TransferableResource::MakeGpu(mailbox_holder.mailbox, filter, mailbox_holder.texture_target, mailbox_holder.sync_token, size,
        ...
        // WebGPU does not.
        if (sii->UsageForMailbox(mailbox_holder.mailbox) & gpu::SHARED_IMAGE_USAGE_WEBGPU_SWAP_CHAIN_TEXTURE) {
        ...
        *out_release_callback = std::move(func);
        ...
        viz::ResourceFormat resource_format = viz::SkColorTypeToResourceFormat(dst_color_type);
        RegisteredBitmap registered = CreateOrRecycleBitmap(size, resource_format, bitmap_registrar);
        ...
        *out_resource = viz::TransferableResource::MakeSoftware(registered.bitmap->id(), size, resource_format);
...
ImageLayerBridge::RegisteredBitmap ImageLayerBridge::CreateOrRecycleBitmap(
    const gfx::Size& size, viz::ResourceFormat format, cc::SharedBitmapIdRegistrar* bitmap_registrar)
{
    auto* it = std::remove_if(recycled_bitmaps_.begin(), recycled_bitmaps_.end(), [&size, &format](const RegisteredBitmap& registered) {
        unsigned src_bytes_per_pixel = viz::BitsPerPixel(registered.bitmap->format()) / 8;
    ...
    registered.bitmap = base::MakeRefCounted<cc::CrossThreadSharedBitmap>(id, std::move(shm), size, format);
