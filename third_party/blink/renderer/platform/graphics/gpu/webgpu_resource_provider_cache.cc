...
ResourceCacheKey::ResourceCacheKey(const SkImageInfo& info, bool is_origin_top_left)
    : info(info)
...
RecyclableCanvasResource::RecyclableCanvasResource(
    std::unique_ptr<CanvasResourceProvider> resource_provider, const ResourceCacheKey& cache_key, base::WeakPtr<WebGPURecyclableResourceCache> cache)
...
RecyclableCanvasResource::~RecyclableCanvasResource()
{
    ...
    DCHECK(cache_key_.is_origin_top_left == resource_provider_->IsOriginTopLeft());
...
std::unique_ptr<RecyclableCanvasResource> WebGPURecyclableResourceCache::GetOrCreateCanvasResource(const SkImageInfo& info, bool is_origin_top_left)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    const ResourceCacheKey cache_key(info, is_origin_top_left);
    std::unique_ptr<CanvasResourceProvider> provider = AcquireCachedProvider(cache_key);
    if (!provider) {
        provider = CanvasResourceProvider::CreateWebGPUImageProvider(info, is_origin_top_left);
        if (!provider)
            return nullptr;
    }

    return std::make_unique<RecyclableCanvasResource>(std::move(provider), cache_key, weak_ptr_);
}

void WebGPURecyclableResourceCache::OnDestroyRecyclableResource(std::unique_ptr<CanvasResourceProvider> resource_provider)
{
    ...
    if (last_seen_max_unused_resources_in_bytes_ < total_unused_resources_in_bytes_) {
...
std::unique_ptr<CanvasResourceProvider> WebGPURecyclableResourceCache::AcquireCachedProvider(const ResourceCacheKey& cache_key)
{
    ...
    for (it = unused_providers_.begin(); it != unused_providers_.end(); ++it) {
        ...
        const auto it_cache_key = ResourceCacheKey(resource_provider->GetSkImageInfo(), resource_provider->IsOriginTopLeft());
...
void WebGPURecyclableResourceCache::ReleaseStaleResources()
{
    ...
    base::UmaHistogramCustomCounts("Blink.Canvas.WebGPUStaleResourceCount", stale_resource_count, /*min=*/0,
        /*max=*/300,
        /*buckets=*/50);
    ...
    int last_seen_max_unused_resources_in_kb = static_cast<int>(last_seen_max_unused_resources_in_bytes_ / 1024);
    base::UmaHistogramCustomCounts("Blink.Canvas.WebGPUMaxRecycledResourcesInKB", last_seen_max_unused_resources_in_kb,
        /*min=*/0,
    ...
    // calls.
    base::UmaHistogramCustomCounts("Blink.Canvas.WebGPUMaxRecycledResourcesCount", last_seen_max_unused_resources_,
        /*min=*/0,
        /*max=*/300,
        /*buckets=*/50);
