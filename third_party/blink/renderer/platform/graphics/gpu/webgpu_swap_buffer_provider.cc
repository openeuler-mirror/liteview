...
WebGPUSwapBufferProvider::WebGPUSwapBufferProvider(
    Client* client, scoped_refptr<DawnControlClientHolder> dawn_control_client, WGPUDevice device, WGPUTextureUsage usage, WGPUTextureFormat format)
...
std::tuple<uint32_t, bool> WebGPUSwapBufferProvider::GetTextureTargetAndOverlayCandidacy() const
{
...
scoped_refptr<WebGPUSwapBufferProvider::SwapBuffer> WebGPUSwapBufferProvider::NewOrRecycledSwapBuffer(
    gpu::SharedImageInterface* sii, base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider, const gfx::Size& size, SkAlphaType alpha_mode)
{
    // Recycled SwapBuffers must be the same size.
    if (!unused_swap_buffers_.empty() && unused_swap_buffers_.back()->size != size) {
        unused_swap_buffers_.clear();
    }

    if (unused_swap_buffers_.empty()) {
        gpu::Mailbox mailbox = sii->CreateSharedImage(Format(), size, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, alpha_mode,
            gpu::SHARED_IMAGE_USAGE_WEBGPU | gpu::SHARED_IMAGE_USAGE_WEBGPU_SWAP_CHAIN_TEXTURE | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, gpu::kNullSurfaceHandle);
        gpu::SyncToken creation_token = sii->GenUnverifiedSyncToken();

        unused_swap_buffers_.push_back(base::MakeRefCounted<SwapBuffer>(std::move(context_provider), mailbox, creation_token, size));
        DCHECK_EQ(unused_swap_buffers_.back()->size, size);
    }

    scoped_refptr<SwapBuffer> swap_buffer = std::move(unused_swap_buffers_.back());
...
void WebGPUSwapBufferProvider::RecycleSwapBuffer(scoped_refptr<SwapBuffer> swap_buffer)
{
    // We don't want to keep an arbitrary large number of swap buffers.
    if (unused_swap_buffers_.size() > static_cast<unsigned int>(kMaxRecycledSwapBuffers))
        return;

    unused_swap_buffers_.push_back(std::move(swap_buffer));
}

scoped_refptr<WebGPUMailboxTexture> WebGPUSwapBufferProvider::GetNewTexture(const WGPUTextureDescriptor& desc, SkAlphaType alpha_mode)
{
    ...
    current_swap_buffer_ = NewOrRecycledSwapBuffer(context_provider->ContextProvider()->SharedImageInterface(), context_provider, size, alpha_mode);

    // Make a mailbox texture from the swap buffer.
    current_swap_buffer_->mailbox_texture = WebGPUMailboxTexture::FromExistingMailbox(dawn_control_client_, device_, desc, current_swap_buffer_->mailbox,
        // Wait on the last usage of this swap buffer.
        current_swap_buffer_->access_finished_token, gpu::webgpu::WEBGPU_MAILBOX_DISCARD,
        ...
        base::BindOnce([](scoped_refptr<SwapBuffer> swap_buffer,
                           const gpu::SyncToken& access_finished_token) { swap_buffer->access_finished_token = access_finished_token; },
...
WebGPUSwapBufferProvider::WebGPUMailboxTextureAndSize WebGPUSwapBufferProvider::GetLastWebGPUMailboxTextureAndSize() const
{
    auto context_provider = GetContextProviderWeakPtr();
    ...
    return WebGPUMailboxTextureAndSize(WebGPUMailboxTexture::FromExistingMailbox(dawn_control_client_, device_, desc, last_swap_buffer_->mailbox,
                                           last_swap_buffer_->access_finished_token, gpu::webgpu::WEBGPU_MAILBOX_NONE),
    ...
    cc::SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* out_resource, viz::ReleaseCallback* out_release_callback)
{
    ...
    *out_resource = viz::TransferableResource::MakeGpu(current_swap_buffer_->mailbox, GL_LINEAR, GetTextureTarget(),
        current_swap_buffer_->access_finished_token, current_swap_buffer_->size, Format(), IsOverlayCandidate());
    ...
    const gfx::ColorSpace& dst_color_space, WebGraphicsContext3DVideoFramePool::FrameReadyCallback callback)
{
    ...
    auto success = frame_pool->CopyRGBATextureToVideoFrame(
        Format(), current_swap_buffer_->size, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, mailbox_holder, dst_color_space, std::move(callback));

    // Subsequent access to this swap buffer (either webgpu or compositor) must
    // wait for the copy operation to finish.
    frame_pool_ri->GenUnverifiedSyncTokenCHROMIUM(current_swap_buffer_->access_finished_token.GetData());
...
WebGPUSwapBufferProvider::SwapBuffer::SwapBuffer(
    base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider, gpu::Mailbox mailbox, gpu::SyncToken creation_token, gfx::Size size)
    : size(size)
    , mailbox(mailbox)
...
{
    if (context_provider) {
        gpu::SharedImageInterface* sii = context_provider->ContextProvider()->SharedImageInterface();
