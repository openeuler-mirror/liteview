...
public:
    ...
    std::unique_ptr<RecyclableCanvasResource> GetOrCreateCanvasResource(const SkImageInfo& info, bool is_origin_top_left);

    // When the holder is destroyed, move the resource provider to
    // |unused_providers_| if the cache is not full.
    void OnDestroyRecyclableResource(std::unique_ptr<CanvasResourceProvider> resource_provider);
...
private:
    ...
    std::unique_ptr<CanvasResourceProvider> AcquireCachedProvider(const ResourceCacheKey& cache_key);
