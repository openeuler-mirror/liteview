...
void VideoFrameResourceProvider::Initialize(viz::RasterContextProvider* media_context_provider, viz::SharedBitmapReporter* shared_bitmap_reporter)
{
    ...
    resource_updater_ = std::make_unique<media::VideoResourceUpdater>(nullptr, media_context_provider, shared_bitmap_reporter, resource_provider_.get(),
        settings_.use_stream_video_draw_quad, settings_.resource_settings.use_gpu_memory_buffer_resources, settings_.resource_settings.use_r16_texture,
        max_texture_size);
}
    ...
    viz::CompositorRenderPass* render_pass, scoped_refptr<media::VideoFrame> frame, media::VideoTransformation media_transform, bool is_opaque)
{
    ...
    resource_updater_->AppendQuads(render_pass, std::move(frame), transform, quad_rect, visible_quad_rect, mask_filter_info,
        /*clip_rect=*/absl::nullopt, is_opaque, draw_opacity, sorting_context_id);
}
