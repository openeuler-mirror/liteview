...
absl::optional<DarkModeImageClassifier::Features> DarkModeImageClassifier::GetFeatures(const SkPixmap& pixmap, const SkIRect& src) const
{
...
float DarkModeImageClassifier::ComputeColorBucketsRatio(const std::vector<SkColor>& sampled_pixels, const ColorMode color_mode) const
{
    ...
    return static_cast<float>(buckets.size()) / max_buckets[color_mode == ColorMode::kColor];
}
