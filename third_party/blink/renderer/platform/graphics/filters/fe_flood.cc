...
sk_sp<PaintFilter> FEFlood::CreateImageFilter()
{
    ...
    return sk_make_sp<ColorFilterPaintFilter>(
        ...
        SkColorFilters::Blend(Color::FromSkColor4f(color).Rgb(), SkBlendMode::kSrc), nullptr, base::OptionalToPtr(crop_rect));
}

WTF::TextStream& FEFlood::ExternalRepresentation(WTF::TextStream& ts, int indent) const
{
