...
sk_sp<PaintFilter> FEConvolveMatrix::CreateImageFilter()
{
    ...
    return sk_make_sp<MatrixConvolutionPaintFilter>(
        kernel_size, kernel.get(), gain, bias, target, tile_mode, convolve_alpha, std::move(input), base::OptionalToPtr(crop_rect));
}

static WTF::TextStream& operator<<(WTF::TextStream& ts, const FEConvolveMatrix::EdgeModeType& type)
{
    switch (type) {
...
WTF::TextStream& FEConvolveMatrix::ExternalRepresentation(WTF::TextStream& ts, int indent) const
{
