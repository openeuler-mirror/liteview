...
scoped_refptr<StaticBitmapImage> CreateBitmap()
{
    ...
    return AcceleratedStaticBitmapImage::CreateFromCanvasMailbox(mailbox, GenTestSyncToken(100), 0, SkImageInfo::MakeN32Premul(100, 100), GL_TEXTURE_2D, true,
        SharedGpuContext::ContextProviderWrapper(), base::PlatformThread::CurrentRef(), base::MakeRefCounted<base::NullTaskRunner>(), base::DoNothing(),
...
TEST_F(AcceleratedStaticBitmapImageTest, CopyToTextureSynchronization)
{
    ...
    // dest context.
    EXPECT_CALL(destination_gl, WaitSyncTokenCHROMIUM(Pointee(SyncTokenMatcher(bitmap->GetMailboxHolder().sync_token))));
