...
public:
    ...
    {
        return gmb_manager_ ? gmb_manager_->CreateGpuMemoryBuffer(size, format, usage, gpu::kNullSurfaceHandle, nullptr) : nullptr;
    }
        ...
        SkAlphaType alpha_type, uint32_t usage, gpu::Mailbox& mailbox, gpu::SyncToken& sync_token) override
    {
        ...
        if (!sii || !gmb_manager_)
            return;
        mailbox = sii->CreateSharedImage(gpu_memory_buffer, gmb_manager_, plane, color_space, surface_origin, alpha_type, usage);
...
bool WebGraphicsContext3DVideoFramePool::CopyRGBATextureToVideoFrame(viz::ResourceFormat src_format, const gfx::Size& src_size,
    const gfx::ColorSpace& src_color_space, GrSurfaceOrigin src_surface_origin, const gpu::MailboxHolder& src_mailbox_holder,
    const gfx::ColorSpace& dst_color_space, FrameReadyCallback callback)
{
    if (!weak_context_provider_)
        return false;
    auto* context_provider = weak_context_provider_->ContextProvider();
    ...
    const bool copy_succeeded = media::CopyRGBATextureToVideoFrame(
        raster_context_provider, src_format, src_size, src_color_space, src_surface_origin, src_mailbox_holder, dst_frame.get());
    if (!copy_succeeded) {
    ...
    auto on_query_done_cb = [](scoped_refptr<media::VideoFrame> frame, base::WeakPtr<blink::WebGraphicsContext3DProviderWrapper> ctx_wrapper, unsigned query_id,
    ...
    context_support->SignalQuery(query_id, base::BindOnce(on_query_done_cb, dst_frame, weak_context_provider_, query_id, std::move(callback)));
    ...
    scoped_refptr<media::VideoFrame> src_video_frame, const gfx::ColorSpace& dst_color_space, FrameReadyCallback callback)
{
    ...
    return CopyRGBATextureToVideoFrame(texture_format, src_video_frame->coded_size(), src_video_frame->ColorSpace(),
        src_video_frame->metadata().texture_origin_is_top_left ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin, src_video_frame->mailbox_holder(0),
        dst_color_space, WTF::BindOnce(ApplyMetadataAndRunCallback, src_video_frame, std::move(callback)));
}
