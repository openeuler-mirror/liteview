...
void PaintArtifactCompositor::SetPrefersLCDText(bool prefers)
{
    if (prefers_lcd_text_ == prefers)
        return;
    SetNeedsUpdate(PaintArtifactCompositorUpdateReason::kPaintArtifactCompositorPrefersLCDText);
...
const TransformPaintPropertyNode& PaintArtifactCompositor::NearestScrollTranslationForLayer(const PendingLayer& pending_layer)
{
    ...
    return pending_layer.GetPropertyTreeState().Transform().NearestScrollTranslationNode();
}
    ...
    const PaintChunk& previous, const PaintArtifact& previous_artifact, const PaintChunk& repainted, const PaintArtifact& repainted_artifact)
{
    ...
    // opaque property.
    if ((previous.background_color == Color()) != (repainted.background_color == Color())) {
        return true;
    }
...
void PaintArtifactCompositor::SetNeedsFullUpdateAfterPaintIfNeeded(const PaintChunkSubset& previous, const PaintChunkSubset& repainted)
{
    if (needs_update_)
        return;
    ...
    if (previous.size() != repainted.size()) {
        SetNeedsUpdate(PaintArtifactCompositorUpdateReason::kPaintArtifactCompositorNeedsFullUpdateChunksChanged);
        return;
    }
    ...
    for (; previous_chunk_it != previous.end(); ++previous_chunk_it, ++repainted_chunk_it) {
        const auto& previous_chunk = *previous_chunk_it;
        const auto& repainted_chunk = *repainted_chunk_it;
        if (NeedsFullUpdateAfterPaintingChunk(previous_chunk, previous.GetPaintArtifact(), repainted_chunk, repainted.GetPaintArtifact())) {
            SetNeedsUpdate(PaintArtifactCompositorUpdateReason::kPaintArtifactCompositorNeedsFullUpdateAfterPaintingChunk);
            return;
        }
    }
}
    ...
    const EffectPaintPropertyNode& parent_effect, wtf_size_t first_layer_in_parent_group_index, const EffectPaintPropertyNode& effect, wtf_size_t layer_index)
{
    ...
    absl::optional<PropertyTreeState> upcast_state = group_state.CanUpcastWith(layer.GetPropertyTreeState());
    if (!upcast_state)
        return false;
    ...
    if (effect.BlendMode() != SkBlendMode::kSrcOver) {
        ...
        if (num_previous_siblings) {
            if (num_previous_siblings > 2)
                return false;
            if (num_previous_siblings == 2 && pending_layers_[first_layer_in_parent_group_index].DrawsContent())
                return false;
            const auto& previous_sibling = pending_layers_[layer_index - 1];
            if (previous_sibling.DrawsContent() && !previous_sibling.CanMerge(layer, *upcast_state, prefers_lcd_text_))
                return false;
        }
    }
...
void PaintArtifactCompositor::LayerizeGroup(const PaintChunkSubset& chunks, const EffectPaintPropertyNode& current_group, PaintChunkIterator& chunk_cursor,
    HashSet<const TransformPaintPropertyNode*>& directly_composited_transforms, bool force_draws_content)
{
    ...
    while (chunk_cursor != chunks.end()) {
        ...
        if (&chunk_effect == &current_group) {
            ...
            LayerizeGroup(chunks, *subgroup, chunk_cursor, directly_composited_transforms, force_draws_content || subgroup->DrawsContent());
        ...
        if (RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled()) {
            if (const auto* composited_transform = new_layer.GetPropertyTreeState().Transform().NearestDirectlyCompositedAncestor()) {
                if (directly_composited_transforms.insert(composited_transform).is_new_entry) {
                    continue;
                }
            }
        }
...
void PaintArtifactCompositor::CollectPendingLayers(scoped_refptr<const PaintArtifact> artifact)
{
    ...
    LayerizeGroup(subset, EffectPaintPropertyNode::Root(), cursor, directly_composited_transforms, /*force_draws_content*/ false);
...
void SynthesizedClip::UpdateLayer(const ClipPaintPropertyNode& clip, const TransformPaintPropertyNode& transform)
{
    ...
    auto new_translation_2d_or_matrix = GeometryMapper::SourceToDestinationProjection(clip.LocalTransformSpace(), transform);
    ...
    new_translation_2d_or_matrix.PostTranslate(-layer_offset.x(), -layer_offset.y());
    ...
    if (!path && new_translation_2d_or_matrix.IsIdentityOr2DTranslation()) {
        ...
        needs_display = rrect_is_local_ || new_rrect != rrect_ || new_translation_2d_or_matrix != translation_2d_or_matrix_ || !clip.ClipPathEquals(path_);
...
{
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>(cc::DisplayItemList::kTopLevelDisplayItemList);
    cc::PaintFlags flags;
...
static void UpdateCompositorViewportProperties(
    const PaintArtifactCompositor::ViewportProperties& properties, PropertyTreeManager& property_tree_manager, cc::LayerTreeHost* layer_tree_host)
{
    // The inner and outer viewports' existence is linked. That is, either they're
    // both null or they both exist.
    DCHECK_EQ(static_cast<bool>(properties.outer_scroll_translation), static_cast<bool>(properties.inner_scroll_translation));
    DCHECK(!properties.outer_clip || static_cast<bool>(properties.inner_scroll_translation));
    ...
    if (properties.overscroll_elasticity_effect) {
        ids.overscroll_elasticity_effect = properties.overscroll_elasticity_effect->GetCompositorElementId();
    }
    ...
    if (properties.inner_scroll_translation) {
        ids.inner_scroll = property_tree_manager.EnsureCompositorInnerScrollNode(*properties.inner_scroll_translation);
        ...
        if (properties.outer_scroll_translation) {
            ids.outer_scroll = property_tree_manager.EnsureCompositorOuterScrollNode(*properties.outer_scroll_translation);
        }
    }
...
void PaintArtifactCompositor::Update(scoped_refptr<const PaintArtifact> artifact, const ViewportProperties& viewport_properties,
    const Vector<const TransformPaintPropertyNode*>& scroll_translation_nodes, Vector<std::unique_ptr<cc::DocumentTransitionRequest>> transition_requests)
{
    const bool unification_enabled = base::FeatureList::IsEnabled(features::kScrollUnification);
    ...
    host->property_trees()->effect_tree_mutable().ClearTransitionPseudoElementEffectNodes();
    ...
    for (auto& pending_layer : pending_layers_) {
        ...
        const auto& scroll_translation = NearestScrollTranslationForLayer(pending_layer);
        int scroll_id = property_tree_manager.EnsureCompositorScrollNode(scroll_translation);
        ...
        if (shared_element_id.IsValid()) {
            host->property_trees()->effect_tree_mutable().AddTransitionPseudoElementEffectId(effect_id);
        }
    }
...
void PaintArtifactCompositor::UpdateRepaintedLayers(scoped_refptr<const PaintArtifact> repainted_artifact)
{
    ...
    for (const auto& chunk : repainted_chunks) {
        ...
        DCHECK(!chunk.properties.GetPropertyTreeState().Unalias().ChangedToRoot(PaintPropertyChangeType::kChangedOnlyNonRerasterValues));
    }
#endif
...
static void UpdateLayerDebugInfo(cc::Layer& layer, const PaintChunk::Id& id, const String& name, DOMNodeId owner_node_id,
    CompositingReasons compositing_reasons, RasterInvalidationTracking* raster_invalidation_tracking)
{
    ...
    if (RasterInvalidationTracking::IsTracingRasterInvalidations() && raster_invalidation_tracking) {
        raster_invalidation_tracking->AddToLayerDebugInfo(debug_info);
        raster_invalidation_tracking->ClearInvalidations();
    }
}

static void UpdateLayerDebugInfo(cc::Layer& layer, const PaintChunk::Id& id, const PaintArtifact& paint_artifact, CompositingReasons compositing_reasons,
    RasterInvalidationTracking* raster_invalidation_tracking)
{
    UpdateLayerDebugInfo(layer, id, paint_artifact.ClientDebugName(id.client_id), paint_artifact.ClientOwnerNodeId(id.client_id), compositing_reasons,
...
void PaintArtifactCompositor::UpdateDebugInfo() const
{
    ...
    for (const auto& pending_layer : pending_layers_) {
        ...
        UpdateLayerDebugInfo(layer, pending_layer.FirstPaintChunk().id, pending_layer.Chunks().GetPaintArtifact(),
            GetCompositingReasons(pending_layer, previous_pending_layer), tracking);
        previous_pending_layer = &pending_layer;
    }
}

CompositingReasons PaintArtifactCompositor::GetCompositingReasons(const PendingLayer& layer, const PendingLayer* previous_layer) const
{
    ...
    if (!previous_layer || &layer.GetPropertyTreeState().Transform() != &previous_layer->GetPropertyTreeState().Transform()) {
        reasons |= layer.GetPropertyTreeState().Transform().DirectCompositingReasonsForDebugging();
        if (!layer.GetPropertyTreeState().Transform().BackfaceVisibilitySameAsParent())
            reasons |= CompositingReason::kBackfaceVisibilityHidden;
    }

    if (!previous_layer || &layer.GetPropertyTreeState().Effect() != &previous_layer->GetPropertyTreeState().Effect()) {
        const auto& effect = layer.GetPropertyTreeState().Effect();
        if (effect.HasDirectCompositingReasons())
            reasons |= effect.DirectCompositingReasonsForDebugging();
        if (reasons == CompositingReason::kNone && layer.GetCompositingType() == PendingLayer::kOther) {
    ...
    if (reasons == CompositingReason::kNone && layer.GetCompositingType() == PendingLayer::kOverlap)
...
void PaintArtifactCompositor::ClearPropertyTreeChangedState()
{
    ...
    for (auto& layer : pending_layers_) {
        ...
        const auto& first_chunk_state = layer.Chunks().begin()->properties.GetPropertyTreeState();
        ...
        for (auto& chunk : layer.Chunks()) {
            chunk.properties.GetPropertyTreeState().ClearChangedToRoot(changed_sequence_number);
        }
    }
...
void PaintArtifactCompositor::SetScrollbarNeedsDisplay(CompositorElementId element_id)
{
    for (auto& pending_layer : pending_layers_) {
        if (pending_layer.GetCompositingType() == PendingLayer::kScrollbarLayer && pending_layer.CcLayer().element_id() == element_id) {
...
void PaintArtifactCompositor::SetNeedsUpdate(PaintArtifactCompositorUpdateReason reason)
{
    UMA_HISTOGRAM_ENUMERATION("Blink.Paint.PaintArtifactCompositorUpdateReason", reason, PaintArtifactCompositorUpdateReason::kCount);
    if (!needs_update_) {
        needs_update_ = true;
        UMA_HISTOGRAM_ENUMERATION("Blink.Paint.PaintArtifactCompositorUpdateFirstReason", reason, PaintArtifactCompositorUpdateReason::kCount);
    }
}
