...
{
    *os << (cc::PaintOp::IsUnsetRect(rect) ? "(unset)" : gfx::SkRectToRectF(rect).ToString());
}
...
class PaintRecordMatcher : public testing::MatcherInterface<const cc::PaintOpBuffer&> {
public:
    static testing::Matcher<const cc::PaintOpBuffer&> Make(std::initializer_list<cc::PaintOpType> args)
    {
    ...
    bool MatchAndExplain(const cc::PaintOpBuffer& buffer, testing::MatchResultListener* listener) const override
    {
        size_t index = 0;
        ...
        if (index == expected_ops_.size())
            return true;
        if (listener->IsInterested()) {
            *listener << "unexpected end of list, expecting " << expected_ops_[index] << " at index " << index << ".";
        }
        return false;
    }
...
#define EXPECT_EFFECT_BOUNDS(x, y, width, height, op_buffer, index)                                                                                            \
    do {                                                                                                                                                       \
        SkRect bounds;                                                                                                                                         \
        if (const auto* save_layer_alpha = (op_buffer).GetOpAtForTesting<cc::SaveLayerAlphaOp>(index)) {                                                       \
            bounds = save_layer_alpha->bounds;                                                                                                                 \
        } else if (const auto* save_layer = (op_buffer).GetOpAtForTesting<cc::SaveLayerOp>(index)) {                                                           \
...
#define EXPECT_TRANSLATE(x, y, op_buffer, index)                                                                                                               \
    do {                                                                                                                                                       \
        const auto* translate = (op_buffer).GetOpAtForTesting<cc::TranslateOp>(index);                                                                         \
...
#define EXPECT_CLIP(r, op_buffer, index)                                                                                                                       \
    do {                                                                                                                                                       \
        const auto* clip_op = (op_buffer).GetOpAtForTesting<cc::ClipRectOp>(index);                                                                            \
...
#define EXPECT_ROUNDED_CLIP(r, op_buffer, index)                                                                                                               \
    do {                                                                                                                                                       \
        const auto* clip_op = (op_buffer).GetOpAtForTesting<cc::ClipRRectOp>(index);                                                                           \
        ASSERT_NE(nullptr, clip_op);                                                                                                                           \
...
public:
    ...
    void AddChunk(const TransformPaintPropertyNodeOrAlias& t, const ClipPaintPropertyNodeOrAlias& c, const EffectPaintPropertyNodeOrAlias& e,
        const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100), const absl::optional<gfx::Rect>& drawable_bounds = absl::nullopt)
    {
        auto record = sk_make_sp<PaintRecord>();
        record->push<cc::DrawRectOp>(gfx::RectToSkRect(drawable_bounds ? *drawable_bounds : bounds), cc::PaintFlags());
        AddChunk(std::move(record), t, c, e, bounds, drawable_bounds);
    }

    // Add a paint chunk with a given paint record and property nodes.
    void AddChunk(sk_sp<PaintRecord> record, const TransformPaintPropertyNodeOrAlias& t, const ClipPaintPropertyNodeOrAlias& c,
        ...
        const absl::optional<gfx::Rect>& drawable_bounds = absl::nullopt)
    {
        ...
        chunks.emplace_back(i, i + 1, DefaultClient(), DefaultId(), PropertyTreeStateOrAlias(t, c, e));
        chunks.back().bounds = bounds;
        chunks.back().drawable_bounds = drawable_bounds ? *drawable_bounds : bounds;
    }

    void AddEmptyChunk(const TransformPaintPropertyNode& t, const ClipPaintPropertyNode& c, const EffectPaintPropertyNode& e,
        const gfx::Rect& bounds = gfx::Rect(0, 0, 100, 100))
    {
        auto& chunks = paint_artifact_->PaintChunks();
        auto i = paint_artifact_->GetDisplayItemList().size();
        chunks.emplace_back(i, i, DefaultClient(), DefaultId(), PropertyTreeState(t, c, e));
        chunks.back().bounds = bounds;
    }

    Vector<PaintChunk>* GetChunks()
    {
        return &paint_artifact_->PaintChunks();
    }
...
private:
    scoped_refptr<PaintArtifact> paint_artifact_ = base::MakeRefCounted<PaintArtifact>();
};
...
TEST_P(PaintChunksToCcLayerTest, EffectGroupingSimple)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
...
TEST_P(PaintChunksToCcLayerTest, EffectFilterGroupingNestedWithTransforms)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::Concat, // <t1*t2>
    ...
    // t2^1
    EXPECT_TRANSLATE(-t2->Translation2D().x(), -t2->Translation2D().y(), *output, 6);
}
...
TEST_P(PaintChunksToCcLayerTest, ClipSpaceInversion)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::Concat, // <t1
...
TEST_P(PaintChunksToCcLayerTest, FilterEffectSpaceInversion)
{
    ...
    auto output = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
                      ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::Concat, // <t1>
...
TEST_P(PaintChunksToCcLayerTest, NonRootLayerSimple)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(*t1, *c1, *e1), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
...
TEST_P(PaintChunksToCcLayerTest, NonRootLayerTransformEscape)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(*t1, *c1, *e1), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::Concat, // <t1^-1>
            cc::PaintOpType::DrawRecord, // <p0/>
            cc::PaintOpType::Restore })); // </t1^-1>
}
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClip)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), *c1, e0()), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::SaveLayerAlpha, // <e1>
            cc::PaintOpType::Save,
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClipNestedInCompositedEffect)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), c0(), *e1), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
...
TEST_P(PaintChunksToCcLayerTest, EffectWithNoOutputClipNestedInCompositedEffectAndClip)
{
    ...
    sk_sp<PaintRecord> output
        = PaintChunksToCcLayer::Convert(chunks.Build(), PropertyTreeState(t0(), *c1, *e1), gfx::Vector2dF(), cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer)
              ->ReleaseAsRecord();
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::SaveLayerAlpha, // <e2>
            cc::PaintOpType::DrawRecord, // <p0/>
...
TEST_P(PaintChunksToCcLayerTest, VisualRect)
{
    ...
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState(*layer_transform, c0(), e0()), gfx::Vector2dF(100, 200), *cc_list);
...
TEST_P(PaintChunksToCcLayerTest, NoncompositedClipPath)
{
    ...
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>(cc::DisplayItemList::kTopLevelDisplayItemList);
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(), *cc_list);
...
TEST_P(PaintChunksToCcLayerTest, CombineClips)
{
    ...
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::ClipRect, // <c1+c2>
...
TEST_P(PaintChunksToCcLayerTest, CombineClipsWithRoundedRects)
{
    ...
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::Save, cc::PaintOpType::ClipRRect, // <c1+c2+c3>
...
TEST_P(PaintChunksToCcLayerTest, ChunksSamePropertyTreeState)
{
    ...
    EXPECT_THAT(*output,
        PaintRecordMatcher::Make({ cc::PaintOpType::DrawRecord, // <p0/>
            cc::PaintOpType::Save, cc::PaintOpType::Concat, // <t1>
            cc::PaintOpType::DrawRecord, // <p1/>
            cc::PaintOpType::DrawRecord, // <p2/>
...
TEST_P(PaintChunksToCcLayerTest, OnlyNoopTransformIsNotEmitted)
{
    ...
    EXPECT_THAT(*output, PaintRecordMatcher::Make({ cc::PaintOpType::DrawRecord, cc::PaintOpType::DrawRecord, cc::PaintOpType::DrawRecord }));
}
...
static sk_sp<cc::PaintFilter> MakeFilter(gfx::RectF bounds)
{
    ...
    return sk_make_sp<ColorFilterPaintFilter>(SkColorFilters::Blend(SK_ColorBLUE, SkBlendMode::kSrc), nullptr, &rect);
}
...
TEST_P(PaintChunksToCcLayerTest, ReferenceFilterOnEmptyChunk)
{
    ...
    auto cc_list = base::MakeRefCounted<cc::DisplayItemList>(cc::DisplayItemList::kTopLevelDisplayItemList);
    PaintChunksToCcLayer::ConvertInto(chunks.Build(), PropertyTreeState::Root(), gfx::Vector2dF(5, 10), *cc_list);
...
TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataSetOnLayer)
{
    ...
    chunks.GetChunks()->back().region_capture_data = std::make_unique<RegionCaptureData>(kMap);

    PaintChunksToCcLayer::UpdateLayerProperties(*layer, PropertyTreeState::Root(), chunks.Build());
...
TEST_P(PaintChunksToCcLayerTest, UpdateLayerPropertiesRegionCaptureDataMultipleChunks)
{
    ...
    const RegionCaptureData kSecondMap { { kSecondCropId, gfx::Rect { 51, 61, 101, 201 } }, { kThirdCropId, gfx::Rect { 52, 62, 102, 202 } } };
    chunks.GetChunks()->back().region_capture_data = std::make_unique<RegionCaptureData>(kSecondMap);
...
TEST_P(PaintChunksToCcLayerTest, NonCompositedBackdropFilter)
{
    ...
    // ignored.
    EXPECT_THAT(*output, PaintRecordMatcher::Make({ cc::PaintOpType::SaveLayerAlpha, cc::PaintOpType::DrawRecord, cc::PaintOpType::Restore }));
}
