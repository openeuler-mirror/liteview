...
class PLATFORM_EXPORT PaintArtifactCompositor final : private PropertyTreeManagerClient {
...
public:
    ...
    // property tree.
    void Update(scoped_refptr<const PaintArtifact> artifact, const ViewportProperties& viewport_properties,
        const Vector<const TransformPaintPropertyNode*>& scroll_translation_nodes, Vector<std::unique_ptr<cc::DocumentTransitionRequest>> requests);
    ...
    void SetNeedsFullUpdateAfterPaintIfNeeded(const PaintChunkSubset& previous, const PaintChunkSubset& repainted);
...
private:
    ...
    void LayerizeGroup(const PaintChunkSubset&, const EffectPaintPropertyNode&, PaintChunkIterator& chunk_cursor,
    ...
    const TransformPaintPropertyNode& NearestScrollTranslationForLayer(const PendingLayer&);
    ...
    CompositingReasons GetCompositingReasons(const PendingLayer& layer, const PendingLayer* previous_layer) const;
