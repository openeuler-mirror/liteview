...
bool IsCompositedScrollHitTest(const PaintChunk& chunk)
{
    ...
    return scroll_translation && scroll_translation->HasDirectCompositingReasons();
}
...
{
    if (std::abs(std::round(bounds.x()) - bounds.x()) <= std::numeric_limits<float>::epsilon()
        && std::abs(std::round(bounds.y()) - bounds.y()) <= std::numeric_limits<float>::epsilon()
        && std::abs(std::round(bounds.right()) - bounds.right()) <= std::numeric_limits<float>::epsilon()
        && std::abs(std::round(bounds.bottom()) - bounds.bottom()) <= std::numeric_limits<float>::epsilon()) {
...
PendingLayer::PendingLayer(const PaintChunkSubset& chunks, const PaintChunkIterator& first_chunk)
    : PendingLayer(chunks, *first_chunk, first_chunk.IndexInPaintArtifact())
{
}

PendingLayer::PendingLayer(const PaintChunkSubset& chunks, const PaintChunk& first_chunk, wtf_size_t first_chunk_index_in_paint_artifact)
    ...
    , property_tree_state_(first_chunk.properties.GetPropertyTreeState().Unalias())
    , compositing_type_(kOther)
{
    ...
    if (const absl::optional<gfx::RectF>& visibility_limit = VisibilityLimit(GetPropertyTreeState())) {
...
gfx::RectF PendingLayer::MapRectKnownToBeOpaque(const PropertyTreeState& new_state) const
{
...
bool PendingLayer::MergeInternal(const PendingLayer& guest, const PropertyTreeState& guest_state, bool prefers_lcd_text, bool dry_run)
{
    ...
    const absl::optional<PropertyTreeState>& merged_state = GetPropertyTreeState().CanUpcastWith(guest_state);
    if (!merged_state)
        return false;

    const absl::optional<gfx::RectF>& merged_visibility_limit = VisibilityLimit(*merged_state);
    ...
    GeometryMapper::LocalToAncestorVisualRect(guest_state, *merged_state, new_guest_bounds);
    if (merged_visibility_limit)
        new_guest_bounds.Rect().Intersect(*merged_visibility_limit);

    gfx::RectF merged_bounds = gfx::UnionRects(new_home_bounds.Rect(), new_guest_bounds.Rect());
    ...
    if (!guest.has_decomposited_blend_mode_) {
        merged_rect_known_to_be_opaque = gfx::MaximumCoveredRect(MapRectKnownToBeOpaque(*merged_state), guest.MapRectKnownToBeOpaque(*merged_state));
...
bool PendingLayer::MightOverlap(const PendingLayer& other) const
{
    return GeometryMapper::MightOverlapForCompositing(
        bounds_, property_tree_state_.GetPropertyTreeState(), other.bounds_, other.property_tree_state_.GetPropertyTreeState());
}
...
void PendingLayer::DecompositeTransforms(Vector<PendingLayer>& pending_layers)
{
    ...
    for (PendingLayer& pending_layer : pending_layers) {
        ...
        while (!transform->IsRoot() && can_be_decomposited.at(transform)) {
            pending_layer.offset_of_decomposited_transforms_ += transform->Translation2D();
...
void PendingLayer::UpdateScrollbarLayer(PendingLayer* old_pending_layer)
{
    ...
    scrollbar_layer->SetOffsetToTransformParent(scrollbar_layer->offset_to_transform_parent() + gfx::Vector2dF(offset_of_decomposited_transforms_));
    DCHECK(!cc_layer_);
...
void PendingLayer::UpdateCompositedLayerForRepaint(scoped_refptr<const PaintArtifact> repainted_artifact, cc::LayerSelection& layer_selection)
{
    ...
    DCHECK_EQ(old_artifact.PaintChunks().size(), repainted_artifact->PaintChunks().size());
...
void PendingLayer::UpdateLayerProperties()
{
    ...
    if (compositing_type_ == PendingLayer::kForeignLayer)
        return;
    PaintChunksToCcLayer::UpdateLayerProperties(CcLayer(), GetPropertyTreeState(), Chunks());
}
...
void PendingLayer::UpdateLayerSelection(cc::LayerSelection& layer_selection)
{
    ...
    if (compositing_type_ == PendingLayer::kForeignLayer)
        return;
    bool any_selection_was_painted = PaintChunksToCcLayer::UpdateLayerSelection(CcLayer(), GetPropertyTreeState(), Chunks(), layer_selection);
    ...
    for (auto it = chunks_.end(); it != chunks_.begin();) {
        ...
        if (chunk.background_color_area >= min_background_area) {
            ...
            if (&chunk_effect != &property_tree_state_.Effect()) {
                ...
                chunk_background_color = chunk_background_color.CombineWithAlpha(chunk_effect.Opacity());
            }
