...
ContentLayerClientImpl::ContentLayerClientImpl()
    ...
    , raster_invalidation_function_(base::BindRepeating(&ContentLayerClientImpl::InvalidateRect, base::Unretained(this)))
{
}
...
void ContentLayerClientImpl::UpdateCcPictureLayer(const PendingLayer& pending_layer)
{
    ...
    raster_invalidator_.Generate(raster_invalidation_function_, paint_chunks, layer_offset, layer_bounds, layer_state);

    absl::optional<RasterUnderInvalidationCheckingParams> raster_under_invalidation_params;
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()) {
        raster_under_invalidation_params.emplace(
            *raster_invalidator_.GetTracking(), gfx::Rect(layer_bounds), paint_chunks.GetPaintArtifact().ClientDebugName(paint_chunks.begin()->id.client_id));
    }
    ...
    if (cc_display_item_list_ && layer_bounds == old_layer_bounds && cc_picture_layer_->draws_content() == pending_layer.DrawsContent()
    ...
    cc_display_item_list_ = PaintChunksToCcLayer::Convert(
        paint_chunks, layer_state, layer_offset, cc::DisplayItemList::kTopLevelDisplayItemList, base::OptionalToPtr(raster_under_invalidation_params));
    ...
    if (!contents_opaque) {
        cc_picture_layer_->SetContentsOpaqueForText(pending_layer.TextKnownToBeOnOpaqueBackground());
    }
}
