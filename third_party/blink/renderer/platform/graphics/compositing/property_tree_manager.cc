...
{
    if (transform_node.GetStickyConstraint() || transform_node.GetAnchorScrollContainersData()) {
        ...
        if (transform_node.ScrollNode()) {
            ...
            compositor_node.scroll_offset = gfx::PointAtOffsetFromOrigin(-translation);
            DCHECK(compositor_node.local.IsIdentity());
            DCHECK_EQ(gfx::Point3F(), compositor_node.origin);
        } else {
...
static void SetTransformTreePageScaleFactor(cc::TransformTree* transform_tree, cc::TransformNode* page_scale_node)
{
...
bool PropertyTreeManager::DirectlyUpdateScrollOffsetTransform(cc::LayerTreeHost& host, const TransformPaintPropertyNode& transform)
{
    ...
    auto* property_trees = host.property_trees();
    auto* cc_scroll_node = property_trees->scroll_tree_mutable().Node(scroll_node->CcNodeId(property_trees->sequence_number()));
    if (!cc_scroll_node)
        return false;
    ...
    gfx::PointF scroll_offset = gfx::PointAtOffsetFromOrigin(-transform.Translation2D());
...
void PropertyTreeManager::EnsureCompositorScrollNodes(const Vector<const TransformPaintPropertyNode*>& scroll_translation_nodes)
{
...
void PropertyTreeManager::SetupRootEffectNode()
{
    ...
    effect_node.stable_id = CompositorElementIdFromUniqueObjectId(unique_id).GetStableId();
...
bool TransformsMayBe2dAxisMisaligned(const TransformPaintPropertyNode& a, const TransformPaintPropertyNode& b)
{
    if (&a == &b)
        return false;
    const auto& translation_2d_or_matrix = GeometryMapper::SourceToDestinationProjection(a, b);
    if (!translation_2d_or_matrix.IsIdentityOr2DTranslation() && !translation_2d_or_matrix.Matrix().Preserves2dAxisAlignment())
        return true;
...
{
    const auto* previous_transform = effect.IsRoot() ? nullptr : current_.transform;
...
int PropertyTreeManager::EnsureCompositorTransformNode(const TransformPaintPropertyNode& transform_node)
{
    ...
    if (const auto* anchor_scroll_data = transform_node.GetAnchorScrollContainersData()) {
        cc::AnchorScrollContainersData& compositor_data = transform_tree_.EnsureAnchorScrollContainersData(id);
        compositor_data.inner_most_scroll_container_id = EnsureCompositorScrollNode(*anchor_scroll_data->inner_most_scroll_container);
        compositor_data.outer_most_scroll_container_id = EnsureCompositorScrollNode(*anchor_scroll_data->outer_most_scroll_container);
        compositor_data.accumulated_scroll_origin = anchor_scroll_data->accumulated_scroll_origin;
    }
    ...
    if (auto* scroll_node = transform_node.ScrollNode()) {
        ...
        CreateCompositorScrollNode(*scroll_node, compositor_node, transform_node.HasDirectCompositingReasons());
    }
...
void PropertyTreeManager::CreateCompositorScrollNode(
    const ScrollPaintPropertyNode& scroll_node, const cc::TransformNode& scroll_offset_translation, bool is_composited)
{
    ...
    compositor_node.main_thread_scrolling_reasons = scroll_node.GetMainThreadScrollingReasons();
    ...
    scroll_tree_.SetScrollOffset(compositor_element_id, scroll_offset_translation.scroll_offset);
}

int PropertyTreeManager::EnsureCompositorScrollNode(const TransformPaintPropertyNode& scroll_offset_translation)
{
...
int PropertyTreeManager::EnsureCompositorInnerScrollNode(const TransformPaintPropertyNode& scroll_offset_translation)
{
...
int PropertyTreeManager::EnsureCompositorOuterScrollNode(const TransformPaintPropertyNode& scroll_offset_translation)
{
    int node_id = EnsureCompositorScrollNode(scroll_offset_translation);
...
void PropertyTreeManager::EmitClipMaskLayer()
{
    ...
    int scroll_id = EnsureCompositorScrollNode(current_.transform->NearestScrollTranslationNode());
    ...
    if (!mask_isolation->backdrop_filters.IsEmpty()) {
        ...
        auto element_id = CompositorElementIdFromUniqueObjectId(mask_effect.stable_id, CompositorElementIdNamespace::kEffectMask);
...
bool PropertyTreeManager::SupportsShaderBasedRoundedCorner(
    const ClipPaintPropertyNode& clip, PropertyTreeManager::CcEffectType type, const EffectPaintPropertyNode* next_effect)
{
    ...
    if (next_effect && next_effect->BackdropFilter() && &next_effect->LocalTransformSpace() != &clip.LocalTransformSpace())
        return false;
    ...
    for (auto i = pending_clips.size(); i--;) {
        ...
        if (pending_clip.type & CcEffectType::kSyntheticFor2dAxisAlignment) {
            if (should_realize_backdrop_effect) {
                ...
                synthetic_effect.stable_id = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId()).GetStableId();
        ...
        if (pending_clip.type & CcEffectType::kSyntheticForNonTrivialClip) {
            ...
            if (SupportsShaderBasedRoundedCorner(*pending_clip.clip, pending_clip.type, next_effect)) {
                synthetic_effect.mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(SkRRect(pending_clip.clip->PaintClipRect())));
...
void PropertyTreeManager::BuildEffectNodesRecursively(const EffectPaintPropertyNode& next_effect)
{
    ...
    if (real_effect_node_id == cc::kInvalidPropertyNodeId) {
        ...
        PopulateCcEffectNode(effect_node, next_effect, output_clip_id, can_be_shared_element_resource);
    } else {
    ...
    if (has_multiple_groups) {
        if (effect_node.stable_id != cc::EffectNode::INVALID_STABLE_ID) {
            ...
            effect_node.stable_id = CompositorElementIdFromUniqueObjectId(NewUniqueObjectId()).GetStableId();
        }
    } else {
...
void PropertyTreeManager::PopulateCcEffectNode(
    cc::EffectNode& effect_node, const EffectPaintPropertyNode& effect, int output_clip_id, bool can_be_shared_element_resource)
{
    ...
    if (can_be_shared_element_resource) {
        effect_node.document_transition_shared_element_id = effect.DocumentTransitionSharedElementId();
...
void PropertyTreeManager::UpdateConditionalRenderSurfaceReasons(const cc::LayerList& layers)
{
    ...
    // effect_layer_counts.
    for (int id = static_cast<int>(effect_tree_.size() - 1); id > cc::kSecondaryRootPropertyNodeId; id--) {
