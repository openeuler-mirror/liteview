...
void ChunkToLayerMapper::SwitchToChunk(const PaintChunk& chunk)
{
    ...
    const auto& new_chunk_state = chunk.properties.GetPropertyTreeState().Unalias();
    if (new_chunk_state == chunk_state_)
        return;

    if (new_chunk_state == layer_state_) {
        has_filter_that_moves_pixels_ = false;
        translation_2d_or_matrix_ = GeometryMapper::Translation2DOrMatrix(-layer_offset_);
    ...
    if (&new_chunk_state.Transform() != &chunk_state_.Transform()) {
        translation_2d_or_matrix_ = GeometryMapper::SourceToDestinationProjection(new_chunk_state.Transform(), layer_state_.Transform());
        translation_2d_or_matrix_.PostTranslate(-layer_offset_.x(), -layer_offset_.y());
    }
...
gfx::Rect ChunkToLayerMapper::MapUsingGeometryMapper(const gfx::Rect& rect) const
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(chunk_state_, layer_state_, visual_rect);
