...
public:
    ConversionContext(const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset, cc::DisplayItemList& cc_list)
...
private:
    ...
    void ApplyTransform(const TransformPaintPropertyNode& target_transform)
    {
        if (&target_transform == current_transform_)
            return;
        const auto& translation_2d_or_matrix = GetTranslation2DOrMatrix(target_transform);
    ...
    GeometryMapper::Translation2DOrMatrix GetTranslation2DOrMatrix(const TransformPaintPropertyNode& target_transform) const
    {
    ...
    void StartClip(const FloatRoundedRect& combined_clip_rect, const ClipPaintPropertyNode& lowest_combined_clip_node);
    ...
    struct StateEntry {
        ...
        explicit StateEntry(PairedType type, const TransformPaintPropertyNode* transform, const ClipPaintPropertyNode* clip,
            const EffectPaintPropertyNode* effect, const TransformPaintPropertyNode* previous_transform)
            : transform(transform)
            , clip(clip)
            , effect(effect)
...
static bool CombineClip(const ClipPaintPropertyNode& clip, FloatRoundedRect& combined_clip_rect)
{
    ...
    const auto& parent_transform_space = clip.UnaliasedParent()->LocalTransformSpace().Unalias();
    if (&transform_space != &parent_transform_space && (transform_space.Parent() != &parent_transform_space || !transform_space.IsIdentity()))
        return false;
...
void ConversionContext::StartClip(const FloatRoundedRect& combined_clip_rect, const ClipPaintPropertyNode& lowest_combined_clip_node)
{
    if (combined_clip_rect.Rect() == gfx::RectF(LayoutRect::InfiniteIntRect())) {
        ...
        if (combined_clip_rect.IsRounded()) {
            cc_list_.push<cc::ClipRRectOp>(SkRRect(combined_clip_rect), SkClipOp::kIntersect, antialias);
        } else {
            cc_list_.push<cc::ClipRectOp>(gfx::RectFToSkRect(combined_clip_rect.Rect()), SkClipOp::kIntersect, antialias);
        }
        if (const auto& clip_path = lowest_combined_clip_node.ClipPath()) {
            cc_list_.push<cc::ClipPathOp>(clip_path->GetSkPath(), SkClipOp::kIntersect, antialias);
        }
        cc_list_.EndPaintOfPairedBegin();
...
void ConversionContext::SwitchToEffect(const EffectPaintPropertyNode& target_effect)
{
...
#if DCHECK_IS_ON()
        state_stack_.back().has_pre_cap_effect_hierarchy_issue = has_effect_hierarchy_issue;
...
void ConversionContext::StartEffect(const EffectPaintPropertyNode& effect)
{
    ...
    if (!has_filter) {
        ...
        if (has_other_effects) {
            ...
            save_layer_id = cc_list_.push<cc::SaveLayerAlphaOp>(nullptr, static_cast<float>(alpha / 255.0f));
        }
    } else {
        ...
        filter_flags.setImageFilter(cc::RenderSurfaceFilters::BuildImageFilter(effect.Filter().AsCcFilterOperations(), empty));
    ...
    if (effect.Filter().HasReferenceFilter()) {
        ...
        cc_list_.StartPaint();
        cc_list_.EndPaintOfUnpaired(chunk_to_layer_mapper_.MapVisualRect(gfx::ToEnclosingRect(filtered_bounds)));
    }
}

void ConversionContext::UpdateEffectBounds(const gfx::RectF& bounds, const TransformPaintPropertyNode& transform)
{
...
void ConversionContext::EndEffect()
{
    ...
    if (current_effect_->Filter().IsEmpty()) {
        if (!bounds.IsEmpty()) {
            cc_list_.UpdateSaveLayerBounds(bounds_info.save_layer_id, gfx::RectFToSkRect(bounds));
        }
    } else {
...
void ConversionContext::SwitchToTransform(const TransformPaintPropertyNode& target_transform)
{
    if (&target_transform == current_transform_)
        return;
...
void ConversionContext::Convert(const PaintChunkSubset& chunks)
{
    for (auto it = chunks.begin(); it != chunks.end(); ++it) {
        ...
        for (const auto& item : it.DisplayItems()) {
            ...
            if ((!record || record->size() == 0) && &chunk_state.Effect() == &EffectPaintPropertyNode::Root()) {
                continue;
            }
            ...
            cc_list_.EndPaintOfUnpaired(chunk_to_layer_mapper_.MapVisualRect(item.VisualRect()));
        }
        ...
        UpdateEffectBounds(gfx::RectF(chunk.drawable_bounds), chunk_state.Transform().Unalias());
    }
}
...
void PaintChunksToCcLayer::ConvertInto(
    const PaintChunkSubset& chunks, const PropertyTreeState& layer_state, const gfx::Vector2dF& layer_offset, cc::DisplayItemList& cc_list)
{
    ConversionContext(layer_state, layer_offset, cc_list).Convert(chunks);
}

scoped_refptr<cc::DisplayItemList> PaintChunksToCcLayer::Convert(const PaintChunkSubset& paint_chunks, const PropertyTreeState& layer_state,
    const gfx::Vector2dF& layer_offset, cc::DisplayItemList::UsageHint hint, RasterUnderInvalidationCheckingParams* under_invalidation_checking_params)
{
    ...
    if (under_invalidation_checking_params) {
        ...
        auto list_clone = base::MakeRefCounted<cc::DisplayItemList>(cc::DisplayItemList::kToBeReleasedAsPaintOpBuffer);
...
static void UpdateTouchActionRegion(const HitTestData& hit_test_data, const PropertyTreeState& layer_state, const PropertyTreeState& chunk_state,
    const gfx::Vector2dF& layer_offset, cc::TouchActionRegion& touch_action_region)
{
    ...
    for (const auto* scroll_node = chunk_state.Transform().ScrollNode(); scroll_node; scroll_node = scroll_node->Parent()) {
    ...
    for (const auto& touch_action_rect : hit_test_data.touch_action_rects) {
        ...
        if (!GeometryMapper::LocalToAncestorVisualRect(chunk_state, layer_state, rect)) {
            continue;
        }
...
static void UpdateWheelEventRegion(const HitTestData& hit_test_data, const PropertyTreeState& layer_state, const PropertyTreeState& chunk_state,
    const gfx::Vector2dF& layer_offset, cc::Region& wheel_event_region)
{
...
static void UpdateNonFastScrollableRegion(cc::Layer& layer, const HitTestData& hit_test_data, const PropertyTreeState& layer_state,
    const PropertyTreeState& chunk_state, const gfx::Vector2dF& layer_offset, cc::Region& non_fast_scrollable_region)
{
...
static void UpdateTouchActionWheelEventHandlerAndNonFastScrollableRegions(
    cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks)
{
    ...
    for (const auto& chunk : chunks) {
        ...
        UpdateTouchActionRegion(*chunk.hit_test_data, layer_state, chunk_state, layer_offset, touch_action_region);
        UpdateWheelEventRegion(*chunk.hit_test_data, layer_state, chunk_state, layer_offset, wheel_event_region);
        UpdateNonFastScrollableRegion(layer, *chunk.hit_test_data, layer_state, chunk_state, layer_offset, non_fast_scrollable_region);
    }
...
static void UpdateRegionCaptureData(cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks)
{
    ...
    for (const PaintChunk& chunk : chunks) {
        ...
        const PropertyTreeState chunk_state = chunk.properties.GetPropertyTreeState().Unalias();
        for (const std::pair<RegionCaptureCropId, gfx::Rect>& pair : *chunk.region_capture_data) {
...
static gfx::Point MapSelectionBoundPoint(
    const gfx::Point& point, const PropertyTreeState& layer_state, const PropertyTreeState& chunk_state, const gfx::Vector2dF& layer_offset)
{
    gfx::PointF mapped_point = GeometryMapper::SourceToDestinationProjection(chunk_state.Transform(), layer_state.Transform()).MapPoint(gfx::PointF(point));

    mapped_point -= layer_offset;
    return gfx::ToRoundedPoint(mapped_point);
}

static cc::LayerSelectionBound ConvertPaintedSelectionBoundToLayerSelectionBound(
    const PaintedSelectionBound& bound, const PropertyTreeState& layer_state, const PropertyTreeState& chunk_state, const gfx::Vector2dF& layer_offset)
{
    cc::LayerSelectionBound layer_bound;
    layer_bound.type = bound.type;
    layer_bound.hidden = bound.hidden;
    layer_bound.edge_start = MapSelectionBoundPoint(bound.edge_start, layer_state, chunk_state, layer_offset);
    layer_bound.edge_end = MapSelectionBoundPoint(bound.edge_end, layer_state, chunk_state, layer_offset);
    return layer_bound;
}

bool PaintChunksToCcLayer::UpdateLayerSelection(
    cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks, cc::LayerSelection& layer_selection)
{
    gfx::Vector2dF layer_offset = layer.offset_to_transform_parent();
    ...
    for (const auto& chunk : chunks) {
        ...
        if (chunk.layer_selection_data->start) {
            const PaintedSelectionBound& bound = chunk.layer_selection_data->start.value();
            layer_selection.start = ConvertPaintedSelectionBoundToLayerSelectionBound(bound, layer_state, chunk_state, layer_offset);
            layer_selection.start.layer_id = layer.id();
        }

        if (chunk.layer_selection_data->end) {
            const PaintedSelectionBound& bound = chunk.layer_selection_data->end.value();
            layer_selection.end = ConvertPaintedSelectionBoundToLayerSelectionBound(bound, layer_state, chunk_state, layer_offset);
...
void PaintChunksToCcLayer::UpdateLayerProperties(cc::Layer& layer, const PropertyTreeState& layer_state, const PaintChunkSubset& chunks)
{
    UpdateTouchActionWheelEventHandlerAndNonFastScrollableRegions(layer, layer_state, chunks);
