...
public:
    ...
    {
        return MergeInternal(guest, guest.property_tree_state_.GetPropertyTreeState(), prefers_lcd_text, /*dry_run*/ false);
    }
    ...
    // current state).
    bool CanMerge(const PendingLayer& guest, const PropertyTreeState& guest_state, bool prefers_lcd_text = false) const
    {
        return const_cast<PendingLayer*>(this)->MergeInternal(guest, guest_state, prefers_lcd_text, /*dry_run*/ true);
    }
    ...
    void UpdateCompositedLayerForRepaint(scoped_refptr<const PaintArtifact> repainted_artifact, cc::LayerSelection&);

    SkColor4f ComputeBackgroundColor() const;

private:
    PendingLayer(const PaintChunkSubset&, const PaintChunk& first_chunk, wtf_size_t first_chunk_index_in_paint_artifact);
    gfx::RectF MapRectKnownToBeOpaque(const PropertyTreeState&) const;
    bool MergeInternal(const PendingLayer& guest, const PropertyTreeState& guest_state, bool prefers_lcd_text, bool dry_run);
