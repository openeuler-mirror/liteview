...
int LayersAsJSON::AddTransformJSON(const TransformPaintPropertyNode& transform)
{
    ...
    if (!transform.IsIdentity()) {
        transform_json->SetArray("transform", TransformAsJSONArray(transform.SlowMatrix()));
    }

    if (!transform.IsIdentityOr2DTranslation() && !transform.Matrix().IsIdentityOrTranslation()) {
...
void LayersAsJSON::AddLayer(const cc::Layer& layer, const TransformPaintPropertyNode& transform, const LayerAsJSONClient* json_client)
{
    if (!(flags_ & kLayerTreeIncludesAllLayers) && !layer.draws_content()) {
        std::string debug_name = layer.DebugName();
        if (debug_name == "LayoutNGView #document" || debug_name == "LayoutView #document" || debug_name == "Inner Viewport Scroll Layer"
