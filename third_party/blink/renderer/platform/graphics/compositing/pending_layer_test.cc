...
TEST(PendingLayerTest, Merge)
{
    ...
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 30, 40)).RectKnownToBeOpaque(gfx::Rect(0, 0, 30, 40));
    artifact.Chunk().Bounds(gfx::Rect(10, 20, 30, 40)).RectKnownToBeOpaque(gfx::Rect(10, 20, 30, 40));
    artifact.Chunk().Bounds(gfx::Rect(-5, -25, 20, 20)).RectKnownToBeOpaque(gfx::Rect(-5, -25, 20, 20));
...
{
    TestPaintArtifact artifact;
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 3, 4)).RectKnownToBeOpaque(gfx::Rect(0, 0, 3, 4));
    artifact.Chunk().Bounds(gfx::Rect(20, 20, 3, 4)).RectKnownToBeOpaque(gfx::Rect(20, 20, 3, 4));
    PaintChunkSubset chunks(artifact.Build());
...
TEST(PendingLayerTest, DontMergeSparse)
{
    TestPaintArtifact artifact;
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 30, 40)).RectKnownToBeOpaque(gfx::Rect(0, 0, 30, 40));
    artifact.Chunk().Bounds(gfx::Rect(200, 200, 30, 40)).RectKnownToBeOpaque(gfx::Rect(200, 200, 30, 40));
    PaintChunkSubset chunks(artifact.Build());
...
TEST(PendingLayerTest, KnownOpaque)
{
    TestPaintArtifact artifact;
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 30, 40));
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 25, 35)).RectKnownToBeOpaque(gfx::Rect(0, 0, 25, 35));
    artifact.Chunk().Bounds(gfx::Rect(0, 0, 50, 60)).RectKnownToBeOpaque(gfx::Rect(0, 0, 50, 60));
    PaintChunkSubset chunks(artifact.Build());
...
class PendingLayerTextOpaquenessTest : public testing::Test, public testing::WithParamInterface<bool> {
protected:
    bool PrefersLCDText() const
    {
        return GetParam();
    }
};

INSTANTIATE_TEST_SUITE_P(All, PendingLayerTextOpaquenessTest, ::testing::Values(false, true));
