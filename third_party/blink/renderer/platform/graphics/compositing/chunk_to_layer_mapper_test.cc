...
protected:
    static PaintChunk Chunk(const PropertyTreeState& state)
    {
        ...
        DEFINE_STATIC_LOCAL(absl::optional<PaintChunk::Id>, id, (PaintChunk::Id(fake_client->Id(), DisplayItem::kDrawingFirst)));
...
TEST_F(ChunkToLayerMapperTest, SwitchToSiblingEffect)
{
    ...
    ChunkToLayerMapper mapper(chunk1.properties.GetPropertyTreeState().Unalias(), gfx::Vector2dF(10, 20));
    mapper.SwitchToChunk(chunk2);
    EXPECT_FALSE(HasFilterThatMovesPixels(mapper));
}
