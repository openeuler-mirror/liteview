...
void SetTransform(PaintChunk& chunk, const TransformPaintPropertyNode& transform)
{
...
public:
    MOCK_METHOD3(DidCompositorScroll, void(CompositorElementId, const gfx::PointF&, const absl::optional<cc::TargetSnapAreaElementIds>&));
...
protected:
    ...
    void SetUp() override
    {
        ...
        paint_artifact_compositor_ = std::make_unique<PaintArtifactCompositor>(scroll_callbacks_.GetWeakPtr());
    ...
    void Update(scoped_refptr<const PaintArtifact> artifact, const ViewportProperties& viewport_properties = ViewportProperties(),
        const WTF::Vector<const TransformPaintPropertyNode*>& scroll_translation_nodes = {})
    {
        paint_artifact_compositor_->SetNeedsUpdate(PaintArtifactCompositorUpdateReason::kTest);
    ...
    void CreateScrollableChunk(TestPaintArtifact& artifact, const RefCountedPropertyTreeState& scroll_state)
    {
        artifact.Chunk(*scroll_state.Transform().Parent(), *scroll_state.Clip().Parent(), scroll_state.Effect())
            .ScrollHitTest(scroll_state.Transform().ScrollNode()->ContainerRect(), &scroll_state.Transform());
    }
...
TEST_P(PaintArtifactCompositorTest, FlattensInheritedTransform)
{
    for (bool transform_is_flattened : { true, false }) {
        ...
        transform3_state.flags.flattens_inherited_transform = transform_is_flattened;
...
{
    auto transform1 = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
    auto clip1 = CreateClip(c0(), *transform1, FloatRoundedRect(100, 100, 700, 700));

    auto transform2 = CreateTransform(*transform1, TransformationMatrix(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
...
{
    auto t1 = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
...
TEST_P(PaintArtifactCompositorTest, EffectTreeConversionWithAlias)
{
    ...
    EXPECT_EQ(real_effect1->GetCompositorElementId().GetStableId(), converted_effect1.stable_id);
...
static RefCountedPropertyTreeState ScrollState1(const PropertyTreeState& parent_state = PropertyTreeState::Root(),
...
static RefCountedPropertyTreeState ScrollState2(const PropertyTreeState& parent_state = PropertyTreeState::Root(),
    CompositingReasons compositing_reasons = CompositingReason::kOverflowScrolling, MainThreadScrollingReasons main_thread_reasons = kNotScrollingOnMain)
{
...
static void CheckCcScrollNode(const ScrollPaintPropertyNode& blink_scroll, const cc::ScrollNode& cc_scroll)
{
    ...
    EXPECT_EQ(blink_scroll.GetMainThreadScrollingReasons(), cc_scroll.main_thread_scrolling_reasons);
}
...
TEST_P(PaintArtifactCompositorTest, OneScrollNodeComposited)
{
    ...
    artifact.Chunk(scroll_state).RectDrawing(gfx::Rect(-110, 12, 170, 19), Color::kWhite);
...
{
    auto scroll_state = ScrollState1();
    auto transform = CreateTransform(scroll_state.Transform(), TransformationMatrix(), gfx::Point3F(), CompositingReason::kWillChangeTransform);
...
TEST_P(PaintArtifactCompositorTest, NestedScrollNodes)
{
    ...
    artifact.Chunk(scroll_state_a).RectDrawing(gfx::Rect(7, 11, 13, 17), Color::kWhite);
    CreateScrollableChunk(artifact, scroll_state_b);
    artifact.Chunk(scroll_state_b).RectDrawing(gfx::Rect(1, 2, 3, 5), Color::kWhite);
    Update(artifact.Build());
...
TEST_P(PaintArtifactCompositorTest, ScrollHitTestLayerOrder)
{
    ...
    TestPaintArtifact artifact;
    artifact.Chunk(scroll_state).RectDrawing(gfx::Rect(0, 0, 100, 100), Color::kWhite);
    CreateScrollableChunk(artifact, scroll_state);
    artifact.Chunk(*transform, scroll_state.Clip(), scroll_state.Effect()).RectDrawing(gfx::Rect(0, 0, 50, 50), Color::kBlack);
    Update(artifact.Build());
    ...
    EXPECT_LT(LayerIndex(NonScrollableLayerAt(0)), LayerIndex(ScrollableLayerAt(0)));
    const cc::ScrollTree& scroll_tree = GetPropertyTrees().scroll_tree();
    auto* scroll_node = scroll_tree.Node(ScrollableLayerAt(0)->scroll_tree_index());
    ASSERT_EQ(scroll.GetCompositorElementId(), scroll_node->element_id);
    EXPECT_EQ(scroll.GetCompositorElementId(), ScrollableLayerAt(0)->element_id());
    EXPECT_TRUE(ScrollableLayerAt(0)->HitTestable());

    // The second content layer should appear after the first.
    EXPECT_LT(LayerIndex(ScrollableLayerAt(0)), LayerIndex(NonScrollableLayerAt(1)));
...
TEST_P(PaintArtifactCompositorTest, NestedScrollableLayerOrder)
{
    ...
    artifact.Chunk(scroll_state_2).RectDrawing(gfx::Rect(0, 0, 50, 50), Color::kWhite);
    Update(artifact.Build());
    ...
    auto* scroll_1_node = scroll_tree.Node(ScrollableLayerAt(0)->scroll_tree_index());
    ASSERT_EQ(scroll_1.GetCompositorElementId(), scroll_1_node->element_id);
    auto* scroll_1_clip_node = clip_tree.Node(ScrollableLayerAt(0)->clip_tree_index());
    // The scroll is not under clip_1.
    EXPECT_EQ(gfx::RectF(0, 0, 0, 0), scroll_1_clip_node->clip);

    auto* scroll_2_node = scroll_tree.Node(ScrollableLayerAt(1)->scroll_tree_index());
    ASSERT_EQ(scroll_2.GetCompositorElementId(), scroll_2_node->element_id);
    auto* scroll_2_clip_node = clip_tree.Node(ScrollableLayerAt(1)->clip_tree_index());
    ...
    EXPECT_LT(LayerIndex(ScrollableLayerAt(1)), LayerIndex(NonScrollableLayerAt(0)));
...
TEST_P(PaintArtifactCompositorTest, AncestorScrollNodes)
{
    ...
    auto& scroll_a = *scroll_state_a.Transform().ScrollNode();
    auto scroll_state_b
        = ScrollState2(scroll_state_a.GetPropertyTreeState(), CompositingReason::kNone, cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText);
...
{
    auto transform = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::k3DTransform);
...
TEST_P(PaintArtifactCompositorTest, MergeOpacity)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        // Transform is applied to this PaintChunk.
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color(Color::kBlack).CombineWithAlpha(opacity)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, MergeNestedWithAlias)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        // Transform is applied to this PaintChunk.
        rects_with_color.push_back(RectWithColor(gfx::RectF(60, 70, 50, 60), Color(Color::kBlack).CombineWithAlpha(opacity)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, EffectPushedUp)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        Vector<RectWithColor> rects_with_color;
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 300, 400), Color(Color::kBlack).CombineWithAlpha(opacity)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, EffectAndClipPushedUp)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        ...
        // of (20, 25) occurs.
        rects_with_color.push_back(RectWithColor(gfx::RectF(30, 45, 50, 60), Color(Color::kBlack).CombineWithAlpha(opacity)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
TEST_P(PaintArtifactCompositorTest, ClipAndEffectNoTransform)
{
    ...
    ASSERT_EQ(1u, LayerCount());
    {
        Vector<RectWithColor> rects_with_color;
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 100, 100), Color::kWhite));
        rects_with_color.push_back(RectWithColor(gfx::RectF(10, 20, 50, 60), Color(Color::kBlack).CombineWithAlpha(opacity)));
        rects_with_color.push_back(RectWithColor(gfx::RectF(0, 0, 200, 300), Color::kGray));
...
scoped_refptr<TransformPaintPropertyNode> CreateSampleTransformNodeWithElementId()
{
...
TEST_P(PaintArtifactCompositorTest, NonContiguousEffectWithElementId)
{
    ...
    EXPECT_EQ(stable_id, GetPropertyTrees().effect_tree().Node(LayerAt(0)->effect_tree_index())->stable_id);
    EXPECT_EQ(1, LayerAt(1)->effect_tree_index());
    EXPECT_EQ(3, LayerAt(2)->effect_tree_index());
    EXPECT_NE(stable_id, GetPropertyTrees().effect_tree().Node(LayerAt(2)->effect_tree_index())->stable_id);
}
...
{
    auto masked = CreateOpacityEffect(e0(), 1.0, CompositingReason::kOpacityWithCompositedDescendants);
...
TEST_P(PaintArtifactCompositorTest, CompositedMaskOneChild)
{
    ...
    masking_state.output_clip = &c0();
    masking_state.blend_mode = SkBlendMode::kDstIn;
    masking_state.direct_compositing_reasons = CompositingReason::kMaskWithCompositedDescendants;
    auto masking = EffectPaintPropertyNode::Create(*masked, std::move(masking_state));
...
TEST_P(PaintArtifactCompositorTest, CompositedMaskTwoChildren)
{
    ...
    auto child_of_masked = CreateOpacityEffect(*masking, 1.0, CompositingReason::kOpacityWithCompositedDescendants);
...
{
    auto masked = CreateOpacityEffect(e0(), 1.0, CompositingReason::kOpacityWithCompositedDescendants);
    auto transform1 = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto transform2 = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::k3DTransform);
    EffectPaintPropertyNode::State masking_state;
    masking_state.local_transform_space = &t0();
    masking_state.output_clip = &c0();
    masking_state.blend_mode = SkBlendMode::kXor;
...
{
    auto parent_effect = CreateOpacityEffect(e0(), 1.0, CompositingReason::kOpacityWithCompositedDescendants);
...
TEST_P(PaintArtifactCompositorTest, ReuseSyntheticClip)
{
    ...
    uint64_t old_stable_id = GetPropertyTrees().effect_tree().Node(content0->effect_tree_index())->stable_id;
    ...
    EXPECT_EQ(GetPropertyTrees().effect_tree().Node(content1->effect_tree_index())->stable_id, old_stable_id);
    ...
    // not be reused.
    EXPECT_NE(GetPropertyTrees().effect_tree().Node(content2->effect_tree_index())->stable_id, old_stable_id);
}
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipIndirectlyCompositedClipPath)
{
    ...
    auto e1 = CreateOpacityEffect(e0(), t0(), c1.get(), 1, CompositingReason::kWillChangeOpacity);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipDelegateBackdropFilter)
{
    ...
    EXPECT_EQ(gfx::RRectF(), mask_isolation_1.mask_filter_info.rounded_corner_bounds());

    EXPECT_EQ(t1_id, clip_mask1->transform_tree_index());
    EXPECT_EQ(c2_id, clip_mask1->clip_tree_index());
    const cc::EffectNode& mask = *GetPropertyTrees().effect_tree().Node(clip_mask1->effect_tree_index());
    ASSERT_EQ(mask_isolation_1_id, mask.parent_id);
    EXPECT_EQ(SkBlendMode::kDstIn, mask.blend_mode);
    EXPECT_TRUE(static_cast<const cc::PictureLayer*>(clip_mask1)->is_backdrop_filter_mask());
    EXPECT_TRUE(clip_mask1->element_id());
    EXPECT_EQ(clip_mask1->element_id(), mask_isolation_1.backdrop_mask_element_id);
...
TEST_P(PaintArtifactCompositorTest, SynthesizedClipMultipleNonBackdropEffects)
{
    ...
    auto e1 = CreateOpacityEffect(e0(), t0(), c2.get(), 0.5, CompositingReason::kWillChangeOpacity);
    auto e2 = CreateOpacityEffect(e0(), t0(), c1.get(), 0.75, CompositingReason::kWillChangeOpacity);
...
TEST_P(PaintArtifactCompositorTest, OpacityIndirectlyAffectingTwoLayers)
{
    ...
    auto child_composited_transform = CreateTransform(t0(), TransformationMatrix(), gfx::Point3F(), CompositingReason::k3DTransform);
    auto grandchild_composited_transform
        = CreateTransform(*child_composited_transform, TransformationMatrix(), gfx::Point3F(), CompositingReason::k3DTransform);
...
TEST_P(PaintArtifactCompositorTest, Non2dAxisAlignedClipUnderLaterRenderSurface)
{
    ...
    // to make it succeed.
    ASSERT_TRUE(GeometryMapper::SourceToDestinationProjection(t0(), *rotate2).Matrix().Preserves2dAxisAlignment());
...
static TransformPaintPropertyNode::State Transform3dState(TransformPaintPropertyNode::TransformAndOrigin&& transform)
{
...
TEST_P(PaintArtifactCompositorTest, AddNonCompositedScrollNodes)
{
    ...
    ASSERT_TRUE(cc::MainThreadScrollingReason::HasNonCompositedScrollReasons(main_thread_scrolling_reason));
