...
std::unique_ptr<CanvasResourceProvider> CanvasResourceHost::ReplaceResourceProvider(std::unique_ptr<CanvasResourceProvider> new_resource_provider)
{
    ...
    if (resource_provider_) {
        ...
        resource_provider_->SetRestoreClipStackCallback(base::BindRepeating(&CanvasResourceHost::InitializeForRecording, base::Unretained(this)));
    }
    if (old_resource_provider) {
        old_resource_provider->SetCanvasResourceHost(nullptr);
        old_resource_provider->SetRestoreClipStackCallback(CanvasResourceProvider::RestoreMatrixClipStackCb());
    }
