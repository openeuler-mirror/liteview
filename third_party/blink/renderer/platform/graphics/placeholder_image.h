...
public:
    static scoped_refptr<PlaceholderImage> Create(ImageObserver* observer, const gfx::Size& size, int64_t original_resource_size)
    {
        return base::AdoptRef(new PlaceholderImage(observer, size, original_resource_size));
    }
    ...
    const String& GetTextForTesting() const
    {
        return text_;
    }
...
private:
    PlaceholderImage(ImageObserver*, const gfx::Size&, int64_t original_resource_size);

    bool CurrentFrameHasSingleSecurityOrigin() const override;

    bool CurrentFrameKnownToBeOpaque() override;

    void DrawPattern(GraphicsContext&, const cc::PaintFlags&, const gfx::RectF& dest_rect, const ImageTilingInfo& tiling_info,
