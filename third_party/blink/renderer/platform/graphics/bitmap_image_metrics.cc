...
void BitmapImageMetrics::CountDecodedImageFrameTime(const String& type, base::TimeDelta elapsed, uint64_t original_frame_rect_area, bool first)
{
...
void BitmapImageMetrics::CountDecodedImageDensity(const String& type, int image_min_side, uint64_t density_centi_bpp, size_t image_size_bytes)
{
    ...
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, webp_density_histogram, ("Blink.DecodedImage.WebPDensity.KiBWeighted", 1, 1000, 100));
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, avif_density_histogram, ("Blink.DecodedImage.AvifDensity.KiBWeighted", 1, 1000, 100));
...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(CustomCountHistogram, image_area_histogram,
        ("Blink.ImageDecoders.Jpeg.Area", kImageAreaHistogramMin, kImageAreaHistogramMax, kImageAreaHistogramBucketCount));
    // A base::HistogramBase::Sample may not fit |size.Area()|. Hence the use of
    // saturated_cast.
    image_area_histogram.Count(base::saturated_cast<base::HistogramBase::Sample>(size.Area64()));
}
