...
CanvasResourceDispatcher::CanvasResourceDispatcher(CanvasResourceDispatcherClient* client,
    ...
    , client_(client)
    ,
    // animation_power_mode_voter_(power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.Animation.Canvas")),
    ...
    scoped_refptr<CanvasResource>&& canvas_resource, base::TimeTicks commit_start_time, const SkIRect& damage_rect, bool needs_vertical_flip, bool is_opaque)
{
    ...
    sink_->SubmitCompositorFrameSync(parent_local_surface_id_allocator_.GetCurrentLocalSurfaceId(), std::move(frame), absl::nullopt, 0, &resources);
    ...
    scoped_refptr<CanvasResource>&& canvas_resource, base::TimeTicks commit_start_time, const SkIRect& damage_rect, bool needs_vertical_flip, bool is_opaque)
{
    ...
    pending_compositor_frames_++;
    sink_->SubmitCompositorFrame(parent_local_surface_id_allocator_.GetCurrentLocalSurfaceId(), std::move(frame), absl::nullopt, 0);
}
    ...
    bool needs_vertical_flip, bool is_opaque, viz::CompositorFrame* frame)
{
    ...
    sqs->SetAll(gfx::Transform(), bounds, bounds, gfx::MaskFilterInfo(), absl::nullopt, is_opaque, 1.f, SkBlendMode::kSrcOver, 0);
    ...
    canvas_resource->PrepareTransferableResource(&resource, &frame_resource->release_callback, kVerifiedSyncToken);
    ...
    quad->SetAll(sqs, bounds, bounds, needs_blending, resource_id, canvas_resource_size, kPremultipliedAlpha, uv_top_left, uv_bottom_right,
        SkColors::kTransparent, vertex_opacity, yflipped, nearest_neighbor, /*secure_output=*/false, gfx::ProtectedVideoType::kClear);
...
void CanvasResourceDispatcher::OnBeginFrame(const viz::BeginFrameArgs& begin_frame_args, const WTF::HashMap<uint32_t, viz::FrameTimingDetails>&)
{
...
void CanvasResourceDispatcher::DidAllocateSharedBitmap(base::ReadOnlySharedMemoryRegion region, const gpu::Mailbox& id)
{
    if (sink_)
...
void CanvasResourceDispatcher::SetPlaceholderCanvasDispatcher(int placeholder_canvas_id)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> dispatcher_task_runner = Thread::Current()->GetDeprecatedTaskRunner();
    ...
    if (IsMainThread()) {
        UpdatePlaceholderDispatcher(GetWeakPtr(), dispatcher_task_runner, placeholder_canvas_id);
    } else {
        PostCrossThreadTask(*agent_group_scheduler_compositor_task_runner_, FROM_HERE,
            CrossThreadBindOnce(UpdatePlaceholderDispatcher, GetWeakPtr(), std::move(dispatcher_task_runner), placeholder_canvas_id));
    }
}
