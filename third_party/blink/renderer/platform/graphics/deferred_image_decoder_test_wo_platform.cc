...
{
    return SkImage::MakeFromGenerator(std::make_unique<SkiaPaintImageGenerator>(decoder->CreateGenerator(), index, cc::PaintImage::kDefaultGeneratorClientId));
}
...
static void MixImages(const char* file_name, size_t bytes_for_first_frame, size_t later_frame)
{
    ...
    scoped_refptr<SharedBuffer> partial_file = SharedBuffer::Create(file.data(), bytes_for_first_frame);
    std::unique_ptr<DeferredImageDecoder> decoder
        = DeferredImageDecoder::Create(partial_file, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::Ignore());
    ASSERT_NE(decoder, nullptr);
    sk_sp<SkImage> partial_image = CreateFrameAtIndex(decoder.get(), 0);

    scoped_refptr<SharedBuffer> almost_complete_file = SharedBuffer::Create(file.data(), file.size() - 1);
...
TEST(DeferredImageDecoderTestWoPlatform, fragmentedSignature)
{
    ...
    for (size_t i = 0; i < SK_ARRAY_COUNT(test_files); ++i) {
        ...
        EXPECT_EQ(nullptr, DeferredImageDecoder::Create(buffer, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::Ignore()));
        ...
        std::unique_ptr<DeferredImageDecoder> decoder = DeferredImageDecoder::Create(buffer, false, ImageDecoder::kAlphaPremultiplied, ColorBehavior::Ignore());
        ASSERT_NE(decoder, nullptr);
