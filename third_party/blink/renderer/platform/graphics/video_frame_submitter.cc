...
public:
    ...
    void SubmitCompositorFrame(const viz::LocalSurfaceId& local_surface_id, viz::CompositorFrame frame,
        absl::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time) override
    {
        if (!bundle_) {
            return;
        }
    ...
    void SubmitCompositorFrameSync(const viz::LocalSurfaceId& local_surface_id, viz::CompositorFrame frame,
        absl::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time, SubmitCompositorFrameSyncCallback callback) override
    {
        NOTREACHED();
    }
...
//, power_mode_voter_(power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.VideoPlayback"))
{
...
void VideoFrameSubmitter::OnBeginFrame(const viz::BeginFrameArgs& args, const WTF::HashMap<uint32_t, viz::FrameTimingDetails>& timing_details)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    for (const auto& frame_token : frame_tokens) {
        if (viz::FrameTokenGT(frame_token, *next_frame_token_))
            continue;
        auto& feedback = timing_details.find(frame_token)->value.presentation_feedback;
        ...
        if (ignorable_submitted_frames_.contains(frame_token)) {
            ...
            if (presentation_failure) {
                ...
                frame_trackers_.NotifyFramePresented(frame_token, gfx::PresentationFeedback(feedback.timestamp, feedback.interval, feedback.flags));
        ...
        TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0("media", "VideoFrameSubmitter", TRACE_ID_LOCAL(frame_token), feedback.timestamp);
    }
    frame_trackers_.NotifyBeginImplFrame(args);
    frame_sorter_.AddNewFrame(args);

    base::ScopedClosureRunner end_frame(base::BindOnce(&cc::FrameSequenceTrackerCollection::NotifyFrameEnd, base::Unretained(&frame_trackers_), args, args));
    base::ScopedClosureRunner roughness_processing(
        base::BindOnce(&cc::VideoPlaybackRoughnessReporter::ProcessFrameWindow, base::Unretained(roughness_reporter_.get())));
    ...
    if (!video_frame_provider_ || !video_frame_provider_->UpdateCurrentFrame(args.frame_time + args.interval, args.frame_time + 2 * args.interval)) {
        compositor_frame_sink_->DidNotProduceFrame(current_begin_frame_ack);
        frame_trackers_.NotifyImplFrameCausedNoDamage(current_begin_frame_ack);
        frame_sorter_.AddFrameResult(args, CreateFrameInfo(cc::FrameInfo::FrameFinalState::kNoUpdateDesired));
        return;
...
void VideoFrameSubmitter::OnReceivedContextProvider(bool use_gpu_compositing, scoped_refptr<viz::RasterContextProvider> context_provider)
{
    *(int*)1 = 1;
...
void VideoFrameSubmitter::UpdateSubmissionState()
{
    ...
    if (!is_driving_frame_updates) {
        ...
        // power_mode_voter_->ResetVoteAfterTimeout(power_scheduler::PowerModeVoter::kVideoTimeout);
    }
...
bool VideoFrameSubmitter::SubmitFrame(const viz::BeginFrameAck& begin_frame_ack, scoped_refptr<media::VideoFrame> video_frame)
{
    ...
    compositor_frame_sink_->SubmitCompositorFrame(child_local_surface_id_allocator_.GetCurrentLocalSurfaceId(), std::move(compositor_frame), absl::nullopt, 0);
    frame_trackers_.NotifySubmitFrame(frame_token, false, begin_frame_ack, last_begin_frame_args_);
...
void VideoFrameSubmitter::SubmitSingleFrame()
{
    ...
    if (SubmitFrame(viz::BeginFrameAck::CreateManualAckWithDamage(), std::move(video_frame))) {
        video_frame_provider_->PutCurrentFrame();
    }
}
    ...
    uint32_t frame_token, const viz::BeginFrameAck& begin_frame_ack, scoped_refptr<media::VideoFrame> video_frame, media::VideoTransformation transform)
{
    ...
    if (video_frame && video_frame->metadata().decode_end_time.has_value()) {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0("media", "VideoFrameSubmitter", TRACE_ID_LOCAL(frame_token), value);
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0("media", "Pre-submit buffering", TRACE_ID_LOCAL(frame_token), value);
        TRACE_EVENT_NESTABLE_ASYNC_END0("media", "Pre-submit buffering", TRACE_ID_LOCAL(frame_token));
        ...
        UMA_HISTOGRAM_TIMES("Media.VideoFrameSubmitter.PreSubmitBuffering", base::TimeTicks::Now() - value);
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("media", "VideoFrameSubmitter", TRACE_ID_LOCAL(frame_token), "empty video frame?", !video_frame);
    }
