...
TEST(VideoFrameImageUtilTest, WillCreateAcceleratedImagesFromVideoFrame)
{
    ...
    {
        auto cpu_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::PIXEL_FORMAT_XRGB);
        EXPECT_FALSE(WillCreateAcceleratedImagesFromVideoFrame(cpu_frame.get()));
    }

    // GpuMemoryBuffer frames aren't supported.
    {
        auto cpu_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, media::PIXEL_FORMAT_XRGB);
        EXPECT_FALSE(WillCreateAcceleratedImagesFromVideoFrame(cpu_frame.get()));
    }

    // Single mailbox shared images should be supported on most platforms.
    {
        auto shared_image_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OPAQUE, media::PIXEL_FORMAT_XRGB);
...
TEST(VideoFrameImageUtilTest, CreateImageFromVideoFrameZeroCopy)
{
    ...
    EXPECT_EQ(memcmp(image->GetMailboxHolder().mailbox.name, shared_image_frame->mailbox_holder(0).mailbox.name, sizeof(gpu::Mailbox::Name)), 0);
}
#endif
...
{
    base::test::SingleThreadTaskEnvironment task_environment_;
    auto cpu_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, media::PIXEL_FORMAT_NV12);
    auto image = CreateImageFromVideoFrame(cpu_frame);
...
{
    base::test::SingleThreadTaskEnvironment task_environment_;
    auto cpu_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_OPAQUE, media::PIXEL_FORMAT_NV12);
    auto image = CreateImageFromVideoFrame(cpu_frame);
...
{
    ScopedFakeGpuContext fake_context(/*disable_imagebitmap=*/false);
    auto gmb_frame = CreateTestFrame(kTestSize, gfx::Rect(kTestSize), kTestSize, media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER, media::PIXEL_FORMAT_NV12);
...
TEST(VideoFrameImageUtilTest, CreateAcceleratedImageFromTextureFrame)
{
    ...
    auto texture_frame = media::CreateSharedImageRGBAFrame(fake_context.context_provider(), kTestSize, gfx::Rect(kTestSize), base::DoNothing());
