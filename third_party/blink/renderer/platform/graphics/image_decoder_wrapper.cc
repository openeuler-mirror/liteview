...
public:
    ExternalMemoryAllocator(const SkImageInfo& info, void* pixels, size_t row_bytes)
        : info_(info)
        , pixels_(pixels)
...
ImageDecoderWrapper::ImageDecoderWrapper(ImageFrameGenerator* generator, SegmentReader* data, const SkISize& scaled_size,
    ImageDecoder::AlphaOption alpha_option, ColorBehavior decoder_color_behavior, ImageDecoder::HighBitDepthDecodingOption decoding_option, wtf_size_t index,
    const SkImageInfo& info, void* pixels, size_t row_bytes, bool all_data_received, cc::PaintImage::GeneratorClientId client_id)
...
bool ImageDecoderWrapper::Decode(ImageDecoderFactory* factory, wtf_size_t* frame_count, bool* has_alpha)
{
    ...
    const bool resume_decoding = ImageDecodingStore::Instance().LockDecoder(generator_, scaled_size_, alpha_option_, client_id_, &decoder);
    ...
    DCHECK(!decode_to_external_memory || scaled_size_bitmap.getPixels() == pixels_);
...
std::unique_ptr<ImageDecoder> ImageDecoderWrapper::CreateDecoderWithData(ImageDecoderFactory* factory) const
{
    ...
    return ImageDecoder::Create(data_, all_data_received_, alpha_option_, decoding_option_, decoder_color_behavior_, scaled_size_);
}
