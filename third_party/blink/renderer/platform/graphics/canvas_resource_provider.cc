...
bool IsImageFromGpuMemoryBufferFormatSupported(enum gfx::BufferFormat, struct gpu::Capabilities const&)
{
    *(int*)1 = 1;
    return false;
}
bool IsImageSizeValidForGpuMemoryBufferFormat(class gfx::Size const&, enum gfx::BufferFormat)
{
    *(int*)1 = 1;
    return false;
}
...
bool IsGMBAllowed(const SkImageInfo& info, const gpu::Capabilities& caps)
{
    ...
    const gfx::BufferFormat buffer_format = viz::BufferFormat(viz::SkColorTypeToResourceFormat(info.colorType()));
...
size_t CanvasResourceProvider::max_pinned_image_bytes_ = kDefaultMaxPinnedImageBytes;
...
public:
    CanvasResourceProviderBitmap(
        const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher)
        : CanvasResourceProvider(
            kBitmap, info, filter_quality, true /*is_origin_top_left*/, nullptr /*context_provider_wrapper*/, std::move(resource_dispatcher))
    {
    }
...
public:
    CanvasResourceProviderSharedBitmap(
        const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher)
        : CanvasResourceProviderBitmap(info, filter_quality, std::move(resource_dispatcher))
    {
...
private:
    scoped_refptr<CanvasResource> CreateResource() final
    {
        ...
        if (!IsBitmapFormatSupported(viz::SkColorTypeToResourceFormat(info.colorType()))) {
            // If the rendering format is not supported, downgrade to 8-bits.
            // TODO(junov): Should we try 12-12-12-12 and 10-10-10-2?
            info = info.makeColorType(kN32_SkColorType);
        }

        return CanvasResourceSharedBitmap::Create(info, CreateWeakPtr(), FilterQuality());
    }
...
public:
    CanvasResourceProviderSharedImage(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, bool is_origin_top_left, bool is_accelerated,
        uint32_t shared_image_usage_flags)
        : CanvasResourceProvider(kSharedImage, info, filter_quality, is_origin_top_left, std::move(context_provider_wrapper), nullptr /* resource_dispatcher */)
        , is_accelerated_(is_accelerated)
        , shared_image_usage_flags_(shared_image_usage_flags)
        , use_oop_rasterization_(is_accelerated && ContextProviderWrapper()->ContextProvider()->GetCapabilities().supports_oop_raster)
    {
    ...
    {
        return shared_image_usage_flags_ & gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE;
    }
    gpu::Mailbox GetBackingMailboxForOverwrite(MailboxSyncMode sync_mode) override
    {
    ...
    bool WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y) override
    {
        ...
        RasterInterface()->WritePixels(
            GetBackingMailboxForOverwrite(kOrderingBarrier), x, y, GetBackingTextureTarget(), base::checked_cast<GLuint>(row_bytes), orig_info, pixels);
    ...
    scoped_refptr<CanvasResource> CreateResource() final
    {
        ...
        return CanvasResourceRasterSharedImage::Create(
            GetSkImageInfo(), ContextProviderWrapper(), CreateWeakPtr(), FilterQuality(), IsOriginTopLeft(), is_accelerated_, shared_image_usage_flags_);
    }
    ...
    void NotifyTexParamsModified(const CanvasResource* resource) override
    {
        ...
        if (resource_.get() == resource) {
            ...
            // surface_->getBackendTexture(SkSurface::kFlushRead_BackendHandleAccess).glTextureParametersModified();
            *(int*)1 = 1;
        }
    }
...
protected:
        ...
        if (is_accelerated_ && ShouldReplaceTargetBuffer(cached_content_id_)) {
            ...
            if (raster_interface) {
                ...
                if (mode_ == SkSurface::kRetain_ContentChangeMode) {
                    auto old_mailbox = old_resource_shared_image->GetOrCreateGpuMailbox(kOrderingBarrier);
...
#if SK_SUPPORT_GPU
                    surface_->replaceBackendTexture(CreateGrTextureForResource(), GetGrSurfaceOrigin(), mode_);
#endif
                    *(int*)1 = 1;
    ...
    void RasterRecord(sk_sp<cc::PaintRecord> last_recording, bool preserve_recording) override
    {
        if (!use_oop_rasterization_) {
            CanvasResourceProvider::RasterRecord(std::move(last_recording), preserve_recording);
            return;
        }
        WillDrawInternal(true);
        const bool needs_clear = !is_cleared_;
        is_cleared_ = true;
        RasterRecordOOP(last_recording, needs_clear, resource()->GetOrCreateGpuMailbox(kUnverifiedSyncToken), preserve_recording);
    }
    ...
    sk_sp<SkSurface> CreateSkSurface() const override
    {
        ...
        const auto props = GetSkSurfaceProps();
        if (is_accelerated_) {
            return SkSurface::MakeFromBackendTexture(GetGrContext(), CreateGrTextureForResource(), GetGrSurfaceOrigin(), 0 /* msaa_sample_count */,
    ...
    void EndWriteAccess()
    {
        ...
        if (is_accelerated_) {
            ...
            if (ShouldReplaceTargetBuffer())
                resource_ = NewOrRecycledResource();
            resource()->CopyRenderingResultsToGpuMemoryBuffer(surface_->makeImageSnapshot());
        }
...
public:
    CanvasResourceProviderPassThrough(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        ...
        : CanvasResourceProvider(kPassThrough, info, filter_quality, is_origin_top_left, std::move(context_provider_wrapper), std::move(resource_dispatcher))
    {
    }
...
public:
    CanvasResourceProviderSwapChain(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
        base::WeakPtr<WebGraphicsContext3DProviderWrapper> context_provider_wrapper, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher)
        : CanvasResourceProvider(
            kSwapChain, info, filter_quality, true /*is_origin_top_left*/, std::move(context_provider_wrapper), std::move(resource_dispatcher))
        , use_oop_rasterization_(ContextProviderWrapper()->ContextProvider()->GetCapabilities().supports_oop_raster)
    {
...
private:
    ...
    void RasterRecord(sk_sp<cc::PaintRecord> last_recording, bool preserve_recording) override
    {
        ...
        WillDraw();
        RasterRecordOOP(last_recording, initial_needs_clear_, resource_->GetBackBufferMailbox(), preserve_recording);
    ...
    bool WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y) override
    {
        ...
        WillDraw();
        RasterInterface()->WritePixels(
            resource_->GetBackBufferMailbox(), x, y, GetBackingTextureTarget(), base::checked_cast<GLuint>(row_bytes), orig_info, pixels);
        return true;
    }
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateBitmapProvider(
    const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize should_initialize)
{
    auto provider = std::make_unique<CanvasResourceProviderBitmap>(info, filter_quality, nullptr /*resource_dispatcher*/);
    if (provider->IsValid()) {
    ...
    cc::PaintFlags::FilterQuality filter_quality, ShouldInitialize should_initialize, base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher)
{
    ...
    auto provider = std::make_unique<CanvasResourceProviderSharedBitmap>(info, filter_quality, std::move(resource_dispatcher));
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateSharedImageProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    ...
    bool is_origin_top_left, uint32_t shared_image_usage_flags)
{
    ...
    const bool is_gpu_memory_buffer_image_allowed
        = is_gpu_compositing_enabled && IsGMBAllowed(adjusted_info, capabilities) && Platform::Current()->GetGpuMemoryBufferManager();
    ...
    if (!is_gpu_memory_buffer_image_allowed || (is_accelerated && !capabilities.texture_storage_image)) {
    ...
    auto provider = std::make_unique<CanvasResourceProviderSharedImage>(
        adjusted_info, filter_quality, context_provider_wrapper, is_origin_top_left, is_accelerated, shared_image_usage_flags);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateWebGPUImageProvider(
    const SkImageInfo& info, bool is_origin_top_left, uint32_t shared_image_usage_flags)
{
    ...
    return CreateSharedImageProvider(info, cc::PaintFlags::FilterQuality::kLow, CanvasResourceProvider::ShouldInitialize::kNo,
        std::move(context_provider_wrapper), RasterMode::kGPU, is_origin_top_left, shared_image_usage_flags | gpu::SHARED_IMAGE_USAGE_WEBGPU);
}
    ...
    bool is_origin_top_left)
{
    ...
    if (!capabilities.shared_image_swap_chain && (!IsGMBAllowed(info, capabilities) || !Platform::Current()->GetGpuMemoryBufferManager()))
        return nullptr;

    auto provider
        = std::make_unique<CanvasResourceProviderPassThrough>(info, filter_quality, context_provider_wrapper, resource_dispatcher, is_origin_top_left);
    if (provider->IsValid()) {
...
std::unique_ptr<CanvasResourceProvider> CanvasResourceProvider::CreateSwapChainProvider(const SkImageInfo& info, cc::PaintFlags::FilterQuality filter_quality,
    ...
    base::WeakPtr<CanvasResourceDispatcher> resource_dispatcher, bool is_origin_top_left)
{
    ...
    const auto& capabilities = context_provider_wrapper->ContextProvider()->GetCapabilities();
    if (info.width() > capabilities.max_texture_size || info.height() > capabilities.max_texture_size || !capabilities.shared_image_swap_chain) {
        return nullptr;
    }

    auto provider = std::make_unique<CanvasResourceProviderSwapChain>(info, filter_quality, context_provider_wrapper, resource_dispatcher);
    if (provider->IsValid()) {
        if (should_initialize == CanvasResourceProvider::ShouldInitialize::kCallClear)
            provider->Clear();
        return provider;
...
{
    absl::optional<cc::PlaybackImageProvider::Settings> settings = cc::PlaybackImageProvider::Settings();
...
{
    if (Canvas()) {
        MemoryManagedPaintCanvas* canvas = static_cast<MemoryManagedPaintCanvas*>(Canvas());
...
scoped_refptr<StaticBitmapImage> CanvasResourceProvider::SnapshotInternal(const ImageOrientation& orientation)
{
    if (!IsValid())
        return nullptr;
...
sk_sp<cc::PaintRecord> CanvasResourceProvider::FlushCanvasAndMaybePreserveRecording(bool printing)
{
...
sk_sp<cc::PaintRecord> CanvasResourceProvider::FlushCanvasInternal(bool preserve_recording)
{
    ...
    cc::PaintCanvas* canvas = recorder_->beginRecording(Size().width(), Size().height());
...
void CanvasResourceProvider::RasterRecord(sk_sp<cc::PaintRecord> last_recording, bool)
{
    EnsureSkiaCanvas();
...
void CanvasResourceProvider::RasterRecordOOP(sk_sp<cc::PaintRecord> last_recording, bool needs_clear, gpu::Mailbox mailbox, bool preserve_recording)
{
    if (IsGpuContextLost())
        return;
    ...
    auto list = base::MakeRefCounted<cc::DisplayItemList>(cc::DisplayItemList::kTopLevelDisplayItemList);
...
bool CanvasResourceProvider::WritePixels(const SkImageInfo& orig_info, const void* pixels, size_t row_bytes, int x, int y)
{
    ...
    return GetSkSurface()->getCanvas()->writePixels(orig_info, pixels, row_bytes, x, y);
}
...
void CanvasResourceProvider::RecycleResource(scoped_refptr<CanvasResource>&& resource)
{
    ...
    if (resource->HasOneRef() && resource_recycling_enabled_ && !is_single_buffered_) {
...
void CanvasResourceProvider::SetRestoreClipStackCallback(RestoreMatrixClipStackCb callback)
{
