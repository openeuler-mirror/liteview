...
sk_sp<SkColorFilter> GetDarkModeFilterForImageOnMainThread(DarkModeFilter* filter, Image* image, const SkIRect& rounded_src)
{
...
SkColor DarkModeFilter::AdjustDarkenColor(SkColor color, DarkModeFilter::ElementRole role, SkColor contrast_background)
{
    ...
    case ElementRole::kBorder: {
        ...
        if (color_utils::GetContrastRatio(color, contrast_background) < color_utils::kMinimumReadableContrastRatio)
            return color;

        return AdjustDarkenColor(Color::FromSkColor(color).Dark().Rgb(), role, contrast_background);
    }
    case ElementRole::kSelection: {
        if (!immutable_.color_filter)
            return color;

        return immutable_.color_filter->AdjustColorForHigherConstrast(color, contrast_background, color_utils::kMinimumVisibleContrastRatio);
    }
    default:
        return color;
    }
...
SkColor DarkModeFilter::InvertColorIfNeeded(SkColor color, ElementRole role, SkColor contrast_background)
{
...
void DarkModeFilter::ApplyFilterToImage(Image* image, cc::PaintFlags* flags, const SkRect& src)
{
    ...
    sk_sp<SkColorFilter> color_filter = GetDarkModeFilterForImageOnMainThread(this, image, src.roundOut());
    if (color_filter)
...
sk_sp<SkColorFilter> DarkModeFilter::GenerateImageFilter(const SkPixmap& pixmap, const SkIRect& src) const
{
...
absl::optional<cc::PaintFlags> DarkModeFilter::ApplyToFlagsIfNeeded(const cc::PaintFlags& flags, ElementRole role, SkColor contrast_background)
{
...
bool DarkModeFilter::ShouldApplyToColor(SkColor color, ElementRole role)
{
    ...
    case ElementRole::kListSymbol:
        ...
        return immutable_.foreground_classifier->ShouldInvertColor(color) == DarkModeResult::kApplyFilter;
    case ElementRole::kBackground:
    case ElementRole::kSelection:
        DCHECK(immutable_.background_classifier);
        return immutable_.background_classifier->ShouldInvertColor(color) == DarkModeResult::kApplyFilter;
    default:
        return false;
    }
    NOTREACHED();
