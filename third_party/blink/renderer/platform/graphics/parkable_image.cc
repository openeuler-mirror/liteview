...
BASE_FEATURE(kDelayParkingImages, "DelayParkingImages", base::FEATURE_DISABLED_BY_DEFAULT);

namespace {

void RecordReadStatistics(size_t size, base::TimeDelta duration, base::TimeDelta time_since_freeze)
{
    int throughput_mb_s = static_cast<int>(size / duration.InSecondsF() / (1024 * 1024));
    ...
    base::UmaHistogramLongTimes("Memory.ParkableImage.Read.TimeSinceFreeze", time_since_freeze);
}
...
void RecordWriteStatistics(size_t size, base::TimeDelta duration)
{
    ...
    base::UmaHistogramCounts1000("Memory.ParkableImage.Write.Throughput", throughput_mb_s);
}
...
BASE_FEATURE(kUseParkableImageSegmentReader, "UseParkableImageSegmentReader", base::FEATURE_DISABLED_BY_DEFAULT);
    ...
    scoped_refptr<ParkableImageImpl> parkable_image, scoped_refptr<base::SingleThreadTaskRunner> callback_task_runner)
{
    ...
    auto metadata = ParkableImageManager::Instance().data_allocator().Write(vector.data(), vector.size());
...
// static
size_t ParkableImageImpl::ReadFromDiskIntoBuffer(DiskDataMetadata* on_disk_metadata, void* buffer, size_t capacity)
{
...
scoped_refptr<SegmentReader> ParkableImage::MakeROSnapshot()
{
    ...
    if (base::FeatureList::IsEnabled(kUseParkableImageSegmentReader)) {
        return SegmentReader::CreateFromParkableImage(scoped_refptr<ParkableImage>(this));
    } else {
