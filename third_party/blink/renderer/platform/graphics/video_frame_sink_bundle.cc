...
void VideoFrameSinkBundle::SubmitCompositorFrame(uint32_t sink_id, const viz::LocalSurfaceId& local_surface_id, viz::CompositorFrame frame,
    absl::optional<viz::HitTestRegionList> hit_test_region_list, uint64_t submit_time)
{
...
void VideoFrameSinkBundle::DidAllocateSharedBitmap(uint32_t sink_id, base::ReadOnlySharedMemoryRegion region, const gpu::Mailbox& id)
{
    bundle_->DidAllocateSharedBitmap(sink_id, std::move(region), id);
}

void VideoFrameSinkBundle::DidDeleteSharedBitmap(uint32_t sink_id, const gpu::Mailbox& id)
{
...
void VideoFrameSinkBundle::SetThreadIds(uint32_t sink_id, const WTF::Vector<int32_t>& thread_ids)
{
