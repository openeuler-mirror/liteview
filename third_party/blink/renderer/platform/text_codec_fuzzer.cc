...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    bool saw_error;
    const String decoded = codec->Decode(byte_string.data(), byte_string.length(), flush_behavior, stop_on_error, saw_error);

    // Treat as blink 8-bit string (latin1).
    if (size % sizeof(LChar) == 0) {
        std::unique_ptr<TextCodec> lchar_codec = NewTextCodec(encoding);
        lchar_codec->Encode(reinterpret_cast<const LChar*>(byte_string.data()), byte_string.length() / sizeof(LChar), unencodable_handling);
    }
    ...
    if (size % sizeof(UChar) == 0) {
        ...
        uchar_codec->Encode(reinterpret_cast<const UChar*>(byte_string.data()), byte_string.length() / sizeof(UChar), unencodable_handling);
    }
    ...
    if (decoded.Is8Bit()) {
        codec->Encode(decoded.Characters8(), decoded.length(), unencodable_handling);
    } else {
        codec->Encode(decoded.Characters16(), decoded.length(), unencodable_handling);
    }
    return 0;
}
