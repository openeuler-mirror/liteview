...
public:
    ...
    {
        return std::make_unique<MockWebAudioDevice>(AudioHardwareSampleRate(), AudioHardwareBufferSize());
    }
...
public:
    void Render(AudioBus*, uint32_t frames_to_process, const AudioIOPosition&, const AudioCallbackMetric&) override
    {
...
void CountWASamplesProcessedForRate(absl::optional<float> sample_rate)
{
    ...
    scoped_refptr<AudioDestination> destination = AudioDestination::Create(callback, channel_count, latency_hint, sample_rate, 128);
    destination->Start();
    ...
    int exact_frames_required = std::ceil(request_frames * destination->SampleRate() / Platform::Current()->AudioHardwareSampleRate());
    int expected_frames_processed
        = std::ceil(exact_frames_required / static_cast<double>(destination->RenderQuantumFrames())) * destination->RenderQuantumFrames();
