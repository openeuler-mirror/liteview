...
size_t PushPullFIFO::Pull(AudioBus* output_bus, uint32_t frames_requested)
{
    ...
    for (unsigned i = 0; i < fifo_bus_->NumberOfChannels(); ++i) {
        ...
        if (frames_requested > frames_to_fill) {
            TRACE_EVENT1("webaudio", "PushPullFIFO::Pull underrun", "missing frames", frames_requested - frames_to_fill);
...
size_t PushPullFIFO::PullAndUpdateEarmark(AudioBus* output_bus, uint32_t frames_requested)
{
    ...
    if (frames_requested > frames_available_) {
        ...
        TRACE_EVENT2("webaudio", "PushPullFIFO::PullAndUpdateEarmark (underrun)", "missing frames", missing_frames, "underflow_count_", underflow_count_);
    ...
    return earmark_frames_ > frames_available_ ? earmark_frames_ - frames_available_ : 0;
}
