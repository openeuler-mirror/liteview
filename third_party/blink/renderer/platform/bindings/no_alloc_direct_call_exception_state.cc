...
void NoAllocDirectCallExceptionState::ThrowDOMException(DOMExceptionCode code, const String& message)
{
    deferred_exception_ = WTF::BindOnce(
        [](v8::Isolate* isolate, ExceptionContext&& exception_context, DOMExceptionCode code, const String& message) {
...
{
    deferred_exception_ = WTF::BindOnce(
        [](v8::Isolate* isolate, ExceptionContext&& exception_context, const String& message) {
            ExceptionState exception_state(isolate, std::move(exception_context));
...
void NoAllocDirectCallExceptionState::ThrowSecurityError(const String& sanitized_message, const String& unsanitized_message)
{
    deferred_exception_ = WTF::BindOnce(
        [](v8::Isolate* isolate, ExceptionContext&& exception_context, const String& sanitized_message, const String& unsanitized_message) {
            ExceptionState exception_state(isolate, std::move(exception_context));
            exception_state.ThrowSecurityError(sanitized_message, unsanitized_message);
        },
        WTF::Unretained(GetIsolate()), GetContext(), sanitized_message, unsanitized_message);
