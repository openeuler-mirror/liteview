...
void RecordStatistics(size_t size, base::TimeDelta duration, ParkingAction action)
{
    ...
    const char *size_histogram = nullptr, *latency_histogram = nullptr, *throughput_histogram = nullptr;
    switch (action) {
    ...
    case ParkingAction::kUnparked:
        ...
        throughput_histogram = "Memory.ParkableString.Decompression.ThroughputMBps";
        break;
    ...
    base::UmaHistogramCustomMicrosecondsTimes(latency_histogram, duration, base::Microseconds(500), base::Seconds(1), 100);
...
public:
    ...
    {
        data_
            = reinterpret_cast<char*>(WTF::Partitions::BufferPartition()->AllocWithFlags(partition_alloc::AllocFlags::kReturnNull, size, "NullableCharBuffer"));
        size_ = size;
    }
...
struct BackgroundTaskParams final {
    BackgroundTaskParams(
        scoped_refptr<ParkableStringImpl> string, const void* data, size_t size, scoped_refptr<base::SingleThreadTaskRunner> callback_task_runner)
...
std::unique_ptr<ParkableStringImpl::SecureDigest> ParkableStringImpl::HashString(StringImpl* string)
{
    ...
    bool ok = ComputeDigest(kHashAlgorithmSha256, static_cast<const char*>(string->Bytes()), string->CharactersSizeInBytes(), digest_result);
...
{
    return CurrentStatus() == Status::kUnreferencedExternally && metadata_->age_ != Age::kYoung;
}
...
void ParkableStringImpl::Unpark()
{
    ...
    if (metadata_->last_disk_parking_time_ != base::TimeTicks()) {
        ...
        // buckets.
        base::UmaHistogramLongTimes100("Memory.ParkableString.Read.SinceLastDiskWrite", base::TimeTicks::Now() - metadata_->last_disk_parking_time_);
...
void ParkableStringImpl::PostBackgroundCompressionTask()
{
    ...
    auto params = std::make_unique<BackgroundTaskParams>(this, string_.Bytes(), string_.CharactersSizeInBytes(), manager.task_runner());
    worker_pool::PostTask(FROM_HERE, CrossThreadBindOnce(&ParkableStringImpl::CompressInBackground, std::move(params)));
}
...
//         });
    ...
    bool ok;
    base::StringPiece data(reinterpret_cast<const char*>(params->data), ` params->size);
        ...
        *task_runner, FROM_HERE,
        CrossThreadBindOnce(
            [](std::unique_ptr<BackgroundTaskParams> params, std::unique_ptr<Vector<uint8_t>> compressed, base::TimeDelta parking_thread_time) {
                ParkableStringImpl* string = params->string.get();
...
void ParkableStringImpl::PostBackgroundWritingTask()
{
    ...
    if (!has_on_disk_data() && data_allocator.may_write()) {
        metadata_->background_task_in_progress_ = true;
        auto params = std::make_unique<BackgroundTaskParams>(this, metadata_->compressed_->data(), metadata_->compressed_->size(), manager.task_runner());
        worker_pool::PostTask(FROM_HERE, { base::MayBlock() },
            CrossThreadBindOnce(&ParkableStringImpl::WriteToDiskInBackground, std::move(params), WTF::CrossThreadUnretained(&data_allocator)));
    }
}
