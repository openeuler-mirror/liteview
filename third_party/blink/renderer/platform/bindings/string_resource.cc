...
struct V8StringTwoBytesTrait {
    ...
    ALWAYS_INLINE static void Write(v8::Isolate* isolate, v8::Local<v8::String> v8_string, CharType* buffer, int length)
    {
...
ALWAYS_INLINE bool CanExternalize(v8::Local<v8::String> v8_string, ExternalMode mode)
{
...
ALWAYS_INLINE StringResourceBase* GetExternalizedString(v8::Local<v8::String> v8_string)
{
    v8::String::Encoding encoding;
    v8::String::ExternalStringResourceBase* resource = v8_string->GetExternalStringResourceBase(&encoding);
    if (LIKELY(!!resource)) {
        // Inheritance:
...
ALWAYS_INLINE StringType ConvertAndExternalizeString(v8::Isolate* isolate, v8::Local<v8::String> v8_string, bool can_externalize, bool* was_externalized)
{
    int length = v8_string->Length();
    bool one_byte = v8_string->ContainsOnlyOneByte();
    StringType result = one_byte ? StringTraits<StringType>::template FromV8String<V8StringOneByteTrait>(isolate, v8_string, length)
...
template <typename StringType> StringType ToBlinkString(v8::Local<v8::String> v8_string, ExternalMode mode)
{
    ...
    return ConvertAndExternalizeString<StringType>(v8::Isolate::GetCurrent(), v8_string, CanExternalize(v8_string, mode), &was_externalized);
}
...
template AtomicString ToBlinkString<AtomicString>(v8::Local<v8::String>, ExternalMode);

StringView ToBlinkStringView(v8::Local<v8::String> v8_string, StringView::StackBackingStore& backing_store, ExternalMode mode)
{
    // Be very careful in this code to ensure it is RVO friendly. Accidentally
    // breaking RVO will degrade some of the blink_perf benchmarks by a few
    // percent. This includes moving the StringTraits<>::FromStringResource() call
    ...
    if (LIKELY(can_externalize)) {
        ...
        AtomicString blink_string = ConvertAndExternalizeString<AtomicString>(isolate, v8_string, can_externalize, &was_externalized);
        if (was_externalized)
