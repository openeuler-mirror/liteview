...
CallbackFunctionBase::CallbackFunctionBase(v8::Local<v8::Object> callback_function)
{
    ...
    if (callback_function->IsFunction()) {
        ...
        callback_relevant_script_state_ = ScriptState::From(callback_function->GetCreationContext().ToLocalChecked());
    } else {
        ...
        if (BindingSecurityForPlatform::ShouldAllowAccessToV8Context(
                incumbent_script_state_->GetContext(), creation_context, BindingSecurityForPlatform::ErrorReportOption::kDoNotReport)) {
            callback_relevant_script_state_ = ScriptState::From(creation_context.ToLocalChecked());
        }
    }
}
...
ScriptState* CallbackFunctionBase::CallbackRelevantScriptStateOrReportError(const char* interface_name, const char* operation_name)
{
    ...
    ExceptionState exception_state(GetIsolate(), ExceptionState::kExecutionContext, interface_name, operation_name);
    exception_state.ThrowSecurityError("An invocation of the provided callback failed due to cross origin "
                                       "access.");
    return nullptr;
}

ScriptState* CallbackFunctionBase::CallbackRelevantScriptStateOrThrowException(const char* interface_name, const char* operation_name)
{
    if (callback_relevant_script_state_)
        return callback_relevant_script_state_;
...
void CallbackFunctionBase::EvaluateAsPartOfCallback(base::OnceCallback<void()> closure)
{
    if (!callback_relevant_script_state_)
        return;
