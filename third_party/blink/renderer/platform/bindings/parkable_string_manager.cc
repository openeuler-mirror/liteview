...
struct ParkableStringManager::SecureDigestHash {
    ...
    static unsigned GetHash(const ParkableStringImpl::SecureDigest* const digest)
    {
        // The first bytes of the hash are as good as anything else.
        return *reinterpret_cast<const unsigned*>(digest->data());
    }

    static inline bool Equal(const ParkableStringImpl::SecureDigest* const a, const ParkableStringImpl::SecureDigest* const b)
    {
...
{
    base::UmaHistogramTimes("Memory.ParkableString.MainThreadTime.5min", total_unparking_time_);
    if (base::ThreadTicks::IsSupported()) {
        base::UmaHistogramTimes("Memory.ParkableString.ParkingThreadTime.5min", total_parking_thread_time_);
    }
    Statistics stats = ComputeStatistics();
    base::UmaHistogramCounts100000("Memory.ParkableString.TotalSizeKb.5min", static_cast<int>(stats.original_size / 1000));
    base::UmaHistogramCounts100000("Memory.ParkableString.CompressedSizeKb.5min", static_cast<int>(stats.compressed_size / 1000));
    size_t savings = stats.compressed_original_size - stats.compressed_size;
    base::UmaHistogramCounts100000("Memory.ParkableString.SavingsKb.5min", static_cast<int>(savings / 1000));
    if (stats.compressed_original_size != 0) {
        int ratio_percentage = static_cast<int>((100 * stats.compressed_size) / stats.compressed_original_size);
        base::UmaHistogramPercentageObsoleteDoNotUse("Memory.ParkableString.CompressionRatio.5min", ratio_percentage);
    }

    // May not be usable, e.g. Incognito, permission or write failure.
    base::UmaHistogramBoolean("Memory.ParkableString.DiskIsUsable.5min", data_allocator().may_write());
    ...
    if (data_allocator().may_write()) {
        ...
        base::UmaHistogramCounts100000("Memory.ParkableString.MemorySavingsKb.5min", std::max(0, static_cast<int>(stats.savings_size)) / 1000);
        base::UmaHistogramCounts100000("Memory.ParkableString.OnDiskSizeKb.5min", static_cast<int>(stats.on_disk_size / 1000));
        base::UmaHistogramCounts100000("Memory.ParkableString.OnDiskFootprintKb.5min", static_cast<int>(data_allocator().disk_footprint()) / 1000);
    }
}
...
void ParkableStringManager::AgeStringsAndPark()
{
    ...
    bool reschedule = (!unparked_strings_.empty() || !parked_strings_.empty()) && can_make_progress;
    if (reschedule)
        ScheduleAgingTaskIfNeeded();
}
...
void ParkableStringManager::ScheduleAgingTaskIfNeeded()
{
    ...
    if (base::FeatureList::IsEnabled(features::kDelayFirstParkingOfStrings) && !first_string_aging_was_delayed_) {
