...
{
    if (string.is8Bit()) {
        return String(reinterpret_cast<const LChar*>(string.characters8()), base::checked_cast<wtf_size_t>(string.length()));
    }
    return String(reinterpret_cast<const UChar*>(string.characters16()), base::checked_cast<wtf_size_t>(string.length()));
}
...
std::unique_ptr<SourceLocation> CaptureSourceLocation(v8::Local<v8::Function> function)
{
    if (!function.IsEmpty())
        return std::make_unique<SourceLocation>(ToCoreStringWithUndefinedOrNullCheck(function->GetScriptOrigin().ResourceName()),
            ToCoreStringWithUndefinedOrNullCheck(function->GetName()), function->GetScriptLineNumber() + 1, function->GetScriptColumnNumber() + 1, nullptr,
            function->ScriptId());
    return std::make_unique<SourceLocation>(String(), String(), 0, 0, nullptr, 0);
}
