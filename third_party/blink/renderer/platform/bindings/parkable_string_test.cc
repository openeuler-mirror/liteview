...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, thread_pool_execution_mode)
    {
        ...
        if (use_snappy) {
            scoped_feature_list_.InitAndEnableFeature(features::kUseSnappyForParkableStrings);
        } else {
            scoped_feature_list_.InitAndDisableFeature(features::kUseSnappyForParkableStrings);
        }
    }
...
protected:
    ...
    void WaitForAging()
    {
        ...
        task_environment_.FastForwardBy(base::Seconds(ParkableStringManager::kAgingIntervalInSeconds));
    }
...
INSTANTIATE_TEST_SUITE_P(WithOrWithoutSnappy, ParkableStringTest, ::testing::Bool());
...
TEST_P(ParkableStringTest, DontCompressRandomString)
{
    ...
    if (features::ParkableStringsUseSnappy()) {
        histogram_tester.ExpectUniqueSample("Memory.ParkableString.Snappy.CompressedLargerThanOriginal", true, 1);
    } else {
        histogram_tester.ExpectTotalCount("Memory.ParkableString.Snappy.CompressedLargerThanOriginal", 0);
    }
}
...
TEST_P(ParkableStringTest, Compression)
{
    ...
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Compression.ThroughputMBps", 1);
    // |parkable| is decompressed twice.
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.Decompression.SizeKb", kSizeKb, 2);
...
TEST_P(ParkableStringTest, ToAndFromDisk)
{
    ...
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.Write.SizeKb", kCompressedSize / 1000, 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Write.Latency", 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Write.ThroughputMBps", 1);

    parkable.ToString();
    EXPECT_FALSE(impl->is_on_disk());
    EXPECT_EQ(ParkableStringImpl::Age::kYoung, impl->age_for_testing());

    histogram_tester.ExpectUniqueSample("Memory.ParkableString.Read.SizeKb", kCompressedSize / 1000, 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Read.Latency", 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Read.ThroughputMBps", 1);
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Read.SinceLastDiskWrite", 1);
}
...
TEST_P(ParkableStringTest, UnparkWhileWritingToDisk)
{
    ...
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Read.SinceLastDiskWrite", 0);
}
...
TEST_P(ParkableStringTest, SynchronousToDisk)
{
    ...
    parkable.ToString();
    histogram_tester.ExpectTotalCount("Memory.ParkableString.Read.SinceLastDiskWrite", 2);
}
...
TEST_P(ParkableStringTest, ReportTotalUnparkingTime)
{
    ...
    histogram_tester.ExpectUniqueSample(
        "Memory.ParkableString.MainThreadTime.5min", base::ScopedMockElapsedTimersForTest::kMockElapsedTime.InMilliseconds() * kNumIterations, 1);

    if (base::ThreadTicks::IsSupported()) {
        // The string is only compressed once despite the multiple parking/unparking
        // calls.
        histogram_tester.ExpectUniqueSample(
            "Memory.ParkableString.ParkingThreadTime.5min", base::ScopedMockElapsedTimersForTest::kMockElapsedTime.InMilliseconds(), 1);
    }

    histogram_tester.ExpectUniqueSample("Memory.ParkableString.TotalSizeKb.5min", kSizeKb, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.CompressedSizeKb.5min", kCompressedSize / 1000, 1);

    size_t expected_savings = kSizeKb * 1000 - kCompressedSize;
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.SavingsKb.5min", expected_savings / 1000, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.CompressionRatio.5min", (100 * kCompressedSize) / (kSizeKb * 1000), 1);
}
...
TEST_P(ParkableStringTest, ReportTotalDiskTime)
{
    ...
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.DiskReadTime.5min", mock_elapsed_time_ms * kNumIterations, 1);

    // The string is only written once despite the multiple parking/unparking
    // calls.
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.DiskIsUsable.5min", true, 1);
    ...
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.TotalSizeKb.5min", kSizeKb, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.CompressedSizeKb.5min", 0, 1);

    size_t expected_savings = kSizeKb * 1000 - kCompressedSize;
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.MemorySavingsKb.5min", expected_savings / 1000, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.OnDiskSizeKb.5min", kCompressedSize / 1000, 1);
    histogram_tester.ExpectUniqueSample("Memory.ParkableString.OnDiskFootprintKb.5min", kCompressedSize / 1000, 1);
}
...
INSTANTIATE_TEST_SUITE_P(WithOrWithoutSnappy, ParkableStringTestWithQueuedThreadPool, ::testing::Bool());
...
TEST_P(ParkableStringTestWithQueuedThreadPool, AgingParkingInProgress)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Seconds(ParkableStringManager::kAgingIntervalInSeconds));
    run_loop.Run();
