...
void V8DOMActivityLogger::LogMethod(const char* api_name, v8::FunctionCallbackInfo<v8::Value> info)
{
...
V8DOMActivityLogger* V8DOMActivityLogger::CurrentActivityLoggerIfIsolatedWorldForMainThread()
{
    DCHECK(IsMainThread());
    if (DomActivityLoggersForIsolatedWorld().empty())
        return nullptr;
    return CurrentActivityLoggerIfIsolatedWorld(V8PerIsolateData::MainThreadIsolate());
}
