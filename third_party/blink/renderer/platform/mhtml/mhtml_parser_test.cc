...
{
    return std::string(resources[index]->Data()->Data(), resources[index]->Data()->size());
}
...
public:
    ...
    HeapVector<Member<ArchiveResource>> ParseArchive(const char* mhtml_data, size_t size)
    {
...
TEST_F(MHTMLParserTest, MHTMLPartHeaders)
{
    ...
    HeapVector<Member<ArchiveResource>> resources = ParseArchive(mhtml_data, sizeof(mhtml_data));
