...
public:
    MockVideoCaptureImpl(const media::VideoCaptureSessionId& session_id, PauseResumeCallback* pause_callback, base::OnceClosure destruct_callback)
        : VideoCaptureImpl(session_id, base::ThreadTaskRunnerHandle::Get(), &GetEmptyBrowserInterfaceBroker())
...
private:
    std::unique_ptr<VideoCaptureImpl> CreateVideoCaptureImpl(const media::VideoCaptureSessionId& session_id, BrowserInterfaceBrokerProxy*) const override
    {
...
public:
    VideoCaptureImplManagerTest()
        : manager_(new MockVideoCaptureImplManager(this, BindToCurrentLoop(cleanup_run_loop_.QuitClosure())))
...
protected:
    ...
    MOCK_METHOD3(OnFrameReady, void(scoped_refptr<media::VideoFrame>, std::vector<scoped_refptr<media::VideoFrame>>, base::TimeTicks estimated_capture_time));
