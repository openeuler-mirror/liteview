...
TEST_F(ShapingLineBreakerPerfTest, ShapeLatinText)
{
    ...
    scoped_refptr<const ShapeResult> reference_result = shaper.Shape(&font, direction);
    HarfBuzzShaperCallbackContext context { &shaper, &font, reference_result->Direction() };
    ShapingLineBreaker reference_breaker(reference_result, &break_iterator, nullptr, HarfBuzzShaperCallback, &context);
