...
#define EXPECT_TRANSFORMATION_MATRIX(expected, actual)                                                                                                         \
    do {                                                                                                                                                       \
        EXPECT_TRANSFORM_NEAR(expected.ToTransform(), actual.ToTransform(), kFloatingPointErrorTolerance);                                                     \
    } while (false)

#define EXPECT_FLOAT(expected, actual) EXPECT_NEAR(expected, actual, kFloatingPointErrorTolerance)

inline TransformationMatrix MakeScaleMatrix(double tx, double ty, double tz = 1)
{
...
inline TransformationMatrix MakeTranslationMatrix(double tx, double ty, double tz = 0)
{
    TransformationMatrix t;
...
inline TransformationMatrix MakeRotationMatrix(double degrees_x, double degrees_y, double degrees_z)
{
    TransformationMatrix t;
