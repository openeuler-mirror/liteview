...
protected:
    TextRun SetupFont(FontName font_name, const String& text, bool ltr)
    {
        ...
        font = CreateTestFont("TestFont", test::PlatformTestDataPath(font_path.find(font_name)->value), 100, &ligatures);

        return TextRun(text, /* xpos */ 0, /* expansion */ 0, TextRun::kAllowTrailingExpansion | TextRun::kForbidLeadingExpansion,
...
public:
    void OffsetForPosition(TextRun& run, IncludePartialGlyphsOption partial, BreakGlyphsOption breakopt)
    {
        timer.Reset();
