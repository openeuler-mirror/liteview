...
template <typename T, bool = WTF::IsTraceable<T>::value> struct TraceIfNeeded;

template <typename T> struct TraceIfNeeded<T, false> {
...
template <typename T> struct TraceIfNeeded<T, true> {
    STATIC_ONLY(TraceIfNeeded);
...
template <WTF::WeakHandlingFlag weakness, typename T, typename Traits, bool = WTF::IsTraceableInCollectionTrait<Traits>::value,
...
template <WTF::WeakHandlingFlag weakness, typename T, typename Traits, bool, WTF::WeakHandlingFlag> struct TraceCollectionIfEnabled {
    ...
    static void Trace(Visitor* visitor, const void* t)
    {
        static_assert(WTF::IsTraceableInCollectionTrait<Traits>::value || weakness == WTF::kWeakHandling, "Traits should be traced");
...
template <typename T, typename Traits> struct TraceInCollectionTrait<kNoWeakHandling, T, Traits> {
    ...
    {
        static_assert(IsTraceableInCollectionTrait<Traits>::value, "T should be traceable");
        visitor->Trace(t);
    }
};
