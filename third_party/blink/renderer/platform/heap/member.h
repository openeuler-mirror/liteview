...
template <typename T> struct MemberHash : IntHash<cppgc::internal::MemberBase::RawStorage::IntegralType> {
...
template <typename T, typename MemberType> struct BaseMemberHashTraits : SimpleClassHashTraits<MemberType> {
    ...
    template <typename U> static void Store(const U& value, MemberType& storage)
    {
        storage = value;
    }
...
template <typename T> struct HashTraits<blink::Member<T>> : BaseMemberHashTraits<T, blink::Member<T>> {
    static constexpr bool kCanTraceConcurrently = true;
};

template <typename T> struct HashTraits<blink::WeakMember<T>> : BaseMemberHashTraits<T, blink::WeakMember<T>> {
    static constexpr bool kCanTraceConcurrently = true;
};

template <typename T> struct HashTraits<blink::UntracedMember<T>> : BaseMemberHashTraits<T, blink::UntracedMember<T>> { };
