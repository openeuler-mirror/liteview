...
{
    auto dump_prefix_depth = std::count(dump_prefix.begin(), dump_prefix.end(), '/');
    for (auto& it : dump.allocator_dumps()) {
        const std::string& key = it.first;
        if ((key.compare(0, dump_prefix.size(), dump_prefix) == 0) && (std::count(key.begin(), key.end(), '/') == dump_prefix_depth)) {
...
{
    base::trace_event::MemoryDumpArgs args = { base::trace_event::MemoryDumpLevelOfDetail::LIGHT };
    ...
    std::unique_ptr<BlinkGCMemoryDumpProvider> dump_provider(
        new BlinkGCMemoryDumpProvider(ThreadState::Current(), base::ThreadTaskRunnerHandle::Get(), BlinkGCMemoryDumpProvider::HeapType::kBlinkMainThread));
...
TEST_F(BlinkGCMemoryDumpProviderTest, WorkerLightDump)
{
    ...
    std::unique_ptr<base::trace_event::ProcessMemoryDump> dump(new base::trace_event::ProcessMemoryDump(args));
    std::unique_ptr<BlinkGCMemoryDumpProvider> dump_provider(
        new BlinkGCMemoryDumpProvider(ThreadState::Current(), base::ThreadTaskRunnerHandle::Get(), BlinkGCMemoryDumpProvider::HeapType::kBlinkWorkerThread));
    dump_provider->OnMemoryDump(args, dump.get());
...
TEST_F(BlinkGCMemoryDumpProviderTest, WorkerDetailedDump)
{
    ...
    for (const auto& kvp : dump->allocator_dumps()) {
        if (kvp.first.find(worker_path_prefix + "/worker_0x") != std::string::npos) {
