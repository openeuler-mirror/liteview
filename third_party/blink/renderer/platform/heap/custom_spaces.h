...
class PLATFORM_EXPORT HeapVectorBackingSpace : public cppgc::CustomSpace<HeapVectorBackingSpace> {
public:
    static constexpr cppgc::CustomSpaceIndex kSpaceIndex = 0;
    static constexpr bool kSupportsCompaction = true;
};

class PLATFORM_EXPORT HeapHashTableBackingSpace : public cppgc::CustomSpace<HeapHashTableBackingSpace> {
public:
...
struct CustomSpaces {
    ...
    static std::vector<cppgc::CustomSpaceBase*> CreateCustomSpaces()
    {
        std::vector<cppgc::CustomSpaceBase*> spaces;
        spaces.emplace_back(new HeapVectorBackingSpace());
        spaces.emplace_back(new HeapHashTableBackingSpace());
        spaces.emplace_back(new NodeSpace());
        spaces.emplace_back(new CSSValueSpace());
        spaces.emplace_back(new LayoutObjectSpace());
        return spaces;
    }
};
