...
class HeapVectorBacking final : public GarbageCollected<HeapVectorBacking<T, Traits>>,
                                public WTF::ConditionalDestructor<HeapVectorBacking<T, Traits>, Traits::kNeedsDestruction> {
...
public:
    ...
    NO_SANITIZE_UNRELATED_CAST ALWAYS_INLINE static T* ToArray(ClassType* backing)
    {
...
template <typename T, typename Traits> void HeapVectorBacking<T, Traits>::Finalize()
{
        ...
        if (std::is_polymorphic<T>::value) {
            for (unsigned i = 0; i < length; ++i) {
                if (blink::internal::VTableInitialized(&array[i])) {
                    blink::TraceIfNeeded<T, IsTraceableInCollectionTrait<Traits>::value>::Trace(visitor, array[i]);
                }
            }
        } else {
            for (size_t i = 0; i < length; ++i) {
...
template <typename T> struct SpaceTrait<blink::HeapVectorBacking<T>> {
