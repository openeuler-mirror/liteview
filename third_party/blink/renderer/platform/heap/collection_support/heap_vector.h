...
// class HeapVector2 final
//   : public VIRTUAL_BASE GarbageCollected<HeapVector2<T, inlineCapacity>>
//   , public Vector2<T, inlineCapacity, HeapAllocator> {
// public:
//   ~HeapVector2() {
//   }
// };
...
class Vector3 {
public:
    // USE_ALLOCATOR(Vector3, Allocator);
...
template <typename T, wtf_size_t inlineCapacity = 0> class HeapVector2 final : public GarbageCollected<HeapVector2<T, inlineCapacity>>, public Vector3 {
public:
    HeapVector2()
    {
    }
...
class HeapVector final 
    : public GarbageCollected<HeapVector<T, inlineCapacity>>
...
#if defined(_MSC_VER) && !defined(__clang__) // 不加这句，会报错delect不明确
    void FinalizeGarbageCollectedObject()
    {
        this->Vector<T, inlineCapacity, HeapAllocator>::~Vector();
    }
#endif
...
private:
    template <typename U> struct IsHeapVector {
    private:
        ...
        template <typename X, wtf_size_t Y> static YesType SubclassCheck(HeapVector<X, Y>*);
    ...
    {
        static_assert(std::is_trivially_destructible<HeapVector>::value || inlineCapacity, "HeapVector must be trivially destructible.");
        ...
        static_assert(!WTF::IsGarbageCollectedType<T>::value || IsHeapVector<T>::value, "GCed types should not be inlined in a HeapVector.");
        static_assert(WTF::IsTraceableInCollectionTrait<VectorTraits<T>>::value, "Type must be traceable in collection");
    }
};
