...
template <typename CharType> Error SkipWhitespaceAndComments(Cursor<CharType>* cursor, const CharType* end, bool* has_comments)
{
    while (cursor->pos < end) {
        CharType c = *(cursor->pos);
        if (c == '\n') {
...
template <typename CharType> Error ParseToken(Cursor<CharType>* cursor, const CharType* end, Token* token, Cursor<CharType>* token_start, bool* has_comments)
{
...
Error BuildValue(Cursor<CharType>* cursor, const CharType* end, int max_depth, std::unique_ptr<JSONValue>* result, bool* has_comments)
{
    if (max_depth == 0)
    ...
    case kNumber: {
        bool ok;
        double value = CharactersToDouble(token_start.pos, cursor->pos - token_start.pos, &ok);
    ...
    case kArrayBegin: {
        ...
        while (token != kArrayEnd) {
            *cursor = before_token;
            ...
            error = BuildValue(cursor, end, max_depth - 1, &array_node, has_comments);
            if (error != Error::kNoError)
                return error;
...
template <typename CharType>
JSONParseError ParseJSONInternal(const CharType* start_ptr, unsigned length, int max_depth, std::unique_ptr<JSONValue>* result, bool* has_comments)
{
...
std::unique_ptr<JSONValue> ParseJSON(const String& json, JSONParseError* opt_error, bool* opt_has_comments)
{
    return ParseJSON(json, kMaxStackLimit, opt_error, opt_has_comments);
}

std::unique_ptr<JSONValue> ParseJSON(const String& json, int max_depth, JSONParseError* opt_error, bool* opt_has_comments)
{
    if (max_depth < 0)
        max_depth = 0;
    ...
    if (json.empty()) {
        ...
        error = ParseJSONInternal(json.Characters8(), json.length(), max_depth, &result, &has_comments);
    } else {
        error = ParseJSONInternal(json.Characters16(), json.length(), max_depth, &result, &has_comments);
    }
