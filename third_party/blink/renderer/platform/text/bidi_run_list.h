...
template <class Run> class BidiRunList final {
    STACK_ALLOCATED();
...
template <class Run> inline void BidiRunList<Run>::AddRun(Run* run)
{
    if (!first_run_)
        first_run_ = run;
    else
...
template <class Run> inline void BidiRunList<Run>::PrependRun(Run* run)
{
    DCHECK(!run->next_);
...
template <class Run> inline void BidiRunList<Run>::MoveRunToEnd(Run* run)
{
    DCHECK(first_run_);
    DCHECK(last_run_);
    DCHECK(run->next_);
...
template <class Run> inline void BidiRunList<Run>::MoveRunToBeginning(Run* run)
{
    DCHECK(first_run_);
    DCHECK(last_run_);
...
template <class Run> void BidiRunList<Run>::ReplaceRunWithRuns(Run* to_replace, BidiRunList<Run>& new_runs)
{
    ...
    run_count_ += new_runs.RunCount() - 1; // We added the new runs and removed toReplace.

    delete to_replace;
    new_runs.ClearWithoutDestroyingRuns();
}

template <class Run> void BidiRunList<Run>::ClearWithoutDestroyingRuns()
{
    first_run_ = nullptr;
    last_run_ = nullptr;
    logically_last_run_ = nullptr;
...
template <class Run> void BidiRunList<Run>::DeleteRuns()
{
    if (!first_run_)
        return;
...
template <class Run> void BidiRunList<Run>::ReverseRuns(unsigned start, unsigned end)
{
    DCHECK(run_count_);
    if (start >= end)
        return;
