...
struct PerThreadData {
    HashMap<AtomicString, scoped_refptr<LayoutLocale>, CaseFoldingHash> locale_map;
...
scoped_refptr<QuotesData> GetQuotesDataForLanguage(const char* locale)
{
    ...
    UChar open1[ucharDelimMaxLength], close1[ucharDelimMaxLength], open2[ucharDelimMaxLength], close2[ucharDelimMaxLength];
...
const LayoutLocale& LayoutLocale::GetDefault()
{
    ...
    if (UNLIKELY(!data.default_locale)) {
        ...
        data.default_locale = LayoutLocale::Get(!language.empty() ? language : "en");
    }
...
AtomicString LayoutLocale::LocaleWithBreakKeyword(LineBreakIteratorMode mode) const
{
    if (string_.empty())
        return string_;
    ...
    ICUError status;
    int32_t length_needed = uloc_setKeywordValue("lb", keyword_value, buffer.data(), buffer.size(), &status);
    if (U_SUCCESS(status))
        return AtomicString::FromUTF8(buffer.data(), length_needed);

    if (status == U_BUFFER_OVERFLOW_ERROR) {
        buffer.Grow(length_needed + 1);
        memset(buffer.data() + utf8_locale.length(), 0, buffer.size() - utf8_locale.length());
        status = U_ZERO_ERROR;
        int32_t length_needed2 = uloc_setKeywordValue("lb", keyword_value, buffer.data(), buffer.size(), &status);
