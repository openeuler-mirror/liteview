...
public:
    ...
    {
        StartIgnoringSpaces(Iterator(nullptr, text_paragraph_separator.GetLineLayoutItem(), text_paragraph_separator.Offset() - 1));
        StopIgnoringSpaces(Iterator(nullptr, text_paragraph_separator.GetLineLayoutItem(), text_paragraph_separator.Offset()));
    }
    ...
    void CheckMidpoints(Iterator& l_break)
    {
        ...
        if (l_break.GetLineLayoutItem() && num_midpoints_ && !(num_midpoints_ % 2)) {
            ...
            while (!currpoint.AtEnd() && currpoint != startpoint && currpoint != l_break)
                currpoint.Increment();
    ...
    const unsigned& NumMidpoints() const
    {
        return num_midpoints_;
    }
    const unsigned& CurrentMidpoint() const
    {
        return current_midpoint_;
    }
    void IncrementCurrentMidpoint()
    {
        current_midpoint_++;
    }
    const bool& BetweenMidpoints() const
    {
        return between_midpoints_;
    }
...
struct BidiStatus final {
    ...
    {
        WTF::unicode::CharDirection direction = text_direction == TextDirection::kLtr ? WTF::unicode::kLeftToRight : WTF::unicode::kRightToLeft;
        eor = last_strong = last = direction;
        context = BidiContext::Create(text_direction == TextDirection::kLtr ? 0 : 1, direction, is_override);
    }

    BidiStatus(WTF::unicode::CharDirection eor_dir, WTF::unicode::CharDirection last_strong_dir, WTF::unicode::CharDirection last_dir,
    ...
    static BidiStatus CreateForIsolate(TextDirection text_direction, bool is_override, unsigned char level)
    {
        ...
        scoped_refptr<BidiContext> context = BidiContext::Create(level, direction, is_override, kFromStyleOrDOM);
...
{
    return status1.eor == status2.eor && status1.last == status2.last && status1.last_strong == status2.last_strong && *(status1.context) == *(status2.context);
}
...
enum VisualDirectionOverride { kNoVisualOverride, kVisualLeftToRightOverride, kVisualRightToLeftOverride };
...
public:
    ...
    void SetEorDir(WTF::unicode::CharDirection eor_dir)
    {
        status_.eor = eor_dir;
    }
    ...
    void SetDir(WTF::unicode::CharDirection d)
    {
        direction_ = d;
    }
    ...
    void SetStatus(const BidiStatus s)
    {
        DCHECK(s.context);
        status_ = s;
        paragraph_directionality_ = s.context->Dir() == WTF::unicode::kLeftToRight ? TextDirection::kLtr : TextDirection::kRtl;
    }

    MidpointState<Iterator>& GetMidpointState()
    {
        return midpoint_state_;
    }
    ...
    {
        return nested_isolate_count_;
    }

    void Embed(WTF::unicode::CharDirection, BidiEmbeddingSource);
    bool CommitExplicitEmbedding(BidiRunList<Run>&);

    void CreateBidiRunsForLine(const Iterator& end, VisualDirectionOverride = kNoVisualOverride, bool hard_line_break = false, bool reorder_runs = true);
    ...
    Vector<IsolatedRun>& IsolatedRuns()
    {
        return isolated_runs_;
    }
    ...
    TextDirection DetermineParagraphDirectionality(bool* has_strong_directionality = nullptr)
    {
        bool break_on_paragraph = true;
        return DetermineDirectionalityInternal(break_on_paragraph, has_strong_directionality);
    }
    TextDirection DetermineDirectionality(bool* has_strong_directionality = nullptr)
    {
    ...
    Iterator EndOfLine() const
    {
        return end_of_line_;
    }

    void SetNeedsTrailingSpace(bool value)
    {
        needs_trailing_space_ = value;
    }
    Run* TrailingSpaceRun() const
    {
        return trailing_space_run_;
    }
...
protected:
    ...
    Run* AddTrailingRun(BidiRunList<Run>&, int, int, Run*, BidiContext*, TextDirection) const
    {
        return nullptr;
    }
    Iterator current_;
...
private:
    ...
    bool NeedsTrailingSpace(BidiRunList<Run>&)
    {
        return needs_trailing_space_;
    }
    int FindFirstTrailingSpaceAtRun(Run*)
    {
        return 0;
    }
    ...
    TextDirection DetermineDirectionalityInternal(bool break_on_paragraph, bool* has_strong_directionality);
...
#if DCHECK_IS_ON()
template <class Iterator, class Run, class IsolatedRun> BidiResolver<Iterator, Run, IsolatedRun>::~BidiResolver()
{
...
template <class Iterator, class Run, class IsolatedRun> void BidiResolver<Iterator, Run, IsolatedRun>::AppendRun(BidiRunList<Run>& runs)
{
    if (!empty_run_ && !eor_.AtEnd()) {
        unsigned start_offset = sor_.Offset();
        unsigned end_offset = eor_.Offset();

        if (!end_of_run_at_end_of_line_.AtEnd() && end_offset >= end_of_run_at_end_of_line_.Offset()) {
        ...
        while (start_offset < end_offset) {
            ...
            const int kLimit = USHRT_MAX; // InlineTextBox stores text length as unsigned short.
            if (end - start_offset > kLimit)
                end = start_offset + kLimit;
            runs.AddRun(new Run(Context()->Override(), Context()->Level(), start_offset, end, direction_, Context()->Dir()));
            start_offset = end;
        }
...
void BidiResolver<Iterator, Run, IsolatedRun>::Embed(WTF::unicode::CharDirection dir, BidiEmbeddingSource source)
{
    ...
    DCHECK(dir == WTF::unicode::kPopDirectionalFormat || dir == WTF::unicode::kLeftToRightEmbedding || dir == WTF::unicode::kLeftToRightOverride
        || dir == WTF::unicode::kRightToLeftEmbedding || dir == WTF::unicode::kRightToLeftOverride);
    current_explicit_embedding_sequence_.push_back(BidiEmbedding(dir, source));
}

template <class Iterator, class Run, class IsolatedRun> void BidiResolver<Iterator, Run, IsolatedRun>::CheckDirectionInLowerRaiseEmbeddingLevel()
{
    ...
    if (direction_ == WTF::unicode::kOtherNeutral) {
        direction_ = status_.last_strong == WTF::unicode::kLeftToRight ? WTF::unicode::kLeftToRight : WTF::unicode::kRightToLeft;
    }
}
...
void BidiResolver<Iterator, Run, IsolatedRun>::LowerExplicitEmbeddingLevel(BidiRunList<Run>& runs, WTF::unicode::CharDirection from)
{
    if (!empty_run_ && eor_ != last_) {
        ...
        } else if (status_.eor == WTF::unicode::kEuropeanNumber || status_.eor == WTF::unicode::kArabicNumber
...
void BidiResolver<Iterator, Run, IsolatedRun>::RaiseExplicitEmbeddingLevel(
    BidiRunList<Run>& runs, WTF::unicode::CharDirection from, WTF::unicode::CharDirection to)
{
    if (!empty_run_ && eor_ != last_) {
        CheckDirectionInLowerRaiseEmbeddingLevel();
        // bidi.sor ... bidi.eor ... bidi.last eor; need to append the
        ...
        if (to == WTF::unicode::kLeftToRight) {
            ...
            if (status_.eor == WTF::unicode::kEuropeanNumber) {
                ...
                direction_ = WTF::unicode::kArabicNumber;
                AppendRun(runs);
            } else if (status_.last_strong != WTF::unicode::kLeftToRight && from == WTF::unicode::kLeftToRight) {
                AppendRun(runs);
                direction_ = WTF::unicode::kLeftToRight;
            }
        } else if (status_.eor == WTF::unicode::kArabicNumber
            || (status_.eor == WTF::unicode::kEuropeanNumber && (status_.last_strong != WTF::unicode::kLeftToRight || from == WTF::unicode::kRightToLeft))
            || (status_.eor != WTF::unicode::kEuropeanNumber && status_.last_strong == WTF::unicode::kLeftToRight && from == WTF::unicode::kRightToLeft)) {
            AppendRun(runs);
            direction_ = WTF::unicode::kRightToLeft;
        }
...
template <class Iterator, class Run, class IsolatedRun> void BidiResolver<Iterator, Run, IsolatedRun>::ComputeTrailingSpace(BidiRunList<Run>& runs)
{
    ...
    bool should_reorder = trailing_space_run != (paragraph_directionality_ == TextDirection::kLtr ? runs.LastRun() : runs.FirstRun());
    if (first_space != trailing_space_run->Start()) {
        BidiContext* base_context = Context();
        while (BidiContext* parent = base_context->Parent())
            base_context = parent;

        trailing_space_run_ = AddTrailingRun(runs, first_space, trailing_space_run->stop_, trailing_space_run, base_context, paragraph_directionality_);
...
template <class Iterator, class Run, class IsolatedRun> bool BidiResolver<Iterator, Run, IsolatedRun>::CommitExplicitEmbedding(BidiRunList<Run>& runs)
{
    ...
    for (const BidiEmbedding& embedding : current_explicit_embedding_sequence_) {
        if (embedding.Direction() == WTF::unicode::kPopDirectionalFormat) {
            ...
            WTF::unicode::CharDirection direction
                = (embedding.Direction() == WTF::unicode::kRightToLeftEmbedding || embedding.Direction() == WTF::unicode::kRightToLeftOverride)
                ? WTF::unicode::kRightToLeft
                : WTF::unicode::kLeftToRight;
            bool override = embedding.Direction() == WTF::unicode::kLeftToRightOverride || embedding.Direction() == WTF::unicode::kRightToLeftOverride;
            ...
            if (level < BidiContext::kMaxLevel) {
                to_context = BidiContext::Create(level, direction, override, embedding.Source(), to_context.get());
            }
        }
    }
    ...
    if (to_level > from_level) {
        RaiseExplicitEmbeddingLevel(runs, from_level % 2 ? WTF::unicode::kRightToLeft : WTF::unicode::kLeftToRight,
            to_level % 2 ? WTF::unicode::kRightToLeft : WTF::unicode::kLeftToRight);
    } else if (to_level < from_level) {
        LowerExplicitEmbeddingLevel(runs, from_level % 2 ? WTF::unicode::kRightToLeft : WTF::unicode::kLeftToRight);
    }
...
template <class Iterator, class Run, class IsolatedRun>
inline void BidiResolver<Iterator, Run, IsolatedRun>::UpdateStatusLastFromCurrentDirection(WTF::unicode::CharDirection dir_current)
{
    switch (dir_current) {
...
template <class Iterator, class Run, class IsolatedRun>
TextDirection BidiResolver<Iterator, Run, IsolatedRun>::DetermineDirectionalityInternal(bool break_on_paragraph, bool* has_strong_directionality)
{
    while (!current_.AtEnd()) {
        ...
        WTF::unicode::CharDirection char_direction = WTF::unicode::Direction(current);
        if (char_direction == WTF::unicode::kLeftToRight) {
            if (has_strong_directionality)
                *has_strong_directionality = true;
            return TextDirection::kLtr;
        }
        if (char_direction == WTF::unicode::kRightToLeft || char_direction == WTF::unicode::kRightToLeftArabic) {
            if (has_strong_directionality)
                *has_strong_directionality = true;
...
{
    WTF::unicode::CharDirection char_direction = WTF::unicode::Direction(character);
    if (char_direction == WTF::unicode::kRightToLeft || char_direction == WTF::unicode::kRightToLeftArabic)
        return TextDirection::kRtl;
    return TextDirection::kLtr;
}
...
void BidiResolver<Iterator, Run, IsolatedRun>::CreateBidiRunsForLine(
    const Iterator& end, VisualDirectionOverride override, bool hard_line_break, bool reorder_runs)
{
    ...
    if (override != kNoVisualOverride) {
        ...
        direction_ = override == kVisualLeftToRightOverride ? WTF::unicode::kLeftToRight : WTF::unicode::kRightToLeft;
        AppendRun(runs_);
    ...
    while (true) {
        ...
        if (last_line_ended && (hard_line_break || current_.AtEnd())) {
            ...
            if (Context()->Override() && dir_current != WTF::unicode::kRightToLeftEmbedding && dir_current != WTF::unicode::kLeftToRightEmbedding
                && dir_current != WTF::unicode::kRightToLeftOverride && dir_current != WTF::unicode::kLeftToRightOverride
        ...
        case WTF::unicode::kLeftToRight:
            ...
            case WTF::unicode::kEuropeanNumber:
            case WTF::unicode::kArabicNumber:
                if (status_.last != WTF::unicode::kEuropeanNumber || status_.last_strong != WTF::unicode::kLeftToRight)
                    AppendRun(runs_);
                break;
            case WTF::unicode::kLeftToRight:
                break;
        ...
        case WTF::unicode::kRightToLeft:
            ...
            case WTF::unicode::kOtherNeutral:
                if (status_.eor == WTF::unicode::kEuropeanNumber) {
                    if (status_.last_strong == WTF::unicode::kLeftToRight && Context()->Dir() == WTF::unicode::kLeftToRight)
                        eor_ = last_;
                    AppendRun(runs_);
                } else if (status_.eor == WTF::unicode::kArabicNumber) {
                    AppendRun(runs_);
                ...
                case WTF::unicode::kOtherNeutral:
                    if (status_.eor == WTF::unicode::kEuropeanNumber) {
                        if (status_.last_strong == WTF::unicode::kRightToLeft) {
                            ...
                            AppendRun(runs_);
                            // Make an R run.
                            eor_ = status_.last == WTF::unicode::kEuropeanNumberTerminator ? last_before_et_ : last_;
                            direction_ = WTF::unicode::kRightToLeft;
                            AppendRun(runs_);
                            // Begin a new EN run.
                            direction_ = WTF::unicode::kEuropeanNumber;
                        ...
                        AppendRun(runs_);
                        if (status_.last_strong == WTF::unicode::kRightToLeft || Context()->Dir() == WTF::unicode::kRightToLeft) {
                            // Make an R run.
                            eor_ = status_.last == WTF::unicode::kEuropeanNumberTerminator ? last_before_et_ : last_;
                            direction_ = WTF::unicode::kRightToLeft;
                            AppendRun(runs_);
        ...
        case WTF::unicode::kArabicNumber:
            ...
            case WTF::unicode::kOtherNeutral:
                if (status_.eor == WTF::unicode::kArabicNumber
                    || (status_.eor == WTF::unicode::kEuropeanNumber
                        && (status_.last_strong == WTF::unicode::kRightToLeft || Context()->Dir() == WTF::unicode::kRightToLeft))
                    || (status_.eor != WTF::unicode::kEuropeanNumber && status_.last_strong == WTF::unicode::kLeftToRight
        ...
        if (last_line_ended && eor_ == current_) {
            if (!reached_end_of_line_) {
                ...
                case WTF::unicode::kArabicNumber:
                    ...
                    break;
                case WTF::unicode::kEuropeanNumber:
                    direction_ = status_.last_strong == WTF::unicode::kLeftToRight ? WTF::unicode::kLeftToRight : WTF::unicode::kEuropeanNumber;
                    break;
                default:
                    NOTREACHED();
                }
...
template <class Iterator, class Run, class IsolatedRun> MidpointState<Iterator> BidiResolver<Iterator, Run, IsolatedRun>::MidpointStateForIsolatedRun(Run& run)
{
