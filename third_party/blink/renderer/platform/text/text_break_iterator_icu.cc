...
static int32_t TextExtract(UText*, int64_t, int64_t, UChar*, int32_t, UErrorCode* error_code)
{
...
static inline TextContext TextGetContext(const UText* text, int64_t native_index, UBool forward)
{
...
static void TextLatin1MoveInPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static void TextLatin1SwitchToPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static void TextLatin1MoveInPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static void TextLatin1SwitchToPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static inline bool TextInChunkOrOutOfRange(UText* text, int64_t native_index, int64_t native_length, UBool forward, UBool& is_accessible)
{
    if (forward) {
...
static void TextInit(UText* text, const UTextFuncs* funcs, const void* string, unsigned length, const UChar* prior_context, int prior_context_length)
{
    text->pFuncs = funcs;
...
static UText* TextOpenLatin1(
    UTextWithBuffer* ut_with_buffer, base::span<const LChar> string, const UChar* prior_context, int prior_context_length, UErrorCode* status)
{
    if (U_FAILURE(*status))
        return nullptr;
...
static void TextUTF16MoveInPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static void TextUTF16SwitchToPrimaryContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    DCHECK(!text->chunkContents || text->chunkContents == text->q);
...
static void TextUTF16MoveInPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
    DCHECK_EQ(text->chunkContents, text->q);
    DCHECK(forward ? native_index < text->b : native_index <= text->b);
    DCHECK(forward ? native_index < native_length : native_index <= native_length);
...
static void TextUTF16SwitchToPriorContext(UText* text, int64_t native_index, int64_t native_length, UBool forward)
{
...
static UText* TextOpenUTF16(UText* text, base::span<const UChar> string, const UChar* prior_context, int prior_context_length, UErrorCode* status)
{
    if (U_FAILURE(*status))
        return nullptr;
...
TextBreakIterator* AcquireLineBreakIterator(
    base::span<const LChar> string, const AtomicString& locale, const UChar* prior_context, unsigned prior_context_length)
{
...
TextBreakIterator* AcquireLineBreakIterator(
    base::span<const UChar> string, const AtomicString& locale, const UChar* prior_context, unsigned prior_context_length)
{
    TextBreakIterator* iterator = LineBreakIteratorPool::SharedPool().Take(locale);
    if (!iterator)
        return nullptr;
