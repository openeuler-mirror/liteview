...
TEST(LayoutLocaleTest, Get)
{
    ...
    EXPECT_STRCASEEQ("en-us", LayoutLocale::Get("en-us")->LocaleString().Ascii().c_str());
    EXPECT_STRCASEEQ("ja-jp", LayoutLocale::Get("ja-jp")->LocaleString().Ascii().c_str());
...
TEST_P(LocaleTestDataFixture, Script)
{
    ...
    scoped_refptr<LayoutLocale> locale = LayoutLocale::CreateForTesting(test.locale);
...
TEST(LayoutLocaleTest, BreakKeyword)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_EQ(test.expected, locale->LocaleWithBreakKeyword(test.mode))
            << String::Format("'%s' with line-break %d should be '%s'", test.locale, static_cast<int>(test.mode), test.expected);
    }
}
...
TEST(LayoutLocaleTest, ExistingKeywordName)
{
    ...
    for (auto* const test : tests) {
        ...
        EXPECT_EQ(test, locale->LocaleWithBreakKeyword(LineBreakIteratorMode::kNormal));
    }
}
