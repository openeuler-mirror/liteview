...
PLATFORM_EXPORT TextBreakIterator* AcquireLineBreakIterator(
    base::span<const LChar>, const AtomicString& locale, const UChar* prior_context, unsigned prior_context_length);
PLATFORM_EXPORT TextBreakIterator* AcquireLineBreakIterator(
    base::span<const UChar>, const AtomicString& locale, const UChar* prior_context, unsigned prior_context_length);
...
public:
    ...
    LazyLineBreakIterator(String string, const AtomicString& locale = AtomicString(), LineBreakType break_type = LineBreakType::kNormal)
    ...
    {
        static_assert(std::extent<decltype(prior_context_)>() == 2, "TextBreakIterator has unexpected prior context length");
    ...
    inline bool IsBreakable(int pos, int& next_breakable, LineBreakType line_break_type) const
    {
...
private:
    ...
    TextBreakIterator* GetIterator(const PriorContext& prior_context) const
    {
        ...
        if (string_.Is8Bit()) {
            iterator_ = AcquireLineBreakIterator(string_.Span8().subspan(start_offset_), locale_, prior_context.text, prior_context.length);
        } else {
            iterator_ = AcquireLineBreakIterator(string_.Span16().subspan(start_offset_), locale_, prior_context.text, prior_context.length);
        }
        return iterator_;
    }

    template <typename CharacterType> bool IsOtherSpaceSeparator(UChar ch) const
    {
        return Character::IsOtherSpaceSeparator(ch);
    }
    template <LChar> bool IsOtherSpaceSeparator(UChar ch) const
    {
        return false;
    }

    template <typename CharacterType, LineBreakType, BreakSpaceType> int NextBreakablePosition(int pos, const CharacterType* str, int len) const;
    template <typename CharacterType, LineBreakType> int NextBreakablePosition(int pos, const CharacterType* str, int len) const;
    template <LineBreakType> int NextBreakablePosition(int pos, int len) const;
