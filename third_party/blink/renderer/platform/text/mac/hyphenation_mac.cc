...
public:
    ...
    wtf_size_t LastHyphenLocation(const StringView& text, wtf_size_t before_index) const override
    {
        ...
        CFIndex result = CFStringGetHyphenationLocationBeforeIndex(
            text.ToString().Impl()->CreateCFString(), before_index, CFRangeMake(0, text.length()), 0, locale_cf_, 0);
    ...
    wtf_size_t FirstHyphenLocation(const StringView& text, wtf_size_t after_index) const override
    {
        ...
        after_index = std::max(after_index, static_cast<wtf_size_t>(kMinimumPrefixLength - 1));
...
{
    base::ScopedCFTypeRef<CFStringRef> locale_cf_string(locale.Impl()->CreateCFString());
    base::ScopedCFTypeRef<CFLocaleRef> locale_cf(CFLocaleCreate(kCFAllocatorDefault, locale_cf_string));
