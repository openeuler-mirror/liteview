...
public:
    BidiCharacterRun(bool override, unsigned char level, int start, int stop, WTF::unicode::CharDirection dir, WTF::unicode::CharDirection override_dir)
        : override_(override)
        , level_(level)
        , next_(nullptr)
        , start_(start)
        , stop_(stop)
    {
        ...
        if (level_ % 2) {
            if (dir == WTF::unicode::kLeftToRight || dir == WTF::unicode::kArabicNumber || dir == WTF::unicode::kEuropeanNumber)
                level_++;
        } else {
            if (dir == WTF::unicode::kRightToLeft)
                level_++;
            else if (dir == WTF::unicode::kArabicNumber || dir == WTF::unicode::kEuropeanNumber)
                level_ += 2;
        }
    }
    ...
    void SetNext(BidiCharacterRun* next)
    {
        next_ = next;
    }
