...
wtf_size_t HyphenationMinikin::LastHyphenLocation(const StringView& text, wtf_size_t before_index) const
{
    ...
    if (before_index <= num_leading_chars || !ShouldHyphenateWord(word))
        return 0;
    before_index = std::min<wtf_size_t>(before_index - num_leading_chars, word.length() - kMinimumSuffixLength);

    if (word.length() < kMinimumPrefixLength + kMinimumSuffixLength || before_index <= kMinimumPrefixLength)
        return 0;
...
Vector<wtf_size_t, 8> HyphenationMinikin::HyphenLocations(const StringView& text) const
{
    ...
    if (word.length() < kMinimumPrefixLength + kMinimumSuffixLength || !ShouldHyphenateWord(word))
        return hyphen_locations;

    Vector<uint8_t> result = Hyphenate(word);
    static_assert(kMinimumPrefixLength >= 1, "Change the 'if' above if this fails");
    for (wtf_size_t i = word.length() - kMinimumSuffixLength - 1; i >= kMinimumPrefixLength; i--) {
        if (result[i])
...
using LocaleMap = HashMap<AtomicString, const HyphenatorLocaleData*, CaseFoldingHash>;
