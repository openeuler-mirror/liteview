...
public:
    ...
    struct SchedulingSettings {
        ...
        CompositorTQPolicyDuringThreadedScroll compositor_tq_policy_during_threaded_scroll;
    };
    ...
    void DidHandleInputEventOnMainThread(const WebInputEvent& web_input_event, WebInputEventResult result);
    ...
    void SetHasVisibleRenderWidgetWithTouchHandler(bool has_visible_render_widget_with_touch_handler) override;
    ...
    void InitializeTaskAttributionTracker(std::unique_ptr<TaskAttributionTracker> tracker) override
    {
    ...
    scoped_refptr<MainThreadTaskQueue> NewLoadingTaskQueue(MainThreadTaskQueue::QueueType queue_type, FrameSchedulerImpl* frame_scheduler);
    ...
    base::sequence_manager::TaskQueue::QueuePriority ComputePriority(MainThreadTaskQueue* task_queue) const;
    ...
    void EndIdlePeriodForTesting(base::OnceClosure callback, base::TimeTicks time_remaining);
    ...
    {
        return main_thread_only().current_policy.should_prioritize_loading_with_compositing();
    }
...
private:
    ...
    void BeginAgentGroupSchedulerScope(WebAgentGroupScheduler* next_agent_group_scheduler);
    ...
    public:
        ...
        RAILMode rail_mode() const
        {
            return rail_mode_;
        }

        bool& frozen_when_backgrounded()
        {
            return frozen_when_backgrounded_;
        }
        bool frozen_when_backgrounded() const
        {
            return frozen_when_backgrounded_;
        }
        ...
        bool& should_defer_task_queues()
        {
            return should_defer_task_queues_;
        }
        bool should_defer_task_queues() const
        {
            return should_defer_task_queues_;
        }

        bool& should_pause_task_queues()
        {
            return should_pause_task_queues_;
        }
        bool should_pause_task_queues() const
        {
            return should_pause_task_queues_;
        }
        ...
        base::sequence_manager::TaskQueue::QueuePriority find_in_page_priority() const
        {
            return find_in_page_priority_;
        }
        ...
        {
            return rail_mode_ == other.rail_mode_ && frozen_when_backgrounded_ == other.frozen_when_backgrounded_
                && should_prioritize_loading_with_compositing_ == other.should_prioritize_loading_with_compositing_
                && should_freeze_compositor_task_queue_ == other.should_freeze_compositor_task_queue_
                && should_defer_task_queues_ == other.should_defer_task_queues_ && should_pause_task_queues_ == other.should_pause_task_queues_
                && should_pause_task_queues_for_android_webview_ == other.should_pause_task_queues_for_android_webview_
                && find_in_page_priority_ == other.find_in_page_priority_ && use_case_ == other.use_case_;
        }
    ...
    private:
        ...
        base::sequence_manager::TaskQueue::QueuePriority find_in_page_priority_ { FindInPageBudgetPoolController::kFindInPageBudgetNotExhaustedPriority };
    ...
    void MaybeUpdateCompositorTaskQueuePriorityOnTaskCompleted(MainThreadTaskQueue* queue, const base::sequence_manager::TaskQueue::TaskTiming& task_timing);
    ...
    absl::optional<TaskQueue::QueuePriority> ComputeCompositorPriorityFromUseCase() const;
    ...
    using TaskQueueVoterMap = std::map<scoped_refptr<MainThreadTaskQueue>, std::unique_ptr<base::sequence_manager::TaskQueue::QueueEnabledVoter>>;
    ...
    struct MainThreadOnly {
        ...
        TraceableCounter<base::TimeDelta, TracingCategory::kDebug> longest_jank_free_task_duration;
        TraceableCounter<int, TracingCategory::kInfo> renderer_pause_count; // Renderer is paused if non-zero.

        TraceableObjectState<RAILMode, TracingCategory::kTopLevel> rail_mode_for_tracing; // Don't use except for tracing.
        ...
        TraceableState<bool, TracingCategory::kDebug> have_reported_blocking_intervention_in_current_policy;
        TraceableState<bool, TracingCategory::kDebug> have_reported_blocking_intervention_since_navigation;
        TraceableState<bool, TracingCategory::kDebug> has_visible_render_widget_with_touch_handler;
        ...
        TraceableState<absl::optional<TaskDescriptionForTracing>, TracingCategory::kInfo> task_description_for_tracing; // Don't use except for tracing.
        TraceableState<absl::optional<base::sequence_manager::TaskQueue::QueuePriority>,
        ...
        // animation frame.
        TraceableState<bool, TracingCategory::kDefault> prioritize_compositing_after_input;
        ...
        TraceableState<TaskQueue::QueuePriority, TracingCategory::kDefault> compositor_priority;
