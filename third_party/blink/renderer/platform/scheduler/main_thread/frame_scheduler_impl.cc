...
FrameSchedulerImpl::FrameSchedulerImpl(MainThreadSchedulerImpl* main_thread_scheduler, PageSchedulerImpl* parent_page_scheduler,
    ...
    , task_queues_throttled_(false, "FrameScheduler.TaskQueuesThrottled", &tracing_controller_, YesNoStateToString)
    ...
    , page_visibility_for_tracing_(
          parent_page_scheduler_ && parent_page_scheduler_->IsPageVisible() ? PageVisibilityState::kVisible : PageVisibilityState::kHidden,
...
//, loading_power_mode_voter_(power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.Loading"))
{
...
void FrameSchedulerImpl::MoveTaskQueuesToCorrectWakeUpBudgetPool()
{
    ...
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
        ...
        auto* new_wake_up_budget_pool = parent_page_scheduler_->GetWakeUpBudgetPool(task_queue, frame_origin_type_, frame_visible_);
        if (task_queue->GetWakeUpBudgetPool() == new_wake_up_budget_pool) {
            continue;
        }

        parent_page_scheduler_->RemoveQueueFromWakeUpBudgetPool(task_queue, &lazy_now);
        parent_page_scheduler_->AddQueueToWakeUpBudgetPool(task_queue, frame_origin_type_, frame_visible_, &lazy_now);
    }
}
...
QueueTraits FrameSchedulerImpl::CreateQueueTraitsForTaskType(TaskType type)
{
    ...
    case TaskType::kLowPriorityScriptExecution:
        return LoadingTaskQueueTraits().SetPrioritisationType(QueueTraits::PrioritisationType::kBestEffort);
...
std::unique_ptr<WebResourceLoadingTaskRunnerHandle> FrameSchedulerImpl::CreateResourceLoadingTaskRunnerHandle()
{
    return CreateResourceLoadingTaskRunnerHandleImpl();
}

std::unique_ptr<WebResourceLoadingTaskRunnerHandle> FrameSchedulerImpl::CreateResourceLoadingMaybeUnfreezableTaskRunnerHandle()
{
    return ResourceLoadingTaskRunnerHandleImpl::WrapTaskRunner(GetTaskQueue(TaskType::kNetworkingUnfreezable));
}

std::unique_ptr<ResourceLoadingTaskRunnerHandleImpl> FrameSchedulerImpl::CreateResourceLoadingTaskRunnerHandleImpl()
{
    return ResourceLoadingTaskRunnerHandleImpl::WrapTaskRunner(GetTaskQueue(TaskType::kNetworking));
}
...
void FrameSchedulerImpl::DidCommitProvisionalLoad(bool is_web_history_inert_commit, NavigationType navigation_type)
{
...
void FrameSchedulerImpl::OnStartedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy)
{
...
void FrameSchedulerImpl::OnStoppedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy)
{
    if (policy.disable_aggressive_throttling)
...
void FrameSchedulerImpl::SetPageVisibilityForTracing(PageVisibilityState page_visibility)
{
...
void FrameSchedulerImpl::UpdatePolicy()
{
    ...
    for (const auto& task_queue_and_voter : frame_task_queue_controller_->GetAllTaskQueuesAndVoters()) {
        ...
        if (task_queues_throttled_ && !task_queues_were_throttled && task_queue_and_voter.first->CanBeThrottled()) {
...
TaskQueue::QueuePriority FrameSchedulerImpl::ComputePriority(MainThreadTaskQueue* task_queue) const
{
    DCHECK(task_queue);
    ...
    if (parent_page_scheduler_->IsBackgrounded()) {
        if (main_thread_scheduler_->scheduling_settings().low_priority_background_page) {
            return TaskQueue::QueuePriority::kLowPriority;
        }

        if (main_thread_scheduler_->scheduling_settings().best_effort_background_page) {
            return TaskQueue::QueuePriority::kBestEffortPriority;
        }
    }

    // Low priority feature enabled for hidden frame.
    if (main_thread_scheduler_->scheduling_settings().low_priority_hidden_frame && !IsFrameVisible()) {
        return TaskQueue::QueuePriority::kLowPriority;
    }

    bool is_subframe = GetFrameType() == FrameScheduler::FrameType::kSubframe;
    bool is_throttleable_task_queue = task_queue->queue_type() == MainThreadTaskQueue::QueueType::kFrameThrottleable;

    // Low priority feature enabled for sub-frame.
    if (main_thread_scheduler_->scheduling_settings().low_priority_subframe && is_subframe) {
        return TaskQueue::QueuePriority::kLowPriority;
    }

    // Low priority feature enabled for sub-frame throttleable task queues.
    if (main_thread_scheduler_->scheduling_settings().low_priority_subframe_throttleable && is_subframe && is_throttleable_task_queue) {
        return TaskQueue::QueuePriority::kLowPriority;
    }

    // Low priority feature enabled for throttleable task queues.
    if (main_thread_scheduler_->scheduling_settings().low_priority_throttleable && is_throttleable_task_queue) {
        return TaskQueue::QueuePriority::kLowPriority;
    }
    ...
    if (IsCrossOriginToNearestMainFrame() && main_thread_scheduler_->scheduling_settings().low_priority_cross_origin) {
        return TaskQueue::QueuePriority::kLowPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kLoadingControl) {
        return main_thread_scheduler_->should_prioritize_loading_with_compositing() ? TaskQueue::QueuePriority::kVeryHighPriority
                                                                                    : TaskQueue::QueuePriority::kHighPriority;
    }

    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kLoading
    ...
    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kInput
        && base::FeatureList::IsEnabled(::blink::features::kInputTargetClientHighPriority)) {
        return TaskQueue::QueuePriority::kHighestPriority;
    }
    ...
    if (task_queue->GetPrioritisationType() == MainThreadTaskQueue::QueueTraits::PrioritisationType::kExperimentalDatabase) {
        ...
        return parent_page_scheduler_->IsPageVisible() ? TaskQueue::QueuePriority::kHighPriority : TaskQueue::QueuePriority::kNormalPriority;
    }
...
uint64_t FrameSchedulerImpl::GetActiveFeaturesTrackedForBackForwardCacheMetricsMask() const
{
    return back_forward_cache_disabling_feature_tracker_.GetActiveFeaturesTrackedForBackForwardCacheMetricsMask();
}

base::WeakPtr<FrameOrWorkerScheduler> FrameSchedulerImpl::GetSchedulingAffectingFeatureWeakPtr()
{
...
std::unique_ptr<WebSchedulingTaskQueue> FrameSchedulerImpl::CreateWebSchedulingTaskQueue(WebSchedulingPriority priority)
{
    ...
    scoped_refptr<MainThreadTaskQueue> immediate_task_queue = frame_task_queue_controller_->NewWebSchedulingTaskQueue(DeferrableTaskQueueTraits(), priority);
    scoped_refptr<MainThreadTaskQueue> delayed_task_queue = frame_task_queue_controller_->NewWebSchedulingTaskQueue(
        DeferrableTaskQueueTraits().SetCanBeThrottled(true).SetCanBeIntensivelyThrottled(true), priority);
