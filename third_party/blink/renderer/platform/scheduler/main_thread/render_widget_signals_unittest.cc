...
public:
    ...
    MOCK_METHOD1(SetHasVisibleRenderWidgetWithTouchHandler, void(bool has_visible_render_widget_with_touch_handler));
};
...
TEST_F(RenderWidgetSignalsTest, TouchHandlerAddedAndRemoved_VisibleWidget)
{
    ...
    scoped_refptr<WidgetSchedulerImpl> widget_scheduler = base::MakeRefCounted<WidgetSchedulerImpl>(
        /*main_thread_scheduler_impl=*/nullptr, render_widget_signals_.get());
    Mock::VerifyAndClearExpectations(mock_observer_.get());

    EXPECT_CALL(*mock_observer_, SetHasVisibleRenderWidgetWithTouchHandler(true)).Times(1);
    widget_scheduler->SetHasTouchHandler(true);
    Mock::VerifyAndClearExpectations(mock_observer_.get());

    EXPECT_CALL(*mock_observer_, SetHasVisibleRenderWidgetWithTouchHandler(false)).Times(1);
...
TEST_F(RenderWidgetSignalsTest, TouchHandlerAddedThriceAndRemovedOnce_VisibleWidget)
{
    IgnoreWidgetCreationCallbacks();
...
TEST_F(RenderWidgetSignalsTest, TouchHandlerAddedAndRemoved_HiddenWidget)
{
    ...
    EXPECT_CALL(*mock_observer_, SetHasVisibleRenderWidgetWithTouchHandler(_)).Times(0);
    widget_scheduler->SetHasTouchHandler(true);
    widget_scheduler->SetHasTouchHandler(false);
    Mock::VerifyAndClearExpectations(mock_observer_.get());
