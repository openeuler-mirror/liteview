...
{
    static const base::FeatureParam<int> kFreezePurgeMemoryAllPagesFrozenDelayInMinutes { &blink::features::kFreezePurgeMemoryAllPagesFrozen,
...
{
    static const base::FeatureParam<int> kMinTimeToPurgeAfterBackgroundedInMinutes { &blink::features::kPurgeRendererMemoryWhenBackgrounded,
        "min-delay-in-minutes", MemoryPurgeManager::kDefaultMinTimeToPurgeAfterBackgrounded };
    return kMinTimeToPurgeAfterBackgroundedInMinutes.Get() * 60;
}

int MaxTimeToPurgeAfterBackgroundedInSeconds()
{
    static const base::FeatureParam<int> kMaxTimeToPurgeAfterBackgroundedInMinutes { &blink::features::kPurgeRendererMemoryWhenBackgrounded,
        "max-delay-in-minutes", MemoryPurgeManager::kDefaultMaxTimeToPurgeAfterBackgrounded };
...
{
    if (!base::FeatureList::IsEnabled(features::kPurgeRendererMemoryWhenBackgrounded))
        return;
...
{
    if (purge_timer_.IsRunning() && (purge_timer_.desired_run_time() - base::TimeTicks::Now()) < delay)
        return;
...
bool MemoryPurgeManager::CanPurge() const
{
    ...
    if (!AreAllPagesFrozen() && base::FeatureList::IsEnabled(features::kFreezePurgeMemoryAllPagesFrozen)) {
        return false;
    }
