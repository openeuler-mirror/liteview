...
PageSchedulerImpl::PageSchedulerImpl(PageScheduler::Delegate* delegate, AgentGroupSchedulerImpl& agent_group_scheduler)
    ...
    , throttle_foreground_timers_(base::FeatureList::IsEnabled(features::kThrottleForegroundTimers))
    , foreground_timers_throttled_wake_up_interval_(GetForegroundTimersThrottledWakeUpInterval())
{
    current_lifecycle_state_
        = (kDefaultPageVisibility == PageVisibilityState::kVisible ? PageLifecycleState::kActive : PageLifecycleState::kHiddenBackgrounded);
    ...
    do_freeze_page_callback_.Reset(base::BindRepeating(&PageSchedulerImpl::DoFreezePage, base::Unretained(this)));
}
...
void PageSchedulerImpl::SetPageVisible(bool page_visible)
{
    ...
    case PageVisibilityState::kHidden:
        SetPageLifecycleState(IsBackgrounded() ? PageLifecycleState::kHiddenBackgrounded : PageLifecycleState::kHiddenForegrounded);
        break;
    }
...
void PageSchedulerImpl::SetPageFrozenImpl(bool frozen, PageSchedulerImpl::NotificationPolicy notification_policy)
{
    ...
    if (notification_policy == PageSchedulerImpl::NotificationPolicy::kNotifyFrames)
        NotifyFrames();
    if (frozen) {
...
{
    return IsWaitingForMainFrameContentfulPaint() || IsWaitingForMainFrameMeaningfulPaint();
}
...
void PageSchedulerImpl::OnAudioSilent()
{
    ...
    if (ShouldFreezePage()) {
        main_thread_scheduler_->ControlTaskRunner()->PostDelayedTask(FROM_HERE, do_freeze_page_callback_.GetCallback(), delay_for_background_tab_freezing_);
    }
}
...
void PageSchedulerImpl::WriteIntoTrace(perfetto::TracedValue context, base::TimeTicks now) const
{
    ...
    dict.Add("normal_wake_up_budget_pool", [&](perfetto::TracedValue context) { normal_wake_up_budget_pool_->WriteIntoTrace(std::move(context), now); });
...
void PageSchedulerImpl::AddQueueToWakeUpBudgetPool(
    MainThreadTaskQueue* task_queue, FrameOriginType frame_origin_type, bool frame_visible, base::LazyNow* lazy_now)
{
    DCHECK(!task_queue->GetWakeUpBudgetPool());
    WakeUpBudgetPool* wake_up_budget_pool = GetWakeUpBudgetPool(task_queue, frame_origin_type, frame_visible);
...
WakeUpBudgetPool* PageSchedulerImpl::GetWakeUpBudgetPool(MainThreadTaskQueue* task_queue, FrameOriginType frame_origin_type, bool frame_visible)
{
    const bool can_be_intensively_throttled = task_queue->CanBeIntensivelyThrottled();
    const bool is_same_origin = frame_origin_type == FrameOriginType::kMainFrame || frame_origin_type == FrameOriginType::kSameOriginToMainFrame;
...
void PageSchedulerImpl::MaybeInitializeWakeUpBudgetPools(base::LazyNow* lazy_now)
{
    ...
    normal_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Normal Wake Up Throttling");
    cross_origin_hidden_normal_wake_up_budget_pool_ = std::make_unique<WakeUpBudgetPool>("Page - Normal Wake Up Throttling - Hidden & Crosss-Origin to Main "
                                                                                         "Frame");
...
void PageSchedulerImpl::UpdatePolicyOnVisibilityChange(NotificationPolicy notification_policy)
{
    base::LazyNow lazy_now(main_thread_scheduler_->GetTickClock());
...
void PageSchedulerImpl::UpdateWakeUpBudgetPools(base::LazyNow* lazy_now)
{
    ...
    normal_wake_up_budget_pool_->SetWakeUpInterval(
        lazy_now->Now(), IsBackgrounded() ? kDefaultThrottledWakeUpInterval : foreground_timers_throttled_wake_up_interval_);
    cross_origin_hidden_normal_wake_up_budget_pool_->SetWakeUpInterval(lazy_now->Now(), kDefaultThrottledWakeUpInterval);
...
bool PageSchedulerImpl::HasWakeUpBudgetPools() const
{
    ...
    DCHECK_EQ(!!normal_wake_up_budget_pool_, !!cross_origin_hidden_normal_wake_up_budget_pool_);
    DCHECK_EQ(!!normal_wake_up_budget_pool_, !!same_origin_intensive_wake_up_budget_pool_);
    DCHECK_EQ(!!normal_wake_up_budget_pool_, !!cross_origin_intensive_wake_up_budget_pool_);
...
{
    return { normal_wake_up_budget_pool_.get(), cross_origin_hidden_normal_wake_up_budget_pool_.get(), same_origin_intensive_wake_up_budget_pool_.get(),
