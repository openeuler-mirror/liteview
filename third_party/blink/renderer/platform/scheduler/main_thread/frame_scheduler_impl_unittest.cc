...
std::unique_ptr<PageSchedulerImpl> CreatePageScheduler(
    PageScheduler::Delegate* page_scheduler_delegate, MainThreadSchedulerImpl* scheduler, WebAgentGroupScheduler& agent_group_scheduler)
{
    std::unique_ptr<PageScheduler> page_scheduler = agent_group_scheduler.AsAgentGroupScheduler().CreatePageScheduler(page_scheduler_delegate);
...
constexpr TaskType kAllFrameTaskTypes[] = { TaskType::kInternalContentCapture, TaskType::kJavascriptTimerImmediate, TaskType::kJavascriptTimerDelayedLowNesting,
    ...
    TaskType::kNetworkingControl, TaskType::kLowPriorityScriptExecution, TaskType::kDOMManipulation, TaskType::kHistoryTraversal, TaskType::kEmbed,
    TaskType::kCanvasBlobSerialization, TaskType::kRemoteEvent, TaskType::kWebSocket, TaskType::kMicrotask, TaskType::kUnshippedPortMessage,
    TaskType::kFileReading, TaskType::kPresentation, TaskType::kSensor, TaskType::kPerformanceTimeline, TaskType::kWebGL, TaskType::kIdleTask,
    TaskType::kInternalDefault, TaskType::kMiscPlatformAPI, TaskType::kFontLoading, TaskType::kApplicationLifeCycle, TaskType::kBackgroundFetch,
    TaskType::kPermission, TaskType::kPostedMessage, TaskType::kServiceWorkerClientMessage, TaskType::kWorkerAnimation, TaskType::kUserInteraction,
    TaskType::kMediaElementEvent, TaskType::kInternalWebCrypto, TaskType::kInternalMedia, TaskType::kInternalMediaRealTime, TaskType::kInternalUserInteraction,
    TaskType::kInternalIntersectionObserver, TaskType::kInternalFindInPage, TaskType::kInternalContinueScriptLoading, TaskType::kDatabaseAccess,
    TaskType::kInternalNavigationAssociated, TaskType::kInternalTest, TaskType::kWebLocks, TaskType::kInternalFrameLifecycleControl,
    ...
    TaskType::kInternalInputBlocking, TaskType::kWakeLock, TaskType::kWebGPU, TaskType::kInternalPostMessageForwarding,
    TaskType::kInternalNavigationCancellation };

static_assert(static_cast<int>(TaskType::kMaxValue) == 81, "When adding a TaskType, make sure that kAllFrameTaskTypes is updated.");
...
public:
    ...
    base::TimeDelta GetTaskTime()
    {
        return frame_scheduler_->task_time_;
    }
    ...
    static uint64_t GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(FrameSchedulerImpl* frame_scheduler)
    {
        return frame_scheduler->GetActiveFeaturesTrackedForBackForwardCacheMetricsMask();
    }
...
protected:
    ...
    std::unique_ptr<ResourceLoadingTaskRunnerHandleImpl> GetResourceLoadingTaskRunnerHandleImpl()
    {
...
public:
    ...
    scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(FrameSchedulerImpl* frame_scheduler)
    {
        ...
        if (task_type == TaskType::kWebSchedulingPostedTask) {
            test_web_scheduling_task_queues_.push_back(frame_scheduler->CreateWebSchedulingTaskQueue(WebSchedulingPriority::kUserVisiblePriority));
...
TEST_F(FrameSchedulerImplTest, FramePostsCpuTasksThroughReloadRenavigate)
{
    ...
    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(String::Format("FrameType: %d, NavigationType: %d : TaskTime.is_zero %d, CallCount %d", test_case.frame_type, test_case.navigation_type,
...
public:
    InputHighPriorityFrameSchedulerImplTest()
        : FrameSchedulerImplTest({}, { ::blink::features::kInputTargetClientHighPriority })
    {
    }
};
...
TEST_F(InputHighPriorityFrameSchedulerImplTest, NormalPriorityInputBlockingTaskQueue)
{
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    page_scheduler_->SetPageVisible(true);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
}
...
{
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kHighestPriority);
    page_scheduler_->SetPageVisible(true);
    EXPECT_EQ(InputBlockingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kHighestPriority);
}
...
{
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(LoadingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);
    EXPECT_EQ(LoadingControlTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);
    EXPECT_EQ(DeferrableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);
    EXPECT_EQ(ThrottleableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);
    EXPECT_EQ(PausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);
    EXPECT_EQ(UnpausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kLowPriority);

    page_scheduler_->AudioStateChanged(true);
    EXPECT_EQ(LoadingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    EXPECT_EQ(LoadingControlTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kHighPriority);
    EXPECT_EQ(DeferrableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    EXPECT_EQ(ThrottleableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    EXPECT_EQ(PausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    EXPECT_EQ(UnpausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
...
class BestEffortPriorityBackgroundPageExperimentTest : public FrameSchedulerImplTest {
public:
...
{
    page_scheduler_->SetPageVisible(false);
    EXPECT_EQ(LoadingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
    EXPECT_EQ(LoadingControlTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
    EXPECT_EQ(DeferrableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
    EXPECT_EQ(ThrottleableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
    EXPECT_EQ(PausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
    EXPECT_EQ(UnpausableTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kBestEffortPriority);
...
class LowPrioritySubFrameThrottleableTaskExperimentTest : public FrameSchedulerImplTest {
public:
...
TEST_F(LowPrioritySubFrameThrottleableTaskExperimentTest, FrameQueuesPriorities)
{
    // Sub-Frame Task Queues.
    EXPECT_EQ(LoadingTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
    EXPECT_EQ(LoadingControlTaskQueue()->GetQueuePriority(), TaskQueue::QueuePriority::kHighPriority);
...
class LowPriorityThrottleableTaskExperimentTest : public FrameSchedulerImplTest {
public:
...
TEST_F(FrameSchedulerImplDatabaseAccessWithoutHighPriority, QueueTraits)
{
    ...
    EXPECT_EQ(da_queue->GetQueuePriority(), TaskQueue::QueuePriority::kNormalPriority);
}
...
TEST_F(FrameSchedulerImplDatabaseAccessWithHighPriority, QueueTraits)
{
    ...
    EXPECT_EQ(da_queue->GetQueuePriority(), TaskQueue::QueuePriority::kHighPriority);
}
...
TEST_F(FrameSchedulerImplTest, ContentCaptureHasIdleTaskQueue)
{
    ...
    EXPECT_EQ(TaskQueue::QueuePriority::kBestEffortPriority, task_queue->GetQueuePriority());
}
...
TEST_F(FrameSchedulerImplTest, LowPriorityScriptExecutionHasBestEffortPriority)
{
...
{
    uint64_t result = 0;
    for (SchedulingPolicy::Feature feature : frame_scheduler->GetActiveFeaturesTrackedForBackForwardCacheMetrics()) {
...
TEST_F(FrameSchedulerImplTest, BackForwardCacheOptOut)
{
    ...
    EXPECT_EQ(ComputeMaskFromFeatures(frame_scheduler_.get()), GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(frame_scheduler_.get()));
...
TEST_F(FrameSchedulerImplTest, BackForwardCacheOptOut_FrameNavigated)
{
    ...
    auto feature_handle = frame_scheduler_->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket));
    EXPECT_EQ(ComputeMaskFromFeatures(frame_scheduler_.get()), GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(frame_scheduler_.get()));

    frame_scheduler_->RegisterStickyFeature(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(),
        testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket, SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));
    EXPECT_EQ(ComputeMaskFromFeatures(frame_scheduler_.get()), GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(frame_scheduler_.get()));

    // Same document navigations don't affect anything.
    frame_scheduler_->DidCommitProvisionalLoad(false, FrameScheduler::NavigationType::kSameDocument);
    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(),
        testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket, SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));
    EXPECT_EQ(ComputeMaskFromFeatures(frame_scheduler_.get()), GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(frame_scheduler_.get()));

    // Regular navigations reset all features.
    frame_scheduler_->DidCommitProvisionalLoad(false, FrameScheduler::NavigationType::kOther);
    EXPECT_THAT(frame_scheduler_->GetActiveFeaturesTrackedForBackForwardCacheMetrics(), testing::UnorderedElementsAre());
    EXPECT_EQ(ComputeMaskFromFeatures(frame_scheduler_.get()), GetActiveFeaturesTrackedForBackForwardCacheMetricsMask(frame_scheduler_.get()));
            ...
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate) {
                    ...
                    EXPECT_CALL(*delegate,
                        UpdateBackForwardCacheDisablingFeatures((1 << static_cast<size_t>(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore))
                            | (1 << static_cast<size_t>(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache))));
                },
        ...
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate, FeatureHandle* feature_handle) {
                    *feature_handle = frame_scheduler->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, { SchedulingPolicy::DisableBackForwardCache() });
                    // Ensure that the feature upload is delayed.
                    testing::Mock::VerifyAndClearExpectations(delegate);
                    EXPECT_CALL(*delegate, UpdateBackForwardCacheDisablingFeatures((1 << static_cast<size_t>(SchedulingPolicy::Feature::kWebSocket))));
                },
        ...
        ->PostTask(FROM_HERE,
            base::BindOnce(
                [](FrameSchedulerImpl* frame_scheduler, testing::StrictMock<FrameSchedulerDelegateForTesting>* delegate, FeatureHandle* feature_handle) {
                    ...
                    // destruction.
                    testing::Mock::VerifyAndClearExpectations(delegate);
                    EXPECT_CALL(*delegate, UpdateBackForwardCacheDisablingFeatures(testing::_)).Times(0);
                },
                frame_scheduler_.get(), frame_scheduler_delegate_.get(), &feature_handle));
...
public:
    void SetUp() override
    {
        ...
        for (int i = 0; i <= static_cast<int>(WebSchedulingPriority::kLastPriority); i++) {
            ...
            std::unique_ptr<WebSchedulingTaskQueue> task_queue = frame_scheduler_->CreateWebSchedulingTaskQueue(priority);
...
protected:
    ...
    // - 'V': UserVisible
    // - 'B': Background
    void PostWebSchedulingTestTasks(Vector<String>* run_order, const String& task_descriptor, base::TimeDelta delay = base::TimeDelta())
    {
        std::istringstream stream(task_descriptor.Utf8());
        while (!stream.eof()) {
            std::string task;
            ...
            web_scheduling_task_runners_[static_cast<int>(priority)]->PostDelayedTask(
                FROM_HERE, base::BindOnce(&AppendToVectorTestTask, run_order, String::FromUTF8(task)), delay);
        }
    }

    Vector<scoped_refptr<base::SingleThreadTaskRunner>> web_scheduling_task_runners_;
...
TEST_F(WebSchedulingTaskQueueTest, TasksRunInPriorityOrder)
{
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("U1", "U2", "V1", "V2", "B1", "B2"));
}
...
TEST_F(WebSchedulingTaskQueueTest, DynamicTaskPriorityOrder)
{
    ...
    PostWebSchedulingTestTasks(&run_order, "B1 B2 V1 V2 U1 U2");
    task_queues_[static_cast<int>(WebSchedulingPriority::kUserBlockingPriority)]->SetPriority(WebSchedulingPriority::kBackgroundPriority);

    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(run_order, testing::ElementsAre("V1", "V2", "B1", "B2", "U1", "U2"));
}
...
TEST_F(FrameSchedulerImplTest, ThrottledJSTimerTasksRunTime)
{
    ...
    std::unique_ptr<WebSchedulingTaskQueue> web_scheduling_task_queue
        = frame_scheduler_->CreateWebSchedulingTaskQueue(WebSchedulingPriority::kUserVisiblePriority);
...
TEST_F(FrameSchedulerImplTest, ReportFMPAndFCPForMainFrames)
{
    ...
    std::unique_ptr<WebAgentGroupScheduler> agent_group_scheduler = mock_main_thread_scheduler.CreateAgentGroupScheduler();
...
TEST_P(FrameSchedulerImplTestWithIntensiveWakeUpThrottling, TaskExecutionSameOriginFrame)
{
    ...
    // Test that wake ups are 1-second aligned if there is no recent wake up.
    {
        ...
        task_environment_.FastForwardBy(kDefaultThrottledWakeUpInterval);
        EXPECT_THAT(run_times, testing::ElementsAre(scope_start + base::Seconds(1)));
    }
    ...
    {
        const base::TimeTicks scope_start = base::TimeTicks::Now();
        EXPECT_EQ(scope_start, test_start + base::Minutes(12) + kDefaultThrottledWakeUpInterval);
        std::vector<base::TimeTicks> run_times;
...
{
    EXPECT_EQ(base::Seconds(kIntensiveWakeUpThrottling_GracePeriodSeconds_Loaded), GetIntensiveWakeUpThrottlingGracePeriod(false));
}
...
TEST_F(FrameSchedulerImplTest, ImmediateWebSchedulingTasksAreNotThrottled)
{
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> task_runner
        = frame_scheduler_->CreateWebSchedulingTaskQueue(WebSchedulingPriority::kUserVisiblePriority)->GetTaskRunner();
...
class FrameSchedulerImplThrottleForegroundTimersEnabledTest : public FrameSchedulerImplTest {
public:
...
TEST_F(FrameSchedulerImplThrottleForegroundTimersEnabledTest, ForegroundPageTimerThrottling)
{
    page_scheduler_->SetPageVisible(true);
...
TEST_F(FrameSchedulerImplThrottleForegroundTimersEnabledTest, BackgroundPageTimerThrottling)
{
    page_scheduler_->SetPageVisible(false);
    ...
    EXPECT_THAT(run_times,
        testing::ElementsAre(
            start, start + base::Milliseconds(1000), start + base::Milliseconds(1000), start + base::Milliseconds(1000), start + base::Milliseconds(1000)));
}

TEST_F(FrameSchedulerImplThrottleForegroundTimersEnabledTest, HiddenAudiblePageTimerThrottling)
{
    page_scheduler_->SetPageVisible(false);
...
TEST_F(FrameSchedulerImplThrottleForegroundTimersEnabledTest, VisibleCrossOriginFrameThrottling)
{
    std::unique_ptr<FrameSchedulerImpl> cross_origin_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    page_scheduler_->SetPageVisible(true);
    ...
    const scoped_refptr<base::SingleThreadTaskRunner> cross_origin_task_runner
        = cross_origin_frame_scheduler->GetTaskRunner(TaskType::kJavascriptTimerDelayedLowNesting);
    ...
    std::vector<base::TimeTicks> run_times;
    for (int i = 0; i < 5; i++) {
        cross_origin_task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&RecordRunTime, &run_times), base::Milliseconds(50) * i);
    }
...
TEST_F(FrameSchedulerImplThrottleForegroundTimersEnabledTest, HiddenCrossOriginFrameThrottling)
{
    std::unique_ptr<FrameSchedulerImpl> cross_origin_frame_scheduler = CreateFrameScheduler(page_scheduler_.get(), frame_scheduler_delegate_.get(),
        /*is_in_embedded_frame_tree=*/false, FrameScheduler::FrameType::kSubframe);
    page_scheduler_->SetPageVisible(true);
...
TEST_F(FrameSchedulerImplTest, PostMessageForwardingHasVeryHighPriority)
{
    ...
    EXPECT_EQ(TaskQueue::QueuePriority::kVeryHighPriority, task_queue->GetQueuePriority());
}
