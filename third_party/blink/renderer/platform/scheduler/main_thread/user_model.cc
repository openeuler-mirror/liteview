...
{
    base::TimeDelta escalated_priority_duration = base::Milliseconds(kGestureEstimationLimitMillis);
...
bool UserModel::IsGestureExpectedSoonImpl(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const
{
    if (is_gesture_active_) {
        if (IsGestureExpectedToContinue(now, prediction_valid_duration))
            return false;
        *prediction_valid_duration = base::Milliseconds(kExpectSubsequentGestureMillis);
        return true;
    } else {
        // If we have finished a gesture then a subsequent gesture is deemed likely.
        base::TimeDelta expect_subsequent_gesture_for = base::Milliseconds(kExpectSubsequentGestureMillis);
...
bool UserModel::IsGestureExpectedToContinue(const base::TimeTicks now, base::TimeDelta* prediction_valid_duration) const
{
    ...
    base::TimeDelta median_gesture_duration = base::Milliseconds(kMedianGestureDurationMillis);
