...
std::unique_ptr<PageSchedulerImpl> CreatePageScheduler(
    PageScheduler::Delegate* page_scheduler_delegate, ThreadSchedulerBase* scheduler, WebAgentGroupScheduler& agent_group_scheduler)
{
...
void AnticipationTestTask(MainThreadSchedulerImpl* scheduler, SimulateInputType simulate_input, bool* is_anticipated_before, bool* is_anticipated_after)
{
    *is_anticipated_before = scheduler->IsHighPriorityWorkAnticipated();
    ...
    case SimulateInputType::kTouchStart:
        scheduler->DidHandleInputEventOnCompositorThread(
            FakeTouchEvent(blink::WebInputEvent::Type::kTouchStart), InputEventState::EVENT_CONSUMED_BY_COMPOSITOR);
        break;

    case SimulateInputType::kTouchEnd:
        scheduler->DidHandleInputEventOnCompositorThread(FakeInputEvent(blink::WebInputEvent::Type::kTouchEnd), InputEventState::EVENT_CONSUMED_BY_COMPOSITOR);
        break;
...
public:
    ...
    void UpdatePolicyLocked(UpdateType update_type) override
    {
        ...
        String use_case = MainThreadSchedulerImpl::UseCaseToString(main_thread_only().current_use_case);
...
public:
    ...
    explicit MainThreadSchedulerImplTest(std::vector<FeatureAndParams> features_to_enable)
    {
    ...
    void SetUp() override
    {
        ...
        Initialize(std::make_unique<MainThreadSchedulerImplForTest>(base::sequence_manager::SequenceManagerForTest::Create(nullptr, test_task_runner_,
            test_task_runner_->GetMockTickClock(), base::sequence_manager::SequenceManager::Settings::Builder().SetRandomisedSamplingEnabled(true).Build())));
    ...
    void Initialize(std::unique_ptr<MainThreadSchedulerImplForTest> scheduler)
    {
        ...
        if (!scheduler_->scheduling_settings().mbi_compositor_task_runner_per_agent_scheduling_group) {
            compositor_task_runner_ = scheduler_->CompositorTaskQueue()->GetTaskRunnerWithDefaultTaskType();
        }
        idle_task_runner_ = scheduler_->IdleTaskRunner();
        v8_task_runner_ = scheduler_->V8TaskQueue()->GetTaskRunnerWithDefaultTaskType();

        agent_group_scheduler_
            = std::unique_ptr<AgentGroupSchedulerImpl>(static_cast<AgentGroupSchedulerImpl*>(scheduler_->CreateAgentGroupScheduler().release()));
        if (scheduler_->scheduling_settings().mbi_compositor_task_runner_per_agent_scheduling_group) {
            compositor_task_runner_ = agent_group_scheduler_->CompositorTaskQueue()->GetTaskRunnerWithDefaultTaskType();
        }
        page_scheduler_ = std::make_unique<NiceMock<MockPageSchedulerImpl>>(scheduler_.get(), static_cast<AgentGroupSchedulerImpl&>(*agent_group_scheduler_));
    ...
    void ForceBlockingInputToBeExpectedSoon()
    {
        ...
        scheduler_->DidHandleInputEventOnCompositorThread(
            FakeInputEvent(blink::WebInputEvent::Type::kGestureScrollEnd), InputEventState::EVENT_CONSUMED_BY_COMPOSITOR);
        test_task_runner_->AdvanceMockTickClock(priority_escalation_after_input_duration() * 2);
    ...
    {
        scoped_refptr<MainThreadTaskQueue> fake_queue = scheduler_->NewLoadingTaskQueue(MainThreadTaskQueue::QueueType::kFrameLoading, nullptr);
...
protected:
    ...
    {
        return base::Milliseconds(MainThreadSchedulerImpl::kRailsResponseTimeMillis);
    }
    ...
    {
        CallForEachEnumValue<UseCase>(UseCase::kFirstUseCase, UseCase::kCount, &MainThreadSchedulerImpl::UseCaseToString);
    }
...
TEST_F(MainThreadSchedulerImplTest, TestCompositorPolicy_CompositorHandlesInput_LongGestureDuration)
{
    ...
    base::TimeTicks loop_end_time = Now() + base::Milliseconds(UserModel::kMedianGestureDurationMillis * 2);
...
TEST_F(MainThreadSchedulerImplTest, NonOrdinaryPageDoesNotTriggerLoadingUseCase)
{
    // `IsWaitingForMainFrame(Contentful|Meaningful)Paint return true for a new
    // page scheduler in production.
    ON_CALL(*page_scheduler_, IsWaitingForMainFrameContentfulPaint).WillByDefault(Return(true));
...
class PrioritizeCompositingAndLoadingInUseCaseLoadingTest : public MainThreadSchedulerImplTest {
public:
    PrioritizeCompositingAndLoadingInUseCaseLoadingTest()
        : MainThreadSchedulerImplTest({ kPrioritizeCompositingAndLoadingDuringEarlyLoading }, {})
    {
    }
};
...
TEST_F(PrioritizeCompositingAndLoadingInUseCaseLoadingTest, LoadingUseCase)
{
    ...
    // other tasks.
    String early_loading_policy_expected[] = { "C1", "L1", "C2", "L2", "D1", "T1", "D2", "T2", "I1" };
    EXPECT_THAT(run_order, testing::ElementsAreArray(early_loading_policy_expected));
    EXPECT_EQ(UseCase::kEarlyLoading, ForceUpdatePolicyAndGetCurrentUseCase());
    ...
    String default_order_expected[] = { "D1", "C1", "T1", "L1", "D2", "C2", "T2", "L2", "I1" };
...
TEST_F(MainThreadSchedulerImplTest, EventForwardedToMainThread_MouseMove_WhenMouseDown)
{
    ...
    scheduler_->DidHandleInputEventOnMainThread(
        FakeInputEvent(blink::WebInputEvent::Type::kMouseMove, blink::WebInputEvent::kLeftButtonDown), WebInputEventResult::kHandledSystem);
}
...
TEST_F(MainThreadSchedulerImplTest, TestIsHighPriorityWorkAnticipated)
{
    ...
    default_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kNone, &is_anticipated_before, &is_anticipated_after));
    base::RunLoop().RunUntilIdle();
    ...
    default_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kTouchStart, &is_anticipated_before, &is_anticipated_after));
    bool dummy;
    default_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kTouchEnd, &dummy, &dummy));
    default_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kGestureScrollBegin, &dummy, &dummy));
    default_task_runner_->PostTask(FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kGestureScrollEnd, &dummy, &dummy));
    ...
    test_task_runner_->AdvanceMockTickClock(subsequent_input_expected_after_input_duration() * 2);
    default_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&AnticipationTestTask, scheduler_.get(), SimulateInputType::kNone, &is_anticipated_before, &is_anticipated_after));
    base::RunLoop().RunUntilIdle();
...
TEST_F(MainThreadSchedulerImplTest, SlowMainThreadInputEvent)
{
    ...
    test_task_runner_->FastForwardBy(priority_escalation_after_input_duration() * 2);
    EXPECT_EQ(UseCase::kNone, CurrentUseCase());
}
...
TEST_F(MainThreadSchedulerImplTest, GetTaskExecutorForCurrentThreadInPostedTask)
{
...
TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_NONE)
{
    EXPECT_EQ(UseCase::kNone, CurrentUseCase());
    EXPECT_EQ(rails_response_time(), scheduler_->EstimateLongestJankFreeTaskDuration());
}

TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_kCompositorGesture)
{
    SimulateCompositorGestureStart(TouchEventPolicy::kDontSendTouchStart);
    EXPECT_EQ(UseCase::kCompositorGesture, ForceUpdatePolicyAndGetCurrentUseCase());
    EXPECT_EQ(rails_response_time(), scheduler_->EstimateLongestJankFreeTaskDuration());
...
TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_EarlyLoading)
{
    ON_CALL(*page_scheduler_, IsWaitingForMainFrameContentfulPaint).WillByDefault(Return(true));
    ON_CALL(*page_scheduler_, IsWaitingForMainFrameMeaningfulPaint).WillByDefault(Return(true));
    scheduler_->DidStartProvisionalLoad(true);
...
TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_Loading)
{
    ON_CALL(*page_scheduler_, IsWaitingForMainFrameContentfulPaint).WillByDefault(Return(false));
    ON_CALL(*page_scheduler_, IsWaitingForMainFrameMeaningfulPaint).WillByDefault(Return(true));
    scheduler_->OnMainFramePaint();
...
TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_MAIN_THREAD_GESTURE)
{
    SimulateMainThreadGestureStart(TouchEventPolicy::kSendTouchStart, blink::WebInputEvent::Type::kGestureScrollUpdate);
    viz::BeginFrameArgs begin_frame_args = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, 0, next_begin_frame_number_++, Now(), base::TimeTicks(), base::Milliseconds(16), viz::BeginFrameArgs::NORMAL);
    ...
    compositor_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MainThreadSchedulerImplTest::SimulateMainThreadInputHandlingCompositorTask, base::Unretained(this), base::Milliseconds(5)));

    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(UseCase::kMainThreadGesture, CurrentUseCase());

    // 16ms frame - 5ms compositor work = 11ms for other stuff.
    EXPECT_EQ(base::Milliseconds(11), scheduler_->EstimateLongestJankFreeTaskDuration());
}

TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_MAIN_THREAD_CUSTOM_INPUT_HANDLING)
{
    viz::BeginFrameArgs begin_frame_args = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, 0, next_begin_frame_number_++, Now(), base::TimeTicks(), base::Milliseconds(16), viz::BeginFrameArgs::NORMAL);
    begin_frame_args.on_critical_path = false;
...
TEST_F(MainThreadSchedulerImplTest, EstimateLongestJankFreeTaskDuration_UseCase_SYNCHRONIZED_GESTURE)
{
    SimulateCompositorGestureStart(TouchEventPolicy::kDontSendTouchStart);
    ...
    compositor_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&MainThreadSchedulerImplTest::SimulateMainThreadCompositorTask, base::Unretained(this), base::Milliseconds(5)));
...
TEST_F(MainThreadSchedulerImplTest, TestCompositorPolicy_TouchStartDuringFling)
{
    scheduler_->SetHasVisibleRenderWidgetWithTouchHandler(true);
    scheduler_->DidAnimateForInputOnCompositorThread();
    // Note DidAnimateForInputOnCompositorThread does not by itself trigger a
...
TEST_F(MainThreadSchedulerImplTest, SYNCHRONIZED_GESTURE_CompositingExpensive)
{
    ...
    // tasks.
    EXPECT_EQ(TaskQueue::kNormalPriority, compositor_task_queue()->GetQueuePriority());
...
TEST_F(MainThreadSchedulerImplTest, MAIN_THREAD_GESTURE)
{
    ...
    EXPECT_EQ(TaskQueue::kHighestPriority, compositor_task_queue()->GetQueuePriority());
...
TEST_F(MainThreadSchedulerImplTest, FreezesCompositorQueueWhenAllPagesFrozen)
{
    ...
    std::unique_ptr<PageScheduler> sched_1 = agent_group_scheduler_->AsAgentGroupScheduler().CreatePageScheduler(nullptr);
    sched_1->SetPageVisible(false);
    std::unique_ptr<PageScheduler> sched_2 = agent_group_scheduler_->AsAgentGroupScheduler().CreatePageScheduler(nullptr);
    ...
    run_order.clear();
    std::unique_ptr<PageScheduler> sched_3 = agent_group_scheduler_->AsAgentGroupScheduler().CreatePageScheduler(nullptr);
    ...
    void SetUp() override
    {
        ...
        auto main_thread_scheduler = std::make_unique<MainThreadSchedulerImplForTest>(
            base::sequence_manager::SequenceManagerForTest::Create(nullptr, test_task_runner_, test_task_runner_->GetMockTickClock(),
                base::sequence_manager::SequenceManager::Settings::Builder().SetRandomisedSamplingEnabled(true).Build()));
...
class MainThreadSchedulerImplWithCompositingAfterInputPrioritizationTest : public MainThreadSchedulerImplTest {
public:
...
TEST_F(MainThreadSchedulerImplWithCompositingAfterInputPrioritizationTest, CompositingAfterInput)
{
    Vector<String> run_order;
...
TEST_F(MainThreadSchedulerImplWithCompositingAfterInputPrioritizationTest, CompositorNotPrioritizedAfterContinuousInput)
{
    Vector<String> run_order;
...
TEST_F(BestEffortPriorityForFindInPageExperimentTest, FindInPageTasksAreBestEffortPriorityUnderExperiment)
{
    ...
    EnableIdleTasks();
    EXPECT_EQ(scheduler_->find_in_page_priority(), QueuePriority::kBestEffortPriority);
    base::RunLoop().RunUntilIdle();
...
struct CompositorTQPolicyDuringThreadedScrollTestParam {
    CompositorTQPolicyDuringThreadedScrollTestParam(CompositorTQPolicyDuringThreadedScroll policy, Vector<String> initial_order)
        : policy(policy)
        , expected_initial_order(std::move(initial_order))
    {
    }
...
class CompositorTQPolicyDuringThreadedScrollTest : public MainThreadSchedulerImplTest,
                                                   public ::testing::WithParamInterface<CompositorTQPolicyDuringThreadedScrollTestParam> {
public:
    CompositorTQPolicyDuringThreadedScrollTest()
    {
        if (GetParam().policy != CompositorTQPolicyDuringThreadedScroll::kLowPriorityAlways) {
            feature_list_.Reset();
            feature_list_.InitWithFeaturesAndParameters(
                { { kThreadedScrollPreventRenderingStarvation,
                    base::FieldTrialParams({ { "policy", kCompositorTQPolicyDuringThreadedScroll.GetName(GetParam().policy) } }) } },
                {});
        }
    }
};
...
INSTANTIATE_TEST_CASE_P(, CompositorTQPolicyDuringThreadedScrollTest,
    testing::Values(CompositorTQPolicyDuringThreadedScrollTestParam(
                        CompositorTQPolicyDuringThreadedScroll::kLowPriorityWithAntiStarvation, Vector<String>({ "D1", "D2", "C1", "C2" })),
        CompositorTQPolicyDuringThreadedScrollTestParam(
            CompositorTQPolicyDuringThreadedScroll::kNormalPriorityWithAntiStarvation, Vector<String>({ "D1", "C1", "D2", "C2" })),
        CompositorTQPolicyDuringThreadedScrollTestParam(
            CompositorTQPolicyDuringThreadedScroll::kVeryHighPriorityAlways, Vector<String>({ "C1", "C2", "D1", "D2" })),
        CompositorTQPolicyDuringThreadedScrollTestParam(
            CompositorTQPolicyDuringThreadedScroll::kLowPriorityAlways, Vector<String>({ "D1", "D2", "C1", "C2" }))),
    [](const testing::TestParamInfo<CompositorTQPolicyDuringThreadedScrollTestParam>& info) {
