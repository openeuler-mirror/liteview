...
public:
    ...
    explicit MainThreadTaskQueueForTest(QueueTraits::PrioritisationType prioritisation_type)
        : MainThreadTaskQueue(nullptr, base::sequence_manager::TaskQueue::Spec(MainThreadTaskQueue::NameForQueueType(MainThreadTaskQueue::QueueType::kTest)),
            QueueCreationParams(MainThreadTaskQueue::QueueType::kTest).SetQueueTraits(QueueTraits().SetPrioritisationType(prioritisation_type)), nullptr)
    {
    }
    explicit MainThreadTaskQueueForTest(QueueType queue_type)
        : MainThreadTaskQueue(nullptr, base::sequence_manager::TaskQueue::Spec(MainThreadTaskQueue::NameForQueueType(MainThreadTaskQueue::QueueType::kTest)),
            QueueCreationParams(queue_type), nullptr)
    {
    }
};
...
public:
    ...
    void DidCommitProvisionalLoad(bool is_web_history_inert_commit, FrameScheduler::NavigationType navigation_type) override
    {
    }
