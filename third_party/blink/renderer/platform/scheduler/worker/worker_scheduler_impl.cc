...
WorkerSchedulerImpl::WorkerSchedulerImpl(WorkerThreadScheduler* worker_thread_scheduler, WorkerSchedulerProxy* proxy)
    : throttleable_task_queue_(worker_thread_scheduler->CreateTaskQueue(base::sequence_manager::QueueName::WORKER_THROTTLEABLE_TQ, true))
...
void WorkerSchedulerImpl::OnStartedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy)
{
...
void WorkerSchedulerImpl::OnStoppedUsingFeature(SchedulingPolicy::Feature feature, const SchedulingPolicy& policy)
{
    if (!policy.disable_back_forward_cache) {
        return;
    }
...
base::WeakPtr<FrameOrWorkerScheduler> WorkerSchedulerImpl::GetSchedulingAffectingFeatureWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

std::unique_ptr<WebSchedulingTaskQueue> WorkerSchedulerImpl::CreateWebSchedulingTaskQueue(WebSchedulingPriority priority)
{
