...
public:
    ...
    {
        worker_scheduler_->GetTaskRunner(task_type)->PostTask(FROM_HERE, WTF::BindOnce(&AppendToVectorTestTask, WTF::Unretained(run_order), task_descriptor));
    }
            ...
            base::BindOnce(
                [](WorkerSchedulerImpl* worker_scheduler, testing::StrictMock<WorkerSchedulerDelegateForTesting>* delegate) {
                    ...
                    EXPECT_CALL(*delegate,
                        UpdateBackForwardCacheDisablingFeatures((1 << static_cast<uint64_t>(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore))
                            | (1 << static_cast<uint64_t>(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache))));
                },
...
public:
    void SetUp() override
    {
        ...
        for (int i = 0; i <= static_cast<int>(WebSchedulingPriority::kLastPriority); i++) {
            ...
            std::unique_ptr<WebSchedulingTaskQueue> task_queue = worker_scheduler_->CreateWebSchedulingTaskQueue(priority);
...
protected:
    ...
    // - 'V': UserVisible
    // - 'B': Background
    void PostWebSchedulingTestTasks(Vector<String>* run_order, const String& task_descriptor)
    {
