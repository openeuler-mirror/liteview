...
public:
    ...
    virtual std::unique_ptr<TaskScope> CreateTaskScope(ScriptState*, absl::optional<TaskAttributionId> parent_task_id, TaskScopeType type) = 0;

    // Get the ID of the currently running task.
    virtual absl::optional<TaskAttributionId> RunningTaskAttributionId(ScriptState*) const = 0;

    // Check for ancestry of the currently running task against an input
    // |parentId|.
    virtual AncestorStatus IsAncestor(ScriptState*, TaskAttributionId parentId) = 0;
    virtual AncestorStatus HasAncestorInSet(ScriptState*, const WTF::HashSet<scheduler::TaskAttributionIdType>&) = 0;
