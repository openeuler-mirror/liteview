...
void BackForwardCacheDisablingFeatureTracker::Add(SchedulingPolicy::Feature feature)
{
...
void BackForwardCacheDisablingFeatureTracker::Remove(SchedulingPolicy::Feature feature)
{
    uint64_t old_mask = GetActiveFeaturesTrackedForBackForwardCacheMetricsMask();
...
uint64_t BackForwardCacheDisablingFeatureTracker::GetActiveFeaturesTrackedForBackForwardCacheMetricsMask() const
{
    auto result = back_forward_cache_disabling_features_.to_ullong();
    static_assert(static_cast<size_t>(SchedulingPolicy::Feature::kMaxValue) < sizeof(result) * 8,
