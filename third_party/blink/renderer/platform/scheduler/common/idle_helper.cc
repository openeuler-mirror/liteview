...
IdleHelper::IdleHelper(SchedulerHelper* helper, Delegate* delegate, const char* idle_period_tracing_name,
    base::TimeDelta required_quiescence_duration_before_long_idle_period, scoped_refptr<TaskQueue> idle_queue)
    : helper_(helper)
    , delegate_(delegate)
    ...
    , is_shutdown_(false)
{
    ...
    idle_task_runner_ = base::MakeRefCounted<SingleThreadIdleTaskRunner>(
        idle_queue_->CreateTaskRunner(static_cast<int>(TaskType::kMainThreadTaskQueueIdle)), helper_->ControlTaskRunner(), this);
