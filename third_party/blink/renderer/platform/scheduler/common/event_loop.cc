...
EventLoop::EventLoop(v8::Isolate* isolate, std::unique_ptr<v8::MicrotaskQueue> microtask_queue)
    : isolate_(isolate)
    ,
...
void EventLoop::EnqueueEndOfMicrotaskCheckpointTask(base::OnceClosure task)
{
    ...
    if (end_of_checkpoint_tasks_.size() == 1) {
        if (microtask_queue_) {
            ...
            AddRef();
            isolate_->AddMicrotasksCompletedCallback(&EventLoop::RunEndOfCheckpointTasks, this);
        }
    }
}
...
void EventLoop::RunEndOfMicrotaskCheckpointTasks()
{
    ...
    if (microtask_queue_) {
        ...
        isolate_->RemoveMicrotasksCompletedCallback(&EventLoop::RunEndOfCheckpointTasks, this);
    }
