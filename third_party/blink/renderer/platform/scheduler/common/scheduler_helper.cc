...
{
    CheckOnValidThread();
    DCHECK(simple_task_executor_.has_value()) << "AttachToCurrentThread() was not invoked.";
...
absl::optional<base::sequence_manager::WakeUp> SchedulerHelper::GetNextWakeUp() const
{
    CheckOnValidThread();
    DCHECK(sequence_manager_);
...
{
    if (sequence_manager_) {
        static_cast<base::sequence_manager::internal::SequenceManagerImpl*>(sequence_manager_)->SetTimerSlack(timer_slack);
    }
}
