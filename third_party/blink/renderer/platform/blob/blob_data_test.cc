...
struct ExpectedElement {
    ...
    static ExpectedElement LargeBytes(Vector<uint8_t> data)
    {
        ...
        return ExpectedElement { DataElement::NewBytes(DataElementBytes::New(size, absl::nullopt, mojo::NullRemote())), String(), std::move(data) };
    }
...
TEST_F(BlobDataHandleTest, CreateFromUUID)
{
    ...
    scoped_refptr<BlobDataHandle> handle = BlobDataHandle::Create(kUuid, kType, kSize);
...
{
    Vector<ExpectedElement> expected_elements;
    expected_elements.push_back(ExpectedElement::File("path", 0, uint64_t(-1), base::Time()));

    TestCreateBlob(BlobData::CreateForFileWithUnknownSize("path"), std::move(expected_elements));
}
