...
TEST_F(BlobBytesProviderTest, Consolidation)
{
    ...
    auto large_data = std::make_unique<char[]>(BlobBytesProvider::kMaxConsolidatedItemSizeInBytes);
    data->AppendData(base::make_span(large_data.get(), BlobBytesProvider::kMaxConsolidatedItemSizeInBytes));

    EXPECT_EQ(2u, data->data_.size());
    EXPECT_EQ(12u, data->data_[0]->length());
    EXPECT_EQ(BlobBytesProvider::kMaxConsolidatedItemSizeInBytes, data->data_[1]->length());
}
...
public:
    void SetUp() override
    {
        ...
        sliced_data_.AppendRange(combined_bytes_.begin() + GetParam().offset, combined_bytes_.begin() + GetParam().offset + GetParam().size);
    }
    ...
    base::File DoRequestAsFile(uint64_t source_offset, uint64_t source_length, uint64_t file_offset)
    {
        ...
        test_provider_->RequestAsFile(source_offset, source_length, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_WRITE), file_offset,
            base::BindOnce(
                [](absl::optional<base::Time>* received_modified, absl::optional<base::Time> modified) { *received_modified = modified; }, &received_modified));
...
TEST_P(RequestAsFile, AtStartOfEmptyFile)
{
    ...
    EXPECT_EQ(static_cast<int>(test.size), file.Read(0, reinterpret_cast<char*>(read_data.data()), test.size));
...
TEST_P(RequestAsFile, OffsetInEmptyFile)
{
    ...
    if (test.size == 0) {
        ...
        EXPECT_EQ(static_cast<int>(sliced_data_.size()), file.Read(0, reinterpret_cast<char*>(read_data.data()), sliced_data_.size()));
        EXPECT_EQ(sliced_data_, read_data);
    }
}
...
TEST_P(RequestAsFile, OffsetInNonEmptyFile)
{
    ...
    base::CreateTemporaryFile(&path);
    {
        ...
        EXPECT_EQ(static_cast<int>(expected_data.size()), file.WriteAtCurrentPos(reinterpret_cast<const char*>(expected_data.data()), expected_data.size()));
    }

    std::copy(sliced_data_.begin(), sliced_data_.end(), expected_data.begin() + file_offset);

    test_provider_->RequestAsFile(test.offset, test.size, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_WRITE), file_offset,
        base::BindOnce([](absl::optional<base::Time> last_modified) { EXPECT_TRUE(last_modified); }));
    ...
    EXPECT_EQ(static_cast<int>(expected_data.size()), file.Read(0, reinterpret_cast<char*>(read_data.data()), expected_data.size()));
...
TEST_F(BlobBytesProviderTest, RequestAsFile_InvaldFile)
{
    ...
    provider->RequestAsFile(0, 16, base::File(), 0, base::BindOnce([](absl::optional<base::Time> last_modified) { EXPECT_FALSE(last_modified); }));
}
...
TEST_F(BlobBytesProviderTest, RequestAsFile_UnwritableFile)
{
    ...
    provider->RequestAsFile(0, 16, base::File(path, base::File::FLAG_OPEN | base::File::FLAG_READ), 0,
        base::BindOnce([](absl::optional<base::Time> last_modified) { EXPECT_FALSE(last_modified); }));
    ...
    watcher.Watch(consumer_handle.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED,
        base::BindRepeating(
            [](mojo::DataPipeConsumerHandle pipe, base::RepeatingClosure quit_closure, Vector<uint8_t>* bytes_out, MojoResult result,
                ...
                MojoResult query_result = pipe.ReadData(nullptr, &num_bytes, MOJO_READ_DATA_FLAG_QUERY);
                if (query_result == MOJO_RESULT_SHOULD_WAIT)
                    return;
                EXPECT_EQ(MOJO_RESULT_OK, query_result);

                Vector<uint8_t> bytes(num_bytes);
                EXPECT_EQ(MOJO_RESULT_OK, pipe.ReadData(bytes.data(), &num_bytes, MOJO_READ_DATA_FLAG_ALL_OR_NONE));
