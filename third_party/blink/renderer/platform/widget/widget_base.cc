    ...
    base::WeakPtr<mojom::blink::FrameWidgetInputHandler> frame_widget_input_handler)
{
    ...
    layer_tree_view_->Initialize(*settings, main_thread_compositor_task_runner_,
        compositing_thread_scheduler ? compositing_thread_scheduler->DefaultTaskRunner() : nullptr, CategorizedWorkerPool::GetOrCreate());
    ...
    widget_input_handler_manager_ = WidgetInputHandlerManager::Create(weak_ptr_factory_.GetWeakPtr(), std::move(frame_widget_input_handler), never_composited_,
        widget_compositing_thread_scheduler, widget_scheduler_, uses_input_handler, client_->AllowsScrollResampling());
...
void WidgetBase::Shutdown()
{
    ...
    if (layer_tree_view_) {
        ...
        scoped_refptr<base::SingleThreadTaskRunner> cleanup_runner = base::ThreadTaskRunnerHandle::Get();
        cleanup_runner->PostNonNestableTask(FROM_HERE,
            base::BindOnce(
                [](std::unique_ptr<LayerTreeView> view, scoped_refptr<WidgetInputHandlerManager> manager, scoped_refptr<scheduler::WidgetScheduler> scheduler) {
                    view.reset();
                    manager.reset();
                    scheduler->Shutdown();
                },
                std::move(layer_tree_view_), std::move(widget_input_handler_manager_), std::move(widget_scheduler_)));
    }
...
void WidgetBase::WasShown(bool was_evicted, mojom::blink::RecordContentToVisibleTimeRequestPtr record_tab_switch_time_request)
{
    ...
    if (record_tab_switch_time_request) {
        LayerTreeHost()->RequestPresentationTimeForNextFrame(tab_switch_time_recorder_.TabWasShown(false /* has_saved_frames */,
...
void WidgetBase::RequestPresentationTimeForNextFrame(mojom::blink::RecordContentToVisibleTimeRequestPtr visible_time_request)
{
...
void WidgetBase::OnDeferCommitsChanged(bool defer, cc::PaintHoldingReason reason, absl::optional<cc::PaintHoldingCommitTrigger> trigger)
{
...
void WidgetBase::RequestNewLayerTreeFrameSink(LayerTreeFrameSinkCallback callback)
{
    ...
    if (Platform::Current()->IsGpuCompositingDisabled()) {
        ...
        std::move(callback).Run(
            std::make_unique<cc::mojo_embedder::AsyncLayerTreeFrameSink>(nullptr, nullptr, params.get()), std::move(render_frame_metadata_observer));
        return;
    }
...
void WidgetBase::AddPresentationCallback(uint32_t frame_token, base::OnceCallback<void(base::TimeTicks)> callback)
{
...
void WidgetBase::BindWidgetCompositor(mojo::PendingReceiver<mojom::blink::WidgetCompositor> receiver)
{
    ...
    widget_compositor_
        = base::MakeRefCounted<WidgetCompositor>(weak_ptr_factory_.GetWeakPtr(), LayerTreeHost()->GetTaskRunnerProvider()->MainThreadTaskRunner(),
...
void WidgetBase::UpdateCompositionInfo(bool immediate_request)
{
    ...
    if (mojom::blink::WidgetInputHandlerHost* host = widget_input_handler_manager_->GetWidgetInputHandlerHost()) {
        host->ImeCompositionRangeChanged(composition_range_, composition_character_bounds_);
    }
}
...
void WidgetBase::QueueSyntheticEvent(std::unique_ptr<WebCoalescedInputEvent> event)
{
    ...
    widget_input_handler_manager_->input_event_queue()->HandleEvent(std::move(event), MainThreadEventQueue::DispatchType::kNonBlocking,
        mojom::blink::InputEventResultState::kNotConsumed, attribution, nullptr, HandledEventCallback());
}
    ...
    const viz::LocalSurfaceId& new_local_surface_id, const gfx::Rect& compositor_viewport_pixel_rect, const display::ScreenInfos& screen_infos)
{
    ...
    if (/*display::Display::HasForceRasterColorProfile()*/ false) {
