...
MomentumScrollJankTracker::~MomentumScrollJankTracker()
{
    ...
    uint32_t jank_percentage = (jank_count_ * 100 + rounding_factor) / total_event_count_;

    base::UmaHistogramPercentageObsoleteDoNotUse("Renderer4.MomentumScrollJankPercentage", jank_percentage);
}

void MomentumScrollJankTracker::OnDispatchedInputEvent(EventWithCallback* event_with_callback, const base::TimeTicks& now)
{
    DCHECK_EQ(event_with_callback->event().GetType(), WebGestureEvent::Type::kGestureScrollUpdate);
    ...
    jank_count_ += event_with_callback->coalesced_count() - kExpectedMomentumEventsPerFrame;
}
