...
public:
    ...
    void HandleResampleScrollEvents(std::unique_ptr<WebInputEvent>& event, double time_delta_in_milliseconds = 0, double display_refresh_rate = 30)
    {
        auto event_with_callback = std::make_unique<EventWithCallback>(
            std::make_unique<WebCoalescedInputEvent>(std::move(event), ui::LatencyInfo()), base::TimeTicks(), base::NullCallback(), nullptr);
    ...
    void ConfigureFilterFieldTrialAndInitialize(const base::Feature& feature, const std::string& filter_name)
    {
        ...
        scoped_feature_list_.Reset();
        scoped_feature_list_.InitAndEnableFeatureWithParameters(feature, params);
        EXPECT_EQ(params["filter"], GetFieldTrialParamValueByFeature(feature, "filter"));
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
    }
        ...
        const base::Feature& pred_feature, const std::string& predictor_type, const base::Feature& filter_feature, const std::string& filter_type)
    {
        ...
        base::test::ScopedFeatureList::FeatureAndParams prediction_params = { pred_feature, pred_field_params };

        base::FieldTrialParams filter_field_params;
        filter_field_params["filter"] = filter_type;
        base::test::ScopedFeatureList::FeatureAndParams filter_params = { filter_feature, filter_field_params };

        scoped_feature_list_.Reset();
        scoped_feature_list_.InitWithFeaturesAndParameters({ prediction_params, filter_params }, {});
    ...
    void InitLinearResamplingTest(bool use_frames_based_experimental_prediction)
    {
        ...
        base::test::ScopedFeatureList::FeatureAndParams prediction_params = { features::kResamplingScrollEvents, params };
        ...
        base::test::ScopedFeatureList::FeatureAndParams experimental_prediction_params
            = { ::features::kResamplingScrollEventsExperimentalPrediction, prediction_type_params };

        scoped_feature_list_.Reset();
        scoped_feature_list_.InitWithFeaturesAndParameters({ prediction_params, experimental_prediction_params }, {});
        scroll_predictor_ = std::make_unique<ScrollPredictor>();
