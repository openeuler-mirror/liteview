...
public:
    ...
    virtual void DispatchNonBlockingEventToMainThread(
        std::unique_ptr<WebCoalescedInputEvent> event, const blink::WebInputEventAttribution& attribution, std::unique_ptr<cc::EventMetrics> metrics)
        = 0;
    ...
    virtual void GenerateScrollBeginAndSendToMainThread(
        const blink::WebGestureEvent& update_event, const blink::WebInputEventAttribution& attribution, const cc::EventMetrics* update_metrics)
        = 0;

    virtual void SetAllowedTouchAction(cc::TouchAction touch_action, uint32_t unique_touch_event_id, InputHandlerProxy::EventDisposition event_disposition) = 0;
