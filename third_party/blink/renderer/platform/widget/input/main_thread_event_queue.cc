...
public:
    ...
    void Dispatch(MainThreadEventQueue* queue) override
    {
        ...
        if (!queue->HandleEventOnMainThread(*event_, attribution(), std::move(metrics_), std::move(callback))) {
            ...
            // indicating error.
            HandledEvent(queue, mojom::blink::InputEventResultState::kNotConsumed, event_->latency_info(), nullptr, absl::nullopt);
        }
    }
    ...
    void HandledEvent(MainThreadEventQueue* queue, mojom::blink::InputEventResultState ack_result, const ui::LatencyInfo& latency_info,
        mojom::blink::DidOverscrollParamsPtr overscroll, absl::optional<cc::TouchAction> touch_action)
    {
        ...
        if (!blocking_coalesced_callbacks_.empty()) {
            ...
            for (auto&& callback : blocking_coalesced_callbacks_) {
                ...
                std::move(callback.first).Run(ack_result, coalesced_latency_info, nullptr, absl::nullopt);
            }
        }
...
MainThreadEventQueue::MainThreadEventQueue(MainThreadEventQueueClient* client, const scoped_refptr<base::SingleThreadTaskRunner>& main_task_runner,
    scoped_refptr<scheduler::WidgetScheduler> widget_scheduler, bool allow_raf_aligned_input)
    : client_(client)
    ...
    HandledEventCallback callback)
{
    ...
    if (is_touch) {
        ...
        if (touch_event->touch_start_or_first_touch_move && touch_event->dispatch_type == WebInputEvent::DispatchType::kBlocking) {
            ...
            if (ack_result == mojom::blink::InputEventResultState::kSetNonBlockingDueToFling || last_touch_start_forced_nonblocking_due_to_fling_) {
    ...
    if (callback) {
        std::move(callback).Run(ack_result, cloned_latency_info, nullptr, absl::nullopt);
    }
}
...
void MainThreadEventQueue::QueueEvent(std::unique_ptr<MainThreadEventQueueTask> event)
{
    ...
    if (is_input_event) {
        auto* queued_input_event = static_cast<const QueuedWebInputEvent*>(event.get());
