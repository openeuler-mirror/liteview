...
int64_t GetEventLatencyMicros(base::TimeTicks event_timestamp, base::TimeTicks now)
{
...
void LogInputEventLatencyUma(const WebInputEvent& event, base::TimeTicks now)
{
    UMA_HISTOGRAM_CUSTOM_COUNTS("Event.AggregatedLatency.Renderer2",
        base::saturated_cast<base::HistogramBase::Sample>(GetEventLatencyMicros(event.TimeStamp(), now)), 1, 10000000, 100);
}
...
{
    return processed == WebInputEventResult::kNotHandled ? mojom::InputEventResultState::kNotConsumed : mojom::InputEventResultState::kConsumed;
}
...
public:
    ...
    absl::optional<WebTouchAction>& touch_action()
    {
        return touch_action_;
    }

    std::vector<WidgetBaseInputHandler::InjectScrollGestureParams>& injected_scroll_params()
    {
...
private:
    ...
    // being processed.
    std::vector<WidgetBaseInputHandler::InjectScrollGestureParams> injected_scroll_params_;
...
WebInputEventResult WidgetBaseInputHandler::HandleTouchEvent(const WebCoalescedInputEvent& coalesced_event)
{
    ...
    for (unsigned i = 0; i < touch_event.touches_length; ++i) {
        ...
        if (touch_point.state != WebTouchPoint::State::kStateStationary) {
            const WebPointerEvent& pointer_event = WebPointerEvent(touch_event, touch_point);
    ...
    const WebCoalescedInputEvent& coalesced_event, std::unique_ptr<cc::EventMetrics> metrics, HandledEventCallback callback)
{
    ...
    TRACE_EVENT1("renderer,benchmark,rail", "WidgetBaseInputHandler::OnHandleInputEvent", "event", WebInputEvent::GetName(input_event.GetType()));
...
bool WidgetBaseInputHandler::DidOverscrollFromBlink(const gfx::Vector2dF& overscroll_delta, const gfx::Vector2dF& accumulated_overscroll,
    const gfx::PointF& position, const gfx::Vector2dF& velocity, const cc::OverscrollBehavior& behavior)
{
    ...
    std::unique_ptr<InputHandlerProxy::DidOverscrollParams> params = std::make_unique<InputHandlerProxy::DidOverscrollParams>();
...
void WidgetBaseInputHandler::InjectGestureScrollEvent(WebGestureDevice device, const gfx::Vector2dF& delta, ui::ScrollGranularity granularity,
    cc::ElementId scrollable_area_element_id, WebInputEvent::Type injected_type)
{
    ...
    if (handling_input_state_) {
        InjectScrollGestureParams params { device, delta, granularity, scrollable_area_element_id, injected_type };
...
void WidgetBaseInputHandler::HandleInjectedScrollGestures(std::vector<InjectScrollGestureParams> injected_scroll_params, const WebInputEvent& input_event,
    const ui::LatencyInfo& original_latency_info, const cc::EventMetrics* original_metrics)
{
    ...
    for (const InjectScrollGestureParams& params : injected_scroll_params) {
        ...
        std::unique_ptr<WebGestureEvent> gesture_event = WebGestureEvent::GenerateInjectedScrollGesture(
            params.type, input_event.TimeStamp(), params.device, position, params.scroll_delta, params.granularity);
        ...
        if (params.type == WebInputEvent::Type::kGestureScrollBegin) {
            gesture_event->data.scroll_begin.scrollable_area_element_id = params.scrollable_area_element_id.GetStableId();
