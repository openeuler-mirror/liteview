...
public:
    ...
    {
        return state_ == other.state_ && coalesced_latency_ == other.coalesced_latency_;
    }
...
public:
    ...
    void DidHandleEvent(size_t index, blink::mojom::InputEventResultState ack_result, const ui::LatencyInfo& latency,
        mojom::blink::DidOverscrollParamsPtr params, absl::optional<cc::TouchAction> touch_action)
    {
        callbacks_received_[index] = ReceivedCallback(
            handling_event_ ? CallbackReceivedState::kCalledWhileHandlingEvent : CallbackReceivedState::kCalledAfterHandleEvent, latency.coalesced());
    }
...
public:
    ...
    MOCK_METHOD2(DidHandleInputEventOnMainThread, void(const WebInputEvent&, WebInputEventResult));
};
...
public:
    ...
    void SetUp() override
    {
        queue_ = new MainThreadEventQueue(this, main_task_runner_, widget_scheduler_, true);
        queue_->ClearRafFallbackTimerForTesting();
    }

    void HandleEvent(WebInputEvent& event, blink::mojom::InputEventResultState ack_result)
    {
    ...
    bool HandleInputEvent(const blink::WebCoalescedInputEvent& event, std::unique_ptr<cc::EventMetrics> metrics, HandledEventCallback callback) override
    {
        ...
        std::move(callback).Run(blink::mojom::InputEventResultState::kNotConsumed, event.latency_info(), nullptr, absl::nullopt);
        return true;
    }
    ...
    {
        std::unique_ptr<HandledEventCallbackTracker> callback = std::make_unique<HandledEventCallbackTracker>();
...
TEST_F(MainThreadEventQueueTest, ClientDoesntHandleInputEvent)
{
    ...
    EXPECT_CALL(*widget_scheduler_, DidHandleInputEventOnMainThread(testing::_, testing::_)).Times(1);
    ...
    EXPECT_THAT(received, testing::Each(ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, false)));
}
...
TEST_F(MainThreadEventQueueTest, NonBlockingWheel)
{
    ...
    EXPECT_CALL(*widget_scheduler_, DidHandleInputEventOnMainThread(testing::_, testing::_)).Times(0);
    ...
    EXPECT_FALSE(main_task_runner_->HasPendingTask());
    RunPendingTasksWithSimulatedRaf();
    EXPECT_THAT(GetAndResetCallbackResults(), testing::Each(ReceivedCallback(CallbackReceivedState::kCalledWhileHandlingEvent, false)));
    EXPECT_FALSE(main_task_runner_->HasPendingTask());
    EXPECT_EQ(0u, event_queue().size());
...
TEST_F(MainThreadEventQueueTest, BlockingTouch)
{
    ...
    EXPECT_CALL(*widget_scheduler_, DidHandleInputEventOnMainThread(testing::_, testing::_)).Times(3);
    {
        ...
        EXPECT_THAT(GetAndResetCallbackResults(),
            testing::ElementsAre(ReceivedCallback(CallbackReceivedState::kCalledWhileHandlingEvent, false),
                ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, false), ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, true),
...
TEST_F(MainThreadEventQueueTest, RafAlignedTouchInput)
{
    ...
    EXPECT_TRUE(needs_main_frame_);
    main_task_runner_->RunUntilIdle();
    EXPECT_THAT(GetAndResetCallbackResults(), testing::Each(ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, false)));
}
...
TEST_F(MainThreadEventQueueTest, RafAlignedTouchInputCoalescedMoves)
{
    ...
    EXPECT_CALL(*widget_scheduler_, DidHandleInputEventOnMainThread(testing::_, testing::_)).Times(4);
...
class MainThreadEventQueueInitializationTest : public testing::Test, public MainThreadEventQueueClient {
public:
...
TEST_F(MainThreadEventQueueTest, QueuingClosureWithRafEvent)
{
    ...
    EXPECT_CALL(*widget_scheduler_, DidHandleInputEventOnMainThread(testing::_, testing::_)).Times(2);
...
TEST_F(MainThreadEventQueueTest, BlockingTouchMoveBecomesNonBlocking)
{
    ...
    RunPendingTasksWithSimulatedRaf();
    EXPECT_THAT(GetAndResetCallbackResults(),
        testing::ElementsAre(ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, false),
            ReceivedCallback(CallbackReceivedState::kCalledWhileHandlingEvent, false),
            ReceivedCallback(CallbackReceivedState::kCalledAfterHandleEvent, false)));
    EXPECT_EQ(0u, event_queue().size());
    EXPECT_FALSE(main_task_runner_->HasPendingTask());
    EXPECT_FALSE(needs_main_frame_);

    EXPECT_EQ(WebInputEvent::DispatchType::kEventNonBlocking, static_cast<const WebTouchEvent&>(handled_tasks_.at(0)->taskAsEvent()->Event()).dispatch_type);
    EXPECT_EQ(WebInputEvent::DispatchType::kEventNonBlocking, static_cast<const WebTouchEvent&>(handled_tasks_.at(1)->taskAsEvent()->Event()).dispatch_type);
}
...
TEST_F(MainThreadEventQueueTest, BlockingTouchMoveWithTouchEnd)
{
    ...
    EXPECT_EQ(WebInputEvent::DispatchType::kBlocking, static_cast<const WebTouchEvent&>(handled_tasks_.at(0)->taskAsEvent()->Event()).dispatch_type);
    EXPECT_EQ(WebInputEvent::DispatchType::kBlocking, static_cast<const WebTouchEvent&>(handled_tasks_.at(1)->taskAsEvent()->Event()).dispatch_type);
}
