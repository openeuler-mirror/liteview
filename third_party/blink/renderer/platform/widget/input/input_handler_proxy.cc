...
cc::ScrollState CreateScrollStateForGesture(const WebGestureEvent& event)
{
    ...
    case WebInputEvent::Type::kGestureScrollBegin:
        ...
        if (cc::ElementId::IsValid(event.data.scroll_begin.scrollable_area_element_id)) {
            ...
            // scroll unification.
            scroll_state_data.is_main_thread_hit_tested = event.data.scroll_begin.main_thread_hit_tested;
            DCHECK(!event.data.scroll_begin.main_thread_hit_tested || base::FeatureList::IsEnabled(::features::kScrollUnification));
        } else {
    ...
    std::unique_ptr<blink::WebCoalescedInputEvent> event, std::unique_ptr<cc::EventMetrics> metrics, EventDispositionCallback callback)
{
    ...
    auto event_with_callback = std::make_unique<EventWithCallback>(std::move(event), tick_clock_->NowTicks(), std::move(callback), std::move(metrics));
    ...
    if (currently_active_gesture_device_.has_value()) {
        ...
        bool is_from_blocking_touch = gesture_event.SourceDevice() == WebGestureDevice::kTouchscreen && gesture_event.is_source_touch_event_set_blocking;
        ...
        if (is_from_blocking_touch || is_scroll_end_from_wheel || is_first_wheel_scroll_update || synchronous_input_handler_) {
            compositor_event_queue_->Queue(std::move(event_with_callback), tick_clock_->NowTicks());
...
void InputHandlerProxy::ContinueScrollBeginAfterMainThreadHitTest(std::unique_ptr<blink::WebCoalescedInputEvent> event,
    std::unique_ptr<cc::EventMetrics> metrics, EventDispositionCallback callback, cc::ElementIdType hit_test_result)
{
    ...
    if (cc::ElementId::IsValid(hit_test_result)) {
        gesture_event->data.scroll_begin.scrollable_area_element_id = hit_test_result;
        ...
        RecordMainThreadScrollingReasons(gesture_event->SourceDevice(), 0, false, 0);
        ...
        std::move(callback).Run(DROP_EVENT, std::move(event),
            /*overscroll_params=*/nullptr, attribution, std::move(metrics), /*scroll_result_data=*/nullptr);
    }
...
void InputHandlerProxy::DispatchSingleInputEvent(std::unique_ptr<EventWithCallback> event_with_callback, const base::TimeTicks now)
{
    ...
    switch (type) {
    case WebGestureEvent::Type::kGestureScrollBegin:
        momentum_scroll_jank_tracker_ = std::make_unique<MomentumScrollJankTracker>();
        break;
    case WebGestureEvent::Type::kGestureScrollUpdate:
        // It's possible to get a scroll update without a begin. Ignore these
        // cases.
        if (momentum_scroll_jank_tracker_) {
            momentum_scroll_jank_tracker_->OnDispatchedInputEvent(event_with_callback.get(), now);
        }
        break;
    case WebGestureEvent::Type::kGestureScrollEnd:
    ...
    event_with_callback->RunCallbacks(
        disposition, monitored_latency_info, std::move(current_overscroll_params_), attribution, std::move(current_scroll_result_data_));
}
    ...
    const cc::EventMetrics* original_metrics)
{
    ...
    std::unique_ptr<WebGestureEvent> synthetic_gesture_event = WebGestureEvent::GenerateInjectedScrollGesture(
        type, original_timestamp, WebGestureDevice::kScrollbar, position_in_widget, scroll_delta, pointer_result.scroll_units);
    ...
    if (type == WebInputEvent::Type::kGestureScrollBegin) {
        ...
        // provided element_id.
        synthetic_gesture_event->data.scroll_begin.scrollable_area_element_id = pointer_result.target_scroller.GetStableId();
    }
    ...
    auto gesture_event_with_callback_update
        = std::make_unique<EventWithCallback>(std::make_unique<WebCoalescedInputEvent>(std::move(synthetic_gesture_event), scrollbar_latency_info),
            original_timestamp, base::DoNothing(), std::move(metrics));

    bool needs_animate_input = compositor_event_queue_->empty();
    compositor_event_queue_->Queue(std::move(gesture_event_with_callback_update), original_timestamp);
    ...
    EventWithCallback* event_with_callback, const WebInputEventAttribution& original_attribution)
{
    ...
    if (absl::optional<InputHandlerProxy::EventDisposition> handled = cursor_control_handler_->ObserveInputEvent(event))
        return *handled;
    ...
    case WebInputEvent::Type::kGestureScrollUpdate:
        return HandleGestureScrollUpdate(static_cast<const WebGestureEvent&>(event), original_attribution, event_with_callback->metrics());
...
void InputHandlerProxy::RecordMainThreadScrollingReasons(
    WebGestureDevice device, uint32_t reasons_from_scroll_begin, bool was_main_thread_hit_tested, uint32_t main_thread_repaint_reasons)
{
    ...
    DCHECK(!cc::MainThreadScrollingReason::HasNonCompositedScrollReasons(reasons_from_scroll_begin));
    ...
    const bool is_compositor_scroll = reasons_from_scroll_begin == cc::MainThreadScrollingReason::kNotScrollingOnMain
        && main_thread_repaint_reasons == cc::MainThreadScrollingReason::kNotScrollingOnMain;

    absl::optional<EventDisposition> disposition = (device == WebGestureDevice::kTouchpad ? mouse_wheel_result_ : touch_result_);
    ...
    bool blocked_on_main_at_begin = blocked_on_main_thread_handler || was_main_thread_hit_tested;
...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleGestureScrollBegin(const WebGestureEvent& gesture_event)
{
    ...
    RecordMainThreadScrollingReasons(gesture_event.SourceDevice(), scroll_status.main_thread_scrolling_reasons, scroll_state.is_main_thread_hit_tested(),
...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleGestureScrollUpdate(
    const WebGestureEvent& gesture_event, const WebInputEventAttribution& original_attribution, cc::EventMetrics* metrics)
{
    TRACE_EVENT2("input", "InputHandlerProxy::HandleGestureScrollUpdate", "dx", -gesture_event.data.scroll_update.delta_x, "dy",
    ...
    if (!base::FeatureList::IsEnabled(::features::kScrollUnification) && input_handler_->ScrollingShouldSwitchtoMainThread()) {
        TRACE_EVENT_INSTANT0("input", "Move Scroll To Main Thread", TRACE_EVENT_SCOPE_THREAD);
        handling_gesture_on_impl_thread_ = false;
        currently_active_gesture_device_ = absl::nullopt;
        client_->GenerateScrollBeginAndSendToMainThread(gesture_event, original_attribution, metrics);
    ...
    cc::InputHandlerScrollResult scroll_result = input_handler_->ScrollUpdate(&scroll_state, delay);
    ...
    if (scroll_result.did_scroll) {
        ...
        if (input_handler_->IsCurrentlyScrollingViewport()) {
            current_scroll_result_data_->root_scroll_offset = scroll_result.current_visual_offset;
        }
    }
    ...
    const WebTouchEvent& touch_event, bool* is_touching_scrolling_layer, cc::TouchAction* allowed_touch_action)
{
    ...
    *is_touching_scrolling_layer = false;
    ...
    for (size_t i = 0; i < touch_event.touches_length; ++i) {
        ...
        if (event_listener_type != cc::InputHandler::TouchStartOrMoveEventListenerType::NO_HANDLER) {
            TRACE_EVENT_INSTANT1("input", "HaveHandler", TRACE_EVENT_SCOPE_THREAD, "Type", event_listener_type);

            *is_touching_scrolling_layer = event_listener_type == cc::InputHandler::TouchStartOrMoveEventListenerType::HANDLER_ON_SCROLLING_LAYER;
...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleTouchStart(EventWithCallback* event_with_callback)
{
    ...
    if (allowed_touch_action != cc::TouchAction::kNone && touch_event.touches_length == 1) {
        ...
        if (pointer_result.type == cc::PointerResultType::kScrollbarScroll) {
            client_->SetAllowedTouchAction(allowed_touch_action, touch_event.unique_touch_event_id, DID_HANDLE);
            return DID_HANDLE;
        }
    }
    ...
    client_->SetAllowedTouchAction(allowed_touch_action, touch_event.unique_touch_event_id, result);
...
bool InputHandlerProxy::GetSnapFlingInfoAndSetAnimatingSnapTarget(
    const gfx::Vector2dF& natural_displacement, gfx::PointF* initial_offset, gfx::PointF* target_offset) const
{
    return input_handler_->GetSnapFlingInfoAndSetAnimatingSnapTarget(natural_displacement, initial_offset, target_offset);
}
...
gfx::PointF InputHandlerProxy::ScrollByForSnapFling(const gfx::Vector2dF& delta)
{
    ...
    cc::InputHandlerScrollResult scroll_result = input_handler_->ScrollUpdate(&scroll_state, base::TimeDelta());
