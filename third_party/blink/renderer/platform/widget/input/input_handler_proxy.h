...
public:
    ...
    using EventDispositionCallback
        = base::OnceCallback<void(EventDisposition, std::unique_ptr<blink::WebCoalescedInputEvent> event, std::unique_ptr<DidOverscrollParams>,
            const blink::WebInputEventAttribution&, std::unique_ptr<cc::EventMetrics> metrics, mojom::blink::ScrollResultDataPtr)>;
    ...
    void ContinueScrollBeginAfterMainThreadHitTest(std::unique_ptr<blink::WebCoalescedInputEvent> event, std::unique_ptr<cc::EventMetrics> metrics,
        EventDispositionCallback callback, cc::ElementIdType hit_tests_result);
    ...
    bool GetSnapFlingInfoAndSetAnimatingSnapTarget(
        const gfx::Vector2dF& natural_displacement, gfx::PointF* initial_offset, gfx::PointF* target_offset) const override;
...
private:
    ...
    void DispatchSingleInputEvent(std::unique_ptr<EventWithCallback>, const base::TimeTicks);
    ...
    EventDisposition HandleGestureScrollUpdate(
        const blink::WebGestureEvent& event, const blink::WebInputEventAttribution& original_attribution, cc::EventMetrics* metrics);
    ...
    void RecordMainThreadScrollingReasons(
        blink::WebGestureDevice device, uint32_t reasons_from_scroll_begin, bool was_main_thread_hit_tested, uint32_t main_thread_repaint_reasons);
