...
SynchronousCompositorProxy::SynchronousCompositorProxy(InputHandlerProxy* input_handler_proxy)
    ...
    , animation_power_mode_voter_(power_scheduler::PowerModeArbiter::GetInstance()->NewVoter("PowerModeVoter.SynchronousCompositorProxy"))
...
void SynchronousCompositorProxy::DemandDrawHw(mojom::blink::SyncCompositorDemandDrawHwParamsPtr params, DemandDrawHwCallback callback)
{
    ...
    if (hardware_draw_reply_) {
        // Did not swap.
        std::move(hardware_draw_reply_).Run(PopulateNewCommonParams(), 0u, 0u, absl::nullopt, absl::nullopt, absl::nullopt);
    }
}
...
void SynchronousCompositorProxy::ZeroSharedMemory()
{
    ...
    memset(software_draw_shm_->shared_memory.memory(), 0, software_draw_shm_->buffer_size);
...
void SynchronousCompositorProxy::DemandDrawSw(mojom::blink::SyncCompositorDemandDrawSwParamsPtr params, DemandDrawSwCallback callback)
{
    ...
    if (software_draw_reply_) {
        // Did not swap.
        std::move(software_draw_reply_).Run(PopulateNewCommonParams(), 0u, absl::nullopt);
    }
}
...
void SynchronousCompositorProxy::DoDemandDrawSw(mojom::blink::SyncCompositorDemandDrawSwParamsPtr params)
{
    ...
    SkBitmap bitmap;
    if (!bitmap.installPixels(info, software_draw_shm_->shared_memory.memory(), stride)) {
        return;
    }
...
void SynchronousCompositorProxy::SubmitCompositorFrame(uint32_t layer_tree_frame_sink_id, const viz::LocalSurfaceId& local_surface_id,
    absl::optional<viz::CompositorFrame> frame, absl::optional<viz::HitTestRegionList> hit_test_region_list)
{
...
void SynchronousCompositorProxy::BeginFrame(const viz::BeginFrameArgs& args, const HashMap<uint32_t, viz::FrameTimingDetails>& timing_details)
{
    if (!layer_tree_frame_sink_ || !needs_begin_frames_) {
        ...
        animation_power_mode_voter_->ResetVoteAfterTimeout(power_scheduler::PowerModeVoter::kAnimationTimeout);
    }
...
void SynchronousCompositorProxy::SendDemandDrawHwAsyncReply(mojom::blink::SyncCompositorCommonRendererParamsPtr, uint32_t layer_tree_frame_sink_id,
    uint32_t metadata_version, const absl::optional<viz::LocalSurfaceId>& local_surface_id, absl::optional<viz::CompositorFrame> frame,
