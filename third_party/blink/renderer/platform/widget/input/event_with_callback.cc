...
EventWithCallback::EventWithCallback(std::unique_ptr<WebCoalescedInputEvent> event, base::TimeTicks timestamp_now,
    InputHandlerProxy::EventDispositionCallback callback, std::unique_ptr<cc::EventMetrics> metrics)
...
EventWithCallback::EventWithCallback(std::unique_ptr<WebCoalescedInputEvent> event, base::TimeTicks creation_timestamp,
    base::TimeTicks last_coalesced_timestamp, OriginalEventList original_events)
...
void EventWithCallback::CoalesceWith(EventWithCallback* other, base::TimeTicks timestamp_now)
{
    ...
    mojom::blink::ScrollResultDataPtr scroll_result_data)
{
    ...
    std::move(oldest_event.callback_)
        .Run(disposition, std::move(oldest_event.event_),
            ...
            std::move(oldest_event.metrics_), scroll_result_data ? scroll_result_data->Clone() : nullptr);
    ...
    for (auto& coalesced_event : original_events_) {
        ...
        std::move(coalesced_event.callback_)
            .Run(disposition, std::move(coalesced_event.event_),
                did_overscroll_params ? std::make_unique<InputHandlerProxy::DidOverscrollParams>(*did_overscroll_params) : nullptr, attribution,
                std::move(coalesced_event.metrics_), scroll_result_data ? scroll_result_data->Clone() : nullptr);
    }
}
