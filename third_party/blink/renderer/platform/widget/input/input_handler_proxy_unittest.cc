...
public:
    ...
    MOCK_METHOD3(
        GetSnapFlingInfoAndSetAnimatingSnapTarget, bool(const gfx::Vector2dF& natural_displacement, gfx::PointF* initial_offset, gfx::PointF* target_offset));
    ...
    MOCK_METHOD3(UpdateBrowserControlsState, void(cc::BrowserControlsState constraints, cc::BrowserControlsState current, bool animate));
...
public:
    ...
    void DispatchNonBlockingEventToMainThread(
        std::unique_ptr<WebCoalescedInputEvent> event, const WebInputEventAttribution&, std::unique_ptr<cc::EventMetrics> metrics) override
    {
    }
    ...
    MOCK_METHOD3(
        SetAllowedTouchAction, void(cc::TouchAction touch_action, uint32_t unique_touch_event_id, InputHandlerProxy::EventDisposition event_disposition));
...
class MockInputHandlerProxyClientWithDidAnimateForInput : public MockInputHandlerProxyClient {
public:
    MockInputHandlerProxyClientWithDidAnimateForInput()
    {
    }
    MockInputHandlerProxyClientWithDidAnimateForInput(const MockInputHandlerProxyClientWithDidAnimateForInput&) = delete;
    MockInputHandlerProxyClientWithDidAnimateForInput& operator=(const MockInputHandlerProxyClientWithDidAnimateForInput&) = delete;
...
const cc::InputHandler::ScrollStatus kImplThreadScrollState(
    cc::InputHandler::ScrollThread::SCROLL_ON_IMPL_THREAD, cc::MainThreadScrollingReason::kNotScrollingOnMain);

const cc::InputHandler::ScrollStatus kRequiresMainThreadHitTestState(cc::InputHandler::ScrollThread::SCROLL_ON_IMPL_THREAD,
...
const cc::InputHandler::ScrollStatus kMainThreadScrollState(cc::InputHandler::ScrollThread::SCROLL_ON_MAIN_THREAD, kSampleMainThreadScrollingReason);

const cc::InputHandler::ScrollStatus kScrollIgnoredScrollState(cc::InputHandler::ScrollThread::SCROLL_IGNORED, cc::MainThreadScrollingReason::kNotScrollable);
...
public:
    ...
    void RecordMainThreadScrollingReasonsForTest(WebGestureDevice device, uint32_t reasons)
    {
...
class InputHandlerProxyTest : public testing::Test, public testing::WithParamInterface<std::tuple<ScrollerType, HandlerType, ScrollUnification>> {
...
InputHandlerProxy::EventDisposition HandleInputEventWithLatencyInfo(TestInputHandlerProxy* input_handler, const WebInputEvent& event)
{
    ...
    input_handler->HandleInputEventWithLatencyInfo(std::move(scoped_input_event), nullptr,
        base::BindLambdaForTesting(
            [&event_disposition](InputHandlerProxy::EventDisposition disposition, std::unique_ptr<blink::WebCoalescedInputEvent> event,
                std::unique_ptr<InputHandlerProxy::DidOverscrollParams> callback, const WebInputEventAttribution& attribution,
                std::unique_ptr<cc::EventMetrics> metrics, mojom::blink::ScrollResultDataPtr scroll_result_data) { event_disposition = disposition; }));
...
public:
    ...
    void DidHandleInputEventAndOverscroll(InputHandlerProxy::EventDisposition event_disposition, std::unique_ptr<WebCoalescedInputEvent> input_event,
        ...
        std::unique_ptr<cc::EventMetrics> metrics, mojom::blink::ScrollResultDataPtr scroll_result_data)
    {
...
TEST_P(InputHandlerProxyTest, NestedGestureBasedScrollsDifferentSourceDevice)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kScrollUnification)) {
        EXPECT_CALL(mock_input_handler_, ScrollingShouldSwitchtoMainThread()).WillOnce(testing::Return(false));
    }
...
TEST_P(InputHandlerProxyTest, GestureScrollOnMainThread)
{
    ...
    EXPECT_CALL(mock_input_handler_, ScrollBegin(_, _)).WillOnce(testing::Return(kMainThreadScrollState));
...
void InputHandlerProxyTest::FlingAndSnap()
{
    ...
    EXPECT_CALL(mock_input_handler_, GetSnapFlingInfoAndSetAnimatingSnapTarget(_, _, _))
        .WillOnce(DoAll(testing::SetArgPointee<1>(gfx::PointF(0, 0)), testing::SetArgPointee<2>(gfx::PointF(0, 100)), testing::Return(true)));
    EXPECT_CALL(mock_input_handler_, ScrollUpdate(_, _)).Times(1);
...
TEST_P(InputHandlerProxyTest, SnapFlingIgnoresFollowingGSUAndGSE)
{
    ...
    EXPECT_CALL(mock_input_handler_, GetSnapFlingInfoAndSetAnimatingSnapTarget(_, _, _)).Times(0);
...
void InputHandlerProxyTest::ScrollHandlingSwitchedToMainThread()
{
    ...
    expected_disposition_ = InputHandlerProxy::DID_NOT_HANDLE;
    EXPECT_CALL(mock_input_handler_, ScrollUpdate(_, _)).Times(0);
    EXPECT_CALL(mock_input_handler_, ScrollingShouldSwitchtoMainThread()).WillOnce(testing::Return(true));
    ...
    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(testing::Property(&gfx::Point::x, testing::Gt(0)), _))
        .WillOnce(testing::Invoke([](const gfx::Point&, cc::TouchAction* touch_action) {
            *touch_action = cc::TouchAction::kPanUp;
            return cc::InputHandler::TouchStartOrMoveEventListenerType::HANDLER_ON_SCROLLING_LAYER;
        }));
...
TEST_F(InputHandlerProxyEventQueueTest, NestedGestureBasedScrollsSameSourceDevice)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kScrollUnification)) {
        EXPECT_CALL(mock_input_handler_, ScrollingShouldSwitchtoMainThread()).WillOnce(Return(false));
    }
    EXPECT_CALL(mock_input_handler_, ScrollUpdate(_, _)).Times(1);
...
TEST_F(InputHandlerProxyEventQueueTest, AckTouchActionNonBlockingForFling)
{
    ...
    // the real checks below.
    {
        ...
        // Start a fling - ScrollUpdate with momentum
        {
            ...
            EXPECT_CALL(mock_input_handler_, GetSnapFlingInfoAndSetAnimatingSnapTarget(_, _, _)).WillOnce(Return(false));
    ...
    // touch-action before dispatching more scrolls.
    {
        ...
        EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(_, _))
            .WillOnce(DoAll(SetArgPointee<1>(TouchAction::kNone), Return(InputHandler::TouchStartOrMoveEventListenerType::HANDLER_ON_SCROLLING_LAYER)));
        ...
        EXPECT_CALL(mock_client_,
            SetAllowedTouchAction(TouchAction::kAuto, touch_start->unique_touch_event_id, InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING_DUE_TO_FLING))
            .WillOnce(Return());
...
TEST_P(InputHandlerProxyTest, HitTestTouchEventNullTouchAction)
{
    ...
    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(testing::Property(&gfx::Point::x, testing::Eq(0)), _))
        .WillOnce(testing::Return(cc::InputHandler::TouchStartOrMoveEventListenerType::NO_HANDLER));

    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(testing::Property(&gfx::Point::x, testing::Gt(0)), _))
        .WillOnce(testing::Return(cc::InputHandler::TouchStartOrMoveEventListenerType::HANDLER_ON_SCROLLING_LAYER));
    // Since the second touch point hits a touch-region, there should be no
    // hit-testing for the third touch point.
...
TEST_P(InputHandlerProxyTest, MultiTouchPointHitTestNegative)
{
    ...
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(cc::TouchAction::kPanUp, 1, InputHandlerProxy::DROP_EVENT)).WillOnce(testing::Return());
...
TEST_P(InputHandlerProxyTest, MultiTouchPointHitTestPositive)
{
    ...
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(cc::TouchAction::kPanY, 1, expected_disposition_)).WillOnce(testing::Return());
    // Since the second touch point hits a touch-region, there should be no
    // hit-testing for the third touch point.
...
TEST_P(InputHandlerProxyTest, MultiTouchPointHitTestPassivePositive)
{
    ...
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(cc::TouchAction::kPanRight, 1, InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING)).WillOnce(testing::Return());
...
TEST_P(InputHandlerProxyTest, TouchStartPassiveAndTouchEndBlocking)
{
    ...
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(cc::TouchAction::kNone, 1, InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING)).WillOnce(testing::Return());
...
TEST_P(InputHandlerProxyTest, TouchMoveBlockingAddedAfterPassiveTouchStart)
{
    ...
    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(_, _))
        .WillOnce(testing::Return(cc::InputHandler::TouchStartOrMoveEventListenerType::NO_HANDLER));
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(_, _, _)).WillOnce(testing::Return());
    ...
    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(_, _))
        .WillOnce(testing::Return(cc::InputHandler::TouchStartOrMoveEventListenerType::HANDLER));
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(_, _, _)).WillOnce(testing::Return());
...
{
    VERIFY_AND_RESET_MOCKS();
    EXPECT_CALL(mock_input_handler_, UpdateBrowserControlsState(cc::BrowserControlsState::kShown, cc::BrowserControlsState::kBoth, true)).Times(1);

    input_handler_->UpdateBrowserControlsState(cc::BrowserControlsState::kShown, cc::BrowserControlsState::kBoth, true);
    VERIFY_AND_RESET_MOCKS();
}
...
public:
    ...
    void ContinueScrollBeginAfterMainThreadHitTest(
        std::unique_ptr<WebCoalescedInputEvent> event, cc::ElementIdType hit_test_result, ReturnedDisposition* out_disposition = nullptr)
    {
...
TEST_F(UnifiedScrollingInputHandlerProxyTest, MainThreadHitTestEvent)
{
    // Inject a GSB that returns RequiresMainThreadHitTest.
    {
        ...
        EXPECT_CALL(mock_input_handler_,
            ScrollBegin(AllOf(Property(&ScrollState::target_element_id, Eq(ElementId())), Property(&ScrollState::is_main_thread_hit_tested, Eq(false))), _))
    ...
    // from the hit test and the main_thread
    {
        ...
        EXPECT_CALL(mock_input_handler_,
            ScrollBegin(AllOf(Property(&ScrollState::target_element_id, Eq(kHitTestResult)), Property(&ScrollState::is_main_thread_hit_tested, Eq(true))), _))
            .Times(1);

        ContinueScrollBeginAfterMainThreadHitTest(ScrollBegin(), kHitTestResult.GetStableId());
        Mock::VerifyAndClearExpectations(&mock_input_handler_);
    }
}
...
TEST_F(InputHandlerProxyEventQueueTest, MAYBE_VSyncAlignedGestureScrollPinchScroll)
{
    ...
    EXPECT_CALL(mock_input_handler_, RecordScrollBegin(_, cc::ScrollBeginThreadState::kScrollingOnCompositor)).Times(1);
    if (!base::FeatureList::IsEnabled(features::kScrollUnification)) {
        EXPECT_CALL(mock_input_handler_, ScrollingShouldSwitchtoMainThread()).WillRepeatedly(testing::Return(false));
    }
    EXPECT_CALL(mock_input_handler_, ScrollUpdate(testing::Property(&cc::ScrollState::delta_y, testing::Gt(0)), _))
...
TEST_F(InputHandlerProxyEventQueueTest, OriginalEventsTracing)
{
    ...
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGestureScrollUpdate), end_events[0]->GetKnownArgAsInt("type"));
    EXPECT_EQ(3, end_events[0]->GetKnownArgAsInt("coalesced_count"));
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGestureScrollEnd), end_events[1]->GetKnownArgAsInt("type"));

    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGestureScrollBegin), end_events[2]->GetKnownArgAsInt("type"));
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGesturePinchBegin), end_events[3]->GetKnownArgAsInt("type"));
    ...
    // traced.
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGesturePinchUpdate), end_events[4]->GetKnownArgAsInt("type"));
    EXPECT_EQ(4, end_events[4]->GetKnownArgAsInt("coalesced_count"));
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGesturePinchEnd), end_events[5]->GetKnownArgAsInt("type"));
    EXPECT_EQ(static_cast<int>(WebInputEvent::Type::kGestureScrollEnd), end_events[6]->GetKnownArgAsInt("type"));
    testing::Mock::VerifyAndClearExpectations(&mock_input_handler_);
}
...
TEST_F(InputHandlerProxyEventQueueTest, CoalescedEventSwitchToMainThread)
{
    ...
    EXPECT_EQ(InputHandlerProxy::DID_NOT_HANDLE, event_disposition_recorder_.back());
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, GestureScrollTouchEventHandlerRegionAndHandlingScrollFromMainThread)
{
    ...
    expected_disposition_ = InputHandlerProxy::DID_NOT_HANDLE_NON_BLOCKING;
    EXPECT_EQ(expected_disposition_, HandleInputEventWithLatencyInfo(input_handler_.get(), touch_start_));
    ...
    EXPECT_EQ(expected_disposition_, HandleInputEventWithLatencyInfo(input_handler_.get(), touch_end_));
}

TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, GestureScrollHandlingScrollFromMainThread)
{
    ...
    EXPECT_CALL(mock_input_handler_, EventListenerTypeForTouchStartOrMoveAt(testing::Property(&gfx::Point::x, testing::Gt(0)), _))
        .WillOnce(testing::Return(cc::InputHandler::TouchStartOrMoveEventListenerType::NO_HANDLER));
    EXPECT_CALL(mock_client_, SetAllowedTouchAction(_, _, _)).WillOnce(testing::Return());
    EXPECT_CALL(mock_input_handler_, GetEventListenerProperties(_)).WillRepeatedly(testing::Return(cc::EventListenerProperties::kPassive));
    EXPECT_CALL(mock_input_handler_, HitTest(_)).WillOnce(testing::Return(cc::PointerResultType::kUnhandled));
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, ImplHandled_MainThreadHitTest)
{
    ...
    EXPECT_MAIN_THREAD_WHEEL_SCROLL_SAMPLE(cc::MainThreadScrollingReason::kFailedHitTest);
}
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, ImplHandled_MainThreadRepaint)
{
    ...
    scroll_status.main_thread_repaint_reasons = cc::MainThreadScrollingReason::kNoScrollingLayer;
    ...
    EXPECT_MAIN_THREAD_WHEEL_SCROLL_SAMPLE(cc::MainThreadScrollingReason::kNoScrollingLayer);
}
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, WheelScrollHistogram)
{
    ...
    EXPECT_MAIN_THREAD_WHEEL_SCROLL_SAMPLE_2(kSampleMainThreadScrollingReason, cc::MainThreadScrollingReason::kThreadedScrollingDisabled);
}
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, WheelScrollWheelEventHandlerRegionAndHandlingScrollFromMainThread)
{
    ...
    EXPECT_MAIN_THREAD_WHEEL_SCROLL_SAMPLE_2(kSampleMainThreadScrollingReason, cc::MainThreadScrollingReason::kWheelEventHandlerRegion);
...
TEST_P(InputHandlerProxyMainThreadScrollingReasonTest, WheelScrollHandlingScrollFromMainThread)
{
    // Gesture scrolling on main thread. We only record
    // HandlingScrollFromMainThread when it's the only available reason.
    SetupEvents(TestEventType::kMouseWheel);
...
public:
    ...
    void HandleScrollBegin()
    {
        auto gesture = std::make_unique<WebGestureEvent>(
            WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, tick_clock_.NowTicks(), WebGestureDevice::kTouchscreen);
    ...
    {
        auto gesture = std::make_unique<WebGestureEvent>(
            WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, tick_clock_.NowTicks(), WebGestureDevice::kTouchscreen);
        HandleGesture(std::move(gesture));
    }
    ...
    {
        auto gesture = std::make_unique<WebGestureEvent>(
            WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, tick_clock_.NowTicks(), WebGestureDevice::kTouchscreen);
        gesture->data.scroll_update.delta_y = -20;
        if (is_momentum) {
            gesture->data.scroll_update.inertial_phase = WebGestureEvent::InertialPhaseState::kMomentum;
        }
        HandleGesture(std::move(gesture));
    }
    ...
    {
        constexpr base::TimeDelta interval = base::Milliseconds(16);
        base::TimeTicks frame_time = base::TimeTicks() + (next_begin_frame_number_ - viz::BeginFrameArgs::kStartingFrameNumber) * interval;
        input_handler_proxy_.DeliverInputForBeginFrame(viz::BeginFrameArgs::Create(
            BEGINFRAME_FROM_HERE, 0, next_begin_frame_number_++, frame_time, frame_time + interval, interval, viz::BeginFrameArgs::NORMAL));
    }
...
protected:
    void HandleGesture(std::unique_ptr<WebInputEvent> event)
    {
        input_handler_proxy_.HandleInputEventWithLatencyInfo(
            std::make_unique<WebCoalescedInputEvent>(std::move(event), ui::LatencyInfo()), nullptr, base::DoNothing());
    }
...
TEST_F(InputHandlerProxyMomentumScrollJankTest, TestJank)
{
    ...
    histogram_tester.ExpectUniqueSample("Renderer4.MomentumScrollJankPercentage", 4, 1);
}
...
TEST_F(InputHandlerProxyMomentumScrollJankTest, TestJankMultipleGestures)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        histogram_tester.ExpectUniqueSample("Renderer4.MomentumScrollJankPercentage", 1, i + 1);
    }
}
...
TEST_F(InputHandlerProxyMomentumScrollJankTest, TestJankRounding)
{
    ...
    histogram_tester.ExpectUniqueSample("Renderer4.MomentumScrollJankPercentage", 1, 1);
}
...
TEST_F(InputHandlerProxyMomentumScrollJankTest, TestSimpleNoJank)
{
    ...
    histogram_tester.ExpectUniqueSample("Renderer4.MomentumScrollJankPercentage", 0, 1);
}
...
TEST_F(InputHandlerProxyMomentumScrollJankTest, TestFirstGestureNoJank)
{
    ...
    histogram_tester.ExpectTotalCount("Renderer4.MomentumScrollJankPercentage", 0);
}
...
const auto kTestCombinations = testing::Combine(testing::Values(ScrollerType::kRoot, ScrollerType::kChild),
    testing::Values(HandlerType::kNormal, HandlerType::kSynchronous), testing::Values(ScrollUnification::kEnabled, ScrollUnification::kDisabled));

const auto kSuffixGenerator = [](const testing::TestParamInfo<std::tuple<ScrollerType, HandlerType, ScrollUnification>>& info) {
    ...
    name += std::get<2>(info.param) == ScrollUnification::kEnabled ? "UnifiedScroll" : "LegacyScroll";
    return name;
};
