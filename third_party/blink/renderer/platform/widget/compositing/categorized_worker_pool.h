...
class PLATFORM_EXPORT CategorizedWorkerPool : public base::TaskRunner, public cc::TaskGraphRunner {
public:
    ...
    void CollectCompletedTasks(cc::NamespaceToken token, cc::Task::Vector* completed_tasks) override;
...
protected:
    ...
    void CollectCompletedTasksWithLockAcquired(cc::NamespaceToken token, cc::Task::Vector* completed_tasks) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    bool ShouldRunTaskForCategoryWithLockAcquired(cc::TaskCategory category) EXCLUSIVE_LOCKS_REQUIRED(lock_);
...
public:
    ...
    void Run(const std::vector<cc::TaskCategory>& categories, base::ConditionVariable* has_ready_to_run_tasks_cv);
...
private:
    void ScheduleTasksWithLockAcquired(cc::NamespaceToken token, cc::TaskGraph* graph) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    // Runs a task from one of the provided categories. Categories listed first
    // have higher priority. Returns false if there were no tasks to run.
    bool RunTaskWithLockAcquired(const std::vector<cc::TaskCategory>& categories) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    void RunTaskInCategoryWithLockAcquired(cc::TaskCategory category) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Helper function which signals worker threads if tasks are ready to run.
    void SignalHasReadyToRunTasksWithLockAcquired() EXCLUSIVE_LOCKS_REQUIRED(lock_);
...
public:
    ...
    // Runs a task from one of the provided categories. Categories listed first
    // have higher priority.
    void Run(base::span<const cc::TaskCategory> categories, base::JobDelegate* job_delegate);
...
private:
    absl::optional<cc::TaskGraphWorkQueue::PrioritizedTask> GetNextTaskToRunWithLockAcquired(base::span<const cc::TaskCategory> categories);

    base::JobHandle* ScheduleTasksWithLockAcquired(cc::NamespaceToken token, cc::TaskGraph* graph) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Helper function which signals worker threads if tasks are ready to run.
    base::JobHandle* GetJobHandleToNotifyWithLockAcquired() EXCLUSIVE_LOCKS_REQUIRED(lock_);

    size_t GetMaxJobConcurrency(base::span<const cc::TaskCategory> categories) const;
