...
BASE_FEATURE(kUseCompositorJob, "UseCompositorJob", base::FEATURE_DISABLED_BY_DEFAULT);

// Task categories running at normal thread priority.
constexpr cc::TaskCategory kNormalThreadPriorityCategories[]
    = { cc::TASK_CATEGORY_NONCONCURRENT_FOREGROUND, cc::TASK_CATEGORY_FOREGROUND, cc::TASK_CATEGORY_BACKGROUND_WITH_NORMAL_THREAD_PRIORITY };

// Task categories running at background thread priority.
constexpr cc::TaskCategory kBackgroundThreadPriorityCategories[] = { cc::TASK_CATEGORY_BACKGROUND };

// Foreground task categories.
constexpr cc::TaskCategory kForegroundCategories[] = { cc::TASK_CATEGORY_NONCONCURRENT_FOREGROUND, cc::TASK_CATEGORY_FOREGROUND };
...
// run.
constexpr cc::TaskCategory kBackgroundCategories[] = { cc::TASK_CATEGORY_BACKGROUND, cc::TASK_CATEGORY_BACKGROUND_WITH_NORMAL_THREAD_PRIORITY };
...
public:
    CategorizedWorkerPoolThread(const std::string& name_prefix, const Options& options, CategorizedWorkerPoolImpl* pool,
        std::vector<cc::TaskCategory> categories, base::ConditionVariable* has_ready_to_run_tasks_cv)
    ...
    void SetBackgroundingCallback(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceCallback<void(base::PlatformThreadId)> callback)
    {
    ...
    void BeforeRun() override
    {
        if (backgrounding_callback_) {
            ...
            background_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(backgrounding_callback_), tid()));
    ...
    void Run() override
    {
        pool_->Run(categories_, has_ready_to_run_tasks_cv_);
    }
...
public:
    explicit CategorizedWorkerPoolSequencedTaskRunner(cc::TaskGraphRunner* task_graph_runner)
    ...
    bool PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) override
    {
        ...
        for (const auto& graph_task : tasks_) {
            ...
            cc::TaskGraph::Node node(graph_task, cc::TASK_CATEGORY_FOREGROUND, 0u /* priority */, dependencies);
            if (dependencies) {
                graph_.edges.push_back(cc::TaskGraph::Edge(graph_.nodes.back().task.get(), node.task.get()));
            }
...
void CategorizedWorkerPoolImpl::Start(int max_concurrency_foreground)
{
    ...
    // priority.
    std::vector<cc::TaskCategory> normal_thread_prio_categories(std::begin(kNormalThreadPriorityCategories), std::end(kNormalThreadPriorityCategories));

    for (int i = 0; i < max_concurrency_foreground; i++) {
        auto thread = std::make_unique<CategorizedWorkerPoolThread>(base::StringPrintf("CompositorTileWorker%d", i + 1), base::SimpleThread::Options(), this,
            normal_thread_prio_categories, &has_task_for_normal_priority_thread_cv_);
        thread->StartAsync();
        threads_.push_back(std::move(thread));
    }

    // Start a single thread running at background thread priority.
    std::vector<cc::TaskCategory> background_thread_prio_categories { std::begin(kBackgroundThreadPriorityCategories),
...
// TODO(1326996): Figure out whether !IS_MAC can be lifted here.
    ...
    auto thread = std::make_unique<CategorizedWorkerPoolThread>(
        "CompositorTileWorkerBackground", thread_options, this, background_thread_prio_categories, &has_task_for_background_priority_thread_cv_);
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    thread->SetBackgroundingCallback(Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted()),
        base::BindOnce([](base::PlatformThreadId thread_id) { Platform::Current()->SetThreadType(thread_id, base::ThreadType::kBackground); }));
#endif
...
// Overridden from base::TaskRunner:
bool CategorizedWorkerPoolImpl::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    base::AutoLock lock(lock_);
    ...
    base::EraseIf(tasks_, [this](const scoped_refptr<cc::Task>& e) EXCLUSIVE_LOCKS_REQUIRED(lock_) { return base::Contains(this->completed_tasks_, e); });
    ...
    for (const auto& graph_task : tasks_) {
        ...
        graph_.nodes.push_back(cc::TaskGraph::Node(graph_task.get(), cc::TASK_CATEGORY_FOREGROUND, 0u /* priority */, 0u /* dependencies */));
    }
...
void CategorizedWorkerPoolImpl::Run(const std::vector<cc::TaskCategory>& categories, base::ConditionVariable* has_ready_to_run_tasks_cv)
{
    base::AutoLock lock(lock_);
...
void CategorizedWorkerPoolImpl::ScheduleTasks(cc::NamespaceToken token, cc::TaskGraph* graph)
{
...
void CategorizedWorkerPoolImpl::ScheduleTasksWithLockAcquired(cc::NamespaceToken token, cc::TaskGraph* graph)
{
...
bool CategorizedWorkerPoolImpl::RunTaskWithLockAcquired(const std::vector<cc::TaskCategory>& categories)
{
...
void CategorizedWorkerPoolImpl::RunTaskInCategoryWithLockAcquired(cc::TaskCategory category)
{
...
void CategorizedWorkerPoolJob::Start(int max_concurrency_foreground)
{
    ...
    background_job_handle_ = base::CreateJob(FROM_HERE, { base::TaskPriority::BEST_EFFORT, base::ThreadPolicy::PREFER_BACKGROUND, base::MayBlock() },
        base::BindRepeating(&CategorizedWorkerPoolJob::Run, base::Unretained(this), base::span<const cc::TaskCategory>(kBackgroundThreadPriorityCategories)),
        base::BindRepeating(
    ...
    foreground_job_handle_ = base::CreateJob(FROM_HERE, { base::TaskPriority::USER_BLOCKING, base::MayBlock() },
        base::BindRepeating(&CategorizedWorkerPoolJob::Run, base::Unretained(this), base::span<const cc::TaskCategory>(kNormalThreadPriorityCategories)),
...
void CategorizedWorkerPoolJob::Run(base::span<const cc::TaskCategory> categories, base::JobDelegate* job_delegate)
{
...
absl::optional<cc::TaskGraphWorkQueue::PrioritizedTask> CategorizedWorkerPoolJob::GetNextTaskToRunWithLockAcquired(
...
void CategorizedWorkerPoolJob::ScheduleTasks(cc::NamespaceToken token, cc::TaskGraph* graph)
{
    TRACE_EVENT2("disabled-by-default-cc.debug", "CategorizedWorkerPool::ScheduleTasks", "num_nodes", graph->nodes.size(), "num_edges", graph->edges.size());
    base::JobHandle* job_handle_to_notify = nullptr;
    {
...
base::JobHandle* CategorizedWorkerPoolJob::ScheduleTasksWithLockAcquired(cc::NamespaceToken token, cc::TaskGraph* graph)
{
    DCHECK(token.IsValid());
    DCHECK(!cc::TaskGraphWorkQueue::DependencyMismatch(graph));
...
size_t CategorizedWorkerPoolJob::GetMaxJobConcurrency(base::span<const cc::TaskCategory> categories) const
{
    base::AutoLock lock(lock_);
...
CategorizedWorkerPool* CategorizedWorkerPool::GetOrCreate()
{
    ...
    scoped_refptr<CategorizedWorkerPool> categorized_worker_pool = base::FeatureList::IsEnabled(kUseCompositorJob)
        ? scoped_refptr<CategorizedWorkerPool>(new CategorizedWorkerPoolJob())
        : scoped_refptr<CategorizedWorkerPool>(new CategorizedWorkerPoolImpl());
...
void CategorizedWorkerPool::WaitForTasksToFinishRunning(cc::NamespaceToken token)
{
...
void CategorizedWorkerPool::CollectCompletedTasks(cc::NamespaceToken token, cc::Task::Vector* completed_tasks)
{
...
void CategorizedWorkerPool::CollectCompletedTasksWithLockAcquired(cc::NamespaceToken token, cc::Task::Vector* completed_tasks)
{
    DCHECK(token.IsValid());
    work_queue_.CollectCompletedTasks(token, completed_tasks);
}

bool CategorizedWorkerPool::ShouldRunTaskForCategoryWithLockAcquired(cc::TaskCategory category)
{
    lock_.AssertAcquired();
