    ...
    const cc::ManagedMemoryPolicy& default_policy, const gfx::Size& initial_screen_size, float initial_device_scale_factor)
{
    ...
    if (actual.bytes_limit_when_visible == 0) {
        ...
        actual.bytes_limit_when_visible = std::max(actual.bytes_limit_when_visible, static_cast<size_t>(8 * 1024 * 1024));
        actual.bytes_limit_when_visible = std::min(actual.bytes_limit_when_visible, static_cast<size_t>(256 * 1024 * 1024));
    }
    ...
    int display_width = std::round(initial_screen_size.width() * initial_device_scale_factor);
    ...
    bool is_threaded, bool is_for_embedded_frame, bool is_for_scalable_page, const gfx::Size& initial_screen_size, float initial_device_scale_factor)
{
    ...
    settings.skip_commits_if_not_synchronizing_compositor_state
        = base::FeatureList::IsEnabled(::features::kSkipCommitsIfNotSynchronizingCompositorState) && !RuntimeEnabledFeatures::DocumentTransitionEnabled();
    ...
    settings.compositor_threaded_scrollbar_scrolling = base::FeatureList::IsEnabled(::features::kCompositorThreadedScrollbarScrolling);

    settings.resource_settings.use_r16_texture = base::FeatureList::IsEnabled(media::kUseR16Texture);
    ...
    settings.enable_checker_imaging = !cmd.HasSwitch(cc::switches::kDisableCheckerImaging) && is_threaded;
    ...
    settings.resource_settings.use_gpu_memory_buffer_resources = cmd.HasSwitch(switches::kEnableGpuMemoryBufferCompositorResources);
    ...
    settings.initial_debug_state.show_fps_counter = cmd.HasSwitch(cc::switches::kShowFPSCounter);
    ...
    settings.initial_debug_state.highlight_non_lcd_text_layers = cmd.HasSwitch(cc::switches::kHighlightNonLCDTextLayers);
    settings.initial_debug_state.show_web_vital_metrics = base::FeatureList::IsEnabled(::features::kHudDisplayForPerformanceMetrics) && !is_for_embedded_frame;
    settings.initial_debug_state.show_smoothness_metrics = base::FeatureList::IsEnabled(::features::kHudDisplayForPerformanceMetrics) && !is_for_embedded_frame;

    settings.initial_debug_state.SetRecordRenderingStats(cmd.HasSwitch(cc::switches::kEnableGpuBenchmarking));

    if (cmd.HasSwitch(cc::switches::kSlowDownRasterScaleFactor)) {
        const int kMinSlowDownScaleFactor = 0;
        const int kMaxSlowDownScaleFactor = INT_MAX;
        switch_value_as_int(cmd, cc::switches::kSlowDownRasterScaleFactor, kMinSlowDownScaleFactor, kMaxSlowDownScaleFactor,
    ...
    if (cmd.HasSwitch(cc::switches::kCCScrollAnimationDurationForTesting)) {
        ...
        int duration;
        if (switch_value_as_int(cmd, cc::switches::kCCScrollAnimationDurationForTesting, kMinScrollAnimationDuration, kMaxScrollAnimationDuration, &duration)) {
    ...
    if (use_synchronous_compositor) {
        ...
        settings.enable_early_damage_check = cmd.HasSwitch(cc::switches::kCheckDamageEarly);
    }
    ...
    settings.enable_scroll_update_optimizations = RuntimeEnabledFeatures::ScrollUpdateOptimizationsEnabled();
