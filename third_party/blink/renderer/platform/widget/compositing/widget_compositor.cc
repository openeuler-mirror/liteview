...
WidgetCompositor::WidgetCompositor(base::WeakPtr<WidgetBase> widget_base, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner,
    ...
    , swap_queue_(std::make_unique<WidgetSwapQueue>())
{
    if (!compositor_task_runner_) {
        ...
        compositor_task_runner_->PostTask(FROM_HERE, base::BindOnce(&WidgetCompositor::BindOnThread, base::Unretained(this), std::move(receiver)));
    }
}
    ...
    base::OnceCallback<void(int)> drain_callback, base::OnceClosure swap_callback, VisualStateRequestCallback callback)
{
    ...
    if (first_message_for_frame) {
        ...
        compositor_task_runner_->PostTask(
            FROM_HERE, base::BindOnce([](base::OnceCallback<void(int)>, base::OnceClosure) {}, std::move(drain_callback), std::move(swap_callback)));
    }
}
