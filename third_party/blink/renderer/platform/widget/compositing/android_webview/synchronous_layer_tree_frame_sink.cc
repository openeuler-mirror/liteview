...
public:
    ...
    void OnBeginFrame(const viz::BeginFrameArgs& args, const viz::FrameTimingDetailsMap& timing_details) override
    {
    }
...
SynchronousLayerTreeFrameSink::SynchronousLayerTreeFrameSink(scoped_refptr<viz::ContextProvider> context_provider,
    ...
    : cc::LayerTreeFrameSink(
        std::move(context_provider), std::move(worker_context_provider_wrapper), std::move(compositor_task_runner), gpu_memory_buffer_manager)
...
bool SynchronousLayerTreeFrameSink::BindToClient(cc::LayerTreeFrameSinkClient* sink_client)
{
    ...
    display_ = std::make_unique<viz::Display>(&shared_bitmap_manager_, software_renderer_settings, &debug_settings_, kRootFrameSinkId,
...
void SynchronousLayerTreeFrameSink::SubmitCompositorFrame(viz::CompositorFrame frame, bool hit_test_data_changed)
{
    ...
    if (in_software_draw_) {
        ...
        if (!root_local_surface_id_.is_valid() || display_size_ != display_size || device_scale_factor_ != frame.metadata.device_scale_factor) {
        ...
        shared_quad_state->SetAll(child_transform, gfx::Rect(child_size), gfx::Rect(child_size), gfx::MaskFilterInfo(), absl::nullopt /* clip_rect */,
            are_contents_opaque /* are_contents_opaque */, 1.f /* opacity */, SkBlendMode::kSrcOver, 0 /* sorting_context_id */);
        surface_quad->SetNew(shared_quad_state, gfx::Rect(child_size), gfx::Rect(child_size),
            viz::SurfaceRange(absl::nullopt, viz::SurfaceId(kChildFrameSinkId, child_local_surface_id_)), SkColors::kWhite,
...
void SynchronousLayerTreeFrameSink::OnBeginFrame(const viz::BeginFrameArgs& args, const HashMap<uint32_t, viz::FrameTimingDetails>& timing_details)
{
    DCHECK(viz_frame_submission_enabled_);
...
void SynchronousLayerTreeFrameSink::BeginFrame(const viz::BeginFrameArgs& args)
{
    ...
    if (!nop_animation_timeout_start_.is_null()) {
        if ((base::TimeTicks::Now() - nop_animation_timeout_start_) > power_scheduler::PowerModeVoter::kAnimationTimeout) {
