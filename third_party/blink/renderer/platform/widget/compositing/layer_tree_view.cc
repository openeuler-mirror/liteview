...
//     Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(recorder.InitWithNewPipeAndPassReceiver());
...
void LayerTreeView::OnDeferCommitsChanged(bool status, cc::PaintHoldingReason reason, absl::optional<cc::PaintHoldingCommitTrigger> trigger)
{
    if (!delegate_)
        return;
...
void LayerTreeView::RequestNewLayerTreeFrameSink()
{
    ...
    delegate_->RequestNewLayerTreeFrameSink(base::BindOnce(&LayerTreeView::SetLayerTreeFrameSink, weak_factory_.GetWeakPtr()));
}
...
void LayerTreeView::DidCommit(base::TimeTicks commit_start_time, base::TimeTicks commit_finish_time)
{
    if (!delegate_)
        return;
...
void LayerTreeView::DidPresentCompositorFrame(uint32_t frame_token, const gfx::PresentationFeedback& feedback)
{
    if (!delegate_)
        return;
...
void LayerTreeView::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    if (!delegate_) {
        return;
    }
...
void LayerTreeView::ReportEventLatency(std::vector<cc::EventLatencyTracker::LatencyData> latencies)
{
...
void LayerTreeView::AddPresentationCallback(uint32_t frame_token, base::OnceCallback<void(base::TimeTicks)> callback)
{
