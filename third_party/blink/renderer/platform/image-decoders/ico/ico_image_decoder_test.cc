...
{
    return std::make_unique<ICOImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ColorBehavior::TransformToSRGB(), ImageDecoder::kNoDecodedImageByteLimit);
}
}

TEST(ICOImageDecoderTests, trunctedIco)
{
    const Vector<char> data = ReadFile("/images/resources/png-in-ico.ico")->CopyAs<Vector<char>>();
...
TEST(ICOImageDecoderTests, NullData)
{
    ...
    scoped_refptr<SharedBuffer> ico_file_data = ReadFile("/images/resources/png-in-ico.ico");
...
protected:
    ...
    {
        return std::make_unique<ICOImageDecoder>(ImageDecoder::kAlphaPremultiplied, ColorBehavior::TransformToSRGB(), ImageDecoder::kNoDecodedImageByteLimit);
    }
};
