...
{
    skcms_TransferFunction_makeScaledHLGish(tf, 1 / 12.0f, 2.0f, 2.0f, 1 / 0.17883277f, 0.28466892f, 0.55991073f);
}

// The input profile must outlive the output one as they will share their
// buffers.
skcms_ICCProfile ReplaceTransferFunction(skcms_ICCProfile profile, const skcms_TransferFunction& tf)
{
...
// transfer function.
bool ApproximatelyMatchesTF(const ColorProfile& profile, const skcms_TransferFunction& tf)
{
    skcms_ICCProfile parsed_copy = ReplaceTransferFunction(*profile.GetProfile(), tf);
    return skcms_ApproximatelyEqualProfiles(profile.GetProfile(), &parsed_copy);
}

std::unique_ptr<ColorProfile> NewColorProfileWithSameBuffer(const ColorProfile& buffer_donor, skcms_ICCProfile new_profile)
{
    ...
    // color profile.
    std::unique_ptr<uint8_t[]> owned_buffer(new uint8_t[buffer_donor.GetProfile()->size]);
    memcpy(owned_buffer.get(), buffer_donor.GetProfile()->buffer, buffer_donor.GetProfile()->size);
...
JXLImageDecoder::JXLImageDecoder(
    AlphaOption alpha_option, HighBitDepthDecodingOption high_bit_depth_decoding_option, const ColorBehavior& color_behavior, wtf_size_t max_decoded_bytes)
    : ImageDecoder(alpha_option, high_bit_depth_decoding_option, color_behavior, max_decoded_bytes)
{
    info_.have_animation = false;
}

bool JXLImageDecoder::ReadBytes(
    size_t remaining, wtf_size_t* offset, WTF::Vector<uint8_t>* segment, FastSharedBufferReader* reader, const uint8_t** jxl_data, size_t* jxl_size)
{
    *offset -= remaining;
...
void JXLImageDecoder::DecodeImpl(wtf_size_t index, bool only_size)
{
    ...
    if (num_decoded_frames_ > index && frame_buffer_cache_[index].GetStatus() == ImageFrame::kFrameComplete) {
    ...
    if ((index < num_decoded_frames_) && dec_) {
        ...
        if (JXL_DEC_SUCCESS != JxlDecoderSubscribeEvents(dec_.get(), JXL_DEC_FULL_IMAGE | JXL_DEC_FRAME_PROGRESSION)) {
            SetFailed();
            return;
        }
    ...
    if (!dec_) {
        ...
        const int events = JXL_DEC_BASIC_INFO | JXL_DEC_COLOR_ENCODING | JXL_DEC_FULL_IMAGE | JXL_DEC_FRAME_PROGRESSION;

        if (JXL_DEC_SUCCESS != JxlDecoderSubscribeEvents(dec_.get(), events)) {
            SetFailed();
            return;
        }
        if (JXL_DEC_SUCCESS != JxlDecoderSetProgressiveDetail(dec_.get(), JxlProgressiveDetail::kDC)) {
            SetFailed();
            return;
        }
    } else {
    ...
    for (;;) {
        ...
        case JXL_DEC_NEED_MORE_INPUT: {
            ...
            size_t jxl_size = 0;
            if (!ReadBytes(remaining, &offset_, &segment_, &reader, &jxl_data, &jxl_size)) {
                if (IsAllDataReceived()) {
            ...
            if (JXL_DEC_SUCCESS != JxlDecoderSetInput(dec_.get(), jxl_data, jxl_size)) {
        ...
        case JXL_DEC_COLOR_ENCODING: {
            ...
            if (JXL_DEC_SUCCESS == JxlDecoderGetColorAsEncodedProfile(dec_.get(), &format, JXL_COLOR_PROFILE_TARGET_ORIGINAL, &color_encoding)) {
                if (color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_PQ || color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_HLG) {
                    is_hdr_ = true;
                }
            }
            ...
            if (is_hdr_ && high_bit_depth_decoding_option_ == kHighBitDepthToHalfFloat) {
                decode_to_half_float_ = true;
            }

            bool have_data_profile = false;
            if (JXL_DEC_SUCCESS == JxlDecoderGetColorAsEncodedProfile(dec_.get(), &format, JXL_COLOR_PROFILE_TARGET_DATA, &color_encoding)) {
                ...
                } else if (color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_HLG) {
                    transfer = gfx::ColorSpace::TransferID::HLG;
                } else if (color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_LINEAR) {
                ...
                if (color_encoding.white_point == JXL_WHITE_POINT_D65 && color_encoding.primaries == JXL_PRIMARIES_2100) {
                    gamut = gfx::ColorSpace::PrimaryID::BT2020;
                } else if (color_encoding.white_point == JXL_WHITE_POINT_D65 && color_encoding.primaries == JXL_PRIMARIES_SRGB) {
                    gamut = gfx::ColorSpace::PrimaryID::BT709;
                } else if (color_encoding.white_point == JXL_WHITE_POINT_D65 && color_encoding.primaries == JXL_PRIMARIES_P3) {
                ...
                if (have_data_profile) {
                    ...
                    gfx::ColorSpace(gamut, transfer).ToSkColorSpace()->toProfile(&dataProfile);
            ...
            if (!have_data_profile) {
                size_t icc_size;
                bool got_size = JXL_DEC_SUCCESS == JxlDecoderGetICCProfileSize(dec_.get(), &format, JXL_COLOR_PROFILE_TARGET_DATA, &icc_size);
                std::vector<uint8_t> icc_profile(icc_size);
                if (got_size
                    && JXL_DEC_SUCCESS
                        == JxlDecoderGetColorAsICCProfile(dec_.get(), &format, JXL_COLOR_PROFILE_TARGET_DATA, icc_profile.data(), icc_profile.size())) {
                    profile = ColorProfile::Create(icc_profile.data(), icc_profile.size());
                    ...
                    if (ApproximatelyMatchesTF(*profile, tf_pq)) {
                        ...
                        hdr10->transferFn(&pq);
                        profile = NewColorProfileWithSameBuffer(*profile, ReplaceTransferFunction(*profile->GetProfile(), pq));
                    } else {
                        for (skcms_TransferFunction tf : { tf_hlg01, tf_hlg12 }) {
                            if (ApproximatelyMatchesTF(*profile, tf)) {
                                is_hdr_ = true;
                                auto hlg_colorspace = gfx::ColorSpace::CreateHLG().ToSkColorSpace();
                                skcms_TransferFunction hlg;
                                hlg_colorspace->transferFn(&hlg);
                                profile = NewColorProfileWithSameBuffer(*profile, ReplaceTransferFunction(*profile->GetProfile(), hlg));
                                break;
                            }
                        }
                    }
        ...
        case JXL_DEC_NEED_IMAGE_OUT_BUFFER: {
            ...
            size_t buffer_size;
            if (JXL_DEC_SUCCESS != JxlDecoderImageOutBufferSize(dec_.get(), &format, &buffer_size)) {
            ...
            // tests for JXL
            xform_ = ColorTransform();
            auto callback = [](void* opaque, size_t x, size_t y, size_t num_pixels, const void* pixels) {
                JXLImageDecoder* self = reinterpret_cast<JXLImageDecoder*>(opaque);
                ImageFrame& frame = self->frame_buffer_cache_[self->num_decoded_frames_ - 1];
                void* row_dst = self->decode_to_half_float_ ? reinterpret_cast<void*>(frame.GetAddrF16(static_cast<int>(x), static_cast<int>(y)))
                                                            : reinterpret_cast<void*>(frame.GetAddr(static_cast<int>(x), static_cast<int>(y)));

                bool dst_premultiply = frame.PremultiplyAlpha();

                const skcms_PixelFormat kSrcFormat = skcms_PixelFormat_RGBA_ffff;
                const skcms_PixelFormat kDstFormat = self->decode_to_half_float_ ? skcms_PixelFormat_RGBA_hhhh : XformColorFormat();

                if (self->xform_ || (kDstFormat != kSrcFormat) || (dst_premultiply && frame.HasAlpha())) {
                    skcms_AlphaFormat src_alpha = skcms_AlphaFormat_Unpremul;
                    skcms_AlphaFormat dst_alpha = (dst_premultiply && self->info_.alpha_bits) ? skcms_AlphaFormat_PremulAsEncoded : skcms_AlphaFormat_Unpremul;
                    const auto* src_profile = self->xform_ ? self->xform_->SrcProfile() : nullptr;
                    const auto* dst_profile = self->xform_ ? self->xform_->DstProfile() : nullptr;
                    bool color_conversion_successful
                        = skcms_Transform(pixels, kSrcFormat, src_alpha, src_profile, row_dst, kDstFormat, dst_alpha, dst_profile, num_pixels);
                    DCHECK(color_conversion_successful);
                }
            };
            if (JXL_DEC_SUCCESS != JxlDecoderSetImageOutCallback(dec_.get(), &format, callback, this)) {
                DVLOG(1) << "JxlDecoderSetImageOutCallback failed";
                SetFailed();
                return;
            }
...
bool JXLImageDecoder::MatchesJXLSignature(const FastSharedBufferReader& fast_reader)
{
    char buffer[12];
    if (fast_reader.size() < sizeof(buffer))
        return false;
    const char* contents = reinterpret_cast<const char*>(fast_reader.GetConsecutiveData(0, sizeof(buffer), buffer));
    // Direct codestream
...
{
    return IsAllDataReceived() || (index < num_decoded_frames_ && frame_buffer_cache_[index].GetStatus() == ImageFrame::kFrameComplete);
}
...
wtf_size_t JXLImageDecoder::DecodeFrameCount()
{
    ...
    if (frame_count_dec_ == nullptr) {
        ...
        if (JXL_DEC_SUCCESS != JxlDecoderSubscribeEvents(frame_count_dec_.get(), JXL_DEC_FRAME)) {
            SetFailed();
            return frame_buffer_cache_.size();
        }
    }
    ...
    for (;;) {
        ...
        case JXL_DEC_NEED_MORE_INPUT: {
            ...
            const uint8_t* jxl_data = nullptr;
            size_t jxl_size = 0;
            if (!ReadBytes(remaining, &frame_count_offset_, &frame_count_segment_, &reader, &jxl_data, &jxl_size)) {
                if (Failed()) {
                    return frame_buffer_cache_.size();
                }
            ...
            if (JXL_DEC_SUCCESS != JxlDecoderSetInput(frame_count_dec_.get(), jxl_data, jxl_size)) {
                DVLOG(1) << "JxlDecoderSetInput failed";
                SetFailed();
                return frame_buffer_cache_.size();
            }
        ...
        case JXL_DEC_FRAME: {
            ...
            if (JxlDecoderGetFrameHeader(frame_count_dec_.get(), &frame_header) != JXL_DEC_SUCCESS) {
            ...
            frame_durations_.push_back(1.0f * frame_header.duration * info_.animation.tps_denominator / info_.animation.tps_numerator);
            break;
        }
        case JXL_DEC_SUCCESS: {
