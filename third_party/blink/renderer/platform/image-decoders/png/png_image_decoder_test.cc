...
std::unique_ptr<ImageDecoder> CreatePNGDecoder(ImageDecoder::AlphaOption alpha_option)
{
    return std::make_unique<PNGImageDecoder>(
        alpha_option, ImageDecoder::kDefaultBitDepth, ColorBehavior::TransformToSRGB(), ImageDecoder::kNoDecodedImageByteLimit);
}
...
{
    return std::make_unique<PNGImageDecoder>(
        ImageDecoder::kAlphaNotPremultiplied, ImageDecoder::kHighBitDepthToHalfFloat, ColorBehavior::Tag(), ImageDecoder::kNoDecodedImageByteLimit);
}
...
TEST(AnimatedPNGTests, fdatBeforeIdat)
{
    ...
    scoped_refptr<SharedBuffer> modified_data = SharedBuffer::Create(data->Data(), kIdatOffset);
    // Copy fcTL and fdAT
...

    {
        ...
        const size_t kOffsetActl = 33u;
        const size_t kAcTLSize = 20u;
        scoped_refptr<SharedBuffer> modified_data2 = SharedBuffer::Create(modified_data->Data(), kOffsetActl);
        modified_data2->Append(modified_data->Data() + kOffsetActl + kAcTLSize, modified_data->size() - kOffsetActl - kAcTLSize);
        auto decoder = CreatePNGDecoder();
...
{
    scoped_refptr<SharedBuffer> full_data = ReadFile("/images/resources/"
                                                     "png-animated-idat-part-of-animation.png");
    ASSERT_FALSE(full_data->empty());
    auto decoder = CreatePNGDecoder();
...
TEST(AnimatedPNGTests, Offset)
{
    ...
    CreateDecodingBaseline(CreatePNGDecoder, original_data.get(), &baseline_hashes);
    ...
    // non-zero offset.
    auto decoder = std::make_unique<PNGImageDecoder>(ImageDecoder::kAlphaNotPremultiplied, ImageDecoder::kDefaultBitDepth, ColorBehavior::TransformToSRGB(),
...
static void TestHighBitDepthPNGDecoding(const PNGSample& png_sample, ImageDecoder* decoder)
{
    ...
    for (int i = 0; i < 16; i++) {
        if (fabs(decoded_pixels_float_32[i] - expected_pixels[i]) > decoding_tolerance) {
            DLOG(DCHECK) << "Pixel comparison failed. File: " << png_sample.filename << ", component index: " << i << ", actual: " << decoded_pixels_float_32[i]
...
{
    auto decoder = CreatePNGDecoder();
    scoped_refptr<SharedBuffer> data = ReadFile(kDecodersTestingDir, "private-critical-chunk-before-ihdr.png");
    EXPECT_FALSE(data->empty());
    decoder->SetData(data.get(), true);
    EXPECT_FALSE(decoder->IsSizeAvailable());
    EXPECT_TRUE(decoder->Failed());
