...
blink::BitmapImageMetrics::JpegColorSpace ExtractUMAJpegColorSpace(const jpeg_decompress_struct& info)
{
...
static bool ReadExifHeader(base::span<const uint8_t> data, bool& is_big_endian)
{
...
static gfx::Size ExtractDensityCorrectedSize(const DecodedImageMetaData& metadata, const gfx::Size& physical_size)
{
    ...
    gfx::SizeF size_from_resolution(
        physical_size.width() * kDefaultResolution / metadata.resolution.width(), physical_size.height() * kDefaultResolution / metadata.resolution.height());
...
static void ReadExifDirectory(
    const uint8_t* dir_start, const uint8_t* data_start, const uint8_t* data_end, bool is_big_endian, DecodedImageMetaData& metadata, bool is_root = true)
{
    ...
    const unsigned max_offset = base::checked_cast<unsigned>(data_end - data_start);
    unsigned tag_count = ReadUint16(dir_start, is_big_endian);
    const uint8_t* ifd = dir_start + 2; // Skip over the uint16 that was just read.
    ...
    for (unsigned i = 0; i < tag_count && data_end - ifd >= kIfdEntrySize; ++i, ifd += kIfdEntrySize) {
        ...
        case ExifTags::kResolutionXTag:
            if (type == kUnsignedRationalType && count == 1) {
                metadata.resolution.set_width(ReadUnsignedRational(value_ptr, is_big_endian));
            }
            break;

        case ExifTags::kResolutionYTag:
            if (type == kUnsignedRationalType && count == 1) {
                metadata.resolution.set_height(ReadUnsignedRational(value_ptr, is_big_endian));
            }
            break;
        ...
        case ExifTags::kExifOffsetTag:
            if (type == kUnsignedLongType && count == 1 && is_root) {
                ...
                ReadExifDirectory(subdir, data_start, data_end, is_big_endian, metadata, false);
            }
            break;
        }
    }
...
static bool ReadExif(base::span<const uint8_t> data, DecodedImageMetaData& metadata)
{
    bool is_big_endian;
...
static void ReadImageMetaData(jpeg_decompress_struct* info, DecodedImageMetaData& metadata)
{
    ...
    for (jpeg_saved_marker_ptr marker = info->marker_list; marker; marker = marker->next) {
        if (!IsExifData(marker))
            continue;
        base::span<const uint8_t> exif_data(marker->data + kExifAPP1SignatureSize, marker->data_length - kExifAPP1SignatureSize);
...
public:
    ...
    bool Decode(JPEGImageDecoder::DecodingMode decoding_mode)
    {
        ...
        case kJpegHeader: {
            ...
            decoder_->SetDensityCorrectedSize(ExtractDensityCorrectedSize(metadata, gfx::Size(info_.output_width, info_.output_height)));
            ...
            if (!decoder_->IgnoresColorSpace()) {
                ...
                if (read_icc_profile(Info(), &profile_buf, &profile_length)) {
                    std::unique_ptr<ColorProfile> profile = ColorProfile::Create(profile_buf, profile_length);
                    if (profile) {
        ...
        case kJpegDone:
            ...
            BitmapImageMetrics::CountJpegColorSpace(ExtractUMAJpegColorSpace(info_));
...
JPEGImageDecoder::JPEGImageDecoder(AlphaOption alpha_option, const ColorBehavior& color_behavior, wtf_size_t max_decoded_bytes, wtf_size_t offset)
    : ImageDecoder(alpha_option, ImageDecoder::kDefaultBitDepth, color_behavior, max_decoded_bytes)
    , offset_(offset)
{
}
...
void JPEGImageDecoder::OnSetData(SegmentReader* data)
{
    ...
    allow_decode_to_yuv_ =
        ...
        (GetYUVSubsampling() == cc::YUVSubsampling::k444 || GetYUVSubsampling() == cc::YUVSubsampling::k422 || GetYUVSubsampling() == cc::YUVSubsampling::k420);
}
