...
void SaveMD5Sum(const base::FilePath& path, blink::ImageFrame* frame_buffer)
{
    ...
    base::MD5Sum(bitmap.getPixels(), bitmap.width() * bitmap.height() * sizeof(uint32_t), &digest);

    // Write sum to disk.
    int bytes_written = base::WriteFile(path, reinterpret_cast<const char*>(&digest), sizeof digest);
...
void VerifyImage(blink::ImageDecoder& decoder, const base::FilePath& path, const base::FilePath& md5_sum_path, size_t frame_index)
{
    ...
    SkBitmap bitmap = frame_buffer->Bitmap();
    base::MD5Sum(bitmap.getPixels(), bitmap.width() * bitmap.height() * sizeof(uint32_t), &actual_digest);
...
void ImageDecoderBaseTest::SetUp()
{
    ...
    if (!base::PathExists(data_dir_)) {
        ...
        VLOG(0) << test_info->name() << " not running because test data wasn't found.";
        data_dir_.clear();
        return;
    }
}
...
void ImageDecoderBaseTest::TestDecoding(blink::ImageDecoderBaseTest::FileSelection file_selection, const int64_t threshold)
{
    ...
    for (Vector<base::FilePath>::const_iterator i = image_files.begin(); i != image_files.end(); ++i) {
