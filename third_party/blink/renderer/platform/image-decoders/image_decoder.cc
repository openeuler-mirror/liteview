...
#if 0 // BUILDFLAG(ENABLE_JXL_DECODER)
...
wtf_size_t CalculateMaxDecodedBytes(ImageDecoder::HighBitDepthDecodingOption high_bit_depth_decoding_option, const SkISize& desired_size)
{
    const wtf_size_t max_decoded_bytes
        = Platform::Current() ? static_cast<wtf_size_t>(Platform::Current()->MaxDecodedImageBytes()) : ImageDecoder::kNoDecodedImageByteLimit;
...
const wtf_size_t ImageDecoder::kNoDecodedImageByteLimit = static_cast<wtf_size_t>(-1);
...
std::unique_ptr<ImageDecoder> ImageDecoder::Create(scoped_refptr<SegmentReader> data, bool data_complete, AlphaOption alpha_option,
    HighBitDepthDecodingOption high_bit_depth_decoding_option, const ColorBehavior& color_behavior, const SkISize& desired_size,
    AnimationOption animation_option)
{
    ...
    return CreateByMimeType(type, std::move(data), data_complete, alpha_option, high_bit_depth_decoding_option, color_behavior, desired_size, animation_option);
}
...
std::unique_ptr<ImageDecoder> ImageDecoder::CreateByMimeType(String mime_type, scoped_refptr<SegmentReader> data, bool data_complete, AlphaOption alpha_option,
    ...
    AnimationOption animation_option)
{
    const wtf_size_t max_decoded_bytes = CalculateMaxDecodedBytes(high_bit_depth_decoding_option, desired_size);
    ...
    if (mime_type == "image/jpeg" || mime_type == "image/pjpeg" || mime_type == "image/jpg") {
        decoder = std::make_unique<JPEGImageDecoder>(alpha_option, color_behavior, max_decoded_bytes);
    } else if (mime_type == "image/png" || mime_type == "image/x-png" || mime_type == "image/apng") {
        decoder = std::make_unique<PNGImageDecoder>(alpha_option, high_bit_depth_decoding_option, color_behavior, max_decoded_bytes);
        ...
        decoder = std::make_unique<AVIFImageDecoder>(alpha_option, high_bit_depth_decoding_option, color_behavior, max_decoded_bytes, animation_option);
#endif
#if 0 // BUILDFLAG(ENABLE_JXL_DECODER)
    } else if (base::FeatureList::IsEnabled(features::kJXL) && mime_type == "image/jxl") {
        decoder = std::make_unique<JXLImageDecoder>(alpha_option, high_bit_depth_decoding_option, color_behavior, max_decoded_bytes);
#endif
    }
...
ImageDecoder::CompressionFormat ImageDecoder::GetCompressionFormat(scoped_refptr<SharedBuffer> image_data, String mime_type)
{
    ...
    if (EqualIgnoringASCIICase(mime_type, "image/webp") && available_data >= 16) {
        ...
        if (!memcmp(contents, "WEBPVP8X", 8)) {
            ...
            contents = reinterpret_cast<const unsigned char*>(fast_reader.GetConsecutiveData(0, available_data, long_buffer.get()));
    ...
    if (frame->GetStatus() != ImageFrame::kFrameComplete) {
        ...
        if (frame->GetStatus() == ImageFrame::kFrameComplete) {
            BitmapImageMetrics::CountDecodedImageFrameTime(
                FilenameExtension(), metrics_time_delta_, frame->OriginalFrameRect().size().Area64(), metrics_first_ && (index == 0));
...
ImagePlanes::ImagePlanes(void* planes[cc::kNumYUVPlanes], const wtf_size_t row_bytes[cc::kNumYUVPlanes], SkColorType color_type)
...
ColorProfile::ColorProfile(const skcms_ICCProfile& profile, std::unique_ptr<uint8_t[]> buffer)
    : profile_(profile)
    , buffer_(std::move(buffer))
{
}

std::unique_ptr<ColorProfile> ColorProfile::Create(const void* buffer, size_t size)
{
