...
// media::VideoColorSpace::VideoColorSpace(int, int, int, enum gfx::ColorSpace::RangeID)
// {
//     *(int*)1 = 1;
// }
// bool media::VideoColorSpace::IsSpecified(void) const
// {
//     *(int*)1 = 1;
//     return false;
// }
// gfx::ColorSpace media::VideoColorSpace::ToGfxColorSpace(void) const
// {
//     *(int*)1 = 1;
...
gfx::ColorSpace VideoColorSpace::ToGfxColorSpace(/*bool allow_guessing*/) const
{
    const bool allow_guessing = false;
...
// scoped_refptr<class media::VideoFrame> media::VideoFrame::WrapExternalYuvData(enum media::VideoPixelFormat, class gfx::Size const&, class gfx::Rect const&,
//     class gfx::Size const&, int, int, int, unsigned char*, unsigned char*, unsigned char*, class base::TimeDelta)
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
// 
// void media::PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(
//     const media::VideoFrame* video_frame, 
//     void* rgb_pixels, size_t row_bytes, 
...
gfx::ColorSpace GetColorSpace(const avifImage* image)
{
    ...
    const auto primaries = image->colorPrimaries == AVIF_COLOR_PRIMARIES_UNSPECIFIED ? AVIF_COLOR_PRIMARIES_BT709 : image->colorPrimaries;
    const auto transfer
        = image->transferCharacteristics == AVIF_TRANSFER_CHARACTERISTICS_UNSPECIFIED ? AVIF_TRANSFER_CHARACTERISTICS_SRGB : image->transferCharacteristics;
    const auto matrix = image->matrixCoefficients == AVIF_MATRIX_COEFFICIENTS_UNSPECIFIED ? AVIF_MATRIX_COEFFICIENTS_BT601 : image->matrixCoefficients;
    const auto range = image->yuvRange == AVIF_RANGE_FULL ? gfx::ColorSpace::RangeID::FULL : gfx::ColorSpace::RangeID::LIMITED;
...
media::VideoPixelFormat AvifToVideoPixelFormat(avifPixelFormat fmt, bool has_alpha, int depth)
{
    ...
    // 12-bit YUVA.
    static constexpr media::VideoPixelFormat kYUV420Formats[][3] = { { media::PIXEL_FORMAT_I420, media::PIXEL_FORMAT_YUV420P10, media::PIXEL_FORMAT_YUV420P12 },
        { media::PIXEL_FORMAT_I420A, media::PIXEL_FORMAT_YUV420AP10, media::PIXEL_FORMAT_UNKNOWN } };
    static constexpr media::VideoPixelFormat kYUV422Formats[][3] = { { media::PIXEL_FORMAT_I422, media::PIXEL_FORMAT_YUV422P10, media::PIXEL_FORMAT_YUV422P12 },
        { media::PIXEL_FORMAT_I422A, media::PIXEL_FORMAT_YUV422AP10, media::PIXEL_FORMAT_UNKNOWN } };
    static constexpr media::VideoPixelFormat kYUV444Formats[][3] = { { media::PIXEL_FORMAT_I444, media::PIXEL_FORMAT_YUV444P10, media::PIXEL_FORMAT_YUV444P12 },
        { media::PIXEL_FORMAT_I444A, media::PIXEL_FORMAT_YUV444AP10, media::PIXEL_FORMAT_UNKNOWN } };
    switch (fmt) {
...
inline void WritePixel(float max_channel, const gfx::Point3F& pixel, float alpha, bool premultiply_alpha, uint32_t* rgba_dest)
{
...
inline void WritePixel(float max_channel, const gfx::Point3F& pixel, float alpha, bool premultiply_alpha, uint64_t* rgba_dest)
{
    float rgba_pixels[4];
    ...
    gfx::FloatToHalfFloat(rgba_pixels, reinterpret_cast<uint16_t*>(rgba_dest), std::size(rgba_pixels));
}

enum class ColorType { kMono, kColor };

template <ColorType color_type, typename InputType, typename OutputType>
void YUVAToRGBA(const avifImage* image, const gfx::ColorTransform* transform, bool premultiply_alpha, OutputType* rgba_dest)
{
    ...
    for (uint32_t j = 0; j < image->height; ++j) {
        ...
        const InputType* y_ptr = reinterpret_cast<InputType*>(&image->yuvPlanes[AVIF_CHAN_Y][j * image->yuvRowBytes[AVIF_CHAN_Y]]);
        const InputType* u_ptr = reinterpret_cast<InputType*>(&image->yuvPlanes[AVIF_CHAN_U][uv_j * image->yuvRowBytes[AVIF_CHAN_U]]);
        const InputType* v_ptr = reinterpret_cast<InputType*>(&image->yuvPlanes[AVIF_CHAN_V][uv_j * image->yuvRowBytes[AVIF_CHAN_V]]);
        const InputType* a_ptr = nullptr;
        if (image->alphaPlane) {
            a_ptr = reinterpret_cast<InputType*>(&image->alphaPlane[j * image->alphaRowBytes]);
        }
        ...
        for (uint32_t i = 0; i < image->width; ++i) {
            ...
            WritePixel(max_channel, pixel, alpha / max_channel, premultiply_alpha, rgba_dest);
            rgba_dest++;
        }
    }
}
...
AVIFImageDecoder::AVIFImageDecoder(AlphaOption alpha_option, HighBitDepthDecodingOption hbd_option, const ColorBehavior& color_behavior,
...
wtf_size_t AVIFImageDecoder::DecodedYUVWidthBytes(cc::YUVIndex index) const
{
    ...
    wtf_size_t aligned_width = static_cast<wtf_size_t>(base::bits::AlignUp(Size().width(), 128));
    if (index == cc::YUVIndex::kU || index == cc::YUVIndex::kV) {
...
absl::optional<base::TimeDelta> AVIFImageDecoder::FrameTimestampAtIndex(wtf_size_t index) const
{
    return index < frame_buffer_cache_.size() ? frame_buffer_cache_[index].Timestamp() : absl::nullopt;
}
...
bool AVIFImageDecoder::ImageHasBothStillAndAnimatedSubImages() const
{
    ...
    const char* major_brand = fast_reader.GetConsecutiveData(kMajorBrandOffset, kMajorBrandSize, buf);
...
bool AVIFImageDecoder::UpdateDemuxer()
{
    ...
    if (!decoder_) {
        decoder_ = std::unique_ptr<avifDecoder, void (*)(avifDecoder*)>(avifDecoderCreate(), avifDecoderDestroy);
        if (!decoder_)
            return false;
    ...
    if (!IgnoresColorSpace()) {
        ...
        if (container->icc.size) {
            std::unique_ptr<ColorProfile> profile = ColorProfile::Create(container->icc.data, container->icc.size);
            if (!profile) {
...
avifResult AVIFImageDecoder::DecodeImage(wtf_size_t index)
{
    ...
    if (gfx::Size(image->width, image->height) != Size()) {
        DVLOG(1) << "Frame size " << gfx::Size(image->width, image->height).ToString() << " differs from container size " << Size().ToString();
        return AVIF_RESULT_UNKNOWN_ERROR;
    }
...
void AVIFImageDecoder::UpdateColorTransform(const gfx::ColorSpace& frame_cs, int bit_depth)
{
    ...
    color_transform_ = gfx::ColorTransform::NewColorTransform(frame_cs, gfx::ColorSpace(), options);
}
...
bool AVIFImageDecoder::RenderImage(const avifImage* image, int from_row, int* to_row, ImageFrame* buffer)
{
    ...
    const bool use_libyuv_bilinear_upsampling = !decode_to_half_float_ && IsColorSpaceSupportedByPCVR(image) && image->yuvFormat == AVIF_PIXEL_FORMAT_YUV420;
    ...
    if (decode_to_half_float_) {
        ...
        if (is_mono) {
            YUVAToRGBA<ColorType::kMono, uint16_t>(image, color_transform_.get(), premultiply_alpha, rgba_hhhh);
        } else {
            // TODO(crbug.com/1099820): Add fast path for 10bit 4:2:0 using libyuv.
            YUVAToRGBA<ColorType::kColor, uint16_t>(image, color_transform_.get(), premultiply_alpha, rgba_hhhh);
        }
        return true;
    }
    ...
    if (IsColorSpaceSupportedByPCVR(image)) {
        ...
        auto pixel_format = AvifToVideoPixelFormat(image->yuvFormat, has_alpha, image->depth);
        if (pixel_format == media::PIXEL_FORMAT_UNKNOWN)
            return false;
        auto size = gfx::Size(image->width, image->height);
        scoped_refptr<media::VideoFrame> frame;
        if (has_alpha) {
            frame = media::VideoFrame::WrapExternalYuvaData(pixel_format, size, gfx::Rect(size), size, image->yuvRowBytes[0], image->yuvRowBytes[1],
                image->yuvRowBytes[2], image->alphaRowBytes, image->yuvPlanes[0], image->yuvPlanes[1], image->yuvPlanes[2], image->alphaPlane,
                base::TimeDelta());
        } else {
            frame = media::VideoFrame::WrapExternalYuvData(pixel_format, size, gfx::Rect(size), size, image->yuvRowBytes[0], image->yuvRowBytes[1],
                image->yuvRowBytes[2], image->yuvPlanes[0], image->yuvPlanes[1], image->yuvPlanes[2], base::TimeDelta());
        }
        if (!frame)
            return false;
        ...
        if (save_top_row) {
            ...
            std::copy(rgba_8888, rgba_8888 + image->width, previous_last_decoded_row_.begin());
        }
        ...
        media::PaintCanvasVideoRenderer::ConvertVideoFrameToRGBPixels(
            frame.get(), rgba_8888, frame->visible_rect().width() * 4, premultiply_alpha, media::PaintCanvasVideoRenderer::kFilterBilinear);

        if (save_top_row) {
            std::copy(previous_last_decoded_row_.begin(), previous_last_decoded_row_.end(), rgba_8888);
        }
        return true;
    }
    ...
    if (ImageIsHighBitDepth()) {
        if (is_mono) {
            YUVAToRGBA<ColorType::kMono, uint16_t>(image, color_transform_.get(), premultiply_alpha, rgba_8888);
        } else {
            YUVAToRGBA<ColorType::kColor, uint16_t>(image, color_transform_.get(), premultiply_alpha, rgba_8888);
        }
    } else {
        if (is_mono) {
            YUVAToRGBA<ColorType::kMono, uint8_t>(image, color_transform_.get(), premultiply_alpha, rgba_8888);
        } else {
            YUVAToRGBA<ColorType::kColor, uint8_t>(image, color_transform_.get(), premultiply_alpha, rgba_8888);
        }
    }
    return true;
}
