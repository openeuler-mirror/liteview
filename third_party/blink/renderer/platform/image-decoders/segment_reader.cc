...
template <class Iter> size_t BufferGetSomeData(Iter& iter, size_t& position_of_block, const char*& data, size_t position)
{
...
SharedBufferSegmentReader::SharedBufferSegmentReader(scoped_refptr<SharedBuffer> buffer)
...
size_t ParkableImageSegmentReader::GetSomeData(const char*& data, size_t position) const
{
    if (!parkable_image_)
        return 0;
...
scoped_refptr<SegmentReader> SegmentReader::CreateFromSharedBuffer(scoped_refptr<SharedBuffer> buffer)
{
...
scoped_refptr<SegmentReader> SegmentReader::CreateFromParkableImage(scoped_refptr<ParkableImage> image)
{
