...
public:
    TestUrlData(const GURL& url, CorsMode cors_mode, UrlIndex* url_index, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
public:
    ...
    scoped_refptr<UrlData> NewUrlData(const GURL& url, UrlData::CorsMode cors_mode) override
    {
    ...
    size_t load_queue_size()
    {
        return loading_queue_.size();
    }
...
public:
    MockMultiBufferDataSource(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, scoped_refptr<UrlData> url_data, BufferedDataSourceHost* host)
        : MultiBufferDataSource(
            task_runner, std::move(url_data), &media_log_, host, base::BindRepeating(&MockMultiBufferDataSource::set_downloading, base::Unretained(this)))
        , downloading_(false)
    {
    }
...
public:
    ...
    void InitializeWithCors(const char* url, bool expected, UrlData::CorsMode cors_mode, size_t file_size = kFileSize)
    {
        GURL gurl(url);
        data_source_ = std::make_unique<MockMultiBufferDataSource>(task_runner_, url_index_.GetByUrl(gurl, cors_mode, UrlIndex::kNormal), &host_);
        data_source_->SetPreload(preload_);

        response_generator_ = std::make_unique<TestResponseGenerator>(gurl, file_size);
...
TEST_F(MultiBufferDataSourceTest, Http_ShareData)
{
    ...
    MockMultiBufferDataSource source2(task_runner_, url_index_.GetByUrl(GURL(kHttpUrl), UrlData::CORS_UNSPECIFIED, UrlIndex::kNormal), &host2);
...
{
    GURL gurl(kHttpUrl);
    data_source_ = std::make_unique<MockMultiBufferDataSource>(task_runner_, url_index_.GetByUrl(gurl, UrlData::CORS_UNSPECIFIED, UrlIndex::kNormal), &host_);
    data_source_->SetPreload(preload_);

    response_generator_ = std::make_unique<TestResponseGenerator>(gurl, kDataSize + 1);
    EXPECT_CALL(*this, OnInitialize(true));
    data_source_->Initialize(base::BindOnce(&MultiBufferDataSourceTest::OnInitialize, base::Unretained(this)));
    base::RunLoop().RunUntilIdle();
