...
void ResourceMultiBufferDataProvider::DidReceiveResponse(const WebURLResponse& response)
{
    ...
    if (!redirects_to_.is_empty()) {
        destination_url_data = url_data_->url_index()->GetByUrl(redirects_to_, cors_mode_, UrlIndex::kNormal);
        redirects_to_ = GURL();
    }
    ...
    destination_url_data->set_cacheable(GetReasonsForUncacheability(response) == 0);
...
void ResourceMultiBufferDataProvider::DidReceiveData(const char* data, int data_length)
{
    ...
    while (data_length) {
        ...
        auto to_append = std::min<int64_t>(data_length, block_size() - last_block_size);
        DCHECK_GT(to_append, 0);
        memcpy(fifo_.back()->writable_data() + last_block_size, data, static_cast<size_t>(to_append));
        data += to_append;
    ...
    const std::string& content_range_str, int64_t* first_byte_position, int64_t* last_byte_position, int64_t* instance_size)
{
    ...
    if (!base::StartsWith(content_range_str, kUpThroughBytesUnit, base::CompareCase::SENSITIVE)) {
        return false;
    }
