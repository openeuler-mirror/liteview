...
public:
    ...
    {
        gurl_ = GURL(url);
        url_data_ = url_index_.GetByUrl(gurl_, UrlData::CORS_UNSPECIFIED, UrlIndex::kNormal);
        ...
        std::unique_ptr<ResourceMultiBufferDataProvider> loader(new ResourceMultiBufferDataProvider(
...
protected:
    std::unique_ptr<WebAssociatedURLLoader> CreateUrlLoader(const WebAssociatedURLLoaderOptions& options)
    {
        ...
        EXPECT_CALL(*url_loader.get(), LoadAsynchronously(Truly(CorrectAcceptEncoding), loader_));
        return url_loader;
    }
