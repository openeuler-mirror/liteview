...
protected:
    std::unique_ptr<DataProvider> CreateWriter(const MultiBufferBlockId& pos, bool) override
    {
        ...
        return std::unique_ptr<DataProvider>(new FakeMultiBufferDataProvider(pos, file_size_, max_blocks_after_defer_, must_read_whole_file_, this, rnd_));
    }
...
TEST_F(MultiBufferTest, ReadAll)
{
    ...
    MultiBufferReader reader(&multibuffer_, pos, end, base::NullCallback(), task_runner_);
...
public:
    ReadHelper(size_t end, size_t max_read_size, MultiBuffer* multibuffer, media::TestRandom* rnd, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        ...
        , read_size_(0)
        , rnd_(rnd)
        , reader_(multibuffer, pos_, end_, base::NullCallback(), std::move(task_runner))
    {
...
TEST_F(MultiBufferTest, RandomTest)
{
    ...
    for (size_t i = 0; i < 20; i++) {
        read_helpers.push_back(new ReadHelper(file_size, 1000, &multibuffer_, &rnd_, task_runner_));
    }
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 100; j++) {
            if (rnd_.Rand() & 1) {
