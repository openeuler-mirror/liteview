...
WebContentDecryptionModuleSessionImpl::WebContentDecryptionModuleSessionImpl(
    const scoped_refptr<CdmSessionAdapter>& adapter, WebEncryptedMediaSessionType session_type)
    : adapter_(adapter)
    ...
    media::EmeInitDataType eme_init_data_type, const unsigned char* init_data, size_t init_data_length, WebContentDecryptionModuleResult result)
{
    ...
    if (!IsSupportedKeySystemWithInitDataType(adapter_->GetKeySystem(), eme_init_data_type)) {
    ...
    adapter_->InitializeNewSession(eme_init_data_type, sanitized_init_data, session_type_,
        std::unique_ptr<media::NewSessionCdmPromise>(new NewSessionCdmResultPromise(result, adapter_->GetKeySystemUMAPrefix(), kGenerateRequestUMAName,
            base::BindOnce(&WebContentDecryptionModuleSessionImpl::OnSessionInitialized, weak_ptr_factory_.GetWeakPtr()), { SessionInitStatus::NEW_SESSION })));
}
...
void WebContentDecryptionModuleSessionImpl::Load(const WebString& session_id, WebContentDecryptionModuleResult result)
{
    ...
    adapter_->LoadSession(session_type_, sanitized_session_id,
        std::unique_ptr<media::NewSessionCdmPromise>(new NewSessionCdmResultPromise(result, adapter_->GetKeySystemUMAPrefix(), kLoadSessionUMAName,
            base::BindOnce(&WebContentDecryptionModuleSessionImpl::OnSessionInitialized, weak_ptr_factory_.GetWeakPtr()),
            { SessionInitStatus::NEW_SESSION, SessionInitStatus::SESSION_NOT_FOUND })));
}
...
void WebContentDecryptionModuleSessionImpl::OnSessionKeysChange(bool has_additional_usable_key, media::CdmKeysInfo keys_info)
{
    ...
    for (size_t i = 0; i < keys_info.size(); ++i) {
        ...
        keys[i].SetId(WebData(reinterpret_cast<char*>(&key_info->key_id[0]), key_info->key_id.size()));
...
void WebContentDecryptionModuleSessionImpl::OnSessionExpirationUpdate(base::Time new_expiry_time)
{
    ...
    client_->OnSessionExpirationUpdate(new_expiry_time.is_null() ? std::numeric_limits<double>::quiet_NaN() : new_expiry_time.ToJsTime());
}
