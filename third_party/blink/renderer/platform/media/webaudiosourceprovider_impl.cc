...
public:
    ...
    int Render(base::TimeDelta delay, base::TimeTicks delay_timestamp, int prior_frames_skipped, media::AudioBus* audio_bus) override
    {
        ...
        const int num_rendered_frames = renderer_->Render(delay, delay_timestamp, prior_frames_skipped, audio_bus);
...
void WebAudioSourceProviderImpl::SetClient(WebAudioSourceProviderClient* client)
{
    ...
    if (client) {
        ...
        set_format_cb_ = media::BindToCurrentLoop(WTF::BindRepeating(&WebAudioSourceProviderImpl::OnSetFormat, weak_factory_.GetWeakPtr()));
...
void WebAudioSourceProviderImpl::ProvideInput(const WebVector<float*>& audio_data, int number_of_frames)
{
    ...
    const int frames = tee_filter_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, bus_wrapper_.get());
...
void WebAudioSourceProviderImpl::GetOutputDeviceInfoAsync(OutputDeviceInfoCB info_cb)
{
    ...
    media::BindToCurrentLoop(WTF::BindOnce(std::move(info_cb), media::OutputDeviceInfo(media::OUTPUT_DEVICE_STATUS_OK))).Run();
}
...
{
    return tee_filter_->Render(base::TimeDelta(), base::TimeTicks::Now(), 0, audio_bus);
}
