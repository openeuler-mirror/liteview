...
bool WebSourceBufferImpl::Append(const unsigned char* data, unsigned length, double* timestamp_offset)
{
    base::TimeDelta old_offset = timestamp_offset_;
    bool success = demuxer_->AppendData(id_, data, length, append_window_start_, append_window_end_, &timestamp_offset_);
...
void WebSourceBufferImpl::InitSegmentReceived(std::unique_ptr<media::MediaTracks> tracks)
{
    ...
    for (const auto& track : tracks->tracks()) {
        ...
        trackInfo.byte_stream_track_id = WebString::FromUTF8(base::NumberToString(track->bytestream_track_id()));
