...
bool IsSupportedMediaType(const std::string& container_mime_type, const std::string& codecs, bool use_aes_decryptor)
{
    ...
    if (!use_aes_decryptor && base::ToLowerASCII(container_mime_type) == "video/mp4" && !codec_vector.empty()) {
...
{
    DCHECK(web_frame_);
    WebContentSettingsClient* content_settings_client = web_frame_->GetContentSettingsClient();
    return !content_settings_client || content_settings_client->AllowStorageAccessSync(storage_type);
}
    ...
    const WebMediaKeySystemConfiguration& candidate, ConfigState* config_state, WebMediaKeySystemConfiguration* accumulated_configuration)
{
    ...
    // not supported.
    if (!web_frame_delegate_->AllowStorageAccessSync(WebContentSettingsClient::StorageType::kLocalStorage)) {
...
void KeySystemConfigSelector::SelectConfigInternal(std::unique_ptr<SelectionRequest> request)
{
    ...
    for (size_t i = 0; i < request->candidate_configurations.size(); i++) {
        ...
        case CONFIGURATION_REQUIRES_PERMISSION:
            ...
            media_permission_->RequestPermission(media::MediaPermission::PROTECTED_MEDIA_IDENTIFIER,
