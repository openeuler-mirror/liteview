...
void CompleteWebContentDecryptionModuleResult(
    std::unique_ptr<WebContentDecryptionModuleResult> result, WebContentDecryptionModule* cdm, const std::string& error_message)
{
    DCHECK(result);

    if (!cdm) {
        result->CompleteWithError(kWebContentDecryptionModuleExceptionNotSupportedError, 0, WebString::FromUTF8(error_message));
        return;
    }
...
WebEncryptedMediaClientImpl::WebEncryptedMediaClientImpl(media::CdmFactory* cdm_factory, media::MediaPermission* media_permission,
    ...
    const WebSecurityOrigin& security_origin, const media::CdmConfig& cdm_config, std::unique_ptr<WebContentDecryptionModuleResult> result)
{
    WebContentDecryptionModuleImpl::Create(
        cdm_factory_, security_origin, cdm_config, base::BindOnce(&CompleteWebContentDecryptionModuleResult, std::move(result)));
}
