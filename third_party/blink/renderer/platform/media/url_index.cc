...
UrlData::UrlData(const GURL& url, CorsMode cors_mode, UrlIndex* url_index, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : url_(url)
...
UrlIndex::~UrlIndex()
{
    ...
    auto dcheck_has_one_ref = [](const UrlDataMap::value_type& entry) { DCHECK(entry.second->HasOneRef()); };
...
scoped_refptr<UrlData> UrlIndex::GetByUrl(const GURL& gurl, UrlData::CorsMode cors_mode, CacheMode cache_mode)
{
...
scoped_refptr<UrlData> UrlIndex::NewUrlData(const GURL& url, UrlData::CorsMode cors_mode)
{
...
scoped_refptr<UrlData> UrlIndex::TryInsert(const scoped_refptr<UrlData>& url_data)
{
    ...
    if (url_data->Valid()) {
        if ((!iter->second->Valid() || url_data->CachedSize() > iter->second->CachedSize())) {
            iter->second = url_data;
        } else {
