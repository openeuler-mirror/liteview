...
public:
    ...
    scoped_refptr<UrlData> GetByUrl(const GURL& gurl, UrlData::CorsMode cors_mode)
    {
        scoped_refptr<UrlData> ret = url_index_.GetByUrl(gurl, cors_mode, UrlIndex::kNormal);
...
TEST_F(UrlIndexTest, GetByUrlCacheDisabled)
{
    ...
    scoped_refptr<UrlData> url_data = url_index_.GetByUrl(url, cors, UrlIndex::kNormal);
    url_data->Use();
