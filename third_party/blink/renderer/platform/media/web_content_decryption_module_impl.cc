...
bool ConvertHdcpVersion(const WebString& hdcp_version_string, media::HdcpVersion* hdcp_version)
{
...
void WebContentDecryptionModuleImpl::Create(
    media::CdmFactory* cdm_factory, const WebSecurityOrigin& security_origin, const media::CdmConfig& cdm_config, WebCdmCreatedCB web_cdm_created_cb)
{
    ...
    if (security_origin.IsOpaque() || security_origin.ToString() == "null") {
        std::move(web_cdm_created_cb).Run(nullptr, "EME use is not allowed on unique origins.");
        return;
    }
...
WebContentDecryptionModuleImpl::WebContentDecryptionModuleImpl(scoped_refptr<CdmSessionAdapter> adapter)
    : adapter_(adapter)
{
}
...
void WebContentDecryptionModuleImpl::GetStatusForPolicy(const WebString& min_hdcp_version_string, WebContentDecryptionModuleResult result)
{
    ...
    adapter_->GetStatusForPolicy(min_hdcp_version,
