...
void MultiBufferDataSource::CreateResourceLoader(int64_t first_byte_position, int64_t last_byte_position)
{
    ...
    SetReader(new MultiBufferReader(url_data_->multibuffer(), first_byte_position, last_byte_position,
...
void MultiBufferDataSource::CreateResourceLoader_Locked(int64_t first_byte_position, int64_t last_byte_position)
{
    ...
    reader_ = std::make_unique<MultiBufferReader>(url_data_->multibuffer(), first_byte_position, last_byte_position,
        base::BindRepeating(&MultiBufferDataSource::ProgressCallback, weak_ptr_), render_task_runner_);
    UpdateBufferSizes();
}
...
void MultiBufferDataSource::OnRedirected(const scoped_refptr<UrlData>& new_destination)
{
    ...
    if (url_data_->url().DeprecatedGetOriginAsURL() != new_destination->url().DeprecatedGetOriginAsURL()) {
...
void MultiBufferDataSource::UpdateBufferSizes()
{
    ...
    int64_t preload = base::clamp(preload_seconds_.value() * bytes_per_second, kMinBufferPreload, kMaxBufferPreload);
    ...
    int64_t pin_backward = base::clamp(keep_after_playback_seconds_.value() * bytes_per_second, kMinBufferPreload, kMaxBufferPreload);
