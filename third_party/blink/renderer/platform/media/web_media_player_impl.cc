...
void DestructionHelper(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> vfc_task_runner,
    std::unique_ptr<Demuxer> demuxer, std::unique_ptr<media::DataSource> data_source, std::unique_ptr<VideoFrameCompositor> compositor,
    ...
    std::unique_ptr<media::RendererFactorySelector> renderer_factory_selector, std::unique_ptr<WebSurfaceLayerBridge> bridge, bool is_chunk_demuxer)
{
    ...
    base::ThreadPool::PostTask(FROM_HERE, { base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        base::BindOnce(
            [](scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, std::unique_ptr<Demuxer> demuxer_to_destroy,
            ...
            std::move(main_task_runner), std::move(demuxer), std::move(cdm_context_1), std::move(cdm_context_2), std::move(media_log)));
}
...
WebMediaPlayerImpl::WebMediaPlayerImpl(WebLocalFrame* frame, WebMediaPlayerClient* client, WebMediaPlayerEncryptedMediaClient* encrypted_client,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> media_task_runner, scoped_refptr<base::TaskRunner> worker_task_runner,
    ...
    WebMediaPlayerBuilder::AdjustAllocatedMemoryCB adjust_allocated_memory_cb, WebContentDecryptionModule* initial_cdm,
    ...
    , demuxer_override_(std::move(demuxer_override))
{
    ...
    pipeline_controller_ = std::make_unique<media::PipelineController>(std::move(pipeline),
        base::BindRepeating(&WebMediaPlayerImpl::OnPipelineSeeked, weak_this_), base::BindRepeating(&WebMediaPlayerImpl::OnPipelineSuspended, weak_this_),
        base::BindRepeating(&WebMediaPlayerImpl::OnBeforePipelineResume, weak_this_), base::BindRepeating(&WebMediaPlayerImpl::OnPipelineResumed, weak_this_),
    ...
    auto on_audio_source_provider_set_client_callback = base::BindOnce(
        [](base::WeakPtr<WebMediaPlayerImpl> self, WebMediaPlayerClient* const client) {
            if (!self)
                return;
    ...
    audio_source_provider_
        = new WebAudioSourceProviderImpl(std::move(audio_renderer_sink), media_log_.get(), std::move(on_audio_source_provider_set_client_callback));
  ...
  /*  main_thread_mem_dumper_ = std::make_unique<media::MemoryDumpProviderProxy>(
    ...
    renderer_factory_selector_->SetRemotePlayStateChangeCB(
        media::BindToCurrentLoop(base::BindRepeating(&WebMediaPlayerImpl::OnRemotePlayStateChange, weak_this_)));
...
WebMediaPlayerImpl::~WebMediaPlayerImpl()
{
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&DestructionHelper, std::move(main_task_runner_), std::move(vfc_task_runner_), std::move(demuxer_), std::move(data_source_),
            std::move(compositor_), std::move(cdm_context_ref_), std::move(pending_cdm_context_ref_), std::move(media_log_),
            std::move(renderer_factory_selector_), std::move(bridge_), !!chunk_demuxer_));
}
...
WebMediaPlayer::LoadTiming WebMediaPlayerImpl::Load(LoadType load_type, const WebMediaPlayerSource& source, CorsMode cors_mode, bool is_cache_disabled)
{
    ...
    DVLOG(1) << __func__ << "(" << load_type << ", " << GURL(url) << ", " << cors_mode << ")";
...
void WebMediaPlayerImpl::DoLoad(LoadType load_type, const WebURL& url, CorsMode cors_mode, bool is_cache_disabled)
{
    ...
    media_log_->AddEvent<MediaLogEvent::kLoad>(url.GetString().Substring(0, media::kMaxUrlLength + 1).Utf8());
    load_start_time_ = base::TimeTicks::Now();

    std::vector<media::TextTrackConfig> text_configs;
    for (const auto& metadata : client_->GetTextTrackMetadata()) {
        text_configs.emplace_back(media::TextTrackConfig::ConvertKind(metadata.kind()), metadata.label(), metadata.language(), metadata.id());
    }
    ...
    //    load_type == kLoadTypeURL ? GetMediaURLScheme(loaded_url_) : media::mojom::MediaURLScheme::kUnknown, media::mojom::MediaStreamType::kNone);
    ...
    if (demuxer_override_ || load_type == kLoadTypeMediaSource) {
        ...
        if (loaded_url_.SchemeIs(url::kDataScheme)) {
            ...
            if (!net::DataURL::Parse(loaded_url_, &mime_type, &charset, &data) || data.empty()) {
        ...
        auto url_data = url_index_->GetByUrl(url, static_cast<UrlData::CorsMode>(cors_mode), is_cache_disabled ? UrlIndex::kCacheDisabled : UrlIndex::kNormal);
        mb_data_source_ = new MultiBufferDataSource(main_task_runner_, std::move(url_data), media_log_.get(), buffered_data_source_host_.get(),
            base::BindRepeating(&WebMediaPlayerImpl::NotifyDownloading, weak_this_));
        data_source_.reset(mb_data_source_);
        mb_data_source_->OnRedirect(base::BindRepeating(&WebMediaPlayerImpl::OnDataSourceRedirected, weak_this_));
        mb_data_source_->SetPreload(preload_);
        mb_data_source_->SetIsClientAudioElement(client_->IsAudioElement());
        mb_data_source_->Initialize(base::BindOnce(&WebMediaPlayerImpl::DataSourceInitialized, weak_this_));
    }
}
...
void WebMediaPlayerImpl::DoSeek(base::TimeDelta time, bool time_updated)
{
    ...
    //      never elided.
    if (paused_ && pipeline_controller_->IsStable() && (paused_time_ == time || (ended_ && time == base::Seconds(Duration()))) && !chunk_demuxer_) {
...
void WebMediaPlayerImpl::SetVolume(double volume)
{
    ...
    //        delegate_has_audio_, HasVideo(), pipeline_metadata_.audio_decoder_config.codec(), pipeline_metadata_.video_decoder_config.codec(), content_type);
...
void WebMediaPlayerImpl::SetWasPlayedWithUserActivation(bool was_played_with_user_activation)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    pipeline_controller_->SetWasPlayedWithUserActivation(was_played_with_user_activation);
}
...
STATIC_ASSERT_ENUM(WebMediaPlayer::kPreloadMetaData, MultiBufferDataSource::METADATA);
...
void WebMediaPlayerImpl::EnabledAudioTracksChanged(const WebVector<WebMediaPlayer::TrackId>& enabledTrackIds)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    ...
    MEDIA_LOG(INFO, media_log_.get()) << "Enabled audio tracks: [" << logstr.str() << "]";
    pipeline_controller_->OnEnabledAudioTracksChanged(enabledMediaTrackIds);
}

void WebMediaPlayerImpl::SelectedVideoTrackChanged(WebMediaPlayer::TrackId* selectedTrackId)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    ...
    MEDIA_LOG(INFO, media_log_.get()) << "Selected video track: [" << selected_video_track_id.value_or(MediaTrack::Id()) << "]";
...
WebTimeRanges WebMediaPlayerImpl::Seekable() const
{
    ...
    const bool is_finite_stream = mb_data_source_ && mb_data_source_->IsStreaming() && std::isfinite(seekable_end);
...
void WebMediaPlayerImpl::Paint(cc::PaintCanvas* canvas, const gfx::Rect& rect, cc::PaintFlags& flags)
{
    ...
    //    video_frame, canvas, gfx::RectF(rect), flags, pipeline_metadata_.video_decoder_config.video_transformation(), raster_context_provider_.get());
}
...
media::PaintCanvasVideoRenderer* WebMediaPlayerImpl::GetPaintCanvasVideoRenderer()
{
    ...
    // todo(mb): 百度首页现在会请求 webgl, 走到这里, 暂时注释掉, 不会影响快乐路径
    //*(int*)1 = 1;
    return nullptr;
}
...
bool WebMediaPlayerImpl::WouldTaintOrigin() const
{
    ...
    if (!mb_data_source_->HasSingleOrigin() && mb_data_source_->cors_mode() == UrlData::CORS_UNSPECIFIED) {
        return true;
    }
...
void WebMediaPlayerImpl::OnFFmpegMediaTracksUpdated(std::unique_ptr<media::MediaTracks> tracks)
{
    ...
    for (const auto& track : tracks->tracks()) {
        if (track->type() == MediaTrack::Audio) {
            client_->AddAudioTrack(WebString::FromUTF8(track->id().value()), WebMediaPlayerClient::kAudioTrackKindMain,
                WebString::FromUTF8(track->label().value()), WebString::FromUTF8(track->language().value()), is_first_audio_track);
            is_first_audio_track = false;
        } else if (track->type() == MediaTrack::Video) {
            client_->AddVideoTrack(WebString::FromUTF8(track->id().value()), WebMediaPlayerClient::kVideoTrackKindMain,
                WebString::FromUTF8(track->label().value()), WebString::FromUTF8(track->language().value()), is_first_video_track);
...
void WebMediaPlayerImpl::OnPipelineSuspended()
{
    ...
    if (mb_data_source_ && !client_->CouldPlayIfEnoughData()) {
        ...
        if (attempting_suspended_start_ && did_lazy_load_) {
            ...
            have_enough_after_lazy_load_cb_.Reset(base::BindOnce(&MultiBufferDataSource::OnBufferingHaveEnough, base::Unretained(mb_data_source_), true));
...
void WebMediaPlayerImpl::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    DVLOG(2) << __func__ << " memory_pressure_level=" << memory_pressure_level;
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    DCHECK(base::FeatureList::IsEnabled(media::kMemoryPressureBasedSourceBufferGC));
    DCHECK(chunk_demuxer_);
    ...
    // SourceBufferStreams.
    bool force_instant_gc = (enable_instant_source_buffer_gc_ && memory_pressure_level == base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    ...
    media_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &media::ChunkDemuxer::OnMemoryPressure, base::Unretained(chunk_demuxer_), base::Seconds(CurrentTime()), memory_pressure_level, force_instant_gc));
}
...
void WebMediaPlayerImpl::OnError(media::PipelineStatus status)
{
    ...
    const bool found_hls = base::FeatureList::IsEnabled(media::kHlsPlayer) && status == media::DEMUXER_ERROR_DETECTED_HLS;
    if (found_hls && mb_data_source_) {
        ...
        UMA_HISTOGRAM_BOOLEAN("Media.WebMediaPlayerImpl.HLS.IsCorsCrossOrigin", mb_data_source_->IsCorsCrossOrigin());
        if (mb_data_source_->IsCorsCrossOrigin()) {
            UMA_HISTOGRAM_BOOLEAN("Media.WebMediaPlayerImpl.HLS.HasAccessControl", mb_data_source_->HasAccessControl());
        }

        MimeType mime_type = TranslateMimeTypeToHistogramEnum(mb_data_source_->mime_type());
        ...
        bool frame_url_is_cryptographic = url::Origin(frame_->GetSecurityOrigin()).GetURL().SchemeIsCryptographic();
        bool manifest_url_is_cryptographic = loaded_url_.SchemeIsCryptographic() && mb_data_source_->GetUrlAfterRedirects().SchemeIsCryptographic();
        UMA_HISTOGRAM_BOOLEAN("Media.WebMediaPlayerImpl.HLS.IsMixedContent", frame_url_is_cryptographic && !manifest_url_is_cryptographic);
        ...
        media_task_runner_->PostTask(FROM_HERE,
            media::BindToCurrentLoop(base::BindOnce(
                [](std::unique_ptr<Demuxer> demuxer, std::unique_ptr<media::DataSource> data_source, base::OnceClosure start_pipeline_cb) {
                ...
                std::move(demuxer_), std::move(data_source_), base::BindOnce(&WebMediaPlayerImpl::StartPipeline, weak_this_))));
...
void WebMediaPlayerImpl::OnDurationChange()
{
    ...
    if (frame_->IsAdFrame()) {
        UMA_HISTOGRAM_CUSTOM_TIMES("Ads.Media.Duration", GetPipelineMediaDuration(), base::Milliseconds(1), base::Days(1), 50 /* bucket_count */);
    }
...
void WebMediaPlayerImpl::OnAddTextTrack(const media::TextTrackConfig& config, media::AddTextTrackDoneCB done_cb)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    *(int*)1 = 1;
    ...
    //std::unique_ptr<WebInbandTextTrackImpl> web_inband_text_track(new WebInbandTextTrackImpl(web_kind, web_label, web_language, web_id));

    //std::unique_ptr<media::TextTrack> text_track(new TextTrackImpl(main_task_runner_, client_, std::move(web_inband_text_track)));
...
void WebMediaPlayerImpl::OnFrameHidden()
{
    ...
    vfc_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VideoFrameCompositor::SetIsPageVisible, base::Unretained(compositor_.get()), !IsHidden()));
}
...
void WebMediaPlayerImpl::OnRemotePlayStateChange(media::MediaStatus::State state)
{
    ...
    if (state == media::MediaStatus::State::PLAYING && Paused()) {
        ...
        client_->PausePlayback(WebMediaPlayerClient::PauseReason::kRemotePlayStateChange);
    }
}
#endif // BUILDFLAG(IS_ANDROID)
...
void WebMediaPlayerImpl::DataSourceInitialized(bool success)
{
    ...
    if (IsStreaming() && preload_ > MultiBufferDataSource::METADATA && mb_data_source_) {
...
std::unique_ptr<media::Renderer> WebMediaPlayerImpl::CreateRenderer(absl::optional<media::RendererType> renderer_type)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
...
#if BUILDFLAG(IS_ANDROID)
    request_overlay_info_cb = media::BindToCurrentLoop(base::BindRepeating(&WebMediaPlayerImpl::OnOverlayInfoRequested, weak_this_));
#endif
...
void WebMediaPlayerImpl::StartPipeline()
{
    ...
    Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb
        = media::BindToCurrentLoop(base::BindRepeating(&WebMediaPlayerImpl::OnEncryptedMediaInitData, weak_this_));

    vfc_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VideoFrameCompositor::SetOnNewProcessedFrameCallback, base::Unretained(compositor_.get()),
            media::BindToCurrentLoop(base::BindOnce(&WebMediaPlayerImpl::OnFirstFrame, weak_this_))));

#if BUILDFLAG(IS_ANDROID)
    if (demuxer_found_hls_ || renderer_factory_selector_->GetCurrentFactory()->GetRequiredMediaResourceType() == media::MediaResource::Type::URL) {
        ...
        SetDemuxer(std::make_unique<media::MediaUrlDemuxer>(media_task_runner_, loaded_url_, frame_->GetDocument().SiteForCookies(),
            frame_->GetDocument().TopFrameOrigin(), allow_media_player_renderer_credentials_, demuxer_found_hls_));
        pipeline_controller_->Start(media::Pipeline::StartType::kNormal, demuxer_.get(), this, false, false);
        return;
    }
#endif // BUILDFLAG(IS_ANDROID)
    ...
    if (demuxer_override_) {
        ...
        Demuxer::MediaTracksUpdatedCB media_tracks_updated_cb
            = media::BindToCurrentLoop(base::BindRepeating(&WebMediaPlayerImpl::OnFFmpegMediaTracksUpdated, weak_this_));

        SetDemuxer(std::make_unique<media::FFmpegDemuxer>(
            media_task_runner_, data_source_.get(), encrypted_media_init_data_cb, media_tracks_updated_cb, media_log_.get(), IsLocalFile(loaded_url_)));
#else
        OnError(media::DEMUXER_ERROR_COULD_NOT_OPEN);
        return;
#endif
        ...
        chunk_demuxer_ = new media::ChunkDemuxer(media::BindToCurrentLoop(base::BindOnce(&WebMediaPlayerImpl::OnDemuxerOpened, weak_this_)),
            media::BindToCurrentLoop(base::BindRepeating(&WebMediaPlayerImpl::OnProgress, weak_this_)), encrypted_media_init_data_cb, media_log_.get());
        ...
        if (base::FeatureList::IsEnabled(media::kMemoryPressureBasedSourceBufferGC)) {
            ...
            memory_pressure_listener_
                = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&WebMediaPlayerImpl::OnMemoryPressure, base::Unretained(this)));
        }
    }

    // If possible attempt to avoid decoder spool up until playback starts.
    auto start_type = media::Pipeline::StartType::kNormal;
    if (!chunk_demuxer_ && preload_ == MultiBufferDataSource::METADATA && !client_->CouldPlayIfEnoughData() && !IsStreaming()) {
        start_type = (has_poster_ || base::FeatureList::IsEnabled(media::kPreloadMetadataLazyLoad))
    ...
    seeking_ = true;
    pipeline_controller_->Start(start_type, demuxer_.get(), this, IsStreaming(), is_static);
}
...
void WebMediaPlayerImpl::SetReadyState(WebMediaPlayer::ReadyState state)
{
    ...
    if (state == WebMediaPlayer::kReadyStateHaveEnoughData && data_source_ && data_source_->AssumeFullyBuffered()
...
void WebMediaPlayerImpl::UpdatePlayState()
{
    ...
    //    power_status_helper_->SetIsPlaying(!paused_ && !seeking_ && !IsHidden() && !state.is_suspended && ready_state_ == kReadyStateHaveEnoughData);
    //}
    UpdateSmoothnessHelper();
}
...
void WebMediaPlayerImpl::SetDemuxer(std::unique_ptr<Demuxer> demuxer)
{
    ...
    media_thread_mem_dumper_ = std::make_unique<media::MemoryDumpProviderProxy>("WebMediaPlayer_MediaThread", media_task_runner_,
        base::BindRepeating(&WebMediaPlayerImpl::OnMediaThreadMemoryDump, media_player_id_, base::Unretained(demuxer_.get())));
}
...
void WebMediaPlayerImpl::ReportMemoryUsage()
{
    ...
    if (demuxer_ && !IsNetworkStateError(network_state_)) {
        base::PostTaskAndReplyWithResult(media_task_runner_.get(), FROM_HERE, base::BindOnce(&Demuxer::GetMemoryUsage, base::Unretained(demuxer_.get())),
            base::BindOnce(&WebMediaPlayerImpl::FinishMemoryUsageReport, weak_this_));
    } else {
...
void WebMediaPlayerImpl::ScheduleIdlePauseTimer()
{
    ...
    if ((paused_ && !paused_when_hidden_) || !pipeline_controller_->IsSuspended() || !HasAudio()) {
        return;
    }
    ...
    background_pause_timer_.Start(FROM_HERE, base::Seconds(5),
        base::BindOnce(&WebMediaPlayerClient::PausePlayback, base::Unretained(client_), WebMediaPlayerClient::PauseReason::kSuspendedPlayerIdleTimeout));
}
...
void WebMediaPlayerImpl::CreateWatchTimeReporter()
{
    ...
    //watch_time_reporter_
    //    = std::make_unique<WatchTimeReporter>(media::mojom::PlaybackProperties::New(pipeline_metadata_.has_audio, has_video, false, false, !!chunk_demuxer_,
    //                                              is_encrypted_, embedded_media_experience_enabled_, media::mojom::MediaStreamType::kNone),
...
{
    compositor_->SetOnFramePresentedCallback(media::BindToCurrentLoop(base::BindOnce(&WebMediaPlayerImpl::OnNewFramePresentedCallback, weak_this_)));
}
...
bool WebMediaPlayerImpl::ShouldPausePlaybackWhenHidden() const
{
    ...
    if (using_media_player_renderer_ && pipeline_metadata_.natural_size.IsEmpty()) {
        return false;
    }
    ...
    return IsBackgroundVideoPauseOptimizationEnabled() && !HasAudio() && IsBackgroundOptimizationCandidate() && !is_flinging_;
}
...
bool WebMediaPlayerImpl::IsBackgroundOptimizationCandidate() const
{
    ...
    constexpr base::TimeDelta kMaxKeyframeDistanceToDisableBackgroundVideo = base::Milliseconds(kMaxKeyframeDistanceToDisableBackgroundVideoMs);
...
void WebMediaPlayerImpl::PauseVideoIfNeeded()
{
    ...
    // return.
    client_->PausePlayback(WebMediaPlayerClient::PauseReason::kBackgroundVideoOptimization);
...
#define UMA_HISTOGRAM_VIDEO_HEIGHT(name, sample) UMA_HISTOGRAM_CUSTOM_COUNTS(name, sample, 100, 10000, 50)
...
void WebMediaPlayerImpl::RecordTimingUMA(const std::string& key, base::TimeDelta elapsed)
{
    if (chunk_demuxer_)
...
{
    if (renderer_type_ != media::RendererType::kDefault && renderer_type_ != media::RendererType::kMediaFoundation) {
        return;
    }
