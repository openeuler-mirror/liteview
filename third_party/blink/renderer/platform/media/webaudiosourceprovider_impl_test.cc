...
TEST_F(WebAudioSourceProviderImplTest, ProvideInput)
{
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), 0, bus2.get());
    ...
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), 0, bus1.get());
    fake_callback_.reset();
    fake_callback_.Render(base::TimeDelta(), base::TimeTicks::Now(), 0, bus2.get());
...
{
    wasp_impl_
        = new WebAudioSourceProviderImpl(mock_sink_, &media_log_, base::BindOnce(&WebAudioSourceProviderImplTest::OnClientSet, weak_factory_.GetWeakPtr()));
