...
PLATFORM_EXPORT bool ComputeDigest(HashAlgorithm, const char* digestable, size_t length, DigestValue& digest_result);
...
template <> struct DefaultHash<blink::DigestValue> {
    STATIC_ONLY(DefaultHash);
    typedef DigestValueHash Hash;
};

template <> struct DefaultHash<blink::HashAlgorithm> {
    STATIC_ONLY(DefaultHash);
    typedef IntHash<blink::HashAlgorithm> Hash;
};
template <> struct HashTraits<blink::HashAlgorithm> : UnsignedWithZeroKeyHashTraits<blink::HashAlgorithm> {
