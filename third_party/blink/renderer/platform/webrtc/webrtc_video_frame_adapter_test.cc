...
TEST(WebRtcVideoFrameAdapterTest, MapFullFrameIsZeroCopy)
{
    ...
    scoped_refptr<MockSharedResources> resources = new testing::StrictMock<MockSharedResources>();

    auto frame_720p = CreateTestFrame(kSize720p, kRect720p, kSize720p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, std::vector<scoped_refptr<media::VideoFrame>>(), resources));
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameCreatesNewFrameWhenNotPreScaled)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    const gfx::Size kSize720p(1280, 720);
    const gfx::Rect kRect720p(0, 0, 1280, 720);
    ...
    EXPECT_CALL(*resources, CreateFrame)
        .WillOnce(testing::Invoke([](media::VideoPixelFormat format, const gfx::Size& coded_size, const gfx::Rect& visible_rect, const gfx::Size& natural_size,
            ...
            return CreateTestFrame(coded_size, visible_rect, natural_size, media::VideoFrame::STORAGE_OWNED_MEMORY, format);
        }));
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameUsesPreScaling)
{
    ...
    auto frame_720p = CreateTestFrame(kSize720p, kRect720p, kSize720p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    auto frame_360p = CreateTestFrame(kSize360p, kRect360p, kSize360p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, std::vector<scoped_refptr<media::VideoFrame>>({ frame_360p }), resources));
...
TEST(WebRtcVideoFrameAdapterTest, MapPreScaledFrameEvenIfOriginalFrameHasSoftAppliedScaling)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    const gfx::Size kSize720p(1280, 720);
    const gfx::Rect kRect720p(0, 0, 1280, 720);
    ...
    scoped_refptr<media::VideoFrame> soft_scaled_frame = media::VideoFrame::WrapVideoFrame(frame_720p, frame_720p->format(), kRect720p, kSize360p);

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(soft_scaled_frame, std::vector<scoped_refptr<media::VideoFrame>>({ frame_360p }), resources));
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameScalesFromClosestFrame)
{
    ...
    auto frame_720p = CreateTestFrame(kSize720p, kRect720p, kSize720p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    auto frame_480p = CreateTestFrame(kSize480p, kRect480p, kSize480p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    auto frame_360p = CreateTestFrame(kSize360p, kRect360p, kSize360p, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, std::vector<scoped_refptr<media::VideoFrame>>({ frame_480p, frame_360p }), resources));

    auto scaled_frame = multi_buffer->Scale(kSize432p.width(), kSize432p.height());
    ...
    // can tell by looking at the coded size.
    auto adapted_frame = multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(kRect720p, kSize432p));
    ASSERT_TRUE(adapted_frame);
...
TEST(WebRtcVideoFrameAdapterTest, MapScaledFrameScalesFromClosestPreviouslyScaledFrameWithCropping)
{
    ...
    auto full_frame = CreateTestFrame(
        kFullCodedSize720p, kFullVisibleRect, kFullNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);

    rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(full_frame, std::vector<scoped_refptr<media::VideoFrame>>(), resources));
...
TEST(WebRtcVideoFrameAdapterTest, CanApplyCropAndScale)
{
    ...
    auto cropped_frame1 = multi_buffer->CropAndScale(
        kCroppedRect1.x(), kCroppedRect1.y(), kCroppedRect1.width(), kCroppedRect1.height(), kCroppedRect1.width(), kCroppedRect1.height());
    ...
    auto adapted_frame = multi_buffer->GetAdaptedVideoBufferForTesting(
        WebRtcVideoFrameAdapter::ScaledBufferSize(kCroppedRect1, gfx::Size(kCroppedRect1.width(), kCroppedRect1.height())));
    ASSERT_TRUE(adapted_frame);
    EXPECT_EQ(adapted_frame->coded_size(), frame_720p->coded_size());

    // Apply further cropping and scaling on the already cropped frame.
    auto cropped_frame2 = cropped_frame1->CropAndScale(
        kCroppedRect2.x(), kCroppedRect2.y(), kCroppedRect2.width(), kCroppedRect2.height(), kScaledSize2.width(), kScaledSize2.height());
    ...
    // can tell by looking at the coded size.
    adapted_frame = multi_buffer->GetAdaptedVideoBufferForTesting(WebRtcVideoFrameAdapter::ScaledBufferSize(
        // The second cropped rectangle is relative to the first one.
        gfx::Rect(kCroppedRect1.x() + kCroppedRect2.x(), kCroppedRect1.y() + kCroppedRect2.y(), kCroppedRect2.width(), kCroppedRect2.height()), kScaledSize2));
    ASSERT_TRUE(adapted_frame);
...
TEST(WebRtcVideoFrameAdapterTest, FrameFeedbackSetsMappedSizesWithScale)
{
    ...
    EXPECT_THAT(resources->GetFeedback().mapped_sizes, ElementsAre(kSize720p, kSize360p));
}

TEST(WebRtcVideoFrameAdapterTest, FrameFeedbackSetsMappedSizesWithCropAndScale)
{
    std::vector<webrtc::VideoFrameBuffer::Type> kNv12 = { webrtc::VideoFrameBuffer::Type::kNV12 };
    const gfx::Size kSize720p(1280, 720);
    const gfx::Rect kRect720p(0, 0, 1280, 720);
    ...
    // a cropped area.
    const gfx::Rect kFullScaleCropRect(100, 100, kSize720p.width() - 200, kSize720p.height() - 200);
    {
        rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
            new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, std::vector<scoped_refptr<media::VideoFrame>>(), resources));
        multi_buffer
            ->CropAndScale(kFullScaleCropRect.x(), kFullScaleCropRect.y(), kFullScaleCropRect.width(), kFullScaleCropRect.height(), kFullScaleCropRect.width(),
                kFullScaleCropRect.height())
    ...
    const gfx::Size kDownScaleBufferSize(kSize720p.width() / kDownScaleFactor, kSize720p.height() / kDownScaleFactor);
    const gfx::Size kDownScaleCropSize(kFullScaleCropRect.width() / kDownScaleFactor, kFullScaleCropRect.height() / kDownScaleFactor);
    {
        rtc::scoped_refptr<WebRtcVideoFrameAdapter> multi_buffer(
            new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame_720p, std::vector<scoped_refptr<media::VideoFrame>>(), resources));
        multi_buffer
            ->CropAndScale(kFullScaleCropRect.x(), kFullScaleCropRect.y(), kFullScaleCropRect.width(), kFullScaleCropRect.height(), kDownScaleCropSize.width(),
                kDownScaleCropSize.height())
            ->GetMappedFrameBuffer(kNv12);
    }
    EXPECT_THAT(resources->GetFeedback().mapped_sizes, ElementsAre(kDownScaleBufferSize));
}
