...
TEST_P(ConvertToWebRtcVideoFrameBufferParamTest, ToI420)
{
    ...
    scoped_refptr<media::VideoFrame> frame = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, storage_type, pixel_format);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, ToI420ADownScale)
{
    ...
    const gfx::Rect kVisibleRect(0, 120, 1280, 720);
    const gfx::Size kNaturalSize(640, 360);
    scoped_refptr<WebRtcVideoFrameAdapter::SharedResources> resources = new WebRtcVideoFrameAdapter::SharedResources(nullptr);
    ...
    auto owned_memory_frame
        = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_I420A);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, Nv12OwnedMemoryFrame)
{
    ...
    // VideoFrame backed by owned memory.
    auto owned_memory_frame
        = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> owned_memory_frame_buffer = ConvertToWebRtcVideoFrameBuffer(std::move(owned_memory_frame), resources);
    EXPECT_EQ(owned_memory_frame_buffer->width(), kNaturalSize.width());
    EXPECT_EQ(owned_memory_frame_buffer->height(), kNaturalSize.height());
...
TEST(ConvertToWebRtcVideoFrameBufferTest, TextureFrameIsBlackWithNoSharedResources)
{
    ...
    // VideoFrame backed by owned memory.
    auto owned_memory_frame
        = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OPAQUE, media::VideoPixelFormat::PIXEL_FORMAT_NV12);
...
TEST(ConvertToWebRtcVideoFrameBufferTest, ConvertsTextureFrameWithSharedResources)
{
    ...
    scoped_refptr<media::VideoFrame> memory_frame
        = CreateTestFrame(kCodedSize, kVisibleRect, kNaturalSize, media::VideoFrame::STORAGE_OWNED_MEMORY, media::VideoPixelFormat::PIXEL_FORMAT_ARGB);
    // fill mock image with whilte color.
    memset(memory_frame->writable_data(media::VideoFrame::kARGBPlane), 0xFF, kCodedSize.GetArea() * 4);
