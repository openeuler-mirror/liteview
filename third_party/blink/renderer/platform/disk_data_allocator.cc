...
std::unique_ptr<DiskDataMetadata> DiskDataAllocator::Write(const void* data, size_t size)
{
    ...
    return std::unique_ptr<DiskDataMetadata>(new DiskDataMetadata(chosen_chunk.start_offset(), chosen_chunk.size()));
}
...
void DiskDataAllocator::Read(const DiskDataMetadata& metadata, void* data)
{
    ...
    DoRead(metadata.start_offset(), data_char, base::checked_cast<int>(metadata.size()));
...
int DiskDataAllocator::DoWrite(int64_t offset, const char* data, int size)
{
    ...
    if (rv != size) {
        LOG(ERROR) << "DISK: Cannot write to disk. written = " << rv << " " << base::File::ErrorToString(base::File::GetLastFileError());
    }
    return rv;
}
