...
String GetDomainAndRegistry(const String& host, PrivateRegistryFilter filter)
{
    ...
    std::string domain = net::registry_controlled_domains::GetDomainAndRegistry(host_utf8.AsStringPiece(), getNetPrivateRegistryFilter(filter));
    return String(domain.data(), domain.length());
    //return host;
}

std::tuple<int, ResourceResponse, scoped_refptr<SharedBuffer>> ParseDataURL(const KURL& url, const String& method)
{
...
Vector<char> ParseMultipartBoundary(const AtomicString& content_type_header)
{
    ...
    Vector<char> result;
    result.Append(boundary.data(), base::checked_cast<wtf_size_t>(boundary.size()));
    return result;
}
