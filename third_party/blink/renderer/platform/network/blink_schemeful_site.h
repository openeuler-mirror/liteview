...
template <> struct DefaultHash<blink::BlinkSchemefulSite> {
    struct Hash {
        STATIC_ONLY(Hash);
        ...
        static bool Equal(const blink::BlinkSchemefulSite& a, const blink::BlinkSchemefulSite& b)
        {
            return blink::SecurityOriginHash::Equal(a.site_as_origin_, b.site_as_origin_);
        }
...
template <> struct HashTraits<blink::BlinkSchemefulSite> : SimpleClassHashTraits<blink::BlinkSchemefulSite> {
    ...
    {
        return HashTraits<scoped_refptr<const blink::SecurityOrigin>>::IsDeletedValue(value.site_as_origin_);
    }

    static void ConstructDeletedValue(blink::BlinkSchemefulSite& slot, bool zero_value)
    {
        HashTraits<scoped_refptr<const blink::SecurityOrigin>>::ConstructDeletedValue(slot.site_as_origin_, zero_value);
    }
};
