...
TEST(NetworkUtilsTest, IsReservedIPAddress)
{
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (i == 0 || i == 10 || i == 127 || i == 192 || i > 223) {
            EXPECT_TRUE(network_utils::IsReservedIPAddress(String::FromUTF8(address_string.data(), address_string.length())));
        } else {
            EXPECT_FALSE(network_utils::IsReservedIPAddress(String::FromUTF8(address_string.data(), address_string.length())));
        }
    }
}
