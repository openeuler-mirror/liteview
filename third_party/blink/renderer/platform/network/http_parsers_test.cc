...
TEST(HTTPParsersTest, ParseCacheControl)
{
    ...
    header = ParseCacheControlDirectives("stale-while-revalidate=2,stale-while-revalidate=3", AtomicString());
    EXPECT_TRUE(header.parsed);
    EXPECT_FALSE(header.contains_no_cache);
    EXPECT_FALSE(header.contains_no_store);
    EXPECT_FALSE(header.contains_must_revalidate);
...
TEST(HTTPParsersTest, ParseMultipartHeadersResult)
{
    ...
    for (size_t i = 0; i < std::size(tests); ++i) {
        ...
        wtf_size_t end = 0;
        bool result = ParseMultipartHeadersFromBody(tests[i].data, static_cast<wtf_size_t>(strlen(tests[i].data)), &response, &end);
...
TEST(HTTPParsersTest, ParseMultipartHeaders)
{
    ...
    const char kData[] = "content-type: image/png\n"
                         ...
                         "\n";
    wtf_size_t end = 0;
    bool result = ParseMultipartHeadersFromBody(kData, static_cast<wtf_size_t>(strlen(kData)), &response, &end);
