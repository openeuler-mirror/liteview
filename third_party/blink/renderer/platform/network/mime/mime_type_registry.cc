...
bool MIMETypeRegistry::IsSupportedImagePrefixedMIMEType(const String& mime_type)
{
    ...
    return (blink::IsSupportedImageMimeType(ascii_mime_type)
        || (base::StartsWith(ascii_mime_type, "image/", base::CompareCase::SENSITIVE) && blink::IsSupportedNonImageMimeType(ascii_mime_type)));
}
...
{
    if (EqualIgnoringASCIICase(mime_type, "text/xml") || EqualIgnoringASCIICase(mime_type, "application/xml") || EqualIgnoringASCIICase(mime_type, "text/xsl"))
        return true;
