...
FormDataElement::FormDataElement(
    const String& filename, int64_t file_start, int64_t file_length, const absl::optional<base::Time>& expected_file_modification_time)
    : type_(kEncodedFile)
    , filename_(filename)
...
FormDataElement::FormDataElement(const String& blob_uuid, scoped_refptr<BlobDataHandle> optional_handle)
    : type_(kEncodedBlob)
...
scoped_refptr<EncodedFormData> EncodedFormData::DeepCopy() const
{
    ...
    for (const FormDataElement& e : elements_) {
        ...
        case FormDataElement::kEncodedBlob:
            form_data->elements_.UncheckedAppend(FormDataElement(e.blob_uuid_, e.optional_blob_data_handle_));
            break;
...
void EncodedFormData::AppendFile(const String& filename, const absl::optional<base::Time>& expected_modification_time)
{
    elements_.push_back(FormDataElement(filename, 0, BlobData::kToEndOfFile, expected_modification_time));
}

void EncodedFormData::AppendFileRange(const String& filename, int64_t start, int64_t length, const absl::optional<base::Time>& expected_modification_time)
{
    elements_.push_back(FormDataElement(filename, start, length, expected_modification_time));
}

void EncodedFormData::AppendBlob(const String& uuid, scoped_refptr<BlobDataHandle> optional_handle)
{
