...
TEST_F(EncodedFormDataTest, DeepCopy)
{
    ...
    original->AppendFileRange("example.txt", 12345, 56789, base::Time::FromDoubleT(9999.0));
    ...
    original->AppendBlob("originalUUID", BlobDataHandle::Create("uuid", "" /* type */, 0u /* size */, std::move(remote)));
    ...
    EXPECT_EQ(9999.0, copy_elements[1].expected_file_modification_time_->ToDoubleT());
