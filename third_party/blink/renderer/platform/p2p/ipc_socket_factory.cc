...
public:
    ...
    bool Init(network::P2PSocketType type, std::unique_ptr<P2PSocketClientImpl> client, const rtc::SocketAddress& local_address, uint16_t min_port,
        uint16_t max_port, const rtc::SocketAddress& remote_address);
    ...
    void OnDataReceived(const net::IPEndPoint& address, const Vector<int8_t>& data, const base::TimeTicks& timestamp) override;
...
private:
    ...
    void InitAcceptedTcp(std::unique_ptr<blink::P2PSocketClient> client, const rtc::SocketAddress& local_address, const rtc::SocketAddress& remote_address);
...
IpcPacketSocket::~IpcPacketSocket()
{
    ...
    if (total_packets_ > 0) {
        UMA_HISTOGRAM_PERCENTAGE("WebRTC.ApplicationPercentPacketsDiscarded", (packets_discarded_ * 100) / total_packets_);
    }
}
...
bool IpcPacketSocket::Init(network::P2PSocketType type, std::unique_ptr<P2PSocketClientImpl> client, const rtc::SocketAddress& local_address, uint16_t min_port,
    uint16_t max_port, const rtc::SocketAddress& remote_address)
{
...
void IpcPacketSocket::InitAcceptedTcp(
    std::unique_ptr<blink::P2PSocketClient> client, const rtc::SocketAddress& local_address, const rtc::SocketAddress& remote_address)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK_EQ(state_, kIsUninitialized);
...
int IpcPacketSocket::SendTo(const void* data, size_t data_size, const rtc::SocketAddress& address, const rtc::PacketOptions& options)
{
    ...
    data_vector.Append(reinterpret_cast<const int8_t*>(data), base::checked_cast<wtf_size_t>(data_size));
...
void IpcPacketSocket::OnDataReceived(const net::IPEndPoint& address, const Vector<int8_t>& data, const base::TimeTicks& timestamp)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    SignalReadPacket(this, reinterpret_cast<const char*>(&data[0]), data.size(), address_lj, timestamp.since_origin().InMicroseconds());
}

AsyncAddressResolverImpl::AsyncAddressResolverImpl(P2PSocketDispatcher* dispatcher)
...
void AsyncAddressResolverImpl::Start(const rtc::SocketAddress& addr)
{
    ...
    resolver_->Start(addr, /*address_family=*/absl::nullopt, WTF::BindOnce(&AsyncAddressResolverImpl::OnAddressResolved, weak_factory_.GetWeakPtr()));
}

void AsyncAddressResolverImpl::Start(const rtc::SocketAddress& addr, int address_family)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    // Port and hostname must be copied to the resolved address returned from
    // GetResolvedAddress.
    ...
    resolver_->Start(addr, absl::make_optional(address_family), WTF::BindOnce(&AsyncAddressResolverImpl::OnAddressResolved, weak_factory_.GetWeakPtr()));
}

bool AsyncAddressResolverImpl::GetResolvedAddress(int family, rtc::SocketAddress* addr) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void AsyncAddressResolverImpl::OnAddressResolved(const Vector<net::IPAddress>& addresses)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
IpcPacketSocketFactory::IpcPacketSocketFactory(P2PSocketDispatcher* socket_dispatcher, const net::NetworkTrafficAnnotationTag& traffic_annotation)
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateUdpSocket(const rtc::SocketAddress& local_address, uint16_t min_port, uint16_t max_port)
{
    ...
    auto socket_client = std::make_unique<P2PSocketClientImpl>(socket_dispatcher, traffic_annotation_);
    std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
    if (!socket->Init(network::P2P_SOCKET_UDP, std::move(socket_client), local_address, min_port, max_port, rtc::SocketAddress())) {
        return nullptr;
    }
...
rtc::AsyncPacketSocket* IpcPacketSocketFactory::CreateClientTcpSocket(const rtc::SocketAddress& local_address, const rtc::SocketAddress& remote_address,
    const rtc::ProxyInfo& proxy_info, const std::string& user_agent, const rtc::PacketSocketTcpOptions& opts)
{
    ...
    auto socket_client = std::make_unique<P2PSocketClientImpl>(socket_dispatcher, traffic_annotation_);
    std::unique_ptr<IpcPacketSocket> socket(new IpcPacketSocket());
    if (!socket->Init(type, std::move(socket_client), local_address, 0, 0, remote_address))
        return nullptr;
    return socket.release();
}
