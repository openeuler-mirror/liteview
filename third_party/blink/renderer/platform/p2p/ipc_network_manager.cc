...
void IpcNetworkManager::StartUpdating()
{
    ...
    if (network_list_received_) {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, WTF::BindOnce(&IpcNetworkManager::SendNetworksChangedSignal, weak_factory_.GetWeakPtr()));
    } else {
    ...
    const net::NetworkInterfaceList& list, const net::IPAddress& default_ipv4_local_address, const net::IPAddress& default_ipv6_local_address)
{
    ...
    for (auto it = list.begin(); it != list.end(); it++) {
        ...
        auto network = std::make_unique<rtc::Network>(it->name, it->name, prefix, it->prefix_length, adapter_type);
    ...
    if (Platform::Current()->AllowsLoopbackInPeerConnection()) {
        ...
        auto network_v4 = std::make_unique<rtc::Network>(name_v4, name_v4, ip_address_v4, 32, rtc::ADAPTER_TYPE_UNKNOWN);
        ...
        if (GetDefaultLocalAddress(AF_INET6, &ipv6_default_address)) {
            ...
            auto network_v6 = std::make_unique<rtc::Network>(name_v6, name_v6, ip_address_v6, 64, rtc::ADAPTER_TYPE_UNKNOWN);
