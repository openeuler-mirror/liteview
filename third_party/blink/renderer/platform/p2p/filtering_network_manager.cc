...
void FilteringNetworkManager::CheckPermission()
{
    ...
    media_permission_->HasPermission(media::MediaPermission::AUDIO_CAPTURE, WTF::BindOnce(&FilteringNetworkManager::OnPermissionStatus, GetWeakPtr()));
    media_permission_->HasPermission(media::MediaPermission::VIDEO_CAPTURE, WTF::BindOnce(&FilteringNetworkManager::OnPermissionStatus, GetWeakPtr()));
}
...
void FilteringNetworkManager::ReportMetrics(bool report_start_latency)
{
    if (report_start_latency) {
        blink::ReportTimeToUpdateNetworkList(base::TimeTicks::Now() - start_updating_time_);
    }
...
void FilteringNetworkManager::FireEventIfStarted()
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, WTF::BindOnce(&FilteringNetworkManager::SendNetworksChangedSignal, GetWeakPtr()));
