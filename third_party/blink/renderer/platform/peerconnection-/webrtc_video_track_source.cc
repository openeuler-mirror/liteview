...
WebRtcVideoTrackSource::WebRtcVideoTrackSource(bool is_screencast, absl::optional<bool> needs_denoising, media::VideoCaptureFeedbackCB feedback_callback,
    base::RepeatingClosure request_refresh_frame_callback, media::GpuVideoAcceleratorFactories* gpu_factories)
    : AdaptedVideoTrackSource(/*required_alignment=*/1)
    , adapter_resources_(new WebRtcVideoFrameAdapter::SharedResources(gpu_factories))
...
void WebRtcVideoTrackSource::OnFrameCaptured(scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    const int64_t translated_camera_time_us = timestamp_aligner_.TranslateTimestamp(frame->timestamp().InMicroseconds(), now_us);
    ...
    if (video_frame->natural_size() == video_frame->visible_rect().size()) {
        DeliverFrame(std::move(video_frame), std::move(scaled_frames), base::OptionalToPtr(accumulated_update_rect_), translated_camera_time_us);
        return;
    }
...
void WebRtcVideoTrackSource::DeliverFrame(
    scoped_refptr<media::VideoFrame> frame, std::vector<scoped_refptr<media::VideoFrame>> scaled_frames, gfx::Rect* update_rect, int64_t timestamp_us)
{
    if (update_rect) {
        DVLOG(3) << "update_rect = "
    ...
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_adapter(
        new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(frame, std::move(scaled_frames), adapter_resources_));

    webrtc::VideoFrame::Builder frame_builder
        = webrtc::VideoFrame::Builder().set_video_frame_buffer(frame_adapter).set_rotation(GetFrameRotation(frame.get())).set_timestamp_us(timestamp_us);
    if (update_rect) {
