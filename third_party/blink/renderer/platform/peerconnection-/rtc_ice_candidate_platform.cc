...
RTCIceCandidatePlatform::RTCIceCandidatePlatform(String candidate, String sdp_mid, absl::optional<uint16_t> sdp_m_line_index, String username_fragment)
...
RTCIceCandidatePlatform::RTCIceCandidatePlatform(String candidate, String sdp_mid, absl::optional<uint16_t> sdp_m_line_index)
    : candidate_(std::move(candidate))
    , sdp_mid_(std::move(sdp_mid))
    , sdp_m_line_index_(std::move(sdp_m_line_index))
{
...
void RTCIceCandidatePlatform::PopulateFields(bool use_username_from_candidate)
{
    ...
    if (!c.related_address().IsNil()) {
        related_address_ = String::FromUTF8(c.related_address().HostAsURIString().data());
