...
absl::optional<webrtc::SdpVideoFormat> VdcToWebRtcFormat(const media::VideoDecoderConfig& config)
{
    switch (config.codec()) {
    ...
    case media::VideoCodec::kH264: {
        ...
        const absl::optional<webrtc::H264Level> h264_level = webrtc::H264SupportedLevel(width * height, kDefaultFps);
...
void MapBaselineProfile(std::vector<webrtc::SdpVideoFormat>* supported_formats)
{
    for (const auto& format : *supported_formats) {
        ...
        if (profile_level_id && profile_level_id->profile == webrtc::H264Profile::kProfileBaseline) {
            ...
            cbp_format.parameters[cricket::kH264FmtpProfileLevelId] = *webrtc::H264ProfileLevelIdToString(cbp_profile);
...
RTCVideoDecoderFactory::RTCVideoDecoderFactory(media::GpuVideoAcceleratorFactories* gpu_factories, base::WeakPtr<media::DecoderFactory> decoder_factory,
    scoped_refptr<base::SequencedTaskRunner> media_task_runner, const gfx::ColorSpace& render_color_space)
...
webrtc::VideoDecoderFactory::CodecSupport RTCVideoDecoderFactory::QueryCodecSupport(const webrtc::SdpVideoFormat& format, bool reference_scaling) const
{
    ...
    if (reference_scaling) {
        ...
        // MFT support it.
        if (codec == media::VideoCodec::kVP9 && !RTCVideoDecoderAdapter::Vp9HwSupportForSpatialLayers()) {
            return { false, false };
        }
    }
...
std::unique_ptr<webrtc::VideoDecoder> RTCVideoDecoderFactory::CreateVideoDecoder(const webrtc::SdpVideoFormat& format)
{
    ...
    if (base::FeatureList::IsEnabled(media::kUseDecoderStreamForWebRTC)) {
        decoder = RTCVideoDecoderStreamAdapter::Create(gpu_factories_, decoder_factory_, media_task_runner_, render_color_space_, format);
    } else {
    ...
    return decoder ? std::make_unique<ScopedVideoDecoder>(media_task_runner_, std::move(decoder)) : nullptr;
}
