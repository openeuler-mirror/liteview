...
class RTCVideoDecoderStreamAdapter::InternalDemuxerStream : public media::DemuxerStream {
public:
    ...
    void set_keyframe_trimming(bool trim)
    {
        trim_ = trim;
    }
...
private:
    ...
    void MaybeSatisfyPendingRead()
    {
        ...
        if (buffers_.front()->new_config) {
            ...
            std::move(pending_read_).Run(DemuxerStream::Status::kConfigChanged, nullptr);
            return;
        }
        ...
        std::move(pending_read_).Run(DemuxerStream::Status::kOk, std::move(pending_buffer->buffer));
    }
...
// static
std::unique_ptr<RTCVideoDecoderStreamAdapter> RTCVideoDecoderStreamAdapter::Create(media::GpuVideoAcceleratorFactories* gpu_factories,
    base::WeakPtr<media::DecoderFactory> decoder_factory, scoped_refptr<base::SequencedTaskRunner> media_task_runner, const gfx::ColorSpace& render_color_space,
    const webrtc::SdpVideoFormat& format)
{
    ...
    if (!gpu_factories && !base::FeatureList::IsEnabled(media::kExposeSwDecodersToWebRTC)) {
    ...
    if (gpu_factories && gpu_factories->IsDecoderConfigSupported(config) == media::GpuVideoAcceleratorFactories::Supported::kFalse) {
    ...
    auto rtc_video_decoder_adapter
        = base::WrapUnique(new RTCVideoDecoderStreamAdapter(gpu_factories, decoder_factory, std::move(media_task_runner), render_color_space, config, format));
    return rtc_video_decoder_adapter;
}

RTCVideoDecoderStreamAdapter::RTCVideoDecoderStreamAdapter(media::GpuVideoAcceleratorFactories* gpu_factories,
    base::WeakPtr<media::DecoderFactory> decoder_factory, scoped_refptr<base::SequencedTaskRunner> media_task_runner, const gfx::ColorSpace& render_color_space,
    const media::VideoDecoderConfig& config, const webrtc::SdpVideoFormat& format)
...
{
    DVLOG(3) << __func__;
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::InitializeOrReinitializeSync");
    ...
    PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::RestartDecoderStreamOnMedia, weak_this_));
}
...
void RTCVideoDecoderStreamAdapter::AttemptLogInitializationState_Locked()
{
    ...
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::AttemptLogInitializationState_Locked");
    ...
    base::UmaHistogramBoolean("Media.RTCVideoDecoderInitDecodeSuccess", !has_error_);
    if (!has_error_) {
        UMA_HISTOGRAM_ENUMERATION("Media.RTCVideoDecoderProfile", WebRtcVideoFormatToMediaVideoCodecProfile(format_), media::VIDEO_CODEC_PROFILE_MAX + 1);
    }
}

int32_t RTCVideoDecoderStreamAdapter::Decode(const webrtc::EncodedImage& input_image, bool missing_frames, int64_t render_time_ms)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    ...
    const bool has_software_fallback = video_codec_type_ != webrtc::kVideoCodecH264;
#else
    ...
    // decoder count already, then just keep going.
    {
        ...
        if (has_software_fallback && contributes_to_decoder_count_ && current_resolution_.GetArea() < kMinResolution.GetArea()
        ...
        if (video_codec_type_ == webrtc::kVideoCodecVP9 && input_image.SpatialIndex().value_or(0) > 0 && !RTCVideoDecoderAdapter::Vp9HwSupportForSpatialLayers()
            && decoder_configured_ && decoder_info_.is_hardware_accelerated) {
            bool need_fallback_to_software = true;
#if BUILDFLAG(IS_WIN)
            if (video_decoder_type_ == media::VideoDecoderType::kD3D11 && base::FeatureList::IsEnabled(media::kD3D11Vp9kSVCHWDecoding)) {
            ...
            if (need_fallback_to_software) {
                ...
                RecordRTCVideoDecoderFallbackReason(config_.codec(), RTCVideoDecoderFallbackReason::kSpatialLayers);
    ...
    if (spatial_layer_frame_size.size() > 1) {
        const uint8_t* side_data = reinterpret_cast<const uint8_t*>(spatial_layer_frame_size.data());
        size_t side_data_size = spatial_layer_frame_size.size() * sizeof(uint32_t) / sizeof(uint8_t);
        pending_buffer->buffer = media::DecoderBuffer::CopyFrom(input_image.data(), input_image.size(), side_data, side_data_size);
    } else {
        pending_buffer->buffer = media::DecoderBuffer::CopyFrom(input_image.data(), input_image.size());
    }
    pending_buffer->buffer->set_timestamp(base::Microseconds(input_image.Timestamp()));
    pending_buffer->buffer->set_is_key_frame(input_image._frameType == webrtc::VideoFrameType::kVideoFrameKey);
    ...
    if (ShouldReinitializeForSettingHDRColorSpace(input_image)) {
        ...
        pending_buffer->new_config->set_color_space_info(blink::WebRtcToMediaVideoColorSpace(*input_image.ColorSpace()));
    }
    ...
    // Queue for decoding.
    {
        ...
        if (has_error_) {
            ...
            RecordRTCVideoDecoderFallbackReason(config_.codec(), RTCVideoDecoderFallbackReason::kPreviousErrorOnDecode);
        ...
        if (IsDecodingPaused_Locked()) {
            ...
            if (has_software_fallback && pending_buffer_count_ > kMaxFramesWhilePausedBeforeFallback) {
                RecordRTCVideoDecoderFallbackReason(config_.codec(), RTCVideoDecoderFallbackReason::kConsecutivePendingBufferOverflowDuringInit);
                return WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE;
            }
            if (!pending_buffer->buffer->is_key_frame() && buffers_since_last_keyframe_ > kMaxKeyFrameIntervalWhilePaused) {
            ...
            if (has_software_fallback && pending_buffer_count_ >= kAbsoluteMaxPendingBuffers) {
                has_error_ = true;
                PostCrossThreadTask(
                    *media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::ShutdownOnMediaThread, weak_this_));
                DLOG(ERROR) << __func__ << " too many errors / pending buffers.";
                RecordRTCVideoDecoderFallbackReason(config_.codec(), RTCVideoDecoderFallbackReason::kConsecutivePendingBufferOverflow);
            ...
            PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::ResetOnMediaThread, weak_this_));
    ...
    // it simpler.
    PostCrossThreadTask(
        *media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::DecodeOnMediaThread, weak_this_, std::move(pending_buffer)));

    return WEBRTC_VIDEO_CODEC_OK;
}

int32_t RTCVideoDecoderStreamAdapter::RegisterDecodeCompleteCallback(webrtc::DecodedImageCallback* callback)
{
    DVLOG(2) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    DCHECK(callback);
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::RegisterDecodeCompleteCallback");
...
int32_t RTCVideoDecoderStreamAdapter::Release()
{
    ...
    PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::ShutdownOnMediaThread, weak_this_));

    return has_error_ ? WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE : WEBRTC_VIDEO_CODEC_OK;
}

webrtc::VideoDecoder::DecoderInfo RTCVideoDecoderStreamAdapter::GetDecoderInfo() const
{
    base::AutoLock auto_lock(lock_);
    return decoder_info_;
}

void RTCVideoDecoderStreamAdapter::InitializeOnMediaThread(const media::VideoDecoderConfig& config, InitCB init_cb)
{
    DVLOG(3) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::InitializeOnMediaThread");
    ...
    auto traits = std::make_unique<media::DecoderStreamTraits<media::DemuxerStream::VIDEO>>(media_log_.get());
    {
        base::AutoLock auto_lock(lock_);
    ...
    auto create_decoders_cb = base::BindRepeating(
        [](scoped_refptr<base::SequencedTaskRunner> task_runner, base::WeakPtr<media::DecoderFactory> decoder_factory,
            media::GpuVideoAcceleratorFactories* gpu_factories, const gfx::ColorSpace render_color_space, media::MediaLog* media_log,
            ...
            if (decoder_factory_ptr) {
                decoder_factory_ptr->CreateVideoDecoders(
                    std::move(task_runner), gpu_factories, media_log, request_overlay_cb, render_color_space, &video_decoders);
            }
            return video_decoders;
        },
        media_task_runner_, decoder_factory_, base::Unretained(gpu_factories_), render_color_space_, media_log_.get(), std::move(request_overlay_cb));

    decoder_stream_ = std::make_unique<media::VideoDecoderStream>(std::move(traits), media_task_runner_, std::move(create_decoders_cb), media_log_.get());
    decoder_stream_->set_decoder_change_observer(base::BindRepeating(&RTCVideoDecoderStreamAdapter::OnDecoderChanged, weak_this_));
    decoder_stream_->Initialize(demuxer_stream_.get(), ConvertToBaseOnceCallback(std::move(init_cb)), cdm_context, base::DoNothing() /* statistics_cb */,
        base::DoNothing() /* waiting_cb */);
}

void RTCVideoDecoderStreamAdapter::OnInitializeDone(base::TimeTicks start_time, bool success)
{
    TRACE_EVENT1("webrtc", "RTCVideoDecoderStreamAdapter::OnInitializeDone", "success", success);
...
void RTCVideoDecoderStreamAdapter::DecodeOnMediaThread(std::unique_ptr<PendingBuffer> pending_buffer)
{
    DVLOG(4) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
...
void RTCVideoDecoderStreamAdapter::OnFrameReady(media::VideoDecoderStream::ReadResult result)
{
    DVLOG(3) << __func__;
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    TRACE_EVENT1("webrtc", "RTCVideoDecoderStreamAdapter::OnFrameReady", "success", result.has_value());
    ...
    default:
        ...
        base::UmaHistogramSparse("Media.RTCVideoDecoderStream.Error.OnFrameReady", static_cast<int>(result.code()));
        {
            base::AutoLock auto_lock(lock_);
            has_error_ = true;
            pending_buffer_count_ = 0;
    ...
    webrtc::VideoFrame rtc_frame
        = webrtc::VideoFrame::Builder()
              .set_video_frame_buffer(rtc::scoped_refptr<webrtc::VideoFrameBuffer>(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(std::move(frame))))
...
void RTCVideoDecoderStreamAdapter::AttemptRead()
{
    ...
    // before it returns.
    decoder_stream_->Read(base::BindOnce(&RTCVideoDecoderStreamAdapter::OnFrameReady, weak_this_));
}

bool RTCVideoDecoderStreamAdapter::ShouldReinitializeForSettingHDRColorSpace(const webrtc::EncodedImage& input_image) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    TRACE_EVENT0("webrtc",
    ...
    if (config_.profile() == media::VP9PROFILE_PROFILE2 && input_image.ColorSpace()) {
        const media::VideoColorSpace& new_color_space = blink::WebRtcToMediaVideoColorSpace(*input_image.ColorSpace());
        if (!config_.color_space_info().IsSpecified() || new_color_space != config_.color_space_info()) {
            return true;
        }
    }
...
void RTCVideoDecoderStreamAdapter::ResetOnMediaThread()
{
    ...
    decoder_stream_->Reset(base::BindOnce(&RTCVideoDecoderStreamAdapter::OnResetCompleteOnMediaThread, weak_this_));
}
...
void RTCVideoDecoderStreamAdapter::OnDecoderChanged(media::VideoDecoder* decoder)
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    base::AutoLock auto_lock(lock_);
    TRACE_EVENT1("webrtc", "RTCVideoDecoderStreamAdapter::OnDecoderChanged", "decoder", (decoder ? static_cast<int>(decoder->GetDecoderType()) : -1));
    ...
    if (decoder->IsPlatformDecoder()) {
        std::string implementation_name_suffix = " (" + media::GetDecoderName(decoder->GetDecoderType()) + ")";
        decoder_info_.implementation_name = kExternalDecoderName + implementation_name_suffix;
        return;
    }
    ...
    case media::VideoDecoderType::kFFmpeg:
        ...
        break;
    default:
        decoder_info_.implementation_name = media::GetDecoderName(decoder->GetDecoderType()) + " (DecoderStream)";
    }
}
...
void RTCVideoDecoderStreamAdapter::RecordMaxInFlightDecodesLockedOnMedia()
{
    ...
    if (!max_buffer_count_metric_) {
        max_buffer_count_metric_ = base::SingleSampleMetricsFactory::Get()->CreateCustomCountsMetric(
            "Media.RTCVideoDecoderMaxInFlightDecodes", 0, kAbsoluteMaxPendingBuffers + 1, 100);
    }

    // It's unclear if the factory can fail, so simply don't record if it does.
    if (max_buffer_count_metric_) {
        max_buffer_count_metric_->SetSample(static_cast<int>(pending_buffer_count_));
    }
...
{
    DCHECK(media_task_runner_->RunsTasksInCurrentSequence());
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::RestartDecoderStreamOnMedia");
    ...
    auto init_cb = CrossThreadBindOnce(&RTCVideoDecoderStreamAdapter::OnInitializeDone, weak_this_, start_time);
...
int32_t RTCVideoDecoderStreamAdapter::FallBackToSoftware_Locked()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    lock_.AssertAcquired();
    TRACE_EVENT0("webrtc", "RTCVideoDecoderStreamAdapter::FallBackToSoftware_Locked");
