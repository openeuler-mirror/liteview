...
public:
    RTCStatsAllowlist()
    {
        ...
        allowlisted_stats_types_.insert(webrtc::RTCRemoteInboundRtpStreamStats::kType);
        allowlisted_stats_types_.insert(webrtc::RTCRemoteOutboundRtpStreamStats::kType);
    ...
    {
        return allowlisted_stats_types_.find(stats.type()) != allowlisted_stats_types_.end();
    }
...
std::vector<const webrtc::RTCStatsMemberInterface*> FilterMembers(
    std::vector<const webrtc::RTCStatsMemberInterface*> stats_members, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
{
    if (base::FeatureList::IsEnabled(blink::features::kWebRtcExposeNonStandardStats)) {
    ...
    base::EraseIf(stats_members, [&exposed_group_ids](const webrtc::RTCStatsMemberInterface* member) {
        if (member->is_standardized()) {
            return false;
        }

        const std::vector<webrtc::NonStandardGroupId>& ids = member->group_ids();
...
size_t CountAllowlistedStats(const scoped_refptr<const webrtc::RTCStatsReport>& stats_report)
{
    size_t size = 0;
...
template <typename T> Vector<T> ToWTFVector(const std::vector<T>& vector)
{
...
RTCStatsReportPlatform::RTCStatsReportPlatform(
    const scoped_refptr<const webrtc::RTCStatsReport>& stats_report, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
...
{
    return std::make_unique<RTCStatsReportPlatform>(stats_report_, exposed_group_ids_);
}

std::unique_ptr<RTCStats> RTCStatsReportPlatform::GetStats(const String& id) const
{
...
RTCStats::RTCStats(
    const scoped_refptr<const webrtc::RTCStatsReport>& stats_owner, const webrtc::RTCStats* stats, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
...
RTCStatsMember::RTCStatsMember(const scoped_refptr<const webrtc::RTCStatsReport>& stats_owner, const webrtc::RTCStatsMemberInterface* member)
    : stats_owner_(stats_owner)
    , member_(member)
{
    DCHECK(stats_owner_);
...
{
    DCHECK(IsDefined());
    return String::FromUTF8(*member_->cast_to<webrtc::RTCStatsMember<std::string>>());
}
...
{
    DCHECK(IsDefined());
    const std::vector<int32_t> vector = *member_->cast_to<webrtc::RTCStatsMember<std::vector<int32_t>>>();
    return ToWTFVector(vector);
}
...
{
    DCHECK(IsDefined());
    const std::vector<uint32_t> vector = *member_->cast_to<webrtc::RTCStatsMember<std::vector<uint32_t>>>();
    return ToWTFVector(vector);
}
...
{
    DCHECK(IsDefined());
    const std::vector<int64_t> vector = *member_->cast_to<webrtc::RTCStatsMember<std::vector<int64_t>>>();
    return ToWTFVector(vector);
}
...
{
    DCHECK(IsDefined());
    const std::vector<uint64_t> vector = *member_->cast_to<webrtc::RTCStatsMember<std::vector<uint64_t>>>();
    return ToWTFVector(vector);
}
...
{
    DCHECK(IsDefined());
    const std::vector<double> vector = *member_->cast_to<webrtc::RTCStatsMember<std::vector<double>>>();
    return ToWTFVector(vector);
}
...
{
    DCHECK(IsDefined());
    const std::map<std::string, uint64_t>& map = *member_->cast_to<webrtc::RTCStatsMember<std::map<std::string, uint64_t>>>();
...
{
    DCHECK(IsDefined());
    const std::map<std::string, double>& map = *member_->cast_to<webrtc::RTCStatsMember<std::map<std::string, double>>>();
...
rtc::scoped_refptr<webrtc::RTCStatsCollectorCallback> CreateRTCStatsCollectorCallback(
    scoped_refptr<base::SingleThreadTaskRunner> main_thread, RTCStatsReportCallback callback, const Vector<webrtc::NonStandardGroupId>& exposed_group_ids)
{
    return rtc::scoped_refptr<RTCStatsCollectorCallbackImpl>(
        new rtc::RefCountedObject<RTCStatsCollectorCallbackImpl>(std::move(main_thread), std::move(callback), exposed_group_ids));
}
...
void RTCStatsCollectorCallbackImpl::OnStatsDeliveredOnMainThread(rtc::scoped_refptr<const webrtc::RTCStatsReport> report)
{
    ...
    std::move(callback_).Run(std::make_unique<RTCStatsReportPlatform>(base::WrapRefCounted(report.get()), exposed_group_ids_));
}
