...
public:
    using TransformerCallback = base::RepeatingCallback<void(std::unique_ptr<webrtc::TransformableVideoFrameInterface>)>;
    explicit RTCEncodedVideoStreamTransformer(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner);
...
private:
    ...
    Vector<std::pair<uint32_t, rtc::scoped_refptr<webrtc::TransformedFrameCallback>>> send_frame_to_sink_callbacks_;
