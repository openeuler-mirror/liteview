...
public:
    ...
    MOCK_METHOD(std::unique_ptr<webrtc::VideoEncoder>, CreateVideoEncoder, (const webrtc::SdpVideoFormat& format), (override));
...
public:
    ...
    void VerifyEncodingInfo(const absl::optional<webrtc::SdpAudioFormat> sdp_audio_format, const absl::optional<webrtc::SdpVideoFormat> sdp_video_format,
        const absl::optional<String> video_scalability_mode, const CodecSupport support)
    {
        if (sdp_video_format) {
            const absl::optional<std::string> expected_scalability_mode
                = video_scalability_mode ? absl::make_optional(video_scalability_mode->Utf8()) : absl::nullopt;

            ON_CALL(*mock_video_encoder_factory_, QueryCodecSupport)
                .WillByDefault(testing::Invoke(
                    [sdp_video_format, expected_scalability_mode, support](const webrtc::SdpVideoFormat& format, absl::optional<std::string> scalability_mode) {
                        EXPECT_TRUE(format.IsSameCodec(*sdp_video_format));
                        EXPECT_EQ(scalability_mode, expected_scalability_mode);
                        return support;
                    }));
            EXPECT_CALL(*mock_video_encoder_factory_, QueryCodecSupport).Times(::testing::AtMost(1));
        }
        WebrtcEncodingInfoHandler encoding_info_handler(std::move(video_encoder_factory_), audio_encoder_factory_);
...
{
    VerifyEncodingInfo(kAudioFormatOpus, /*sdp_video_format=*/absl::nullopt,
        /*video_scalability_mode=*/absl::nullopt, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/true });
}

TEST_F(WebrtcEncodingInfoHandlerTests, UnsupportedAudio)
{
    VerifyEncodingInfo(kAudioFormatFoo, /*sdp_video_format=*/absl::nullopt,
        /*video_scalability_mode=*/absl::nullopt, CodecSupport { /*is_supported=*/false, /*is_power_efficient=*/false });
}
...
TEST_F(WebrtcEncodingInfoHandlerTests, BasicVideo)
{
    VerifyEncodingInfo(
        ...
        /*video_scalability_mode=*/absl::nullopt, CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}
...
{
    VerifyEncodingInfo(
        /*sdp_audio_format=*/absl::nullopt, kVideoFormatVp9, "L1T3", CodecSupport { /*is_supported=*/true, /*is_power_efficient=*/false });
}
