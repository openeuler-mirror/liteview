...
public:
    ...
    {
        task_environment_.FastForwardBy(metronome_source_->EnsureNextTickAndGetDelayForTesting());
    }
...
public:
    ...
    void PostDelayedTask(absl::AnyInvocable<void() &&> task, webrtc::TimeDelta delay) override
    {
        NOTREACHED();
    }

    void PostDelayedHighPrecisionTask(absl::AnyInvocable<void() &&> task, webrtc::TimeDelta delay) override
    {
        NOTREACHED();
    }
...
public:
    explicit FakeTickListener(base::RepeatingCallback<void()> cb, FakeTaskQueue* queue)
        : cb_(std::move(cb))
        , queue_(queue)
    {
    }
    ...
    webrtc::TaskQueueBase* OnTickTaskQueue() override
    {
        return queue_;
    }
...
public:
    explicit DestructorSetter(bool* flag)
        : flag_(flag)
    {
    }
    ~DestructorSetter()
...
TEST_F(MetronomeSourceTest, InactiveListenerDoesNotCauseScheduling)
{
    ...
    scoped_refptr<MetronomeSource::ListenerHandle> listener_handle
        = metronome_source_->AddListener(task_runner, base::BindRepeating([](bool* was_called) { *was_called = true; }, base::Unretained(&was_called)),
...
TEST_F(MetronomeSourceTest, ListenerCalledOnItsTaskQueue)
{
    ...
    bool was_called = false;
    scoped_refptr<MetronomeSource::ListenerHandle> listener_handle = metronome_source_->AddListener(task_runner,
        base::BindRepeating(
            [](scoped_refptr<base::SequencedTaskRunner> task_runner, bool* was_called) {
...
TEST_F(MetronomeSourceTest, ListenerCalledOnEachTick)
{
    ...
    scoped_refptr<MetronomeSource::ListenerHandle> listener_handle
        = metronome_source_->AddListener(task_runner, base::BindRepeating([](int* callback_count) { ++(*callback_count); }, base::Unretained(&callback_count)));

    // Fast-forward slightly less than a tick should not increment the counter.
    task_environment_.FastForwardBy(MetronomeSource::Tick() - base::Milliseconds(1));
...
TEST_F(MetronomeSourceTest, RemovingListenerDestroysCallback)
{
    ...
    DestructorSetter* destructor_setter = new DestructorSetter(&callback_destroyed);

    // Add a listener where the binding takes ownership of DestructorSetter.
    scoped_refptr<MetronomeSource::ListenerHandle> listener_handle = metronome_source_->AddListener(task_runner,
        base::BindRepeating([](DestructorSetter*) {},
...
TEST_F(MetronomeSourceTest, ListenerWithInitialDelay)
{
    ...
    base::TimeTicks wakeup_time = base::TimeTicks::Now() + MetronomeSource::Tick() * 2;

    bool was_called = false;
    scoped_refptr<MetronomeSource::ListenerHandle> listener_handle = metronome_source_->AddListener(
        task_runner, base::BindRepeating([](bool* was_called) { *was_called = true; }, base::Unretained(&was_called)), wakeup_time);
...
TEST_F(MetronomeSourceTest, SetWakeupTime)
{
    ...
    // Wakeup next tick.
    listener_handle->SetWakeupTime(base::TimeTicks::Now() + MetronomeSource::Tick());
    task_environment_.FastForwardBy(MetronomeSource::Tick());
    EXPECT_EQ(callback_count, 1);
    ...
    listener_handle->SetWakeupTime(base::TimeTicks::Now() + MetronomeSource::Tick() * 3);
    ...
    listener_handle->SetWakeupTime(base::TimeTicks::Now() + MetronomeSource::Tick() - base::Milliseconds(1));
    task_environment_.FastForwardBy(MetronomeSource::Tick());
    EXPECT_EQ(callback_count, 3);
...
{
    EXPECT_EQ(MetronomeSource::Tick().InMicroseconds(), metronome_source_->CreateWebRtcMetronome()->TickPeriod().us());
}
...
TEST_F(MetronomeSourceTest, WebRtcMetronomeAdapterAddsHandler)
{
    ...
    auto tick_listener
        = FakeTickListener(base::BindRepeating([](int* callback_count) { ++(*callback_count); }, base::Unretained(&callback_count)), &fake_queue);
    auto metronome_adapter = metronome_source_->CreateWebRtcMetronome();

    EXPECT_FALSE(metronome_source_->HasListenersForTesting());
    fake_queue.PostOnRunner(FROM_HERE, base::BindLambdaForTesting([&] { metronome_adapter->AddListener(&tick_listener); }));
    ...
    fake_queue.PostOnRunner(FROM_HERE, base::BindLambdaForTesting([&] { metronome_adapter->RemoveListener(&tick_listener); }));
    task_environment_.FastForwardBy(MetronomeSource::Tick());
    EXPECT_EQ(11, callback_count);
...
TEST_F(MetronomeSourceTest, WebRtcMetronomeAdapterDoesNotExecuteListenerAfterRemoval)
{
    int callback_count = 0;
...
TEST(MetronomeSourcePhaseTest, MultipleMetronomesAreAligned)
{
    base::test::TaskEnvironment task_environment(
        base::test::TaskEnvironment::ThreadingMode::MULTIPLE_THREADS, base::test::TaskEnvironment::TimeSource::MOCK_TIME);
    scoped_refptr<base::SequencedTaskRunner> task_runner = base::ThreadPool::CreateSequencedTaskRunner({});

    // Create and align with first metronome.
    scoped_refptr<MetronomeSource> metronome_a = base::MakeRefCounted<MetronomeSource>();
    task_environment.FastForwardBy(metronome_a->EnsureNextTickAndGetDelayForTesting());
    ...
    scoped_refptr<MetronomeSource> metronome_b = base::MakeRefCounted<MetronomeSource>();
    base::TimeDelta next_tick_a = metronome_a->EnsureNextTickAndGetDelayForTesting();
    base::TimeDelta next_tick_b = metronome_b->EnsureNextTickAndGetDelayForTesting();
    ...
    scoped_refptr<MetronomeSource::ListenerHandle> handle_a = metronome_a->AddListener(
        task_runner, base::BindRepeating([](bool* metronome_a_ticked) { *metronome_a_ticked = true; }, base::Unretained(&metronome_a_ticked)));
    bool metronome_b_ticked = false;
    scoped_refptr<MetronomeSource::ListenerHandle> handle_b = metronome_b->AddListener(
        task_runner, base::BindRepeating([](bool* metronome_b_ticked) { *metronome_b_ticked = true; }, base::Unretained(&metronome_b_ticked)));

    // Get close to the next tick without hitting it.
    task_environment.FastForwardBy(MetronomeSource::Tick() - base::Milliseconds(1));
