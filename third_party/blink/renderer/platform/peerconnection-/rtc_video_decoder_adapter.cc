...
scoped_refptr<media::DecoderBuffer> ConvertToDecoderBuffer(const webrtc::EncodedImage& input_image)
{
    ...
    for (int i = 0; i <= max_sl_index; i++) {
        const absl::optional<size_t>& frame_size = input_image.SpatialLayerFrameSize(i);
        if (!frame_size)
            continue;
    ...
    if (spatial_layer_frame_size.size() > 1) {
        ...
        buffer = media::DecoderBuffer::CopyFrom(input_image.data(), input_image.size(), side_data, side_data_size);
    } else {
        buffer = media::DecoderBuffer::CopyFrom(input_image.data(), input_image.size());
    }
...
void RTCVideoDecoderAdapter::InitializeOnMediaThread(
    const media::VideoDecoderConfig& config, CrossThreadOnceFunction<void(bool)> init_cb, base::TimeTicks start_time, std::string* decoder_name)
{
    DVLOG(3) << __func__;
    ...
    if (!video_decoder_) {
        ...
        if (!video_decoder_) {
            PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(std::move(init_cb), false));
            return;
        }
    }

    media::VideoDecoder::OutputCB output_cb = ConvertToBaseRepeatingCallback(CrossThreadBindRepeating(&RTCVideoDecoderAdapter::OnOutput, weak_this_));
    video_decoder_->Initialize(config, /*low_delay=*/false,
        /*cdm_context=*/nullptr,
        base::BindOnce(
            [](base::OnceCallback<void(bool)> cb, std::string* decoder_name, media::VideoDecoder* video_decoder, media::DecoderStatus status) {
                *decoder_name = media::GetDecoderName(video_decoder->GetDecoderType());
                std::move(cb).Run(status.is_ok());
            },
            ConvertToBaseOnceCallback(std::move(init_cb)), CrossThreadUnretained(decoder_name), CrossThreadUnretained(video_decoder_.get())),
        output_cb, base::DoNothing());
}

absl::optional<RTCVideoDecoderFallbackReason> RTCVideoDecoderAdapter::FallbackOrRegisterConcurrentInstanceOnce(media::VideoCodec codec)
{
    ...
    if (HasSoftwareFallback(codec) && current_resolution_ < kMinResolution && GetDecoderCounter()->Count() > kMaxDecoderInstances) {
...
absl::optional<RTCVideoDecoderFallbackReason> RTCVideoDecoderAdapter::NeedSoftwareFallback(
    const media::VideoCodec codec, const media::DecoderBuffer& buffer) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    ...
    if (codec == media::VideoCodec::kVP9 && is_spatial_layer_buffer && !Vp9HwSupportForSpatialLayers()) {
...
#if BUILDFLAG(IS_WIN)
        if (video_decoder_->GetDecoderType() == media::VideoDecoderType::kD3D11 && base::FeatureList::IsEnabled(media::kD3D11Vp9kSVCHWDecoding)) {
            return absl::nullopt;
        }
#endif
...
absl::variant<RTCVideoDecoderAdapter::DecodeResult, RTCVideoDecoderFallbackReason> RTCVideoDecoderAdapter::EnqueueBuffer(
    scoped_refptr<media::DecoderBuffer> buffer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
    if (auto fallback_reason = NeedSoftwareFallback(config_.codec(), *buffer))
        return *fallback_reason;
    if (auto fallback_reason = FallbackOrRegisterConcurrentInstanceOnce(config_.codec())) {
        return *fallback_reason;
    }
    ...
    if (HasSoftwareFallback(config_.codec()) && pending_buffers_.size() >= kMaxPendingBuffers) {
...
void RTCVideoDecoderAdapter::DecodeOnMediaThread()
{
    ...
    while (outstanding_decode_requests_ < max_decode_requests) {
        ...
        video_decoder_->Decode(std::move(buffer), WTF::BindRepeating(&RTCVideoDecoderAdapter::OnDecodeDone, weak_this_));
    }
}

void RTCVideoDecoderAdapter::FlushOnMediaThread(WTF::CrossThreadOnceClosure flush_success_cb, WTF::CrossThreadOnceClosure flush_fail_cb)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(media_sequence_checker_);
...
void RTCVideoDecoderAdapter::OnOutput(scoped_refptr<media::VideoFrame> frame)
{
    ...
    webrtc::VideoFrame rtc_frame
        = webrtc::VideoFrame::Builder()
              .set_video_frame_buffer(rtc::scoped_refptr<WebRtcVideoFrameAdapter>(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(std::move(frame))))
    ...
    current_resolution_ = static_cast<int32_t>(rtc_frame.width()) * rtc_frame.height();
...
// static
std::unique_ptr<RTCVideoDecoderAdapter> RTCVideoDecoderAdapter::Create(media::GpuVideoAcceleratorFactories* gpu_factories, const webrtc::SdpVideoFormat& format)
{
    ...
    if (gpu_factories->IsDecoderConfigSupported(config) != media::GpuVideoAcceleratorFactories::Supported::kFalse) {
        ...
        rtc_video_decoder_adapter = base::WrapUnique(new RTCVideoDecoderAdapter(gpu_factories, config));
        ...
        gpu_factories->GetTaskRunner()->DeleteSoon(FROM_HERE, std::move(rtc_video_decoder_adapter));
    }
...
RTCVideoDecoderAdapter::RTCVideoDecoderAdapter(media::GpuVideoAcceleratorFactories* gpu_factories, const media::VideoDecoderConfig& config)
...
bool RTCVideoDecoderAdapter::InitializeSync(const media::VideoDecoderConfig& config)
{
    ...
    if (PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCVideoDecoderAdapter::InitializeOnMediaThread, CrossThreadUnretained(this), config, std::move(init_cb), start_time,
...
bool RTCVideoDecoderAdapter::Configure(const Settings& settings)
{
    ...
    current_resolution_ = static_cast<int32_t>(settings.max_render_resolution().Width()) * settings.max_render_resolution().Height();
...
int32_t RTCVideoDecoderAdapter::Decode(const webrtc::EncodedImage& input_image, bool missing_frames, int64_t render_time_ms)
{
    ...
    if (const auto* fallback_reason = absl::get_if<RTCVideoDecoderFallbackReason>(&result)) {
        RecordRTCVideoDecoderFallbackReason(config_.codec(), *fallback_reason);
        return WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE;
    }

    return *absl::get_if<DecodeResult>(&result) == DecodeResult::kOk ? WEBRTC_VIDEO_CODEC_OK : WEBRTC_VIDEO_CODEC_ERROR;
}

absl::variant<RTCVideoDecoderAdapter::DecodeResult, RTCVideoDecoderFallbackReason> RTCVideoDecoderAdapter::DecodeInternal(
    const webrtc::EncodedImage& input_image, bool missing_frames, int64_t render_time_ms)
{
    ...
    if (ShouldReinitializeForSettingHDRColorSpace(input_image)) {
        config_.set_color_space_info(blink::WebRtcToMediaVideoColorSpace(*input_image.ColorSpace()));
    ...
    if (ret && *ret == DecodeResult::kOk) {
        PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE, CrossThreadBindOnce(&RTCVideoDecoderAdapter::DecodeOnMediaThread, weak_this_));
    }
...
int32_t RTCVideoDecoderAdapter::Release()
{
    ...
    pending_buffers_.clear();
    decode_timestamps_.clear();
    return status_ == Status::kError ? WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE : WEBRTC_VIDEO_CODEC_OK;
}

bool RTCVideoDecoderAdapter::ShouldReinitializeForSettingHDRColorSpace(const webrtc::EncodedImage& input_image) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(decoding_sequence_checker_);
...
bool RTCVideoDecoderAdapter::ReinitializeSync(const media::VideoDecoderConfig& config)
{
    ...
    auto init_cb = CrossThreadBindOnce(&FinishWait, CrossThreadUnretained(&waiter), CrossThreadUnretained(&result));
    std::string decoder_name;
    WTF::CrossThreadOnceClosure flush_success_cb = CrossThreadBindOnce(&RTCVideoDecoderAdapter::InitializeOnMediaThread, weak_this_, config, std::move(init_cb),
    ...
    if (PostCrossThreadTask(*media_task_runner_.get(), FROM_HERE,
            CrossThreadBindOnce(&RTCVideoDecoderAdapter::FlushOnMediaThread, weak_this_, std::move(flush_success_cb), std::move(flush_fail_cb)))) {
        waiter.Wait();
