...
TEST(WebRtcAudioSinkTest, CaptureTimestamp)
{
...

    {
        ...
        EXPECT_CALL(sink_1, OnData(_, _, kSampleRateHz, kInputChannels, kOutputFramesPerBuffer, absl::make_optional<int64_t>(kStartRtcTimestampMs)));
        EXPECT_CALL(sink_2, OnData(_, _, kSampleRateHz, kInputChannels, kOutputFramesPerBuffer, absl::make_optional<int64_t>(kStartRtcTimestampMs)));
        ...
        constexpr int64_t kExpectedTimestampMs = kStartRtcTimestampMs + kCaptureIntervalMs - kEnqueueFrames * 1000 / kSampleRateHz;
        EXPECT_CALL(sink_1, OnData(_, _, kSampleRateHz, kInputChannels, kOutputFramesPerBuffer, absl::make_optional<int64_t>(kExpectedTimestampMs)));
        EXPECT_CALL(sink_2, OnData(_, _, kSampleRateHz, kInputChannels, kOutputFramesPerBuffer, absl::make_optional<int64_t>(kExpectedTimestampMs)));
