...
public:
    ...
    MOCK_METHOD3(CreateAudioDecoders,
        void(scoped_refptr<base::SequencedTaskRunner> task_runner, media::MediaLog* media_log,
            std::vector<std::unique_ptr<media::AudioDecoder>>* audio_decoders));

    void CreateVideoDecoders(scoped_refptr<base::SequencedTaskRunner> task_runner, media::GpuVideoAcceleratorFactories* gpu_factories,
        media::MediaLog* media_log, media::RequestOverlayInfoCB request_overlay_info_cb, const gfx::ColorSpace& target_color_space,
        std::vector<std::unique_ptr<media::VideoDecoder>>* video_decoders) override
    {
        std::move(std::begin(decoders_), std::end(decoders_), std::back_inserter(*video_decoders));
        decoders_.clear();
    }
    ...
    bool has_pending_decoders() const
    {
    ...
    {
        decoders_.push_back(std::make_unique<MockVideoDecoder>(is_platform_decoder));
    }
...
public:
    explicit DecodedImageCallback(base::RepeatingCallback<void(const webrtc::VideoFrame&)> callback)
        : callback_(callback)
    {
    }
...
class RTCVideoDecoderStreamAdapterTest : public ::testing::TestWithParam<TestParams> {
public:
    RTCVideoDecoderStreamAdapterTest()
        : task_environment_(base::test::TaskEnvironment::ThreadPoolExecutionMode::QUEUED)
        , media_thread_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({}))
        , use_hw_decoders_(GetParam().use_hw_decoders == TestParams::UseHwDecoders::kYes)
...
#endif
        if (GetParam().use_chrome_sw_decoders == TestParams::UseChromeSwDecoders::kYes) {
    ...
    RTCVideoDecoderStreamAdapterTest(const RTCVideoDecoderStreamAdapterTest&) = delete;
    RTCVideoDecoderStreamAdapterTest& operator=(const RTCVideoDecoderStreamAdapterTest&) = delete;
...
protected:
    ...
    bool CreateDecoderStream()
    {
        adapter_ = RTCVideoDecoderStreamAdapter::Create(
            use_hw_decoders_ ? &gpu_factories_ : nullptr, decoder_factory_->GetWeakPtr(), media_thread_task_runner_, gfx::ColorSpace {}, sdp_format_);
        return !!adapter_;
    }
    ...
    int32_t Decode(uint32_t timestamp, bool missing_frames = false, bool is_keyframe = true)
    {
        ...
        input_image._frameType = is_keyframe ? webrtc::VideoFrameType::kVideoFrameKey : webrtc::VideoFrameType::kVideoFrameDelta;
    ...
    void FinishDecode(uint32_t timestamp)
    {
        media_thread_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&RTCVideoDecoderStreamAdapterTest::FinishDecodeOnMediaThread, base::Unretained(this), timestamp));
    }
    ...
    void FinishDecodeOnMediaThread(uint32_t timestamp)
    {
        ...
        scoped_refptr<media::VideoFrame> frame = media::VideoFrame::WrapNativeTextures(media::PIXEL_FORMAT_ARGB, mailbox_holders,
    ...
    void SetGpuFactorySupport(bool supported)
    {
        ...
        EXPECT_CALL(gpu_factories_, IsDecoderConfigSupported(_))
            .Times(AtLeast(0))
            .WillRepeatedly(Return(supported ? media::GpuVideoAcceleratorFactories::Supported::kTrue : media::GpuVideoAcceleratorFactories::Supported::kFalse));
    }

    bool GetUseHwDecoders() const
    {
        return use_hw_decoders_;
    }
    ...
    StrictMock<media::MockGpuVideoAcceleratorFactories> gpu_factories_ { nullptr /* SharedImageInterface* */ };
...
{
    auto adapter = RTCVideoDecoderStreamAdapter::Create(use_hw_decoders_ ? &gpu_factories_ : nullptr, decoder_factory_->GetWeakPtr(), media_thread_task_runner_,
        gfx::ColorSpace {}, webrtc::SdpVideoFormat(webrtc::CodecTypeToPayloadString(webrtc::kVideoCodecGeneric)));
...
TEST_P(RTCVideoDecoderStreamAdapterTest, FailInitIfNoHwNorSwDecoders)
{
    ...
    const bool allow_sw = GetParam().use_chrome_sw_decoders == TestParams::UseChromeSwDecoders::kYes;
    if (!allow_sw) {
...
TEST_P(RTCVideoDecoderStreamAdapterTest, DecodeOneFrame)
{
    auto* decoder = decoder_factory_->decoder();
    EXPECT_TRUE(BasicSetup());
    EXPECT_CALL(*decoder, Decode_(_, _)).WillOnce(base::test::RunOnceCallback<1>(media::DecoderStatus::Codes::kOk));
...
TEST_P(RTCVideoDecoderStreamAdapterTest, SlowDecodingCausesReset)
{
    ...
    EXPECT_CALL(*decoder, Decode_(_, _)).WillRepeatedly(base::test::RunOnceCallback<1>(media::DecoderStatus::Codes::kOk));
...
TEST_P(RTCVideoDecoderStreamAdapterTest, LowResSelectsCorrectDecoder)
{
    ...
    if (allow_sw) {
        ...
        EXPECT_CALL(*decoder_factory_->decoder(), Initialize_(_, _, _, _, _, _)).Times(0);
        // The sw one is the correct one
        correct_decoder = decoder_factory_->last_decoder();
    } else {
        // The software decoder is the wrong one.
        EXPECT_CALL(*decoder_factory_->last_decoder(), Initialize_(_, _, _, _, _, _)).Times(0);
    }
    ...
    EXPECT_CALL(*correct_decoder, Decode_(_, _)).WillOnce(base::test::RunOnceCallback<1>(media::DecoderStatus::Codes::kOk));
    Decode(0);
    task_environment_.RunUntilIdle();
}
...
TEST_P(RTCVideoDecoderStreamAdapterTest, FallbackToSoftwareWhenDecodeVP9kSVCStream)
{
    auto* decoder = decoder_factory_->decoder();
    EXPECT_TRUE(decoder->IsPlatformDecoder());
    SetSpatialIndex(2);
...
INSTANTIATE_TEST_SUITE_P(UseHwDecoding, RTCVideoDecoderStreamAdapterTest,
    ::testing::Values(TestParams { TestParams::UseHwDecoders::kNo, TestParams::UseChromeSwDecoders::kYes },
        TestParams { TestParams::UseHwDecoders::kYes, TestParams::UseChromeSwDecoders::kNo },
        TestParams { TestParams::UseHwDecoders::kYes, TestParams::UseChromeSwDecoders::kYes }));
