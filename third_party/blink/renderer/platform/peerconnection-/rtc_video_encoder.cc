...
public:
    EncodedDataWrapper(uint8_t* data, size_t size, base::OnceClosure reuse_buffer_callback)
        : data_(data)
        , size_(size)
...
static_assert(static_cast<int>(RTCVideoEncoderShutdownReason::kMaxValue) == media::VideoEncodeAccelerator::kErrorMax + 1,
    "RTCVideoEncoderShutdownReason should follow "
    "VideoEncodeAccelerator::Error (+1 for the success case)");

webrtc::VideoEncoder::EncoderInfo CopyToWebrtcEncoderInfo(const media::VideoEncoderInfo& enc_info)
{
    ...
    for (size_t i = 0; i < std::size(enc_info.fps_allocation); ++i) {
        ...
        info.fps_allocation[i]
            = absl::InlinedVector<uint8_t, webrtc::kMaxTemporalStreams>(enc_info.fps_allocation[i].begin(), enc_info.fps_allocation[i].end());
    }
    for (const auto& limit : enc_info.resolution_bitrate_limits) {
        info.resolution_bitrate_limits.emplace_back(limit.frame_size.GetArea(), limit.min_start_bitrate_bps, limit.min_bitrate_bps, limit.max_bitrate_bps);
    }
    return info;
}

media::VideoEncodeAccelerator::Config::InterLayerPredMode CopyFromWebRtcInterLayerPredMode(const webrtc::InterLayerPredMode inter_layer_pred)
{
    ...
    case webrtc::InterLayerPredMode::kOnKeyPic:
        return media::VideoEncodeAccelerator::Config::InterLayerPredMode::kOnKeyPic;
    }
}
...
{
    absl::optional<webrtc::ScalabilityMode> scalability_mode = codec_settings.GetScalabilityMode();

    if (codec_settings.codecType == webrtc::kVideoCodecVP9 && codec_settings.VP9().numberOfSpatialLayers > 1
    ...
    case webrtc::kVideoCodecVP8: {
        ...
        if (number_of_temporal_layers > 1) {
            ...
            sl.num_of_temporal_layers = base::saturated_cast<uint8_t>(number_of_temporal_layers);
        }
        break;
    }
...
class PendingFrame {
public:
    PendingFrame(const base::TimeDelta& media_timestamp, int32_t rtp_timestamp, int64_t capture_time_ms, const std::vector<gfx::Size>& resolutions)
...
webrtc::VideoCodecType ProfileToWebRtcVideoCodecType(media::VideoCodecProfile profile)
{
    ...
    } else if (profile >= media::H264PROFILE_MIN && profile <= media::H264PROFILE_MAX) {
...
void RecordEncoderShutdownReasonUMA(RTCVideoEncoderShutdownReason reason, webrtc::VideoCodecType type)
{
    switch (type) {
    case webrtc::VideoCodecType::kVideoCodecH264:
        base::UmaHistogramEnumeration("Media.RTCVideoEncoderShutdownReason.H264", reason);
        break;
    case webrtc::VideoCodecType::kVideoCodecVP8:
        base::UmaHistogramEnumeration("Media.RTCVideoEncoderShutdownReason.VP8", reason);
        break;
    case webrtc::VideoCodecType::kVideoCodecVP9:
        base::UmaHistogramEnumeration("Media.RTCVideoEncoderShutdownReason.VP9", reason);
        break;
    case webrtc::VideoCodecType::kVideoCodecAV1:
        base::UmaHistogramEnumeration("Media.RTCVideoEncoderShutdownReason.AV1", reason);
        break;
    default:
        base::UmaHistogramEnumeration("Media.RTCVideoEncoderShutdownReason.Other", reason);
    }
}
} // namespace
...
class RTCVideoEncoder::Impl : public media::VideoEncodeAccelerator::Client, public base::RefCountedThreadSafe<RTCVideoEncoder::Impl> {
public:
    Impl(media::GpuVideoAcceleratorFactories* gpu_factories, webrtc::VideoCodecType video_codec_type, webrtc::VideoContentType video_content_type);
    ...
    void CreateAndInitializeVEA(const gfx::Size& input_visible_size, uint32_t bitrate, media::VideoCodecProfile profile, bool is_constrained_h264,
        const std::vector<media::VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers,
        media::VideoEncodeAccelerator::Config::InterLayerPredMode inter_layer_pred, SignaledValue init_event);
    ...
    void Enqueue(const webrtc::VideoFrame* input_frame, bool force_keyframe, SignaledValue encode_event);
    ...
    void RegisterEncodeCompleteCallback(SignaledValue scoped_event, webrtc::EncodedImageCallback* callback);
...
private:
    ...
    void LogAndNotifyError(const base::Location& location, const String& str, media::VideoEncodeAccelerator::Error error);
    ...
    Vector<std::pair<base::UnsafeSharedMemoryRegion, base::WritableSharedMemoryMapping>> output_buffers_;
...
RTCVideoEncoder::Impl::Impl(
    media::GpuVideoAcceleratorFactories* gpu_factories, webrtc::VideoCodecType video_codec_type, webrtc::VideoContentType video_content_type)
...
void RTCVideoEncoder::Impl::CreateAndInitializeVEA(const gfx::Size& input_visible_size, uint32_t bitrate, media::VideoCodecProfile profile,
    bool is_constrained_h264, const std::vector<media::VideoEncodeAccelerator::Config::SpatialLayer>& spatial_layers,
    media::VideoEncodeAccelerator::Config::InterLayerPredMode inter_layer_pred, SignaledValue init_event)
{
    DVLOG(3) << __func__;
    ...
    if (!ConvertKbpsToBps(bitrate, &bitrate_bps)) {
        LogAndNotifyError(FROM_HERE, "Overflow converting bitrate from kbps to bps", media::VideoEncodeAccelerator::kInvalidArgumentError);
    ...
    if (base::FeatureList::IsEnabled(features::kWebRtcUseMinMaxVEADimensions)) {
        ...
        for (const auto& vea_profile : vea_supported_profiles) {
            if (vea_profile.profile == profile
                ...
                LogAndNotifyError(FROM_HERE,
                    base::StringPrintf("Requested dimensions (%s) beyond accelerator limits (%s - %s)", input_visible_size.ToString().c_str(),
                        vea_profile.min_resolution.ToString().c_str(), vea_profile.max_resolution.ToString().c_str())
                        .c_str(),
    ...
    if (!video_encoder_) {
        LogAndNotifyError(FROM_HERE, "Error creating VideoEncodeAccelerator", media::VideoEncodeAccelerator::kPlatformFailureError);
        return;
    }
    ...
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableVideoCaptureUseGpuMemoryBuffer)
    ...
    const media::VideoEncodeAccelerator::Config config(pixel_format, input_visible_size_, profile, media::Bitrate::ConstantBitrate(bitrate_bps), absl::nullopt,
        absl::nullopt, absl::nullopt, is_constrained_h264, storage_type,
        video_content_type_ == webrtc::VideoContentType::SCREENSHARE ? media::VideoEncodeAccelerator::Config::ContentType::kDisplay
                                                                     : media::VideoEncodeAccelerator::Config::ContentType::kCamera,
        spatial_layers, inter_layer_pred);
    if (!video_encoder_->Initialize(config, this, std::make_unique<media::NullMediaLog>())) {
        LogAndNotifyError(FROM_HERE, "Error initializing video_encoder", media::VideoEncodeAccelerator::kInvalidArgumentError);
        return;
    }
    ...
    highest_active_spatial_index_ = spatial_layers.empty() ? 0u : spatial_layers.size() - 1;

    // RequireBitstreamBuffers or NotifyError will be called and the waiter will
    // be signaled.
}

webrtc::VideoEncoder::EncoderInfo RTCVideoEncoder::Impl::GetEncoderInfo() const
{
    base::AutoLock lock(lock_);
...
void RTCVideoEncoder::Impl::Enqueue(const webrtc::VideoFrame* input_frame, bool force_keyframe, SignaledValue encode_event)
{
    DVLOG(3) << __func__;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!use_native_input_ && input_buffers_free_.empty() && output_buffers_free_count_ == 0) {
...
void RTCVideoEncoder::Impl::UseOutputBitstreamBufferId(int32_t bitstream_buffer_id)
{
...
void RTCVideoEncoder::Impl::RequestEncodingParametersChange(const webrtc::VideoEncoder::RateControlParameters& parameters)
{
    ...
    for (size_t spatial_id = 0; spatial_id < media::VideoBitrateAllocation::kMaxSpatialLayers; ++spatial_id) {
        ...
        if (spatial_layer_active && spatial_id < current_spatial_layer_resolutions_.size())
...
void RTCVideoEncoder::Impl::Destroy(SignaledValue event)
{
    ...
    if (video_encoder_) {
        ...
        status_ = WEBRTC_VIDEO_CODEC_UNINITIALIZED;
        RecordEncoderShutdownReasonUMA(RTCVideoEncoderShutdownReason::kSuccessfulRelease, video_codec_type_);
    }
...
std::vector<gfx::Size> RTCVideoEncoder::Impl::ActiveSpatialResolutions() const
{
    if (current_spatial_layer_resolutions_.empty())
        return {};
    DCHECK_LT(highest_active_spatial_index_, current_spatial_layer_resolutions_.size());
    return std::vector<gfx::Size>(current_spatial_layer_resolutions_.begin(), current_spatial_layer_resolutions_.begin() + highest_active_spatial_index_ + 1);
}
...
void RTCVideoEncoder::Impl::RequireBitstreamBuffers(unsigned int input_count, const gfx::Size& input_coded_size, size_t output_buffer_size)
{
    ...
    for (int i = 0; i < kOutputBufferCount; ++i) {
        ...
        if (!mapping.IsValid()) {
            LogAndNotifyError(FROM_HERE, "failed to create output buffer", media::VideoEncodeAccelerator::kPlatformFailureError);
            return;
        }
        output_buffers_.push_back(std::make_pair(std::move(region), std::move(mapping)));
    }

    // Immediately provide all output buffers to the VEA.
    for (wtf_size_t i = 0; i < output_buffers_.size(); ++i) {
        video_encoder_->UseOutputBitstreamBuffer(media::BitstreamBuffer(i, output_buffers_[i].first.Duplicate(), output_buffers_[i].first.GetSize()));
        output_buffers_free_count_++;
    }
...
void RTCVideoEncoder::Impl::BitstreamBufferReady(int32_t bitstream_buffer_id, const media::BitstreamBufferMetadata& metadata)
{
    DVLOG(3) << __func__ << " bitstream_buffer_id=" << bitstream_buffer_id << ", payload_size=" << metadata.payload_size_bytes
             << ", key_frame=" << metadata.key_frame << ", timestamp ms=" << metadata.timestamp.InMilliseconds();
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    if (bitstream_buffer_id < 0 || bitstream_buffer_id >= static_cast<int>(output_buffers_.size())) {
        LogAndNotifyError(FROM_HERE, "invalid bitstream_buffer_id", media::VideoEncodeAccelerator::kPlatformFailureError);
        return;
    }
    void* output_mapping_memory = output_buffers_[bitstream_buffer_id].second.memory();
    if (metadata.payload_size_bytes > output_buffers_[bitstream_buffer_id].second.size()) {
        LogAndNotifyError(FROM_HERE, "invalid payload_size", media::VideoEncodeAccelerator::kPlatformFailureError);
        return;
    }
    output_buffers_free_count_--;
    ...
    if (!failed_timestamp_match_) {
        ...
        while (!pending_frames_.empty()) {
            ...
            if (front_frame.media_timestamp_ == metadata.timestamp) {
                ...
                const size_t num_resolutions = std::max(front_frame.resolutions_.size(), size_t { 1 });
                ++front_frame.produced_frames_;

                if (front_frame.produced_frames_ == num_resolutions && !end_of_picture) {
                    // The top layer must always have the end-of-picture indicator.
                    LogAndNotifyError(FROM_HERE, "missing end-of-picture", media::VideoEncodeAccelerator::kPlatformFailureError);
                    return;
                }
                if (end_of_picture) {
                    // Remove pending timestamp at the top spatial layer in the case of
                    // SVC encoding.
                    if (!front_frame.resolutions_.empty() && front_frame.produced_frames_ != front_frame.resolutions_.size()) {
                        // At least one resolution was not produced.
                        LogAndNotifyError(FROM_HERE, "missing resolution", media::VideoEncodeAccelerator::kPlatformFailureError);
                        return;
                    }
    ...
    image.SetEncodedData(rtc::make_ref_counted<EncodedDataWrapper>(static_cast<uint8_t*>(output_mapping_memory), metadata.payload_size_bytes,
        media::BindToCurrentLoop(base::BindOnce(&RTCVideoEncoder::Impl::UseOutputBitstreamBufferId, WrapRefCounted(this), bitstream_buffer_id))));
    ...
    case webrtc::kVideoCodecVP9: {
        ...
        if (metadata.vp9) {
            ...
            if (!metadata.vp9->spatial_layer_resolutions.empty() && expected_resolutions != metadata.vp9->spatial_layer_resolutions) {
                LogAndNotifyError(FROM_HERE, "Encoded SVC resolution set does not match request.", media::VideoEncodeAccelerator::kPlatformFailureError);
                return;
            }

            const uint8_t spatial_index = metadata.vp9->spatial_idx;
            if (spatial_index >= current_spatial_layer_resolutions_.size()) {
                LogAndNotifyError(FROM_HERE, "invalid spatial index", media::VideoEncodeAccelerator::kPlatformFailureError);
                return;
            }
            image.SetSpatialIndex(spatial_index);
            image._encodedWidth = current_spatial_layer_resolutions_[spatial_index].width();
            image._encodedHeight = current_spatial_layer_resolutions_[spatial_index].height();
...
void RTCVideoEncoder::Impl::NotifyError(media::VideoEncodeAccelerator::Error error)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    case media::VideoEncodeAccelerator::kInvalidArgumentError:
        ...
        RecordEncoderShutdownReasonUMA(RTCVideoEncoderShutdownReason::kInvalidArgument, video_codec_type_);
        break;
    case media::VideoEncodeAccelerator::kIllegalStateError:
        RecordEncoderShutdownReasonUMA(RTCVideoEncoderShutdownReason::kIllegalState, video_codec_type_);
    ...
    case media::VideoEncodeAccelerator::kPlatformFailureError:
        ...
        retval = video_codec_type_ != webrtc::kVideoCodecH264 || webrtc::H264Encoder::IsSupported() ? WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE
                                                                                                    : WEBRTC_VIDEO_CODEC_ERROR;
        RecordEncoderShutdownReasonUMA(RTCVideoEncoderShutdownReason::kPlatformFailure, video_codec_type_);
    }
    video_encoder_.reset();
...
void RTCVideoEncoder::Impl::LogAndNotifyError(const base::Location& location, const String& str, media::VideoEncodeAccelerator::Error error)
{
    static const char* const kErrorNames[] = { "kIllegalStateError", "kInvalidArgumentError", "kPlatformFailureError" };
    static_assert(std::size(kErrorNames) == media::VideoEncodeAccelerator::kErrorMax + 1, "Different number of errors and textual descriptions");
...
void RTCVideoEncoder::Impl::EncodeOneFrame()
{
    ...
    const base::TimeDelta timestamp = base::Microseconds(next_frame->timestamp_us());

    scoped_refptr<media::VideoFrame> frame;
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> buffer = next_frame->video_frame_buffer();

    // All non-native frames require a copy because we can't tell if non-copy
    // conditions are met.
    bool requires_copy_or_scale = buffer->type() != webrtc::VideoFrameBuffer::Type::kNative;
    if (!requires_copy_or_scale) {
        const WebRtcVideoFrameAdapter* frame_adapter = static_cast<WebRtcVideoFrameAdapter*>(buffer.get());
        ...
        const bool is_gmb_frame = storage == media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER;
        requires_copy_or_scale = RequiresSizeChange(*frame) || !(is_memory_based_frame || is_gmb_frame);
    }
    ...
    if (requires_copy_or_scale) {
        ...
        bool native_buffer_scaling =
            ...
            buffer->type() == webrtc::VideoFrameBuffer::Type::kNative && input_frame_coded_size_ == input_visible_size_;
#else
        ...
        if (native_buffer_scaling) {
            ...
            auto scaled_buffer = buffer->Scale(input_visible_size_.width(), input_visible_size_.height());
            auto mapped_buffer = scaled_buffer->GetMappedFrameBuffer(preferred_pixel_formats_);
            if (!mapped_buffer) {
                mapped_buffer = scaled_buffer->ToI420();
            }
            if (!mapped_buffer) {
                LogAndNotifyError(FROM_HERE, "Failed to map or convert buffer to I420", media::VideoEncodeAccelerator::kPlatformFailureError);
                NOTREACHED();
                return;
            }
            ...
            if (!frame) {
                LogAndNotifyError(
                    FROM_HERE, "Failed to convert WebRTC mapped buffer to media::VideoFrame", media::VideoEncodeAccelerator::kPlatformFailureError);
                NOTREACHED();
                return;
            }
        } else {
            ...
            if (!input_buffers_[index]) {
                ...
                if (!input_buffers_[index]) {
                    LogAndNotifyError(FROM_HERE, "Failed to create input buffer", media::VideoEncodeAccelerator::kPlatformFailureError);
                    return;
                }
            }
            ...
            if (!frame) {
                LogAndNotifyError(FROM_HERE, "failed to create frame", media::VideoEncodeAccelerator::kPlatformFailureError);
                async_encode_event_.SetAndReset(WEBRTC_VIDEO_CODEC_ERROR);
                return;
            }
            ...
            rtc::scoped_refptr<webrtc::I420BufferInterface> i420_buffer = next_frame->video_frame_buffer()->ToI420();
            if (libyuv::I420Scale(i420_buffer->DataY(), i420_buffer->StrideY(), i420_buffer->DataU(), i420_buffer->StrideU(), i420_buffer->DataV(),
                    i420_buffer->StrideV(), next_frame->width(), next_frame->height(), frame->GetWritableVisibleData(media::VideoFrame::kYPlane),
                    frame->stride(media::VideoFrame::kYPlane), frame->GetWritableVisibleData(media::VideoFrame::kUPlane),
                    frame->stride(media::VideoFrame::kUPlane), frame->GetWritableVisibleData(media::VideoFrame::kVPlane),
                    frame->stride(media::VideoFrame::kVPlane), frame->visible_rect().width(), frame->visible_rect().height(), libyuv::kFilterBox)) {
                LogAndNotifyError(FROM_HERE, "Failed to copy buffer", media::VideoEncodeAccelerator::kPlatformFailureError);
                async_encode_event_.SetAndReset(WEBRTC_VIDEO_CODEC_ERROR);
                return;
            }
            ...
            frame->AddDestructionObserver(
                media::BindToCurrentLoop(WTF::BindOnce(&RTCVideoEncoder::Impl::InputBufferReleased, scoped_refptr<RTCVideoEncoder::Impl>(this), index)));
        }
    }
    if (!failed_timestamp_match_) {
        DCHECK(!base::Contains(pending_frames_, timestamp, &PendingFrame::media_timestamp_));
        pending_frames_.emplace_back(timestamp, next_frame->timestamp(), next_frame->render_time_ms(), ActiveSpatialResolutions());
    }
...
{
    TRACE_EVENT0("webrtc", "RTCVideoEncoder::Impl::EncodeOneFrameWithNativeInput");
    ...
    scoped_refptr<media::VideoFrame> frame;
    if (next_frame->video_frame_buffer()->type() != webrtc::VideoFrameBuffer::Type::kNative) {
        ...
        frame = media::VideoFrame::WrapVideoFrame(
            black_gmb_frame_, black_gmb_frame_->format(), black_gmb_frame_->visible_rect(), black_gmb_frame_->natural_size());
    } else {
        frame = static_cast<WebRtcVideoFrameAdapter*>(next_frame->video_frame_buffer().get())->getMediaVideoFrame();
    }
    frame->set_timestamp(base::Microseconds(next_frame->timestamp_us()));

    if (frame->storage_type() != media::VideoFrame::STORAGE_GPU_MEMORY_BUFFER) {
        async_encode_event_.SetAndReset(WEBRTC_VIDEO_CODEC_ERROR);
        LogAndNotifyError(FROM_HERE, "frame isn't GpuMemoryBuffer based VideoFrame", media::VideoEncodeAccelerator::kPlatformFailureError);
        return;
    }

    if (!failed_timestamp_match_) {
        DCHECK(!base::Contains(pending_frames_, frame->timestamp(), &PendingFrame::media_timestamp_));
        pending_frames_.emplace_back(frame->timestamp(), next_frame->timestamp(), next_frame->render_time_ms(), ActiveSpatialResolutions());
    }
    video_encoder_->Encode(frame, next_frame_keyframe);
    async_encode_event_.SetAndReset(WEBRTC_VIDEO_CODEC_OK);
}

bool RTCVideoEncoder::Impl::CreateBlackGpuMemoryBufferFrame(const gfx::Size& natural_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    auto gmb = gpu_factories_->CreateGpuMemoryBuffer(natural_size, gfx::BufferFormat::YUV_420_BIPLANAR, gfx::BufferUsage::VEA_READ_CAMERA_AND_CPU_READ_WRITE);
    ...
    memset(static_cast<uint8_t*>(gmb->memory(0)), 0x0, gmb->stride(0) * gmb_size.height());
    memset(static_cast<uint8_t*>(gmb->memory(1)), 0x80, gmb->stride(1) * gmb_size.height() / 2);
    gmb->Unmap();
    ...
    black_gmb_frame_ = media::VideoFrame::WrapExternalGpuMemoryBuffer(
        gfx::Rect(gmb_size), natural_size, std::move(gmb), empty_mailboxes, base::NullCallback(), base::TimeDelta());
    return true;
}
...
void RTCVideoEncoder::Impl::RegisterEncodeCompleteCallback(SignaledValue event, webrtc::EncodedImageCallback* callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DVLOG(3) << __func__;
...
int32_t RTCVideoEncoder::InitEncode(const webrtc::VideoCodec* codec_settings, const webrtc::VideoEncoder::Settings& settings)
{
    ...
    if (profile_ >= media::H264PROFILE_MIN && profile_ <= media::H264PROFILE_MAX && (codec_settings->width % 2 != 0 || codec_settings->height % 2 != 0)) {
        DLOG(ERROR) << "Input video size is " << codec_settings->width << "x" << codec_settings->height << ", "
    ...
    impl_ = new Impl(gpu_factories_, ProfileToWebRtcVideoCodecType(profile_),
        (codec_settings->mode == webrtc::VideoCodecMode::kScreensharing) ? webrtc::VideoContentType::SCREENSHARE : webrtc::VideoContentType::UNSPECIFIED);

    std::vector<media::VideoEncodeAccelerator::Config::SpatialLayer> spatial_layers;
    auto inter_layer_pred = media::VideoEncodeAccelerator::Config::InterLayerPredMode::kOff;
    if (!CreateSpatialLayersConfig(*codec_settings, &spatial_layers, &inter_layer_pred)) {
        return WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE;
    }
    ...
    PostCrossThreadTask(*gpu_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&RTCVideoEncoder::Impl::CreateAndInitializeVEA, scoped_refptr<Impl>(impl_),
            gfx::Size(codec_settings->width, codec_settings->height), codec_settings->startBitrate, profile_, is_constrained_h264_, spatial_layers,
            inter_layer_pred, SignaledValue(&initialization_waiter, &initialization_retval)));
...
int32_t RTCVideoEncoder::Encode(const webrtc::VideoFrame& input_image, const std::vector<webrtc::VideoFrameType>* frame_types)
{
    ...
    base::ScopedAllowBaseSyncPrimitivesOutsideBlockingScope allow_wait;
    base::WaitableEvent encode_waiter(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    int32_t encode_retval = WEBRTC_VIDEO_CODEC_UNINITIALIZED;
    PostCrossThreadTask(*gpu_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&RTCVideoEncoder::Impl::Enqueue, scoped_refptr<Impl>(impl_), CrossThreadUnretained(&input_image), want_key_frame,
...
int32_t RTCVideoEncoder::RegisterEncodeCompleteCallback(webrtc::EncodedImageCallback* callback)
{
    ...
    base::ScopedAllowBaseSyncPrimitivesOutsideBlockingScope allow_wait;
    base::WaitableEvent register_waiter(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    int32_t register_retval = WEBRTC_VIDEO_CODEC_UNINITIALIZED;
    PostCrossThreadTask(*gpu_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&RTCVideoEncoder::Impl::RegisterEncodeCompleteCallback, scoped_refptr<Impl>(impl_),
            SignaledValue(&register_waiter, &register_retval), CrossThreadUnretained(callback)));
...
int32_t RTCVideoEncoder::Release()
{
    ...
    PostCrossThreadTask(*gpu_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&RTCVideoEncoder::Impl::Destroy, scoped_refptr<Impl>(impl_), SignaledValue(&release_waiter, nullptr /* val */)));
    release_waiter.Wait();
    impl_ = nullptr;
    return WEBRTC_VIDEO_CODEC_OK;
}
...
void RTCVideoEncoder::SetRates(const webrtc::VideoEncoder::RateControlParameters& parameters)
{
    ...
    PostCrossThreadTask(*gpu_task_runner_.get(), FROM_HERE,
        CrossThreadBindOnce(&RTCVideoEncoder::Impl::RequestEncodingParametersChange, scoped_refptr<Impl>(impl_), parameters));
    return;
}
