...
public:
    ...
    ~RTCVideoDecoderAdapterTest() override
    {
        ...
        media_thread_.task_runner()->DeleteSoon(FROM_HERE, std::move(rtc_video_decoder_adapter_));
...
protected:
    ...
    bool CreateAndInitialize(bool init_cb_result = true)
    {
        ...
        rtc_video_decoder_adapter_ = RTCVideoDecoderAdapter::Create(&gpu_factories_, sdp_format_);
    ...
    {
        return rtc_video_decoder_adapter_->RegisterDecodeCompleteCallback(&decoded_image_callback_);
    }
    ...
    int32_t Release()
    {
        return rtc_video_decoder_adapter_->Release();
    }
...
TEST_F(RTCVideoDecoderAdapterTest, Create_UnknownFormat)
{
    rtc_video_decoder_adapter_
        = RTCVideoDecoderAdapter::Create(&gpu_factories_, webrtc::SdpVideoFormat(webrtc::CodecTypeToPayloadString(webrtc::kVideoCodecGeneric)));
...
TEST_F(RTCVideoDecoderAdapterTest, Create_UnsupportedFormat)
{
    ...
    rtc_video_decoder_adapter_
        = RTCVideoDecoderAdapter::Create(&gpu_factories_, webrtc::SdpVideoFormat(webrtc::CodecTypeToPayloadString(webrtc::kVideoCodecVP9)));
    ASSERT_FALSE(rtc_video_decoder_adapter_);
}
...
TEST_F(RTCVideoDecoderAdapterTest, ReinitializesForHDRColorSpaceInitially)
{
    ...
    EXPECT_CALL(*video_decoder_, Decode_(_, _)).Times(3).WillRepeatedly(base::test::RunOnceCallback<1>(media::DecoderStatus::Codes::kOk));
    ...
    ASSERT_EQ(rtc_video_decoder_adapter_->Decode(first_input_image, false, 0), WEBRTC_VIDEO_CODEC_OK);
    media_thread_.FlushForTesting();
    ...
    ASSERT_EQ(rtc_video_decoder_adapter_->Decode(second_input_image, false, 0), WEBRTC_VIDEO_CODEC_OK);
    FinishDecode(1);
    media_thread_.FlushForTesting();
}
...
TEST_F(RTCVideoDecoderAdapterTest, HandlesReinitializeFailure)
{
    ...
    ASSERT_EQ(rtc_video_decoder_adapter_->Decode(input_image, false, 0), WEBRTC_VIDEO_CODEC_FALLBACK_SOFTWARE);
}
...
TEST_F(RTCVideoDecoderAdapterTest, FallsBackForLowResolution)
{
    ...
    EXPECT_EQ(RTCVideoDecoderAdapter::GetCurrentDecoderCountForTesting(), max_decoder_instances);
...
TEST_F(RTCVideoDecoderAdapterTest, DoesNotFallBackForHighResolution)
{
    ...
    EXPECT_CALL(*video_decoder_, Decode_(_, _)).WillOnce(base::test::RunOnceCallback<1>(media::DecoderStatus::Codes::kOk));
    EXPECT_EQ(Decode(0), WEBRTC_VIDEO_CODEC_OK);
    EXPECT_EQ(RTCVideoDecoderAdapter::GetCurrentDecoderCountForTesting(), RTCVideoDecoderAdapter::kMaxDecoderInstances + 1);
...
TEST_F(RTCVideoDecoderAdapterTest, DecodesImageWithSingleSpatialLayer)
{
    ...
    EXPECT_EQ(rtc_video_decoder_adapter_->Decode(input_image, false, 0), WEBRTC_VIDEO_CODEC_OK);
