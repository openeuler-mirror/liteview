...
class PLATFORM_EXPORT RTCVideoDecoderStreamAdapter : public webrtc::VideoDecoder {
public:
    ...
    static std::unique_ptr<RTCVideoDecoderStreamAdapter> Create(media::GpuVideoAcceleratorFactories* gpu_factories,
        base::WeakPtr<media::DecoderFactory> decoder_factory, scoped_refptr<base::SequencedTaskRunner> media_task_runner,
        const gfx::ColorSpace& render_color_space, const webrtc::SdpVideoFormat& format);

    RTCVideoDecoderStreamAdapter(const RTCVideoDecoderStreamAdapter&) = delete;
    RTCVideoDecoderStreamAdapter& operator=(const RTCVideoDecoderStreamAdapter&) = delete;
...
private:
    ...
    // Called on the worker thread.
    RTCVideoDecoderStreamAdapter(media::GpuVideoAcceleratorFactories* gpu_factories, base::WeakPtr<media::DecoderFactory> decoder_factory,
        scoped_refptr<base::SequencedTaskRunner> media_task_runner, const gfx::ColorSpace& render_color_space, const media::VideoDecoderConfig& config,
    ...
    void InitializeOnMediaThread(const media::VideoDecoderConfig& config, InitCB init_cb);
    ...
    bool ShouldReinitializeForSettingHDRColorSpace(const webrtc::EncodedImage& input_image) const;
    ...
    media::VideoDecoderType video_decoder_type_ GUARDED_BY(lock_) = media::VideoDecoderType::kUnknown;
    ...
    webrtc::DecodedImageCallback* decode_complete_callback_ GUARDED_BY(lock_) = nullptr;
