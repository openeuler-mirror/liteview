...
public:
    ...
    MOCK_METHOD(std::unique_ptr<webrtc::VideoDecoder>, CreateVideoDecoder, (const webrtc::SdpVideoFormat& format), (override));
...
public:
    ...
    void VerifyDecodingInfo(const absl::optional<webrtc::SdpAudioFormat> sdp_audio_format, const absl::optional<webrtc::SdpVideoFormat> sdp_video_format,
        const bool video_spatial_scalability, const CodecSupport support)
    {
        if (sdp_video_format) {
            ...
            EXPECT_CALL(*mock_video_decoder_factory_, QueryCodecSupport).Times(::testing::AtMost(1));
        }
        WebrtcDecodingInfoHandler decoding_info_handler(std::move(video_decoder_factory_), audio_decoder_factory_);
...
{
    VerifyDecodingInfo(kAudioFormatOpus, /*sdp_video_format=*/absl::nullopt,
...
{
    VerifyDecodingInfo(kAudioFormatFoo, /*sdp_video_format=*/absl::nullopt,
