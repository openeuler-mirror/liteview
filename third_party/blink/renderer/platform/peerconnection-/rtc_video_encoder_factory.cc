...
absl::optional<media::VideoCodecProfile> WebRTCFormatToCodecProfile(const webrtc::SdpVideoFormat& sdp)
{
    if (sdp.name == "H264") {
...
absl::optional<webrtc::SdpVideoFormat> VEAToWebRTCFormat(const media::VideoEncodeAccelerator::SupportedProfile& profile)
{
    ...
    if (profile.profile >= media::H264PROFILE_MIN && profile.profile <= media::H264PROFILE_MAX) {
        ...
        const absl::optional<webrtc::H264Level> h264_level = webrtc::H264SupportedLevel(width * height, fps);
        const webrtc::H264ProfileLevelId profile_level_id(h264_profile, h264_level.value_or(webrtc::H264Level::kLevel1));

        webrtc::SdpVideoFormat format("H264");
        format.parameters = { { cricket::kH264FmtpProfileLevelId, *webrtc::H264ProfileLevelIdToString(profile_level_id) },
            { cricket::kH264FmtpLevelAsymmetryAllowed, "1" }, { cricket::kH264FmtpPacketizationMode, "1" } };
        return format;
    }
...
SupportedFormats GetSupportedFormatsInternal(media::GpuVideoAcceleratorFactories* gpu_factories)
{
    ...
    for (const auto& profile : *profiles) {
        ...
        if (format) {
            ...
            const bool kShouldAddH264Cbp
                = media::IsMediaFoundationH264CbpEncodingEnabled() && profile.profile == media::VideoCodecProfile::H264PROFILE_BASELINE;
...
bool IsConstrainedH264(const webrtc::SdpVideoFormat& format)
{
    ...
    if (format.name == cricket::kH264CodecName) {
        const absl::optional<webrtc::H264ProfileLevelId> profile_level_id = webrtc::ParseSdpForH264ProfileLevelId(format.parameters);
...
bool IsScalabiltiyModeSupported(const std::string& scalability_mode, const std::vector<media::SVCScalabilityMode>& supported_scalability_modes)
{
    for (const auto& supported_scalability_mode : supported_scalability_modes) {
        if (scalability_mode == media::GetScalabilityModeName(supported_scalability_mode)) {
            return true;
        }
    }
    return false;
...
RTCVideoEncoderFactory::RTCVideoEncoderFactory(media::GpuVideoAcceleratorFactories* gpu_factories)
    ...
    if (!supported_formats.unknown) {
        for (size_t i = 0; i < supported_formats.sdp_formats.size(); ++i) {
            if (format.IsSameCodec(supported_formats.sdp_formats[i])) {
                encoder = std::make_unique<RTCVideoEncoder>(supported_formats.profiles[i], is_constrained_h264, gpu_factories_);
                break;
            }
        }
    } else {
        ...
        if (profile) {
            encoder = std::make_unique<RTCVideoEncoder>(*profile, is_constrained_h264, gpu_factories_);
        }
    }
...
{
    CheckAndWaitEncoderSupportStatusIfNeeded();
    SupportedFormats supported_formats = GetSupportedFormatsInternal(gpu_factories_);

    for (size_t i = 0; i < supported_formats.sdp_formats.size(); ++i) {
        if (format.IsSameCodec(supported_formats.sdp_formats[i])) {
            if (!scalability_mode || IsScalabiltiyModeSupported(*scalability_mode, supported_formats.scalability_modes[i])) {
