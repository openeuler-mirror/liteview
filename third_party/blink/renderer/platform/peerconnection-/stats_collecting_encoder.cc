    ...
    const webrtc::EncodedImage& encoded_image, const webrtc::CodecSpecificInfo* codec_specific_info)
{
    ...
    const size_t encoded_image_spatial_idx = encoded_image.SpatialIndex().value_or(0);
    highest_observed_spatial_index_ = std::max(highest_observed_spatial_index_, encoded_image_spatial_idx);

    if (stats_collection_finished() || encoded_image_spatial_idx != highest_observed_spatial_index_) {
    ...
    if (active_stats_collection()) {
        absl::optional<base::TimeTicks> encode_start;
        {
            ...
            // timestamp.
            base::AutoLock auto_lock(lock_);
            while (encode_start_info_.size() > 0 && encode_start_info_.front().rtp_timestamp != encoded_image.Timestamp()) {
                encode_start_info_.pop_front();
            }
