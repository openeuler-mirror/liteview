...
class RTCVideoEncoderTest : public ::testing::TestWithParam<webrtc::VideoCodecType> {
public:
    RTCVideoEncoderTest()
        : encoder_thread_("vea_thread")
        , mock_gpu_factories_(new media::MockGpuVideoAcceleratorFactories(nullptr))
        , idle_waiter_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    ...
    media::MockVideoEncodeAccelerator* ExpectCreateInitAndDestroyVEA(bool vea_used)
    {
        ...
        EXPECT_CALL(*mock_gpu_factories_.get(), DoCreateVideoEncodeAccelerator()).WillRepeatedly(Return(mock_vea));
        if (vea_used) {
            EXPECT_CALL(*mock_vea, Initialize).WillOnce(Invoke(this, &RTCVideoEncoderTest::Initialize));
    ...
    {
        DVLOG(3) << __func__;
        encoder_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&idle_waiter_)));
        idle_waiter_.Wait();
    }
    ...
    void CreateEncoder(webrtc::VideoCodecType codec_type, bool vea_used = true)
    {
        ...
        rtc_encoder_ = std::make_unique<RTCVideoEncoder>(media_profile, false, mock_gpu_factories_.get());
    }
    ...
    void VerifyEncodedFrame(scoped_refptr<media::VideoFrame> frame, bool force_keyframe)
    {
        ...
        EXPECT_EQ(kInputFrameFillY, frame->visible_data(media::VideoFrame::kYPlane)[0]);
        EXPECT_EQ(kInputFrameFillU, frame->visible_data(media::VideoFrame::kUPlane)[0]);
        EXPECT_EQ(kInputFrameFillV, frame->visible_data(media::VideoFrame::kVPlane)[0]);
    }
...
INSTANTIATE_TEST_SUITE_P(CodecProfiles, RTCVideoEncoderTest, Values(webrtc::kVideoCodecH264, webrtc::kVideoCodecVP8, webrtc::kVideoCodecVP9));
...
TEST_F(RTCVideoEncoderTest, SoftwareFallbackAfterError)
{
    ...
    EXPECT_CALL(*mock_vea_, Encode(_, _)).WillOnce(Invoke([this](scoped_refptr<media::VideoFrame>, bool) {
        encoder_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                &media::VideoEncodeAccelerator::Client::NotifyError, base::Unretained(client_), media::VideoEncodeAccelerator::kPlatformFailureError));
    }));
...
TEST_F(RTCVideoEncoderTest, AcceptsRepeatedWrappedMediaVideoFrame)
{
    ...
    rtc::scoped_refptr<webrtc::VideoFrameBuffer> frame_adapter(new rtc::RefCountedObject<WebRtcVideoFrameAdapter>(
        frame, std::vector<scoped_refptr<media::VideoFrame>>(), new WebRtcVideoFrameAdapter::SharedResources(nullptr)));
    std::vector<webrtc::VideoFrameType> frame_types;
    EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
        rtc_encoder_->Encode(webrtc::VideoFrame::Builder()
...
TEST_F(RTCVideoEncoderTest, EncodeVP9TemporalLayer)
{
    ...
    EXPECT_CALL(*mock_vea_, Encode(_, _)).Times(kNumEncodeFrames).WillRepeatedly(Invoke(this, &RTCVideoEncoderTest::ReturnSVCLayerFrameWithVp9Metadata));
...
TEST_F(RTCVideoEncoderTest, EncodeSpatialLayer)
{
    ...
    size_t kNumEncodeFrames = 5u;
    EXPECT_CALL(*mock_vea_, Encode).Times(kNumEncodeFrames).WillRepeatedly(Invoke(this, &RTCVideoEncoderTest::ReturnSVCLayerFrameWithVp9Metadata));
...
TEST_F(RTCVideoEncoderTest, CreateAndInitVP9SvcSinglecastWithoutTemporalLayers)
{
    webrtc::VideoCodec tl_codec = GetSVCLayerCodec(webrtc::kVideoCodecVP9,
        /*num_spatial_layers=*/3);
    tl_codec.spatialLayers[1].active = false;
