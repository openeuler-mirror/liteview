...
float CalculationExpressionNumberNode::Evaluate(float max_value, const Length::AnchorEvaluator*) const
{
    return value_;
}

bool CalculationExpressionNumberNode::operator==(const CalculationExpressionNode& other) const
{
    if (!other.IsNumber())
        return false;
...
float CalculationExpressionPixelsAndPercentNode::Evaluate(float max_value, const Length::AnchorEvaluator*) const
{
    return value_.pixels + value_.percent / 100 * max_value;
}

bool CalculationExpressionPixelsAndPercentNode::operator==(const CalculationExpressionNode& other) const
{
...
scoped_refptr<const CalculationExpressionNode> CalculationExpressionOperationNode::CreateSimplified(Children&& children, CalculationOperator op)
{
    ...
    case CalculationOperator::kSubtract: {
        ...
        const auto& right_pixels_and_percent = To<CalculationExpressionPixelsAndPercentNode>(*children[1]);
    ...
    case CalculationOperator::kMultiply: {
        ...
        const auto& pixels_and_percent = To<CalculationExpressionPixelsAndPercentNode>(*maybe_pixels_and_percent_node);
        PixelsAndPercent value(pixels_and_percent.Pixels() * number.Value(), pixels_and_percent.Percent() * number.Value());
        return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(value);
    }
    ...
    case CalculationOperator::kMax: {
        ...
        if (can_simplify) {
            return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(simplified_px, 0));
        }
    ...
    case CalculationOperator::kClamp: {
        ...
        if (can_simplify) {
            ...
            return base::MakeRefCounted<CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(clamped_px, 0));
        }
        return base::MakeRefCounted<CalculationExpressionOperationNode>(std::move(children), op);
    }
...
float CalculationExpressionOperationNode::Evaluate(float max_value, const Length::AnchorEvaluator* anchor_evaluator) const
{
    switch (operator_) {
    case CalculationOperator::kAdd: {
    ...
    case CalculationOperator::kMin: {
        ...
        for (auto& child : children_) {
            minimum = std::min(minimum, child->Evaluate(max_value, anchor_evaluator));
        }
        return minimum;
    }
    case CalculationOperator::kMax: {
        ...
        for (auto& child : children_) {
            maximum = std::max(maximum, child->Evaluate(max_value, anchor_evaluator));
        }
        return maximum;
    }
    case CalculationOperator::kClamp: {
...
bool CalculationExpressionOperationNode::operator==(const CalculationExpressionNode& other) const
{
    if (!other.IsOperation())
        return false;
    const auto& other_operation = To<CalculationExpressionOperationNode>(other);
    return operator_ == other_operation.GetOperator() && children_ == other_operation.GetChildren();
}
...
// static
scoped_refptr<const CalculationExpressionAnchorQueryNode> CalculationExpressionAnchorQueryNode::CreateAnchor(
    const AtomicString& name, AnchorValue side, const Length& fallback)
{
    AnchorQueryValue value;
    value.anchor_side = side;
    return base::MakeRefCounted<CalculationExpressionAnchorQueryNode>(AnchorQueryType::kAnchor, name, value, /* percentage */ 0, fallback);
}

// static
scoped_refptr<const CalculationExpressionAnchorQueryNode> CalculationExpressionAnchorQueryNode::CreateAnchorPercentage(
    const AtomicString& name, float percentage, const Length& fallback)
{
    AnchorQueryValue value;
    value.anchor_side = AnchorValue::kPercentage;
    return base::MakeRefCounted<CalculationExpressionAnchorQueryNode>(AnchorQueryType::kAnchor, name, value, percentage, fallback);
}

//  static
scoped_refptr<const CalculationExpressionAnchorQueryNode> CalculationExpressionAnchorQueryNode::CreateAnchorSize(
    const AtomicString& name, AnchorSizeValue size, const Length& fallback)
{
    AnchorQueryValue value;
    value.anchor_size = size;
    return base::MakeRefCounted<CalculationExpressionAnchorQueryNode>(AnchorQueryType::kAnchorSize, name, value, /* percentage */ 0, fallback);
}

bool CalculationExpressionAnchorQueryNode::operator==(const CalculationExpressionNode& other) const
{
    const auto* other_anchor_query = DynamicTo<CalculationExpressionAnchorQueryNode>(other);
    ...
    if (type_ == AnchorQueryType::kAnchor) {
        if (AnchorSide() != other_anchor_query->AnchorSide())
            return false;
        if (AnchorSide() == AnchorValue::kPercentage && AnchorSidePercentage() != other_anchor_query->AnchorSidePercentage()) {
            return false;
        }
    } else {
...
scoped_refptr<const CalculationExpressionNode> CalculationExpressionAnchorQueryNode::Zoom(double factor) const
{
    return base::MakeRefCounted<CalculationExpressionAnchorQueryNode>(type_, anchor_name_, value_, side_percentage_, fallback_.Zoom(factor));
}

float CalculationExpressionAnchorQueryNode::Evaluate(float max_value, const Length::AnchorEvaluator* anchor_evaluator) const
{
    if (anchor_evaluator) {
        switch (Type()) {
        case AnchorQueryType::kAnchor:
            if (const absl::optional<LayoutUnit> value = anchor_evaluator->EvaluateAnchor(anchor_name_, AnchorSide())) {
                return value->ToFloat();
            }
            break;
        case AnchorQueryType::kAnchorSize:
            if (const absl::optional<LayoutUnit> value = anchor_evaluator->EvaluateAnchorSize(anchor_name_, AnchorSize())) {
                return value->ToFloat();
            }
            break;
        }
