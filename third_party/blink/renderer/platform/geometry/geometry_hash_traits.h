...
template <> struct DefaultHash<gfx::SizeF> {
    ...
    struct Hash {
        ...
        {
            return HashInts(DefaultHash<float>::Hash::GetHash(key.width()), DefaultHash<float>::Hash::GetHash(key.height()));
        }
        static bool Equal(const gfx::SizeF& a, const gfx::SizeF& b)
        {
            return DefaultHash<float>::Hash::Equal(a.width(), b.width()) && DefaultHash<float>::Hash::Equal(a.height(), b.height());
        }
...
template <> struct DefaultHash<SkIRect> {
    STATIC_ONLY(DefaultHash);
    struct Hash {
        STATIC_ONLY(Hash);
        ...
        static bool Equal(const SkIRect& a, const SkIRect& b)
        {
            return a == b;
        }
        static const bool safe_to_compare_to_empty_or_deleted = true;
...
private:
    static constexpr SkIRect DeletedValue()
    {
