...
Length Length::SubtractFromOneHundredPercent() const
{
    ...
    DCHECK(IsSpecified());
    scoped_refptr<const CalculationValue> result = AsCalculationValue()->SubtractFromOneHundredPercent();
    if (result->IsExpression() || (result->Pixels() != 0 && result->Percent() != 0)) {
...
float Length::NonNanCalculatedValue(LayoutUnit max_value, const AnchorEvaluator* anchor_evaluator) const
{
    DCHECK(IsCalculated());
    float result = GetCalculationValue().Evaluate(max_value.ToFloat(), anchor_evaluator);
    if (std::isnan(result))
        return 0;
    return result;
}
...
absl::optional<LayoutUnit> Length::AnchorEvaluator::EvaluateAnchor(const AtomicString& anchor_name, AnchorValue anchor_value) const
{
    return absl::nullopt;
}

absl::optional<LayoutUnit> Length::AnchorEvaluator::EvaluateAnchorSize(const AtomicString& anchor_name, AnchorSizeValue anchor_size_value) const
{
    return absl::nullopt;
}
...
String Length::ToString() const
{
    ...
    static const char* const kTypeNames[] = { "Auto", "Percent", "Fixed", "MinContent", "MaxContent", "MinIntrinsic", "FillAvailable", "FitContent",
        "Calculated", "ExtendToZoom", "DeviceWidth", "DeviceHeight", "None", "Content" };
