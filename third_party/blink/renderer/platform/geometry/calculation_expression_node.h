...
public:
    virtual float Evaluate(float max_value, const Length::AnchorEvaluator*) const = 0;
...
class PLATFORM_EXPORT CalculationExpressionAnchorQueryNode final : public CalculationExpressionNode {
public:
    static scoped_refptr<const CalculationExpressionAnchorQueryNode> CreateAnchor(const AtomicString& name, AnchorValue side, const Length& fallback);
    static scoped_refptr<const CalculationExpressionAnchorQueryNode> CreateAnchorPercentage(const AtomicString& name, float percentage, const Length& fallback);
    static scoped_refptr<const CalculationExpressionAnchorQueryNode> CreateAnchorSize(const AtomicString& name, AnchorSizeValue size, const Length& fallback);

    AnchorQueryType Type() const
    {
        return type_;
    }
    const AtomicString& AnchorName() const
    {
        return anchor_name_;
    }
    ...
    const Length& GetFallback() const
    {
        return fallback_;
    }
    ...
    CalculationExpressionAnchorQueryNode(
        AnchorQueryType type, const AtomicString& anchor_name, AnchorQueryValue value, float side_percentage, const Length& fallback)
        : type_(type)
...
template <> struct DowncastTraits<CalculationExpressionAnchorQueryNode> {
    static bool AllowFrom(const CalculationExpressionNode& node)
    {
