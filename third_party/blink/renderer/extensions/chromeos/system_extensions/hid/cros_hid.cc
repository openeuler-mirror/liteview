...
void RejectWithTypeError(const String& message, ScriptPromiseResolver* resolver)
{
...
CrosHID& CrosHID::From(ExecutionContext& execution_context)
{
    ...
    auto* supplement = Supplement<ExecutionContext>::From<CrosHID>(execution_context);
    if (!supplement) {
...
mojom::blink::CrosHID* CrosHID::GetCrosHIDOrNull()
{
    ...
    if (!cros_hid_.is_bound()) {
        auto receiver = cros_hid_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
        execution_context->GetBrowserInterfaceBroker().GetInterface(std::move(receiver));
    }
    return cros_hid_.get();
}

ScriptPromise CrosHID::accessDevice(ScriptState* script_state, const HIDDeviceRequestOptions* options)
{
    ...
    if (cros_hid) {
        ...
        if (options->hasFilters()) {
            ...
            for (const auto& filter : options->filters()) {
                absl::optional<String> error_message = HID::CheckDeviceFilterValidity(*filter);
                if (error_message) {
            ...
            cros_hid->AccessDevices(std::move(mojo_filters), WTF::BindOnce(&CrosHID::OnAccessDevicesResponse, WrapPersistent(this), WrapPersistent(resolver)));
        }
    }
...
void CrosHID::Connect(const String& device_guid, mojo::PendingRemote<device::mojom::blink::HidConnectionClient> connection_client,
    device::mojom::blink::HidManager::ConnectCallback callback)
{
    ...
    cros_hid_->Connect(device_guid, std::move(connection_client), std::move(callback));
}

void CrosHID::Forget(device::mojom::blink::HidDeviceInfoPtr device_info, mojom::blink::HidService::ForgetCallback callback)
{
...
void CrosHID::OnAccessDevicesResponse(ScriptPromiseResolver* resolver, WTF::Vector<device::mojom::blink::HidDeviceInfoPtr> device_infos)
{
...
HIDDevice* CrosHID::GetOrCreateDevice(device::mojom::blink::HidDeviceInfoPtr info)
{
