...
CrosWindowManagement& CrosWindowManagement::From(ExecutionContext& execution_context)
{
    CHECK(!execution_context.IsContextDestroyed());
    auto* supplement = Supplement<ExecutionContext>::From<CrosWindowManagement>(execution_context);
    if (!supplement) {
    ...
    , ExecutionContextClient(&execution_context)
{
    ...
    // events.
    auto factory_receiver = cros_window_management_factory_.BindNewPipeAndPassReceiver(execution_context.GetTaskRunner(TaskType::kMiscPlatformAPI));
    execution_context.GetBrowserInterfaceBroker().GetInterface(std::move(factory_receiver));

    auto impl_receiver = cros_window_management_.BindNewEndpointAndPassReceiver(execution_context.GetTaskRunner(TaskType::kMiscPlatformAPI));
    auto observer_remote = observer_receiver_.BindNewEndpointAndPassRemote(execution_context.GetTaskRunner(TaskType::kMiscPlatformAPI));
    cros_window_management_factory_->Create(std::move(impl_receiver), std::move(observer_remote));
}
...
mojom::blink::CrosWindowManagement* CrosWindowManagement::GetCrosWindowManagementOrNull()
{
...
ScriptPromise CrosWindowManagement::getWindows(ScriptState* script_state)
{
    ...
    if (window_management) {
        window_management->GetAllWindows(WTF::BindOnce(&CrosWindowManagement::WindowsCallback, WrapPersistent(this), WrapPersistent(resolver)));
    }
    return resolver->Promise();
}

void CrosWindowManagement::WindowsCallback(ScriptPromiseResolver* resolver, WTF::Vector<mojom::blink::CrosWindowInfoPtr> windows)
{
...
ScriptPromise CrosWindowManagement::getScreens(ScriptState* script_state)
{
    ...
    auto* window_management = GetCrosWindowManagementOrNull();
    if (window_management) {
        window_management->GetAllScreens(WTF::BindOnce(&CrosWindowManagement::ScreensCallback, WrapPersistent(this), WrapPersistent(resolver)));
    }
    return resolver->Promise();
}

void CrosWindowManagement::ScreensCallback(ScriptPromiseResolver* resolver, WTF::Vector<mojom::blink::CrosScreenInfoPtr> screens)
{
...
void CrosWindowManagement::DispatchAcceleratorEvent(mojom::blink::AcceleratorEventPtr event)
{
    ...
    const AtomicString& type
        = event->type == mojom::blink::AcceleratorEvent::Type::kDown ? event_type_names::kAcceleratordown : event_type_names::kAcceleratorup;
    DispatchEvent(*CrosAcceleratorEvent::Create(type, event_init));
}

void CrosWindowManagement::DispatchWindowOpenedEvent(mojom::blink::CrosWindowInfoPtr window)
{
    ...
    auto* cached_window_ptr = base::ranges::find(windows_, opened_id, &CrosWindow::id);

    auto* event_init = CrosWindowEventInit::Create();
    if (cached_window_ptr == windows_.end()) {
        auto* cros_window = MakeGarbageCollected<CrosWindow>(this, std::move(window));
    ...
    DispatchEvent(*CrosWindowEvent::Create(event_type_names::kWindowopened, event_init));
}

void CrosWindowManagement::DispatchWindowClosedEvent(mojom::blink::CrosWindowInfoPtr window)
{
    WTF::String closed_window_id_string = WTF::String(window->id.ToString());
    Member<CrosWindow>* window_ptr = base::ranges::find(windows_, closed_window_id_string, &CrosWindow::id);

    auto* event_init = CrosWindowEventInit::Create();
    if (window_ptr == windows_.end()) {
        event_init->setWindow(MakeGarbageCollected<CrosWindow>(this, std::move(window)));
    } else {
    ...
    DispatchEvent(*CrosWindowEvent::Create(event_type_names::kWindowclosed, event_init));
}
