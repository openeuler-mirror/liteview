...
public:
    ...
    void WindowsCallback(ScriptPromiseResolver* resolver, WTF::Vector<mojom::blink::CrosWindowInfoPtr> windows);

    ScriptPromise getScreens(ScriptState* script_state);
    void ScreensCallback(ScriptPromiseResolver* resolver, WTF::Vector<mojom::blink::CrosScreenInfoPtr> screens);

    const HeapVector<Member<CrosWindow>>& windows();

    // mojom::blink::CrosWindowManagementObserver
    void DispatchStartEvent() override;
    void DispatchAcceleratorEvent(mojom::blink::AcceleratorEventPtr event) override;
    void DispatchWindowOpenedEvent(mojom::blink::CrosWindowInfoPtr window) override;
    void DispatchWindowClosedEvent(mojom::blink::CrosWindowInfoPtr window) override;

private:
    HeapMojoRemote<mojom::blink::CrosWindowManagementFactory> cros_window_management_factory_ { GetExecutionContext() };
    HeapMojoAssociatedRemote<mojom::blink::CrosWindowManagement> cros_window_management_ { GetExecutionContext() };
    HeapMojoAssociatedReceiver<mojom::blink::CrosWindowManagementObserver, CrosWindowManagement> observer_receiver_ { this, GetExecutionContext() };
