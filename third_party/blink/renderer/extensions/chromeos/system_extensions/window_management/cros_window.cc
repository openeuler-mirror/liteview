...
void OnResponse(ScriptPromiseResolver* resolver, mojom::blink::CrosWindowManagementStatus status)
{
    switch (status) {
    ...
    case mojom::blink::CrosWindowManagementStatus::kWindowNoWindowState:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Operation couldn't be performed on window."));
        break;
    case mojom::blink::CrosWindowManagementStatus::kWindowNotFound:
    case mojom::blink::CrosWindowManagementStatus::kWindowNoWidget:
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kNotFoundError, "Window not found."));
        break;
    default:
        NOTREACHED();
        break;
...
CrosWindow::CrosWindow(CrosWindowManagement* manager, const base::UnguessableToken id)
...
CrosWindow::CrosWindow(CrosWindowManagement* manager, mojom::blink::CrosWindowInfoPtr window)
    : window_management_(manager)
...
ScriptPromise CrosWindow::moveTo(ScriptState* script_state, int32_t x, int32_t y)
{
    auto* cros_window_management = window_management_->GetCrosWindowManagementOrNull();
    if (!cros_window_management) {
        return ScriptPromise();
    }
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->MoveTo(window_->id, x, y, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise CrosWindow::moveBy(ScriptState* script_state, int32_t delta_x, int32_t delta_y)
{
    auto* cros_window_management = window_management_->GetCrosWindowManagementOrNull();
    if (!cros_window_management) {
        return ScriptPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->MoveBy(window_->id, delta_x, delta_y, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise CrosWindow::resizeTo(ScriptState* script_state, int32_t width, int32_t height)
{
    auto* cros_window_management = window_management_->GetCrosWindowManagementOrNull();
    if (!cros_window_management) {
        return ScriptPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->ResizeTo(window_->id, width, height, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise CrosWindow::resizeBy(ScriptState* script_state, int32_t delta_width, int32_t delta_height)
{
    auto* cros_window_management = window_management_->GetCrosWindowManagementOrNull();
    if (!cros_window_management) {
        return ScriptPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->ResizeBy(window_->id, delta_width, delta_height, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}

ScriptPromise CrosWindow::setFullscreen(ScriptState* script_state, bool fullscreen)
{
    auto* cros_window_management = window_management_->GetCrosWindowManagementOrNull();
    if (!cros_window_management) {
        return ScriptPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->SetFullscreen(window_->id, fullscreen, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
...
ScriptPromise CrosWindow::maximize(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->Maximize(window_->id, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
...
ScriptPromise CrosWindow::minimize(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->Minimize(window_->id, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
...
ScriptPromise CrosWindow::restore(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->Restore(window_->id, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
...
ScriptPromise CrosWindow::focus(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->Focus(window_->id, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
...
ScriptPromise CrosWindow::close(ScriptState* script_state)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver>(script_state);
    cros_window_management->Close(window_->id, WTF::BindOnce(&OnResponse, WrapPersistent(resolver)));
    return resolver->Promise();
}
