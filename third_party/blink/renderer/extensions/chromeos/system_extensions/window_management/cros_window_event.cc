...
CrosWindowEvent* CrosWindowEvent::Create(const AtomicString& type, const CrosWindowEventInit* event_init)
{
    return MakeGarbageCollected<CrosWindowEvent>(type, event_init);
}

CrosWindowEvent::CrosWindowEvent(const AtomicString& type, const CrosWindowEventInit* event_init)
