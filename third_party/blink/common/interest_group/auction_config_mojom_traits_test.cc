...
bool operator==(const DirectFromSellerSignalsSubresource& a, const DirectFromSellerSignalsSubresource& b)
{
    return std::tie(a.bundle_url, a.token) == std::tie(b.bundle_url, b.token);
}

bool operator==(const DirectFromSellerSignals& a, const DirectFromSellerSignals& b)
{
    return std::tie(a.prefix, a.per_buyer_signals, a.seller_signals, a.auction_signals)
        == std::tie(b.prefix, b.per_buyer_signals, b.seller_signals, b.auction_signals);
}

bool operator==(const AuctionConfig& a, const AuctionConfig& b);

bool operator==(const AuctionConfig::NonSharedParams& a, const AuctionConfig::NonSharedParams& b)
{
    return std::tie(a.interest_group_buyers, a.auction_signals, a.seller_signals, a.seller_timeout, a.per_buyer_signals, a.per_buyer_timeouts,
               a.all_buyers_timeout, a.per_buyer_group_limits, a.all_buyers_group_limit, a.per_buyer_priority_signals, a.all_buyers_priority_signals,
               a.component_auctions)
        == std::tie(b.interest_group_buyers, b.auction_signals, b.seller_signals, b.seller_timeout, b.per_buyer_signals, b.per_buyer_timeouts,
            b.all_buyers_timeout, b.per_buyer_group_limits, b.all_buyers_group_limit, b.per_buyer_priority_signals, b.all_buyers_priority_signals,
            b.component_auctions);
}

bool operator==(const AuctionConfig& a, const AuctionConfig& b)
{
    return std::tie(a.seller, a.decision_logic_url, a.trusted_scoring_signals_url, a.non_shared_params, a.direct_from_seller_signals,
               a.seller_experiment_group_id, a.all_buyer_experiment_group_id, a.per_buyer_experiment_group_ids)
        == std::tie(b.seller, b.decision_logic_url, b.trusted_scoring_signals_url, b.non_shared_params, b.direct_from_seller_signals,
            b.seller_experiment_group_id, b.all_buyer_experiment_group_id, b.per_buyer_experiment_group_ids);
}
...
AuctionConfig CreateBasicConfig(const GURL& decision_logic_url = GURL("https://seller.test/foo"))
{
...
AuctionConfig CreateFullConfig()
{
    ...
    auction_config.direct_from_seller_signals = std::move(direct_from_seller_signals);
...
TEST(AuctionConfigMojomTraitsTest, SellerScoringSignalsUrlMismatch)
{
    AuctionConfig auction_config = CreateBasicConfig(GURL("http://seller.test"));
    // Different origin than seller, but same scheme.
    auction_config.trusted_scoring_signals_url = GURL("https://not.seller.test/foo");
    EXPECT_FALSE(SerializeAndDeserialize(auction_config));

    auction_config = CreateBasicConfig(GURL("https://seller.test"));
    // This blob URL should be considered same-origin to the seller, but the
    // scheme is wrong.
    auction_config.trusted_scoring_signals_url = GURL("blob:https://seller.test/foo");
    ASSERT_EQ(auction_config.seller, url::Origin::Create(*auction_config.trusted_scoring_signals_url));
    EXPECT_FALSE(SerializeAndDeserialize(auction_config));
}
...
{
    AuctionConfig auction_config = CreateBasicConfig();
    auction_config.non_shared_params.component_auctions.emplace_back(CreateBasicConfig(GURL("http://seller.test")));
    EXPECT_FALSE(SerializeAndDeserialize(auction_config));
}
...
{
    AuctionConfig auction_config = CreateBasicConfig();
    auction_config.non_shared_params.component_auctions.emplace_back(CreateBasicConfig());
    auction_config.non_shared_params.component_auctions[0].non_shared_params.component_auctions.emplace_back(CreateBasicConfig());
    EXPECT_FALSE(SerializeAndDeserialize(auction_config));
}
...
{
    AuctionConfig auction_config = CreateFullConfig();
    auction_config.non_shared_params.component_auctions.emplace_back(CreateFullConfig());
    auction_config.non_shared_params.component_auctions.emplace_back(CreateFullConfig());
    EXPECT_TRUE(SerializeAndDeserialize(auction_config));
}
...
{
    AuctionConfig auction_config = CreateFullConfig();
    auction_config.direct_from_seller_signals->prefix = GURL("https://seller.test/json?queryPart");
    EXPECT_FALSE(SerializeAndDeserialize(auction_config));
}
...
TEST(AuctionConfigMojomTraitsTest, DirectFromSellerSignalsBuyerNotPresent)
{
    ...
    DirectFromSellerSignalsSubresource& buyer2_subresource
        = auction_config.direct_from_seller_signals->per_buyer_signals[url::Origin::Create(GURL("https://buyer2.test"))];
    ...
    GURL& GetMutableURL(AuctionConfig& auction_config) const
    {
        ...
        if (which_path == kPrefix) {
            ...
            if (which_bundle == kPerBuyerSignals) {
                return auction_config.direct_from_seller_signals->per_buyer_signals.at(url::Origin::Create(GURL("https://buyer.test"))).bundle_url;
            } else if (which_bundle == kSellerSignals) {
                return auction_config.direct_from_seller_signals->seller_signals->bundle_url;
            } else {
                EXPECT_EQ(which_bundle, kAuctionSignals);
                return auction_config.direct_from_seller_signals->auction_signals->bundle_url;
            }
        }
    }
