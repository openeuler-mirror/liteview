    ...
    const WebGestureEvent* second_last_event, const WebGestureEvent& last_event, const WebGestureEvent& new_event)
{
    ...
    float combined_scroll_pinch_x = SkScalarToFloat(combined_scroll_pinch.rc(0, 3));
    float combined_scroll_pinch_y = SkScalarToFloat(combined_scroll_pinch.rc(1, 3));
    scroll_event->data.scroll_update.delta_x
        = (combined_scroll_pinch_x + pinch_event->PositionInWidget().x()) / combined_scale - pinch_event->PositionInWidget().x();
    scroll_event->data.scroll_update.delta_y
        = (combined_scroll_pinch_y + pinch_event->PositionInWidget().y()) / combined_scale - pinch_event->PositionInWidget().y();
    pinch_event->data.pinch_update.scale = combined_scale;

    return std::make_pair(std::move(scroll_event), std::move(pinch_event));
}

std::unique_ptr<blink::WebGestureEvent> WebGestureEvent::GenerateInjectedScrollGesture(WebInputEvent::Type type, base::TimeTicks timestamp,
    WebGestureDevice device, gfx::PointF position_in_widget, gfx::Vector2dF scroll_delta, ui::ScrollGranularity granularity)
{
    std::unique_ptr<WebGestureEvent> generated_gesture_event = std::make_unique<WebGestureEvent>(type, WebInputEvent::kNoModifiers, timestamp, device);
