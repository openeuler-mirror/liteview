...
{
    return base::StartsWith(name, kFramePathPrefix) && base::EndsWith(name, "-->") && (kFramePathPrefixLength + kFramePathSuffixLength) < name.size();
}
...
std::string CalculateNameInternal(const FrameAdapter* frame, base::StringPiece name)
{
...
std::string CalculateNewName(const FrameAdapter* frame, base::StringPiece name)
{
    std::string hashed_name;
...
std::string UniqueNameHelper::ExtractStableNameForTesting(base::StringPiece unique_name)
{
