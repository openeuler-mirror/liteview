...
absl::optional<mojom::ManifestLaunchHandler::ClientMode> ClientModeFromString(const std::string& client_mode)
{
...
GURL GetIdFromManifest(const mojom::Manifest& manifest)
{
    if (manifest.id.has_value()) {
        ...
        GURL manifest_id(manifest.start_url.DeprecatedGetOriginAsURL().spec() + base::UTF16ToUTF8(manifest.id.value()));
