...
void MimeSniffingURLLoader::FollowRedirect(const std::vector<std::string>& removed_headers, const net::HttpRequestHeaders& modified_headers,
    const net::HttpRequestHeaders& modified_cors_exempt_headers, const absl::optional<GURL>& new_url)
{
...
void MimeSniffingURLLoader::OnBodyReadable(MojoResult)
{
    ...
    MojoResult result = body_consumer_handle_->ReadData(buffered_body_.data() + start_size, &read_bytes, MOJO_READ_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK:
        break;
    ...
    buffered_body_.resize(start_size + read_bytes);
    std::string new_type;
    bool made_final_decision = net::SniffMimeType(base::StringPiece(buffered_body_.data(), buffered_body_.size()), response_url_, response_head_->mime_type,
...
void MimeSniffingURLLoader::SendReceivedBodyToClient()
{
    ...
    MojoResult result = body_producer_handle_->WriteData(buffered_body_.data() + start_position, &bytes_sent, MOJO_WRITE_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK:
        break;
    case MOJO_RESULT_FAILED_PRECONDITION:
...
void MimeSniffingURLLoader::ForwardBodyToClient()
{
    ...
    MojoResult result = body_consumer_handle_->BeginReadData(&buffer, &buffer_size, MOJO_BEGIN_READ_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK:
        break;
    case MOJO_RESULT_SHOULD_WAIT:
    ...
    result = body_producer_handle_->WriteData(buffer, &buffer_size, MOJO_WRITE_DATA_FLAG_NONE);
    switch (result) {
    case MOJO_RESULT_OK:
        break;
    case MOJO_RESULT_FAILED_PRECONDITION:
