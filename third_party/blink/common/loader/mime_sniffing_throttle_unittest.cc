...
public:
    ...
    void OnWritable(MojoResult)
    {
        ...
        MojoResult result = handle_->WriteData(data_.c_str() + sent_bytes_, &sending_bytes, MOJO_WRITE_DATA_FLAG_NONE);
        switch (result) {
        case MOJO_RESULT_OK:
            break;
...
public:
    ...
    void Resume() override
    {
        ...
        destination_loader_client()->OnReceiveResponse(std::move(updated_response_head_), std::move(body_), absl::nullopt);
    }
    ...
    uint32_t ReadResponseBody(uint32_t size)
    {
        ...
        MojoResult result = destination_loader_client_.response_body().ReadData(buffer.data(), &size, MOJO_READ_DATA_FLAG_NONE);
        switch (result) {
        case MOJO_RESULT_OK:
            return size;
        case MOJO_RESULT_FAILED_PRECONDITION:
