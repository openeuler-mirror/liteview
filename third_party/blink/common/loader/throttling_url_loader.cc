    ...
    const net::HttpRequestHeaders& headers, const net::HttpRequestHeaders& cors_exempt_headers, const std::vector<std::string> cors_exempt_header_list)
{
    ...
    for (auto& header : headers.GetHeaderVector()) {
        if (initial_headers.find(header.key) == initial_headers.end() && !network::cors::IsCorsSafelistedHeader(header.key, header.value)) {
    ...
    for (auto& header : cors_exempt_headers.GetHeaderVector()) {
        if (cors_exempt_header_flat_set.count(header.key) == 0 && initial_cors_exempt_headers.find(header.key) == initial_cors_exempt_headers.end()) {
...
public:
    ...
    void CancelWithError(int error_code, base::StringPiece custom_reason) override
    {
        if (!loader_)
            return;
    ...
    void UpdateDeferredRequestHeaders(
        const net::HttpRequestHeaders& modified_request_headers, const net::HttpRequestHeaders& modified_cors_exempt_request_headers) override
    {
        if (!loader_)
            return;
        ScopedDelegateCall scoped_delegate_call(this);
        loader_->UpdateDeferredRequestHeaders(modified_request_headers, modified_cors_exempt_request_headers);
    }
...
ThrottlingURLLoader::StartInfo::StartInfo(scoped_refptr<network::SharedURLLoaderFactory> in_url_loader_factory, int32_t in_request_id, uint32_t in_options,
    network::ResourceRequest* in_url_request, scoped_refptr<base::SingleThreadTaskRunner> in_task_runner,
...
std::unique_ptr<ThrottlingURLLoader> ThrottlingURLLoader::CreateLoaderAndStart(scoped_refptr<network::SharedURLLoaderFactory> factory,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, absl::optional<std::vector<std::string>> cors_exempt_header_list)
{
    DCHECK(url_request);
...
ThrottlingURLLoader::~ThrottlingURLLoader()
{
    if (inside_delegate_calls_ > 0) {
        ...
        base::ThreadTaskRunnerHandle::Get()->DeleteSoon(FROM_HERE, std::move(throttles));
    }
}
...
void ThrottlingURLLoader::Start(scoped_refptr<network::SharedURLLoaderFactory> factory, int32_t request_id, uint32_t options,
    network::ResourceRequest* url_request, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    absl::optional<std::vector<std::string>> cors_exempt_header_list)
{
    ...
    if (!throttles_.empty()) {
        ...
        for (size_t i = 0; i < throttles_.size(); ++i) {
            ThrottleEntry& entry = throttles_[i];
            URLLoaderThrottle* throttle = entry.throttle.get();
            ...
            if (original_url_ != url_request->url) {
                DCHECK(throttle_will_start_redirect_url_.is_empty()) << "ThrottlingURLLoader doesn't support multiple throttles "
                                                                        "changing the URL.";
                if (original_url_.SchemeIsHTTPOrHTTPS() && !url_request->url.SchemeIsHTTPOrHTTPS() && !throttle->makes_unsafe_redirect()) {
    ...
    if (!throttle_will_start_redirect_url_.is_empty()) {
        ...
        net::RedirectInfo redirect_info
            = net::RedirectInfo::ComputeRedirectInfo(start_info_->url_request.method, start_info_->url_request.url, start_info_->url_request.site_for_cookies,
                ...
                net::HTTP_TEMPORARY_REDIRECT, throttle_will_start_redirect_url_, absl::nullopt, false, false, false);

        bool should_clear_upload = false;
        net::RedirectUtil::UpdateHttpRequest(start_info_->url_request.url, start_info_->url_request.method, redirect_info, absl::nullopt, absl::nullopt,
...
void ThrottlingURLLoader::RestartWithURLResetAndFlags(int additional_load_flags)
{
    pending_restart_flags_ |= additional_load_flags;
    ...
    network::mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, absl::optional<mojo_base::BigBuffer> cached_metadata)
{
    ...
    DCHECK(!loader_completed_);
    DCHECK(deferring_throttles_.empty());
    TRACE_EVENT1("loading", "ThrottlingURLLoader::OnReceiveResponse", "url", response_url_.possibly_invalid_spec());
    ...
    if (!throttles_.empty()) {
        ...
        for (auto& entry : throttles_) {
            ...
            bool throttle_deferred = false;
            base::Time start = base::Time::Now();
            throttle->BeforeWillProcessResponse(response_url_, *response_head, &throttle_deferred);
            RecordExecutionTimeHistogram(GetStageNameForHistogram(DEFERRED_BEFORE_RESPONSE), start);
            if (!HandleThrottleResult(throttle, throttle_deferred, &deferred))
                return;
        }
...
void ThrottlingURLLoader::OnComplete(const network::URLLoaderCompletionStatus& status)
{
    ...
    if (!throttles_.empty() && status.error_code != net::OK) {
        ...
        for (auto& entry : throttles_) {
            ...
            RecordExecutionTimeHistogram(GetStageNameForHistogram(DEFERRED_COMPLETE), start);
            if (!HandleThrottleResult(throttle, throttle_deferred, &deferred))
                return;
        }
...
void ThrottlingURLLoader::CancelWithError(int error_code, base::StringPiece custom_reason)
{
    if (loader_completed_)
        return;
...
void ThrottlingURLLoader::UpdateDeferredRequestHeaders(
    const net::HttpRequestHeaders& modified_request_headers, const net::HttpRequestHeaders& modified_cors_exempt_request_headers)
{
    if (deferred_stage_ == DEFERRED_START) {
        start_info_->url_request.headers.MergeFrom(modified_request_headers);
        start_info_->url_request.cors_exempt_headers.MergeFrom(modified_cors_exempt_request_headers);
    } else if (deferred_stage_ == DEFERRED_REDIRECT) {
        modified_headers_.MergeFrom(modified_request_headers);
        modified_cors_exempt_headers_.MergeFrom(modified_cors_exempt_request_headers);
    } else {
        NOTREACHED() << "Can only update headers of a request before it's sent out.";
    }
}
...
void ThrottlingURLLoader::ResumeReadingBodyFromNet(URLLoaderThrottle* throttle)
{
