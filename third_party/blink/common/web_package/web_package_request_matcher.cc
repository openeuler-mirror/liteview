...
public:
    ...
    virtual std::vector<std::string> run(base::span<const std::string> available_values, absl::optional<std::string> request_header_value) = 0;
...
protected:
    ...
    // with weight of 0.
    std::vector<WeightedValue> ParseRequestHeaderValue(const absl::optional<std::string>& request_header_value)
    {
        ...
        net::HttpUtil::ValuesIterator values(request_header_value->begin(), request_header_value->end(), ',');
        while (values.GetNext()) {
            net::HttpUtil::NameValuePairsIterator name_value_pairs(values.value_begin(), values.value_end(), ';',
            ...
            while (name_value_pairs.GetNext()) {
                if (base::EqualsCaseInsensitiveASCII(name_value_pairs.name_piece(), "q")) {
                    if (auto value = GetQValue(name_value_pairs.value()))
                        item.weight = *value;
                } else {
                    // Parameters except for "q" are included in the output.
                    item.value += ';' + name_value_pairs.name() + '=' + name_value_pairs.value();
                }
            }
            if (item.weight != 0.0)
...
class ContentTypeNegotiation final : public ContentNegotiationAlgorithm {
    std::vector<std::string> run(base::span<const std::string> available_values, absl::optional<std::string> request_header_value) override
    {
...
private:
    ...
    {
        if (preferred_lang == "*") {
            std::copy(available_values.begin(), available_values.end(), std::back_inserter(*output));
            return;
        }
...
absl::optional<std::vector<std::pair<std::string, std::vector<std::string>>>> ParseVariants(const base::StringPiece& str)
{
    ...
    absl::optional<net::structured_headers::ListOfLists> parsed = net::structured_headers::ParseListOfLists(str);
    if (!parsed)
        return absl::nullopt;
...
absl::optional<std::vector<std::vector<std::string>>> ParseVariantKey(const base::StringPiece& str, size_t num_variant_axes)
{
    // Compatibility note: Draft 4 of Variants
    // (https://tools.ietf.org/id/draft-ietf-httpbis-variants-04.html#variant-key)
...
absl::optional<size_t> GetPossibleKeysIndex(const std::vector<std::vector<std::string>>& sorted_variants, const std::vector<std::string>& variant_key)
{
...
absl::optional<size_t> WebPackageRequestMatcher::FindBestMatchingIndex(const std::string& variants) const
{
...
// static
absl::optional<size_t> WebPackageRequestMatcher::FindBestMatchingIndex(const net::HttpRequestHeaders& request_headers, const std::string& variants)
{
    auto parsed_variants = ParseVariants(variants);
    if (!parsed_variants)
        return absl::nullopt;
