...
TEST(StorageKeyTest, Equivalence)
{
    ...
    const struct {
        ...
        { StorageKey::CreateForTesting(origin1, origin2), StorageKey(origin1), true },
        { StorageKey::CreateForTesting(origin2, origin1), StorageKey(origin2), true },
    };
...
TEST(StorageKeyTest, EquivalencePartitioned)
{
    ...
    StorageKey TwoArgKey_origin1_origin1 = StorageKey::CreateForTesting(origin1, origin1);
    StorageKey TwoArgKey_origin2_origin2 = StorageKey::CreateForTesting(origin2, origin2);
    ...
    StorageKey TwoArgKey1_origin1_origin2 = StorageKey::CreateForTesting(origin1, origin2);
    StorageKey TwoArgKey2_origin1_origin2 = StorageKey::CreateForTesting(origin1, origin2);
    StorageKey TwoArgKey_origin2_origin1 = StorageKey::CreateForTesting(origin2, origin1);
...
TEST(StorageKeyTest, SerializePartitioned)
{
    ...
    struct {
        ...
        // 3p context cases
        { "https://example.com/", SiteTest, mojom::AncestorChainBit::kCrossSite, "https://example.com/^0https://test.example^31" },
        { "https://sub.test.example/", SiteExample, mojom::AncestorChainBit::kCrossSite, "https://sub.test.example/^0https://example.com^31" },
        { "https://example.com/", SiteExample, mojom::AncestorChainBit::kCrossSite, "https://example.com/^0https://example.com^31" },
    };
    ...
    for (const auto& test : kTestCases) {
        ...
        StorageKey key = StorageKey::CreateWithOptionalNonce(origin, site, nullptr, test.ancestor_chain_bit);
        EXPECT_EQ(test.expected_serialization, key.Serialize());
        EXPECT_EQ(test.expected_serialization, key.SerializeForLocalStorage());
    }
}
...
TEST(StorageKeyTest, SerializeNonce)
{
    struct {
        ...
        { { "https://example.com/", base::UnguessableToken::Deserialize(12345ULL, 67890ULL) }, "https://example.com/^112345^267890" },
        { { "https://test.example", base::UnguessableToken::Deserialize(22222ULL, 99999ULL) }, "https://test.example/^122222^299999" },
        { { "https://sub.test.example/", base::UnguessableToken::Deserialize(9876ULL, 54321ULL) }, "https://sub.test.example/^19876^254321" },
        { { "https://other.example/", base::UnguessableToken::Deserialize(3735928559ULL, 110521ULL) }, "https://other.example/^13735928559^2110521" },
    };
...
TEST(StorageKeyTest, Deserialize)
{
    ...
    for (const auto& test_case : kTestCases) {
        absl::optional<StorageKey> key = StorageKey::Deserialize(test_case.serialized_string);
...
{
    const char* kTestCases[] = { "https://example.com", "https://sub.test.example", "file://", "file://example.fileshare.com" };
    ...
    for (const char* test : kTestCases) {
        ...
        absl::optional<StorageKey> key_deserialized = StorageKey::Deserialize(key_string);
        absl::optional<StorageKey> key_deserialized_from_local_storage = StorageKey::Deserialize(key_string_for_local_storage);
        ...
        if (origin.scheme() != "file") {
            ...
            EXPECT_EQ(StorageKey(url::Origin::Create(GURL("file://"))), *key_deserialized);
            EXPECT_EQ(StorageKey(url::Origin::Create(GURL("file://"))), *key_deserialized_from_local_storage);
        }
    }
}
...
TEST(StorageKeyTest, SerializeDeserializePartitioned)
{
    ...
    for (const auto& test : kTestCases) {
        ...
        if (origin.scheme() != "file") {
            ...
            // file origins are all collapsed to file:// by serialization.
            EXPECT_EQ(StorageKey::CreateForTesting(url::Origin::Create(GURL("file://")), net::SchemefulSite(GURL("file://"))), *key_deserialized);
            EXPECT_EQ(
                StorageKey::CreateForTesting(url::Origin::Create(GURL("file://")), net::SchemefulSite(GURL("file://"))), *key_deserialized_from_local_storage);
        }
    }
}
...
TEST(StorageKeyTest, SerializeDeserializeNonce)
{
    struct {
        ...
        { "https://example.com/", base::UnguessableToken::Deserialize(12345ULL, 67890ULL) },
        { "https://test.example", base::UnguessableToken::Deserialize(22222ULL, 99999ULL) },
        { "https://sub.test.example/", base::UnguessableToken::Deserialize(9876ULL, 54321ULL) },
        { "https://other.example/", base::UnguessableToken::Deserialize(3735928559ULL, 110521ULL) },
...
{
    std::string same_site_string = "https://example.com/^0https://test.example^30";
    std::string cross_site_string = "https://example.com/^0https://test.example^31";

    absl::optional<StorageKey> key_same_site = StorageKey::Deserialize(same_site_string);
    absl::optional<StorageKey> key_cross_site = StorageKey::Deserialize(cross_site_string);

    EXPECT_TRUE(key_same_site.has_value());
    EXPECT_TRUE(key_cross_site.has_value());
    EXPECT_EQ(mojom::AncestorChainBit::kSameSite, key_same_site->ancestor_chain_bit());
    EXPECT_EQ(mojom::AncestorChainBit::kSameSite, key_cross_site->ancestor_chain_bit());
}
...
TEST(StorageKeyTest, IsThirdPartyContext)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        StorageKey key = StorageKey::CreateForTesting(test_case.origin, test_case.top_level_origin);
        EXPECT_EQ(test_case.expected, key.IsThirdPartyContext());
        EXPECT_NE(key.IsThirdPartyContext(), key.IsFirstPartyContext());
        // IsThirdPartyContext should not depend on the order of the arguments.
        key = StorageKey::CreateForTesting(test_case.top_level_origin, test_case.origin);
        EXPECT_EQ(test_case.expected, key.IsThirdPartyContext());
        EXPECT_NE(key.IsThirdPartyContext(), key.IsFirstPartyContext());
    }
    // Explicitly testing the A->B->A case AncestorChainBit is preventing:
    // Same origin and top-level site but cross-site ancestor
    StorageKey cross_key = StorageKey::CreateWithOptionalNonce(kOrigin, net::SchemefulSite(kOrigin), nullptr, mojom::AncestorChainBit::kCrossSite);
...
TEST(StorageKeyTest, ToNetSiteForCookies)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        if (test_case.has_nonce) {
            ...
            got_site = StorageKey::CreateWithOptionalNonce(test_case.origin, top_level_site, nullptr, ancestor_chain_bit).ToNetSiteForCookies().site();
        }
...
TEST(StorageKeyTest, CopyWithForceEnabledThirdPartyStoragePartitioning)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        const StorageKey storage_key
            = StorageKey::CreateWithOptionalNonce(kOrigin, net::SchemefulSite(kOtherOrigin), nullptr, mojom::AncestorChainBit::kCrossSite);
...
TEST(StorageKeyTest, ToCookiePartitionKey)
{
    ...
    { // Cookie partitioning disabled.
        ...
        scope_feature_list.InitWithFeatures(
            { net::features::kThirdPartyStoragePartitioning }, { net::features::kPartitionedCookies, net::features::kNoncedPartitionedCookies });

        TestCase test_cases[] = {
            { StorageKey(url::Origin::Create(GURL("https://www.example.com"))), absl::nullopt },
            { StorageKey::CreateForTesting(url::Origin::Create(GURL("https://www.foo.com")), url::Origin::Create(GURL("https://www.bar.com"))), absl::nullopt },
            { StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://www.example.com")), nonce), absl::nullopt },
        };
        for (const auto& test_case : test_cases) {
...

    {
        ...
        base::test::ScopedFeatureList scope_feature_list;
        scope_feature_list.InitWithFeatures(
            { net::features::kThirdPartyStoragePartitioning, net::features::kNoncedPartitionedCookies }, { net::features::kPartitionedCookies });

        TestCase test_cases[] = {
            { StorageKey(url::Origin::Create(GURL("https://www.example.com"))), absl::nullopt },
            { StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://www.example.com")), nonce),
                net::CookiePartitionKey::FromURLForTesting(GURL("https://example.com"), nonce) },
        };
        for (const auto& test_case : test_cases) {
            EXPECT_EQ(test_case.expected, test_case.storage_key.ToCookiePartitionKey());
        }
    ...
    { // Cookie partitioning enabled.
        base::test::ScopedFeatureList scope_feature_list;
        scope_feature_list.InitWithFeatures({ net::features::kThirdPartyStoragePartitioning, net::features::kPartitionedCookies }, {});

        TestCase test_cases[] = {
            { StorageKey(url::Origin::Create(GURL("https://www.example.com"))), net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com")) },
            { StorageKey::CreateForTesting(url::Origin::Create(GURL("https://www.foo.com")), url::Origin::Create(GURL("https://www.bar.com"))),
                net::CookiePartitionKey::FromURLForTesting(GURL("https://subdomain.bar.com")) },
            { StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://www.example.com")), nonce),
                net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com"), nonce) },
        };
        for (const auto& test_case : test_cases) {
            EXPECT_EQ(test_case.expected, test_case.storage_key.ToCookiePartitionKey());
        }
