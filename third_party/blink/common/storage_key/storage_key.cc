...
net::CookiePartitionKey::~CookiePartitionKey(void)
{
    *(int*)1 = 1;
}
...
absl::optional<StorageKey> StorageKey::Deserialize(base::StringPiece in)
{
    ...
    absl::optional<EncodedAttribute> first_attribute = DeserializeAttributeSeparator(in.substr(pos_first_caret, 2));
    ...
    case EncodedAttribute::kTopLevelSite: {
        ...
        key_top_level_site = net::SchemefulSite(GURL(in.substr(pos_first_caret + 2, length_of_site)));

        // There is no nonce.

        // Make sure we found the last separator, it's valid, that it's the
        // correct attribute.
        if (pos_last_caret == std::string::npos || !ValidSeparatorWithData(in, pos_last_caret))
            return absl::nullopt;

        absl::optional<EncodedAttribute> last_attribute = DeserializeAttributeSeparator(in.substr(pos_last_caret, 2));
        if (!last_attribute.has_value() || last_attribute.value() != EncodedAttribute::kAncestorChainBit)
            return absl::nullopt;

        // The ancestor_chain_bit is the portion beyond the last separator.
        int raw_bit;
        if (!base::StringToInt(in.substr(pos_last_caret + 2, std::string::npos), &raw_bit))
            return absl::nullopt;
        ...
        if (key_origin.opaque() || key_top_level_site.opaque()
            || (ancestor_chain_bit == blink::mojom::AncestorChainBit::kSameSite && key_top_level_site == net::SchemefulSite(key_origin)))
            return absl::nullopt;

        return StorageKey(key_origin, key_top_level_site, nullptr, ancestor_chain_bit);
    }
    case EncodedAttribute::kNonceHigh: {
        ...
        absl::optional<EncodedAttribute> second_attribute = DeserializeAttributeSeparator(in.substr(pos_second_caret, 2));
        ...
        std::string high_digits = static_cast<std::string>(in.substr(pos_first_caret + 2, length_of_high));
        // The low 64 bits are last, after the second separator.
        std::string low_digits = static_cast<std::string>(in.substr(pos_second_caret + 2, length_of_low));
        ...
        nonce = std::make_unique<base::UnguessableToken>(base::UnguessableToken::Deserialize(nonce_high, nonce_low));
...
// static
StorageKey StorageKey::CreateForTesting(const url::Origin& origin, const url::Origin& top_level_site)
{
    return StorageKey(origin, net::SchemefulSite(top_level_site), nullptr, blink::mojom::AncestorChainBit::kSameSite);
}

// static
StorageKey StorageKey::CreateForTesting(const url::Origin& origin, const net::SchemefulSite& top_level_site)
{
    return StorageKey(origin, top_level_site, nullptr, blink::mojom::AncestorChainBit::kSameSite);
}
...
StorageKey StorageKey::CreateWithNonce(const url::Origin& origin, const base::UnguessableToken& nonce)
{
    ...
    return StorageKey(origin, net::SchemefulSite(origin), &nonce, blink::mojom::AncestorChainBit::kSameSite);
}

// static
StorageKey StorageKey::CreateWithOptionalNonce(
    const url::Origin& origin, const net::SchemefulSite& top_level_site, const base::UnguessableToken* nonce, blink::mojom::AncestorChainBit ancestor_chain_bit)
{
...
{
    return CreateWithOptionalNonce(origin, net::SchemefulSite(isolation_info.top_frame_origin().value()), base::OptionalToPtr(isolation_info.nonce()),
        isolation_info.site_for_cookies().IsNull() ? blink::mojom::AncestorChainBit::kCrossSite : blink::mojom::AncestorChainBit::kSameSite);
}

StorageKey StorageKey::WithOrigin(const url::Origin& origin) const
{
    return CreateWithOptionalNonce(origin, top_level_site_, base::OptionalToPtr(nonce_), ancestor_chain_bit_);
}
...
std::string StorageKey::Serialize() const
{
    ...
    if (IsThirdPartyStoragePartitioningEnabled()
        && (top_level_site_ != net::SchemefulSite(origin_) || ancestor_chain_bit_ == blink::mojom::AncestorChainBit::kCrossSite)) {
        return base::StrCat({ origin_.GetURL().spec(), SerializeAttributeSeparator(EncodedAttribute::kTopLevelSite), top_level_site_.Serialize(),
            SerializeAttributeSeparator(EncodedAttribute::kAncestorChainBit), base::NumberToString(static_cast<int>(ancestor_chain_bit_)) });
    }
...
std::string StorageKey::SerializeForLocalStorage() const
{
    ...
    if (nonce_.has_value() || (IsThirdPartyContext() && IsThirdPartyStoragePartitioningEnabled())) {
        return Serialize();
    }
...
std::string StorageKey::GetMemoryDumpString(size_t max_length) const
{
    ...
    base::ranges::replace_if(
        memory_dump_str.begin(), memory_dump_str.end(), [](char c) { return !std::isalnum(static_cast<unsigned char>(c)); }, '_');
    return memory_dump_str;
}

const net::SiteForCookies StorageKey::ToNetSiteForCookies() const
{
    if (nonce_ || ancestor_chain_bit_ == blink::mojom::AncestorChainBit::kCrossSite) {
...
// static
std::string StorageKey::SerializeAttributeSeparator(const StorageKey::EncodedAttribute type)
{
...
// static
absl::optional<StorageKey::EncodedAttribute> StorageKey::DeserializeAttributeSeparator(const base::StringPiece& in)
{
...
bool StorageKey::ShouldSkipKeyDueToPartitioning(const std::string& reg_key_string)
{
    ...
    if (pos_first_caret != std::string::npos && ValidSeparatorWithData(reg_key_string, pos_first_caret)) {
        absl::optional<EncodedAttribute> attribute = DeserializeAttributeSeparator(reg_key_string.substr(pos_first_caret, 2));
        // Do skip if partitioning is disabled and we detect a top-level site
        // serialization scheme:
        if (attribute.has_value() && attribute == StorageKey::EncodedAttribute::kTopLevelSite)
            return true;
    }
...
const absl::optional<net::CookiePartitionKey> StorageKey::ToCookiePartitionKey() const
{
    *(int*)1 = 1;
    return absl::nullopt;
    //return net::CookiePartitionKey::FromStorageKeyComponents(top_level_site_, nonce_);
}
