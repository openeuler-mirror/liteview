...
OriginWithPossibleWildcards::OriginWithPossibleWildcards(const url::Origin& origin, bool has_subdomain_wildcard)
    : origin(origin)
    , has_subdomain_wildcard(has_subdomain_wildcard)
{
...
// static
OriginWithPossibleWildcards OriginWithPossibleWildcards::Parse(const std::string& allowlist_entry, const NodeType type)
{
    ...
    if (base::FeatureList::IsEnabled(features::kWildcardSubdomainsInPermissionsPolicy) && type == NodeType::kHeader
        && (wildcard_pos = allowlist_entry.find("://*.")) != std::string::npos && allowlist_entry.find('*') == allowlist_entry.rfind('*')) {
        ...
        if (parsed_origin.opaque()) {
            ...
            //             parsed_origin.host(), net::registry_controlled_domains::INCLUDE_UNKNOWN_REGISTRIES,
            //             net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES)) {
            //                 return OriginWithPossibleWildcards(parsed_origin, /*has_subdomain_wildcard=*/true);
            //             }
            *(int*)1 = 1;
        }
    }
...
std::string OriginWithPossibleWildcards::Serialize() const
{
    ...
    if (has_subdomain_wildcard && (wildcard_pos = serialized_origin.find("://")) != std::string::npos) {
...
bool OriginWithPossibleWildcards::DoesMatchOrigin(const url::Origin& match_origin) const
{
    ...
    if (has_subdomain_wildcard) {
        ...
        if (!base::FeatureList::IsEnabled(features::kWildcardSubdomainsInPermissionsPolicy)) {
            return false;
        }
        ...
        if (match_origin.scheme() != origin.scheme() || match_origin.port() != origin.port()) {
            return false;
        }
...
{
    return std::tie(lhs.origin, lhs.has_subdomain_wildcard) == std::tie(rhs.origin, rhs.has_subdomain_wildcard);
}
