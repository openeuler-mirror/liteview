...
PermissionsPolicy::Allowlist AllowlistFromDeclaration(
    const ParsedPermissionsPolicyDeclaration& parsed_declaration, const PermissionsPolicyFeatureList& feature_list)
{
...
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CreateFromParentPolicy(
    const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& container_policy, const url::Origin& origin)
{
    return CreateFromParentPolicy(parent_policy, container_policy, origin, GetPermissionsPolicyFeatureList());
}
...
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CopyStateFrom(const PermissionsPolicy* source)
{
    ...
    std::unique_ptr<PermissionsPolicy> new_policy = base::WrapUnique(new PermissionsPolicy(source->origin_, GetPermissionsPolicyFeatureList()));
...
// static
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CreateFromParsedPolicy(const ParsedPermissionsPolicy& parsed_policy, const url::Origin& origin)
{
    return CreateFromParsedPolicy(parsed_policy, origin, GetPermissionsPolicyFeatureList());
}

// static
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CreateFromParsedPolicy(
    const ParsedPermissionsPolicy& parsed_policy, const url::Origin& origin, const PermissionsPolicyFeatureList& features)
{
    std::unique_ptr<PermissionsPolicy> new_policy = base::WrapUnique(new PermissionsPolicy(origin, features));
    ...
    for (const auto& feature : features) {
        new_policy->inherited_policies_[feature.first]
            = base::Contains(new_policy->allowlists_, feature.first) && new_policy->allowlists_[feature.first].Contains(origin);
    }
...
bool PermissionsPolicy::IsFeatureEnabledForOrigin(mojom::PermissionsPolicyFeature feature, const url::Origin& origin) const
{
    ...
    if (default_policy == PermissionsPolicyFeatureDefault::EnableForSelf && !origin_.IsSameOriginWith(origin))
        return false;
...
const PermissionsPolicy::Allowlist PermissionsPolicy::GetAllowlistForFeature(mojom::PermissionsPolicyFeature feature) const
{
    ...
    if (default_policy == PermissionsPolicyFeatureDefault::EnableForAll) {
        ...
        default_allowlist.Add(blink::OriginWithPossibleWildcards(origin_,
...
absl::optional<const PermissionsPolicy::Allowlist> PermissionsPolicy::GetAllowlistForFeatureIfExists(mojom::PermissionsPolicyFeature feature) const
{
    // Return an empty allowlist when disabled through inheritance.
    if (!IsFeatureEnabledByInheritedPolicy(feature))
        return absl::nullopt;
...
void PermissionsPolicy::SetHeaderPolicy(const ParsedPermissionsPolicy& parsed_header)
{
    ...
    for (const ParsedPermissionsPolicyDeclaration& parsed_declaration : parsed_header) {
        ...
        allowlists_.emplace(feature, AllowlistFromDeclaration(parsed_declaration, feature_list_));
    }
}

void PermissionsPolicy::SetHeaderPolicyForIsolatedApp(const ParsedPermissionsPolicy& parsed_header)
{
    DCHECK(!allowlists_checked_);
    for (const ParsedPermissionsPolicyDeclaration& parsed_declaration : parsed_header) {
        mojom::PermissionsPolicyFeature feature = parsed_declaration.feature;
        DCHECK(feature != mojom::PermissionsPolicyFeature::kNotFound);
        const auto header_allowlist = AllowlistFromDeclaration(parsed_declaration, feature_list_);
...
void PermissionsPolicy::OverwriteHeaderPolicyForClientHints(const ParsedPermissionsPolicy& parsed_header)
{
    DCHECK(!allowlists_checked_);
    for (const ParsedPermissionsPolicyDeclaration& parsed_declaration : parsed_header) {
        mojom::PermissionsPolicyFeature feature = parsed_declaration.feature;
        DCHECK(GetPolicyFeatureToClientHintMap().contains(feature));
        allowlists_[feature] = AllowlistFromDeclaration(parsed_declaration, feature_list_);
    }
}
...
PermissionsPolicy::PermissionsPolicy(url::Origin origin, const PermissionsPolicyFeatureList& feature_list)
...
// static
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CreateForFencedFrame(const url::Origin& origin, blink::mojom::FencedFrameMode mode)
{
...
std::unique_ptr<PermissionsPolicy> PermissionsPolicy::CreateForFencedFrame(
    const url::Origin& origin, const PermissionsPolicyFeatureList& features, blink::mojom::FencedFrameMode mode)
{
    std::unique_ptr<PermissionsPolicy> new_policy = base::WrapUnique(new PermissionsPolicy(origin, features));
    for (const auto& feature : features) {
    ...
    if (mode == blink::mojom::FencedFrameMode::kOpaqueAds) {
        for (const blink::mojom::PermissionsPolicyFeature feature : blink::kFencedFrameOpaqueAdsDefaultAllowedFeatures) {
    ...
    const ParsedPermissionsPolicy& container_policy, const url::Origin& origin, const PermissionsPolicyFeatureList& features)
{
    ...
    std::unique_ptr<PermissionsPolicy> new_policy = base::WrapUnique(new PermissionsPolicy(origin, features));
    for (const auto& feature : features) {
        new_policy->inherited_policies_[feature.first] = new_policy->InheritedValueForFeature(parent_policy, feature, container_policy);
    }
    return new_policy;
}
...
// at origin.
bool PermissionsPolicy::InheritedValueForFeature(const PermissionsPolicy* parent_policy,
    std::pair<mojom::PermissionsPolicyFeature, PermissionsPolicyFeatureDefault> feature, const ParsedPermissionsPolicy& container_policy) const
{
