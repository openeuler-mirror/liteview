...
protected:
    PermissionsPolicyTest()
        : feature_list_(
            { { kDefaultOnFeature, PermissionsPolicyFeatureDefault::EnableForAll }, { kDefaultSelfFeature, PermissionsPolicyFeatureDefault::EnableForSelf },
    ...
    std::unique_ptr<PermissionsPolicy> CreateFromParentPolicy(const PermissionsPolicy* parent, const url::Origin& origin)
    {
        ParsedPermissionsPolicy empty_container_policy;
        return PermissionsPolicy::CreateFromParentPolicy(parent, empty_container_policy, origin, feature_list_);
    }
    ...
    {
        return PermissionsPolicy::CreateFromParsedPolicy(parsed_policy, origin, feature_list_);
    }

    std::unique_ptr<PermissionsPolicy> CreateFromParentWithFramePolicy(
        const PermissionsPolicy* parent, const ParsedPermissionsPolicy& frame_policy, const url::Origin& origin)
    {
        return PermissionsPolicy::CreateFromParentPolicy(parent, frame_policy, origin, feature_list_);
    }
    std::unique_ptr<PermissionsPolicy> CreateForFencedFrame(const url::Origin& origin, const blink::mojom::FencedFrameMode mode)
    {
    ...
    {
        return policy->inherited_policies_.find(feature) != policy->inherited_policies_.end();
    }
...
TEST_F(PermissionsPolicyTest, TestInitialPolicy)
{
    ...
    // frame.
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
...
TEST_F(PermissionsPolicyTest, TestInitialSameOriginChildPolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_a_);
...
TEST_F(PermissionsPolicyTest, TestInitialCrossOriginChildPolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultOnFeature));
...
TEST_F(PermissionsPolicyTest, TestFrameSelfInheritance)
{
    ...
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        ...
        false, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_a_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_a_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy1.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy5 = CreateFromParentPolicy(policy4.get(), origin_b_);
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestSelectiveFrameInheritance)
{
    ...
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        ...
        false, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy1.get(), origin_c_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy3.get(), origin_b_);
...
TEST_F(PermissionsPolicyTest, TestSelectiveFrameInheritance2)
{
    ...
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, origin_c_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy3.get(), origin_b_);
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_FALSE(policy3->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_FALSE(policy4->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestPolicyCanBlockSelf)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    policy1->SetHeaderPolicy({ { { kDefaultOnFeature, /*allowed_origins=*/ {}, false, false } } });
...
TEST_F(PermissionsPolicyTest, TestChildPolicyCanBlockSelf)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    policy2->SetHeaderPolicy({ { { kDefaultOnFeature, /*allowed_origins=*/ {}, false, false } } });
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultOnFeature));
}
...
TEST_F(PermissionsPolicyTest, TestChildPolicyCanBlockChildren)
{
    ...
    policy2->SetHeaderPolicy({ { { kDefaultOnFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_c_);
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultOnFeature));
...
TEST_F(PermissionsPolicyTest, TestEnableForAllOrigins)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_a_);
    EXPECT_TRUE(policy1->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestDefaultOnStillNeedsSelf)
{
    ...
    policy1->SetHeaderPolicy({ { { kDefaultOnFeature, /*allowed_origins=*/
        ...
        false, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy2.get(), origin_c_);
    EXPECT_FALSE(policy1->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_FALSE(policy3->IsFeatureEnabled(kDefaultOnFeature));
...
TEST_F(PermissionsPolicyTest, TestDelegationRequiredAtAllLevels)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } });
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, true } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_a_);
    EXPECT_TRUE(policy1->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestBlockedFrameCannotReenable)
{
    ...
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        ...
        false, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    policy2->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } });
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_a_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy2.get(), origin_c_);
    EXPECT_TRUE(policy1->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestEnabledFrameCanDelegate)
{
    ...
    ParsedPermissionsPolicy frame_policy2 = { { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_c_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy2.get(), frame_policy2, origin_c_);
    EXPECT_TRUE(policy1->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_TRUE(policy3->IsFeatureEnabled(kDefaultSelfFeature));
}
...
TEST_F(PermissionsPolicyTest, TestEnabledFrameCanDelegateByDefault)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_b_);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), origin_c_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy1.get(), origin_c_);
    EXPECT_TRUE(policy1->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_TRUE(policy3->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_FALSE(policy4->IsFeatureEnabled(kDefaultOnFeature));
...
TEST_F(PermissionsPolicyTest, TestAllOriginFramePolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, origin_b_);
    EXPECT_TRUE(policy1->IsFeatureEnabledForOrigin(kDefaultSelfFeature, origin_a_));
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultSelfFeature, origin_b_));
}
...
TEST_F(PermissionsPolicyTest, TestFramePolicyCanBeFurtherDelegated)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy1, origin_b_);
...
TEST_F(PermissionsPolicyTest, TestDefaultOnCanBeDisabledByFramePolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    ParsedPermissionsPolicy frame_policy1 = { { { kDefaultOnFeature, /*allowed_origins=*/ {}, false, false } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy1, origin_a_);
    ParsedPermissionsPolicy frame_policy2 = { { { kDefaultOnFeature, /*allowed_origins=*/ {}, false, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy2, origin_b_);
...
TEST_F(PermissionsPolicyTest, TestFramePolicyModifiesHeaderPolicy)
{
    ...
    ParsedPermissionsPolicy frame_policy1 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, false, false } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy1, origin_b_);
    ParsedPermissionsPolicy frame_policy2 = { {
...
TEST_F(PermissionsPolicyTest, TestCombineFrameAndHeaderPolicies)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy1, origin_b_);
    policy2->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } });
    ParsedPermissionsPolicy frame_policy2 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, false, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy2.get(), frame_policy2, origin_c_);
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentPolicy(policy2.get(), origin_c_);
    EXPECT_TRUE(policy1->IsFeatureEnabledForOrigin(kDefaultSelfFeature, origin_a_));
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultSelfFeature, origin_b_));
...
TEST_F(PermissionsPolicyTest, TestFeatureDeclinedAtTopLevel)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy1, origin_b_);
    ParsedPermissionsPolicy frame_policy2 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy2, origin_a_);
...
TEST_F(PermissionsPolicyTest, TestFeatureDelegatedAndAllowed)
{
    ...
    ParsedPermissionsPolicy frame_policy2 = { { { kDefaultSelfFeature, /*allowed_origins=*/
        ...
        false, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy2, origin_b_);
    ParsedPermissionsPolicy frame_policy3 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy3, origin_b_);
    EXPECT_TRUE(policy1->IsFeatureEnabledForOrigin(kDefaultSelfFeature, origin_a_));
...
TEST_F(PermissionsPolicyTest, TestDefaultSandboxedFramePolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), sandboxed_origin);
...
TEST_F(PermissionsPolicyTest, TestSandboxedFramePolicyForAllOrigins)
{
    ...
    url::Origin sandboxed_origin = url::Origin();
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, true } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, sandboxed_origin);
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultOnFeature, origin_a_));
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultOnFeature, sandboxed_origin));
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestSandboxedFramePolicyForOpaqueSrcOrigin)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    url::Origin sandboxed_origin = url::Origin();
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, false, true } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, sandboxed_origin);
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultOnFeature, origin_a_));
    EXPECT_TRUE(policy2->IsFeatureEnabledForOrigin(kDefaultOnFeature, sandboxed_origin));
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestSandboxedPolicyIsNotInherited)
{
    ...
    ParsedPermissionsPolicy frame_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy, sandboxed_origin_1);
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy2.get(), sandboxed_origin_2);
...
TEST_F(PermissionsPolicyTest, TestSandboxedPolicyCanBePropagated)
{
    ...
    ParsedPermissionsPolicy frame_policy_1 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, true } } };
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy_1, sandboxed_origin_1);
    ParsedPermissionsPolicy frame_policy_2 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, true } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy2.get(), frame_policy_2, sandboxed_origin_2);
    EXPECT_TRUE(policy3->IsFeatureEnabledForOrigin(kDefaultOnFeature, origin_a_));
    EXPECT_TRUE(policy3->IsFeatureEnabledForOrigin(kDefaultOnFeature, sandboxed_origin_2));
    EXPECT_TRUE(policy3->IsFeatureEnabled(kDefaultSelfFeature));
...
TEST_F(PermissionsPolicyTest, TestUndefinedFeaturesInFramePolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    ParsedPermissionsPolicy frame_policy = { { { mojom::PermissionsPolicyFeature::kNotFound, /*allowed_origins=*/ {}, false, true },
...
TEST_F(PermissionsPolicyTest, ProposedTestImplicitPolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentPolicy(policy1.get(), origin_b_);
    EXPECT_TRUE(policy3->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_FALSE(policy3->IsFeatureEnabled(kDefaultSelfFeature));
}
...
TEST_F(PermissionsPolicyTest, ProposedTestCompletelyBlockedPolicy)
{
    ...
    std::unique_ptr<PermissionsPolicy> policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    policy1->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/ {}, false, false } } });
    std::unique_ptr<PermissionsPolicy> policy2 = CreateFromParentPolicy(policy1.get(), origin_a_);
    EXPECT_FALSE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
    ...
    ParsedPermissionsPolicy frame_policy4 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy4 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy4, origin_b_);
    EXPECT_FALSE(policy4->IsFeatureEnabled(kDefaultSelfFeature));
    ...
    ParsedPermissionsPolicy frame_policy5 = { { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } };
    std::unique_ptr<PermissionsPolicy> policy5 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy5, origin_b_);
    EXPECT_FALSE(policy5->IsFeatureEnabled(kDefaultSelfFeature));
    ...
    ParsedPermissionsPolicy frame_policy6 = { { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_c_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } };
    std::unique_ptr<PermissionsPolicy> policy6 = CreateFromParentWithFramePolicy(policy1.get(), frame_policy6, origin_b_);
...
TEST_F(PermissionsPolicyTest, ProposedTestNestedPolicyPropagates)
{
    ...
    ParsedPermissionsPolicy frame_policy3 = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, true, false } } };
    std::unique_ptr<PermissionsPolicy> policy3 = CreateFromParentWithFramePolicy(policy2.get(), frame_policy3, origin_b_);
    EXPECT_FALSE(policy3->IsFeatureEnabled(kDefaultSelfFeature));
}

TEST_F(PermissionsPolicyTest, CreateForDefaultFencedFrame)
{
    std::unique_ptr<PermissionsPolicy> policy = CreateForFencedFrame(origin_a_, blink::mojom::FencedFrameMode::kDefault);
    ...
    EXPECT_FALSE(policy->IsFeatureEnabled(mojom::PermissionsPolicyFeature::kSharedStorage));
}

TEST_F(PermissionsPolicyTest, CreateForOpaqueFencedFrame)
{
    std::unique_ptr<PermissionsPolicy> policy = CreateForFencedFrame(origin_a_, blink::mojom::FencedFrameMode::kOpaqueAds);
    EXPECT_FALSE(policy->IsFeatureEnabled(kDefaultOnFeature));
    EXPECT_FALSE(policy->IsFeatureEnabled(kDefaultSelfFeature));
...
{
    ParsedPermissionsPolicy parsed_policy = { { { kDefaultSelfFeature, /*allowed_origins=*/ {}, false, false } } };
    auto policy = CreateFromParsedPolicy(parsed_policy, origin_a_);
    EXPECT_FALSE(policy->IsFeatureEnabled(kDefaultSelfFeature));
}
...
TEST_F(PermissionsPolicyTest, SetHeaderPolicy)
{
    ...
    EXPECT_TRUE(policy2->IsFeatureEnabled(kDefaultSelfFeature));
    EXPECT_DCHECK_DEATH(policy2->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } }));
    ...
    policy3->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    EXPECT_DCHECK_DEATH(policy3->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } }));
    ...
    EXPECT_DCHECK_DEATH(policy4->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kNotFound, {}, false, false } } }));
}
...
TEST_F(PermissionsPolicyTest, OverwriteHeaderPolicyForClientHints)
{
    ...
    policy1->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        ...
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    policy1->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    ...
    policy2->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        ...
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    policy2->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    ...
    policy3->SetHeaderPolicy({ { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    policy3->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    ...
    auto policy4 = CreateFromParentPolicy(nullptr, origin_a_);
    EXPECT_DCHECK_DEATH(policy4->OverwriteHeaderPolicyForClientHints({ { { kDefaultSelfFeature, /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } }));
    ...
    policy5->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        ...
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } });
    EXPECT_TRUE(policy5->IsFeatureEnabled(mojom::PermissionsPolicyFeature::kClientHintDPR));
    EXPECT_DCHECK_DEATH(policy5->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR,
        /*allowed_origins=*/
        { blink::OriginWithPossibleWildcards(origin_a_,
            /*has_subdomain_wildcard=*/false) },
        false, false } } }));
...
TEST_F(PermissionsPolicyTest, GetAllowlistForFeatureIfExists)
{
    ...
    auto policy1 = CreateFromParentPolicy(nullptr, origin_a_);
    const std::vector<blink::OriginWithPossibleWildcards> origins1({ blink::OriginWithPossibleWildcards(origin_b_,
        /*has_subdomain_wildcard=*/false) });
    policy1->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kClientHintDPR, origins1, false, false } } });
    ...
    auto policy3 = CreateFromParentPolicy(nullptr, origin_a_);
    const std::vector<blink::OriginWithPossibleWildcards> origins3({ blink::OriginWithPossibleWildcards(origin_a_,
        /*has_subdomain_wildcard=*/false) });
    policy3->SetHeaderPolicy({ { { mojom::PermissionsPolicyFeature::kClientHintDPR, {}, false, false } } });
    policy3->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR, origins3, false, false } } });
    const auto& maybe_allow_list3 = policy3->GetAllowlistForFeatureIfExists(mojom::PermissionsPolicyFeature::kClientHintDPR);
    ...
    auto policy4 = CreateFromParentPolicy(nullptr, origin_a_);
    const std::vector<blink::OriginWithPossibleWildcards> origins4({ blink::OriginWithPossibleWildcards(origin_a_,
                                                                         /*has_subdomain_wildcard=*/false),
        blink::OriginWithPossibleWildcards(origin_b_,
            /*has_subdomain_wildcard=*/false) });
    policy4->OverwriteHeaderPolicyForClientHints({ { { mojom::PermissionsPolicyFeature::kClientHintDPR, origins4, false, false } } });
