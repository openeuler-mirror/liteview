...
//#include "third_party/openssl/openssl/include/openssl/curve25519.h"
#include "url/gurl.h"
...
// static
std::unique_ptr<TrialToken> TrialToken::From(base::StringPiece token_text, const OriginTrialPublicKey& public_key, OriginTrialTokenStatus* out_status)
{
    DCHECK(out_status);
...
// static
OriginTrialTokenStatus TrialToken::Extract(base::StringPiece token_text, const OriginTrialPublicKey& public_key, std::string* out_token_payload,
    std::string* out_token_signature, uint8_t* out_token_version)
{
    ...
    base::ReadBigEndian(reinterpret_cast<const uint8_t*>(&(token_contents[kPayloadLengthOffset])), &payload_length);
    ...
    base::StringPiece payload_piece(token_bytes + kPayloadLengthOffset, kPayloadLengthSize + payload_length);
...
std::unique_ptr<TrialToken> TrialToken::Parse(const std::string& token_payload, const uint8_t version)
{
    ...
    return base::WrapUnique(new TrialToken(origin, is_subdomain, *feature_name, base::Time::FromDoubleT(expiry_timestamp), is_third_party, usage));
}
...
// static
bool TrialToken::ValidateSignature(base::StringPiece signature, const std::string& data, const OriginTrialPublicKey& public_key)
{
...
std::unique_ptr<TrialToken> TrialToken::CreateTrialTokenForTesting(const url::Origin& origin, bool match_subdomains, const std::string& feature_name,
    base::Time expiry_time, bool is_third_party, UsageRestriction usage_restriction)
{
    return base::WrapUnique(new TrialToken(origin, match_subdomains, feature_name, expiry_time, is_third_party, usage_restriction));
}
