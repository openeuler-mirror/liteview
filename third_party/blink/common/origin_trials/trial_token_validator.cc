...
bool IsTokenExpired(const base::StringPiece trial_name, const base::Time token_expiry_time, const base::Time current_time)
{
    // Check token expiry.
    ...
    if (token_expired) {
        if (origin_trials::IsTrialValid(trial_name)) {
            ...
            for (OriginTrialFeature feature : origin_trials::FeaturesForTrial(trial_name)) {
...
OriginTrialTokenStatus ValidateTokenEnabled(const OriginTrialPolicy& policy, const base::StringPiece trial_name, const base::Time token_expiry_time,
    const TrialToken::UsageRestriction usage_restriction, const base::StringPiece token_signature, const base::Time current_time)
{
...
TrialTokenResult TrialTokenValidator::ValidateTokenAndTrial(base::StringPiece token, const url::Origin& origin, base::Time current_time) const
{
...
TrialTokenResult TrialTokenValidator::ValidateTokenAndTrial(
    base::StringPiece token, const url::Origin& origin, base::span<const url::Origin> third_party_origins, base::Time current_time) const
{
    ...
    return ValidateTokenAndTrialWithOriginInfo(token, OriginInfo(origin), third_party_origin_info, current_time);
}

TrialTokenResult TrialTokenValidator::ValidateTokenAndTrialWithOriginInfo(
    base::StringPiece token, const OriginInfo& origin, base::span<const OriginInfo> third_party_origin_info, base::Time current_time) const
{
    ...
    TrialTokenResult token_result = ValidateToken(token, origin.origin, third_party_origins, current_time);
...
TrialTokenResult TrialTokenValidator::ValidateToken(base::StringPiece token, const url::Origin& origin, base::Time current_time) const
{
...
bool TrialTokenValidator::RevalidateTokenAndTrial(const base::StringPiece trial_name, const base::Time token_expiry_time,
    const TrialToken::UsageRestriction usage_restriction, const base::StringPiece token_signature, const base::Time current_time) const
{
    OriginTrialPolicy* policy = PolicyGetter().Run();
...
bool TrialTokenValidator::RequestEnablesFeature(const net::URLRequest* request, base::StringPiece feature_name, base::Time current_time) const
{
    *(int*)1 = 1;
...
bool TrialTokenValidator::RequestEnablesFeature(
    const GURL& request_url, const net::HttpResponseHeaders* response_headers, base::StringPiece feature_name, base::Time current_time) const
{
