    ...
    v8::Isolate* isolate = v8::Isolate::New(params);
    {
        ...
        if (const auto* str = absl::get_if<std::u16string>(&message)) {
            v8::Local<v8::String> message_as_value
                = v8::String::NewFromTwoByte(isolate, reinterpret_cast<const uint16_t*>(str->data()), v8::NewStringType::kNormal, str->size()).ToLocalChecked();
            EXPECT_TRUE(serializer.WriteValue(context, message_as_value).ToChecked());
        } else if (const auto* array_buffer = absl::get_if<std::vector<uint8_t>>(&message)) {
            v8::Local<v8::ArrayBuffer> message_as_array_buffer = v8::ArrayBuffer::New(isolate, array_buffer->size());
            memcpy(message_as_array_buffer->GetBackingStore()->Data(), array_buffer->data(), array_buffer->size());
            if (transferable) {
            ...
            transferable_message.array_buffer_contents_array.push_back(mojom::SerializedArrayBufferContents::New(std::move(big_buffer)));
        } else {
            NOTREACHED();
        }
...
void CheckStringEQ(const absl::optional<WebMessagePayload>& optional_payload, const std::u16string& str)
{
...
void CheckVectorEQ(const absl::optional<WebMessagePayload>& optional_payload, const std::vector<uint8_t>& buffer)
{
    EXPECT_TRUE(optional_payload);
    auto& payload = optional_payload.value();
...
TEST(StringMessageCodecTest, SelfTest_ArrayBuffer)
{
    ...
    CheckVectorEQ(DecodeToWebMessagePayload(EncodeWebMessagePayload(WebMessagePayload(message))), message);
}
...
{
    std::vector<uint8_t> message(200, 0xFF);
    CheckVectorEQ(DecodeWithV8(EncodeWebMessagePayload(WebMessagePayload(message))), message);
}
...
{
    std::vector<uint8_t> message(200, 0xFF);
    CheckVectorEQ(DecodeToWebMessagePayload(EncodeWithV8(message, true)), message);
}
