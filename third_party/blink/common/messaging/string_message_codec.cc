...
void WriteBytes(const char* bytes, size_t num_bytes, std::vector<uint8_t>* buffer)
{
...
TransferableMessage EncodeWebMessagePayload(const WebMessagePayload& payload)
{
    ...
    absl::visit(overloaded { [&](const std::u16string& str) {
                    ...
                    [&](const std::vector<uint8_t>& array_buffer) {
                        ...
                        message.array_buffer_contents_array.push_back(mojom::SerializedArrayBufferContents::New(std::move(big_buffer)));
                    } },
        payload);
...
absl::optional<WebMessagePayload> DecodeToWebMessagePayload(const TransferableMessage& message)
{
    ...
    case kOneByteStringTag: {
        ...
        return span.size_bytes() == num_bytes ? absl::make_optional(WebMessagePayload(std::move(str))) : absl::nullopt;
    }
    ...
    case kArrayBuffer: {
        ...
        if (!ReadUint32(iter, &num_bytes))
            return absl::nullopt;
        base::span<const uint8_t> span = iter.Span<uint8_t>(num_bytes);
        return span.size_bytes() == num_bytes ? absl::make_optional(WebMessagePayload(std::vector<uint8_t>(span.begin(), span.end()))) : absl::nullopt;
    }
    case kArrayBufferTransferTag: {
        ...
        if (message.array_buffer_contents_array.size() != 1)
            return absl::nullopt;
        const ::mojo_base::BigBuffer& big_buffer = message.array_buffer_contents_array[0]->contents;
        // Data is from renderer process, copy it first before use.
        return std::vector<uint8_t>(big_buffer.data(), big_buffer.data() + big_buffer.size());
    }
    }
