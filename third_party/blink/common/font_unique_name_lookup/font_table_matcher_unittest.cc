...
TEST_F(FontTableMatcherTest, CaseInsensitiveMatchingBothNames)
{
    ...
    absl::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName("font name uppercase");
...
TEST_F(FontTableMatcherTest, MatchTamilChinese)
{
    ...
    for (std::string font_name : { "எழுத்துரு பெயர்", "எழுத்துரு-பெயர்", "字體名稱" }) {
        absl::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName(font_name);
        ASSERT_TRUE(result.has_value());
        ASSERT_EQ(result->font_path, kTestFilePath1);
        ASSERT_EQ(result->ttc_index, 0u);

        absl::optional<FontTableMatcher::MatchResult> result_for_substring = matcher_->MatchName(font_name.substr(0, font_name.size() - 2u));
...
{
    ASSERT_EQ(matcher_->AvailableFonts(), 1u);
    absl::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName("font name");
