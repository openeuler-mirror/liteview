...
bool LoadFromFile(base::FilePath file_path, base::MappedReadOnlyRegion* name_table_region)
{
    ...
    *name_table_region = base::MappedReadOnlyRegion();
    std::vector<char> file_contents;
    {
        ...
        if (table_cache_file.Read(0, file_contents.data(), file_contents.size()) <= 0) {
            return false;
        }
    }

    base::PickleIterator pickle_iterator(base::Pickle(file_contents.data(), file_contents.size()));
    ...
    if (!pickle_iterator.ReadData(&proto_data, &proto_length) || !proto_data || proto_length == 0) {
        return false;
    }
...
bool PersistToFile(const base::MappedReadOnlyRegion& name_table_region, base::FilePath file_path)
{
    ...
    base::Pickle pickle;
    uint32_t checksum = base::PersistentHash(name_table_region.mapping.memory(), name_table_region.mapping.size());
    pickle.WriteUInt32(checksum);
    pickle.WriteData(static_cast<char*>(name_table_region.mapping.memory()), name_table_region.mapping.size());
    DCHECK(pickle.size());
    {
        base::ScopedBlockingCall scoped_blocking_call(FROM_HERE, base::BlockingType::MAY_BLOCK);

        if (table_cache_file.Write(0, static_cast<const char*>(pickle.data()), pickle.size()) == -1) {
