...
{
    if (HandlePropertyChange<bool>(on_battery_power, reporting_timer_.IsRunning(), power_component_.get()) == PropertyAction::kFinalizeRequired) {
...
void WatchTimeReporter::RecordWatchTime()
{
    ...
    if (!pending_underflow_events_.empty()) {
        ...
        base::EraseIf(pending_underflow_events_, [](const UnderflowEvent& ufe) { return ufe.reported && ufe.duration != media::kNoTimestamp; });
