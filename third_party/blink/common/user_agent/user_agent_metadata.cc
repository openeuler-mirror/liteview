    ...
    for (const UserAgentBrandVersion& brand_version : ua_brand_version_list) {
        if (brand_version.version.empty()) {
            brand_version_header.push_back(
                net::structured_headers::ParameterizedMember(net::structured_headers::Item(brand_version.brand), {}));
        } else {
            brand_version_header.push_back(
                net::structured_headers::ParameterizedMember(
                    net::structured_headers::Item(brand_version.brand),
                    { std::make_pair("v", net::structured_headers::Item(brand_version.version)) }));
        }
    }
...
// static
absl::optional<std::string> UserAgentMetadata::Marshal(const absl::optional<UserAgentMetadata>& in)
{
    if (!in)
        return absl::nullopt;
    base::Pickle out;
...
// static
absl::optional<UserAgentMetadata> UserAgentMetadata::Demarshal(const absl::optional<std::string>& encoded)
{
    if (!encoded)
        return absl::nullopt;
