...
public:
    ...
    void PopulateHttpBody(ExplodedHttpBody* http_body, std::vector<absl::optional<std::u16string>>* referenced_files)
    {
        ...
        http_body->request_body->AppendFileRange(base::FilePath(path), 100, 1024, base::Time::FromDoubleT(9999.0));
    ...
    void PopulateFrameStateForBackwardsCompatTest(ExplodedFrameState* frame_state, bool is_child, int version)
    {
        ...
        if (!is_child) {
            ...
            frame_state->http_body.request_body->AppendFileRange(
                base::FilePath(FILE_PATH_LITERAL("file.txt")), 0, std::numeric_limits<uint64_t>::max(), base::Time::FromDoubleT(0.0));
