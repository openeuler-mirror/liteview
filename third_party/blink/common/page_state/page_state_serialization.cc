...
void AppendFileRangeToRequestBody(const scoped_refptr<network::ResourceRequestBody>& request_body, const absl::optional<std::u16string>& file_path,
...
void AppendReferencedFilesFromHttpBody(const std::vector<network::DataElement>& elements, std::vector<absl::optional<std::u16string>>* referenced_files)
{
...
bool AppendReferencedFilesFromDocumentState(
    const std::vector<absl::optional<std::u16string>>& document_state, std::vector<absl::optional<std::u16string>>* referenced_files)
{
...
bool RecursivelyAppendReferencedFiles(const ExplodedFrameState& frame_state, std::vector<absl::optional<std::u16string>>* referenced_files)
{
...
{
    obj->pickle.WriteData(reinterpret_cast<const char*>(str.data()), str.length() * sizeof(char16_t));
}

// If str is a null optional, this simply pickles a length of -1. Otherwise,
// delegates to the std::u16string overload.
void WriteString(const absl::optional<std::u16string>& str, SerializeObject* obj)
{
    if (!str) {
...
void WriteStringVector(const std::vector<absl::optional<std::u16string>>& data, SerializeObject* obj)
{
...
void ReadStringVector(SerializeObject* obj, std::vector<absl::optional<std::u16string>>* result)
{
    size_t num_elements = ReadAndValidateVectorSize(obj, sizeof(absl::optional<std::u16string>));
...
void ReadResourceRequestBody(SerializeObject* obj, const scoped_refptr<network::ResourceRequestBody>& request_body)
{
    ...
    for (int i = 0; i < num_elements; ++i) {
        ...
        if (type == HTTPBodyElementType::kTypeData) {
            ...
            if (!obj->parse_error) {
                AppendDataToRequestBody(request_body, static_cast<const char*>(data), length);
            }
            ...
            AppendFileRangeToRequestBody(request_body, file_path, file_start, file_length, base::Time::FromDoubleT(file_modification_time));
...
void ReadFrameState(SerializeObject* obj, bool is_top, std::vector<UniqueNameHelper::Replacement>* unique_name_replacements, ExplodedFrameState* state)
{
...
void WriteFrameState(const ExplodedFrameState& state, SerializeObject* obj, bool is_top)
{
...
void WriteResourceRequestBody(const network::ResourceRequestBody& request_body, mojom::RequestBody* mojo_body)
{
    for (const auto& element : *request_body.elements()) {
        ...
        case network::DataElement::Tag::kBytes: {
            ...
            data_element = mojom::Element::NewBytes(std::vector<unsigned char>(data, data + bytes.size()));
            break;
        }
        case network::DataElement::Tag::kFile: {
...
void WriteFrameState(const ExplodedFrameState& state, mojom::FrameState* frame)
{
...
void WriteMojoPageState(const ExplodedPageState& state, SerializeObject* obj)
{
    ...
    obj->pickle.WriteData(reinterpret_cast<char*>(page_bytes.data()), page_bytes.size());
}
...
scoped_refptr<network::ResourceRequestBody> DecodeResourceRequestBody(const char* data, size_t size)
{
