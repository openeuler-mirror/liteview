...
void AddHeader(net::HttpResponseHeaders* response_headers, const std::string& header, const std::string& value)
{
    response_headers->AddHeader(header, value);
}

void VerifyClientHintEnabledWithOriginTrialTokenInner(net::HttpResponseHeaders* response_headers, const std::string& token, const GURL* third_party_url,
    const WebClientHintsType client_hint_type, bool expected_client_hint_enabled)
{
    ...
    hints.SetIsEnabled(GURL(kOriginUrl), maybe_third_party_url, response_headers, client_hint_type, true);
    EXPECT_TRUE(hints.IsEnabled(client_hint_type) == expected_client_hint_enabled);
}
...
public:
        ...
        : response_headers_(base::MakeRefCounted<net::HttpResponseHeaders>(""))
    {
        ...
        scoped_feature_list_.InitWithFeatures(
            ...
            /*disabled_features=*/ { blink::features::kPrefersColorSchemeClientHintHeader });
    ...
    void VerifyClientHintEnabledWithOriginTrialToken(
        const std::string& token, const GURL* third_party_url, const WebClientHintsType client_hint_type, bool expected_client_hint_enabled)
    {
        VerifyClientHintEnabledWithOriginTrialTokenInner(response_headers_.get(), token, third_party_url, client_hint_type, expected_client_hint_enabled);
    }
...
TEST_F(EnabledClientHintsTest, EnabledUaReducedClientHintWithValidOriginTrialToken)
{
    ...
    static constexpr char kValidOriginTrialToken[] = "A93QtcQ0CRKf5ioPasUwNbweXQWgbI4ZEshiz+"
    ...
    VerifyClientHintEnabledWithOriginTrialToken(kValidOriginTrialToken,
...
TEST_F(EnabledClientHintsTest, EnabledUaReducedClientHintWithInvalidOriginTrialToken)
{
    ...
    static constexpr char kInvalidOriginTrialToken[] = "A93QtcQ0CRKf5ioPasUwNbweXQWgbI4ZEshiz+"
                                                       "YS7dkQEWVfW9Ua2pTnA866sZwRzuElkPwsUdGdIaW0fRUP8AwAAABceyJvcmlnaW4iOiAiaH"
                                                       "R0"
                                                       "cHM6Ly8xMjcuMC4wLjE6NDQ0NDQiLCAiZmVhdHVyzSI6ICJVc2VyQWdlbnRSZWR1Y3Rpb24i"
                                                       "LC"
    ...
    VerifyClientHintEnabledWithOriginTrialToken(kInvalidOriginTrialToken,
        /*third_party_url=*/nullptr, WebClientHintsType::kUAReduced,
        /*expected_client_hint_enabled=*/false);
}

TEST_F(EnabledClientHintsTest, EnabledUaReducedClientHintWithValidThirdPartyOriginTrialToken)
{
    // Generated by running (in tools/origin_trials):
    ...
    // The Origin Trial token expires in 2033.  Generate a new token by then, or
    // find a better way to re-generate a test trial token.
    static constexpr char kValidThirdPartyOriginTrialToken[] = "A4lFNHY7SM+O+"
    ...
    VerifyClientHintEnabledWithOriginTrialToken(
        kValidThirdPartyOriginTrialToken, &third_party_url, WebClientHintsType::kUAReduced, /*expected_client_hint_enabled=*/true);
}

TEST_F(EnabledClientHintsTest, EnabledUaReducedClientHintThirdPartyWithValidOriginTrialToken)
{
    // Generated by running (in tools/origin_trials):
    // generate_token.py https://127.0.0.1:44445 UserAgentReduction
    // --expire-timestamp=2000000000
    ...
    // The Origin Trial token expires in 2033.  Generate a new token by then, or
    // find a better way to re-generate a test trial token.
    static constexpr char kValidOriginTrialTokenForThirdPartyUrl[] = "Axn1AIzTAEaYasceEQyucSkZpDyq6bH4NxYHECXpSXr9LBG22yjc8NXkRZWbiWzTIrjEtHkm"
    ...
    const GURL third_party_url = GURL(kThirdPartyOriginUrl);
    VerifyClientHintEnabledWithOriginTrialToken(
        kValidOriginTrialTokenForThirdPartyUrl, &third_party_url, WebClientHintsType::kUAReduced, /*expected_client_hint_enabled=*/false);
}

TEST_F(EnabledClientHintsTest, EnabledUADeprecationClientHintWithValidOriginTrialToken)
{
    // Generated by running (in tools/origin_trials):
    ...
    // The Origin Trial token expires in 2033.  Generate a new token by then, or
    // find a better way to re-generate a test trial token.
    static constexpr char kValidOriginTrialToken[] = "A6+Ti/9KuXTgmFzOQwkTuO8k0QFH8vUaxmv0CllAET1/"
...
TEST_F(EnabledClientHintsTest, EnabledUADeprecationClientHintWithInvalidOriginTrialToken)
{
    // A slight corruption (changing a character) of a valid OT token.
    static constexpr char kInvalidOriginTrialToken[] = "A6+Ti/9KuXTgmFzOQwkTuO8k0QFH8vUaxmv0CllAET1/"
                                                       "307KShF6fhskMuBqFUvqO7ViAkZ+"
                                                       "NSeJhQI0n5aLggsAAABpeyJvcmlnaW4iOiAiaHR0cHM6Ly8xMjcuMC4wLjE6NDQ0NDQiLCAi"
...
TEST_F(EnabledClientHintsTest, EnabledUADeprecationClientHintWithValidThirdPartyOriginTrialToken)
{
    // Generated by running (in tools/origin_trials):
    ...
    // The Origin Trial token expires in 2033.  Generate a new token by then, or
    // find a better way to re-generate a test trial token.
    static constexpr char kValidThirdPartyOriginTrialToken[] = "A0q1jQxOoBMkORITt4dborMF2TE0MVT71JbLomfT4tg8nKuEiRcDNTLVEfSffhxcwqMYEmXs"
                                                               "p4CXXypHENjrvwgAAAB/"
    ...
    const GURL third_party_url = GURL(kThirdPartyOriginUrl);
    VerifyClientHintEnabledWithOriginTrialToken(kValidThirdPartyOriginTrialToken, &third_party_url, WebClientHintsType::kFullUserAgent,
        /*expected_client_hint_enabled=*/true);
}

TEST_F(EnabledClientHintsTest, EnabledUADeprecationClientHintThirdPartyWithValidOriginTrialToken)
{
    // Generated by running (in tools/origin_trials):
    // generate_token.py https://127.0.0.1:44445 SendFullUserAgentAfterReduction
    // --expire-timestamp=2000000000
    ...
    // The Origin Trial token expires in 2033.  Generate a new token by then, or
    // find a better way to re-generate a test trial token.
    static constexpr char kValidOriginTrialTokenForThirdPartyUrl[] = "A0kGFcySC9Pfb0ouX/"
    ...
    const GURL third_party_url = GURL(kThirdPartyOriginUrl);
    VerifyClientHintEnabledWithOriginTrialToken(kValidOriginTrialTokenForThirdPartyUrl, &third_party_url, WebClientHintsType::kFullUserAgent,
        /*expected_client_hint_enabled=*/false);
}
