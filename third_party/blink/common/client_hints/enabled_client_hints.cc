...
bool IsDisabledByFeature(const WebClientHintsType type)
{
    ...
    case WebClientHintsType::kPrefersColorScheme:
        if (!base::FeatureList::IsEnabled(features::kPrefersColorSchemeClientHintHeader)) {
            return true;
        }
        break;
    case WebClientHintsType::kViewportHeight:
        if (!base::FeatureList::IsEnabled(features::kViewportHeightClientHintHeader)) {
            return true;
        }
        break;
    ...
    case WebClientHintsType::kPrefersReducedMotion:
        if (!base::FeatureList::IsEnabled(features::kPrefersReducedMotionClientHintHeader)) {
            return true;
        }
        break;
    default:
...
bool IsOriginTrialEnabled(
    const GURL& url, const absl::optional<GURL>& third_party_url, const net::HttpResponseHeaders* response_headers, base::StringPiece feature_name)
{
    ...
    if (!third_party_url) {
        ...
        // token as normal.
        return validator.RequestEnablesFeature(url, response_headers, feature_name, now);
    }
    ...
    if (validator.IsTrialPossibleOnOrigin(*third_party_url)) {
        ...
        while (response_headers->EnumerateHeader(&iter, "Origin-Trial", &token)) {
            blink::TrialTokenResult result = validator.ValidateToken(token, origin, third_party_origins, now);
...
void EnabledClientHints::SetIsEnabled(const GURL& url, const absl::optional<GURL>& third_party_url, const net::HttpResponseHeaders* response_headers,
    const network::mojom::WebClientHintsType type, const bool should_send)
{
    bool enabled = should_send;
    if (enabled && type == WebClientHintsType::kUAReduced) {
        enabled = IsOriginTrialEnabled(url, third_party_url, response_headers, "UserAgentReduction");
    }
    if (enabled && type == WebClientHintsType::kFullUserAgent) {
        enabled = IsOriginTrialEnabled(url, third_party_url, response_headers, "SendFullUserAgentAfterReduction");
    }
