...
extern "C" JNIEXPORT jlong JNICALL Java_org_skia_skottie_SkottieRunner_nCreateProxy(JNIEnv* env, jclass clazz)
{
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_skottie_SkottieRunner_nDeleteProxy(JNIEnv* env, jclass clazz, jlong nativeProxy)
{
    if (!nativeProxy) {
        return;
    }
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_skottie_SkottieRunner_nSetMaxCacheSize(JNIEnv* env, jclass clazz, jint maxCacheSize, jlong nativeProxy)
{
    if (!nativeProxy) {
        return;
    }
...
extern "C" JNIEXPORT jlong JNICALL Java_org_skia_skottie_SkottieAnimation_nCreateProxy(JNIEnv* env, jobject clazz, jlong runner, jobject bufferObj)
{
    ...
    sk_sp<SkData> data(SkData::MakeWithProc(buffer, bufferSize, release_global_jni_ref, reinterpret_cast<void*>(bufferRef)));
    ...
    skottieAnimation->mAnimation
        = skottie::Animation::Builder().setResourceProvider(skresources::DataURIResourceProviderProxy::Make(nullptr)).make(skottieAnimation->mStream.get());
...
extern "C" JNIEXPORT void JNICALL Java_org_skia_skottie_SkottieAnimation_nDeleteProxy(JNIEnv* env, jclass clazz, jlong nativeProxy)
{
    if (!nativeProxy) {
        return;
    }
...
extern "C" JNIEXPORT bool JNICALL Java_org_skia_skottie_SkottieAnimation_nDrawFrame(
    JNIEnv* env, jclass clazz, jlong nativeProxy, jint width, jint height, jboolean wideColorGamut, jfloat progress, jint backgroundColor, jboolean forceDraw)
{
    ...
    sk_sp<SkSurface> renderTarget(SkSurface::MakeFromBackendRenderTarget(dContext, backendRT, kBottomLeft_GrSurfaceOrigin, colorType, nullptr, &props));
...
extern "C" JNIEXPORT jlong JNICALL Java_org_skia_skottie_SkottieAnimation_nGetDuration(JNIEnv* env, jclass clazz, jlong nativeProxy)
{
    if (!nativeProxy) {
        return 0;
    }
