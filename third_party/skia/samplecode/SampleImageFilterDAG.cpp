...
private:
    void computeOutputBounds()
    {
        if (fFilter) {
            ...
            skif::DeviceSpace<SkIRect> pseudoDeviceBounds = as_IFB(fFilter)->getOutputBounds(layerOnly, fContent);
...
static void draw_dag(SkCanvas* canvas, sk_sp<SkImageFilter> filter, const SkRect& rect, const SkISize& surfaceSize)
{
...
public:
    ...
    void onDrawContent(SkCanvas* canvas) override
    {
        ...
        static const SkISize kFilterSurfaceSize = SkISize::Make(2 * (kFilterRect.fRight + kFilterRect.fLeft), 2 * (kFilterRect.fBottom + kFilterRect.fTop));
    ...
    {
        return SkString("ImageFilterDAG");
    }
