...
struct SBIXView : public Sample {
    ...
    sk_sp<SkData> updateSBIXData(SkData* originalData, bool setPts)
    {
        ...
        if (glyphData) {
            if (setPts) {
                fPts[0].set((int16_t)SkEndian_SwapBE16(glyphData->xMin) / toEm, (int16_t)SkEndian_SwapBE16(glyphData->yMin) / -toEm);
                fPts[1].set((int16_t)SkEndian_SwapBE16(glyphData->xMax) / toEm, (int16_t)SkEndian_SwapBE16(glyphData->yMax) / -toEm);
            } else {
