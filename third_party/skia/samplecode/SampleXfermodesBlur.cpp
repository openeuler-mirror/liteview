...
class XfermodesBlurView : public Sample {
    ...
    {
        SkPaint p;
        p.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(5)));
    ...
    void onDrawContent(SkCanvas* canvas) override
    {
        ...
        for (int twice = 0; twice < 2; twice++) {
            ...
            for (size_t i = 0; i < std::size(gModes); i++) {
                ...
                SkTextUtils::DrawString(canvas, label, x + w / 2, y - font.getSize() / 2, font, SkPaint(), SkTextUtils::kCenter_Align);
