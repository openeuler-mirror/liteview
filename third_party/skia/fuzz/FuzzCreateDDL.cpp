...
static SkSurfaceCharacterization make_characterization(Fuzz* fuzz, GrDirectContext* dContext, SkImageInfo& ii, SkColorType surfaceType, GrSurfaceOrigin origin)
{
    ...
    c = dContext->threadSafeProxy()->createCharacterization(
        maxResourceBytes, ii, backendFormat, kSampleCount, origin, gen_fuzzed_surface_props(fuzz), true, false, true, protect);
    if (!c.isValid()) {
...
static sk_sp<SkDeferredDisplayList> make_ddl(Fuzz* fuzz, GrDirectContext* dContext, const SkSurfaceCharacterization& c)
{
...
static sk_sp<SkSurface> make_surface(Fuzz* fuzz, GrDirectContext* dContext, const SkImageInfo& ii, GrSurfaceOrigin origin)
{
    ...
    auto surface = SkSurface::MakeRenderTarget(dContext, budgeted, ii, kSampleCount, origin, &surfaceProps);
    return surface;
}
