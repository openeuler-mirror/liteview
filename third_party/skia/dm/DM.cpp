...
static DEFINE_bool(runVerifiers, false, "if true, run SkQP-style verification of GM-produced images.");

#if defined(__MSVC_RUNTIME_CHECKS)
#include <rtcapi.h>
int RuntimeCheckErrorFunc(int errorType, const char* filename, int linenumber, const char* moduleName, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
struct Task {
    ...
    static void RunGMVerifiers(const Task& task, const SkBitmap& actualBitmap)
    {
        ...
        skiagm::verifiers::VerifierResult res = verifierList->verifyAll(task.sink->colorInfo(), actualBitmap);
        if (!res.ok()) {
            fail(SkStringPrintf("%s %s %s %s: verifier failed: %s", task.sink.tag.c_str(), task.src.tag.c_str(), task.src.options.c_str(), name.c_str(),
                res.message().c_str()));
        }
    }
};
...
int main(int argc, char** argv)
{
    ...
    info("%d srcs * %d sinks + %d tests == %d tasks\n", gSrcs->count(), gSinks->count(), testCount, gPending);
