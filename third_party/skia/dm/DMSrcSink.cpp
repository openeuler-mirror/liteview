...
static DEFINE_bool(multiPage, false, "For document-type backends, render the source into multiple pages");
...
Result BisectSrc::draw(GrDirectContext* context, SkCanvas* canvas) const
{
    ...
    public:
        ...
        const SkTArray<FoundPath>& foundPaths() const
        {
            return fFoundPaths;
        }
...
{
    auto resource_provider = skresources::DataURIResourceProviderProxy::Make(skresources::FileResourceProvider::Make(SkOSPath::Dirname(fPath.c_str()),
                                                                                 /*predecode=*/true),
        /*predecode=*/true);
    ...
    , fScale(1)
{
    ...
    auto rp = skresources::DataURIResourceProviderProxy::Make(skresources::FileResourceProvider::Make(SkOSPath::Dirname(path.c_str()),
                                                                  /*predecode=*/true),
        /*predecode=*/true);
...
sk_sp<SkSurface> GPUSink::createDstSurface(GrDirectContext* context, SkISize size) const
{
    ...
    case SkCommandLineConfigGpu::SurfType::kBackendTexture:
        surface = sk_gpu_test::MakeBackendTextureSurface(context, info, kTopLeft_GrSurfaceOrigin, fSampleCount, GrMipmapped::kNo, GrProtected::kNo, &props);
        break;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
GPUThreadTestingSink::GPUThreadTestingSink(const SkCommandLineConfigGpu* config, const GrContextOptions& grCtxOptions)
...
Result GPUThreadTestingSink::draw(const Src& src, SkBitmap* dst, SkWStream* wStream, SkString* log) const
{
...
Result GPUOOPRSink::ooprDraw(const Src& src, sk_sp<SkSurface> dstSurface, GrDirectContext* context) const
{
    ...
    sk_gpu_test::TestContext* gpuTestCtx, GrDirectContext* dContext) const
{
    ...
    Result result = src.draw(dContext, recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
    if (!result.isOk()) {
        return result;
    }
...
Result SVGSink::draw(const Src& src, SkBitmap*, SkWStream* dst, SkString*) const
{
    ...
    return src.draw(fPageIndex, nullptr, SkSVGCanvas::Make(SkRect::MakeWH(SkIntToScalar(src.size().width()), SkIntToScalar(src.size().height())), dst).get());
#else
    (void)fPageIndex;
...
Result RasterSink::draw(const Src& src, SkBitmap* dst, SkWStream*, SkString*) const
{
    ...
    dst->allocPixelsFlags(SkImageInfo::Make(size, fColorType, alphaType, fColorSpace), SkBitmap::kZeroPixels_AllocFlag);
...
Result GraphiteSink::draw(const Src& src, SkBitmap* dst, SkWStream* dstStream, SkString* log) const
{
...

    {
        ...
        // that instead.
        SkPixmap pm;
        if (!dst->peekPixels(&pm) || !static_cast<skgpu::graphite::Surface*>(surface.get())->onReadPixels(context, recorder.get(), pm, 0, 0)) {
            return Result::Fatal("Could not readback from surface.");
        }
    }
...
Result ViaMatrix::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    ...
    return draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](GrDirectContext* context, SkCanvas* canvas) {
...
Result ViaSerialization::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    ...
    auto size = src.size();
    SkPictureRecorder recorder;
    Result result = src.draw(nullptr, recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
    if (!result.isOk()) {
        return result;
    }
    ...
    result = draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](GrDirectContext*, SkCanvas* canvas) {
...
{
    auto size = src.size();
    Result result = draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](GrDirectContext* context, SkCanvas* canvas) {
        SkPictureRecorder recorder;
        sk_sp<SkPicture> pic;
        Result result = src.draw(context, recorder.beginRecording(SkIntToScalar(size.width()), SkIntToScalar(size.height())));
        if (!result.isOk()) {
            return result;
        }
...
Result ViaRuntimeBlend::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const
{
    ...
    return draw_to_canvas(fSink.get(), bitmap, stream, log, src.size(), [&](GrDirectContext* context, SkCanvas* canvas) {
...
{
    auto size = src.size();
    return draw_to_canvas(fSink.get(), bitmap, stream, log, size, [&](GrDirectContext*, SkCanvas* canvas) -> Result {
        SkDynamicMemoryWStream wstream;
