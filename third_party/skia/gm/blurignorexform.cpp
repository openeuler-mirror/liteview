...
protected:
    ...
    {
        SkString name;
        name.printf("blur_ignore_xform_%s", DrawType::kCircle == fDrawType ? "circle" : DrawType::kRect == fDrawType ? "rect" : "rrect");
        return name;
    }
    ...
    void onOnceBeforeDraw() override
    {
        for (int i = 0; i < kNumBlurs; ++i) {
            fBlurFilters[i] = SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(SkIntToScalar(20)), kBlurFlags[i].fRespectCTM);
        }
    }
...
const BlurIgnoreXformGM::BlurFlags BlurIgnoreXformGM::kBlurFlags[] = { { true, "none" }, { false, "IgnoreTransform" } };

const BlurIgnoreXformGM::MatrixScale BlurIgnoreXformGM::kMatrixScales[] = { { 1.0f, "Identity" }, { 0.5f, "Scale = 0.5" }, { 2.0f, "Scale = 2.0" } };
