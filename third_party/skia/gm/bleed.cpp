...
std::tuple<sk_sp<SkImage>, SkRect> make_ringed_image(SkCanvas* canvas, int width, int height)
{
    ...
    static constexpr uint32_t kOuterRingColor = 0xFFFF0000, kInnerRingColor = 0xFF0000FF, kCheckColor1 = 0xFF000000, kCheckColor2 = 0xFFFFFFFF;
    ...
    bitmap.setImmutable();
    return { ToolUtils::MakeTextureImage(canvas, bitmap.asImage()), SkRect::Make({ 2, 2, width - 2, height - 2 }) };
}
...
public:
        ...
        , fBatch(batch)
    {
        ...
        SkASSERT(this->onISize().width() <= kMaxTextureSize && this->onISize().height() <= kMaxTextureSize);
    }
...
protected:
    ...
    void drawImage(SkCanvas* canvas, sk_sp<SkImage> image, SkRect srcRect, SkRect dstRect, const SkSamplingOptions& sampling, SkPaint* paint)
    {
        if (fBatch) {
            ...
            imageSetEntry[0].fAAFlags = paint->isAntiAlias() ? SkCanvas::kAll_QuadAAFlags : SkCanvas::kNone_QuadAAFlags;
    ...
    {
        SkRect dst = SkRect::MakeXYWH(SkIntToScalar(transX), SkIntToScalar(transY), SkIntToScalar(kBlockSize), SkIntToScalar(kBlockSize));
    ...
    {
        SkRect src = SkRect::MakeXYWH(fBigSrcRect.fLeft, fBigSrcRect.fTop, fBigSrcRect.width() / 2, fBigSrcRect.height() / 2);
        SkRect dst = SkRect::MakeXYWH(SkIntToScalar(transX), SkIntToScalar(transY), SkIntToScalar(kBlockSize), SkIntToScalar(kBlockSize));
    ...
    void drawCase4(SkCanvas* canvas, int transX, int transY, bool aa, const SkSamplingOptions& sampling)
    {
        ...
        SkPaint paint;
        paint.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(3)));
        paint.setColor(SK_ColorBLUE);
        paint.setAntiAlias(aa);
    ...
    void drawCase5(SkCanvas* canvas, int transX, int transY, bool aa, const SkSamplingOptions& sampling)
    {
        ...
        SkPaint paint;
        paint.setMaskFilter(SkMaskFilter::MakeBlur(kOuter_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(7)));
        paint.setColor(SK_ColorBLUE);
        paint.setAntiAlias(aa);
    ...
    {
        if (!fSmallImage) {
            std::tie(fBigImage, fBigSrcRect) = make_ringed_image(canvas, 2 * kMaxTextureSize, 2 * kMaxTextureSize);
        ...
        m.setTranslate(
            std::max({ corners[0].fX, corners[1].fX, corners[2].fX, corners[3].fX }), (corners[0].fY + corners[1].fY + corners[2].fY + corners[3].fY) / 4);
        ...
        for (bool antiAlias : { false, true }) {
            ...
            for (const SkMatrix& matrix : matrices) {
                ...
                SkScalar x = kBlockSize + std::max({ innerCorners[0].fX, innerCorners[1].fX, innerCorners[2].fX, innerCorners[3].fX });
                maxX = std::max(maxX, x);
                canvas->restore();
            }
            canvas->restore();
...
DEF_GM(return new SrcRectConstraintGM("strict_constraint_no_red_allowed", SkCanvas::kStrict_SrcRectConstraint,
    /*batch=*/false););
DEF_GM(return new SrcRectConstraintGM("strict_constraint_batch_no_red_allowed", SkCanvas::kStrict_SrcRectConstraint,
    /*batch=*/true););
DEF_GM(return new SrcRectConstraintGM("fast_constraint_red_is_allowed", SkCanvas::kFast_SrcRectConstraint,
    /*batch=*/false););
...
DEF_SIMPLE_GM(bleed_downscale, canvas, 360, 240)
{
    ...
    const SkCanvas::SrcRectConstraint constraints[] = { SkCanvas::kStrict_SrcRectConstraint, SkCanvas::kFast_SrcRectConstraint };
    ...
    for (auto constraint : constraints) {
        ...
        for (auto sampling : samplings) {
            ...
            surf->getCanvas()->drawImageRect(img, src, SkRect::MakeWH(1, 1), sampling, nullptr, constraint);
            // now blow up the 1 pixel result
            canvas->drawImageRect(surf->makeImageSnapshot(), SkRect::MakeWH(100, 100), SkSamplingOptions());
