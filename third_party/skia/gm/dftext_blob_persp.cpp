...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        for (int i = 0; i < 3; ++i) {
            SkFont font;
            font.setSize(32);
            font.setEdging(i == 0 ? SkFont::Edging::kAlias : (i == 1 ? SkFont::Edging::kAntiAlias : SkFont::Edging::kSubpixelAntiAlias));
    ...
    void onDraw(SkCanvas* inputCanvas) override
    {
        ...
        SkImageInfo info = SkImageInfo::MakeN32(size.width(), size.height(), kPremul_SkAlphaType, inputCanvas->imageInfo().refColorSpace());
        SkSurfaceProps inputProps;
        inputCanvas->getProps(&inputProps);
        SkSurfaceProps props(SkSurfaceProps::kUseDeviceIndependentFonts_Flag | inputProps.flags(), inputProps.pixelGeometry());
...
private:
    ...
    void drawBlob(SkCanvas* canvas, SkTextBlob* blob, SkColor color, SkScalar x, SkScalar y, PerspMode perspMode, TranslateWithMatrix translateWithMatrix)
    {
        canvas->save();
