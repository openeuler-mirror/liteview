...
static void draw(SkCanvas* canvas, int width, int height, SkColor colors[2])
{
    ...
    SkPaint paint;
    paint.setShader(SkGradientShader::MakeRadial(center, radius, colors, nullptr, 2, SkTileMode::kMirror));
...
static sk_sp<SkImage> make_picture_image(int width, int height, SkColor colors[2])
{
    ...
    return SkImage::MakeFromPicture(recorder.finishRecordingAsPicture(), { width, height }, nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}

typedef sk_sp<SkImage> (*ImageMakerProc)(int width, int height, SkColor colors[2]);

static void show_image(SkCanvas* canvas, int width, int height, SkColor colors[2], ImageMakerProc proc)
{
    ...
    canvas->drawImageRect(image, subset, dstRect, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
    canvas->drawRect(dstRect, borderPaint);
...
public:
    VeryLargeBitmapGM(ImageMakerProc proc, const char name[])
        : fProc(proc)
        , fName(name)
    {
    }
