...
private:
    ...
    {
        return SkString("batchedconvexpaths");
    }
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override
    {
        ...
        for (uint32_t i = 0; i < 10; ++i) {
            ...
            for (float j = 1; j < numPoints; j += 3) {
                ...
                path.cubicTo(cosf(j / numPoints * k2PI), sinf(j / numPoints * k2PI), cosf((j + 1) / numPoints * k2PI), sinf((j + 1) / numPoints * k2PI),
                    j + 2 == numPoints ? 1 : cosf((j + 2) / numPoints * k2PI), j + 2 == numPoints ? 0 : sinf((j + 2) / numPoints * k2PI));
            }
