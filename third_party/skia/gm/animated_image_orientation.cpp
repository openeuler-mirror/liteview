...
static sk_sp<SkPicture> post_processor(const SkRect& bounds)
{
    ...
    pathBuilder.setFillType(SkPathFillType::kInverseEvenOdd).addRRect(SkRRect::MakeRectXY(bounds, radius, radius));
...
class AnimatedImageGM : public skiagm::GM {
    ...
    void init()
    {
        if (!fData) {
            ...
            fSize = { fTranslate * kMaxFrames * 2 // crop and no-crop
            ...
            for (float scale : { 1.25f, 1.0f, .75f, .5f }) {
                ...
                for (bool doCrop : { false, true }) {
                    for (bool doPostProcess : { false, true }) {
                        ...
                        const auto unscaledSize = SkEncodedOriginSwapsWidthHeight(origin) ? SkISize { info.height(), info.width() } : info.dimensions();

                        SkISize scaledSize = { SkScalarFloorToInt(unscaledSize.width() * scale), SkScalarFloorToInt(unscaledSize.height() * scale) };
                        info = info.makeDimensions(scaledSize);

                        auto cropRect = SkIRect::MakeSize(scaledSize);
                        if (doCrop) {
                            auto matrix = SkMatrix::RectToRect(SkRect::Make(unscaledSize), SkRect::Make(scaledSize));
                            matrix.preConcat(SkEncodedOriginToMatrix(origin, unscaledSize.width(), unscaledSize.height()));
                            SkRect cropRectFloat = SkRect::Make(fCropRect);
                            matrix.mapRect(&cropRectFloat);
                            cropRectFloat.roundOut(&cropRect);
                        }

                        sk_sp<SkPicture> postProcessor = doPostProcess ? post_processor(SkRect::Make(cropRect.size())) : nullptr;
                        auto animatedImage = SkAnimatedImage::Make(std::move(androidCodec), info, cropRect, std::move(postProcessor));
