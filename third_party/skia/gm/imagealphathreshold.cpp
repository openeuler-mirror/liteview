...
// best we can
static sk_sp<SkSurface> make_color_matching_surface(SkCanvas* canvas, int width, int height, SkAlphaType at)
{
...
protected:
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override
    {
        ...
        sk_sp<SkSurface> surface(make_color_matching_surface(canvas, WIDTH, HEIGHT, kPremul_SkAlphaType));
        if (!surface) {
...
DEF_SIMPLE_GM_BG(imagealphathreshold_image, canvas, WIDTH * 2, HEIGHT, SK_ColorBLACK)
{
    ...
    filterPaint.setImageFilter(SkImageFilters::AlphaThreshold(region, 0.2f, 0.7f, std::move(imageSource)));
