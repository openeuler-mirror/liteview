...
protected:
    ...
    void draw(SkCanvas* canvas, const SkRect& rect, const SkSize& deviceSize, const SkSamplingOptions& sampling, sk_sp<SkImageFilter> input)
    {
        ...
        sk_sp<SkImageFilter> filter(SkImageFilters::MatrixTransform(matrix, sampling, std::move(input)));
    ...
    void onDraw(SkCanvas* canvas) override
    {
...

        {
            ...
            sk_sp<SkImageFilter> source(SkImageFilters::Image(std::move(image), inRect, outRect, SkSamplingOptions({ 1 / 3.0f, 1 / 3.0f })));
