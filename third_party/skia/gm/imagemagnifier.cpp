...
DEF_SIMPLE_GM_BG(imagemagnifier, canvas, WIDTH, HEIGHT, SK_ColorBLACK)
{
    ...
    filterPaint.setImageFilter(
        SkImageFilters::Magnifier(SkRect::MakeXYWH(SkIntToScalar(100), SkIntToScalar(100), SkIntToScalar(WIDTH / 2), SkIntToScalar(HEIGHT / 2)), 100, nullptr));
...
DEF_SIMPLE_GM_BG(imagemagnifier_cropped, canvas, WIDTH_HEIGHT, WIDTH_HEIGHT, SK_ColorBLACK)
{
    ...
    SkRect srcRect = SkRect::MakeWH(SkIntToScalar(WIDTH_HEIGHT - 32), SkIntToScalar(WIDTH_HEIGHT - 32));
    ...
    SkPaint filterPaint;
    filterPaint.setImageFilter(SkImageFilters::Magnifier(srcRect, kInset, std::move(imageSource), &cropRect));
