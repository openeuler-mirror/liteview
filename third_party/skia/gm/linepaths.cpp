...
static void drawPath(SkPath& path, SkCanvas* canvas, SkColor color, const SkRect& clip, SkPaint::Cap cap, SkPaint::Join join, SkPaint::Style style,
    ...
    for (size_t cap = 0; cap < std::size(gCaps); ++cap) {
        ...
        for (size_t fill = 0; fill < std::size(gFills); ++fill) {
            ...
            for (size_t style = 0; style < std::size(gStyles); ++style) {
                ...
                drawPath(path.fPath, canvas, color, rect, gCaps[cap].fCap, gCaps[cap].fJoin, gStyles[style].fStyle, gFills[fill].fFill, SK_Scalar1 * 10);
