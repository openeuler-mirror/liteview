...
public:
    MaskGenerator(const SkImageInfo& info)
        : INHERITED(info)
    {
    }
...
const MakerT makers[] = {
    // SkImage_Raster
    [](SkCanvas*, const SkImageInfo& info) -> sk_sp<SkImage> { return make_mask(SkSurface::MakeRaster(info)); },
    ...
    // SkImage_Lazy
    [](SkCanvas*, const SkImageInfo& info) -> sk_sp<SkImage> { return SkImage::MakeFromGenerator(std::make_unique<MaskGenerator>(info)); },
};
...
DEF_SIMPLE_GM(imagemasksubset, canvas, 480, 480)
{
    ...
    for (size_t i = 0; i < std::size(makers); ++i) {
        ...
        if (image) {
            canvas->drawImageRect(image, SkRect::Make(kSubset), kDest, SkSamplingOptions(), &paint, SkCanvas::kStrict_SrcRectConstraint);
            auto direct = GrAsDirectContext(canvas->recordingContext());
            sk_sp<SkImage> subset = image->makeSubset(kSubset, direct);
            canvas->drawImageRect(subset, kDest.makeOffset(kSize.width() * 1.5f, 0), SkSamplingOptions(), &paint);
        }
