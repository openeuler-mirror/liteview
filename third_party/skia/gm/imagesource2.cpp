...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        for (int x = 0; x < kImageSize; x += 3) {
            SkRect r = SkRect::MakeXYWH(SkIntToScalar(x), SkIntToScalar(0), SkIntToScalar(3), SkIntToScalar(kImageSize));
            SkPaint p;
    ...
    {
        const SkRect srcRect = SkRect::MakeLTRB(0, 0, SkIntToScalar(kImageSize), SkIntToScalar(kImageSize));
...
DEF_GM(return new ImageSourceGM("med", SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kLinear));)
