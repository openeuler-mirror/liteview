...
SkBitmap render_level(SkISize dimensions, SkColor color, SkColorType colorType, bool opaque)
{
    ...
    SkImageInfo ii = SkImageInfo::Make(dimensions.width(), dimensions.height(), colorType, opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType);
    SkBitmap bm;
    bm.allocPixels(ii);
...
static sk_sp<SkImage> make_compressed_image(
    GrDirectContext* dContext, const SkISize dimensions, SkColorType colorType, bool opaque, SkImage::CompressionType compression)
{
    ...
    for (int i = 0; i < numMipLevels; ++i) {
        ...
        if (compression == SkImage::CompressionType::kETC2_RGB8_UNORM) {
            ...
            if (etc1_encode_image((unsigned char*)bm.getAddr16(0, 0), bm.width(), bm.height(), 2, bm.rowBytes(), (unsigned char*)&pixels[offset])) {
                return nullptr;
            }
        } else {
    ...
    sk_sp<SkImage> image;
    if (dContext) {
        image = SkImage::MakeTextureFromCompressed(dContext, std::move(tmp), dimensions.width(), dimensions.height(), compression, GrMipmapped::kYes);
    } else {
        image = SkImage::MakeRasterFromCompressed(std::move(tmp), dimensions.width(), dimensions.height(), compression);
    }
    return image;
}
...
protected:
    ...
    DrawResult onGpuSetup(GrDirectContext* dContext, SkString* errorMsg) override
    {
        ...
        if (dContext && dContext->backend() == GrBackendApi::kDirect3D && fType == Type::kNonMultipleOfFour) {
            // skbug.com/10541 - Are non-multiple-of-four BC1 textures supported in D3D?
            return DrawResult::kSkip;
        }

        fOpaqueETC2Image = make_compressed_image(dContext, fImgDimensions, kRGB_565_SkColorType, true, SkImage::CompressionType::kETC2_RGB8_UNORM);

        fOpaqueBC1Image = make_compressed_image(dContext, fImgDimensions, kRGBA_8888_SkColorType, true, SkImage::CompressionType::kBC1_RGB8_UNORM);

        fTransparentBC1Image = make_compressed_image(dContext, fImgDimensions, kRGBA_8888_SkColorType, false, SkImage::CompressionType::kBC1_RGBA8_UNORM);
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        this->drawCell(canvas, fTransparentBC1Image.get(), { 2 * kPad + kCellWidth, 2 * kPad + kBaseTexHeight });
    }
...
private:
    void drawCell(SkCanvas* canvas, SkImage* image, SkIVector offset)
    {
        ...
        int numMipLevels = SkMipmap::ComputeLevelCount(levelDimensions.width(), levelDimensions.height()) + 1;
        ...
        for (int i = 0; i < numMipLevels; ++i) {
            SkRect r = SkRect::MakeXYWH(offset.fX, offset.fY, levelDimensions.width(), levelDimensions.height());
