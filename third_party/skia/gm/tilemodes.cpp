...
static void makebm(SkBitmap* bm, SkColorType ct, int w, int h)
{
    ...
    paint.setShader(SkGradientShader::MakeLinear(pts, colors, pos, std::size(colors), SkTileMode::kClamp));
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t kx = 0; kx < std::size(gModes); kx++) {
            for (size_t ky = 0; ky < std::size(gModes); ky++) {
                ...
                SkTextUtils::DrawString(canvas, str.c_str(), x + r.width() / 2, y, font, p, SkTextUtils::kCenter_Align);
    ...
    // on SGX where resizing NPOT textures to POT textures exhibits a driver bug.
                ...
                canvas->drawString(SkStringPrintf("%s, %s", gConfigNames[i], gFilterNames[j]), x, y + r.height() * 2 / 3, font, textPaint);
...
static sk_sp<SkShader> make_grad(SkTileMode tx, SkTileMode ty)
{
    ...
    case 2:
        return SkGradientShader::MakeSweep(center.fX, center.fY, colors, nullptr, std::size(colors), tx, 135, 225, 0, nullptr);
    }
    return nullptr;
}
...
private:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t kx = 0; kx < std::size(gModes); kx++) {
            ...
            SkTextUtils::DrawString(canvas, str.c_str(), x + r.width() / 2, y, font, SkPaint(), SkTextUtils::kCenter_Align);
            x += r.width() * 4 / 3;
        }
...
DEF_SIMPLE_GM(tilemode_decal, canvas, 720, 1100)
{
    ...
    std::function<void(SkPaint*, SkTileMode, SkTileMode)> shader_procs[] = {
        ...
        [img](SkPaint* paint, SkTileMode tx, SkTileMode ty) {
            ...
            const SkScalar* pos = nullptr;
            const int count = std::size(colors);
            paint->setShader(SkGradientShader::MakeRadial({ img->width() * 0.5f, img->width() * 0.5f }, img->width() * 0.5f, colors, pos, count, tx));
        },
    };
