...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        sk_sp<SkShader> gradient = SkGradientShader::MakeRadial(
            { 200.f, 200.f }, 25.f, kColors, nullptr, 2, SkTileMode::kMirror, SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr);
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        SkM44 proj { 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, -1.f / radius, 1.f };
        ...
        SkColor faceColors[6] = {
            SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorYELLOW, SkColorSetARGB(0xFF, 0xFF, 0xA5, 0x00), // orange css
            SkColorSetARGB(0xFF, 0x80, 0x00, 0x80) // purple css
        };

        for (int i = 0; i < 6; ++i) {
            SkM44 model = SkM44::Rotate({ axisAngles[i].x, axisAngles[i].y, axisAngles[i].z }, SkDegreesToRadians(axisAngles[i].w));
            ...
            canvas->drawImageRect(fCubeImage.get(), SkRect::MakeWH(fCubeImage->width(), fCubeImage->height()), SkRect::MakeWH(viewportWidth, viewportWidth),
                SkSamplingOptions(SkFilterMode::kLinear), &fillPaint, SkCanvas::kFast_SrcRectConstraint);
