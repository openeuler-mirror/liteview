...
constexpr SkColor gColors[] = { SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorWHITE, SK_ColorBLACK };
...
constexpr GradData gGradData[] = { { 2, gColors, gColors4f, nullptr }, { 2, gColors, gColors4f, gPos0 }, { 2, gColors, gColors4f, gPos1 },
    { 5, gColors, gColors4f, nullptr }, { 5, gColors, gColors4f, gPos2 }, { 4, gColorClamp, gColor4fClamp, gPosClamp } };

static sk_sp<SkShader> MakeLinear(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    return SkGradientShader::MakeLinear(pts, data.fColors, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> MakeLinear4f(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeLinear(pts, data.fColors4f, srgb, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> MakeRadial(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    return SkGradientShader::MakeRadial(center, center.fX, data.fColors, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> MakeRadial4f(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeRadial(center, center.fX, data.fColors4f, srgb, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> MakeSweep(const SkPoint pts[2], const GradData& data, SkTileMode, const SkMatrix& localMatrix)
{
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    return SkGradientShader::MakeSweep(center.fX, center.fY, data.fColors, data.fPos, data.fCount, 0, &localMatrix);
}

static sk_sp<SkShader> MakeSweep4f(const SkPoint pts[2], const GradData& data, SkTileMode, const SkMatrix& localMatrix)
{
    SkPoint center;
    center.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeSweep(center.fX, center.fY, data.fColors4f, srgb, data.fPos, data.fCount, 0, &localMatrix);
}

static sk_sp<SkShader> Make2Radial(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    ...
    return SkGradientShader::MakeTwoPointConical(
        center1, (pts[1].fX - pts[0].fX) / 7, center0, (pts[1].fX - pts[0].fX) / 2, data.fColors, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> Make2Radial4f(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    SkPoint center0, center1;
    center0.set(SkScalarAve(pts[0].fX, pts[1].fX), SkScalarAve(pts[0].fY, pts[1].fY));
    center1.set(SkScalarInterp(pts[0].fX, pts[1].fX, SkIntToScalar(3) / 5), SkScalarInterp(pts[0].fY, pts[1].fY, SkIntToScalar(1) / 4));
...
static sk_sp<SkShader> Make2Conical(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    SkPoint center0, center1;
    ...
    return SkGradientShader::MakeTwoPointConical(center1, radius1, center0, radius0, data.fColors, data.fPos, data.fCount, tm, 0, &localMatrix);
}

static sk_sp<SkShader> Make2Conical4f(const SkPoint pts[2], const GradData& data, SkTileMode tm, const SkMatrix& localMatrix)
{
    SkPoint center0, center1;
    SkScalar radius0 = (pts[1].fX - pts[0].fX) / 10;
    SkScalar radius1 = (pts[1].fX - pts[0].fX) / 3;
    ...
    center1.set(pts[1].fX - radius1, pts[1].fY - radius1);
    auto srgb = SkColorSpace::MakeSRGB();
    return SkGradientShader::MakeTwoPointConical(center1, radius1, center0, radius0, data.fColors4f, srgb, data.fPos, data.fCount, tm, 0, &localMatrix);
}
...
constexpr GradMaker gGradMakers4f[] = { MakeLinear4f, MakeRadial4f, MakeSweep4f, Make2Radial4f, Make2Conical4f };
...
private:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        center.iset(0, 300);
        canvas->translate(SkIntToScalar(20), SkIntToScalar(20));
        paint.setShader(SkGradientShader::MakeRadial(SkPoint(center), SkIntToScalar(200), gColors, nullptr, 5, SkTileMode::kClamp));
        canvas->drawRect(r, paint);
    }
...
class RadialGradientGM : public skiagm::GM {
    SkString onShortName() override
    {
        return SkString("radial_gradient");
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        paint.setShader(SkGradientShader::MakeRadial(center, radius, colors, pos, std::size(pos), SkTileMode::kClamp));
...
private:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t i = 0; i < std::size(flags); i++) {
            paint1.setShader(SkGradientShader::MakeSweep(cx, cy, sweep_colors, nullptr, std::size(sweep_colors), flags[i], nullptr));
            paint2.setShader(SkGradientShader::MakeRadial(center, radius, colors1, nullptr, std::size(colors1), SkTileMode::kClamp, flags[i], nullptr));
            paint3.setShader(SkGradientShader::MakeRadial(center, radius, colors2, nullptr, std::size(colors2), SkTileMode::kClamp, flags[i], nullptr));
...
private:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        fShader = SkGradientShader::MakeRadial(center, kRadius, kColors, nullptr, 2, SkTileMode::kClamp);
    }
...
private:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        fShader = SkGradientShader::MakeRadial(center, kRadius, colors, pos, std::size(gColors), SkTileMode::kClamp);
    }
...
private:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        const SkColor colors[] = { SK_ColorWHITE, SK_ColorWHITE, 0xFF008200, 0xFF008200, SK_ColorWHITE, SK_ColorWHITE };
        ...
        for (int index = 0; index < (int)std::size(fShader); ++index) {
            ...
            fShader[index] = SkGradientShader::MakeLinear(pts, colors, pos, std::size(gColors), SkTileMode::kClamp);
        }
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int index = 0; index < (int)std::size(fShader); ++index) {
            ...
            canvas->drawRect(SkRect::MakeLTRB(0, index * kHeight, kMinWidth + index * kWidthBump, (index + 1) * kHeight), paint);
        }
    }
...
class LinearGradientTinyGM : public skiagm::GM {
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (unsigned i = 0; i < std::size(configs); ++i) {
            ...
            paint.setShader(SkGradientShader::MakeLinear(configs[i].pts, colors, configs[i].pos, kStopCount, SkTileMode::kClamp, kFlags, nullptr));
...
static sk_sp<SkShader> make_radial(const GradRun& run, SkTileMode mode)
{
    ...
    return SkGradientShader::MakeRadial({ half, half }, half - 10, run.fColors, run.fPos, run.fCount, mode);
}
...
static sk_sp<SkShader> make_conical(const GradRun& run, SkTileMode mode)
{
    ...
    return SkGradientShader::MakeTwoPointConical(center, 20, center, half - 10, run.fColors, run.fPos, run.fCount, mode);
}
...
DEF_SIMPLE_GM(gradients_dup_color_stops, canvas, 704, 564)
{
    ...
    sk_sp<SkShader> (*factories[])(const GradRun&, SkTileMode) { make_linear, make_radial, make_conical, make_sweep };
...
static void draw_many_stops(SkCanvas* canvas)
{
    ...
    SkPaint p;
    p.setShader(SkGradientShader::MakeLinear(pts, colors, nullptr, std::size(colors), SkTileMode::kClamp));
...
DEF_SIMPLE_GM(fancy_gradients, canvas, 800, 300)
{
    draw_circle_shader(canvas, 150, 150, 100, []() -> sk_sp<SkShader> {
        ...
        p.setShader(SkGradientShader::MakeLinear(pts1, colors1, pos, std::size(colors1), SkTileMode::kClamp, 0, nullptr));
        recorder.getRecordingCanvas()->drawPaint(p);

        SkPoint pts2[] = { { 0, kTileSize }, { kTileSize, 0 } };
        p.setShader(SkGradientShader::MakeLinear(pts2, colors2, pos, std::size(colors2), SkTileMode::kClamp, 0, nullptr));
        recorder.getRecordingCanvas()->drawPaint(p);

        SkMatrix m = SkMatrix::I();
        m.preRotate(45);
        return recorder.finishRecordingAsPicture()->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest, &m, nullptr);
    });
    ...
    draw_circle_shader(canvas, 400, 150, 100, []() -> sk_sp<SkShader> {
        ...
        SkColor colors[] = { 0xff000000, 0xff000000, 0xffffffff, 0xffffffff, 0xff000000, 0xff000000, 0xffffffff, 0xffffffff };
        SkScalar pos[] = { 0, .25f, .25f, .5f, .5f, .75f, .75f, 1 };
        static_assert(std::size(colors) == std::size(pos), "color/pos size mismatch");

        SkPaint p;
        p.setShader(SkGradientShader::MakeSweep(kTileSize / 2, kTileSize / 2, colors, pos, std::size(colors), 0, nullptr));
        SkPictureRecorder recorder;
        recorder.beginRecording(SkRect::MakeWH(kTileSize, kTileSize))->drawPaint(p);

        return recorder.finishRecordingAsPicture()->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest);
    });
    ...
    draw_circle_shader(canvas, 650, 150, 100, []() -> sk_sp<SkShader> {
        ...
        SkScalar pos[] = { 0, .125f, .125f, .25f, .25f, .375f, .375f, .5f, .5f, .625f, .625f, .75f, .75f, .875f, .875f, 1 };
        static_assert(std::size(colors) == std::size(pos), "color/pos size mismatch");

        SkPoint center = { 650, 150 };
        sk_sp<SkShader> sweep1 = SkGradientShader::MakeSweep(center.x(), center.y(), colors, pos, std::size(colors), 0, nullptr);
        SkMatrix m = SkMatrix::I();
        m.preRotate(22.5f, center.x(), center.y());
        sk_sp<SkShader> sweep2 = SkGradientShader::MakeSweep(center.x(), center.y(), colors, pos, std::size(colors), 0, &m);

        sk_sp<SkShader> sweep(SkShaders::Blend(SkBlendMode::kExclusion, sweep1, sweep2));

        SkScalar radialPos[] = { 0, .02f, .02f, .04f, .04f, .08f, .08f, .16f, .16f, .31f, .31f, .62f, .62f, 1, 1, 1 };
        static_assert(std::size(colors) == std::size(radialPos), "color/pos size mismatch");

        return SkShaders::Blend(
            SkBlendMode::kExclusion, sweep, SkGradientShader::MakeRadial(center, 100, colors, radialPos, std::size(radialPos), SkTileMode::kClamp));
    });
}
...
DEF_SIMPLE_GM(sweep_tiling, canvas, 690, 512)
{
    ...
    static constexpr SkTileMode modes[] = { SkTileMode::kClamp, SkTileMode::kRepeat, SkTileMode::kMirror };
    ...
    for (auto mode : modes) {
        {
            ...
            for (auto angle : angles) {
                p.setShader(SkGradientShader::MakeSweep(size / 2, size / 2, colors, pos, std::size(colors), mode, angle.start, angle.end, 0, nullptr));
...
DEF_SIMPLE_GM(rgbw_sweep_gradient, canvas, 100, 100)
{
    ...
    static constexpr SkColor colors[] = { SK_ColorWHITE, SK_ColorWHITE, SK_ColorBLUE, SK_ColorBLUE, SK_ColorRED, SK_ColorRED, SK_ColorGREEN, SK_ColorGREEN };
