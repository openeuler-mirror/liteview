...
static void draw_label(SkCanvas* canvas, const char* label, const SkPoint& offset)
{
    ...
    canvas->drawSimpleText(label, len, SkTextEncoding::kUTF8, offset.x() - width / 2, offset.y(), font, SkPaint());
}
...
DEF_SIMPLE_GM(AlternateLuma, canvas, 384, 128)
{
    ...
    auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(SkString { "half4 main(half4 inColor) { return inColor.ggga; }" });
    ...
    paint.setColorFilter(SkColorFilterPriv::WithWorkingFormat(std::move(filter), &SkNamedTransferFn::kLinear, &SkNamedGamut::kXYZ, &unpremul));
    canvas->drawImage(img, 0, 0, SkSamplingOptions {}, &paint);
}
