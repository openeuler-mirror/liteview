...
protected:
    ...
    {
        unsigned flags = (edgeAA[0] * SkCanvas::kTop_QuadAAFlag) | (edgeAA[1] * SkCanvas::kRight_QuadAAFlag) | (edgeAA[2] * SkCanvas::kBottom_QuadAAFlag)
    ...
    // drawTile at leaves.
    int clipTile(
        SkCanvas* canvas, int tileID, const SkRect& baseRect, const SkPoint quad[4], const bool edgeAA[4], const SkPoint lines[], int lineCount, int* quadCount)
    {
        if (lineCount == 0) {
        ...
        for (int i = 0; i < 4; ++i) {
            SkPoint intersect;
            if (intersect_line_segments(points[i], points[i == 3 ? 0 : i + 1], lines[0], lines[1], &intersect)) {
        ...
        if (intersectionCount < 2) {
            ...
            return this->clipTile(canvas, tileID, baseRect, quad, edgeAA, lines + 2, lineCount - 1, quadCount);
        }
        ...
        for (int i = 0; i < subtiles.count(); ++i) {
            ...
            draws += this->clipTile(canvas, tileID, baseRect, sub, subAA, lines + 2, lineCount - 1, quadCount);
        }
        return draws;
    }
};
...
protected:
    SkISize onISize() override
    {
        ...
        return SkISize::Make(SkScalarRoundToInt(kCellWidth * kMatrixCount + 175.f), SkScalarRoundToInt(kCellHeight * fRenderers.count() + 75.f));
    }
...
private:
    ...
    void configureMatrices()
    {
        ...
        SkPoint dst[4] = { { 0, 0 }, { kColCount * kTileWidth + 10.f, 15.f }, { kColCount * kTileWidth - 28.f, kRowCount * kTileHeight + 40.f },
...
public:
    ...
    void drawBanner(SkCanvas* canvas) override
    {
        ...
        if (fEnableAAOverride) {
            SkASSERT(fAAOverride == SkCanvas::kAll_QuadAAFlags || fAAOverride == SkCanvas::kNone_QuadAAFlags);
...
public:
    ...
    int drawTile(SkCanvas* canvas, const SkRect& rect, const SkPoint clip[4], const bool edgeAA[4], int tileID, int quadID) override
    {
        canvas->experimental_DrawEdgeAAQuad(rect, clip, this->maskToFlags(edgeAA), fColor.toSkColor(), SkBlendMode::kSrcOver);
        return 1;
    }
...
private:
    ...
    SolidColorRenderer(const SkColor4f& color)
        : fColor(color)
    {
    }
...
public:
    ...
    static sk_sp<ClipTileRenderer> MakeBatched(sk_sp<SkImage> image, int transformCount)
    {
        ...
        return Make("Texture Set", subtitle, std::move(image), nullptr, nullptr, nullptr, nullptr, 1.f, false, transformCount);
    }

    static sk_sp<ClipTileRenderer> MakeShader(const char* name, sk_sp<SkImage> image, sk_sp<SkShader> shader, bool local)
    {
        return Make("Shader", name, std::move(image), std::move(shader), nullptr, nullptr, nullptr, 1.f, local, 0);
    }

    static sk_sp<ClipTileRenderer> MakeColorFilter(const char* name, sk_sp<SkImage> image, sk_sp<SkColorFilter> filter)
    {
        return Make("Color Filter", name, std::move(image), nullptr, std::move(filter), nullptr, nullptr, 1.f, false, 0);
    }

    static sk_sp<ClipTileRenderer> MakeImageFilter(const char* name, sk_sp<SkImage> image, sk_sp<SkImageFilter> filter)
    {
        return Make("Image Filter", name, std::move(image), nullptr, nullptr, std::move(filter), nullptr, 1.f, false, 0);
    }

    static sk_sp<ClipTileRenderer> MakeMaskFilter(const char* name, sk_sp<SkImage> image, sk_sp<SkMaskFilter> filter)
    {
        return Make("Mask Filter", name, std::move(image), nullptr, nullptr, nullptr, std::move(filter), 1.f, false, 0);
    }

    static sk_sp<ClipTileRenderer> MakeAlpha(sk_sp<SkImage> image, SkScalar alpha)
    {
        return Make("Alpha", SkStringPrintf("a = %.2f", alpha).c_str(), std::move(image), nullptr, nullptr, nullptr, nullptr, alpha, false, 0);
    }

    static sk_sp<ClipTileRenderer> Make(const char* topBanner, const char* bottomBanner, sk_sp<SkImage> image, sk_sp<SkShader> shader,
        sk_sp<SkColorFilter> colorFilter, sk_sp<SkImageFilter> imageFilter, sk_sp<SkMaskFilter> maskFilter, SkScalar paintAlpha, bool resetAfterEachQuad,
        int transformCount)
    {
        return sk_sp<ClipTileRenderer>(new TextureSetRenderer(topBanner, bottomBanner, std::move(image), std::move(shader), std::move(colorFilter),
            std::move(imageFilter), std::move(maskFilter), paintAlpha, resetAfterEachQuad, transformCount));
    }
    ...
    int drawTile(SkCanvas* canvas, const SkRect& rect, const SkPoint clip[4], const bool edgeAA[4], int tileID, int quadID) override
    {
        ...
        SkMatrix gridToImage
            = SkMatrix::RectToRect(SkRect::MakeWH(kColCount * kTileWidth, kRowCount * kTileHeight), SkRect::MakeWH(fImage->width(), fImage->height()));
        ...
        // is not null.
        fSetEntries.push_back({ fImage, localRect, rect, matrixIdx, 1.f, this->maskToFlags(edgeAA), hasClip });
...
private:
    ...
    TextureSetRenderer(const char* topBanner, const char* bottomBanner, sk_sp<SkImage> image, sk_sp<SkShader> shader, sk_sp<SkColorFilter> colorFilter,
        sk_sp<SkImageFilter> imageFilter, sk_sp<SkMaskFilter> maskFilter, SkScalar paintAlpha, bool resetEachQuad, int transformBatchCount)
    ...
    int drawAndReset(SkCanvas* canvas)
    {
        ...
        for (int i = 0; i < fSetEntries.count(); ++i) {
            ...
            SkASSERT(fSetEntries[i].fMatrixIndex < 0 || fSetEntries[i].fMatrixIndex < fPreViewMatrices.count());
        }
        ...
        canvas->experimental_DrawEdgeAAImageSet(fSetEntries.begin(), fSetEntries.count(), fDstClips.begin(), fPreViewMatrices.begin(),
...
public:
    ...
    int drawTiles(SkCanvas* canvas) override
    {
        ...
        if (fYUVData) {
            fImage = fYUVData->refImage(canvas->recordingContext(), sk_gpu_test::LazyYUVImage::Type::kFromPixmaps);
            if (!fImage) {
                return 0;
            }
        }
...
private:
    ...
    int drawAndReset(SkCanvas* canvas)
    {
        ...
        canvas->experimental_DrawEdgeAAImageSet(fSetEntries.begin(), fSetEntries.count(), fDstClips.begin(), nullptr, SkSamplingOptions(SkFilterMode::kLinear),
...
{
    return ClipTileRendererArray { DebugTileRenderer::Make(), DebugTileRenderer::MakeAA(), DebugTileRenderer::MakeNonAA() };
}
...
static ClipTileRendererArray make_shader_renderers()
{
    ...
    return ClipTileRendererArray { TextureSetRenderer::MakeShader("Gradient", image, gradient, false),
...
static ClipTileRendererArray make_image_renderers()
{
    ...
    return ClipTileRendererArray { TextureSetRenderer::MakeUnbatched(mandrill), TextureSetRenderer::MakeBatched(mandrill, 0),
        TextureSetRenderer::MakeBatched(mandrill, kMatrixCount), YUVTextureSetRenderer::MakeFromJPEG(mandrillJpeg) };
}
...
static ClipTileRendererArray make_filtered_renderers()
{
    ...
    auto alphaGradient = SkGradientShader::MakeRadial(
        { 0.5f * kTileWidth * kColCount, 0.5f * kTileHeight * kRowCount }, 0.25f * kTileWidth * kColCount, kAlphas, nullptr, 2, SkTileMode::kClamp);
    sk_sp<SkMaskFilter> maskFilter = SkShaderMaskFilter::Make(std::move(alphaGradient));

    return ClipTileRendererArray { TextureSetRenderer::MakeAlpha(mandrill, 0.5f),
