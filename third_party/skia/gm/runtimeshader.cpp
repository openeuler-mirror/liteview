...
public:
    ...
    {
        auto [effect, error] = (fFlags & kColorFilter_RTFlag) ? SkRuntimeEffect::MakeForColorFilter(fSkSL) : SkRuntimeEffect::MakeForShader(fSkSL);
        if (!effect) {
    ...
    {
        return SkToBool(fFlags & kBench_RTFlag);
    }
...
public:
    SimpleRT()
        : RuntimeShaderGM("runtime_shader", { 512, 256 }, R"(
...
{
    SkMatrix scale = SkMatrix::Scale(size.width() / (float)img->width(), size.height() / (float)img->height());
    return img->makeShader(SkSamplingOptions(), scale);
}

static sk_sp<SkShader> make_threshold(SkISize size)
{
    auto info = SkImageInfo::Make(size.width(), size.height(), kAlpha_8_SkColorType, kPremul_SkAlphaType);
...
public:
    ThresholdRT()
        : RuntimeShaderGM("threshold_rt", { 256, 256 }, R"(
...
public:
    SpiralRT()
        : RuntimeShaderGM("spiral_rt", { 512, 512 }, R"(
...
public:
    ColorCubeColorFilterRT()
        : RuntimeShaderGM("color_cube_cf_rt", { 512, 512 }, R"(
        uniform shader color_cube;
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        paint.setColorFilter(fEffect->makeColorFilter(SkData::MakeWithCopy(uniforms, sizeof(uniforms)), SkSpan(children)));
...
public:
    ...
    void drawSuperRRect(SkCanvas* canvas, const SkRect& superRRect, float radX, float radY, SkColor color)
    {
        ...
        builder.uniform("stretch_factor") = SkV2 { 1 - superRRect.width() * .5f / cornerWidth, 1 - superRRect.height() * .5f / cornerHeight };
        ...
        cornerToLocal.setScaleTranslate(cornerWidth, cornerHeight, superRRect.centerX(), superRRect.centerY());
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        canvas->save();
        canvas->rotate(21);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(-5, 25, 175, 100), 50, 30, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(94);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(95, 75, 125, 100), 30, 30, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(132);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(0, 75, 150, 100), 40, 30, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(282);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(15, -20, 100, 100), 20, 20, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(0);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(140, -50, 90, 110), 25, 25, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(-35);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(160, -60, 60, 90), 18, 18, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(65);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(220, -120, 60, 90), 18, 18, rand.nextU() | 0xff808080);
        canvas->restore();

        canvas->save();
        canvas->rotate(265);
        this->drawSuperRRect(canvas, SkRect::MakeXYWH(150, -129, 80, 160), 24, 39, rand.nextU() | 0xff808080);
        canvas->restore();
...
public:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (auto cs : { static_cast<SkColorSpace*>(nullptr), sk_srgb_singleton() }) {
            SkImageInfo info = SkImageInfo::Make(256, 64, kN32_SkColorType, kPremul_SkAlphaType, sk_ref_sp(cs));
...
{
    static constexpr char scale[] = "uniform shader child;"
                                    "half4 main(float2 xy) {"
...
DEF_SIMPLE_GM(raw_image_shader_normals_rt, canvas, 768, 512)
{
    ...
    auto surfInfo = SkImageInfo::Make(512, 512, kN32_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB()->makeColorSpin());
    ...
    sk_sp<SkShader> colorNormals = normal_map_image_shader(), rawNormals = normal_map_raw_image_shader();
    ...
    canvas->drawImage(surface->makeImageSnapshot()->reinterpretColorSpace(SkColorSpace::MakeSRGB()), 0, 0);
...
{
    // First, make an offscreen surface, so we can control the destination color space:
    auto surfInfo = SkImageInfo::Make(512, 256, kN32_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
    auto surface = canvas->makeSurface(surfInfo);
    if (!surface) {
        surface = SkSurface::MakeRaster(surfInfo);
    }
