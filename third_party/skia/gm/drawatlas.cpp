...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        canvas->drawAtlas(atlas.get(), xform, tex, nullptr, N, SkBlendMode::kDst, sampling, nullptr, &paint);
        canvas->translate(0, 100);
        canvas->drawAtlas(atlas.get(), xform, tex, colors, N, SkBlendMode::kSrcIn, sampling, nullptr, &paint);
    }
...
static void draw_text_on_path(
    SkCanvas* canvas, const void* text, size_t length, const SkPoint xy[], const SkPath& path, const SkFont& font, const SkPaint& paint, float baseline_offset)
{
    ...
    const SkScalar max = std::max(std::max(SkScalarAbs(fontb.fLeft), SkScalarAbs(fontb.fRight)), std::max(SkScalarAbs(fontb.fTop), SkScalarAbs(fontb.fBottom)));
    ...
    canvas->drawTextBlob(SkTextBlob::MakeFromRSXform(glyphs.get(), count * sizeof(SkGlyphID), &xform[0], font, SkTextEncoding::kGlyphID), 0, 0, paint);
...
DEF_SIMPLE_GM(blob_rsxform_distortable, canvas, 500, 100)
{
    ...
    if (distortable) {
        ...
        const SkFontArguments::VariationPosition::Coordinate position[] = { { SkSetFourByteTag('w', 'g', 'h', 't'), 1.618033988749895f } };
    ...
    for (SkBlendMode mode : modes) {
        for (float alpha : { 1.0f, 0.5f }) {
            ...
            for (const sk_sp<SkColorFilter>& cf : filters) {
                ...
                canvas->drawAtlas(image.get(), &xform, &tex, &color, 1, mode, SkSamplingOptions(), &tex, &paint);
