...
protected:
    ...
    {
        {
            ...
            auto [spec, error] = SkMeshSpecification::Make(kAttributes, sizeof(ColorVertex), kVaryings, SkString(kVS), SkString(kFS));
            if (!spec) {
                SkDebugf("%s\n", error.c_str());
            }
        ...
        }
        {
            ...
            auto [spec, error] = SkMeshSpecification::Make(kAttributes, sizeof(NoColorVertex), kVaryings, SkString(kVS), SkString(kFS));
            if (!spec) {
                SkDebugf("%s\n", error.c_str());
            }
    ...
    DrawResult onGpuSetup(GrDirectContext* dc, SkString* string) override
    {
        ...
        fColorIndexedVB = SkMesh::MakeVertexBuffer(dc, CpuVBPeek(fColorIndexedVB), fColorIndexedVB->size());
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString*) override
    {
        ...
        for (const sk_sp<SkBlender>& blender :
            { SkBlender::Mode(SkBlendMode::kDst), SkBlender::Mode(SkBlendMode::kSrc), SkBlender::Mode(SkBlendMode::kSaturation) }) {
            canvas->save();
            for (uint8_t alpha : { 0xFF, 0x40 })
                for (bool colors : { false, true })
                    for (bool shader : { false, true }) {
                        ...
                        if ((i & 1) == 0) {
                            if (colors) {
                                mesh = SkMesh::Make(fSpecWithColor, SkMesh::Mode::kTriangleStrip, fColorVB,
                                    /*vertexCount= */ 4,
                                    /*vertexOffset=*/0,
                                    /*uniforms=    */ nullptr, kRect);
                            } else {
                                mesh = SkMesh::Make(fSpecWithNoColor, SkMesh::Mode::kTriangleStrip, fNoColorVB,
                            ...
                            if (colors) {
                                mesh = SkMesh::MakeIndexed(fSpecWithColor, SkMesh::Mode::kTriangles, fColorIndexedVB,
                                    /*vertexCount=*/6, kColorIndexedOffset, std::move(ib),
                                    /*indexCount=*/6, kIndexOffset,
                                    /*uniforms=*/nullptr, kRect);
                            } else {
                                mesh = SkMesh::MakeIndexed(fSpecWithNoColor, SkMesh::Mode::kTriangles, fNoColorIndexedVB,
                                    /*vertexCount=*/6,
...
private:
    ...
    {
        if (!fColorVB) {
            fColorVB = SkMesh::MakeVertexBuffer(/*GrDirectContext*=*/nullptr, kColorQuad, sizeof(kColorQuad));
        }
        ...
        if (!fNoColorVB) {
            ...
            std::memcpy(SkTAddOffset<void>(data->writable_data(), kNoColorOffset), kNoColorQuad, sizeof(kNoColorQuad));
            fNoColorVB = SkMesh::MakeVertexBuffer(/*GrDirectContext*=*/nullptr, data->data(), data->size());
        }

        if (!fColorIndexedVB) {
            // This buffer also has an offset.
            auto data = SkData::MakeUninitialized(sizeof(kColorIndexedQuad) + kColorIndexedOffset);
            std::memcpy(SkTAddOffset<void>(data->writable_data(), kColorIndexedOffset), kColorIndexedQuad, sizeof(kColorIndexedQuad));
            fColorIndexedVB = SkMesh::MakeVertexBuffer(/*GrDirectContext*=*/nullptr, data->data(), data->size());
        }

        if (!fNoColorIndexedVB) {
            fNoColorIndexedVB = SkMesh::MakeVertexBuffer(/*GrDirectContext*=*/nullptr, kNoColorIndexedQuad, sizeof(kNoColorIndexedQuad));
        }

        if (!fIB[0]) {
            // The index buffer has an offset.
            auto data = SkData::MakeUninitialized(sizeof(kIndices) + kIndexOffset);
            std::memcpy(SkTAddOffset<void>(data->writable_data(), kIndexOffset), kIndices, sizeof(kIndices));
            fIB[0] = SkMesh::MakeIndexBuffer(/*GrDirectContext*=*/nullptr, data->data(), data->size());
        }
    ...
    void onOnceBeforeDraw() override
    {
        ...
        for (bool unpremul : { false, true }) {
            ...
            for (bool spin : { false, true }) {
                ...
                auto [spec, error] = SkMeshSpecification::Make(kAttributes, sizeof(Vertex), kVaryings, SkString(vs), SkString(kFS), std::move(cs), at);
                if (!spec) {
                    SkDebugf("%s\n", error.c_str());
                }
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* error) override
    {
        ...
        for (bool useShader : { false, true })
            for (bool unpremul : { false, true }) {
                c->save();
                for (bool spin : { false, true }) {
                    SkMesh mesh = SkMesh::Make(fSpecs[SpecIndex(unpremul, spin)], SkMesh::Mode::kTriangleStrip, fVB,
                        /*vertexCount= */ 4,
                        /*vertexOffset=*/0,
                        /*uniforms=    */ nullptr, kRect);
...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        auto [spec, error]
            = SkMeshSpecification::Make(kAttributes, sizeof(Vertex), kVaryings, SkString(kVS), SkString(kFS), SkColorSpace::MakeSRGB(), kPremul_SkAlphaType);
        if (!spec) {
            SkDebugf("%s\n", error.c_str());
        }
        fSpec = std::move(spec);
        ...
        fShader = SkGradientShader::MakeRadial(kGradCenter, .4f, colors, nullptr, 2, SkTileMode::kMirror);
    ...
    SkString onShortName() override
    {
        return SkString("custommesh_uniforms");
    }
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* error) override
    {
        ...
        for (const auto& m : matrices) {
            ...
            SkASSERT(u);
            std::memcpy(SkTAddOffset<void>(unis->writable_data(), u->offset), (void*)&trans, 2 * sizeof(float));
            ...
            SkASSERT(u);
            std::memcpy(SkTAddOffset<void>(unis->writable_data(), u->offset), fColor.vec(), 4 * sizeof(float));

            SkMesh mesh = SkMesh::Make(fSpec, SkMesh::Mode::kTriangleStrip, fVB,
                /*vertexCount= */ 4,
                /*vertexOffset=*/0,
...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        SkPixmap pixmap(SkImageInfo::Make({ 2, 2 }, kBGRA_8888_SkColorType, kPremul_SkAlphaType), colors,
            /*rowBytes=*/8);
        fShader = SkImage::MakeRasterCopy(pixmap)->makeShader(SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions { SkFilterMode::kLinear });
    }

    SkString onShortName() override
    {
        return SkString("mesh_updates");
    }
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* error) override
    {
        ...
        for (bool gpuBuffer : { false, true }) {
            ...
            auto vb = SkMesh::MakeVertexBuffer(ctx, /*data=*/nullptr, kVBRects * 6 * sizeof(Vertex));
            ...
            for (int i = 0; i < kUpdatesRects; ++i) {
                ...
                auto mesh = SkMesh::Make(fSpec, SkMesh::Mode::kTriangles, vb,
                    /*vertexCount=*/6 * rectCount,
                    /*vertexOffset=*/0, nullptr, bounds);
            ...
            vb = SkMesh::MakeVertexBuffer(ctx, /*data=*/nullptr, kNumIBUpdates * 4 * sizeof(Vertex));
            for (int i = 0; i < kNumIBUpdates; ++i) {
                ...
                SkAssertResult(vb->update(ctx, vertices, i * 4 * sizeof(Vertex), 4 * sizeof(Vertex)));
            }

            auto ib = SkMesh::MakeIndexBuffer(ctx, /*data=*/nullptr, kIBRects * 6 * sizeof(uint16_t));

            for (int i = 0; i < kNumIBUpdates; ++i) {
                uint16_t indices[6]
                    = { SkToU16(0 + 4 * i), SkToU16(3 + 4 * i), SkToU16(1 + 4 * i), SkToU16(1 + 4 * i), SkToU16(3 + 4 * i), SkToU16(2 + 4 * i) };
                ...
                auto mesh = SkMesh::MakeIndexed(fSpec, SkMesh::Mode::kTriangles, vb,
                    /*vertexCount= */ 4 * kNumIBUpdates,
                    /*vertexOffset=*/0, ib,
                    /*indexCount= */ 6,
                    /*indexOffset=*/offset,
                    /*uniforms=   */ nullptr, bounds);
...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        auto result = SkMeshSpecification::Make(kAttributes1,
            /*vertexStride==*/12, kVaryings, SkString(kVS), SkString(kFS), SkColorSpace::MakeSRGB(), kPremul_SkAlphaType);
    ...
    SkString onShortName() override
    {
        return SkString("mesh_zero_init");
    }
        ...
        for (bool gpuBuffer : { false, true }) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                for (size_t j = 0; j < std::size(kTri); ++j) {
                    SkAssertResult(vb->update(ctx, &kTri[j], spec->stride() * j + posOffset, sizeof(kTri[j])));
                }
                ...
                auto mesh = SkMesh::MakeIndexed(spec, SkMesh::Mode::kTriangles, std::move(vb),
                    /*vertexCount=*/std::size(kTri),
                    /*vertexOffset=*/0, std::move(ib),
                    /*indexCount=*/std::size(kTiIndices) + 1, indexMeshOffset,
                    /*uniforms=*/nullptr, bounds);
