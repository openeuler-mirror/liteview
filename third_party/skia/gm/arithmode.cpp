    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        while (k < stop) {
            {
                ...
                canvas->translate(gap, 0);
                SkPaint paint;
                paint.setImageFilter(SkImageFilters::Arithmetic(k[0], k[1], k[2], k[3], true, dstFilter, srcFilter, nullptr));
        ...
        for (int i = 0; i < 2; i++) {
...

            {
                ...
                sk_sp<SkImageFilter> bg = SkImageFilters::Arithmetic(0, 0, -one / 2, 1, enforcePMColor, dstFilter, nullptr, nullptr);
                SkPaint p;
                p.setImageFilter(SkImageFilters::Arithmetic(0, one / 2, -one, 1, true, std::move(bg), dstFilter, nullptr));
...
class ArithmodeBlenderGM : public skiagm::GM {
    ...
    SkString onShortName() override
    {
        return SkString("arithmode_blender");
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        canvas->drawImage(fChecker, 0, 0);
        imageFilterPaint.setImageFilter(SkImageFilters::Blend(blender,
            /*background=*/nullptr,
