...
static sk_sp<SkImage> make_img(int w, int h)
{
    ...
    SkColor colors[] = { SK_ColorDKGRAY, ToolUtils::color_to_565(0xFF222255), ToolUtils::color_to_565(0xFF331133), ToolUtils::color_to_565(0xFF884422),
        ToolUtils::color_to_565(0xFF000022), SK_ColorWHITE, ToolUtils::color_to_565(0xFFAABBCC) };
    ...
    canvas->drawSimpleText(kTxt, std::size(kTxt) - 1, SkTextEncoding::kUTF8, texPos.fX, texPos.fY, font, paint);
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        canvas->drawImageRect(fImg, SkRect::MakeIWH(size.fWidth, size.fHeight), SkSamplingOptions(), &bgPaint);
        ...
        for (int doLayer = 0; doLayer <= testLayers; ++doLayer) {
            for (const Clip& clip : fClips) {
                ...
                for (int aa = 0; aa < 2; ++aa) {
                    ...
                    canvas->drawSimpleText(kTxt, std::size(kTxt) - 1, SkTextEncoding::kUTF8, 0, 1.5f * font.getSize(), font, txtPaint);
                    canvas->restore();
                    x += textW + 2 * kMargin;
                }
