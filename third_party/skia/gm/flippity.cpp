...
static const SkMatrix kUVMatrices[kNumMatrices] = { SkMatrix::MakeAll(0, -1, 1, -1, 0, 1, 0, 0, 1), SkMatrix::MakeAll(1, 0, 0, 0, -1, 1, 0, 0, 1),
    // flip x
...
// or top-left.
static sk_sp<SkImage> make_reference_image(GrDirectContext* dContext, const SkTArray<sk_sp<SkImage>>& labels, bool bottomLeftOrigin)
{
    SkASSERT(kNumLabels == labels.count());

    SkImageInfo ii = SkImageInfo::Make(kImageSize, kImageSize, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
    SkBitmap bm;
    bm.allocPixels(ii);
    SkCanvas canvas(bm);
    ...
    for (int i = 0; i < kNumLabels; ++i) {
        canvas.drawImage(labels[i], 0.0 != kPoints[i].fX ? kPoints[i].fX - kLabelSize - kInset : kInset,
    ...
    auto view = sk_gpu_test::MakeTextureProxyViewFromData(dContext, GrRenderable::kNo, origin, bm.pixmap());
    if (!view) {
        return nullptr;
    }

    return sk_make_sp<SkImage_Gpu>(sk_ref_sp(dContext), kNeedNewImageUniqueID, std::move(view), ii.colorInfo());
}
...
private:
    ...
    void drawImageWithMatrixAndLabels(SkCanvas* canvas, SkImage* image, int matIndex, bool drawSubset, bool drawScaled)
    {
        ...
        if (drawSubset) {
            canvas->drawImageRect(image, kSubsets[matIndex], drawScaled ? SkRect::MakeWH(kImageSize, kImageSize) : kSubsets[matIndex], SkSamplingOptions(),
        ...
        // draw the labels
        for (int i = 0; i < kNumLabels; ++i) {
            canvas->drawImage(fLabels[i], 0.0f == kPoints[i].fX ? -kLabelSize : kPoints[i].fX, 0.0f == kPoints[i].fY ? -kLabelSize : kPoints[i].fY);
        }
        canvas->restore();
    }
    ...
    void drawRow(SkCanvas* canvas, bool bottomLeftImage, bool drawSubset, bool drawScaled)
    {
        ...
        for (int i = 0; i < kNumMatrices; ++i) {
            this->drawImageWithMatrixAndLabels(canvas, fReferenceImages[bottomLeftImage].get(), i, drawSubset, drawScaled);
    ...
    void makeLabels(GrDirectContext* dContext)
    {
        ...
        static const SkColor kLabelColors[kNumLabels] = { SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorCYAN };
