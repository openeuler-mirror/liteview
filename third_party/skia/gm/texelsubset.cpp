...
protected:
    ...
    DrawResult onDraw(GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg) override
    {
        ...
        GrMipmapped mipmapped = (fMipmapMode != MipmapMode::kNone) ? GrMipmapped::kYes : GrMipmapped::kNo;
        if (mipmapped == GrMipmapped::kYes && !rContext->priv().caps()->mipmapSupport()) {
            return DrawResult::kSkip;
        }
        auto view = std::get<0>(GrMakeCachedBitmapProxyView(rContext, fBitmap, /*label=*/"DrawResult_Draw_BitMap", mipmapped));
        if (!view) {
        ...
        // will draw.
        if (fUpscale) {
            texelSubset = SkIRect::MakeXYWH(fBitmap.width() / 3 - 1, 2 * fBitmap.height() / 5 - 1, fBitmap.width() / 4 + 2, fBitmap.height() / 5 + 2);
        } else {
            texelSubset = SkIRect::MakeXYWH(fBitmap.width() / 8 - 1, fBitmap.height() / 7 - 1, 3 * fBitmap.width() / 5 + 2, 4 * fBitmap.height() / 5 + 2);
        }

        SkTArray<SkMatrix> textureMatrices;

        SkRect a = SkRect::Make(texelSubset);
        SkRect b = fUpscale ? a.makeInset(.31f * a.width(), .31f * a.height()) : a.makeOutset(.25f * a.width(), .25f * a.height());
        textureMatrices.push_back() = SkMatrix::RectToRect(a, b);

        b = fUpscale ? a.makeInset(.25f * a.width(), .35f * a.height()) : a.makeOutset(.20f * a.width(), .35f * a.height());
        textureMatrices.push_back() = SkMatrix::RectToRect(a, b);
        ...
        auto subsetView = std::get<0>(GrMakeCachedBitmapProxyView(rContext, subsetBmp, /*label=*/"DrawResult_Draw_SubsetBitMap", mipmapped));
        ...
        for (int tm = 0; tm < textureMatrices.count(); ++tm) {
            for (int my = 0; my < GrSamplerState::kWrapModeCount; ++my) {
                ...
                for (int mx = 0; mx < GrSamplerState::kWrapModeCount; ++mx) {
                    ...
                    fp1 = GrTextureEffect::MakeSubset(view, fBitmap.alphaType(), textureMatrices[tm], sampler, SkRect::Make(texelSubset), caps);
                    if (!fp1) {
                        continue;
                    }
                    ...
                    if (auto op = sk_gpu_test::test_ops::MakeRect(rContext, std::move(fp1), drawRect, localRect.makeOffset(kT), SkMatrix::Translate(-kT))) {
                    ...
                    drawRect = localRect.makeOffset(x, y);
                    SkMatrix subsetTextureMatrix = SkMatrix::Concat(SkMatrix::Translate(-texelSubset.topLeft()), textureMatrices[tm]);

                    auto fp2 = GrTextureEffect::Make(subsetView, fBitmap.alphaType(), subsetTextureMatrix, sampler, caps);
                    if (auto op = sk_gpu_test::test_ops::MakeRect(rContext, std::move(fp2), drawRect, localRect)) {
                        sdc->addDrawOp(std::move(op));
                    }
