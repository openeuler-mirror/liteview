...
SkPath (*paths[])(SkScalar, SkScalar, SkScalar) = { generate_square, generate_rect_line, generate_circle, generate_line };
...
DEF_SIMPLE_GM(inverse_paths, canvas, 800, 1200)
{
    ...
    SkRect clipRect = SkRect::MakeLTRB(slideBoundary, slideBoundary, slideBoundary + slideWidth, slideBoundary + slideHeight);
    SkPaint clipPaint;
    ...
    for (size_t styleIndex = 0; styleIndex < std::size(styles); styleIndex++) {
        for (size_t sizeIndex = 0; sizeIndex < std::size(pathSizes); sizeIndex++) {
            SkScalar size = pathSizes[sizeIndex];

            canvas->save();

            for (size_t widthIndex = 0; widthIndex < std::size(strokeWidths); widthIndex++) {
                SkPaint paint;
                ...
                for (size_t pathIndex = 0; pathIndex < std::size(paths); pathIndex++) {
...
DEF_SIMPLE_GM(inverse_windingmode_filters, canvas, 256, 100)
{
    ...
    for (auto fillType : { SkPathFillType::kWinding, SkPathFillType::kEvenOdd, SkPathFillType::kInverseWinding, SkPathFillType::kInverseEvenOdd }) {
