...
protected:
    ...
    {
        SkString str;
        str.printf("complexclip_%s%s%s", fDoAAClip ? "aa" : "bw", fDoSaveLayer ? "_layer" : "", fInvertDraw ? "_invert" : "");
        return str;
    }

    SkISize onISize() override
    {
        return SkISize::Make(388, 780);
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        if (fDoSaveLayer) {
            ...
            SkRect bounds = SkRect::MakeLTRB(
                4.0f / 3.0f * -20, 4.0f / 3.0f * -20, 4.0f / 3.0f * (this->getISize().fWidth - 20), 4.0f / 3.0f * (this->getISize().fHeight - 20));
        ...
        for (int invBits = 0; invBits < 4; ++invBits) {
            ...
            for (size_t op = 0; op < std::size(gOps); ++op) {
                ...
                canvas->drawSimpleText(gOps[op].fName, strlen(gOps[op].fName), SkTextEncoding::kUTF8, txtX, 220, font, paint);
...
private:
    void drawHairlines(SkCanvas* canvas, const SkPath& path, const SkPath& clipA, const SkPath& clipB)
    {
        SkPaint paint;
...
DEF_SIMPLE_GM(clip_shader, canvas, 840, 650)
{
    ...
    canvas->clipShader(img->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), lm));
    canvas->drawImage(img, 0, 0);
...
DEF_SIMPLE_GM(clip_shader_nested, canvas, 256, 256)
{
    ...
    auto s = SkGradientShader::MakeRadial({ 0.5f * w, 0.5f * h }, 0.1f * w, gradColors, nullptr, 2, SkTileMode::kRepeat, 0, nullptr);
...
static void draw_banner(SkCanvas* canvas, Config config)
{
    ...
    if (config.fConcat == kConcatBeforeClips || config.fLM == kBothWithLocalMat) {
        ...
        SkASSERT((config.fConcat == kConcatBetweenClips && config.fLM == kNoLocalMat)
            || (config.fConcat == kConcatAfterClips && (config.fLM == kImageWithLocalMat || config.fLM == kGradientWithLocalMat)));
        if ((config.fConcat == kConcatBetweenClips && config.fOrder == kClipImageFirst) || config.fLM == kGradientWithLocalMat) {
...
DEF_SIMPLE_GM(clip_shader_persp, canvas, 1370, 1030)
{
    ...
    Config matches[][2] = { // Everything has perspective
        { { kConcatBeforeClips, kDoesntMatter, kNoLocalMat }, { kConcatAfterClips, kDoesntMatter, kBothWithLocalMat } },
        // Image shader has perspective
        { { kConcatBetweenClips, kClipGradientFirst, kNoLocalMat }, { kConcatAfterClips, kDoesntMatter, kImageWithLocalMat } },
        // Gradient shader has perspective
        { { kConcatBetweenClips, kClipImageFirst, kNoLocalMat }, { kConcatAfterClips, kDoesntMatter, kGradientWithLocalMat } }
    };
    ...
    SkPoint dst[4] = { { 0, 80.f }, { img->width() + 28.f, -100.f }, { img->width() - 28.f, img->height() + 100.f }, { 0.f, img->height() - 80.f } };
    SkMatrix persp;
    ...
    auto drawConfig = [&](Config config) {
        ...
        auto gradShader = SkGradientShader::MakeRadial(
            { 0.5f * img->width(), 0.5f * img->height() }, 0.1f * img->width(), gradColors, nullptr, 2, SkTileMode::kRepeat, 0, gradLM ? &persp : nullptr);
        bool imageLM = config.fLM == kImageWithLocalMat || config.fLM == kBothWithLocalMat;
        auto imgShader = img->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), imageLM ? perspScale : scale);
...
DEF_SIMPLE_GM(clip_shader_difference, canvas, 512, 512)
{
    ...
    SkMatrix local = SkMatrix::RectToRect(SkRect::MakeWH(image->width(), image->height()), SkRect::MakeWH(64, 64));
    auto shader = image->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), &local);
