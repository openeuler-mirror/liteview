...
DEF_SIMPLE_GM(arcto, canvas, 500, 600)
{
    ...
    for (SkScalar angle = 0; angle <= 45; angle += 45) {
        for (int oHeight = 2; oHeight >= 1; --oHeight) {
            ...
            svgArc.arcTo({ oval.width() / 2, ovalHeight }, angle, SkPathBuilder::kSmall_ArcSize, SkPathDirection::kCW, { oval.right(), oval.bottom() });
            canvas->drawPath(svgArc.detach(), paint);

            svgArc.moveTo(oval.fLeft + 100, oval.fTop + 100);
            svgArc.arcTo({ oval.width() / 2, ovalHeight }, angle, SkPathBuilder::kLarge_ArcSize, SkPathDirection::kCCW, { oval.right(), oval.bottom() + 100 });
            canvas->drawPath(svgArc.detach(), paint);
            oval.offset(50, 0);
        }
    }
    ...
    const char* arcstrs[]
        = { "M250,400  A120,80 0 0,0 250,500", "M250,400  A120,80 0 1,1 250,500", "M250,400  A120,80 0 1,0 250,500", "M250,400  A120,80 0 0,1 250,500" };
    int cIndex = 0;
    ...
    SkPathBuilder path;
    path.moveTo(100, 100).arcTo({ 0, 0 }, 0, SkPathBuilder::kLarge_ArcSize, SkPathDirection::kCW, { 200, 200 });
    canvas->drawPath(path.detach(), paint);

    path.moveTo(200, 100).arcTo({ 80, 80 }, 0, SkPathBuilder::kLarge_ArcSize, SkPathDirection::kCW, { 200, 100 });
    canvas->drawPath(path.detach(), paint);
}
        ...
        for (int yStart = 0; yStart < kParsePathTestDimension; yStart += 100) {
            ...
            do {
                ...
                if (GENERATE_SVG_REFERENCE) {
                    ...
                    str.printf("\n' fill='#%06x' fill-opacity='%g'", paint.getColor() & 0xFFFFFF, paint.getAlpha() / 255.f);
                    sk_fwrite(str.c_str(), str.size(), file);
