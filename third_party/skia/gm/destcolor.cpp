...
DEF_SIMPLE_GPU_GM_CAN_FAIL(destcolor, rContext, canvas, errorMsg, 640, 640)
{
    ...
    // Draw the mandrill.
    SkPaint p;
    p.setImageFilter(SkImageFilters::Image(GetResourceAsImage("images/mandrill_512.png"), bounds, bounds, SkSamplingOptions()));
    canvas->drawPaint(p);
    ...
    sdc->drawOval(/*clip*/ nullptr, std::move(invertPaint), GrAA::kYes, SkMatrix::I(), SkRect::MakeLTRB(128, 128, 640, 640), GrStyle::SimpleFill());
