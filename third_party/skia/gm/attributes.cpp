...
private:
    AttributeTestProcessor(AttrMode mode)
        : GrGeometryProcessor(kAttributeTestProcessor_ClassID)
        , fMode(mode)
    {
        switch (fMode) {
        case AttrMode::kAuto:
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> AttributeTestProcessor::makeProgramImpl(const GrShaderCaps&) const
{
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            args.fVaryingHandler->addPassThroughAttribute(GrShaderVar("color", SkSLType::kHalf4), args.fOutputColor);
...
private:
    AttributeTestOp(AttrMode mode, SkRect rect)
        : GrDrawOp(ClassID())
        , fMode(mode)
        , fRect(rect)
    {
    ...
    template <typename V> void makeVB(GrOpFlushState* flushState, const SkRect rect)
    {
        ...
        fVertexBuffer = flushState->resourceProvider()->createBuffer(v, sizeof(v), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
    }
