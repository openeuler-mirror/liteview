...
private:
    ...
    static sk_sp<SkImage> MakeImage(const SkVector& vec, SkColor color)
    {
        ...
        const SkPoint end = SkPoint::Make(start.x() + vec.x() * (kSegLen - 1), start.y() + vec.y() * (kSegLen - 1));
    ...
    void drawSets(SkCanvas* canvas) const
    {
        ...
        for (int i = 0; i < fSets.count(); ++i) {
            ...
            canvas->translate(lastPt.x() + kSegLen, -SkIntToScalar(kSegLen + 4) * std::size(samplings) * std::size(AAs));
        }
    }

    SkPoint drawSet(SkCanvas* canvas, const ImageSet& set, const SkSamplingOptions& sampling, const SkPaint& paint) const
    {
        ...
        for (int i = 0; i < set.fImages.count(); ++i) {
            ...
            const SkRect dst = SkRect::MakeXYWH(
                pt.x(), pt.y(), img->width() * (1 + (set.fScales[i] - 1) * set.fVector.x()), img->height() * (1 + (set.fScales[i] - 1) * set.fVector.y()));
