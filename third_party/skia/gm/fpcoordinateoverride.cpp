...
DEF_SIMPLE_GPU_GM_BG(fpcoordinateoverride, rContext, canvas, 512, 512, ToolUtils::color_to_565(0xFF66AA99))
{
    ...
    auto view = std::get<0>(GrMakeCachedBitmapProxyView(rContext, bmp, /*label=*/"FpCoordinateOverride", GrMipmapped::kNo));
    if (!view) {
        return;
    }
    std::unique_ptr<GrFragmentProcessor> imgFP = GrTextureEffect::Make(std::move(view), bmp.alphaType(), SkMatrix());
