...
static sk_sp<SkImage> makebm(int w, int h)
{
    ...
    constexpr SkColor colors[] = { SK_ColorRED, SK_ColorYELLOW, SK_ColorGREEN, SK_ColorMAGENTA, SK_ColorBLUE, SK_ColorCYAN, SK_ColorRED };

    constexpr SkScalar pos[] = { 0, SK_Scalar1 / 6, 2 * SK_Scalar1 / 6, 3 * SK_Scalar1 / 6, 4 * SK_Scalar1 / 6, 5 * SK_Scalar1 / 6, SK_Scalar1 };
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int w = 1; w <= kMaxSrcRectSize; w *= 3) {
            for (int h = 1; h <= kMaxSrcRectSize; h *= 3) {
                ...
                canvas->drawImageRect(fImage.get(), SkRect::Make(srcRect), dstRect, SkSamplingOptions(), &paint, SkCanvas::kFast_SrcRectConstraint);
                canvas->restore();
