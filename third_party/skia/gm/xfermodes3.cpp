...
public:
    Xfermodes3GM()
    {
        this->setBGColor(ToolUtils::color_to_565(0xFF70D0E0));
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t s = 0; s < std::size(kStrokes); ++s) {
            for (size_t m = 0; m < kSkBlendModeCount; ++m) {
                ...
                canvas->drawString(SkBlendMode_Name(mode), SkIntToScalar(x), SkIntToScalar(y + kSize + 3) + font.getSize(), font, labelP);
...
private:
    ...
    sk_sp<SkSurface> makeTempSurface(SkCanvas* baseCanvas, int w, int h)
    {
        ...
        SkImageInfo info = SkImageInfo::Make(w, h, baseInfo.colorType(), baseInfo.alphaType(), baseInfo.refColorSpace());
        return baseCanvas->makeSurface(info);
    }

    void drawMode(SkCanvas* canvas, int x, int y, int w, int h, const SkPaint& modePaint, SkSurface* surface)
    {
        canvas->save();
    ...
    {
        const uint32_t kCheckData[] = { SkPackARGB32(0xFF, 0x42, 0x41, 0x42), SkPackARGB32(0xFF, 0xD6, 0xD3, 0xD6), SkPackARGB32(0xFF, 0xD6, 0xD3, 0xD6),
        ...
        bmpPaint.setShader(SkGradientShader::MakeRadial(kCenter, 3 * SkIntToScalar(kSize) / 4, kColors, nullptr, std::size(kColors), SkTileMode::kRepeat));
        ...
        SkRect rect = { SkIntToScalar(kSize) / 8, SkIntToScalar(kSize) / 8, 7 * SkIntToScalar(kSize) / 8, 7 * SkIntToScalar(kSize) / 8 };
