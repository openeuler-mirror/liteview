...
sk_sp<SkTextBlob> MakeFancyBlob(const SkPaint& paint, const SkFont& font, const char* text)
{
    ...
    // Horizontal-positioned run.
    {
        ...
        memcpy(buf.glyphs, glyphs.get() + glyphIndex, SkTo<uint32_t>(horizontalRunLen) * sizeof(SkGlyphID));
...
{
    return SkTextBlob::MakeFromText(glyphs, count * sizeof(SkGlyphID), font, SkTextEncoding::kGlyphID);
}
...
static sk_sp<SkTextBlob> make_posh(const SkFont& font, const SkGlyphID glyphs[], int count, SkScalar spacing)
{
    ...
    return SkTextBlob::MakeFromPosTextH(glyphs, count * sizeof(SkGlyphID), xpos.get(), 0, font, SkTextEncoding::kGlyphID);
}
...
static sk_sp<SkTextBlob> make_pos(const SkFont& font, const SkGlyphID glyphs[], int count, SkScalar spacing)
{
    ...
    return SkTextBlob::MakeFromPosText(glyphs, count * sizeof(SkGlyphID), pos.get(), font, SkTextEncoding::kGlyphID);
}
