...
private:
    ...
    {
        return SkISize::Make(kDeviceRect.width(), kDeviceRect.height());
    }
    SkString onShortName() override
    {
        return SkString("windowrectangles");
    }
    DrawResult onDraw(SkCanvas*, SkString* errorMsg) override;
};

DrawResult WindowRectanglesGM::coverClipStack(const SkClipStack& stack, SkCanvas* canvas, SkString* errorMsg)
{
    SkPaint paint;
...
DrawResult WindowRectanglesGM::onDraw(SkCanvas* canvas, SkString* errorMsg)
{
    ...
    SkClipStack stack;
    stack.clipRect(SkRect::MakeXYWH(370.75, 80.25, 149, 100), SkMatrix::I(), SkClipOp::kDifference, false);
    stack.clipRect(SkRect::MakeXYWH(80.25, 420.75, 150, 100), SkMatrix::I(), SkClipOp::kDifference, true);
    stack.clipRRect(SkRRect::MakeRectXY(SkRect::MakeXYWH(200, 200, 200, 200), 60, 45), SkMatrix::I(), SkClipOp::kDifference, true);
