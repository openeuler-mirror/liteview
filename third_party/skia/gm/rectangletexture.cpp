...
private:
    ...
    sk_sp<SkImage> createRectangleTextureImg(GrDirectContext* dContext, GrSurfaceOrigin origin, const SkBitmap content)
    {
        SkASSERT(content.colorType() == kRGBA_8888_SkColorType);
        auto format = GrBackendFormat::MakeGL(GR_GL_RGBA8, GR_GL_TEXTURE_RECTANGLE);
        auto bet = dContext->createBackendTexture(content.width(), content.height(), format, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo,
    ...
    DrawResult onGpuSetup(GrDirectContext* context, SkString* errorMsg) override
    {
        ...
        if (context->backend() != GrBackendApi::kOpenGL_GrBackend || !static_cast<const GrGLCaps*>(context->priv().caps())->rectangleTextureSupport()) {
        ...
        fGradImgs[1] = this->createRectangleTextureImg(context, kBottomLeft_GrSurfaceOrigin, gradCircle);
        ...
        fSmallImg = this->createRectangleTextureImg(context, kTopLeft_GrSurfaceOrigin, this->makeImagePixels(2, ImageType::k2x2));
        if (!fSmallImg) {
        ...
        for (size_t i = 0; i < kNumGradImages; ++i) {
            ...
            for (auto scale : kScales) {
                ...
                for (auto s : kSamplings) {
                    ...
                    SkPaint repeatPaint;
                    repeatPaint.setShader(fGradImgs[i]->makeShader(SkTileMode::kRepeat, SkTileMode::kMirror, s));
                    ...
                    canvas->drawImageRect(fGradImgs[i], srcRect, dstRect, s, nullptr, SkCanvas::kStrict_SrcRectConstraint);
        ...
        for (const auto& sampling : gSamplings) {
            ...
            for (int ty = 0; ty < kSkTileModeCount; ++ty) {
                ...
                for (int tx = 0; tx < kSkTileModeCount; ++tx) {
                    ...
                    SkPaint paint;
                    paint.setShader(fSmallImg->makeShader(static_cast<SkTileMode>(tx), static_cast<SkTileMode>(ty), sampling, lm));
