...
int make_bm(SkBitmap* bm, int height)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        SkBitmap smallBM;
        SkIRect subRect = SkIRect::MakeXYWH(0, i * (kMargin + 2 * kRadius), 2 * kRadius + kMargin, 2 * kRadius + kMargin);
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t i = 0; i < std::size(fTallBmps); ++i) {
            ...
            canvas->drawImageRect(ToolUtils::MakeTextureImage(canvas, bmp.asImage()), SkRect::Make(subRect), dstRect, SkSamplingOptions(SkFilterMode::kLinear),
