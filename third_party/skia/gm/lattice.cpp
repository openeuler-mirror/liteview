...
protected:
    ...
    void onDrawHelper(GrDirectContext* dContext, SkCanvas* canvas, int padLeft, int padTop, int padRight, int padBottom)
    {
        ...
        SkBitmap bitmap;
        sk_sp<SkImage> image = make_image(canvas, xDivs + 1, yDivs + 1, padLeft, padTop, padRight, padBottom);
        ...
        SkIRect bounds = SkIRect::MakeLTRB(padLeft, padTop, image->width() - padRight, image->height() - padBottom);
        lattice.fBounds = (bounds == SkIRect::MakeWH(image->width(), image->height())) ? nullptr : &bounds;
...
public:
    ...
    void onDrawHelper(SkCanvas* canvas, int padLeft, int padTop, int padRight, int padBottom, SkPaint& paint)
    {
        ...
        SkColor colors[9] = { SK_ColorBLACK, SK_ColorBLACK, SK_ColorBLACK, 0xFFFF0000, 0x880000FF, 0xFF00FF00, SK_ColorBLACK, SK_ColorBLACK, SK_ColorBLACK };
        lattice.fColors = colors;
        paint.setColor(0xFFFFFFFF);
        canvas->drawImageLattice(image.get(), lattice, SkRect::MakeXYWH(100, 100, 200, 200), SkFilterMode::kNearest, &paint);
...
DEF_SIMPLE_GM_BG(lattice_alpha, canvas, 120, 120, SK_ColorWHITE)
{
    ...
    canvas->drawImageLattice(image.get(), lattice, SkRect::MakeWH(120, 120), SkFilterMode::kNearest, &paint);
}
