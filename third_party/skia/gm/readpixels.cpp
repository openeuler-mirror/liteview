...
static sk_sp<SkImage> make_raster_image(SkColorType colorType)
{
    ...
    SkImageInfo info = codec->getInfo().makeWH(kWidth, kHeight).makeColorType(colorType).makeAlphaType(kPremul_SkAlphaType);
...
static sk_sp<SkImage> make_picture_image()
{
    ...
    return SkImage::MakeFromPicture(
        recorder.finishRecordingAsPicture(), SkISize::Make(kWidth, kHeight), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}
...
static void draw_image(GrDirectContext* dContext, SkCanvas* canvas, SkImage* image, SkColorType dstColorType, SkAlphaType dstAlphaType,
    sk_sp<SkColorSpace> dstColorSpace, SkImage::CachingHint hint)
{
    ...
    SkImageInfo dstInfo = SkImageInfo::Make(image->width(), image->height(), dstColorType, dstAlphaType, dstColorSpace);
                ...
                if (image) {
                    for (SkColorType dstColorType : colorTypes) {
                        for (SkAlphaType dstAlphaType : alphaTypes) {
                            draw_image(dContext, canvas, image.get(), dstColorType, dstAlphaType, dstColorSpace, SkImage::kAllow_CachingHint);
            ...
            for (SkColorType dstColorType : colorTypes) {
                for (SkAlphaType dstAlphaType : alphaTypes) {
                    for (SkImage::CachingHint hint : hints) {
                        draw_image(nullptr, canvas, image.get(), dstColorType, dstAlphaType, dstColorSpace, hint);
