...
static sk_sp<SkShader> shader_radial(const SkColor colors[], int count, const SkSize& size)
{
    ...
    return SkGradientShader::MakeRadial(center, size.width() / 2, colors, nullptr, count, SkTileMode::kClamp);
}

static sk_sp<SkShader> shader_conical(const SkColor colors[], int count, const SkSize& size)
{
    SkPoint center = { size.width() / 2, size.height() / 2 };
    return SkGradientShader::MakeTwoPointConical(center, size.width() / 64, center, size.width() / 2, colors, nullptr, count, SkTileMode::kClamp);
}
