...
static sk_sp<SkShader> make_shader(const SkRect& bounds)
{
    ...
    return SkGradientShader::MakeLinear(pts, colors, nullptr, std::size(colors), SkTileMode::kClamp);
}
...
class ColorFiltersGM : public skiagm::GM {
    ...
    {
        return SkString("lightingcolorfilter");
    }
...
protected:
    SkString onShortName() override
    {
        return SkString("hslcolorfilter");
    }
    ...
    void onOnceBeforeDraw() override
    {
        ...
        const auto lm = SkMatrix::RectToRect(SkRect::MakeWH(mandrill->width(), mandrill->height()), SkRect::MakeWH(kWheelSize, kWheelSize));
        ...
        for (const auto& cols : gGrads) {
            fShaders.push_back(
                SkGradientShader::MakeSweep(kWheelSize / 2, kWheelSize / 2, cols, nullptr, std::size(cols), SkTileMode::kRepeat, -90, 270, 0, nullptr));
        }
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (const auto& shader : fShaders) {
            ...
            for (const auto& tst : gTests) {
                ...
                const auto dh = (std::get<1>(tst.h) - std::get<0>(tst.h)) / (kSteps - 1), ds = (std::get<1>(tst.s) - std::get<0>(tst.s)) / (kSteps - 1),
...
private:
    ...
    static sk_sp<SkColorFilter> make_filter(float h, float s, float l)
    {
        ...
        const auto h_bias = h, h_scale = 1.0f, s_bias = std::max(s, 0.0f), s_scale = 1 - std::abs(s), l_bias = std::max(l, 0.0f), l_scale = 1 - std::abs(l);
