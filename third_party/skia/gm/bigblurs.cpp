...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        SkPath rectori = SkPathBuilder().addRect(bigRect).addRect(insetRect, SkPathDirection::kCCW).detach();
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j <= kLastEnum_SkBlurStyle; ++j) {
                ...
                for (int k = 0; k < (int)std::size(origins); ++k) {
                    ...
                    SkRect clipRect
                        = SkRect::MakeXYWH(SkIntToScalar(desiredX), SkIntToScalar(desiredY), SkIntToScalar(kCloseUpSize), SkIntToScalar(kCloseUpSize));
