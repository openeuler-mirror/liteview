...
public:
    CGGraphicsPort(SkCanvas* canvas)
        : GraphicsPort(canvas)
    {
    }
    ...
    void drawRect(const SkRect& r, SkColor c) override
    {
        ...
        CGColorRef color = CGColorCreateGenericRGB(SkColorGetR(c) / 255.f, SkColorGetG(c) / 255.f, SkColorGetB(c) / 255.f, SkColorGetA(c) / 255.f);
...
static CGAffineTransform matrix_to_transform(CGContextRef cg, const SkMatrix& ctm)
{
    ...
    return CGAffineTransformMake(matrix[SkMatrix::kMScaleX], matrix[SkMatrix::kMSkewY], matrix[SkMatrix::kMSkewX], matrix[SkMatrix::kMScaleY],
...
DEF_SIMPLE_GM(rasterallocator, canvas, 600, 300)
{
    ...
    std::unique_ptr<SkCanvas> nativeCanvas = SkRasterHandleAllocator::MakeCanvas(std::make_unique<MyAllocator>(), info);
