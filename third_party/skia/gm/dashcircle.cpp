...
struct DashExample {
    int* pattern;
    int length;
} dashExamples[] = { { dash1, std::size(dash1) }, { dash2, std::size(dash2) }, { dash3, std::size(dash3) }, { dash4, std::size(dash4) } };
...
protected:
    ...
    {
        return SkString("dashcircle");
    }

    SkISize onISize() override
    {
        return SkISize::Make(900, 1200);
    }
...
protected:
    SkString onShortName() override
    {
        return SkString("dashcircle2");
    }

    SkISize onISize() override
    {
        return SkISize::Make(635, 900);
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        static constexpr SkScalar kIntervals[][2] {
            { 0.333f, 0.333f }, { 0.015f, 0.015f }, { 0.01f, 0.09f }, { 0.097f, 0.003f }, { 0.02f, 0.04f }, { 0.1f, 0.2f }, { 0.25f, 0.25f },
            { 0.6f, 0.7f }, // adds to > 1
            { 1.2f, 0.8f }, // on is > 1
            { 0.1f, 1.1f }, // off is > 1*/
        };
        ...
        for (int i = 0; i < kN; ++i) {
            ...
            SkScalar scaledIntervals[2] = { kCircumference * kIntervals[i][0], kCircumference * kIntervals[i][1] };
            ...
            thinDEffects[i] = SkDashPathEffect::Make(scaledIntervals, 2, kThinCircumference * fPhaseDegrees * kTau / 360.f);
        }

        SkMatrix rotate;
        rotate.setRotate(25.f);
        const SkMatrix kMatrices[] { SkMatrix::I(), SkMatrix::Scale(1.2f, 1.2f), SkMatrix::MakeAll(1, 0, 0, 0, -1, 0, 0, 0, 1), // y flipper
            SkMatrix::MakeAll(-1, 0, 0, 0, 1, 0, 0, 0, 1), // x flipper
            SkMatrix::Scale(0.7f, 0.7f), rotate, SkMatrix::Concat(SkMatrix::Concat(SkMatrix::MakeAll(-1, 0, 0, 0, 1, 0, 0, 0, 1), rotate), rotate) };
