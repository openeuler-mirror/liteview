...
static sk_sp<SkImage> make_image(SkColorType colorType, SkAlphaType alphaType)
{
    ...
    case kBGR_101010x_SkColorType:
        ...
        break;
    default:
        resource = (kOpaque_SkAlphaType == alphaType) ? "images/color_wheel.jpg" : "images/rainbow-gradient.png";
        break;
    }
    ...
    auto surface = SkSurface::MakeRaster(SkImageInfo::Make(image->width(), image->height(), colorType, alphaType, image->refColorSpace()));
...
protected:
    ...
    {
        const char* variant = fVariant == Variant::kOpaque ? "opaque-" : fVariant == Variant::kGray ? "gray-" : "";
    ...
    DrawResult onDraw(SkCanvas* canvas, SkString* errorMsg) override
    {
        ...
        case Variant::kOpaque:
            if (colorType != kRGB_565_SkColorType && colorType != kRGB_888x_SkColorType && colorType != kRGB_101010x_SkColorType
                && colorType != kBGR_101010x_SkColorType) {
                return DrawResult::kSkip;
            }
            break;
        case Variant::kNormal:
            if (colorType != kARGB_4444_SkColorType && colorType != kRGBA_8888_SkColorType && colorType != kBGRA_8888_SkColorType
                && colorType != kRGBA_1010102_SkColorType && colorType != kBGRA_1010102_SkColorType && colorType != kRGBA_F16Norm_SkColorType
...
#define DEF_ENCODE_GM(format, quality, variant, name)                                                                                                          \
    static skiagm::GMRegistry SK_MACRO_CONCAT(SK_MACRO_APPEND_LINE(REG_), variant)(                                                                            \
        []() { return std::unique_ptr<skiagm::GM>([]() { return new EncodeColorTypesGM(format, quality, Variant::variant, name); }()); });
