...
constexpr drawMth gDrawMthds[] = { draw_rect, draw_oval, draw_rrect, draw_drrect, draw_path, draw_points, draw_bitmap };
...
{
    {
        ...
        sk_sp<SkImageFilter> scaleMIF(SkImageFilters::MatrixTransform(scale, SkSamplingOptions(SkFilterMode::kLinear), source));
...

    {
        ...
        sk_sp<SkImageFilter> rotMIF(SkImageFilters::MatrixTransform(rot, SkSamplingOptions(SkFilterMode::kLinear), source));
    ...
    {
        sk_sp<SkImageFilter> dsif = SkImageFilters::DropShadow(10.0f, 10.0f, 3.0f, 3.0f, SK_ColorRED, source);

        add_paint(paints, std::move(dsif));
    }

    {
        sk_sp<SkImageFilter> dsif = SkImageFilters::DropShadowOnly(27.0f, 27.0f, 3.0f, 3.0f, SK_ColorRED, source);
...
protected:
    ...
    {
        return SkString("filterfastbounds");
    }

    SkISize onISize() override
    {
        return SkISize::Make((std::size(gDrawMthds) + kNumXtraCols) * kTileWidth, kNumVertTiles * kTileHeight);
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int i = 1; i < paints.count(); ++i) {
            canvas->drawLine(0, i * SkIntToScalar(kTileHeight), SkIntToScalar((std::size(gDrawMthds) + kNumXtraCols) * kTileWidth),
                i * SkIntToScalar(kTileHeight), blackFill);
        }
        // vertical separators
        for (int i = 0; i < (int)std::size(gDrawMthds) + kNumXtraCols; ++i) {
            canvas->drawLine(SkIntToScalar(i * kTileWidth), 0, SkIntToScalar(i * kTileWidth), SkIntToScalar(paints.count() * kTileWidth), blackFill);
        }
        ...
        for (int i = 0; i < (int)std::size(gDrawMthds); ++i) {
            for (int j = 0; j < paints.count(); ++j) {
                draw_geom_with_paint(*gDrawMthds[i], SkIPoint::Make((i + kNumXtraCols) * kTileWidth, j * kTileHeight), canvas, paints[j]);
            }
        }
    }
