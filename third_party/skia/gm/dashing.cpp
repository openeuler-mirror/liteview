...
static void drawline(SkCanvas* canvas, int on, int off, const SkPaint& paint, SkScalar finalX = SkIntToScalar(600), SkScalar finalY = SkIntToScalar(0),
    SkScalar phase = SkIntToScalar(0), SkScalar startX = SkIntToScalar(0), SkScalar startY = SkIntToScalar(0))
{
    SkPaint p(paint);
...
{
    return SkPathBuilder().moveTo(bounds.left(), bounds.top()).lineTo(bounds.right(), bounds.bottom()).detach();
}
...
class Dashing3GM : public skiagm::GM {
    ...
    void drawDashedLines(SkCanvas* canvas, SkScalar lineLength, SkScalar phase, SkScalar dashLength, int strokeWidth, bool circles)
    {
        SkPaint p;
...
class Dashing4GM : public skiagm::GM {
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int width = 0; width <= 2; ++width) {
            for (const Intervals& data : { Intervals { 1, 1 }, Intervals { 4, 2 }, Intervals { 0, 4 } }) { // test for zero length on interval.
...
private:
    ...
    SkString onShortName() override
    {
        return SkString(fDoAA ? "dashing5_aa" : "dashing5_bw");
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        constexpr SkColor gColors[kIntervalLength]
            = { SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE, SK_ColorCYAN, SK_ColorMAGENTA, SK_ColorYELLOW, SK_ColorGRAY, SK_ColorDKGRAY };
        ...
        for (int x = 0; x < 200; x += 10) {
            ...
            sign = (x % 20) ? 1 : -1;
            drawline(
                canvas, kOn, kOff, paint, SkIntToScalar(x), -sign * SkIntToScalar(10003), SkIntToScalar(phase), SkIntToScalar(x), sign * SkIntToScalar(10003));
        ...
        for (int y = -400; y < 0; y += 10) {
            ...
            sign = (y % 20) ? 1 : -1;
            drawline(
                canvas, kOn, kOff, paint, -sign * SkIntToScalar(10003), SkIntToScalar(y), SkIntToScalar(phase), sign * SkIntToScalar(10003), SkIntToScalar(y));
            phase = (phase + 1) % kIntervalLength;
        }
    }
    ...
    for (int x = 32; x < 256; x += 16) {
        for (SkScalar a = 0; a < 3.141592f * 2; a += 0.03141592f) {
            SkPoint pts[2] = { { 256 + (float)sin(a) * x, 256 + (float)cos(a) * x },
                { 256 + (float)sin(a + 3.141592 / 3) * (x + 64), 256 + (float)cos(a + 3.141592 / 3) * (x + 64) } };
            lines.moveTo(pts[0]);
