...
static void paint_rgn(SkCanvas* canvas, const SkAAClip& clip, const SkPaint& paint)
{
    ...
    canvas->drawImage(bm2.asImage(), SK_Scalar1 * mask.fBounds.fLeft, SK_Scalar1 * mask.fBounds.fTop, SkSamplingOptions(), &paint);
}
...
protected:
    ...
    void drawPathsOped(SkCanvas* canvas, SkClipOp op, SkColor color)
    {
        ...
        SkRect r = SkRect::MakeLTRB(SkIntToScalar(90), SkIntToScalar(90), SkIntToScalar(180), SkIntToScalar(180));
    ...
    {
        SkString str;
        str.printf("simpleaaclip_%s", kRect_GeomType == fGeomType ? "rect" : (kPath_GeomType == fGeomType ? "path" : "aaclip"));
        return str;
    }
