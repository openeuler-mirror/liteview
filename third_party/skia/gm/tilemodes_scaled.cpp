...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t kx = 0; kx < std::size(gModes); kx++) {
            for (size_t ky = 0; ky < std::size(gModes); ky++) {
                ...
                SkTextUtils::DrawString(canvas, str.c_str(), scale * (x + r.width() / 2), y, font, SkPaint(), SkTextUtils::kCenter_Align);
    ...
    // on SGX where resizing NPOT textures to POT textures exhibits a driver bug.
                ...
                canvas->drawString(SkStringPrintf("%s, %s", gColorTypeNames[i], gFilterNames[j]), scale * x, scale * (y + r.height() * 2 / 3), font, textPaint);
