...
protected:
    ...
    DrawResult createYCbCrImage(GrDirectContext* dContext, SkString* errorMsg)
    {
        ...
        fYCbCrImage = SkImage::MakeFromTexture(dContext, ycbcrHelper->backendTexture(), kTopLeft_GrSurfaceOrigin, kRGB_888x_SkColorType, kPremul_SkAlphaType,
            nullptr, release_ycbcrhelper, ycbcrHelper.get());
        ycbcrHelper.release();
        if (!fYCbCrImage) {
