...
static sk_sp<SkData> make_compressed_data()
{
    ...
    size_t totalSize = SkCompressedDataSize(SkImage::CompressionType::kBC1_RGB8_UNORM, dim, nullptr, false);
...
static sk_sp<SkImage> data_to_img(GrDirectContext* direct, sk_sp<SkData> data, SkImage::CompressionType compression)
{
    if (direct) {
        return SkImage::MakeTextureFromCompressed(direct, std::move(data), kImgWidth, kImgHeight, compression, GrMipmapped::kNo);
    } else {
...
static void draw_image(SkCanvas* canvas, sk_sp<SkImage> image, int x, int y)
{
    ...
    if (image && image->isTextureBacked()) {
        ...
        GrTextureProxy* proxy = sk_gpu_test::GetTextureImageProxy(image.get(), canvas->recordingContext());
...
protected:
    ...
    DrawResult onGpuSetup(GrDirectContext* dContext, SkString* errorMsg) override
    {
        ...
        fRGBAImage = data_to_img(dContext, std::move(bc1Data), SkImage::CompressionType::kBC1_RGBA8_UNORM);
