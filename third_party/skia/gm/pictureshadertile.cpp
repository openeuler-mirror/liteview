...
static void draw_scene(SkCanvas* canvas, SkScalar pictureSize)
{
    ...
    canvas->drawRect(SkRect::MakeXYWH(pictureSize / 2, pictureSize / 2, pictureSize / 2, pictureSize / 2), paint);

    paint.setColor(SK_ColorRED);
    canvas->drawLine(pictureSize / 2, pictureSize * 1 / 3, pictureSize / 2, pictureSize * 2 / 3, paint);
    canvas->drawLine(pictureSize * 1 / 3, pictureSize / 2, pictureSize * 2 / 3, pictureSize / 2, paint);
...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        pictureCanvas = recorder.beginRecording(SkRect::MakeXYWH(offset.x(), offset.y(), kPictureSize, kPictureSize));
        ...
        for (unsigned i = 0; i < std::size(tiles); ++i) {
            SkRect tile = SkRect::MakeXYWH(tiles[i].x * kPictureSize, tiles[i].y * kPictureSize, tiles[i].w * kPictureSize, tiles[i].h * kPictureSize);
            ...
            fShaders[i] = pictureRef->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest, &localMatrix, tilePtr);
        }
    }
