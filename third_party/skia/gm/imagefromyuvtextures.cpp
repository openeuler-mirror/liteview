...
protected:
    ...
    static std::unique_ptr<sk_gpu_test::LazyYUVImage> CreatePlanes(const char* name)
    {
        ...
        SkYUVAPixmapInfo pixmapInfo({ bmp.dimensions(), SkYUVAInfo::PlaneConfig::kY_U_V_A, SkYUVAInfo::Subsampling::k420, kJPEG_Full_SkYUVColorSpace },
        ...
        for (int j = 0; j < pixmaps.planes()[0].height(); ++j) {
            for (int i = 0; i < pixmaps.planes()[0].width(); ++i) {
                ...
                yuvPixels[0][j * pixmaps.planes()[0].width() + i] = SkToU8(sk_float_round2int(m[0] * r + m[1] * g + m[2] * b + m[3] * a + 255 * m[4]));
                yuvPixels[3][j * pixmaps.planes()[0].width() + i] = SkToU8(sk_float_round2int(m[15] * r + m[16] * g + m[17] * b + m[18] * a + 255 * m[19]));
            }
        }
        for (int j = 0; j < pixmaps.planes()[1].height(); ++j) {
            for (int i = 0; i < pixmaps.planes()[1].width(); ++i) {
                ...
                yuvPixels[1][uvIndex] = SkToU8(sk_float_round2int(m[5] * rgba[0] + m[6] * rgba[1] + m[7] * rgba[2] + m[8] * rgba[3] + 255 * m[9]));
                yuvPixels[2][uvIndex] = SkToU8(sk_float_round2int(m[10] * rgba[0] + m[11] * rgba[1] + m[12] * rgba[2] + m[13] * rgba[3] + 255 * m[14]));
            }
        }
    ...
    sk_sp<SkImage> createReferenceImage(GrDirectContext* dContext)
    {
        ...
        auto resultInfo = SkImageInfo::Make(fLazyYUVImage->dimensions(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
        auto resultSurface = SkSurface::MakeRenderTarget(dContext, SkBudgeted::kYes, resultInfo, 1, kTopLeft_GrSurfaceOrigin, nullptr);
        if (!resultSurface) {
            return nullptr;
        }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        auto draw_image_rect = [canvas](SkImage* image, const SkSamplingOptions& sampling) -> SkSize {
            ...
            canvas->drawImageRect(image, subset, dst, sampling, nullptr, SkCanvas::kStrict_SrcRectConstraint);
        ...
        for (const auto& draw : { DF(draw_image), DF(draw_image_rect), DF(draw_image_shader) }) {
            for (auto scale : { 1.f, 4.f, 0.75f }) {
                SkScalar h = 0;
                canvas->save();
                for (const auto& sampling : { SkSamplingOptions(SkFilterMode::kNearest), SkSamplingOptions(SkFilterMode::kLinear),
                         SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest), SkSamplingOptions(SkCubicResampler::Mitchell()) }) {
                    canvas->save();
