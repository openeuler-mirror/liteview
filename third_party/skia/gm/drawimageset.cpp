...
static void make_image_tiles(int tileW, int tileH, int m, int n, const SkColor colors[4], SkCanvas::ImageSetEntry set[], const SkColor bgColor = SK_ColorLTGRAY)
{
    const int w = tileW * m;
    const int h = tileH * n;
    auto surf = SkSurface::MakeRaster(SkImageInfo::Make(w, h, kRGBA_8888_SkColorType, kPremul_SkAlphaType));
...
private:
    ...
    {
        return SkString("draw_image_set");
    }
    ...
    {
        static constexpr SkColor kColors[] = { SK_ColorCYAN, SK_ColorBLACK, SK_ColorMAGENTA, SK_ColorBLACK };
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        SkPoint dst[4] = { { 0, 0 }, { kM * kTileW + 10.f, -5.f }, { kM * kTileW - 28.f, kN * kTileH + 40.f }, { 45.f, kN * kTileH - 25.f } };
        ...
        for (auto fm : { SkFilterMode::kNearest, SkFilterMode::kLinear }) {
            ...
            for (size_t m = 0; m < std::size(matrices); ++m) {
                ...
                canvas->experimental_DrawEdgeAAImageSet(fSet, kM * kN, nullptr, nullptr, sampling, &setPaint, SkCanvas::kFast_SrcRectConstraint);
                canvas->restore();
            }
            ...
            canvas->experimental_DrawEdgeAAImageSet(&entry, 1, nullptr, nullptr, sampling, &setPaint, SkCanvas::kFast_SrcRectConstraint);
            ...
            canvas->experimental_DrawEdgeAAImageSet(&entry, 1, nullptr, nullptr, sampling, &cfPaint, SkCanvas::kFast_SrcRectConstraint);
            canvas->restore();
...
private:
    SkString onShortName() override
    {
        return SkString("draw_image_set_rect_to_rect");
    }
    SkISize onISize() override
    {
        return { 1250, 850 };
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (SkScalar frac : { 0.f, 0.5f }) {
            ...
            for (size_t m = 0; m < std::size(matrices); ++m) {
                ...
                canvas->experimental_DrawEdgeAAImageSet(
                    fSet, kM * kN, nullptr, nullptr, SkSamplingOptions(SkFilterMode::kLinear), &paint, SkCanvas::kFast_SrcRectConstraint);
                canvas->restore();
        ...
        for (SkVector scale : { SkVector { 2.f, 0.5f }, SkVector { 0.5, 2.f } }) {
            ...
            for (size_t m = 0; m < std::size(matrices); ++m) {
                ...
                canvas->concat(matrices[m]);
                canvas->experimental_DrawEdgeAAImageSet(
                    scaledSet, kM * kN, nullptr, nullptr, SkSamplingOptions(SkFilterMode::kLinear), &paint, SkCanvas::kFast_SrcRectConstraint);
                canvas->restore();
                canvas->translate(kTranslate, 0);
            }
            canvas->restore();
...
private:
    SkString onShortName() override
    {
        return SkString("draw_image_set_alpha_only");
    }
    SkISize onISize() override
    {
    ...
    {
        static constexpr SkColor kColors[] = { SK_ColorBLUE, SK_ColorTRANSPARENT, SK_ColorRED, SK_ColorTRANSPARENT };
        ...
        for (int y = 0; y < kN; ++y) {
            for (int x = 0; x < kM; ++x) {
                ...
                if (y % 2 == 0) {
                    fSet[i].fImage = fSet[i].fImage->makeColorTypeAndColorSpace(kAlpha_8_SkColorType, alphaSpace, direct);
                }
            }
        }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int y = 0; y < kN; ++y) {
            for (int x = 0; x < kM; ++x) {
                ...
                canvas->drawImageRect(ToolUtils::MakeTextureImage(canvas, std::move(orig)), fSet[i].fSrcRect, fSet[i].fDstRect, SkSamplingOptions(),
