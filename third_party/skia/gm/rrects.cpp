...
#ifdef SK_DEBUG
        const SkRect kMaxImageBound = SkRect::MakeWH(SkIntToScalar(kImageWidth), SkIntToScalar(kImageHeight));
#endif
            ...
            for (int curRRect = 0; curRRect < kNumRRects; ++curRRect) {
                ...
                if (kEffect_Type == fType) {
                    ...
                    if (success) {
                        ...
                        sdc->addDrawOp(skgpu::v1::FillRectOp::MakeNonAARect(rContext, std::move(grPaint), SkMatrix::I(), bounds));
                    } else {
                        drew = false;
                    }
