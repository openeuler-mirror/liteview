...
static sk_sp<SkSurface> make_surface(GrRecordingContext* ctx, const SkImageInfo& info, SkPixelGeometry geo)
{
...
#define DEF_BASIC_SURFACE_TEST(name, canvas, main, W, H)                                                                                                       \
    ...
    {                                                                                                                                                          \
        auto make = [canvas](const SkImageInfo& ii) { return make_surface(ii, canvas, SurfaceType::kManaged); };                                               \
...
#define DEF_BET_SURFACE_TEST(name, canvas, main, W, H) DEF_BACKEND_SURFACE_TEST(SK_MACRO_CONCAT(name, _bet), canvas, main, SurfaceType::kBackendTexture, W, H)

#define DEF_BERT_SURFACE_TEST(name, canvas, main, W, H)                                                                                                        \
    DEF_BACKEND_SURFACE_TEST(SK_MACRO_CONCAT(name, _bert), canvas, main, SurfaceType::kBackendRenderTarget, W, H)
...
{
    auto ct = canvas->imageInfo().colorType() == kUnknown_SkColorType ? kRGBA_8888_SkColorType : canvas->imageInfo().colorType();
    auto ii = SkImageInfo::Make({ 32, 32 }, ct, kPremul_SkAlphaType, canvas->imageInfo().refColorSpace());
    ...
    auto nextImage = [&](SkColor color) {
        ...
        surface->getCanvas()->drawRect(
            SkRect::MakeLTRB(surface->width() * 2 / 5.f, surface->height() * 2 / 5.f, surface->width() * 3 / 5.f, surface->height() * 3 / 5.f), paint);
...
DEF_SURFACE_TESTS(surface_underdraw, canvas, 256, 256)
{
    ...
    // restore the original strip, drawing it "under" the current foreground
    {
        ...
        surf->getCanvas()->drawImage(saveImg, SkIntToScalar(subset.left()), SkIntToScalar(subset.top()), SkSamplingOptions(), &paint);
    }
