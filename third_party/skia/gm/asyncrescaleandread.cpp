...
static sk_sp<SkImage> do_read_and_scale(
    Src* src, GrDirectContext* direct, const SkIRect& srcRect, const SkImageInfo& ii, SkImage::RescaleGamma rescaleGamma, SkImage::RescaleMode rescaleMode)
{
    auto* asyncContext = new AsyncContext();
    src->asyncRescaleAndReadPixels(ii, srcRect, rescaleGamma, rescaleMode, async_callback, asyncContext);
    if (direct) {
        direct->submit();
    }
...
template <typename Src>
static sk_sp<SkImage> do_read_and_scale_yuv(Src* src, GrDirectContext* direct, SkYUVColorSpace yuvCS, const SkIRect& srcRect, SkISize size,
    SkImage::RescaleGamma rescaleGamma, SkImage::RescaleMode rescaleMode, SkScopeExit* cleanup)
{
    ...
    src->asyncRescaleAndReadPixelsYUV420(yuvCS, SkColorSpace::MakeSRGB(), srcRect, size, rescaleGamma, rescaleMode, async_callback, &asyncContext);
    if (direct) {
        direct->submit();
    }
    while (!asyncContext.fCalled) {
    ...
    SkPixmap yuvPMs[] = { { yII, asyncContext.fResult->data(0), asyncContext.fResult->rowBytes(0) },
...
static skiagm::DrawResult do_rescale_grid(
    SkCanvas* canvas, Src* src, GrDirectContext* direct, const SkIRect& srcRect, SkISize newSize, bool doYUV420, SkString* errorMsg, int pad = 0)
{
    if (doYUV420 && !direct) {
    ...
    for (auto gamma : { SkImage::RescaleGamma::kSrc, SkImage::RescaleGamma::kLinear }) {
        canvas->save();
        for (auto mode : { SkImage::RescaleMode::kNearest, SkImage::RescaleMode::kRepeatedLinear, SkImage::RescaleMode::kRepeatedCubic }) {
            SkScopeExit cleanup;
            sk_sp<SkImage> result;
            if (doYUV420) {
                result = do_read_and_scale_yuv(src, direct, yuvColorSpace, srcRect, newSize, gamma, mode, &cleanup);
                if (!result) {
...
static skiagm::DrawResult do_rescale_image_grid(
    SkCanvas* canvas, const char* imageFile, const SkIRect& srcRect, SkISize newSize, bool doSurface, bool doYUV420, SkString* errorMsg)
{
...
DEF_RESCALE_AND_READ_YUV_SURF_GM(images / yellow_rose.webp, rose, SkIRect::MakeXYWH(50, 5, 200, 150), 410, 376)

DEF_RESCALE_AND_READ_YUV_IMG_GM(images / yellow_rose.webp, rose_down, SkIRect::MakeXYWH(50, 5, 200, 150), 106, 60)

DEF_RESCALE_AND_READ_SURF_GM(images / yellow_rose.webp, rose, SkIRect::MakeXYWH(100, 20, 100, 100), 410, 410)
...
DEF_RESCALE_AND_READ_IMG_GM(images / text.png, text_down, SkIRect::MakeWH(637, 105), (int)(0.7 * 637), (int)(0.7 * 105))
DEF_RESCALE_AND_READ_SURF_GM(images / text.png, text_up, SkIRect::MakeWH(637, 105), (int)(1.2 * 637), (int)(1.2 * 105))
DEF_RESCALE_AND_READ_IMG_GM(images / text.png, text_up_large, SkIRect::MakeXYWH(300, 0, 300, 105), (int)(2.4 * 300), (int)(2.4 * 105))
...
DEF_SIMPLE_GM_CAN_FAIL(async_yuv_no_scale, canvas, errorMsg, 400, 300)
{
    ...
    auto yuvImage = do_read_and_scale_yuv(surface, dContext, kRec601_SkYUVColorSpace, SkIRect::MakeWH(400, 300), { 400, 300 }, SkImage::RescaleGamma::kSrc,
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_no_bleed, canvas, errorMsg, 60, 60)
{
    ...
    auto surfaceII = SkImageInfo::Make(kInner + 2 * kBorder, kInner + 2 * kBorder, kRGBA_8888_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB());
    ...
    result = do_rescale_grid(canvas, surface.get(), dContext, srcRect, downSize, false, errorMsg, kPad);
...
DEF_SIMPLE_GM_CAN_FAIL(async_rescale_and_read_alpha_type, canvas, errorMsg, 512, 512)
{
    ...
    auto shader = SkGradientShader::MakeRadial({ 100, 100 }, 230, colors, nullptr, nullptr, std::size(colors), SkTileMode::kRepeat);
    SkPaint paint;
    ...
    for (const auto& img : { pmImg, upmImg }) {
        ...
        for (auto readAT : { kPremul_SkAlphaType, kUnpremul_SkAlphaType }) {
            ...
            auto* asyncContext = new AsyncContext();
            img->asyncRescaleAndReadPixels(readInfo, SkIRect::MakeSize(img->dimensions()), SkImage::RescaleGamma::kSrc, SkImage::RescaleMode::kRepeatedCubic,
            ...
            if (asyncContext->fResult) {
                SkPixmap pixmap(readInfo, asyncContext->fResult->data(0), asyncContext->fResult->rowBytes(0));
                auto releasePixels = [](const void*, void* c) { delete static_cast<AsyncContext*>(c); };
