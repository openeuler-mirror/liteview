...
static sk_sp<SkShader> make_bg_shader(int checkSize)
{
    ...
    SkRect rect1 = SkRect::MakeXYWH(SkIntToScalar(checkSize), SkIntToScalar(checkSize), SkIntToScalar(checkSize), SkIntToScalar(checkSize));
            ...
            for (size_t cfc = 0; cfc < std::size(colors); ++cfc) {
                ...
                for (size_t s = 0; s < std::size(shaders); ++s) {
                    ...
                    for (int pc = 0; pc < paintColorCnt; ++pc) {
                        ...
                        SkRect rect = SkRect::MakeXYWH(x * kRectWidth, y * kRectHeight, SkIntToScalar(kRectWidth), SkIntToScalar(kRectHeight));
