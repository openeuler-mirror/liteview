...
public:
    ParticlesGM(const char* name, double startTime, SkISize size, SkPoint origin, std::vector<UniformValue> uniforms = {})
        : GM(SK_ColorBLACK)
        , fName(name)
    ...
    {
        return SkStringPrintf("particles_%s", fName);
    }
...
DEF_GM(
    return new ParticlesGM("uniforms", 2.0, { 250, 250 }, { 125, 125 }, { { "rate", { 2.0f } }, { "spin", { 4.0f } }, { "color", { 0.25f, 0.75f, 0.75f } } });)
