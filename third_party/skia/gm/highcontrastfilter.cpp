...
static void draw_label(SkCanvas* canvas, const SkHighContrastConfig& config)
{
    ...
    const char* invertStr
        = (config.fInvertStyle == InvertStyle::kInvertBrightness ? "InvBright"
                                                                 : (config.fInvertStyle == InvertStyle::kInvertLightness ? "InvLight" : "NoInvert"));

    snprintf(labelBuffer, sizeof(labelBuffer), "%s%s contrast=%.1f", config.fGrayscale ? "Gray " : "", invertStr, config.fContrast);
...
static void draw_scene(SkCanvas* canvas, const SkHighContrastConfig& config)
{
    ...
    paint.setShader(SkGradientShader::MakeLinear(pts, colors2, pos, std::size(colors2), SkTileMode::kClamp));
    canvas->drawRect(bounds, paint);
...
protected:
    void onOnceBeforeDraw() override
    {
        ...
        fGr1 = SkGradientShader::MakeLinear(g1Points, g1Colors, pos, std::size(g1Colors), SkTileMode::kClamp);
        fGr2 = SkGradientShader::MakeLinear(g2Points, g2Colors, pos, std::size(g2Colors), SkTileMode::kClamp);
    }
