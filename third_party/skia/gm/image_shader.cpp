...
{
    return SkImage::MakeFromPicture(sk_ref_sp(pic), info.dimensions(), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}
...
DEF_SIMPLE_GM(drawimage_sampling, canvas, 500, 500)
{
    ...
    for (auto mm : { SkMipmapMode::kNone, SkMipmapMode::kNearest, SkMipmapMode::kLinear }) {
        for (auto fm : { SkFilterMode::kNearest, SkFilterMode::kLinear }) {
            ...
            canvas->drawImageRect(img.get(), src, dst, sampling, nullptr, SkCanvas::kFast_SrcRectConstraint);
            canvas->restore();
