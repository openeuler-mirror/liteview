...
protected:
    ...
    {
        return SkStringPrintf("pictureshader%s%s", fUseLocalMatrixWrapper ? "_localwrapper" : "", fAlpha < 1 ? "_alpha" : "");
    }
...
private:
    void drawSceneColumn(SkCanvas* canvas, const SkPoint& pos, SkScalar scale, SkScalar localScale, unsigned tileMode)
    {
        SkMatrix ctm, localMatrix;
    ...
    void drawTile(SkCanvas* canvas)
    {
        ...
        canvas->drawRect(SkRect::MakeXYWH(fTileSize / 2, fTileSize / 2, fTileSize / 2, fTileSize / 2), paint);
    ...
    void drawScene(SkCanvas* canvas, const SkMatrix& matrix, const SkMatrix& localMatrix, unsigned tileMode)
    {
        ...
        auto pictureShader = fPicture->makeShader(
            kTileConfigs[tileMode].tmx, kTileConfigs[tileMode].tmy, SkFilterMode::kNearest, fUseLocalMatrixWrapper ? nullptr : &localMatrix, nullptr);
        paint.setShader(fUseLocalMatrixWrapper ? pictureShader->makeWithLocalMatrix(localMatrix) : pictureShader);
        canvas->drawRect(SkRect::MakeWH(fSceneSize, fSceneSize), paint);

        canvas->translate(fSceneSize * 1.1f, 0);

        auto bitmapShader
            = fBitmap.makeShader(kTileConfigs[tileMode].tmx, kTileConfigs[tileMode].tmy, SkSamplingOptions(), fUseLocalMatrixWrapper ? nullptr : &localMatrix);
        paint.setShader(fUseLocalMatrixWrapper ? bitmapShader->makeWithLocalMatrix(localMatrix) : bitmapShader);
        canvas->drawRect(SkRect::MakeWH(fSceneSize, fSceneSize), paint);
...
DEF_SIMPLE_GM(tiled_picture_shader, canvas, 400, 400)
{
    ...
    p.setShader(picture->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest));
    canvas->drawPaint(p);
}
...
DEF_SIMPLE_GM(pictureshader_persp, canvas, 215, 110)
{
    ...
    auto drawPicture = [](SkCanvas* canvas, sk_sp<SkPicture> picture, DrawStrategy strategy) {
        ...
        case DrawStrategy::kPictureShader: {
            SkPaint paint;
            paint.setShader(picture->makeShader(SkTileMode::kDecal, SkTileMode::kDecal, SkFilterMode::kLinear, nullptr, &bounds));
