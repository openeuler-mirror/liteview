...
private:
    SkString onShortName() final
    {
        return SkString("croppedrects");
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
...

        {
            ...
            canvas->drawImageRect(fSrcImage.get(), kSrcImageClip.makeOutset(0.5f * kSrcImageClip.width(), kSrcImageClip.height()),
                drawRect.makeOutset(0.5f * drawRect.width(), drawRect.height()), SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
        }

        {
            // skgpu::v1::SurfaceDrawContext::fillRectWithLocalMatrix.
            SkAutoCanvasRestore acr(canvas, true);
            SkPath path = SkPath::Line({ kSrcImageClip.fLeft - kSrcImageClip.width(), kSrcImageClip.centerY() },
