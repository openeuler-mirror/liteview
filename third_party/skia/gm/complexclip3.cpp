            ...
            for (int aaBits = 0; aaBits < 4; ++aaBits) {
                ...
                for (size_t op = 0; op < std::size(gOps); ++op) {
                    for (int invB = 0; invB < 2; ++invB) {
                        ...
                        canvas->save();
                        // set clip
                        firstClip->setFillType(doInvA ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
                        secondClip->setFillType(doInvB ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
                        ...
                        SkString str;
                        str.printf("%s%s %s %s%s", doAAA ? "A" : "B", doInvA ? "I" : "N", gOps[op].fName, doAAB ? "A" : "B", doInvB ? "I" : "N");
