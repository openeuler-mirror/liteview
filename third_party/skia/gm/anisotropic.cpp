...
protected:
    ...
    {
        return SkISize::Make(2 * kImageSize + 3 * kSpacer, kNumVertImages * kImageSize + (kNumVertImages + 1) * kSpacer);
    }
    ...
    void onOnceBeforeDraw() override
    {
        ...
        for (int i = 0; i < kNumLines; ++i, angle += kAngleStep) {
            ...
            canvas->drawLine(cos * kInnerOffset, sin * kInnerOffset, cos * kImageSize / 2, sin * kImageSize / 2, p);
        }
    ...
    {
        SkRect r = SkRect::MakeXYWH(SkIntToScalar(x), SkIntToScalar(y), SkIntToScalar(xSize), SkIntToScalar(ySize));
...
protected:
    SkString onShortName() override
    {
        return SkString("anisomips");
    }

    SkISize onISize() override
    {
        return SkISize::Make(520, 260);
    }
    ...
    sk_sp<SkImage> updateImage(SkSurface* surf, SkColor color)
    {
        ...
        surf->getCanvas()->drawRect(
            SkRect::MakeLTRB(surf->width() * 2 / 5.f, surf->height() * 2 / 5.f, surf->width() * 3 / 5.f, surf->height() * 3 / 5.f), paint);
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        auto ii = SkImageInfo::Make(kImageSize, kImageSize, ct, kPremul_SkAlphaType, canvas->imageInfo().refColorSpace());
        ...
        if (auto rc = canvas->recordingContext()) {
            surface = SkSurface::MakeRenderTarget(rc, SkBudgeted::kYes, ii, 1, kTopLeft_GrSurfaceOrigin,
