...
static void draw_color_bm(SkBitmap* bm, int length)
{
    ...
    SkCanvas canvas(*bm);
    canvas.drawCircle(SkIntToScalar(length / 2), SkIntToScalar(length / 2), SkIntToScalar(length / 2), paint);
}
...
static void draw_alpha8_bm(SkBitmap* bm, int length)
{
    ...
    SkCanvas canvas(*bm);
    canvas.drawCircle(SkIntToScalar(length / 2), SkIntToScalar(length / 2), SkIntToScalar(length / 4), circlePaint);
}
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        if (!fInitialized) {
            ...
            fColorBitmapShader = img->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), SkMatrix::I());

            draw_alpha8_bm(&fAlpha8Bitmap, squareLength);
            img = SkImage::MakeFromBitmap(fAlpha8Bitmap);
            img = ToolUtils::MakeTextureImage(canvas, std::move(img));
            fAlpha8BitmapShader = fAlpha8Bitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), SkMatrix::I());
...
DEF_SIMPLE_GM(composeshader_bitmap2, canvas, 200, 200)
{
    ...
    SkImageInfo imageInfo = SkImageInfo::Make(width, height, SkColorType::kN32_SkColorType, kPremul_SkAlphaType);
    skBitmap.installPixels(imageInfo, dst32Storage.begin(), width * sizeof(int32_t), nullptr, nullptr);
    imageInfo = SkImageInfo::Make(width, height, SkColorType::kAlpha_8_SkColorType, kPremul_SkAlphaType);
    ...
    paint.setShader(SkShaders::Blend(SkBlendMode::kSrcIn, skMaskImage->makeShader(SkSamplingOptions()), skSrc->makeShader(SkSamplingOptions())));
    canvas->drawRect(r, paint);
}
