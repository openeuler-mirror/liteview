...
class ClippedCubic2GM : public skiagm::GM {
    ...
    void onOnceBeforeDraw() override
    {
        ...
        fPath.cubicTo(69.7030518991886f, 21.831149999999997f, 58.08369508178456f, 43.66448333333333f, 34.8449814469765f, 65.5f);
        fPath.cubicTo(11.608591683531916f, 87.33115f, -0.010765133872116195f, 109.16448333333332f, -0.013089005235602302f, 131);
        fPath.close();
        fFlipped = fPath;
        SkMatrix matrix;
        matrix.reset();
...
class CubicPathGM : public skiagm::GM {
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        path.fPath.cubicTo(40 * SK_Scalar1, 20 * SK_Scalar1, 60 * SK_Scalar1, 20 * SK_Scalar1, 75 * SK_Scalar1, 10 * SK_Scalar1);
        ...
        for (size_t cap = 0; cap < std::size(gCaps); ++cap) {
            ...
            for (size_t fill = 0; fill < std::size(gFills); ++fill) {
                ...
                for (size_t style = 0; style < std::size(gStyles); ++style) {
                    ...
                    this->drawPath(
                        path.fPath, canvas, color, rect, gCaps[cap].fCap, gCaps[cap].fJoin, gStyles[style].fStyle, gFills[fill].fFill, SK_Scalar1 * 10);
