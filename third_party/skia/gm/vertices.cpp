...
static sk_sp<SkShader> make_shader1(SkScalar shaderScale)
{
    ...
    sk_sp<SkShader> grad = SkGradientShader::MakeLinear(pts, colors, nullptr, std::size(colors), SkTileMode::kMirror, 0, &localMatrix);
    // Throw in a couple of local matrix wrappers for good measure.
    return shaderScale == 1
        ? grad
        : sk_make_sp<SkLocalMatrixShader>(sk_make_sp<SkLocalMatrixShader>(std::move(grad), SkMatrix::Translate(-10, 0)), SkMatrix::Translate(10, 0));
}
                ...
                for (const auto& cf : { sk_sp<SkColorFilter>(nullptr), fColorFilter }) {
                    for (const auto& shader : { fShader1, fShader2 }) {
                        ...
                        for (auto attrs : kAttrs) {
                            ...
                            auto v = SkVertices::MakeCopy(SkVertices::kTriangleFan_VertexMode, kMeshVertexCnt, fPts, texs, colors, kMeshIndexCnt, kMeshFan);
...
static void draw_batching(SkCanvas* canvas)
{
    ...
    SkVertices::Builder builder(
        SkVertices::kTriangles_VertexMode, kMeshVertexCnt, 3 * kNumTris, SkVertices::kHasColors_BuilderFlag | SkVertices::kHasTexCoords_BuilderFlag);
    ...
    matrices.append()->setRotate(45, kMeshSize / 2, kMeshSize / 2).postScale(1.2f, .8f, kMeshSize / 2, kMeshSize / 2).postTranslate(0, 80);
    ...
    for (bool useShader : { false, true }) {
        for (bool useTex : { false, true }) {
            for (const auto& m : matrices) {
                ...
                auto v = SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, kMeshVertexCnt, pts, t, colors, kNumTris * 3, indices);
...
DEF_SIMPLE_GM(skbug_13047, canvas, 200, 200)
{
    ...
    auto v = SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, 4, verts, texs, nullptr, 6, indices);
