...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        const SkIRect src[] = { { 0, 0, 32, 32 }, { 0, 0, 80, 80 }, { 32, 32, 96, 96 },
            {
                -32,
                -32,
                32,
        ...
        for (size_t i = 0; i < std::size(src); i++) {
            ...
            if (!fUseIRect) {
                canvas->drawImageRect(image.get(), srcR, dstR, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
            } else {
                canvas->drawImageRect(image.get(), SkRect::Make(src[i]), dstR, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
            }
...
static void make_3x3_bitmap(SkBitmap* bitmap)
{
    ...
    const SkColor textureData[xSize][ySize]
        = { { SK_ColorRED, SK_ColorWHITE, SK_ColorBLUE }, { SK_ColorGREEN, SK_ColorBLACK, SK_ColorCYAN }, { SK_ColorYELLOW, SK_ColorGRAY, SK_ColorMAGENTA } };
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        canvas->drawImageRect(
            ToolUtils::MakeTextureImage(canvas, bitmap.asImage()), srcR, dstR, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
    }
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        if (!fUseIRect) {
            canvas->drawImageRect(fBigImage, srcR1, dstR1, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
            canvas->drawImageRect(fBigImage, srcR2, dstR2, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
        } else {
            canvas->drawImageRect(fBigImage, SkRect::Make(srcR1.roundOut()), dstR1, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
            canvas->drawImageRect(fBigImage, SkRect::Make(srcR2.roundOut()), dstR2, sampling, &paint, SkCanvas::kStrict_SrcRectConstraint);
        }
    }
