...
static void draw_1_bitmap(SkCanvas* canvas, const SkBitmap& bm, bool doClip, int dx, int dy, sk_sp<SkImageFilter> filter)
{
    ...
    SkRect clipR = SkRect::MakeXYWH(SkIntToScalar(dx), SkIntToScalar(dy), SkIntToScalar(bm.width()), SkIntToScalar(bm.height()));
    ...
    canvas->drawImage(bm.asImage(), SkIntToScalar(dx), SkIntToScalar(dy), SkSamplingOptions(), &paint);
    if (doClip) {
        canvas->restore();
    }
}
