...
static void shade_rect(SkCanvas* canvas, sk_sp<SkShader> shader, int cellRow, int cellCol)
{
    ...
    SkRect rect = SkRect::MakeXYWH(SkIntToScalar(cellCol * CELL_WIDTH + PAD_WIDTH), SkIntToScalar(cellRow * CELL_HEIGHT + PAD_HEIGHT),
        SkIntToScalar(RECT_WIDTH), SkIntToScalar(RECT_HEIGHT));
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int cellRow = 0; cellRow < NUM_ROWS; cellRow++) {
            for (int cellCol = 0; cellCol < NUM_COLS; cellCol++) {
                ...
                auto shader = SkGradientShader::MakeLinear(points, colors, positions[cellRow], numGradientColors[cellRow], tilemodes[cellCol], 0, nullptr);
