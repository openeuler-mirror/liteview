...
DEF_SIMPLE_GM_BG(skbug_12212, canvas, 400, 400, SK_ColorCYAN)
{
    ...
    auto imageInfo = SkImageInfo::Make(/*width=*/400, /*height=*/400, kAlpha_8_SkColorType, kPremul_SkAlphaType);
    SkSurfaceProps props(/*flags=*/0, kRGB_H_SkPixelGeometry);
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(canvas->recordingContext(), SkBudgeted::kNo, imageInfo, /*sampleCount=*/0, &props);
    if (!surface) {
