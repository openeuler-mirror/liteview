...
{
    const float K = 0.01f;
    return fabsf(x.fR - y.fR) < K && fabsf(x.fG - y.fG) < K && fabsf(x.fB - y.fB) < K && fabsf(x.fA - y.fA) < K;
}
...
DEF_SIMPLE_GM(p3, canvas, 450, 1300)
{
    ...
    // Draw a P3 red bitmap, using SkPixmap::erase().
    {
        ...
        canvas->drawImage(bm.asImage(), 10, 10);
        compare_pixel("drawBitmap P3 red, from SkPixmap::erase", canvas, 10, 10, { 1, 0, 0, 1 }, p3.get());
    }
    ...
    // Draw a P3 red bitmap wrapped in a shader, using SkPixmap::erase().
    {
        ...
        canvas->drawRect({ 10, 10, 70, 70 }, paint);
        compare_pixel("drawBitmapAsShader P3 red, from SkPixmap::erase", canvas, 10, 10, { 1, 0, 0, 1 }, p3.get());
    }
    ...
    // Draw a gradient from P3 red to P3 green interpolating in unpremul P3, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(points, colors, p3, nullptr, std::size(colors), SkTileMode::kClamp));
        canvas->drawRect({ 10, 10, 70, 70 }, paint);
        canvas->save();
    ...
    // Draw a gradient from P3 red to P3 green interpolating in premul P3, checking the corners.
    {
        ...
        paint.setShader(SkGradientShader::MakeLinear(
            points, colors, p3, nullptr, std::size(colors), SkTileMode::kClamp, SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr /*local matrix*/));
        canvas->drawRect({ 10, 10, 70, 70 }, paint);
        canvas->save();
    ...
    // Draw a gradient from P3 red to P3 green interpolating in unpremul sRGB, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(points, colors, srgb, nullptr, std::size(colors), SkTileMode::kClamp));
        canvas->drawRect({ 10, 10, 70, 70 }, paint);
        canvas->save();
    ...
    // Draw a gradient from P3 red to P3 green interpolating in premul sRGB, checking the corners.
    {
        ...
        SkPaint paint;
        paint.setShader(SkGradientShader::MakeLinear(
            points, colors, srgb, nullptr, std::size(colors), SkTileMode::kClamp, SkGradientShader::kInterpolateColorsInPremul_Flag, nullptr /*local matrix*/));
        canvas->drawRect({ 10, 10, 70, 70 }, paint);
        canvas->save();
