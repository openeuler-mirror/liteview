...
static SkColor blend(SkColor dst, SkColor src, void (*mode)(float, float, float, float*, float*, float*))
{
    ...
    SkASSERT(SkColorGetA(dst) == 0xff && SkColorGetA(src) == 0xff); // Not fundamental, just simplifying for this GM.
...
DEF_SIMPLE_GM(HSL_duck, canvas, 1110, 620)
{
    ...
    for (SkScalar src_a : { 1.0f, 0.5f }) {
        ...
        canvas->drawSimpleText(str.c_str(), str.size(), SkTextEncoding::kUTF8, 10, r.height() / 2, font, SkPaint());
