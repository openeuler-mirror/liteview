...
{
    const auto r_lo = SkColorGetR(lo), g_lo = SkColorGetG(lo), b_lo = SkColorGetB(lo), a_lo = SkColorGetA(lo), r_hi = SkColorGetR(hi), g_hi = SkColorGetG(hi),
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        paint.setShader(SkGradientShader::MakeSweep(fTileSize.width() / 2, fTileSize.height() / 2, gradient_colors, nullptr, std::size(gradient_colors)));
...
private:
    ...
    void mixRow(SkCanvas* canvas, SkPaint& paint, sk_sp<SkColorFilter> cf0, sk_sp<SkColorFilter> cf1)
    {
