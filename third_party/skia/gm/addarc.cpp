...
protected:
    ...
    {
        return SkString("addarc");
    }

    SkISize onISize() override
    {
        return SkISize::Make(1040, 1040);
    }
...
protected:
    SkString onShortName() override
    {
        return SkString("strokecircle");
    }

    SkISize onISize() override
    {
        return SkISize::Make(520, 520);
    }
...
DEF_SIMPLE_GM(manyarcs, canvas, 620, 330)
{
    ...
    SkScalar sweepAngles[]
        = { -123.7f, -2.3f, -2, -1, -0.3f, -0.000001f, 0, 0.000001f, 0.3f, 0.7f, 1, 1.3f, 1.5f, 1.7f, 1.99999f, 2, 2.00001f, 2.3f, 4.3f, 3934723942837.3f };
    ...
    for (size_t i = 0; i < std::size(startAngles) * 2; ++i) {
        ...
        for (size_t j = 0; j < std::size(sweepAngles); ++j) {
            SkPathBuilder path;
            path.moveTo(0, 2);
            html_canvas_arc(&path, 18, 15, 10, startAngle, startAngle + (sweepAngles[j] * sign), anticlockwise, true);
            path.lineTo(0, 28);
            canvas->drawPath(path.detach().setIsVolatile(true), paint);
...
DEF_SIMPLE_GM(tinyanglearcs, canvas, 620, 330)
{
    ...
    for (size_t i = 0; i < std::size(startAngles); ++i) {
        ...
        path.moveTo(centerX + innerRadius * sk_float_cos(startAngles[i]), centerY + innerRadius * sk_float_sin(startAngles[i]));
        path.lineTo(centerX + outerRadius * sk_float_cos(startAngles[i]), centerY + outerRadius * sk_float_sin(startAngles[i]));
        // A combination of tiny sweepAngle + large radius, we should draw a line.
        html_canvas_arc(&path, centerX, outerRadius, outerRadius, startAngles[i] * 180 / SK_ScalarPI, endAngle * 180 / SK_ScalarPI, true, true);
        path.lineTo(centerX + innerRadius * sk_float_cos(endAngle), centerY + innerRadius * sk_float_sin(endAngle));
        html_canvas_arc(&path, centerX, outerRadius, innerRadius, endAngle * 180 / SK_ScalarPI, startAngles[i] * 180 / SK_ScalarPI, true, false);
