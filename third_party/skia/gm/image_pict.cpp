...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        fImage0 = SkImage::MakeFromPicture(fPicture, size, &matrix, nullptr, SkImage::BitDepth::kU8, srgbColorSpace);
        matrix.postTranslate(-50, -50);
        matrix.postRotate(45);
        matrix.postTranslate(50, 50);
        fImage1 = SkImage::MakeFromPicture(fPicture, size, &matrix, nullptr, SkImage::BitDepth::kU8, srgbColorSpace);
    }
...
static std::unique_ptr<SkImageGenerator> make_pic_generator(GrDirectContext*, sk_sp<SkPicture> pic)
{
    SkMatrix matrix;
    matrix.setTranslate(-100, -100);
    return SkImageGenerator::MakeFromPicture({ 100, 100 }, std::move(pic), &matrix, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}
...
static std::unique_ptr<SkImageGenerator> make_ras_generator(GrDirectContext*, sk_sp<SkPicture> pic)
{
    SkBitmap bm;
...
public:
        ...
        , fRContext(SkRef(rContext))
    {
        ...
        sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(rContext, SkBudgeted::kYes, info, 0, kTopLeft_GrSurfaceOrigin, nullptr));
        if (surface) {
...
protected:
    GrSurfaceProxyView onGenerateTexture(GrRecordingContext* rContext, const SkImageInfo& info, GrMipmapped mipmapped, GrImageTexGenPolicy policy) override
    {
        SkASSERT(rContext);
        ...
        return GrSurfaceProxyView::Copy(fRContext.get(), fView, mipmapped, SkIRect::MakeWH(info.width(), info.height()), SkBackingFit::kExact, budgeted,
...
static std::unique_ptr<SkImageGenerator> make_tex_generator(GrDirectContext* dContext, sk_sp<SkPicture> pic)
{
    if (!dContext) {
        return nullptr;
    }
...
public:
    ImageCacheratorGM(const char suffix[], FactoryFunc factory)
        : fFactory(factory)
    {
...
protected:
    ...
    static void draw_as_tex(SkCanvas* canvas, SkImage* image, SkScalar x, SkScalar y)
    {
        ...
        sk_sp<SkImage> texImage(new SkImage_Gpu(sk_ref_sp(canvas->recordingContext()), image->uniqueID(), std::move(view), std::move(colorInfo)));
