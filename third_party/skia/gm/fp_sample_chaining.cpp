...
public:
    ...
    {
        this->registerChild(std::move(child), SkSL::SampleUsage::UniformMatrix(/*hasPerspective=*/false));
    }
...
static std::unique_ptr<GrFragmentProcessor> wrap(std::unique_ptr<GrFragmentProcessor> fp, EffectType effectType, int drawX, int drawY)
{
    switch (effectType) {
    case kUniform:
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(fp_sample_chaining, rContext, canvas, errorMsg, 232, 306)
{
    ...
    // Switching it on actually triggers *more* shader compilation failures.
...
#else
        auto view = std::get<0>(GrMakeCachedBitmapProxyView(rContext, bmp, /*label=*/"FpSampleChaining", GrMipmapped::kNo));
        ...
        sdc->drawRect(nullptr, std::move(paint), GrAA::kNo, SkMatrix::Translate(x, y), SkRect::MakeIWH(64, 64));
        nextCol();
    };
