...
static sk_sp<SkShader> MakeLinear()
{
    ...
    return SkGradientShader::MakeLinear(kPts, kColors, kPos, std::size(kColors), SkTileMode::kClamp);
}
...
{
    return SkColorFilters::Lighting(SkColorSetRGB(0x00, 0x80, 0xFF), SkColorSetRGB(0xFF, 0x20, 0x00));
}
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
                ...
                for (int makeGray = 0; makeGray < 2; makeGray++) {
                    for (int makeMode = 0; makeMode < 2; ++makeMode) {
                        for (int alpha = 0; alpha < 2; ++alpha) {
                            ...
                            y += -metrics.fAscent;
                            canvas->drawSimpleText(text, textLen, SkTextEncoding::kUTF8, 380, y, shaderFont, shaderPaint);
                            y += metrics.fDescent + metrics.fLeading;
                        }
                    }
                }
        ...
        SkRect upperLeftClip = SkRect::MakeXYWH(bounds.left(), bounds.top(), boundsHalfWidth, boundsHalfHeight);
        SkRect lowerRightClip = SkRect::MakeXYWH(bounds.centerX(), bounds.centerY(), boundsHalfWidth, boundsHalfHeight);
