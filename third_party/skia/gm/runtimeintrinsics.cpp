...
static void draw_label(SkCanvas* canvas, const char* label)
{
    ...
    canvas->drawSimpleText(label, strlen(label), SkTextEncoding::kUTF8, (kBoxSize - bounds.width()) * 0.5f, (kLabelHeight + bounds.height()) * 0.5f, font, p);
...
{
    return SkStringPrintf("#version %s\n"
...
DEF_SIMPLE_GPU_GM_CAN_FAIL(runtime_intrinsics_common_es3, ctx, canvas, errorMsg, columns_to_width(6), rows_to_height(5))
{
    ...
    plot_es3(canvas, "floatBitsToInt(p).x", -2, 2, -2'000'000'000, 2'000'000'000, "floatBitsToInt(v)");
    ...
    plot_es3(canvas, "intBitsToFloat(pi).x", -2'000'000'000, 2'000'000'000, -2, 2, "intBitsToFloat(v)");
...
{
    return SkStringPrintf("uniform float%dx%d m1;" // dim, dim
                          "uniform float%dx%d m2;" // dim, dim
        SKSL_MATRIX_SELECTORS "half4 main(float2 p) {"
...
template <int N> static void plot_matrix_comp_mult(SkCanvas* canvas, std::array<float, N * N> mtx1, std::array<float, N * N> mtx2, const char* label)
{
    canvas->save();
...
{
    return SkStringPrintf("uniform float scale; uniform float bias;"
...
template <int N> static void plot_matrix_inverse(SkCanvas* canvas, std::array<float, N * N> mtx, const char* label)
{
    canvas->save();
...
DEF_SIMPLE_GM(runtime_intrinsics_matrix, canvas, columns_to_width(3), rows_to_height(2))
{
    ...
    plot_matrix_comp_mult<2>(canvas, { 1.00f, 0.0f, 2.0f, 0.5f }, { 0.75f, 2.0f, 0.2f, 1.2f }, "compMult(2x2)");

    plot_matrix_comp_mult<3>(canvas, { 1.00f, 0.0f, 2.0f, 0.5f, -1.0f, -2.0f, -0.5f, 4.00f, 0.25f },
        { 0.75f, 2.0f, 0.2f, 1.2f, -0.8f, -0.1f, -1.8f, 0.25f, 2.00f }, "compMult(3x3)");

    plot_matrix_comp_mult<4>(canvas, { 1.00f, 0.0f, 2.0f, 0.5f, -1.0f, -2.0f, -0.5f, 4.00f, 0.25f, 0.05f, 10.00f, -0.66f, -1.0f, -0.5f, 0.5f, 0.66f },
        { 0.75f, 2.0f, 0.2f, 1.2f, -0.8f, -0.1f, -1.8f, 0.25f, 2.00f, 2.00f, 0.03f, -1.00f, -1.0f, -0.5f, 1.7f, 0.66f }, "compMult(4x4)");
    next_row(canvas);
    ...
    plot_matrix_inverse<3>(canvas, { -1.13f, -2.96f, -0.14f, 1.45f, -1.88f, -1.02f, -2.54f, -2.58f, -1.17f }, "inverse(3x3)");

    plot_matrix_inverse<4>(canvas,
        { -1.51f, -3.95f, -0.19f, 1.93f, -2.51f, -1.35f, -3.39f, -3.45f, -1.56f, 1.61f, -0.22f, -1.08f, -2.81f, -2.14f, -0.09f, 3.00f }, "inverse(4x4)");
    next_row(canvas);
}
...
static SkString make_bvec_sksl(const char* type, const char* fn)
{
    ...
    return SkStringPrintf("uniform %s2 v1;"
                          "half4 main(float2 p) {"
...
template <typename T = float> static void plot_bvec(SkCanvas* canvas, const char* fn, const char* label)
{
    canvas->save();
