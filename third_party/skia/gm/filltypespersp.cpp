...
public:
    ...
    void makePath()
    {
        if (fPath.isEmpty()) {
            ...
            fPath = SkPathBuilder().addCircle(SkIntToScalar(50), SkIntToScalar(50), radius).addCircle(SkIntToScalar(100), SkIntToScalar(100), radius).detach();
        }
    }
...
protected:
    ...
    void showFour(SkCanvas* canvas, SkScalar scale, bool aa)
    {
        ...
        paint.setShader(SkGradientShader::MakeRadial(center, SkIntToScalar(100), colors, pos, std::size(colors), SkTileMode::kClamp));
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        bkgnrd.setShader(SkGradientShader::MakeRadial(center, SkIntToScalar(1000), colors, pos, std::size(colors), SkTileMode::kClamp));
        canvas->save();
