...
protected:
    ...
    void makePixmaps()
    {
        SkYUVAInfo yuvaInfo = SkYUVAInfo({ 8, 8 }, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, kJPEG_Full_SkYUVColorSpace);
        ...
        unsigned char innerY[16] = { 149, 160, 130, 105, 160, 130, 105, 149, 130, 105, 149, 160, 105, 149, 160, 130 };
    ...
    DrawResult onGpuSetup(GrDirectContext* context, SkString* errorMsg) override
    {
        ...
        for (int i = 0; i < fPixmaps.numPlanes(); ++i) {
            ...
            bitmap.setImmutable();
            views[i] = std::get<0>(GrMakeCachedBitmapProxyView(context, bitmap, /*label=*/"DrawResult_GpuSetup", GrMipmapped::kNo));
            if (!views[i]) {
    ...
    DrawResult onDraw(GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg) override
    {
        ...
        static const GrSamplerState::Filter kFilters[] = { GrSamplerState::Filter::kNearest, GrSamplerState::Filter::kLinear };
        ...
        for (uint32_t i = 0; i < std::size(kFilters); ++i) {
            ...
            for (uint32_t j = 0; j < GrSamplerState::kWrapModeCount + 1; ++j) {
                ...
                std::unique_ptr<GrFragmentProcessor> fp = GrYUVtoRGBEffect::Make(fProxies, samplerState, caps, SkMatrix::I(), subset);
                if (fp) {
                    GrPaint grPaint;
