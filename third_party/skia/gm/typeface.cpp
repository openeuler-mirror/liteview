...
static void draw_typeface_rendering_gm(SkCanvas* canvas, sk_sp<SkTypeface> face, SkGlyphID glyph)
{
    ...
    constexpr SkFontHinting hintingTypes[] = { SkFontHinting::kNone, SkFontHinting::kSlight, SkFontHinting::kNormal, SkFontHinting::kFull };
                    ...
                    for (const SkFontHinting& hinting : hintingTypes) {
                        ...
                        for (const bool& rotateABit : rotateABitTypes) {
                            ...
                            canvas->drawSimpleText(
                                &glyph, sizeof(glyph), SkTextEncoding::kGlyphID, x + subpixel.offset.x(), y + subpixel.offset.y(), font, paint);

                            SkScalar dx = SkScalarCeilToScalar(font.measureText(&glyph, sizeof(glyph), SkTextEncoding::kGlyphID)) + 5;
                            x += dx;
                            xMax = std::max(x, xMax);
                        }
                    }
        ...
        for (const bool& fakeBold : fakeBoldTypes) {
            ...
            for (const AliasType& alias : aliasTypes) {
                ...
                for (const StyleTests& style : styleTypes) {
                    ...
                    canvas->drawSimpleText(&glyph, sizeof(glyph), SkTextEncoding::kGlyphID, x, y, font, paint);
