...
protected:
    void onOnceBeforeDraw() override
    {
        ...
        text = "LCD!!!!!";
        font.measureText(text, strlen(text), SkTextEncoding::kUTF8, &bounds);
        ToolUtils::add_to_text_blob(&builder, text, font, xOffset - bounds.width() * 0.25f, yOffset - bounds.height() * 0.5f);
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (size_t x = 0; x < count; ++x) {
            ...
            if (x == (count >> 1) - 1) {
                canvas->translate(SkScalarFloorToScalar(bounds.width() + SkIntToScalar(25)), -(x * SkScalarFloorToScalar(bounds.height() + SkIntToScalar(25))));
            } else {
