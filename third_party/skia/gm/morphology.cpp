    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (unsigned j = 0; j < 4; ++j) {
            for (unsigned i = 0; i < std::size(samples); ++i) {
                ...
                if (j & 0x01) {
                    paint.setImageFilter(SkImageFilters::Erode(samples[i].fRadiusX, samples[i].fRadiusY, nullptr, cr));
                } else {
                    paint.setImageFilter(SkImageFilters::Dilate(samples[i].fRadiusX, samples[i].fRadiusY, nullptr, cr));
                }
