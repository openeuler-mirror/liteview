...
protected:
    ...
    void onOnceBeforeDraw() override
    {
        ...
        fPaths.push_back(SkPathBuilder().moveTo(6.5f, 5.5f).lineTo(3.5f, 0.5f).moveTo(0.5f, 5.5f).lineTo(3.5f, 0.5f).detach());
        ...
        fPaths.push_back(SkPathBuilder().moveTo(5.5f, 5.5f).lineTo(5.5f, 0.5f).lineTo(0.5f, 0.5f).detach());

        {
            ...
            SkPoint p0 = { kRad * SkScalarCos(SkDegreesToRadians(kStartAngle)), kRad * SkScalarSin(SkDegreesToRadians(kStartAngle)) };

            SkPoint p1
                = { kRad * SkScalarCos(SkDegreesToRadians(kStartAngle + kSweepAngle)), kRad * SkScalarSin(SkDegreesToRadians(kStartAngle + kSweepAngle)) };
...
static void draw_subdivided_quad(SkCanvas* canvas, int x0, int y0, int x1, int y1, SkColor color)
{
    ...
    canvas->drawPath(SkPathBuilder().moveTo(0, 0).quadTo(SkIntToScalar(x0), SkIntToScalar(y0), SkIntToScalar(x1), SkIntToScalar(y1)).detach(), paint);
}
