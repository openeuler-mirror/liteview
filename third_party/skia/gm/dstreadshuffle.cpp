...
protected:
    ...
    void drawShape(SkCanvas* canvas, SkPaint* paint, ShapeType type)
    {
        ...
        case kConvexPath_ShapeType:
            if (fConvexPath.isEmpty()) {
                SkPoint points[4];
                kRect.toQuad(points);
                fConvexPath = SkPathBuilder().moveTo(points[0]).quadTo(points[1], points[2]).quadTo(points[3], points[0]).detach();
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        if (SkColorType::kUnknown_SkColorType == canvas->imageInfo().colorType()) {
            ...
            info = SkImageInfo::Make(35, 35, canvas->imageInfo().colorType(), canvas->imageInfo().alphaType(), canvas->imageInfo().refColorSpace());
        }
        ...
        if (!surf) {
            ...
            if ((info.colorType() == kRGBA_8888_SkColorType || info.colorType() == kBGRA_8888_SkColorType) && info.colorType() != kN32_SkColorType) {
                info = SkImageInfo::Make(35, 35, kN32_SkColorType, canvas->imageInfo().alphaType(), canvas->imageInfo().refColorSpace());
            }
