...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        for (unsigned b = 0; b < std::size(blobConfigs); ++b) {
            ...
            SkPaint p;
            p.setAntiAlias(true);
            SkPoint offset = SkPoint::Make(SkIntToScalar(10 + 300 * (b % 2)), SkIntToScalar(20 + 150 * (b / 2)));
    ...
    sk_sp<SkTextBlob> makeBlob(unsigned blobIndex)
    {
        ...
        for (unsigned l = 0; l < std::size(blobConfigs[blobIndex]); ++l) {
            ...
            for (unsigned c = 0; c < std::size(blobConfigs[blobIndex][l]); ++c) {
                ...
                switch (cfg->pos) {
                case kDefault_Pos: {
                    const SkTextBlobBuilder::RunBuffer& buf = builder.allocRun(font, count, offset.x(), offset.y());
