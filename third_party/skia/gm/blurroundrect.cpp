...
class SimpleBlurRoundRectGM : public skiagm::GM {
    ...
    {
        return SkString("simpleblurroundrect");
    }
        ...
        for (size_t i = 0; i < std::size(blurRadii); ++i) {
            ...
            for (size_t j = 0; j < std::size(cornerRadii); ++j) {
                for (int k = 0; k <= 1; k++) {
                    ...
                    paint.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkBlurMask::ConvertRadiusToSigma(SkIntToScalar(blurRadii[i]))));
