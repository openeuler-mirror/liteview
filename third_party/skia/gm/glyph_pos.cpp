...
static void drawTestCase(SkCanvas* canvas, SkScalar textScale, SkScalar strokeWidth, SkPaint::Style strokeStyle)
{
    ...
    bool drawRef = !(canvas->getLocalToDeviceAs3x3().getType() & ~(SkMatrix::kIdentity_Mask | SkMatrix::kTranslate_Mask));

    SkRect bounds;
    if (drawRef) {
        SkScalar advance = font.measureText(kText, sizeof(kText) - 1, SkTextEncoding::kUTF8, &bounds, &paint);
