...
#define DEF_GM(CODE) static skiagm::GMRegistry SK_MACRO_APPEND_COUNTER(REG_)([]() { return std::unique_ptr<skiagm::GM>([]() { CODE; }()); });
...
#define DEF_SIMPLE_GPU_GM(NAME, GR_CONTEXT, CANVAS, W, H) DEF_SIMPLE_GPU_GM_BG(NAME, GR_CONTEXT, CANVAS, W, H, SK_ColorWHITE)
...
#define DEF_SIMPLE_GPU_GM_BG_CAN_FAIL(NAME, GR_CONTEXT, CANVAS, ERR_MSG, W, H, BGCOLOR)                                                                        \
    ...
    DEF_GM(return new skiagm::SimpleGpuGM(BGCOLOR, SkString(#NAME), { W, H }, SK_MACRO_CONCAT(NAME, _GM));)                                                    \
    skiagm::DrawResult SK_MACRO_CONCAT(NAME, _GM)(GrRecordingContext * GR_CONTEXT, SkCanvas * CANVAS, SkString * ERR_MSG)
...
public:
    ...
    inline static constexpr char kErrorMsg_DrawSkippedGpuOnly[] = "This test is for GPU configs only.";
    ...
    {
        return this->onGetControls(controls);
    }
    void setControls(const SkMetaData& controls)
    {
        this->onSetControls(controls);
    }
...
protected:
    ...
    virtual DrawResult onGpuSetup(GrDirectContext*, SkString*)
    {
...
public:
    GpuGM(SkColor backgroundColor = SK_ColorWHITE)
        : GM(backgroundColor)
    {
    }
