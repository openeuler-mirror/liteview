...
static void test_surface(SkCanvas* canvas, SkSurface* surf, bool usePaint)
{
    ...
    src2.setLTRB(SkIntToScalar(-surf->width() / 2), SkIntToScalar(-surf->height() / 2), SkIntToScalar(surf->width()), SkIntToScalar(surf->height()));
    ...
    canvas->drawImageRect(imgR, src1, dst1, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
    canvas->drawImageRect(imgG, src2, dst2, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
    canvas->drawImageRect(imgR, src3, dst3, sampling, usePaint ? &paint : nullptr, SkCanvas::kStrict_SrcRectConstraint);
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        sk_sp<SkSurface> surf2(SkSurface::MakeRenderTarget(canvas->recordingContext(), SkBudgeted::kNo, info));
...
static sk_sp<SkImage> make_picture(const SkImageInfo& info, GrRecordingContext*, void (*draw)(SkCanvas*))
{
    ...
    return SkImage::MakeFromPicture(recorder.finishRecordingAsPicture(), info.dimensions(), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}
...
DEF_SIMPLE_GPU_GM(new_texture_image, rContext, canvas, 280, 60)
{
    ...
    std::function<sk_sp<SkImage>()> imageFactories[] = { // Create sw raster image.
        ...
        [render_image] {
            ...
            return SkImage::MakeFromPicture(
                recorder.finishRecordingAsPicture(), SkISize::Make(kSize, kSize), nullptr, nullptr, SkImage::BitDepth::kU8, srgbColorSpace);
        },
        // Create a texture image
        [rContext, render_image]() -> sk_sp<SkImage> {
            auto surface(SkSurface::MakeRenderTarget(rContext, SkBudgeted::kYes, SkImageInfo::MakeS32(kSize, kSize, kPremul_SkAlphaType)));
            if (!surface) {
                return nullptr;
            }
