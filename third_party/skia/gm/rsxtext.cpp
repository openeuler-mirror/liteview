...
private:
    ...
    {
        const SkFontStyle style(SkFontStyle::kExtraBlack_Weight, SkFontStyle::kNormal_Width, SkFontStyle::kUpright_Slant);
        ...
        SkGlyphID glyphs[16];
        float widths[16];
        const auto glyph_count = font.textToGlyphs(txt, strlen(txt), SkTextEncoding::kUTF8, glyphs, std::size(glyphs));
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        this->draw_one(canvas, { kSZ * 1.1f, kSZ * 1.1f }, SkMatrix::Scale(2, 2), SkMatrix::RotateDeg(45));
    }
    ...
    static sk_sp<SkShader> make_shader(const SkMatrix& lm, const SkMatrix& outer_lm)
    {
        ...
        return surface->makeImageSnapshot()
            ->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(SkFilterMode::kLinear), &lm)
