...
static sk_sp<SkShader> make_shader0(int w, int h)
{
    ...
    SkColor colors[] = { SK_ColorBLACK, SK_ColorGREEN, SK_ColorCYAN, SK_ColorRED, 0, SK_ColorBLUE, SK_ColorWHITE };
...
static sk_sp<SkShader> make_shader1(int w, int h)
{
    ...
    return SkGradientShader::MakeRadial(SkPoint::Make(cx, cy), cx, colors, nullptr, std::size(colors), SkTileMode::kClamp);
}
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        static sk_sp<SkColorFilter> (*gColorFilterMakers[])() = { make_null_cf, make_cf0, make_cf1, make_cf2, make_cf3 };
        ...
        for (size_t bitmapMaker = 0; bitmapMaker < std::size(gBitmapMakers); ++bitmapMaker) {
            ...
            for (unsigned i = 0; i < std::size(gColorFilterMakers); ++i) {
                ...
                sk_sp<SkImageFilter> imageFilter1(SkImageFilters::ColorFilter(std::move(colorFilter1), nullptr));
                ...
                for (unsigned j = 1; j < std::size(gColorFilterMakers); ++j) {
                    ...
                    sk_sp<SkImageFilter> imageFilter2(SkImageFilters::ColorFilter(std::move(colorFilter2), imageFilter1, nullptr));
