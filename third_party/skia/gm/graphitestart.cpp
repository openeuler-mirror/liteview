...
sk_sp<SkShader> create_gradient_shader(SkRect r, const std::array<SkColor, 3>& colors, const std::array<float, 3>& offsets)
{
    SkPoint pts[2] = { { r.fLeft, r.fTop }, { r.fRight, r.fTop } };

    return SkGradientShader::MakeLinear(pts, colors.data(), offsets.data(), std::size(colors), SkTileMode::kClamp);
}
...
sk_sp<SkShader> create_image_shader(SkCanvas* destCanvas, SkTileMode tmX, SkTileMode tmY)
{
...

    {
        ...
        SkColor colors[3][3] = { { SK_ColorRED, SK_ColorDKGRAY, SK_ColorBLUE }, { SK_ColorLTGRAY, SK_ColorCYAN, SK_ColorYELLOW },
...
sk_sp<SkShader> create_blend_shader(SkCanvas* destCanvas, SkBlendMode bm)
{
    ...
    return SkShaders::Blend(bm, std::move(dst), create_image_shader(destCanvas, SkTileMode::kRepeat, SkTileMode::kRepeat));
}
...
void draw_gradient_tile(SkCanvas* canvas, SkRect clipRect)
{
    ...
    SkPaint p;
    p.setShader(create_gradient_shader(r, { SK_ColorRED, SK_ColorGREEN, SK_ColorBLUE }, { 0.0f, 0.75f, 1.0f }));
...
void draw_colorfilter_swatches(SkCanvas* canvas, SkRect clipRect)
{
    ...
    colorFilters[2]
        = SkColorFilters::Compose(SkColorFilters::TableARGB(nullptr, table3, table3, table3), SkColorFilters::TableARGB(nullptr, table2, table2, table2));
    ...
    for (int y = 0; y < kNumTilesPerSide; ++y) {
        for (int x = 0; x < kNumTilesPerSide; ++x) {
            SkRect r = SkRect::MakeXYWH(x * tileSize.width(), y * tileSize.height(), tileSize.width(), tileSize.height()).makeInset(1.0f, 1.0f);
            int colorFilterIndex = x * kNumTilesPerSide + y;
            p.setShader(create_gradient_shader(r, kGradientColors[colorFilterIndex], { 0.0f, 0.5f, 1.0f }));
...
protected:
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        // Upper-right corner
        draw_colorfilter_swatches(canvas, SkRect::MakeXYWH(2 * kTileWidth, 0, kTileWidth, kTileWidth));
        ...
        // Bottom-middle tile
        draw_blend_mode_swatches(canvas, SkRect::MakeXYWH(kTileWidth, 2 * kTileHeight, kTileWidth, kTileHeight));

        // Bottom-right corner
        {
            const SkRect kTile = SkRect::MakeXYWH(2 * kTileWidth, 2 * kTileHeight, kTileWidth, kTileHeight);
