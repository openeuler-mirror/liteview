...
protected:
    ...
    {
        return SkISize::Make((kSubPixelSteps + 1) * kTrans + kLabelOffsetX + kLabelPad, (kSubPixelSteps + 1) * kTrans + kLabelOffsetY + kLabelPad);
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int i = 0; i <= kSubPixelSteps + 1; ++i) {
            canvas->drawLine(
                0.5f, i * SkIntToScalar(kTrans) + 0.5f, SkIntToScalar(kTrans) * (kSubPixelSteps + 1) + 0.5f, i * SkIntToScalar(kTrans) + 0.5f, linePaint);
            canvas->drawLine(
                i * SkIntToScalar(kTrans) + 0.5f, 0.5f, i * SkIntToScalar(kTrans) + 0.5f, SkIntToScalar(kTrans) * (kSubPixelSteps + 1) + 0.5f, linePaint);
        }
        ...
        for (int i = 0; i <= kSubPixelSteps; ++i) {
            for (int j = 0; j <= kSubPixelSteps; ++j) {
                ...
                canvas->translate(j * (kTrans + 1.f / kSubPixelSteps) + 1, i * (kTrans + 1.f / kSubPixelSteps) + 1);
...
protected:
    ...
    {
        return SkString("pixel_snap_point");
    }
...
protected:
    SkString onShortName() override
    {
        return SkString("pixel_snap_line");
    }
    void drawElement(SkCanvas* canvas) override
    {
        SkPaint paint;
...
protected:
    SkString onShortName() override
    {
        return SkString("pixel_snap_rect");
    }
    void drawElement(SkCanvas* canvas) override
    {
        SkPaint paint;
...
protected:
    SkString onShortName() override
    {
        return SkString("pixel_snap_combo");
    }
    void drawElement(SkCanvas* canvas) override
    {
        SkPaint paint;
