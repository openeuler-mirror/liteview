    ...
    for (int i = 0; i < kRowCount; ++i) {
        for (int j = 0; j < kColCount; ++j) {
            ...
            if (sdc) {
                ...
                GrPaint grPaint;
                SkPaintToGrPaint(rContext, sdc->colorInfo(), paint, matrixProvider, props, &grPaint);
                sdc->fillRectWithEdgeAA(nullptr, std::move(grPaint), static_cast<GrQuadAAFlags>(aa), view, tile);
            } else {
                // Fallback to solid color on raster backend since the public API only has color
                SkColor color = alignGradients ? SK_ColorBLUE : (i * kColCount + j) % 2 == 0 ? SK_ColorBLUE : SK_ColorWHITE;
                canvas->experimental_DrawEdgeAAQuad(tile, nullptr, static_cast<SkCanvas::QuadAAFlags>(aa), color, SkBlendMode::kSrcOver);
            }
...
static void draw_color_tiles(SkCanvas* canvas, bool multicolor)
{
    for (int i = 0; i < kRowCount; ++i) {
        for (int j = 0; j < kColCount; ++j) {
            ...
            canvas->experimental_DrawEdgeAAQuad(tile, nullptr, static_cast<SkCanvas::QuadAAFlags>(aa), color.toSkColor(), SkBlendMode::kSrcOver);
        }
    }
}
