...
static constexpr GrGeometryProcessor::Attribute gVertex = { "position", kFloat2_GrVertexAttribType, SkSLType::kFloat2 };
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) ClockwiseTestProcessor(readSkFragCoord); });
    }
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> ClockwiseTestProcessor::makeProgramImpl(const GrShaderCaps&) const
{
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            args.fFragBuilder->codeAppendf("half4 %s = sk_Clockwise ? half4(0,1,0,1) : half4(1,0,0,1);", args.fOutputColor);
            if (!proc.readSkFragCoord()) {
                args.fFragBuilder->codeAppendf("const half4 %s = half4(1);", args.fOutputCoverage);
            } else {
                // Verify layout(origin_upper_left) on gl_FragCoord does not affect gl_FrontFacing.
                args.fFragBuilder->codeAppendf("half4 %s = half4(min(half(sk_FragCoord.y), 1));", args.fOutputCoverage);
            }
        }
    };
...
private:
        ...
        GrAppliedClip&& appliedClip, const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) const
    {
        ...
        return sk_gpu_test::CreateProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, geomProc, SkBlendMode::kPlus,
    ...
    void onPrepare(GrOpFlushState* flushState) override
    {
        ...
        fVertexBuffer = flushState->resourceProvider()->createBuffer(vertices, sizeof(vertices), GrGpuBufferType::kVertex, kStatic_GrAccessPattern);
    }
...
DrawResult ClockwiseGM::onDraw(GrRecordingContext* rContext, SkCanvas* canvas, SkString* errorMsg)
{
    ...
    if (auto topLeftSDC = skgpu::v1::SurfaceDrawContext::Make(rContext, sdcColorType, nullptr, SkBackingFit::kExact, { 100, 200 }, SkSurfaceProps(),
            /*label=*/ {}, 1, GrMipmapped::kNo, GrProtected::kNo, kTopLeft_GrSurfaceOrigin, SkBudgeted::kYes)) {
        ...
        sdc->drawTexture(nullptr, topLeftSDC->readSurfaceView(), sdc->colorInfo().alphaType(), GrSamplerState::Filter::kNearest,
            GrSamplerState::MipmapMode::kNone, SkBlendMode::kSrcOver, SK_PMColor4fWHITE, { 0, 0, 100, 200 }, { 100, 0, 200, 200 }, GrQuadAAFlags::kNone,
            SkCanvas::SrcRectConstraint::kStrict_SrcRectConstraint, SkMatrix::I(), nullptr);
    }

    // Draw the test to an off-screen, bottom-up render target.
    if (auto topLeftSDC = skgpu::v1::SurfaceDrawContext::Make(rContext, sdcColorType, nullptr, SkBackingFit::kExact, { 100, 200 }, SkSurfaceProps(),
            /*label=*/ {}, 1, GrMipmapped::kNo, GrProtected::kNo, kBottomLeft_GrSurfaceOrigin, SkBudgeted::kYes)) {
        topLeftSDC->clear(SK_PMColor4fTRANSPARENT);
        topLeftSDC->addDrawOp(ClockwiseTestOp::Make(rContext, false, 0));
        topLeftSDC->addDrawOp(ClockwiseTestOp::Make(rContext, true, 100));
        sdc->drawTexture(nullptr, topLeftSDC->readSurfaceView(), sdc->colorInfo().alphaType(), GrSamplerState::Filter::kNearest,
            GrSamplerState::MipmapMode::kNone, SkBlendMode::kSrcOver, SK_PMColor4fWHITE, { 0, 0, 100, 200 }, { 200, 0, 300, 200 }, GrQuadAAFlags::kNone,
            SkCanvas::SrcRectConstraint::kStrict_SrcRectConstraint, SkMatrix::I(), nullptr);
    }
