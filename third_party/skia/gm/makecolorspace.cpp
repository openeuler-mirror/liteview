...
sk_sp<SkImage> make_color_space(sk_sp<SkImage> orig, sk_sp<SkColorSpace> colorSpace, GrDirectContext* direct)
{
...
{
    sk_sp<SkColorSpace> wideGamut = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kAdobeRGB);
...
DEF_SIMPLE_GM_BG(makecolortypeandspace, canvas, 128 * 3, 128 * 4, SK_ColorWHITE)
{
    ...
    for (bool lazy : { true, false }) {
        for (int j = 0; j < 2; ++j) {
            ...
            auto imageGray = image->makeColorTypeAndColorSpace(kGray_8_SkColorType, image->refColorSpace(), direct);
...
DEF_SIMPLE_GM_CAN_FAIL(reinterpretcolorspace, canvas, errorMsg, 128 * 3, 128 * 3)
{
    ...
    canvas->drawImage(image->makeColorSpace(spin, nullptr)->reinterpretColorSpace(srgb), 256.0f, 0.0f);
