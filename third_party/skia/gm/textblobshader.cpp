...
private:
    void onOnceBeforeDraw() override
    {
        ...
        fShader = SkGradientShader::MakeRadial(SkPoint::Make(SkIntToScalar(sz.width() / 2), SkIntToScalar(sz.height() / 2)), sz.width() * .66f, colors, pos,
            std::size(colors), SkTileMode::kRepeat);
    }
    ...
    void onDraw(SkCanvas* canvas) override
    {
        ...
        for (int i = 0; i < kXCount; ++i) {
            for (int j = 0; j < kYCount; ++j) {
                canvas->drawTextBlob(fBlob, SkIntToScalar(i * sz.width() / kXCount), SkIntToScalar(j * sz.height() / kYCount), p);
            }
        }
    }
