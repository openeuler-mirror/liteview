...
static const TrickyCubic kTrickyCubics[] = {
    ...
    { { { 400.75f, 100.05f }, { 400.75f, 100.05f }, { 100.05f, 300.95f }, { 100.05f, 300.95f } }, 4, CellFillMode::kStretch }, // Flat line with no turns
    ...
    { { { 1, 1 }, { 2, 1 }, { 1, 1 }, { 1, std::numeric_limits<float>::quiet_NaN() } }, 3, CellFillMode::kStretch }, // Flat QUAD with a cusp
    { { { 1, 1 }, { 100, 1 }, { 25, 1 }, { .3f, std::numeric_limits<float>::quiet_NaN() } }, 3, CellFillMode::kStretch }, // Flat CONIC with a cusp
    { { { 1, 1 }, { 100, 1 }, { 25, 1 }, { 1.5f, std::numeric_limits<float>::quiet_NaN() } }, 3, CellFillMode::kStretch }, // Flat CONIC with a cusp
};
...
static void draw_test(SkCanvas* canvas, SkPaint::Cap cap, SkPaint::Join join)
{
    ...
    for (size_t i = 0; i < std::size(kTrickyCubics); ++i) {
        ...
        auto cellRect = SkRect::MakeXYWH((i % kNumCols) * kCellSize, (i / kNumCols) * kCellSize, kCellSize, kCellSize);
        ...
        if (fillMode == CellFillMode::kStretch) {
            ...
            matrix.setTranslate(cellRect.x() + kStrokeWidth + (cellRect.width() - strokeBounds.width()) / 2,
