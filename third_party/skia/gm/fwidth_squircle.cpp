...
static constexpr GrGeometryProcessor::Attribute gVertex = { "bboxcoord", kFloat2_GrVertexAttribType, SkSLType::kFloat2 };
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) FwidthSquircleTestProcessor(viewMatrix); });
    }
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> FwidthSquircleTestProcessor::makeProgramImpl(const GrShaderCaps&) const
{
    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            fViewMatrixHandle = uniforms->addUniform(nullptr, kVertex_GrShaderFlag, SkSLType::kFloat3x3, "viewmatrix");
            ...
            f->codeAppendf("float x = abs(%s.x), y = abs(%s.y);", squircleCoord.fsIn(), squircleCoord.fsIn());
...
private:
        ...
        GrAppliedClip&& appliedClip, const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) const
    {
        ...
        return sk_gpu_test::CreateProgramInfo(caps, arena, writeView, usesMSAASurface, std::move(appliedClip), dstProxyView, geomProc, SkBlendMode::kSrcOver,
