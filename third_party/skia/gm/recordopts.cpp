...
{
    drawPaint->setImageFilter(SkImageFilters::ColorFilter(make_detector_color_filter(), drawPaint->refImageFilter()));
}
...
static void draw_svg_opacity_and_filter_layer_sequence(SkCanvas* canvas, SkColor shapeColor, InstallDetectorFunc installDetector)
{
    ...
    sk_sp<SkPicture> shape;
    {
        ...
        SkCanvas* recordCanvas = recorder.beginRecording(SkIntToScalar(kTestRectSize + 2), SkIntToScalar(kTestRectSize + 2));
        SkPaint shapePaint;
...
DEF_SIMPLE_GM(recordopts, canvas, (kTestRectSize + 1) * 2, (kTestRectSize + 1) * 15)
{
    ...
    for (size_t k = 0; k < std::size(funcs); ++k) {
        ...
        {
            SkPictureRecorder recorder;
            drawTestSequence(recorder.beginRecording(SkIntToScalar(kTestRectSize), SkIntToScalar(kTestRectSize)), shapeColor, no_detector_install);
    ...
    SkColor shapeColors[] = {
        SkColorSetARGB(255, 0, kDetectorGreenValue, 0), SkColorSetARGB(255, 0, (kDetectorGreenValue + 1), 0) // This tests that detectors work.
    };

    InstallDetectorFunc detectorInstallFuncs[] = { install_detector_image_filter, install_detector_color_filter };
    ...
    for (size_t i = 0; i < std::size(shapeColors); ++i) {
        ...
        for (size_t j = 0; j < std::size(detectorInstallFuncs); ++j) {
            ...
            for (size_t k = 0; k < std::size(funcs); ++k) {
                ...
                {
                    SkPictureRecorder recorder;
                    drawTestSequence(recorder.beginRecording(SkIntToScalar(kTestRectSize), SkIntToScalar(kTestRectSize)), shapeColor, detectorInstallFunc);
                    recorder.finishRecordingAsPicture()->playback(canvas);
                    if (direct) {
                        direct->flushAndSubmit();
                    }
