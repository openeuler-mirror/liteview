...
struct TileModePair {
    ...
    bool operator==(const TileModePair& other) const
    {
...
public:
    BlenderID()
        : fID(0)
    {
    } // 0 is an invalid blender ID
    BlenderID(const BlenderID& src)
        : fID(src.fID)
    {
    }
    ...
    bool operator==(const BlenderID& other) const
    {
...
public:
    ...
    CombinationOption addChildOption(int childIndex, ShaderType, int minNumStops, int maxNumStops);

    CombinationOption addChildOption(int childIndex, ShaderType, SkSpan<TileModePair> tileModes);
...
private:
    ...
    SkArenaAllocWithReset* arena()
    {
        return fArena.get();
    }

    template <typename T, typename... Args> Option* allocInArena(Args&&... args);
    ...
    void buildCombinations(SkShaderCodeDictionary*, const std::function<void(SkUniquePaintParamsID)>&);
