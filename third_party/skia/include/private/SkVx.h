...
template <typename D, typename S> SI D bit_pun(const S&);

// All Vec have the same simple memory layout, the same as `T vec[N]`.
template <int N, typename T> struct alignas(N * sizeof(T)) VecStorage {
    SKVX_ALWAYS_INLINE VecStorage() = default;
    SKVX_ALWAYS_INLINE VecStorage(T s)
        : lo(s)
        , hi(s)
    {
    }
...
template <typename T> struct VecStorage<4, T> {
    SKVX_ALWAYS_INLINE VecStorage() = default;
    SKVX_ALWAYS_INLINE VecStorage(T s)
        : lo(s)
        , hi(s)
    ...
    SKVX_ALWAYS_INLINE VecStorage(T x, T y, T z, T w)
        : lo(x, y)
        , hi(z, w)
    {
    }
    SKVX_ALWAYS_INLINE VecStorage(Vec<2, T> xy, T z, T w)
        : lo(xy)
        , hi(z, w)
    {
    }
    SKVX_ALWAYS_INLINE VecStorage(T x, T y, Vec<2, T> zw)
        : lo(x, y)
        , hi(zw)
    {
    }
    SKVX_ALWAYS_INLINE VecStorage(Vec<2, T> xy, Vec<2, T> zw)
        : lo(xy)
        , hi(zw)
    {
    }
...
template <typename T> struct VecStorage<2, T> {
    SKVX_ALWAYS_INLINE VecStorage() = default;
    SKVX_ALWAYS_INLINE VecStorage(T s)
        : lo(s)
        , hi(s)
    ...
    SKVX_ALWAYS_INLINE VecStorage(T x, T y)
        : lo(x)
        , hi(y)
    {
    }
...
template <int N, typename T> struct alignas(N * sizeof(T)) Vec : public VecStorage<N, T> {
...
template <typename T> struct Vec<1, T> {
    ...
    SKVX_ALWAYS_INLINE T operator[](int) const
    {
        return val;
    }
    SKVX_ALWAYS_INLINE T& operator[](int)
    {
        return val;
    }
...
template <typename D, typename S> SI D bit_pun(const S& s)
{
...
SINT Vec<N, T> if_then_else(const Vec<N, M<T>>& cond, const Vec<N, T>& t, const Vec<N, T>& e)
{
    ...
    if constexpr (N * sizeof(T) == 32) {
        return bit_pun<Vec<N, T>>(_mm256_blendv_epi8(bit_pun<__m256i>(e), bit_pun<__m256i>(t), bit_pun<__m256i>(cond)));
    }
#endif
#if SKVX_USE_SIMD && defined(__SSE4_1__)
    if constexpr (N * sizeof(T) == 16) {
        return bit_pun<Vec<N, T>>(_mm_blendv_epi8(bit_pun<__m128i>(e), bit_pun<__m128i>(t), bit_pun<__m128i>(cond)));
    }
#endif
#if SKVX_USE_SIMD && defined(__ARM_NEON)
    if constexpr (N * sizeof(T) == 16) {
        return bit_pun<Vec<N, T>>(vbslq_u8(bit_pun<uint8x16_t>(cond), bit_pun<uint8x16_t>(t), bit_pun<uint8x16_t>(e)));
    }
#endif
...
SINT bool any(const Vec<N, T>& x)
{
    ...
    if constexpr (N * sizeof(T) == 8) {
        return vmaxv_u8(bit_pun<uint8x8_t>(x)) > 0;
    }
    if constexpr (N * sizeof(T) == 16) {
        return vmaxvq_u8(bit_pun<uint8x16_t>(x)) > 0;
    }
#endif
...
// true lane has a mix of 0 and 1 bits.
    ...
    if constexpr (sizeof(T) == 1 && N == 8) {
        return vminv_u8(bit_pun<uint8x8_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 1 && N == 16) {
        return vminvq_u8(bit_pun<uint8x16_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 2 && N == 4) {
        return vminv_u16(bit_pun<uint16x4_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 2 && N == 8) {
        return vminvq_u16(bit_pun<uint16x8_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 4 && N == 2) {
        return vminv_u32(bit_pun<uint32x2_t>(x)) > 0;
    }
    if constexpr (sizeof(T) == 4 && N == 4) {
        return vminvq_u32(bit_pun<uint32x4_t>(x)) > 0;
    }
#endif
#if SKVX_USE_SIMD && defined(__wasm_simd128__)
    if constexpr (N == 4 && sizeof(T) == 4) {
...
{
    Vec<N, uint32_t> wide = cast<uint32_t>(x), s = wide & 0x8000, em = wide ^ s, is_norm = em > 0x3ff, norm = (em << 13) + ((127 - 15) << 23);
...
SI Vec<1, uint16_t> to_half(const Vec<1, float>& x)
{
    return to_half_finite_ftz(x);
}
SI Vec<1, float> from_half(const Vec<1, uint16_t>& x)
{
...
SIN Vec<N, uint16_t> to_half(const Vec<N, float>& x)
{
    ...
    if constexpr (N == 8) {
        return bit_pun<Vec<N, uint16_t>>(_mm256_cvtps_ph(bit_pun<__m256>(x), _MM_FROUND_TO_NEAREST_INT));
    }
#endif
#if SKVX_USE_SIMD && defined(__aarch64__)
    if constexpr (N == 4) {
