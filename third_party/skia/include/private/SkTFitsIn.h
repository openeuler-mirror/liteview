    ...
    SkTFitsIn(S src)
{
    ...
    if ((std::is_signed<S>::value && std::is_unsigned<D>::value && sizeof(S) <= sizeof(D))) {
        ...
        if ((std::is_signed<D>::value && std::is_unsigned<S>::value && sizeof(D) <= sizeof(S)))
            return src <= (S)std::numeric_limits<typename sk_strip_enum<D>::type>::max();
        else {
#if !defined(SK_DEBUG) && !defined(__MSVC_RUNTIME_CHECKS)
            // Correct (simple) version. This trips up MSVC's /RTCc run-time checking.
            return (S)(D)src == src;
            ...
            if ((std::is_signed<S>::value)) {
                bool b = (intmax_t)src >= (intmax_t)std::numeric_limits<typename sk_strip_enum<D>::type>::min();
                b &= (intmax_t)src <= (intmax_t)std::numeric_limits<typename sk_strip_enum<D>::type>::max();
                return b;
            } else {
                return (uintmax_t)src <= (uintmax_t)std::numeric_limits<typename sk_strip_enum<D>::type>::max();
            }
            return false;
#endif
        }
