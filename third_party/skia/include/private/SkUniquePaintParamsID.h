...
public:
    explicit SkUniquePaintParamsID(uint32_t id)
        : fID(id)
    {
        SkASSERT(id != SK_InvalidUniqueID);
    }

    static SkUniquePaintParamsID InvalidID()
    {
        return SkUniquePaintParamsID();
    }

    SkUniquePaintParamsID()
        : fID(SK_InvalidUniqueID)
    {
    }

    bool operator==(const SkUniquePaintParamsID& that) const
    {
        return fID == that.fID;
    }
    bool operator!=(const SkUniquePaintParamsID& that) const
