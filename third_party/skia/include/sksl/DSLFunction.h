...
public:
    ...
    template <class... Parameters> DSLFunction(const DSLModifiers& modifiers, const DSLType& returnType, std::string_view name, Parameters&... parameters)
    {
    ...
    DSLFunction(std::string_view name, const DSLModifiers& modifiers, const DSLType& returnType, SkTArray<DSLParameter*> parameters, Position pos = {})
    {
    ...
    template <class... Stmt> void define(Stmt... stmts)
    {
     ...
     */
    template <class... Args> DSLExpression operator()(Args&&... args)
    {
...
private:
    ...
    template <class... RemainingArgs> void collectArgs(ExpressionArray& args, DSLVar& var, RemainingArgs&&... remaining)
    {
        args.push_back(DSLExpression(var).release());
        collectArgs(args, std::forward<RemainingArgs>(remaining)...);
    }

    template <class... RemainingArgs> void collectArgs(ExpressionArray& args, DSLExpression expr, RemainingArgs&&... remaining)
    {
        args.push_back(expr.release());
        collectArgs(args, std::forward<RemainingArgs>(remaining)...);
    }

    void init(DSLModifiers modifiers, const DSLType& returnType, std::string_view name, SkTArray<DSLParameter*> params, Position pos);
