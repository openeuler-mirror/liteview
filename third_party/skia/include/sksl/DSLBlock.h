...
class DSLBlock {
public:
    template <class... Statements> DSLBlock(Statements... statements)
    {
        fStatements.reserve_back(sizeof...(statements));
        ((void)fStatements.push_back(DSLStatement(statements.release()).release()), ...);
    }

    DSLBlock(SkSL::StatementArray statements, std::shared_ptr<SymbolTable> symbols = nullptr, Position pos = {});

    DSLBlock(SkTArray<DSLStatement> statements, std::shared_ptr<SymbolTable> symbols = nullptr, Position pos = {});
