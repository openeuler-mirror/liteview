...
private:
    ...
    SkMeshSpecification(SkSpan<const Attribute>, size_t, SkSpan<const Varying>, std::vector<Uniform> uniforms, std::unique_ptr<const SkSL::Program>,
        std::unique_ptr<const SkSL::Program>, ColorType, bool hasLocalCoords, sk_sp<SkColorSpace>, SkAlphaType);
...
public:
    ...
    static SkMesh Make(
        sk_sp<SkMeshSpecification>, Mode, sk_sp<VertexBuffer>, size_t vertexCount, size_t vertexOffset, sk_sp<const SkData> uniforms, const SkRect& bounds);

    static SkMesh MakeIndexed(sk_sp<SkMeshSpecification>, Mode, sk_sp<VertexBuffer>, size_t vertexCount, size_t vertexOffset, sk_sp<IndexBuffer>,
