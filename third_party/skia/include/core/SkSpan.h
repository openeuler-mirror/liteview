...
public:
    ...
    constexpr SkSpan(T* ptr, size_t size)
        : fPtr { ptr }
        , fSize { size }
    {
...
template <typename T> SkSpan(std::initializer_list<T>) -> SkSpan<std::remove_pointer_t<decltype(std::data(std::declval<std::initializer_list<T>>()))>>;
