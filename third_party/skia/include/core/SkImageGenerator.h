...
public:
     ...
     */
    GrSurfaceProxyView generateTexture(GrRecordingContext*, const SkImageInfo& info, GrMipmapped mipmapped, GrImageTexGenPolicy);
#endif

#if SK_GRAPHITE_ENABLED
    sk_sp<SkImage> makeTextureImage(skgpu::graphite::Recorder*, const SkImageInfo&, skgpu::graphite::Mipmapped);
#endif
     ...
     */
    static std::unique_ptr<SkImageGenerator> MakeFromEncoded(sk_sp<SkData>, std::optional<SkAlphaType> = std::nullopt);
    ...
    static std::unique_ptr<SkImageGenerator> MakeFromPicture(
        const SkISize&, sk_sp<SkPicture>, const SkMatrix*, const SkPaint*, SkImage::BitDepth, sk_sp<SkColorSpace>, SkSurfaceProps props = {});
...
#if SK_SUPPORT_GPU
    // returns nullptr
    virtual GrSurfaceProxyView onGenerateTexture(GrRecordingContext*, const SkImageInfo&, GrMipmapped, GrImageTexGenPolicy);
...
#if SK_GRAPHITE_ENABLED
    virtual sk_sp<SkImage> onMakeTextureImage(skgpu::graphite::Recorder*, const SkImageInfo&, skgpu::graphite::Mipmapped);
#endif
...
private:
    ...
    static std::unique_ptr<SkImageGenerator> MakeFromEncodedImpl(sk_sp<SkData>, std::optional<SkAlphaType>);
