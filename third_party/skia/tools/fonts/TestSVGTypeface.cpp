...
TestSVGTypeface::TestSVGTypeface(
    const char* name, int upem, const SkFontMetrics& fontMetrics, SkSpan<const SkSVGTestTypefaceGlyphData> data, const SkFontStyle& style)
...
protected:
    ...
    bool generateAdvance(SkGlyph* glyph) override
    {
        ...
        const SkVector advance = fMatrix.mapXY(SkFloatToScalar(glyph->fAdvanceX), SkFloatToScalar(glyph->fAdvanceY));
    ...
    void generateImage(const SkGlyph& glyph) override
    {
        ...
        bm.installPixels(SkImageInfo::MakeN32(glyph.fWidth, glyph.fHeight, kPremul_SkAlphaType), glyph.fImage, glyph.rowBytes());
        bm.eraseColor(0);
...
void TestSVGTypeface::exportTtxCommon(SkWStream* out, const char* type, const SkTArray<GlyfInfo>* glyfInfo) const
{
...
void convert_noninflect_cubic_to_quads(const SkPoint p[4], SkScalar toleranceSqd, SkTArray<SkPoint, true>* quads, int sublevel = 0)
{
...
public:
    SkCOLRCanvas(SkRect glyphBounds, const TestSVGTypeface& typeface, SkGlyphID glyphId, TestSVGTypeface::GlyfInfo* glyf, SkTHashMap<SkColor, int>* colors,
        SkWStream* out)
