...
static bool ColrV1VariationsEnabledForTest()
{
    return true;
}
    ...
    , fPerspectiveMode(kPerspective_Off)
{
    ...
    fCommands.addCommand(skui::Key::kRight, "Right", "Navigation", "Next slide",
        [this]() { this->setCurrentSlide(fCurrentSlide < fSlides.count() - 1 ? fCurrentSlide + 1 : 0); });
    fCommands.addCommand(skui::Key::kLeft, "Left", "Navigation", "Previous slide",
        [this]() { this->setCurrentSlide(fCurrentSlide > 0 ? fCurrentSlide - 1 : fSlides.count() - 1); });
    ...
    fCommands.addCommand('d', "Modes", "Change rendering backend", [this]() {
        sk_app::Window::BackendType newBackend = (sk_app::Window::BackendType)((fBackendType + 1) % sk_app::Window::kBackendTypeCount);
        ...
        if (newBackend == sk_app::Window::kVulkan_BackendType) {
            newBackend = (sk_app::Window::BackendType)((newBackend + 1) % sk_app::Window::kBackendTypeCount);
...
#endif
              { ".riv", "rive-dir", FLAGS_rives, [](const SkString& name, const SkString& path) -> sk_sp<Slide> { return sk_make_sp<RiveSlide>(name, path); } },
#if defined(SK_ENABLE_SVG)
...
void Viewer::drawSlide(SkSurface* surface)
{
    ...
    if (fSaveToSKP) {
        ...
        SkCanvas* recorderCanvas = recorder.beginRecording(SkRect::Make(fSlides[fCurrentSlide]->getDimensions()));
    ...
    auto make_surface = [=](int w, int h) {
        ...
        return Window::kRaster_BackendType == this->fBackendType ? SkSurface::MakeRaster(info, &props) : slideCanvas->makeSurface(info, &props);
    };
        ...
        if (ImGui::Begin("Tools", &fShowImGuiDebugWindow, ImGuiWindowFlags_AlwaysVerticalScrollbar)) {
            if (ImGui::CollapsingHeader("Backend")) {
                ...
                if (newBackend != fBackendType) {
                    fDeferredActions.push_back([=]() { this->setBackend(static_cast<sk_app::Window::BackendType>(newBackend)); });
                }
            ...
            if (ImGui::CollapsingHeader("Paint")) {
                ...
                if (ImGui::Combo("Anti-Alias", &aliasIdx, "Default\0Alias\0Normal\0AnalyticAAEnabled\0AnalyticAAForced\0\0")) {
                    gSkUseAnalyticAA = fPaintOverrides.fOriginalSkUseAnalyticAA;
                    gSkForceAnalyticAA = fPaintOverrides.fOriginalSkForceAnalyticAA;
                    if (aliasIdx == 0) {
                        fPaintOverrides.fAntiAliasState = SkPaintFields::AntiAliasState::Alias;
            ...
            if (ImGui::CollapsingHeader("Shaders")) {
                ...
                if (gLoadPending) {
                    auto collectShaders = [this](sk_sp<const SkData> key, sk_sp<SkData> data, const SkString& description, int hitCount) {
                        ...
                        GrPersistentCacheUtils::UnpackCachedShaders(&reader, entry.fShader, entry.fInputs, kGrShaderTypeCount);
                    };
                    fCachedShaders.reset();
                ...
                if (doApply) {
                    ...
                    for (auto& entry : fCachedShaders) {
                        ...
                        auto data = GrPersistentCacheUtils::PackCachedShaders(entry.fShaderType, entry.fShader, entry.fInputs, kGrShaderTypeCount);
                ...
                auto showVMEntry = [this](const SkVMBlitter::Key* key, skvm::Program* program) {
                    ...
                    if (inTreeNode) {
                        ...
                        if (!asmString) {
                            ...
                            asmString = fDisassemblyCache.set(*key, std::string((const char*)asmData->data(), asmData->size()));
                        }
                    ...
                    if (hovered) {
                        ...
                        skvm::Program highlightProgram = build_skvm_highlight_program(static_cast<SkColorType>(key->colorType), program->nargs());
...
void Viewer::updateUIState()
{
    ...
    // Backend state
    WriteStateObject(writer, kBackendStateName, kBackendTypeStrings[fBackendType], [](SkJSONWriter& writer) {
    ...
    // Path renderer state
    GpuPathRenderers pr = fWindow->getRequestedDisplayParams().fGrContextOptions.fGpuPathRenderers;
    WriteStateObject(writer, kPathRendererStateName, gPathRendererNames[pr].c_str(), [this](SkJSONWriter& writer) {
        auto ctx = fWindow->directContext();
        if (!ctx) {
