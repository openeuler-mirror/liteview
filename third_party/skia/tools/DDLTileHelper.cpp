...
void DDLTileHelper::TileData::init(
    int id, GrDirectContext* direct, const SkSurfaceCharacterization& dstSurfaceCharacterization, const SkIRect& clip, const SkIRect& paddingOutsets)
{
    fID = id;
    fClip = clip;
...
sk_sp<SkSurface> DDLTileHelper::TileData::makeWrappedTileDest(GrRecordingContext* rContext)
{
    ...
    return SkSurface::MakeFromBackendTexture(rContext, promiseImageTexture->backendTexture(), fPlaybackChar.origin(), fPlaybackChar.sampleCount(),
...
sk_sp<SkImage> DDLTileHelper::TileData::makePromiseImageForDst(sk_sp<GrContextThreadSafeProxy> threadSafeProxy)
{
    ...
    sk_sp<SkImage> promiseImage = SkImage::MakePromiseTexture(std::move(threadSafeProxy), fCallbackContext->backendFormat(), this->paddedRectSize(),
        GrMipmapped::kNo, GrSurfaceOrigin::kBottomLeft_GrSurfaceOrigin, fPlaybackChar.colorType(), kPremul_SkAlphaType, fPlaybackChar.refColorSpace(),
...
void DDLTileHelper::TileData::CreateBackendTexture(GrDirectContext* direct, TileData* tile)
{
    ...
    GrBackendTexture beTex
        = direct->createBackendTexture(c.width(), c.height(), c.colorType(), GrMipmapped(c.isMipMapped()), GrRenderable::kYes, GrProtected::kNo,
...
DDLTileHelper::DDLTileHelper(GrDirectContext* direct, const SkSurfaceCharacterization& dstChar, const SkIRect& viewport, int numXDivisions, int numYDivisions,
