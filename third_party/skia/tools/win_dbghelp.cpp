...
// q - quit cdb.exe
#define CDB_PRINT_CALLSTACK_CURRENT_THREAD                                                                                                                     \
    "? " MARKER_THREAD_CALLSTACK_START_NUMBER "; k; ? " MARKER_THREAD_CALLSTACK_STOP_NUMBER "; .ecxr; ? " MARKER_EXCEPTION_CALLSTACK_START_NUMBER              \
    "; k; ? " MARKER_EXCEPTION_CALLSTACK_STOP_NUMBER "; q"
...
{
    strncpyOrSetBlank(debug_binaries_path, binaries_path, sizeof(debug_binaries_path));
}
 ...
 */
static void printCallstack(const char* filename, const char* start, const char* stop)
{
...
#define BUILD_UNIQUE_FILENAME(var, ext, szPath, szAppName, szVersion, stLocalTime)                                                                             \
    sprintf(szFileName, "%s%s\\%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld" ext, szPath, szAppName, szVersion, stLocalTime.wYear, stLocalTime.wMonth,                 \
        stLocalTime.wDay, stLocalTime.wHour, stLocalTime.wMinute, stLocalTime.wSecond, GetCurrentProcessId(), GetCurrentThreadId());
...
int GenerateDumpAndPrintCallstack(EXCEPTION_POINTERS* pExceptionPointers)
{
    ...
    if (cdbExePath && *cdbExePath != '\0') {
        ...
        sprintf(command, "%s -y \"%s\" -i \"%s\" -z \"%s\" -c \"%s\" -kqm >\"%s\"", cdbExePath, getBinariesPath(), getBinariesPath(), szFileName,
            CDB_PRINT_CALLSTACK_CURRENT_THREAD, szFileNameOutput);
        system(command);

        printf("\nThread Callstack:\n");
        printCallstack(szFileNameOutput, MARKER_THREAD_CALLSTACK_START, MARKER_THREAD_CALLSTACK_STOP);

        printf("\nException Callstack:\n");
        printCallstack(szFileNameOutput, MARKER_EXCEPTION_CALLSTACK_START, MARKER_EXCEPTION_CALLSTACK_STOP);
    } else {
