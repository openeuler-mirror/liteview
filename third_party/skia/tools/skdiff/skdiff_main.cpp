    ...
    bool getBounds, bool verbose, DiffSummary* summary)
{
    ...
    if (!baseFiles.empty()) {
        qsort(baseFiles.begin(), baseFiles.count(), sizeof(SkString), SkCastForQSort(compare_file_name_metrics));
    }
    if (!comparisonFiles.empty()) {
        qsort(comparisonFiles.begin(), comparisonFiles.count(), sizeof(SkString), SkCastForQSort(compare_file_name_metrics));
    }
