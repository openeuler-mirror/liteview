...
public:
    ...
    public:
        ScopedPurgeCache(ClientDiscardableManager* manager)
            : fManager(manager)
        {
            if (fManager)
                fManager->allowPurging = true;
        }
        ~ScopedPurgeCache()
        {
            if (fManager)
                fManager->allowPurging = false;
        }
    ...
    bool deleteHandle(SkDiscardableHandleId) override
    {
        return allowPurging;
    }
...
static bool push_font_data(const SkPicture& pic, SkStrikeServer* strikeServer, sk_sp<SkColorSpace> colorSpace, int writeFd)
{
    const SkIRect bounds = pic.cullRect().round();
    const SkSurfaceProps props(0, kRGB_H_SkPixelGeometry);
    std::unique_ptr<SkCanvas> filter = strikeServer->makeAnalysisCanvas(bounds.width(), bounds.height(), props, std::move(colorSpace), true, true);
...
static void final_draw(std::string outFilename, SkData* picData, SkStrikeClient* client, ClientDiscardableManager* discardableManager, int readFd, int writeFd)
{
    ...
    std::cout << "useProcess: " << gUseProcess << " useGPU: " << gUseGpu << " purgeCache: " << gPurgeFontCaches << std::endl;
...
static void gpu(int readFd, int writeFd)
{
    ...
    if (gUseGpu) {
        ...
        final_draw("test.png", picData.get(), &strikeClient, discardableManager.get(), readFd, writeFd);
    }
...
static int renderer(const std::string& skpName, int readFd, int writeFd)
{
    ...
    if (gUseGpu) {
        ...
        SkSerialProcs procs;
        auto encode = [](SkTypeface* tf, void* ctx) -> sk_sp<SkData> { return reinterpret_cast<SkStrikeServer*>(ctx)->serializeTypeface(tf); };
...
int main(int argc, char** argv)
{
    ...
    int render_to_gpu[2], gpu_to_render[2];
