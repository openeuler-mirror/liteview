...
sk_sp<SkImage> DDLPromiseImageHelper::CreatePromiseImages(const void* rawData, size_t length, void* ctxIn)
{
    ...
    if (curImage.isYUV()) {
        ...
        GrYUVABackendTextureInfo yuvaBackendTextures(yuvaInfo, backendFormats, GrMipmapped::kNo, kTopLeft_GrSurfaceOrigin);
        image = SkImage::MakePromiseYUVATexture(procContext->fThreadSafeProxy, yuvaBackendTextures, curImage.refOverallColorSpace(),
        ...
        image = SkImage::MakePromiseTexture(procContext->fThreadSafeProxy, backendFormat, curImage.overallDimensions(), curImage.mipmapped(0),
...
int DDLPromiseImageHelper::addImage(SkImage* image)
{
    ...
    PromiseImageInfo& newImageInfo = fImageInfo.emplace_back(fImageInfo.count(), image->uniqueID(), overallII);
