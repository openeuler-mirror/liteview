...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(sk_sp<SkData> data, GrMipmapped mipmapped, sk_sp<SkColorSpace> cs)
{
...
std::unique_ptr<LazyYUVImage> LazyYUVImage::Make(SkYUVAPixmaps pixmaps, GrMipmapped mipmapped, sk_sp<SkColorSpace> cs)
{
    std::unique_ptr<LazyYUVImage> image(new LazyYUVImage());
...
bool LazyYUVImage::ensureYUVImage(GrRecordingContext* rContext, Type type)
{
    ...
    case Type::kFromPixmaps:
        ...
        fYUVImage[idx] = SkImage::MakeFromYUVAPixmaps(rContext, fPixmaps, fMipmapped,
        ...
        if (auto direct = rContext->asDirectContext()) {
            ...
            for (int i = 0; i < fPixmaps.numPlanes(); ++i) {
                mbets[i] = sk_gpu_test::ManagedBackendTexture::MakeWithData(
                    direct, fPixmaps.plane(i), kTopLeft_GrSurfaceOrigin, GrRenderable::kNo, GrProtected::kNo);
                if (mbets[i]) {
            ...
            fYUVImage[idx] = SkImage::MakeFromYUVATextures(direct, yuvaTextures, fColorSpace, sk_gpu_test::ManagedBackendTexture::ReleaseProc, planeRelContext);
        }
    }
