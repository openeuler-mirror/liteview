...
static bool init_instance_extensions_and_layers(PFN_vkGetInstanceProcAddr getInstProc, uint32_t specVersion,
    SkTArray<VkExtensionProperties>* instanceExtensions, SkTArray<VkLayerProperties>* instanceLayers)
{
...
static bool init_device_extensions_and_layers(skgpu::VulkanGetProc getProc, uint32_t specVersion, VkInstance inst, VkPhysicalDevice physDev,
...
static bool setup_features(skgpu::VulkanGetProc getProc, VkInstance inst, VkPhysicalDevice physDev, uint32_t physDeviceVersion,
...
bool CreateVkBackendContext(PFN_vkGetInstanceProcAddr getInstProc, GrVkBackendContext* ctx, skgpu::VulkanExtensions* extensions,
    VkPhysicalDeviceFeatures2* features, VkDebugReportCallbackEXT* debugCallback, uint32_t* presentQueueIndexPtr, CanPresentFn canPresent, bool isProtected)
{
    skgpu::VulkanBackendContext skgpuCtx;
    if (!CreateVkBackendContext(getInstProc, &skgpuCtx, extensions, features, debugCallback, presentQueueIndexPtr, canPresent, isProtected)) {
        return false;
    }
    ...
    ctx->fProtectedContext = skgpuCtx.fProtectedContext == skgpu::Protected::kYes ? GrProtected::kYes : GrProtected::kNo;
    return true;
}
    ...
    VkPhysicalDeviceFeatures2* features, VkDebugReportCallbackEXT* debugCallback, uint32_t* presentQueueIndexPtr, CanPresentFn canPresent, bool isProtected)
{
    ...
    *debugCallback = VK_NULL_HANDLE;
    ...
    for (int i = 0; i < deviceExtensions.count(); ++i) {
        ...
        if (0 != strncmp(deviceExtensions[i].extensionName, "VK_KHX", 6) && 0 != strncmp(deviceExtensions[i].extensionName, "VK_NVX", 6)) {
            ...
            if (!hasKHRBufferDeviceAddress || 0 != strcmp(deviceExtensions[i].extensionName, "VK_EXT_buffer_device_address")) {
                deviceExtensionNames.push_back(deviceExtensions[i].extensionName);
            }
        }
    }

    extensions->init(getProc, inst, physDev, (uint32_t)instanceExtensionNames.count(), instanceExtensionNames.begin(), (uint32_t)deviceExtensionNames.count(),
