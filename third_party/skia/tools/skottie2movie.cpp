...
int main(int argc, char** argv)
{
    ...
    auto animation = skottie::Animation::Builder().setResourceProvider(skresources::FileResourceProvider::Make(assetPath)).makeFromFile(FLAGS_input[0]);
    if (!animation) {
    ...
    do {
        ...
        if (!surf) {
            if (FLAGS_gpu) {
                ...
                surf = SkSurface::MakeRenderTarget(grctx, SkBudgeted::kNo, info, 0, GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, nullptr);
                if (!surf) {
                    grctx = nullptr;
                }
            }
