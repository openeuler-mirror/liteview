...
static DEFINE_bool(releaseAndAbandonGpuContext, false, "Release all GPU resources and abandon the GrContext after drawing.");

static DEFINE_bool(decodeToDst, false, "Decode images to destination format rather than suggested natural format.");

static DEFINE_double(rasterDPI, SK_ScalarDefaultRasterDPI, "DPI for rasterized content in vector backends like --backend pdf.");
...
template <typename T> struct FlagOption {
    const char* label;
    T value;
};

template <typename T, int N> static bool parse_flag(const CommandLineFlags::StringArray& flag, const char* flag_name, const FlagOption<T> (&array)[N], T* value)
{
    ...
    source->draw = [animation](SkCanvas* canvas) {
        ...
        for (int y : order)
            for (int x : order) {
                ...
                canvas->concat(SkMatrix::RectToRect(SkRect::MakeSize(animation->size()), dst, SkMatrix::kCenter_ScaleToFit));
...
static sk_sp<SkImage> draw_with_cpu(std::function<bool(SkCanvas*)> draw, SkImageInfo info)
{
...
static sk_sp<SkData> draw_as_skp(std::function<bool(SkCanvas*)> draw, SkImageInfo info)
{
...
static sk_sp<SkData> draw_as_pdf(std::function<bool(SkCanvas*)> draw, SkImageInfo info, SkString name)
{
...
static sk_sp<SkImage> draw_with_gpu(std::function<bool(SkCanvas*)> draw, SkImageInfo info, GrContextFactory::ContextType api, GrContextFactory* factory)
{
    ...
    if (!FLAGS_stencils) {
        overrides |= GrContextFactory::ContextOverrides::kAvoidStencilBuffers;
    }

    auto context = factory->getContextInfo(api, overrides).directContext();

    uint32_t flags = FLAGS_dit ? SkSurfaceProps::kUseDeviceIndependentFonts_Flag : 0;
    ...
    case SurfaceType::kDefault:
        surface = SkSurface::MakeRenderTarget(context, SkBudgeted::kNo, info, FLAGS_samples, &props);
        break;

    case SurfaceType::kBackendTexture:
        surface = sk_gpu_test::MakeBackendTextureSurface(context, info, kTopLeft_GrSurfaceOrigin, FLAGS_samples, GrMipmapped::kNo, GrProtected::kNo, &props);
        break;

    case SurfaceType::kBackendRenderTarget:
        surface = sk_gpu_test::MakeBackendRenderTargetSurface(context, info, kBottomLeft_GrSurfaceOrigin, FLAGS_samples, GrProtected::kNo, &props);
        break;
    }
...
int main(int argc, char** argv)
{
    ...
    if (!parse_flag(FLAGS_backend, "backend", kBackends, &backend) || !parse_flag(FLAGS_ct, "ct", kColorTypes, &ct)
        || !parse_flag(FLAGS_at, "at", kAlphaTypes, &at) || !parse_flag(FLAGS_gamut, "gamut", kGamuts, &gamut)
        || !parse_flag(FLAGS_tf, "tf", kTransferFunctions, &tf)) {
        return 1;
    }

    sk_sp<SkColorSpace> cs = FLAGS_legacy ? nullptr : SkColorSpace::MakeRGB(tf, gamut);
    ...
    for (int i = 0; i < sources.count(); i += replicas)
        SkTaskGroup {}.batch(replicas, [=](int replica) {
            ...
            fprintf(stdout, "%50s  %s  %7dms\n", source.name.c_str(), md5.c_str(), (int)std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count());
            fflush(stdout);
        });

    if (!FLAGS_writeShaders.isEmpty()) {
        sk_mkdir(FLAGS_writeShaders[0]);
        GrBackendApi api = GrContextFactory::ContextTypeBackend((GrContextFactory::ContextType)backend);
