...
public:
    enum class ContextType { kDirect3D, kMetal, kVulkan, kMock, kLastContextType = kMock };
    ...
    public:
        ...
        ContextFactory::ContextType type() const
        {
            return fType;
        }
        ...
        GraphiteTestContext* testContext() const
        {
            return fTestContext.get();
        }

    private:
        friend class ContextFactory; // for ctor

        ContextInfo(ContextFactory::ContextType type, std::unique_ptr<GraphiteTestContext> testContext, std::unique_ptr<skgpu::graphite::Context> context);
