...
static std::optional<SkSL::LoadedModule> compile_module_list(SkSpan<const std::string> paths)
{
    ...
    for (auto modulePath = paths.rbegin(); modulePath != paths.rend(); ++modulePath) {
        ...
        const SkSL::BuiltinMap* base = modules.empty() ? SkSL::ModuleLoader::Get().rootModule() : modules.front().get();
        ...
        loadedModule = compiler.compileModule(SkSL::ProgramKind::kFragment, modulePath->c_str(), std::move(moduleSource), base,
        ...
        if (!gUnoptimized) {
            ...
            // renames.)
            compiler.optimizeModuleBeforeMinifying(SkSL::ProgramKind::kFragment, loadedModule, base);
        }
    }
...
bool find_boolean_flag(std::vector<std::string>& args, std::string_view flagName)
{
    ...
    args.erase(std::remove_if(args.begin(), args.end(), [&](const std::string& a) { return a == flagName; }), args.end());
