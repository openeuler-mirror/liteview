...
void* SkiaAndroidApp::pthread_main(void* arg)
{
    ...
    skiaAndroidApp->fApp = Application::Create(std::size(gCmdLine), const_cast<char**>(gCmdLine), skiaAndroidApp);
...
extern "C" // extern "C" is needed for JNI (although the method itself is in C++)
    ...
    Java_org_skia_viewer_ViewerApplication_createNativeApp(JNIEnv* env, jobject application, jobject assetManager)
{
