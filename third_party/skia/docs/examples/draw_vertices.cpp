...
REG_FIDDLE(draw_vertices, 256, 256, false, 6)
{
    ...
    void draw(SkCanvas * canvas)
    {
        ...
        canvas->drawVertices(SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, 3, pts, nullptr, colors), SkBlendMode::kSrcOver, p);

        canvas->translate(120, 0);

        p.setShader(image->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions()));
        SkPoint texs[3] = { { 0, 0 }, { 0, 128 }, { 64, 256 } };
        canvas->drawVertices(SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, 3, pts, texs, nullptr), SkBlendMode::kSrcOver, p);
    }
} // END FIDDLE
