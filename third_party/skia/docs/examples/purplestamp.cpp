...
REG_FIDDLE(purplestamp, 256, 256, false, 0)
{
    ...
    void drawStamp(SkCanvas * canvas, int size)
    {
        ...
        canvas->drawRRect(SkRRect::MakeOval(SkRect::MakeXYWH(size / 4, size / 4, size / 2, size / 2)), paint);
    ...
    {
        sk_sp<SkSurface> surface = SkSurface::MakeRaster(SkImageInfo::Make(size, size, colorType, kPremul_SkAlphaType));
