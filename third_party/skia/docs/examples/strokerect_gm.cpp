...
REG_FIDDLE(strokerect_gm, 1400, 740, false, 0)
{
    void draw(SkCanvas * canvas)
    {
        ...
        constexpr SkPaint::Join gJoins[] = { SkPaint::kMiter_Join, SkPaint::kRound_Join, SkPaint::kBevel_Join };
        ...
        for (int doFill = 0; doFill <= 1; ++doFill) {
            ...
            for (size_t i = 0; i < std::size(gJoins); ++i) {
                ...
                for (size_t j = 0; j < std::size(gRects); ++j) {
                    ...
                    canvas->translate(j * (W + 2 * kStrokeWidth), (i + doFill * std::size(gJoins)) * (H + 2 * kStrokeWidth));
