    ...
    void draw(SkCanvas * canvas)
    {
        ...
        for (int i = 0; i < mgr->countFamilies(); ++i) {
            ...
            for (int j = 0; j < styleSet->count(); ++j) {
                ...
                auto s = SkStringPrintf("SkFont font(mgr->legacyMakeTypeface(\"%s\", SkFontStyle(%3d, %1d, %-27s), "
                                        "%g);",
                    familyName.c_str(), fontStyle.weight(), fontStyle.width(), tostr(fontStyle.slant()), textScale);
