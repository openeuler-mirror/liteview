...
REG_FIDDLE(unexpected_setAlphaType, 256, 256, true, 0)
{
    ...
    {
        static const SkAlphaType kAlphaTypes[] = { kUnknown_SkAlphaType, kOpaque_SkAlphaType, kPremul_SkAlphaType, kUnpremul_SkAlphaType };
        ...
        for (SkColorType colorType : kColorTypes) {
            for (SkAlphaType canonicalAlphaType : kAlphaTypes) {
                ...
                for (SkAlphaType alphaType : kAlphaTypes) {
                    ...
                    SkDebugf("%s %s    ", result ? "true " : "false", alphatype_name(bitmap.alphaType()));
                }
                SkDebugf("\n");
            }
        }
