...
REG_FIDDLE(Image_MakeBackendTextureFromSkImage, 256, 64, false, 0)
{
    ...
    void draw(SkCanvas * canvas)
    {
        ...
        if (!SkImage::MakeBackendTextureFromSkImage(dContext, std::move(backEndImage), &texture, &proc)) {
            return;
        }
        sk_sp<SkImage> i2 = SkImage::MakeFromTexture(dContext, texture, kTopLeft_GrSurfaceOrigin, kN32_SkColorType, kOpaque_SkAlphaType, nullptr);
