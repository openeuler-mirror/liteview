...
{
    static void draw_centered(const char* s, const SkFont& font, SkColor color, SkPoint xy, SkCanvas* c)
    {
    ...
    void draw_wheel(SkCanvas * c)
    {
        ...
        const SkColor sweep_colors[] = { SK_ColorRED, SK_ColorYELLOW, SK_ColorGREEN, SK_ColorCYAN, SK_ColorBLUE, SK_ColorMAGENTA, SK_ColorRED };
        SkMatrix rot;
        rot.setRotate(90.0f);
        p.setShader(SkGradientShader::MakeSweep(0, 0, sweep_colors, nullptr, std::size(sweep_colors), 0, &rot));
    ...
    void draw(SkCanvas * canvas)
    {
        ...
        paint.setShader(rec.finishRecordingAsPicture()->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest, &matrix, nullptr));
