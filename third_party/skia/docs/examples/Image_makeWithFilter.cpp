...
REG_FIDDLE(Image_makeWithFilter, 256, 256, false, 5)
{
    ...
    {
        sk_sp<SkImageFilter> shadowFilter = SkImageFilters::DropShadow(-10.0f * frame, 5.0f * frame, 3.0f, 3.0f, SK_ColorBLUE, nullptr);
        ...
        SkIRect outSubset;
        SkIPoint offset;
        sk_sp<SkImage> filtered(image->makeWithFilter(canvas->recordingContext(), offsetFilter.get(), subset, clipBounds, &outSubset, &offset));
        SkPaint paint;
