...
REG_FIDDLE(Image_getBackendTexture, 256, 256, false, 3)
{
    void draw(SkCanvas * canvas)
    {
        ...
        sk_sp<SkImage> imageFromBackend
            = SkImage::MakeFromAdoptedTexture(direct, backEndTexture, kBottomLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
        GrBackendTexture textureFromImage = imageFromBackend->getBackendTexture(false);
        if (!textureFromImage.isValid()) {
            return;
        }
        sk_sp<SkImage> imageFromTexture
            = SkImage::MakeFromAdoptedTexture(direct, textureFromImage, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
