...
REG_FIDDLE(Pixmap_info, 256, 256, true, 3)
{
    void draw(SkCanvas * canvas)
    {
        ...
        SkPixmap pixmap(
            SkImageInfo::Make(image->width(), image->height(), kN32_SkColorType, image->alphaType()), (const void*)&pixels.front(), image->width() * 4);
        ...
        if (pixmap.extractSubset(&inset, { 128, 128, 512, 512 })) {
            ...
            SkDebugf("width: %d height: %d color: %s alpha: %s\n", info.width(), info.height(), colors[info.colorType()], alphas[info.alphaType()]);
        }
    }
} // END FIDDLE
