    ...
    void draw(SkCanvas * canvas)
    {
        auto debugster = [](const SkRect& test) -> void {
            ...
            SkDebugf("{%g, %g, %g, %g} %c= {%g, %g, %g, %g} %s numerically equal\n", test.fLeft, test.fTop, test.fRight, test.fBottom,
                test == negZero ? '=' : '!', negZero.fLeft, negZero.fTop, negZero.fRight, negZero.fBottom,
                (test.fLeft == negZero.fLeft && test.fTop == negZero.fTop && test.fRight == negZero.fRight && test.fBottom == negZero.fBottom) ? "and are"
                                                                                                                                               : "yet are not");
        };
        SkRect tests[] = { { 0, 0, 2, 2 }, { -0, -0, 2, 2 }, { 0.0f, 0.0f, 2, 2 } };
        SkDebugf("tests are %s"
                 "equal\n",
