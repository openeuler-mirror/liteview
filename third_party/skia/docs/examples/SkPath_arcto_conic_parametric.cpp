...
REG_FIDDLE(SkPath_arcto_conic_parametric, 512, 512, false, 0)
{
    ...
    {
        float s = 1 - t;
        return { ((s * s * p0.x()) + (2 * s * t * w * p1.x()) + (t * t * p2.x())) / ((s * s) + (w * 2 * s * t) + (t * t)),
            ((s * s * p0.y()) + (2 * s * t * w * p1.y()) + (t * t * p2.y())) / ((s * s) + (w * 2 * s * t) + (t * t)) };
    }
    ...
    void draw(SkCanvas * canvas)
    {
        ...
        SkPoint p0 = { r * SkScalarCos(SkDegreesToRadians(startAngle)), r * SkScalarSin(SkDegreesToRadians(startAngle)) };
        float d = r / weight;
        SkPoint p1 = { d * SkScalarCos(SkDegreesToRadians(middleAngle)), d * SkScalarSin(SkDegreesToRadians(middleAngle)) };
        SkPoint p2 = { r * SkScalarCos(SkDegreesToRadians(finalAngle)), r * SkScalarSin(SkDegreesToRadians(finalAngle)) };
        p0 += center;
        p1 += center;
        p2 += center;
