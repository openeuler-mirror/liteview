...
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VkYCbcrSampler_DrawImageWithYcbcrSampler, reporter, context_info, CtsEnforcement::kApiLevel_T)
{
    ...
    sk_sp<SkImage> srcImage = SkImage::MakeFromTexture(
        testHelper.directContext(), ycbcrHelper.backendTexture(), kTopLeft_GrSurfaceOrigin, kRGB_888x_SkColorType, kPremul_SkAlphaType, nullptr);
    if (!srcImage) {
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(
        testHelper.directContext(), SkBudgeted::kNo, SkImageInfo::Make(kImageWidth, kImageHeight, kN32_SkColorType, kPremul_SkAlphaType));
    if (!surface) {
    ...
    if (!surface->readPixels(
            SkImageInfo::Make(kImageWidth, kImageHeight, kRGBA_8888_SkColorType, kOpaque_SkAlphaType), readbackData.data(), kImageWidth * 4, 0, 0)) {
    ...
    for (size_t y = 0; y < kImageHeight; y += 2) {
        for (size_t x = 0; x < kImageWidth; x += 2) {
            ...
            int expectedG = round_and_clamp((yChannel - 0.13397432f / 0.7152f * uChannel - 0.33480248f / 0.7152f * vChannel) * 255.0);
...
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VkYCbcrSampler_NoYcbcrSurface, reporter, context_info, CtsEnforcement::kApiLevel_T)
{
    VkTestHelper testHelper(false);
    if (!testHelper.init()) {
        ERRORF(reporter, "VkTestHelper initialization failed.");
    ...
    GrBackendTexture texture = testHelper.directContext()->createBackendTexture(
        kImageWidth, kImageHeight, GrBackendFormat::MakeVk(VK_FORMAT_G8_B8R8_2PLANE_420_UNORM), GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo);
    if (texture.isValid()) {
        ERRORF(reporter, "GrDirectContext::createBackendTexture() didn't fail as expected for Ycbcr format.");
    }
}
