...
static bool check_rect(
    GrDirectContext* dContext, SurfaceDrawContext* sdc, const SkIRect& rect, uint32_t expectedValue, uint32_t* actualValue, int* failX, int* failY)
{
    int w = rect.width();
    int h = rect.height();
...
{
    return SurfaceDrawContext::Make(rContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { w, h }, SkSurfaceProps(),
        /*label=*/ {});
}
...
static void clear_op_test(skiatest::Reporter* reporter, GrDirectContext* dContext)
{
    ...
    if (!check_rect(dContext, sdc.get(), fullRect, kColor1, &actualValue, &failX, &failY)) {
        ERRORF(reporter, "Expected 0x%08x but got 0x%08x at (%d, %d).", kColor1, actualValue, failX, failY);
    }
    ...
    if (!check_rect(dContext, sdc.get(), fullRect, kColor2, &actualValue, &failX, &failY)) {
        ERRORF(reporter, "Expected 0x%08x but got 0x%08x at (%d, %d).", kColor2, actualValue, failX, failY);
    }
    ...
    if (!dContext->priv().caps()->performColorClearsAsDraws() && !dContext->priv().caps()->performStencilClearsAsDraws()
