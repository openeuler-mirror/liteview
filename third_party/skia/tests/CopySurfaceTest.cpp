...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(CopySurface, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    static const SkImageInfo kImageInfos[] { SkImageInfo::Make(kW, kH, kRGBA_8888_SkColorType, kPremul_SkAlphaType),
                    ...
                    for (const SkIRect& srcRect : kSrcRects) {
                        for (const SkIPoint& dstPoint : kDstPoints) {
                            for (const SkImageInfo& ii : kImageInfos) {
                                ...
                                auto srcView = sk_gpu_test::MakeTextureProxyViewFromData(dContext, sRenderable, sOrigin, srcPM);
                                auto dstView = sk_gpu_test::MakeTextureProxyViewFromData(dContext, dRenderable, dOrigin, dstPM);
                                ...
                                if (ii.colorType() == kRGBA_8888_SkColorType) {
                                    ...
                                    if (!dContext->defaultBackendFormat(kBGRA_8888_SkColorType, GrRenderable::kNo).isValid()) {
                                        continue;
                                    }
                                    if (!srcView || !dstView) {
                                        ERRORF(reporter, "Could not create surfaces for copy surface test.");
                                ...
                                if (sOrigin == dOrigin) {
                                    ...
                                    result = dstContext->asFillContext()->blitTexture(std::move(srcView), srcRect, dstPoint);
                                }

                                bool expectedResult = true;
                                SkIPoint dstOffset = { dstPoint.fX - srcRect.fLeft, dstPoint.fY - srcRect.fTop };
                                SkIRect copiedDstRect = SkIRect::MakeXYWH(dstPoint.fX, dstPoint.fY, srcRect.width(), srcRect.height());
