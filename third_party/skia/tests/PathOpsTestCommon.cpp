...
static void addTs(const SkDCubic& cubic, double precision, double start, double end, SkTArray<double, true>* ts)
{
    double tDiv = calc_t_div(cubic, precision, 0);
    ...
    for (auto [verb, pts, w] : SkPathPriv::Iterate(cubicPath)) {
        ...
        case SkPathVerb::kCubic: {
            ...
            for (int index = 0; index <= inflections; ++index) {
                ...
                simplePath->cubicTo(cPts[0].fX, cPts[0].fY, cPts[1].fX, cPts[1].fY, cPts[2].fX, cPts[2].fY);
                lo = hi;
            }
            break;
        }
