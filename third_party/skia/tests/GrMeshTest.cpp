...
static void run_test(GrDirectContext*, const char* testName, skiatest::Reporter*, const std::unique_ptr<skgpu::v1::SurfaceDrawContext>&, const SkBitmap& gold,
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrMeshTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(
        dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { kImageWidth, kImageHeight }, SkSurfaceProps(), /*label=*/ {});
    if (!sdc) {
    ...
    for (int y = 0; y < kBoxCountY; ++y) {
        for (int x = 0; x < kBoxCountX; ++x) {
            ...
            const Box box = boxes.push_back() = { float(x * kBoxSize), float(y * kBoxSize), GrColorPackRGBA(rgb[0], rgb[1], rgb[2], 255) };
    ...
    run_test(
        ...
        [&](DrawMeshHelper* helper) {
            ...
            while (i < kBoxCount) {
                ...
                pass->drawIndexed(
                    repetitionCount * 6, baseRepetition * 6, baseRepetition * 4, (baseRepetition + repetitionCount) * 4 - 1, (i - baseRepetition) * 4);
    ...
    for (bool indexed : { false, true }) {
        ...
        run_test(
            ...
            [&](DrawMeshHelper* helper) {
                ...
                helper->fIndexBuffer2 = helper->makeIndexBuffer(baseIndexData.begin(), baseIndexData.count());
                helper->fInstBuffer = helper->makeVertexBuffer(boxes);
                VALIDATE(helper->fInstBuffer);
                helper->fVertBuffer = helper->makeVertexBuffer(std::vector<float> { 0, 0, 0, 1, 1, 0, 1, 1 });
                VALIDATE(helper->fVertBuffer);
            ...
            [&](DrawMeshHelper* helper) {
                ...
                for (int y = 0; y < kBoxCountY; ++y) {
                    ...
                    GrPrimitiveType primitiveType = indexed ? GrPrimitiveType::kTriangles : GrPrimitiveType::kTriangleStrip;
                    auto pass = helper->bindPipeline(primitiveType, true, SkToBool(vertexBuffer));
                    if (indexed) {
                        sk_sp<const GrBuffer> indexBuffer = (y % 2) ? helper->fIndexBuffer2 : helper->fIndexBuffer;
                        VALIDATE(indexBuffer);
                        int baseIndex = (y % 2);
                        pass->bindBuffers(std::move(indexBuffer), helper->fInstBuffer, std::move(vertexBuffer));
    ...
    for (bool indexed : { false, true }) {
        ...
        run_test(
            ...
            [&](DrawMeshHelper* helper) {
                ...
                helper->fInstBuffer = helper->makeVertexBuffer(boxes);
                VALIDATE(helper->fInstBuffer);
                helper->fVertBuffer = helper->makeVertexBuffer(std::vector<float> { -1, -1, 0, 0, 0, 1, 1, 0, 1, 1, -1, -1, 0, 0, 1, 0, 0, 1, 1, 1 });
                VALIDATE(helper->fVertBuffer);
                ...
                if (indexed) {
                    ...
                    indexedIndirectWriter
                        = helper->target()->makeDrawIndexedIndirectSpace(kBoxCountY, &helper->fDrawIndirectBuffer, &helper->fDrawIndirectBufferOffset);
                } else {
                    // Make helper->fDrawIndirectBufferOffset nonzero.
                    sk_sp<const GrBuffer> ignoredBuff;
                    size_t ignoredOffset;
                    ...
                    indirectWriter = helper->target()->makeDrawIndirectSpace(kBoxCountY, &helper->fDrawIndirectBuffer, &helper->fDrawIndirectBufferOffset);
                }
                ...
                for (int y = 0; y < kBoxCountY; ++y) {
                    ...
                    if (indexed) {
                        int baseIndex = 1 + y * 6;
                        indexedIndirectWriter.writeIndexed(6, baseIndex, kBoxCountX, y * kBoxCountX, baseVertex);
                    } else {
            ...
            [&](DrawMeshHelper* helper) {
                ...
                if (indexed) {
                    ...
                    for (int i = 0; i < 3; ++i) {
                        ...
                        size_t offset = helper->fDrawIndirectBufferOffset + start * sizeof(GrDrawIndexedIndirectCommand);
                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        size_t offset = helper->fDrawIndirectBufferOffset + start * sizeof(GrDrawIndirectCommand);
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* rContext, std::function<void(DrawMeshHelper*)> prepareFn, std::function<void(DrawMeshHelper*)> executeFn)
    {
...
private:
    ...
    MeshTestOp(std::function<void(DrawMeshHelper*)> prepareFn, std::function<void(DrawMeshHelper*)> executeFn)
        : INHERITED(ClassID())
...
public:
    ...
    {
        return arena->make([&](void* ptr) { return new (ptr) MeshTestProcessor(instanced, hasVertexBuffer); });
    }
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> MeshTestProcessor::makeProgramImpl(const GrShaderCaps&) const
{
    class Impl : public ProgramImpl {
    public:
        void setData(const GrGLSLProgramDataManager&, const GrShaderCaps&, const GrGeometryProcessor&) final
        {
        }
...
{
    return fState->resourceProvider()->createBuffer(indices, count * sizeof(uint16_t), GrGpuBufferType::kIndex, kDynamic_GrAccessPattern);
}

template <typename T> sk_sp<const GrBuffer> DrawMeshHelper::makeVertexBuffer(const T* data, int count)
{
    return fState->resourceProvider()->createBuffer(data, count * sizeof(T), GrGpuBufferType::kVertex, kDynamic_GrAccessPattern);
}

sk_sp<const GrBuffer> DrawMeshHelper::getIndexBuffer()
{
    SKGPU_DEFINE_STATIC_UNIQUE_KEY(gIndexBufferKey);
    return fState->resourceProvider()->findOrCreatePatternedIndexBuffer(kIndexPattern, 6, kIndexPatternRepeatCount, 4, gIndexBufferKey);
}

GrOpsRenderPass* DrawMeshHelper::bindPipeline(GrPrimitiveType primitiveType, bool isInstanced, bool hasVertexBuffer)
{
    ...
    processorSet.finalize(
        GrProcessorAnalysisColor(), GrProcessorAnalysisCoverage::kNone, fState->appliedClip(), nullptr, fState->caps(), GrClampType::kAuto, &overrideColor);

    auto pipeline = GrSimpleMeshDrawOpHelper::CreatePipeline(fState, std::move(processorSet), GrPipeline::InputFlags::kNone);

    GrGeometryProcessor* mtp = MeshTestProcessor::Make(fState->allocator(), isInstanced, hasVertexBuffer);

    GrProgramInfo programInfo(fState->caps(), fState->writeView(), fState->usesMSAASurface(), pipeline, &GrUserStencilSettings::kUnused, mtp, primitiveType,
...
static void run_test(GrDirectContext* dContext, const char* testName, skiatest::Reporter* reporter, const std::unique_ptr<skgpu::v1::SurfaceDrawContext>& sdc,
    const SkBitmap& gold, std::function<void(DrawMeshHelper*)> prepareFn, std::function<void(DrawMeshHelper*)> executeFn)
{
...
#define TOSTRING(X) STRINGIFY(X)
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            if (expected != actual) {
                ERRORF(reporter, "[%s] pixel (%i,%i): got 0x%x expected 0x%x", testName, x, y, actual, expected);
                return;
            }
        }
    }
