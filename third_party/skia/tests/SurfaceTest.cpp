...
static sk_sp<SkSurface> create_direct_surface(SkAlphaType at = kPremul_SkAlphaType, SkImageInfo* requestedInfo = nullptr)
{
    ...
    return SkSurface::MakeRasterDirectReleaseProc(info, storage, rowBytes, release_direct_surface_storage, storage);
}
...
{
    const SkImageInfo info = SkImageInfo::Make(0, 0, kN32_SkColorType, kPremul_SkAlphaType);
    REPORTER_ASSERT(reporter, nullptr == SkSurface::MakeRenderTarget(ctxInfo.directContext(), SkBudgeted::kNo, info));
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsSurface, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
...

        {
            ...
            REPORTER_ASSERT(reporter, can == SkToBool(surf), "ct: %d, can: %d, surf: %d", colorType, can, SkToBool(surf));

            surf = sk_gpu_test::MakeBackendTextureSurface(context, { kSize, kSize }, kTopLeft_GrSurfaceOrigin,
        ...
        if (context->colorTypeSupportedAsSurface(colorType)) {
            ...
            surf = sk_gpu_test::MakeBackendTextureSurface(context, { kSize, kSize }, kTopLeft_GrSurfaceOrigin, kSampleCnt, colorType);
            REPORTER_ASSERT(reporter, can == SkToBool(surf), "colorTypeSupportedAsSurface:%d, surf:%d, ct:%d", can, SkToBool(surf), colorType);
            ...
            if (surf) {
                ...
                REPORTER_ASSERT(reporter, storedCnt == allowedCnt, "Should store an allowed sample count (%d vs %d)", allowedCnt, storedCnt);
            }
        }

        for (int sampleCnt : { 1, 2 }) {
            auto surf = sk_gpu_test::MakeBackendRenderTargetSurface(context, { 16, 16 }, kTopLeft_GrSurfaceOrigin, sampleCnt, colorType);
            bool can = context->colorTypeSupportedAsSurface(colorType) && context->maxSurfaceSampleCountForColorType(colorType) >= sampleCnt;
            if (!surf && can && colorType == kBGRA_8888_SkColorType && sampleCnt > 1 && context->backend() == GrBackendApi::kOpenGL) {
            ...
            REPORTER_ASSERT(reporter, can == SkToBool(surf), "ct: %d, sc: %d, can: %d, surf: %d", colorType, sampleCnt, can, SkToBool(surf));
            if (surf) {
                auto rtp = SkCanvasPriv::TopDeviceTargetProxy(surf->getCanvas());
                int storedCnt = rtp->numSamples();
                const GrBackendFormat& backendFormat = rtp->backendFormat();
                int allowedCnt = context->priv().caps()->getRenderTargetSampleCount(storedCnt, backendFormat);
                REPORTER_ASSERT(reporter, storedCnt == allowedCnt, "Should store an allowed sample count (%d vs %d)", allowedCnt, storedCnt);
            }
        }
    }
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrContext_maxSurfaceSamplesForColorType, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        auto info = SkImageInfo::Make(kSize, kSize, colorType, kOpaque_SkAlphaType, nullptr);
        auto surf = sk_gpu_test::MakeBackendTextureSurface(context, info, kTopLeft_GrSurfaceOrigin, maxSampleCnt);
        if (!surf) {
        ...
        REPORTER_ASSERT(reporter, sampleCnt == maxSampleCnt, "Exected: %d, actual: %d", maxSampleCnt, sampleCnt);
    }
}
...
static void test_backend_rendertarget_access_copy_on_write(skiatest::Reporter* reporter, SkSurface* surface, SkSurface::BackendHandleAccess access)
{
    ...
    // If the access mode triggers CoW, then the backend objects should reflect it.
    REPORTER_ASSERT(reporter, GrBackendRenderTarget::TestingOnly_Equals(rt1, rt2) == (snap1 == snap2));
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SurfaceBackendHandleAccessIDs_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    for (auto& surface_func : { &create_gpu_surface, &create_gpu_scratch_surface }) {
        {
            auto surface(surface_func(ctxInfo.directContext(), kPremul_SkAlphaType, nullptr));
            test_backend_unique_id<GrBackendTexture, &SkSurface::getBackendTexture>(reporter, surface.get());
        }
        {
            auto surface(surface_func(ctxInfo.directContext(), kPremul_SkAlphaType, nullptr));
            test_backend_unique_id<GrBackendRenderTarget, &SkSurface::getBackendRenderTarget>(reporter, surface.get());
        }
    }
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SurfaceBackendAccessAbandoned_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    GrBackendRenderTarget beRT = surface->getBackendRenderTarget(SkSurface::kFlushRead_BackendHandleAccess);
...
static void test_copy_on_write(skiatest::Reporter* reporter, SkSurface* surface)
{
    ...
    const SkRect testRect = SkRect::MakeXYWH(SkIntToScalar(0), SkIntToScalar(0), SkIntToScalar(4), SkIntToScalar(5));
    SkPath testPath;
    testPath.addRect(SkRect::MakeXYWH(SkIntToScalar(0), SkIntToScalar(0), SkIntToScalar(2), SkIntToScalar(1)));
    ...
    const SkPoint testPoints[3] = { { SkIntToScalar(0), SkIntToScalar(0) }, { SkIntToScalar(2), SkIntToScalar(1) }, { SkIntToScalar(0), SkIntToScalar(2) } };
...
#define EXPECT_COPY_ON_WRITE(command)                                                                                                                          \
    ...
    EXPECT_COPY_ON_WRITE(drawPoints(SkCanvas::kPoints_PointMode, testPointCount, testPoints, testPaint))
...
static sk_sp<SkSurface> create_gpu_surface_backend_texture(GrDirectContext* dContext, int sampleCnt, const SkColor4f& color)
{
    ...
    auto surf = sk_gpu_test::MakeBackendTextureSurface(dContext, kSize, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType);
    if (!surf) {
        return nullptr;
    }
...
static sk_sp<SkSurface> create_gpu_surface_backend_render_target(GrDirectContext* dContext, int sampleCnt, const SkColor4f& color)
{
    ...
    auto surf = sk_gpu_test::MakeBackendRenderTargetSurface(dContext, { kWidth, kHeight }, kTopLeft_GrSurfaceOrigin, sampleCnt, kRGBA_8888_SkColorType);
    if (!surf) {
        return nullptr;
    }
    surf->getCanvas()->clear(color);
...
static void test_surface_context_clear(
    skiatest::Reporter* reporter, GrDirectContext* dContext, skgpu::v1::SurfaceContext* surfaceContext, uint32_t expectedValue)
{
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            if (pixel != expectedValue) {
                ...
                ERRORF(reporter, "%s but read 0x%08x (instead of 0x%08x) at %x,%d", msg.c_str(), pixel, expectedValue, x, y);
                return;
            }
        }
    }
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(SurfaceClear_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    auto dContext = ctxInfo.directContext();
    ...
    for (auto& surfaceFunc : { &create_gpu_surface_backend_texture, &create_gpu_surface_backend_render_target }) {
...
static void test_surface_draw_partially(skiatest::Reporter* reporter, sk_sp<SkSurface> surface, SkColor origColor)
{
    ...
    REPORTER_ASSERT(reporter, surface->readPixels(readback.info(), readback.writable_addr(), readback.rowBytes(), 0, 0));
    bool stop = false;

    SkPMColor origColorPM = SkPackARGB_as_RGBA(SkColorGetA(origColor), SkColorGetR(origColor), SkColorGetG(origColor), SkColorGetB(origColor));
    SkPMColor rectColorPM = SkPackARGB_as_RGBA(SkColorGetA(kRectColor), SkColorGetR(kRectColor), SkColorGetG(kRectColor), SkColorGetB(kRectColor));
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SurfaceWrappedWithRelease_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (bool useTexture : { false, true }) {
        ...
        if (useTexture) {
            SkImageInfo ii = SkImageInfo::Make(kWidth, kHeight, SkColorType::kRGBA_8888_SkColorType, kPremul_SkAlphaType);
            mbet = sk_gpu_test::ManagedBackendTexture::MakeFromInfo(ctx, ii, GrMipmapped::kNo, GrRenderable::kYes);
            if (!mbet) {
                continue;
            }

            surface = SkSurface::MakeFromBackendTexture(ctx, mbet->texture(), texOrigin,
                /*sample count*/ 1, kRGBA_8888_SkColorType,
                /*color space*/ nullptr,
                /*surface props*/ nullptr, sk_gpu_test::ManagedBackendTexture::ReleaseProc, mbet->releaseContext(ReleaseChecker::Release, &releaseChecker));
        } else {
            backendRT = gpu->createTestingOnlyBackendRenderTarget({ kWidth, kHeight }, GrColorType::kRGBA_8888);
            if (!backendRT.isValid()) {
                continue;
            }
            surface = SkSurface::MakeFromBackendRenderTarget(
                ctx, backendRT, texOrigin, kRGBA_8888_SkColorType, nullptr, nullptr, ReleaseChecker::Release, &releaseChecker);
        }
        if (!surface) {
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(SurfaceAttachStencil_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    auto context = ctxInfo.directContext();
    ...
    for (auto& surfaceFunc : { &create_gpu_surface_backend_texture, &create_gpu_surface_backend_render_target }) {
        for (int sampleCnt : { 1, 4, 8 }) {
            ...
            REPORTER_ASSERT(reporter, resourceProvider->attachStencilAttachment(rt, rt->numSamples() > 1));
        }
    }
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ReplaceSurfaceBackendTexture, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int sampleCnt : { 1, 2 }) {
        ...
        auto mbet1 = sk_gpu_test::ManagedBackendTexture::MakeFromInfo(context, ii, GrMipmapped::kNo, GrRenderable::kYes);
        if (!mbet1) {
            continue;
        }
        auto mbet2 = sk_gpu_test::ManagedBackendTexture::MakeFromInfo(context, ii, GrMipmapped::kNo, GrRenderable::kYes);
        if (!mbet2) {
            ERRORF(reporter, "Expected to be able to make second texture");
            continue;
        }
        auto ii2 = ii.makeWH(8, 8);
        auto mbet3 = sk_gpu_test::ManagedBackendTexture::MakeFromInfo(context, ii2, GrMipmapped::kNo, GrRenderable::kYes);
        ...
        for (int y = 0; y < ii.height() && !bad; ++y) {
            for (int x = 0; x < ii.width() && !bad; ++x) {
                ...
                if (found != expected) {
                    bad = true;
                    ERRORF(reporter, "Expected color 0x%08x, found color 0x%08x at %d, %d.", expected, found, x, y);
                }
            }
        }
        ...
        REPORTER_ASSERT(reporter, !surf->replaceBackendTexture(mbet1->texture(), kTopLeft_GrSurfaceOrigin));
        ...
        REPORTER_ASSERT(reporter, !surf->replaceBackendTexture(mbet3->texture(), kTopLeft_GrSurfaceOrigin));
    ...
    auto do_test = [reporter](const SkImageInfo& info) {
        ...
        if (surf) {
            ...
            if ((false)) { // change to true to document the differences
                if (!img) {
                    SkDebugf("image failed: [%08X %08X] %14s %s\n", info.width(), info.height(), ToolUtils::colortype_name(info.colorType()),
...
DEF_TEST(VMBlitterInfiniteColorShader, r)
{
    ...
    SkColor4f filtered = paint.getColorFilter()->filterColor4f(paint.getColor4f(), nullptr, nullptr);
