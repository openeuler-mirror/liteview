...
static inline bool double_is_NaN(double x)
{
    return x != x;
}
...
void SkIntersections::dump() const
{
    ...
    for (int index = 0; index < fUsed; ++index) {
        SkDebugf(" t=(%s%1.9g,%s%1.9g) pt=(%1.9g,%1.9g)", fIsCoincident[0] & (1 << index) ? "*" : "", fT[0][index], fIsCoincident[1] & (1 << index) ? "*" : "",
            fT[1][index], fPt[index].fX, fPt[index].fY);
...
void SkPathOpsDebug::CoinDict::dump(const char* str, bool visitCheck) const
{
    ...
    for (int index = 0; index < count; ++index) {
        ...
        if (visitCheck || entry.fGlitchType == kUninitialized_Glitch) {
            SkDebugf("%s %s : line %d iteration %d", str, entry.fFunctionName, entry.fLineNumber, entry.fIteration);
...
void SkOpAngle::dumpOne(bool functionHeader) const
{
    ...
    SkDebugf(" s=%1.9g [%d] e=%1.9g [%d]", fStart->t(), fStart->debugID(), fEnd->t(), fEnd->debugID());
...
void SkOpAngle::dumpTest() const
{
    ...
    do {
        ...
        segment->dumpPts();
        SkDebugf(", %d, %1.9g, %1.9g, {} },\n", SkPathOpsVerbToPoints(segment->verb()) + 1, next->start()->t(), next->end()->t());
        next = next->fNext;
    } while (next && next != first);
}
...
{
    SkDebugf("seg=%d span=%d ptT=%d", this->segment()->debugID(), this->span()->debugID(), this->debugID());
    this->dumpBase();
    SkDebugf("\n");
}
...
void SkOpPtT::dumpAll() const
{
    ...
    do {
        ...
        SkDebugf("seg=%d span=%d ptT=%d", next->segment()->debugID(), next->span()->debugID(), next->debugID());
        next->dumpBase();
        SkDebugf("\n");
        if (limit && ++loop >= limit) {
            SkDebugf("*** abort loop ***\n");
...
{
    SkDebugf(" t=%1.9g pt=(%1.9g,%1.9g)%s%s%s", this->fT, this->fPt.fX, this->fPt.fY, this->fCoincident ? " coin" : "", this->fDuplicatePt ? " dup" : "",
...
{
    SkDebugf("- seg=%d span=%d ptT=%d ", fCoinPtTStart->segment()->debugID(), fCoinPtTStart->span()->debugID(), fCoinPtTStart->debugID());
    ...
    SkDebugf("\n");
    SkDebugf("+ seg=%d span=%d ptT=%d ", fOppPtTStart->segment()->debugID(), fOppPtTStart->span()->debugID(), fOppPtTStart->debugID());
