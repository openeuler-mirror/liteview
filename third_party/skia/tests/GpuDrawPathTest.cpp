...
DEF_GANESH_TEST_FOR_ALL_GL_CONTEXTS(GpuDrawPath, reporter, ctxInfo, CtsEnforcement::kNever)
{
    for (auto& test_func : { &test_drawPathEmpty, &test_drawSameRectOvals }) {
        for (auto& sampleCount : { 1, 4, 16 }) {
            ...
            auto surface(SkSurface::MakeRenderTarget(ctxInfo.directContext(), SkBudgeted::kNo, info, sampleCount, nullptr));
            if (!surface) {
                continue;
            }
...
DEF_GANESH_TEST_FOR_ALL_CONTEXTS(GrDrawCollapsedPath, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    SkMatrix m;
    m.setAll(0.966006875f, -0.125156224f, 72.0899811f, -0.00885376986f, -0.112347461f, 64.7121124f, -8.94321693e-06f, -0.00173384184f, 0.998692870f);
