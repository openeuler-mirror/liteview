    ...
    for (int cy = 0; cy < DEV_H; ++cy) {
        for (int cx = 0; cx < DEV_W; ++cx) {
            ...
            if (writeRect.contains(cx, cy)) {
                int bx = cx - writeX;
                int by = cy - writeY;
                uint32_t bmpColor8888 = get_bitmap_color(bx, by, bitmap.width(), bmInfo.colorType(), bmInfo.alphaType());
                bool mul = (kUnpremul_SkAlphaType == bmInfo.alphaType());
                SkPMColor bmpPMColor = convert_to_PMColor(bmInfo.colorType(), bmInfo.alphaType(), bmpColor8888);
    ...
    for (size_t r = 0; r < std::size(testRects); ++r) {
        ...
        for (int tightBmp = 0; tightBmp < 2; ++tightBmp) {
            for (size_t c = 0; c < std::size(gSrcConfigs); ++c) {
                ...
                REPORTER_ASSERT(reporter, check_write(reporter, surface, surfaceInfo.alphaType(), bmp, rect.fLeft, rect.fTop));
                ...
                bool expectSuccess = SkIRect::Intersects(canvasRect, writeRect) && write_should_succeed(surfaceInfo, bmp.info(), isGPU);
...
DEF_TEST(WritePixels, reporter)
{
    ...
    for (auto& tightRowBytes : { true, false }) {
        ...
        auto surface(SkSurface::MakeRasterDirectReleaseProc(info, pixels, rowBytes, free_pixels, nullptr));
        test_write_pixels(reporter, surface.get(), info);
    }
}

static void test_write_pixels(skiatest::Reporter* reporter, GrRecordingContext* rContext, int sampleCnt)
{
    const SkImageInfo ii = SkImageInfo::MakeN32Premul(DEV_W, DEV_H);
    for (auto& origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
        sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(rContext, SkBudgeted::kNo, ii, sampleCnt, origin, nullptr));
        if (surface) {
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(WritePixelsMSAA_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    test_write_pixels(reporter, ctxInfo.directContext(), 1);
}
...
static void test_write_pixels_non_texture(skiatest::Reporter* reporter, GrDirectContext* dContext, int sampleCnt)
{
    ...
    for (auto& origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
        ...
        auto surface = sk_gpu_test::MakeBackendRenderTargetSurface(dContext, { DEV_W, DEV_H }, origin, sampleCnt, colorType);
        if (surface) {
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(WritePixelsNonTexture_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    test_write_pixels_non_texture(reporter, ctxInfo.directContext(), 1);
}

DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(WritePixelsNonTextureMSAA_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
...
// skbug.com/11834
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(WritePixelsPendingIO, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
...

    {
        ...
        sk_sp<GrTextureProxy> temp = proxyProvider->createProxy(
            format, kDims, GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kApprox, SkBudgeted::kYes, GrProtected::kNo, /*label=*/"WritePixelsTest");
