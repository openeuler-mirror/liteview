...
DEF_TEST(SkVx, r)
{
    ...
    REPORTER_ASSERT(r, all(if_then_else(float4 { 1, 2, 3, 2 } <= float4 { 2, 2, 2, 2 }, float4(42), float4(47)) == float4 { 42, 42, 47, 42 }));
    ...
    REPORTER_ASSERT(r, buf[0] == 2 && buf[1] == 3 && buf[2] == 4 && buf[3] == 5 && buf[4] == 5 && buf[5] == 6);
...
DEF_TEST(SkVx_xyzw, r)
{
    ...
    REPORTER_ASSERT(r, all(naive_if_then_else(int2(0, ~0), shuffle<3, 2>(float4(0, 1, 2, 3)), float4(4, 5, 6, 7).xy()) == float2(4, 2)));
    REPORTER_ASSERT(r, all(if_then_else(int2(0, ~0), shuffle<3, 2>(float4(0, 1, 2, 3)), float4(4, 5, 6, 7).xy()) == float2(4, 2)));
    REPORTER_ASSERT(r, all(naive_if_then_else(int2(0, ~0).xyxy(), float4(0, 1, 2, 3).zwxy(), float4(4, 5, 6, 7)) == float4(4, 3, 6, 1)));
    REPORTER_ASSERT(r, all(if_then_else(int2(0, ~0).xyxy(), float4(0, 1, 2, 3).zwxy(), float4(4, 5, 6, 7)) == float4(4, 3, 6, 1)));

    REPORTER_ASSERT(r, all(pin(float4(0, 1, 2, 3).yxwz(), float2(1).xyxy(), float2(2).xyxy()) == float4(1, 1, 2, 2)));
}
...
DEF_TEST(SkVx_cross_dot, r)
{
    ...
    SkRandom rand;
    for (int i = 0; i < 100; ++i) {
        float a = rand.nextRangeF(-1, 1), b = rand.nextRangeF(-1, 1), c = rand.nextRangeF(-1, 1), d = rand.nextRangeF(-1, 1);
        constexpr static float kTolerance = 1.f / (1 << 20);
        REPORTER_ASSERT(r, SkScalarNearlyEqual(cross({ a, b }, { c, d }), SkPoint::CrossProduct({ a, b }, { c, d }), kTolerance));
        REPORTER_ASSERT(r, SkScalarNearlyEqual(dot(float2 { a, b }, float2 { c, d }), SkPoint::DotProduct({ a, b }, { c, d }), kTolerance));
    }
}
...
DEF_TEST(SkVx_ScaledDividerU32, r)
{
    ...
    auto errorBounds = [&](uint32_t actual, uint32_t expected) {
        uint32_t lowerLimit = expected == 0 ? 0 : expected - 1, upperLimit = expected == kMax ? kMax : expected + 1;
