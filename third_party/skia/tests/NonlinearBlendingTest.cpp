...
DEF_TEST(SkColorSpaceXformSteps_vs_skcms, r)
{
    ...
    for (auto color : colors) {
        ...
        float via_skcms[4];
        skcms_Transform(&color, bgra, unpremul, &srgb_profile, &via_skcms, f32, premul, &dp3_profile, 1);
        ...
        skcms_Transform(via_steps, f32, premul, &dp3_profile, steps_to_skcms, f32, premul, &srgb_profile, 1);

        float skcms_to_steps[4] = { via_skcms[0], via_skcms[1], via_skcms[2], via_skcms[3] };
        SkColorSpaceXformSteps inv_steps(dp3.get(), kPremul_SkAlphaType, srgb.get(), kPremul_SkAlphaType);
