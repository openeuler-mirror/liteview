...
static void test_find_existing(skiatest::Reporter* reporter, const sk_sp<SkSpecialImage>& image, const sk_sp<SkSpecialImage>& subset)
{
    ...
    REPORTER_ASSERT(reporter, SkIRect::MakeXYWH(offset.fX, offset.fY, image->width(), image->height()) == SkIRect(foundImage.layerBounds()));
...
static void test_dont_find_if_diff_key(skiatest::Reporter* reporter, const sk_sp<SkSpecialImage>& image, const sk_sp<SkSpecialImage>& subset)
{
    ...
    SkImageFilterCacheKey key2(0, SkMatrix::Translate(5, 5), clip1, image->uniqueID(), image->subset());
...
static void test_image_backed(skiatest::Reporter* reporter, GrRecordingContext* rContext, const sk_sp<SkImage>& srcImage)
{
    ...
    sk_sp<SkSpecialImage> fullImg(SkSpecialImage::MakeFromImage(rContext, full, srcImage, SkSurfaceProps()));

    const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

    sk_sp<SkSpecialImage> subsetImg(SkSpecialImage::MakeFromImage(rContext, subset, srcImage, SkSurfaceProps()));
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ImageFilterCache_ImageBackedGPU, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    sk_sp<SkImage> srcImage(SkImage::MakeFromTexture(dContext, backendTex, texOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr, nullptr, nullptr));
    if (!srcImage) {
        return;
    }
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ImageFilterCache_GPUBacked, reporter, ctxInfo, CtsEnforcement::kNever)
{
    auto dContext = ctxInfo.directContext();
    ...
    sk_sp<SkSpecialImage> fullImg(SkSpecialImage::MakeDeferredFromGpu(
        dContext, full, kNeedNewImageUniqueID_SpecialImage, srcView, { GrColorType::kRGBA_8888, kPremul_SkAlphaType, nullptr }, SkSurfaceProps()));

    const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

    sk_sp<SkSpecialImage> subsetImg(SkSpecialImage::MakeDeferredFromGpu(
        dContext, subset, kNeedNewImageUniqueID_SpecialImage, std::move(srcView), { GrColorType::kRGBA_8888, kPremul_SkAlphaType, nullptr }, SkSurfaceProps()));
