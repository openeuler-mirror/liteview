...
namespace {
SkString to_string(bool boolean)
{
    return boolean ? SkString("true") : SkString("false");
}
...
template <typename T> void reporter_assert_equals(skiatest::Reporter* r, const char* name, int i, const char* prop, T expected, T actual)
{
...
DEF_TEST(Codec_frames, r)
{
    ...
    constexpr SkCodecAnimation::DisposalMethod kRestoreBG = SkCodecAnimation::DisposalMethod::kRestoreBGColor;
    constexpr SkCodecAnimation::DisposalMethod kRestorePrev = SkCodecAnimation::DisposalMethod::kRestorePrevious;
    ...
    static const struct {
        ...
        = { {
                ...
                { { 0, 0, 100, 100 }, { 0, 0, 75, 75 }, { 0, 0, 50, 50 }, { 0, 0, 60, 60 }, { 0, 0, 100, 100 }, { 0, 0, 50, 50 }, { 0, 0, 75, 75 } },
            },
              {
                  "images/alphabetAnim.gif",
                  13,
                  { kNoFrame, 0, 0, 0, 0, 5, 6, kNoFrame, kNoFrame, 9, 10, 11 },
                  { kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul, kUnpremul },
                  { 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100 },
                  0,
                  { kKeep, kRestorePrev, kRestorePrev, kRestorePrev, kRestorePrev, kRestoreBG, kKeep, kRestoreBG, kRestoreBG, kKeep, kKeep, kRestoreBG, kKeep },
                  { true, false, true, false, true, true, true, true, true, true, true, true, true },
                  { kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver },
                  { { 25, 25, 75, 75 }, { 25, 25, 75, 75 }, { 25, 25, 75, 75 }, { 37, 37, 62, 62 }, { 37, 37, 62, 62 }, { 25, 25, 75, 75 }, { 0, 0, 50, 50 },
                      { 0, 0, 100, 100 }, { 25, 25, 75, 75 }, { 25, 25, 75, 75 }, { 0, 0, 100, 100 }, { 25, 25, 75, 75 }, { 37, 37, 62, 62 } },
              },
              {
              ...
              },
              {
                  ...
                  { kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver, kSrcOver },
                  { { 4, 4, 12, 12 }, { 4, 4, 12, 12 }, { 4, 4, 12, 12 }, { 0, 0, 8, 8 }, { 8, 8, 16, 16 }, { 8, 8, 16, 16 }, { 8, 8, 16, 16 },
                      { 2, 2, 10, 10 }, { 7, 7, 15, 15 }, { 7, 7, 15, 15 }, { 7, 7, 15, 15 }, { 0, 0, 8, 8 }, { 14, 14, 16, 16 } },
              },
              ...
              },
              {
                  ...
                  { { 0, 0, 200, 200 }, { 0, 0, 200, 200 }, { 0, 0, 200, 200 }, { 0, 0, 200, 200 }, { 0, 0, 200, 200 }, { 100, 100, 200, 200 },
                      { 100, 100, 200, 200 } },
              },
              {
    ...
    for (const auto& rec : gRecs) {
        ...
        if (repetitionCount != rec.fRepetitionCount) {
            ERRORF(r, "%s repetition count does not match! expected: %i\tactual: %i", rec.fName, rec.fRepetitionCount, repetitionCount);
        }

        const int expected = rec.fFrameCount;
        if (rec.fRequiredFrames.size() + 1 != static_cast<size_t>(expected)) {
            ERRORF(r, "'%s' has wrong number entries in fRequiredFrames; expected: %i\tactual: %zu", rec.fName, expected - 1, rec.fRequiredFrames.size());
            continue;
        }

        if (expected > 1) {
            if (rec.fDurations.size() != static_cast<size_t>(expected)) {
                ERRORF(r, "'%s' has wrong number entries in fDurations; expected: %i\tactual: %zu", rec.fName, expected, rec.fDurations.size());
                continue;
            }

            if (rec.fAlphas.size() + 1 != static_cast<size_t>(expected)) {
                ERRORF(r, "'%s' has wrong number entries in fAlphas; expected: %i\tactual: %zu", rec.fName, expected - 1, rec.fAlphas.size());
                continue;
            }
        ...
        for (auto mode : { TestMode::kVector, TestMode::kIndividual }) {
            ...
            for (int i = 0; i < frameCount; i++) {
                ...
                if (rec.fDurations[i] != frameInfo.fDuration) {
                    ERRORF(r, "%s frame %i's durations do not match! expected: %i\tactual: %i", rec.fName, i, rec.fDurations[i], frameInfo.fDuration);
                }
                ...
                if (expectedAlpha != alpha) {
                    ERRORF(r, "%s's frame %i has wrong alpha type! expected: %s\tactual: %s", rec.fName, i, to_string(expectedAlpha), to_string(alpha));
                }
                ...
                if (0 == i) {
                    ...
                    ERRORF(
                        r, "%s's frame %i has wrong dependency! expected: %i\tactual: %i", rec.fName, i, rec.fRequiredFrames[i - 1], frameInfo.fRequiredFrame);
                }

                REPORTER_ASSERT(r, frameInfo.fDisposalMethod == rec.fDisposalMethods[i]);

                reporter_assert_equals<bool>(r, rec.fName, i, "alpha within bounds", rec.fAlphaWithinBounds[i], frameInfo.fHasAlphaWithinBounds);

                reporter_assert_equals(r, rec.fName, i, "blend mode", rec.fBlends[i], frameInfo.fBlend);

                reporter_assert_equals(r, rec.fName, i, "frame rect", rec.fFrameRects[i], frameInfo.fFrameRect);
            }
...
static void test_animated_AndroidCodec(skiatest::Reporter* r, const char* file)
{
    ...
    for (int sampleSize : { 8, 32, 100 }) {
        ...
        for (int i = 0; i < codec->codec()->getFrameCount(); ++i) {
            ...
            if (5 == i) {
                REPORTER_ASSERT(r, frameInfo.fDisposalMethod == SkCodecAnimation::DisposalMethod::kRestoreBGColor);
            }
...
DEF_TEST(EncodedOriginToMatrixTest, r)
{
    ...
    for (auto origin : { kTopLeft_SkEncodedOrigin, kTopRight_SkEncodedOrigin, kBottomRight_SkEncodedOrigin, kBottomLeft_SkEncodedOrigin,
             kLeftTop_SkEncodedOrigin, kRightTop_SkEncodedOrigin, kRightBottom_SkEncodedOrigin, kLeftBottom_SkEncodedOrigin }) {
...
DEF_TEST(AnimCodecPlayer, r)
{
    ...
    for (const auto& test : gTests) {
        ...
        REPORTER_ASSERT(r, f0->bounds().size() == test.fSize, "Mismatched size for initial frame of %s", test.fFile);

        player->seek(500);
        auto f1 = player->getFrame();
        REPORTER_ASSERT(r, f1);
        REPORTER_ASSERT(r, f1->bounds().size() == test.fSize, "Mismatched size for frame at 500 ms of %s", test.fFile);
    }
}
