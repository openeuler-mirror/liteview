...
static void raster_tests(skiatest::Reporter* reporter, const TestCase& test)
{
    ...
    const SkImageInfo f32Unpremul = SkImageInfo::Make(kSize, kSize, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType);
...
static void compare_pixmaps(skiatest::Reporter* reporter, const SkPixmap& expected, const SkPixmap& actual, SkColorType ct, const char* label)
{
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([reporter, ct, label](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(reporter, "%s %s - mismatch at %d, %d (%f, %f, %f %f)", ToolUtils::colortype_name(ct), label, x, y, diffs[0], diffs[1], diffs[2], diffs[3]);
    });
    ...
    for (bool fullInit : { false, true }) {
        ...
        if (fullInit) {
            backendTex = dContext->createBackendTexture(
                nativeExpected, kTopLeft_GrSurfaceOrigin, GrRenderable::kNo, GrProtected::kNo, markFinished, &finishedBECreate);
        } else {
            backendTex = dContext->createBackendTexture(
                kSize, kSize, test.fColorType, SkColors::kWhite, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo, markFinished, &finishedBECreate);
        }
        ...
        auto img = SkImage::MakeFromTexture(dContext, backendTex, kTopLeft_GrSurfaceOrigin, test.fColorType, test.fAlphaType, nullptr);
...

        {
            ...
            if (img->readPixels(dContext, nativeActual, 0, 0)) {
                compare_pixmaps(reporter, nativeExpected, nativeActual, test.fColorType, "SkImage::readPixels to native CT");
            }
            ...
            if (dContext->colorTypeSupportedAsSurface(test.fColorType)) {
                ...
                compare_pixmaps(reporter, nativeExpected, nativeActual, test.fColorType, "SkSurface::readPixels to native CT");
            }
        }

        {
            ...
            // read back to F32 if possible
            {
                ...
                if (img->readPixels(dContext, f32Actual, 0, 0)) {
                    compare_pixmaps(reporter, f32Expected, f32Actual, test.fColorType, "SkImage::readPixels to F32");
                }
            }
            ...
            {
                const SkImageInfo rgba8888Premul = SkImageInfo::Make(kSize, kSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
                ...
                compare_pixmaps(reporter, f32Expected, f32Actual, test.fColorType, "SkSurface::drawn to RGBA8888");
            }
        }
