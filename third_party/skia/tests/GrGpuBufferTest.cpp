...
public:
    ...
    {
        return GrOp::Make<TestVertexOp>(context, std::move(buffer), baseVertex, vertexCount, bounds);
    }
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        auto analysis = fProcessorSet.finalize(
            GrProcessorAnalysisColor::Opaque::kYes, GrProcessorAnalysisCoverage::kNone, clip, &GrUserStencilSettings::kUnused, caps, clampType, &color);
...
private:
    ...
    TestVertexOp(sk_sp<GrGpuBuffer> buffer, int baseVertex, int vertexCount, const SkRect& bounds)
    ...
    public:
        GP()
            : GrGeometryProcessor(kTestFP_ClassID)
        {
        ...
        std::unique_ptr<ProgramImpl> makeProgramImpl(const GrShaderCaps&) const override
        {
            ...
            private:
                void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
                {
                    ...
                    args.fFragBuilder->codeAppendf("half4 %s = half4(0, 1, 0, 1);", args.fOutputColor);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrGpuBufferTransferTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto create_cpu_to_gpu_buffer = [&](int baseVertex) {
        ...
        static constexpr SkPoint kUnitQuad[] { { 0, 0 }, { 0, 1 }, { 1, 0 }, { 1, 0 }, { 0, 1 }, { 1, 1 } };
        std::copy_n(kUnitQuad, 6, points.get() + baseVertex);

        return rp->createBuffer(points.get(), totalVertices * sizeof(SkPoint), GrGpuBufferType::kXferCpuToGpu, kDynamic_GrAccessPattern);
    };

    auto create_vertex_buffer = [&](sk_sp<GrGpuBuffer> srcBuffer, int srcBaseVertex, int vbBaseVertex, bool useTask, bool minSizedTransfers) {
        ...
        sk_sp<GrGpuBuffer> vb = rp->createBuffer(points.get(), dstVertexCount * sizeof(SkPoint), GrGpuBufferType::kVertex, kDynamic_GrAccessPattern);
        ...
        if (minSizedTransfers) {
            for (size_t n = kTotalSize / alignment, i = 0; i < n; ++i) {
                if (useTask) {
                    dm->newBufferTransferTask(srcBuffer, srcOffset + i * alignment, vb, vbOffset + i * alignment, alignment);
                } else {
                    gpu->transferFromBufferToBuffer(srcBuffer, srcOffset + i * alignment, vb, vbOffset + i * alignment, alignment);
                }
            }
        } else if (useTask) {
    ...
    auto sdc
        = skgpu::v1::SurfaceDrawContext::Make(dc, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { 1, 1 }, SkSurfaceProps {}, std::string_view {});
    if (!sdc) {
        ...
        for (bool minSizedTransfers : { false, true }) {
            for (int srcBaseVertex : { 0, 5 }) {
                ...
                for (int vbBaseVertex : { 0, 2 }) {
                    auto vb = create_vertex_buffer(src, srcBaseVertex, vbBaseVertex, useTask, minSizedTransfers);
                    if (!vb) {
                    ...
                    sdc->addDrawOp(nullptr,
                        TestVertexOp::Make(dc, vb, vbBaseVertex,
                            /*vertexCount=*/6, kBounds));
                    ...
                    *color = kRed.premul();
                    ...
                    REPORTER_ASSERT(reporter, *color == kGreen,
                        ...
                        "use task: %d, "
                        "minSizedTransfers: %d",
                        srcBaseVertex, vbBaseVertex, useTask, minSizedTransfers);
                }
            }
        }
    }
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrGpuBufferUpdateDataTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    GrDirectContext* dc = ctxInfo.directContext();
    ...
    for (bool piecewise : { false, true }) {
        ...
        for (size_t offset : { size_t { 0 }, 4 * sizeof(SkPoint), size_t { 1 }, size_t { 27 } }) {
            ...
            for (auto accessPattern : { kStatic_GrAccessPattern,
                ...
                auto vb = gpu->createBuffer(sizeof(kUnitQuad) + offset + sizeof(SkPoint), GrGpuBufferType::kVertex, accessPattern);
                if (!vb) {
                    ERRORF(reporter, "Could not create vertex buffer");
                    return;
                }
                ...
                sdc->addDrawOp(nullptr, TestVertexOp::Make(dc, vb, baseVertex, std::size(kUnitQuad), kBounds));
                ...
                *color = kRed.premul();
                ...
                REPORTER_ASSERT(reporter, *color == kGreen, "piecewise: %d, offset: %zu", piecewise, offset);
            }
        }
    }
}
