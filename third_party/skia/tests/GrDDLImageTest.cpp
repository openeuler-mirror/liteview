...
DEF_GANESH_TEST(GrDDLImage_MakeSubset, reporter, options, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int ct = 0; ct < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++ct) {
        ...
        GrBackendTexture tex = dContext->createBackendTexture(ii.width(), ii.height(), ii.colorType(), GrMipmapped(sc.isMipMapped()), GrRenderable::kYes);
        auto gpuImage = SkImage::MakeFromTexture(dContext, tex, kTopLeft_GrSurfaceOrigin, ii.colorType(), ii.alphaType(), ii.refColorSpace());
