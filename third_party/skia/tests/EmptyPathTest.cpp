...
static void drawAndTest(skiatest::Reporter* reporter, const SkPath& path, const SkPaint& paint, bool shouldDraw)
{
    ...
    if (!success) {
        ...
        ERRORF(reporter,
            ...
            str, paint.getStyle(), paint.getStrokeCap(), paint.getStrokeJoin(), paint.isAntiAlias(), (int)path.getFillType(), path.countPoints());
...
{
    static const SkPaint::Cap gCaps[] = { SkPaint::kButt_Cap, SkPaint::kRound_Cap, SkPaint::kSquare_Cap };
    static const SkPaint::Join gJoins[] = { SkPaint::kMiter_Join, SkPaint::kRound_Join, SkPaint::kBevel_Join };
    static const SkPaint::Style gStyles[] = { SkPaint::kFill_Style, SkPaint::kStroke_Style, SkPaint::kStrokeAndFill_Style };
    for (size_t cap = 0; cap < std::size(gCaps); ++cap) {
        for (size_t join = 0; join < std::size(gJoins); ++join) {
            for (size_t style = 0; style < std::size(gStyles); ++style) {
                if (drawCaps && SkPaint::kButt_Cap != gCaps[cap] && SkPaint::kFill_Style != gStyles[style]) {
                    continue;
                }
...
{
    path->moveTo(CX, CY).moveTo(CX, CY);
}
static void make_MZM(SkPath* path)
{
    path->moveTo(CX, CY).close().moveTo(CX, CY);
}
...
{
    path->moveTo(CX, CY).quadTo(CX, CY, CX, CY);
}
static void make_C(SkPath* path)
{
    path->moveTo(CX, CY).cubicTo(CX, CY, CX, CY, CX, CY);
}
...
{
    static void (*gMakeProc[])(SkPath*) = { make_empty, make_M, make_MM, make_MZM, make_L, make_Q, make_C };
    static SkPathFillType gFills[] = { SkPathFillType::kWinding, SkPathFillType::kEvenOdd, SkPathFillType::kInverseWinding, SkPathFillType::kInverseEvenOdd };
    for (int doClose = 0; doClose < 2; ++doClose) {
        for (size_t i = 0; i < std::size(gMakeProc); ++i) {
            ...
            bool allowCaps = make_L == gMakeProc[i] || make_Q == gMakeProc[i] || make_C == gMakeProc[i] || make_MZM == gMakeProc[i];
