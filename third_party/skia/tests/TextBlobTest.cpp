...
public:
    ...
    static void TestBuilder(skiatest::Reporter* reporter)
    {
        ...
        RunBuilderTest(reporter, builder, set5, std::size(set5), mergedSet5, std::size(mergedSet5));
        ...
        RunBuilderTest(reporter, builder, set6, std::size(set6), mergedSet6, std::size(mergedSet6));
        ...
        RunBuilderTest(reporter, builder, set7, std::size(set7), mergedSet7, std::size(mergedSet7));
    }
...
private:
    ...
    static void RunBuilderTest(
        skiatest::Reporter* reporter, SkTextBlobBuilder& builder, const RunDef in[], unsigned inCount, const RunDef out[], unsigned outCount)
    {
        SkFont font;
    ...
    static void AddRun(const SkFont& font, int count, SkTextBlobRunIterator::GlyphPositioning pos, const SkPoint& offset, SkTextBlobBuilder& builder,
        const SkRect* bounds = nullptr)
    {
        switch (pos) {
        case SkTextBlobRunIterator::kDefault_Positioning: {
            const SkTextBlobBuilder::RunBuffer& rb = builder.allocRun(font, count, offset.x(), offset.y(), bounds);
            for (int i = 0; i < count; ++i) {
                rb.glyphs[i] = i;
            }
        } break;
        case SkTextBlobRunIterator::kHorizontal_Positioning: {
            const SkTextBlobBuilder::RunBuffer& rb = builder.allocRunPosH(font, count, offset.y(), bounds);
            for (int i = 0; i < count; ++i) {
                rb.glyphs[i] = i;
...
{
    auto surf = SkSurface::MakeRasterN32Premul(SkScalarRoundToInt(blob->bounds().width()), SkScalarRoundToInt(blob->bounds().height()));
    if (!surf) {
