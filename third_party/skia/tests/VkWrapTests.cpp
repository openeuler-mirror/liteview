...
void wrap_tex_test(skiatest::Reporter* reporter, GrDirectContext* dContext)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(dContext, kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    if (!mbet) {
    ...
    {
        sk_sp<GrTexture> tex = gpu->wrapBackendTexture(origBackendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
    ...
    // image is null
    {
        ...
        sk_sp<GrTexture> tex = gpu->wrapBackendTexture(backendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
        REPORTER_ASSERT(reporter, !tex);
        tex = gpu->wrapBackendTexture(backendTex, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
        REPORTER_ASSERT(reporter, !tex);
    }
    ...
    // check adopt creation
    {
        GrVkImageInfo backendCopy = imageInfo;
        GrBackendTexture backendTex = GrBackendTexture(kW, kH, backendCopy);
        sk_sp<GrTexture> tex = gpu->wrapBackendTexture(backendTex, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType);
...
void wrap_rt_test(skiatest::Reporter* reporter, GrDirectContext* dContext)
{
    ...
    for (int sampleCnt : { 1, 4 }) {
        ...
        GrBackendRenderTarget origBackendRT = gpu->createTestingOnlyBackendRenderTarget({ kW, kH }, ct, sampleCnt);
...
void wrap_trt_test(skiatest::Reporter* reporter, GrDirectContext* dContext)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(dContext, kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kYes);
    if (!mbet) {
    ...
    sk_sp<GrTexture> tex = gpu->wrapRenderableBackendTexture(origBackendTex, 1, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
    REPORTER_ASSERT(reporter, tex);
    ...
    // image is null
    {
        ...
        backendCopy.fImage = VK_NULL_HANDLE;
        GrBackendTexture backendTex = GrBackendTexture(kW, kH, backendCopy);
        tex = gpu->wrapRenderableBackendTexture(backendTex, 1, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
        REPORTER_ASSERT(reporter, !tex);
        tex = gpu->wrapRenderableBackendTexture(backendTex, 1, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo);
        REPORTER_ASSERT(reporter, !tex);
    }
    ...
    // check rendering with MSAA
    {
        ...
        tex = gpu->wrapRenderableBackendTexture(origBackendTex, 2, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo);
