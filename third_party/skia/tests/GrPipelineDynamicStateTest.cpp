...
static const GrColor kMeshColors[kNumMeshes] { GrColorPackRGBA(255, 0, 0, 255), GrColorPackRGBA(0, 255, 0, 255), GrColorPackRGBA(0, 0, 255, 255),
...
private:
    PipelineDynamicStateTestProcessor()
    ...
    const Attribute& inVertex() const
    {
...
std::unique_ptr<GrGeometryProcessor::ProgramImpl> PipelineDynamicStateTestProcessor::makeProgramImpl(const GrShaderCaps&) const
{
    ...
    public:
        ...
        {
            const PipelineDynamicStateTestProcessor& mp = args.fGeomProc.cast<PipelineDynamicStateTestProcessor>();
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* context, GrScissorTest scissorTest, sk_sp<const GrBuffer> vbuff)
    {
...
private:
    ...
    {
        GrPipeline pipeline(fScissorTest, SkBlendMode::kSrc, flushState->drawOpArgs().writeView().swizzle());
        ...
        GrProgramInfo programInfo(flushState->caps(), flushState->writeView(), flushState->usesMSAASurface(), &pipeline, &GrUserStencilSettings::kUnused,
            geomProc, GrPrimitiveType::kTriangleStrip, flushState->renderPassBarriers(), flushState->colorLoadOp());
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrPipelineDynamicStateTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(
        dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { kScreenSize, kScreenSize }, SkSurfaceProps(), /*label=*/ {});
    if (!sdc) {
    ...
    Vertex vdata[kNumMeshes * 4] = { { 0, 0, kMeshColors[0] }, { 0, d, kMeshColors[0] }, { d, 0, kMeshColors[0] }, { d, d, kMeshColors[0] },

        { 0, 0, kMeshColors[1] }, { 0, d, kMeshColors[1] }, { d, 0, kMeshColors[1] }, { d, d, kMeshColors[1] },

        { 0, 0, kMeshColors[2] }, { 0, d, kMeshColors[2] }, { d, 0, kMeshColors[2] }, { d, d, kMeshColors[2] },

        { 0, 0, kMeshColors[3] }, { 0, d, kMeshColors[3] }, { d, 0, kMeshColors[3] }, { d, d, kMeshColors[3] } };

    sk_sp<const GrBuffer> vbuff(rp->createBuffer(vdata, sizeof(vdata), GrGpuBufferType::kVertex, kDynamic_GrAccessPattern));
    if (!vbuff) {
    ...
    for (GrScissorTest scissorTest : { GrScissorTest::kEnabled, GrScissorTest::kDisabled }) {
        ...
        auto ii = SkImageInfo::Make(kScreenSize, kScreenSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
        ...
        for (int y = 0; y < kScreenSize; ++y) {
            for (int x = 0; x < kScreenSize; ++x) {
                ...
                if (expected != actual) {
                    ERRORF(reporter, "[scissor=%s] pixel (%i,%i): got 0x%x expected 0x%x", GrScissorTest::kEnabled == scissorTest ? "enabled" : "disabled", x,
                        y, actual, expected);
                    return;
                }
            }
