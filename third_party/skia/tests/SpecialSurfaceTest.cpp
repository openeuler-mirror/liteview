...
static void test_surface(const sk_sp<SkSpecialSurface>& surf, skiatest::Reporter* reporter, int offset)
{
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SpecialSurface_Gpu1, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (auto colorType : { kRGBA_8888_SkColorType, kRGBA_1010102_SkColorType }) {
        ...
        SkImageInfo ii = SkImageInfo::Make({ kSurfaceSize, kSurfaceSize }, colorType, kPremul_SkAlphaType);

        auto surf(SkSpecialSurface::MakeRenderTarget(dContext, ii, SkSurfaceProps(), kTopLeft_GrSurfaceOrigin));
        test_surface(surf, reporter, 0);
    }
}
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(SpecialSurface_Graphite, reporter, context)
{
    ...
    for (auto colorType : { kRGBA_8888_SkColorType, kRGBA_1010102_SkColorType }) {
        TextureInfo info = caps->getDefaultSampledTextureInfo(colorType,
            /*levelCount=*/1, skgpu::Protected::kNo, Renderable::kYes);
        if (!info.isValid()) {
            continue;
        }
