...
void runFPTest(skiatest::Reporter* reporter, GrDirectContext* dContext, T min, T max, T epsilon, T maxInt, int arraySize, GrColorType colorType)
{
    ...
    for (auto origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
        ...
        auto fpView = sk_gpu_test::MakeTextureProxyViewFromData(dContext, GrRenderable::kYes, origin, controlPixmap);
        ...
        REPORTER_ASSERT(reporter, !memcmp(readBuffer.begin(), controlPixelData.begin(), readBuffer.size_bytes()));
    }
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(HalfFloatAlphaTextureTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    runFPTest<SkHalf>(reporter, direct, SK_HalfMin, SK_HalfMax, SK_HalfEpsilon, kMaxIntegerRepresentableInHalfFloatingPoint, HALF_ALPHA_CONTROL_ARRAY_SIZE,
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(HalfFloatRGBATextureTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    runFPTest<SkHalf>(reporter, direct, SK_HalfMin, SK_HalfMax, SK_HalfEpsilon, kMaxIntegerRepresentableInHalfFloatingPoint, HALF_RGBA_CONTROL_ARRAY_SIZE,
