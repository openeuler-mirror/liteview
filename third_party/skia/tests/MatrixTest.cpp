...
static void assert9(
    skiatest::Reporter* reporter, const SkMatrix& m, SkScalar a, SkScalar b, SkScalar c, SkScalar d, SkScalar e, SkScalar f, SkScalar g, SkScalar h, SkScalar i)
{
    SkScalar buffer[9];
    m.get9(buffer);
...
static void test_matrix_min_max_scale(skiatest::Reporter* reporter)
{
    ...
    // skbug.com/4718
    SkMatrix big;
    big.setAll(2.39394089e+36f, 8.85347779e+36f, 9.26526204e+36f, 3.9159619e+36f, 1.44823453e+37f, 1.51559342e+37f, 0.f, 0.f, 1.f);
    success = big.getMinMaxScales(scales);
    REPORTER_ASSERT(reporter, !success);

    // skbug.com/4718
    SkMatrix givingNegativeNearlyZeros;
    givingNegativeNearlyZeros.setAll(0.00436534f, 0.114138f, 0.37141f, 0.00358857f, 0.0936228f, -0.0174198f, 0.f, 0.f, 1.f);
...
static bool check_matrix_recomposition(const SkMatrix& mat, const SkPoint& rotation1, const SkPoint& scale, const SkPoint& rotation2)
{
    ...
    bool result = scalar_nearly_equal_relative(mat[SkMatrix::kMScaleX], scaleX * c1 * c2 - scaleY * s1 * s2)
...
static bool check_decompScale(const SkMatrix& original)
{
    ...
    const SkPoint testPts[kNumPoints] = { { 0.0f, 0.0f }, { 1.0f, 1.0f }, { 1.0f, 0.5f }, { -1.0f, -0.5f }, { -1.0f, 2.0f } };
...
DEF_TEST(Matrix, reporter)
{
    ...
    // rectStaysRect test
    {
        static const struct {
            ...
            bool mStaysRect;
        } gRectStaysRectSamples[] = { { 0, 0, 0, 0, false }, { 0, 0, 0, 1, false }, { 0, 0, 1, 0, false }, { 0, 0, 1, 1, false }, { 0, 1, 0, 0, false },
            { 0, 1, 0, 1, false }, { 0, 1, 1, 0, true }, { 0, 1, 1, 1, false }, { 1, 0, 0, 0, false }, { 1, 0, 0, 1, true }, { 1, 0, 1, 0, false },
            { 1, 0, 1, 1, false }, { 1, 1, 0, 0, false }, { 1, 1, 0, 1, false }, { 1, 1, 1, 0, false }, { 1, 1, 1, 1, false } };
...
DEF_TEST(Matrix_maprects, r)
{
    ...
    for (int i = 0; i < 10000; ++i) {
        SkRect src = SkRect::MakeLTRB(rand.nextSScalar1() * scale, rand.nextSScalar1() * scale, rand.nextSScalar1() * scale, rand.nextSScalar1() * scale);
        SkRect dst[4];
