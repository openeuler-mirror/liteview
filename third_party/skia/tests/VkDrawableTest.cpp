...
public:
    TestDrawable(const skgpu::VulkanInterface* interface, GrDirectContext* dContext, int32_t width, int32_t height)
        : INHERITED()
    ...
    public:
        ...
        void draw(const GrBackendDrawableInfo& info) override
        {
            ...
            GR_VK_CALL(fInterface, CmdClearAttachments(vkInfo.fSecondaryCommandBuffer, 1, &attachment, 1, &clearRect));
    ...
    typedef void (*DrawProc)(TestDrawable*, const SkMatrix&, const SkIRect&, const SkImageInfo&, const GrVkDrawableInfo&);
    ...
    public:
        DrawHandlerImport(
            TestDrawable* td, DrawProc drawProc, SubmitProc submitProc, const SkMatrix& matrix, const SkIRect& clipBounds, const SkImageInfo& bufferInfo)
            : INHERITED()
            , fTestDrawable(td)
            , fDrawProc(drawProc)
    ...
    static void ImportDraw(TestDrawable* td, const SkMatrix& matrix, const SkIRect& clipBounds, const SkImageInfo& bufferInfo, const GrVkDrawableInfo& info)
    {
        ...
        sk_sp<SkSurface> surf = SkSurface::MakeRenderTarget(td->fDContext, SkBudgeted::kYes, bufferInfo);
        ...
        canvas->drawImageRect(surf->makeImageSnapshot(), srcRect, dstRect, SkSamplingOptions(), &paint, SkCanvas::kStrict_SrcRectConstraint);
    ...
    std::unique_ptr<GpuDrawHandler> onSnapGpuDrawHandler(
        GrBackendApi backendApi, const SkMatrix& matrix, const SkIRect& clipBounds, const SkImageInfo& bufferInfo) override
    {
        ...
        if (fDContext) {
            draw = std::make_unique<DrawHandlerImport>(this, ImportDraw, ImportSubmitted, matrix, clipBounds, bufferInfo);
        } else {
...
void draw_drawable_test(skiatest::Reporter* reporter, GrDirectContext* dContext, GrDirectContext* childDContext)
{
    GrVkGpu* gpu = static_cast<GrVkGpu*>(dContext->priv().getGpu());

    const SkImageInfo ii = SkImageInfo::Make(DEV_W, DEV_H, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    sk_sp<SkSurface> surface(SkSurface::MakeRenderTarget(dContext, SkBudgeted::kNo, ii, 0, kTopLeft_GrSurfaceOrigin, nullptr));
