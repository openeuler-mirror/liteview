...
static bool nearly_equal(const SkColor& x, const SkColor& y)
{
    ...
    return abs((int)SkColorGetA(x) - (int)SkColorGetA(y)) <= kTolerance && abs((int)SkColorGetR(x) - (int)SkColorGetR(y)) <= kTolerance
        && abs((int)SkColorGetG(x) - (int)SkColorGetG(y)) <= kTolerance && abs((int)SkColorGetB(x) - (int)SkColorGetB(y)) <= kTolerance;
}
        ...
        for (int alpha : { 0x80, 0xFF }) {
            for (bool useShader : { false, true }) {
                ...
                for (bool useRuntimeBlend : { false, true }) {
                    ...
                    REPORTER_ASSERT(r,
                        surface->readPixels(bitmap.info(), bitmap.getPixels(), bitmap.rowBytes(),
                            /*srcX=*/0,
                            /*srcY=*/0));
                ...
                REPORTER_ASSERT(r, nearly_equal(colors[0], colors[1]),
                    ...
                    SkBlendMode_Name(mode), (alpha == 0xFF) ? "solid" : "transparent", useShader ? "shader" : "paint", SkColorGetA(colors[0]),
                    SkColorGetR(colors[0]), SkColorGetG(colors[0]), SkColorGetB(colors[0]), SkColorGetA(colors[1]), SkColorGetR(colors[1]),
