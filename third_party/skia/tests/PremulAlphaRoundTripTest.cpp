...
static void test_premul_alpha_roundtrip(skiatest::Reporter* reporter, SkSurface* surf)
{
    for (size_t upmaIdx = 0; upmaIdx < std::size(gUnpremul); ++upmaIdx) {
        ...
        const SkImageInfo info = SkImageInfo::Make(256, 256, gUnpremul[upmaIdx].fColorType, kUnpremul_SkAlphaType);
        SkBitmap readBmp1;
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(PremulAlphaRoundTrip_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    sk_sp<SkSurface> surf(SkSurface::MakeRenderTarget(ctxInfo.directContext(), SkBudgeted::kNo, info));
    test_premul_alpha_roundtrip(reporter, surf.get());
}
...
DEF_TEST(PremulAlphaRoundTripGrConvertPixels, reporter)
{
    ...
    const SkImageInfo upmInfo = SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
    const SkImageInfo pmInfo = SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
...
DEF_TEST(PremulAlphaRoundTripSkConvertPixels, reporter)
{
    ...
    auto convert = [](const SkBitmap& dst, const SkBitmap& src) {
        SkAssertResult(SkConvertPixels(dst.info(), dst.getAddr(0, 0), dst.rowBytes(), src.info(), src.getAddr(0, 0), src.rowBytes()));
    };
