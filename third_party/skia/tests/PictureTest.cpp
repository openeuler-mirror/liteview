...
{
    SaveCountingCanvas canvas(SkScalarCeilToInt(picture->cullRect().width()), SkScalarCeilToInt(picture->cullRect().height()));
...
static void test_clip_bound_opt(skiatest::Reporter* reporter)
{
    ...
    SkRect rect1 = SkRect::MakeXYWH(SkIntToScalar(4), SkIntToScalar(4), SkIntToScalar(2), SkIntToScalar(2));
    SkRect rect2 = SkRect::MakeXYWH(SkIntToScalar(7), SkIntToScalar(7), SkIntToScalar(1), SkIntToScalar(1));
    SkRect rect3 = SkRect::MakeXYWH(SkIntToScalar(6), SkIntToScalar(6), SkIntToScalar(1), SkIntToScalar(1));
...
static void draw_bitmaps(const SkBitmap bitmap, SkCanvas* canvas)
{
    ...
    canvas->drawImageRect(img, rect, rect, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
...
public:
    explicit SpoonFedBBHFactory(sk_sp<SkBBoxHierarchy> bbh)
        : fBBH(bbh)
    {
    }
...
DEF_TEST(Picture_fillsBBH, r)
{
    ...
    for (int n = 0; n <= 2; n++) {
        ...
        REPORTER_ASSERT(r, (int)results.size() == n, "results.size() == %d, want %d\n", (int)results.size(), n);
    }
}
