...
DEF_TEST(Bitmap_erase_srgb, r)
{
    ...
    bm.allocPixels(SkImageInfo::Make(1, 1, kN32_SkColorType, kPremul_SkAlphaType, SkColorSpace::MakeSRGB()->makeColorSpin()));
...
DEF_TEST(getalphaf, reporter)
{
    ...
    auto nearly_half = [](float expected, float actual) -> bool { return unit_compare(expected, actual, 1.0f / (1 << 10)); };
