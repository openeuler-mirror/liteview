...
DEF_TEST(MallocPixelRef, reporter)
{
    ...
    }
    {
        ...
        sk_sp<SkPixelRef> pr(SkMakePixelRefWithProc(info.width(), info.height(), rowBytes, addr, delete_uint8_proc, nullptr));
        REPORTER_ASSERT(reporter, pr.get() != nullptr);
    ...
    }
    {
        int x = 0;
        SkAutoMalloc memory(size);
        sk_sp<SkPixelRef> pr(SkMakePixelRefWithProc(info.width(), info.height(), rowBytes, memory.get(), set_to_one_proc, static_cast<void*>(&x)));
        REPORTER_ASSERT(reporter, pr.get() != nullptr);
        REPORTER_ASSERT(reporter, memory.get() == pr->pixels());
