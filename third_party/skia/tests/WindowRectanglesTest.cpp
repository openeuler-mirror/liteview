...
DEF_TEST(WindowRectangles, reporter)
{
    ...
    SkASSERT(wr.count() == GrWindowRectangles::kMaxWindows - 1);
    {
        ...
        REPORTER_ASSERT(reporter, A.data()[GrWindowRectangles::kMaxWindows - 1] == windowData[GrWindowRectangles::kMaxWindows - 1]);
        REPORTER_ASSERT(reporter, B.data()[GrWindowRectangles::kMaxWindows - 1] == SkRectPriv::MakeILarge());
    }
    {
        GrWindowRectangles A(wr), B(wr);
        REPORTER_ASSERT(reporter, B == A);
