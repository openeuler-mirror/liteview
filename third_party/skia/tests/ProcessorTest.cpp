...
public:
    ...
    GrProcessorSet::Analysis finalize(const GrCaps& caps, const GrAppliedClip* clip, GrClampType clampType) override
    {
        ...
        return fProcessors.finalize(kUnknownColor, GrProcessorAnalysisCoverage::kNone, clip, &GrUserStencilSettings::kUnused, caps, clampType, &overrideColor);
    }
...
DEF_GANESH_TEST_FOR_ALL_CONTEXTS(ProcessorRefTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    const GrBackendFormat format = dContext->priv().caps()->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kNo);
    skgpu::Swizzle swizzle = dContext->priv().caps()->getReadSwizzle(format, GrColorType::kRGBA_8888);

    for (bool makeClone : { false, true }) {
        for (int parentCnt = 0; parentCnt < 2; parentCnt++) {
            auto sdc = skgpu::v1::SurfaceDrawContext::Make(
                dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kApprox, { 1, 1 }, SkSurfaceProps(), /*label=*/ {});
            {
                sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(format, kDims, GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kExact,
...
static GrColor input_texel_color(int x, int y, SkScalar delta)
{
    ...
    SkColor color = SkColorSetARGB((uint8_t)(x & 0xFF), (uint8_t)(y & 0xFF), (uint8_t)((x + y) & 0xFF), (uint8_t)((2 * y - x) & 0xFF));
...
static void render_fp(GrDirectContext* dContext, skgpu::v1::SurfaceDrawContext* sdc, std::unique_ptr<GrFragmentProcessor> fp, GrColor* outBuffer)
{
...
public:
    ...
    bool init()
    {
...

        {
            ...
            for (int y = 0; y < kTestTextureSize; ++y) {
                for (int x = 0; x < kTestTextureSize; ++x) {
                    rgbaData[kTestTextureSize * y + x] = input_texel_color(random.nextULessThan(256), random.nextULessThan(256), 0.0f);
                }
            }

            SkImageInfo ii = SkImageInfo::Make(kTestTextureSize, kTestTextureSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
            SkBitmap bitmap;
            bitmap.installPixels(
                ii, rgbaData, ii.minRowBytes(), [](void* addr, void* context) { delete[] (GrColor*)addr; }, nullptr);
            bitmap.setImmutable();
            ...
            fTestViews[0] = GrProcessorTestData::ViewInfo { view, GrColorType::kRGBA_8888, kPremul_SkAlphaType };
        }

        {
            ...
            SkImageInfo ii = SkImageInfo::Make(kTestTextureSize, kTestTextureSize, kAlpha_8_SkColorType, kPremul_SkAlphaType);
            SkBitmap bitmap;
            bitmap.installPixels(
                ii, alphaData, ii.minRowBytes(), [](void* addr, void* context) { delete[] (uint8_t*)addr; }, nullptr);
            bitmap.setImmutable();
            auto view = std::get<0>(GrMakeUncachedBitmapProxyView(fContext, bitmap));
            if (!view || !view.proxy()->instantiate(fResourceProvider)) {
            ...
            fTestViews[1] = GrProcessorTestData::ViewInfo { view, GrColorType::kAlpha_8, kPremul_SkAlphaType };
        }
    ...
    std::unique_ptr<GrFragmentProcessor> make(int type, int randomTreeDepth, std::unique_ptr<GrFragmentProcessor> inputFP)
    {
        ...
        SkRandom random { fRandomSeed };
        GrProcessorTestData testData { &random, fContext, randomTreeDepth, static_cast<int>(std::size(fTestViews)), fTestViews, std::move(inputFP) };
        return GrFragmentProcessorTestFactory::MakeIdx(type, &testData);
    }

    std::unique_ptr<GrFragmentProcessor> make(int type, int randomTreeDepth, GrSurfaceProxyView view, SkAlphaType alpha = kPremul_SkAlphaType)
    {
...
static GrSurfaceProxyView make_input_texture(GrRecordingContext* context, int width, int height, GrColor* pixel)
{
...
{
    SkImageInfo info = SkImageInfo::Make(widthHeight, widthHeight, kRGBA_8888_SkColorType, kLogAlphaType);
    SkBitmap bmp;
...
{
    SkImageInfo ii = SkImageInfo::Make(src.proxy()->dimensions(), kRGBA_8888_SkColorType, kLogAlphaType);
...
static bool legal_modulation(const GrColor inGr[3], const GrColor outGr[3])
{
    ...
    bool isLegalColorModulation = fuzzy_color_equals(outf[0], expectedForColorModulation[0]) && fuzzy_color_equals(outf[1], expectedForColorModulation[1])
        && fuzzy_color_equals(outf[2], expectedForColorModulation[2]);

    bool isLegalAlphaModulation = fuzzy_color_equals(outf[0], expectedForAlphaModulation[0]) && fuzzy_color_equals(outf[1], expectedForAlphaModulation[1])
    ...
    if ((false)) {
        if (!isLegalColorModulation && !isLegalAlphaModulation) {
            SkDebugf("Color modulation test\n\timplied mod color: (%.03f, %.03f, %.03f, %.03f)\n", fpPreColorModulation[0], fpPreColorModulation[1],
                fpPreColorModulation[2], fpPreColorModulation[3]);
            for (int i = 0; i < 3; ++i) {
                SkDebugf("\t(%.03f, %.03f, %.03f, %.03f) -> "
                         "(%.03f, %.03f, %.03f, %.03f) | "
                         "(%.03f, %.03f, %.03f, %.03f), ok: %d\n",
                    inf[i].fR, inf[i].fG, inf[i].fB, inf[i].fA, outf[i].fR, outf[i].fG, outf[i].fB, outf[i].fA, expectedForColorModulation[i].fR,
                    expectedForColorModulation[i].fG, expectedForColorModulation[i].fB, expectedForColorModulation[i].fA,
                    fuzzy_color_equals(outf[i], expectedForColorModulation[i]));
            }
            SkDebugf("Alpha modulation test\n\timplied mod color: (%.03f, %.03f, %.03f, %.03f)\n", fpPreAlphaModulation[0], fpPreAlphaModulation[1],
                fpPreAlphaModulation[2], fpPreAlphaModulation[3]);
            for (int i = 0; i < 3; ++i) {
                SkDebugf("\t(%.03f, %.03f, %.03f, %.03f) -> "
                         "(%.03f, %.03f, %.03f, %.03f) | "
                         "(%.03f, %.03f, %.03f, %.03f), ok: %d\n",
                    inf[i].fR, inf[i].fG, inf[i].fB, inf[i].fA, outf[i].fR, outf[i].fG, outf[i].fB, outf[i].fA, expectedForAlphaModulation[i].fR,
                    expectedForAlphaModulation[i].fG, expectedForAlphaModulation[i].fB, expectedForAlphaModulation[i].fA,
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(ProcessorOptimizationValidationTest, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(
        context, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { kRenderSize, kRenderSize }, SkSurfaceProps(), /*label=*/ {});
    ...
    GrSurfaceProxyView inputTexture1 = make_input_texture(context, kRenderSize, kRenderSize, inputPixels1.data());
    GrSurfaceProxyView inputTexture2 = make_input_texture(context, kRenderSize, kRenderSize, inputPixels2.data());
    GrSurfaceProxyView inputTexture3 = make_input_texture(context, kRenderSize, kRenderSize, inputPixels3.data());
    ...
    for (int i = 0; i < FPFactory::Count(); ++i) {
        ...
        for (int trial = 0;; ++trial) {
            ...
            fpGenerator.reroll();
            std::unique_ptr<GrFragmentProcessor> fp = fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture1);
            ...
            if (trial >= kMinimumTrials) {
                bool moreTrialsNeeded = (optimizedForOpaqueInput > 0 && optimizedForOpaqueInput < kExpectedSuccesses)
                    || (optimizedForCoverageAsAlpha > 0 && optimizedForCoverageAsAlpha < kExpectedSuccesses)
                    || (optimizedForConstantOutputForInput > 0 && optimizedForConstantOutputForInput < kExpectedSuccesses);
                if (!moreTrialsNeeded)
                    break;
            ...
            if (!fp->hasConstantOutputForConstantInput() && !fp->preservesOpaqueInput() && !fp->compatibleWithCoverageAsAlpha()) {
                continue;
            }
            ...
            if (fp->compatibleWithCoverageAsAlpha()) {
                ...
                render_fp(context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture2), readData2.data());
                render_fp(context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture3), readData3.data());
            ...
            render_fp(context, sdc.get(), fpGenerator.make(i, /*randomTreeDepth=*/1, inputTexture1), readData1.data());
            ...
            static const int kMaxAcceptableFailedPixels = CurrentTestHarnessIsSkQP() ? 0 : // Strict when running as SKQP
                ...
                for (int x = 0; x < kRenderSize; ++x) {
                    ...
                    if (fp->compatibleWithCoverageAsAlpha()) {
                        ...
                        if (!legal_modulation(ins, outs)) {
                            ...
                            if (coverageMessage.isEmpty()) {
                                coverageMessage.printf("\"Modulating\" processor did not match alpha-modulation "
                    ...
                    if (fp->hasConstantOutputForConstantInput(input4f, &expected4f)) {
                        ...
                        if (rDiff > kTol || gDiff > kTol || bDiff > kTol || aDiff > kTol) {
                            if (constMessage.isEmpty()) {
                                ...
                                constMessage.printf("Processor claimed output for const input doesn't match "
                                                    "actual output.\n"
                                                    "Error: %f, Tolerance: %f, input: (%f, %f, %f, %f), "
                                                    "actual: (%f, %f, %f, %f), expected(%f, %f, %f, %f).",
                                    std::max(rDiff, std::max(gDiff, std::max(bDiff, aDiff))), kTol, input4f.fR, input4f.fG, input4f.fB, input4f.fA, output4f.fR,
                                    output4f.fG, output4f.fB, output4f.fA, expected4f.fR, expected4f.fG, expected4f.fB, expected4f.fA);
                            }
                        }
                    }
                    if (input4f.isOpaque() && fp->preservesOpaqueInput() && !output4f.isOpaque()) {
                        ...
                        if (opaqueMessage.isEmpty()) {
                            opaqueMessage.printf("Processor claimed opaqueness is preserved but "
            ...
            if (failedPixelCount > kMaxAcceptableFailedPixels) {
                ERRORF(reporter,
                    ...
                    failedPixelCount, kRenderSize * kRenderSize, fpGenerator.initialSeed(), fp->dumpTreeInfo().c_str());
                ...
                INFOF(reporter,
                    ...
                    "0x%08x, processor: %s",
                    failedPixelCount, kRenderSize * kRenderSize, fpGenerator.initialSeed(), fp->dumpInfo().c_str());
                if (!coverageMessage.isEmpty()) {
...
static void assert_processor_equality(skiatest::Reporter* reporter, const GrFragmentProcessor& fp, const GrFragmentProcessor& clone)
{
    REPORTER_ASSERT(reporter, !strcmp(fp.name(), clone.name()), "\n%s", fp.dumpTreeInfo().c_str());
    REPORTER_ASSERT(reporter, fp.compatibleWithCoverageAsAlpha() == clone.compatibleWithCoverageAsAlpha(), "\n%s", fp.dumpTreeInfo().c_str());
    REPORTER_ASSERT(reporter, fp.isEqual(clone), "\n%s", fp.dumpTreeInfo().c_str());
    REPORTER_ASSERT(reporter, fp.preservesOpaqueInput() == clone.preservesOpaqueInput(), "\n%s", fp.dumpTreeInfo().c_str());
    REPORTER_ASSERT(reporter, fp.hasConstantOutputForConstantInput() == clone.hasConstantOutputForConstantInput(), "\n%s", fp.dumpTreeInfo().c_str());
...
static bool verify_identical_render(
    skiatest::Reporter* reporter, int renderSize, const char* processorType, const GrColor readData1[], const GrColor readData2[])
{
    ...
    // separate problem.
    static const int maxAcceptableFailedPixels = CurrentTestHarnessIsSkQP() ? 0 : // Strict when running as SKQP
        ...
        for (int x = 0; x < renderSize; ++x, ++idx) {
            ...
            if (failedPixelCount > maxAcceptableFailedPixels) {
                ...
                ERRORF(reporter,
                    ...
                    processorType, firstWrongX, firstWrongY, input_texel_color(x, y, 0.0f), readData1[idx], readData2[idx]);
...
static void log_clone_failure(skiatest::Reporter* reporter, int renderSize, GrDirectContext* context, const GrSurfaceProxyView& inputTexture,
    GrColor pixelsFP[], GrColor pixelsClone[], GrColor pixelsRegen[])
{
    ...
    if (log_texture_view(context, inputTexture, &inputURL) && log_pixels(pixelsFP, renderSize, &origURL) && log_pixels(pixelsClone, renderSize, &cloneURL)
...
// progenitor.
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(ProcessorCloneTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    GrDirectContext* context = ctxInfo.directContext();
    GrResourceProvider* resourceProvider = context->priv().resourceProvider();
    ...
    GrSurfaceProxyView inputTexture = make_input_texture(context, kRenderSize, kRenderSize, inputPixels.data());
    ...
    for (int i = 0; i < GrFragmentProcessorTestFactory::Count(); ++i) {
        ...
        for (int j = 0; j < kTimesToInvokeFactory; ++j) {
            fpGenerator.reroll();
            std::unique_ptr<GrFragmentProcessor> fp = fpGenerator.make(i, /*randomTreeDepth=*/1, /*inputFP=*/nullptr);
            std::unique_ptr<GrFragmentProcessor> regen = fpGenerator.make(i, /*randomTreeDepth=*/1, /*inputFP=*/nullptr);
            ...
            if (!verify_identical_render(reporter, kRenderSize, "Processor clone", readDataFP.data(), readDataClone.data())) {
                ...
                if (!verify_identical_render(reporter, kRenderSize, "Regenerated processor", readDataFP.data(), readDataRegen.data())) {
                ...
                if (!loggedFirstFailure) {
                    log_clone_failure(reporter, kRenderSize, context, inputTexture, readDataFP.data(), readDataClone.data(), readDataRegen.data());
                    loggedFirstFailure = true;
                }
            }
        }
