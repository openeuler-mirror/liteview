...
public:
    template <typename T> static void TestAlignment(T* testObj, skiatest::Reporter* reporter)
    {
...
template <typename T, bool testInvalid> static void TestObjectSerializationNoAlign(T* testObj, skiatest::Reporter* reporter)
{
...
template <typename T> static void TestObjectSerialization(T* testObj, skiatest::Reporter* reporter)
{
    TestObjectSerializationNoAlign<T, false>(testObj, reporter);
    SerializationTest::TestAlignment(testObj, reporter);
}

template <typename T> static T* TestFlattenableSerialization(T* testObj, bool shouldSucceed, skiatest::Reporter* reporter)
{
    SkBinaryWriteBuffer writer;
    SerializationUtils<T>::Write(writer, testObj);
    size_t bytesWritten = writer.bytesWritten();
...
template <typename T> static void TestArraySerialization(T* data, skiatest::Reporter* reporter)
{
    SkBinaryWriteBuffer writer;
...
static void TestBitmapSerialization(const SkBitmap& validBitmap, const SkBitmap& invalidBitmap, bool shouldSucceed, skiatest::Reporter* reporter)
{
    ...
    sk_sp<SkImageFilter> xfermodeImageFilter(
        SkImageFilters::Blend(SkBlendMode::kSrcOver, std::move(invalidBitmapSource), std::move(validBitmapSource), nullptr));

    sk_sp<SkImageFilter> deserializedFilter(
        TestFlattenableSerialization<SkImageFilter_Base>((SkImageFilter_Base*)xfermodeImageFilter.get(), shouldSucceed, reporter));
...
{
    SkBitmap bitmap;
    bitmap.allocN32Pixels(SkScalarCeilToInt(picture.cullRect().width()), SkScalarCeilToInt(picture.cullRect().height()));
    SkCanvas canvas(bitmap);
...
static void serialize_and_compare_typeface(
    sk_sp<SkTypeface> typeface, const char* text, const SkSerialProcs* serial_procs, const SkDeserialProcs* deserial_procs, skiatest::Reporter* reporter)
{
    ...
    SkCanvas* canvas = recorder.beginRecording(SkIntToScalar(canvasRect.width()), SkIntToScalar(canvasRect.height()));
...
{
    {
        ...
        if (!typeface) {
            ...
            serialize_and_compare_typeface(std::move(typeface), "A!", serial_procs, deserial_procs, reporter);
        }
    }

    {
        ...
        if (!typeface) {
            ...
            serialize_and_compare_typeface(std::move(typeface), "ab", serial_procs, deserial_procs, reporter);
        }
    }
}
...
static void TestTypefaceSerialization(skiatest::Reporter* reporter, sk_sp<SkTypeface> typeface)
{
    ...
    REPORTER_ASSERT(
        reporter, typeface->countGlyphs() == cloneTypeface->countGlyphs(), "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(), cloneName.c_str());
    REPORTER_ASSERT(reporter, typeface->fontStyle() == cloneTypeface->fontStyle(), "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(), cloneName.c_str());
    ...
    REPORTER_ASSERT(reporter, fontMetrics == cloneMetrics, "Typeface: \"%s\" CloneTypeface: \"%s\"", name.c_str(), cloneName.c_str());
}
...
{
    auto surf = SkSurface::MakeRasterN32Premul(SkScalarRoundToInt(p.cullRect().width()), SkScalarRoundToInt(p.cullRect().height()));
    if (!surf) {
...
DEF_TEST(Serialization, reporter)
{
    ...
    // Test readColor4fArray
    {
        SkColor4f data[kArraySize]
            = { SkColor4f::FromColor(SK_ColorBLACK), SkColor4f::FromColor(SK_ColorWHITE), SkColor4f::FromColor(SK_ColorRED), { 1.f, 2.f, 4.f, 8.f } };
        TestArraySerialization(data, reporter);
    }
