...
{
    std::string originalPathStr = "M5.93 -3.12C5.93 -5.03 4.73 -6.06 3.5 -6.06C2.67 -6.06 1.98 -5.59 1.76 -5.34L1.67 "
...
{
    std::string originalPathStr = "M4 0"
                                  "L0 0"
                                  "L0 5"
                                  "L4 4"
                                  "Z"
...
DEF_TEST(PathOpsAsWinding, reporter)
{
    ...
    for (bool aFirst : { false, true }) {
        for (auto dirA : { SkPathDirection::kCW, SkPathDirection::kCCW }) {
            for (auto dirB : { SkPathDirection::kCW, SkPathDirection::kCCW }) {
                for (auto curveA : { SkPath::kLine_Verb, SkPath::kQuad_Verb, SkPath::kConic_Verb, SkPath::kCubic_Verb }) {
                    SkPath pathA = build_squircle(curveA, rectA, dirA);
                    for (auto curveB : { SkPath::kLine_Verb, SkPath::kQuad_Verb, SkPath::kConic_Verb, SkPath::kCubic_Verb }) {
