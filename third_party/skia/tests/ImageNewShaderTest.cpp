...
static void paint_source(SkSurface* sourceSurface)
{
    ...
    SkRect rect = SkRect::MakeXYWH(SkIntToScalar(1), SkIntToScalar(0), SkIntToScalar(1), SkIntToScalar(sourceSurface->height()));
...
static void run_shader_test(skiatest::Reporter* reporter, SkSurface* sourceSurface, SkSurface* destinationSurface, SkImageInfo& info)
{
    ...
    sk_sp<SkShader> sourceShader = sourceImage->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions());
    ...
    sk_sp<SkShader> sourceShaderTranslated = sourceImage->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), &matrix);
