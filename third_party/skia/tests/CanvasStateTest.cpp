...
DEF_TEST(CanvasState_test_complex_layers, reporter)
{
    ...
    SkRect rect
        = SkRect::MakeXYWH(SkIntToScalar(SPACER), SkIntToScalar(SPACER), SkIntToScalar(WIDTH - (2 * SPACER)), SkIntToScalar((HEIGHT - (2 * SPACER)) / 7));
    ...
    *(void**)(&drawFn) = openLibResult.procAddress("complex_layers_draw_from_canvas_state");
    ...
    for (size_t i = 0; i < std::size(colorTypes); ++i) {
        ...
        for (int j = 0; j < 2; ++j) {
            auto surf = SkSurface::MakeRaster(SkImageInfo::Make(WIDTH, HEIGHT, colorTypes[i], kPremul_SkAlphaType));
            ...
            for (size_t k = 0; k < std::size(layerAlpha); ++k) {
                ...
                if (j) {
                    ...
                    bool success = complex_layers_draw_from_canvas_state(state, rect.fLeft, rect.fTop, rect.fRight, rect.fBottom, SPACER);
...
DEF_TEST(CanvasState_test_complex_clips, reporter)
{
    ...
    bool (*drawFn)(SkCanvasState* state, int32_t l, int32_t t, int32_t r, int32_t b, int32_t clipOp, int32_t regionRects, int32_t* rectCoords);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        for (size_t j = 0; j < std::size(clipOps); ++j) {
            ...
            if (i) {
                ...
                bool success
                    = drawFn(state, clipRect.fLeft, clipRect.fTop, clipRect.fRight, clipRect.fBottom, clipOps[j], rectCoords.size() / 4, rectCoords.begin());
                REPORTER_ASSERT(reporter, success);

                SkCanvasStateUtils::ReleaseCanvasState(state);
            } else {
                complex_clips_draw(canvas, clipRect.fLeft, clipRect.fTop, clipRect.fRight, clipRect.fBottom, clipOps[j], localRegion);
            }
