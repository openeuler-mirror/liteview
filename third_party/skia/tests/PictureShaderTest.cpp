...
DEF_TEST(PictureShader_caching, reporter)
{
    ...
    {
        SkPaint paint;
        paint.setShader(picture->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest));
    ...
    {
        SkPaint paint;
        paint.setShader(makePicture()->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kNearest));
        surface->getCanvas()->drawPaint(paint);
    }
...
DEF_TEST(PictureShader_caching2, reporter)
{
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRasterN32Premul(100, 100);
    for (SkTileMode m : { SkTileMode::kClamp, SkTileMode::kRepeat, SkTileMode::kRepeat, SkTileMode::kDecal }) {
        SkPaint paint;
