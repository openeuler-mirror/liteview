...
static bool nonsampler_params_invalid(const GrGLTextureParameters& parameters)
{
    ...
    return nsState.fBaseMipMapLevel == invalidNSState.fBaseMipMapLevel && nsState.fMaxMipmapLevel == invalidNSState.fMaxMipmapLevel
...
DEF_GANESH_TEST_FOR_ALL_GL_CONTEXTS(GLTextureParameters, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    auto dContext = ctxInfo.directContext();
    auto caps = static_cast<const GrGLCaps*>(dContext->priv().caps());

    GrBackendTexture backendTex = dContext->createBackendTexture(1, 1, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo);
    ...
    sk_sp<SkImage> wrappedImage
        = SkImage::MakeFromTexture(dContext, backendTex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr);
    ...
    auto surf = SkSurface::MakeRenderTarget(dContext, SkBudgeted::kYes, SkImageInfo::Make(1, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType), 1, nullptr);
