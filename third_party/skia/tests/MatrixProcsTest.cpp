...
DEF_TEST(MatrixProcs_pack_clamp, r)
{
    ...
    for (size_t i = 0; i < NUM_TESTS; i++) {
        ...
        REPORTER_ASSERT(r, rv == tc.expectedOutput, "%s | %x != %x | {%x, %x, %x} != {%x, %x, %x}\n", tc.name.c_str(), rv, exp, highBits(rv), middleBits(rv),
            lowBits(rv), highBits(exp), middleBits(exp), lowBits(exp));
    }
}
...
DEF_TEST(MatrixProcs_unpack_int, r)
{
    ...
    for (size_t i = 0; i < NUM_TESTS; i++) {
        ...
        REPORTER_ASSERT(r, lower == tc.expectedLowerBound, "%s lower %x != %x", tc.name.c_str(), lower, tc.expectedLowerBound);
        REPORTER_ASSERT(r, lerp == tc.expectedLerp, "%s lerp %x != %x", tc.name.c_str(), lerp, tc.expectedLerp);
        REPORTER_ASSERT(r, upper == tc.expectedUpperBound, "%s upper %x != %x", tc.name.c_str(), upper, tc.expectedUpperBound);
