...
void TestReadPixels(skiatest::Reporter*, GrDirectContext*, skgpu::v1::SurfaceContext*, uint32_t expectedPixelValues[], const char* testName);

// See if trying to write RGBA 8888 pixels to 'dstContext' matches the
// expectation ('expectedToWork')
void TestWritePixels(skiatest::Reporter*, GrDirectContext*, skgpu::v1::SurfaceContext*, bool expectedToWork, const char* testName);
...
void TestCopyFromSurface(skiatest::Reporter*, GrDirectContext*, sk_sp<GrSurfaceProxy> proxy, GrSurfaceOrigin origin, GrColorType colorType,
...
std::unique_ptr<skgpu::v1::SurfaceContext> CreateSurfaceContext(GrRecordingContext*, const GrImageInfo&, SkBackingFit = SkBackingFit::kExact,
    GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, GrRenderable = GrRenderable::kNo, int sampleCount = 1, GrMipmapped = GrMipmapped::kNo,
