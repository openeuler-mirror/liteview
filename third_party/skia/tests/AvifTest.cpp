...
static void run_avif_test(skiatest::Reporter* r, const AvifTestCase& t)
{
    ...
    SkImageInfo imageInfo = SkImageInfo::Make(t.bitmapWidth, t.bitmapHeight, t.color_type, t.alpha_type);
    SkBitmap bm;
    ...
    for (int i = 0; i < codec->getFrameCount(); i++) {
        ...
        if (result != SkCodec::kSuccess) {
            ERRORF(r, "Failed to decode %s frame %i - error %s", t.path, i, SkCodec::ResultToString(result));
            return;
        }
