...
const SkScalar widths[] = { -FLT_MAX, -1, -0.1f, -FLT_EPSILON, 0, FLT_EPSILON, 0.0000001f, 0.000001f, 0.00001f, 0.0001f, 0.001f, 0.01f, 0.1f, 0.2f, 0.3f, 0.4f,
    0.5f, 1, 1.1f, 2, 10, 10e2f, 10e3f, 10e4f, 10e5f, 10e6f, 10e7f, 10e8f, 10e9f, 10e10f, 10e20f, FLT_MAX };
...
static void cubicSetTest(const CubicPts* dCubic, size_t count)
{
    ...
    for (size_t index = 0; index < count; ++index) {
        ...
        SkPoint c[4] = { { (float)d[0].fX, (float)d[0].fY }, { (float)d[1].fX, (float)d[1].fY }, { (float)d[2].fX, (float)d[2].fY },
...
static void quadSetTest(const QuadPts* dQuad, size_t count)
{
    ...
    for (size_t index = 0; index < count; ++index) {
        ...
        SkDQuad d;
        d.debugSet(dPts.fPts);
        SkPoint c[3] = { { (float)d[0].fX, (float)d[0].fY }, { (float)d[1].fX, (float)d[1].fY }, { (float)d[2].fX, (float)d[2].fY } };
        quadTest(c);
        if (FLAGS_timeout && timer.elapsedMs() > MS_TEST_DURATION) {
            return;
        }
...
DEF_TEST(CubicStrokerUnbounded, reporter)
{
    ...
    for (int i = 0; i < 1000000; ++i) {
        SkPath path, fill;
        path.moveTo(unbounded(r), unbounded(r));
        path.cubicTo(unbounded(r), unbounded(r), unbounded(r), unbounded(r), unbounded(r), unbounded(r));
        p.setStrokeWidth(unboundedPos(r));
        p.getFillPath(path, &fill);
#if defined(SK_DEBUG) && QUAD_STROKE_APPROX_EXTENDED_DEBUGGING
        if (bestTan < gMaxRecursion[0] || bestCubic < gMaxRecursion[1]) {
            if (reporter->verbose()) {
                SkDebugf("\n%s tan=%d cubic=%d width=%1.9g\n", __FUNCTION__, gMaxRecursion[0], gMaxRecursion[1], p.getStrokeWidth());
                path.dumpHex();
                SkDebugf("fill:\n");
                fill.dumpHex();
            }
...
DEF_TEST(QuadStrokerConstrained, reporter)
{
    ...
    for (int i = 0; i < 1000000; ++i) {
        ...
        } while (SkPointPriv::DistanceToSqd(quad[0], quad[2]) < halfSquared || SkPointPriv::DistanceToSqd(quad[1], quad[2]) < halfSquared);
...
DEF_TEST(CubicStrokerConstrained, reporter)
{
    ...
    for (int i = 0; i < 1000000; ++i) {
        ...
        } while (SkPointPriv::DistanceToSqd(cubic[0], cubic[2]) < halfSquared || SkPointPriv::DistanceToSqd(cubic[1], cubic[2]) < halfSquared);
        do {
            cubic[3].fX = r.nextRangeF(0, 500);
            cubic[3].fY = r.nextRangeF(0, 500);
        } while (SkPointPriv::DistanceToSqd(cubic[0], cubic[3]) < halfSquared || SkPointPriv::DistanceToSqd(cubic[1], cubic[3]) < halfSquared
...
DEF_TEST(CubicStrokerRange, reporter)
{
    ...
    for (int i = 0; i < 1000000; ++i) {
        ...
        path.cubicTo(r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500), r.nextRangeF(0, 500));
...
DEF_TEST(CubicStrokerOneOff, reporter)
{
    ...
    path.cubicTo(SkBits2Float(0x4331cb76), SkBits2Float(0x43ea3340), SkBits2Float(0x4388f498), SkBits2Float(0x42f7f08d), SkBits2Float(0x43f1cd32),
        SkBits2Float(0x42802ec1));
    p.getFillPath(path, &fill);
    if (reporter->verbose()) {
        SkDebugf("\n%s path\n", __FUNCTION__);
        path.dump();
