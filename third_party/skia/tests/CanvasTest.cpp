...
struct ClipRectVisitor {
    ...
    template <typename T> SkRect operator()(const T&)
    {
...
{
    multi_canvas_driver(gBaseRestrictedR.width(), gBaseRestrictedR.height(), [reporter](SkCanvas* canvas) { test_restriction(reporter, canvas); });
}
...
static CanvasTest kCanvasTests[] = { [](SkCanvas* c, skiatest::Reporter* r) { c->translate(SkIntToScalar(1), SkIntToScalar(2)); },
    ...
    [](SkCanvas* c, skiatest::Reporter* r) { c->setMatrix(SkMatrix::Scale(2, 3)); }, [](SkCanvas* c, skiatest::Reporter* r) { c->clipRect(kRect); },
    ...
    [](SkCanvas* c, skiatest::Reporter* r) {
        ...
        c->drawVertices(SkVertices::MakeCopy(SkVertices::kTriangleFan_VertexMode, 4, pts, pts, nullptr), SkBlendMode::kModulate, paint);
    } };
...
protected:
    bool onFilter(SkPaint&) const override
    {
        return true;
    }
...
public:
    LifeLineCanvas(int w, int h, bool* lifeline)
        : SkCanvas(w, h)
        , fLifeLine(lifeline)
    {
        *fLifeLine = true;
...
public:
    ...
    {
        return sk_sp<SkImageFilter>(new ZeroBoundsImageFilter);
    }
...
protected:
    ...
    SkIRect onFilterNodeBounds(const SkIRect&, const SkMatrix&, MapDirection, const SkIRect* inputRect) const override
    {
