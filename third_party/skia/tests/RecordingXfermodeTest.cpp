...
public:
    ...
    void draw(SkCanvas* canvas, const SkRect& clipRect, SkBlendMode mode) const
    {
        ...
        canvas->drawImageRect(fCircleImage, SkRect::MakeXYWH(20, 20, 60, 60), SkSamplingOptions(), &blackPaint);
        canvas->restore();
        canvas->restore();
    }
...
public:
    ...
    virtual const SkBitmap& recordAndReplay(const Drawer& drawer, const SkRect& intoClip, SkBlendMode) = 0;
};
...
public:
    ...
    const SkBitmap& recordAndReplay(const Drawer& drawer, const SkRect& intoClip, SkBlendMode mode) override
    {
...
public:
    ...
    const SkBitmap& recordAndReplay(const Drawer& drawer, const SkRect& intoClip, SkBlendMode mode) override
    {
        ...
        SkCanvas* canvas = recorder.beginRecording(SkIntToScalar(fWidth), SkIntToScalar(fHeight), &factory);
