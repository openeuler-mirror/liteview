...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(MatrixColorFilter_TransparentBlack, reporter, info, CtsEnforcement::kApiLevel_T)
{
    ...
    auto imgSurf = SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, SkImageInfo::MakeN32(5, 5, kPremul_SkAlphaType));
    ...
    auto surf = SkSurface::MakeRenderTarget(context, SkBudgeted::kYes, SkImageInfo::MakeN32(5, 5, kPremul_SkAlphaType));
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([reporter](int x, int y, const float diffs[4]) {
        ERRORF(reporter, "Expected transparent black, instead got (%f, %f, %f, %f)", diffs[0], diffs[1], diffs[2], diffs[3]);
    });
