...
DEF_TEST(FontMgr_AliasNames, reporter)
{
    ...
    for (size_t i = 0; i < std::size(inNames); ++i) {
        ...
        for (int j = 0; j < 10; ++j) {
            ...
            REPORTER_ASSERT(reporter, first->uniqueID() == face->uniqueID(), "Request \"%s\" First Name: \"%s\" Id: %x Received Name \"%s\" Id %x", inNames[i],
...
DEF_TEST(FontMgr_MatchStyleCSS3, reporter)
{
    ...
    public:
        TestTypeface(const SkFontStyle& fontStyle)
            : SkTypeface(fontStyle, false)
        {
        }
    ...
    struct StyleSetTest {
        ...
        {
            { condensed_normal_100, condensed_normal_900, condensed_italic_100, condensed_italic_900, expanded_normal_100, expanded_normal_900,
