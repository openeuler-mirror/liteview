...
DEF_TEST(Encode_JPG, r)
{
    ...
    for (auto ct :
        { kRGBA_8888_SkColorType, kBGRA_8888_SkColorType, kRGB_565_SkColorType, kARGB_4444_SkColorType, kGray_8_SkColorType, kRGBA_F16_SkColorType }) {
        for (auto at : { kPremul_SkAlphaType, kUnpremul_SkAlphaType, kOpaque_SkAlphaType }) {
            ...
            for (auto alphaOption : { SkJpegEncoder::AlphaOption::kIgnore, SkJpegEncoder::AlphaOption::kBlendOnBlack }) {
                SkJpegEncoder::Options opts;
                opts.fAlphaOption = alphaOption;
                SkNullWStream ignored;
                if (!SkJpegEncoder::Encode(&ignored, bm.pixmap(), opts)) {
                    REPORTER_ASSERT(r, ct == kARGB_4444_SkColorType && alphaOption == SkJpegEncoder::AlphaOption::kBlendOnBlack);
                }
            }
        }
    }
...
static inline void pushComment(std::vector<std::string>& comments, const char* keyword, const char* text)
{
...
static void testPngComments(const SkPixmap& src, SkPngEncoder::Options& options, skiatest::Reporter* r)
{
    ...
    options.fComments = SkDataTable::MakeCopyArrays((void const* const*)commentPointers.data(), commentSizes.data(), commentStrings.size());
    ...
    auto search1 = std::search(output.begin(), output.end(), kExpected1, kExpected1 + sizeof(kExpected1));
    auto search2 = std::search(output.begin(), output.end(), kExpected2, kExpected2 + sizeof(kExpected2));
    auto search3 = std::search(output.begin(), output.end(), kExpected3, kExpected3 + sizeof(kExpected3));
    auto search4 = std::search(output.begin(), output.end(), longKeyRecord.begin(), longKeyRecord.end());
    auto search5 = std::search(output.begin(), output.end(), tooLongRecord.begin(), tooLongRecord.end());
...
DEF_TEST(Encode_WebpQuality, r)
{
    ...
    auto test = [&r](const sk_sp<SkData>& data, Format expected) {
        ...
        if (expected != features.format) {
            ERRORF(r, "Expected %s encode, but got format %s", printFormat(expected), printFormat(features.format));
        }
    };
