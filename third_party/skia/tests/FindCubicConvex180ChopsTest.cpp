    ...
    if (int inflectN = SkFindCubicInflections(p, inflectT)) {
        ...
        if (totalRotation < SK_ScalarPI - SK_ScalarNearlyZero) {
            ...
            if (totalRotation > SK_ScalarPI + SK_ScalarNearlyZero) {
                ...
                REPORTER_ASSERT(r, SkScalarNearlyEqual(SkMeasureNonInflectCubicRotation(chops), SK_ScalarPI));
                REPORTER_ASSERT(r, SkScalarNearlyEqual(SkMeasureNonInflectCubicRotation(chops + 3), totalRotation - SK_ScalarPI));
            }
...
DEF_TEST(FindCubicConvex180Chops, r)
{
    ...
    for (int i = 0; i < (1 << 8); ++i) {
        SkPoint p[4] = { SkPoint::Make((i >> 0) & 1, (i >> 1) & 1), SkPoint::Make((i >> 2) & 1, (i >> 3) & 1), SkPoint::Make((i >> 4) & 1, (i >> 5) & 1),
...

    {
        ...
        static const uint32_t hexPts[] = { 0x3ee0ac74, 0x3f1e061a, 0x3e0fc408, 0x3f457230, 0x3f42ac7c, 0x3f70d76c, 0x3f4e6520, 0x3f6acafa };
        SkPoint p[4];
