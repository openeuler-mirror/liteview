...
#define ACQUIRE_VK_PROC_NOCHECK(name, instance) PFN_vk##name grVk##name = reinterpret_cast<PFN_vk##name>(getProc("vk" #name, instance, VK_NULL_HANDLE))

#define ACQUIRE_VK_PROC(name, instance)                                                                                                                        \
    PFN_vk##name grVk##name = reinterpret_cast<PFN_vk##name>(getProc("vk" #name, instance, VK_NULL_HANDLE));                                                   \
    do {                                                                                                                                                       \
...
// doesn't crash.
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VulkanPriorityExtension, reporter, context_info, CtsEnforcement::kApiLevel_T)
{
    ...
    auto getProc = [instProc](const char* proc_name, VkInstance instance, VkDevice) { return instProc(instance, proc_name); };
