...
public:
    ...
    bool operator==(const TestClass& c) const
    {
...
template <typename T, bool MEM_MOVE> static void TestTSet_basic(skiatest::Reporter* reporter)
{
...
template <typename T, typename U> static void test_skstarray_compatibility(skiatest::Reporter* reporter)
{
...
template <typename T> static void test_swap(skiatest::Reporter* reporter, SkTArray<T>* (&arrays)[4], int (&sizes)[7])
{
    for (auto a : arrays) {
        for (auto b : arrays) {
            if (a == b) {
...
template <typename Array> static void test_array_reserve(skiatest::Reporter* reporter, Array* array, int reserveCount)
{
    SkRandom random;
