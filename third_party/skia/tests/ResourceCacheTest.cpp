...
{
    if (type == sk_gpu_test::GrContextFactory::kANGLE_D3D11_ES3_ContextType || type == sk_gpu_test::GrContextFactory::kANGLE_GL_ES3_ContextType
...
{
    return rt->getStencilAttachment();
}

static sk_sp<GrRenderTarget> create_RT_with_SB(GrResourceProvider* provider, int size, int sampleCount, SkBudgeted budgeted)
{
    auto format = provider->caps()->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kYes);
    sk_sp<GrTexture> tex(
        provider->createTexture({ size, size }, format, GrTextureType::k2D, GrRenderable::kYes, sampleCount, GrMipmapped::kNo, budgeted, GrProtected::kNo,
            /*label=*/ {}));
...
DEF_GANESH_TEST_FOR_CONTEXTS(ResourceCacheStencilBuffers, &is_rendering_and_not_angle_es3, reporter, ctxInfo, nullptr, CtsEnforcement::kApiLevel_T)
{
    ...
    if (smallSampleCount > 1) {
        ...
        sk_sp<GrRenderTarget> smallMSAART0 = create_RT_with_SB(resourceProvider, 4, smallSampleCount, SkBudgeted::kNo);
...

        {
            ...
            sk_sp<GrRenderTarget> smallMSAART1 = create_RT_with_SB(resourceProvider, 4, smallSampleCount, SkBudgeted::kNo);
        ...
        if (bigSampleCount > 0 && bigSampleCount != smallSampleCount) {
            sk_sp<GrRenderTarget> smallMSAART2 = create_RT_with_SB(resourceProvider, 4, bigSampleCount, SkBudgeted::kNo);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ResourceCacheWrappedResources, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(context, kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    GrBackendTexture unmbet = context->createBackendTexture(kW, kH, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    ...
    sk_sp<GrTexture> borrowed(resourceProvider->wrapBackendTexture(mbet->texture(), kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType));

    sk_sp<GrTexture> adopted(resourceProvider->wrapBackendTexture(unmbet, kAdopt_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType));
...
public:
    ...
    TestResource(GrGpu* gpu, std::string_view label, SkBudgeted budgeted = SkBudgeted::kYes, size_t size = kDefaultSize)
        : INHERITED(gpu, label)
        , fToDelete(nullptr)
        , fSize(size)
    ...
    static TestResource* CreateScratch(GrGpu* gpu, SkBudgeted budgeted, SimulatedProperty property, size_t size = kDefaultSize)
    {
...
private:
    ...
    TestResource(GrGpu* gpu, SkBudgeted budgeted, SimulatedProperty property, ScratchConstructor, std::string_view label, size_t size = kDefaultSize)
        : INHERITED(gpu, label)
        , fToDelete(nullptr)
        , fSize(size)
        , fProperty(property)
...
public:
    ...
    {
        return fDContext->priv().getResourceCache();
    }
    GrGpu* gpu()
    {
        return fDContext->priv().getGpu();
    }
    GrDirectContext* dContext()
    {
        return fDContext.get();
    }
...
template <int> static void make_unique_key(skgpu::UniqueKey* key, int data, const char* tag = nullptr)
{
...
static void test_purge_unlocked(skiatest::Reporter* reporter)
{
    ...
    TestResource* a = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 11);

    skgpu::UniqueKey uniqueKey;
    make_unique_key<0>(&uniqueKey, 0);

    TestResource* b = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 12);
    b->resourcePriv().setUniqueKey(uniqueKey);

    TestResource* c = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 13);

    skgpu::UniqueKey uniqueKey2;
    make_unique_key<0>(&uniqueKey2, 1);

    TestResource* d = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 14);
...
static void test_budgeting(skiatest::Reporter* reporter)
{
    ...
    TestResource* scratch = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 10);
    ...
    REPORTER_ASSERT(reporter,
        scratch->gpuMemorySize() + unique->gpuMemorySize() + wrappedCacheable->gpuMemorySize() + wrappedUncacheable->gpuMemorySize()
    ...
    REPORTER_ASSERT(reporter, 4 == cache->getResourceCount());
    REPORTER_ASSERT(reporter,
        scratch->gpuMemorySize() + unique->gpuMemorySize() + wrappedCacheable->gpuMemorySize() + unbudgeted->gpuMemorySize() == cache->getResourceBytes());
    REPORTER_ASSERT(reporter, 0 == cache->getPurgeableBytes());
    ...
    REPORTER_ASSERT(reporter,
        scratch->gpuMemorySize() + wrappedCacheable->gpuMemorySize() + wrappedUncacheable->gpuMemorySize() + unbudgeted->gpuMemorySize()
            == cache->getResourceBytes());
    ...
    REPORTER_ASSERT(reporter, 3 == cache->getResourceCount());
    REPORTER_ASSERT(
        reporter, unbudgeted->gpuMemorySize() + wrappedCacheable->gpuMemorySize() + wrappedUncacheable->gpuMemorySize() == cache->getResourceBytes());
...
static void test_unbudgeted(skiatest::Reporter* reporter)
{
    ...
    scratch = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 10);
...
/*static*/ void test_unbudgeted_to_scratch(skiatest::Reporter* reporter)
{
    ...
    TestResource* resource = TestResource::CreateScratch(gpu, SkBudgeted::kNo, TestResource::kA_SimulatedProperty);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        REPORTER_ASSERT(reporter, GrBudgetedType::kUnbudgetedUncacheable == resource->resourcePriv().budgetedType());
...
static void test_duplicate_scratch_key(skiatest::Reporter* reporter)
{
    ...
    TestResource* a = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 11);
    TestResource* b = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 12);
    ...
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 0 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 2 == cache->getResourceCount());
    REPORTER_ASSERT(reporter, a->gpuMemorySize() + b->gpuMemorySize() == cache->getResourceBytes());
...
static void test_remove_scratch_key(skiatest::Reporter* reporter)
{
    ...
    // Create two resources that have the same scratch key.
    TestResource* a = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty);
    TestResource* b = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty);
    a->unref();
    b->unref();
    ...
    TestResource::ComputeScratchKey(TestResource::kB_SimulatedProperty, &scratchKey);
    REPORTER_ASSERT(reporter, 2 == TestResource::NumAlive());
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 2 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 2 == cache->getResourceCount());
    ...
    REPORTER_ASSERT(reporter, 2 == TestResource::NumAlive());
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 1 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 2 == cache->getResourceCount());

    // The cache should immediately delete it when it's unrefed since it isn't accessible.
    find->unref();
    REPORTER_ASSERT(reporter, 1 == TestResource::NumAlive());
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 1 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 1 == cache->getResourceCount());

    // Repeat for the second resource.
    find = cache->findAndRefScratchResource(scratchKey);
    find->resourcePriv().removeScratchKey();
    REPORTER_ASSERT(reporter, 1 == TestResource::NumAlive());
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 0 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 1 == cache->getResourceCount());
    ...
    find->unref();
    REPORTER_ASSERT(reporter, 0 == TestResource::NumAlive());
    SkDEBUGCODE(REPORTER_ASSERT(reporter, 0 == cache->countScratchEntriesForKey(scratchKey));) REPORTER_ASSERT(reporter, 0 == cache->getResourceCount());
}
...
static void test_purge_invalidated(skiatest::Reporter* reporter)
{
    ...
    TestResource* c = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty);
...
static void test_timestamp_wrap(skiatest::Reporter* reporter)
{
    ...
    for (int i = 0; i < 2 * kCount; ++i) {
        ...
        for (int j = 0; j < kCount; ++j) {
            ...
            if (currShouldPurgeIdx < shouldPurgeIdxs.size() && shouldPurgeIdxs[currShouldPurgeIdx] == j) {
                ++currShouldPurgeIdx;
...
static void test_time_purge(skiatest::Reporter* reporter)
{
    ...
    for (int cnt : kCnts) {
        std::unique_ptr<GrStdSteadyClock::time_point[]> timeStamps(new GrStdSteadyClock::time_point[cnt]);
        {
        ...
        // then purge old scratch resources.
        {
            for (int i = 0; i < cnt; ++i) {
                ...
                TestResource* r = isScratch ? TestResource::CreateScratch(gpu, budgeted, property) : new TestResource(gpu, /*label=*/ {}, budgeted, property);
                if (!isScratch) {
                    skgpu::UniqueKey k;
                    make_unique_key<1>(&k, i);
                    r->resourcePriv().setUniqueKey(k);
...
static void test_partial_purge(skiatest::Reporter* reporter)
{
    ...
    for (int testCase = 0; testCase < kEndTests_TestCase; testCase++) {
        ...
        TestResource* scratch1 = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kA_SimulatedProperty, 13);
        TestResource* scratch2 = TestResource::CreateScratch(gpu, SkBudgeted::kYes, TestResource::kB_SimulatedProperty, 14);
...
static void test_free_texture_messages(skiatest::Reporter* reporter)
{
    ...
    for (int i = 0; i < 3; ++i) {
        backends[i] = dContext->createBackendTexture(16, 16, SkColorType::kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
        wrapped[i] = gpu->wrapBackendTexture(
            backends[i], GrWrapOwnership::kBorrow_GrWrapOwnership, (i < 2) ? GrWrapCacheable::kYes : GrWrapCacheable::kNo, GrIOType::kRead_GrIOType);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ResourceMessagesAfterAbandon, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    GrBackendTexture backend = dContext->createBackendTexture(16, 16, SkColorType::kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    sk_sp<GrTexture> tex = gpu->wrapBackendTexture(backend, GrWrapOwnership::kBorrow_GrWrapOwnership, GrWrapCacheable::kYes, GrIOType::kRead_GrIOType);
...
static sk_sp<GrTexture> make_normal_texture(GrResourceProvider* provider, GrRenderable renderable, SkISize dims, int sampleCnt)
{
    ...
    return provider->createTexture(dims, format, GrTextureType::k2D, renderable, sampleCnt, GrMipmapped::kNo, SkBudgeted::kYes, GrProtected::kNo,
        /*label=*/ {});
}
...
static sk_sp<GrTextureProxy> make_mipmap_proxy(GrRecordingContext* rContext, GrRenderable renderable, SkISize dims, int sampleCnt)
{
    ...
    const GrBackendFormat format = caps->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kNo);

    return proxyProvider->createProxy(format, dims, renderable, sampleCnt, GrMipmapped::kYes, SkBackingFit::kExact, SkBudgeted::kYes, GrProtected::kNo,
        /*label=*/ {});
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(PurgeToMakeHeadroom, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (bool success : { true, false }) {
        ...
        REPORTER_ASSERT(reporter, expectedPurgeable == resourceCache->getPurgeableBytes(), "%zu vs %zu", expectedPurgeable, resourceCache->getPurgeableBytes());
        ...
        REPORTER_ASSERT(
            reporter, expectedBudgeted == resourceCache->getBudgetedResourceBytes(), "%zu vs %zu", expectedBudgeted, resourceCache->getBudgetedResourceBytes());
        reporter->pop();
    }
}
...
DEF_GANESH_TEST_FOR_MOCK_CONTEXT(OverbudgetFlush, reporter, ctxInfo)
{
    ...
    auto drawToSurf = [](SkSurface* surf) { surf->getCanvas()->drawRect(SkRect::MakeWH(1, 1), SkPaint()); };
