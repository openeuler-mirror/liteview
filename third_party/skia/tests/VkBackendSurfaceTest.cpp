...
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VkDRMModifierTest, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(dContext, 1, 1, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    if (!mbet) {
    ...
    sk_sp<SkImage> drmImage = SkImage::MakeFromTexture(dContext, drmBETex, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr);
    REPORTER_ASSERT(reporter, drmImage);

    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(drmImage->getBackendTexture(false), drmBETex));
...
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VkImageLayoutTest, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    sk_sp<SkImage> wrappedImage = SkImage::MakeFromTexture(dContext, backendTex1, kTopLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
