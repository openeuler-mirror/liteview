...
DEF_TEST(srgb_roundtrip, r)
{
    ...
    SkColorSpaceXformSteps linearize { sRGB.get(), upm, linear.get(), upm }, reencode { linear.get(), upm, sRGB.get(), upm };
...
DEF_TEST(srgb_roundtrip_extended, r)
{
    ...
    SkColorSpaceXformSteps linearize { cs.get(), upm, linear.get(), upm }, reencode { linear.get(), upm, cs.get(), upm };
