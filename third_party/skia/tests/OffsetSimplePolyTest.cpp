...
DEF_TEST(OffsetSimplePoly, reporter)
{
    ...
    *rrectPoly.append() = SkPoint::Make(-100 - 2.5f, 50 + 4.330127f);
    ...
    bool result = SkOffsetSimplePolygon(rrectPoly.begin(), rrectPoly.size(), bounds, 3, &offsetPoly);
    ...
    *clippedRRectPoly.append() = SkPoint::Make(342.289948f, 432.947998f);
    ...
    REPORTER_ASSERT(reporter, SkIsConvexPolygon(clippedRRectPoly.begin(), clippedRRectPoly.size()));

    result = SkOffsetSimplePolygon(clippedRRectPoly.begin(), clippedRRectPoly.size(), bounds, 32.3699417f, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsConvexPolygon(offsetPoly.begin(), offsetPoly.size()));
    ...
    *starPoly.append() = SkPoint::Make(-14.43f, -25.0f);
    ...
    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, 0.1f, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));

    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, 5.665f, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));

    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, 28, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));

    // down to a point
    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, 28.866f, &offsetPoly);
    REPORTER_ASSERT(reporter, !result);

    // and past
    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, 50.5f, &offsetPoly);
    REPORTER_ASSERT(reporter, !result);
    ...
    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, -5.6665f, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));

    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, -50, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));

    result = SkOffsetSimplePolygon(starPoly.begin(), starPoly.size(), bounds, -100, &offsetPoly);
    REPORTER_ASSERT(reporter, result);
    REPORTER_ASSERT(reporter, SkIsSimplePolygon(offsetPoly.begin(), offsetPoly.size()));
