...
static void cleanup(GLTestContext* glctx0, GrGLuint texID0, GLTestContext* glctx1, sk_sp<GrDirectContext> dContext, GrEGLImage image1)
{
    if (glctx1) {
        glctx1->makeCurrent();
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(EGLImageTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
        context1.get(), kSize, kSize, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo);
    ...
    GR_GL_CALL(glCtx1->gl(), TexSubImage2D(texInfo.fTarget, 0, 0, 0, kSize, kSize, GR_GL_RGBA, GR_GL_UNSIGNED_BYTE, pixels.get()));
    GR_GL_CALL(glCtx1->gl(), Finish());
    ...
    sk_sp<GrSurfaceProxy> texProxy
        = context0->priv().proxyProvider()->wrapBackendTexture(backendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
    if (!texProxy) {
        ERRORF(reporter, "Error wrapping external texture in GrTextureProxy.");
        cleanup(glCtx0, externalTexture.fID, glCtx1.get(), context1, image);
        return;
    }
    skgpu::Swizzle swizzle = context0->priv().caps()->getReadSwizzle(texProxy->backendFormat(), colorInfo.colorType());
    ...
    {
        auto temp = context0->priv().makeSFCFromBackendTexture(colorInfo, backendTex, 1, origin,
    ...
    TestCopyFromSurface(
        reporter, context0, surfaceContext->asSurfaceProxyRef(), surfaceContext->origin(), colorInfo.colorType(), pixels.get(), "EGLImageTest-copy");
