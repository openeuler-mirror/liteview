...
DEF_TEST(TypefaceMacVariation, reporter)
{
    ...
    const int retrievedOriginalPositionCount = typeface->getVariationDesignPosition(originalPosition.data(), originalPosition.size());
    ...
    for (auto& originalCoordinate : originalPosition) {
        SkMaybeDebugf("(%s: %f) ", tagToString(originalCoordinate.axis).c_str(), originalCoordinate.value);
    }
    SkMaybeDebugf("\n\n");
    ...
    const int returnedOriginalAxisCount = typeface->getVariationDesignParameters(originalAxes.data(), originalAxes.size());
        ...
        for (SkFourByteTag axisToBump : { 0u, kOpszTag, kWdthTag, kGRADTag }) {
            for (float testCoordinate : { 100, 300, 400, 500, 700, 900 }) {
                ...
                for (auto& expectedCoordinate : expectedPosition) {
                    SkMaybeDebugf("(%s: %f) ", tagToString(expectedCoordinate.axis).c_str(), expectedCoordinate.value);
                }
                SkMaybeDebugf("\n");

                SkFontArguments::VariationPosition variationPosition = { requestPosition.data(), (int)requestPosition.size() };
                sk_sp<SkTypeface> cloneTypeface(typeface->makeClone(SkFontArguments().setVariationDesignPosition(variationPosition)));
                ...
                const int retrievedCloneAxisCount = cloneTypeface->getVariationDesignPosition(clonePosition.data(), clonePosition.size());
                ...
                for (auto& cloneCoordinate : clonePosition) {
                    SkMaybeDebugf("(%s: %f) ", tagToString(cloneCoordinate.axis).c_str(), cloneCoordinate.value);
                }
                SkMaybeDebugf("\n");
                ...
                auto compareCoordinate = [](const Coordinate& a, const Coordinate& b) -> bool { return a.axis < b.axis; };
