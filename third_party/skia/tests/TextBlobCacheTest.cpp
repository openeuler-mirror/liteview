...
static void text_blob_cache_inner(
    skiatest::Reporter* reporter, GrDirectContext* dContext, int maxTotalText, int maxGlyphID, int maxFamilies, bool normal, bool stressTest)
{
    // setup surface
    uint32_t flags = 0;
            ...
            for (int aa = 0; aa < 2; aa++) {
                for (int subpixel = 0; subpixel < 2; subpixel++) {
                    for (int lcd = 0; lcd < 2; lcd++) {
                        ...
                        const SkTextBlobBuilder::RunBuffer& run = builder.allocRun(font, maxTotalText, 0, 0, nullptr);
...
static sk_sp<SkTextBlob> make_blob()
{
    ...
    int glyphCount = font.textToGlyphs(text, sizeof(text), SkTextEncoding::kUTF8, glyphs, maxGlyphLen);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(TextBlobJaggedGlyph, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    const SkImageInfo info = SkImageInfo::Make(kScreenDim, kScreenDim, kN32_SkColorType, kPremul_SkAlphaType);
