...
{
    return cheap_distance(points[1].fX * 2 - points[2].fX - points[0].fX, points[1].fY * 2 - points[2].fY - points[0].fY);
}
...
static const int gSawtooth[] = { 0, 0, 10, 10, 20, 20, 30, 10, 40, 0, 50, -10, 60, -20, 70, -10, 80, 0 };
    ...
    for (unsigned i = 4; i < count; i += 2) {
        ...
        if (!isAccurate) {
            ERRORF(reporter,
                ...
                path[0].fX, path[0].fY, path[1].fX, path[1].fY, path[2].fX, path[2].fY, computedCount, estimatedCount);
            numErrors++;
        }
    }
