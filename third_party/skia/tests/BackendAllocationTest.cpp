...
void test_wrapping(GrDirectContext* dContext, skiatest::Reporter* reporter,
    std::function<sk_sp<ManagedBackendTexture>(GrDirectContext*, GrMipmapped, GrRenderable)> create, GrColorType grColorType, GrMipmapped mipmapped,
    GrRenderable renderable)
{
    ...
    if (GrRenderable::kYes == renderable && dContext->colorTypeSupportedAsSurface(skColorType)) {
        sk_sp<SkSurface> surf = SkSurface::MakeFromBackendTexture(dContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, 0, skColorType, nullptr, nullptr);
        if (!surf) {
    ...
    {
        sk_sp<SkImage> img = SkImage::MakeFromTexture(dContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, skColorType, kUnpremul_SkAlphaType, nullptr);
        if (!img) {
...
static void check_solid_pixmap(
    skiatest::Reporter* reporter, const SkColor4f& expected, const SkPixmap& actual, GrColorType ct, const char* label1, const char* label2)
{
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([reporter, ct, label1, label2, expected](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(reporter,
            "%s %s %s - mismatch at %d, %d "
            "expected (%.2f, %.2f, %.2f %.2f) "
            "- diffs (%.2f, %.2f, %.2f %.2f)",
            GrColorTypeToStr(ct), label1, label2, x, y, expected.fR, expected.fG, expected.fB, expected.fA, diffs[0], diffs[1], diffs[2], diffs[3]);
    });
...
static SkColor4f get_expected_color(SkColor4f orig, GrColorType ct)
{
    ...
    GrConvertPixels(GrPixmap(resultII, &result.fR, sizeof(result)), GrPixmap(ii, data.get(), ii.minRowBytes()));
    return result;
}
...
static void check_base_readbacks(GrDirectContext* dContext, const GrBackendTexture& backendTex, GrColorType colorType, GrRenderable renderableTexture,
    const SkColor4f& color, skiatest::Reporter* reporter, const char* label)
{
    ...
    {
        SkImageInfo readBackII = SkImageInfo::Make(32, 32, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
    ...
    for (GrRenderable renderableCtx : { GrRenderable::kNo, GrRenderable::kYes }) {
        ...
        if (renderableCtx == GrRenderable::kYes) {
            proxy = dContext->priv().proxyProvider()->wrapRenderableBackendTexture(backendTex, 1, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, nullptr);
        } else {
            proxy = dContext->priv().proxyProvider()->wrapBackendTexture(backendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
        }
        if (!proxy) {
        ...
        if (!surfaceContext->readPixels(dContext, actual, { 0, 0 })) {
            ...
            auto name = SkStringPrintf("%s::readPixels", (renderableCtx == GrRenderable::kYes ? "SurfaceFillContext" : "SurfaceContext"));
...
void test_color_init(GrDirectContext* dContext, skiatest::Reporter* reporter,
    std::function<sk_sp<ManagedBackendTexture>(GrDirectContext*, const SkColor4f&, GrMipmapped, GrRenderable)> create, GrColorType colorType,
    const SkColor4f& color, GrMipmapped mipmapped, GrRenderable renderable)
{
    ...
    auto checkBackendTexture = [&](const SkColor4f& testColor) {
        if (mipmapped == GrMipmapped::kYes) {
            ...
            SkColor4f expectedColors[6] = { expectedColor, expectedColor, expectedColor, expectedColor, expectedColor, expectedColor };
        ...
        check_base_readbacks(dContext, mbet->texture(), colorType, renderable, testColor, reporter, "colorinit");
    };
    ...
    if (skColorType != kUnknown_SkColorType) {
        dContext->updateBackendTexture(mbet->texture(), skColorType, newColor, ManagedBackendTexture::ReleaseProc, mbet->releaseContext());
...
// levels.
static void check_mipmaps(GrDirectContext* dContext, const GrBackendTexture& backendTex, GrColorType colorType, const SkColor4f expectedColors[6],
    skiatest::Reporter* reporter, const char* label)
{
    ...
    auto proxy = dContext->priv().proxyProvider()->wrapBackendTexture(backendTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
    if (!proxy) {
        ERRORF(reporter, "Could not make proxy from backend texture");
        return;
    }
    ...
    for (int i = 0, rectSize = 32; i < kNumMipLevels; ++i, rectSize /= 2) {
        ...
        static constexpr GrSamplerState kNearestNearest(GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNearest);
        auto fp = GrTextureEffect::Make(readView, kUnpremul_SkAlphaType, texMatrix, kNearestNearest, *dstFillContext->caps());
        dstFillContext->fillRectWithFP(SkIRect::MakeWH(rectSize, rectSize), std::move(fp));

        SkImageInfo readbackII = SkImageInfo::Make(rectSize, rectSize, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
        SkAutoPixmapStorage actual;
        SkAssertResult(actual.tryAlloc(readbackII));
...
void test_pixmap_init(GrDirectContext* dContext, skiatest::Reporter* reporter,
    std::function<sk_sp<ManagedBackendTexture>(GrDirectContext*, const SkPixmap srcData[], int numLevels, GrSurfaceOrigin, GrRenderable)> create,
    SkColorType skColorType, GrSurfaceOrigin origin, GrMipmapped mipmapped, GrRenderable renderable)
{
    SkPixmap pixmaps[6];
    ...
    int numMipLevels = ToolUtils::make_pixmaps(skColorType, kUnpremul_SkAlphaType, mipmapped == GrMipmapped::kYes, kColors, pixmaps, &memForPixmaps);
    ...
    auto checkBackendTexture = [&](const SkColor4f colors[6]) {
        ...
        // The last step in this test will dirty the mipmaps so do it last
        check_base_readbacks(dContext, mbet->texture(), grColorType, renderable, colors[0], reporter, "pixmap");
    };
    ...
    ToolUtils::make_pixmaps(skColorType, kUnpremul_SkAlphaType, mipmapped == GrMipmapped::kYes, kColorsNew, pixmaps, &memForPixmaps);

    // Upload new data and make sure everything still works
    dContext->updateBackendTexture(mbet->texture(), pixmaps, numMipLevels, origin, ManagedBackendTexture::ReleaseProc, mbet->releaseContext());
    ...
    for (auto combo : combinations) {
        ...
        for (auto mipmapped : { GrMipmapped::kNo, GrMipmapped::kYes }) {
            ...
            for (auto renderable : { GrRenderable::kNo, GrRenderable::kYes }) {
                if (!caps->getDefaultBackendFormat(SkColorTypeToGrColorType(colorType), renderable).isValid()) {
                    continue;
                }
                ...
                {
                    auto uninitCreateMtd = [colorType](GrDirectContext* dContext, GrMipmapped mipmapped, GrRenderable renderable) {
                        auto mbet = ManagedBackendTexture::MakeWithoutData(dContext, 32, 32, colorType, mipmapped, renderable, GrProtected::kNo);
                    ...
                    test_wrapping(context, reporter, uninitCreateMtd, SkColorTypeToGrColorType(colorType), mipmapped, renderable);
                }

                {
                    auto createWithColorMtd = [colorType](GrDirectContext* dContext, const SkColor4f& color, GrMipmapped mipmapped, GrRenderable renderable) {
                        auto mbet = ManagedBackendTexture::MakeWithData(dContext, 32, 32, colorType, color, mipmapped, renderable, GrProtected::kNo);
                        check_vk_tiling(mbet->texture());
                    ...
                    test_color_init(context, reporter, createWithColorMtd, SkColorTypeToGrColorType(colorType), combo.fColor, mipmapped, renderable);
                }

                for (auto origin : { kTopLeft_GrSurfaceOrigin, kBottomLeft_GrSurfaceOrigin }) {
                    auto createWithSrcDataMtd
                        = [](GrDirectContext* dContext, const SkPixmap srcData[], int numLevels, GrSurfaceOrigin origin, GrRenderable renderable) {
                              SkASSERT(srcData && numLevels);
                              auto mbet = ManagedBackendTexture::MakeWithData(dContext, srcData, numLevels, origin, renderable, GrProtected::kNo);
                              check_vk_tiling(mbet->texture());
#ifdef SK_DEBUG
                              {
                    ...
                    test_pixmap_init(context, reporter, createWithSrcDataMtd, colorType, origin, mipmapped, renderable);
                }
            }
        }
    }
...
DEF_GANESH_TEST(ColorTypeBackendAllocationTest, reporter, options, CtsEnforcement::kApiLevel_T)
{
    for (int t = 0; t < sk_gpu_test::GrContextFactory::kContextTypeCnt; ++t) {
        ...
        if (info.directContext()->priv().caps()->writePixelsRowBytesSupport() && info.directContext()->backend() == GrBackendApi::kOpenGL) {
...
DEF_GANESH_TEST_FOR_ALL_GL_CONTEXTS(GLBackendAllocationTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (auto combo : combinations) {
        for (GrTextureType textureType : { GrTextureType::k2D, GrTextureType::kRectangle }) {
            GrGLenum target = textureType == GrTextureType::k2D ? GR_GL_TEXTURE_2D : GR_GL_TEXTURE_RECTANGLE;
            GrBackendFormat format = GrBackendFormat::MakeGL(combo.fFormat, target);
            if (!glCaps->isFormatTexturable(format, textureType)) {
                continue;
            }

            if (GrColorType::kBGRA_8888 == combo.fColorType || GrColorType::kBGRA_1010102 == combo.fColorType) {
                // We allow using a GL_RGBA8 or GR_GL_RGB10_A2 texture as BGRA on desktop GL but not
                // ES
                if (kGL_GrGLStandard != standard && (GR_GL_RGBA8 == combo.fFormat || GR_GL_RGB10_A2 == combo.fFormat)) {
                    continue;
                }
            }

            for (auto mipmapped : { GrMipmapped::kNo, GrMipmapped::kYes }) {
                if (GrMipmapped::kYes == mipmapped && (!glCaps->mipmapSupport() || target == GR_GL_TEXTURE_RECTANGLE)) {
                    continue;
                }
                ...
                for (auto renderable : { GrRenderable::kNo, GrRenderable::kYes }) {
                    ...
                    {
                        auto uninitCreateMtd = [format](GrDirectContext* dContext, GrMipmapped mipmapped, GrRenderable renderable) {
                            return ManagedBackendTexture::MakeWithoutData(dContext, 32, 32, format, mipmapped, renderable, GrProtected::kNo);
                        };

                        test_wrapping(context, reporter, uninitCreateMtd, combo.fColorType, mipmapped, renderable);
                    }

                    {
                        ...
                        auto createWithColorMtd
                            = [format, swizzle](GrDirectContext* dContext, const SkColor4f& color, GrMipmapped mipmapped, GrRenderable renderable) {
                                  auto swizzledColor = swizzle.applyTo(color);
                                  return ManagedBackendTexture::MakeWithData(dContext, 32, 32, format, swizzledColor, mipmapped, renderable, GrProtected::kNo);
                              };
                        test_color_init(context, reporter, createWithColorMtd, combo.fColorType, combo.fColor, mipmapped, renderable);
                    }
                }
            }
        }
...
DEF_GANESH_TEST_FOR_VULKAN_CONTEXT(VkBackendAllocationTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    auto context = ctxInfo.directContext();
        ...
        for (auto mipmapped : { GrMipmapped::kNo, GrMipmapped::kYes }) {
            ...
            for (auto renderable : { GrRenderable::kNo, GrRenderable::kYes }) {
                ...
                if (GrRenderable::kYes == renderable) {
                    ...
                    // color type.
                    if (!vkCaps->isFormatAsColorTypeRenderable(combo.fColorType, GrBackendFormat::MakeVk(combo.fFormat), 1)) {
                        continue;
                    }
                }
                ...
                {
                    auto uninitCreateMtd = [format](GrDirectContext* dContext, GrMipmapped mipmapped, GrRenderable renderable) {
                        auto mbet = ManagedBackendTexture::MakeWithoutData(dContext, 32, 32, format, mipmapped, renderable, GrProtected::kNo);
                        check_vk_tiling(mbet->texture());
                        return mbet;
                    };
...

                {
                    ...
                    auto createWithColorMtd
                        = [format, swizzle](GrDirectContext* dContext, const SkColor4f& color, GrMipmapped mipmapped, GrRenderable renderable) {
                              auto swizzledColor = swizzle.applyTo(color);
                              auto mbet = ManagedBackendTexture::MakeWithData(dContext, 32, 32, format, swizzledColor, mipmapped, renderable, GrProtected::kNo);
                              check_vk_tiling(mbet->texture());
                              return mbet;
                          };
                    test_color_init(context, reporter, createWithColorMtd, combo.fColorType, combo.fColor, mipmapped, renderable);
