...
static void check_equivalence(skiatest::Reporter* r, const GrStyledShape& a, const GrStyledShape& b, const Key& keyA, const Key& keyB)
{
    ...
    if (SkPathFillType_ConvertToNonInverse(pathA.getFillType()) != SkPathFillType_ConvertToNonInverse(pathB.getFillType())) {
    ...
    if (allowSameRRectButDiffStartAndDir) {
        ...
        if (ignoreWindingVsEvenOdd) {
            pA.setFillType(pA.isInverseFillType() ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
            pB.setFillType(pB.isInverseFillType() ? SkPathFillType::kInverseEvenOdd : SkPathFillType::kEvenOdd);
        }
...
static void check_original_path_ids(
    skiatest::Reporter* r, const GrStyledShape& base, const GrStyledShape& pe, const GrStyledShape& peStroke, const GrStyledShape& full)
{
...
{
    GrStyledShape preserve = GrStyledShape::MakeFilled(shape, GrStyledShape::FillInversion::kPreserve);
    Key preserveKey;
    ...
    GrStyledShape inverted = GrStyledShape::MakeFilled(shape, GrStyledShape::FillInversion::kForceInverted);
    Key invertedKey;
    make_key(&invertedKey, inverted);

    GrStyledShape noninverted = GrStyledShape::MakeFilled(shape, GrStyledShape::FillInversion::kForceNoninverted);
    Key noninvertedKey;
...
public:
    ...
    virtual bool fillChangesGeom() const
    {
        return false;
    }
    virtual bool strokeIsConvertedToFill() const
    {
        return false;
    }
    virtual bool strokeAndFillIsConvertedToFill(const SkPaint&) const
    {
        return false;
    }
    // Is this something we expect GrStyledShape to recognize as something simpler than a path.
    virtual bool isNonPath(const SkPaint& paint) const
    {
        return true;
    }
};
...
public:
    ...
    bool strokeAndFillIsConvertedToFill(const SkPaint& paint) const override
    {
        ...
        return (paint.getStrokeJoin() == SkPaint::kMiter_Join && paint.getStrokeMiter() >= SK_ScalarSqrt2) || paint.getStrokeJoin() == SkPaint::kRound_Join;
    }
...
public:
    ...
    PathGeo(const SkPath& path, Invert invert)
        : fPath(path)
    {
...
public:
    ...
    RRectPathGeo(const SkPath& path, const SkRRect& equivalentRRect, RRectForStroke rrectForStroke, Invert invert)
        : PathGeo(path, invert)
    ...
    RRectPathGeo(const SkPath& path, const SkRect& equivalentRect, RRectForStroke rrectForStroke, Invert invert)
...
public:
    ...
    const GrStyledShape& baseShape() const
    {
        return *fBase;
    }
    const GrStyledShape& appliedPathEffectShape() const
    {
        return *fAppliedPE;
    }
    const GrStyledShape& appliedFullStyleShape() const
    {
        return *fAppliedFull;
    }

    // The returned array's count will be 0 if the key shape has no key.
    const Key& baseKey() const
    {
        return fBaseKey;
    }
    const Key& appliedPathEffectKey() const
    {
        return fAppliedPEKey;
    }
    const Key& appliedFullStyleKey() const
    {
        return fAppliedFullKey;
    }
    const Key& appliedPathEffectThenStrokeKey() const
    {
...
void TestCase::compare(skiatest::Reporter* r, const TestCase& that, ComparisonExpecation expectation) const
{
    SkPath a, b;
    switch (expectation) {
    ...
    case kAllSame_ComparisonExpecation:
        check_equivalence(r, *fBase, *that.fBase, fBaseKey, that.fBaseKey);
        check_equivalence(r, *fAppliedPE, *that.fAppliedPE, fAppliedPEKey, that.fAppliedPEKey);
        check_equivalence(r, *fAppliedFull, *that.fAppliedFull, fAppliedFullKey, that.fAppliedFullKey);
        break;
    }
}
} // namespace
...
template <typename... Args> static std::unique_ptr<TestCase> make_TestCase(Args&&... args)
{
...
static void test_basic(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    make_TestCase(geo, fill, reporter)->compare(reporter, fillCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    make_TestCase(geo, stroke2RoundBevel, reporter)->compare(reporter, stroke2RoundBevelCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    make_TestCase(geo, stroke2RoundBevelDash, reporter)->compare(reporter, stroke2RoundBevelDashCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    if (geo.fillChangesGeom() || geo.strokeIsConvertedToFill()) {
        ...
        fillCase.compare(reporter, stroke2RoundBevelCase, TestCase::kSameUpToStroke_ComparisonExpecation);
        fillCase.compare(reporter, stroke2RoundBevelDashCase, TestCase::kSameUpToPE_ComparisonExpecation);
    }
    if (geo.strokeIsConvertedToFill()) {
        stroke2RoundBevelCase.compare(reporter, stroke2RoundBevelDashCase, TestCase::kAllDifferent_ComparisonExpecation);
    } else {
        stroke2RoundBevelCase.compare(reporter, stroke2RoundBevelDashCase, TestCase::kSameUpToPE_ComparisonExpecation);
    }
    ...
    make_TestCase(geo, stroke2RoundBevelAndFill, reporter)->compare(reporter, stroke2RoundBevelAndFillCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    make_TestCase(geo, stroke2RoundBevelAndFillDash, reporter)->compare(reporter, stroke2RoundBevelAndFillDashCase, TestCase::kAllSame_ComparisonExpecation);
    stroke2RoundBevelAndFillDashCase.compare(reporter, stroke2RoundBevelAndFillCase, TestCase::kAllSame_ComparisonExpecation);
...
static void test_scale(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    if (geo.strokeAndFillIsConvertedToFill(strokeAndFillDash)) {
        ...
        strokeAndFillCase1.compare(reporter, strokeAndFillCase2, TestCase::kAllSame_ComparisonExpecation);
        strokeAndFillDashCase1.compare(reporter, strokeAndFillDashCase2, TestCase::kAllSame_ComparisonExpecation);
    } else {
        strokeAndFillCase1.compare(reporter, strokeAndFillCase2, TestCase::kSameUpToStroke_ComparisonExpecation);
    }
    strokeAndFillDashCase1.compare(reporter, strokeAndFillCase1, TestCase::kAllSame_ComparisonExpecation);
    strokeAndFillDashCase2.compare(reporter, strokeAndFillCase2, TestCase::kAllSame_ComparisonExpecation);
}

template <typename T>
static void test_stroke_param_impl(
    skiatest::Reporter* reporter, const Geo& geo, std::function<void(SkPaint*, T)> setter, T a, T b, bool paramAffectsStroke, bool paramAffectsDashAndStroke)
{
    ...
    if (paramAffectsStroke) {
        // If stroking is immediately incorporated into a geometric transformation then the base
        // shapes will differ.
        if (geo.strokeAndFillIsConvertedToFill(strokeAndFillA) || geo.strokeAndFillIsConvertedToFill(strokeAndFillB)) {
            strokeAndFillACase.compare(reporter, strokeAndFillBCase, TestCase::kAllDifferent_ComparisonExpecation);
        } else {
            strokeAndFillACase.compare(reporter, strokeAndFillBCase, TestCase::kSameUpToStroke_ComparisonExpecation);
        }
    } else {
        strokeAndFillACase.compare(reporter, strokeAndFillBCase, TestCase::kAllSame_ComparisonExpecation);
    }
...
template <typename T> static void test_stroke_param(skiatest::Reporter* reporter, const Geo& geo, std::function<void(SkPaint*, T)> setter, T a, T b)
{
...
static void test_stroke_cap(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    test_stroke_param_impl<SkPaint::Cap>(
        reporter, geo, [](SkPaint* p, SkPaint::Cap c) { p->setStrokeCap(c); }, SkPaint::kButt_Cap, SkPaint::kRound_Cap, affectsStroke, affectsDashAndStroke);
};
...
static void test_stroke_join(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    test_stroke_param_impl<SkPaint::Join>(
        reporter, geo, [](SkPaint* p, SkPaint::Join j) { p->setStrokeJoin(j); }, SkPaint::kRound_Join, SkPaint::kBevel_Join, affectsStroke, true);
};
...
static void test_miter_limit(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    // miter.
    test_stroke_param_impl<SkScalar>(reporter, geo, setMiterJoinAndLimit, 0.5f, 0.75f, mayHaveJoins, true);
...
void test_path_effect_makes_rrect(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    public:
        ...
        {
            return sk_sp<SkPathEffect>(new RRectPathEffect);
        }
    ...
    protected:
        bool onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect* cullR, const SkMatrix&) const override
        {
            dst->reset();
            dst->addRRect(RRect());
            return true;
...
void test_unknown_path_effect(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    public:
        static sk_sp<SkPathEffect> Make()
        {
            return sk_sp<SkPathEffect>(new AddLineTosPathEffect);
        }
        Factory getFactory() const override
        {
            return nullptr;
...
void test_make_hairline_path_effect(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    protected:
        bool onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* strokeRec, const SkRect* cullR, const SkMatrix&) const override
        {
            *dst = src;
...
void test_path_effect_makes_empty_shape(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    REPORTER_ASSERT(reporter, geoPEInvertStrokeCase.appliedPathEffectThenStrokeKey() == invertedEmptyKey);
...
void test_path_effect_fails(skiatest::Reporter* reporter, const Geo& geo)
{
    ...
    public:
        static sk_sp<SkPathEffect> Make()
        {
            return sk_sp<SkPathEffect>(new FailurePathEffect);
        }
        Factory getFactory() const override
        {
            return nullptr;
...
DEF_TEST(GrStyledShape_empty_shape, reporter)
{
    ...
    strokeInvertedEmptyCase.compare(reporter, fillInvertedEmptyCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    dashAndStrokeInvertexEmptyCase.compare(reporter, fillEmptyCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    dashAndStrokeEmptyRRectCase.compare(reporter, fillEmptyCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    fillInvertedEmptyRRectCase.compare(reporter, fillInvertedEmptyCase, TestCase::kAllSame_ComparisonExpecation);

    TestCase strokeInvertedEmptyRRectCase(reporter, emptyRRect, kDir, kStart, true, GrStyle(stroke));
    strokeInvertedEmptyRRectCase.compare(reporter, strokeInvertedEmptyCase, TestCase::kAllSame_ComparisonExpecation);

    TestCase dashAndStrokeEmptyInvertedRRectCase(reporter, emptyRRect, kDir, kStart, true, GrStyle(dashAndStroke));
    dashAndStrokeEmptyInvertedRRectCase.compare(reporter, fillEmptyCase, TestCase::kAllSame_ComparisonExpecation);
    ...
    dashAndStrokeEmptyRectCase.compare(reporter, fillEmptyCase, TestCase::kAllSame_ComparisonExpecation);

    TestCase dashAndStrokeEmptyInvertedRectCase(reporter, SkRRect::MakeRect(emptyRect), kDir, kStart, true, GrStyle(dashAndStroke));
    // Dashing ignores inverseness so this is equivalent to the non-inverted empty fill.
    dashAndStrokeEmptyInvertedRectCase.compare(reporter, fillEmptyCase, TestCase::kAllSame_ComparisonExpecation);
}
...
void test_rrect(skiatest::Reporter* r, const SkRRect& rrect)
{
    ...
    SkStrokeRec strokeRecs[4] { SkStrokeRec::kFill_InitStyle, SkStrokeRec::kFill_InitStyle, SkStrokeRec::kFill_InitStyle, SkStrokeRec::kFill_InitStyle };
    ...
    auto index = [](bool inverted, SkPathDirection dir, unsigned start, Style style, bool dash) -> int {
        return inverted * (2 * 8 * kStyleCnt * 2) + (int)dir * (8 * kStyleCnt * 2) + start * (kStyleCnt * 2) + style * (2) + dash;
    };
                ...
                for (Style style : { kFill, kStroke, kHairline, kStrokeAndFill }) {
                    for (bool dash : { false, true }) {
                        ...
                        shapes[index(inverted, dir, start, style, dash)]
                            = GrStyledShape(rrect, dir, start, SkToBool(inverted), GrStyle(strokeRecs[style], std::move(pe)));
                    }
                }
            }
        }
    ...
    const GrStyledShape& exampleFillCase = shapes[index(false, kExamplesDir, kExamplesStart, kFill, false)];
    Key exampleFillCaseKey;
    make_key(&exampleFillCaseKey, exampleFillCase);

    const GrStyledShape& exampleStrokeAndFillCase = shapes[index(false, kExamplesDir, kExamplesStart, kStrokeAndFill, false)];
    Key exampleStrokeAndFillCaseKey;
    make_key(&exampleStrokeAndFillCaseKey, exampleStrokeAndFillCase);

    const GrStyledShape& exampleInvFillCase = shapes[index(true, kExamplesDir, kExamplesStart, kFill, false)];
    Key exampleInvFillCaseKey;
    make_key(&exampleInvFillCaseKey, exampleInvFillCase);

    const GrStyledShape& exampleInvStrokeAndFillCase = shapes[index(true, kExamplesDir, kExamplesStart, kStrokeAndFill, false)];
    Key exampleInvStrokeAndFillCaseKey;
    make_key(&exampleInvStrokeAndFillCaseKey, exampleInvStrokeAndFillCase);

    const GrStyledShape& exampleStrokeCase = shapes[index(false, kExamplesDir, kExamplesStart, kStroke, false)];
    Key exampleStrokeCaseKey;
    make_key(&exampleStrokeCaseKey, exampleStrokeCase);

    const GrStyledShape& exampleInvStrokeCase = shapes[index(true, kExamplesDir, kExamplesStart, kStroke, false)];
    Key exampleInvStrokeCaseKey;
    make_key(&exampleInvStrokeCaseKey, exampleInvStrokeCase);

    const GrStyledShape& exampleHairlineCase = shapes[index(false, kExamplesDir, kExamplesStart, kHairline, false)];
    Key exampleHairlineCaseKey;
    make_key(&exampleHairlineCaseKey, exampleHairlineCase);

    const GrStyledShape& exampleInvHairlineCase = shapes[index(true, kExamplesDir, kExamplesStart, kHairline, false)];
        ...
        for (SkPathDirection dir : { SkPathDirection::kCW, SkPathDirection::kCCW }) {
            for (unsigned start = 0; start < 8; ++start) {
                for (bool dash : { false, true }) {
                    ...
                    const GrStyledShape& strokeAndFillCase = shapes[index(inverted, dir, start, kStrokeAndFill, dash)];
...
DEF_TEST(GrStyledShape_lines, r)
{
    ...
    strokeAB.compare(r, canonicalizeAsAB ? dashAB : dashBA, TestCase::kSameUpToPE_ComparisonExpecation);
...
DEF_TEST(GrStyledShape_stroked_lines, r)
{
    ...
    for (const auto& pe : pathEffects) {
        ...
        make_TestCase(r, linePath, buttCap)->compare(r, TestCase(r, SkRect::MakeLTRB(2, 4, 6, 5), fill), TestCase::kAllSame_ComparisonExpecation);

        make_TestCase(r, linePath, squareCap)->compare(r, TestCase(r, SkRect::MakeLTRB(2, 2, 6, 7), fill), TestCase::kAllSame_ComparisonExpecation);

        make_TestCase(r, linePath, roundCap)
            ->compare(r, TestCase(r, SkRRect::MakeRectXY(SkRect::MakeLTRB(2, 2, 6, 7), 2, 2), fill), TestCase::kAllSame_ComparisonExpecation);
        ...
        make_TestCase(r, linePath, buttCap)->compare(r, TestCase(r, SkRect::MakeLTRB(4, 2, 5, 6), fill), TestCase::kAllSame_ComparisonExpecation);
        make_TestCase(r, linePath, squareCap)->compare(r, TestCase(r, SkRect::MakeLTRB(2, 2, 7, 6), fill), TestCase::kAllSame_ComparisonExpecation);
        make_TestCase(r, linePath, roundCap)
            ->compare(r, TestCase(r, SkRRect::MakeRectXY(SkRect::MakeLTRB(2, 2, 7, 6), 2, 2), fill), TestCase::kAllSame_ComparisonExpecation);
        ...
        make_TestCase(r, linePath, buttCap)->compare(r, TestCase(r, SkRect::MakeEmpty(), fill), TestCase::kAllSame_ComparisonExpecation);
        make_TestCase(r, linePath, squareCap)->compare(r, TestCase(r, SkRect::MakeLTRB(2, 2, 6, 6), fill), TestCase::kAllSame_ComparisonExpecation);
        make_TestCase(r, linePath, roundCap)
            ->compare(r, TestCase(r, SkRRect::MakeRectXY(SkRect::MakeLTRB(2, 2, 6, 6), 2, 2), fill), TestCase::kAllSame_ComparisonExpecation);
    }
}
...
DEF_TEST(GrStyledShape_short_path_keys, r)
{
    ...
    auto compare = [r, &paints](const SkPath& pathA, const SkPath& pathB, TestCase::ComparisonExpecation expectation) {
...
DEF_TEST(GrStyledShape, reporter)
{
    ...
    for (auto r : { SkRect::MakeWH(10, 20), SkRect::MakeWH(-10, -20), SkRect::MakeWH(-10, 20), SkRect::MakeWH(10, -20) }) {
        geos.emplace_back(new RectGeo(r));
        SkPath rectPath;
        rectPath.addRect(r);
        geos.emplace_back(new RRectPathGeo(rectPath, r, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kNo));
        geos.emplace_back(new RRectPathGeo(rectPath, r, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kYes));
        rrectPathGeos.emplace_back(new RRectPathGeo(rectPath, r, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kNo));
    }
    for (auto rr :
        { SkRRect::MakeRect(SkRect::MakeWH(10, 10)), SkRRect::MakeRectXY(SkRect::MakeWH(10, 10), 3, 4), SkRRect::MakeOval(SkRect::MakeWH(20, 20)) }) {
        geos.emplace_back(new RRectGeo(rr));
        test_rrect(reporter, rr);
        SkPath rectPath;
        rectPath.addRRect(rr);
        geos.emplace_back(new RRectPathGeo(rectPath, rr, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kNo));
        geos.emplace_back(new RRectPathGeo(rectPath, rr, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kYes));
        rrectPathGeos.emplace_back(new RRectPathGeo(rectPath, rr, RRectPathGeo::RRectForStroke::kYes, PathGeo::Invert::kNo));
    }
...

    {
        ...
        geos.emplace_back(new RRectPathGeo(openRectPath, SkRect::MakeWH(10, 10), RRectPathGeo::RRectForStroke::kNo, PathGeo::Invert::kNo));
        geos.emplace_back(new RRectPathGeo(openRectPath, SkRect::MakeWH(10, 10), RRectPathGeo::RRectForStroke::kNo, PathGeo::Invert::kYes));
        rrectPathGeos.emplace_back(new RRectPathGeo(openRectPath, SkRect::MakeWH(10, 10), RRectPathGeo::RRectForStroke::kNo, PathGeo::Invert::kNo));
    }
    ...
    for (int i = 0; i < geos.count(); ++i) {
        ...
        test_stroke_param<SkScalar>(
            reporter, *geos[i], [](SkPaint* p, SkScalar w) { p->setStrokeWidth(w); }, SkIntToScalar(2), SkIntToScalar(4));
    ...
    for (int i = 0; i < rrectPathGeos.count(); ++i) {
        ...
        SkRRect rrect;
        REPORTER_ASSERT(reporter, rrgeo.isNonPath(fillPaint) == fillPathCase.baseShape().asRRect(&rrect, nullptr, nullptr, nullptr));
        ...
        if (rrgeo.isNonPath(strokePaint)) {
            REPORTER_ASSERT(reporter, strokePathCase.baseShape().asRRect(&rrect, nullptr, nullptr, nullptr));
            REPORTER_ASSERT(reporter, rrect == rrgeo.rrect());
...
DEF_TEST(GrStyledShape_arcs, reporter)
{
    ...
    for (const auto& style : styles) {
        ...
        auto reversedExepectations = style.hasPathEffect() ? TestCase::kAllDifferent_ComparisonExpecation : TestCase::kAllSame_ComparisonExpecation;
        ...
        arc1CWWithCenter.compare(reporter, arc2CWWithCenter, TestCase::kAllDifferent_ComparisonExpecation);
        ...
        auto ovalExpectations = style.hasPathEffect() ? TestCase::kAllDifferent_ComparisonExpecation : TestCase::kAllSame_ComparisonExpecation;
        ...
        // simple fills.
        TestCase ovalArcWithCenter(GrStyledShape::MakeArc(kOval1, 304.f, 1225.f, true, style), reporter);
        ovalExpectations = style.isSimpleFill() ? TestCase::kAllSame_ComparisonExpecation : TestCase::kAllDifferent_ComparisonExpecation;
