...
const SkMatrix gMatrices[] = {
    ...
    SkMatrix::MakeAll(1000.1f, .0f, -100, 0.0f, .5f, -3000, 0.0f, .0f, 1),
    SkMatrix::MakeAll(0, 1, 0, 1, 0, 0, 0, 0, 1),
    SkMatrix::MakeAll(2, 7.0f, -100, -8000, .5f, 2000, 0, .0f, 1),
};
    ...
    for (SkMatrix m : gMatrices) {
        ...
        for (const float* y : { valuesT, valuesB }) {
            for (const float* x : { valuesL, valuesR }) {
                for (int i = 0; i < 500; ++i) {
                    int mask = rand.nextU();
                    const SkPoint devPts[4] = { { x[(mask >> 0) & 3], y[(mask >> 2) & 3] }, { x[(mask >> 4) & 3], y[(mask >> 6) & 3] },
                        { x[(mask >> 8) & 3], y[(mask >> 10) & 3] }, { x[(mask >> 12) & 3], y[(mask >> 14) & 3] } };

                    SkPoint localPts[4];
                    inverse.mapPoints(localPts, devPts, 4);

                    REPORTER_ASSERT(reporter, cullTest.isVisible(localPts[0]) == viewportRect.contains(devPts[0].fX, devPts[0].fY));

                    {
                        ...
                        REPORTER_ASSERT(reporter, cullTest.areVisible3(localPts) == viewportRect.intersects(devBounds3));
                    }

                    {
                        ...
                        REPORTER_ASSERT(reporter, cullTest.areVisible4(localPts) == viewportRect.intersects(devBounds4));
                    }
                }
            }
        }
