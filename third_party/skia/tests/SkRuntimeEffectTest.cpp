...
void test_invalid_effect(skiatest::Reporter* r, const char* src, const char* expected)
{
    ...
    REPORTER_ASSERT(r, errorText.contains(expected), "Expected error message to contain \"%s\". Actual message: \"%s\"", expected, errorText.c_str());
};
...
{
    test_invalid_effect(r, "half4 missing(); half4 main(float2 p) { return missing(); }", "function 'half4 missing()' is not defined");
}
...
DEF_TEST(SkRuntimeEffectInvalid_SkCapsDisallowed, r)
{
    ...
    test_invalid_effect(r, "half4 main(float2 p) { return sk_Caps.floatIs32Bits ? half4(1) : half4(0); }", "name 'sk_Caps' is reserved");
}
...
DEF_TEST(SkRuntimeEffectUniformFlags, r)
{
    ...
    REPORTER_ASSERT(r,
        uniforms[5].flags == (SkRuntimeEffect::Uniform::kArray_Flag | SkRuntimeEffect::Uniform::kColor_Flag | SkRuntimeEffect::Uniform::kHalfPrecision_Flag));
}
...
DEF_TEST(SkRuntimeEffectForShader, r)
{
    ...
    auto test_invalid = [r](const char* sksl, const char* expected, SkRuntimeEffect::Options options = {}) {
        auto [effect, errorText] = SkRuntimeEffect::MakeForShader(SkString(sksl));
        REPORTER_ASSERT(r, !effect);
        REPORTER_ASSERT(r, errorText.contains(expected), "Expected error message to contain \"%s\". Actual message: \"%s\"", expected, errorText.c_str());
    };
    ...
    test_invalid("half4 main(float2 p) { return sk_FragCoord.xy01; }", "unknown identifier 'sk_FragCoord'");
...
static bool read_pixels(SkSurface* surface, const GraphiteInfo* graphite, GrColor* pixels)
{
...
static void verify_2x2_surface_results(
    skiatest::Reporter* r, const SkRuntimeEffect* effect, SkSurface* surface, const GraphiteInfo* graphite, std::array<GrColor, 4> expected)
{
    ...
    if (actual != expected) {
        REPORT_FAILURE(r, "Runtime effect didn't match expectations",
            SkStringPrintf("\n"
                           ...
                           "Got     : [ %08x %08x %08x %08x ]\n"
                           "SkSL:\n%s\n",
                expected[0], expected[1], expected[2], expected[3], actual[0], actual[1], actual[2], actual[3], effect->source().c_str()));
    }
}
...
public:
    ...
    void test(std::array<GrColor, 4> expected, PreTestFn preTestCallback = nullptr)
    {
        ...
        verify_2x2_surface_results(fReporter, fBuilder->effect(), fSurface.get(), fGraphite, expected);
    }
...
static void test_RuntimeEffect_Shaders(skiatest::Reporter* r, GrRecordingContext* grContext, const GraphiteInfo* graphite)
{
    ...
    effect.build("half4 main(float2 p) { return half4(0.498 * (half2(sk_FragCoord.xy) - 0.5), 0, 1); }");
    effect.test({ 0xFF000000, 0xFF00007F, 0xFF007F00, 0xFF007F7F }, [](SkCanvas* canvas, SkPaint*) { canvas->rotate(45.0f); });
    ...
    if (!graphite) {
        ...
        effect.test(0xFF00FFFF, [](SkCanvas*, SkPaint* paint) { paint->setColor4f({ 1.0f, 1.0f, 0.0f, 1.0f }); });
    }
...
static void verify_draw_obeys_capabilities(skiatest::Reporter* r, const SkRuntimeEffect* effect, SkSurface* surface, const SkPaint& paint)
{
    ...
    verify_2x2_surface_results(r, effect, surface, /*graphite=*/nullptr, { kExpected, kExpected, kExpected, kExpected });
}
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRuntimeEffectObeysCapabilities_GPU, r, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    SkImageInfo info = SkImageInfo::Make(2, 2, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
...
DEF_TEST(SkRuntimeEffectTraceShader, r)
{
    for (int imageSize : { 2, 80 }) {
        TestEffect effect(r, /*grContext=*/nullptr, /*graphite=*/nullptr, SkISize { imageSize, imageSize });
        effect.build(R"(
            half4 main(float2 p) {
                float2 val = p - 0.5;
                return val.0y01;
...
exit half4 main(float2 p)
    ...
    REPORTER_ASSERT(r, dump == kExpectation, "Trace output does not match expectation:\n%.*s\n", (int)dump.size(), dump.data());
}
...
static void test_RuntimeEffect_Blenders(skiatest::Reporter* r, GrRecordingContext* grContext, const GraphiteInfo* graphite)
{
    ...
    effect.build("uniform blender child;"
                 "half4 main(half4 s, half4 d) { return child.eval(s, d); }");
    effect.child("child") = nullptr;
    effect.test({ 0xFF000080, 0xFF008000, 0xFF800000, 0xFF808080 }, [](SkCanvas*, SkPaint* paint) { paint->setColor4f({ 0.0f, 0.0f, 0.0f, 0.497f }); });
    ...
    effect.test({ 0xFF4523FF, 0xFF45FF01, 0xFFFF2301, 0xFFFFFFFF }, [](SkCanvas*, SkPaint* paint) { paint->setColor(0xFF012345); });
...
DEF_TEST(SkRuntimeColorFilterSingleColor, r)
{
    ...
    auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(SkString { "half4 main(half4 c) { return c*c; }" });
    REPORTER_ASSERT(r, effect);
...
static void test_RuntimeEffectStructNameReuse(skiatest::Reporter* r, GrRecordingContext* rContext)
{
    ...
    auto [childEffect, err] = SkRuntimeEffect::MakeForShader(SkString("uniform shader paint;"
                                                                      "struct S { half4 rgba; };"
    ...
    effect.test(0xFF00407F, [](SkCanvas*, SkPaint* paint) { paint->setColor4f({ 0.99608f, 0.50196f, 0.0f, 1.0f }); });
}
...
DEF_TEST(SkRuntimeColorFilterFlags, r)
{
    { // Here's a non-trivial filter that doesn't change alpha.
        auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(SkString { "half4 main(half4 color) { return color + half4(1,1,1,0); }" });
    ...
    { // Here's one that definitely changes alpha.
        auto [effect, err] = SkRuntimeEffect::MakeForColorFilter(SkString { "half4 main(half4 color) { return color + half4(0,0,0,4); }" });
        REPORTER_ASSERT(r, effect && err.isEmpty());
        sk_sp<SkColorFilter> filter = effect->makeColorFilter(SkData::MakeEmpty());
        REPORTER_ASSERT(r, filter && !filter->isAlphaUnchanged());
    }
...
DEF_TEST(SkRuntimeShaderSampleCoords, r)
{
    ...
    auto test = [&](const char* src, bool expectExplicit, bool expectReferencesSampleCoords) {
        auto [effect, err] = SkRuntimeEffect::MakeForShader(SkStringPrintf("uniform shader child; %s", src));
        REPORTER_ASSERT(r, effect);

        auto child = GrFragmentProcessor::MakeColor({ 1, 1, 1, 1 });
        auto fp = GrSkSLFP::Make(effect.get(), "test_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "child", std::move(child));
...
DEF_GANESH_TEST_FOR_ALL_CONTEXTS(GrSkSLFP_Specialized, r, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto make_color_fp = [&](SkPMColor4f color, bool specialize) {
        ...
        FpAndKey result;
        result.fp = GrSkSLFP::Make(effect, "color_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "color", GrSkSLFP::SpecializeIf(specialize, color));
    ...
    FpAndKey uRed = make_color_fp({ 1, 0, 0, 1 }, false), uGreen = make_color_fp({ 0, 1, 0, 1 }, false), sRed = make_color_fp({ 1, 0, 0, 1 }, true),
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrSkSLFP_UniformArray, r, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    std::unique_ptr<skgpu::v1::SurfaceFillContext> testCtx = directContext->priv().makeSFC(info, /*label=*/ {}, SkBackingFit::kExact);
    ...
    for (const auto& colorArray : { kRed, kGreen, kBlue, kGray }) {
        ...
        testCtx->fillWithFP(GrSkSLFP::Make(effect, "test_fp", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "color", SkSpan(colorArray)));
        ...
        if (actual != GrColorPackRGBA(255 * colorArray[0], 255 * colorArray[1], 255 * colorArray[2], 255 * colorArray[3])) {
