...
template <typename Fn> static void test_jit_and_interpreter(const skvm::Builder& b, Fn&& test)
{
...
DEF_TEST(SkVM_select_is_NaN, r)
{
    ...
    test_jit_and_interpreter(b, [&](const skvm::Program& program) {
        // ±NaN, ±0, ±1, ±inf
        uint32_t src[] = { 0x7f80'0001, 0xff80'0001, 0x0000'0000, 0x8000'0000, 0x3f80'0000, 0xbf80'0000, 0x7f80'0000, 0xff80'0000 };
...
DEF_TEST(SkVM_cmp_i32, r)
{
    skvm::Builder b;
    {
        ...
        auto to_bit = [&](int shift, skvm::I32 mask) { return b.shl(b.bit_and(mask, b.splat(0x1)), shift); };
...
DEF_TEST(SkVM_mad, r)
{
    ...
    skvm::Builder b;
    {
        ...
        skvm::F32 x = b.to_F32(b.load32(arg)), y = b.mad(x, x, x), // x is needed in the future, so r[x] != r[y].
...
DEF_TEST(SkVM_trace_line, r)
{
    ...
    public:
        void var(int, int32_t) override
        {
            fBuffer.push_back(-9999999);
        }
        ...
        void scope(int) override
        {
            fBuffer.push_back(-9999999);
        }
        void line(int lineNum) override
        {
            fBuffer.push_back(lineNum);
        }
...
template <typename Fn> static void test_asm(skiatest::Reporter* r, Fn&& fn, std::initializer_list<uint8_t> expected)
{
    uint8_t buf[4096];
...
DEF_TEST(SkVM_Assembler, r)
{
    ...
    test_asm(r, [&](A& a) { a.vpblendvb(A::ymm0, A::ymm1, A::ymm2, A::ymm3); },
        {
            0xc4,
            0xe3,
            0x75,
    ...
    test_asm(r,
        ...
        },
        {
            ...
            0xff,
            0xff,
            0xff, // near jne -6 bytes
            0x0f,
            0x85,
            0xf4,
            0xff,
            0xff,
            0xff, // near jne -12 bytes
            0x0f,
            0x84,
            0xee,
            0xff,
            0xff,
            0xff, // near je  -18 bytes
            0xe9,
            0xe9,
            0xff,
            0xff,
            0xff, // near jmp -23 bytes
            0x0f,
            0x8c,
            0xe3,
            0xff,
            0xff,
            0xff, // near jl  -29 bytes
            0x0f,
            0x82,
            0xdd,
            0xff,
            0xff,
            0xff, // near jc  -35 bytes
    ...
    test_asm(r, [&](A& a) { a.vpandn(A::ymm3, A::ymm12, A::ymm2); },
        {
            0xc5,
            0x9d,
            0xdf,
...
DEF_TEST(SkVM_approx_math, r)
{
    ...
    auto compare = [r](int N, const float values[], const float expected[]) {
        for (int i = 0; i < N; ++i) {
            REPORTER_ASSERT(r, (values[i] == expected[i]) || SkScalarNearlyEqual(values[i], expected[i], 0.001f), "evaluated to %g, but expected %g", values[i],
                expected[i]);
        }
    };
    ...
    // powf -- 1^x
    {
        ...
        eval(N, exps, [](skvm::Builder* b, skvm::F32 exp) { return b->approx_powf(b->splat(1.0f), exp); });
    ...
    // powf -- 3^x
    {
        float exps[] = { -2, -1, 0, 1, 2 };
        constexpr int N = std::size(exps);
        eval(N, exps, [](skvm::Builder* b, skvm::F32 exp) { return b->approx_powf(b->splat(3.0f), exp); });
    ...
    // powf -- x^0.5
    {
        ...
        eval(N, bases, [](skvm::Builder* b, skvm::F32 base) { return b->approx_powf(base, b->splat(0.5f)); });
    ...
    // powf -- x^1
    {
        ...
        constexpr int N = std::size(bases);
        eval(N, bases, [](skvm::Builder* b, skvm::F32 base) { return b->approx_powf(base, b->splat(1.0f)); });
        const float expected[] = { 0, 1, 2, 3, 4 };
        compare(N, bases, expected);
    }
    // powf -- x^2
    {
        float bases[] = { 0, 1, 2, 3, 4 };
        constexpr int N = std::size(bases);
        eval(N, bases, [](skvm::Builder* b, skvm::F32 base) { return b->approx_powf(base, b->splat(2.0f)); });
    ...
    // asin, acos, atan
    {
        ...
        for (float y = -3; y <= 3; y += 1) {
            for (float x = -3; x <= 3; x += 1) {
                err += test2(y, x, atan2(y, x), tol, [](skvm::F32 y, skvm::F32 x) { return approx_atan2(y, x); });
            }
        }
        if ((false)) {
...
DEF_TEST(SkVM_min_max, r)
{
    ...
    // ±NaN, ±0, ±1, ±inf
    const uint32_t bits[] = { 0x7f80'0001, 0xff80'0001, 0x0000'0000, 0x8000'0000, 0x3f80'0000, 0xbf80'0000, 0x7f80'0000, 0xff80'0000 };
...
DEF_TEST(SkVM_64bit, r)
{
...

    {
        skvm::Builder b;
        {
            skvm::Ptr widePtr = b.varying<uint64_t>(), loPtr = b.varying<int>(), hiPtr = b.varying<int>();
...
DEF_TEST(SkVM_128bit, r)
{
    ...
    skvm::PixelFormat rgba_ffff = skvm::SkColorType_to_PixelFormat(kRGBA_F32_SkColorType), rgba_8888 = skvm::SkColorType_to_PixelFormat(kRGBA_8888_SkColorType);
...
DEF_TEST(SkVM_args, r)
{
    ...
    skvm::Builder b;
    {
        skvm::Ptr dst = b.varying<float>(), A = b.varying<float>(), B = b.varying<float>(), C = b.varying<float>(), D = b.varying<float>(),
                  E = b.varying<float>();
