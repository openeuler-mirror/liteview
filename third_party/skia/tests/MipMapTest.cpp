...
DEF_TEST(MipMap_ComputeLevelCount, reporter)
{
    const LevelCountScenario tests[] = { // Test mipmaps with negative sizes
        ...
        // 1
        { 1, 100, 6 }, { 100, 1, 6 }, { 1, 1, 0 },
        // 2
        { 2, 100, 6 }, { 100, 2, 6 }, { 2, 2, 1 },

        // Test a handful of boundaries such as 63x63 and 64x64
        { 63, 63, 5 }, { 64, 64, 6 }, { 127, 127, 6 }, { 128, 128, 7 }, { 255, 255, 7 }, { 256, 256, 8 },

        // Test different dimensions, such as 256x64
        { 64, 129, 7 }, { 255, 32, 7 }, { 500, 1000, 9 }
    };
...
DEF_TEST(MipMap_ComputeLevelSize, reporter)
{
    ...
    for (auto& currentTest : tests) {
        SkISize levelSize = SkMipmap::ComputeLevelSize(currentTest.fBaseWidth, currentTest.fBaseHeight, currentTest.fLevel);
...
static void fill_in_mips(SkMipmapBuilder* builder, sk_sp<SkImage> img)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        surf->getCanvas()->drawImageRect(img, SkRect::MakeIWH(pm.width(), pm.height()), SkSamplingOptions());
    }
}
