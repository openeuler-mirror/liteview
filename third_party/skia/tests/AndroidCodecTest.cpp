    ...
    for (const char* file : {
        ...
        for (SkISize size : downscales) {
            ...
            if (codec->getEncodedFormat() == SkEncodedImageFormat::kWEBP) {
                ...
                REPORTER_ASSERT(r, size.width() >= requested.width() && size.height() >= requested.height());
        ...
        for (int sampleSize : { 1, 2, 3, 4, 8, 16, 32 }) {
            ...
            if (sampledDims != size) {
                ERRORF(r,
                    ...
                    file, sampleSize, computedSampleSize, sampledDims.width(), sampledDims.height(), size.width(), size.height());
            }
        }
    }
}
