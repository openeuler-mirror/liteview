...
static GrProcessorSet::Analysis do_analysis(
    const GrXPFactory* xpf, const GrProcessorAnalysisColor& colorInput, GrProcessorAnalysisCoverage coverageInput, const GrCaps& caps)
{
    GrPaint paint;
    ...
    GrProcessorSet::Analysis analysis
        = procs.finalize(colorInput, coverageInput, nullptr, &GrUserStencilSettings::kUnused, caps, GrClampType::kAuto, &overrideColor);
    return analysis;
}
...
public:
    struct XPInfo {
        XPInfo(skiatest::Reporter* reporter, SkBlendMode xfermode, const GrCaps& caps, GrProcessorAnalysisColor inputColor,
            GrProcessorAnalysisCoverage inputCoverage)
        {
            ...
            sk_sp<const GrXferProcessor> xp(GrXPFactory::MakeXferProcessor(xpf, inputColor, inputCoverage, caps, GrClampType::kAuto));
            TEST_ASSERT(!analysis.requiresDstTexture()
                || (isLCD && !caps.shaderCaps()->fDstReadInShaderSupport && (SkBlendMode::kSrcOver != xfermode || !inputColor.isOpaque())));
            ...
            TEST_ASSERT(!xp->willReadDstColor() || (isLCD && (SkBlendMode::kSrcOver != xfermode || !inputColor.isOpaque())));
...
static void test_color_not_opaque_no_coverage(skiatest::Reporter* reporter, const GrCaps& caps)
{
    GrProcessorAnalysisColor inputColor(SkPMColor4f::FromBytes_RGBA(GrColorPackRGBA(229, 0, 154, 240)));
...
static void test_lcd_coverage_fallback_case(skiatest::Reporter* reporter, const GrCaps& caps)
{
    ...
    TEST_ASSERT(!(GrXPFactory::GetAnalysisProperties(xpf, color, coverage, caps, autoClamp) & GrXPFactory::AnalysisProperties::kRequiresDstTexture));
    sk_sp<const GrXferProcessor> xp_opaque(GrXPFactory::MakeXferProcessor(xpf, color, coverage, caps, autoClamp));
    if (!xp_opaque) {
    ...
    TEST_ASSERT(!(GrXPFactory::GetAnalysisProperties(xpf, color, coverage, caps, autoClamp) & GrXPFactory::AnalysisProperties::kRequiresDstTexture));
    sk_sp<const GrXferProcessor> xp(GrXPFactory::MakeXferProcessor(xpf, color, coverage, caps, autoClamp));
    if (!xp) {
        ERRORF(reporter, "Failed to create an XP with LCD coverage.");
        return;
    }
...
DEF_GANESH_TEST(PorterDuffNoDualSourceBlending, reporter, options, CtsEnforcement::kApiLevel_T)
{
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(ctx, 100, 100, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo);
    if (!mbet) {
    ...
    GrDstProxyView fakeDstProxyView;
    {
        sk_sp<GrTextureProxy> proxy
            = proxyProvider->wrapBackendTexture(mbet->texture(), kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRead_GrIOType, mbet->refCountedCallback());
        skgpu::Swizzle swizzle = caps.getReadSwizzle(mbet->texture().getBackendFormat(), GrColorType::kRGBA_8888);
        fakeDstProxyView.setProxyView({ std::move(proxy), kTopLeft_GrSurfaceOrigin, swizzle });
    }

    static const GrProcessorAnalysisColor colorInputs[] = { GrProcessorAnalysisColor::Opaque::kNo, GrProcessorAnalysisColor::Opaque::kYes,
    ...
    for (const auto& colorInput : colorInputs) {
        for (GrProcessorAnalysisCoverage coverageType : { GrProcessorAnalysisCoverage::kSingleChannel, GrProcessorAnalysisCoverage::kNone }) {
            for (int m = 0; m <= (int)SkBlendMode::kLastCoeffMode; m++) {
                SkBlendMode xfermode = static_cast<SkBlendMode>(m);
                const GrXPFactory* xpf = GrPorterDuffXPFactory::Get(xfermode);
                sk_sp<const GrXferProcessor> xp(GrXPFactory::MakeXferProcessor(xpf, colorInput, coverageType, caps, GrClampType::kAuto));
                if (!xp) {
