...
static void test_partial(skiatest::Reporter* r, const char* name, const sk_sp<SkData>& file, size_t minBytes, size_t increment)
{
    ...
    while (true) {
        const SkCodec::Result startResult = partialCodec->startIncrementalDecode(info, incremental.getPixels(), incremental.rowBytes());
...
DEF_TEST(Codec_requiredFrame, r)
{
    ...
    while (true) {
        ...
        for (; frameToCompare < partialInfo.size(); frameToCompare++) {
            REPORTER_ASSERT(r, partialInfo[frameToCompare].fRequiredFrame == frameInfo[frameToCompare].fRequiredFrame);
        }
...
DEF_TEST(Codec_partialAnim, r)
{
    ...
    for (size_t i = 0; true; i++) {
        ...
        opts.fFrameIndex = i;
        const SkCodec::Result result = fullCodec->getPixels(info, frame.getPixels(), frame.rowBytes(), &opts);
    ...
    std::unique_ptr<SkCodec> partialCodec(SkCodec::MakeFromStream(std::unique_ptr<SkStream>(haltingStream)));
    if (!partialCodec) {
        ERRORF(r, "Failed to create a partial codec from %s with %zu bytes out of %zu", path, frameByteCounts[0], file->size());
        return;
    }
    ...
    for (size_t i = 0; i < frames.size(); i++) {
        ...
        SkCodec::Options opts;
        opts.fFrameIndex = i;
        SkCodec::Result result = partialCodec->startIncrementalDecode(info, frame.getPixels(), frame.rowBytes(), &opts);
        if (result != SkCodec::kSuccess) {
...
static void test_interleaved(skiatest::Reporter* r, const char* name)
{
    ...
    std::unique_ptr<SkCodec> partialCodec(SkCodec::MakeFromStream(std::make_unique<HaltingStream>(std::move(file), halfSize)));
    if (!partialCodec) {
...
DEF_TEST(Codec_incomplete, r)
{
    for (const char* name : {
        ...
        for (size_t len = 14; len <= file->size(); len += 5) {
            ...
            std::unique_ptr<SkCodec> codec(SkCodec::MakeFromStream(std::make_unique<SkMemoryStream>(file->data(), len), &result));
            if (codec) {
                if (result != SkCodec::kSuccess) {
                    ERRORF(r,
