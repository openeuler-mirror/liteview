...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(RepeatedClippedBlurTest, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    // Shrink the album artwork down to the screen's size
    {
        ...
        sk_sp<SkSurface> s = SkSurface::MakeRenderTarget(dContext, SkBudgeted::kYes, screenII, 1, kTopLeft_GrSurfaceOrigin, nullptr);
    ...
    for (int i = 0; i < 30; ++i) {
        ...
        SkIRect outSubset;
        SkIPoint offset;
        sk_sp<SkImage> filteredImg = smImg->makeWithFilter(dContext, blur.get(), subset, clip, &outSubset, &offset);

        SkRect dstRect = SkRect::MakeXYWH(offset.fX, offset.fY, outSubset.width(), outSubset.height());
        dstCanvas->drawImageRect(filteredImg, SkRect::Make(outSubset), dstRect, SkSamplingOptions(), nullptr, SkCanvas::kStrict_SrcRectConstraint);
