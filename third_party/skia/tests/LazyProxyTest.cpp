...
public:
    ...
    public:
        ...
        static GrOp::Owner Make(GrRecordingContext* context, GrProxyProvider* proxyProvider, LazyProxyTest* test, bool nullTexture)
        {
    ...
    private:
        ...
        Op(GrRecordingContext* ctx, GrProxyProvider* proxyProvider, LazyProxyTest* test, bool nullTexture)
            : GrDrawOp(ClassID())
            , fTest(test)
        {
            const GrBackendFormat format = ctx->priv().caps()->getDefaultBackendFormat(GrColorType::kBGR_565, GrRenderable::kNo);
            fProxy = GrProxyProvider::MakeFullyLazyProxy(
                [this, nullTexture](GrResourceProvider* rp, const GrSurfaceProxy::LazySurfaceDesc& desc) -> GrSurfaceProxy::LazyCallbackResult {
                    REPORTER_ASSERT(fTest->fReporter, !fTest->fHasOpTexture);
                    ...
                    if (nullTexture) {
                        ...
                        sk_sp<GrTexture> texture = rp->createTexture(kDimensions, desc.fFormat, desc.fTextureType, desc.fRenderable, desc.fSampleCnt,
                ...
                format, GrRenderable::kNo, 1, GrProtected::kNo, *proxyProvider->caps(), GrSurfaceProxy::UseAllocator::kYes);
    ...
    public:
            ...
            , fAtlas(atlas)
        {
            ...
            const GrBackendFormat format = ctx->priv().caps()->getDefaultBackendFormat(kColorType, GrRenderable::kYes);
            skgpu::Swizzle readSwizzle = ctx->priv().caps()->getReadSwizzle(format, kColorType);
            fLazyProxy = GrProxyProvider::MakeFullyLazyProxy(
                [this](GrResourceProvider* rp, const GrSurfaceProxy::LazySurfaceDesc&) -> GrSurfaceProxy::LazyCallbackResult {
                    REPORTER_ASSERT(fTest->fReporter, !fTest->fHasClipTexture);
                    fTest->fHasClipTexture = true;
                    fAtlas->instantiate(rp);
                    return sk_ref_sp(fAtlas->peekTexture());
                },
                format, GrRenderable::kYes, 1, GrProtected::kNo, *proxyProvider->caps(), GrSurfaceProxy::UseAllocator::kYes);
            auto atlasEffect = GrTextureEffect::Make({ fLazyProxy, kOrigin, readSwizzle }, kPremul_SkAlphaType);
    ...
    private:
        ...
        Effect apply(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext*, GrDrawOp*, GrAAType, GrAppliedClip* out, SkRect* bounds) const override
        {
...
DEF_GANESH_TEST(LazyProxyTest, reporter, /* options */, CtsEnforcement::kApiLevel_T)
{
    ...
    mockOptions.fConfigOptions[(int)GrColorType::kAlpha_F16].fRenderability = GrMockOptions::ConfigOptions::Renderability::kNonMSAA;
    ...
    for (bool nullTexture : { false, true }) {
        ...
        auto sdc = skgpu::v1::SurfaceDrawContext::Make(
            ctx.get(), GrColorType::kRGBA_8888, nullptr, SkBackingFit::kExact, { 100, 100 }, SkSurfaceProps(), /*label=*/ {});
        REPORTER_ASSERT(reporter, sdc);
        auto mockAtlas = skgpu::v1::SurfaceDrawContext::Make(ctx.get(), GrColorType::kAlpha_F16, nullptr, SkBackingFit::kExact, { 10, 10 }, SkSurfaceProps(),
            /*label=*/ {});
...
DEF_GANESH_TEST(LazyProxyReleaseTest, reporter, /* options */, CtsEnforcement::kApiLevel_T)
{
    ...
    GrBackendFormat format = caps->getDefaultBackendFormat(GrColorType::kRGBA_8888, GrRenderable::kNo);

    auto tex = ctx->priv().resourceProvider()->createTexture({ kSize, kSize }, format, GrTextureType::k2D, GrRenderable::kNo, 1, GrMipmapped::kNo,
    ...
    for (bool doInstantiate : { true, false }) {
        for (bool releaseCallback : { false, true }) {
            ...
            public:
                ...
                TestCallback(const TestCallback& that)
                {
                    SkASSERT(0);
                }
                ...
                LazyInstantiationResult operator()(GrResourceProvider*, const GrSurfaceProxy::LazySurfaceDesc&) const
                {
                    *fValue = 1;
            ...
            sk_sp<GrTextureProxy> proxy = proxyProvider->createLazyProxy(TestCallback(&testCount, releaseCallback, tex), format, { kSize, kSize },
                GrMipmapped::kNo, GrMipmapStatus::kNotAllocated, GrInternalSurfaceFlags::kNone, SkBackingFit::kExact, SkBudgeted::kNo, GrProtected::kNo,
...
public:
    ...
    static GrOp::Owner Make(GrRecordingContext* rContext, GrProxyProvider* proxyProvider, int* testExecuteValue, bool shouldFailInstantiation)
    {
        return GrOp::Make<LazyFailedInstantiationTestOp>(rContext, rContext->priv().caps(), proxyProvider, testExecuteValue, shouldFailInstantiation);
    }
...
private:
    ...
    LazyFailedInstantiationTestOp(const GrCaps* caps, GrProxyProvider* proxyProvider, int* testExecuteValue, bool shouldFailInstantiation)
        : INHERITED(ClassID())
        , fTestExecuteValue(testExecuteValue)
    {
        ...
        fLazyProxy = proxyProvider->createLazyProxy(
            [testExecuteValue, shouldFailInstantiation](
                GrResourceProvider* rp, const GrSurfaceProxy::LazySurfaceDesc& desc) -> GrSurfaceProxy::LazyCallbackResult {
                if (shouldFailInstantiation) {
                    *testExecuteValue = 1;
                    return {};
                }
                return { rp->createTexture(desc.fDimensions, desc.fFormat, desc.fTextureType, desc.fRenderable, desc.fSampleCnt, desc.fMipmapped,
                             desc.fBudgeted, desc.fProtected,
                             /*label=*/ {}),
                    true, GrSurfaceProxy::LazyInstantiationKeyMode::kUnsynced };
            },
            format, dims, GrMipmapped::kNo, GrMipmapStatus::kNotAllocated, GrInternalSurfaceFlags::kNone, SkBackingFit::kExact, SkBudgeted::kNo,
...
// associated with.
DEF_GANESH_TEST(LazyProxyFailedInstantiationTest, reporter,
    /* options */, CtsEnforcement::kApiLevel_T)
{
    ...
    for (bool failInstantiation : { false, true }) {
        ...
        sdc->addDrawOp(LazyFailedInstantiationTestOp::Make(ctx.get(), proxyProvider, &executeTestValue, failInstantiation));
        ctx->flushAndSubmit();
