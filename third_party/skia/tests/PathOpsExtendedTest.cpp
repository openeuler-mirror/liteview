...
template <typename T> std::string std_to_string(T value)
{
...
static int pathsDrawTheSame(SkBitmap& bits, const SkPath& scaledOne, const SkPath& scaledTwo, int& error2x2)
{
    ...
    for (int y = 0; y < kBitHeight - 1; ++y) {
        ...
        for (int x = 0; x < kBitWidth - 1; ++x) {
            ...
            if (err) {
                errors2 += addr1[x + 1] != addr3[x + 1] && addr2[x] != addr4[x] && addr2[x + 1] != addr4[x + 1];
                errors++;
            }
        }
    }
...
static void showPathOpPath(const char* testName, const SkPath& one, const SkPath& two, const SkPath& a, const SkPath& b, const SkPath& scaledOne,
    const SkPath& scaledTwo, const SkPathOp shapeOp, const SkMatrix& scale)
{
...
static int comparePaths(skiatest::Reporter* reporter, const char* testName, const SkPath& one, const SkPath& scaledOne, const SkPath& two,
    const SkPath& scaledTwo, SkBitmap& bitmap, const SkPath& a, const SkPath& b, const SkPathOp shapeOp, const SkMatrix& scale, ExpectMatch expectMatch)
{
...
static void json_path_out(const SkPath& path, const char* pathName, const char* fillTypeName, bool lastField)
{
    ...
    if (PathOpsDebug::gOutputSVG) {
        ...
        for (auto [verb, points, w] : SkPathPriv::Iterate(path)) {
            ...
            for (int i = pointIndex[verbIndex]; i < pointCount[verbIndex]; ++i) {
                fprintf(PathOpsDebug::gOut, ", \"0x%08x\", \"0x%08x\"", SkFloat2Bits(points[i].fX), SkFloat2Bits(points[i].fY));
            }
    ...
    fprintf(PathOpsDebug::gOut, "  \"fillType%s\": \"k%s_FillType\"%s", fillTypeName, gFillTypeStrs[(int)path.getFillType()], lastField ? "\n}" : ",\n");
}

static bool check_for_duplicate_names(const char* testName)
{
    if (PathOpsDebug::gCheckForDuplicateNames) {
        if (gUniqueNames.end() != std::find(gUniqueNames.begin(), gUniqueNames.end(), std::string(testName))) {
...
static bool inner_simplify(
    skiatest::Reporter* reporter, const SkPath& path, const char* filename, ExpectSuccess expectSuccess, SkipAssert skipAssert, ExpectMatch expectMatch)
{
    if (PathOpsDebug::gJson) {
    ...
    SkPath out;
    if (!SimplifyDebug(path, &out SkDEBUGPARAMS(SkipAssert::kYes == skipAssert) SkDEBUGPARAMS(sTestName))) {
...
{
    return inner_simplify(reporter, path, filename, ExpectSuccess::kYes, SkipAssert::kNo, ExpectMatch::kYes);
}

bool testSimplifyFuzz(skiatest::Reporter* reporter, const SkPath& path, const char* filename)
{
    return inner_simplify(reporter, path, filename, ExpectSuccess::kFlaky, SkipAssert::kYes, ExpectMatch::kFlaky);
}

bool testSimplifyCheck(skiatest::Reporter* reporter, const SkPath& path, const char* filename, bool checkFail)
{
    return inner_simplify(reporter, path, filename, checkFail ? ExpectSuccess::kYes : ExpectSuccess::kNo, SkipAssert::kNo, ExpectMatch::kNo);
}

bool testSimplifyFail(skiatest::Reporter* reporter, const SkPath& path, const char* filename)
{
    return inner_simplify(reporter, path, filename, ExpectSuccess::kNo, SkipAssert::kYes, ExpectMatch::kNo);
}

static bool innerPathOp(skiatest::Reporter* reporter, const SkPath& a, const SkPath& b, const SkPathOp shapeOp, const char* testName,
    ExpectSuccess expectSuccess, SkipAssert skipAssert, ExpectMatch expectMatch)
{
    ...
    SkPath out;
    if (!OpDebug(a, b, shapeOp, &out SkDEBUGPARAMS(SkipAssert::kYes == skipAssert) SkDEBUGPARAMS(testName))) {
        if (ExpectSuccess::kYes == expectSuccess) {
    ...
    int result = comparePaths(reporter, testName, pathOut, scaledPathOut, out, scaledOut, bitmap, a, b, shapeOp, scale, expectMatch);
    reporter->bumpTestCount();
    return result == 0;
}
...
{
    return innerPathOp(reporter, a, b, shapeOp, testName, ExpectSuccess::kYes, SkipAssert::kNo, ExpectMatch::kYes);
}
...
{
    return innerPathOp(reporter, a, b, shapeOp, testName, checkFail ? ExpectSuccess::kYes : ExpectSuccess::kNo, SkipAssert::kNo, ExpectMatch::kNo);
}

bool testPathOpFuzz(skiatest::Reporter* reporter, const SkPath& a, const SkPath& b, const SkPathOp shapeOp, const char* testName)
{
    return innerPathOp(reporter, a, b, shapeOp, testName, ExpectSuccess::kFlaky, SkipAssert::kYes, ExpectMatch::kFlaky);
}
...
void RunTestSet(skiatest::Reporter* reporter, TestDesc tests[], size_t count, void (*firstTest)(skiatest::Reporter*, const char* filename),
    void (*skipTest)(skiatest::Reporter*, const char* filename), void (*stopTest)(skiatest::Reporter*, const char* filename), bool reverse)
{
    size_t index;
    if (firstTest) {
        index = count - 1;
