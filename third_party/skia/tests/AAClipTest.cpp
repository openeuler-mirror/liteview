...
static void copyToMask(const SkRegion& rgn, SkMask* mask)
{
    ...
    SkImageInfo info = SkImageInfo::Make(mask->fBounds.width(), mask->fBounds.height(), kAlpha_8_SkColorType, kPremul_SkAlphaType);
    SkBitmap bitmap;
...
{
    path.cubicTo(SkIntToScalar(x0), SkIntToScalar(y0), SkIntToScalar(x1), SkIntToScalar(y1), SkIntToScalar(x2), SkIntToScalar(y2));
}
        ...
        for (SkClipOp op : { SkClipOp::kDifference, SkClipOp::kIntersect }) {
            ...
            if (nonEmptyAA != nonEmptyBW || clip2.getBounds() != rgn2.getBounds()) {
                ERRORF(reporter,
                    ...
                    nonEmptyAA == nonEmptyBW ? "true" : "false", clip2.getBounds() == rgn2.getBounds() ? "true" : "false", r0.fLeft, r0.fTop, r0.right(),
                    r0.bottom(), op == SkClipOp::kDifference ? "DIFF" : "INTERSECT", r1.fLeft, r1.fTop, r1.right(), r1.bottom(), rgn2.getBounds().fLeft,
                    rgn2.getBounds().fTop, rgn2.getBounds().right(), rgn2.getBounds().bottom(), clip2.getBounds().fLeft, clip2.getBounds().fTop,
