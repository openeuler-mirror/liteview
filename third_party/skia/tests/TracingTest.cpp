...
struct TracingShape {
    ...
    void traceSnapshot()
    {
        ...
        TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID("skia.objects", this->typeName(), this, TRACE_STR_COPY(this->toString().c_str()));
    }
    ...
    {
        return SkString("Shape()");
    }
};

struct TracingCircle : public TracingShape {
    TracingCircle(SkPoint center, SkScalar radius)
        : fCenter(center)
        , fRadius(radius)
    {
    }
...
struct TracingRect : public TracingShape {
    ...
    SkString toString() override
    {
        return SkStringPrintf("Rect(%f, %f, %f, %f)", fRect.fLeft, fRect.fTop, fRect.fRight, fRect.fBottom);
    }
...
static void test_trace_counters()
{
...

    {
        ...
        for (int i = 0; i < 180; ++i) {
            SkScalar rad = SkDegreesToRadians(SkIntToScalar(i));
            TRACE_COUNTER2("skia", "trig", "sin", SkScalarSin(rad) * 1000.0f + 1000.0f, "cos", SkScalarCos(rad) * 1000.0f + 1000.0f);
            do_work(10);
        }
    }
}
