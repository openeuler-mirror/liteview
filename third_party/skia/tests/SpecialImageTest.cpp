...
{
    SkImageInfo ii = SkImageInfo::Make(kFullSize, kFullSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    temp.drawRect(SkRect::MakeXYWH(SkIntToScalar(kPad), SkIntToScalar(kPad), SkIntToScalar(kSmallerSize), SkIntToScalar(kSmallerSize)), p);
...
static void test_image(const sk_sp<SkSpecialImage>& img, skiatest::Reporter* reporter, GrRecordingContext* rContext, bool isGPUBacked)
{
    ...
    sk_sp<SkSpecialSurface> surf(
        img->makeSurface(kN32_SkColorType, img->getColorSpace(), SkISize::Make(kFullSize, kFullSize), kPremul_SkAlphaType, SkSurfaceProps()));
    ...
    {
        sk_sp<SkSurface> tightSurf(img->makeTightSurface(kN32_SkColorType, img->getColorSpace(), subset.size()));

        REPORTER_ASSERT(reporter, tightSurf->width() == subset.width());
        REPORTER_ASSERT(reporter, tightSurf->height() == subset.height());
        GrBackendTexture backendTex = tightSurf->getBackendTexture(SkSurface::kDiscardWrite_BackendHandleAccess);
...
DEF_TEST(SpecialImage_Raster, reporter)
{
    ...
    sk_sp<SkSpecialImage> fullSImage(SkSpecialImage::MakeFromRaster(SkIRect::MakeWH(kFullSize, kFullSize), bm, SkSurfaceProps()));
...
static void test_specialimage_image(skiatest::Reporter* reporter)
{
    ...
    sk_sp<SkSpecialImage> fullSImage(SkSpecialImage::MakeFromImage(nullptr, SkIRect::MakeWH(kFullSize, kFullSize), fullImage, SkSurfaceProps()));

    const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

    {
        sk_sp<SkSpecialImage> subSImg1(SkSpecialImage::MakeFromImage(nullptr, subset, fullImage, SkSurfaceProps()));
        test_image(subSImg1, reporter, nullptr, false);
    }
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SpecialImage_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    sk_sp<SkSpecialImage> fullSImg = SkSpecialImage::MakeDeferredFromGpu(
        context, SkIRect::MakeWH(kFullSize, kFullSize), kNeedNewImageUniqueID_SpecialImage, view, { ct, kPremul_SkAlphaType, nullptr }, SkSurfaceProps());

    const SkIRect& subset = SkIRect::MakeXYWH(kPad, kPad, kSmallerSize, kSmallerSize);

    {
        sk_sp<SkSpecialImage> subSImg1 = SkSpecialImage::MakeDeferredFromGpu(
            context, subset, kNeedNewImageUniqueID_SpecialImage, std::move(view), { ct, kPremul_SkAlphaType, nullptr }, SkSurfaceProps());
