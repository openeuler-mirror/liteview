...
static void ValidateLoadedFonts(SkTDArray<FontFamily*> fontFamilies, const char* firstExpectedFile, skiatest::Reporter* reporter)
{
    ...
    REPORTER_ASSERT(reporter, !strcmp(fontFamilies[0]->fFonts[0].fFileName.c_str(), firstExpectedFile));
...
static void DumpFiles(const FontFamily& fontFamily)
{
    for (int j = 0; j < fontFamily.fFonts.count(); ++j) {
        ...
        for (const auto& coordinate : ffi.fVariationDesignPosition) {
            SkDebugf(" @'%c%c%c%c'=%f", (coordinate.axis >> 24) & 0xFF, (coordinate.axis >> 16) & 0xFF, (coordinate.axis >> 8) & 0xFF, (coordinate.axis) & 0xFF,
                coordinate.value);
        }
        SkDebugf("\n");
    }
...
template <int N, typename T> static double test_parse_fixed_r(skiatest::Reporter* reporter, double low, double high, double inc)
{
...
DEF_TEST(FontMgrAndroidParser, reporter)
{
    ...
    SkFontMgr_Android_Parser::GetCustomFontFamilies(preV17FontFamilies, SkString("/custom/font/path/"),
        GetResourcePath("android_fonts/pre_v17/system_fonts.xml").c_str(), GetResourcePath("android_fonts/pre_v17/fallback_fonts.xml").c_str());
    ...
    SkFontMgr_Android_Parser::GetCustomFontFamilies(v17FontFamilies, SkString("/custom/font/path/"),
        GetResourcePath("android_fonts/v17/system_fonts.xml").c_str(), GetResourcePath("android_fonts/v17/fallback_fonts.xml").c_str(),
    ...
    SkFontMgr_Android_Parser::GetCustomFontFamilies(
        v22FontFamilies, SkString("/custom/font/path/"), GetResourcePath("android_fonts/v22/fonts.xml").c_str(), nullptr);
...
DEF_TEST(FontMgrAndroidSystemVariableTypeface, reporter)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        SkFontArguments::VariationPosition position = { coordinates, std::size(coordinates) };

        SkFont fontStream(fontMgr->makeFromStream(distortableStream->duplicate(), SkFontArguments().setVariationDesignPosition(position)), kTextSize);
        fontStream.setEdging(SkFont::Edging::kSubpixelAntiAlias);

        SkFont fontClone(typeface->makeClone(SkFontArguments().setVariationDesignPosition(position)), kTextSize);
...
DEF_TEST(FontMgrAndroidSystemFallbackFor, reporter)
{
    ...
    sk_sp<SkTypeface> typeface(fontMgr->matchFamilyStyleCharacter("sans-serif", SkFontStyle(), nullptr, 0, '!'));
