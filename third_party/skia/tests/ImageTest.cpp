...
static sk_sp<SkImage> create_picture_image()
{
    ...
    return SkImage::MakeFromPicture(
        recorder.finishRecordingAsPicture(), SkISize::Make(10, 10), nullptr, nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
};
...
static sk_sp<SkImage> create_rasterproc_image(RasterDataHolder* dataHolder)
{
    ...
    return SkImage::MakeFromRaster(SkPixmap(info, dataHolder->fData->data(), info.minRowBytes()), RasterDataHolder::Release, dataHolder);
}
...
static sk_sp<SkImage> create_gpu_image(GrRecordingContext* rContext, bool withMips = false, SkBudgeted budgeted = SkBudgeted::kYes)
{
    const SkImageInfo info = SkImageInfo::MakeN32(20, 20, kOpaque_SkAlphaType);
    auto surface = SkSurface::MakeRenderTarget(rContext, budgeted, info, 0, kBottomLeft_GrSurfaceOrigin, nullptr, withMips);
    draw_image_test_pattern(surface->getCanvas());
    return surface->makeImageSnapshot();
}
 ...
 */
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkImage_Gpu2Cpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkImage_makeTextureImage, reporter, contextInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    std::function<sk_sp<SkImage>()> imageFactories[] = { create_image, create_codec_image, create_data_image,
        // Create an image from a picture.
        create_picture_image,
        // Create a texture image.
        [dContext] { return create_gpu_image(dContext, true, SkBudgeted::kYes); }, [dContext] { return create_gpu_image(dContext, false, SkBudgeted::kNo); },
    ...
    for (auto mipmapped : { GrMipmapped::kNo, GrMipmapped::kYes }) {
        for (const auto& factory : imageFactories) {
            ...
            if ((origProxy = sk_gpu_test::GetTextureImageProxy(image.get(), dContext))) {
                REPORTER_ASSERT(reporter, (origProxy->mipmapped() == GrMipmapped::kYes) == image->hasMipmaps());
            ...
            for (auto budgeted : { SkBudgeted::kNo, SkBudgeted::kYes }) {
                ...
                bool validRequestForMips = (mipmapped == GrMipmapped::kYes && dContext->priv().caps()->mipmapSupport());
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrContext_colorTypeSupportedAsImage, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int ct = 0; ct < kLastEnum_SkColorType; ++ct) {
        ...
        auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(dContext, kSize, kSize, colorType, GrMipmapped::kNo, GrRenderable::kNo);
        sk_sp<SkImage> img;
        if (mbet) {
            img = SkImage::MakeFromTexture(dContext, mbet->texture(), kTopLeft_GrSurfaceOrigin, colorType, kOpaque_SkAlphaType, nullptr);
        }
        REPORTER_ASSERT(reporter, can == SkToBool(img), "colorTypeSupportedAsImage:%d, actual:%d, ct:%d", can, SkToBool(img), colorType);
    }
}
...
{
    SkBitmap bmp;
    bmp.allocPixels(SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType, nullptr));
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
    ...
    SkBitmap unpremul;
    unpremul.allocPixels(SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType, nullptr));
    if (!texImage->readPixels(dContext, unpremul.info(), unpremul.getPixels(), unpremul.rowBytes(), 0, 0)) {
    ...
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            if (*bmp.getAddr32(x, y) != *unpremul.getAddr32(x, y)) {
                ERRORF(reporter, "unpremul(0x%08x)->unpremul(0x%08x) at %d, %d.", *bmp.getAddr32(x, y), *unpremul.getAddr32(x, y), x, y);
                return;
            }
        }
    }
    SkBitmap premul;
    premul.allocPixels(SkImageInfo::Make(256, 256, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr));
    ...
    for (int y = 0; y < 256; ++y) {
        for (int x = 0; x < 256; ++x) {
            ...
            if (origA != readA || SkTAbs(readB - origB) > tol || SkTAbs(readG - origG) > tol || SkTAbs(readR - origR) > tol) {
                ERRORF(
                    reporter, "unpremul(0x%08x)->premul(0x%08x) expected(0x%08x) at %d, %d.", *bmp.getAddr32(x, y), *premul.getAddr32(x, y), origColor, x, y);
                return;
            }
        }
    }
...
DEF_GANESH_TEST(AbandonedContextImage, reporter, options, CtsEnforcement::kApiLevel_T)
{
    ...
    for (int ct = 0; ct < Factory::kContextTypeCnt; ++ct) {
        ...
        auto gsurf = SkSurface::MakeRenderTarget(
            factory->get(type), SkBudgeted::kYes, SkImageInfo::Make(100, 100, kRGBA_8888_SkColorType, kPremul_SkAlphaType), 1, nullptr);
        if (!gsurf) {
            continue;
        }
...
DEF_TEST(ImageEmpty, reporter)
{
    ...
    REPORTER_ASSERT(reporter, nullptr == SkImage::MakeFromGenerator(std::make_unique<EmptyGenerator>()));
}
...
static void image_test_read_pixels(GrDirectContext* dContext, skiatest::Reporter* reporter, SkImage* image)
{
    ...
    REPORTER_ASSERT(reporter, !image->readPixels(dContext, info, pixels, rowBytes, image->width(), 0));
    REPORTER_ASSERT(reporter, !image->readPixels(dContext, info, pixels, rowBytes, 0, image->height()));
    ...
    sk_memset32(pixels, notExpected, w * h);
    REPORTER_ASSERT(reporter, image->readPixels(dContext, info, pixels, rowBytes, image->width() - w, image->height() - h));
    REPORTER_ASSERT(reporter, has_pixels(pixels, w * h, expected));
    ...
    sk_memset32(pixels, notExpected, w * h);
    REPORTER_ASSERT(reporter, image->readPixels(dContext, info, pixels, rowBytes, image->width() - 1, image->height() - 1));
...
static void check_legacy_bitmap(skiatest::Reporter* reporter, GrDirectContext* dContext, const SkImage* image, const SkBitmap& bitmap)
{
    ...
    REPORTER_ASSERT(reporter, image->readPixels(dContext, info, &imageColor, sizeof(SkPMColor), 0, 0));
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(SkImage_NewFromTextureRelease, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    const int kWidth = 10;
    ...
    auto mbet = sk_gpu_test::ManagedBackendTexture::MakeWithoutData(
        dContext, kWidth, kHeight, kRGBA_8888_SkColorType, GrMipmapped::kNo, GrRenderable::kNo, GrProtected::kNo);
    if (!mbet) {
    ...
    sk_sp<SkImage> refImg = SkImage::MakeFromTexture(dContext, mbet->texture(), texOrigin, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
        /*color space*/ nullptr, sk_gpu_test::ManagedBackendTexture::ReleaseProc, mbet->releaseContext(TextureReleaseChecker::Release, &releaseChecker));
    ...
    REPORTER_ASSERT(reporter, GrBackendTexture::TestingOnly_Equals(readBackBackendTex, mbet->texture()));
...
static void test_cross_context_image(
    skiatest::Reporter* reporter, const GrContextOptions& options, const char* testName, std::function<sk_sp<SkImage>(GrDirectContext*)> imageMaker)
{
...
DEF_GANESH_TEST(SkImage_MakeCrossContextFromPixmapRelease, reporter, options, CtsEnforcement::kApiLevel_T)
{
    ...
    test_cross_context_image(reporter, options, "SkImage_MakeCrossContextFromPixmapRelease",
        [&pixmap](GrDirectContext* dContext) { return SkImage::MakeCrossContextFromPixmap(dContext, pixmap, false); });
}
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(makeBackendTexture, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto createLarge = [context] { return create_image_large(context->priv().caps()->maxTextureSize()); };
    struct TestCase {
        std::function<sk_sp<SkImage>()> fImageFactory;
        bool fExpectation;
        bool fCanTakeDirectly;
    };
    TestCase testCases[] = { { create_image, true, false }, { create_codec_image, true, false }, { create_data_image, true, false },
        { create_picture_image, true, false }, { [context] { return create_gpu_image(context); }, true, true },
        // Create a texture image in a another context.
        { [otherContextInfo] {
             auto restore = otherContextInfo.testContext()->makeCurrentAndAutoRestore();
    ...
    for (const TestCase& testCase : testCases) {
        ...
        bool result = SkImage::MakeBackendTextureFromSkImage(context, std::move(image), &newBackend, &proc);
        ...
        if (testCase.fCanTakeDirectly != tookDirectly) {
            ...
            ERRORF(reporter, "This backend texture was %s to be taken directly.", kExpectedState[testCase.fCanTakeDirectly]);
        }
...
static sk_sp<SkImage> create_picture_image(sk_sp<SkColorSpace> space)
{
    ...
    SkCanvas* canvas = recorder.beginRecording(10, 10);
    canvas->clear(SK_ColorCYAN);
    return SkImage::MakeFromPicture(recorder.finishRecordingAsPicture(), SkISize::Make(10, 10), nullptr, nullptr, SkImage::BitDepth::kU8, std::move(space));
};
...
DEF_TEST(Image_ColorSpace, r)
{
    ...
    sk_sp<SkColorSpace> rec2020 = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kRec2020);
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRaster(SkImageInfo::MakeN32Premul(SkISize::Make(10, 10)));
...
static void check_scaled_pixels(skiatest::Reporter* reporter, SkPixmap* pmap, uint32_t expected)
{
    ...
    for (auto y = 0; y < pmap->height(); ++y) {
        for (auto x = 0; x < pmap->width(); ++x) {
            ...
            if (pixel != expected) {
                ERRORF(reporter, "Expected scaled pixels to be the same. At %d,%d 0x%08x != 0x%08x", x, y, pixel, expected);
                return;
            }
        }
    }
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(ImageScalePixels_Gpu, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(ctxInfo.directContext(), SkBudgeted::kNo, info);
    surface->getCanvas()->clear(red);
...
static sk_sp<SkImage> make_yuva_image(GrDirectContext* dContext)
{
    ...
    SkYUVAInfo yuvaInfo({ 1, 1 }, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k444, kJPEG_Full_SkYUVColorSpace);
...
DEF_GANESH_TEST_FOR_ALL_CONTEXTS(ImageFlush, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    dContext->flushAndSubmit();
    auto numSubmits = [dContext, submitCnt = dContext->priv().getGpu()->stats()->numSubmitToGpus()]() mutable {
...
constexpr SkM44 gCentripetalCatmulRom(0.0f / 2, -1.0f / 2, 2.0f / 2, -1.0f / 2, 2.0f / 2, 0.0f / 2, -5.0f / 2, 3.0f / 2, 0.0f / 2, 1.0f / 2, 4.0f / 2,
    -3.0f / 2, 0.0f / 2, 0.0f / 2, -1.0f / 2, 1.0f / 2);

constexpr SkM44 gMitchellNetravali(1.0f / 18, -9.0f / 18, 15.0f / 18, -7.0f / 18, 16.0f / 18, 0.0f / 18, -36.0f / 18, 21.0f / 18, 1.0f / 18, 9.0f / 18,
    27.0f / 18, -21.0f / 18, 0.0f / 18, 0.0f / 18, -6.0f / 18, 7.0f / 18);
