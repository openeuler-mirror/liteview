...
SkBitmap RasterBlob(
    sk_sp<SkTextBlob> blob, int width, int height, const SkPaint& paint, GrRecordingContext* rContext, const SkMatrix* matrix = nullptr, SkScalar x = 0)
{
...
SkBitmap RasterSlug(
    sk_sp<Slug> slug, int width, int height, const SkPaint& paint, GrRecordingContext* rContext, const SkMatrix* matrix = nullptr, SkScalar x = 0)
{
    auto surface = MakeSurface(width, height, rContext);
    auto canvas = surface->getCanvas();
    if (matrix) {
...
DEF_TEST(SkRemoteGlyphCache_TypefaceSerialization, reporter)
{
    ...
    REPORTER_ASSERT(reporter, static_cast<SkTypefaceProxy*>(client_tf.get())->remoteTypefaceID() == server_tf->uniqueID());
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_StrikeSerialization, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    std::unique_ptr<SkCanvas> cache_diff_canvas
        = server.makeAnalysisCanvas(10, 10, props, nullptr, dContext->supportsDistanceFieldText(), !dContext->priv().caps()->disablePerspectiveSDFText());
    ...
    REPORTER_ASSERT(reporter, client.readStrikeData(serverStrikeData.data(), serverStrikeData.size()));
...
DEF_GANESH_TEST_FOR_CONTEXTS(SkRemoteGlyphCache_StrikeSerializationSlug, sk_gpu_test::GrContextFactory::IsRenderingContext, reporter, ctxInfo,
    use_padding_options, CtsEnforcement::kNever)
{
    auto dContext = ctxInfo.directContext();
    sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
    SkStrikeServer server(discardableManager.get());
...
DEF_GANESH_TEST_FOR_CONTEXTS(SkRemoteGlyphCache_StrikeSerializationSlugForcePath, sk_gpu_test::GrContextFactory::IsRenderingContext, reporter, ctxInfo,
    use_padding_options, CtsEnforcement::kNever)
{
    auto dContext = ctxInfo.directContext();
    sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
...
DEF_GANESH_TEST_FOR_CONTEXTS(
    SkRemoteGlyphCache_SlugSerialization, sk_gpu_test::GrContextFactory::IsRenderingContext, reporter, ctxInfo, use_padding_options, CtsEnforcement::kNever)
{
    auto dContext = ctxInfo.directContext();
    sk_sp<DiscardableManager> discardableManager = sk_make_sp<DiscardableManager>();
    SkStrikeServer server(discardableManager.get());
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_ReleaseTypeFace, reporter, ctxInfo, CtsEnforcement::kNever)
{
...

    {
        ...
        std::unique_ptr<SkCanvas> cache_diff_canvas = server.makeAnalysisCanvas(
            10, 10, props, nullptr, ctxInfo.directContext()->supportsDistanceFieldText(), !ctxInfo.directContext()->priv().caps()->disablePerspectiveSDFText());
        cache_diff_canvas->drawTextBlob(serverBlob.get(), 0, 0, paint);
        REPORTER_ASSERT(reporter, !serverTf->unique());
...
DEF_TEST(SkRemoteGlyphCache_StrikeLockingServer, reporter)
{
    ...
    const SkSurfaceProps props;
    std::unique_ptr<SkCanvas> cache_diff_canvas = server.makeAnalysisCanvas(10, 10, props, nullptr, true, true);
    SkPaint paint;
    cache_diff_canvas->drawTextBlob(serverBlob.get(), 0, 0, paint);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsPath, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    std::unique_ptr<SkCanvas> cache_diff_canvas
        = server.makeAnalysisCanvas(10, 10, props, nullptr, direct->supportsDistanceFieldText(), !direct->priv().caps()->disablePerspectiveSDFText());
    cache_diff_canvas->drawTextBlob(serverBlob.get(), 0, 0, paint);
...
sk_sp<SkTextBlob> make_blob_causing_fallback(sk_sp<SkTypeface> targetTf, const SkTypeface* glyphTf, skiatest::Reporter* reporter)
{
    ...
    SkFont(sk_ref_sp(glyphTf)).textToGlyphs(s, strlen(s), SkTextEncoding::kUTF8, runBuffer.glyphs, runSize);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_DrawTextAsDFT, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    sktext::gpu::SDFTControl control = direct->priv().asRecordingContext()->priv().getSDFTControl(true);
    SkScalar approximateDeviceTextSize = SkFontPriv::ApproximateTransformedTextSize(font, matrix, { 0, 0 });
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_CacheMissReporting, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    REPORTER_ASSERT(reporter, discardableManager->cacheMissCount(SkStrikeClient::kGlyphMetrics) == 10);
...
sk_sp<SkTextBlob> MakeEmojiBlob(sk_sp<SkTypeface> serverTf, SkScalar textSize, sk_sp<SkTypeface> clientTf = nullptr)
{
    ...
    s_procs.fTypefaceProc = [](SkTypeface*, void* ctx) -> sk_sp<SkData> { return SkData::MakeUninitialized(1u); };
    auto serialized = blob->serialize(s_procs);

    SkDeserialProcs d_procs;
    d_procs.fTypefaceCtx = &clientTf;
    d_procs.fTypefaceProc = [](const void* data, size_t length, void* ctx) -> sk_sp<SkTypeface> { return *(static_cast<sk_sp<SkTypeface>*>(ctx)); };
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(SkRemoteGlyphCache_TypefaceWithNoPaths, reporter, ctxInfo, CtsEnforcement::kNever)
{
    ...
    auto props = FindSurfaceProps(direct);
    std::unique_ptr<SkCanvas> cache_diff_canvas
        = server.makeAnalysisCanvas(500, 500, props, nullptr, direct->supportsDistanceFieldText(), !direct->priv().caps()->disablePerspectiveSDFText());
