...
static void test_unscaled(skiatest::Reporter* reporter)
{
    ...
    sk_sp<SkShader> gradient = SkGradientShader::MakeRadial(center, radius, colors, pos, std::size(colors), SkTileMode::kClamp);
...
static void test_runtime_shader(skiatest::Reporter* r, SkSurface* surface)
{
    ...
    std::vector<sk_sp<SkImageFilter>> filters = { SkMakeRuntimeImageFilter(effect, /*uniforms=*/nullptr, input),
        SkImageFilters::RuntimeShader(builder, /*childShaderName=*/"", input), SkImageFilters::RuntimeShader(builder, /*childShaderName=*/"child", input) };
