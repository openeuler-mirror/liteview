...
bool run_test(skiatest::Reporter* reporter, Context* context, Recorder* recorder)
{
    ...
    // that instead.
    if (!static_cast<skgpu::graphite::Surface*>(surface.get())->onReadPixels(context, recorder, pm0, 0, 0)) {
    ...
    if (!static_cast<skgpu::graphite::Surface*>(surface.get())->onReadPixels(context, recorder, pm1, 0, 0)) {
        ERRORF(reporter, "readPixels failed");
        return false;
    }
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(reporter, "Error at %d, %d. Diff in floats: (%f, %f, %f, %f)", x, y, diffs[0], diffs[1], diffs[2], diffs[3]);
    });
