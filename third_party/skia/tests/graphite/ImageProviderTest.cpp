...
{
    SkImageInfo ii = SkImageInfo::Make(kImageSize.width(), kImageSize.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    SkBitmap bm;
...
sk_sp<SkImage> create_picture_backed_image(Recorder*)
{
    ...
    return SkImage::MakeFromPicture(std::move(picture), r.size(),
        /* matrix= */ nullptr,
        /* paint= */ nullptr, SkImage::BitDepth::kU8, SkColorSpace::MakeSRGB());
}
...
sk_sp<SkImage> create_bitmap_generator_backed_image(Recorder*)
{
    ...
    public:
        BitmapBackedGenerator()
            : SkImageGenerator(SkImageInfo::Make(kImageSize.width(), kImageSize.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType))
        {
        }

        bool onGetPixels(const SkImageInfo& dstInfo, void* pixels, size_t rowBytes, const Options&) override
        {
...
bool check_img(skiatest::Reporter* reporter, Context* context, Recorder* recorder, SkImage* imageToDraw, Mipmapped mipmapped, const char* testcase,
    const SkColor4f& expectedColor)
{
...

    {
        ...
        SkSamplingOptions sampling
            = (mipmapped == Mipmapped::kYes) ? SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest) : SkSamplingOptions(SkFilterMode::kLinear);

        canvas->drawImageRect(imageToDraw, SkRect::MakeWH(kSurfaceSize.width(), kSurfaceSize.height()), sampling);
        ...
        // that instead.
        if (!static_cast<skgpu::graphite::Surface*>(surface.get())->onReadPixels(context, recorder, pm, 0, 0)) {
    ...
    auto error = std::function<ComparePixmapsErrorReporter>([&](int x, int y, const float diffs[4]) {
        ERRORF(reporter, "case %s %s: expected (%.1f %.1f %.1f %.1f) got (%.1f, %.1f, %.1f, %.1f)", testcase,
            (mipmapped == Mipmapped::kYes) ? "w/ mipmaps" : "w/o mipmaps", expectedColor.fR, expectedColor.fG, expectedColor.fB, expectedColor.fA,
            expectedColor.fR - diffs[0], expectedColor.fG - diffs[1], expectedColor.fB - diffs[2], expectedColor.fA - diffs[3]);
    });
...
void run_test(skiatest::Reporter* reporter, Context* context, Recorder* recorder, SkSpan<TestCase> testcases)
{
    ...
    for (auto t : testcases) {
        for (auto mm : { Mipmapped::kNo, Mipmapped::kYes }) {
            ...
            check_img(reporter, context, recorder, image.get(), mm, t.fTestCase, t.fExpectedColors[static_cast<int>(mm)]);
        }
    }
}
