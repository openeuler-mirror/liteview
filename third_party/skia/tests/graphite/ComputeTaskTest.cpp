...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(ComputeTaskTest, reporter, context)
{
    ...
    pipelineDesc.setProgram("layout(set=0, binding=0) readonly buffer inputBlock"
                            "{"
                            "    float in_factor;"
                            "    float in_data[];"
                            "};"
    ...
    sk_sp<Buffer> inputBuffer = provider->findOrCreateBuffer(sizeof(float) * (kProblemSize + 1), BufferType::kStorage, PrioritizeGpuReads::kNo);
    sk_sp<Buffer> outputBuffer = provider->findOrCreateBuffer(sizeof(float) * kProblemSize, BufferType::kStorage, PrioritizeGpuReads::kNo);
