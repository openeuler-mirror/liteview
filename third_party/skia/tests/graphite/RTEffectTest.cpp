...
{
    SkRuntimeEffect::Result result = SkRuntimeEffect::MakeForBlender(SkString("uniform float blendFrac;"
                                                                              "uniform blender a;"
                                                                              "uniform blender b;"
...
{
    SkRuntimeEffect::Result result = SkRuntimeEffect::MakeForBlender(SkString("uniform float redColor;"
                                                                              "half4 main(half4 src, half4 dst) {"
...
{
    SkRuntimeEffect::Result result = SkRuntimeEffect::MakeForBlender(SkString("uniform float blueColor;"
                                                                              "half4 main(half4 src, half4 dst) {"
...
static sk_sp<SkBlender> get_blender(sk_sp<SkRuntimeEffect> comboEffect, sk_sp<SkRuntimeEffect> redEffect, sk_sp<SkRuntimeEffect> blueEffect)
{
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(ColorFilter_FindOrCreateSnippetForRuntimeEffect, reporter, context)
{
    ...
    std::unique_ptr<SkRuntimeEffect> testEffect(SkMakeRuntimeEffect(SkRuntimeEffect::MakeForColorFilter,
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(ShaderUniforms_FindOrCreateSnippetForRuntimeEffect, reporter, context)
{
    SkShaderCodeDictionary* dict = context->priv().shaderCodeDictionary();
    ...
    REPORTER_ASSERT(reporter, std::string_view(snippet->fUniforms[0].name()) == "MyFloat3x3Uniform");
    REPORTER_ASSERT(reporter, snippet->fUniforms[0].type() == SkSLType::kFloat3x3);
    REPORTER_ASSERT(reporter, snippet->fUniforms[0].count() == 0);

    REPORTER_ASSERT(reporter, std::string_view(snippet->fUniforms[1].name()) == "MyInt4ArrayUniform");
    REPORTER_ASSERT(reporter, snippet->fUniforms[1].type() == SkSLType::kInt4);
    REPORTER_ASSERT(reporter, snippet->fUniforms[1].count() == 1);

    REPORTER_ASSERT(reporter, std::string_view(snippet->fUniforms[2].name()) == "MyHalf2ArrayUniform");
    REPORTER_ASSERT(reporter, snippet->fUniforms[2].type() == SkSLType::kHalf2);
    REPORTER_ASSERT(reporter, snippet->fUniforms[2].count() == 99);
}

DEF_GRAPHITE_TEST_FOR_CONTEXTS(ColorFilterUniforms_FindOrCreateSnippetForRuntimeEffect, reporter, context)
{
    SkShaderCodeDictionary* dict = context->priv().shaderCodeDictionary();
