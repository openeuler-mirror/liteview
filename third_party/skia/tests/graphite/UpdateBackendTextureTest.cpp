...
void check_solid_pixmap(skiatest::Reporter* reporter, const SkColor4f& expected, const SkPixmap& actual, SkColorType ct, const char* label)
{
    const float kTols[4] = { 0.01f, 0.01f, 0.01f, 0.01f };

    auto error = std::function<ComparePixmapsErrorReporter>([reporter, ct, label, expected](int x, int y, const float diffs[4]) {
        SkASSERT(x >= 0 && y >= 0);
        ERRORF(reporter,
            "%s %s - mismatch at %d, %d "
            "expected: (%.2f, %.2f, %.2f, %.2f) "
            "- diffs: (%.2f, %.2f, %.2f, %.2f)",
            ToolUtils::colortype_name(ct), label, x, y, expected.fR, expected.fG, expected.fB, expected.fA, diffs[0], diffs[1], diffs[2], diffs[3]);
    });

    CheckSolidPixels(expected, actual, kTols, error);
}

void update_backend_texture(Recorder* recorder, const BackendTexture& backendTex, SkColorType ct, bool withMips, const SkColor4f colors[6])
{
    SkPixmap pixmaps[6];
    std::unique_ptr<char[]> memForPixmaps;

    int numMipLevels = ToolUtils::make_pixmaps(ct, kPremul_SkAlphaType, withMips, colors, pixmaps, &memForPixmaps);
...
BackendTexture create_backend_texture(
    skiatest::Reporter* reporter, const Caps* caps, Recorder* recorder, SkColorType ct, bool withMips, Renderable renderable, const SkColor4f colors[6])
{
    int numMipLevels = withMips ? kNumMipLevels : 1;

    TextureInfo info = caps->getDefaultSampledTextureInfo(ct, numMipLevels, Protected::kNo, renderable);
...
sk_sp<SkImage> wrap_backend_texture(
    skiatest::Reporter* reporter, Recorder* recorder, const skgpu::graphite::BackendTexture& backendTex, SkColorType ct, bool withMips)
{
    sk_sp<SkImage> image = SkImage::MakeGraphiteFromBackendTexture(recorder, backendTex, ct, kPremul_SkAlphaType,
        /* colorSpace= */ nullptr);
...
void check_levels(skiatest::Reporter* reporter, Context* context, Recorder* recorder, SkImage* image, bool withMips, const SkColor4f colors[6])
{
    int numLevels = withMips ? kNumMipLevels : 1;

    SkSamplingOptions sampling = withMips ? SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest) : SkSamplingOptions(SkFilterMode::kNearest);
    ...
    for (int i = 0, drawSize = kSize.width(); i < numLevels; ++i, drawSize /= 2) {
        ...
        SkImageInfo readbackII = SkImageInfo::Make({ drawSize, drawSize }, kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
        ...
        // that instead.
        if (!static_cast<skgpu::graphite::Surface*>(surf.get())->onReadPixels(context, recorder, actual, 0, 0)) {
    ...
    for (SkColorType ct : { kRGBA_8888_SkColorType }) {
        for (bool withMips : { true, false }) {
            for (Renderable renderable : { Renderable::kYes, Renderable::kNo }) {
                ...
                BackendTexture backendTex = create_backend_texture(reporter, caps, recorder.get(), ct, withMips, renderable, kColors);

                sk_sp<SkImage> image = wrap_backend_texture(reporter, recorder.get(), backendTex, ct, withMips);
                if (!image) {
                    continue;
                }
