...
{
    SkImageInfo info = SkImageInfo::Make(32, 32, SkColorType::kRGBA_8888_SkColorType, kPremul_SkAlphaType);
...
std::tuple<SkPaint, int> create_paint(Recorder* recorder, ShaderType shaderType, SkTileMode tm, SkBlendMode bm)
{
    ...
    case ShaderType::kConicalGradient:
        s = SkGradientShader::MakeTwoPointConical({ 100, 100 }, 100, { -100, -100 }, 100, colors, offsets, 2, tm);
        break;
...
SkUniquePaintParamsID create_key(Context* context, SkPaintParamsKeyBuilder* keyBuilder, ShaderType shaderType, SkTileMode tileMode, SkBlendMode blendMode)
{
    ...
    std::vector<SkUniquePaintParamsID> uniqueIDs = CombinationBuilderTestAccess::BuildCombinations(dict, &combinationBuilder);
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(PaintParamsKeyTest, reporter, context)
{
    ...
    for (auto s : { ShaderType::kSolidColor, ShaderType::kLinearGradient, ShaderType::kRadialGradient, ShaderType::kSweepGradient, ShaderType::kConicalGradient,
             ShaderType::kLocalMatrix, ShaderType::kImage, ShaderType::kPorterDuffBlendShader, ShaderType::kBlendShader }) {
        for (auto tm : { SkTileMode::kClamp, SkTileMode::kRepeat, SkTileMode::kMirror, SkTileMode::kDecal }) {
            if (s == ShaderType::kSolidColor || s == ShaderType::kLocalMatrix || s == ShaderType::kPorterDuffBlendShader || s == ShaderType::kBlendShader) {
            ...
            for (auto bm : { SkBlendMode::kSrc, SkBlendMode::kSrcOver }) {
                ...
                auto [uniqueID1, uData, tData] = ExtractPaintData(recorder.get(), &gatherer, &builder, {}, PaintParams(p));
