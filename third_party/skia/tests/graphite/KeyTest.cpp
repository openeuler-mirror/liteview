...
SkPaintParamsKey create_key_with_data(SkPaintParamsKeyBuilder* builder, int snippetID, SkSpan<const uint8_t> dataPayload)
{
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(KeyValidBlockSizeTest, reporter, context)
{
    ...
    static const int kMaxBlockDataSize = SkPaintParamsKey::kMaxBlockSize - sizeof(SkPaintParamsKey::Header);
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(KeyTooLargeBlockSizeTest, reporter, context)
{
    ...
    // Too big by one byte
    static const int kBlockDataSize = SkPaintParamsKey::kMaxBlockSize - sizeof(SkPaintParamsKey::Header) + 1;
    static constexpr SkPaintParamsKey::DataPayloadField kDataFields[] = {
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(KeyBlockReaderWorks, reporter, context)
{
    ...
    REPORTER_ASSERT(reporter, reader.blockSize() == (sizeof(SkPaintParamsKey::Header) + sizeof(kDataX) + sizeof(kDataY) + sizeof(kDataZ)));
