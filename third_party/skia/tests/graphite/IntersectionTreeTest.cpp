...
DEF_GRAPHITE_TEST(skgpu_IntersectionTree, reporter)
{
    SkRandom rand;
    {
        ...
        for (int i = 0; i < 1000; ++i) {
            Rect rect = Rect::XYWH(rand.nextRangeF(0, 500), rand.nextRangeF(0, 500), rand.nextRangeF(0, 70), rand.nextRangeF(0, 70));
    ...
    }
    {
        ...
        IntersectionTree tree;
        for (int i = 0; i < 100; ++i) {
            Rect rect = Rect::XYWH(rand.nextRangeF(0, 500), rand.nextRangeF(0, 500), rand.nextRangeF(0, 200), rand.nextRangeF(0, 200));
            CHECK(tree.add(rect) == simpleTree.add({ rect.left(), rect.top(), rect.right(), rect.bot() }));
        }
    }
    {
