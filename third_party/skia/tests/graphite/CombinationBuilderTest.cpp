...
void big_test_new(SkShaderCodeDictionary* dict, skiatest::Reporter* reporter)
{
    ...
    auto blendShader_0 = PrecompileShaders::Blend(SkSpan<SkBlendMode>(blendModes), // std::array
        { // initializer_list
            PrecompileShaders::LinearGradient(), PrecompileShaders::Color() },
        { PrecompileShaders::LinearGradient(),
            PrecompileShaders::Blend(SkSpan<SkBlendMode>(moreBlendModes), // std::vector
                { PrecompileShaders::RadialGradient(), PrecompileShaders::Color() }, { PrecompileShaders::Image() }) });

    paintOptions.setShaders({ sweepGrad_0, blendShader_0 });

    SkBlendMode evenMoreBlendModes[] = { SkBlendMode::kSrcOver, SkBlendMode::kSrc, SkBlendMode::kDstOver, SkBlendMode::kDst };
...
template <typename T>
std::vector<sk_sp<T>> create_runtime_combos(skiatest::Reporter* reporter, SkRuntimeEffect::Result effectFactory(SkString),
    sk_sp<T> precompileFactory(sk_sp<SkRuntimeEffect>, SkSpan<const PrecompileChildOptions> childOptions), const char* redCode, const char* greenCode,
...
void runtime_effect_test(SkShaderCodeDictionary* dict, skiatest::Reporter* reporter)
{
    ...
    // shaders
    {
        ...
        std::vector<sk_sp<PrecompileShader>> combinations
            = create_runtime_combos<PrecompileShader>(reporter, SkRuntimeEffect::MakeForShader, MakePrecompileShader, kRedS, kGreenS, kCombineS);
    ...
    // color filters
    {
        ...
        std::vector<sk_sp<PrecompileColorFilter>> combinations = create_runtime_combos<PrecompileColorFilter>(
            reporter, SkRuntimeEffect::MakeForColorFilter, MakePrecompileColorFilter, kRedCF, kGreenCF, kCombineCF);
    ...
    // blenders
    {
        ...
        std::vector<sk_sp<PrecompileBlender>> combinations
            = create_runtime_combos<PrecompileBlender>(reporter, SkRuntimeEffect::MakeForBlender, MakePrecompileBlender, kRedB, kGreenB, kCombineB);
...
void big_test(SkShaderCodeDictionary* dict, skiatest::Reporter* reporter)
{
    ...
    // first, shaders. First top-level option (sweepGrad_0)
    [[maybe_unused]] auto sweepGrad_0 = builder.addOption(ShaderType::kSweepGradient, kMinNumStops, kMaxNumStops);
    ...
    // first child slot of blendShader_0
    {
        ...
        [[maybe_unused]] auto linearGrad_0 = blendShader_0.addChildOption(0, ShaderType::kLinearGradient, kMinNumStops, kMaxNumStops);

        // second option (solid_0)
        [[maybe_unused]] auto solid_0 = blendShader_0.addChildOption(0, ShaderType::kSolidColor);
    }
    ...
    // second child slot of blendShader_0
    {
        ...
        {
            [[maybe_unused]] auto linearGrad_1 = blendShader_0.addChildOption(1, ShaderType::kLinearGradient, kMinNumStops, kMaxNumStops);
        }
        ...
        // second option (blendShader_1)
        {
            ...
            // nested: first child slot of blendShader_1
            {
                ...
                [[maybe_unused]] auto radialGrad_0 = blendShader_1.addChildOption(0, ShaderType::kRadialGradient, kMinNumStops, kMaxNumStops);

                // second option (solid_1)
                [[maybe_unused]] auto solid_1 = blendShader_1.addChildOption(0, ShaderType::kSolidColor);
            }

            // nested: second child slot of blendShader_1
            {
                TileModePair tilingOptions[] = { { SkTileMode::kRepeat, SkTileMode::kRepeat }, { SkTileMode::kClamp, SkTileMode::kClamp } };

                // only option (imageShader_0)
                [[maybe_unused]] auto imageShader_0 = blendShader_1.addChildOption(1, ShaderType::kImage, tilingOptions);
            }
        }
    }
