...
public:
    static sk_sp<TestResource> Make(const SharedContext* sharedContext, Ownership owned, SkBudgeted budgeted, Shareable shareable)
    {
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(GraphiteBudgetedResourcesTest, reporter, context)
{
    ...
    auto resource = TestResource::Make(sharedContext, Ownership::kOwned, SkBudgeted::kNo, Shareable::kNo);
    if (!resource) {
    ...
    // Test making a non budgeted, non shareable resource.
    resource = TestResource::Make(sharedContext, Ownership::kOwned, SkBudgeted::kYes, Shareable::kYes);
    if (!resource) {
        ERRORF(reporter, "Failed to make TestResource");
        return;
    }
