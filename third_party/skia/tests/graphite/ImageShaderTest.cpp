...
void test_draw(skiatest::Reporter* reporter, Context* context, SkISize canvasSize, SkISize imageSize, SkRect srcRect, SkRect dstRect, SkTileMode tileMode,
    SkSamplingOptions samplingOptions, std::vector<Expectation> expectations)
{
    std::unique_ptr<Recorder> recorder = context->makeRecorder();
    sk_sp<SkSurface> surface = SkSurface::MakeGraphite(recorder.get(), SkImageInfo::Make(canvasSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType));
    SkCanvas* canvas = surface->getCanvas();

    SkBitmap bitmap;
    bitmap.allocPixels(SkImageInfo::Make(imageSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType), 0);
    ...
    p.setShader(SkImageShader::MakeSubset(std::move(image), srcRect, tileMode, tileMode, samplingOptions, &srcToDst));
    ...
    bool readPixelsSuccess = static_cast<Surface*>(surface.get())->onReadPixels(context, recorder.get(), pm, 0, 0);
    ...
    for (const Expectation& e : expectations) {
        ...
        REPORTER_ASSERT(reporter, a == b,
            ...
            "found {%.1f, %.1f, %.1f, %.1f}",
            e.pos.fX, e.pos.fY, a.fR, a.fG, a.fB, a.fA, b.fR, b.fG, b.fB, b.fA);
    }
}
