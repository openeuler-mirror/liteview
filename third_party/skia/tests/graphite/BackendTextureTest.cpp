...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(SurfaceBackendTextureTest, reporter, context)
{
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeGraphiteFromBackendTexture(recorder.get(), texture, kRGBA_8888_SkColorType,
        /*colorSpace=*/nullptr,
        /*props=*/nullptr);
    ...
    // color type.
    surface = SkSurface::MakeGraphiteFromBackendTexture(recorder.get(), texture, kAlpha_8_SkColorType,
        /*colorSpace=*/nullptr,
        /*props=*/nullptr);
    REPORTER_ASSERT(reporter, !surface);
    ...
    surface = SkSurface::MakeGraphiteFromBackendTexture(recorder.get(), texture, kRGBA_8888_SkColorType,
        /*colorSpace=*/nullptr,
        /*props=*/nullptr);
...
DEF_GRAPHITE_TEST_FOR_CONTEXTS(ImageBackendTextureTest, reporter, context)
{
    ...
    for (bool withMips : { true, false }) {
        for (Renderable renderable : { Renderable::kYes, Renderable::kNo }) {
            ...
            TextureInfo info = caps->getDefaultSampledTextureInfo(kRGBA_8888_SkColorType, withMips ? kNumLevels : 1, Protected::kNo, renderable);

            BackendTexture texture = recorder->createBackendTexture(kSize, info);
            REPORTER_ASSERT(reporter, texture.isValid());

            sk_sp<SkImage> image = SkImage::MakeGraphiteFromBackendTexture(recorder.get(), texture, kRGBA_8888_SkColorType, kPremul_SkAlphaType,
                /*colorSpace=*/nullptr);
            ...
            // color type.
            image = SkImage::MakeGraphiteFromBackendTexture(recorder.get(), texture, kAlpha_8_SkColorType, kPremul_SkAlphaType,
                /* colorSpace= */ nullptr);
            REPORTER_ASSERT(reporter, !image);
