                ...
                for (int d = c + 1; d < 7; ++d) {
                    for (auto e : fts) {
                        for (auto f : fts) {
                            ...
                            pathA.addRect(
                                SkIntToScalar(state.fA), SkIntToScalar(state.fA), SkIntToScalar(state.fB), SkIntToScalar(state.fB), SkPathDirection::kCW);
                            pathA.addRect(
                                SkIntToScalar(state.fC), SkIntToScalar(state.fC), SkIntToScalar(state.fD), SkIntToScalar(state.fD), SkPathDirection::kCW);
                            pathA.close();
                            pathB.setFillType((SkPathFillType)f);
                            pathB.addRect(SkIntToScalar(a), SkIntToScalar(a), SkIntToScalar(b), SkIntToScalar(b), SkPathDirection::kCW);
                            pathB.addRect(SkIntToScalar(c), SkIntToScalar(c), SkIntToScalar(d), SkIntToScalar(d), SkPathDirection::kCW);
                            pathB.close();
                            for (int op = 0; op <= kXOR_SkPathOp; ++op) {
                                if (state.fReporter->verbose()) {
                                    pathStr.printf("static void rects%d(skiatest::Reporter* reporter,"
                                                   "const char* filename) {\n",
                                        loopNo);
...
DEF_TEST(PathOpsRectsThreaded, reporter)
{
    ...
    for (int a = 0; a < 6; ++a) { // outermost
        for (int b = a + 1; b < 7; ++b) {
            for (int c = 0; c < 6; ++c) {
                for (int d = c + 1; d < 7; ++d) {
                    *testRunner.fRunnables.append() = new PathOpsThreadedRunnable(&testPathOpsRectsMain, a, b, c, d, &testRunner);
                }
            }
...
{
    SkASSERT(data);
    const SkPathFillType fts[] = { SkPathFillType::kWinding, SkPathFillType::kEvenOdd, SkPathFillType::kInverseWinding, SkPathFillType::kInverseEvenOdd };
    PathOpsThreadState& state = *data;
    SkString pathStr;
                ...
                for (int d = 0; d < 6; d += step) {
                    for (auto e : fts) {
                        for (auto f : fts) {
                            ...
                            pathA.setFillType(e);
                            if (a) {
                                pathA.addRect(SkIntToScalar(state.fA), SkIntToScalar(state.fA), SkIntToScalar(state.fB) + c, SkIntToScalar(state.fB),
                                    SkPathDirection::kCW);
                            }
                            pathA.close();
                            pathB.setFillType(f);
                            if (b) {
                                pathB.addRect(SkIntToScalar(state.fC), SkIntToScalar(state.fC), SkIntToScalar(state.fD) + d, SkIntToScalar(state.fD),
                                    SkPathDirection::kCW);
                            }
                            pathB.close();
                            ...
                            for (int op = 0; op <= kXOR_SkPathOp; ++op) {
                                if (state.fReporter->verbose()) {
                                    pathStr.printf("static void fast%d(skiatest::Reporter* reporter,"
                                                   "const char* filename) {\n",
                                        loopNo);
                                    pathStr.appendf("    SkPath path, pathB;");
                                    pathStr.appendf("    path.setFillType(SkPathFillType::k%s);\n", fillTypeStr[(int)e]);
        ...
        for (int b = a + 1; b < 7; b += step) {
            for (int c = 0; c < 6; c += step) {
                for (int d = c + 1; d < 7; d += step) {
                    *testRunner.fRunnables.append() = new PathOpsThreadedRunnable(&testPathOpsFastMain, a, b, c, d, &testRunner);
                }
            }
        }
    }
