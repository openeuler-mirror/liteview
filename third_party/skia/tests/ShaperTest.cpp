...
struct RunHandler final : public SkShaper::RunHandler {
    ...
    void commitRunBuffer(const RunInfo& info) override
    {
        ...
        for (unsigned i = 0; i < fGlyphCount; ++i) {
            REPORTER_ASSERT(fReporter, fClusters[i] >= fRange.begin(), "%s %u %u", fResource, i, fGlyphCount);
            REPORTER_ASSERT(fReporter, fClusters[i] < fRange.end(), "%s %u %u", fResource, i, fGlyphCount);
        }
    }
...
void shaper_test(skiatest::Reporter* reporter, const char* name, SkData* data)
{
    ...
    shaper->shape((const char*)data->data(), data->size(), fontIterator, bidiIterator, scriptIterator, languageIterator, kWidth, &rh);
}
