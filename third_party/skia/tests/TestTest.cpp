...
{
    auto verifyRunMode = [&](CtsEnforcement e, int apiLevel, CtsEnforcement::RunMode runMode) { REPORTER_ASSERT(reporter, e.eval(apiLevel) == runMode); };
    ...
    CtsEnforcement e4 = CtsEnforcement(CtsEnforcement::kNextRelease).withWorkarounds(CtsEnforcement::kApiLevel_T);
