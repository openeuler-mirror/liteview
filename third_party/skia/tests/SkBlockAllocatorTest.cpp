...
public:
    template <size_t N> static size_t ScratchBlockSize(SkSBlockAllocator<N>& pool)
    {
...
template <size_t N> static int block_count(const SkSBlockAllocator<N>& pool)
{
    int ct = 0;
...
template <size_t N> static Block* get_block(SkSBlockAllocator<N>& pool, int blockIndex)
{
    Block* found = nullptr;
    int i = 0;
...
template <size_t N> static size_t total_size(SkSBlockAllocator<N>& pool)
{
    return pool->totalSize() - BlockAllocatorTestAccess::ScratchBlockSize(pool);
}

template <size_t N> static size_t add_block(SkSBlockAllocator<N>& pool)
{
...
template <size_t N> static void* alloc_byte(SkSBlockAllocator<N>& pool)
{
...
DEF_TEST(SkBlockAllocatorPreallocSize, r)
{
    ...
    SkBlockAllocator* placement = new (mem) SkBlockAllocator(GrowthPolicy::kLinear, 1024, 1024 - sizeof(SkBlockAllocator));
    REPORTER_ASSERT(r, placement->preallocSize() == 1024);
    REPORTER_ASSERT(r, placement->preallocUsableSpace() < 1024 && placement->preallocUsableSpace() >= (1024 - sizeof(SkBlockAllocator)));
    ...
    REPORTER_ASSERT(r, inlined->preallocUsableSpace() < 2048 && inlined->preallocUsableSpace() >= (2048 - sizeof(SkBlockAllocator)));
}
...
DEF_TEST(SkBlockAllocatorAlloc, r)
{
    ...
    SkDEBUGCODE(pool->validate();)
        ...
        auto validate_ptr
        = [&](int align, int size, SkBlockAllocator::ByteRange br, SkBlockAllocator::ByteRange* prevBR) {
...
DEF_TEST(SkBlockAllocatorResize, r)
{
    ...
    REPORTER_ASSERT(
        r, reinterpret_cast<uintptr_t>(pNext.fBlock->ptr(pNext.fAlignedOffset)) - reinterpret_cast<uintptr_t>(pNext.fBlock->ptr(p.fAlignedOffset)) == 32);
...
DEF_TEST(SkBlockAllocatorRewind, r)
{
    ...
    SkDEBUGCODE(pool->validate();)
        ...
        std::vector<SkBlockAllocator::ByteRange>
            ptrs;
...
DEF_TEST(SkBlockAllocatorScratchBlockReserve, r)
{
    ...
    REPORTER_ASSERT(r, BlockAllocatorTestAccess::ScratchBlockSize(pool) >= reserve && BlockAllocatorTestAccess::ScratchBlockSize(pool) % 256 == 0);
    ...
    REPORTER_ASSERT(r, BlockAllocatorTestAccess::ScratchBlockSize(pool) > 0 && BlockAllocatorTestAccess::ScratchBlockSize(pool) < 256);
...
DEF_TEST(SkBlockAllocatorMetadata, r)
{
    ...
    TestMetaBig* metaBig = static_cast<TestMetaBig*>(p2.fBlock->ptr(p2.fAlignedOffset - sizeof(TestMetaBig)));
    // Confirm alignment for both pointers
...
template <size_t Align, size_t Padding> static void run_owning_block_test(skiatest::Reporter* r, SkBlockAllocator* pool)
{
...
template <size_t Padding> static void run_owning_block_tests(skiatest::Reporter* r, SkBlockAllocator* pool)
{
