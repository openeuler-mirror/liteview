...
DEF_TEST(SkStrikePromise_Basic, reporter)
{
    ...
    std::optional<SkStrikePromise> dstPromise = SkStrikePromise::MakeFromBuffer(readBuffer, nullptr, strikeCache.get());

    REPORTER_ASSERT(reporter, dstPromise.has_value());
    REPORTER_ASSERT(reporter, reinterpret_cast<intptr_t>(dstPromise->strike()) == toCompareWith);
}
