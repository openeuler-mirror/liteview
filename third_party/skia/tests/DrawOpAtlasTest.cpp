...
static bool fill_plot(GrDrawOpAtlas* atlas, GrResourceProvider* resourceProvider, GrDeferredUploadTarget* target, skgpu::AtlasLocator* atlasLocator, int alpha)
{
    ...
    code = atlas->addToAtlas(resourceProvider, target, kPlotSize, kPlotSize, data.getAddr(0, 0), atlasLocator);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(BasicDrawOpAtlas, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    std::unique_ptr<GrDrawOpAtlas> atlas
        = GrDrawOpAtlas::Make(proxyProvider, format, GrColorTypeToSkColorType(atlasColorType), GrColorTypeBytesPerPixel(atlasColorType), kAtlasSize, kAtlasSize,
            kAtlasSize / kNumPlots, kAtlasSize / kNumPlots, &counter, GrDrawOpAtlas::AllowMultitexturing::kYes, &evictor,
    ...
    for (int i = 0; i < kNumPlots * kNumPlots; ++i) {
        bool result = fill_plot(atlas.get(), resourceProvider, &uploadTarget, &atlasLocators[i], i * 32);
...
DEF_GANESH_TEST_FOR_RENDERING_CONTEXTS(GrAtlasTextOpPreparation, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::Make(dContext, GrColorType::kRGBA_8888, nullptr, SkBackingFit::kApprox, { 32, 32 }, SkSurfaceProps(),
    ...
    GrOp::Owner op = AtlasTextOp::CreateOpTestingOnly(sdc.get(), paint, font, matrixProvider, text, 16, 16);
    if (!op) {
        return;
    }
    ...
    GrOpFlushState::OpArgs opArgs(op.get(), surfaceView, false /*usesMSAASurface*/, nullptr, GrDstProxyView(), GrXferBarrierFlags::kNone, GrLoadOp::kLoad);
...
void test_atlas_config(
    skiatest::Reporter* reporter, int maxTextureSize, size_t maxBytes, MaskFormat maskFormat, SkISize expectedDimensions, SkISize expectedPlotDimensions)
{
