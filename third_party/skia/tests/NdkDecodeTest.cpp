...
DEF_TEST(NdkDecode_reportedColorSpace, r)
{
    for (sk_sp<SkColorSpace> cs : {
        ...
        for (auto format : { SkEncodedImageFormat::kPNG, SkEncodedImageFormat::kJPEG, SkEncodedImageFormat::kWEBP }) {
...
DEF_TEST(NdkDecode_reuseNoColorSpace, r)
{
    static const struct {
        ...
        { "images/example_1.png", SkColorSpace::MakeRGB(SkNamedTransferFn::k2Dot2, SkNamedGamut::kAdobeRGB), false },
    };
    for (auto& rec : recs) {
        auto gen = make_generator(rec.fPath, r);
        if (!gen)
...
DEF_TEST(NdkDecode_webpArbitraryDownscale, r)
{
    for (const char* path : {
        ...
        const int width = actualDimensions.width();
        const int height = actualDimensions.height();
        for (SkISize dims : { SkISize { width / 2, height / 2 }, SkISize { width / 4, height / 4 }, SkISize { width / 7, height / 7 },
                 SkISize { width - 1, height - 1 }, SkISize { 1, 1 }, SkISize { 5, 20 } }) {
            auto info = gen->getInfo().makeDimensions(dims);
            SkBitmap bm;
            bm.allocPixels(info);
            REPORTER_ASSERT(r, gen->getPixels(bm.pixmap()));
...
DEF_TEST(NdkDecode_Gray8, r)
{
    ...
    for (auto& rec : recs) {
        ...
        if (success != rec.fGrayscale) {
            ERRORF(r, "Expected decoding %s to Gray8 to %s. Actual: %s\n", rec.fPath, (rec.fGrayscale ? "succeed" : "fail"), (success ? "succeed" : "fail"));
        }
    }
}
