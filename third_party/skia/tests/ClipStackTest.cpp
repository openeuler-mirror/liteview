...
static void test_iterators(skiatest::Reporter* reporter)
{
    ...
    static const SkRect gRects[] = { { 0, 0, 40, 40 }, { 60, 0, 100, 40 }, { 0, 60, 40, 100 }, { 60, 60, 100, 100 } };
    ...
    // bottom to top iteration
    {
        ...
        for (i = 0, element = iter.next(); element; ++i, element = iter.next()) {
            REPORTER_ASSERT(reporter, SkClipStack::Element::DeviceSpaceType::kRect == element->getDeviceSpaceType());
...
static void set_region_to_stack(const SkClipStack& stack, const SkIRect& bounds, SkRegion* region)
{
    ...
    while (const SkClipStack::Element* element = iter.next()) {
        ...
        region->op(elemRegion, element->isReplaceOp() ? SkRegion::kReplace_Op : (SkRegion::Op)element->getOp());
    }
}
...
static void test_is_rrect_deep_rect_stack(skiatest::Reporter* reporter)
{
    ...
    for (bool aa : { false, true }) {
        ...
        for (int i = 0; i <= 100; ++i) {
            stack.save();
            stack.clipRect(SkRect::MakeLTRB(i, 0.5, kTargetBounds.width(), kTargetBounds.height()), SkMatrix::I(), SkClipOp::kIntersect, aa);
        }
        SkRRect rrect;
        bool isAA;
        SkRRect expected = SkRRect::MakeRect(SkRect::MakeLTRB(100, 0.5, kTargetBounds.width(), kTargetBounds.height()));
    ...
    for (int i = 0; i <= 100; ++i) {
        ...
        int j = 100 - i;
        stack.save();
        stack.clipRect(SkRect::MakeLTRB(i, j + 0.5, kTargetBounds.width(), kTargetBounds.height()), SkMatrix::I(), SkClipOp::kIntersect, aa);
    }
    SkRRect rrect;
    bool isAA;
...
DEF_TEST(ClipStack, reporter)
{
    ...
    static const SkIRect gRects[] = { { 0, 0, 100, 100 }, { 25, 25, 125, 125 }, { 0, 0, 1000, 1000 }, { 0, 0, 75, 75 } };
    for (size_t i = 0; i < std::size(gRects); i++) {
