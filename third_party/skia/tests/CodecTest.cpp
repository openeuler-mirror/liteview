...
static void test_incremental_decode(skiatest::Reporter* r, SkCodec* codec, const SkImageInfo& info, const SkMD5::Digest& goodDigest)
{
    ...
    REPORTER_ASSERT(r, SkCodec::kSuccess == codec->startIncrementalDecode(info, bm.getPixels(), bm.rowBytes()));
...
static void test_in_stripes(skiatest::Reporter* r, SkCodec* codec, const SkImageInfo& info, const SkMD5::Digest& goodDigest)
{
    ...
    for (int oddEven = 1; oddEven >= 0; oddEven--) {
        for (int y = oddEven * stripeHeight; y < height; y += 2 * stripeHeight) {
            ...
            if (SkCodec::kSuccess != codec->startIncrementalDecode(info, bm.getAddr(0, y), bm.rowBytes(), &options)) {
...
template <typename Codec>
static void test_codec(skiatest::Reporter* r, const char* path, Codec* codec, SkBitmap& bm, const SkImageInfo& info, const SkISize& size,
    ...
    *digest = md5(bm);
...

    {
        ...
        if (info.alphaType() == kOpaque_SkAlphaType) {
            ...
            if (actualResult == expectedResult) {
                ...
                ERRORF(r, "Decoding %s to 565 failed with result \"%s\"\n\t\t\t\texpected:\"%s\"", path, SkCodec::ResultToString(actualResult),
...

    {
        ...
        if (info.alphaType() == kOpaque_SkAlphaType) {
            ...
            test_info(r, codec, info.makeAlphaType(kOpaque_SkAlphaType), SkCodec::kInvalidConversion, nullptr);
...
static void check_scanline_decode(skiatest::Reporter* r, SkCodec* codec, SkMD5::Digest* codecDigest, const SkImageInfo& info, const char path[], SkISize size,
    bool supportsScanlineDecoding, bool supportsIncomplete, bool supportsNewScanlineDecoding)
{

    // Test full image decodes with SkCodec
    SkBitmap bm;
    const SkCodec::Result expectedResult = supportsIncomplete ? SkCodec::kIncompleteInput : SkCodec::kSuccess;
...
static void check_subset_decode(
    skiatest::Reporter* r, SkCodec* codec, const SkImageInfo& info, SkISize size, bool supportsSubsetDecoding, bool supportsIncomplete)
{
...
static void check_android_codec(skiatest::Reporter* r, std::unique_ptr<SkCodec> codec, const SkMD5::Digest& codecDigest, const SkImageInfo& info,
    const char path[], SkISize size, bool supportsScanlineDecoding, bool supportsSubsetDecoding, bool supportsIncomplete, bool supportsNewScanlineDecoding)
{
    if (supportsScanlineDecoding || supportsSubsetDecoding || supportsNewScanlineDecoding) {
        ...
        const SkCodec::Result expectedResult = supportsIncomplete ? SkCodec::kIncompleteInput : SkCodec::kSuccess;
        test_codec(r, path, androidCodec.get(), bm, info, size, expectedResult, &androidCodecDigest, &codecDigest);
    }
}

static void check_codec_image_generator(
    skiatest::Reporter* r, const SkMD5::Digest& codecDigest, const SkImageInfo& info, const char path[], bool supportsIncomplete)
{
    ...
    if (!supportsIncomplete) {
        ...
        auto bufferedStream = android::skia::FrontBufferedStream::Make(SkMemoryStream::Make(std::move(fullData)), SkCodec::MinBufferedBytesNeeded());
...
static void check(skiatest::Reporter* r, const char path[], SkISize size, bool supportsScanlineDecoding, bool supportsSubsetDecoding, bool supportsIncomplete,
    bool supportsNewScanlineDecoding = false)
{
    ...
    check_scanline_decode(r, codec.get(), &codecDigest, info, path, size, supportsScanlineDecoding, supportsIncomplete, supportsNewScanlineDecoding);

    check_subset_decode(r, codec.get(), info, size, supportsSubsetDecoding, supportsIncomplete);

    check_android_codec(
        r, std::move(codec), codecDigest, info, path, size, supportsScanlineDecoding, supportsSubsetDecoding, supportsIncomplete, supportsNewScanlineDecoding);
...
static void test_invalid_stream(skiatest::Reporter* r, const void* stream, size_t len)
{
    ...
    REPORTER_ASSERT(r, !SkCodec::MakeFromStream(std::make_unique<SkMemoryStream>(stream, len, false)));
    REPORTER_ASSERT(r, !SkAndroidCodec::MakeFromStream(std::make_unique<SkMemoryStream>(stream, len, false)));
}
...
static void test_dimensions(skiatest::Reporter* r, const char path[])
{
    ...
    for (int sampleSize = 1; sampleSize < 32; sampleSize++) {
        ...
        SkImageInfo scaledInfo = codec->getInfo().makeDimensions(scaledDims).makeColorType(kN32_SkColorType);
        ...
        if (result != SkCodec::kSuccess) {
            ERRORF(r, "Failed to decode %s with sample size %i; error: %s", path, sampleSize, SkCodec::ResultToString(result));
        }
    }
}
...
DEF_TEST(Codec_pngChunkReader, r)
{
    ...
    png_set_IHDR(
        png, info, (png_uint_32)w, (png_uint_32)h, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
    ...
    SkCodec::Result result = codec->getPixels(codec->getInfo(), decodedBm.getPixels(), decodedBm.rowBytes());
...
DEF_TEST(Codec_raw_notseekable, r)
{
    ...
    std::unique_ptr<SkCodec> codec(SkCodec::MakeFromStream(std::make_unique<NotAssetMemStream>(std::move(data))));
    REPORTER_ASSERT(r, codec);
...
DEF_TEST(Codec_webp_peek, r)
{
    ...
    std::unique_ptr<SkCodec> codec(SkCodec::MakeFromStream(std::make_unique<LimitedPeekingMemStream>(data, 25)));
    REPORTER_ASSERT(r, codec);
...
DEF_TEST(Codec_jpeg_rewind, r)
{
    ...
    // Now perform a subset decode.
    {
        ...
        opts.fSubset = &subset;
        result = codec->getAndroidPixels(codec->getInfo().makeWH(100, 100), pixelStorage.get(), rowBytes, &opts);
...
static void check_color_xform(skiatest::Reporter* r, const char* path)
{
    ...
    SkImageInfo dstInfo = codec->getInfo().makeWH(dstWidth, dstHeight).makeColorType(kN32_SkColorType).makeColorSpace(colorSpace);
...
DEF_TEST(Codec_PngRoundTrip, r)
{
    ...
    SkColorType colorTypesOpaque[] = { kRGB_565_SkColorType, kRGBA_8888_SkColorType, kBGRA_8888_SkColorType };
    ...
    for (SkColorType colorType : colorTypesWithAlpha) {
        for (SkAlphaType alphaType : alphaTypes) {
            ...
            SkImageInfo newInfo = codec->getInfo().makeColorType(colorType).makeAlphaType(alphaType).makeColorSpace(nullptr);
            check_round_trip(r, codec.get(), newInfo);
        }
    }
...
static void decode_frame(skiatest::Reporter* r, SkCodec* codec, size_t frame)
{
    ...
    REPORTER_ASSERT(r, SkCodec::kSuccess == codec->getPixels(info, bm.getPixels(), bm.rowBytes(), &opts));
}
...
DEF_TEST(Codec_skipFullParse, r)
{
    ...
    REPORTER_ASSERT(r, positionAfterFirstFrame > sizePosition && positionAfterFirstFrame < stream->getLength());
...
DEF_TEST(Codec_fallBack, r)
{
    ...
    for (auto file : files) {
        ...
        if (SkCodec::kUnimplemented != codec->startIncrementalDecode(info, bm.getPixels(), bm.rowBytes())) {
...
DEF_TEST(Wuffs_seek_and_decode, r)
{
    ...
    // Test using FrontBufferedStream, as Android does
    auto bufferedStream = android::skia::FrontBufferedStream::Make(GetResourceAsStream(file), SkCodec::MinBufferedBytesNeeded());
...
static void test_invalid_images(skiatest::Reporter* r, const char* path, SkCodec::Result expectedResult)
{
    ...
    test_info(r, codec.get(), codec->getInfo().makeColorType(kN32_SkColorType), expectedResult, nullptr);
}
