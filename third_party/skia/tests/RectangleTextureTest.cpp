...
// skbug.com/5932
static void test_basic_draw_as_src(skiatest::Reporter* reporter, GrDirectContext* dContext, GrSurfaceProxyView rectView, GrColorType colorType,
    SkAlphaType alphaType, uint32_t expectedPixelValues[])
{
    auto sfc = dContext->priv().makeSFC({ colorType, kPremul_SkAlphaType, nullptr, rectView.dimensions() }, /*label=*/ {});
    for (auto filter : { GrSamplerState::Filter::kNearest, GrSamplerState::Filter::kLinear }) {
        for (auto mm : { GrSamplerState::MipmapMode::kNone, GrSamplerState::MipmapMode::kLinear }) {
            ...
            TestReadPixels(reporter, dContext, sfc.get(), expectedPixelValues, "RectangleTexture-basic-draw");
        }
    }
}

static void test_clear(skiatest::Reporter* reporter, GrDirectContext* dContext, skgpu::v1::SurfaceContext* rectContext)
{
...
static void test_copy_to_surface(skiatest::Reporter* reporter, GrDirectContext* dContext, skgpu::v1::SurfaceContext* dstContext, const char* testName)
{
    ...
    for (int y = 0; y < dstContext->width(); ++y) {
        for (int x = 0; x < dstContext->height(); ++x) {
            pixels.get()[y * dstContext->width() + x] = SkColorToPremulGrColor(SkColorSetARGB(2 * y, y, x, x * y));
        }
    }

    for (auto renderable : { GrRenderable::kNo, GrRenderable::kYes }) {
        auto origin = dstContext->origin();
        GrImageInfo info(GrColorType::kRGBA_8888, kPremul_SkAlphaType, nullptr, dstContext->dimensions());
...
DEF_GANESH_TEST_FOR_GL_RENDERING_CONTEXTS(RectangleTexture, reporter, ctxInfo, CtsEnforcement::kApiLevel_T)
{
    ...
    for (auto origin : { kBottomLeft_GrSurfaceOrigin, kTopLeft_GrSurfaceOrigin }) {
        ...
        GrBackendTexture rectangleTex = dContext->createBackendTexture(kWidth, kHeight, format, GrMipmapped::kNo, GrRenderable::kYes);
        ...
        sk_sp<GrTextureProxy> rectProxy = proxyProvider->wrapBackendTexture(rectangleTex, kBorrow_GrWrapOwnership, GrWrapCacheable::kNo, kRW_GrIOType);
        ...
        GrImageInfo grII = ii;
        skgpu::Swizzle swizzle = dContext->priv().caps()->getReadSwizzle(rectangleTex.getBackendFormat(), grII.colorType());
        GrSurfaceProxyView view(rectProxy, origin, swizzle);

        test_basic_draw_as_src(reporter, dContext, view, grII.colorType(), kPremul_SkAlphaType, refPixels);

        // Test copy to both a texture and RT
        TestCopyFromSurface(reporter, dContext, rectProxy, origin, grII.colorType(), refPixels, "RectangleTexture-copy-from");
