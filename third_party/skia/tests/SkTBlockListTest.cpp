...
public:
    template <int N> static size_t ScratchBlockSize(SkTBlockList<C, N>& list)
    {
        return (size_t)list.fAllocator->scratchBlockSize();
    }

    template <int N> static size_t TotalSize(SkTBlockList<C, N>& list)
    {
...
template <int N> static void check_allocator_helper(SkTBlockList<C, N>* allocator, int cnt, int popCnt, skiatest::Reporter* reporter)
{
...
template <int N> static void check_iterator_helper(SkTBlockList<C, N>* allocator, const std::vector<C*>& expected, skiatest::Reporter* reporter)
{
...
template <int N> static void check_allocator(SkTBlockList<C, N>* allocator, int cnt, int popCnt, skiatest::Reporter* reporter)
{
...
template <int N> static void run_allocator_test(SkTBlockList<C, N>* allocator, skiatest::Reporter* reporter)
{
...
template <int N1, int N2> static void run_concat_test(skiatest::Reporter* reporter, int aCount, int bCount)
{
...
template <int N1, int N2> static void run_concat_trivial_test(skiatest::Reporter* reporter, int aCount, int bCount)
{
...
template <int N> static void run_reserve_test(skiatest::Reporter* reporter)
{
    ...
    REPORTER_ASSERT(reporter, TBlockListTestAccess::ScratchBlockSize(list) >= extraReservedCount * sizeof(C));
    ...
    list.reserve(2);
    REPORTER_ASSERT(reporter, TBlockListTestAccess::ScratchBlockSize(list) >= kItemsPerBlock * sizeof(C));
