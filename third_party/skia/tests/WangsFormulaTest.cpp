...
const SkPoint kSerp[4] = { { 285.625f, 499.687f }, { 411.625f, 808.188f }, { 1064.62f, 135.688f }, { 1042.63f, 585.187f } };

const SkPoint kLoop[4] = { { 635.625f, 614.687f }, { 171.625f, 236.188f }, { 1064.62f, 135.688f }, { 516.625f, 570.187f } };
...
static float wangs_formula_conic_reference_impl(float precision, const SkPoint P[3], const float w)
{
    ...
    const float denom = (tP[2] - tP[1] * 2 * w + tP[0]).length() + r_minus_eps * std::abs(1 - 2 * w + 1);
...
static void for_random_beziers(int numPoints, SkRandom* rand, std::function<void(const SkPoint[])> f, int maxExponent = 30)
{
...
DEF_TEST(wangs_formula_log2, r)
{
    ...
    auto setupCubicLengthTerm = [](int seed, SkPoint pts[], float term) {
        ...
        SkPoint term2d = (seed & 1) ? SkPoint::Make(term, 0) : SkPoint::Make(.5f, std::sqrt(3) / 2) * term;
        seed >>= 1;
...
DEF_TEST(wangs_formula_vectorXforms, r)
{
    ...
    for_random_matrices(&rand, [&](const SkMatrix& m) {
        ...
        for_random_beziers(4, &rand, [&](const SkPoint pts[]) { check_cubic_log2_with_transform(pts, m); });

        for_random_beziers(3, &rand, [&](const SkPoint pts[]) { check_quadratic_log2_with_transform(pts, m); });
    });
}
...
{
    {
        ...
        REPORTER_ASSERT(r, wangs_formula::worst_case_cubic(kPrecision, 100, 100) == wangs_formula_cubic_reference_impl(kPrecision, worstP));
        REPORTER_ASSERT(r, wangs_formula::worst_case_cubic_log2(kPrecision, 100, 100) == wangs_formula::cubic_log2(kPrecision, worstP));
    }
    {
...
DEF_TEST(wangs_formula_quad_within_tol, r)
{
    ...
    for_random_beziers(
        3, &rand,
        [&r](const SkPoint pts[]) {
            const int nsegs = static_cast<int>(std::ceil(wangs_formula_quadratic_reference_impl(kPrecision, pts)));
            ...
            for (int j = 0; j < nsegs; ++j) {
                ...
                const SkPoint n = { sectionPts[2].fY - sectionPts[0].fY, sectionPts[0].fX - sectionPts[2].fX };
...
DEF_TEST(wangs_formula_rational_quad_reduces, r)
{
    ...
    for (int i = 0; i < 100; ++i) {
        for_random_beziers(3, &rand, [&r](const SkPoint pts[]) {
            ...
            REPORTER_ASSERT(r, SkScalarNearlyEqual(rational_nsegs, integral_nsegs, kTessellationTolerance));
        });
    }
}
...
DEF_TEST(wangs_formula_conic_vectorXforms, r)
{
    ...
    for (int i = -10; i <= 10; ++i) {
        ...
        for_random_beziers(3, &rand, [&](const SkPoint pts[]) {
            ...
            check_conic_with_transform(pts, w, SkMatrix::Scale(rand.nextRangeF(-10, 10), rand.nextRangeF(-10, 10)));
