...
static bool check_for_failure(
    skiatest::Reporter* r, SkSpan<const Attribute> attributes, size_t stride, SkSpan<const Varying> varyings, const SkString& vs, const SkString& fs)
{
    ...
    ERRORF(r, "Expected to fail but succeeded:\n%s", make_description(attributes, stride, varyings, vs, fs).c_str());
    return false;
}

static bool check_for_success(skiatest::Reporter* r, SkSpan<const Attribute> attributes, size_t stride, SkSpan<const Varying> varyings, const SkString& vs,
    const SkString& fs, sk_sp<SkMeshSpecification>* spec = nullptr)
{
    ...
    ERRORF(r, "Expected to succeed but failed:\n%sError:\n%s", make_description(attributes, stride, varyings, vs, fs).c_str(), error.c_str());
    return false;
}

// Simple valid strings to make specifications
static const SkString kValidVS { "float2 main(Attributes attrs, out Varyings v) { return float2(10); }" };
...
static void test_good(skiatest::Reporter* r)
{
    for (const auto& validFS : kValidFSes) {
        if (!check_for_success(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, validFS)) {
            return;
        }
    }
}
...
static void test_bad_sig(skiatest::Reporter* r)
{
    ...
    for (const char* vsSig : kInvalidVSSigs) {
        ...
        for (const auto& validFS : kValidFSes) {
            if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, invalidVS, validFS)) {
                return;
            }
        }
    }
    ...
    for (const char* noColorFSSig : kInvalidNoColorFSSigs) {
        ...
        if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, invalidFS)) {
            return;
        }
    }
...
static void test_float4_color(skiatest::Reporter* r)
{
    ...
    check_for_success(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, kFloat4FS);
}
...
static void test_bad_globals(skiatest::Reporter* r)
{
    ...
    for (const auto& global : kBadGlobals) {
        ...
        if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, badVS, kValidFSes[0])) {
            return;
        }
    }
    for (const auto& global : kBadGlobals) {
        ...
        if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, badFS)) {
            return;
        }
    }
}
...
static void test_good_uniforms(skiatest::Reporter* r)
{
    ...
    auto make_uni = [](Type type, std::string_view name, size_t offset, uint32_t flags, int count = 0) {
        if (count) {
    ...
    struct {
        ...
        { {
            ...
            { make_uni(Type::kFloat4, "color", 0, kVS | kColor, 0), make_uni(Type::kFloat, "x", 16, kVS | kFS, 5) } },
        ...
        { {
            ...
            { make_uni(Type::kFloat, "x", 0, kVS | kFS | kHalfP, 2), make_uni(Type::kInt, "y", 8, kVS, 0) } },
        ...
        { {
            ...
            { make_uni(Type::kFloat4x4, "m", 0, kVS | kFS | kHalfP, 4), make_uni(Type::kInt3, "i3", 256, kFS, 1) } },
    ...
    for (const auto& c : kTestCases) {
        ...
        if (uniforms.size() != c.expectations.size()) {
            ERRORF(r, "Expected %zu uniforms but actually %zu:\n%s", c.expectations.size(), uniforms.size(), desc.c_str());
            return;
        }
        for (const auto& [actual, expected] : SkMakeZip(uniforms, c.expectations)) {
            std::string name = std::string(actual.name);
            if (name != expected.name) {
                ERRORF(r, "Actual uniform name (%s) does not match expected name (%.*s)", name.c_str(), (int)expected.name.size(), expected.name.data());
                return;
            }
            if (actual.type != expected.type) {
                ERRORF(r, "Uniform %s: Actual type (%d) does not match expected type (%d)", name.c_str(), static_cast<int>(actual.type),
                    static_cast<int>(expected.type));
                return;
            }
            if (actual.count != expected.count) {
                ERRORF(r, "Uniform %s: Actual count (%d) does not match expected count (%d)", name.c_str(), actual.count, expected.count);
                return;
            }
            if (actual.flags != expected.flags) {
                ERRORF(r, "Uniform %s: Actual flags (0x%04x) do not match expected flags (0x%04x)", name.c_str(), actual.flags, expected.flags);
                return;
            }
            if (actual.offset != expected.offset) {
                ERRORF(r, "Uniform %s: Actual offset (%zu) does not match expected offset (%zu)", name.c_str(), actual.offset, expected.offset);
                return;
            }
        }
    }
...
static void test_no_main(skiatest::Reporter* r)
{
    ...
    // Empty VS
    if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, SkString {}, kValidFSes[0])) {
        return;
    }

    // VS with helper function but no main
    if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, kHelper, kValidFSes[0])) {
        return;
    }

    // Empty FS
    if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, SkString {})) {
        return;
    }

    // VS with helper function but no main
    if (!check_for_failure(r, kValidAttrs, kValidStride, kValidVaryings, kValidVS, kHelper)) {
        return;
    }
}
...
static void test_zero_attrs(skiatest::Reporter* r)
{
    ...
    check_for_failure(r, SkSpan<Attribute>(), kValidStride, kValidVaryings, kValidVS, kValidFSes[0]);
}

static void test_zero_varyings(skiatest::Reporter* r)
{
    // Varyings are not required.
    check_for_success(r, kValidAttrs, kValidStride, SkSpan<Varying>(), kValidVS, kValidFSes[0]);
}

static void test_bad_strides(skiatest::Reporter* r)
{
    // Zero stride
    if (!check_for_failure(r, kValidAttrs, 0, kValidVaryings, kValidVS, kValidFSes[0])) {
        return;
    }

    // Unaligned
    if (!check_for_failure(r, kValidAttrs, kValidStride + 1, kValidVaryings, kValidVS, kValidFSes[0])) {
        return;
    }

    // Too large
    if (!check_for_failure(r, kValidAttrs, 1 << 20, kValidVaryings, kValidVS, kValidFSes[0])) {
        return;
    }
}
...
static void test_bad_offsets(skiatest::Reporter* r)
{
    { // offset isn't aligned
        ...
        if (!check_for_failure(r, kAttributes, 32, kValidVaryings, kValidVS, kValidFSes[0])) {
            return;
        }
    }
    { // straddles stride boundary
        ...
        if (!check_for_failure(r, kAttributes, 20, kValidVaryings, kValidVS, kValidFSes[0])) {
            return;
        }
    }
    { // straddles stride boundary with attempt to overflow
        ...
        if (!check_for_failure(r, kAttributes, 4, kValidVaryings, kValidVS, kValidFSes[0])) {
            return;
        }
    }
}
...
static void test_too_many_attrs(skiatest::Reporter* r)
{
    ...
    check_for_failure(r, attrs, 4 * 4, kValidVaryings, kValidVS, kValidFSes[0]);
}
...
static void test_too_many_varyings(skiatest::Reporter* r)
{
    ...
    check_for_failure(r, kValidAttrs, kValidStride, SkSpan(varyings), kValidVS, kValidFSes[0]);
}

static void test_duplicate_attribute_names(skiatest::Reporter* r)
{
    static const Attribute kAttributes[] { { Attribute::Type::kFloat4, 0, SkString { "var" } }, { Attribute::Type::kFloat2, 16, SkString { "var" } } };
    check_for_failure(r, kAttributes, 24, kValidVaryings, kValidVS, kValidFSes[0]);
}

static void test_duplicate_varying_names(skiatest::Reporter* r)
{
    static const Varying kVaryings[] { { Varying::Type::kFloat4, SkString { "var" } }, { Varying::Type::kFloat3, SkString { "var" } } };
    check_for_failure(r, kValidAttrs, kValidStride, kVaryings, kValidVS, kValidFSes[0]);
}
...
static void test_sneaky_attribute_name(skiatest::Reporter* r)
{
    ...
    check_for_failure(r, kAttributes, 16, kValidVaryings, kValidVS, kValidFSes[0]);
}
