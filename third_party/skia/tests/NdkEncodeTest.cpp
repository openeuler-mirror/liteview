...
DEF_TEST(NdkEncode_unsupportedFormats, r)
{
    for (auto ct : { kRGBA_8888_SkColorType, kRGB_565_SkColorType, kRGBA_F16_SkColorType }) {
        ...
        bm.eraseColor(SK_ColorBLUE);
        for (auto format :
            { SkEncodedImageFormat::kBMP, SkEncodedImageFormat::kGIF, SkEncodedImageFormat::kICO, SkEncodedImageFormat::kWBMP, SkEncodedImageFormat::kPKM,
                SkEncodedImageFormat::kKTX, SkEncodedImageFormat::kASTC, SkEncodedImageFormat::kDNG, SkEncodedImageFormat::kHEIF }) {
...
DEF_TEST(NdkEncode_badQuality, r)
{
    for (auto ct : { kRGBA_8888_SkColorType, kRGB_565_SkColorType, kRGBA_F16_SkColorType }) {
        ...
        bm.allocPixels(SkImageInfo::Make(10, 10, ct, kOpaque_SkAlphaType));
        bm.eraseColor(SK_ColorBLUE);
        for (auto format : { SkEncodedImageFormat::kJPEG, SkEncodedImageFormat::kPNG, SkEncodedImageFormat::kWEBP }) {
...
{
    for (auto info : { SkImageInfo::MakeUnknown(), SkImageInfo::MakeN32Premul(10, 10), SkImageInfo::MakeN32Premul(0, 0) }) {
...
DEF_TEST(NdkEncode_badInfo, r)
{
    ...
    std::vector<SkPixmap> pixmaps { SkPixmap(SkImageInfo::MakeN32Premul(-10, 10), pixels, 1000), SkPixmap(SkImageInfo::MakeN32Premul(10, -10), pixels, 200),
        SkPixmap(SkImageInfo::MakeN32Premul(10, 10), pixels, 20), SkPixmap(SkImageInfo::MakeN32Premul(10, 10), pixels, 41),
        SkPixmap(SkImageInfo::MakeN32Premul(10, 10), pixels, 0), SkPixmap(SkImageInfo::MakeN32Premul(0, 0), pixels, 40) };
    if (sizeof(size_t) > sizeof(uint32_t)) {
        pixmaps.emplace_back(SkImageInfo::MakeN32Premul(10, 10), pixels, static_cast<size_t>(UINT32_MAX) + 1);
    }
...
static bool nearly_equal(SkColorSpace* a, SkColorSpace* b)
{
    ...
    return a && b && a->isNumericalTransferFn(&fnA) && a->toXYZD50(&gamutA) && b->isNumericalTransferFn(&fnB) && b->toXYZD50(&gamutB) && nearly_equal(fnA, fnB)
...
DEF_TEST(NdkEncode_unsupportedColorSpace, r)
{
    ...
    for (auto gamut : { SkNamedGamut::kSRGB, SkNamedGamut::kAdobeRGB, SkNamedGamut::kDisplayP3, SkNamedGamut::kRec2020, SkNamedGamut::kXYZ }) {
    ...
    for (auto gamut : { SkNamedGamut::kSRGB, SkNamedGamut::kDisplayP3, SkNamedGamut::kRec2020, SkNamedGamut::kXYZ }) {
    ...
    for (auto gamut : { SkNamedGamut::kAdobeRGB, SkNamedGamut::kDisplayP3, SkNamedGamut::kRec2020, SkNamedGamut::kXYZ }) {
    ...
    for (auto fn : { SkNamedTransferFn::kSRGB, SkNamedTransferFn::k2Dot2, SkNamedTransferFn::kLinear, SkNamedTransferFn::kRec2020 }) {
