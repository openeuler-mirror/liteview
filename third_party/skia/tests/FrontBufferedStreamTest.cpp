...
static void test_read(skiatest::Reporter* reporter, SkStream* bufferedStream, const void* expectations, size_t bytesToRead)
{
...
static void test_rewind(skiatest::Reporter* reporter, SkStream* bufferedStream, bool shouldSucceed)
{
...
static void test_hasLength(skiatest::Reporter* reporter, const SkStream& bufferedStream, const SkStream& streamBeingBuffered)
{
...
static void test_incremental_buffering(skiatest::Reporter* reporter, size_t bufferSize)
{
    ...
    auto bufferedStream = android::skia::FrontBufferedStream::Make(std::unique_ptr<SkStream>(memStream), bufferSize);
...
static void test_length_combos(skiatest::Reporter* reporter, size_t bufferSize)
{
    for (int hasLen = 0; hasLen <= 1; hasLen++) {
        for (int hasPos = 0; hasPos <= 1; hasPos++) {
            ...
            auto buffered = android::skia::FrontBufferedStream::Make(std::unique_ptr<SkStream>(stream), bufferSize);
...
static void test_initial_offset(skiatest::Reporter* reporter, size_t bufferSize)
{
    ...
    while (currentPosition < bufferedLength) {
        ...
        test_read(reporter, bufferedStream.get(), gAbcs + arbitraryOffset + currentPosition, amountToRead);
...
DEF_TEST(ShortFrontBufferedStream, reporter)
{
    ...
    auto stream = android::skia::FrontBufferedStream::Make(std::unique_ptr<SkStream>(failingStream), 64);
