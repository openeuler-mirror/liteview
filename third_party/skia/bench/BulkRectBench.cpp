...
template <int kRectCount, RectangleLayout kLayout, ImageMode kImageMode, DrawMode kDrawMode> class BulkRectBench : public Benchmark {
public:
    static_assert(kImageMode == ImageMode::kNone || kDrawMode != DrawMode::kQuad, "kQuad only supported for solid color draws");
    ...
    inline static constexpr int kImageCount = kImageMode == ImageMode::kShared ? 1 : (kImageMode == ImageMode::kNone ? 0 : kRectCount);
...
protected:
    ...
    void drawImagesBatch(SkCanvas* canvas) const
    {
        ...
        for (int i = 0; i < kRectCount; ++i) {
            ...
            batch[i].fSrcRect = SkRect::MakeIWH(fImages[imageIndex]->width(), fImages[imageIndex]->height());
        ...
        canvas->experimental_DrawEdgeAAImageSet(
            batch, kRectCount, nullptr, nullptr, SkSamplingOptions(SkFilterMode::kLinear), &paint, SkCanvas::kFast_SrcRectConstraint);
    }
    ...
    void drawImagesRef(SkCanvas* canvas) const
    {
        ...
        for (int i = 0; i < kRectCount; ++i) {
            ...
            canvas->drawImageRect(
                fImages[imageIndex].get(), srcRect, fRects[i], SkSamplingOptions(SkFilterMode::kLinear), &paint, SkCanvas::kFast_SrcRectConstraint);
        }
    }
    ...
    void drawSolidColorsRef(SkCanvas* canvas) const
    {
        ...
        for (int i = 0; i < kRectCount; ++i) {
            if (kDrawMode == DrawMode::kRef) {
                ...
                canvas->experimental_DrawEdgeAAQuad(fRects[i], nullptr, SkCanvas::kAll_QuadAAFlags, fColors[i], SkBlendMode::kSrcOver);
            }
        }
    }
