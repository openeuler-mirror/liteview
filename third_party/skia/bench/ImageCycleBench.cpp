...
protected:
    ...
    {
        auto ii = SkImageInfo::Make(kImageSize.fWidth, kImageSize.fHeight, kRGBA_8888_SkColorType, kPremul_SkAlphaType, nullptr);
        SkRandom random;
        ...
        for (int i = 0; i < fImageCnt; ++i) {
            ...
            surf->getCanvas()->drawRect(SkRect::MakeLTRB(1, 1, kImageSize.fWidth - 1, kImageSize.fHeight - 1), paint);
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        static const int imagesPerRow = SkScalarFloorToInt(kDeviceSize.fWidth / (kImageSize.fWidth + kPad));
        ...
        for (int l = 0; l < loops; ++l) {
            for (int r = 0; r < fRepeatCnt; ++r) {
                for (int i = 0; i < fImageCnt; ++i) {
                    ...
                    canvas->drawImage(fImages[i].get(), x, imageYOffset + rowYOffset, SkSamplingOptions(), &paint);
                }
            }
