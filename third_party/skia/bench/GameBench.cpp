...
protected:
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        SkPoint verts[4] = { // for drawVertices path
            { 0, 0 }, { 0, SkIntToScalar(height) }, { SkIntToScalar(width), SkIntToScalar(height) }, { SkIntToScalar(width), 0 }
        };
        ...
        for (int i = 0; i < loops; ++i, ++fNumSaved) {
            ...
            if (fUseAtlas) {
                ...
                if (fUseDrawVertices) {
                    ...
                    canvas->drawVertices(
                        SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, 4, verts, uvs, nullptr, 6, indices), SkBlendMode::kModulate, p2);
                } else {
                    canvas->drawImageRect(fAtlas, src, dst, SkSamplingOptions(), &p, SkCanvas::kFast_SrcRectConstraint);
                }
            } else {
    ...
    void makeAtlas()
    {
        ...
        for (int y = 0; y < kNumAtlasedY; ++y) {
            for (int x = 0; x < kNumAtlasedX; ++x) {
                ...
                fAtlasRects[x][y] = SkIRect::MakeXYWH(kAtlasSpacer + x * (kAtlasCellWidth + kAtlasSpacer), kAtlasSpacer + y * (kAtlasCellHeight + kAtlasSpacer),
...
DEF_BENCH(return new GameBench(GameBench::kTranslate_Type, GameBench::kFull_Clear, false, true, true);)
