...
class VertBench : public Benchmark {
    ...
    void onDelayedSetup() override
    {
        if (fFlags & kTexture_VertFlag) {
            ...
            if (img) {
                SkFilterMode fm = (fFlags & kBilerp_VertFlag) ? SkFilterMode::kLinear : SkFilterMode::kNearest;
...
protected:
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        auto verts = SkVertices::MakeCopy(SkVertices::kTriangles_VertexMode, PTS, fPts, texs, cols, IDX, fIdx);
...
protected:
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        for (int i = 0; i < loops; i++) {
            canvas->drawAtlas(fAtlas.get(), fXforms, fRects, colors, N, SkBlendMode::kModulate, SkSamplingOptions(), cullRect, paintPtr);
        }
    }
