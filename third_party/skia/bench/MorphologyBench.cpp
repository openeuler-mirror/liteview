    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        for (int i = 0; i < loops; i++) {
            ...
            if (fRadius > 0) {
                ...
                switch (fStyle) {
                case kDilate_MT:
                    mf = SkImageFilters::Dilate(SkScalarFloorToInt(fRadius), SkScalarFloorToInt(fRadius), nullptr);
                    break;
                case kErode_MT:
                    mf = SkImageFilters::Erode(SkScalarFloorToInt(fRadius), SkScalarFloorToInt(fRadius), nullptr);
                    break;
                }
