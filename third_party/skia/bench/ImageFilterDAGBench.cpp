...
protected:
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        for (int j = 0; j < loops; j++) {
            image = image->makeWithFilter(dContext, mergeFilter.get(), subset, subset, &discardSubset, &offset);
...
protected:
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        SkPaint paint;
        paint.setImageFilter(SkImageFilters::DisplacementMap(SkColorChannel::kR, SkColorChannel::kR, scale, blur, blur));
