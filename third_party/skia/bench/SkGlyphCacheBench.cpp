...
static void do_font_stuff(SkFont* font)
{
    ...
    for (SkScalar i = 8; i < 64; i++) {
        font->setSize(i);
        auto strikeSpec = SkStrikeSpec::MakeMask(*font, defaultPaint, SkSurfaceProps(0, kUnknown_SkPixelGeometry), SkScalerContextFlags::kNone, SkMatrix::I());
...
protected:
    ...
    void onDraw(int loops, SkCanvas*) override
    {
        ...
        sk_sp<SkTypeface> typefaces[]
            = { ToolUtils::create_portable_typeface("serif", SkFontStyle::Italic()), ToolUtils::create_portable_typeface("sans-serif", SkFontStyle::Italic()) };
...
class DiffCanvasBench : public Benchmark {
    ...
    void onDraw(int loops, SkCanvas* modelCanvas) override
    {
        ...
        std::unique_ptr<SkCanvas> canvas = fServer->makeAnalysisCanvas(1024, 1024, props, nullptr, true, true);
        loops *= 100;
        while (loops-- > 0) {
...
public:
    DiffCanvasBench(SkString n, std::function<std::unique_ptr<SkStreamAsset>()> f)
        : fBenchName(std::move(n))
        , fDataProvider(std::move(f))
    {
    }
};
} // namespace
