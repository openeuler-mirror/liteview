...
class DirectMaskGlyphVertexFillBenchmark : public Benchmark {
    ...
    void onPerCanvasPreDraw(SkCanvas* canvas) override
    {
        ...
        fBlob = sktext::gpu::TextBlob::Make(glyphRunList, paint, drawMatrix, device->strikeDeviceInfo(), SkStrikeCache::GlobalStrikeCache());
    ...
    void onDraw(int loops, SkCanvas* canvas) override
    {
        ...
        for (int loop = 0; loop < loops; loop++) {
            subRun->fillVertexData(fVertices.get(), 0, subRun->glyphCount(), grColor, positionMatrix, { 0, 0 }, clip);
        }
    }
