...
static sk_sp<GrDirectContext> make_mock_context()
{
    ...
    mockOptions.fConfigOptions[(int)GrColorType::kAlpha_8].fRenderability = GrMockOptions::ConfigOptions::Renderability::kMSAA;
...
#define DEF_PATH_TESS_BENCH(NAME, PATH, MATRIX)                                                                                                                \
    ...
    public:                                                                                                                                                    \
        PathTessellateBenchmark_##NAME()                                                                                                                       \
            : PathTessellateBenchmark(#NAME, (PATH), (MATRIX))                                                                                                 \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
...
static const SkMatrix gAlmostIdentity = SkMatrix::MakeAll(1.0001f, 0.0001f, 0.0001f, -.0001f, 0.9999f, -.0001f, 0, 0, 1);

DEF_PATH_TESS_BENCH(GrPathCurveTessellator, make_cubic_path(8), SkMatrix::I())
{
    SkArenaAlloc arena(1024);
    GrPipeline noVaryingsPipeline(GrScissorTest::kDisabled, SkBlendMode::kSrcOver, skgpu::Swizzle::RGBA());
    auto tess = PathCurveTessellator::Make(&arena, fTarget->caps().shaderCaps()->fInfinitySupport);
    tess->prepare(fTarget.get(), fMatrix, { gAlmostIdentity, fPath, SK_PMColor4fTRANSPARENT }, fPath.countVerbs());
}
...
DEF_PATH_TESS_BENCH(middle_out_triangulation, ToolUtils::make_star(SkRect::MakeWH(500, 500), kNumCubicsInChalkboard), SkMatrix::I())
{
    ...
    int baseVertex;
    VertexWriter vertexWriter = fTarget->makeVertexWriter(sizeof(SkPoint), maxVerts, &buffer, &baseVertex);
        ...
        for (int j = 0; j < numVerbs; ++j) {
            ...
            case 2:
                if (rand.nextULessThan(10) == 0) {
                    // Cusp.
                    auto [x, y] = (path.isEmpty()) ? SkPoint { 0, 0 } : SkPathPriv::PointData(path)[path.countPoints() - 1];
                    path.quadTo(x + rand.nextRangeF(0, 150), y, x - rand.nextRangeF(0, 150), y);
                } else {
                    path.quadTo(rand.nextRangeF(0, 150), rand.nextRangeF(0, 150), rand.nextRangeF(0, 150), rand.nextRangeF(0, 150));
                }
                break;
            case 3:
                if (rand.nextULessThan(10) == 0) {
                    ...
                    path.cubicTo(rand.nextRangeF(0, 150), y, rand.nextRangeF(0, 150), y, rand.nextRangeF(0, 150), y);
                } else {
                    path.cubicTo(rand.nextRangeF(0, 150), rand.nextRangeF(0, 150), rand.nextRangeF(0, 150), rand.nextRangeF(0, 150), rand.nextRangeF(0, 150),
...
private:
    ...
    {
        for (int i = 0; i < loops; ++i) {
            fTessellator->prepare(fTarget.get(), SkMatrix::Scale(fMatrixScale, fMatrixScale), fPathStrokes.data(), fTotalVerbCount);
            fTarget->resetAllocator();
        }
    }
...
DEF_BENCH(return new TessPrepareBench(make_simple_cubic_path, PatchAttribs::kNone, 1, "GrStrokeFixedCountTessellator");)

DEF_BENCH(return new TessPrepareBench(make_simple_cubic_path, PatchAttribs::kNone, 5, "GrStrokeFixedCountTessellator_one_chop");)

DEF_BENCH(return new TessPrepareBench(make_motionmark_paths, PatchAttribs::kStrokeParams, 1, "GrStrokeFixedCountTessellator_motionmark");)
