...
SKPAnimationBench::SKPAnimationBench(const char* name, const SkPicture* pic, const SkIRect& clip, sk_sp<Animation> animation, bool doLooping)
...
void SKPAnimationBench::drawPicture()
{
    for (int j = 0; j < this->tileRects().size(); ++j) {
        SkMatrix trans = SkMatrix::Translate(-1.f * this->tileRects()[j].fLeft, -1.f * this->tileRects()[j].fTop);
...
public:
    ...
    void preConcatFrameMatrix(double animationTimeMs, const SkIRect& devBounds, SkMatrix* drawMatrix) override
    {
        ...
        SkPoint center = SkPoint::Make((devBounds.fLeft + devBounds.fRight) / 2.0f, (devBounds.fTop + devBounds.fBottom) / 2.0f);
...
sk_sp<SKPAnimationBench::Animation> SKPAnimationBench::MakeZoomAnimation(SkScalar zoomMax, double zoomPeriodMs)
{
