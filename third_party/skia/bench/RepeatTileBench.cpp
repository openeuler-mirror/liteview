...
static void draw_into_bitmap(const SkBitmap& bm)
{
    ...
    canvas.drawCircle(SkIntToScalar(w) / 2, SkIntToScalar(h) / 2, SkIntToScalar(std::min(w, h)) * 3 / 8, p);
...
public:
        ...
        : fAlphaType(at)
    {
        ...
        fName.printf("repeatTile_%s_%c", ToolUtils::colortype_name(ct), kOpaque_SkAlphaType == at ? 'X' : 'A');
    }
...
protected:
    ...
    void onDelayedSetup() override
    {
        ...
        fPaint.setShader(fBitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions()));
    }
