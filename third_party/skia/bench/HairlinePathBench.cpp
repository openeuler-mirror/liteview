...
static const int points[] = { 10, 10, 15, 5, 20, 20, 30, 5, 25, 20, 15, 12, 21, 21, 30, 30, 12, 4, 32, 28, 20, 18, 12, 10 };
...
protected:
    ...
    {
        fName.printf("path_hairline_%s_%s_", fFlags & kBig_Flag ? "big" : "small", fFlags & kAA_Flag ? "AA" : "noAA");
...
public:
    ...
    void makePath(SkPath* path) override
    {
        ...
        for (int i = 0; i < kMaxPathSize; ++i) {
            ...
            path->moveTo(SkIntToScalar(points[base1] + xTrans), SkIntToScalar(points[base1 + 1] + yTrans));
            path->lineTo(SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans));
            path->lineTo(SkIntToScalar(points[base3] + xTrans), SkIntToScalar(points[base3 + 1] + yTrans));
        }
    }
...
public:
    ...
    void makePath(SkPath* path) override
    {
        ...
        for (int i = 0; i < kMaxPathSize; ++i) {
            ...
            int base3 = 2 * rand.nextULessThan(hSize);
            path->moveTo(SkIntToScalar(points[base1] + xTrans), SkIntToScalar(points[base1 + 1] + yTrans));
            path->quadTo(SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans), SkIntToScalar(points[base3] + xTrans),
...
public:
    ...
    void makePath(SkPath* path) override
    {
        ...
        for (int i = 0; i < kMaxPathSize; ++i) {
            ...
            path->moveTo(SkIntToScalar(points[base1] + xTrans), SkIntToScalar(points[base1 + 1] + yTrans));
            path->conicTo(SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans), SkIntToScalar(points[base3] + xTrans),
                SkIntToScalar(points[base3 + 1] + yTrans), weight);
        }
    }
...
public:
    ...
    void makePath(SkPath* path) override
    {
        ...
        for (int i = 0; i < kMaxPathSize; ++i) {
            ...
            path->moveTo(SkIntToScalar(points[base1] + xTrans), SkIntToScalar(points[base1 + 1] + yTrans));
            path->cubicTo(SkIntToScalar(points[base2] + xTrans), SkIntToScalar(points[base2 + 1] + yTrans), SkIntToScalar(points[base3] + xTrans),
                SkIntToScalar(points[base3 + 1] + yTrans), SkIntToScalar(points[base4] + xTrans), SkIntToScalar(points[base4 + 1] + yTrans));
        }
    }
