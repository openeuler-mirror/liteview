...
private:
    void gatherRects(SkTArray<SkRect>* rects) override
    {
        ...
        for (int i = 0; i < fNumRandomRects; ++i) {
            rects->push_back(SkRect::MakeXYWH(rand.nextRangeF(0, 2000), rand.nextRangeF(0, 2000), rand.nextRangeF(0, 70), rand.nextRangeF(0, 70)));
        }
    }
...
private:
    ...
    void gatherRects(SkTArray<SkRect>* rects) override
    {
        ...
        ToolUtils::sniff_paths(FLAGS_boundsManagerFile[0], [&](const SkMatrix& matrix, const SkPath& path, const SkPaint& paint) {
...
#if PRINT_DRAWSET_COUNT
        SkDebugf("%s bounds are [%f %f %f %f]\n", FLAGS_boundsManagerFile[0], fileBounds.fLeft, fileBounds.fTop, fileBounds.fRight, fileBounds.fBottom);
#endif
    }
};
