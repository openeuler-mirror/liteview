...
struct GPUTarget : public Target {
    ...
    bool init(SkImageInfo info, Benchmark* bench) override
    {
        ...
        this->surface = SkSurface::MakeRenderTarget(
            this->factory->get(this->config.ctxType, this->config.ctxOverrides), SkBudgeted::kNo, info, this->config.samples, &props);
        this->contextInfo = this->factory->getContextInfo(this->config.ctxType, this->config.ctxOverrides);
        if (!this->surface) {
            return false;
        }
...
struct GraphiteTarget : public Target {
    explicit GraphiteTarget(const Config& c)
        : Target(c)
    {
    }
...
static std::optional<Config> create_config(const SkCommandLineConfig* config)
{
    if (const auto* gpuConfig = config->asConfigGpu()) {
        ...
        if (const auto ctx = factory.get(ctxType, ctxOverrides)) {
            ...
            int supportedSampleCount = ctx->priv().caps()->getRenderTargetSampleCount(sampleCount, format);
        ...
        return Config { gpuConfig->getTag(), Benchmark::kGPU_Backend, colorType, kPremul_SkAlphaType, config->refColorSpace(), sampleCount, ctxType,
            ctxOverrides, kBogusGraphiteContextType, gpuConfig->getSurfaceFlags() };
    }
#ifdef SK_GRAPHITE_ENABLED
    if (const auto* gpuConfig = config->asConfigGraphite()) {
        ...
        return Config { gpuConfig->getTag(), Benchmark::kGraphite_Backend, colorType, kPremul_SkAlphaType, config->refColorSpace(), sampleCount,
            kBogusContextType, kBogusContextOverrides, graphiteCtxType, 0 };
    }
#endif
...
#define CPU_CONFIG(name, backend, color, alpha)                                                                                                                \
    if (config->getBackend().equals(name)) {                                                                                                                   \
        ...
        return Config { SkString(name), Benchmark::backend, color, alpha, config->refColorSpace(), 0, kBogusContextType, kBogusContextOverrides,               \
            kBogusGraphiteContextType, 0 };                                                                                                                    \
    }
...
static Target* is_enabled(Benchmark* bench, const Config& config)
{
    ...
    SkImageInfo info = SkImageInfo::Make(bench->getSize().fX, bench->getSize().fY, config.color, config.alpha, config.colorSpace);
...
#ifdef SK_ENABLE_ANDROID_UTILS
static bool valid_brd_bench(sk_sp<SkData> encoded, SkColorType colorType, uint32_t sampleSize, uint32_t minOutputSize, int* width, int* height)
{
...
static void collect_files(const CommandLineFlags::StringArray& paths, const char* ext, SkTArray<SkString>* list)
{
...
public:
    ...
    static sk_sp<SkPicture> ReadSVGPicture(const char* path)
    {
        ...
        svgDom->render(recorder.beginRecording(svgDom->containerSize().width(), svgDom->containerSize().height()));
    ...
    Benchmark* next()
    {
        ...
        } while (CommandLineFlags::ShouldSkip(FLAGS_sourceType, fSourceType) || CommandLineFlags::ShouldSkip(FLAGS_benchType, fBenchType));
    ...
    Benchmark* rawNext()
    {
        ...
        while (fCurrentTextBlobTrace < fTextBlobTraces.count()) {
            ...
            fBenchType = "micro";
            return CreateDiffCanvasBench(SkStringPrintf("SkDiffBench-%s", basename.c_str()), [path]() { return SkStream::MakeFromFile(path.c_str()); });
        }
        ...
        while (fCurrentScale < fScales.count()) {
            while (fCurrentSKP < fSKPs.count()) {
                ...
                if (FLAGS_bbh) {
                    ...
                    SkPictureRecorder recorder;
                    pic->playback(recorder.beginRecording(pic->cullRect().width(), pic->cullRect().height(), &factory));
            ...
            while (fCurrentSVG < fSVGs.count()) {
                ...
                if (sk_sp<SkPicture> pic = ReadSVGPicture(path)) {
                    fSourceType = "svg";
                    fBenchType = "playback";
                    return new SKPBench(SkOSPath::Basename(path).c_str(), pic.get(), fClip, fScales[fCurrentScale], FLAGS_loopSKP);
                }
            }
        ...
        if (fZoomMax != 1.0f && fZoomPeriodMs > 0) {
            while (fCurrentAnimSKP < fSKPs.count()) {
                ...
                fCurrentAnimSKP++;
                SkString name = SkOSPath::Basename(path.c_str());
                sk_sp<SKPAnimationBench::Animation> animation = SKPAnimationBench::MakeZoomAnimation(fZoomMax, fZoomPeriodMs);
                return new SKPAnimationBench(name.c_str(), pic.get(), fClip, std::move(animation), FLAGS_loopSKP);
            }
        }
        ...
        for (; fCurrentCodec < fImages.count(); fCurrentCodec++) {
            ...
            while (fCurrentColorType < fColorTypes.count()) {
                ...
                case SkCodec::kIncompleteInput:
                    return new CodecBench(SkOSPath::Basename(path.c_str()), encoded.get(), colorType, alphaType);
                ...
                while (fCurrentSampleSize < (int)std::size(brdSampleSizes)) {
                    while (fCurrentSubsetType <= kLastSingle_SubsetType) {
                        ...
                        case kMiddle_SubsetType:
                            ...
                            subset = SkIRect::MakeXYWH((width - subsetSize) / 2, (height - subsetSize) / 2, subsetSize, subsetSize);
                            break;
                        ...
                        case kBottomRight_SubsetType:
                            ...
                            subset = SkIRect::MakeXYWH(width - subsetSize, height - subsetSize, subsetSize, subsetSize);
                            break;
                        default:
                            SkASSERT(false);
                        }

                        return new BitmapRegionDecoderBench(basename.c_str(), encoded.get(), colorType, sampleSize, subset);
                    }
    ...
    void fillCurrentOptions(NanoJSONResultsWriter& log) const
    {
        ...
        if (0 == strcmp(fSourceType, "skp")) {
            log.appendString("clip", SkStringPrintf("%d %d %d %d", fClip.fLeft, fClip.fTop, fClip.fRight, fClip.fBottom));
...
int main(int argc, char** argv)
{
    ...
    while (Benchmark* b = benchStream.next()) {
        ...
        for (int i = 0; i < configs.count(); ++i) {
            ...
            TRACE_EVENT2("skia", "Benchmark", "name", TRACE_STR_COPY(bench->getUniqueName()), "config", TRACE_STR_COPY(config));
            ...
            int maxFrameLag;
            int loops
                = target->needsFrameTiming(&maxFrameLag) ? setup_gpu_bench(target, bench.get(), maxFrameLag) : setup_cpu_bench(overhead, target, bench.get());
            ...
            if (kAutoTuneLoops != FLAGS_loops) {
                ...
                SkDebugf("%4d/%-4dMB\t%s\t%s ", sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(), bench->getUniqueName(), config);
                ...
                const double stddev_percent = sk_ieee_double_divide(100 * sqrt(stats.var), stats.mean);
                SkDebugf("%g,%g,%g,%g,%g,%s,%s\n", stats.min, stats.median, stats.mean, stats.max, stddev_percent, config, bench->getUniqueName());
            } else {
                const double stddev_percent = sk_ieee_double_divide(100 * sqrt(stats.var), stats.mean);
                SkDebugf("%4d/%-4dMB\t%d\t%s\t%s\t%s\t%s\t%.0f%%\t%s\t%s\t%s\n", sk_tools::getCurrResidentSetSizeMB(), sk_tools::getMaxResidentSetSizeMB(),
                    loops, HUMANIZE(stats.min), HUMANIZE(stats.median), HUMANIZE(stats.mean), HUMANIZE(stats.max), stddev_percent,
                    FLAGS_ms ? to_string(samples.count()).c_str() : stats.plot.c_str(), config, bench->getUniqueName());
            }
