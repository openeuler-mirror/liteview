...
public:
    ...
    SkSLCompileBench(std::string name, const char* src, bool optimize, Output output)
        : fName(std::string("sksl_") + (optimize ? "" : "unoptimized_") + output_string(output) + name)
        , fSrc(src)
...
protected:
    ...
    void fixUpSource()
    {
        auto fixup = [this](const char* input, const char* replacement) { fSrc = std::regex_replace(fSrc, std::regex(input), replacement); };
    ...
    {
        const SkSL::ProgramKind kind = this->usesRuntimeShader() ? SkSL::ProgramKind::kRuntimeShader : SkSL::ProgramKind::kFragment;
        for (int i = 0; i < loops; i++) {
            ...
            case Output::kGLSL:
                ...
                break;
            case Output::kMetal:
                SkAssertResult(fCompiler.toMetal(*program, &result));
                break;
            case Output::kSPIRV:
                SkAssertResult(fCompiler.toSPIRV(*program, &result));
                break;
            case Output::kSkVM:
            case Output::kSkVMOpt:
            case Output::kSkVMJIT:
                SkAssertResult(CompileToSkVM(*program, fOutput));
                break;
            }
        }
    }
...
{
    SkDEBUGCODE(SkDebugf("%s: %d bytes\n", name, bytes);) log->beginObject(name); // test
...
void RunSkSLModuleBenchmarks(NanoJSONResultsWriter* log)
{
    ...
    int compilerGPUBinarySize = std::size(SKSL_MINIFIED_sksl_shared) + std::size(SKSL_MINIFIED_sksl_gpu) + std::size(SKSL_MINIFIED_sksl_vert)
        + std::size(SKSL_MINIFIED_sksl_frag) + std::size(SKSL_MINIFIED_sksl_public) + std::size(SKSL_MINIFIED_sksl_rt_shader);
    bench(log, "sksl_binary_size_gpu", compilerGPUBinarySize);

#if defined(SK_GRAPHITE_ENABLED)
    int compilerGraphiteBinarySize = std::size(SKSL_MINIFIED_sksl_graphite_frag) + std::size(SKSL_MINIFIED_sksl_graphite_vert);
...
public:
    SkSLModuleLoaderBench(const char* name, std::vector<SkSL::ProgramKind> moduleList)
        : fName(name)
        , fModuleList(std::move(moduleList))
    {
    }
