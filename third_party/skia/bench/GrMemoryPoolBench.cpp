...
template <typename T> static void run_stack(GrMemoryPool* pool, int loops)
{
...
template <typename T> static void run_queue(GrMemoryPool* pool, int loops)
{
    static const int kMaxObjects = 4 * (1 << 10);
    T* objs[kMaxObjects];
    for (int i = 0; i < loops; ++i) {
...
template <typename T> static void run_pushpop(GrMemoryPool* pool, int loops)
{
    static const int kMaxObjects = 4 * (1 << 10);
    T* objs[kMaxObjects];
    for (int i = 0; i < loops; ++i) {
...
template <typename T> static void run_random(GrMemoryPool* pool, int loops)
{
    static const int kMaxObjects = 4 * (1 << 10);
    T* objs[kMaxObjects];
