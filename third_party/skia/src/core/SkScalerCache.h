...
public:
    ...
    std::tuple<SkGlyph*, size_t> mergeGlyphAndImage(SkPackedGlyphID toID, const SkGlyph& from) SK_EXCLUDES(fMu);

    // If the path has never been set, then add a path to glyph.
    std::tuple<const SkPath*, size_t> mergePath(SkGlyph* glyph, const SkPath* path, bool hairline) SK_EXCLUDES(fMu);

    // If the drawable has never been set, then add a drawble to glyph.
    std::tuple<SkDrawable*, size_t> mergeDrawable(SkGlyph* glyph, sk_sp<SkDrawable> drawable) SK_EXCLUDES(fMu);
    ...
    */
    void findIntercepts(const SkScalar bounds[2], SkScalar scale, SkScalar xPos, SkGlyph*, SkScalar* array, int* count) SK_EXCLUDES(fMu);
    ...
    std::tuple<SkSpan<const SkGlyph*>, size_t> metrics(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[]) SK_EXCLUDES(fMu);

    std::tuple<SkSpan<const SkGlyph*>, size_t> preparePaths(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[]) SK_EXCLUDES(fMu);

    std::tuple<SkSpan<const SkGlyph*>, size_t> prepareImages(SkSpan<const SkPackedGlyphID> glyphIDs, const SkGlyph* results[]) SK_EXCLUDES(fMu);

    std::tuple<SkSpan<const SkGlyph*>, size_t> prepareDrawables(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[]) SK_EXCLUDES(fMu);
    ...
    std::tuple<SkRect, size_t> prepareForMaskDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) SK_EXCLUDES(fMu);

    std::tuple<SkRect, size_t> prepareForSDFTDrawing(SkScalar strikeToSourceScale, SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
        SK_EXCLUDES(fMu);

    size_t prepareForPathDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) SK_EXCLUDES(fMu);

    size_t prepareForDrawableDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) SK_EXCLUDES(fMu);
    ...
    std::tuple<SkScalar, size_t> findMaximumGlyphDimension(SkSpan<const SkGlyphID> glyphs) SK_EXCLUDES(fMu);
...
private:
    template <typename Fn> size_t commonFilterLoop(SkDrawableGlyphBuffer* accepted, Fn&& fn) SK_REQUIRES(fMu);
    ...
    std::tuple<SkSpan<const SkGlyph*>, size_t> internalPrepare(SkSpan<const SkGlyphID> glyphIDs, PathDetail pathDetail, const SkGlyph** results)
        SK_REQUIRES(fMu);
    ...
    SkTHashMap<SkPackedGlyphID, SkGlyphDigest, SkPackedGlyphID::Hash> fDigestForPackedGlyphID SK_GUARDED_BY(fMu);
