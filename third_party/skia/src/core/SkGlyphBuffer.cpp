...
void SkDrawableGlyphBuffer::startSourceWithMatrixAdjustment(const SkZip<const SkGlyphID, const SkPoint>& source, const SkMatrix& creationMatrix)
{
    fInputSize = source.size();
    fAcceptedSize = 0;
...
void SkDrawableGlyphBuffer::startDevicePositioning(
    const SkZip<const SkGlyphID, const SkPoint>& source, const SkMatrix& positionMatrix, const SkGlyphPositionRoundingSpec& roundingSpec)
{
    fInputSize = source.size();
    fAcceptedSize = 0;
    ...
    auto floor = [](SkPoint pt) -> SkPoint { return { SkScalarFloorToScalar(pt.x()), SkScalarFloorToScalar(pt.y()) }; };
...
SkString SkDrawableGlyphBuffer::dumpInput() const
{
    ...
    SkString msg;
    for (auto [packedGlyphID, pos] : SkZip<SkGlyphVariant, SkPoint> { fInputSize, fMultiBuffer.get(), fPositions.get() }) {
