...
public:
    ...
    {
        return sk_sp<SkTypeface>(new SkEmptyTypeface);
    }
...
{
    return SkFontStyle((oldStyle & SkTypeface::kBold) ? SkFontStyle::kBold_Weight : SkFontStyle::kNormal_Weight, SkFontStyle::kNormal_Width,
...
{
    if (nullptr == name && (fontStyle.slant() == SkFontStyle::kItalic_Slant || fontStyle.slant() == SkFontStyle::kUpright_Slant)
        && (fontStyle.weight() == SkFontStyle::kBold_Weight || fontStyle.weight() == SkFontStyle::kNormal_Weight)) {
        return sk_ref_sp(
            GetDefaultTypeface(static_cast<SkTypeface::Style>((fontStyle.slant() == SkFontStyle::kItalic_Slant ? SkTypeface::kItalic : SkTypeface::kNormal)
                | (fontStyle.weight() == SkFontStyle::kBold_Weight ? SkTypeface::kBold : SkTypeface::kNormal))));
    }
...
sk_sp<SkTypeface> SkTypeface::MakeDeserialize(SkStream* stream)
{
    ...
    if (desc.hasStream()) {
        ...
        args.setPalette({ desc.getPaletteIndex(), desc.getPaletteEntryOverrides(), desc.getPaletteEntryOverrideCount() });
