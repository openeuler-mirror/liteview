...
bool SkPixmap::scalePixels(const SkPixmap& actualDst, const SkSamplingOptions& sampling) const
{
    ...
    sk_sp<SkSurface> surface = SkSurface::MakeRasterDirect(dst.info(), dst.writable_addr(), dst.rowBytes());
    if (!shader || !surface) {
        return false;
    }
...
SkColor SkPixmap::getColor(int x, int y) const
{
    ...
    case kSRGBA_8888_SkColorType: {
        auto srgb_to_linear = [](float x) { return (x <= 0.04045f) ? x * (1 / 12.92f) : sk_float_pow(x * (1 / 1.055f) + (0.055f / 1.055f), 2.4f); };
