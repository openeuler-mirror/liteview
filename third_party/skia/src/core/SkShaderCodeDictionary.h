...
public:
    constexpr SkTextureAndSampler(const char* name)
        : fName(name)
    {
    }
...
struct SkShaderSnippet {
    using GeneratePreambleForSnippetFn = void (*)(const SkShaderInfo& shaderInfo, int* entryIndex, const SkPaintParamsKey::BlockReader&, std::string* preamble);
    struct Args {
    ...
    using GenerateExpressionForSnippetFn
        = std::string (*)(const SkShaderInfo& shaderInfo, int entryIndex, const SkPaintParamsKey::BlockReader&, const Args& args);

    SkShaderSnippet() = default;

    SkShaderSnippet(const char* name, SkSpan<const SkUniform> uniforms, SkEnumBitMask<SnippetRequirementFlags> snippetRequirementFlags,
        SkSpan<const SkTextureAndSampler> texturesAndSamplers, const char* functionName, GenerateExpressionForSnippetFn expressionGenerator,
        GeneratePreambleForSnippetFn preambleGenerator, int numChildren, SkSpan<const SkPaintParamsKey::DataPayloadField> dataPayloadExpectations)
        : fName(name)
        , fUniforms(uniforms)
    ...
    std::string getMangledUniformName(const SkShaderInfo& shaderInfo, int uniformIdx, int mangleId) const;
...
class SkShaderInfo {
public:
    SkShaderInfo(const SkRuntimeEffectDictionary* rteDict = nullptr, const char* ssboIndex = nullptr)
    ...
    std::string toSkSL(const skgpu::graphite::RenderStep* step, const bool defineShadingSsboIndexVarying, const bool defineLocalCoordsVarying) const;
#endif
...
public:
    ...
    public:
        ...
        const SkPaintParamsKey& paintParamsKey() const
        {
            return fKey;
        }
#ifdef SK_GRAPHITE_ENABLED
...
#else
        Entry(const SkPaintParamsKey& key)
            : fKey(key.asSpan())
        {
        }
#endif
    ...
    SkEnumBitMask<SnippetRequirementFlags> getSnippetRequirementFlags(SkBuiltInCodeSnippetID id) const
    {
    ...
    int addUserDefinedSnippet(const char* name, SkSpan<const SkPaintParamsKey::DataPayloadField> expectations);
...
private:
    ...
    int addUserDefinedSnippet(const char* name, SkSpan<const SkUniform> uniforms, SkEnumBitMask<SnippetRequirementFlags> snippetRequirementFlags,
        SkSpan<const SkTextureAndSampler> texturesAndSamplers, const char* functionName, SkShaderSnippet::GenerateExpressionForSnippetFn expressionGenerator,
        SkShaderSnippet::GeneratePreambleForSnippetFn preambleGenerator, int numChildren,
