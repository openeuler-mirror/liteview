...
void SkDraw::paintMasks(SkDrawableGlyphBuffer* accepted, const SkPaint& paint) const
{
    ...
    SkBlitter* blitter = SkBlitter::Choose(fDst, *fMatrixProvider, paint, &alloc, false, fRC->clipShader(), SkSurfacePropsCopyOrDefault(fProps));
            ...
            if (check_glyph_position(pos)) {
                ...
                if (!clipper.done()) {
                    if (SkMask::kARGB32_Format == mask.fFormat) {
                        SkBitmap bm;
                        bm.installPixels(SkImageInfo::MakeN32Premul(mask.fBounds.size()), mask.fImage, mask.fRowBytes);
...
void SkDraw::drawGlyphRunList(SkCanvas* canvas, SkGlyphRunListPainterCPU* glyphPainter, const sktext::GlyphRunList& glyphRunList, const SkPaint& paint) const
{
    ...
    glyphPainter->drawForBitmapDevice(canvas, this, glyphRunList, paint, fMatrixProvider->localToDevice());
}
