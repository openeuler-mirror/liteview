...
void SkPaintParamsKeyBuilder::endBlock()
{
    ...
    SkASSERT(fStack.back().fCurDataPayloadEntry == SkTo<int>(fStack.back().fDataPayloadExpectations.size()));
...
static int field_size(DataPayloadType type)
{
    ...
    case DataPayloadType::kByte:
        return 1;
    case DataPayloadType::kInt:
        return 4;
    case DataPayloadType::kFloat4:
        return 16;
    }
    SkUNREACHABLE;
}

void SkPaintParamsKeyBuilder::addToKey(uint32_t count, const void* data, DataPayloadType payloadType)
{
    if (!this->isValid()) {
        return;
    }
    ...
    SkDEBUGCODE(this->checkExpectations(payloadType, count);) SkASSERT(!this->isLocked());
...
//--------------------------------------------------------------------------------------------------
SkPaintParamsKey::SkPaintParamsKey(SkSpan<const uint8_t> span, SkPaintParamsKeyBuilder* originatingBuilder)
    : fData(span)
...
SkPaintParamsKey::BlockReader SkPaintParamsKey::reader(const SkShaderCodeDictionary* dict, int headerOffset) const
{
...
void SkPaintParamsKey::AddBlockToShaderInfo(const SkShaderCodeDictionary* dict, const SkPaintParamsKey::BlockReader& reader, SkShaderInfo* result)
{
...
void SkPaintParamsKey::toShaderInfo(const SkShaderCodeDictionary* dict, SkShaderInfo* result) const
{
...
bool SkPaintParamsKey::isErrorKey() const
{
    ...
    return header.codeSnippetID == (int32_t)SkBuiltInCodeSnippetID::kError && header.blockSize == sizeof(Header);
}
#endif

////////////////////////////////////////////////////////////////////////////////////////////////////

SkPaintParamsKey::BlockReader::BlockReader(const SkShaderCodeDictionary* dict, SkSpan<const uint8_t> parentSpan, int offsetInParent)
{
...
int SkPaintParamsKey::BlockReader::numChildren() const
{
    return fEntry->fNumChildren;
}

SkPaintParamsKey::BlockReader SkPaintParamsKey::BlockReader::child(const SkShaderCodeDictionary* dict, int childIndex) const
{
...
template <typename T> static SkSpan<const T> payload_subspan_for_field(SkSpan<const uint8_t> dataPayload, SkSpan<const DataPayloadField> fields, int fieldIndex)
{
...
SkSpan<const uint8_t> SkPaintParamsKey::BlockReader::bytes(int fieldIndex) const
{
    ...
    return payload_subspan_for_field<uint8_t>(this->dataPayload(), fEntry->fDataPayloadExpectations, fieldIndex);
}
...
SkSpan<const int32_t> SkPaintParamsKey::BlockReader::ints(int fieldIndex) const
{
    ...
    return payload_subspan_for_field<int32_t>(this->dataPayload(), fEntry->fDataPayloadExpectations, fieldIndex);
}
...
SkSpan<const SkColor4f> SkPaintParamsKey::BlockReader::colors(int fieldIndex) const
{
    ...
    return payload_subspan_for_field<SkColor4f>(this->dataPayload(), fEntry->fDataPayloadExpectations, fieldIndex);
}
...
void SkPaintParamsKey::BlockReader::dump(const SkShaderCodeDictionary* dict, int indent) const
{
    ...
    for (int i = 0; i < (int)fEntry->fDataPayloadExpectations.size(); ++i) {
        output_indent(indent);
        SkDebugf("%s[%d]: ", fEntry->fDataPayloadExpectations[i].fName, fEntry->fDataPayloadExpectations[i].fCount);
