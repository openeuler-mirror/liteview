...
public:
    ...
    bool SK_WARN_UNUSED_RESULT decomposeCTM(const SkMatrix& ctm, const SkImageFilter* filter, const skif::ParameterSpace<SkPoint>& representativePt);
...
public:
    ...
    FilterResult(sk_sp<SkSpecialImage> image, const LayerSpace<SkIPoint>& origin)
        ...
        , fLayerBounds(SkIRect::MakeXYWH(origin.x(), origin.y(), fImage ? fImage->width() : 0, fImage ? fImage->height() : 0))
    {
    }
    ...
    FilterResult applyCrop(const Context& ctx, const LayerSpace<SkIRect>& crop) const;
...
private:
    ...
    void concatTransform(const LayerSpace<SkMatrix>& transform, const SkSamplingOptions& newSampling, const LayerSpace<SkIRect>& outputBounds);
...
public:
    ...
    Context(const SkMatrix& layerMatrix, const SkIRect& clipBounds, SkImageFilterCache* cache, SkColorType colorType, SkColorSpace* colorSpace,
    ...
    Context(const Mapping& mapping, const LayerSpace<SkIRect>& desiredOutput, SkImageFilterCache* cache, SkColorType colorType, SkColorSpace* colorSpace,
