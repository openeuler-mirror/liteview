...
static uint32_t read_cpu_features()
{
    ...
    for (int core = 0; features & SkCpu::ASIMDHP; core++) {
        ...
        SkString path = SkStringPrintf("/sys/devices/system/cpu/cpu%d/regs/identification/midr_el1", core);
        ...
        if (std::size(buf) != midr_el1.read(buf, std::size(buf)) || 0 == memcmp(kMongoose3, buf, std::size(buf))) {
            features &= ~(SkCpu::ASIMDHP);
        }
    }
    return features;
