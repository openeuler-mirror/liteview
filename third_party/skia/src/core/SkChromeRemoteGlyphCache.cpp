...
template <typename T> size_t serialization_alignment()
{
...
public:
    ...
    bool readDescriptor(SkAutoDescriptor* ad)
    {
        ...
        if (!this->read<uint32_t>(&descLength))
            return false;
...
public:
    ...
    SkRect prepareForMaskDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) override;

    SkRect prepareForSDFTDrawing(SkScalar strikeToSourceScale, SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) override;

    void prepareForPathDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) override;

    void prepareForDrawableDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected) override;
...
SkRect RemoteStrike::prepareForMaskDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
...
SkRect RemoteStrike::prepareForSDFTDrawing(SkScalar strikeToSourceScale, SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    SkGlyphRect boundingRect = skglyph::empty_rect();
    for (auto [i, variant, pos] : SkMakeEnumerate(accepted->input())) {
        SkPackedGlyphID packedID = variant.packedID();
        ...
        if (digest.canDrawAsSDFT()) {
            if (!digest.isEmpty()) {
                ...
                const SkGlyphRect glyphBounds = digest.bounds().inset(SK_DistanceFieldInset, SK_DistanceFieldInset).scaleAndOffset(strikeToSourceScale, pos);
                boundingRect = skglyph::rect_union(boundingRect, glyphBounds);
                accepted->accept(packedID, glyphBounds.leftTop(), digest.maskFormat());
            }
        } else {
...
void RemoteStrike::prepareForPathDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
...
void RemoteStrike::prepareForDrawableDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    accepted->forEachInput([&](size_t i, SkPackedGlyphID packedID, SkPoint position) {
...
struct WireTypeface {
    ...
    WireTypeface(SkTypefaceID typefaceId, int glyphCount, SkFontStyle style, bool isFixed, bool needsCurrentColor)
        : fTypefaceID(typefaceId)
...
sk_sp<SkData> SkStrikeServerImpl::serializeTypeface(SkTypeface* tf)
{
    ...
    WireTypeface wire(SkTypeface::UniqueID(tf), tf->countGlyphs(), tf->fontStyle(), tf->isFixedPitch(), tf->glyphMaskNeedsCurrentColor());
    data = fSerializedTypefaces.set(SkTypeface::UniqueID(tf), SkData::MakeWithCopy(&wire, sizeof(wire)));
    return *data;
}
...
sktext::ScopedStrikeForGPU SkStrikeServerImpl::findOrCreateScopedStrike(const SkStrikeSpec& strikeSpec)
{
...
RemoteStrike* SkStrikeServerImpl::getOrCreateCache(const SkStrikeSpec& strikeSpec)
{
    ...
    if (!fCachedTypefaces.contains(typefaceId)) {
        ...
        fTypefacesToSend.emplace_back(typefaceId, typeface.countGlyphs(), typeface.fontStyle(), typeface.isFixedPitch(), typeface.glyphMaskNeedsCurrentColor());
    }
...
public:
    ...
    SkBaseDevice* onCreateDevice(const CreateInfo& cinfo, const SkPaint*) override
    {
        ...
        return new GlyphTrackingDevice(cinfo.fInfo.dimensions(), surfaceProps, fStrikeServerImpl, cinfo.fInfo.refColorSpace(), fSDFTControl);
    }
...
protected:
    void onDrawGlyphRunList(SkCanvas*, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint) override
    {
        ...
        auto container = SubRunContainer::MakeInAlloc(glyphRunList, drawMatrix, drawingPaint, this->strikeDeviceInfo(), fStrikeServerImpl, &tempAlloc,
            SubRunContainer::kStrikeCalculationsOnly, "Cache Diff");
        // Calculations only. No SubRuns.
        SkASSERT(container->isEmpty());
    }

    sk_sp<sktext::gpu::Slug> convertGlyphRunListToSlug(
        const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint) override
    {
        ...
        STSubRunAllocator<sizeof(SubRunContainer), alignof(SubRunContainer)> tempAlloc;
        auto container = SubRunContainer::MakeInAlloc(glyphRunList, positionMatrix, drawingPaint, this->strikeDeviceInfo(), fStrikeServerImpl, &tempAlloc,
            SubRunContainer::kStrikeCalculationsOnly, "Convert Slug Analysis");
        // Calculations only. No SubRuns.
        SkASSERT(container->isEmpty());

        // Use the glyph strike cache to get actual glyph information.
        return skgpu::v1::MakeSlug(
            this->localToDevice(), glyphRunList, initialPaint, drawingPaint, this->strikeDeviceInfo(), SkStrikeCache::GlobalStrikeCache());
#else
        // Use the SkStrikeServer's strike cache to generate the Slug.
        return skgpu::v1::MakeSlug(this->localToDevice(), glyphRunList, initialPaint, drawingPaint, this->strikeDeviceInfo(), fStrikeServerImpl);
#endif
    }
    ...
    int width, int height, const SkSurfaceProps& props, sk_sp<SkColorSpace> colorSpace, bool DFTSupport, bool DFTPerspSupport)
{
    ...
    auto control = sktext::gpu::SDFTControl { 
        ...
        DFTPerspSupport, 
        /*ctxOptions.fMinDistanceFieldFontSize*/18,
        /*ctxOptions.fGlyphsAsPathsFontSize*/ 324
    };

    sk_sp<SkBaseDevice> trackingDevice(new GlyphTrackingDevice(SkISize::Make(width, height), props, this->impl(), std::move(colorSpace), control));
#else
    sk_sp<SkBaseDevice> trackingDevice(new SkNoPixelsDevice(SkIRect::MakeWH(width, height), props, std::move(colorSpace)));
#endif
...
bool SkStrikeClientImpl::ReadGlyph(SkTLazy<SkGlyph>& glyph, Deserializer* deserializer)
{
    ...
    if (!deserializer->read<int16_t>(&glyph->fLeft))
        return false;
    uint8_t maskFormat;
    if (!deserializer->read<uint8_t>(&maskFormat))
        return false;
    if (!SkMask::IsValidFormat(maskFormat))
        return false;
...
bool SkStrikeClientImpl::readStrikeData(const volatile void* memory, size_t memorySize)
{
    ...
    for (size_t i = 0; i < strikeCount; ++i) {
        ...
        SkAutoDescriptor ad;
        if (!deserializer.readDescriptor(&ad))
            READ_FAILURE
#if defined(SK_TRACE_GLYPH_RUN_PROCESS)
        ...
        // side descriptor.
        if (!this->translateTypefaceID(&ad))
            READ_FAILURE
        ...
        if (strike == nullptr) {
            ...
            strike = fStrikeCache->createStrike(
                strikeSpec, &fontMetrics, std::make_unique<DiscardableStrikePinner>(spec.fDiscardableHandleId, fDiscardableHandleManager));
        }
        ...
        for (size_t j = 0; j < glyphImagesCount; j++) {
            ...
            if (!glyph->isEmpty() && SkGlyphDigest::FitsInAtlas(*glyph)) {
                const volatile void* image = deserializer.read(glyph->imageSize(), glyph->formatAlignment());
                if (!image)
                    READ_FAILURE
        ...
        for (size_t j = 0; j < glyphPathsCount; j++) {
            ...
            if (pathSize > 0) {
                ...
                if (!path.readFromMemory(const_cast<const void*>(pathData), pathSize))
                    READ_FAILURE
                pathPtr = &path;
                if (!deserializer.read<bool>(&hairline))
                    READ_FAILURE
            }
        ...
        if (!deserializer.read<uint64_t>(&glyphDrawablesCount))
            READ_FAILURE
        for (size_t j = 0; j < glyphDrawablesCount; j++) {
            ...
            if (!deserializer.read<uint64_t>(&drawableSize))
                READ_FAILURE
            ...
            if (drawableSize > 0) {
                ...
                if (!drawableData)
                    READ_FAILURE
                sk_sp<SkPicture> picture(SkPicture::MakeFromData(const_cast<const void*>(drawableData), drawableSize));
                if (!picture)
                    READ_FAILURE
...
sk_sp<SkTypeface> SkStrikeClientImpl::addTypeface(const WireTypeface& wire)
{
    ...
    auto newTypeface = sk_make_sp<SkTypefaceProxy>(
        wire.fTypefaceID, wire.fGlyphCount, wire.fStyle, wire.fIsFixed, wire.fGlyphMaskNeedsCurrentColor, fDiscardableHandleManager, fIsLogging);
