 ...
 */
std::unique_ptr<skgpu::v1::SurfaceDrawContext> GaussianBlur(GrRecordingContext*, GrSurfaceProxyView srcView, GrColorType srcColorType, SkAlphaType srcAlphaType,
    sk_sp<SkColorSpace> srcColorSpace, SkIRect dstBounds, SkIRect srcBounds, float sigmaX, float sigmaY, SkTileMode mode,
...
inline int KernelWidth(int radius)
{
    return 2 * radius + 1;
}

inline int LinearKernelWidth(int radius)
{
    return radius + 1;
}
