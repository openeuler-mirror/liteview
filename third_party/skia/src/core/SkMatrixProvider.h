...
public:
    SkMatrixProvider(const SkMatrix& localToDevice)
        : SkMatrixProvider(localToDevice, true)
    {
    }
    ...
    bool localToDeviceHitsPixelCenters() const
    {
        return fHitsPixelCenters;
    }
...
public:
    SkPostTranslateMatrixProvider(const SkMatrixProvider& parent, SkScalar dx, SkScalar dy)
        : SkMatrixProvider(SkM44::Translate(dx, dy) * parent.localToDevice44(), parent.localToDeviceHitsPixelCenters())
    {
    }
};
...
public:
    SkPreConcatMatrixProvider(const SkMatrixProvider& parent, const SkMatrix& preMatrix)
        : SkMatrixProvider(parent.localToDevice44() * SkM44(preMatrix), parent.localToDeviceHitsPixelCenters())
    {
    }
};
