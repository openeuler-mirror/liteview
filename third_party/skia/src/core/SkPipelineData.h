...
public:
    ...
    SkUniformDataBlock(SkSpan<const char> data)
        : fData(data)
    {
    }
...
public:
    using SampledTexture = std::pair<sk_sp<skgpu::graphite::TextureProxy>, skgpu::graphite::SamplerDesc>;
    ...
    {
        return fTextureData[index];
    }

    bool operator==(const SkTextureDataBlock&) const;
    bool operator!=(const SkTextureDataBlock& other) const
    {
        return !(*this == other);
    }
    uint32_t hash() const;

    void add(const SkSamplingOptions& sampling, const SkTileMode tileModes[2], sk_sp<skgpu::graphite::TextureProxy> proxy)
    {
...
public:
    ...
    {
        return !fTextureDataBlock.empty();
    }
#endif // SK_GRAPHITE_ENABLED
...
#ifdef SK_GRAPHITE_ENABLED
    const SkTextureDataBlock& textureDataBlock()
    {
        return fTextureDataBlock;
    }

    void write(const SkM44& mat)
    {
        fUniformManager.write(mat);
    }
    void write(const SkColor4f* colors, int numColors)
    {
        fUniformManager.write(colors, numColors);
    }
    void write(const SkPMColor4f& premulColor)
    {
        fUniformManager.write(&premulColor, 1);
    }
    void write(const SkRect& rect)
    {
        fUniformManager.write(rect);
    }
    void write(SkPoint point)
    {
        fUniformManager.write(point);
    }
    void write(const float* floats, int count)
    {
        fUniformManager.write(floats, count);
    }
    void write(float f)
    {
        fUniformManager.write(&f, 1);
    }
    void write(int i)
    {
        fUniformManager.write(i);
    }
    void write(skvx::float2 v)
    {
        fUniformManager.write(v);
    }
    void write(skvx::float4 v)
    {
        fUniformManager.write(v);
    }
    void write(SkSLType t, unsigned int cnt, const void* v)
    {
        fUniformManager.write(t, cnt, v);
    }
    ...
    {
        return fUniformManager.finishUniformDataBlock();
    }
...
private:
    ...
    {
        fUniformManager.doneWithExpectedUniforms();
    }
#endif // SK_DEBUG
...
public:
    UniformExpectationsValidator(SkPipelineDataGatherer* gatherer, SkSpan<const SkUniform> expectedUniforms)
        : fGatherer(gatherer)
    {
