...
struct SkMeshSpecificationPriv {
    ...
    static bool HasLocalCoords(const SkMeshSpecification& spec)
    {
...
struct SkMeshPriv {
...
#if SK_SUPPORT_GPU
        virtual sk_sp<const GrGpuBuffer> asGpuBuffer() const
        {
            return nullptr;
        }
#endif
    ...
    public:
        ...
        sk_sp<const GrGpuBuffer> asGpuBuffer() const override
        {
            return fBuffer;
        }
...
private:
#if SK_SUPPORT_GPU
    static bool UpdateGpuBuffer(GrDirectContext*, sk_sp<GrGpuBuffer>, const void*, size_t offset, size_t size);
#endif
};
...
template <typename Base, GrGpuBufferType Type> sk_sp<Base> SkMeshPriv::GpuBuffer<Base, Type>::Make(GrDirectContext* dc, const void* data, size_t size)
{
    SkASSERT(dc);
...
bool SkMeshPriv::GpuBuffer<Base, Type>::onUpdate(GrDirectContext* dc, const void* data, size_t offset, size_t size)
{
