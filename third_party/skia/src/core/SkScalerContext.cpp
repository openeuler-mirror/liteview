...
SkMaskGamma::PreBlend SkScalerContext::GetMaskPreBlend(const SkScalerContextRec& rec)
{
    ...
    const SkMaskGamma& maskGamma = cached_mask_gamma(rec.getContrast(), rec.getPaintGamma(), rec.getDeviceGamma());
...
SkGlyph SkScalerContext::internalMakeGlyph(SkPackedGlyphID packedID, SkMask::Format format, SkArenaAlloc* alloc)
{
    ...
    SkDEBUGCODE(glyph.fAdvancesBoundsFormatAndInitialPathDone = true;) if (fGenerateImageFromPath)
    {
...
static void generateMask(const SkMask& mask, const SkPath& path, const SkMaskGamma::PreBlend& maskPreBlend, const bool doBGR, const bool doVert,
    const bool a8FromLCD, const bool hairline)
{
...
#else
        SkScalar fakeBoldScale = SkScalarInterpFunc(font.getSize(), kStdFakeBoldInterpKeys, kStdFakeBoldInterpValues, kStdFakeBoldInterpLength);
