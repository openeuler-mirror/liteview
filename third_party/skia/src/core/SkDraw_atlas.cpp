...
public:
    ...
    skvm::Color onProgram(skvm::Builder* builder, skvm::Coord device, skvm::Coord local, skvm::Color paint, const SkMatrixProvider& provider,
        const SkMatrix* localM, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override
    {
...
void SkDraw::drawAtlas(const SkRSXform xform[], const SkRect textures[], const SkColor colors[], int count, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ...
    auto rpblit = [&]() {
        ...
        SkStageRec rec = { &pipeline, &alloc, fDst.colorType(), fDst.colorSpace(), p, nullptr, *fMatrixProvider, props };
    ...
    if (gUseSkVMBlitter || !rpblit()) {
        ...
        if (colors) {
            ...
            shader = SkShaders::Blend(std::move(blender), sk_ref_sp(colorShader), sk_ref_sp(updateShader));
        } else {
