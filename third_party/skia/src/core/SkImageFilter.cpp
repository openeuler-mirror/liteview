...
SkIRect SkImageFilter::filterBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection direction, const SkIRect* inputRect) const
{
    ...
    if (kReverse_MapDirection == direction) {
        ...
        if (as_IFB(this)->cropRectIsSet()) {
            skif::LayerSpace<SkIRect> outputCrop = mapping.paramToLayer(skif::ParameterSpace<SkRect>(as_IFB(this)->getCropRect().rect())).roundOut();
        ...
        SkIRect dst;
        as_IFB(this)->getCropRect().applyTo(SkIRect(output), ctm, as_IFB(this)->onAffectsTransparentBlack(), &dst);
        return dst;
    }
}
...
SkImageFilter_Base::SkImageFilter_Base(sk_sp<SkImageFilter> const* inputs, int inputCount, const SkRect* cropRect)
    : fUsesSrcInput(false)
    , fCropRect(cropRect)
...
skif::FilterResult SkImageFilter_Base::filterImage(const skif::Context& context) const
{
    ...
    SkASSERT(context.source().layerBounds().left() == 0 && context.source().layerBounds().top() == 0
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::getInputBounds(
    const skif::Mapping& mapping, const skif::DeviceSpace<SkIRect>& desiredOutput, const skif::ParameterSpace<SkRect>* knownContentBounds) const
{
    ...
    if (this->cropRectIsSet()) {
        skif::LayerSpace<SkIRect> outputCrop = mapping.paramToLayer(skif::ParameterSpace<SkRect>(fCropRect.rect())).roundOut();
    ...
    skif::LayerSpace<SkIRect> contentBounds = knownContentBounds ? mapping.paramToLayer(*knownContentBounds).roundOut() : desiredBounds;

    // Process the layer-space desired output with the filter DAG to determine required input
    skif::LayerSpace<SkIRect> requiredInput = this->onGetInputLayerBounds(mapping, desiredBounds, contentBounds);
...
skif::DeviceSpace<SkIRect> SkImageFilter_Base::getOutputBounds(const skif::Mapping& mapping, const skif::ParameterSpace<SkRect>& contentBounds) const
{
    ...
    skif::LayerSpace<SkIRect> filterOutput = this->onGetOutputLayerBounds(mapping, layerContent.roundOut());
    // FIXME (michaelludwig) - To be removed once cropping is isolated, but remain consistent with
    // old filterBounds(kForward) behavior.
    SkIRect dst;
    as_IFB(this)->getCropRect().applyTo(SkIRect(filterOutput), mapping.layerMatrix(), as_IFB(this)->onAffectsTransparentBlack(), &dst);
...
void SkImageFilter_Base::CropRect::applyTo(const SkIRect& imageBounds, const SkMatrix& ctm, bool embiggen, SkIRect* cropped) const
{
    *cropped = imageBounds;
    if (fFlags) {
        SkRect devCropR;
...
sk_sp<SkSpecialImage> SkImageFilter_Base::applyCropRectAndPad(const Context& ctx, SkSpecialImage* src, SkIPoint* srcOffset, SkIRect* bounds) const
{
    ...
    if (srcBounds.contains(*bounds)) {
        ...
        sk_sp<SkSpecialImage> img(
            pad_image(src, ctx, bounds->width(), bounds->height(), Sk32_sat_sub(srcOffset->x(), bounds->x()), Sk32_sat_sub(srcOffset->y(), bounds->y())));
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::visitInputLayerBounds(
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, const skif::LayerSpace<SkIRect>& contentBounds) const
{
    if (this->countInputs() < 1) {
    ...
    for (int i = 0; i < this->countInputs(); ++i) {
        ...
        skif::LayerSpace<SkIRect> requiredInput = filter ? as_IFB(filter)->onGetInputLayerBounds(mapping, desiredOutput, contentBounds) : desiredOutput;
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::visitOutputLayerBounds(const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& contentBounds) const
{
    if (this->countInputs() < 1) {
    ...
    for (int i = 0; i < this->countInputs(); ++i) {
        ...
        skif::LayerSpace<SkIRect> output = filter ? as_IFB(filter)->onGetOutputLayerBounds(mapping, contentBounds) : contentBounds;
        // Accumulate with all other filters
        if (i == 0) {
            netOutput = output;
        } else {
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::onGetInputLayerBounds(
    const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& desiredOutput, const skif::LayerSpace<SkIRect>& contentBounds, VisitChildren recurse) const
{
...
skif::LayerSpace<SkIRect> SkImageFilter_Base::onGetOutputLayerBounds(const skif::Mapping& mapping, const skif::LayerSpace<SkIRect>& contentBounds) const
{
...
SkImageFilter_Base::Context SkImageFilter_Base::mapContext(const Context& ctx) const
{
    ...
    skif::LayerSpace<SkIRect> childOutput = this->onGetInputLayerBounds(ctx.mapping(), ctx.desiredOutput(), ctx.desiredOutput(), VisitChildren::kNo);
    return ctx.withNewDesiredOutput(childOutput);
}

#if SK_SUPPORT_GPU
sk_sp<SkSpecialImage> SkImageFilter_Base::DrawWithFP(GrRecordingContext* rContext, std::unique_ptr<GrFragmentProcessor> fp, const SkIRect& bounds,
    SkColorType colorType, const SkColorSpace* colorSpace, const SkSurfaceProps& surfaceProps, GrSurfaceOrigin surfaceOrigin, GrProtected isProtected)
{
    GrImageInfo info(SkColorTypeToGrColorType(colorType), kPremul_SkAlphaType, sk_ref_sp(colorSpace), bounds.size());

    auto sfc = rContext->priv().makeSFC(info, "ImageFilterBase_DrawWithFP", SkBackingFit::kApprox, 1, GrMipmapped::kNo, isProtected, surfaceOrigin);
    if (!sfc) {
        return nullptr;
    }
    ...
    return SkSpecialImage::MakeDeferredFromGpu(rContext, dstIRect, kNeedNewImageUniqueID_SpecialImage, sfc->readSurfaceView(), sfc->colorInfo(), surfaceProps);
}

sk_sp<SkSpecialImage> SkImageFilter_Base::ImageToColorSpace(
    SkSpecialImage* src, SkColorType colorType, SkColorSpace* colorSpace, const SkSurfaceProps& surfaceProps)
{
    ...
    sk_sp<SkSpecialSurface> surf(src->makeSurface(colorType, colorSpace, SkISize::Make(src->width(), src->height()), kPremul_SkAlphaType, surfaceProps));
    if (!surf) {
        return sk_ref_sp(src);
    }
...
SkIRect SkImageFilter_Base::DetermineRepeatedSrcBound(
    const SkIRect& srcBounds, const SkIVector& filterOffset, const SkISize& filterSize, const SkIRect& originalSrcBounds)
{
