    ...
    for (auto& glyphRun : glyphRunList) {
        ...
        if (!rejected->source().empty()) {
            ...
            for (auto [variant, srcPos] : SkMakeZip(variants, sourcePositions)) {
                ...
                SkPoint realPos = srcPos + SkPoint::Make(mask.fBounds.left(), mask.fBounds.top()) * (1.0f / maxScale);
                ...
                bitmapDevice->drawBitmap(bm, translate, nullptr, SkSamplingOptions { SkFilterMode::kLinear }, paint);
            }
            rejected->flipRejectsToSource();
        }
