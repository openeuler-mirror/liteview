...
static void bw_pt_rect_16_hair_proc(const PtProcRec& rec, const SkPoint devPts[], int count, SkBlitter* blitter)
{
...
static void bw_pt_rect_32_hair_proc(const PtProcRec& rec, const SkPoint devPts[], int count, SkBlitter* blitter)
{
    SkASSERT(rec.fRC->isRect());
    const SkIRect& r = rec.fRC->getBounds();
    uint32_t value;
                ...
                if (as_PEB(paint.getPathEffect())->asPoints(&pointData, path, stroke, ctm, &cullRect)) {
                    ...
                    if (pointData.fSize.fX == pointData.fSize.fY) {
                        ...
                        if (device) {
                            ...
                            this->drawPoints(SkCanvas::kPoints_PointMode, pointData.fNumPoints, pointData.fPoints, newP, device);
                        }
                        break;
                    } else {
...
SkDraw::RectType SkDraw::ComputeRectType(const SkRect& rect, const SkPaint& paint, const SkMatrix& matrix, SkPoint* strokeSize)
{
    RectType rtype;
...
void SkDraw::drawRect(const SkRect& prePaintRect, const SkPaint& paint, const SkMatrix* paintMatrix, const SkRect* postPaintRect) const
{
    ...
    if (paint.getStyle() != SkPaint::kFill_Style) {
        ...
        if (paint.getStrokeWidth() == 0) {
            ...
            const SkPoint& ssize = (kStroke_RectType == rtype) ? strokeSize : compute_stroke_size(paint, ctm);
...
void SkDraw::drawDevPath(const SkPath& devPath, const SkPaint& paint, bool drawCoverage, SkBlitter* customBlitter, bool doFill) const
{
    ...
    if (paint.getMaskFilter()) {
        ...
        if (as_MFB(paint.getMaskFilter())->filterPath(devPath, fMatrixProvider->localToDevice(), *fRC, blitter, style)) {
...
void SkDraw::drawBitmapAsMask(const SkBitmap& bitmap, const SkSamplingOptions& sampling, const SkPaint& paint) const
{
    ...
    if (SkTreatAsSprite(ctm, bitmap.dimensions(), sampling, paint)) {
        ...
        {
            SkBitmap device;
            device.installPixels(SkImageInfo::MakeA8(mask.fBounds.width(), mask.fBounds.height()), mask.fImage, mask.fRowBytes);
    ...
    const SkBitmap& bitmap, const SkMatrix& prematrix, const SkRect* dstBounds, const SkSamplingOptions& sampling, const SkPaint& origPaint) const
{
    ...
    if (!SkColorTypeIsAlphaOnly(bitmap.colorType()) && SkTreatAsSprite(matrix, bitmap.dimensions(), sampling, *paint)) {
        //
...
bool SkDraw::DrawToMask(const SkPath& devPath, const SkIRect& clipBounds, const SkMaskFilter* filter, const SkMatrix* filterMatrix, SkMask* mask,
