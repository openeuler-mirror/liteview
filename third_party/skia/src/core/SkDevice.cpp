...
void SkBaseDevice::drawPatch(const SkPoint cubics[12], const SkColor colors[4], const SkPoint texCoords[4], sk_sp<SkBlender> blender, const SkPaint& paint)
{
...
void SkBaseDevice::drawImageLattice(const SkImage* image, const SkCanvas::Lattice& lattice, const SkRect& dst, SkFilterMode filter, const SkPaint& paint)
{
...
void SkBaseDevice::drawAtlas(const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int quadCount, sk_sp<SkBlender> blender, const SkPaint& paint)
{
...
void SkBaseDevice::drawEdgeAAImageSet(const SkCanvas::ImageSetEntry images[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[],
...
void SkBaseDevice::drawSpecial(SkSpecialImage*, const SkMatrix&, const SkSamplingOptions&, const SkPaint&)
{
}
...
sk_sp<SkSpecialImage> SkBaseDevice::makeSpecial(const SkImage*)
{
    return nullptr;
}
sk_sp<SkSpecialImage> SkBaseDevice::snapSpecial(const SkIRect&, bool forceCopy)
{
    return nullptr;
}
...
void SkBaseDevice::drawDevice(SkBaseDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
...
void SkBaseDevice::drawFilteredImage(
    const skif::Mapping& mapping, SkSpecialImage* src, const SkImageFilter* filter, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    ...
    skif::Context ctx(mapping, targetOutput, cache.get(), colorType, this->imageInfo().colorSpace(), skif::FilterResult(sk_ref_sp(src)));
...
void SkBaseDevice::drawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    ...
    SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    for (const sktext::GlyphRun& run : glyphRunList) {
        if (run.scaledRotations().empty()) {
            auto subList = glyphRunList.builder()->makeGlyphRunList(run, drawingPaint, { 0, 0 });
            ...
            for (auto [i, glyphID, pos] : SkMakeEnumerate(run.source())) {
                ...
                sktext::GlyphRunList subList = glyphRunList.builder()->makeGlyphRunList(glyphRun, drawingPaint, { 0, 0 });
...
sk_sp<sktext::gpu::Slug> SkBaseDevice::convertGlyphRunListToSlug(
    const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    return nullptr;
}
