...
public:
    ...
    operator SkPackedGlyphID() const
    {
        return this->packedID();
    }
    operator const SkGlyph*() const
    {
        return this->glyph();
    }
...
public:
    ...
    void startSourceWithMatrixAdjustment(const SkZip<const SkGlyphID, const SkPoint>& source, const SkMatrix& creationMatrix);
    ...
    SkZip<SkGlyphVariant, SkPoint, SkMask::Format> acceptedWithMaskFormat()
    {
        SkASSERT(fPhase == kProcess);
        SkDEBUGCODE(fPhase = kDraw);
        return SkZip<SkGlyphVariant, SkPoint, SkMask::Format> { fAcceptedSize, fMultiBuffer.get(), fPositions, fFormats };
    }
