...
struct Matrix43 {
    ...
    {
        return skvx::float4::Load(&fMat[0]) * x + skvx::float4::Load(&fMat[4]) * y + skvx::float4::Load(&fMat[8]);
    }
...
static bool SK_WARN_UNUSED_RESULT texture_to_matrix(const VertState& state, const SkPoint verts[], const SkPoint texs[], SkMatrix* matrix)
{
    SkPoint src[3], dst[3];
...
skvm::Color SkTriColorShader::onProgram(skvm::Builder* b, skvm::Coord device, skvm::Coord local, skvm::Color, const SkMatrixProvider& matrices,
    const SkMatrix* localM, const SkColorInfo&, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
    ...
    if (fUsePersp) {
        ...
        auto dot = [&, x, y](int row) {
            return b->mad(x, b->arrayF(fCoordMatrix, row), b->mad(y, b->arrayF(fCoordMatrix, row + 3), b->arrayF(fCoordMatrix, row + 6)));
        };
    ...
    auto colorDot
        = [&, x, y](int row) { return b->mad(x, b->arrayF(fColorMatrix, row), b->mad(y, b->arrayF(fColorMatrix, row + 4), b->arrayF(fColorMatrix, row + 8))); };
    ...
    const SkPoint3* dev3, SkArenaAlloc* outerAlloc, bool skipColorXform) const
{
    ...
    auto rpblit = [&]() {
        ...
        if (!texCoords) { // only tricolor shader
            auto blitter = SkCreateRasterPipelineBlitter(fDst, shaderPaint, *fMatrixProvider, outerAlloc, this->fRC->clipShader(), props);
            if (!blitter) {
                return false;
            }
        ...
        SkStageRec rec = { &pipeline, outerAlloc, fDst.colorType(), fDst.colorSpace(), shaderPaint, nullptr, *fMatrixProvider, props };
        if (auto updater = as_SB(blendShader)->appendUpdatableStages(rec)) {
            ...
            auto blitter = SkCreateRasterPipelineBlitter(fDst, shaderPaint, pipeline, isOpaque, outerAlloc, fRC->clipShader());
            if (!blitter) {
                return false;
            }
            while (vertProc(&state)) {
                ...
                SkMatrix localM;
                if ((texCoords == positions) || (texture_to_matrix(state, positions, texCoords, &localM) && updater->update(SkMatrix::Concat(ctm, localM)))) {
                    fill_triangle(state, blitter, *fRC, dev2, dev3);
                }
            }
        } else {
            ...
            while (vertProc(&state)) {
                ...
                auto blitter = SkCreateRasterPipelineBlitter(fDst, shaderPaint, *matrixProvider, &innerAlloc, this->fRC->clipShader(), props);
                if (!blitter) {
                    return false;
                }
                fill_triangle(state, blitter, *fRC, dev2, dev3);
    ...
    if (gUseSkVMBlitter || !rpblit()) {
        ...
        auto blitter = SkVMBlitter::Make(fDst, shaderPaint, *fMatrixProvider, outerAlloc, this->fRC->clipShader());
        if (!blitter) {
            return;
        }
        while (vertProc(&state)) {
            SkMatrix localM;
            if (texCoordShader && !(texture_to_matrix(state, positions, texCoords, &localM) && texCoordShader->update(SkMatrix::Concat(ctm, localM)))) {
                continue;
            }
