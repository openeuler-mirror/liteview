...
GrFPResult SkColorFilterBase::asFragmentProcessor(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkSurfaceProps& props) const
{
...
skvm::Color SkColorFilterBase::program(skvm::Builder* p, skvm::Color c, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
...
SkPMColor4f SkColorFilterBase::onFilterColor4f(const SkPMColor4f& color, SkColorSpace* dstCS) const
{
    ...
    SkStageRec rec = { &pipeline, &alloc, kRGBA_F32_SkColorType, dstCS, blankPaint, nullptr, matrixProvider, props };
    ...
    if (skvm::Color filtered = as_CFB(this)->program(&b, b.uniformColor(uniColor, &uni), dstInfo, &uni, &alloc)) {

        b.store({ skvm::PixelFormat::FLOAT, 32, 32, 32, 32, 0, 32, 64, 96 }, b.varying<SkColor4f>(), filtered);
...
#ifdef SK_ENABLE_SKSL
void SkColorFilterBase::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
...
public:
    ...
    skvm::Color onProgram(skvm::Builder* p, skvm::Color c, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override
    {
    ...
    GrFPResult asFragmentProcessor(
        std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkSurfaceProps& props) const override
    {
        ...
        auto [innerSuccess, innerFP] = fInner->asFragmentProcessor(std::move(inputFP), context, dstColorInfo, props);
        if (!innerSuccess) {
            return GrFPFailure(std::move(inputClone));
        }

        auto [outerSuccess, outerFP] = fOuter->asFragmentProcessor(std::move(innerFP), context, dstColorInfo, props);
        if (!outerSuccess) {
            return GrFPFailure(std::move(inputClone));
        }
...
public:
        ...
        std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkSurfaceProps& props) const override
    {
        ...
        case Direction::kLinearToSRGB:
            return GrFPSuccess(GrColorSpaceXformEffect::Make(std::move(inputFP), sk_srgb_linear_singleton(), alphaType, sk_srgb_singleton(), alphaType));
        case Direction::kSRGBToLinear:
            return GrFPSuccess(GrColorSpaceXformEffect::Make(std::move(inputFP), sk_srgb_singleton(), alphaType, sk_srgb_linear_singleton(), alphaType));
        }
        SkUNREACHABLE;
    }
#endif
...
public:
        ...
        std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkSurfaceProps& props) const override
    {
        ...
        auto [ok, fp] = as_CFB(fChild)->asFragmentProcessor(GrColorSpaceXformEffect::Make(std::move(inputFP), dst, working), context, working, props);
    ...
    skvm::Color onProgram(skvm::Builder* p, skvm::Color c, const SkColorInfo& rawDst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override
    {
        ...
        SkColorInfo dst = { rawDst.colorType(), kPremul_SkAlphaType, dstCS }, working = { rawDst.colorType(), workingAT, workingCS };
...
sk_sp<SkColorFilter> SkColorFilters::Lerp(float weight, sk_sp<SkColorFilter> cf0, sk_sp<SkColorFilter> cf1)
{
    ...
    static const SkRuntimeEffect* effect = SkMakeCachedRuntimeEffect(SkRuntimeEffect::MakeForColorFilter,
    ...
    return effect->makeColorFilter(SkData::MakeWithCopy(&weight, sizeof(weight)), inputs, std::size(inputs));
#else
    // TODO(skia:12197)
    return nullptr;
#endif
