...
{
    skvm::F32 G = p->uniformF(uniforms->pushF(tf.g)), A = p->uniformF(uniforms->pushF(tf.a)), B = p->uniformF(uniforms->pushF(tf.b)),
              C = p->uniformF(uniforms->pushF(tf.c)), D = p->uniformF(uniforms->pushF(tf.d)), E = p->uniformF(uniforms->pushF(tf.e)),
...
skvm::Color SkColorSpaceXformSteps::program(skvm::Builder* p, skvm::Uniforms* uniforms, skvm::Color c) const
{
    ...
    if (flags.gamut_transform) {
        ...
        auto R = c.r * m(0) + c.g * m(3) + c.b * m(6), G = c.r * m(1) + c.g * m(4) + c.b * m(7), B = c.r * m(2) + c.g * m(5) + c.b * m(8);
        c = { R, G, B, c.a };
    }
    if (flags.encode) {
