...
{
    skvm::I32 dx = p->uniform32(uniforms->base, offsetof(BlitterUniforms, right)) - p->index(), dy = p->uniform32(uniforms->base, offsetof(BlitterUniforms, y));
    return {
        to_F32(dx) + 0.5f,
        to_F32(dy) + 0.5f,
    };
...
struct NoopColorFilter final : public SkColorFilterBase {
    skvm::Color onProgram(skvm::Builder*, skvm::Color c, const SkColorInfo&, skvm::Uniforms*, SkArenaAlloc*) const override
    {
        return c;
    }
    ...
    {
        return "NoopColorFilter";
    }
};

struct SpriteShader : public SkShaderBase {
    explicit SpriteShader(SkPixmap sprite)
        : fSprite(sprite)
    {
    }
    ...
    {
        return fSprite.isOpaque();
    }

    skvm::Color onProgram(skvm::Builder* p, skvm::Coord /*device*/, skvm::Coord /*local*/, skvm::Color /*paint*/, const SkMatrixProvider&,
        const SkMatrix* /*localM*/, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc*) const override
    {
...
struct DitherShader : public SkShaderBase {
    explicit DitherShader(sk_sp<SkShader> shader)
        ...
        const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override
    {
        ...
        skvm::Color c = as_SB(fShader)->program(p, device, local, paint, matrices, localM, dst, uniforms, alloc);
        if (!c) {
            return {};
        }
        ...
        skvm::I32 M = shl(Y & 1, 5) | shl(X & 1, 4) | shl(Y & 2, 2) | shl(X & 2, 1) | shr(Y & 4, 1) | shr(X & 4, 2);
...
{
    return this->shader == that.shader && this->clip == that.clip && this->blender == that.blender && this->colorSpace == that.colorSpace
        && this->colorType == that.colorType && this->alphaType == that.alphaType && this->coverage == that.coverage;
}
...
SkVMBlitter::Params SkVMBlitter::EffectiveParams(
    const SkPixmap& device, const SkPixmap* sprite, SkPaint paint, const SkMatrixProvider& matrices, sk_sp<SkShader> clip)
{
    ...
    if (!shader) {
        ...
        shader = sk_make_sp<SkColorFilterShader>(std::move(shader), paint.getAlphaf(), sk_make_sp<NoopColorFilter>());
    ...
    SkColorSpaceXformSteps { sk_srgb_singleton(), kUnpremul_SkAlphaType, device.colorSpace(), kUnpremul_SkAlphaType }.apply(paintColor.vec());
...
void SkVMBlitter::BuildProgram(skvm::Builder* p, const Params& params, skvm::Uniforms* uniforms, SkArenaAlloc* alloc)
{
    ...
    skvm::Color src = as_SB(params.shader)->program(p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr, params.dst, uniforms, alloc);
    SkASSERT(src);
    if (params.coverage == Coverage::Mask3D) {
        skvm::F32 M = from_unorm(8, p->load8(p->varying<uint8_t>())), A = from_unorm(8, p->load8(p->varying<uint8_t>()));
    ...
    if (params.clip) {
        skvm::Color clip = as_SB(params.clip)->program(p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr, params.dst, uniforms, alloc);
        SkAssertResult(clip);
    ...
    if (as_blendmode && SkBlendMode_ShouldPreScaleCoverage(as_blendmode.value(), params.coverage == Coverage::MaskLCD16)) {
...
SkVMBlitter::Key SkVMBlitter::CacheKey(const Params& params, skvm::Uniforms* uniforms, SkArenaAlloc* alloc, bool* ok)
{
    // Take care to match buildProgram()'s reuse of the paint color uniforms.
    skvm::Uniform r = uniforms->pushF(params.paint.fR), g = uniforms->pushF(params.paint.fG), b = uniforms->pushF(params.paint.fB),
...
SkVMBlitter::SkVMBlitter(const SkPixmap& device, const SkPaint& paint, const SkPixmap* sprite, SkIPoint spriteOffset, const SkMatrixProvider& matrices,
...
SkLRUCache<SkVMBlitter::Key, skvm::Program>* SkVMBlitter::TryAcquireProgramCache()
{
    ...
    //     return &cache;
    static SimpleTlsPointerXp<SkLRUCache<Key, skvm::Program>> cache;
    if (!cache.Get())
        cache.Set(new SkLRUCache<Key, skvm::Program>(64));
    return cache.Get();
#else
...
{
    return SkStringPrintf("Shader-%" PRIx64 "_Clip-%" PRIx64 "_Blender-%" PRIx64 "_CS-%" PRIx64 "_CT-%d_AT-%d_Cov-%d", key.shader, key.clip, key.blender,
        key.colorSpace, key.colorType, key.alphaType, key.coverage);
}
...
void SkVMBlitter::blitMask(const SkMask& mask, const SkIRect& clip)
{
    ...
    SkASSERT(program);
    if (program) {
        SK_BLITTER_TRACE_STEP(blitMask, true,
...
SkVMBlitter* SkVMBlitter::Make(const SkPixmap& device, const SkPaint& paint, const SkMatrixProvider& matrices, SkArenaAlloc* alloc, sk_sp<SkShader> clip)
{
    bool ok = true;
    SkVMBlitter* blitter = alloc->make<SkVMBlitter>(device, paint, /*sprite=*/nullptr, SkIPoint { 0, 0 }, matrices, std::move(clip), &ok);
    return ok ? blitter : nullptr;
}

SkVMBlitter* SkVMBlitter::Make(
    const SkPixmap& device, const SkPaint& paint, const SkPixmap& sprite, int left, int top, SkArenaAlloc* alloc, sk_sp<SkShader> clip)
{
    ...
    bool ok = true;
    auto blitter = alloc->make<SkVMBlitter>(device, paint, &sprite, SkIPoint { left, top }, SkMatrixProvider { SkMatrix {} }, std::move(clip), &ok);
    return ok ? blitter : nullptr;
}
