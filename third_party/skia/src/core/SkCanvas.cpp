...
#if 1 // (SK_SUPPORT_GPU || defined(SK_GRAPHITE_ENABLED))
...
{
    static_assert((int)SkPaintPriv::kNone_ShaderOverrideOpacity == (int)kNone_ShaderOverrideOpacity, "need_matching_enums0");
    static_assert((int)SkPaintPriv::kOpaque_ShaderOverrideOpacity == (int)kOpaque_ShaderOverrideOpacity, "need_matching_enums1");
    static_assert((int)SkPaintPriv::kNotOpaque_ShaderOverrideOpacity == (int)kNotOpaque_ShaderOverrideOpacity, "need_matching_enums2");
...
bool SkCanvas::predrawNotify(const SkRect* rect, const SkPaint* paint, ShaderOverrideOpacity overrideOpacity)
{
    if (fSurfaceBase) {
...
SkCanvas::Layer::Layer(sk_sp<SkBaseDevice> device, sk_sp<SkImageFilter> imageFilter, const SkPaint& paint)
...
void SkCanvas::MCRec::newLayer(sk_sp<SkBaseDevice> layerDevice, sk_sp<SkImageFilter> filter, const SkPaint& restorePaint)
{
    SkASSERT(!fBackImage);
...
public:
        ...
        , fTempLayerForImageFilter(false)
    {
            ...
            if (fPaint.getImageFilter() && !image_to_color_filter(&fPaint))
        {
            ...
            canvas->fSaveCount += 1;
            (void)canvas->internalSaveLayer(SkCanvas::SaveLayerRec(rawBounds, &restorePaint), SkCanvas::kFullLayer_SaveLayerStrategy);
...
std::optional<AutoLayerForImageFilter> SkCanvas::aboutToDraw(
    SkCanvas* canvas, const SkPaint& paint, const SkRect* rawBounds, CheckForOverwrite checkOverwrite, ShaderOverrideOpacity overrideOpacity)
{
...
static skif::ParameterSpace<SkPoint> compute_decomposition_center(
    const SkMatrix& dstToLocal, const skif::ParameterSpace<SkRect>* contentBounds, const skif::DeviceSpace<SkIRect>& targetOutput)
{
...
static std::pair<skif::Mapping, skif::LayerSpace<SkIRect>> get_layer_mapping_and_bounds(const SkImageFilter* filter, const SkMatrix& localToDst,
    const skif::DeviceSpace<SkIRect>& targetOutput, const skif::ParameterSpace<SkRect>* contentBounds = nullptr, bool mustCoverDst = true,
    SkScalar scaleFactor = 1.0f)
{
    auto failedMapping = []() { return std::make_pair<skif::Mapping, skif::LayerSpace<SkIRect>>({}, skif::LayerSpace<SkIRect>::Empty()); };
...
static SkImageInfo make_layer_info(const SkImageInfo& prev, int w, int h, bool f16)
{
    ...
    if (!f16 && prev.bytesPerPixel() <= 4 && prev.colorType() != kRGBA_8888_SkColorType && prev.colorType() != kBGRA_8888_SkColorType) {
...
void SkCanvas::internalDrawDeviceWithFilter(
    SkBaseDevice* src, SkBaseDevice* dst, const SkImageFilter* filter, const SkPaint& paint, DeviceCompatibleWithFilter compat, SkScalar scaleFactor)
{
    ...
    if (compat == DeviceCompatibleWithFilter::kYes) {
        ...
        std::tie(mapping, requiredInput) = get_layer_mapping_and_bounds(
            filter, dst->localToDevice(), skif::DeviceSpace<SkIRect>(dst->devClipBounds()), nullptr, true, SkTPin(scaleFactor, 0.f, 1.f));
        ...
        if (draw_layer_as_sprite(srcToIntermediate, srcDims)) {
            ...
            skif::LayerSpace<SkIPoint> srcOrigin({ (int)srcToIntermediate.getTranslateX(), (int)srcToIntermediate.getTranslateY() });
    ...
    if (!needsIntermediateImage) {
        ...
        SkRect srcRect;
        if (!SkMatrixPriv::InverseMapRect(srcToIntermediate, &srcRect, SkRect::Make(SkIRect(requiredInput)))) {
            return;
        }

        SkIRect srcSubset = srcRect.roundOut();
        sk_sp<SkSpecialImage> srcImage;
        if (srcSubset.intersect(SkIRect::MakeSize(srcDims)) && (srcImage = src->snapSpecial(srcSubset))) {
            ...
            SkBaseDevice::CreateInfo info(make_layer_info(src->imageInfo(), requiredInput.width(), requiredInput.height(), false),
                SkPixelGeometry::kUnknown_SkPixelGeometry, SkBaseDevice::TileUsage::kNever_TileUsage, fAllocator.get());
            sk_sp<SkBaseDevice> intermediateDevice(src->onCreateDevice(info, &paint));
            if (!intermediateDevice) {
                return;
            }
            intermediateDevice->setOrigin(SkM44(srcToIntermediate), requiredInput.left(), requiredInput.top());
            ...
            SkPaint imageFill;
            imageFill.setShader(srcImage->asShader(SkTileMode::kClamp, SkSamplingOptions { SkFilterMode::kLinear }, SkMatrix::Translate(srcSubset.topLeft())));
...
static const SkImageFilter* optimize_layer_filter(const SkImageFilter* filter, SkPaint* paint, bool* coversDevice = nullptr)
{
    SkASSERT(paint);
    SkColorFilter* cf;
    if (filter && filter->isColorFilterNode(&cf)) {
        ...
        if (paint->getAlphaf() < 1.f) {
            ...
            inner = SkColorFilters::Compose(SkColorFilters::Blend(/*src*/ paint->getColor4f(), nullptr, SkBlendMode::kDstIn),
                /*dst*/ std::move(inner));
            paint->setAlphaf(1.f);
        }
...
void SkCanvas::internalSaveLayer(const SaveLayerRec& rec, SaveLayerStrategy strategy)
{
    ...
    const SkImageFilter* filter = optimize_layer_filter(rec.fPaint ? rec.fPaint->getImageFilter() : nullptr, &restorePaint, &optimizedCFAffectsTransparent);
    ...
    skif::LayerSpace<SkIRect> layerBounds;
    std::tie(newLayerMapping, layerBounds)
        = get_layer_mapping_and_bounds(filter, priorDevice->localToDevice(), skif::DeviceSpace<SkIRect>(priorDevice->devClipBounds()),
            skif::ParameterSpace<SkRect>::Optional(rec.fBounds), must_cover_prior_device(rec.fBackdrop, restorePaint) || optimizedCFAffectsTransparent);
    ...
    if (strategy == kFullLayer_SaveLayerStrategy) {
        ...
        SkImageInfo info = make_layer_info(priorDevice->imageInfo(), layerBounds.width(), layerBounds.height(), SkToBool(rec.fSaveLayerFlags & kF16ColorType));
        ...
        const auto createInfo = SkBaseDevice::CreateInfo(info, geo, SkBaseDevice::kNever_TileUsage, fAllocator.get());
    ...
    if (initBackdrop) {
        ...
        auto compat = (filter || backdropFilter || scaleBackdrop) ? DeviceCompatibleWithFilter::kUnknown : DeviceCompatibleWithFilter::kYes;
...
bool SkCanvas::canDrawBitmapAsSprite(SkScalar x, SkScalar y, int w, int h, const SkSamplingOptions& sampling, const SkPaint& paint)
{
...
void SkCanvas::onDrawImage2(const SkImage* image, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    ...
    if (realPaint.getImageFilter() && this->canDrawBitmapAsSprite(x, y, image->width(), image->height(), sampling, realPaint)
    ...
    if (layer) {
        this->topDevice()->drawImageRect(image, nullptr, bounds, sampling, layer->paint(), kFast_SrcRectConstraint);
    }
}
    ...
    const SkImage* image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    ...
    auto layer = this->aboutToDraw(
        this, realPaint, &dst, CheckForOverwrite::kYes, image->isOpaque() ? kOpaque_ShaderOverrideOpacity : kNotOpaque_ShaderOverrideOpacity);
    if (layer) {
...
void SkCanvas::drawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint)
{
    ...
    // form.
    if (!gSkBlobAsSlugTesting || this->topDevice()->asGaneshDevice() == nullptr || SkTextBlobPriv::HasRSXForm(*blob))
#endif
    {
...
void SkTestCanvas<SkSlugTestKey>::onDrawGlyphRunList(const sktext::GlyphRunList& glyphRunList, const SkPaint& paint)
{
    SkRect bounds = glyphRunList.sourceBoundsWithOrigin();
    if (this->internalQuickReject(bounds, paint)) {
        return;
