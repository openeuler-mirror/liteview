...
private:
    void stepAndSetupTileDraw()
    {
        ...
        fDraw.fMatrixProvider = fTileMatrixProvider.init(fDevice->asMatrixProvider(), SkIntToScalar(-fOrigin.x()), SkIntToScalar(-fOrigin.y()));
...
SkBitmapDevice* SkBitmapDevice::Create(const SkImageInfo& origInfo, const SkSurfaceProps& surfaceProps, SkRasterHandleAllocator* allocator)
{
    ...
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    if (src && !src->contains(bitmapBounds) && SkCanvas::kFast_SrcRectConstraint == constraint && sampling != SkSamplingOptions()) {
...
void SkBitmapDevice::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
...
void SkBitmapDevice::drawDevice(SkBaseDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
...
void SkBitmapDevice::drawSpecial(SkSpecialImage* src, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    SkASSERT(!paint.getImageFilter());
    SkASSERT(!paint.getMaskFilter());
...
{
    return SkSpecialImage::MakeFromImage(nullptr, SkIRect::MakeWH(image->width(), image->height()), image->makeNonTextureImage(), this->surfaceProps());
}
