...
#pragma pack(pop)
    ...
    public:
        ...
        const SkShaderSnippet* entry() const
        {
            return fEntry;
        }
    ...
    private:
        ...
        BlockReader(const SkShaderCodeDictionary*, SkSpan<const uint8_t> parentSpan, int offsetInParent);
    ...
    SkSpan<const uint8_t> asSpan() const
    {
        return fData;
    }
...
private:
    ...
    static void AddBlockToShaderInfo(const SkShaderCodeDictionary*, const SkPaintParamsKey::BlockReader&, SkShaderInfo*);
...
public:
    ...
    void beginBlock(SkBuiltInCodeSnippetID id)
    {
    ...
    void unlock()
    {
        ...
        fBlendInfo = {};
#endif
        SkDEBUGCODE(fLocked = false;) SkDEBUGCODE(this->checkReset();)
    }

    SkDEBUGCODE(bool isLocked() const { return fLocked; })

        private : void addToKey(uint32_t count, const void* data, SkPaintParamsKey::DataPayloadType payloadType);
    void makeInvalid();
