...
SkScalerCache::SkScalerCache(std::unique_ptr<SkScalerContext> scaler, const SkFontMetrics* fontMetrics)
...
std::tuple<const SkPath*, size_t> SkScalerCache::mergePath(SkGlyph* glyph, const SkPath* path, bool hairline)
{
...
std::tuple<SkDrawable*, size_t> SkScalerCache::mergeDrawable(SkGlyph* glyph, sk_sp<SkDrawable> drawable)
{
    SkAutoMutexExclusive lock { fMu };
    size_t delta = 0;
...
std::tuple<SkSpan<const SkGlyph*>, size_t> SkScalerCache::internalPrepare(SkSpan<const SkGlyphID> glyphIDs, PathDetail pathDetail, const SkGlyph** results)
{
...
std::tuple<SkGlyph*, size_t> SkScalerCache::mergeGlyphAndImage(SkPackedGlyphID toID, const SkGlyph& from)
{
    SkAutoMutexExclusive lock { fMu };
...
std::tuple<SkSpan<const SkGlyph*>, size_t> SkScalerCache::metrics(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[])
{
    SkAutoMutexExclusive lock { fMu };
...
std::tuple<SkSpan<const SkGlyph*>, size_t> SkScalerCache::preparePaths(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[])
{
    SkAutoMutexExclusive lock { fMu };
...
std::tuple<SkSpan<const SkGlyph*>, size_t> SkScalerCache::prepareImages(SkSpan<const SkPackedGlyphID> glyphIDs, const SkGlyph* results[])
{
    const SkGlyph** cursor = results;
    SkAutoMutexExclusive lock { fMu };
    size_t delta = 0;
...
std::tuple<SkSpan<const SkGlyph*>, size_t> SkScalerCache::prepareDrawables(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[])
{
    const SkGlyph** cursor = results;
    SkAutoMutexExclusive lock { fMu };
    size_t delta = 0;
...
std::tuple<SkScalar, size_t> SkScalerCache::findMaximumGlyphDimension(SkSpan<const SkGlyphID> glyphs)
{
...
template <typename Fn> size_t SkScalerCache::commonFilterLoop(SkDrawableGlyphBuffer* accepted, Fn&& fn)
{
    size_t total = 0;
...
size_t SkScalerCache::prepareForDrawingMasksCPU(SkDrawableGlyphBuffer* accepted)
{
    SkAutoMutexExclusive lock { fMu };
    size_t imageDelta = 0;
    size_t delta = this->commonFilterLoop(accepted, [&](size_t i, SkGlyphDigest digest, SkPoint pos) SK_REQUIRES(fMu) {
...
std::tuple<SkRect, size_t> SkScalerCache::prepareForMaskDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    SkAutoMutexExclusive lock { fMu };
...
std::tuple<SkRect, size_t> SkScalerCache::prepareForSDFTDrawing(SkScalar strikeToSourceScale, SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    SkAutoMutexExclusive lock { fMu };
            ...
            if (!digest.isEmpty()) {
                if (digest.canDrawAsSDFT()) {
                    ...
                    const SkGlyphRect glyphBounds
                        = digest.bounds().inset(SK_DistanceFieldInset, SK_DistanceFieldInset).scaleAndOffset(strikeToSourceScale, pos);
                    boundingRect = skglyph::rect_union(boundingRect, glyphBounds);
                    accepted->accept(packedID, glyphBounds.leftTop(), digest.maskFormat());
                } else {
...
size_t SkScalerCache::prepareForPathDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    SkAutoMutexExclusive lock { fMu };
    size_t increase = 0;
    for (auto [i, packedID, pos] : SkMakeEnumerate(accepted->input())) {
...
size_t SkScalerCache::prepareForDrawableDrawing(SkDrawableGlyphBuffer* accepted, SkSourceGlyphBuffer* rejected)
{
    SkAutoMutexExclusive lock { fMu };
    size_t increase = 0;
    for (auto [i, packedID, pos] : SkMakeEnumerate(accepted->input())) {
