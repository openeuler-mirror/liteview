...
void SkRecordPartialDraw(
    const SkRecord& record, SkCanvas* canvas, SkPicture const* const drawablePicts[], int drawableCount, int start, int stop, const SkM44& initialCTM)
{
    SkAutoCanvasRestore saveRestore(canvas, true /*save now, restore at exit*/);
...
DRAW(Save, save());
DRAW(SaveLayer, saveLayer(SkCanvasPriv::ScaledBackdropLayer(r.bounds, r.paint, r.backdrop.get(), r.backdropScale, r.saveLayerFlags)));
...
private:
    ...
    Bounds bounds(const Flush&) const
    {
        return fCullRect;
    }
