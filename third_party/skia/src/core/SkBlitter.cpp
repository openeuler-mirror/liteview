...
bool SkBlitter::UseLegacyBlitter(const SkPixmap& device, const SkPaint& paint, const SkMatrix& matrix)
{
    ...
    if (device.alphaType() == kUnpremul_SkAlphaType || !mode || mode.value() > SkBlendMode::kLastCoeffMode || (mf && mf->getFormat() == SkMask::k3D_Format)) {
        return false;
    }
...
SkBlitter* SkBlitter::Choose(const SkPixmap& device, const SkMatrixProvider& matrixProvider, const SkPaint& origPaint, SkArenaAlloc* alloc, bool drawCoverage,
    sk_sp<SkShader> clipShader, const SkSurfaceProps& props)
{
    ...
    auto create_SkRP_or_SkVMBlitter = [&]() -> SkBlitter* {
        ...
        if (auto blitter = SkCreateRasterPipelineBlitter(device, *paint, matrixProvider, alloc, clipShader, props)) {
            return blitter;
        }
        if (!gUseSkVMBlitter) {
            if (auto blitter = SkVMBlitter::Make(device, *paint, matrixProvider, alloc, clipShader)) {
    ...
    if (paint->getShader()) {
        shaderContext = as_SB(paint->getShader())->makeContext({ *paint, ctm, nullptr, device.colorType(), device.colorSpace(), props }, alloc);
