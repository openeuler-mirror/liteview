...
static bool is_nearly_integer_translation(const skif::LayerSpace<SkMatrix>& m, skif::LayerSpace<SkIPoint>* out = nullptr)
{
    ...
    SkMatrix expected = SkMatrix::MakeAll(1.f, 0.f, tx, 0.f, 1.f, ty, 0.f, 0.f, 1.f);
    for (int i = 0; i < 9; ++i) {
...
bool Mapping::decomposeCTM(const SkMatrix& ctm, const SkImageFilter* filter, const skif::ParameterSpace<SkPoint>& representativePt)
{
    ...
    MatrixCapability capability = filter ? as_IFB(filter)->getCTMCapability() : MatrixCapability::kComplex;
...
FilterResult FilterResult::applyCrop(const Context& ctx, const LayerSpace<SkIRect>& crop) const
{
    ...
    if (crop.contains(fLayerBounds)) {
        ...
        if (is_nearly_integer_translation(fTransform, &origin)) {
            ...
            auto subsetImage = fImage->makeSubset(SkIRect::MakeXYWH(originShift.x(), originShift.y(), tightBounds.width(), tightBounds.height()));
...
static bool compatible_sampling(const SkSamplingOptions& current, SkSamplingOptions* next)
{
    ...
    if (current.isAniso() && next->isAniso()) {
        ...
        *next = SkSamplingOptions::Aniso(std::max(current.maxAniso, next->maxAniso));
        return true;
    } else if (current.useCubic
        && (next->filter == SkFilterMode::kLinear || (next->useCubic && current.cubic.B == next->cubic.B && current.cubic.C == next->cubic.C))) {
...
FilterResult FilterResult::applyTransform(const Context& ctx, const LayerSpace<SkMatrix>& transform, const SkSamplingOptions& sampling) const
{
    ...
    SkSamplingOptions nextSampling = is_nearly_integer_translation(transform) ? kDefaultSampling : sampling;
...
void FilterResult::concatTransform(const LayerSpace<SkMatrix>& transform, const SkSamplingOptions& newSampling, const LayerSpace<SkIRect>& desiredOutput)
{
    if (!fImage) {
    ...
    fLayerBounds = fTransform.mapRect(LayerSpace<SkRect>(SkRect::MakeIWH(fImage->width(), fImage->height()))).roundOut();
...
FilterResult FilterResult::resolve(const LayerSpace<SkIRect>& dstBounds) const
{
    ...
    sk_sp<SkSpecialSurface> surface = fImage->makeSurface(fImage->colorType(), fImage->getColorSpace(), SkISize(dstBounds.size()), kPremul_SkAlphaType, {});
    if (!surface) {
        return {};
    }
