...
SkStrikeCache* SkStrikeCache::GlobalStrikeCache()
{
    if (gSkUseThreadLocalStrikeCaches_IAcknowledgeThisIsIncrediblyExperimental) {
        ...
        // return cache;
        static SimpleTlsPointerXp<SkStrikeCache> cache;
        if (!cache.Get())
            cache.Set(new SkStrikeCache());
        return cache.Get();
    }
...
sk_sp<SkStrike> SkStrikeCache::findOrCreateStrike(const SkStrikeSpec& strikeSpec)
{
...
void SkStrikeCache::DumpMemoryStatistics(SkTraceMemoryDump* dump)
{
    ...
    auto visitor = [&dump](const SkStrike& strike) {
        ...
        SkString dumpName = SkStringPrintf("%s/%s_%d/%p", gGlyphCacheDumpName, fontName.c_str(), rec.fTypefaceID, &strike);

        dump->dumpNumericValue(dumpName.c_str(), "size", "bytes", strike.fMemoryUsed);
        dump->dumpNumericValue(dumpName.c_str(), "glyph_count", "objects", strike.fScalerCache.countCachedGlyphs());
...
sk_sp<SkStrike> SkStrikeCache::internalFindStrikeOrNull(const SkDescriptor& desc)
{
...
#if SK_SUPPORT_GPU
sk_sp<sktext::gpu::TextStrike> SkStrike::findOrCreateTextStrike(sktext::gpu::StrikeCache* gpuStrikeCache) const
{
