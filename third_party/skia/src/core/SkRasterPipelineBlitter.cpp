...
public:
    ...
    static SkBlitter* Create(
        const SkPixmap&, const SkPaint&, SkArenaAlloc*, const SkRasterPipeline& shaderPipeline, bool is_opaque, bool is_constant, sk_sp<SkShader> clipShader);
...
SkBlitter* SkCreateRasterPipelineBlitter(const SkPixmap& dst, const SkPaint& paint, const SkMatrixProvider& matrixProvider, SkArenaAlloc* alloc,
    sk_sp<SkShader> clipShader, const SkSurfaceProps& props)
{
    ...
    if (shader->appendStages({ &shaderPipeline, alloc, dstCT, dstCS, paint, nullptr, matrixProvider, props })) {
    ...
    bool is_opaque, bool is_constant, sk_sp<SkShader> clipShader)
{
    ...
    if (clipShader) {
        ...
        SkStageRec rec = { clipP, alloc, clipCT, clipCS, clipPaint, nullptr, clipMatrixProvider, props };
    ...
    if (auto colorFilter = paint.getColorFilter()) {
        ...
        SkStageRec rec = { colorPipeline, alloc, dst.colorType(), dst.colorSpace(), paint, nullptr, matrixProvider, props };
...
{
    if (fMemset2D) {
        SK_BLITTER_TRACE_STEP(blitRectByMemset, trace,
            /*scanlines=*/h,
            /*pixels=*/w * h);
    ...
    if (!fBlitRect) {
        ...
        if (fBlend == SkBlendMode::kSrcOver && (fDst.info().colorType() == kRGBA_8888_SkColorType || fDst.info().colorType() == kBGRA_8888_SkColorType)
