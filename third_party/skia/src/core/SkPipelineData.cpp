...
SkUniformDataBlock* SkUniformDataBlock::Make(const SkUniformDataBlock& other, SkArenaAlloc* arena)
{
    ...
    return arena->make([&](void* ptr) { return new (ptr) SkUniformDataBlock(SkSpan<const char>(mem, other.size())); });
}
...
SkTextureDataBlock* SkTextureDataBlock::Make(const SkTextureDataBlock& other, SkArenaAlloc* arena)
{
    return arena->make([&](void* ptr) { return new (ptr) SkTextureDataBlock(other); });
}
