...
SkStrikeSpec SkStrikeSpec::MakePDFVector(const SkTypeface& typeface, int* size)
{
    ...
    return SkStrikeSpec(font, SkPaint(), SkSurfaceProps(0, kUnknown_SkPixelGeometry), SkScalerContextFlags::kFakeGammaAndBoostContrast, SkMatrix::I());
}

#if 1 // SK_SUPPORT_GPU || defined(SK_GRAPHITE_ENABLED)
std::tuple<SkStrikeSpec, SkScalar, sktext::gpu::SDFTMatrixRange> SkStrikeSpec::MakeSDFT(const SkFont& font, const SkPaint& paint,
    const SkSurfaceProps& surfaceProps, const SkMatrix& deviceMatrix, const SkPoint& textLocation, const sktext::gpu::SDFTControl& control)
{
...
sk_sp<sktext::gpu::TextStrike> SkStrikeSpec::findOrCreateTextStrike(sktext::gpu::StrikeCache* cache) const
{
...
sktext::ScopedStrikeForGPU SkStrikeSpec::findOrCreateScopedStrike(sktext::StrikeForGPUCacheInterface* cache) const
{
