...
public:
    ...
    void findIntercepts(const SkScalar bounds[2], SkScalar scale, SkScalar xPos, SkGlyph* glyph, SkScalar* array, int* count)
    {
    ...
    SkSpan<const SkGlyph*> prepareDrawables(SkSpan<const SkGlyphID> glyphIDs, const SkGlyph* results[])
    {
...
#if SK_SUPPORT_GPU
    sk_sp<sktext::gpu::TextStrike> findOrCreateTextStrike(sktext::gpu::StrikeCache* gpuStrikeCache) const;
#endif
    ...
    {
        auto [rect, increase] = fScalerCache.prepareForSDFTDrawing(strikeToSourceScale, accepted, rejected);
        this->updateDelta(increase);
        return rect;
    }
...
public:
    ...
    sktext::ScopedStrikeForGPU findOrCreateScopedStrike(const SkStrikeSpec& strikeSpec) override SK_EXCLUDES(fLock);
