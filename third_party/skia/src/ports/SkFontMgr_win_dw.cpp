...
SkTypeface* SkFontMgr_DirectWrite::onMatchFamilyStyle(const char familyName[], const SkFontStyle& fontstyle) const
{
...
sk_sp<SkTypeface> SkFontMgr_DirectWrite::onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
    ...
    SkAutoIDWriteUnregister<StreamFontFileLoader> autoUnregisterFontFileLoader(fFactory.get(), fontFileLoader.get());
    ...
    SkAutoIDWriteUnregister<StreamFontCollectionLoader> autoUnregisterFontCollectionLoader(fFactory.get(), fontCollectionLoader.get());
    ...
    for (UINT32 familyIndex = 0; familyIndex < familyCount; ++familyIndex) {
        ...
        for (UINT32 fontIndex = 0; fontIndex < fontCount; ++fontIndex) {
            ...
            return DWriteFontTypeface::Make(fFactory.get(), fontFace.get(), font.get(), fontFamily.get(),
                sk_make_sp<DWriteFontTypeface::Loaders>(fFactory.get(), autoUnregisterFontFileLoader.detatch(), autoUnregisterFontCollectionLoader.detatch()),
                args.getPalette());
        }
    }
...
SK_API sk_sp<SkFontMgr> SkFontMgr_New_DirectWrite(IDWriteFactory* factory, IDWriteFontCollection* collection)
{
