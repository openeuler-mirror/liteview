...
public:
    SkTypeface_AndroidSystem(const SkString& pathName, const bool cacheFontFiles, int index, const SkFixed* axes, int axesCount, const SkFontStyle& style,
        bool isFixedPitch, const SkString& familyName, const SkTArray<SkLanguage, true>& lang, FontVariant variantStyle)
    ...
    {
        return std::make_unique<SkFontData>(this->makeStream(), fIndex, 0, fAxes.begin(), fAxes.count(), nullptr, 0);
    }
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_make_sp<SkTypeface_AndroidSystem>(
            fPathName, fFile, fIndex, data->getAxis(), data->getAxisCount(), this->fontStyle(), this->isFixedPitch(), fFamilyName, fLang, fVariantStyle);
    }
...
public:
    SkTypeface_AndroidStream(std::unique_ptr<SkFontData> data, const SkFontStyle& style, bool isFixedPitch, const SkString& familyName)
        : INHERITED(style, isFixedPitch, familyName)
        , fData(std::move(data))
    {
    }
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_make_sp<SkTypeface_AndroidStream>(std::move(data), this->fontStyle(), this->isFixedPitch(), fFamilyName);
    }
...
public:
    explicit SkFontStyleSet_Android(const FontFamily& family, const Scanner& scanner, const bool cacheFontFiles)
    {
        ...
        for (int i = 0; i < family.fFonts.count(); ++i) {
            ...
            if (!scanner.scanFont(stream.get(), ttcIndex, &familyName, &style, &isFixedWidth, &axisDefinitions)) {
            ...
            SkFontArguments::VariationPosition position = { fontFile.fVariationDesignPosition.begin(), fontFile.fVariationDesignPosition.count() };
            ...
            fStyles.push_back().reset(new SkTypeface_AndroidSystem(
                pathName, cacheFontFiles, ttcIndex, axisValues.get(), axisDefinitions.count(), style, isFixedWidth, familyName, family.fLanguages, variant));
        }
    }
...
protected:
    ...
    static sk_sp<SkTypeface_AndroidSystem> find_family_style_character(const SkString& familyName, const SkTArray<NameToFamily, true>& fallbackNameToFamilyMap,
    ...
    sk_sp<SkTypeface> onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const override
    {
        ...
        return sk_sp<SkTypeface>(new SkTypeface_AndroidStream(std::move(data), style, isFixedPitch, name));
    }
    ...
    sk_sp<SkTypeface> onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const override
    {
        ...
        SkString name;
        Scanner::AxisDefinitions axisDefinitions;
        if (!fScanner.scanFont(stream.get(), args.getCollectionIndex(), &name, &style, &isFixedPitch, &axisDefinitions)) {
            return nullptr;
        }
        ...
        auto data = std::make_unique<SkFontData>(std::move(stream), args.getCollectionIndex(), args.getPalette().index, axisValues.get(),
...
private:
    ...
    void addFamily(FontFamily& family, const bool isolated, int familyIndex)
    {
        ...
        sk_sp<SkFontStyleSet_Android> newSet = sk_make_sp<SkFontStyleSet_Android>(family, fScanner, isolated);
        if (0 == newSet->count()) {
            return;
        }
