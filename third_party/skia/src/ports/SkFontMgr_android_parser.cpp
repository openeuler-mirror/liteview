...
static void parse_space_separated_languages(const char* value, size_t valueLen, SkTArray<SkLanguage, true>& languages)
{
    size_t i = 0;
    while (true) {
                ...
                if (valueLen == 4) {
                    ...
                    for (int j = 0; j < file.fVariationDesignPosition.count() - 1; ++j) {
                        if (file.fVariationDesignPosition[j].axis == axisTag) {
                            ...
                            SK_FONTCONFIGPARSER_WARNING("'%c%c%c%c' axis specified more than once", (axisTag >> 24) & 0xFF, (axisTag >> 16) & 0xFF,
                                (axisTag >> 8) & 0xFF, (axisTag)&0xFF);
                        }
                    }
                } else {
