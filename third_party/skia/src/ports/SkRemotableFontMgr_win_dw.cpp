...
private:
    ...
    int FindOrAdd(IDWriteFontFileLoader* fontFileLoader, const void* refKey, UINT32 refKeySize) const
    {
        SkTScopedComPtr<IUnknown> fontFileLoaderId;
        HR_GENERAL(fontFileLoader->QueryInterface(&fontFileLoaderId), "Failed to re-convert to IDWriteFontFileLoader.", SkFontIdentity::kInvalidDataId);
...
public:
    ...
    SkRemotableFontMgr_DirectWrite(IDWriteFontCollection* fontCollection, WCHAR* localeName, int localeNameLength)
    ...
    SkRemotableFontIdentitySet* getIndex(int familyIndex) const override
    {
        ...
        HRNM(fFontCollection->GetFontFamily(familyIndex, &fontFamily), "Could not get requested family.");
    ...
    virtual SkFontIdentity matchIndexStyle(int familyIndex, const SkFontStyle& pattern) const override
    {
        SkFontIdentity identity = { SkFontIdentity::kInvalidDataId };

        SkTScopedComPtr<IDWriteFontFamily> fontFamily;
        HR_GENERAL(fFontCollection->GetFontFamily(familyIndex, &fontFamily), "Could not get requested family.", identity);

        const DWriteStyle dwStyle(pattern);
        SkTScopedComPtr<IDWriteFont> font;
        HR_GENERAL(fontFamily->GetFirstMatchingFont(dwStyle.fWeight, dwStyle.fWidth, dwStyle.fSlant, &font), "Could not match font in family.", identity);
    ...
    SkRemotableFontIdentitySet* matchName(const char familyName[]) const override
    {
        ...
        if (nullptr == familyName) {
            HR_GENERAL(getDefaultFontFamilyName(&dwFamilyName), nullptr, SkRemotableFontIdentitySet::NewEmpty());
        } else {
            HR_GENERAL(sk_cstring_to_wchar(familyName, &dwFamilyName), nullptr, SkRemotableFontIdentitySet::NewEmpty());
        }

        UINT32 index;
        BOOL exists;
        HR_GENERAL(fFontCollection->FindFamilyName(dwFamilyName.get(), &index, &exists), "Failed while finding family by name.",
    ...
    virtual SkFontIdentity matchNameStyle(const char familyName[], const SkFontStyle& style) const override
    {
        SkFontIdentity identity = { SkFontIdentity::kInvalidDataId };
        ...
        UINT32 index;
        BOOL exists;
        HR_GENERAL(fFontCollection->FindFamilyName(dwFamilyName.get(), &index, &exists), "Failed while finding family by name.", identity);
        if (!exists) {
            return identity;
        }
    ...
    public:
        ...
        const SkFontIdentity FallbackIdentity()
        {
            return fIdentity;
        }
    ...
    virtual SkFontIdentity matchNameStyleCharacter(
        const char familyName[], const SkFontStyle& pattern, const char* bcp47[], int bcp47Count, SkUnichar character) const override
    {
        SkFontIdentity identity = { SkFontIdentity::kInvalidDataId };
        ...
        HR_GENERAL(
            dwFactory->CreateTextFormat(dwFamilyName, fFontCollection.get(), dwStyle.fWeight, dwStyle.fSlant, dwStyle.fWidth, 72.0f, *dwBcp47, &fallbackFormat),
            "Could not create text format.", identity);

        WCHAR str[16];
        UINT32 strLen = static_cast<UINT32>(SkUTF::ToUTF16(character, reinterpret_cast<uint16_t*>(str)));
        SkTScopedComPtr<IDWriteTextLayout> fallbackLayout;
        HR_GENERAL(dwFactory->CreateTextLayout(str, strLen, fallbackFormat.get(), 200.0f, 200.0f, &fallbackLayout), "Could not create text layout.", identity);

        SkTScopedComPtr<FontFallbackRenderer> fontFallbackRenderer(new FontFallbackRenderer(this, character));

        HR_GENERAL(fallbackLayout->Draw(nullptr, fontFallbackRenderer.get(), 50.0f, 50.0f), "Could not draw layout with renderer.", identity);
    ...
    SkStreamAsset* getData(int dataId) const override
    {
        ...
        HRNM(loader->CreateStreamFromKey(id.fKey, id.fKeySize, &fontFileStream), "Could not create font file stream.");
...
SkRemotableFontMgr* SkRemotableFontMgr_New_DirectWrite()
{
    ...
    HRNM(factory->GetSystemFontCollection(&sysFontCollection, FALSE), "Could not get system font collection.");
