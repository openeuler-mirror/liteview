...
private:
    ...
    static void load_directory_fonts(
        const SkTypeface_FreeType::Scanner& scanner, const SkString& directory, const char* suffix, SkFontMgr_Custom::Families* families)
    {
        ...
        while (iter.next(&name, false)) {
            ...
            for (int faceIndex = 0; faceIndex < numFaces; ++faceIndex) {
                ...
                addTo->appendTypeface(sk_make_sp<SkTypeface_File>(style, isFixedPitch, true, realname, filename.c_str(), faceIndex));
            }
        }
