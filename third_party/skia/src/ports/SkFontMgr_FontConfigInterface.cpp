...
protected:
    ...
    SkTypeface* onMatchFamilyStyle(const char requestedFamilyName[], const SkFontStyle& requestedStyle) const override
    {
    ...
    sk_sp<SkTypeface> onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const override
    {
        ...
        auto fontData = std::make_unique<SkFontData>(std::move(stream), ttcIndex, 0, nullptr, 0, nullptr, 0);
        return sk_sp<SkTypeface>(SkTypeface_FCI::Create(std::move(fontData), std::move(name), style, isFixedPitch));
    }
