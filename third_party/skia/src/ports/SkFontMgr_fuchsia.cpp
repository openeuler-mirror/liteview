...
sk_sp<SkTypeface> CreateTypefaceFromSkStream(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args, TypefaceId id)
{
    ...
    if (!scanner.scanFont(stream.get(), args.getCollectionIndex(), &name, &style, &isFixedPitch, &axisDefinitions)) {
        return nullptr;
    }
...
SkTypeface* SkFontMgr_Fuchsia::onMatchFamilyStyle(const char familyName[], const SkFontStyle& style) const
{
    sk_sp<SkTypeface> typeface = FetchTypeface(familyName, style, /*bcp47=*/nullptr,
        /*bcp47Count=*/0, /*character=*/0,
...
{
    sk_sp<SkTypeface> typeface = FetchTypeface(familyName, style, bcp47, bcp47Count, character, /*allow_fallback=*/true,
