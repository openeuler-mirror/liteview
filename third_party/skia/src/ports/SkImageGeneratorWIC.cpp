...
public:
     ...
     */
    ImageGeneratorWIC(const SkImageInfo& info, IWICImagingFactory* imagingFactory, IWICBitmapSource* imageSource, sk_sp<SkData>);
...
std::unique_ptr<SkImageGenerator> SkImageGeneratorWIC::MakeFromEncodedWIC(sk_sp<SkData> data)
{
    ...
    return std::unique_ptr<SkImageGenerator>(new ImageGeneratorWIC(info, imagingFactory.release(), imageSource.release(), std::move(data)));
}

ImageGeneratorWIC::ImageGeneratorWIC(const SkImageInfo& info, IWICImagingFactory* imagingFactory, IWICBitmapSource* imageSource, sk_sp<SkData> data)
    : INHERITED(info)
...
bool ImageGeneratorWIC::onGetPixels(const SkImageInfo& info, void* pixels, size_t rowBytes, const Options&)
{
    ...
    hr = formatConverterSrc->CopyPixels(nullptr, (UINT)rowBytes, (UINT)rowBytes * info.height(), (BYTE*)pixels);
