...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_Mac::onGetAdvancedMetrics() const
{
    ...
    if (!this->getTableSize(SkSetFourByteTag('g', 'l', 'y', 'f')) || !this->getTableSize(SkSetFourByteTag('l', 'o', 'c', 'a'))) {
        return info;
    }
...
CTFontVariation SkCTVariationFromSkFontArguments(CTFontRef ct, CFArrayRef ctAxes, const SkFontArguments& args)
{
    OpszVariation opsz;
...
{
    CTFontVariation ctVariation = SkCTVariationFromSkFontArguments(fFontRef.get(), this->getVariationAxes(), args);
