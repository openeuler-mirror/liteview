...
using SkUniqueFTFace = std::unique_ptr<FT_FaceRec, SkFunctionWrapper<decltype(FT_Done_Face), FT_Done_Face>>;
...
public:
        ...
        : fLibrary(nullptr)
    {
        ...
        if (SkGraphics::GetVariableColrV1Enabled()) {
            ...
            FT_Property_Set(fLibrary, "truetype", "TEMPORARY-enable-variable-colrv1", &variableColrV1Enabled);
        }
#endif
...
std::unique_ptr<SkAdvancedTypefaceMetrics> SkTypeface_FreeType::onGetAdvancedMetrics() const
{
    ...
    if (info->fType == SkAdvancedTypefaceMetrics::kTrueType_Font && !is_opentype_font_data_standard_format(*this)) {
...
{
    auto c = std::make_unique<SkScalerContext_FreeType>(sk_ref_sp(const_cast<SkTypeface_FreeType*>(this)), effects, desc);
    if (c->success()) {
        return std::move(c);
    }
...
std::unique_ptr<SkFontData> SkTypeface_FreeType::cloneFontData(const SkFontArguments& args) const
{
    ...
    Scanner::computeAxisValues(
        axisDefinitions, args.getVariationDesignPosition(), axisValues, name, currentAxisCount == axisCount ? currentPosition.get() : nullptr);
...
SkScalerContext_FreeType::SkScalerContext_FreeType(sk_sp<SkTypeface_FreeType> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
...
bool SkScalerContext_FreeType::generateAdvance(SkGlyph* glyph)
{
    ...
    error = FT_Get_Advance(fFace, glyph->getGlyphID(), fLoadGlyphFlags | FT_ADVANCE_FLAG_FAST_ONLY, &advance);
...
void SkScalerContext_FreeType::setGlyphBounds(SkGlyph* glyph, SkRect* bounds, bool subpixel)
{
    ...
    if (bounds->isEmpty()) {
        ...
        if (!SkTFitsIn<decltype(glyph->fWidth)>(irect.width()) || !SkTFitsIn<decltype(glyph->fHeight)>(irect.height())
            || !SkTFitsIn<decltype(glyph->fTop)>(irect.top()) || !SkTFitsIn<decltype(glyph->fLeft)>(irect.left())) {
            irect = SkIRect::MakeEmpty();
        }
    }
...
void SkScalerContext_FreeType::generateMetrics(SkGlyph* glyph, SkArenaAlloc* alloc)
{
    ...
    if (FT_IS_SCALABLE(fFace)) {
        ...
        if (FT_Get_Color_Glyph_Paint(fFace, glyph->getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM, &opaqueLayerPaint)) {
            ...
            if (FT_Get_Color_Glyph_ClipBox(fFace, glyph->getGlyphID(), &clipBox)) {
                ...
                if (!computeColrV1GlyphBoundingBox(fFace, glyph->getGlyphID(), &bounds) || this->setupSize()) {
                    glyph->zeroMetrics();
                    return;
                }
            }
    ...
    if (!haveLayers) {
        ...
        if (fFace->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
            ...
            SkRect bounds = SkRect::MakeXYWH(SkIntToScalar(fFace->glyph->bitmap_left), -SkIntToScalar(fFace->glyph->bitmap_top),
...
void SkScalerContext_FreeType::generateImage(const SkGlyph& glyph)
{
    ...
    if (glyph.fScalerContextBits == ScalerContextBits::COLRv0 || glyph.fScalerContextBits == ScalerContextBits::COLRv1
        || glyph.fScalerContextBits == ScalerContextBits::SVG) {
        SkASSERT(glyph.maskFormat() == SkMask::kARGB32_Format);
        SkBitmap dstBitmap;
        // TODO: mark this as sRGB when the blits will be sRGB.
        dstBitmap.setInfo(SkImageInfo::Make(glyph.fWidth, glyph.fHeight, kN32_SkColorType, kPremul_SkAlphaType), glyph.rowBytes());
...
void SkScalerContext_FreeType::emboldenIfNeeded(FT_Face face, FT_GlyphSlot glyph, SkGlyphID gid)
{
    ...
    case FT_GLYPH_FORMAT_OUTLINE:
        FT_Pos strength;
        strength = FT_MulFix(face->units_per_EM, face->size->metrics.y_scale) / SK_OUTLINE_EMBOLDEN_DIVISOR;
...
FT_Face SkTypeface_FreeType::Scanner::openFace(SkStreamAsset* stream, int ttcIndex, FT_Stream ftStream) const
{
...
bool SkTypeface_FreeType::Scanner::scanFont(
    SkStreamAsset* stream, int ttcIndex, SkString* name, SkFontStyle* style, bool* isFixedPitch, AxisDefinitions* axes) const
{
    SkAutoMutexExclusive libraryLock(fLibraryMutex);
        ...
        if (GetAxes(face.get(), &axisDefinitions)) {
            ...
            for (size_t i = 0; i < numAxes; ++i) {
                if (axisDefinitions[i].fTag == wghtTag) {
                    ...
                    // within 0-1000.
                    int wghtRange = SkFixedToScalar(axisDefinitions[i].fMaximum) - SkFixedToScalar(axisDefinitions[i].fMinimum);
                    if (wghtRange > 5 && wghtRange <= 1000 && SkFixedToScalar(axisDefinitions[i].fMaximum) <= 1000) {
                        wghtIndex = i;
                    }
                }
                if (axisDefinitions[i].fTag == wdthTag) {
                    ...
                    // 0-500.
                    int widthRange = SkFixedToScalar(axisDefinitions[i].fMaximum) - SkFixedToScalar(axisDefinitions[i].fMinimum);
                    if (widthRange > 0 && widthRange <= 500 && SkFixedToScalar(axisDefinitions[i].fMaximum) <= 500)
                        wdthIndex = i;
                }
...
/*static*/ void SkTypeface_FreeType::Scanner::computeAxisValues(AxisDefinitions axisDefinitions, const SkFontArguments::VariationPosition position,
    SkFixed* axisValues, const SkString& name, const SkFontArguments::VariationPosition::Coordinate* current)
{
