...
void copyFT2LCD16(const FT_Bitmap& bitmap, const SkMask& mask, int lcdIsBGR, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
...
void truncateToStopInterpolating(SkScalar zeroRadiusStop, std::vector<SkColor>& colors, std::vector<SkScalar>& stops, TruncateStops truncateStops)
{
    if (stops.size() <= 1u || zeroRadiusStop < *stops.begin() || *(stops.end() - 1) < zeroRadiusStop) {
        return;
    }
...
bool colrv1_configure_skpaint(FT_Face face, const SkSpan<SkColor>& palette, const SkColor foregroundColor, const FT_COLR_Paint& colrPaint, SkPaint* paint)
{
    auto fetchColorStops = [&face, &palette, &foregroundColor](
                               const FT_ColorStopIterator& colorStopIterator, std::vector<SkScalar>& stops, std::vector<SkColor>& colors) -> bool {
        ...
        while (FT_Get_Colorline_Stops(face, &color_stop, &mutable_color_stop_iterator)) {
            ...
            if (palette_index == kForegroundColorPaletteIndex) {
                U8CPU newAlpha = SkColorGetA(foregroundColor) * SkColrV1AlphaToFloat(color_stop.color.alpha);
                colorStopsSorted[index].color = SkColorSetA(foregroundColor, newAlpha);
            } else if (palette_index >= palette.size()) {
                return false;
            } else {
                U8CPU newAlpha = SkColorGetA(palette[palette_index]) * SkColrV1AlphaToFloat(color_stop.color.alpha);
    ...
    case FT_COLR_PAINTFORMAT_SOLID: {
        ...
        if (solid.color.palette_index == kForegroundColorPaletteIndex) {
            ...
            U8CPU newAlpha = SkColorGetA(palette[solid.color.palette_index]) * SkColrV1AlphaToFloat(solid.color.alpha);
    ...
    case FT_COLR_PAINTFORMAT_LINEAR_GRADIENT: {
        ...
        sk_sp<SkShader> shader(SkGradientShader::MakeLinear(linePositions, colors.data(), stops.data(), stops.size(), tileMode));
        SkASSERT(shader);
    ...
    case FT_COLR_PAINTFORMAT_RADIAL_GRADIENT: {
        ...
        paint->setShader(SkGradientShader::MakeTwoPointConical(start, startRadius, end, endRadius, colors.data(), stops.data(), stops.size(), tileMode));
        return true;
    }
    case FT_COLR_PAINTFORMAT_SWEEP_GRADIENT: {
        ...
        if (SkGraphics::GetVariableColrV1Enabled()) {
            ...
            paint->setShader(SkGradientShader::MakeSweep(
                center.x(), center.y(), colors.data(), stops.data(), stops.size(), tileMode, startAngleScaled, endAngleScaled, 0, nullptr));
            ...
            SkScalar sectorAngle = endAngle > startAngle ? endAngle - startAngle : endAngle + 360.0f - startAngle;
            ...
            paint->setShader(
                SkGradientShader::MakeSweep(center.x(), center.y(), colors.data(), stops.data(), stops.size(), tileMode, 0, sectorAngle, 0, &localMatrix));
        }
        return true;
    }
    default: {
...
#ifdef TT_SUPPORT_COLRV1
bool SkScalerContext_FreeType_Base::drawCOLRv1Glyph(FT_Face face, const SkGlyph& glyph, uint32_t loadGlyphFlags, SkSpan<SkColor> palette, SkCanvas* canvas)
{
    if (this->isSubpixel()) {
    ...
    bool haveLayers = colrv1_start_glyph(canvas, palette, fRec.fForegroundColor, face, glyph.getGlyphID(), FT_COLOR_INCLUDE_ROOT_TRANSFORM, &activePaints);
...
#ifdef FT_COLOR_H
bool SkScalerContext_FreeType_Base::drawCOLRv0Glyph(FT_Face face, const SkGlyph& glyph, uint32_t loadGlyphFlags, SkSpan<SkColor> palette, SkCanvas* canvas)
{
    if (this->isSubpixel()) {
        canvas->translate(SkFixedToScalar(glyph.getSubXFixed()), SkFixedToScalar(glyph.getSubYFixed()));
    }
...
#if defined(FT_CONFIG_OPTION_SVG)
bool SkScalerContext_FreeType_Base::drawSVGGlyph(FT_Face face, const SkGlyph& glyph, uint32_t loadGlyphFlags, SkSpan<SkColor> palette, SkCanvas* canvas)
{
    ...
    return svgDecoder->render(*canvas, ftSvg->units_per_EM, glyph.getGlyphID(), fRec.fForegroundColor, palette);
}
#endif // FT_CONFIG_OPTION_SVG

void SkScalerContext_FreeType_Base::generateGlyphImage(FT_Face face, const SkGlyph& glyph, const SkMatrix& bitmapTransform)
{
    ...
    case FT_GLYPH_FORMAT_OUTLINE: {
        ...
        if (SkMask::kLCD16_Format == glyph.fMaskFormat) {
            ...
            if (fPreBlend.isApplicable()) {
                ...
                copyFT2LCD16<false>(ftGlyph.bitmap, mask, doBGR, fPreBlend.fR, fPreBlend.fG, fPreBlend.fB);
            }
    ...
    case FT_GLYPH_FORMAT_BITMAP: {
        ...
        SkBitmap dstBitmap;
        // TODO: mark this as sRGB when the blits will be sRGB.
        dstBitmap.setInfo(SkImageInfo::Make(glyph.fWidth, glyph.fHeight, SkColorType_for_SkMaskFormat(maskFormat), kPremul_SkAlphaType), bitmapRowBytes);
        if (SkMask::kBW_Format == maskFormat || SkMask::kLCD16_Format == maskFormat) {
...
bool SkScalerContext_FreeType_Base::generateFacePath(FT_Face face, SkGlyphID glyphID, uint32_t loadGlyphFlags, SkPath* path)
{
    return generateFacePathStatic(face, glyphID, loadGlyphFlags, path);
}

#ifdef TT_SUPPORT_COLRV1
bool SkScalerContext_FreeType_Base::computeColrV1GlyphBoundingBox(FT_Face face, SkGlyphID glyphID, SkRect* bounds)
{
    SkMatrix ctm;
