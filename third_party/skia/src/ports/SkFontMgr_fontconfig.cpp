...
public:
    SkTypeface_stream(std::unique_ptr<SkFontData> data, SkString familyName, const SkFontStyle& style, bool fixedWidth)
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_make_sp<SkTypeface_stream>(std::move(data), fFamilyName, this->fontStyle(), this->isFixedPitch());
    }
...
public:
    ...
    sk_sp<SkTypeface> onMakeClone(const SkFontArguments& args) const override
    {
        ...
        return sk_make_sp<SkTypeface_stream>(std::move(data), familyName, this->fontStyle(), this->isFixedPitch());
    }
...
private:
    SkTypeface_fontconfig(SkAutoFcPattern pattern, SkString sysroot)
        : INHERITED(skfontstyle_from_fcpattern(pattern), FC_PROPORTIONAL != get_int(pattern, FC_SPACING, FC_PROPORTIONAL))
...
protected:
    ...
    sk_sp<SkTypeface> onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const override
    {
        ...
        return sk_sp<SkTypeface>(new SkTypeface_stream(std::move(data), std::move(name), style, isFixedWidth));
    }
    ...
    sk_sp<SkTypeface> onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const override
    {
        ...
        return sk_sp<SkTypeface>(new SkTypeface_stream(std::move(data), std::move(name), style, isFixedPitch));
    }
