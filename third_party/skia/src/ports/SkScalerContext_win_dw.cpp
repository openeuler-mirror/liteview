...
HRESULT SkScalerContext_DW::getBoundingBox(SkGlyph* glyph, DWRITE_RENDERING_MODE renderingMode, DWRITE_TEXTURE_TYPE textureType, RECT* bbox)
{
    DWriteFontTypeface* typeface = this->getDWriteTypeface();
    ...
    SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
    {
        ...
        if (typeface->fFactory2 && (fGridFitMode == DWRITE_GRID_FIT_MODE_DISABLED || fAntiAliasMode == DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE)) {
            HRM(typeface->fFactory2->CreateGlyphRunAnalysis(&run, &fXform, renderingMode, fMeasuringMode, fGridFitMode, fAntiAliasMode,
    ...
    {
        Shared l(maybe_dw_mutex(*typeface));
        HRM(glyphRunAnalysis->GetAlphaTextureBounds(textureType, bbox), "Could not get texture bounds.");
    }
    return S_OK;
}
...
bool SkScalerContext_DW::isPngGlyph(const SkGlyph& glyph)
{
    ...
    HRBM(fontFace4->GetGlyphImageFormats(glyph.getGlyphID(), 0, UINT32_MAX, &f), "Cannot get glyph image formats.");
    return f & DWRITE_GLYPH_IMAGE_FORMATS_PNG;
}

bool SkScalerContext_DW::isSVGGlyph(const SkGlyph& glyph)
{
    if (!SkGraphics::GetOpenTypeSVGDecoderFactory() || !this->getDWriteTypeface()->fDWriteFontFace4) {
        return false;
    }
...
void SkScalerContext_DW::SetGlyphBounds(SkGlyph* glyph, const SkRect& bounds)
{
    ...
    if (!SkTFitsIn<decltype(glyph->fWidth)>(ibounds.width()) || !SkTFitsIn<decltype(glyph->fHeight)>(ibounds.height())
        || !SkTFitsIn<decltype(glyph->fTop)>(ibounds.top()) || !SkTFitsIn<decltype(glyph->fLeft)>(ibounds.left())) {
...
bool SkScalerContext_DW::generatePngMetrics(SkGlyph* glyph)
{
    *(int*)1 = 1;
    ...
    //     SkRect bounds
    //         = SkRect::MakeLTRB(SkIntToScalar(info.bounds().fLeft), SkIntToScalar(info.bounds().fTop), SkIntToScalar(info.bounds().fRight), SkIntToScalar(info.bounds().fBottom));
    //
    //     SkMatrix matrix = fSkXform;
...
void SkScalerContext_DW::generateMetrics(SkGlyph* glyph, SkArenaAlloc* alloc)
{
    ...
    auto glyphCheckAndSetBounds = [](SkGlyph* glyph, const RECT& bbox) {
        ...
        if (!SkIRect::MakeXYWH(-32767, -32767, 65535, 65535).contains(SkIRect::MakeLTRB(bbox.left, bbox.top, bbox.right, bbox.bottom))) {
            return false;
        }
    ...
    RECT bbox;
    HRVM(this->getBoundingBox(glyph, fRenderingMode, fTextureType, &bbox), "Requested bounding box could not be determined.");
    ...
    if (DWRITE_TEXTURE_ALIASED_1x1 != fTextureType || DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE == fAntiAliasMode) {
        HRVM(this->getBoundingBox(glyph, DWRITE_RENDERING_MODE_ALIASED, DWRITE_TEXTURE_ALIASED_1x1, &bbox), "Fallback bounding box could not be determined.");
        if (glyphCheckAndSetBounds(glyph, bbox)) {
...
template <bool APPLY_PREBLEND> void SkScalerContext_DW::GrayscaleToA8(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, const uint8_t* table8)
{
...
template <bool APPLY_PREBLEND> void SkScalerContext_DW::RGBToA8(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, const uint8_t* table8)
{
    const size_t dstRB = glyph.rowBytes();
    const int width = glyph.width();
    uint8_t* SK_RESTRICT dst = static_cast<uint8_t*>(glyph.fImage);
...
void SkScalerContext_DW::RGBToLcd16(const uint8_t* SK_RESTRICT src, const SkGlyph& glyph, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB)
{
    const size_t dstRB = glyph.rowBytes();
    const int width = glyph.width();
...
const void* SkScalerContext_DW::drawDWMask(const SkGlyph& glyph, DWRITE_RENDERING_MODE renderingMode, DWRITE_TEXTURE_TYPE textureType)
{
    DWriteFontTypeface* typeface = this->getDWriteTypeface();
    ...
    run.glyphOffsets = &offset;
    {
        SkTScopedComPtr<IDWriteGlyphRunAnalysis> glyphRunAnalysis;
        {
            ...
            if (this->getDWriteTypeface()->fFactory2
                && (fGridFitMode == DWRITE_GRID_FIT_MODE_DISABLED || fAntiAliasMode == DWRITE_TEXT_ANTIALIAS_MODE_GRAYSCALE)) {
                HRNM(this->getDWriteTypeface()->fFactory2->CreateGlyphRunAnalysis(&run, &fXform, renderingMode, fMeasuringMode, fGridFitMode, fAntiAliasMode,
                         0.0f, // baselineOriginX,
                         0.0f, // baselineOriginY,
                         &glyphRunAnalysis),
                    "Could not create DW2 glyph run analysis.");
