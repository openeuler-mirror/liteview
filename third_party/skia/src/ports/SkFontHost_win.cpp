...
private:
    ...
    template <bool APPLY_PREBLEND> static void RGBToA8(const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, const uint8_t* table8);

    template <bool APPLY_PREBLEND>
    static void RGBToLcd16(
        const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, const uint8_t* tableR, const uint8_t* tableG, const uint8_t* tableB);
...
void SkScalerContext_GDI::generateMetrics(SkGlyph* glyph, SkArenaAlloc* alloc)
{
    ...
    if (fType == SkScalerContext_GDI::kBitmap_Type || fType == SkScalerContext_GDI::kLine_Type) {
        ...
        if (glyph->fWidth > 300 || glyph->fHeight > 300) // SUPPORT_XP_CODE
            glyph->zeroMetrics();
...
#define SK_GDI_ALWAYS_USE_TEXTMETRICS_FOR_FONT_METRICS 1 // weolar:SUPPORT_XP_CODE
#endif
...
#ifdef SUPPORT_XP_CODE
    metrics->fCapHeight = fTM.tmHeight - fTM.tmInternalLeading; // weolar
#endif
...
template <bool APPLY_PREBLEND> void SkScalerContext_GDI::RGBToA8(const SkGdiRGB* SK_RESTRICT src, size_t srcRB, const SkGlyph& glyph, const uint8_t* table8)
{
...
DWORD SkScalerContext_GDI::getGDIGlyphPath(SkGlyphID glyph, UINT flags, SkAutoSTMalloc<BUFFERSIZE, uint8_t>* glyphbuf)
{
    GLYPHMETRICS gm;
...
protected:
    ...
    virtual SkTypeface* onMatchFamilyStyle(const char familyName[], const SkFontStyle& fontstyle) const override
    {
