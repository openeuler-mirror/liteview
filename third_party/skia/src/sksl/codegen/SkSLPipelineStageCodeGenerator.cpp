...
void PipelineStageCodeGenerator::writeFunction(const FunctionDefinition& f)
{
    ...
    if (decl.isMain() && fProgram.fConfig->fKind != SkSL::ProgramKind::kMeshVertex && fProgram.fConfig->fKind != SkSL::ProgramKind::kMeshFragment) {
...
std::string PipelineStageCodeGenerator::functionDeclaration(const FunctionDeclaration& decl)
{
    ...
    std::string declString = String::printf("%s%s%s %s(", (decl.modifiers().fFlags & Modifiers::kInline_Flag) ? "inline " : "",
        (decl.modifiers().fFlags & Modifiers::kNoInline_Flag) ? "noinline " : "", this->typeName(decl.returnType()).c_str(), this->functionName(decl).c_str());
    const char* separator = "";
...
void PipelineStageCodeGenerator::writeGlobalVarDeclaration(const GlobalVarDeclaration& g)
{
    ...
    if (var.isBuiltin() || var.type().isOpaque()) {
        ...
        std::string declaration = this->modifierString(var.modifiers()) + this->typedVariable(var.type(), std::string_view(mangledName.c_str()));
        if (decl.value()) {
