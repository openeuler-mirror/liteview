...
static FastF32 operator*(skvm::F32 y)
{
    return { y };
}
static skvm::F32 operator*(skvm::F32 x, FastF32 y)
{
    return fast_mul(x, y.val);
}
static skvm::F32 operator*(float x, FastF32 y)
{
    return fast_mul(x, y.val);
}
...
class SkVMGenerator {
public:
    SkVMGenerator(const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace, SkVMCallbacks* callbacks);

    void writeProgram(
        SkSpan<skvm::Val> uniforms, skvm::Coord device, const FunctionDefinition& function, SkSpan<skvm::Val> arguments, SkSpan<skvm::Val> outReturn);
...
private:
    ...
    void addDebugSlotInfoForGroup(const std::string& varName, const Type& type, int line, int* groupIndex, int fnReturnValue);
    ...
    size_t writeFunction(const IRNode& caller, const FunctionDefinition& function, SkSpan<skvm::Val> arguments);
    ...
    template <typename Fn> Value unary(const Value& v, Fn&& fn)
    {
    ...
    void recursiveBinaryCompare(const Value& lVal, const Type& lType, const Value& rVal, const Type& rType, size_t* slotOffset, Value* result,
        const std::function<Value(skvm::F32 x, skvm::F32 y)>& float_comp, const std::function<Value(skvm::I32 x, skvm::I32 y)>& int_comp);
    ...
    public:
        ScopedCondition(SkVMGenerator* generator, skvm::I32 mask)
            ...
            , fOldConditionMask(fGenerator->fConditionMask)
        {
        ...
        ~ScopedCondition()
        {
            fGenerator->fConditionMask = fOldConditionMask;
        }
...
SkVMGenerator::SkVMGenerator(const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace, SkVMCallbacks* callbacks)
    : fProgram(program)
    , fBuilder(builder)
...
void SkVMGenerator::setupGlobals(SkSpan<skvm::Val> uniforms, skvm::Coord device)
{
    if (fDebugTrace) {
        ...
        skvm::Coord traceCoord = { to_F32(fBuilder->splat(fDebugTrace->fTraceCoord.fX)) + 0.5f, to_F32(fBuilder->splat(fDebugTrace->fTraceCoord.fY)) + 0.5f };
...
size_t SkVMGenerator::writeFunction(const IRNode& caller, const FunctionDefinition& function, SkSpan<skvm::Val> arguments)
{
...
void SkVMGenerator::addDebugSlotInfoForGroup(const std::string& varName, const Type& type, int line, int* groupIndex, int fnReturnValue)
{
    SkASSERT(fDebugTrace);
    ...
    case Type::TypeKind::kArray: {
        ...
        for (int slot = 0; slot < nslots; ++slot) {
            this->addDebugSlotInfoForGroup(varName + "[" + std::to_string(slot) + "]", elemType, line, groupIndex, fnReturnValue);
        }
        break;
    }
    case Type::TypeKind::kStruct: {
        for (const Type::Field& field : type.fields()) {
            this->addDebugSlotInfoForGroup(varName + "." + std::string(field.fName), *field.fType, line, groupIndex, fnReturnValue);
        }
        break;
    }
    default:
...
void SkVMGenerator::addDebugSlotInfo(const std::string& varName, const Type& type, int line, int fnReturnValue)
{
    int groupIndex = 0;
...
size_t SkVMGenerator::createSlot(const std::string& name, const Type& type, int line, int fnReturnValue)
{
...
int SkVMGenerator::getLine(Position pos)
{
    if (pos.valid()) {
        ...
        return std::distance(fLineOffsets.begin(), std::upper_bound(fLineOffsets.begin(), fLineOffsets.end(), pos.startOffset()));
    } else {
        return -1;
    }
}
...
size_t SkVMGenerator::getFunctionSlot(const IRNode& callSite, const FunctionDefinition& fn)
{
    ...
    size_t slot = this->createSlot("[" + std::string(decl.name()) + "].result", decl.returnType(), this->getLine(fn.fPosition),
        /*fnReturnValue=*/1);
    fSlotMap.set(&callSite, slot);
    return slot;
}

void SkVMGenerator::recursiveBinaryCompare(const Value& lVal, const Type& lType, const Value& rVal, const Type& rType, size_t* slotOffset, Value* result,
    const std::function<Value(skvm::F32 x, skvm::F32 y)>& float_comp, const std::function<Value(skvm::I32 x, skvm::I32 y)>& int_comp)
{
    ...
    case Type::TypeKind::kStruct:
        ...
        for (size_t f = 0; f < lType.fields().size(); ++f) {
            ...
            this->recursiveBinaryCompare(lVal, *lField.fType, rVal, *rField.fType, slotOffset, result, float_comp, int_comp);
        }
        break;
    ...
    case Type::TypeKind::kMatrix:
        ...
        for (int c = 0; c < lType.columns(); ++c) {
            this->recursiveBinaryCompare(lVal, lType.componentType(), rVal, rType.componentType(), slotOffset, result, float_comp, int_comp);
        }
        break;
    default:
        ...
        (*result)[*slotOffset] = i32(nk == Type::NumberKind::kFloat ? float_comp(f32(L), f32(R)) : int_comp(i32(L), i32(R))).id;
...
Value SkVMGenerator::writeBinaryExpression(const BinaryExpression& b)
{
    ...
    if (op.kind() == Operator::Kind::STAR && lVecOrMtx && rVecOrMtx && !(lType.isVector() && rType.isVector())) {
        int rCols = rType.columns(), rRows = rType.rows(), lCols = lType.columns(), lRows = lType.rows();
    ...
    auto binary = [&](const std::function<Value(skvm::F32 x, skvm::F32 y)>& f_fn, const std::function<Value(skvm::I32 x, skvm::I32 y)>& i_fn,
        ...
        if (op.isEquality() && (lType.isStruct() || lType.isArray())) {
            ...
            for (size_t slot = 0; slot < nslots; ++slot) {
                ...
                skvm::Val L = lVal.slots() == 1 ? lVal[0] : lVal[slot], R = rVal.slots() == 1 ? rVal[0] : rVal[slot];
    ...
    case Operator::Kind::EQEQ:
        SkASSERT(!isAssignment);
        return binary([](skvm::F32 x, skvm::F32 y) { return x == y; }, [](skvm::I32 x, skvm::I32 y) { return x == y; }, /*foldResults=*/true);
    case Operator::Kind::NEQ:
        SkASSERT(!isAssignment);
        return binary([](skvm::F32 x, skvm::F32 y) { return x != y; }, [](skvm::I32 x, skvm::I32 y) { return x != y; }, /*foldResults=*/true);
    case Operator::Kind::GT:
        return binary([](skvm::F32 x, skvm::F32 y) { return x > y; }, [](skvm::I32 x, skvm::I32 y) { return x > y; });
    ...
    case Operator::Kind::SLASH:
        ...
        return binary([](skvm::F32 x, skvm::F32 y) { return x / y; }, [](skvm::I32 x, skvm::I32 y) { return skvm::trunc(skvm::to_F32(x) / skvm::to_F32(y)); });
...
Value SkVMGenerator::writeTypeConversion(const Value& src, Type::NumberKind srcKind, Type::NumberKind dstKind)
{
...
{
    SkASSERT(m.slots() == 9);
    skvm::F32 a11 = f32(m[0]), a12 = f32(m[3]), a13 = f32(m[6]), a21 = f32(m[1]), a22 = f32(m[4]), a23 = f32(m[7]), a31 = f32(m[2]), a32 = f32(m[5]),
              a33 = f32(m[8]);
    skvm::F32 idet = 1.0f / (a11 * a22 * a33 + a12 * a23 * a31 + a13 * a21 * a32 - a11 * a23 * a32 - a12 * a21 * a33 - a13 * a22 * a31);
...
{
    SkASSERT(m.slots() == 16);
    skvm::F32 a00 = f32(m[0]), a10 = f32(m[4]), a20 = f32(m[8]), a30 = f32(m[12]), a01 = f32(m[1]), a11 = f32(m[5]), a21 = f32(m[9]), a31 = f32(m[13]),
              a02 = f32(m[2]), a12 = f32(m[6]), a22 = f32(m[10]), a32 = f32(m[14]), a03 = f32(m[3]), a13 = f32(m[7]), a23 = f32(m[11]), a33 = f32(m[15]);

    skvm::F32 b00 = a00 * *a11 - a01 * *a10, b01 = a00 * *a12 - a02 * *a10, b02 = a00 * *a13 - a03 * *a10, b03 = a01 * *a12 - a02 * *a11,
...
Value SkVMGenerator::writeChildCall(const ChildCall& c)
{
    ...
    case Type::TypeKind::kColorFilter: {
        SkASSERT(c.arguments().size() == 1);
        SkASSERT(arg->type().matches(*fProgram.fContext->fTypes.fHalf4) || arg->type().matches(*fProgram.fContext->fTypes.fFloat4));
...
Value SkVMGenerator::writeIntrinsicCall(const FunctionCall& c)
{
    ...
    auto binary = [&](auto&& fn) {
        ...
        for (size_t i = 0; i < nslots; ++i) {
            result[i] = fn({ fBuilder, args[0][args[0].slots() == 1 ? 0 : i] }, { fBuilder, args[1][args[1].slots() == 1 ? 0 : i] });
        }
        return result;
    };
    ...
    case k_sin_IntrinsicKind:
        return unary(args[0], skvm::approx_sin);
    case k_cos_IntrinsicKind:
        return unary(args[0], skvm::approx_cos);
    case k_tan_IntrinsicKind:
        return unary(args[0], skvm::approx_tan);

    case k_asin_IntrinsicKind:
        return unary(args[0], skvm::approx_asin);
    case k_acos_IntrinsicKind:
        return unary(args[0], skvm::approx_acos);
    ...
    case k_exp_IntrinsicKind:
        return unary(args[0], skvm::approx_exp);
    case k_log_IntrinsicKind:
        return unary(args[0], skvm::approx_log);
    case k_exp2_IntrinsicKind:
        return unary(args[0], skvm::approx_pow2);
    case k_log2_IntrinsicKind:
        return unary(args[0], skvm::approx_log2);

    case k_sqrt_IntrinsicKind:
        return unary(args[0], skvm::sqrt);
    case k_inversesqrt_IntrinsicKind:
        return unary(args[0], [](skvm::F32 x) { return 1.0f / skvm::sqrt(x); });

    case k_abs_IntrinsicKind:
        return unary(args[0], skvm::abs);
    case k_sign_IntrinsicKind:
        return unary(args[0], [](skvm::F32 x) { return select(x < 0, -1.0f, select(x > 0, +1.0f, 0.0f)); });
    case k_floor_IntrinsicKind:
        return unary(args[0], skvm::floor);
    case k_ceil_IntrinsicKind:
        return unary(args[0], skvm::ceil);
    case k_fract_IntrinsicKind:
        return unary(args[0], skvm::fract);
    ...
    case k_clamp_IntrinsicKind:
        return ternary([](skvm::F32 x, skvm::F32 lo, skvm::F32 hi) { return skvm::clamp(x, lo, hi); });
    ...
    case k_length_IntrinsicKind:
        return skvm::sqrt(dot(args[0], args[0]));
    ...
    case k_cross_IntrinsicKind: {
        skvm::F32 ax = f32(args[0][0]), ay = f32(args[0][1]), az = f32(args[0][2]), bx = f32(args[1][0]), by = f32(args[1][1]), bz = f32(args[1][2]);
        Value result(3);
    ...
    case k_refract_IntrinsicKind: {
        ...
        return binary([&](skvm::F32 i, skvm::F32 n) { return select(k < 0, 0.0f, eta * *i - (eta * *dotNI + sqrt(k)) * *n); });
    }
    ...
    case k_inverse_IntrinsicKind: {
        switch (args[0].slots()) {
        case 4:
            return this->writeMatrixInverse2x2(args[0]);
        case 9:
            return this->writeMatrixInverse3x3(args[0]);
        case 16:
            return this->writeMatrixInverse4x4(args[0]);
        default:
    ...
    case k_lessThan_IntrinsicKind:
        return nk == Type::NumberKind::kFloat ? binary([](skvm::F32 x, skvm::F32 y) { return x < y; }) : binary([](skvm::I32 x, skvm::I32 y) { return x < y; });
    case k_lessThanEqual_IntrinsicKind:
        return nk == Type::NumberKind::kFloat ? binary([](skvm::F32 x, skvm::F32 y) { return x <= y; })
    ...
    case k_not_IntrinsicKind:
        return unary(args[0], [](skvm::I32 x) { return ~x; });

    case k_toLinearSrgb_IntrinsicKind: {
        skvm::Color color = { f32(args[0][0]), f32(args[0][1]), f32(args[0][2]), fBuilder->splat(1.0f) };
...
{
    SkASSERTF(rhs.slots() == lhs.type().slotCount(), "lhs=%s (%s)\nrhs=%zu slot", lhs.type().description().c_str(), lhs.description().c_str(), rhs.slots());
...
void SkVMGenerator::writeForStatement(const ForStatement& f)
{
    ...
    if (loop.fCount > 0) {
        ...
        for (int i = 0; i < loop.fCount; ++i) {
            this->writeToSlot(
                indexSlot, (indexKind == Type::NumberKind::kFloat) ? fBuilder->splat(static_cast<float>(val)).id : fBuilder->splat(static_cast<int>(val)).id);
...
void SkVMGenerator::writeReturnStatement(const ReturnStatement& r)
{
    ...
    if (r.expression()) {
        ...
        for (size_t i = 0; i < nslots; ++i) {
            ...
            skvm::Val conditionalStore = this->writeConditionalStore(fSlots[slot + i].val, val[i], returnsHere);
...
void SkVMGenerator::writeSwitchStatement(const SwitchStatement& s)
{
    ...
    for (const std::unique_ptr<Statement>& stmt : s.cases()) {
        ...
        if (!c.isDefault()) {
            ...
            ScopedCondition conditionalCaseBlock(this, switchFallthrough | (i32(caseValue) == i32(switchValue)));
            ...
            switchFallthrough.id = this->writeConditionalStore(switchFallthrough.id, trueValue.id, this->mask());
        } else {
...
skvm::Color ProgramToSkVM(const Program& program, const FunctionDefinition& function, skvm::Builder* builder, SkVMDebugTrace* debugTrace,
    SkSpan<skvm::Val> uniforms, skvm::Coord device, skvm::Coord local, skvm::Color inputColor, skvm::Color destColor, SkVMCallbacks* callbacks)
{
    ...
    return skvm::Color { { builder, result[0] }, { builder, result[1] }, { builder, result[2] }, { builder, result[3] } };
}

bool ProgramToSkVM(const Program& program, const FunctionDefinition& function, skvm::Builder* b, SkVMDebugTrace* debugTrace, SkSpan<skvm::Val> uniforms,
...
const FunctionDefinition* Program_GetFunction(const Program& program, const char* function)
{
    for (const ProgramElement* e : program.elements()) {
        if (e->is<FunctionDefinition>() && e->as<FunctionDefinition>().declaration().name() == function) {
...
static void gather_uniforms(UniformInfo* info, const Type& type, const std::string& name)
{
    ...
    case Type::TypeKind::kVector:
    case Type::TypeKind::kMatrix:
        info->fUniforms.push_back({ name, base_number_kind(type), type.rows(), type.columns(), info->fUniformSlotCount });
 ...
 */
bool testingOnly_ProgramToSkVMShader(const Program& program, skvm::Builder* builder, SkVMDebugTrace* debugTrace)
{
    ...
    skvm::Color result = SkSL::ProgramToSkVM(program, *main, builder, debugTrace, SkSpan(uniformVals), device, local, inColor, destColor, &callbacks);
