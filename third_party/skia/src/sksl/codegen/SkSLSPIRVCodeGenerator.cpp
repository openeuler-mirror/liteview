...
#undef PACK
    ...
    case k_makeSampler2D_IntrinsicKind:
        return SPECIAL(SampledImage);
...
SPIRVCodeGenerator::Instruction SPIRVCodeGenerator::BuildInstructionKey(SpvOp_ opCode, const SkTArray<Word>& words)
{
...
SpvId SPIRVCodeGenerator::writeInstruction(SpvOp_ opCode, const SkTArray<Word>& words, OutputStream& out)
{
...
void SPIRVCodeGenerator::writeOpStore(SpvStorageClass_ storageClass, SpvId pointer, SpvId value, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeOpConstantComposite(const Type& type, const SkTArray<SpvId>& values)
{
...
SpvId SPIRVCodeGenerator::writeOpCompositeConstruct(const Type& type, const SkTArray<SpvId>& values, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::getType(const Type& rawType, const MemoryLayout& layout)
{
    ...
    case Type::TypeKind::kArray: {
        ...
        SpvId result = this->writeInstruction(SpvOpTypeArray, Words { Word::Result(), typeId, countId }, fConstantBuffer);
        this->writeInstruction(SpvOpDecorate, { result, SpvDecorationArrayStride, Word::Number(layout.stride(*type)) }, fDecorationBuffer);
        return result;
    }
    ...
    case Type::TypeKind::kTexture: {
        ...
        return this->writeInstruction(SpvOpTypeImage,
            Words { Word::Result(), floatTypeId, Word::Number(type->dimensions()), Word::Number(type->isDepth()), Word::Number(type->isArrayedTexture()),
                Word::Number(type->isMultisampled()), Word::Number(sampled), SpvImageFormatUnknown },
            fConstantBuffer);
    }
    default: {
...
SpvId SPIRVCodeGenerator::getPointerType(const Type& type, const MemoryLayout& layout, SpvStorageClass_ storageClass)
{
    return this->writeInstruction(SpvOpTypePointer, Words { Word::Result(), Word::Number(storageClass), this->getType(type) }, fConstantBuffer);
}
...
SpvId SPIRVCodeGenerator::vectorize(const Expression& arg, int vectorSize, OutputStream& out)
{
    ...
    if (argType.isScalar() && vectorSize > 1) {
        ConstructorSplat splat { arg.fPosition, argType.toCompound(fContext, vectorSize, /*rows=*/1), arg.clone() };
...
void SPIRVCodeGenerator::writeGLSLExtendedInstruction(
    const Type& type, SpvId id, SpvId floatInst, SpvId signedInst, SpvId unsignedInst, const SkTArray<SpvId>& args, OutputStream& out)
{
...
SpvId SPIRVCodeGenerator::writeSpecialIntrinsic(const FunctionCall& c, SpecialIntrinsic kind, OutputStream& out)
{
    ...
    case kTextureGrad_SpecialIntrinsic: {
        ...
        this->writeInstruction(
            op, type, result, sampler, uv, SpvImageOperandsGradMask, this->writeExpression(*arguments[2], out), this->writeExpression(*arguments[3], out), out);
        break;
    }
    ...
    case kDFdy_SpecialIntrinsic: {
        ...
        if (!fProgram.fConfig->fSettings.fForceNoRTFlip) {
            ...
            using namespace dsl;
            DSLExpression rtFlip(ThreadContext::Compiler().convertIdentifier(Position(), SKSL_RTFLIP_NAME));
...
SpvId SPIRVCodeGenerator::writeFunctionCallArgument(const FunctionCall& call, int argIndex, std::vector<TempVar>* tempVars, OutputStream& out)
{
    ...
    this->writeInstruction(SpvOpVariable, this->getPointerType(arg.type(), SpvStorageClassFunction), tmpVar, SpvStorageClassFunction, fVariableBuffer);
    if (tmpValueId != NA) {
...
void SPIRVCodeGenerator::addColumnEntry(
    const Type& columnType, SkTArray<SpvId>* currentColumn, SkTArray<SpvId>* columnIds, int rows, SpvId entry, OutputStream& out)
{
...
static SpvStorageClass_ get_storage_class(const Variable& var, SpvStorageClass_ fallbackStorageClass)
{
...
public:
    PointerLValue(
        SPIRVCodeGenerator& gen, SpvId pointer, bool isMemoryObject, SpvId type, SPIRVCodeGenerator::Precision precision, SpvStorageClass_ storageClass)
        : fGen(gen)
        , fPointer(pointer)
...
public:
    SwizzleLValue(SPIRVCodeGenerator& gen, SpvId vecPointer, const ComponentArray& components, const Type& baseType, const Type& swizzleType,
...
std::unique_ptr<SPIRVCodeGenerator::LValue> SPIRVCodeGenerator::getLValue(const Expression& expr, OutputStream& out)
{
    ...
    case Expression::Kind::kFieldAccess: {
        ...
        return std::make_unique<PointerLValue>(*this, member, /*isMemoryObjectPointer=*/false, this->getType(type), precision, storageClass);
    }
    case Expression::Kind::kSwizzle: {
    ...
    default: {
        ...
        return std::make_unique<PointerLValue>(*this, result, /*isMemoryObjectPointer=*/true, this->getType(type), precision, SpvStorageClassFunction);
    }
    }
}
    ...
    case SK_FRAGCOORD_BUILTIN: {
        ...
        if (!symbols.find(DEVICE_COORDS_NAME)) {
            ...
            auto coordsVar = std::make_unique<Variable>(/*pos=*/Position(),
                /*modifiersPosition=*/Position(), fContext.fModifiersPool->add(modifiers), DEVICE_COORDS_NAME, fContext.fTypes.fFloat4.get(),
        ...
        return this->writeExpression(
            *dsl::Float4(deviceCoord.x(), std::move(x) + std::move(y) * deviceCoord.y(), deviceCoord.z(), deviceCoord.w()).release(), out);
    }
    case SK_CLOCKWISE_BUILTIN: {
        ...
        if (!symbols.find(DEVICE_CLOCKWISE_NAME)) {
            ...
            auto clockwiseVar = std::make_unique<Variable>(/*pos=*/Position(),
                /*modifiersPosition=*/Position(), fContext.fModifiersPool->add(modifiers), DEVICE_CLOCKWISE_NAME, fContext.fTypes.fBool.get(),
                /*builtin=*/true, Variable::Storage::kGlobal);
        ...
        return this->writeExpression(*dsl::Bool(Select(rtFlip.y() > 0, !deviceClockwise, deviceClockwise)).release(), out);
    }
    default:
...
SpvId SPIRVCodeGenerator::writeScalarToMatrixSplat(const Type& matrixType, SpvId scalarId, OutputStream& out)
{
    ...
    const Type& leftType, SpvId lhs, Operator op, const Type& rightType, SpvId rhs, const Type& resultType, OutputStream& out)
{
    ...
    case Operator::Kind::PLUS:
        ...
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
    case Operator::Kind::MINUS:
        if (leftType.isMatrix() && rightType.isMatrix()) {
            SkASSERT(leftType.matches(rightType));
            return this->writeComponentwiseMatrixBinary(leftType, lhs, rhs, SpvOpFSub, out);
        }
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
    case Operator::Kind::STAR:
        if (leftType.isMatrix() && rightType.isMatrix()) {
            // matrix multiply
            SpvId result = this->nextId(&resultType);
    ...
    case Operator::Kind::SLASH:
        ...
        return this->writeBinaryOperation(resultType, *operandType, lhs, rhs, SpvOpFDiv, SpvOpSDiv, SpvOpUDiv, SpvOpUndef, out);
...
SpvId SPIRVCodeGenerator::writeTernaryExpression(const TernaryExpression& t, OutputStream& out)
{
    ...
    this->writeInstruction(SpvOpVariable, this->getPointerType(type, SpvStorageClassFunction), var, SpvStorageClassFunction, fVariableBuffer);
...
SpvId SPIRVCodeGenerator::writePrefixExpression(const PrefixExpression& p, OutputStream& out)
{
    ...
    case Operator::Kind::PLUSPLUS: {
        ...
        SpvId one = this->writeLiteral(1.0, type);
        SpvId result = this->writeBinaryOperation(type, type, lv->load(out), one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
        lv->store(result, out);
        return result;
    }
    case Operator::Kind::MINUSMINUS: {
        std::unique_ptr<LValue> lv = this->getLValue(*p.operand(), out);
        SpvId one = this->writeLiteral(1.0, type);
        SpvId result = this->writeBinaryOperation(type, type, lv->load(out), one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
        lv->store(result, out);
        return result;
    }
    ...
    default:
        SkDEBUGFAILF("unsupported prefix expression: %s", p.description(OperatorPrecedence::kTopLevel).c_str());
        return NA;
    }
}
...
SpvId SPIRVCodeGenerator::writePostfixExpression(const PostfixExpression& p, OutputStream& out)
{
    ...
    switch (p.getOperator().kind()) {
    case Operator::Kind::PLUSPLUS: {
        SpvId temp = this->writeBinaryOperation(type, type, result, one, SpvOpFAdd, SpvOpIAdd, SpvOpIAdd, SpvOpUndef, out);
        lv->store(temp, out);
        return result;
    }
    case Operator::Kind::MINUSMINUS: {
        SpvId temp = this->writeBinaryOperation(type, type, result, one, SpvOpFSub, SpvOpISub, SpvOpISub, SpvOpUndef, out);
        lv->store(temp, out);
        return result;
    }
    default:
        SkDEBUGFAILF("unsupported postfix expression %s", p.description(OperatorPrecedence::kTopLevel).c_str());
        return NA;
    }
}
...
SpvId SPIRVCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf, bool appendRTFlip)
{
    ...
    if (fProgram.fInputs.fUseFlipRTUniform && appendRTFlip && type.isStruct()) {
        ...
        {
            AutoAttachPoolToThread attach(fProgram.fPool.get());
            const Type* rtFlipStructType = fProgram.fSymbols->takeOwnershipOfSymbol(Type::MakeStructType(type.fPosition, type.name(), std::move(fields),
                /*interfaceBlock=*/true));
            const Variable* modifiedVar = fProgram.fSymbols->takeOwnershipOfSymbol(std::make_unique<Variable>(intfVar.fPosition, intfVar.modifiersPosition(),
                &intfVar.modifiers(), intfVar.name(), rtFlipStructType, intfVar.isBuiltin(), intfVar.storage()));
            fSPIRVBonusVariables.add(modifiedVar);
            InterfaceBlock modifiedCopy(intf.fPosition, *modifiedVar, intf.typeName(), intf.instanceName(), intf.arraySize(), intf.typeOwner());
            result = this->writeInterfaceBlock(modifiedCopy, false);
            fProgram.fSymbols->add(std::make_unique<Field>(Position(), modifiedVar, rtFlipStructType->fields().size() - 1));
        }
...
bool SPIRVCodeGenerator::isDead(const Variable& var) const
{
    ...
    return !(var.modifiers().fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag | Modifiers::kUniform_Flag));
}

void SPIRVCodeGenerator::writeGlobalVar(ProgramKind kind, const VarDeclaration& varDecl)
{
    const Variable& var = varDecl.var();
    if (var.modifiers().fLayout.fBuiltin == SK_FRAGCOLOR_BUILTIN && !ProgramConfig::IsFragment(kind)) {
...
SPIRVCodeGenerator::EntrypointAdapter SPIRVCodeGenerator::writeEntrypointAdapter(const FunctionDeclaration& main)
{
    ...
    auto callMainFn = std::make_unique<FunctionCall>(Position(), &main.returnType(), &main, std::move(args));
    ...
    auto entrypointBlock = Block::Make(Position(), std::move(entrypointStmts), Block::Kind::kBracedScope, symbolTable);
    // Declare an entrypoint function.
    EntrypointAdapter adapter;
    adapter.fLayout = {};
    adapter.fModifiers = Modifiers { adapter.fLayout, Modifiers::kNo_Flag };
    adapter.entrypointDecl = std::make_unique<FunctionDeclaration>(Position(), &adapter.fModifiers, "_entrypoint",
...
void SPIRVCodeGenerator::writeUniformBuffer(std::shared_ptr<SymbolTable> topLevelSymbolTable)
{
    ...
    fUniformBuffer.fInnerVariable = std::make_unique<Variable>(
        /*pos=*/Position(), /*modifiersPosition=*/Position(), fContext.fModifiersPool->add(modifiers), kUniformBufferName, fUniformBuffer.fStruct.get(),
    ...
    fUniformBuffer.fInterfaceBlock = std::make_unique<InterfaceBlock>(
        Position(), *fUniformBuffer.fInnerVariable, kUniformBufferName, kUniformBufferName, /*arraySize=*/0, topLevelSymbolTable);
...
void SPIRVCodeGenerator::addRTFlipUniform(Position pos)
{
    ...
    const Type* intfStruct = fSynthetics.takeOwnershipOfSymbol(Type::MakeStructType(Position(), name, fields, /*interfaceBlock=*/true));
    ...
    const Variable* intfVar = fSynthetics.takeOwnershipOfSymbol(std::make_unique<Variable>(/*pos=*/Position(),
    ...
    InterfaceBlock intf(Position(), *intfVar, name,
        /*instanceName=*/"",
        /*arraySize=*/0, std::make_shared<SymbolTable>(/*builtin=*/false));
...
void SPIRVCodeGenerator::writeInstructions(const Program& program, OutputStream& out)
{
    ...
    for (const ProgramElement* e : program.elements()) {
        if (e->is<InterfaceBlock>()) {
            ...
            if ((modifiers.fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag)) && modifiers.fLayout.fBuiltin == -1 && !this->isDead(intf.variable())) {
    ...
    for (const ProgramElement* e : program.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            this->writeGlobalVar(program.fConfig->fKind, e->as<GlobalVarDeclaration>().declaration()->as<VarDeclaration>());
        }
    }
    ...
    for (const auto& [var, spvId] : fVariableMap) {
        if (var->storage() == Variable::Storage::kGlobal && (var->modifiers().fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag)) && !this->isDead(*var)) {
    ...
    if (ProgramConfig::IsFragment(program.fConfig->fKind)) {
        this->writeInstruction(SpvOpExecutionMode, fFunctionMap[main], SpvExecutionModeOriginUpperLeft, out);
    }
    for (const ProgramElement* e : program.elements()) {
        if (e->is<Extension>()) {
