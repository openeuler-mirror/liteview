...
public:
    ...
    virtual void visitTexture(const Type& type, const Modifiers& modifiers, std::string_view name)
    {
    }
...
std::string MetalCodeGenerator::typeName(const Type& type)
{
    ...
    case Type::TypeKind::kArray:
        ...
        return String::printf("array<%s, %d>", this->typeName(type.componentType()).c_str(), type.columns());
...
std::string MetalCodeGenerator::getOutParamHelper(const FunctionCall& call, const ExpressionArray& arguments, const SkTArray<VariableReference*>& outVars)
{
    ...
    std::string name = "_skOutParamHelper" + std::to_string(fSwizzleHelperCount++) + "_" + function.mangledName();
...
void MetalCodeGenerator::writeConstructorCompoundMatrix(const ConstructorCompound& c, Precedence parentPrecedence)
{
    ...
    const Type& columnType = matrixType.componentType().toCompound(fContext, /*columns=*/matrixType.rows(), /*rows=*/1);
...
static bool is_input(const Variable& var)
{
    ...
    return var.modifiers().fFlags & Modifiers::kIn_Flag && (var.modifiers().fLayout.fBuiltin == -1 || var.modifiers().fLayout.fBuiltin == SK_THREADPOSITION)
...
static bool is_output(const Variable& var)
{
    ...
    return (var.modifiers().fFlags & Modifiers::kOut_Flag) && !(var.modifiers().fFlags & Modifiers::kIn_Flag) && var.modifiers().fLayout.fBuiltin == -1
        && var.type().typeKind() != Type::TypeKind::kTexture;
}
...
{
    SkASSERT(var.storage() == VariableStorage::kGlobal);
    return var.modifiers().fFlags & Modifiers::kUniform_Flag && var.type().typeKind() != Type::TypeKind::kSampler;
}
...
void MetalCodeGenerator::writeBinaryExpression(const BinaryExpression& b, Precedence parentPrecedence)
{
    ...
    bool needMatrixSplatOnScalar
        = rightType.isMatrix() && leftType.isNumber() && op.isValidForMatrixOrVector() && op.removeAssignment().kind() != Operator::Kind::STAR;
    if (needMatrixSplatOnScalar) {
        this->writeNumberAsMatrix(left, rightType);
    } else {
        this->writeExpression(left, precedence);
    }
    if (op.kind() != Operator::Kind::EQ && op.isAssignment() && left.kind() == Expression::Kind::kSwizzle && !Analysis::HasSideEffects(left)) {
    ...
    needMatrixSplatOnScalar
        = leftType.isMatrix() && rightType.isNumber() && op.isValidForMatrixOrVector() && op.removeAssignment().kind() != Operator::Kind::STAR;
    if (needMatrixSplatOnScalar) {
...
{
    return (m.fLayout.fBinding >= 0) ? m.fLayout.fBinding : fProgram.fConfig->fSettings.fDefaultUniformBinding;
}
...
{
    if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && (modifiers.fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag))) {
        this->write("device ");
...
void MetalCodeGenerator::writeInterfaceBlock(const InterfaceBlock& intf)
{
    ...
    if (intf.instanceName().size()) {
        ...
        fInterfaceBlockNameMap.set(&intf, *fProgram.fSymbols->takeOwnershipOfString("_anonInterface" + std::to_string(fAnonInterfaceCount++)));
    }
    this->writeLine(";");
}

void MetalCodeGenerator::writeFields(const std::vector<Type::Field>& fields, Position parentPos, const InterfaceBlock* parentIntf)
{
...
void MetalCodeGenerator::writeUniformStruct()
{
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            const GlobalVarDeclaration& decls = e->as<GlobalVarDeclaration>();
            const Variable& var = decls.declaration()->as<VarDeclaration>().var();
            if (var.modifiers().fFlags & Modifiers::kUniform_Flag && var.type().typeKind() != Type::TypeKind::kSampler
                && var.type().typeKind() != Type::TypeKind::kTexture) {
...
void MetalCodeGenerator::writeInputStruct()
{
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (is_input(var)) {
                this->write("    ");
                if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && needs_address_space(var.type(), var.modifiers())) {
                ...
                if (-1 != var.modifiers().fLayout.fLocation) {
                    if (ProgramConfig::IsVertex(fProgram.fConfig->fKind)) {
                        this->write("  [[attribute(" + std::to_string(var.modifiers().fLayout.fLocation) + ")]]");
                    } else if (ProgramConfig::IsFragment(fProgram.fConfig->fKind)) {
                        this->write("  [[user(locn" + std::to_string(var.modifiers().fLayout.fLocation) + ")]]");
                    }
                }
                this->write(";\n");
            }
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            ...
            if (is_output(var)) {
                ...
                this->writeType(var.type());
                if (ProgramConfig::IsCompute(fProgram.fConfig->fKind) && pass_by_reference(var.type(), var.modifiers())) {
                    this->write("&");
                }
                this->write(" ");
                this->writeName(var.mangledName());
...
void MetalCodeGenerator::visitGlobalStruct(GlobalStructVisitor* visitor)
{
    for (const ProgramElement* element : fProgram.elements()) {
        ...
        if (!(var.modifiers().fFlags & ~Modifiers::kConst_Flag) && var.modifiers().fLayout.fBuiltin == -1) {
...
void MetalCodeGenerator::writeGlobalStruct()
{
    ...
    public:
        ...
        void visitTexture(const Type& type, const Modifiers& modifiers, std::string_view name) override
        {
            this->addElement();
            fCodeGen->write("    ");
