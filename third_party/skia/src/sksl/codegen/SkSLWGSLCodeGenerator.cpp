...
std::string to_wgsl_type(const Type& type)
{
    ...
    case Type::TypeKind::kVector:
        return "vec" + std::to_string(type.columns()) + "<" + std::string(to_scalar_type(type.componentType())) + ">";
    default:
        break;
    }
...
std::string to_ptr_type(const Type& type, PtrAddressSpace addressSpace = PtrAddressSpace::kFunction)
{
...
void WGSLCodeGenerator::writePipelineIODeclaration(Modifiers modifiers, const Type& type, std::string_view name, Delimiter delimiter)
{
...
void WGSLCodeGenerator::writeUserDefinedIODecl(const Type& type, std::string_view name, int location, Delimiter delimiter)
{
...
void WGSLCodeGenerator::writeExpression(const Expression& e, Precedence parentPrecedence)
{
    ...
    case Expression::Kind::kVariableReference:
        ...
        break;
    default:
        SkDEBUGFAILF("unsupported expression (kind: %d) %s", static_cast<int>(e.kind()), e.description().c_str());
        break;
    }
}

void WGSLCodeGenerator::writeBinaryExpression(const BinaryExpression& b, Precedence parentPrecedence)
{
...
void WGSLCodeGenerator::writeFieldAccess(const FieldAccess& f)
{
    ...
    if (FieldAccess::OwnerKind::kDefault == f.ownerKind()) {
        ...
        if (v.modifiers().fFlags & Modifiers::kIn_Flag) {
            this->write("_stageIn.");
        } else if (v.modifiers().fFlags & Modifiers::kOut_Flag && field->fModifiers.fLayout.fBuiltin != SK_POINTSIZE_BUILTIN) {
            this->write("(*_stageOut).");
        } else {
...
void WGSLCodeGenerator::writeConstructorCompound(const ConstructorCompound& c, Precedence parentPrecedence)
{
...
void WGSLCodeGenerator::writeConstructorCompoundVector(const ConstructorCompound& c, Precedence parentPrecedence)
{
...
void WGSLCodeGenerator::writeStageInputStruct()
{
    ...
    for (const ProgramElement* e : fProgram.elements()) {
        if (e->is<GlobalVarDeclaration>()) {
            const Variable& v = e->as<GlobalVarDeclaration>().declaration()->as<VarDeclaration>().var();
            if (v.modifiers().fFlags & Modifiers::kIn_Flag) {
                this->writePipelineIODeclaration(v.modifiers(), v.type(), v.mangledName(), Delimiter::kComma);
    ...
    if (ProgramConfig::IsFragment(fProgram.fConfig->fKind) && fRequirements.mainNeedsCoordsArgument && !declaredFragCoordsBuiltin) {
