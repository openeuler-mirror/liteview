...
class ThreadContext {
public:
    ThreadContext(SkSL::Compiler* compiler, SkSL::ProgramKind kind, const SkSL::ProgramSettings& settings, const SkSL::BuiltinMap* module, bool isModule);
    ~ThreadContext();
     ...
     */
    static std::unique_ptr<Pool>& MemoryPool()
    {
        return Instance().fPool;
    }

    /**
     * Returns the current modifiers pool.
     */
    static std::unique_ptr<ModifiersPool>& GetModifiersPool()
    {
     ...
     */
    static const std::unique_ptr<ProgramConfig>& GetProgramConfig()
    {
    ...
    {
        return GetProgramConfig()->fIsBuiltinCode;
    }
