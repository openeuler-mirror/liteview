...
static int parse_modifier_token(Token::Kind token)
{
    ...
    case Token::Kind::TK_UNIFORM:
        return Modifiers::kUniform_Flag;
    case Token::Kind::TK_CONST:
        return Modifiers::kConst_Flag;
    case Token::Kind::TK_IN:
        return Modifiers::kIn_Flag;
    case Token::Kind::TK_OUT:
        return Modifiers::kOut_Flag;
    case Token::Kind::TK_INOUT:
        return Modifiers::kIn_Flag | Modifiers::kOut_Flag;
    case Token::Kind::TK_FLAT:
        return Modifiers::kFlat_Flag;
    case Token::Kind::TK_NOPERSPECTIVE:
        return Modifiers::kNoPerspective_Flag;
    case Token::Kind::TK_PURE:
        return Modifiers::kPure_Flag;
    case Token::Kind::TK_INLINE:
        return Modifiers::kInline_Flag;
    case Token::Kind::TK_NOINLINE:
        return Modifiers::kNoInline_Flag;
    case Token::Kind::TK_HIGHP:
        return Modifiers::kHighp_Flag;
    case Token::Kind::TK_MEDIUMP:
        return Modifiers::kMediump_Flag;
    case Token::Kind::TK_LOWP:
        return Modifiers::kLowp_Flag;
    case Token::Kind::TK_EXPORT:
        return Modifiers::kExport_Flag;
    case Token::Kind::TK_ES3:
        return Modifiers::kES3_Flag;
    case Token::Kind::TK_THREADGROUP:
        return Modifiers::kThreadgroup_Flag;
    case Token::Kind::TK_READONLY:
        return Modifiers::kReadOnly_Flag;
    case Token::Kind::TK_WRITEONLY:
        return Modifiers::kWriteOnly_Flag;
    case Token::Kind::TK_BUFFER:
        return Modifiers::kBuffer_Flag;
    default:
        return 0;
    }
}
...
Parser::Parser(Compiler* compiler, const ProgramSettings& settings, ProgramKind kind, std::string text)
    : fCompiler(*compiler)
    , fSettings(settings)
    , fKind(kind)
...
bool Parser::declaration()
{
    ...
    if (lookahead.fKind == Token::Kind::TK_IDENTIFIER && !CurrentSymbolTable()->isType(this->text(lookahead))) {
...
bool Parser::functionDeclarationEnd(Position start, DSLModifiers& modifiers, DSLType type, const Token& name)
{
    ...
    DSLFunction result(this->text(name), modifiers, type, parameterPointers, this->rangeFrom(start));
...
void Parser::globalVarDeclarationEnd(Position pos, const dsl::DSLModifiers& mods, dsl::DSLType baseType, Token name)
{
    using namespace dsl;
    ...
    DSLGlobalVar first(mods, type, this->text(name), std::move(initializer), this->rangeFrom(pos), this->position(name));
    Declare(first);
    ...
    while (this->checkNext(Token::Kind::TK_COMMA)) {
        ...
        DSLGlobalVar next(mods, type, this->text(identifierName), std::move(anotherInitializer), this->rangeFrom(identifierName));
        Declare(next);
...
/* (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)? (COMMA IDENTIFER
   (LBRACKET expression? RBRACKET)* (EQ assignmentExpression)?)* SEMICOLON */
DSLStatement Parser::localVarDeclarationEnd(Position pos, const dsl::DSLModifiers& mods, dsl::DSLType baseType, Token name)
{
    using namespace dsl;
    DSLType type = baseType;
    DSLExpression initializer;
    ...
    DSLVar first(mods, type, this->text(name), std::move(initializer), this->rangeFrom(pos), this->position(name));
    ...
    while (this->checkNext(Token::Kind::TK_COMMA)) {
        ...
        DSLVar next(mods, type, this->text(identifierName), std::move(anotherInitializer), this->rangeFrom(identifierName), this->position(identifierName));
...
DSLType Parser::structDeclaration()
{
    ...
    while (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        do {
            ...
            if (!fieldNames.contains(nameText)) {
                fields.push_back(DSLField(modifiers, std::move(actualType), nameText, this->rangeFrom(fieldStart)));
                fieldNames.add(nameText);
            } else {
                this->error(
                    memberName, "field '" + std::string(nameText) + "' was already defined in the same struct ('" + std::string(this->text(name)) + "')");
            }
        } while (this->checkNext(Token::Kind::TK_COMMA));
    ...
    if (fields.empty()) {
        this->error(this->rangeFrom(start), "struct '" + std::string(this->text(name)) + "' must contain at least one field");
    }
    return dsl::Struct(this->text(name), SkSpan(fields), this->rangeFrom(start));
}

/* structDeclaration ((IDENTIFIER varDeclarationEnd) | SEMICOLON) */
SkTArray<dsl::DSLGlobalVar> Parser::structVarDeclaration(Position start, const DSLModifiers& modifiers)
{
...
DSLModifiers Parser::modifiers()
{
    ...
    for (;;) {
        ...
        if (int duplicateFlags = (tokenFlag & flags)) {
            this->error(modifier, "'" + Modifiers::DescribeFlags(duplicateFlags) + "' appears more than once");
        }
        flags |= tokenFlag;
...
bool Parser::interfaceBlock(const dsl::DSLModifiers& modifiers)
{
    ...
    while (!this->checkNext(Token::Kind::TK_RBRACE)) {
        ...
        do {
            ...
            if (!fieldNames.contains(nameText)) {
                fields.push_back(DSLField(fieldModifiers, std::move(actualType), nameText, this->rangeFrom(fieldPos)));
                fieldNames.add(nameText);
            } else {
                this->error(fieldName,
                    "field '" + std::string(nameText) + "' was already defined in the same interface block ('" + std::string(this->text(typeName)) + "')");
            }
        } while (this->checkNext(Token::Kind::TK_COMMA));
    }
    if (fields.empty()) {
        this->error(this->rangeFrom(typeName), "interface block '" + std::string(this->text(typeName)) + "' must contain at least one member");
    }
    ...
    if (!fields.empty()) {
        dsl::InterfaceBlock(modifiers, this->text(typeName), std::move(fields), instanceName, size, this->position(typeName));
    }
    this->expect(Token::Kind::TK_SEMICOLON, "';'");
    return true;
}
...
DSLStatement Parser::ifStatement()
{
    ...
    if (isStatic) {
        return StaticIf(std::move(test), std::move(ifTrue), ifFalse.hasValue() ? std::move(ifFalse) : DSLStatement(), pos);
    } else {
        return If(std::move(test), std::move(ifTrue), ifFalse.hasValue() ? std::move(ifFalse) : DSLStatement(), pos);
    }
}
...
dsl::DSLStatement Parser::forStatement()
{
    ...
    return For(initializer.hasValue() ? std::move(initializer) : DSLStatement(), test.hasValue() ? std::move(test) : DSLExpression(),
        next.hasValue() ? std::move(next) : DSLExpression(), std::move(statement), this->rangeFrom(start),
        ForLoopPositions { range_of_at_least_one_char(lparen.fOffset + 1, firstSemicolonOffset),
...
DSLStatement Parser::returnStatement()
{
    ...
    return Return(expression.hasValue() ? std::move(expression) : DSLExpression(), this->rangeFrom(start));
}
...
bool Parser::operatorRight(Parser::AutoDepth& depth, Operator::Kind op, BinaryParseFn rightFn, DSLExpression& result)
{
    this->nextToken();
    if (!depth.increase()) {
        return false;
...
DSLExpression Parser::assignmentExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_EQ:
            if (!operatorRight(depth, Operator::Kind::EQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_STAREQ:
            if (!operatorRight(depth, Operator::Kind::STAREQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_SLASHEQ:
            if (!operatorRight(depth, Operator::Kind::SLASHEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_PERCENTEQ:
            if (!operatorRight(depth, Operator::Kind::PERCENTEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_PLUSEQ:
            if (!operatorRight(depth, Operator::Kind::PLUSEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_MINUSEQ:
            if (!operatorRight(depth, Operator::Kind::MINUSEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_SHLEQ:
            if (!operatorRight(depth, Operator::Kind::SHLEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_SHREQ:
            if (!operatorRight(depth, Operator::Kind::SHREQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_BITWISEANDEQ:
            if (!operatorRight(depth, Operator::Kind::BITWISEANDEQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_BITWISEXOREQ:
            if (!operatorRight(depth, Operator::Kind::BITWISEXOREQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_BITWISEOREQ:
            if (!operatorRight(depth, Operator::Kind::BITWISEOREQ, &Parser::assignmentExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::logicalOrExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALOR) {
        if (!operatorRight(depth, Operator::Kind::LOGICALOR, &Parser::logicalXorExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::logicalXorExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALXOR) {
        if (!operatorRight(depth, Operator::Kind::LOGICALXOR, &Parser::logicalAndExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::logicalAndExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_LOGICALAND) {
        if (!operatorRight(depth, Operator::Kind::LOGICALAND, &Parser::bitwiseOrExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::bitwiseOrExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEOR) {
        if (!operatorRight(depth, Operator::Kind::BITWISEOR, &Parser::bitwiseXorExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::bitwiseXorExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEXOR) {
        if (!operatorRight(depth, Operator::Kind::BITWISEXOR, &Parser::bitwiseAndExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::bitwiseAndExpression()
{
    ...
    while (this->peek().fKind == Token::Kind::TK_BITWISEAND) {
        if (!operatorRight(depth, Operator::Kind::BITWISEAND, &Parser::equalityExpression, result)) {
            return {};
        }
    }
    return result;
...
DSLExpression Parser::equalityExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_EQEQ:
            if (!operatorRight(depth, Operator::Kind::EQEQ, &Parser::relationalExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_NEQ:
            if (!operatorRight(depth, Operator::Kind::NEQ, &Parser::relationalExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::relationalExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_LT:
            if (!operatorRight(depth, Operator::Kind::LT, &Parser::shiftExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_GT:
            if (!operatorRight(depth, Operator::Kind::GT, &Parser::shiftExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_LTEQ:
            if (!operatorRight(depth, Operator::Kind::LTEQ, &Parser::shiftExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_GTEQ:
            if (!operatorRight(depth, Operator::Kind::GTEQ, &Parser::shiftExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::shiftExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_SHL:
            if (!operatorRight(depth, Operator::Kind::SHL, &Parser::additiveExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_SHR:
            if (!operatorRight(depth, Operator::Kind::SHR, &Parser::additiveExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::additiveExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_PLUS:
            if (!operatorRight(depth, Operator::Kind::PLUS, &Parser::multiplicativeExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_MINUS:
            if (!operatorRight(depth, Operator::Kind::MINUS, &Parser::multiplicativeExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::multiplicativeExpression()
{
    ...
    for (;;) {
        ...
        case Token::Kind::TK_STAR:
            if (!operatorRight(depth, Operator::Kind::STAR, &Parser::unaryExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_SLASH:
            if (!operatorRight(depth, Operator::Kind::SLASH, &Parser::unaryExpression, result)) {
                return {};
            }
            break;
        case Token::Kind::TK_PERCENT:
            if (!operatorRight(depth, Operator::Kind::PERCENT, &Parser::unaryExpression, result)) {
                return {};
            }
            break;
        default:
...
DSLExpression Parser::unaryExpression()
{
    ...
    case Token::Kind::TK_MINUSMINUS: {
        ...
        switch (start.fKind) {
        case Token::Kind::TK_PLUS:
            return expr.prefix(Operator::Kind::PLUS, p);
        case Token::Kind::TK_MINUS:
            return expr.prefix(Operator::Kind::MINUS, p);
        case Token::Kind::TK_LOGICALNOT:
            return expr.prefix(Operator::Kind::LOGICALNOT, p);
        case Token::Kind::TK_BITWISENOT:
            return expr.prefix(Operator::Kind::BITWISENOT, p);
        case Token::Kind::TK_PLUSPLUS:
            return expr.prefix(Operator::Kind::PLUSPLUS, p);
        case Token::Kind::TK_MINUSMINUS:
            return expr.prefix(Operator::Kind::MINUSMINUS, p);
        default:
            SkUNREACHABLE;
        }
    }
...
DSLExpression Parser::swizzle(Position pos, DSLExpression base, std::string_view swizzleMask, Position maskPos)
{
    ...
    for (int i = 0; i < length; ++i) {
        if (i >= 4) {
            Position errorPos = maskPos.valid() ? Position::Range(maskPos.startOffset() + 4, maskPos.endOffset()) : pos;
            this->error(errorPos, "too many components in swizzle mask");
            return DSLExpression::Poison(pos);
        }
        switch (swizzleMask[i]) {
        case '0':
            components[i] = SwizzleComponent::ZERO;
            break;
        case '1':
            components[i] = SwizzleComponent::ONE;
            break;
        case 'r':
            components[i] = SwizzleComponent::R;
            break;
        case 'x':
            components[i] = SwizzleComponent::X;
            break;
        case 's':
            components[i] = SwizzleComponent::S;
            break;
        case 'L':
            components[i] = SwizzleComponent::UL;
            break;
        case 'g':
            components[i] = SwizzleComponent::G;
            break;
        case 'y':
            components[i] = SwizzleComponent::Y;
            break;
        case 't':
            components[i] = SwizzleComponent::T;
            break;
        case 'T':
            components[i] = SwizzleComponent::UT;
            break;
        case 'b':
            components[i] = SwizzleComponent::B;
            break;
        case 'z':
            components[i] = SwizzleComponent::Z;
            break;
        case 'p':
            components[i] = SwizzleComponent::P;
            break;
        case 'R':
            components[i] = SwizzleComponent::UR;
            break;
        case 'a':
            components[i] = SwizzleComponent::A;
            break;
        case 'w':
            components[i] = SwizzleComponent::W;
            break;
        case 'q':
            components[i] = SwizzleComponent::Q;
            break;
        case 'B':
            components[i] = SwizzleComponent::UB;
            break;
        default: {
            Position componentPos = Position::Range(maskPos.startOffset() + i, maskPos.startOffset() + i + 1);
            this->error(componentPos, String::printf("invalid swizzle component '%c'", swizzleMask[i]).c_str());
            return DSLExpression::Poison(pos);
        }
        }
    }
    switch (length) {
    case 1:
        return dsl::Swizzle(std::move(base), components[0], pos, maskPos);
    case 2:
        return dsl::Swizzle(std::move(base), components[0], components[1], pos, maskPos);
    case 3:
        return dsl::Swizzle(std::move(base), components[0], components[1], components[2], pos, maskPos);
    case 4:
        return dsl::Swizzle(std::move(base), components[0], components[1], components[2], components[3], pos, maskPos);
    default:
        SkUNREACHABLE;
    }
}
