    ...
    , fCaps(caps)
{
    ...
    fContext = std::make_shared<Context>(moduleLoader.builtinTypes(), /*caps=*/nullptr, fErrorReporter);
}
...
LoadedModule Compiler::compileModule(
    ProgramKind kind, const char* moduleName, std::string moduleSource, const BuiltinMap* base, ModifiersPool& modifiersPool, bool shouldInline)
{
    SkASSERT(base);
...
std::unique_ptr<Program> Compiler::convertProgram(ProgramKind kind, std::string text, ProgramSettings settings)
{
...
std::unique_ptr<Expression> Compiler::convertIdentifier(Position pos, std::string_view name)
{
    ...
    case Symbol::Kind::kFunctionDeclaration: {
        return std::make_unique<FunctionReference>(*fContext, pos, &result->as<FunctionDeclaration>());
    }
    ...
    case Symbol::Kind::kField: {
        ...
        return FieldAccess::Make(*fContext, pos, std::move(base), field->fieldIndex(), FieldAccess::OwnerKind::kAnonymousInterfaceBlock);
    }
...
bool Compiler::optimizeModuleBeforeMinifying(ProgramKind kind, LoadedModule& module, const BuiltinMap* base)
{
    SkASSERT(this->errorCount() == 0);
...
bool Compiler::optimizeModuleAfterLoading(ProgramKind kind, LoadedModule& module, const BuiltinMap* base)
{
    SkASSERT(this->errorCount() == 0);
...
bool Compiler::runInliner(
    Inliner* inliner, const std::vector<std::unique_ptr<ProgramElement>>& elements, std::shared_ptr<SymbolTable> symbols, ProgramUsage* usage)
{
...
static bool validate_spirv(ErrorReporter& reporter, std::string_view program)
{
    ...
    std::string errors;
    auto msgFn
        = [&errors](spv_message_level_t, const char*, const spv_position_t&, const char* m) { String::appendf(&errors, "SPIR-V validation error: %s\n", m); };
