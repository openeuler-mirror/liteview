...
static std::unique_ptr<BuiltinMap> compile_to_builtin_map(
    SkSL::Compiler* compiler, ProgramKind kind, const char* moduleName, std::string moduleSource, const BuiltinMap* parent, ModifiersPool& modifiersPool)
{
    return compiler->compileModule(kind, moduleName, std::move(moduleSource), parent, modifiersPool, /*shouldInline=*/true).convertToBuiltinMap(parent);
}
...
std::shared_ptr<SymbolTable>& ModuleLoader::rootSymbolTableWithPublicTypes()
{
    if (!fModuleLoader.fRootSymbolTableWithPublicTypes) {
        fModuleLoader.fRootSymbolTableWithPublicTypes = std::make_shared<SymbolTable>(this->rootModule()->symbols(), /*builtin=*/true);
        add_public_type_aliases(fModuleLoader.fRootSymbolTableWithPublicTypes.get(), this->builtinTypes());
    }
...
const BuiltinMap* ModuleLoader::loadPublicModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fPublicModule) {
        ...
        fModuleLoader.fPublicModule = compile_to_builtin_map(compiler, ProgramKind::kGeneric, MODULE_DATA(sksl_public), sharedModule, this->coreModifiers());
...
const BuiltinMap* ModuleLoader::loadPrivateRTShaderModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fRuntimeShaderModule) {
        ...
        fModuleLoader.fRuntimeShaderModule
            = compile_to_builtin_map(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_rt_shader), publicModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadSharedModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fSharedModule) {
        ...
        fModuleLoader.fSharedModule = compile_to_builtin_map(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_shared), rootModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadGPUModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fGPUModule) {
        const BuiltinMap* sharedModule = this->loadSharedModule(compiler);
        fModuleLoader.fGPUModule = compile_to_builtin_map(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_gpu), sharedModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadFragmentModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fFragmentModule) {
        ...
        fModuleLoader.fFragmentModule = compile_to_builtin_map(compiler, ProgramKind::kFragment, MODULE_DATA(sksl_frag), gpuModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadVertexModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fVertexModule) {
        const BuiltinMap* gpuModule = this->loadGPUModule(compiler);
        fModuleLoader.fVertexModule = compile_to_builtin_map(compiler, ProgramKind::kVertex, MODULE_DATA(sksl_vert), gpuModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadComputeModule(SkSL::Compiler* compiler)
{
    if (!fModuleLoader.fComputeModule) {
        const BuiltinMap* gpuModule = this->loadGPUModule(compiler);
        fModuleLoader.fComputeModule = compile_to_builtin_map(compiler, ProgramKind::kCompute, MODULE_DATA(sksl_compute), gpuModule, this->coreModifiers());
        add_compute_type_aliases(fModuleLoader.fComputeModule->symbols().get(), this->builtinTypes());
    }
...
const BuiltinMap* ModuleLoader::loadGraphiteFragmentModule(SkSL::Compiler* compiler)
{
    ...
    if (!fModuleLoader.fGraphiteFragmentModule) {
        ...
        fModuleLoader.fGraphiteFragmentModule
            = compile_to_builtin_map(compiler, ProgramKind::kGraphiteFragment, MODULE_DATA(sksl_graphite_frag), fragmentModule, this->coreModifiers());
    }
...
const BuiltinMap* ModuleLoader::loadGraphiteVertexModule(SkSL::Compiler* compiler)
{
    ...
    if (!fModuleLoader.fGraphiteVertexModule) {
        ...
        fModuleLoader.fGraphiteVertexModule
            = compile_to_builtin_map(compiler, ProgramKind::kGraphiteVertex, MODULE_DATA(sksl_graphite_vert), vertexModule, this->coreModifiers());
    }
...
void ModuleLoader::Impl::makeRootSymbolTable()
{
    ...
    symbols->add(std::make_unique<Variable>(/*pos=*/Position(),
        /*modifiersPosition=*/Position(), fCoreModifiers.add(Modifiers {}), "sk_Caps", fBuiltinTypes.fSkCaps.get(),
        /*builtin=*/false, Variable::Storage::kGlobal));
    fRootModule = std::make_unique<BuiltinMap>(/*parent=*/nullptr, std::move(symbols),
