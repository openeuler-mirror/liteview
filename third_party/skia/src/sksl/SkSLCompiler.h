...
public:
     ...
     * alive.
     */
    std::unique_ptr<Program> convertProgram(ProgramKind kind, std::string text, ProgramSettings settings);
    ...
    bool optimizeModuleBeforeMinifying(ProgramKind kind, LoadedModule& module, const BuiltinMap* base);
...
private:
    ...
    bool runInliner(Inliner* inliner, const std::vector<std::unique_ptr<ProgramElement>>& elements, std::shared_ptr<SymbolTable> symbols, ProgramUsage* usage);
