...
static std::unique_ptr<Expression> assemble_compound(const Context& context, Position pos, const Type& returnType, double value[])
{
    ...
    const Context& context, Position pos, IntrinsicKind intrinsic, const ExpressionArray& argArray, const Type& returnType)
{
    ...
    case k_unpackUnorm2x16_IntrinsicKind: {
        ...
        return Float2(double(a) / 65535.0, double(b) / 65535.0).release();
    }
    case k_unpackSnorm2x16_IntrinsicKind: {
        SKSL_INT x = *arguments[0]->getConstantValue(0);
        int16_t a = ((x >> 0) & 0x0000FFFF);
        int16_t b = ((x >> 16) & 0x0000FFFF);
        return Float2(Intrinsics::evaluate_clamp(double(a) / 32767.0, -1.0, 1.0), Intrinsics::evaluate_clamp(double(b) / 32767.0, -1.0, 1.0)).release();
    }
    case k_unpackHalf2x16_IntrinsicKind: {
        SKSL_INT x = *arguments[0]->getConstantValue(0);
        uint16_t a = ((x >> 0) & 0x0000FFFF);
        uint16_t b = ((x >> 16) & 0x0000FFFF);
        return Float2(SkHalfToFloat(a), SkHalfToFloat(b)).release();
    }
    // 8.5 : Geometric Functions
    case k_length_IntrinsicKind:
        return coalesce_vector<float>(arguments, /*startingState=*/0, returnType, Intrinsics::coalesce_length, Intrinsics::finalize_length);
    case k_distance_IntrinsicKind:
        return coalesce_pairwise_vectors<float>(arguments, /*startingState=*/0, returnType, Intrinsics::coalesce_distance, Intrinsics::finalize_distance);
    case k_dot_IntrinsicKind:
        return coalesce_pairwise_vectors<float>(arguments, /*startingState=*/0, returnType, Intrinsics::coalesce_dot,
            /*finalize=*/nullptr);
    ...
    case k_refract_IntrinsicKind: {
        ...
        return ((kValue < 0) ? (0 * I()) : (Eta() * I() - (Eta() * Dot(N(), I()) + std::sqrt(kValue)) * N())).release();
    }
...
{
    return std::make_unique<FunctionCall>(pos, &this->type(), &this->function(), this->arguments().clone());
}
...
std::unique_ptr<Expression> FunctionCall::Convert(const Context& context, Position pos, std::unique_ptr<Expression> functionValue, ExpressionArray arguments)
{
    ...
    case Expression::Kind::kExternalFunctionReference: {
        ...
        if (count != (int)arguments.size()) {
            context.fErrors->error(pos, "external function expected " + std::to_string(count) + " arguments, but found " + std::to_string(arguments.size()));
            return nullptr;
        }
...
std::unique_ptr<Expression> FunctionCall::Convert(const Context& context, Position pos, const FunctionDeclaration& function, ExpressionArray arguments)
{
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
        ...
        if (paramModifiers.fFlags & Modifiers::kOut_Flag) {
            const VariableRefKind refKind
                = paramModifiers.fFlags & Modifiers::kIn_Flag ? VariableReference::RefKind::kReadWrite : VariableReference::RefKind::kPointer;
