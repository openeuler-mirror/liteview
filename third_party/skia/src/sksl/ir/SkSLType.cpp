...
public:
    ...
    bool isPrivate() const override
    {
        return std::any_of(fFields.begin(), fFields.end(), [](const Field& f) { return f.fType->isPrivate(); });
    }

    bool isAllowedInES2() const override
    {
        return std::all_of(fFields.begin(), fFields.end(), [](const Field& f) { return f.fType->isAllowedInES2(); });
    }
...
std::unique_ptr<Type> Type::MakeArrayType(std::string_view name, const Type& componentType, int columns)
{
    return std::make_unique<ArrayType>(std::move(name), componentType.abbreviatedName(), componentType, columns);
}
...
std::unique_ptr<Type> Type::MakeStructType(Position pos, std::string_view name, std::vector<Field> fields, bool interfaceBlock)
{
...
const Type* Type::applyQualifiers(const Context& context, Modifiers* modifiers, SymbolTable* symbols, Position pos) const
{
    const Type* type;
...
const Type* Type::applyPrecisionQualifiers(const Context& context, Modifiers* modifiers, SymbolTable* symbols, Position pos) const
{
    int precisionQualifiers = modifiers->fFlags & (Modifiers::kHighp_Flag | Modifiers::kMediump_Flag | Modifiers::kLowp_Flag);
...
const Type* Type::applyAccessQualifiers(const Context& context, Modifiers* modifiers, SymbolTable* symbols, Position pos) const
{
    int accessQualifiers = modifiers->fFlags & (Modifiers::kReadOnly_Flag | Modifiers::kWriteOnly_Flag);
    if (!accessQualifiers) {
    ...
    // We're going to return a whole new type, so the modifier bits can be cleared out.
    modifiers->fFlags &= ~(Modifiers::kReadOnly_Flag | Modifiers::kWriteOnly_Flag);
    ...
    if (this->matches(*context.fTypes.fReadWriteTexture2D)) {
        ...
        default:
            context.fErrors->error(pos,
                "'readonly' and 'writeonly' qualifiers "
                "cannot be combined");
            return this;
        }
    }

    context.fErrors->error(pos, "type '" + this->displayName() + "' does not support qualifier '" + Modifiers::DescribeFlags(accessQualifiers) + "'");
    return this;
}
...
const Type* Type::clone(SymbolTable* symbolTable) const
{
    ...
    case TypeKind::kStruct: {
        ...
        return symbolTable->add(Type::MakeStructType(this->fPosition, *name, this->fields(), this->isInterfaceBlock()));
    }
    default:
...
static bool is_or_contains_array(const Type* type, bool onlyMatchUnsizedArrays)
{
    ...
    if (type->isArray()) {
        return onlyMatchUnsizedArrays ? (type->isUnsizedArray() || is_or_contains_array(&type->componentType(), true)) : true;
    }
