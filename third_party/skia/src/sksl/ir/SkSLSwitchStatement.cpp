...
std::unique_ptr<Statement> SwitchStatement::clone() const
{
    ...
    return std::make_unique<SwitchStatement>(
        fPosition, this->isStatic(), this->value()->clone(), std::move(cases), SymbolTable::WrapIfBuiltin(this->symbols()));
}
...
static void move_all_but_break(std::unique_ptr<Statement>& stmt, StatementArray* target)
{
    ...
    case Statement::Kind::kBlock: {
        ...
        target->push_back(Block::Make(block.fPosition, std::move(blockStmts), block.blockKind(), block.symbolTable()));
        break;
    }
...
std::unique_ptr<Statement> SwitchStatement::BlockForCase(StatementArray* cases, SwitchCase* caseToCapture, std::shared_ptr<SymbolTable> symbolTable)
{
...
std::unique_ptr<Statement> SwitchStatement::Convert(const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> value,
    ExpressionArray caseValues, StatementArray caseStatements, std::shared_ptr<SymbolTable> symbolTable)
{
    ...
    return SwitchStatement::Make(context, pos, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}

std::unique_ptr<Statement> SwitchStatement::Make(
    const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> value, StatementArray cases, std::shared_ptr<SymbolTable> symbolTable)
{
    ...
    return std::make_unique<SwitchStatement>(pos, isStatic, std::move(value), std::move(cases), std::move(symbolTable));
}
