...
std::unique_ptr<Expression> FieldAccess::Convert(
    const Context& context, Position pos, SymbolTable& symbolTable, std::unique_ptr<Expression> base, std::string_view field)
{
...
{
    return this->base()->description(OperatorPrecedence::kPostfix) + "." + std::string(this->base()->type().fields()[this->fieldIndex()].fName);
}
