...
static void append_rtadjust_fixup_to_vertex_main(const Context& context, const FunctionDeclaration& decl, Block& body)
{
    ...
    if (rtAdjust.fVar || rtAdjust.fInterfaceBlock) {
        ...
        const Field& skPositionField = ThreadContext::SymbolTable()->find(Compiler::POSITION_NAME)->as<Field>();

        auto Ref = [](const Variable* var) -> std::unique_ptr<Expression> { return VariableReference::Make(Position(), var); };
        ...
        auto Pos = [&]() -> DSLExpression { return DSLExpression(Field(&skPositionField.owner(), skPositionField.fieldIndex())); };
        auto Adjust = [&]() -> DSLExpression {
            return DSLExpression(rtAdjust.fInterfaceBlock ? Field(rtAdjust.fInterfaceBlock, rtAdjust.fFieldIndex) : Ref(rtAdjust.fVar));
        };

        auto fixupStmt
            = DSLStatement(Pos().assign(Float4(Swizzle(Pos(), X, Y) * Swizzle(Adjust(), X, Z) + Swizzle(Pos(), W, W) * Swizzle(Adjust(), Y, W), 0, Pos().w())));
...
std::unique_ptr<FunctionDefinition> FunctionDefinition::Convert(
    const Context& context, Position pos, const FunctionDeclaration& function, std::unique_ptr<Statement> body, bool builtin)
{
        ...
        bool visitStatement(Statement& stmt) override
        {
            ...
            case Statement::Kind::kVarDeclaration: {
                ...
                if (var.type().isOrContainsUnsizedArray()) {
                    fContext.fErrors->error(stmt.fPosition, "unsized arrays are not permitted here");
                    break;
                }
                ...
                if (prevSlotsUsed < kVariableSlotLimit && fSlotsUsed >= kVariableSlotLimit) {
                    fContext.fErrors->error(stmt.fPosition, "variable '" + std::string(var.name()) + "' exceeds the stack size limit");
                }
                break;
            }
    ...
    SkASSERTF(!function.isIntrinsic(), "Intrinsic function '%.*s' should not have a definition", (int)function.name().size(), function.name().data());
