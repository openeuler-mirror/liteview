...
static bool check_valid_uniform_type(Position pos, const Type* t, const Context& context, bool topLevel = true)
{
    ...
    // 32-bit signed integers, 16-bit and 32-bit floats, and their composites.
    {
        ...
        if (error || (ct.isBoolean() && (t->isScalar() || t->isVector()))) {
            context.fErrors->error(pos, "variables of type '" + t->displayName() + "' may not be uniform");
            return false;
        }
    }
...
std::unique_ptr<Statement> VarDeclaration::clone() const
{
    ...
    if (ThreadContext::Settings().fAllowVarDeclarationCloneForTesting) {
        return std::make_unique<VarDeclaration>(&this->var(), &this->baseType(), fArraySize, this->value() ? this->value()->clone() : nullptr,
            /*isClone=*/true);
    } else {
...
{
    std::string result = this->var().modifiers().description() + this->baseType().description() + " " + std::string(this->var().name());
    if (this->arraySize() > 0) {
...
void VarDeclaration::ErrorCheck(
    const Context& context, Position pos, Position modifiersPosition, const Modifiers& modifiers, const Type* type, Variable::Storage storage)
{
    ...
    if ((modifiers.fFlags & Modifiers::kReadOnly_Flag) && (modifiers.fFlags & Modifiers::kWriteOnly_Flag)) {
        context.fErrors->error(pos, "'readonly' and 'writeonly' qualifiers cannot be combined");
    }
    if ((modifiers.fFlags & Modifiers::kUniform_Flag) && (modifiers.fFlags & Modifiers::kBuffer_Flag)) {
        context.fErrors->error(pos, "'uniform buffer' variables not permitted");
    }
    if ((modifiers.fFlags & Modifiers::kThreadgroup_Flag) && (modifiers.fFlags & (Modifiers::kIn_Flag | Modifiers::kOut_Flag))) {
    ...
    if (baseType->isEffectChild() && (context.fConfig->fKind == ProgramKind::kMeshVertex || context.fConfig->fKind == ProgramKind::kMeshFragment)) {
    ...
    int permitted = Modifiers::kConst_Flag | Modifiers::kHighp_Flag | Modifiers::kMediump_Flag | Modifiers::kLowp_Flag;
    if (storage == Variable::Storage::kGlobal) {
        ...
        if (baseType->isInterfaceBlock()) {
            ...
            const size_t illegalRangeEnd = fields.size() - ((modifiers.fFlags & Modifiers::kBuffer_Flag) ? 1 : 0);
    ...
    if (storage != Variable::Storage::kGlobal || ((modifiers.fFlags & Modifiers::kUniform_Flag) && !permitBindingAndSet)) {
...
bool VarDeclaration::ErrorCheckAndCoerce(const Context& context, const Variable& var, std::unique_ptr<Expression>& value)
{
    ErrorCheck(context, var.fPosition, var.modifiersPosition(), var.modifiers(), &var.type(), var.storage());
    if (value) {
...
std::unique_ptr<Statement> VarDeclaration::Convert(
    const Context& context, std::unique_ptr<Variable> var, std::unique_ptr<Expression> value, bool addToSymbolTable)
{
    ...
    std::unique_ptr<Statement> varDecl = VarDeclaration::Make(context, var.get(), baseType, arraySize, std::move(value));
    if (!varDecl) {
        return nullptr;
    }
    ...
    } else if ((var->storage() == Variable::Storage::kGlobal || var->storage() == Variable::Storage::kInterfaceBlock)
...
std::unique_ptr<Statement> VarDeclaration::Make(const Context& context, Variable* var, const Type* baseType, int arraySize, std::unique_ptr<Expression> value)
{
    SkASSERT(!baseType->isArray());
    ...
    SkASSERT(!(var->modifiers().fFlags & Modifiers::kConst_Flag) || Analysis::IsConstantExpression(*value));
