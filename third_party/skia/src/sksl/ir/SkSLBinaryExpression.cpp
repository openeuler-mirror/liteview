...
static bool is_low_precision_matrix_vector_multiply(const Expression& left, const Operator& op, const Expression& right, const Type& resultType)
{
    return !resultType.highPrecision() && op.kind() == Operator::Kind::STAR && left.type().isMatrix() && right.type().isVector()
        && left.type().rows() == right.type().columns() && Analysis::IsTrivialExpression(left) && Analysis::IsTrivialExpression(right);
}

static std::unique_ptr<Expression> rewrite_matrix_vector_multiply(
    const Context& context, Position pos, const Expression& left, const Operator& op, const Expression& right, const Type& resultType)
{
    ...
    for (int n = 0; n < left.type().rows(); ++n) {
        ...
        std::unique_ptr<Expression> matN = IndexExpression::Make(context, pos, left.clone(), Literal::MakeInt(context, left.fPosition, n));
        // Get vec[N] with a swizzle expression.
        std::unique_ptr<Expression> vecN
            = Swizzle::Make(context, left.fPosition.rangeThrough(right.fPosition), right.clone(), ComponentArray { (SkSL::SwizzleComponent::Type)n });
        // Multiply them together.
        const Type* matNType = &matN->type();
        std::unique_ptr<Expression> product = BinaryExpression::Make(context, pos, std::move(matN), op, std::move(vecN), matNType);
        // Sum all the components together.
        if (!sum) {
            sum = std::move(product);
        } else {
            sum = BinaryExpression::Make(context, pos, std::move(sum), Operator(Operator::Kind::PLUS), std::move(product), matNType);
        }
    }
    ...
    const Context& context, Position pos, std::unique_ptr<Expression> left, Operator op, std::unique_ptr<Expression> right)
{
    ...
    if (context.fConfig->strictES2Mode()) {
        ...
        if (leftType->isOrContainsArray()) {
            ...
            context.fErrors->error(pos, "operator '" + std::string(op.tightOperatorName()) + "' can not operate on arrays (or structs containing arrays)");
            return nullptr;
        }
    }
    ...
    if (context.fConfig->fSettings.fOptimize && !context.fConfig->fIsBuiltinCode) {
        ...
        if (is_low_precision_matrix_vector_multiply(*left, op, *right, *resultType)) {
            ...
            if (capsBitIsTrue || !caps->isBoolLiteral()) {
                ...
                std::unique_ptr<Expression> rewrite = rewrite_matrix_vector_multiply(context, pos, *left, op, *right, *resultType);
                ...
                return TernaryExpression::Make(context, pos, std::move(caps), std::move(rewrite),
                    std::make_unique<BinaryExpression>(pos, std::move(left), op, std::move(right), resultType));
            }
        }
    }
