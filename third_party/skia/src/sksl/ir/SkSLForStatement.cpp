...
std::unique_ptr<Statement> ForStatement::clone() const
{
    ...
    return std::make_unique<ForStatement>(fPosition, fForLoopPositions, this->initializer() ? this->initializer()->clone() : nullptr,
        this->test() ? this->test()->clone() : nullptr, this->next() ? this->next()->clone() : nullptr, this->statement()->clone(), std::move(unrollInfo),
...
std::unique_ptr<Statement> ForStatement::Convert(const Context& context, Position pos, ForLoopPositions positions, std::unique_ptr<Statement> initializer,
    std::unique_ptr<Expression> test, std::unique_ptr<Expression> next, std::unique_ptr<Statement> statement, std::shared_ptr<SymbolTable> symbolTable)
{
    ...
    if (context.fConfig->strictES2Mode()) {
        ...
        unrollInfo = Analysis::GetLoopUnrollInfo(pos, positions, initializer.get(), test.get(), next.get(), statement.get(), context.fErrors);
        if (!unrollInfo) {
            return nullptr;
        }
    } else {
        ...
        unrollInfo = Analysis::GetLoopUnrollInfo(pos, positions, initializer.get(), test.get(), next.get(), statement.get(), /*errors=*/nullptr);
    }
    ...
    if (isVardeclBlockInitializer) {
        ...
        scope.push_back(ForStatement::Make(context, pos, positions, /*initializer=*/nullptr, std::move(test), std::move(next), std::move(statement),
...
std::unique_ptr<Statement> ForStatement::ConvertWhile(
    const Context& context, Position pos, std::unique_ptr<Expression> test, std::unique_ptr<Statement> statement, std::shared_ptr<SymbolTable> symbolTable)
{
    if (context.fConfig->strictES2Mode()) {
    ...
    return ForStatement::Convert(
        context, pos, ForLoopPositions(), /*initializer=*/nullptr, std::move(test), /*next=*/nullptr, std::move(statement), std::move(symbolTable));
}
