...
public:
    ...
    Variable(Position pos, Position modifiersPosition, const Modifiers* modifiers, std::string_view name, const Type* type, bool builtin, Storage storage)
    ...
    static std::unique_ptr<Variable> Convert(const Context& context, Position pos, Position modifiersPos, const Modifiers& modifiers, const Type* baseType,
        Position namePos, std::string_view name, bool isArray, std::unique_ptr<Expression> arraySize, Variable::Storage storage);

    static std::unique_ptr<Variable> Make(const Context& context, Position pos, Position modifiersPos, const Modifiers& modifiers, const Type* baseType,
    ...
    static ScratchVariable MakeScratchVariable(const Context& context, Mangler& mangler, std::string_view baseName, const Type* type,
    ...
    {
        return this->modifiers().description() + this->type().displayName() + " " + std::string(this->name());
    }
