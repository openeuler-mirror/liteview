...
std::unique_ptr<Statement> IfStatement::clone() const
{
    return std::make_unique<IfStatement>(
        fPosition, this->isStatic(), this->test()->clone(), this->ifTrue()->clone(), this->ifFalse() ? this->ifFalse()->clone() : nullptr);
}
...
std::unique_ptr<Statement> IfStatement::Convert(const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> test,
    std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse)
{
    ...
    return IfStatement::Make(context, pos, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
...
std::unique_ptr<Statement> IfStatement::Make(const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> test,
    std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse)
{
    ...
    return std::make_unique<IfStatement>(pos, isStatic, std::move(test), std::move(ifTrue), std::move(ifFalse));
}
