...
std::unique_ptr<Expression> IndexExpression::Convert(
    const Context& context, SymbolTable& symbolTable, Position pos, std::unique_ptr<Expression> base, std::unique_ptr<Expression> index)
{
    ...
    if (base->is<TypeReference>()) {
        ...
        return TypeReference::Convert(context, pos, symbolTable.addArrayDimension(&baseType, arraySize));
    }
...
{
    return this->base()->description(OperatorPrecedence::kPostfix) + "[" + this->index()->description(OperatorPrecedence::kTopLevel) + "]";
}
