...
std::unique_ptr<Statement> Block::Make(Position pos, StatementArray statements, Kind kind, std::shared_ptr<SymbolTable> symbols)
{
...
std::unique_ptr<Block> Block::MakeBlock(Position pos, StatementArray statements, Kind kind, std::shared_ptr<SymbolTable> symbols)
{
...
std::unique_ptr<Statement> Block::clone() const
{
    ...
    return std::make_unique<Block>(fPosition, std::move(cloned), fBlockKind, SymbolTable::WrapIfBuiltin(this->symbolTable()));
}
