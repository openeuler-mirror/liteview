...
public:
    ...
    IfStatement(Position pos, bool isStatic, std::unique_ptr<Expression> test, std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse)
    ...
    static std::unique_ptr<Statement> Convert(const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> test,
        std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse);

    // Creates a potentially-simplified form of the if-statement; reports errors via ASSERT.
    static std::unique_ptr<Statement> Make(const Context& context, Position pos, bool isStatic, std::unique_ptr<Expression> test,
        std::unique_ptr<Statement> ifTrue, std::unique_ptr<Statement> ifFalse);
