...
public:
    ...
    InterfaceBlock(
        Position pos, const Variable& var, std::string_view typeName, std::string_view instanceName, int arraySize, std::shared_ptr<SymbolTable> typeOwner)
    ...
    {
        SkASSERT(fVariable.type().isInterfaceBlock() || (fVariable.type().isArray() && fVariable.type().componentType().isInterfaceBlock()));
    }
    ...
    std::unique_ptr<ProgramElement> clone() const override
    {
        return std::make_unique<InterfaceBlock>(
            fPosition, this->variable(), this->typeName(), this->instanceName(), this->arraySize(), SymbolTable::WrapIfBuiltin(this->typeOwner()));
    }

    std::string description() const override
    {
        std::string result = this->variable().modifiers().description() + std::string(this->typeName()) + " {\n";
