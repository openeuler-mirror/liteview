...
public:
    ...
    static std::unique_ptr<Type> MakeArrayType(std::string_view name, const Type& componentType, int columns);
    ...
    static std::unique_ptr<Type> MakeStructType(Position pos, std::string_view name, std::vector<Field> fields, bool interfaceBlock = false);
...
protected:
    ...
    const Type* applyPrecisionQualifiers(const Context& context, Modifiers* modifiers, SymbolTable* symbols, Position pos) const;
