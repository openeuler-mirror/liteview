 ...
 */
bool IsConstantIndexExpression(const Expression& expr, const std::set<const Variable*>* loopIndices);
 ...
 * null is returned.
 */
std::unique_ptr<LoopUnrollInfo> GetLoopUnrollInfo(Position pos, const ForLoopPositions& positions, const Statement* loopInitializer, const Expression* loopTest,
 ...
 */
SkTArray<const SkSL::Variable*> GetComputeShaderMainParams(const Context& context, const Program& program);
...
public:
    ...
    SymbolTableStackBuilder(const Statement* stmt, std::vector<std::shared_ptr<SymbolTable>>* stack);
