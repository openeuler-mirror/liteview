...
public:
    ...
    void checkOutParamsAreAssigned(const FunctionDefinition& funcDef)
    {
        ...
        for (const Variable* param : funcDecl.parameters()) {
            const int paramInout = param->modifiers().fFlags & (Modifiers::Flag::kIn_Flag | Modifiers::Flag::kOut_Flag);
    ...
    bool visitStatement(const Statement& stmt) override
    {
        ...
        case Statement::Kind::kIf:
            if (stmt.as<IfStatement>().isStatic()) {
                fContext.fErrors->error(stmt.as<IfStatement>().test()->fPosition, "static if has non-static test");
            }
            break;

        case Statement::Kind::kSwitch:
            if (stmt.as<SwitchStatement>().isStatic()) {
                fContext.fErrors->error(stmt.as<SwitchStatement>().value()->fPosition, "static switch has non-static test");
            }
            break;
