...
std::unique_ptr<ProgramUsage> Analysis::GetUsage(const LoadedModule& module, const BuiltinMap* base)
{
    auto usage = std::make_unique<ProgramUsage>();
    ProgramUsageVisitor addRefs(usage.get(), /*delta=*/+1);
    ...
    if (base) {
        base->foreach ([&](const Symbol*, const ProgramElement& element) { addRefs.visitProgramElement(element); });
    }
    return usage;
}
