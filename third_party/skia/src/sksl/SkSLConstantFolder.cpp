...
{
    return expr.type().isMatrix() ? is_constant_diagonal(expr, value) : is_constant_splat(expr, value);
}
...
std::unique_ptr<Expression> ConstantFolder::MakeConstantValueForVariable(Position pos, std::unique_ptr<Expression> expr)
{
    ...
    const Context& context, Position pos, const Expression& leftExpr, Operator op, const Expression& rightExpr, const Type& resultType)
{
    ...
    if (context.fConfig->fSettings.fOptimize) {
        if (std::unique_ptr<Expression> expr = simplify_no_op_arithmetic(context, pos, *left, op, *right, resultType)) {
            return expr;
        }
    }
