...
public:
    ...
    bool isWGSL() const
    {
        return fStd == Standard::kWGSLUniform || fStd == Standard::kWGSLStorage;
    }
    ...
    size_t isSupported(const Type& type) const
    {
        ...
        case Type::TypeKind::kStruct:
            return std::all_of(type.fields().begin(), type.fields().end(), [this](const Type::Field& f) { return this->isSupported(*f.fType); });
