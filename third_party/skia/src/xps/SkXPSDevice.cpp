...
SkXPSDevice::SkXPSDevice(SkISize s)
    : INHERITED(SkImageInfo::MakeUnknown(s.width(), s.height()), SkSurfaceProps(0, kUnknown_SkPixelGeometry))
    , fCurrentPage(0)
...
// The CreateFontPackage API is only supported on desktop, not in UWP
    ...
    unsigned long result = CreateFontPackage((unsigned char*)current.fontData->getMemoryBase(), (unsigned long)current.fontData->getLength(),
...
HRESULT SkXPSDevice::createXpsImageBrush(
    const SkBitmap& bitmap, const SkMatrix& localMatrix, const SkTileMode (&xy)[2], const SkAlpha alpha, IXpsOMTileBrush** xpsBrush)
{
...
static SkIRect size(const SkBaseDevice& dev)
{
    return { 0, 0, dev.width(), dev.height() };
}
...
void SkXPSDevice::drawPath(const SkPath& platonicPath, const SkPaint& origPaint, bool pathIsMutable)
{
    ...
    if (filter) {
        SkIRect clipIRect;
        SkVector ppuScale;
        this->convertToPpm(filter, &matrix, &ppuScale, this->cs().bounds(size(*this)).roundOut(), &clipIRect);
...
void SkXPSDevice::onDrawGlyphRunList(SkCanvas*, const sktext::GlyphRunList& glyphRunList, const SkPaint& initailPaint, const SkPaint& drawingPaint)
{
    ...
    for (const auto& run : glyphRunList) {
        ...
        if (FAILED(CreateTypefaceUse(font, &typeface)) || text_must_be_pathed(drawingPaint, this->localToDevice())) {
            SkPath path;
        ...
        HRV(AddGlyphs(this->fXpsFactory.get(), this->fCurrentXpsCanvas.get(), typeface, nullptr, xpsGlyphs.get(), actualGlyphCount, &origin,
            SkScalarToFLOAT(font.getSize()), XPS_STYLE_SIMULATION_NONE, this->localToDevice(), drawingPaint));
    }
}
