...
static void draw_points(SkCanvas::PointMode mode, size_t count, const SkPoint* points, const SkPaint& paint, const SkIRect& bounds, SkBaseDevice* device)
{
...
static SkTCopyOnFirstWrite<SkPaint> clean_paint(const SkPaint& srcPaint)
{
    ...
    if (!paint->isSrcOver() && kSrcOver_SkXfermodeInterpretation == SkInterpretXfermode(*paint, false)) {
...
SkPDFDevice::SkPDFDevice(SkISize pageSize, SkPDFDocument* doc, const SkMatrix& transform)
    : INHERITED(SkImageInfo::MakeUnknown(pageSize.width(), pageSize.height()), SkSurfaceProps(0, kUnknown_SkPixelGeometry))
...
void SkPDFDevice::drawAnnotation(const SkRect& rect, const char key[], SkData* value)
{
    ...
    if (linkType != SkPDFLink::Type::kNone) {
        std::unique_ptr<SkPDFLink> link = std::make_unique<SkPDFLink>(linkType, value, transformedRect, fNodeId);
...
void SkPDFDevice::internalDrawPathWithFilter(const SkClipStack& clipStack, const SkMatrix& ctm, const SkPath& origPath, const SkPaint& origPaint)
{
    ...
    SkStrokeRec::InitStyle initStyle = paint->getFillPath(path, &path) ? SkStrokeRec::kFill_InitStyle : SkStrokeRec::kHairline_InitStyle;
    path.transform(ctm, &path);

    SkIRect bounds = clipStack.bounds(this->bounds()).roundOut();
    SkMask sourceMask;
    if (!SkDraw::DrawToMask(path, bounds, paint->getMaskFilter(), &SkMatrix::I(), &sourceMask, SkMask::kComputeBoundsAndRenderImage_CreateMode, initStyle)) {
        return;
    }
...
void SkPDFDevice::internalDrawGlyphRun(const sktext::GlyphRun& glyphRun, SkPoint offset, const SkPaint& runPaint)
{
    ...
    if (runPaint.getPathEffect() || runPaint.getMaskFilter() || glyphRunFont.isEmbolden() || this->localToDevice().hasPerspective()
    ...
    while (SkClusterator::Cluster c = clusterator.next()) {
        ...
        for (; index < glyphLimit; ++index) {
            ...
            SkRect glyphBounds = get_glyph_bounds_device_space(glyphs[index], textScaleX, textScaleY, xy + offset, this->localToDevice());
...
void SkPDFDevice::onDrawGlyphRunList(SkCanvas*, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
...
static void populate_graphic_state_entry_from_paint(SkPDFDocument* doc, const SkMatrix& matrix, const SkClipStack* clipStack, SkIRect deviceBounds,
    ...
    SkTHashSet<SkPDFIndirectReference>* shaderResources, SkTHashSet<SkPDFIndirectReference>* graphicStateResources)
{
    ...
    if (shader) {
        ...
        if (as_SB(shader)->asGradient() == SkShaderBase::GradientType::kColor) {
            ...
            SkAssertResult(as_SB(shader)->asGradient(&gradientInfo) == SkShaderBase::GradientType::kColor);
    ...
    SkKeyedImage imageSubset, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& srcPaint, const SkMatrix& ctm)
{
    ...
    SkTCopyOnFirstWrite<SkPaint> paint(srcPaint);
    if (!paint->isSrcOver() && imageSubset.image()->isOpaque() && kSrcOver_SkXfermodeInterpretation == SkInterpretXfermode(*paint, false)) {
        paint.writable()->setBlendMode(SkBlendMode::kSrcOver);
    }
    ...
    if (imageSubset.image()->isAlphaOnly() && paint->getColorFilter()) {
        ...
        auto surface = SkSurface::MakeRaster(SkImageInfo::MakeN32Premul(imageSubset.image()->dimensions()));
        SkCanvas* canvas = surface->getCanvas();
        SkPaint tmpPaint;
...
void SkPDFDevice::drawDevice(SkBaseDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
...
void SkPDFDevice::drawSpecial(SkSpecialImage* srcImg, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    if (this->hasEmptyClip()) {
        return;
    }
...
{
    return SkSpecialImage::MakeFromImage(nullptr, image->bounds(), image->makeNonTextureImage(), this->surfaceProps());
}
