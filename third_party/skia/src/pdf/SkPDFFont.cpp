...
SkAdvancedTypefaceMetrics::FontType SkPDFFont::FontType(const SkTypeface& typeface, const SkAdvancedTypefaceMetrics& metrics)
{
...
SkPDFFont* SkPDFFont::GetFontResource(SkPDFDocument* doc, const SkGlyph* glyph, SkTypeface* face)
{
    ...
    return doc->fFontMap.set(typefaceID, SkPDFFont(std::move(typeface), firstNonZeroGlyph, lastGlyph, type, ref));
}

SkPDFFont::SkPDFFont(sk_sp<SkTypeface> typeface, SkGlyphID firstGlyphID, SkGlyphID lastGlyphID, SkAdvancedTypefaceMetrics::FontType fontType,
...
static void emit_subset_type0(const SkPDFFont& font, SkPDFDocument* doc)
{
        ...
        case SkAdvancedTypefaceMetrics::kTrueType_Font: {
            if (!SkToBool(metrics.fFlags & SkAdvancedTypefaceMetrics::kNotSubsettable_FontFlag)) {
                ...
                sk_sp<SkData> subsetFontData
                    = SkPDFSubsetFont(stream_to_data(std::move(fontAsset)), font.glyphUsage(), doc->metadata().fSubsetter, metrics.fFontName.c_str(), ttcIndex);
                if (subsetFontData) {
                    std::unique_ptr<SkPDFDict> tmp = SkPDFMakeDict();
                    tmp->insertInt("Length1", SkToInt(subsetFontData->size()));
                    descriptor->insertRef("FontFile2", SkPDFStreamOut(std::move(tmp), SkMemoryStream::Make(std::move(subsetFontData)), doc, true));
                    break;
                }
            ...
            descriptor->insertRef("FontFile2", SkPDFStreamOut(std::move(tmp), std::move(fontAsset), doc, true));
            break;
        }
        case SkAdvancedTypefaceMetrics::kType1CID_Font: {
            std::unique_ptr<SkPDFDict> tmp = SkPDFMakeDict();
            tmp->insertName("Subtype", "CIDFontType0C");
            descriptor->insertRef("FontFile3", SkPDFStreamOut(std::move(tmp), std::move(fontAsset), doc, true));
            break;
        }
        default:
            SkASSERT(false);
    ...
    {
        std::unique_ptr<SkPDFArray> widths = SkPDFMakeCIDGlyphWidthsArray(*face, font.glyphUsage(), &defaultWidth);
    ...
    std::unique_ptr<SkStreamAsset> toUnicode
        = SkPDFMakeToUnicodeCmap(glyphToUnicode.data(), &font.glyphUsage(), font.multiByteGlyphs(), font.firstGlyphID(), font.lastGlyphID());
...
static ImageAndOffset to_image(SkGlyphID gid, SkBulkGlyphMetricsAndImages* smallGlyphs)
{
    ...
    case SkMask::kA8_Format:
        bm.installPixels(SkImageInfo::MakeA8(bounds.width(), bounds.height()), mask.fImage, mask.fRowBytes);
        return { SkMakeImageFromRasterBitmap(bm, kAlways_SkCopyPixelsMode), { bounds.x(), bounds.y() } };
    case SkMask::kARGB32_Format:
        bm.installPixels(SkImageInfo::MakeN32Premul(bounds.width(), bounds.height()), mask.fImage, mask.fRowBytes);
        return { SkMakeImageFromRasterBitmap(bm, kAlways_SkCopyPixelsMode), { bounds.x(), bounds.y() } };
...
SkStrikeSpec make_small_strike(const SkTypeface& typeface)
{
    ...
    return SkStrikeSpec::MakeMask(
        font, SkPaint(), SkSurfaceProps(0, kUnknown_SkPixelGeometry), SkScalerContextFlags::kFakeGammaAndBoostContrast, SkMatrix::I());
}
...
static void emit_subset_type3(const SkPDFFont& pdfFont, SkPDFDocument* doc)
{
    ...
    auto toUnicodeCmap = SkPDFMakeToUnicodeCmap(glyphToUnicode.data(), &subset, false, firstGlyphID, lastGlyphID);
