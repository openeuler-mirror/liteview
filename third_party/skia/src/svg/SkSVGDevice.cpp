...
public:
    ...
    {
        return SkStringPrintf("cfilter_%d", fColorFilterCount++);
    }
...
void SkSVGDevice::AutoElement::addGradientShaderResources(const SkShader* shader, const SkPaint& paint, Resources* resources)
{
    ...
    if (gradient_type != SkShaderBase::GradientType::kColor && gradient_type != SkShaderBase::GradientType::kLinear) {
...
sk_sp<SkBaseDevice> SkSVGDevice::Make(const SkISize& size, std::unique_ptr<SkXMLWriter> writer, uint32_t flags)
{
...
SkSVGDevice::SkSVGDevice(const SkISize& size, std::unique_ptr<SkXMLWriter> writer, uint32_t flags)
    : INHERITED(SkImageInfo::MakeUnknown(size.fWidth, size.fHeight), SkSurfaceProps(0, kUnknown_SkPixelGeometry))
...
void SkSVGDevice::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    SkASSERT(!glyphRunList.hasRSXForm());
    const auto draw_as_path = (fFlags & SkSVGCanvas::kConvertTextToPaths_Flag) || drawingPaint.getPathEffect();
