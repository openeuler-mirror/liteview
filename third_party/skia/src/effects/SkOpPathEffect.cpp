...
sk_sp<SkPathEffect> SkMergePathEffect::Make(sk_sp<SkPathEffect> one, sk_sp<SkPathEffect> two, SkPathOp op)
{
...
bool SkOpPE::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect* cull, const SkMatrix& ctm) const
{
    SkPath one, two;
    if (fOne) {
...
{
    if (!bounds) {
        return (!SkToBool(fOne) || as_PEB(fOne)->computeFastBounds(nullptr)) && (!SkToBool(fTwo) || as_PEB(fTwo)->computeFastBounds(nullptr));
    }
...
SkMatrixPE::SkMatrixPE(const SkMatrix& matrix)
    : fMatrix(matrix)
{
    SkASSERT(matrix.isFinite());
}

bool SkMatrixPE::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect*, const SkMatrix&) const
{
...
sk_sp<SkPathEffect> SkStrokePathEffect::Make(SkScalar width, SkPaint::Join join, SkPaint::Cap cap, SkScalar miter)
{
...
bool SkStrokePE::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect*, const SkMatrix&) const
{
...
bool SkStrokeAndFillPE::onFilterPath(SkPath* dst, const SkPath& src, SkStrokeRec* rec, const SkRect*, const SkMatrix&) const
{
