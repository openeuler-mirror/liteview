...
public:
    SkBlurImageFilter(SkScalar sigmaX, SkScalar sigmaY, SkTileMode tileMode, sk_sp<SkImageFilter> input, const SkRect* cropRect)
...
#if SK_SUPPORT_GPU
    sk_sp<SkSpecialImage> gpuFilter(const Context& ctx, SkVector sigma, const sk_sp<SkSpecialImage>& input, SkIRect inputBounds, SkIRect dstBounds,
...
sk_sp<SkSpecialImage> copy_image_with_bounds(const SkImageFilter_Base::Context& ctx, const sk_sp<SkSpecialImage>& input, SkIRect srcBounds, SkIRect dstBounds)
{
    SkBitmap inputBM;
    ...
    for (; y < srcBounds.bottom(); y++) {
        ...
        memcpy(dstPtr, src.getAddr32(x - srcBounds.left(), y - srcBounds.top()), srcW * sizeof(uint32_t));
...
sk_sp<SkSpecialImage> cpu_blur(const SkImageFilter_Base::Context& ctx, SkVector sigma, const sk_sp<SkSpecialImage>& input, SkIRect srcBounds, SkIRect dstBounds)
{
    ...
    auto srcW = srcBounds.width(), srcH = srcBounds.height(), dstW = dstBounds.width(), dstH = dstBounds.height();
    ...
    if (makerX->window() > 1) {
        ...
        intermediateSrc = static_cast<uint32_t*>(dst.getPixels()) + (shift > 0 ? shift * dst.rowBytesAsPixels() : 0);
        ...
        for (auto y = 0; y < srcH; y++) {
            pass->blur(srcBounds.left(), srcBounds.right(), dstBounds.right(), srcCursor, 1, dstCursor, 1);
    ...
    if (makerY->window() > 1) {
        ...
        for (auto x = 0; x < intermediateWidth; x++) {
            pass->blur(srcBounds.top(), srcBounds.bottom(), dstBounds.bottom(), srcCursor, intermediateRowBytesAsPixels, dstCursor, dst.rowBytesAsPixels());
            srcCursor += 1;
            dstCursor += 1;
        }
    }
...
sk_sp<SkSpecialImage> SkBlurImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
    ...
    SkIRect inputBounds = SkIRect::MakeXYWH(inputOffset.fX, inputOffset.fY, input->width(), input->height());
    ...
    SkASSERT(
        SkScalarIsFinite(sigma.x()) && sigma.x() >= 0.f && sigma.x() <= kMaxSigma && SkScalarIsFinite(sigma.y()) && sigma.y() >= 0.f && sigma.y() <= kMaxSigma);
...
#if SK_SUPPORT_GPU
sk_sp<SkSpecialImage> SkBlurImageFilter::gpuFilter(const Context& ctx, SkVector sigma, const sk_sp<SkSpecialImage>& input, SkIRect inputBounds,
    SkIRect dstBounds, SkIPoint inputOffset, SkIPoint* offset) const
{
#if SK_GPU_V1
    if (SkGpuBlurUtils::IsEffectivelyZeroSigma(sigma.x()) && SkGpuBlurUtils::IsEffectivelyZeroSigma(sigma.y())) {
    ...
    auto sdc = SkGpuBlurUtils::GaussianBlur(context, std::move(inputView), SkColorTypeToGrColorType(input->colorType()), input->alphaType(),
        ctx.refColorSpace(), dstBounds, inputBounds, sigma.x(), sigma.y(), fTileMode);
    if (!sdc) {
        return nullptr;
    }

    return SkSpecialImage::MakeDeferredFromGpu(
        context, SkIRect::MakeSize(dstBounds.size()), kNeedNewImageUniqueID_SpecialImage, sdc->readSurfaceView(), sdc->colorInfo(), ctx.surfaceProps());
#else // SK_GPU_V1
    return nullptr;
#endif // SK_GPU_V1
}
...
SkIRect SkBlurImageFilter::onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection, const SkIRect* inputRect) const
{
