...
sk_sp<SkSpecialImage> SkTileImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
    ...
    if (inputBounds.contains(srcIRect)) {
        ...
        sk_sp<SkSurface> surf(input->makeTightSurface(ctx.colorType(), ctx.colorSpace(), srcIRect.size()));
        if (!surf) {
            return nullptr;
        }
        ...
        input->draw(canvas, SkIntToScalar(inputOffset.x()), SkIntToScalar(inputOffset.y()), SkSamplingOptions(), &paint);
    ...
    SkPaint paint;
    paint.setBlendMode(SkBlendMode::kSrc);
    paint.setShader(subset->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions()));
...
SkIRect SkTileImageFilter::onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection dir, const SkIRect* inputRect) const
{
...
SkIRect SkTileImageFilter::onFilterBounds(const SkIRect& src, const SkMatrix&, MapDirection, const SkIRect* inputRect) const
{
