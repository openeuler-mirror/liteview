...
public:
    SkMorphologyImageFilter(MorphType type, SkScalar radiusX, SkScalar radiusY, sk_sp<SkImageFilter> input, const SkRect* cropRect)
    ...
    typedef void (*Proc)(const SkPMColor* src, SkPMColor* dst, int radius, int width, int height, int srcStride, int dstStride);
...
sk_sp<SkImageFilter> SkImageFilters::Dilate(SkScalar radiusX, SkScalar radiusY, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    ...
    return sk_sp<SkImageFilter>(new SkMorphologyImageFilter(MorphType::kDilate, radiusX, radiusY, std::move(input), cropRect));
}
...
sk_sp<SkImageFilter> SkImageFilters::Erode(SkScalar radiusX, SkScalar radiusY, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    ...
    return sk_sp<SkImageFilter>(new SkMorphologyImageFilter(MorphType::kErode, radiusX, radiusY, std::move(input), cropRect));
}
...
static void call_proc_X(SkMorphologyImageFilter::Proc procX, const SkBitmap& src, SkBitmap* dst, int radiusX, const SkIRect& bounds)
{
    procX(src.getAddr32(bounds.left(), bounds.top()), dst->getAddr32(0, 0), radiusX, bounds.width(), bounds.height(), src.rowBytesAsPixels(),
        dst->rowBytesAsPixels());
}

static void call_proc_Y(SkMorphologyImageFilter::Proc procY, const SkPMColor* src, int srcRowBytesAsPixels, SkBitmap* dst, int radiusY, const SkIRect& bounds)
{
    procY(src, dst->getAddr32(0, 0), radiusY, bounds.height(), bounds.width(), srcRowBytesAsPixels, dst->rowBytesAsPixels());
}
...
SkIRect SkMorphologyImageFilter::onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection, const SkIRect* inputRect) const
{
...
public:
    static std::unique_ptr<GrFragmentProcessor> Make(
        std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView view, SkAlphaType srcAlphaType, MorphDirection dir, int radius, MorphType type)
    {
        return std::unique_ptr<GrFragmentProcessor>(
            new GrMorphologyEffect(std::move(inputFP), std::move(view), srcAlphaType, dir, radius, type, /*range=*/nullptr));
    }

    static std::unique_ptr<GrFragmentProcessor> Make(std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView view, SkAlphaType srcAlphaType,
        MorphDirection dir, int radius, MorphType type, const float range[2])
    {
        return std::unique_ptr<GrFragmentProcessor>(new GrMorphologyEffect(std::move(inputFP), std::move(view), srcAlphaType, dir, radius, type, range));
    }
...
private:
    ...
    GrMorphologyEffect(std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView, SkAlphaType srcAlphaType, MorphDirection, int radius, MorphType,
        const float range[2]);
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrMorphologyEffect::onMakeProgramImpl() const
{
    ...
    public:
        void emitCode(EmitArgs& args) override
        {
            ...
            fRangeUni = uniformHandler->addUniform(&me, kFragment_GrShaderFlag, SkSLType::kFloat2, "Range");
            ...
            if (me.fUseRange) {
                ...
                fragBuilder->codeAppendf("float highBound = min(%s.y, coord.%c + %f);", range, dir, float(width - 1));
...
GrMorphologyEffect::GrMorphologyEffect(std::unique_ptr<GrFragmentProcessor> inputFP, GrSurfaceProxyView view, SkAlphaType srcAlphaType,
    ...
    , fUseRange(SkToBool(range))
{
    ...
    this->registerChild(GrTextureEffect::Make(std::move(view), srcAlphaType), SkSL::SampleUsage::Explicit());
    if (fUseRange) {
        fRange[0] = range[0];
        fRange[1] = range[1];
    }
...
bool GrMorphologyEffect::onIsEqual(const GrFragmentProcessor& sBase) const
{
    ...
    return this->fRadius == s.fRadius && this->fDirection == s.fDirection && this->fUseRange == s.fUseRange && this->fType == s.fType;
}
...
static void apply_morphology_rect(skgpu::v1::SurfaceFillContext* sfc, GrSurfaceProxyView view, SkAlphaType srcAlphaType, const SkIRect& srcRect,
    const SkIRect& dstRect, int radius, MorphType morphType, const float range[2], MorphDirection direction)
{
    auto fp = GrMorphologyEffect::Make(/*inputFP=*/nullptr, std::move(view), srcAlphaType, direction, radius, morphType, range);
    sfc->fillRectToRectWithFP(srcRect, dstRect, std::move(fp));
}

static void apply_morphology_rect_no_bounds(skgpu::v1::SurfaceFillContext* sfc, GrSurfaceProxyView view, SkAlphaType srcAlphaType, const SkIRect& srcRect,
...
static void apply_morphology_pass(skgpu::v1::SurfaceFillContext* sfc, GrSurfaceProxyView view, SkAlphaType srcAlphaType, const SkIRect& srcRect,
    const SkIRect& dstRect, int radius, MorphType morphType, MorphDirection direction)
{
    ...
    if (middleSrcRect.width() <= 0) {
        ...
        apply_morphology_rect(sfc, std::move(view), srcAlphaType, srcRect, dstRect, radius, morphType, bounds, direction);
    } else {
        // Draw upper and lower margins with bounds; middle without.
        apply_morphology_rect(sfc, view, srcAlphaType, lowerSrcRect, lowerDstRect, radius, morphType, bounds, direction);
        apply_morphology_rect(sfc, view, srcAlphaType, upperSrcRect, upperDstRect, radius, morphType, bounds, direction);
        apply_morphology_rect_no_bounds(sfc, std::move(view), srcAlphaType, middleSrcRect, middleDstRect, radius, morphType, direction);
    }
}

static sk_sp<SkSpecialImage> apply_morphology(
    GrRecordingContext* rContext, SkSpecialImage* input, const SkIRect& rect, MorphType morphType, SkISize radius, const SkImageFilter_Base::Context& ctx)
{
    ...
    if (radius.fWidth > 0) {
        auto dstFillContext = rContext->priv().makeSFC(
            info, "SpecialImage_ApplyMorphology_Width", SkBackingFit::kApprox, 1, GrMipmapped::kNo, proxy->isProtected(), kBottomLeft_GrSurfaceOrigin);
        if (!dstFillContext) {
            return nullptr;
        }

        apply_morphology_pass(dstFillContext.get(), std::move(srcView), srcAlphaType, srcRect, dstRect, radius.fWidth, morphType, MorphDirection::kX);
        SkIRect clearRect = SkIRect::MakeXYWH(dstRect.fLeft, dstRect.fBottom, dstRect.width(), radius.fHeight);
        SkPMColor4f clearColor = MorphType::kErode == morphType ? SK_PMColor4fWHITE : SK_PMColor4fTRANSPARENT;
    ...
    if (radius.fHeight > 0) {
        auto dstFillContext = rContext->priv().makeSFC(info, "SpecialImage_ApplyMorphology_Height", SkBackingFit::kApprox, 1, GrMipmapped::kNo,
            srcView.proxy()->isProtected(), kBottomLeft_GrSurfaceOrigin);
        if (!dstFillContext) {
            return nullptr;
        }

        apply_morphology_pass(dstFillContext.get(), std::move(srcView), srcAlphaType, srcRect, dstRect, radius.fHeight, morphType, MorphDirection::kY);

        srcView = dstFillContext->readSurfaceView();
    }

    return SkSpecialImage::MakeDeferredFromGpu(
        rContext, SkIRect::MakeWH(rect.width(), rect.height()), kNeedNewImageUniqueID_SpecialImage, std::move(srcView), info.colorInfo(), input->props());
}
#endif
...
static void morph(const SkPMColor* src, SkPMColor* dst, int radius, int width, int height, int srcStride, int dstStride)
{
    ...
    for (int x = 0; x < width; ++x) {
        ...
        for (int y = 0; y < height; ++y) {
            __m128i extreme = (type == MorphType::kDilate) ? _mm_setzero_si128() : _mm_set1_epi32(0xFFFFFFFF);
            for (const SkPMColor* p = lp; p <= up; p += srcStrideX) {
                __m128i src_pixel = _mm_cvtsi32_si128(*p);
                extreme = (type == MorphType::kDilate) ? _mm_max_epu8(src_pixel, extreme) : _mm_min_epu8(src_pixel, extreme);
            }
    ...
    for (int x = 0; x < width; ++x) {
        ...
        for (int y = 0; y < height; ++y) {
            ...
            for (const SkPMColor* p = lp; p <= up; p += srcStrideX) {
                ...
                extreme = (type == MorphType::kDilate) ? vmax_u8(src_pixel, extreme) : vmin_u8(src_pixel, extreme);
            }
...
static void morph(const SkPMColor* src, SkPMColor* dst, int radius, int width, int height, int srcStride, int dstStride)
{
    ...
    for (int x = 0; x < width; ++x) {
        ...
        for (int y = 0; y < height; ++y) {
            ...
            for (const SkPMColor* p = lp; p <= up; p += srcStrideX) {
                int b = SkGetPackedB32(*p), g = SkGetPackedG32(*p), r = SkGetPackedR32(*p), a = SkGetPackedA32(*p);
...
sk_sp<SkSpecialImage> SkMorphologyImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
    ...
    if (ctx.gpuBacked()) {
        ...
        sk_sp<SkSpecialImage> result(apply_morphology(context, input.get(), srcBounds, fType, SkISize::Make(width, height), ctx));
        if (result) {
            offset->fX = bounds.left();
            offset->fY = bounds.top();
        }
    ...
    if (width > 0 && height > 0) {
        ...
        call_proc_Y(procY, tmp.getAddr32(tmpBounds.left(), tmpBounds.top()), tmp.rowBytesAsPixels(), &dst, height, tmpBounds);
    } else if (width > 0) {
        call_proc_X(procX, inputBM, &dst, width, srcBounds);
    } else if (height > 0) {
        call_proc_Y(procY, inputBM.getAddr32(srcBounds.left(), srcBounds.top()), inputBM.rowBytesAsPixels(), &dst, height, srcBounds);
    }
    offset->fX = bounds.left();
    offset->fY = bounds.top();
