...
sk_sp<SkSpecialImage> SkPictureImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
    if (!fPicture) {
        return nullptr;
    }
...
SkIRect SkPictureImageFilter::onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection direction, const SkIRect* inputRect) const
{
