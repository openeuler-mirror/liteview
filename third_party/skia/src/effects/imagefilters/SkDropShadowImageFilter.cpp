...
public:
    SkDropShadowImageFilter(
        SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor color, bool shadowOnly, sk_sp<SkImageFilter> input, const SkRect* cropRect)
    ...
    static sk_sp<SkImageFilter> Make(
        SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor color, bool shadowOnly, sk_sp<SkImageFilter> input, const SkRect* cropRect)
    {
        return sk_sp<SkImageFilter>(new SkDropShadowImageFilter(dx, dy, sigmaX, sigmaY, color, shadowOnly, std::move(input), cropRect));
    }
...
{
    return SkDropShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, /* shadowOnly */ false, std::move(input), cropRect);
}

sk_sp<SkImageFilter> SkImageFilters::DropShadowOnly(
    SkScalar dx, SkScalar dy, SkScalar sigmaX, SkScalar sigmaY, SkColor color, sk_sp<SkImageFilter> input, const CropRect& cropRect)
{
    return SkDropShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, /* shadowOnly */ true, std::move(input), cropRect);
}
...
sk_sp<SkFlattenable> SkDropShadowImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkDropShadowImageFilter::Make(dx, dy, sigmaX, sigmaY, color, shadowOnly, common.getInput(0), common.cropRect());
}
...
sk_sp<SkSpecialImage> SkDropShadowImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
    ...
    canvas->translate(SkIntToScalar(inputOffset.fX) - SkIntToScalar(bounds.fLeft), SkIntToScalar(inputOffset.fY) - SkIntToScalar(bounds.fTop));
...
SkIRect SkDropShadowImageFilter::onFilterNodeBounds(const SkIRect& src, const SkMatrix& ctm, MapDirection dir, const SkIRect* inputRect) const
{
    SkVector offsetVec = SkVector::Make(fDx, fDy);
    ...
    dst.outset(SkScalarCeilToInt(SkScalarAbs(sigma.x() * 3)), SkScalarCeilToInt(SkScalarAbs(sigma.y() * 3)));
    if (!fShadowOnly) {
        dst.join(src);
    }
    return dst;
