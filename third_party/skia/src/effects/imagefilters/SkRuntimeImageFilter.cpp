...
public:
    SkRuntimeImageFilter(sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, sk_sp<SkImageFilter> input)
    ...
    SkRuntimeImageFilter(const SkRuntimeShaderBuilder& builder, std::string_view childShaderNames[], const sk_sp<SkImageFilter> inputs[], int inputCount)
...
sk_sp<SkImageFilter> SkMakeRuntimeImageFilter(sk_sp<SkRuntimeEffect> effect, sk_sp<SkData> uniforms, sk_sp<SkImageFilter> input)
{
    ...
    return sk_sp<SkImageFilter>(new SkRuntimeImageFilter(std::move(effect), std::move(uniforms), std::move(input)));
}
...
sk_sp<SkFlattenable> SkRuntimeImageFilter::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkImageFilters::RuntimeShader(builder, childShaderNames.data(), common.inputs(), common.inputCount());
}
...
sk_sp<SkSpecialImage> SkRuntimeImageFilter::onFilterImage(const Context& ctx, SkIPoint* offset) const
{
...
sk_sp<SkImageFilter> SkImageFilters::RuntimeShader(const SkRuntimeShaderBuilder& builder, std::string_view childShaderName, sk_sp<SkImageFilter> input)
{
...
sk_sp<SkImageFilter> SkImageFilters::RuntimeShader(
    const SkRuntimeShaderBuilder& builder, std::string_view childShaderNames[], const sk_sp<SkImageFilter> inputs[], int inputCount)
{
    for (int i = 0; i < inputCount; i++) {
    ...
    return sk_sp<SkImageFilter>(new SkRuntimeImageFilter(builder, childShaderNames, inputs, inputCount));
}
