...
sk_sp<SkColorFilter> SkHighContrastFilter::Make(const SkHighContrastConfig& config)
{
    ...
    static const SkRuntimeEffect* effect = SkMakeCachedRuntimeEffect(SkRuntimeEffect::MakeForColorFilter, SkString(kHighContrastFilterCode)).release();
    ...
    return SkColorFilterPriv::WithWorkingFormat(
        effect->makeColorFilter(SkData::MakeWithCopy(&uniforms, sizeof(uniforms))), &linear, nullptr /*use dst gamut*/, &unpremul);
}
#else // SK_ENABLE_SKSL
sk_sp<SkColorFilter> SkHighContrastFilter::Make(const SkHighContrastConfig& config)
{
