...
public:
    SkTable_ColorFilter(const uint8_t tableA[], const uint8_t tableR[], const uint8_t tableG[], const uint8_t tableB[])
    {
        fBitmap.allocPixels(SkImageInfo::MakeA8(256, 4));
        uint8_t *a = fBitmap.getAddr8(0, 0), *r = fBitmap.getAddr8(0, 1), *g = fBitmap.getAddr8(0, 2), *b = fBitmap.getAddr8(0, 3);
        for (int i = 0; i < 256; i++) {
...
#if SK_SUPPORT_GPU
    GrFPResult asFragmentProcessor(std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext*, const GrColorInfo&, const SkSurfaceProps&) const override;
#endif
    ...
    bool onAppendStages(const SkStageRec& rec, bool shaderIsOpaque) const override
    {
        ...
        const uint8_t *a = fBitmap.getAddr8(0, 0), *r = fBitmap.getAddr8(0, 1), *g = fBitmap.getAddr8(0, 2), *b = fBitmap.getAddr8(0, 3);
        struct Tables {
    ...
    skvm::Color onProgram(skvm::Builder* p, skvm::Color c, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc*) const override
    {
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::Make(std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const SkBitmap& bitmap)
{
    ...
    auto view = std::get<0>(GrMakeCachedBitmapProxyView(context, bitmap,
        /*label=*/"MakeColorTableEffect", GrMipmapped::kNo));
    if (!view) {
        return nullptr;
    }
...
std::unique_ptr<GrFragmentProcessor> ColorTableEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    auto [success, fp] = as_CFB(filter)->asFragmentProcessor(
        d->inputFP(), d->context(), GrColorInfo(GrColorType::kRGBA_8888, kUnknown_SkAlphaType, std::move(colorSpace)), props);
    SkASSERT(success);
    return std::move(fp);
}
#endif
...
GrFPResult SkTable_ColorFilter::asFragmentProcessor(
    std::unique_ptr<GrFragmentProcessor> inputFP, GrRecordingContext* context, const GrColorInfo&, const SkSurfaceProps&) const
{
...
void SkTable_ColorFilter::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
