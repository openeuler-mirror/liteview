...
void SkCodec::Register(bool (*peek)(const void*, size_t), std::unique_ptr<SkCodec> (*make)(std::unique_ptr<SkStream>, SkCodec::Result*))
{
...
SkCodec::Result SkCodec::handleFrameIndex(const SkImageInfo& info, void* pixels, size_t rowBytes, const Options& options, SkAndroidCodec* androidCodec)
{
    if (androidCodec) {
    ...
    if (requiredFrame != kNoFrame) {
        ...
        if (options.fPriorFrame == kNoFrame) {
            ...
            if (androidCodec) {
#ifdef SK_HAS_ANDROID_CODEC
                SkAndroidCodec::AndroidOptions prevFrameOptions(reinterpret_cast<const SkAndroidCodec::AndroidOptions&>(options));
