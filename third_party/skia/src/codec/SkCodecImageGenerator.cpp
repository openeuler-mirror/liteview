...
{
    return codec ? std::unique_ptr<SkImageGenerator>(new SkCodecImageGenerator(std::move(codec), nullptr, std::nullopt)) : nullptr;
}
...
SkCodecImageGenerator::SkCodecImageGenerator(std::unique_ptr<SkCodec> codec, sk_sp<SkData> data, std::optional<SkAlphaType> at)
