...
public:
    ...
    SkStyleSet_Indirect(const SkFontMgr_Indirect* owner, int familyIndex, SkRemotableFontIdentitySet* data)
        : fOwner(SkRef(owner))
...
SkTypeface* SkFontMgr_Indirect::onMatchFamilyStyle(const char familyName[], const SkFontStyle& fontStyle) const
{
...
{
    SkFontIdentity id = fProxy->matchNameStyleCharacter(familyName, style, bcp47, bcp47Count, character);
    return this->createTypefaceFromFontId(id);
}

sk_sp<SkTypeface> SkFontMgr_Indirect::onMakeFromStreamIndex(std::unique_ptr<SkStreamAsset> stream, int ttcIndex) const
{
    return fImpl->makeFromStream(std::move(stream), ttcIndex);
}

sk_sp<SkTypeface> SkFontMgr_Indirect::onMakeFromStreamArgs(std::unique_ptr<SkStreamAsset> stream, const SkFontArguments& args) const
{
...
sk_sp<SkTypeface> SkFontMgr_Indirect::onLegacyMakeTypeface(const char familyName[], SkFontStyle style) const
{
