...
struct SkOTTableEmbeddedBitmapLocation {
    ...
    struct IndexSubTable3 {
        IndexSubHeader header;
        // SK_OT_USHORT offsetArray[lastGlyphIndex - firstGlyphIndex + 1 + 1]; //last element points to one past end of last glyph, may have extra element to force even number of
        // elements glyphData = offsetArray[glyphIndex - firstGlyphIndex] + imageDataOffset
    };
