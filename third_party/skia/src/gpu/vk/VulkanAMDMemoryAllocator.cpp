...
sk_sp<VulkanMemoryAllocator> VulkanAMDMemoryAllocator::Make(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device,
    uint32_t physicalDeviceVersion, const VulkanExtensions* extensions, sk_sp<const VulkanInterface> interface, bool mustUseCoherentHostVisibleMemory,
    bool threadSafe)
{
    return nullptr;
}
...
#define SKGPU_COPY_FUNCTION_KHR(NAME) functions.vk##NAME##KHR = interface->fFunctions.f##NAME
    ...
    return sk_sp<VulkanAMDMemoryAllocator>(new VulkanAMDMemoryAllocator(allocator, std::move(interface), mustUseCoherentHostVisibleMemory));
}

VulkanAMDMemoryAllocator::VulkanAMDMemoryAllocator(VmaAllocator allocator, sk_sp<const VulkanInterface> interface, bool mustUseCoherentHostVisibleMemory)
    ...
    VkBuffer buffer, BufferUsage usage, uint32_t allocationPropertyFlags, skgpu::VulkanBackendMemory* backendMemory)
{
    ...
    if (fMustUseCoherentHostVisibleMemory && (info.requiredFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
