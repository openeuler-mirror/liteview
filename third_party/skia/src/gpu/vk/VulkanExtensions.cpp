...
static int find_info(const SkTArray<VulkanExtensions::Info>& infos, const char ext[])
{
    ...
    int idx = SkTSearch<VulkanExtensions::Info, SkString, VulkanExtensions::Info::Less>(
        &infos.front(), infos.count(), extensionStr, sizeof(VulkanExtensions::Info), less);
    return idx;
}
...
void VulkanExtensions::getSpecVersions(VulkanGetProc getProc, VkInstance instance, VkPhysicalDevice physDevice)
{
