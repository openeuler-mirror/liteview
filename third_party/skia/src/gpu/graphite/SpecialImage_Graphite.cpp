...
public:
    SkSpecialImage_Graphite(
        Recorder* recorder, const SkIRect& subset, uint32_t uniqueID, TextureProxyView view, const SkColorInfo& colorInfo, const SkSurfaceProps& props)
    ...
    void onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint) const override
    {
        ...
        sk_sp<SkImage> img = sk_sp<SkImage>(new skgpu::graphite::Image(this->uniqueID(), fTextureProxyView, this->colorInfo()));
    ...
    TextureProxyView onTextureProxyView() const override
    {
    ...
    {
        return SkSpecialImage::MakeGraphite(fRecorder, subset, this->uniqueID(), fTextureProxyView, this->colorInfo(), this->props());
    }
...
sk_sp<SkSpecialImage> SkSpecialImage::MakeGraphite(skgpu::graphite::Recorder* recorder, const SkIRect& subset, uint32_t uniqueID,
    skgpu::graphite::TextureProxyView view, const SkColorInfo& colorInfo, const SkSurfaceProps& props)
{
    if (!recorder || !view) {
        return nullptr;
    }
    ...
    return sk_make_sp<skgpu::graphite::SkSpecialImage_Graphite>(recorder, subset, uniqueID, std::move(view), colorInfo, props);
}
