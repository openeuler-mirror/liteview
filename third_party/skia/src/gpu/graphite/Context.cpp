...
#ifdef SK_DAWN
std::unique_ptr<Context> Context::MakeDawn(const DawnBackendContext& backendContext, const ContextOptions& options)
{
    ...
    auto context = std::unique_ptr<Context>(new Context(std::move(sharedContext), std::move(queueManager)));
    SkASSERT(context);
    return context;
}
#endif
...
#ifdef SK_METAL
std::unique_ptr<Context> Context::MakeMetal(const MtlBackendContext& backendContext, const ContextOptions& options)
{
...
#ifdef SK_VULKAN
std::unique_ptr<Context> Context::MakeVulkan(const VulkanBackendContext& backendContext, const ContextOptions& options)
{
...
void Context::precompile(CombinationBuilder* combinationBuilder)
{
    ...
    combinationBuilder->buildCombinations(fSharedContext->shaderCodeDictionary(), [&](SkUniquePaintParamsID uniqueID) {
