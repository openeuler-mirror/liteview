...
std::tuple<TextureProxyView, SkColorType> MakeBitmapProxyView(
    Recorder* recorder, const SkBitmap& bitmap, sk_sp<SkMipmap> mipmapsIn, Mipmapped mipmapped, SkBudgeted budgeted)
{
    ...
    auto textureInfo = caps->getDefaultSampledTextureInfo(ct, mipLevelCount, Protected::kNo, Renderable::kNo);
    if (!textureInfo.isValid()) {
        ct = kRGBA_8888_SkColorType;
        textureInfo = caps->getDefaultSampledTextureInfo(ct, mipLevelCount, Protected::kNo, Renderable::kNo);
    }
    ...
    if (mipLevelCount == 1) {
        ...
        mipmaps = SkToBool(mipmapsIn) ? mipmapsIn : sk_ref_sp(SkMipmap::Build(bmpToUpload.pixmap(), nullptr));
        if (!mipmaps) {
            return {};
        }
    ...
    UploadInstance upload = UploadInstance::Make(recorder, proxy, ct, texels, SkIRect::MakeSize(bmpToUpload.dimensions()));
...
sk_sp<SkImage> MakeFromBitmap(Recorder* recorder, const SkColorInfo& colorInfo, const SkBitmap& bitmap, sk_sp<SkMipmap> mipmaps, SkBudgeted budgeted,
    SkImage::RequiredImageProperties requiredProps)
{
    auto [view, ct] = MakeBitmapProxyView(recorder, bitmap, std::move(mipmaps), requiredProps.fMipmapped, budgeted);
    if (!view) {
        return nullptr;
    }

    SkASSERT(requiredProps.fMipmapped == skgpu::graphite::Mipmapped::kNo || view.proxy()->mipmapped() == skgpu::graphite::Mipmapped::kYes);
    return sk_make_sp<skgpu::graphite::Image>(std::move(view), colorInfo.makeColorType(ct));
}

bool ReadPixelsHelper(FlushPendingWorkCallback&& flushPendingWork, Context* context, Recorder* recorder, TextureProxy* srcProxy, const SkImageInfo& dstInfo,
    void* dstPixels, size_t dstRowBytes, int srcX, int srcY)
{
    ...
    sk_sp<Buffer> dstBuffer = resourceProvider->findOrCreateBuffer(size, BufferType::kXferGpuToCpu, PrioritizeGpuReads::kNo);
    if (!dstBuffer) {
        return false;
    }

    SkIRect srcRect = SkIRect::MakeXYWH(srcX, srcY, dstInfo.width(), dstInfo.height());
    sk_sp<CopyTextureToBufferTask> copyTask = CopyTextureToBufferTask::Make(sk_ref_sp(srcProxy), srcRect, dstBuffer,
