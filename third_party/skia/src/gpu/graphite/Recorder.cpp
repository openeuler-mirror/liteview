...
public:
    ...
    sk_sp<SkImage> findOrCreate(Recorder* recorder, const SkImage* image, SkImage::RequiredImageProperties) override
    {
    ...
    , fTextBlobCache(std::make_unique<sktext::gpu::TextBlobRedrawCoordinator>(fRecorderID))
{
    ...
    fDrawBufferManager.reset(new DrawBufferManager(
        fResourceProvider.get(), fSharedContext->caps()->requiredUniformBufferAlignment(), fSharedContext->caps()->requiredStorageBufferAlignment()));
...
bool Recorder::updateBackendTexture(const BackendTexture& backendTex, const SkPixmap srcData[], int numLevels)
{
    ASSERT_SINGLE_OWNER
    ...
    UploadInstance upload = UploadInstance::Make(this, std::move(proxy), ct, mipLevels, SkIRect::MakeSize(backendTex.dimensions()));
