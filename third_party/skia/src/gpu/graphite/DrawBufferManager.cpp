...
void* map_offset(BindBufferInfo binding)
{
    ...
    return SkTAddOffset<void>(const_cast<Buffer*>(binding.fBuffer)->map(), static_cast<ptrdiff_t>(binding.fOffset));
}

template <size_t BufferBlockSize> size_t sufficient_block_size(size_t requiredBytes)
{
...
DrawBufferManager::DrawBufferManager(ResourceProvider* resourceProvider, size_t uniformStartAlignment, size_t ssboStartAlignment)
...
std::tuple<VertexWriter, BindBufferInfo> DrawBufferManager::getVertexWriter(size_t requiredBytes)
{
    ...
    if (fCurrentVertexBuffer && !can_fit(requiredBytes, fCurrentVertexBuffer.get(), fVertexOffset, /*alignment=*/1)) {
    ...
    if (!fCurrentVertexBuffer) {
        ...
        fCurrentVertexBuffer = fResourceProvider->findOrCreateBuffer(bufferSize, BufferType::kVertex, PrioritizeGpuReads::kNo);
        fVertexOffset = 0;
        if (!fCurrentVertexBuffer) {
            return { VertexWriter(), BindBufferInfo() };
        }
...
std::tuple<IndexWriter, BindBufferInfo> DrawBufferManager::getIndexWriter(size_t requiredBytes)
{
    ...
    if (fCurrentIndexBuffer && !can_fit(requiredBytes, fCurrentIndexBuffer.get(), fIndexOffset, /*alignment=*/1)) {
    ...
    if (!fCurrentIndexBuffer) {
        ...
        fCurrentIndexBuffer = fResourceProvider->findOrCreateBuffer(bufferSize, BufferType::kIndex, PrioritizeGpuReads::kNo);
        fIndexOffset = 0;
        if (!fCurrentIndexBuffer) {
            return { IndexWriter(), BindBufferInfo() };
        }
...
std::tuple<UniformWriter, BindBufferInfo> DrawBufferManager::getUniformWriter(size_t requiredBytes)
{
    if (!requiredBytes) {
        return { UniformWriter(), BindBufferInfo() };
    }
    if (fCurrentUniformBuffer && !can_fit(requiredBytes, fCurrentUniformBuffer.get(), fUniformOffset, fUniformStartAlignment)) {
    ...
    if (!fCurrentUniformBuffer) {
        ...
        fCurrentUniformBuffer = fResourceProvider->findOrCreateBuffer(bufferSize, BufferType::kUniform, PrioritizeGpuReads::kNo);
        fUniformOffset = 0;
        if (!fCurrentUniformBuffer) {
            return { UniformWriter(), BindBufferInfo() };
        }
...
std::tuple<UniformWriter, BindBufferInfo> DrawBufferManager::getSsboWriter(size_t requiredBytes)
{
    ...
    if (fCurrentStorageBuffer && !can_fit(requiredBytes, fCurrentStorageBuffer.get(), fSsboOffset, fSsboStartAlignment)) {
    ...
    if (!fCurrentStorageBuffer) {
        ...
        fCurrentStorageBuffer = fResourceProvider->findOrCreateBuffer(bufferSize, BufferType::kStorage, PrioritizeGpuReads::kNo);
        fSsboOffset = 0;
        if (!fCurrentStorageBuffer) {
            return { UniformWriter(), BindBufferInfo() };
        }
...
BindBufferInfo DrawBufferManager::getStaticBuffer(BufferType type, InitializeBufferFn initFn, BufferSizeFn sizeFn)
{
