...
public:
    ...
    sk_sp<GraphicsPipeline> findOrCreateGraphicsPipeline(const SkRuntimeEffectDictionary*, const GraphicsPipelineDesc&, const RenderPassDesc&);
    ...
    sk_sp<Sampler> findOrCreateCompatibleSampler(const SkSamplingOptions&, SkTileMode xTileMode, SkTileMode yTileMode);
...
protected:
    ResourceProvider(SharedContext* sharedContext, SingleOwner* singleOwner);

    SharedContext* fSharedContext;

private:
    virtual sk_sp<GraphicsPipeline> createGraphicsPipeline(const SkRuntimeEffectDictionary*, const GraphicsPipelineDesc&, const RenderPassDesc&) = 0;
    ...
    virtual sk_sp<Sampler> createSampler(const SkSamplingOptions&, SkTileMode xTileMode, SkTileMode yTileMode) = 0;

    sk_sp<Texture> findOrCreateTextureWithKey(SkISize dimensions, const TextureInfo& info, const GraphiteResourceKey& key, SkBudgeted);
