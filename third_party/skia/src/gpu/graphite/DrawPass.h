...
public:
    ...
    static std::unique_ptr<DrawPass> Make(
        Recorder*, std::unique_ptr<DrawList>, sk_sp<TextureProxy>, std::pair<LoadOp, StoreOp>, std::array<float, 4> clearColor);
    ...
    // etc.
    bool prepareResources(ResourceProvider*, const SkRuntimeEffectDictionary*, const RenderPassDesc&);
