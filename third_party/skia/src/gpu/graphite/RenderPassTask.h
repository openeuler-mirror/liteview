...
public:
    static sk_sp<RenderPassTask> Make(std::vector<std::unique_ptr<DrawPass>> passes, const RenderPassDesc&, sk_sp<TextureProxy> target);
...
private:
    RenderPassTask(std::vector<std::unique_ptr<DrawPass>> passes, const RenderPassDesc&, sk_sp<TextureProxy> target);
