...
sk_sp<TextureProxy> TextureProxy::Make(
    const Caps* caps, SkISize dimensions, SkColorType colorType, Mipmapped mipmapped, Protected isProtected, Renderable renderable, SkBudgeted budgeted)
{
    if (dimensions.width() < 1 || dimensions.height() < 1) {
        return nullptr;
    }

    int mipLevelCount = (mipmapped == Mipmapped::kYes) ? SkMipmap::ComputeLevelCount(dimensions.width(), dimensions.height()) + 1 : 1;

    TextureInfo textureInfo = caps->getDefaultSampledTextureInfo(colorType, mipLevelCount, isProtected, renderable);
