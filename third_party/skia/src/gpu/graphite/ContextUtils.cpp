...
std::tuple<SkUniquePaintParamsID, const SkUniformDataBlock*, const SkTextureDataBlock*> ExtractPaintData(
    Recorder* recorder, SkPipelineDataGatherer* gatherer, SkPaintParamsKeyBuilder* builder, const SkM44& dev2Local, const PaintParams& p)
{
    ...
    const SkUniformDataBlock* uniforms = gatherer->hasUniforms() ? uniformDataCache->insert(gatherer->finishUniformDataBlock()) : nullptr;
    const SkTextureDataBlock* textures = gatherer->hasTextures() ? textureDataCache->insert(gatherer->textureDataBlock()) : nullptr;
...
std::tuple<const SkUniformDataBlock*, const SkTextureDataBlock*> ExtractRenderStepData(
    UniformDataCache* uniformDataCache, TextureDataCache* textureDataCache, SkPipelineDataGatherer* gatherer, const RenderStep* step, const DrawParams& params)
{
    SkDEBUGCODE(gatherer->checkReset());
...
std::string get_uniforms(SkSpan<const SkUniform> uniforms, int* offset, int manglingSuffix)
{
    ...
    for (const SkUniform& u : uniforms) {
        SkSL::String::appendf(&result, "    layout(offset=%zu) %s %s", offsetter.calculateOffset(u.type(), u.count()), SkSLTypeString(u.type()), u.name());
        if (manglingSuffix >= 0) {
            result.append("_");
...
std::string EmitPaintParamsUniforms(int bufferID, const char* name, const std::vector<SkPaintParamsKey::BlockReader>& readers, bool needsLocalCoords)
{
...
std::string EmitRenderStepUniforms(int bufferID, const char* name, SkSpan<const SkUniform> uniforms)
{
    int offset = 0;
...
std::string EmitPaintParamsStorageBuffer(
    int bufferID, const char* bufferTypePrefix, const char* bufferNamePrefix, const std::vector<SkPaintParamsKey::BlockReader>& readers, bool needsLocalCoords)
{
    if (!needsLocalCoords && !have_uniforms(readers)) {
        return {};
    }
    ...
    for (int i = 0; i < (int)readers.size(); ++i) {
        ...
        for (const SkUniform& u : uniforms) {
            SkSL::String::appendf(&result, "    %s %s_%d", SkSLTypeString(u.type()), u.name(), manglingSuffix);
            if (u.count()) {
    ...
    if (needsLocalCoords) {
        result.append("// NeedsLocalCoords\n"
    ...
    SkSL::String::appendf(&result,
        ...
        "};\n",
        bufferID, bufferTypePrefix, bufferTypePrefix, bufferNamePrefix);
    return result;
}
...
std::string EmitTexturesAndSamplers(const std::vector<SkPaintParamsKey::BlockReader>& readers, int* binding)
{
    std::string result;
    for (int i = 0; i < (int)readers.size(); ++i) {
        ...
        if (!samplers.empty()) {
            ...
            for (const SkTextureAndSampler& t : samplers) {
                SkSL::String::appendf(&result, "layout(binding=%d) uniform sampler2D %s_%d;\n", *binding, t.name(), i);
                (*binding)++;
            }
        }
    }
...
namespace {
std::string emit_attributes(SkSpan<const Attribute> vertexAttrs, SkSpan<const Attribute> instanceAttrs)
{
    std::string result;
...
std::string EmitVaryings(const RenderStep* step, const char* direction, bool emitShadingSsboIndexVarying, bool emitLocalCoordsVarying)
{
    ...
    if (emitShadingSsboIndexVarying) {
        SkSL::String::appendf(&result, "    layout(location=%d) %s int shadingSsboIndexVar;\n", location++, direction);
    }
...
std::string GetSkSLVS(const RenderStep* step, bool defineShadingSsboIndexVarying, bool defineLocalCoordsVarying)
{
    ...
    std::string sksl = "layout (binding=0) uniform intrinsicUniforms {\n"
...
std::string GetSkSLFS(const SkShaderCodeDictionary* dict, const SkRuntimeEffectDictionary* rteDict, const RenderStep* step, SkUniquePaintParamsID paintID,
    bool useStorageBuffers, BlendInfo* blendInfo, bool* requiresLocalCoordsVarying)
{
    if (!paintID.isValid()) {
