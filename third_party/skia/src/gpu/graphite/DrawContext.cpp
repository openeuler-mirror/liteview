...
sk_sp<DrawContext> DrawContext::Make(sk_sp<TextureProxy> target, const SkColorInfo& colorInfo, const SkSurfaceProps& props)
{
    if (!target) {
        return nullptr;
    }
...
DrawContext::DrawContext(sk_sp<TextureProxy> target, const SkImageInfo& ii, const SkSurfaceProps& props)
...
TextureProxyView DrawContext::readSurfaceView(const Caps* caps)
{
    ...
    Swizzle swizzle = caps->getReadSwizzle(this->imageInfo().colorType(), proxy->textureInfo());
...
bool DrawContext::recordUpload(
    Recorder* recorder, sk_sp<TextureProxy> targetProxy, SkColorType colorType, const std::vector<MipLevel>& levels, const SkIRect& dstRect)
{
    ...
    return fPendingUploads->recordUpload(recorder, std::move(targetProxy), colorType, levels, dstRect);
}

#ifdef SK_ENABLE_PIET_GPU
bool DrawContext::recordPietSceneRender(Recorder*, sk_sp<TextureProxy> targetProxy, sk_sp<const skgpu::piet::Scene> scene)
{
...
void DrawContext::snapDrawPass(Recorder* recorder)
{
    ...
    auto pass = DrawPass::Make(recorder, std::move(fPendingDraws), fTarget, std::make_pair(fPendingLoadOp, fPendingStoreOp), fPendingClearColor);
...
sk_sp<Task> DrawContext::snapRenderPassTask(Recorder* recorder)
{
    ...
    if (drawPass->requiresMSAA()) {
        ...
        desc.fColorAttachment.fTextureInfo = caps->getDefaultMSAATextureInfo(targetInfo, Discardable::kYes);
    ...
    if (drawPass->depthStencilFlags() != DepthStencilFlags::kNone) {
        desc.fDepthStencilAttachment.fTextureInfo
            = caps->getDefaultDepthStencilTextureInfo(drawPass->depthStencilFlags(), desc.fColorAttachment.fTextureInfo.numSamples(), Protected::kNo);
