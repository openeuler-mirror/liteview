 ...
 */
template <typename BaseType, int RowsOrVecLength = 1, int Cols = 1> struct Rules140 {
    /**
 ...
 */
template <typename BaseType, int RowsOrVecLength = 1, int Cols = 1> struct Rules430 {
    static constexpr size_t Stride(int count)
    {
        SkASSERT(count >= 1 || count == SkUniform::kNonArray);
        static_assert(RowsOrVecLength >= 1 && RowsOrVecLength <= 4);
...
template <typename BaseType, int RowsOrVecLength = 1, int Cols = 1> struct RulesMetal {
    static constexpr size_t Stride(int count)
    {
        SkASSERT(count >= 1 || count == SkUniform::kNonArray);
        static_assert(RowsOrVecLength >= 1 && RowsOrVecLength <= 4);
...
template <template <typename BaseType, int RowsOrVecLength, int Cols> class Rules> class Writer {
private:
    template <typename MemType, typename UniformType> static void CopyUniforms(void* dst, const void* src, int numUniforms)
    {
        ...
        if constexpr (std::is_same<MemType, int32_t>::value && std::is_same<UniformType, int16_t>::value) {
    ...
    template <typename MemType, typename UniformType, int RowsOrVecLength = 1, int Cols = 1> static uint32_t Write(void* dst, int n, const MemType src[])
    {
    ...
    template <typename UniformType> static uint32_t WriteSkMatrices(void* dst, int n, const SkMatrix m[])
    {
...
{
    SkDEBUGCODE(fExpectedUniforms = expectedUniforms;) SkDEBUGCODE(fExpectedUniformIndex = 0;)
}
...
void UniformManager::checkExpected(SkSLType type, unsigned int count)
{
    ...
    SkASSERT((fExpectedUniforms[fExpectedUniformIndex].count() == 0 && count == 1) || fExpectedUniforms[fExpectedUniformIndex].count() == count);
