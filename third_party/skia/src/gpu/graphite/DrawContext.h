...
public:
    static sk_sp<DrawContext> Make(sk_sp<TextureProxy> target, const SkColorInfo&, const SkSurfaceProps&);
    ...
    int pendingDrawCount() const
    {
        return fPendingDraws->drawCount();
    }
    ...
    bool recordUpload(Recorder* recorder, sk_sp<TextureProxy> targetProxy, SkColorType colorType, const std::vector<MipLevel>& levels, const SkIRect& dstRect);

#ifdef SK_ENABLE_PIET_GPU
    bool recordPietSceneRender(Recorder* recorder, sk_sp<TextureProxy> targetProxy, sk_sp<const skgpu::piet::Scene> pietScene);
#endif
