...
bool oriented_bbox_intersection(const Rect& a, const Transform& aXform, const Rect& b, const Transform& bXform)
{
    ...
    SkASSERT(aXform.type() != Transform::Type::kProjection && bXform.type() != Transform::Type::kProjection);
...
bool ClipStack::TransformedShape::intersects(const TransformedShape& o) const
{
    ...
    } else if (fLocalToDevice.type() != Transform::Type::kProjection && o.fLocalToDevice.type() != Transform::Type::kProjection) {
...
ClipStack::RawElement::RawElement(const Rect& deviceBounds, const Transform& localToDevice, const Shape& shape, SkClipOp op)
...
std::pair<Clip, CompressedPaintersOrder> ClipStack::applyClipToDraw(
    const BoundsManager* boundsManager, const Transform& localToDevice, const Geometry& geometry, const SkStrokeRec& style, PaintersDepth z)
{
    ...
    TransformedShape draw { style.isHairlineStyle() ? kIdentity : localToDevice, *styledShape,
