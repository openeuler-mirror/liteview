...
PaintParams::PaintParams(const SkColor4f& color, sk_sp<SkBlender> finalBlender, sk_sp<SkShader> shader, sk_sp<SkColorFilter> colorFilter,
    sk_sp<SkBlender> primitiveBlender, bool skipColorXform)
    : fColor(color)
...
PaintParams::PaintParams(const SkPaint& paint, sk_sp<SkBlender> primitiveBlender, bool skipColorXform)
...
void PaintParams::toKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{

    if (fPrimitiveBlender) {
        as_BB(fPrimitiveBlender)
