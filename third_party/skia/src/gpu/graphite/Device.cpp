...
bool paint_depends_on_dst(SkColor4f color, const SkShader* shader, const SkColorFilter* colorFilter, const SkBlender* blender)
{
    ...
    if (bm.value() == SkBlendMode::kSrcOver) {
        ...
        return !color.isOpaque() || (shader && !shader->isOpaque()) || (colorFilter && !colorFilter->isAlphaUnchanged());
    }
...
{
    return paint_depends_on_dst(paintParams.color(), paintParams.shader(), paintParams.colorFilter(), paintParams.finalBlender());
}
...
bool create_img_shader_paint(sk_sp<SkImage> image, const SkRect& subset, const SkSamplingOptions& sampling, const SkMatrix* localMatrix, SkPaint* paint)
{
    bool imageIsAlphaOnly = SkColorTypeIsAlphaOnly(image->colorType());

    sk_sp<SkShader> imgShader = SkImageShader::MakeSubset(std::move(image), subset, SkTileMode::kClamp, SkTileMode::kClamp, sampling, localMatrix);
    if (!imgShader) {
...
sk_sp<Device> Device::Make(
    Recorder* recorder, const SkImageInfo& ii, SkBudgeted budgeted, Mipmapped mipmapped, const SkSurfaceProps& props, bool addInitialClear)
{
    if (!recorder) {
        return nullptr;
    }
    ...
    sk_sp<TextureProxy> target
        = TextureProxy::Make(recorder->priv().caps(), ii.dimensions(), ii.colorType(), mipmapped, Protected::kNo, Renderable::kYes, budgeted);
    if (!target) {
        return nullptr;
    }
...
sk_sp<Device> Device::Make(Recorder* recorder, sk_sp<TextureProxy> target, const SkColorInfo& colorInfo, const SkSurfaceProps& props, bool addInitialClear)
{
    if (!recorder) {
        return nullptr;
    }
...
Device::Device(Recorder* recorder, sk_sp<DrawContext> dc, bool addInitialClear)
    ...
    , fDC(std::move(dc))
    , fClip(this)
    , fColorDepthBoundsManager(std::make_unique<HybridBoundsManager>(fDC->imageInfo().dimensions(), kGridCellSize, kMaxBruteForceN))
...
SkBaseDevice* Device::onCreateDevice(const CreateInfo& info, const SkPaint*)
{
    ...
    return Make(fRecorder, info.fInfo, SkBudgeted::kYes, Mipmapped::kNo, props, addInitialClear).release();
}
...
TextureProxyView Device::createCopy(const SkIRect* subset, Mipmapped mipmapped)
{
    ...
    sk_sp<TextureProxy> dest = TextureProxy::Make(this->recorder()->priv().caps(), srcRect.size(), this->imageInfo().colorType(), mipmapped,
        srcView.proxy()->textureInfo().isProtected(), Renderable::kNo, SkBudgeted::kNo);
    if (!dest) {
        return {};
    }

    sk_sp<CopyTextureToTextureTask> copyTask = CopyTextureToTextureTask::Make(srcView.refProxy(), srcRect, dest, { 0, 0 });
    if (!copyTask) {
        return {};
    }
...
bool Device::readPixels(Context* context, Recorder* recorder, const SkPixmap& pm, int srcX, int srcY)
{
    return ReadPixelsHelper(
        [this]() { this->flushPendingWorkToRecorder(); }, context, recorder, fDC->target(), pm.info(), pm.writable_addr(), pm.rowBytes(), srcX, srcY);
}

void Device::asyncReadPixels(const SkImageInfo& info, SkIRect srcRect, ReadPixelsCallback callback, ReadPixelsContext context)
{
...
void Device::asyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize,
...
SkBaseDevice::ClipType Device::onGetClipType() const
{
    ...
    } else if (state == ClipStack::ClipState::kDeviceRect || state == ClipStack::ClipState::kWideOpen) {
...
void Device::drawPaint(const SkPaint& paint)
{
    ...
    this->drawGeometry(localToDevice, Geometry(Shape(localCoveringBounds)), paint, kFillStyle, DrawFlags::kIgnorePathEffect | DrawFlags::kIgnoreMaskFilter);
}

void Device::drawRect(const SkRect& r, const SkPaint& paint)
{
    this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(r)), paint, SkStrokeRec(paint));
}
...
void Device::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform)
{
    ...
    this->drawGeometry(this->localToDeviceTransform(), Geometry(sk_ref_sp(vertices)), paint, kFillStyle,
        DrawFlags::kIgnorePathEffect | DrawFlags::kIgnoreMaskFilter, std::move(blender), skipColorXform);
}
...
void Device::drawOval(const SkRect& oval, const SkPaint& paint)
{
    ...
    this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(SkRRect::MakeOval(oval))), paint, SkStrokeRec(paint));
}

void Device::drawRRect(const SkRRect& rr, const SkPaint& paint)
{
    this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(rr)), paint, SkStrokeRec(paint));
}
...
void Device::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint* points, const SkPaint& paint)
{
    ...
    if (mode == SkCanvas::kPoints_PointMode) {
        ...
        for (size_t i = 0; i < count; ++i) {
            SkRect pointRect = SkRect::MakeLTRB(points[i].fX - radius, points[i].fY - radius, points[i].fX + radius, points[i].fY + radius);
            // drawOval/drawRect with a forced fill style
            if (paint.getStrokeCap() == SkPaint::kRound_Cap) {
                this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(SkRRect::MakeOval(pointRect))), paint, kFillStyle);
            } else {
                this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(pointRect)), paint, kFillStyle);
            }
        }
    } else {
        ...
        for (size_t i = 0; i < count; i += inc) {
            this->drawGeometry(this->localToDeviceTransform(), Geometry(Shape(points[i], points[(i + 1) % count])), paint, stroke);
        }
    }
}
    ...
    const SkImage* image, const SkRect* src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    auto [imageToDraw, newSampling] = skgpu::graphite::GetGraphiteBacked(this->recorder(), image, sampling);
    if (!imageToDraw) {
    ...
    if (!create_img_shader_paint(std::move(imageToDraw), tmpSrc, newSampling, &matrix, &paintWithShader)) {
        return;
    }
...
void Device::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    fRecorder->priv().textBlobCache()->drawGlyphRunList(canvas, this->localToDevice(), glyphRunList, drawingPaint, this->strikeDeviceInfo(), this);
}

void Device::drawAtlasSubRun(const sktext::gpu::AtlasSubRun* subRun, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage)
{
    ...
    for (int subRunCursor = 0; subRunCursor < subRunEnd;) {
        ...
        if (glyphsRegenerated) {
            auto [bounds, localToDevice] = subRun->boundsAndDeviceMatrix(this->localToDeviceTransform(), drawOrigin);
            ...
            this->drawGeometry(localToDevice, Geometry(SubRunData(subRun, subRunStorage, bounds, subRunCursor, glyphsRegenerated, fRecorder)), subRunPaint,
                kFillStyle, DrawFlags::kIgnorePathEffect | DrawFlags::kIgnoreMaskFilter);
        }
    ...
    SkEnumBitMask<DrawFlags> flags, sk_sp<SkBlender> primitiveBlender, bool skipColorXform)
{
    ...
    if (!(flags & DrawFlags::kIgnorePathEffect) && paint.getPathEffect()) {
        ...
        if (paint.getPathEffect()->filterPath(&dst, geometry.shape().asPath(), &newStyle, nullptr, localToDevice)) {
            ...
            this->drawGeometry(
                localToDevice, Geometry(Shape(dst)), paint, style, flags | DrawFlags::kIgnorePathEffect, std::move(primitiveBlender), skipColorXform);
            return;
        } else {
    ...
    if (!(flags & DrawFlags::kIgnoreMaskFilter) && paint.getMaskFilter()) {
        ...
        this->drawGeometry(localToDevice, geometry, paint, style, flags | DrawFlags::kIgnoreMaskFilter, std::move(primitiveBlender), skipColorXform);
        return;
    }
    ...
    auto [clip, clipOrder] = fClip.applyClipToDraw(fColorDepthBoundsManager.get(), localToDevice, geometry, style, order.depth());
    ...
    if (styleType == SkStrokeRec::kStroke_Style || styleType == SkStrokeRec::kHairline_Style || styleType == SkStrokeRec::kStrokeAndFill_Style) {
        ...
        fDC->recordDraw(styleType == SkStrokeRec::kStrokeAndFill_Style ? fRecorder->priv().rendererProvider()->tessellatedStrokes() : renderer, localToDevice,
            geometry, clip, order, &shading, &stroke);
    }
...
const Renderer* Device::chooseRenderer(const Geometry& geometry, const Clip& clip, const SkStrokeRec& style, bool requireMSAA) const
{
    ...
    const bool simpleShape = !requireMSAA && !shape.inverted() && type != SkStrokeRec::kStrokeAndFill_Style
        && (shape.isRect() || shape.isRRect() /* || shape.isQuadrilateral()*/)
        && (!shape.isRRect() || type == SkStrokeRec::kFill_Style || SkRRectPriv::AllCornersCircular(shape.rrect()));
    if (simpleShape) {
    ...
    if (shape.convex() && !shape.inverted()) {
        ...
        const bool preferWedges = (shape.isPath() && shape.path().countVerbs() < 50) || clip.drawBounds().area() <= (256 * 256);
...
void Device::drawDevice(SkBaseDevice* device, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    this->SkBaseDevice::drawDevice(device, sampling, paint);
}

void Device::drawSpecial(SkSpecialImage* special, const SkMatrix& localToDevice, const SkSamplingOptions& sampling, const SkPaint& paint)
{
    ...
    this->drawGeometry(
        Transform(SkM44(localToDevice)), Geometry(Shape(dst)), paintWithShader, kFillStyle, DrawFlags::kIgnorePathEffect | DrawFlags::kIgnoreMaskFilter);
}
...
sk_sp<SkSpecialImage> Device::snapSpecial(const SkIRect& subset, bool forceCopy)
{
    ...
    return SkSpecialImage::MakeGraphite(
        fRecorder, finalSubset, kNeedNewImageUniqueID_SpecialImage, std::move(view), this->imageInfo().colorInfo(), this->surfaceProps());
}
