...
public:
    static sk_sp<Buffer> Make(const MtlSharedContext*, size_t size, BufferType type, PrioritizeGpuReads);
...
private:
    MtlBuffer(const MtlSharedContext*, size_t size, BufferType type, PrioritizeGpuReads, sk_cfp<id<MTLBuffer>>);
