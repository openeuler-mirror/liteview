...
public:
    ...
    static sk_sp<MtlGraphicsPipeline> Make(const MtlSharedContext*, std::string label, MSLFunction vertexMain, SkSpan<const Attribute> vertexAttrs,
        SkSpan<const Attribute> instanceAttrs, MSLFunction fragmentMain, sk_cfp<id<MTLDepthStencilState>>, uint32_t stencilRefValue, const BlendInfo& blendInfo,
...
private:
    MtlGraphicsPipeline(
        const skgpu::graphite::SharedContext* sharedContext, sk_cfp<id<MTLRenderPipelineState>> pso, sk_cfp<id<MTLDepthStencilState>> dss, uint32_t refValue)
