...
void MiddleOutFanRenderStep::writeVertices(DrawWriter* writer, const DrawParams& params, int ssboIndex) const
{
    ...
    for (tess::PathMiddleOutFanIter it(path); !it.done();) {
        for (auto [p0, p1, p2] : it.nextStack()) {
            verts.append(3) << p0 << depth << ssboIndex << p1 << depth << ssboIndex << p2 << depth << ssboIndex;
        }
    }
}

void MiddleOutFanRenderStep::writeUniformsAndTextures(const DrawParams& params, SkPipelineDataGatherer* gatherer) const
{
