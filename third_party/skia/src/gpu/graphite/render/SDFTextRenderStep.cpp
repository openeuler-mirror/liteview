...
SDFTextRenderStep::SDFTextRenderStep(bool isA8)
    : RenderStep("SDFTextRenderStep", isA8 ? "A8" : "565", Flags::kPerformsShading | Flags::kHasTextures | Flags::kEmitsCoverage,
        /*uniforms=*/ { { "deviceMatrix", SkSLType::kFloat4x4 }, { "atlasSizeInv", SkSLType::kFloat2 }, { "distanceAdjust", SkSLType::kFloat } },
...
std::string SDFTextRenderStep::texturesAndSamplersSkSL(int binding) const
{
    ...
    for (unsigned int i = 0; i < kNumSDFAtlasTextures; ++i) {
        SkSL::String::appendf(&result, "layout(binding=%d) uniform sampler2D sdf_atlas_%d;\n", binding, i);
        binding++;
    }
...
void SDFTextRenderStep::writeVertices(DrawWriter* dw, const DrawParams& params, int ssboIndex) const
{
...
void SDFTextRenderStep::writeUniformsAndTextures(const DrawParams& params, SkPipelineDataGatherer* gatherer) const
{
