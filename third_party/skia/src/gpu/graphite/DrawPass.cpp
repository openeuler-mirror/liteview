...
public:
    ...
    static constexpr Index kInvalidIndex { 1 << SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws) };
...
struct CpuOrGpuData {
    ...
    CpuOrGpuData(const SkUniformDataBlock* cpuData)
        : fCpuData(cpuData)
    {
    }
};
...
public:
    TextureBindingCache::Index trackTextures(const SkTextureDataBlock* paintTextures, const SkTextureDataBlock* stepTextures)
    {
    ...
    SkTArray<sk_sp<TextureProxy>>&& detachTextures()
    {
        return fProxyCache.detach();
    }
    SkTArray<SamplerDesc>&& detachSamplers()
    {
...
public:
    UniformSsboTracker(bool useStorageBuffers)
    ...
    UniformSsboCache::Index trackUniforms(GraphicsPipelineCache::Index pipelineIndex, const SkUniformDataBlock* cpuData)
    {
        if (!cpuData) {
    ...
    void writeUniforms(DrawBufferManager* bufferMgr)
    {
        for (UniformSsboCache& cache : fPerPipelineCaches) {
            ...
            auto [writer, bufferInfo]
                = fUseStorageBuffers ? bufferMgr->getSsboWriter(udbSize * cache.size()) : bufferMgr->getUniformWriter(udbSize * cache.size());
    ...
    bool setCurrentUniforms(GraphicsPipelineCache::Index pipelineIndex, UniformSsboCache::Index ssboIndex)
    {
        if (ssboIndex >= UniformSsboCache::kInvalidIndex) {
            return false;
        }
        SkASSERT(pipelineIndex < fPerPipelineCaches.size() && ssboIndex < fPerPipelineCaches[pipelineIndex].size());
    ...
    {
        SkASSERT(fLastPipeline < GraphicsPipelineCache::kInvalidIndex && fLastIndex < UniformSsboCache::kInvalidIndex);
        SkASSERT(!fUseStorageBuffers || fLastIndex == 0);
        const BindBufferInfo& binding = fPerPipelineCaches[fLastPipeline].lookup(fLastIndex).fGpuData;
...
public:
    SortKey(const DrawList::Draw* draw, int renderStep, GraphicsPipelineCache::Index pipelineIndex, UniformSsboCache::Index geomSsboIndex,
        UniformSsboCache::Index shadingSsboIndex, TextureBindingCache::Index textureBindingIndex)
        ...
        , fUniformKey(GeometryUniformField::set(geomSsboIndex) | ShadingUniformField::set(shadingSsboIndex) | TextureBindingsField::set(textureBindingIndex))
        , fDraw(draw)
    {
...
private:
    ...
    static_assert(PipelineField::kBits >= SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
    static_assert(GeometryUniformField::kBits >= 1 + SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
    static_assert(ShadingUniformField::kBits >= 1 + SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
    static_assert(TextureBindingsField::kBits >= 1 + SkNextLog2_portable(Renderer::kMaxRenderSteps * DrawList::kMaxDraws));
};
#pragma pack(pop)
///////////////////////////////////////////////////////////////////////////////////////////////////
...
std::unique_ptr<DrawPass> DrawPass::Make(
    Recorder* recorder, std::unique_ptr<DrawList> draws, sk_sp<TextureProxy> target, std::pair<LoadOp, StoreOp> ops, std::array<float, 4> clearColor)
{
    ...
    for (const DrawList::Draw& draw : draws->fDraws.items()) {
        ...
        if (draw.fPaintParams.has_value()) {
            std::tie(shaderID, shadingUniforms, paintTextures)
                = ExtractPaintData(recorder, &gatherer, &builder, draw.fDrawParams.transform().inverse(), draw.fPaintParams.value());
        } // else depth-only
        ...
        for (int stepIndex = 0; stepIndex < draw.fRenderer->numRenderSteps(); ++stepIndex) {
            ...
            GraphicsPipelineCache::Index pipelineIndex = pipelineCache.insert({ step, performsShading ? shaderID : SkUniquePaintParamsID::InvalidID() });

            auto [geometryUniforms, stepTextures] = ExtractRenderStepData(&geometryUniformDataCache, textureDataCache, &gatherer, step, draw.fDrawParams);

            UniformSsboCache::Index geomSsboIndex = geometrySsboTracker.trackUniforms(pipelineIndex, geometryUniforms);
            UniformSsboCache::Index shadingSsboIndex = shadingSsboTracker.trackUniforms(pipelineIndex, performsShading ? shadingUniforms : nullptr);
            TextureBindingCache::Index textureIndex = textureBindingTracker.trackTextures(performsShading ? paintTextures : nullptr, stepTextures);

            keys.push_back({ &draw, stepIndex, pipelineIndex, geomSsboIndex, shadingSsboIndex, textureIndex });
        }
    ...
    for (const SortKey& key : keys) {
        ...
        const bool geomBindingChange = geometrySsboTracker.setCurrentUniforms(key.pipelineIndex(), key.geometrySsboIndex());
        const bool shadingBindingChange = shadingSsboTracker.setCurrentUniforms(key.pipelineIndex(), key.shadingSsboIndex());
...
bool DrawPass::prepareResources(ResourceProvider* resourceProvider, const SkRuntimeEffectDictionary* runtimeDict, const RenderPassDesc& renderPassDesc)
{
    ...
    for (int i = 0; i < fSamplerDescs.count(); ++i) {
        sk_sp<Sampler> sampler = resourceProvider->findOrCreateCompatibleSampler(
            fSamplerDescs[i].fSamplingOptions, fSamplerDescs[i].fTileModes[0], fSamplerDescs[i].fTileModes[1]);
        if (!sampler) {
