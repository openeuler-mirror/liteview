...
UploadInstance::UploadInstance(const Buffer* buffer, sk_sp<TextureProxy> textureProxy, std::vector<BufferTextureCopyData> copyData)
    : fBuffer(buffer)
...
size_t compute_combined_buffer_size(
    int mipLevelCount, size_t bytesPerPixel, size_t minTransferBufferAlignment, const SkISize& baseDimensions, SkTArray<size_t>* individualMipOffsets)
{
...
UploadInstance UploadInstance::Make(
    Recorder* recorder, sk_sp<TextureProxy> textureProxy, SkColorType dataColorType, const std::vector<MipLevel>& levels, const SkIRect& dstRect)
{
    ...
    size_t combinedBufferSize = compute_combined_buffer_size(mipLevelCount, bpp, minAlignment, dstRect.size(), &individualMipOffsets);
    ...
    for (unsigned int currentMipLevel = 0; currentMipLevel < mipLevelCount; currentMipLevel++) {
        ...
        copyData[currentMipLevel].fRect = { dstRect.left(), dstRect.top(), // TODO: can we recompute this for mips?
    ...
    ATRACE_ANDROID_FRAMEWORK("Upload %sTexture [%ux%u]", mipLevelCount > 1 ? "MipMap " : "", dstRect.width(), dstRect.height());
...
void UploadInstance::addCommand(CommandBuffer* commandBuffer) const
{
    ...
    commandBuffer->copyBufferToTexture(fBuffer, fTextureProxy->refTexture(), fCopyData.data(), fCopyData.size());
}
...
bool UploadList::recordUpload(
    Recorder* recorder, sk_sp<TextureProxy> textureProxy, SkColorType dataColorType, const std::vector<MipLevel>& levels, const SkIRect& dstRect)
{
    UploadInstance instance = UploadInstance::Make(recorder, std::move(textureProxy), dataColorType, levels, dstRect);
    if (!instance.isValid()) {
        return false;
    }
...
UploadTask::UploadTask(std::vector<UploadInstance> instances)
...
bool UploadTask::prepareResources(ResourceProvider* resourceProvider, const SkRuntimeEffectDictionary*)
{
