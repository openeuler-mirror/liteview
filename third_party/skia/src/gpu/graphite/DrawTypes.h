...
struct SamplerDesc {
    ...
    {
        return fSamplingOptions == o.fSamplingOptions && fTileModes[0] == o.fTileModes[0] && fTileModes[1] == o.fTileModes[1];
    }
    bool operator!=(const SamplerDesc& o) const
    {
        return !(*this == o);
    ...
    uint32_t asKey() const
    {
        ...
        SkASSERT(!fSamplingOptions.useCubic || (fSamplingOptions.filter == SkFilterMode::kNearest && fSamplingOptions.mipmap == SkMipmapMode::kNone));
        // TODO: Add support for anisotropic filtering
        return (static_cast<int>(fTileModes[0]) << 0) | (static_cast<int>(fTileModes[1]) << 2) | (static_cast<int>(fSamplingOptions.filter) << 4)
