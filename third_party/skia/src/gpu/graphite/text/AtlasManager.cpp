    ...
    if (maskFormat == expectedMaskFormat) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                uint32_t colorRGBA = masks.getRed(color565) | (masks.getGreen(color565) << 8) | (masks.getBlue(color565) << 16) | (0xFF << 24);
...
MaskFormat AtlasManager::resolveMaskFormat(MaskFormat format) const
{
    if (MaskFormat::kA565 == format
        && !fRecorder->priv().caps()->getDefaultSampledTextureInfo(kRGB_565_SkColorType, 1, Protected::kNo, Renderable::kNo).isValid()) {
...
DrawAtlas::ErrorCode AtlasManager::addGlyphToAtlas(const SkGlyph& skGlyph, Glyph* glyph, int srcPadding)
{
...
void AtlasManager::addGlyphToBulkAndSetUseToken(BulkUsePlotUpdater* updater, MaskFormat format, Glyph* glyph, DrawToken token)
{
    SkASSERT(glyph);
...
std::tuple<bool, int> GlyphVector::regenerateAtlas(int begin, int end, skgpu::MaskFormat maskFormat, int srcPadding, skgpu::graphite::Recorder* recorder)
{
    ...
    if (fAtlasGeneration != currentAtlasGen) {
        ...
        for (const Variant& variant : glyphs) {
            ...
            atlasManager->addGlyphToBulkAndSetUseToken(&fBulkUseUpdater, maskFormat, gpuGlyph, tokenTracker->nextTokenToFlush());
            glyphsPlacedInAtlas++;
        }
        ...
        if (end == SkCount(fGlyphs)) {
            ...
            atlasManager->setUseTokenBulk(fBulkUseUpdater, tokenTracker->nextTokenToFlush(), maskFormat);
        }
