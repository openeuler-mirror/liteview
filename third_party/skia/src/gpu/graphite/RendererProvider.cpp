...
RendererProvider::RendererProvider()
{
    ...
    fConvexTessellatedWedges = makeFromStep(std::make_unique<TessellateWedgesRenderStep>("convex", kDirectDepthGreaterPass));
    ...
    for (PrimitiveType primType : { PrimitiveType::kTriangles, PrimitiveType::kTriangleStrip }) {
        for (bool color : { false, true }) {
            for (bool texCoords : { false, true }) {
                ...
                fVertices[index] = makeFromStep(std::make_unique<VerticesRenderStep>(primType, color, texCoords));
            }
        }
    }
    ...
    for (bool evenOdd : { false, true }) {
        ...
        auto stencilWedge = evenOdd ? std::make_unique<TessellateWedgesRenderStep>("evenodd", kEvenOddStencilPass)
        ...
        for (bool inverse : { false, true }) {
            ...
            fStencilTessellatedCurves[index] = Renderer("StencilTessellatedCurvesAndTris" + variant, stencilFan.get(), stencilCurve.get(), coverStep);

            fStencilTessellatedWedges[index] = Renderer("StencilTessellatedWedges" + variant, stencilWedge.get(), coverStep);
        }
