...
sk_sp<RenderPassTask> RenderPassTask::Make(std::vector<std::unique_ptr<DrawPass>> passes, const RenderPassDesc& desc, sk_sp<TextureProxy> target)
{
...
RenderPassTask::RenderPassTask(std::vector<std::unique_ptr<DrawPass>> passes, const RenderPassDesc& desc, sk_sp<TextureProxy> target)
...
bool RenderPassTask::prepareResources(ResourceProvider* resourceProvider, const SkRuntimeEffectDictionary* runtimeDict)
{
    SkASSERT(fTarget);
...
bool RenderPassTask::addCommands(ResourceProvider* resourceProvider, CommandBuffer* commandBuffer)
{
    ...
    if (fRenderPassDesc.fDepthStencilAttachment.fTextureInfo.isValid()) {
        ...
        depthStencilAttachment
            = resourceProvider->findOrCreateDepthStencilAttachment(fTarget->dimensions(), fRenderPassDesc.fDepthStencilAttachment.fTextureInfo);
    ...
    return commandBuffer->addRenderPass(
        fRenderPassDesc, std::move(colorAttachment), std::move(resolveAttachment), std::move(depthStencilAttachment), fDrawPasses);
}
