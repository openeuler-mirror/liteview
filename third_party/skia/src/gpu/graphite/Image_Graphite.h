...
public:
    ...
    bool testingOnly_ReadPixels(Context*, Recorder*, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRowBytes, int srcX, int srcY);
    ...
    void onAsyncReadPixels(const SkImageInfo&, SkIRect srcRect, ReadPixelsCallback, ReadPixelsContext) const override;
    ...
    void onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace, sk_sp<SkColorSpace>, SkIRect srcRect, SkISize dstSize, RescaleGamma, RescaleMode,
...
private:
    ...
    std::unique_ptr<GrFragmentProcessor> onAsFragmentProcessor(
        GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&, const SkRect* subset, const SkRect* domain) const override;

    std::tuple<GrSurfaceProxyView, GrColorType> onAsView(GrRecordingContext*, GrMipmapped, GrImageTexGenPolicy policy) const override
    {
        return {};
    }
#endif
