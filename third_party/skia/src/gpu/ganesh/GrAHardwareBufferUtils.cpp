    ...
    if (backend == GrBackendApi::kOpenGL) {
        ...
        default: {
            if (requireKnownFormat) {
                ...
                if (VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_LINEAR_FILTER_BIT & hwbFormatProps.formatFeatures) {
    ...
    bool fromAndroidWindow)
{
    ...
    if (GrBackendApi::kOpenGL == dContext->backend()) {
        ...
        return make_gl_backend_texture(
            dContext, hardwareBuffer, width, height, deleteProc, updateProc, imageCtx, createProtectedImage, backendFormat, isRenderable);
#else
        return GrBackendTexture();
#endif // SK_GL
    } else {
        ...
        return make_vk_backend_texture(
            dContext, hardwareBuffer, width, height, deleteProc, updateProc, imageCtx, createProtectedImage, backendFormat, isRenderable, fromAndroidWindow);
#else
        return GrBackendTexture();
#endif // SK_VULKAN
    }
