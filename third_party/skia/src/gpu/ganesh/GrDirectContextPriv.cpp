...
void GrDirectContextPriv::createDDLTask(sk_sp<const SkDeferredDisplayList> ddl, sk_sp<GrRenderTargetProxy> newDest, SkIPoint offset)
{
...
void GrDirectContextPriv::dumpCacheStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const
{
#if GR_CACHE_STATS
...
void GrDirectContextPriv::dumpGpuStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const
{
#if GR_GPU_STATS
...
void GrDirectContextPriv::dumpContextStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const
{
#if GR_GPU_STATS
...
sk_sp<SkImage> GrDirectContextPriv::testingOnly_getFontAtlasImage(MaskFormat format, unsigned int index)
{
    ...
    return sk_make_sp<SkImage_Gpu>(sk_ref_sp(this->context()), kNeedNewImageUniqueID, views[index], SkColorInfo(colorType, kPremul_SkAlphaType, nullptr));
}
