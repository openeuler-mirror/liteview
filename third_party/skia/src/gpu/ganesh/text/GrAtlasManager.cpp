    ...
    if (maskFormat == expectedMaskFormat) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                uint32_t colorRGBA = GrColorPackRGBA(masks.getRed(color565), masks.getGreen(color565), masks.getBlue(color565), 0xFF);
...
void GrAtlasManager::addGlyphToBulkAndSetUseToken(skgpu::BulkUsePlotUpdater* updater, MaskFormat format, Glyph* glyph, skgpu::DrawToken token)
{
    SkASSERT(glyph);
...
std::tuple<bool, int> GlyphVector::regenerateAtlas(int begin, int end, MaskFormat maskFormat, int srcPadding, GrMeshDrawTarget* target)
{
