...
struct GrDawnProgram : public SkRefCnt {
    ...
    wgpu::BindGroup setTextures(GrDawnGpu*, const GrGeometryProcessor&, const GrPipeline&, const GrSurfaceProxy* const geomProcTextures[]);
};

class GrDawnProgramBuilder : public GrGLSLProgramBuilder {
public:
    static sk_sp<GrDawnProgram> Build(GrDawnGpu*, GrRenderTarget*, const GrProgramInfo&, wgpu::TextureFormat colorFormat, bool hasDepthStencil,
        wgpu::TextureFormat depthStencilFormat, GrProgramDesc*);
...
private:
    ...
    wgpu::ShaderModule createShaderModule(const GrGLSLShaderBuilder&, SkSL::ProgramKind, bool flipY, SkSL::Program::Inputs* inputs);
    GrDawnGpu* fGpu;
