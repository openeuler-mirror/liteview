...
GrDawnCaps::GrDawnCaps(const GrContextOptions& contextOptions)
...
static skgpu::Swizzle get_swizzle(const GrBackendFormat& format, GrColorType colorType, bool forOutput)
{
    switch (colorType) {
...
bool GrDawnCaps::isFormatAsColorTypeRenderable(GrColorType ct, const GrBackendFormat& format, int sampleCount) const
{
    return isFormatRenderable(format, sampleCount);
}

GrCaps::SurfaceReadPixelsSupport GrDawnCaps::surfaceSupportsReadPixels(const GrSurface* surface) const
{
    // We currently support readbacks only from Textures and TextureRenderTargets.
    return surface->asTexture() ? SurfaceReadPixelsSupport::kSupported : SurfaceReadPixelsSupport::kUnsupported;
}
...
int GrDawnCaps::getRenderTargetSampleCount(int requestedCount, const GrBackendFormat& backendFormat) const
{
    wgpu::TextureFormat dawnFormat;
...
skgpu::Swizzle GrDawnCaps::onGetReadSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    return get_swizzle(format, colorType, false);
}

skgpu::Swizzle GrDawnCaps::getWriteSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
...
bool GrDawnCaps::onAreColorTypeAndFormatCompatible(GrColorType ct, const GrBackendFormat& format) const
{
    return true;
}
...
GrProgramDesc GrDawnCaps::makeDesc(GrRenderTarget* rt, const GrProgramInfo& programInfo, ProgramDescOverrideFlags overrideFlags) const
{
