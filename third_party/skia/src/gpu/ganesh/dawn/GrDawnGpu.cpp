...
sk_sp<GrGpu> GrDawnGpu::Make(const wgpu::Device& device, const GrContextOptions& options, GrDirectContext* direct)
{
    if (!device) {
        return nullptr;
    }
...
GrDawnGpu::GrDawnGpu(GrDirectContext* direct, const GrContextOptions& options, const wgpu::Device& device)
    : INHERITED(direct)
    , fDevice(device)
    ...
    , fFinishCallbacks(this)
{
    ...
    device.SetUncapturedErrorCallback(
        [](WGPUErrorType type, char const* message, void*) { SkDebugf("GrDawnGpu: ERROR type %u, msg: %s", type, message); }, nullptr);
}
...
GrOpsRenderPass* GrDawnGpu::onGetOpsRenderPass(GrRenderTarget* rt, bool /*useMSAASurface*/, GrAttachment*, GrSurfaceOrigin origin, const SkIRect& bounds,
    const GrOpsRenderPass::LoadAndStoreInfo& colorInfo, const GrOpsRenderPass::StencilLoadAndStoreInfo& stencilInfo,
    const SkTArray<GrSurfaceProxy*, true>& sampledProxies, GrXferBarrierFlags renderPassXferBarriers)
{
...
///////////////////////////////////////////////////////////////////////////////
sk_sp<GrGpuBuffer> GrDawnGpu::onCreateBuffer(size_t size, GrGpuBufferType type, GrAccessPattern accessPattern)
{
...
////////////////////////////////////////////////////////////////////////////////
bool GrDawnGpu::onWritePixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType srcColorType, const GrMipLevel texels[],
...
bool GrDawnGpu::onTransferFromBufferToBuffer(sk_sp<GrGpuBuffer> src, size_t srcOffset, sk_sp<GrGpuBuffer> dst, size_t dstOffset, size_t size)
{
    // skbug.com/13453
...
bool GrDawnGpu::onTransferPixelsTo(GrTexture* texture, SkIRect rect, GrColorType textureColorType, GrColorType bufferColorType,
...
////////////////////////////////////////////////////////////////////////////////
sk_sp<GrTexture> GrDawnGpu::onCreateTexture(SkISize dimensions, const GrBackendFormat& backendFormat, GrRenderable renderable, int renderTargetSampleCnt,
    SkBudgeted budgeted, GrProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    if (levelClearMask) {
        return nullptr;
    }
    ...
    return GrDawnTexture::Make(this, dimensions, format, renderable, renderTargetSampleCnt, budgeted, mipLevelCount, mipmapStatus, label);
}

sk_sp<GrTexture> GrDawnGpu::onCreateCompressedTexture(
    SkISize dimensions, const GrBackendFormat&, SkBudgeted, GrMipmapped, GrProtected, const void* data, size_t dataSize)
{
    SkASSERT(!"unimplemented");
    return nullptr;
}

sk_sp<GrTexture> GrDawnGpu::onWrapBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable, GrIOType ioType)
{
    ...
    return GrDawnTexture::MakeWrapped(this, dimensions, GrRenderable::kNo, 1, cacheable, ioType, info, backendTex.getLabel());
}

sk_sp<GrTexture> GrDawnGpu::onWrapCompressedBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    return nullptr;
}

sk_sp<GrTexture> GrDawnGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex, int sampleCnt, GrWrapOwnership, GrWrapCacheable cacheable)
{
    GrDawnTextureInfo info;
    ...
    sk_sp<GrTexture> result = GrDawnTexture::MakeWrapped(this, dimensions, GrRenderable::kYes, sampleCnt, cacheable, kRW_GrIOType, info, tex.getLabel());
...
sk_sp<GrRenderTarget> GrDawnGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt)
{
    ...
    int sampleCnt = 1;
    return GrDawnRenderTarget::MakeWrapped(this, dimensions, sampleCnt, info, /*label=*/"DawnGpu_WrapBackendRenderTarget");
}

sk_sp<GrAttachment> GrDawnGpu::makeStencilAttachment(const GrBackendFormat& /*colorFormat*/, SkISize dimensions, int numStencilSamples)
{
...
GrBackendTexture GrDawnGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& backendFormat, GrRenderable renderable, GrMipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    wgpu::TextureFormat format;
    if (!backendFormat.asDawnFormat(&format)) {
    ...
    desc.usage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::CopyDst;
...
void GrDawnGpu::uploadTextureData(GrColorType srcColorType, const GrMipLevel texels[], int mipLevelCount, const SkIRect& rect, wgpu::Texture texture)
{
    uint32_t x = rect.x();
    uint32_t y = rect.y();
    ...
    for (int i = 0; i < mipLevelCount; i++) {
        ...
        GrStagingBufferManager::Slice slice = this->stagingBufferManager()->allocateStagingBufferSlice(size);
...
bool GrDawnGpu::onClearBackendTexture(const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, std::array<float, 4> color)
{
    GrDawnTextureInfo info;
    ...
    for (uint32_t i = 0; i < info.fLevelCount; i++) {
        ...
        GrStagingBufferManager::Slice stagingBuffer = this->stagingBufferManager()->allocateStagingBufferSlice(size);
...
GrBackendTexture GrDawnGpu::onCreateCompressedBackendTexture(SkISize dimensions, const GrBackendFormat&, GrMipmapped, GrProtected)
{
    return {};
}

bool GrDawnGpu::onUpdateCompressedBackendTexture(const GrBackendTexture&, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t size)
{
    return false;
}
...
GrBackendRenderTarget GrDawnGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected isProtected)
{
    if (dimensions.width() > this->caps()->maxTextureSize() || dimensions.height() > this->caps()->maxTextureSize()) {
        return {};
    }
...
void GrDawnGpu::addFinishedProc(GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
...
bool GrDawnGpu::onSubmitToGpu(bool syncCpu)
{
    ...
    if (!fSubmittedWorkDoneCallbackPending) {
        auto callback = [](WGPUQueueWorkDoneStatus status, void* userData) { static_cast<GrDawnGpu*>(userData)->onSubmittedWorkDone(status); };
        ...
        static_cast<GrDawnBuffer*>(buffer.get())
            ->mapAsync(
                ...
                [this, buffer = std::move(buffer)](bool success) {
                    ...
                    if (!success) {
                        SkDebugf("Failed to map staging buffer before making it available "
                                 "again");
                    }
...
bool GrDawnGpu::onCopySurface(GrSurface* dst, const SkIRect& dstRect, GrSurface* src, const SkIRect& srcRect, GrSamplerState::Filter)
{
...
bool GrDawnGpu::onReadPixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* buffer, size_t rowBytes)
{
    ...
    sk_sp<GrDawnBuffer> dawnBuffer = GrDawnBuffer::Make(this, sizeInBytes, GrGpuBufferType::kXferGpuToCpu, kStatic_GrAccessPattern, "onReadPixels");
    if (!dawnBuffer) {
...
bool GrDawnGpu::onRegenerateMipMapLevels(GrTexture* tex)
{
    ...
    texDesc.usage = wgpu::TextureUsage::TextureBinding | wgpu::TextureUsage::CopySrc | wgpu::TextureUsage::RenderAttachment;
    ...
    const char* vs = "layout(location = 0) out float2 texCoord;"
                     ...
                     "}";
    std::string vsSPIRV = this->SkSLToSPIRV(vs, SkSL::ProgramKind::kVertex,
        /*rtFlipOffset*/ 0, nullptr);

    const char* fs = "layout(set = 0, binding = 0) uniform sampler samp;"
                     "layout(set = 0, binding = 1) uniform texture2D tex;"
                     "layout(location = 0) in float2 texCoord;"
                     "void main() {"
                     "sk_FragColor = sample(makeSampler2D(tex, samp), texCoord);"
                     "}";
    std::string fsSPIRV = this->SkSLToSPIRV(fs, SkSL::ProgramKind::kFragment,
        /*rtFlipOffset=*/0, nullptr);
...
std::unique_ptr<GrSemaphore> GrDawnGpu::wrapBackendSemaphore(
    const GrBackendSemaphore& /* sema */, GrSemaphoreWrapType /* wrapType */, GrWrapOwnership /* ownership */)
{
    SkASSERT(!"unimplemented");
    return nullptr;
}
...
sk_sp<GrDawnProgram> GrDawnGpu::getOrCreateRenderPipeline(GrRenderTarget* rt, const GrProgramInfo& programInfo)
{
    ...
    sk_sp<GrDawnProgram> program = GrDawnProgramBuilder::Build(this, rt, programInfo, colorFormat, hasDepthStencil, stencilFormat, &desc);
...
std::string GrDawnGpu::SkSLToSPIRV(const char* shaderString, SkSL::ProgramKind kind, uint32_t rtFlipOffset, SkSL::Program::Inputs* inputs)
{
    ...
    std::unique_ptr<SkSL::Program> program = this->shaderCompiler()->convertProgram(kind, shaderString, settings);
    if (!program) {
