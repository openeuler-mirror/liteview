...
static wgpu::DepthStencilState create_depth_stencil_state(const GrProgramInfo& programInfo, wgpu::TextureFormat depthStencilFormat)
{
...
static wgpu::BindGroupEntry make_bind_group_entry(uint32_t binding, const wgpu::Sampler& sampler, const wgpu::TextureView& textureView)
{
...
static wgpu::BindGroupEntry make_bind_group_entry(uint32_t binding, const wgpu::TextureView& textureView)
{
    return make_bind_group_entry(binding, nullptr, textureView);
}

sk_sp<GrDawnProgram> GrDawnProgramBuilder::Build(GrDawnGpu* gpu, GrRenderTarget* renderTarget, const GrProgramInfo& programInfo,
    wgpu::TextureFormat colorFormat, bool hasDepthStencil, wgpu::TextureFormat depthStencilFormat, GrProgramDesc* desc)
{
    ...
    auto vsModule = builder.createShaderModule(builder.fVS, SkSL::ProgramKind::kVertex, flipY, &vertInputs);
    auto fsModule = builder.createShaderModule(builder.fFS, SkSL::ProgramKind::kFragment, flipY, &fragInputs);
    ...
    if (textureCount > 0) {
        ...
        result->fBindGroupLayouts.push_back(gpu->device().CreateBindGroupLayout(&textureBindGroupLayoutDesc));
    }
    ...
    if (primitiveType == GrPrimitiveType::kTriangleStrip || primitiveType == GrPrimitiveType::kLineStrip) {
...
GrDawnProgramBuilder::GrDawnProgramBuilder(GrDawnGpu* gpu, const GrProgramInfo& programInfo, GrProgramDesc* desc)
...
wgpu::ShaderModule GrDawnProgramBuilder::createShaderModule(
    const GrGLSLShaderBuilder& builder, SkSL::ProgramKind kind, bool flipY, SkSL::Program::Inputs* inputs)
{
    ...
    std::string spirvSource = fGpu->SkSLToSPIRV(source.c_str(), kind, fUniformHandler.getRTFlipOffset(), inputs);
...
static void set_texture(GrDawnGpu* gpu, GrSamplerState state, GrTexture* texture, std::vector<wgpu::BindGroupEntry>* bindings, int* binding)
{
    ...
    // available levels.
    viewDesc.mipLevelCount = GrSamplerState::MipmapMode::kNone == state.mipmapMode() ? 1 : WGPU_MIP_LEVEL_COUNT_UNDEFINED;
    wgpu::TextureView textureView = tex->texture().CreateView(&viewDesc);
    bindings->push_back(make_bind_group_entry((*binding)++, textureView));
}

wgpu::BindGroup GrDawnProgram::setUniformData(GrDawnGpu* gpu, const GrRenderTarget* renderTarget, const GrProgramInfo& programInfo)
{
...
wgpu::BindGroup GrDawnProgram::setTextures(
    GrDawnGpu* gpu, const GrGeometryProcessor& geomProc, const GrPipeline& pipeline, const GrSurfaceProxy* const geomProcTextures[])
{
    ...
    if (geomProcTextures) {
        for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
            ...
            set_texture(gpu, sampler.samplerState(), geomProcTextures[i]->peekTexture(), &bindings, &binding);
        }
    }
    ...
    pipeline.visitTextureEffects([&](const GrTextureEffect& te) { set_texture(gpu, te.samplerState(), te.texture(), &bindings, &binding); });
