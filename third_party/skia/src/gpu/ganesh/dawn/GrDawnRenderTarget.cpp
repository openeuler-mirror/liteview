...
GrDawnRenderTarget::GrDawnRenderTarget(GrDawnGpu* gpu, SkISize dimensions, int sampleCnt, const GrDawnRenderTargetInfo& info, std::string_view label)
...
sk_sp<GrDawnRenderTarget> GrDawnRenderTarget::MakeWrapped(
    GrDawnGpu* gpu, SkISize dimensions, int sampleCnt, const GrDawnRenderTargetInfo& info, std::string_view label)
{
    sk_sp<GrDawnRenderTarget> rt(new GrDawnRenderTarget(gpu, dimensions, sampleCnt, info, label));
...
size_t GrDawnRenderTarget::onGpuMemorySize() const
{
    ...
    return GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), numSamples, GrMipmapped::kNo);
}
...
{
    return GrBackendRenderTarget(this->width(), this->height(), this->numSamples(), this->numSamples(), fInfo);
}
