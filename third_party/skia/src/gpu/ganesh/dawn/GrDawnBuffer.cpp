...
// static
sk_sp<GrDawnBuffer> GrDawnBuffer::Make(GrDawnGpu* gpu, size_t sizeInBytes, GrGpuBufferType type, GrAccessPattern pattern, std::string_view label)
{
    ...
    return sk_sp<GrDawnBuffer>(new GrDawnBuffer(gpu, sizeInBytes, type, pattern, label, mappable, std::move(buffer), mapPtr));
}

GrDawnBuffer::GrDawnBuffer(GrDawnGpu* gpu, size_t sizeInBytes, GrGpuBufferType type, GrAccessPattern pattern, std::string_view label, Mappable mappable,
    wgpu::Buffer buffer, void* mapPtr)
...
void* GrDawnBuffer::internalMap(MapType type, size_t offset, size_t size)
{
    ...
    if (fMappable == Mappable::kNot) {
        ...
        GrStagingBufferManager::Slice slice = this->getDawnGpu()->stagingBufferManager()->allocateStagingBufferSlice(size, /*requiredAlignment=*/4);
...
void GrDawnBuffer::internalUnmap(MapType type, size_t offset, size_t size)
{
    if (fMappable == Mappable::kNot) {
        ...
        this->getDawnGpu()->getCopyEncoder().CopyBufferToBuffer(fStagingBuffer, fStagingOffset, fBuffer, offset, size);
    } else {
        fBuffer.Unmap();
        fUnmapped = true;
    }
...
void GrDawnBuffer::mapAsync(MapAsyncCallback callback)
{
    ...
    fBuffer.MapAsync((fMappable == Mappable::kReadOnly) ? wgpu::MapMode::Read : wgpu::MapMode::Write, 0, wgpu::kWholeMapSize,
        [](WGPUBufferMapAsyncStatus status, void* userData) { static_cast<GrDawnBuffer*>(userData)->mapAsyncDone(status); }, this);
}
...
void* GrDawnBuffer::blockingMap(size_t offset, size_t size)
{
    ...
    fBuffer.MapAsync((fMappable == Mappable::kReadOnly) ? wgpu::MapMode::Read : wgpu::MapMode::Write, offset, size,
