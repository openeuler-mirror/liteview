...
static bool draw_mask(
    skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkMatrix& viewMatrix, const SkIRect& maskBounds, GrPaint&& paint, GrSurfaceProxyView mask)
{
    SkMatrix inverse;
    ...
    const SkMaskFilter* filter, const SkIRect& unclippedDevShapeBounds, const SkIRect& clipBounds, SkIRect* drawRect, skgpu::UniqueKey* key)
{
    ...
    if (filteredMaskView) {
        ...
        *drawRect = extract_draw_rect_from_data(data.get(), unclippedDevShapeBounds);
        ...
        SkMask srcM, dstM;
        if (!SkDraw::DrawToMask(devPath, clipBounds, filter, &viewMatrix, &srcM, SkMask::kComputeBoundsAndRenderImage_CreateMode, fillOrHairline)) {
            return {};
        }
        ...
        std::tie(filteredMaskView, std::ignore) = GrMakeUncachedBitmapProxyView(rContext, bm, GrMipmapped::kNo, SkBackingFit::kApprox);
        if (!filteredMaskView) {
            return {};
        }
    ...
    GrRecordingContext* rContext, const SkIRect& maskRect, const SkMatrix& origViewMatrix, const GrStyledShape& shape, int sampleCnt)
{
    ...
    auto sdc = skgpu::v1::SurfaceDrawContext::MakeWithFallback(rContext, GrColorType::kAlpha_8, nullptr, SkBackingFit::kExact, approxSize, defaultSurfaceProps,
...
// is no intersection.
static bool get_shape_and_clip_bounds(skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const GrStyledShape& shape, const SkMatrix& matrix,
    ...
    const SkMaskFilterBase* maskFilter, const GrStyledShape& shape, const SkIRect& unclippedDevShapeBounds, const SkIRect& devClipBounds)
{
    *boundsForClip = devClipBounds;
    ...
    if (useCache) {
        ...
        maskFilter->canFilterMaskGPU(shape, unclippedDevShapeBounds, devClipBounds, viewMatrix, &clippedMaskRect);
        maskFilter->canFilterMaskGPU(shape, unclippedDevShapeBounds, unclippedDevShapeBounds, viewMatrix, &unClippedMaskRect);
...
static GrSurfaceProxyView hw_create_filtered_mask(GrDirectContext* dContext, skgpu::v1::SurfaceDrawContext* sdc, const SkMatrix& viewMatrix,
    ...
    skgpu::UniqueKey* key)
{
    ...
    std::unique_ptr<skgpu::v1::SurfaceDrawContext> maskSDC(create_mask_GPU(dContext, *maskRect, viewMatrix, shape, sdc->numSamples()));
    if (!maskSDC) {
        if (key->isValid()) {
    ...
    auto filteredMaskView = filter->filterMaskGPU(
        dContext, maskSDC->readSurfaceView(), maskSDC->colorInfo().colorType(), maskSDC->colorInfo().alphaType(), viewMatrix, *maskRect);
    if (!filteredMaskView) {
        if (key->isValid()) {
...
static void draw_shape_with_mask_filter(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, GrPaint&& paint,
...
void GrBlurUtils::drawShapeWithMaskFilter(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const GrStyledShape& shape,
...
void GrBlurUtils::drawShapeWithMaskFilter(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkPaint& paint,
    const SkMatrixProvider& matrixProvider, const GrStyledShape& shape)
{
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(rContext, sdc->colorInfo(), paint, matrixProvider, sdc->surfaceProps(), &grPaint)) {
        return;
    }
    ...
    if (mf && !mf->hasFragmentProcessor()) {
        ...
        sdc->drawShape(clip, std::move(grPaint), sdc->chooseAA(paint), viewMatrix, GrStyledShape(shape));
    }
}
