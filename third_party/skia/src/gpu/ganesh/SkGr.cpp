...
sk_sp<GrSurfaceProxy> GrCopyBaseMipMapToTextureProxy(
    GrRecordingContext* ctx, sk_sp<GrSurfaceProxy> baseProxy, GrSurfaceOrigin origin, std::string_view label, SkBudgeted budgeted)
{
    SkASSERT(baseProxy);
    ...
    auto copy = GrSurfaceProxy::Copy(ctx, std::move(baseProxy), origin, GrMipmapped::kYes, SkBackingFit::kExact, budgeted, label);
    if (!copy) {
        return nullptr;
    }
...
GrSurfaceProxyView GrCopyBaseMipMapToView(GrRecordingContext* context, GrSurfaceProxyView src, SkBudgeted budgeted)
{
...
static GrMipmapped adjust_mipmapped(GrMipmapped mipmapped, const SkBitmap& bitmap, const GrCaps* caps)
{
...
static sk_sp<GrTextureProxy> make_bmp_proxy(
    GrProxyProvider* proxyProvider, const SkBitmap& bitmap, GrColorType ct, GrMipmapped mipmapped, SkBackingFit fit, SkBudgeted budgeted)
{
...
std::tuple<GrSurfaceProxyView, GrColorType> GrMakeCachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, std::string_view label, GrMipmapped mipmapped)
{
    ...
    if (!proxy) {
        proxy = make_bmp_proxy(proxyProvider, bitmap, ct, mipmapped, SkBackingFit::kExact, SkBudgeted::kYes);
        if (!proxy) {
            return {};
        }
...
std::tuple<GrSurfaceProxyView, GrColorType> GrMakeUncachedBitmapProxyView(
    GrRecordingContext* rContext, const SkBitmap& bitmap, GrMipmapped mipmapped, SkBackingFit fit, SkBudgeted budgeted)
{
    GrProxyProvider* proxyProvider = rContext->priv().proxyProvider();
    const GrCaps* caps = rContext->priv().caps();
    ...
    GrRecordingContext* rContext, std::unique_ptr<GrFragmentProcessor> inputFP, float range, const GrCaps* caps)
{
    ...
    auto [tex, ct] = GrMakeCachedBitmapProxyView(rContext, gLUT, /*label=*/"MakeDitherEffect", GrMipmapped::kNo);
    if (!tex) {
        return inputFP;
    }
...
static inline bool skpaint_to_grpaint_impl(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint,
    const SkMatrixProvider& matrixProvider, std::optional<std::unique_ptr<GrFragmentProcessor>> shaderFP, SkBlender* primColorBlender,
    const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    ...
    if (paintFP) {
        if (primColorBlender) {
            ...
            paintFP = as_BB(primColorBlender)
                          ->asFragmentProcessor(std::move(paintFP),
    ...
    if (colorFilter) {
        if (applyColorFilterToPaintColor) {
            ...
            auto [success, fp] = as_CFB(colorFilter)->asFragmentProcessor(std::move(paintFP), context, dstColorInfo, surfaceProps);
            if (!success) {
                return false;
            }
            paintFP = std::move(fp);
    ...
    if (auto bm = skPaint.asBlendMode()) {
        ...
        paintFP = as_BB(skPaint.getBlender())->asFragmentProcessor(std::move(paintFP), GrFragmentProcessor::SurfaceColor(), fpArgs);
        if (!paintFP) {
            return false;
        }
...
bool SkPaintToGrPaint(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrixProvider& matrixProvider,
    const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
...
bool SkPaintToGrPaintReplaceShader(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrixProvider& matrixProvider,
    std::unique_ptr<GrFragmentProcessor> shaderFP, const SkSurfaceProps& surfaceProps, GrPaint* grPaint)
{
    return skpaint_to_grpaint_impl(context, dstColorInfo, skPaint, matrixProvider, std::move(shaderFP),
        /*primColorBlender=*/nullptr, surfaceProps, grPaint);
}

/** Blends the SkPaint's shader (or color if no shader) with a per-primitive color which must
    be setup as a vertex attribute using the specified SkBlender. */
bool SkPaintToGrPaintWithBlend(GrRecordingContext* context, const GrColorInfo& dstColorInfo, const SkPaint& skPaint, const SkMatrixProvider& matrixProvider,
