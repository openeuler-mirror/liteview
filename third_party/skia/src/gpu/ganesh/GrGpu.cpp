...
sk_sp<GrTexture> GrGpu::createTextureCommon(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrRenderable renderable,
    int renderTargetSampleCnt, SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
...
sk_sp<GrTexture> GrGpu::createTexture(SkISize dimensions, const GrBackendFormat& format, GrTextureType textureType, GrRenderable renderable,
    int renderTargetSampleCnt, SkBudgeted budgeted, GrProtected isProtected, GrColorType textureColorType, GrColorType srcColorType, const GrMipLevel texels[],
    int texelLevelCount, std::string_view label)
{
    ...
    SkISize dimensions, const GrBackendFormat& format, SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    if (dataSize < SkCompressedDataSize(compressionType, dimensions, nullptr, mipmapped == GrMipmapped::kYes)) {
        return nullptr;
    }
...
bool GrGpu::CompressedDataIsCorrect(SkISize dimensions, SkImage::CompressionType compressionType, GrMipmapped mipmapped, const void* data, size_t length)
{
    size_t computedSize = SkCompressedDataSize(compressionType, dimensions, nullptr, mipmapped == GrMipmapped::kYes);
...
GrBackendTexture GrGpu::createCompressedBackendTexture(SkISize dimensions, const GrBackendFormat& format, GrMipmapped mipmapped, GrProtected isProtected)
{
    const GrCaps* caps = this->caps();
