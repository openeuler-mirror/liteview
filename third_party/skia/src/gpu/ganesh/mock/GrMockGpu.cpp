...
sk_sp<GrGpu> GrMockGpu::Make(const GrMockOptions* mockOptions, const GrContextOptions& contextOptions, GrDirectContext* direct)
{
...
GrOpsRenderPass* GrMockGpu::onGetOpsRenderPass(GrRenderTarget* rt, bool /*useMSAASurface*/, GrAttachment*, GrSurfaceOrigin origin, const SkIRect& bounds,
    const GrOpsRenderPass::LoadAndStoreInfo& colorInfo, const GrOpsRenderPass::StencilLoadAndStoreInfo&, const SkTArray<GrSurfaceProxy*, true>& sampledProxies,
    ...
    SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    if (renderable == GrRenderable::kYes) {
        ...
        return sk_sp<GrTexture>(
            new GrMockTextureRenderTarget(this, budgeted, dimensions, renderTargetSampleCnt, isProtected, mipmapStatus, texInfo, rtInfo, label));
    }
    return sk_sp<GrTexture>(new GrMockTexture(this, budgeted, dimensions, isProtected, mipmapStatus, texInfo, label));
}
    ...
    SkISize dimensions, const GrBackendFormat& format, SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    return sk_sp<GrTexture>(new GrMockTexture(this, budgeted, dimensions, isProtected, mipmapStatus, texInfo,
...
sk_sp<GrTexture> GrMockGpu::onWrapBackendTexture(const GrBackendTexture& tex, GrWrapOwnership ownership, GrWrapCacheable wrapType, GrIOType ioType)
{
    ...
    return sk_sp<GrTexture>(new GrMockTexture(this, tex.dimensions(), isProtected, mipmapStatus, texInfo, wrapType, ioType,
...
sk_sp<GrTexture> GrMockGpu::onWrapRenderableBackendTexture(const GrBackendTexture& tex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    auto isProtected = GrProtected(tex.isProtected());
    return sk_sp<GrTexture>(new GrMockTextureRenderTarget(this, tex.dimensions(), sampleCnt, isProtected, mipmapStatus, texInfo, rtInfo, cacheable,
...
sk_sp<GrRenderTarget> GrMockGpu::onWrapBackendRenderTarget(const GrBackendRenderTarget& rt)
{
    ...
    return sk_sp<GrRenderTarget>(new GrMockRenderTarget(this, GrMockRenderTarget::kWrapped, rt.dimensions(), rt.sampleCnt(), isProtected, info,
...
GrBackendTexture GrMockGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable, GrMipmapped mipmapped, GrProtected, std::string_view label)
{
    SkImage::CompressionType compression = format.asMockCompressionType();
    if (compression != SkImage::CompressionType::kNone) {
...
GrBackendTexture GrMockGpu::onCreateCompressedBackendTexture(SkISize dimensions, const GrBackendFormat& format, GrMipmapped mipmapped, GrProtected)
{
    SkImage::CompressionType compression = format.asMockCompressionType();
    if (compression == SkImage::CompressionType::kNone) {
...
GrBackendRenderTarget GrMockGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected)
{
