...
public:
    static sk_sp<GrVkTexture> MakeNewTexture(
        GrVkGpu*, SkBudgeted budgeted, SkISize dimensions, VkFormat format, uint32_t mipLevels, GrProtected, GrMipmapStatus, std::string_view label);

    static sk_sp<GrVkTexture> MakeWrappedTexture(
        GrVkGpu*, SkISize dimensions, GrWrapOwnership, GrWrapCacheable, GrIOType, const GrVkImageInfo&, sk_sp<skgpu::MutableTextureStateRef>);
...
private:
    GrVkTexture(GrVkGpu*, SkBudgeted, SkISize, sk_sp<GrVkImage> texture, GrMipmapStatus, std::string_view label);
