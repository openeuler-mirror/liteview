...
public:
    ...
    static sk_sp<GrVkImage> MakeWrapped(GrVkGpu* gpu, SkISize dimensions, const GrVkImageInfo&, sk_sp<skgpu::MutableTextureStateRef>,
        UsageFlags attachmentUsages, GrWrapOwnership, GrWrapCacheable, std::string_view label, bool forSecondaryCB = false);
...
private:
    static sk_sp<GrVkImage> Make(GrVkGpu* gpu, SkISize dimensions, UsageFlags attachmentUsages, int sampleCnt, VkFormat format, uint32_t mipLevels,
        VkImageUsageFlags vkUsageFlags, GrProtected isProtected, GrMemoryless, SkBudgeted);

    GrVkImage(GrVkGpu* gpu, SkISize dimensions, UsageFlags supportedUsages, const GrVkImageInfo&, sk_sp<skgpu::MutableTextureStateRef> mutableState,
        sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView, SkBudgeted, std::string_view label);
