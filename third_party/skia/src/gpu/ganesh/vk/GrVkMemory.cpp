...
bool GrVkMemory::AllocAndBindBufferMemory(GrVkGpu* gpu, VkBuffer buffer, BufferUsage usage, skgpu::VulkanAlloc* alloc)
{
...
bool GrVkMemory::AllocAndBindImageMemory(GrVkGpu* gpu, VkImage image, GrMemoryless memoryless, skgpu::VulkanAlloc* alloc)
{
    skgpu::VulkanMemoryAllocator* allocator = gpu->memoryAllocator();
    skgpu::VulkanBackendMemory memory = 0;
    ...
    // Bind buffer
    VkResult err;
    GR_VK_CALL_RESULT(gpu, err, BindImageMemory(gpu->device(), image, alloc->fMemory, alloc->fOffset));
    if (err) {
...
void GrVkMemory::GetNonCoherentMappedMemoryRange(
    const skgpu::VulkanAlloc& alloc, VkDeviceSize offset, VkDeviceSize size, VkDeviceSize alignment, VkMappedMemoryRange* range)
{
...
void GrVkMemory::FlushMappedAlloc(GrVkGpu* gpu, const skgpu::VulkanAlloc& alloc, VkDeviceSize offset, VkDeviceSize size)
{
...
void GrVkMemory::InvalidateMappedAlloc(GrVkGpu* gpu, const skgpu::VulkanAlloc& alloc, VkDeviceSize offset, VkDeviceSize size)
{
    if (alloc.fFlags & skgpu::VulkanAlloc::kNoncoherent_Flag) {
        SkASSERT(offset == 0);
        SkASSERT(size <= alloc.fSize);
