...
sk_sp<GrGpu> GrVkGpu::Make(const GrVkBackendContext& backendContext, const GrContextOptions& options, GrDirectContext* direct)
{
    ...
    PFN_vkEnumerateInstanceVersion localEnumerateInstanceVersion
        = reinterpret_cast<PFN_vkEnumerateInstanceVersion>(backendContext.fGetProc("vkEnumerateInstanceVersion", VK_NULL_HANDLE, VK_NULL_HANDLE));
    ...
    uint32_t apiVersion = backendContext.fMaxAPIVersion ? backendContext.fMaxAPIVersion : instanceVersion;
    ...
    if (backendContext.fVkExtensions) {
        interface.reset(new skgpu::VulkanInterface(
            backendContext.fGetProc, backendContext.fInstance, backendContext.fDevice, instanceVersion, physDevVersion, backendContext.fVkExtensions));
        ...
        if (backendContext.fExtensions & kKHR_swapchain_GrVkExtensionFlag) {
            ...
            extensions.init(backendContext.fGetProc, backendContext.fInstance, backendContext.fPhysicalDevice, 0, nullptr, 1, &swapChainExtName);
        }
        interface.reset(new skgpu::VulkanInterface(
            backendContext.fGetProc, backendContext.fInstance, backendContext.fDevice, instanceVersion, physDevVersion, &extensions));
    ...
    if (backendContext.fDeviceFeatures2) {
        ...
        caps.reset(new GrVkCaps(options, interface.get(), backendContext.fPhysicalDevice, features2, instanceVersion, physDevVersion,
            *backendContext.fVkExtensions, backendContext.fProtectedContext));
    } else {
        ...
        caps.reset(new GrVkCaps(
            options, interface.get(), backendContext.fPhysicalDevice, features, instanceVersion, physDevVersion, extensions, backendContext.fProtectedContext));
    }
    ...
    if (!memoryAllocator) {
        ...
        memoryAllocator = skgpu::VulkanAMDMemoryAllocator::Make(backendContext.fInstance, backendContext.fPhysicalDevice, backendContext.fDevice,
            physDevVersion, backendContext.fVkExtensions, interface, mustUseCoherentHostVisibleMemory,
            /*=threadSafe=*/false);
    }
    if (!memoryAllocator) {
    ...
    sk_sp<GrVkGpu> vkGpu(new GrVkGpu(direct, backendContext, std::move(caps), interface, instanceVersion, physDevVersion, std::move(memoryAllocator)));
    if (backendContext.fProtectedContext == GrProtected::kYes && !vkGpu->vkCaps().supportsProtectedMemory()) {
        return nullptr;
    }
...
GrVkGpu::GrVkGpu(GrDirectContext* direct, const GrVkBackendContext& backendContext, sk_sp<GrVkCaps> caps, sk_sp<const skgpu::VulkanInterface> interface,
    uint32_t instanceVersion, uint32_t physicalDeviceVersion, sk_sp<skgpu::VulkanMemoryAllocator> memoryAllocator)
    : INHERITED(direct)
...
bool GrVkGpu::submitCommandBuffer(SyncQueue sync)
{
    ...
    if (!this->currentCommandBuffer()->hasWork() && kForce_SyncQueue != sync && !fSemaphoresToSignal.count() && !fSemaphoresToWaitOn.count()) {
    ...
    fMainCmdPool->close();
    bool didSubmit = fMainCmdBuffer->submitToQueue(this, fQueue, fSemaphoresToSignal, fSemaphoresToWaitOn);
...
static size_t fill_in_compressed_regions(GrStagingBufferManager* stagingBufferManager, SkTArray<VkBufferImageCopy>* regions,
    SkTArray<size_t>* individualMipOffsets, GrStagingBufferManager::Slice* slice, SkImage::CompressionType compression, VkFormat vkFormat, SkISize dimensions,
    GrMipmapped mipmapped)
{
    ...
    size_t bufferSize = SkCompressedDataSize(compression, dimensions, individualMipOffsets, mipmapped == GrMipmapped::kYes);
...
bool GrVkGpu::uploadTexDataOptimal(GrVkImage* texImage, SkIRect rect, GrColorType dataColorType, const GrMipLevel texels[], int mipLevelCount)
{
    ...
    this->currentCommandBuffer()->copyBufferToImage(
        this, vkBuffer->vkBuffer(), texImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.count(), regions.begin());
    return true;
}

// It's probably possible to roll this into uploadTexDataOptimal,
// but for now it's easier to maintain as a separate entity.
bool GrVkGpu::uploadTexDataCompressed(GrVkImage* uploadTexture, SkImage::CompressionType compression, VkFormat vkFormat, SkISize dimensions,
    GrMipmapped mipmapped, const void* data, size_t dataSize)
{
    ...
    GrVkBuffer* vkBuffer = static_cast<GrVkBuffer*>(slice.fBuffer);
    this->currentCommandBuffer()->copyBufferToImage(
        this, vkBuffer->vkBuffer(), uploadTexture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, regions.count(), regions.begin());
...
sk_sp<GrTexture> GrVkGpu::onCreateTexture(SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt,
    SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    if (levelClearMask) {
        ...
        texImage->setImageLayout(this, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, false);
        this->currentCommandBuffer()->clearColorImage(this, texImage, &kZeroClearColor, ranges.count(), ranges.begin());
    }
    return std::move(tex);
}

sk_sp<GrTexture> GrVkGpu::onCreateCompressedTexture(
    SkISize dimensions, const GrBackendFormat& format, SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected, const void* data, size_t dataSize)
{
    VkFormat pixelFormat;
    SkAssertResult(format.asVkFormat(&pixelFormat));
    SkASSERT(GrVkFormatIsCompressed(pixelFormat));
    ...
    GrMipmapStatus mipmapStatus = (mipmapped == GrMipmapped::kYes) ? GrMipmapStatus::kValid : GrMipmapStatus::kNotAllocated;
...
bool copy_src_data(char* mapPtr, VkFormat vkFormat, const SkTArray<size_t>& individualMipOffsets, const GrPixmap srcData[], int numMipLevels)
{
...
GrBackendTexture GrVkGpu::onCreateBackendTexture(
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, GrMipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    const GrVkCaps& caps = this->vkCaps();
...
GrBackendTexture GrVkGpu::onCreateCompressedBackendTexture(SkISize dimensions, const GrBackendFormat& format, GrMipmapped mipmapped, GrProtected isProtected)
{
    ...
    const GrBackendTexture& backendTexture, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t size)
{
    ...
    SkImage::CompressionType compression = GrBackendFormatToCompressionType(backendTexture.getBackendFormat());
    ...
    cmdBuffer->copyBufferToImage(this, static_cast<GrVkBuffer*>(slice.fBuffer)->vkBuffer(), image, image->currentLayout(), regions.count(), regions.begin());
...
void set_layout_and_queue_from_mutable_state(GrVkGpu* gpu, GrVkImage* image, const skgpu::VulkanMutableTextureState& newState)
{
...
bool GrVkGpu::setBackendSurfaceState(GrVkImageInfo info, sk_sp<skgpu::MutableTextureStateRef> currentState, SkISize dimensions,
    const skgpu::VulkanMutableTextureState& newState, skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    ...
    skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    ...
    return this->setBackendSurfaceState(
        info, std::move(currentState), backendTeture.dimensions(), newState.fVkState, previousState, std::move(finishedCallback));
}
    ...
    skgpu::MutableTextureState* previousState, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    ...
    return this->setBackendSurfaceState(
        info, std::move(currentState), backendRenderTarget.dimensions(), newState.fVkState, previousState, std::move(finishedCallback));
}
...
GrBackendRenderTarget GrVkGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType ct, int sampleCnt, GrProtected isProtected)
{
    ...
    GrVkImageInfo info;
    if (!this->createVkImageForBackendSurface(vkFormat, dimensions, sampleCnt, GrTexturable::kNo, GrRenderable::kYes, GrMipmapped::kNo, &info, isProtected)) {
        return {};
    }
...
void GrVkGpu::prepareSurfacesForBackendAccessAndStateUpdates(
    SkSpan<GrSurfaceProxy*> proxies, SkSurface::BackendSurfaceAccess access, const skgpu::MutableTextureState* newState)
{
