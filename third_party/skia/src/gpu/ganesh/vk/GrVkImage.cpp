...
sk_sp<GrVkImage> GrVkImage::Make(GrVkGpu* gpu, SkISize dimensions, UsageFlags attachmentUsages, int sampleCnt, VkFormat format, uint32_t mipLevels,
    VkImageUsageFlags vkUsageFlags, GrProtected isProtected, GrMemoryless memoryless, SkBudgeted budgeted)
{
    ...
    sk_sp<skgpu::MutableTextureStateRef> mutableState(new skgpu::MutableTextureStateRef(info.fImageLayout, info.fCurrentQueueFamily));
...
sk_sp<GrVkImage> GrVkImage::MakeWrapped(GrVkGpu* gpu, SkISize dimensions, const GrVkImageInfo& info, sk_sp<skgpu::MutableTextureStateRef> mutableState,
...
GrVkImage::GrVkImage(GrVkGpu* gpu, SkISize dimensions, UsageFlags supportedUsages, const GrVkImageInfo& info, sk_sp<skgpu::MutableTextureStateRef> mutableState,
    sk_sp<const GrVkImageView> framebufferView, sk_sp<const GrVkImageView> textureView, SkBudgeted budgeted, std::string_view label)
    : GrAttachment(gpu, dimensions, supportedUsages, info.fSampleCount, info.fLevelCount > 1 ? GrMipmapped::kYes : GrMipmapped::kNo, info.fProtected, label,
        info.fAlloc.fFlags & skgpu::VulkanAlloc::kLazilyAllocated_Flag ? GrMemoryless::kYes : GrMemoryless::kNo)
    , fInfo(info)
...
bool GrVkImage::InitImageInfo(GrVkGpu* gpu, const ImageDesc& imageDesc, GrVkImageInfo* info)
{
    ...
    GrMemoryless memoryless = imageDesc.fUsageFlags & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT ? GrMemoryless::kYes : GrMemoryless::kNo;
    skgpu::VulkanAlloc alloc;
    if (!GrVkMemory::AllocAndBindImageMemory(gpu, image, memoryless, &alloc)
        || (memoryless == GrMemoryless::kYes && !SkToBool(alloc.fFlags & skgpu::VulkanAlloc::kLazilyAllocated_Flag))) {
