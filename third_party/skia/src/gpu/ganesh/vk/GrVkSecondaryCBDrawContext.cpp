    ...
    GrRecordingContext* rContext, const SkImageInfo& imageInfo, const GrVkDrawableInfo& vkInfo, const SkSurfaceProps* props)
{
    ...
    sk_sp<GrSurfaceProxy> proxy(rContext->priv().proxyProvider()->wrapVulkanSecondaryCBAsRenderTarget(imageInfo, vkInfo));
    if (!proxy) {
        return nullptr;
    }
    ...
    auto device = rContext->priv().createDevice(SkColorTypeToGrColorType(imageInfo.colorType()), std::move(proxy), imageInfo.refColorSpace(),
        kTopLeft_GrSurfaceOrigin, SkSurfacePropsCopyOrDefault(props), skgpu::v1::Device::InitContents::kUninit);
    if (!device) {
        return nullptr;
    }
...
GrVkSecondaryCBDrawContext::GrVkSecondaryCBDrawContext(sk_sp<skgpu::v1::Device> device, const SkSurfaceProps* props)
    : fDevice(device)
...
bool GrVkSecondaryCBDrawContext::characterize(SkSurfaceCharacterization* characterization) const
{
    ...
    characterization->set(direct->threadSafeProxy(), maxResourceBytes, ii, format, readSurfaceView.origin(), numSamples,
        SkSurfaceCharacterization::Textureable(false), SkSurfaceCharacterization::MipMapped(false), SkSurfaceCharacterization::UsesGLFBO0(false),
        SkSurfaceCharacterization::VkRTSupportsInputAttachment(false), SkSurfaceCharacterization::VulkanSecondaryCBCompatible(true), isProtected,
        this->props());

    return true;
}

bool GrVkSecondaryCBDrawContext::isCompatible(const SkSurfaceCharacterization& characterization) const
{
