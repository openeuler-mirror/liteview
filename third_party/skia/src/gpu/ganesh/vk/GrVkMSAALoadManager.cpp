...
bool GrVkMSAALoadManager::createMSAALoadProgram(GrVkGpu* gpu)
{
    ...
    vertShaderText.append("layout(set = 0, binding = 0) uniform vertexUniformBuffer {"
                          "half4 uPosXform;"
                          "};"
    ...
    fragShaderText.append("layout(input_attachment_index = 0, set = 2, binding = 0) uniform subpassInput uInput;"
    ...
    if (!GrCompileVkShaderModule(gpu, vertShaderText, VK_SHADER_STAGE_VERTEX_BIT, &fVertShaderModule, &fShaderStageInfo[0], settings, &spirv, &inputs)) {
        this->destroyResources(gpu);
        return false;
    }
    SkASSERT(inputs == SkSL::Program::Inputs());

    if (!GrCompileVkShaderModule(gpu, fragShaderText, VK_SHADER_STAGE_FRAGMENT_BIT, &fFragShaderModule, &fShaderStageInfo[1], settings, &spirv, &inputs)) {
        this->destroyResources(gpu);
        return false;
    }
    SkASSERT(inputs == SkSL::Program::Inputs());
