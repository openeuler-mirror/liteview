...
void GrVkCommandBuffer::submitPipelineBarriers(const GrVkGpu* gpu, bool forSelfDependency)
{
    ...
    if (fBufferBarriers.count() || fImageBarriers.count()) {
        ...
        GR_VK_CALL(gpu->vkInterface(),
            CmdPipelineBarrier(fCmdBuffer, fSrcStageMask, fDstStageMask, dependencyFlags, 0, nullptr, fBufferBarriers.count(), fBufferBarriers.begin(),
...
static bool submit_to_queue(GrVkGpu* gpu, VkQueue queue, VkFence fence, uint32_t waitCount, const VkSemaphore* waitSemaphores,
...
bool GrVkPrimaryCommandBuffer::submitToQueue(
    GrVkGpu* gpu, VkQueue queue, SkTArray<GrVkSemaphore::Resource*>& signalSemaphores, SkTArray<GrVkSemaphore::Resource*>& waitSemaphores)
{
    SkASSERT(!fIsActive);
    ...
    if (0 == signalCount && 0 == waitCount) {
        ...
        submitted = submit_to_queue(
            gpu, queue, fSubmitFence, 0, nullptr, nullptr, 1, &fCmdBuffer, 0, nullptr, gpu->protectedContext() ? GrProtected::kYes : GrProtected::kNo);
    } else {
        ...
        submitted = submit_to_queue(gpu, queue, fSubmitFence, vkWaitSems.count(), vkWaitSems.begin(), vkWaitStages.begin(), 1, &fCmdBuffer,
            vkSignalSems.count(), vkSignalSems.begin(), gpu->protectedContext() ? GrProtected::kYes : GrProtected::kNo);
        if (submitted) {
            for (int i = 0; i < signalCount; ++i) {
