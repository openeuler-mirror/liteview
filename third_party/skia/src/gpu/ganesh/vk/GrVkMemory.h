 ...
 */
bool AllocAndBindBufferMemory(GrVkGpu* gpu, VkBuffer buffer, skgpu::VulkanMemoryAllocator::BufferUsage, skgpu::VulkanAlloc* alloc);
void FreeBufferMemory(const GrVkGpu* gpu, const skgpu::VulkanAlloc& alloc);

bool AllocAndBindImageMemory(GrVkGpu* gpu, VkImage image, GrMemoryless, skgpu::VulkanAlloc* alloc);
...
void FlushMappedAlloc(GrVkGpu* gpu, const skgpu::VulkanAlloc& alloc, VkDeviceSize offset, VkDeviceSize size);
void InvalidateMappedAlloc(GrVkGpu* gpu, const skgpu::VulkanAlloc& alloc, VkDeviceSize offset, VkDeviceSize size);

// Helper for aligning and setting VkMappedMemoryRange for flushing/invalidating noncoherent
// memory.
void GetNonCoherentMappedMemoryRange(const skgpu::VulkanAlloc&, VkDeviceSize offset, VkDeviceSize size, VkDeviceSize alignment, VkMappedMemoryRange*);
} // namespace GrVkMemory
