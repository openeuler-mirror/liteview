...
void GrGLSLFragmentShaderBuilder::enableCustomOutput()
{
    if (!fCustomColorOutput) {
        fCustomColorOutput = &fOutputs.emplace_back(DeclaredColorOutputName(), SkSLType::kHalf4, GrShaderVar::TypeModifier::Out);
...
{
    return fCustomColorOutput && fCustomColorOutput->getTypeModifier() == GrShaderVar::TypeModifier::InOut;
}
