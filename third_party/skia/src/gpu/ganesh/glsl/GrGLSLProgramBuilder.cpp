...
bool GrGLSLProgramBuilder::emitAndInstallPrimProc(SkString* outputColor, SkString* outputCoverage)
{
    ...
    for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
        ...
        texSamplers[i] = this->emitSampler(geomProc.textureSampler(i).backendFormat(), sampler.samplerState(), sampler.swizzle(), name.c_str());
...
SkString GrGLSLProgramBuilder::emitRootFragProc(const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl, const SkString& input, SkString output)
{
    ...
    bool ok = true;
    fp.visitWithImpls(
        [&, samplerIdx = 0](const GrFragmentProcessor& fp, GrFragmentProcessor::ProgramImpl& impl) mutable {
    ...
    if (fp.isBlendFunction()) {
        if (this->fragmentProcessorHasCoordsParam(&fp)) {
            fFS.codeAppendf("%s = %s(%s, half4(1), %s);", output.c_str(), impl.functionName(), input.c_str(), fLocalCoordsVar.c_str());
        } else {
            fFS.codeAppendf("%s = %s(%s, half4(1));", output.c_str(), impl.functionName(), input.c_str());
        }
    } else {
        if (this->fragmentProcessorHasCoordsParam(&fp)) {
            fFS.codeAppendf("%s = %s(%s, %s);", output.c_str(), impl.functionName(), input.c_str(), fLocalCoordsVar.c_str());
        } else {
