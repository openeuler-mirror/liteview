...
void GrGLSLShaderBuilder::appendColorGamutXform(SkString* out, const char* srcColor, GrGLSLColorSpaceXformHelper* colorXformHelper)
{
    ...
    auto emitTFFunc = [=](const char* name, GrGLSLProgramDataManager::UniformHandle uniform, TFKind kind) {
        ...
        this->emitFunction(SkSLType::kHalf, funcName.c_str(), { gTFArgs, std::size(gTFArgs) }, body.c_str());
        return funcName;
    };

    SkString srcTFFuncName;
    if (colorXformHelper->applySrcTF()) {
        srcTFFuncName = emitTFFunc("src_tf", colorXformHelper->srcTFUniform(), colorXformHelper->srcTFKind());
    }

    SkString dstTFFuncName;
    if (colorXformHelper->applyDstTF()) {
        dstTFFuncName = emitTFFunc("dst_tf", colorXformHelper->dstTFUniform(), colorXformHelper->dstTFKind());
    }
    ...
    if (colorXformHelper->applyGamutXform()) {
        ...
        this->emitFunction(SkSLType::kHalf4, gamutXformFuncName.c_str(), { gGamutXformArgs, std::size(gGamutXformArgs) }, body.c_str());
    }
    ...
    // Now define a wrapper function that applies all the intermediate steps
    {
        ...
        const GrShaderVar gColorXformArgs[] = { GrShaderVar("color", useFloat ? SkSLType::kFloat4 : SkSLType::kHalf4) };
        SkString body;
