...
bool SurfaceContext::readPixels(GrDirectContext* dContext, GrPixmap dst, SkIPoint pt)
{
    ...
    if (readFlag == GrCaps::SurfaceReadPixelsSupport::kCopyToTexture2D || canvas2DFastPath) {
        ...
        if (this->asTextureProxy()) {
            GrColorType colorType = (canvas2DFastPath || srcIsCompressed) ? GrColorType::kRGBA_8888 : this->colorInfo().colorType();
...
bool SurfaceContext::internalWritePixels(GrDirectContext* dContext, const GrCPixmap src[], int numLevels, SkIPoint pt)
{
    ...
    SkASSERT(numLevels == 1 || (this->asTextureProxy() && this->asTextureProxy()->mipmapped() == GrMipmapped::kYes));
    ...
    if ((!caps->surfaceSupportsWritePixels(dstSurface) || canvas2DFastPath) && numLevels == 1) {
        ...
        auto tempProxy = dContext->priv().proxyProvider()->createProxy(format, src[0].dimensions(), GrRenderable::kNo, 1, GrMipmapped::kNo,
    ...
    RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext callbackContext)
{
    ...
    auto readInfo = this->caps()->supportedReadPixelsColorType(colorTypeOfFinalContext, backendFormatOfFinalContext, dstCT);
...
public:
    ...
    bool addTransferResult(const PixelTransferResult& result, SkISize dimensions, size_t rowBytes, GrClientMappedBufferManager* manager)
    {
...
private:
    ...
    public:
        ...
        {
            if (fMappedBuffer) {
                GrClientMappedBufferManager::BufferFinishedMessageBus::Post({ std::move(fMappedBuffer), intendedRecipient });
            }
        }
    ...
    GrDirectContext* dContext, const SkIRect& rect, SkColorType colorType, ReadPixelsCallback callback, ReadPixelsContext callbackContext)
{
    ...
    auto* finishContext = new FinishContext { callback, callbackContext, rect.size(), colorType, this->caps()->transferBufferRowBytesAlignment(),
        mappedBufferManager, std::move(transferResult) };
    auto finishCallback = [](GrGpuFinishedContext c) {
        ...
        size_t rowBytes = SkAlignTo(context->fSize.width() * SkColorTypeBytesPerPixel(context->fColorType), context->fBufferAlignment);
    ...
    dContext->priv().flushSurface(this->asSurfaceProxy(), SkSurface::BackendSurfaceAccess::kNoAccess, flushInfo);
}

void SurfaceContext::asyncRescaleAndReadPixelsYUV420(GrDirectContext* dContext, SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace,
    const SkIRect& srcRect, SkISize dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext callbackContext)
{
    ...
    if (needsRescale) {
        ...
        srcView = GrSurfaceProxyView::Copy(fContext, std::move(srcView), GrMipmapped::kNo, srcRect, SkBackingFit::kApprox, SkBudgeted::kYes,
    ...
    // explicit flush from the caller. We'd have to have a way to defer attaching the finish
    // callback to GrGpu until after the next flush that flushes our op list, though.
    auto* finishContext = new FinishContext { callback, callbackContext, dContext->priv().clientMappedBufferManager(), dstSize,
        this->caps()->transferBufferRowBytesAlignment(), std::move(yTransfer), std::move(uTransfer), std::move(vTransfer) };
    auto finishCallback = [](GrGpuFinishedContext c) {
        const auto* context = reinterpret_cast<const FinishContext*>(c);
        auto manager = context->fMappedBufferManager;
        auto result = std::make_unique<AsyncReadResult>(manager->ownerID());
...
sk_sp<GrRenderTask> SurfaceContext::copy(sk_sp<GrSurfaceProxy> src, SkIRect srcRect, SkIPoint dstPoint)
{
    ...
    SkDEBUGCODE(this->validate();) GR_CREATE_TRACE_MARKER_CONTEXT("SurfaceContext", "copy", fContext);
    ...
    return this->drawingManager()->newCopyRenderTask(
        this->asSurfaceProxyRef(), dstRect, std::move(src), srcRect, GrSamplerState::Filter::kNearest, this->origin());
}
...
{
    auto sfc = fContext->priv().makeSFCWithFallback(info, SkBackingFit::kExact, 1, GrMipmapped::kNo, this->asSurfaceProxy()->isProtected(), origin);
...
bool SurfaceContext::rescaleInto(SurfaceFillContext* dst, SkIRect dstRect, SkIRect srcRect, RescaleGamma rescaleGamma, RescaleMode rescaleMode)
{
    ...
    if (!texView.asTextureProxy()) {
        ...
        texView = GrSurfaceProxyView::Copy(fContext, std::move(texView), GrMipmapped::kNo, srcRect, SkBackingFit::kApprox, SkBudgeted::kNo,
    ...
    if (rescaleGamma == RescaleGamma::kLinear && this->colorInfo().colorSpace() && !this->colorInfo().colorSpace()->gammaIsLinear()) {
        ...
        auto linearRTC = fContext->priv().makeSFCWithFallback(std::move(ii), SkBackingFit::kApprox, 1, GrMipmapped::kNo, GrProtected::kNo, dst->origin());
        if (!linearRTC) {
            return false;
        }
    ...
    do {
        ...
        if (rescaleMode == RescaleMode::kRepeatedCubic) {
            ...
            fp = GrTextureEffect::MakeSubset(std::move(texView), this->colorInfo().alphaType(), SkMatrix::I(), { filter, GrSamplerState::MipmapMode::kNone },
                srcRectF, srcRectF, *this->caps());
        }
        if (xform) {
...
SurfaceContext::PixelTransferResult SurfaceContext::transferPixels(GrColorType dstCT, const SkIRect& rect)
{
    ...
    if (supportedRead.fColorType != dstCT || flip) {
        result.fPixelConverter = [w = rect.width(), h = rect.height(), dstCT, supportedRead, at](void* dst, const void* src) {
            GrImageInfo srcInfo(supportedRead.fColorType, at, nullptr, w, h);
            GrImageInfo dstInfo(dstCT, at, nullptr, w, h);
            GrConvertPixels(GrPixmap(dstInfo, dst, dstInfo.minRowBytes()), GrCPixmap(srcInfo, src, srcInfo.minRowBytes()));
        };
    }
    return result;
}
