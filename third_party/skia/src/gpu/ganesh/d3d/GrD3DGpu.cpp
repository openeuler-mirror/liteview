...
sk_sp<GrGpu> GrD3DGpu::Make(const GrD3DBackendContext& backendContext, const GrContextOptions& contextOptions, GrDirectContext* direct)
{
...
{
    this->initCapsAndCompiler(sk_make_sp<GrD3DCaps>(contextOptions, backendContext.fAdapter.get(), backendContext.fDevice.get()));
...
bool GrD3DGpu::submitDirectCommandList(SyncQueue sync)
{
    ...
    new (fOutstandingCommandLists.push_back()) OutstandingCommandList(std::move(fCurrentDirectCommandList), fence);
...
sk_sp<GrD3DTexture> GrD3DGpu::createD3DTexture(SkISize dimensions, DXGI_FORMAT dxgiFormat, GrRenderable renderable, int renderTargetSampleCnt,
    SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, GrMipmapStatus mipmapStatus, std::string_view label)
{
    ...
    SkISize dimensions, const GrBackendFormat& format, SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    SkASSERT(dataSize == SkCompressedFormatDataSize(compression, dimensions, mipmapped == GrMipmapped::kYes));
...
bool GrD3DGpu::onReadPixels(GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* buffer, size_t rowBytes)
{
    ...
    sk_sp<GrGpuBuffer> transferBuffer = this->createBuffer(transferTotalBytes, GrGpuBufferType::kXferGpuToCpu, kDynamic_GrAccessPattern);
...
bool GrD3DGpu::onRegenerateMipMapLevels(GrTexture* tex)
{
    ...
    if (caps.isFormatUnorderedAccessible(originalFormat) && (originalDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)) {
        ...
        uavTexture = GrD3DTexture::MakeNewTexture(this, SkBudgeted::kNo, tex->dimensions(), uavDesc, grProtected, GrMipmapStatus::kDirty,
...
GrBackendTexture GrD3DGpu::onCreateCompressedBackendTexture(SkISize dimensions, const GrBackendFormat& format, GrMipmapped mipmapped, GrProtected isProtected)
{
