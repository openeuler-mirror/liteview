...
gr_cp<ID3DBlob> GrD3DPipelineStateBuilder::compileD3DProgram(
    SkSL::ProgramKind kind, const std::string& sksl, const SkSL::ProgramSettings& settings, SkSL::Program::Inputs* outInputs, std::string* outHLSL)
{
#ifdef SK_DEBUG
    ...
    std::unique_ptr<SkSL::Program> program = fGpu->shaderCompiler()->convertProgram(kind, src, settings);
...
std::unique_ptr<GrD3DPipelineState> GrD3DPipelineStateBuilder::finalize()
{
    ...
    if (kHLSL_Tag == shaderType && this->loadHLSLFromCache(&reader, shaders)) {
        ...
        auto compile = [&](SkSL::ProgramKind kind, GrShaderType shaderType) {
            shaders[shaderType] = this->compileD3DProgram(kind, *sksl[shaderType], settings, &inputs[shaderType], &hlsl[shaderType]);
            return shaders[shaderType].get();
        };
        ...
        if (persistentCache && !cached) {
            ...
            sk_sp<SkData> data = GrPersistentCacheUtils::PackCachedShaders(cacheSkSL ? kSKSL_Tag : kHLSL_Tag, hlsl, inputs, kGrShaderTypeCount);
            persistentCache->store(*key, *data, description);
        }
    }

    sk_sp<GrD3DRootSignature> rootSig = fGpu->resourceProvider().findOrCreateRootSignature(fUniformHandler.fTextures.count());
    if (!rootSig) {
        return nullptr;
    }
