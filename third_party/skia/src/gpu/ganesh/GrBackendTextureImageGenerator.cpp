...
std::unique_ptr<SkImageGenerator> GrBackendTextureImageGenerator::Make(sk_sp<GrTexture> texture, GrSurfaceOrigin origin, std::unique_ptr<GrSemaphore> semaphore,
    SkColorType colorType, SkAlphaType alphaType, sk_sp<SkColorSpace> colorSpace)
{
    ...
    return std::unique_ptr<SkImageGenerator>(
        new GrBackendTextureImageGenerator(info, std::move(texture), origin, dContext->directContextID(), std::move(semaphore)));
}

GrBackendTextureImageGenerator::GrBackendTextureImageGenerator(const SkColorInfo& info, sk_sp<GrTexture> texture, GrSurfaceOrigin origin,
...
GrSurfaceProxyView GrBackendTextureImageGenerator::onGenerateTexture(
    GrRecordingContext* rContext, const SkImageInfo& info, GrMipmapped mipmapped, GrImageTexGenPolicy texGenPolicy)
{
    SkASSERT(rContext);
    ...
    GrMipmapped textureIsMipMapped = fBackendTexture.hasMipmaps() ? GrMipmapped::kYes : GrMipmapped::kNo;
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createLazyProxy(
        ...
        backendFormat, fBackendTexture.dimensions(), textureIsMipMapped, mipmapStatus, GrInternalSurfaceFlags::kReadOnly, SkBackingFit::kExact, SkBudgeted::kNo,
        GrProtected::kNo, GrSurfaceProxy::UseAllocator::kYes, "BackendTextureImageGenerator");
    if (!proxy) {
        return {};
    }

    if (texGenPolicy == GrImageTexGenPolicy::kDraw && (mipmapped == GrMipmapped::kNo || proxy->mipmapped() == GrMipmapped::kYes)) {
