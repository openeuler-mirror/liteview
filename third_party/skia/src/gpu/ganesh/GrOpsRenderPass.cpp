...
void GrOpsRenderPass::bindPipeline(const GrProgramInfo& programInfo, const SkRect& drawBounds)
{
    ...
    if (!hasTextures) {
        programInfo.pipeline().visitProxies([&hasTextures](GrSurfaceProxy*, GrMipmapped) { hasTextures = true; });
    }
...
void GrOpsRenderPass::bindTextures(const GrGeometryProcessor& geomProc, const GrSurfaceProxy* const geomProcTextures[], const GrPipeline& pipeline)
{
    ...
    for (int i = 0; i < geomProc.numTextureSamplers(); ++i) {
        ...
        SkASSERT(tex);
        if (sampler.samplerState().mipmapped() == GrMipmapped::kYes && (tex->width() != 1 || tex->height() != 1)) {
