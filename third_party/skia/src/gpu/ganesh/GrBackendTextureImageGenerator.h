...
public:
    static std::unique_ptr<SkImageGenerator> Make(
        sk_sp<GrTexture>, GrSurfaceOrigin, std::unique_ptr<GrSemaphore>, SkColorType, SkAlphaType, sk_sp<SkColorSpace>);
...
protected:
    ...
    GrSurfaceProxyView onGenerateTexture(GrRecordingContext*, const SkImageInfo&, GrMipmapped mipmapped, GrImageTexGenPolicy) override;

private:
    GrBackendTextureImageGenerator(
        const SkColorInfo&, sk_sp<GrTexture>, GrSurfaceOrigin, GrDirectContext::DirectContextID owningContextID, std::unique_ptr<GrSemaphore>);
