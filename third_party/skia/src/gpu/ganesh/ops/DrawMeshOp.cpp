...
public:
    static GrGeometryProcessor* Make(SkArenaAlloc* arena, sk_sp<SkMeshSpecification> spec, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix,
        const std::optional<SkPMColor4f>& color, bool needsLocalCoords, sk_sp<const SkData> uniforms)
    {
        return arena->make([&](void* ptr) {
            return new (ptr) MeshGP(std::move(spec), std::move(colorSpaceXform), viewMatrix, std::move(color), needsLocalCoords, std::move(uniforms));
        });
    }
...
private:
    ...
    private:
        ...
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            if (meshColorType == SkMeshSpecificationPriv::ColorType::kNone) {
                fragBuilder->codeAppendf("%s %s(varyings);", localCoordAssignment.c_str(), userFragName.c_str());
                ...
                fragBuilder->codeAppendf("%s %s(varyings, color);", localCoordAssignment.c_str(), userFragName.c_str());
    ...
    MeshGP(sk_sp<SkMeshSpecification> spec, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrix& viewMatrix, const std::optional<SkPMColor4f>& color,
        bool needsLocalCoords, sk_sp<const SkData> uniforms)
        ...
        , fNeedsLocalCoords(needsLocalCoords)
    {
        ...
        for (const auto& srcAttr : fSpec->attributes()) {
            fAttributes.emplace_back(srcAttr.name.c_str(), SkMeshSpecificationPriv::AttrTypeAsVertexAttribType(srcAttr.type),
...
public:
    ...
    MeshOp(GrProcessorSet*, const SkPMColor4f&, const SkMesh&, GrAAType, sk_sp<GrColorSpaceXform>, const SkMatrixProvider&);

    MeshOp(GrProcessorSet*, const SkPMColor4f&, sk_sp<SkVertices>, const GrPrimitiveType*, GrAAType, sk_sp<GrColorSpaceXform>, const SkMatrixProvider&);
...
MeshOp::MeshOp(GrProcessorSet* processorSet, const SkPMColor4f& color, const SkMesh& mesh, GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform,
    ...
    , fViewMatrix(matrixProvider.localToDevice())
{
    ...
    if (fColorSpaceXform) {
        fUniforms = SkRuntimeEffectPriv::TransformUniforms(mesh.spec()->uniforms(), mesh.refUniforms(), fColorSpaceXform->steps());
    } else {
...
MeshOp::MeshOp(GrProcessorSet* processorSet, const SkPMColor4f& color, sk_sp<SkVertices> vertices, const GrPrimitiveType* overridePrimitiveType,
    GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform, const SkMatrixProvider& matrixProvider)
    : INHERITED(ClassID())
    , fHelper(processorSet, aaType)
    , fColorSpaceXform(std::move(colorSpaceXform))
    , fColor(color)
...
GrGeometryProcessor* MeshOp::makeGP(SkArenaAlloc* arena)
{
    ...
    return MeshGP::Make(arena, fSpecification, fColorSpaceXform, vm, color, fHelper.usesLocalCoords(), fUniforms);
}
...
void MeshOp::onPrepareDraws(GrMeshDrawTarget* target)
{
    ...
    if (indexBuffer) {
        fMesh->setIndexed(std::move(indexBuffer), fIndexCount, firstIndex,
            /*minIndexValue=*/0, fVertexCount, GrPrimitiveRestart::kNo, std::move(vertexBuffer), firstVertex);
    } else {
...
GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, const SkMesh& mesh, const SkMatrixProvider& matrixProvider, GrAAType aaType,
    sk_sp<GrColorSpaceXform> colorSpaceXform)
{
    return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(context, std::move(paint), mesh, aaType, std::move(colorSpaceXform), matrixProvider);
}
...
GrOp::Owner Make(GrRecordingContext* context, GrPaint&& paint, sk_sp<SkVertices> vertices, const GrPrimitiveType* overridePrimitiveType,
    const SkMatrixProvider& matrixProvider, GrAAType aaType, sk_sp<GrColorSpaceXform> colorSpaceXform)
{
    return GrSimpleMeshDrawOpHelper::FactoryHelper<MeshOp>(
        context, std::move(paint), std::move(vertices), overridePrimitiveType, aaType, std::move(colorSpaceXform), matrixProvider);
}
