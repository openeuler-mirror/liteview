    ...
    private:
        void onEmitCode(EmitArgs& args, GrGPArgs* gpArgs) override
        {
            ...
            fragBuilder->codeAppend("float d = length(circleEdge.xy) * circleEdge.z;"
                                    ...
                                    "half(dashParams.w));"
                                    "half dashAlpha = 0;");
            fragBuilder->codeAppendf("if (angleFromStart - x + dashParams.y >= 6.28318530718) {"
            ...
            fragBuilder->codeAppendf("if (angleFromStart - x - dashParams.y < -0.01) {"
...
static const uint16_t gStrokeCircleIndices[] = {
    // enter the octagon
    // clang-format on
    0, 1, 9, 0, 9, 8, 1, 2, 10, 1, 10, 9, 2, 3, 11, 2, 11, 10, 3, 4, 12, 3, 12, 11, 4, 5, 13, 4, 13, 12, 5, 6, 14, 5, 14, 13, 6, 7, 15, 6, 15, 14, 7, 0, 8, 7,
    8, 15,
    // clang-format on
};
...
public:
        ...
        , fHelper(processorSet, GrAAType::kCoverage)
    {
        ...
        if (arcParams) {
            ...
            fRoundCaps = style.strokeRec().getWidth() > 0 && style.strokeRec().getCap() == SkPaint::kRound_Cap;
            SkPoint roundCaps[2];
            if (fRoundCaps) {
...
private:
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        for (const auto& ellipse : fEllipses) {
            ...
            } invRadii = { SkScalarInvert(xRadius), SkScalarInvert(yRadius), SkScalarInvert(ellipse.fInnerXRadius), SkScalarInvert(ellipse.fInnerYRadius) };
...
private:
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        PatternHelper helper(target, GrPrimitiveType::kTriangles, fProgramInfo->geomProc().vertexStride(), std::move(indexBuffer), kVertsPerStandardRRect,
            indicesPerInstance, fRRects.count(), kNumRRectsInIndexBuffer);
        VertexWriter verts { helper.vertices() };
        if (!verts) {
            SkDebugf("Could not allocate vertices\n");
            return;
        ...
        for (const auto& rrect : fRRects) {
            ...
            float reciprocalRadii[4]
                = { SkScalarInvert(rrect.fXRadius), SkScalarInvert(rrect.fYRadius), SkScalarInvert(rrect.fInnerXRadius), SkScalarInvert(rrect.fInnerYRadius) };
