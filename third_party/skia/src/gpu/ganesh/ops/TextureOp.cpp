...
std::tuple<bool /* filter */, bool /* mipmap */> filter_and_mm_have_effect(const GrQuad& srcQuad, const GrQuad& dstQuad)
{
...
private:
        ...
        , fMetadata(set[0].fProxyView.swizzle(), GrSamplerState::Filter::kNearest, GrSamplerState::MipmapMode::kNone, Subset::kNo, saturate)
    {
        ...
        int p = 0;
        for (int q = 0; q < cnt; ++q) {
            SkASSERT(mm == GrSamplerState::MipmapMode::kNone || (set[0].fProxyView.proxy()->asTextureProxy()->mipmapped() == GrMipmapped::kYes));
            if (q == 0) {
        ...
        const GrDstProxyView& dstProxyView, GrXferBarrierFlags renderPassXferBarriers, GrLoadOp colorLoadOp) override
    {
...

        {
            ...
            gp = skgpu::v1::QuadPerEdgeAA::MakeTexturedProcessor(arena, fDesc->fVertexSpec, *caps->shaderCaps(), backendFormat, samplerState,
    ...
    void characterize(Desc* desc) const
    {
        ...
        auto indexBufferOption = skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(overallAAType, maxQuadsPerMesh);
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        if (fDesc->fVertexSpec.needsIndexBuffer()) {
            fDesc->fIndexBuffer = skgpu::v1::QuadPerEdgeAA::GetIndexBuffer(target, fDesc->fVertexSpec.indexBufferOption());
...
public:
    BatchSizeLimiter(v1::SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* rContext, int numEntries, GrSamplerState::Filter filter,
...
void TextureOp::AddTextureSetOps(v1::SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* context, GrTextureSetEntry set[], int cnt,
...
GR_DRAW_OP_TEST_DEFINE(TextureOpImpl)
{
    ...
    sk_sp<GrTextureProxy> proxy = proxyProvider->createProxy(format, dims, GrRenderable::kNo, 1, mipmapped, fit, SkBudgeted::kNo, GrProtected::kNo,
