...
private:
    ...
    {
        auto indexBufferOption = skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), fQuads.count());
    ...
    void onPrepareDraws(GrMeshDrawTarget* target) override
    {
        ...
        if (vertexSpec.needsIndexBuffer()) {
            fIndexBuffer = skgpu::v1::QuadPerEdgeAA::GetIndexBuffer(target, vertexSpec.indexBufferOption());
            if (!fIndexBuffer) {
    ...
    void onExecute(GrOpFlushState* flushState, const SkRect& chainBounds) override
    {
        ...
        skgpu::v1::QuadPerEdgeAA::IssueDraw(flushState->caps(), flushState->opsRenderPass(), vertexSpec, 0, fQuads.count(), totalNumVertices, fBaseVertex);
    }
    ...
    bool canAddQuads(int numQuads, GrAAType aaType)
    {
        ...
        if (aaType != fHelper.aaType() && aaType != GrAAType::kNone) {
            ...
            auto indexBufferOption = skgpu::v1::QuadPerEdgeAA::CalcIndexBufferOption(fHelper.aaType(), quadCount);
            if (quadCount > skgpu::v1::QuadPerEdgeAA::QuadLimit(indexBufferOption)) {
...
void FillRectOp::AddFillRectOps(skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, GrRecordingContext* context, GrPaint&& paint, GrAAType aaType,
