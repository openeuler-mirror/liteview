...
size_t GrSurface::ComputeSize(const GrBackendFormat& format, SkISize dimensions, int colorSamplesPerPixel, GrMipmapped mipmapped, bool binSize)
{
    ...
    if (compressionType != SkImage::CompressionType::kNone) {
        colorSize = SkCompressedFormatDataSize(compressionType, dimensions, mipmapped == GrMipmapped::kYes);
    } else {
