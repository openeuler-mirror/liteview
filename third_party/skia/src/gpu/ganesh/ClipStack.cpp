...
GrFPResult analytic_clip_fp(const skgpu::v1::ClipStack::Element& e, const GrShaderCaps& caps, std::unique_ptr<GrFragmentProcessor> fp)
{
...
// into a shared atlas.
GrFPResult clip_atlas_fp(const skgpu::v1::SurfaceDrawContext* sdc, const GrOp* opBeingClipped, skgpu::v1::AtlasPathRenderer* atlasPathRenderer,
    const SkIRect& scissorBounds, const skgpu::v1::ClipStack::Element& e, std::unique_ptr<GrFragmentProcessor> inputFP)
{
    if (e.fAA != GrAA::kYes) {
...
void draw_to_sw_mask(GrSWMaskHelper* helper, const skgpu::v1::ClipStack::Element& e, bool clearMask)
{
...
GrSurfaceProxyView render_sw_mask(GrRecordingContext* context, const SkIRect& bounds, const skgpu::v1::ClipStack::Element** elements, int count)
{
    SkASSERT(count > 0);
    ...
    if (taskGroup) {
        ...
        auto proxy = proxyProvider->createProxy(format, bounds.size(), GrRenderable::kNo, 1, GrMipmapped::kNo, SkBackingFit::kApprox, SkBudgeted::kYes,
            GrProtected::kNo,
...
void render_stencil_mask(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, uint32_t genID, const SkIRect& bounds,
    const skgpu::v1::ClipStack::Element** elements, int count, GrAppliedClip* out)
{
...
ClipStack::ClipStack(const SkIRect& deviceBounds, const SkMatrixProvider* matrixProvider, bool forceAA)
...
GrClip::Effect ClipStack::apply(GrRecordingContext* rContext, SurfaceDrawContext* sdc, GrDrawOp* op, GrAAType aa, GrAppliedClip* out, SkRect* bounds) const
{
    ...
    if (!elementsForMask.empty()) {
        ...
        if ((sdc->numSamples() <= 1 && !sdc->canUseDynamicMSAA() && maskRequiresAA) || stencilUnavailable) {
            ...
            std::tie(hasSWMask, clipFP)
                = GetSWMaskFP(rContext, &fMasks, cs, scissorBounds, elementsForMask.begin(), elementsForMask.count(), std::move(clipFP));
        }
        ...
        if (!hasSWMask) {
            if (stencilUnavailable) {
                ...
                render_stencil_mask(rContext, sdc, cs.genID(), scissorBounds, elementsForMask.begin(), elementsForMask.count(), out);
            }
        }
    }
