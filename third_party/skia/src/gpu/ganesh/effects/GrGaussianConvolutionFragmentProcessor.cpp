...
{
    const GrGaussianConvolutionFragmentProcessor& ce = args.fFp.cast<GrGaussianConvolutionFragmentProcessor>();

    GrGLSLUniformHandler* uniformHandler = args.fUniformHandler;

    const char* increment;
    fIncrementUni = uniformHandler->addUniform(&ce, kFragment_GrShaderFlag, SkSLType::kHalf2, "Increment", &increment);
    ...
    fOffsetsAndKernelUni = uniformHandler->addUniformArray(&ce, kFragment_GrShaderFlag, SkSLType::kHalf2, "OffsetsAndKernel", arrayCount, &offsetsAndKernel);
    ...
    const GrShaderVar smoothArgs[] = { { args.fInputColor, SkSLType::kHalf4 }, { "coord", SkSLType::kFloat2 }, { "offsetAndKernel", SkSLType::kHalf2 } };
    ...
    fragBuilder->emitFunction(SkSLType::kHalf4, smoothFuncName.c_str(), { smoothArgs, std::size(smoothArgs) }, smoothBody.c_str());
    ...
    if (variableLengthLoop) {
        ...
        fKernelWidthUni = uniformHandler->addUniform(&ce, kFragment_GrShaderFlag, SkSLType::kInt, "KernelWidth", &kernelWidth);
...
void GrGaussianConvolutionFragmentProcessor::Impl::onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& processor)
{
...
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::Make(GrSurfaceProxyView view, SkAlphaType alphaType, Direction dir, int halfWidth,
    float gaussianSigma, GrSamplerState::WrapMode wm, const SkIRect& subset, const SkIRect* pixelDomain, const GrCaps& caps)
{
    ...
    GrSamplerState::Filter filter = is_zero_sigma ? GrSamplerState::Filter::kNearest : GrSamplerState::Filter::kLinear;
    ...
    if (pixelDomain && !alwaysUseShaderTileMode) {
        ...
        switch (dir) {
        case Direction::kX:
            domain.outset(halfWidth, 0);
            break;
        case Direction::kY:
            domain.outset(0, halfWidth);
            break;
        }
        child = GrTextureEffect::MakeSubset(
            std::move(view), alphaType, SkMatrix::I(), sampler, SkRect::Make(subset), domain, caps, GrTextureEffect::kDefaultBorder);
    } else {
        child = GrTextureEffect::MakeSubset(
            std::move(view), alphaType, SkMatrix::I(), sampler, SkRect::Make(subset), caps, GrTextureEffect::kDefaultBorder, alwaysUseShaderTileMode);
    }

    if (is_zero_sigma) {
        return child;
    }
    return std::unique_ptr<GrFragmentProcessor>(new GrGaussianConvolutionFragmentProcessor(std::move(child), dir, halfWidth, gaussianSigma));
}

GrGaussianConvolutionFragmentProcessor::GrGaussianConvolutionFragmentProcessor(
    std::unique_ptr<GrFragmentProcessor> child, Direction direction, int radius, float gaussianSigma)
    : INHERITED(kGrGaussianConvolutionFragmentProcessor_ClassID, ProcessorOptimizationFlags(child.get()))
    , fRadius(radius)
...
GrGaussianConvolutionFragmentProcessor::GrGaussianConvolutionFragmentProcessor(const GrGaussianConvolutionFragmentProcessor& that)
    : INHERITED(that)
...
void GrGaussianConvolutionFragmentProcessor::onAddToKey(const GrShaderCaps& shaderCaps, skgpu::KeyBuilder* b) const
{
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrGaussianConvolutionFragmentProcessor::onMakeProgramImpl() const
{
...
bool GrGaussianConvolutionFragmentProcessor::onIsEqual(const GrFragmentProcessor& sBase) const
{
    ...
    return fRadius == that.fRadius && fDirection == that.fDirection
        && std::equal(fOffsetsAndKernel, fOffsetsAndKernel + SkGpuBlurUtils::LinearKernelWidth(fRadius), that.fOffsetsAndKernel);
}
...
#if GR_TEST_UTILS
std::unique_ptr<GrFragmentProcessor> GrGaussianConvolutionFragmentProcessor::TestCreate(GrProcessorTestData* d)
{
    ...
    auto wm = static_cast<GrSamplerState::WrapMode>(d->fRandom->nextULessThan(GrSamplerState::kWrapModeCount));
    ...
    return GrGaussianConvolutionFragmentProcessor::Make(std::move(view), at, dir, radius, sigma, wm, subset, domain, *d->caps());
}
#endif
