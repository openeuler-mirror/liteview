...
void GrMatrixConvolutionEffect::Impl::emitCode(EmitArgs& args)
{
    ...
    GrGLSLUniformHandler* uniformHandler = args.fUniformHandler;
    if (mce.fKernel.isSampled()) {
        fKernelBiasUni = uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, SkSLType::kHalf, "KernelBias");
    } else {
        fKernelUni = uniformHandler->addUniformArray(&mce, kFragment_GrShaderFlag, SkSLType::kHalf4, "Kernel", arrayCount);
    }
    fKernelOffsetUni = uniformHandler->addUniform(&mce, kFragment_GrShaderFlag, SkSLType::kHalf2, "KernelOffset");
...
void GrMatrixConvolutionEffect::Impl::onSetData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& processor)
{
...
GrMatrixConvolutionEffect::GrMatrixConvolutionEffect(std::unique_ptr<GrFragmentProcessor> child, const KernelWrapper& kernel,
    std::unique_ptr<GrFragmentProcessor> kernelFP, SkScalar gain, SkScalar bias, const SkIPoint& kernelOffset, bool convolveAlpha)
...
std::unique_ptr<GrFragmentProcessor::ProgramImpl> GrMatrixConvolutionEffect::onMakeProgramImpl() const
{
...
bool GrMatrixConvolutionEffect::onIsEqual(const GrFragmentProcessor& sBase) const
{
    ...
    return fKernel == s.fKernel && fGain == s.fGain && fBias == s.fBias && fKernelOffset == s.fKernelOffset && fConvolveAlpha == s.fConvolveAlpha;
}

std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::Make(GrRecordingContext* context, GrSurfaceProxyView srcView, const SkIRect& srcBounds,
    const SkISize& kernelSize, const SkScalar* kernel, SkScalar gain, SkScalar bias, const SkIPoint& kernelOffset, GrSamplerState::WrapMode wm,
    bool convolveAlpha, const GrCaps& caps)
{
    ...
    auto child = GrTextureEffect::MakeSubset(std::move(srcView), kPremul_SkAlphaType, SkMatrix::I(), sampler, SkRect::Make(srcBounds), caps);
    return std::unique_ptr<GrFragmentProcessor>(
        new GrMatrixConvolutionEffect(std::move(child), kernelWrapper, std::move(kernelFP), gain, bias, kernelOffset, convolveAlpha));
}
...
std::unique_ptr<GrFragmentProcessor> GrMatrixConvolutionEffect::TestCreate(GrProcessorTestData* d)
{
    ...
    return GrMatrixConvolutionEffect::Make(
        d->context(), std::move(view), bounds, kernelSize, kernel.get(), gain, bias, kernelOffset, wm, convolveAlpha, *d->caps());
}
#endif
