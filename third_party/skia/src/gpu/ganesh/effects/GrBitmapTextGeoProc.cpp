...
GrBitmapTextGeoProc::GrBitmapTextGeoProc(const GrShaderCaps& caps, const SkPMColor4f& color, bool wideColor, const GrSurfaceProxyView* views,
    int numActiveViews, GrSamplerState params, MaskFormat format, const SkMatrix& localMatrix, bool usesW)
...
GrGeometryProcessor* GrBitmapTextGeoProc::TestCreate(GrProcessorTestData* d)
{
    ...
    return GrBitmapTextGeoProc::Make(
        d->allocator(), *d->caps()->shaderCaps(), SkPMColor4f::FromBytes_RGBA(color), wideColor, &view, 1, samplerState, format, localMatrix, usesW);
}
#endif
