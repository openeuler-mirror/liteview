    ...
    bool alwaysUseShaderTileMode, const GrCaps& caps, SkVector linearFilterInset)
{
    ...
    if (aniso) {
        bool anisoSubset = !proxy.backingStoreBoundsRect().contains(subset) && (!domain || !subset.contains(*domain));
        ...
        if (needsShaderWrap || anisoSubset) {
            MipmapMode newMM = proxy.asTextureProxy()->mipmapped() == GrMipmapped::kYes ? MipmapMode::kLinear : MipmapMode::kNone;
