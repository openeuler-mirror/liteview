    ...
    void emitCode(EmitArgs& args) override
    {
        ...
        public:
            ...
            std::string declareUniform(const SkSL::VarDeclaration* decl) override
            {
                ...
                auto handle = fArgs.fUniformHandler->addUniformArray(&fArgs.fFp.cast<GrSkSLFP>(), kFragment_GrShaderFlag, gpuType, SkString(var.name()).c_str(),
                    isArray ? var.type().columns() : 0, &uniformName);
...
std::unique_ptr<GrSkSLFP> GrSkSLFP::MakeWithData(sk_sp<SkRuntimeEffect> effect, const char* name, sk_sp<SkColorSpace> dstColorSpace,
    std::unique_ptr<GrFragmentProcessor> inputFP, std::unique_ptr<GrFragmentProcessor> destColorFP, sk_sp<const SkData> uniforms,
...
GrSkSLFP::GrSkSLFP(sk_sp<SkRuntimeEffect> effect, const char* name, OptFlags optFlags)
    : INHERITED(kGrSkSLFP_ClassID,
        static_cast<OptimizationFlags>(optFlags)
            | (effect->getFilterColorProgram() ? kConstantOutputForConstantInput_OptimizationFlag : kNone_OptimizationFlags))
...
void GrSkSLFP::addColorTransformChildren(sk_sp<SkColorSpace> dstColorSpace)
{
    ...
    auto workingToLinear
        = GrColorSpaceXformEffect::Make(nullptr, dstColorSpace.get(), kUnpremul_SkAlphaType, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType);
    auto linearToWorking
        = GrColorSpaceXformEffect::Make(nullptr, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType, dstColorSpace.get(), kUnpremul_SkAlphaType);
...
SkPMColor4f GrSkSLFP::constantOutputForConstantInput(const SkPMColor4f& inputColor) const
{
    ...
    auto evalChild = [&](int index, SkPMColor4f color) { return ConstantOutputForConstantInput(this->childProcessor(index), color); };
...
std::unique_ptr<GrFragmentProcessor> GrSkSLFP::TestCreate(GrProcessorTestData* d)
{
    ...
    auto [success, fp] = as_CFB(filter)->asFragmentProcessor(/*inputFP=*/nullptr, d->context(), GrColorInfo {}, props);
    SkASSERT(success);
    return std::move(fp);
}
