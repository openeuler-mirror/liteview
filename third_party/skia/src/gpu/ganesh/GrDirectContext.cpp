...
void GrDirectContext::performDeferredCleanup(std::chrono::milliseconds msNotUsed, bool scratchResourcesOnly)
{
...
GrSemaphoresSubmitted GrDirectContext::flush(const GrFlushInfo& info)
{
    ...
    return this->drawingManager()->flushSurfaces({}, SkSurface::BackendSurfaceAccess::kNoAccess, info, nullptr);
}
...
GrBackendTexture GrDirectContext::createBackendTexture(int width, int height, const GrBackendFormat& backendFormat, GrMipmapped mipmapped,
...
GrBackendTexture GrDirectContext::createBackendTexture(
    int width, int height, SkColorType skColorType, GrMipmapped mipmapped, GrRenderable renderable, GrProtected isProtected, std::string_view label)
{
    if (this->abandoned()) {
        return GrBackendTexture();
    }
...
static GrBackendTexture create_and_clear_backend_texture(GrDirectContext* dContext, SkISize dimensions, const GrBackendFormat& backendFormat,
    GrMipmapped mipmapped, GrRenderable renderable, GrProtected isProtected, sk_sp<skgpu::RefCntedCallback> finishedCallback, std::array<float, 4> color,
    ...
    GrSurfaceOrigin textureOrigin, sk_sp<skgpu::RefCntedCallback> finishedCallback)
{
    ...
    GrFlushInfo info;
    context->priv().drawingManager()->flushSurfaces({ &p, 1 }, SkSurface::BackendSurfaceAccess::kNoAccess, info, nullptr);
    return true;
}
...
GrBackendTexture GrDirectContext::createBackendTexture(int width, int height, const GrBackendFormat& backendFormat, const SkColor4f& color,
    GrMipmapped mipmapped, GrRenderable renderable, GrProtected isProtected, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext,
    std::string_view label)
{
...
GrBackendTexture GrDirectContext::createBackendTexture(int width, int height, SkColorType skColorType, const SkColor4f& color, GrMipmapped mipmapped,
...
static GrBackendTexture create_and_update_compressed_backend_texture(GrDirectContext* dContext, SkISize dimensions, const GrBackendFormat& backendFormat,
    GrMipmapped mipmapped, GrProtected isProtected, sk_sp<skgpu::RefCntedCallback> finishedCallback, const void* data, size_t size)
{
    GrGpu* gpu = dContext->priv().getGpu();
...
GrBackendTexture GrDirectContext::createCompressedBackendTexture(int width, int height, const GrBackendFormat& backendFormat, const SkColor4f& color,
    GrMipmapped mipmapped, GrProtected isProtected, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
    auto finishedCallback = skgpu::RefCntedCallback::Make(finishedProc, finishedContext);
    ...
    size_t size = SkCompressedDataSize(compression, { width, height }, nullptr, mipmapped == GrMipmapped::kYes);
...
GrBackendTexture GrDirectContext::createCompressedBackendTexture(int width, int height, SkImage::CompressionType compression, const SkColor4f& color,
    GrMipmapped mipmapped, GrProtected isProtected, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
...
GrBackendTexture GrDirectContext::createCompressedBackendTexture(int width, int height, const GrBackendFormat& backendFormat, const void* compressedData,
    size_t dataSize, GrMipmapped mipmapped, GrProtected isProtected, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
    auto finishedCallback = skgpu::RefCntedCallback::Make(finishedProc, finishedContext);
...
GrBackendTexture GrDirectContext::createCompressedBackendTexture(int width, int height, SkImage::CompressionType compression, const void* data, size_t dataSize,
    GrMipmapped mipmapped, GrProtected isProtected, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
    TRACE_EVENT0("skia.gpu", TRACE_FUNC);
    GrBackendFormat format = this->compressedBackendFormat(compression);
    ...
    const GrBackendTexture& backendTexture, const SkColor4f& color, GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext)
{
    ...
    GrFillInCompressedData(compression, backendTexture.dimensions(), backendTexture.mipmapped(), static_cast<char*>(storage.get()), color);
...
sk_sp<GrDirectContext> GrDirectContext::MakeGL(sk_sp<const GrGLInterface> glInterface, const GrContextOptions& options)
{
...
sk_sp<GrDirectContext> GrDirectContext::MakeVulkan(const GrVkBackendContext& backendContext, const GrContextOptions& options)
{
...
sk_sp<GrDirectContext> GrDirectContext::MakeMetal(const GrMtlBackendContext& backendContext, const GrContextOptions& options)
{
...
sk_sp<GrDirectContext> GrDirectContext::MakeMetal(void* device, void* queue, const GrContextOptions& options)
{
    sk_sp<GrDirectContext> direct(new GrDirectContext(GrBackendApi::kMetal, options));
...
sk_sp<GrDirectContext> GrDirectContext::MakeDawn(const wgpu::Device& device, const GrContextOptions& options)
{
