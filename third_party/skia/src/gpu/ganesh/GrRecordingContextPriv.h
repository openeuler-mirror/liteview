...
public:
    ...
    sk_sp<skgpu::v1::Device> createDevice(
        GrColorType, sk_sp<GrSurfaceProxy>, sk_sp<SkColorSpace>, GrSurfaceOrigin, const SkSurfaceProps&, skgpu::v1::Device::InitContents);
    sk_sp<skgpu::v1::Device> createDevice(SkBudgeted, const SkImageInfo&, SkBackingFit, int sampleCount, GrMipmapped, GrProtected, GrSurfaceOrigin,
        const SkSurfaceProps&, skgpu::v1::Device::InitContents);
    ...
    std::unique_ptr<skgpu::v1::SurfaceContext> makeSC(GrSurfaceProxyView readView, const GrColorInfo&);

    // Makes either a SurfaceContext, SurfaceFillContext, or a SurfaceDrawContext, depending on
    // GrRenderable and the GrImageInfo.
    std::unique_ptr<skgpu::v1::SurfaceContext> makeSC(const GrImageInfo&, const GrBackendFormat&, std::string_view label, SkBackingFit = SkBackingFit::kExact,
        GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, GrRenderable = GrRenderable::kNo, int renderTargetSampleCnt = 1, GrMipmapped = GrMipmapped::kNo,
        GrProtected = GrProtected::kNo, SkBudgeted = SkBudgeted::kYes);

    /**
     * Uses GrImageInfo's color type to pick the default texture format. Will return a
     * SurfaceDrawContext if possible.
     */
    std::unique_ptr<skgpu::v1::SurfaceFillContext> makeSFC(GrImageInfo, std::string_view label, SkBackingFit = SkBackingFit::kExact, int sampleCount = 1,
        GrMipmapped = GrMipmapped::kNo, GrProtected = GrProtected::kNo, GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, SkBudgeted = SkBudgeted::kYes);
     ...
     * if possible.
     */
    std::unique_ptr<skgpu::v1::SurfaceFillContext> makeSFC(SkAlphaType, sk_sp<SkColorSpace>, SkISize dimensions, SkBackingFit, const GrBackendFormat&,
        int sampleCount, GrMipmapped, GrProtected, skgpu::Swizzle readSwizzle, skgpu::Swizzle writeSwizzle, GrSurfaceOrigin, SkBudgeted,
     ...
     */
    std::unique_ptr<skgpu::v1::SurfaceFillContext> makeSFCWithFallback(GrImageInfo, SkBackingFit = SkBackingFit::kExact, int sampleCount = 1,
        GrMipmapped = GrMipmapped::kNo, GrProtected = GrProtected::kNo, GrSurfaceOrigin = kTopLeft_GrSurfaceOrigin, SkBudgeted = SkBudgeted::kYes);
