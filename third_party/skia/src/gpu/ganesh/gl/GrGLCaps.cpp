...
void GrGLCaps::init(const GrContextOptions& contextOptions, const GrGLContextInfo& ctxInfo, const GrGLInterface* gli)
{
    ...
    if (GR_IS_GR_GL(standard) || GR_IS_GR_GL_ES(standard)) {
        if (ctxInfo.angleBackend() == GrGLANGLEBackend::kUnknown && !ctxInfo.isOverCommandBuffer() && !fIsCoreProfile
...
void GrGLCaps::initGLSL(const GrGLContextInfo& ctxInfo, const GrGLInterface* gli)
{
    ...
    shaderCaps->fPreferFlatInterpolation = shaderCaps->fFlatInterpolationSupport && ctxInfo.vendor() != GrGLVendor::kQualcomm
    ...
    GrShaderCaps* shaderCaps, FormatWorkarounds* formatWorkarounds)
{
    ...
    if (ctxInfo.renderer() == GrGLRenderer::kAdreno430 || ctxInfo.renderer() == GrGLRenderer::kAdreno4xx_other || ctxInfo.renderer() == GrGLRenderer::kAdreno530
        || ctxInfo.renderer() == GrGLRenderer::kAdreno5xx_other || ctxInfo.driver() == GrGLDriver::kIntel || ctxInfo.isOverCommandBuffer()
    ...
    GrColorType srcColorType, const GrBackendFormat& srcBackendFormat, GrColorType dstColorType) const
{
    ...
    if (compression != SkImage::CompressionType::kNone) {
        return { SkCompressionTypeIsOpaque(compression) ? GrColorType::kRGB_888x : GrColorType::kRGBA_8888, 0 };
    }
...
skgpu::Swizzle GrGLCaps::getWriteSwizzle(const GrBackendFormat& format, GrColorType colorType) const
{
    ...
    SkDEBUGFAILF("Illegal color type (%d) and format (%d) combination.", (int)colorType, (int)format.asGLFormat());
    return {};
}
...
std::vector<GrCaps::TestFormatColorTypeCombination> GrGLCaps::getTestingCombinations() const
{
    std::vector<GrCaps::TestFormatColorTypeCombination> combos = {
        ...
        { GrColorType::kRGB_888x, GrBackendFormat::MakeGL(GR_GL_COMPRESSED_RGB8_ETC2, GR_GL_TEXTURE_2D) },
        { GrColorType::kRGB_888x, GrBackendFormat::MakeGL(GR_GL_COMPRESSED_ETC1_RGB8, GR_GL_TEXTURE_2D) },
        { GrColorType::kRGB_888x, GrBackendFormat::MakeGL(GR_GL_COMPRESSED_RGB_S3TC_DXT1_EXT, GR_GL_TEXTURE_2D) },
        { GrColorType::kRGBA_8888, GrBackendFormat::MakeGL(GR_GL_COMPRESSED_RGBA_S3TC_DXT1_EXT, GR_GL_TEXTURE_2D) },
        ...
        { GrColorType::kGrayAlpha_88, GrBackendFormat::MakeGL(GR_GL_LUMINANCE8_ALPHA8, GR_GL_TEXTURE_2D) },
    ...
    if (GR_IS_GR_GL(fStandard)) {
        combos.push_back({ GrColorType::kBGRA_8888, GrBackendFormat::MakeGL(GR_GL_RGBA8, GR_GL_TEXTURE_2D) });
        combos.push_back({ GrColorType::kBGRA_1010102, GrBackendFormat::MakeGL(GR_GL_RGB10_A2, GR_GL_TEXTURE_2D) });
    } else {
        SkASSERT(GR_IS_GR_GL_ES(fStandard) || GR_IS_GR_WEBGL(fStandard));

        combos.push_back({ GrColorType::kBGRA_8888, GrBackendFormat::MakeGL(GR_GL_BGRA8, GR_GL_TEXTURE_2D) });
    }
    if (this->rectangleTextureSupport()) {
        ...
        for (size_t i = 0; i < count2D; ++i) {
            ...
            combos.push_back({ combo2D.fColorType, GrBackendFormat::MakeGL(formatEnum, GR_GL_TEXTURE_RECTANGLE) });
        }
    }
    return combos;
}
