...
public:
    ...
    static std::unique_ptr<GrGLSemaphore> MakeWrapped(GrGLGpu* gpu, GrGLsync sync, GrWrapOwnership ownership)
    {
        auto sema = std::unique_ptr<GrGLSemaphore>(new GrGLSemaphore(gpu, kBorrow_GrWrapOwnership != ownership));
        sema->setSync(sync);
        return sema;
    }
