...
GrGLRenderTarget::GrGLRenderTarget(
    GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& ids, sk_sp<GrGLAttachment> stencil, std::string_view label)
...
GrGLRenderTarget::GrGLRenderTarget(GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& ids, std::string_view label)
    : GrSurface(gpu, dimensions, GrProtected::kNo, label)
...
sk_sp<GrGLRenderTarget> GrGLRenderTarget::MakeWrapped(
    GrGLGpu* gpu, const SkISize& dimensions, GrGLFormat format, int sampleCount, const IDs& idDesc, int stencilBits, std::string_view label)
{
    ...
    return sk_sp<GrGLRenderTarget>(new GrGLRenderTarget(gpu, dimensions, format, sampleCount, idDesc, std::move(sb), label));
}
...
GrBackendRenderTarget GrGLRenderTarget::getBackendRenderTarget() const
{
    ...
    return GrBackendRenderTarget(this->width(), this->height(), this->numSamples(), numStencilBits, fbi);
}
...
{
    return GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), fTotalMemorySamplesPerPixel, GrMipmapped::kNo);
}
...
void GrGLRenderTarget::dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const
{
    ...
    if (numSamplesNotInTexture >= 1) {
        size_t size = GrSurface::ComputeSize(this->backendFormat(), this->dimensions(), numSamplesNotInTexture, GrMipmapped::kNo);
