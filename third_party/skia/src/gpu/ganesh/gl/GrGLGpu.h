...
private:
    ...
    sk_sp<GrTexture> onCreateTexture(SkISize dimensions, const GrBackendFormat&, GrRenderable, int renderTargetSampleCnt, SkBudgeted, GrProtected,
    ...
    GrBackendFormat getPreferredStencilFormat(const GrBackendFormat& format) override
    {
        ...
        return GrBackendFormat::MakeGL(GrGLFormatToEnum(this->glCaps().stencilFormats()[idx]), GR_GL_TEXTURE_NONE);
    }
    ...
    GrGLuint createCompressedTexture2D(
        SkISize dimensions, SkImage::CompressionType compression, GrGLFormat, GrMipmapped, GrGLTextureParameters::SamplerOverriddenState*);
    ...
    bool uploadCompressedTexData(
        SkImage::CompressionType compressionType, GrGLFormat, SkISize dimensions, GrMipmapped, GrGLenum target, const void* data, size_t dataSize);
