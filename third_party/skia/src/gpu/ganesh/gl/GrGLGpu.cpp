...
sk_sp<GrGpu> GrGLGpu::Make(sk_sp<const GrGLInterface> interface, const GrContextOptions& options, GrDirectContext* direct)
{
    if (!interface) {
...
sk_sp<GrTexture> GrGLGpu::onWrapBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable, GrIOType ioType)
{
    ...
    auto texture = GrGLTexture::MakeWrapped(this, mipmapStatus, desc, backendTex.getGLTextureParams(), cacheable, ioType, backendTex.getLabel());
...
sk_sp<GrTexture> GrGLGpu::onWrapCompressedBackendTexture(const GrBackendTexture& backendTex, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    auto texture = GrGLTexture::MakeWrapped(this, mipmapStatus, desc, backendTex.getGLTextureParams(), cacheable, kRead_GrIOType, backendTex.getLabel());
    return std::move(texture);
}
    ...
    const GrBackendTexture& backendTex, int sampleCnt, GrWrapOwnership ownership, GrWrapCacheable cacheable)
{
    ...
    sk_sp<GrGLTextureRenderTarget> texRT(
        GrGLTextureRenderTarget::MakeWrapped(this, sampleCnt, desc, backendTex.getGLTextureParams(), rtIDs, cacheable, mipmapStatus, backendTex.getLabel()));
...
bool GrGLGpu::uploadColorToTex(GrGLFormat textureFormat, SkISize texDims, GrGLenum target, std::array<float, 4> color, uint32_t levelMask)
{
    ...
    this->uploadTexData(texDims, target, SkIRect::MakeSize(texDims), externalFormat, externalType, bpp, levels.begin(), levels.count());
    return true;
}
...
bool GrGLGpu::uploadCompressedTexData(
    SkImage::CompressionType compressionType, GrGLFormat format, SkISize dimensions, GrMipmapped mipmapped, GrGLenum target, const void* data, size_t dataSize)
{
    ...
    if (useTexStorage) {
        ...
        for (int level = 0; level < numMipLevels; ++level) {
            ...
            error = GL_ALLOC_CALL(CompressedTexSubImage2D(target, level,
                0, // left
                0, // top
                dimensions.width(), dimensions.height(), internalFormat, SkToInt(levelDataSize), &((char*)data)[offset]));
    ...
    SkBudgeted budgeted, GrProtected isProtected, int mipLevelCount, uint32_t levelClearMask, std::string_view label)
{
    ...
    texDesc.fID = this->createTexture(dimensions, texDesc.fFormat, texDesc.fTarget, renderable, &initialState, mipLevelCount, isProtected, label);
    ...
    if (levelClearMask) {
        ...
        } else if (this->glCaps().canFormatBeFBOColorAttachment(format.asGLFormat()) && !this->glCaps().performColorClearsAsDraws()) {
    ...
    SkISize dimensions, const GrBackendFormat& format, SkBudgeted budgeted, GrMipmapped mipmapped, GrProtected isProtected, const void* data, size_t dataSize)
{
    ...
    desc.fID = this->createCompressedTexture2D(desc.fSize, compression, desc.fFormat, mipmapped, &initialState);
    if (!desc.fID) {
        return nullptr;
    }
...
GrBackendTexture GrGLGpu::onCreateCompressedBackendTexture(SkISize dimensions, const GrBackendFormat& format, GrMipmapped mipmapped, GrProtected isProtected)
{
    // We don't support protected textures in GL.
    if (isProtected == GrProtected::kYes) {
        return {};
    ...
    info.fID = this->createCompressedTexture2D(dimensions, compression, glFormat, mipmapped, &initialState);
    if (!info.fID) {
        return {};
    }
    ...
    return GrBackendTexture(dimensions.width(), dimensions.height(), mipmapped, info, std::move(parameters));
}
...
int GrGLGpu::getCompatibleStencilIndex(GrGLFormat format)
{
    ...
    if (!this->glCaps().hasStencilFormatBeenDeterminedForFormat(format)) {
        ...
        GrGLuint colorID = this->createTexture({ kSize, kSize }, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, nullptr, 1, GrProtected::kNo,
            /*label=*/"Skia");
        if (!colorID) {
            return -1;
        }
...
GrGLuint GrGLGpu::createCompressedTexture2D(SkISize dimensions, SkImage::CompressionType compression, GrGLFormat format, GrMipmapped mipmapped,
    GrGLTextureParameters::SamplerOverriddenState* initialState)
{
    ...
    if (GrGLCaps::kNone_InvalidateFBType != this->glCaps().invalidateFBType()) {
        ...
        if (!discardAttachments.empty()) {
            if (GrGLCaps::kInvalidate_InvalidateFBType == this->glCaps().invalidateFBType()) {
                GL_CALL(InvalidateFramebuffer(GR_GL_FRAMEBUFFER, discardAttachments.count(), discardAttachments.begin()));
            } else {
                SkASSERT(GrGLCaps::kDiscard_InvalidateFBType == this->glCaps().invalidateFBType());
                GL_CALL(DiscardFramebuffer(GR_GL_FRAMEBUFFER, discardAttachments.count(), discardAttachments.begin()));
            }
        }
    }
    ...
    GrSurface* surface, SkIRect rect, GrColorType surfaceColorType, GrColorType dstColorType, void* offsetOrPtr, int rowWidthInPixels)
{
    ...
    GrGLenum externalFormat = 0;
    GrGLenum externalType = 0;
    this->glCaps().getReadPixelsFormat(surface->backendFormat().asGLFormat(), surfaceColorType, dstColorType, &externalFormat, &externalType);
    if (!externalFormat || !externalType) {
        return false;
    }
...
bool GrGLGpu::createCopyProgram(GrTexture* srcTex)
{
    ...
    std::string glsl;
    std::unique_ptr<SkSL::Program> program = GrSkSLtoGLSL(this, SkSL::ProgramKind::kVertex, sksl, settings, &glsl, errorHandler);
    GrGLuint vshader
        = GrGLCompileAndAttachShader(*fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl, fProgramCache->stats(), errorHandler);
    ...
    GrGLuint fshader
        = GrGLCompileAndAttachShader(*fGLContext, fCopyPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl, fProgramCache->stats(), errorHandler);
    SkASSERT(program->fInputs == SkSL::Program::Inputs());
...
bool GrGLGpu::createMipmapProgram(int progIdx)
{
    ...
    GrShaderVar aVertex("a_vertex", SkSLType::kHalf2, GrShaderVar::TypeModifier::In);
    GrShaderVar uTexCoordXform("u_texCoordXform", SkSLType::kHalf4, GrShaderVar::TypeModifier::Uniform);
    GrShaderVar uTexture("u_texture", SkSLType::kTexture2DSampler, GrShaderVar::TypeModifier::Uniform);
    ...
    std::unique_ptr<SkSL::Program> program = GrSkSLtoGLSL(this, SkSL::ProgramKind::kVertex, sksl, settings, &glsl, errorHandler);
    GrGLuint vshader
        = GrGLCompileAndAttachShader(*fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_VERTEX_SHADER, glsl, fProgramCache->stats(), errorHandler);
    SkASSERT(program->fInputs == SkSL::Program::Inputs());

    sksl.assign(fshaderTxt.c_str(), fshaderTxt.size());
    program = GrSkSLtoGLSL(this, SkSL::ProgramKind::kFragment, sksl, settings, &glsl, errorHandler);
    GrGLuint fshader
        = GrGLCompileAndAttachShader(*fGLContext, fMipmapPrograms[progIdx].fProgram, GR_GL_FRAGMENT_SHADER, glsl, fProgramCache->stats(), errorHandler);
    SkASSERT(program->fInputs == SkSL::Program::Inputs());
    ...
    SkISize dimensions, const GrBackendFormat& format, GrRenderable renderable, GrMipmapped mipmapped, GrProtected isProtected, std::string_view label)
{
    ...
    info.fFormat = GrGLFormatToEnum(glFormat);
    info.fID = this->createTexture(dimensions, glFormat, info.fTarget, renderable, &initialState, numMipLevels, isProtected, label);
    if (!info.fID) {
        return {};
    }
    ...
    return GrBackendTexture(dimensions.width(), dimensions.height(), mipmapped, info, std::move(parameters), label);
}
...
GrBackendRenderTarget GrGLGpu::createTestingOnlyBackendRenderTarget(SkISize dimensions, GrColorType colorType, int sampleCnt, GrProtected isProtected)
{
    ...
    if (useTexture) {
        GrGLTextureParameters::SamplerOverriddenState initialState;
        colorID = this->createTexture(dimensions, format, GR_GL_TEXTURE_2D, GrRenderable::kYes, &initialState, 1, isProtected,
            /*label=*/"Skia");
        if (!colorID) {
            deleteIDs();
            return {};
    ...
    GrBackendRenderTarget beRT = GrBackendRenderTarget(dimensions.width(), dimensions.height(), sampleCnt, stencilBits, info);
...
std::unique_ptr<GrSemaphore> GrGLGpu::wrapBackendSemaphore(const GrBackendSemaphore& semaphore, GrSemaphoreWrapType /* wrapType */, GrWrapOwnership ownership)
{
    SkASSERT(this->caps()->semaphoreSupport());
