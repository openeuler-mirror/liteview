...
GrGLTexture::GrGLTexture(GrGLGpu* gpu, SkBudgeted budgeted, const Desc& desc, GrMipmapStatus mipmapStatus, std::string_view label)
    : GrSurface(gpu, desc.fSize, GrProtected::kNo, label)
    , INHERITED(gpu, desc.fSize, GrProtected::kNo, TextureTypeFromTarget(desc.fTarget), mipmapStatus, label)
...
{
    return GrBackendFormat::MakeGL(GrGLFormatToEnum(fFormat), target_from_texture_type(this->textureType()));
}
...
bool GrGLTexture::onStealBackendTexture(GrBackendTexture* backendTexture, SkImage::BackendTextureReleaseProc* releaseProc)
{
