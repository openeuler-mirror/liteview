...
bool init_vertices_paint(GrRecordingContext* rContext, const GrColorInfo& colorInfo, const SkPaint& skPaint, const SkMatrixProvider& matrixProvider,
    sk_sp<SkBlender> blender, bool hasColors, const SkSurfaceProps& props, GrPaint* grPaint)
{
    if (hasColors) {
        return SkPaintToGrPaintWithBlend(rContext, colorInfo, skPaint, matrixProvider, blender.get(), props, grPaint);
    } else {
...
sk_sp<Device> Device::Make(GrRecordingContext* rContext, SkBudgeted budgeted, const SkImageInfo& ii, SkBackingFit fit, int sampleCount, GrMipmapped mipmapped,
    GrProtected isProtected, GrSurfaceOrigin origin, const SkSurfaceProps& props, InitContents init)
{
    if (!rContext) {
        return nullptr;
    }
...
Device::Device(std::unique_ptr<SurfaceDrawContext> sdc, DeviceFlags flags)
    ...
    , fSDFTControl(sdc->recordingContext()->priv().getSDFTControl(sdc->surfaceProps().isUseDeviceIndependentFonts()))
    , fSurfaceDrawContext(std::move(sdc))
    , fClip(SkIRect::MakeSize(fSurfaceDrawContext->dimensions()), &this->asMatrixProvider(), force_aa_clip(fSurfaceDrawContext.get()))
{
...
void Device::drawPaint(const SkPaint& paint)
{
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(
            this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(), fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
...
void Device::drawPoints(SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
{
    ...
    if (count == 2 && mode == SkCanvas::kLines_PointMode) {
        if (paint.getPathEffect()) {
            ...
            GrPaint grPaint;
            if (SkPaintToGrPaint(this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(),
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaint(this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, *matrixProvider, fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
    ...
    fSurfaceDrawContext->drawVertices(this->clip(), std::move(grPaint), *matrixProvider, std::move(vertices), &primitiveType);
}
...
void Device::drawRect(const SkRect& rect, const SkPaint& paint)
{
    ...
    if (paint.getMaskFilter() || paint.getPathEffect()) {
        ...
        GrBlurUtils::drawShapeWithMaskFilter(fContext.get(), fSurfaceDrawContext.get(), this->clip(), paint, this->asMatrixProvider(), shape);
        return;
    }
...
void Device::drawArc(const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, const SkPaint& paint)
{
    ASSERT_SINGLE_OWNER
    ...
    fSurfaceDrawContext->drawArc(
        this->clip(), std::move(grPaint), fSurfaceDrawContext->chooseAA(paint), this->localToDevice(), oval, startAngle, sweepAngle, useCenter, GrStyle(paint));
}
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkBitmap& bitmap)
{
    ...
    // the special image
    return SkSpecialImage::MakeDeferredFromGpu(fContext.get(), rect, bitmap.getGenerationID(), std::move(view),
...
sk_sp<SkSpecialImage> Device::makeSpecial(const SkImage* image)
{
    ...
    if (image->isTextureBacked()) {
        ...
        return SkSpecialImage::MakeDeferredFromGpu(fContext.get(), SkIRect::MakeWH(image->width(), image->height()), image->uniqueID(), std::move(view),
...
sk_sp<SkSpecialImage> Device::snapSpecial(const SkIRect& subset, bool forceCopy)
{
    ...
    if (forceCopy || !view.asTextureProxy()) {
        ...
        view = GrSurfaceProxyView::Copy(fContext.get(), std::move(view),
            ...
            subset, SkBackingFit::kApprox, SkBudgeted::kYes,
    ...
    GrSurfaceProxyView view, const GrColorInfo& info, std::unique_ptr<SkLatticeIter> iter, const SkRect& dst, SkFilterMode filter, const SkPaint& origPaint)
{
    ...
    if (!SkPaintToGrPaintReplaceShader(this->recordingContext(), fSurfaceDrawContext->colorInfo(), *paint, this->asMatrixProvider(),
...
void Device::drawVertices(const SkVertices* vertices, sk_sp<SkBlender> blender, const SkPaint& paint, bool skipColorXform)
{
    ...
    GrPaint grPaint;
    if (!init_vertices_paint(fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(), std::move(blender), info.hasColors(),
            fSurfaceDrawContext->surfaceProps(), &grPaint)) {
        return;
    }
    fSurfaceDrawContext->drawVertices(
        this->clip(), std::move(grPaint), this->asMatrixProvider(), sk_ref_sp(const_cast<SkVertices*>(vertices)), nullptr, skipColorXform);
}
...
void Device::drawMesh(const SkMesh& mesh, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ...
    GrPaint grPaint;
    if (!init_vertices_paint(fContext.get(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(), std::move(blender),
...
void Device::drawAtlas(const SkRSXform xform[], const SkRect texRect[], const SkColor colors[], int count, sk_sp<SkBlender> blender, const SkPaint& paint)
{
    ...
    GrPaint grPaint;
    if (colors) {
        if (!SkPaintToGrPaintWithBlend(this->recordingContext(), fSurfaceDrawContext->colorInfo(), paint, this->asMatrixProvider(), blender.get(),
                fSurfaceDrawContext->surfaceProps(), &grPaint)) {
            return;
        }
    } else {
...
void Device::testingOnly_drawGlyphRunListWithSlug(
    SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    ...
    SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    ...
    auto srcSlug = Slug::ConvertBlob(canvas, *glyphRunList.blob(), glyphRunList.origin(), initialPaint);
...
public:
    ...
    void notifyCacheMiss(SkStrikeClient::CacheMissType type, int fontSize) override SK_EXCLUDES(fLock)
    {
        SkAutoMutexExclusive l(fLock);
    ...
    SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    ...
    auto analysisCanvas
        = server.makeAnalysisCanvas(canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().width(), surfaceProps, std::move(colorSpace), useDFT);

    analysisCanvas->setMatrix(canvas->getTotalMatrix());
    auto srcSlug = Slug::ConvertBlob(analysisCanvas.get(), *glyphRunList.blob(), glyphRunList.origin(), initialPaint);
...
void Device::onDrawGlyphRunList(SkCanvas* canvas, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    ...
    this->testingOnly_drawGlyphRunListWithSerializedSlug(canvas, glyphRunList, initialPaint, drawingPaint);
#elif defined(SK_EXPERIMENTAL_SIMULATE_DRAWGLYPHRUNLIST_WITH_SLUG_STRIKE_SERIALIZE)
    this->testingOnly_drawGlyphRunListWithSerializedSlugAndStrike(canvas, glyphRunList, initialPaint, drawingPaint);
#else
    if (glyphRunList.blob() == nullptr) {
        ...
        fSurfaceDrawContext->drawGlyphRunList(canvas, this->clip(), this->asMatrixProvider(), glyphRunList, this->strikeDeviceInfo(), drawingPaint);
    }
#endif
}
...
void Device::asyncReadPixels(const SkImageInfo& info, const SkIRect& srcRect, ReadPixelsCallback callback, ReadPixelsContext context)
{
    auto* sdc = fSurfaceDrawContext.get();
    // Context TODO: Elevate direct context requirement to public API.
...
void Device::asyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace, const SkIRect& srcRect, SkISize dstSize,
    RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    ...
    sdc->asyncRescaleAndReadPixelsYUV420(dContext, yuvColorSpace, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
SkBaseDevice* Device::onCreateDevice(const CreateInfo& cinfo, const SkPaint*)
{
    ...
    SkBackingFit fit = kNever_TileUsage == cinfo.fTileUsage ? SkBackingFit::kApprox : SkBackingFit::kExact;

    SkASSERT(cinfo.fInfo.colorType() != kRGBA_1010102_SkColorType);

    auto sdc = SurfaceDrawContext::MakeWithFallback(fContext.get(), SkColorTypeToGrColorType(cinfo.fInfo.colorType()),
        fSurfaceDrawContext->colorInfo().refColorSpace(), fit, cinfo.fInfo.dimensions(), props, fSurfaceDrawContext->numSamples(), GrMipmapped::kNo,
        fSurfaceDrawContext->asSurfaceProxy()->isProtected(), fSurfaceDrawContext->origin(), SkBudgeted::kYes);
    if (!sdc) {
        return nullptr;
    }
...
sk_sp<SkSurface> Device::makeSurface(const SkImageInfo& info, const SkSurfaceProps& props)
{
    ...
    return SkSurface::MakeRenderTarget(fContext.get(), kBudgeted, info, fSurfaceDrawContext->numSamples(), fSurfaceDrawContext->origin(), &props);
}
