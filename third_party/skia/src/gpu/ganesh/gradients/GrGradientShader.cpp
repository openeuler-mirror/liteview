    ...
    const SkPMColor4f* colors, const SkScalar* positions, int count, bool premul, const GrFPArgs& args)
{
    ...
    auto view = std::get<0>(GrMakeCachedBitmapProxyView(args.fContext, bitmap, /*label=*/"MakeTexturedColorizer", GrMipmapped::kNo));
    if (!view) {
...
// the gradient.
static std::unique_ptr<GrFragmentProcessor> make_colorizer(const SkPMColor4f* colors, const SkScalar* positions, int count, bool premul, const GrFPArgs& args)
{
...
static std::unique_ptr<GrFragmentProcessor> make_clamped_gradient(std::unique_ptr<GrFragmentProcessor> colorizer,
    std::unique_ptr<GrFragmentProcessor> gradLayout, SkPMColor4f leftBorderColor, SkPMColor4f rightBorderColor, bool makePremul, bool colorsAreOpaque)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
    ...
    return GrSkSLFP::Make(effect, "ClampedGradient", /*inputFP=*/nullptr, optFlags, "colorizer", GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
        GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "leftBorderColor", leftBorderColor, "rightBorderColor", rightBorderColor, "makePremul",
        GrSkSLFP::Specialize<int>(makePremul), "layoutPreservesOpacity", GrSkSLFP::Specialize<int>(layoutPreservesOpacity));
}

static std::unique_ptr<GrFragmentProcessor> make_tiled_gradient(const GrFPArgs& args, std::unique_ptr<GrFragmentProcessor> colorizer,
    std::unique_ptr<GrFragmentProcessor> gradLayout, bool mirror, bool makePremul, bool colorsAreOpaque)
{
    static const SkRuntimeEffect* effect = SkMakeRuntimeEffect(SkRuntimeEffect::MakeForShader,
        "uniform shader colorizer;"
        "uniform shader gradLayout;"
    ...
    return GrSkSLFP::Make(effect, "TiledGradient", /*inputFP=*/nullptr, optFlags, "colorizer", GrSkSLFP::IgnoreOptFlags(std::move(colorizer)), "gradLayout",
        GrSkSLFP::IgnoreOptFlags(std::move(gradLayout)), "mirror", GrSkSLFP::Specialize<int>(mirror), "makePremul", GrSkSLFP::Specialize<int>(makePremul),
        "layoutPreservesOpacity", GrSkSLFP::Specialize<int>(layoutPreservesOpacity), "useFloorAbsWorkaround", GrSkSLFP::Specialize<int>(useFloorAbsWorkaround));
}
...
std::unique_ptr<GrFragmentProcessor> MakeGradientFP(
    const SkGradientShaderBase& shader, const GrFPArgs& args, std::unique_ptr<GrFragmentProcessor> layout, const SkMatrix* overrideMatrix)
{
    ...
    SkColor4fXformer xformedColors(shader.fOrigColors4f, shader.fColorCount, shader.fColorSpace.get(), args.fDstColorInfo->colorSpace());
    for (int i = 0; i < shader.fColorCount; i++) {
        const SkColor4f& upmColor = xformedColors.fColors[i];
        colors[i] = inputPremul ? upmColor.premul() : SkPMColor4f { upmColor.fR, upmColor.fG, upmColor.fB, upmColor.fA };
    ...
    std::unique_ptr<GrFragmentProcessor> colorizer = make_colorizer(colors.get(), positions, shader.fColorCount, inputPremul, args);
    ...
    case SkTileMode::kClamp:
        ...
        // border.
        gradient = make_clamped_gradient(std::move(colorizer), std::move(layout), colors[0], colors[shader.fColorCount - 1], makePremul, allOpaque);
        break;
    case SkTileMode::kDecal:
        ...
        gradient = make_clamped_gradient(
            std::move(colorizer), std::move(layout), SK_PMColor4fTRANSPARENT, SK_PMColor4fTRANSPARENT, makePremul, /* colorsAreOpaque */ false);
        break;
    }
