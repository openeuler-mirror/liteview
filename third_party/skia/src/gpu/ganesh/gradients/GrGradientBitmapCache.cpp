...
void GrGradientBitmapCache::fillGradient(const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType, SkBitmap* bitmap)
{
    ...
    pixelWriteFn_t write8888Pixel = [&](const skvx::float4& c, int index) { pixels32[index] = Sk4f_toL32(c); };
...
void GrGradientBitmapCache::getGradient(
    const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType, SkAlphaType alphaType, SkBitmap* bitmap)
{
