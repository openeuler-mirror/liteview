...
bool GrDrawingManager::flush(
    SkSpan<GrSurfaceProxy*> proxies, SkSurface::BackendSurfaceAccess access, const GrFlushInfo& info, const skgpu::MutableTextureState* newState)
{
    ...
    if (!proxies.empty() && !info.fNumSemaphores && !info.fFinishedProc && access == SkSurface::BackendSurfaceAccess::kNoAccess && !newState) {
...
template <typename T> static void reorder_array_by_llist(const SkTInternalLList<T>& llist, SkTArray<sk_sp<T>>* array)
{
    int i = 0;
    for (T* t : llist) {
...
void GrDrawingManager::createDDLTask(sk_sp<const SkDeferredDisplayList> ddl, sk_sp<GrRenderTargetProxy> newDest, SkIPoint offset)
{
    SkDEBUGCODE(this->validate());
    ...
    SkDEBUGCODE(auto ddlTask =) this->appendTask(sk_make_sp<GrDDLTask>(this, std::move(newDest), std::move(ddl), offset));
...
sk_sp<skgpu::v1::OpsTask> GrDrawingManager::newOpsTask(GrSurfaceProxyView surfaceView, sk_sp<GrArenas> arenas)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
    ...
    sk_sp<skgpu::v1::OpsTask> opsTask(new skgpu::v1::OpsTask(this, std::move(surfaceView), fContext->priv().auditTrail(), std::move(arenas)));
...
void GrDrawingManager::newWaitRenderTask(sk_sp<GrSurfaceProxy> proxy, std::unique_ptr<std::unique_ptr<GrSemaphore>[]> semaphores, int numSemaphores)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
...
void GrDrawingManager::newTransferFromRenderTask(sk_sp<GrSurfaceProxy> srcProxy, const SkIRect& srcRect, GrColorType surfaceColorType, GrColorType dstColorType,
...
sk_sp<GrRenderTask> GrDrawingManager::newCopyRenderTask(
    sk_sp<GrSurfaceProxy> dst, SkIRect dstRect, sk_sp<GrSurfaceProxy> src, SkIRect srcRect, GrSamplerState::Filter filter, GrSurfaceOrigin origin)
{
    SkDEBUGCODE(this->validate());
    SkASSERT(fContext);
    ...
    sk_sp<GrSurfaceProxy> dst, SkIRect rect, GrColorType srcColorType, GrColorType dstColorType, const GrMipLevel levels[], int levelCount)
{
    ...
    if (!caps.preferVRAMUseOverFlushes()) {
        this->flushSurfaces(SkSpan<GrSurfaceProxy*> {}, SkSurface::BackendSurfaceAccess::kNoAccess, GrFlushInfo {}, nullptr);
    }
...
skgpu::v1::PathRenderer* GrDrawingManager::getSoftwarePathRenderer()
{
    if (!fSoftwarePathRenderer) {
        fSoftwarePathRenderer.reset(new skgpu::v1::SoftwarePathRenderer(fContext->priv().proxyProvider(), fOptionsForPathRendererChain.fAllowPathMaskCaching));
    }
