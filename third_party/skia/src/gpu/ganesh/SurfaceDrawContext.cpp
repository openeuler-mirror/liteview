...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(GrRecordingContext* rContext, sk_sp<SkColorSpace> colorSpace, SkBackingFit fit, SkISize dimensions,
    const GrBackendFormat& format, int sampleCnt, GrMipmapped mipmapped, GrProtected isProtected, skgpu::Swizzle readSwizzle, skgpu::Swizzle writeSwizzle,
    GrSurfaceOrigin origin, SkBudgeted budgeted, const SkSurfaceProps& surfaceProps, std::string_view label)
{
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::Make(GrRecordingContext* rContext, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, SkISize dimensions, const SkSurfaceProps& surfaceProps, std::string_view label, int sampleCnt, GrMipmapped mipmapped,
...
std::unique_ptr<SurfaceDrawContext> SurfaceDrawContext::MakeWithFallback(GrRecordingContext* rContext, GrColorType colorType, sk_sp<SkColorSpace> colorSpace,
    SkBackingFit fit, SkISize dimensions, const SkSurfaceProps& surfaceProps, int sampleCnt, GrMipmapped mipmapped, GrProtected isProtected,
...
void SurfaceDrawContext::drawGlyphRunList(SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, const sktext::GlyphRunList& glyphRunList,
    SkStrikeDeviceInfo strikeDeviceInfo, const SkPaint& paint)
{
    ...
    textBlobCache->drawGlyphRunList(canvas, clip, viewMatrix, glyphRunList, paint, strikeDeviceInfo, this);
}
...
void SurfaceDrawContext::drawVertices(const GrClip* clip, GrPaint&& paint, const SkMatrixProvider& matrixProvider, sk_sp<SkVertices> vertices,
    GrPrimitiveType* overridePrimType, bool skipColorXform)
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    ...
    GrOp::Owner op = DrawMeshOp::Make(fContext, std::move(paint), std::move(vertices), overridePrimType, matrixProvider, aaType, std::move(xform));
    this->addDrawOp(clip, std::move(op));
}

void SurfaceDrawContext::drawMesh(const GrClip* clip, GrPaint&& paint, const SkMatrixProvider& matrixProvider, const SkMesh& mesh)
{
    ASSERT_SINGLE_OWNER
    RETURN_IF_ABANDONED
    ...
    GrAAType aaType = fCanUseDynamicMSAA ? GrAAType::kMSAA : this->chooseAAType(GrAA::kNo);
    GrOp::Owner op = DrawMeshOp::Make(fContext, std::move(paint), mesh, matrixProvider, aaType, std::move(xform));
    this->addDrawOp(clip, std::move(op));
}
...
void SurfaceDrawContext::drawArc(const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, const SkRect& oval, SkScalar startAngle,
    SkScalar sweepAngle, bool useCenter, const GrStyle& style)
{
    ...
    if (aaType == GrAAType::kCoverage) {
        ...
        GrOp::Owner op = GrOvalOpFactory::MakeArcOp(fContext, std::move(paint), viewMatrix, oval, startAngle, sweepAngle, useCenter, style, shaderCaps);
        if (op) {
            this->addDrawOp(clip, std::move(op));
            return;
        }
...
#endif
    this->drawShapeUsingPathRenderer(
        clip, std::move(paint), aa, viewMatrix, GrStyledShape::MakeArc(oval, startAngle, sweepAngle, useCenter, style, DoSimplify::kNo));
}
    ...
    const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix, GrStyledShape&& shape, bool attemptDrawSimple)
{
    ...
    if (!pr) {
        if (shape.style().applies()) {
            ...
            pr = this->drawingManager()->getPathRenderer(canDrawArgs, kAllowSWPathRenderer, DrawType::kColor);
        } else {
...
bool SurfaceDrawContext::setupDstProxyView(const SkRect& opBounds, bool opRequiresMSAA, GrDstProxyView* dstProxyView)
{
    ...
    auto copy = GrSurfaceProxy::Copy(fContext, this->asSurfaceProxyRef(), this->origin(), GrMipmapped::kNo, copyRect, fit, SkBudgeted::kYes,
