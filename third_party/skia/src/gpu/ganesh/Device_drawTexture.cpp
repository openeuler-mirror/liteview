...
SkIRect determine_clipped_src_rect(int width, int height, const GrClip* clip, const SkMatrix& viewMatrix, const SkMatrix& srcToDstRect,
    const SkISize& imageDimensions, const SkRect* srcRectPtr)
{
...
bool should_tile_image_id(GrRecordingContext* context, SkISize rtSize, const GrClip* clip, uint32_t imageID, const SkISize& imageSize, const SkMatrix& ctm,
    const SkMatrix& srcToDst, const SkRect* src, int maxTileSize, int* tileSize, SkIRect* clippedSubset)
{
    // if it's larger than the max tile size, then we have no choice but tiling.
    if (imageSize.width() > maxTileSize || imageSize.height() > maxTileSize) {
        *clippedSubset = determine_clipped_src_rect(rtSize.width(), rtSize.height(), clip, ctm, srcToDst, imageSize, src);
...
inline void clamped_outset_with_offset(SkIRect* iRect, int outset, SkPoint* offset, const SkIRect& clamp)
{
 ...
 */
ImageDrawMode optimize_sample_area(const SkISize& image, const SkRect* origSrcRect, const SkRect* origDstRect, const SkPoint dstClip[4], SkRect* outSrcRect,
    SkRect* outDstRect, SkMatrix* srcToDst)
{
...
void draw_texture(skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkMatrix& ctm, const SkPaint& paint, GrSamplerState::Filter filter,
    ...
    GrSurfaceProxyView view, const GrColorInfo& srcColorInfo)
{
    ...
    if (constraint != SkCanvas::kStrict_SrcRectConstraint && !proxy->isFunctionallyExact()) {
        ...
        float buffer = 0.5f * (aaFlags != GrQuadAAFlags::kNone) + 0.5f * (filter == GrSamplerState::Filter::kLinear);
...
// Assumes srcRect and dstRect have already been optimized to fit the proxy.
void draw_image(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkMatrixProvider& matrixProvider,
    const SkPaint& paint, const SkImage_Base& image, const SkRect& src, const SkRect& dst, const SkPoint dstClip[4], const SkMatrix& srcToDst,
    GrQuadAAFlags aaFlags, SkCanvas::SrcRectConstraint constraint, SkSamplingOptions sampling, SkTileMode tm = SkTileMode::kClamp)
{
    ...
    if (tm == SkTileMode::kClamp && !image.isYUVA() && can_use_draw_texture(paint, sampling)) {
        ...
        draw_texture(sdc, clip, ctm, paint, sampling.filter, src, dst, dstClip, aaFlags, constraint, std::move(view), info);
        return;
    }
    ...
    if (sampling.isAniso() && !sampling.useCubic && sampling.filter == SkFilterMode::kLinear && restrictToSubset && sampling.mipmap == SkMipmapMode::kNone
        && coordsAllInsideSrcRect && !image.isYUVA()) {
    ...
    std::unique_ptr<GrFragmentProcessor> fp = image.asFragmentProcessor(rContext, sampling, tileModes, textureMatrix, subset, domain);
    fp = GrColorSpaceXformEffect::Make(std::move(fp), image.imageInfo().colorInfo(), sdc->colorInfo());
    if (image.isAlphaOnly()) {
        if (const auto* shader = as_SB(paint.getShader())) {
            auto shaderFP = shader->asFragmentProcessor(GrFPArgs(rContext, matrixProvider, &sdc->colorInfo(), sdc->surfaceProps()));
            if (!shaderFP) {
                return;
            }
    ...
    GrPaint grPaint;
    if (!SkPaintToGrPaintReplaceShader(rContext, sdc->colorInfo(), paint, matrixProvider, std::move(fp), sdc->surfaceProps(), &grPaint)) {
        return;
    }
    ...
    if (!mf) {
        ...
        if (dstClip) {
            ...
            sdc->fillRectWithEdgeAA(clip, std::move(grPaint), aaFlags, ctm, dst, canUseTextureCoordsAsLocalCoords ? &src : nullptr);
        }
    } else {
...
void draw_tiled_bitmap(GrRecordingContext* rContext, skgpu::v1::SurfaceDrawContext* sdc, const GrClip* clip, const SkBitmap& bitmap, int tileSize,
    const SkMatrixProvider& matrixProvider, const SkMatrix& srcToDst, const SkRect& srcRect, const SkIRect& clippedSrcIRect, const SkPaint& paint,
    GrQuadAAFlags origAAFlags, SkCanvas::SrcRectConstraint constraint, SkSamplingOptions sampling, SkTileMode tileMode)
{
    if (sampling.isAniso()) {
    ...
    for (int x = 0; x <= nx; x++) {
        for (int y = 0; y <= ny; y++) {
            ...
            if (SkBitmap subsetBmp; bitmap.extractSubset(&subsetBmp, iTileR)) {
                ...
                SkASSERT(image->width() <= rContext->priv().caps()->maxTextureSize() && image->height() <= rContext->priv().caps()->maxTextureSize());
                ...
                draw_image(rContext, sdc, clip, matrixProvider, paint, *as_IB(image.get()), tileR, rectToDraw, nullptr, offsetSrcToDst, aaFlags, constraint,
                    sampling, tileMode);
            }
        }
    }
...
void Device::drawSpecial(SkSpecialImage* special, const SkMatrix& localToDevice, const SkSamplingOptions& origSampling, const SkPaint& paint)
{
    ...
    SkImage_Gpu image(sk_ref_sp(special->getContext()), special->uniqueID(), std::move(view), special->colorInfo());
    ...
    draw_image(fContext.get(), fSurfaceDrawContext.get(), this->clip(), matrixProvider, paint, image, src, dst, nullptr, srcToDst, aaFlags,
        SkCanvas::kStrict_SrcRectConstraint, sampling);
}

void Device::drawImageQuad(const SkImage* image, const SkRect* srcRect, const SkRect* dstRect, const SkPoint dstClip[4], GrQuadAAFlags aaFlags,
    const SkMatrix* preViewMatrix, const SkSamplingOptions& origSampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    SkRect dst;
    SkMatrix srcToDst;
    ImageDrawMode mode = optimize_sample_area(SkISize::Make(image->width(), image->height()), srcRect, dstRect, dstClip, &src, &dst, &srcToDst);
    ...
    SkPreConcatMatrixProvider matrixProvider(this->asMatrixProvider(), preViewMatrix ? *preViewMatrix : SkMatrix::I());
    const SkMatrix& ctm(matrixProvider.localToDevice());
    ...
    if (!image->isTextureBacked() && !as_IB(image)->isPinnedOnContext(fContext.get())) {
        ...
        if (should_tile_image_id(fContext.get(), fSurfaceDrawContext->dimensions(), clip, image->unique(), image->dimensions(), ctm, srcToDst, &src,
            ...
            if (SkBitmap bm; as_IB(image)->getROPixels(nullptr, &bm)) {
                ...
                draw_tiled_bitmap(fContext.get(), fSurfaceDrawContext.get(), clip, bm, tileSize, matrixProvider, srcToDst, src, clippedSubset, paint, aaFlags,
                    constraint, sampling, tileMode);
                return;
            }
        }
    }
    ...
    draw_image(
        fContext.get(), fSurfaceDrawContext.get(), clip, matrixProvider, paint, *as_IB(image), src, dst, dstClip, srcToDst, aaFlags, constraint, sampling);
    return;
}
    ...
    const SkSamplingOptions& sampling, const SkPaint& paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    if (!can_use_draw_texture(paint, sampling)) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            this->drawImageQuad(set[i].fImage.get(), &set[i].fSrcRect, &set[i].fDstRect, set[i].fHasClip ? dstClips + dstClipIndex : nullptr,
                SkToGrQuadAAFlags(set[i].fAAFlags), set[i].fMatrixIndex < 0 ? nullptr : preViewMatrices + set[i].fMatrixIndex, sampling, *entryPaint,
                constraint);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!view) {
            ...
            this->drawImageQuad(image, &set[i].fSrcRect, &set[i].fDstRect, clip, SkToGrQuadAAFlags(set[i].fAAFlags),
