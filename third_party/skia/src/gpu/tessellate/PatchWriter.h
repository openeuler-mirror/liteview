...
#undef DEF_ATTRIB_TYPE
    ...
    static constexpr size_t kMaxStride = 4 * sizeof(SkPoint) + // control points
        ...
        + (CurveTypeAttrib::kEnabled ? sizeof(float) : 0) + (SsboIndexAttrib::kEnabled ? sizeof(int) : 0);
...
public:
    ...
    {
        this->writeConic(skvx::bit_pun<float2>(pts[0]), skvx::bit_pun<float2>(pts[1]), skvx::bit_pun<float2>(pts[2]), w);
    }
    ...
    {
        this->writeQuadratic(skvx::bit_pun<float2>(pts[0]), skvx::bit_pun<float2>(pts[1]), skvx::bit_pun<float2>(pts[2]));
    }
    ...
    {
        this->writeTriangle(skvx::bit_pun<float2>(p0), skvx::bit_pun<float2>(p1), skvx::bit_pun<float2>(p2));
    }
