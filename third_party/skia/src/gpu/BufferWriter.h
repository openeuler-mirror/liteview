...
struct UploadWriter : public BufferWriter {
    ...
    UploadWriter(void* ptr, size_t size)
        : BufferWriter(ptr, size)
    {
    }

    UploadWriter(const UploadWriter&) = delete;
    UploadWriter(UploadWriter&& that)
    {
        *this = std::move(that);
    }
    ...
    void write(size_t offset, const void* src, size_t srcRowBytes, size_t trimRowBytes, int rowCount)
    {
