...
void SkImage::asyncReadPixels(const SkImageInfo& info, const SkIRect& srcRect, ReadPixelsCallback callback, ReadPixelsContext context) const
{
    if (!SkIRect::MakeSize(this->dimensions()).contains(srcRect) || !SkImageInfoIsValid(info)) {
        callback(context, nullptr);
        return;
    }
    as_IB(this)->onAsyncReadPixels(info, srcRect, callback, context);
}
...
{
    if (!SkIRect::MakeSize(this->dimensions()).contains(srcRect) || dstSize.isZero() || (dstSize.width() & 0b1) || (dstSize.height() & 0b1)) {
        callback(context, nullptr);
        return;
    }
    as_IB(this)->onAsyncRescaleAndReadPixelsYUV420(yuvColorSpace, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
sk_sp<SkImage> SkImage::MakeFromEncoded(sk_sp<SkData> encoded, std::optional<SkAlphaType> alphaType)
{
    if (nullptr == encoded || 0 == encoded->size()) {
        return nullptr;
    }
    return SkImage::MakeFromGenerator(SkImageGenerator::MakeFromEncoded(std::move(encoded), alphaType));
}
...
{
#if SK_SUPPORT_GPU
    return as_IB(this)->onTextureSize();
#else
    *(int*)1 = 1;
    return 0;
#endif
...
GrBackendTexture SkImage::getBackendTexture(bool flushPendingGrContextIO, GrSurfaceOrigin* origin) const
{
...
GrSemaphoresSubmitted SkImage::flush(GrDirectContext* dContext, const GrFlushInfo& flushInfo) const
{
...
void SkImage_Base::onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize, RescaleGamma,
    RescaleMode, ReadPixelsCallback callback, ReadPixelsContext context) const
{
...
#if SK_SUPPORT_GPU
std::tuple<GrSurfaceProxyView, GrColorType> SkImage_Base::asView(GrRecordingContext* context, GrMipmapped mipmapped, GrImageTexGenPolicy policy) const
{
    if (!context) {
        return {};
    }
...
std::unique_ptr<GrFragmentProcessor> SkImage_Base::asFragmentProcessor(GrRecordingContext* rContext, SkSamplingOptions sampling, const SkTileMode tileModes[2],
    const SkMatrix& m, const SkRect* subset, const SkRect* domain) const
{
    ...
    if (sampling.mipmap != SkMipmapMode::kNone && (!rContext->priv().caps()->mipmapSupport() || this->dimensions().area() <= 1)) {
        sampling = SkSamplingOptions(sampling.filter);
    }
    return this->onAsFragmentProcessor(rContext, sampling, tileModes, m, subset, domain);
}

std::unique_ptr<GrFragmentProcessor> SkImage_Base::MakeFragmentProcessorFromView(GrRecordingContext* rContext, GrSurfaceProxyView view, SkAlphaType at,
...
GrSurfaceProxyView SkImage_Base::FindOrMakeCachedMipmappedView(GrRecordingContext* rContext, GrSurfaceProxyView view, uint32_t imageUniqueID)
{
    SkASSERT(rContext);
...
GrBackendTexture SkImage_Base::onGetBackendTexture(bool flushPendingGrContextIO, GrSurfaceOrigin* origin) const
{
...
#ifdef SK_GRAPHITE_ENABLED
std::tuple<skgpu::graphite::TextureProxyView, SkColorType> SkImage_Base::asView(skgpu::graphite::Recorder* recorder, skgpu::graphite::Mipmapped mipmapped) const
{
    if (!recorder) {
        return {};
    }
    ...
    if (mipmapped == skgpu::graphite::Mipmapped::kYes && image->textureProxyView().proxy()->mipmapped() != skgpu::graphite::Mipmapped::kYes) {
...
sk_sp<SkImage> SkImage::MakeFromPicture(sk_sp<SkPicture> picture, const SkISize& dimensions, const SkMatrix* matrix, const SkPaint* paint, BitDepth bitDepth,
    sk_sp<SkColorSpace> colorSpace, SkSurfaceProps props)
{
    return MakeFromGenerator(SkImageGenerator::MakeFromPicture(dimensions, std::move(picture), matrix, paint, bitDepth, std::move(colorSpace), props));
}

sk_sp<SkImage> SkImage::makeWithFilter(
    GrRecordingContext* rContext, const SkImageFilter* filter, const SkIRect& subset, const SkIRect& clipBounds, SkIRect* outSubset, SkIPoint* offset) const
{
    ...
    srcSpecialImage = SkSpecialImage::MakeFromImage(rContext, subset, sk_ref_sp(const_cast<SkImage*>(this)), SkSurfaceProps());
#else
    srcSpecialImage = SkSpecialImage::MakeFromImage(nullptr, subset, sk_ref_sp(const_cast<SkImage*>(this)), SkSurfaceProps());
#endif
    if (!srcSpecialImage) {
        return nullptr;
    }

    sk_sp<SkImageFilterCache> cache(SkImageFilterCache::Create(SkImageFilterCache::kDefaultTransientSize));
    ...
    SkImageFilter_Base::Context context(SkMatrix::Translate(-subset.x(), -subset.y()), clipBounds.makeOffset(-subset.topLeft()), cache.get(), fInfo.colorType(),
...
sk_sp<SkImage> SkImage::makeColorTypeAndColorSpace(SkColorType targetColorType, sk_sp<SkColorSpace> targetColorSpace, GrDirectContext* dContext) const
{
    ...
    if (colorType == targetColorType && (SkColorSpace::Equals(colorSpace, targetColorSpace.get()) || this->isAlphaOnly())) {
        return sk_ref_sp(const_cast<SkImage*>(this));
    }

    return as_IB(this)->onMakeColorTypeAndColorSpace(targetColorType, std::move(targetColorSpace), dContext);
}
