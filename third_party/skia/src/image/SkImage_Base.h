...
public:
     ...
     */
    virtual void onAsyncReadPixels(const SkImageInfo& info, SkIRect srcRect, ReadPixelsCallback callback, ReadPixelsContext context) const
    {
        this->onAsyncRescaleAndReadPixels(info, srcRect, RescaleGamma::kSrc, RescaleMode::kNearest, callback, context);
    }
     ...
     */
    virtual void onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize, RescaleGamma,
        RescaleMode, ReadPixelsCallback, ReadPixelsContext) const;
    ...
    std::tuple<GrSurfaceProxyView, GrColorType> asView(
        GrRecordingContext* context, GrMipmapped mipmapped, GrImageTexGenPolicy policy = GrImageTexGenPolicy::kDraw) const;
    ...
    std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(
        GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&, const SkRect* subset = nullptr, const SkRect* domain = nullptr) const;
    ...
    // texture-backed and the capabilities of the GPU require a data type conversion to put
    // the data in a texture.
    std::tuple<skgpu::graphite::TextureProxyView, SkColorType> asView(skgpu::graphite::Recorder*, skgpu::graphite::Mipmapped) const;
#endif

    virtual bool onPinAsTexture(GrRecordingContext*) const
    {
        return false;
    }
    ...
    virtual bool isPinnedOnContext(GrRecordingContext*) const
    {
        return false;
    }
    ...
    virtual size_t onTextureSize() const
    {
        return 0;
    }
...
#ifdef SK_GRAPHITE_ENABLED
    virtual sk_sp<SkImage> onMakeTextureImage(skgpu::graphite::Recorder*, RequiredImageProperties) const = 0;
#endif
...
protected:
    ...
    static GrSurfaceProxyView CopyView(GrRecordingContext*, GrSurfaceProxyView src, GrMipmapped, GrImageTexGenPolicy, std::string_view label);

    static std::unique_ptr<GrFragmentProcessor> MakeFragmentProcessorFromView(GrRecordingContext*, GrSurfaceProxyView, SkAlphaType, SkSamplingOptions,
        const SkTileMode[2], const SkMatrix&, const SkRect* subset, const SkRect* domain);
     ...
     * function.
     */
    static GrSurfaceProxyView FindOrMakeCachedMipmappedView(GrRecordingContext*, GrSurfaceProxyView, uint32_t imageUniqueID);
#endif

private:
#if SK_SUPPORT_GPU
    virtual std::tuple<GrSurfaceProxyView, GrColorType> onAsView(GrRecordingContext*, GrMipmapped, GrImageTexGenPolicy policy) const = 0;

    virtual std::unique_ptr<GrFragmentProcessor> onAsFragmentProcessor(
        GrRecordingContext*, SkSamplingOptions, const SkTileMode[2], const SkMatrix&, const SkRect* subset, const SkRect* domain) const
        = 0;
#endif
...
inline GrSurfaceProxyView SkImage_Base::CopyView(
    GrRecordingContext* context, GrSurfaceProxyView src, GrMipmapped mipmapped, GrImageTexGenPolicy policy, std::string_view label)
{
    SkBudgeted budgeted = policy == GrImageTexGenPolicy::kNew_Uncached_Budgeted ? SkBudgeted::kYes : SkBudgeted::kNo;
