...
bool SkImage_GpuBase::ValidateCompressedBackendTexture(const GrCaps* caps, const GrBackendTexture& tex, SkAlphaType at)
{
...
bool SkImage_GpuBase::getROPixels(GrDirectContext* dContext, SkBitmap* dst, CachingHint chint) const
{
...
sk_sp<SkImage> SkImage_GpuBase::onMakeSubset(const SkIRect& subset, GrDirectContext* direct) const
{
    ...
    auto copyView = GrSurfaceProxyView::Copy(direct, std::move(view), GrMipmapped::kNo, subset, SkBackingFit::kExact, isBudgeted,
    ...
    return sk_make_sp<SkImage_Gpu>(sk_ref_sp(direct), kNeedNewImageUniqueID, std::move(copyView), this->imageInfo().colorInfo());
}

bool SkImage_GpuBase::onReadPixels(GrDirectContext* dContext, const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB, int srcX, int srcY, CachingHint) const
{
...
sk_sp<GrTextureProxy> SkImage_GpuBase::MakePromiseImageLazyProxy(GrContextThreadSafeProxy* tsp, SkISize dimensions, GrBackendFormat backendFormat,
    GrMipmapped mipmapped, PromiseImageTextureFulfillProc fulfillProc, sk_sp<skgpu::RefCntedCallback> releaseHelper)
{
    SkASSERT(tsp);
    ...
    if (mipmapped == GrMipmapped::kYes && GrTextureTypeHasRestrictedSampling(backendFormat.textureType())) {
    ...
    public:
        PromiseLazyInstantiateCallback(PromiseImageTextureFulfillProc fulfillProc, sk_sp<skgpu::RefCntedCallback> releaseHelper)
