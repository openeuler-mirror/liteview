...
bool SkImage_Lazy::readPixelsProxy(GrDirectContext* ctx, const SkPixmap& pixmap) const
{
    ...
    GrSurfaceProxyView view = this->lockTextureProxyView(ctx, GrImageTexGenPolicy::kDraw, GrMipmapped::kNo);
...
std::tuple<GrSurfaceProxyView, GrColorType> SkImage_Lazy::onAsView(GrRecordingContext* context, GrMipmapped mipmapped, GrImageTexGenPolicy policy) const
{
...
std::unique_ptr<GrFragmentProcessor> SkImage_Lazy::onAsFragmentProcessor(GrRecordingContext* rContext, SkSamplingOptions sampling,
...
GrSurfaceProxyView SkImage_Lazy::textureProxyViewFromPlanes(GrRecordingContext* ctx, SkBudgeted budgeted) const
{
    ...
    for (int i = 0; i < yuvaPixmaps.numPlanes(); ++i) {
        ...
        SkBackingFit fit = yuvaPixmaps.plane(i).dimensions() == this->dimensions() ? SkBackingFit::kApprox : SkBackingFit::kExact;
        ...
        std::tie(views[i], std::ignore) = GrMakeUncachedBitmapProxyView(ctx, bitmap, GrMipmapped::kNo, fit);
        if (!views[i]) {
            return {};
        }
    ...
    GrImageInfo info(SkColorTypeToGrColorType(this->colorType()), kPremul_SkAlphaType,
        /*color space*/ nullptr, this->dimensions());

    auto sfc = ctx->priv().makeSFC(
        info, "ImageLazy_TextureProxyViewFromPlanes", SkBackingFit::kExact, 1, GrMipmapped::kNo, GrProtected::kNo, kTopLeft_GrSurfaceOrigin, budgeted);
    if (!sfc) {
        return {};
    }
 ...
 */
GrSurfaceProxyView SkImage_Lazy::lockTextureProxyView(GrRecordingContext* rContext, GrImageTexGenPolicy texGenPolicy, GrMipmapped mipmapped) const
{
    ...
    if (key.isValid()) {
        ...
        if (proxy) {
            ...
            if (mipmapped == GrMipmapped::kNo || view.asTextureProxy()->mipmapped() == GrMipmapped::kYes) {
                return view;
            } else {
    ...
    {
        ScopedGenerator generator(fSharedGenerator);
        if (auto view = generator->generateTexture(rContext, this->imageInfo(), mipmapped, texGenPolicy)) {
            installKey(view);
            return view;
        }
    }
    ...
    if (mipmapped == GrMipmapped::kNo && !rContext->priv().options().fDisableGpuYUVConversion) {
        ...
        SkBudgeted budgeted = texGenPolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted ? SkBudgeted::kNo : SkBudgeted::kYes;
    ...
    auto hint = texGenPolicy == GrImageTexGenPolicy::kDraw ? CachingHint::kAllow_CachingHint : CachingHint::kDisallow_CachingHint;
    if (SkBitmap bitmap; this->getROPixels(nullptr, &bitmap, hint)) {
        ...
        auto budgeted = texGenPolicy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted ? SkBudgeted::kNo : SkBudgeted::kYes;
 ...
 */
sk_sp<SkImage> SkImage_Lazy::onMakeTextureImage(skgpu::graphite::Recorder* recorder, RequiredImageProperties requiredProps) const
{
    ...
    // 1. Ask the generator to natively create one.
    {
        ...
        ScopedGenerator generator(fSharedGenerator);
        sk_sp<SkImage> newImage = generator->makeTextureImage(recorder, this->imageInfo(), requiredProps.fMipmapped);
        if (newImage) {
    ...
    if (SkBitmap bitmap; this->getROPixels(nullptr, &bitmap, CachingHint::kDisallow_CachingHint)) {
        return skgpu::graphite::MakeFromBitmap(recorder, this->imageInfo().colorInfo(), bitmap, nullptr, SkBudgeted::kNo, requiredProps);
    }
