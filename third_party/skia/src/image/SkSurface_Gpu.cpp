...
static GrRenderTarget* prepare_rt_for_external_access(SkSurface_Gpu* surface, SkSurface::BackendHandleAccess access)
{
...
sk_sp<SkSurface> SkSurface_Gpu::onNewSurface(const SkImageInfo& info)
{
    ...
    return SkSurface::MakeRenderTarget(fDevice->recordingContext(), kBudgeted, info, sampleCount, origin, &this->props());
}
...
sk_sp<SkImage> SkSurface_Gpu::onNewImageSnapshot(const SkIRect* subset)
{
    ...
    if (subset || !srcView.asTextureProxy() || rtp->refsWrappedObjects()) {
        ...
        if (!subset && srcView.asTextureProxy()) {
            return SkImage_Gpu::MakeWithVolatileSrc(sk_ref_sp(rContext), srcView, fDevice->imageInfo().colorInfo());
        }
    ...
    return sk_make_sp<SkImage_Gpu>(sk_ref_sp(rContext), kNeedNewImageUniqueID, std::move(srcView), info.colorInfo());
}
...
void SkSurface_Gpu::onAsyncReadPixels(const SkImageInfo& info, SkIRect srcRect, ReadPixelsCallback callback, ReadPixelsContext context)
{
    fDevice->asyncReadPixels(info, srcRect, callback, context);
}
...
void SkSurface_Gpu::onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize,
    RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    fDevice->asyncRescaleAndReadPixelsYUV420(yuvColorSpace, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
void SkSurface_Gpu::onResolveMSAA()
{
    fDevice->resolveMSAA();
}

GrSemaphoresSubmitted SkSurface_Gpu::onFlush(BackendSurfaceAccess access, const GrFlushInfo& info, const skgpu::MutableTextureState* newState)
{
...
bool SkSurface_Gpu::onWait(int numSemaphores, const GrBackendSemaphore* waitSemaphores, bool deleteSemaphoresAfterWait)
{
...
bool SkSurface_Gpu::onCharacterize(SkSurfaceCharacterization* characterization) const
{
    ...
    bool mipmapped = readSurfaceView.asTextureProxy() ? GrMipmapped::kYes == readSurfaceView.asTextureProxy()->mipmapped() : false;
    ...
    characterization->set(direct->threadSafeProxy(), maxResourceBytes, ii, format, readSurfaceView.origin(), numSamples,
        SkSurfaceCharacterization::Textureable(SkToBool(readSurfaceView.asTextureProxy())), SkSurfaceCharacterization::MipMapped(mipmapped),
        SkSurfaceCharacterization::UsesGLFBO0(usesGLFBO0), SkSurfaceCharacterization::VkRTSupportsInputAttachment(vkRTSupportsInputAttachment),
        SkSurfaceCharacterization::VulkanSecondaryCBCompatible(false), isProtected, this->props());
    return true;
}

void SkSurface_Gpu::onDraw(SkCanvas* canvas, SkScalar x, SkScalar y, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    ...
    auto tryDraw = [&] {
        ...
        const SkImageInfo info = fDevice->imageInfo();
        sk_sp<SkImage> image = sk_make_sp<SkImage_Gpu>(sk_ref_sp(canvasContext), kNeedNewImageUniqueID, std::move(srcView), info.colorInfo());
...
bool SkSurface_Gpu::onIsCompatible(const SkSurfaceCharacterization& characterization) const
{
    ...
    if (characterization.isTextureable()) {
        ...
        if (characterization.isMipMapped() && GrMipmapped::kNo == targetView.asTextureProxy()->mipmapped()) {
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(GrRecordingContext* rContext, const SkSurfaceCharacterization& c, SkBudgeted budgeted)
{
    ...
    auto device = rContext->priv().createDevice(budgeted, c.imageInfo(), SkBackingFit::kExact, c.sampleCount(), GrMipmapped(c.isMipMapped()), c.isProtected(),
        c.origin(), c.surfaceProps(), skgpu::v1::Device::InitContents::kClear);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(GrRecordingContext* rContext, SkBudgeted budgeted, const SkImageInfo& info, int sampleCount,
    GrSurfaceOrigin origin, const SkSurfaceProps* props, bool shouldCreateWithMips)
{
    ...
    auto device = rContext->priv().createDevice(budgeted, info, SkBackingFit::kExact, sampleCount, mipmapped, GrProtected::kNo, origin,
        SkSurfacePropsCopyOrDefault(props), skgpu::v1::Device::InitContents::kClear);
    if (!device) {
        return nullptr;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendTexture(GrRecordingContext* rContext, const GrBackendTexture& tex, GrSurfaceOrigin origin, int sampleCnt,
    SkColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props, SkSurface::TextureReleaseProc textureReleaseProc,
    SkSurface::ReleaseContext releaseContext)
{
    ...
    auto device = rContext->priv().createDevice(
        grColorType, std::move(proxy), std::move(colorSpace), origin, SkSurfacePropsCopyOrDefault(props), skgpu::v1::Device::InitContents::kUninit);
    if (!device) {
    ...
    const GrBackendTexture& backendTexture, GrSurfaceOrigin origin, ContentChangeMode mode, TextureReleaseProc releaseProc, ReleaseContext releaseContext)
{
    ...
    if (!validate_backend_texture(rContext->priv().caps(), backendTexture, sampleCnt, grColorType, true)) {
        return false;
    }
...
sk_sp<SkSurface> SkSurface::MakeFromBackendRenderTarget(GrRecordingContext* rContext, const GrBackendRenderTarget& rt, GrSurfaceOrigin origin,
    SkColorType colorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props, SkSurface::RenderTargetReleaseProc relProc,
    SkSurface::ReleaseContext releaseContext)
{
...
sk_sp<SkSurface> SkSurface::MakeFromAHardwareBuffer(
    GrDirectContext* dContext, AHardwareBuffer* hardwareBuffer, GrSurfaceOrigin origin, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* surfaceProps
...
)
{
    ...
    if (isTextureable) {
        ...
        sk_sp<SkSurface> surface = SkSurface::MakeFromBackendTexture(
            dContext, backendTexture, origin, 0, colorType, std::move(colorSpace), surfaceProps, deleteImageProc, deleteImageCtx);
