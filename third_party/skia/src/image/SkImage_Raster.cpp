...
public:
    ...
    SkImage_Raster(const SkBitmap& bm, bool bitmapMayBeMutable = false)
        : INHERITED(bm.info(), is_not_subset(bm) ? bm.getGenerationID() : (uint32_t)kNeedNewImageUniqueID)
        , fBitmap(bm)
    {
...
#endif
#ifdef SK_GRAPHITE_ENABLED
    sk_sp<SkImage> onMakeTextureImage(skgpu::graphite::Recorder*, RequiredImageProperties) const override;
#endif
...
bool SkImage_Raster::onPinAsTexture(GrRecordingContext* rContext) const
{
    if (fPinnedView) {
        ...
        SkASSERT(fPinnedCount == 0);
        SkASSERT(fPinnedUniqueID == 0);
        std::tie(fPinnedView, fPinnedColorType) = GrMakeCachedBitmapProxyView(rContext, fBitmap,
            /*label=*/"SkImageRaster_PinAsTexture", GrMipmapped::kNo);
        if (!fPinnedView) {
...
sk_sp<SkImage> SkImage::MakeRasterData(const SkImageInfo& info, sk_sp<SkData> data, size_t rowBytes)
{
    size_t size;
...
sk_sp<SkImage> SkImage::MakeRasterFromCompressed(sk_sp<SkData> data, int width, int height, CompressionType type)
{
...
sk_sp<SkImage> SkImage::MakeFromRaster(const SkPixmap& pmap, RasterReleaseProc proc, ReleaseContext ctx)
{
    size_t size;
    if (!SkImage_Raster::ValidArgs(pmap.info(), pmap.rowBytes(), &size) || !pmap.addr()) {
        return nullptr;
...
#if SK_SUPPORT_GPU
std::tuple<GrSurfaceProxyView, GrColorType> SkImage_Raster::onAsView(GrRecordingContext* rContext, GrMipmapped mipmapped, GrImageTexGenPolicy policy) const
{
    if (fPinnedView) {
        ...
        if (policy != GrImageTexGenPolicy::kDraw) {
            return { CopyView(rContext, fPinnedView, mipmapped, policy,
    ...
    if (policy == GrImageTexGenPolicy::kDraw) {
        ...
        return GrMakeCachedBitmapProxyView(rContext, fBitmap,
            /*label=*/"TextureForImageRasterWithPolicyEqualKDraw", mipmapped);
    }
    auto budgeted = (policy == GrImageTexGenPolicy::kNew_Uncached_Unbudgeted) ? SkBudgeted::kNo : SkBudgeted::kYes;
    return GrMakeUncachedBitmapProxyView(rContext, fBitmap, mipmapped, SkBackingFit::kExact, budgeted);
}

std::unique_ptr<GrFragmentProcessor> SkImage_Raster::onAsFragmentProcessor(GrRecordingContext* rContext, SkSamplingOptions sampling,
...
#ifdef SK_GRAPHITE_ENABLED
sk_sp<SkImage> SkImage_Raster::onMakeTextureImage(skgpu::graphite::Recorder* recorder, RequiredImageProperties requiredProps) const
{
    return skgpu::graphite::MakeFromBitmap(recorder, this->imageInfo().colorInfo(), fBitmap, this->refMips(), SkBudgeted::kNo, requiredProps);
}
#endif
