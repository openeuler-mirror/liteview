...
bool SkSurface_Base::onReplaceBackendTexture(const GrBackendTexture&, GrSurfaceOrigin, ContentChangeMode, TextureReleaseProc, ReleaseContext)
{
    return false;
}
#endif
...
void SkSurface_Base::onAsyncRescaleAndReadPixelsYUV420(SkYUVColorSpace yuvColorSpace, sk_sp<SkColorSpace> dstColorSpace, SkIRect srcRect, SkISize dstSize,
    ...
    const SkISize& dstSize, RescaleGamma rescaleGamma, RescaleMode rescaleMode, ReadPixelsCallback callback, ReadPixelsContext context)
{
    ...
    asSB(this)->onAsyncRescaleAndReadPixelsYUV420(yuvColorSpace, std::move(dstColorSpace), srcRect, dstSize, rescaleGamma, rescaleMode, callback, context);
}
...
bool SkSurface::replaceBackendTexture(const GrBackendTexture& backendTexture, GrSurfaceOrigin origin, ContentChangeMode mode,
    TextureReleaseProc textureReleaseProc, ReleaseContext releaseContext)
{
    return asSB(this)->onReplaceBackendTexture(backendTexture, origin, mode, textureReleaseProc, releaseContext);
}
...
GrSemaphoresSubmitted SkSurface::flush(const GrFlushInfo& info, const skgpu::MutableTextureState* newState)
{
...
void SkSurface::flush()
{
} // Flush is a no-op for CPU surfaces
...
sk_sp<SkSurface> SkSurface::MakeRenderTarget(GrRecordingContext*, SkBudgeted, const SkImageInfo&, int, GrSurfaceOrigin, const SkSurfaceProps*, bool)
{
    return nullptr;
}
#endif
