...
SkShaderBase::Context* SkImageShader::onMakeContext(const ContextRec& rec, SkArenaAlloc* alloc) const
{
    ...
    SkMatrix inv;
    if (!this->computeTotalInverse(*rec.fMatrix, rec.fLocalMatrix, &inv) || !legacy_shader_can_handle(inv)) {
        return nullptr;
    }
...
sk_sp<SkShader> SkImageShader::MakeRaw(sk_sp<SkImage> image, SkTileMode tmx, SkTileMode tmy, const SkSamplingOptions& options, const SkMatrix* localMatrix)
{
    ...
    return SkLocalMatrixShader::MakeWrapped<SkImageShader>(localMatrix, image, subset, tmx, tmy, options,
        /*raw=*/true,
    ...
    const SkMatrix* localMatrix, bool clampAsIfUnpremul)
{
    ...
    return SkLocalMatrixShader::MakeWrapped<SkImageShader>(localMatrix, std::move(image), subset, tmx, tmy, options,
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(const GrFPArgs& args) const
{
    ...
    auto fp = as_IB(fImage.get())->asFragmentProcessor(args.fContext, fSampling, tileModes, lmInverse, subset);
    if (!fp) {
        return nullptr;
    }

    if (!fRaw) {
        fp = GrColorSpaceXformEffect::Make(std::move(fp), fImage->colorSpace(), fImage->alphaType(), args.fDstColorInfo->colorSpace(), kPremul_SkAlphaType);
...
#ifdef SK_ENABLE_SKSL
void SkImageShader::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
    ImageShaderBlock::ImageData imgData(fSampling, fTileModeX, fTileModeY, fSubset);

#ifdef SK_GRAPHITE_ENABLED
    auto [imageToDraw, newSampling] = skgpu::graphite::GetGraphiteBacked(keyContext.recorder(), fImage.get(), fSampling);

    if (imageToDraw) {
        imgData.fSampling = newSampling;
        skgpu::graphite::Mipmapped mipmapped = (newSampling.mipmap != SkMipmapMode::kNone) ? skgpu::graphite::Mipmapped::kYes : skgpu::graphite::Mipmapped::kNo;
...
{
    sk_sp<SkShader> s = SkImageShader::Make(SkMakeImageFromRasterBitmap(src, mode), tmx, tmy, sampling, localMatrix);
    if (!s) {
        return nullptr;
    }
...
public:
    ...
    skvm::Color onProgram(skvm::Builder* b, skvm::Coord device, skvm::Coord local, skvm::Color color, const SkMatrixProvider& matrices, const SkMatrix* localM,
        const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const override
    {
        return fImageShader.makeProgram(b, device, local, color, matrices, localM, dst, uniforms, this, alloc);
    }
...
bool SkImageShader::doStages(const SkStageRec& rec, TransformShader* updater) const
{
    ...
    if (!this->computeTotalInverse(rec.fMatrixProvider.localToDevice(), rec.fLocalM, &totalInverse)) {
        return false;
    }
    ...
    auto limit_x = alloc->make<SkRasterPipeline_TileCtx>(), limit_y = alloc->make<SkRasterPipeline_TileCtx>();
    ...
    auto append_tiling_and_gather = [&] {
        ...
        case kA16_unorm_SkColorType:
            p->append(SkRasterPipeline::gather_a16, ctx);
            break;
        case kA16_float_SkColorType:
            p->append(SkRasterPipeline::gather_af16, ctx);
            break;
        ...
        case kR8G8_unorm_SkColorType:
            p->append(SkRasterPipeline::gather_rg88, ctx);
            break;
        case kR16G16_unorm_SkColorType:
            p->append(SkRasterPipeline::gather_rg1616, ctx);
            break;
        case kR16G16_float_SkColorType:
            p->append(SkRasterPipeline::gather_rgf16, ctx);
            break;
    ...
    auto append_misc = [&] {
        ...
        if (sampling.useCubic) {
            p->append(at == kUnpremul_SkAlphaType || fClampAsIfUnpremul ? SkRasterPipeline::clamp_01 : SkRasterPipeline::clamp_gamut);
        }
...
skvm::Color SkImageShader::onProgram(skvm::Builder* b, skvm::Coord device, skvm::Coord origLocal, skvm::Color paint, const SkMatrixProvider& matrices,
    const SkMatrix* localM, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
    return this->makeProgram(b, device, origLocal, paint, matrices, localM, dst, uniforms, nullptr, alloc);
}

skvm::Color SkImageShader::makeProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord origLocal, skvm::Color paint, const SkMatrixProvider& matrices,
    const SkMatrix* localM, const SkColorInfo& dst, skvm::Uniforms* uniforms, const TransformShader* coordShader, SkArenaAlloc* alloc) const
{
    SkASSERT(!needs_subset(fImage.get(), fSubset)); // TODO(skbug.com/12784)
    SkMatrix baseInv;
    ...
    const bool input_is_opaque = SkAlphaTypeIsOpaque(upper.alphaType()) || SkColorTypeIsAlwaysOpaque(upper.colorType());
    ...
    auto sample_texel = [&](const Uniforms& u, skvm::F32 sx, skvm::F32 sy) -> skvm::Color {
        ...
        switch (fTileModeX) {
        case SkTileMode::kDecal: /* handled after gather */
            break;
        case SkTileMode::kClamp: /*    we always clamp   */
            break;
        case SkTileMode::kRepeat:
            sx = repeat(sx, u.w, u.iw);
            break;
        case SkTileMode::kMirror:
            sx = mirror(sx, u.w, u.i2w);
            break;
        }
        switch (fTileModeY) {
        case SkTileMode::kDecal: /* handled after gather */
        ...
        case SkTileMode::kClamp: /*    we always clamp   */
            break;
        case SkTileMode::kRepeat:
            sy = repeat(sy, u.h, u.ih);
            break;
        case SkTileMode::kMirror:
            sy = mirror(sy, u.h, u.i2h);
            break;
        }
    ...
    auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv, skvm::Coord local) {
        ...
        if (sampling.useCubic) {
            ...
            auto dot = [](const skvm::F32 a[], const skvm::F32 b[]) { return a[0] * b[0] + a[1] * b[1] + a[2] * b[2] + a[3] * b[3]; };
            ...
            skvm::F32 left = local.x - 0.5f, top = local.y - 0.5f, right = local.x + 0.5f, bottom = local.y + 0.5f;
            ...
            return lerp(
                lerp(sample_texel(u, left, top), sample_texel(u, right, top), fx), lerp(sample_texel(u, left, bottom), sample_texel(u, right, bottom), fx), fy);
        } else {
    ...
    if (lower) {
        ...
        c = lerp(c, sample_level(*lower, lowerInv, lowerLocal), p->uniformF(uniforms->pushF(lowerWeight)));
    }
    ...
    return fRaw ? c : SkColorSpaceXformSteps { cs, at, dst.colorSpace(), dst.alphaType() }.program(p, uniforms, c);
}
