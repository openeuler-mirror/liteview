...
struct CachedImageInfo {
    ...
    static CachedImageInfo Make(const SkRect& bounds, const SkMatrix& viewMatrix,
        SkMatrix* localMatrix, // in/out
        SkColorType dstColorType, SkColorSpace* dstColorSpace, const int maxTextureSize, const SkSurfaceProps& propsIn)
    {
        ...
        return { true, tileScale, SkMatrix::RectToRect(bounds, SkRect::MakeIWH(tileSize.width(), tileSize.height())),
            SkImageInfo::Make(tileSize.width(), tileSize.height(), imgCT, kPremul_SkAlphaType, imgCS), props };
    }
...
sk_sp<SkShader> SkPictureShader::rasterShader(
    const SkMatrix& viewMatrix, SkMatrix* localMatrix, SkColorType dstColorType, SkColorSpace* dstColorSpace, const SkSurfaceProps& propsIn) const
{
    const int maxTextureSize_NotUsedForCPU = 0;
    CachedImageInfo info = CachedImageInfo::Make(fTile, viewMatrix, localMatrix, dstColorType, dstColorSpace, maxTextureSize_NotUsedForCPU, propsIn);
    if (!info.success) {
        return nullptr;
    }
...
bool SkPictureShader::onAppendStages(const SkStageRec& rec) const
{
    ...
    bitmapShader = this->rasterShader(rec.fMatrixProvider.localToDevice(), &lm, rec.fDstColorType, rec.fDstCS, rec.fSurfaceProps);
    if (!bitmapShader) {
        return false;
    }
...
skvm::Color SkPictureShader::onProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord local, skvm::Color paint, const SkMatrixProvider& matrices,
    const SkMatrix* localM, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
    ...
    bitmapShader = this->rasterShader(matrices.localToDevice(), &lm, dst.colorType(), dst.colorSpace(), props);
    if (!bitmapShader) {
        return {};
    }
...
SkShaderBase::Context* SkPictureShader::onMakeContext(const ContextRec& rec, SkArenaAlloc* alloc) const
{
    ...
    sk_sp<SkShader> bitmapShader = this->rasterShader(*rec.fMatrix, &lm, rec.fDstColorType, rec.fDstColorSpace, rec.fProps);
    if (!bitmapShader) {
        return nullptr;
    }
...
std::unique_ptr<GrFragmentProcessor> SkPictureShader::asFragmentProcessor(const GrFPArgs& args) const
{
    ...
    auto info = CachedImageInfo::Make(fTile, viewMatrix, &lm, dstColorType, dstCS.get(), ctx->priv().caps()->maxTextureSize(), args.fSurfaceProps);
    SkMatrix inv;
    ...
    skgpu::UniqueKey key;
    std::tuple keyData
        = { dstCS->toXYZD50Hash(), dstCS->transferFnHash(), static_cast<uint32_t>(dstColorType), fPicture->uniqueID(), fTile, info.tileScale, info.props };
    ...
    if (auto proxy = provider->findOrCreateProxyByUniqueKey(key)) {
        ...
        auto image = info.makeImage(
            SkSurface::MakeRenderTarget(ctx, SkBudgeted::kYes, info.imageInfo, msaaSampleCount, kTopLeft_GrSurfaceOrigin, &info.props, createWithMips),
            fPicture.get());
        if (!image) {
            return nullptr;
        }
    ...
    const GrSamplerState sampler(static_cast<GrSamplerState::WrapMode>(fTmx), static_cast<GrSamplerState::WrapMode>(fTmy), fFilter);

    return GrTextureEffect::Make(std::move(view), kPremul_SkAlphaType, inv, sampler, *ctx->priv().caps());
}
#endif
