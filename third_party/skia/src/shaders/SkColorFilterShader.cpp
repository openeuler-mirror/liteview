...
skvm::Color SkColorFilterShader::onProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord local, skvm::Color paint, const SkMatrixProvider& matrices,
...
std::unique_ptr<GrFragmentProcessor> SkColorFilterShader::asFragmentProcessor(const GrFPArgs& args) const
{
    ...
    auto [success, fp] = fFilter->asFragmentProcessor(std::move(shaderFP), args.fContext, *args.fDstColorInfo, args.fSurfaceProps);
