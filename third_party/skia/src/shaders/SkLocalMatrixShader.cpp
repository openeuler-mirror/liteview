...
#if SK_SUPPORT_GPU
std::unique_ptr<GrFragmentProcessor> SkLocalMatrixShader::asFragmentProcessor(const GrFPArgs& args) const
{
    return as_SB(fWrappedShader)->asFragmentProcessor(GrFPArgs::ConcatLocalMatrix(args, fLocalMatrix));
}
#endif

#ifdef SK_ENABLE_SKSL
void SkLocalMatrixShader::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
...
skvm::Color SkLocalMatrixShader::onProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord local, skvm::Color paint, const SkMatrixProvider& matrices,
    const SkMatrix* localM, const SkColorInfo& dst, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
    ...
    return as_SB(fWrappedShader)->program(p, device, local, paint, matrices, lm.get(), dst, uniforms, alloc);
}
...
protected:
    ...
    bool onAppendStages(const SkStageRec& rec) const override
    {
        ...
        SkStageRec newRec = { rec.fPipeline, rec.fAlloc, rec.fDstColorType, rec.fDstCS, rec.fPaint, rec.fLocalM, matrixProvider, rec.fSurfaceProps };
    ...
    {
        SkOverrideDeviceMatrixProvider matrixProvider(fCTM);
        return as_SB(fProxyShader)->program(p, device, local, paint, matrixProvider, localM, dst, uniforms, alloc);
    }
