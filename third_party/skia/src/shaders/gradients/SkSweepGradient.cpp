...
sk_sp<SkFlattenable> SkSweepGradient::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkGradientShader::MakeSweep(center.x(), center.y(), desc.fColors, std::move(desc.fColorSpace), desc.fPos, desc.fCount, desc.fTileMode, startAngle,
        endAngle, desc.fInterpolation, &legacyLocalMatrix);
}
...
skvm::F32 SkSweepGradient::transformT(skvm::Builder* p, skvm::Uniforms* uniforms, skvm::Coord coord, skvm::I32* mask) const
{
    ...
    skvm::F32 phi = slope
        * poly(s, -7.0547382347285747528076171875e-3f, +2.476101927459239959716796875e-2f, -5.185396969318389892578125e-2f, +0.15912117063999176025390625f);
...
std::unique_ptr<GrFragmentProcessor> SkSweepGradient::asFragmentProcessor(const GrFPArgs& args) const
{
    ...
    auto fp = GrSkSLFP::Make(effect, "SweepLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kPreservesOpaqueInput, "bias", fTBias, "scale", fTScale,
...
#ifdef SK_ENABLE_SKSL
void SkSweepGradient::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
    GradientShaderBlocks::GradientData data(
        SkShaderBase::GradientType::kSweep, fCenter, { 0.0f, 0.0f }, 0.0, 0.0f, fTBias, fTScale, fTileMode, fColorCount, fOrigColors4f, fOrigPos);
    ...
    int colorCount, SkTileMode mode, SkScalar startAngle, SkScalar endAngle, const Interpolation& interpolation, const SkMatrix* localMatrix)
{
    ...
    return SkLocalMatrixShader::MakeWrapped<SkSweepGradient>(localMatrix, SkPoint::Make(cx, cy), t0, t1, desc);
}
