...
void ramp(const skvx::float4& c, const skvx::float4& dc, SkPMColor dst[], int n, const skvx::float4& bias0, const skvx::float4& bias1)
{
    SkASSERT(n > 0);

    const auto dc2 = dc + dc, dc4 = dc2 + dc2;

    auto c0 = c + DstTraits<premul>::pre_lerp_bias(bias0), c1 = c + dc + DstTraits<premul>::pre_lerp_bias(bias1), c2 = c0 + dc2, c3 = c1 + dc2;
...
template <> SkScalar pinFx<SkTileMode::kClamp>(SkScalar fx)
{
    return fx;
}
...
SkLinearGradient::LinearGradient4fContext::LinearGradient4fContext(const SkLinearGradient& shader, const ContextRec& rec)
    : INHERITED(shader, rec)
{
    ...
    fIntervals.init(shader, rec.fDstColorSpace, shader.fTileMode, fColorsArePremul, rec.fPaintAlpha * (1.0f / 255), reverseIntervals);

    SkASSERT(!fIntervals->empty());
    fCachedInterval = fIntervals->begin();
}

const Sk4fGradientInterval* SkLinearGradient::LinearGradient4fContext::findInterval(SkScalar fx) const
{
...
void SkLinearGradient::LinearGradient4fContext::shadeSpan(int x, int y, SkPMColor dst[], int count)
{
    SkASSERT(count > 0);
...
template <ApplyPremul premul>
void SkLinearGradient::LinearGradient4fContext::shadePremulSpan(int x, int y, SkPMColor dst[], int count, float bias0, float bias1) const
{
...
void SkLinearGradient::LinearGradient4fContext::shadeSpanInternal(int x, int y, SkPMColor dst[], int count, float bias0, float bias1) const
{
    SkPoint pt;
    ...
    LinearIntervalProcessor<premul, tileMode> proc(fIntervals->begin(), fIntervals->end() - 1, this->findInterval(fx), fx, dx, SkScalarNearlyZero(dx * count));
...
template <ApplyPremul premul, SkTileMode tileMode> class SkLinearGradient::LinearGradient4fContext::LinearIntervalProcessor {
public:
    LinearIntervalProcessor(const Sk4fGradientInterval* firstInterval, const Sk4fGradientInterval* lastInterval, const Sk4fGradientInterval* i, SkScalar fx,
        SkScalar dx, bool is_vertical)
    ...
    bool currentRampIsZero() const
    {
        return fZeroRamp;
    }
    const skvx::float4& currentColor() const
    {
        return fCc;
    }
    const skvx::float4& currentColorGrad() const
    {
        return fDcDx;
    }
...
private:
    ...
    void init_average_props()
    {
        ...
        for (const auto* i = fFirstInterval; i <= fLastInterval; ++i) {
            ...
            //   Avg += C * (t1 - t0)
            //
            const auto c = DstTraits<premul>::load(i->fCb) + DstTraits<premul>::load(i->fCg) * (i->fT0 + i->fT1) * 0.5f;
