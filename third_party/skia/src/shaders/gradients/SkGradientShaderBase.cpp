...
template <int N, typename T, bool MEM_MOVE> static bool validate_array(SkReadBuffer& buffer, size_t count, SkSTArray<N, T, MEM_MOVE>* array)
{
...
bool SkGradientShaderBase::DescriptorScope::unflatten(SkReadBuffer& buffer, SkMatrix* legacyLocalMatrix)
{
    ...
    if (!(validate_array(buffer, fCount, &fColorStorage) && buffer.readColor4fArray(fColorStorage.begin(), fCount))) {
        return false;
    }
    ...
    if (SkToBool(flags & kHasPosition_GSF)) {
        if (!(validate_array(buffer, fCount, &fPosStorage) && buffer.readScalarArray(fPosStorage.begin(), fCount))) {
            return false;
        }
...
static void init_stop_pos(SkRasterPipeline_GradientCtx* ctx, size_t stop, float t_l, float t_r, SkPMColor4f c_l, SkPMColor4f c_r)
{
    ...
    if (fColorCount == 2 && fOrigPos == nullptr) {
        ...
        if (fOrigPos == nullptr) {
            ...
            if (fColorCount > 2) {
                ...
                lastStop = fOrigColors4f[fColorCount - 2] != fOrigColors4f[fColorCount - 1] ? fColorCount - 1 : fColorCount - 2;
            } else {
                firstStop = 0;
                lastStop = 1;
            }
...
skvm::Color SkGradientShaderBase::onProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord local, skvm::Color /*paint*/, const SkMatrixProvider& mats,
    const SkMatrix* localM, const SkColorInfo& dstInfo, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
    SkMatrix inv;
    ...
    SkImageInfo common = SkImageInfo::Make(fColorCount, 1, kRGBA_F32_SkColorType, kUnpremul_SkAlphaType), src = common.makeColorSpace(fColorSpace),
    ...
    SkAssertResult(SkConvertPixels(dst, rgba.data(), dst.minRowBytes(), src, fOrigColors4f, src.minRowBytes()));
...
SkColor4fXformer::SkColor4fXformer(const SkColor4f* colors, int colorCount, SkColorSpace* src, SkColorSpace* dst)
{
    fColors = colors;
    ...
    if (dst && !SkColorSpace::Equals(src, dst)) {
        ...
        SkAssertResult(SkConvertPixels(dstInfo, fStorage.begin(), info.minRowBytes(), srcInfo, fColors, info.minRowBytes()));
...
//
bool SkGradientShaderBase::ValidGradient(const SkColor4f colors[], const SkScalar pos[], int count, SkTileMode tileMode)
{
...
sk_sp<SkShader> SkGradientShaderBase::MakeDegenerateGradient(
...
SkGradientShaderBase::ColorStopOptimizer::ColorStopOptimizer(const SkColor4f* colors, const SkScalar* pos, int count, SkTileMode mode)
    : fColors(colors)
    , fPos(pos)
    , fCount(count)
{
    ...
    if (SkScalarNearlyEqual(pos[0], 0.0f) && SkScalarNearlyEqual(pos[1], 0.0f) && SkScalarNearlyEqual(pos[2], 1.0f)) {
    ...
    } else if (SkScalarNearlyEqual(pos[0], 0.0f) && SkScalarNearlyEqual(pos[1], 1.0f) && SkScalarNearlyEqual(pos[2], 1.0f)) {
