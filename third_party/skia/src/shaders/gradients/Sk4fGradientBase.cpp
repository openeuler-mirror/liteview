...
public:
    ...
    void iterate(const SkColor4f* colors, std::function<void(const SkColor4f&, const SkColor4f&, SkScalar, SkScalar)> func) const
    {
        if (!fShader.fOrigPos) {
...
private:
    void iterateImplicitPos(const SkColor4f* colors, std::function<void(const SkColor4f&, const SkColor4f&, SkScalar, SkScalar)> func) const
    {
...
void addMirrorIntervals(const SkGradientShaderBase& shader, const SkColor4f* colors, const skvx::float4& componentScale, bool premulColors, bool reverse,
    Sk4fGradientIntervalBuffer::BufferType* buffer)
{
    ...
    iter.iterate(colors, [&](const SkColor4f& c0, const SkColor4f& c1, SkScalar t0, SkScalar t1) {
        ...
        if (mirror_t0 != mirror_t1) {
            buffer->emplace_back(pack_color(c0, premulColors, componentScale), mirror_t0, pack_color(c1, premulColors, componentScale), mirror_t1);
        }
    });
}
...
Sk4fGradientInterval::Sk4fGradientInterval(const skvx::float4& c0, SkScalar t0, const skvx::float4& c1, SkScalar t1)
    : fT0(t0)
    , fT1(t1)
{
    SkASSERT(t0 != t1);
...
void Sk4fGradientIntervalBuffer::init(
    const SkGradientShaderBase& shader, SkColorSpace* dstCS, SkTileMode tileMode, bool premulColors, SkScalar alpha, bool reverse)
{
    ...
    const skvx::float4 componentScale = premulColors ? skvx::float4(alpha) : skvx::float4(1.0f, 1.0f, 1.0f, alpha);
    ...
    if (tileMode == SkTileMode::kClamp) {
        ...
        const auto clamp_color = pack_color(xformedColors.fColors[first_index], premulColors, componentScale);
        ...
        addMirrorIntervals(shader, xformedColors.fColors, componentScale, premulColors, false, &fIntervals);
    }

    const IntervalIterator iter(shader, reverse);
    iter.iterate(xformedColors.fColors, [&](const SkColor4f& c0, const SkColor4f& c1, SkScalar t0, SkScalar t1) {
        SkASSERT(fIntervals.empty() || fIntervals.back().fT1 == t0);

        fIntervals.emplace_back(pack_color(c0, premulColors, componentScale), t0, pack_color(c1, premulColors, componentScale), t1);
    });

    if (tileMode == SkTileMode::kClamp) {
        // synthetic edge interval: Pn .. +/-inf
        const auto clamp_color = pack_color(xformedColors.fColors[last_index], premulColors, componentScale);
        ...
        addMirrorIntervals(shader, xformedColors.fColors, componentScale, premulColors, true, &fIntervals);
    }
}
...
const Sk4fGradientInterval* Sk4fGradientIntervalBuffer::findNext(SkScalar t, const Sk4fGradientInterval* prev, bool increasing) const
{
...
SkGradientShaderBase::GradientShaderBase4fContext::GradientShaderBase4fContext(const SkGradientShaderBase& shader, const ContextRec& rec)
