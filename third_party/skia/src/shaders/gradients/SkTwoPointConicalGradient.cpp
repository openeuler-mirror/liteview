...
#endif
#ifdef SK_ENABLE_SKSL
    void addToKey(const SkKeyContext&, SkPaintParamsKeyBuilder*, SkPipelineDataGatherer*) const override;
#endif
...
protected:
    ...
    skvm::F32 transformT(skvm::Builder*, skvm::Uniforms*, skvm::Coord coord, skvm::I32* mask) const final;
...
sk_sp<SkShader> SkTwoPointConicalGradient::Create(
    const SkPoint& c0, SkScalar r0, const SkPoint& c1, SkScalar r1, const Descriptor& desc, const SkMatrix* localMatrix)
{
    ...
    return SkLocalMatrixShader::MakeWrapped<SkTwoPointConicalGradient>(localMatrix, c0, r0, c1, r1, desc, gradientType, gradientMatrix, focalData);
}

SkTwoPointConicalGradient::SkTwoPointConicalGradient(const SkPoint& start, SkScalar startRadius, const SkPoint& end, SkScalar endRadius, const Descriptor& desc,
    Type type, const SkMatrix& gradientMatrix, const FocalData& data)
...
SkShaderBase::GradientType SkTwoPointConicalGradient::asGradient(GradientInfo* info, SkMatrix* localMatrix) const
{
    if (info) {
...
sk_sp<SkFlattenable> SkTwoPointConicalGradient::CreateProc(SkReadBuffer& buffer)
{
    ...
    return SkGradientShader::MakeTwoPointConical(
        c1, r1, c2, r2, desc.fColors, std::move(desc.fColorSpace), desc.fPos, desc.fCount, desc.fTileMode, desc.fInterpolation, &legacyLocalMatrix);
}
...
void SkTwoPointConicalGradient::appendGradientStages(SkArenaAlloc* alloc, SkRasterPipeline* p, SkRasterPipeline* postPipeline) const
{
...
skvm::F32 SkTwoPointConicalGradient::transformT(skvm::Builder* p, skvm::Uniforms* uniforms, skvm::Coord coord, skvm::I32* mask) const
{
    ...
    if (fType == Type::kRadial) {
        float denom = 1.0f / (fRadius2 - fRadius1), scale = std::max(fRadius1, fRadius2) * denom, bias = -fRadius1 * denom;
...
std::unique_ptr<GrFragmentProcessor> SkTwoPointConicalGradient::asFragmentProcessor(const GrFPArgs& args) const
{
    ...
    case SkTwoPointConicalGradient::Type::kStrip: {
        ...
        fp = GrSkSLFP::Make(kEffect, "TwoPointConicalStripLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "r0_2", r0 * r0);
    } break;
    ...
    case SkTwoPointConicalGradient::Type::kRadial: {
        ...
        fp = GrSkSLFP::Make(
            kEffect, "TwoPointConicalRadialLayout", /*inputFP=*/nullptr, GrSkSLFP::OptFlags::kNone, "r0", r0, "lengthScale", isRadiusIncreasing ? 1.0f : -1.0f);
        ...
        // Map center to (0, 0)
        matrix.set(SkMatrix::Translate(-this->getStartCenter().fX, -this->getStartCenter().fY));
...
#ifdef SK_ENABLE_SKSL
void SkTwoPointConicalGradient::addToKey(const SkKeyContext& keyContext, SkPaintParamsKeyBuilder* builder, SkPipelineDataGatherer* gatherer) const
{
    GradientShaderBlocks::GradientData data(
        GradientType::kConical, fCenter1, fCenter2, fRadius1, fRadius2, 0.0f, 0.0f, fTileMode, fColorCount, fOrigColors4f, fOrigPos);
    ...
    const SkMatrix* localMatrix)
{
    ...
    if (SkScalarNearlyZero((start - end).length(), SkGradientShaderBase::kDegenerateThreshold)) {
        ...
        if (SkScalarNearlyEqual(startRadius, endRadius, SkGradientShaderBase::kDegenerateThreshold)) {
            ...
            if (mode == SkTileMode::kClamp && endRadius > SkGradientShaderBase::kDegenerateThreshold) {
                ...
                return SkGradientShaderBase::MakeDegenerateGradient(colors, pos, colorCount, std::move(colorSpace), mode);
            }
    ...
    SkGradientShaderBase::Descriptor desc(opt.fColors, std::move(colorSpace), opt.fPos, opt.fCount, mode, interpolation);
