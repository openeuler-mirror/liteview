...
/*not static*/ inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy, int count, uint32_t* colors)
{
    ...
    auto bilerp = [&](skvx::Vec<8, uint32_t> packed_x_coordinates) -> skvx::Vec<8, uint32_t> {
        ...
        auto gather = [](const uint32_t* ptr, skvx::Vec<8, uint32_t> ix) {
            ...
            return skvx::bit_pun<skvx::Vec<8, uint32_t>>(_mm256_i32gather_epi32((const int*)ptr, skvx::bit_pun<__m256i>(ix), 4));
#else
        ...
        skvx::Vec<8, uint32_t> tl = gather(row0, x0), tr = gather(row0, x1), bl = gather(row1, x0), br = gather(row1, x1);
        ...
        auto lerp_x = [&](skvx::Vec<8, uint32_t> L, skvx::Vec<8, uint32_t> R) {
            __m256i l = skvx::bit_pun<__m256i>(L), r = skvx::bit_pun<__m256i>(R), wr = skvx::bit_pun<__m256i>(wx),
            ...
            __m256i abcd = _mm256_permute2x128_si256(lo, hi, 0x20), efgh = _mm256_permute2x128_si256(lo, hi, 0x31);

            return skvx::join(skvx::bit_pun<skvx::Vec<16, uint16_t>>(abcd), skvx::bit_pun<skvx::Vec<16, uint16_t>>(efgh));
        };

        skvx::Vec<32, uint16_t> top = lerp_x(tl, tr), bot = lerp_x(bl, br), sum = 16 * top + (bot - top) * wy;
#else
        // Treat 32-bit pixels as 4 8-bit values, and expand to 16-bit for room to multiply.
        auto to_16x4 = [](auto v) -> skvx::Vec<32, uint16_t> { return skvx::cast<uint16_t>(skvx::bit_pun<skvx::Vec<32, uint8_t>>(v)); };
        ...
        skvx::Vec<32, uint16_t> sum = lerp(lerp(to_16x4(tl), to_16x4(bl), wy), lerp(to_16x4(tr), to_16x4(br), wy), to_16x4(wx));
#endif
    ...
    if (count > 0) {
        __m256i active = skvx::bit_pun<__m256i>(count > skvx::Vec<8, int> { 0, 1, 2, 3, 4, 5, 6, 7 }), coords = _mm256_maskload_epi32((const int*)xy, active),
                pixels;
