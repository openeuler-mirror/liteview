...
static void premul_should_swapRB(bool kSwapRB, uint32_t* dst, const uint32_t* src, int count)
{
    ...
    auto premul8 = [=](__m512i* lo, __m512i* hi) {
        ...
        if (kSwapRB) {
            mask = { 2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15, 2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15, 2, 6, 10, 14, 1, 5, 9, 13, 0,
                4, 8, 12, 3, 7, 11, 15, 2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15 };
        } else {
            mask = { 0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 0, 4, 8, 12, 1, 5, 9, 13, 2, 6,
                10, 14, 3, 7, 11, 15, 0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15 };
        }
        ...
        *hi = _mm512_shuffle_epi8(*hi, planar);
        ...
        __m512i r = _mm512_unpacklo_epi8(rg, zeros), g = _mm512_unpackhi_epi8(rg, zeros), b = _mm512_unpacklo_epi8(ba, zeros),
    ...
    while (count >= 32) {
        __m512i lo = _mm512_loadu_si512((const __m512i*)(src + 0)), hi = _mm512_loadu_si512((const __m512i*)(src + 16));
...
/*not static*/ inline void RGBA_to_BGRA(uint32_t* dst, const uint32_t* src, int count)
{
    const uint8_t mask[64] = { 2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15, 2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15, 2, 1, 0, 3, 6, 5, 4,
        7, 10, 9, 8, 11, 14, 13, 12, 15, 2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15 };
...
/*not static*/ inline void grayA_to_RGBA(uint32_t dst[], const uint8_t* src, int count)
{
    while (count >= 32) {
        ...
        __m512i gg = _mm512_or_si512(_mm512_and_si512(ga, _mm512_set1_epi16(0x00FF)), _mm512_slli_epi16(ga, 8));
        ...
        __m512i ggga_lo_shuffle_1 = _mm512_shuffle_i32x4(ggga_lo, ggga_hi, 0x44), ggga_hi_shuffle_1 = _mm512_shuffle_i32x4(ggga_lo, ggga_hi, 0xee);
...
static void inverted_cmyk_to(Format format, uint32_t* dst, const uint32_t* src, int count)
{
    auto convert8 = [=](__m512i* lo, __m512i* hi) {
        ...
        *hi = _mm512_shuffle_epi8(*hi, planar);
        ...
        // Unpack to 16-bit planar.
        __m512i c = _mm512_unpacklo_epi8(cm, zeros), m = _mm512_unpackhi_epi8(cm, zeros), y = _mm512_unpacklo_epi8(yk, zeros),
        ...
        // Repack into interlaced pixels.
        __m512i rg = _mm512_or_si512(r, _mm512_slli_epi16(g, 8)), ba = _mm512_or_si512(b, _mm512_set1_epi16((uint16_t)0xFF00));
        *lo = _mm512_unpacklo_epi16(rg, ba);
        *hi = _mm512_unpackhi_epi16(rg, ba);
    };
...
static void premul_should_swapRB(bool kSwapRB, uint32_t* dst, const uint32_t* src, int count)
{
    ...
    auto premul8 = [=](__m256i* lo, __m256i* hi) {
        ...
        __m256i planar;
        if (kSwapRB) {
            planar = _mm256_setr_epi8(2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15, 2, 6, 10, 14, 1, 5, 9, 13, 0, 4, 8, 12, 3, 7, 11, 15);
        } else {
            planar = _mm256_setr_epi8(0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15);
        }
    ...
    while (count >= 16) {
        __m256i lo = _mm256_loadu_si256((const __m256i*)(src + 0)), hi = _mm256_loadu_si256((const __m256i*)(src + 8));
...
/*not static*/ inline void RGBA_to_BGRA(uint32_t* dst, const uint32_t* src, int count)
{
    const __m256i swapRB = _mm256_setr_epi8(2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15, 2, 1, 0, 3, 6, 5, 4, 7, 10, 9, 8, 11, 14, 13, 12, 15);
...
/*not static*/ inline void grayA_to_RGBA(uint32_t dst[], const uint8_t* src, int count)
{
    while (count >= 16) {
        ...
        __m256i gg = _mm256_or_si256(_mm256_and_si256(ga, _mm256_set1_epi16(0x00FF)), _mm256_slli_epi16(ga, 8));
        ...
        __m256i ggga_lo_shuffle = _mm256_permute2x128_si256(ggga_lo, ggga_hi, 0x20), ggga_hi_shuffle = _mm256_permute2x128_si256(ggga_lo, ggga_hi, 0x31);
...
static void inverted_cmyk_to(Format format, uint32_t* dst, const uint32_t* src, int count)
{
    auto convert8 = [=](__m256i* lo, __m256i* hi) {
        ...
        *hi = _mm256_shuffle_epi8(*hi, planar); // CCCCMMMM YYYYKKKK CCCCMMMM YYYYKKKK
        ...
        __m256i rg = _mm256_or_si256(r, _mm256_slli_epi16(g, 8)), ba = _mm256_or_si256(b, _mm256_set1_epi16((uint16_t)0xFF00));
...
/*not static*/ inline void gray_to_RGB1(uint32_t dst[], const uint8_t* src, int count)
{
    ...
    while (count >= 32) {
        ...
        __m256i ggga0_shuffle = _mm256_permute2x128_si256(ggga0, ggga1, 0x20), ggga1_shuffle = _mm256_permute2x128_si256(ggga2, ggga3, 0x20),
                ggga2_shuffle = _mm256_permute2x128_si256(ggga0, ggga1, 0x31), ggga3_shuffle = _mm256_permute2x128_si256(ggga2, ggga3, 0x31);
...
#if defined(SK_ARM_HAS_NEON)
static void insert_alpha_should_swaprb(bool kSwapRB, uint32_t dst[], const uint8_t* src, int count)
{
    while (count >= 16) {
        // Load 16 pixels.
