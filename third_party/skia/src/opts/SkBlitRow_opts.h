...
static inline __m512i SkPMSrcOver_SKX(const __m512i& src, const __m512i& dst)
{
    ...
    const uint8_t mask[64] = { 3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _, 19, _, 19, _, 23, _, 23, _, 27, _, 27, _, 31, _, 31, _, 35, _, 35, _, 39, _,
        39, _, 43, _, 43, _, 47, _, 47, _, 51, _, 51, _, 55, _, 55, _, 59, _, 59, _, 63, _, 63, _ };
...
#if SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SKX
    while (len >= 16) {
        _mm512_storeu_si512((__m512*)dst, SkPMSrcOver_SKX(_mm512_loadu_si512((const __m512i*)src), _mm512_loadu_si512((const __m512i*)dst)));
        src += 16;
        dst += 16;
        len -= 16;
    }
...
inline void blit_row_color32(SkPMColor* dst, const SkPMColor* src, int count, SkPMColor color)
{
    ...
    auto kernel = [color](U32 src) {
        ...
        U16 c = skvx::cast<uint16_t>(skvx::bit_pun<U8>(U32(color))), d = (mull(s, a) + (c << 8) + 128) >> 8;
