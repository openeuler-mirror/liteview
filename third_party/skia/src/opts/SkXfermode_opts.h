...
/*not static*/ inline SkXfermode* create_xfermode(SkBlendMode)
{
    return nullptr;
}
}
...
XFERMODE(SrcIn)
{
    return s.approxMulDiv255(d.alphas());
}
XFERMODE(SrcOut)
{
    return s.approxMulDiv255(d.alphas().inv());
}
XFERMODE(SrcOver)
{
    return s + d.approxMulDiv255(s.alphas().inv());
}
XFERMODE(DstIn)
{
    return SrcIn()(s, d);
...
XFERMODE(SrcATop)
{
    return (s * d.alphas() + d * s.alphas().inv()).div255();
}
XFERMODE(DstATop)
{
    return SrcATop()(s, d);
...
XFERMODE(Xor)
{
    return (s * d.alphas().inv() + d * s.alphas().inv()).div255();
}
// [S + D ]
XFERMODE(Plus)
{
    return s.saturatedAdd(d);
}
// [S * D ]
XFERMODE(Modulate)
{
    return s.approxMulDiv255(d);
}
// [S + D - S * D]
XFERMODE(Screen)
{
...
#define XFERMODE_AA(Xfermode) template <> inline Sk4px xfer_aa<Xfermode>(const Sk4px& d, const Sk4px& s, const Sk4px& aa)
