...
SI U32 round(F v, F scale)
{
    return _mm256_cvtps_epi32(v * scale);
}
SI U16 pack(U32 v)
{
...
SI void load2(const uint16_t* ptr, size_t tail, U16* r, U16* g)
{
    ...
    *r = _mm_packs_epi32(_mm_srai_epi32(_mm_slli_epi32(_0123, 16), 16), _mm_srai_epi32(_mm_slli_epi32(_4567, 16), 16));
...
SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b)
{
    ...
    auto _02 = _mm_unpacklo_epi16(_0, _2), // r0 r2 g0 g2 b0 b2 xx xx
        _13 = _mm_unpacklo_epi16(_1, _3), _46 = _mm_unpacklo_epi16(_4, _6), _57 = _mm_unpacklo_epi16(_5, _7);
...
SI void load2(const float* ptr, size_t tail, F* r, F* g)
{
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        if (tail > 0) {
            *d = high ? _mm256_insertf128_ps(*d, _mm_loadu_si64(ptr), 1) : _mm256_insertf128_ps(*d, _mm_loadu_si64(ptr), 0);
        }
    } else {
    ...
    F _0145 = _mm256_permute2f128_pd(_0123, _4567, 0x20), _2367 = _mm256_permute2f128_pd(_0123, _4567, 0x31);
...
SI void store2(float* ptr, size_t tail, F r, F g)
{
    ...
    F _0123 = _mm256_permute2f128_pd(_0145, _2367, 0x20), _4567 = _mm256_permute2f128_pd(_0145, _2367, 0x31);
...
SI void load3(const uint16_t* ptr, size_t tail, U16* r, U16* g, U16* b)
{
    ...
    if (__builtin_expect(tail, 0)) {
        ...
        auto _01 = _mm_loadu_si128((const __m128i*)(ptr + 0)), _23 = _mm_srli_si128(_mm_loadu_si128((const __m128i*)(ptr + 4)), 4);
...
{
    F f = fract(x);
    return sk_bit_cast<F>(round(1.0f * (1 << 23), x + 121.274057500f - 1.490129070f * f + 27.728023300f / (4.84252568f - f)));
}
...
#if JUMPER_NARROW_STAGES
#define STAGE(name, ...)                                                                                                                                       \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da);                                      \
    static void ABI name(Params* params, void** program, F r, F g, F b, F a)                                                                                   \
    {                                                                                                                                                          \
        name##_k(Ctx { program }, params->dx, params->dy, params->tail, r, g, b, a, params->dr, params->dg, params->db, params->da);                           \
...
#define STAGE(name, ...)                                                                                                                                       \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& r, F& g, F& b, F& a, F& dr, F& dg, F& db, F& da);                                      \
    static void ABI name(size_t tail, void** program, size_t dx, size_t dy, F r, F g, F b, F a, F dr, F dg, F db, F da)                                        \
    {                                                                                                                                                          \
...
#else
static void ABI stack_checkpoint(size_t tail, void** program, size_t dx, size_t dy, F r, F g, F b, F a, F dr, F dg, F db, F da)
{
    SkRasterPipeline_RewindCtx* ctx = Ctx { program };
    while (program) {
        auto next = (Stage)load_and_inc(program);
...
static void ABI stack_rewind(size_t tail, void** program, size_t dx, size_t dy, F r, F g, F b, F a, F dr, F dg, F db, F da)
{
    SkRasterPipeline_RewindCtx* ctx = Ctx { program };
    sk_unaligned_store(ctx->r, r);
    sk_unaligned_store(ctx->g, g);
...
STAGE(force_opaque, Ctx::None)
{
    a = 1;
}
STAGE(force_opaque_dst, Ctx::None)
{
    da = 1;
}
...
STAGE(rgb_to_hsl, Ctx::None)
{
    ...
    F h = (1 / 6.0f)
        * if_then_else(mx == mn, 0,
            if_then_else(mx == r, (g - b) * d_rcp + if_then_else(g < b, 6.0f, 0), if_then_else(mx == g, (b - r) * d_rcp + 2.0f, (r - g) * d_rcp + 4.0f)));
...
STAGE(PQish, const skcms_TransferFunction* ctx)
{
    auto fn = [&](F v) {
        ...
        F r = approx_powf(max(mad(ctx->b, approx_powf(v, ctx->c), ctx->a), 0) / (mad(ctx->e, approx_powf(v, ctx->c), ctx->d)), ctx->f);
...
STAGE(store_u16_be, const SkRasterPipeline_MemoryCtx* ctx)
{
    ...
    U16 R = bswap(pack(to_unorm(r, 65535))), G = bswap(pack(to_unorm(g, 65535))), B = bswap(pack(to_unorm(b, 65535))), A = bswap(pack(to_unorm(a, 65535)));
...
STAGE(clamp_x_1, Ctx::None)
{
    r = clamp_01_(r);
}
STAGE(repeat_x_1, Ctx::None)
{
...
using Stage = void(ABI*)(size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b, U16 a, U16 dr, U16 dg, U16 db, U16 da);
#endif

static void start_pipeline(const size_t x0, const size_t y0, const size_t xlimit, const size_t ylimit, void** program)
{
    auto start = (Stage)load_and_inc(program);
...
#define STAGE_GP(name, ...)                                                                                                                                    \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F x, F y, U16& r, U16& g, U16& b, U16& a, U16& dr, U16& dg, U16& db, U16& da);            \
    static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a)                                                                           \
    {                                                                                                                                                          \
        auto x = join<F>(r, g), y = join<F>(b, a);                                                                                                             \
        name##_k(Ctx { program }, params->dx, params->dy, params->tail, x, y, r, g, b, a, params->dr, params->dg, params->db, params->da);                     \
        auto next = (Stage)load_and_inc(program);                                                                                                              \
        next(params, program, r, g, b, a);                                                                                                                     \
    }                                                                                                                                                          \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F x, F y, U16& r, U16& g, U16& b, U16& a, U16& dr, U16& dg, U16& db, U16& da)
...
#define STAGE_PP(name, ...)                                                                                                                                    \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, U16& r, U16& g, U16& b, U16& a, U16& dr, U16& dg, U16& db, U16& da);                      \
    static void ABI name(Params* params, void** program, U16 r, U16 g, U16 b, U16 a)                                                                           \
    {                                                                                                                                                          \
        name##_k(Ctx { program }, params->dx, params->dy, params->tail, r, g, b, a, params->dr, params->dg, params->db, params->da);                           \
        auto next = (Stage)load_and_inc(program);                                                                                                              \
...
#define STAGE_GG(name, ...)                                                                                                                                    \
    SI void name##_k(__VA_ARGS__, size_t dx, size_t dy, size_t tail, F& x, F& y);                                                                              \
    static void ABI name(size_t tail, void** program, size_t dx, size_t dy, U16 r, U16 g, U16 b, U16 a, U16 dr, U16 dg, U16 db, U16 da)                        \
    {                                                                                                                                                          \
        auto x = join<F>(r, g), y = join<F>(b, a);                                                                                                             \
...
STAGE_PP(force_opaque, Ctx::None)
{
    a = 255;
}
STAGE_PP(force_opaque_dst, Ctx::None)
{
    da = 255;
}
...
{
    // Exclusive -> inclusive.
    const F w = sk_bit_cast<float>(sk_bit_cast<uint32_t>(ctx->width) - 1), h = sk_bit_cast<float>(sk_bit_cast<uint32_t>(ctx->height) - 1);
...
SI void load_88_(const uint16_t* ptr, size_t tail, U16* r, U16* g)
{
    ...
    switch (tail & (N - 1)) {
    case 0:
        rg = vld2_u8((const uint8_t*)(ptr + 0));
        break;
    case 7:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 6), rg, 6);
        [[fallthrough]];
    case 6:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 5), rg, 5);
        [[fallthrough]];
    case 5:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 4), rg, 4);
        [[fallthrough]];
    case 4:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 3), rg, 3);
        [[fallthrough]];
    case 3:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 2), rg, 2);
        [[fallthrough]];
    case 2:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 1), rg, 1);
        [[fallthrough]];
    case 1:
        rg = vld2_lane_u8((const uint8_t*)(ptr + 0), rg, 0);
    }
    *r = cast<U16>(rg.val[0]);
    *g = cast<U16>(rg.val[1]);
#else
...
SI void store_88_(uint16_t* ptr, size_t tail, U16 r, U16 g)
{
    ...
    switch (tail & (N - 1)) {
    case 0:
        vst2_u8((uint8_t*)(ptr + 0), rg);
        break;
    case 7:
        vst2_lane_u8((uint8_t*)(ptr + 6), rg, 6);
        [[fallthrough]];
    case 6:
        vst2_lane_u8((uint8_t*)(ptr + 5), rg, 5);
        [[fallthrough]];
    case 5:
        vst2_lane_u8((uint8_t*)(ptr + 4), rg, 4);
        [[fallthrough]];
    case 4:
        vst2_lane_u8((uint8_t*)(ptr + 3), rg, 3);
        [[fallthrough]];
    case 3:
        vst2_lane_u8((uint8_t*)(ptr + 2), rg, 2);
        [[fallthrough]];
    case 2:
        vst2_lane_u8((uint8_t*)(ptr + 1), rg, 1);
        [[fallthrough]];
    case 1:
        vst2_lane_u8((uint8_t*)(ptr + 0), rg, 0);
    }
#else
...
{
    U16 mul = load_8(ptr_at_xy<const uint8_t>(&ctx->mul, dx, dy), tail), add = load_8(ptr_at_xy<const uint8_t>(&ctx->add, dx, dy), tail);
...
STAGE_GG(clamp_x_1, Ctx::None)
{
    x = clamp_01_(x);
}
STAGE_GG(repeat_x_1, Ctx::None)
{
...
SI void round_F_to_U16(F R, F G, F B, F A, bool interpolatedInPremul, U16* r, U16* g, U16* b, U16* a)
{
...
SI void gradient_lookup(const SkRasterPipeline_GradientCtx* c, U32 idx, F t, U16* r, U16* g, U16* b, U16* a)
{
    ...
    round_F_to_U16(mad(t, fr, br), mad(t, fg, bg), mad(t, fb, bb), mad(t, fa, ba), c->interpolatedInPremul, r, g, b, a);
}
...
STAGE_GP(evenly_spaced_2_stop_gradient, const SkRasterPipeline_EvenlySpaced2StopGradientCtx* c)
{
    auto t = x;
    round_F_to_U16(
        mad(t, c->f[0], c->b[0]), mad(t, c->f[1], c->b[1]), mad(t, c->f[2], c->b[2]), mad(t, c->f[3], c->b[3]), c->interpolatedInPremul, &r, &g, &b, &a);
}
