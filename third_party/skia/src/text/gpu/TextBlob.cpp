...
public:
    SlugImpl(SubRunAllocator&& alloc, SubRunContainerOwner subRuns, SkRect sourceBounds, const SkPaint& paint, SkPoint origin);
    ~SlugImpl() override = default;

    static sk_sp<SlugImpl> Make(const SkMatrixProvider& viewMatrix, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint,
        const SkPaint& drawingPaint, SkStrikeDeviceInfo strikeDeviceInfo, sktext::StrikeForGPUCacheInterface* strikeCache);
    static sk_sp<Slug> MakeFromBuffer(SkReadBuffer& buffer, const SkStrikeClient* client);
    void doFlatten(SkWriteBuffer& buffer) const override;

#if 1 // SK_SUPPORT_GPU
    void surfaceDraw(SkCanvas*, const GrClip* clip, const SkMatrixProvider& viewMatrix, const SkPaint& paint, skgpu::v1::SurfaceDrawContext* sdc) const;
#endif
    ...
    const SkPaint& initialPaint() const override
    {
        return fInitialPaint;
    }
...
SlugImpl::SlugImpl(SubRunAllocator&& alloc, SubRunContainerOwner subRuns, SkRect sourceBounds, const SkPaint& paint, SkPoint origin)
...
#if 1 // SK_SUPPORT_GPU
void SlugImpl::surfaceDraw(
    SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, const SkPaint& drawingPaint, skgpu::v1::SurfaceDrawContext* sdc) const
{
...
sk_sp<Slug> SlugImpl::MakeFromBuffer(SkReadBuffer& buffer, const SkStrikeClient* client)
{
    ...
    if (!buffer.isValid()) {
        //OutputDebugStringA("SlugImpl::MakeFromBuffer fail\n");
        return nullptr;
    }

    return sk_sp<SlugImpl>(initializer.initialize(std::move(alloc), std::move(container), sourceBounds, paint, origin));
}

sk_sp<SlugImpl> SlugImpl::Make(const SkMatrixProvider& viewMatrix, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint,
    const SkPaint& drawingPaint, SkStrikeDeviceInfo strikeDeviceInfo, sktext::StrikeForGPUCacheInterface* strikeCache)
{
    ...
    const SkMatrix positionMatrix = position_matrix(viewMatrix.localToDevice(), glyphRunList.origin());

    auto subRuns = SubRunContainer::MakeInAlloc(
        glyphRunList, positionMatrix, drawingPaint, strikeDeviceInfo, strikeCache, &alloc, SubRunContainer::kAddSubRuns, "Make Slug");

    sk_sp<SlugImpl> slug
        = sk_sp<SlugImpl>(initializer.initialize(std::move(alloc), std::move(subRuns), glyphRunList.sourceBounds(), initialPaint, glyphRunList.origin()));
    ...
    -> std::tuple<bool, Key>
{
    ...
    if (canCache) {
        ...
        for (auto& run : glyphRunList) {
            ...
            key.fHasSomeDirectSubRuns |= strikeDevice.fSDFTControl->isDirect(approximateDeviceTextSize, paint, drawMatrix);
        }
...
#if SK_SUPPORT_GPU
void TextBlob::draw(
    SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint, skgpu::v1::SurfaceDrawContext* sdc)
{
...
void TextBlob::draw(SkCanvas* canvas, SkPoint drawOrigin, const SkPaint& paint, skgpu::graphite::Device* device)
{
...
namespace skgpu::v1 {
#if SK_SUPPORT_GPU
sk_sp<Slug> Device::convertGlyphRunListToSlug(const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint)
{
    return SlugImpl::Make(this->asMatrixProvider(), glyphRunList, initialPaint, drawingPaint, this->strikeDeviceInfo(), SkStrikeCache::GlobalStrikeCache());
}
...
#endif
    slugImpl->surfaceDraw(canvas, this->clip(), matrixProvider, drawingPaint, fSurfaceDrawContext.get());
}
...
sk_sp<Slug> MakeSlug(const SkMatrixProvider& drawMatrix, const sktext::GlyphRunList& glyphRunList, const SkPaint& initialPaint, const SkPaint& drawingPaint,
    SkStrikeDeviceInfo strikeDeviceInfo, sktext::StrikeForGPUCacheInterface* strikeCache)
{
    return SlugImpl::Make(drawMatrix, glyphRunList, initialPaint, drawingPaint, strikeDeviceInfo, strikeCache);
}
