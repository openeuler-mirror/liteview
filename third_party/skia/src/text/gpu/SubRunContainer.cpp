...
void AtlasSubRun::draw(skgpu::graphite::Device* device, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage) const
{
...
public:
    TransformedMaskVertexFiller(MaskFormat maskFormat, SkScalar strikeToSourceScale, SkRect sourceBounds, SkSpan<const SkPoint> leftTop);

    static TransformedMaskVertexFiller Make(SkRect sourceBounds, MaskFormat maskType, int strikePadding, SkScalar strikeToSourceScale,
        const SkZip<SkGlyphVariant, SkPoint>& accepted, SubRunAllocator* alloc);

    static std::optional<TransformedMaskVertexFiller> MakeFromBuffer(SkReadBuffer& buffer, SubRunAllocator* alloc);
...
#endif // SK_SUPPORT_GPU
#if defined(SK_GRAPHITE_ENABLED)
    SkRect localRect() const
    {
        return fSourceBounds;
    }

    void fillVertexData(
        DrawWriter* dw, int offset, int count, int ssboIndex, SkSpan<const Glyph*> glyphs, SkScalar depth, const skgpu::graphite::Transform& toDevice) const;
    void fillInstanceData(DrawWriter* dw, int offset, int count, unsigned short flags, int ssboIndex, SkSpan<const Glyph*> glyphs, SkScalar depth) const;
#endif
    SkRect deviceRect(const SkMatrix& drawMatrix, SkPoint drawOrigin) const;
    MaskFormat grMaskType() const
    {
        return fMaskType;
    }
    int count() const
...
private:
    ...
    void fill2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& matrix) const;

    template <typename Quad, typename VertexData>
    void fill3D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& matrix) const;
#endif // SK_SUPPORT_GPU
...
TransformedMaskVertexFiller::TransformedMaskVertexFiller(
    MaskFormat maskFormat, SkScalar strikeToSourceScale, SkRect sourceBounds, SkSpan<const SkPoint> leftTop)
...
TransformedMaskVertexFiller TransformedMaskVertexFiller::Make(SkRect sourceBounds, MaskFormat maskType, int strikePadding, SkScalar strikeToSourceScale,
    const SkZip<SkGlyphVariant, SkPoint>& accepted, SubRunAllocator* alloc)
{
    SkSpan<SkPoint> leftTop = alloc->makePODArray<SkPoint>(accepted, [&](auto e) -> SkPoint {
        auto [variant, pos] = e;
        return pos;
    });
...
std::optional<TransformedMaskVertexFiller> TransformedMaskVertexFiller::MakeFromBuffer(SkReadBuffer& buffer, SubRunAllocator* alloc)
{
...
SkRect TransformedMaskVertexFiller::deviceRect(const SkMatrix& drawMatrix, SkPoint drawOrigin) const
{
...
#if SK_SUPPORT_GPU
void TransformedMaskVertexFiller::fillVertexData(
    int offset, int count, SkSpan<const Glyph*> glyphs, GrColor color, const SkMatrix& positionMatrix, SkIRect clip, void* vertexBuffer) const
{
...
template <typename Quad, typename VertexData>
void TransformedMaskVertexFiller::fill2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& positionMatrix) const
{
    for (auto [quad, glyph, leftTop] : quadData) {
        SkPoint widthHeight = SkPoint::Make(glyph->fAtlasLocator.width() * fStrikeToSourceScale, glyph->fAtlasLocator.height() * fStrikeToSourceScale);
        auto [l, t] = leftTop;
        auto [r, b] = leftTop + widthHeight;
        SkPoint lt = positionMatrix.mapXY(l, t), lb = positionMatrix.mapXY(l, b), rt = positionMatrix.mapXY(r, t), rb = positionMatrix.mapXY(r, b);
...
template <typename Quad, typename VertexData>
void TransformedMaskVertexFiller::fill3D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& positionMatrix) const
{
...
#if defined(SK_GRAPHITE_ENABLED)
void TransformedMaskVertexFiller::fillVertexData(
    DrawWriter* dw, int offset, int count, int ssboIndex, SkSpan<const Glyph*> glyphs, SkScalar depth, const Transform& toDevice) const
{
    ...
    for (auto [glyph, leftTop] : quadData()) {
        ...
        verts.append(6) << SkPoint { devOut[0].x, devOut[0].y } << depth << AtlasPt { al, at } // L,T
                        << ssboIndex << SkPoint { devOut[3].x, devOut[3].y } << depth << AtlasPt { al, ab } // L,B
                        << ssboIndex << SkPoint { devOut[1].x, devOut[1].y } << depth << AtlasPt { ar, at } // R,T
                        << ssboIndex << SkPoint { devOut[3].x, devOut[3].y } << depth << AtlasPt { al, ab } // L,B
                        << ssboIndex << SkPoint { devOut[2].x, devOut[2].y } << depth << AtlasPt { ar, ab } // R,B
                        << ssboIndex << SkPoint { devOut[1].x, devOut[1].y } << depth << AtlasPt { ar, at } // R,T
                        << ssboIndex;
    }
}

void TransformedMaskVertexFiller::fillInstanceData(
    DrawWriter* dw, int offset, int count, unsigned short flags, int ssboIndex, SkSpan<const Glyph*> glyphs, SkScalar depth) const
{
    auto quadData = [&]() { return SkMakeZip(glyphs.subspan(offset, count), fLeftTop.subspan(offset, count)); };
    ...
    for (auto [glyph, leftTop] : quadData()) {
        ...
        instances.append(1) << AtlasPt { uint16_t(ar - al), uint16_t(ab - at) } << AtlasPt { uint16_t(al & 0x1fff), at } << leftTop
                            << /*index=*/uint16_t(al >> 13) << flags << fStrikeToSourceScale << depth << ssboIndex;
    }
}
...
public:
    TransformedMaskVertexFiller2(MaskFormat maskFormat, const SkMatrix& creationMatrix, SkRect creationBounds, SkSpan<const SkPoint> leftTop)
        : fMaskType { maskFormat }
    ...
    static TransformedMaskVertexFiller2 Make(
        MaskFormat maskType, const SkMatrix& creationMatrix, SkRect creationBounds, const SkZip<SkGlyphVariant, SkPoint>& accepted, SubRunAllocator* alloc)
    {
        SkSpan<SkPoint> leftTop = alloc->makePODArray<SkPoint>(accepted, [&](auto e) -> SkPoint {
            auto [variant, pos] = e;
            return pos;
    ...
    static std::optional<TransformedMaskVertexFiller2> MakeFromBuffer(SkReadBuffer& buffer, SubRunAllocator* alloc);
    ...
    SkRect creationBounds() const
    {
        return fCreationBounds;
    }
    MaskFormat grMaskType() const
    {
        return fMaskType;
...
private:
    ...
    template <typename Quad, typename VertexData>
    void fill2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& viewDifference) const;

    template <typename Quad, typename VertexData>
    void fill3D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& viewDifference) const;
#endif // SK_SUPPORT_GPU
...
std::optional<TransformedMaskVertexFiller2> TransformedMaskVertexFiller2::MakeFromBuffer(SkReadBuffer& buffer, SubRunAllocator* alloc)
{
    int checkingMaskType = buffer.readInt();
    if (!buffer.validate(0 <= checkingMaskType && checkingMaskType < skgpu::kMaskFormatCount)) {
        return std::nullopt;
...
#if SK_SUPPORT_GPU
void TransformedMaskVertexFiller2::fillVertexData(
    int offset, int count, SkSpan<const Glyph*> glyphs, GrColor color, const SkMatrix& positionMatrix, SkIRect clip, void* vertexBuffer) const
{
    auto quadData = [&](auto dst) { return SkMakeZip(dst, glyphs.subspan(offset, count), fLeftTop.subspan(offset, count)); };
...
template <typename Quad, typename VertexData>
void TransformedMaskVertexFiller2::fill2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& viewDifference) const
{
    for (auto [quad, glyph, leftTop] : quadData) {
        auto [l, t] = leftTop;
...
template <typename Quad, typename VertexData>
void TransformedMaskVertexFiller2::fill3D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& viewDifference) const
{
    auto mapXYZ = [&](SkScalar x, SkScalar y) {
        SkPoint pt { x, y };
        SkPoint3 result;
...
void TransformedMaskVertexFiller2::fillInstanceData(
    DrawWriter* dw, int offset, int count, unsigned short flags, int ssboIndex, SkSpan<const Glyph*> glyphs, SkScalar depth) const
{
    auto quadData = [&]() { return SkMakeZip(glyphs.subspan(offset, count), fLeftTop.subspan(offset, count)); };
...
SkPMColor4f calculate_colors(skgpu::v1::SurfaceDrawContext* sdc, const SkPaint& paint, const SkMatrixProvider& matrix, MaskFormat maskFormat, GrPaint* grPaint)
{
...
public:
    ...
    static PathOpSubmitter Make(const SkZip<SkPackedGlyphID, SkPoint>& accepted, bool isAntiAliased, SkScalar strikeToSourceScale,
...
PathOpSubmitter PathOpSubmitter::Make(
    const SkZip<SkPackedGlyphID, SkPoint>& accepted, bool isAntiAliased, SkScalar strikeToSourceScale, SkStrikePromise&& strikePromise, SubRunAllocator* alloc)
{
    ...
    for (auto [dstIdOrPath, dstPosition, srcPackedGlyphID, srcPosition] : SkMakeZip(idsOrPaths, positions, accepted.get<0>(), accepted.get<1>())) {
        dstPosition = srcPosition;
        dstIdOrPath.fGlyphID = srcPackedGlyphID.glyphID();
    }

    return PathOpSubmitter { isAntiAliased, strikeToSourceScale, SkSpan(positions, glyphCount), SkSpan(idsOrPaths, glyphCount), std::move(strikePromise) };
}
...
public:
    ...
    static SubRunOwner Make(const SkZip<SkPackedGlyphID, SkPoint>& accepted, bool isAntiAliased, SkScalar strikeToSourceScale, SkStrikePromise&& strikePromise,
...
#if SK_SUPPORT_GPU
    void draw(SkCanvas* canvas, const GrClip*, const SkMatrixProvider&, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt>,
...
#endif // SK_SUPPORT_GPU
#if defined(SK_GRAPHITE_ENABLED)
    void draw(SkCanvas* canvas, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage, Device* device) const override
    {
        fPathDrawing.submitDraws(canvas, drawOrigin, paint);
    }
    ...
    static SubRunOwner MakeFromBuffer(const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client);

protected:
    SubRunType subRunType() const override
    {
        return kPath;
    }
...
SubRunOwner PathSubRun::MakeFromBuffer(const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client)
{
...
public:
    ...
    static DrawableOpSubmitter Make(
        const SkZip<SkPackedGlyphID, SkPoint>& accepted, SkScalar strikeToSourceScale, SkStrikePromise&& strikePromise, SubRunAllocator* alloc);
...
DrawableOpSubmitter DrawableOpSubmitter::Make(
    const SkZip<SkPackedGlyphID, SkPoint>& accepted, SkScalar strikeToSourceScale, SkStrikePromise&& strikePromise, SubRunAllocator* alloc)
{
    int glyphCount = SkCount(accepted);
    SkPoint* positions = alloc->makePODArray<SkPoint>(glyphCount);
    ...
    return DrawableOpSubmitter { strikeToSourceScale, SkSpan(positions, glyphCount), SkSpan(idsOrDrawables, glyphCount), std::move(strikePromise) };
}
...
SubRunOwner make_drawable_sub_run(
    const SkZip<SkPackedGlyphID, SkPoint>& drawables, SkScalar strikeToSourceScale, SkStrikePromise&& strikePromise, SubRunAllocator* alloc)
{
    return alloc->makeUnique<SubRunT>(DrawableOpSubmitter::Make(drawables, strikeToSourceScale, std::move(strikePromise), alloc));
}
...
public:
    ...
    static SubRunOwner MakeFromBuffer(const SkMatrix&, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client);
#if SK_SUPPORT_GPU
    void draw(SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage,
...
SubRunOwner DrawableSubRun::MakeFromBuffer(const SkMatrix&, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client)
{
...
template <typename Rect> auto ltbr(const Rect& r)
{
...
template <typename Quad, typename VertexData>
void generalized_direct_2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, SkPoint originOffset, SkIRect* clip = nullptr)
{
    for (auto [quad, glyph, leftTop] : quadData) {
        auto [al, at, ar, ab] = glyph->fAtlasLocator.getUVs();
...
void direct_2D(SkZip<Mask2DVertex[4], const Glyph*, const SkPoint> quadData, GrColor color, SkPoint originOffset)
{
    for (auto [quad, glyph, leftTop] : quadData) {
        auto [al, at, ar, ab] = glyph->fAtlasLocator.getUVs();
...
public:
    DirectMaskSubRun(
        MaskFormat format, const SkMatrix& initialPositionMatrix, SkRect deviceBounds, SkSpan<const SkPoint> devicePositions, GlyphVector&& glyphs);

    static SubRunOwner Make(SkRect runBounds, const SkZip<SkGlyphVariant, SkPoint>& accepted, const SkMatrix& initialPositionMatrix,
        SkStrikePromise&& strikePromise, MaskFormat format, SubRunAllocator* alloc);

    static SubRunOwner MakeFromBuffer(const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client);
#if SK_SUPPORT_GPU
    void draw(SkCanvas*, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunOwner,
        skgpu::v1::SurfaceDrawContext* sdc) const override;
#endif // SK_SUPPORT_GPU

#ifdef SK_GRAPHITE_ENABLED
    void draw(SkCanvas*, SkPoint drawOrigin, const SkPaint&, sk_sp<SkRefCnt> subRunStorage, Device*) const override;
#endif
    ...
    std::tuple<const GrClip*, GrOp::Owner> makeAtlasTextOp(const GrClip*, const SkMatrixProvider& viewMatrix, SkPoint, const SkPaint&,
        sk_sp<SkRefCnt>&& subRunStorage, skgpu::v1::SurfaceDrawContext*) const override;
    ...
    std::tuple<Rect, Transform> boundsAndDeviceMatrix(const Transform&, SkPoint drawOrigin) const override;
    ...
    void fillVertexData(DrawWriter*, int offset, int count, int ssboIndex, SkScalar depth, const Transform& transform) const override;
    void fillInstanceData(skgpu::graphite::DrawWriter*, int offset, int count, int ssboIndex, SkScalar depth) const override;
#endif
...
SubRunOwner DirectMaskSubRun::Make(SkRect runBounds, const SkZip<SkGlyphVariant, SkPoint>& accepted, const SkMatrix& initialPositionMatrix,
    SkStrikePromise&& strikePromise, MaskFormat format, SubRunAllocator* alloc)
{
    ...
    return alloc->makeUnique<DirectMaskSubRun>(
        format, initialPositionMatrix, runBounds, leftTop, GlyphVector { std::move(strikePromise), { glyphIDs, accepted.size() } });
}
...
SubRunOwner DirectMaskSubRun::MakeFromBuffer(const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client)
{
    ...
    if (!buffer.validate(SkCount(glyphVector->glyphs()) == glyphCount)) {
        return nullptr;
    }
    SkASSERT(buffer.isValid());
    return alloc->makeUnique<DirectMaskSubRun>(maskType, initialPositionMatrix, runBounds, leftTop, std::move(glyphVector.value()));
}
...
{
    return sizeof(DirectMaskSubRun) + fGlyphs.unflattenSize() + sizeof(SkPoint) * fGlyphs.glyphs().size();
}
...
void DirectMaskSubRun::draw(SkCanvas*, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint,
    sk_sp<SkRefCnt> subRunStorage, skgpu::v1::SurfaceDrawContext* sdc) const
{
    auto [drawingClip, op] = this->makeAtlasTextOp(clip, viewMatrix, drawOrigin, paint, std::move(subRunStorage), sdc);
    if (op != nullptr) {
        sdc->addDrawOp(drawingClip, std::move(op));
    }
}

std::tuple<const GrClip*, GrOp::Owner> DirectMaskSubRun::makeAtlasTextOp(const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin,
    const SkPaint& paint, sk_sp<SkRefCnt>&& subRunStorage, skgpu::v1::SurfaceDrawContext* sdc) const
{
    ...
    auto geometry = AtlasTextOp::Geometry::Make(*this, drawMatrix, drawOrigin, geometricClipRect, std::move(subRunStorage), drawingColor, sdc->arenaAlloc());

    GrRecordingContext* const rContext = sdc->recordingContext();
    GrOp::Owner op
        = GrOp::Make<AtlasTextOp>(rContext, op_mask_type(fMaskFormat), !integerTranslate, this->glyphCount(), subRunDeviceBounds, geometry, std::move(grPaint));
    return { clip, std::move(op) };
}
#endif // SK_SUPPORT_GPU

#ifdef SK_GRAPHITE_ENABLED
void DirectMaskSubRun::draw(SkCanvas*, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage, Device* device) const
{
...
template <typename Quad, typename VertexData>
void transformed_direct_2D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& matrix)
{
    for (auto [quad, glyph, leftTop] : quadData) {
        auto [al, at, ar, ab] = glyph->fAtlasLocator.getUVs();
        SkScalar dl = leftTop.x(), dt = leftTop.y(), dr = dl + (ar - al), db = dt + (ab - at);
        SkPoint lt = matrix.mapXY(dl, dt), lb = matrix.mapXY(dl, db), rt = matrix.mapXY(dr, dt), rb = matrix.mapXY(dr, db);
        quad[0] = { lt, color, { al, at } }; // L,T
        quad[1] = { lb, color, { al, ab } }; // L,B
        quad[2] = { rt, color, { ar, at } }; // R,T
        quad[3] = { rb, color, { ar, ab } }; // R,B
...
template <typename Quad, typename VertexData>
void transformed_direct_3D(SkZip<Quad, const Glyph*, const VertexData> quadData, GrColor color, const SkMatrix& matrix)
{
    auto mapXYZ = [&](SkScalar x, SkScalar y) {
        SkPoint pt { x, y };
        SkPoint3 result;
    ...
    for (auto [quad, glyph, leftTop] : quadData) {
        auto [al, at, ar, ab] = glyph->fAtlasLocator.getUVs();
        SkScalar dl = leftTop.x(), dt = leftTop.y(), dr = dl + (ar - al), db = dt + (ab - at);
        SkPoint3 lt = mapXYZ(dl, dt), lb = mapXYZ(dl, db), rt = mapXYZ(dr, dt), rb = mapXYZ(dr, db);
        quad[0] = { lt, color, { al, at } }; // L,T
        quad[1] = { lb, color, { al, ab } }; // L,B
        quad[2] = { rt, color, { ar, at } }; // R,T
        quad[3] = { rb, color, { ar, ab } }; // R,B
...
void DirectMaskSubRun::fillVertexData(void* vertexDst, int offset, int count, GrColor color, const SkMatrix& drawMatrix, SkPoint drawOrigin, SkIRect clip) const
{
    auto quadData = [&](auto dst) { return SkMakeZip(dst, fGlyphs.glyphs().subspan(offset, count), fLeftTopDevicePos.subspan(offset, count)); };

    const SkMatrix positionMatrix = position_matrix(drawMatrix, drawOrigin);
    auto [noTransformNeeded, originOffset] = can_use_direct(fInitialPositionMatrix, positionMatrix);
...
#if defined(SK_GRAPHITE_ENABLED)
std::tuple<bool, int> DirectMaskSubRun::regenerateAtlas(int begin, int end, Recorder* recorder) const
{
    return fGlyphs.regenerateAtlas(begin, end, fMaskFormat, 0, recorder);
}

std::tuple<Rect, Transform> DirectMaskSubRun::boundsAndDeviceMatrix(const Transform& localToDevice, SkPoint drawOrigin) const
{
    ...
    const bool compatibleMatrix = positionMatrix.rc(0, 0) == fInitialPositionMatrix.rc(0, 0) && positionMatrix.rc(0, 1) == fInitialPositionMatrix.rc(0, 1)
        && positionMatrix.rc(1, 0) == fInitialPositionMatrix.rc(1, 0) && positionMatrix.rc(1, 1) == fInitialPositionMatrix.rc(1, 1)
        && localToDevice.type() != Transform::Type::kProjection && !fInitialPositionMatrix.hasPerspective();

    if (compatibleMatrix) {
        const SkV4 mappedOrigin = positionMatrix.map(drawOrigin.x(), drawOrigin.y(), 0.f, 1.f);
        const SkV2 offset = { mappedOrigin.x - fInitialPositionMatrix.getTranslateX(), mappedOrigin.y - fInitialPositionMatrix.getTranslateY() };
        if (SkScalarIsInt(offset.x) && SkScalarIsInt(offset.y)) {
            ...
            return { Rect(fGlyphDeviceBounds), Transform(SkM44::Translate(SkScalarRoundToInt(offset.x), SkScalarRoundToInt(offset.y))) };
        }
    }
    ...
    return { Rect(fGlyphDeviceBounds), localToDevice.preTranslate(drawOrigin.x(), drawOrigin.y()).concatInverse(SkM44(fInitialPositionMatrix)) };
}

template <typename VertexData> void direct_dw(DrawWriter* dw, int ssboIndex, SkZip<const Glyph*, const VertexData> quadData, SkScalar depth)
{
    DrawWriter::Vertices verts { *dw };
...
void transformed_direct_dw(DrawWriter* dw, int ssboIndex, SkZip<const Glyph*, const VertexData> quadData, SkScalar depth, const Transform& transform)
{
    DrawWriter::Vertices verts { *dw };
    verts.reserve(6 * quadData.size());
    for (auto [glyph, leftTop] : quadData) {
...
void DirectMaskSubRun::fillVertexData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth, const Transform& toDevice) const
{
    auto quadData = [&]() { return SkMakeZip(fGlyphs.glyphs().subspan(offset, count), fLeftTopDevicePos.subspan(offset, count)); };
...
void DirectMaskSubRun::fillInstanceData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth) const
{
    auto quadData = [&]() { return SkMakeZip(fGlyphs.glyphs().subspan(offset, count), fLeftTopDevicePos.subspan(offset, count)); };
...
std::tuple<bool, SkRect> DirectMaskSubRun::deviceRectAndCheckTransform(const SkMatrix& positionMatrix) const
{
    ...
    const bool compatibleMatrix = positionMatrix[0] == initialMatrix[0] && positionMatrix[1] == initialMatrix[1] && positionMatrix[3] == initialMatrix[3]
        && positionMatrix[4] == initialMatrix[4] && !positionMatrix.hasPerspective() && !initialMatrix.hasPerspective();
...
public:
    TransformedMaskSubRun(const SkMatrix& initialPositionMatrix, TransformedMaskVertexFiller2&& vertexFiller, GlyphVector&& glyphs)
        : fInitialPositionMatrix { initialPositionMatrix }
    ...
    static SubRunOwner Make(const SkZip<SkGlyphVariant, SkPoint>& accepted, const SkMatrix& initialPositionMatrix, SkStrikePromise&& strikePromise,
        SkMatrix creationMatrix, SkRect creationBounds, MaskFormat maskType, SubRunAllocator* alloc)
    {
        auto vertexFiller = TransformedMaskVertexFiller2::Make(maskType, creationMatrix, creationBounds, accepted, alloc);

        auto glyphVector = GlyphVector::Make(std::move(strikePromise), accepted.get<0>(), alloc);

        return alloc->makeUnique<TransformedMaskSubRun>(initialPositionMatrix, std::move(vertexFiller), std::move(glyphVector));
    }
    ...
    void draw(SkCanvas*, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage,
        skgpu::v1::SurfaceDrawContext* sdc) const override
    {
        auto [drawingClip, op] = this->makeAtlasTextOp(clip, viewMatrix, drawOrigin, paint, std::move(subRunStorage), sdc);
        if (op != nullptr) {
    ...
    std::tuple<const GrClip*, GrOp::Owner> makeAtlasTextOp(const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint,
        sk_sp<SkRefCnt>&& subRunStorage, skgpu::v1::SurfaceDrawContext* sdc) const override
    {
        SkASSERT(this->glyphCount() != 0);
        ...
        auto geometry
            = AtlasTextOp::Geometry::Make(*this, drawMatrix, drawOrigin, SkIRect::MakeEmpty(), std::move(subRunStorage), drawingColor, sdc->arenaAlloc());

        GrRecordingContext* const rContext = sdc->recordingContext();
        SkMatrix positionMatrix = position_matrix(drawMatrix, drawOrigin);
        GrOp::Owner op = GrOp::Make<AtlasTextOp>(
            rContext, fVertexFiller.opMaskType(), true, this->glyphCount(), this->deviceRect(positionMatrix), geometry, std::move(grPaint));
        return { clip, std::move(op) };
    }

    std::tuple<bool, int> regenerateAtlas(int begin, int end, GrMeshDrawTarget* target) const override
    {
    ...
    void draw(SkCanvas*, SkPoint drawOrigin, const SkPaint& paint, sk_sp<SkRefCnt> subRunStorage, Device* device) const override
    {
        this->AtlasSubRun::draw(device, drawOrigin, paint, std::move(subRunStorage));
    }
    ...
    std::tuple<Rect, Transform> boundsAndDeviceMatrix(const Transform& localToDevice, SkPoint drawOrigin) const override
    {
        const SkMatrix viewDifference = fVertexFiller.viewDifference(localToDevice.preTranslate(drawOrigin.x(), drawOrigin.y()));
    ...
    void fillVertexData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth, const skgpu::graphite::Transform& transform) const override
    {
        fVertexFiller.fillVertexData(dw, offset, count, ssboIndex, fGlyphs.glyphs(), depth, transform);
    }

    void fillInstanceData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth) const override
    {
        unsigned short flags = (unsigned short)fVertexFiller.grMaskType();
        fVertexFiller.fillInstanceData(dw, offset, count, flags, ssboIndex, fGlyphs.glyphs(), depth);
    }
...
static std::tuple<AtlasTextOp::MaskType, uint32_t, bool> calculate_sdf_parameters(
    const skgpu::v1::SurfaceDrawContext& sdc, const SkMatrix& drawMatrix, bool useLCDText, bool isAntiAliased)
{
    const GrColorInfo& colorInfo = sdc.colorInfo();
    const SkSurfaceProps& props = sdc.surfaceProps();
    ...
    MT maskType = !isAntiAliased ? MT::kAliasedDistanceField : isLCD ? (isBGR ? MT::kLCDBGRDistanceField : MT::kLCDDistanceField) : MT::kGrayscaleDistanceField;
...
public:
    SDFTSubRun(bool useLCDText, bool antiAliased, const SDFTMatrixRange& matrixRange, TransformedMaskVertexFiller&& vertexFiller, GlyphVector&& glyphs)
    ...
    static SubRunOwner Make(const SkZip<SkGlyphVariant, SkPoint>& accepted, const SkFont& runFont, SkStrikePromise&& strikePromise, SkRect sourceBounds,
        SkScalar strikeToSourceScale, const SDFTMatrixRange& matrixRange, SubRunAllocator* alloc)
    {
        auto vertexFiller = TransformedMaskVertexFiller::Make(sourceBounds, MaskFormat::kA8, SK_DistanceFieldInset, strikeToSourceScale, accepted, alloc);
        ...
        sk_sp<SkRefCnt>&& subRunStorage, skgpu::v1::SurfaceDrawContext* sdc) const override
    {
        ...
        auto [maskType, DFGPFlags, useGammaCorrectDistanceTable] = calculate_sdf_parameters(*sdc, drawMatrix, fUseLCDText, fAntiAliased);

        auto geometry
            = AtlasTextOp::Geometry::Make(*this, drawMatrix, drawOrigin, SkIRect::MakeEmpty(), std::move(subRunStorage), drawingColor, sdc->arenaAlloc());

        GrRecordingContext* const rContext = sdc->recordingContext();
        GrOp::Owner op = GrOp::Make<AtlasTextOp>(rContext, maskType, true, this->glyphCount(), this->deviceRect(drawMatrix, drawOrigin),
            SkPaintPriv::ComputeLuminanceColor(paint), useGammaCorrectDistanceTable, DFGPFlags, geometry, std::move(grPaint));
    ...
    std::tuple<bool, int> regenerateAtlas(int begin, int end, Recorder* recorder) const override
    {
        return fGlyphs.regenerateAtlas(begin, end, MaskFormat::kA8, SK_DistanceFieldInset, recorder);
    }

    std::tuple<Rect, Transform> boundsAndDeviceMatrix(const Transform& localToDevice, SkPoint drawOrigin) const override
    {
        return { Rect(fVertexFiller.localRect()), localToDevice.preTranslate(drawOrigin.x(), drawOrigin.y()) };
    }
    ...
    void fillVertexData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth, const Transform& transform) const override
    {
        fVertexFiller.fillVertexData(dw, offset, count, ssboIndex, fGlyphs.glyphs(), depth, transform);
    }
    ...
    void fillInstanceData(DrawWriter* dw, int offset, int count, int ssboIndex, SkScalar depth) const override
    {
        fVertexFiller.fillInstanceData(dw, offset, count, /*flags=*/0, ssboIndex, fGlyphs.glyphs(), depth);
    }
...
void add_multi_mask_format(AddSingleMaskFormat addSingleMaskFormat, const SkZip<SkGlyphVariant, SkPoint, SkMask::Format>& accepted, sk_sp<SkStrike>&& strike)
{
    if (accepted.empty()) {
        return;
    }
...
SubRunOwner SubRun::MakeFromBuffer(const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, SubRunAllocator* alloc, const SkStrikeClient* client)
{
    using Maker = SubRunOwner (*)(const SkMatrix&, SkReadBuffer&, SubRunAllocator*, const SkStrikeClient*);
...
SubRunContainerOwner SubRunContainer::MakeFromBufferInAlloc(SkReadBuffer& buffer, const SkStrikeClient* client, SubRunAllocator* alloc)
{
    ...
    for (int i = 0; i < subRunCount; ++i) {
        auto subRunOwner = SubRun::MakeFromBuffer(container->initialPosition(), buffer, alloc, client);
    ...
    const char* tag)
{
    ...
    if constexpr (kTrace) {
        ...
        msg.appendf("\n   matrix\n");
        msg.appendf("   %7.3g %7.3g %7.3g\n   %7.3g %7.3g %7.3g\n", positionMatrix[0], positionMatrix[1], positionMatrix[2], positionMatrix[3],
            positionMatrix[4], positionMatrix[5]);
    }
    ...
    // in place.
    auto convertToGlyphIDs = [&](SkZip<SkGlyphVariant, SkPoint> good) -> SkZip<SkPackedGlyphID, SkPoint> {
    ...
    for (auto& glyphRun : glyphRunList) {
        ...
        if ((runPaint.getStyle() != SkPaint::kStroke_Style || runPaint.getStrokeWidth() != 0)) {
            ...
            if (SDFTControl.isSDFT(approximateDeviceTextSize, runPaint, positionMatrix)) {
                ...
                const auto& [strikeSpec, strikeToSourceScale, matrixRange]
                    = SkStrikeSpec::MakeSDFT(runFont, runPaint, deviceProps, positionMatrix, glyphRunListLocation, SDFTControl);
                ...
                if (!SkScalarNearlyZero(strikeToSourceScale)) {
                    ...
                    SkRect sourceBounds = strike->prepareForSDFTDrawing(strikeToSourceScale, accepted, rejected);
                    rejected->flipRejectsToSource();

                    if (creationBehavior == kAddSubRuns && !accepted->empty()) {
                        container->fSubRuns.append(
                            SDFTSubRun::Make(accepted->accepted(), runFont, strike->strikePromise(), sourceBounds, strikeToSourceScale, matrixRange, alloc));
                    }
                }
            }
            ...
            if (!rejected->source().empty() && !positionMatrix.hasPerspective()) {
                ...
                if (creationBehavior == kAddSubRuns && !accepted->empty()) {
                    auto addGlyphsWithSameFormat = [&](const SkZip<SkGlyphVariant, SkPoint>& acceptedGlyphsAndLocations, MaskFormat format,
                                                       sk_sp<SkStrike>&& runStrike) {
                        container->fSubRuns.append(
                            DirectMaskSubRun::Make(bounds, acceptedGlyphsAndLocations, container->initialPosition(), strike->strikePromise(), format, alloc));
                    };
                    add_multi_mask_format(addGlyphsWithSameFormat, accepted->acceptedWithMaskFormat(), strike->getUnderlyingStrike());
                }
            }
        }
        ...
        if (!rejected->source().empty()) {
            ...
            if (!SkScalarNearlyZero(strikeToSourceScale)) {
                ...
                if (creationBehavior == kAddSubRuns && !accepted->empty()) {
                    container->fSubRuns.append(
                        make_drawable_sub_run<DrawableSubRun>(convertToGlyphIDs(accepted->accepted()), strikeToSourceScale, strike->strikePromise(), alloc));
                }
            }
        }
        if (!rejected->source().empty()) {
            ...
            if (!SkScalarNearlyZero(strikeToSourceScale)) {
                ...
                if (creationBehavior == kAddSubRuns && !accepted->empty()) {
                    container->fSubRuns.append(PathSubRun::Make(
                        convertToGlyphIDs(accepted->accepted()), has_some_antialiasing(runFont), strikeToSourceScale, strike->strikePromise(), alloc));
                }
            }
        }
        ...
        if (!rejected->source().empty()) {
            ...
            auto maxGlyphDimension = [&](const SkMatrix& m) {
                ...
                const ScopedStrikeForGPU gaugingStrike = strikeSpec.findOrCreateScopedStrike(strikeCache);
            ...
            for (SkScalar maxDimension = maxGlyphDimension(creationMatrix); 0 >= maxDimension || maxDimension > kMaxBilerpAtlasDimension;
            ...
            if (creationBehavior == kAddSubRuns && !accepted->empty()) {
                auto addGlyphsWithSameFormat
                    = [&](const SkZip<SkGlyphVariant, SkPoint>& acceptedGlyphsAndLocations, MaskFormat format, sk_sp<SkStrike>&& runStrike) {
                          container->fSubRuns.append(TransformedMaskSubRun::Make(acceptedGlyphsAndLocations, container->initialPosition(),
                              strike->strikePromise(), creationMatrix, creationBounds, format, alloc));
                      };
                add_multi_mask_format(addGlyphsWithSameFormat, accepted->acceptedWithMaskFormat(), strike->getUnderlyingStrike());
            }
        }
...
#if SK_SUPPORT_GPU
void SubRunContainer::draw(SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint,
    const SkRefCnt* subRunStorage, skgpu::v1::SurfaceDrawContext* sdc) const
{
    for (auto& subRun : fSubRuns) {
...
#if defined(SK_GRAPHITE_ENABLED)
void SubRunContainer::draw(SkCanvas* canvas, SkPoint drawOrigin, const SkPaint& paint, const SkRefCnt* subRunStorage, skgpu::graphite::Device* device) const
{
    for (auto& subRun : fSubRuns) {
