...
public:
    ...
    virtual std::tuple<const GrClip*, GrOp::Owner> makeAtlasTextOp(const GrClip*, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint&,
        sk_sp<SkRefCnt>&& subRunStorage, skgpu::v1::SurfaceDrawContext*) const
        = 0;
    ...
    // is single threaded.
    virtual std::tuple<bool, int> regenerateAtlas(int begin, int end, GrMeshDrawTarget* target) const = 0;
#endif

#if defined(SK_GRAPHITE_ENABLED)
    virtual std::tuple<bool, int> regenerateAtlas(int begin, int end, skgpu::graphite::Recorder*) const = 0;
    ...
    virtual const skgpu::graphite::Renderer* renderer(const skgpu::graphite::RendererProvider*) const = 0;

    virtual void fillVertexData(
        skgpu::graphite::DrawWriter*, int offset, int count, int ssboIndex, SkScalar depth, const skgpu::graphite::Transform& transform) const
        = 0;
    virtual void fillInstanceData(skgpu::graphite::DrawWriter*, int offset, int count, int ssboIndex, SkScalar depth) const = 0;
#endif
...
protected:
#if defined(SK_GRAPHITE_ENABLED)
    void draw(skgpu::graphite::Device*, SkPoint drawOrigin, const SkPaint&, sk_sp<SkRefCnt> subRunStorage) const;
#endif
};
...
public:
    ...
    virtual void draw(SkCanvas*, const GrClip*, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint&, sk_sp<SkRefCnt> subRunStorage,
        skgpu::v1::SurfaceDrawContext*) const
        = 0;
#endif
#if defined(SK_GRAPHITE_ENABLED)
    // Produce uploads and draws for this subRun
    virtual void draw(SkCanvas*, SkPoint drawOrigin, const SkPaint&, sk_sp<SkRefCnt> subRunStorage, skgpu::graphite::Device*) const = 0;
#endif

    void flatten(SkWriteBuffer& buffer) const;
    static SubRunOwner MakeFromBuffer(
        const SkMatrix& initialPositionMatrix, SkReadBuffer& buffer, sktext::gpu::SubRunAllocator* alloc, const SkStrikeClient* client);
...
public:
    ...
    static SK_WARN_UNUSED_RESULT SubRunContainerOwner MakeInAlloc(const GlyphRunList& glyphRunList, const SkMatrix& positionMatrix, const SkPaint& runPaint,
...
#if 1 // SK_SUPPORT_GPU
    void draw(SkCanvas* canvas, const GrClip* clip, const SkMatrixProvider& viewMatrix, SkPoint drawOrigin, const SkPaint& paint, const SkRefCnt* subRunStorage,
        skgpu::v1::SurfaceDrawContext* sdc) const;
#endif
#if 1 // def SK_GRAPHITE_ENABLED
    void draw(SkCanvas*, SkPoint drawOrigin, const SkPaint&, const SkRefCnt* subRunStorage, skgpu::graphite::Device*) const;
#endif
