...
class VisualRun {
public:
    VisualRun(TextRange textRange, GlyphIndex trailingSpacesStart, const SkFont& font, SkScalar lineBaseLine, SkPoint runOffset, bool leftToRight,
    ...
    SkScalar firstGlyphPosition() const
    {
        return fPositions[0].fX;
    }
    ...
    {
        return fDirTextRange.fLeftToRight;
    }
    size_t size() const
    {
        return fGlyphs.size();
    ...
    {
        return fLineBaseLine;
    }
    GlyphIndex trailingSpacesStart() const
    {
        return fTrailingSpacesStart;
    }
    DirTextRange dirTextRange() const
    {
        return fDirTextRange;
    }

    template <typename Callback> void forEachTextBlockInGlyphRange(SkSpan<TextIndex> textBlock, Callback&& callback) const
    {
        if (this->leftToRight()) {
...
public:
    ...
    GlyphRange trailingSpaces() const
    {
        return fTrailingSpaces;
    }
    ...
    bool isFirst(VisualRun* run)
    {
        return &fRuns.front() == run;
    }
    bool isLast(VisualRun* run)
    {
        return &fRuns.back() == run;
    }
