...
void UnicodeText::initialize(SkSpan<uint16_t> utf16)
{
    ...
    if (!fUnicode->computeCodeUnitFlags((char16_t*)utf16.data(), utf16.size(), false, &fCodeUnitProperties)) {
        return;
    }
}
...
std::unique_ptr<FontResolvedText> UnicodeText::resolveFonts(SkSpan<FontBlock> blocks)
{
    ...
    for (auto& block : blocks) {
        ...
        while (adjustedBlock.fEnd < this->fText16.size() && !this->hasProperty(adjustedBlock.fEnd, SkUnicode::CodeUnitFlags::kGraphemeStart)) {
            ++adjustedBlock.fEnd;
        }
    ...
    std::sort(fontResolvedText->fResolvedFonts.begin(), fontResolvedText->fResolvedFonts.end(),
        [](const ResolvedFontBlock& a, const ResolvedFontBlock& b) { return a.textRange.fStart < b.textRange.fStart; });
    /*
        SkDebugf("Resolved:\n");
    ...
    for (auto fontIndex = 0; fontIndex < fontChain.count(); ++fontIndex) {
        ...
        while (!unresolvedTexts.empty()) {
            ...
            unicodeText->forEachGrapheme(unresolvedText, [&](TextRange grapheme) {
                auto count
                    = typeface->textToGlyphs(unicodeText->getText16().data() + grapheme.fStart, grapheme.width() * 2, SkTextEncoding::kUTF16, nullptr, 0);
...
public:
    FormattingFontIterator(TextIndex textCount, SkSpan<ResolvedFontBlock> fontBlocks, SkSpan<TextIndex> marks)
...
std::unique_ptr<ShapedText> FontResolvedText::shape(UnicodeText* unicodeText, TextDirection textDirection)
{
    ...
    unicodeText->getUnicode()->forEachCodepoint(text8.c_str(), text8.size(), [&](SkUnichar unichar, int32_t start, int32_t end, int32_t count) {
    ...
    FormattingFontIterator fontIter(
        text8.size(), SkSpan<ResolvedFontBlock>(fontBlocks8.data(), fontBlocks8.size()), SkSpan<TextIndex>(&formattingMarks[0], formattingMarks.size()));
    SkShaper::TrivialLanguageRunIterator langIter(text8.c_str(), text8.size());
    std::unique_ptr<SkShaper::BiDiRunIterator> bidiIter(
        SkShaper::MakeSkUnicodeBidiRunIterator(unicodeText->getUnicode(), text8.c_str(), text8.size(), textDirection == TextDirection::kLtr ? 0 : 1));
    std::unique_ptr<SkShaper::ScriptRunIterator> scriptIter(SkShaper::MakeSkUnicodeHbScriptRunIterator(text8.c_str(), text8.size()));
    auto shaper = SkShaper::MakeShapeDontWrapOrReorder();
    if (shaper == nullptr) {
        // For instance, loadICU does not work. We have to stop the process
        return nullptr;
    }
    shaper->shape(text8.c_str(), text8.size(), fontIter, *bidiIter, *scriptIter, langIter, std::numeric_limits<SkScalar>::max(), shapedText.get());
    if (shapedText->fLogicalRuns.empty()) {
        // Create a fake run for an empty text (to avoid all the checks)
        SkShaper::RunHandler::RunInfo emptyInfo { fontIter.createFont(fResolvedFonts.front()), 0, SkVector::Make(0.0f, 0.0f), 0,
    ...
    for (auto& logicalRun : shapedText->fLogicalRuns) {
        ...
        logicalRun.convertClusterIndexes([&](TextIndex clusterIndex8) { return UTF16FromUTF8[clusterIndex8]; });
        // Detect and mark line break runs
        if (logicalRun.getTextRange().width() == 1 && logicalRun.size() == 1 && unicodeText->isHardLineBreak(logicalRun.getTextRange().fStart)) {
...
void ShapedText::addLine(WrappedText* wrappedText, SkUnicode* unicode, Stretch& stretch, Stretch& spaces, bool hardLineBreak)
{
    ...
    for (auto visualIndex : visualOrder) {
        ...
        if (glyphSpaces > glyphStart) {
            ...
            wrappedText->fVisualRuns.emplace_back(TextRange(textStart, textEnd), glyphSpaces - glyphStart, logicalRun.fFont,
                lineStretch.textMetrics().baseline(), SkPoint::Make(runOffsetInLine, wrappedText->fActualSize.fHeight), logicalRun.leftToRight(),
                SkSpan<SkPoint>(&logicalRun.fPositions[glyphStart], glyphSize + 1), SkSpan<SkGlyphID>(&logicalRun.fGlyphs[glyphStart], glyphSize),
...
void WrappedText::visit(Visitor* visitor) const
{
    ...
    for (auto& line : fVisualLines) {
        ...
        for (auto& run : line.fRuns) {
            ...
            visitor->onGlyphRun(run.fFont, run.dirTextRange(), boundingRect, run.trailingSpacesStart(), run.size(), run.fGlyphs.data(), run.fPositions.data(),
                run.fClusters.data());
            glyphCount += run.size();
        }
        ...
        for (auto& run : fVisualRuns) {
            run.forEachTextBlockInGlyphRange(lineBlocks, [&](DirTextRange dirTextRange) {
                ...
                SkRect boundingRect = SkRect::MakeXYWH(
                    line.fOffset.fX + run.fPositions[glyphRange.fStart].fX, line.fOffset.fY + diff, run.calculateWidth(glyphRange), run.fTextMetrics.height());
                visitor->onGlyphRun(run.fFont, dirTextRange, boundingRect, run.trailingSpacesStart(), glyphRange.width(), &run.fGlyphs[glyphRange.fStart],
                    &run.fPositions[glyphRange.fStart], &run.fClusters[glyphRange.fStart]);
            });
            ++runIndex;
            glyphCount += run.size();
        }
...
GlyphRange WrappedText::textToGlyphs(UnicodeText* unicodeText, PositionType positionType, RunIndex runIndex, DirTextRange dirTextRange) const
{
    ...
    for (GlyphIndex glyph = 0; glyph < run.size(); ++glyph) {
        ...
        if (positionType == PositionType::kGraphemeCluster && unicodeText->hasProperty(textIndex, SkUnicode::CodeUnitFlags::kGraphemeStart)) {
...
void SelectableText::onGlyphRun(const SkFont& font, DirTextRange dirTextRange, SkRect bounds, TextIndex trailingSpaces, size_t glyphCount,
    const uint16_t glyphs[], const SkPoint positions[], const TextIndex clusters[])
{
    auto& line = fBoxLines.back();
