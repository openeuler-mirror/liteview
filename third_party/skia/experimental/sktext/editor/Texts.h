...
class DynamicText {
public:
    DynamicText(std::u16string text, SkPoint offset, SkSize size, SkSpan<FontBlock> fontBlocks, SkSpan<DecoratedBlock> decorations, TextDirection textDirection,
        TextAlign textAlign)
    {
        fOffset = offset;
        fRequiredSize = size;
    ...
    {
        return fSelectableText->countLines();
    }
...
public:
    EditableText(std::u16string text, SkPoint offset, SkSize size, SkSpan<FontBlock> fontBlocks, SkSpan<DecoratedBlock> decorations,
    ...
    {
        return fSelectableText->nextPosition(current);
    }
    ...
    {
        return fSelectableText->isLastOnTheLine(element);
    }
    ...
    void select(TextRange text, SkRect boundaries)
    {
        fSelection->select(text, boundaries);
    }
    void clearSelection()
    {
        fSelection->clear();
