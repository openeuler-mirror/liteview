    ...
    , fInsertMode(true)
{
    ...
    fMouse = std::make_unique<Mouse>();
    {
        ...
        fEditableText = std::make_unique<EditableText>(text, SkPoint::Make(0, 0), textSize, SkSpan<FontBlock>(&textBlock, 1),
    ...
    }
    {
        ...
        fStatus = std::make_unique<DynamicText>(status, statusPoint, SkSize::Make(size.width(), SK_ScalarInfinity), SkSpan<FontBlock>(&statusBlock, 1),
            SkSpan<DecoratedBlock>(&statusDecor, 1), DEFAULT_TEXT_DIRECTION, TextAlign::kCenter);
    }
