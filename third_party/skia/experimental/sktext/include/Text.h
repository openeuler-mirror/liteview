...
public:
    ...
    */
    ...
    template <typename Callback> void forEachGrapheme(TextRange textRange, Callback&& callback)
    {
...
public:
    ...
    */
    template <class Drawable> std::unique_ptr<Drawable> prepareToDraw(UnicodeText* unicodeText, PositionType positionType, SkSpan<TextIndex> blocks) const
    {
    ...
    */
    ...
    SkSize actualSize() const
    {
        return fActualSize;
    }
    size_t countLines() const
    {
        return fVisualLines.size();
    }
...
private:
    ...
    WrappedText()
        : fActualSize(SkSize::MakeEmpty())
        , fAligned(TextAlign::kNothing)
    {
    }
...
public:
    ...
    void onGlyphRun(const SkFont& font, DirTextRange dirTextRange, SkRect bounds, TextIndex trailingSpaces, size_t glyphCount, const uint16_t glyphs[],
        const SkPoint positions[], const TextIndex clusters[]) override
    {
