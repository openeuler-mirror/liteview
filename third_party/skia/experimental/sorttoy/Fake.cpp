...
FakeMCBlob::FakeMCBlob(const std::vector<MCState>& stack)
    : fID(NextID())
    , fStack(stack)
{
...
static SkColor blend(float t, SkColor c0, SkColor c1)
{
    ...
    SkColor4f result
        = { t * bot.fR + (1.0f - t) * top.fR, t * bot.fG + (1.0f - t) * top.fG, t * bot.fB + (1.0f - t) * top.fB, t * bot.fA + (1.0f - t) * top.fA };
...
void FakeDevice::sort()
{
    ...
    std::sort(fSortedCmds.begin(), fSortedCmds.end(), [](const sk_sp<Cmd>& a, const sk_sp<Cmd>& b) { return a->getKey() < b->getKey(); });
}
