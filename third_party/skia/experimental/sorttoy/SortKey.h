...
public:
    ...
    SortKey()
        : fKey((kMaxDepth - 1) << kMaterialShift)
    {
    }
    explicit SortKey(bool transparent, uint32_t depth, uint32_t material)
    {
        ...
        if (transparent) {
            fKey = (0x1 << kTransparentShift) | (depth & kDepthMask) << kDepthShift | (material & kMaterialMask) << kMaterialShift;
        } else {
            ...
            fKey = (munged & kDepthMask) << kMaterialShift | (material & kMaterialMask) << kDepthShift;
        }
    }
    ...
    void dump() const
    {
        SkDebugf("transparent: %d depth: %d mat: %d\n", this->transparent(), this->depth(), this->material());
    }
