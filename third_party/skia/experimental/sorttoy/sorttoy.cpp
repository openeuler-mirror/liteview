...
static void check_state(FakeMCBlob* actualState, SkIPoint expectedCTM, const std::vector<SkIRect>& expectedClips)
{
...
static void mcstack_test()
{
    ...
    const std::vector<SkIRect> expectedS1Clips { r.makeOffset(s1Trans) };
    const std::vector<SkIRect> expectedS2aClips { r.makeOffset(s1Trans), r.makeOffset(s2TransA) };
    const std::vector<SkIRect> expectedS2bClips { r.makeOffset(s1Trans), r.makeOffset(s2TransA), r.makeOffset(s2TransA + s2TransB) };
...
static void check_order(int testID, const std::vector<ID>& actualOrder, const std::vector<ID>& expectedOrder)
{
    if (expectedOrder.size() != actualOrder.size()) {
        exitf("Op count mismatch in test %d. Expected %zu - got %zu\n", testID, expectedOrder.size(), actualOrder.size());
    }
...
typedef int (*PFTest)(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder);
...
static int test1(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
...
static int test2(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
    // front-to-back order bc all opaque
...
static int test3(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
...
static int test4(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
...
static int test5(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
...
static int test6(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
...
static int test7(std::vector<sk_sp<Cmd>>* test, Shape shape, std::vector<ID>* expectedOrder)
{
