...
public:
    ...
    public:
        ...
        const std::vector<sk_sp<ClipCmd>>& cmds() const
        {
            return fCmds;
        }
    ...
    const std::vector<MCState>& mcStates() const
    {
        return fStack;
    }
    const MCState& operator[](int index) const
    {
        return fStack[index];
    }
    SkIRect scissor() const
    {
        return fScissor;
    }
...
protected:
private:
    enum class Type { kNormal, kLinear, kRadial };
