...
using SkUnicodeBidi = std::unique_ptr<UBiDi, SkFunctionWrapper<decltype(ubidi_close), ubidi_close_wrapper>>;
using ICUUText = std::unique_ptr<UText, SkFunctionWrapper<decltype(utext_close), utext_close_wrapper>>;
using ICUBreakIterator = std::unique_ptr<UBreakIterator, SkFunctionWrapper<decltype(ubrk_close), ubrk_close_wrapper>>;
...
public:
    explicit SkBidiIterator_icu(SkUnicodeBidi bidi)
        : fBidi(std::move(bidi))
    {
    }
    Position getLength() override
    {
        return sk_ubidi_getLength(fBidi.get());
    }
    Level getLevelAt(Position pos) override
    {
        return sk_ubidi_getLevelAt(fBidi.get(), pos);
    }
...
public:
    ...
    ICUBreakIterator makeBreakIterator(SkUnicode::BreakType type)
    {
        ...
        ICUBreakIterator* cachedIterator;
        {
            ...
            if (!cachedIterator) {
                ICUBreakIterator newIterator(sk_ubrk_open(convertType(type), sk_uloc_getDefault(), nullptr, 0, &status));
                if (U_FAILURE(status)) {
                    SkDEBUGF("Break error: %s", sk_u_errorName(status));
                } else {
...
class SkUnicode_icu : public SkUnicode {
    static bool extractBidi(const char utf8[], int utf8Units, TextDirection dir, std::vector<BidiRegion>* bidiRegions)
    {
        ...
        // The required lifetime of utf16 isn't well documented.
        // It appears it isn't used after ubidi_setPara except through ubidi_getText.
        sk_ubidi_setPara(bidi.get(), (const UChar*)utf16.c_str(), utf16.size(), bidiLevel, nullptr, &status);
        if (U_FAILURE(status)) {
            SkDEBUGF("Bidi error: %s", sk_u_errorName(status));
            return false;
        }
...
public:
    ...
    SkString toUpper(const SkString& str) override
    {
        ...
        sk_u_strToUpper((UChar*)(upper16.get()), SkToS32(upper16.size()), (UChar*)(str16.c_str()), str16.size(), nullptr, &icu_err);
    ...
    bool computeCodeUnitFlags(char utf8[], int utf8Units, bool replaceTabs, SkTArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        ...
        SkUnicode_icu::extractPositions(utf8, utf8Units, BreakType::kGraphemes, [&](int pos, int status) { (*results)[pos] |= CodeUnitFlags::kGraphemeStart; });
    ...
    bool computeCodeUnitFlags(char16_t utf16[], int utf16Units, bool replaceTabs, SkTArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        ...
        // Get white spaces
        this->forEachCodepoint((char16_t*)&utf16[0], utf16Units, [results, replaceTabs, &utf16](SkUnichar unichar, int32_t start, int32_t end) {
