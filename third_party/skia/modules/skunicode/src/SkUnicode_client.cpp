...
public:
    ...
    SkUnicode_client(SkSpan<char> text, std::vector<SkUnicode::BidiRegion> bidiRegions, std::vector<SkUnicode::Position> words,
    ...
    {
        std::u16string whitespaces = u"\u0009" // character tabulation
                                     "\u000A" // line feed
                                     "\u000B" // line tabulation
                                     "\u000C" // form feed
                                     "\u000D" // carriage return
    ...
    {
        std::u16string spaces = u"\u0009" // character tabulation
                                "\u000A" // line feed
                                "\u000B" // line tabulation
                                "\u000C" // form feed
                                "\u000D" // carriage return
...
public:
    explicit SkBidiIterator_client(SkUnicode_client* unicode)
        : fUnicode(unicode)
    {
    }
    Position getLength() override
    {
        return fUnicode->fBidiRegions.size();
    }
    Level getLevelAt(Position pos) override
    {
        auto found = std::lower_bound(fUnicode->fBidiRegions.begin(), fUnicode->fBidiRegions.end(), SkUnicode::BidiRegion(pos, pos, 0),
...
public:
    explicit SkBreakIterator_client(SkUnicode_client* unicode)
        : fUnicode(unicode)
    {
    }
    ...
    {
        return fUnicode->fLineBreaks[fStart + fLastResult].breakType == SkUnicode::LineBreakType::kHardLineBreak
    ...
    {
        SkASSERT(utftext8 >= fUnicode->fText8.data() && utf8Units <= SkToS16(fUnicode->fText8.size()));
    ...
    {
        SkASSERT(utftext16 >= fUnicode->fText16.data() && utf16Units <= SkToS16(fUnicode->fText16.size()));
...
std::unique_ptr<SkUnicode> SkUnicode::Make(SkSpan<char> text, std::vector<SkUnicode::BidiRegion> bidiRegions, std::vector<SkUnicode::Position> words,
    std::vector<SkUnicode::Position> graphemeBreaks, std::vector<SkUnicode::LineBreakBefore> lineBreaks)
{
