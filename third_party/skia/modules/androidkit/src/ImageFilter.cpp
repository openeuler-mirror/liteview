...
static long ImageFilter_Blur(JNIEnv* env, jobject, jfloat sigmaX, jfloat sigmaY, jint jTileMode, jlong native_input)
{
    auto input = sk_ref_sp(reinterpret_cast<SkImageFilter*>(native_input));
    auto filter = SkImageFilters::Blur(sigmaX, sigmaY, androidkit::utils::TileMode(jTileMode), std::move(input));
    return reinterpret_cast<jlong>(filter.release());
}
