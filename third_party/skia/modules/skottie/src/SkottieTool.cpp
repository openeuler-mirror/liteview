...
private:
    GPUGenerator(FrameSink* sink, const SkMatrix& matrix)
        : FrameGenerator(sink)
    {
        fCtx = fFactory.getContextInfo(sk_gpu_test::GrContextFactory::kGL_ContextType).directContext();
        fSurface = SkSurface::MakeRenderTarget(
            fCtx, SkBudgeted::kNo, SkImageInfo::MakeN32Premul(FLAGS_width, FLAGS_height), 0, GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, nullptr);
        if (fSurface) {
...
int main(int argc, char** argv)
{
    ...
    auto rp = skresources::CachingResourceProvider::Make(
        skresources::DataURIResourceProviderProxy::Make(skresources::FileResourceProvider::Make(SkOSPath::Dirname(FLAGS_input[0]),
                                                            /*predecode=*/true),
            /*predecode=*/true));
    ...
    auto anim = skottie::Animation::Builder().setLogger(logger).setResourceProvider(rp).make(static_cast<const char*>(data->data()), data->size());
    if (!anim) {
