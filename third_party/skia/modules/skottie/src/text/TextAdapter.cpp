...
public:
    explicit GlyphTextNode(skottie::Shaper::ShapedGlyphs&& glyphs)
...
sk_sp<TextAdapter> TextAdapter::Make(const skjson::ObjectValue& jlayer, const AnimationBuilder* abuilder, sk_sp<SkFontMgr> fontmgr,
    sk_sp<CustomFont::GlyphCompMapper> custom_glyph_mapper, sk_sp<Logger> logger)
{
    ...
    auto adapter = sk_sp<TextAdapter>(new TextAdapter(std::move(fontmgr), std::move(custom_glyph_mapper), std::move(logger), gGroupingMap[SkToSizeT(apg - 1)]));
...
TextAdapter::TextAdapter(sk_sp<SkFontMgr> fontmgr, sk_sp<CustomFont::GlyphCompMapper> custom_glyph_mapper, sk_sp<Logger> logger, AnchorPointGrouping apg)
...
std::vector<sk_sp<sksg::RenderNode>> TextAdapter::buildGlyphCompNodes(Shaper::Fragment& frag) const
{
        ...
        for (const auto& run : frag.fGlyphs.fRuns) {
            for (size_t i = 0; i < run.fSize; ++i) {
                ...
                if (auto gcomp = fCustomGlyphMapper->getGlyphComp(run.fFont.getTypeface(), gid)) {
                    ...
                    const auto m = SkMatrix::Translate(frag.fGlyphs.fGlyphPos[offset + i]) * SkMatrix::Scale(fText->fTextSize, fText->fTextSize);
