...
std::unique_ptr<SkShaper, std::default_delete<class SkShaper>> SkShaper::Make(sk_sp<SkFontMgr>)
{
    *(int*)1 = 1;
    return nullptr;
}
...
public:
    ...
    Buffer runBuffer(const RunInfo& info) override
    {
        ...
        fLineGlyphs.reallocMem(fLineGlyphCount);
        fLinePos.reallocMem(fLineGlyphCount);
        fLineClusters.reallocMem(fLineGlyphCount);
...
private:
    ...
    void commitConsolidatedRun(const skottie::Shaper::RunRec& run, const SkGlyphID* glyphs, const SkPoint* pos, const uint32_t*, uint32_t)
    {
...
Shaper::Result Shaper::Shape(const SkString& orig_txt, const TextDesc& desc, const SkPoint& point, const sk_sp<SkFontMgr>& fontmgr)
{
...
Shaper::Result Shaper::Shape(const SkString& orig_txt, const TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr)
{
    const AdjustedText txt(orig_txt, desc);
