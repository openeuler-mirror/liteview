...
AnimationBuilder::AnimationBuilder(sk_sp<ResourceProvider> rp, sk_sp<SkFontMgr> fontmgr, sk_sp<PropertyObserver> pobserver, sk_sp<Logger> logger,
    sk_sp<MarkerObserver> mobserver, sk_sp<PrecompInterceptor> pi, sk_sp<ExpressionManager> expressionmgr, Animation::Builder::Stats* stats,
...
sk_sp<Animation> Animation::Builder::make(const char* data, size_t data_len)
{
    ...
    internal::AnimationBuilder builder(std::move(resolvedProvider), fFontMgr, std::move(fPropertyObserver), std::move(fLogger), std::move(fMarkerObserver),
        std::move(fPrecompInterceptor), std::move(fExpressionManager), &fStats, size, duration, fps, fFlags);
    ...
    return sk_sp<Animation>(
        new Animation(std::move(ainfo.fScene), std::move(ainfo.fAnimators), std::move(version), size, inPoint, outPoint, duration, fps, flags));
}
...
Animation::Animation(std::unique_ptr<sksg::Scene> scene, std::vector<sk_sp<internal::Animator>>&& animators, SkString version, const SkSize& size,
