...
DEF_TEST(Skottie_Properties, reporter)
{
    ...
    public:
        FakeFontMgr(sk_sp<SkTypeface> test_font)
            : fTestFont(test_font)
        {
        }
    ...
    REPORTER_ASSERT(reporter,
        texts[0].handle->get()
            == skottie::TextPropertyValue(
                { test_typeface, SkString("inline_text"), 100, 0, 100, 0, 120, 12, 0, 0, SkTextUtils::kLeft_Align, Shaper::VAlign::kTopBaseline,
                    Shaper::ResizePolicy::kNone, Shaper::LinebreakPolicy::kExplicit, Shaper::Direction::kLTR, Shaper::Capitalization::kNone,
                    SkRect::MakeEmpty(), SK_ColorTRANSPARENT, SK_ColorTRANSPARENT, TextPaintOrder::kFillStroke, SkPaint::Join::kDefault_Join, false, false }));
}
...
DEF_TEST(Skottie_Shaper_VAlign, reporter)
{
    ...
    for (const auto& tsize : kTestSizes) {
        for (const auto& talign : kTestAligns) {
            ...
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.top() - expected_t) < tsize.tolerance, "%f %f %f %f %d", shape_bounds.top(), expected_t,
                tsize.tolerance, tsize.text_size, SkToU32(talign.align));

            const auto expected_b = text_box.bottom() - v_diff * (1 - talign.topFactor);
            REPORTER_ASSERT(reporter, std::fabs(shape_bounds.bottom() - expected_b) < tsize.tolerance, "%f %f %f %f %d", shape_bounds.bottom(), expected_b,
                tsize.tolerance, tsize.text_size, SkToU32(talign.align));
        }
    }
}
...
DEF_TEST(Skottie_Shaper_ExplicitFontMgr, reporter)
{
...

    {
        ...
        const auto shape_result = skottie::Shaper::Shape(SkString("foo\U000DFFFFbar"), desc, text_box, fontmgr);
