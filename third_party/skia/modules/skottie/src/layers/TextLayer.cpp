...
bool AnimationBuilder::resolveNativeTypefaces()
{
    ...
    fFonts.foreach ([&](const SkString& name, FontInfo* finfo) {
        ...
        if (!finfo->fTypeface) {
            finfo->fTypeface = fmgr->makeFromData(fResourceProvider->loadFont(name.c_str(), finfo->fPath.c_str()));
        }

        if (!finfo->fTypeface) {
            finfo->fTypeface.reset(fmgr->matchFamilyStyle(finfo->fFamily.c_str(), FontStyle(this, finfo->fStyle.c_str())));

            if (!finfo->fTypeface) {
                this->log(Logger::Level::kError, nullptr, "Could not create typeface for %s|%s.", finfo->fFamily.c_str(), finfo->fStyle.c_str());
                // Last resort.
                finfo->fTypeface = fmgr->legacyMakeTypeface(nullptr, FontStyle(this, finfo->fStyle.c_str()));
...
{
    return this->attachDiscardableAdapter<TextAdapter>(jlayer, this, fLazyFontMgr.getMaybeNull(), fCustomGlyphMapper, fLogger);
}
