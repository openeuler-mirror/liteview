...
// #if !defined(NDEBUG) && !defined(__clang__) && defined(_M_IX86) || defined(__i386__)
...
#ifdef SK_DEBUG
    std::vector<Node*>::const_iterator it = node->fInvalObserverArray->begin();
    for (; it != node->fInvalObserverArray->end(); ++it) {
        if (*it == this) {
            SkASSERT(false);
        }
    }
