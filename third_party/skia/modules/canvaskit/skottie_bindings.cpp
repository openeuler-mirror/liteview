...
public:
    static sk_sp<ManagedAnimation> Make(const std::string& json, sk_sp<skottie::ResourceProvider> rp, std::string prop_prefix, emscripten::val logger)
    {
        auto mgr = std::make_unique<skottie_utils::CustomPropertyManager>(
            skottie_utils::CustomPropertyManager::Mode::kCollapseProperties, prop_prefix.empty() ? nullptr : prop_prefix.c_str());
        ...
        return animation ? sk_sp<ManagedAnimation>(new ManagedAnimation(std::move(animation), std::move(mgr))) : nullptr;
    }
...
private:
    ManagedAnimation(sk_sp<skottie::Animation> animation, std::unique_ptr<skottie_utils::CustomPropertyManager> propMgr)
    ...
    function("_MakeManagedAnimation",
        optional_override([](std::string json, size_t assetCount, WASMPointerU32 nptr, WASMPointerU32 dptr, WASMPointerU32 sptr, std::string prop_prefix,
            ...
            return ManagedAnimation::Make(json,
                skresources::DataURIResourceProviderProxy::Make(SkottieAssetProvider::Make(std::move(assets), std::move(soundMap))), prop_prefix,
                std::move(logger));
        }));
