...
SI F min_(F x, F y)
{
    return (F)vminq_f16((float16x8_t)x, (float16x8_t)y);
}
SI F max_(F x, F y)
{
    return (F)vmaxq_f16((float16x8_t)x, (float16x8_t)y);
}
#elif defined(USING_NEON)
SI F min_(F x, F y)
{
...
static void exec_ops(const Op* ops, const void** args, const char* src, char* dst, int i)
{
    ...
    while (true) {
        ...
        case Op_store_8888: {
            store(dst + 4 * i,
                cast<U32>(to_fixed(r * 255)) << 0 | cast<U32>(to_fixed(g * 255)) << 8 | cast<U32>(to_fixed(b * 255)) << 16
        ...
        case Op_store_1010102: {
            store(dst + 4 * i,
                cast<U32>(to_fixed(r * 1023)) << 0 | cast<U32>(to_fixed(g * 1023)) << 10 | cast<U32>(to_fixed(b * 1023)) << 20
...
static void run_program(const Op* program, const void** arguments, const char* src, char* dst, int n, const size_t src_bpp, const size_t dst_bpp)
{
    int i = 0;
    while (n >= N) {
