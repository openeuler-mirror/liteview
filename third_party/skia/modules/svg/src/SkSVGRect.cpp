...
SkRRect SkSVGRect::resolve(const SkSVGLengthContext& lctx) const
{
    ...
    //
    auto radii = [this]() {
        return fRx.isValid() ? fRy.isValid() ? std::make_tuple(*fRx, *fRy) : std::make_tuple(*fRx, *fRx)
            : fRy.isValid() ? std::make_tuple(*fRy, *fRy)
                             : std::make_tuple(SkSVGLength(0), SkSVGLength(0));
    };

    const auto [rxlen, rylen] = radii();
    const auto rx = std::min(lctx.resolve(rxlen, SkSVGLengthContext::LengthType::kHorizontal), rect.width() / 2),
               ry = std::min(lctx.resolve(rylen, SkSVGLengthContext::LengthType::kVertical), rect.height() / 2);
