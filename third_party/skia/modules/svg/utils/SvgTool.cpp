...
int main(int argc, char** argv)
{
    ...
    auto rp = skresources::DataURIResourceProviderProxy::Make(skresources::FileResourceProvider::Make(SkOSPath::Dirname(FLAGS_input[0]),
                                                                  /*predecode=*/true),
        /*predecode=*/true);

    auto svg_dom = SkSVGDOM::Builder().setFontManager(SkFontMgr::RefDefault()).setResourceProvider(std::move(rp)).make(in);
    if (!svg_dom) {
