...
ParagraphBuilderImpl::ParagraphBuilderImpl(const ParagraphStyle& style, sk_sp<FontCollection> fontCollection, std::unique_ptr<SkUnicode> unicode)
...
std::unique_ptr<Paragraph> ParagraphBuilderImpl::BuildWithClientInfo(std::vector<SkUnicode::BidiRegion> bidiRegionsUtf16,
    std::vector<SkUnicode::Position> wordsUtf16, std::vector<SkUnicode::Position> graphemeBreaksUtf16, std::vector<SkUnicode::LineBreakBefore> lineBreaksUtf16)
{
    ...
    SkUnicode::extractUtfConversionMapping(
        text, [&](size_t index) { utf8IndexForUtf16Index.emplace_back(index); }, [&](size_t index) {});
    ...
    for (SkUnicode::BidiRegion bidiRegionUtf16 : bidiRegionsUtf16) {
        bidiRegionsUtf8.emplace_back(
            SkUnicode::BidiRegion(utf8IndexForUtf16Index[bidiRegionUtf16.start], utf8IndexForUtf16Index[bidiRegionUtf16.end], bidiRegionUtf16.level));
    }
    ...
    for (SkUnicode::LineBreakBefore lineBreakUtf16 : lineBreaksUtf16) {
        lineBreaksUtf8.emplace_back(SkUnicode::LineBreakBefore(utf8IndexForUtf16Index[lineBreakUtf16.pos], lineBreakUtf16.breakType));
    }

    utf8IndexForUtf16Index.reset();

    // This is the place where SkUnicode is paired with SkParagraph
    fUnicode = SkUnicode::Make(text, std::move(bidiRegionsUtf8), std::move(wordsUtf8), std::move(graphemeBreaksUtf8), std::move(lineBreaksUtf8));
    return this->Build();
}
