...
void TextLine::paint(SkCanvas* textCanvas, SkScalar x, SkScalar y)
{
    if (fHasBackground) {
        this->iterateThroughVisualRuns(
            false, [textCanvas, x, y, this](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kBackground,
    ...
    if (fHasShadows) {
        this->iterateThroughVisualRuns(
            false, [textCanvas, x, y, this](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kShadow,
                    [textCanvas, x, y, this](TextRange textRange, const TextStyle& style, const ClipContext& context) {
    ...
    if (fHasDecorations) {
        this->iterateThroughVisualRuns(
            false, [textCanvas, x, y, this](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kDecorations,
                    [textCanvas, x, y, this](TextRange textRange, const TextStyle& style, const ClipContext& context) {
...
void TextLine::ensureTextBlobCachePopulated()
{
    ...
    if (fBlockRange.width() == 1 && fRunsInVisualOrder.size() == 1 && fEllipsis == nullptr
        ...
        this->iterateThroughVisualRuns(false, [this](const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
            ...
            *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kForeground,
...
void TextLine::scanStyles(StyleType styleType, const RunStyleVisitor& visitor)
{
    ...
    this->iterateThroughVisualRuns(false, [this, visitor, styleType](const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
        *width = this->iterateThroughSingleRunByStyles(run, runOffset, textRange, styleType,
...
SkScalar TextLine::metricsWithoutMultiplier(TextHeightBehavior correction)
{
    ...
    this->iterateThroughVisualRuns(true, [&result](const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
...
void TextLine::paintShadow(SkCanvas* canvas, SkScalar x, SkScalar y, TextRange textRange, const TextStyle& style, const ClipContext& context) const
{
    SkScalar correctedBaseline = SkScalarFloorToScalar(this->baseline() + style.getBaselineShift() + 0.5);
    ...
    for (TextShadow shadow : style.getShadows()) {
        ...
        if (shadow.fBlurSigma != 0.0) {
            auto filter = SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, SkDoubleToScalar(shadow.fBlurSigma), false);
        ...
        canvas->drawTextBlob(
            blob, x + this->offset().fX + shadow.fOffset.x() + context.fTextShift, y + this->offset().fY + shadow.fOffset.y() + correctedBaseline, paint);
        if (context.clippingNeeded) {
            canvas->restore();
        }
    }
...
std::unique_ptr<Run> TextLine::shapeEllipsis(const SkString& ellipsis, const Cluster* cluster)
{
    ...
    auto shaped = [&](sk_sp<SkTypeface> typeface, bool fallback) -> std::unique_ptr<Run> {
        ...
        std::unique_ptr<SkShaper> shaper = SkShaper::MakeShapeDontWrapOrReorder(fallback ? SkFontMgr::RefDefault() : SkFontMgr::RefEmpty());
        shaper->shape(ellipsis.c_str(), ellipsis.size(), font, true, std::numeric_limits<SkScalar>::max(), &handler);
...
TextLine::ClipContext TextLine::measureTextInsideOneRun(
    TextRange textRange, const Run* run, SkScalar runOffsetInLine, SkScalar textOffsetInRunInLine, bool includeGhostSpaces, bool limitToGraphemes) const
{
...
SkScalar TextLine::iterateThroughSingleRunByStyles(
    const Run* run, SkScalar runOffset, TextRange textRange, StyleType styleType, const RunStyleVisitor& visitor) const
{

    auto correctContext = [&](TextRange textRange, SkScalar textOffsetInRun) -> ClipContext {
        auto result = this->measureTextInsideOneRun(textRange, run, runOffset, textOffsetInRun, false, true);
...
void TextLine::getRectsForRange(TextRange textRange0, RectHeightStyle rectHeightStyle, RectWidthStyle rectWidthStyle, std::vector<TextBox>& boxes)
{
    ...
    this->iterateThroughVisualRuns(true,
        [textRange0, rectHeightStyle, rectWidthStyle, &boxes, &lastRun, startBox, this](
            ...
            *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kNone,
        ...
        *runWidthInLine = this->iterateThroughSingleRunByStyles(run, runOffsetInLine, textRange, StyleType::kNone,
            [this, run, dx, &result, &keepLooking](TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context0) {
                ...
                if ((dx < center) == context.run->leftToRight()) {
                    size_t utf16Index = context.run->leftToRight() ? fOwner->getUTF16Index(clusterIndex8) : fOwner->getUTF16Index(clusterEnd8) + 1;
                    result = { SkToS32(utf16Index), kDownstream };
                } else {
