        ...
        while (!fUnresolvedBlocks.empty()) {
            ...
            while (true) {
                ...
                if (found != nullptr) {
                    ...
                    typeface = fParagraph->fFontCollection->defaultFallback(unicode, textStyle.getFontStyle(), textStyle.getLocale());
...
bool OneLineShaper::iterateThroughShapingRegions(const ShapeVisitor& shape)
{
    ...
    for (auto& placeholder : fParagraph->fPlaceholders) {
        ...
        auto& run = fParagraph->fRuns.emplace_back(this->fParagraph, runInfo, placeholder.fRange.start, 0.0f, 0.0f, false, fParagraph->fRuns.count(), advanceX);
    ...
    auto result = iterateThroughShapingRegions(
        [this, limitlessWidth](TextRange textRange, SkSpan<Block> styleSpan, SkScalar& advanceX, TextIndex textStart, uint8_t defaultBidiLevel) {
            ...
            iterateThroughFontStyles(
                textRange, styleSpan, [this, &shaper, defaultBidiLevel, limitlessWidth, &advanceX](Block block, SkTArray<SkShaper::Feature> features) {
                    ...
                    this->matchResolvedFonts(block.fStyle, [&](sk_sp<SkTypeface> typeface) {
                        ...
                        while (unresolvedCount-- > 0) {
                            ...
                            auto scriptIter = SkShaper::MakeSkUnicodeHbScriptRunIterator(unresolvedText.begin(), unresolvedText.size());
