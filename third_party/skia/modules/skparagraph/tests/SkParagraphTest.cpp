...
public:
    ...
    SkUnicode_test(SkSpan<char> text, const ParagraphStyle& style)
    {
        ...
        fIcu->getBidiRegions(text.data(), text.size(),
            style.getTextDirection() == skia::textlayout::TextDirection::kLtr ? TextDirection::kLTR : TextDirection::kRTL, &bidiRegions);
        ...
        fClient->computeCodeUnitFlags(text.data(), text.size(), style.getReplaceTabCharacters(), &codeUnitFlags1);
        ...
        for (auto i = 0ul; i < codeUnitFlags.size(); ++i) {
            ...
            if ((codeUnitFlags1[i] & CodeUnitFlags::kTabulation) == CodeUnitFlags::kTabulation) {
                SkASSERT((codeUnitFlags[i] | CodeUnitFlags::kTabulation) == (codeUnitFlags1[i] | CodeUnitFlags::kControl));
            } else {
    ...
    bool computeCodeUnitFlags(char utf8[], int utf8Units, bool replaceTabs, SkTArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
        return fClient->computeCodeUnitFlags(utf8, utf8Units, replaceTabs, results);
    }

    bool computeCodeUnitFlags(char16_t utf16[], int utf16Units, bool replaceTabs, SkTArray<SkUnicode::CodeUnitFlags, true>* results) override
    {
...
public:
    SkBidiIterator_test(SkUnicode* icu, SkUnicode* client, const char text[], int count, SkBidiIterator::Direction dir)
        : fIcuIter(icu->makeBidiIterator(text, count, dir))
        , fClientIter(client->makeBidiIterator(text, count, dir))
    {
    }
    SkBidiIterator_test(SkUnicode* icu, SkUnicode* client, const uint16_t text[], int count, SkBidiIterator::Direction dir)
        : fIcuIter(icu->makeBidiIterator(text, count, dir))
        , fClientIter(client->makeBidiIterator(text, count, dir))
    {
    }
...
public:
    ...
    explicit SkBreakIterator_test(SkUnicode* icu, SkUnicode* client, const char locale[], SkUnicode::BreakType breakType)
        : fIcuIter(icu->makeBreakIterator(breakType))
...
std::unique_ptr<SkBidiIterator> SkUnicode_test::makeBidiIterator(const uint16_t text[], int count, SkBidiIterator::Direction dir)
{
    return std::make_unique<SkBidiIterator_test>(fIcu.get(), fClient.get(), text, count, dir);
}
std::unique_ptr<SkBidiIterator> SkUnicode_test::makeBidiIterator(const char text[], int count, SkBidiIterator::Direction dir)
{
    return std::make_unique<SkBidiIterator_test>(fIcu.get(), fClient.get(), text, count, dir);
}
std::unique_ptr<SkBreakIterator> SkUnicode_test::makeBreakIterator(const char locale[], BreakType breakType)
{
...
public:
    ...
    {
        this->SetUnicode(std::make_unique<SkUnicode_test>(this->getText(), this->getParagraphStyle()));
...
UNIX_ONLY_TEST(SkParagraph_GetRectsForRangeTight, reporter)
{
    ...
    S   S    S     S   S G  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG
    G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GGG  G G
    G  G  G  GGG  G G  G  G  G  GGG  G G  G  G  G  GG W                  W                  W                  W                  W                  W                  W W W W W W
    W                  W                  W                  W                  W                  W                  W                  W
...
UNIX_ONLY_TEST(SkParagraph_Shaping, reporter)
{
    ...
    sk_sp<TestFontCollection> fontCollection = sk_make_sp<TestFontCollection>(dir, /*GetResourcePath("fonts").c_str(), */ false);
    if (!fontCollection->fontsFound())
        return;
