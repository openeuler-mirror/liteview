...
public:
    ...
    template <typename T, bool MEM_MOVE> void visit(const char* name, SkTArray<T, MEM_MOVE>& arr)
    {
    ...
    template <typename T> void visit(const char* name, sk_sp<T>& obj)
    {
...
protected:
    ...
    struct ArrayEdit {
        ...
        template <typename T, bool MEM_MOVE> void apply(SkTArray<T, MEM_MOVE>& arr) const
        {
            switch (fVerb) {
            case Verb::kNone:
                break;
