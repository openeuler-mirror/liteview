...
public:
    SkPathExternalFunction(const char* name, SkSL::Compiler& compiler, const LinearizedPath& path, skvm::Uniforms* uniforms, SkArenaAlloc* alloc)
        : SkParticleExternalFunction(name, compiler, *compiler.context().fTypes.fFloat4, uniforms, alloc)
        , fPath(path)
    {
    }
    ...
    void call(skvm::Builder* builder, skvm::F32* arguments, skvm::F32* outResult, skvm::I32 mask) const override
    {
        if (fPath.empty()) {
            return;
        }
...
public:
    ...
    std::unique_ptr<SkParticleExternalFunction> toFunction(SkSL::Compiler& compiler, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) override
    {
        return std::make_unique<SkPathExternalFunction>(fName.c_str(), compiler, fData, uniforms, alloc);
    }
...
public:
    SkShaderExternalFunction(const char* name, SkSL::Compiler& compiler, sk_sp<SkShader> shader, skvm::Uniforms* uniforms, SkArenaAlloc* alloc)
        : SkParticleExternalFunction(name, compiler, *compiler.context().fTypes.fFloat4, uniforms, alloc)
    ...
    void call(skvm::Builder* builder, skvm::F32* arguments, skvm::F32* outResult, skvm::I32 mask) const override
    {
        ...
        skvm::Color result = as_SB(fShader)->program(builder, /*device=*/coord, /*local=*/coord, /*paint=*/ { zero, zero, zero, zero }, matrixProvider,
            /*localM=*/nullptr, colorInfo, fUniforms, fAlloc);
        SkASSERT(result);
...
public:
    ...
    std::unique_ptr<SkParticleExternalFunction> toFunction(SkSL::Compiler& compiler, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) override
    {
        return std::make_unique<SkShaderExternalFunction>(fName.c_str(), compiler, fShader, uniforms, alloc);
    }
