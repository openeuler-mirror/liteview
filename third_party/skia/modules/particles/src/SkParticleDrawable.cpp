...
static inline SkRSXform make_rsxform(SkPoint ofs, float posX, float posY, float dirX, float dirY, float scale)
{
...
public:
    ...
    void draw(SkCanvas* canvas, const SkParticles& particles, int count) override
    {
        ...
        canvas->drawAtlas(fImage.get(), arrays.fXforms.get(), arrays.fRects.get(), arrays.fColors.get(), count, SkBlendMode::kModulate, sampling,
            /*cullRect=*/nullptr,
            /*paint=*/nullptr);
    }
...
public:
    ...
    void draw(SkCanvas* canvas, const SkParticles& particles, int count) override
    {
        ...
        SkRect baseRect = SkRect::MakeWH(static_cast<float>(fImage->width()) / cols, static_cast<float>(fImage->height()) / rows);
        ...
        canvas->drawAtlas(fImage.get(), arrays.fXforms.get(), arrays.fRects.get(), arrays.fColors.get(), count, SkBlendMode::kModulate,
...
public:
    ...
    void draw(SkCanvas* canvas, const SkParticles& particles, int count) override
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            auto mat = SkMatrix::Translate(particles.fData[SkParticles::kPositionX][i], particles.fData[SkParticles::kPositionY][i]) * SkMatrix::Scale(s, s)
                * SkMatrix::RotateRad(rads) * SkMatrix::Translate(width / -2, height / -2);
            canvas->concat(mat);
...
sk_sp<SkParticleDrawable> SkParticleDrawable::MakeImage(const char* imagePath, const char* imageName, int cols, int rows)
{
    return sk_sp<SkParticleDrawable>(new SkImageDrawable(imagePath, imageName, cols, rows));
}

sk_sp<SkParticleDrawable> SkParticleDrawable::MakeSkottie(const char* animPath, const char* animName, int cols, int rows)
{
