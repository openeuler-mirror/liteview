...
struct SkParticleProgram {
    SkParticleProgram(skvm::Program effectSpawn, skvm::Program effectUpdate, skvm::Program spawn, skvm::Program update,
        std::vector<std::unique_ptr<SkSL::ExternalFunction>> externalFunctions, skvm::Uniforms externalFunctionUniforms, std::unique_ptr<SkArenaAlloc> alloc,
...
static const char* kCommonHeader = "struct Effect {"
                                   "float  age;"
                                   "float  lifetime;"
                                   "int    loop;"
                                   "float  rate;"
...
static const char* kDefaultCode = "void effectSpawn(inout Effect effect) {"
                                  "}"
                                  ""
...
void SkParticleEffectParams::prepare(const skresources::ResourceProvider* resourceProvider)
{
    ...
    auto buildProgram = [this](const std::string& code) -> std::unique_ptr<SkParticleProgram> {
        ...
        auto buildFunction = [&](const char* name) {
            ...
            if (!SkSL::ProgramToSkVM(*program, *fn, &b, /*debugTrace=*/nullptr, SkSpan(uniformIDs))) {
                return skvm::Program {};
            }
            return b.done();
        };

        skvm::Program effectSpawn = buildFunction("effectSpawn"), effectUpdate = buildFunction("effectUpdate"), spawn = buildFunction("spawn"),
                      update = buildFunction("update");

        return std::make_unique<SkParticleProgram>(std::move(effectSpawn), std::move(effectUpdate), std::move(spawn), std::move(update), std::move(externalFns),
            std::move(efUniforms), std::move(alloc), std::move(uniformInfo));
    };
...
bool SkParticleEffect::setUniform(const char* name, const float* val, int count)
{
    ...
    auto it = std::find_if(info->fUniforms.begin(), info->fUniforms.end(), [name](const auto& u) { return u.fName == name; });
...
void SkParticleEffect::runEffectScript(EntryPoint entryPoint)
{
    ...
    const skvm::Program& prog = entryPoint == EntryPoint::kSpawn ? fParams->fProgram->fEffectSpawn : fParams->fProgram->fEffectUpdate;
    if (prog.empty()) {
        return;
    }

    constexpr size_t kNumEffectArgs = sizeof(EffectState) / sizeof(int);
    void* args[kNumEffectArgs + 1 // external function uniforms
        + 1]; // SkSL uniforms
...
void SkParticleEffect::runParticleScript(EntryPoint entryPoint, int start, int count)
{
    ...
    const skvm::Program& prog = entryPoint == EntryPoint::kSpawn ? fParams->fProgram->fSpawn : fParams->fProgram->fUpdate;
    if (prog.empty()) {
        return;
    }

    void* args[SkParticles::kNumChannels + 1 // external function uniforms
        + 1]; // SkSL uniforms
    args[0] = fParams->fProgram->fExternalFunctionUniforms.buf.data();
    args[1] = fUniforms.data();
...
void SkParticleEffect::advanceTime(double now)
{
    ...
    for (int i = 0; i < fCount; ++i) {
        fParticles.fData[SkParticles::kAge][i] += fParticles.fData[SkParticles::kLifetime][i] * deltaTime;
    ...
    // Do integration of effect position and orientation
    {
        ...
        fState.fHeading.setNormalize(fState.fHeading.fX * c - fState.fHeading.fY * s, fState.fHeading.fX * s + fState.fHeading.fY * c);
    }
    ...
    if (numToSpawn) {
        ...
        for (int i = spawnBase; i < fCount; ++i) {
            fParticles.fData[SkParticles::kLifetime][i] = sk_ieee_float_divide(1.0f, fParticles.fData[SkParticles::kLifetime][i]);
    ...
    for (int i = 0; i < fCount; ++i) {
        fParticles.fData[SkParticles::kPositionX][i] += fParticles.fData[SkParticles::kVelocityX][i] * deltaTime;
        fParticles.fData[SkParticles::kPositionY][i] += fParticles.fData[SkParticles::kVelocityY][i] * deltaTime;
        ...
        float oldHeadingX = fParticles.fData[SkParticles::kHeadingX][i], oldHeadingY = fParticles.fData[SkParticles::kHeadingY][i];
