...
std::unique_ptr<SkShaper::BiDiRunIterator> SkShaper::MakeBiDiRunIterator(const char* utf8, size_t utf8Bytes, uint8_t bidiLevel)
{
    ...
    std::unique_ptr<SkShaper::BiDiRunIterator> bidi = SkShaper::MakeSkUnicodeBidiRunIterator(unicode.get(), utf8, utf8Bytes, bidiLevel);
    if (bidi) {
        return bidi;
    }
#endif
...
std::unique_ptr<SkShaper::ScriptRunIterator> SkShaper::MakeScriptRunIterator(const char* utf8, size_t utf8Bytes, SkFourByteTag scriptTag)
{
    ...
    std::unique_ptr<SkShaper::ScriptRunIterator> script = SkShaper::MakeSkUnicodeHbScriptRunIterator(utf8, utf8Bytes, scriptTag);
    if (script) {
        return script;
    }
#endif
