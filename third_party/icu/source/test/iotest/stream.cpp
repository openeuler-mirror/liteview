...
static void U_CALLCONV TestStream(void)
{
    ...
    outFormatStream << "1234567890" << setw(10) << left << ustr << " "
                    << "0123456789";
...
{
    return (stream.good() << IOSTREAM_GOOD_SHIFT) | (stream.bad() << IOSTREAM_BAD_SHIFT) | (stream.eof() << IOSTREAM_EOF_SHIFT)
...
void testString(UnicodeString& str, const char* testString, const UChar* expectedString, int32_t expectedStatus)
{
#ifdef USE_SSTREAM
    stringstream sstrm;
#else
