    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(fData); i++) {
        if ((newRules->select(fData[i]) == KEYWORD_A) != isKeywordA[i]) {
            errln("File %s, Line %d, ERROR: plural rules for decimal fractions test failed!\n"
                  "  number = %g, expected %s",
                __FILE__, __LINE__, fData[i], isKeywordA[i] ? "TRUE" : "FALSE");
        }
    }
...
void PluralRulesTest::assertRuleKeyValue(const UnicodeString& rule, const UnicodeString& key, double expected)
{
    UErrorCode status = U_ZERO_ERROR;
...
void PluralRulesTest::testGetSamples()
{
    ...
    for (int32_t i = 0; U_SUCCESS(status) && i < numLocales; ++i) {
        ...
        while (NULL != (keyword = keywords->snext(status))) {
            ...
            if (U_FAILURE(status)) {
                errln(UnicodeString(u"getSamples() failed for locale ") + locales[i].getName() + UnicodeString(u", keyword ") + *keyword);
                continue;
            }
            if (count == 0) {
                // TODO: Lots of these.
            ...
            if (count > UPRV_LENGTHOF(values)) {
                errln(UnicodeString(u"getSamples()=") + count + UnicodeString(u", too many values, for locale ") + locales[i].getName()
            ...
            for (int32_t j = 0; j < count; ++j) {
                if (values[j] == UPLRULES_NO_UNIQUE_VALUE) {
                    ...
                    if (*keyword != resultKeyword) {
                        errln("file %s, line %d, Locale %s, sample for keyword \"%s\":  %g, select(%g) returns keyword \"%s\"", __FILE__, __LINE__,
                            locales[i].getName(), US(*keyword).cstr(), values[j], values[j], US(resultKeyword).cstr());
                    }
                }
            }
        }
                ...
                if (values[j] == UPLRULES_NO_UNIQUE_VALUE_DECIMAL) {
                    ...
                    if (*keyword != resultKeyword) {
                        if (values[j].exponent == 0 || !logKnownIssue("21714", "PluralRules::select treats 1c6 as 1")) {
                            ...
                            errln("file %s, line %d, Locale %s, sample for keyword \"%s\":  %s, select(%s) returns keyword \"%s\"", __FILE__, __LINE__,
                                locales[i].getName(), US(*keyword).cstr(), valueBuf, valueBuf, US(resultKeyword).cstr());
                        }
                    }
                }
            }
...
void PluralRulesTest::testSamplesWithExponent()
{
    // integer samples
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString description(u"one: i = 0,1 @integer 0, 1, 1e5 @decimal 0.0~1.5, 1.1e5; "
    ...
    // decimal samples
    status = U_ZERO_ERROR;
    UnicodeString description2(u"one: i = 0,1 @decimal 0.0~1.5, 1.1e5; "
                               u"many: e = 0 and i != 0 and i % 1000000 = 0 and v = 0 or e != 0..5"
    ...
    checkNewSamples(description2, test2, u"other",
        u"@decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1e5, 3.1e5, 4.1e5, 5.1e5, 6.1e5, 7.1e5, …", FixedDecimal(2.0, 1));
}
...
void PluralRulesTest::testSamplesWithCompactNotation()
{
    // integer samples
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString description(u"one: i = 0,1 @integer 0, 1, 1c5 @decimal 0.0~1.5, 1.1c5; "
    ...
    // decimal samples
    status = U_ZERO_ERROR;
    UnicodeString description2(u"one: i = 0,1 @decimal 0.0~1.5, 1.1c5; "
                               u"many: c = 0 and i != 0 and i % 1000000 = 0 and v = 0 or c != 0..5"
    ...
    checkNewSamples(description2, test2, u"other",
        u"@decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1c5, 3.1c5, 4.1c5, 5.1c5, 6.1c5, 7.1c5, …", FixedDecimal(2.0, 1));
}

void PluralRulesTest::checkNewSamples(
    UnicodeString description, const LocalPointer<PluralRules>& test, UnicodeString keyword, UnicodeString samplesString, FixedDecimal firstInRange)
{
    ...
    if (!(firstInRange == actualFirstSample)) {
        ...
        char errMsg[1000];
        snprintf(
            errMsg, sizeof(errMsg), "First parsed sample FixedDecimal not equal to expected for samples: %s in rule string: %s\n", descCstr(), samplesCstr());
        errln(errMsg);
    }
}
...
{
    const char* data[]
        = { "a: n in 2..5", "a: 2,3,4,5; other: null; b:", "a: n not in 2..5", "a: null; other: null", "a: n within 2..5", "a: null; other: null",
              "a: n not within 2..5", "a: null; other: null", "a: n in 2..5 or n within 6..8", "a: null", // ignore 'other' here on out, always null
              "a: n in 2..5 and n within 6..8", "a:", "a: n in 2..5 and n within 5..8", "a: 5", "a: n within 2..5 and n within 6..8",
              "a:", // our sampling catches these
              ...
              "or n within 5..6 and n within 6..7",
              "a: null", // but not this...
              "a: n mod 3 is 0", "a: null", "a: n mod 3 is 0 and n within 1..2", "a:", "a: n mod 3 is 0 and n within 0..5", "a: 0,3",
              "a: n mod 3 is 0 and n within 0..6",
              "a: null", // similarly with mod, we don't catch...
              "a: n mod 3 is 0 and n in 3..12", "a: 3,6,9,12", NULL };
...
void PluralRulesTest::testScientificPluralKeyword()
{
    ...
    LocalPointer<PluralRules> rules(PluralRules::createRules(u"one: i = 0,1 @integer 0, 1 @decimal 0.0~1.5;  "
                                                             u"many: e = 0 and i % 1000000 = 0 and v = 0 or e != 0 .. 5;  "
                                                             u"other:  @integer 2~17, 100, 1000, 10000, 100000, 1000000,  "
                                                             u"  @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, …",
        errorCode));
    ...
    for (const auto& cas : cases) {
        ...
        UnicodeString actualPluralRuleKeyword = getPluralKeyword(rules, locale, input, skeleton);
...
void PluralRulesTest::testDoubleValue()
{
    ...
    struct IntTestCase {
        ...
        const double expVal;
    } intCases[] = { { -101, -101.0 }, { -100, -100.0 }, { -1, -1.0 }, { 0, 0.0 }, { 1, 1.0 }, { 100, 100.0 } };
    ...
    struct DoubleTestCase {
        ...
        const double expVal;
    } dblCases[] = { { -0.0, -0.0 }, { 0.1, 0.1 }, { 1.999, 1.999 }, { 2.0, 2.0 }, { 100.001, 100.001 } };
...
void PluralRulesTest::testLongValue()
{
    ...
    } intCases[] = { { -101, 101 }, { -100, 100 }, { -1, 1 }, { 0, 0 }, { 1, 1 }, { 100, 100 } };
    for (const auto& cas : intCases) {
        const int64_t inputNum = cas.inputNum;
    ...
    struct DoubleTestCase {
        const double inputNum;
        const int64_t expVal;
    } dblCases[] = { { -0.0, 0 }, { 0.1, 0 }, { 1.999, 1 }, { 2.0, 2 }, { 100.001, 100 } };
    for (const auto& cas : dblCases) {
        const double inputNum = cas.inputNum;
        const int64_t expVal = cas.expVal;
...
void PluralRulesTest::checkSelect(const LocalPointer<PluralRules>& rules, UErrorCode& status, int32_t line, const char* keyword, ...)
{
    ...
    for (;;) {
        ...
        if (actualKeyword != UnicodeString(keyword)) {
            errln("file %s, line %d, select(%s) returned incorrect keyword. Expected %s, got %s", __FILE__, line, num, keyword, US(actualKeyword).cstr());
        }
    }
    va_end(ap);
}
...
void PluralRulesTest::testSelectRange()
{
    ...
    auto bare = LocalPointer<PluralRules>(PluralRules::createRules(u"a: i = 0,1", status), status);
    if (status.errIfFailureAndReset()) {
        return;
    }
    ...
    auto xyz = LocalPointer<PluralRules>(PluralRules::forLocale("xyz", status));
...
void PluralRulesTest::testAvailableLocales()
{
    ...
    if (localeCount != localeCount2) {
        errln("file %s,  line %d: locale counts differ. They are (%d, %d)", __FILE__, __LINE__, localeCount, localeCount2);
    }
    ...
    for (;;) {
        ...
        if (U_FAILURE(status)) {
            errln("file %s,  line %d: Error status = %s, locale = %s", __FILE__, __LINE__, u_errorName(status), localeName);
            return;
        }
        if (localeName == NULL) {
            break;
        ...
        if (U_FAILURE(status)) {
            errln("file %s,  line %d: Error %s creating plural rules for locale %s", __FILE__, __LINE__, u_errorName(status), localeName);
            continue;
        }
        if (pr == NULL) {
        ...
        for (double n = 0; n < 120.0; n += 0.5) {
            ...
            if (keyword.length() == 0) {
                errln("file %s, line %d, empty keyword for n = %g, locale %s", __FILE__, __LINE__, n, localeName);
            }
        }
        delete pr;
    }
...
void PluralRulesTest::testParseErrors()
{
    ...
    static const char* testCases[]
        = { "a: n mod 10, is 1", "a: q is 13", "a  n is 13", "a: n is 13,", "a: n is 13, 15,   b: n is 4", "a: n is 1, 3, 4.. ", "a: n within 5..4",
...
void PluralRulesTest::testFixedDecimal()
{
    ...
    static DoubleTestCase testCases[] = { { 1.0, 0, 0 }, { 123456.0, 0, 0 }, { 1.1, 1, 1 }, { 1.23, 2, 23 }, { 1.234, 3, 234 }, { 1.2345, 4, 2345 },
        { 1.23456, 5, 23456 }, { .1234, 4, 1234 }, { .01234, 5, 1234 }, { .001234, 6, 1234 }, { .0001234, 7, 1234 }, { 100.1234, 4, 1234 },
        { 100.01234, 5, 1234 }, { 100.001234, 6, 1234 }, { 100.0001234, 7, 1234 } };
    ...
    for (int i = 0; i < UPRV_LENGTHOF(testCases); ++i) {
        ...
        if (numFractionDigits != tc.fractionDigitCount) {
            errln("file %s, line %d: decimals(%g) expected %d, actual %d", __FILE__, __LINE__, tc.n, tc.fractionDigitCount, numFractionDigits);
            continue;
        }
        ...
        if (actualFractionDigits != tc.fractionDigits) {
            errln("file %s, line %d: getFractionDigits(%g, %d): expected %ld, got %ld", __FILE__, __LINE__, tc.n, numFractionDigits, tc.fractionDigits,
...
void PluralRulesTest::testSelectTrailingZeros()
{
    ...
    number::UnlocalizedNumberFormatter unf = number::NumberFormatter::with().precision(number::Precision::fixedFraction(2));
    struct TestCase {
        ...
        { "si", u"one", u"one", 0.0 }, { "si", u"one", u"one", 1.0 }, { "si", u"one", u"other", 0.1 }, // 0.1 => one, but 0.10 => other
