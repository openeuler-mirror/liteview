...
#if !UCONFIG_NO_BREAK_ITERATION
    m_en_wordbreaker_ = BreakIterator::createWordInstance(Locale::getEnglish(), status);
    m_en_characterbreaker_ = BreakIterator::createCharacterInstance(Locale::getEnglish(), status);
#endif
#endif
}
...
void StringSearchTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char*)
{
#if !UCONFIG_NO_BREAK_ITERATION
    UBool areBroken = FALSE;
    if (m_en_us_ == NULL && m_fr_fr_ == NULL && m_de_ == NULL && m_es_ == NULL && m_en_wordbreaker_ == NULL && m_en_characterbreaker_ == NULL && exec) {
        areBroken = TRUE;
    }
...
UBool StringSearchTest::assertEqualWithStringSearch(StringSearch* strsrch, const SearchData* search)
{
    int32_t count = 0;
    UErrorCode status = U_ZERO_ERROR;
    ...
    while (U_SUCCESS(status) && matchindex >= 0) {
        ...
        strsrch->next(status);
        if (matchindex != strsrch->getMatchedStart() || matchlength != strsrch->getMatchedLength()) {
            char* str = toCharString(strsrch->getText());
            errln("Text: %s", str);
            str = toCharString(strsrch->getPattern());
            errln("Pattern: %s", str);
            errln("Error next match found at %d (len:%d); expected %d (len:%d)", strsrch->getMatchedStart(), strsrch->getMatchedLength(), matchindex,
                matchlength);
            return FALSE;
        }
        count++;
        ...
        if (U_FAILURE(status) || strsrch->getText().compareBetween(matchindex, matchindex + matchlength, matchtext, 0, matchtext.length())) {
    ...
    if (strsrch->getMatchedStart() != USEARCH_DONE || strsrch->getMatchedLength() != 0) {
        ...
        str = toCharString(strsrch->getPattern());
        errln("Pattern: %s", str);
        errln("Error next match found at %d (len:%d); expected <NO MATCH>", strsrch->getMatchedStart(), strsrch->getMatchedLength());
        return FALSE;
    }
    ...
    while (U_SUCCESS(status) && matchindex >= 0) {
        ...
        if (matchindex != strsrch->getMatchedStart() || matchlength != strsrch->getMatchedLength()) {
            ...
            str = toCharString(strsrch->getPattern());
            errln("Pattern: %s", str);
            errln("Error previous match found at %d (len:%d); expected %d (len:%d)", strsrch->getMatchedStart(), strsrch->getMatchedLength(), matchindex,
                matchlength);
            return FALSE;
        }
    ...
    if (strsrch->getMatchedStart() != USEARCH_DONE || strsrch->getMatchedLength() != 0) {
        ...
        str = toCharString(strsrch->getPattern());
        errln("Pattern: %s", str);
        errln("Error previous match found at %d (len:%d); expected <NO MATCH>", strsrch->getMatchedStart(), strsrch->getMatchedLength());
        return FALSE;
    }
    ...
    while (TRUE) {
        ...
        if (matchindex < 0) {
            if (strsrch->getMatchedStart() != USEARCH_DONE || strsrch->getMatchedLength() != 0) {
                ...
                str = toCharString(strsrch->getPattern());
                errln("Pattern: %s", str);
                errln("Error following match starting at %d (overlap:%d) found at %d (len:%d); expected <NO MATCH>", nextStart, isOverlap,
        ...
        matchlength = search->size[count];
        if (strsrch->getMatchedStart() != matchindex || strsrch->getMatchedLength() != matchlength || U_FAILURE(status)) {
            char* str = toCharString(strsrch->getText());
            errln("Text: %s\n", str);
            str = toCharString(strsrch->getPattern());
            errln("Pattern: %s\n", str);
            errln("Error following match starting at %d (overlap: %d) found at %d (len:%d); expected %d (len:%d)\n", nextStart, isOverlap,
                strsrch->getMatchedStart(), strsrch->getMatchedLength(), matchindex, matchlength);
            return FALSE;
        }
...
UBool StringSearchTest::assertEqual(const SearchData* search)
{
    ...
    strsrch = new StringSearch(pattern, text, (RuleBasedCollator*)collator, breaker, status);
    if (U_FAILURE(status)) {
    ...
    if (strsrch2 == strsrch || *strsrch2 != *strsrch || !assertEqualWithStringSearch(strsrch2, search)) {
...
UBool StringSearchTest::assertEqualWithAttribute(const SearchData* search, USearchAttributeValue canonical, USearchAttributeValue overlap)
{
    UErrorCode status = U_ZERO_ERROR;
    Collator* collator = getCollator(search->collator);
    BreakIterator* breaker = getBreakIterator(search->breaker);
...
void StringSearchTest::TestInitialization()
{
    ...
    if (*(copy->getCollator()) != *(result->getCollator()) || copy->getBreakIterator() != result->getBreakIterator()
        || copy->getMatchedLength() != result->getMatchedLength() || copy->getMatchedStart() != result->getMatchedStart()
        || copy->getOffset() != result->getOffset() || copy->getPattern() != result->getPattern() || copy->getText() != result->getText()
        || *(copy) != *(result)) {
...
void StringSearchTest::TestNormExact()
{
    ...
    if (U_FAILURE(status)) {
        errln("Error setting collation normalization %s", u_errorName(status));
    }
    while (BASIC[count].text != NULL) {
        if (!assertEqual(&BASIC[count])) {
            infoln("Error at test number %d", count);
...
void StringSearchTest::TestBreakIterator()
{
    ...
    if (U_FAILURE(status) || strsrch->getBreakIterator() != m_en_characterbreaker_) {
        errln("Error usearch_getBreakIterator returned wrong object");
    }

    strsrch->setBreakIterator(m_en_wordbreaker_, status);
    if (U_FAILURE(status) || strsrch->getBreakIterator() != m_en_wordbreaker_) {
        errln("Error usearch_getBreakIterator returned wrong object");
    }
...
void StringSearchTest::TestVariable()
{
    ...
    if (U_FAILURE(status)) {
        errln("Error setting collation alternate attribute %s", u_errorName(status));
    }
    ...
    m_en_us_->setAttribute(UCOL_ALTERNATE_HANDLING, UCOL_NON_IGNORABLE, status);
}
...
void StringSearchTest::TestOverlap()
{
    ...
    while (OVERLAP[count].text != NULL) {
        if (!assertEqualWithAttribute(&OVERLAP[count], USEARCH_OFF, USEARCH_ON)) {
    ...
    while (count < 1) {
        ...
        RuleBasedCollator* collator = getCollator(search->collator);
        UErrorCode status = U_ZERO_ERROR;
        StringSearch* strsrch = new StringSearch(pattern, text, collator, NULL, status);

        strsrch->setAttribute(USEARCH_OVERLAP, USEARCH_ON, status);
        if (U_FAILURE(status) || strsrch->getAttribute(USEARCH_OVERLAP) != USEARCH_ON) {
        ...
        if (U_FAILURE(status) || strsrch->getAttribute(USEARCH_OVERLAP) != USEARCH_OFF) {
            errln("Error setting overlap option");
        }
        strsrch->reset();
        if (!assertEqualWithStringSearch(strsrch, search)) {
...
void StringSearchTest::TestGetSetOffset()
{
    ...
    while (BASIC[index].text != NULL) {
        ...
        strsrch->setText(text, status);
        strsrch->setPattern(pattern, status);
        strsrch->getCollator()->setStrength(getECollationStrength(search.strength));
        strsrch->reset();
        ...
        while (U_SUCCESS(status) && matchindex >= 0) {
            ...
            if (matchindex != strsrch->getMatchedStart() || matchlength != strsrch->getMatchedLength()) {
                ...
                str = toCharString(strsrch->getPattern());
                errln("Pattern: %s", str);
                errln("Error match found at %d %d", strsrch->getMatchedStart(), strsrch->getMatchedLength());
                return;
            }
    ...
    strsrch->getCollator()->setStrength(getECollationStrength(UCOL_TERTIARY));
    delete strsrch;
}
...
void StringSearchTest::TestGetSetAttribute()
{
    ...
    strsrch->setAttribute(USEARCH_OVERLAP, USEARCH_ATTRIBUTE_VALUE_COUNT, status);
    if (U_SUCCESS(status)) {
        errln("Error setting overlap to illegal value");
    }
    status = U_ZERO_ERROR;
    strsrch->setAttribute(USEARCH_CANONICAL_MATCH, USEARCH_DEFAULT, status);
    if (U_FAILURE(status) || strsrch->getAttribute(USEARCH_CANONICAL_MATCH) != USEARCH_OFF) {
        errln("Error setting canonical match to the default");
    }
    strsrch->setAttribute(USEARCH_CANONICAL_MATCH, USEARCH_ON, status);
    if (U_FAILURE(status) || strsrch->getAttribute(USEARCH_CANONICAL_MATCH) != USEARCH_ON) {
    ...
    strsrch->setAttribute(USEARCH_CANONICAL_MATCH, USEARCH_ATTRIBUTE_VALUE_COUNT, status);
    if (U_SUCCESS(status)) {
...
void StringSearchTest::TestSetMatch()
{
    ...
    while (MATCH[count].text != NULL) {
        ...
        if (strsrch->last(status) != search.offset[size - 1] || U_FAILURE(status)) {
        ...
        while (index < size) {
            if (index + 2 < size) {
                if (strsrch->following(search.offset[index + 2] - 1, status) != search.offset[index + 2] || U_FAILURE(status)) {
                    errln("Error getting following match at index %d", search.offset[index + 2] - 1);
                }
            }
            if (index + 1 < size) {
                if (strsrch->preceding(search.offset[index + 1] + search.size[index + 1] + 1, status) != search.offset[index + 1] || U_FAILURE(status)) {
                    errln("Error getting preceding match at index %d", search.offset[index + 1] + 1);
                }
            }
            index += 2;
        }
...
void StringSearchTest::TestReset()
{
    ...
    if (U_FAILURE(status)) {
        ...
        strsrch->reset();
        if (strsrch->getAttribute(USEARCH_OVERLAP) != USEARCH_OFF || strsrch->getAttribute(USEARCH_CANONICAL_MATCH) != USEARCH_OFF || strsrch->getOffset() != 0
            || strsrch->getMatchedLength() != 0 || strsrch->getMatchedStart() != USEARCH_DONE) {
            errln("Error resetting string search");
        }
        strsrch->previous(status);
        if (strsrch->getMatchedStart() != 7 || strsrch->getMatchedLength() != 1) {
            errln("Error resetting string search\n");
        }
    }
    delete strsrch;
...
void StringSearchTest::TestContraction()
{
    ...
    UnicodeString rules;
    rules.setTo(temp, u_strlen(temp));
    RuleBasedCollator* collator = new RuleBasedCollator(rules, getECollationStrength(UCOL_TERTIARY), UCOL_ON, status);
    if (U_FAILURE(status)) {
...
void StringSearchTest::TestIgnorable()
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    int count = 0;
    RuleBasedCollator* collator = new RuleBasedCollator(rules, getECollationStrength(IGNORABLE[count].strength), UCOL_ON, status);
    if (U_FAILURE(status)) {
        errln("Error opening collator %s", u_errorName(status));
        return;
    }
...
void StringSearchTest::TestBreakIteratorCanonical()
{
    ...
    while (count < 4) {
        ...
        BreakIterator* breaker = getBreakIterator(search->breaker);
        StringSearch* strsrch = new StringSearch(pattern, text, collator, breaker, status);
        if (U_FAILURE(status)) {
...
void StringSearchTest::TestOverlapCanonical()
{
    ...
    while (OVERLAPCANONICAL[count].text != NULL) {
        if (!assertEqualWithAttribute(&OVERLAPCANONICAL[count], USEARCH_ON, USEARCH_ON)) {
            errln("Error at overlap test number %d", count);
        }
        count++;
    }
...
void StringSearchTest::TestCollatorCanonical()
{
    ...
    UnicodeString rules;
    rules.setTo(temp, u_strlen(temp));
    RuleBasedCollator* tailored = new RuleBasedCollator(rules, getECollationStrength(COLLATORCANONICAL[1].strength), UCOL_ON, status);
...
TestSearch::TestSearch(const TestSearch& obj)
    : SearchIterator(obj)
{
...
TestSearch::TestSearch(const UnicodeString& text, BreakIterator* breakiter, const UnicodeString& pattern)
    : SearchIterator()
{
