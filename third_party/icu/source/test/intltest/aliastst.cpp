...
const char* _LOCALE_ALIAS[][2] = { { "in", "id" }, { "in_ID", "id_ID" }, { "iw", "he" }, { "iw_IL", "he_IL" }, { "ji", "yi" }, { "en_BU", "en_MM" },
    { "en_DY", "en_BJ" }, { "en_HV", "en_BF" }, { "en_NH", "en_VU" }, { "en_RH", "en_ZW" }, { "en_TP", "en_TL" }, { "en_ZR", "en_CD" } };
    ...
    for (int i = 0; i < _LOCALE_NUMBER; i++) {
        ...
        for (int j = 0; j < availableNum; j++) {
            ...
            if (newLang != newLoc.getLanguage()) {
                if (oldLang != newLang) {
                    errln("getLanguage() failed for " + UnicodeString(oldLoc.getName()) + " oldLang = " + prettify(oldLang) + " newLang = " + prettify(newLang)
