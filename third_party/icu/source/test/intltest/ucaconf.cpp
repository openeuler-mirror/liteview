...
static UBool skipLineBecauseOfBug(const UChar* s, int32_t length, uint32_t flags)
{
    ...
    while (fgets(lineB, 1024, testFile) != NULL) {
        ...
        if (oldSk != NULL) {
            ...
            if (withSortKeys && cmpres != normalizeResult(skres)) {
                errln("Difference between coll->compare (%d) and sortkey compare (%d) on line %i", cmpres, skres, line);
                ok = FALSE;
            }
