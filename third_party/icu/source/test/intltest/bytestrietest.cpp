...
void BytesTrieTest::TestDelta()
{
    char intBytes0[5];
    char intBytes1[5];
    static constexpr int32_t sampleDeltas[] = { -1, 0, 1, 2, 3, 0xa5, 0xbe, 0xbf, -2, 0xc0, 0xc1, 0xeee, 0x1234, 0x2ffe, 0x2fff, -3, 0x3000, 0x3001, 0x3003,
        0x50005, 0xdfffe, 0xdffff, -4, 0xe0000, 0xe0001, 0xef0123, 0xfffffe, 0xffffff, -5, 0x1000000, 0x1000001, 0x7fffffff };
    ...
    for (int32_t delta : sampleDeltas) {
        ...
        assertEquals(UnicodeString(u"expected length to encode delta ") + delta, expectedLength, length0);
        for (int32_t i = 0; i < length0; ++i) {
            uint8_t b0 = intBytes0[i];
            uint8_t b1 = intBytes1[i];
            assertEquals(UnicodeString(u"differently encoded delta ") + delta + u" at byte index " + i, b0, b1);
        }
        const uint8_t* start = (const uint8_t*)intBytes0;
        const uint8_t* pos = BytesTrie::jumpByDelta(start);
        assertEquals(UnicodeString(u"roundtrip for delta ") + delta, delta, (int32_t)(pos - start) - length0);
    }
}
...
BytesTrie* BytesTrieTest::buildTrie(const StringAndValue data[], int32_t dataLength, UStringTrieBuildOption buildOption)
{
...
void BytesTrieTest::checkFirst(BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
    for (int32_t i = 0; i < dataLength; ++i) {
        int c = *data[i].s;
        if (c == 0) {
        ...
        if (firstResult != trie.reset().next(c) || firstResult != trie.current() || firstValue != (USTRINGTRIE_HAS_VALUE(firstResult) ? trie.getValue() : -1)
            ...
            errln("trie.first(%c)!=trie.reset().next(same) for %s", c, data[i].s);
        }
    }
    trie.reset();
}

void BytesTrieTest::checkNext(BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
    BytesTrie::State state;
    for (int32_t i = 0; i < dataLength; ++i) {
        int32_t stringLength = (i & 1) ? -1 : static_cast<int32_t>(strlen(data[i].s));
        UStringTrieResult result;
        if (!USTRINGTRIE_HAS_VALUE(result = trie.next(data[i].s, stringLength)) || result != trie.current()) {
        ...
        if ((result == USTRINGTRIE_INTERMEDIATE_VALUE) != nextContinues) {
            errln("(trie.current()==USTRINGTRIE_INTERMEDIATE_VALUE) contradicts "
                  "(trie.next(some byte)!=USTRINGTRIE_NO_MATCH) after end of %s",
                data[i].s);
        }
        trie.reset();
    }
...
void BytesTrieTest::checkNextWithState(BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
    ...
    for (int32_t i = 0; i < dataLength; ++i) {
        ...
        if (resultAtState != trie.resetToState(state).current() || (USTRINGTRIE_HAS_VALUE(resultAtState) && valueAtState != trie.getValue())) {
            errln("trie.next(part of %s) changes current()/getValue() after "
                ...
                data[i].s);
        } else if (!USTRINGTRIE_HAS_VALUE(result = trie.next(expectedString + partialLength, stringLength - partialLength)) || result != trie.current()) {
            errln("trie.next(rest of %s) does not seem to contain %s after "
                  "saveState/next(0)/resetToState",
                data[i].s, data[i].s);
        } else if (!USTRINGTRIE_HAS_VALUE(result = trie.resetToState(state).next(expectedString + partialLength, stringLength - partialLength))
            || result != trie.current()) {
...
void BytesTrieTest::checkNextWithState64(BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
    ...
    for (int32_t i = 0; i < dataLength; ++i) {
        ...
        if (resultAtState != trie.resetToState64(state).current() || (USTRINGTRIE_HAS_VALUE(resultAtState) && valueAtState != trie.getValue())) {
            ...
            errln("trie.next(rest of %s) does not seem to contain %s after "
                  "getState64/next(0)/resetToState64",
                data[i].s, data[i].s);
        } else if (!USTRINGTRIE_HAS_VALUE(result = trie.resetToState64(state).next(expectedString + partialLength, stringLength - partialLength))
            || result != trie.current()) {
...
void BytesTrieTest::checkNextString(BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
    for (int32_t i = 0; i < dataLength; ++i) {
        const char* expectedString = data[i].s;
        int32_t stringLength = static_cast<int32_t>(strlen(expectedString));
...
void BytesTrieTest::checkIterator(const BytesTrie& trie, const StringAndValue data[], int32_t dataLength)
{
...
void BytesTrieTest::checkIterator(BytesTrie::Iterator& iter, const StringAndValue data[], int32_t dataLength)
{
    IcuTestErrorCode errorCode(*this, "checkIterator()");
    for (int32_t i = 0; i < dataLength; ++i) {
        if (!iter.hasNext()) {
        ...
        if (iter.getString() != StringPiece(data[i].s)) {
            errln("trie iterator next().getString()=%s but expected %s for item %d", iter.getString().data(), data[i].s, (int)i);
        }
