...
void UnitsDataTest::testGetAllConversionRates()
{
    ...
    for (int i = 0; i < conversionInfo.length(); i++) {
        ...
        logln("* conversionInfo %d: source=\"%s\", baseUnit=\"%s\", factor=\"%s\", offset=\"%s\"", i, cri->sourceUnit.data(), cri->baseUnit.data(),
...
public:
    UnitPreferencesOpenedUp(UErrorCode& status)
        : UnitPreferences(status)
    {
    }
    const MaybeStackVector<UnitPreferenceMetadata>* getInternalMetadata() const
    {
        return &metadata_;
    }
    const MaybeStackVector<UnitPreference>* getInternalUnitPrefs() const
    {
        return &unitPrefs_;
    }
};
...
void UnitsDataTest::testGetPreferencesFor()
{
    ...
    struct TestCase {
        ...
        { "AT person-height", "length", "person-height", "AT", "meter-and-centimeter", "meter-and-centimeter" },
    };
    ...
    assertTrue(UnicodeString("Metadata count: ") + metadata->length() + " > 200", metadata->length() > 200);
    assertTrue(UnicodeString("Preferences count: ") + unitPrefs->length() + " > 250", unitPrefs->length() > 250);
    ...
    for (const auto& t : testCases) {
        ...
        if (status.errIfFailureAndReset("getPreferencesFor(\"%s\", \"%s\", \"%s\", ...", t.category, t.usage, t.region)) {
            continue;
        }
        if (prefsCount > 0) {
            assertEquals(UnicodeString(t.name) + " - max unit", t.expectedBiggest, prefs[0]->unit.data());
            assertEquals(UnicodeString(t.name) + " - min unit", t.expectedSmallest, prefs[prefsCount - 1]->unit.data());
        } else {
