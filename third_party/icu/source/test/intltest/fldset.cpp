...
UnicodeString FieldsSet::diffFrom(const FieldsSet& other, UErrorCode& status) const
{
    ...
    for (int i = 0; i < fieldCount(); i++) {
        if (isSet((UCalendarDateFields)i)) {
            ...
            if (fEnum != -1) {
                const UnicodeString& fieldName = udbg_enumString(fEnum, i);
...
int32_t FieldsSet::parseFrom(const UnicodeString& str, const FieldsSet* inheritFrom, UErrorCode& status)
{
...
{
    return ((&other == this) || ((other.fFieldCount == fFieldCount) && (other.fEnum == fEnum)));
}
...
void FieldsSet::parseValueDefault(const FieldsSet* inheritFrom, int32_t field, const UnicodeString& substr, UErrorCode& status)
{
    ...
    if (substr.length() == 0) { // inherit requested
        ...
        if ((inheritFrom == NULL) || !inheritFrom->isSet((UCalendarDateFields)field)) {
            ...
            it_errln(UnicodeString("Parse Failed: Couldn't inherit field ") + field + UnicodeString(" [") + UnicodeString(udbg_enumName(fEnum, field))
                + UnicodeString("]"));
            status = U_ILLEGAL_ARGUMENT_ERROR;
            return;
        }
