    ...
    : map(NULL)
{
    ...
    map = uhash_open(uhash_hashLong, uhash_compareLong, uhash_compareCaselessUnicodeString, &status);
    ...
    , ceToCharsStartingWith(NULL)
{
    ...
    for (int32_t item = 0; item < itemCount; item += 1) {
        ...
        UChar buffer[16];
        int32_t len = uset_getItem(charsToTest, item, &start, &end, buffer, 16, &status);
