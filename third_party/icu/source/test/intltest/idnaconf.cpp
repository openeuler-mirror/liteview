...
static const UChar C_USESTD3ASCIIRULES[]
    = { 0x55, 0x73, 0x65, 0x53, 0x54, 0x44, 0x33, 0x41, 0x53, 0x43, 0x49, 0x49, 0x52, 0x75, 0x6C, 0x65, 0x73, 0 }; // UseSTD3ASCIIRules
        ...
        if (passfail == 0) {
            ...
            if (U_FAILURE(status)) {
                ...
                if (namebase.compare(result, -1) == 0) {
                    ...
                    logln(UnicodeString("ICU will not recognize malformed ACE-Prefixes or incorrect ACE-Prefixes. ") + UnicodeString("namebase: ")
                        + prettify(namebase) + UnicodeString(" result: ") + prettify(result));
                } else {
