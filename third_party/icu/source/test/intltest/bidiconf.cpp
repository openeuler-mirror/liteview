...
static UCharDirection U_CALLCONV biDiConfUBiDiClassCallback(const void* /*context*/, UChar32 c)
{
...
static const int8_t biDiClassNameLengths[U_CHAR_DIRECTION_COUNT + 1] = { 1, 1, 2, 2, 2, 2, 2, 1, 1, 2, 2, 3, 3, 2, 3, 3, 3, 3, 2, 3, 3, 3, 3, 0 };
...
void BiDiConformanceTest::TestBidiTest()
{
    ...
    ubidi_setClassCallback(ubidi.getAlias(), biDiConfUBiDiClassCallback, NULL, NULL, NULL, errorCode);
    ...
    while (errorCount < 10 && fgets(line, (int)sizeof(line), bidiTestFile.getAlias()) != NULL) {
        ...
        if (*start == '@') {
            ...
            for (int i = 0; i <= 3; ++i) {
                if (bitset & (1 << i)) {
                    ubidi_setPara(ubidi.getAlias(), inputString.getBuffer(), inputString.length(), paraLevels[i], NULL, errorCode);
...
void BiDiConformanceTest::TestBidiCharacterTest()
{
    ...
    while (errorCount < 20 && fgets(line, (int)sizeof(line), bidiTestFile.getAlias()) != NULL) {
        ...
        if (end <= start || (!U_IS_INV_WHITESPACE(*end) && *end != ';' && *end != 0) || paraLevel == (UBIDI_MAX_EXPLICIT_LEVEL + 2)) {
        ...
        if (end <= start || (!U_IS_INV_WHITESPACE(*end) && *end != ';' && *end != 0) || resolvedParaLevel > 1) {
        ...
        ubidi_setPara(ubidi.getAlias(), inputString.getBuffer(), inputString.length(), paraLevel, NULL, errorCode);
        const UBiDiLevel* actualLevels = ubidi_getLevels(ubidi.getAlias(), errorCode);
        if (errorCode.errIfFailureAndReset("ubidi_setPara() or ubidi_getLevels()")) {
            errln("Input line %d: %s", (int)lineNumber, line);
            continue;
        ...
        if ((actualLevel = ubidi_getParaLevel(ubidi.getAlias())) != resolvedParaLevel) {
            printErrorLine();
            errln("\nError on line %d: Wrong resolved paragraph level; expected %d actual %d", (int)lineNumber, resolvedParaLevel, actualLevel);
            continue;
        }
        if (!checkLevels(actualLevels, ubidi_getProcessedLength(ubidi.getAlias()))) {
            continue;
...
UBool BiDiConformanceTest::checkLevels(const UBiDiLevel actualLevels[], int32_t actualCount)
{
    ...
    if (levelsCount != actualCount) {
        errln("\nError on line %d: Wrong number of level values; expected %d actual %d", (int)lineNumber, (int)levelsCount, (int)actualCount);
        isOk = FALSE;
    } else {
        for (int32_t i = 0; i < actualCount; ++i) {
            if (levels[i] != actualLevels[i] && levels[i] < UBIDI_DEFAULT_LTR) {
                if (directionBits != 3 && directionBits == getDirectionBits(actualLevels, actualCount)) {
                    ...
                    errln("\nError on line %d: Wrong level value at index %d; expected %d actual %d", (int)lineNumber, (int)i, levels[i], actualLevels[i]);
                    isOk = FALSE;
                    break;
                }
            }
...
UBool BiDiConformanceTest::checkOrdering(UBiDi* ubidi)
{
    ...
    for (i = visualIndex = 0; i < resultLength; ++i) {
        ...
        if (visualIndex < orderingCount && logicalIndex != ordering[visualIndex]) {
            errln("\nError on line %d: Wrong ordering value at visual index %d; expected %d actual %d", (int)lineNumber, (int)visualIndex,
                ordering[visualIndex], logicalIndex);
            isOk = FALSE;
            break;
        }
        ++visualIndex;
    ...
    if (isOk && orderingCount != visualIndex) {
        errln("\nError on line %d: Wrong number of ordering values; expected %d actual %d", (int)lineNumber, (int)orderingCount, (int)visualIndex);
        isOk = FALSE;
    }
    if (!isOk) {
        printErrorLine();
