...
void DateFormatTest::TestPatterns()
{
    static const struct {
    ...
    } EXPECTED[] = { { UDAT_YEAR, "y", "en", "y" },

        { UDAT_QUARTER, "QQQQ", "en", "QQQQ" }, { UDAT_ABBR_QUARTER, "QQQ", "en", "QQQ" }, { UDAT_YEAR_QUARTER, "yQQQQ", "en", "QQQQ y" },
        { UDAT_YEAR_ABBR_QUARTER, "yQQQ", "en", "QQQ y" },

        { UDAT_NUM_MONTH, "M", "en", "L" }, { UDAT_ABBR_MONTH, "MMM", "en", "LLL" }, { UDAT_MONTH, "MMMM", "en", "LLLL" },
        { UDAT_YEAR_NUM_MONTH, "yM", "en", "M/y" }, { UDAT_YEAR_ABBR_MONTH, "yMMM", "en", "MMM y" }, { UDAT_YEAR_MONTH, "yMMMM", "en", "MMMM y" },

        { UDAT_DAY, "d", "en", "d" }, { UDAT_YEAR_NUM_MONTH_DAY, "yMd", "en", "M/d/y" }, { UDAT_YEAR_ABBR_MONTH_DAY, "yMMMd", "en", "MMM d, y" },
        { UDAT_YEAR_MONTH_DAY, "yMMMMd", "en", "MMMM d, y" }, { UDAT_YEAR_NUM_MONTH_WEEKDAY_DAY, "yMEd", "en", "EEE, M/d/y" },
        { UDAT_YEAR_ABBR_MONTH_WEEKDAY_DAY, "yMMMEd", "en", "EEE, MMM d, y" }, { UDAT_YEAR_MONTH_WEEKDAY_DAY, "yMMMMEEEEd", "en", "EEEE, MMMM d, y" },

        { UDAT_NUM_MONTH_DAY, "Md", "en", "M/d" }, { UDAT_ABBR_MONTH_DAY, "MMMd", "en", "MMM d" }, { UDAT_MONTH_DAY, "MMMMd", "en", "MMMM d" },
        { UDAT_NUM_MONTH_WEEKDAY_DAY, "MEd", "en", "EEE, M/d" }, { UDAT_ABBR_MONTH_WEEKDAY_DAY, "MMMEd", "en", "EEE, MMM d" },
        ...
        { UDAT_MINUTE, "m", "en", "m" }, { UDAT_HOUR_MINUTE, "jm", "en", "h:mm a" }, // (fixed expected result per ticket 6872<-7180)
        { UDAT_HOUR24_MINUTE, "Hm", "en", "HH:mm" }, // (fixed expected result per ticket 6872<-6626)

        { UDAT_SECOND, "s", "en", "s" }, { UDAT_HOUR_MINUTE_SECOND, "jms", "en", "h:mm:ss a" }, // (fixed expected result per ticket 6872<-7180)
        ...
        { UDAT_LOCATION_TZ, "VVVV", "en", "VVVV" }, { UDAT_GENERIC_TZ, "vvvv", "en", "vvvv" }, { UDAT_ABBR_GENERIC_TZ, "v", "en", "v" },
        { UDAT_SPECIFIC_TZ, "zzzz", "en", "zzzz" }, { UDAT_ABBR_SPECIFIC_TZ, "z", "en", "z" }, { UDAT_ABBR_UTC_TZ, "ZZZZ", "en", "ZZZZ" },
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(EXPECTED); i++) {
        ...
        if (errorCode.errIfFailureAndReset("DateFormat::getInstanceForSkeleton() failed")) {
            errln("  for actualPattern \"%s\" & locale ID \"%s\"", EXPECTED[i].actualPattern, EXPECTED[i].localeID);
            continue;
        }
...
void DateFormatTest::TestWallyWedel()
{
    ...
    for (int32_t i = 0; i < ids_length; i++) {
        ...
        UnicodeString dstOffset = (UnicodeString) "" + sign + (hours < 10 ? "0" : "") + (int32_t)hours + ":" + (minutes < 10 ? "0" : "") + (int32_t)minutes;
        if (seconds != 0) {
        ...
        if (ok) {
            logln(UnicodeString() + i + " " + *id + " " + dstOffset + " " + fmtOffset + (fmtDstOffset != 0 ? " ok" : " ?"));
        } else {
            errln(UnicodeString() + i + " " + *id + " " + dstOffset + " " + fmtOffset + " *** FAIL ***");
        }
        delete ttz;
        delete fmtDstOffset;
    }
 ...
 * Test operator==
 */
void DateFormatTest::TestEquals()
{
 ...
 */
void DateFormatTest::TestTwoDigitYearDSTParse(void)
{
    UErrorCode status = U_ZERO_ERROR;
...
static const int32_t DATEFORMAT_FIELD_NAMES_LENGTH = UPRV_LENGTHOF(DATEFORMAT_FIELD_NAMES);
...
void DateFormatTest::TestFieldPosition()
{
    ...
    for (j = 0, exp = 0; j < COUNT; ++j) {
        ...
        for (i = 0; i < UDAT_FIELD_COUNT; ++i, ++exp) {
            ...
            assertEquals((UnicodeString) "field #" + i + " " + DATEFORMAT_FIELD_NAMES[i], ctou(EXPECTED[exp]), field);
        }
        ...
        logln("FieldPositionIterator");
        {
            ...
            while (posIter.next(fp)) {
                ...
                assertEquals((UnicodeString) "field #" + i + " " + DATEFORMAT_FIELD_NAMES[i], ctou(EXPECTED[expBase + i]), field);
            }
        }
    }
 ...
 */
void DateFormatTest::TestPartialParse994()
{
    UErrorCode status = U_ZERO_ERROR;
...
void DateFormatTest::tryPat994(SimpleDateFormat* format, const char* pat, const char* str, UDate expected)
{
    UErrorCode status = U_ZERO_ERROR;
    UDate null = 0;
    ...
    if (U_FAILURE(status) || date == null) {
        ...
        if (expected != null)
            errln((UnicodeString) "FAIL: Expected " + dateToString(expected));
    } else {
        UnicodeString f;
        ...
        if (expected == null || !(date == expected))
            errln((UnicodeString) "FAIL: Expected null"); //" + expected);
        if (!(f == str))
 ...
 */
void DateFormatTest::TestRunTogetherPattern985()
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (date2 == 0)
        then = UnicodeString("Parse stopped at ") + pos.getIndex();
    else
        ((DateFormat*)formatter)->format(date2, then);
    logln(then);
    if (!(date2 == date1))
        errln((UnicodeString) "FAIL");
    delete formatter;
 ...
 * without intervening separators.
 */
void DateFormatTest::TestRunTogetherPattern917()
{
    UErrorCode status = U_ZERO_ERROR;
...
void DateFormatTest::testIt917(SimpleDateFormat* fmt, UnicodeString& str, UDate expected)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (!(o.getDate() == expected))
        errln((UnicodeString) "FAIL: Expected " + dateToString(expected));
    UnicodeString formatted;
    ((Format*)fmt)->format(o, formatted, status);
 ...
 */
void DateFormatTest::TestCzechMonths459()
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    UnicodeString juneStr;
    fmt->format(june, juneStr);
    UnicodeString julyStr;
    fmt->format(july, julyStr);
    // try {
    logln((UnicodeString) "format(June 15 1997) = " + juneStr);
    UDate d = fmt->parse(juneStr, status);
    UnicodeString s;
    fmt->format(d, s);
    int32_t month, yr, day, hr, min, sec;
    dateToFields(d, yr, month, day, hr, min, sec);
    logln((UnicodeString) "  -> parse -> " + s + " (month = " + month + ")");
    if (month != UCAL_JUNE)
        errln((UnicodeString) "FAIL: Month should be June");
    ...
    logln((UnicodeString) "  -> parse -> " + s + " (month = " + month + ")");
    if (month != UCAL_JULY)
        errln((UnicodeString) "FAIL: Month should be July");
    //}
    // catch(ParseException e) {
    if (U_FAILURE(status))
 ...
 */
void DateFormatTest::TestLetterDPattern212()
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (myDate != expBigD)
        errln((UnicodeString) "FAIL: bigD - Expected " + dateToString(expBigD));
    delete formatter;
    ...
    if (myDate != expLittleD)
        errln((UnicodeString) "FAIL: littleD - Expected " + dateToString(expLittleD));
    delete formatter;
    if (U_FAILURE(status))
        errln((UnicodeString) "FAIL: UErrorCode received during test: " + (int32_t)status);
}
 ...
 */
void DateFormatTest::TestDayOfYearPattern195()
{
    UErrorCode status = U_ZERO_ERROR;
    UDate today = Calendar::getNow();
    int32_t year, month, day, hour, min, sec;
    dateToFields(today, year, month, day, hour, min, sec);
...
void DateFormatTest::tryPattern(SimpleDateFormat& sdf, UDate d, const char* pattern, UDate expected)
{
    UErrorCode status = U_ZERO_ERROR;
    if (pattern != 0)
    ...
    (*(DateFormat*)&sdf).format(d, formatResult);
    ...
    if (d2 != expected)
        errln((UnicodeString) "FAIL: Expected " + dateToString(expected));
    UnicodeString format2;
    (*(DateFormat*)&sdf).format(d2, format2);
    logln((UnicodeString) " format -> " + format2);
    if (!(formatResult == format2))
        errln((UnicodeString) "FAIL: Round trip drift");
    //}
    // catch(Exception e) {
    if (U_FAILURE(status))
 ...
 */
void DateFormatTest::TestQuotePattern161()
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    ((DateFormat*)formatter)->format(currentTime_1, dateString);
    UnicodeString exp("08/13/1997 at 10:42:28 AM ");
    logln((UnicodeString) "format(" + dateToString(currentTime_1) + ") = " + dateString);
    if (0 != dateString.compareBetween(0, exp.length(), exp, 0, exp.length()))
        errln((UnicodeString) "FAIL: Expected " + exp);
    delete formatter;
    if (U_FAILURE(status))
        errln((UnicodeString) "FAIL: UErrorCode received during test: " + (int32_t)status);
}
 ...
 */
void DateFormatTest::TestBadInput135()
{
    UErrorCode status = U_ZERO_ERROR;
    DateFormat::EStyle looks[] = { DateFormat::SHORT, DateFormat::MEDIUM, DateFormat::LONG, DateFormat::FULL };
    int32_t looks_length = UPRV_LENGTHOF(looks);
    const char* strings[] = { "Mar 15", "Mar 15 1997", "asdf", "3/1/97 1:23:", "3/1/00 1:23:45 AM" };
...
static const char* const parseFormats[]
    = { "MMMM d, yyyy", "MMMM d yyyy", "M/d/yy", "d MMMM, yyyy", "d MMMM yyyy", "d MMMM", "MMMM d", "yyyy", "h:mm a MMMM d, yyyy" };
 ...
 */
void DateFormatTest::TestBadInput135a()
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    for (uint32_t i = 0; i < INPUT_LENGTH; i += (PF_LENGTH + 1)) {
        ...
        for (uint32_t index = 0; index < PF_LENGTH; ++index) {
            ...
            if (parsePosition.getIndex() != 0) {
                ...
                if (date == 0) {
                    errln((UnicodeString) "ERROR: null result fmt=\"" + parseFormats[index] + "\" pos=" + parsePosition.getIndex() + " " + s1 + "|" + s2);
                } else {
                ...
                errln(UnicodeString("FAIL: Expected ") + expected + " from \"" + s + "\" with \"" + dateParse->toPattern(thePat) + "\"");
            }
            //}
            // catch(Exception ex) {
            if (U_FAILURE(status))
 ...
 */
void DateFormatTest::TestTwoDigitYear()
{
    UErrorCode ec = U_ZERO_ERROR;
...
void DateFormatTest::parse2DigitYear(DateFormat& fmt, const char* str, UDate expected)
{
    UErrorCode status = U_ZERO_ERROR;
    // try {
    UDate d = fmt.parse(str, status);
    UnicodeString thePat;
    logln(UnicodeString("Parsing \"") + str + "\" with " + ((SimpleDateFormat*)&fmt)->toPattern(thePat) + "  => " + dateToString(d));
    if (d != expected)
 ...
 */
void DateFormatTest::TestDateFormatZone061()
{
    UErrorCode status = U_ZERO_ERROR;
    UDate date;
    ...
    if (tempDate != date)
        errln((UnicodeString) "FAIL: Expected " + dateToString(date));
    //}
    // catch(Throwable t) {
    if (U_FAILURE(status))
 ...
 * Test the formatting of time zones.
 */
void DateFormatTest::TestDateFormatZone146()
{
    ...
    GregorianCalendar* greenwichcalendar = new GregorianCalendar(1997, 3, 4, 23, 0, status);
    if (U_FAILURE(status)) {
        ...
        // format every way
        UnicodeString DATA[] = { UnicodeString("simple format:  "), UnicodeString("04/04/97 23:00 GMT"), UnicodeString("MM/dd/yy HH:mm z"),
            UnicodeString("full format:    "), UnicodeString("Friday, April 4, 1997 11:00:00 o'clock PM GMT"),
            UnicodeString("EEEE, MMMM d, yyyy h:mm:ss 'o''clock' a z"), UnicodeString("long format:    "), UnicodeString("April 4, 1997 11:00:00 PM GMT"),
            UnicodeString("MMMM d, yyyy h:mm:ss a z"), UnicodeString("default format: "), UnicodeString("04-Apr-97 11:00:00 PM"),
            UnicodeString("dd-MMM-yy h:mm:ss a"), UnicodeString("short format:   "), UnicodeString("4/4/97 11:00 PM"), UnicodeString("M/d/yy h:mm a") };
 ...
 */
void DateFormatTest::TestLocaleDateFormat() // Bug 495
{
    UDate testDate = date(97, UCAL_SEPTEMBER, 15);
    DateFormat* dfFrench = DateFormat::createDateTimeInstance(DateFormat::FULL, DateFormat::FULL, Locale::getFrench());
    DateFormat* dfUS = DateFormat::createDateTimeInstance(DateFormat::FULL, DateFormat::FULL, Locale::getUS());
...
void DateFormatTest::TestFormattingLocaleTimeSeparator()
{
    ...
    const LocalPointer<DateFormat> dfArab(DateFormat::createTimeInstance(DateFormat::SHORT, Locale("ar", "EG")));

    const LocalPointer<DateFormat> dfLatn(DateFormat::createTimeInstance(DateFormat::SHORT, Locale("ar", "EG", NULL, "numbers=latn")));
    ...
    const UnicodeString expectedArab = UnicodeString("\\u0669:\\u0665\\u0662 \\u0645", -1, US_INV).unescape();

    const UnicodeString expectedLatn = UnicodeString("9:52 \\u0645", -1, US_INV).unescape();
...
void DateFormatTest::TestDateFormatCalendar()
{
    ...
    full = DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, Locale::getUS());
    if (full == NULL) {
    ...
    if (cal == NULL || U_FAILURE(ec)) {
        errln((UnicodeString) "FAIL: Calendar::createInstance failed with " + u_errorName(ec));
        goto FAIL;
    }
    ...
    if (pos.getIndex() != str.length()) {
        errln((UnicodeString) "FAIL: DateFormat::parse(4/5/2001) failed at " + pos.getIndex());
        goto FAIL;
    }
    ...
    if (pos.getIndex() != str.length()) {
        errln((UnicodeString) "FAIL: DateFormat::parse(17:45) failed at " + pos.getIndex());
        goto FAIL;
    }
...
{
    const char* DATA[] = { "yyyy MM dd HH:mm:ss",

        // pattern, input, expected parse or NULL if expect parse failure
        "MMMM d yy", " 04 05 06", "2006 04 05 00:00:00", NULL, "04 05 06", "2006 04 05 00:00:00",

        "MM d yy", " 04 05 06", "2006 04 05 00:00:00", NULL, "04 05 06", "2006 04 05 00:00:00", NULL, "04/05/06", "2006 04 05 00:00:00", NULL, "04-05-06",
        "2006 04 05 00:00:00", NULL, "04.05.06", "2006 04 05 00:00:00", NULL, "04 / 05 / 06", "2006 04 05 00:00:00", NULL, "Apr / 05/ 06",
        "2006 04 05 00:00:00", NULL, "Apr-05-06", "2006 04 05 00:00:00", NULL, "Apr 05, 2006", "2006 04 05 00:00:00",

        "MMMM d yy", " Apr 05 06", "2006 04 05 00:00:00", NULL, "Apr 05 06", "2006 04 05 00:00:00", NULL, "Apr05 06", "2006 04 05 00:00:00",

        "hh:mm:ss a", "12:34:56 PM", "1970 01 01 12:34:56", NULL, "12:34:56PM", "1970 01 01 12:34:56", NULL, "12.34.56PM", "1970 01 01 12:34:56", NULL,
        "12 : 34 : 56  PM", "1970 01 01 12:34:56",

        "MM d yy 'at' hh:mm:ss a", "04/05/06 12:34:56 PM", "2006 04 05 12:34:56",

        "MMMM dd yyyy hh:mm a", "September 27, 1964 21:56 PM", "1964 09 28 09:56:00", NULL, "November 4, 2008 0:13 AM", "2008 11 04 00:13:00",

        "HH'h'mm'min'ss's'", "12h34min56s", "1970 01 01 12:34:56", NULL, "12h34mi56s", "1970 01 01 12:34:56", NULL, "12h34m56s", "1970 01 01 12:34:56", NULL,
 ...
 */
void DateFormatTest::expectParse(const char** data, int32_t data_length, const Locale& loc)
{
    ...
    while (i < data_length) {
        ...
        if (got == exp) {
            logln((UnicodeString) "Ok: " + input + " x " + currentPat + " => " + gotstr);
        } else {
            errln((UnicodeString) "FAIL: " + input + " x " + currentPat + " => " + gotstr + ", expected " + expstr);
        }
    }
}
 ...
 */
void DateFormatTest::expect(const char** data, int32_t data_length, const Locale& loc)
{
    int32_t i = 0;
    UErrorCode ec = U_ZERO_ERROR;
    ...
    while (i < data_length) {
        ...
        if (uprv_strcmp(control, "fp") == 0) {
            ...
            if (!assertSuccess("parse", ec))
                return;
            assertEquals((UnicodeString) "\"" + currentPat + "\".format(" + datestr + ")", ctou(string), fmt.format(date, str.remove()));
            // 'p'
            datestr = data[i++];
            ...
            if (assertSuccess((UnicodeString) "\"" + currentPat + "\".parse(" + string + ")", ec)) {
                assertEquals(
                    (UnicodeString) "\"" + currentPat + "\".parse(" + string + ")", univ.format(date, str.remove()), univ.format(parsedate, str2.remove()));
            }
        }
 ...
 */
void DateFormatTest::expectFormat(const char** data, int32_t data_length, const Locale& loc)
{
    int32_t i = 0;
    UErrorCode ec = U_ZERO_ERROR;
    UnicodeString str, str2;
...
void DateFormatTest::TestGenericTime()
{
    ...
    */
    ...
    SimpleDateFormat formats[] = { SimpleDateFormat(basepat + "vvv", en, status), SimpleDateFormat(basepat + "vvvv", en, status),
        SimpleDateFormat(basepat + "zzz", en, status), SimpleDateFormat(basepat + "zzzz", en, status) };
    if (U_FAILURE(status)) {
    ...
    ASSERT_OK(status);
    const UnicodeString times[] = { "2004 01 02 03:04 PST", "2004 07 08 09:10 PDT" };
    ...
    for (int i = 0; i < times_length; ++i) {
        ...
        for (int j = 0; j < formats_length; ++j) {
            ...
            for (int k = 0; k < formats_length; ++k) {
                ...
                if (U_SUCCESS(status)) {
                    if (d != t) {
                        errln((UnicodeString) "FAIL: format " + k + " incorrectly parsed output of format " + j + " (" + test + "), returned " + dateToString(t)
                            + " instead of " + dateToString(d));
                    } else {
                        logln((UnicodeString) "OK: format " + k + " parsed ok");
                    }
                } else if (status == U_PARSE_ERROR) {
                    errln((UnicodeString) "FAIL: format " + k + " could not parse output of format " + j + " (" + test + ")");
                }
            }
        }
    }
...
void DateFormatTest::TestZTimeZoneParsing(void)
{
    ...
    if (failure(status, "construct SimpleDateFormat", TRUE))
        return;
    ...
    } tests[] = { { "11:00 -0200", "13:00 +0000" }, { "11:00 +0200", "09:00 +0000" }, { "11:00 +0400", "07:00 +0000" }, { "11:00 +0530", "05:30 +0000" } };
    ...
    for (int i = 0; i < tests_length; ++i) {
        ...
        if (pp.getIndex() != tests[i].input.length()) {
            errln("Test %i: setZoneString() did not succeed. Consumed: %i instead of %i", i, pp.getIndex(), tests[i].input.length());
            return;
        }
        result.remove();
        univ.format(d, result);
        if (result != tests[i].expected_result) {
            errln("Expected " + tests[i].expected_result + " got " + result);
            return;
        }
        logln("SUCCESS: Parsed " + tests[i].input + " got " + result + " expected " + tests[i].expected_result);
    }
}
...
void DateFormatTest::TestRelative(int daysdelta, const Locale& loc, const char* expectChars)
{
    char banner[25];
...
void DateFormatTest::TestHebrewClone(void)
{
    ...
    */
    ...
    UDate now = Calendar::getNow();
    LocalPointer<DateFormat> fmt(DateFormat::createDateInstance(DateFormat::kLong, loc));
    if (fmt.isNull()) {
...
static UBool getActualAndValidLocales(const Format& fmt, Locale& valid, Locale& actual)
{
...
void DateFormatTest::TestDateFormatSymbolsClone(void)
{
    ...
    */
    ...
    LocalPointer<DateFormat> fmt(DateFormat::createDateInstance(DateFormat::kDefault, loc));
    if (fmt.isNull()) {
...
void DateFormatTest::TestTimeZoneDisplayName()
{
    ...
    const char* fallbackTests[][6] = {
        ...
        { "zh", "America/Los_Angeles", "2004-01-15T00:00:00Z", "zzzz", "\\u5317\\u7f8e\\u592a\\u5e73\\u6d0b\\u6807\\u51c6\\u65f6\\u95f4",
            "America/Los_Angeles" },
        ...
        { "zh", "America/Los_Angeles", "2004-07-15T00:00:00Z", "zzzz", "\\u5317\\u7f8e\\u592a\\u5e73\\u6d0b\\u590f\\u4ee4\\u65f6\\u95f4",
            "America/Los_Angeles" },
        ...
        { "zh", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u5E03\\u5B9C\\u8BFA\\u65AF\\u827E\\u5229\\u65AF\\u65F6\\u95F4",
        ...
        { "zh", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u5E03\\u5B9C\\u8BFA\\u65AF\\u827E\\u5229\\u65AF\\u65F6\\u95F4",
            "America/Buenos_Aires" },
        ...
        { "hi", "America/Los_Angeles", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0909\\u0924\\u094d\\u0924\\u0930\\u0940 \\u0905\\u092e\\u0947\\u0930\\u093f\\u0915\\u0940 \\u092a\\u094d\\u0930\\u0936\\u093e\\u0902\\u0924 "
            "\\u092e\\u093e\\u0928\\u0915 \\u0938\\u092e\\u092f",
            "-8:00" },
        ...
        { "hi", "America/Los_Angeles", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0909\\u0924\\u094d\\u0924\\u0930\\u0940 \\u0905\\u092e\\u0947\\u0930\\u093f\\u0915\\u0940 \\u092a\\u094d\\u0930\\u0936\\u093e\\u0902\\u0924 "
            "\\u0921\\u0947\\u0932\\u093e\\u0907\\u091f \\u0938\\u092e\\u092f",
            "-7:00" },
        { "hi", "America/Los_Angeles", "2004-07-15T00:00:00Z", "v",
            "\\u0932\\u0949\\u0938 \\u090f\\u0902\\u091c\\u093f\\u0932\\u094d\\u0938 \\u0938\\u092e\\u092f", "America/Los_Angeles" },
        { "hi", "America/Los_Angeles", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0909\\u0924\\u094d\\u0924\\u0930\\u0940 \\u0905\\u092e\\u0947\\u0930\\u093f\\u0915\\u0940 \\u092a\\u094d\\u0930\\u0936\\u093e\\u0902\\u0924 "
            "\\u0938\\u092e\\u092f",
            "America/Los_Angeles" },
        ...
        { "hi", "America/Argentina/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "-3:00" },
        ...
        { "hi", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "-3:00" },
        { "hi", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "v",
            "\\u092C\\u094D\\u092F\\u0942\\u0928\\u0938 \\u0906\\u092F\\u0930\\u0938 \\u0938\\u092E\\u092F", "America/Buenos_Aires" },
        { "hi", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F",
            "America/Buenos_Aires" },
        ...
        { "hi", "America/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "-3:00" },
        ...
        { "hi", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "-3:00" },
        { "hi", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "v",
            "\\u092C\\u094D\\u092F\\u0942\\u0928\\u0938 \\u0906\\u092F\\u0930\\u0938 \\u0938\\u092E\\u092F", "America/Buenos_Aires" },
        { "hi", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0905\\u0930\\u094D\\u091C\\u0947\\u0902\\u091F\\u0940\\u0928\\u093E \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F",
            "America/Buenos_Aires" },
        ...
        { "hi", "America/Havana", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0915\\u094d\\u092f\\u0942\\u092c\\u093e \\u092e\\u093e\\u0928\\u0915 \\u0938\\u092e\\u092f", "-5:00" },
        ...
        { "hi", "America/Havana", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0915\\u094d\\u092f\\u0942\\u092c\\u093e \\u0921\\u0947\\u0932\\u093e\\u0907\\u091f \\u0938\\u092e\\u092f", "-4:00" },
        ...
        { "hi", "Australia/ACT", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0911\\u0938\\u094d\\u200d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e\\u0908 \\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 "
            "\\u0921\\u0947\\u0932\\u093e\\u0907\\u091f \\u0938\\u092e\\u092f",
            "+11:00" },
        ...
        { "hi", "Australia/ACT", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0911\\u0938\\u094d\\u200d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e\\u0908 \\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 "
            "\\u092e\\u093e\\u0928\\u0915 "
            "\\u0938\\u092e\\u092f",
            "+10:00" },
        { "hi", "Australia/ACT", "2004-07-15T00:00:00Z", "v", "\\u0938\\u093F\\u0921\\u0928\\u0940 \\u0938\\u092E\\u092F", "Australia/Sydney" },
        { "hi", "Australia/ACT", "2004-07-15T00:00:00Z", "vvvv",
            "\\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 \\u0911\\u0938\\u094d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e \\u0938\\u092e\\u092f",
            "Australia/Sydney" },
        ...
        { "hi", "Australia/Sydney", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0911\\u0938\\u094d\\u200d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e\\u0908 \\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 "
            "\\u0921\\u0947\\u0932\\u093e\\u0907\\u091f \\u0938\\u092e\\u092f",
            "+11:00" },
        ...
        { "hi", "Australia/Sydney", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0911\\u0938\\u094d\\u200d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e\\u0908 \\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 "
            "\\u092e\\u093e\\u0928\\u0915 "
            "\\u0938\\u092e\\u092f",
            "+10:00" },
        { "hi", "Australia/Sydney", "2004-07-15T00:00:00Z", "v", "\\u0938\\u093F\\u0921\\u0928\\u0940 \\u0938\\u092E\\u092F", "Australia/Sydney" },
        { "hi", "Australia/Sydney", "2004-07-15T00:00:00Z", "vvvv",
            "\\u092a\\u0942\\u0930\\u094d\\u0935\\u0940 \\u0911\\u0938\\u094d\\u091f\\u094d\\u0930\\u0947\\u0932\\u093f\\u092f\\u093e \\u0938\\u092e\\u092f",
            "Australia/Sydney" },
        ...
        { "hi", "Europe/London", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0917\\u094d\\u0930\\u0940\\u0928\\u0935\\u093f\\u091a \\u092e\\u0940\\u0928 \\u091f\\u093e\\u0907\\u092e", "+0:00" },
        ...
        { "hi", "Europe/London", "2004-07-15T00:00:00Z", "zzzz",
            "\\u092c\\u094d\\u0930\\u093f\\u091f\\u093f\\u0936 \\u0917\\u094d\\u0930\\u0940\\u0937\\u094d\\u092e\\u0915\\u093e\\u0932\\u0940\\u0928 "
            "\\u0938\\u092e\\u092f",
            "+1:00" },
        { "hi", "Europe/London", "2004-07-15T00:00:00Z", "v",
            "\\u092f\\u0942\\u0928\\u093e\\u0907\\u091f\\u0947\\u0921 \\u0915\\u093f\\u0902\\u0917\\u0921\\u092e \\u0938\\u092e\\u092f", "Europe/London" },
        { "hi", "Europe/London", "2004-07-15T00:00:00Z", "vvvv",
            "\\u092f\\u0942\\u0928\\u093e\\u0907\\u091f\\u0947\\u0921 \\u0915\\u093f\\u0902\\u0917\\u0921\\u092e \\u0938\\u092e\\u092f", "Europe/London" },
        ...
        { "hi", "Asia/Calcutta", "2004-01-15T00:00:00Z", "zzzz",
            "\\u092D\\u093E\\u0930\\u0924\\u0940\\u092F \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "+5:30" },
        ...
        { "hi", "Asia/Calcutta", "2004-07-15T00:00:00Z", "zzzz",
            "\\u092D\\u093E\\u0930\\u0924\\u0940\\u092F \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "+5:30" },
        { "hi", "Asia/Calcutta", "2004-07-15T00:00:00Z", "v", "IST", "Asia/Calcutta" },
        { "hi", "Asia/Calcutta", "2004-07-15T00:00:00Z", "vvvv",
            "\\u092D\\u093E\\u0930\\u0924\\u0940\\u092F \\u092E\\u093E\\u0928\\u0915 \\u0938\\u092E\\u092F", "Asia/Calcutta" },
        ...
        { "bg", "America/Los_Angeles", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0421\\u0435\\u0432\\u0435\\u0440\\u043d\\u043e\\u0430\\u043c\\u0435\\u0440\\u0438\\u043a\\u0430\\u043d\\u0441\\u043a\\u043e "
            "\\u0442\\u0438\\u0445\\u043e\\u043e\\u043a\\u0435\\u0430\\u043d\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "America/Los_Angeles" },
        ...
        { "bg", "America/Los_Angeles", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0421\\u0435\\u0432\\u0435\\u0440\\u043d\\u043e\\u0430\\u043c\\u0435\\u0440\\u0438\\u043a\\u0430\\u043d\\u0441\\u043a\\u043e "
            "\\u0442\\u0438\\u0445\\u043e\\u043e\\u043a\\u0435\\u0430\\u043d\\u0441\\u043a\\u043e \\u043b\\u044f\\u0442\\u043d\\u043e "
            "\\u0447\\u0430\\u0441\\u043e\\u0432\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "America/Los_Angeles" },
        // icu bg.txt has exemplar city for this time zone
        { "bg", "America/Los_Angeles", "2004-07-15T00:00:00Z", "v", "\\u041B\\u043E\\u0441 \\u0410\\u043D\\u0434\\u0436\\u0435\\u043B\\u0438\\u0441",
            "America/Los_Angeles" },
        { "bg", "America/Los_Angeles", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0421\\u0435\\u0432\\u0435\\u0440\\u043d\\u043e\\u0430\\u043c\\u0435\\u0440\\u0438\\u043a\\u0430\\u043d\\u0441\\u043a\\u043e "
            "\\u0442\\u0438\\u0445\\u043e\\u043e\\u043a\\u0435\\u0430\\u043d\\u0441\\u043a\\u043e \\u0432\\u0440\\u0435\\u043c\\u0435",
            "America/Los_Angeles" },
        { "bg", "America/Los_Angeles", "2004-07-15T00:00:00Z", "VVVV", "\\u041B\\u043E\\u0441 \\u0410\\u043D\\u0434\\u0436\\u0435\\u043B\\u0438\\u0441",
            "America/Los_Angeles" },
        ...
        { "bg", "America/Argentina/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-3:00" },
        ...
        { "bg", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-3:00" },
        { "bg", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u0411\\u0443\\u0435\\u043D\\u043E\\u0441 \\u0410\\u0439\\u0440\\u0435\\u0441",
            "America/Buenos_Aires" },
        { "bg", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "America/Buenos_Aires" },
        ...
        { "bg", "America/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-3:00" },
        ...
        { "bg", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-3:00" },
        // icu bg.txt does not have info for this time zone
        { "bg", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u0411\\u0443\\u0435\\u043D\\u043E\\u0441 \\u0410\\u0439\\u0440\\u0435\\u0441",
            "America/Buenos_Aires" },
        { "bg", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0410\\u0440\\u0436\\u0435\\u043D\\u0442\\u0438\\u043D\\u0441\\u043a\\u043e "
            "\\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "America/Buenos_Aires" },
        ...
        { "bg", "America/Havana", "2004-01-15T00:00:00Z", "zzzz",
            "\\u041a\\u0443\\u0431\\u0438\\u043d\\u0441\\u043a\\u043e \\u0441\\u0442\\u0430\\u043d\\u0434\\u0430\\u0440\\u0442\\u043d\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-5:00" },
        ...
        { "bg", "America/Havana", "2004-07-15T00:00:00Z", "zzzz",
            "\\u041a\\u0443\\u0431\\u0438\\u043d\\u0441\\u043a\\u043e \\u043b\\u044f\\u0442\\u043d\\u043e \\u0447\\u0430\\u0441\\u043e\\u0432\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "-4:00" },
        { "bg", "America/Havana", "2004-07-15T00:00:00Z", "v", "\\u041a\\u0443\\u0431\\u0430", "America/Havana" },
        { "bg", "America/Havana", "2004-07-15T00:00:00Z", "vvvv",
            "\\u041a\\u0443\\u0431\\u0438\\u043d\\u0441\\u043a\\u043e \\u0432\\u0440\\u0435\\u043C\\u0435", "America/Havana" },
        ...
        { "bg", "Australia/ACT", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u043B\\u044F\\u0442\\u043D\\u043E \\u0447\\u0430\\u0441\\u043E\\u0432\\u043E \\u0432\\u0440\\u0435\\u043C\\u0435",
            "+11:00" },
        ...
        { "bg", "Australia/ACT", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u0441\\u0442\\u0430\\u043D\\u0434\\u0430\\u0440\\u0442\\u043D\\u043E \\u0432\\u0440\\u0435\\u043C\\u0435",
            "+10:00" },
        { "bg", "Australia/ACT", "2004-07-15T00:00:00Z", "v", "\\u0421\\u0438\\u0434\\u043D\\u0438", "Australia/Sydney" },
        { "bg", "Australia/ACT", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u0432\\u0440\\u0435\\u043C\\u0435",
            "Australia/Sydney" },
        ...
        { "bg", "Australia/Sydney", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u043B\\u044F\\u0442\\u043D\\u043E \\u0447\\u0430\\u0441\\u043E\\u0432\\u043E \\u0432\\u0440\\u0435\\u043C\\u0435",
            "+11:00" },
        ...
        { "bg", "Australia/Sydney", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u0441\\u0442\\u0430\\u043D\\u0434\\u0430\\u0440\\u0442\\u043D\\u043E \\u0432\\u0440\\u0435\\u043C\\u0435",
            "+10:00" },
        { "bg", "Australia/Sydney", "2004-07-15T00:00:00Z", "v", "\\u0421\\u0438\\u0434\\u043D\\u0438", "Australia/Sydney" },
        { "bg", "Australia/Sydney", "2004-07-15T00:00:00Z", "vvvv",
            "\\u0418\\u0437\\u0442\\u043E\\u0447\\u043D\\u043E\\u0430\\u0432\\u0441\\u0442\\u0440\\u0430\\u043B\\u0438\\u0439\\u0441\\u043A\\u043E "
            "\\u0432\\u0440\\u0435\\u043C\\u0435",
            "Australia/Sydney" },
        ...
        { "bg", "Europe/London", "2004-01-15T00:00:00Z", "zzzz",
            "\\u0421\\u0440\\u0435\\u0434\\u043d\\u043e \\u0433\\u0440\\u0438\\u043d\\u0443\\u0438\\u0447\\u043a\\u043e \\u0432\\u0440\\u0435\\u043c\\u0435",
            "+0:00" },
        ...
        { "bg", "Europe/London", "2004-07-15T00:00:00Z", "zzzz",
            "\\u0411\\u0440\\u0438\\u0442\\u0430\\u043d\\u0441\\u043a\\u043e \\u043b\\u044f\\u0442\\u043d\\u043e \\u0447\\u0430\\u0441\\u043e\\u0432\\u043e "
            "\\u0432\\u0440\\u0435\\u043c\\u0435",
            "+1:00" },
        { "bg", "Europe/London", "2004-07-15T00:00:00Z", "v",
            "\\u041E\\u0431\\u0435\\u0434\\u0438\\u043D\\u0435\\u043D\\u043E\\u0442\\u043E \\u043A\\u0440\\u0430\\u043B\\u0441\\u0442\\u0432\\u043E",
            "Europe/London" },
        { "bg", "Europe/London", "2004-07-15T00:00:00Z", "vvvv",
            "\\u041E\\u0431\\u0435\\u0434\\u0438\\u043D\\u0435\\u043D\\u043E\\u0442\\u043E \\u043A\\u0440\\u0430\\u043B\\u0441\\u0442\\u0432\\u043E",
            "Europe/London" },
        ...
        { "bg", "Asia/Calcutta", "2004-01-15T00:00:00Z", "zzzz", "\\u0418\\u043D\\u0434\\u0438\\u0439\\u0441\\u043A\\u043E \\u0432\\u0440\\u0435\\u043c\\u0435",
            "+5:30" },
        ...
        { "bg", "Asia/Calcutta", "2004-07-15T00:00:00Z", "zzzz", "\\u0418\\u043D\\u0434\\u0438\\u0439\\u0441\\u043A\\u043E \\u0432\\u0440\\u0435\\u043c\\u0435",
            "+5:30" },
        { "bg", "Asia/Calcutta", "2004-07-15T00:00:00Z", "v", "\\u0418\\u043D\\u0434\\u0438\\u044F", "Asia/Calcutta" },
        { "bg", "Asia/Calcutta", "2004-07-15T00:00:00Z", "vvvv", "\\u0418\\u043D\\u0434\\u0438\\u0439\\u0441\\u043A\\u043E \\u0432\\u0440\\u0435\\u043c\\u0435",
            "Asia/Calcutta" },
        // ==========
        ...
        { "ja", "America/Los_Angeles", "2004-01-15T00:00:00Z", "zzzz", "\\u30a2\\u30e1\\u30ea\\u30ab\\u592a\\u5e73\\u6d0b\\u6a19\\u6e96\\u6642",
            "America/Los_Angeles" },
        ...
        { "ja", "America/Los_Angeles", "2004-07-15T00:00:00Z", "zzzz", "\\u30a2\\u30e1\\u30ea\\u30ab\\u592a\\u5e73\\u6d0b\\u590f\\u6642\\u9593",
            "America/Los_Angeles" },
        ...
        { "ja", "America/Los_Angeles", "2004-07-15T00:00:00Z", "vvvv", "\\u30A2\\u30E1\\u30EA\\u30AB\\u592A\\u5e73\\u6D0B\\u6642\\u9593",
            "America/Los_Angeles" },
        ...
        { "ja", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u30D6\\u30A8\\u30CE\\u30B9\\u30A2\\u30A4\\u30EC\\u30B9\\u6642\\u9593",
            "America/Buenos_Aires" },
        { "ja", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv", "\\u30A2\\u30EB\\u30BC\\u30F3\\u30C1\\u30F3\\u6A19\\u6E96\\u6642",
            "America/Buenos_Aires" },
        ...
        { "ja", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u30D6\\u30A8\\u30CE\\u30B9\\u30A2\\u30A4\\u30EC\\u30B9\\u6642\\u9593",
            "America/Buenos_Aires" },
        { "ja", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv", "\\u30A2\\u30EB\\u30BC\\u30F3\\u30C1\\u30F3\\u6A19\\u6E96\\u6642",
            "America/Buenos_Aires" },
        ...
        { "ja", "Australia/ACT", "2004-01-15T00:00:00Z", "zzzz", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u590F\\u6642\\u9593",
            "+11:00" },
        ...
        { "ja", "Australia/ACT", "2004-07-15T00:00:00Z", "zzzz", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u6A19\\u6E96\\u6642",
            "+10:00" },
        // icu ja.txt does not have info for this time zone
        { "ja", "Australia/ACT", "2004-07-15T00:00:00Z", "v", "\\u30B7\\u30C9\\u30CB\\u30FC\\u6642\\u9593", "Australia/Sydney" },
        { "ja", "Australia/ACT", "2004-07-15T00:00:00Z", "vvvv", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u6642\\u9593",
            "Australia/Sydney" },
        ...
        { "ja", "Australia/Sydney", "2004-01-15T00:00:00Z", "zzzz", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u590F\\u6642\\u9593",
            "+11:00" },
        ...
        { "ja", "Australia/Sydney", "2004-07-15T00:00:00Z", "zzzz", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u6A19\\u6E96\\u6642",
            "+10:00" },
        { "ja", "Australia/Sydney", "2004-07-15T00:00:00Z", "v", "\\u30B7\\u30C9\\u30CB\\u30FC\\u6642\\u9593", "Australia/Sydney" },
        { "ja", "Australia/Sydney", "2004-07-15T00:00:00Z", "vvvv", "\\u30AA\\u30FC\\u30B9\\u30C8\\u30E9\\u30EA\\u30A2\\u6771\\u90E8\\u6642\\u9593",
            "Australia/Sydney" },
        ...
        { "ti", "America/Los_Angeles", "2004-07-15T00:00:00Z", "zzzz", "GMT-07:00", "-7:00" },
        { "ti", "America/Los_Angeles", "2004-07-15T00:00:00Z", "v", "\\u130D\\u12DC \\u120E\\u1235 \\u12A3\\u1295\\u1300\\u1208\\u1235",
            "America/Los_Angeles" },
        { "ti", "America/Los_Angeles", "2004-07-15T00:00:00Z", "vvvv", "\\u130D\\u12DC \\u120E\\u1235 \\u12A3\\u1295\\u1300\\u1208\\u1235",
            "America/Los_Angeles" },
        ...
        { "ti", "America/Argentina/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz",
            "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293", "-3:00" },
        ...
        { "ti", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz",
            "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293", "-3:00" },
        { "ti", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u130D\\u12DC \\u1265\\u12C8\\u1296\\u1235 \\u12A3\\u12ED\\u1228\\u1235",
            "America/Buenos_Aires" },
        { "ti", "America/Argentina/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv",
            "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293", "America/Buenos_Aires" },
        ...
        { "ti", "America/Buenos_Aires", "2004-01-15T00:00:00Z", "zzzz", "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293",
            "-3:00" },
        ...
        { "ti", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "zzzz", "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293",
            "-3:00" },
        { "ti", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "v", "\\u130D\\u12DC \\u1265\\u12C8\\u1296\\u1235 \\u12A3\\u12ED\\u1228\\u1235",
            "America/Buenos_Aires" },
        { "ti", "America/Buenos_Aires", "2004-07-15T00:00:00Z", "vvvv", "\\u121D\\u12F1\\u1265 \\u130D\\u12DC \\u12A3\\u122D\\u1300\\u1295\\u1272\\u1293",
            "America/Buenos_Aires" },
    ...
    if (failure(status, "GregorianCalendar::createInstance", TRUE))
        return;
    SimpleDateFormat testfmt(UnicodeString("yyyy-MM-dd'T'HH:mm:ss'Z'"), status);
    if (failure(status, "SimpleDateFormat constructor", TRUE))
        return;
...
void DateFormatTest::TestTimeZoneInLocale()
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(tests); ++i) {
        ...
        ASSERT_OK(status);
        assertEquals("TimeZone from SimpleDateFormat constructor", expectedTimezone, smptfmt.getTimeZone().getID(actual));
        assertEquals("Calendar from SimpleDateFormat constructor", testLine[2], smptfmt.getCalendar()->getType());

        LocalPointer<DateFormat> datefmt(DateFormat::createDateInstance(DateFormat::kDefault, locale));
        if (datefmt == nullptr) {
            dataerrln("Error calling DateFormat::createDateInstance()");
            return;
        }
        assertEquals("TimeZone from DateFormat::createDateInstance", expectedTimezone, datefmt->getTimeZone().getID(actual));
        assertEquals("Calendar from DateFormat::createDateInstance", testLine[2], datefmt->getCalendar()->getType());
        LocalPointer<DateFormat> timefmt(DateFormat::createTimeInstance(DateFormat::kDefault, locale));
        if (timefmt == nullptr) {
            dataerrln("Error calling DateFormat::createTimeInstance()");
            return;
        }
        assertEquals("TimeZone from TimeFormat::createTimeInstance", expectedTimezone, timefmt->getTimeZone().getID(actual));
        assertEquals("Calendar from DateFormat::createTimeInstance", testLine[2], timefmt->getCalendar()->getType());

        LocalPointer<DateFormat> datetimefmt(DateFormat::createDateTimeInstance(DateFormat::kDefault, DateFormat::kDefault, locale));
        if (datetimefmt == nullptr) {
            dataerrln("Error calling DateFormat::createDateTimeInstance()");
            return;
        }
        assertEquals("TimeZone from DateTimeFormat::createDateTimeInstance", expectedTimezone, datetimefmt->getTimeZone().getID(actual));
        assertEquals("Calendar from DateFormat::createDateTimeInstance", testLine[2], datetimefmt->getCalendar()->getType());
    }
}
...
void DateFormatTest::TestRoundtripWithCalendar(void)
{
    ...
    Calendar* calendars[] = { Calendar::createInstance(*tz, Locale("und@calendar=gregorian"), status),
        ...
        Calendar::createInstance(*tz, Locale("und@calendar=islamic"), status), Calendar::createInstance(*tz, Locale("und@calendar=japanese"), status), NULL };
    if (U_FAILURE(status)) {
    ...
    DateFormat* formatters[] = { DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, Locale("en_US")), // calendar=gregorian
    ...
    for (int i = 0; formatters[i] != NULL; i++) {
        ...
        for (int j = 0; calendars[j] != NULL; j++) {
            ...
            if (calendars[i]->getTime(status) != calendars[j]->getTime(status) || calendars[i]->getTimeZone() != calendars[j]->getTimeZone()) {
                UnicodeString tzid;
...
void DateFormatTest::Test6338(void)
{
    ...
    if (str1 != str11) {
        errln((UnicodeString) "FAIL: Different dates str1:" + str1 + " str2:" + str11);
    }
    ...
    if (str2 != str22) {
        errln((UnicodeString) "FAIL: Different dates str1:" + str2 + " str2:" + str22);
    }
    ...
    if (str3 != str33) {
        errln((UnicodeString) "FAIL: Different dates str1:" + str3 + " str2:" + str33);
    }
    ...
    if (str4 != str44) {
        errln((UnicodeString) "FAIL: Different dates str1:" + str4 + " str2:" + str44);
    }
}
...
void DateFormatTest::Test6880()
{
    ...
    if (failure(status, "construct GregorianCalendar", TRUE))
        return;
...
{
    const NumAsStringItem items[] = { // loc lenient fail?  datePattern                                         dateString
        ...
        { "ko", TRUE, FALSE, CharsToUnicodeString("y\\uB144 MMM d\\uC77C"),
            CharsToUnicodeString("2009\\uB144 7\\uC6D4 14\\uC77C") }, // #8820 fixes test failure
    ...
    for (itemPtr = items; itemPtr->localeStr != NULL; itemPtr++) {
        ...
        formatter.setBooleanAttribute(UDAT_PARSE_ALLOW_WHITESPACE, itemPtr->lenient, status)
            .setBooleanAttribute(UDAT_PARSE_ALLOW_NUMERIC, itemPtr->lenient, status);
        ...
        if (U_FAILURE(status)) {
            ...
            errln("FAIL, expected err but got none: Locale \"" + UnicodeString(itemPtr->localeStr) + "\", lenient " + itemPtr->lenient + ": using pattern \""
                + itemPtr->datePattern + "\", did parse \"" + itemPtr->dateString + "\".");
...
void DateFormatTest::TestISOEra()
{
    ...
    const char* data[] = {
        // input, output
        "BC 4004-10-23T07:00:00Z",
        "BC 4004-10-23T07:00:00Z",
        "AD 4004-10-23T07:00:00Z",
        "AD 4004-10-23T07:00:00Z",
        "-4004-10-23T07:00:00Z",
        "BC 4005-10-23T07:00:00Z",
        "4004-10-23T07:00:00Z",
        "AD 4004-10-23T07:00:00Z",
    };
...
void DateFormatTest::TestParsePosition()
{
    ...
    for (int32_t i = 0; TestData[i][0]; i++) {
        ...
        if (pos.getIndex() != resPos) {
            errln(UnicodeString("FAIL: Parsing [") + input + "] with pattern [" + TestData[i][0] + "] returns position - " + pos.getIndex() + ", expected - "
                + resPos);
        }
    }
}
...
void DateFormatTest::TestMonthPatterns()
{
    ...
    const MonthPatternItem items[] = { // locale                     date style;           expected formats for the 3 dates above
        { "root@calendar=chinese", DateFormat::kLong,
            { UnicodeString("2012(ren-chen) M04 2"), UnicodeString("2012(ren-chen) M04bis 2"), UnicodeString("2012(ren-chen) M05 2") } },
        ...
        { "en@calendar=chinese", DateFormat::kLong,
            { UnicodeString("Fourth Month 2, 2012(ren-chen)"), UnicodeString("Fourth Monthbis 2, 2012(ren-chen)"),
                UnicodeString("Fifth Month 2, 2012(ren-chen)") } },
        { "en@calendar=chinese", DateFormat::kShort, { UnicodeString("4/2/2012"), UnicodeString("4bis/2/2012"), UnicodeString("5/2/2012") } },
        { "zh@calendar=chinese", DateFormat::kLong,
            { CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u56DB\\u6708\\u521D\\u4E8C"),
                CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u95F0\\u56DB\\u6708\\u521D\\u4E8C"),
                CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u4E94\\u6708\\u521D\\u4E8C") } },
        { "zh@calendar=chinese", DateFormat::kShort,
            { CharsToUnicodeString("2012/4/2"), CharsToUnicodeString("2012/\\u95F04/2"), CharsToUnicodeString("2012/5/2") } },
        { "zh@calendar=chinese", -3,
            { CharsToUnicodeString("\\u58EC\\u8FB0-4-2"), CharsToUnicodeString("\\u58EC\\u8FB0-\\u95F04-2"), CharsToUnicodeString("\\u58EC\\u8FB0-5-2") } },
        { "zh@calendar=chinese", -4,
            { CharsToUnicodeString("\\u58EC\\u8FB0 \\u56DB\\u6708 2"), CharsToUnicodeString("\\u58EC\\u8FB0 \\u95F0\\u56DB\\u6708 2"),
                CharsToUnicodeString("\\u58EC\\u8FB0 \\u4E94\\u6708 2") } },
        { "zh_Hant@calendar=chinese", DateFormat::kLong,
            { CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u56DB\\u6708\\u521D\\u4E8C"),
                CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u958F\\u56DB\\u6708\\u521D\\u4E8C"),
                CharsToUnicodeString("2012\\u58EC\\u8FB0\\u5E74\\u4E94\\u6708\\u521D\\u4E8C") } },
        { "zh_Hant@calendar=chinese", DateFormat::kShort,
            { CharsToUnicodeString("2012/4/2"), CharsToUnicodeString("2012/\\u958F4/2"), CharsToUnicodeString("2012/5/2") } },
        { "fr@calendar=chinese", DateFormat::kLong,
            { CharsToUnicodeString("2 s\\u00ECyu\\u00E8 ren-chen"), CharsToUnicodeString("2 s\\u00ECyu\\u00E8bis ren-chen"),
        ...
        { "en@calendar=dangi", DateFormat::kLong,
            { UnicodeString("Third Monthbis 2, 2012(ren-chen)"), UnicodeString("Fourth Month 2, 2012(ren-chen)"),
                UnicodeString("Fifth Month 1, 2012(ren-chen)") } },
        ...
        { "ko@calendar=dangi", DateFormat::kLong,
            { CharsToUnicodeString("\\uC784\\uC9C4\\uB144 \\uC7243\\uC6D4 2\\uC77C"), CharsToUnicodeString("\\uC784\\uC9C4\\uB144 4\\uC6D4 2\\uC77C"),
                CharsToUnicodeString("\\uC784\\uC9C4\\uB144 5\\uC6D4 1\\uC77C") } },
        { "ko@calendar=dangi", DateFormat::kShort,
            { CharsToUnicodeString("29. \\uC7243. 2."), CharsToUnicodeString("29. 4. 2."), CharsToUnicodeString("29. 5. 1.") } },
        // terminator
    ...
    if (U_SUCCESS(status)) {
        ...
        for (itemPtr = items; itemPtr->locale != NULL; itemPtr++) {
            ...
            DateFormat* dmft = (itemPtr->style >= 0) ? DateFormat::createDateInstance((DateFormat::EStyle)itemPtr->style, locale)
                ...
                if (U_SUCCESS(status)) {
                    ...
                    for (idate = 0; idate < NUM_TEST_DATES; idate++, datePtr++) {
                        ...
                        if (result.compare(itemPtr->dateString[idate]) != 0) {
                            ...
                            if (ppos.getIndex() < result.length() || year != datePtr->year || month != datePtr->month || isLeapMonth != datePtr->isLeapMonth
                                || day != datePtr->day) {
                                errln(UnicodeString("FAIL: Chinese calendar parse for locale ") + UnicodeString(itemPtr->locale) + ", style " + itemPtr->style
                                    + ", string \"" + result + "\", expected " + datePtr->year + "-" + datePtr->month + "(" + datePtr->isLeapMonth + ")-"
                                    + datePtr->day + ", got pos " + ppos.getIndex() + " " + year + "-" + month + "(" + isLeapMonth + ")-" + day);
                            }
                        }
                    }
                } else {
        ...
        for (itemPtr = items; itemPtr->locale != NULL; itemPtr++) {
            ...
            if (U_FAILURE(status)) {
                ...
                if (result.compare(itemPtr->expectedFormat) != 0) {
                    errln(UnicodeString("FAIL: format for locale ") + UnicodeString(itemPtr->locale) + ", status " + (int)status + ", capitalizationContext "
                        + (int)itemPtr->capitalizationContext + ", expected " + itemPtr->expectedFormat + ", got " + result);
                }
            }
            if (sdmft) {
                delete sdmft;
...
void DateFormatTest::TestNonGregoFmtParse()
{
    ...
    const CalAndFmtTestItem cafti_zh_chinese_custU[] = {
        ...
        { 77, 31, 0, 1, 12, 0, CharsToUnicodeString("1954\\u7532\\u5348\\u5E74\\u6B63\\u67081") }, { 0, 0, 0, 0, 0, 0, UnicodeString("") } // terminator
    };
    const CalAndFmtTestItem cafti_zh_chinese_custNoU[] = {
        { 78, 31, 0, 1, 12, 0, CharsToUnicodeString("2014\\u5E74\\u6B63\\u67081") },
        { 77, 31, 0, 1, 12, 0, CharsToUnicodeString("1954\\u5E74\\u6B63\\u67081") }, { 0, 0, 0, 0, 0, 0, UnicodeString("") } // terminator
    };
    const CalAndFmtTestItem cafti_ja_japanese_custGy[] = {
        { 235, 26, 2, 5, 12, 0, CharsToUnicodeString("2014(\\u5E73\\u621026)\\u5E743\\u67085\\u65E5") },
        { 234, 60, 2, 5, 12, 0, CharsToUnicodeString("1985(\\u662D\\u548C60)\\u5E743\\u67085\\u65E5") }, { 0, 0, 0, 0, 0, 0, UnicodeString("") } // terminator
    };
    const CalAndFmtTestItem cafti_ja_japanese_custNoGy[] = {
        { 235, 26, 2, 5, 12, 0, CharsToUnicodeString("2014\\u5E743\\u67085\\u65E5") },
        { 234, 60, 2, 5, 12, 0, CharsToUnicodeString("1985\\u5E743\\u67085\\u65E5") }, { 0, 0, 0, 0, 0, 0, UnicodeString("") } // terminator
    };
    const CalAndFmtTestItem cafti_en_islamic_cust[] = {
        { 0, 1384, 0, 1, 12, 0, UnicodeString("1 Muh. 1384 AH, 1964") }, { 0, 1436, 0, 1, 12, 0, UnicodeString("1 Muh. 1436 AH, 2014") },
        { 0, 1487, 0, 1, 12, 0, UnicodeString("1 Muh. 1487 AH, 2064") }, { 0, 0, 0, 0, 0, 0, UnicodeString("") } // terminator
    };
    // overal test items
            ...
            if (!cal.isValid()) {
                ...
                for (caftItemPtr = itemPtr->caftItems; caftItemPtr->year != 0; caftItemPtr++) {
                    ...
                    if (result.compare(caftItemPtr->formattedDate) != 0) {
                        errln(UnicodeString("FAIL: date format for locale ") + UnicodeString(itemPtr->locale) + ", style " + itemPtr->style + ", expected \""
                            + caftItemPtr->formattedDate + "\", got \"" + result + "\"");
                    } else {
                        // formatted OK, try parse
                        ParsePosition ppos(0);
                        ...
                        if (U_FAILURE(status) || ppos.getIndex() < result.length() || era != caftItemPtr->era || year != caftItemPtr->year
                            || month != caftItemPtr->month || day != caftItemPtr->day) {
                            errln(UnicodeString("FAIL: date parse for locale ") + UnicodeString(itemPtr->locale) + ", style " + itemPtr->style + ", string \""
                                + result + "\", expected " + caftItemPtr->era + ":" + caftItemPtr->year + "-" + caftItemPtr->month + "-" + caftItemPtr->day
                                + ", got pos " + ppos.getIndex() + " " + year + "-" + month + "-" + day + " status " + UnicodeString(u_errorName(status)));
                        }
                    }
                }
            }
...
void DateFormatTest::TestFormatsWithNumberSystems()
{
    ...
    const TestFmtWithNumSysItem items[] = {
        ...
        { "he@calendar=hebrew", DateFormat::kLong, CharsToUnicodeString("d \\u05D1MMMM y"),
            CharsToUnicodeString("\\u05D9\\u05F4\\u05D8 \\u05D1\\u05D8\\u05D1\\u05EA \\u05EA\\u05E9\\u05E2\\u05F4\\u05D5") },
        { "zh@calendar=chinese", DateFormat::kLong, CharsToUnicodeString("rU\\u5E74MMMd"),
            CharsToUnicodeString("2015\\u4E59\\u672A\\u5E74\\u5341\\u4E00\\u6708\\u5EFF\\u4E00") },
        { "zh_Hant@calendar=chinese", DateFormat::kLong, CharsToUnicodeString("rU\\u5E74MMMd"),
            CharsToUnicodeString("2015\\u4E59\\u672A\\u5E74\\u51AC\\u6708\\u5EFF\\u4E00") },
        { "ja@calendar=chinese", DateFormat::kLong, CharsToUnicodeString("U\\u5E74MMMd\\u65E5"),
            CharsToUnicodeString("\\u4E59\\u672A\\u5E74\\u5341\\u4E00\\u6708\\u4E8C\\u4E00\\u65E5") },
...
void DateFormatTest::TestDotAndAtLeniency()
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(locales); ++i) {
        ...
        for (DateFormat::EStyle dateStyle = DateFormat::FULL; dateStyle <= DateFormat::SHORT; dateStyle = static_cast<DateFormat::EStyle>(dateStyle + 1)) {
            LocalPointer<DateFormat> dateFormat(DateFormat::createDateInstance(dateStyle, locale));

            for (DateFormat::EStyle timeStyle = DateFormat::FULL; timeStyle <= DateFormat::SHORT; timeStyle = static_cast<DateFormat::EStyle>(timeStyle + 1)) {
...
void DateFormatTest::TestDateFormatLeniency()
{
    ...
    const UDate july022008 = 1215000001979.0;
    const TestDateFormatLeniencyItem items[] = { // locale    leniency    parse String                    pattern                             expected result
    ...
    for (itemPtr = items; itemPtr->locale != NULL; itemPtr++) {
        ...
        if (pos.getErrorIndex() != -1) {
            errln("error: parse error for string - " + itemPtr->parseString + " - pattern " + itemPtr->pattern + " - idx " + pos.getIndex() + " - error index "
                + pos.getErrorIndex() + " - leniency " + itemPtr->leniency);
            continue;
        }
    ...
    for (itemPtr = items; itemPtr->parseString != "--end--"; itemPtr++) {
        ...
        if (itemPtr->expectedResult.length() == 0) {
            if (pos.getErrorIndex() != -1) {
                ...
                errln(
                    "error: unexpected parse success - " + itemPtr->parseString + " - error index " + pos.getErrorIndex() + " - leniency " + itemPtr->leniency);
                continue;
            }
        }
        if (pos.getErrorIndex() != -1) {
...
void DateFormatTest::TestNumberFormatOverride()
{
    ...
    const char* DATA[][2] = { { "", "\\u521D\\u516D \\u5341\\u4E94" }, { "M", "\\u521D\\u516D 15" }, { "Mo", "\\u521D\\u516D 15" },
        { "Md", "\\u521D\\u516D \\u5341\\u4E94" }, { "MdMMd", "\\u521D\\u516D \\u5341\\u4E94" }, { "mixed", "\\u521D\\u516D \\u5341\\u4E94" } };
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<DateFormat> fmt(DateFormat::createInstanceForSkeleton("yMMMMd", "en", status));
    ...
    fmt.adoptInstead(DateFormat::createInstanceForSkeleton("yMd", "en", status));
...
void DateFormatTest::TestCreateInstanceForSkeletonDefault()
{
    ...
    LocalPointer<DateFormat> fmt(DateFormat::createInstanceForSkeleton("yMMMd", status));
...
void DateFormatTest::TestCreateInstanceForSkeletonWithCalendar()
{
    ...
    LocalPointer<DateFormat> fmt(
        DateFormat::createInstanceForSkeleton(Calendar::createInstance(TimeZone::createTimeZone("GMT-3:00"), status), "yMdHm", "en", status));
    if (!assertSuccess("Create with pattern yMMMMd", status)) {
        return;
    }
    UnicodeString result;
    ...
    LocalPointer<Calendar> cal(Calendar::createInstance(TimeZone::createTimeZone("GMT-7:00"), status));
...
void DateFormatTest::TestDFSCreateForLocaleNonGregorianLocale()
{
    UErrorCode status = U_ZERO_ERROR;
    Locale fa("fa");
    LocalPointer<DateFormatSymbols> sym(DateFormatSymbols::createForLocale(fa, status));
...
void DateFormatTest::TestDFSCreateForLocaleWithCalendarInLocale()
{
    ...
    LocalPointer<DateFormatSymbols> sym(DateFormatSymbols::createForLocale(en_heb, status));
    if (!assertSuccess("", status)) {
        return;
    }
...
void DateFormatTest::TestChangeCalendar()
{
    ...
    Locale en("en");
    Locale en_heb("en@calendar=hebrew");
    LocalPointer<DateFormat> fmt(DateFormat::createInstanceForSkeleton("yMMMd", en, status));
    if (!assertSuccess("", status)) {
        return;
    }
...
void DateFormatTest::TestPatternFromSkeleton()
{
    ...
    } TESTDATA[] = { // Ticket #11985
        { Locale::getEnglish(), "jjmm", "h:mm a" }, { Locale::getEnglish(), "JJmm", "hh:mm" }, { Locale::getGerman(), "jjmm", "HH:mm" },
        { Locale::getGerman(), "JJmm", "HH:mm" },
        // Ticket #20739
        // minutes+milliseconds, seconds missing, should be repaired
        { Locale::getEnglish(), "SSSSm", "mm:ss.SSSS" }, { Locale::getEnglish(), "mSSSS", "mm:ss.SSSS" }, { Locale::getEnglish(), "SSSm", "mm:ss.SSS" },
        { Locale::getEnglish(), "mSSS", "mm:ss.SSS" }, { Locale::getEnglish(), "SSm", "mm:ss.SS" }, { Locale::getEnglish(), "mSS", "mm:ss.SS" },
        { Locale::getEnglish(), "Sm", "mm:ss.S" }, { Locale::getEnglish(), "mS", "mm:ss.S" },
        // only milliseconds, untouched, no repairs
        { Locale::getEnglish(), "S", "S" }, { Locale::getEnglish(), "SS", "SS" }, { Locale::getEnglish(), "SSS", "SSS" },
        { Locale::getEnglish(), "SSSS", "SSSS" },
        // hour:minute+seconds+milliseconds, correct, no repairs, proper pattern
        { Locale::getEnglish(), "jmsSSS", "h:mm:ss.SSS a" }, { Locale::getEnglish(), "jmSSS", "h:mm:ss.SSS a" },
        // Ticket #20738
        // seconds+milliseconds, correct, no repairs, proper pattern
        { Locale::getEnglish(), "sS", "s.S" }, { Locale::getEnglish(), "sSS", "s.SS" }, { Locale::getEnglish(), "sSSS", "s.SSS" },
        { Locale::getEnglish(), "sSSSS", "s.SSSS" }, { Locale::getEnglish(), "sS", "s.S" },
        // minutes+seconds+milliseconds, correct, no repairs, proper pattern
        { Locale::getEnglish(), "msS", "mm:ss.S" }, { Locale::getEnglish(), "msSS", "mm:ss.SS" }, { Locale::getEnglish(), "msSSS", "mm:ss.SSS" },
        { Locale::getEnglish(), "msSSSS", "mm:ss.SSSS" }
    };

    for (size_t i = 0; i < UPRV_LENGTHOF(TESTDATA); i++) {
        UErrorCode status = U_ZERO_ERROR;
        LocalPointer<DateFormat> fmt(DateFormat::createInstanceForSkeleton(TESTDATA[i].skeleton, TESTDATA[i].locale, status));
...
void DateFormatTest::TestDayPeriodWithLocales()
{
    ...
    assertEquals("hh:mm:ss bbbb | 00:00:00 | de", "12:00:00 AM", sdf.format(k000000, out.remove()));
    assertEquals("hh:mm:ss bbbb | 12:00:00 | de", "12:00:00 PM", sdf.format(k120000, out.remove()));
    ...
    assertEquals("hh:mm:ss BBBB | 22:00:00 | ee", UnicodeString("10:00:00 z\\u00E3").unescape(), sdf.format(k220000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 00:00:00 | ee", UnicodeString("12:00:00 z\\u00E3").unescape(), sdf.format(k000000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 01:00:00 | ee", UnicodeString("01:00:00 z\\u00E3").unescape(), sdf.format(k010000, out.remove()));
    ...
    assertEquals("hh:mm:ss BBBB | 00:00:00 | root", "12:00:00 AM", sdf.format(k000000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 12:00:00 | root", "12:00:00 PM", sdf.format(k120000, out.remove()));
    ...
    //     sdf.format(k000000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 00:00:00 | en_US", "12:00:00 at night", sdf.format(k000000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 01:00:00 | en_US", "01:00:00 at night", sdf.format(k010000, out.remove()));
    assertEquals("hh:mm:ss BBBB | 12:00:00 | en_US", "12:00:00 noon", sdf.format(k120000, out.remove()));
    ...
    sdf.applyPattern(UnicodeString("hh:mm:ss BBBB"));
    assertEquals("hh:mm:ss BBBB | 01:00:00 | es_CO", u"01:00:00 de la mañana", sdf.format(k010000, out.remove()));

    sdf = SimpleDateFormat(UnicodeString(), Locale("es"), errorCode);
    sdf.setTimeZone(*tz);

    sdf.applyPattern(UnicodeString("hh:mm:ss BBBB"));
    assertEquals("hh:mm:ss BBBB | 01:00:00 | es", "01:00:00 de la madrugada", sdf.format(k010000, out.remove()));
    ...
    sdf.applyPattern(UnicodeString("hh:mm:ss BBBB"));
    assertEquals("hh:mm:ss BBBB | 01:00:00 | en@calendar=buddhist", "01:00:00 at night", sdf.format(k010000, out.remove()));
}
...
void DateFormatTest::TestMinuteSecondFieldsInOddPlaces()
{
    ...
    assertEquals("hh:mm 'ss' bbbb | 00:00:30", "12:00 ss AM", sdf.format(k000030, out.remove()));
    assertEquals("hh:mm 'ss' bbbb | 06:00:30", "06:00 ss AM", sdf.format(k060030, out.remove()));

    sdf.applyPattern(UnicodeString("hh:mm 'ss' BBBB"));

    // assertEquals("hh:mm 'ss' BBBB | 00:00:30", "12:00 ss midnight",
    //     sdf.format(k000030, out.remove()));
    assertEquals("hh:mm 'ss' BBBB | 00:00:30", "12:00 ss at night", sdf.format(k000030, out.remove()));
    assertEquals("hh:mm 'ss' BBBB | 06:00:30", "06:00 ss in the morning", sdf.format(k060030, out.remove()));
    ...
    assertEquals("hh 'mm ss' bbbb | 00:30:00", "12 mm ss AM", sdf.format(k003000, out.remove()));
    assertEquals("hh 'mm ss' bbbb | 06:30:00", "06 mm ss AM", sdf.format(k063000, out.remove()));

    sdf.applyPattern(UnicodeString("hh 'mm ss' BBBB"));

    // assertEquals("hh 'mm ss' BBBB | 00:30:00", "12 mm ss midnight",
    //     sdf.format(k003000, out.remove()));
    assertEquals("hh 'mm ss' BBBB | 00:30:00", "12 mm ss at night", sdf.format(k003000, out.remove()));
    assertEquals("hh 'mm ss' BBBB | 06:30:00", "06 mm ss in the morning", sdf.format(k063000, out.remove()));
    ...
    //     sdf.format(k000000, out.remove()));
    assertEquals("bbbb hh:mm:ss | 00:00:00", "AM 12:00:00", sdf.format(k000000, out.remove()));
    assertEquals("bbbb hh:mm:ss | 00:00:30", "AM 12:00:30", sdf.format(k000030, out.remove()));
    assertEquals("bbbb hh:mm:ss | 00:30:00", "AM 12:30:00", sdf.format(k003000, out.remove()));

    sdf.applyPattern(UnicodeString("BBBB hh:mm:ss"));

    // assertEquals("BBBB hh:mm:ss | 00:00:00", "midnight 12:00:00",
    //     sdf.format(k000000, out.remove()));
    assertEquals("BBBB hh:mm:ss | 00:00:00", "at night 12:00:00", sdf.format(k000000, out.remove()));
    assertEquals("BBBB hh:mm:ss | 00:00:30", "at night 12:00:30", sdf.format(k000030, out.remove()));
    assertEquals("BBBB hh:mm:ss | 00:30:00", "at night 12:30:00", sdf.format(k003000, out.remove()));
    ...
    //     sdf.format(k000030, out.remove()));
    assertEquals("BBBB hh | 00:00:30", "at night 12", sdf.format(k000030, out.remove()));

    sdf.applyPattern(UnicodeString("BBBB hh:mm:'ss'"));
    // assertEquals("BBBB hh:mm:'ss' | 00:00:30", "midnight 12:00:ss",
    //     sdf.format(k000030, out.remove()));
    assertEquals("BBBB hh | 00:00:30", "at night 12:00:ss", sdf.format(k000030, out.remove()));
...
void DateFormatTest::TestDayPeriodParsing()
{
    ...
    assertEquals("yyyy-MM-dd B | 2015-11-13 midnight", k000000, sdf.parse(UnicodeString("2015-11-13 midnight"), errorCode));
    assertEquals("yyyy-MM-dd B | 2015-11-13 noon", k120000, sdf.parse(UnicodeString("2015-11-13 noon"), errorCode));
    assertEquals("yyyy-MM-dd B | 2015-11-13 in the afternoon", k150000, sdf.parse(UnicodeString("2015-11-13 in the afternoon"), errorCode));
    assertEquals("yyyy-MM-dd B | 2015-11-13 in the evening", k193000, sdf.parse(UnicodeString("2015-11-13 in the evening"), errorCode));
    assertEquals("yyyy-MM-dd B | 2015-11-13 at night", k013000, sdf.parse(UnicodeString("2015-11-13 at night"), errorCode));
    ...
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 12:00 midnight", k000000, sdf.parse(UnicodeString("2015-11-13 12:00 midnight"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 12:00 noon", k120000, sdf.parse(UnicodeString("2015-11-13 12:00 noon"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 01:00 at night", k010000, sdf.parse(UnicodeString("2015-11-13 01:00 at night"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 01:00 in the afternoon", k130000, sdf.parse(UnicodeString("2015-11-13 01:00 in the afternoon"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 09:00 in the morning", k090000, sdf.parse(UnicodeString("2015-11-13 09:00 in the morning"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 09:00 at night", k210000, sdf.parse(UnicodeString("2015-11-13 09:00 at night"), errorCode));

    // If the hour is 13 thru 23 then day period has no effect on time (since time is assumed
    // to be in 24-hour format).
    sdf.applyPattern(UnicodeString("yyyy-MM-dd HH:mm B"));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 midnight", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 midnight"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 noon", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 noon"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 at night", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 at night"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 in the afternoon", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 in the afternoon"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 in the morning", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 in the morning"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 13:37 at night", k133700, sdf.parse(UnicodeString("2015-11-13 13:37 at night"), errorCode));
    ...
    sdf.applyPattern(UnicodeString("yyyy-MM-dd hh:mm B"));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 00:00 midnight", k000000, sdf.parse(UnicodeString("2015-11-13 00:00 midnight"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 00:00 noon", k120000, sdf.parse(UnicodeString("2015-11-13 00:00 noon"), errorCode));

    // But when parsed with 'H', 0 indicates a 24-hour time, therefore we disregard the day period.
    sdf.applyPattern(UnicodeString("yyyy-MM-dd HH:mm B"));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 midnight", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 midnight"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 noon", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 noon"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 at night", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 at night"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 in the afternoon", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 in the afternoon"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 in the morning", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 in the morning"), errorCode));
    assertEquals("yyyy-MM-dd HH:mm B | 2015-11-13 00:37 at night", k003700, sdf.parse(UnicodeString("2015-11-13 00:37 at night"), errorCode));
    ...
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 07:00 in the afternoon", k190000, sdf.parse(UnicodeString("2015-11-13 07:00 in the afternoon"), errorCode));
    // NIGHT1 is [21, 6), but "8 at night" parses to 20:00.
    assertEquals("yyyy-MM-dd hh:mm B | 2015-11-13 08:00 at night", k200000, sdf.parse(UnicodeString("2015-11-13 08:00 at night"), errorCode));
    ...
    sdf.applyPattern(UnicodeString("yyyy-MM-dd hh:mm b"));
    assertEquals("yyyy-MM-dd hh:mm b | 2015-11-13 01:00 AM", k010000, sdf.parse(UnicodeString("2015-11-13 01:00 AM"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm b | 2015-11-13 01:00 PM", k130000, sdf.parse(UnicodeString("2015-11-13 01:00 PM"), errorCode));
    ...
    sdf.applyPattern(UnicodeString("yyyy-MM-dd hh:mm b"));
    assertEquals("yyyy-MM-dd hh:mm b | 2015-11-13 03:00 midnight", k030000, sdf.parse(UnicodeString("2015-11-13 03:00 midnight"), errorCode));
    assertEquals("yyyy-MM-dd hh:mm b | 2015-11-13 03:00 noon", k150000, sdf.parse(UnicodeString("2015-11-13 03:00 noon"), errorCode));
}

void DateFormatTest::TestParseRegression13744()
{
    LocalPointer<DateFormat> dfmt(DateFormat::createDateTimeInstance(DateFormat::SHORT, DateFormat::SHORT, Locale("en", "US")));
    if (dfmt.isNull()) {
    ...
    for (int32_t j = 0; j < 2; j++) {
        ...
        for (int32_t i = 0; i < count; i++) {
            ...
            while (fpositer.next(curFieldPosition)) {
                ...
                if (curFieldPosition.getBeginIndex() == lastFieldPosition.getBeginIndex()
                    && curFieldPosition.getEndIndex() == lastFieldPosition.getEndIndex()) {
...
void DateFormatTest::Test22023_UTCWithMinusZero()
{
    ...
    ASSERT_OK(status);
    fmt.adoptCalendar(Calendar::createInstance(TimeZone::createTimeZone("UTC"), locale, status));
    ASSERT_OK(status);
