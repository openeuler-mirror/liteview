...
{
    static const char16_t* cases[] = { u"maesure-unit", u"measure-unit/foo-bar", u"numbering-system/dummy", u"français",
...
{
    static const char16_t* cases[] = { u".00/w/w", u"group-thousands/foo", u"precision-integer//@## group-off", u"precision-integer//@##  group-off",
        u"precision-integer/ group-off", u"precision-integer// group-off" };
...
{
    static const char16_t* cases[] = { u"precision-integer precision-integer", u"precision-integer .00+", u"precision-integer precision-unlimited",
        u"precision-integer @@@", u"scientific engineering", u"engineering compact-long", u"sign-auto sign-always" };
...
void NumberSkeletonTest::stemsRequiringOption()
{
    static const char16_t* stems[]
        = { u"precision-increment", u"measure-unit", u"per-measure-unit", u"currency", u"integer-width", u"numbering-system", u"scale" };
...
void NumberSkeletonTest::defaultTokens()
{
    ...
    static const char16_t* cases[]
        = { u"notation-simple", u"base-unit", u"group-auto", u"integer-width/+0", u"sign-auto", u"unit-width-short", u"decimal-auto" };

    for (auto& cas : cases) {
        UnicodeString skeletonString(cas);
        status.setScope(skeletonString);
        UnicodeString normalized = NumberFormatter::forSkeleton(skeletonString, status).toSkeleton(status);
...
void NumberSkeletonTest::flexibleSeparators()
{
    ...
    } cases[] = { { u"precision-integer group-off", u"5142" }, { u"precision-integer  group-off", u"5142" }, { u"precision-integer/@## group-off", u"5140" },
    ...
    for (auto& cas : cases) {
        ...
        status.setScope(skeletonString);
        UnicodeString actual = NumberFormatter::forSkeleton(skeletonString, status).locale("en").formatDouble(5142.3, status).toString(status);
...
void NumberSkeletonTest::wildcardCharacters()
{
    ...
    for (const auto& cas : cases) {
        ...
        UnicodeString normalized = NumberFormatter::forSkeleton(plus, status).toSkeleton(status);
...
void NumberSkeletonTest::perUnitInArabic()
{
    ...
    for (const auto& cas1 : cases) {
        for (const auto& cas2 : cases) {
            ...
            UnicodeString actual = NumberFormatter::forSkeleton(skeleton, status).locale("ar").formatDouble(5142.3, status).toString(status);
            status.errIfFailureAndReset();
        }
    }
}
    ...
    for (const auto& cas1 : cases) {
        for (const auto& cas2 : cases) {
            ...
            if (cas1.type != cas2.type && cas1.subtype != cas2.subtype) {
                UnicodeString toSkeleton = NumberFormatter::forSkeleton(skeleton, status).toSkeleton(status);
                if (status.errIfFailureAndReset()) {
                    continue;
                }
                // Ensure both subtype are in the toSkeleton.
                UnicodeString msg;
                msg.append(toSkeleton).append(" should contain '").append(UnicodeString(cas1.subtype)).append("' when constructed from ").append(skeleton);
                assertTrue(msg, toSkeleton.indexOf(cas1.subtype) >= 0);

                msg.remove();
                msg.append(toSkeleton).append(" should contain '").append(UnicodeString(cas2.subtype)).append("' when constructed from ").append(skeleton);
