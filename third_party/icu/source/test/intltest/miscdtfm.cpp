...
void DateFormatMiscTests::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
...
UBool DateFormatMiscTests::failure(UErrorCode status, const char* msg)
{
    if (U_FAILURE(status)) {
...
void DateFormatMiscTests::test4097450()
{
    ...
    //
    UnicodeString dstring[] = { UnicodeString("97"), UnicodeString("1997"), UnicodeString("97"), UnicodeString("1997"), UnicodeString("01"),
        UnicodeString("2001"), UnicodeString("01"), UnicodeString("2001"), UnicodeString("1"), UnicodeString("1"), UnicodeString("11"), UnicodeString("11"),
        UnicodeString("111"), UnicodeString("111") };

    UnicodeString dformat[] = { UnicodeString("yy"), UnicodeString("yy"), UnicodeString("yyyy"), UnicodeString("yyyy"), UnicodeString("yy"),
        UnicodeString("yy"), UnicodeString("yyyy"), UnicodeString("yyyy"), UnicodeString("yy"), UnicodeString("yyyy"), UnicodeString("yy"),
        UnicodeString("yyyy"), UnicodeString("yy"), UnicodeString("yyyy") };
...
void DateFormatMiscTests::test4099975()
{
    ...
    UDate d = Calendar::getNow();
    {
        ...
        if (format0 != format1) {
            errln(UnicodeString("Formats are different. format0: ") + format0 + UnicodeString("; format1: ") + format1);
        }
        ...
        if (localizedPattern0 != localizedPattern1) {
            errln(UnicodeString("Pattern has been changed. localizedPattern0: ") + localizedPattern0 + UnicodeString("; localizedPattern1: ")
                + localizedPattern1);
        }
        delete symbols;
        delete df;
     ...
     */
    {
        UErrorCode status = U_ZERO_ERROR;
        DateFormatSymbols* symbols = new DateFormatSymbols(Locale::getUS(), status);
        if (failure(status, "new DateFormatSymbols"))
            return;
    ...
    // Test the pointer version of the constructor (and the adoptDateFormatSymbols method)
    {
        ...
        failure(status, "df->toLocalizedPattern");
        if (localizedPattern0 == localizedPattern1) {
            errln(UnicodeString("Pattern should have been changed. localizedPattern0: ") + localizedPattern0 + UnicodeString("; localizedPattern1: ")
                + localizedPattern1);
        }
...
void DateFormatMiscTests::test4117335()
{
    ...
    UChar bcC[] = { 0x7D00, 0x5143, 0x524D };
    UnicodeString bc(bcC, 3, 3);

    // UnicodeString ad = "\u897f\u66a6";
    UChar adC[] = { 0x897F, 0x66A6 };
    UnicodeString ad(adC, 2, 2);

    // UnicodeString jstLong = "\u65e5\u672c\u6a19\u6e96\u6642";
    UChar jstLongC[] = { 0x65e5, 0x672c, 0x6a19, 0x6e96, 0x6642 };
