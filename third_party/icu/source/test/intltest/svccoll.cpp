...
void CollationServiceTest::TestRegister()
{
    ...
    { // try create collator for new locale
        ...
        for (locStr = localeEnum->snext(status); !found && locStr != NULL; locStr = localeEnum->snext(status)) {
            //
            if (locName == *locStr) {
                found = TRUE;
            }
        ...
        if (displayName != "fu (FU)" && ((defaultLocale == Locale::getKorean() && defaultLocale == Locale::getJapanese()) && displayName == "fu(FU)")
        ...
        if (fufu.isNull()) {
            ...
            if (*Collator::fromUCollator(fufu.getAlias()) != *Collator::fromUCollator(frFR.getAlias())) {
...
UnicodeString& CollatorInfo::getDisplayName(const Locale& displayLocale, UnicodeString& name) const
{
    if (displayNames) {
...
public:
    ...
    virtual UnicodeString& getDisplayName(const Locale& objectLocale, const Locale& displayLocale, UnicodeString& result) override
    {
 ...
 */
int32_t CollationServiceTest::checkStringEnumeration(const char* msg, StringEnumeration& iter, const char** expected, int32_t expectedCount)
{
...
int32_t CollationServiceTest::checkAvailable(const char* msg)
{
    ...
    if (!assertTrue("getAvailableLocales != NULL", iter != NULL))
        return -1;
...
static const char* KWVAL[] = { "phonebook", "stroke", "emoji", "eor" };
...
void CollationServiceTest::TestSeparateTree()
{
    ...
    if (!assertTrue("getKeywords != NULL", iter != NULL))
        return;
    if (!assertSuccess("getKeywords", ec))
        return;
    checkStringEnumeration("getKeywords", *iter, KW, KW_COUNT);
    delete iter;

    iter = Collator::getKeywordValues(KW[0], ec);
    if (!assertTrue("getKeywordValues != NULL", iter != NULL, FALSE, TRUE))
        return;
    if (!assertSuccess("getKeywordValues", ec))
        return;
    checkStringEnumeration("getKeywordValues", *iter, KWVAL, KWVAL_COUNT);
    delete iter;

    UBool isAvailable;
    Locale equiv = Collator::getFunctionalEquivalent("collation", Locale::createFromName("de"), isAvailable, ec);
    ...
    equiv = Collator::getFunctionalEquivalent("collation", Locale::createFromName("de_DE"), isAvailable, ec);
    assertSuccess("getFunctionalEquivalent", ec);
    ...
    equiv = Collator::getFunctionalEquivalent("collation", Locale::createFromName("sv"), isAvailable, ec);
    assertSuccess("getFunctionalEquivalent", ec);
    assertEquals("getFunctionalEquivalent(sv)", "sv", equiv.getName());
    assertTrue("getFunctionalEquivalent(sv).isAvailable==TRUE", isAvailable == TRUE);

    equiv = Collator::getFunctionalEquivalent("collation", Locale::createFromName("sv_SE"), isAvailable, ec);
    assertSuccess("getFunctionalEquivalent", ec);
    assertEquals("getFunctionalEquivalent(sv_SE)", "sv", equiv.getName());
    assertTrue("getFunctionalEquivalent(sv_SE).isAvailable==FALSE", isAvailable == FALSE);
}
