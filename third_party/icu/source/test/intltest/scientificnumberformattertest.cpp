...
void ScientificNumberFormatterTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char*)
{
    if (exec) {
...
void ScientificNumberFormatterTest::TestBasic()
{
    ...
    LocalPointer<ScientificNumberFormatter> fmt(ScientificNumberFormatter::createMarkupInstance("en", "<sup>", "</sup>", status));
    ...
    assertEquals("markup style", UnicodeString(expected).unescape(), appendTo);

    // Test superscript style
    fmt.adoptInstead(ScientificNumberFormatter::createSuperscriptInstance("en", status));
    ...
    assertEquals("superscript style", UnicodeString(expected).unescape(), appendTo);
...
void ScientificNumberFormatterTest::TestFarsi()
{
    ...
    UnicodeString prefix("String: ");
    UnicodeString appendTo(prefix);
    LocalPointer<ScientificNumberFormatter> fmt(ScientificNumberFormatter::createMarkupInstance("fa", "<sup>", "</sup>", status));
    if (!assertSuccess("Can't create ScientificNumberFormatter", status)) {
        return;
    }
    fmt->format(1.23456e-78, appendTo, status);
    const char* expected = "String: \\u06F1\\u066B\\u06F2\\u06F3\\u06F4\\u06F5\\u06F6\\u00d7\\u06F1\\u06F0<sup>\\u200E\\u2212\\u06F7\\u06F8</sup>";
    assertEquals("", UnicodeString(expected).unescape(), appendTo);
    assertSuccess("", status);
}
...
void ScientificNumberFormatterTest::TestPlusSignInExponentMarkup()
{
    ...
    LocalPointer<ScientificNumberFormatter> fmt(ScientificNumberFormatter::createMarkupInstance(new DecimalFormat(*decfmt), "<sup>", "</sup>", status));
    if (!assertSuccess("Can't create ScientificNumberFormatter", status)) {
        return;
    }
...
void ScientificNumberFormatterTest::TestPlusSignInExponentSuperscript()
{
    ...
    UnicodeString appendTo;
    LocalPointer<ScientificNumberFormatter> fmt(ScientificNumberFormatter::createSuperscriptInstance(new DecimalFormat(*decfmt), status));
    if (!assertSuccess("Can't create ScientificNumberFormatter", status)) {
        return;
    }
    fmt->format(6.02e23, appendTo, status);
