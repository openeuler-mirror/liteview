...
//
//---------------------------------------------------------------------------
#define DF_CHECK_STATUS                                                                                                                                        \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
            dataerrln("DecimalFormatTest failure at line %d.  status=%s", __LINE__, u_errorName(status));                                                      \
            return 0;                                                                                                                                          \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
...
#define DF_ASSERT_FAIL(expr, errcode)                                                                                                                          \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        UErrorCode status = U_ZERO_ERROR;                                                                                                                      \
        (expr);                                                                                                                                                \
        if (status != errcode) {                                                                                                                               \
            dataerrln("DecimalFormatTest failure at line %d.  Expected status=%s, got %s", __LINE__, u_errorName(errcode), u_errorName(status));               \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END

#define DF_CHECK_STATUS_L(line)                                                                                                                                \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
...
void DecimalFormatTest::DataDrivenTests()
{
    ...
    RegexMatcher parseLineMat(UnicodeString("(?i)\\s*parse\\s+"
    ...
    RegexMatcher formatLineMat(UnicodeString("(?i)\\s*format\\s+"
    ...
    while (lineMat.find()) {
        ...
        if (parseLineMat.lookingAt(status)) {
            execParseTest(lineNum, parseLineMat.group(1, status), // input
        ...
        if (formatLineMat.lookingAt(status)) {
            execFormatTest(lineNum, formatLineMat.group(1, status), // Pattern
...
void DecimalFormatTest::execParseTest(
    int32_t lineNum, const UnicodeString& inputText, const UnicodeString& expectedType, const UnicodeString& expectedDecimal, UErrorCode& status)
{
    ...
    Formattable result;
    if (U_FAILURE(status)) {
        dataerrln("file dcfmtest.txt, line %d: %s error creating the formatter.", lineNum, u_errorName(status));
        return;
    }
    ...
    if (expectedParseEndPosition != pos.getIndex()) {
        errln("file dcfmtest.txt, line %d: Expected parse position afeter parsing: %d.  "
              "Actual parse position: %d",
            expectedParseEndPosition, pos.getIndex());
        return;
    }
    ...
    case 'd':
        expectType = Formattable::kDouble;
        break;
    case 'i':
        expectType = Formattable::kLong;
        break;
    case 'l':
        expectType = Formattable::kInt64;
        break;
    default:
        errln("file dcfmtest.tx, line %d: unrecognized expected type \"%s\"", lineNum, InvariantStringPiece(expectedType).data());
        return;
    }
    if (result.getType() != expectType) {
        errln("file dcfmtest.txt, line %d: expectedParseType(%s) != actual parseType(%s)", lineNum, formattableType(expectType),
            formattableType(result.getType()));
        return;
    }

    StringPiece decimalResult = result.getDecimalNumber(status);
    if (U_FAILURE(status)) {
        errln("File %s, line %d: error %s.  Line in file dcfmtest.txt:  %d:", __FILE__, __LINE__, u_errorName(status), lineNum);
        return;
    }

    InvariantStringPiece expectedResults(expectedDecimal);
    if (decimalResult != expectedResults) {
        errln("file dcfmtest.txt, line %d: expected \"%s\", got \"%s\"", lineNum, expectedResults.data(), decimalResult.data());
    }
    ...
    UErrorCode& status)
{
    ...
    if (round == "ceiling") {
        ...
        fmtr.setRoundingMode(DecimalFormat::kRoundFloor);
        errln("file dcfmtest.txt, line %d: Bad rounding mode \"%s\"", lineNum, UnicodeStringPiece(round).data());
    }
    ...
    if (U_FAILURE(status)) {
        errln("[%s] file dcfmtest.txt, line %d: format() returned %s.", typeStr, lineNum, u_errorName(status));
        status = U_ZERO_ERROR;
        return;
    }

    if (result != expected) {
        errln("[%s] file dcfmtest.txt, line %d: expected \"%s\", got \"%s\", %s", typeStr, lineNum, UnicodeStringPiece(expected).data(),
            UnicodeStringPiece(result).data(), u_errorName(status));
    }
}
