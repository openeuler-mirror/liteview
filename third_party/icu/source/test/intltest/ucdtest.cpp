...
static const char* ignorePropNames[]
    = { "FC_NFKC", "NFD_QC", "NFC_QC", "NFKD_QC", "NFKC_QC", "Expands_On_NFD", "Expands_On_NFC", "Expands_On_NFKD", "Expands_On_NFKC", "NFKC_CF" };
...
static const char* const derivedPropsNames[] = { "Math", "Alphabetic", "Lowercase", "Uppercase", "ID_Start", "ID_Continue", "XID_Start", "XID_Continue",
    "Default_Ignorable_Code_Point", "Full_Composition_Exclusion", "Grapheme_Extend", "Grapheme_Link", /* Unicode 5 moves this property here from PropList.txt */
    "Grapheme_Base", "Cased", "Case_Ignorable", "Changes_When_Lowercased", "Changes_When_Uppercased", "Changes_When_Titlecased", "Changes_When_Casefolded",
    "Changes_When_Casemapped", "Changes_When_NFKC_Casefolded" };

static const UProperty derivedPropsIndex[] = { UCHAR_MATH, UCHAR_ALPHABETIC, UCHAR_LOWERCASE, UCHAR_UPPERCASE, UCHAR_ID_START, UCHAR_ID_CONTINUE,
    UCHAR_XID_START, UCHAR_XID_CONTINUE, UCHAR_DEFAULT_IGNORABLE_CODE_POINT, UCHAR_FULL_COMPOSITION_EXCLUSION, UCHAR_GRAPHEME_EXTEND, UCHAR_GRAPHEME_LINK,
    UCHAR_GRAPHEME_BASE, UCHAR_CASED, UCHAR_CASE_IGNORABLE, UCHAR_CHANGES_WHEN_LOWERCASED, UCHAR_CHANGES_WHEN_UPPERCASED, UCHAR_CHANGES_WHEN_TITLECASED,
    UCHAR_CHANGES_WHEN_CASEFOLDED, UCHAR_CHANGES_WHEN_CASEMAPPED, UCHAR_CHANGES_WHEN_NFKC_CASEFOLDED };
...
UBool UnicodeTest::compareUSets(const UnicodeSet& a, const UnicodeSet& b, const char* a_name, const char* b_name, UBool diffIsError)
{
    UBool same = a == b;
    if (!same && diffIsError) {
...
void UnicodeTest::TestScriptMetadata()
{
    ...
    for (int32_t sci = 0; sci < USCRIPT_CODE_LIMIT; ++sci) {
        ...
        if (usage == USCRIPT_USAGE_NOT_ENCODED) {
            ...
            assertEquals("script(sample(script))", (int32_t)charScript, (int32_t)uscript_getScript(firstChar, errorCode));
    ...
    assertEquals("no remaining RTL characters", UnicodeString("[]"), rtl.toPattern(pattern));
    assertEquals("no remaining cased characters", UnicodeString("[]"), cased.toPattern(pattern));
...
void UnicodeTest::TestEmojiProperties()
{
    ...
    assertTrue("shooting star is Emoji_Presentation", u_hasBinaryProperty(0x1F320, UCHAR_EMOJI_PRESENTATION));
    assertTrue("Fitzpatrick 6 is Emoji_Modifier", u_hasBinaryProperty(0x1F3FF, UCHAR_EMOJI_MODIFIER));
    assertTrue("happy person is Emoji_Modifier_Base", u_hasBinaryProperty(0x1F64B, UCHAR_EMOJI_MODIFIER_BASE));
    assertTrue("asterisk is Emoji_Component", u_hasBinaryProperty(0x2A, UCHAR_EMOJI_COMPONENT));
    assertTrue("copyright is Extended_Pictographic", u_hasBinaryProperty(0xA9, UCHAR_EXTENDED_PICTOGRAPHIC));
}
...
void UnicodeTest::TestEmojiPropertiesOfStrings()
{
    ...
    assertFalse("4+emoji is not Emoji_Keycap_Sequence", hbp(u"4\uFE0F", UCHAR_EMOJI_KEYCAP_SEQUENCE));
    assertTrue("4+emoji+keycap is Emoji_Keycap_Sequence", hbp(u"4\uFE0F\u20E3", UCHAR_EMOJI_KEYCAP_SEQUENCE));

    assertFalse("[B] is not RGI_Emoji_Flag_Sequence", hbp(u"\U0001F1E7", UCHAR_RGI_EMOJI_FLAG_SEQUENCE));
    assertTrue("[BE] is RGI_Emoji_Flag_Sequence", hbp(u"🇧🇪", UCHAR_RGI_EMOJI_FLAG_SEQUENCE));

    assertFalse("[flag] is not RGI_Emoji_Tag_Sequence", hbp(u"\U0001F3F4", UCHAR_RGI_EMOJI_TAG_SEQUENCE));
    assertTrue("[Scotland] is RGI_Emoji_Tag_Sequence", hbp(u"🏴󠁧󠁢󠁳󠁣󠁴󠁿", UCHAR_RGI_EMOJI_TAG_SEQUENCE));

    assertFalse("bicyclist is not RGI_Emoji_Modifier_Sequence", hbp(u"🚴", UCHAR_RGI_EMOJI_MODIFIER_SEQUENCE));
    assertTrue("bicyclist+medium is RGI_Emoji_Modifier_Sequence", hbp(u"🚴\U0001F3FD", UCHAR_RGI_EMOJI_MODIFIER_SEQUENCE));

    assertFalse("woman+dark+ZWJ is not RGI_Emoji_ZWJ_Sequence", hbp(u"👩\U0001F3FF\u200D", UCHAR_RGI_EMOJI_ZWJ_SEQUENCE));
    assertTrue("woman pilot: dark skin tone is RGI_Emoji_ZWJ_Sequence", hbp(u"👩\U0001F3FF\u200D✈\uFE0F", UCHAR_RGI_EMOJI_ZWJ_SEQUENCE));
    ...
    assertTrue("woman pilot: dark skin tone is RGI_Emoji", hbp(u"👩\U0001F3FF\u200D✈\uFE0F", UCHAR_RGI_EMOJI));
    ...
    assertEquals("RGI_Emoji/only-cp.size() == Basic_Emoji/only-cp.size()", rgiOnlyCp.size(), basicOnlyCp.size());
    ...
    assertTrue("Emoji_Keycap_Sequence.contains(4+emoji+keycap)", keycaps.contains(u"4\uFE0F\u20E3"));
    ...
    assertTrue("RGI_Emoji_Modifier_Sequence.contains(bicyclist+medium)", modified.contains(u"🚴\U0001F3FD"));
    assertTrue("RGI_Emoji_ZWJ_Sequence.contains(woman pilot: dark skin tone)", combos.contains(u"👩\U0001F3FF\u200D✈\uFE0F"));
...
void UnicodeTest::TestIndicPositionalCategory()
{
    ...
    assertEquals("U+08FF: NA", U_INPC_NA, u_getIntPropertyValue(0x08FF, UCHAR_INDIC_POSITIONAL_CATEGORY));
    assertEquals("U+0900: Top", U_INPC_TOP, u_getIntPropertyValue(0x0900, UCHAR_INDIC_POSITIONAL_CATEGORY));
    assertEquals("U+10A06: Overstruck", U_INPC_OVERSTRUCK, u_getIntPropertyValue(0x10A06, UCHAR_INDIC_POSITIONAL_CATEGORY));
}
...
void UnicodeTest::TestIndicSyllabicCategory()
{
    ...
    assertEquals("U+08FF: Other", U_INSC_OTHER, u_getIntPropertyValue(0x08FF, UCHAR_INDIC_SYLLABIC_CATEGORY));
    assertEquals("U+0900: Bindu", U_INSC_BINDU, u_getIntPropertyValue(0x0900, UCHAR_INDIC_SYLLABIC_CATEGORY));
    assertEquals("U+11065: Brahmi_Joining_Number", U_INSC_BRAHMI_JOINING_NUMBER, u_getIntPropertyValue(0x11065, UCHAR_INDIC_SYLLABIC_CATEGORY));
}
...
void UnicodeTest::TestVerticalOrientation()
{
    ...
    assertEquals("U+0E01: Rotated", U_VO_ROTATED, u_getIntPropertyValue(0x0E01, UCHAR_VERTICAL_ORIENTATION));
    assertEquals("U+3008: Transformed_Rotated", U_VO_TRANSFORMED_ROTATED, u_getIntPropertyValue(0x3008, UCHAR_VERTICAL_ORIENTATION));
    assertEquals("U+33333: Upright", U_VO_UPRIGHT, u_getIntPropertyValue(0x33333, UCHAR_VERTICAL_ORIENTATION));
}
...
void UnicodeTest::TestInvalidCodePointFolding(void)
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(invalidCodePoints); ++i) {
        ...
        assertEquals("Invalid code points should be echoed back", cp, u_foldCase(cp, U_FOLD_CASE_DEFAULT));
        assertEquals("Invalid code points should be echoed back", cp, u_foldCase(cp, U_FOLD_CASE_EXCLUDE_SPECIAL_I));
    }
}
...
void UnicodeTest::TestBinaryCharacterProperties()
{
    ...
    for (int32_t prop = 0; prop < UCHAR_BINARY_LIMIT; ++prop) {
        ...
        if (count == 0) {
            assertFalse(UnicodeString("!hasBinaryProperty(U+0020, ") + prop + u")", u_hasBinaryProperty(0x20, (UProperty)prop));
            assertFalse(UnicodeString("!hasBinaryProperty(U+0061, ") + prop + u")", u_hasBinaryProperty(0x61, (UProperty)prop));
            assertFalse(UnicodeString("!hasBinaryProperty(U+4E00, ") + prop + u")", u_hasBinaryProperty(0x4e00, (UProperty)prop));
        } else {
            UChar32 c = set.getRangeStart(0);
            if (c > 0) {
                assertFalse(UnicodeString("!hasBinaryProperty(") + TestUtility::hex(c - 1) + u", " + prop + u")", u_hasBinaryProperty(c - 1, (UProperty)prop));
            }
            assertTrue(UnicodeString("hasBinaryProperty(") + TestUtility::hex(c) + u", " + prop + u")", u_hasBinaryProperty(c, (UProperty)prop));
...
void UnicodeTest::TestIntCharacterProperties()
{
    ...
    for (int32_t prop = UCHAR_INT_START; prop < UCHAR_INT_LIMIT; ++prop) {
        ...
        UChar32 c = end / 2;
        assertEquals(UnicodeString("int property first range value at ") + TestUtility::hex(c), u_getIntPropertyValue(c, (UProperty)prop), value);
        end = ucpmap_getRange(map, 0x5000, UCPMAP_RANGE_NORMAL, 0, nullptr, nullptr, &value);
        assertTrue("int property later range", end >= 0);
        assertEquals(UnicodeString("int property later range value at ") + TestUtility::hex(end), u_getIntPropertyValue(end, (UProperty)prop), value);
        // ucpmap_get() API coverage
        // TODO: move to cucdtst.c
        assertEquals("int property upcmap_get(U+0061)", u_getIntPropertyValue(0x61, (UProperty)prop), ucpmap_get(map, 0x61));
    }
#endif
}
