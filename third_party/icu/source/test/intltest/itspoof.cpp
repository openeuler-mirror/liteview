...
#define TEST_ASSERT_EQ(a, b)                                                                                                                                   \
    ...
    {                                                                                                                                                          \
        if ((a) != (b)) {                                                                                                                                      \
            errln("Test Failure at file %s, line %d: \"%s\" (%d) != \"%s\" (%d)", __FILE__, __LINE__, #a, (a), #b, (b));                                       \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END
...
//
void IntlTestSpoof::checkSkeleton(const USpoofChecker* sc, uint32_t type, const char* input, const char* expected, int32_t lineNum)
{
    ...
    if (U_FAILURE(status)) {
        errln("File %s, Line %d, Test case from line %d, status is %s", __FILE__, __LINE__, lineNum, u_errorName(status));
        return;
    }
    if (uExpected != actual) {
        errln("File %s, Line %d, Test case from line %d, Actual and Expected skeletons differ.", __FILE__, __LINE__, lineNum);
        errln(UnicodeString(" Actual   Skeleton: \"") + actual + UnicodeString("\"\n") + UnicodeString(" Expected Skeleton: \"") + uExpected
            + UnicodeString("\""));
    }
}
...
void IntlTestSpoof::testScriptSet()
{
    ...
    for (int32_t i = 0; i < 4; i++) {
        ...
        switch (i) {
        case 0:
            TEST_ASSERT_EQ(USCRIPT_INHERITED, n);
            break;
        case 1:
            TEST_ASSERT_EQ(USCRIPT_VAI, n);
            break;
        case 2:
            TEST_ASSERT_EQ(USCRIPT_AFAKA, n);
            break;
        case 3:
            TEST_ASSERT_EQ(-1, (int32_t)n);
            break;
        default:
            TEST_ASSERT(FALSE);
        }
    ...
    // Additional tests
    ScriptSet bitset12;
    bitset12.set(USCRIPT_LATIN, status).set(USCRIPT_HANGUL, status);
    ScriptSet bitset2;
    bitset2.set(USCRIPT_HANGUL, status);
    TEST_ASSERT(bitset12.contains(bitset2));
    TEST_ASSERT(bitset12.contains(bitset12));
    TEST_ASSERT(!bitset2.contains(bitset12));

    ScriptSet arabSet;
    arabSet.set(USCRIPT_ARABIC, status);
    ScriptSet latinSet;
    latinSet.set(USCRIPT_LATIN, status);
    UElement arabEl;
    arabEl.pointer = &arabSet;
    UElement latinEl;
    latinEl.pointer = &latinSet;
...
void IntlTestSpoof::testRestrictionLevel()
{
    ...
    } tests[] = { { "\\u0061\\u03B3\\u2665", USPOOF_UNRESTRICTIVE }, { "a", USPOOF_ASCII }, { "\\u03B3", USPOOF_SINGLE_SCRIPT_RESTRICTIVE },
        { "\\u0061\\u30A2\\u30FC", USPOOF_HIGHLY_RESTRICTIVE }, { "\\u0061\\u0904", USPOOF_MODERATELY_RESTRICTIVE },
        { "\\u0061\\u03B3", USPOOF_MINIMALLY_RESTRICTIVE }, { "\\u0061\\u2665", USPOOF_UNRESTRICTIVE }, { "\\u0061\\u303C", USPOOF_HIGHLY_RESTRICTIVE },
        { "\\u0061\\u30FC\\u303C", USPOOF_HIGHLY_RESTRICTIVE }, { "\\u0061\\u30FC\\u303C\\u30A2", USPOOF_HIGHLY_RESTRICTIVE },
        { "\\u30A2\\u0061\\u30FC\\u303C", USPOOF_HIGHLY_RESTRICTIVE }, { "\\u0061\\u0031\\u0661", USPOOF_MODERATELY_RESTRICTIVE },
    ...
    char msgBuffer[100];
    URestrictionLevel restrictionLevels[] = { USPOOF_ASCII, USPOOF_SINGLE_SCRIPT_RESTRICTIVE, USPOOF_HIGHLY_RESTRICTIVE, USPOOF_MODERATELY_RESTRICTIVE,
        USPOOF_MINIMALLY_RESTRICTIVE, USPOOF_UNRESTRICTIVE };
    ...
    for (int32_t testNum = 0; testNum < UPRV_LENGTHOF(tests); testNum++) {
        ...
        for (int levelIndex = 0; levelIndex < UPRV_LENGTHOF(restrictionLevels); levelIndex++) {
            ...
            sprintf(msgBuffer, "testNum = %d, levelIndex = %d, expected = %#x, actual = %#x", testNum, levelIndex, expectedValue, actualValue);
...
void IntlTestSpoof::testMixedNumbers()
{
    struct Test {
        ...
        = { { "1", "[0]" }, { "\\u0967", "[\\u0966]" }, { "1\\u0967", "[0\\u0966]" }, { "\\u0661\\u06F1", "[\\u0660\\u06F0]" }, { "\\u0061\\u2665", "[]" },
              { "\\u0061\\u303C", "[]" }, { "\\u0061\\u30FC\\u303C", "[]" }, { "\\u0061\\u30FC\\u303C\\u30A2", "[]" }, { "\\u30A2\\u0061\\u30FC\\u303C", "[]" },
              { "\\u0061\\u0031\\u0661", "[\\u0030\\u0660]" }, { "\\u0061\\u0031\\u0661\\u06F1", "[\\u0030\\u0660\\u06F0]" },
...
void IntlTestSpoof::testBug13328_MixedCombiningMarks()
{
    ...
    TEST_ASSERT_SUCCESS(status);
    assertEquals("The mismatched combining marks string fails spoof", USPOOF_RESTRICTION_LEVEL, failedChecks);
}
