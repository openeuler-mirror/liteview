...
void TimeUnitTest::testBasic()
{
    ...
    for (unsigned int locIndex = 0; locIndex < UPRV_LENGTHOF(locales); ++locIndex) {
        UErrorCode status = U_ZERO_ERROR;
        ...
        if (!assertSuccess("TimeUnitFormat(full)", status, TRUE))
            return;
        formats[UTMUTFMT_ABBREVIATED_STYLE] = new TimeUnitFormat(loc, UTMUTFMT_ABBREVIATED_STYLE, status);
        if (!assertSuccess("TimeUnitFormat(short)", status))
            return;
#ifdef TUFMTTS_DEBUG
        std::cout << "locale: " << locales[locIndex] << "\n";
#endif
        for (int style = UTMUTFMT_FULL_STYLE; style <= UTMUTFMT_ABBREVIATED_STYLE; ++style) {
            for (TimeUnit::UTimeUnitFields j = TimeUnit::UTIMEUNIT_YEAR; j < TimeUnit::UTIMEUNIT_FIELD_COUNT; j = (TimeUnit::UTimeUnitFields)(j + 1)) {
#ifdef TUFMTTS_DEBUG
                ...
                for (unsigned int i = 0; i < UPRV_LENGTHOF(tests); ++i) {
                    ...
                    if (!assertSuccess("parseObject()", status))
                        return;
...
void TimeUnitTest::testAPI()
{
    ...
    if (!assertSuccess("TimeUnit::createInstance", status))
        return;
    ...
    for (TimeUnit::UTimeUnitFields j = TimeUnit::UTIMEUNIT_YEAR; j < TimeUnit::UTIMEUNIT_FIELD_COUNT; j = (TimeUnit::UTimeUnitFields)(j + 1)) {
        ...
        assertTrue("Time unit should be equal to corresponding MeasureUnit", MeasureUnit(*ptr) == *ptrs[j]);
        delete ptr;
    }
    delete tmunit;
    delete another;
    ...
    if (!assertSuccess("TimeUnitAmount(formattable...)", status))
        return;
    ...
    if (!assertSuccess("TimeUnitAmount(number...)", status))
        return;
    ...
    if (!assertSuccess("TimeUnitFormat(en...)", status, TRUE))
        return;
    TimeUnitFormat tmf_fr(Locale("fr"), status);
    if (!assertSuccess("TimeUnitFormat(fr...)", status))
        return;
    ...
    if (!assertSuccess("setLocale(fr...)", status))
        return;

    NumberFormat* numberFmt = NumberFormat::createInstance(Locale("fr"), status);
    if (!assertSuccess("NumberFormat::createInstance()", status))
        return;
    tmf_en->setNumberFormat(*numberFmt, status);
    if (!assertSuccess("setNumberFormat(en...)", status))
        return;
    ...
    if (!assertSuccess("TimeUnitFormat(en...)", status))
        return;
    delete en_long;
...
void TimeUnitTest::testGreekWithFallback()
{
    ...
    TimeUnit::UTimeUnitFields tunits[] = { TimeUnit::UTIMEUNIT_SECOND, TimeUnit::UTIMEUNIT_MINUTE, TimeUnit::UTIMEUNIT_HOUR, TimeUnit::UTIMEUNIT_DAY,
    ...
    const UnicodeString expected[] = { oneSecondStr, oneMinuteStr, oneHourStr, oneDayStr, oneMonthStr, oneYearStr, oneSecondShortStr, oneMinuteShortStr,
        oneHourShortStr, oneDayStr, oneMonthShortStr, oneYearShortStr, sevenSecondsStr, sevenMinutesStr, sevenHoursStr, sevenDaysStr, sevenMonthsStr,
        sevenYearsStr, sevenSecondsShortStr, sevenMinutesShortStr, sevenHoursShortStr, sevenDaysStr, sevenMonthsShortStr, sevenYearsShortStr,

        oneSecondStr, oneMinuteStr, oneHourStr, oneDayStr, oneMonthStr, oneYearStr, oneSecondShortStr, oneMinuteShortStr, oneHourShortStr, oneDayStr,
        oneMonthShortStr, oneYearShortStr, sevenSecondsStr, sevenMinutesStr, sevenHoursStr, sevenDaysStr, sevenMonthsStr, sevenYearsStr, sevenSecondsShortStr,
        sevenMinutesShortStr, sevenHoursShortStr, sevenDaysStr, sevenMonthsShortStr, sevenYearsShortStr };
    ...
    for (unsigned int locIndex = 0; locIndex < UPRV_LENGTHOF(locales); ++locIndex) {
        ...
        for (unsigned int numberIndex = 0; numberIndex < UPRV_LENGTHOF(numbers); ++numberIndex) {

            for (unsigned int styleIndex = 0; styleIndex < UPRV_LENGTHOF(styles); ++styleIndex) {

                for (unsigned int unitIndex = 0; unitIndex < UPRV_LENGTHOF(tunits); ++unitIndex) {
...
#endif
                    if (!assertEquals("formatted time string is not expected, locale: " + UnicodeString(locales[locIndex])
                                + " style: " + (int)styles[styleIndex] + " units: " + (int)tunits[unitIndex],
...
void TimeUnitTest::testGreekWithSanitization()
{
    ...
    if (!assertSuccess("NumberFormat::createInstance for el locale", status, TRUE))
        return;
    numberFmt->setMaximumFractionDigits(1);

    TimeUnitFormat* timeUnitFormat = new TimeUnitFormat(elLoc, status);
    if (!assertSuccess("TimeUnitFormat::TimeUnitFormat for el locale", status))
        return;
...
void TimeUnitTest::test10219Plurals()
{
    ...
    UnicodeString expected[2][3] = { { "1 minute", "1.5 minutes", "1.58 minutes" }, { "1 minute", "1.0 minutes", "1.01 minutes" } };
    UErrorCode status = U_ZERO_ERROR;
    ...
    for (int32_t j = 0; j < UPRV_LENGTHOF(values); ++j) {
        for (int32_t i = 0; i < UPRV_LENGTHOF(expected[j]); ++i) {
            ...
            Formattable fmt;
            LocalPointer<TimeUnitAmount> tamt(new TimeUnitAmount(values[j], TimeUnit::UTIMEUNIT_MINUTE, status), status);
            if (U_FAILURE(status)) {
