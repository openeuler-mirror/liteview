...
UCollationResult IntlTestCollator::compareUsingPartials(
...
void IntlTestCollator::doTestVariant(Collator* col, const UnicodeString& source, const UnicodeString& target, Collator::EComparisonResult result)
{
...
// (maybe moved later)
void IntlTestCollator::reportCResult(const UnicodeString& source, const UnicodeString& target, CollationKey& sourceKey, CollationKey& targetKey,
    Collator::EComparisonResult compareResult, Collator::EComparisonResult keyResult, Collator::EComparisonResult incResult,
...
UnicodeString& IntlTestCollator::appendCompareResult(Collator::EComparisonResult result, UnicodeString& target)
{
    if (result == Collator::LESS) {
        target += "LESS";
    ...
    while ((o = iter.previous(status)) != CollationElementIterator::NULLORDER) {
        ...
        if (o != orders[index].order) {
            if (o == 0)
                ...
                if (o != orders[index].order) {
                    errln("Mismatched order at index %d: 0x%0:8X vs. 0x%0:8X", index, orders[index].order, o);
                    // break;
                    return;
                }
            }
        ...
        if (offset != orders[index].offset) {
            errln("Mismatched offset at index %d: %d vs. %d", index, orders[index].offset, offset);
            // break;
            return;
        }
#endif
...
IntlTestCollator::Order* IntlTestCollator::getOrders(CollationElementIterator& iter, int32_t& orderLength)
{
    ...
    if (U_FAILURE(status)) {
        errln("CollationElementIterator.next() failed - %s", u_errorName(status));
    }
