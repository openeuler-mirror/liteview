...
void UnitsTest::testUnitConstantFreshness()
{
    ...
    LocalUResourceBundlePointer unitConstants(ures_getByKey(unitsBundle.getAlias(), "unitConstants", NULL, status));
    ...
    while (ures_hasNext(unitConstants.getAlias())) {
        ...
        if (status.errDataIfFailureAndReset("addSingleFactorConstant(<%s>, ...).\n\n"
        ...
        assertEquals(UnicodeString("Constant ") + constant + u" numerator", expectedNumerator, factor.factorNum);
...
void UnitsTest::testExtractConvertibility()
{
    ...
    for (const auto& testCase : testCases) {
        ...
        if (status.errIfFailureAndReset("source MeasureUnitImpl::forIdentifier(\"%s\", ...)", testCase.source)) {
            continue;
        }
        MeasureUnitImpl target = MeasureUnitImpl::forIdentifier(testCase.target, status);
        if (status.errIfFailureAndReset("target MeasureUnitImpl::forIdentifier(\"%s\", ...)", testCase.target)) {
            continue;
        }
        ...
        if (status.errIfFailureAndReset("extractConvertibility(<%s>, <%s>, ...)", testCase.source, testCase.target)) {
            continue;
        }

        assertEquals(UnicodeString("Conversion Capability: ") + testCase.source + " to " + testCase.target, testCase.expectedState, convertibility);
    }
}
...
void UnitsTest::testConversionInfo()
{
    ...
    for (const auto& testCase : testCases) {
        ...
        UnicodeString message = UnicodeString("testConverter: ") + testCase.source + " to " + testCase.target;
        ...
        assertEqualsNear(message + ", conversion rate: ", testCase.expectedConversionInfo.conversionRate, actualConversionInfo.conversionRate, maxDelta);

        maxDelta = 1e-6 * uprv_fabs(testCase.expectedConversionInfo.offset);
        if (testCase.expectedConversionInfo.offset == 0) {
            maxDelta = 1e-12;
        }
        assertEqualsNear(message + ", offset: ", testCase.expectedConversionInfo.offset, actualConversionInfo.offset, maxDelta);

        assertEquals(message + ", reciprocal: ", testCase.expectedConversionInfo.reciprocal, actualConversionInfo.reciprocal);
    }
}
...
void UnitsTest::testConverter()
{
    ...
    for (const auto& testCase : testCases) {
        ...
        if (status.errIfFailureAndReset("UnitsConverter(<%s>, <%s>, ...)", testCase.source, testCase.target)) {
            continue;
        }
        assertEqualsNear(UnicodeString("testConverter: ") + testCase.source + " to " + testCase.target, testCase.expectedValue,
            converter.convert(testCase.inputValue), maxDelta);
        assertEqualsNear(UnicodeString("testConverter inverse: ") + testCase.target + " back to " + testCase.source, testCase.inputValue,
            converter.convertInverse(testCase.expectedValue), inverseMaxDelta);
        ...
        assertEqualsNear(UnicodeString("testConverter2: ") + testCase.source + " to " + testCase.target, testCase.expectedValue,
            converter2.convert(testCase.inputValue), maxDelta);
        assertEqualsNear(UnicodeString("testConverter2 inverse: ") + testCase.target + " back to " + testCase.source, testCase.inputValue,
            converter2.convertInverse(testCase.expectedValue), inverseMaxDelta);
    }
}
...
void unitsTestDataLineFn(void* context, char* fields[][2], int32_t fieldCount, UErrorCode* pErrorCode)
{
    ...
    unitsTest->logln("Quantity (Category): \"%.*s\", "
        ...
        quantity.length(), quantity.data(), x.length(), x.data(), y.length(), y.data(), expected, commentConversionFormula.length(),
        commentConversionFormula.data());

    // Convertibility:
    auto convertibility = extractConvertibility(sourceUnit, targetUnit, *ctx->conversionRates, status);
    if (status.errIfFailureAndReset("extractConvertibility(<%s>, <%s>, ...)", sourceIdent.data(), targetIdent.data())) {
        return;
    }
    CharString msg;
    msg.append("convertible: ", status).append(sourceIdent.data(), status).append(" -> ", status).append(targetIdent.data(), status);
    ...
    if (status.errIfFailureAndReset("UnitsConverter(<%s>, <%s>, ...)", sourceIdent.data(), targetIdent.data())) {
        return;
    }
...
void UnitsTest::testComplexUnitsConverter()
{
    ...
    struct TestCase {
        ...
        { "1.9999", "foot", "foot-and-inch", 1.9999,
            { Measure(1, MeasureUnit::createFoot(status), status), Measure(11.9988, MeasureUnit::createInch(status), status) }, 2, 0 },

        // A minimal nudge under 2.0, rounding up to 2.0 ft, 0 in.
        { "2-eps", "foot", "foot-and-inch", 2.0 - DBL_EPSILON,
            { Measure(2, MeasureUnit::createFoot(status), status), Measure(0, MeasureUnit::createInch(status), status) }, 2, 0 },

        // A slightly bigger nudge under 2.0, *not* rounding up to 2.0 ft!
        { "2-3eps", "foot", "foot-and-inch", 2.0 - 3 * DBL_EPSILON,
        ...
        { "2-eps", "light-year", "light-year-and-meter", 2.0 - DBL_EPSILON,
            { Measure(2, MeasureUnit::createLightYear(status), status), Measure(0, MeasureUnit::createMeter(status), status) }, 2, 0 },

        // A 2.1 meter nudge under 1.0 light years, rounding up to 1.0 ly, 0 m.
        { "1-eps", "light-year", "light-year-and-meter", 1.0 - DBL_EPSILON,
            { Measure(1, MeasureUnit::createLightYear(status), status), Measure(0, MeasureUnit::createMeter(status), status) }, 2, 0 },
        ...
        { "1 + 1e-15", "light-year", "light-year-and-meter", 1.0 + 1e-15,
            { Measure(1, MeasureUnit::createLightYear(status), status), Measure(9.46073, MeasureUnit::createMeter(status), status) }, 2,
            1.5 /* meters, precision */ },

        // 2.1 meters more than 1 light year is not rounded away.
        { "1 + eps", "light-year", "light-year-and-meter", 1.0 + DBL_EPSILON,
            { Measure(1, MeasureUnit::createLightYear(status), status), Measure(2.1, MeasureUnit::createMeter(status), status) }, 2, 0.001 },

        // Negative numbers
        { "Negative number conversion", "yard", "mile-and-yard", -1800,
            { Measure(-1, MeasureUnit::createMile(status), status), Measure(-40, MeasureUnit::createYard(status), status) }, 2, 1e-10 },
    };
    ...
    auto testATestCase = [&](const ComplexUnitsConverter& converter, StringPiece initMsg, const TestCase& testCase) {
        ...
        for (int i = 0; i < measures.length() && i < testCase.expectedCount; i++) {
            if (i == testCase.expectedCount - 1) {
                assertEqualsNear(msg.data(), testCase.expected[i].getNumber().getDouble(status), measures[i]->getNumber().getDouble(status), testCase.accuracy);
            } else {
                assertEquals(msg.data(), testCase.expected[i].getNumber().getDouble(status), measures[i]->getNumber().getDouble(status));
            }
            assertEquals(msg.data(), testCase.expected[i].getUnit().getIdentifier(), measures[i]->getUnit().getIdentifier());
        }
    };
...
void UnitsTest::testComplexUnitsConverterSorting()
{
    ...
    struct TestCase {
        ...
        // For mixed units, accuracy of the smallest unit
        double accuracy;
    } testCases[] { { "inch-and-foot", "meter", "inch-and-foot", 10.0,
                        {
                        ...
                        2, 0.00001 },
        { "inch-and-yard-and-foot", "meter", "inch-and-yard-and-foot", 100.0,
            {
    ...
    for (const auto& testCase : testCases) {
        ...
        for (int i = 0; i < testCase.expectedCount; i++) {
            assertEquals(testCase.msg, testCase.expected[i].getUnit().getIdentifier(), actual[i]->getUnit().getIdentifier());

            if (testCase.expected[i].getNumber().getType() == Formattable::Type::kInt64) {
                assertEquals(testCase.msg, testCase.expected[i].getNumber().getInt64(), actual[i]->getNumber().getInt64());
            } else {
                assertEqualsNear(testCase.msg, testCase.expected[i].getNumber().getDouble(), actual[i]->getNumber().getDouble(), testCase.accuracy);
            }
        }
    }
}
...
void checkOutput(UnitsTest* unitsTest, const char* msg, ExpectedOutput expected, const MaybeStackVector<Measure>& actual, double precision)
{
    ...
    for (int i = 0; i < actual.length(); i++) {
        ...
        unitsTest->assertEqualsNear(testMessage.data(), expected._amounts[i], actual[i]->getNumber().getDouble(status), permittedDiff);
    }
}
 ...
 */
void unitPreferencesTestDataLineFn(void* context, char* fields[][2], int32_t fieldCount, UErrorCode* pErrorCode)
{
    if (U_FAILURE(*pErrorCode))
        return;
    ...
    if (status.errIfFailureAndReset("parsing decimal quantity: \"%.*s\"", inputD.length(), inputD.data())) {
        return;
    }
    ...
    unitsTest->logln("Quantity (Category): \"%.*s\", Usage: \"%.*s\", Region: \"%.*s\", "
        ...
        quantity.length(), quantity.data(), usage.length(), usage.data(), region.length(), region.data(), inputAmount, inputMeasureUnit.getIdentifier(),
    ...
    if (status.errIfFailureAndReset(
            "UnitsRouter(<%s>, \"%.*s\", \"%.*s\", status)", inputMeasureUnit.getIdentifier(), region.length(), region.data(), usage.length(), usage.data())) {
        return;
    }
    ...
    if (status.errIfFailureAndReset(
            "UnitsRouter2(<%s>, \"%.*s\", \"%.*s\", status)", inputUnitIdentifier.data(), region.length(), region.data(), usage.length(), usage.data())) {
        return;
    }
 ...
 * line.
 */
void parsePreferencesTests(
    const char* filename, char delimiter, char* fields[][2], int32_t maxFieldCount, UParseLineFn* lineFn, void* context, UErrorCode* pErrorCode)
{
    FileStream* file;
    char line[10000];
    char *start, *limit;
...
void UnitsTest::testUnitPreferencesWithCLDRTests()
{
    ...
    parsePreferencesTests(path.data(), ';', fields, maxFields, unitPreferencesTestDataLineFn, this, errorCode);
    if (errorCode.errIfFailureAndReset("error parsing %s: %s\n", path.data(), u_errorName(errorCode))) {
        return;
    }
}
