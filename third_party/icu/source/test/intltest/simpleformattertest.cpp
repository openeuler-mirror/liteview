...
private:
    void verifyOffsets(const int32_t* expected, const int32_t* actual, int32_t count);
};
...
void SimpleFormatterTest::TestNoArguments()
{
    ...
    assertEquals("format", "This doesn't have templates {0}", fmt.format("unused", appendTo, status));
    appendTo.remove();
    int32_t offsets[] = { 0 };
    assertEquals("formatAndAppend", "This doesn't have templates {0}", fmt.formatAndAppend(NULL, 0, appendTo, offsets, 1, status));
    assertEquals("formatAndAppend offsets[0]", -1, offsets[0]);
    assertEquals("formatAndReplace", "This doesn't have templates {0}", fmt.formatAndReplace(NULL, 0, appendTo, NULL, 0, status));
...
void SimpleFormatterTest::TestOneArgument()
{
    ...
    UnicodeString appendTo;
    assertEquals("format", "1 meter", fmt.format("1", appendTo, status));

    // assignment
    SimpleFormatter s;
    s = fmt;
    appendTo.remove();
    assertEquals("Assignment", "1 meter", s.format("1", appendTo, status));

    // Copy constructor
    SimpleFormatter r(fmt);
    appendTo.remove();
    assertEquals("Copy constructor", "1 meter", r.format("1", appendTo, status));
    assertSuccess("Status", status);
}
...
void SimpleFormatterTest::TestBigArgument()
{
    ...
    UnicodeString b("b");
    UnicodeString* values[] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &b };
...
void SimpleFormatterTest::TestManyArguments()
{
    UErrorCode status = U_ZERO_ERROR;
    SimpleFormatter fmt;
    fmt.applyPattern("Templates {2}{1}{5} and {4} are out of order.", status);
    if (!assertSuccess("Status", status)) {
        return;
    }
    assertEquals("getArgumentLimit", 6, fmt.getArgumentLimit());
    UnicodeString values[] = { "freddy", "tommy", "frog", "billy", "leg", "{0}" };
    UnicodeString* params[] = { &values[0], &values[1], &values[2], &values[3], &values[4], &values[5] };
    int32_t offsets[6];
    int32_t expectedOffsets[6] = { -1, 22, 18, -1, 35, 27 };
    UnicodeString appendTo("Prefix: ");
    assertEquals("format", "Prefix: Templates frogtommy{0} and leg are out of order.",
        fmt.formatAndAppend(params, UPRV_LENGTHOF(params), appendTo, offsets, UPRV_LENGTHOF(offsets), status));
    if (!assertSuccess("Status", status)) {
        return;
    }
    ...
    appendTo.remove();
    fmt.formatAndAppend(params, UPRV_LENGTHOF(params), appendTo, offsets, UPRV_LENGTHOF(offsets) - 1, status);
    ...
    s = fmt;
    appendTo.remove();
    assertEquals("Assignment", "Templates frogtommy{0} and leg are out of order.", s.formatAndAppend(params, UPRV_LENGTHOF(params), appendTo, NULL, 0, status));

    // Copy constructor
    SimpleFormatter r(fmt);
    appendTo.remove();
    assertEquals(
        "Copy constructor", "Templates frogtommy{0} and leg are out of order.", r.formatAndAppend(params, UPRV_LENGTHOF(params), appendTo, NULL, 0, status));
    r.applyPattern("{0} meter", status);
    assertEquals("getArgumentLimit", 1, r.getArgumentLimit());
    appendTo.remove();
    assertEquals("Replace with new applyPattern", "freddy meter", r.format("freddy", appendTo, status));
    r.applyPattern("{0}, {1}", status);
    assertEquals("getArgumentLimit", 2, r.getArgumentLimit());
    appendTo.remove();
    assertEquals("2 arg", "foo, bar", r.format("foo", "bar", appendTo, status));
    r.applyPattern("{0}, {1} and {2}", status);
    assertEquals("getArgumentLimit", 3, r.getArgumentLimit());
    appendTo.remove();
    assertEquals("3 arg", "foo, bar and baz", r.format("foo", "bar", "baz", appendTo, status));
    assertSuccess("Status", status);
}
...
void SimpleFormatterTest::TestTooFewArgumentValues()
{
    ...
    fmt.format(firstValue, appendTo, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
        errln("Expected U_ILLEGAL_ARGUMENT_ERROR");
    }

    status = U_ZERO_ERROR;
    fmt.formatAndAppend(params, UPRV_LENGTHOF(params), appendTo, NULL, 0, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
        errln("Expected U_ILLEGAL_ARGUMENT_ERROR");
    }

    status = U_ZERO_ERROR;
    fmt.formatAndReplace(params, UPRV_LENGTHOF(params), appendTo, NULL, 0, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
        errln("Expected U_ILLEGAL_ARGUMENT_ERROR");
    }
}
...
void SimpleFormatterTest::TestBadArguments()
{
    ...
    // These succeed
    fmt.formatAndAppend(NULL, 0, appendTo, NULL, 0, status);
    fmt.formatAndReplace(NULL, 0, appendTo, NULL, 0, status);
    assertSuccess("", status);
    status = U_ZERO_ERROR;

    // fails
    fmt.formatAndAppend(NULL, 1, appendTo, NULL, 0, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
    ...
    // fails
    fmt.formatAndAppend(NULL, 0, appendTo, NULL, 1, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
    ...
    // fails
    fmt.formatAndReplace(NULL, 1, appendTo, NULL, 0, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
    ...
    // fails
    fmt.formatAndReplace(NULL, 0, appendTo, NULL, 1, status);
    if (status != U_ILLEGAL_ARGUMENT_ERROR) {
...
void SimpleFormatterTest::TestFormatReplaceNoOptimization()
{
    ...
    assertEquals("", "frog, original, freddy and by", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, offsets, UPRV_LENGTHOF(offsets), status));
    if (!assertSuccess("Status", status)) {
        return;
    }
...
void SimpleFormatterTest::TestFormatReplaceNoOptimizationLeadingText()
{
    ...
    assertEquals("", "boo original, freddy, frog and by", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, offsets, UPRV_LENGTHOF(offsets), status));
    if (!assertSuccess("Status", status)) {
        return;
    }
...
void SimpleFormatterTest::TestFormatReplaceOptimization()
{
    ...
    UnicodeString by("by");
    const UnicodeString* params[] = { &freddy, &frog, &result, &by };
    assertEquals("", "original, freddy, frog and by", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, offsets, UPRV_LENGTHOF(offsets), status));
    if (!assertSuccess("Status", status)) {
        return;
    }
...
void SimpleFormatterTest::TestFormatReplaceNoOptimizationLeadingArgumentUsedTwice()
{
    ...
    const UnicodeString* params[] = { &freddy, &frog, &result, &by };
    assertEquals(
        "", "original, freddy, frog and by original", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, offsets, UPRV_LENGTHOF(offsets), status));
    if (!assertSuccess("Status", status)) {
        return;
    }
...
void SimpleFormatterTest::TestFormatReplaceOptimizationNoOffsets()
{
    ...
    UnicodeString by("by");
    const UnicodeString* params[] = { &freddy, &frog, &result, &by };
    assertEquals("", "original, freddy, frog and by", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, NULL, 0, status));
    assertSuccess("Status", status);
}
...
void SimpleFormatterTest::TestFormatReplaceNoOptimizationNoOffsets()
{
    ...
    assertEquals("", "Arguments previous: and frog", fmt.formatAndReplace(params, UPRV_LENGTHOF(params), result, NULL, 0, status));
    assertSuccess("Status", status);
}
