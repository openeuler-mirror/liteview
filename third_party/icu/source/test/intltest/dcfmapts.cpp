...
{
    if (exec)
        logln((UnicodeString) "TestSuite DecimalFormatAPI");
    switch (index) {
    case 0:
        name = "DecimalFormat API test";
        if (exec) {
            logln((UnicodeString) "DecimalFormat API test---");
            logln((UnicodeString) "");
...
void IntlTestDecimalFormatAPI::TestScale()
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(testData); i++) {
        ...
        message = UnicodeString("Unexpected output for ") + testData[i].inputValue + UnicodeString(" and scale ") + testData[i].inputScale
            + UnicodeString(". Got: ");
...
void IntlTestDecimalFormatAPI::TestFixedDecimal()
{
    ...
    df.adoptInsteadAndCheckErrorCode(new DecimalFormat("#####################0.00####################", status), status);
    assertSuccess(WHERE, status);
...
void IntlTestDecimalFormatAPI::testInvalidObject()
{
...

    {
        ...
        for (DecimalFormat* df : { &dfBogus1, &dfBogus2, &dfBogus3, &dfBogus4, &dfBogus5 }) {
            ...
            assertEquals("toNumberFormatter should return nullptr", (int64_t) nullptr, (int64_t)lnf);
