...
{
    return cp - 48 < 10 ? cp - 22 : cp - 65 < 26 ? cp - 65 : cp - 97 < 26 ? cp - 97 : static_cast<punycode_uint>(base);
}
...
enum punycode_status punycode_encode(
    punycode_uint input_length, const punycode_uint input[], const unsigned char case_flags[], punycode_uint* output_length, char output[])
{
    ...
    for (j = 0; j < input_length; ++j) {
        if (basic(input[j])) {
            ...
            output[out++] = (char)(case_flags ? encode_basic(input[j], case_flags[j]) : input[j]);
        }
            ...
            if (input[j] == n) {
                ...
                for (q = delta, k = base;; k += base) {
                    ...
                    t = k <= bias /* + tmin */ ? static_cast<punycode_uint>(tmin) : /* +tmin not needed */
                        k >= bias + tmax ? static_cast<punycode_uint>(tmax)
                                         : k - bias;
                    if (q < t)
                        break;
...
enum punycode_status punycode_decode(
    punycode_uint input_length, const char input[], punycode_uint* output_length, punycode_uint output[], unsigned char case_flags[])
{
