...
void CollationIteratorTest::TestPrevious(/* char* par */)
{
    ...
    c1 = new RuleBasedCollator((UnicodeString) "&a,A < b,B < c,C, d,D < z,Z < ch,cH,Ch,CH", status);
...
void CollationIteratorTest::TestSetText(/* char* par */)
{
    ...
    if (U_FAILURE(status) || iter1->next(status) != (int32_t)CollationElementIterator::NULLORDER) {
...
void CollationIteratorTest::TestMaxExpansion(/* char* par */)
{
    ...
    while (ch < 0xFFFF && U_SUCCESS(status)) {
        ...
        while (U_SUCCESS(status) && iter->previous(status) != (int32_t)CollationElementIterator::NULLORDER) {
            count++;
        }
...
void CollationIteratorTest::TestAssignment()
{
    ...
    UnicodeString source("abcd");
    CollationElementIterator* iter1 = coll->createCollationElementIterator(source);
    ...
    CollationElementIterator* iter4 = coll->createCollationElementIterator(source);
...
void CollationIteratorTest::TestConstructors()
{
    ...
    CollationElementIterator* iter1 = coll->createCollationElementIterator(chariter);
    if (U_FAILURE(status)) {
        errln("Couldn't create collation element iterator with character iterator.");
        return;
    }
    CollationElementIterator* iter2 = coll->createCollationElementIterator(test1);

    // initially the 2 collation element iterators should be the same
    if (*iter1 != *iter1 || *iter2 != *iter2 || *iter1 != *iter2 || *iter2 != *iter1) {
...
void CollationIteratorTest::TestStrengthOrder()
{
    ...
    CollationElementIterator* iter = coll->createCollationElementIterator(test1);
...
void CollationIteratorTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    ...
    if (en_us) {
        ...
        case 2:
            ...
            break;
        case 3:
            name = "TestMaxExpansion";
            if (exec)
                TestMaxExpansion(/* par */);
            break;
        case 4:
            name = "TestClearBuffers";
            if (exec)
                TestClearBuffers(/* par */);
            break;
        case 5:
            name = "TestUnicodeChar";
            if (exec)
                TestUnicodeChar(/* par */);
            break;
        case 6:
        ...
        case 7:
            ...
            if (exec)
                TestConstructors(/* par */);
            break;
        case 8:
            name = "TestStrengthOrder";
            if (exec)
                TestStrengthOrder(/* par */);
            break;
        default:
            name = "";
            break;
