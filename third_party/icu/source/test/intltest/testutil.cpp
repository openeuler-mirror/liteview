 ...
 */
int32_t srcIndexFromDest(const EditChange expected[], int32_t expLength, int32_t srcLength, int32_t destLength, int32_t index)
{
...
int32_t destIndexFromSrc(const EditChange expected[], int32_t expLength, int32_t srcLength, int32_t destLength, int32_t index)
{
    int32_t srcIndex = srcLength;
    int32_t destIndex = destLength;
    int32_t i = expLength;
...
UBool TestUtility::checkEqualEdits(IntlTest& test, const UnicodeString& name, const Edits& e1, const Edits& e2, UErrorCode& errorCode)
{
    ...
    for (int32_t i = 0; ok; ++i) {
        ...
        ok &= test.assertEquals(name + u" next()[" + i + u"]" + __LINE__, ei1HasNext, ei2HasNext);
        ok &= test.assertSuccess(name + u" errorCode[" + i + u"]" + __LINE__, errorCode);
        ok &= test.assertEquals(name + u" edit[" + i + u"]" + __LINE__, printOneEdit(ei1), printOneEdit(ei2));
...
void TestUtility::checkEditsIter(IntlTest& test, const UnicodeString& name, Edits::Iterator ei1, Edits::Iterator ei2, // two equal iterators
    const EditChange expected[], int32_t expLength, UBool withUnchanged, UErrorCode& errorCode)
{
    ...
    for (auto i = 0; i < (int32_t)srcIndexes.size(); ++i) {
        for (int32_t ij = 0; ij < UPRV_LENGTHOF(ZIG_ZAG); ++ij) {
            ...
            if ((i + j) < (int32_t)srcIndexes.size()) {
                ...
                test.assertEquals(name + u" destIndexFromSrc(" + si + u"):" + __LINE__, destIndexFromSrc(expected, expLength, srcLength, destLength, si),
    ...
    for (auto i = 0; i < (int32_t)destIndexes.size(); ++i) {
        for (int32_t ij = 0; ij < UPRV_LENGTHOF(ZIG_ZAG); ++ij) {
            ...
            if ((i + j) < (int32_t)destIndexes.size()) {
                ...
                test.assertEquals(name + u" srcIndexFromDest(" + di + u"):" + __LINE__, srcIndexFromDest(expected, expLength, srcLength, destLength, di),
