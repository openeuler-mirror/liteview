...
void AlphabeticIndexTest::APITest()
{
    ...
    //
    status = U_ZERO_ERROR;
    LocalPointer<RuleBasedCollator> coll(dynamic_cast<RuleBasedCollator*>(Collator::createInstance(Locale::getGerman(), status)), status);
    TEST_CHECK_STATUS;
    ...
    assertEquals("only the underflow label in an index built from a collator", 1, index->getBucketCount(status));
    TEST_CHECK_STATUS;
    index.adoptInstead(nullptr);
    ...
    lc = index->getBucketCount(status);
    TEST_CHECK_STATUS;
    assertEquals("underflow, A-Z, inflow, 2 Cyrillic, overflow", 31, index->getBucketCount(status));
    ...
    ELLIPSIS.append((UChar32)0x2026);
...
static const char* KEY_LOCALES[] = { "en", "es", "de", "fr", "ja", "it", "tr", "pt", "zh", "nl", "pl", "ar", "ru", "zh_Hant", "ko", "th", "sv", "fi", "da",
    "he", "nb", "el", "hr", "bg", "sk", "lt", "vi", "lv", "sr", "pt_PT", "ro", "hu", "cs", "id", "sl", "fil", "fa", "uk", "ca", "hi", "et", "eu", "is", "sw",
    "ms", "bn", "am", "ta", "te", "mr", "ur", "ml", "kn", "gu", "or", "" };
...
void AlphabeticIndexTest::ManyLocalesTest()
{
    ...
    for (int i = 0;; ++i) {
        ...
        while (index.nextBucket(status)) {
            ...
            assertEquals("bucket label vs. immutable: locale=" + UnicodeString(localeName) + " index=" + bucketIndex, label, bucket->getLabel());
...
static const char* pinyinTestData[] = { "\\u0101", "\\u5416", "\\u58ba", //
    "b", "\\u516b", "\\u62d4", "\\u8500", //
    "c", "\\u5693", "\\u7938", "\\u9e7e", //
    "d", "\\u5491", "\\u8fcf", "\\u964a", //
    "\\u0113", "\\u59b8", "\\u92e8", "\\u834b", //
    ...
    while (aindex.nextBucket(status)) {
        ...
        while (aindex.nextRecord(status)) {
            ...
            if (aindex.getBucketLabelType() != U_ALPHAINDEX_NORMAL) {
                errln("File %s, Line %d, Name \"\\u%x\" is in an under or overflow bucket.", __FILE__, __LINE__, name.char32At(0));
            }
            // s.clear();
...
static const char* localeAndIndexCharactersLists[][2] = {
    ...
    { "zh_Hant",
        "1\\u5283:2\\u5283:3\\u5283:4\\u5283:5\\u5283:6\\u5283:7\\u5283:8\\u5283:9\\u5283:10\\u5283:11\\u5283:12\\u5283:13\\u5283:14\\u5283:15\\u5283:"
        "16\\u5283:17\\u5283:"
        "18\\u5283:19\\u5283:20\\u5283:21\\u5283:22\\u5283:23\\u5283:24\\u5283:25\\u5283:26\\u5283:27\\u5283:28\\u5283:29\\u5283:30\\u5283:31\\u5283:32\\u5283:"
        "33\\u5283:35\\u5283:"
        "36\\u5283:39\\u5283:48\\u5283" },
};
...
void AlphabeticIndexTest::TestIndexCharactersList()
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(localeAndIndexCharactersLists); ++i) {
        ...
        UnicodeString expectedIndexCharacters = (UnicodeString("\\u2026:") + localeAndIndexCharacters[1] + ":\\u2026").unescape();
        ...
        assertEquals(locale, expectedIndexCharacters, joinLabelsAndAppend(*immIndex, actualIndexCharacters));
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedCollator> coll(static_cast<RuleBasedCollator*>(Collator::createInstance(Locale::getRoot(), status)));
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedCollator> coll(static_cast<RuleBasedCollator*>(Collator::createInstance(Locale::getChinese(), status)));
    if (U_FAILURE(status)) {
        dataerrln("Failed Collator::createInstance call - %s", u_errorName(status));
        return;
    }
...
void AlphabeticIndexTest::TestSchSt()
{
    ...
    } testCases[] = { // name, bucket index, bucket label
        { "Adelbert", 1, "A" }, { "Afrika", 1, "A" }, { "\\u00C6sculap", 2, "\\u00C6" }, { "Aesthet", 2, "\\u00C6" }, { "Berlin", 3, "B" },
        { "Rilke", 19, "R" }, { "Sacher", 20, "S" }, { "Seiler", 20, "S" }, { "Sultan", 20, "S" }, { "Schiller", 21, "Sch" }, { "Steiff", 22, "St" },
        { "Thomas", 23, "T" }
    };
...
void AlphabeticIndexTest::TestChineseUnihan()
{
    ...
    if (bucketCount < 216) {
        ...
        dataerrln("too few buckets/labels for Chinese/unihan: %d (is zh/unihan data available?)", bucketCount);
        return;
    } else {
...
void AlphabeticIndexTest::checkHasBuckets(const Locale& locale, UScriptCode script)
{
    ...
    assertEquals(loc + u" expected script", script, uscript_getScript(bucket->getLabel().char32At(0), errorCode));
}
