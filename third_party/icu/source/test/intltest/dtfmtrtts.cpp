...
void DateFormatRoundTripTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* par)
{
    optionv = (par && *par == 'v');
    switch (index) {
...
UBool DateFormatRoundTripTest::failure(UErrorCode status, const char* msg)
{
    if (U_FAILURE(status)) {
...
UBool DateFormatRoundTripTest::failure(UErrorCode status, const char* msg, const UnicodeString& str)
{
    if (U_FAILURE(status)) {
...
void DateFormatRoundTripTest::test(const Locale& loc)
{
    ...
    for (style = DateFormat::FULL; style <= DateFormat::SHORT; ++style) {
        if (TEST_TABLE[itable++]) {
            ...
            if (df == NULL) {
                errln(UnicodeString("Could not DF::createDateInstance ") + UnicodeString(styleName((DateFormat::EStyle)style))
                    + " Locale: " + loc.getDisplayName(temp));
            } else {
                test(df, loc);
                delete df;
            }
    ...
    for (style = DateFormat::FULL; style <= DateFormat::SHORT; ++style) {
        if (TEST_TABLE[itable++]) {
            ...
            if (df == NULL) {
                errln(UnicodeString("Could not DF::createTimeInstance ") + UnicodeString(styleName((DateFormat::EStyle)style))
                    + " Locale: " + loc.getDisplayName(temp));
            } else {
                test(df, loc, TRUE);
                delete df;
    ...
    for (int32_t dstyle = DateFormat::FULL; dstyle <= DateFormat::SHORT; ++dstyle) {
        for (int32_t tstyle = DateFormat::FULL; tstyle <= DateFormat::SHORT; ++tstyle) {
            if (TEST_TABLE[itable++]) {
                logln("Testing dstyle" + UnicodeString(styleName((DateFormat::EStyle)dstyle)) + ", tstyle"
                    + UnicodeString(styleName((DateFormat::EStyle)tstyle)));
                DateFormat* df = DateFormat::createDateTimeInstance((DateFormat::EStyle)dstyle, (DateFormat::EStyle)tstyle, loc);
                if (df == NULL) {
                    dataerrln(UnicodeString("Could not DF::createDateTimeInstance ") + UnicodeString(styleName((DateFormat::EStyle)dstyle)) + ", tstyle"
                        + UnicodeString(styleName((DateFormat::EStyle)tstyle)) + "Locale: " + loc.getDisplayName(temp));
                } else {
                    test(df, loc);
                    delete df;
                }
...
void DateFormatRoundTripTest::test(DateFormat* fmt, const Locale& origLocale, UBool timeOnly)
{
    ...
    UBool hasZoneDisplayName = (pat.indexOf(UnicodeString("z")) != -1) || (pat.indexOf(UnicodeString("v")) != -1) || (pat.indexOf(UnicodeString("V")) != -1);
    ...
    for (int i = 0; i < TRIALS; ++i) {
            ...
            if (loop > 0) {
                ...
                if (U_FAILURE(status)) {
                    ...
                    failure(status, "fmt->parse",
                        s[loop - 1] + " in locale: " + origLocale.getName() + " with pattern: " + pat + " has errIndex: " + ppos.getErrorIndex());
        ...
        if (smatch > maxSmatch) { // Don't compute unless necessary
            ...
            else if (!in0 && fmt->getTimeZone().inDaylightTime(d[1], status) && !failure(status, "gettingDaylightTime"))
                maxSmatch = 2;
            // Two digit year with no time zone change,
            // unless timezone isn't used or we aren't close to the DST changover
            else if (pat.indexOf(UnicodeString("y")) != -1 && pat.indexOf(UnicodeString("yyyy")) == -1
                && getField(d[0], UCAL_YEAR) != getField(d[dmatch], UCAL_YEAR) && !failure(status, "error status [smatch>maxSmatch]")
                && ((hasZoneDisplayName
                        && (fmt->getTimeZone().inDaylightTime(d[0], status) == fmt->getTimeZone().inDaylightTime(d[dmatch], status)
                            || getField(d[0], UCAL_MONTH) == UCAL_APRIL || getField(d[0], UCAL_MONTH) == UCAL_OCTOBER))
                    || !hasZoneDisplayName)) {
                maxSmatch = 2;
            }
        ...
        if (optionv || fail) {
            if (fail) {
                errln(UnicodeString("\nFAIL: Pattern: ") + pat + " in Locale: " + origLocale.getName() + "\nget dmatch: " + dmatch + " (expected max "
                    + maxDmatch + "), smatch: " + smatch + " (expected max " + maxSmatch + ")");
            } else {
                errln(UnicodeString("\nOk: Pattern: ") + pat + " in Locale: " + origLocale.getName());
            }

            logln("Date iters until match=%d (max allowed=%d), string iters until match=%d (max allowed=%d)", dmatch, maxDmatch, smatch, maxSmatch);

            for (int j = 0; j <= loop && j < DEPTH; ++j) {
                UnicodeString temp;
                FieldPosition pos(FieldPosition::DONT_CARE);
                errln((j > 0 ? " P> " : "    ") + fullFormat(d[j]) + " F> " + escape(s[j], temp) + UnicodeString(" d=") + d[j]
                    + (j > 0 && d[j] /*.getTime()*/ == d[j - 1] /*.getTime()*/ ? " d==" : "") + (j > 0 && s[j] == s[j - 1] ? " s==" : ""));
            }
        }
        delete[] d;
        delete[] s;
