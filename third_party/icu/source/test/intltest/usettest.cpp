...
#define TEST_ASSERT_SUCCESS(status)                                                                                                                            \
    ...
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
            dataerrln("fail in file \"%s\", line %d: \"%s\"", __FILE__, __LINE__, u_errorName(status));                                                        \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
...
void UnicodeSetTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    if (exec) {
...
void UnicodeSetTest::TestToPattern()
{
    ...
    // whitespace.
    {
        static const char* OTHER_TOPATTERN_TESTS[] = { "[[:latin:]&[:greek:]]", "[[:latin:]-[:greek:]]", "[:nonspacing mark:]", NULL };
...
UBool UnicodeSetTest::checkPat(const UnicodeString& source, const UnicodeSet& testSet)
{
    ...
    if (!checkPat(source + " (escaped)", testSet, pat0))
        return FALSE;
    ...
    if (!checkPat(source, testSet, pat2))
        return FALSE;
...
UBool UnicodeSetTest::checkPat(const UnicodeString& source, const UnicodeSet& testSet, const UnicodeString& pat)
{
    UErrorCode ec = U_ZERO_ERROR;
...
void UnicodeSetTest::TestPatterns(void)
{
    UnicodeSet set;
...
void UnicodeSetTest::TestCategories(void)
{
    ...
    for (i = 0; i < 0x200; ++i) {
        ...
        if (l != set.contains(i)) {
            errln((UnicodeString) "FAIL: L contains " + (unsigned short)i + " = " + set.contains(i));
            if (++failures == 10)
                break;
        }
    }
    ...
    for (i = 0; i < 0x200; ++i) {
        ...
        if (lu != set.contains(i)) {
            errln((UnicodeString) "FAIL: Lu contains " + (unsigned short)i + " = " + set.contains(i));
            if (++failures == 20)
                break;
        }
    }
}
void UnicodeSetTest::TestCloneEqualHash(void)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (*set1 != *set1copy || *set1 == *set2 || getPairs(*set1) != getPairs(*set1copy) || set1->hashCode() != set1copy->hashCode()) {
    ...
    if (set1equal != *set1 || set1equal != *set1copy || set2equal != *set2 || set2equal == *set1 || set2equal == *set1copy || set2equal == set1equal) {
    ...
    if (*set1clone != *set1 || *set1clone != *set1copy || *set1clone != set1equal || *set2clone != *set2 || *set2clone == *set1copy || *set2clone != set2equal
        || *set2clone == *set1 || *set2clone == set1equal || *set2clone == *set1clone) {
        errln("FAIL: Error in clone");
    }

    logln("Testing hashcode");
    if (set1->hashCode() != set1equal.hashCode() || set1->hashCode() != set1clone->hashCode() || set2->hashCode() != set2equal.hashCode()
        || set2->hashCode() != set2clone->hashCode() || set1copy->hashCode() != set1equal.hashCode() || set1copy->hashCode() != set1clone->hashCode()
        || set1->hashCode() == set2->hashCode() || set1copy->hashCode() == set2->hashCode() || set2->hashCode() == set1clone->hashCode()
        || set2->hashCode() == set1equal.hashCode()) {
...
void UnicodeSetTest::TestAddRemove(void)
{
...
void UnicodeSetTest::TestAPI()
{
    ...
    set.clear();
    if (set.size() != 0) {
        errln((UnicodeString) "FAIL, size should be 0, but is " + set.size() + ": " + set);
    }
    set.add(0x0061);
    if (set.size() != 1) {
        errln((UnicodeString) "FAIL, size should be 1, but is " + set.size() + ": " + set);
    }
    set.add(0x0031, 0x0039);
    if (set.size() != 10) {
        errln((UnicodeString) "FAIL, size should be 10, but is " + set.size() + ": " + set);
    }
    ...
    for (int32_t i = 0; i < set.getRangeCount(); ++i) {
        ...
        if (!set.contains(a, b)) {
            errln((UnicodeString) "FAIL, should contain " + (unsigned short)a + '-' + (unsigned short)b + " but doesn't: " + set);
        }
        if (set.contains((UChar32)(a - 1), b)) {
            errln((UnicodeString) "FAIL, shouldn't contain " + (unsigned short)(a - 1) + '-' + (unsigned short)b + " but does: " + set);
        }
        if (set.contains(a, (UChar32)(b + 1))) {
            errln((UnicodeString) "FAIL, shouldn't contain " + (unsigned short)a + '-' + (unsigned short)(b + 1) + " but does: " + set);
        }
    }
    ...
    if (ac.span(longString, -5, USET_SPAN_CONTAINED) != 10 || ac.span(longString, 0, USET_SPAN_CONTAINED) != 10
        || ac.span(longString, 5, USET_SPAN_CONTAINED) != 10 || ac.span(longString, 10, USET_SPAN_CONTAINED) != 10
        || ac.span(longString, 15, USET_SPAN_CONTAINED) != 15 || ac.span(longString, 20, USET_SPAN_CONTAINED) != 30
        || ac.span(longString, 25, USET_SPAN_CONTAINED) != 30 || ac.span(longString, 30, USET_SPAN_CONTAINED) != 30
        || ac.span(longString, 35, USET_SPAN_CONTAINED) != 30 || ac.span(longString, INT32_MAX, USET_SPAN_CONTAINED) != 30) {
        errln("UnicodeSet.span(UnicodeString, ...) returns incorrect end indexes");
    }
    if (ac.spanBack(longString, -5, USET_SPAN_CONTAINED) != 0 || ac.spanBack(longString, 0, USET_SPAN_CONTAINED) != 0
        || ac.spanBack(longString, 5, USET_SPAN_CONTAINED) != 0 || ac.spanBack(longString, 10, USET_SPAN_CONTAINED) != 0
        || ac.spanBack(longString, 15, USET_SPAN_CONTAINED) != 15 || ac.spanBack(longString, 20, USET_SPAN_CONTAINED) != 20
        || ac.spanBack(longString, 25, USET_SPAN_CONTAINED) != 20 || ac.spanBack(longString, 30, USET_SPAN_CONTAINED) != 20
        || ac.spanBack(longString, 35, USET_SPAN_CONTAINED) != 20 || ac.spanBack(longString, INT32_MAX, USET_SPAN_CONTAINED) != 20) {
...
void UnicodeSetTest::TestStrings()
{
    ...
    UnicodeSet* testList[] = { UnicodeSet::createFromAll("abc"), new UnicodeSet("[a-c]", ec),

        &(UnicodeSet::createFrom("ch")->add('a', 'z').add("ll")), new UnicodeSet("[{ll}{ch}a-z]", ec),

        UnicodeSet::createFrom("ab}c"), new UnicodeSet("[{ab\\}c}]", ec),

        &((new UnicodeSet('a', 'z'))->add('A', 'Z').retain('M', 'm').complement('X')), new UnicodeSet("[[a-zA-Z]&[M-m]-[X]]", ec),
...
void UnicodeSetTest::TestPropertySet()
{
    ...
    for (int32_t i = 0; i < DATA_LEN; i += 3) {
        expectContainment(UnicodeString(DATA[i], -1, US_INV), CharsToUnicodeString(DATA[i + 1]), CharsToUnicodeString(DATA[i + 2]));
    }
}
...
void UnicodeSetTest::TestPosixClasses()
{
    ...
    }
    {
        ...
        TEST_ASSERT_SUCCESS(status);
        UnicodeSet s2(u"[\\p{Whitespace}-[\\u000a\\u000B\\u000c\\u000d\\u0085\\p{LineSeparator}\\p{ParagraphSeparator}]]", status);
        TEST_ASSERT_SUCCESS(status);
        TEST_ASSERT(s1 == s2);
    }
    {
...
void UnicodeSetTest::TestIndexOf()
{
    ...
    for (int32_t i = 0; i < set.size(); ++i) {
        ...
        if (set.indexOf(c) != i) {
            errln("FAIL: charAt(%d) = %X => indexOf() => %d", i, c, set.indexOf(c));
        }
    }
...
void UnicodeSetTest::TestCloseOver()
{
    ...
    const char* DATA[] = {
        ...
        CASE,
        "[{a\\u02BE}]",
        "[\\u1E9A{a\\u02BE}]", // first in sorted table

        CASE,
        "[{\\u1f7c\\u03b9}]",
        "[\\u1ff2{\\u1f7c\\u03b9}]", // last in sorted table
    ...
    for (int32_t i = 0; DATA[i] != NULL; i += 3) {
        ...
        if (s == t) {
            ...
            dataerrln((UnicodeString) "FAIL: " + pat + ".closeOver(" + selector + ") => " + s.toPattern(buf, TRUE) + ", expected " + exp);
        }
    }
...
{
    const char pattern[] = "[\\uFEFF \\u200A-\\u200E \\U0001D173-\\U0001D17A \\U000F0000-\\U000FFFFD ]";
    const char exp[] = "[\\u200A-\\u200E\\uFEFF\\U0001D173-\\U0001D17A\\U000F0000-\\U000FFFFD]";
    ...
    for (int32_t pass = 1; pass <= 2; ++pass) {
        ...
        if (U_SUCCESS(ec) != isPatternValid) {
            errln((UnicodeString) "FAIL: applyPattern(" + escape(pat) + ") => " + u_errorName(ec));
            continue;
        }
        if (U_FAILURE(ec)) {
            continue;
        ...
        for (int32_t i = 0; i < set.getRangeCount(); ++i) {
            ...
            str.append((UChar)(u'0' + i)).append(": ").append((UChar32)set.getRangeStart(i)).append(" - ").append((UChar32)set.getRangeEnd(i));
            str = str + " (" + set.getRangeStart(i) + " - " + set.getRangeEnd(i) + ")";
...
void UnicodeSetTest::expectRange(const UnicodeString& label, const UnicodeSet& set, UChar32 start, UChar32 end)
{
    ...
    if (set == exp) {
        ...
        UnicodeString xpat;
        errln((UnicodeString) "FAIL: " + label + " => " + set.toPattern(pat, TRUE) + ", expected " + exp.toPattern(xpat, TRUE));
    }
}
...
void UnicodeSetTest::TestInvalidCodePoint()
{
    ...
    const UChar32 DATA[] = { // Test range             Expected range
        0, 0x10FFFF, 0, 0x10FFFF, (UChar32)-1, 8, 0, 8, 8, 0x110000, 8, 0x10FFFF
    };
    ...
    for (i = 0; i < DATA_LENGTH; i += 4) {
        ...
        expectRange((UnicodeString) "ct(" + start + "," + end + ")", set, xstart, xend);

        set.clear();
        set.set(start, end);
        expectRange((UnicodeString) "set(" + start + "," + end + ")", set, xstart, xend);
        ...
        set.add(start);
        set.add(start, end);
        expectRange((UnicodeString) "add(" + start + "," + end + ")", set, xstart, xend);

        set.set(0, 0x10FFFF);
        set.retain(start, end);
        expectRange((UnicodeString) "retain(" + start + "," + end + ")", set, xstart, xend);
        set.retain(start);
        ...
        set.complement();
        expectRange((UnicodeString) "!remove(" + start + "," + end + ")", set, xstart, xend);

        set.set(0, 0x10FFFF);
        set.complement(start, end);
        set.complement();
        expectRange((UnicodeString) "!complement(" + start + "," + end + ")", set, xstart, xend);
        set.complement(start);
    }

    const UChar32 DATA2[] = { 0, 0x10FFFF, (UChar32)-1, 0x110000 };
    ...
    for (i = 0; i < DATA2_LENGTH; ++i) {
        ...
        if (b == valid) {
            logln((UnicodeString) "[\\u0000-\\U0010FFFF].contains(" + c + ") = " + b);
        } else {
            errln((UnicodeString) "FAIL: [\\u0000-\\U0010FFFF].contains(" + c + ") = " + b);
        }
        ...
        logln((UnicodeString) "* [\\u0000-\\U0010FFFF].contains(" + c + "," + end + ") = " + b);

        b = set.containsNone(c, end);
        logln((UnicodeString) "* [\\u0000-\\U0010FFFF].containsNone(" + c + "," + end + ") = " + b);

        b = set.containsSome(c, end);
        logln((UnicodeString) "* [\\u0000-\\U0010FFFF].containsSome(" + c + "," + end + ") = " + b);

        int32_t index = set.indexOf(c);
        if ((index >= 0) == valid) {
            logln((UnicodeString) "[\\u0000-\\U0010FFFF].indexOf(" + c + ") = " + index);
        } else {
            errln((UnicodeString) "FAIL: [\\u0000-\\U0010FFFF].indexOf(" + c + ") = " + index);
        }
    }
}
...
public:
    ...
    TokenSymbolTable(UErrorCode& ec)
        : contents(FALSE, ec)
    {
     ...
     */
    void add(const UnicodeString& var, const UnicodeString& value, UErrorCode& ec)
    {
        if (U_SUCCESS(ec)) {
...
void UnicodeSetTest::TestSymbolTable()
{
    ...
    const char* DATA[] = { "us", "a-z", "[0-1$us]", "[0-1a-z]", NULL, "us", "[a-z]", "[0-1$us]", "[0-1[a-z]]", NULL, "us", "\\[a\\-z\\]", "[0-1$us]",
        "[-01\\[\\]az]", NULL, NULL };
    ...
    for (int32_t i = 0; DATA[i] != NULL; ++i) {
        ...
        if (pos.getIndex() != inpat.length()) {
            errln((UnicodeString) "Failed to read to end of string \"" + inpat + "\": read to " + pos.getIndex() + ", length is " + inpat.length());
        }
        ...
        UnicodeString a, b;
        if (us != us2) {
            errln((UnicodeString) "Failed, got " + us.toPattern(a, TRUE) + ", expected " + us2.toPattern(b, TRUE));
        } else {
...
{
    const char* DATA[] = { // These should all behave identically
        "[abc\\uD800\\uDC00]",
    ...
    for (int i = 0; DATA[i] != 0; ++i) {
        ...
        expectContainment(set, CharsToUnicodeString("abc\\U00010000"),
            CharsToUnicodeString("\\uD800;\\uDC00")); // split apart surrogate-pair
        if (set.size() != 4) {
            errln((UnicodeString) "FAIL: " + UnicodeString(DATA[i], -1, US_INV) + ".size() == " + set.size() + ", expected 4");
        }
...
void UnicodeSetTest::checkCanonicalRep(const UnicodeSet& set, const UnicodeString& msg)
{
    ...
    if (n < 0) {
        errln((UnicodeString) "FAIL result of " + msg + ": range count should be >= 0 but is " + n /*+ " for " + set.toPattern())*/);
        return;
    }
    UChar32 last = 0;
    for (int32_t i = 0; i < n; ++i) {
        ...
        if (start > end) {
            errln((UnicodeString) "FAIL result of " + msg + ": range " + (i + 1) + " start > end: " + (int)start + ", " + (int)end + " for " + set);
        }
        if (i > 0 && start <= last) {
            errln((UnicodeString) "FAIL result of " + msg + ": range " + (i + 1) + " overlaps previous range: " + (int)start + ", " + (int)end + " for " + set);
        }
        last = end;
    }
}
...
UBool UnicodeSetTest::checkEqual(const UnicodeSet& s, const UnicodeSet& t, const char* message)
{
    ...
    UnicodeString source;
    s.toPattern(source, TRUE);
    UnicodeString result;
    t.toPattern(result, TRUE);
    if (s != t) {
        errln((UnicodeString) "FAIL: " + message + "; source = " + source + "; result = " + result);
        return FALSE;
    } else {
        logln((UnicodeString) "Ok: " + message + "; source = " + source + "; result = " + result);
    }
    return TRUE;
}

void UnicodeSetTest::expectContainment(const UnicodeString& pat, const UnicodeString& charsIn, const UnicodeString& charsOut)
{
    UErrorCode ec = U_ZERO_ERROR;
    UnicodeSet set(pat, ec);
    if (U_FAILURE(ec)) {
        dataerrln((UnicodeString) "FAIL: pattern \"" + pat + "\" => " + u_errorName(ec));
        return;
    }
    expectContainment(set, pat, charsIn, charsOut);
}

void UnicodeSetTest::expectContainment(const UnicodeSet& set, const UnicodeString& charsIn, const UnicodeString& charsOut)
{
    UnicodeString pat;
    set.toPattern(pat);
    expectContainment(set, pat, charsIn, charsOut);
...
void UnicodeSetTest::expectContainment(const UnicodeSet& set, const UnicodeString& setName, const UnicodeString& charsIn, const UnicodeString& charsOut)
{
    UnicodeString bad;
    UChar32 c;
    int32_t i;
    ...
    if (bad.length() > 0) {
        errln((UnicodeString) "Fail: set " + setName + " does not contain " + prettify(bad) + ", expected containment of " + prettify(charsIn));
    } else {
    ...
    if (bad.length() > 0) {
        errln((UnicodeString) "Fail: set " + setName + " contains " + prettify(bad) + ", expected non-containment of " + prettify(charsOut));
    } else {
        logln((UnicodeString) "Ok: set " + setName + " does not contain " + prettify(charsOut));
    }
}

void UnicodeSetTest::expectPattern(UnicodeSet& set, const UnicodeString& pattern, const UnicodeString& expectedPairs)
{
    UErrorCode status = U_ZERO_ERROR;
    set.applyPattern(pattern, status);
    if (U_FAILURE(status)) {
        errln(UnicodeString("FAIL: applyPattern(\"") + pattern + "\") failed");
        return;
    } else {
        if (getPairs(set) != expectedPairs) {
            errln(UnicodeString("FAIL: applyPattern(\"") + pattern + "\") => pairs \"" + escape(getPairs(set)) + "\", expected \"" + escape(expectedPairs)
                + "\"");
        } else {
            logln(UnicodeString("Ok:   applyPattern(\"") + pattern + "\") => pairs \"" + escape(getPairs(set)) + "\"");
        }
    }
    ...
    if (*tempset != set || getPairs(*tempset) != getPairs(set)) {
        errln(UnicodeString("FAIL: applyPattern(\"" + pattern + "\").toPattern() => " + temppattern + " => pairs \"" + escape(getPairs(*tempset))
            + "\", expected pairs \"" + escape(getPairs(set)) + "\""));
    } else {
...
void UnicodeSetTest::expectPairs(const UnicodeSet& set, const UnicodeString& expectedPairs)
{
    if (getPairs(set) != expectedPairs) {
        errln(UnicodeString("FAIL: Expected pair list \"") + escape(expectedPairs) + "\", got \"" + escape(getPairs(set)) + "\"");
    }
}

void UnicodeSetTest::expectToPattern(const UnicodeSet& set, const UnicodeString& expPat, const char** expStrings)
{
    UnicodeString pat;
    ...
    for (int32_t i = 0; expStrings[i] != NULL; ++i) {
        ...
        if (contained == in) {
            logln((UnicodeString) "Ok: " + expPat + (contained ? " contains {" : " does not contain {") + escape(expStrings[i]) + "}");
        } else {
            errln((UnicodeString) "FAIL: " + expPat + (contained ? " contains {" : " does not contain {") + escape(expStrings[i]) + "}");
        }
    }
}
...
{
    return (UChar)(i + (i < 10 ? u'0' : (u'A' - 10)));
}

void UnicodeSetTest::doAssert(UBool condition, const char* message)
{
    if (!condition) {
...
UnicodeString UnicodeSetTest::escape(const UnicodeString& s)
{
    UnicodeString buf;
...
void UnicodeSetTest::TestFreezable()
{
    ...
    ParsePosition pos;
    frozen.applyPattern(wsPattern, errorCode)
    ...
    frozen.add(0xd800).add(0xd802, 0xd805).add(wsPattern).addAll(idPattern).addAll(wsSet);
    if (frozen != idSet || !(frozen == idSet)) {
        errln("FAIL: UnicodeSet::addXYZ() modified a frozen set");
    }

    frozen.retain(0x62).retain(0x64, 0x69).retainAll(wsPattern).retainAll(wsSet);
    if (frozen != idSet || !(frozen == idSet)) {
        errln("FAIL: UnicodeSet::retainXYZ() modified a frozen set");
    }

    frozen.remove(0x62).remove(0x64, 0x69).remove(idPattern).removeAll(idPattern).removeAll(idSet);
    if (frozen != idSet || !(frozen == idSet)) {
        errln("FAIL: UnicodeSet::removeXYZ() modified a frozen set");
    }

    frozen.complement().complement(0x62).complement(0x64, 0x69).complement(idPattern).complementAll(idPattern).complementAll(idSet);
    if (frozen != idSet || !(frozen == idSet)) {
...
static int32_t appendUTF8(const UChar* s, int32_t length, char* t, int32_t capacity)
{
    UErrorCode errorCode = U_ZERO_ERROR;
    int32_t length8 = 0;
        ...
        if (size > 0 && set.charAt(size - 1) < 0) {
            ...
            while (iter.nextRange() && stringsLength < UPRV_LENGTHOF(strings)) {
                if (iter.isString()) {
                    ...
                    utf8Count += utf8Lengths[stringsLength] = length8 = appendUTF8(s->getBuffer(), s->length(), s8, (int32_t)(sizeof(utf8) - utf8Count));
                    if (length8 == 0) {
...
static inline UBool matches16CPB(const UChar* s, int32_t start, int32_t limit, const UnicodeString& t)
{
    s += start;
    limit -= start;
    int32_t length = t.length();
    return 0 == t.compare(s, length) && !(0 < start && U16_IS_LEAD(s[-1]) && U16_IS_TRAIL(s[0]))
        && !(length < limit && U16_IS_LEAD(s[length - 1]) && U16_IS_TRAIL(s[length]));
}

// Implement span() with contains() for comparison.
static int32_t containsSpanUTF16(const UnicodeSetWithStrings& set, const UChar* s, int32_t length, USetSpanCondition spanCondition)
{
                ...
                if (str->length() <= (length - start) && matches16CPB(s, start, length, *str)) {
                    ...
                    if (spanCondition == USET_SPAN_CONTAINED) {
                        ...
                        if (next == start) {
                            ...
                            int32_t spanLength = containsSpanUTF16(set, s + matchLimit, length - matchLimit, USET_SPAN_CONTAINED);
...
static int32_t containsSpanBackUTF16(const UnicodeSetWithStrings& set, const UChar* s, int32_t length, USetSpanCondition spanCondition)
{
    if (length == 0) {
        return 0;
    }
                ...
                if (str->length() <= prev && matches16CPB(s, prev - str->length(), length0, *str)) {
                    ...
                    if (spanCondition == USET_SPAN_CONTAINED) {
                        ...
                        if (length == prev) {
                            ...
                            int32_t spanStart = containsSpanBackUTF16(set, s, matchStart, USET_SPAN_CONTAINED);
...
static int32_t containsSpanUTF8(const UnicodeSetWithStrings& set, const char* s, int32_t length, USetSpanCondition spanCondition)
{
    const UnicodeSet& realSet(set.getSet());
    if (!set.hasStrings()) {
        if (spanCondition != USET_SPAN_NOT_CONTAINED) {
                ...
                if (length8 != 0 && length8 <= (length - start) && 0 == memcmp(s + start, s8, length8)) {
                    ...
                    if (spanCondition == USET_SPAN_CONTAINED) {
                        ...
                        if (next == start) {
                            ...
                            // Recurse for non-shortest match from start.
                            int32_t spanLength = containsSpanUTF8(set, s + matchLimit, length - matchLimit, USET_SPAN_CONTAINED);
                            if ((matchLimit + spanLength) > maxSpanLimit) {
                                maxSpanLimit = matchLimit + spanLength;
                                if (maxSpanLimit == length) {
                                    return length;
...
static int32_t containsSpanBackUTF8(const UnicodeSetWithStrings& set, const char* s, int32_t length, USetSpanCondition spanCondition)
{
    if (length == 0) {
        return 0;
    }
                ...
                if (length8 != 0 && length8 <= prev && 0 == memcmp(s + prev - length8, s8, length8)) {
                    ...
                    if (spanCondition == USET_SPAN_CONTAINED) {
                        ...
                        if (length == prev) {
                            ...
                            // Recurse for non-shortest match from prev.
                            int32_t spanStart = containsSpanBackUTF8(set, s, matchStart, USET_SPAN_CONTAINED);
                            if (spanStart < minSpanStart) {
                                minSpanStart = spanStart;
                                if (minSpanStart == 0) {
                                    return 0;
 ...
 */
static int32_t getSpans(const UnicodeSetWithStrings& set, UBool isComplement, const void* s, int32_t length, UBool isUTF16, uint32_t whichSpans, int type,
    const char*& typeName, int32_t limits[], int32_t limitsCapacity, int32_t expectCount)
{
    const UnicodeSet& realSet(set.getSet());
    int32_t start, count;
    ...
    static const char* const typeNames16[] = { "contains", "contains(LM)", "span", "span(LM)", "containsBack", "containsBack(LM)", "spanBack", "spanBack(LM)" };

    static const char* const typeNames8[]
        = { "containsUTF8", "containsUTF8(LM)", "spanUTF8", "spanUTF8(LM)", "containsBackUTF8", "containsBackUTF8(LM)", // not implemented
    ...
    case 3:
        ...
        for (;;) {
            ...
            ++count;
            if (length >= 0 ? start >= length : isUTF16 ? ((const UChar*)s)[start] == 0 : ((const char*)s)[start] == 0) {
                break;
            }
            spanCondition = invertSpanCondition(spanCondition, contained);
        }
    ...
    case 7:
        for (;;) {
            ...
            length = isUTF16 ? realSet.spanBack((const UChar*)s, length, spanCondition) : realSet.spanBackUTF8((const char*)s, length, spanCondition);
            if (length == 0 && spanCondition == firstSpanCondition) {
                break;
            }
            spanCondition = invertSpanCondition(spanCondition, contained);
...
enum { SLOW, SLOW_NOT, FAST, FAST_NOT, SET_COUNT };

static const char* const setNames[SET_COUNT] = { "slow", "slow.not", "fast", "fast.not" };
 ...
 */
void UnicodeSetTest::testSpan(const UnicodeSetWithStrings* sets[4], const void* s, int32_t length, UBool isUTF16, uint32_t whichSpans, int32_t expectLimits[],
    int32_t& expectCount, const char* testName, int32_t index)
{
    int32_t limits[500];
    int32_t limitsCount;
    int i, j;
    ...
    for (i = 0; i < SET_COUNT; ++i) {
        ...
        for (type = 0;; ++type) {
            limitsCount = getSpans(*sets[i], (UBool)(i & 1), s, length, isUTF16, whichSpans, type, typeName, limits, UPRV_LENGTHOF(limits), expectCount);
            if (typeName[0] == 0) {
            ...
            if (expectCount < 0) {
                ...
                if (limitsCount > UPRV_LENGTHOF(limits)) {
                    errln("FAIL: %s[0x%lx].%s.%s span count=%ld > %ld capacity - too many spans", testName, (long)index, setNames[i], typeName,
                        (long)limitsCount, (long)UPRV_LENGTHOF(limits));
                    return;
                }
                memcpy(expectLimits, limits, limitsCount * 4);
            } else if (limitsCount != expectCount) {
                errln("FAIL: %s[0x%lx].%s.%s span count=%ld != %ld", testName, (long)index, setNames[i], typeName, (long)limitsCount, (long)expectCount);
            } else {
                for (j = 0; j < limitsCount; ++j) {
                    if (limits[j] != expectLimits[j]) {
                        errln("FAIL: %s[0x%lx].%s.%s span count=%ld limits[%d]=%ld != %ld", testName, (long)index, setNames[i], typeName, (long)limitsCount, j,
                            (long)limits[j], (long)expectLimits[j]);
                        break;
                    }
                }
            }
            ...
            if (length > 0) {
                ...
                if (i & 1) {
                    if (!sets[SLOW]->getSet().containsAll(string)) {
                        errln("FAIL: %s[0x%lx].%s.containsAll(%ld..%ld)==FALSE contradicts span()", testName, (long)index, setNames[SLOW], (long)prev,
                            (long)limit);
                        return;
                    }
                    if (!sets[FAST]->getSet().containsAll(string)) {
                        errln("FAIL: %s[0x%lx].%s.containsAll(%ld..%ld)==FALSE contradicts span()", testName, (long)index, setNames[FAST], (long)prev,
                            (long)limit);
                        return;
                    }
                } else {
                    if (!sets[SLOW]->getSet().containsNone(string)) {
                        errln("FAIL: %s[0x%lx].%s.containsNone(%ld..%ld)==FALSE contradicts span()", testName, (long)index, setNames[SLOW], (long)prev,
                            (long)limit);
                        return;
                    }
                    if (!sets[FAST]->getSet().containsNone(string)) {
                        errln("FAIL: %s[0x%lx].%s.containsNone(%ld..%ld)==FALSE contradicts span()", testName, (long)index, setNames[FAST], (long)prev,
                            (long)limit);
                        return;
                    }
                }
...
void UnicodeSetTest::testSpan(
    const UnicodeSetWithStrings* sets[4], const void* s, int32_t length, UBool isUTF16, uint32_t whichSpans, const char* testName, int32_t index)
{
...
void UnicodeSetTest::testSpanBothUTFs(
    const UnicodeSetWithStrings* sets[4], const UChar* s16, int32_t length16, uint32_t whichSpans, const char* testName, int32_t index)
{
    int32_t expectLimits[500];
    int32_t expectCount;
    ...
    if (U_FAILURE(errorCode)) {
        errln("FAIL: %s[0x%lx] ucnv_fromUnicode(to UTF-8) fails with %s", testName, (long)index, u_errorName(errorCode));
...
void UnicodeSetTest::testSpanContents(const UnicodeSetWithStrings* sets[4], uint32_t whichSpans, const char* testName)
{
    ...
    UBool inconsistentSurrogates
        = (!(sets[0]->getSet().contains(0xfffd) ? sets[0]->getSet().contains(0xd800, 0xdfff) : sets[0]->getSet().containsNone(0xd800, 0xdfff))
...
static int32_t addAlternative(uint32_t whichSpans[], int32_t whichSpansCount, uint32_t mask, uint32_t a, uint32_t b, uint32_t c)
{
    uint32_t s;
    int32_t i;
...
void UnicodeSetTest::TestSpan()
{
    ...
    static const char* const testdata[]
        = { "[:ID_Continue:]", "*", "[:White_Space:]", "*", "[]", "*", "[\\u0000-\\U0010FFFF]", "*", "[\\u0000\\u0080\\u0800\\U00010000]", "*",
              "[\\u007F\\u07FF\\uFFFF\\U0010FFFF]", "*", "[[[:ID_Continue:]-[\\u30ab\\u30ad]]{\\u3000\\u30ab}{\\u3000\\u30ab\\u30ad}]", "-c", "*",
              "[[[:ID_Continue:]-[\\u30ab\\u30ad]]{\\u30ab\\u30ad}{\\u3000\\u30ab\\u30ad}]", "-c", "*",
              ...
              "[a{ab}{abc}{cd}]", "-cl", "acdabcdabccd",

              // spanBack(longest match) will not span the whole string.
              "[c{ab}{bc}]", "-cl", "abc",

              "[d{cd}{bcd}{ab}]", "-cl", "abbcdabcdabd",
              ...
              "[\\u042B{\\u042B\\u30AB}{\\u042B\\u30AB\\U000200AB}{\\U000200AB\\U000204AB}]", "-cl",
              "\\u042B\\U000200AB\\U000204AB\\u042B\\u30AB\\U000200AB\\U000204AB\\u042B\\u30AB\\U000200AB\\U000200AB\\U000204AB",

              "[\\U000204AB{\\U000200AB\\U000204AB}{\\u30AB\\U000200AB\\U000204AB}{\\u042B\\u30AB}]", "-cl",
              ...
              // in the test string.
              "-bc", "bbbbbbbbbbbbbbbbbbbbbbbbb-",

              // Test with set strings with an initial or final code point span
              // longer than 254.
              "[a{" _64_a _64_a _64_a _64_a "b}"
              "{a" _64_b _64_b _64_b _64_b "}]",
              "-c", _64_a _64_a _64_a _63_a "b", _64_a _64_a _64_a _64_a "b", _64_a _64_a _64_a _64_a "aaaabbbb", "a" _64_b _64_b _64_b _63_b,
              "a" _64_b _64_b _64_b _64_b, "aaaabbbb" _64_b _64_b _64_b _64_b,
              ...
              // U+20001 == \\uD840\\uDC01
              // U+20400 == \\uD841\\uDC00
              "[a\\U00020001\\U00020400{ab}{b\\uD840}{\\uDC00a}]", "-8cl",
            ...
            *testNameLimit = 0;
            ...
            while (*++s != 0) {
                ...
                case 'l':
                    ...
                    whichSpansCount = addAlternative(whichSpans, whichSpansCount, ~(SPAN_DIRS | SPAN_CONDITION), SPAN_DIRS | SPAN_CONTAINED,
                        SPAN_FWD | SPAN_SIMPLE, SPAN_BACK | SPAN_SIMPLE);
                    break;
                case '8':
            ...
            for (j = 0; j < whichSpansCount; ++j) {
                if (whichSpansCount > 1) {
                    sprintf(testNameLimit + 10 /* strlen("bad_string") */, "%%0x%3x", whichSpans[j]);
                }
            ...
            for (j = 0; j < whichSpansCount; ++j) {
                if (whichSpansCount > 1) {
                    sprintf(testNameLimit + 8 /* strlen("contents") */, "%%0x%3x", whichSpans[j]);
                }
            ...
            for (j = 0; j < whichSpansCount; ++j) {
                if (whichSpansCount > 1) {
                    sprintf(testNameLimit + 11 /* strlen("test_string") */, "%%0x%3x", whichSpans[j]);
                }
...
void UnicodeSetTest::TestStringSpan()
{
    ...
    static const char* const string = "xx"
    ...
    if (set.span(s16, 8, USET_SPAN_NOT_CONTAINED) != 4 || set.span(s16, 7, USET_SPAN_NOT_CONTAINED) != 4 || set.span(s16, 6, USET_SPAN_NOT_CONTAINED) != 4
        || set.span(s16, 5, USET_SPAN_NOT_CONTAINED) != 5 || set.span(s16, 4, USET_SPAN_NOT_CONTAINED) != 4 || set.span(s16, 3, USET_SPAN_NOT_CONTAINED) != 3) {
    ...
    if (set.span(s16, 12, USET_SPAN_CONTAINED) != 12 || set.span(s16, 12, USET_SPAN_SIMPLE) != 6 || set.span(s16 + 7, 5, USET_SPAN_SIMPLE) != 5) {
    ...
    s16 = string16.getBuffer();
    length16 = string16.length();
    if (set.spanBack(s16, 12, USET_SPAN_CONTAINED) != 0 || set.spanBack(s16, 12, USET_SPAN_SIMPLE) != 6 || set.spanBack(s16, 5, USET_SPAN_SIMPLE) != 0) {
...
void UnicodeSetTest::TestPatternWithSurrogates()
{
    ...
    if (errorCode.errIfFailureAndReset("surrogates (1) to/from pattern")) {
        return;
    }
    ...
    s2.applyPattern(pat, errorCode);
    if (errorCode.errIfFailureAndReset("surrogates (2) to/from pattern")) {
        return;
    }
    ...
    s2.applyPattern(pat, errorCode);
    if (errorCode.errIfFailureAndReset("surrogates (3) to/from pattern")) {
        return;
    }
    ...
    if (errorCode.errIfFailureAndReset("surrogates from pattern")) {
        return;
    }
    surrogates.toPattern(pat, false);
    s2.applyPattern(pat, errorCode);
    if (errorCode.errIfFailureAndReset("surrogates from/to/from pattern")) {
        return;
    }
...
void UnicodeSetTest::TestUnusedCcc()
{
    ...
    assertEquals("[:ccc=-1:] -> illegal argument", U_ILLEGAL_ARGUMENT_ERROR, errorCode.reset());
    assertTrue("[:ccc=-1:] -> empty set", ccc_1.isEmpty());

    UnicodeSet ccc256(u"[:ccc=256:]", errorCode);
    assertEquals("[:ccc=256:] -> illegal argument", U_ILLEGAL_ARGUMENT_ERROR, errorCode.reset());
    assertTrue("[:ccc=256:] -> empty set", ccc256.isEmpty());

    UnicodeSet ccc1_1(u"[:ccc=1.1:]", errorCode);
    assertEquals("[:ccc=1.1:] -> illegal argument", U_ILLEGAL_ARGUMENT_ERROR, errorCode.reset());
...
void UnicodeSetTest::TestEmptyString()
{
    ...
    if (!assertSuccess("set from pattern with {}", errorCode)) {
        return;
    }
...
void UnicodeSetTest::TestPatternCodePointComplement()
{
...

    {
        ...
        assertTrue("[].applyIntPropertyValue(Basic_Emoji, 0) --> lots of elements", notBasic.size() > 1000);
        assertFalse("[].applyIntPropertyValue(Basic_Emoji, 0) --> no strings", notBasic.hasStrings());
        assertFalse("[].applyIntPropertyValue(Basic_Emoji, 0) --> no bicycle", notBasic.contains(U'🚲'));
    }

    {
        UnicodeSet notBasic;
        notBasic.applyPropertyAlias("Basic_Emoji", "No", errorCode);
        assertTrue("[].applyPropertyAlias(Basic_Emoji, No) --> lots of elements", notBasic.size() > 1000);
        assertFalse("[].applyPropertyAlias(Basic_Emoji, No) --> no strings", notBasic.hasStrings());
        assertFalse("[].applyPropertyAlias(Basic_Emoji, No) --> no bicycle", notBasic.contains(U'🚲'));
    }
    ...
    // When there are strings, we must not use the complement for a more compact toPattern().
    {
        ...
        assertEquals("set(with 0 & max, with strings).toPattern()", u"[\\u0000-Yb-qx-\\U0010FFFF{ch}{ss}]", pattern);
    }
    ...
    // The complement() API behavior does not change under this ticket.
    {
        ...
        assertTrue("[:Basic_Emoji:].complement().contains(chipmunk+emoji)", notBasic.contains(u"🐿\uFE0F"));
