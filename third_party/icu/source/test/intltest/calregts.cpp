...
void CalendarRegressionTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
...
const char* CalendarRegressionTest::FIELD_NAME[] = { "ERA", "YEAR", "MONTH", "WEEK_OF_YEAR", "WEEK_OF_MONTH", "DAY_OF_MONTH", "DAY_OF_YEAR", "DAY_OF_WEEK",
    "DAY_OF_WEEK_IN_MONTH", "AM_PM", "HOUR", "HOUR_OF_DAY", "MINUTE", "SECOND", "MILLISECOND", "ZONE_OFFSET", "DST_OFFSET", "YEAR_WOY", "DOW_LOCAL" };

UBool CalendarRegressionTest::failure(UErrorCode status, const char* msg)
{
    if (U_FAILURE(status)) {
    ...
    for (int32_t i = 0; i < ids->count(status); ++i) {
        ...
        if (cal->get(UCAL_HOUR, status) != 5 || U_FAILURE(status)) {
            ...
            logln(zone->getID(temp) + " " +
                ...
                cal->get(UCAL_DST_OFFSET, status) / (60 * 60 * 1000) + " " + zone->getRawOffset() / (60 * 60 * 1000)
...
void CalendarRegressionTest::test4035301()
{
    ...
    if (c->after(*d, status) || c->after(*c, status) || c->before(*d, status) || c->before(*c, status) || *c != *c || *c != *d)
        dataerrln("Fail");
    delete c;
    delete d;
}
...
void CalendarRegressionTest::test4040996()
{
    ...
    logln(UnicodeString("DAY_OF_MONTH: ") + calendar->get(UCAL_DATE, status));
    ...
    logln(UnicodeString("MINUTE: ") + calendar->get(UCAL_MINUTE, status));
    logln(UnicodeString("SECOND: ") + calendar->get(UCAL_SECOND, status));
    if (calendar->get(UCAL_MONTH, status) != 3 || calendar->get(UCAL_DATE, status) != 18 || calendar->get(UCAL_SECOND, status) != 36)
...
void CalendarRegressionTest::test4061476()
{
    ...
    Calendar* cal = Calendar::createInstance(TimeZone::createTimeZone("GMT"), Locale::getUK(), status);
    if (U_FAILURE(status)) {
        dataerrln("Error creating Calendar: %s", u_errorName(status));
        delete cal;
        delete fmt;
...
void CalendarRegressionTest::test4070502()
{
    ...
    if (cal->get(UCAL_DAY_OF_WEEK, status) == UCAL_SATURDAY || cal->get(UCAL_DAY_OF_WEEK, status) == UCAL_SUNDAY)
...
void CalendarRegressionTest::test4073929()
{
    ...
    logln("foo1@%.0f - %d-%d-%d %d:%d:%d.%ds\n", foo1->getTime(status), foo1->get(UCAL_YEAR, status), foo1->get(UCAL_MONTH, status),
        foo1->get(UCAL_DATE, status), foo1->get(UCAL_HOUR, status), foo1->get(UCAL_MINUTE, status), foo1->get(UCAL_SECOND, status),
...
void CalendarRegressionTest::test4096231()
{
    ...
    cal2->set(cal1->get(UCAL_YEAR, status), cal1->get(UCAL_MONTH, status), cal1->get(UCAL_DATE, status), cal1->get(UCAL_HOUR_OF_DAY, status),
        cal1->get(UCAL_MINUTE, status), cal1->get(UCAL_SECOND, status));
    ...
    cal3->set(cal1->get(UCAL_YEAR, status), cal1->get(UCAL_MONTH, status), cal1->get(UCAL_DATE, status), cal1->get(UCAL_HOUR_OF_DAY, status),
        cal1->get(UCAL_MINUTE, status), cal1->get(UCAL_SECOND, status));
...
void CalendarRegressionTest::test4096539()
{
    ...
    for (int32_t x = 0; x < 12; x++) {
        GregorianCalendar* gc = new GregorianCalendar(1997, x, y[x], status);
        if (U_FAILURE(status)) {
            dataerrln("Fail new GregorianCalendar: %s", u_errorName(status));
            delete gc;
            return;
        ...
        int32_t m1, m2;
        log(UnicodeString("") + (m1 = gc->get(UCAL_MONTH, status) + 1) + UnicodeString("/") + gc->get(UCAL_DATE, status) + "/" + gc->get(UCAL_YEAR, status)
            + " + 1mo = ");

        gc->add(UCAL_MONTH, 1, status);
        logln(UnicodeString("") + (m2 = gc->get(UCAL_MONTH, status) + 1) + UnicodeString("/") + gc->get(UCAL_DATE, status) + "/" + gc->get(UCAL_YEAR, status));
        int32_t m = (m1 % 12) + 1;
        if (m2 != m)
...
void CalendarRegressionTest::test4103271()
{
    ...
    for (int32_t firstDay = 1; firstDay <= 2; firstDay++) {
        for (int32_t minDays = 1; minDays <= 7; minDays++) {
            ...
            logln(testDesc + " => 1st day of week=" + firstDay + ", minimum days in first week=" + minDays);
    ...
    for (int32_t j = 0; j < 44; j += 22) {
        logln(UnicodeString("Minimal days in first week = ") + DATA[j] + "  Week starts on Sunday");
        ...
        for (int32_t i = 0; i < 21; ++i) {
            ...
            str.remove();
            log(UnicodeString("") + sdf.format(testCal->getTime(status), str) + UnicodeString(" ") + woy);
            if (woy != DATA[j + 1 + i]) {
                log(" ERROR");
                fail = TRUE;
            }
            ...
            if (testCal->getTime(status) != save) {
                str.remove();
                logln(UnicodeString("  Parse failed: ") + sdf.format(testCal->getTime(status), str));
                fail = TRUE;
            }
    ...
    int32_t DISAM_int[] = { // y y_woy woy dow
        1997, 1998, 1, UCAL_SUNDAY, (1998), (1998), (2), (UCAL_SATURDAY), (1998), (1998), (53), (UCAL_THURSDAY), (1999), (1998), (53), (UCAL_FRIDAY)
    };

    UDate DISAM_date[]
        = { makeDate(1997, UCAL_DECEMBER, 28), makeDate(1998, UCAL_JANUARY, 10), makeDate(1998, UCAL_DECEMBER, 31), makeDate(1999, UCAL_JANUARY, 1) };
    ...
    */
    ...
    for (i = 0; i < 16; i += 4) {
        ...
        str.remove();
        str2.remove();
        log(UnicodeString("Y") + y + "-W" + woy + "-DOW" + dow + " expect:" + sdf.format(exp, str) + " got:" + sdf.format(got, str2));
        if (got != exp) {
        ...
        str.remove();
        str2.remove();
        log(UnicodeString("YWOY") + ywoy + "-W" + woy + "-DOW" + dow + " expect:" + sdf.format(exp, str) + " got:" + sdf.format(got, str2));
        if (got != exp) {
            log("  FAIL");
            fail = TRUE;
        }
    ...
    UDate ADDROLL_date[] = {
        makeDate(1998, UCAL_DECEMBER, 25),
        makeDate(1999, UCAL_JANUARY, 1),
        makeDate(1997, UCAL_DECEMBER, 28),
        makeDate(1998, UCAL_JANUARY, 4),
        makeDate(1998, UCAL_DECEMBER, 27),
        makeDate(1997, UCAL_DECEMBER, 28),
        makeDate(1999, UCAL_JANUARY, 2),
        makeDate(1998, UCAL_JANUARY, 3),
    };

    int32_t ADDROLL_int[] = { (1), (1), (1), (1) };

    UBool ADDROLL_bool[] = { TRUE, // ADD,
        TRUE, FALSE, FALSE };
    ...
    for (i = 0; i < 8; i += 2) {
        ...
        log(opTypeStr + amount + ") " + sdf.format(before, str) + " => " + sdf.format(got, str2));
        if (after != got) {
            str.remove();
        ...
        str.remove();
        str2.remove();
        log(opTypeStr + (-amount) + ") " + sdf.format(after, str) + " => " + sdf.format(got, str2));
        if (before != got) {
            str.remove();
...
void CalendarRegressionTest::test4106136()
{
    ...
    for (int32_t i = 0; i < 2; ++i) {
        ...
        int32_t n[] = { count1, count2, count3 };
...
void CalendarRegressionTest::test4118384()
{
    ...
    if (cal->getMaximum(UCAL_HOUR) != 11 || cal->getLeastMaximum(UCAL_HOUR) != 11 || cal->getActualMaximum(UCAL_HOUR, status) != 11)
        errln("Fail: maximum of HOUR field should be 11");

    // test deprecated functions
    if (cal->getLeastMaximum(Calendar::HOUR) != 11 || cal->getMaximum(Calendar::HOUR) != 11) {
        errln("Fail: [deprecated functions] maximum of HOUR field should be 11\n");
    }

    if (cal->getGreatestMinimum(Calendar::HOUR) != 0 || cal->getMinimum(Calendar::HOUR) != 0) {
...
void CalendarRegressionTest::test4125881()
{
    ...
    for (int32_t y = -20; y <= 10; ++y) {
        ...
        UnicodeString temp;
        logln(UnicodeString("") + y + UnicodeString(" = ") + fmt.format(cal->getTime(status), temp) + " " + cal->isLeapYear(y));
...
void CalendarRegressionTest::test4125892()
{
    ...
    if (cal->get(UCAL_DATE, status) != 29 || !cal->isLeapYear(-80)) // -80 == 81 BC
...
void CalendarRegressionTest::test4145983()
{
    ...
    for (int32_t i = 0; i < 2; ++i) {
        ...
        if (year > maxYear) {
            errln(UnicodeString("Failed for ") + DATES[i] + " ms: year=" + year + ", maxYear=" + maxYear);
        }
    }
...
void CalendarRegressionTest::test4147269()
{
    ...
    for (int32_t field = 0; field < UCAL_FIELD_COUNT; field++) {
        ...
        if (U_SUCCESS(status))
            errln(UnicodeString("Test failed with field ") + FIELD_NAME[field] + ", date before: " + date + ", date after: " + calendar->getTime(status)
                + ", value: " + value + " (max = " + max + ")");
        //} catch (IllegalArgumentException e) {}
    }
...
void CalendarRegressionTest::Test4149677()
{
    ...
    TimeZone* zones[] = { TimeZone::createTimeZone("GMT"), TimeZone::createTimeZone("PST"), TimeZone::createTimeZone("EAT") };
    if (U_FAILURE(status)) {
...
void CalendarRegressionTest::Test4166109()
{
    ...
    for (int32_t firstInWeek = UCAL_SUNDAY; firstInWeek <= UCAL_SATURDAY; firstInWeek++) {
        ...
        logln(UnicodeString("First day of week = ") + firstInWeek + "  getActualMaximum(WEEK_OF_MONTH, status) = " + returned + "  expected = " + expected
...
void CalendarRegressionTest::Test4167060()
{
    ...
    LocalPointer<DateFormat> format(new SimpleDateFormat(UnicodeString("EEE MMM dd HH:mm:ss zzz yyyy G"), Locale::getUS(), status));
    if (U_FAILURE(status)) {
        dataerrln("Couldn't create SimpleDateFormat - %s", u_errorName(status));
        return;
    }

    GregorianCalendar calendars[] = { { 100, UCAL_NOVEMBER, 1, status }, { -99 /*100BC*/, UCAL_JANUARY, 1, status }, { 1996, UCAL_FEBRUARY, 29, status } };
    if (U_FAILURE(status)) {
    ...
    for (int32_t k = 0; k < 3; ++k) {
        ...
        for (int32_t j = 0; j < 3; ++j) {
            ...
            for (int32_t i = 0; i < 4; i++) {
                ...
                temp.remove();
                logln(UnicodeString(" Year ") + years[i] + (valid ? " ok " : " bad") + " => " + format->format(dateAfter, temp));
...
void CalendarRegressionTest::Test4197699()
{
    ...
    SimpleDateFormat fmt("E dd MMM yyyy  'DOY='D 'WOY='w", Locale::getUS(), status);
    fmt.setCalendar(cal);
    if (U_FAILURE(status)) {
    ...
    for (int32_t i = 0; i < DATA_length;) {
        ...
        if (expWOY == actWOY) {
            ...
            errln(UnicodeString("FAIL: ") + dfmt.format(cal.getTime(status), str.remove()) + ", expected WOY=" + expWOY);
...
void CalendarRegressionTest::TestJ81()
{
    ...
    UDate days = ONE_DAY;
    days = cutover / days;
    logln(UnicodeString("Cutover: {") + fmt.format(cutover, temp) + "}(epoch days-" + (int)days + ", jd" + (2440588 + days) + ")");
    ...
    int32_t DOY_DATA[] = { // dom, woy, doy
        1, 40, 274, UCAL_MONDAY, 4, 40, 277, UCAL_THURSDAY, 15, 40, 278, UCAL_FRIDAY, 17, 41, 280, UCAL_SUNDAY, 24, 42, 287, UCAL_SUNDAY, 25, 42, 288,
        UCAL_MONDAY, 26, 42, 289, UCAL_TUESDAY, 27, 42, 290, UCAL_WEDNESDAY, 28, 42, 291, UCAL_THURSDAY, 29, 42, 292, UCAL_FRIDAY, 30, 42, 293, UCAL_SATURDAY,
    ...
    for (i = 0; i < DOY_DATA_length; i += 4) {
        ...
        if (woy != DOY_DATA[i + 1] || doy != DOY_DATA[i + 2] || dow != DOY_DATA[i + 3]) {
            errln((UnicodeString) "Fail: expect woy=" + DOY_DATA[i + 1] + ", doy=" + DOY_DATA[i + 2] + ", dow=" + DOY_DATA[i + 3] + " on "
                + fmt.format(cal.getTime(status), temp.remove()) + " set(1582,OCTOBER, " + DOY_DATA[i] + ")");
            logln(CalendarTest::calToStr(cal));
            status = U_ZERO_ERROR;
        } else {
            logln((UnicodeString) "PASS: expect woy=" + DOY_DATA[i + 1] + ", doy=" + DOY_DATA[i + 2] + ", dow=" + DOY_DATA[i + 3] + " on "
        ...
        if (dom != DOY_DATA[i]) {
            errln((UnicodeString) "Fail: set woy=" + DOY_DATA[i + 1] + " dow=" + DOY_DATA[i + 3] + " => " + fmt.format(cal.getTime(status), temp.remove())
        ...
        if (dom != DOY_DATA[i]) {
            errln((UnicodeString) "Fail: set doy=" + DOY_DATA[i + 2] + " => " + fmt.format(cal.getTime(status), temp.remove()) + ", expected 1582 Oct "
                + DOY_DATA[i]);
            status = U_ZERO_ERROR;
        }
    }
...
#define PLUS_MINUS PLUS | MINUS
    // Test cases
    J81_DATA DATA[] = {
        { UCAL_WEEK_OF_YEAR, ADD_ROLL, PLUS_MINUS, 1, -ONE_DAY, +6 * ONE_DAY }, { UCAL_WEEK_OF_YEAR, ADD_ROLL, PLUS_MINUS, 1, -ONE_DAY, +6 * ONE_DAY },
        { UCAL_WEEK_OF_MONTH, ADD | ROLL, PLUS | MINUS, 1, -ONE_DAY, +6 * ONE_DAY }, { UCAL_DATE, ADD | ROLL, PLUS | MINUS, 2, -ONE_DAY, +1 * ONE_DAY },
        { UCAL_DATE, ROLL, PLUS, -6, -ONE_DAY, +14 * ONE_DAY }, { UCAL_DATE, ROLL, PLUS, -7, 0, +14 * ONE_DAY },
        { UCAL_DATE, ROLL, PLUS, -7, +ONE_DAY, +15 * ONE_DAY }, { UCAL_DATE, ROLL, PLUS, +18, -ONE_DAY, -4 * ONE_DAY },
        { UCAL_DAY_OF_YEAR, ADD | ROLL, PLUS | MINUS, 2, -ONE_DAY, +1 * ONE_DAY }, { UCAL_DAY_OF_WEEK, ADD | ROLL, PLUS | MINUS, 2, -ONE_DAY, +1 * ONE_DAY },
        { UCAL_DAY_OF_WEEK_IN_MONTH, ADD | ROLL, PLUS | MINUS, 1, -ONE_DAY, +6 * ONE_DAY },
        { UCAL_AM_PM, ADD, PLUS | MINUS, 4, -12 * ONE_HOUR, +36 * ONE_HOUR }, { UCAL_HOUR, ADD, PLUS | MINUS, 48, -12 * ONE_HOUR, +36 * ONE_HOUR },
    ...
    for (i = 0; i < DATA_length; ++i) {
        for (Action action = ADD; action <= ROLL; action = (Action)(action + 1)) {
            ...
            for (Sign sign = PLUS; sign <= MINUS; sign = (Sign)(sign + 1)) {
                ...
                if (U_FAILURE(status)) {
                    errln((UnicodeString) "FAIL: " + (action == ADD ? "add " : "roll ") + FIELD_NAME[DATA[i].field] + " returned error code "
                        + u_errorName(status));
                    continue;
                }
                ...
                if (result == expected) {
                    logln((UnicodeString) "Ok: {" + fmt.format(date, temp.remove()) + "}(" + date / ONE_DAY + (action == ADD ? ") add " : ") roll ") + amount
                        + " " + FIELD_NAME[DATA[i].field] + " -> {" + fmt.format(result, temp2.remove()) + "}(" + result / ONE_DAY + ")");
                } else {
                    errln((UnicodeString) "FAIL: {" + fmt.format(date, temp.remove()) + "}(" + date / ONE_DAY + (action == ADD ? ") add " : ") roll ") + amount
                        + " " + FIELD_NAME[DATA[i].field] + " -> {" + fmt.format(result, temp2.remove()) + "}(" + result / ONE_DAY + "), expect {"
                        + fmt.format(expected, temp3.remove()) + "}(" + expected / ONE_DAY + ")");
                }
            }
        }
    }
        ...
        for (i = 0; i < DATA_length; i += 6) {
...

            {
                ...
                if (failure(ec2, "fieldDifference(date, Calendar::DATE, ec)"))
                    break;
                if ((dd2 != dd) || (dm2 != dm) || (dy2 != dy)) {
                    errln("fieldDifference(UCAL_...) and fieldDifference(Calendar::...) give different results!\n");
                }
            }

            logln(UnicodeString("") + fmt.format(date2, s.remove()) + " - " + fmt.format(date1, t.remove()) + " = " + dy + "y " + dm + "m " + dd + "d");
            ...
            if (failure(ec, "getTime"))
                break;
            if (date2 != date22) {
                errln(UnicodeString("FAIL: ") + fmt.format(date1, s.remove()) + " + " + dy + "y " + dm + "m " + dd + "d = " + fmt.format(date22, t.remove())
                    + ", exp " + fmt.format(date2, u.remove()));
            } else {
                logln(UnicodeString("Ok: ") + fmt.format(date1, s.remove()) + " + " + dy + "y " + dm + "m " + dd + "d = " + fmt.format(date22, t.remove()));
            }
        }
    } else {
...
static const CoptEthCalTestItem coptEthCalTestItems[] = {
    { 1724, 12, 1, UCAL_MONTH, +1, 1725, 0, 1 }, { 1724, 12, 1, UCAL_MONTH, +9, 1725, 8, 1 },
    { 1723, 12, 2, UCAL_MONTH, +1, 1724, 0, 2 }, // 1723 is a leap year
    { 1723, 12, 2, UCAL_MONTH, +9, 1724, 8, 2 }, { 1725, 0, 1, UCAL_MONTH, -1, 1724, 12, 1 }, { 1725, 0, 1, UCAL_MONTH, -6, 1724, 7, 1 },
    { 1724, 12, 1, UCAL_DATE, +8, 1725, 0, 4 }, { 1723, 12, 1, UCAL_DATE, +8, 1724, 0, 3 }, // 1723 is a leap year
...
static const CoptEthCalLocale copEthCalLocales[] = {
    { "en@calendar=coptic", 0 }, { "en@calendar=ethiopic", 276 }, { NULL, 0 } // terminator
};
...
void CalendarRegressionTest::TestT6745()
{
    ...
    for (testLocalePtr = copEthCalLocales; testLocalePtr->locale != NULL; ++testLocalePtr) {
        ...
        for (testItemPtr = coptEthCalTestItems; testItemPtr->fieldDelta != 0; ++testItemPtr) {
            ...
            if (U_FAILURE(status)) {
                errln((UnicodeString) "FAIL: Calendar::add, locale " + testLocalePtr->locale + ", field/delta " + testItemPtr->fieldToChange + "/"
                    + testItemPtr->fieldDelta + ", status " + u_errorName(status));
                continue;
            }
            ...
            if (U_FAILURE(status) || year != endYear || month != testItemPtr->endMonth || day != testItemPtr->endDay) {
                errln((UnicodeString) "ERROR: Calendar::add, locale " + testLocalePtr->locale + ", field/delta " + testItemPtr->fieldToChange + "/"
                    + testItemPtr->fieldDelta + ", status " + u_errorName(status) + ", expected " + endYear + "/" + testItemPtr->endMonth + "/"
                    + testItemPtr->endDay + ", got " + year + "/" + month + "/" + day);
            }
        }
        delete cal;
    }
...
void CalendarRegressionTest::TestLeapFieldDifference()
{
    ...
    if (d == -d2 && d == 13359) {
        ...
        logln((UnicodeString) "FAIL: large field difference incorrect " + d + ", " + d2 + ", expect +/- 13359");
    }
    delete cal;
}
...
{
    UErrorCode ec = U_ZERO_ERROR;
    GregorianCalendar cal(TimeZone::createTimeZone("America/Los_Angeles"), Locale("en", "US"), ec);
    if (U_FAILURE(ec)) {
    ...
    for (int32_t pass = 0; pass < 2; ++pass) {
        ...
        if (woy1 == woy2 && wom1 == wom2) {
            logln((UnicodeString) "Ok: WEEK_OF_YEAR: " + woy1 + ", WEEK_OF_MONTH: " + wom1);
        } else {
            errln((UnicodeString) "FAIL: WEEK_OF_YEAR: " + woy1 + " => " + woy2 + ", WEEK_OF_MONTH: " + wom1 + " => " + wom2 + " after week shift");
        }
    }
}
    ...
    while ((id = tz->unext(NULL, ec)) != NULL && U_SUCCESS(ec)) {
        ...
        for (int32_t i = 0; i < 365 + 10 && U_SUCCESS(ec); ++i) {
            ...
            if (yesterday_day == cal->get(UCAL_DATE, ec)) {
                errln(UnicodeString(id) + " " + dateFormat.format(yesterday, buf1) + " +1d= " + dateFormat.format(cal->getTime(ec), buf2));
                buf1.truncate(0);
                buf2.truncate(0);
            }
        }
...
UDate CalendarRegressionTest::makeDate(int32_t y, int32_t m, int32_t d, int32_t hr, int32_t min, int32_t sec)
{
    UDate result;
    ...
    if (d != 0)
        cal->set(UCAL_DATE, d);
    if (hr != 0)
        cal->set(UCAL_HOUR, hr);
    if (min != 0)
        cal->set(UCAL_MINUTE, min);
    if (sec != 0)
        cal->set(UCAL_SECOND, sec);
    ...
    if (U_FAILURE(status)) {
        ...
        for (jd = 67023580; jd <= 67023584; jd++) { // year 178171, int32_t overflow if jd >= 67023582
            ...
            if (U_FAILURE(status)) {
                ...
                errln("FAIL: localeID %s, julianDay %d; maxMonth %d, got month %d; maxDayOfMonth %d, got dayOfMonth %d", localeID, jd, maxMonth, month,
    ...
    if (U_FAILURE(status)) {
        ...
        for (jd = 73530872; jd <= 73530876; jd++) { // year 202002, int32_t overflow if jd >= 73530874
            ...
            if (U_FAILURE(status)) {
                ...
                errln("FAIL: localeID %s, julianDay %d; got year %d; maxMonth %d, got month %d; maxDayOfMonth %d, got dayOfMonth %d", localeID, jd, year,
                    maxMonth, month, maxDayOfMonth, dayOfMonth);
            }
        }
        delete cal;
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<Calendar> utc(Calendar::createInstance(TimeZone::createTimeZone(u"UTC"), status));
    ...
    status = U_ZERO_ERROR;
    LocalPointer<Calendar> gmt(Calendar::createInstance(*TimeZone::getGMT(), status));
    ...
    if (value != UCAL_AM && value != UCAL_PM) {
        errln("UTC %e UCAL_AM_PM should be either UCAL_AM | UCAL_PM  but is %d", time, value);
    }
    ...
    if (value != UCAL_AM && value != UCAL_PM) {
        errln("GMT %e UCAL_AM_PM should be either UCAL_AM | UCAL_PM  but is %d", time, value);
    }

    int32_t fields[] = { UCAL_WEEK_OF_YEAR, UCAL_YEAR_WOY, UCAL_DAY_OF_MONTH, UCAL_WEEK_OF_MONTH, UCAL_DAY_OF_WEEK_IN_MONTH, UCAL_MILLISECONDS_IN_DAY,
        UCAL_MILLISECOND, UCAL_SECOND, UCAL_MINUTE, UCAL_HOUR_OF_DAY, UCAL_AM_PM, UCAL_HOUR, UCAL_ZONE_OFFSET, UCAL_DST_OFFSET };
    for (auto& f : fields) {
        ...
        if (value < min) {
            errln("UTC %e get(%d) < getMinimum(%d) : %d < %d", time, field, field, value, min);
        }
        if (max < value) {
            errln("UTC %e getMaximum(%d) < get(%d) : %d < %d", time, field, field, max, value);
        }
        ...
        if (value < min) {
            errln("GMT %e get(%d) < getMinimum(%d) : %d < %d", time, field, field, value, min);
        }
        if (max < value) {
            errln("GMT %e getMaximum(%d) < get(%d) : %d < %d", time, field, field, max, value);
        }
    }
}
...
{
    UErrorCode status = U_ZERO_ERROR;
    std::unique_ptr<Calendar> cal(Calendar::createInstance(TimeZone::createTimeZone(UnicodeString(timezone, -1, US_INV)), Locale::getEnglish(), status));
    cal->setTime(Calendar::getNow(), status);

    if (cal->getDynamicClassID() == GregorianCalendar::getStaticClassID()) {
        GregorianCalendar* gc = static_cast<GregorianCalendar*>(cal.get());
