...
void UTextTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    TESTCASE_AUTO_BEGIN;
    ...
    for (srcLengthType = 1; srcLengthType <= 3; srcLengthType++) {
        ...
        for (srcPosType = 1; srcPosType <= 5; srcPosType++) {
            ...
            case 2:
                ...
                break;
            case 3:
                srcIndex = cpCount - srcLength;
                break;
            case 4:
                srcIndex = cpCount - srcLength - 1;
                break;
            case 5:
                srcIndex = cpCount / 2;
                break;
            ...
            for (destPosType = 1; destPosType <= 4; destPosType++) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    destIndex = srcIndex + srcLength + 1;
                    break;
                case 5:
                    destIndex = cpCount - 1;
                    break;
                ...
                gFailed = FALSE;
                TestCopyMove(us, ut, FALSE, nativeStart, nativeLimit, nativeDest, u16Start, u16Limit, u16Dest);

                TestCopyMove(us, ut, TRUE, nativeStart, nativeLimit, nativeDest, u16Start, u16Limit, u16Dest);
            ...
            for (int32_t replStrLen = 0; replStrLen < 20; replStrLen++) {
                ...
                TestReplace(us, ut, nativeStart, nativeLimit, u16Start, u16Limit, repStr);
                if (gFailed) {
                    return;
                }
            }
...
//
void UTextTest::TestCopyMove(const UnicodeString& us, UText* ut, UBool move, int32_t nativeStart, int32_t nativeLimit, int32_t nativeDest, int32_t u16Start,
    int32_t u16Limit, int32_t u16Dest)
{
    UErrorCode status = U_ZERO_ERROR;
    UText* targetUT = NULL;
    gTestNum++;
...
//
void UTextTest::TestReplace(const UnicodeString& us, // reference UnicodeString in which to do the replace
...
void UTextTest::TestAccessNoClone(const UnicodeString& us, UText* ut, int cpCount, m* cpMap)
{
    ...
    if (us.length() == 0) {
        ...
        TEST_ASSERT(buf[0] == us.charAt(0) || (buf[0] == 0x5555 && U_IS_SUPPLEMENTARY(us.char32At(0))));
...
U_CDECL_BEGIN
static UBool U_CALLCONV fragTextAccess(UText* ut, int64_t index, UBool forward)
{
...
//   out of the tests.
//
UText* cloneFragmentedUnicodeString(UText* dest, const UText* src, UBool deep, UErrorCode* status)
{
    if (U_FAILURE(*status)) {
        return NULL;
    }
...
void UTextTest::Ticket12130()
{
    ...
    const char* text8 = "Fundamentally, computers just deal with numbers. They store letters and other characters "
    ...
    for (int32_t startIdx = 0; startIdx < str.length(); ++startIdx) {
        ...
        if (0 != str.tempSubString(startIdx, 20).compare(extractBuffer)) {
            errln("%s:%d utext_extract() failed. expected \"%s\", got \"%s\"", __FILE__, __LINE__, CStr(str.tempSubString(startIdx, 20))(),
