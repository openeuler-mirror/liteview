...
static void parseMappings(const char* filename, UBool reportError, TestIDNA& test, UErrorCode* pErrorCode);

static void compareMapping(uint32_t codepoint, uint32_t* mapping, int32_t mapLength, UStringPrepType option);

static void compareFlagsForRange(uint32_t start, uint32_t end, UStringPrepType option);

static void testAllCodepoints(TestIDNA& test);

static TestIDNA* pTestIDNA = NULL;

static const char* fileNames[] = { "rfc3491.txt" };
...
extern int testData(TestIDNA& test)
{
    char* basename = NULL;
...
static inline UStringPrepType getValues(uint32_t result, int32_t& value, UBool& isIndex)
{
...
static void compareMapping(uint32_t codepoint, uint32_t* mapping, int32_t mapLength, UStringPrepType type)
{
    uint32_t result = 0;
    ...
    if (isIndex) {
        for (int8_t i = 0; i < mapLength; i++) {
            if (mapping[i] <= 0xFFFF) {
                ...
                if (mappingData[index + i] != lead || mappingData[index + i + 1] != trail) {
                    pTestIDNA->errln("Did not get the expected result. Expected: 0x%04X 0x%04X  Got: 0x%04X 0x%04X", lead, trail, mappingData[index + i],
                        mappingData[index + i + 1]);
                }
            }
        }
    } else {
...
static void compareFlagsForRange(uint32_t start, uint32_t end, UStringPrepType type)
{
    ...
    */
    while (start < end + 1) {
        ...
        if (result > _SPREP_TYPE_THRESHOLD) {
            if (retType != type) {
                pTestIDNA->errln(
                    "FAIL: Did not get the expected type for 0x%06X. Expected: %s Got: %s\n", start, usprepTypeNames[type], usprepTypeNames[retType]);
            }
        } else {
