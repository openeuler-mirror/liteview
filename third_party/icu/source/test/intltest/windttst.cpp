...
void Win32DateTimeTest::testLocales(DateFormatTest* log)
{
    ...
    for (int i = 0; i < lcidCount; i += 1) {
        ...
        if (ubBuffer.indexOf((const UChar*)wdBuffer, wdLength - 1, 0) < 0) {
            ...
            log->errln("DateTime format error for locale " + baseName + ": expected date \"" + expected + "\" got \"" + ubBuffer + "\"");
        }
        ...
        if (ubBuffer.indexOf((const UChar*)wtBuffer, wtLength - 1, 0) < 0) {
            ...
            log->errln("DateTime format error for locale " + baseName + ": expected time \"" + expected + "\" got \"" + ubBuffer + "\"");
        }
        ...
        if (udBuffer.compare((const UChar*)wdBuffer) != 0) {
            ...
            log->errln("Date format error for locale " + baseName + ": expected \"" + expected + "\" got \"" + udBuffer + "\"");
        }
        ...
        if (utBuffer.compare((const UChar*)wtBuffer) != 0) {
            ...
            log->errln("Time format error for locale " + baseName + ": expected \"" + expected + "\" got \"" + utBuffer + "\"");
        }
        delete wbf;
        delete wdf;
        delete wtf;
