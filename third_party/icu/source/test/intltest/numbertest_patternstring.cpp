...
void PatternStringTest::testLocalized()
{
    ...
    assertEquals("standard to localized", localized, PatternStringUtils::convertLocalized(standard, symbols, true, status));
    assertEquals("localized to standard", toStandard, PatternStringUtils::convertLocalized(localized, symbols, false, status));
}

void PatternStringTest::testToPatternSimple()
{
    const char16_t* cases[][2] = {
        { u"#", u"0" },
        { u"0", u"0" },
        { u"#0", u"0" },
        { u"###", u"0" },
    ...
    for (const char16_t** cas : cases) {
        ...
        DecimalFormatProperties properties = PatternParser::parseToProperties(input, IGNORE_ROUNDING_NEVER, status);
...
{
    static const char16_t* invalidPatterns[] = { u"#.#.#", u"0#", u"0#.", u".#0", u"0#.#0", u"@0", u"0@", u"0,", u"0,,", u"0,,0", u"0,,0,", u"#,##0E0" };
...
{
    UErrorCode status = U_ZERO_ERROR;
    DecimalFormatProperties expected = PatternParser::parseToProperties(u"0", IGNORE_ROUNDING_NEVER, status);
    DecimalFormatProperties actual = PatternParser::parseToProperties(u"0;", IGNORE_ROUNDING_NEVER, status);
...
void PatternStringTest::testCurrencyDecimal()
{
    ...
    static const UFieldPosition expectedFieldPositions[] = { { UNUM_INTEGER_FIELD, 1, 2 }, { UNUM_CURRENCY_FIELD, 2, 3 }, { UNUM_FRACTION_FIELD, 3, 5 } };
    checkFormattedValue(
        u"Currency as decimal basic field positions", fn, u"a3€14b", UFIELD_CATEGORY_NUMBER, expectedFieldPositions, UPRV_LENGTHOF(expectedFieldPositions));
}
