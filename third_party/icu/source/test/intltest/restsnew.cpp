...
enum E_Where { e_Root, e_te, e_te_IN, e_Where_count };

//***************************************************************************************

#define CONFIRM_EQ(actual, expected)                                                                                                                           \
...
#define CONFIRM_GE(actual, expected)                                                                                                                           \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if ((actual) >= (expected)) {                                                                                                                          \
            record_pass();                                                                                                                                     \
    ...
    UPRV_BLOCK_MACRO_END
#define CONFIRM_NE(actual, expected)                                                                                                                           \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if ((expected) != (actual)) {                                                                                                                          \
            record_pass();                                                                                                                                     \
...
static UBool equalRB(ResourceBundle& a, ResourceBundle& b)
{
    UResType type;
    UErrorCode status;
    ...
    type = a.getType();
    status = U_ZERO_ERROR;
    return type == b.getType() && a.getLocale() == b.getLocale() && 0 == strcmp(a.getName(), b.getName()) && type == URES_STRING
        ? a.getString(status) == b.getString(status)
        : type == URES_INT ? a.getInt(status) == b.getInt(status)
                           : TRUE;
}
...
void NewResourceBundleTest::TestOtherAPI()
{
    ...
    if (strcmp(copyRes.getName(), defaultresource.getName()) != 0 || strcmp(test1.getName(), defaultresource.getName()) == 0
    ...
    if (defaultresource.getSize() != 0) {
        ...
        if (strcmp(defSubCopy.getName(), defaultSub.getName()) != 0 || strcmp(defSubCopy.getLocale().getName(), defaultSub.getLocale().getName()) != 0) {
...
UBool NewResourceBundleTest::testTag(const char* frag, UBool in_Root, UBool in_te, UBool in_te_IN)
{
    ...
    for (i = 0; i < bundles_count; ++i) {
        ...
        for (j = 0; j < 200; ++j) {
            ...
            expected_status = (row >= 0 && row < row_count && col >= 0 && col < column_count) ? expected_resource_status : U_MISSING_RESOURCE_ERROR;
            CONFIRM_UErrorCode(status, expected_status);
    ...
    /* this tests if escapes are preserved or not */
    {
        ...
        if (U_SUCCESS(status)) {
            ...
            if (str.compare(uExpect) != 0) {
                errln("Did not get the expected string for testescape expected. Expected : " + UnicodeString(uExpect) + " Got: " + str);
            }
        }
    }
...
void NewResourceBundleTest::TestIntervalAliasFallbacks()
{
    ...
    for (int lidx = 0; lidx < UPRV_LENGTHOF(locales); lidx++) {
        ...
        for (int cidx = 0; cidx < UPRV_LENGTHOF(calendars); cidx++) {
            ...
            if (U_FAILURE(status)) {
                errln("Cannot ures_getStringByKeyWithFallback('%s') on locale %s", key.data(), locales[lidx]);
                break;
            }
        }
        ures_close(rb);
...
void NewResourceBundleTest::TestTrace()
{
...

    {
        ...
        assertEquals("Should touch expected resource paths", { "/boundaries", "/boundaries/word", "/boundaries/word" }, gResourcePathsTraced);
        assertEquals("Should touch expected resource bundles", { U_ICUDATA_NAME "-brkitr/zh.res" }, gBundlesTraced);
        assertEquals("Should touch expected data files", { U_ICUDATA_NAME "-brkitr/word.brk" }, gDataFilesTraced);
...

    {
        ...
        assertEquals("Should touch expected resource paths", { "/CurrencyMeta", "/CurrencyMeta/DEFAULT", "/CurrencyMeta/DEFAULT" }, gResourcePathsTraced);
        assertEquals("Should touch expected resource bundles", { U_ICUDATA_NAME "-curr/supplementalData.res" }, gBundlesTraced);
        assertEquals("Should touch expected data files", {}, gDataFilesTraced);
        gResourcePathsTraced.clear();
        gDataFilesTraced.clear();
        gBundlesTraced.clear();
    }
