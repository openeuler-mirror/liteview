...
static const char16_t* EXAMPLE_STRINGS[] = { u"", u"xyz", u"The quick brown fox jumps over the lazy dog", u"😁", u"mixed 😇 and ASCII",
    u"with combining characters like 🇦🇧🇨🇩", u"A very very very very very very very very very very long string to force heap" };
...
void FormattedStringBuilderTest::testSplice()
{
    ...
    } cases[] = { { u"", 0, 0 }, { u"abc", 0, 0 }, { u"abc", 1, 1 }, { u"abc", 1, 2 }, { u"abc", 0, 2 }, { u"abc", 0, 3 },
        { u"lorem ipsum dolor sit amet", 8, 8 }, { u"lorem ipsum dolor sit amet", 8, 11 }, // 3 chars, equal to replacement "xyz"
...
{
    static const UChar32 cases[] = { 0, 1, 60, 127, 128, 0x7fff, 0x8000, 0xffff, 0x10000, 0x1f000, 0x10ffff };
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString sb1;
    FormattedStringBuilder sb2;
    for (UChar32 cas : cases) {
        ...
        assertEquals("First code unit in sb3", !U_IS_SUPPLEMENTARY(cas) ? (char16_t)cas : U16_LEAD(cas), sb3.charAt(0));
...
void FormattedStringBuilderTest::testFields()
{
    ...
    for (UnicodeString str : EXAMPLE_STRINGS) {
        ...
        for (int32_t i = 0; i < str.length(); i++) {
            assertEquals("Null field first", kUndefinedField.bits, sb.fieldAt(i).bits);
            assertEquals("Currency field second", Field(UFIELD_CATEGORY_NUMBER, UNUM_CURRENCY_FIELD).bits, sb.fieldAt(i + str.length()).bits);
        }
        ...
        if (str.length() > 0) {
            ...
            assertEquals("Integer field", Field(UFIELD_CATEGORY_NUMBER, UNUM_INTEGER_FIELD).bits, sb.fieldAt(2).bits);
        }
        ...
        for (int32_t i = 0; i < sb.length(); i++) {
            ...
            assertEquals("Field should equal location in old", old.fieldAt(i % old.length()).bits, field.bits);
...
void FormattedStringBuilderTest::testInsertOverflow()
{
    ...
    sb.splice(0, 1, ustr, 1, ustr.length(), kUndefinedField, status);
    assertEquals("splice() long text should not crash but return U_INPUT_TOO_LONG_ERROR", U_INPUT_TOO_LONG_ERROR, status);

    // Test sb.insert() of the first FormattedStringBuilder with the second one.
    sb.insert(0, sb2, status);
    assertEquals("insert() long FormattedStringBuilder should not crash but return "
    ...
    assertEquals("insert() long UnicodeString should not crash but return "
                 "U_INPUT_TOO_LONG_ERROR",
        U_INPUT_TOO_LONG_ERROR, status);
}
...
void FormattedStringBuilderTest::assertEqualsImpl(const UnicodeString& a, const FormattedStringBuilder& b)
{
    ...
    for (int32_t i = 0; i < a.length(); i++) {
        IntlTest::assertEquals(UnicodeString(u"Char at position ") + Int64ToUnicodeString(i) + UnicodeString(u" in \"") + a + UnicodeString("\" versus \"")
                + b.toUnicodeString() + UnicodeString("\""),
