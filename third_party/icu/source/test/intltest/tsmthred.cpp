...
void MultithreadTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    if (exec)
...
void TestArabicShapeThreads::doTailTest(void)
{
    ...
    for (int32_t loopCount = 0; loopCount < 100; loopCount++) {
        status = U_ZERO_ERROR;
        length = u_shapeArabic(src, -1, dst, UPRV_LENGTHOF(dst), U_SHAPE_LETTERS_SHAPE | U_SHAPE_SEEN_TWOCELL_NEAR, &status);
        if (U_FAILURE(status)) {
        ...
        // Trying new tail
        status = U_ZERO_ERROR;
        length = u_shapeArabic(src, -1, dst, UPRV_LENGTHOF(dst), U_SHAPE_LETTERS_SHAPE | U_SHAPE_SEEN_TWOCELL_NEAR | U_SHAPE_TAIL_NEW_UNICODE, &status);
        if (U_FAILURE(status)) {
            IntlTest::gTest->errln("Fail: status %s\n", u_errorName(status));
            return;
        } else if (length != 2) {
...
struct FormatThreadTestData {
    ...
    FormatThreadTestData(double a, const UnicodeString& b)
        : number(a)
        , string(b)
    {
    }
...
UBool ThreadSafeFormat::doStuff(int32_t offset, UnicodeString& appendErr, UErrorCode& status) const
{
    ...
    if (u_strcmp(fFormat->getCurrency(), kUSD)) {
        appendErr.append(u"fFormat currency != ").append(kUSD).append(u", =").append(fFormat->getCurrency()).append(u"! ");
        okay = FALSE;
    }

    if (u_strcmp(gSharedData->fFormat->getCurrency(), kUSD)) {
        appendErr.append(u"gFormat currency != ").append(kUSD).append(u", =").append(gSharedData->fFormat->getCurrency()).append(u"! ");
        okay = FALSE;
    }
    UnicodeString str;
    ...
    switch (offset % 4) {
    case 0:
        f = gSharedData->fYDDThing;
        o = &gSharedData->fYDDStr;
        nf = gSharedData->fFormat.getAlias();
        break;
    case 1:
        f = gSharedData->fBBDThing;
        o = &gSharedData->fBBDStr;
        nf = gSharedData->fFormat.getAlias();
        break;
    case 2:
        f = gSharedData->fYDDThing;
...
public:
    ...
    virtual void run() override
    {
        ...
        FormatThreadTestData kPercentFormatTestData[] = {
            ...
            FormatThreadTestData(16384.99, CharsToUnicodeString("1\\u202F638\\u202F499\\u00a0%")), // U+202F = NNBSP
        ...
        if (U_FAILURE(status)) {
            IntlTest::gTest->dataerrln("%s:%d Error %s on NumberFormat::createInstance().", __FILE__, __LINE__, u_errorName(status));
            goto cleanupAndReturn;
        }

        percentFormatter.adoptInstead(NumberFormat::createPercentInstance(Locale::getFrench(), status));
        if (U_FAILURE(status)) {
            IntlTest::gTest->errln("%s:%d Error %s on NumberFormat::createPercentInstance().", __FILE__, __LINE__, u_errorName(status));
            goto cleanupAndReturn;
        }
        ...
        for (iteration = 0; !IntlTest::gTest->getErrors() && iteration < kFormatThreadIterations; iteration++) {
            ...
            if (0 != output.compare(kNumberFormatTestData[whichLine].string)) {
                IntlTest::gTest->errln("format().. expected " + kNumberFormatTestData[whichLine].string + " got " + output);
                goto cleanupAndReturn;
            }
            ...
            if (U_FAILURE(status)) {
                ...
                IntlTest::gTest->errln(u"Failure on message format, pattern=" + patternToCheck + ", error = " + tmp);
                goto cleanupAndReturn;
            }
...
void MultithreadTest::TestThreadedIntl()
{
    ...
    //
    logln("Spawning: %d threads * %d iterations each.", kFormatThreadThreads, kFormatThreadIterations);
        ...
        if (testFile == 0) {
            ...
            if (testFile == 0) {
                *(buffer + bufLen) = 0;
                ...
                infoln("INFO: Working with the stub file.\n"
...
static const char* gCacheLocales[] = { "en_US", "en_GB", "fr_FR", "fr", "de", "sr_ME", "sr_BA", "sr_CS" };
static const char* gCacheLocales2[] = { "bs", "ca", "ca_AD", "ca_ES", "en_US", "fi", "ff_CM", "ff_GN" };
...
public:
    char* value;
    UCTMultiThreadItem(const char* x)
        : value(NULL)
    {
        value = uprv_strdup(x);
    }
...
template <> U_EXPORT const UCTMultiThreadItem* LocaleCacheKey<UCTMultiThreadItem>::createObject(const void* context, UErrorCode& status) const
{
    ...
    const UCTMultiThreadItem* result = new UCTMultiThreadItem(fLoc.getLanguage());
    if (result == NULL) {
...
public:
    UnifiedCacheThread(const UnifiedCache* cache, const char* loc, const char* loc2)
        : fCache(cache)
        , fLoc(loc)
        , fLoc2(loc2)
    {
...
void UnifiedCacheThread::exerciseByLocale(const Locale& locale)
{
    ...
    fCache->get(LocaleCacheKey<UCTMultiThreadItem>(locale), fCache, origItem, status);
    ...
    for (int32_t i = 0; i < 1000; ++i) {
        ...
        fCache->get(LocaleCacheKey<UCTMultiThreadItem>(locale), fCache, item, status);
...
void MultithreadTest::TestUnifiedCache()
{
    ...
    for (int32_t i = 0; i < CACHE_LOAD; ++i) {
        for (int32_t j = 0; j < UPRV_LENGTHOF(gCacheLocales); ++j) {
            ...
            threads[i][j] = new UnifiedCacheThread(&cache, gCacheLocales[j], gCacheLocales2[j]);
            threads[i][j]->start();
        }
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString input(u"\u0E42\u0E14\u0E22\u0E1E\u0E37\u0E49\u0E19\u0E10\u0E32\u0E19\u0E41\u0E25\u0E49\u0E27,");
    // Thai script, โดยพื้นฐานแล้ว
    gTranslitInput = &input;

    gSharedTransliterator
        = Transliterator::createInstance(UnicodeString(u"Any-Latin; Lower; NFD; [:Diacritic:]Remove; NFC; Latin-ASCII;"), UTRANS_FORWARD, status);
