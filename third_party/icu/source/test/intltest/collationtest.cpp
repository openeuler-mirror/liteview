...
private:
    ...
    void checkAllocWeights(CollationWeights& cw, uint32_t lowerLimit, uint32_t upperLimit, int32_t n, int32_t someLength, int32_t minCount);
    ...
    static UBool isCROrLF(UChar c)
    {
        return c == 0xa || c == 0xd;
    }
    static UBool isSpace(UChar c)
    {
        return c == 9 || c == 0x20 || c == 0x3000;
    }
    static UBool isSectionStarter(UChar c)
    {
    ...
    UBool getSortKeyParts(const UChar* s, int32_t length, CharString& dest, int32_t partSize, IcuTestErrorCode& errorCode);
    ...
    UBool checkCompareTwo(const char* norm, const UnicodeString& prevFileLine, const UnicodeString& prevString, const UnicodeString& s,
        UCollationResult expectedOrder, Collation::Level expectedLevel, IcuTestErrorCode& errorCode);
...
void CollationTest::TestImplicits()
{
    ...
    UnicodeSet someHanInCPOrder("[\\u4E00-\\u4E16\\u4E18-\\u4E2B\\u4E2D-\\u4E3C\\u4E3E-\\u4E48"
                                "\\u4E4A-\\u4E60\\u4E63-\\u4E8F\\u4E91-\\u4F63\\u4F65-\\u50F1\\u50F3-\\u50F6]",
        errorCode);
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(sets); ++i) {
        ...
        while (iter->next()) {
            ...
            if ((ce & 0xffffffff) != Collation::COMMON_SEC_AND_TER_CE) {
                errln("CollationIterator.nextCE(U+%04lx) has non-common sec/ter weights: %08lx", (long)c, (long)(ce & 0xffffffff));
                continue;
            }
            uint32_t primary = (uint32_t)(ce >> 32);
            if (!(primary > prevPrimary) && inOrder.contains(c) && inOrder.contains(prev)) {
                errln("CE(U+%04lx)=%04lx.. not greater than CE(U+%04lx)=%04lx..", (long)c, (long)primary, (long)prev, (long)prevPrimary);
            }
            prev = c;
            prevPrimary = primary;
        }
...
void CollationTest::TestIllegalUTF8()
{
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(strings); i += 2) {
        ...
        if (order != UCOL_EQUAL) {
            errln("compareUTF8(pair %d: U+FFFD, illegal UTF-8)=%d != UCOL_EQUAL", (int)i, order);
        }
    }
}
...
class CodePointIterator {
public:
    CodePointIterator(const UChar32* cp, int32_t length)
        : cp(cp)
        , length(length)
        , pos(0)
    {
...
void CollationTest::checkFCD(const char* name, CollationIterator& ci, CodePointIterator& cpi)
{
    ...
    for (;;) {
        ...
        UChar32 c2 = cpi.next();
        if (c1 != c2) {
            errln("%s.nextCodePoint(to limit, 1st pass) = U+%04lx != U+%04lx at %d", name, (long)c1, (long)c2, cpi.getIndex());
            return;
        }
        if (c1 < 0) {
            break;
    ...
    for (int32_t n = (cpi.getLength() * 2) / 3; n > 0; --n) {
        ...
        if (c1 != c2) {
            errln("%s.previousCodePoint() = U+%04lx != U+%04lx at %d", name, (long)c1, (long)c2, cpi.getIndex());
            return;
        }
    }
    ...
    for (;;) {
        ...
        UChar32 c2 = cpi.next();
        if (c1 != c2) {
            errln("%s.nextCodePoint(to limit again) = U+%04lx != U+%04lx at %d", name, (long)c1, (long)c2, cpi.getIndex());
            return;
        }
        if (c1 < 0) {
            break;
    ...
    for (;;) {
        ...
        UChar32 c2 = cpi.previous();
        if (c1 != c2) {
            errln("%s.previousCodePoint(to start) = U+%04lx != U+%04lx at %d", name, (long)c1, (long)c2, cpi.getIndex());
            return;
        }
        if (c1 < 0) {
            break;
...
void CollationTest::TestFCD()
{
    ...
    static const UChar s[]
        = { 0x308, 0xe1, 0x62, 0x301, 0x327, 0x430, 0x62, U16_LEAD(0x1D15F), U16_TRAIL(0x1D15F), // MUSICAL SYMBOL QUARTER NOTE=1D158 1D165, ccc=0, 216
              0x327, 0x308, // ccc=202, 230
              U16_LEAD(0x1D16D), U16_TRAIL(0x1D16D), // MUSICAL SYMBOL COMBINING AUGMENTATION DOT, ccc=226
              U16_LEAD(0x1D15F), U16_TRAIL(0x1D15F), U16_LEAD(0x1D16D), U16_TRAIL(0x1D16D), 0xac01,
    ...
    static const UChar32 cp[] = { 0x308, 0xe1, 0x62, 0x327, 0x301, 0x430, 0x62, 0x1D158, 0x327, 0x1D165, 0x1D16D, 0x308, 0x1D15F, 0x1D16D, 0xac01, 0x63, 0x327,
        0x1D165, 0x1D16D, 0x61, 0xf71, 0xf71, 0xf72, 0xf74, 0x301, 0x4e00, 0xf71, 0xf80 };
    ...
    FCDUTF8CollationIterator u8ci(data, FALSE, reinterpret_cast<const uint8_t*>(utf8.c_str()), 0, -1);
...
void CollationTest::checkAllocWeights(CollationWeights& cw, uint32_t lowerLimit, uint32_t upperLimit, int32_t n, int32_t someLength, int32_t minCount)
{
    if (!cw.allocWeights(lowerLimit, upperLimit, n)) {
        errln("CollationWeights::allocWeights(%lx, %lx, %ld) = FALSE", (long)lowerLimit, (long)upperLimit, (long)n);
        return;
    }
    ...
    for (int32_t i = 0; i < n; ++i) {
        ...
        if (!(previous < w && w < upperLimit)) {
            errln("CollationWeights::allocWeights(%lx, %lx, %ld).nextWeight() "
                ...
                (long)lowerLimit, (long)upperLimit, (long)n, (long)(i + 1), (long)w, (long)previous, (long)upperLimit);
            return;
        }
    ...
    if (count < minCount) {
        errln("CollationWeights::allocWeights(%lx, %lx, %ld).nextWeight() "
            ...
            (long)lowerLimit, (long)upperLimit, (long)n, (long)count, (long)minCount, (int)someLength);
    }
}
...
UBool isValidCE(const CollationRootElements& re, const CollationData& data, uint32_t p, uint32_t s, uint32_t ctq)
{
    uint32_t p1 = p >> 24;
    ...
    if ((p1 != 0 && p1 <= Collation::MERGE_SEPARATOR_BYTE) || s1 == Collation::LEVEL_SEPARATOR_BYTE || t1 == Collation::LEVEL_SEPARATOR_BYTE || t1 > 0x3f) {
        return FALSE;
    }
    if (c > 2) {
        return FALSE;
    ...
    if (p2 != 0) {
        if (data.isCompressibleLeadByte(p1)) {
            if (p2 <= Collation::PRIMARY_COMPRESSION_LOW_BYTE || Collation::PRIMARY_COMPRESSION_HIGH_BYTE <= p2) {
                return FALSE;
            }
        } else {
    ...
    if (p3 == Collation::LEVEL_SEPARATOR_BYTE || p4 == Collation::LEVEL_SEPARATOR_BYTE || s2 == Collation::LEVEL_SEPARATOR_BYTE
    ...
    if (p == 0) {
        ...
        // Primary CE.
        if (s == 0 || (Collation::COMMON_WEIGHT16 < s && s <= re.getLastCommonSecondary()) || s >= re.getSecondaryBoundary()) {
            return FALSE;
        }
...
public:
    RootElementsIterator(const CollationData& root)
        ...
        , length(root.rootElementsLength)
        , pri(0)
        , secTer(0)
...
void CollationTest::TestRootElements()
{
    ...
    while (iter.next()) {
        ...
        if ((secTer & Collation::CASE_AND_QUATERNARY_MASK) != 0) {
            errln("CollationRootElements CE has non-zero case and/or quaternary bits: %08lx %08lx", (long)pri, (long)secTer);
        }
            ...
            if (pri != prevPri) {
                ...
                if (prevPri == 0 || prevPri >= Collation::FFFD_PRIMARY) {
                    ...
                    if (!cw1c.allocWeights(prevPri, pri, 1)) {
                        errln("no primary/compressible tailoring gap between %08lx and %08lx", (long)prevPri, (long)pri);
                    } else {
                        newWeight = cw1c.nextWeight();
                    }
                } else {
                    if (!cw1u.allocWeights(prevPri, pri, 1)) {
                        errln("no primary/uncompressible tailoring gap between %08lx and %08lx", (long)prevPri, (long)pri);
                    } else {
                        newWeight = cw1u.nextWeight();
                    }
                }
                if (newWeight != 0 && !(prevPri < newWeight && newWeight < pri)) {
                    errln("mis-allocated primary weight, should get %08lx < %08lx < %08lx", (long)prevPri, (long)newWeight, (long)pri);
                }
            } else if (sec != prevSec) {
                uint32_t lowerLimit = prevSec == 0 ? rootElements.getSecondaryBoundary() - 0x100 : prevSec;
                if (!cw2.allocWeights(lowerLimit, sec, 1)) {
                    ...
                    if (!(prevSec < newWeight && newWeight < sec)) {
                        errln("mis-allocated secondary weight, should get %04x < %04x < %04x", (long)lowerLimit, (long)newWeight, (long)sec);
                    }
                }
            } else if (ter != prevTer) {
                uint32_t lowerLimit = prevTer == 0 ? rootElements.getTertiaryBoundary() - 0x100 : prevTer;
                if (!cw3.allocWeights(lowerLimit, ter, 1)) {
                    ...
                    if (!(prevTer < newWeight && newWeight < ter)) {
                        errln("mis-allocated secondary weight, should get %04x < %04x < %04x", (long)lowerLimit, (long)newWeight, (long)ter);
                    }
                }
            } else {
...
void CollationTest::TestTailoredElements()
{
    ...
    do {
        ...
        LocalPointer<StringEnumeration> types(Collator::getKeywordValuesForLocale("collation", locale, FALSE, errorCode));
        errorCode.assertSuccess();
        const char* type; // first: default type
        while ((type = types->next(NULL, errorCode)) != NULL) {
            if (strncmp(type, "private-", 8) == 0) {
                errln("Collator::getKeywordValuesForLocale(%s) returns private collation keyword: %s", localeID, type);
            }
            ...
            if (errorCode.errIfFailureAndReset("Collator::createInstance(%s)", localeWithType.getName())) {
                continue;
            }
            ...
            errorCode.assertSuccess();
            logln("TestTailoredElements(): requested %s -> actual %s", localeWithType.getName(), actual.getName());
            ...
            while (iter.next()) {
                ...
                for (int32_t i = 0; i < ces.size(); ++i) {
                    ...
                    if (!isValidCE(rootElements, *root, ce)) {
                        errln("invalid tailored CE %016llx at CE index %d from string:", (long long)ce, (int)i);
                        infoln(prettify(s));
                    }
                }
            }
...
void CollationTest::parseString(int32_t& start, UnicodeString& prefix, UnicodeString& s, UErrorCode& errorCode)
{
...
static const struct {
    ...
    = { { "backwards", UCOL_FRENCH_COLLATION }, { "alternate", UCOL_ALTERNATE_HANDLING }, { "caseFirst", UCOL_CASE_FIRST }, { "caseLevel", UCOL_CASE_LEVEL },
...
} attributeValues[] = { { "default", UCOL_DEFAULT }, { "primary", UCOL_PRIMARY }, { "secondary", UCOL_SECONDARY }, { "tertiary", UCOL_TERTIARY },
    { "quaternary", UCOL_QUATERNARY }, { "identical", UCOL_IDENTICAL }, { "off", UCOL_OFF }, { "on", UCOL_ON }, { "shifted", UCOL_SHIFTED },
    { "non-ignorable", UCOL_NON_IGNORABLE }, { "lower", UCOL_LOWER_FIRST }, { "upper", UCOL_UPPER_FIRST } };
...
void CollationTest::parseAndSetAttribute(IcuTestErrorCode& errorCode)
{
    ...
    if (attrString == UNICODE_STRING("maxVariable", 11)) {
        ...
        if (coll != NULL) {
            ...
            if (errorCode.isFailure()) {
                errln("setMaxVariable() failed on line %d: %s", (int)fileLineNumber, errorCode.errorName());
                infoln(fileLine);
                return;
            }
        }
    ...
    if (coll != NULL) {
        ...
        if (errorCode.isFailure()) {
            errln("illegal attribute=value combination on line %d: %s", (int)fileLineNumber, errorCode.errorName());
            infoln(fileLine);
            return;
        }
    }
...
void CollationTest::buildTailoring(UCHARBUF* f, IcuTestErrorCode& errorCode)
{
    ...
    if (errorCode.isFailure()) {
        ...
        if (parseError.preContext[0] != 0 || parseError.postContext[0] != 0) {
            infoln(UnicodeString("  snippet: ...") + parseError.preContext + "(!)" + parseError.postContext + "...");
        }
        delete coll;
        coll = NULL;
        errorCode.reset();
    } else {
        assertEquals("no error reason when RuleBasedCollator(rules) succeeds", UnicodeString(), reason);
    }
}
...
void CollationTest::setLocaleCollator(IcuTestErrorCode& errorCode)
{
    ...
    if (errorCode.isFailure()) {
        dataerrln("unable to create a collator for locale %s on line %d", locale.getName(), (int)fileLineNumber);
        infoln(fileLine);
        delete coll;
        coll = NULL;
        errorCode.reset();
...
UBool CollationTest::getSortKeyParts(const UChar* s, int32_t length, CharString& dest, int32_t partSize, IcuTestErrorCode& errorCode)
{
    if (errorCode.isFailure()) {
        return FALSE;
    }
...
UBool CollationTest::getCollationKey(
    const char* norm, const UnicodeString& line, const UChar* s, int32_t length, CollationKey& key, IcuTestErrorCode& errorCode)
{
    if (errorCode.isFailure()) {
        return FALSE;
    }
    ...
    if (errorCode.isFailure()) {
        infoln(fileTestName);
        errln("Collator(%s).getCollationKey() failed: %s", norm, errorCode.errorName());
        infoln(line);
        return FALSE;
    }
    int32_t keyLength;
    ...
    if (keyLength == 0 || keyBytes[keyLength - 1] != 0) {
        infoln(fileTestName);
        errln("Collator(%s).getCollationKey() wrote an empty or unterminated key", norm);
        infoln(line);
    ...
    if (numLevelSeparators != (numLevels - 1)) {
        infoln(fileTestName);
        errln("Collator(%s).getCollationKey() has %d level separators for %d levels", norm, (int)numLevelSeparators, (int)numLevels);
        infoln(line);
        infoln(printCollationKey(key));
        return FALSE;
    }
    ...
    for (int32_t psi = 0; psi < UPRV_LENGTHOF(partSizes); ++psi) {
        ...
        if (!getSortKeyParts(s, length, parts, 32, errorCode)) {
            infoln(fileTestName);
            errln("Collator(%s).internalNextSortKeyPart(%d) failed: %s", norm, (int)partSize, errorCode.errorName());
            infoln(line);
            return FALSE;
        }
        if (keyLength != parts.length() || uprv_memcmp(keyBytes, parts.data(), keyLength) != 0) {
            infoln(fileTestName);
            errln("Collator(%s).getCollationKey() != internalNextSortKeyPart(%d)", norm, (int)partSize);
            infoln(line);
            infoln(printCollationKey(key));
 ...
 */
UBool CollationTest::getMergedCollationKey(const UChar* s, int32_t length, CollationKey& key, IcuTestErrorCode& errorCode)
{
    if (errorCode.isFailure()) {
        return FALSE;
    }
    ...
    for (int32_t i = 0;;) {
        ...
        if (key1Length == 0) {
            ...
            mergedKeyLength = ucol_mergeSortkeys(key1Bytes, key1Length, key2Bytes, key2Length, dest, mergedKeyCapacity);
        }
        if (i == sLength) {
            break;
        }
...
int32_t getDifferenceLevel(const CollationKey& prevKey, const CollationKey& key, UCollationResult order, UBool collHasCaseLevel)
{
    if (order == UCOL_EQUAL) {
        return Collation::NO_LEVEL;
    }
...
UBool CollationTest::checkCompareTwo(const char* norm, const UnicodeString& prevFileLine, const UnicodeString& prevString, const UnicodeString& s,
    UCollationResult expectedOrder, Collation::Level expectedLevel, IcuTestErrorCode& errorCode)
{
    if (errorCode.isFailure()) {
        return FALSE;
    ...
    if (!getCollationKey(norm, prevFileLine, prevString.getBuffer(), prevString.length(), prevKey, errorCode)) {
        return FALSE;
    }
    CollationKey key;
    ...
    if (order != expectedOrder || errorCode.isFailure()) {
        infoln(fileTestName);
        errln("line %d Collator(%s).compare(previous, current) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, expectedOrder,
            errorCode.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
    ...
    if (order != -expectedOrder || errorCode.isFailure()) {
        infoln(fileTestName);
        errln("line %d Collator(%s).compare(current, previous) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, -expectedOrder,
            errorCode.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
        infoln(printCollationKey(prevKey));
    ...
    if (!containNUL) {
        ...
        if (order != expectedOrder || errorCode.isFailure()) {
            infoln(fileTestName);
            errln("line %d Collator(%s).compare(previous-NUL, current-NUL) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, expectedOrder,
                errorCode.errorName());
            infoln(prevFileLine);
            infoln(fileLine);
            infoln(printCollationKey(prevKey));
        ...
        if (order != -expectedOrder || errorCode.isFailure()) {
            infoln(fileTestName);
            errln("line %d Collator(%s).compare(current-NUL, previous-NUL) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, -expectedOrder,
                errorCode.errorName());
            infoln(prevFileLine);
            infoln(fileLine);
            infoln(printCollationKey(prevKey));
    ...
    if (order != expectedUTF8Order || errorCode.isFailure()) {
        infoln(fileTestName);
        errln("line %d Collator(%s).compareUTF8(previous, current) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, expectedUTF8Order,
            errorCode.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
        infoln(printCollationKey(prevKey));
    ...
    if (order != -expectedUTF8Order || errorCode.isFailure()) {
        infoln(fileTestName);
        errln("line %d Collator(%s).compareUTF8(current, previous) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, -expectedUTF8Order,
            errorCode.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
        infoln(printCollationKey(prevKey));
    ...
    if (!containNUL) {
        ...
        if (order != expectedUTF8Order || errorCode.isFailure()) {
            infoln(fileTestName);
            errln("line %d Collator(%s).internalCompareUTF8(previous-NUL, current-NUL) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order,
                expectedUTF8Order, errorCode.errorName());
            infoln(prevFileLine);
            infoln(fileLine);
            infoln(printCollationKey(prevKey));
            infoln(printCollationKey(key));
        ...
        if (order != -expectedUTF8Order || errorCode.isFailure()) {
            infoln(fileTestName);
            errln("line %d Collator(%s).internalCompareUTF8(current-NUL, previous-NUL) wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order,
                -expectedUTF8Order, errorCode.errorName());
            infoln(prevFileLine);
            infoln(fileLine);
            infoln(printCollationKey(prevKey));
    ...
    if (order != expectedOrder || errorCode.isFailure()) {
        infoln(fileTestName);
        errln("line %d Collator(%s).getCollationKey(previous, current).compareTo() wrong order: %d != %d (%s)", (int)fileLineNumber, norm, order, expectedOrder,
            errorCode.errorName());
        infoln(prevFileLine);
        infoln(fileLine);
        infoln(printCollationKey(prevKey));
    ...
    if (order != UCOL_EQUAL && expectedLevel != Collation::NO_LEVEL) {
        if (level != expectedLevel) {
            infoln(fileTestName);
            errln("line %d Collator(%s).getCollationKey(previous, current).compareTo()=%d wrong level: %d != %d", (int)fileLineNumber, norm, order, level,
                expectedLevel);
            infoln(prevFileLine);
            infoln(fileLine);
            infoln(printCollationKey(prevKey));
...
void CollationTest::checkCompareStrings(UCHARBUF* f, IcuTestErrorCode& errorCode)
{
    ...
    while (readNonEmptyLine(f, errorCode) && !isSectionStarter(fileLine[0])) {
        ...
        if (!needsNormalization(prevString, errorCode) && !needsNormalization(s, errorCode)) {
            ...
            isOk = checkCompareTwo("normalization=on", prevFileLine, prevString, s, expectedOrder, expectedLevel, errorCode);
        }
        if (isOk) {
            coll->setAttribute(UCOL_NORMALIZATION_MODE, UCOL_ON, errorCode);
            isOk = checkCompareTwo("normalization=off", prevFileLine, prevString, s, expectedOrder, expectedLevel, errorCode);
        }
        if (isOk && (!nfd->isNormalized(prevString, errorCode) || !nfd->isNormalized(s, errorCode))) {
            ...
            errorCode.assertSuccess();
            isOk = checkCompareTwo("NFD input", prevFileLine, pn, n, expectedOrder, expectedLevel, errorCode);
        }
        if (!isOk) {
...
void CollationTest::TestBuilderContextsOverflow()
{
    ...
    char16_t rules[] = { u'&', 0x10, 0x2ff, 0x503c, 0x4617, u'=', 0x80, 0x4f7f, 0xff, 0x3c3d, 0x1c4f, 0x3c3c, u'<', 0, 0, 0, 0, u'|', 0, 0, 0, 0, 0, 0xf400,
        0x30ff, 0, 0, 0x4f7f, 0xff, u'=', 0, u'|', 0, 0, 0, 0, 0, 0, 0x1f00, 0xe30, 0x3035, 0, 0, 0xd200, 0, 0x7f00, 0xff4f, 0x3d00, 0, 0x7c00, 0, 0, 0, 0, 0,
        0, 0, 0x301f, 0x350e, 0x30, 0, 0, 0xd2, 0x7c00, 0, 0, 0, 0, 0, 0, 0, 0x301f, 0x350e, 0x30, 0, 0, 0x52d2, 0x2f3c, 0x5552, 0x493c, 0x1f10, 0x1f50, 0x300,
        0, 0, 0xf400, 0x30ff, 0, 0, 0x4f7f, 0xff, u'=', 0, u'|', 0, 0, 0, 0, 0x5000, 0x4617, u'=', 0x80, 0x4f7f, 0, 0, 0xd200, 0 };
