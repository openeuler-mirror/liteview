...
UBool NumberFormatRoundTripTest::failure(UErrorCode status, const char* msg, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
...
uint32_t NumberFormatRoundTripTest::randLong()
{
 ...
 */
double NumberFormatRoundTripTest::randomDouble(double range)
{
    double a = randFraction();
    return (2.0 * range * a) - range;
}

void NumberFormatRoundTripTest::start()
{
...
void NumberFormatRoundTripTest::test(NumberFormat* fmt)
{
...
void NumberFormatRoundTripTest::test(NumberFormat* fmt, double value)
{
    test(fmt, Formattable(value));
}

void NumberFormatRoundTripTest::test(NumberFormat* fmt, int32_t value)
{
    test(fmt, Formattable(value));
}

void NumberFormatRoundTripTest::test(NumberFormat* fmt, const Formattable& value)
{
    ...
    if (show) {
        errln(/*value.getString(temp) +*/ typeOf(value, temp) + " F> " + escape(s) + " P> "
            + (n.getType() == Formattable::kDouble ? n.getDouble() : (double)n.getLong())
            /*n.getString(temp) */
            + typeOf(n, temp) + " F> " + escape(s2));
    }
}
...
UnicodeString& NumberFormatRoundTripTest::typeOf(const Formattable& n, UnicodeString& result)
{
...
UnicodeString& NumberFormatRoundTripTest::escape(UnicodeString& s)
{
