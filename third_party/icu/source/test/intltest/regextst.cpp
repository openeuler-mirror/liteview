...
#define REGEX_VERBOSE_TEXT(text)                                                                                                                               \
...
#define REGEX_CHECK_STATUS                                                                                                                                     \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
            dataerrln("%s:%d: RegexTest failure.  status=%s", __FILE__, __LINE__, u_errorName(status));                                                        \
            return;                                                                                                                                            \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END
...
#define REGEX_ASSERT_FAIL(expr, errcode)                                                                                                                       \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        UErrorCode status = U_ZERO_ERROR;                                                                                                                      \
        (expr);                                                                                                                                                \
        if (status != errcode) {                                                                                                                               \
            dataerrln("RegexTest failure at line %d.  Expected status=%s, got %s", __LINE__, u_errorName(errcode), u_errorName(status));                       \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END

#define REGEX_CHECK_STATUS_L(line)                                                                                                                             \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
...
#define REGEX_ASSERT_L(expr, line)                                                                                                                             \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        if ((expr) == FALSE) {                                                                                                                                 \
...
void RegexTest::assertUText(const char* expected, UText* actual, const char* file, int line)
{
    ...
    if (!testUTextEqual(&expectedText, actual)) {
        ...
        errln("%s:%d: assertUText: Failure: expected \"%s\" (%d chars), got \"%s\" (%d chars)", file, line, expectedBuf, (int)utext_nativeLength(&expectedText),
            buf, (int)utext_nativeLength(actual));
    }
...
#define REGEX_TESTLM(pat, text, looking, match)                                                                                                                \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
...
UBool RegexTest::doRegexLMTest(const char* pat, const char* text, UBool looking, UBool match, int32_t line)
{
    ...
    if (U_FAILURE(status)) {
        dataerrln("RegexTest failure in RegexPattern::compile() at line %d.  Status = %s", line, u_errorName(status));
        return FALSE;
    }
    if (line == 376) {
...
void RegexTest::regex_err(const char* pat, int32_t errLine, int32_t errCol, UErrorCode expectedStatus, int32_t line)
{
...
void RegexTest::API_Match_UTF8()
{
    ...
    //
    {
        ...
        REGEX_CHECK_STATUS;
        REGEX_ASSERT(m1->lookingAt(status) == TRUE);
        const char str_abcdefthisisatest[] = { 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x20, 0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x74, 0x65,
            0x73, 0x74, 0x00 }; /* abcdef this is a test */
    ...
    //
    {
        ...
        UErrorCode status = U_ZERO_ERROR;
        UText re = UTEXT_INITIALIZER;
        const char str_Gabcabc[] = { 0x2e, 0x2a, 0x3f, 0x28, 0x3f, 0x3a, 0x28, 0x5c, 0x47, 0x61, 0x62, 0x63, 0x29, 0x7c, 0x28, 0x61, 0x62, 0x63, 0x29, 0x29,
            0x00 }; /* .*?(?:(\\Gabc)|(abc)) */
...
void RegexTest::API_Replace_UTF8()
{
    ...
    const char str_v[24] = { 0x54, 0x68, 0x65, 0x20, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20, 0x6f, 0x66, 0x20, 0x5c, 0x24, 0x31, 0x20, 0x69, 0x73, 0x20, 0x24, 0x31,
        0x2e, 0x00 }; /* The value of \$1 is $1. */
    utext_openUTF8(&replText, str_v, -1, &status);
    REGEX_VERBOSE_TEXT(&replText);
    result = matcher2->replaceFirst(&replText, NULL, status);
    REGEX_CHECK_STATUS;
    const char str_Thevalueof1isbcdefg[] = { 0x54, 0x68, 0x65, 0x20, 0x76, 0x61, 0x6c, 0x75, 0x65, 0x20, 0x6f, 0x66, 0x20, 0x24, 0x31, 0x20, 0x69, 0x73, 0x20,
        0x62, 0x63, 0x2e, 0x64, 0x65, 0x66, 0x67, 0x00 }; /* The value of $1 is bc.defg */
    ...
    const char str_byitselfnogroupnumber[] = { 0x5c, 0x24, 0x20, 0x62, 0x79, 0x20, 0x69, 0x74, 0x73, 0x65, 0x6c, 0x66, 0x2c, 0x20, 0x6e, 0x6f, 0x20, 0x67, 0x72,
        0x6f, 0x75, 0x70, 0x20, 0x6e, 0x75, 0x6d, 0x62, 0x65, 0x72, 0x20, 0x5c, 0x24, 0x5c, 0x24, 0x5c, 0x24, 0x00 }; /* \$ by itself, no group number \$\$\$ */
    utext_openUTF8(&replText, str_byitselfnogroupnumber, -1, &status);
    result = matcher2->replaceFirst(&replText, NULL, status);
    REGEX_CHECK_STATUS;
    const char str_byitselfnogroupnumberdefg[]
        = { 0x24, 0x20, 0x62, 0x79, 0x20, 0x69, 0x74, 0x73, 0x65, 0x6c, 0x66, 0x2c, 0x20, 0x6e, 0x6f, 0x20, 0x67, 0x72, 0x6f, 0x75, 0x70, 0x20, 0x6e, 0x75,
              0x6d, 0x62, 0x65, 0x72, 0x20, 0x24, 0x24, 0x24, 0x64, 0x65, 0x66, 0x67, 0x00 }; /* $ by itself, no group number $$$defg */
    ...
    unsigned char supplDigitChars[] = { 0x53, 0x75, 0x70, 0x70, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x74, 0x61, 0x6c, 0x20, 0x44, 0x69, 0x67, 0x69, 0x74, 0x20, 0x31,
    ...
    result = matcher2->replaceFirst(&replText, NULL, status);
    REGEX_CHECK_STATUS;
    const char str_SupplementalDigit1bcdefg[] = { 0x53, 0x75, 0x70, 0x70, 0x6c, 0x65, 0x6d, 0x65, 0x6e, 0x74, 0x61, 0x6c, 0x20, 0x44, 0x69, 0x67, 0x69, 0x74,
        0x20, 0x31, 0x20, 0x62, 0x63, 0x2e, 0x64, 0x65, 0x66, 0x67, 0x00 }; /* Supplemental Digit 1 bc.defg */
    ...
    REGEX_ASSERT(result == &destText);
    REGEX_ASSERT_UTEXT_UTF8(str_SupplementalDigit1bcdefg, result);
    const char str_badcapturegroupnumber5[] = { 0x62, 0x61, 0x64, 0x20, 0x63, 0x61, 0x70, 0x74, 0x75, 0x72, 0x65, 0x20, 0x67, 0x72, 0x6f, 0x75, 0x70, 0x20,
        0x6e, 0x75, 0x6d, 0x62, 0x65, 0x72, 0x20, 0x24, 0x35, 0x2e, 0x2e, 0x2e, 0x00 }; /* bad capture group number $5..." */
    ...
    //
    {
        ...
        result = matcher->replaceAll(&replText, NULL, status);
        REGEX_CHECK_STATUS;
        const char str_C1C2C3[] = { 0x2d, 0x2d, 0x43, 0x2d, 0x2d, 0x20, 0x31, 0x20, 0x2d, 0x2d, 0x43, 0x2d, 0x2d, 0x20, 0x32, 0x20, 0x2d, 0x2d, 0x43, 0x2d,
            0x2d, 0x20, 0x33, 0x00 }; /* --C-- 1 --C-- 2 --C-- 3 */
    ...
    }
    {
        ...
        unsigned char expected[] = { 0x2d, 0x2d, 0x78, 0x78, 0x78, 0x78, 0x2d, 0x2d, 0x20, 0x21, 0x00 };
            /* --xxxx-- ! */ // \U00010000, "LINEAR B SYLLABLE B008 A"
        //                          0123456789
        expected[2] = 0xF0;
        expected[3] = 0x90;
        expected[4] = 0x80;
    ...
    //
    {
        ...
        const char str_blah[] = { 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77, 0x69, 0x74,
            0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x20, 0x65, 0x65, 0x20, 0x73, 0x73, 0x20,
            0x73, 0x74, 0x75, 0x66, 0x66, 0x20, 0x65, 0x65, 0x20, 0x66, 0x69, 0x6e, 0x00 }; /* The matches start with ss and end with ee ss stuff ee fin */
        ...
        REGEX_CHECK_STATUS;
        const char str_blah2[] = { 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77, 0x69, 0x74,
            0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68,
            0x00 }; /* The matches start with ss and end with ee ooh */
        ...
        m.appendReplacement(&resultText, &replText, status);
        REGEX_CHECK_STATUS;
        const char str_blah3[] = { 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77, 0x69, 0x74,
            0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68,
            0x00 }; /* The matches start with ss and end with ee ooh */
        ...
        m.appendReplacement(&resultText, &replText, status);
        REGEX_CHECK_STATUS;
        const char str_blah8[] = { 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77, 0x69, 0x74,
            0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68,
            0x00 }; /* The matches start with ss and end with ee ooh */
        REGEX_ASSERT_UTEXT_UTF8(str_blah8, &resultText);

        m.appendTail(&resultText, status);
        const char str_blah9[] = { 0x54, 0x68, 0x65, 0x20, 0x6d, 0x61, 0x74, 0x63, 0x68, 0x65, 0x73, 0x20, 0x73, 0x74, 0x61, 0x72, 0x74, 0x20, 0x77, 0x69, 0x74,
            0x68, 0x20, 0x73, 0x73, 0x20, 0x61, 0x6e, 0x64, 0x20, 0x65, 0x6e, 0x64, 0x20, 0x77, 0x69, 0x74, 0x68, 0x20, 0x65, 0x65, 0x20, 0x6f, 0x6f, 0x68,
            0x20, 0x66, 0x69, 0x6e, 0x00 }; /* The matches start with ss and end with ee ooh fin */
...
void RegexTest::regex_find(const UnicodeString& pattern, const UnicodeString& flags, const UnicodeString& inputString, const char* srcPath, int32_t line)
{
    ...
    if (UTF8Matcher != NULL) {
        ...
        if (regionStart >= 0)
            (void)utextOffsetToNative(&inputText, regionStart, regionStartUTF8);
        if (regionEnd >= 0)
            (void)utextOffsetToNative(&inputText, regionEnd, regionEndUTF8);
    ...
    for (i = 0; i <= matcher->groupCount(); i++) {
        ...
        if (matcher->start(i, status) != expectedStart) {
            errln("Error at line %d: incorrect start position for group %d.  Expected %d, got %d", line, i, expectedStart, matcher->start(i, status));
            failed = TRUE;
            goto cleanupAndReturn; // Good chance of subsequent bogus errors.  Stop now.
        } else if (UTF8Matcher != NULL && UTF8Matcher->start(i, status) != expectedStartUTF8) {
            errln("Error at line %d: incorrect start position for group %d.  Expected %d, got %d (UTF8)", line, i, expectedStartUTF8,
                UTF8Matcher->start(i, status));
            failed = TRUE;
            goto cleanupAndReturn; // Good chance of subsequent bogus errors.  Stop now.
        }
        ...
        if (matcher->end(i, status) != expectedEnd) {
            errln("Error at line %d: incorrect end position for group %d.  Expected %d, got %d", line, i, expectedEnd, matcher->end(i, status));
            failed = TRUE;
            ...
            errln("Error at line %d: incorrect end position for group %d.  Expected %d, got %d (UTF8)", line, i, expectedEndUTF8, UTF8Matcher->end(i, status));
            failed = TRUE;
            // Error on end position;  keep going; real error is probably yet to come as group
            //   end positions work from end of the input data towards the front.
        }
    ...
    if (matcher->groupCount() + 1 < groupStarts.size()) {
        errln("Error at line %d: Expected %d capture groups, found %d.", line, groupStarts.size() - 1, matcher->groupCount());
        failed = TRUE;
    } else if (UTF8Matcher != NULL && UTF8Matcher->groupCount() + 1 < groupStarts.size()) {
        errln("Error at line %d: Expected %d capture groups, found %d. (UTF8)", line, groupStarts.size() - 1, UTF8Matcher->groupCount());
        failed = TRUE;
    }
...
cleanupAndReturn:
    if (failed) {
        infoln((UnicodeString) "\"" + pattern + (UnicodeString) "\"  " + flags + (UnicodeString) "  \"" + inputString + (UnicodeString) "\"");
        // callerPattern->dump();
    }
    delete parseMatcher;
    delete parsePat;
...
void RegexTest::Errors()
{
    ...
    // Attempt to use non-default flags
    {
        UParseError pe;
        UErrorCode status = U_ZERO_ERROR;
        int32_t flags = UREGEX_CANON_EQ | UREGEX_COMMENTS | UREGEX_DOTALL | UREGEX_MULTILINE;
...
void RegexTest::PerlTests()
{
    ...
    while (lineMat->find()) {
        ...
        if (expected != found) {
            errln("line %d: Expected %smatch, got %smatch", lineNum, expected ? "" : "no ", found ? "" : "no ");
            delete testMat;
            delete testPat;
            continue;
        }
...
U_CDECL_BEGIN
static UBool U_CALLCONV testCallBackFn(const void* context, int32_t steps)
{
...
U_CDECL_BEGIN
static UBool U_CALLCONV testProgressCallBackFn(const void* context, int64_t matchIndex)
{
...
{
    UErrorCode status = U_ZERO_ERROR;
    RegexPattern* pat = RegexPattern::compile(UnicodeString("abc()()(?<three>xyz)(de)(?<five>hmm)(?<six>oh)f\\k<five>"), 0, status);
    REGEX_CHECK_STATUS;
    ...
    delete pat;
    pat = NULL; // Delete original, copy should have no references back to it.
...
void RegexTest::Bug7651()
{
    UnicodeString pattern1(
        "((?<![A-Za-z0-9])[#\\uff03][A-Za-z0-9_][A-Za-z0-9_\\u00c0-\\u00d6\\u00c8-\\u00f6\\u00f8-\\u00ff]*|(?<![A-Za-z0-9_])[@\\uff20][A-Za-z0-9_]+(?:\\/"
        "[\\w-]+)?|(https?\\:\\/\\/|www\\.)\\S+(?<![\\!\\),\\.:;\\]\\u0080-\\uFFFF])|\\$[A-Za-z]+)");
    ...
    UnicodeString pattern2("((https?\\:\\/\\/|www\\.)\\S+(?<![\\!\\),\\.:;\\]\\u0080-\\uFFFF])|(?<![A-Za-z0-9_])[\\@\\uff20][A-Za-z0-9_]+(?:\\/"
                           "[\\w\\-]+)?|(?<![A-Za-z0-9])[\\#\\uff03][A-Za-z0-9_][A-Za-z0-9_\\u00c0-\\u00d6\\u00c8-\\u00f6\\u00f8-\\u00ff]*|\\$[A-Za-z]+)");
...
void RegexTest::TestCase11049(const char* pattern, const char* data, UBool expectMatch, int32_t lineNumber)
{
    ...
    if (result != expectMatch) {
        errln("File %s, line %d: expected %d, got %d. Pattern = \"%s\", text = \"%s\"", __FILE__, lineNumber, expectMatch, result, pattern, data);
    }
    ...
    result = matcher->find();
    if (result != expectMatch) {
        errln("File %s, line %d (UTF-8 check): expected %d, got %d. Pattern = \"%s\", text = \"%s\"", __FILE__, lineNumber, expectMatch, result, pattern, data);
    }
    delete[] utf8Buffer;
...
void RegexTest::TestBug11371()
{
    ...
    if (status != U_REGEX_PATTERN_TOO_BIG) {
        errln("File %s, line %d expected status=U_REGEX_PATTERN_TOO_BIG; got %s.", __FILE__, __LINE__, u_errorName(status));
    }
...
void RegexTest::TestBug20359()
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    LocalURegularExpressionPointer re(uregex_open(pattern.getBuffer(), pattern.length(), 0, nullptr, &status));
