...
void DateFormatRegressionTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
...
void DateFormatRegressionTest::Test4052408(void)
{
    ...
    DateFormat* fmt = DateFormat::createDateTimeInstance(DateFormat::SHORT, DateFormat::SHORT, Locale::getUS());
    if (fmt == NULL) {
        dataerrln("Error calling DateFormat::createDateTimeInstance");
        return;
    }
    ...
    UnicodeString fieldNames[] = { (UnicodeString) "ERA_FIELD", (UnicodeString) "YEAR_FIELD", (UnicodeString) "MONTH_FIELD", (UnicodeString) "DATE_FIELD",
        (UnicodeString) "HOUR_OF_DAY1_FIELD", (UnicodeString) "HOUR_OF_DAY0_FIELD", (UnicodeString) "MINUTE_FIELD", (UnicodeString) "SECOND_FIELD",
        (UnicodeString) "MILLISECOND_FIELD", (UnicodeString) "DAY_OF_WEEK_FIELD", (UnicodeString) "DAY_OF_YEAR_FIELD",
        (UnicodeString) "DAY_OF_WEEK_IN_MONTH_FIELD", (UnicodeString) "WEEK_OF_YEAR_FIELD", (UnicodeString) "WEEK_OF_MONTH_FIELD",
        (UnicodeString) "AM_PM_FIELD", (UnicodeString) "HOUR1_FIELD", (UnicodeString) "HOUR0_FIELD", (UnicodeString) "TIMEZONE_FIELD" };
    ...
    for (int i = 0; i <= 17; ++i) {
        ...
        logln((UnicodeString) "" + i + (UnicodeString) ": " + fieldNames[i] + (UnicodeString) ", \"" + str + (UnicodeString) "\", " + pos.getBeginIndex()
            + (UnicodeString) ", " + pos.getEndIndex());
...
void DateFormatRegressionTest::Test4056591(void)
{
    ...
    UnicodeString strings[] = { (UnicodeString) "091225", (UnicodeString) "091224", (UnicodeString) "091226", (UnicodeString) "611225" };
...
void DateFormatRegressionTest::Test4060212(void)
{
    ...
    LocalPointer<DateFormat> fmt(DateFormat::createDateTimeInstance(DateFormat::FULL, DateFormat::LONG));
    if (!fmt.isValid()) {
        dataerrln("Error calling DateFormat::createDateTimeInstance");
        return;
    }
...
void DateFormatRegressionTest::Test4065240(void)
{
    ...
    ResourceBundle* bundle = new ResourceBundle(NULL, *curLocale, status);
...
void DateFormatRegressionTest::Test4073003(void)
{
    ...
    UnicodeString tests[] = { (UnicodeString) "12/25/61", (UnicodeString) "12/25/1961", (UnicodeString) "4/3/2010", (UnicodeString) "4/3/10" };
    UErrorCode status = U_ZERO_ERROR;
    for (int i = 0; i < 4; i += 2) {
...
void DateFormatRegressionTest::Test4104136(void)
{
    ...
    UnicodeString strings[] = { (UnicodeString) "time 10:30", (UnicodeString) "time 10:x", (UnicodeString) "time 10x" };

    ParsePosition ppos[] = { ParsePosition(10), ParsePosition(0), ParsePosition(0) };

    UDate dates[] = { date(70, UCAL_JANUARY, 1, 10, 30), -1, -1 };
...
void DateFormatRegressionTest::Test4106807(void)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    SimpleDateFormat* sdfs[] = { new SimpleDateFormat(UnicodeString("yyyyMMddHHmmss"), status),
        new SimpleDateFormat(UnicodeString("yyyyMMddHHmmss'Z'"), status), new SimpleDateFormat(UnicodeString("yyyyMMddHHmmss''"), status),
        new SimpleDateFormat(UnicodeString("yyyyMMddHHmmss'a''a'"), status), new SimpleDateFormat(UnicodeString("yyyyMMddHHmmss %"), status) };
    if (U_FAILURE(status)) {
        dataerrln("Couldn't create SimpleDateFormat, error %s", u_errorName(status));
        delete sdfs[0];
        delete sdfs[1];
    ...
    UnicodeString strings[] = { (UnicodeString) "19980211140000", (UnicodeString) "19980211140000", (UnicodeString) "19980211140000",
        (UnicodeString) "19980211140000a", (UnicodeString) "19980211140000 " };
 ...
 */
void DateFormatRegressionTest::Test4210209(void)
{
    UErrorCode status = U_ZERO_ERROR;
...
void DateFormatRegressionTest::Test714(void)
{
    ...
    UnicodeString s;
    UnicodeString tests = (UnicodeString) "7:25:43 AM";
    UErrorCode status = U_ZERO_ERROR;
    fmt->format(d, s);
    if (U_FAILURE(status)) {
...
public:
    ...
    Test1684Data(
        int32_t xyear, int32_t xmonth, int32_t xdate, int32_t xwomyear, int32_t xwommon, int32_t xwom, int32_t xdow, const char* xdata, const char* xnormalized)
        : year(xyear)
        , month(xmonth - 1)
        , date(xdate)
        , womyear(xwomyear)
...
void DateFormatRegressionTest::Test1684(void)
{
    ...
    Test1684Data* tests[] = { new Test1684Data(2001, 8, 6, 2001, 8, 2, UCAL_MONDAY, "2001 08 02 Mon", NULL),
...
#define kTest1684Count UPRV_LENGTHOF(tests)
    ...
    for (i = 0; i < kTest1684Count; ++i) {
        ...
        logln(UnicodeString("#") + i + UnicodeString("\n-----\nTesting round trip of ") + test.year + " " + (test.month + 1) + " " + test.date
    ...
    UnicodeString info = UnicodeString("Passed: ") + pass + ", Warnings: " + warning + ", Errors: " + error;
    if (error > 0) {
        errln(info);
    } else {
        logln(info);
...
void DateFormatRegressionTest::TestT10855(void)
{
    ...
    const T10855Data items[] = { // parse String                          pattern                         initial parse pos
        { UnicodeString("September 30, 1998"), UnicodeString("MM-dd-yyyy"), 0 }, { UnicodeString("123-73-1950"), UnicodeString("MM-dd-yyyy"), -1 },
...
void DateFormatRegressionTest::TestT10858(void)
{
    ...
    UnicodeString res_rhs;
    fmt_rhs.format(0.0, res_rhs);
    UnicodeString res_lhs_pre_assign;
    fmt_lhs.format(0.0, res_lhs_pre_assign);
    ...
    fmt_lhs = fmt_rhs;
    UnicodeString res_lhs_post_assign;
    fmt_lhs.format(0.0, res_lhs_post_assign);
