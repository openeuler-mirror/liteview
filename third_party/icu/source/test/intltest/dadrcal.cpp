...
void DataDrivenCalendarTest::testOps(TestData* testData, const DataMap* /*settings*/)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (U_FAILURE(status)) {
        dataerrln("FAIL: Couldn't create SimpleDateFormat: %s", u_errorName(status));
        return;
    }
    ...
    while (testData->nextCase(currentCase, status)) {
        ...
        if (U_FAILURE(status)) {
            errln(caseString + ": Unable to get param '" + param + "' " + UnicodeString(" - "));
            continue;
        }
        testSetting.extract(0, testSetting.length(), toCalLoc, (const char*)0);
        fromCalendar = Calendar::createInstance(toCalLoc, status);
        if (U_FAILURE(status)) {
            errln(caseString + ": Unable to instantiate calendar for " + testSetting);
            continue;
        }
        ...
        if (U_FAILURE(status)) {
            errln(caseString + ": Unable to get parameter '" + param + "' " + UnicodeString(" - "));
            continue;
        }
        ...
        if (from.startsWith(kMILLIS)) {
            ...
            errln(caseString + ": Failed to parse '" + param + "' parameter: " + testSetting);
            continue;
        }

        // #4 'operation' info
        param = "operation";
        UnicodeString operation = testSetting = currentCase->getString(param, status);
        if (U_FAILURE(status)) {
            errln(caseString + ": Unable to get parameter '" + param + "' " + UnicodeString(" - "));
            continue;
        }
        ...
        // #3 'params' info
        param = "params";
        UnicodeString params = testSetting = currentCase->getString(param, status);
        if (U_FAILURE(status)) {
            errln(caseString + ": Unable to get parameter '" + param + "' " + UnicodeString(" - "));
            continue;
        }
        ...
        UnicodeString caseContentsString = locale + ":  from " + from + ": " + operation + " [[[ " + params + " ]]]   >>> " + to;
        ...
        if (useDate) {
            ...
            if (U_FAILURE(status)) {
                errln(caseString + " FAIL: Failed to set time on Source calendar: " + u_errorName(status));
                return;
            }
        } else {
            fromSet.setOnCalendar(fromCalendar, status);
            if (U_FAILURE(status)) {
                errln(caseString + " FAIL: Failed to set on Source calendar: " + u_errorName(status));
                return;
            }
        }
        ...
        if (!fromSet.matches(fromCalendar, diffSet, status)) {
            ...
            errln((UnicodeString) "FAIL: " + caseString + ", SET SOURCE calendar was not set: Differences: " + diffs + "', status: " + u_errorName(status));
        } else if (U_FAILURE(status)) {
            errln("FAIL: " + caseString + " SET SOURCE calendar Failed to match: " + u_errorName(status));
        } else {
        ...
        for (int q = 0; q < UCAL_FIELD_COUNT; q++) {
            if (paramsSet.isSet((UCalendarDateFields)q)) {
                if (operation == kROLL) {
                    ...
                    toCalendar->add((UCalendarDateFields)q, paramsSet.get((UCalendarDateFields)q), status);
                } else {
                    errln(caseString + " FAIL: unknown operation " + operation);
                }
                logln(operation + " of " + paramsSet.get((UCalendarDateFields)q) + " -> " + u_errorName(status));
            }
        }
        if (U_FAILURE(status)) {
            errln(caseString + " FAIL: after " + operation + " of " + params + " -> " + u_errorName(status));
            continue;
        }
        ...
        if (useDate) {
            if (!(toCalendar->getTime(status) == toDate) || U_FAILURE(status)) {
                errln("FAIL: " + caseString + " Match operation had an error: " + u_errorName(status));
            } else {
            ...
            errln((UnicodeString) "FAIL: " + caseString + " - , " + caseContentsString + " Differences: " + diffs + "', status: " + u_errorName(status));
        } else if (U_FAILURE(status)) {
            errln("FAIL: " + caseString + " Match operation had an error: " + u_errorName(status));
        } else {
            logln("PASS: " + caseString + " matched!");
...
void DataDrivenCalendarTest::testConvert(
    int32_t n, const CalendarFieldsSet& fromSet, Calendar* fromCalendar, const CalendarFieldsSet& toSet, Calendar* toCalendar, UBool forward)
{
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString thisString = (UnicodeString) "#" + n + " " + (forward ? "forward" : "reverse") + " " + fromCalendar->getType() + "->" + toCalendar->getType();
...
void DataDrivenCalendarTest::testConvert(TestData* testData, const DataMap* settings, UBool forward)
{
    ...
    SimpleDateFormat fmt(UnicodeString("EEE MMM dd yyyy / YYYY'-W'ww-ee"), status);
    if (U_FAILURE(status)) {
        errcheckln(status, "FAIL: Couldn't create SimpleDateFormat: %s", u_errorName(status));
        return;
    }
    // Start the processing
    int n = 0;
