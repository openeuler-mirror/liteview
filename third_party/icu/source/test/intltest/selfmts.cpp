...
#define VERBOSE_INT(x)                                                                                                                                         \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
        logln("%s:%d:  int %s=%d\n", __FILE__, __LINE__, #x, (x));                                                                                             \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END
#define VERBOSE_USTRING(text)                                                                                                                                  \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
...
void SelectFormatTest::selectFormatUnitTest(/*char *par*/)
{
    ...
    UnicodeString formatArgs[NUM_OF_FORMAT_ARGS] = { UNICODE_STRING_SIMPLE("fem"), UNICODE_STRING_SIMPLE("other"), UNICODE_STRING_SIMPLE("odd") };

    UnicodeString expFormatResult[][NUM_OF_FORMAT_ARGS] = { { UNICODE_STRING_SIMPLE("femValue"), UNICODE_STRING_SIMPLE("even"), UNICODE_STRING_SIMPLE("even") },
        {
    ...
    UnicodeString checkSyntaxData[SELECT_SYNTAX_DATA] = { UNICODE_STRING_SIMPLE("odd{foo}"), UNICODE_STRING_SIMPLE("*odd{foo} other{bar}"),
        UNICODE_STRING_SIMPLE("odd{foo},other{bar}"), UNICODE_STRING_SIMPLE("od d{foo} other{bar}"), UNICODE_STRING_SIMPLE("odd{foo}{foobar}other{foo}"),
        UNICODE_STRING_SIMPLE("odd{foo1}other{foo2}}"), UNICODE_STRING_SIMPLE("odd{foo1}other{{foo2}"), UNICODE_STRING_SIMPLE("odd{fo{o1}other{foo2}}") };
    ...
    status = U_ZERO_ERROR;
    result.remove();
    UnicodeString keywords[] = { "9Keyword-_", "-Keyword-_", "_Keyword-_", "\\u00E9Keyword-_", "Key word-_", " Keyword-_", "Key*word-_", "*Keyword-_" };
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(keywords); i++) {
        ...
        if (!U_FAILURE(status)) {
            errln("ERROR: SelectFormat Unit test failed in format() with keyWord and with an invalid keyword as : " + keywords[i] + " (" + u_errorName(status)
                + ")");
        }
    }
