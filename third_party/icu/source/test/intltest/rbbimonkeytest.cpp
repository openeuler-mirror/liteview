    ...
    , fType(UBRK_COUNT)
{
    ...
    fSetRefsMatcher.adoptInstead(new RegexMatcher(UnicodeString("(?!(?:\\{|=|\\[:)[ \\t]{0,4})" // Negative look behind for '{' or '=' or '[:'
    ...
    fCommentsMatcher.adoptInstead(new RegexMatcher(UnicodeString("(^|(?<=;))" // Start either at start of line, or just after a ';' (look-behind for ';')
                                                                 "[ \\t]*+" //   Match white space.
    ...
    fClassDefMatcher.adoptInstead(new RegexMatcher(UnicodeString("[ \\t]*" // leading white space
                                                                 "(?<ClassName>[A-Za-z_][A-Za-z0-9_]*)" // The char class name
                                                                 "[ \\t]*=[ \\t]*" //   =
    ...
    fRuleDefMatcher.adoptInstead(new RegexMatcher(UnicodeString("[ \\t]*" // leading white space
...
CharClass* BreakRules::addCharClass(const UnicodeString& name, const UnicodeString& definition, UErrorCode& status)
{
    ...
    while (fSetRefsMatcher->find() && U_SUCCESS(status)) {
        const UnicodeString name = fSetRefsMatcher->group(fSetRefsMatcher->pattern().groupNumberFromName("ClassName", status), status);
    ...
    if (U_FAILURE(status)) {
        IntlTest::gTest->errln("%s:%d: error %s creating UnicodeSet %s\n    Expanded set definition: %s", __FILE__, __LINE__, u_errorName(status), CStr(name)(),
            CStr(expandedDef)());
        return nullptr;
    }
...
void BreakRules::addRule(const UnicodeString& name, const UnicodeString& definition, UErrorCode& status)
{
    ...
    while (fSetRefsMatcher->find() && U_SUCCESS(status)) {
        ...
        CharClass* nameClass = static_cast<CharClass*>(uhash_get(fCharClasses.getAlias(), &name));
        if (!nameClass) {
            IntlTest::gTest->errln("%s:%d char class \"%s\" unrecognized in rule \"%s\"", __FILE__, __LINE__, CStr(name)(), CStr(definition)());
        }
        const UnicodeString& expansionForName = nameClass ? nameClass->fExpandedDef : name;
    ...
    if (U_FAILURE(status)) {
        IntlTest::gTest->errln("%s:%d Error creating regular expression for %s", __FILE__, __LINE__, CStr(thisRule->fExpandedRule)());
        return;
    }
...
void MonkeyTestData::set(BreakRules* rules, IntlTest::icu_rand& rand, UErrorCode& status)
{
    ...
    while (strIdx < fString.length()) {
        ...
        if (matchingRule == NULL) {
            ...
            IntlTest::gTest->errln("%s:%d Trouble with monkey test reference rules at position %d. ", __FILE__, __LINE__, strIdx);
            dump(strIdx);
        ...
        if (matchingRule->fRuleMatcher->group(status).length() == 0) {
            ...
            IntlTest::gTest->errln("%s:%d Zero length rule match.", __FILE__, __LINE__);
            status = U_INVALID_FORMAT_ERROR;
            return;
        }
        ...
        if (hasBreak) {
            ...
            if (strIdx == matchStart) {
                ...
                IntlTest::gTest->errln("%s:%d Rule %s internal error", __FILE__, __LINE__, ruleName.data());
                status = U_INVALID_FORMAT_ERROR;
                break;
            }
        }
        if (U_FAILURE(status)) {
            IntlTest::gTest->errln("%s:%d status = %s. Unexpected failure, perhaps problem internal to test.", __FILE__, __LINE__, u_errorName(status));
            break;
        }
    }
}
...
void MonkeyTestData::dump(int32_t around) const
{
    ...
    for (int charIdx = start; charIdx < end; charIdx = fString.moveIndex32(charIdx, 1)) {
        ...
        printf("  %4.1d %6.4x   %-20s  %c %c   %-10s %-10s    %s\n", charIdx, c, ccName.data(), fExpectedBreaks.charAt(charIdx) ? '*' : '.',
            fActualBreaks.charAt(charIdx) ? '*' : '.', ruleName.data(), secondRuleName.data(), cName);
    }
}
...
RBBIMonkeyImpl::RBBIMonkeyImpl(UErrorCode& status)
...
#define MONKEY_ERROR(msg, index)                                                                                                                               \
    ...
    {                                                                                                                                                          \
        IntlTest::gTest->errln("%s:%d %s at index %d. Parameters to reproduce: @rules=%s,seed=%u,loop=1,verbose ", __FILE__, __LINE__, msg, index,             \
            fRuleFileName, fTestData->fRandomSeed);                                                                                                            \
        if (fVerbose) {                                                                                                                                        \
            fTestData->dump(index);                                                                                                                            \
        }                                                                                                                                                      \
...
void RBBIMonkeyImpl::testIsBoundaryRandom(UErrorCode& status)
{
    ...
    for (int i = stringLen; i >= 0; --i) {
        ...
        if (fTestData->fExpectedBreaks.charAt(strIdx) != fBI->isBoundary(strIdx)) {
            IntlTest::gTest->errln("%s:%d testIsBoundaryRandom failure at index %d. Parameters to reproduce: @rules=%s,seed=%u,loop=1,verbose ", __FILE__,
                __LINE__, strIdx, fRuleFileName, fTestData->fRandomSeed);
            if (fVerbose) {
                fTestData->dump(i);
            }
            status = U_INVALID_STATE_ERROR;
    ...
    if (direction == FORWARD) {
        for (int i = 0; i <= fTestData->fString.length(); ++i) {
            if (fTestData->fExpectedBreaks.charAt(i) != fTestData->fActualBreaks.charAt(i)) {
                IntlTest::gTest->errln("%s:%d %s failure at index %d. Parameters to reproduce: @rules=%s,seed=%u,loop=1,verbose ", __FILE__, __LINE__, msg, i,
                    fRuleFileName, fTestData->fRandomSeed);
                if (fVerbose) {
                    fTestData->dump(i);
                }
...
void RBBIMonkeyTest::testMonkey()
{
    ...
    const char* tests[] = { "grapheme.txt", "word.txt", "line.txt", "line_cj.txt", "sentence.txt", "line_normal.txt", "line_normal_cj.txt", "line_loose.txt",
        "line_loose_cj.txt", "word_POSIX.txt", NULL };
