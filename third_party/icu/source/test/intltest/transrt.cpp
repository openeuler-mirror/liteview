...
#define EXHAUSTIVE(id, test)                                                                                                                                   \
    case id:                                                                                                                                                   \
        ...
        break
void TransliteratorRoundTripTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    switch (index) {
        CASE(0, TestCyrillic);
        // CASE(0,TestKana);
...
public:
    ...
    inline operator const Transliterator*() const
    {
        return t;
    }
...
UBool LegalGreek::is(const UnicodeString& sourceString) const
{
    ...
    if (full == FALSE) {
        ...
        for (int32_t i = 0; i < decomp.length(); ++i) {
            ...
            if (c == 0x0313 || c == 0x0314 || c == 0x0300 || c == 0x0302 || c == 0x0342 || c == 0x0345)
                return FALSE;
        }
        return TRUE;
    }
 ...
 */
const UnicodeSet& abbreviateSet(const UnicodeSet& set, bool abbreviated, int density, UnicodeSet& copy)
{
    if (!abbreviated) {
        return set;
    }
...
public:
    ...
    void test(const UnicodeString& sourceRange, const UnicodeString& targetRange, const char* roundtripExclusions, IntlTest* parent, UBool quick,
        Legal* adoptedLegal, int32_t density = 100);
...
private:
    ...
    void logWrongScript(const UnicodeString& label, const UnicodeString& from, const UnicodeString& to);

    void logNotCanonical(
        const UnicodeString& label, const UnicodeString& from, const UnicodeString& to, const UnicodeString& fromCan, const UnicodeString& toCan);

    void logFails(const UnicodeString& label);

    void logToRulesFails(const UnicodeString& label, const UnicodeString& from, const UnicodeString& to, const UnicodeString& toCan);

    void logRoundTripFailure(
        const UnicodeString& from, const UnicodeString& toID, const UnicodeString& to, const UnicodeString& backID, const UnicodeString& back);
};
...
UBool RTTest::isSame(const UnicodeString& a, const UnicodeString& b)
{
    if (a == b)
        return TRUE;
    if (a.caseCompare(b, U_FOLD_CASE_DEFAULT) == 0 && isCamel(a))
        return TRUE;
    UnicodeString aa, bb;
    UErrorCode ec = U_ZERO_ERROR;
    ...
    if (aa == bb)
        return TRUE;
    if (aa.caseCompare(bb, U_FOLD_CASE_DEFAULT) == 0 && isCamel(aa))
        return TRUE;
    return FALSE;
}
...
void RTTest::test(const UnicodeString& sourceRangeVal, const UnicodeString& targetRangeVal, const char* roundtripExclusions, IntlTest* logVal, UBool quickRt,
    Legal* adoptedLegal, int32_t density)
{
    ...
    if (U_FAILURE(status)) {
        parent->errln("FAIL: UnicodeSet::applyPattern(" + sourceRangeVal + ")");
        return;
    }
    ...
    if (U_FAILURE(status)) {
        parent->errln("FAIL: UnicodeSet::applyPattern(" + targetRangeVal + ")");
        return;
    }
...
UBool RTTest::checkIrrelevants(Transliterator* t, const UnicodeString& irrelevants)
{
...
void RTTest::test2(UBool quickRt, int32_t density)
{
    ...
    TransliteratorPointer sourceToTarget(Transliterator::createInstance(transliteratorID, UTRANS_FORWARD, parseError, status));
    if ((Transliterator*)sourceToTarget == NULL) {
        parent->dataerrln("FAIL: createInstance(" + transliteratorID + ") returned NULL. Error: " + u_errorName(status)
    ...
    if ((Transliterator*)targetToSource == NULL) {
        parent->errln("FAIL: " + transliteratorID + ".createInverse() returned NULL. Error:" + u_errorName(status)
            + "\n\tpreContext : " + prettify(parseError.preContext) + "\n\tpostContext : " + prettify(parseError.postContext));
        return;
    }
    ...
    if (!quickRt) {
        ...
        TransliteratorPointer sourceToTarget2(Transliterator::createFromRules("s2t2", rules, UTRANS_FORWARD, parseError, status));
        if (U_FAILURE(status)) {
        ...
        TransliteratorPointer targetToSource2(Transliterator::createFromRules("t2s2", rules, UTRANS_FORWARD, parseError, status));
        if (U_FAILURE(status)) {
            parent->errln("FAIL: createFromRules %s\n", u_errorName(status));
            return;
        }
    ...
    for (;;) {
        ...
        UnicodeString targ = srcStr;
        sourceToTarget->transliterate(targ);
        if (toTarget.containsAll(targ) == FALSE || badCharacters.containsSome(targ) == TRUE) {
            UnicodeString targD;
            ...
            if (toTarget.containsAll(targD) == FALSE || badCharacters.containsSome(targD) == TRUE) {
    ...
    for (;;) {
        ...
        if (toSource.containsAll(targ) == FALSE || badCharacters.containsSome(targ) == TRUE) {
            UnicodeString targD;
            Normalizer::decompose(targ, FALSE, 0, targD, status);
            if (U_FAILURE(status)) {
                parent->errln("FAIL: Internal error during decomposition%s\n", u_errorName(status));
        ...
        if (isSame(srcStr, reverse) == FALSE && roundtripExclusionsSet.contains(c) == FALSE && roundtripExclusionsSet.contains(srcStr) == FALSE) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (toSource.containsAll(targ) == FALSE || badCharacters.containsSome(targ) == TRUE) {
                ...
                if (toSource.containsAll(targD) == FALSE || badCharacters.containsSome(targD) == TRUE) {
                    logWrongScript("Target-Source", srcStr, targ);
                    continue;
                }
            }
            if (isSame(srcStr, reverse) == FALSE && roundtripExclusionsSet.contains(c) == FALSE && roundtripExclusionsSet.contains(d) == FALSE
...
void RTTest::logWrongScript(const UnicodeString& label, const UnicodeString& from, const UnicodeString& to)
{
    parent->errln((UnicodeString) "FAIL " + label + ": " + from + "(" + TestUtility::hex(from) + ") => " + to + "(" + TestUtility::hex(to) + ")");
    ++errorCount;
}

void RTTest::logNotCanonical(
    const UnicodeString& label, const UnicodeString& from, const UnicodeString& to, const UnicodeString& fromCan, const UnicodeString& toCan)
{
    parent->errln((UnicodeString) "FAIL (can.equiv)" + label + ": " + from + "(" + TestUtility::hex(from) + ") => " + to + "(" + TestUtility::hex(to) + ")"
        + fromCan + "(" + TestUtility::hex(fromCan) + ") => " + toCan + " (" + TestUtility::hex(toCan) + ")");
    ++errorCount;
}
...
void RTTest::logToRulesFails(const UnicodeString& label, const UnicodeString& from, const UnicodeString& to, const UnicodeString& otherTo)
{
    parent->errln((UnicodeString) "FAIL: " + label + ": " + from + "(" + TestUtility::hex(from) + ") => " + to + "(" + TestUtility::hex(to) + ")"
        + "!=" + otherTo + " (" + TestUtility::hex(otherTo) + ")");
    ++errorCount;
}

void RTTest::logRoundTripFailure(
    const UnicodeString& from, const UnicodeString& toID, const UnicodeString& to, const UnicodeString& backID, const UnicodeString& back)
{
    if (legalSource->is(from) == FALSE)
        return; // skip illegals

    parent->errln((UnicodeString) "FAIL Roundtrip: " + from + "(" + TestUtility::hex(from) + ") => " + to + "(" + TestUtility::hex(to) + ")  " + toID + " => "
...
void TransliteratorRoundTripTest::TestKana()
{
    ...
    strcat(temp, LENGTH);
    strcat(temp, "]");
    test.test(KATAKANA, UnicodeString("[") + HIRAGANA + LENGTH + UnicodeString("]"), temp, this, quick, legal);
    delete legal;
}
...
void TransliteratorRoundTripTest::TestHiragana()
{
    ...
    Legal* legal = new Legal();
    test.test(UnicodeString("[a-zA-Z]", ""), UnicodeString(HIRAGANA, -1, US_INV), HIRAGANA_ITERATION, this, quick, legal);
    delete legal;
}
...
void TransliteratorRoundTripTest::TestKatakana()
{
    ...
    strcat(temp, HALFWIDTH_KATAKANA);
    strcat(temp, "]");
    test.test(UnicodeString("[a-zA-Z]", ""), UnicodeString(KATAKANA, -1, US_INV), temp, this, quick, legal);
    delete legal;
}
...
void TransliteratorRoundTripTest::TestJamo()
{
    ...
    t.test(UnicodeString("[a-zA-Z]", ""), UnicodeString("[\\u1100-\\u1112 \\u1161-\\u1175 \\u11A8-\\u11C2]", ""), NULL, this, quick, legal);
    delete legal;
}
...
void TransliteratorRoundTripTest::TestHangul()
{
    ...
    if (quick)
        t.setPairLimit(1000);
    t.test(UnicodeString("[a-zA-Z]", ""), UnicodeString("[\\uAC00-\\uD7A4]", ""), NULL, this, quick, legal, 1);
    delete legal;
}
...
#define ASSERT_SUCCESS(status)                                                                                                                                 \
    ...
    {                                                                                                                                                          \
        if (U_FAILURE(status)) {                                                                                                                               \
            errcheckln(status, "error at file %s, line %d, status = %s", __FILE__, __LINE__, u_errorName(status));                                             \
            return;                                                                                                                                            \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
...
void TransliteratorRoundTripTest::TestHan()
{
    ...
    if (allHan.containsSome(target)) {
        errln("file %s, line %d, No Han must be left after Han-Latin transliteration", __FILE__, __LINE__);
    }
    ...
    UnicodeSet allMarks(UNICODE_STRING_SIMPLE("[\\u0304\\u0301\\u030C\\u0300\\u0306]"),
        status); // look only for Pinyin tone marks, not all marks (there are some others in there)
    ASSERT_SUCCESS(status);
...
void TransliteratorRoundTripTest::TestHebrew()
{
    ...
    test.test(UNICODE_STRING_SIMPLE("[a-zA-Z\\u02BC\\u02BB]"), UNICODE_STRING_SIMPLE("[[[:hebrew:]-[\\u05BD\\uFB00-\\uFBFF]]&[:Age=4.0:]]"),
        "[\\u05F0\\u05F1\\u05F2]", this, quick, legal);
...
void TransliteratorRoundTripTest::TestCyrillic()
{
    ...
    test.test(UnicodeString("[a-zA-Z\\u0110\\u0111\\u02BA\\u02B9]", ""), UnicodeString("[[\\u0400-\\u045F] & [:Age=3.2:]]", ""), NULL, this, quick, legal);
...
UBool LegalIndic::is(const UnicodeString& sourceString) const
{
    ...
    if (vowelSignSet.contains(cp)) {
        ...
        return FALSE;
    } else if (chandrabindu.contains(cp) && ((sourceString.length() > 1) && vowelSignSet.contains(sourceString.charAt(1)))) {
        return FALSE;
    }
    return TRUE;
}
...
{
    {
        UErrorCode status = U_ZERO_ERROR;
        UParseError parseError;
        TransliteratorPointer t1(
            Transliterator::createInstance("[\\u0964-\\u0965\\u0981-\\u0983\\u0985-\\u098C\\u098F-\\u0990\\u0993-\\u09A8\\u09AA-\\u09B0\\u09B2\\u09B6-"
                                           "\\u09B9\\u09BC\\u09BE-\\u09C4\\u09C7-\\u09C8\\u09CB-"
                                           "\\u09CD\\u09D7\\u09DC-\\u09DD\\u09DF-\\u09E3\\u09E6-\\u09FA];NFD;Bengali-InterIndic;InterIndic-Gujarati;NFC;",
    ...
    test.test(UnicodeString(latinForIndic, ""), UnicodeString("[[[:Devanagari:][\\u094d][\\u0964\\u0965]]&[:Age=4.1:]-[\\u0970]]", ""), "[\\u0965\\u0904]",
        this, quick, legal, 50);
...
static const char* const interIndicArray[] = {
    ...
    "[\\u0904\\u0951-\\u0954\\u0943-"
    "\\u0949\\u094a\\u0962\\u0963\\u090D\\u090e\\u0911\\u0912\\u0929\\u0933\\u0934\\u0935\\u093d\\u0950\\u0958\\u0959\\u095a\\u095b\\u095e\\u097d]", /*roundtrip exclusions*/

    "DEVANAGARI-BENGALI", "[[:Devanagari:]-[\\u0970]]", "[:BENGALI:]",
    "[\\u0951-\\u0954\\u0951-"
    "\\u0954\\u09D7\\u090D\\u090e\\u0911\\u0912\\u0929\\u0933\\u0934\\u0935\\u093d\\u0950\\u0958\\u0959\\u095a\\u095b\\u095e\\u09f0\\u09f1\\u09f2-"
    "\\u09fa\\u09ce]", /*roundtrip exclusions*/

    "GURMUKHI-DEVANAGARI", "[:GURMUKHI:]", "[[:Devanagari:]-[\\u0970]]",
    "[\\u0904\\u0901\\u0902\\u0936\\u0933\\u0951-\\u0954\\u0902\\u0903\\u0943-"
    "\\u0949\\u094a\\u0962\\u0963\\u090B\\u090C\\u090D\\u090e\\u0911\\u0912\\u0934\\u0937\\u093D\\u0950\\u0960\\u0961\\u097d]", /*roundtrip exclusions*/

    "DEVANAGARI-GURMUKHI", "[[:Devanagari:]-[\\u0970]]", "[:GURMUKHI:]",
    "[\\u0904\\u0A02\\u0946\\u0A5C\\u0951-"
    "\\u0954\\u0A70\\u0A71\\u090B\\u090C\\u090D\\u090e\\u0911\\u0912\\u0934\\u0937\\u093D\\u0950\\u0960\\u0961\\u0a72\\u0a73\\u0a74]", /*roundtrip exclusions*/

    "GUJARATI-DEVANAGARI", "[:GUJARATI:]", "[[:Devanagari:]-[\\u0970]]",
    "[\\u0946\\u094A\\u0962\\u0963\\u0951-\\u0954\\u0961\\u090c\\u090e\\u0912\\u097d]", /*roundtrip exclusions*/

    "DEVANAGARI-GUJARATI", "[[:Devanagari:]-[\\u0970]]", "[:GUJARATI:]", "[\\u0951-\\u0954\\u0961\\u090c\\u090e\\u0912]", /*roundtrip exclusions*/
    ...
    "[\\u0901\\u0904\\u093c\\u0943-\\u094a\\u0951-"
    "\\u0954\\u0962\\u0963\\u090B\\u090C\\u090D\\u0911\\u0916\\u0917\\u0918\\u091B\\u091D\\u0920\\u0921\\u0922\\u0925\\u0926\\u0927\\u092B\\u092C\\u092D\\u0936"
    "\\u093d\\u0950"
    "[\\u0958-\\u0961]\\u097d]", /*roundtrip exclusions*/

    "DEVANAGARI-Tamil", "[[:Devanagari:]-[\\u0970]]", "[:tamil:]", "[\\u0bd7\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/
    ...
    "[\\u0901\\u0904\\u094a\\u094b\\u094c\\u093c\\u0950\\u0944\\u0945\\u0949\\u0951-\\u0954\\u0962\\u0963\\u090D\\u0911\\u093d\\u0929\\u0934[\\u0958-\\u095f]"
    "\\u097d]", /*roundtrip
                                                                                                                                                                                 exclusions*/
    ...
    "GUJARATI-BENGALI", "[:GUJARATI:]", "[:BENGALI:]", "[\\u09d7\\u09e2\\u09e3\\u098c\\u09e1\\u09f0\\u09f1\\u09f2-\\u09fa\\u09ce]", /*roundtrip exclusions*/

    "BENGALI-GUJARATI", "[:BENGALI:]", "[:GUJARATI:]",
    "[\\u0A82\\u0a83\\u0Ac9\\u0Ac5\\u0ac7\\u0A8D\\u0A91\\u0AB3\\u0AB5\\u0ABD\\u0AD0]", /*roundtrip exclusions*/

    "ORIYA-BENGALI", "[:ORIYA:]", "[:BENGALI:]", "[\\u09c4\\u09e2\\u09e3\\u09f0\\u09f1\\u09f2-\\u09fa\\u09ce]", /*roundtrip exclusions*/

    "BENGALI-ORIYA", "[:BENGALI:]", "[:ORIYA:]", "[\\u0b35\\u0b71\\u0b5f\\u0b56\\u0b33\\u0b3d]", /*roundtrip exclusions*/

    "Tamil-BENGALI", "[:tamil:]", "[:BENGALI:]",
    "[\\u0981\\u09bc\\u09c3\\u09c4\\u09e2\\u09e3\\u09f0\\u09f1\\u098B\\u098C\\u0996\\u0997\\u0998\\u099B\\u099D\\u09A0\\u09A1\\u09A2\\u09A5\\u09A6\\u09A7\\u09A"
    "B\\u09AC\\u09A"
    "D\\u09B6\\u09DC\\u09DD\\u09DF\\u09E0\\u09E1\\u09f2-\\u09fa\\u09ce]", /*roundtrip exclusions*/
    ...
    "BENGALI-TELUGU", "[:BENGALI:]", "[:TELUGU:]", "[\\u0c55\\u0c56\\u0c47\\u0c46\\u0c4a\\u0C0E\\u0C12\\u0C31\\u0C33\\u0C35]", /*roundtrip exclusions*/
    ...
    "BENGALI-MALAYALAM", "[:BENGALI:]", "[:MALAYALAM:]", "[\\u0d46\\u0d4a\\u0d47\\u0d31-\\u0d35\\u0d0e\\u0d12]", /*roundtrip exclusions*/
    ...
    "[\\u0a5c\\u0A70\\u0a71\\u0a72\\u0a73\\u0a74\\u0a82\\u0a83\\u0a8b\\u0a8c\\u0a8d\\u0a91\\u0ab3\\u0ab6\\u0ab7\\u0abd\\u0ac3\\u0ac4\\u0ac5\\u0ac9\\u0ad0\\u0ae"
    "0\\u0ae1]", /*roundtrip exclusions*/

    "ORIYA-GURMUKHI", "[:ORIYA:]", "[:GURMUKHI:]",
    "[\\u0A01\\u0A02\\u0a5c\\u0a21\\u0a47\\u0a71\\u0b02\\u0b03\\u0b33\\u0b36\\u0b43\\u0b56\\u0b57\\u0B0B\\u0B0C\\u0B37\\u0B3D\\u0B5F\\u0B60\\u0B61\\u0a35\\u0a7"
    "2\\u0a73\\u0a7"
    "4]", /*roundtrip exclusions*/

    "GURMUKHI-ORIYA", "[:GURMUKHI:]", "[:ORIYA:]",
    "[\\u0b01\\u0b02\\u0b03\\u0b33\\u0b36\\u0b43\\u0b56\\u0b57\\u0B0B\\u0B0C\\u0B37\\u0B3D\\u0B5F\\u0B60\\u0B61\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "TAMIL-GURMUKHI", "[:TAMIL:]", "[:GURMUKHI:]",
    "[\\u0A01\\u0A02\\u0a33\\u0a36\\u0a3c\\u0a70\\u0a71\\u0a47\\u0A16\\u0A17\\u0A18\\u0A1B\\u0A1D\\u0A20\\u0A21\\u0A22\\u0A25\\u0A26\\u0A27\\u0A2B\\u0A2C\\u0A2"
    "D\\u0A59\\u0A5"
    "A\\u0A5B\\u0A5C\\u0A5E\\u0A72\\u0A73\\u0A74]", /*roundtrip exclusions*/
    ...
    "[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}"
    "\\u0c82\\u0c83\\u0cb3\\u0cb6\\u0cc4\\u0cc3\\u0cc6\\u0cca\\u0cd5\\u0cd6\\u0C8B\\u0C8C\\u0C8E\\u0C92\\u0CB1\\u0CB7\\u0cbd\\u0CE0\\u0CE1\\u0cde]", /*roundtrip exclusions*/
    ...
    "GUJARATI-ORIYA", "[:GUJARATI:]", "[:ORIYA:]", "[\\u0b56\\u0b57\\u0B0C\\u0B5F\\u0B61\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "ORIYA-GUJARATI", "[:ORIYA:]", "[:GUJARATI:]", "[\\u0Ac4\\u0Ac5\\u0Ac9\\u0Ac7\\u0A8D\\u0A91\\u0AB5\\u0Ad0]", /*roundtrip exclusions*/

    "TAMIL-GUJARATI", "[:TAMIL:]", "[:GUJARATI:]",
    "[\\u0A81\\u0a8c\\u0abc\\u0ac3\\u0Ac4\\u0Ac5\\u0Ac9\\u0Ac7\\u0A8B\\u0A8D\\u0A91\\u0A96\\u0A97\\u0A98\\u0A9B\\u0A9D\\u0AA0\\u0AA1\\u0AA2\\u0AA5\\u0AA6\\u0AA"
    "7\\u0AAB\\u0AA"
    "C\\u0AAD\\u0AB6\\u0ABD\\u0AD0\\u0AE0\\u0AE1]", /*roundtrip exclusions*/

    "GUJARATI-TAMIL", "[:GUJARATI:]", "[:TAMIL:]", "[\\u0Bc6\\u0Bca\\u0Bd7\\u0B8E\\u0B92\\u0BA9\\u0BB1\\u0BB4\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/

    "TELUGU-GUJARATI", "[:TELUGU:]", "[:GUJARATI:]", "[\\u0abc\\u0Ac5\\u0Ac9\\u0A8D\\u0A91\\u0ABD\\u0Ad0]", /*roundtrip exclusions*/

    "GUJARATI-TELUGU", "[:GUJARATI:]", "[:TELUGU:]", "[\\u0c46\\u0c4a\\u0c55\\u0c56\\u0C0C\\u0C0E\\u0C12\\u0C31\\u0C61]", /*roundtrip exclusions*/

    "KANNADA-GUJARATI", "[:KANNADA:]", "[:GUJARATI:]", "[\\u0A81\\u0abc\\u0Ac5\\u0Ac9\\u0A8D\\u0A91\\u0ABD\\u0Ad0]", /*roundtrip exclusions*/
    ...
    "[\\u0B01\\u0b3c\\u0b43\\u0b56\\u0B0B\\u0B0C\\u0B16\\u0B17\\u0B18\\u0B1B\\u0B1D\\u0B20\\u0B21\\u0B22\\u0B25\\u0B26\\u0B27\\u0B2B\\u0B2C\\u0B2D\\u0B36\\u0B3"
    "D\\u0B5C\\u0B5"
    "D\\u0B5F\\u0B60\\u0B61\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "ORIYA-TAMIL", "[:ORIYA:]", "[:TAMIL:]", "[\\u0bc6\\u0bca\\u0bc7\\u0B8E\\u0B92\\u0BA9\\u0BB1\\u0BB4\\u0BB5\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/

    "TELUGU-ORIYA", "[:TELUGU:]", "[:ORIYA:]", "[\\u0b3c\\u0b57\\u0b56\\u0B3D\\u0B5C\\u0B5D\\u0B5F\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "ORIYA-TELUGU", "[:ORIYA:]", "[:TELUGU:]", "[\\u0c44\\u0c46\\u0c4a\\u0c55\\u0c47\\u0C0E\\u0C12\\u0C31\\u0C35]", /*roundtrip exclusions*/

    "KANNADA-ORIYA", "[:KANNADA:]", "[:ORIYA:]", "[\\u0B01\\u0b3c\\u0b57\\u0B3D\\u0B5C\\u0B5D\\u0B5F\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "ORIYA-KANNADA", "[:ORIYA:]", "[:KANNADA:]",
    "[{\\u0cb0\\u0cbc}{\\u0cb3\\u0cbc}\\u0cc4\\u0cc6\\u0cca\\u0cd5\\u0cc7\\u0C8E\\u0C92\\u0CB1\\u0CB5\\u0CDE]", /*roundtrip exclusions*/

    "MALAYALAM-ORIYA", "[:MALAYALAM:]", "[:ORIYA:]", "[\\u0B01\\u0b3c\\u0b56\\u0B3D\\u0B5C\\u0B5D\\u0B5F\\u0b70\\u0b71]", /*roundtrip exclusions*/

    "ORIYA-MALAYALAM", "[:ORIYA:]", "[:MALAYALAM:]", "[\\u0D47\\u0D46\\u0D4a\\u0D0E\\u0D12\\u0D31\\u0D34\\u0D35]", /*roundtrip exclusions*/

    "TELUGU-TAMIL", "[:TELUGU:]", "[:TAMIL:]", "[\\u0bd7\\u0ba9\\u0bb4\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/

    "TAMIL-TELUGU", "[:TAMIL:]", "[:TELUGU:]",
    "[\\u0C01\\u0c43\\u0c44\\u0c46\\u0c47\\u0c55\\u0c56\\u0c66\\u0C0B\\u0C0C\\u0C16\\u0C17\\u0C18\\u0C1B\\u0C1D\\u0C20\\u0C21\\u0C22\\u0C25\\u0C26\\u0C27\\u0C2"
    "B\\u0C2C\\u0C2"
    "D\\u0C36\\u0C60\\u0C61]", /*roundtrip exclusions*/

    "KANNADA-TAMIL", "[:KANNADA:]", "[:TAMIL:]", "[\\u0bd7\\u0bc6\\u0ba9\\u0bb4\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/

    "TAMIL-KANNADA", "[:TAMIL:]", "[:KANNADA:]",
    "[\\u0cc3\\u0cc4\\u0cc6\\u0cc7\\u0cd5\\u0cd6\\u0C8B\\u0C8C\\u0C96\\u0C97\\u0C98\\u0C9B\\u0C9D\\u0CA0\\u0CA1\\u0CA2\\u0CA5\\u0CA6\\u0CA7\\u0CAB\\u0CAC\\u0CA"
    "D\\u0CB6\\u0cb"
    "c\\u0cbd\\u0CDE\\u0CE0\\u0CE1]", /*roundtrip exclusions*/

    "MALAYALAM-TAMIL", "[:MALAYALAM:]", "[:TAMIL:]", "[\\u0ba9\\u0BF0\\u0BF1\\u0BF2]", /*roundtrip exclusions*/

    "TAMIL-MALAYALAM", "[:TAMIL:]", "[:MALAYALAM:]",
    "[\\u0d43\\u0d12\\u0D0B\\u0D0C\\u0D16\\u0D17\\u0D18\\u0D1B\\u0D1D\\u0D20\\u0D21\\u0D22\\u0D25\\u0D26\\u0D27\\u0D2B\\u0D2C\\u0D2D\\u0D36\\u0D60\\u0D61]", /*roundtrip
                                                                                                                                                                      exclusions*/

    "KANNADA-TELUGU", "[:KANNADA:]", "[:TELUGU:]", "[\\u0C01\\u0c3f\\u0c46\\u0c48\\u0c4a]", /*roundtrip exclusions*/

    "TELUGU-KANNADA", "[:TELUGU:]", "[:KANNADA:]", "[\\u0cc8\\u0cd5\\u0cd6\\u0cbc\\u0cbd\\u0CDE]", /*roundtrip exclusions*/

    "MALAYALAM-TELUGU", "[:MALAYALAM:]", "[:TELUGU:]", "[\\u0C01\\u0c44\\u0c4a\\u0c4c\\u0c4b\\u0c55\\u0c56]", /*roundtrip exclusions*/

    "TELUGU-MALAYALAM", "[:TELUGU:]", "[:MALAYALAM:]", "[\\u0d4c\\u0d57\\u0D34]", /*roundtrip exclusions*/

    "MALAYALAM-KANNADA", "[:MALAYALAM:]", "[:KANNADA:]", "[\\u0cbc\\u0cbd\\u0cc4\\u0cc6\\u0cca\\u0ccc\\u0ccb\\u0cd5\\u0cd6\\u0cDe]", /*roundtrip exclusions*/

    "KANNADA-MALAYALAM", "[:KANNADA:]", "[:MALAYALAM:]", "[\\u0d4c\\u0d57\\u0d46\\u0D34]", /*roundtrip exclusions*/

    "Latin-Bengali", latinForIndic, "[[:Bengali:][\\u0964\\u0965]]", "[\\u0965\\u09f0-\\u09fa\\u09ce]" /*roundtrip exclusions*/,

    "Latin-Gurmukhi", latinForIndic, "[[:Gurmukhi:][\\u0964\\u0965]]", "[\\u0a01\\u0965\\u0a02\\u0a72\\u0a73\\u0a74]" /*roundtrip exclusions*/,

    "Latin-Gujarati", latinForIndic, "[[:Gujarati:][\\u0964\\u0965]]", "[\\u0965]" /*roundtrip exclusions*/,

    "Latin-Oriya", latinForIndic, "[[:Oriya:][\\u0964\\u0965]]", "[\\u0965\\u0b70]" /*roundtrip exclusions*/,

    "Latin-Tamil", latinForIndic, "[:Tamil:]", "[\\u0BF0\\u0BF1\\u0BF2]" /*roundtrip exclusions*/,

    "Latin-Telugu", latinForIndic, "[:Telugu:]", NULL /*roundtrip exclusions*/,

    "Latin-Kannada", latinForIndic, "[:Kannada:]", NULL /*roundtrip exclusions*/,

    "Latin-Malayalam", latinForIndic, "[:Malayalam:]", NULL /*roundtrip exclusions*/
};

void TransliteratorRoundTripTest::TestDebug(const char* name, const char fromSet[], const char* toSet, const char* exclusions)
{
...
void TransliteratorRoundTripTest::TestInterIndic()
{
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (!logKnownIssue("cldrbug:1911")) {
            ...
            test.test(interIndicArray[i * INTER_INDIC_ARRAY_WIDTH + 1], interIndicArray[i * INTER_INDIC_ARRAY_WIDTH + 2],
