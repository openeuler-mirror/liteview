...
static int32_t convertToPuny(const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, UErrorCode& status)
{
...
static int32_t convertFromPuny(const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, UErrorCode& status)
{
...
U_CFUNC int32_t U_EXPORT2 idnaref_toASCII(
    const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, int32_t options, UParseError* parseError, UErrorCode* status)
{
    ...
    int32_t b1Len = 0, b2Len = 0, b1Capacity = MAX_LABEL_BUFFER_SIZE, b2Capacity = MAX_LABEL_BUFFER_SIZE, reqLength = 0;
...
U_CFUNC int32_t U_EXPORT2 idnaref_toUnicode(
    const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, int32_t options, UParseError* parseError, UErrorCode* status)
{
    ...
    int32_t b1Len, b2Len, b1PrimeLen, b3Len, b1Capacity = MAX_LABEL_BUFFER_SIZE, b2Capacity = MAX_LABEL_BUFFER_SIZE, b3Capacity = MAX_LABEL_BUFFER_SIZE,
                                             reqLength = 0;
...
static int32_t getNextSeparator(UChar* src, int32_t srcLength, NamePrepTransform* prep, UChar** limit, UBool* done, UErrorCode* status)
{
    if (srcLength == -1) {
        int32_t i;
        for (i = 0;; i++) {
...
U_CFUNC int32_t U_EXPORT2 idnaref_IDNToASCII(
    const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, int32_t options, UParseError* parseError, UErrorCode* status)
{
    ...
    if (srcLength == -1) {
        for (;;) {
            ...
            if (!(labelLen == 0 && done)) { // make sure this is not a root label separator.
                ...
                b1Len = idnaref_toASCII(labelStart, labelLen, b1, b1Capacity, options, parseError, status);
                ...
                if (*status == U_BUFFER_OVERFLOW_ERROR) {
                    ...
                    *status = U_ZERO_ERROR; // reset error
                    ...
                    b1Len = idnaref_toASCII(labelStart, labelLen, b1, b1Len, options, parseError, status);
                }
            }
    ...
    const UChar* src, int32_t srcLength, UChar* dest, int32_t destCapacity, int32_t options, UParseError* parseError, UErrorCode* status)
{
    ...
    if (srcLength == -1) {
        for (;;) {
            ...
            b1Len = idnaref_toUnicode(labelStart, labelLen, b1, b1Capacity, options, parseError, status);
            ...
            if (*status == U_BUFFER_OVERFLOW_ERROR) {
                ...
                *status = U_ZERO_ERROR; // reset error
                ...
                b1Len = idnaref_toUnicode(labelStart, labelLen, b1, b1Len, options, parseError, status);
            }
...
U_CFUNC int32_t U_EXPORT2 idnaref_compare(const UChar* s1, int32_t length1, const UChar* s2, int32_t length2, int32_t options, UErrorCode* status)
{
