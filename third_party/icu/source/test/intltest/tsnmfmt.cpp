...
void IntlTestNumberFormat::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    ...
    if (exec)
        logln((UnicodeString) "TestSuite NumberFormat");
    switch (index) {
    case 0:
        name = "createInstance";
        if (exec) {
...
void IntlTestNumberFormat::tryIt(double aNumber)
{
    ...
    for (i = 0; i < DEPTH; ++i) {
        ...
        if (number[i].getType() == Formattable::kLong)
            ...
            errMsg = ("**** FAIL: Parse of " + prettify(string[i - 1]) + " returned non-numeric Formattable, type "
                + UnicodeString(formattableTypeName(number[i].getType())) + ", Locale=" + UnicodeString(fLocale.getName())
    ...
    if (errMsg.length() != 0) {
        for (int32_t k = 0; k <= i; ++k) {
            logln((UnicodeString) "" + k + ": " + number[k].getDouble() + " F> " + prettify(string[k]) + " P> ");
        }
        errln(errMsg);
    }
}
...
void IntlTestNumberFormat::tryIt(int32_t aNumber)
{
    ...
    if (number.getType() != Formattable::kLong) {
        errln("**** FAIL: Parse of " + prettify(stringNum) + " returned non-long Formattable, type " + UnicodeString(formattableTypeName(number.getType()))
            + ", Locale=" + UnicodeString(fLocale.getName()) + ", doubleValue=" + number.getDouble() + ", longValue=" + number.getLong()
            + ", origValue=" + aNumber);
    }
