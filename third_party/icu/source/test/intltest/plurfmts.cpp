...
void PluralFormatTest::pluralFormatUnitTest(/*char *par*/)
{
    ...
    for (int32_t i = 0; i < PLURAL_PATTERN_DATA; ++i) {
        ...
        numberFormatTest(
            &plFmt, numFmt.getAlias(), 1, 10, (UnicodeString*)&patternOddTestResult[i], (UnicodeString*)&patternEvenTestResult[i], overwrite[i], &message);
    }
...
void PluralFormatTest::pluralFormatLocaleTest(/*char *par*/)
{
    ...
    // add hu tr others
    const char* singular1Locales[56] = { "af", "asa", "az", "bem", "bez", "bg", "brx", "chr", "ckb", "dv", "ee", "el", "eo", "es", "eu", "fo", "fur", "fy",
        "gsw", "ha", "haw", "hu", "jgo", "ka", "kk", "kl", "ks", "ku", "lb", "ml", "mn", "nah", "nb", "ne", "nn", "no", "nr", "om", "or", "pap", "ps", "rm",
        "rof", "sn", "so", "sq", "ta", "te", "tk", "tn", "tr", "ts", "vo", "wae", "xh", "xog" };
...
void PluralFormatTest::pluralFormatExtendedTest(void)
{
    const char* targets[] = {
    ...
    MessageFormat mf(UNICODE_STRING_SIMPLE("{0,plural,").append(fmtString).append((UChar)0x7d /* '}' */), Locale::getEnglish(), status);
    Formattable args;
...
void PluralFormatTest::pluralFormatExtendedParseTest(void)
{
    const char* failures[] = {
        "offset:1..0 =0 {Foo}",
        "offset:1.0 {Foo}",
...
void PluralFormatTest::ordinalFormatTest(void)
{
    ...
    if (!errorCode.errIfFailureAndReset("PluralFormat.format(321) failed") && result != UNICODE_STRING_SIMPLE("321st file")) {
        errln(UnicodeString("PluralFormat.format(321) wrong result string: ") + result);
    }
    result = pf.format((int32_t)22, errorCode);
    if (!errorCode.errIfFailureAndReset("PluralFormat.format(22) failed") && result != UNICODE_STRING_SIMPLE("22nd file")) {
        errln(UnicodeString("PluralFormat.format(22) wrong result string: ") + result);
    }
    result = pf.format((int32_t)3, errorCode);
    if (!errorCode.errIfFailureAndReset("PluralFormat.format(3) failed") && result != UNICODE_STRING_SIMPLE("3rd file")) {
    ...
    if (!errorCode.errIfFailureAndReset("PluralFormat.format(456) failed") && result != UNICODE_STRING_SIMPLE("456th file")) {
        errln(UnicodeString("PluralFormat.format(456) wrong result string: ") + result);
    }
    result = pf2.format((int32_t)111, errorCode);
    if (!errorCode.errIfFailureAndReset("PluralFormat.format(111) failed") && result != UNICODE_STRING_SIMPLE("111th file")) {
...
void PluralFormatTest::TestDecimals()
{
    ...
    PluralFormat pf2(Locale::getEnglish(), "offset:1 one{another meter}other{another # meters}", errorCode);
...
void PluralFormatTest::numberFormatTest(PluralFormat* plFmt, NumberFormat* numFmt, int32_t start, int32_t end, UnicodeString* numOddAppendStr,
...
void PluralFormatTest::helperTestResults(const char** localeArray, int32_t capacityOfArray, UnicodeString& testPattern, int8_t* expResults)
{
    UErrorCode status;
    ...
    for (int32_t i = 0; i < capacityOfArray; ++i) {
        ...
        for (int32_t n = 0; n < PLURAL_TEST_ARRAY_SIZE; ++n) {
            if (expResults[n] != -1) {
                ...
                if (U_FAILURE(status)) {
                    errln("ERROR: Failed to format number in locale data tests with locale: " + UnicodeString(localeArray[i]));
                }
                if (plResult != PLKeywordLookups[expResults[n]]) {
                    plResult = plFmt.format(n, status);
                    errln("ERROR: Unexpected format result in locale: " + UnicodeString(localeArray[i]) + UnicodeString(" for value: ") + n
                        + UnicodeString("  got:") + plResult + UnicodeString("  expecting:") + PLKeywordLookups[expResults[n]]);
                }
            }
        }
    }
