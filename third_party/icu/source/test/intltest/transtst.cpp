...
void TransliteratorTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    switch (index) {
...
void TransliteratorTest::TestInstantiation()
{
    ...
    assertTrue("getAvailableIDs().count()==countAvailableIDs()", avail->count(ec) == n);
    assertSuccess("count()", ec);
    UnicodeString name;
    for (int32_t i = 0; i < n; ++i) {
        const UnicodeString& id = *avail->snext(ec);
        if (!assertSuccess("snext()", ec) || !assertTrue("snext()!=NULL", (&id) != NULL, TRUE)) {
            break;
        }
        UnicodeString id2 = Transliterator::getAvailableID(i);
        if (id.length() < 1) {
            errln(UnicodeString("FAIL: getAvailableID(") + i + ") returned empty string");
            continue;
        }
        if (id != id2) {
            errln(UnicodeString("FAIL: getAvailableID(") + i + ") != getAvailableIDs().snext()");
            continue;
        }
        ...
        if (t == 0) {
            ...
            if (id.compare((UnicodeString) "Thai-Latn") != 0 && id.compare((UnicodeString) "Thai-Latin") != 0)
#endif
                dataerrln(UnicodeString("FAIL: Couldn't create ") + id +
                    /*", parse error " + parseError.code +*/
                    ", line " + parseError.line + ", offset " + parseError.offset + ", pre-context " + prettify(parseError.preContext, TRUE) + ", post-context "
                    + prettify(parseError.postContext, TRUE) + ", Error: " + u_errorName(status));
            ...
            Transliterator* u = Transliterator::createFromRules("x", rules, UTRANS_FORWARD, parseError, status);
            if (u == 0) {
                errln(UnicodeString("FAIL: ") + id + ".createFromRules() => bad rules" +
                    /*", parse error " + parseError.code +*/
                    ", line " + parseError.line + ", offset " + parseError.offset + ", context " + prettify(parseError.preContext, TRUE)
...
void TransliteratorTest::TestSimpleRules(void)
{
     ...
     * [exzd]|   done
     */
    expect(UnicodeString("ab>x|y;", "") + "yc>z", "eabcd", "exzd");
     ...
     * [xyqn]|     Done
     */
    expect(UnicodeString("ab>x|yzacw;") + "za>q;" + "qc>r;" + "cw>n", "ab", "xyqn");
...
void TransliteratorTest::TestInlineSet(void)
{
    ...
    expect(UnicodeString("$digit = [0-9];"
                         "$alpha = [a-zA-Z];"
...
{
    UnicodeString RULES = UnicodeString("abc>zyx;") + "ab>yz;" + "bc>zx;" + "ca>xy;" + "a>x;" + "b>y;" + "c>z;" +

        "abc<zyx;" + "ab<yz;" + "bc<zx;" + "ca<xy;" + "a<x;" + "b<y;" + "c<z;" +
    ...
    UErrorCode status = U_ZERO_ERROR;
    UParseError parseError;
    Transliterator* fwd = Transliterator::createFromRules("<ID>", RULES, UTRANS_FORWARD, parseError, status);
...
void TransliteratorTest::TestKeyboard(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createFromRules("<ID>", UnicodeString("psch>Y;") + "ps>y;" + "ch>x;" + "a>A;", UTRANS_FORWARD, parseError, status);
    if (U_FAILURE(status)) {
        errln("FAIL: RBT constructor failed");
        return;
    }
    const char* DATA[] = {
        // insertion, buffer
        "a", "A", "p", "Ap", "s", "Aps", "c", "Apsc", "a", "AycA", "psch", "AycAY", 0, "AycAY", // null means finishKeyboardTransliteration
    };
...
void TransliteratorTest::TestKeyboard2(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createFromRules("<ID>", UnicodeString("ych>Y;") + "ps>|y;" + "ch>x;" + "a>A;", UTRANS_FORWARD, parseError, status);
    if (U_FAILURE(status)) {
        errln("FAIL: RBT constructor failed");
        return;
    }
    const char* DATA[] = {
        // insertion, buffer
        "a", "A", "p", "Ap", "s", "Aps", // modified for rollback - "Ay",
        "c", "Apsc", // modified for rollback - "Ayc",
        "a", "AycA", "p", "AycAp", "s", "AycAps", // modified for rollback - "AycAy",
        "c", "AycApsc", // modified for rollback - "AycAyc",
        "h", "AycAY", 0, "AycAY", // null means finishKeyboardTransliteration
    };
...
void TransliteratorTest::TestKeyboard3(void)
{
    ...
    const char* DATA[] = {
        ...
        "a", "a", "b", "ab", "t", "abt", // modified for rollback - "aby",
        "c", "abyc", "t", "abyct", // modified for rollback - "abycy",
        "h", "abycz", 0, "abycz", // null means finishKeyboardTransliteration
    };
...
void TransliteratorTest::keyboardAux(const Transliterator& t, const char* DATA[], int32_t DATA_length)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    for (int32_t i = 0; i < DATA_length; i += 2) {
        ...
        log.append(a).append((UChar)LEFT_BRACE).append(b).append((UChar)PIPE).append(c);
...
class TestFilter : public UnicodeFilter {
    ...
    // Stubs
    virtual UnicodeString& toPattern(UnicodeString& result, UBool /*escapeUnprintable*/) const override
    {
        return result;
    }
...
{
    expect(UnicodeString("^a  > 0; a$ > 2 ; a > 1;", ""), "aaa", "012");
    expect(UnicodeString("$s=[z$]; $s{a>0; a}$s>2; a>1;", ""), "aaa", "012");
    ...
    expect(UnicodeString("$s = [z$] ;"
        ...
        "abzababbabxzabxabx", "01z018k45z01x45");
}
...
void TransliteratorTest::TestPatternQuoting(void)
{
    ...
    const UnicodeString DATA[] = { UnicodeString(UChar(0x4E01)) + ">'[male adult]'", UnicodeString(UChar(0x4E01)), "[male adult]" };
...
void TransliteratorTest::TestJ277(void)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        UBool ok = TRUE;
        if (data[i].length() >= 2 && out.length() >= 2 && u_isupper(data[i].charAt(0)) && u_islower(data[i].charAt(1))) {
...
void TransliteratorTest::TestJ243(void)
{
    ...
    // \u, \U, u+, and U+
    Transliterator* hex = Transliterator::createInstance("Hex-Any", UTRANS_FORWARD, ec);
...
void TransliteratorTest::TestJ329(void)
{
    ...
    for (int32_t i = 0; i < DATA_length; ++i) {
        UErrorCode status = U_ZERO_ERROR;
        UParseError parseError;
        Transliterator* rbt = Transliterator::createFromRules("<ID>", DATA[i].rule, UTRANS_FORWARD, parseError, status);
        UBool gotError = U_FAILURE(status);
        UnicodeString desc(DATA[i].rule);
        desc.append(gotError ? " -> error" : " -> no error");
        if (gotError) {
            desc = desc + ", ParseError code=" + u_errorName(status) + " line=" + parseError.line + " offset=" + parseError.offset
...
void TransliteratorTest::TestPositionHandling(void)
{
    ...
    for (int32_t i = 0; i < n; i++) {
        UErrorCode status = U_ZERO_ERROR;
        UParseError parseError;
        Transliterator* t = Transliterator::createFromRules("<ID>", DATA[3 * i], UTRANS_FORWARD, parseError, status);
        if (U_FAILURE(status)) {
            delete t;
            errln("FAIL: RBT constructor");
            return;
        ...
        expectAux(DATA[3 * i], DATA[3 * i + 1], rsource, DATA[3 * i + 2]);
        delete t;
    }
}
...
void TransliteratorTest::TestCopyJ476(void)
{
    ...
    Transliterator* t2 = 0;
    {
        UParseError parseError;
        UErrorCode status = U_ZERO_ERROR;
        Transliterator* t1 = Transliterator::createFromRules("t1", "a>A;b>B;'foo'+>'bar'", UTRANS_FORWARD, parseError, status);
        if (U_FAILURE(status)) {
            errln("FAIL: RBT constructor");
            return;
        }
...
void TransliteratorTest::TestFilterIDs(void)
{
    ...
    for (int i = 0; i < DATA_length; i += 4) {
        ...
        if (u == 0) {
            ...
            errln("FAIL: " + ID + ".createInverse().getID() => " + u->getID() + ", expected " + uID);
        }
...
void TransliteratorTest::TestCaseMap(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* toUpper = Transliterator::createInstance("Any-Upper[^xyzXYZ]", UTRANS_FORWARD, parseError, status);
    Transliterator* toLower = Transliterator::createInstance("Any-Lower[^xyzXYZ]", UTRANS_FORWARD, parseError, status);
    Transliterator* toTitle = Transliterator::createInstance("Any-Title[^xyzXYZ]", UTRANS_FORWARD, parseError, status);
...
void TransliteratorTest::TestNameMap(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* uni2name = Transliterator::createInstance("Any-Name[^abc]", UTRANS_FORWARD, parseError, status);
    Transliterator* name2uni = Transliterator::createInstance("Name-Any", UTRANS_FORWARD, parseError, status);
    ...
    expect(*uni2name, CharsToUnicodeString("\\u00A0abc\\u4E01\\u00B5\\u0A81\\uFFFD\\u0004\\u0009\\u0081\\uFFFF"),
        CharsToUnicodeString("\\\\N{NO-BREAK SPACE}abc\\\\N{CJK UNIFIED IDEOGRAPH-4E01}\\\\N{MICRO SIGN}\\\\N{GUJARATI SIGN CANDRABINDU}\\\\N{REPLACEMENT "
                             "CHARACTER}\\\\N{<control-0004>}\\\\N{<control-0009>}\\\\N{<control-0081>}\\\\N{<noncharacter-FFFF>}"));
    expect(*name2uni,
        UNICODE_STRING_SIMPLE("{\\N { NO-BREAK SPACE}abc\\N{  CJK UNIFIED  IDEOGRAPH-4E01  }\\N{x\\N{MICRO SIGN}\\N{GUJARATI SIGN CANDRABINDU}\\N{REPLACEMENT "
                              "CHARACTER}\\N{<control-0004>}\\N{<control-0009>}\\N{<control-0081>}\\N{<noncharacter-FFFF>}\\N{<control-0004>}\\N{"),
    ...
    // round trip
    Transliterator* t = Transliterator::createInstance("Any-Name;Name-Any", UTRANS_FORWARD, parseError, status);
    if (t == 0) {
        errln("FAIL: createInstance returned NULL");
        delete t;
        return;
...
void TransliteratorTest::TestLiberalizedID(void)
{
    ...
    for (int32_t i = 0; i < DATA_length; i += 3) {
        ...
        if (t == 0) {
            dataerrln(UnicodeString("FAIL: ") + DATA[i + 2] + " cannot create ID \"" + DATA[i] + "\" - " + u_errorName(status));
        } else {
            UnicodeString exp;
            if (DATA[i + 1]) {
            ...
            if (exp.length() == 0 || exp == t->getID()) {
                logln(UnicodeString("Ok: ") + DATA[i + 2] + " create ID \"" + DATA[i] + "\" => \"" + exp + "\"");
            } else {
                errln(UnicodeString("FAIL: ") + DATA[i + 2] + " create ID \"" + DATA[i] + "\" => \"" + t->getID() + "\", exp \"" + exp + "\"");
            }
            delete t;
        }
    }
...
void TransliteratorTest::TestCreateInstance()
{
    ...
    const char* DATA[] = { // Column 1: id
        // Column 2: direction
        // Column 3: expected ID, or "" if expect failure
        "Latin-Hangul", REVERSE, "Hangul-Latin", // JB#912

        // JB#2689: bad compound causes crash
        "InvalidSource-InvalidTarget", FORWARD, "", "InvalidSource-InvalidTarget", REVERSE, "", "Hex-Any;InvalidSource-InvalidTarget", FORWARD, "",
        "Hex-Any;InvalidSource-InvalidTarget", REVERSE, "", "InvalidSource-InvalidTarget;Hex-Any", FORWARD, "", "InvalidSource-InvalidTarget;Hex-Any", REVERSE,
        "",
    ...
    for (int32_t i = 0; DATA[i]; i += 3) {
        ...
        UErrorCode ec = U_ZERO_ERROR;
        UnicodeString id(DATA[i]);
        UTransDirection dir = (DATA[i + 1] == FORWARD) ? UTRANS_FORWARD : UTRANS_REVERSE;
        UnicodeString expID(DATA[i + 2]);
        Transliterator* t = Transliterator::createInstance(id, dir, err, ec);
        UnicodeString newID;
        if (t) {
            newID = t->getID();
        }
        ...
        if (ok) {
            logln((UnicodeString) "Ok: createInstance(" + id + "," + DATA[i + 1] + ") => " + newID);
        } else {
            dataerrln((UnicodeString) "FAIL: createInstance(" + id + "," + DATA[i + 1] + ") => " + newID + ", expected " + expID);
        }
        delete t;
    }
}
...
void TransliteratorTest::TestNormalizationTransliterator()
{
    ...
    const char* CANON[] = {
        ...
        "cat", "cat", "cat", "\\u00e0ardvark", "a\\u0300ardvark", "\\u00e0ardvark",
        ...
        "Henry IV", "Henry IV", "Henry IV", "Henry \\u2163", "Henry \\u2163", "Henry \\u2163",
    ...
    const char* COMPAT[] = {
        ...
        "Henry IV", "Henry IV", "Henry IV", "Henry \\u2163", "Henry IV", "Henry IV",
    ...
    UParseError pe;
    status = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createInstance("NFD; [x]Remove", UTRANS_FORWARD, pe, status);
    if (t == 0) {
        errln("FAIL: createInstance failed");
    }
    expect(*t, CharsToUnicodeString("\\u010dx"), CharsToUnicodeString("c\\u030C"));
    delete t;
}
...
void TransliteratorTest::TestBasicTransliteratorEvenWithoutData()
{
    ...
    const char16_t* EXPECTED_RESULTS[] = { u"H\u0302e\u0301 \uFB01nd x", // NFD
        u"\u0124\u00E9 \uFB01nd x", // NFC
        u"H\u0302e\u0301 find x", // NFKD
        u"\u0124\u00E9 find x", // NFKC
        u"\u0124e\u0301 \uFB01nd x", // Hex-Any
    ...
    const char* BASIC_TRANSLITERATOR_ID[] = { "NFD", "NFC", "NFKD", "NFKC", "Hex-Any", "Lower", "[:^L:]Remove", "NFD; [x]Remove", "Lower; NFKD;",
        "Lower; NFKD; [:^L:]Remove; NFC;", "[:Nonspacing Mark:] Remove;", "NFD; [:Nonspacing Mark:] Remove; NFC;",
        // end
        0 };
    const char* BASIC_TRANSLITERATOR_RULES[]
        = { "::Lower; ::NFKD;", "::Lower; ::NFKD; ::[:^L:]Remove; ::NFC;", "::[:Nonspacing Mark:] Remove;", "::NFD; ::[:Nonspacing Mark:] Remove; ::NFC;",
              // end
              0 };
    for (int32_t i = 0; BASIC_TRANSLITERATOR_ID[i]; i++) {
        UErrorCode status = U_ZERO_ERROR;
        UParseError parseError;
        std::unique_ptr<Transliterator> translit(Transliterator::createInstance(BASIC_TRANSLITERATOR_ID[i], UTRANS_FORWARD, parseError, status));
        ...
        if (data != expected) {
            dataerrln(UnicodeString("FAIL: expected translit(") + BASIC_TRANSLITERATOR_ID[i] + ") = '" + EXPECTED_RESULTS[i] + "' but got '" + data);
            continue;
        }
    }
    for (int32_t i = 0; BASIC_TRANSLITERATOR_RULES[i]; i++) {
        UErrorCode status = U_ZERO_ERROR;
        UParseError parseError;
        std::unique_ptr<Transliterator> translit(Transliterator::createFromRules("Test", BASIC_TRANSLITERATOR_RULES[i], UTRANS_FORWARD, parseError, status));
        if (translit.get() == nullptr || !U_SUCCESS(status)) {
...
void TransliteratorTest::TestCompoundRBT(void)
{
    ...
    expect(*t, UNICODE_STRING_SIMPLE("\\u0043at in the hat, bat on the mat"), "C.A.t IN tHE H.A.t, .B..A.t ON tHE M.A.t");
    UnicodeString r;
    t->toRules(r, TRUE);
    if (r == rule) {
        logln((UnicodeString) "OK: toRules() => " + r);
    } else {
        errln((UnicodeString) "FAIL: toRules() => " + r + ", expected " + rule);
    }
    delete t;
    ...
    t->toRules(r, TRUE);
    if (r != exp) {
        errln((UnicodeString) "FAIL: toRules() => " + r + ", expected " + exp);
    } else {
        logln((UnicodeString) "OK: toRules() => " + r);
    }
    delete t;
    ...
    if (t->getID() == id) {
        ...
        errln((UnicodeString) "FAIL: createInstance(" + id + ").getID() => " + t->getID());
    }
    ...
    if (u->getID() == exp) {
        logln((UnicodeString) "OK: createInverse(" + id + ") => " + u->getID());
    } else {
        errln((UnicodeString) "FAIL: createInverse(" + id + ") => " + u->getID());
    }
    delete t;
    delete u;
}
...
void TransliteratorTest::TestCompoundFilter(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createInstance("Greek-Latin; Latin-Greek; Lower", UTRANS_FORWARD, parseError, status);
    if (t == 0) {
        dataerrln("FAIL: createInstance failed - %s", u_errorName(status));
        return;
    }
...
void TransliteratorTest::TestToRules(void)
{
    ...
    for (int32_t d = 0; d < DATA_length; d += 3) {
        if (DATA[d] == RBT) {
            ...
            UParseError parseError;
            UErrorCode status = U_ZERO_ERROR;
            Transliterator* t = Transliterator::createFromRules("ID", UnicodeString(DATA[d + 1], -1, US_INV), UTRANS_FORWARD, parseError, status);
            if (t == 0) {
            ...
            if (rules == expRules) {
                logln((UnicodeString) "Ok: " + UnicodeString(DATA[d + 1], -1, US_INV) + " => " + rules);
            } else {
                errln((UnicodeString) "FAIL: " + UnicodeString(DATA[d + 1], -1, US_INV) + " => " + rules + ", exp " + expRules);
            }
            if (escapedRules == expEscapedRules) {
                logln((UnicodeString) "Ok: " + UnicodeString(DATA[d + 1], -1, US_INV) + " => " + escapedRules);
            } else {
                errln((UnicodeString) "FAIL: " + UnicodeString(DATA[d + 1], -1, US_INV) + " => " + escapedRules + ", exp " + expEscapedRules);
            }
            delete t;
            ...
            set.toPattern(toPat);
            if (expToPat == toPat) {
                logln((UnicodeString) "Ok: " + pat + " => " + toPat);
            } else {
                errln((UnicodeString) "FAIL: " + pat + " => " + prettify(toPat, TRUE) + ", exp " + prettify(pat, TRUE));
            }
        }
    }
}
...
void TransliteratorTest::TestSupplemental()
{
    ...
    expect(CharsToUnicodeString("$a=\\U00010300; $s=[\\U00010300-\\U00010323];"
                                "a > $a; $s > i;"),
        CharsToUnicodeString("ab\\U0001030Fx"), CharsToUnicodeString("\\U00010300bix"));

    expect(CharsToUnicodeString("$a=[a-z\\U00010300-\\U00010323];"
                                "$b=[A-Z\\U00010400-\\U0001044D];"
                                "($a)($b) > $2 $1;"),
        CharsToUnicodeString("aB\\U00010300\\U00010400c\\U00010401\\U00010301D"), CharsToUnicodeString("Ba\\U00010400\\U00010300\\U00010401cD\\U00010301"));
    ...
    expect(CharsToUnicodeString("$a=[a\\U00010300-\\U00010323];"
                                "$a {x} > | @ \\U00010400;"
                                "{$a} [^\\u0000-\\uFFFF] > y;"),
        CharsToUnicodeString("kax\\U00010300xm"), CharsToUnicodeString("ky\\U00010400y\\U00010400m"));

    expectT("Any-Name", CharsToUnicodeString("\\U00010330\\U000E0061\\u00A0"),
        UNICODE_STRING_SIMPLE("\\N{GOTHIC LETTER AHSA}\\N{TAG LATIN SMALL LETTER A}\\N{NO-BREAK SPACE}"));

    expectT("Any-Hex/Unicode", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"), UNICODE_STRING_SIMPLE("U+10330U+10FF00U+E0061U+00A0"));

    expectT("Any-Hex/C", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"), UNICODE_STRING_SIMPLE("\\U00010330\\U0010FF00\\U000E0061\\u00A0"));

    expectT("Any-Hex/Perl", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"), UNICODE_STRING_SIMPLE("\\x{10330}\\x{10FF00}\\x{E0061}\\x{A0}"));

    expectT("Any-Hex/Java", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"),
        UNICODE_STRING_SIMPLE("\\uD800\\uDF30\\uDBFF\\uDF00\\uDB40\\uDC61\\u00A0"));

    expectT("Any-Hex/XML", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"), "&#x10330;&#x10FF00;&#xE0061;&#xA0;");

    expectT("Any-Hex/XML10", CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"), "&#66352;&#1113856;&#917601;&#160;");

    expectT(UNICODE_STRING_SIMPLE("[\\U000E0000-\\U000E0FFF] Remove"), CharsToUnicodeString("\\U00010330\\U0010FF00\\U000E0061\\u00A0"),
...
void TransliteratorTest::TestSTV(void)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        if (t == NULL) {
            errln((UnicodeString) "FAIL: Registration/creation failed for ID " + IDS[i]);
        } else {
            logln((UnicodeString) "Ok: Registration/creation succeeded for ID " + IDS[i]);
            delete t;
        }
        Transliterator::unregister(IDS[i]);
        t = Transliterator::createInstance(IDS[i], UTRANS_FORWARD, status);
        if (t != NULL) {
            errln((UnicodeString) "FAIL: Unregistration failed for ID " + IDS[i]);
            delete t;
        }
    }
...
void TransliteratorTest::TestCompoundInverse(void)
{
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createInstance("Greek-Latin; Title()", UTRANS_REVERSE, parseError, status);
    if (t == 0) {
...
void TransliteratorTest::TestNFDChainRBT()
{
    UParseError pe;
    UErrorCode ec = U_ZERO_ERROR;
    Transliterator* t = Transliterator::createFromRules("TEST", "::NFD; aa > Q; a > q;", UTRANS_FORWARD, pe, ec);
...
void TransliteratorTest::TestAliasInverseID()
{
    ...
    if (got != exp) {
        errln((UnicodeString) "FAIL: Inverse of " + ID + " is " + got + ", expected " + exp);
    }
    delete t;
    delete u;
}
...
void TransliteratorTest::TestUndefinedVariable()
{
    ...
    delete t;
    if (U_FAILURE(ec)) {
        logln((UnicodeString) "OK: Got exception for " + rule + ", as expected: " + u_errorName(ec));
        return;
    }
    errln((UnicodeString) "Fail: bogus rule " + rule + " compiled with error " + u_errorName(ec));
}
...
void TransliteratorTest::TestCompoundFilterID(void)
{
    ...
    for (int32_t i = 0; DATA[i]; i += 4) {
        ...
        UTransDirection direction = (DATA[i + 1] != NULL && DATA[i + 1][0] == 'R') ? UTRANS_REVERSE : UTRANS_FORWARD;
        UnicodeString source;
        UnicodeString exp;
        if (DATA[i + 2] != NULL) {
        ...
        if (ok == expOk) {
            logln((UnicodeString) "Ok: " + id + " => " + transID + ", " + u_errorName(ec));
            if (source.length() != 0) {
                expect(*t, source, exp);
            }
        } else {
            dataerrln((UnicodeString) "FAIL: " + id + " => " + transID + ", " + u_errorName(ec));
        }
    }
}
...
void TransliteratorTest::TestNewEngine()
{
    ...
    expect(*t, CharsToUnicodeString("a\\u3042\\u30A2"), CharsToUnicodeString("\\u3042\\u3042\\u30A2"));
    ...
    Transliterator* a = Transliterator::createFromRules("a_to_A", "a > A;", UTRANS_FORWARD, pe, ec);
    Transliterator* A = Transliterator::createFromRules("A_to_b", "A > b;", UTRANS_FORWARD, pe, ec);
    if (U_FAILURE(ec)) {
        delete a;
        delete A;
        return;
    ...
    UnicodeString gr = CharsToUnicodeString("$ddot = \\u0308 ;"
...
void TransliteratorTest::TestDevanagariLatinRT()
{
    ...
    const char* const source[MAX_LEN] = { "bh\\u0101rata", "kra", "k\\u1E63a", "khra", "gra", "\\u1E45ra", "cra", "chra", "j\\u00F1a", "jhra", "\\u00F1ra",
        "\\u1E6Dya", "\\u1E6Dhra", "\\u1E0Dya",
        //"r\\u0323ya", // \u095c is not valid in Devanagari
        "\\u1E0Dhya", "\\u1E5Bhra", "\\u1E47ra", "tta", "thra", "dda", "dhra", "nna", "pra", "phra", "bra", "bhra", "mra", "\\u1E49ra",
        //"l\\u0331ra",
        "yra", "\\u1E8Fra",
        //"l-",
        "vra", "\\u015Bra", "\\u1E63ra", "sra", "hma", "\\u1E6D\\u1E6Da", "\\u1E6D\\u1E6Dha", "\\u1E6Dh\\u1E6Dha", "\\u1E0D\\u1E0Da", "\\u1E0D\\u1E0Dha",
        "\\u1E6Dya", "\\u1E6Dhya", "\\u1E0Dya", "\\u1E0Dhya",
        // Not roundtrippable --
        // \\u0939\\u094d\\u094d\\u092E  - hma
        // \\u0939\\u094d\\u092E         - hma
        // CharsToUnicodeString("hma"),
        "hya", "\\u015Br\\u0325", "\\u015Bca", "\\u0115", "san\\u0304j\\u012Bb s\\u0113nagupta", "\\u0101nand vaddir\\u0101ju", "\\u0101", "a" };
...
{
    const char* const source[] = { "rmk\\u1E63\\u0113t", "\\u015Br\\u012Bmad", "bhagavadg\\u012Bt\\u0101", "adhy\\u0101ya", "arjuna", "vi\\u1E63\\u0101da",
        "y\\u014Dga", "dhr\\u0325tar\\u0101\\u1E63\\u1E6Dra", "uv\\u0101cr\\u0325", "dharmak\\u1E63\\u0113tr\\u0113", "kuruk\\u1E63\\u0113tr\\u0113",
        "samav\\u0113t\\u0101", "yuyutsava\\u1E25", "m\\u0101mak\\u0101\\u1E25",
        // "p\\u0101\\u1E47\\u1E0Dav\\u0101\\u015Bcaiva",
        "kimakurvata", "san\\u0304java" };
    const int MAX_LEN = UPRV_LENGTHOF(source);
    const char* const expected[MAX_LEN] = { "\\u0930\\u094D\\u092E\\u094D\\u0915\\u094D\\u0937\\u0947\\u0924\\u094D",
        "\\u0936\\u094d\\u0930\\u0940\\u092e\\u0926\\u094d", "\\u092d\\u0917\\u0935\\u0926\\u094d\\u0917\\u0940\\u0924\\u093e",
        "\\u0905\\u0927\\u094d\\u092f\\u093e\\u092f", "\\u0905\\u0930\\u094d\\u091c\\u0941\\u0928", "\\u0935\\u093f\\u0937\\u093e\\u0926",
        "\\u092f\\u094b\\u0917", "\\u0927\\u0943\\u0924\\u0930\\u093e\\u0937\\u094d\\u091f\\u094d\\u0930", "\\u0909\\u0935\\u093E\\u091A\\u0943",
        "\\u0927\\u0930\\u094d\\u092e\\u0915\\u094d\\u0937\\u0947\\u0924\\u094d\\u0930\\u0947",
        "\\u0915\\u0941\\u0930\\u0941\\u0915\\u094d\\u0937\\u0947\\u0924\\u094d\\u0930\\u0947", "\\u0938\\u092e\\u0935\\u0947\\u0924\\u093e",
        "\\u092f\\u0941\\u092f\\u0941\\u0924\\u094d\\u0938\\u0935\\u0903", "\\u092e\\u093e\\u092e\\u0915\\u093e\\u0903",
        //  "\\u092a\\u093e\\u0923\\u094d\\u0921\\u0935\\u093e\\u0936\\u094d\\u091a\\u0948\\u0935",
        "\\u0915\\u093f\\u092e\\u0915\\u0941\\u0930\\u094d\\u0935\\u0924", "\\u0938\\u0902\\u091c\\u0935" };
...
void TransliteratorTest::TestGurmukhiDevanagari()
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    UnicodeSet vowel(
        UnicodeString("[\\u0905-\\u090A \\u090F\\u0910\\u0913\\u0914 \\u093e-\\u0942\\u0947\\u0948\\u094B\\u094C\\u094D]", -1, US_INV).unescape(), status);
...
void TransliteratorTest::TestLocaleResource()
{
    ...
    for (int32_t i = 0; i < DATA_length; i += 3) {
        ...
        expect(*t, CharsToUnicodeString(DATA[i + 1]), CharsToUnicodeString(DATA[i + 2]));
        delete t;
    }
}
...
void TransliteratorTest::TestParseError()
{
    ...
    static const char* maskingRule = "a>x;\n"
                                     "# more stuff\n"
                                     "ab>y;";
    ec = U_ZERO_ERROR;
...
void TransliteratorTest::TestIDForms()
{
    ...
    for (int32_t i = 0; i < DATA_length; i += 3) {
        ...
        if (t->getID() == expID && u->getID() == expInvID) {
            logln((UnicodeString) "Ok: " + ID + ".getInverse() => " + expInvID);
        } else {
            errln((UnicodeString) "FAIL: getInstance(" + ID + ") => " + t->getID() + " x getInverse() => " + u->getID() + ", expected " + expInvID);
        }
        delete t;
        delete u;
    }
...
void TransliteratorTest::checkRules(const UnicodeString& label, Transliterator& t2, const UnicodeString& testRulesForward)
{
    ...
    UnicodeString testRules(testRulesForward);
    testRules.findAndReplace(SPACE, EMPTY);
...
{
    const char* testRules = "::[[:Latin:][:Mark:]];"
                            "::NFKD (NFC);"
                            "::Lower (Lower);"
                            "a <> \\u03B1;" // alpha
                            "::NFKC (NFD);"
                            ...
                            "::Lower ();"
                            "::([[:Greek:][:Mark:]]);";
    const char* testRulesForward = "::[[:Latin:][:Mark:]];"
                                   "::NFKD(NFC);"
                                   "::Lower(Lower);"
                                   "a > \\u03B1;"
                                   "::NFKC(NFD);"
                                   "::Upper (Lower);"
                                   "::Lower ();";
    const char* testRulesBackward = "::[[:Greek:][:Mark:]];"
                                    "::Lower (Upper);"
                                    "::NFD(NFKC);"
                                    "\\u03B1 > a;"
                                    "::Lower(Lower);"
    ...
    UParseError pe;
    UErrorCode ec = U_ZERO_ERROR;
    LocalPointer<Transliterator> t2(Transliterator::createFromRules("source-target", UnicodeString(testRules, -1, US_INV), UTRANS_FORWARD, pe, ec));
    LocalPointer<Transliterator> t3(Transliterator::createFromRules("target-source", UnicodeString(testRules, -1, US_INV), UTRANS_REVERSE, pe, ec));
...
void TransliteratorTest::TestEscape()
{
    ...
    if (U_FAILURE(ec)) {
        ...
        expect(*t, UNICODE_STRING_SIMPLE("\\x{40}\\U00000031&#x32;&#81;"), "@12Q");
    }
    delete t;
    ...
    if (U_FAILURE(ec)) {
        ...
        expect(*t, CharsToUnicodeString("A\\U0010BEEF\\uFEED"), UNICODE_STRING_SIMPLE("\\u0041\\U0010BEEF\\uFEED"));
    }
    delete t;
    ...
    if (U_FAILURE(ec)) {
        ...
        expect(*t, CharsToUnicodeString("A\\U0010BEEF\\uFEED"), UNICODE_STRING_SIMPLE("\\u0041\\uDBEF\\uDEEF\\uFEED"));
    }
    delete t;
    ...
    if (U_FAILURE(ec)) {
        ...
        expect(*t, CharsToUnicodeString("A\\U0010BEEF\\uFEED"), UNICODE_STRING_SIMPLE("\\x{41}\\x{10BEEF}\\x{FEED}"));
    }
    delete t;
}
...
void TransliteratorTest::TestDisplayName()
{
    ...
    static const char* DATA[] = {
        ...
        // Variants
        "Any-Hex/Perl",
        "Any to Hex Escape/Perl",
        "Hex Escape to Any/Perl",
    ...
    for (int32_t i = 0; i < DATA_length; i += 3) {
        ...
        if (name != DATA[i + 1]) {
            dataerrln((UnicodeString) "FAIL: " + DATA[i] + ".getDisplayName() => " + name + ", expected " + DATA[i + 1]);
        } else {
        ...
        if (name != DATA[i + 2]) {
            dataerrln((UnicodeString) "FAIL: " + t->getID() + ".getDisplayName() => " + name + ", expected " + DATA[i + 2]);
        } else {
...
void TransliteratorTest::TestSpecialCases(void)
{
    const UnicodeString registerRules[] = {
        "Any-Dev1", "x > X; y > Y;", "Any-Dev2", "XY > Z", "Greek-Latin/FAKE",
        CharsToUnicodeString("[^[:L:][:M:]] { \\u03bc\\u03c0 > b ; \\u03bc\\u03c0 } [^[:L:][:M:]] > b ; [^[:L:][:M:]] { [\\u039c\\u03bc][\\u03a0\\u03c0] > B ; "
                             "[\\u039c\\u03bc][\\u03a0\\u03c0] } [^[:L:][:M:]] > B ;"),
        "" // END MARKER
    };
    ...
    const UnicodeString testCases[] = {
        ...
        "NFD", CharsToUnicodeString("a\\u0300 \\u00E0 \\u1100\\u1161 \\uFF76\\uFF9E\\u03D3"), "", "NFC",
        CharsToUnicodeString("a\\u0300 \\u00E0 \\u1100\\u1161 \\uFF76\\uFF9E\\u03D3"), "", "NFKD",
        CharsToUnicodeString("a\\u0300 \\u00E0 \\u1100\\u1161 \\uFF76\\uFF9E\\u03D3"), "", "NFKC",
        CharsToUnicodeString("a\\u0300 \\u00E0 \\u1100\\u1161 \\uFF76\\uFF9E\\u03D3"), "",

        // mp -> b BUG
        "Greek-Latin/UNGEGN", CharsToUnicodeString("(\\u03BC\\u03C0)"), "(b)", "Greek-Latin/FAKE", CharsToUnicodeString("(\\u03BC\\u03C0)"), "(b)",
        ...
        CharsToUnicodeString("AB'CD FFIII\\u0130 \\u01C7\\u01C7\\u01C7 ") + DESERET_DEE + DESERET_DEE, "Lower",
        CharsToUnicodeString("ab'cD \\uFB00i\\u0131I\\u0130 \\u01C7\\u01C8\\u01C9 ") + DESERET_dee + DESERET_DEE,
        CharsToUnicodeString("ab'cd \\uFB00i\\u0131ii\\u0307 \\u01C9\\u01C9\\u01C9 ") + DESERET_dee + DESERET_dee,

        "Upper", CharsToUnicodeString("ab'cD \\uFB00i\\u0131I\\u0130 \\u01C7\\u01C8\\u01C9 ") + DESERET_dee + DESERET_DEE, "", "Lower",
        CharsToUnicodeString("ab'cD \\uFB00i\\u0131I\\u0130 \\u01C7\\u01C8\\u01C9 ") + DESERET_dee + DESERET_DEE, "",

        // FORMS OF S
        "Greek-Latin/UNGEGN", CharsToUnicodeString("\\u03C3 \\u03C3\\u03C2 \\u03C2\\u03C3"), CharsToUnicodeString("s ss s\\u0331s\\u0331"),
        "Latin-Greek/UNGEGN", CharsToUnicodeString("s ss s\\u0331s\\u0331"), CharsToUnicodeString("\\u03C3 \\u03C3\\u03C2 \\u03C2\\u03C3"), "Greek-Latin",
        CharsToUnicodeString("\\u03C3 \\u03C3\\u03C2 \\u03C2\\u03C3"), CharsToUnicodeString("s ss s\\u0331s\\u0331"), "Latin-Greek",
        CharsToUnicodeString("s ss s\\u0331s\\u0331"), CharsToUnicodeString("\\u03C3 \\u03C3\\u03C2 \\u03C2\\u03C3"),
        // Tatiana bug
        // Upper: TAT\\u02B9\\u00C2NA
        // Lower: tat\\u02B9\\u00E2na
        // Title: Tat\\u02B9\\u00E2na
        "Upper", CharsToUnicodeString("tat\\u02B9\\u00E2na"), CharsToUnicodeString("TAT\\u02B9\\u00C2NA"), "Lower", CharsToUnicodeString("TAT\\u02B9\\u00C2NA"),
        CharsToUnicodeString("tat\\u02B9\\u00E2na"), "Title", CharsToUnicodeString("tat\\u02B9\\u00E2na"), CharsToUnicodeString("Tat\\u02B9\\u00E2na"),
    ...
    for (i = 0; registerRules[i].length() != 0; i += 2) {
        ...
        Transliterator* t = Transliterator::createFromRules(registerRules[0 + i], registerRules[i + 1], UTRANS_FORWARD, pos, status);
        if (U_FAILURE(status)) {
...
static void _trans(Transliterator& t, const UnicodeString& src, UnicodeString& result)
{
    result = src;
    t.transliterate(result);
}

static void _trans(const UnicodeString& id, const UnicodeString& src, UnicodeString& result, UErrorCode ec)
{
    UParseError pe;
    Transliterator* t = Transliterator::createInstance(id, UTRANS_FORWARD, pe, ec);
    if (U_SUCCESS(ec)) {
...
static UnicodeString _findMatch(const UnicodeString& source, const UnicodeString* pairs)
{
    UnicodeString empty;
...
void TransliteratorTest::TestIncrementalProgress(void)
{
    ...
    const UnicodeString tests[] = {
        "Any", latinTest, "Latin", latinTest, "Halfwidth", latinTest, "Devanagari", devaTest, "Katakana", kataTest,
        "" // END MARKER
    };
                ...
                if (U_FAILURE(status)) {
                    ...
                    if (id.compare((UnicodeString) "Devanagari-Arabic/") != 0
                        ...
                        && !(logKnownIssue("21911", "ICU4C cannot create inverse of Any-Xxxx/Variant transform created from both-direction transform")
                            && id.startsWith((UnicodeString) "Any-")
                            && (id.endsWith((UnicodeString) "/UNGEGN") || id.startsWith((UnicodeString) "Any-Ethiopic/")
                                || id.startsWith((UnicodeString) "Any-Braille/") || id.endsWith((UnicodeString) "/Gurage_2013")
                                || id.endsWith((UnicodeString) "/Gutgarts") || id.endsWith((UnicodeString) "/Tekie_Alibekit")
...
void TransliteratorTest::CheckIncrementalAux(const Transliterator* t, const UnicodeString& input)
{
    UErrorCode ec = U_ZERO_ERROR;
    UTransPosition pos;
    ...
    if (pos.start == 0 && pos.limit != 0 && t->getID() != "Hex-Any/Unicode") {
        errln((UnicodeString) "No Progress, " + t->getID() + ": " + formatInput(test, input, pos));
        gotError = TRUE;
    } else {
        logln((UnicodeString) "PASS Progress, " + t->getID() + ": " + formatInput(test, input, pos));
    }
    t->finishTransliteration(test, pos);
    if (pos.start != pos.limit) {
        errln((UnicodeString) "Incomplete, " + t->getID() + ": " + formatInput(test, input, pos));
        gotError = TRUE;
    }
}
...
void TransliteratorTest::TestFunction()
{
    ...
    // as toRules() is going to return it.  If toRules() changes
    // with regard to spacing or ';', then adjust this string.
    UnicodeString rule = "([:Lu:]) > $1 '(' &Lower( $1 ) '=' &Hex( &Any-Lower( $1 ) ) ')';";
    ...
    expect(*t, "The Quick Brown Fox", UNICODE_STRING_SIMPLE("T(t=\\u0074)he Q(q=\\u0071)uick B(b=\\u0062)rown F(f=\\u0066)ox"));
...
static Transliterator* U_EXPORT2 _TUFFactory(const UnicodeString& /*ID*/, Transliterator::Token context)
{
...
void TransliteratorTest::TestUserFunction()
{
    ...
    t = Transliterator::createFromRules(
        "gif", UNICODE_STRING_SIMPLE("'\\'u(..)(..) > '<img src=\"http://www.unicode.org/gifs/24/' $1 '/U' $1$2 '.gif\">';"), UTRANS_FORWARD, pe, ec);
    if (t == NULL || U_FAILURE(ec)) {
    ...
    t = Transliterator::createFromRules("RemoveCurly", UNICODE_STRING_SIMPLE("[\\{\\}] > ; '\\N' > ;"), UTRANS_FORWARD, pe, ec);
    if (t == NULL || U_FAILURE(ec)) {
    ...
    logln("Trying &hex");
    t = Transliterator::createFromRules("hex2", "(.) > &hex($1);", UTRANS_FORWARD, pe, ec);
    if (t == NULL || U_FAILURE(ec)) {
        errln("FAIL: createFromRules");
        goto FAIL;
    }
    ...
    logln("Trying &gif");
    t = Transliterator::createFromRules("gif2", "(.) > &Gif(&Hex2($1));", UTRANS_FORWARD, pe, ec);
    if (t == NULL || U_FAILURE(ec)) {
    ...
    t = Transliterator::createFromRules("test", "(.) > &Hex($1) ' ' &RemoveCurly(&Name($1)) ' ';", UTRANS_FORWARD, pe, ec);
    if (t == NULL || U_FAILURE(ec)) {
        errln((UnicodeString) "FAIL: createFromRules test " + u_errorName(ec));
        goto FAIL;
    }
    expect(*t, "abc", UNICODE_STRING_SIMPLE("\\u0061 LATIN SMALL LETTER A \\u0062 LATIN SMALL LETTER B \\u0063 LATIN SMALL LETTER C "));
    delete t;
...
void TransliteratorTest::TestAnyX(void)
{
    ...
    expect(*anyLatin, CharsToUnicodeString("greek:\\u03B1\\u03B2\\u03BA\\u0391\\u0392\\u039A hiragana:\\u3042\\u3076\\u304F cyrillic:\\u0430\\u0431\\u0446"),
...
void TransliteratorTest::TestSourceTargetSet()
{
    ...
    if (src == expSrc && trg == expTrg) {
        UnicodeString a, b;
        logln((UnicodeString) "Ok: " + r + " => source = " + src.toPattern(a, TRUE) + ", target = " + trg.toPattern(b, TRUE));
    } else {
        UnicodeString a, b, c, d;
        errln((UnicodeString) "FAIL: " + r + " => source = " + src.toPattern(a, TRUE) + ", expected " + expSrc.toPattern(b, TRUE)
            + "; target = " + trg.toPattern(c, TRUE) + ", expected " + expTrg.toPattern(d, TRUE));
    }
...
void TransliteratorTest::TestAlternateSyntax()
{
    ...
    expect(CharsToUnicodeString("([:^ASCII:]) \\u2192 \\u2206Name($1);"), CharsToUnicodeString("<=\\u2190; >=\\u2192; <>=\\u2194; &=\\u2206"),
...
static const char* BEGIN_END_TEST_CASES[] = {
    ...
    BEGIN_END_RULES[4], "abc ababc aba", "xy abxy z", BEGIN_END_RULES[5], "abccabaacababcbc", "PXAARXQBR",

    BEGIN_END_RULES[6], "e   e - e---e-  e", "e e e-e-e", BEGIN_END_RULES[7], "e   e - e---e-  e", "e e e-e-e", BEGIN_END_RULES[8], "e   e - e---e-  e",
    "e e e-e-e", BEGIN_END_RULES[9], "e   e - e---e-  e", "e e e-e-e",
    ...
    BEGIN_END_RULES[13], "e   e - e---e-  e", "e e e-e-e", BEGIN_END_RULES[13], "a    a    a    a", "a%a%a%a", BEGIN_END_RULES[13], "a a-b c b a", "a%a-b cb-a",
...
void TransliteratorTest::TestBeginEnd()
{
    ...
    for (i = 0; i < BEGIN_END_TEST_CASES_length; i += 3) {
        expect((UnicodeString) "Test case #" + (i / 3), UnicodeString(BEGIN_END_TEST_CASES[i], -1, US_INV),
            UnicodeString(BEGIN_END_TEST_CASES[i + 1], -1, US_INV), UnicodeString(BEGIN_END_TEST_CASES[i + 2], -1, US_INV));
    }

    // instantiate the one reversible rule set in the reverse direction and make sure it does the right thing
    UParseError parseError;
    UErrorCode status = U_ZERO_ERROR;
    Transliterator* reversed = Transliterator::createFromRules("Reversed", UnicodeString(BEGIN_END_RULES[17]), UTRANS_REVERSE, parseError, status);
...
void TransliteratorTest::TestBeginEndToRules()
{
    ...
    for (int32_t i = 0; i < BEGIN_END_TEST_CASES_length; i += 3) {
        UParseError parseError;
        UErrorCode status = U_ZERO_ERROR;
        Transliterator* t = Transliterator::createFromRules("--", UnicodeString(BEGIN_END_TEST_CASES[i], -1, US_INV), UTRANS_FORWARD, parseError, status);
        if (U_FAILURE(status)) {
            reportParseError(UnicodeString("FAIL: Couldn't create transliterator"), parseError, status);
        } else {
            UnicodeString rules;
            t->toRules(rules, TRUE);
            Transliterator* t2 = Transliterator::createFromRules((UnicodeString) "Test case #" + (i / 3), rules, UTRANS_FORWARD, parseError, status);
            if (U_FAILURE(status)) {
                reportParseError(UnicodeString("FAIL: Couldn't create transliterator from generated rules"), parseError, status);
                delete t;
            } else {
                expect(*t2, UnicodeString(BEGIN_END_TEST_CASES[i + 1], -1, US_INV), UnicodeString(BEGIN_END_TEST_CASES[i + 2], -1, US_INV));
                delete t;
                delete t2;
            }
        }
    ...
    if (U_FAILURE(status)) {
        ...
        Transliterator* reversed2 = Transliterator::createFromRules("Reversed", rules, UTRANS_FORWARD, parseError, status);
        if (U_FAILURE(status)) {
            reportParseError(UnicodeString("FAIL: Couldn't create reversed transliterator from generated rules"), parseError, status);
            delete reversed;
        } else {
            expect(*reversed2, UnicodeString("xy XY XYZ yz YZ"), UnicodeString("xy abc xaba yz aba"));
            delete reversed;
            delete reversed2;
        }
    }
...
\uE001>\u0C01; # SIGN
    */
    static const UChar rule[] = { 0x0023, 0x0020, 0x000D, 0x000A, 0xE001, 0x003E, 0x0C01, 0x003B, 0x0020, 0x0023, 0x0020, 0x0053, 0x0049, 0x0047, 0x004E, 0 };
    static const UChar expectedRule[] = { 0xE001, 0x003E, 0x0C01, 0x003B, 0 };
...
void TransliteratorTest::TestThai(void)
{
    ...
    const char* thaiText = "\\u0e42\\u0e14\\u0e22\\u0e1e\\u0e37\\u0e49\\u0e19\\u0e10\\u0e32\\u0e19\\u0e41\\u0e25\\u0e49\\u0e27, \\u0e04\\u0e2d"
    ...
    const char* latinText = "doy ph\\u1ee5\\u0304\\u0302n \\u1e6d\\u0304h\\u0101n l\\u00e6\\u0302w, khxmphiwtexr\\u0312 ca ke\\u012b\\u0300"
...
void TransliteratorTest::expect(
    const UnicodeString& id, const UnicodeString& rules, const UnicodeString& source, const UnicodeString& expectedResult, UTransPosition* pos)
{
    UErrorCode status = U_ZERO_ERROR;
    UParseError parseError;
    Transliterator* t = Transliterator::createFromRules(id, rules, UTRANS_FORWARD, parseError, status);
...
void TransliteratorTest::expect(
    const Transliterator& t, const UnicodeString& source, const UnicodeString& expectedResult, const Transliterator& reverseTransliterator)
{
    expect(t, source, expectedResult);
...
UnicodeString& TransliteratorTest::formatInput(UnicodeString& appendTo, const UnicodeString& input, const UTransPosition& pos)
{
    ...
    if (0 <= pos.contextStart && pos.contextStart <= pos.start && pos.start <= pos.limit && pos.limit <= pos.contextLimit
        ...
        appendTo.append(a).append((UChar)123 /*{*/).append(b).append((UChar)PIPE).append(c).append((UChar)PIPE).append(d).append((UChar)125 /*}*/).append(e);
    } else {
        appendTo.append((UnicodeString) "INVALID UTransPosition {cs=" + pos.contextStart + ", s=" + pos.start + ", l=" + pos.limit + ", cl=" + pos.contextLimit
            + "} on " + input);
    }
    return appendTo;
}
...
void TransliteratorTest::expectAux(const UnicodeString& tag, const UnicodeString& summary, UBool pass, const UnicodeString& expectedResult)
{
    if (pass) {
