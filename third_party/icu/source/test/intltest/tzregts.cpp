...
void TimeZoneRegressionTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
...
UBool TimeZoneRegressionTest::failure(UErrorCode status, const char* msg)
{
    if (U_FAILURE(status)) {
...
UDate TimeZoneRegressionTest::findTransitionBinary(const SimpleTimeZone& tz, UDate min, UDate max)
{
    ...
    if (failure(status, "SimpleTimeZone::inDaylightTime"))
        return 0;
...
void TimeZoneRegressionTest::Test4084933()
{
    ...
    int32_t offset1 = tz->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (2 * 60 * 60 * 1000), status);
    int32_t offset2 = tz->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (2 * 60 * 60 * 1000) - 1, status);

    int32_t offset3 = tz->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (1 * 60 * 60 * 1000), status);
    int32_t offset4 = tz->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (1 * 60 * 60 * 1000) - 1, status);

    /*
     *  The following was added just for consistency.  It shows that going *to* Daylight
     *  Savings Time (PDT) does work at 2am.
     */
    int32_t offset5 = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (2 * 60 * 60 * 1000), status);
    int32_t offset6 = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (2 * 60 * 60 * 1000) - 1, status);
    int32_t offset5a = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (3 * 60 * 60 * 1000), status);
    int32_t offset6a = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (3 * 60 * 60 * 1000) - 1, status);
    int32_t offset7 = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (1 * 60 * 60 * 1000), status);
    int32_t offset8 = tz->getOffset(1, 1997, UCAL_APRIL, 6, UCAL_SUNDAY, (1 * 60 * 60 * 1000) - 1, status);
    int32_t SToffset = (int32_t)(-8 * 60 * 60 * 1000L);
    int32_t DToffset = (int32_t)(-7 * 60 * 60 * 1000L);

#define ERR_IF_FAIL(x)                                                                                                                                         \
    if (x) {                                                                                                                                                   \
        dataerrln("FAIL: TimeZone misbehaving - %s", #x);                                                                                                      \
    }
...
void TimeZoneRegressionTest::Test4109314()
{
    ...
    UDate testData[] = { CalendarRegressionTest::makeDate(98, UCAL_APRIL, 4, 22, 0), CalendarRegressionTest::makeDate(98, UCAL_APRIL, 5, 6, 0),
        CalendarRegressionTest::makeDate(98, UCAL_OCTOBER, 24, 22, 0), CalendarRegressionTest::makeDate(98, UCAL_OCTOBER, 25, 6, 0) };
    UBool pass = TRUE;
...
UBool TimeZoneRegressionTest::checkCalendar314(GregorianCalendar* testCal, TimeZone* testTZ)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    UDate millis = testCal->get(UCAL_MILLISECOND, status)
        + 1000.0 * (testCal->get(UCAL_SECOND, status) + 60.0 * (testCal->get(UCAL_MINUTE, status) + 60.0 * (testCal->get(UCAL_HOUR_OF_DAY, status))))
    ...
    tzOffset = testTZ->getOffset((uint8_t)testCal->get(UCAL_ERA, status), testCal->get(UCAL_YEAR, status), testCal->get(UCAL_MONTH, status), date, (uint8_t)dow,
        (int32_t)millis, status);
    ...
    LocalPointer<SimpleDateFormat> sdf(new SimpleDateFormat((UnicodeString) "MM/dd/yyyy HH:mm", status), status);
    if (U_FAILURE(status)) {
    ...
    output = testTZ->getID(output) + " " + sdf->format(testDate, output, pos) + " Offset(" + tzOffsetFloat + ")" + " RawOffset(" + tzRawOffsetFloat + ")" + " "
        + millis / (float)3600000 + " " + inDaylightTimeString;
...
void TimeZoneRegressionTest::Test4126678()
{
    ...
    int32_t millis = cal->get(UCAL_MILLISECOND, status)
        + (cal->get(UCAL_SECOND, status) + (cal->get(UCAL_MINUTE, status) + (cal->get(UCAL_HOUR, status) * 60) * 60) * 1000)
...
void TimeZoneRegressionTest::Test4151406()
{
    ...
    for (int32_t h = -28; h <= 30; ++h) {
        ...
        int32_t hh = (h < 0) ? -h : h;
        UnicodeString hname = UnicodeString((h < 0) ? "GMT-" : "GMT+") + ((hh / 2 < 10) ? "0" : "") + (hh / 2) + ':' + ((hh % 2 == 0) ? "00" : "30");
        // try {
...
void TimeZoneRegressionTest::Test4154542()
{
    ...
    for (int32_t i = 0; i < 18 * 5; i += 5) {
        ...
        if (U_SUCCESS(status) != shouldBeGood) {
            errln(UnicodeString("setStartRule(month=") + month + ", day=" + day + ", dayOfWeek=" + dayOfWeek + ", time=" + time
                + (shouldBeGood ? (") should work") : ") should fail but doesn't"));
        }
        ...
        //}
        if (U_SUCCESS(status) != shouldBeGood) {
            errln(UnicodeString("setEndRule(month=") + month + ", day=" + day + ", dayOfWeek=" + dayOfWeek + ", time=" + time
                + (shouldBeGood ? (") should work") : ") should fail but doesn't"));
        }
        ...
        SimpleTimeZone* temp = new SimpleTimeZone(
            0, "Z", (int8_t)month, (int8_t)day, (int8_t)dayOfWeek, time, (int8_t)GOOD_MONTH, (int8_t)GOOD_DAY, (int8_t)GOOD_DAY_OF_WEEK, GOOD_TIME, status);
        if (temp == nullptr) {
            errln("Fail: failed to create SimpleTimeZone %s", u_errorName(status));
            return;
        }
        ...
        //}
        if (U_SUCCESS(status) != shouldBeGood) {
            errln(UnicodeString("SimpleTimeZone(month=") + month + ", day=" + day + ", dayOfWeek=" + dayOfWeek + ", time=" + time
        ...
        // try {
        status = U_ZERO_ERROR;
        temp = new SimpleTimeZone(
            0, "Z", (int8_t)GOOD_MONTH, (int8_t)GOOD_DAY, (int8_t)GOOD_DAY_OF_WEEK, GOOD_TIME, (int8_t)month, (int8_t)day, (int8_t)dayOfWeek, time, status);
        if (temp == nullptr) {
            errln("Fail: failed to create SimpleTimeZone %s", u_errorName(status));
            return;
        }
        ...
        //}
        if (U_SUCCESS(status) != shouldBeGood) {
            errln(UnicodeString("SimpleTimeZone(<start>, month=") + month + ", day=" + day + ", dayOfWeek=" + dayOfWeek + ", time=" + time
...
void TimeZoneRegressionTest::Test4154525()
{
    ...
    int32_t DATA[] = {
        1, GOOD, 0, BAD, -1, GOOD, // #13566 updates SimpleTimeZone to support negative DST saving amount
        60 * 60 * 1000, GOOD, INT32_MAX, GOOD, // no upper limit on DST savings at this time
    ...
    for (int32_t i = 0; i < 10; i += 2) {
        ...
        for (int32_t j = 0; j < 2; ++j) {
            ...
            case 0:
                method = "constructor";
                z.adoptInsteadAndCheckErrorCode(
                    new SimpleTimeZone(0, "id", UCAL_JANUARY, 1, 0, 0, UCAL_MARCH, 1, 0, 0, savings, status), status); // <- what we're interested in
                break;
            case 1:
...
void TimeZoneRegressionTest::Test4154650()
{
    ...
    for (int32_t i = 0; i < dataLen; i += 7) {
        ...
        // try {
        /*int32_t offset = */
        tz->getOffset((uint8_t)DATA[i + 1], DATA[i + 2], DATA[i + 3], DATA[i + 4], (uint8_t)DATA[i + 5], DATA[i + 6], status);
        ...
        if (good != U_SUCCESS(status)) {
            ...
            errln(UnicodeString("Fail: getOffset(") + DATA[i + 1] + ", " + DATA[i + 2] + ", " + DATA[i + 3] + ", " + DATA[i + 4] + ", " + DATA[i + 5] + ", "
                + DATA[i + 6] + errMsg);
        }
...
void TimeZoneRegressionTest::Test4162593()
{
    ...
    LocalPointer<SimpleTimeZone> asuncion(new SimpleTimeZone(-4 * ONE_HOUR, "America/Asuncion" /*PY%sT*/, UCAL_OCTOBER, 1, 0 /*DOM*/, 0 * ONE_HOUR, UCAL_MARCH,
                                              1, 0 /*DOM*/, 0 * ONE_HOUR, 1 * ONE_HOUR, status),
        status);
    ...
    DATA_TZ[0] = new SimpleTimeZone(
        2 * ONE_HOUR, "Asia/Damascus" /*EE%sT*/, UCAL_APRIL, 1, 0 /*DOM*/, 0 * ONE_HOUR, UCAL_OCTOBER, 1, 0 /*DOM*/, 0 * ONE_HOUR, 1 * ONE_HOUR, status);
    DATA_TZ[1] = asuncion.getAlias();
    DATA_TZ[2] = asuncion.getAlias();
    ...
    for (int32_t j = 0; j < 3; j++) {
        ...
        if (zone[0] == zone[1] && (zone[1] == zone[2]) != transitionExpected && zone[2] == zone[3]) {
...
void TimeZoneRegressionTest::Test4176686()
{
    ...
    if (!assertSuccess("trying to construct", status))
        return;
    ...
    UnicodeString DATA[] = {
        ...
        z1.getDisplayName(FALSE, TimeZone::SHORT, a),
        "GMT+1:30",
        "z1.getDisplayName(false, LONG)/std zone",
        z1.getDisplayName(FALSE, TimeZone::LONG, b),
        "GMT+01:30",
        "z1.getDisplayName(true, SHORT)/std zone",
        z1.getDisplayName(TRUE, TimeZone::SHORT, c),
        "GMT+1:30",
        "z1.getDisplayName(true, LONG)/std zone",
        z1.getDisplayName(TRUE, TimeZone::LONG, d),
        "GMT+01:30",
        "z2.getDisplayName(false, SHORT)/dst zone",
        z2.getDisplayName(FALSE, TimeZone::SHORT, e),
        "GMT+1:30",
        "z2.getDisplayName(false, LONG)/dst zone",
        z2.getDisplayName(FALSE, TimeZone::LONG, f),
        "GMT+01:30",
        "z2.getDisplayName(true, SHORT)/dst zone",
        z2.getDisplayName(TRUE, TimeZone::SHORT, g),
        "GMT+2:15",
        "z2.getDisplayName(true, LONG)/dst zone",
        z2.getDisplayName(TRUE, TimeZone::LONG, h),
        "GMT+02:15",
...
void TimeZoneRegressionTest::TestJ449()
{
    ...
    if (zoneWith->getID(str) != UnicodeString(idWithLocaleData) || zoneWithout->getID(str) != UnicodeString(idWithoutLocaleData)) {
        dataerrln(UnicodeString("Fail: Unable to create zones - wanted ") + idWithLocaleData + ", got " + zoneWith->getID(str) + ", and wanted "
            + idWithoutLocaleData + " but got " + zoneWithout->getID(str));
    } else {
        ...
        if (U_FAILURE(status)) {
            ...
            if (strWith == strWithout) {
                logln((UnicodeString) "Ok: " + idWithLocaleData + " -> " + strWith + "; " + idWithoutLocaleData + " -> " + strWithout);
            } else {
                errln((UnicodeString) "FAIL: " + idWithLocaleData + " -> " + strWith + "; " + idWithoutLocaleData + " -> " + strWithout);
            }
        }
    }
...
void TimeZoneRegressionTest::TestJDK12API()
{
    ...
    LocalPointer<TimeZone> pst(new SimpleTimeZone(-28800 * U_MILLIS_PER_SECOND, "PST", 3, 1, -1, 120 * U_MILLIS_PER_MINUTE, SimpleTimeZone::WALL_TIME, 9, -1, 1,
                                   120 * U_MILLIS_PER_MINUTE, SimpleTimeZone::WALL_TIME, 60 * U_MILLIS_PER_MINUTE, ec),
        ec);
    // d,-21600,3,1,-1,120,w,9,-1,1,120,w,60
    LocalPointer<TimeZone> cst1(new SimpleTimeZone(-21600 * U_MILLIS_PER_SECOND, "CST", 3, 1, -1, 120 * U_MILLIS_PER_MINUTE, SimpleTimeZone::WALL_TIME, 9, -1,
                                    1, 120 * U_MILLIS_PER_MINUTE, SimpleTimeZone::WALL_TIME, 60 * U_MILLIS_PER_MINUTE, ec),
        ec);
    if (U_FAILURE(ec)) {
    ...
    UErrorCode status = U_ZERO_ERROR;
    int32_t offset1 = pst->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (2 * 60 * 60 * 1000), status);
    failure(status, "getOffset() failed");

    int32_t offset2 = cst->getOffset(1, 1997, UCAL_OCTOBER, 26, UCAL_SUNDAY, (2 * 60 * 60 * 1000), 31, status);
    failure(status, "getOffset() failed");
    ...
    pst->getOffset(1, 1997, UCAL_FIELD_COUNT + 1, 26, UCAL_SUNDAY, (2 * 60 * 60 * 1000), status);
    if (U_SUCCESS(status))
...
void TimeZoneRegressionTest::TestNegativeDaylightSaving()
{
    ...
    SimpleTimeZone stzDublin(
        stdOff, "Dublin-2018", UCAL_OCTOBER, -1, -UCAL_SUNDAY, 2 * 60 * 60 * 1000, UCAL_MARCH, -1, -UCAL_SUNDAY, 1 * 60 * 60 * 1000, save, status);
    ...
    if (rawOffset != stdOff || dstOffset != save) {
        errln((UnicodeString) "FAIL: Expected [stdoff=" + stdOff + ",save=" + save + "] on the test date (Jan 15), actual[stdoff=" + rawOffset
            + ",save=" + dstOffset + "]");
    }
    ...
    if (rawOffset != stdOff || dstOffset != 0) {
        errln((UnicodeString) "FAIL: Expected [stdoff=" + stdOff + ",save=" + 0 + "] on the test date (Jul 15), actual[stdoff=" + rawOffset
            + ",save=" + dstOffset + "]");
    }
}
