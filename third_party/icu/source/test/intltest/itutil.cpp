...
static void RefPlusOne(UErrorCode& code)
{
    code = (UErrorCode)(code + 1);
}
static void PtrPlusTwo(UErrorCode* code)
{
    *code = (UErrorCode)(*code + 2);
}
...
void ErrorCodeTest::TestErrorCode()
{
    ...
    if (errorCode.reset() != U_PARSE_ERROR || errorCode.get() != U_ZERO_ERROR || !errorCode.isSuccess() || errorCode.isFailure()) {
...
void ErrorCodeTest::TestSubclass()
{
    ...
    int32_t countDests = 0;
    {
        ...
        if (errorCode.get() != U_ZERO_ERROR || !errorCode.isSuccess() || errorCode.isFailure() || countChecks != 0 || countDests != 0) {
            errln("ErrorCode did not initialize properly");
            return;
        }
        errorCode.assertSuccess();
...
template <typename T> void moveFrom(T& dest, T& src)
{
    dest = std::move(src);
}
...
void LocalPointerTest::TestLocalXyzPointer()
{
    ...
    LocalUConverterSelectorPointer sel(ucnvsel_open(&encoding, 1, NULL, UCNV_ROUNDTRIP_SET, errorCode));
    ...
    LocalUMessageFormatPointer msg(umsg_open(hello.getBuffer(), hello.length(), "root", NULL, errorCode));
    ...
    LocalURegularExpressionPointer regex(uregex_open(pattern.getBuffer(), pattern.length(), 0, NULL, errorCode));
    ...
    LocalUTransliteratorPointer trans(utrans_openU(id.getBuffer(), id.length(), UTRANS_FORWARD, NULL, 0, NULL, errorCode));
...
void LocalPointerTest::TestLocalXyzStdUniquePtr()
{
    ...
    // Implicit conversion operator
    std::unique_ptr<UConstrainedFieldPosition, void (*)(UConstrainedFieldPosition*)> a = LocalUConstrainedFieldPositionPointer(ptr);
    // Explicit move constructor
...
enum myEnum { MAX_NONBOOLEAN = -1, THING1, THING2, THING3, LIMIT_BOOLEAN };

void EnumSetTest::TestEnumSet()
{
    EnumSet<myEnum, MAX_NONBOOLEAN + 1, LIMIT_BOOLEAN> flags;
