...
void ModifiersTest::testSimpleModifier()
{
    ...
    static const char16_t* expectedCharFields[][2]
        = { { u"|", u"n" }, { u"X|Y", u"%n%" }, { u"XX|YYY", u"%%n%%%" }, { u"|YY", u"n%%" }, { u"XX📺XX|", u"%%%%%%n" } };
    static const char16_t* expecteds[][NUM_CASES] = // force auto-format line break
        { { u"", u"XY", u"XXYYY", u"YY", u"XX📺XX" }, { u"a📻bcde", u"XYa📻bcde", u"XXYYYa📻bcde", u"YYa📻bcde", u"XX📺XXa📻bcde" },
    ...
    for (int32_t i = 0; i < NUM_CASES; i++) {
        ...
        assertModifierEquals(mod, prefixLens[i], false, expectedCharFields[i][0], expectedCharFields[i][1], status);
        assertSuccess("Spot 2", status);
...
void ModifiersTest::assertModifierEquals(
    const Modifier& mod, int32_t expectedPrefixLength, bool expectedStrong, UnicodeString expectedChars, UnicodeString expectedFields, UErrorCode& status)
{
    ...
    assertModifierEquals(mod, sb, expectedPrefixLength, expectedStrong, expectedChars, expectedFields, status);
}

void ModifiersTest::assertModifierEquals(const Modifier& mod, FormattedStringBuilder& sb, int32_t expectedPrefixLength, bool expectedStrong,
    UnicodeString expectedChars, UnicodeString expectedFields, UErrorCode& status)
{
    ...
    if (dynamic_cast<const CurrencySpacingEnabledModifier*>(&mod) == nullptr) {
        ...
        assertEquals("Code point count equals actual code point count", sb.codePointCount() - oldCount, mod.getCodePointCount());
    }
