...
#define TEST_ASSERT_SUCCESS(errcode)                                                                                                                           \
    ...
    {                                                                                                                                                          \
        if (U_FAILURE(errcode)) {                                                                                                                              \
            dataerrln("Failure in file %s, line %d, test ID \"%s\", status = \"%s\"", __FILE__, __LINE__, testId, u_errorName(errcode));                       \
        }                                                                                                                                                      \
    }                                                                                                                                                          \
    UPRV_BLOCK_MACRO_END
...
void SSearchTest::searchTest()
{
    ...
    while ((testCase = root->nextChildElement(tc)) != NULL) {
        ...
        *testId = 0;
        ...
        if (strength == NULL) {
            collatorStrength = UCOL_TERTIARY;
        } else if (*strength == "PRIMARY") {
            collatorStrength = UCOL_PRIMARY;
        } else if (*strength == "SECONDARY") {
            collatorStrength = UCOL_SECONDARY;
        } else if (*strength == "TERTIARY") {
            collatorStrength = UCOL_TERTIARY;
        } else if (*strength == "QUATERNARY") {
            collatorStrength = UCOL_QUATERNARY;
        } else if (*strength == "IDENTICAL") {
            collatorStrength = UCOL_IDENTICAL;
        } else {
        ...
        LocalUStringSearchPointer uss(usearch_openFromCollator(pattern.getBuffer(), pattern.length(), target.getBuffer(), target.length(), collator.getAlias(),
        ...
        TEST_ASSERT_SUCCESS(status);
        if ((foundMatch && expectedMatchStart < 0) || (foundStart != expectedMatchStart) || (foundLimit != expectedMatchLimit)) {
        ...
        uss.adoptInstead(
            usearch_openFromCollator(pattern.getBuffer(), pattern.length(), target.getBuffer(), target.length(), collator.getAlias(), NULL, &status));
...
UBool OrderList::compare(const OrderList& other) const
{
    ...
    for (int32_t i = 0; i < listSize; i += 1) {
        if (list[i].order != other.list[i].order || list[i].lowOffset != other.list[i].lowOffset || list[i].highOffset != other.list[i].highOffset) {
            return FALSE;
        }
    }
...
void SSearchTest::sharpSTest()
{
    ...
    UnicodeString targets[] = { "fu\\u00DF", "fu\\u00DFball", "1fu\\u00DFball", "12fu\\u00DFball", "123fu\\u00DFball", "1234fu\\u00DFball", "ffu\\u00DF",
        "fufu\\u00DF", "fusfu\\u00DF", "fuss", "ffuss", "fufuss", "fusfuss", "1fuss", "12fuss", "123fuss", "1234fuss", "fu\\u00DF", "1fu\\u00DF", "12fu\\u00DF",
        "123fu\\u00DF", "1234fu\\u00DF" };
    ...
    LocalUStringSearchPointer ussLong(usearch_openFromCollator(lpUnescaped.getBuffer(), lpUnescaped.length(), lpUnescaped.getBuffer(),
        lpUnescaped.length(), // actual test data will be set later
        coll,
        NULL, // the break iterator
        &status));

    LocalUStringSearchPointer ussShort(usearch_openFromCollator(spUnescaped.getBuffer(), spUnescaped.length(), spUnescaped.getBuffer(),
...
void SSearchTest::goodSuffixTest()
{
    ...
    LocalUStringSearchPointer ss(usearch_openFromCollator(pat.getBuffer(), pat.length(), target.getBuffer(), target.length(), coll,
        NULL, // the break iterator
        &status));
    TEST_ASSERT_SUCCESS(status);
...
{
    static const char* longishText = "Whylom, as olde stories tellen us,\n"
    ...
    LocalUStringSearchPointer uss(usearch_openFromCollator(uPattern.getBuffer(), uPattern.length(), target.getBuffer(), target.length(), collator.getAlias(),
        NULL, // the break iterator
        &status));
    TEST_ASSERT_SUCCESS(status);
...
static UBool simpleSearch(UCollator* coll, const UnicodeString& target, int32_t offset, const UnicodeString& pattern, int32_t& matchStart, int32_t& matchEnd)
{
    ...
    UBreakIterator* charBreakIterator
        = ubrk_open(UBRK_CHARACTER, ucol_getLocaleByType(coll, ULOC_VALID_LOCALE, &status), target.getBuffer(), target.length(), &status);
    ...
    for (int32_t i = 0; i < targetSize; i += 1) {
        if (targetOrders.matchesAt(i, patternOrders)) {
            ...
            if (maxLimit == targetOrders.getHighOffset(i + patternSize) && targetOrders.getOrder(i + patternSize) != UCOL_NULLORDER) {
                continue;
            }
    ...
    const char* name, const char* strength, uint32_t seed)
{
    ...
    LocalUStringSearchPointer uss(usearch_openFromCollator(pattern.getBuffer(), pattern.length(), testCase.getBuffer(), testCase.length(), coll,
        NULL, // the break iterator
        &status));
...
void SSearchTest::monkeyTest(char* params)
{
    ...
    Monkey* monkeys[] = { &letterMonkey, &contractionMonkey, &expansionMonkey, &contractionMonkey, &expansionMonkey, &contractionMonkey, &expansionMonkey,
