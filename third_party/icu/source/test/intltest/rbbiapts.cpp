...
void RBBIAPITest::TestCloneEquals()
{
    ...
    *bix = *bi2;
    ...
    if (*bi1clone != *bi1 || *bi1clone != *biequal || *bi1clone == *bi3 || *bi1clone == *bi2)
        errln("%s:%d ERROR:1 RBBI's clone() method failed", __FILE__, __LINE__);

    if (*bi2clone == *bi1 || *bi2clone == *biequal || *bi2clone == *bi3 || *bi2clone != *bi2)
        errln("%s:%d ERROR:2 RBBI's clone() method failed", __FILE__, __LINE__);

    if (bi1->getText() != bi1clone->getText() || bi2clone->getText() != bi2->getText() || *bi2clone == *bi1clone)
...
void RBBIAPITest::TestgetRules()
{
    ...
    LocalPointer<RuleBasedBreakIterator> bi1((RuleBasedBreakIterator*)RuleBasedBreakIterator::createCharacterInstance(Locale::getDefault(), status), status);
    LocalPointer<RuleBasedBreakIterator> bi2((RuleBasedBreakIterator*)RuleBasedBreakIterator::createWordInstance(Locale::getDefault(), status), status);
    if (U_FAILURE(status)) {
...
void RBBIAPITest::TestHashCode()
{
    ...
    if (bi1->hashCode() != bi1clone->hashCode() || bi1->hashCode() != bi3->hashCode() || bi1clone->hashCode() != bi3->hashCode()
        || bi2->hashCode() != bi2clone->hashCode())
        errln((UnicodeString) "ERROR: identical objects have different hashcodes");

    if (bi1->hashCode() == bi2->hashCode() || bi2->hashCode() == bi3->hashCode() || bi1clone->hashCode() == bi2clone->hashCode()
...
void RBBIAPITest::TestGetSetAdoptText()
{
    ...
    if (wordIter1->current() != 0)
        errln((UnicodeString) "ERROR:1 setText did not set the iteration position to the beginning of the text, it is" + wordIter1->current()
            + (UnicodeString) "\n");
    ...
    if (wordIter1->current() != 0)
        errln((UnicodeString) "ERROR:2 setText did not reset the iteration position to the beginning of the text, it is" + wordIter1->current()
            + (UnicodeString) "\n");
...
void RBBIAPITest::TestRuleStatus()
{
    ...
    int32_t tag_lo[] = { UBRK_WORD_NONE, UBRK_WORD_LETTER, UBRK_WORD_NONE, UBRK_WORD_LETTER, UBRK_WORD_NONE, UBRK_WORD_NUMBER, UBRK_WORD_NONE, UBRK_WORD_IDEO,
        UBRK_WORD_NONE };

    int32_t tag_hi[] = { UBRK_WORD_NONE_LIMIT, UBRK_WORD_LETTER_LIMIT, UBRK_WORD_NONE_LIMIT, UBRK_WORD_LETTER_LIMIT, UBRK_WORD_NONE_LIMIT,
        UBRK_WORD_NUMBER_LIMIT, UBRK_WORD_NONE_LIMIT, UBRK_WORD_IDEO_LIMIT, UBRK_WORD_NONE_LIMIT };
    ...
    if (U_FAILURE(status)) {
        ...
        for (i = 0; i < 3; i++) {
            switch (i) {
            case 0:
                success = pos == 0 && tag == UBRK_LINE_SOFT;
                break;
            case 1:
                success = pos == 5 && tag == UBRK_LINE_SOFT;
                break;
            case 2:
                success = pos == 12 && tag == UBRK_LINE_HARD;
                break;
            default:
                success = FALSE;
                break;
            ...
            if (success == FALSE) {
                errln("%s:%d: incorrect line break status or position.  i=%d, pos=%d, tag=%d", __FILE__, __LINE__, i, pos, tag);
                break;
            }
            pos = bi->next();
            tag = bi->getRuleStatus();
        }
        if (UBRK_LINE_SOFT >= UBRK_LINE_SOFT_LIMIT || UBRK_LINE_HARD >= UBRK_LINE_HARD_LIMIT
...
void RBBIAPITest::TestFilteredBreakIteratorBuilder()
{
    ...
    const UnicodeString text(
        "In the meantime Mr. Weston arrived with his small ship, which he had now recovered. Capt. Gorges, who informed the Sgt. here that one purpose of his "
        "going east was to "
        "meet with Mr. Weston, took this opportunity to call him to account for some abuses he had to lay to his charge."); // (William Bradford, public domain.
...
{
    if (exec)
        logln((UnicodeString) "TestSuite RuleBasedBreakIterator API ");
    TESTCASE_AUTO_BEGIN;
