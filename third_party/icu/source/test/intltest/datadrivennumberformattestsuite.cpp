    ...
    while (U_SUCCESS(status)) {
        ...
        if (state == 0) {
            ...
            for (int32_t i = 0; i < columnCount; ++i) {
                columnTypes[i] = NumberFormatTestTuple::getFieldByName(columnValues[i]);
            ...
            for (int32_t i = 0; i < columnsInThisRow; ++i) {
                fTuple.setField(columnTypes[i], columnValues[i].unescape(), status);
            }
            ...
            if (runAllTests || !breaksC()) {
                ...
                UBool shouldFail = (NFTT_GET_FIELD(fTuple, output, "") == "fail") ? !breaksC() : breaksC();
...
void DataDrivenNumberFormatTestSuite::setTupleField(UErrorCode& status)
{
    ...
    if (!fTuple.setField(NumberFormatTestTuple::getFieldByName(parts[1]), parts[2].unescape(), status)) {
        showError("Invalid field value");
    }
}

int32_t DataDrivenNumberFormatTestSuite::splitBy(UnicodeString* columnValues, int32_t columnValuesCount, UChar delimiter)
{
    int32_t colIdx = 0;
    int32_t colStart = 0;
    ...
    for (int32_t idx = 0; colIdx < columnValuesCount - 1 && idx < len; ++idx) {
        ...
        if (ch == delimiter) {
            columnValues[colIdx++] = fFileLine.tempSubString(colStart, idx - colStart);
            colStart = idx + 1;
        }
    }
    columnValues[colIdx++] = fFileLine.tempSubString(colStart, len - colStart);
    return colIdx;
}
...
{
    UChar lineStr[20];
    uprv_itou(lineStr, UPRV_LENGTHOF(lineStr), (uint32_t)fFileLineNumber, 10, 1);
    UnicodeString fullMessage("line ");
    dataerrln(fullMessage.append(lineStr).append(": ").append(prettify(message)));
    showLineInfo();
}

UBool DataDrivenNumberFormatTestSuite::readLine(UCHARBUF* f, UErrorCode& status)
{
    int32_t lineLength;
...
UBool DataDrivenNumberFormatTestSuite::isPass(const NumberFormatTestTuple& tuple, UnicodeString& appendErrorMessage, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
    ...
    if (tuple.formatFlag && tuple.outputFlag) {
        ++fFormatTestNumber;
        result = isFormatPass(tuple, fPreviousFormatters[fFormatTestNumber % UPRV_LENGTHOF(fPreviousFormatters)], appendErrorMessage, status);
...
UBool DataDrivenNumberFormatTestSuite::isFormatPass(const NumberFormatTestTuple& /* tuple */, UnicodeString& /*appendErrorMessage*/, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
...
UBool DataDrivenNumberFormatTestSuite::isFormatPass(
    const NumberFormatTestTuple& tuple, UObject* /* somePreviousFormatter */, UnicodeString& appendErrorMessage, UErrorCode& status)
{
...
UBool DataDrivenNumberFormatTestSuite::isToPatternPass(const NumberFormatTestTuple& /* tuple */, UnicodeString& /*appendErrorMessage*/, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
...
UBool DataDrivenNumberFormatTestSuite::isParsePass(const NumberFormatTestTuple& /* tuple */, UnicodeString& /*appendErrorMessage*/, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
...
UBool DataDrivenNumberFormatTestSuite::isParseCurrencyPass(const NumberFormatTestTuple& /* tuple */, UnicodeString& /*appendErrorMessage*/, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
...
UBool DataDrivenNumberFormatTestSuite::isSelectPass(const NumberFormatTestTuple& /* tuple */, UnicodeString& /*appendErrorMessage*/, UErrorCode& status)
{
    if (U_FAILURE(status)) {
        return FALSE;
    }
