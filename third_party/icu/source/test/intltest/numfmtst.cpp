...
public:
    ...
    virtual StubNumberFormat* clone() const override
    {
        return NULL;
    }
};

void NumberFormatTest::TestCoverage(void)
{
...
void NumberFormatTest::TestPatterns(void)
{
    ...
    if (U_FAILURE(status)) {
        errcheckln(status, "FAIL: Could not construct DecimalFormatSymbols - %s", u_errorName(status));
        return;
    }
    ...
    for (int32_t i = 0; i < pat_length; ++i) {
        ...
        if (U_FAILURE(status)) {
            errln((UnicodeString) "FAIL: DecimalFormat constructor failed for " + pat[i]);
            continue;
        }
        UnicodeString newp;
        fmt.toPattern(newp);
        if (!(newp == newpat[i]))
            errln((UnicodeString) "FAIL: Pattern " + pat[i] + " should transmute to " + newpat[i] + "; " + newp + " seen instead");

        UnicodeString s;
        (*(NumberFormat*)&fmt).format((int32_t)0, s);
        if (!(s == num[i])) {
            errln((UnicodeString) "FAIL: Pattern " + pat[i] + " should format zero as " + num[i] + "; " + s + " seen instead");
...
void NumberFormatTest::TestExponential(void)
{
    ...
    DecimalFormatSymbols sym(Locale::getUS(), status);
    if (U_FAILURE(status)) {
        errcheckln(status, "FAIL: Bad status returned by DecimalFormatSymbols ct - %s", u_errorName(status));
        return;
    }
    ...
    for (int32_t p = 0; p < pat_length; ++p) {
        ...
        if (U_FAILURE(status)) {
            errln("FAIL: Bad status returned by DecimalFormat ct");
            continue;
        }
        UnicodeString pattern;
        logln((UnicodeString) "Pattern \"" + pat[p] + "\" -toPattern-> \"" + fmt.toPattern(pattern) + "\"");
        int32_t v;
        for (v = 0; v < val_length; ++v) {
            UnicodeString s;
            (*(NumberFormat*)&fmt).format(val[v], s);
            ...
            if (pos.getIndex() == s.length()) {
                ...
                if ((useEpsilon && (uprv_fabs(a - valParse[v + ival]) / a > (2 * DBL_EPSILON))) || (!useEpsilon && a != valParse[v + ival])) {
                    errln((UnicodeString) "FAIL: Expected " + valParse[v + ival] + " but got " + a + " on input " + s);
                }
            } else {
        ...
        for (v = 0; v < lval_length; ++v) {
            ...
            Formattable af;
            fmt.parse(s, af, pos);
            if (af.getType() == Formattable::kLong || af.getType() == Formattable::kInt64) {
                UErrorCode status = U_ZERO_ERROR;
                ...
                errln((UnicodeString) "FAIL: Non-long Formattable returned for " + s + " Double: " + af.getDouble() + ", Long: " + af.getLong());
        }
        ival += val_length;
        ilval += lval_length;
    }
...
void NumberFormatTest::TestCurrencySign(void)
{
    ...
    if (s != "$1,234.56")
        dataerrln((UnicodeString) "FAIL: Expected $1,234.56");
    s.truncate(0);
    ((NumberFormat*)fmt)->format(-1234.56, s);
    logln((UnicodeString) " Format " + (-1234.56) + " -> " + escape(s));
    if (s != "-$1,234.56")
    ...
    logln((UnicodeString) " Format " + 1234.56 + " -> " + escape(s));
    if (s != "USD 1,234.56")
        dataerrln((UnicodeString) "FAIL: Expected USD 1,234.56");
    s.truncate(0);
    ((NumberFormat*)fmt)->format(-1234.56, s);
    logln((UnicodeString) " Format " + (-1234.56) + " -> " + escape(s));
    if (s != "USD -1,234.56")
    ...
    delete sym;
    if (U_FAILURE(status))
        errln((UnicodeString) "FAIL: Status " + u_errorName(status));
}
...
static const char* testCases[][2] = {
    /* locale ID */ /* expected */
    { "ca_ES@currency=ESP", "\\u20A7\\u00A01.150" }, { "de_LU@currency=LUF", "1,150\\u00A0F" },
    { "el_GR@currency=GRD", "1.150,50\\u00A0\\u0394\\u03C1\\u03C7" }, { "en_BE@currency=BEF", "1.150,50\\u00A0BEF" },
    { "es_ES@currency=ESP", "1.150\\u00A0\\u20A7" }, { "eu_ES@currency=ESP", "\\u20A7\\u00A01.150" }, { "gl_ES@currency=ESP", "1.150\\u00A0\\u20A7" },
    { "it_IT@currency=ITL", "ITL\\u00A01.150" }, { "pt_PT@currency=PTE", "1,150$50\\u00A0\\u200B" }, // per cldrbug 7670
    { "en_US@currency=JPY", "\\u00A51,150" }, { "en_US@currency=jpy", "\\u00A51,150" }, { "en-US-u-cu-jpy", "\\u00A51,150" }
};
/**
...
void NumberFormatTest::TestCurrency(void)
{
    ...
    for (int i = 0; i < UPRV_LENGTHOF(testCases); i++) {
        ...
        if (s != expected) {
            errln(UnicodeString("FAIL: Expected: ") + expected + UnicodeString(" Got: ") + s + UnicodeString(" for locale: ") + UnicodeString(localeID));
        }
        if (U_FAILURE(status)) {
            errln((UnicodeString) "FAIL: Status " + (int32_t)status);
        }
...
void NumberFormatTest::TestCurrencyObject()
{
    ...
    NumberFormat* fmt = NumberFormat::createCurrencyInstance(Locale::getUS(), ec);
    ...
    expectCurrency(*fmt, Locale::getFrance(), 1234.56, CharsToUnicodeString("\\u20AC1,234.56")); // Euro

    expectCurrency(*fmt, Locale::getJapan(), 1234.56, CharsToUnicodeString("\\u00A51,235")); // Yen

    expectCurrency(*fmt, Locale("fr", "CH", ""), 1234.56, "CHF 1,234.56"); // no more 0.05 rounding here, see cldrbug 5548

    expectCurrency(*fmt, Locale::getUS(), 1234.56, "$1,234.56");
    ...
    expectCurrency(*fmt, Locale::getJapan(), 1234.56, CharsToUnicodeString("1\\u202F235 JPY")); // Yen

    expectCurrency(*fmt, Locale("fr", "CH", ""), 1234.56, CharsToUnicodeString("1\\u202F234,56 CHF")); // no more 0.05 rounding here, see cldrbug 5548

    expectCurrency(*fmt, Locale::getUS(), 1234.56, CharsToUnicodeString("1\\u202F234,56 $US"));
...
void NumberFormatTest::TestParse(void)
{
    ...
    if (n.getType() != Formattable::kLong || n.getLong() != 0)
        errln((UnicodeString) "FAIL: Expected 0");
    delete format;
    if (U_FAILURE(status))
        errcheckln(status, (UnicodeString) "FAIL: Status " + u_errorName(status));
    //}
    // catch(Exception e) {
...
static const char* lenientAffixTestCases[] = { "(1)", "( 1)", "(1 )", "( 1 )" };

static const char* lenientMinusTestCases[] = { "-5", "\\u22125", "\\u27965" };

static const char* lenientCurrencyTestCases[] = { "$1,000", "$ 1,000", "$1000", "$ 1000", "$1 000.00", "$ 1 000.00", "$ 1\\u00A0000.00", "1000.00" };

// changed from () to - per cldrbug 5674
static const char* lenientNegativeCurrencyTestCases[]
    = { "-$1,000", "-$ 1,000", "-$1000", "-$ 1000", "-$1 000.00", "-$ 1 000.00", "- $ 1,000.00 ", "-$ 1\\u00A0000.00", "-1000.00" };

static const char* lenientPercentTestCases[] = { "25%", " 25%", " 25 %", "25 %", "25\\u00A0%", "25" };

static const char* lenientNegativePercentTestCases[] = { "-25%", " -25%", " - 25%", "- 25 %", " - 25 %", "-25 %", "-25\\u00A0%", "-25", "- 25" };

static const char* strictFailureTestCases[] = { " 1000", "10,00", "1,000,.0" };
...
void NumberFormatTest::TestLenientParse(void)
{
    ...
    if (format == NULL || U_FAILURE(status)) {
        ...
        for (int32_t t = 0; t < UPRV_LENGTHOF(lenientAffixTestCases); t += 1) {
            ...
            if (U_FAILURE(status) || n.getType() != Formattable::kLong || n.getLong() != 1) {
    ...
    if (cFormat == NULL || U_FAILURE(status)) {
        ...
        for (int32_t t = 0; t < UPRV_LENGTHOF(lenientCurrencyTestCases); t += 1) {
            ...
            if (U_FAILURE(status) || n.getType() != Formattable::kLong || n.getLong() != 1000) {
    ...
    if (pFormat == NULL || U_FAILURE(status)) {
        ...
        for (int32_t t = 0; t < UPRV_LENGTHOF(lenientPercentTestCases); t += 1) {
            ...
            if (U_FAILURE(status) || n.getType() != Formattable::kDouble || n.getDouble() != 0.25) {
                errln((UnicodeString) "Lenient parse failed for \"" + (UnicodeString)lenientPercentTestCases[t]
                    + (UnicodeString) "\"; error code = " + u_errorName(status) + "; got: " + n.getDouble(status));
                status = U_ZERO_ERROR;
            }
        }
...
void NumberFormatTest::TestSecondaryGrouping(void)
{
    ...
    if (!ok) {
        errln((UnicodeString) "FAIL  Expected " + l + " x hi_IN -> \"1,87,65,43,210\" (with Hindi digits), got \"" + escape(out) + "\"");
    } else {
        logln((UnicodeString) "Ok    " + l + " x hi_IN -> \"" + escape(out) + "\"");
    }
}
...
void NumberFormatTest::TestScientific(void)
{
    ...
    const char* PAT[] = { "#E0", "0.####E0", "00.000E00", "##0.####E000", "0.###E0;[0.###E0]" };
    int32_t PAT_length = UPRV_LENGTHOF(PAT);
    int32_t DIGITS[] = {
    ...
    for (int32_t i = 0; i < PAT_length; ++i) {
        ...
        df.toPattern(pat2);
        if (pat == pat2) {
            logln(UnicodeString("Ok   Pattern rt \"") + pat + "\" -> \"" + pat2 + "\"");
        } else {
            errln(UnicodeString("FAIL Pattern rt \"") + pat + "\" -> \"" + pat2 + "\"");
        }
        // Make sure digit counts match what we expect
        if (df.getMinimumIntegerDigits() != DIGITS[4 * i] || df.getMaximumIntegerDigits() != DIGITS[4 * i + 1]
            || df.getMinimumFractionDigits() != DIGITS[4 * i + 2] || df.getMaximumFractionDigits() != DIGITS[4 * i + 3]) {
            errln(UnicodeString("FAIL \"" + pat + "\" min/max int; min/max frac = ") + df.getMinimumIntegerDigits() + "/" + df.getMaximumIntegerDigits() + ";"
                + df.getMinimumFractionDigits() + "/" + df.getMaximumFractionDigits() + ", expect " + DIGITS[4 * i] + "/" + DIGITS[4 * i + 1] + ";"
                + DIGITS[4 * i + 2] + "/" + DIGITS[4 * i + 3]);
        }
    }
    ...
    expect2(NumberFormat::createScientificInstance(status), 12345.678901, "1.2345678901E4", status);
    Locale::setDefault(def, status);

    expect2(new DecimalFormat("#E0", US, status), 12345.0, "1.2345E4", status);
    expect(new DecimalFormat("0E0", US, status), 12345.0, "1E4", status);
    expect2(NumberFormat::createScientificInstance(Locale::getUS(), status), 12345.678901, "1.2345678901E4", status);
    expect(new DecimalFormat("##0.###E0", US, status), 12345.0, "12.34E3", status);
    expect(new DecimalFormat("##0.###E0", US, status), 12345.00001, "12.35E3", status);
    expect2(new DecimalFormat("##0.####E0", US, status), (int32_t)12345, "12.345E3", status);
    expect2(NumberFormat::createScientificInstance(Locale::getFrance(), status), 12345.678901, "1,2345678901E4", status);
    expect(new DecimalFormat("##0.####E0", US, status), 789.12345e-9, "789.12E-9", status);
    expect2(new DecimalFormat("##0.####E0", US, status), 780.e-9, "780E-9", status);
    expect(new DecimalFormat(".###E0", US, status), 45678.0, ".457E5", status);
    expect2(new DecimalFormat(".###E0", US, status), (int32_t)0, ".0E0", status);
    /*
    ...
    !
    */
    expect2(new DecimalFormat("#E0", US, status), (int32_t)45678000, "4.5678E7", status);
    expect2(new DecimalFormat("##E0", US, status), (int32_t)45678000, "45.678E6", status);
    expect2(new DecimalFormat("####E0", US, status), (int32_t)45678000, "4567.8E4", status);
    expect(new DecimalFormat("0E0", US, status), (int32_t)45678000, "5E7", status);
    expect(new DecimalFormat("00E0", US, status), (int32_t)45678000, "46E6", status);
    expect(new DecimalFormat("000E0", US, status), (int32_t)45678000, "457E5", status);
    /*
    ...
    !
    */
    expect2(new DecimalFormat("###E0", US, status), 0.0000123, "12.3E-6", status);
    expect2(new DecimalFormat("###E0", US, status), 0.000123, "123E-6", status);
    expect2(new DecimalFormat("###E0", US, status), 0.00123, "1.23E-3", status);
    expect2(new DecimalFormat("###E0", US, status), 0.0123, "12.3E-3", status);
    expect2(new DecimalFormat("###E0", US, status), 0.123, "123E-3", status);
    expect2(new DecimalFormat("###E0", US, status), 1.23, "1.23E0", status);
    expect2(new DecimalFormat("###E0", US, status), 12.3, "12.3E0", status);
    expect2(new DecimalFormat("###E0", US, status), 123.0, "123E0", status);
    expect2(new DecimalFormat("###E0", US, status), 1230.0, "1.23E3", status);
    /*
    ...
    !
    */
    expect2(new DecimalFormat("0.#E+00", US, status), 0.00012, "1.2E-04", status);
    expect2(new DecimalFormat("0.#E+00", US, status), (int32_t)12000, "1.2E+04", status);
}
...
void NumberFormatTest::TestPad(void)
{
    ...
    expect2(new DecimalFormat("*^##.##", US, status), int32_t(0), "^^^^0", status);
    expect2(new DecimalFormat("*^##.##", US, status), -1.3, "^-1.3", status);
    expect2(new DecimalFormat("##0.0####E0*_ 'g-m/s^2'", US, status), int32_t(0), "0.0E0______ g-m/s^2", status);
    ...
    expect2(new DecimalFormat(formatStr, US, status), int32_t(-10), "xxxxxxxxxx(10.0)", status);
    expect2(new DecimalFormat(formatStr, US, status), int32_t(-1000), "xxxxxxx(1,000.0)", status);
    expect2(new DecimalFormat(formatStr, US, status), int32_t(-1000000), "xxx(1,000,000.0)", status);
    expect2(new DecimalFormat(formatStr, US, status), -100.37, "xxxxxxxx(100.37)", status);
    expect2(new DecimalFormat(formatStr, US, status), -10456.37, "xxxxx(10,456.37)", status);
    expect2(new DecimalFormat(formatStr, US, status), -1120456.37, "xx(1,120,456.37)", status);
    expect2(new DecimalFormat(formatStr, US, status), -112045600.37, "(112,045,600.37)", status);
    expect2(new DecimalFormat(formatStr, US, status), -1252045600.37, "(1,252,045,600.37)", status);

    expect2(new DecimalFormat(formatStr, US, status), int32_t(10), "xxxxxxxxxxxx10.0", status);
    expect2(new DecimalFormat(formatStr, US, status), int32_t(1000), "xxxxxxxxx1,000.0", status);
    expect2(new DecimalFormat(formatStr, US, status), int32_t(1000000), "xxxxx1,000,000.0", status);
    expect2(new DecimalFormat(formatStr, US, status), 100.37, "xxxxxxxxxx100.37", status);
    expect2(new DecimalFormat(formatStr, US, status), 10456.37, "xxxxxxx10,456.37", status);
    expect2(new DecimalFormat(formatStr, US, status), 1120456.37, "xxxx1,120,456.37", status);
    expect2(new DecimalFormat(formatStr, US, status), 112045600.37, "xx112,045,600.37", status);
    expect2(new DecimalFormat(formatStr, US, status), 10252045600.37, "10,252,045,600.37", status);

    // Test padding between a sign and a number
    const char* formatStr2 = "#,###,###,##0.0#*x;(###,###,##0.0#*x)";
    expect2(new DecimalFormat(formatStr2, US, status), int32_t(-10), "(10.0xxxxxxxxxx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), int32_t(-1000), "(1,000.0xxxxxxx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), int32_t(-1000000), "(1,000,000.0xxx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), -100.37, "(100.37xxxxxxxx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), -10456.37, "(10,456.37xxxxx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), -1120456.37, "(1,120,456.37xx)", status);
    expect2(new DecimalFormat(formatStr2, US, status), -112045600.37, "(112,045,600.37)", status);
    expect2(new DecimalFormat(formatStr2, US, status), -1252045600.37, "(1,252,045,600.37)", status);

    expect2(new DecimalFormat(formatStr2, US, status), int32_t(10), "10.0xxxxxxxxxxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), int32_t(1000), "1,000.0xxxxxxxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), int32_t(1000000), "1,000,000.0xxxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), 100.37, "100.37xxxxxxxxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), 10456.37, "10,456.37xxxxxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), 1120456.37, "1,120,456.37xxxx", status);
    expect2(new DecimalFormat(formatStr2, US, status), 112045600.37, "112,045,600.37xx", status);
    expect2(new DecimalFormat(formatStr2, US, status), 10252045600.37, "10,252,045,600.37", status);
...
void NumberFormatTest::TestSurrogateSupport(void)
{
    ...
    expect2(new DecimalFormat(patternStr, custom, status), int32_t(0), expStr, status);

    status = U_ZERO_ERROR;
    expect2(new DecimalFormat("*^##.##", custom, status), int32_t(0), "^^^^0", status);
    status = U_ZERO_ERROR;
    expect2(new DecimalFormat("##.##", custom, status), -1.3, " minus 1decimal3", status);
    status = U_ZERO_ERROR;
    expect2(new DecimalFormat("##0.0####E0 'g-m/s^2'", custom, status), int32_t(0), "0decimal0exponent0 g-m/s^2", status);
    status = U_ZERO_ERROR;
    expect(new DecimalFormat("##0.0####E0 'g-m/s^2'", custom, status), 1.0 / 3, "333decimal333exponent minus 3 g-m/s^2", status);
    status = U_ZERO_ERROR;
    expect2(new DecimalFormat("##0.0#### 'g-m/s^2'", custom, status), int32_t(0), "0decimal0 g-m/s^2", status);
    status = U_ZERO_ERROR;
    expect(new DecimalFormat("##0.0#### 'g-m/s^2'", custom, status), 1.0 / 3, "0decimal33333 g-m/s^2", status);
    ...
    expStr = expStr.unescape();
    status = U_ZERO_ERROR;
    expect2(new DecimalFormat("##0.000", custom, status), 1.25, expStr, status);
    ...
    status = U_ZERO_ERROR;
    expect2(new DecimalFormat(patternStr, custom, status), int32_t(-20), expStr, status);
...
void NumberFormatTest::TestCurrencyNames(void)
{
    ...
    assertEquals("USD.getName(SYMBOL_NAME, en)", UnicodeString("$"), UnicodeString(ucurr_getName(USD, "en", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    assertEquals("USD.getName(NARROW_SYMBOL_NAME, en)", UnicodeString("$"),
        UnicodeString(ucurr_getName(USD, "en", UCURR_NARROW_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("USD.getName(LONG_NAME, en)", UnicodeString("US Dollar"), UnicodeString(ucurr_getName(USD, "en", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    assertEquals("CAD.getName(SYMBOL_NAME, en)", UnicodeString("CA$"), UnicodeString(ucurr_getName(CAD, "en", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    assertEquals("CAD.getName(NARROW_SYMBOL_NAME, en)", UnicodeString("$"),
        UnicodeString(ucurr_getName(CAD, "en", UCURR_NARROW_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("CAD.getName(SYMBOL_NAME, en_CA)", UnicodeString("$"),
        UnicodeString(ucurr_getName(CAD, "en_CA", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("USD.getName(SYMBOL_NAME, en_CA)", UnicodeString("US$"),
        UnicodeString(ucurr_getName(USD, "en_CA", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("USD.getName(NARROW_SYMBOL_NAME, en_CA)", UnicodeString("$"),
        UnicodeString(ucurr_getName(USD, "en_CA", UCURR_NARROW_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("USD.getName(SYMBOL_NAME) in en_NZ", UnicodeString("US$"),
        UnicodeString(ucurr_getName(USD, "en_NZ", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("CAD.getName(SYMBOL_NAME)", UnicodeString("CA$"), UnicodeString(ucurr_getName(CAD, "en_NZ", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    assertEquals("USX.getName(SYMBOL_NAME)", UnicodeString("USX"), UnicodeString(ucurr_getName(USX, "en_US", UCURR_SYMBOL_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    assertEquals("USX.getName(NARROW_SYMBOL_NAME)", UnicodeString("USX"),
        UnicodeString(ucurr_getName(USX, "en_US", UCURR_NARROW_SYMBOL_NAME, &isChoiceFormat, &len, &ec)), possibleDataError);
    assertEquals("USX.getName(LONG_NAME)", UnicodeString("USX"), UnicodeString(ucurr_getName(USX, "en_US", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec)),
        possibleDataError);
    ...
    ucurr_getName(CAD, "es_ES", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec);
    assertTrue("ucurr_getName (es_ES fallback)", U_USING_FALLBACK_WARNING == ec, TRUE, possibleDataError);

    ucurr_getName(CAD, "zh_TW", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec);
    assertTrue("ucurr_getName (zh_TW fallback)", U_USING_FALLBACK_WARNING == ec, TRUE, possibleDataError);

    ucurr_getName(CAD, "en_US", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec);
    assertTrue("ucurr_getName (en_US default)", U_USING_DEFAULT_WARNING == ec || U_USING_FALLBACK_WARNING == ec, TRUE);

    ucurr_getName(CAD, "ti", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec);
    assertTrue("ucurr_getName (ti default)", U_USING_DEFAULT_WARNING == ec, TRUE);

    // Test that a default warning is being returned when falling back to root. JB 4536.
    ucurr_getName(ITL, "cy", UCURR_LONG_NAME, &isChoiceFormat, &len, &ec);
    assertTrue("ucurr_getName (cy default to root)", U_USING_DEFAULT_WARNING == ec, TRUE);
...
void NumberFormatTest::TestCurrencyVariants()
{
    ...
    for (const auto& cas : cases) {
        ...
        const UChar* actualShort = ucurr_getName(cas.isoCode, cas.locale, UCURR_SYMBOL_NAME, &choiceFormatIgnored, &lengthIgnored, status);
        const UChar* actualFormal = ucurr_getName(cas.isoCode, cas.locale, UCURR_FORMAL_SYMBOL_NAME, &choiceFormatIgnored, &lengthIgnored, status);
        const UChar* actualVarant = ucurr_getName(cas.isoCode, cas.locale, UCURR_VARIANT_SYMBOL_NAME, &choiceFormatIgnored, &lengthIgnored, status);
        status.errIfFailureAndReset();
        const UChar* actualNarrow = ucurr_getName(cas.isoCode, cas.locale, UCURR_NARROW_SYMBOL_NAME, &choiceFormatIgnored, &lengthIgnored, status);
        status.expectErrorAndReset(cas.expectedNarrowError);
        assertEquals(UnicodeString("Short symbol: ") + cas.locale + u": " + cas.isoCode, cas.expectedShort, actualShort);
        assertEquals(UnicodeString("Narrow symbol: ") + cas.locale + u": " + cas.isoCode, cas.expectedNarrow, actualNarrow);
        assertEquals(UnicodeString("Formal symbol: ") + cas.locale + u": " + cas.isoCode, cas.expectedFormal, actualFormal);
        assertEquals(UnicodeString("Variant symbol: ") + cas.locale + u": " + cas.isoCode, cas.expectedVariant, actualVarant);
    }
}
...
{
    Locale locDefault;
    static const char* badLocales[] = { // length < ULOC_FULLNAME_CAPACITY
        ...
        "x-crazy_ZZ_MY_SPECIAL_ADMINISTRATION_REGION_NEEDS_A_SPECIAL_VARIANT_WITH_A_REALLY_REALLY_REALLY_REALLY_REALLY_REALLY_REALLY_REALLY_REALLY_REALLY_"
        "REALLY_LONG_NAME"
    ...
    for (i = 0; i < UPRV_LENGTHOF(badLocales); i++) {
        ...
        for (; symbolEnum < (int)DecimalFormatSymbols::kFormatSymbolCount; symbolEnum++) {
            ...
            if (symbolString.length() == 0 && symbolEnum != (int)DecimalFormatSymbols::kGroupingSeparatorSymbol
...
void NumberFormatTest::TestPerMill()
{
    ...
    if (!assertSuccess("DecimalFormat ct", ec))
        return;
    assertEquals("0.4857 x ###.###\\u2030", ctou("485.7\\u2030"), fmt.format(0.4857, str), true);
    ...
    if (!assertSuccess("setup", ec))
        return;
    str.truncate(0);
    assertEquals("0.4857 x ###.###m", "485.7m", fmt2.format(0.4857, str));
}
...
void NumberFormatTest::TestIllegalPatterns()
{
    ...
    const char* DATA[] = { // Unquoted special characters in the suffix are illegal
        "-:000.000|###", "+:000.000'|###'", 0
    };
    for (int32_t i = 0; DATA[i]; ++i) {
        ...
        if (U_SUCCESS(ec) == valid) {
            logln("Ok: pattern \"%s\": %s", pat, u_errorName(ec));
        } else {
            errcheckln(ec, "FAIL: pattern \"%s\" should have %s; got %s", pat, (valid ? "succeeded" : "failed"), u_errorName(ec));
        }
    }
}
 ...
 */
static void parseCurrencyAmount(const UnicodeString& str, const NumberFormat& fmt, UChar delim, Formattable& result, UErrorCode& ec)
{
    ...
    for (;;) {
        ...
        case 0:
            ...
            if (!tokens.next(tok, ec))
                goto error;
            delete ref;
            ref = new DecimalFormat(tok, new DecimalFormatSymbols(Locale::getUS(), ec), ec);
            if (U_FAILURE(ec)) {
        ...
        case 5: // p:
            ...
            if (cmd == 2 || cmd == 3 || cmd == 4) {
                ...
                UnicodeString num;
                if (!tokens.next(num, ec))
                    goto error;
                if (!tokens.next(str, ec))
                    goto error;
                ref->parse(num, n, ec);
                assertSuccess("parse", ec);
                assertEquals(where + "\"" + pat + "\".format(" + num + ")", str, fmt->format(n, out.remove(), ec));
                ...
                if (cmd != 2) { // != f:
                    ...
                    fmt->parse(str, m, ec);
                    assertSuccess("parse", ec);
                    assertEquals(where + "\"" + pat + "\".parse(\"" + str + "\")", n, m);
                }
            }
            // p: <pattern or '-'> <string to parse> <exp. number>
                ...
                fmt->parse(str, n, ec);
                assertSuccess("parse", ec);
                assertEquals(where + "\"" + pat + "\".parse(\"" + str + "\")", exp, n);
            }
            break;
        case 8: // fpc:
            if (!tokens.next(tok, ec))
            ...
            if (tok != "-") {
                mloc = tok;
                delete mfmt;
                mfmt = MeasureFormat::createCurrencyFormat(Locale::createFromName(CharString().appendInvariantChars(mloc, ec).data()), ec);
                if (U_FAILURE(ec)) {
            ...
            if (!tokens.next(currAmt, ec))
                goto error;
            if (!tokens.next(str, ec))
                goto error;
            parseCurrencyAmount(currAmt, *ref, (UChar)0x2F /*'/'*/, n, ec);
            if (assertSuccess("parseCurrencyAmount", ec)) {
                assertEquals(where + "getCurrencyFormat(" + mloc + ").format(" + currAmt + ")", str, mfmt->format(n, out.remove(), ec));
                assertSuccess("format", ec);
            }
            if (!tokens.next(currAmt, ec))
                goto error;
            ...
            if (assertSuccess("parseCurrencyAmount", ec)) {
                ...
                if (assertSuccess("parseCurrency", ec)) {
                    assertEquals(where + "getCurrencyFormat(" + mloc + ").parse(\"" + str + "\")", n, m);
                } else {
        ...
        case 7: {
            ...
            if (!tokens.next(testpat, ec))
                goto error;
            if (!tokens.next(exppat, ec))
                goto error;
            ...
            if (U_SUCCESS(ec2)) {
                if (err) {
                    errln("FAIL: " + where + "Invalid pattern \"" + testpat + "\" was accepted");
                } else {
                    UnicodeString pat2;
                    assertEquals(where + "\"" + testpat + "\".toPattern()", exppat, f->toPattern(pat2));
                }
            } else {
                if (err) {
                    logln("Ok: " + where + "Invalid pattern \"" + testpat + "\" failed: " + u_errorName(ec2));
                } else {
                    errln("FAIL: " + where + "Valid pattern \"" + testpat + "\" failed: " + u_errorName(ec2));
                }
            }
            if (!existingPat)
                delete f;
...
void NumberFormatTest::expect2(NumberFormat* fmt, const Formattable& n, const UnicodeString& exp, UErrorCode status)
{
...
void NumberFormatTest::expect(NumberFormat& fmt, const UnicodeString& str, const Formattable& n)
{
    ...
    if (equalValue(num, n)) {
        logln(UnicodeString("Ok   \"") + str + "\" x " + pat + " = " + toString(num));
    } else {
        dataerrln(UnicodeString("FAIL \"") + str + "\" x " + pat + " = " + toString(num) + ", expected " + toString(n));
    }
}
...
void NumberFormatTest::expect_rbnf(NumberFormat& fmt, const UnicodeString& str, const Formattable& n)
{
    ...
    if (equalValue(num, n)) {
        logln(UnicodeString("Ok   \"") + str + " = " + toString(num));
    } else {
        errln(UnicodeString("FAIL \"") + str + " = " + toString(num) + ", expected " + toString(n));
    }
}

void NumberFormatTest::expect_rbnf(NumberFormat& fmt, const Formattable& n, const UnicodeString& exp, UBool rt)
{
    UnicodeString saw;
    FieldPosition pos;
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (saw == exp) {
        logln(UnicodeString("Ok   ") + toString(n) + " = \"" + escape(saw) + "\"");
        ...
        if (rt) {
            ...
            CHECK(status, "NumberFormat::format");
            if (saw2 != exp) {
                errln((UnicodeString) "FAIL \"" + exp + "\" => " + toString(n2) + " => \"" + saw2 + "\"");
            }
        }
    } else {
        errln(UnicodeString("FAIL ") + toString(n) + " = \"" + escape(saw) + "\", expected \"" + exp + "\"");
    }
}

void NumberFormatTest::expect(NumberFormat& fmt, const Formattable& n, const UnicodeString& exp, UBool rt)
{
    UnicodeString saw;
    FieldPosition pos;
    UErrorCode status = U_ZERO_ERROR;
    ...
    ((DecimalFormat*)&fmt)->toPattern(pat);
    if (saw == exp) {
        logln(UnicodeString("Ok   ") + toString(n) + " x " + escape(pat) + " = \"" + escape(saw) + "\"");
        // We should be able to round-trip the formatted string =>
        // number => string (but not the other way around: number
        // => string => number2, might have number2 != number):
        if (rt) {
        ...
        dataerrln(UnicodeString("FAIL ") + toString(n) + " x " + escape(pat) + " = \"" + escape(saw) + "\", expected \"" + exp + "\"");
    }
}

void NumberFormatTest::expect(NumberFormat* fmt, const Formattable& n, const UnicodeString& exp, UBool rt, UErrorCode status)
{
    if (fmt == NULL || U_FAILURE(status)) {
        dataerrln("FAIL: NumberFormat constructor");
    } else {
...
void NumberFormatTest::expectCurrency(NumberFormat& nf, const Locale& locale, double value, const UnicodeString& string)
{
    UErrorCode ec = U_ZERO_ERROR;
    ...
    NumberFormat* f = NumberFormat::createInstance(Locale::getUS(), ec);
    UnicodeString v;
    if (U_FAILURE(ec)) {
    ...
    if (s == string) {
        ...
        errln((UnicodeString) "FAIL: " + v + " x " + curr + " => " + prettify(s) + ", expected " + prettify(string));
    }
}
...
void NumberFormatTest::expectPad(DecimalFormat& fmt, const UnicodeString& pat, int32_t pos)
{
    expectPad(fmt, pat, pos, 0, (UnicodeString) "");
}
void NumberFormatTest::expectPad(DecimalFormat& fmt, const UnicodeString& pat, int32_t pos, int32_t width, UChar pad)
{
    expectPad(fmt, pat, pos, width, UnicodeString(pad));
}
void NumberFormatTest::expectPad(DecimalFormat& fmt, const UnicodeString& pat, int32_t pos, int32_t width, const UnicodeString& pad)
{
    ...
    if (apos == pos && awidth == width && apadStr == pad) {
        ...
        errln(UnicodeString("FAIL \"") + pat + "\" pos=" + apos + " width=" + awidth + " pad=" + apadStr + ", expected " + pos + " " + width + " " + pad);
    }
}
...
void NumberFormatTest::expectParseCurrency(const NumberFormat& fmt, const UChar* currency, double amount, const char* text)
{
    ...
    char theInfo[100];
    sprintf(theInfo, "For locale %s, string \"%s\", currency ", fmt.getLocale(ULOC_ACTUAL_LOCALE, status).getBaseName(), text);
...
void NumberFormatTest::TestHost()
{
    ...
    for (UNumberFormatStyle k = UNUM_DECIMAL; k < UNUM_FORMAT_STYLE_COUNT; k = (UNumberFormatStyle)(k + 1)) {
        UErrorCode status = U_ZERO_ERROR;
        LocalPointer<NumberFormat> full(NumberFormat::createInstance(loc, k, status));
        if (!NumberFormat::isStyleSupported(k)) {
            if (status != U_UNSUPPORTED_ERROR) {
                errln("FAIL: expected style %d to be unsupported - %s", k, u_errorName(status));
            }
            continue;
        }
        if (full.isNull() || U_FAILURE(status)) {
            dataerrln("FAIL: Can't create number instance of style %d for host - %s", k, u_errorName(status));
            return;
        }
...
void NumberFormatTest::TestRoundingPattern()
{
    ...
    } tests[] = { { (UnicodeString) "##0.65", 1.234, (UnicodeString) "1.30" }, { (UnicodeString) "#50", 1230, (UnicodeString) "1250" } };
    ...
    for (int32_t i = 0; i < numOfTests; i++) {
        ...
        if (result != tests[i].expected) {
            errln("String Pattern Rounding Test Failed: Pattern: \"" + tests[i].pattern + "\" Number: " + tests[i].testCase + " - Got: " + result
...
void NumberFormatTest::TestSpaceParsing()
{
    ...
    for (uint32_t i = 0; i < UPRV_LENGTHOF(DATA); ++i) {
        ...
        if (parsePosition.getIndex() != parsedPosition || parsePosition.getErrorIndex() != errorIndex) {
            errln("FAILED parse " + stringToBeParsed + "; lenient: " + DATA[i].lenient + "; wrong position, expected: (" + parsedPosition + ", " + errorIndex
                + "); got (" + parsePosition.getIndex() + ", " + parsePosition.getErrorIndex() + ")");
        }
        if (parsePosition.getErrorIndex() == -1 && result.getType() == Formattable::kLong && result.getLong() != 124) {
...
void NumberFormatTest::TestNumberingSystems()
{
    ...
    const TestNumberingSystemItem DATA[] = { { "en_US@numbers=thai", 1234.567, FALSE, "\\u0E51,\\u0E52\\u0E53\\u0E54.\\u0E55\\u0E56\\u0E57" },
        ...
        { "ar_MA", 1234.567, FALSE, "1.234,567" }, { "en_US@numbers=hanidec", 1234.567, FALSE, "\\u4e00,\\u4e8c\\u4e09\\u56db.\\u4e94\\u516d\\u4e03" },
...
{
    const char* DATA[][6] = { // the fields in the following test are:
        // locale,
    ...
    for (uint32_t i = 0; i < UPRV_LENGTHOF(DATA); ++i) {
        ...
        for (int j = 1; j <= 3; ++j) {
            ...
            for (int k = 3; k <= 5; ++k) {
                ...
                if (U_FAILURE(status) || (parseRes.getType() == Formattable::kDouble && parseRes.getDouble() != numberToBeFormat)
                    || (parseRes.getType() == Formattable::kLong && parseRes.getLong() != numberToBeFormat)) {
                    errln("FAILED parse " + oneCurrencyFormat + "; (i, j, k): " + i + ", " + j + ", " + k);
                }
            }
            delete fmt;
        }
...
void NumberFormatTest::TestCurrencyFormatForMixParsing()
{
    ...
    const char* formats[] = {
        ...
        "USD1,234.56", "US dollars1,234.56",
    ...
    for (uint32_t i = 0; i < UPRV_LENGTHOF(formats); ++i) {
        ...
        } else if (result.getType() != Formattable::kObject || (curramt = dynamic_cast<const CurrencyAmount*>(result.getObject())) == NULL
            || curramt->getNumber().getDouble() != 1234.56 || UnicodeString(curramt->getISOCurrency()).compare(ISO_CURRENCY_USD)) {
...
void NumberFormatTest::TestMismatchedCurrencyFormatFail()
{
    ...
    LocalPointer<DecimalFormat> df(dynamic_cast<DecimalFormat*>(DecimalFormat::createCurrencyInstance("en", status)), status);
    if (!assertSuccess("createCurrencyInstance() failed.", status, true, __FILE__, __LINE__)) {
        return;
    }
    UnicodeString pattern;
    assertEquals("Test assumes that currency sign is at the beginning", u"\u00A4#,##0.00", df->toPattern(pattern));
...
void NumberFormatTest::TestDecimalFormatCurrencyParse()
{
    ...
    for (uint32_t i = 0; i < UPRV_LENGTHOF(DATA); ++i) {
        ...
        if (U_FAILURE(status) || (result.getType() == Formattable::kDouble && result.getDouble() != parsedResult)
            || (result.getType() == Formattable::kLong && result.getLong() != parsedResult)) {
...
void NumberFormatTest::TestCurrencyIsoPluralFormat()
{
    ...
    static const UNumberFormatStyle currencyStyles[] = { UNUM_CURRENCY, UNUM_CURRENCY_ISO, UNUM_CURRENCY_PLURAL };
    ...
    for (int32_t i = 0; i < UPRV_LENGTHOF(DATA); ++i) {
        ...
        for (int32_t kIndex = 0; kIndex < UPRV_LENGTHOF(currencyStyles); ++kIndex) {
            ...
            for (int j = 3; j < 6; ++j) {
                ...
                if (U_FAILURE(status) || (parseResult.getType() == Formattable::kDouble && parseResult.getDouble() != numberToBeFormat)
                    || (parseResult.getType() == Formattable::kLong && parseResult.getLong() != numberToBeFormat)) {
                    errln((UnicodeString) "FAIL: getCurrencyFormat of locale " + localeString + " failed roundtripping the number");
...
{
    static const char* DATA[][6] = { // the data are:
        // locale,
        // currency amount to be formatted,
        // currency ISO code to be formatted,
        // format result using CURRENCYSTYLE,
    ...
    for (;;) {
        ...
        for (uint32_t i = 0; i < UPRV_LENGTHOF(DATA); ++i) { /* i = test case #  - should be i=0*/
            for (int32_t kIndex = 2; kIndex < UPRV_LENGTHOF(currencyStyles); ++kIndex) {
                ...
                UErrorCode status = U_ZERO_ERROR;
                NumberFormat* numFmt = NumberFormat::createInstance(locale, k, status);
                logln("#%d NumberFormat(%s, %s) Currency=%s\n", i, localeString, currencyStyleNames[kIndex], currencyISOCode);
                ...
                for (int j = 3; j < 6; ++j) {
                    ...
                    if (U_FAILURE(status) || (parseResult.getType() == Formattable::kDouble && parseResult.getDouble() != numberToBeFormat)
                        || (parseResult.getType() == Formattable::kLong && parseResult.getLong() != numberToBeFormat)) {
                        errln((UnicodeString) "FAIL: NumberFormat(" + localeString + ", " + currencyStyleNames[kIndex] + "), Currency=" + currencyISOCode
                            + ", parse(" + DATA[i][j] + ") returned error " + (UnicodeString)u_errorName(status) + ".  Testcase: data[" + i + "]["
                            + currencyStyleNames[j - 3] + "=" + j + "]");
                        if (parseResult.getType() == Formattable::kDouble) {
...
void NumberFormatTest::expectPositions(FieldPositionIterator& iter, int32_t* values, int32_t tupleCount, const UnicodeString& str)
{
    UBool found[10];
    FieldPosition fp;
...
void NumberFormatTest::expectPosition(FieldPosition& pos, int32_t id, int32_t start, int32_t limit, const UnicodeString& str)
{
    logln(str);
...
void NumberFormatTest::TestFormatAttributes()
{
    ...
    val = -0.0000123;
    {
        int32_t expected[] = { UNUM_SIGN_FIELD, 0, 1, UNUM_INTEGER_FIELD, 1, 2, UNUM_DECIMAL_SEPARATOR_FIELD, 2, 3, UNUM_FRACTION_FIELD, 3, 5,
            UNUM_EXPONENT_SYMBOL_FIELD, 5, 6, UNUM_EXPONENT_SIGN_FIELD, 6, 7, UNUM_EXPONENT_FIELD, 7, 8 };
        int32_t tupleCount = UPRV_LENGTHOF(expected) / 3;
...
void NumberFormatTest::TestCurrencyFractionDigits()
{
    ...
    if (U_FAILURE(status) || fmt == NULL) {
        ...
        if (text1 != text2) {
            errln((UnicodeString) "NumberFormat::format() should return the same result - text1=" + text1 + " text2=" + text2);
        }
    }
    delete fmt;
}
...
void NumberFormatTest::TestExponentParse()
{
    ...
    if (result.getType() != Formattable::kDouble && result.getDouble() != 5.06E-27 && parsePos.getIndex() != 8) {
        errln("ERROR: parse failed - expected 5.06E-27, 8  - returned %d, %i", result.getDouble(), parsePos.getIndex());
    }
}
...
void NumberFormatTest::TestAvailableNumberingSystems()
{
    ...
    for (int32_t i = 0; i < nsCount; i++) {
        ...
        if (prevName != nullptr) {
            ...
            assertTrue(UnicodeString(u"NS names should be in alphabetical order: ") + prevName + u" vs " + nsname,
...
void NumberFormatTest::TestFormatFastpaths()
{
    // get some additional case
    {
        ...
        if (U_FAILURE(status)) {
            ...
            if (U_FAILURE(status) || expect != result) {
                dataerrln("%s:%d FAIL: expected '%s' got '%s' status %s", __FILE__, __LINE__, CStr(expect)(), CStr(result)(), u_errorName(status));
            } else {
    ...
    }
    {
        ...
        if (U_FAILURE(status)) {
            ...
            df.format(long_number, result, pos);
            if (U_FAILURE(status) || expect != result) {
                dataerrln("%s:%d FAIL: expected '%s' got '%s' status %s on -9223372036854775808", __FILE__, __LINE__, CStr(expect)(), CStr(result)(),
                    u_errorName(status));
            } else {
    ...
    }
    {
        ...
        if (U_FAILURE(status)) {
            ...
            if (U_FAILURE(status) || expect != result) {
                dataerrln(
                    "%s:%d FAIL: expected '%s' got '%s' status %s on U_INT64_MAX", __FILE__, __LINE__, CStr(expect)(), CStr(result)(), u_errorName(status));
            } else {
    ...
    }
    {
        ...
        if (U_FAILURE(status)) {
            ...
            df.format(long_number, result, pos);
            if (U_FAILURE(status) || expect != result) {
                dataerrln("%s:%d FAIL: expected '%s' got '%s' status %s on 0", __FILE__, __LINE__, CStr(expect)(), CStr(result)(), u_errorName(status));
            } else {
    ...
    }
    {
        ...
        if (U_FAILURE(status)) {
            ...
            df.format(long_number, result, pos);
            if (U_FAILURE(status) || expect != result) {
                dataerrln("%s:%d FAIL: expected '%s' got '%s' status %s on -9223372036854775807", __FILE__, __LINE__, CStr(expect)(), CStr(result)(),
                    u_errorName(status));
            } else {
...
UBool NumberFormatTest::testFormattableAsUFormattable(const char* file, int line, Formattable& f)
{
            ...
            if (exactMatch) {
                for (int i = 0; U_SUCCESS(valueStatus) && i < count; i++) {
                    ...
                    if (*Formattable::fromUFormattable(uu) != (array2[i])) {
                        errln("%s:%d: operator== did not match at index[%d] - %p vs %p", file, line, i, (const void*)Formattable::fromUFormattable(uu),
                            (const void*)&(array2[i]));
                        exactMatch = FALSE;
                    } else {
    ...
    if (assertEquals(fileLine + " isNumeric()", f.isNumeric(), ufmt_isNumeric(u)) && f.isNumeric()) {
        ...
        if (assertSuccess(fileLine + " (numeric conversion status)", convStatus)) {
            ...
            if ((l == r) && (uType != UFMT_INT64) // int64 better not overflow
                && (U_INVALID_FORMAT_ERROR == int64ConversionU) && (U_INVALID_FORMAT_ERROR == int64ConversionF)) {
...
{
    {
        ...
        assertTrue((UnicodeString) "comparing ufmt_open() with Formattable()",
            (defaultFormattable == *(Formattable::fromUFormattable(defaultUFormattable.getAlias()))));
        assertTrue((UnicodeString) "comparing ufmt_open() with Formattable()",
            (defaultFormattable == *(Formattable::fromUFormattable(defaultUFormattable.getAlias()))));
        assertTrue((UnicodeString) "comparing Formattable() round tripped through UFormattable",
            (defaultFormattable == *(Formattable::fromUFormattable(defaultFormattable.toUFormattable()))));
        assertTrue((UnicodeString) "comparing &Formattable() round tripped through UFormattable",
            ((&defaultFormattable) == Formattable::fromUFormattable(defaultFormattable.toUFormattable())));
    ...
    {
        Formattable f(
            (int64_t)80994231587905127LL); // weight of the moon, in kilotons http://solarsystem.nasa.gov/planets/profile.cfm?Display=Facts&Object=Moon
        testFormattableAsUFormattable(__FILE__, __LINE__, f);
    }
    {
...
void NumberFormatTest::TestSignificantDigits(void)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    Locale locale("en_US");
    LocalPointer<DecimalFormat> numberFormat(static_cast<DecimalFormat*>(NumberFormat::createInstance(locale, status)));
...
void NumberFormatTest::TestRoundingScientific10542()
{
    ...
    DecimalFormat::ERoundingMode roundingModes[] = { DecimalFormat::kRoundCeiling, DecimalFormat::kRoundDown, DecimalFormat::kRoundFloor,
        DecimalFormat::kRoundHalfDown, DecimalFormat::kRoundHalfEven, DecimalFormat::kRoundHalfUp, DecimalFormat::kRoundUp };
    const char* descriptions[] = { "Round Ceiling", "Round Down", "Round Floor", "Round half down", "Round half even", "Round half up", "Round up" };

    {
        ...
        const char* expected[] = { "-3.00E-3", "-3.00E-3", "-3.00E-3", "3.02E-3", "3.02E-3", "3.02E-3", "-3.00E-3", "-3.00E-3", "-3.00E-3", "3.01E-3",
            "3.01E-3", "3.01E-3", "-3.01E-3", "-3.01E-3", "-3.01E-3", "3.01E-3", "3.01E-3", "3.01E-3", "-3.01E-3", "-3.00E-3", "-3.00E-3", "3.01E-3", "3.01E-3",
            "3.02E-3", "-3.01E-3", "-3.00E-3", "-3.00E-3", "3.01E-3", "3.02E-3", "3.02E-3", "-3.01E-3", "-3.01E-3", "-3.00E-3", "3.01E-3", "3.02E-3", "3.02E-3",
            "-3.01E-3", "-3.01E-3", "-3.01E-3", "3.02E-3", "3.02E-3", "3.02E-3" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
    {
        double values[] = { -3006.0, -3005, -3004, 3014, 3015, 3016 };
        // The order of these expected values correspond to the order of roundingModes and the order of values.
        const char* expected[] = { "-3.00E3", "-3.00E3", "-3.00E3", "3.02E3", "3.02E3", "3.02E3", "-3.00E3", "-3.00E3", "-3.00E3", "3.01E3", "3.01E3", "3.01E3",
            "-3.01E3", "-3.01E3", "-3.01E3", "3.01E3", "3.01E3", "3.01E3", "-3.01E3", "-3.00E3", "-3.00E3", "3.01E3", "3.01E3", "3.02E3", "-3.01E3", "-3.00E3",
            "-3.00E3", "3.01E3", "3.02E3", "3.02E3", "-3.01E3", "-3.01E3", "-3.00E3", "3.01E3", "3.02E3", "3.02E3", "-3.01E3", "-3.01E3", "-3.01E3", "3.02E3",
            "3.02E3", "3.02E3" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
    ...
    */
    {
        ...
        // The order of these expected values correspond to the order of roundingModes and the order of values.
        const char* expected[] = { "1.00E25", "1.01E25", "1.00E25", "1.00E25", "1.00E25", "9.99E24", "1.00E25", "1.00E25", "9.99E24", "1.00E25", "1.00E25",
            "1.00E25", "1.00E25", "1.00E25", "1.00E25", "1.00E25", "1.00E25", "1.00E25", "1.00E25", "1.01E25", "1.00E25" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
    {
        double values[] = { -1e25, -1e25 + 1e15, -1e25 - 1e15 };
        // The order of these expected values correspond to the order of roundingModes and the order of values.
        const char* expected[] = { "-1.00E25", "-9.99E24", "-1.00E25", "-1.00E25", "-9.99E24", "-1.00E25", "-1.00E25", "-1.00E25", "-1.01E25", "-1.00E25",
            "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.00E25", "-1.01E25" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
    {
        double values[] = { 1e-25, 1e-25 + 1e-35, 1e-25 - 1e-35 };
        // The order of these expected values correspond to the order of roundingModes and the order of values.
        const char* expected[] = { "1.00E-25", "1.01E-25", "1.00E-25", "1.00E-25", "1.00E-25", "9.99E-26", "1.00E-25", "1.00E-25", "9.99E-26", "1.00E-25",
            "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.00E-25", "1.01E-25", "1.00E-25" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
    {
        double values[] = { -1e-25, -1e-25 + 1e-35, -1e-25 - 1e-35 };
        // The order of these expected values correspond to the order of roundingModes and the order of values.
        const char* expected[]
            = { "-1.00E-25", "-9.99E-26", "-1.00E-25", "-1.00E-25", "-9.99E-26", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.01E-25", "-1.00E-25", "-1.00E-25",
                  "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.00E-25", "-1.01E-25" };
        verifyRounding(format, values, expected, roundingModes, descriptions, UPRV_LENGTHOF(values), UPRV_LENGTHOF(roundingModes));
    }
}
...
void NumberFormatTest::verifyRounding(DecimalFormat& format, const double* values, const char* const* expected,
    const DecimalFormat::ERoundingMode* roundingModes, const char* const* descriptions, int32_t valueSize, int32_t roundingModeSize)
{
    for (int32_t i = 0; i < roundingModeSize; ++i) {
        ...
        for (int32_t j = 0; j < valueSize; j++) {
            ...
            if (currentExpected != actual) {
                dataerrln("For %s value %f, expected '%s', got '%s'", descriptions[i], values[j], CStr(currentExpected)(), CStr(actual)());
            }
        }
    }
}
...
void NumberFormatTest::TestAccountingCurrency()
{
    ...
    expect(NumberFormat::createInstance("en_US", style, status), (Formattable)(double)1234.5, "$1,234.50", TRUE, status);
    expect(NumberFormat::createInstance("en_US", style, status), (Formattable)(double)-1234.5, "($1,234.50)", TRUE, status);
    expect(NumberFormat::createInstance("en_US", style, status), (Formattable)(double)0, "$0.00", TRUE, status);
    expect(NumberFormat::createInstance("en_US", style, status), (Formattable)(double)-0.2, "($0.20)", TRUE, status);
    expect(NumberFormat::createInstance("ja_JP", style, status), (Formattable)(double)10000, UnicodeString("\\uFFE510,000").unescape(), TRUE, status);
    expect(NumberFormat::createInstance("ja_JP", style, status), (Formattable)(double)-1000.5, UnicodeString("(\\uFFE51,000)").unescape(), FALSE, status);
    expect(NumberFormat::createInstance("de_DE", style, status), (Formattable)(double)-23456.7, UnicodeString("-23.456,70\\u00A0\\u20AC").unescape(), TRUE,
        status);
    expect(NumberFormat::createInstance("en_ID", style, status), (Formattable)(double)0, UnicodeString("IDR\\u00A00.00").unescape(), TRUE, status);
    expect(NumberFormat::createInstance("en_ID", style, status), (Formattable)(double)-0.2, UnicodeString("(IDR\\u00A00.20)").unescape(), TRUE, status);
    expect(NumberFormat::createInstance("sh_ME", style, status), (Formattable)(double)0, UnicodeString("0,00\\u00A0\\u20AC").unescape(), TRUE, status);
    expect(NumberFormat::createInstance("sh_ME", style, status), (Formattable)(double)-0.2, UnicodeString("(0,20\\u00A0\\u20AC)").unescape(), TRUE, status);
}
...
void NumberFormatTest::TestCtorApplyPatternDifference()
{
    ...
    UnicodeString result;
    assertEquals("ctor favors precision of currency", "$5.00", fmt.format((double)5, result));
    result.remove();
    fmt.applyPattern(pattern.unescape(), status);
    assertEquals("applyPattern favors precision of pattern", "$5", fmt.format((double)5, result));
}
...
void NumberFormatTest::Test11868()
{
    ...
    LocalPointer<NumberFormat> fmt(NumberFormat::createInstance(selectedLocale, UNUM_CURRENCY_PLURAL, status));
    if (!assertSuccess("Format creation", status)) {
        return;
    }
    ...
    {
        NumberFormatTest_Attributes attributes[] = { { UNUM_INTEGER_FIELD, 0, 2 }, { UNUM_DECIMAL_SEPARATOR_FIELD, 2, 3 }, { UNUM_FRACTION_FIELD, 3, 5 },
            { UNUM_CURRENCY_FIELD, 6, 16 }, { 0, -1, 0 } };
        UnicodeString result;
    ...
    // Test field position iterator
    {
        NumberFormatTest_Attributes attributes[] = { { UNUM_SIGN_FIELD, 0, 1 }, { UNUM_GROUPING_SEPARATOR_FIELD, 2, 3 }, { UNUM_INTEGER_FIELD, 1, 6 },
            { UNUM_DECIMAL_SEPARATOR_FIELD, 6, 7 }, { UNUM_FRACTION_FIELD, 7, 9 }, { UNUM_CURRENCY_FIELD, 10, 20 }, { 0, -1, 0 } };
        UnicodeString result;
        FieldPositionIterator iter;
...
void NumberFormatTest::Test11376_getAndSetPositivePrefix()
{
    ...
    }
    {
        const UChar USD[] = { 0x55, 0x53, 0x44, 0x0 };
        UErrorCode status = U_ZERO_ERROR;
        LocalPointer<NumberFormat> fmt(NumberFormat::createInstance("en", UNUM_CURRENCY_PLURAL, status));
        if (!assertSuccess("", status)) {
            return;
        }
        DecimalFormat* dfmt = (DecimalFormat*)fmt.getAlias();
...
void NumberFormatTest::Test11475_signRecognition()
{
    ...
    UnicodeString result;
    {
        ...
        NumberFormatTest_Attributes attributes[]
            = { { UNUM_SIGN_FIELD, 0, 1 }, { UNUM_INTEGER_FIELD, 1, 2 }, { UNUM_DECIMAL_SEPARATOR_FIELD, 2, 3 }, { UNUM_FRACTION_FIELD, 3, 5 }, { 0, -1, 0 } };
        UnicodeString result;
        FieldPositionIterator iter;
    ...
    }
    {
        ...
        {
            NumberFormatTest_Attributes attributes[] = { { UNUM_SIGN_FIELD, 0, 2 }, { UNUM_INTEGER_FIELD, 2, 3 }, { UNUM_DECIMAL_SEPARATOR_FIELD, 3, 4 },
                { UNUM_FRACTION_FIELD, 4, 6 }, { UNUM_SIGN_FIELD, 6, 7 }, { 0, -1, 0 } };
            UnicodeString result;
            FieldPositionIterator iter;
            fmt.format(2.3, result, &iter, status);
            assertEquals("", "++2.30+", result);
        ...
        {
            NumberFormatTest_Attributes attributes[] = { { UNUM_SIGN_FIELD, 0, 1 }, { UNUM_INTEGER_FIELD, 2, 3 }, { UNUM_DECIMAL_SEPARATOR_FIELD, 3, 4 },
                { UNUM_FRACTION_FIELD, 4, 6 }, { UNUM_SIGN_FIELD, 7, 9 }, { 0, -1, 0 } };
            UnicodeString result;
            FieldPositionIterator iter;
            fmt.format(-2.3, result, &iter, status);
            assertEquals("", "-(2.30)--", result);
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<DecimalFormat> df(dynamic_cast<DecimalFormat*>(NumberFormat::createInstance(Locale("ccp"), status)));
    if (df == nullptr) {
    ...
    df.adoptInstead(static_cast<DecimalFormat*>(NumberFormat::createScientificInstance(Locale("ccp"), status)));
    df->format(130, actualScientific, status);
    assertSuccess("Should not fail when formatting scientific in ccp", status);
    assertEquals("Should produce expected scientific output in ccp", expectedScientific, actualScientific);
...
void NumberFormatTest::Test11035_FormatCurrencyAmount()
{
    ...
    LocalPointer<NumberFormat> fmt1(NumberFormat::createCurrencyInstance(loc1, status), status);
    if (U_FAILURE(status)) {
...
void NumberFormatTest::TestParsePercentRegression()
{
    ...
    }
    {
        ...
        Formattable result;
        df2->parse("50", result, ppos);
        assertEquals("df2 should return the number as 0.5 even though the percent sign is missing", 0.5, result.getDouble(status));
    }
}
...
void NumberFormatTest::TestFastFormatInt32()
{
    ...
    double nums[] = { 0.0, -0.0, NAN, INFINITY, 0.1, 1.0, 1.1, 2.0, 3.0, 9.0, 10.0, 99.0, 100.0, 999.0, 1000.0, 9999.0, 10000.0, 99999.0, 100000.0, 999999.0,
        1000000.0, static_cast<double>(INT32_MAX) - 1, static_cast<double>(INT32_MAX), static_cast<double>(INT32_MAX) + 1, static_cast<double>(INT32_MIN) - 1,
        static_cast<double>(INT32_MIN), static_cast<double>(INT32_MIN) + 1 };
...
void NumberFormatTest::TestMinimumGroupingDigits()
{
    ...
    for (const auto& cas : cases) {
        UnicodeString message = UnicodeString(cas.locale) + u" " + Int64ToUnicodeString(cas.minGroup) + u" " + DoubleToUnicodeString(cas.input);
...
void NumberFormatTest::Test11897_LocalizedPatternSeparator()
{
    ...
    // kPatternSeparatorSymbol should still be ';'
    {
        ...
        assertEquals("pattern separator symbol should be ;", u";", dfs.getSymbol(DecimalFormatSymbols::kPatternSeparatorSymbol));
    }
...
void NumberFormatTest::Test12567()
{
    ...
    LocalPointer<DecimalFormat> df1((DecimalFormat*)NumberFormat::createInstance(Locale::getUS(), UNUM_CURRENCY, errorCode));
    LocalPointer<DecimalFormat> df2((DecimalFormat*)NumberFormat::createInstance(Locale::getUS(), UNUM_DECIMAL, errorCode));
    if (!assertSuccess("", errorCode, true, __FILE__, __LINE__)) {
        return;
    }
...
void NumberFormatTest::Test11648_ExpDecFormatMalPattern()
{
    ...
    assertEquals("A valid scientific notation pattern should be produced", "0.00E0", fmt.toPattern(pattern));
...
void NumberFormatTest::Test13148_ParseGroupingSeparators()
{
    ...
    LocalPointer<DecimalFormat> fmt((DecimalFormat*)NumberFormat::createInstance("en-ZA", status), status);
    if (!assertSuccess("", status, true, __FILE__, __LINE__)) {
        return;
    }
...
void NumberFormatTest::Test12753_PatternDecimalPoint()
{
    ...
    assertEquals("Parsing integer succeeded even though setDecimalPatternMatchRequired was set", U_INVALID_FORMAT_ERROR, status);
}
...
void NumberFormatTest::Test13777_ParseLongNameNonCurrencyMode()
{
    ...
    LocalPointer<NumberFormat> df(NumberFormat::createInstance("en-us", UNumberFormatStyle::UNUM_CURRENCY_PLURAL, status), status);
    if (!assertSuccess("", status, true, __FILE__, __LINE__)) {
        return;
    }
...
void NumberFormatTest::Test13804_EmptyStringsWhenParsing()
{
    ...
    const char16_t* samples[] = { u"", u"123", u"$123", u"-", u"+", u"44%", u"1E+2.3" };
...
void NumberFormatTest::Test20037_ScientificIntegerOverflow()
{
    ...
    assertEquals(u"Should snap to zero", u"0", { sp.data(), sp.length(), US_INV });

    // Test edge case overflow of exponent
    result = Formattable();
    nf->parse(u"1E-2147483647E-1", result, status);
    sp = result.getDecimalNumber(status);
    assertEquals(u"Should not overflow and should parse only the first exponent", u"1E-2147483647", { sp.data(), sp.length(), US_INV });

    // Test edge case overflow of exponent
    result = Formattable();
    nf->parse(u".0003e-2147483644", result, status);
    sp = result.getDecimalNumber(status);
    assertEquals(u"Should not overflow", u"3E-2147483648", { sp.data(), sp.length(), US_INV });

    // Test largest parseable exponent
    result = Formattable();
    nf->parse(u"9876e2147483643", result, status);
    sp = result.getDecimalNumber(status);
    assertEquals(u"Should not overflow", u"9.876E+2147483646", { sp.data(), sp.length(), US_INV });
    ...
    for (const auto& input : infinityInputs) {
        ...
        sp = result.getDecimalNumber(status);
        assertEquals(UnicodeString("Should become Infinity: ") + input, u"Infinity", { sp.data(), sp.length(), US_INV });
    }
}
...
void NumberFormatTest::Test13840_ParseLongStringCrash()
{
    ...
    Formattable result;
    static const char16_t* bigString = u"111111111111111111111111111111111111111111111111111111111111111111111"
...
void NumberFormatTest::Test13850_EmptyStringCurrency()
{
    ...
    } cases[] = { { u"", U_USING_FALLBACK_WARNING }, { u"U", U_ILLEGAL_ARGUMENT_ERROR }, { u"Us", U_ILLEGAL_ARGUMENT_ERROR },
        { nullptr, U_USING_FALLBACK_WARNING }, { u"U$D", U_INVARIANT_CONVERSION_ERROR }, { u"Xxx", U_USING_FALLBACK_WARNING } };
    for (const auto& cas : cases) {
...
void NumberFormatTest::Test20348_CurrencyPrefixOverride()
{
    ...
    LocalPointer<DecimalFormat> fmt(static_cast<DecimalFormat*>(NumberFormat::createCurrencyInstance("en", status)));
    if (status.errIfFailureAndReset()) {
        return;
    }
    UnicodeString result;
    assertEquals("Initial pattern", u"¤#,##0.00", fmt->toPattern(result.remove()));
    assertEquals("Initial prefix", u"¤", fmt->getPositivePrefix(result.remove()));
    assertEquals("Initial suffix", u"-¤", fmt->getNegativePrefix(result.remove()));
    assertEquals("Initial format", u"\u00A4100.00", fmt->format(100, result.remove(), NULL, status));

    fmt->setPositivePrefix(u"$");
    assertEquals("Set positive prefix pattern", u"$#,##0.00;-\u00A4#,##0.00", fmt->toPattern(result.remove()));
    assertEquals("Set positive prefix prefix", u"$", fmt->getPositivePrefix(result.remove()));
    assertEquals("Set positive prefix suffix", u"-¤", fmt->getNegativePrefix(result.remove()));
    assertEquals("Set positive prefix format", u"$100.00", fmt->format(100, result.remove(), NULL, status));

    fmt->setNegativePrefix(u"-$");
    assertEquals("Set negative prefix pattern", u"$#,##0.00;'-'$#,##0.00", fmt->toPattern(result.remove()));
    assertEquals("Set negative prefix prefix", u"$", fmt->getPositivePrefix(result.remove()));
    assertEquals("Set negative prefix suffix", u"-$", fmt->getNegativePrefix(result.remove()));
    assertEquals("Set negative prefix format", u"$100.00", fmt->format(100, result.remove(), NULL, status));
}

void NumberFormatTest::Test20956_MonetarySymbolGetters()
{
    IcuTestErrorCode status(*this, "Test20956_MonetarySymbolGetters");
    LocalPointer<DecimalFormat> decimalFormat(static_cast<DecimalFormat*>(NumberFormat::createCurrencyInstance("et", status)));
    if (status.errDataIfFailureAndReset()) {
        return;
    }
    ...
    assertEquals("MONETARY DECIMAL SEPARATOR", u".", decimalFormatSymbols->getSymbol(DecimalFormatSymbols::kMonetarySeparatorSymbol));
    assertEquals("DECIMAL SEPARATOR", u",", decimalFormatSymbols->getSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol));
    assertEquals("MONETARY GROUPING SEPARATOR", u" ", decimalFormatSymbols->getSymbol(DecimalFormatSymbols::kMonetaryGroupingSeparatorSymbol));
    assertEquals("GROUPING SEPARATOR", u" ", decimalFormatSymbols->getSymbol(DecimalFormatSymbols::kGroupingSeparatorSymbol));
    assertEquals("CURRENCY SYMBOL", u"kr", decimalFormatSymbols->getSymbol(DecimalFormatSymbols::kCurrencySymbol));
...
void NumberFormatTest::Test20358_GroupingInPattern()
{
    ...
    LocalPointer<DecimalFormat> fmt(static_cast<DecimalFormat*>(NumberFormat::createInstance("en", status)));
    if (status.errIfFailureAndReset()) {
        return;
    }
    UnicodeString result;
    assertEquals("Initial pattern", u"#,##0.###", fmt->toPattern(result.remove()));
    assertTrue("Initial grouping", fmt->isGroupingUsed());
    assertEquals("Initial format", u"54,321", fmt->format(54321, result.remove(), NULL, status));

    fmt->setGroupingUsed(false);
    assertEquals("Set grouping false", u"0.###", fmt->toPattern(result.remove()));
    assertFalse("Set grouping false grouping", fmt->isGroupingUsed());
    assertEquals("Set grouping false format", u"54321", fmt->format(54321, result.remove(), NULL, status));

    fmt->setGroupingUsed(true);
    assertEquals("Set grouping true", u"#,##0.###", fmt->toPattern(result.remove()));
    assertTrue("Set grouping true grouping", fmt->isGroupingUsed());
    assertEquals("Set grouping true format", u"54,321", fmt->format(54321, result.remove(), NULL, status));
}
...
void NumberFormatTest::Test13731_DefaultCurrency()
{
    ...
    UnicodeString result;
    {
        LocalPointer<NumberFormat> nf(NumberFormat::createInstance("en", UNumberFormatStyle::UNUM_CURRENCY, status), status);
        if (status.errIfFailureAndReset()) {
            return;
        }
        assertEquals("symbol", u"¤1.10", nf->format(1.1, result.remove(), status));
        assertEquals("currency", u"XXX", nf->getCurrency());
    }
    {
        LocalPointer<NumberFormat> nf(NumberFormat::createInstance("en", UNumberFormatStyle::UNUM_CURRENCY_ISO, status), status);
        if (status.errIfFailureAndReset()) {
            return;
        }
        assertEquals("iso_code", u"XXX 1.10", nf->format(1.1, result.remove(), status));
        assertEquals("currency", u"XXX", nf->getCurrency());
    }
    {
        LocalPointer<NumberFormat> nf(NumberFormat::createInstance("en", UNumberFormatStyle::UNUM_CURRENCY_PLURAL, status), status);
        if (status.errIfFailureAndReset()) {
            return;
        }
        assertEquals("plural", u"1.10 (unknown currency)", nf->format(1.1, result.remove(), status));
        assertEquals("currency", u"XXX", nf->getCurrency());
    }
}
...
void NumberFormatTest::Test20499_CurrencyVisibleDigitsPlural()
{
    ...
    LocalPointer<NumberFormat> nf(NumberFormat::createInstance("ro-RO", UNumberFormatStyle::UNUM_CURRENCY_PLURAL, status), status);
    const char16_t* expected = u"24,00 lei românești";
    for (int32_t i = 0; i < 5; i++) {
        UnicodeString actual;
        nf->format(24, actual, status);
        assertEquals(UnicodeString(u"iteration ") + Int64ToUnicodeString(i), expected, actual);
    }
}
...
void NumberFormatTest::Test20961_CurrencyPluralPattern()
{
    ...
    {
        LocalPointer<DecimalFormat> decimalFormat(static_cast<DecimalFormat*>(NumberFormat::createInstance("en-US", UNUM_CURRENCY_PLURAL, status)));
        if (status.errDataIfFailureAndReset()) {
            return;
        }
        UnicodeString result;
...
void NumberFormatTest::Test21134_ToNumberFormatter()
{
    ...
    }
    {
        ...
        LocalPointer<DecimalFormat> inner(static_cast<DecimalFormat*>(DecimalFormat::createInstance("en-US", UNUM_CURRENCY_PLURAL, status)));
        if (auto ptr = inner->toNumberFormatter(status)) {
            // Copy constructor
    ...
    assertEquals("Using NumberFormatter from DecimalFormat, new version", u"a99b", result1.toTempString(status));
    auto result2 = outer2.formatDouble(99, status);
    assertEquals("Using NumberFormatter from DecimalFormat, compiled version", u"c99d", result2.toTempString(status));
    auto result3 = outer3->formatDouble(99, status);
    assertEquals("Using NumberFormatter from DecimalFormat, compiled version", u"99.00 US dollars", result3.toTempString(status));
}
...
void NumberFormatTest::Test13733_StrictAndLenient()
{
    ...
    } cases[] = { { u"CA$ 12", u"¤ 0", 12, 12 }, { u"CA$12", u"¤0", 12, 12 }, { u"CAD 12", u"¤¤ 0", 12, 12 }, { u"12 CAD", u"0 ¤¤", 12, 12 },
        { u"12 Canadian dollars", u"0 ¤¤¤", 12, 12 }, { u"$12 ", u"¤¤¤¤0", 12, 12 }, { u"12$", u"0¤¤¤¤", 12, 12 }, { u"CA$ 12", u"¤0", 0, 12 },
        { u"CA$ 12", u"0 ¤¤", 0, 12 }, { u"CA$ 12", u"0 ¤¤¤", 0, 12 }, { u"CA$ 12", u"¤¤¤¤0", 0, 12 }, { u"CA$ 12", u"0¤¤¤¤", 0, 12 },
        { u"CA$12", u"¤ 0", 0, 12 }, { u"CA$12", u"¤¤ 0", 0, 12 }, { u"CA$12", u"0 ¤¤", 0, 12 }, { u"CA$12", u"0 ¤¤¤", 0, 12 }, { u"CA$12", u"0¤¤¤¤", 0, 12 },
        { u"CAD 12", u"¤0", 0, 12 }, { u"CAD 12", u"0 ¤¤", 0, 12 }, { u"CAD 12", u"0 ¤¤¤", 0, 12 }, { u"CAD 12", u"¤¤¤¤0", 0, 12 },
        { u"CAD 12", u"0¤¤¤¤", 0, 12 }, { u"12 CAD", u"¤ 0", 0, 12 }, { u"12 CAD", u"¤0", 0, 12 }, { u"12 CAD", u"¤¤ 0", 0, 12 },
        { u"12 CAD", u"¤¤¤¤0", 0, 12 }, { u"12 CAD", u"0¤¤¤¤", 0, 12 }, { u"12 Canadian dollars", u"¤ 0", 0, 12 }, { u"12 Canadian dollars", u"¤0", 0, 12 },
        { u"12 Canadian dollars", u"¤¤ 0", 0, 12 }, { u"12 Canadian dollars", u"¤¤¤¤0", 0, 12 }, { u"12 Canadian dollars", u"0¤¤¤¤", 0, 12 },
        { u"$12 ", u"¤ 0", 0, 12 }, { u"$12 ", u"¤¤ 0", 0, 12 }, { u"$12 ", u"0 ¤¤", 0, 12 }, { u"$12 ", u"0 ¤¤¤", 0, 12 }, { u"$12 ", u"0¤¤¤¤", 0, 12 },
        { u"12$", u"¤ 0", 0, 12 }, { u"12$", u"¤0", 0, 12 }, { u"12$", u"¤¤ 0", 0, 12 }, { u"12$", u"0 ¤¤", 0, 12 }, { u"12$", u"0 ¤¤¤", 0, 12 },
        { u"12$", u"¤¤¤¤0", 0, 12 } };
    for (auto& cas : cases) {
        ...
        assertEquals("Strict parse of " + inputString + " using " + patternString, parsedStrictValue, cas.expectedStrictParse);
        ...
        assertEquals("Lenient parse of " + inputString + " using " + patternString, parsedLenientValue, cas.expectedLenientParse);
    }
}
