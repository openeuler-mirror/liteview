...
{
    unsigned char c = 'A';
    if ((U_CHARSET_FAMILY == U_ASCII_FAMILY && c != 0x41) || (U_CHARSET_FAMILY == U_EBCDIC_FAMILY && c != 0xc1)) {
...
void StringTest::Test_U_STRING()
{
    ...
    if (u_strlen(ustringVar) != 5 || ustringVar[0] != 0x61 || ustringVar[1] != 0x5a || ustringVar[2] != 0x30 || ustringVar[3] != 0x20 || ustringVar[4] != 0x2d
        || ustringVar[5] != 0) {
...
void StringTest::Test_UNICODE_STRING()
{
    ...
    if (ustringVar.length() != 5 || ustringVar[0] != 0x61 || ustringVar[1] != 0x5a || ustringVar[2] != 0x30 || ustringVar[3] != 0x20 || ustringVar[4] != 0x2d) {
...
const char* nativeInvChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                             "abcdefghijklmnopqrstuvwxyz"
                             "0123456789 \"%&'()*+,-./:;<=>?_";
const char16_t* asciiInvChars = u"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
void StringTest::TestUpperOrdinal()
{
    for (int32_t i = 0;; ++i) {
        ...
        if (0 <= expected && expected <= 25) {
            if (actual != expected) {
                errln("uprv_upperOrdinal('%c')=%d != expected %d", ic, (int)actual, (int)expected);
            }
        } else {
            if (0 <= actual && actual <= 25) {
                errln("uprv_upperOrdinal('%c')=%d should have been outside 0..25", ic, (int)actual);
            }
        }
        if (ic == 0) {
            break;
...
void StringTest::TestLowerOrdinal()
{
    for (int32_t i = 0;; ++i) {
        ...
        if (0 <= expected && expected <= 25) {
            if (actual != expected) {
                errln("uprv_lowerOrdinal('%c')=%d != expected %d", ic, (int)actual, (int)expected);
            }
        } else {
            if (0 <= actual && actual <= 25) {
                errln("uprv_lowerOrdinal('%c')=%d should have been outside 0..25", ic, (int)actual);
            }
        }
        if (ic == 0) {
            break;
...
void StringTest::Test_UTF8_COUNT_TRAIL_BYTES()
{
    ...
    if (UTF8_COUNT_TRAIL_BYTES(0x7F) != 0 || UTF8_COUNT_TRAIL_BYTES(0xC2) != 1 || UTF8_COUNT_TRAIL_BYTES(0xE0) != 2 || UTF8_COUNT_TRAIL_BYTES(0xF0) != 3) {
    ...
    if (U8_COUNT_TRAIL_BYTES(0x7F) != 0 || U8_COUNT_TRAIL_BYTES(0xC2) != 1 || U8_COUNT_TRAIL_BYTES(0xE0) != 2 || U8_COUNT_TRAIL_BYTES(0xF0) != 3) {
...
void StringTest::TestStringPieceFind()
{
    struct TestCase {
        ...
        const char* needle;
        int32_t expected;
    } cases[] = { { "", "", 0 }, { "", "x", -1 }, { "x", "", 0 }, { "x", "x", 0 }, { "xy", "x", 0 }, { "xy", "y", 1 }, { "xy", "xy", 0 }, { "xy", "xyz", -1 },
        { "qwerty", "qqw", -1 }, { "qwerty", "qw", 0 }, { "qwerty", "er", 2 }, { "qwerty", "err", -1 }, { "qwerty", "ert", 2 }, { "qwerty", "ty", 4 },
        { "qwerty", "tyy", -1 }, { "qwerty", "a", -1 }, { "qwerty", "abc", -1 } };
    int32_t caseNumber = 0;
    for (auto& cas : cases) {
        StringPiece haystack(cas.haystack);
        StringPiece needle(cas.needle);
        assertEquals(Int64ToUnicodeString(caseNumber), cas.expected, haystack.find(needle, 0));
        // Should be same as std::string::find
        std::string stdhaystack(cas.haystack);
        std::string stdneedle(cas.needle);
        assertEquals(Int64ToUnicodeString(caseNumber) + u" (std)", cas.expected, static_cast<int32_t>(stdhaystack.find(stdneedle, 0)));
        // Test offsets against std::string::find
        for (int32_t offset = 0; offset < haystack.length(); offset++) {
            assertEquals(Int64ToUnicodeString(caseNumber) + "u @ " + Int64ToUnicodeString(offset), static_cast<int32_t>(stdhaystack.find(stdneedle, offset)),
                haystack.find(needle, offset));
        }
        caseNumber++;
    }
}
...
void StringTest::TestCheckedArrayByteSink()
{
    ...
    sink.Append("abc", 3);
    if (!(sink.NumberOfBytesAppended() == 3 && sink.NumberOfBytesWritten() == 3 && 0 == memcmp("abc", buffer, 3) && buffer[3] == '!') && !sink.Overflowed()) {
    ...
    sink.Append(dest, 10);
    if (!(sink.NumberOfBytesAppended() == 13 && sink.NumberOfBytesWritten() == 13 && 0 == memcmp("abcdefghijklm", buffer, 13) && !sink.Overflowed())) {
    ...
    sink.Append(dest, 10);
    if (!(sink.NumberOfBytesAppended() == 23 && sink.NumberOfBytesWritten() == (int32_t)sizeof(buffer)
            && 0 == memcmp("abcdefghijklmnopqrstuvwxyz", buffer, (int32_t)sizeof(buffer)) && sink.Overflowed())) {
        errln("CheckedArrayByteSink did not Append(scratch buffer) as expected");
        return;
    }
    sink.Reset().Append("123", 3);
    if (!(sink.NumberOfBytesAppended() == 3 && sink.NumberOfBytesWritten() == 3 && 0 == memcmp("123defghijklmnopqrstuvwxyz", buffer, (int32_t)sizeof(buffer))
            && !sink.Overflowed())) {
...
void StringTest::TestStringByteSink()
{
    ...
    if (result.capacity() < (result.length() + 20)) {
        errln("StringByteSink should have 20 append capacity, has only %d", (int)(result.capacity() - result.length()));
    }
    sink.Append("ghi", 3);
    if (result != "abcdefghi") {
        errln("StringByteSink did not Append() as expected");
...
void StringTest::TestCharString()
{
    ...
    chStr.appendInvariantChars(
        UNICODE_STRING_SIMPLE("This is a long string that is meant to cause reallocation of the internal buffer of CharString."), errorCode);
...

    {
        ...
        CharString s2(std::move(s1));
        assertEquals("s2 should have content of s1", "Long string over 40 characters to trigger heap allocation", s2.data());
        CharString s3("Dummy string with over 40 characters to trigger heap allocation", errorCode);
        s3 = std::move(s2);
        assertEquals("s3 should have content of s2", "Long string over 40 characters to trigger heap allocation", s3.data());
    }

    {
        ...
        assertEquals("abc.extract(3) not terminated", U_STRING_NOT_TERMINATED_WARNING, errorCode.reset());
