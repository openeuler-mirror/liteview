...
static NumberFormat* nfWithPattern(const char* pattern)
{
    ...
    DecimalFormat* result = new DecimalFormat(upattern, new DecimalFormatSymbols("fr", status), status);
    if (U_FAILURE(status)) {
        return NULL;
    }
...
private:
    void assertPatternFr(const char* expected, double x, const char* pattern, UBool possibleDataError = FALSE);
};

void NumberFormatSpecificationTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char*)
{
    if (exec) {
...
void NumberFormatSpecificationTest::TestPadding()
{
    ...
    assertPatternFr("EUR *433,00", 433.0, "\\u00a4\\u00a4 **#######0");
    {
        ...
        UErrorCode status = U_ZERO_ERROR;
        UnicodeString result;
        DecimalFormat fmt(upattern, new DecimalFormatSymbols("fr", status), status);
        if (U_FAILURE(status)) {
    ...
    }
    {
        ...
        UErrorCode status = U_ZERO_ERROR;
        UnicodeString result;
        DecimalFormat fmt(upattern, new DecimalFormatSymbols("en_US", status), status);
        if (U_FAILURE(status)) {
            dataerrln("Error creating DecimalFormat - %s", u_errorName(status));
        } else {
            fmt.format(-433.22, result);
    ...
    assertPatternFr("QU***43,3E-1", 4.33, paddedSciPattern, TRUE);
    {
        ...
        DecimalFormat fmt(paddedSciPattern, sym, status);
        if (U_FAILURE(status)) {
            dataerrln("Error creating DecimalFormat - %s", u_errorName(status));
        } else {
            UnicodeString result;
...
void NumberFormatSpecificationTest::assertPatternFr(const char* expected, double x, const char* pattern, UBool possibleDataError)
{
    UnicodeString upattern(pattern, -1, US_INV);
    ...
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString result;
    DecimalFormat fmt(upattern, new DecimalFormatSymbols("fr_FR", status), status);
    if (U_FAILURE(status)) {
