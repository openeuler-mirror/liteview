...
void DataDrivenFormatTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par */)
{
    if (driver != NULL) {
        if (exec) {
            //  logln("Begin ");
...
void DataDrivenFormatTest::testConvertDate(TestData* testData, const DataMap* /* settings */, UBool fmt)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    SimpleDateFormat basicFmt(UnicodeString("EEE MMM dd yyyy / YYYY'-W'ww-ee"), status);
    if (U_FAILURE(status)) {
        dataerrln("FAIL: Couldn't create basic SimpleDateFormat: %s", u_errorName(status));
        return;
    }
        ...
        if (date.startsWith(kMILLIS)) {
            ...
            for (int q = 0; q < UCAL_FIELD_COUNT; q++) {
                if (fromSet.isSet((UCalendarDateFields)q)) {
                    ...
                    if (q == UCAL_DATE) {
                        cal->add((UCalendarDateFields)q, fromSet.get((UCalendarDateFields)q), status);
                    } else {
                        cal->set((UCalendarDateFields)q, fromSet.get((UCalendarDateFields)q));
                    }
        ...
        if (fmt) {
            ...
            if (useDate) {
                ...
                if (!fromSet.matches(cal, diffSet, status)) {
                    ...
                    errln((UnicodeString) "FAIL: " + caseString + ", Differences: '" + diffs + "', status: " + u_errorName(status));
                } else if (U_FAILURE(status)) {
                    errln("FAIL: " + caseString + " parse SET SOURCE calendar Failed to match: " + u_errorName(status));
                } else {
