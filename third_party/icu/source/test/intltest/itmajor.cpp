...
void MajorTestLevel::runIndexedTest(int32_t index, UBool exec, const char*& name, char* par)
{
...
#if !UCONFIG_NO_FORMATTING
        if (exec) {
            logln("TestSuite RuleBasedNumberFormat RT----");
            logln();
...
#if !UCONFIG_NO_FORMATTING
        if (exec) {
            logln("TestSuite RuleBasedNumberParse ----");
            logln();
    ...
    case 13:
        name = "csdet";
        if (exec) {
            logln("TestSuite CharsetDetection---");
            logln();
    ...
    case 14:
        ...
        name = "spoof";
        if (exec) {
            logln("TestSuite SpoofDetection---");
            logln();
            IntlTestSpoof test;
            callTest(test, par);
        }
