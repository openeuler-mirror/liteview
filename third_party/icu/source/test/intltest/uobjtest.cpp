...
#define TESTCLASSID_NONE_DEFAULT(c) delete testClassNoClassID(new c, #c, "new " #c)
...
#define TESTCLASSID_TRANSLIT(c, t)                                                                                                                             \
    ...
    {                                                                                                                                                          \
        delete testClass(                                                                                                                                      \
            Transliterator::createInstance(UnicodeString(t), UTRANS_FORWARD, parseError, status), #c, "Transliterator: " #t, c ::getStaticClassID());          \
        if (U_FAILURE(status)) {                                                                                                                               \
...
public:
    TestLocaleKeyFactory(int32_t coverage)
...
void UObjectTest::testIDs()
{
    ...
    TESTCLASSID_FACTORY(
        RuleBasedTransliterator, Transliterator::createFromRules(UnicodeString("abcd"), UnicodeString("a>b;"), UTRANS_FORWARD, parseError, status));
...
void UObjectTest::TestCompilerRTTI()
{
    ...
    UnicodeSet emptySet;
    if (&typeid(*nf) == NULL || typeid(*nf) == typeid(UObject) || typeid(*nf) == typeid(Format) || typeid(*nf) != typeid(DecimalFormat)
        || typeid(*nf) == typeid(ChoiceFormat) || typeid(*nf) == typeid(emptySet)) {
