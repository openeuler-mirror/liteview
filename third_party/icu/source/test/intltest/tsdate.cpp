...
void IntlTestDateFormat::testLocale(/*char* par, */ const Locale& locale, const UnicodeString& localeName)
{
    ...
    for (timeStyle = (DateFormat::EStyle)0; timeStyle < (DateFormat::EStyle)4; timeStyle = (DateFormat::EStyle)(timeStyle + 1)) {
    ...
    for (dateStyle = (DateFormat::EStyle)0; dateStyle < (DateFormat::EStyle)4; dateStyle = (DateFormat::EStyle)(dateStyle + 1)) {
...
void IntlTestDateFormat::describeTest()
{
...
void IntlTestDateFormat::tryDate(UDate theDate)
{
    ...
    for (i = 1; i < DEPTH; ++i) {
        ...
        if (U_FAILURE(status)) {
            describeTest();
            errln("**** FAIL, locale " + UnicodeString(locID, -1, US_INV) + ": Parse of " + prettify(string[i - 1], FALSE) + " failed.");
            dump = TRUE;
            break;
        }
        ...
        if (dateMatch == 0 && date[i] == date[i - 1])
            ...
            describeTest();
            errln("**** FAIL, locale " + UnicodeString(locID, -1, US_INV) + ": Date mismatch after match for " + string[i]);
            dump = TRUE;
            break;
        }
        if (stringMatch == 0 && string[i] == string[i - 1])
            ...
            describeTest();
            errln("**** FAIL, locale " + UnicodeString(locID, -1, US_INV) + ": String mismatch after match for " + string[i]);
            dump = TRUE;
            break;
        }
    ...
    if (stringMatch > fLimit || dateMatch > fLimit) {
        describeTest();
        errln((UnicodeString) "**** FAIL: No string and/or date match within " + fLimit + " iterations for the Date " + string[0] + "\t(" + theDate + ").");
        dump = TRUE;
    }

    if (dump) {
        for (int32_t k = 0; k <= i; ++k) {
            logln((UnicodeString) "" + k + ": " + date[k] + " F> " + string[k] + " P> ");
        }
    }
}
