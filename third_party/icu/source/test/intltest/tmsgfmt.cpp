...
void TestMessageFormat::testBug3()
{
    ...
    Locale locale[] = {
        Locale("ar", "", ""), Locale("be", "", ""), Locale("bg", "", ""), Locale("ca", "", ""), Locale("cs", "", ""), Locale("da", "", ""),
        Locale("de", "", ""), Locale("de", "AT", ""), Locale("de", "CH", ""), Locale("el", "", ""), // 10
        Locale("en", "CA", ""), Locale("en", "GB", ""), Locale("en", "IE", ""), Locale("en", "US", ""), Locale("es", "", ""), Locale("et", "", ""),
        Locale("fi", "", ""), Locale("fr", "", ""), Locale("fr", "BE", ""), Locale("fr", "CA", ""), // 20
        Locale("fr", "CH", ""), Locale("he", "", ""), Locale("hr", "", ""), Locale("hu", "", ""), Locale("is", "", ""), Locale("it", "", ""),
        Locale("it", "CH", ""), Locale("ja", "", ""), Locale("ko", "", ""), Locale("lt", "", ""), // 30
        Locale("lv", "", ""), Locale("mk", "", ""), Locale("nl", "", ""), Locale("nl", "BE", ""), Locale("no", "", ""), Locale("pl", "", ""),
        Locale("pt", "", ""), Locale("ro", "", ""), Locale("ru", "", ""), Locale("sh", "", ""), // 40
        Locale("sk", "", ""), Locale("sl", "", ""), Locale("sq", "", ""), Locale("sr", "", ""), Locale("sv", "", ""), Locale("tr", "", ""),
        Locale("uk", "", ""), Locale("zh", "", ""), Locale("zh", "TW", "") // 49
    };
    int32_t i;
    for (i = 0; i < 49; i++) {
...
void TestMessageFormat::testBug1()
{
    ...
    const UnicodeString formats[] = { "0.0<=Arg<1.0", "1.0<=Arg<2.0", "2.0<-Arg" };
...
void TestMessageFormat::PatternTest()
{
    Formattable testArgs[]
        = { Formattable(double(1)), Formattable(double(3456)), Formattable("Disk"), Formattable(UDate((int32_t)1000000000L), Formattable::kIsDate) };
    ...
    UnicodeString testResultStrings[] = { "Quotes ', {, 'a' 1 {0}", "Quotes ', {, 'a' 1 {0}", "{1,number,'#',##} #34,56",
        "There are 3,456 files on Disk at 1/12/70, 5:46 AM.", "On Disk, there are 3,456 files, with $1.00.", "{1,number,percent}, 345,600%,",
        "{1,date,full}, Wednesday, December 31, 1969,", "{3,date,full}, Monday, January 12, 1970,", "{1,number,#,##} 34,56" };
...
void TestMessageFormat::sample()
{
    ...
    FieldPosition fieldpos(FieldPosition::DONT_CARE);
    assertEquals("format", "There are abc files on def", form->format(testArgs1, 2, buffer2, fieldpos, success));
...
void TestMessageFormat::testStaticFormat()
{
    ...
    Formattable arguments[] = { (int32_t)7, Formattable(UDate(8.71068e+011), Formattable::kIsDate), "a disturbance in the Force" };

    UnicodeString result;
    result = MessageFormat::format("At {1,time} on {1,date}, there was {2} on planet {0,number,integer}.", arguments, 3, result, err);
    ...
    const UnicodeString expected("At 12:20:00 PM on Aug 8, 1997, there was a disturbance in the Force on planet 7.", "");
    if (result != expected) {
...
void TestMessageFormat::TestTurkishCasing()
{
    ...
    UnicodeString result;
    result = MessageFormat::format("At {1,TIME} on {1,DATE,SHORT}, there was {2} on planet {0,NUMBER,INTEGER}.", arguments, 3, result, err);
    ...
    const UnicodeString expected("At 12:20:00 on 8.08.1997, there was a disturbance in the Force on planet 7.", "");
    if (result != expected) {
...
void TestMessageFormat::testSimpleFormat(/* char* par */)
{
    ...
    MessageFormat* form = new MessageFormat("The disk \"{1}\" contains {0} file(s).", err);
...
void TestMessageFormat::testApostropheInPluralAndSelect()
{
    ...
    MessageFormat msgFmt(UNICODE_STRING_SIMPLE("abc_{0,plural,other{#'#'#'{'#''}}_def_{1,select,other{sel'}'ect''}}_xyz"), Locale::getEnglish(), errorCode);
    ...
    internalFormat(&msgFmt, args, 2, expected, "MessageFormat with apostrophes in plural/select arguments failed:\n");
}

void TestMessageFormat::internalFormat(MessageFormat* msgFmt, Formattable* args, int32_t numOfArgs, UnicodeString expected, const char* errMsg)
{
    UnicodeString result;
    FieldPosition ignore(FieldPosition::DONT_CARE);
    UErrorCode status = U_ZERO_ERROR;
...
MessageFormat* TestMessageFormat::internalCreate(UnicodeString pattern, Locale locale, UErrorCode& status, char* errMsg)
{
...
void TestMessageFormat::testMsgFormatSelect(/* char* par */)
{
    ...
    if (!U_FAILURE(err)) {
        ...
        UnicodeString exp[] = { "Kirti est all\\u00E9e \\u00E0 Paris.", "Victor est all\\u00E9 \\u00E0 Paris.", "Ash est all\\u00E9 \\u00E0 Paris." };
        // Format
        for (int i = 0; i < 3; i++) {
    ...
    if (!U_FAILURE(err)) {
        ...
        Formattable* testArgs[] = { testArgs10, testArgs11, testArgs12 };
        UnicodeString exp[]
            = { "Kirti est all\\u00E9e c'est \\u00E0 Paris.", "Victor est all\\u00E9 c'est \\u00E0 Paris.", "Ash est all\\u00E9 c'est \\u00E0 Paris." };
        // Format
        for (int i = 0; i < 3; i++) {
    ...
    if (!U_FAILURE(err)) {
        ...
        Formattable* testArgs[] = { testArgs10, testArgs11, testArgs12 };
        UnicodeString exp[] = { "FEMALE FR company published new books.", "FR otherValue published new books.", "MALE FR company published new books." };
        // Format
        for (int i = 0; i < 3; i++) {
    ...
    if (!U_FAILURE(err)) {
        ...
        Formattable* testArgs[] = { testArgs10, testArgs11, testArgs12 };
        UnicodeString exp[] = { "Kirti est 6 all\\u00E9e \\u00E0 Paris.", "Kirti est 100 all\\u00E9e \\u00E0 Paris.", "Kirti est all\\u00E9 \\u00E0 Paris." };
        // Format
        for (int i = 0; i < 3; i++) {
    ...
    UnicodeString t5("{0} {1, plural, one {est {2, select, female {all\\u00E9e} other {all\\u00E9}}} other {sont {2, select, female {all\\u00E9es} other "
                     "{all\\u00E9s}}}} \\u00E0 Paris.");
    err = U_ZERO_ERROR;
    ...
    if (!U_FAILURE(err)) {
        ...
        UnicodeString exp[] = { "Kirti sont all\\u00E9es \\u00E0 Paris.", "Kirti est all\\u00E9e \\u00E0 Paris.", "Ash est all\\u00E9 \\u00E0 Paris.",
    ...
    UnicodeString t6("{0} und {1, select, female {{2, plural, one {{3, select, female {ihre Freundin} other {ihr Freund}} } other {ihre {2, number, integer} "
                     "{3, select, female "
                     "{Freundinnen} other {Freunde}} } }} other{{2, plural, one {{3, select, female {seine Freundin} other {sein Freund}}} other {seine {2, "
                     "number, integer} {3, "
                     "select, female {Freundinnen} other {Freunde}}}}} } gingen nach Paris.");
    // Create the MessageFormat with Select, plural, and number formats heavily nested
    LocalPointer<MessageFormat> msgFmt6(internalCreate(t6, Locale("de"), err, (char*)"From TestMessageFormat::TestSelectFormat create t6"));
    if (!U_FAILURE(err)) {
        // Arguments
        ...
        Formattable* testArgs[] = { testArgs10, testArgs11, testArgs12, testArgs13, testArgs14, testArgs15, testArgs16, testArgs17, testArgs18, testArgs19,
            testArgs20, testArgs21, testArgs22, testArgs23 };
        UnicodeString exp[] = { "Kirti und sein Freund gingen nach Paris.", "Kirti und seine 6 Freunde gingen nach Paris.",
            "Kirti und seine Freundin gingen nach Paris.", "Kirti und seine 3 Freundinnen gingen nach Paris.", "Kirti und ihre Freundin  gingen nach Paris.",
            "Kirti und ihre 5 Freundinnen  gingen nach Paris.", "Kirti und ihr Freund  gingen nach Paris.", "Kirti und ihre 5 Freunde  gingen nach Paris.",
            "Kirti und sein Freund gingen nach Paris.", "Kirti und sein Freund gingen nach Paris.", "Kirti und ihr Freund  gingen nach Paris.",
            "Kirti und seine 5 Freunde gingen nach Paris.", "Kirti und seine 5 Freunde gingen nach Paris.", "Kirti und ihre 5 Freunde  gingen nach Paris." };
        // Format
        for (int i = 0; i < 14; i++) {
...
void TestMessageFormat::testSetLocale()
{
    ...
    Formattable arguments[] = { 456.83, Formattable(UDate(8.71068e+011), Formattable::kIsDate), "deposit" };
    ...
    result = "";
    FieldPosition pos(FieldPosition::DONT_CARE);
    result = msg.format(arguments, 3, result, pos, err);
...
void TestMessageFormat::testFormat()
{
    ...
    const Formattable ftarray[] = { Formattable(UDate(8.71068e+011), Formattable::kIsDate) };
    ...
    result = "";
    fp = 0;
    result = msg.format(ft_arr, result,
        // FieldPosition(FieldPosition::DONT_CARE),
        fp, err);
...
void TestMessageFormat::testParse()
{
    ...
    if (U_FAILURE(err) || (!fmt_arr)) {
        ...
        if (count != 2) {
            ...
            if ((fmt_arr[0].getType() == Formattable::kString) && (fmt_arr[1].getType() == Formattable::kString) && (fmt_arr[0].getString(tmp1) == "abc")
    ...
    if (pp == 0) {
        ...
        if (count != 2) {
            ...
            if ((fmta[0].getType() == Formattable::kString) && (fmta[1].getType() == Formattable::kString) && (fmta[0].getString(tmp1) == "abc")
...
void TestMessageFormat::TestUnlimitedArgsAndSubformats()
{
    ...
    const UnicodeString pattern = "On {0,date} (aka {0,date,short}, aka {0,date,long}) "
    ...
    UnicodeString expected = "On Nov 20, 2286 (aka 11/20/86, aka November 20, 2286) "
    ...
    if (result == expected) {
        ...
        errln((UnicodeString) "FAIL: Got " + result + ", expected " + expected);
    }
}
...
void TestMessageFormat::TestApostropheMode()
{
    ...
    UnicodeString tuples[] = { // Desired output
        // DOUBLE_OPTIONAL pattern
        // DOUBLE_REQUIRED pattern (empty=same as DOUBLE_OPTIONAL)
        "I see {many}", "I see '{many}'", "", "I said {'Wow!'}", "I said '{''Wow!''}'", "", "I dont know", "I dont know", "I don't know", "I don't know",
    ...
    for (int i = 0; i < tuples_count; i += 3) {
        ...
        UErrorCode status = U_ZERO_ERROR;
        assertEquals("DOUBLE_OPTIONAL failure", desired, GetPatternAndSkipSyntax(ado_mp->parse(ado_pattern, NULL, status)));
        UnicodeString& adr_pattern = tuples[i + 2].isEmpty() ? ado_pattern : tuples[i + 2];
        assertEquals("DOUBLE_REQUIRED failure", desired, GetPatternAndSkipSyntax(adr_mp->parse(adr_pattern, NULL, status)));
    }
    delete adr_mp;
    delete ado_mp;
}
...
void TestMessageFormat::TestCompatibleApostrophe()
{
    ...
    FieldPosition fieldpos(FieldPosition::DONT_CARE);
    UnicodeString buffer1, buffer2;
    assertEquals("incompatible ICU MessageFormat compatibility-apostrophe behavior", "ab12'3'4''.yz", compMsg.format(zero0, 1, buffer1, fieldpos, ec));
    assertEquals("unexpected ICU MessageFormat double-apostrophe-optional behavior", "ab1'2'3''4''.yz", icuMsg.format(zero0, 1, buffer2, fieldpos, ec));
    ...
    if (U_FAILURE(ec)) {
        ...
        assertEquals("incompatible ICU MessageFormat compatibility-apostrophe behavior", "ab1234'.0xyz", compMsg.format(zero0, 1, buffer1, fieldpos, ec));
        assertEquals("unexpected ICU MessageFormat double-apostrophe-optional behavior", "ab1'2'3''4''.#x0yz", icuMsg.format(zero0, 1, buffer2, fieldpos, ec));
    }
...
void TestMessageFormat::TestTrimArgumentName()
{
    ...
    FieldPosition ignore(FieldPosition::DONT_CARE);
    UnicodeString result;
    assertEquals("trim-numbered-arg format() failed", "a  #,#2.0  z", m.format(args, 1, result, ignore, errorCode));

    m.applyPattern("x { _oOo_ , number , integer } y", errorCode);
    UnicodeString argName = UNICODE_STRING_SIMPLE("_oOo_");
    args[0].setLong(3);
    result.remove();
    assertEquals("trim-named-arg format() failed", "x 3 y", m.format(&argName, args, 1, result, errorCode));
}
...
void TestMessageFormat::TestSelectOrdinal()
{
    ...
    MessageFormat m("{0,plural,one{1 file}other{# files}}, "
    ...
    FieldPosition ignore(FieldPosition::DONT_CARE);
    UnicodeString result;
    assertEquals("plural-and-ordinal format(21) failed", "21 files, 21st file", m.format(args, 1, result, ignore, errorCode), TRUE);

    args[0].setLong(2);
    assertEquals("plural-and-ordinal format(2) failed", "2 files, 2nd file", m.format(args, 1, result.remove(), ignore, errorCode), TRUE);

    args[0].setLong(1);
    assertEquals("plural-and-ordinal format(1) failed", "1 file, 1st file", m.format(args, 1, result.remove(), ignore, errorCode), TRUE);

    args[0].setLong(3);
    assertEquals("plural-and-ordinal format(3) failed", "3 files, 3rd file", m.format(args, 1, result.remove(), ignore, errorCode), TRUE);
...
void TestMessageFormat::TestDecimals()
{
    ...
    MessageFormat m("{0,plural,one{one meter}other{# meters}}", Locale::getEnglish(), errorCode);
    Formattable args[1] = { (int32_t)1 };
    FieldPosition ignore;
    UnicodeString result;
    assertEquals("simple format(1)", "one meter", m.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (double)1.5;
    result.remove();
    assertEquals("simple format(1.5)", "1.5 meters", m.format(args, 1, result, ignore, errorCode), TRUE);

    // Simple but explicit.
    MessageFormat m0("{0,plural,one{one meter}other{{0} meters}}", Locale::getEnglish(), errorCode);
    args[0] = (int32_t)1;
    result.remove();
    assertEquals("explicit format(1)", "one meter", m0.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (double)1.5;
    result.remove();
    assertEquals("explicit format(1.5)", "1.5 meters", m0.format(args, 1, result, ignore, errorCode), TRUE);

    // With offset and specific simple format with optional decimals.
    MessageFormat m1("{0,plural,offset:1 one{another meter}other{{0,number,00.#} meters}}", Locale::getEnglish(), errorCode);
    args[0] = (int32_t)1;
    result.remove();
    assertEquals("offset format(1)", "01 meters", m1.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (int32_t)2;
    result.remove();
    assertEquals("offset format(1)", "another meter", m1.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (double)2.5;
    result.remove();
    assertEquals("offset format(1)", "02.5 meters", m1.format(args, 1, result, ignore, errorCode), TRUE);

    // With offset and specific simple format with forced decimals.
    MessageFormat m2("{0,plural,offset:1 one{another meter}other{{0,number,0.0} meters}}", Locale::getEnglish(), errorCode);
    args[0] = (int32_t)1;
    result.remove();
    assertEquals("offset-decimals format(1)", "1.0 meters", m2.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (int32_t)2;
    result.remove();
    assertEquals("offset-decimals format(1)", "2.0 meters", m2.format(args, 1, result, ignore, errorCode), TRUE);

    args[0] = (double)2.5;
    result.remove();
    assertEquals("offset-decimals format(1)", "2.5 meters", m2.format(args, 1, result, ignore, errorCode), TRUE);
    errorCode.reset();
}
...
void TestMessageFormat::TestMessageFormatNumberSkeleton()
{
    ...
    static const struct TestCase {
        ...
        { u"{0,number,::percent}", "en", 50, u"50%" }, { u"{0,number,::percent scale/100}", "en", 0.5, u"50%" },
        { u"{0,number,   ::   percent   scale/100   }", "en", 0.5, u"50%" }, { u"{0,number,::currency/USD}", "en", 23, u"$23.00" },
        { u"{0,number,::precision-integer}", "en", 514.23, u"514" }, { u"{0,number,::.000}", "en", 514.23, u"514.230" },
        { u"{0,number,::.}", "en", 514.23, u"514" }, { u"{0,number,::}", "fr", 514.23, u"514,23" },
        { u"Cost: {0,number,::currency/EUR}.", "en", 4.3, u"Cost: €4.30." }, { u"{0,number,'::'0.00}", "en", 50, u"::50.00" }, // pattern literal
    };
...
void TestMessageFormat::doTheRealDateTimeSkeletonTesting(
    UDate testDate, const char16_t* messagePattern, const char* localeName, const char16_t* expected, IcuTestErrorCode& status)
{
