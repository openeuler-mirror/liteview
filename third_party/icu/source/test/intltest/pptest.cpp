...
UBool ParsePositionTest::failure(UErrorCode status, const char* msg, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
...
void ParsePositionTest::TestFieldPosition_example()
{
    ...
    for (int i = 0; i < dNumSize; i++) {
        ...
        int tempOffset = (tempLen <= (tempLen - pos.getEndIndex())) ? tempLen : (tempLen - pos.getEndIndex());
