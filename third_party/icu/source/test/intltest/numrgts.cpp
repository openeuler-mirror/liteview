...
public:
    ...
    virtual UnicodeString& format(const Formattable& obj, UnicodeString& toAppendTo, FieldPosition& pos, UErrorCode& status) const override
    {
    ...
    */
    ...
    virtual UnicodeString& format(int32_t, UnicodeString& foo, FieldPosition&) const override
    {
        return foo.remove();
    }

    virtual UnicodeString& format(int64_t, UnicodeString& foo, FieldPosition&) const override
    {
        return foo.remove();
    }
...
void NumberFormatRegressionTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
...
UBool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString& msg, const Locale& l, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
        if (possibleDataError) {
            dataerrln(UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), "")
                + UnicodeString(l.getName(), ""));
        } else {
            errcheckln(status,
                UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), "")
                    + UnicodeString(l.getName(), ""));
        }
        return TRUE;
    }
...
UBool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString& msg, const char* l, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
        if (possibleDataError) {
            dataerrln(UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), "") + UnicodeString(l, ""));
        } else {
            errcheckln(status,
                UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), "") + UnicodeString(l, ""));
        }
        return TRUE;
    }
...
UBool NumberFormatRegressionTest::failure(UErrorCode status, const UnicodeString& msg, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
        if (possibleDataError) {
            dataerrln(UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), ""));
        } else {
            errcheckln(status, UnicodeString("FAIL: ", "") + msg + UnicodeString(" failed, error ", "") + UnicodeString(u_errorName(status), ""));
        }
        return TRUE;
    }
...
float NumberFormatRegressionTest::assignFloatValue(float returnfloat)
{
    ...
    float floatResult = (float)(result.getType() == Formattable::kDouble ? result.getDouble() : result.getLong());
...
void NumberFormatRegressionTest::Test4086575(void)
{
    ...
    UChar patChars[] = { 0x23, 0x202f, 0x23, 0x23, 0x23, 0x2c, 0x30, 0x30, 0x3b, 0x28, 0x23, 0x202f, 0x23, 0x23, 0x23, 0x2c, 0x30, 0x30, 0x29 };
    ...
    UChar c[] = { 0x31, 0x202f, 0x32, 0x33, 0x34, 0x2c, 0x30, 0x30 };
    ...
    buffer.remove();
    buffer = nf->format((int32_t)-1234, buffer, pos);
    UChar c1[] = { 0x28, 0x31, 0x202f, 0x32, 0x33, 0x34, 0x2c, 0x30, 0x30, 0x29 };
...
void NumberFormatRegressionTest::Test4087244(void)
{
    ...
    if (str.indexOf(monStr) >= 0 && str.indexOf(decStr) < 0) {
        logln(UnicodeString("OK: 1.23 -> \"") + str + "\" contains \"" + monStr + "\" and not \"" + decStr + '"');
    } else {
        errln(UnicodeString("FAIL: 1.23 -> \"") + str + "\", should contain \"" + monStr + "\" and not \"" + decStr + '"');
    }
}
/* @bug 4070798
...
void NumberFormatRegressionTest::Test4070798(void)
{
    ...
    */
    UChar chars1[] = { 0x2d, 0x35, 0x202f, 0x37, 0x38, 0x39, 0x2c, 0x39, 0x38, 0x38 };
    UChar chars2[] = { 0x35, 0x202f, 0x37, 0x38, 0x39, 0x2c, 0x39, 0x39, 0x00a0, 0x46 };
    UChar chars3[] = { 0x2d, 0x35, 0x37, 0x38, 0x202f, 0x39, 0x39, 0x39, 0x00a0, 0x25 };
    ...
    if (tempString == expectedDefault) {
        ...
        errln(UnicodeString("Failed:") + " Expected " + expectedDefault + " Received " + tempString);
    }
    delete formatter;
    ...
    if (tempString == expectedCurrency) {
        ...
        errln(UnicodeString("Failed:") + " Expected " + expectedCurrency + " Received " + tempString);
    }
    delete formatter;
    ...
    if (tempString == expectedPercent) {
        ...
        errln(UnicodeString("Failed:") + " Expected " + expectedPercent + " Received " + tempString);
    }
...
void NumberFormatRegressionTest::Test4071005(void)
{
    ...
    String expectedPercent = "-578\u00a0998%";
    */
    UChar chars1[] = { 0x2d, 0x35, 0x00a0, 0x37, 0x38, 0x39, 0x2c, 0x39, 0x38, 0x38 };
    UChar chars2[] = { 0x35, 0x00a0, 0x37, 0x38, 0x39, 0x2c, 0x39, 0x39, 0x00a0, 0x24 };
    UChar chars3[] = { 0x2d, 0x35, 0x37, 0x38, 0x00a0, 0x39, 0x39, 0x39, 0x00a0, 0x25 };
    UnicodeString expectedDefault(chars1, 10, 10);
    UnicodeString expectedCurrency(chars2, 10, 10);
    UnicodeString expectedPercent(chars3, 10, 10);
...
void NumberFormatRegressionTest::Test4061302(void)
{
    ...
    if (currency == UnicodeString("") || intlCurrency == UnicodeString("") || monDecSeparator == UnicodeString("")) {
    ...
    logln((UnicodeString) "Before set ==> Currency : " + currency + (UnicodeString) " Intl Currency : " + intlCurrency
        + (UnicodeString) " Monetary Decimal Separator : " + monDecSeparatorStr);
    ...
    if (currency != UnicodeString("XYZ") || intlCurrency != UnicodeString("ABC") || monDecSeparator != UnicodeString((UChar)0x002A /*'*'*/)) {
...
void NumberFormatRegressionTest::Test4106664(void)
{
    ...
    UnicodeString temp;
    FieldPosition pos(FieldPosition::DONT_CARE);
    logln("formatted: " + df->format(n, temp, pos));
...
void NumberFormatRegressionTest::Test4110936(void)
{
    ...
    if (nf->getMaximumIntegerDigits() != MAX_INT_DIGITS)
        errln(UnicodeString("getMaximumIntegerDigits() returns ") + nf->getMaximumIntegerDigits());
...
void NumberFormatRegressionTest::Test4122840(void)
{
    ...
    for (int i = 0; i < count; i++) {
        UErrorCode status = U_ZERO_ERROR;
        ResourceBundle* rb = new ResourceBundle(NULL /*"java.text.resources.LocaleElements"*/, locales[i], status);
        ...
        if (pattern.indexOf(foo) == -1) {
            errln(UnicodeString("Currency format for ") + UnicodeString(locales[i].getName()) + " does not contain generic currency symbol:" + pattern);
        }
        ...
        if (symbols->getSymbol(DecimalFormatSymbols::kCurrencySymbol).indexOf(bar) == -1) {
            ...
            symbols->setSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol, symbols->getSymbol(DecimalFormatSymbols::kMonetarySeparatorSymbol));
            ...
            if (result1 != result2) {
                errln("Results for " + (UnicodeString)(locales[i].getName()) + " differ: " + result1 + " vs " + result2);
            }
...
{
    UnicodeString DATA[] = { // Pattern      Expected string
        UnicodeString("#.00"), UnicodeString("1.20"), UnicodeString("0.00"), UnicodeString("1.20"), UnicodeString("00.00"), UnicodeString("01.20"),
        UnicodeString("#0.0#"), UnicodeString("1.2"), UnicodeString("#0.00"), UnicodeString("1.20")
    };
...
void NumberFormatRegressionTest::Test4145457()
{
    ...
    UnicodeString PATS[] = { UnicodeString("#.00 'num''ber'"), UnicodeString("''#.00''") };
    ...
    for (int32_t i = 0; i < 2; ++i) {
        ...
        if (pat != pat2)
            errln("Fail with \"" + PATS[i] + "\": Patterns should concur, \"" + pat + "\" vs. \"" + pat2 + "\"");
        else
            logln("Ok \"" + PATS[i] + "\" toPattern() -> \"" + pat + '"');

        if (val == val2 && out == out2) {
            logln(UnicodeString("Ok ") + pi + " x \"" + PATS[i] + "\" -> \"" + out + "\" -> " + val + " -> \"" + out2 + "\" -> " + val2);
        } else {
            errln(UnicodeString("Fail ") + pi + " x \"" + PATS[i] + "\" -> \"" + out + "\" -> " + val + " -> \"" + out2 + "\" -> " + val2);
        }
    }
    /*}
...
void NumberFormatRegressionTest::Test4162852(void)
{
    UErrorCode status = U_ZERO_ERROR;
    for (int32_t i = 0; i < 2; ++i) {
        LocalPointer<NumberFormat> f(((i == 0) ? NumberFormat::createInstance(status) : NumberFormat::createPercentInstance(status)), status);
        if (U_FAILURE(status)) {
            dataerrln("Couldn't create number format - %s", u_errorName(status));
            return;
        }
...
void NumberFormatRegressionTest::Test4167494(void)
{
    ...
    if (num.getType() == Formattable::kDouble && _u_abs(num.getDouble() - a) / a < 0.01) { // RT within 1%
        logln(UnicodeString("") + a + " -> \"" + s + "\" -> " + toString(num) + " ok");
    } else {
        errln(UnicodeString("") + a + " -> \"" + s + "\" -> " + toString(num) + " FAIL");
    }
...
void NumberFormatRegressionTest::Test4170798(void)
{
    ...
    LocalPointer<DecimalFormat> df(dynamic_cast<DecimalFormat*>(NumberFormat::createInstance(Locale::getUS(), status)), status);
    ...
    }
    {
        ...
        ParsePosition pos(0);
        df->parse("-0.0", n, pos);
        if (n.getType() != Formattable::kDouble || n.getDouble() != -0.0) {
...
{
    const char* DATA[] = {
        "00", "00", "000", "000", // No grouping
        "#000", "000", // No grouping
        "#,##0", "#,##0", "#,000", "#,000", "0,000", "0,000", "00,000", "00,000", "000,000", "000,000", "0,000,000,000,000.0000",
        "0,000,000,000,000.0000", // Reported
    };
    ...
    for (int i = 0; i < DATA_length; i += 2) {
        ...
        if (!failure(status, "DecimalFormat constructor")) {
            ...
            if (s != exp) {
                errln(UnicodeString("FAIL: ") + DATA[i] + " -> " + s + ", want " + exp);
            }
        }
    }
}
    ...
    if (!failure(status, "Construct DecimalFormat")) {
        for (int i = 0; i < DATA_length; i += 3) {
            ...
            if (out == exp) {
                ...
                errln(UnicodeString("FAIL: ") + in + " x  " + pat + " = " + out + ", expected " + exp);
            }
        }
    }
}
...
void NumberFormatRegressionTest::Test4212072(void)
{
    ...
    if (fmt.format((int32_t)-1, s, pos) != UNICODE_STRING("^1", 2)) {
        errln(UnicodeString("FAIL: -1 x (minus=^) -> ") + s + ", exp ^1");
    }
    s.remove();
    if (fmt.getNegativePrefix(s) != UnicodeString((UChar)0x5e)) {
        errln(UnicodeString("FAIL: (minus=^).getNegativePrefix -> ") + s + ", exp ^");
    }
    ...
    if (fmt.format(0.25, s, pos) != UNICODE_STRING("25^", 3)) {
        errln(UnicodeString("FAIL: 0.25 x (percent=^) -> ") + s + ", exp 25^");
    }
    s.remove();
    if (fmt.getPositiveSuffix(s) != UnicodeString((UChar)0x5e)) {
        errln(UnicodeString("FAIL: (percent=^).getPositiveSuffix -> ") + s + ", exp ^");
    }
    ...
    if (fmt.format(0.25, s, pos) != UNICODE_STRING("250^", 4)) {
        errln(UnicodeString("FAIL: 0.25 x (permill=^) -> ") + s + ", exp 250^");
    }
    s.remove();
    if (fmt.getPositiveSuffix(s) != UnicodeString((UChar)0x5e)) {
        errln(UnicodeString("FAIL: (permill=^).getPositiveSuffix -> ") + s + ", exp ^");
    }
    ...
    if (fmt.format(12.5, s, pos) != UnicodeString(u"usd\u00A012.50")) {
        errln(UnicodeString("FAIL: 12.5 x (currency=usd) -> ") + s + u", exp usd\u00A012.50");
    }
    s.remove();
    if (fmt.getPositivePrefix(s) != UnicodeString("usd")) {
        errln(UnicodeString("FAIL: (currency=usd).getPositivePrefix -> ") + s + ", exp usd");
    }
    ...
    if (fmt.format(12.5, s, pos) != UnicodeString(u"DOL\u00A012.50")) {
        errln(UnicodeString("FAIL: 12.5 x (intlcurrency=DOL) -> ") + s + u", exp DOL\u00A012.50");
    }
    s.remove();
    if (fmt.getPositivePrefix(s) != UnicodeString("DOL")) {
        errln(UnicodeString("FAIL: (intlcurrency=DOL).getPositivePrefix -> ") + s + ", exp DOL");
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            if (failure(status, UnicodeString("Construct DecimalFormat(") + pat + ")")) {
                continue;
            }
            if (*df != f2) {
                UnicodeString l, p;
                errln(UnicodeString("FAIL: ") + type[j] + avail[i].getDisplayName(l) + " -> \"" + pat + "\" -> \"" + f2.toPattern(p) + "\"");
            } else {
                UnicodeString l, p;
                logln(UnicodeString("PASS: ") + type[j] + avail[i].getDisplayName(l) + " -> \"" + pat + "\"");
            }
            ...
            failure(status, UnicodeString("applyLocalizedPattern(") + pat + ")", avail[i]);
            if (U_FAILURE(status)) {
                continue;
            }
            ...
            //            }
            failure(status, UnicodeString("setCurrency() for (") + pat + ")", avail[i]);
            if (U_FAILURE(status)) {
                continue;
            }
            ...
            if (*df != f2) {
                UnicodeString l, p;
                errln(UnicodeString("FAIL: ") + type[j] + avail[i].getDisplayName(l) + " -> localized \"" + pat + "\" -> \"" + f2.toPattern(p) + "\"");
            }
...
void NumberFormatRegressionTest::Test4216742(void)
{
    ...
    for (int i = 0; i < DATA_length; ++i) {
        ...
        for (int m = 1; m <= 100; m++) {
            ...
            if (num.getType() != Formattable::kLong && num.getType() != Formattable::kDouble) {
                errln(UnicodeString("FAIL: Wanted number, got ") + toString(num));
            } else {
                double d = num.getType() == Formattable::kDouble ? num.getDouble() : (double)num.getLong();
                if ((d > 0) != (DATA[i] > 0)) {
                    errln(UnicodeString("\"") + str + "\" parse(x " + fmt->getMultiplier() + ") => " + toString(num));
                }
            }
        }
    }
...
void NumberFormatRegressionTest::Test4161100(void)
{
    ...
    nf->format(a, s);
    UnicodeString pat;
    logln(UnicodeString() + a + " x " + ((DecimalFormat*)nf)->toPattern(pat) + " = " + s);
    ...
    if (!failure(status, "DecimalFormat ct", Locale::getUS())) {
        ...
        for (int32_t i = 0; i < N; ++i) {
            ...
            if (str == exp) {
                ...
                errln(UnicodeString("FAIL ") + NUM[i] + " x 0. = " + str + ", want " + exp);
            }
        }
    }
}
...
void NumberFormatRegressionTest::Test4243108(void)
{
    ...
    if (str == exp) {
        ...
        errln(UnicodeString("FAIL 0.0 x #.# = ") + str + ", want " + exp);
    }
    ...
    if (val.getType() == Formattable::kDouble && val.getDouble() == 99.99) {
        logln(UnicodeString("Ok   99.99 / #.# = ") + toString(val));
    } else {
        errln(UnicodeString("FAIL 99.99 / #.# = ") + toString(val) + ", want " + 99.99);
    }
...
void NumberFormatRegressionTest::Test9780(void)
{
...

    {
        ...
        if (n.getType() != Formattable::kLong || n.getLong() != 1234) {
