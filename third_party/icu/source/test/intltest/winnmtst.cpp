...
static void testLocale(const char* localeID, int32_t lcid, NumberFormat* wnf, UBool currency, TestLog* log)
{
    for (int n = 0; n < LOOP_COUNT; n += 1) {
        ...
        if (udBuffer.compare(wdBuffer) != 0) {
            ...
            log->errln("Double format error for locale " + locale + ": got " + udBuffer + " expected " + wdBuffer);
        }
        ...
        if (u3Buffer.compare(w3Buffer) != 0) {
            ...
            log->errln("int32_t format error for locale " + locale + ": got " + u3Buffer + " expected " + w3Buffer);
        }
        ...
        if (u6Buffer.compare(w6Buffer) != 0) {
            ...
            log->errln("int64_t format error for locale " + locale + ": got " + u6Buffer + " expected " + w6Buffer);
        }
    }
}
