...
void CanonicalIteratorTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    switch (index) {
        CASE(0, TestBasic);
...
void CanonicalIteratorTest::TestExhaustive()
{
    ...
    for (i = 0; i < 0x10FFFF; quick ? i += 0x10 : ++i) {
        ...
        if (type == U_UNASSIGNED || type == U_PRIVATE_USE_CHAR || type == U_SURROGATE)
            continue;
