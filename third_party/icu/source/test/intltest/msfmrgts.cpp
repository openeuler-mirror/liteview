...
void MessageFormatRegressionTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    TESTCASE_AUTO_BEGIN;
...
UBool MessageFormatRegressionTest::failure(UErrorCode status, const char* msg, UBool possibleDataError)
{
    if (U_FAILURE(status)) {
...
{
    UnicodeString pattern[] = { "Message without param", "Message with param:{0}", "Longer Message with param {0}" };
    ...
    Formattable params[] = { Formattable(UnicodeString("BUG")), Formattable(0, Formattable::kIsDate) };
...
void MessageFormatRegressionTest::Test4031438()
{
    ...
    Formattable params[] = { Formattable((int32_t)7) };
    UnicodeString tempBuffer;
    FieldPosition pos(FieldPosition::DONT_CARE);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (obj.getType() == Formattable::kString)
            ...
            case Formattable::kLong:
                fmt->format(obj.getLong(), temp);
                break;
            case Formattable::kInt64:
                fmt->format(obj.getInt64(), temp);
                break;
            case Formattable::kDouble:
                fmt->format(obj.getDouble(), temp);
                break;
            default:
                break;
            }
        ...
        if (obj1.getType() == Formattable::kString)
            ...
            switch (obj1.getType()) {
            case Formattable::kLong:
                fmt->format(obj1.getLong(), temp1);
                break;
            case Formattable::kInt64:
                fmt->format(obj1.getInt64(), temp1);
                break;
            case Formattable::kDouble:
                fmt->format(obj1.getDouble(), temp1);
                break;
            default:
                break;
            }
...
void MessageFormatRegressionTest::Test4052223()
{
    ...
    ChoiceFormat* f = new ChoiceFormat("-1#are negative|0#are no or fraction|1#is one|1.0<is 1+|2#are two|2<are more than 2.", status);
...
{
    double limits[] = { 1, 20 };
    UnicodeString formats[] = { UnicodeString("xyz"), UnicodeString("abc") };
...
{
    double limits[] = { 3, 1, 2 };
    UnicodeString formats[] = { UnicodeString("Three"), UnicodeString("One"), UnicodeString("Two") };
...
{
    UnicodeString patterns[] = { (UnicodeString) "", (UnicodeString) "one", (UnicodeString) "{0,date,short}" };
...
{
    UErrorCode status = U_ZERO_ERROR;
    ChoiceFormat* fmt = new ChoiceFormat("-1#are negative| 0#are no or fraction | 1#is one |1.0<is 1+ |2#are two |2<are more than 2.", status);
    failure(status, "new ChoiceFormat");
    UnicodeString pat;
...
void MessageFormatRegressionTest::Test4105380()
{
    ...
    UnicodeString filepart[] = { (UnicodeString) "no files", (UnicodeString) "one file", (UnicodeString) "{0,number} files" };
    ChoiceFormat* fileform = new ChoiceFormat(filelimits, filepart, 3);
    form1->setFormat(1, *fileform);
    form2->setFormat(0, *fileform);
    // Object[] testArgs = {new Long(12373), "MyDisk"};
    Formattable testArgs[] = { Formattable((int32_t)12373), Formattable((UnicodeString) "MyDisk") };
...
void MessageFormatRegressionTest::Test4120552()
{
    ...
    failure(status, "new MessageFormat");
    UnicodeString texts[] = { (UnicodeString) "pattern", (UnicodeString) "pat", (UnicodeString) "1234" };
    UnicodeString pat;
    logln("pattern: \"" + mf->toPattern(pat) + "\"");
    for (int i = 0; i < 3; i++) {
        ParsePosition pp(0);
...
void MessageFormatRegressionTest::Test4142938()
{
    ...
    UnicodeString PREFIX[] = { CharsToUnicodeString("'Vous' n'avez s\\u00E9lectionn\\u00E9 aucun clients personnels."),
        CharsToUnicodeString("'Vous' avez s\\u00E9lectionn\\u00E9 "), CharsToUnicodeString("'Vous' avez s\\u00E9lectionn\\u00E9 ") };
    UnicodeString SUFFIX[] = { UnicodeString(), UNICODE_STRING(" client personnel.", 18), UNICODE_STRING(" clients personnels.", 20) };

    for (int i = 0; i < 3; i++) {
        UnicodeString out;
        // out = mf->format(new Object[]{new Integer(i)});
        Formattable objs[] = { Formattable((int32_t)i) };
        FieldPosition pos(FieldPosition::DONT_CARE);
        ...
        if (!failure(status, "mf->format", TRUE)) {
            if (SUFFIX[i] == "") {
                ...
                if (!out.startsWith(PREFIX[i]) || !out.endsWith(SUFFIX[i]))
                    errln((UnicodeString) "" + i + ": Got \"" + out + "\"; Want \"" + PREFIX[i] + "\"...\"" + SUFFIX[i] + "\"");
            }
        }
    }
...
void MessageFormatRegressionTest::TestICU12584()
{
    ...
    UnicodeString pattern(u"{1}{2}{3}{1}{2}{3}{1}{2}{3}{1}{2}{3}{1}{2}{3}{1}{2}{3}{1}{2}{3}{1}"
