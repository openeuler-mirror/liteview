...
void NormalizerConformanceTest::compare(const UnicodeString& s1, const UnicodeString& s2)
{
    ...
    if (s1.indexOf((UChar32)0x0345) >= 0)
        return;
...
FileStream* NormalizerConformanceTest::openNormalizationTestFile(const char* filename)
{
    ...
    if (U_SUCCESS(errorCode)) {
        strcpy(unidataPath, folder);
        strcat(unidataPath,
            U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING ".." U_FILE_SEP_STRING "data" U_FILE_SEP_STRING
...
void NormalizerConformanceTest::TestConformance(FileStream* input, int32_t options)
{
    ...
    if (failCount != 0) {
        dataerrln((UnicodeString) "Total: " + failCount + " lines/code points failed, " + passCount + " lines/code points passed");
    } else {
 ...
 */
UBool NormalizerConformanceTest::checkConformance(const UnicodeString* field, const char* line, int32_t options, UErrorCode& status)
{
...
static const char* const kMessages[UNORM_MODE_COUNT] = { "?!=?", "?!=?", "c3!=D(c%d)", "c5!=KC(c%d)", "c2!=C(c%d)", "c4!=KC(c%d)", "FCD" };

UBool NormalizerConformanceTest::checkNorm(
    UNormalizationMode mode, int32_t options, const Normalizer2* norm2, const UnicodeString& s, const UnicodeString& exp, int32_t field)
{
    ...
    if (U_FAILURE(errorCode)) {
        errln("Normalizer2.%s.normalizeUTF8(%s) failed: %s", modeString, s8.c_str(), u_errorName(errorCode));
        return FALSE;
    }
    if (out8 != exp8) {
        errln("Normalizer2.%s.normalizeUTF8(%s)=%s != %s", modeString, s8.c_str(), out8.c_str(), exp8.c_str());
        return FALSE;
    }
    if (editsPtr == nullptr) {
        return TRUE;
    ...
    pass &= assertEquals("edits.lengthDelta()", (int32_t)(out8.length() - s8.length()), edits.lengthDelta());
 ...
 */
void NormalizerConformanceTest::iterativeNorm(const UnicodeString& str, UNormalizationMode mode, int32_t options, UnicodeString& result, int8_t dir)
{
    UErrorCode status = U_ZERO_ERROR;
...
UBool NormalizerConformanceTest::assertEqual(
    const char* op, const char* op2, const UnicodeString& s, const UnicodeString& got, const UnicodeString& exp, const char* msg)
{
    if (exp == got)
        return TRUE;
 ...
 */
UBool NormalizerConformanceTest::hexsplit(const char* s, char delimiter, UnicodeString output[], int32_t outputLength)
{
    const char* t = s;
    char* end = NULL;
    UChar32 c;
    ...
    for (i = 0; i < outputLength; ++i) {
        ...
        for (;;) {
            ...
            if ((char*)t == end || (uint32_t)c > 0x10ffff || (*end != ' ' && *end != '\t' && *end != delimiter)) {
