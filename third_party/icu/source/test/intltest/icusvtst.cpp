...
public:
    ...
    {
        return typeid(*this) == typeid(other) && _val == ((Integer&)other)._val;
    }
...
void ICUServiceTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    switch (index) {
...
void ICUServiceTest::confirmBoolean(const UnicodeString& message, UBool val)
{
    if (val) {
        logln(message);
    } else {
...
{
    UBool equ = (lhs == NULL) ? (rhs == NULL) : (rhs != NULL && lhs->operator==(*rhs));
...
void ICUServiceTest::msgstr(const UnicodeString& message, UObject* obj, UBool err)
{
    if (obj) {
...
void ICUServiceTest::testAPI_Two()
{
    ...
    {
        UnicodeString xids[] = { "en_US_VALLEY_GIRL", "en_US_VALLEY_BOY", "en_US_SURFER_GAL", "en_US_SURFER_DUDE" };
...
public:
    SimpleListener(ICUServiceTest* test, const UnicodeString& name)
        : _test(test)
        , _name(name)
    {
    }
