...
void IntlTestDateTimePatternGeneratorAPI::testAPI(/*char *par*/)
{
    ...
    const char* testGetSkeletonAndBase[][3] = { // pattern       skeleton    baseSkeleton
        { "dd-MMM", "MMMdd", "MMMd" }, { "dd/MMMM/yy", "yyMMMMdd", "yMMMMd" }, { "h", "h", "h" }, { "ah", "ah", "ah" }, { "aaaah", "aaaah", "aaaah" },
        { "Bh", "Bh", "Bh" }
    };
    ...
    format = (SimpleDateFormat*)DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, deLocale);
    format->setTimeZone(*zone);
...

    {
        ...
        if (expR != bPattern) {
            errln(UnicodeString("\nERROR: Test EN time format Got: ") + bPattern + UnicodeString(" Expected: ") + expR);
        }
    ...
    while (localeIndex < MAX_LOCALE) {
        ...
        while (patternData[dataIndex].length() > 0) {
            ...
            if (resultDate != patternResults[localeIndex][resultIndex]) {
                ...
                errln(UnicodeString("\nERROR: Test various skeletons[") + (dataIndex - 1) + UnicodeString("], localeIndex ") + localeIndex + u". Got: \""
                    + resultDate + u"\" with calendar " + calendar->getType() + u" Expected: \"" + patternResults[localeIndex][resultIndex] + u"\"");
            }
    ...
    testDate = LocaleTest::date(99, 9, 13, 23, 58, 59);
    {
        ...
        SimpleDateFormat* enFormat = (SimpleDateFormat*)DateFormat::createDateTimeInstance(DateFormat::kFull, DateFormat::kFull, Locale::getEnglish());
        ...
        while (patternTests2[dataIndex].length() > 0) {
            ...
            if (resultDate != patternResults2[resultIndex]) {
                errln(UnicodeString("\nERROR: Test various skeletons[") + dataIndex + UnicodeString("]. Got: ") + resultDate + UnicodeString(" Expected: ")
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (returnPattern != testSkeletonsResults[i]) {
            errln(UnicodeString("ERROR: Unexpected result from getSkeletons and getPatternForSkeleton\nGot: ") + returnPattern + UnicodeString("\nExpected: ")
                + testSkeletonsResults[i] + UnicodeString("\n"));
        }
    }
...
void IntlTestDateTimePatternGeneratorAPI::testOptions(/*char *par*/)
{
    ...
    for (; count-- > 0; ++testDataPtr) {
        ...
        if (pattern.compare(expectedPattern) != 0) {
            errln(UnicodeString("ERROR in getBestPattern, locale ") + UnicodeString(testDataPtr->locale) + UnicodeString(", skeleton ") + skel
                + ((options) ? UnicodeString(", options!=0") : UnicodeString(", options==0")) + UnicodeString(", expected pattern ") + expectedPattern
...
void IntlTestDateTimePatternGeneratorAPI::testAllFieldPatterns(/*char *par*/)
{
    const char* localeNames[] = {
        "root", "root@calendar=japanese", "root@calendar=chinese", "en", "en@calendar=japanese", "en@calendar=chinese",
        NULL // terminator
    };
        ...
        if (U_SUCCESS(status)) {
            ...
            for (; itemCount-- > 0; ++testDataPtr) {
                ...
                for (lenIndx = 0; lenIndx < FIELD_LENGTHS_COUNT; lenIndx++) {
                    ...
                    if (U_FAILURE(status)) {
                        errln("DateTimePatternGenerator getBestPattern for locale %s, skelChar %c skelLength %d fails: %s", locale.getName(),
                            testDataPtr->patternChar, skelLen, u_errorName(status));
                    } else if (pattern.length() <= 0) {
                        errln("DateTimePatternGenerator getBestPattern for locale %s, skelChar %c skelLength %d produces 0-length pattern", locale.getName(),
                            testDataPtr->patternChar, skelLen);
                    } else {
                        ...
                        if (patIndx >= patLen) {
                            errln(UnicodeString("DateTimePatternGenerator getBestPattern for locale ") + UnicodeString(locale.getName(), -1, US_INV)
                                + ", skeleton " + skeleton + ", produces pattern without required chars: " + pattern);
                        }
                    }
                }
            }
            ...
            dataerrln("Create DateTimePatternGenerator instance for locale(%s) fails: %s", locale.getName(), u_errorName(status));
        }
    }
}
...
void IntlTestDateTimePatternGeneratorAPI::testStaticGetSkeleton(/*char *par*/)
{
    ...
    static const char* const testData[] = { "jmm", "jjmm", "Jmm", "JJmm" };
...
{
    const char* tests[][3] = { // These may change with actual data for Bhmm/bhmm skeletons
        { "zh-TW", "Cm", "Bh:mm" }, { "zh-TW", "CCm", "Bhh:mm" }, { "zh-TW", "CCCm", "BBBBh:mm" }, { "zh-TW", "CCCCm", "BBBBhh:mm" },
        { "zh-TW", "CCCCCm", "BBBBBh:mm" }, { "zh-TW", "CCCCCCm", "BBBBBhh:mm" }, { "de", "Cm", "HH:mm" }, { "de", "CCm", "HH:mm" }, { "de", "CCCm", "HH:mm" },
        { "de", "CCCCm", "HH:mm" }, { "en", "Cm", "h:mm a" }, { "en", "CCm", "hh:mm a" }, { "en", "CCCm", "h:mm aaaa" }, { "en", "CCCCm", "hh:mm aaaa" },
        { "en", "CCCCCm", "h:mm aaaaa" }, { "en", "CCCCCCm", "hh:mm aaaaa" }, { "en-BN", "Cm", "h:mm b" }, { "gu-IN", "Cm", "h:mm B" },
        { "und-IN", "Cm", "h:mm B" }
    };
    ...
    for (int32_t i = 0; i < numTests; ++i) {
        DateTimePatternGenerator* gen = DateTimePatternGenerator::createInstance(Locale::forLanguageTag(tests[i][0], status), status);
        if (gen == NULL) {
        ...
        for (; *charPtr != (UChar)0; charPtr++) {
            if (jPatSkeleton.indexOf(*charPtr) >= 0) {
                if (shortPatSkeleton.indexOf(*charPtr) < 0) {
                    ...
                    errln("ERROR: locale %s (valid %s), expected j resolved char %s to occur in short time pattern '%s' for %s (best pattern: '%s')", localeID,
                        validLoc, jcBuf, spBuf, dfmtCalType, jpBuf);
                }
                break;
            }
        }
...
void IntlTestDateTimePatternGeneratorAPI::test20640_HourCyclArsEnNH()
{
    ...
    for (auto& cas : cases) {
        ...
        assertEquals(UnicodeString("dtpgPattern ") + cas.localeName, cas.expectedDtpgPattern, dtpgPattern);
        assertEquals(UnicodeString("timePattern ") + cas.localeName, cas.expectedTimePattern, timePattern);
        assertEquals(UnicodeString("defaultHour ") + cas.localeName, cas.expectedDefaultHourCycle, defaultHourCycle);
    }
}
...
void IntlTestDateTimePatternGeneratorAPI::test_jConsistencyOddLocales()
{ // ICU-20590
    ...
    for (const char* localeID : localeIDs) {
        ...
        if (dtfShortPattern != dtfSkelPattern || dtfSkelPattern != dtpgPattern) {
            ...
            errln(UnicodeString("For locale ") + localeID + " expected same pattern from DateTimePatGen: " + dtpgPattern + ", DateFmt-forSkel: "
                + dtfSkelPattern + ", DateFmt-short: " + dtfShortPattern + "; latter has validLoc " + dtfShortValidLoc + ", actualLoc " + dtfShortActualLoc);
        }
    }
}
...
void IntlTestDateTimePatternGeneratorAPI::testDateTimePatterns()
{
    ...
    DTPLocaleAndResults localeAndResults[] = {
        { "en",
            { UnicodeString(u"EEEE, MMMM d, y 'at' h:mm a"), // long != medium
                UnicodeString(u"MMMM d, y 'at' h:mm a"), UnicodeString(u"MMM d, y, h:mm a"), UnicodeString(u"M/d/y, h:mm a") } },
        { "fr",
            { UnicodeString(u"EEEE d MMMM y 'à' HH:mm"), // medium != short
                UnicodeString(u"d MMMM y 'à' HH:mm"), UnicodeString(u"d MMM y, HH:mm"), UnicodeString(u"dd/MM/y HH:mm") } },
        { "ha",
            { UnicodeString(u"EEEE d MMMM, y HH:mm"), // full != long
                UnicodeString(u"d MMMM, y 'da' HH:mm"), UnicodeString(u"d MMM, y, HH:mm"), UnicodeString(u"y-MM-dd, HH:mm") } },
        { nullptr,
            { UnicodeString(""), UnicodeString(""), // terminator
                UnicodeString(""), UnicodeString("") } },
    };

    UnicodeString enDTPatterns[kNumDateTimePatterns]
        = { UnicodeString(u"{1} 'at' {0}"), UnicodeString(u"{1} 'at' {0}"), UnicodeString(u"{1}, {0}"), UnicodeString(u"{1}, {0}") };
    UnicodeString modDTPatterns[kNumDateTimePatterns]
        = { UnicodeString(u"{1} _0_ {0}"), UnicodeString(u"{1} _1_ {0}"), UnicodeString(u"{1} _2_ {0}"), UnicodeString(u"{1} _3_ {0}") };
    DTPLocaleAndResults enModResults = { "en",
        { UnicodeString(u"EEEE, MMMM d, y _0_ h:mm a"), UnicodeString(u"MMMM d, y _1_ h:mm a"), UnicodeString(u"MMM d, y _2_ h:mm a"),
    ...
    for (; localeAndResultsPtr->localeID != nullptr; localeAndResultsPtr++) {
        ...
        if (U_FAILURE(status)) {
            dataerrln("FAIL: DateTimePatternGenerator::createInstance for locale %s: %s", localeAndResultsPtr->localeID, u_errorName(status));
        } else {
    ...
    if (U_FAILURE(status)) {
        ...
        if (status != U_ILLEGAL_ARGUMENT_ERROR || dtFormat0Len != 0) {
            errln("ERROR: getDateTimeFormat with invalid style, expected U_ILLEGAL_ARGUMENT_ERROR and lero-length string, "
                  "got %s with length %d",
                u_errorName(status), dtFormat0Len);
        }
        ...
        for (int32_t patStyle = 0; patStyle < kNumDateTimePatterns; patStyle++) {
            ...
            if (U_FAILURE(status)) {
                ...
                errln("ERROR: getDateTimeFormat for en before mod, style %d, expect \"%s\", get \"%s\"", patStyle, bExpect, bGet);
            }
            status = U_ZERO_ERROR;
            ...
            if (U_FAILURE(status)) {
                ...
                if (U_FAILURE(status)) {
                    ...
                    errln("ERROR: getDateTimeFormat for en after mod, style %d, expect \"%s\", get \"%s\"", patStyle, bExpect, bGet);
                }
            }
        }
        ...
        for (int32_t patStyle = 0; patStyle < kNumDateTimePatterns; patStyle++) {
            ...
            if (U_FAILURE(status)) {
                ...
                errln("ERROR: getDateTimeFormat for en after second mod, style %d, expect \"%s\", get \"%s\"", patStyle, bExpect, bGet);
            }
        }
    }
}
...
void IntlTestDateTimePatternGeneratorAPI::doDTPatternTest(DateTimePatternGenerator* dtpg, UnicodeString* skeletons, DTPLocaleAndResults* localeAndResultsPtr)
{
    for (int32_t patStyle = 0; patStyle < kNumDateTimePatterns; patStyle++) {
        ...
        if (U_FAILURE(status)) {
            errln("FAIL: DateTimePatternGenerator::getBestPattern locale %s, style %d: %s", localeAndResultsPtr->localeID, patStyle, u_errorName(status));
            ...
            errln("ERROR: DateTimePatternGenerator::getBestPattern locale %s, style %d, expect \"%s\", get \"%s\"", localeAndResultsPtr->localeID, patStyle,
                bExpect, bGet);
        }
    }
}
