...
#define TEST_ASSERT_SUCCESS(errcode)                                                                                                                           \
    UPRV_BLOCK_MACRO_BEGIN                                                                                                                                     \
    {                                                                                                                                                          \
...
#define MONKEY_ERROR(msg, fRuleFileName, index, seed)                                                                                                          \
    {                                                                                                                                                          \
        IntlTest::gTest->errln(                                                                                                                                \
            "\n%s:%d %s at index %d. Parameters to reproduce: @\"type=%s seed=%u loop=1\"", __FILE__, __LINE__, msg, index, fRuleFileName, seed);              \
    }
...
static void printStringBreaks(UText* tstr, int expected[], int expectedCount)
{
    ...
    for (int j = 0; j < static_cast<int>(utext_nativeLength(tstr)); j = static_cast<int>(utext_getNativeIndex(tstr))) {
        ...
        printf("%7x %5d %6d %8d %4s %4s %4s %4s %s\n", (int)c, u_isUAlphabetic(c), u_hasBinaryProperty(c, UCHAR_GRAPHEME_EXTEND), u_isalnum(c),
            u_getPropertyValueName(UCHAR_GENERAL_CATEGORY, u_charType(c), U_SHORT_PROPERTY_NAME),
            u_getPropertyValueName(UCHAR_WORD_BREAK, u_getIntPropertyValue(c, UCHAR_WORD_BREAK), U_SHORT_PROPERTY_NAME),
            u_getPropertyValueName(UCHAR_SENTENCE_BREAK, u_getIntPropertyValue(c, UCHAR_SENTENCE_BREAK), U_SHORT_PROPERTY_NAME),
            u_getPropertyValueName(UCHAR_LINE_BREAK, u_getIntPropertyValue(c, UCHAR_LINE_BREAK), U_SHORT_PROPERTY_NAME), name);
    }
}
...
void RBBITest::TestBug3818()
{
    ...
    // Four Thai words...
    static const UChar thaiWordData[]
        = { 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0x0E43, 0x0E2B, 0x0E0D, 0x0E48, 0 };
...
void RBBITest::TestGetDisplayName()
{
    ...
    if (Locale::getDefault() == Locale::getUS() && result != "English (United States)")
        dataerrln("BreakIterator::getDisplayName() failed: expected \"English (United States)\", got \"" + result);

    BreakIterator::getDisplayName(Locale::getFrance(), Locale::getUS(), result);
    if (result != "French (France)")
        dataerrln("BreakIterator::getDisplayName() failed: expected \"French (France)\", got \"" + result);
}
/**
 * Test End Behaviour
 * @bug 4068137
...
void RBBITest::TestBug4153072()
{
    ...
    for (index = -1; index < begin + 1; ++index) {
        ...
        if (index == 0 ? !onBoundary : onBoundary) {
            errln((UnicodeString) "Didn't handle isBoundary correctly with offset = " + index + " and begin index = " + begin);
        }
    }
    delete iter;
}
...
static void CharStringAppend(CharString& dest, const UnicodeString& src, UErrorCode& status)
{
    ...
    u_strToUTF8WithSub(buffer, utf8Length, NULL, src.getBuffer(), src.length(), 0xfffd, NULL, &status);
...
void RBBITest::executeTest(TestParams* t, UErrorCode& status)
{
    ...
    for (bp = t->bi->first(); bp != BreakIterator::DONE; bp = t->bi->next()) {
        if (prevBP == bp) {
            ...
            errln("Forward Iteration, no forward progress.  Break Pos=%4d  File line,col=%4d,%4d", bp, t->getSrcLine(bp), t->getSrcCol(bp));
            break;
        }
        ...
        for (i = prevBP + 1; i < bp; i++) {
            if (t->getExpectedBreak(i) != 0) {
                ...
                errln("Forward Iteration, break expected, but not found.  Pos=%4d  File line,col= %4d,%4d", i, t->getSrcLine(i), t->getSrcCol(i));
            }
        }
        ...
        if (t->getExpectedBreak(bp) == 0) {
            ...
            errln("Forward Iteration, break found, but not expected.  Pos=%4d  File line,col= %4d,%4d", bp, t->getSrcLine(bp), t->getSrcCol(bp));
        } else {
    ...
    while (bp != BreakIterator::DONE) {
        if (prevBP == bp) {
            ...
            errln("Reverse Iteration, no progress.  Break Pos=%4d  File line,col=%4d,%4d", bp, t->getSrcLine(bp), t->getSrcCol(bp));
            break;
        }
        ...
        for (i = prevBP - 1; i > bp; i--) {
            if (t->getExpectedBreak(i) != 0) {
                errln("Reverse Iteration, break expected, but not found.  Pos=%4d  File line,col= %4d,%4d", i, t->getSrcLine(i), t->getSrcCol(i));
            }
        }

        // Check that the break we did find was expected
        if (t->getExpectedBreak(bp) == 0) {
            errln("Reverse Itertion, break found, but not expected.  Pos=%4d  File line,col= %4d,%4d", bp, t->getSrcLine(bp), t->getSrcCol(bp));
        } else {
            // The break was expected.
            //   Check that the {nnn} tag value is correct.
            int32_t expectedTagVal = t->getExpectedBreak(bp);
    ...
    for (i = prevBP - 1; i >= 0; i--) {
        if (t->getExpectedBreak(i) != 0) {
            errln("Forward Itertion, break expected, but not found.  Pos=%4d  File line,col= %4d,%4d", i, t->getSrcLine(i), t->getSrcCol(i));
        }
    }
    ...
    for (i = 0; i < utext_nativeLength(t->textToBreak); i++) {
        ...
        if (boundaryExpected != boundaryFound) {
            errln("isBoundary(%d) incorrect. File line,col= %4d,%4d\n"
                  "        Expected, Actual= %s, %s",
                i, t->getSrcLine(i), t->getSrcCol(i), boundaryExpected ? "true" : "false", boundaryFound ? "true" : "false");
        }
    }
...
void RBBITest::TestExtended()
{
    ...
    enum EParseState { PARSE_COMMENT, PARSE_TAG, PARSE_DATA, PARSE_NUM, PARSE_RULES } parseState = PARSE_TAG;
    ...
    for (charIdx = 0; charIdx < len;) {
        ...
        case PARSE_TAG: {
            ...
            if (testString.compare(charIdx - 1, 7, u"<rules>") == 0 || testString.compare(charIdx - 1, 10, u"<badrules>") == 0) {
        ...
        case PARSE_RULES:
            if (testString.compare(charIdx - 1, 8, u"</rules>") == 0) {
                ...
                if (U_FAILURE(status)) {
                    errln("file rbbitst.txt: %d - Error %s creating break iterator from rules.", rulesFirstLine + pe.line - 1, u_errorName(status));
                }
                ...
                if (U_SUCCESS(ec)) {
                    errln("file rbbitst.txt: %d - Expected, but did not get, a failure creating break iterator from rules.", rulesFirstLine + pe.line - 1);
                }
            } else {
                rules.append(c);
            }
        ...
        case PARSE_DATA:
            if (c == u'•') {
                ...
                if (tp.expectedBreaks->size() > breakIdx) {
                    errln("rbbitst.txt:%d:%d adjacent expected breaks with no intervening test text", lineNum, column);
                }
            ...
            if (testString.compare(charIdx - 1, 3, u"\\N{") == 0) {
                ...
                if (theChar == -1) {
                    errln("Error in named character in test file at line %d, col %d", lineNum, column);
                } else {
        ...
        case PARSE_NUM:
            ...
            errln("Syntax Error in test file at line %d, col %d", lineNum, column);
            parseState = PARSE_COMMENT;
            goto end_test; // Stop the test
            break;
        }
        ...
        if (U_FAILURE(status)) {
            dataerrln("ICU Error %s while parsing test file at line %d.", u_errorName(status), lineNum);
            status = U_ZERO_ERROR;
            goto end_test; // Stop the test
        }
    }
    ...
    if (parseState == PARSE_RULES) {
        errln("rbbitst.txt:%d <rules> block beginning at line %d is not closed.", lineNum, rulesFirstLine);
    }
...
UBool RBBITest::testCaseIsKnownIssue(const UnicodeString& testCase, const char* fileName)
{
    ...
    for (int n = 0; n < UPRV_LENGTHOF(badTestCases); n++) {
        ...
        if (!strcmp(fileName, badCase.fFileName) && testCase == UnicodeString(badCase.fString)) {
    ...
    RuleBasedBreakIterator* bi)
{
    ...
    while (pos != BreakIterator::DONE) {
        if (expectedI >= breakPositions->size()) {
            errln("Test file \"%s\", line %d, unexpected break found at position %d", testFileName, lineNumber, pos);
            break;
        }
        ...
        if (pos > expectedPos) {
            errln("Test file \"%s\", line %d, failed to find expected break at position %d", testFileName, lineNumber, expectedPos);
            break;
        }
        pos = bi->next();
        expectedI++;
    ...
    if (pos == BreakIterator::DONE && expectedI < breakPositions->size()) {
        errln("Test file \"%s\", line %d, failed to find expected break at position %d", testFileName, lineNumber, breakPositions->elementAti(expectedI));
    }
}
...
int32_t RBBICharMonkey::next(int32_t prevPos)
{
    ...
    for (;;) {
        ...
        if (fControlSet->contains(c1) || c1 == 0x0D || c1 == 0x0A) {
            setAppliedRule(p2, "GB4   ( Control | CR | LF ) <break>");
            break;
        }

        if (fControlSet->contains(c2) || c2 == 0x0D || c2 == 0x0A) {
            setAppliedRule(p2, "GB5   <break>  ( Control | CR | LF )");
            break;
        }

        if (fLSet->contains(c1) && (fLSet->contains(c2) || fVSet->contains(c2) || fLVSet->contains(c2) || fLVTSet->contains(c2))) {
            setAppliedRule(p2, "GB6   L x ( L | V | LV | LVT )");
            continue;
        }

        if ((fLVSet->contains(c1) || fVSet->contains(c1)) && (fVSet->contains(c2) || fTSet->contains(c2))) {
            setAppliedRule(p2, "GB7    ( LV | V )  x  ( V | T )");
            continue;
        }

        if ((fLVTSet->contains(c1) || fTSet->contains(c1)) && fTSet->contains(c2)) {
        ...
        if (fRegionalIndicatorSet->contains(c0) && fRegionalIndicatorSet->contains(c1) && fRegionalIndicatorSet->contains(c2)) {
...
RBBIWordMonkey::RBBIWordMonkey()
{
    ...
    classNames.push_back("CR");
    fSets->addElement(fLFSet, status);
    classNames.push_back("LF");
    fSets->addElement(fNewlineSet, status);
    classNames.push_back("Newline");
    fSets->addElement(fRegionalIndicatorSet, status);
    classNames.push_back("RegionalIndicator");
    fSets->addElement(fHebrew_LetterSet, status);
    classNames.push_back("Hebrew");
    fSets->addElement(fALetterSet, status);
    classNames.push_back("ALetter");
    fSets->addElement(fSingle_QuoteSet, status);
    classNames.push_back("Single Quote");
    fSets->addElement(fDouble_QuoteSet, status);
    classNames.push_back("Double Quote");
    ...
    classNames.push_back("MidLetter");
    fSets->addElement(fMidNumLetSet, status);
    classNames.push_back("MidNumLet");
    fSets->addElement(fMidNumSet, status);
    classNames.push_back("MidNum");
    fSets->addElement(fNumericSet, status);
    classNames.push_back("Numeric");
    fSets->addElement(fFormatSet, status);
    classNames.push_back("Format");
    fSets->addElement(fExtendSet, status);
    classNames.push_back("Extend");
    fSets->addElement(fOtherSet, status);
    classNames.push_back("Other");
    fSets->addElement(fExtendNumLetSet, status);
    classNames.push_back("ExtendNumLet");
    fSets->addElement(fWSegSpaceSet, status);
    classNames.push_back("WSegSpace");
...
int32_t RBBIWordMonkey::next(int32_t prevPos)
{
    ...
    for (;;) {
        ...
        if ((fALetterSet->contains(c1) || fHebrew_LetterSet->contains(c1)) && (fALetterSet->contains(c2) || fHebrew_LetterSet->contains(c2))) {
        ...
        if ((fALetterSet->contains(c1) || fHebrew_LetterSet->contains(c1))
            ...
            setAppliedRule(p2, "WB6   (ALetter | Hebrew_Letter)  x  (MidLetter | MidNumLet | Single_Quote) (ALetter _Letter)");
            continue;
        }
        ...
        if ((fALetterSet->contains(c0) || fHebrew_LetterSet->contains(c0))
            ...
            setAppliedRule(p2, "WB7   (ALetter | Hebrew_Letter) (MidLetter | MidNumLet | Single_Quote)  x  (ALetter | Hebrew_Letter)");
            continue;
        }
        ...
        if (fNumericSet->contains(c1) && fNumericSet->contains(c2)) {
            setAppliedRule(p2, "WB8   Numeric x Numeric");
            continue;
        }

        if ((fALetterSet->contains(c1) || fHebrew_LetterSet->contains(c1)) && fNumericSet->contains(c2)) {
            setAppliedRule(p2, "WB9   (ALetter | Hebrew_Letter) x Numeric");
            continue;
        }

        if (fNumericSet->contains(c1) && (fALetterSet->contains(c2) || fHebrew_LetterSet->contains(c2))) {
            setAppliedRule(p2, "WB10   Numeric x (ALetter | Hebrew_Letter)");
            continue;
        }

        if (fNumericSet->contains(c0) && (fMidNumSet->contains(c1) || fMidNumLetSet->contains(c1) || fSingle_QuoteSet->contains(c1))
            && fNumericSet->contains(c2)) {
            setAppliedRule(p2, "WB11  Numeric (MidNum | MidNumLet | Single_Quote)  x  Numeric");
            continue;
        }

        if (fNumericSet->contains(c1) && (fMidNumSet->contains(c2) || fMidNumLetSet->contains(c2) || fSingle_QuoteSet->contains(c2))
        ...
        if (fKatakanaSet->contains(c1) && fKatakanaSet->contains(c2)) {
            setAppliedRule(p2, "WB13  Katakana x Katakana");
            continue;
        }

        if ((fALetterSet->contains(c1) || fHebrew_LetterSet->contains(c1) || fNumericSet->contains(c1) || fKatakanaSet->contains(c1)
                || fExtendNumLetSet->contains(c1))
            && fExtendNumLetSet->contains(c2)) {
            setAppliedRule(p2, "WB13a (ALetter | Hebrew_Letter | Numeric | KataKana | ExtendNumLet) x ExtendNumLet");
            continue;
        }

        if (fExtendNumLetSet->contains(c1)
            && (fALetterSet->contains(c2) || fHebrew_LetterSet->contains(c2) || fNumericSet->contains(c2) || fKatakanaSet->contains(c2))) {
...
int32_t RBBISentMonkey::next(int32_t prevPos)
{
    ...
    for (;;) {
        ...
        if ((fUpperSet->contains(c0) || fLowerSet->contains(c0)) && fATermSet->contains(c1) && fUpperSet->contains(c2)) {
        ...
        if (fATermSet->contains(cAt(p8))) {
            ...
            for (;;) {
                c = cAt(p8);
                if (c == -1 || fOLetterSet->contains(c) || fUpperSet->contains(c) || fLowerSet->contains(c) || fSepSet->contains(c) || fATermSet->contains(c)
                    || fSTermSet->contains(c)) {

                    setAppliedRule(p2, "SB8   ATerm Close* Sp*  x  (not (OLettter | Upper | Lower | Sep | STerm | ATerm))* ");
                    break;
                }
                p8 = moveForward(p8);
            }
...

{
    ...
    classNames.push_back("fBK");
    fSets->addElement(fCR, status);
    classNames.push_back("fCR");
    fSets->addElement(fLF, status);
    classNames.push_back("fLF");
    fSets->addElement(fCM, status);
    classNames.push_back("fCM");
    fSets->addElement(fNL, status);
    classNames.push_back("fNL");
    fSets->addElement(fWJ, status);
    classNames.push_back("fWJ");
    fSets->addElement(fZW, status);
    classNames.push_back("fZW");
    fSets->addElement(fGL, status);
    classNames.push_back("fGL");
    fSets->addElement(fCB, status);
    classNames.push_back("fCB");
    fSets->addElement(fSP, status);
    classNames.push_back("fSP");
    fSets->addElement(fB2, status);
    classNames.push_back("fB2");
    fSets->addElement(fBA, status);
    classNames.push_back("fBA");
    fSets->addElement(fBB, status);
    classNames.push_back("fBB");
    fSets->addElement(fHY, status);
    classNames.push_back("fHY");
    fSets->addElement(fH2, status);
    classNames.push_back("fH2");
    fSets->addElement(fH3, status);
    classNames.push_back("fH3");
    fSets->addElement(fCL, status);
    classNames.push_back("fCL");
    fSets->addElement(fCP, status);
    classNames.push_back("fCP");
    fSets->addElement(fEX, status);
    classNames.push_back("fEX");
    fSets->addElement(fIN, status);
    classNames.push_back("fIN");
    fSets->addElement(fJL, status);
    classNames.push_back("fJL");
    fSets->addElement(fJT, status);
    classNames.push_back("fJT");
    fSets->addElement(fJV, status);
    classNames.push_back("fJV");
    fSets->addElement(fNS, status);
    classNames.push_back("fNS");
    fSets->addElement(fOP, status);
    classNames.push_back("fOP");
    fSets->addElement(fQU, status);
    classNames.push_back("fQU");
    fSets->addElement(fIS, status);
    classNames.push_back("fIS");
    fSets->addElement(fNU, status);
    classNames.push_back("fNU");
    fSets->addElement(fPO, status);
    classNames.push_back("fPO");
    fSets->addElement(fPR, status);
    classNames.push_back("fPR");
    fSets->addElement(fSY, status);
    classNames.push_back("fSY");
    fSets->addElement(fAI, status);
    classNames.push_back("fAI");
    fSets->addElement(fAL, status);
    classNames.push_back("fAL");
    fSets->addElement(fHL, status);
    classNames.push_back("fHL");
    fSets->addElement(fID, status);
    classNames.push_back("fID");
    fSets->addElement(fRI, status);
    classNames.push_back("fRI");
    fSets->addElement(fSG, status);
    classNames.push_back("fSG");
    fSets->addElement(fEB, status);
    classNames.push_back("fEB");
    fSets->addElement(fEM, status);
    classNames.push_back("fEM");
    fSets->addElement(fZWJ, status);
    classNames.push_back("fZWJ");
    // TODO: fOP30 & fCP30 overlap with plain fOP. Probably OK, but fOP/CP chars will be over-represented.
    fSets->addElement(fOP30, status);
    classNames.push_back("fOP30");
    fSets->addElement(fCP30, status);
    classNames.push_back("fCP30");
    fSets->addElement(fExtPictUnassigned, status);
    classNames.push_back("fExtPictUnassigned");

    const char* rules = "((\\p{Line_Break=PR}|\\p{Line_Break=PO})(\\p{Line_Break=CM}|\\u200d)*)?"
    ...
    fNumberMatcher = new RegexMatcher(UnicodeString(rules, -1, US_INV), 0, status);
...
void RBBILineMonkey::rule9Adjust(int32_t pos, UChar32* posChar, int32_t* nextPos, UChar32* nextChar)
{
    ...
    if (!(fSP->contains(*posChar) || fBK->contains(*posChar) || *posChar == 0x0d || *posChar == 0x0a || fNL->contains(*posChar) || fZW->contains(*posChar))) {
        for (;;) {
...
int32_t RBBILineMonkey::next(int32_t startPos)
{
    ...
    for (;;) {
        ...
        if (prevChar == 0x0d || prevChar == 0x0a || prevChar == 0x85) {
            setAppliedRule(pos, "LB 5  Break after CR, LF, NL, but not inside CR LF");
            break;
        }
        ...
        if (!(fSP->contains(prevChar) || fBA->contains(prevChar) || fHY->contains(prevChar)) && fGL->contains(thisChar)) {
            setAppliedRule(pos, "LB 12a  [^SP BA HY] x GL");
            continue;
        }

        if (fCL->contains(thisChar) || fCP->contains(thisChar) || fEX->contains(thisChar) || fSY->contains(thisChar)) {
        ...
        //           ^($HY | $HH) $AL;
        if (fAL->contains(thisChar) && (fHY->contains(prevChar) || fHH->contains(prevChar)) && prevPosX2 == -1) {
            setAppliedRule(pos, "LB 20.09");
            continue;
        }

        if (fBA->contains(thisChar) || fHY->contains(thisChar) || fNS->contains(thisChar) || fBB->contains(prevChar)) {
        ...
        //      PR x (ID | EB | EM)
        //     (ID | EB | EM) x PO
        if (fPR->contains(prevChar) && (fID->contains(thisChar) || fEB->contains(thisChar) || fEM->contains(thisChar))) {
            setAppliedRule(pos, "LB 23a");
            continue;
        }
        if ((fID->contains(prevChar) || fEB->contains(prevChar) || fEM->contains(prevChar)) && fPO->contains(thisChar)) {
            setAppliedRule(pos, "LB 23a");
            continue;
        }
        ...
        //         (PR | PO) x (AL | HL)
        //         (AL | HL) x (PR | PO)
        if ((fPR->contains(prevChar) || fPO->contains(prevChar)) && (fAL->contains(thisChar) || fHL->contains(thisChar))) {
            setAppliedRule(pos, "LB 24 no break between prefix and letters or ideographs");
            continue;
        }
        if ((fAL->contains(prevChar) || fHL->contains(prevChar)) && (fPR->contains(thisChar) || fPO->contains(thisChar))) {
            setAppliedRule(pos, "LB 24 no break between prefix and letters or ideographs");
            continue;
        }
        ...
        if (fJL->contains(prevChar) && (fJL->contains(thisChar) || fJV->contains(thisChar) || fH2->contains(thisChar) || fH3->contains(thisChar))) {
            setAppliedRule(pos, "LB 26 Do not break a Korean syllable.");
            continue;
        }

        if ((fJV->contains(prevChar) || fH2->contains(prevChar)) && (fJV->contains(thisChar) || fJT->contains(thisChar))) {
            setAppliedRule(pos, "LB 26 Do not break a Korean syllable.");
            continue;
        }

        if ((fJT->contains(prevChar) || fH3->contains(prevChar)) && fJT->contains(thisChar)) {
            setAppliedRule(pos, "LB 26 Do not break a Korean syllable.");
            continue;
        }

        if ((fJL->contains(prevChar) || fJV->contains(prevChar) || fJT->contains(prevChar) || fH2->contains(prevChar) || fH3->contains(prevChar))
            && fPO->contains(thisChar)) {
            setAppliedRule(pos, "LB 27 Treat a Korean Syllable Block the same as ID.");
            continue;
        }
        if (fPR->contains(prevChar)
            && (fJL->contains(thisChar) || fJV->contains(thisChar) || fJT->contains(thisChar) || fH2->contains(thisChar) || fH3->contains(thisChar))) {
            setAppliedRule(pos, "LB 27 Treat a Korean Syllable Block the same as ID.");
            continue;
        }
...
#if !UCONFIG_NO_REGULAR_EXPRESSIONS
static void testBreakBoundPreceding(RBBITest* test, UnicodeString ustr, BreakIterator* bi, int expected[], int expectedcount)
{
    int count = 0;
    int i = 0;
    int forward[50];
    ...
    for (i = bi->first(); i != BreakIterator::DONE; i = bi->next()) {
        ...
        if (count < expectedcount && expected[count] != i) {
            test->errln("%s:%d break forward test failed: expected %d but got %d", __FILE__, __LINE__, expected[count], i);
            break;
        }
        count++;
    }
    if (count != expectedcount) {
        ...
        test->errln("%s:%d break forward test failed: missed %d match", __FILE__, __LINE__, expectedcount - count);
        return;
    }
    // testing boundaries
    for (i = 1; i < expectedcount; i++) {
        ...
        if (!bi->isBoundary(j)) {
            printStringBreaks(ustr, expected, expectedcount);
            test->errln("%s:%d isBoundary() failed.  Expected boundary at position %d", __FILE__, __LINE__, j);
            return;
        }
        for (j = expected[i - 1] + 1; j < expected[i]; j++) {
            if (bi->isBoundary(j)) {
                printStringBreaks(ustr, expected, expectedcount);
                test->errln("%s:%d isBoundary() failed.  Not expecting boundary at position %d", __FILE__, __LINE__, j);
                return;
            }
        }
    }
    ...
    for (i = bi->last(); i != BreakIterator::DONE; i = bi->previous()) {
        ...
        if (forward[count] != i) {
            printStringBreaks(ustr, expected, expectedcount);
            test->errln("%s:%d happy break test previous() failed: expected %d but got %d", __FILE__, __LINE__, forward[count], i);
            break;
        }
    }
    if (count != 0) {
    ...
    for (i = 0; i < expectedcount - 1; i++) {
        ...
        for (; j <= expected[i + 1]; j++) {
            ...
            if (actualPreceding != expectedPreceding) {
                printStringBreaks(ustr, expected, expectedcount);
                test->errln("%s:%d preceding(%d): expected %d, got %d", __FILE__, __LINE__, j, expectedPreceding, actualPreceding);
                return;
            }
        }
    }
...
void RBBITest::TestWordBoundary(void)
{
    ...
    if (U_FAILURE(status)) {
        errcheckln(status, "%s:%d Creation of break iterator failed %s", __FILE__, __LINE__, u_errorName(status));
        return;
    }
    UChar str[50];
    static const char* strlist[] = {
    ...
    for (loop = 0; loop < UPRV_LENGTHOF(strlist); loop++) {
        ...
        for (int32_t boundary = bi->first(); boundary != BreakIterator::DONE; boundary = bi->next()) {
            ...
            if (count >= UPRV_LENGTHOF(forward)) {
                errln("%s:%d too many breaks found. (loop, count, boundary) = (%d, %d, %d)", __FILE__, __LINE__, loop, count, boundary);
                return;
            }
            ...
            for (int32_t nonBoundary = prev + 1; nonBoundary < boundary; nonBoundary++) {
                if (bi->isBoundary(nonBoundary)) {
                    ...
                    errln("%s:%d isBoundary(nonBoundary) failed. (loop, prev, nonBoundary, boundary) = (%d, %d, %d, %d)", __FILE__, __LINE__, loop, prev,
            ...
            if (!bi->isBoundary(boundary)) {
                printStringBreaks(ustr, forward, count);
                errln("%s:%d happy boundary test failed: expected %d a boundary", __FILE__, __LINE__, boundary);
                return;
            }
            prev = boundary;
        }
...
void RBBITest::TestSentBreaks(void)
{
    ...
    UChar str[200];
    static const char* strlist[] = { "Now\ris\nthe\r\ntime\n\rfor\r\r", "This\n",
        "Hello! how are you? I'am fine. Thankyou. How are you doing? This\n costs $20,00,000.", "\"Sentence ending with a quote.\" Bye.",
        ...
        "Nel mezzo del cammin di nostra vita, mi ritrovai in una selva oscura. ", "Che la dritta via aveo smarrita. He said, that I said, that you said!! ",
...
//       numIterations
//
void RBBITest::RunMonkey(BreakIterator* bi, RBBIMonkeyKind& mk, const char* name, uint32_t seed, int32_t numIterations, UBool useUText)
{
    ...
    while (loopCount < numIterations || numIterations == -1) {
        ...
        for (i = testText.length(); i > 0; i--) {
            ...
            if (breakPos >= i || breakPos > lastBreakPos || (breakPos < 0 && testText.getChar32Start(i) > 0)
        ...
        for (i = 0; i <= testText.length(); i++) {
            ...
            if (errorType != NULL) {
                ...
                MONKEY_ERROR((expectedBreaks[i] ? "Break expected but not found" : "Break found but not expected"), name, i, seed);
                ...
                for (ci = startContext; (ci = testText.moveIndex32(ci, 1));) {
                    ...
                    char buffer[200];
                    auto ret = snprintf(buffer, UPRV_LENGTHOF(buffer), "%4s %3i :  %1s  %1s  %10s  %-*s  %-40s  %-40s", currentLineFlag.c_str(), ci,
                        expectedBreaks[ci] == 0 ? "." : "|", // Reference break
                        currentBreakData[ci] == 0 ? "." : "|", // Actual break
                        hexCodePoint, classNameSize, mk.classNameFromCodepoint(c).c_str(), mk.getAppliedRule(ci).c_str(), cName);
                    (void)ret;
...
void RBBITest::TestBug5532(void)
{
    ...
    const unsigned char utf8Data[] = { 0xE0u, 0xB8u, 0x82u, 0xE0u, 0xB8u, 0xB2u, 0xE0u, 0xB8u, 0xA2u, 0xE0u, 0xB9u, 0x80u, 0xE0u, 0xB8u, 0x84u, 0xE0u, 0xB8u,
        0xA3u, 0xE0u, 0xB8u, 0xB7u, 0xE0u, 0xB9u, 0x88u, 0xE0u, 0xB8u, 0xADu, 0xE0u, 0xB8u, 0x87u, 0xE0u, 0xB9u, 0x80u, 0xE0u, 0xB8u, 0xA5u, 0xE0u, 0xB9u,
        0x88u, 0xE0u, 0xB8u, 0x99u, 0xE0u, 0xB8u, 0x8Bu, 0xE0u, 0xB8u, 0xB5u, 0xE0u, 0xB8u, 0x94u, 0xE0u, 0xB8u, 0xB5u, 0x20u, 0x73u, 0x69u, 0x6Du, 0x20u,
        0x61u, 0x75u, 0x64u, 0x69u, 0x6Fu, 0x2Fu, 0x20u, 0x4Du, 0x4Fu, 0x4Fu, 0x4Eu, 0x20u, 0x65u, 0x63u, 0x6Cu, 0x69u, 0x70u, 0x73u, 0x65u, 0x20u, 0xE0u,
        0xB8u, 0xA3u, 0xE0u, 0xB8u, 0xB2u, 0xE0u, 0xB8u, 0x84u, 0xE0u, 0xB8u, 0xB2u, 0x20u, 0x34u, 0x37u, 0x30u, 0x30u, 0x20u, 0xE0u, 0xB8u, 0xA2u, 0xE0u,
        0xB8u, 0xB9u, 0xE0u, 0xB9u, 0x82u, 0xE0u, 0xB8u, 0xA3u, 0x00 };
...
void RBBITest::TestBug9983(void)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedBreakIterator> brkiter(static_cast<RuleBasedBreakIterator*>(BreakIterator::createWordInstance(Locale::getRoot(), status)));
    TEST_ASSERT_SUCCESS(status);
    LocalPointer<RuleBasedBreakIterator> brkiterPOSIX(
        static_cast<RuleBasedBreakIterator*>(BreakIterator::createWordInstance(Locale::createFromName("en_US_POSIX"), status)));
    TEST_ASSERT_SUCCESS(status);
    if (U_FAILURE(status)) {
        return;
    }
    ...
    while (lineMatcher.find()) {
        ...
        while (hexMatcher.find() && hexMatcher.group(1, status).length() > 0) {
            ...
            if (c <= 0x10ffff) {
                ...
                errln("%s:%d emoji-test.txt:%d Error: Unicode Character %s value out of range.", __FILE__, __LINE__, lineNumber, hex8.data());
                break;
            }
        }
...
void RBBITest::TestTableRedundancies()
{
    ...
    LocalPointer<RuleBasedBreakIterator> bi((RuleBasedBreakIterator*)BreakIterator::createLineInstance(Locale::getEnglish(), status));
    assertSuccess(WHERE, status);
    if (U_FAILURE(status))
        return;
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedBreakIterator> bi((RuleBasedBreakIterator*)BreakIterator::createWordInstance(Locale::getEnglish(), status));
    assertSuccess(WHERE, status);
    if (U_FAILURE(status))
        return;
...
void RBBITest::TestReverse()
{
    ...
    TestReverse(std::unique_ptr<RuleBasedBreakIterator>((RuleBasedBreakIterator*)BreakIterator::createCharacterInstance(Locale::getEnglish(), status)));
    assertSuccess(WHERE, status, true);
    status = U_ZERO_ERROR;
    TestReverse(std::unique_ptr<RuleBasedBreakIterator>((RuleBasedBreakIterator*)BreakIterator::createWordInstance(Locale::getEnglish(), status)));
    assertSuccess(WHERE, status, true);
    status = U_ZERO_ERROR;
    TestReverse(std::unique_ptr<RuleBasedBreakIterator>((RuleBasedBreakIterator*)BreakIterator::createLineInstance(Locale::getEnglish(), status)));
    assertSuccess(WHERE, status, true);
    status = U_ZERO_ERROR;
    TestReverse(std::unique_ptr<RuleBasedBreakIterator>((RuleBasedBreakIterator*)BreakIterator::createSentenceInstance(Locale::getEnglish(), status)));
    assertSuccess(WHERE, status, true);
}
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedBreakIterator> bi((RuleBasedBreakIterator*)BreakIterator::createWordInstance(Locale::getEnglish(), status), status);
...
void RBBITest::TestProperties()
{
    ...
    if (!prependSet.isEmpty()) {
        errln("[:GCB=Prepend:] is not empty any more. "
...
{
    UErrorCode status = U_ZERO_ERROR;
    LocalPointer<RuleBasedBreakIterator> bi((RuleBasedBreakIterator*)BreakIterator::createCharacterInstance(Locale::getEnglish(), status), status);
    if (!assertSuccess(WHERE, status, true)) {
        return;
    }
...
void RBBITest::Test8BitsTrieWith16BitStateTable()
{
    ...
    assertEquals(WHERE, false, RBBI_8BITS_ROWS == (bi->fData->fForwardTable->fFlags & RBBI_8BITS_ROWS));
    bi->setText(text);
...
static void U_CALLCONV traceData(const void*, int32_t fnNumber, int32_t, const char*, va_list args)
{
...
void RBBITest::TestTraceCreateCharacter(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createCharacterInstance("zh-CN", status));
...
void RBBITest::TestTraceCreateTitle(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createTitleInstance("zh-CN", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateSentence(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createSentenceInstance("zh-CN", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLine(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("zh-CN", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineStrict(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("zh-CN-u-lb-strict", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineNormal(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("zh-CN-u-lb-normal", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineLoose(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("zh-CN-u-lb-loose", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineLoosePhrase(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("ja-u-lb-loose-lw-phrase", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineNormalPhrase(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("ja-u-lb-normal-lw-phrase", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLineStrictPhrase(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("ja-u-lb-strict-lw-phrase", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateLinePhrase(void)
{
    ...
    LocalPointer<BreakIterator> brkitr(BreakIterator::createLineInstance("ja-u-lw-phrase", status));
    status.errIfFailureAndReset();
...
void RBBITest::TestTraceCreateBreakEngine(void)
{
    ...
    for (std::vector<int>::size_type i = 0; i < gDataFn.size(); i++) {
        assertEquals("utrace_entry should be called ", UTRACE_UBRK_CREATE_BREAK_ENGINE, gEntryFn[i + 1]);
        assertEquals("utrace_exit should be called ", UTRACE_UBRK_CREATE_BREAK_ENGINE, gExitFn[i + 1]);
        assertEquals("utrace_data should be called ", UTRACE_UBRK_CREATE_BREAK_ENGINE, gDataFn[i]);
    }
    ...
    do {
        ...
        if (line.length() > 0) {
            ...
            if (key == "Model:") {
                ...
                if (U_FAILURE(status)) {
                    dataerrln("%s:%d Error %s Cannot create LSTM data for script %s", __FILE__, __LINE__, u_errorName(status), uscript_getName(script));
                    return;
                }
                ...
                if (value != name) {
                    ...
                    dataerrln("%s:%d Error %s The LSTM data for script %s is %s instead of %s", __FILE__, __LINE__, u_errorName(status),
                        uscript_getName(script), name.toUTF8String<std::string>(utf8Name).c_str(), value.toUTF8String<std::string>(utf8Value).c_str());
                    return;
                }
            } else if (key == "Input:") {
