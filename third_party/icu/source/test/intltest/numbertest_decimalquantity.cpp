...
void DecimalQuantityTest::checkDoubleBehavior(double d, bool explicitRequired)
{
    ...
    assertDoubleEquals(UnicodeString(u"After conversion to exact BCD (double): ") + baseStr + u" vs " + newStr, d, fq.toDouble());
}
...
void DecimalQuantityTest::testConvertToAccurateDouble()
{
    ...
    static double exactDoubles[] = { 51423, 51423e10, -5074790912492772E-327, 83602530019752571E-327, 4.503599627370496E15, 6.789512076111555E15,
        9.007199254740991E15, 9.007199254740992E15 };
    ...
    assertDoubleEquals(u"Inf check failed", INFINITY, DecimalQuantity().setToDouble(INFINITY).toDouble());
    assertDoubleEquals(u"-Inf check failed", -INFINITY, DecimalQuantity().setToDouble(-INFINITY).toDouble());
...
void DecimalQuantityTest::testUseApproximateDoubleWhenAble()
{
    static const struct TestCase {
        ...
        bool usesExact;
    } cases[] = { { 1.2345678, 1, RoundingMode::UNUM_ROUND_HALFEVEN, false }, { 1.2345678, 7, RoundingMode::UNUM_ROUND_HALFEVEN, false },
        { 1.2345678, 12, RoundingMode::UNUM_ROUND_HALFEVEN, false }, { 1.2345678, 13, RoundingMode::UNUM_ROUND_HALFEVEN, true },
        { 1.235, 1, RoundingMode::UNUM_ROUND_HALFEVEN, false }, { 1.235, 2, RoundingMode::UNUM_ROUND_HALFEVEN, true },
        { 1.235, 3, RoundingMode::UNUM_ROUND_HALFEVEN, false }, { 1.000000000000001, 0, RoundingMode::UNUM_ROUND_HALFEVEN, false },
        { 1.000000000000001, 0, RoundingMode::UNUM_ROUND_CEILING, true }, { 1.235, 1, RoundingMode::UNUM_ROUND_CEILING, false },
        { 1.235, 2, RoundingMode::UNUM_ROUND_CEILING, false }, { 1.235, 3, RoundingMode::UNUM_ROUND_CEILING, true } };
...
void DecimalQuantityTest::testHardDoubleConversion()
{
    ...
    } cases[] = { { 512.0000000000017, u"512.0000000000017" }, { 4095.9999999999977, u"4095.9999999999977" }, { 4095.999999999998, u"4095.999999999998" },
        { 4095.9999999999986, u"4095.9999999999986" }, { 4095.999999999999, u"4095.999999999999" }, { 4095.9999999999995, u"4095.9999999999995" },
        { 4096.000000000001, u"4096.000000000001" }, { 4096.000000000002, u"4096.000000000002" }, { 4096.000000000003, u"4096.000000000003" },
        { 4096.000000000004, u"4096.000000000004" }, { 4096.000000000005, u"4096.000000000005" }, { 4096.0000000000055, u"4096.0000000000055" },
        { 4096.000000000006, u"4096.000000000006" }, { 4096.000000000007, u"4096.000000000007" } };
...
void DecimalQuantityTest::testToDouble()
{
    ...
    static const struct TestCase {
        ...
        double expected;
    } cases[] = { { "0", 0.0 }, { "514.23", 514.23 }, { "-3.142E-271", -3.142e-271 } };
...
void DecimalQuantityTest::testNickelRounding()
{
    ...
    for (const auto& cas : cases) {
        UnicodeString message
            = DoubleToUnicodeString(cas.input) + u" @ " + Int64ToUnicodeString(cas.magnitude) + u" / " + Int64ToUnicodeString(cas.roundingMode);
...
void DecimalQuantityTest::testScientificAndCompactSuppressedExponent()
{
    ...
    for (const auto& cas : cases) {
        ...
        LocalizedNumberFormatter formatter = NumberFormatter::forSkeleton(cas.skeleton, status).locale(ulocale);
        ...
        assertEquals(u"formatted number " + cas.skeleton + u" toString: " + cas.input, cas.expectedString, actualString);
        assertEquals(u"formatted number " + cas.skeleton + u" toLong: " + cas.input, cas.expectedLong, actualLong);
        assertDoubleEquals(u"formatted number " + cas.skeleton + u" toDouble: " + cas.input, cas.expectedDouble, actualDouble);
        assertEquals(u"formatted number " + cas.skeleton + u" toPlainString: " + cas.input, cas.expectedPlainString, actualPlainString);
        assertEquals(u"formatted number " + cas.skeleton + u" suppressed scientific exponent: " + cas.input, cas.expectedSuppressedScientificExponent,
            actualSuppressedScientificExponent);
        assertEquals(u"formatted number " + cas.skeleton + u" suppressed compact exponent: " + cas.input, cas.expectedSuppressedCompactExponent,
        ...
        assertDoubleEquals(u"formatted number " + cas.skeleton + u" n operand: " + cas.input, expectedNOperand, actualNOperand);
        assertDoubleEquals(u"formatted number " + cas.skeleton + u" i operand: " + cas.input, expectedIOperand, actualIOperand);
        assertDoubleEquals(u"formatted number " + cas.skeleton + " e operand: " + cas.input, expectedEOperand, actualEOperand);
        assertDoubleEquals(u"formatted number " + cas.skeleton + " c operand: " + cas.input, expectedCOperand, actualCOperand);
    }
}
...
void DecimalQuantityTest::testSuppressedExponentUnchangedByInitialScaling()
{
    ...
    for (const auto& cas : cases) {
        ...
        assertEquals(u"formatted number " + Int64ToUnicodeString(cas.input) + " compactLong toString: ", cas.expectedString, actualString);
        assertDoubleEquals(u"compact decimal " + DoubleToUnicodeString(cas.input) + ", n operand vs. expected", cas.expectedNOperand, compactNOperand);
        assertDoubleEquals(u"compact decimal " + DoubleToUnicodeString(cas.input) + ", i operand vs. expected", cas.expectedIOperand, compactIOperand);
        assertDoubleEquals(u"compact decimal " + DoubleToUnicodeString(cas.input) + ", e operand vs. expected", cas.expectedEOperand, compactEOperand);
        assertDoubleEquals(u"compact decimal " + DoubleToUnicodeString(cas.input) + ", c operand vs. expected", cas.expectedCOperand, compactCOperand);
        ...
        assertDoubleEquals(
            u"decimal " + DoubleToUnicodeString(cas.input) + ", c operand for compact vs. compact scaled", compactCOperand + 3, compactScaledCOperand);
    }
}
...
void DecimalQuantityTest::testDecimalQuantityParseFormatRoundTrip()
{
    ...
    struct TestCase {
        ...
        { u"0" }, { u"1" }, { u"1.0" }, { u"1.00" }, { u"1.1" }, { u"1.10" }, { u"-1.10" }, { u"0.0" }, { u"1c5" }, { u"1.0c5" }, { u"1.1c5" }, { u"1.10c5" },
        { u"0.00" }, { u"0.1" }, { u"1c-1" }, { u"1.0c-1" }
    };
    ...
    assertEquals("Zero ignored for visible exponent", u"1", dq.toExponentString());

    dq.clear();
    dq = DecimalQuantity::fromExponentString(u"1.0c0", status);
    assertEquals("Zero ignored for visible exponent", u"1.0", dq.toExponentString());
}
