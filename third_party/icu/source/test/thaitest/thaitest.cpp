...
private:
    ...
    UBool compareWordBreaks(const UChar* spaces, int32_t spaceCount, const UChar* noSpaces, int32_t noSpaceCount);
 ...
 */
ThaiWordbreakTest::ThaiWordbreakTest(const UChar* spaces, int32_t spaceCount, const UChar* noSpaces, int32_t noSpaceCount, UBool verbose)
    : fBreaksNotFound(0)
    , fInvalidBreaks(0)
    , fWordCount(0)
    , fVerbose(verbose)
 ...
 * without spaces.
 */
UBool ThaiWordbreakTest::compareWordBreaks(const UChar* spaces, int32_t spaceCount, const UChar* noSpaces, int32_t noSpaceCount)
{
    UBool result = TRUE;
    Locale thai("th");
    ...
    while (TRUE) {
        ...
        while (nextSpaceBreak != nextBreak && nextSpaceBreak != BreakIterator::DONE && nextBreak != BreakIterator::DONE) {
...
int generateFile(const UChar* chars, int32_t length)
{
    ...
    while (U_SUCCESS(status)) {
        ...
        printf("%s", u_strToUTF8(outbuf, sizeof(outbuf), &strlength, &chars[prevbreak], nextbreak - prevbreak, &status));
        if (nextbreak > 0 && complexContext.contains(chars[nextbreak - 1]) && complexContext.contains(chars[nextbreak])) {
            printf(" ");
        }
        prevbreak = nextbreak;
    }
...
int32_t SpaceBreakIterator::next()
{
    ...
    } while (nextBreak > 0 && fComplexContext.contains(fText[nextBreak - 1]) && fComplexContext.contains(fText[nextBreak]));
