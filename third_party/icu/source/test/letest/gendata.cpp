        ...
        if (testCase->getTagName().compare(test_case) == 0) {
            ...
            while ((element = testCase->nextChildElement(ec)) != NULL) {
                ...
                if (tag.compare(test_font) == 0) {
                    ...
                    if (version == NULL) {
                        const LEUnicode16* uversion
                            = pfi->getUnicodeNameString(NAME_VERSION_STRING, PLATFORM_MICROSOFT, MICROSOFT_UNICODE_BMP, MICROSOFT_ENGLISH);
                        ...
                        if (uversion != NULL) {
                            ...
                            fprintf(outputFile, "        <test-font name=\"%s\" version=\"%s\" checksum=\"0x%8.8X\" rchecksum=\"0x%8.8X\"/>\n\n", fontName,
                                uversion_utf8, pfi->getFontChecksum(), pfi->getRawChecksum());
                        ...
                        fprintf(outputFile, "        <test-font name=\"%s\" version=\"%s\" checksum=\"0x%8.8X\" rchecksum=\"0x%8.8X\"/>\n\n", fontName, version,
                            pfi->getFontChecksum(), pfi->getRawChecksum());
