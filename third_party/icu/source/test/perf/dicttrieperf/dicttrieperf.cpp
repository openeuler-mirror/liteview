...
public:
    ...
    UBool hasFile() const
    {
        return ucharBuf != NULL;
    }
    const ULine* getCachedLines() const
    {
        return lines;
    }
...
protected:
    PackageLookup(const DictionaryTriePerfTest& perf)
    {
        ...
        if (filenameLength > 0 && filename[filenameLength - 1] != U_FILE_SEP_CHAR && filename[filenameLength - 1] != U_FILE_ALT_SEP_CHAR) {
...
public:
    DictLookup(const DictionaryTriePerfTest& perfTest)
        : perf(perfTest)
    {
    }
...
static int32_t ucharsTrieMatches(UCharsTrie& trie, UText* text, int32_t textLimit, int32_t* lengths, int& count, int limit)
{
...
public:
    ...
    virtual void call(UErrorCode* pErrorCode)
    {
        ...
        for (int32_t i = 0; i < numLines; ++i) {
            ...
            int32_t count = 0;
            ucharsTrieMatches(*trie, &text, lines[i].len, lengths, count, UPRV_LENGTHOF(lengths));
...
static UBool thaiWordToBytes(const UChar* s, int32_t length, CharString& str, UErrorCode& errorCode)
{
...
public:
    ...
    virtual void call(UErrorCode* pErrorCode)
    {
        ...
        for (int32_t i = 0; i < numLines; ++i) {
            ...
            utext_openUChars(&text, lines[i].name, lines[i].len, pErrorCode);
            int32_t count = 0;
            bytesTrieMatches(*trie, &text, lines[i].len, lengths, count, UPRV_LENGTHOF(lengths));
            if (count == 0 || lengths[count - 1] != lines[i].len) {
                fprintf(stderr, "word %ld (0-based) not found\n", (long)i);
            }
        }
...
UPerfFunction* DictionaryTriePerfTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* /*par*/)
{
    if (hasFile()) {
        switch (index) {
        case 0:
