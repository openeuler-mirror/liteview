...
public:
    ICUToUnicodePerfFunction(const char* name, const char* source, int32_t sourceLen, UErrorCode& status)
    {
        ...
        target = NULL;
        targetLimit = NULL;
        int32_t reqdLen = ucnv_toUChars(conv, target, 0, source, srcLen, &status);
...
public:
    ICUFromUnicodePerfFunction(const char* name, const UChar* source, int32_t sourceLen, UErrorCode& status)
    {
        ...
        target = NULL;
        targetLimit = NULL;
        int32_t reqdLen = ucnv_fromUChars(conv, target, 0, source, srcLen, &status);
        if (status == U_BUFFER_OVERFLOW_ERROR) {
            status = U_ZERO_ERROR;
...
public:
    WinANSIToUnicodePerfFunction(const char* cpName, char* pszIn, UINT szLen, UErrorCode& status)
    {
        ...
        CoCreateInstance(__uuidof(CMultiLanguage), NULL, CLSCTX_SERVER, __uuidof(IMultiLanguage2), (void**)&pMulti);
