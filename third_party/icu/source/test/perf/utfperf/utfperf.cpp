...
enum { CHARSET, CHUNK_LENGTH, PIVOT_LENGTH, UTFPERF_OPTIONS_COUNT };

static UOption options[UTFPERF_OPTIONS_COUNT]
    = { UOPTION_DEF("charset", '\x01', UOPT_REQUIRES_ARG), UOPTION_DEF("chunk", '\x01', UOPT_REQUIRES_ARG), UOPTION_DEF("pivot", '\x01', UOPT_REQUIRES_ARG) };

static const char* const utfperf_usage = "\t--charset   Charset for which to test performance, e.g. windows-1251.\n"
                                         "\t            Default: UTF-8\n"
...
static void U_CALLCONV fromUCallback(const void* context, UConverterFromUnicodeArgs* fromUArgs, const UChar* codeUnits, int32_t length, UChar32 codePoint,
...
protected:
    Command(const UtfPerformanceTest& testcase)
        ...
        , inputLength(testcase.getBufferLen())
...
protected:
    Roundtrip(const UtfPerformanceTest& testcase)
        : Command(testcase)
    {
    }
...
protected:
    FromUnicode(const UtfPerformanceTest& testcase)
        : Command(testcase)
    {
    }
...
public:
    ...
    virtual void call(UErrorCode* pErrorCode)
    {
        ...
        for (;;) {
            pInter = intermediate;
            ucnv_convertEx(cnv, utf8Cnv, &pInter, pInterLimit, &pIn, pInLimit, pivot, &pivotSource, &pivotTarget, pivotLimit, FALSE, TRUE, pErrorCode);
            encodedLength += (int32_t)(pInter - intermediate);
...
UPerfFunction* UtfPerformanceTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* par)
{
    ...
    case 0:
        name = "Roundtrip";
        if (exec)
            return Roundtrip::get(*this);
        break;
    case 1:
        name = "FromUnicode";
        if (exec)
            return FromUnicode::get(*this);
        break;
    case 2:
        name = "FromUTF8";
        if (exec)
            return FromUTF8::get(*this);
        break;
    default:
        name = "";
        break;
