...
#define UTRIE_GET8_FROM_LEAD(trie, c16)                                                                                                                        \
    ((const uint8_t*)(trie)->data32)[((int32_t)((trie)->index[(c16) >> UTRIE_SHIFT]) << UTRIE_INDEX_SHIFT) + ((c16)&UTRIE_MASK)]
...
public:
        ...
        , restSet(set.clone())
    {
        ...
        if (U_SUCCESS(errorCode)) {
            ...
            uprv_memcpy((uint16_t*)trie.index + (0xd800 >> UTRIE_SHIFT), trie.index + UTRIE_BMP_INDEX_LENGTH, (0x800 >> UTRIE_SHIFT) * 2);
