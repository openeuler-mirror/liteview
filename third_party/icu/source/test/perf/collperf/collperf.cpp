...
#define COMPATCT_ARRAY(CompactArrays, UNIT)                                                                                                                    \
    struct CompactArrays {                                                                                                                                     \
        ...
        UNIT* dataOf(int32_t i)                                                                                                                                \
        {                                                                                                                                                      \
            return data + index[i];                                                                                                                            \
        }                                                                                                                                                      \
        int32_t lengthOf(int i)                                                                                                                                \
        {                                                                                                                                                      \
...
public:
    ...
    virtual long getOperationsPerIteration()
    {
        return exec_count ? exec_count : 1;
    }
...
struct CmdQsort : public UPerfFunction {
    ...
    static int q_random(const void* a, const void* b)
    {
        ...
        int val_b = 0;
        while (*key_a != 0) {
            val_a += val_a * 37 + *key_a++;
        }
        while (*key_b != 0) {
            val_b += val_b * 37 + *key_b++;
        }
        return val_a - val_b;
    }
...
public:
    ...
    CollPerfTest(int32_t argc, const char* argv[], UErrorCode& status)
        : UPerfTest(argc, argv, status)
    {
        ...
        if (options[s].doesOccur) {
            ...
            switch (tmp) {
            case 1:
                ucol_setAttribute(col, UCOL_STRENGTH, UCOL_PRIMARY, &status);
                break;
            case 2:
                ucol_setAttribute(col, UCOL_STRENGTH, UCOL_SECONDARY, &status);
                break;
            case 3:
...
#define TEST_KEYGEN(testname, func) TEST(testname, CmdKeyGen, col, win_langid, count, rnd_index, &CmdKeyGen::func, 0)
...
#define TEST_ITER(testname, func) TEST(testname, CmdIter, col, count, icu_data, &CmdIter::func, 0, 0)
...
#define TEST_ITER_ALL(testname, func) TEST(testname, CmdIterAll, col, icu_data_all_len, icu_data_all, CmdIterAll::func, 0, 0)
...
#define TEST_QSORT(testname, func) TEST(testname, CmdQsort, rnd_index, count, sizeof(DataIndex), CmdQsort::func, 0, 0)
...
#define TEST_BIN(testname, func) TEST(testname, CmdBinSearch, col, win_langid, count, rnd_index, ord_icu_key, &CmdBinSearch::func)
