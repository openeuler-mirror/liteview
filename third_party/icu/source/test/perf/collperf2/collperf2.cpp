...
#define COMPACT_ARRAY(CompactArrays, UNIT)                                                                                                                     \
    struct CompactArrays {                                                                                                                                     \
        ...
        void append_one(int32_t theLen)                                                                                                                        \
        { /*include terminal NULL*/                                                                                                                            \
            count++;                                                                                                                                           \
        ...
        const UNIT* dataOf(int32_t i) const                                                                                                                    \
        {                                                                                                                                                      \
            return data + index[i];                                                                                                                            \
        }                                                                                                                                                      \
        int32_t lengthOf(int i) const                                                                                                                          \
        {                                                                                                                                                      \
...
struct CollatorAndCounter {
    CollatorAndCounter(const Collator& coll)
        : coll(coll)
        , ucoll(NULL)
        , counter(0)
    {
...
int32_t U_CALLCONV UniStrCollatorComparator(const void* context, const void* left, const void* right)
{
...
void UniStrSort::call(UErrorCode* status)
{
    ...
    uprv_sortArray(dest, count, (int32_t)sizeof(UnicodeString*), UniStrCollatorComparator, &cc, TRUE, status);
    ops = cc.counter;
}

namespace {

int32_t U_CALLCONV StringPieceCollatorComparator(const void* context, const void* left, const void* right)
{
    CollatorAndCounter& cc = *(CollatorAndCounter*)context;
...
int32_t U_CALLCONV StringPieceUCollatorComparator(const void* context, const void* left, const void* right)
{
    CollatorAndCounter& cc = *(CollatorAndCounter*)context;
    const StringPiece& leftString = *(const StringPiece*)left;
    const StringPiece& rightString = *(const StringPiece*)right;
    UErrorCode errorCode = U_ZERO_ERROR;
    ++cc.counter;
    return ucol_strcollUTF8(cc.ucoll, leftString.data(), leftString.length(), rightString.data(), rightString.length(), &errorCode);
}
...
void StringPieceSortCpp::call(UErrorCode* status)
{
    ...
    uprv_sortArray(dest, count, (int32_t)sizeof(StringPiece), StringPieceCollatorComparator, &cc, TRUE, status);
    ops = cc.counter;
}
...
void StringPieceSortC::call(UErrorCode* status)
{
    ...
    int32_t count = d8->count;
    memcpy(dest, source, count * sizeof(StringPiece));
    uprv_sortArray(dest, count, (int32_t)sizeof(StringPiece), StringPieceUCollatorComparator, &cc, TRUE, status);
    ops = cc.counter;
}
...
void UniStrBinSearch::call(UErrorCode* status)
{
    ...
    for (int32_t i = 0; i < count; ++i) {
        (void)uprv_stableBinarySearch((char*)source, count, source + i, (int32_t)sizeof(UnicodeString*), UniStrCollatorComparator, &cc);
    }
    ops = cc.counter;
}
...
void StringPieceBinSearchCpp::call(UErrorCode* status)
{
    ...
    int32_t count = d8->count;
    for (int32_t i = 0; i < count; ++i) {
        (void)uprv_stableBinarySearch((char*)source, count, source + i, (int32_t)sizeof(StringPiece), StringPieceCollatorComparator, &cc);
    }
    ops = cc.counter;
}
...
void StringPieceBinSearchC::call(UErrorCode* status)
{
    ...
    int32_t count = d8->count;
    for (int32_t i = 0; i < count; ++i) {
        (void)uprv_stableBinarySearch((char*)source, count, source + i, (int32_t)sizeof(StringPiece), StringPieceUCollatorComparator, &cc);
    }
    ops = cc.counter;
}
...
private:
    ...
    static CA_uchar* sortData16(const CA_uchar* d16, UComparator* cmp, const void* context, UErrorCode& status);
...
struct ArrayAndColl {
    ArrayAndColl(const CA_uchar* a, const Collator& c)
        : d16(a)
        , coll(c)
    {
    }
...
int32_t U_CALLCONV U16CollatorComparator(const void* context, const void* left, const void* right)
{
    ...
    UErrorCode errorCode = U_ZERO_ERROR;
    return ac.coll.compare(d16->dataOf(leftIndex), d16->lengthOf(leftIndex), d16->dataOf(rightIndex), d16->lengthOf(rightIndex), errorCode);
}

int32_t U_CALLCONV U16HashComparator(const void* context, const void* left, const void* right)
{
...
CA_uchar* CollPerf2Test::sortData16(const CA_uchar* d16, UComparator* cmp, const void* context, UErrorCode& status)
{
    if (U_FAILURE(status))
        return NULL;
...
UPerfFunction* CollPerf2Test::runIndexedTest(int32_t index, UBool exec, const char*& name, char* par /*= NULL*/)
{
    (void)par;
    TESTCASE_AUTO_BEGIN;
