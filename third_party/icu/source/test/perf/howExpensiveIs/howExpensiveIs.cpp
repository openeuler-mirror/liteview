...
protected:
    HowExpensiveTest(const char* name, const char* file, int32_t line)
        : fName(name)
        , fFile(file)
        , fLine(line)
    {
...
public:
    ...
    SieveTest()
        : HowExpensiveTest("SieveTest", __FILE__, __LINE__)
    {
    }
    virtual int32_t run()
    {
...
#define OpenCloseTest(n, svc, suffix, c, a, d)                                                                                                                 \
    class OCName(svc, _, Test_, suffix, n)                                                                                                                     \
        : public HowExpensiveTest {                                                                                                                            \
            public : OCName(svc, _, Test_, suffix, n)() : HowExpensiveTest(OCStr(svc, _, suffix, n), __FILE__, __LINE__) c int32_t run() { int32_t i;          \
    for (i = 0; i < U_LOTS_OF_TIMES; i++) {                                                                                                                    \
        OCRun(svc, _, close)(OCRun(svc, _, suffix) a);                                                                                                         \
    }                                                                                                                                                          \
    return i;                                                                                                                                                  \
    }                                                                                                                                                          \
    void warmup()                                                                                                                                              \
    ...
    virtual ~OCName(svc, _, Test_, suffix, n)() d                                                                                                              \
    }                                                                                                                                                          \
    ;
#define QuickTest(n, c, r, d)                                                                                                                                  \
    class n : public HowExpensiveTest {                                                                                                                        \
    public:                                                                                                                                                    \
        n()                                                                                                                                                    \
            : HowExpensiveTest(#n, __FILE__, __LINE__) c int32_t run() r virtual ~n() d                                                                        \
    };
...
#define DO_NumTest(p, n, x)                                                                                                                                    \
    {                                                                                                                                                          \
        NumTest t(p, n, x, __FILE__, __LINE__);                                                                                                                \
        runTestOn(t);                                                                                                                                          \
    }
...
#define DO_AttrNumTest(p, n, x, a, v)                                                                                                                          \
    {                                                                                                                                                          \
        AttrNumTest t(p, n, x, __FILE__, __LINE__, a, v);                                                                                                      \
        runTestOn(t);                                                                                                                                          \
    }
...
#define DO_NOXNumTest(p, n, x)                                                                                                                                 \
    {                                                                                                                                                          \
        NOXNumTest t(p, n, x, __FILE__, __LINE__);                                                                                                             \
        runTestOn(t);                                                                                                                                          \
    }

#define DO_TripleNumTest(p, n, x)                                                                                                                              \
    DO_AttrNumTest(p, n, x, UNUM_PARSE_ALL_INPUT, UNUM_YES) DO_AttrNumTest(p, n, x, UNUM_PARSE_ALL_INPUT, UNUM_NO)                                             \
...
public:
    ...
    void warmup()
    {
        ...
        if (U_SUCCESS(setupStatus)) {
            ...
            if (!U_SUCCESS(setupStatus) || trial != fLen || trial <= 0 || u_strncmp(fStr, buf, trial)) {
                char strBuf[200];
...
#define DO_NumFmtTest(p, n, x)                                                                                                                                 \
    {                                                                                                                                                          \
        NumFmtTest t(p, n, x, __FILE__, __LINE__);                                                                                                             \
        runTestOn(t);                                                                                                                                          \
    }

class NumFmtInt64Test : public HowExpensiveTest {
public:
    enum EMode { kDefault, kPattern, kApplyPattern, kGroupOff, kApplyGroupOff };
 ...
 */
#define DO_NumFmtInt64Test(p, n, x)                                                                                                                            \
    {                                                                                                                                                          \
        NumFmtInt64Test t(p, n, x, __FILE__, __LINE__, NumFmtInt64Test::EMode::kPattern);                                                                      \
        runTestOn(t);                                                                                                                                          \
    }
/**
 * unum_open(UNUM_DECIMAL), then
 */
#define DO_NumFmtInt64Test_apply(p, n, x)                                                                                                                      \
    {                                                                                                                                                          \
        NumFmtInt64Test t(p, n, x, __FILE__, __LINE__, NumFmtInt64Test::EMode::kApplyPattern);                                                                 \
        runTestOn(t);                                                                                                                                          \
    }

#define DO_NumFmtInt64Test_default(p, n, x)                                                                                                                    \
    {                                                                                                                                                          \
        NumFmtInt64Test t(p, n, x, __FILE__, __LINE__, NumFmtInt64Test::EMode::kDefault);                                                                      \
        runTestOn(t);                                                                                                                                          \
    }
#define DO_NumFmtInt64Test_gr0(p, n, x)                                                                                                                        \
    {                                                                                                                                                          \
        NumFmtInt64Test t(p, n, x, __FILE__, __LINE__, NumFmtInt64Test::EMode::kGroupOff);                                                                     \
        runTestOn(t);                                                                                                                                          \
    }
#define DO_NumFmtInt64Test_applygr0(p, n, x)                                                                                                                   \
    {                                                                                                                                                          \
        NumFmtInt64Test t(p, n, x, __FILE__, __LINE__, NumFmtInt64Test::EMode::kApplyGroupOff);                                                                \
        runTestOn(t);                                                                                                                                          \
    }
...
public:
    ...
    void warmup()
    {
        ...
        if (U_SUCCESS(setupStatus)) {
            ...
            if (!U_SUCCESS(setupStatus) || fString != buf) {
                char strBuf[200];
                u_strToUTF8(strBuf, 200, NULL, buf.getTerminatedBuffer(), buf.length() + 1, &setupStatus);
                printf("%s:%d: warmup() %s got %s (len %d) expected %s (len %d), err %s\n", fFile, fLine, getName(), strBuf, buf.length(), fCStr, fLen,
                    u_errorName(setupStatus));
                setupStatus = U_INTERNAL_PROGRAM_ERROR;
            }
        }
...
#define DO_NumFmtStringPieceTest(p, n, x)                                                                                                                      \
    {                                                                                                                                                          \
        NumFmtStringPieceTest t(p, n, x, __FILE__, __LINE__);                                                                                                  \
        runTestOn(t);                                                                                                                                          \
    }
...
QuickTest(
    ...
    {
        static UChar pattern[] = { 0x23 };
        NumParseTest_fmt = unum_open(UNUM_PATTERN_DECIMAL, pattern, 1, TEST_LOCALE, 0, &setupStatus);
    },
    {
        int32_t i;
        static UChar str[] = { 0x31 };
        double val;
        for (i = 0; i < U_LOTS_OF_TIMES; i++) {
            val = unum_parse(NumParseTest_fmt, str, 1, NULL, &setupStatus);
        }
        return i;
    },
    ...
    QuickTest(
        ...
        },
        {
            ...
            double val;
            for (i = 0; i < U_LOTS_OF_TIMES; i++) {
                val = unum_parse(NumParseTest_fmt, strdot, 1, NULL, &setupStatus);
            }
            return i;
        },
        { unum_close(NumParseTest_fmt); })
        QuickTest(
            ...
            },
            {
                ...
                double val;
                for (i = 0; i < U_LOTS_OF_TIMES; i++) {
                    val = unum_parse(NumParseTest_fmt, strspc, 1, NULL, &setupStatus);
                }
                return i;
            },
            { unum_close(NumParseTest_fmt); })
            QuickTest(
                ...
                },
                {
                    ...
                    double val;
                    for (i = 0; i < U_LOTS_OF_TIMES; i++) {
                        val = unum_parse(NumParseTest_fmt, strgrp, -1, NULL, &setupStatus);
                    }
                    return i;
                },
                { unum_close(NumParseTest_fmt); })
                ...
                QuickTest(
                    ...
                    },
                    {
                        ...
                        double val;
                        for (i = 0; i < U_LOTS_OF_TIMES; i++) {
                            val = unum_parse(NumParseTest_fmt, strbeng, -1, NULL, &setupStatus);
                        }
                        return i;
                    },
                    { unum_close(NumParseTest_fmt); })

                    UDateFormat* DateFormatTest_fmt
    = NULL;
...
QuickTest(
    DateFormatTestBasic, { DateFormatTest_fmt = udat_open(UDAT_DEFAULT, UDAT_DEFAULT, NULL, NULL, -1, NULL, -1, &setupStatus); },
    {
        int i;
        for (i = 0; i < U_LOTS_OF_TIMES; i++) {
