...
const char gUsageString[] = "usage:  collperf options...\n"
...
//
typedef int DWORD;
inline int CompareStringW(DWORD, DWORD, UChar*, int, UChar*, int)
{
    return 0;
}
#include <sys/time.h>
...
inline int LCMapStringW(DWORD, DWORD, UChar*, int, UChar*, int)
{
    return 0;
}
...
OptSpec opts[] = { { "-file", OptSpec::STRING, &opt_fName }, { "-locale", OptSpec::STRING, &opt_locale }, { "-langid", OptSpec::NUM, &opt_langid },
    { "-rules", OptSpec::STRING, &opt_rules }, { "-qsort", OptSpec::FLAG, &opt_qsort }, { "-binsearch", OptSpec::FLAG, &opt_binsearch },
    { "-iter", OptSpec::FLAG, &opt_itertest }, { "-win", OptSpec::FLAG, &opt_win }, { "-unix", OptSpec::FLAG, &opt_unix },
    { "-uselen", OptSpec::FLAG, &opt_uselen }, { "-usekeys", OptSpec::FLAG, &opt_usekeys }, { "-strcmp", OptSpec::FLAG, &opt_strcmp },
    { "-strcmpCPO", OptSpec::FLAG, &opt_strcmpCPO }, { "-norm", OptSpec::FLAG, &opt_norm }, { "-french", OptSpec::FLAG, &opt_french },
    { "-frenchoff", OptSpec::FLAG, &opt_frenchoff }, { "-shifted", OptSpec::FLAG, &opt_shifted }, { "-lower", OptSpec::FLAG, &opt_lower },
    { "-upper", OptSpec::FLAG, &opt_upper }, { "-case", OptSpec::FLAG, &opt_case }, { "-level", OptSpec::NUM, &opt_level },
    { "-keyhist", OptSpec::FLAG, &opt_keyhist }, { "-keygen", OptSpec::FLAG, &opt_keygen }, { "-loop", OptSpec::NUM, &opt_loopCount },
    { "-iloop", OptSpec::NUM, &opt_iLoopCount }, { "-terse", OptSpec::FLAG, &opt_terse }, { "-dump", OptSpec::FLAG, &opt_dump },
    { "-help", OptSpec::FLAG, &opt_help }, { "-?", OptSpec::FLAG, &opt_help }, { 0, OptSpec::FLAG, 0 } };
        ...
        for (loops = 0; loops < adj_loopCount; loops++) {
            for (line = 0; line < gNumFileLines; line++) {
                ...
                for (iLoop = 0; iLoop < opt_iLoopCount; iLoop++) {
                    LCMapStringW(
                        gWinLCID, LCMAP_SORTKEY, gFileLines[line].name, len, (UChar*)gFileLines[line].winSortKey, 5000); // TODO  something with length.
                }
            }
        }
    } else if (opt_icu) {
...
void doKeyHist()
{
    ...
    for (i = 0; i < gNumFileLines; i++) {
        if (gFileLines[i].len > maxLen)
            maxLen = gFileLines[i].len;
    }
    ...
    for (i = 1; i <= maxLen; i++) {
        if (numKeysOfSize[i] > 0) {
            printf("%d, %f, %f\n", i, (float)accumulatedLen[i] / (float)numKeysOfSize[i], (float)accumulatedLen[i] / (float)(numKeysOfSize[i] * i));
        }
    }
...
void doForwardIterTest(UBool haslen)
{
    ...
    int linecount = 0;
    while (linecount < gNumFileLines) {
        strlen += haslen ? gFileLines[linecount].len : u_strlen(gFileLines[linecount].name);
        linecount++;
    }
    ...
    while (strindex < strlen) {
        int len = 0;
        len += haslen ? gFileLines[linecount].len : u_strlen(gFileLines[linecount].name);
        memcpy(str + strindex, gFileLines[linecount].name, sizeof(UChar) * len);
        strindex += len;
        linecount++;
    }
...
private:
    ...
    UCharFile& operator=(const UCharFile& /*other*/)
    {
        return *this;
    }; // No assignment op
...
UCollator* openRulesCollator()
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    UCollator* coll = ucol_openRules(buf, u_strlen(buf), UCOL_OFF, UCOL_DEFAULT_STRENGTH, NULL, &status);
    if (U_FAILURE(status)) {
