...
protected:
    Command(const UTrie2PerfTest& testcase)
        : testcase(testcase)
    {
    }
...
protected:
    CheckFCD(const UTrie2PerfTest& testcase)
        : Command(testcase)
    {
    }
...
public:
    ...
    virtual void call(UErrorCode* pErrorCode)
    {
        ...
        qcResult = unorm_quickCheck(testcase.getBuffer(), testcase.getBufferLen(), UNORM_FCD, &errorCode);
...
protected:
    ToNFC(const UTrie2PerfTest& testcase)
        : Command(testcase)
    {
        UErrorCode errorCode = U_ZERO_ERROR;
        destCapacity = unorm_normalize(testcase.getBuffer(), testcase.getBufferLen(), UNORM_NFC, 0, NULL, 0, &errorCode);
...
public:
    ...
    {
        UErrorCode errorCode = U_ZERO_ERROR;
        int32_t destLength = unorm_normalize(testcase.getBuffer(), testcase.getBufferLen(), UNORM_NFC, 0, dest, destCapacity, &errorCode);
...
protected:
    GetBiDiClass(const UTrie2PerfTest& testcase)
        : Command(testcase)
    {
    }
...
UPerfFunction* UTrie2PerfTest::runIndexedTest(int32_t index, UBool exec, const char*& name, char* par)
{
    ...
    case 0:
        name = "CheckFCD";
        if (exec)
            return CheckFCD::get(*this);
        break;
    case 1:
        name = "ToNFC";
        if (exec)
            return ToNFC::get(*this);
        break;
    case 2:
        name = "GetBiDiClass";
        if (exec)
            return GetBiDiClass::get(*this);
        break;
#if 0 // See comment at unorm_initUTrie2() forward declaration.
