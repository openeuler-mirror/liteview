...
public:
    ...
    static UnicodeString getLatin1(int32_t minLength)
    {
        return extend(latin1, minLength);
    }
    static UnicodeString getLowercaseLatin1(int32_t minLength)
    {
        return extend(lowercaseLatin1, minLength);
    }
    static UnicodeString getASCII(int32_t minLength)
    {
        return extend(ascii, minLength);
    }
    static UnicodeString getJapanese(int32_t minLength)
    {
        return extend(japanese, minLength);
    }
...
{
    return kLengths[UPRV_LENGTHOF(kLengths) - 1];
}
...
public:
    NormalizeUTF8(const Normalizer2& n2, const UnicodeString& text)
        : norm2(n2)
        , sink(&dest)
    {
...
extern int main(int /*argc*/, const char* /*argv*/[])
{
    ...
    if (U_FAILURE(errorCode)) {
        fprintf(stderr, "simplenormperf: failed to get Normalizer2 instances - %s\n", u_errorName(errorCode));
    }
    {
