...
public:
     ...
     */
    V8_INLINE T FromMaybe(const T& default_value) const { return has_value_ ? value_ : default_value; }
...
template <class T, std::enable_if_t<!std::is_lvalue_reference_v<T>>* = nullptr> inline Maybe<T> Just(T&& t) { return Maybe<T>(std::move(t)); }
