...
public:
    ...
    constexpr Identifier GetId() const { return static_cast<uint8_t>(type_) << 16 | static_cast<uint8_t>(sequence_type_) << 8 | static_cast<uint8_t>(flags_); }
    ...
    static constexpr bool IsFloatingPointType(Type type) { return type == Type::kFloat32 || type == Type::kFloat64; }

    static constexpr bool IsPrimitive(Type type) { return IsIntegralType(type) || IsFloatingPointType(type) || type == Type::kBool; }
...
template <typename T> struct FastApiTypedArray : public FastApiTypedArrayBase {
public:
...
public:
    ...
    // if present.
    unsigned int ArgumentCount() const { return HasOptions() ? arg_count_ - 1 : arg_count_; }
...
public:
    ...
    const CTypeInfo& ArgumentInfo(unsigned int index) const { return type_info_->ArgumentInfo(index); }
    ...
    OverloadResolution GetOverloadResolution(const CFunction* other)
    {
        ...
        for (unsigned int i = 0; i < ArgumentCount(); ++i) {
            if (ArgumentInfo(i).GetSequenceType() != other->ArgumentInfo(i).GetSequenceType()) {
                ...
                if (ArgumentInfo(i).GetSequenceType() == CTypeInfo::SequenceType::kScalar || other->ArgumentInfo(i).GetSequenceType() == CTypeInfo::SequenceType::kScalar) {
                    return OverloadResolution::kImpossible;
                }
            }
        }
    ...
    template <typename F> static CFunction Make(F* func) { return ArgUnwrap<F*>::Make(func); }

    // Provided for testing purposes.
    template <typename R, typename... Args, typename R_Patch, typename... Args_Patch> static CFunction Make(R (*func)(Args...), R_Patch (*patching_func)(Args_Patch...))
    {
...
struct FastApiCallbackOptions {
     ...
     */
    static FastApiCallbackOptions CreateForTesting(Isolate* isolate) { return { false, { 0 }, nullptr }; }
...
template <typename RetBuilder, typename... ArgBuilders> class CFunctionInfoImpl : public CFunctionInfo {
...
public:
    constexpr CFunctionInfoImpl()
        : CFunctionInfo(RetBuilder::Build(), sizeof...(ArgBuilders), arg_info_storage_)
        , arg_info_storage_ { ArgBuilders::Build()... }
    {
        ...
        static_assert(kReturnType == CTypeInfo::Type::kVoid || kReturnType == CTypeInfo::Type::kBool || kReturnType == CTypeInfo::Type::kInt32
                || kReturnType == CTypeInfo::Type::kUint32 || kReturnType == CTypeInfo::Type::kFloat32 || kReturnType == CTypeInfo::Type::kFloat64
...
#define SPECIALIZE_GET_TYPE_INFO_HELPER_FOR(T, Enum)                                                                                                                               \
    template <> struct TypeInfoHelper<T> {                                                                                                                                         \
        ...
        static constexpr CTypeInfo::SequenceType SequenceType() { return CTypeInfo::SequenceType::kScalar; }                                                                       \
    };
...
#define SPECIALIZE_GET_TYPE_INFO_HELPER_FOR_TA(T, Enum)                                                                                                                            \
    template <> struct TypeInfoHelper<const FastApiTypedArray<T>&> {                                                                                                               \
        static constexpr CTypeInfo::Flags Flags() { return CTypeInfo::Flags::kNone; }                                                                                              \
                                                                                                                                                                                   \
        static constexpr CTypeInfo::Type Type() { return CTypeInfo::Type::Enum; }                                                                                                  \
        static constexpr CTypeInfo::SequenceType SequenceType() { return CTypeInfo::SequenceType::kIsTypedArray; }                                                                 \
    };
...
template <> struct TypeInfoHelper<v8::Local<v8::Array>> {
    ...
    static constexpr CTypeInfo::SequenceType SequenceType() { return CTypeInfo::SequenceType::kIsSequence; }
};

template <> struct TypeInfoHelper<v8::Local<v8::Uint32Array>> {
    static constexpr CTypeInfo::Flags Flags() { return CTypeInfo::Flags::kNone; }
...
template <> struct TypeInfoHelper<FastApiCallbackOptions&> {
    ...
    static constexpr CTypeInfo::Type Type() { return CTypeInfo::kCallbackOptionsType; }
    static constexpr CTypeInfo::SequenceType SequenceType() { return CTypeInfo::SequenceType::kScalar; }
};
...
public:
    ...
    static constexpr CTypeInfo Build()
    {
        ...
        STATIC_ASSERT_IMPLIES(
            uint8_t(kFlags) & uint8_t(CTypeInfo::Flags::kEnforceRangeBit), CTypeInfo::IsIntegralType(kType), "kEnforceRangeBit is only allowed for integral types.");
        STATIC_ASSERT_IMPLIES(uint8_t(kFlags) & uint8_t(CTypeInfo::Flags::kClampBit), CTypeInfo::IsIntegralType(kType), "kClampBit is only allowed for integral types.");
        STATIC_ASSERT_IMPLIES(
            uint8_t(kFlags) & uint8_t(CTypeInfo::Flags::kIsRestrictedBit), CTypeInfo::IsFloatingPointType(kType), "kIsRestrictedBit is only allowed for floating point types.");
        STATIC_ASSERT_IMPLIES(kSequenceType == CTypeInfo::SequenceType::kIsSequence, kType == CTypeInfo::Type::kVoid, "Sequences are only supported from void type.");
...
// static
template <typename R, typename... Args> CFunction CFunction::ArgUnwrap<R (*)(Args...)>::Make(R (*func)(Args...)) { return internal::CFunctionBuilder().Fn(func).Build(); }
 ...
 */
template <CTypeInfo::Identifier type_info_id, typename T> bool V8_EXPORT V8_WARN_UNUSED_RESULT TryToCopyAndConvertArrayToCppBuffer(Local<Array> src, T* dst, uint32_t max_length);
