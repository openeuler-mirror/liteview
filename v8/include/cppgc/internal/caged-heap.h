...
public:
    V8_INLINE static uintptr_t OffsetFromAddress(const void* address) { return reinterpret_cast<uintptr_t>(address) & (api_constants::kCagedHeapReservationAlignment - 1); }
    ...
    V8_INLINE static bool AreWithinCage(const void* addr1, const void* addr2)
    {
        ...
        static_assert((static_cast<size_t>(1) << kHalfWordShift) == api_constants::kCagedHeapReservationSize);
        CPPGC_DCHECK(g_heap_base_);
        return !(((reinterpret_cast<uintptr_t>(addr1) ^ g_heap_base_) | (reinterpret_cast<uintptr_t>(addr2) ^ g_heap_base_)) >> kHalfWordShift);
    }
