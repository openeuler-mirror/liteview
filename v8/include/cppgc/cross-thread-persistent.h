...
public:
    ...
    V8_CLANG_NO_SANITIZE("address") const void* GetValueFromGC() const { return raw_; }
    ...
    PersistentNode* GetNodeSafe() const { return reinterpret_cast<std::atomic<PersistentNode*>*>(&node_)->load(std::memory_order_acquire); }
...
public:
    ...
    BasicCrossThreadPersistent(
        const BasicCrossThreadPersistent<U, OtherWeaknessPolicy, OtherLocationPolicy, OtherCheckingPolicy>& other, const SourceLocation& loc = SourceLocation::Current())
        : BasicCrossThreadPersistent(loc)
    {
        *this = other;
    }
    ...
    V8_CLANG_NO_SANITIZE("cfi-unrelated-cast") T* Get() const { return static_cast<T*>(const_cast<void*>(GetValue())); }
...
private:
    static bool IsValid(const void* ptr) { return ptr && ptr != kSentinelPointer; }

    static void TraceAsRoot(RootVisitor& root_visitor, const void* ptr) { root_visitor.Trace(*static_cast<const BasicCrossThreadPersistent*>(ptr)); }
    ...
    T* GetFromGC() const { return static_cast<T*>(const_cast<void*>(GetValueFromGC())); }
