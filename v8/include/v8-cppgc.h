...
struct V8_EXPORT CppHeapCreateParams {
    ~CppHeapCreateParams()
    {
        for (size_t i = 0; i < custom_spaces.size(); ++i) {
            delete custom_spaces[i];
        }
    }

    // std::vector<std::unique_ptr<cppgc::CustomSpaceBase>> custom_spaces; // weolar
    std::vector<cppgc::CustomSpaceBase*> custom_spaces;
...
template <typename T> struct TraceTrait<v8::TracedReference<T>> {
    static cppgc::TraceDescriptor GetTraceDescriptor(const void* self) { return { nullptr, Trace }; }

    static void Trace(Visitor* visitor, const void* self) { static_cast<v8::JSVisitor*>(visitor)->Trace(*static_cast<const v8::TracedReference<T>*>(self)); }
};
