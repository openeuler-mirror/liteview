...
template <class F> class Eternal;
...
template <class F1, class F2> class PersistentValueVector;
class Primitive;
class Private;
...
public:
    ...
    V8_INLINE Isolate* GetIsolate() const { return reinterpret_cast<Isolate*>(i_isolate_); }
...
public:
     ...
     * of the target type.
     */
    template <class S> V8_INLINE Local<S> As() const { return Local<S>::Cast(*this); }
     ...
     */
    V8_INLINE static Local<T> New(Isolate* isolate, Local<T> that) { return New(isolate, that.val_); }

    V8_INLINE static Local<T> New(Isolate* isolate, const PersistentBase<T>& that) { return New(isolate, that.val_); }

    V8_INLINE static Local<T> New(Isolate* isolate, const BasicTracedReference<T>& that) { return New(isolate, *that); }
...
private:
    ...
    template <class F> friend class BasicTracedReference;
...
public:
     ...
     * MaybeLocal<> is empty.
     */
    template <class S> V8_INLINE Local<S> FromMaybe(Local<S> default_value) const { return IsEmpty() ? default_value : Local<S>(val_); }
