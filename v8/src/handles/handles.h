...
public:
    ...
    V8_INLINE Address address() const { return base::bit_cast<Address>(location_); }
...
public:
        ...
        : HandleBase(location)
    {
        ...
        static_assert(std::is_convertible<T*, Object*>::value, "static type violation");
    ...
    template <typename S> inline static const Handle<T> cast(Handle<S> that);
    ...
    struct equal_to {
        V8_INLINE bool operator()(Handle<T> lhs, Handle<T> rhs) const { return lhs.equals(rhs); }
    };

    // Provide function object for location hashing.
    struct hash {
        V8_INLINE size_t operator()(Handle<T> const& handle) const { return base::hash<Address>()(handle.address()); }
    };
...
public:
    ...
    // Allow placement new.
    void* operator new(size_t size, void* storage) { return ::operator new(size, storage); }
    ...
    template <typename T> Handle<T> CloseAndEscape(Handle<T> handle_value);
...
template <class CompilationInfoT> class V8_EXPORT_PRIVATE V8_NODISCARD CanonicalHandleScopeForOptimization final : public CanonicalHandleScope {
public:
    // We created the
    ...
    explicit CanonicalHandleScopeForOptimization(Isolate* isolate, CompilationInfoT* info);
...
using CanonicalHandleScopeForTurbofan = CanonicalHandleScopeForOptimization<OptimizedCompilationInfo>;
using CanonicalHandleScopeForMaglev = CanonicalHandleScopeForOptimization<maglev::ExportedMaglevCompilationInfo>;
...
template <typename T> class DirectHandle final {
public:
    V8_INLINE explicit DirectHandle()
        : obj_(kTaggedNullAddress)
    {
    ...
    template <typename S> inline static const DirectHandle<T> cast(DirectHandle<S> that);
