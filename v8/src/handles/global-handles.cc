...
public:
    ...
    NodeBlock(GlobalHandles* global_handles, GlobalHandles::NodeSpace<NodeType>* space, NodeBlock* next) V8_NOEXCEPT : next_(next), global_handles_(global_handles), space_(space)
    {
    }
...
public:
    ...
    bool operator==(const NodeIterator& other) const { return block_ == other.block_; }
    bool operator!=(const NodeIterator& other) const { return block_ != other.block_; }
...
template <class NodeType> typename GlobalHandles::NodeSpace<NodeType>::NodeBounds GlobalHandles::NodeSpace<NodeType>::GetNodeBlockBounds() const
{
    NodeBounds block_bounds;
    for (BlockType* current = first_used_block_; current; current = current->next_used()) {
...
public:
    static const Child* FromLocation(const Address* location) { return reinterpret_cast<const Child*>(location); }

    static Child* FromLocation(Address* location) { return reinterpret_cast<Child*>(location); }
    ...
    bool has_wrapper_class_id() const { return class_id_ != v8::HeapProfiler::kPersistentHandleNoClassId; }
...
public:
    ...
    const char* label() const { return state() == NORMAL ? reinterpret_cast<char*>(data_.parameter) : nullptr; }
    ...
    WeaknessType weakness_type() const { return NodeWeaknessType::decode(flags_); }
    void set_weakness_type(WeaknessType weakness_type) { flags_ = NodeWeaknessType::update(flags_, weakness_type); }

    bool IsWeak() const { return state() == WEAK; }

    bool IsInUse() const { return state() != FREE; }

    bool IsPhantomResetHandle() const { return weakness_type() == WeaknessType::kNoCallback; }

    bool IsWeakOrStrongRetainer() const { return state() == NORMAL || state() == WEAK; }
    ...
    void CollectPhantomCallbackData(std::vector<std::pair<Node*, PendingPhantomCallback>>* pending_phantom_callbacks)
    {
        ...
        if (weakness_type() == WeaknessType::kCallbackWithTwoEmbedderFields && object().IsJSObject()) {
            ExtractInternalFields(JSObject::cast(object()), embedder_fields, v8::kEmbedderFieldsInWeakCallback);
        }
    ...
    GlobalHandles* global_handles() { return NodeBlock<Node>::From(this)->global_handles(); }
...
class GlobalHandles::TracedNode final : public NodeBase<GlobalHandles::TracedNode> {
public:
    TracedNode()
    {
    ...
    template <AccessMode access_mode = AccessMode::NON_ATOMIC> void set_markbit()
    {
        if constexpr (access_mode == AccessMode::NON_ATOMIC) {
            flags_ = Markbit::update(flags_, true);
            return;
        }
        std::atomic<uint8_t>& atomic_flags = reinterpret_cast<std::atomic<uint8_t>&>(flags_);
        const uint8_t new_value = Markbit::update(atomic_flags.load(std::memory_order_relaxed), true);
        atomic_flags.fetch_or(new_value, std::memory_order_relaxed);
    }

    template <AccessMode access_mode = AccessMode::NON_ATOMIC> bool markbit() const
    {
        if constexpr (access_mode == AccessMode::NON_ATOMIC) {
            return Markbit::decode(flags_);
        }
        const auto flags = reinterpret_cast<const std::atomic<uint8_t>&>(flags_).load(std::memory_order_relaxed);
        return Markbit::decode(flags);
    }

    void clear_markbit() { flags_ = Markbit::update(flags_, false); }

    void clear_object() { reinterpret_cast<std::atomic<Address>*>(&object_)->store(kNullAddress, std::memory_order_relaxed); }

    void CopyObjectReference(const TracedNode& other) { reinterpret_cast<std::atomic<Address>*>(&object_)->store(other.object_, std::memory_order_relaxed); }
...
void GlobalHandles::TracedNode::Verify(const Address* const* slot)
{
    ...
    if (incremental_marking && incremental_marking->IsMarking()) {
        ...
        if (object.IsHeapObject()) {
            DCHECK_IMPLIES(node->markbit<AccessMode::ATOMIC>(), !heap->marking_state()->IsWhite(HeapObject::cast(object)));
        }
    }
    DCHECK_IMPLIES(ObjectInYoungGeneration(node->object()), node->is_in_young_list());
    const bool in_young_list = std::find(global_handles->traced_young_nodes_.begin(), global_handles->traced_young_nodes_.end(), node) != global_handles->traced_young_nodes_.end();
    DCHECK_EQ(in_young_list, node->is_in_young_list());
#endif // DEBUG
}

size_t GlobalHandles::TotalSize() const { return regular_nodes_->TotalSize() + traced_nodes_->TotalSize(); }

size_t GlobalHandles::UsedSize() const { return regular_nodes_->handles_count() * sizeof(Node) + traced_nodes_->handles_count() * sizeof(TracedNode); }

size_t GlobalHandles::handles_count() const { return regular_nodes_->handles_count() + traced_nodes_->handles_count(); }

GlobalHandles::GlobalHandles(Isolate* isolate)
    : isolate_(isolate)
    , regular_nodes_(std::make_unique<NodeSpace<GlobalHandles::Node>>(this))
    , traced_nodes_(std::make_unique<NodeSpace<GlobalHandles::TracedNode>>(this))
{
}
...
template <typename NodeType> bool NeedsTrackingInYoungNodes(Object value, NodeType* node) { return ObjectInYoungGeneration(value) && !node->is_in_young_list(); }
...
Handle<Object> GlobalHandles::Create(Address value) { return Create(Object(value)); }

Handle<Object> GlobalHandles::CreateTraced(Object value, Address* slot, GlobalHandleStoreMode store_mode)
{
...
Handle<Object> GlobalHandles::CreateTraced(Address value, Address* slot, GlobalHandleStoreMode store_mode) { return CreateTraced(Object(value), slot, store_mode); }
...
namespace {
void SetSlotThreadSafe(Address** slot, Address* val) { reinterpret_cast<std::atomic<Address*>*>(slot)->store(val, std::memory_order_relaxed); }
} // namespace

// static
void GlobalHandles::CopyTracedReference(const Address* const* from, Address** to)
{
    DCHECK_NOT_NULL(*from);
    DCHECK_NULL(*to);
    const TracedNode* from_node = TracedNode::FromLocation(*from);
    DCHECK_NE(kGlobalHandleZapValue, from_node->raw_object());
    GlobalHandles* global_handles = GlobalHandles::From(const_cast<TracedNode*>(from_node));
    Handle<Object> o = global_handles->CreateTraced(from_node->object(), reinterpret_cast<Address*>(to), GlobalHandleStoreMode::kAssigningStore);
...
// static
GlobalHandles* GlobalHandles::From(const TracedNode* node) { return NodeBlock<TracedNode>::From(node)->global_handles(); }
...
// static
Object GlobalHandles::MarkTracedConservatively(Address* inner_location, Address* traced_node_block_base)
{
...
void GlobalHandles::DestroyTracedReference(Address* location)
{
    ...
    TracedNode* node = TracedNode::FromLocation(location);
    auto* global_handles = GlobalHandles::From(node);
    DCHECK_IMPLIES(global_handles->is_marking_, !global_handles->is_sweeping_on_mutator_thread_);
    DCHECK_IMPLIES(global_handles->is_sweeping_on_mutator_thread_, !global_handles->is_marking_);
...
void GlobalHandles::MakeWeak(Address** location_addr) { Node::FromLocation(*location_addr)->MakeWeak(location_addr); }

void* GlobalHandles::ClearWeakness(Address* location) { return Node::FromLocation(location)->ClearWeakness(); }

void GlobalHandles::AnnotateStrongRetainer(Address* location, const char* label) { Node::FromLocation(location)->AnnotateStrongRetainer(label); }

bool GlobalHandles::IsWeak(Address* location) { return Node::FromLocation(location)->IsWeak(); }
...
void GlobalHandles::ComputeWeaknessForYoungObjects(WeakSlotCallback is_unmodified)
{
    ...
    for (TracedNode* node : traced_young_nodes_) {
        if (node->IsInUse()) {
            ...
            if (is_unmodified(node->location())) {
                ...
                node->set_root(handler->IsRoot(*reinterpret_cast<v8::TracedReference<v8::Value>*>(&value)));
            }
        }
    }
}
...
void GlobalHandles::ProcessWeakYoungObjects(RootVisitor* v, WeakSlotCallbackWithHeap should_reset_handle)
{
    ...
    for (TracedNode* node : traced_young_nodes_) {
        ...
        DCHECK_IMPLIES(node->is_root(), !should_reset_handle(isolate_->heap(), node->location()));
        if (should_reset_handle(isolate_->heap(), node->location())) {
            v8::Value* value = ToApi<v8::Value>(node->handle());
            handler->ResetRoot(*reinterpret_cast<v8::TracedReference<v8::Value>*>(&value));
...
void GlobalHandles::InvokeSecondPassPhantomCallbacks()
{
    ...
    if (scope.CheckReenter()) {
        ...
        isolate()->heap()->CallGCPrologueCallbacks(GCType::kGCTypeProcessWeakCallbacks, kNoGCCallbackFlags);
        {
        ...
        isolate()->heap()->CallGCEpilogueCallbacks(GCType::kGCTypeProcessWeakCallbacks, kNoGCCallbackFlags);
    }
}
...
template <typename T> void ClearListOfYoungNodesImpl(Isolate* isolate, std::vector<T*>* node_list)
{
    for (T* node : *node_list) {
        DCHECK(node->is_in_young_list());
        node->set_in_young_list(false);
        DCHECK_IMPLIES(node->IsInUse() && node->state() != T::NEAR_DEATH, !ObjectInYoungGeneration(node->object()));
    }
...
template <typename T> size_t GlobalHandles::InvokeFirstPassWeakCallbacks(std::vector<std::pair<T*, PendingPhantomCallback>>* pending)
{
...
size_t GlobalHandles::InvokeFirstPassWeakCallbacks() { return InvokeFirstPassWeakCallbacks(&regular_pending_phantom_callbacks_); }
...
void GlobalHandles::PostGarbageCollectionProcessing(GarbageCollector collector, const v8::GCCallbackFlags gc_callback_flags)
{
...
GlobalHandles::NodeBounds GlobalHandles::GetTracedNodeBounds() const { return traced_nodes_->GetNodeBlockBounds(); }

START_ALLOW_USE_DEPRECATED()

DISABLE_CFI_PERF void GlobalHandles::IterateTracedNodes(v8::EmbedderHeapTracer::TracedGlobalHandleVisitor* visitor)
{
    for (TracedNode* node : *traced_nodes_) {
        if (node->IsInUse()) {
            v8::Value* value = ToApi<v8::Value>(node->handle());
            visitor->VisitTracedReference(*reinterpret_cast<v8::TracedReference<v8::Value>*>(&value));
        }
    }
}
