...
template <typename T> Handle<T> LocalHandleScope::CloseAndEscape(Handle<T> handle_value)
{
...
void LocalHandleScope::CloseScope(LocalHeap* local_heap, Address* prev_next, Address* prev_limit)
{
    ...
    MSAN_ALLOCATED_UNINITIALIZED_MEMORY(handles->scope_.next, static_cast<size_t>(reinterpret_cast<Address>(old_limit) - reinterpret_cast<Address>(handles->scope_.next)));
}
