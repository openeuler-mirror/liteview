...
public:
    ...
    static Object MarkTracedConservatively(Address* inner_location, Address* traced_node_block_base);
    ...
    template <typename T> inline Handle<T> Create(T value);

    Handle<Object> CreateTraced(Object value, Address* slot, GlobalHandleStoreMode store_mode);
    Handle<Object> CreateTraced(Address value, Address* slot, GlobalHandleStoreMode store_mode);
    ...
    void PostGarbageCollectionProcessing(GarbageCollector collector, const v8::GCCallbackFlags gc_callback_flags);
    ...
    void IterateTracedNodes(v8::EmbedderHeapTracer::TracedGlobalHandleVisitor* visitor);
    ...
    void NotifyStartSweepingOnMutatorThread() { is_sweeping_on_mutator_thread_ = true; }
    void NotifyEndSweepingOnMutatorThread() { is_sweeping_on_mutator_thread_ = false; }
...
private:
    ...
    template <typename T> size_t InvokeFirstPassWeakCallbacks(std::vector<std::pair<T*, PendingPhantomCallback>>* pending);
    ...
    std::vector<std::pair<Node*, PendingPhantomCallback>> regular_pending_phantom_callbacks_;
...
public:
    ...
    inline Handle<Object> Get(int index) { return Handle<Object>(GetLocation(index)); }
...
public:
    class Iterator {
    public:
        explicit Iterator(std::vector<Address, StrongRootBlockAllocator>::iterator it)
            : it_(it)
        {
        }
        Iterator& operator++()
