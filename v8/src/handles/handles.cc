...
bool HandleBase::IsDereferenceAllowed() const
{
    ...
    if (IsReadOnlyHeapObject(heap_object))
        return true;
...
template <typename T> bool DirectHandle<T>::IsDereferenceAllowed() const
{
...
Address HandleScope::current_level_address(Isolate* isolate) { return reinterpret_cast<Address>(&isolate->handle_scope_data()->level); }

Address HandleScope::current_next_address(Isolate* isolate) { return reinterpret_cast<Address>(&isolate->handle_scope_data()->next); }

Address HandleScope::current_limit_address(Isolate* isolate) { return reinterpret_cast<Address>(&isolate->handle_scope_data()->limit); }
    ...
    , isolate_(isolate)
{
    ...
    identity_map_ = std::make_unique<CanonicalHandlesMap>(isolate->heap(), ZoneAllocationPolicy(zone_));
...
std::unique_ptr<CanonicalHandlesMap> CanonicalHandleScope::DetachCanonicalHandles() { return std::move(identity_map_); }

template <class CompilationInfoT>
CanonicalHandleScopeForOptimization<CompilationInfoT>::CanonicalHandleScopeForOptimization(Isolate* isolate, CompilationInfoT* info)
    : CanonicalHandleScope(isolate, info->zone())
    , info_(info)
{
}

template <class CompilationInfoT> CanonicalHandleScopeForOptimization<CompilationInfoT>::~CanonicalHandleScopeForOptimization()
{
...
#ifdef V8_ENABLE_MAGLEV
template class CanonicalHandleScopeForOptimization<maglev::ExportedMaglevCompilationInfo>;
#endif // V8_ENABLE_MAGLEV
