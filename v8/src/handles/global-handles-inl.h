...
template <typename T> Handle<T> GlobalHandles::Create(T value)
{
...
template <typename T> T GlobalHandleVector<T>::Pop()
{
...
// static
Object GlobalHandles::Acquire(Address* location) { return Object(reinterpret_cast<std::atomic<Address>*>(location)->load(std::memory_order_acquire)); }
