...
// domain
void VTuneDomainSupportExtension::Mark(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    if (args.Length() != 3 || !args[0]->IsString() || !args[1]->IsString() || !args[2]->IsString()) {
        args.GetIsolate()->ThrowError("Parameter number should be exactly three, first domain name"
    ...
    if ((r = libvtune::invoke(params.str().c_str())) != 0) {
        args.GetIsolate()->ThrowError(v8::String::NewFromUtf8(args.GetIsolate(), std::to_string(r).c_str()).ToLocalChecked());
    }
}
