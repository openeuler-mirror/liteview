...
public:
    SanitizeNativeContextScope(Isolate* isolate, NativeContext native_context, bool allow_active_isolate_for_testing, const DisallowGarbageCollection& no_gc)
...
#endif
        microtask_queue_external_pointer_ = native_context.RawExternalPointerField(NativeContext::kMicrotaskQueueOffset).GetAndClearContentForSerialization(no_gc);
    ...
    ~SanitizeNativeContextScope()
    {
        ...
        native_context_.RawExternalPointerField(NativeContext::kMicrotaskQueueOffset).RestoreContentAfterSerialization(microtask_queue_external_pointer_, no_gc_);
    }
...
ContextSerializer::ContextSerializer(Isolate* isolate, Snapshot::SerializerFlags flags, StartupSerializer* startup_serializer, v8::SerializeEmbedderFieldsCallback callback)
...
ContextSerializer::~ContextSerializer() { OutputStatistics("ContextSerializer"); }
...
bool ContextSerializer::ShouldBeInTheStartupObjectCache(HeapObject o)
{
    ...
    // would cause dupes.
    DCHECK(!o.IsScript());
    return o.IsName() || o.IsSharedFunctionInfo() || o.IsHeapNumber() || (V8_EXTERNAL_CODE_SPACE_BOOL && o.IsCodeDataContainer()) || o.IsCode() || o.IsScopeInfo()
        || o.IsAccessorInfo() || o.IsTemplateInfo() || o.IsClassPositions() || o.map() == ReadOnlyRoots(isolate()).fixed_cow_array_map();
}
...
bool ContextSerializer::SerializeJSObjectWithEmbedderFields(Handle<JSObject> obj)
{
    DisallowGarbageCollection no_gc;
    JSObject js_obj = *obj;
    ...
    for (int i = 0; i < embedder_fields_count; i++) {
        ...
        if (object.IsHeapObject()) {
            ...
            if (serialize_embedder_fields_.callback == nullptr && object == Smi::zero()) {
                ...
                StartupData data = serialize_embedder_fields_.callback(api_obj, i, serialize_embedder_fields_.data);
    ...
    const SerializerReference* reference = reference_map()->LookupReference(js_obj);
    ...
    for (int i = 0; i < embedder_fields_count; i++) {
        ...
        embedder_fields_sink_.PutRaw(reinterpret_cast<const byte*>(data.data), data.raw_size, "embedder fields data");
        delete[] data.data;
    }
