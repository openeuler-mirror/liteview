...
ReadOnlySerializer::~ReadOnlySerializer() { OutputStatistics("ReadOnlySerializer"); }
...
void ReadOnlySerializer::SerializeReadOnlyRoots()
{
    ...
    CHECK_IMPLIES(!allow_active_isolate_for_testing(), isolate()->handle_scope_implementer()->blocks()->empty());
...
void ReadOnlySerializer::FinalizeSerialization()
{
    ...
    VisitRootPointer(Root::kReadOnlyObjectCache, nullptr, FullObjectSlot(&undefined));
...
{
    if (root_has_been_serialized(RootIndex::kFreeSpaceMap) && root_has_been_serialized(RootIndex::kOnePointerFillerMap)
...
bool ReadOnlySerializer::SerializeUsingReadOnlyObjectCache(SnapshotByteSink* sink, Handle<HeapObject> obj)
{
...
void ReadOnlySerializer::ReconstructReadOnlyObjectCacheForTesting()
{
    ...
    for (size_t i = 0, size = ro_heap->read_only_object_cache_size(); i < size; i++) {
        Handle<HeapObject> obj(HeapObject::cast(ro_heap->cached_read_only_object(i)), isolate());
        int cache_index = SerializeInObjectCache(obj);
        USE(cache_index);
