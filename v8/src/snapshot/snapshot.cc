...
public:
    static v8::StartupData CreateSnapshotBlob(const SnapshotData* startup_snapshot_in, const SnapshotData* read_only_snapshot_in, const SnapshotData* shared_heap_snapshot_in,
        const std::vector<SnapshotData*>& context_snapshots_in, bool can_be_rehashed);
    ...
    static base::Vector<const byte> ExtractStartupData(const v8::StartupData* data);
    static base::Vector<const byte> ExtractReadOnlyData(const v8::StartupData* data);
    static base::Vector<const byte> ExtractSharedHeapData(const v8::StartupData* data);
    static base::Vector<const byte> ExtractContextData(const v8::StartupData* data, uint32_t index);

    static uint32_t GetHeaderValue(const v8::StartupData* data, uint32_t offset) { return base::ReadLittleEndianValue<uint32_t>(reinterpret_cast<Address>(data->data) + offset); }
    static void SetHeaderValue(char* data, uint32_t offset, uint32_t value) { base::WriteLittleEndianValue(reinterpret_cast<Address>(data) + offset, value); }
    ...
    static base::Vector<const byte> ChecksummedContent(const v8::StartupData* data)
    {
        static_assert(kVersionStringOffset == kChecksumOffset + kUInt32Size);
        const uint32_t kChecksumStart = kVersionStringOffset;
        return base::Vector<const byte>(reinterpret_cast<const byte*>(data->data + kChecksumStart), data->raw_size - kChecksumStart);
    }

    static uint32_t StartupSnapshotOffset(int num_contexts) { return POINTER_SIZE_ALIGN(kFirstContextOffsetOffset + num_contexts * kInt32Size); }

    static uint32_t ContextSnapshotOffsetOffset(int index) { return kFirstContextOffsetOffset + index * kInt32Size; }
};

} // namespace

SnapshotData MaybeDecompress(Isolate* isolate, const base::Vector<const byte>& snapshot_data)
{
#ifdef V8_SNAPSHOT_COMPRESSION
...
#ifdef DEBUG
bool Snapshot::SnapshotIsValid(const v8::StartupData* snapshot_blob) { return SnapshotImpl::ExtractNumContexts(snapshot_blob) > 0; }
#endif // DEBUG
...
bool Snapshot::Initialize(Isolate* isolate)
{
    ...
    base::Vector<const byte> startup_data = SnapshotImpl::ExtractStartupData(blob);
    base::Vector<const byte> read_only_data = SnapshotImpl::ExtractReadOnlyData(blob);
    base::Vector<const byte> shared_heap_data = SnapshotImpl::ExtractSharedHeapData(blob);
    ...
    bool success = isolate->InitWithSnapshot(&startup_snapshot_data, &read_only_snapshot_data, &shared_heap_snapshot_data, ExtractRehashability(blob));
    if (v8_flags.profile_deserialization) {
...
MaybeHandle<Context> Snapshot::NewContextFromSnapshot(
    Isolate* isolate, Handle<JSGlobalProxy> global_proxy, size_t context_index, v8::DeserializeEmbedderFieldsCallback embedder_fields_deserializer)
{
    if (!isolate->snapshot_available())
    ...
    base::Vector<const byte> context_data = SnapshotImpl::ExtractContextData(blob, static_cast<uint32_t>(context_index));
    SnapshotData snapshot_data(MaybeDecompress(isolate, context_data));

    MaybeHandle<Context> maybe_result = ContextDeserializer::DeserializeContext(isolate, &snapshot_data, can_rehash, global_proxy, embedder_fields_deserializer);
...
void Snapshot::ClearReconstructableDataForSerialization(Isolate* isolate, bool clear_recompilable_data)
{
        ...
        std::vector<i::Handle<i::SharedFunctionInfo>> sfis_to_clear;
        {
            ...
            for (i::HeapObject o = it.Next(); !o.is_null(); o = it.Next()) {
                if (o.IsSharedFunctionInfo(cage_base)) {
                    ...
                    if (shared.script(cage_base).IsScript(cage_base) && Script::cast(shared.script(cage_base)).type() == Script::TYPE_EXTENSION) {
    ...
    for (i::HeapObject o = it.Next(); !o.is_null(); o = it.Next()) {
        ...
        if (clear_recompilable_data) {
            ...
            DCHECK(
                fun.shared().HasWasmExportedFunctionData() || fun.shared().HasBuiltinId() || fun.shared().IsApiFunction() || fun.shared().HasUncompiledDataWithoutPreparseData());
#else
...
// static
void Snapshot::SerializeDeserializeAndVerifyForTesting(Isolate* isolate, Handle<Context> default_context)
{
    ...
    // Test serialization.
    {
        ...
        Snapshot::SerializerFlags flags(Snapshot::kAllowUnknownExternalReferencesForTesting | Snapshot::kAllowActiveIsolateForTesting
            | ((isolate->has_shared_heap() || ReadOnlyHeap::IsReadOnlySpaceShared()) ? Snapshot::kReconstructReadOnlyAndSharedObjectCachesForTesting : 0));
        serialized_data = Snapshot::Create(isolate, *default_context, global_safepoint, no_gc, flags);
    ...
    std::unique_ptr<v8::ArrayBuffer::Allocator> array_buffer_allocator(v8::ArrayBuffer::Allocator::NewDefaultAllocator());
    {
        ...
        Handle<Context> new_native_context = new_isolate->bootstrapper()->CreateEnvironmentForTesting();
...
// static
v8::StartupData Snapshot::Create(Isolate* isolate, std::vector<Context>* contexts, const std::vector<SerializeInternalFieldsCallback>& embedder_fields_serializers,
    const GlobalSafepointScope& global_safepoint, const DisallowGarbageCollection& no_gc, SerializerFlags flags)
{
    ...
    SharedHeapSerializer shared_heap_serializer(isolate, flags, &read_only_serializer);
    ...
    if (v8_flags.serialization_statistics) {
        ...
        PrintF("%10d bytes per isolate\n", read_only_serializer.TotalAllocationSize() + startup_serializer.TotalAllocationSize());
        for (int i = 0; i < num_contexts; i++) {
    ...
    v8::StartupData result = SnapshotImpl::CreateSnapshotBlob(&startup_snapshot, &read_only_snapshot, &shared_heap_snapshot, context_snapshots, can_be_rehashed);
...
v8::StartupData Snapshot::Create(
    Isolate* isolate, Context default_context, const GlobalSafepointScope& global_safepoint, const DisallowGarbageCollection& no_gc, SerializerFlags flags)
{
    std::vector<Context> contexts { default_context };
    std::vector<SerializeInternalFieldsCallback> callbacks { {} };
    return Snapshot::Create(isolate, &contexts, callbacks, global_safepoint, no_gc, flags);
}

v8::StartupData SnapshotImpl::CreateSnapshotBlob(const SnapshotData* startup_snapshot_in, const SnapshotData* read_only_snapshot_in, const SnapshotData* shared_heap_snapshot_in,
    const std::vector<SnapshotData*>& context_snapshots_in, bool can_be_rehashed)
{
...
uint32_t Snapshot::GetExpectedChecksum(const v8::StartupData* data) { return SnapshotImpl::GetHeaderValue(data, SnapshotImpl::kChecksumOffset); }
uint32_t Snapshot::CalculateChecksum(const v8::StartupData* data) { return Checksum(SnapshotImpl::ChecksummedContent(data)); }
...
{
    CHECK_LT(SnapshotImpl::kRehashabilityOffset, static_cast<uint32_t>(data->raw_size));
...
namespace {
base::Vector<const byte> ExtractData(const v8::StartupData* snapshot, uint32_t start_offset, uint32_t end_offset)
{
...
base::Vector<const byte> SnapshotImpl::ExtractStartupData(const v8::StartupData* data)
{
...
base::Vector<const byte> SnapshotImpl::ExtractReadOnlyData(const v8::StartupData* data)
{
    DCHECK(Snapshot::SnapshotIsValid(data));
...
base::Vector<const byte> SnapshotImpl::ExtractSharedHeapData(const v8::StartupData* data)
{
    DCHECK(Snapshot::SnapshotIsValid(data));
...
base::Vector<const byte> SnapshotImpl::ExtractContextData(const v8::StartupData* data, uint32_t index)
{
    uint32_t num_contexts = ExtractNumContexts(data);
...
v8::StartupData CreateSnapshotDataBlobInternal(v8::SnapshotCreator::FunctionCodeHandling function_code_handling, const char* embedded_source, v8::Isolate* isolate)
{
