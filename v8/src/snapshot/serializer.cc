...
void Serializer::OutputStatistics(const char* name)
{
    ...
    for (int space = 0; space < kNumberOfSnapshotSpaces; space++) {
        PrintF("%16s", BaseSpace::GetSpaceName(static_cast<AllocationSpace>(space)));
    }
    PrintF("\n");
...
#define PRINT_INSTANCE_TYPE(Name)                                                                                                                                                  \
    for (int space = 0; space < kNumberOfSnapshotSpaces; ++space) {                                                                                                                \
        if (instance_type_count_[space][Name]) {                                                                                                                                   \
            PrintF("%10d %10zu  %-10s %s\n", instance_type_count_[space][Name], instance_type_size_[space][Name], BaseSpace::GetSpaceName(static_cast<AllocationSpace>(space)),    \
                #Name);                                                                                                                                                            \
        }                                                                                                                                                                          \
    }
...
void Serializer::PutRoot(RootIndex root)
{
    ...
    if (root_index < kRootArrayConstantsCount && !Heap::InYoungGeneration(object)) {
...
void Serializer::PutBackReference(HeapObject object, SerializerReference reference)
{
...
Code Serializer::CopyCode(Code code)
{
    ...
    code_buffer_.insert(code_buffer_.end(), reinterpret_cast<byte*>(code.address()), reinterpret_cast<byte*>(code.address() + size));
    // When pointer compression is enabled the checked cast will try to
    // decompress map field of off-heap Code object.
    return Code::unchecked_cast(HeapObject::FromAddress(reinterpret_cast<Address>(&code_buffer_.front())));
}

void Serializer::ObjectSerializer::SerializePrologue(SnapshotSpace space, int size, Map map)
{
...
uint32_t Serializer::ObjectSerializer::SerializeBackingStore(void* backing_store, int32_t byte_length, Maybe<int32_t> max_byte_length)
{
    DisallowGarbageCollection no_gc;
...
void Serializer::ObjectSerializer::SerializeExternalStringAsSequentialString()
{
    ...
    if (object_->IsExternalOneByteString(cage_base)) {
        map = internalized ? roots.one_byte_internalized_string_map() : roots.one_byte_string_map();
        allocation_size = SeqOneByteString::SizeFor(length);
        content_size = length * kCharSize;
        resource = reinterpret_cast<const byte*>(Handle<ExternalOneByteString>::cast(string)->resource()->data());
    } else {
        map = internalized ? roots.internalized_string_map() : roots.string_map();
        allocation_size = SeqTwoByteString::SizeFor(length);
        content_size = length * kShortSize;
        resource = reinterpret_cast<const byte*>(Handle<ExternalTwoByteString>::cast(string)->resource()->data());
    }
...
public:
    explicit UnlinkWeakNextScope(Heap* heap, HeapObject object)
    {
        ...
        if (object.IsAllocationSite(isolate) && AllocationSite::cast(object).HasWeakNext()) {
            object_ = object;
            next_ = AllocationSite::cast(object).weak_next();
            AllocationSite::cast(object).set_weak_next(ReadOnlyRoots(isolate).undefined_value());
        }
    }
...
void Serializer::ObjectSerializer::Serialize()
{
...

    {
        ...
        if ((recursion.ExceedsMaximum() && CanBeDeferred(raw)) || serializer_->MustBeDeferred(raw)) {
...
SnapshotSpace GetSnapshotSpace(HeapObject object)
{
    if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL) {
        ...
        AllocationSpace heap_space = MemoryChunk::FromHeapObject(object)->owner_identity();
...
void Serializer::ObjectSerializer::VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) { VisitPointers(host, MaybeObjectSlot(start), MaybeObjectSlot(end)); }

void Serializer::ObjectSerializer::VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end)
{
    ...
    while (current < end) {
        ...
        while (current < end && current.load(cage_base).GetHeapObject(&current_contents, &reference_type)) {
            ...
            if (repeat_end < end && serializer_->root_index_map()->Lookup(*obj, &root_index) && RootsTable::IsImmortalImmovable(root_index)
...
void Serializer::ObjectSerializer::VisitCodePointer(HeapObject host, CodeObjectSlot slot)
{
...
void Serializer::ObjectSerializer::OutputExternalReference(Address target, int target_size, bool sandboxify, ExternalPointerTag tag)
{
    ...
    if (!encoded_successfully) {
        ...
        if (sandboxify) {
            ...
            sink_->PutRaw(reinterpret_cast<byte*>(&target), target_size, "raw pointer");
        } else {
    ...
    if (sandboxify) {
        sink_->PutInt(static_cast<uint32_t>(tag >> kExternalPointerTagShift), "external pointer tag");
    }
}
...
void Serializer::ObjectSerializer::VisitEmbeddedPointer(Code host, RelocInfo* rinfo)
{
...
void Serializer::ObjectSerializer::VisitExternalReference(Code host, RelocInfo* rinfo)
{
    Address target = rinfo->target_external_reference();
    DCHECK_NE(target, kNullAddress); // Code does not reference null.
    DCHECK_IMPLIES(serializer_->EncodeExternalReference(target).is_from_api(), !rinfo->IsCodedSpecially());
    // Don't "sandboxify" external references embedded in the code.
    OutputExternalReference(target, rinfo->target_address_size(), false, kExternalPointerNullTag);
}

void Serializer::ObjectSerializer::VisitInternalReference(Code host, RelocInfo* rinfo)
{
...
void Serializer::ObjectSerializer::VisitExternalPointer(HeapObject host, ExternalPointerSlot slot, ExternalPointerTag tag)
{
    PtrComprCageBase cage_base(isolate());
    InstanceType instance_type = object_->map(cage_base).instance_type();
    if (InstanceTypeChecker::IsForeign(instance_type) || InstanceTypeChecker::IsJSExternalObject(instance_type) || InstanceTypeChecker::IsAccessorInfo(instance_type)
        ...
        const bool sandboxify = V8_ENABLE_SANDBOX_BOOL && tag != kExternalPointerNullTag;
        ...
        DCHECK(
            ...
            (InstanceTypeChecker::IsJSObject(instance_type) && JSObject::cast(host).GetEmbedderFieldCount() > 0) ||
            // See ObjectSerializer::OutputRawData().
            (V8_EXTERNAL_CODE_SPACE_BOOL && InstanceTypeChecker::IsCodeDataContainer(instance_type)));
    }
}

void Serializer::ObjectSerializer::VisitOffHeapTarget(Code host, RelocInfo* rinfo)
{
...
void Serializer::ObjectSerializer::VisitCodeTarget(Code host, RelocInfo* rinfo)
{
    // Target object should be pre-serialized by RelocInfoObjectPreSerializer, so
    // just track the pointer's existence as kTaggedSize in
    // bytes_processed_so_far_.
...
void OutputRawWithCustomField(SnapshotByteSink* sink, Address object_start, int written_so_far, int bytes_to_write, int field_offset, int field_size, const byte* field_value)
{
    int offset = field_offset - written_so_far;
    if (0 <= offset && offset < bytes_to_write) {
        DCHECK_GE(bytes_to_write, offset + field_size);
        sink->PutRaw(reinterpret_cast<byte*>(object_start + written_so_far), offset, "Bytes");
        sink->PutRaw(field_value, field_size, "Bytes");
        written_so_far += offset + field_size;
        bytes_to_write -= offset + field_size;
        sink->PutRaw(reinterpret_cast<byte*>(object_start + written_so_far), bytes_to_write, "Bytes");
    } else {
        sink->PutRaw(reinterpret_cast<byte*>(object_start + written_so_far), bytes_to_write, "Bytes");
    }
}
...
void Serializer::ObjectSerializer::OutputRawData(Address up_to)
{
    ...
    if (bytes_to_output != 0) {
        ...
        if (object_->IsSeqString()) {
            ...
            msan_bytes_to_output -= SeqString::cast(*object_).GetDataAndPaddingSizes().padding_size;
        }
        __msan_check_mem_is_initialized(reinterpret_cast<void*>(object_start + base), msan_bytes_to_output);
        ...
        if (object_->IsBytecodeArray(cage_base)) {
            ...
            OutputRawWithCustomField(sink_, object_start, base, bytes_to_output, BytecodeArray::kBytecodeAgeOffset, sizeof(field_value), reinterpret_cast<byte*>(&field_value));
            ...
            OutputRawWithCustomField(sink_, object_start, base, bytes_to_output, DescriptorArray::kRawNumberOfMarkedDescriptorsOffset, sizeof(field_value), field_value);
        } else if (V8_EXTERNAL_CODE_SPACE_BOOL && object_->IsCodeDataContainer(cage_base)) {
            ...
            CHECK_EQ(CodeDataContainer::kCodeCageBaseUpper32BitsOffset + kTaggedSize, CodeDataContainer::kCodeEntryPointOffset);
            static byte field_value[kTaggedSize + kSystemPointerSize] = { 0 };
            OutputRawWithCustomField(sink_, object_start, base, bytes_to_output, CodeDataContainer::kCodeCageBaseUpper32BitsOffset, sizeof(field_value), field_value);
            ...
            SeqString::DataAndPaddingSizes sizes = SeqString::cast(*object_).GetDataAndPaddingSizes();
            DCHECK_EQ(bytes_to_output, sizes.data_size - base + sizes.padding_size);
            int data_bytes_to_output = sizes.data_size - base;
            sink_->PutRaw(reinterpret_cast<byte*>(object_start + base), data_bytes_to_output, "SeqStringData");
            sink_->PutN(sizes.padding_size, 0, "SeqStringPadding");
        } else {
            sink_->PutRaw(reinterpret_cast<byte*>(object_start + base), bytes_to_output, "Bytes");
        }
    }
}
...
{
    static const int kWipeOutModeMask = RelocInfo::ModeMask(RelocInfo::CODE_TARGET) | RelocInfo::ModeMask(RelocInfo::FULL_EMBEDDED_OBJECT)
        | RelocInfo::ModeMask(RelocInfo::COMPRESSED_EMBEDDED_OBJECT) | RelocInfo::ModeMask(RelocInfo::EXTERNAL_REFERENCE) | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE)
    ...
    for (RelocIterator it(off_heap_code, relocation_info, kWipeOutModeMask); !it.done(); it.next()) {
...
#ifdef MEMORY_SANITIZER
    // Check that we do not serialize uninitialized memory.
    __msan_check_mem_is_initialized(reinterpret_cast<void*>(start), bytes_to_output);
#endif // MEMORY_SANITIZER
    ...
    DCHECK_EQ(HeapObject::kHeaderSize, bytes_processed_so_far_);
    VisitPointers(*on_heap_code, on_heap_code->RawField(HeapObject::kHeaderSize), on_heap_code->RawField(Code::kDataStart));
    ...
    for (RelocIterator it(*on_heap_code, relocation_info, Code::BodyDescriptor::kRelocModeMask); !it.done(); it.next()) {
    ...
    // object).
    DCHECK_EQ(bytes_processed_so_far_, Code::kDataStart + kTaggedSize * pre_serializer.num_serialized_objects());
}
...
Serializer::HotObjectsList::~HotObjectsList() { heap_->UnregisterStrongRoots(strong_roots_entry_); }
