...
bool EmbeddedData::BuiltinContains(Builtin builtin, Address pc) const
{
    ...
    Address start = reinterpret_cast<Address>(RawCode() + desc.instruction_offset);
...
uint32_t EmbeddedData::CodeCommentsSizeOf(Builtin builtin) const
{
    DCHECK(Builtins::IsBuiltinId(builtin));
    const struct LayoutDescription& desc = LayoutDescription(builtin);
    DCHECK_LE(desc.code_comments_offset_offset, desc.unwinding_info_offset_offset);
...
Address EmbeddedData::InstructionStartOfBytecodeHandlers() const { return InstructionStartOfBuiltin(Builtin::kFirstBytecodeHandler); }

Address EmbeddedData::InstructionEndOfBytecodeHandlers() const
{
    static_assert(static_cast<int>(Builtin::kFirstBytecodeHandler) + kNumberOfBytecodeHandlers + 2 * kNumberOfWideBytecodeHandlers == Builtins::kBuiltinCount);
    Builtin lastBytecodeHandler = Builtins::FromInt(Builtins::kBuiltinCount - 1);
    return InstructionStartOfBuiltin(lastBytecodeHandler) + InstructionSizeOfBuiltin(lastBytecodeHandler);
}
