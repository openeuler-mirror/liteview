...
bool BuiltinAliasesOffHeapTrampolineRegister(Isolate* isolate, Code code)
{
    ...
    static_assert(CallInterfaceDescriptor::ContextRegister() != kOffHeapTrampolineRegister);
...
void FinalizeEmbeddedCodeTargets(Isolate* isolate, EmbeddedData* blob)
{
...
#if defined(V8_TARGET_ARCH_X64) || defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_ARM) || defined(V8_TARGET_ARCH_IA32) || defined(V8_TARGET_ARCH_S390)                    \
    || defined(V8_TARGET_ARCH_RISCV64) || defined(V8_TARGET_ARCH_LOONG64) || defined(V8_TARGET_ARCH_RISCV32)
        ...
        while (!on_heap_it.done()) {
            ...
            off_heap_it.rinfo()->set_off_heap_target_address(blob->InstructionStartOfBuiltin(target.builtin_id()));
...
EmbeddedData EmbeddedData::FromIsolate(Isolate* isolate)
{
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        uint32_t instruction_size = static_cast<uint32_t>(code.raw_instruction_size());
        ...
        DCHECK_EQ(0, raw_code_size % kCodeAlignment);
        {
            ...
            struct LayoutDescription& layout_desc = layout_descriptions[builtin_index];
            ...
            layout_desc.handler_table_offset = raw_data_size + static_cast<uint32_t>(code.handler_table_offset());
#if V8_EMBEDDED_CONSTANT_POOL_BOOL
            layout_desc.constant_pool_offset = raw_data_size + static_cast<uint32_t>(code.constant_pool_offset());
#endif
            layout_desc.code_comments_offset_offset = raw_data_size + static_cast<uint32_t>(code.code_comments_offset());
            layout_desc.unwinding_info_offset_offset = raw_data_size + static_cast<uint32_t>(code.unwinding_info_offset());
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        DCHECK_LE(RawMetadataOffset() + offset + code.raw_metadata_size(), blob_data_size);
        std::memcpy(dst, reinterpret_cast<uint8_t*>(code.raw_metadata_start()), code.raw_metadata_size());
    }
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        DCHECK_LE(RawCodeOffset() + offset + code.raw_instruction_size(), blob_code_size);
...
size_t EmbeddedData::CreateEmbeddedBlobDataHash() const
{
    ...
    // themselves.
    base::Vector<const byte> payload(data_ + kFirstHashedDataOffset, data_size_ - kFirstHashedDataOffset);
