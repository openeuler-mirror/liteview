...
// static
ScriptCompiler::CachedData* CodeSerializer::Serialize(Handle<SharedFunctionInfo> info)
{
...
bool CodeSerializer::SerializeReadOnlyObject(HeapObject obj, const DisallowGarbageCollection& no_gc)
{
...
void CodeSerializer::SerializeObjectImpl(Handle<HeapObject> obj)
{
    ...
    InstanceType instance_type;
    {
        ...
        if (SerializeBackReference(raw))
            return;
        if (SerializeReadOnlyObject(raw, no_gc))
            return;
    ...
    if (InstanceTypeChecker::IsScript(instance_type)) {
        ...
        Handle<UncompiledDataWithoutPreparseDataWithJob> data = Handle<UncompiledDataWithoutPreparseDataWithJob>::cast(obj);
        ...
        Handle<UncompiledDataWithPreparseDataAndJob> data = Handle<UncompiledDataWithPreparseDataAndJob>::cast(obj);
        Address job = data->job();
        data->set_job(kNullAddress);
        SerializeGeneric(data);
        data->set_job(job);
    ...
    if (V8_UNLIKELY(v8_flags.interpreted_frames_native_stack) && obj->IsInterpreterData()) {
...
void CreateInterpreterDataForDeserializedCode(Isolate* isolate, Handle<SharedFunctionInfo> sfi, bool log_code_creation)
{
    ...
    for (SharedFunctionInfo shared_info = iter.Next(); !shared_info.is_null(); shared_info = iter.Next()) {
        ...
        if (info->HasBaselineCode()) {
            FromCodeT(info->baseline_code(kAcquireLoad)).set_bytecode_or_interpreter_data(*interpreter_data);
        } else {
        ...
        PROFILE(isolate, CodeCreateEvent(LogEventListener::CodeTag::kFunction, abstract_code, info, name_handle, line_num, column_num));
    }
}
...
public:
    ...
    MaybeHandle<SharedFunctionInfo> Finalize(Isolate* isolate, Handle<String> source, ScriptOriginOptions origin_options)
    {
        return CodeSerializer::FinishOffThreadDeserialize(isolate, std::move(off_thread_data_), cached_data_, source, origin_options);
    }
...
void FinalizeDeserialization(Isolate* isolate, Handle<SharedFunctionInfo> result, const base::ElapsedTimer& timer)
{
    const bool log_code_creation = isolate->v8_file_logger()->is_listening_to_code_events() || isolate->is_profiling() || isolate->logger()->is_listening_to_code_events();
    ...
    if (log_code_creation || v8_flags.log_function_events) {
        ...
        Handle<String> name(script->name().IsString() ? String::cast(script->name()) : ReadOnlyRoots(isolate).empty_string(), isolate);
        ...
        if (log_code_creation) {
            ...
            for (SharedFunctionInfo info = iter.Next(); !info.is_null(); info = iter.Next()) {
                if (info.is_compiled()) {
                    ...
                    DisallowGarbageCollection no_gc;
                    int line_num = script->GetLineNumber(shared_info->StartPosition()) + 1;
                    int column_num = script->GetColumnNumber(shared_info->StartPosition()) + 1;
                    PROFILE(isolate,
...
MaybeHandle<SharedFunctionInfo> CodeSerializer::Deserialize(Isolate* isolate, AlignedCachedData* cached_data, Handle<String> source, ScriptOriginOptions origin_options)
{
    ...
    const SerializedCodeData scd = SerializedCodeData::FromCachedData(cached_data, SerializedCodeData::SourceHash(source, origin_options), &sanity_check_result);
    ...
    BaselineBatchCompileIfSparkplugCompiled(isolate, Script::cast(result->script()));
    if (v8_flags.profile_deserialization) {
        double ms = timer.Elapsed().InMillisecondsF();
        int length = cached_data->length();
...
CodeSerializer::OffThreadDeserializeData CodeSerializer::StartDeserializeOffThread(LocalIsolate* local_isolate, AlignedCachedData* cached_data)
{
    ...
    const SerializedCodeData scd = SerializedCodeData::FromCachedDataWithoutSource(cached_data, &result.sanity_check_result);
    ...
    MaybeHandle<SharedFunctionInfo> local_maybe_result = OffThreadObjectDeserializer::DeserializeSharedFunctionInfo(local_isolate, &scd, &result.scripts);
...
MaybeHandle<SharedFunctionInfo> CodeSerializer::FinishOffThreadDeserialize(Isolate* isolate, OffThreadDeserializeData&& data, AlignedCachedData* cached_data, Handle<String> source,
    ScriptOriginOptions origin_options, BackgroundMergeTask* background_merge_task)
{
    base::ElapsedTimer timer;
    if (v8_flags.profile_deserialization || v8_flags.log_function_events)
        timer.Start();
    ...
    const SerializedCodeData scd
        = SerializedCodeData::FromPartiallySanityCheckedCachedData(cached_data, SerializedCodeData::SourceHash(source, origin_options), &sanity_check_result);
    if (sanity_check_result != SerializedCodeSanityCheckResult::kSuccess) {
    ...
    if (background_merge_task && background_merge_task->HasPendingForegroundWork()) {
        ...
        DCHECK(isolate->factory()->script_list()->Contains(MaybeObject::MakeWeak(MaybeObject::FromObject(result->script()))));
    } else {
        ...
        DCHECK_EQ(Script::cast(result->script()).source(), ReadOnlyRoots(isolate).empty_string());
        ...
        for (Handle<Script> script : data.scripts) {
            ...
            list = WeakArrayList::AddToEnd(isolate, list, MaybeObjectHandle::Weak(script));
        }
...
SerializedCodeData::SerializedCodeData(const std::vector<byte>* payload, const CodeSerializer* cs)
{
    DisallowGarbageCollection no_gc;
...
SerializedCodeSanityCheckResult SerializedCodeData::SanityCheck(uint32_t expected_source_hash) const
{
...
SerializedCodeSanityCheckResult SerializedCodeData::SanityCheckWithoutSource() const
{
...
uint32_t SerializedCodeData::SourceHash(Handle<String> source, ScriptOriginOptions origin_options)
{
...
SerializedCodeData SerializedCodeData::FromCachedData(AlignedCachedData* cached_data, uint32_t expected_source_hash, SerializedCodeSanityCheckResult* rejection_result)
{
    DisallowGarbageCollection no_gc;
...
SerializedCodeData SerializedCodeData::FromCachedDataWithoutSource(AlignedCachedData* cached_data, SerializedCodeSanityCheckResult* rejection_result)
{
    DisallowGarbageCollection no_gc;
    SerializedCodeData scd(cached_data);
