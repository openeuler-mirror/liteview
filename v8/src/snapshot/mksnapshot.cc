...
public:
    void SetSnapshotFile(const char* snapshot_cpp_file) { snapshot_cpp_path_ = snapshot_cpp_file; }

    void SetStartupBlobFile(const char* snapshot_blob_file) { snapshot_blob_path_ = snapshot_blob_file; }
    ...
    void WriteSnapshot(v8::StartupData blob) const
    {
        ...
        v8::base::Vector<const i::byte> blob_vector(reinterpret_cast<const i::byte*>(blob.data), blob.raw_size);
        MaybeWriteSnapshotFile(blob_vector);
        MaybeWriteStartupBlob(blob_vector);
    }

private:
    void MaybeWriteStartupBlob(const v8::base::Vector<const i::byte>& blob) const
    {
    ...
    void MaybeWriteSnapshotFile(const v8::base::Vector<const i::byte>& blob) const
    {
    ...
    static void WriteSnapshotFileData(FILE* fp, const v8::base::Vector<const i::byte>& blob)
    {
    ...
    static void WriteBinaryContentsAsCArray(FILE* fp, const v8::base::Vector<const i::byte>& blob)
    {
...
v8::StartupData CreateSnapshotDataBlob(v8::Isolate* isolate, const char* embedded_source)
{
    v8::base::ElapsedTimer timer;
    timer.Start();

    v8::StartupData result = i::CreateSnapshotDataBlobInternal(v8::SnapshotCreator::FunctionCodeHandling::kClear, embedded_source, isolate);
...
int main(int argc, char** argv)
{
    ...
    std::string usage = "Usage: " + std::string(argv[0]) + " [--startup-src=file]" + " [--startup-blob=file]" + " [--embedded-src=file]" + " [--embedded-variant=label]"
        + " [--target-arch=arch]" + " [--target-os=os] [extras]\n\n";
...

    {
        ...
        v8::StartupData blob;
        {
            ...
            size_t code_range_size_mb = i::kMaximalCodeRangeSize == 0 ? i::kMaxPCRelativeCodeRangeInMB : std::min(i::kMaximalCodeRangeSize / i::MB, i::kMaxPCRelativeCodeRangeInMB);
