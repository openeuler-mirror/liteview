...
public:
    static SlotAccessorForHeapObject ForSlotIndex(Handle<HeapObject> object, int index) { return SlotAccessorForHeapObject(object, index * kTaggedSize); }
    static SlotAccessorForHeapObject ForSlotOffset(Handle<HeapObject> object, int offset) { return SlotAccessorForHeapObject(object, offset); }

    MaybeObjectSlot slot() const { return object_->RawMaybeWeakField(offset_); }
    ExternalPointerSlot external_pointer_slot() const { return object_->RawExternalPointerField(offset_); }
    ...
    int Write(HeapObject value, HeapObjectReferenceType ref_type, int slot_offset = 0) { return Write(HeapObjectReference::From(value, ref_type), slot_offset); }
    int Write(Handle<HeapObject> value, HeapObjectReferenceType ref_type, int slot_offset = 0) { return Write(*value, ref_type, slot_offset); }
...
public:
    ...
    int Write(HeapObject value, HeapObjectReferenceType ref_type, int slot_offset = 0)
    {
    ...
    int Write(Handle<HeapObject> value, HeapObjectReferenceType ref_type, int slot_offset = 0)
    {
        DCHECK_EQ(slot_offset, 0);
        DCHECK_EQ(ref_type, HeapObjectReferenceType::STRONG);
        *handle_ = value;
...
template <typename IsolateT> template <typename TSlot> int Deserializer<IsolateT>::WriteAddress(TSlot dest, Address value)
{
...
template <typename IsolateT> int Deserializer<IsolateT>::WriteExternalPointer(ExternalPointerSlot dest, Address value, ExternalPointerTag tag)
{
    DCHECK(!next_reference_is_weak_);
...
Deserializer<IsolateT>::Deserializer(IsolateT* isolate, base::Vector<const byte> payload, uint32_t magic_number, bool deserializing_user_code, bool can_rehash)
    : isolate_(isolate)
    , source_(payload)
...
template <typename IsolateT> void Deserializer<IsolateT>::LogScriptEvents(Script script)
{
    DisallowGarbageCollection no_gc;
    LOG(isolate(), ScriptEvent(V8FileLogger::ScriptEventType::kDeserialize, script.id()));
    LOG(isolate(), ScriptDetails(script));
}

namespace {
template <typename IsolateT> uint32_t ComputeRawHashField(IsolateT* isolate, String string)
{
...
StringTableInsertionKey::StringTableInsertionKey(Isolate* isolate, Handle<String> string, DeserializingUserCodeOption deserializing_user_code)
...
StringTableInsertionKey::StringTableInsertionKey(LocalIsolate* isolate, Handle<String> string, DeserializingUserCodeOption deserializing_user_code)
    : StringTableKey(ComputeRawHashField(isolate, *string), string->length())
    , string_(string)
{
#ifdef DEBUG
...
template <typename IsolateT> bool StringTableInsertionKey::IsMatch(IsolateT* isolate, String string)
{
...
template bool StringTableInsertionKey::IsMatch(LocalIsolate* isolate, String string);
...
template <typename IsolateT> void Deserializer<IsolateT>::PostProcessNewJSReceiver(Map map, Handle<JSReceiver> obj, InstanceType instance_type, SnapshotSpace space)
{
...
template <typename IsolateT> void Deserializer<IsolateT>::PostProcessNewObject(Handle<Map> map, Handle<HeapObject> obj, SnapshotSpace space)
{
    DisallowGarbageCollection no_gc;
    Map raw_map = *map;
        ...
        if (deserializing_user_code()) {
            if (InstanceTypeChecker::IsInternalizedString(instance_type)) {
                ...
                if (result != raw_obj) {
                    ...
                    // transitioned so far.
                    String::cast(raw_obj).MakeThin(isolate(), result, UpdateInvalidatedObjectSize::kNo);
                ...
                DCHECK(CanBeDeferred(*obj) || InstanceTypeChecker::IsByteArray(instance_type));
            }
        }
    }
    ...
    if (InstanceTypeChecker::IsCode(instance_type)) {
    ...
    } else if (V8_EXTERNAL_CODE_SPACE_BOOL && InstanceTypeChecker::IsCodeDataContainer(instance_type)) {
        auto code_data_container = CodeDataContainer::cast(raw_obj);
        code_data_container.set_code_cage_base(isolate()->code_cage_base());
        code_data_container.init_code_entry_point(main_thread_isolate(), kNullAddress);
#ifdef V8_EXTERNAL_CODE_SPACE
        if (V8_REMOVE_BUILTINS_CODE_OBJECTS && code_data_container.is_off_heap_trampoline()) {
            Address entry = OffHeapInstructionStart(code_data_container, code_data_container.builtin_id());
            code_data_container.SetEntryPointForOffHeapBuiltin(main_thread_isolate(), entry);
        } else {
            code_data_container.UpdateCodeEntryPoint(main_thread_isolate(), code_data_container.code());
        }
#endif
        ...
        PostProcessExternalString(ExternalString::cast(raw_obj), main_thread_isolate());
    } else if (InstanceTypeChecker::IsJSReceiver(instance_type)) {
        // PostProcessNewJSReceiver may trigger GC.
        no_gc.Release();
        return PostProcessNewJSReceiver(raw_map, Handle<JSReceiver>::cast(obj), instance_type, space);
        ...
        NativeContext::cast(raw_obj).init_microtask_queue(main_thread_isolate(), nullptr);
    } else if (InstanceTypeChecker::IsScript(instance_type)) {
        LogScriptEvents(Script::cast(*obj));
    }
}

template <typename IsolateT> HeapObjectReferenceType Deserializer<IsolateT>::GetAndResetNextReferenceType()
{
    HeapObjectReferenceType type = next_reference_is_weak_ ? HeapObjectReferenceType::WEAK : HeapObjectReferenceType::STRONG;
...
template <typename IsolateT> Handle<HeapObject> Deserializer<IsolateT>::ReadObject()
{
...
template <typename IsolateT> Handle<HeapObject> Deserializer<IsolateT>::ReadObject(SnapshotSpace space)
{
    ...
    HeapObject raw_obj = Allocate(allocation, size_in_bytes, HeapObject::RequiredAlignment(*map));
    ...
    if (raw_obj.IsBytecodeArray(isolate())) {
        ...
        MemsetTagged(table.RawField(table.kElementsStartOffset), ReadOnlyRoots(isolate()).undefined_value(), (size_in_bytes - table.kElementsStartOffset) / kTaggedSize);
    }

#ifdef DEBUG
    PtrComprCageBase cage_base(isolate());
    // We want to make sure that all embedder pointers are initialized to null.
    if (raw_obj.IsJSObject(cage_base) && JSObject::cast(raw_obj).MayHaveEmbedderFields()) {
...
template <typename IsolateT> Handle<HeapObject> Deserializer<IsolateT>::ReadMetaMap()
{
    ...
    HeapObject raw_obj = Allocate(SpaceToAllocation(space), size_in_bytes, kTaggedAligned);
...
public:
    DeserializerRelocInfoVisitor(Deserializer<Isolate>* deserializer, const std::vector<Handle<HeapObject>>* objects)
    ...
    DeserializerRelocInfoVisitor(Deserializer<LocalIsolate>* deserializer, const std::vector<Handle<HeapObject>>* objects) { UNREACHABLE(); }

    ~DeserializerRelocInfoVisitor() { DCHECK_EQ(current_object_, objects_->size()); }
...
void DeserializerRelocInfoVisitor::VisitCodeTarget(Code host, RelocInfo* rinfo)
{
    HeapObject object = *objects_->at(current_object_++);
    rinfo->set_target_address(Code::cast(object).raw_instruction_start());
}

void DeserializerRelocInfoVisitor::VisitEmbeddedPointer(Code host, RelocInfo* rinfo)
{
    HeapObject object = *objects_->at(current_object_++);
...
void DeserializerRelocInfoVisitor::VisitExternalReference(Code host, RelocInfo* rinfo)
{
    ...
    if (rinfo->IsCodedSpecially()) {
        ...
        Assembler::deserialization_set_special_target_at(location_of_branch_data, host, address);
    } else {
        WriteUnalignedValue(rinfo->target_address_address(), address);
    }
}

void DeserializerRelocInfoVisitor::VisitInternalReference(Code host, RelocInfo* rinfo)
{
    byte data = source().Get();
    ...
    DCHECK_LT(static_cast<unsigned>(target_offset), static_cast<unsigned>(host.raw_body_size()));
    Address target = host.entry() + target_offset;
    Assembler::deserialization_set_target_internal_reference_at(rinfo->pc(), target, rinfo->rmode());
}

void DeserializerRelocInfoVisitor::VisitOffHeapTarget(Code host, RelocInfo* rinfo)
{
...
template <typename IsolateT> template <typename SlotAccessor> int Deserializer<IsolateT>::ReadRepeatedObject(SlotAccessor slot_accessor, int repeat_count)
{
...
template <int byte_code_count, int expected> constexpr byte VerifyBytecodeCount(byte bytecode)
{
...
template <typename IsolateT> template <typename SlotAccessor> int Deserializer<IsolateT>::ReadSingleBytecodeData(byte data, SlotAccessor slot_accessor)
{
    ...
    case kRootArray: {
        ...
        Handle<HeapObject> heap_object = Handle<HeapObject>::cast(isolate()->root_handle(root_index));
    ...
    case kStartupObjectCache: {
        ...
        HeapObject heap_object = HeapObject::cast(main_thread_isolate()->startup_object_cache()->at(cache_index));
        return slot_accessor.Write(heap_object, GetAndResetNextReferenceType());
    }
    ...
    case kReadOnlyObjectCache: {
        ...
        // entry as a Handle backing?
        HeapObject heap_object = HeapObject::cast(isolate()->read_only_heap()->cached_read_only_object(cache_index));
        return slot_accessor.Write(heap_object, GetAndResetNextReferenceType());
    }
    ...
    case kSharedHeapObjectCache: {
        ...
        HeapObject heap_object = HeapObject::cast(main_thread_isolate()->shared_heap_object_cache()->at(cache_index));
    ...
    case kExternalReference: {
        ...
        return WriteExternalPointer(slot_accessor.external_pointer_slot(), address, tag);
    }
    ...
    case kRegisterPendingForwardRef: {
        HeapObjectReferenceType ref_type = GetAndResetNextReferenceType();
        unresolved_forward_refs_.emplace_back(slot_accessor.object(), slot_accessor.offset(), ref_type);
    ...
    case kResolvePendingForwardRef: {
        ...
        SlotAccessorForHeapObject::ForSlotOffset(forward_ref.object, forward_ref.offset).Write(*obj, forward_ref.ref_type);
        num_unresolved_forward_refs_--;
    ...
    case kCodeBody: {
...

        {
            ...
            source_.CopyRaw(reinterpret_cast<void*>(code.address() + Code::kDataStart), size_in_bytes);
        }

        // Then deserialize the code header
        ReadData(slot_accessor.object(), HeapObject::kHeaderSize / kTaggedSize, Code::kDataStart / kTaggedSize);
        ...
        // in the preserialized_objects vector.
        {
            ...
            for (RelocIterator it(code, Code::BodyDescriptor::kRelocModeMask); !it.done(); it.next()) {
                it.rinfo()->Visit(&visitor);
            }
        }

        // Advance to the end of the code object.
        return (int { Code::kDataStart } - HeapObject::kHeaderSize) / kTaggedSize + size_in_tagged;
    }
    ...
    case kOffHeapResizableBackingStore: {
        ...
        if (data == kOffHeapBackingStore) {
            ...
            Maybe<bool> result
                = JSArrayBuffer::GetResizableBackingStorePageConfiguration(nullptr, byte_length, max_byte_length, kDontThrow, &page_size, &initial_pages, &max_pages);
...
template <typename IsolateT> HeapObject Deserializer<IsolateT>::Allocate(AllocationType allocation, int size, AllocationAlignment alignment)
{
#ifdef DEBUG
    ...
    HeapObject obj = HeapObject::FromAddress(isolate()->heap()->AllocateRawOrFail(size, allocation, AllocationOrigin::kRuntime, alignment));
