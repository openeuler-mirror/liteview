...
#if defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_PPC) || defined(V8_TARGET_ARCH_S390) || defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_RISCV32)              \
...
protected:
    ...
    Deserializer(IsolateT* isolate, base::Vector<const byte> payload, uint32_t magic_number, bool deserializing_user_code, bool can_rehash);
    ...
    void AddAttachedObject(Handle<HeapObject> attached_object) { attached_objects_.push_back(attached_object); }
    ...
    const std::vector<Handle<AllocationSite>>& new_allocation_sites() const { return new_allocation_sites_; }
    const std::vector<Handle<Code>>& new_code_objects() const { return new_code_objects_; }
    const std::vector<Handle<Map>>& new_maps() const { return new_maps_; }
    const std::vector<Handle<AccessorInfo>>& accessor_infos() const { return accessor_infos_; }
    const std::vector<Handle<CallHandlerInfo>>& call_handler_infos() const { return call_handler_infos_; }
    const std::vector<Handle<Script>>& new_scripts() const { return new_scripts_; }

    const std::vector<Handle<DescriptorArray>>& new_descriptor_arrays() const { return new_descriptor_arrays_; }
...
private:
    ...
    template <typename TSlot> inline int WriteAddress(TSlot dest, Address value);

    inline int WriteExternalPointer(ExternalPointerSlot dest, Address value, ExternalPointerTag tag);
    ...
    template <typename SlotAccessor> int ReadSingleBytecodeData(byte data, SlotAccessor slot_accessor);
    ...
    template <typename SlotGetter> int ReadRepeatedObject(SlotGetter slot_getter, int repeat_count);

    // Special handling for serialized code like hooking up internalized strings.
    void PostProcessNewObject(Handle<Map> map, Handle<HeapObject> obj, SnapshotSpace space);
    void PostProcessNewJSReceiver(Map map, Handle<JSReceiver> obj, InstanceType instance_type, SnapshotSpace space);

    HeapObject Allocate(AllocationType allocation, int size, AllocationAlignment alignment);
    ...
    struct UnresolvedForwardRef {
        UnresolvedForwardRef(Handle<HeapObject> object, int offset, HeapObjectReferenceType ref_type)
            : object(object)
            , offset(offset)
            , ref_type(ref_type)
        {
...
extern template class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Deserializer<Isolate>;
extern template class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Deserializer<LocalIsolate>;
...
public:
    explicit StringTableInsertionKey(Isolate* isolate, Handle<String> string, DeserializingUserCodeOption deserializing_user_code);
    explicit StringTableInsertionKey(LocalIsolate* isolate, Handle<String> string, DeserializingUserCodeOption deserializing_user_code);

    template <typename IsolateT> bool IsMatch(IsolateT* isolate, String string);
    ...
    V8_WARN_UNUSED_RESULT Handle<String> GetHandleForInsertion() { return string_; }
