...
bool SharedHeapSerializer::CanBeInSharedOldSpace(HeapObject obj)
{
    if (ReadOnlyHeap::Contains(obj))
        return false;
    if (obj.IsString()) {
        return obj.IsInternalizedString() || String::IsInPlaceInternalizable(String::cast(obj));
    }
    return false;
}
...
SharedHeapSerializer::SharedHeapSerializer(Isolate* isolate, Snapshot::SerializerFlags flags, ReadOnlySerializer* read_only_serializer)
...
SharedHeapSerializer::~SharedHeapSerializer() { OutputStatistics("SharedHeapSerializer"); }
...
bool SharedHeapSerializer::SerializeUsingReadOnlyObjectCache(SnapshotByteSink* sink, Handle<HeapObject> obj)
{
...
bool SharedHeapSerializer::SerializeUsingSharedHeapObjectCache(SnapshotByteSink* sink, Handle<HeapObject> obj)
{
    ...
    if (ShouldReconstructSharedHeapObjectCacheForTesting()) {
        std::vector<Object>* existing_cache = isolate()->shared_heap_isolate()->shared_heap_object_cache();
...
void SharedHeapSerializer::SerializeStringTable(StringTable* string_table)
{
    ...
    sink_.PutInt(string_table->NumberOfElements(), "String table number of elements");
...
bool SharedHeapSerializer::ShouldReconstructSharedHeapObjectCacheForTesting() const
{
    ...
    // shared.
    return reconstruct_read_only_and_shared_object_caches_for_testing() && isolate()->has_shared_heap();
}

void SharedHeapSerializer::ReconstructSharedHeapObjectCacheForTesting()
{
    std::vector<Object>* cache = isolate()->shared_heap_isolate()->shared_heap_object_cache();
