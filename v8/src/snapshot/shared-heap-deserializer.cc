...
void SharedHeapDeserializer::DeserializeStringTable()
{
    ...
    for (int i = 0; i < string_table_size; ++i) {
        ...
        StringTableInsertionKey key(isolate(), string, DeserializingUserCodeOption::kNotDeserializingUserCode);
        Handle<String> result = isolate()->string_table()->LookupKey(isolate(), &key);
