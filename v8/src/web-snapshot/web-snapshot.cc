...
void WebSnapshotSerializerDeserializer::Throw(const char* message)
{
    ...
    if (!isolate_->has_pending_exception()) {
        isolate_->Throw(*factory()->NewError(MessageTemplate::kWebSnapshotError, factory()->NewStringFromAsciiChecked(error_message_)));
    }
}

void WebSnapshotSerializerDeserializer::IterateBuiltinObjects(std::function<void(Handle<String>, Handle<HeapObject>)> func)
{
    // TODO(v8:11525): Add more builtins.
    auto roots = ReadOnlyRoots(isolate_);

    func(handle(roots.Error_string(), isolate_), handle(isolate_->context().error_function(), isolate_));

    func(factory()->NewStringFromAsciiChecked("Error.prototype"), handle(isolate_->context().error_function().instance_prototype(), isolate_));

    func(handle(roots.Object_string(), isolate_), handle(isolate_->context().object_function(), isolate_));

    func(factory()->NewStringFromAsciiChecked("Object.prototype"), handle(isolate_->context().initial_object_prototype(), isolate_));

    func(handle(roots.Function_string(), isolate_), handle(isolate_->context().function_function(), isolate_));

    func(factory()->NewStringFromAsciiChecked("Function.prototype"), handle(isolate_->context().function_prototype(), isolate_));

    // TODO(v8:11525): There are no obvious names for these, since AsyncFunction
    // etc are not properties of the global object.

    func(factory()->NewStringFromAsciiChecked("AsyncFunction"), handle(isolate_->context().async_function_constructor(), isolate_));

    func(factory()->NewStringFromAsciiChecked("AsyncFunction"), handle(isolate_->context().async_function_constructor().instance_prototype(), isolate_));

    auto generator_function = handle(JSFunction::cast(isolate_->context().generator_function_map().constructor_or_back_pointer()), isolate_);
    func(factory()->NewStringFromAsciiChecked("GeneratorFunction"), generator_function);

    func(factory()->NewStringFromAsciiChecked("GeneratorFunction.prototype"), handle(generator_function->instance_prototype(), isolate_));

    auto async_generator_function = handle(JSFunction::cast(isolate_->context().async_generator_function_map().constructor_or_back_pointer()), isolate_);
    func(factory()->NewStringFromAsciiChecked("AsyncGeneratorFunction"), async_generator_function);

    func(factory()->NewStringFromAsciiChecked("AsyncGeneratorFunction.prototype"), handle(async_generator_function->instance_prototype(), isolate_));

    static_assert(kBuiltinObjectCount == 12);
}

uint8_t WebSnapshotSerializerDeserializer::FunctionKindToFunctionFlags(FunctionKind kind)
{
    ...
    auto flags = AsyncFunctionBitField::encode(IsAsyncFunction(kind)) | GeneratorFunctionBitField::encode(IsGeneratorFunction(kind))
        | ArrowFunctionBitField::encode(IsArrowFunction(kind)) | MethodBitField::encode(IsConciseMethod(kind)) | StaticBitField::encode(IsStatic(kind))
        | ClassConstructorBitField::encode(IsClassConstructor(kind)) | DefaultConstructorBitField::encode(IsDefaultConstructor(kind))
        | DerivedConstructorBitField::encode(IsDerivedConstructor(kind));
    return flags;
}

// TODO(v8:11525): Optionally, use an enum instead.
FunctionKind WebSnapshotSerializerDeserializer::FunctionFlagsToFunctionKind(uint8_t flags)
{
    FunctionKind kind;
    if (IsFunctionOrMethod(flags)) {
        if (ArrowFunctionBitField::decode(flags) && MethodBitField::decode(flags)) {
            ...
            uint32_t index = AsyncFunctionBitField::decode(flags) << 0 | GeneratorFunctionBitField::decode(flags) << 1
                | (ArrowFunctionBitField::decode(flags) || StaticBitField::decode(flags)) << 2 | MethodBitField::decode(flags) << 3;
...
{
    uint32_t mask = AsyncFunctionBitField::kMask | GeneratorFunctionBitField::kMask | ArrowFunctionBitField::kMask | MethodBitField::kMask | StaticBitField::kMask;
    return (flags & mask) == flags;
}

bool WebSnapshotSerializerDeserializer::IsConstructor(uint8_t flags)
{
    uint32_t mask = ClassConstructorBitField::kMask | DefaultConstructorBitField::kMask | DerivedConstructorBitField::kMask;
...
{
    auto flags = ReadOnlyBitField::encode(false) | ConfigurableBitField::encode(true) | EnumerableBitField::encode(true);
    return flags;
}

uint8_t WebSnapshotSerializerDeserializer::AttributesToFlags(PropertyDetails details)
{
    auto flags = ReadOnlyBitField::encode(details.IsReadOnly()) | ConfigurableBitField::encode(details.IsConfigurable()) | EnumerableBitField::encode(details.IsEnumerable());
    return flags;
}

PropertyAttributes WebSnapshotSerializerDeserializer::FlagsToAttributes(uint8_t flags)
{
    int attributes = ReadOnlyBitField::decode(flags) * READ_ONLY + !ConfigurableBitField::decode(flags) * DONT_DELETE + !EnumerableBitField::decode(flags) * DONT_ENUM;
...
bool WebSnapshotSerializer::TakeSnapshot(Handle<Object> object, MaybeHandle<FixedArray> maybe_externals, WebSnapshotData& data_out)
{
    ...
    v8::Local<v8::Context> context = reinterpret_cast<v8::Isolate*>(isolate_)->GetCurrentContext();
    ShallowDiscoverBuiltinObjects(context);

    if (object->IsHeapObject())
        Discover(Handle<HeapObject>::cast(object));
...
bool WebSnapshotSerializer::TakeSnapshot(v8::Local<v8::Context> context, v8::Local<v8::PrimitiveArray> exports, WebSnapshotData& data_out)
{
    if (string_ids_.size() > 0) {
        Throw("Can't reuse WebSnapshotSerializer");
        return false;
    ...
    Handle<FixedArray> export_objects = isolate_->factory()->NewFixedArray(exports->Length());
    for (int i = 0, length = exports->Length(); i < length; ++i) {
        v8::Local<v8::String> str = exports->Get(v8_isolate, i)->ToString(context).ToLocalChecked();
        if (str->Length() == 0) {
            continue;
        }
        ...
        if (script_result.IsEmpty() || !script_result.ToLocalChecked()->ToObject(context).ToLocal(&v8_object)) {
    ...
    for (int i = 0, length = exports->Length(); i < length; ++i) {
        ...
        SerializeExport(handle(export_objects->get(i), isolate_), Handle<String>::cast(Utils::OpenHandle(*str)));
    }
...
void WebSnapshotSerializer::SerializePendingItems()
{
    ...
    for (int i = 0; i < contexts_->Length(); ++i) {
        Handle<Context> context = handle(Context::cast(contexts_->Get(i)), isolate_);
    ...
    for (int i = functions_->Length() - 1; i >= 0; --i) {
        Handle<JSFunction> function = handle(JSFunction::cast(functions_->Get(i)), isolate_);
        SerializeFunction(function);
    }
    for (int i = classes_->Length() - 1; i >= 0; --i) {
        Handle<JSFunction> function = handle(JSFunction::cast(classes_->Get(i)), isolate_);
    ...
    for (int i = array_buffers_->Length() - 1; i >= 0; --i) {
        Handle<JSArrayBuffer> array_buffer = handle(JSArrayBuffer::cast(array_buffers_->Get(i)), isolate_);
        SerializeArrayBuffer(array_buffer);
    }
    for (int i = typed_arrays_->Length() - 1; i >= 0; --i) {
        Handle<JSTypedArray> typed_array = handle(JSTypedArray::cast(typed_arrays_->Get(i)), isolate_);
        SerializeTypedArray(typed_array);
    }
    for (int i = data_views_->Length() - 1; i >= 0; --i) {
        Handle<JSDataView> data_view = handle(JSDataView::cast(data_views_->Get(i)), isolate_);
        SerializeDataView(data_view);
    }
    for (int i = objects_->Length() - 1; i >= 0; --i) {
        Handle<JSObject> object = handle(JSObject::cast(objects_->Get(i)), isolate_);
...
// - For each export:
//   - Serialized export
void WebSnapshotSerializer::WriteSnapshot(uint8_t*& buffer, size_t& buffer_size)
{
    if (has_error()) {
        return;
    }
    ...
    size_t needed_size = sizeof(kMagicNumber) + string_serializer_.buffer_size_ + symbol_serializer_.buffer_size_ + bigint_serializer_.buffer_size_
        + builtin_object_serializer_.buffer_size_ + map_serializer_.buffer_size_ + context_serializer_.buffer_size_ + function_serializer_.buffer_size_
        + class_serializer_.buffer_size_ + array_serializer_.buffer_size_ + array_buffer_serializer_.buffer_size_ + typed_array_serializer_.buffer_size_
        + data_view_serializer_.buffer_size_ + object_serializer_.buffer_size_ + export_serializer_.buffer_size_ + 14 * sizeof(uint32_t);
    ...
    WriteObjects(total_serializer, builtin_object_count(), builtin_object_serializer_, "builtin_objects");
    WriteObjects(total_serializer, map_count(), map_serializer_, "maps");
    WriteObjects(total_serializer, context_count(), context_serializer_, "contexts");
    WriteObjects(total_serializer, function_count(), function_serializer_, "functions");
    WriteObjects(total_serializer, array_count(), array_serializer_, "arrays");
    WriteObjects(total_serializer, array_buffer_count(), array_buffer_serializer_, "array buffers");
    WriteObjects(total_serializer, typed_array_count(), typed_array_serializer_, "typed arrays");
    WriteObjects(total_serializer, data_view_count(), data_view_serializer_, "data views");
...
void WebSnapshotSerializer::WriteObjects(ValueSerializer& destination, size_t count, ValueSerializer& source, const char* name)
{
...
bool WebSnapshotSerializer::InsertIntoIndexMap(ObjectCacheIndexMap& map, HeapObject heap_object, uint32_t& id)
{
...
// - Length
// - Raw bytes (data)
void WebSnapshotSerializer::SerializeString(Handle<String> string, ValueSerializer& serializer)
{
    DisallowGarbageCollection no_gc;
...
void WebSnapshotSerializer::SerializeSymbol(Handle<Symbol> symbol)
{
    if (symbol->description().IsUndefined()) {
        ...
        symbol_serializer_.WriteUint32(symbol->is_in_public_symbol_table() ? SymbolType::kGlobal : SymbolType::kNonGlobal);
        WriteStringId(handle(String::cast(symbol->description()), isolate_), symbol_serializer_);
    }
}
...
void WebSnapshotSerializer::SerializeMap(Handle<Map> map)
{
    ...
    for (InternalIndex i : map->IterateOwnDescriptors()) {
        PropertyDetails details = map->instance_descriptors(kRelaxedLoad).GetDetails(i);
        ...
        Handle<Name> key(map->instance_descriptors(kRelaxedLoad).GetKey(i), isolate_);
        if (!ShouldBeSerialized(key)) {
            continue;
        }
        keys.push_back(key);
        if (first_custom_index >= 0 || details.IsReadOnly() || !details.IsConfigurable() || details.IsDontEnum()) {
            if (first_custom_index == -1)
                first_custom_index = i.as_int();
            attributes.push_back(AttributesToFlags(details));
        }
    }

    map_serializer_.WriteUint32(first_custom_index == -1 ? PropertyAttributesType::DEFAULT : PropertyAttributesType::CUSTOM);
...
void WebSnapshotSerializer::SerializeBuiltinObject(uint32_t name_id) { builtin_object_serializer_.WriteUint32(name_id); }
...
void WebSnapshotSerializer::ConstructSource()
{
    ...
    for (const auto& interval : source_intervals_) {
        ...
        if (interval.second <= current_interval_end) {
            ...
            source_offset_to_compacted_source_offset_[interval.first] = source_offset_to_compacted_source_offset_[current_interval_start] + offset_within_parent;
            continue;
        }
        // Start a new interval.
        current_interval_start = interval.first;
        current_interval_end = interval.second;
        source_offset_to_compacted_source_offset_[current_interval_start] = source_string->length();
        MaybeHandle<String> new_source_string = factory()->NewConsString(source_string, factory()->NewSubString(full_source_, current_interval_start, current_interval_end));
...
void WebSnapshotSerializer::SerializeFunctionProperties(Handle<JSFunction> function, ValueSerializer& serializer)
{
    Handle<Map> map(function->map(), isolate_);
    if (function->map() == isolate_->context().get(function->shared().function_map_index())) {
    ...
    for (InternalIndex i : map->IterateOwnDescriptors()) {
        ...
        if (!ShouldBeSerialized(handle(map->instance_descriptors().GetKey(i), isolate_))) {
            continue;
        }
        FieldIndex field_index = FieldIndex::ForDescriptor(*map, i);
        Handle<Object> value = JSObject::FastPropertyAt(isolate_, function, details.representation(), field_index);
        WriteValue(value, serializer);
    }
}

void WebSnapshotSerializer::SerializeFunctionInfo(Handle<JSFunction> function, ValueSerializer& serializer)
{
    ...
    Handle<Script> script = handle(Script::cast(function->shared().script()), isolate_);
    int start = function->shared().StartPosition();
    int end = function->shared().EndPosition();
    int final_offset = source_offset_to_compacted_source_offset_[script_offsets_[script->id()] + start];
    serializer.WriteUint32(final_offset);
    serializer.WriteUint32(end - start);

    serializer.WriteUint32(function->shared().internal_formal_parameter_count_without_receiver());
...
void WebSnapshotSerializer::ShallowDiscoverExternals(FixedArray externals)
{
    ...
    for (int i = 0; i < externals.length(); i++) {
        ...
        InsertIntoIndexMap(external_object_ids_, HeapObject::cast(object), unused_id);
    }
}

void WebSnapshotSerializer::ShallowDiscoverBuiltinObjects(v8::Local<v8::Context> context)
{
    ...
    builtin_object_name_strings_ = isolate_->factory()->NewFixedArray(kBuiltinObjectCount);

    int i = 0;
    IterateBuiltinObjects([&](Handle<String> name, Handle<HeapObject> object) {
        builtin_object_name_strings_->set(i, *name);
        uint32_t id;
        bool already_exists = InsertIntoIndexMap(builtin_object_to_name_, *object, id);
...
void WebSnapshotSerializer::Discover(Handle<HeapObject> start_object)
{
    ...
    while (!discovery_queue_.empty()) {
        ...
        case JS_PRIMITIVE_WRAPPER_TYPE: {
            Handle<JSPrimitiveWrapper> wrapper = Handle<JSPrimitiveWrapper>::cast(object);
        ...
        case JS_REG_EXP_TYPE: {
            ...
            Handle<String> flags_string = JSRegExp::StringFromFlags(isolate_, regexp->flags());
            DiscoverString(flags_string);
            break;
        }
        case JS_ARRAY_BUFFER_TYPE: {
            Handle<JSArrayBuffer> array_buffer = Handle<JSArrayBuffer>::cast(object);
...
void WebSnapshotSerializer::DiscoverMap(Handle<Map> map, bool allow_property_in_descriptor)
{
...
void WebSnapshotSerializer::DiscoverString(Handle<String> string, AllowInPlace can_be_in_place)
{
...
void WebSnapshotSerializer::DiscoverMapForFunction(Handle<JSFunction> function)
{
...
void WebSnapshotSerializer::DiscoverContextAndPrototype(Handle<JSFunction> function)
{
    ...
    if (function->has_prototype_slot() && function->map().has_non_instance_prototype()) {
        Throw("Functions with non-instance prototypes not supported");
        return;
    }

    if (function->has_prototype_slot() && function->has_instance_prototype()) {
        Handle<JSObject> prototype = Handle<JSObject>::cast(handle(function->instance_prototype(), isolate_));
...
void WebSnapshotSerializer::DiscoverContext(Handle<Context> context)
{
    ...
    if (!context->previous().IsNativeContext() && !context->previous().IsScriptContext()) {
    ...
    for (auto it : ScopeInfo::IterateLocalNames(scope_info)) {
        ...
        Object value = context->get(scope_info->ContextHeaderLength() + it->index());
        if (!value.IsHeapObject())
            continue;
...
void WebSnapshotSerializer::DiscoverSource(Handle<JSFunction> function)
{
    ...
    Handle<Script> script = handle(Script::cast(function->shared().script()), isolate_);
    Handle<String> function_script_source = handle(String::cast(script->source()), isolate_);
    ...
    if (full_source_.is_null()) {
        ...
        if (it == script_offsets_.end()) {
            ...
            script_offsets_.insert({ script->id(), script_offset_int });
            if (!factory()->NewConsString(full_source_, function_script_source).ToHandle(&full_source_)) {
    ...
    source_intervals_.emplace(script_offset_int + function->shared().StartPosition(), script_offset_int + function->shared().EndPosition());
}
...
void WebSnapshotSerializer::DiscoverArrayBuffer(Handle<JSArrayBuffer> array_buffer)
{
...
void WebSnapshotSerializer::DiscoverTypedArray(Handle<JSTypedArray> typed_array)
{
    uint32_t id;
...
template <typename T> void WebSnapshotSerializer::DiscoverObjectPropertiesWithDictionaryMap(T dict)
{
    DisallowGarbageCollection no_gc;
...
void WebSnapshotSerializer::DiscoverObject(Handle<JSObject> object)
{
    ...
    if (object->HasFastProperties()) {
        ...
        for (InternalIndex i : map->IterateOwnDescriptors()) {
            PropertyDetails details = map->instance_descriptors(kRelaxedLoad).GetDetails(i);
            FieldIndex field_index = FieldIndex::ForDescriptor(*map, i);
            Handle<Object> value = JSObject::FastPropertyAt(isolate_, object, details.representation(), field_index);
            if (!value->IsHeapObject())
                continue;
            discovery_queue_.push(Handle<HeapObject>::cast(value));
        }
        ...
        if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
            Handle<SwissNameDictionary> swiss_dictionary = handle(object->property_dictionary_swiss(), isolate_);
            DiscoverObjectPropertiesWithDictionaryMap(swiss_dictionary);
        } else {
            Handle<NameDictionary> dictionary = handle(object->property_dictionary(), isolate_);
...
bool WebSnapshotSerializer::DiscoverIfBuiltinObject(Handle<HeapObject> object)
{
    ...
    Handle<String> name_string = handle(String::cast(builtin_object_name_strings_->get(name_index)), isolate_);
...
void WebSnapshotSerializer::DiscoverSymbol(Handle<Symbol> symbol)
{
    ...
    uint32_t id;
    if (InsertIntoIndexMap(symbol_ids_, *symbol, id))
        return;
...
{
    uint32_t id;
    if (InsertIntoIndexMap(bigint_ids_, *bigint, id))
        return;
...
void WebSnapshotSerializer::SerializeFunction(Handle<JSFunction> function)
{
    ...
    WriteValue(handle(function->map().prototype(), isolate_), function_serializer_);
}
...
// - For each variable:
//   - Serialized value
void WebSnapshotSerializer::SerializeContext(Handle<Context> context, uint32_t id)
{
    ...
    for (auto it : ScopeInfo::IterateLocalNames(scope_info)) {
        Handle<Object> value(context->get(scope_info->ContextHeaderLength() + it->index()), isolate_);
        WriteValue(value, context_serializer_);
    }
}

template <typename T> void WebSnapshotSerializer::SerializeObjectPropertiesWithDictionaryMap(T dict)
{
    DisallowGarbageCollection no_gc;
    ...
    for (InternalIndex index : dict->IterateEntries()) {
        ...
        if (first_custom_index >= 0 || details.IsReadOnly() || !details.IsConfigurable() || details.IsDontEnum()) {
            if (first_custom_index == -1)
                first_custom_index = index.as_int();
            attributes.push_back(AttributesToFlags(details));
        }
    }
    object_serializer_.WriteUint32(first_custom_index == -1 ? PropertyAttributesType::DEFAULT : PropertyAttributesType::CUSTOM);
    ...
    for (InternalIndex index : dict->IterateEntries()) {
        ...
        if (first_custom_index >= 0) {
            if (index.as_int() < first_custom_index) {
                ...
                object_serializer_.WriteByte(attributes[index.as_int() - first_custom_index]);
            }
        }
    }
}
...
void WebSnapshotSerializer::SerializeObject(Handle<JSObject> object)
{
    ...
    if (!IsDictionaryElementsKind(kind)) {
        ...
        if (IsHoleyElementsKindForRead(kind)) {
            ...
            return SerializeElements(object, object_serializer_, Just(max_element_index));
        } else {
            return SerializeElements(object, object_serializer_, Just(elements_length));
        }
    }
...
void WebSnapshotSerializer::SerializeElements(Handle<JSObject> object, ValueSerializer& serializer, Maybe<uint32_t> length)
{
    // TODO(v8:11525): Handle sealed & frozen elements correctly. (Also: handle
    // sealed & frozen objects.)
    ...
    case HOLEY_SEALED_ELEMENTS: {
        ...
        Handle<FixedArray> elements = handle(FixedArray::cast(object->elements()), isolate_);
    ...
    case HOLEY_DOUBLE_ELEMENTS: {
        serializer.WriteUint32(ElementsType::kDense);
        Handle<FixedDoubleArray> elements = handle(FixedDoubleArray::cast(object->elements()), isolate_);
        serializer.WriteUint32(length.ToChecked());
        for (uint32_t i = 0; i < length.ToChecked(); ++i) {
            if (!elements->is_the_hole(i)) {
                double double_value = elements->get_scalar(i);
                Handle<Object> element_value = isolate_->factory()->NewNumber(double_value);
...
uint8_t WebSnapshotSerializerDeserializer::ArrayBufferKindToFlags(Handle<JSArrayBuffer> array_buffer)
{
    return DetachedBitField::encode(array_buffer->was_detached()) | SharedBitField::encode(array_buffer->is_shared())
        | ResizableBitField::encode(array_buffer->is_resizable_by_js());
}

uint32_t WebSnapshotSerializerDeserializer::BigIntSignAndLengthToFlags(Handle<BigInt> bigint)
{
    ...
    return BigIntSignBitField::encode(sign) | BigIntLengthBitField::encode(byte_length);
}

uint32_t WebSnapshotSerializerDeserializer::BigIntFlagsToBitField(uint32_t flags)
{
    int byte_length = BigIntLengthBitField::decode(flags);
    int sign = BigIntSignBitField::decode(flags);
    return BigInt::SignBits::encode(sign) | BigInt::LengthBits::encode(byte_length);
}
...
//   - Max byte length
// - Raw bytes
void WebSnapshotSerializer::SerializeArrayBuffer(Handle<JSArrayBuffer> array_buffer)
{
    ...
    if (array_buffer->is_resizable_by_js()) {
        ...
        array_buffer_serializer_.WriteUint32(static_cast<uint32_t>(array_buffer->max_byte_length()));
    }
    array_buffer_serializer_.WriteRawBytes(array_buffer->backing_store(), byte_length);
}

uint8_t WebSnapshotSerializerDeserializer::ArrayBufferViewKindToFlags(Handle<JSArrayBufferView> array_buffer_view)
{
    return LengthTrackingBitField::encode(array_buffer_view->is_length_tracking());
}

// static
ExternalArrayType WebSnapshotSerializerDeserializer::TypedArrayTypeToExternalArrayType(TypedArrayType type)
{
    switch (type) {
...
// static
WebSnapshotSerializerDeserializer::TypedArrayType WebSnapshotSerializerDeserializer::ExternalArrayTypeToTypedArrayType(ExternalArrayType type)
{
    switch (type) {
#define TYPED_ARRAY_CASE(Type, type, TYPE, ctype)                                                                                                                                  \
...
//   - Byte length
void WebSnapshotSerializer::SerializeArrayBufferView(Handle<JSArrayBufferView> array_buffer_view, ValueSerializer& serializer)
{
    ...
    serializer.WriteUint32(static_cast<uint32_t>(array_buffer_view->byte_offset()));
    ...
    if (!array_buffer_view->is_length_tracking()) {
        serializer.WriteUint32(static_cast<uint32_t>(array_buffer_view->byte_length()));
    }
}
...
// - TypedArrayType
// - ArrayBufferView
void WebSnapshotSerializer::SerializeTypedArray(Handle<JSTypedArray> typed_array)
{
    TypedArrayType typed_array_type = ExternalArrayTypeToTypedArrayType(typed_array->type());
...
// - ArrayBufferView
void WebSnapshotSerializer::SerializeDataView(Handle<JSDataView> data_view) { SerializeArrayBufferView(data_view, data_view_serializer_); }

// Format (serialized export):
// - String id (export name)
// - Serialized value (export value)
void WebSnapshotSerializer::SerializeExport(Handle<Object> object, Handle<String> export_name)
{
    ++export_count_;
...
void WebSnapshotSerializer::WriteValue(Handle<Object> object, ValueSerializer& serializer)
{
    if (object->IsSmi()) {
    ...
    case JS_ARRAY_BUFFER_TYPE: {
        Handle<JSArrayBuffer> array_buffer = Handle<JSArrayBuffer>::cast(heap_object);
        serializer.WriteByte(ValueType::ARRAY_BUFFER_ID);
        serializer.WriteUint32(GetArrayBufferId(*array_buffer));
        break;
    }
    case JS_TYPED_ARRAY_TYPE: {
        Handle<JSTypedArray> typed_array = Handle<JSTypedArray>::cast(heap_object);
...
void WebSnapshotSerializer::WriteStringMaybeInPlace(Handle<String> string, ValueSerializer& serializer)
{
...
void WebSnapshotSerializer::WriteStringId(Handle<String> string, ValueSerializer& serializer)
{
    bool in_place = false;
    uint32_t id = GetStringId(string, in_place);
...
uint32_t WebSnapshotSerializer::GetStringId(Handle<String> string, bool& in_place)
{
...
bool WebSnapshotSerializer::GetBuiltinObjectNameIndex(HeapObject object, uint32_t& index)
{
    int index_int = 0;
...
bool WebSnapshotSerializer::GetBuiltinObjectId(HeapObject object, uint32_t& id)
{
    int id_int;
...
Handle<FixedArray> WebSnapshotSerializer::GetExternals() { return external_object_ids_.Values(isolate_); }

WebSnapshotDeserializer::WebSnapshotDeserializer(v8::Isolate* isolate, const uint8_t* data, size_t buffer_size)
    : WebSnapshotDeserializer(reinterpret_cast<i::Isolate*>(isolate), Handle<Object>(), { data, buffer_size })
{
}

WebSnapshotDeserializer::WebSnapshotDeserializer(Isolate* isolate, Handle<Script> snapshot_as_script)
    : WebSnapshotSerializerDeserializer(isolate)
    , script_name_(handle(snapshot_as_script->name(), isolate_))
    , roots_(isolate)
{
    auto [data, buffer_size, buffer_owned] = ExtractScriptBuffer(isolate, snapshot_as_script);
...
WebSnapshotDeserializer::WebSnapshotDeserializer(Isolate* isolate, Handle<Object> script_name, base::Vector<const uint8_t> buffer)
    : WebSnapshotSerializerDeserializer(isolate)
    , script_name_(script_name)
    , deserializer_(new ValueDeserializer(isolate_, buffer.data(), buffer.length()))
    , roots_(isolate)
{
    ...
    isolate_->heap()->AddGCEpilogueCallback(UpdatePointersCallback, v8::kGCTypeAll, this);
}

WebSnapshotDeserializer::~WebSnapshotDeserializer() { isolate_->heap()->RemoveGCEpilogueCallback(UpdatePointersCallback, this); }
...
// static
std::tuple<const uint8_t*, uint32_t, bool> WebSnapshotDeserializer::ExtractScriptBuffer(Isolate* isolate, Handle<Script> snapshot_as_script)
{
    Handle<String> source = handle(String::cast(snapshot_as_script->source()), isolate);
    if (source->IsExternalOneByteString()) {
        const v8::String::ExternalOneByteStringResource* resource = ExternalOneByteString::cast(*source).resource();
        return std::make_tuple(reinterpret_cast<const uint8_t*>(resource->data()), resource->length(), false);
        ...
        const v8::String::ExternalStringResource* resource = ExternalTwoByteString::cast(*source).resource();
...
bool WebSnapshotDeserializer::Deserialize(MaybeHandle<FixedArray> external_references, bool skip_exports)
{
...
bool WebSnapshotDeserializer::DeserializeSnapshot(bool skip_exports)
{
    ...
    if (!deserializer_->ReadRawBytes(sizeof(kMagicNumber), &magic_bytes) || memcmp(magic_bytes, kMagicNumber, sizeof(kMagicNumber)) != 0) {
...
void WebSnapshotDeserializer::CollectBuiltinObjects()
{
    ...
    builtin_object_name_to_object_ = ObjectHashTable::New(isolate_, kBuiltinObjectCount);
#if DEBUG
    int i = 0;
#endif
    IterateBuiltinObjects([&](Handle<String> name, Handle<HeapObject> object) {
        auto new_builtin_object_name_to_object = ObjectHashTable::Put(builtin_object_name_to_object_, name, object);
        USE(new_builtin_object_name_to_object);
        // We preallocated the correct size, so the hash table doesn't grow.
        DCHECK_EQ(*new_builtin_object_name_to_object, *builtin_object_name_to_object_);
#if DEBUG
        ++i;
#endif
    });
...
bool WebSnapshotDeserializer::DeserializeScript()
{
    ...
    if (remaining_bytes > 0 && remaining_bytes < v8::String::kMaxLength) {
        ...
        v8::Local<v8::String> source
            = v8::String::NewFromUtf8(v8_isolate, reinterpret_cast<const char*>(deserializer_->position_), NewStringType::kNormal, static_cast<int>(remaining_bytes))
                  .ToLocalChecked();
        ...
        if (!ScriptCompiler::CompileUnboundScript(v8_isolate, &script_source).ToLocal(&script)) {
            // The exception has already been reported.
            DCHECK(!isolate_->has_pending_exception());
            return false;
        }
        Local<Value> result;
        if (!script->BindToCurrentContext()->Run(v8_isolate->GetCurrentContext()).ToLocal(&result)) {
            // The exception has already been reported.
            DCHECK(!isolate_->has_pending_exception());
            return false;
        }
...
void WebSnapshotDeserializer::DeserializeStrings()
{
    ...
    for (uint32_t i = 0; i < string_count_; ++i) {
        MaybeHandle<String> maybe_string = deserializer_->ReadUtf8String(AllocationType::kOld);
...
String WebSnapshotDeserializer::ReadString(InternalizeStrings internalize_strings)
{
    ...
    if (internalize_strings == InternalizeStrings::kYes && !string.IsInternalizedString(isolate_)) {
...
String WebSnapshotDeserializer::ReadInPlaceString(InternalizeStrings internalize_strings)
{
    MaybeHandle<String> maybe_string = deserializer_->ReadUtf8String(AllocationType::kOld);
    Handle<String> string;
    if (!maybe_string.ToHandle(&string)) {
...
void WebSnapshotDeserializer::DeserializeBigInts()
{
    ...
    for (uint32_t i = 0; i < bigint_count_; ++i) {
        ...
        if (!BigInt::FromSerializedDigits(isolate_, BigIntFlagsToBitField(flags), digits_storage).ToHandle(&bigint)) {
            Throw("malformed bigint");
            return;
        }
    ...
    for (uint32_t i = 0; i < symbol_count_; ++i) {
        ...
        if (symbol_type == SymbolType::kNonGlobalNoDesription) {
            ...
            if (symbol_type == SymbolType::kNonGlobal) {
                ...
                symbol = isolate_->SymbolFor(RootIndex::kPublicSymbolTable, handle(String::cast(strings_.get(string_id)), isolate_), false);
            }
        }
...
void WebSnapshotDeserializer::DeserializeMaps()
{
    ...
    for (uint32_t i = 0; i < map_count_; ++i) {
        ...
        Handle<DescriptorArray> descriptors = factory()->NewDescriptorArray(property_count, 0);
        for (InternalIndex i : InternalIndex::Range(property_count)) {
            // No deferred references here, since strings and symbols have already
            // been deserialized.
            Object key = std::get<0>(ReadValue(Handle<HeapObject>(), 0, InternalizeStrings::kYes));
            DisallowGarbageCollection no_gc;
            if (!key.IsName()) {
            ...
            Descriptor desc = Descriptor::DataField(isolate_, handle(Name::cast(key), isolate_), i.as_int(), attributes, Representation::None());
            descriptors->Set(i, &desc);
        }
        DCHECK_EQ(descriptors->number_of_descriptors(), property_count);
        descriptors->Sort();

        Handle<Map> map = factory()->NewMap(JS_OBJECT_TYPE, JSObject::kHeaderSize, HOLEY_ELEMENTS, 0);
...
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kWebSnapshotDeserialize_BuiltinObjects);
    ...
    for (uint32_t i = 0; i < builtin_object_count_; ++i) {
        ...
        builtin_objects_.set(static_cast<int>(i), builtin_object_name_to_object_->Lookup(name));
    }
}
...
void WebSnapshotDeserializer::DeserializeContexts()
{
    ...
    for (uint32_t i = 0; i < context_count_; ++i) {
        ...
        if (!deserializer_->ReadUint32(&parent_context_id) || parent_context_id > i) {
        ...
        const bool has_inlined_local_names = variable_count < kScopeInfoMaxInlinedLocalNamesSize;
        // TODO(v8:11525): Enforce upper limit for variable count.
        Handle<ScopeInfo> scope_info = CreateScopeInfo(variable_count, parent_context_id > 0, static_cast<ContextType>(context_type), has_inlined_local_names);

        Handle<Context> parent_context;
        if (parent_context_id > 0) {
            parent_context = handle(Context::cast(contexts_.get(parent_context_id - 1)), isolate_);
            scope_info->set_outer_scope_info(parent_context->scope_info());
        } else {
            parent_context = handle(isolate_->context(), isolate_);
        }

        const int local_names_container_size = has_inlined_local_names ? variable_count : 1;
        const int context_local_base = ScopeInfo::kVariablePartIndex;
        const int context_local_info_base = context_local_base + local_names_container_size;

        for (int variable_index = 0; variable_index < static_cast<int>(variable_count); ++variable_index) {
            {
                ...
                if (has_inlined_local_names) {
                    ...
                    Handle<NameToIndexHashTable> local_names_hashtable(scope_info->context_local_names_hashtable(), isolate_);

                    Handle<NameToIndexHashTable> new_table = NameToIndexHashTable::Add(isolate_, local_names_hashtable, handle(name, isolate_), variable_index);
            ...
            uint32_t info = ScopeInfo::VariableModeBits::encode(VariableMode::kLet) | ScopeInfo::InitFlagBit::encode(InitializationFlag::kNeedsInitialization)
                | ScopeInfo::MaybeAssignedFlagBit::encode(MaybeAssignedFlag::kMaybeAssigned) | ScopeInfo::ParameterNumberBits::encode(ScopeInfo::ParameterNumberBits::kMax)
                | ScopeInfo::IsStaticFlagBit::encode(IsStaticFlag::kNotStatic);
            scope_info->set(context_local_info_base + variable_index, Smi::FromInt(info));
        }
...
Handle<ScopeInfo> WebSnapshotDeserializer::CreateScopeInfo(uint32_t variable_count, bool has_parent, ContextType context_type, bool has_inlined_local_names)
{
    ...
    int flags = ScopeInfo::SloppyEvalCanExtendVarsBit::encode(false) | ScopeInfo::LanguageModeBit::encode(LanguageMode::kStrict) | ScopeInfo::DeclarationScopeBit::encode(false)
        | ScopeInfo::ReceiverVariableBits::encode(VariableAllocationInfo::NONE) | ScopeInfo::ClassScopeHasPrivateBrandBit::encode(false)
        | ScopeInfo::HasSavedClassVariableBit::encode(false) | ScopeInfo::HasNewTargetBit::encode(false) | ScopeInfo::FunctionVariableBits::encode(VariableAllocationInfo::NONE)
        | ScopeInfo::HasInferredFunctionNameBit::encode(false) | ScopeInfo::IsAsmModuleBit::encode(false) | ScopeInfo::HasSimpleParametersBit::encode(false)
        | ScopeInfo::FunctionKindBits::encode(FunctionKind::kNormalFunction) | ScopeInfo::HasOuterScopeInfoBit::encode(has_parent)
        | ScopeInfo::IsDebugEvaluateScopeBit::encode(false) | ScopeInfo::ForceContextAllocationBit::encode(false) | ScopeInfo::PrivateNameLookupSkipsOuterClassBit::encode(false)
        | ScopeInfo::HasContextExtensionSlotBit::encode(false) | ScopeInfo::IsReplModeScopeBit::encode(false) | ScopeInfo::HasLocalsBlockListBit::encode(false);
    switch (context_type) {
    case ContextType::FUNCTION:
        scope_type = ScopeType::FUNCTION_SCOPE;
        flags |= ScopeInfo::DeclarationScopeBit::encode(true) | ScopeInfo::HasSimpleParametersBit::encode(true);
        break;
    case ContextType::BLOCK:
    ...
    const int local_names_container_size = has_inlined_local_names ? variable_count : 1;
    const int length = ScopeInfo::kVariablePartIndex + (ScopeInfo::NeedsPositionInfo(scope_type) ? ScopeInfo::kPositionInfoEntries : 0) + (has_parent ? 1 : 0)
        + local_names_container_size + variable_count;
    Handle<NameToIndexHashTable> local_names_hashtable;
    if (!has_inlined_local_names) {
        local_names_hashtable = NameToIndexHashTable::New(isolate_, variable_count, AllocationType::kOld);
    }
...
Handle<JSFunction> WebSnapshotDeserializer::CreateJSFunction(
    int shared_function_info_index, uint32_t start_position, uint32_t length, uint32_t parameter_count, uint8_t flags, uint32_t context_id)
{
    // TODO(v8:11525): Deduplicate the SFIs for class methods.
    FunctionKind kind = FunctionFlagsToFunctionKind(flags);
    Handle<SharedFunctionInfo> shared = factory()->NewSharedFunctionInfo(factory()->empty_string(), MaybeHandle<Code>(), Builtin::kCompileLazy, kind);
    Handle<UncompiledData> uncompiled_data = factory()->NewUncompiledDataWithoutPreparseData(roots_.empty_string_handle(), start_position, start_position + length);
    {
        DisallowGarbageCollection no_gc;
        ...
        shared_function_infos_.Set(shared_function_info_index, HeapObjectReference::Weak(raw));
    }
    shared_function_info_table_
        = ObjectHashTable::Put(shared_function_info_table_, handle(Smi::FromInt(start_position), isolate_), handle(Smi::FromInt(shared_function_info_index), isolate_));

    Handle<JSFunction> function = Factory::JSFunctionBuilder(isolate_, shared, isolate_->native_context()).Build();
    if (context_id > 0) {
...
void WebSnapshotDeserializer::DeserializeFunctionProperties(Handle<JSFunction> function)
{
    uint32_t map_id;
    ...
    if (map_id > 0) {
        ...
        Handle<DescriptorArray> descriptors = handle(map->instance_descriptors(kRelaxedLoad), isolate_);
        Handle<PropertyArray> property_array = DeserializePropertyArray(descriptors, no_properties);
        ...
        if (iter != deserialized_function_maps_.end()) {
            ...
            Handle<Map> function_map = Map::Copy(isolate_, handle(function->map(), isolate_), "Web Snapshot");
            Map::EnsureDescriptorSlack(isolate_, function_map, descriptors->number_of_descriptors());
            {
                for (InternalIndex i : map->IterateOwnDescriptors()) {
                    Descriptor d = Descriptor::DataField(isolate_, handle(descriptors->GetKey(i), isolate_), descriptors->GetDetails(i).field_index(),
                        descriptors->GetDetails(i).attributes(), descriptors->GetDetails(i).representation());
                ...
                function_map->SetNumberOfOwnDescriptors(function_map->NumberOfOwnDescriptors() + descriptors->number_of_descriptors());
...
void WebSnapshotDeserializer::DeserializeFunctions()
{
    ...
    shared_function_infos_handle_ = factory()->NewWeakFixedArray(WeakArrayList::CapacityForLength(function_count_ + 1), AllocationType::kOld);
    ...
    for (; current_function_count_ < function_count_; ++current_function_count_) {
        ...
        uint8_t flags;
        if (!deserializer_->ReadUint32(&start_position) || !deserializer_->ReadUint32(&length) || !deserializer_->ReadUint32(&parameter_count)
        ...
        Handle<JSFunction> function = CreateJSFunction(current_function_count_ + 1, start_position, length, parameter_count, flags, context_id);
...
void WebSnapshotDeserializer::DeserializeClasses()
{
    ...
    shared_function_infos_handle_ = WeakFixedArray::EnsureSpace(isolate_, shared_function_infos_handle_, WeakArrayList::CapacityForLength(function_count_ + 1 + class_count_));
    shared_function_infos_ = *shared_function_infos_handle_;
    ...
    for (; current_class_count_ < class_count_; ++current_class_count_) {
        ...
        // Index 0 is reserved for top-level shared function info (which web
        // snapshot scripts don't have).
        Handle<JSFunction> function = CreateJSFunction(function_count_ + current_class_count_ + 1, start_position, length, parameter_count, flags, context_id);
...
bool WebSnapshotDeserializer::IsInitialFunctionPrototype(Object prototype)
{
    return prototype == isolate_->context().function_prototype() ||
        ...
        prototype == isolate_->context().async_function_constructor().instance_prototype() ||
        // GeneratorFunction prototype.
        prototype == JSFunction::cast(isolate_->context().generator_function_map().constructor_or_back_pointer()).instance_prototype() ||
        // AsyncGeneratorFunction prototype
        prototype == JSFunction::cast(isolate_->context().async_generator_function_map().constructor_or_back_pointer()).instance_prototype();
}

void WebSnapshotDeserializer::DeserializeObjectPrototypeForFunction(Handle<JSFunction> function)
{
    Handle<Map> map(function->map(), isolate_);
...
Handle<Map> WebSnapshotDeserializer::DeserializeObjectPrototypeAndCreateEmptyMap()
{
    Handle<Map> map = factory()->NewMap(JS_OBJECT_TYPE, JSObject::kHeaderSize, HOLEY_ELEMENTS, 0);
    auto result = ReadValue(map, 0, InternalizeStrings::kNo);
    Object prototype = std::get<0>(result);
    ...
    if (prototype == isolate_->context().initial_object_prototype()) {
        ...
        // other JS engines.
        return handle(isolate_->native_context()->object_function().initial_map(), isolate_);
    }
    if (!was_deferred) {
        SetPrototype(map, handle(prototype, isolate_));
    }
...
void WebSnapshotDeserializer::SetPrototype(Handle<Map> map, Handle<Object> prototype)
{
...
template <typename T> void WebSnapshotDeserializer::DeserializeObjectPropertiesWithDictionaryMap(T dict, uint32_t property_count, bool has_custom_property_attributes)
{
    for (uint32_t i = 0; i < property_count; i++) {
        Handle<Object> key(std::get<0>(ReadValue(Handle<HeapObject>(), 0, InternalizeStrings::kYes)), isolate_);
        if (!key->IsName()) {
            Throw("Invalid map key");
            return;
        }
        ...
        PropertyDetails details(PropertyKind::kData, attributes, PropertyDetails::kConstIfDictConstnessTracking);
        auto new_dict = dict->Add(isolate_, dict, Handle<Name>::cast(key), value, details);
...
Handle<PropertyArray> WebSnapshotDeserializer::DeserializePropertyArray(Handle<DescriptorArray> descriptors, int no_properties)
{
    Handle<PropertyArray> property_array = factory()->NewPropertyArray(no_properties);
...
void WebSnapshotDeserializer::DeserializeObjects()
{
    ...
    for (; current_object_count_ < object_count_; ++current_object_count_) {
        ...
        if (map_id > 0) {
            ...
            Handle<DescriptorArray> descriptors = handle(raw_map.instance_descriptors(kRelaxedLoad), isolate_);
            ...
            if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
                Handle<SwissNameDictionary> swiss_dictionary = isolate_->factory()->NewSwissNameDictionary(property_count);
                DeserializeObjectPropertiesWithDictionaryMap(swiss_dictionary, property_count, has_custom_property_attributes);
                object = factory()->NewJSObjectFromMap(map);
                object->SetProperties(*swiss_dictionary);
            } else {
                Handle<NameDictionary> dictionary = isolate_->factory()->NewNameDictionary(property_count);
                DeserializeObjectPropertiesWithDictionaryMap(dictionary, property_count, has_custom_property_attributes);
                object = factory()->NewJSObjectFromMap(map);
...
void WebSnapshotDeserializer::DeserializeObjectElements(Handle<JSObject> object, bool map_from_snapshot)
{
...
WebSnapshotDeserializer::ElementsType WebSnapshotDeserializer::ReadElementsType()
{
    ...
    if (elements_type != ElementsType::kDense && elements_type != ElementsType::kSparse) {
...
std::tuple<Handle<FixedArrayBase>, ElementsKind, uint32_t> WebSnapshotDeserializer::DeserializeElements()
{
    uint32_t length;
...
std::tuple<Handle<FixedArrayBase>, ElementsKind, uint32_t> WebSnapshotDeserializer::ReadDenseElements(uint32_t length)
{
    ...
    if (has_hole) {
        elements_kind = elements_kind == PACKED_ELEMENTS ? HOLEY_ELEMENTS : HOLEY_SMI_ELEMENTS;
    }
    ...
    if (!has_non_number && !IsSmiElementsKind(elements_kind)) {
        ...
        ElementsKind new_elements_kind = has_hole ? HOLEY_DOUBLE_ELEMENTS : PACKED_DOUBLE_ELEMENTS;
        Handle<FixedArrayBase> new_elements = isolate_->factory()->NewFixedDoubleArray(length);
        ElementsAccessor* element_accessor = ElementsAccessor::ForKind(new_elements_kind);
        element_accessor->CopyElements(isolate_, elements, elements_kind, new_elements, length);
        return std::make_tuple(new_elements, new_elements_kind, length);
    }
    return std::make_tuple(elements, elements_kind, length);
}

std::tuple<Handle<FixedArrayBase>, ElementsKind, uint32_t> WebSnapshotDeserializer::ReadSparseElements(uint32_t length)
{
    ...
    for (uint32_t i = 0; i < length; ++i) {
        ...
        Handle<NumberDictionary> new_dict = dict->Set(isolate_, dict, element_index, handle(value, isolate_));
...
void WebSnapshotDeserializer::DeserializeArrays()
{
    ...
    for (; current_array_count_ < array_count_; ++current_array_count_) {
        ...
        if (IsDictionaryElementsKind(elements_kind)) {
            ...
            Handle<Object> array_length = isolate_->factory()->NewNumberFromUint(length);
            Handle<Map> map = JSObject::GetElementsTransitionMap(array, DICTIONARY_ELEMENTS);
            array->set_length(*array_length);
            array->set_elements(*elements);
            array->set_map(*map, kReleaseStore);
        } else {
            array = factory()->NewJSArrayWithElements(elements, elements_kind, length);
        }
...
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kWebSnapshotDeserialize_ArrayBuffers);
    ...
    array_buffers_ = *array_buffers_handle_;
    for (; current_array_buffer_count_ < array_buffer_count_; ++current_array_buffer_count_) {
        uint8_t flags;
        uint32_t byte_length;
        if (!deserializer_->ReadByte(&flags) || !deserializer_->ReadUint32(&byte_length) || byte_length > static_cast<size_t>(deserializer_->end_ - deserializer_->position_)) {
            Throw("Malformed array buffer");
            return;
        }

        uint32_t mask = DetachedBitField::kMask | SharedBitField::kMask | ResizableBitField::kMask;
        if ((flags | mask) != mask) {
            Throw("Malformed array buffer");
            return;
        }
        ...
        SharedFlag shared = SharedBitField::decode(flags) ? SharedFlag::kShared : SharedFlag::kNotShared;
        CHECK_IMPLIES(was_detached, (shared == SharedFlag::kNotShared));
        ResizableFlag resizable = ResizableBitField::decode(flags) ? ResizableFlag::kResizable : ResizableFlag::kNotResizable;
        ...
        if (shared == SharedFlag::kNotShared) {
            map = handle(isolate_->raw_native_context().array_buffer_fun().initial_map(), isolate_);
        } else {
            map = handle(isolate_->raw_native_context().shared_array_buffer_fun().initial_map(), isolate_);
        }
        Handle<JSArrayBuffer> array_buffer = Handle<JSArrayBuffer>::cast(isolate_->factory()->NewJSObjectFromMap(map, AllocationType::kYoung));
        ...
        if (was_detached) {
            ...
            if (resizable == ResizableFlag::kNotResizable) {
                backing_store = BackingStore::Allocate(isolate_, byte_length, shared, InitializedFlag::kUninitialized);
            } else {
                size_t page_size, initial_pages, max_pages;
                if (JSArrayBuffer::GetResizableBackingStorePageConfiguration(isolate_, byte_length, max_byte_length, kThrowOnError, &page_size, &initial_pages, &max_pages)
                        .IsNothing()) {
                    Throw("Create array buffer failed");
                    return;
                }
                backing_store = BackingStore::TryAllocateAndPartiallyCommitMemory(
                    isolate_, byte_length, max_byte_length, page_size, initial_pages, max_pages, WasmMemoryFlag::kNotWasm, shared);
            }
            if (!backing_store) {
                Throw("Create array buffer failed");
                return;
        ...
        if (byte_length > 0) {
            memcpy(array_buffer->backing_store(), deserializer_->position_, byte_length);
        }
        deserializer_->position_ += byte_length;
        array_buffers_.set(static_cast<int>(current_array_buffer_count_), *array_buffer);
    }
}
...
void WebSnapshotDeserializer::DeserializeDataViews()
{
    ...
    data_views_ = *data_views_handle_;
    for (; current_data_view_count_ < data_view_count_; ++current_data_view_count_) {
        Handle<JSArrayBuffer> array_buffer(JSArrayBuffer::cast(std::get<0>(ReadValue())), isolate_);
        uint32_t byte_offset = 0;
        uint8_t flags = 0;
        if (!deserializer_->ReadByte(&flags) || !deserializer_->ReadUint32(&byte_offset)) {
            Throw("Malformed data view");
            return;
        }

        Handle<Map> map(isolate_->raw_native_context().data_view_fun().initial_map(), isolate_);
        ...
        Handle<JSDataView> data_view = Handle<JSDataView>::cast(factory()->NewJSArrayBufferView(map, factory()->empty_fixed_array(), array_buffer, byte_offset, byte_length));

        {
            DisallowGarbageCollection no_gc;
            JSDataView raw_data_view = *data_view;
            JSArrayBuffer raw_array_buffer = *array_buffer;
            raw_data_view.set_data_pointer(isolate_, static_cast<uint8_t*>(raw_array_buffer.backing_store()) + byte_offset);
            raw_data_view.set_is_length_tracking(is_length_tracking);
            raw_data_view.set_is_backed_by_rab(!raw_array_buffer.is_shared() && raw_array_buffer.is_resizable_by_js());
        }
...
bool WebSnapshotDeserializer::ReadCount(uint32_t& count) { return deserializer_->ReadUint32(&count) && count <= kMaxItemCount; }

void WebSnapshotDeserializer::DeserializeTypedArrays()
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kWebSnapshotDeserialize_TypedArrays);
    ...
    typed_arrays_ = *typed_arrays_handle_;
    for (; current_typed_array_count_ < typed_array_count_; ++current_typed_array_count_) {
        ...
        JSTypedArray::ForFixedTypedArray(TypedArrayTypeToExternalArrayType(static_cast<TypedArrayType>(typed_array_type)), &element_size, &element_kind);

        Handle<Map> map(isolate_->raw_native_context().TypedArrayElementsKindToCtorMap(element_kind), isolate_);
        uint32_t mask = LengthTrackingBitField::kMask;
        if ((flags | mask) != mask) {
            Throw("Malformed typed array");
            return;
        ...
        bool rabGsab = array_buffer->is_resizable_by_js() && (!array_buffer->is_shared() || is_length_tracking);
        if (rabGsab) {
            map = handle(isolate_->raw_native_context().TypedArrayElementsKindToRabGsabCtorMap(element_kind), isolate_);
        }

        Handle<JSTypedArray> typed_array = Handle<JSTypedArray>::cast(factory()->NewJSArrayBufferView(map, factory()->empty_byte_array(), array_buffer, byte_offset, byte_length));

        {
            DisallowGarbageCollection no_gc;
            JSTypedArray raw = *typed_array;
            raw.set_length(length);
            raw.SetOffHeapDataPtr(isolate_, array_buffer->backing_store(), byte_offset);
            raw.set_is_length_tracking(is_length_tracking);
            raw.set_is_backed_by_rab(array_buffer->is_resizable_by_js() && !array_buffer->is_shared());
        }

        typed_arrays_.set(static_cast<int>(current_typed_array_count_), *typed_array);
    }
}
...
void WebSnapshotDeserializer::DeserializeExports(bool skip_exports)
{
    ...
    dictionary = GlobalDictionary::EnsureCapacity(isolate_, dictionary, dictionary->NumberOfElements() + count, AllocationType::kYoung);
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        PropertyDetails property_details = PropertyDetails(PropertyKind::kData, NONE, PropertyCell::InitialType(isolate_, export_value));
        Handle<Object> export_value_handle(export_value, isolate_);
        AllowGarbageCollection allow_gc;
        Handle<PropertyCell> transition_cell = factory()->NewPropertyCell(export_name, property_details, export_value_handle);
        dictionary = GlobalDictionary::Add(isolate_, dictionary, export_name, transition_cell, property_details, &entry);
        has_exported_values = true;
    }

    if (!has_exported_values)
        return;
...
std::tuple<Object, bool> WebSnapshotDeserializer::ReadValue(Handle<HeapObject> container, uint32_t container_index, InternalizeStrings internalize_strings)
{
    uint8_t value_type;
...
std::tuple<Object, bool> WebSnapshotDeserializer::ReadArray(Handle<HeapObject> container, uint32_t index)
{
    uint32_t array_id;
    ...
    return std::make_tuple(AddDeferredReference(container, index, ARRAY_ID, array_id), true);
}

std::tuple<Object, bool> WebSnapshotDeserializer::ReadArrayBuffer(Handle<HeapObject> container, uint32_t index)
{
    uint32_t array_buffer_id;
    if (!deserializer_->ReadUint32(&array_buffer_id) || array_buffer_id >= kMaxItemCount) {
        Throw("Malformed variable");
        return std::make_tuple(Smi::zero(), false);
    }
    ...
    return std::make_tuple(AddDeferredReference(container, index, ARRAY_BUFFER_ID, array_buffer_id), true);
}
std::tuple<Object, bool> WebSnapshotDeserializer::ReadTypedArray(Handle<HeapObject> container, uint32_t index)
{
    uint32_t typed_array_id;
    if (!deserializer_->ReadUint32(&typed_array_id) || typed_array_id >= kMaxItemCount) {
        Throw("Malformed variable");
        return std::make_tuple(Smi::zero(), false);
    }
    ...
    return std::make_tuple(AddDeferredReference(container, index, TYPED_ARRAY_ID, typed_array_id), true);
}

std::tuple<Object, bool> WebSnapshotDeserializer::ReadDataView(Handle<HeapObject> container, uint32_t index)
{
    uint32_t data_view_id;
    if (!deserializer_->ReadUint32(&data_view_id) || data_view_id >= kMaxItemCount) {
        Throw("Malformed variable");
        return std::make_tuple(Smi::zero(), false);
    }
    ...
    return std::make_tuple(AddDeferredReference(container, index, DATA_VIEW_ID, data_view_id), true);
}

std::tuple<Object, bool> WebSnapshotDeserializer::ReadObject(Handle<HeapObject> container, uint32_t index)
{
    uint32_t object_id;
    ...
    return std::make_tuple(AddDeferredReference(container, index, OBJECT_ID, object_id), true);
}

std::tuple<Object, bool> WebSnapshotDeserializer::ReadFunction(Handle<HeapObject> container, uint32_t index)
{
    ...
    return std::make_tuple(AddDeferredReference(container, index, FUNCTION_ID, function_id), true);
}

std::tuple<Object, bool> WebSnapshotDeserializer::ReadClass(Handle<HeapObject> container, uint32_t index)
{
    uint32_t class_id;
    ...
    return std::make_tuple(AddDeferredReference(container, index, CLASS_ID, class_id), true);
}
...
Object WebSnapshotDeserializer::ReadRegexp()
{
    ...
    base::Optional<JSRegExp::Flags> flags = JSRegExp::FlagsFromString(isolate_, flags_string);
    if (!flags.has_value()) {
        Throw("Malformed flags in regular expression");
        return Smi::zero();
    }
    MaybeHandle<JSRegExp> maybe_regexp = JSRegExp::New(isolate_, pattern, flags.value());
...
{
    uint32_t ref_id;
    if (!deserializer_->ReadUint32(&ref_id) || ref_id >= static_cast<uint32_t>(external_references_.length())) {
...
{
    uint32_t ref_id;
    if (!deserializer_->ReadUint32(&ref_id) || ref_id >= static_cast<uint32_t>(builtin_objects_.length())) {
...
void WebSnapshotDeserializer::ReadFunctionPrototype(Handle<JSFunction> function)
{
    uint32_t object_id;
    ...
    --object_id;
    if (object_id < current_object_count_) {
        if (!SetFunctionPrototype(*function, JSReceiver::cast(objects_.get(object_id)))) {
...
bool WebSnapshotDeserializer::SetFunctionPrototype(JSFunction function, JSReceiver prototype)
{
    DisallowGarbageCollection no_gc;
...
HeapObject WebSnapshotDeserializer::AddDeferredReference(Handle<HeapObject> container, uint32_t index, ValueType target_type, uint32_t target_index)
{
    ...
    DCHECK(container->IsPropertyArray() || container->IsContext() || container->IsFixedArray() || container->IsJSFunction() || container->IsMap());
    deferred_references_ = ArrayList::Add(isolate_, deferred_references_, container, Smi::FromInt(index), Smi::FromInt(target_type), Smi::FromInt(target_index));
...
void WebSnapshotDeserializer::ProcessDeferredReferences()
{
    ...
    for (int i = 0; i < raw_deferred_references.Length() - 3; i += 4) {
        ...
        ValueType target_type = static_cast<ValueType>(raw_deferred_references.Get(i + 2).ToSmi().value());
        ...
        if (InstanceTypeChecker::IsPropertyArray(instance_type)) {
            ...
            AllowGarbageCollection allow_gc;
            Handle<NumberDictionary> new_container = NumberDictionary::Set(isolate_, handle(NumberDictionary::cast(container), isolate_), index, handle(target, isolate_));
            // The number dictionary didn't grow, since it was preallocated to be
            ...
            if (!SetFunctionPrototype(JSFunction::cast(container), JSReceiver::cast(target))) {
                AllowGarbageCollection allow_gc;
                Throw("Can't reuse function prototype");
                return;
            }
            ...
            DCHECK(target.IsJSReceiver());
            AllowGarbageCollection allow_gc;
            SetPrototype(handle(Map::cast(container), isolate_), handle(target, isolate_));
            raw_deferred_references = *deferred_references_;
        } else {
            UNREACHABLE();
        }
