...
public:
    ...
    JsonString(int start, int length, bool needs_conversion, bool needs_internalization, bool has_escape)
        : start_(start)
        , length_(length)
        , needs_conversion_(needs_conversion)
        , internalize_(needs_internalization || length_ <= kMaxInternalizedStringValueLength)
...
public:
    static MaybeHandle<Object> Internalize(Isolate* isolate, Handle<Object> result, Handle<Object> reviver, Handle<String> source);
...
private:
    ...
    MaybeHandle<Object> InternalizeJsonProperty(Handle<JSReceiver> holder, Handle<String> key, Handle<Object> val_node);

    bool RecurseAndApply(Handle<JSReceiver> holder, Handle<String> name, Handle<Object> val_node);
...
enum class JsonToken : uint8_t { NUMBER, STRING, LBRACE, RBRACE, LBRACK, RBRACK, TRUE_LITERAL, FALSE_LITERAL, NULL_LITERAL, WHITESPACE, COLON, COMMA, ILLEGAL, EOS };
...
public:
    ...
    V8_WARN_UNUSED_RESULT static bool CheckRawJson(Isolate* isolate, Handle<String> source) { return JsonParser(isolate, source).ParseRawJson(); }
    ...
    V8_WARN_UNUSED_RESULT static MaybeHandle<Object> Parse(Isolate* isolate, Handle<String> source, Handle<Object> reviver)
    {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, result, JsonParser(isolate, source).ParseJson(reviver), Object);
...
private:
    ...
    void Expect(JsonToken token, base::Optional<MessageTemplate> errorMessage = base::nullopt)
    {
    ...
    void ExpectNext(JsonToken token, base::Optional<MessageTemplate> errorMessage = base::nullopt)
    {
        SkipWhitespace();
    ...
    template <bool should_track_json_source> MaybeHandle<Object> ParseJsonValue(Handle<Object> reviver);

    Handle<Object> BuildJsonObject(const JsonContinuation& cont, const SmallVector<JsonProperty>& property_stack, Handle<Map> feedback);
    Handle<Object> BuildJsonArray(const JsonContinuation& cont, const SmallVector<Handle<Object>>& element_stack);
    ...
    MessageTemplate GetErrorMessageWithEllipses(Handle<Object>& arg, Handle<Object>& arg2, int pos);
    MessageTemplate LookUpErrorMessageForJsonToken(JsonToken token, Handle<Object>& arg, Handle<Object>& arg2, int pos);
    // Mark that a parsing error has happened at the current token.
    void ReportUnexpectedToken(JsonToken token, base::Optional<MessageTemplate> errorMessage = base::nullopt);
    ...
    static void UpdatePointersCallback(LocalIsolate*, GCType, GCCallbackFlags, void* parser) { reinterpret_cast<JsonParser<Char>*>(parser)->UpdatePointers(); }
