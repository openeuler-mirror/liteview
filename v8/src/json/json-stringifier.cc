...
public:
    ...
    V8_WARN_UNUSED_RESULT MaybeHandle<Object> Stringify(Handle<Object> object, Handle<Object> replacer, Handle<Object> gap);
...
private:
    ...
    V8_WARN_UNUSED_RESULT MaybeHandle<Object> ApplyToJsonFunction(Handle<Object> object, Handle<Object> key);
    V8_WARN_UNUSED_RESULT MaybeHandle<Object> ApplyReplacerFunction(Handle<Object> value, Handle<Object> key, Handle<Object> initial_holder);

    // Entry point to serialize the object.
    V8_INLINE Result SerializeObject(Handle<Object> obj) { return Serialize_<false>(obj, false, factory()->empty_string()); }

    // Serialize an array element.
    // The index may serve as argument for the toJSON function.
    V8_INLINE Result SerializeElement(Isolate* isolate, Handle<Object> object, int i) { return Serialize_<false>(object, false, Handle<Object>(Smi::FromInt(i), isolate)); }
    ...
    V8_INLINE Result SerializeProperty(Handle<Object> object, bool deferred_comma, Handle<String> deferred_key)
    {
        DCHECK(!deferred_key.is_null());
        return Serialize_<true>(object, deferred_comma, deferred_key);
    }

    template <bool deferred_string_key> Result Serialize_(Handle<Object> object, bool comma, Handle<Object> key);
    ...
    V8_INLINE Result SerializeHeapNumber(Handle<HeapNumber> object) { return SerializeDouble(object->value()); }
    ...
    V8_INLINE static void SerializeStringUnchecked_(base::Vector<const SrcChar> src, IncrementalStringBuilder::NoExtend<DestChar>* dest);

    template <typename SrcChar, typename DestChar> V8_INLINE void SerializeString_(Handle<String> string);
    ...
    Handle<JSReceiver> CurrentHolder(Handle<Object> value, Handle<Object> inital_holder);
    ...
    Handle<String> ConstructCircularStructureErrorMessage(Handle<Object> last_key, size_t start_index);
...
MaybeHandle<Object> JsonStringify(Isolate* isolate, Handle<Object> object, Handle<Object> replacer, Handle<Object> gap)
{
...
MaybeHandle<Object> JsonStringifier::Stringify(Handle<Object> object, Handle<Object> replacer, Handle<Object> gap)
{
...
bool JsonStringifier::InitializeReplacer(Handle<Object> replacer)
{
    ...
    if (is_array.FromJust()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate_, length_obj, Object::GetLengthFromArrayLike(isolate_, Handle<JSReceiver>::cast(replacer)), false);
        uint32_t length;
        ...
        for (uint32_t i = 0; i < length; i++) {
            ...
            if (element->IsNumber() || element->IsString()) {
                ...
                Handle<Object> value(Handle<JSPrimitiveWrapper>::cast(element)->value(), isolate_);
...
bool JsonStringifier::InitializeGap(Handle<Object> gap)
{
    ...
    if (gap->IsJSPrimitiveWrapper()) {
        Handle<Object> value(Handle<JSPrimitiveWrapper>::cast(gap)->value(), isolate_);
        if (value->IsString()) {
...
MaybeHandle<Object> JsonStringifier::ApplyToJsonFunction(Handle<Object> object, Handle<Object> key)
{
    HandleScope scope(isolate_);
    ...
    Handle<Object> fun;
    LookupIterator it(isolate_, object, tojson_string_, LookupIterator::PROTOTYPE_CHAIN_SKIP_INTERCEPTOR);
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate_, object, Execution::Call(isolate_, fun, object, 1, argv), Object);
    return scope.CloseAndEscape(object);
}

MaybeHandle<Object> JsonStringifier::ApplyReplacerFunction(Handle<Object> value, Handle<Object> key, Handle<Object> initial_holder)
{
    HandleScope scope(isolate_);
    if (key->IsSmi())
        key = factory()->NumberToString(key);
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate_, value, Execution::Call(isolate_, replacer_function_, holder, 2, argv), Object);
    return scope.CloseAndEscape(value);
}

Handle<JSReceiver> JsonStringifier::CurrentHolder(Handle<Object> value, Handle<Object> initial_holder)
{
    if (stack_.empty()) {
        ...
        return Handle<JSReceiver>(JSReceiver::cast(*stack_.back().second), isolate_);
    }
}
...

    {
        ...
        for (size_t i = 0; i < size; ++i) {
            if (*stack_[i].second == raw_obj) {
                ...
                Handle<Object> error = factory()->NewTypeError(MessageTemplate::kCircularStructure, circle_description);
...
private:
    void AppendConstructorName(Handle<Object> object)
    {
        ...
        Handle<String> constructor_name = JSReceiver::GetConstructorName(builder_.isolate(), Handle<JSReceiver>::cast(object));
...
Handle<String> JsonStringifier::ConstructCircularStructureErrorMessage(Handle<Object> last_key, size_t start_index)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate_, result, builder.Finish(), factory()->empty_string());
    return result;
}

template <bool deferred_string_key> JsonStringifier::Result JsonStringifier::Serialize_(Handle<Object> object, bool comma, Handle<Object> key)
{
    ...
    if (!object->IsSmi()) {
        InstanceType instance_type = HeapObject::cast(*object).map(cage_base).instance_type();
        if (InstanceTypeChecker::IsJSReceiver(instance_type) || InstanceTypeChecker::IsBigInt(instance_type)) {
    ...
    case JS_PRIMITIVE_WRAPPER_TYPE:
        if (deferred_string_key)
            SerializeDeferredKey(comma, key);
        return SerializeJSPrimitiveWrapper(Handle<JSPrimitiveWrapper>::cast(object), key);
    case SYMBOL_TYPE:
        return UNCHANGED;
    case JS_RAW_JSON_TYPE:
        ...
        if (deferred_string_key)
            SerializeDeferredKey(comma, key);
        builder_.AppendString(Handle<String>::cast(handle(Handle<JSRawJson>::cast(object)->InObjectPropertyAt(JSRawJson::kRawJsonIndex), isolate_)));
        return SUCCESS;
    default:
        if (InstanceTypeChecker::IsString(instance_type)) {
            ...
            if (HeapObject::cast(*object).IsCallable(cage_base))
                return UNCHANGED;
...
JsonStringifier::Result JsonStringifier::SerializeJSArray(Handle<JSArray> object, Handle<Object> key)
{
    ...
    if (replacer_function_.is_null()) {
        ...
        const uint32_t kMaxAllowedFastPackedLength = std::numeric_limits<uint32_t>::max() - kInterruptLength;
        static_assert(FixedArray::kMaxLength < kMaxAllowedFastPackedLength);
        switch (object->GetElementsKind(cage_base)) {
        case PACKED_SMI_ELEMENTS: {
            Handle<FixedArray> elements(FixedArray::cast(object->elements(cage_base)), isolate_);
            while (true) {
                for (; i < limit; i++) {
                    Separator(i == 0);
        ...
        case PACKED_DOUBLE_ELEMENTS: {
            Handle<FixedDoubleArray> elements(FixedDoubleArray::cast(object->elements(cage_base)), isolate_);
            while (true) {
                for (; i < limit; i++) {
                    Separator(i == 0);
        ...
        case PACKED_ELEMENTS: {
            ...
            for (i = 0; i < length; i++) {
                if (object->length() != *old_length || object->GetElementsKind(cage_base) != PACKED_ELEMENTS) {
                    // Fall back to slow path.
                    break;
                }
                Separator(i == 0);
                Result result = SerializeElement(isolate_, handle(FixedArray::cast(object->elements()).get(cage_base, i), isolate_), i);
                if (result == UNCHANGED) {
                    builder_.AppendCStringLiteral("null");
...
namespace {
V8_INLINE bool CanFastSerializeJSObject(PtrComprCageBase cage_base, JSObject raw_object, Isolate* isolate)
{
    DisallowGarbageCollection no_gc;
    if (raw_object.map(cage_base).IsCustomElementsReceiverMap())
        return false;
...
JsonStringifier::Result JsonStringifier::SerializeJSObject(Handle<JSObject> object, Handle<Object> key)
{
    ...
    for (InternalIndex i : map->IterateOwnDescriptors()) {
        ...
        if (details.location() == PropertyLocation::kField && *map == object->map(cage_base)) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate_, property, Object::GetPropertyOrElement(isolate_, object, key_name), EXCEPTION);
        }
...
JsonStringifier::Result JsonStringifier::SerializeJSReceiverSlow(Handle<JSReceiver> object)
{
    ...
    if (contents.is_null()) {
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(
            isolate_, contents, KeyAccumulator::GetKeys(isolate_, object, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS, GetKeysConversion::kConvertToString), EXCEPTION);
    }
    builder_.AppendCharacter('{');
    Indent();
    bool comma = false;
...
JsonStringifier::Result JsonStringifier::SerializeJSProxy(Handle<JSProxy> object, Handle<Object> key)
{
    ...
    if (is_array.FromJust()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate_, length_object, Object::GetLengthFromArrayLike(isolate_, Handle<JSReceiver>::cast(object)), EXCEPTION);
        uint32_t length;
...
template <typename SrcChar, typename DestChar> void JsonStringifier::SerializeStringUnchecked_(base::Vector<const SrcChar> src, IncrementalStringBuilder::NoExtend<DestChar>* dest)
{
...
template <typename SrcChar, typename DestChar> void JsonStringifier::SerializeString_(Handle<String> string)
{
    ...
    if (int worst_case_length = builder_.EscapedLengthIfCurrentPartFits(length)) {
        ...
        IncrementalStringBuilder::NoExtendBuilder<DestChar> no_extend(&builder_, worst_case_length, no_gc);
...
template <> bool JsonStringifier::DoNotEscape(uint8_t c)
{
    ...
    return base::IsInRange(c, static_cast<uint8_t>(0x23), static_cast<uint8_t>(0x7E)) && c != 0x5C;
}
