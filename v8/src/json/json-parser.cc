...
constexpr bool MayTerminateJsonString(uint8_t flags) { return MayTerminateStringField::decode(flags); }

constexpr EscapeKind GetEscapeKind(uint8_t flags) { return EscapeKindField::decode(flags); }

constexpr bool IsNumberPart(uint8_t flags) { return NumberPartField::decode(flags); }
...
MaybeHandle<Object> JsonParseInternalizer::Internalize(Isolate* isolate, Handle<Object> result, Handle<Object> reviver, Handle<String> source)
{
    DCHECK(reviver->IsCallable());
    JsonParseInternalizer internalizer(isolate, Handle<JSReceiver>::cast(reviver), source);
...
MaybeHandle<Object> JsonParseInternalizer::InternalizeJsonProperty(Handle<JSReceiver> holder, Handle<String> name, Handle<Object> val_node)
{
    DCHECK(reviver_->IsCallable());
    HandleScope outer_scope(isolate_);
    Handle<Object> value;
    ASSIGN_RETURN_ON_EXCEPTION(isolate_, value, Object::GetPropertyOrElement(isolate_, holder, name), Object);
    if (value->IsJSReceiver()) {
        ...
        if (is_array.FromJust()) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate_, length_object, Object::GetLengthFromArrayLike(isolate_, object), Object);
            ...
            if (v8_flags.harmony_json_parse_with_source) {
                ...
                for (double i = 0; i < length; i++) {
                    ...
                    if (!RecurseAndApply(object, index_name, handle(val_nodes->get(i), isolate_))) {
                        return MaybeHandle<Object>();
                    }
                }
            } else {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(
                isolate_, contents, KeyAccumulator::GetKeys(isolate_, object, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS, GetKeysConversion::kConvertToString), Object);
            if (v8_flags.harmony_json_parse_with_source) {
                DCHECK(val_node->IsObjectHashTable());
                Handle<ObjectHashTable> val_nodes = Handle<ObjectHashTable>::cast(val_node);
    ...
    if (v8_flags.harmony_json_parse_with_source) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate_, result, Execution::Call(isolate_, reviver_, holder, 3, argv), Object);
    } else {
        DCHECK(val_node.is_null());
        Handle<Object> argv[] = { name, value };
        ASSIGN_RETURN_ON_EXCEPTION(isolate_, result, Execution::Call(isolate_, reviver_, holder, 2, argv), Object);
    }
    return outer_scope.CloseAndEscape(result);
}

bool JsonParseInternalizer::RecurseAndApply(Handle<JSReceiver> holder, Handle<String> name, Handle<Object> val_node)
{
    STACK_CHECK(isolate_, false);
    DCHECK(reviver_->IsCallable());
    Handle<Object> result;
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate_, result, InternalizeJsonProperty(holder, name, val_node), false);
    Maybe<bool> change_result = Nothing<bool>();
    if (result->IsUndefined(isolate_)) {
        change_result = JSReceiver::DeletePropertyOrElement(holder, name, LanguageMode::kSloppy);
    } else {
    ...
    , original_source_(source)
{
    ...
    if (StringShape(*source_, cage_base).IsExternal()) {
        chars_ = static_cast<const Char*>(SeqExternalString::cast(*source_).GetChars(cage_base));
...
template <typename Char> bool JsonParser<Char>::IsSpecialString()
{
    ...
    // parse method
    int offset = original_source_->IsSlicedString() ? SlicedString::cast(*original_source_).offset() : 0;
...
template <typename Char> MessageTemplate JsonParser<Char>::GetErrorMessageWithEllipses(Handle<Object>& arg, Handle<Object>& arg2, int pos)
{
...
template <typename Char> MessageTemplate JsonParser<Char>::LookUpErrorMessageForJsonToken(JsonToken token, Handle<Object>& arg, Handle<Object>& arg2, int pos)
{
    MessageTemplate message;
    switch (token) {
    case JsonToken::EOS:
...
template <typename Char> void JsonParser<Char>::ReportUnexpectedToken(JsonToken token, base::Optional<MessageTemplate> errorMessage)
{
    ...
    if (!it.done() && it.is_javascript()) {
        ...
        if (summary.script()->IsScript()) {
            script->set_origin_options(Script::cast(*summary.script()).origin_options());
        }
    }
...
template <typename Char> MaybeHandle<Object> JsonParser<Char>::ParseJson(Handle<Object> reviver)
{
    Handle<Object> result;
    // Only record the val node when reviver is callable.
    bool reviver_is_callable = reviver->IsCallable();
    bool should_track_json_source = v8_flags.harmony_json_parse_with_source && reviver_is_callable;
    if (should_track_json_source) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result, ParseJsonValue<true>(reviver), Object);
    } else {
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result, ParseJsonValue<false>(reviver), Object);
    }
...
template <typename Char> void JsonParser<Char>::SkipWhitespace()
{
    ...
    cursor_ = std::find_if(cursor_, end_, [this](Char c) {
        JsonToken current = V8_LIKELY(c <= unibrow::Latin1::kMaxChar) ? one_char_json_tokens[c] : JsonToken::ILLEGAL;
...
namespace {
Handle<Map> ParentOfDescriptorOwner(Isolate* isolate, Handle<Map> maybe_root, Handle<Map> source, int descriptor)
{
    if (descriptor == 0) {
        DCHECK_EQ(0, maybe_root->NumberOfOwnDescriptors());
        return maybe_root;
    }
    return handle(source->FindFieldOwner(isolate, InternalIndex(descriptor - 1)), isolate);
}
} // namespace

template <typename Char> Handle<Object> JsonParser<Char>::BuildJsonObject(const JsonContinuation& cont, const SmallVector<JsonProperty>& property_stack, Handle<Map> feedback)
{
    size_t start = cont.index;
    int length = static_cast<int>(property_stack.size() - start);
    int named_length = length - cont.elements;

    Handle<Map> initial_map = factory()->ObjectLiteralMapFromCache(isolate_->native_context(), named_length);
    ...
    int feedback_descriptors
        = (feedback.is_null() || feedback->elements_kind() != map->elements_kind() || feedback->instance_size() != map->instance_size()) ? 0 : feedback->NumberOfOwnDescriptors();
    ...
    for (i = 0; i < length; i++) {
        ...
        if (descriptor < feedback_descriptors) {
            expected = handle(String::cast(feedback->instance_descriptors(isolate_).GetKey(descriptor_index)), isolate_);
        } else {
            ...
            if (!expected.is_null()) {
                ...
                target = TransitionsAccessor(isolate(), *map).ExpectedTransitionTarget();
            }
        }
        ...
        if (key.is_identical_to(expected)) {
            ...
            if (!TransitionsAccessor(isolate(), *map).FindTransitionToField(key).ToHandle(&target)) {
                break;
            }
        }
        ...
        PropertyDetails details = target->instance_descriptors(isolate_).GetDetails(descriptor_index);
        ...
        if (!value->FitsRepresentation(expected_representation)) {
            ...
            Handle<FieldType> value_type = value->OptimalType(isolate(), representation);
            MapUpdater::GeneralizeField(isolate(), target, descriptor_index, details.constness(), representation, value_type);
        } else if (expected_representation.IsHeapObject() && !target->instance_descriptors(isolate()).GetFieldType(descriptor_index).NowContains(value)) {
            Handle<FieldType> value_type = value->OptimalType(isolate(), expected_representation);
            MapUpdater::GeneralizeField(isolate(), target, descriptor_index, details.constness(), expected_representation, value_type);
        } else if (expected_representation.IsDouble() && value->IsSmi()) {
            new_mutable_double++;
        }

        DCHECK(target->instance_descriptors(isolate()).GetFieldType(descriptor_index).NowContains(value));
        map = target;
        descriptor++;
    }
    ...
    if (new_mutable_double > 0) {
        mutable_double_buffer = factory()->NewByteArray(kMutableDoubleSize * new_mutable_double);
    }

    Handle<JSObject> object = initial_map->is_dictionary_map() ? factory()->NewSlowJSObjectFromMap(map) : factory()->NewJSObjectFromMap(map);
...

    {
        ...
        Address mutable_double_address = mutable_double_buffer.is_null() ? 0 : reinterpret_cast<Address>(mutable_double_buffer->GetDataStartAddress());
        ...
        for (int j = 0; j < i; j++) {
            ...
            if (property.string.is_index())
                continue;
            InternalIndex descriptor_index(descriptor);
            PropertyDetails details = map->instance_descriptors(isolate()).GetDetails(descriptor_index);
            ...
            if (details.representation().IsDouble()) {
                if (value.IsSmi()) {
                    ...
                    uint64_t bits = base::bit_cast<uint64_t>(static_cast<double>(Smi::ToInt(value)));
                    ...
                    value = hn;
                    mutable_double_address += ALIGN_TO_ALLOCATION_ALIGNMENT(kMutableDoubleSize);
                } else {
                    DCHECK(value.IsHeapNumber());
                    HeapObject::cast(value).set_map(*factory()->heap_number_map(), kReleaseStore);
                }
            }
        ...
        if (!mutable_double_buffer.is_null()) {
#ifdef DEBUG
            Address end = reinterpret_cast<Address>(mutable_double_buffer->GetDataEndAddress());
            if (!V8_COMPRESS_POINTERS_8GB_BOOL && kTaggedSize != kDoubleSize) {
            ...
            isolate()->heap()->EnsureSweepingCompletedForObject(*mutable_double_buffer);
...
template <typename Char> Handle<Object> JsonParser<Char>::BuildJsonArray(const JsonContinuation& cont, const SmallVector<Handle<Object>>& element_stack)
{
    size_t start = cont.index;
...
template <typename Char> bool JsonParser<Char>::ParseRawJson()
{
    ...
    next_ = V8_LIKELY(*cursor_ <= unibrow::Latin1::kMaxChar) ? one_char_json_tokens[*cursor_] : JsonToken::ILLEGAL;
    switch (peek()) {
    case JsonToken::STRING:
...
// Parse any JSON value.
template <typename Char> template <bool should_track_json_source> MaybeHandle<Object> JsonParser<Char>::ParseJsonValue(Handle<Object> reviver)
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            case JsonToken::LBRACE: {
                ...
                cont = JsonContinuation(isolate_, JsonContinuation::kObjectProperty, property_stack.size());
            ...
            case JsonToken::LBRACK:
                ...
                cont_stack.emplace_back(std::move(cont));
                cont = JsonContinuation(isolate_, JsonContinuation::kArrayElement, element_stack.size());
        ...
        while (true) {
            ...
            case JsonContinuation::kObjectProperty: {
                ...
                Handle<Map> feedback;
                if (cont_stack.size() > 0 && cont_stack.back().type() == JsonContinuation::kArrayElement && cont_stack.back().index < element_stack.size()
                ...
                if (should_track_json_source) {
                    size_t start = cont.index;
                    int length = static_cast<int>(property_stack.size() - start);
                    Handle<ObjectHashTable> table = ObjectHashTable::New(isolate(), length);
                    for (int i = 0; i < length; i++) {
                        const JsonProperty& property = property_stack[start + i];
                        if (property.string.is_index()) {
                            table = ObjectHashTable::Put(table, factory()->Uint32ToString(property.string.index()), property_val_node_stack[start + i]);
                        } else {
                            table = ObjectHashTable::Put(table, MakeString(property.string), property_val_node_stack[start + i]);
                        }
                    }
...

    {
        ...
        if (c == '0') {
            ...
            if ((cursor_ - smi_start) <= kMaxSmiLength
                && (!base::IsInRange(c, 0, static_cast<int32_t>(unibrow::Latin1::kMaxChar)) || !IsNumberPart(character_json_scan_flags[c]))) {
                // Smi.
                int32_t i = 0;
...
template <typename Char> bool Matches(const base::Vector<const Char>& chars, Handle<String> string)
{
...
template <typename Char> Handle<String> JsonParser<Char>::MakeString(const JsonString& string, Handle<String> hint)
{
    ...
    if (string.internalize() && !string.has_escape()) {
        ...
        if (chars_may_relocate_) {
            return factory()->InternalizeString(Handle<SeqString>::cast(source_), string.start(), string.length(), string.needs_conversion());
        }
...
template <typename Char> JsonString JsonParser<Char>::ScanJsonString(bool needs_internalization)
{
    ...
    while (true) {
        ...
        if (*cursor_ == '"') {
            ...
            return JsonString(start, length, convert, needs_internalization, has_escape);
        }
