...
public:
    ...
    //
    // This method is atomic and can be called from background threads.
    inline ExternalPointerHandle AllocateAndInitializeEntry(Isolate* isolate, Address initial_value, ExternalPointerTag tag);
...
private:
    ...
    static constexpr uint64_t kTableIsCurrentlySweepingMarker = static_cast<uint64_t>(-1);
    ...
    void set_capacity(uint32_t new_capacity) { base::Relaxed_Store(&capacity_, new_capacity); }

    // Start of evacuation area accessors.
    uint32_t start_of_evacuation_area() const { return base::Relaxed_Load(&start_of_evacuation_area_); }
    void set_start_of_evacuation_area(uint32_t value) { base::Relaxed_Store(&start_of_evacuation_area_, value); }
    ...
    private:
        explicit Freelist(uint64_t encoded_form)
            : encoded_(encoded_form)
        {
        }
    ...
    // Freelist accessors.
    Freelist Relaxed_GetFreelist() { return Freelist::Decode(base::Relaxed_Load(&freelist_)); }
    Freelist Acquire_GetFreelist() { return Freelist::Decode(base::Acquire_Load(&freelist_)); }
    void Relaxed_SetFreelist(Freelist new_freelist) { base::Relaxed_Store(&freelist_, new_freelist.Encode()); }
    void Release_SetFreelist(Freelist new_freelist) { base::Release_Store(&freelist_, new_freelist.Encode()); }
    bool Relaxed_CompareAndSwapFreelist(Freelist old_freelist, Freelist new_freelist)
    {
        uint64_t old_val = base::Relaxed_CompareAndSwap(&freelist_, old_freelist.Encode(), new_freelist.Encode());
    ...
    //
    // This method is atomic and can be called from background threads.
    inline ExternalPointerHandle AllocateEvacuationEntry(uint32_t start_of_evacuation_area);
    ...
    inline uint32_t HandleToIndex(ExternalPointerHandle handle) const
    {
        ...
        DCHECK_EQ(handle & ~kVisitedHandleMarker, index << kExternalPointerIndexShift);
...
#ifdef DEBUG
    inline bool HandleWasVisitedDuringMarking(ExternalPointerHandle handle) { return (handle & kVisitedHandleMarker) == kVisitedHandleMarker; }
#endif // DEBUG
    ...
    public:
        ...
        bool HasTag(ExternalPointerTag tag) const { return (value_ & kExternalPointerTagMask) == tag; }
        ...
        bool IsFreelistEntry() const { return HasTag(kExternalPointerFreeEntryTag); }

        // Returns true if this is a evacuation entry, in which case
        // ExtractHandleLocation may be used.
        bool IsEvacuationEntry() const { return HasTag(kExternalPointerEvacuationEntryTag); }

        // Returns true if this is neither a freelist- nor an evacuation entry.
        bool IsRegularEntry() const { return !IsFreelistEntry() && !IsEvacuationEntry(); }

        // Extract the index of the next entry on the freelist. This method may be
        // called even when the entry is not a freelist entry. However, the result
        // is only valid if this is a freelist entry. This behaviour is required
        // for efficient entry allocation, see TryAllocateEntryFromFreelist.
        uint32_t ExtractNextFreelistEntry() const { return static_cast<uint32_t>(value_); }
        ...
        static Entry MakeEvacuationEntry(Address handle_location) { return Entry(handle_location | kExternalPointerEvacuationEntryTag); }
    ...
    inline Address entry_address(uint32_t index) const { return buffer_ + index * sizeof(Address); }
    ...
    Entry RelaxedCompareAndSwap(uint32_t index, Entry old_entry, Entry new_entry)
    {
        ...
        auto addr = reinterpret_cast<base::Atomic64*>(entry_address(index));
        auto raw_value = base::Relaxed_CompareAndSwap(addr, old_entry.Encode(), new_entry.Encode());
        return Entry::Decode(raw_value);
    }
