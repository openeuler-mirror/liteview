...
void SandboxMemoryView(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!args[0]->ToInteger(context).ToLocal(&arg1) || !args[1]->ToInteger(context).ToLocal(&arg2)) {
    ...
    std::unique_ptr<BackingStore> memory
        = BackingStore::WrapAllocation(reinterpret_cast<void*>(sandbox->base() + offset), size, v8::BackingStore::EmptyDeleter, nullptr, SharedFlag::kNotShared);
    if (!memory) {
...
Handle<JSFunction> CreateFunc(Isolate* isolate, FunctionCallback func, Handle<String> name, bool is_constructor)
{
    ...
    return ApiNatives::InstantiateFunction(function_template, name).ToHandleChecked();
}
...
#error "This function should not be available in any shipping build "          \
    ...
    Handle<JSObject> sandbox = factory->NewJSObject(isolate->object_function(), AllocationType::kOld);
...
void SandboxSignalHandler(int signal, siginfo_t* info, void* void_context)
{
    ...
    if (GetProcessWideSandbox()->Contains(faultaddr)) {
        ...
        PrintToStderr("Caught harmless memory access violaton (inside sandbox address "
    ...
    if (info->si_code == SI_KERNEL && faultaddr == 0) {
        ...
        PrintToStderr("Caught harmless memory access violaton (non-canonical address). "
                      "Exiting process...\n");
        _exit(0);
    }
    ...
    if (faultaddr < 0x1000) {
        ...
        PrintToStderr("Caught harmless memory access violaton (nullptr dereference). Exiting "
                      "process...\n");
        _exit(0);
    }
    ...
    if (info->si_code == SEGV_ACCERR) {
        ...
        PrintToStderr("Caught harmless memory access violaton (memory permission violation). "
                      "Exiting process...\n");
        _exit(0);
    }
