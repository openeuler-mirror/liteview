...
void ExternalPointerTable::Init(Isolate* isolate)
{
    ...
    DCHECK(IsAligned(kExternalPointerTableReservationSize, root_space->allocation_granularity()));
    ...
    buffer_ = root_space->AllocatePages(VirtualAddressSpace::kNoHint, reservation_size, root_space->allocation_granularity(), PagePermissions::kNoAccess);
    if (!buffer_) {
        V8::FatalProcessOutOfMemory(isolate, "Failed to reserve memory for ExternalPointerTable backing buffer");
    }

    mutex_ = new base::Mutex;
    if (!mutex_) {
        V8::FatalProcessOutOfMemory(isolate, "Failed to allocate mutex for ExternalPointerTable");
    }
...
uint32_t ExternalPointerTable::SweepAndCompact(Isolate* isolate)
{
    ...
    if (IsCompacting()) {
        ...
        } else if (old_freelist.IsEmpty() || old_freelist.Head() > first_block_of_evacuation_area) {
        ...
        isolate->counters()->external_pointer_table_compaction_outcome()->AddSample(static_cast<int>(outcome));
    ...
    for (uint32_t i = table_end - 1; i > 0; i--) {
        ...
        if (entry.IsEvacuationEntry()) {
            ...
            ExternalPointerHandle* handle_location = reinterpret_cast<ExternalPointerHandle*>(entry.ExtractHandleLocation());
...
#if defined(LEAK_SANITIZER)
        Address new_shadow_table_end = buffer_ + kExternalPointerTableReservationSize + new_capacity * sizeof(Address);
...
void ExternalPointerTable::StartCompactingIfNeeded()
{
    ...
    double free_ratio = static_cast<double>(freelist_size) / static_cast<double>(current_capacity);
    uint32_t num_blocks_to_evacuate = (freelist_size / 2) / kEntriesPerBlock;
    bool should_compact = (table_size >= 1 * MB) && (free_ratio >= 0.10) && (num_blocks_to_evacuate >= 1);

    if (should_compact) {
        uint32_t num_entries_to_evacuate = num_blocks_to_evacuate * kEntriesPerBlock;
...
ExternalPointerTable::Freelist ExternalPointerTable::Grow(Isolate* isolate)
{
    ...
    if (new_capacity > kMaxExternalPointers) {
        V8::FatalProcessOutOfMemory(isolate, "Cannot grow ExternalPointerTable past its maximum capacity");
    }
    if (!root_space->SetPagePermissions(buffer_ + old_capacity * sizeof(Address), kBlockSize, PagePermissions::kReadWrite)) {
        V8::FatalProcessOutOfMemory(isolate, "Failed to grow the ExternalPointerTable backing buffer");
    }

#if defined(LEAK_SANITIZER)
    if (!root_space->SetPagePermissions(buffer_ + kExternalPointerTableReservationSize + old_capacity * sizeof(Address), kBlockSize, PagePermissions::kReadWrite)) {
        V8::FatalProcessOutOfMemory(isolate, "Failed to grow the ExternalPointerTabl shadow table");
    }
#endif // LEAK_SANITIZER
