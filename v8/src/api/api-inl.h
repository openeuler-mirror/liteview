...
template <typename T> inline T ToCData(v8::internal::Object obj)
{
    ...
    if (obj == v8::internal::Smi::zero())
        return nullptr;
    return reinterpret_cast<T>(v8::internal::Foreign::cast(obj).foreign_address());
}
...
template <typename T> inline v8::internal::Handle<v8::internal::Object> FromCData(v8::internal::Isolate* isolate, T obj)
{
    ...
    return isolate->factory()->NewForeign(reinterpret_cast<v8::internal::Address>(obj));
}
...
template <class From, class To> inline Local<To> Utils::Convert(v8::internal::Handle<From> obj)
{
...
#define MAKE_TO_LOCAL(Name, From, To)                                                                                                                                              \
    Local<v8::To> Utils::Name(v8::internal::Handle<v8::internal::From> obj) { return Convert<v8::internal::From, v8::To>(obj); }

#define MAKE_TO_LOCAL_TYPED_ARRAY(Type, typeName, TYPE, ctype)                                                                                                                     \
    Local<v8::Type##Array> Utils::ToLocal##Type##Array(v8::internal::Handle<v8::internal::JSTypedArray> obj)                                                                       \
    {                                                                                                                                                                              \
...
public:
    CallDepthScope(i::Isolate* isolate, Local<Context> context)
        ...
        , interrupts_scope_(isolate_, i::StackGuard::TERMINATE_EXECUTION,
              isolate_->only_terminate_in_safe_scope() ? (safe_for_termination_ ? i::InterruptsScope::kRunInterrupts : i::InterruptsScope::kPostponeInterrupts)
...
private:
    ...
    bool CheckKeptObjectsClearedAfterMicrotaskCheckpoint(i::MicrotaskQueue* microtask_queue)
    {
        bool did_perform_microtask_checkpoint
            = isolate_->thread_local_top()->CallDepthIsZero() && do_callback && microtask_queue && microtask_queue->microtasks_policy() == MicrotasksPolicy::kAuto;
        return !did_perform_microtask_checkpoint || isolate_->heap()->weak_refs_keep_during_job().IsUndefined(isolate_);
    }
#endif
...
template <typename T> void CopySmiElementsToTypedBuffer(T* dst, uint32_t length, i::FixedArray elements)
{
...
template <typename T> void CopyDoubleElementsToTypedBuffer(T* dst, uint32_t length, i::FixedDoubleArray elements)
{
    for (uint32_t i = 0; i < length; ++i) {
...
template <CTypeInfo::Identifier type_info_id, typename T> bool CopyAndConvertArrayToCppBuffer(Local<Array> src, T* dst, uint32_t max_length)
{
    ...
    case i::PACKED_DOUBLE_ELEMENTS:
        CopyDoubleElementsToTypedBuffer(dst, length, i::FixedDoubleArray::cast(elements));
        return true;
    default:
        return false;
    }
...
template <CTypeInfo::Identifier type_info_id, typename T> inline bool V8_EXPORT TryToCopyAndConvertArrayToCppBuffer(Local<Array> src, T* dst, uint32_t max_length)
{
