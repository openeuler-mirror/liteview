...
static ScriptOrigin GetScriptOriginForScript(i::Isolate* i_isolate, i::Handle<i::Script> script)
{
    ...
    i::Handle<i::Object> host_defined_options(script->host_defined_options(), i_isolate);
    ScriptOriginOptions options(script->origin_options());
    bool is_wasm = false;
#if V8_ENABLE_WEBASSEMBLY
    is_wasm = script->type() == i::Script::TYPE_WASM;
#endif // V8_ENABLE_WEBASSEMBLY
    v8::ScriptOrigin origin(reinterpret_cast<v8::Isolate*>(i_isolate), Utils::ToLocal(scriptName), script->line_offset(), script->column_offset(), options.IsSharedCrossOrigin(),
        script->id(), Utils::ToLocal(source_map_url), options.IsOpaque(), is_wasm, options.IsModule(), Utils::ToLocal(host_defined_options));
    return origin;
}
...
void Utils::ReportOOMFailure(i::Isolate* i_isolate, const char* location, const OOMDetails& details)
{
    if (auto oom_callback = i_isolate->oom_behavior()) {
        ...
        if (fatal_callback == nullptr) {
            base::OS::PrintError("\n#\n# Fatal %s OOM in %s\n#\n\n", details.is_heap_oom ? "javascript" : "process", location);
#ifdef V8_FUZZILLI
...
void V8::SetSnapshotDataBlob(StartupData* snapshot_blob) { i::V8::SetSnapshotBlob(snapshot_blob); }
...
public:
    ...
    void* Allocate(size_t length) override { return page_allocator_->AllocatePages(nullptr, RoundUp(length, page_size_), page_size_, PageAllocator::kReadWrite); }

    void* AllocateUninitialized(size_t length) override { return Allocate(length); }

    void Free(void* data, size_t length) override { page_allocator_->FreePages(data, RoundUp(length, page_size_)); }
...
public:
    ...
    void* AllocateUninitialized(size_t length) override { return base::Malloc(length); }
...
struct SnapshotCreatorData {
    ...
    static SnapshotCreatorData* cast(void* data) { return reinterpret_cast<SnapshotCreatorData*>(data); }
...
SnapshotCreator::SnapshotCreator(Isolate* v8_isolate, const intptr_t* external_references, StartupData* existing_snapshot)
{
    ...
    v8_isolate->Enter();
    const StartupData* blob = existing_snapshot ? existing_snapshot : i::Snapshot::DefaultSnapshotBlob();
...
SnapshotCreator::SnapshotCreator(const intptr_t* external_references, StartupData* existing_snapshot)
    : SnapshotCreator(Isolate::Allocate(), external_references, existing_snapshot)
{
}
...
Isolate* SnapshotCreator::GetIsolate() { return SnapshotCreatorData::cast(data_)->isolate_; }
...
size_t SnapshotCreator::AddData(i::Address object)
{
    ...
    if (!i_isolate->heap()->serialized_objects().IsArrayList()) {
        ...
        list = i::Handle<i::ArrayList>(i::ArrayList::cast(i_isolate->heap()->serialized_objects()), i_isolate);
    }
...
size_t SnapshotCreator::AddData(Local<Context> context, i::Address object)
{
    ...
    if (!ctx->serialized_objects().IsArrayList()) {
        ...
        list = i::Handle<i::ArrayList>(i::ArrayList::cast(ctx->serialized_objects()), i_isolate);
    }
    size_t index = static_cast<size_t>(list->Length());
    list = i::ArrayList::Add(i_isolate, list, obj);
...
void ConvertSerializedObjectsToFixedArray(Local<Context> context)
{
    ...
    i::Isolate* i_isolate = ctx->GetIsolate();
    if (!ctx->serialized_objects().IsArrayList()) {
        ctx->set_serialized_objects(i::ReadOnlyRoots(i_isolate).empty_fixed_array());
    } else {
        i::Handle<i::ArrayList> list(i::ArrayList::cast(ctx->serialized_objects()), i_isolate);
...
{
    if (!i_isolate->heap()->serialized_objects().IsArrayList()) {
        i_isolate->heap()->SetSerializedObjects(i::ReadOnlyRoots(i_isolate).empty_fixed_array());
    } else {
        i::Handle<i::ArrayList> list(i::ArrayList::cast(i_isolate->heap()->serialized_objects()), i_isolate);
        i::Handle<i::FixedArray> elements = i::ArrayList::Elements(i_isolate, list);
...
StartupData SnapshotCreator::CreateBlob(SnapshotCreator::FunctionCodeHandling function_code_handling)
{
    ...
    Utils::ApiCheck(!data->created_, "v8::SnapshotCreator::CreateBlob",
        ...
        "SnapshotCreator");
    Utils::ApiCheck(!data->default_context_.IsEmpty(), "v8::SnapshotCreator::CreateBlob", "CreateBlob() cannot be called before the default context is set");
    ...
    // serialization.
    {
        ...
        ConvertSerializedObjectsToFixedArray(data->default_context_.Get(data->isolate_));
        ...
        i::Handle<i::FixedArray> global_proxy_sizes = i_isolate->factory()->NewFixedArray(num_additional_contexts, i::AllocationType::kOld);
        for (int i = 0; i < num_additional_contexts; i++) {
            i::Handle<i::Context> context = v8::Utils::OpenHandle(*data->contexts_[i].Get(isolate));
            global_proxy_sizes->set(i, i::Smi::FromInt(context->global_proxy().Size()));
        }
    ...
    i::Snapshot::ClearReconstructableDataForSerialization(i_isolate, function_code_handling == FunctionCodeHandling::kClear);
    ...
    {
        i::HandleScope scope(i_isolate);
        contexts.push_back(*v8::Utils::OpenHandle(*data->default_context_.Get(data->isolate_)));
    ...
    embedder_fields_serializers.push_back(data->default_embedder_fields_serializer_);
    for (int i = 0; i < num_additional_contexts; i++) {
        embedder_fields_serializers.push_back(data->embedder_fields_serializers_[i]);
    }

    data->created_ = true;
    return i::Snapshot::Create(i_isolate, &contexts, embedder_fields_serializers, global_safepoint, no_gc_from_here_on);
}
...
void V8::SetDcheckErrorHandler(DcheckErrorCallback that) { v8::base::SetDcheckFunction(that); }

void V8::SetFlagsFromString(const char* str) { SetFlagsFromString(str, strlen(str)); }
...
void RegisterExtension(std::unique_ptr<Extension> extension) { RegisteredExtension::Register(std::move(extension)); }
...
i::Address* GlobalizeTracedReference(i::Isolate* i_isolate, i::Address* obj, internal::Address* slot, GlobalHandleStoreMode store_mode)
{
    API_RCS_SCOPE(i_isolate, TracedGlobal, New);
#ifdef DEBUG
    Utils::ApiCheck((slot != nullptr), "v8::GlobalizeTracedReference", "the address slot must be not null");
#endif
    i::Handle<i::Object> result = i_isolate->global_handles()->CreateTraced(*obj, slot, store_mode);
#ifdef VERIFY_HEAP
...
void MoveTracedReference(internal::Address** from, internal::Address** to) { GlobalHandles::MoveTracedReference(from, to); }

void CopyTracedReference(const internal::Address* const* from, internal::Address** to) { GlobalHandles::CopyTracedReference(from, to); }

void DisposeTracedReference(internal::Address* location) { GlobalHandles::DestroyTracedReference(location); }
...
void MoveGlobalReference(internal::Address** from, internal::Address** to) { i::GlobalHandles::MoveGlobal(from, to); }
...
void MakeWeak(i::Address** location_addr) { i::GlobalHandles::MakeWeak(location_addr); }

void* ClearWeak(i::Address* location) { return i::GlobalHandles::ClearWeakness(location); }

void AnnotateStrongRetainer(i::Address* location, const char* label) { i::GlobalHandles::AnnotateStrongRetainer(location, label); }

void DisposeGlobal(i::Address* location) { i::GlobalHandles::Destroy(location); }
...
Value* Eternalize(Isolate* v8_isolate, Value* value)
{
    ...
    return reinterpret_cast<Value*>(i_isolate->eternal_handles()->Get(index).location());
}

void FromJustIsNothing() { Utils::ApiCheck(false, "v8::FromJust", "Maybe value is Nothing"); }

void ToLocalEmpty() { Utils::ApiCheck(false, "v8::ToLocalChecked", "Empty MaybeLocal"); }
...
void HandleScope::Initialize(Isolate* v8_isolate)
{
    ...
    Utils::ApiCheck(!i_isolate->was_locker_ever_used() || i_isolate->thread_manager()->IsLockedByCurrentThread() || i_isolate->serializer_enabled(), "HandleScope::HandleScope",
...
HandleScope::~HandleScope() { i::HandleScope::CloseScope(i_isolate_, prev_next_, prev_limit_); }
...
int HandleScope::NumberOfHandles(Isolate* v8_isolate) { return i::HandleScope::NumberOfHandles(reinterpret_cast<i::Isolate*>(v8_isolate)); }

i::Address* HandleScope::CreateHandle(i::Isolate* i_isolate, i::Address value) { return i::HandleScope::CreateHandle(i_isolate, value); }
...
i::Address* EscapableHandleScope::Escape(i::Address* escape_value)
{
    ...
    Utils::ApiCheck(i::Object(*escape_slot_).IsTheHole(heap->isolate()), "EscapableHandleScope::Escape", "Escape value set twice");
...
bool Data::IsFixedArray() const { return Utils::OpenHandle(this)->IsFixedArray(); }
...
bool Data::IsPrivate() const { return Utils::OpenHandle(this)->IsPrivateSymbol(); }

bool Data::IsObjectTemplate() const { return Utils::OpenHandle(this)->IsObjectTemplateInfo(); }

bool Data::IsFunctionTemplate() const { return Utils::OpenHandle(this)->IsFunctionTemplateInfo(); }
...
static i::Handle<i::EmbedderDataArray> EmbedderDataFor(Context* context, int index, bool can_grow, const char* location)
{
    ...
    bool ok = Utils::ApiCheck(env->IsNativeContext(), location, "Not a native context") && Utils::ApiCheck(index >= 0, location, "Negative index");
    if (!ok)
        return i::Handle<i::EmbedderDataArray>();
    // TODO(ishell): remove cast once embedder_data slot has a proper type.
    i::Handle<i::EmbedderDataArray> data(i::EmbedderDataArray::cast(env->embedder_data()), i_isolate);
...
uint32_t Context::GetNumberOfEmbedderDataFields()
{
    ...
    Utils::ApiCheck(context->IsNativeContext(), "Context::GetNumberOfEmbedderDataFields", "Not a native context");
    // TODO(ishell): remove cast once embedder_data slot has a proper type.
    return static_cast<uint32_t>(i::EmbedderDataArray::cast(context->embedder_data()).length());
}
...
v8::Local<v8::Value> Context::SlowGetEmbedderData(int index)
{
    ...
    i::Handle<i::Object> result(i::EmbedderDataSlot(*data, index).load_tagged(), i_isolate);
...
void Context::SetEmbedderData(int index, v8::Local<Value> value)
{
    ...
    DCHECK_EQ(*Utils::OpenHandle(*value), *Utils::OpenHandle(*GetEmbedderData(index)));
}
...
void Context::SetAlignedPointerInEmbedderData(int index, void* value)
{
    ...
    i::Isolate* i_isolate = Utils::OpenHandle(this)->GetIsolate();
    i::Handle<i::EmbedderDataArray> data = EmbedderDataFor(this, index, true, location);
    bool ok = i::EmbedderDataSlot(*data, index).store_aligned_pointer(i_isolate, value);
...
static void InitializeTemplate(i::TemplateInfo that, int type, bool do_not_cache)
{
    that.set_number_of_properties(0);
    that.set_tag(type);
    int serial_number = do_not_cache ? i::TemplateInfo::kDoNotCache : i::TemplateInfo::kUncached;
...
{
    Set(Utils::ToLocal(Utils::OpenHandle(reinterpret_cast<Name*>(*name))), value, attribute);
}

void Template::SetAccessorProperty(
    v8::Local<v8::Name> name, v8::Local<FunctionTemplate> getter, v8::Local<FunctionTemplate> setter, v8::PropertyAttribute attribute, v8::AccessControl access_control)
{
    Utils::ApiCheck(getter.IsEmpty() || !Utils::OpenHandle(*getter)->call_code(kAcquireLoad).IsUndefined(), "v8::Template::SetAccessorProperty", "Getter must have a call handler");
    Utils::ApiCheck(setter.IsEmpty() || !Utils::OpenHandle(*setter)->call_code(kAcquireLoad).IsUndefined(), "v8::Template::SetAccessorProperty", "Setter must have a call handler");
    ...
    i::ApiNatives::AddAccessorProperty(
        i_isolate, templ, Utils::OpenHandle(*name), Utils::OpenHandle(*getter, true), Utils::OpenHandle(*setter, true), static_cast<i::PropertyAttributes>(attribute));
}

// --- F u n c t i o n   T e m p l a t e ---
static void InitializeFunctionTemplate(i::FunctionTemplateInfo info, bool do_not_cache)
{
...
namespace {
Local<ObjectTemplate> ObjectTemplateNew(i::Isolate* i_isolate, v8::Local<FunctionTemplate> constructor, bool do_not_cache)
{
    API_RCS_SCOPE(i_isolate, ObjectTemplate, New);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::Struct> struct_obj = i_isolate->factory()->NewStruct(i::OBJECT_TEMPLATE_INFO_TYPE, i::AllocationType::kOld);
...
Local<ObjectTemplate> FunctionTemplate::PrototypeTemplate()
{
    ...
    if (result->IsUndefined(i_isolate)) {
        ...
        result = Utils::OpenHandle(*ObjectTemplateNew(i_isolate, Local<FunctionTemplate>(), true));
...
void FunctionTemplate::SetPrototypeProviderTemplate(Local<FunctionTemplate> prototype_provider)
{
    ...
    i::Isolate* i_isolate = self->GetIsolate();
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::FunctionTemplateInfo> result = Utils::OpenHandle(*prototype_provider);
    Utils::ApiCheck(self->GetPrototypeTemplate().IsUndefined(i_isolate), "v8::FunctionTemplate::SetPrototypeProviderTemplate", "Protoype must be undefined");
    Utils::ApiCheck(self->GetParentTemplate().IsUndefined(i_isolate), "v8::FunctionTemplate::SetPrototypeProviderTemplate", "Prototype provider must be empty");
    i::FunctionTemplateInfo::SetPrototypeProviderTemplate(i_isolate, self, result);
}

namespace {
static void EnsureNotPublished(i::Handle<i::FunctionTemplateInfo> info, const char* func)
{
    DCHECK_IMPLIES(info->instantiated(), info->published());
    Utils::ApiCheck(!info->published(), func, "FunctionTemplate already instantiated");
}

Local<FunctionTemplate> FunctionTemplateNew(i::Isolate* i_isolate, FunctionCallback callback, v8::Local<Value> data, v8::Local<Signature> signature, int length,
    ConstructorBehavior behavior, bool do_not_cache, v8::Local<Private> cached_property_name = v8::Local<Private>(),
    SideEffectType side_effect_type = SideEffectType::kHasSideEffect, const MemorySpan<const CFunction>& c_function_overloads = {}, uint8_t instance_type = 0,
    uint8_t allowed_receiver_instance_type_range_start = 0, uint8_t allowed_receiver_instance_type_range_end = 0)
{
    i::Handle<i::Struct> struct_obj = i_isolate->factory()->NewStruct(i::FUNCTION_TEMPLATE_INFO_TYPE, i::AllocationType::kOld);
    i::Handle<i::FunctionTemplateInfo> obj = i::Handle<i::FunctionTemplateInfo>::cast(struct_obj);
    {
        ...
        raw.set_cached_property_name(cached_property_name.IsEmpty() ? i::ReadOnlyRoots(i_isolate).the_hole_value() : *Utils::OpenHandle(*cached_property_name));
        if (behavior == ConstructorBehavior::kThrow)
            raw.set_remove_prototype(true);
        raw.SetInstanceType(instance_type);
        raw.set_allowed_receiver_instance_type_range_start(allowed_receiver_instance_type_range_start);
        raw.set_allowed_receiver_instance_type_range_end(allowed_receiver_instance_type_range_end);
    }
    if (callback != nullptr) {
...
void FunctionTemplate::Inherit(v8::Local<FunctionTemplate> value)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    Utils::ApiCheck(info->GetPrototypeProviderTemplate().IsUndefined(i_isolate), "v8::FunctionTemplate::Inherit", "Protoype provider must be empty");
...
Local<FunctionTemplate> FunctionTemplate::New(Isolate* v8_isolate, FunctionCallback callback, v8::Local<Value> data, v8::Local<Signature> signature, int length,
    ConstructorBehavior behavior, SideEffectType side_effect_type, const CFunction* c_function, uint16_t instance_type, uint16_t allowed_receiver_instance_type_range_start,
    uint16_t allowed_receiver_instance_type_range_end)
{
    ...
    if (!Utils::ApiCheck(!c_function || behavior == ConstructorBehavior::kThrow, "FunctionTemplate::New", "Fast API calls are not supported for constructor functions")) {
        return Local<FunctionTemplate>();
    }

    if (instance_type != 0) {
        if (!Utils::ApiCheck(instance_type >= i::Internals::kFirstJSApiObjectType && instance_type <= i::Internals::kLastJSApiObjectType, "FunctionTemplate::New",
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    return FunctionTemplateNew(i_isolate, callback, data, signature, length, behavior, false, Local<Private>(), side_effect_type,
        c_function ? MemorySpan<const CFunction> { c_function, 1 } : MemorySpan<const CFunction> {}, instance_type, allowed_receiver_instance_type_range_start,
        allowed_receiver_instance_type_range_end);
}

Local<FunctionTemplate> FunctionTemplate::NewWithCFunctionOverloads(Isolate* v8_isolate, FunctionCallback callback, v8::Local<Value> data, v8::Local<Signature> signature,
    int length, ConstructorBehavior behavior, SideEffectType side_effect_type, const MemorySpan<const CFunction>& c_function_overloads)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    API_RCS_SCOPE(i_isolate, FunctionTemplate, New);

    if (!Utils::ApiCheck(c_function_overloads.size() == 0 || behavior == ConstructorBehavior::kThrow, "FunctionTemplate::NewWithCFunctionOverloads",
            "Fast API calls are not supported for constructor functions")) {
        return Local<FunctionTemplate>();
    }

    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    return FunctionTemplateNew(i_isolate, callback, data, signature, length, behavior, false, Local<Private>(), side_effect_type, c_function_overloads);
}

Local<FunctionTemplate> FunctionTemplate::NewWithCache(
    Isolate* v8_isolate, FunctionCallback callback, Local<Private> cache_property, Local<Value> data, Local<Signature> signature, int length, SideEffectType side_effect_type)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    API_RCS_SCOPE(i_isolate, FunctionTemplate, NewWithCache);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    return FunctionTemplateNew(i_isolate, callback, data, signature, length, ConstructorBehavior::kAllow, false, cache_property, side_effect_type);
}

Local<Signature> Signature::New(Isolate* v8_isolate, Local<FunctionTemplate> receiver) { return Utils::SignatureToLocal(Utils::OpenHandle(*receiver)); }
...
void FunctionTemplate::SetCallHandler(FunctionCallback callback, v8::Local<Value> data, SideEffectType side_effect_type, const MemorySpan<const CFunction>& c_function_overloads)
{
    auto info = Utils::OpenHandle(this);
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::HandleScope scope(i_isolate);
    i::Handle<i::CallHandlerInfo> obj = i_isolate->factory()->NewCallHandlerInfo(side_effect_type == SideEffectType::kHasNoSideEffect);
    ...
    if (c_function_overloads.size() > 0) {
        ...
        for (int i = 0; i < function_count; i++) {
            ...
            i::Handle<i::Object> address = FromCData(i_isolate, c_function.GetAddress());
            function_overloads->set(i::FunctionTemplateInfo::kFunctionOverloadEntrySize * i, *address);
            i::Handle<i::Object> signature = FromCData(i_isolate, c_function.GetTypeInfo());
...
i::Handle<i::AccessorInfo> MakeAccessorInfo(i::Isolate* i_isolate, v8::Local<Name> name, Getter getter, Setter setter, v8::Local<Value> data, v8::AccessControl settings,
    bool is_special_data_property, bool replace_on_access)
{
    ...
    DCHECK_IMPLIES(replace_on_access, is_special_data_property && setter == nullptr);
    ...
    if (!accessor_name->IsUniqueName()) {
        accessor_name = i_isolate->factory()->InternalizeString(i::Handle<i::String>::cast(accessor_name));
    }
    i::DisallowGarbageCollection no_gc;
    ...
    if (settings & ALL_CAN_READ)
        raw_obj.set_all_can_read(true);
    if (settings & ALL_CAN_WRITE)
        raw_obj.set_all_can_write(true);
...
Local<ObjectTemplate> FunctionTemplate::InstanceTemplate()
{
    ...
    if (!Utils::ApiCheck(!handle.is_null(), "v8::FunctionTemplate::InstanceTemplate()", "Reading from empty handle")) {
        return Local<ObjectTemplate>();
    }
    i::Isolate* i_isolate = handle->GetIsolate();
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    if (handle->GetInstanceTemplate().IsUndefined(i_isolate)) {
        Local<ObjectTemplate> templ = ObjectTemplate::New(i_isolate, ToApiHandle<FunctionTemplate>(handle));
        i::FunctionTemplateInfo::SetInstanceTemplate(i_isolate, handle, Utils::OpenHandle(*templ));
    }
    i::Handle<i::ObjectTemplateInfo> result(i::ObjectTemplateInfo::cast(handle->GetInstanceTemplate()), i_isolate);
    return Utils::ToLocal(result);
}
...
Local<ObjectTemplate> ObjectTemplate::New(Isolate* v8_isolate, v8::Local<FunctionTemplate> constructor) { return New(reinterpret_cast<i::Isolate*>(v8_isolate), constructor); }

Local<ObjectTemplate> ObjectTemplate::New(i::Isolate* i_isolate, v8::Local<FunctionTemplate> constructor) { return ObjectTemplateNew(i_isolate, constructor, false); }
...
void TemplateSetAccessor(Template* template_obj, v8::Local<Name> name, Getter getter, Setter setter, Data data, AccessControl settings, PropertyAttribute attribute,
    bool is_special_data_property, bool replace_on_access, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    auto info = Utils::OpenHandle(template_obj);
    auto i_isolate = info->GetIsolate();
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::HandleScope scope(i_isolate);
    i::Handle<i::AccessorInfo> accessor_info = MakeAccessorInfo(i_isolate, name, getter, setter, data, settings, is_special_data_property, replace_on_access);
    {
        i::DisallowGarbageCollection no_gc;
...
void Template::SetNativeDataProperty(v8::Local<String> name, AccessorGetterCallback getter, AccessorSetterCallback setter, v8::Local<Value> data, PropertyAttribute attribute,
    AccessControl settings, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    TemplateSetAccessor(this, name, getter, setter, data, settings, attribute, true, false, getter_side_effect_type, setter_side_effect_type);
}

void Template::SetNativeDataProperty(v8::Local<Name> name, AccessorNameGetterCallback getter, AccessorNameSetterCallback setter, v8::Local<Value> data, PropertyAttribute attribute,
    AccessControl settings, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    TemplateSetAccessor(this, name, getter, setter, data, settings, attribute, true, false, getter_side_effect_type, setter_side_effect_type);
}
...
{
    TemplateSetAccessor(
        this, name, getter, static_cast<AccessorNameSetterCallback>(nullptr), data, DEFAULT, attribute, true, true, getter_side_effect_type, setter_side_effect_type);
}
...
void ObjectTemplate::SetAccessor(v8::Local<String> name, AccessorGetterCallback getter, AccessorSetterCallback setter, v8::Local<Value> data, AccessControl settings,
    PropertyAttribute attribute, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    TemplateSetAccessor(this, name, getter, setter, data, settings, attribute, i::v8_flags.disable_old_api_accessors, false, getter_side_effect_type, setter_side_effect_type);
}

void ObjectTemplate::SetAccessor(v8::Local<Name> name, AccessorNameGetterCallback getter, AccessorNameSetterCallback setter, v8::Local<Value> data, AccessControl settings,
    PropertyAttribute attribute, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    TemplateSetAccessor(this, name, getter, setter, data, settings, attribute, i::v8_flags.disable_old_api_accessors, false, getter_side_effect_type, setter_side_effect_type);
}
...
i::Handle<i::InterceptorInfo> CreateInterceptorInfo(i::Isolate* i_isolate, Getter getter, Setter setter, Query query, Descriptor descriptor, Deleter remover, Enumerator enumerator,
    Definer definer, Local<Value> data, PropertyHandlerFlags flags)
{
    auto obj = i::Handle<i::InterceptorInfo>::cast(i_isolate->factory()->NewStruct(i::INTERCEPTOR_INFO_TYPE, i::AllocationType::kOld));
    obj->set_flags(0);
    ...
    if (query != nullptr)
        SET_FIELD_WRAPPED(i_isolate, obj, set_query, query);
    ...
    obj->set_can_intercept_symbols(!(static_cast<int>(flags) & static_cast<int>(PropertyHandlerFlags::kOnlyInterceptStrings)));
    ...
    obj->set_has_no_side_effect(static_cast<int>(flags) & static_cast<int>(PropertyHandlerFlags::kHasNoSideEffect));
...
template <typename Getter, typename Setter, typename Query, typename Descriptor, typename Deleter, typename Enumerator, typename Definer>
i::Handle<i::InterceptorInfo> CreateNamedInterceptorInfo(i::Isolate* i_isolate, Getter getter, Setter setter, Query query, Descriptor descriptor, Deleter remover,
    Enumerator enumerator, Definer definer, Local<Value> data, PropertyHandlerFlags flags)
{
    auto interceptor = CreateInterceptorInfo(i_isolate, getter, setter, query, descriptor, remover, enumerator, definer, data, flags);
    interceptor->set_is_named(true);
    return interceptor;
}

template <typename Getter, typename Setter, typename Query, typename Descriptor, typename Deleter, typename Enumerator, typename Definer>
i::Handle<i::InterceptorInfo> CreateIndexedInterceptorInfo(i::Isolate* i_isolate, Getter getter, Setter setter, Query query, Descriptor descriptor, Deleter remover,
    Enumerator enumerator, Definer definer, Local<Value> data, PropertyHandlerFlags flags)
{
    auto interceptor = CreateInterceptorInfo(i_isolate, getter, setter, query, descriptor, remover, enumerator, definer, data, flags);
...
template <typename Getter, typename Setter, typename Query, typename Descriptor, typename Deleter, typename Enumerator, typename Definer>
void ObjectTemplateSetNamedPropertyHandler(ObjectTemplate* templ, Getter getter, Setter setter, Query query, Descriptor descriptor, Deleter remover, Enumerator enumerator,
    Definer definer, Local<Value> data, PropertyHandlerFlags flags)
{
...
void ObjectTemplate::SetAccessCheckCallbackAndHandler(
    AccessCheckCallback callback, const NamedPropertyHandlerConfiguration& named_handler, const IndexedPropertyHandlerConfiguration& indexed_handler, Local<Value> data)
{
    i::Isolate* i_isolate = Utils::OpenHandle(this)->GetIsolate();
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::HandleScope scope(i_isolate);
    ...
    SET_FIELD_WRAPPED(i_isolate, info, set_callback, callback);
    auto named_interceptor = CreateNamedInterceptorInfo(i_isolate, named_handler.getter, named_handler.setter, named_handler.query, named_handler.descriptor, named_handler.deleter,
        named_handler.enumerator, named_handler.definer, named_handler.data, named_handler.flags);
    info->set_named_interceptor(*named_interceptor);
    auto indexed_interceptor = CreateIndexedInterceptorInfo(i_isolate, indexed_handler.getter, indexed_handler.setter, indexed_handler.query, indexed_handler.descriptor,
        indexed_handler.deleter, indexed_handler.enumerator, indexed_handler.definer, indexed_handler.data, indexed_handler.flags);
...
int ObjectTemplate::InternalFieldCount() const { return Utils::OpenHandle(this)->embedder_field_count(); }
...
bool ObjectTemplate::IsImmutableProto() const { return Utils::OpenHandle(this)->immutable_proto(); }
...
bool ObjectTemplate::IsCodeLike() const { return Utils::OpenHandle(this)->code_like(); }
...
namespace {
inline Local<Value> GetSharedFunctionInfoSourceMappingURL(i::Isolate* isolate, i::Handle<i::SharedFunctionInfo> obj)
{
...
inline Local<Value> GetSharedFunctionInfoSourceURL(i::Isolate* isolate, i::Handle<i::SharedFunctionInfo> obj)
{
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(isolate);
    if (obj->script().IsScript()) {
...
Local<Script> UnboundScript::BindToCurrentContext()
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::JSFunction> function = i::Factory::JSFunctionBuilder { i_isolate, function_info, i_isolate->native_context() }.Build();
...
MaybeLocal<Value> Script::Run(Local<Context> context) { return Run(context, Local<Data>()); }
...
MaybeLocal<Value> Script::Run(Local<Context> context, Local<Data> host_defined_options)
{
    ...
    ENTER_V8(i_isolate, context, Script, Run, MaybeLocal<Value>(), InternalEscapableScope);
    ...
    if (i::v8_flags.script_delay_once > 0 && !i_isolate->did_run_script_delay()) {
        delta = v8::base::TimeDelta::FromMillisecondsD(i::v8_flags.script_delay_once);
    ...
    if (V8_UNLIKELY(i::v8_flags.experimental_web_snapshots)) {
        i::Handle<i::HeapObject> maybe_script = handle(fun->shared().script(), i_isolate);
        if (maybe_script->IsScript() && i::Script::cast(*maybe_script).type() == i::Script::TYPE_WEB_SNAPSHOT) {
            i::WebSnapshotDeserializer deserializer(reinterpret_cast<i::Isolate*>(v8_isolate), i::Handle<i::Script>::cast(maybe_script));
    ...
    i::Handle<i::Object> options(i::Script::cast(fun->shared().script()).host_defined_options(), i_isolate);
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::CallScript(i_isolate, fun, receiver, options), &result);

    if (i::v8_flags.script_delay_fraction > 0.0) {
        delta = v8::base::TimeDelta::FromMillisecondsD(timer.Elapsed().InMillisecondsF() * i::v8_flags.script_delay_fraction);
        timer.Restart();
        while (timer.Elapsed() < delta) {
            // Busy wait.
        }
...
Local<Value> Script::GetResourceName()
{
    ...
    return ToApiHandle<Value>(i::handle(i::Script::cast(sfi.script()).name(), i_isolate));
}
...
void v8::PrimitiveArray::CheckCast(v8::Data* that)
{
    ...
    Utils::ApiCheck(obj->IsFixedArray(), "v8::PrimitiveArray::Cast",
...
int ModuleRequest::GetSourceOffset() const { return Utils::OpenHandle(this)->position(); }

Local<FixedArray> ModuleRequest::GetImportAssertions() const
{
    i::Handle<i::ModuleRequest> self = Utils::OpenHandle(this);
    i::Isolate* i_isolate = self->GetIsolate();
    return ToApiHandle<FixedArray>(i::handle(self->import_assertions(), i_isolate));
}
...
Local<FixedArray> Module::GetModuleRequests() const
{
    ...
    if (self->IsSyntheticModule()) {
        ...
        i::Handle<i::FixedArray> module_requests(i::Handle<i::SourceTextModule>::cast(self)->info().module_requests(), i_isolate);
...
Location Module::SourceOffsetToLocation(int offset) const
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::HandleScope scope(i_isolate);
    Utils::ApiCheck(self->IsSourceTextModule(), "v8::Module::SourceOffsetToLocation", "v8::Module::SourceOffsetToLocation must be used on an SourceTextModule");
    i::Handle<i::Script> script(i::Handle<i::SourceTextModule>::cast(self)->GetScript(), i_isolate);
...
Local<Value> Module::GetModuleNamespace()
{
    ...
    DCHECK_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::JSModuleNamespace> module_namespace = i::Module::GetModuleNamespace(i_isolate, self);
...
{
    i::Handle<i::Module> self = Utils::OpenHandle(this);
    Utils::ApiCheck(self->IsSourceTextModule(), "v8::Module::GetUnboundModuleScript", "v8::Module::GetUnboundModuleScript must be used on an SourceTextModule");
    auto i_isolate = self->GetIsolate();
    DCHECK_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    return ToApiHandle<UnboundModuleScript>(i::handle(i::Handle<i::SourceTextModule>::cast(self)->GetSharedFunctionInfo(), i_isolate));
}

int Module::ScriptId() const
{
    i::Module self = *Utils::OpenHandle(this);
    Utils::ApiCheck(self.IsSourceTextModule(), "v8::Module::ScriptId", "v8::Module::ScriptId must be used on an SourceTextModule");
...
Maybe<bool> Module::InstantiateModule(Local<Context> context, Module::ResolveModuleCallback callback)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Module, InstantiateModule, Nothing<bool>(), i::HandleScope);
    has_pending_exception = !i::Module::Instantiate(i_isolate, Utils::OpenHandle(this), context, callback, nullptr);
...
MaybeLocal<Value> Module::Evaluate(Local<Context> context)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.Execute");
    ENTER_V8(i_isolate, context, Module, Evaluate, MaybeLocal<Value>(), InternalEscapableScope);
    i::TimerEventScope<i::TimerEventExecute> timer_scope(i_isolate);
    i::NestedTimedHistogramScope execute_timer(i_isolate->counters()->execute(), i_isolate);
    ...
    Local<Value> result;
    has_pending_exception = !ToLocal(i::Module::Evaluate(i_isolate, self), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}

Local<Module> Module::CreateSyntheticModule(
    Isolate* v8_isolate, Local<String> module_name, const std::vector<Local<v8::String>>& export_names, v8::Module::SyntheticModuleEvaluationSteps evaluation_steps)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::String> i_module_name = Utils::OpenHandle(*module_name);
    i::Handle<i::FixedArray> i_export_names = i_isolate->factory()->NewFixedArray(static_cast<int>(export_names.size()));
    for (int i = 0; i < i_export_names->length(); ++i) {
        i::Handle<i::String> str = i_isolate->factory()->InternalizeString(Utils::OpenHandle(*export_names[i]));
...
Maybe<bool> Module::SetSyntheticModuleExport(Isolate* v8_isolate, Local<String> export_name, Local<v8::Value> export_value)
{
    ...
    i::Handle<i::Module> self = Utils::OpenHandle(this);
    Utils::ApiCheck(self->IsSyntheticModule(), "v8::Module::SyntheticModuleSetExport",
        "v8::Module::SyntheticModuleSetExport must only be called on "
        "a SyntheticModule");
    ENTER_V8_NO_SCRIPT(i_isolate, v8_isolate->GetCurrentContext(), Module, SetSyntheticModuleExport, Nothing<bool>(), i::HandleScope);
    has_pending_exception = i::SyntheticModule::SetExport(i_isolate, i::Handle<i::SyntheticModule>::cast(self), i_export_name, i_export_value).IsNothing();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(true);
}

std::vector<std::tuple<Local<Module>, Local<Message>>> Module::GetStalledTopLevelAwaitMessage(Isolate* isolate)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    i::Handle<i::Module> self = Utils::OpenHandle(this);
    Utils::ApiCheck(self->IsSourceTextModule(), "v8::Module::GetStalledTopLevelAwaitMessage",
    ...
    std::vector<std::tuple<i::Handle<i::SourceTextModule>, i::Handle<i::JSMessageObject>>> stalled_awaits
        = i::Handle<i::SourceTextModule>::cast(self)->GetStalledTopLevelAwaitMessage(i_isolate);
    ...
    for (size_t i = 0; i < stalled_awaits_count; ++i) {
        ...
        result.push_back(std::make_tuple(ToApiHandle<Module>(module), ToApiHandle<Message>(message)));
    }
    return result;
}
...
i::ScriptDetails GetScriptDetails(i::Isolate* i_isolate, Local<Value> resource_name, int resource_line_offset, int resource_column_offset, Local<Value> source_map_url,
    Local<Data> host_defined_options, ScriptOriginOptions origin_options)
{
    ...
    script_details.host_defined_options = host_defined_options.IsEmpty() ? i_isolate->factory()->empty_fixed_array() : Utils::OpenHandle(*(host_defined_options));
...
MaybeLocal<UnboundScript> ScriptCompiler::CompileUnboundInternal(Isolate* v8_isolate, Source* source, CompileOptions options, NoCacheReason no_cache_reason)
{
    ...
    ENTER_V8_NO_SCRIPT(i_isolate, v8_isolate->GetCurrentContext(), ScriptCompiler, CompileUnbound, MaybeLocal<UnboundScript>(), InternalEscapableScope);

    i::Handle<i::String> str = Utils::OpenHandle(*(source->source_string));

    i::Handle<i::SharedFunctionInfo> result;
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.CompileScript");
    i::ScriptDetails script_details = GetScriptDetails(i_isolate, source->resource_name, source->resource_line_offset, source->resource_column_offset, source->source_map_url,
    ...
    if (options == kConsumeCodeCache) {
        if (source->consume_cache_task) {
            ...
            maybe_function_info = i::Compiler::GetSharedFunctionInfoForScriptWithDeserializeTask(
                i_isolate, str, script_details, deserialize_task.get(), options, no_cache_reason, i::NOT_NATIVES_CODE);
            ...
            maybe_function_info
                = i::Compiler::GetSharedFunctionInfoForScriptWithCachedData(i_isolate, str, script_details, cached_data.get(), options, no_cache_reason, i::NOT_NATIVES_CODE);
            source->cached_data->rejected = cached_data->rejected();
        }
    } else {
        // Compile without any cache.
        maybe_function_info = i::Compiler::GetSharedFunctionInfoForScript(i_isolate, str, script_details, options, no_cache_reason, i::NOT_NATIVES_CODE);
    }
...
{
    Utils::ApiCheck(!source->GetResourceOptions().IsModule(), "v8::ScriptCompiler::CompileUnboundScript", "v8::ScriptCompiler::CompileModule must be used to compile modules");
...
{
    Utils::ApiCheck(!source->GetResourceOptions().IsModule(), "v8::ScriptCompiler::Compile", "v8::ScriptCompiler::CompileModule must be used to compile modules");
...
{
    Utils::ApiCheck(options == kNoCompileOptions || options == kConsumeCodeCache, "v8::ScriptCompiler::CompileModule", "Invalid CompileOptions");
...
// static
V8_WARN_UNUSED_RESULT MaybeLocal<Function> ScriptCompiler::CompileFunction(Local<Context> context, Source* source, size_t arguments_count, Local<String> arguments[],
    size_t context_extension_count, Local<Object> context_extensions[], CompileOptions options, NoCacheReason no_cache_reason)
{
    return CompileFunctionInternal(context, source, arguments_count, arguments, context_extension_count, context_extensions, options, no_cache_reason, nullptr);
}

#ifdef V8_SCRIPTORMODULE_LEGACY_LIFETIME
// static
MaybeLocal<Function> ScriptCompiler::CompileFunctionInContext(Local<Context> context, Source* source, size_t arguments_count, Local<String> arguments[],
    size_t context_extension_count, Local<Object> context_extensions[], CompileOptions options, NoCacheReason no_cache_reason, Local<ScriptOrModule>* script_or_module_out)
{
    return CompileFunctionInternal(context, source, arguments_count, arguments, context_extension_count, context_extensions, options, no_cache_reason, script_or_module_out);
}
#endif // V8_SCRIPTORMODULE_LEGACY_LIFETIME

MaybeLocal<Function> ScriptCompiler::CompileFunctionInternal(Local<Context> v8_context, Source* source, size_t arguments_count, Local<String> arguments[],
    size_t context_extension_count, Local<Object> context_extensions[], CompileOptions options, NoCacheReason no_cache_reason, Local<ScriptOrModule>* script_or_module_out)
{
    Local<Function> result;

    {
        PREPARE_FOR_EXECUTION(v8_context, ScriptCompiler, CompileFunction, Function);
        TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.ScriptCompiler");
        ...
        for (size_t i = 0; i < context_extension_count; ++i) {
            ...
            context = i_isolate->factory()->NewWithContext(context,
                i::ScopeInfo::CreateForWithScope(
                    i_isolate, context->IsNativeContext() ? i::Handle<i::ScopeInfo>::null() : i::Handle<i::ScopeInfo>(context->scope_info(), i_isolate)),
                extension);
        }
        ...
        has_pending_exception
            = !i::Compiler::GetWrappedFunction(Utils::OpenHandle(*source->source_string), arguments_list, context, script_details, cached_data.get(), options, no_cache_reason)
    ...
    if (script_or_module_out != nullptr) {
        i::Handle<i::JSFunction> function = i::Handle<i::JSFunction>::cast(Utils::OpenHandle(*result));
        ...
        auto script_or_module = i::Handle<i::ScriptOrModule>::cast(i_isolate->factory()->NewStruct(i::SCRIPT_OR_MODULE_TYPE));
        script_or_module->set_resource_name(script->name());
        script_or_module->set_host_defined_options(script->host_defined_options());
#ifdef V8_SCRIPTORMODULE_LEGACY_LIFETIME
        i::Handle<i::ArrayList> list = i::handle(script->script_or_modules(), i_isolate);
...
{
    Utils::ApiCheck(options == kNoCompileOptions || options == kEagerCompile, "v8::ScriptCompiler::StartStreaming", "Invalid CompileOptions");
    if (!i::v8_flags.script_streaming)
        return nullptr;
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i::ScriptStreamingData* data = source->impl();
    std::unique_ptr<i::BackgroundCompileTask> task = std::make_unique<i::BackgroundCompileTask>(data, i_isolate, type, options);
...
void ScriptCompiler::ConsumeCodeCacheTask::SourceTextAvailable(Isolate* v8_isolate, Local<String> source_text, const ScriptOrigin& origin)
{
    ...
    i::ScriptDetails script_details
        = GetScriptDetails(i_isolate, origin.ResourceName(), origin.LineOffset(), origin.ColumnOffset(), origin.SourceMapUrl(), origin.GetHostDefinedOptions(), origin.Options());
    ...
    i::Isolate* i_isolate, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
    ...
    return i::Compiler::GetSharedFunctionInfoForStreamedScript(i_isolate, str, script_details, data);
}
...
MaybeLocal<Script> ScriptCompiler::Compile(Local<Context> context, StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
    ...
    i::MaybeHandle<i::SharedFunctionInfo> maybe_sfi = CompileStreamedSource(i_isolate, v8_source, full_source_string, origin);
...
MaybeLocal<Module> ScriptCompiler::CompileModule(Local<Context> context, StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
...
{
    return static_cast<uint32_t>(base::hash_combine(internal::Version::Hash(), internal::FlagList::Hash(), static_cast<uint32_t>(internal::CpuFeatures::SupportedFeatures())));
}

ScriptCompiler::CachedData* ScriptCompiler::CreateCodeCache(Local<UnboundScript> unbound_script)
{
    i::Handle<i::SharedFunctionInfo> shared = i::Handle<i::SharedFunctionInfo>::cast(Utils::OpenHandle(*unbound_script));
...
{
    i::Handle<i::SharedFunctionInfo> shared = i::Handle<i::SharedFunctionInfo>::cast(Utils::OpenHandle(*unbound_module_script));
    DCHECK_NO_SCRIPT_NO_EXCEPTION(shared->GetIsolate());
    DCHECK(shared->is_toplevel());
    return i::CodeSerializer::Serialize(shared);
}
...
ScriptCompiler::CachedData* ScriptCompiler::CreateCodeCacheForFunction(Local<Function> function)
{
    ...
    i::Handle<i::SharedFunctionInfo> shared(js_function->shared(), js_function->GetIsolate());
    DCHECK_NO_SCRIPT_NO_EXCEPTION(shared->GetIsolate());
...
v8::TryCatch::~TryCatch()
{
    if (rethrow_) {
        ...
        v8::Local<v8::Value> exc = v8::Local<v8::Value>::New(v8_isolate, Exception());
...
bool v8::TryCatch::HasCaught() const { return !i::Object(reinterpret_cast<i::Address>(exception_)).IsTheHole(i_isolate_); }
...
MaybeLocal<Value> v8::TryCatch::StackTrace(Local<Context> context, Local<Value> exception)
{
    ...
    if (!maybe.FromJust())
        return v8::Local<Value>();
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::JSReceiver::GetProperty(i_isolate, obj, name), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}
...
void ScriptOrigin::VerifyHostDefinedOptions() const
{
    ...
    i::Handle<i::FixedArray> options = Utils::OpenHandle(*host_defined_options_.As<FixedArray>());
    for (int i = 0; i < options->length(); i++) {
        Utils::ApiCheck(options->get(i).IsPrimitive(), "ScriptOrigin()", "PrimitiveArray can only contain primtive values");
    }
}
...
Maybe<int> Message::GetStartColumn(Local<Context> context) const { return Just(GetStartColumn()); }
...
Maybe<int> Message::GetEndColumn(Local<Context> context) const { return Just(GetEndColumn()); }
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i::Handle<i::StackFrameInfo> info(i::StackFrameInfo::cast(Utils::OpenHandle(this)->get(index)), i_isolate);
    return Utils::StackFrameToLocal(info);
}

int StackTrace::GetFrameCount() const { return Utils::OpenHandle(this)->length(); }

Local<StackTrace> StackTrace::CurrentStackTrace(Isolate* v8_isolate, int frame_limit, StackTraceOptions options)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::FixedArray> stackTrace = i_isolate->CaptureDetailedStackTrace(frame_limit, options);
...
Location StackFrame::GetLocation() const
{
    ...
    i::Handle<i::Script> script(self->script(), i_isolate);
    i::Script::PositionInfo info;
    CHECK(i::Script::GetPositionInfo(script, i::StackFrameInfo::GetSourcePosition(self), &info, i::Script::WITH_OFFSET));
...
int StackFrame::GetScriptId() const { return Utils::OpenHandle(this)->script().id(); }
...
Local<String> StackFrame::GetScriptNameOrSourceURL() const
{
    i::Handle<i::StackFrameInfo> self = Utils::OpenHandle(this);
    i::Isolate* i_isolate = self->GetIsolate();
    i::Handle<i::Object> name_or_source_url(self->script().GetNameOrSourceURL(), i_isolate);
    if (!name_or_source_url->IsString())
        return {};
...
Local<String> StackFrame::GetScriptSourceMappingURL() const
{
    i::Handle<i::StackFrameInfo> self = Utils::OpenHandle(this);
    i::Isolate* i_isolate = self->GetIsolate();
    i::Handle<i::Object> source_mapping_url(self->script().source_mapping_url(), i_isolate);
...
bool StackFrame::IsConstructor() const { return Utils::OpenHandle(this)->is_constructor(); }

bool StackFrame::IsWasm() const { return !IsUserJavaScript(); }

bool StackFrame::IsUserJavaScript() const { return Utils::OpenHandle(this)->script().IsUserJavaScript(); }
...
MaybeLocal<Value> JSON::Parse(Local<Context> context, Local<String> json_string)
{
    ...
    auto maybe = source->IsOneByteRepresentation() ? i::JsonParser<uint8_t>::Parse(i_isolate, source, undefined) : i::JsonParser<uint16_t>::Parse(i_isolate, source, undefined);
    Local<Value> result;
...
MaybeLocal<String> JSON::Stringify(Local<Context> context, Local<Value> json_object, Local<String> gap)
{
    ...
    has_pending_exception = !i::JsonStringify(i_isolate, object, replacer, gap_string).ToHandle(&maybe);
    RETURN_ON_FAILED_EXECUTION(String);
    Local<String> result;
    has_pending_exception = !ToLocal<String>(i::Object::ToString(i_isolate, maybe), &result);
    RETURN_ON_FAILED_EXECUTION(String);
    RETURN_ESCAPED(result);
}
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i_isolate->ScheduleThrow(*i_isolate->factory()->NewError(i_isolate->error_function(), i::MessageTemplate::kDataCloneError, Utils::OpenHandle(*object)));
    return Nothing<bool>();
}
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i_isolate->ScheduleThrow(*i_isolate->factory()->NewError(i_isolate->error_function(), i::MessageTemplate::kDataCloneError, Utils::OpenHandle(*shared_array_buffer)));
    return Nothing<uint32_t>();
}

Maybe<uint32_t> ValueSerializer::Delegate::GetWasmModuleTransferId(Isolate* v8_isolate, Local<WasmModuleObject> module) { return Nothing<uint32_t>(); }

bool ValueSerializer::Delegate::AdoptSharedValueConveyor(Isolate* v8_isolate, SharedValueConveyor&& conveyor)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i_isolate->ScheduleThrow(
        *i_isolate->factory()->NewError(i_isolate->error_function(), i::MessageTemplate::kDataCloneError, i_isolate->factory()->NewStringFromAsciiChecked("shared value")));
    return false;
}
...
void ValueSerializer::Delegate::FreeBufferMemory(void* buffer) { return base::Free(buffer); }
...
void ValueSerializer::SetTreatArrayBufferViewsAsHostObjects(bool mode) { private_->serializer.SetTreatArrayBufferViewsAsHostObjects(mode); }

Maybe<bool> ValueSerializer::WriteValue(Local<Context> context, Local<Value> value)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, ValueSerializer, WriteValue, Nothing<bool>(), i::HandleScope);
...
std::pair<uint8_t*, size_t> ValueSerializer::Release() { return private_->serializer.Release(); }
...
void ValueSerializer::WriteUint32(uint32_t value) { private_->serializer.WriteUint32(value); }

void ValueSerializer::WriteUint64(uint64_t value) { private_->serializer.WriteUint64(value); }

void ValueSerializer::WriteDouble(double value) { private_->serializer.WriteDouble(value); }

void ValueSerializer::WriteRawBytes(const void* source, size_t length) { private_->serializer.WriteRawBytes(source, length); }

MaybeLocal<Object> ValueDeserializer::Delegate::ReadHostObject(Isolate* v8_isolate)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i_isolate->ScheduleThrow(*i_isolate->factory()->NewError(i_isolate->error_function(), i::MessageTemplate::kDataCloneDeserializationError));
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, ValueDeserializer, ReadHeader, Nothing<bool>(), i::HandleScope);
...
void ValueDeserializer::SetSupportsLegacyWireFormat(bool supports_legacy_wire_format) { private_->supports_legacy_wire_format = supports_legacy_wire_format; }

uint32_t ValueDeserializer::GetWireFormatVersion() const { return private_->deserializer.GetWireFormatVersion(); }
...
bool ValueDeserializer::ReadUint32(uint32_t* value) { return private_->deserializer.ReadUint32(value); }

bool ValueDeserializer::ReadUint64(uint64_t* value) { return private_->deserializer.ReadUint64(value); }

bool ValueDeserializer::ReadDouble(double* value) { return private_->deserializer.ReadDouble(value); }

bool ValueDeserializer::ReadRawBytes(size_t length, const void** data) { return private_->deserializer.ReadRawBytes(length, data); }
...
bool Value::IsSymbol() const { return Utils::OpenHandle(this)->IsPublicSymbol(); }
...
bool Value::IsArrayBufferView() const { return Utils::OpenHandle(this)->IsJSArrayBufferView(); }

bool Value::IsTypedArray() const { return Utils::OpenHandle(this)->IsJSTypedArray(); }
...
bool Value::IsDataView() const { return Utils::OpenHandle(this)->IsJSDataView(); }
...
bool Value::IsNativeError() const { return Utils::OpenHandle(this)->IsJSError(); }
...
bool Value::IsGeneratorObject() const { return Utils::OpenHandle(this)->IsJSGeneratorObject(); }

bool Value::IsMapIterator() const { return Utils::OpenHandle(this)->IsJSMapIterator(); }

bool Value::IsSetIterator() const { return Utils::OpenHandle(this)->IsJSSetIterator(); }

bool Value::IsPromise() const { return Utils::OpenHandle(this)->IsJSPromise(); }

bool Value::IsModuleNamespaceObject() const { return Utils::OpenHandle(this)->IsJSModuleNamespace(); }
...
MaybeLocal<String> Value::ToString(Local<Context> context) const
{
    ...
    Local<String> result;
    has_pending_exception = !ToLocal<String>(i::Object::ToString(i_isolate, obj), &result);
    RETURN_ON_FAILED_EXECUTION(String);
    RETURN_ESCAPED(result);
}
...
MaybeLocal<String> Value::ToDetailString(Local<Context> context) const
{
    ...
    Local<String> result = Utils::ToLocal(i::Object::NoSideEffectsToString(i_isolate, obj));
    RETURN_ON_FAILED_EXECUTION(String);
    RETURN_ESCAPED(result);
}
...
MaybeLocal<Object> Value::ToObject(Local<Context> context) const
{
    ...
    Local<Object> result;
    has_pending_exception = !ToLocal<Object>(i::Object::ToObject(i_isolate, obj), &result);
...
MaybeLocal<BigInt> Value::ToBigInt(Local<Context> context) const
{
    ...
    Local<BigInt> result;
    has_pending_exception = !ToLocal<BigInt>(i::BigInt::FromObject(i_isolate, obj), &result);
    RETURN_ON_FAILED_EXECUTION(BigInt);
    RETURN_ESCAPED(result);
}

bool Value::BooleanValue(Isolate* v8_isolate) const { return Utils::OpenHandle(this)->BooleanValue(reinterpret_cast<i::Isolate*>(v8_isolate)); }
...
MaybeLocal<Number> Value::ToNumber(Local<Context> context) const
{
    ...
    Local<Number> result;
    has_pending_exception = !ToLocal<Number>(i::Object::ToNumber(i_isolate, obj), &result);
...
MaybeLocal<Integer> Value::ToInteger(Local<Context> context) const
{
    ...
    has_pending_exception = !ToLocal<Integer>(i::Object::ToInteger(i_isolate, obj), &result);
...
MaybeLocal<Int32> Value::ToInt32(Local<Context> context) const
{
    ...
    has_pending_exception = !ToLocal<Int32>(i::Object::ToInt32(i_isolate, obj), &result);
...
MaybeLocal<Uint32> Value::ToUint32(Local<Context> context) const
{
    ...
    has_pending_exception = !ToLocal<Uint32>(i::Object::ToUint32(i_isolate, obj), &result);
    RETURN_ON_FAILED_EXECUTION(Uint32);
    RETURN_ESCAPED(result);
}

i::Isolate* i::IsolateFromNeverReadOnlySpaceObject(i::Address obj) { return i::GetIsolateFromWritableObject(i::HeapObject::cast(i::Object(obj))); }

bool i::ShouldThrowOnError(i::Isolate* i_isolate) { return i::GetShouldThrow(i_isolate, Nothing<i::ShouldThrow>()) == i::ShouldThrow::kThrowOnError; }
...
void v8::Value::CheckCast(Data* that) { Utils::ApiCheck(that->IsValue(), "v8::Value::Cast", "Data is not a Value"); }

void External::CheckCast(v8::Value* that) { Utils::ApiCheck(that->IsExternal(), "v8::External::Cast", "Value is not an External"); }

void v8::Object::CheckCast(Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSReceiver(), "v8::Object::Cast", "Value is not an Object");
}

void v8::Function::CheckCast(Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsCallable(), "v8::Function::Cast", "Value is not a Function");
}

void v8::Boolean::CheckCast(v8::Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsBoolean(), "v8::Boolean::Cast", "Value is not a Boolean");
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsSymbol() && i::Handle<i::Symbol>::cast(obj)->is_private(), "v8::Private::Cast", "Value is not a Private");
}

void v8::FixedArray::CheckCast(v8::Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsFixedArray(), "v8::FixedArray::Cast", "Value is not a FixedArray");
}

void v8::ModuleRequest::CheckCast(v8::Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsModuleRequest(), "v8::ModuleRequest::Cast", "Value is not a ModuleRequest");
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsNumber(), "v8::Number::Cast()", "Value is not a Number");
}

void v8::Integer::CheckCast(v8::Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsNumber(), "v8::Integer::Cast", "Value is not an Integer");
}

void v8::Int32::CheckCast(v8::Data* that) { Utils::ApiCheck(Value::Cast(that)->IsInt32(), "v8::Int32::Cast", "Value is not a 32-bit signed integer"); }

void v8::Uint32::CheckCast(v8::Data* that) { Utils::ApiCheck(Value::Cast(that)->IsUint32(), "v8::Uint32::Cast", "Value is not a 32-bit unsigned integer"); }

void v8::BigInt::CheckCast(v8::Data* that) { Utils::ApiCheck(Value::Cast(that)->IsBigInt(), "v8::BigInt::Cast", "Value is not a BigInt"); }

void v8::Context::CheckCast(v8::Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsContext(), "v8::Context::Cast", "Value is not a Context");
}
...
void v8::Promise::CheckCast(Value* that) { Utils::ApiCheck(that->IsPromise(), "v8::Promise::Cast", "Value is not a Promise"); }

void v8::Promise::Resolver::CheckCast(Value* that) { Utils::ApiCheck(that->IsPromise(), "v8::Promise::Resolver::Cast", "Value is not a Promise::Resolver"); }

void v8::Proxy::CheckCast(Value* that) { Utils::ApiCheck(that->IsProxy(), "v8::Proxy::Cast", "Value is not a Proxy"); }

void v8::WasmMemoryObject::CheckCast(Value* that) { Utils::ApiCheck(that->IsWasmMemoryObject(), "v8::WasmMemoryObject::Cast", "Value is not a WasmMemoryObject"); }

void v8::WasmModuleObject::CheckCast(Value* that) { Utils::ApiCheck(that->IsWasmModuleObject(), "v8::WasmModuleObject::Cast", "Value is not a WasmModuleObject"); }
...
void* v8::BackingStore::Data() const { return reinterpret_cast<const i::BackingStore*>(this)->buffer_start(); }

size_t v8::BackingStore::ByteLength() const { return reinterpret_cast<const i::BackingStore*>(this)->byte_length(); }

bool v8::BackingStore::IsShared() const { return reinterpret_cast<const i::BackingStore*>(this)->is_shared(); }
...
std::unique_ptr<v8::BackingStore> v8::BackingStore::Reallocate(v8::Isolate* v8_isolate, std::unique_ptr<v8::BackingStore> backing_store, size_t byte_length)
{
    ...
    Utils::ApiCheck(byte_length <= i::JSArrayBuffer::kMaxByteLength, "v8::BackingStore::Reallocate", "byte_lenght is too large");
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
...
// static
void v8::BackingStore::EmptyDeleter(void* data, size_t length, void* deleter_data) { DCHECK_NULL(deleter_data); }
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSArrayBuffer() && !i::JSArrayBuffer::cast(*obj).is_shared(), "v8::ArrayBuffer::Cast()", "Value is not an ArrayBuffer");
}

void v8::ArrayBufferView::CheckCast(Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSArrayBufferView(), "v8::ArrayBufferView::Cast()", "Value is not an ArrayBufferView");
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSTypedArray(), "v8::TypedArray::Cast()", "Value is not a TypedArray");
}
...
#define CHECK_TYPED_ARRAY_CAST(Type, typeName, TYPE, ctype)                                                                                                                        \
    ...
    {                                                                                                                                                                              \
        i::Handle<i::Object> obj = Utils::OpenHandle(that);                                                                                                                        \
        Utils::ApiCheck(obj->IsJSTypedArray() && i::JSTypedArray::cast(*obj).type() == i::kExternal##Type##Array, "v8::" #Type "Array::Cast()", "Value is not a " #Type "Array");  \
    }
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSDataView(), "v8::DataView::Cast()", "Value is not a DataView");
}

void v8::SharedArrayBuffer::CheckCast(Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSArrayBuffer() && i::JSArrayBuffer::cast(*obj).is_shared(), "v8::SharedArrayBuffer::Cast()", "Value is not a SharedArrayBuffer");
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsStringWrapper(), "v8::StringObject::Cast()", "Value is not a StringObject");
}

void v8::SymbolObject::CheckCast(v8::Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsSymbolWrapper(), "v8::SymbolObject::Cast()", "Value is not a SymbolObject");
}

void v8::NumberObject::CheckCast(v8::Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsNumberWrapper(), "v8::NumberObject::Cast()", "Value is not a NumberObject");
}

void v8::BigIntObject::CheckCast(v8::Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsBigIntWrapper(), "v8::BigIntObject::Cast()", "Value is not a BigIntObject");
}

void v8::BooleanObject::CheckCast(v8::Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsBooleanWrapper(), "v8::BooleanObject::Cast()", "Value is not a BooleanObject");
}

void v8::RegExp::CheckCast(v8::Value* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsJSRegExp(), "v8::RegExp::Cast()", "Value is not a RegExp");
}
...
Maybe<double> Value::NumberValue(Local<Context> context) const
{
    ...
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Value, NumberValue, Nothing<double>(), i::HandleScope);
...
Maybe<int64_t> Value::IntegerValue(Local<Context> context) const
{
    ...
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Value, IntegerValue, Nothing<int64_t>(), i::HandleScope);
    i::Handle<i::Object> num;
...
Maybe<int32_t> Value::Int32Value(Local<Context> context) const
{
    ...
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Value, Int32Value, Nothing<int32_t>(), i::HandleScope);
    i::Handle<i::Object> num;
    has_pending_exception = !i::Object::ToInt32(i_isolate, obj).ToHandle(&num);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(int32_t);
    return Just(num->IsSmi() ? i::Smi::ToInt(*num) : static_cast<int32_t>(num->Number()));
}
...
Maybe<uint32_t> Value::Uint32Value(Local<Context> context) const
{
    ...
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Value, Uint32Value, Nothing<uint32_t>(), i::HandleScope);
    i::Handle<i::Object> num;
    has_pending_exception = !i::Object::ToUint32(i_isolate, obj).ToHandle(&num);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(uint32_t);
    return Just(num->IsSmi() ? static_cast<uint32_t>(i::Smi::ToInt(*num)) : static_cast<uint32_t>(num->Number()));
}
...
MaybeLocal<Uint32> Value::ToArrayIndex(Local<Context> context) const
{
    ...
    has_pending_exception = !i::Object::ToString(i_isolate, self).ToHandle(&string_obj);
    RETURN_ON_FAILED_EXECUTION(Uint32);
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Value, InstanceOf, Nothing<bool>(), i::HandleScope);
    auto left = Utils::OpenHandle(this);
    auto right = Utils::OpenHandle(*object);
    i::Handle<i::Object> result;
    has_pending_exception = !i::Object::InstanceOf(i_isolate, left, right).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
...
Maybe<bool> v8::Object::Set(v8::Local<v8::Context> context, v8::Local<Value> key, v8::Local<Value> value)
{
    ...
    auto value_obj = Utils::OpenHandle(*value);
    has_pending_exception = i::Runtime::SetObjectProperty(i_isolate, self, key_obj, value_obj, i::StoreOrigin::kMaybeKeyed, Just(i::ShouldThrow::kDontThrow)).is_null();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(true);
}
...
Maybe<bool> v8::Object::Set(v8::Local<v8::Context> context, uint32_t index, v8::Local<Value> value)
{
    ...
    auto self = Utils::OpenHandle(this);
    auto value_obj = Utils::OpenHandle(*value);
    has_pending_exception = i::Object::SetElement(i_isolate, self, index, value_obj, i::ShouldThrow::kDontThrow).is_null();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(true);
}
...
Maybe<bool> v8::Object::CreateDataProperty(v8::Local<v8::Context> context, v8::Local<Name> key, v8::Local<Value> value)
{
    ...
    if (self->IsJSProxy()) {
        ENTER_V8(i_isolate, context, Object, CreateDataProperty, Nothing<bool>(), i::HandleScope);
        Maybe<bool> result = i::JSReceiver::CreateDataProperty(&it, value_obj, Just(i::kDontThrow));
        has_pending_exception = result.IsNothing();
        RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
        return result;
    } else {
        ENTER_V8_NO_SCRIPT(i_isolate, context, Object, CreateDataProperty, Nothing<bool>(), i::HandleScope);
        Maybe<bool> result = i::JSObject::CreateDataProperty(&it, value_obj, Just(i::kDontThrow));
        has_pending_exception = result.IsNothing();
        RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
        return result;
    }
...
bool v8::PropertyDescriptor::has_value() const { return private_->desc.has_value(); }
bool v8::PropertyDescriptor::has_get() const { return private_->desc.has_get(); }
bool v8::PropertyDescriptor::has_set() const { return private_->desc.has_set(); }
...
bool v8::PropertyDescriptor::has_writable() const { return private_->desc.has_writable(); }

void v8::PropertyDescriptor::set_enumerable(bool enumerable) { private_->desc.set_enumerable(enumerable); }
...
bool v8::PropertyDescriptor::has_enumerable() const { return private_->desc.has_enumerable(); }

void v8::PropertyDescriptor::set_configurable(bool configurable) { private_->desc.set_configurable(configurable); }
...
bool v8::PropertyDescriptor::has_configurable() const { return private_->desc.has_configurable(); }
...
Maybe<bool> v8::Object::DefineOwnProperty(v8::Local<v8::Context> context, v8::Local<Name> key, v8::Local<Value> value, v8::PropertyAttribute attributes)
{
    ...
    if (self->IsJSProxy()) {
        ENTER_V8(i_isolate, context, Object, DefineOwnProperty, Nothing<bool>(), i::HandleScope);
        ...
        // a script.
        ENTER_V8_NO_SCRIPT(i_isolate, context, Object, DefineOwnProperty, Nothing<bool>(), i::HandleScope);
        Maybe<bool> success = i::JSReceiver::DefineOwnProperty(i_isolate, self, key_obj, &desc, Just(i::kDontThrow));
        RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
        return success;
    }
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, Object, SetPrivate, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    ...
    if (self->IsJSProxy()) {
        ...
        desc.set_value(value_obj);
        return i::JSProxy::SetPrivateSymbol(i_isolate, i::Handle<i::JSProxy>::cast(self), i::Handle<i::Symbol>::cast(key_obj), &desc, Just(i::kDontThrow));
    }
    auto js_object = i::Handle<i::JSObject>::cast(self);
    i::LookupIterator it(i_isolate, js_object, key_obj, js_object);
    has_pending_exception = i::JSObject::DefineOwnPropertyIgnoreAttributes(&it, value_obj, i::DONT_ENUM).is_null();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(true);
}
...
MaybeLocal<Value> v8::Object::Get(Local<v8::Context> context, Local<Value> key)
{
    ...
    auto key_obj = Utils::OpenHandle(*key);
    i::Handle<i::Object> result;
    has_pending_exception = !i::Runtime::GetObjectProperty(i_isolate, self, key_obj).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION(Value);
...
MaybeLocal<Value> v8::Object::Get(Local<Context> context, uint32_t index)
{
    ...
    auto self = Utils::OpenHandle(this);
    i::Handle<i::Object> result;
    has_pending_exception = !i::JSReceiver::GetElement(i_isolate, self, index).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(Utils::ToLocal(result));
}

MaybeLocal<Value> v8::Object::GetPrivate(Local<Context> context, Local<Private> key) { return Get(context, Local<Value>(reinterpret_cast<Value*>(*key))); }

Maybe<PropertyAttribute> v8::Object::GetPropertyAttributes(Local<Context> context, Local<Value> key)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Object, GetPropertyAttributes, Nothing<PropertyAttribute>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    auto key_obj = Utils::OpenHandle(*key);
    if (!key_obj->IsName()) {
        has_pending_exception = !i::Object::ToString(i_isolate, key_obj).ToHandle(&key_obj);
...
Maybe<bool> v8::Object::SetPrototype(Local<Context> context, Local<Value> value)
{
    ...
    if (self->IsJSProxy()) {
        ...
        auto result = i::JSProxy::SetPrototype(i_isolate, i::Handle<i::JSProxy>::cast(self), value_obj, false, i::kThrowOnError);
        has_pending_exception = result.IsNothing();
        RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    } else {
        ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
        auto result = i::JSObject::SetPrototype(i_isolate, i::Handle<i::JSObject>::cast(self), value_obj, false, i::kThrowOnError);
        if (result.IsNothing()) {
...
{
    return GetPropertyNames(context, v8::KeyCollectionMode::kIncludePrototypes, static_cast<v8::PropertyFilter>(ONLY_ENUMERABLE | SKIP_SYMBOLS), v8::IndexFilter::kIncludeIndices);
}
    ...
    Local<Context> context, KeyCollectionMode mode, PropertyFilter property_filter, IndexFilter index_filter, KeyConversionMode key_conversion)
{
    ...
    DCHECK(self->map().EnumLength() == i::kInvalidEnumCacheSentinel || self->map().EnumLength() == 0 || self->map().instance_descriptors(i_isolate).enum_cache().keys() != *value);
    auto result = i_isolate->factory()->NewJSArrayWithElements(value);
    RETURN_ESCAPED(Utils::ToLocal(result));
}

MaybeLocal<Array> v8::Object::GetOwnPropertyNames(Local<Context> context) { return GetOwnPropertyNames(context, static_cast<v8::PropertyFilter>(ONLY_ENUMERABLE | SKIP_SYMBOLS)); }
...
MaybeLocal<String> v8::Object::ObjectProtoToString(Local<Context> context)
{
    ...
    auto self = Utils::OpenHandle(this);
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::CallBuiltin(i_isolate, i_isolate->object_to_string(), self, 0, nullptr), &result);
    RETURN_ON_FAILED_EXECUTION(String);
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Object, SetIntegrityLevel, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
...
Maybe<bool> v8::Object::Delete(Local<Context> context, Local<Value> key)
{
    ...
    if (self->IsJSProxy()) {
        ...
        // a script.
        ENTER_V8_NO_SCRIPT(i_isolate, context, Object, Delete, Nothing<bool>(), i::HandleScope);
        Maybe<bool> result = i::Runtime::DeleteObjectProperty(i_isolate, self, key_obj, i::LanguageMode::kSloppy);
        has_pending_exception = result.IsNothing();
        RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
        return result;
...
Maybe<bool> v8::Object::HasPrivate(Local<Context> context, Local<Private> key) { return HasOwnProperty(context, Local<Name>(reinterpret_cast<Name*>(*key))); }
...
static Maybe<bool> ObjectSetAccessor(Local<Context> context, Object* self, Local<Name> name, Getter getter, Setter setter, Data data, AccessControl settings,
    PropertyAttribute attributes, bool is_special_data_property, bool replace_on_access, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, Object, SetAccessor, Nothing<bool>(), i::HandleScope);
    if (!Utils::OpenHandle(self)->IsJSObject())
        return Just(false);
    i::Handle<i::JSObject> obj = i::Handle<i::JSObject>::cast(Utils::OpenHandle(self));
    i::Handle<i::AccessorInfo> info = MakeAccessorInfo(i_isolate, name, getter, setter, data, settings, is_special_data_property, replace_on_access);
    ...
    has_pending_exception = !i::JSObject::SetAccessor(obj, accessor_name, info, attrs).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    if (result->IsUndefined(i_isolate))
        return Just(false);
    if (fast) {
...
Maybe<bool> Object::SetAccessor(Local<Context> context, Local<Name> name, AccessorNameGetterCallback getter, AccessorNameSetterCallback setter, MaybeLocal<Value> data,
...
{
    return ObjectSetAccessor(context, this, name, getter, setter, data.FromMaybe(Local<Value>()), settings, attribute, i::v8_flags.disable_old_api_accessors, false,
        getter_side_effect_type, setter_side_effect_type);
}

void Object::SetAccessorProperty(Local<Name> name, Local<Function> getter, Local<Function> setter, PropertyAttribute attribute, AccessControl settings)
{
    ...
    i::JSObject::DefineAccessor(i::Handle<i::JSObject>::cast(self), v8::Utils::OpenHandle(*name), getter_i, setter_i, static_cast<i::PropertyAttributes>(attribute));
}

Maybe<bool> Object::SetNativeDataProperty(v8::Local<v8::Context> context, v8::Local<Name> name, AccessorNameGetterCallback getter, AccessorNameSetterCallback setter,
    v8::Local<Value> data, PropertyAttribute attributes, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    return ObjectSetAccessor(context, this, name, getter, setter, data, DEFAULT, attributes, true, false, getter_side_effect_type, setter_side_effect_type);
}
    ...
    PropertyAttribute attributes, SideEffectType getter_side_effect_type, SideEffectType setter_side_effect_type)
{
    return ObjectSetAccessor(
        context, this, name, getter, static_cast<AccessorNameSetterCallback>(nullptr), data, DEFAULT, attributes, true, true, getter_side_effect_type, setter_side_effect_type);
}

Maybe<bool> v8::Object::HasOwnProperty(Local<Context> context, Local<Name> key)
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Object, HasOwnProperty, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, Object, HasRealNamedProperty, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    if (!self->IsJSObject())
        return Just(false);
    auto key_val = Utils::OpenHandle(*key);
    auto result = i::JSObject::HasRealNamedProperty(i_isolate, i::Handle<i::JSObject>::cast(self), key_val);
    has_pending_exception = result.IsNothing();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return result;
}
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, Object, HasRealIndexedProperty, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    if (!self->IsJSObject())
        return Just(false);
    auto result = i::JSObject::HasRealElementProperty(i_isolate, i::Handle<i::JSObject>::cast(self), index);
    has_pending_exception = result.IsNothing();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return result;
}
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8_NO_SCRIPT(i_isolate, context, Object, HasRealNamedCallbackProperty, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    if (!self->IsJSObject())
        return Just(false);
    auto key_val = Utils::OpenHandle(*key);
    auto result = i::JSObject::HasRealNamedCallbackProperty(i_isolate, i::Handle<i::JSObject>::cast(self), key_val);
    has_pending_exception = result.IsNothing();
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return result;
}
...
{
    PREPARE_FOR_EXECUTION(context, Object, GetRealNamedPropertyInPrototypeChain, Value);
    i::Handle<i::JSReceiver> self = Utils::OpenHandle(this);
    if (!self->IsJSObject())
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Object, GetRealNamedPropertyAttributesInPrototypeChain, Nothing<PropertyAttribute>(), i::HandleScope);
    i::Handle<i::JSReceiver> self = Utils::OpenHandle(this);
    if (!self->IsJSObject())
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Object, GetRealNamedPropertyAttributes, Nothing<PropertyAttribute>(), i::HandleScope);
    i::Handle<i::JSReceiver> self = Utils::OpenHandle(this);
    i::Handle<i::Name> key_obj = Utils::OpenHandle(*key);
    i::PropertyKey lookup_key(i_isolate, key_obj);
    i::LookupIterator it(i_isolate, self, lookup_key, self, i::LookupIterator::PROTOTYPE_CHAIN_SKIP_INTERCEPTOR);
...
Local<v8::Context> v8::Object::GetCreationContextChecked()
{
    ...
    Utils::ApiCheck(GetCreationContext().ToLocal(&context), "v8::Object::GetCreationContextChecked", "No creation context available");
    return context;
}
...
MaybeLocal<Value> Object::CallAsFunction(Local<Context> context, Local<Value> recv, int argc, Local<Value> argv[])
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.Execute");
    ENTER_V8(i_isolate, context, Object, CallAsFunction, MaybeLocal<Value>(), InternalEscapableScope);
    i::TimerEventScope<i::TimerEventExecute> timer_scope(i_isolate);
    i::NestedTimedHistogramScope execute_timer(i_isolate->counters()->execute(), i_isolate);
    auto self = Utils::OpenHandle(this);
    ...
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::Call(i_isolate, self, recv_obj, argc, args), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}
...
MaybeLocal<Value> Object::CallAsConstructor(Local<Context> context, int argc, Local<Value> argv[])
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.Execute");
    ENTER_V8(i_isolate, context, Object, CallAsConstructor, MaybeLocal<Value>(), InternalEscapableScope);
    i::TimerEventScope<i::TimerEventExecute> timer_scope(i_isolate);
    i::NestedTimedHistogramScope execute_timer(i_isolate->counters()->execute(), i_isolate);
    auto self = Utils::OpenHandle(this);
    static_assert(sizeof(v8::Local<v8::Value>) == sizeof(i::Handle<i::Object>));
    i::Handle<i::Object>* args = reinterpret_cast<i::Handle<i::Object>*>(argv);
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::New(i_isolate, self, self, argc, args), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}

MaybeLocal<Function> Function::New(Local<Context> context, FunctionCallback callback, Local<Value> data, int length, ConstructorBehavior behavior, SideEffectType side_effect_type)
{
    i::Isolate* i_isolate = Utils::OpenHandle(*context)->GetIsolate();
...
MaybeLocal<Object> Function::NewInstanceWithSideEffectType(Local<Context> context, int argc, v8::Local<v8::Value> argv[], SideEffectType side_effect_type) const
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.Execute");
    ENTER_V8(i_isolate, context, Function, NewInstance, MaybeLocal<Object>(), InternalEscapableScope);
    i::TimerEventScope<i::TimerEventExecute> timer_scope(i_isolate);
    i::NestedTimedHistogramScope execute_timer(i_isolate->counters()->execute(), i_isolate);
    auto self = Utils::OpenHandle(this);
    static_assert(sizeof(v8::Local<v8::Value>) == sizeof(i::Handle<i::Object>));
    bool should_set_has_no_side_effect = side_effect_type == SideEffectType::kHasNoSideEffect && i_isolate->debug_execution_mode() == i::DebugInfo::kSideEffects;
    if (should_set_has_no_side_effect) {
        CHECK(self->IsJSFunction() && i::JSFunction::cast(*self).shared().IsApiFunction());
        i::Object obj = i::JSFunction::cast(*self).shared().get_api_func_data().call_code(kAcquireLoad);
    ...
    i::Handle<i::Object>* args = reinterpret_cast<i::Handle<i::Object>*>(argv);
    Local<Object> result;
    has_pending_exception = !ToLocal<Object>(i::Execution::New(i_isolate, self, self, argc, args), &result);
    if (should_set_has_no_side_effect) {
        i::Object obj = i::JSFunction::cast(*self).shared().get_api_func_data().call_code(kAcquireLoad);
        if (obj.IsCallHandlerInfo()) {
            i::CallHandlerInfo handler_info = i::CallHandlerInfo::cast(obj);
...
MaybeLocal<v8::Value> Function::Call(Local<Context> context, v8::Local<v8::Value> recv, int argc, v8::Local<v8::Value> argv[])
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    TRACE_EVENT_CALL_STATS_SCOPED(i_isolate, "v8", "V8.Execute");
    ENTER_V8(i_isolate, context, Function, Call, MaybeLocal<Value>(), InternalEscapableScope);
    i::TimerEventScope<i::TimerEventExecute> timer_scope(i_isolate);
    i::NestedTimedHistogramScope execute_timer(i_isolate->counters()->execute(), i_isolate);
    auto self = Utils::OpenHandle(this);
...
Local<Value> Function::GetInferredName() const
{
    ...
    auto func = i::Handle<i::JSFunction>::cast(self);
    return Utils::ToLocal(i::Handle<i::Object>(func->shared().inferred_name(), func->GetIsolate()));
}
...
ScriptOrigin Function::GetScriptOrigin() const
{
    ...
    if (func->shared().script().IsScript()) {
        i::Handle<i::Script> script(i::Script::cast(func->shared().script()), func->GetIsolate());
...
MaybeLocal<String> v8::Function::FunctionProtoToString(Local<Context> context)
{
    ...
    auto self = Utils::OpenHandle(this);
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::CallBuiltin(i_isolate, i_isolate->function_to_string(), self, 0, nullptr), &result);
    RETURN_ON_FAILED_EXECUTION(String);
    RETURN_ESCAPED(Local<String>::Cast(result));
}
...
static inline bool Unaligned(const uint16_t* chars) { return reinterpret_cast<const uintptr_t>(chars) & kAlignmentMask; }

static inline const uint16_t* Align(const uint16_t* chars) { return reinterpret_cast<uint16_t*>(reinterpret_cast<uintptr_t>(chars) & ~kAlignmentMask); }
...
template <typename Char> static int WriteUtf8Impl(base::Vector<const Char> string, char* write_start, int write_capacity, int options, int* utf16_chars_read_out)
{
    ...
    if (read_index < read_length) {
        ...
        for (; read_index < read_length && remaining_capacity > 0; read_index++) {
            ...
            if (unibrow::Utf16::IsSurrogatePair(prev_char, character)) {
                ...
                if (replace_invalid_utf8 && remaining_capacity < kSurrogatePairEncodedSize && unibrow::Utf16::IsLeadSurrogate(character) && read_index + 1 < read_length
...
bool HasExternalStringResource(i::String string) { return i::StringShape(string).IsExternal() || string.HasExternalForwardingIndex(kAcquireLoad); }

v8::String::ExternalStringResourceBase* GetExternalResourceFromForwardingTable(i::String string, uint32_t raw_hash, bool* is_one_byte)
{
    ...
    auto resource = isolate->string_forwarding_table()->GetExternalResource(index, is_one_byte);
...
void String::ExternalStringResource::CheckCachedDataInvariants() const { DCHECK(IsCacheable() && cached_data_ != nullptr); }
...
void String::ExternalOneByteStringResource::CheckCachedDataInvariants() const { DCHECK(IsCacheable() && cached_data_ != nullptr); }
...
Local<Value> Symbol::Description(Isolate* v8_isolate) const
{
    ...
    i::Handle<i::Object> description(sym->description(), reinterpret_cast<i::Isolate*>(v8_isolate));
...
static bool InternalFieldOK(i::Handle<i::JSReceiver> obj, int index, const char* location)
{
    return Utils::ApiCheck(obj->IsJSObject() && (index < i::Handle<i::JSObject>::cast(obj)->GetEmbedderFieldCount()), location, "Internal field out of bounds");
}
...
Local<Value> v8::Object::SlowGetInternalField(int index)
{
    ...
    if (!InternalFieldOK(obj, index, location))
        return Local<Value>();
    i::Handle<i::Object> value(i::JSObject::cast(*obj).GetEmbedderField(index), obj->GetIsolate());
...
void* v8::Object::SlowGetAlignedPointerFromInternalField(int index)
{
    ...
    if (!InternalFieldOK(obj, index, location))
        return nullptr;
    void* result;
    Utils::ApiCheck(i::EmbedderDataSlot(i::JSObject::cast(*obj), index).ToAlignedPointer(obj->GetIsolate(), &result), location, "Unaligned pointer");
    return result;
}
...
void v8::Object::SetAlignedPointerInInternalField(int index, void* value)
{
    ...
    i::DisallowGarbageCollection no_gc;
    Utils::ApiCheck(i::EmbedderDataSlot(i::JSObject::cast(*obj), index).store_aligned_pointer(obj->GetIsolate(), value), location, "Unaligned pointer");
...
void v8::V8::InitializePlatform(Platform* platform) { i::V8::InitializePlatform(platform); }
...
void v8::V8::SetFatalMemoryErrorCallback(v8::OOMErrorCallback oom_error_callback) { g_oom_error_callback = oom_error_callback; }

void v8::V8::SetEntropySource(EntropySource entropy_source) { base::RandomNumberGenerator::SetEntropySource(entropy_source); }

void v8::V8::SetReturnAddressLocationResolver(ReturnAddressLocationResolver return_address_resolver) { i::StackFrame::SetReturnAddressLocationResolver(return_address_resolver); }
...
bool v8::V8::InitializeICU(const char* icu_data_file) { return i::InitializeICU(icu_data_file); }

bool v8::V8::InitializeICUDefaultLocation(const char* exec_path, const char* icu_data_file) { return i::InitializeICUDefaultLocation(exec_path, icu_data_file); }

void v8::V8::InitializeExternalStartupData(const char* directory_path) { i::InitializeExternalStartupData(directory_path); }

// static
void v8::V8::InitializeExternalStartupDataFromFile(const char* snapshot_blob) { i::InitializeExternalStartupDataFromFile(snapshot_blob); }
...
void V8::GetSharedMemoryStatistics(SharedMemoryStatistics* statistics) { i::ReadOnlyHeap::PopulateReadOnlySpaceStatistics(statistics); }

template <typename ObjectType> struct InvokeBootstrapper;

template <> struct InvokeBootstrapper<i::Context> {
    i::Handle<i::Context> Invoke(i::Isolate* i_isolate, i::MaybeHandle<i::JSGlobalProxy> maybe_global_proxy, v8::Local<v8::ObjectTemplate> global_proxy_template,
        v8::ExtensionConfiguration* extensions, size_t context_snapshot_index, v8::DeserializeInternalFieldsCallback embedder_fields_deserializer,
...
template <> struct InvokeBootstrapper<i::JSGlobalProxy> {
    i::Handle<i::JSGlobalProxy> Invoke(i::Isolate* i_isolate, i::MaybeHandle<i::JSGlobalProxy> maybe_global_proxy, v8::Local<v8::ObjectTemplate> global_proxy_template,
        v8::ExtensionConfiguration* extensions, size_t context_snapshot_index, v8::DeserializeInternalFieldsCallback embedder_fields_deserializer,
        v8::MicrotaskQueue* microtask_queue)
    {
        USE(extensions);
...
static i::Handle<ObjectType> CreateEnvironment(i::Isolate* i_isolate, v8::ExtensionConfiguration* extensions, v8::MaybeLocal<ObjectTemplate> maybe_global_template,
    v8::MaybeLocal<Value> maybe_global_proxy, size_t context_snapshot_index, v8::DeserializeInternalFieldsCallback embedder_fields_deserializer,
    v8::MicrotaskQueue* microtask_queue)
{
...

    {
        ...
        i::Handle<i::HeapObject> named_interceptor(i_isolate->factory()->undefined_value());
        i::Handle<i::HeapObject> indexed_interceptor(i_isolate->factory()->undefined_value());
        ...
        if (!maybe_global_template.IsEmpty()) {
            ...
            if (!global_constructor->GetNamedPropertyHandler().IsUndefined(i_isolate)) {
            ...
            if (!global_constructor->GetIndexedPropertyHandler().IsUndefined(i_isolate)) {
        ...
        if (!maybe_global_proxy.IsEmpty()) {
            maybe_proxy = i::Handle<i::JSGlobalProxy>::cast(Utils::OpenHandle(*maybe_global_proxy.ToLocalChecked()));
        }
...
Local<Context> NewContext(v8::Isolate* external_isolate, v8::ExtensionConfiguration* extensions, v8::MaybeLocal<ObjectTemplate> global_template,
    v8::MaybeLocal<Value> global_object, size_t context_snapshot_index, v8::DeserializeInternalFieldsCallback embedder_fields_deserializer, v8::MicrotaskQueue* microtask_queue)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(external_isolate);
    ...
    i::Handle<i::Context> env
        = CreateEnvironment<i::Context>(i_isolate, extensions, global_template, global_object, context_snapshot_index, embedder_fields_deserializer, microtask_queue);
    if (env.is_null()) {
...
Local<Context> v8::Context::New(v8::Isolate* external_isolate, v8::ExtensionConfiguration* extensions, v8::MaybeLocal<ObjectTemplate> global_template,
    v8::MaybeLocal<Value> global_object, DeserializeInternalFieldsCallback internal_fields_deserializer, v8::MicrotaskQueue* microtask_queue)
{
    return NewContext(external_isolate, extensions, global_template, global_object, 0, internal_fields_deserializer, microtask_queue);
}

MaybeLocal<Context> v8::Context::FromSnapshot(v8::Isolate* external_isolate, size_t context_snapshot_index, v8::DeserializeInternalFieldsCallback embedder_fields_deserializer,
    v8::ExtensionConfiguration* extensions, MaybeLocal<Value> global_object, v8::MicrotaskQueue* microtask_queue)
{
    ...
    return NewContext(external_isolate, extensions, MaybeLocal<ObjectTemplate>(), global_object, index_including_default_context, embedder_fields_deserializer, microtask_queue);
}

MaybeLocal<Object> v8::Context::NewRemoteContext(v8::Isolate* external_isolate, v8::Local<ObjectTemplate> global_template, v8::MaybeLocal<v8::Value> global_object)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(external_isolate);
    API_RCS_SCOPE(i_isolate, Context, NewRemoteContext);
    i::HandleScope scope(i_isolate);
    i::Handle<i::FunctionTemplateInfo> global_constructor = EnsureConstructor(i_isolate, *global_template);
    ...
    i::Handle<i::AccessCheckInfo> access_check_info = i::handle(i::AccessCheckInfo::cast(global_constructor->GetAccessCheckInfo()), i_isolate);
    Utils::ApiCheck(access_check_info->named_interceptor() != i::Object(), "v8::Context::NewRemoteContext", "Global template needs to have access check handlers");
    i::Handle<i::JSObject> global_proxy = CreateEnvironment<i::JSGlobalProxy>(i_isolate, nullptr, global_template, global_object, 0, DeserializeInternalFieldsCallback(), nullptr);
...
{
    i::Handle<i::Context> env = Utils::OpenHandle(this);
    Utils::ApiCheck(env->IsNativeContext(), "v8::Context::GetMicrotaskQueue", "Must be calld on a native context");
...
void Context::AllowCodeGenerationFromStrings(bool allow)
{
    ...
    i::Isolate* i_isolate = context->GetIsolate();
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    context->set_allow_code_gen_from_strings(allow ? i::ReadOnlyRoots(i_isolate).true_value() : i::ReadOnlyRoots(i_isolate).false_value());
}
...
void Context::SetAbortScriptExecution(Context::AbortScriptExecutionCallback callback)
{
    ...
    if (callback == nullptr) {
        ...
        SET_FIELD_WRAPPED(i_isolate, context, set_script_execution_callback, callback);
    }
}
...
Local<Value> Context::GetContinuationPreservedEmbedderData() const
{
    i::Handle<i::Context> context = Utils::OpenHandle(this);
    i::Isolate* i_isolate = context->GetIsolate();
    i::Handle<i::Object> data(context->native_context().continuation_preserved_embedder_data(), i_isolate);
...
void Context::SetContinuationPreservedEmbedderData(Local<Value> data)
{
    ...
    if (data.IsEmpty())
        data = v8::Undefined(reinterpret_cast<v8::Isolate*>(i_isolate));
    context->native_context().set_continuation_preserved_embedder_data(*i::Handle<i::HeapObject>::cast(Utils::OpenHandle(*data)));
}
...
bool Context::HasTemplateLiteralObject(Local<Value> object)
{
    ...
    if (!i_object.IsJSArray())
        return false;
    return Utils::OpenHandle(this)->native_context().HasTemplateLiteralObject(i::JSArray::cast(i_object));
}
...
namespace {
i::Address* GetSerializedDataFromFixedArray(i::Isolate* i_isolate, i::FixedArray list, size_t index)
{
...
MaybeLocal<v8::Object> ObjectTemplate::NewInstance(Local<Context> context)
{
    ...
    auto self = Utils::OpenHandle(this);
    Local<Object> result;
    has_pending_exception = !ToLocal<Object>(i::ApiNatives::InstantiateObject(i_isolate, self), &result);
    RETURN_ON_FAILED_EXECUTION(Object);
    RETURN_ESCAPED(result);
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsObjectTemplateInfo(), "v8::ObjectTemplate::Cast", "Value is not an ObjectTemplate");
}

void v8::FunctionTemplate::CheckCast(Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsFunctionTemplateInfo(), "v8::FunctionTemplate::Cast", "Value is not a FunctionTemplate");
}

void v8::Signature::CheckCast(Data* that)
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsFunctionTemplateInfo(), "v8::Signature::Cast", "Value is not a Signature");
}
...
MaybeLocal<v8::Object> FunctionTemplate::NewRemoteInstance()
{
    ...
    i::HandleScope scope(i_isolate);
    i::Handle<i::FunctionTemplateInfo> constructor = EnsureConstructor(i_isolate, *InstanceTemplate());
    ...
    i::Handle<i::AccessCheckInfo> access_check_info = i::handle(i::AccessCheckInfo::cast(constructor->GetAccessCheckInfo()), i_isolate);
    Utils::ApiCheck(access_check_info->named_interceptor() != i::Object(), "v8::FunctionTemplate::NewRemoteInstance", "InstanceTemplate needs to have access check handlers");
...
bool FunctionTemplate::HasInstance(v8::Local<v8::Value> value)
{
    ...
    if (obj->IsJSGlobalProxy()) {
        ...
        i::PrototypeIterator iter(self->GetIsolate(), i::JSObject::cast(*obj).map());
...
inline int StringLength(const uint8_t* string) { return StringLength(reinterpret_cast<const char*>(string)); }
...
bool ApiStringIsWellFormed(v8::Isolate* isolate, v8::Local<v8::String> source_str)
{
    i::Isolate* i_isolate = (i::Isolate*)isolate;
    i::Handle<i::String> source = Utils::OpenHandle(*(source_str));
    return i::String::IsWellFormedUnicode(i_isolate, source);
}

v8::Local<v8::String> ApiStringToWellFormed(v8::Isolate* isolate, v8::Local<v8::String> source_str)
{
    i::Isolate* i_isolate = (i::Isolate*)isolate;
    i::Handle<i::String> source = Utils::OpenHandle(*(source_str));
    if (i::String::IsWellFormedUnicode(i_isolate, source))
        return Utils::ToLocal(source);

    // String::IsWellFormedUnicode would have returned true above otherwise.
    DCHECK(!i::String::IsOneByteRepresentationUnderneath(*source));
    const int length = source->length();
    i::Handle<v8::internal::SeqTwoByteString> dest = i_isolate->factory()->NewRawTwoByteString(length).ToHandleChecked();

    i::DisallowGarbageCollection no_gc;
    i::String::FlatContent source_contents = source->GetFlatContent(no_gc);
...
// Handle.
template <typename T> i::Handle<T> ToHandle(i::Handle<T> h) { return h; }
template <typename T> i::Handle<T> ToHandle(i::MaybeHandle<T> h) { return h.ToHandleChecked(); }
    ...
    i::Isolate* i_isolate, i::Handle<Dictionary>& properties, i::Handle<i::FixedArrayBase>& elements, Local<Name>* names, Local<Value>* values, size_t length)
{
    for (size_t i = 0; i < length; ++i) {
        ...
        if (name->AsArrayIndex(&index)) {
            ...
            elements = i::NumberDictionary::Set(i_isolate, i::Handle<i::NumberDictionary>::cast(elements), index, value);
        } else {
...
Local<v8::Object> v8::Object::New(Isolate* v8_isolate, Local<Value> prototype_or_null, Local<Name>* names, Local<Value>* values, size_t length)
{
    ...
    if (!Utils::ApiCheck(proto->IsNull() || proto->IsJSReceiver(), "v8::Object::New", "prototype must be null or object")) {
    ...
    if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
        ...
        i::Handle<i::JSObject> obj = i_isolate->factory()->NewSlowJSObjectWithPropertiesAndElements(i::Handle<i::HeapObject>::cast(proto), properties, elements);
        return Utils::ToLocal(obj);
    } else {
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(this);
    i::Handle<i::JSPrimitiveWrapper> js_primitive_wrapper = i::Handle<i::JSPrimitiveWrapper>::cast(obj);
...
Local<v8::BigInt> v8::BigIntObject::ValueOf() const
{
    ...
    return Utils::ToLocal(i::Handle<i::BigInt>(i::BigInt::cast(js_primitive_wrapper->value()), i_isolate));
}
...
Local<v8::Value> v8::BooleanObject::New(Isolate* v8_isolate, bool value)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::Object> boolean(value ? i::ReadOnlyRoots(i_isolate).true_value() : i::ReadOnlyRoots(i_isolate).false_value(), i_isolate);
...
Local<v8::String> v8::StringObject::ValueOf() const
{
    ...
    return Utils::ToLocal(i::Handle<i::String>(i::String::cast(js_primitive_wrapper->value()), i_isolate));
}
...
Local<v8::Symbol> v8::SymbolObject::ValueOf() const
{
    ...
    return Utils::ToLocal(i::Handle<i::Symbol>(i::Symbol::cast(js_primitive_wrapper->value()), i_isolate));
}
...
MaybeLocal<v8::Value> v8::Date::New(Local<Context> context, double time)
{
    ...
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::JSDate::New(i_isolate->date_function(), i_isolate->date_function(), time), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}
...
v8::Local<v8::String> v8::Date::ToISOString() const
{
    ...
    i::DateBuffer buffer = i::ToDateString(jsdate->value().Number(), i_isolate->date_cache(), i::ToDateStringMode::kISODateAndTime);
...
MaybeLocal<v8::RegExp> v8::RegExp::New(Local<Context> context, Local<String> pattern, Flags flags)
{
    ...
    has_pending_exception = !ToLocal<RegExp>(i::JSRegExp::New(i_isolate, Utils::OpenHandle(*pattern), static_cast<i::JSRegExp::Flags>(flags)), &result);
...
MaybeLocal<v8::RegExp> v8::RegExp::NewWithBacktrackLimit(Local<Context> context, Local<String> pattern, Flags flags, uint32_t backtrack_limit)
{
    ...
    PREPARE_FOR_EXECUTION(context, RegExp, New, RegExp);
    Local<v8::RegExp> result;
    has_pending_exception = !ToLocal<RegExp>(i::JSRegExp::New(i_isolate, Utils::OpenHandle(*pattern), static_cast<i::JSRegExp::Flags>(flags), backtrack_limit), &result);
    RETURN_ON_FAILED_EXECUTION(RegExp);
    RETURN_ESCAPED(result);
}
...
Local<v8::String> v8::RegExp::GetSource() const
{
    ...
    return Utils::ToLocal(i::Handle<i::String>(obj->EscapedPattern(), obj->GetIsolate()));
}
...
MaybeLocal<v8::Object> v8::RegExp::Exec(Local<Context> context, Local<v8::String> subject)
{
    ...
    has_pending_exception = !ToLocal<Object>(i::RegExpUtils::RegExpExec(i_isolate, regexp, subject_string, i_isolate->factory()->undefined_value()), &result);
...
MaybeLocal<Value> Map::Get(Local<Context> context, Local<Value> key)
{
    ...
    has_pending_exception = !ToLocal<Value>(i::Execution::CallBuiltin(i_isolate, i_isolate->map_get(), self, arraysize(argv), argv), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}
...
MaybeLocal<Map> Map::Set(Local<Context> context, Local<Value> key, Local<Value> value)
{
    ...
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->map_set(), self, arraysize(argv), argv).ToHandle(&result);
...
Maybe<bool> Map::Has(Local<Context> context, Local<Value> key)
{
    ...
    i::Handle<i::Object> result;
    i::Handle<i::Object> argv[] = { Utils::OpenHandle(*key) };
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->map_has(), self, arraysize(argv), argv).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(result->IsTrue(i_isolate));
}
...
Maybe<bool> Map::Delete(Local<Context> context, Local<Value> key)
{
    ...
    i::Handle<i::Object> result;
    i::Handle<i::Object> argv[] = { Utils::OpenHandle(*key) };
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->map_delete(), self, arraysize(argv), argv).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(result->IsTrue(i_isolate));
}
...
i::Handle<i::JSArray> MapAsArray(i::Isolate* i_isolate, i::Object table_obj, int offset, MapAsArrayKind kind)
{
    i::Factory* factory = i_isolate->factory();
    i::Handle<i::OrderedHashMap> table(i::OrderedHashMap::cast(table_obj), i_isolate);
...
MaybeLocal<Set> Set::Add(Local<Context> context, Local<Value> key)
{
    ...
    i::Handle<i::Object> result;
    i::Handle<i::Object> argv[] = { Utils::OpenHandle(*key) };
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->set_add(), self, arraysize(argv), argv).ToHandle(&result);
...
Maybe<bool> Set::Has(Local<Context> context, Local<Value> key)
{
    ...
    i::Handle<i::Object> result;
    i::Handle<i::Object> argv[] = { Utils::OpenHandle(*key) };
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->set_has(), self, arraysize(argv), argv).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(result->IsTrue(i_isolate));
}
...
Maybe<bool> Set::Delete(Local<Context> context, Local<Value> key)
{
    ...
    i::Handle<i::Object> result;
    i::Handle<i::Object> argv[] = { Utils::OpenHandle(*key) };
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->set_delete(), self, arraysize(argv), argv).ToHandle(&result);
    RETURN_ON_FAILED_EXECUTION_PRIMITIVE(bool);
    return Just(result->IsTrue(i_isolate));
}

namespace {
i::Handle<i::JSArray> SetAsArray(i::Isolate* i_isolate, i::Object table_obj, int offset, SetAsArrayKind kind)
{
    i::Factory* factory = i_isolate->factory();
    i::Handle<i::OrderedHashSet> table(i::OrderedHashSet::cast(table_obj), i_isolate);
...
MaybeLocal<Promise::Resolver> Promise::Resolver::New(Local<Context> context)
{
    ...
    has_pending_exception = !ToLocal<Promise::Resolver>(i_isolate->factory()->NewJSPromise(), &result);
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Promise_Resolver, Resolve, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
...
{
    auto i_isolate = reinterpret_cast<i::Isolate*>(context->GetIsolate());
    ENTER_V8(i_isolate, context, Promise_Resolver, Reject, Nothing<bool>(), i::HandleScope);
    auto self = Utils::OpenHandle(this);
    auto promise = i::Handle<i::JSPromise>::cast(self);
...
MaybeLocal<Promise> Promise::Catch(Local<Context> context, Local<Function> handler)
{
    ...
    has_pending_exception = !i::Execution::CallBuiltin(i_isolate, i_isolate->promise_then(), self, arraysize(argv), argv).ToHandle(&result);
...
MaybeLocal<Proxy> Proxy::New(Local<Context> context, Local<Object> local_target, Local<Object> local_handler)
{
    ...
    Local<Proxy> result;
    has_pending_exception = !ToLocal<Proxy>(i::JSProxy::New(i_isolate, target, handler), &result);
...
CompiledWasmModule WasmModuleObject::GetCompiledModule()
{
    ...
    auto url = i::handle(i::String::cast(obj->script().name()), obj->GetIsolate());
    int length;
    std::unique_ptr<char[]> cstring = url->ToCString(i::DISALLOW_NULLS, i::FAST_STRING_TRAVERSAL, &length);
...
MaybeLocal<WasmModuleObject> WasmModuleObject::FromCompiledModule(Isolate* v8_isolate, const CompiledWasmModule& compiled_module)
{
    ...
    return Local<WasmModuleObject>::Cast(Utils::ToLocal(i::Handle<i::JSObject>::cast(module_object)));
#else
    UNREACHABLE();
#endif // V8_ENABLE_WEBASSEMBLY
}
...
MaybeLocal<WasmModuleObject> WasmModuleObject::Compile(Isolate* v8_isolate, MemorySpan<const uint8_t> wire_bytes)
{
    ...
    i::MaybeHandle<i::JSObject> maybe_compiled;
    {
        ...
        maybe_compiled = i::wasm::GetWasmEngine()->SyncCompile(i_isolate, enabled_features, &thrower, i::wasm::ModuleWireBytes(start, start + length));
    }
    ...
    return Local<WasmModuleObject>::Cast(Utils::ToLocal(maybe_compiled.ToHandleChecked()));
#else
...
// static
v8::ArrayBuffer::Allocator* v8::ArrayBuffer::Allocator::NewDefaultAllocator() { return new ArrayBufferAllocator(); }
...
v8::Maybe<bool> v8::ArrayBuffer::Detach(v8::Local<v8::Value> key)
{
    Detach();
    return v8::Just(true);
}
...
namespace {
std::shared_ptr<i::BackingStore> ToInternal(std::shared_ptr<i::BackingStoreBase> backing_store) { return std::static_pointer_cast<i::BackingStore>(backing_store); }
} // namespace
...
Local<ArrayBuffer> v8::ArrayBuffer::New(Isolate* v8_isolate, size_t byte_length)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    API_RCS_SCOPE(i_isolate, ArrayBuffer, New);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::MaybeHandle<i::JSArrayBuffer> result = i_isolate->factory()->NewJSArrayBufferAndBackingStore(byte_length, i::InitializedFlag::kZeroInitialized);
...
std::unique_ptr<v8::BackingStore> v8::ArrayBuffer::NewBackingStore(Isolate* v8_isolate, size_t byte_length)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    API_RCS_SCOPE(i_isolate, ArrayBuffer, NewBackingStore);
    CHECK_LE(byte_length, i::JSArrayBuffer::kMaxByteLength);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    std::unique_ptr<i::BackingStoreBase> backing_store = i::BackingStore::Allocate(i_isolate, byte_length, i::SharedFlag::kNotShared, i::InitializedFlag::kZeroInitialized);
    if (!backing_store) {
...
std::unique_ptr<v8::BackingStore> v8::ArrayBuffer::NewBackingStore(void* data, size_t byte_length, v8::BackingStore::DeleterCallback deleter, void* deleter_data)
{
    CHECK_LE(byte_length, i::JSArrayBuffer::kMaxByteLength);
#ifdef V8_ENABLE_SANDBOX
...
Local<ArrayBuffer> v8::ArrayBufferView::Buffer()
{
    ...
    if (obj->IsJSDataView()) {
        i::Handle<i::JSDataView> data_view(i::JSDataView::cast(*obj), obj->GetIsolate());
        DCHECK(data_view->buffer().IsJSArrayBuffer());
        buffer = i::handle(i::JSArrayBuffer::cast(data_view->buffer()), data_view->GetIsolate());
    } else {
...
static_assert(v8::TypedArray::kMaxLength == i::JSTypedArray::kMaxLength, "v8::TypedArray::kMaxLength must match i::JSTypedArray::kMaxLength");
...
#define TYPED_ARRAY_NEW(Type, type, TYPE, ctype)                                                                                                                                   \
    ...
    {                                                                                                                                                                              \
        ...
        i::Handle<i::JSTypedArray> obj = i_isolate->factory()->NewJSTypedArray(i::kExternal##Type##Array, buffer, byte_offset, length);                                            \
    ...
    {                                                                                                                                                                              \
        ...
        i::Isolate* i_isolate = Utils::OpenHandle(*shared_array_buffer)->GetIsolate();                                                                                             \
        API_RCS_SCOPE(i_isolate, Type##Array, New);                                                                                                                                \
        ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);                                                                                                                                \
...
Local<DataView> DataView::New(Local<ArrayBuffer> array_buffer, size_t byte_offset, size_t byte_length)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::JSDataView> obj = i_isolate->factory()->NewJSDataView(buffer, byte_offset, byte_length);
    return Utils::ToLocal(obj);
}
...
Local<SharedArrayBuffer> v8::SharedArrayBuffer::New(Isolate* v8_isolate, size_t byte_length)
{
    CHECK(i::v8_flags.harmony_sharedarraybuffer);
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    ...
    std::unique_ptr<i::BackingStore> backing_store = i::BackingStore::Allocate(i_isolate, byte_length, i::SharedFlag::kShared, i::InitializedFlag::kZeroInitialized);
...
std::unique_ptr<v8::BackingStore> v8::SharedArrayBuffer::NewBackingStore(Isolate* v8_isolate, size_t byte_length)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    API_RCS_SCOPE(i_isolate, SharedArrayBuffer, NewBackingStore);
    Utils::ApiCheck(byte_length <= i::JSArrayBuffer::kMaxByteLength, "v8::SharedArrayBuffer::NewBackingStore", "Cannot construct SharedArrayBuffer, requested length is too big");
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    std::unique_ptr<i::BackingStoreBase> backing_store = i::BackingStore::Allocate(i_isolate, byte_length, i::SharedFlag::kShared, i::InitializedFlag::kZeroInitialized);
    if (!backing_store) {
...
std::unique_ptr<v8::BackingStore> v8::SharedArrayBuffer::NewBackingStore(void* data, size_t byte_length, v8::BackingStore::DeleterCallback deleter, void* deleter_data)
{
    CHECK_LE(byte_length, i::JSArrayBuffer::kMaxByteLength);
...
Local<Symbol> v8::Symbol::New(Isolate* v8_isolate, Local<String> name)
{
    ...
    if (!name.IsEmpty())
        result->set_description(*Utils::OpenHandle(*name));
    return Utils::ToLocal(result);
}
...
Local<Private> v8::Private::New(Isolate* v8_isolate, Local<String> name)
{
    ...
    if (!name.IsEmpty())
        symbol->set_description(*Utils::OpenHandle(*name));
...
MaybeLocal<BigInt> v8::BigInt::NewFromWords(Local<Context> context, int sign_bit, int word_count, const uint64_t* words)
{
    ...
    ENTER_V8_NO_SCRIPT(i_isolate, context, BigInt, NewFromWords, MaybeLocal<BigInt>(), InternalEscapableScope);
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(this);
    i::Handle<i::Object> last = i_isolate->handle_scope_implementer()->LastEnteredOrMicrotaskContext();
    if (last.is_null())
        return Local<Context>();
...
v8::Local<Value> Isolate::ThrowError(v8::Local<v8::String> message) { return ThrowException(v8::Exception::Error(message)); }
...
static void CallGCCallbackWithoutData(Isolate* v8_isolate, GCType type, GCCallbackFlags flags, void* data) { reinterpret_cast<Isolate::GCCallback>(data)(v8_isolate, type, flags); }
...
void Isolate::RequestGarbageCollectionForTesting(GarbageCollectionType type)
{
    ...
    if (type == kMinorGarbageCollection) {
        ...
        reinterpret_cast<i::Isolate*>(this)->heap()->PreciseCollectAllGarbage(i::Heap::kNoGCFlags, i::GarbageCollectionReason::kTesting, kGCCallbackFlagForced);
    }
}
...
void Isolate::RequestGarbageCollectionForTesting(GarbageCollectionType type, StackState stack_state)
{
    ...
    if (type == kFullGarbageCollection) {
        stack_scope.emplace(reinterpret_cast<i::Isolate*>(this)->heap(), i::EmbedderStackStateScope::kExplicitInvocation, stack_state);
    }
...
bool Isolate::IsCurrent() const { return reinterpret_cast<const i::Isolate*>(this)->IsCurrent(); }

// static
Isolate* Isolate::Allocate() { return reinterpret_cast<Isolate*>(i::Isolate::New()); }
...
#pragma clang diagnostic pop
...
#else
        i_isolate->InitWithoutSnapshot(); // weolar
#endif
    }
    ...
    i_isolate->set_only_terminate_in_safe_scope(params.only_terminate_in_safe_scope);
    ...
    if (!i_isolate->is_shared() && !i::V8::GetCurrentPlatform()->GetForegroundTaskRunner(v8_isolate)->NonNestableTasksEnabled()) {
...
Isolate::DisallowJavascriptExecutionScope::DisallowJavascriptExecutionScope(Isolate* v8_isolate, Isolate::DisallowJavascriptExecutionScope::OnFailure on_failure)
    ...
    , v8_isolate_(v8_isolate)
{
    ...
    case CRASH_ON_FAILURE:
        i::DisallowJavascriptExecution::Open(i_isolate, &was_execution_allowed_assert_);
        break;
    case THROW_ON_FAILURE:
        i::ThrowOnJavascriptExecution::Open(i_isolate, &was_execution_allowed_throws_);
        break;
    case DUMP_ON_FAILURE:
        i::DumpOnJavascriptExecution::Open(i_isolate, &was_execution_allowed_dump_);
        break;
    default:
        UNREACHABLE();
    }
...
Isolate::DisallowJavascriptExecutionScope::~DisallowJavascriptExecutionScope()
{
    ...
    switch (on_failure_) {
    case CRASH_ON_FAILURE:
        i::DisallowJavascriptExecution::Close(i_isolate, was_execution_allowed_assert_);
        break;
    case THROW_ON_FAILURE:
        i::ThrowOnJavascriptExecution::Close(i_isolate, was_execution_allowed_throws_);
        break;
    case DUMP_ON_FAILURE:
        i::DumpOnJavascriptExecution::Close(i_isolate, was_execution_allowed_dump_);
        break;
    default:
        UNREACHABLE();
    }
...
Isolate::SafeForTerminationScope::SafeForTerminationScope(v8::Isolate* v8_isolate)
    : i_isolate_(reinterpret_cast<i::Isolate*>(v8_isolate))
...
Isolate::SafeForTerminationScope::~SafeForTerminationScope() { i_isolate_->set_next_v8_call_is_safe_for_termination(prev_value_); }
...
void Isolate::GetHeapStatistics(HeapStatistics* heap_statistics)
{
    ...
    heap_statistics->external_memory_ = i_isolate->heap()->backing_store_bytes() < SIZE_MAX ? static_cast<size_t>(i_isolate->heap()->backing_store_bytes()) : SIZE_MAX;
...
size_t Isolate::NumberOfHeapSpaces() { return i::LAST_SPACE - i::FIRST_SPACE + 1; }
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate);
    i::Handle<i::NativeContext> native_context = handle(Utils::OpenHandle(*context)->native_context(), i_isolate);
    i::Handle<i::JSPromise> js_promise = i::Handle<i::JSPromise>::cast(Utils::OpenHandle(*promise_resolver));
    return i_isolate->heap()->MeasureMemoryDelegate(native_context, js_promise, mode);
}
...
void Isolate::SetEventLogger(LogEventCallback that)
{
    ...
    if (i::v8_flags.log_internal_timer_events)
        return;
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(this);
...
void Isolate::AtomicsWaitWakeHandle::Wake() { reinterpret_cast<i::AtomicsWaitWakeHandle*>(this)->Wake(); }
...
void Isolate::SetUseCounterCallback(UseCounterCallback callback) { reinterpret_cast<i::Isolate*>(this)->SetUseCounterCallback(callback); }
...
void Isolate::SetAddHistogramSampleFunction(AddHistogramSampleCallback callback) { reinterpret_cast<i::Isolate*>(this)->counters()->SetAddHistogramSampleFunction(callback); }
...
bool Isolate::IdleNotificationDeadline(double deadline_in_seconds)
{
    ...
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(this);
    if (!i::v8_flags.use_idle_notification)
        return true;
...
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(this);
    bool on_isolate_thread = i_isolate->was_locker_ever_used() ? i_isolate->thread_manager()->IsLockedByCurrentThread() : i::ThreadId::Current() == i_isolate->thread_id();
...
JSEntryStubs Isolate::GetJSEntryStubs()
{
    ...
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(this);
    std::array<std::pair<i::Builtin, JSEntryStub*>, 3> stubs = { { { i::Builtin::kJSEntry, &entry_stubs.js_entry_stub },
        { i::Builtin::kJSConstructEntry, &entry_stubs.js_construct_entry_stub }, { i::Builtin::kJSRunMicrotasksEntry, &entry_stubs.js_run_microtasks_entry_stub } } };
    for (auto& pair : stubs) {
        i::CodeT js_entry = i_isolate->builtins()->code(pair.first);
        pair.second->code.start = reinterpret_cast<const void*>(js_entry.InstructionStart());
...
CALLBACK_SETTER(ModifyCodeGenerationFromStringsCallback, ModifyCodeGenerationFromStringsCallback2, modify_code_gen_callback2)
...
CALLBACK_SETTER(WasmSimdEnabledCallback, WasmSimdEnabledCallback, wasm_simd_enabled_callback)

CALLBACK_SETTER(WasmExceptionsEnabledCallback, WasmExceptionsEnabledCallback, wasm_exceptions_enabled_callback)
...
bool Isolate::AddMessageListener(MessageCallback that, Local<Value> data) { return AddMessageListenerWithErrorLevel(that, kMessageError, data); }
...
bool Isolate::AddMessageListenerWithErrorLevel(MessageCallback that, int message_levels, Local<Value> data)
{
    ...
    i::Handle<i::Foreign> foreign = i_isolate->factory()->NewForeign(FUNCTION_ADDR(that));
    listener->set(0, *foreign);
    listener->set(1, data.IsEmpty() ? i::ReadOnlyRoots(i_isolate).undefined_value() : *Utils::OpenHandle(*data));
...
MicrotasksScope::MicrotasksScope(Isolate* v8_isolate, MicrotasksScope::Type type)
...
MicrotasksScope::~MicrotasksScope()
{
    if (run_) {
        ...
        if (MicrotasksPolicy::kScoped == microtask_queue_->microtasks_policy() && !i_isolate_->has_scheduled_exception()) {
            microtask_queue_->PerformCheckpoint(reinterpret_cast<Isolate*>(i_isolate_));
            DCHECK_IMPLIES(i_isolate_->has_scheduled_exception(), i_isolate_->is_execution_terminating());
        }
    }
#ifdef DEBUG
    if (!run_)
...
v8::MaybeLocal<v8::Array> v8::Object::PreviewEntries(bool* is_key_value)
{
    ...
    if (object->IsJSWeakCollection()) {
        *is_key_value = object->IsJSWeakMap();
        return Utils::ToLocal(i::JSWeakCollection::GetEntries(i::Handle<i::JSWeakCollection>::cast(object), 0));
    }
...
Local<String> CpuProfileNode::GetFunctionName() const
{
    ...
    i::Handle<i::String> name = i_isolate->factory()->InternalizeUtf8String(entry->name());
...
int CpuProfileNode::GetLineNumber() const { return reinterpret_cast<const i::ProfileNode*>(this)->line_number(); }

int CpuProfileNode::GetColumnNumber() const { return reinterpret_cast<const i::ProfileNode*>(this)->entry()->column_number(); }
...
unsigned CpuProfileNode::GetHitCount() const { return reinterpret_cast<const i::ProfileNode*>(this)->self_ticks(); }

unsigned CpuProfileNode::GetNodeId() const { return reinterpret_cast<const i::ProfileNode*>(this)->id(); }

CpuProfileNode::SourceType CpuProfileNode::GetSourceType() const { return reinterpret_cast<const i::ProfileNode*>(this)->source_type(); }

int CpuProfileNode::GetChildrenCount() const { return static_cast<int>(reinterpret_cast<const i::ProfileNode*>(this)->children()->size()); }
...
int CpuProfile::GetSamplesCount() const { return reinterpret_cast<const i::CpuProfile*>(this)->samples_count(); }
...
void* CpuProfilingOptions::raw_filter_context() const { return reinterpret_cast<void*>(i::Context::cast(*Utils::OpenPersistent(filter_context_)).native_context().address()); }

void CpuProfiler::Dispose() { delete reinterpret_cast<i::CpuProfiler*>(this); }

// static
void CpuProfiler::CollectSample(Isolate* v8_isolate) { i::CpuProfiler::CollectSample(reinterpret_cast<i::Isolate*>(v8_isolate)); }
...
void CpuProfiler::SetUsePreciseSampling(bool use_precise_sampling) { reinterpret_cast<i::CpuProfiler*>(this)->set_use_precise_sampling(use_precise_sampling); }
...
{
    return reinterpret_cast<i::CpuProfiler*>(this)->StartProfiling(*Utils::OpenHandle(*title), std::move(options), std::move(delegate));
}
...
CpuProfilingResult CpuProfiler::Start(Local<String> title, bool record_samples)
{
    ...
    return reinterpret_cast<i::CpuProfiler*>(this)->StartProfiling(*Utils::OpenHandle(*title), std::move(options));
}
...
CpuProfilingStatus CpuProfiler::StartProfiling(Local<String> title, bool record_samples) { return Start(title, record_samples).status; }
...
{
    return reinterpret_cast<CpuProfile*>(reinterpret_cast<i::CpuProfiler*>(this)->StopProfiling(*Utils::OpenHandle(*title)));
}

CpuProfile* CpuProfiler::Stop(ProfilerId id) { return reinterpret_cast<CpuProfile*>(reinterpret_cast<i::CpuProfiler*>(this)->StopProfiling(id)); }

void CpuProfiler::UseDetailedSourcePositionsForProfiling(Isolate* v8_isolate) { reinterpret_cast<i::Isolate*>(v8_isolate)->SetDetailedSourcePositionsForProfiling(true); }

uintptr_t CodeEvent::GetCodeStartAddress() { return reinterpret_cast<i::CodeEvent*>(this)->code_start_address; }

size_t CodeEvent::GetCodeSize() { return reinterpret_cast<i::CodeEvent*>(this)->code_size; }

Local<String> CodeEvent::GetFunctionName() { return ToApiHandle<String>(reinterpret_cast<i::CodeEvent*>(this)->function_name); }

Local<String> CodeEvent::GetScriptName() { return ToApiHandle<String>(reinterpret_cast<i::CodeEvent*>(this)->script_name); }

int CodeEvent::GetScriptLine() { return reinterpret_cast<i::CodeEvent*>(this)->script_line; }

int CodeEvent::GetScriptColumn() { return reinterpret_cast<i::CodeEvent*>(this)->script_column; }

CodeEventType CodeEvent::GetCodeType() { return reinterpret_cast<i::CodeEvent*>(this)->code_type; }

const char* CodeEvent::GetComment() { return reinterpret_cast<i::CodeEvent*>(this)->comment; }

uintptr_t CodeEvent::GetPreviousCodeStartAddress() { return reinterpret_cast<i::CodeEvent*>(this)->previous_code_start_address; }
...
CodeEventHandler::CodeEventHandler(Isolate* v8_isolate) { internal_listener_ = new i::ExternalLogEventListener(reinterpret_cast<i::Isolate*>(v8_isolate)); }

CodeEventHandler::~CodeEventHandler() { delete reinterpret_cast<i::ExternalLogEventListener*>(internal_listener_); }

void CodeEventHandler::Enable() { reinterpret_cast<i::ExternalLogEventListener*>(internal_listener_)->StartListening(this); }

void CodeEventHandler::Disable() { reinterpret_cast<i::ExternalLogEventListener*>(internal_listener_)->StopListening(); }

static i::HeapGraphEdge* ToInternal(const HeapGraphEdge* edge) { return const_cast<i::HeapGraphEdge*>(reinterpret_cast<const i::HeapGraphEdge*>(edge)); }

HeapGraphEdge::Type HeapGraphEdge::GetType() const { return static_cast<HeapGraphEdge::Type>(ToInternal(this)->type()); }
...
static i::HeapEntry* ToInternal(const HeapGraphNode* entry) { return const_cast<i::HeapEntry*>(reinterpret_cast<const i::HeapEntry*>(entry)); }

HeapGraphNode::Type HeapGraphNode::GetType() const { return static_cast<HeapGraphNode::Type>(ToInternal(this)->type()); }
...
size_t HeapGraphNode::GetShallowSize() const { return ToInternal(this)->self_size(); }

int HeapGraphNode::GetChildrenCount() const { return ToInternal(this)->children_count(); }

const HeapGraphEdge* HeapGraphNode::GetChild(int index) const { return reinterpret_cast<const HeapGraphEdge*>(ToInternal(this)->child(index)); }

static i::HeapSnapshot* ToInternal(const HeapSnapshot* snapshot) { return const_cast<i::HeapSnapshot*>(reinterpret_cast<const i::HeapSnapshot*>(snapshot)); }
...
const HeapGraphNode* HeapSnapshot::GetRoot() const { return reinterpret_cast<const HeapGraphNode*>(ToInternal(this)->root()); }

const HeapGraphNode* HeapSnapshot::GetNodeById(SnapshotObjectId id) const { return reinterpret_cast<const HeapGraphNode*>(ToInternal(this)->GetEntryById(id)); }

int HeapSnapshot::GetNodesCount() const { return static_cast<int>(ToInternal(this)->entries().size()); }

const HeapGraphNode* HeapSnapshot::GetNode(int index) const { return reinterpret_cast<const HeapGraphNode*>(&ToInternal(this)->entries().at(index)); }

SnapshotObjectId HeapSnapshot::GetMaxSnapshotJSObjectId() const { return ToInternal(this)->max_snapshot_js_object_id(); }
...
int HeapProfiler::GetSnapshotCount() { return reinterpret_cast<i::HeapProfiler*>(this)->GetSnapshotsCount(); }

const HeapSnapshot* HeapProfiler::GetHeapSnapshot(int index) { return reinterpret_cast<const HeapSnapshot*>(reinterpret_cast<i::HeapProfiler*>(this)->GetSnapshot(index)); }
...
SnapshotObjectId HeapProfiler::GetObjectId(NativeObject value) { return reinterpret_cast<i::HeapProfiler*>(this)->GetSnapshotObjectId(value); }
...
void HeapProfiler::ClearObjectIds() { reinterpret_cast<i::HeapProfiler*>(this)->ClearHeapObjectMap(); }
...
void HeapProfiler::StartTrackingHeapObjects(bool track_allocations) { reinterpret_cast<i::HeapProfiler*>(this)->StartHeapObjectsTracking(track_allocations); }

void HeapProfiler::StopTrackingHeapObjects() { reinterpret_cast<i::HeapProfiler*>(this)->StopHeapObjectsTracking(); }
...
void HeapProfiler::StopSamplingHeapProfiler() { reinterpret_cast<i::HeapProfiler*>(this)->StopSamplingHeapProfiler(); }

AllocationProfile* HeapProfiler::GetAllocationProfile() { return reinterpret_cast<i::HeapProfiler*>(this)->GetAllocationProfile(); }

void HeapProfiler::DeleteAllHeapSnapshots() { reinterpret_cast<i::HeapProfiler*>(this)->DeleteAllSnapshots(); }
...
{
    CHECK(v8_isolate_);
    reinterpret_cast<i::Isolate*>(v8_isolate_)->heap()->SetStackStart(stack_start);
}
...
void EmbedderHeapTracer::FinalizeTracing()
{
    if (v8_isolate_) {
        ...
        if (i_isolate->heap()->incremental_marking()->IsMarking()) {
            i_isolate->heap()->FinalizeIncrementalMarkingAtomically(i::GarbageCollectionReason::kExternalFinalize);
        }
    }
}
...
{
    if (v8_isolate_) {
        i::LocalEmbedderHeapTracer* const tracer = reinterpret_cast<i::Isolate*>(v8_isolate_)->heap()->local_embedder_heap_tracer();
...
void EmbedderHeapTracer::RegisterEmbedderReference(const BasicTracedReference<v8::Data>& ref)
{
    if (ref.IsEmpty())
        return;

    i::Heap* const heap = reinterpret_cast<i::Isolate*>(v8_isolate_)->heap();
    heap->RegisterExternallyReferencedObject(reinterpret_cast<i::Address*>(ref.val_));
}

void EmbedderHeapTracer::IterateTracedGlobalHandles(TracedGlobalHandleVisitor* visitor)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(v8_isolate_);
    i::DisallowGarbageCollection no_gc;
...
bool EmbedderHeapTracer::IsRootForNonTracingGC(const v8::TracedReference<v8::Value>& handle) { return true; }

void EmbedderHeapTracer::ResetHandleInNonTracingGC(const v8::TracedReference<v8::Value>& handle) { UNREACHABLE(); }
...
CFunctionInfo::CFunctionInfo(const CTypeInfo& return_info, unsigned int arg_count, const CTypeInfo* arg_info)
...
void FastApiTypedArrayBase::ValidateIndex(size_t index) const { DCHECK_LT(index, length_); }
...
WasmStreaming::WasmStreaming(std::unique_ptr<WasmStreamingImpl>) { UNREACHABLE(); }

WasmStreaming::~WasmStreaming() = default;

void WasmStreaming::OnBytesReceived(const uint8_t* bytes, size_t size) { UNREACHABLE(); }
...
bool WasmStreaming::SetCompiledModuleBytes(const uint8_t* bytes, size_t size) { UNREACHABLE(); }

void WasmStreaming::SetMoreFunctionsCanBeSerializedCallback(std::function<void(CompiledWasmModule)>) { UNREACHABLE(); }

void WasmStreaming::SetUrl(const char* url, size_t length) { UNREACHABLE(); }

// static
std::shared_ptr<WasmStreaming> WasmStreaming::Unpack(Isolate* v8_isolate, Local<Value> value) { FATAL("WebAssembly is disabled"); }
#endif // !V8_ENABLE_WEBASSEMBLY
...
int HandleScopeImplementer::ArchiveSpacePerThread() { return sizeof(HandleScopeImplementer); }
...
void HandleScopeImplementer::IterateThis(RootVisitor* v)
{
    ...
    for (int i = static_cast<int>(blocks()->size()) - 2; i >= 0; --i) {
        ...
        if (last_handle_before_deferred_block_ != nullptr && (reinterpret_cast<Address>(last_handle_before_deferred_block_) <= reinterpret_cast<Address>(&block[kHandleBlockSize]))
            && (reinterpret_cast<Address>(last_handle_before_deferred_block_) >= reinterpret_cast<Address>(block))) {
            v->VisitRootPointers(Root::kHandleScope, nullptr, FullObjectSlot(block), FullObjectSlot(last_handle_before_deferred_block_));
    ...
    DetachableVector<Context>* context_lists[2] = { &saved_contexts_, &entered_contexts_ };
    for (unsigned i = 0; i < arraysize(context_lists); i++) {
        ...
        v->VisitRootPointers(Root::kHandleScope, nullptr, start, start + static_cast<int>(context_lists[i]->size()));
    }
...
void InvokeFinalizationRegistryCleanupFromTask(Handle<Context> context, Handle<JSFinalizationRegistry> finalization_registry, Handle<Object> callback)
{
    ...
    if (Execution::CallBuiltin(i_isolate, i_isolate->finalization_registry_cleanup_some(), finalization_registry, arraysize(argv), argv).is_null()) {
        call_depth_scope.Escape();
    }
}

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) int32_t ConvertDouble(double d) { return internal::DoubleToInt32(d); }

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) uint32_t ConvertDouble(double d) { return internal::DoubleToUint32(d); }

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) float ConvertDouble(double d) { return internal::DoubleToFloat32(d); }

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) double ConvertDouble(double d) { return d; }

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) int64_t ConvertDouble(double d) { return internal::DoubleToWebIDLInt64(d); }

template <> EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) uint64_t ConvertDouble(double d) { return internal::DoubleToWebIDLUint64(d); }
...
template <>
bool V8_EXPORT V8_WARN_UNUSED_RESULT TryToCopyAndConvertArrayToCppBuffer<CTypeInfoBuilder<int32_t>::Build().GetId(), int32_t>(Local<Array> src, int32_t* dst, uint32_t max_length)
{
...
template <>
bool V8_EXPORT V8_WARN_UNUSED_RESULT TryToCopyAndConvertArrayToCppBuffer<CTypeInfoBuilder<float>::Build().GetId(), float>(Local<Array> src, float* dst, uint32_t max_length)
{
...
template <>
bool V8_EXPORT V8_WARN_UNUSED_RESULT TryToCopyAndConvertArrayToCppBuffer<CTypeInfoBuilder<double>::Build().GetId(), double>(Local<Array> src, double* dst, uint32_t max_length)
{
