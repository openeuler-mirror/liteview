...
#define ENTER_V8_HELPER_INTERNAL(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass, do_callback)                                                      \
...
#define PREPARE_FOR_EXECUTION_WITH_CONTEXT(context, class_name, function_name, bailout_value, HandleScopeClass, do_callback)                                                       \
    auto i_isolate = context.IsEmpty() ? i::Isolate::Current() : reinterpret_cast<i::Isolate*>(context->GetIsolate());                                                             \
    ENTER_V8_HELPER_INTERNAL(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass, do_callback);

#define PREPARE_FOR_EXECUTION(context, class_name, function_name, T)                                                                                                               \
    PREPARE_FOR_EXECUTION_WITH_CONTEXT(context, class_name, function_name, MaybeLocal<T>(), InternalEscapableScope, false)

#define ENTER_V8(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass)                                                                                   \
    ENTER_V8_HELPER_INTERNAL(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass, true)

#ifdef DEBUG
#define ENTER_V8_NO_SCRIPT(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass)                                                                         \
    ENTER_V8_HELPER_INTERNAL(i_isolate, context, class_name, function_name, bailout_value, HandleScopeClass, false);                                                               \
...
#define ENTER_V8_MAYBE_TEARDOWN(i_isolate) i::VMState<v8::OTHER> __state__((i_isolate));
...
#define RETURN_ON_FAILED_EXECUTION(T) EXCEPTION_BAILOUT_CHECK_SCOPED_DO_NOT_USE(i_isolate, MaybeLocal<T>())

#define RETURN_ON_FAILED_EXECUTION_PRIMITIVE(T) EXCEPTION_BAILOUT_CHECK_SCOPED_DO_NOT_USE(i_isolate, Nothing<T>())
