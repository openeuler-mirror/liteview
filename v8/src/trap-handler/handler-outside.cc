...
void ValidateCodeObjects()
{
    ...
    for (unsigned i = 0; i < gNumCodeObjects; ++i) {
        ...
        for (unsigned j = 0; j < data->num_protected_instructions; ++j) {
            ...
            TH_DCHECK(data->instructions[j].landing_offset > data->instructions[j].instr_offset);
        }
    }
...
bool IsThreadInWasm()
{
    int* thread_in_wasm_code_ptr = GetThreadInWasmThreadLocalAddress();
...
int* GetThreadInWasmThreadLocalAddress()
{
    ...
    do {
        flag = MB_InterlockedCompareExchange((long volatile*)&(run_once), 2, 0);
        if (0 == flag) {
            g_thread_in_wasm_code_tls = MbTlsAlloc();

            thread_in_wasm_code_ptr = (int*)malloc(sizeof(int));
            *thread_in_wasm_code_ptr = 0;
            MbTlsSetValue(g_thread_in_wasm_code_tls, thread_in_wasm_code_ptr);

            MB_InterlockedExchange((long volatile*)&(run_once), 1);
            break;
        } else if (1 == flag) {
            thread_in_wasm_code_ptr = (int*)MbTlsGetValue(g_thread_in_wasm_code_tls);
            if (!thread_in_wasm_code_ptr) {
                thread_in_wasm_code_ptr = (int*)malloc(sizeof(int));
                *thread_in_wasm_code_ptr = 0;
            }
            break;
...
size_t GetRecoveredTrapCount() { return gRecoveredTrapCount.load(std::memory_order_relaxed); }
