...
constexpr digit_t digit_pow_rec(digit_t base, digit_t exponent) { return exponent == 1 ? base : base * digit_pow_rec(base, exponent - 1); }
...
public:
    ToStringFormatter(Digits X, int radix, bool sign, char* out, int chars_available, ProcessorImpl* processor)
        : digits_(X)
        , radix_(radix)
        , sign_(sign)
        , out_start_(out)
...
void ProcessorImpl::ToString(char* out, int* out_length, Digits X, int radix, bool sign)
{
...
void ProcessorImpl::ToStringImpl(char* out, int* out_length, Digits X, int radix, bool sign, bool fast)
{
#if DEBUG
...
Status Processor::ToString(char* out, int* out_length, Digits X, int radix, bool sign)
{
...
int ToStringResultLength(Digits X, int radix, bool sign)
{
    ...
    if (IsPowerOfTwo(radix)) {
        ...
        DCHECK(chars_required < static_cast<uint64_t>(std::numeric_limits<int>::max()));
