...
inline double JunkStringValue() { return base::bit_cast<double, uint64_t>(kQuietNaNMask); }

inline double SignedZero(bool negative) { return negative ? base::uint64_to_double(base::Double::kSignMask) : 0.0; }
...
template <class Iterator, class EndMark> bool SubStringEquals(Iterator* current, EndMark end, const char* substring)
{
...
template <class Iterator, class EndMark> inline bool AdvanceToNonspace(Iterator* current, EndMark end)
{
    while (*current != end) {
...
template <int radix_log_2, class Iterator, class EndMark> double InternalStringToIntDouble(Iterator current, EndMark end, bool negative, bool allow_trailing_junk)
{
    DCHECK(current != end);
...
template <typename IsolateT> class StringToIntHelper {
public:
...
protected:
    ...
    // ParseInt().
    void set_allow_binary_and_octal_prefixes() { allow_binary_and_octal_prefixes_ = true; }
    void set_disallow_trailing_junk() { allow_trailing_junk_ = false; }
    bool allow_trailing_junk() { return allow_trailing_junk_; }

    bool IsOneByte() const { return raw_one_byte_subject_ != nullptr || String::IsOneByteRepresentationUnderneath(*subject_); }
    ...
    base::Vector<const base::uc16> GetTwoByteVector(const DisallowGarbageCollection& no_gc) { return subject_->GetFlatContent(no_gc).ToUC16Vector(); }
...
private:
    template <class Char> void DetectRadixInternal(Char current, int length);
...
template <typename IsolateT> void StringToIntHelper<IsolateT>::ParseInt()
{
...
template <typename IsolateT> template <class Char> void StringToIntHelper<IsolateT>::DetectRadixInternal(Char current, int length)
{
    Char start = current;
    length_ = length;
...
public:
    ...
    template <class Char> void ParseInternal(Char start)
    {
    ...
    void ParseTwoByte(const base::uc16* start) final { return ParseInternal(start); }
...
private:
    template <class Char> void HandleGenericCase(Char current, Char end);

    template <class Char> double HandlePowerOfTwoCase(Char current, Char end)
    {
    ...
    template <class Char> void HandleBaseTenCase(Char current, Char end)
    {
        // Parsing with strtod.
...
template <class Char> void NumberParseIntHelper::HandleGenericCase(Char current, Char end)
{
...
template <class Iterator, class EndMark> double InternalStringToDouble(Iterator current, EndMark end, int flags, double empty_string_val)
{
    ...
    if (*current == '0') {
        ++current;
        if (current == end)
            return SignedZero(sign == Sign::kNegative);
        ...
        if ((flags & ALLOW_HEX) && (*current == 'x' || *current == 'X')) {
            ...
            return InternalStringToIntDouble<4>(current, end, false, allow_trailing_junk);
            ...
            return InternalStringToIntDouble<3>(current, end, false, allow_trailing_junk);
            ...
            return InternalStringToIntDouble<1>(current, end, false, allow_trailing_junk);
        }
...
parsing_done:
    ...
    if (octal) {
        return InternalStringToIntDouble<3>(buffer, buffer + buffer_pos, sign == Sign::kNegative, allow_trailing_junk);
    }
    ...
    double converted = Strtod(base::Vector<const char>(buffer, buffer_pos), exponent);
...
double StringToDouble(base::Vector<const uint8_t> str, int flags, double empty_string_val) { return InternalStringToDouble(str.begin(), str.end(), flags, empty_string_val); }

double StringToDouble(base::Vector<const base::uc16> str, int flags, double empty_string_val)
{
...
template <typename IsolateT> class StringToBigIntHelper : public StringToIntHelper<IsolateT> {
public:
    ...
    {
        this->ParseInt();
        if (behavior_ == Behavior::kStringToBigInt && this->sign() != Sign::kNone && this->radix() != 10) {
        ...
        case State::kDone:
            return BigInt::Allocate(this->isolate(), &accumulator_, this->negative(), allocation_type());
        case State::kEmpty:
        case State::kRunning:
            break;
        }
...
char* DoubleToExponentialCString(double value, int f)
{
    ...
    if (f == -1) {
        ...
        base::DoubleToAscii(value, base::DTOA_PRECISION, f + 1, base::Vector<char>(decimal_rep, kV8DtoaBufferCapacity), &sign, &decimal_rep_length, &decimal_point);
    }
...
double StringToDouble(Isolate* isolate, Handle<String> string, int flags, double empty_string_val)
{
...
base::Optional<double> TryStringToDouble(LocalIsolate* isolate, Handle<String> object, int max_length_for_conversion)
{
    DisallowGarbageCollection no_gc;
