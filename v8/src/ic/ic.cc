...
void IC::TraceIC(const char* type, Handle<Object> name, State old_state, State new_state)
{
    ...
    JavaScriptFrame::CollectFunctionAndOffsetForICStats(function, code, code_offset);
...
static void LookupForRead(LookupIterator* it, bool is_has_property)
{
    for (; it->IsFound(); it->Next()) {
        ...
        case LookupIterator::INTERCEPTOR: {
            ...
            if (!holder->GetNamedInterceptor().getter().IsUndefined(it->isolate())) {
                return;
            }
...
bool IC::ShouldRecomputeHandler(Handle<String> name)
{
    ...
    if (maybe_handler.is_null()) {
        if (!lookup_start_object_map()->IsJSObjectMap())
            return false;
...
// static
void IC::OnFeedbackChanged(Isolate* isolate, FeedbackVector vector, FeedbackSlot slot, const char* reason)
{
    if (v8_flags.trace_opt_verbose) {
        if (vector.profiler_ticks() != 0) {
            ...
            os << " from " << vector.profiler_ticks() << " due to IC change: " << reason << "]" << std::endl;
        }
    }
...
bool IC::ConfigureVectorState(IC::State new_state, Handle<Object> key)
{
    ...
    bool changed = nexus()->ConfigureMegamorphic(key->IsName() ? IcCheckType::kProperty : IcCheckType::kElement);
    OnFeedbackChanged("Megamorphic");
    return changed;
}

void IC::ConfigureVectorState(Handle<Name> name, Handle<Map> map, Handle<Object> handler) { ConfigureVectorState(name, map, MaybeObjectHandle(handler)); }

void IC::ConfigureVectorState(Handle<Name> name, Handle<Map> map, const MaybeObjectHandle& handler)
{
    if (IsGlobalIC()) {
...
void IC::ConfigureVectorState(Handle<Name> name, MapHandles const& maps, MaybeObjectHandles* handlers)
{
    DCHECK(!IsGlobalIC());
...
MaybeHandle<Object> LoadIC::Load(Handle<Object> object, Handle<Name> name, bool update_feedback, Handle<Object> receiver)
{
    ...
    if (IsAnyHas() ? !object->IsJSReceiver() : object->IsNullOrUndefined(isolate())) {
        if (use_ic) {
        ...
        if (*name == ReadOnlyRoots(isolate()).iterator_symbol()) {
            return isolate()->Throw<Object>(ErrorUtils::NewIteratorError(isolate(), object));
        }
    ...
    if (name->IsPrivate()) {
        ...
        if (!IsAnyHas() && private_symbol->is_private_name() && !it.IsFound()) {
            Handle<String> name_string(String::cast(private_symbol->description()), isolate());
            if (private_symbol->is_private_brand()) {
                Handle<String> class_name = (name_string->length() == 0) ? isolate()->factory()->anonymous_string() : name_string;
                return TypeError(MessageTemplate::kInvalidPrivateBrandInstance, object, class_name);
            }
            return TypeError(MessageTemplate::kInvalidPrivateMemberRead, object, name_string);
        }
    ...
    if (it.IsFound() || !ShouldThrowReferenceError()) {
        ...
        if (IsAnyHas()) {
            ...
            if (maybe.IsNothing())
                return MaybeHandle<Object>();
            return maybe.FromJust() ? ReadOnlyRoots(isolate()).true_value_handle() : ReadOnlyRoots(isolate()).false_value_handle();
        }

        // Get the property.
        Handle<Object> result;

        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result, Object::GetProperty(&it, IsLoadGlobalIC()), Object);
        if (it.IsFound()) {
            return result;
...
MaybeHandle<Object> LoadGlobalIC::Load(Handle<Name> name, bool update_feedback)
{
    ...
    if (name->IsString()) {
        ...
        Handle<ScriptContextTable> script_contexts(global->native_context().script_context_table(), isolate());

        VariableLookupResult lookup_result;
        if (script_contexts->Lookup(str_name, &lookup_result)) {
            Handle<Context> script_context = ScriptContextTable::GetContext(isolate(), script_contexts, lookup_result.context_index);
            ...
            if (result->IsTheHole(isolate())) {
                ...
                THROW_NEW_ERROR(isolate(), NewReferenceError(MessageTemplate::kAccessedUninitializedVariable, name), Object);
            }
            ...
            if (use_ic) {
                ...
                if (nexus()->ConfigureLexicalVarMode(
                        lookup_result.context_index, lookup_result.slot_index, (lookup_result.mode == VariableMode::kConst && !lookup_result.is_repl_mode))) {
...
static bool AddOneReceiverMapIfMissing(MapHandles* receiver_maps, Handle<Map> new_receiver_map)
{
...
static bool AddOneReceiverMapIfMissing(std::vector<MapAndHandler>* receiver_maps_and_handlers, Handle<Map> new_receiver_map)
{
    DCHECK(!new_receiver_map.is_null());
...
{
    if (!v8_flags.enable_mega_dom_ic)
        return false;
    ...
    Handle<Context> accessor_context(call_optimization.GetAccessorContext(*map), isolate());

    Handle<FunctionTemplateInfo> fti;
    if (accessor_obj->IsJSFunction()) {
        fti = handle(JSFunction::cast(*accessor_obj).shared().get_api_func_data(), isolate());
    } else {
...
bool IC::IsTransitionOfMonomorphicTarget(Map source_map, Map target_map)
{
    ...
    if (more_general_transition) {
        ...
        transitioned_map = source_map.FindElementsKindTransitionedMap(isolate(), map_list, ConcurrencyMode::kSynchronous);
    }
    return transitioned_map == target_map;
}

void IC::SetCache(Handle<Name> name, Handle<Object> handler) { SetCache(name, MaybeObjectHandle(handler)); }
...
void LoadIC::UpdateCaches(LookupIterator* lookup)
{
    ...
    if (lookup->state() == LookupIterator::ACCESS_CHECK) {
        ...
        handler = MaybeObjectHandle(LoadHandler::LoadFullChain(isolate(), lookup_start_object_map(), MaybeObjectHandle(isolate()->factory()->null_value()), smi_handler));
...
void IC::UpdateMegamorphicCache(Handle<Map> map, Handle<Name> name, const MaybeObjectHandle& handler)
{
...
MaybeObjectHandle LoadIC::ComputeHandler(LookupIterator* lookup)
{
    ...
    if (!IsAnyHas() && !lookup->IsElement()) {
        if (lookup_start_object->IsString() && *lookup->name() == roots.length_string()) {
            TRACE_HANDLER_STATS(isolate(), LoadIC_StringLength);
            return MaybeObjectHandle(BUILTIN_CODE(isolate(), LoadIC_StringLength));
        }

        if (lookup_start_object->IsStringWrapper() && *lookup->name() == roots.length_string()) {
        ...
        if (lookup_start_object->IsJSFunction() && *lookup->name() == roots.prototype_string() && !JSFunction::cast(*lookup_start_object).PrototypeRequiresRuntimeLookup()) {
    ...
    case LookupIterator::ACCESSOR: {
        ...
        if (holder->IsJSModuleNamespace()) {
            Handle<ObjectHashTable> exports(Handle<JSModuleNamespace>::cast(holder)->module().exports(), isolate());
            InternalIndex entry = exports->FindEntry(isolate(), roots, lookup->name(), Smi::ToInt(lookup->name()->GetHash()));
        ...
        if (accessors->IsAccessorPair()) {
            Handle<AccessorPair> accessor_pair = Handle<AccessorPair>::cast(accessors);
            ...
            if ((getter->IsFunctionTemplateInfo() && FunctionTemplateInfo::cast(*getter).BreakAtEntry())
                || (getter->IsJSFunction() && JSFunction::cast(*getter).shared().BreakAtEntry())) {
            ...
            if (call_optimization.is_simple_api_call()) {
                ...
                Handle<Context> context(call_optimization.GetAccessorContext(holder->map()), isolate());

                TRACE_HANDLER_STATS(isolate(), LoadIC_LoadApiGetterFromPrototypeDH);
                return MaybeObjectHandle(LoadHandler::LoadFromPrototype(
                    isolate(), map, holder, smi_handler, MaybeObjectHandle::Weak(call_optimization.api_call_info()), MaybeObjectHandle::Weak(context)));
            }
            ...
            if (holder->HasFastProperties()) {
                ...
                return MaybeObjectHandle(
                    LoadHandler::LoadFromPrototype(isolate(), map, holder, LoadHandler::LoadAccessorFromPrototype(isolate()), MaybeObjectHandle::Weak(getter)));
            }

            if (holder->IsJSGlobalObject()) {
                TRACE_HANDLER_STATS(isolate(), LoadIC_LoadGlobalFromPrototypeDH);
                smi_handler = LoadHandler::LoadGlobal(isolate());
                return MaybeObjectHandle(LoadHandler::LoadFromPrototype(isolate(), map, holder, smi_handler, MaybeObjectHandle::Weak(lookup->GetPropertyCell())));
            } else {
        ...
        if (!info->has_getter() || !holder->HasFastProperties() || (info->is_sloppy() && !receiver->IsJSReceiver())) {
            TRACE_HANDLER_STATS(isolate(), LoadIC_SlowStub);
            return MaybeObjectHandle(LoadHandler::LoadSlow(isolate()));
        }
    ...
    case LookupIterator::DATA: {
        ...
        if (lookup->constness() == PropertyConstness::kConst && !holder_is_lookup_start_object) {
            ...
            if (!value->IsString() || (value->IsString() && value->IsInternalizedString())) {
                MaybeObjectHandle weak_value = value->IsSmi() ? MaybeObjectHandle(*value, isolate()) : MaybeObjectHandle::Weak(*value, isolate());
...
void KeyedLoadIC::UpdateLoadElement(Handle<HeapObject> receiver, KeyedAccessLoadMode load_mode)
{
    ...
    if (state() == MONOMORPHIC) {
        if ((receiver->IsJSObject() && IsMoreGeneralElementsKindTransition(target_receiver_maps.at(0)->elements_kind(), Handle<JSObject>::cast(receiver)->GetElementsKind()))
...
bool AllowConvertHoleElementToUndefined(Isolate* isolate, Handle<Map> receiver_map)
{
    ...
    if (Protectors::IsNoElementsIntact(isolate)) {
        ...
        if (receiver_map->IsJSObjectMap()) {
            ...
            if (isolate->IsInAnyContext(*receiver_prototype, Context::INITIAL_ARRAY_PROTOTYPE_INDEX)
                || isolate->IsInAnyContext(*receiver_prototype, Context::INITIAL_OBJECT_PROTOTYPE_INDEX)) {
                return true;
            }
        }
    }
...
Handle<Object> KeyedLoadIC::LoadElementHandler(Handle<Map> receiver_map, KeyedAccessLoadMode load_mode)
{
    // Has a getter interceptor, or is any has and has a query interceptor.
    if (receiver_map->has_indexed_interceptor()
        && (!receiver_map->GetIndexedInterceptor().getter().IsUndefined(isolate()) || (IsAnyHas() && !receiver_map->GetIndexedInterceptor().query().IsUndefined(isolate())))
    ...
    if (elements_kind == DICTIONARY_ELEMENTS) {
        ...
        return LoadHandler::LoadElement(isolate(), elements_kind, false, is_js_array, load_mode);
    }
    ...
    TRACE_HANDLER_STATS(isolate(), KeyedLoadIC_LoadElementDH);
    return LoadHandler::LoadElement(isolate(), elements_kind, convert_hole_to_undefined, is_js_array, load_mode);
}

void KeyedLoadIC::LoadElementPolymorphicHandlers(MapHandles* receiver_maps, MaybeObjectHandles* handlers, KeyedAccessLoadMode load_mode)
{
    // Filter out deprecated maps to ensure their instances get migrated.
    receiver_maps->erase(std::remove_if(receiver_maps->begin(), receiver_maps->end(), [](const Handle<Map>& map) { return map->is_deprecated(); }), receiver_maps->end());
    ...
    for (Handle<Map> receiver_map : *receiver_maps) {
        ...
        if (receiver_map->is_stable()) {
            Map tmap = receiver_map->FindElementsKindTransitionedMap(isolate(), *receiver_maps, ConcurrencyMode::kSynchronous);
            if (!tmap.is_null()) {
                receiver_map->NotifyLeafMapLayoutChange(isolate());
            }
        }
        handlers->push_back(MaybeObjectHandle(LoadElementHandler(receiver_map, load_mode)));
    }
}
...
KeyedAccessLoadMode GetLoadMode(Isolate* isolate, Handle<Object> receiver, size_t index)
{
...
MaybeHandle<Object> KeyedLoadIC::RuntimeLoad(Handle<Object> object, Handle<Object> key)
{
    Handle<Object> result;

    if (IsKeyedLoadIC()) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result, Runtime::GetObjectProperty(isolate(), object, key), Object);
    } else {
        DCHECK(IsKeyedHasIC());
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result, Runtime::HasProperty(isolate(), object, key), Object);
    }
    return result;
}

MaybeHandle<Object> KeyedLoadIC::Load(Handle<Object> object, Handle<Object> key)
{
    if (MigrateDeprecated(isolate(), object)) {
    ...
    if (key_type == kName) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), load_handle, LoadIC::Load(object, maybe_name), Object);
    } else if (key_type == kIntPtr && CanCache(object, state()) && IntPtrKeyToSize(maybe_index, Handle<HeapObject>::cast(object), &index)) {
...
bool StoreIC::LookupForWrite(LookupIterator* it, Handle<Object> value, StoreOrigin store_origin)
{
    ...
    if (it->state() != LookupIterator::TRANSITION) {
        for (; it->IsFound(); it->Next()) {
            ...
            case LookupIterator::INTERCEPTOR: {
                ...
                if (it->HolderIsReceiverOrHiddenPrototype() || !info.getter().IsUndefined(isolate()) || !info.query().IsUndefined(isolate())) {
                    return true;
                }
                break;
            }
...
MaybeHandle<Object> StoreGlobalIC::Store(Handle<Name> name, Handle<Object> value)
{
    ...
    if (script_contexts->Lookup(str_name, &lookup_result)) {
        ...
        Handle<Object> previous_value(script_context->get(lookup_result.slot_index), isolate());
        ...
        if (use_ic) {
            if (nexus()->ConfigureLexicalVarMode(lookup_result.context_index, lookup_result.slot_index, lookup_result.mode == VariableMode::kConst)) {
...
namespace {
Maybe<bool> DefineOwnDataProperty(LookupIterator* it, LookupIterator::State original_state, Handle<Object> value, Maybe<ShouldThrow> should_throw, StoreOrigin store_origin)
{
    ...
    return JSObject::DefineOwnPropertyIgnoreAttributes(it, value, NONE, should_throw, JSObject::DONT_FORCE_FIELD, EnforceDefineSemantics::kDefine, store_origin);
}
} // namespace

MaybeHandle<Object> StoreIC::Store(Handle<Object> object, Handle<Name> name, Handle<Object> value, StoreOrigin store_origin)
{
    ...
    if (MigrateDeprecated(isolate(), object)) {
        ...
        PropertyKey key(isolate(), name);
        LookupIterator it(isolate(), object, key, IsDefineNamedOwnIC() ? LookupIterator::OWN : LookupIterator::DEFAULT);
        ...
        if (IsDefineNamedOwnIC()) {
            MAYBE_RETURN_NULL(JSReceiver::CreateDataProperty(&it, value, Nothing<ShouldThrow>()));
        } else {
    ...
    if (IsAnyDefineOwn() && !name->IsPrivateName() && !object->IsJSProxy() && !Handle<JSObject>::cast(object)->HasNamedInterceptor()) {
        Maybe<bool> can_define = JSReceiver::CheckIfCanDefine(isolate(), &it, value, Nothing<ShouldThrow>());
    ...
    if (IsAnyDefineOwn()) {
        if (name->IsPrivateName()) {
            ...
            MAYBE_RETURN_NULL(DefineOwnDataProperty(&it, original_state, value, Nothing<ShouldThrow>(), store_origin));
        }
    } else {
...
void StoreIC::UpdateCaches(LookupIterator* lookup, Handle<Object> value, StoreOrigin store_origin)
{
    MaybeObjectHandle handler;
...
MaybeObjectHandle StoreIC::ComputeHandler(LookupIterator* lookup)
{
    ...
    case LookupIterator::TRANSITION: {
        ...
        if (store_target->IsJSGlobalObject()) {
            ...
            Handle<Object> handler
                = StoreHandler::StoreThroughPrototype(isolate(), lookup_start_object_map(), store_target, smi_handler, MaybeObjectHandle::Weak(lookup->transition_cell()));
    ...
    case LookupIterator::INTERCEPTOR: {
        ...
        DCHECK(!info.getter().IsUndefined(isolate()) || !info.query().IsUndefined(isolate()));
    ...
    case LookupIterator::ACCESSOR: {
        ...
        if (accessors->IsAccessorInfo()) {
            ...
            Handle<Object> setter(Handle<AccessorPair>::cast(accessors)->setter(), isolate());
            ...
            if ((setter->IsFunctionTemplateInfo() && FunctionTemplateInfo::cast(*setter).BreakAtEntry())
                || (setter->IsJSFunction() && JSFunction::cast(*setter).shared().BreakAtEntry())) {
                // Do not install an IC if the api function has a breakpoint.
                TRACE_HANDLER_STATS(isolate(), StoreIC_SlowStub);
                return MaybeObjectHandle(StoreHandler::StoreSlow(isolate()));
            }
            ...
            if (call_optimization.is_simple_api_call()) {
                ...
                if (call_optimization.IsCompatibleReceiverMap(api_holder, holder, holder_lookup)) {
                    Handle<Smi> smi_handler = StoreHandler::StoreApiSetter(isolate(), holder_lookup == CallOptimization::kHolderIsReceiver);
                    ...
                    return MaybeObjectHandle(StoreHandler::StoreThroughPrototype(
                        isolate(), lookup_start_object_map(), holder, smi_handler, MaybeObjectHandle::Weak(call_optimization.api_call_info()), MaybeObjectHandle::Weak(context)));
                }
...
void KeyedStoreIC::UpdateStoreElement(Handle<Map> receiver_map, KeyedAccessStoreMode store_mode, Handle<Map> new_receiver_map)
{
    ...
    if (state() == MONOMORPHIC) {
        ...
        if (receiver_map.is_identical_to(previous_receiver_map) && new_receiver_map.is_identical_to(receiver_map) && old_store_mode == STANDARD_STORE
            && store_mode != STANDARD_STORE) {
            if (receiver_map->IsJSArrayMap() && JSArray::MayHaveReadOnlyLength(*receiver_map)) {
...
Handle<Object> KeyedStoreIC::StoreElementHandler(Handle<Map> receiver_map, KeyedAccessStoreMode store_mode, MaybeHandle<Object> prev_validity_cell)
{
    ...
    DCHECK_IMPLIES(!receiver_map->has_dictionary_elements() && receiver_map->MayHaveReadOnlyElementsInPrototypeChain(isolate()), IsStoreInArrayLiteralIC());
...
void KeyedStoreIC::StoreElementPolymorphicHandlers(std::vector<MapAndHandler>* receiver_maps_and_handlers, KeyedAccessStoreMode store_mode)
{
    ...
    for (size_t i = 0; i < receiver_maps_and_handlers->size(); i++) {
        ...
        if (receiver_map->instance_type() < FIRST_JS_RECEIVER_TYPE || receiver_map->MayHaveReadOnlyElementsInPrototypeChain(isolate())) {
            ...
            if (!old_handler.is_null() && old_handler->GetHeapObject(&old_handler_obj) && old_handler_obj.IsDataHandler()) {
                validity_cell = MaybeHandle<Object>(DataHandler::cast(old_handler_obj).validity_cell(), isolate());
            }
...
{
    for (PrototypeIterator iter(object->GetIsolate(), *object); !iter.IsAtEnd(); iter.Advance()) {
        // Be conservative, don't walk into proxies.
        if (iter.GetCurrent().IsJSProxy())
            return true;
        if (iter.GetCurrent().IsJSTypedArray())
            return true;
    }
    return false;
}
...
KeyedAccessStoreMode GetStoreMode(Handle<JSObject> receiver, size_t index)
{
    ...
    // dictionary mode.
    bool allow_growth = receiver->IsJSArray() && oob_access && index <= JSArray::kMaxArrayIndex && !receiver->WouldConvertToSlowElements(static_cast<uint32_t>(index));
    if (allow_growth) {
...
MaybeHandle<Object> KeyedStoreIC::Store(Handle<Object> object, Handle<Object> key, Handle<Object> value)
{
    // TODO(verwaest): Let SetProperty do the migration, since storing a property
    // might deprecate the current map again, if value does not fit.
    if (MigrateDeprecated(isolate(), object)) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), result,
            IsDefineKeyedOwnIC() ? Runtime::DefineObjectOwnProperty(isolate(), object, key, value, StoreOrigin::kMaybeKeyed)
    ...
    if (key_type == kName) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate(), store_handle, StoreIC::Store(object, maybe_name, value, StoreOrigin::kMaybeKeyed), Object);
        if (vector_needs_update()) {
    ...
    bool use_ic = (state() != NO_FEEDBACK) && v8_flags.use_ic && !object->IsStringWrapper() && !object->IsAccessCheckNeeded() && !object->IsJSGlobalProxy();
    ...
    // TODO(v8:12548): refactor DefineKeyedOwnIC as a subclass of StoreIC
    // so the logic doesn't get mixed here.
    MaybeHandle<Object> result = IsDefineKeyedOwnIC() ? Runtime::DefineObjectOwnProperty(isolate(), object, key, value, StoreOrigin::kMaybeKeyed)
                                                      : Runtime::SetObjectProperty(isolate(), object, key, value, StoreOrigin::kMaybeKeyed);
    if (result.is_null()) {
    ...
    if (use_ic) {
        if (!old_receiver_map.is_null()) {
            if (is_arguments) {
            ...
            } else if (object->IsJSArray() && IsGrowStoreMode(store_mode) && JSArray::HasReadOnlyLength(Handle<JSArray>::cast(object))) {
                set_slow_stub_reason("array has read only length");
            } else if (object->IsJSObject() && MayHaveTypedArrayInPrototypeChain(Handle<JSObject>::cast(object))) {
                ...
                } else if (old_receiver_map->has_dictionary_elements() || !old_receiver_map->MayHaveReadOnlyElementsInPrototypeChain(isolate())) {
...
RUNTIME_FUNCTION(Runtime_DefineNamedOwnIC_Slow)
{
    ...
    MAYBE_RETURN(JSReceiver::CreateDataProperty(&it, value, Nothing<ShouldThrow>()), ReadOnlyRoots(isolate).exception());
    return *value;
}
...
RUNTIME_FUNCTION(Runtime_KeyedStoreIC_Miss)
{
    ...
    if (IsKeyedStoreICKind(kind) || IsDefineKeyedOwnICKind(kind)) {
        ...
        ic.UpdateState(receiver, key);
        RETURN_RESULT_OR_FAILURE(isolate, ic.Store(Handle<JSArray>::cast(receiver), key, value));
    }
}
...
RUNTIME_FUNCTION(Runtime_ElementsTransitionAndStoreIC_Miss)
{
    ...
    if (object->IsJSObject()) {
        JSObject::TransitionElementsKind(Handle<JSObject>::cast(object), map->elements_kind());
    }
    ...
    if (IsStoreInArrayLiteralICKind(kind)) {
        ...
        RETURN_RESULT_OR_FAILURE(isolate,
            IsDefineKeyedOwnICKind(kind) ? Runtime::DefineObjectOwnProperty(isolate, object, key, value, StoreOrigin::kMaybeKeyed)
                                         : Runtime::SetObjectProperty(isolate, object, key, value, StoreOrigin::kMaybeKeyed));
    }
}
...
static Handle<Map> FastCloneObjectMap(Isolate* isolate, Handle<Map> source_map, int flags)
{
    ...
    if (source_map->IsJSObjectMap() && source_map->GetInObjectProperties() != initial_map->GetInObjectProperties()) {
        ...
        map = Map::CopyInitialMap(isolate, map, instance_size, inobject_properties, unused);
    }
    ...
    Handle<DescriptorArray> source_descriptors(source_map->instance_descriptors(isolate), isolate);
    int size = source_map->NumberOfOwnDescriptors();
    int slack = 0;
    Handle<DescriptorArray> descriptors = DescriptorArray::CopyForFastObjectClone(isolate, source_descriptors, size, slack);
    map->InitializeDescriptors(isolate, *descriptors);
    map->CopyUnusedPropertyFieldsAdjustedForInstanceSize(*source_map);

    // Update bitfields
    map->set_may_have_interesting_symbols(source_map->may_have_interesting_symbols());
...
static MaybeHandle<JSObject> CloneObjectSlowPath(Isolate* isolate, Handle<Object> source, int flags)
{
    ...
    MAYBE_RETURN(JSReceiver::SetOrCopyDataProperties(isolate, new_object, source, PropertiesEnumerationMode::kPropertyAdditionOrder, nullptr, false), MaybeHandle<JSObject>());
    return new_object;
}
        ...
        if (maybe_vector->IsFeedbackVector()) {
            ...
            if (!source->IsSmi() && !nexus.IsMegamorphic()) {
                ...
                if (CanFastCloneObject(source_map)) {
                    Handle<Map> target_map = FastCloneObjectMap(isolate, source_map, flags);
...
RUNTIME_FUNCTION(Runtime_LoadPropertyWithInterceptor)
{
    ...
    if (!receiver->IsJSReceiver()) {
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ConvertReceiver(isolate, receiver));
    }
...
RUNTIME_FUNCTION(Runtime_StorePropertyWithInterceptor)
{
    ...
    if (receiver->IsJSGlobalProxy() && (!receiver->HasNamedInterceptor() || receiver->GetNamedInterceptor().non_masking())) {
        interceptor_holder = handle(JSObject::cast(receiver->map().prototype()), isolate);
    }
    DCHECK(interceptor_holder->HasNamedInterceptor());
    {
        Handle<InterceptorInfo> interceptor(interceptor_holder->GetNamedInterceptor(), isolate);
...
RUNTIME_FUNCTION(Runtime_HasElementWithInterceptor)
{
    ...
    return maybe.FromJust() ? ReadOnlyRoots(isolate).true_value() : ReadOnlyRoots(isolate).false_value();
}
