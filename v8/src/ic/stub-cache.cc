...
// static
void StubCache::ClearCallback(v8::Isolate* isolate, v8::GCType type, v8::GCCallbackFlags flags, void* data)
{
    ...
    : isolate_(isolate)
{
    ...
    isolate_->heap()->AddGCEpilogueCallback(ClearCallback, kGCTypeMarkSweepCompact, this);
}

StubCache::~StubCache() { isolate_->heap()->RemoveGCEpilogueCallback(ClearCallback, this); }
...
int StubCache::PrimaryOffsetForTesting(Name name, Map map) { return PrimaryOffset(name, map); }

int StubCache::SecondaryOffsetForTesting(Name name, Map map) { return SecondaryOffset(name, map); }

#ifdef DEBUG
namespace {

bool CommonStubCacheChecks(StubCache* stub_cache, Name name, Map map, MaybeObject handler)
{
...
void StubCache::Set(Name name, Map map, MaybeObject handler)
{
    ...
    MaybeObject old_handler(TaggedValue::ToMaybeObject(isolate(), primary->value));
    // If the primary entry has useful data in it, we retire it to the
    // secondary cache before overwriting it.
    if (old_handler != MaybeObject::FromObject(isolate()->builtins()->code(Builtin::kIllegal)) && !primary->map.IsSmi()) {
        Map old_map = Map::cast(StrongTaggedValue::ToObject(isolate(), primary->map));
        Name old_name = Name::cast(StrongTaggedValue::ToObject(isolate(), primary->key));
...
{
    MaybeObject empty = MaybeObject::FromObject(isolate_->builtins()->code(Builtin::kIllegal));
