...
template <typename BitField> Handle<Smi> SetBitFieldValue(Isolate* isolate, Handle<Smi> smi_handler, typename BitField::FieldType value)
{
...
int InitPrototypeChecksImpl(
    Isolate* isolate, Handle<ICHandler> handler, Handle<Smi>* smi_handler, Handle<Map> lookup_start_object_map, MaybeObjectHandle data1, MaybeObjectHandle maybe_data2)
{
    int data_size = 1;
    // Holder-is-receiver case itself does not add entries unless there is an
    // optional data2 value provided.

    DCHECK_IMPLIES(lookup_start_object_map->IsJSGlobalObjectMap(), lookup_start_object_map->is_prototype_map());

    if (lookup_start_object_map->IsPrimitiveMap() || lookup_start_object_map->is_access_check_needed()) {
        ...
        data_size++;
    } else if (lookup_start_object_map->is_dictionary_map() && !lookup_start_object_map->IsJSGlobalObjectMap()) {
        if (!fill_handler) {
...
int GetHandlerDataSize(
    Isolate* isolate, Handle<Smi>* smi_handler, Handle<Map> lookup_start_object_map, MaybeObjectHandle data1, MaybeObjectHandle maybe_data2 = MaybeObjectHandle())
{
...
void InitPrototypeChecks(
    Isolate* isolate, Handle<ICHandler> handler, Handle<Map> lookup_start_object_map, MaybeObjectHandle data1, MaybeObjectHandle maybe_data2 = MaybeObjectHandle())
{
...
Handle<Object> LoadHandler::LoadFromPrototype(
    Isolate* isolate, Handle<Map> lookup_start_object_map, Handle<JSReceiver> holder, Handle<Smi> smi_handler, MaybeObjectHandle maybe_data1, MaybeObjectHandle maybe_data2)
{
    ...
    Handle<Object> validity_cell = Map::GetOrCreatePrototypeChainValidityCell(lookup_start_object_map, isolate);
...
// static
Handle<Object> LoadHandler::LoadFullChain(Isolate* isolate, Handle<Map> lookup_start_object_map, const MaybeObjectHandle& holder, Handle<Smi> smi_handler)
{
...
KeyedAccessLoadMode LoadHandler::GetKeyedAccessLoadMode(MaybeObject handler)
{
    ...
    if (handler->IsSmi()) {
        ...
        if ((kind == Kind::kElement || kind == Kind::kIndexedString) && AllowOutOfBoundsBits::decode(raw_handler)) {
...
// static
KeyedAccessStoreMode StoreHandler::GetKeyedAccessStoreMode(MaybeObject handler)
{
    DisallowGarbageCollection no_gc;
    if (handler->IsSmi()) {
        int const raw_handler = handler.ToSmi().value();
...
Handle<Object> StoreHandler::StoreElementTransition(
    Isolate* isolate, Handle<Map> receiver_map, Handle<Map> transition, KeyedAccessStoreMode store_mode, MaybeHandle<Object> prev_validity_cell)
{
...
MaybeObjectHandle StoreHandler::StoreOwnTransition(Isolate* isolate, Handle<Map> transition_map)
{
    ...
    if (!is_dictionary_map) {
        ...
        Handle<DescriptorArray> descriptors(transition_map->instance_descriptors(isolate), isolate);
...
Handle<Object> StoreHandler::StoreThroughPrototype(
    Isolate* isolate, Handle<Map> receiver_map, Handle<JSReceiver> holder, Handle<Smi> smi_handler, MaybeObjectHandle maybe_data1, MaybeObjectHandle maybe_data2)
{
    MaybeObjectHandle data1;
    if (maybe_data1.is_null()) {
        data1 = MaybeObjectHandle::Weak(holder);
...
// static
MaybeObjectHandle StoreHandler::StoreGlobal(Handle<PropertyCell> cell) { return MaybeObjectHandle::Weak(cell); }
...
void PrintSmiLoadHandler(int raw_handler, std::ostream& os)
{
    ...
    case LoadHandler::Kind::kElement:
        ...
        if (LoadHandler::IsWasmArrayBits::decode(raw_handler)) {
            ...
            os << "allow out of bounds = " << LoadHandler::AllowOutOfBoundsBits::decode(raw_handler) << ", is JSArray = " << LoadHandler::IsJsArrayBits::decode(raw_handler)
               << ", convert hole = " << LoadHandler::ConvertHoleBits::decode(raw_handler)
    ...
    case LoadHandler::Kind::kField: {
        if (LoadHandler::IsWasmStructBits::decode(raw_handler)) {
            ...
            os << "kField, is in object = " << LoadHandler::IsInobjectBits::decode(raw_handler) << ", is double = " << LoadHandler::IsDoubleBits::decode(raw_handler)
               << ", field index = " << LoadHandler::FieldIndexBits::decode(raw_handler);
        }
        break;
    }
...
void PrintSmiStoreHandler(int raw_handler, std::ostream& os)
{
    ...
    case StoreHandler::Kind::kConstField: {
        ...
        os << "Field, descriptor = " << StoreHandler::DescriptorBits::decode(raw_handler) << ", is in object = " << StoreHandler::IsInobjectBits::decode(raw_handler)
           << ", representation = " << representation.Mnemonic() << ", field index = " << StoreHandler::FieldIndexBits::decode(raw_handler);
        break;
    }
    case StoreHandler::Kind::kAccessor:
        os << "kAccessor, descriptor = " << StoreHandler::DescriptorBits::decode(raw_handler);
        break;
    ...
    case StoreHandler::Kind::kSlow: {
        ...
        os << "kSlow, keyed access store mode = " << KeyedAccessStoreModeToString(keyed_access_store_mode);
        break;
    }
...
void LoadHandler::PrintHandler(Object handler, std::ostream& os)
{
    ...
    if (handler.IsSmi()) {
        ...
        os << "LoadHandler(Code)(" << Builtins::name(CodeT::cast(handler).builtin_id()) << ")";
...
std::ostream& operator<<(std::ostream& os, WasmValueType type) { return os << WasmValueType2String(type); }
