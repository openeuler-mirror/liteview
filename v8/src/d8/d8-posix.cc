...
static bool GetTimeouts(const v8::FunctionCallbackInfo<v8::Value>& args, int* read_timeout, int* total_timeout)
{
    if (args.Length() > 3) {
        if (args[3]->IsNumber()) {
            *total_timeout = args[3]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
        } else {
    ...
    if (args.Length() > 2) {
        if (args[2]->IsNumber()) {
            *read_timeout = args[2]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
        } else {
...
namespace {
v8::Local<v8::String> v8_strerror(v8::Isolate* isolate, int err) { return v8::String::NewFromUtf8(isolate, strerror(err)).ToLocalChecked(); }
} // namespace
...
void Shell::ChangeDirectory(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (*directory == nullptr) {
        args.GetIsolate()->ThrowError("os.chdir(): String conversion of argument failed.");
        return;
    }
...
void Shell::SetUMask(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (args[0]->IsNumber()) {
        int previous = umask(args[0]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust());
...
void Shell::MakeDirectory(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (args.Length() == 2) {
        if (args[1]->IsNumber()) {
            mask = args[1]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromJust();
        } else {
    ...
    String::Utf8Value directory(args.GetIsolate(), args[0]);
    if (*directory == nullptr) {
        args.GetIsolate()->ThrowError("os.mkdirp(): String conversion of argument failed.");
        return;
    }
...
void Shell::RemoveDirectory(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    String::Utf8Value directory(args.GetIsolate(), args[0]);
    if (*directory == nullptr) {
        args.GetIsolate()->ThrowError("os.rmdir(): String conversion of argument failed.");
        return;
    }
    rmdir(*directory);
}
...
void Shell::SetEnvironment(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (*var == nullptr) {
        args.GetIsolate()->ThrowError("os.setenv(): String conversion of variable name failed.");
        return;
    }
    if (*value == nullptr) {
        args.GetIsolate()->ThrowError("os.setenv(): String conversion of variable contents failed.");
        return;
    }
    setenv(*var, *value, 1);
}
