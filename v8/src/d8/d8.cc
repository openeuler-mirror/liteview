...
public:
    void* Allocate(size_t length) override { return allocator_->Allocate(length); }

    void* AllocateUninitialized(size_t length) override { return allocator_->AllocateUninitialized(length); }

    void Free(void* data, size_t length) override { allocator_->Free(data, length); }
...
protected:
    void* Allocate(size_t length) override { return ArrayBufferAllocatorBase::Allocate(Adjust(length)); }

    void* AllocateUninitialized(size_t length) override { return ArrayBufferAllocatorBase::AllocateUninitialized(Adjust(length)); }

    void Free(void* data, size_t length) override { return ArrayBufferAllocatorBase::Free(data, Adjust(length)); }
...
private:
    static int UpdateIncludedCategoriesList(v8::Isolate* isolate, Local<Context> context, Local<v8::Object> object, platform::tracing::TraceConfig* trace_config)
    {
...
public:
    ...
    const char* data() const override { return static_cast<char*>(file_->memory()); }
...
void Shell::StoreInCodeCache(Isolate* isolate, Local<Value> source, const ScriptCompiler::CachedData* cache_data)
{
    ...
    cached_code_map_[*key] = std::unique_ptr<ScriptCompiler::CachedData>(new ScriptCompiler::CachedData(cache, length, ScriptCompiler::CachedData::BufferOwned));
}
...
public:
    explicit DummySourceStream(Local<String> source)
        : done_(false)
    {
        source_buffer_ = Utils::OpenHandle(*source)->ToCString(i::ALLOW_NULLS, i::FAST_STRING_TRAVERSAL, &source_length_);
    }
...
public:
    StreamingCompileTask(Isolate* isolate, v8::ScriptCompiler::StreamedSource* streamed_source, v8::ScriptType type)
        : isolate_(isolate)
        , script_streaming_task_(v8::ScriptCompiler::StartStreaming(isolate, streamed_source, type))
    {
...
private:
    ...
    public:
        explicit FinishTask(Isolate* isolate)
            : isolate_(isolate)
        {
        }
    ...
    Isolate* isolate_;
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task_;
};
...
template <> MaybeLocal<Script> CompileStreamed(Local<Context> context, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
    return ScriptCompiler::Compile(context, v8_source, full_source_string, origin);
}

template <> MaybeLocal<Module> CompileStreamed(Local<Context> context, ScriptCompiler::StreamedSource* v8_source, Local<String> full_source_string, const ScriptOrigin& origin)
{
...
template <class T> MaybeLocal<T> Shell::CompileString(Isolate* isolate, Local<Context> context, Local<String> source, const ScriptOrigin& origin)
{
    if (options.streaming_compile) {
        v8::ScriptCompiler::StreamedSource streamed_source(std::make_unique<DummySourceStream>(source), v8::ScriptCompiler::StreamedSource::UTF8);
        PostBlockingBackgroundTask(
            std::make_unique<StreamingCompileTask>(isolate, &streamed_source, std::is_same<T, Module>::value ? v8::ScriptType::kModule : v8::ScriptType::kClassic));
...
private:
    ...
    public:
        ...
        size_t operator()(const Global<Module>& module) const { return module.Get(isolate_)->GetIdentityHash(); }
...
public:
    ...
    static ModuleType ModuleTypeFromImportAssertions(Local<Context> context, Local<FixedArray> import_assertions, bool hasPositions)
    {
        Isolate* isolate = context->GetIsolate();
        const int kV8AssertionEntrySize = hasPositions ? 3 : 2;
        for (int i = 0; i < import_assertions->Length(); i += kV8AssertionEntrySize) {
            Local<String> v8_assertion_key = import_assertions->Get(context, i).As<v8::String>();
            std::string assertion_key = ToSTLString(isolate, v8_assertion_key);

            if (assertion_key == "type") {
                Local<String> v8_assertion_value = import_assertions->Get(context, i + 1).As<String>();
...
std::shared_ptr<ModuleEmbedderData> InitializeModuleEmbedderData(Local<Context> context)
{
    ...
    i::Handle<i::Managed<ModuleEmbedderData>> module_data_managed = i::Managed<ModuleEmbedderData>::Allocate(i_isolate, kModuleEmbedderDataEstimate, context->GetIsolate());
...
std::shared_ptr<ModuleEmbedderData> GetModuleDataFromContext(Local<Context> context)
{
    ...
    i::Handle<i::Managed<ModuleEmbedderData>> module_data_managed = i::Handle<i::Managed<ModuleEmbedderData>>::cast(Utils::OpenHandle<Value, i::Object>(module_data));
    return module_data_managed->get();
}
...
ScriptOrigin CreateScriptOrigin(Isolate* isolate, Local<String> resource_name, v8::ScriptType type)
{
    ...
    return ScriptOrigin(isolate, resource_name, 0, 0, false, -1, Local<Value>(), false, false, type == v8::ScriptType::kModule, options);
}
...
bool Shell::ExecuteString(
    Isolate* isolate, Local<String> source, Local<String> name, PrintResult print_result, ReportExceptions report_exceptions, ProcessMessageQueue process_message_queue)
{
    ...
    bool success = true;
    {
        ...
        if (options.compile_only)
            return true;
        if (options.compile_options == ScriptCompiler::kConsumeCodeCache) {
            i::Handle<i::Script> i_script(i::Script::cast(Utils::OpenHandle(*script)->shared().script()), i_isolate);
            // TODO(cbruni, chromium:1244145): remove once context-allocated.
            i_script->set_host_defined_options(i::FixedArray::cast(*Utils::OpenHandle(*(origin.GetHostDefinedOptions()))));
        }
        ...
        if (process_message_queue) {
            if (!EmptyMessageQueues(isolate))
                success = false;
            if (!HandleUnhandledPromiseRejections(isolate))
                success = false;
        }
...
bool Shell::TakeWebSnapshot(Isolate* isolate)
{
    ...
    if (!options.web_snapshot_config) {
        isolate->ThrowError("Web snapshots: --web-snapshot-config is needed when "
    ...
    MaybeLocal<PrimitiveArray> maybe_exports = ReadLines(isolate, options.web_snapshot_config);
    ...
    if (serializer.TakeSnapshot(context, exports, snapshot_data)) {
        ...
        WriteChars(web_snapshot_output_file_name, snapshot_data.buffer, snapshot_data.buffer_size);
    } else {
        CHECK(try_catch.HasCaught());
        return false;
    }
...
MaybeLocal<Module> ResolveModuleCallback(Local<Context> context, Local<String> specifier, Local<FixedArray> import_assertions, Local<Module> referrer)
{
    Isolate* isolate = context->GetIsolate();
    std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(context);
    auto specifier_it = module_data->module_to_specifier_map.find(Global<Module>(isolate, referrer));
    CHECK(specifier_it != module_data->module_to_specifier_map.end());
    std::string absolute_path = NormalizePath(ToSTLString(isolate, specifier), DirName(specifier_it->second));
    ModuleType module_type = ModuleEmbedderData::ModuleTypeFromImportAssertions(context, import_assertions, true);
...
MaybeLocal<Module> Shell::FetchModuleTree(Local<Module> referrer, Local<Context> context, const std::string& file_name, ModuleType module_type)
{
    ...
    if (source_text.IsEmpty()) {
        ...
        isolate->ThrowError(v8::String::NewFromUtf8(isolate, msg.c_str()).ToLocalChecked());
        return MaybeLocal<Module>();
    }

    Local<String> resource_name = String::NewFromUtf8(isolate, file_name.c_str()).ToLocalChecked();
    ...
    if (module_type == ModuleType::kJavaScript) {
        ...
        std::vector<Local<String>> export_names { String::NewFromUtf8(isolate, "default").ToLocalChecked() };

        module = v8::Module::CreateSyntheticModule(isolate, String::NewFromUtf8(isolate, file_name.c_str()).ToLocalChecked(), export_names, Shell::JSONModuleEvaluationSteps);
    ...
    CHECK(module_data->module_map.insert(std::make_pair(std::make_pair(file_name, module_type), Global<Module>(isolate, module))).second);
    CHECK(module_data->module_to_specifier_map.insert(std::make_pair(Global<Module>(isolate, module), file_name)).second);
    ...
    for (int i = 0, length = module_requests->Length(); i < length; ++i) {
        ...
        ModuleType request_module_type = ModuleEmbedderData::ModuleTypeFromImportAssertions(context, import_assertions, true);
        ...
        if (module_data->module_map.count(std::make_pair(absolute_path, request_module_type))) {
            continue;
        }

        if (FetchModuleTree(module, context, absolute_path, request_module_type).IsEmpty()) {
            return MaybeLocal<Module>();
        }
    }
...
MaybeLocal<Value> Shell::JSONModuleEvaluationSteps(Local<Context> context, Local<Module> module)
{
    ...
    std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(context);
    auto json_value_it = module_data->json_module_to_parsed_json_map.find(Global<Module>(isolate, module));
...
struct DynamicImportData {
    DynamicImportData(
        Isolate* isolate_, Local<Context> context_, Local<Value> referrer_, Local<String> specifier_, Local<FixedArray> import_assertions_, Local<Promise::Resolver> resolver_)
        : isolate(isolate_)
    {
...
struct ModuleResolutionData {
    ModuleResolutionData(Isolate* isolate_, Local<Value> module_namespace_, Local<Promise::Resolver> resolver_)
        : isolate(isolate_)
    {
...
{
    std::unique_ptr<ModuleResolutionData> module_resolution_data(static_cast<ModuleResolutionData*>(info.Data().As<v8::External>()->Value()));
    Isolate* isolate(module_resolution_data->isolate);
    HandleScope handle_scope(isolate);

    Local<Promise::Resolver> resolver(module_resolution_data->resolver.Get(isolate));
    Local<Value> module_namespace(module_resolution_data->module_namespace.Get(isolate));
...
MaybeLocal<Promise> Shell::HostImportModuleDynamically(
    Local<Context> context, Local<Data> host_defined_options, Local<Value> resource_name, Local<String> specifier, Local<FixedArray> import_assertions)
{
    Isolate* isolate = context->GetIsolate();
    ...
    if (!resource_name->IsNull() && !IsValidHostDefinedOptions(context, host_defined_options, resource_name)) {
        ...
        DynamicImportData* data = new DynamicImportData(isolate, context, resource_name, specifier, import_assertions, resolver);
...
void Shell::HostInitializeImportMetaObject(Local<Context> context, Local<Module> module, Local<Object> meta)
{
    ...
    std::shared_ptr<ModuleEmbedderData> module_data = GetModuleDataFromContext(context);
    auto specifier_it = module_data->module_to_specifier_map.find(Global<Module>(isolate, module));
    CHECK(specifier_it != module_data->module_to_specifier_map.end());

    Local<String> url_key = String::NewFromUtf8Literal(isolate, "url", NewStringType::kInternalized);
    Local<String> url = String::NewFromUtf8(isolate, specifier_it->second.c_str()).ToLocalChecked();
...
void Shell::DoHostImportModuleDynamically(void* import_data)
{
    ...
    Local<FixedArray> import_assertions(import_data_->import_assertions.Get(isolate));
    ...
    ModuleType module_type = ModuleEmbedderData::ModuleTypeFromImportAssertions(realm, import_assertions, false);
...
bool Shell::ExecuteModule(Isolate* isolate, const char* file_name)
{
    ...
    Local<Promise> result_promise(result.As<Promise>());
    while (result_promise->State() == Promise::kPending && reinterpret_cast<i::Isolate*>(isolate)->default_microtask_queue()->size() > 0) {
        Shell::CompleteMessageLoop(isolate);
    }
    ...
    std::vector<std::tuple<Local<Module>, Local<Message>>> stalled = root_module->GetStalledTopLevelAwaitMessage(isolate);
    if (stalled.size() > 0) {
...
bool Shell::ExecuteWebSnapshot(Isolate* isolate, const char* file_name)
{
    ...
    int length = 0;
    std::unique_ptr<uint8_t[]> snapshot_data(reinterpret_cast<uint8_t*>(ReadChars(absolute_path.c_str(), &length)));
    if (length == 0) {
        TryCatch try_catch(isolate);
        ...
        for (int r = 0; r < DeserializationRunCount(); ++r) {
            ...
            i::WebSnapshotDeserializer deserializer(isolate, snapshot_data.get(), static_cast<size_t>(length));
...
void PerIsolateData::SetTimeout(Local<Function> callback, Local<Context> context)
{
...
void PerIsolateData::AddUnhandledPromise(Local<Promise> promise, Local<Message> message, Local<Value> exception)
{
    if (ignore_unhandled_promises_)
        return;
    DCHECK_EQ(promise->GetIsolate(), isolate_);
    unhandled_promises_.emplace_back(v8::Global<v8::Promise>(isolate_, promise), v8::Global<v8::Message>(isolate_, message), v8::Global<v8::Value>(isolate_, exception));
}
...
Local<FunctionTemplate> PerIsolateData::GetTestApiObjectCtor() const { return test_api_object_ctor_.Get(isolate_); }

void PerIsolateData::SetTestApiObjectCtor(Local<FunctionTemplate> ctor) { test_api_object_ctor_.Reset(isolate_, ctor); }

Local<FunctionTemplate> PerIsolateData::GetSnapshotObjectCtor() const { return snapshot_object_ctor_.Get(isolate_); }

void PerIsolateData::SetSnapshotObjectCtor(Local<FunctionTemplate> ctor) { snapshot_object_ctor_.Reset(isolate_, ctor); }

Local<FunctionTemplate> PerIsolateData::GetDomNodeCtor() const { return dom_node_ctor_.Get(isolate_); }

void PerIsolateData::SetDomNodeCtor(Local<FunctionTemplate> ctor) { dom_node_ctor_.Reset(isolate_, ctor); }
    ...
    : data_(data)
{
    ...
    data_->realms_[0].Reset(data_->isolate_, data_->isolate_->GetEnteredOrMicrotaskContext());
}
...
Local<Context> PerIsolateData::ExplicitRealmScope::context() const { return realm_; }
...
int PerIsolateData::RealmIndexOrThrow(const v8::FunctionCallbackInfo<v8::Value>& args, int arg_offset)
{
    if (args.Length() < arg_offset || !args[arg_offset]->IsNumber()) {
        args.GetIsolate()->ThrowError("Invalid argument");
        return -1;
    }
    int index = args[arg_offset]->Int32Value(args.GetIsolate()->GetCurrentContext()).FromMaybe(-1);
...
int64_t Shell::GetTracingTimestampFromPerformanceTimestamp(double performance_timestamp)
{
...
void Shell::PerformanceNow(const v8::FunctionCallbackInfo<v8::Value>& args) { args.GetReturnValue().Set(GetTimestamp()); }
...
void Shell::PerformanceMark(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "entryType"), String::NewFromUtf8Literal(isolate, "mark"), ReadOnly).Check();
...
void Shell::PerformanceMeasure(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    Local<Context> context = isolate->GetCurrentContext();
    ...
    if (args.Length() >= 2) {
        ...
        if (!start_mark->IsObject()) {
            args.GetIsolate()->ThrowError("Invalid 'startMark' argument: Not an Object");
            return;
        }
        ...
        if (!start_time_field->IsNumber()) {
            args.GetIsolate()->ThrowError("Invalid 'startMark' argument: No numeric 'startTime' field");
            return;
        }
    ...
    if (options.trace_enabled) {
        ...
        TRACE_EVENT_COPY_NESTABLE_ASYNC_END_WITH_TIMESTAMP0("v8", *utf8, static_cast<uint64_t>(hash), GetTracingTimestampFromPerformanceTimestamp(end_timestamp));
    }

    Local<Object> performance_entry = Object::New(isolate);
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "entryType"), String::NewFromUtf8Literal(isolate, "measure"), ReadOnly).Check();
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "name"), name, ReadOnly).Check();
    ...
    performance_entry->DefineOwnProperty(context, String::NewFromUtf8Literal(isolate, "duration"), Number::New(isolate, end_timestamp - start_timestamp), ReadOnly).Check();
...
void Shell::PerformanceMeasureMemory(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    args.GetIsolate()->MeasureMemory(v8::MeasureMemoryDelegate::Default(isolate, context, promise_resolver, mode), v8::MeasureMemoryExecution::kEager);
...
void Shell::RealmOwner(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    Local<Object> object = args[0]->ToObject(isolate->GetCurrentContext()).ToLocalChecked();
    i::Handle<i::JSReceiver> i_object = Utils::OpenHandle(*object);
    if (i_object->IsJSGlobalProxy() && i::Handle<i::JSGlobalProxy>::cast(i_object)->IsDetached()) {
        return;
    }
...
void Shell::RealmGlobal(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    // global objects.
    Local<Object> global = Local<Context>::New(args.GetIsolate(), data->realms_[index])->Global();
    ...
    if (i_global->IsJSGlobalObject()) {
        ...
        i::Handle<i::JSObject> i_global_proxy = handle(i::Handle<i::JSGlobalObject>::cast(i_global)->global_proxy(), i_isolate);
        global = Utils::ToLocal(i_global_proxy);
    }
    args.GetReturnValue().Set(global);
}

MaybeLocal<Context> Shell::CreateRealm(const v8::FunctionCallbackInfo<v8::Value>& args, int index, v8::MaybeLocal<Value> global_object)
{
...
void Shell::DisposeRealm(const v8::FunctionCallbackInfo<v8::Value>& args, int index)
{
    Isolate* isolate = args.GetIsolate();
    PerIsolateData* data = PerIsolateData::Get(isolate);
...
void Shell::RealmCreate(const v8::FunctionCallbackInfo<v8::Value>& args) { CreateRealm(args, -1, v8::MaybeLocal<Value>()); }

// Realm.createAllowCrossRealmAccess() creates a new realm with the same
// security token as the current realm.
void Shell::RealmCreateAllowCrossRealmAccess(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Local<Context> context;
    if (CreateRealm(args, -1, v8::MaybeLocal<Value>()).ToLocal(&context)) {
        context->SetSecurityToken(args.GetIsolate()->GetEnteredOrMicrotaskContext()->GetSecurityToken());
    }
}
...
void Shell::RealmNavigate(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!global_object.IsEmpty()) {
        HandleScope scope(isolate);
        if (!Utils::OpenHandle(*global_object.ToLocalChecked())->IsJSGlobalProxy()) {
...
void Shell::RealmSharedGet(Local<String> property, const PropertyCallbackInfo<Value>& info)
{
...
void Shell::RealmSharedSet(Local<String> property, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
    Isolate* isolate = info.GetIsolate();
    PerIsolateData* data = PerIsolateData::Get(isolate);
...
void Shell::RealmTakeWebSnapshot(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    ...
    for (int i = 0; i < length; ++i) {
        Local<Value> value;
        Local<String> str;
        if (!exports_array->Get(current_context, i).ToLocal(&value) || !value->ToString(current_context).ToLocal(&str) || str.IsEmpty()) {
            isolate->ThrowError("Invalid argument");
            return;
        }
    ...
    auto snapshot_data_shared = std::make_shared<i::WebSnapshotData>();
    {
        ...
        PerIsolateData::ExplicitRealmScope realm_scope(data, index);
        i::WebSnapshotSerializer serializer(isolate);
        if (!serializer.TakeSnapshot(realm_scope.context(), exports, *snapshot_data_shared)) {
            CHECK(try_catch.HasCaught());
    ...
    // signature checks.
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    i::Handle<i::Object> snapshot_data_managed = i::Managed<i::WebSnapshotData>::FromSharedPtr(i_isolate, snapshot_data_shared->buffer_size, snapshot_data_shared);
    v8::Local<v8::Value> shapshot_data = Utils::ToLocal(snapshot_data_managed);
    Local<ObjectTemplate> snapshot_template = data->GetSnapshotObjectCtor()->InstanceTemplate();
    Local<Object> snapshot_instance = snapshot_template->NewInstance(isolate->GetCurrentContext()).ToLocalChecked();
...
void Shell::RealmUseWebSnapshot(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    ...
    auto snapshot_data_managed = i::Handle<i::Managed<i::WebSnapshotData>>::cast(snapshot_data_handle);
    std::shared_ptr<i::WebSnapshotData> snapshot_data_shared = snapshot_data_managed->get();
    // Deserialize the snapshot in the specified Realm.
    {
        PerIsolateData::ExplicitRealmScope realm_scope(data, index);
        i::WebSnapshotDeserializer deserializer(isolate, snapshot_data_shared->buffer, snapshot_data_shared->buffer_size);
...
void Shell::TestVerifySourcePositions(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    ...
    if (!arg_handle->IsHeapObject() || !i::Handle<i::HeapObject>::cast(arg_handle)->IsJSFunctionOrBoundFunctionOrWrappedFunction()) {
        isolate->ThrowError("Expected function as single argument.");
        return;
    }
    ...
    auto callable = i::Handle<i::JSFunctionOrBoundFunctionOrWrappedFunction>::cast(arg_handle);
    while (callable->IsJSBoundFunction()) {
        ...
        callable = handle(i::JSFunctionOrBoundFunctionOrWrappedFunction::cast(bound_target), i_isolate);
    }
    ...
    if (has_baseline) {
        bytecode_offsets = handle(i::ByteArray::cast(function->shared().GetCode().bytecode_offset_table()), i_isolate);
        ...
        if (has_baseline) {
            ...
            for (i::Address pc = offset_iterator->current_pc_start_offset() + 1; pc <= offset_iterator->current_pc_end_offset(); ++pc) {
                ...
                if (pc_lookup.current_bytecode_offset() != bytecode_iterator.current_offset()) {
                    isolate->ThrowError("Baseline bytecode offset mismatch for PC lookup.");
                    return;
                }
            }
        }
...
void Shell::InstallConditionalFeatures(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
...
void Shell::AsyncHooksCreateHook(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
// context.
void Shell::AsyncHooksExecutionAsyncId(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    HandleScope handle_scope(isolate);
    args.GetReturnValue().Set(v8::Number::New(isolate, PerIsolateData::Get(isolate)->GetAsyncHooks()->GetExecutionAsyncId()));
}

void Shell::AsyncHooksTriggerAsyncId(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    HandleScope handle_scope(isolate);
    args.GetReturnValue().Set(v8::Number::New(isolate, PerIsolateData::Get(isolate)->GetAsyncHooks()->GetTriggerAsyncId()));
}

static v8::debug::DebugDelegate dummy_delegate;

void Shell::EnableDebugger(const v8::FunctionCallbackInfo<v8::Value>& args) { v8::debug::SetDebugDelegate(args.GetIsolate(), &dummy_delegate); }

void Shell::DisableDebugger(const v8::FunctionCallbackInfo<v8::Value>& args) { v8::debug::SetDebugDelegate(args.GetIsolate(), nullptr); }
...
void Shell::SetPromiseHooks(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (i::v8_flags.correctness_fuzzer_suppressions) {
        ...
        // creation.
        isolate->ThrowError("d8.promise.setHooks is disabled with "
    ...
    context->SetPromiseHooks(args[0]->IsFunction() ? args[0].As<Function>() : Local<Function>(), args[1]->IsFunction() ? args[1].As<Function>() : Local<Function>(),
        args[2]->IsFunction() ? args[2].As<Function>() : Local<Function>(), args[3]->IsFunction() ? args[3].As<Function>() : Local<Function>());

    args.GetReturnValue().Set(v8::Undefined(isolate));
#else // V8_ENABLE_JAVASCRIPT_PROMISE_HOOKS
    isolate->ThrowError("d8.promise.setHooks is disabled due to missing build flag "
                        "v8_enabale_javascript_in_promise_hooks");
#endif // V8_ENABLE_JAVASCRIPT_PROMISE_HOOKS
}

void Shell::SerializerSerialize(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    HandleScope handle_scope(isolate);
    Local<Context> context = isolate->GetCurrentContext();
    ...
    for (int i = 0; i < args.Length(); i++) {
        bool ok;
        if (!serializer.WriteValue(context, args[i]).To(&ok))
            return;
    }
...
void Shell::SerializerDeserialize(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    HandleScope handle_scope(isolate);
    Local<Context> context = isolate->GetCurrentContext();
...
void WriteToFile(FILE* file, const v8::FunctionCallbackInfo<v8::Value>& args)
{
    for (int i = 0; i < args.Length(); i++) {
        ...
        if (!arg->ToString(args.GetIsolate()->GetCurrentContext()).ToLocal(&str_obj)) {
            try_catch.ReThrow();
            return;
        }
...
void Shell::Print(const v8::FunctionCallbackInfo<v8::Value>& args) { WriteAndFlush(stdout, args); }

void Shell::PrintErr(const v8::FunctionCallbackInfo<v8::Value>& args) { WriteAndFlush(stderr, args); }

void Shell::WriteStdout(const v8::FunctionCallbackInfo<v8::Value>& args) { WriteToFile(stdout, args); }
...
void Shell::ExecuteFile(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    for (int i = 0; i < args.Length(); i++) {
        ...
        if (*file_name == nullptr) {
            ...
            isolate->ThrowError(String::NewFromUtf8(isolate, oss.str().c_str()).ToLocalChecked());
            return;
        }
        Local<String> source;
        if (!ReadFile(isolate, *file_name).ToLocal(&source))
            return;
        if (!ExecuteString(args.GetIsolate(), source, String::NewFromUtf8(isolate, *file_name).ToLocalChecked(), kNoPrintResult,
...
void Shell::ReadCodeTypeAndArguments(const v8::FunctionCallbackInfo<v8::Value>& args, int index, CodeType* code_type, Local<Value>* arguments)
{
    Isolate* isolate = args.GetIsolate();
...
MaybeLocal<String> Shell::ReadSource(const v8::FunctionCallbackInfo<v8::Value>& args, int index, CodeType default_type)
{
    ...
    case CodeType::kFunction:
        ...
        if (!FunctionAndArgumentsToString(args[index].As<Function>(), arguments, &source, isolate)) {
            return MaybeLocal<String>();
        }
        break;
...
void Shell::WorkerNew(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...

    {
        ...
        i::Handle<i::Object> managed = i::Managed<Worker>::FromSharedPtr(i_isolate, kWorkerSizeEstimate, worker);
...
void Shell::WorkerPostMessage(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    std::shared_ptr<Worker> worker = GetWorkerFromInternalField(isolate, args.Holder());
    if (!worker.get()) {
        return;
    }

    Local<Value> message = args[0];
    Local<Value> transfer = args.Length() >= 2 ? args[1] : Undefined(isolate).As<Value>();
...
void Shell::WorkerTerminateAndWait(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    Isolate* isolate = args.GetIsolate();
    HandleScope handle_scope(isolate);
    std::shared_ptr<Worker> worker = GetWorkerFromInternalField(isolate, args.Holder());
    ...
    i::ParkedScope parked(reinterpret_cast<i::Isolate*>(isolate)->main_thread_local_isolate());
...
void Shell::Quit(const v8::FunctionCallbackInfo<v8::Value>& args) { base::CallOnce(&quit_once_, &QuitOnce, const_cast<v8::FunctionCallbackInfo<v8::Value>*>(&args)); }

void Shell::WaitUntilDone(const v8::FunctionCallbackInfo<v8::Value>& args) { SetWaitUntilDone(args.GetIsolate(), true); }

void Shell::NotifyDone(const v8::FunctionCallbackInfo<v8::Value>& args) { SetWaitUntilDone(args.GetIsolate(), false); }

void Shell::Version(const v8::FunctionCallbackInfo<v8::Value>& args) { args.GetReturnValue().Set(String::NewFromUtf8(args.GetIsolate(), V8::GetVersion()).ToLocalChecked()); }
...
void Shell::ReportException(v8::Isolate* isolate, v8::TryCatch* try_catch) { ReportException(isolate, try_catch->Message(), try_catch->Exception()); }
...
void* Shell::CreateHistogram(const char* name, int min, int max, size_t buckets) { return GetCounter(name, true); }
...
Local<FunctionTemplate> NewDOMFunctionTemplate(Isolate* isolate, uint16_t instance_type)
{
    return FunctionTemplate::New(isolate, nullptr, Local<Value>(), Local<Signature>(), 0, ConstructorBehavior::kAllow, SideEffectType::kHasSideEffect, nullptr, instance_type);
}

Local<FunctionTemplate> Shell::CreateEventTargetTemplate(Isolate* isolate)
{
    Local<FunctionTemplate> event_target = NewDOMFunctionTemplate(isolate, i::Internals::kFirstJSApiObjectType + 1);
    return event_target;
}

Local<FunctionTemplate> Shell::CreateNodeTemplates(Isolate* isolate, Local<FunctionTemplate> event_target)
{
    Local<FunctionTemplate> node = NewDOMFunctionTemplate(isolate, i::Internals::kFirstJSApiObjectType + 2);
    ...
    Local<FunctionTemplate> nodeType = FunctionTemplate::New(isolate, NodeTypeCallback, Local<Value>(), signature, 0, ConstructorBehavior::kThrow, SideEffectType::kHasSideEffect,
        nullptr, i::Internals::kFirstJSApiObjectType, i::Internals::kFirstJSApiObjectType + 3, i::Internals::kFirstJSApiObjectType + 5);
    ...
    Local<FunctionTemplate> element = NewDOMFunctionTemplate(isolate, i::Internals::kFirstJSApiObjectType + 3);
    element->Inherit(node);

    Local<FunctionTemplate> html_element = NewDOMFunctionTemplate(isolate, i::Internals::kFirstJSApiObjectType + 4);
    html_element->Inherit(element);

    Local<FunctionTemplate> div_element = NewDOMFunctionTemplate(isolate, i::Internals::kFirstJSApiObjectType + 5);
    div_element->Inherit(html_element);

    return div_element;
}

static bool D8AccessCheckCallback(Local<Context> accessing_context, Local<Object> accessed_object, Local<Value> data)
{
    return Isolate::GetCurrent()->GetCurrentContext()->GetSecurityToken()->StrictEquals(accessed_object->GetCreationContextChecked()->GetSecurityToken());
}

static void AccessNamedGetter(Local<Name> property, const PropertyCallbackInfo<Value>& info) { }
static void AccessNamedSetter(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<Value>& info) { }
static void AccessNamedQuery(Local<Name> property, const PropertyCallbackInfo<Integer>& info) { }
static void AccessNamedDeleter(Local<Name> property, const PropertyCallbackInfo<Boolean>& info) { }
static void AccessNamedEnumerator(const PropertyCallbackInfo<Array>& info) { }
static void AccessIndexedGetter(uint32_t index, const PropertyCallbackInfo<Value>& info) { }

static void AccessIndexedSetter(uint32_t index, Local<Value> value, const PropertyCallbackInfo<Value>& info) { }

static void AccessIndexedQuery(uint32_t index, const PropertyCallbackInfo<Integer>& info) { }

static void AccessIndexedDeleter(uint32_t index, const PropertyCallbackInfo<Boolean>& info) { }
...
Local<ObjectTemplate> Shell::CreateGlobalTemplate(Isolate* isolate)
{
...
#ifdef V8_FUZZILLI
    global_template->Set(String::NewFromUtf8(isolate, "fuzzilli", NewStringType::kNormal).ToLocalChecked(), FunctionTemplate::New(isolate, Fuzzilli), PropertyAttribute::DontEnum);
#endif // V8_FUZZILLI
    ...
    if (options.throw_on_failed_access_check || options.noop_on_failed_access_check) {
        global_template->SetAccessCheckCallbackAndHandler(D8AccessCheckCallback,
            v8::NamedPropertyHandlerConfiguration(AccessNamedGetter, AccessNamedSetter, AccessNamedQuery, AccessNamedDeleter, AccessNamedEnumerator),
            v8::IndexedPropertyHandlerConfiguration(AccessIndexedGetter, AccessIndexedSetter, AccessIndexedQuery, AccessIndexedDeleter, AccessIndexedEnumerator));
    }
...
Local<FunctionTemplate> Shell::CreateWorkerTemplate(Isolate* isolate)
{
    ...
    worker_fun_template->PrototypeTemplate()->Set(isolate, "terminateAndWait", FunctionTemplate::New(isolate, WorkerTerminateAndWait, Local<Value>(), worker_signature));
...
Local<ObjectTemplate> Shell::CreateTestRunnerTemplate(Isolate* isolate)
{
    ...
    test_template->Set(isolate, "notifyDone", FunctionTemplate::New(isolate, NotifyDone));
    test_template->Set(isolate, "waitUntilDone", FunctionTemplate::New(isolate, WaitUntilDone));
...
Local<ObjectTemplate> Shell::CreateRealmTemplate(Isolate* isolate)
{
    ...
    realm_template->SetAccessor(String::NewFromUtf8Literal(isolate, "shared"), RealmSharedGet, RealmSharedSet);
    if (options.d8_web_snapshot_api) {
        realm_template->Set(isolate, "takeWebSnapshot", FunctionTemplate::New(isolate, RealmTakeWebSnapshot));
        realm_template->Set(isolate, "useWebSnapshot", FunctionTemplate::New(isolate, RealmUseWebSnapshot));
    }
...
static void ThrowOnFailedAccessCheck(Local<Object> host, v8::AccessType type, Local<Value> data) { Isolate::GetCurrent()->ThrowError("Error in failed access check callback"); }
...
void Shell::Initialize(Isolate* isolate, D8Console* console, bool isOnMainThread)
{
    ...
    if (isOnMainThread) {
        ...
        isolate->AddMessageListenerWithErrorLevel(
            PrintMessageCallback, v8::Isolate::kMessageError | v8::Isolate::kMessageWarning | v8::Isolate::kMessageInfo | v8::Isolate::kMessageDebug | v8::Isolate::kMessageLog);
    }
    ...
    if (options.throw_on_failed_access_check) {
        ...
        isolate->SetFailedAccessCheckCallbackFunction([](Local<Object> host, v8::AccessType type, Local<Value> data) {});
    }
...
Local<String> Shell::WasmLoadSourceMapCallback(Isolate* isolate, const char* path) { return Shell::ReadFile(isolate, path, false).ToLocalChecked(); }
...
MaybeLocal<String> Shell::ReadFile(Isolate* isolate, const char* name, bool should_throw)
{
    ...
    if (!file) {
        if (should_throw) {
            ...
            isolate->ThrowError(v8::String::NewFromUtf8(isolate, oss.str().substr(0, String::kMaxLength).c_str()).ToLocalChecked());
        }
        return MaybeLocal<String>();
    }
...
void Shell::RunShell(Isolate* isolate)
{
    ...
    while (true) {
        ...
        if (input.IsEmpty())
            break;
        ExecuteString(isolate, input, name, kPrintResult, kReportExceptions, kProcessMessageQueue);
    }
    printf("\n");
}
...
private:
    ...
    void Send(const v8_inspector::StringView& string)
    {
        ...
        Local<String> message
            = (string.is8Bit() ? v8::String::NewFromOneByte(isolate_, reinterpret_cast<const uint8_t*>(string.characters8()), v8::NewStringType::kNormal, length)
...
public:
    InspectorClient(Local<Context> context, bool connect)
    {
        ...
        session_ = inspector_->connect(1, channel_.get(), v8_inspector::StringView(), v8_inspector::V8Inspector::kFullyTrusted);
...
bool SourceGroup::Execute(Isolate* isolate)
{
    ...
    if (fuzzilli_reprl) {
        ...
        if (!Shell::ExecuteString(isolate, source, file_name, Shell::kNoPrintResult, Shell::kReportExceptions, Shell::kNoProcessMessageQueue)) {
            return false;
        }
    }
#endif // V8_FUZZILLI
    for (int i = begin_offset_; i < end_offset_; ++i) {
        ...
        if (!Shell::ExecuteString(isolate, source, file_name, Shell::kNoPrintResult, Shell::kReportExceptions, Shell::kProcessMessageQueue)) {
            success = false;
            break;
        }
    }
    ...
    if (Shell::options.web_snapshot_config || Shell::options.web_snapshot_output) {
        ...
        }
        {
            ...
            PerIsolateData data(isolate);
            {
                ...
                Local<Context> context = Shell::CreateEvaluationContext(isolate);
                {
                    ...
                    InspectorClient inspector_client(context, Shell::options.enable_inspector);
                    PerIsolateData::RealmScope realm_scope(PerIsolateData::Get(isolate));
                    Execute(isolate);
                    Shell::CompleteMessageLoop(isolate);
                }
...
bool Worker::StartWorkerThread(Isolate* requester, std::shared_ptr<Worker> worker)
{
    ...
    for (bool execute = true; execute; execute = false) {
        Isolate::Scope iscope(isolate_);
        {
            ...
            context_.Reset(isolate_, context);
            {
                ...
                Local<FunctionTemplate> postmessage_fun_template = FunctionTemplate::New(isolate_, PostMessageOut, this_value);
                ...
                if (Shell::ExecuteString(isolate_, source, file_name, Shell::kNoPrintResult, Shell::kReportExceptions, Shell::kProcessMessageQueue)) {
...
bool Shell::SetOptions(int argc, char* argv[])
{
    ...
    for (int i = 0; i < argc; i++) {
        if (strcmp(argv[i], "--") == 0) {
            ...
            argv[i] = nullptr;
        } else if (strcmp(argv[i], "--fuzzing") == 0 || strcmp(argv[i], "--no-abort-on-contradictory-flags") == 0 || strcmp(argv[i], "--noabort-on-contradictory-flags") == 0) {
            ...
            argv[i] = nullptr;
        } else if ((strcmp(argv[i], "--no-streaming-compile") == 0) || (strcmp(argv[i], "--nostreaming-compile") == 0)) {
    ...
    if (options.throw_on_failed_access_check && options.noop_on_failed_access_check && check_d8_flag_contradictions) {
        FATAL("Flag --throw-on-failed-access-check is incompatible with "
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(str, "--module") == 0 || strcmp(str, "--web-snapshot") == 0 || strcmp(str, "--json") == 0) {
...
void Shell::CollectGarbage(Isolate* isolate)
{
    if (options.send_idle_notification) {
        ...
        isolate->ContextDisposedNotification();
        isolate->IdleNotificationDeadline(g_platform->MonotonicallyIncreasingTime() + kLongIdlePauseInSeconds);
    }
...
bool ProcessMessages(Isolate* isolate, const std::function<platform::MessageLoopBehavior()>& behavior)
{
    while (true) {
        ...
        for (bool ran_tasks = true; ran_tasks;) {
            ...
            ran_tasks = v8::platform::PumpMessageLoop(g_default_platform, isolate, behavior());
            if (ran_tasks)
            ...
            // task.
            if (i::v8_flags.verify_predictable) {
                while (v8::platform::PumpMessageLoop(g_default_platform, kProcessGlobalPredictablePlatformWorkerTaskQueue, platform::MessageLoopBehavior::kDoNotWait)) {
                    ran_tasks = true;
                }
            }
        }
...
bool Shell::CompleteMessageLoop(Isolate* isolate)
{
    auto get_waiting_behaviour = [isolate]() {
        ...
        bool should_wait = (options.wait_for_background_tasks && isolate->HasPendingBackgroundTasks()) || isolate_status_[isolate] || isolate_running_streaming_tasks_[isolate] > 0;
        return should_wait ? platform::MessageLoopBehavior::kWaitForWork : platform::MessageLoopBehavior::kDoNotWait;
    };
    if (i::v8_flags.verify_predictable) {
...
void Shell::PostForegroundTask(Isolate* isolate, std::unique_ptr<Task> task) { g_default_platform->GetForegroundTaskRunner(isolate)->PostTask(std::move(task)); }

void Shell::PostBlockingBackgroundTask(std::unique_ptr<Task> task) { g_default_platform->CallBlockingTaskOnWorkerThread(std::move(task)); }
...
protected:
    ...
    void ThrowDataCloneError(Local<String> message) override { isolate_->ThrowException(Exception::Error(message)); }
...
int Shell::Main(int argc, char* argv[])
{
    ...
    g_platform = v8::platform::NewDefaultPlatform(options.thread_pool_size, v8::platform::IdleTaskSupport::kEnabled, in_process_stack_dumping, std::move(tracing));
