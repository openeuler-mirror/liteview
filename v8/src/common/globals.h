...
#if defined(V8_SHARED_RO_HEAP) && (!defined(V8_COMPRESS_POINTERS) || defined(V8_COMPRESS_POINTERS_IN_SHARED_CAGE)) && !defined(V8_DISABLE_WRITE_BARRIERS)
...
#if defined(USE_SIMULATOR) && (defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_LOONG64))
...
#if (V8_HOST_ARCH_PPC || V8_HOST_ARCH_PPC64) && (V8_TARGET_ARCH_PPC || V8_TARGET_ARCH_PPC64) && V8_OS_LINUX
...
template <typename F> F FUNCTION_CAST(byte* addr) { return reinterpret_cast<F>(reinterpret_cast<Address>(addr)); }

template <typename F> F FUNCTION_CAST(Address addr) { return reinterpret_cast<F>(addr); }
...
#if (V8_HOST_ARCH_PPC || V8_HOST_ARCH_PPC64) && (V8_OS_AIX || (V8_TARGET_ARCH_PPC64 && V8_TARGET_BIG_ENDIAN && (!defined(_CALL_ELF) || _CALL_ELF == 1)))
...
inline size_t hash_value(LanguageMode mode) { return static_cast<size_t>(mode); }
...
inline std::ostream& operator<<(std::ostream& os, LanguageMode mode) { return os << LanguageMode2String(mode); }

inline bool is_sloppy(LanguageMode language_mode) { return language_mode == LanguageMode::kSloppy; }

inline bool is_strict(LanguageMode language_mode) { return language_mode != LanguageMode::kSloppy; }
...
inline LanguageMode construct_language_mode(bool strict_bit) { return static_cast<LanguageMode>(strict_bit); }
...
inline size_t hash_value(DeoptimizeKind kind) { return static_cast<size_t>(kind); }
...
inline std::ostream& operator<<(std::ostream& os, DeoptimizeKind kind) { return os << ToString(kind); }
...
static_assert(kGarbageCollectionReasonMaxValue == static_cast<int>(GarbageCollectionReason::kLastReason), "The value of kGarbageCollectionReasonMaxValue is inconsistent.");

inline size_t hash_value(AllocationType kind) { return static_cast<uint8_t>(kind); }
...
inline constexpr bool IsSharedAllocationType(AllocationType kind) { return kind == AllocationType::kSharedOld || kind == AllocationType::kSharedMap; }
...
bool inline IsBaselineCodeFlushingEnabled(base::EnumSet<CodeFlushMode> mode) { return mode.contains(CodeFlushMode::kFlushBaselineCode); }

bool inline IsByteCodeFlushingEnabled(base::EnumSet<CodeFlushMode> mode) { return mode.contains(CodeFlushMode::kFlushBytecode); }

bool inline IsStressFlushingEnabled(base::EnumSet<CodeFlushMode> mode) { return mode.contains(CodeFlushMode::kStressFlushCode); }

bool inline IsFlushingDisabled(base::EnumSet<CodeFlushMode> mode) { return mode.empty(); }
...
inline REPLMode construct_repl_mode(bool is_repl_mode) { return is_repl_mode ? REPLMode::kYes : REPLMode::kNo; }
...
inline size_t hash_value(InlineCacheState mode) { return base::bit_cast<int>(mode); }
...
enum ShouldThrow { kThrowOnError = Internals::kThrowOnError, kDontThrow = Internals::kDontThrow };
...
inline size_t hash_value(ConvertReceiverMode mode) { return base::bit_cast<unsigned>(mode); }
...
inline size_t hash_value(CreateArgumentsType type) { return base::bit_cast<uint8_t>(type); }
...
inline bool IsDynamicVariableMode(VariableMode mode) { return mode >= VariableMode::kDynamic && mode <= VariableMode::kDynamicLocal; }
...
inline bool IsPrivateMethodOrAccessorVariableMode(VariableMode mode) { return mode >= VariableMode::kPrivateMethod && mode <= VariableMode::kPrivateGetterAndSetter; }

inline bool IsSerializableVariableMode(VariableMode mode) { return IsDeclaredVariableMode(mode) || IsPrivateMethodOrAccessorVariableMode(mode); }

inline bool IsConstVariableMode(VariableMode mode) { return mode == VariableMode::kConst || IsPrivateMethodOrAccessorVariableMode(mode); }
...
inline size_t hash_value(InterpreterPushArgsMode mode) { return base::bit_cast<unsigned>(mode); }
...
public:
    enum { kNone = 0x0, kSignedSmall = 0x1, kSignedSmallInputs = 0x3, kNumber = 0x7, kNumberOrOddball = 0xF, kString = 0x10, kBigInt64 = 0x20, kBigInt = 0x60, kAny = 0x7F };
};
...
static_assert((static_cast<int>(ForInFeedback::kNone) | static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices)) == static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices));
static_assert((static_cast<int>(ForInFeedback::kEnumCacheKeysAndIndices) | static_cast<int>(ForInFeedback::kEnumCacheKeys)) == static_cast<int>(ForInFeedback::kEnumCacheKeys));
...
inline size_t hash_value(UnicodeEncoding encoding) { return static_cast<uint8_t>(encoding); }
...
// set or not.
enum class DefineKeyedOwnPropertyInLiteralFlag { kNoFlags = 0, kDontEnum = 1 << 0, kSetFunctionName = 1 << 1 };
...
#define V(Name, Value)                                                                                                                                                             \
    constexpr bool Is##Name(TieringState state) { return state == TieringState::k##Name; }
TIERING_STATE_LIST(V)
#undef V

constexpr bool IsRequestMaglev(TieringState state) { return IsRequestMaglev_Concurrent(state) || IsRequestMaglev_Synchronous(state); }
constexpr bool IsRequestTurbofan(TieringState state) { return IsRequestTurbofan_Concurrent(state) || IsRequestTurbofan_Synchronous(state); }
...
inline std::ostream& operator<<(std::ostream& os, TieringState marker) { return os << ToString(marker); }
...
constexpr bool IsSynchronous(ConcurrencyMode mode) { return mode == ConcurrencyMode::kSynchronous; }
constexpr bool IsConcurrent(ConcurrencyMode mode) { return mode == ConcurrencyMode::kConcurrent; }
...
inline std::ostream& operator<<(std::ostream& os, ConcurrencyMode mode) { return os << ToString(mode); }
...
enum KeyedAccessStoreMode { STANDARD_STORE, STORE_AND_GROW_HANDLE_COW, STORE_IGNORE_OUT_OF_BOUNDS, STORE_HANDLE_COW };

enum MutableMode { MUTABLE, IMMUTABLE };

inline bool IsCOWHandlingStoreMode(KeyedAccessStoreMode store_mode) { return store_mode == STORE_HANDLE_COW || store_mode == STORE_AND_GROW_HANDLE_COW; }

inline bool IsGrowStoreMode(KeyedAccessStoreMode store_mode) { return store_mode == STORE_AND_GROW_HANDLE_COW; }
...
inline constexpr int JSParameterCount(int param_count_without_receiver) { return param_count_without_receiver + kJSArgcReceiverSlots; }
...
V8_INLINE bool operator!(ExceptionStatus status) { return !static_cast<bool>(status); }
...
public:
    ...
    bool operator==(const PtrComprCageBase& other) const { return address_ == other.address_; }
