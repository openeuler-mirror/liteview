...
{
    if (code_space_write_nesting_level_.Get() == 0) {
...
void RwxMemoryWriteScope::SetWritable()
{
    ...
    if (!IsSupported())
        return;
    if (code_space_write_nesting_level_.Get() == 0) {
        DCHECK_NE(base::MemoryProtectionKey::GetKeyPermission(memory_protection_key_), base::MemoryProtectionKey::kNoRestrictions);
        base::MemoryProtectionKey::SetPermissionsForKey(memory_protection_key_, base::MemoryProtectionKey::kNoRestrictions);
    }
    ++code_space_write_nesting_level_;
}
...
void RwxMemoryWriteScope::SetExecutable()
{
    ...
    --code_space_write_nesting_level_;
    if (code_space_write_nesting_level_.Get() == 0) {
        DCHECK_EQ(base::MemoryProtectionKey::GetKeyPermission(memory_protection_key_), base::MemoryProtectionKey::kNoRestrictions);
