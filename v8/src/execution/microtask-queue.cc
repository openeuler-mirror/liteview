...
void MicrotaskQueue::EnqueueMicrotask(v8::Isolate* v8_isolate, v8::Local<Function> function)
{
    ...
    Handle<CallableTask> microtask = isolate->factory()->NewCallableTask(Utils::OpenHandle(*function), isolate->native_context());
...
void MicrotaskQueue::EnqueueMicrotask(v8::Isolate* v8_isolate, v8::MicrotaskCallback callback, void* data)
{
    ...
    Handle<CallbackTask> microtask
        = isolate->factory()->NewCallbackTask(isolate->factory()->NewForeign(reinterpret_cast<Address>(callback)), isolate->factory()->NewForeign(reinterpret_cast<Address>(data)));
    EnqueueMicrotask(*microtask);
}
...
void MicrotaskQueue::PerformCheckpointInternal(v8::Isolate* v8_isolate)
{
    ...
    if (microtasks_policy_ == v8::MicrotasksPolicy::kScoped) {
        ...
        microtasks_scope.reset(new v8::MicrotasksScope(v8_isolate, this, v8::MicrotasksScope::kDoNotRunMicrotasks));
    }
    Isolate* isolate = reinterpret_cast<Isolate*>(v8_isolate);
...
int MicrotaskQueue::RunMicrotasks(Isolate* isolate)
{
    ...
    int processed_microtask_count;
    {
        ...
        v8::Isolate::SuppressMicrotaskExecutionScope suppress(reinterpret_cast<v8::Isolate*>(isolate));
...
void MicrotaskQueue::IterateMicrotasks(RootVisitor* visitor)
{
    if (size_) {
        ...
        visitor->VisitRootPointers(Root::kStrongRoots, nullptr, FullObjectSlot(ring_buffer_ + start_), FullObjectSlot(ring_buffer_ + std::min(start_ + size_, capacity_)));
        visitor->VisitRootPointers(
            Root::kStrongRoots, nullptr, FullObjectSlot(ring_buffer_), FullObjectSlot(ring_buffer_ + std::max(start_ + size_ - capacity_, static_cast<intptr_t>(0))));
    }
...
void MicrotaskQueue::AddMicrotasksCompletedCallback(MicrotasksCompletedCallbackWithData callback, void* data)
{
    ...
    auto pos = std::find(microtasks_completed_callbacks_.begin(), microtasks_completed_callbacks_.end(), callback_with_data);
