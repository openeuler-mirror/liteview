...
std::ostream& operator<<(std::ostream& os, OptimizationReason reason) { return os << OptimizationReasonToString(reason); }
...
public:
    ...
    static constexpr OptimizationDecision TurbofanHotAndStable() { return { OptimizationReason::kHotAndStable, CodeKind::TURBOFAN, ConcurrencyMode::kConcurrent }; }
    static constexpr OptimizationDecision TurbofanSmallFunction() { return { OptimizationReason::kSmallFunction, CodeKind::TURBOFAN, ConcurrencyMode::kConcurrent }; }
    ...
    constexpr bool should_optimize() const { return optimization_reason != OptimizationReason::kDoNotOptimize; }
...
void TraceRecompile(Isolate* isolate, JSFunction function, OptimizationDecision d)
{
...
void TraceManualRecompile(JSFunction function, CodeKind code_kind, ConcurrencyMode concurrency_mode)
{
    if (v8_flags.trace_opt) {
...
bool TiersUpToMaglev(CodeKind code_kind)
{
    ...
    return V8_UNLIKELY(v8_flags.maglev) && CodeKindIsUnoptimizedJSFunction(code_kind);
}

bool TiersUpToMaglev(base::Optional<CodeKind> code_kind) { return code_kind.has_value() && TiersUpToMaglev(code_kind.value()); }

int InterruptBudgetFor(base::Optional<CodeKind> code_kind) { return TiersUpToMaglev(code_kind) ? v8_flags.interrupt_budget_for_maglev : v8_flags.interrupt_budget; }
...
int TieringManager::InterruptBudgetFor(Isolate* isolate, JSFunction function)
{
    ...
    return function.shared().GetBytecodeArray(isolate).length() * v8_flags.interrupt_budget_factor_for_feedback_allocation;
}

// static
int TieringManager::InitialInterruptBudget()
{
    return V8_LIKELY(v8_flags.lazy_feedback_allocation) ? v8_flags.interrupt_budget_for_feedback_allocation : v8_flags.interrupt_budget;
}

namespace {

bool SmallEnoughForOSR(Isolate* isolate, JSFunction function, CodeKind code_kind)
{
    ...
    const double scale_factor_for_active_tier = InterruptBudgetFor(code_kind) / static_cast<double>(v8_flags.interrupt_budget);

    const double raw_limit = kOSRBytecodeSizeAllowanceBase + scale_factor_for_active_tier * kOSRBytecodeSizeAllowancePerTick * function.feedback_vector().profiler_ticks();
    const int limit = raw_limit < BytecodeArray::kMaxLength ? static_cast<int>(raw_limit) : BytecodeArray::kMaxLength;
    DCHECK_GT(limit, 0);
...
void TryRequestOsrAtNextOpportunity(Isolate* isolate, JSFunction function) { TrySetOsrUrgency(isolate, function, FeedbackVector::kMaxOsrUrgency); }

bool ShouldOptimizeAsSmallFunction(int bytecode_size, bool any_ic_changed) { return !any_ic_changed && bytecode_size < v8_flags.max_bytecode_size_for_early_opt; }
...
void TieringManager::MaybeOptimizeFrame(JSFunction function, CodeKind calling_code_kind)
{
    const TieringState tiering_state = function.feedback_vector().tiering_state();
    const TieringState osr_tiering_state = function.feedback_vector().osr_tiering_state();
    if (V8_UNLIKELY(IsInProgress(tiering_state)) || V8_UNLIKELY(IsInProgress(osr_tiering_state))) {
    ...
    if (V8_UNLIKELY(v8_flags.testing_d8_test_runner) && !PendingOptimizationTable::IsHeuristicOptimizationAllowed(isolate_, function)) {
    ...
    // implemented.
    if (IsRequestTurbofan(tiering_state) || function.HasAvailableCodeKind(CodeKind::TURBOFAN)) {
    ...
    if (d.should_optimize() && d.code_kind == CodeKind::MAGLEV) {
        bool is_marked_for_maglev_optimization = IsRequestMaglev(tiering_state) || function.HasAvailableCodeKind(CodeKind::MAGLEV);
...
OptimizationDecision TieringManager::ShouldOptimize(JSFunction function, CodeKind calling_code_kind)
{
    if (TiersUpToMaglev(calling_code_kind) && function.shared().PassesFilter(v8_flags.maglev_filter) && !function.shared(isolate_).maglev_compilation_failed()) {
    ...
    if (!v8_flags.turbofan || !function.shared().PassesFilter(v8_flags.turbo_filter)) {
        return OptimizationDecision::DoNotOptimize();
    }

    BytecodeArray bytecode = function.shared().GetBytecodeArray(isolate_);
    const int ticks = function.feedback_vector().profiler_ticks();
    const int ticks_for_optimization = v8_flags.ticks_before_optimization + (bytecode.length() / v8_flags.bytecode_size_allowance_per_tick);
    if (ticks >= ticks_for_optimization) {
        return OptimizationDecision::TurbofanHotAndStable();
    } else if (ShouldOptimizeAsSmallFunction(bytecode.length(), any_ic_changed_)) {
        // If no IC was patched since the last tick and this function is very
        // small, optimistically optimize it now.
        return OptimizationDecision::TurbofanSmallFunction();
    } else if (v8_flags.trace_opt_verbose) {
        PrintF("[not yet optimizing %s, not enough ticks: %d/%d and ", function.DebugNameCStr().get(), ticks, ticks_for_optimization);
        if (any_ic_changed_) {
            PrintF("ICs changed]\n");
        } else {
            PrintF(" too large for small function optimization: %d/%d]\n", bytecode.length(), v8_flags.max_bytecode_size_for_early_opt.value());
        }
    }
...
TieringManager::OnInterruptTickScope::OnInterruptTickScope(TieringManager* profiler)
    : profiler_(profiler)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.MarkCandidatesForOptimization");
}

TieringManager::OnInterruptTickScope::~OnInterruptTickScope() { profiler_->any_ic_changed_ = false; }

void TieringManager::OnInterruptTick(Handle<JSFunction> function, CodeKind code_kind)
{
    ...
    // tiering.
    if (CanCompileWithBaseline(isolate_, function->shared()) && function->ActiveTierIsIgnition()) {
