...
#define ISOLATE_DATA_FIELDS(V)                                                                                                                                                     \
    ...
    IF_TARGET_ARCH_64_BIT(V, kTablesAlignmentPaddingOffset, kSystemPointerSize - 4, tables_alignment_padding)                                                                      \
    /* Tier 0 tables (small but fast access). */                                                                                                                                   \
    V(kBuiltinTier0EntryTableOffset, Builtins::kBuiltinTier0Count* kSystemPointerSize, builtin_tier0_entry_table)                                                                  \
    V(kBuiltinsTier0TableOffset, Builtins::kBuiltinTier0Count* kSystemPointerSize, builtin_tier0_table)                                                                            \
    /* Misc. fields. */                                                                                                                                                            \
    V(kEmbedderDataOffset, Internals::kNumIsolateDataSlots* kSystemPointerSize, embedder_data)                                                                                     \
    ...
    V(kRootsTableOffset, RootsTable::kEntriesCount* kSystemPointerSize, roots_table)                                                                                               \
    V(kExternalReferenceTableOffset, ExternalReferenceTable::kSizeInBytes, external_reference_table)                                                                               \
    V(kThreadLocalTopOffset, ThreadLocalTop::kSizeInBytes, thread_local_top)                                                                                                       \
    V(kBuiltinEntryTableOffset, Builtins::kBuiltinCount* kSystemPointerSize, builtin_entry_table)                                                                                  \
    V(kBuiltinTableOffset, Builtins::kBuiltinCount* kSystemPointerSize, builtin_table)                                                                                             \
...
#define ISOLATE_DATA_FIELDS_POINTER_COMPRESSION(V)                                                                                                                                 \
    V(kExternalPointerTableOffset, ExternalPointerTable::kSize, external_pointer_table)                                                                                            \
    V(kSharedExternalPointerTableOffset, kSystemPointerSize, shared_external_pointer_table)
#else
#define ISOLATE_DATA_FIELDS_POINTER_COMPRESSION(V)
...
public:
    ...
    Address isolate_root() const { return reinterpret_cast<Address>(this) + kIsolateRootBias; }
...
#undef V
    ...
    static constexpr int root_slot_offset(RootIndex root_index) { return roots_table_offset() + RootsTable::offset_of(root_index); }
    ...
    static constexpr int builtin_slot_offset(int builtin_index) { return BuiltinSlotOffset(Builtins::FromInt(builtin_index)); }
...
#undef V
    ...
    ExternalReferenceTable* external_reference_table() { return &external_reference_table_; }
...
#define FIELDS(V)                                                                                                                                                                  \
    ...
    V(kPaddingOffset, 8 + RoundUp<8>(static_cast<int>(kPaddingOffset)) - kPaddingOffset)                                                                                           \
    /* Total size. */                                                                                                                                                              \
    V(kSize, 0)
...
#define V(Offset, Size, Name) static_assert(offsetof(IsolateData, Name##_) == Offset);
    ISOLATE_DATA_FIELDS(V)
#undef V
