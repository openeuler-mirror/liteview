...
Object Isolate::pending_message() { return thread_local_top()->pending_message_; }

void Isolate::clear_pending_message() { set_pending_message(ReadOnlyRoots(this).the_hole_value()); }

bool Isolate::has_pending_message() { return !pending_message().IsTheHole(this); }
...
void Isolate::set_scheduled_exception(Object exception) { thread_local_top()->scheduled_exception_ = exception; }

bool Isolate::is_execution_termination_pending() { return thread_local_top()->pending_exception_ == i::ReadOnlyRoots(this).termination_exception(); }

bool Isolate::is_execution_terminating() { return thread_local_top()->scheduled_exception_ == i::ReadOnlyRoots(this).termination_exception(); }
...
Object Isolate::VerifyBuiltinsResult(Object result)
{
    ...
    // builtin).
    if (!result.IsSmi()) {
        DCHECK_EQ(result.ptr(), V8HeapCompressionScheme::DecompressTaggedPointer(this, static_cast<Tagged_t>(result.ptr())));
    }
#endif
    return result;
}
...
ObjectPair Isolate::VerifyBuiltinsResult(ObjectPair pair)
{
    ...
    DCHECK_EQ(has_pending_exception(), pair.x == ReadOnlyRoots(this).exception().ptr());
#ifdef V8_COMPRESS_POINTERS
    // Check that the returned pointer is actually part of the current isolate,
    // because that's the assumption in generated code (which might call this
    // builtin).
    if (!HAS_SMI_TAG(pair.x)) {
        DCHECK_EQ(pair.x, V8HeapCompressionScheme::DecompressTaggedPointer(this, static_cast<Tagged_t>(pair.x)));
    }
    if (!HAS_SMI_TAG(pair.y)) {
        DCHECK_EQ(pair.y, V8HeapCompressionScheme::DecompressTaggedPointer(this, static_cast<Tagged_t>(pair.y)));
    }
...
bool Isolate::is_catchable_by_javascript(Object exception) { return exception != ReadOnlyRoots(heap()).termination_exception(); }
...
bool Isolate::is_catchable_by_wasm(Object exception)
{
    ...
    LookupIterator it(this, handle(JSReceiver::cast(exception), this), factory()->wasm_uncatchable_symbol(), LookupIterator::OWN_SKIP_INTERCEPTOR);
...
Handle<JSGlobalObject> Isolate::global_object() { return handle(context().global_object(), this); }

Handle<JSGlobalProxy> Isolate::global_proxy() { return handle(context().global_proxy(), this); }
...
Isolate::ExceptionScope::~ExceptionScope() { isolate_->set_pending_exception(*pending_exception_); }
...
#define NATIVE_CONTEXT_FIELD_ACCESSOR(index, type, name)                                                                                                                           \
    Handle<type> Isolate::name() { return Handle<type>(raw_native_context().name(), this); }                                                                                       \
    bool Isolate::is_##name(type value) { return raw_native_context().is_##name(value); }
