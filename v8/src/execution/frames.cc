...
public:
        ...
        , handler_(handler)
    {
        ...
        DCHECK_IMPLIES(!v8_flags.experimental_wasm_stack_switching, frame->sp() <= AddressOf(handler));
...
StackFrameIteratorBase::StackFrameIteratorBase(Isolate* isolate, bool can_access_heap_objects)
    : isolate_(isolate)
...
#if V8_ENABLE_WEBASSEMBLY
    DCHECK_IMPLIES(done() && !v8_flags.experimental_wasm_stack_switching, handler_ == nullptr);
#else
...
{
    StackFrame::State state;
    StackFrame::Type type = ExitFrame::GetStateForFramePointer(Isolate::c_entry_fp(top), &state);
...
StackFrame* StackFrameIteratorBase::SingletonFor(StackFrame::Type type, StackFrame::State* state)
{
...
StackTraceFrameIterator::StackTraceFrameIterator(Isolate* isolate, StackFrameId id)
...
FrameSummary StackTraceFrameIterator::GetTopValidFrame() const
{
    ...
    if (is_javascript()) {
        for (int i = static_cast<int>(frames.size()) - 1; i >= 0; i--) {
            if (!IsValidJSFunction(*frames[i].AsJavaScript().function()))
                continue;
            return frames[i];
        }
        UNREACHABLE();
...
bool IsInterpreterFramePc(Isolate* isolate, Address pc, StackFrame::State* state)
{
    Builtin builtin = OffHeapInstructionStream::TryLookupCode(isolate, pc);
    if (builtin != Builtin::kNoBuiltinId
        && (builtin == Builtin::kInterpreterEntryTrampoline || builtin == Builtin::kInterpreterEnterAtBytecode || builtin == Builtin::kInterpreterEnterAtNextBytecode
            || builtin == Builtin::kBaselineOrInterpreterEnterAtBytecode || builtin == Builtin::kBaselineOrInterpreterEnterAtNextBytecode)) {
        return true;
        ...
        Object maybe_function = Object(Memory<Address>(state->fp + StandardFrameConstants::kFunctionOffset));
        ...
        CodeLookupResult interpreter_entry_trampoline = isolate->heap()->GcSafeFindCodeForInnerPointer(pc);
        return interpreter_entry_trampoline.code().is_interpreter_trampoline_builtin();
    } else {
        return false;
    }
}
...
bool SafeStackFrameIterator::IsNoFrameBytecodeHandlerPc(Isolate* isolate, Address pc, Address fp) const
{
    ...
    if (!IsValidStackAddress(fp + CommonFrameConstants::kContextOrFrameTypeOffset)) {
        return false;
    }

    // Check if top stack frame is a bytecode handler stub frame.
    MSAN_MEMORY_IS_INITIALIZED(fp + CommonFrameConstants::kContextOrFrameTypeOffset, kSystemPointerSize);
...
SafeStackFrameIterator::SafeStackFrameIterator(Isolate* isolate, Address pc, Address fp, Address sp, Address lr, Address js_entry_sp)
    ...
    , top_link_register_(lr)
{
    ...
    if (fast_c_fp) {
        ...
        state.fp = fp;
        state.sp = sp;
        state.pc_address = StackFrame::ResolveReturnAddressLocation(reinterpret_cast<Address*>(CommonFrame::ComputePCAddress(fp)));
        ...
        if (IsValidStackAddress(frame_marker)) {
            ...
            top_frame_type_ = type;
            MSAN_MEMORY_IS_INITIALIZED(fp + CommonFrameConstants::kContextOrFrameTypeOffset, kSystemPointerSize);
            Address type_or_context_address = Memory<Address>(fp + CommonFrameConstants::kContextOrFrameTypeOffset);
...
bool SafeStackFrameIterator::IsValidFrame(StackFrame* frame) const { return IsValidStackAddress(frame->sp()) && IsValidStackAddress(frame->fp()); }
...
namespace {
CodeLookupResult GetContainingCode(Isolate* isolate, Address pc) { return isolate->inner_pointer_to_code_cache()->GetCacheEntry(pc)->code; }
} // namespace
...
void StackFrame::IteratePc(RootVisitor* v, Address* pc_address, Address* constant_pool_address, CodeLookupResult lookup_result) const
{
    ...
    DCHECK(ReadOnlyHeap::Contains(holder) || holder.GetHeap()->GcSafeCodeContains(holder, old_pc));
...
template <typename CodeOrCodeT> inline StackFrame::Type ComputeBuiltinFrameType(CodeOrCodeT code)
{
...
StackFrame::Type StackFrame::ComputeType(const StackFrameIteratorBase* iterator, State* state)
{
#if V8_ENABLE_WEBASSEMBLY
    ...
    if (!iterator->can_access_heap_objects_) {
        ...
        if (lookup_result.IsFound()) {
            ...
            case CodeKind::BUILTIN: {
                ...
                // middle of GC.
                if (lookup_result.IsCodeDataContainer()) {
                    return ComputeBuiltinFrameType(CodeT::cast(lookup_result.code_data_container()));
                }
...
#ifdef DEBUG
bool StackFrame::can_access_heap_objects() const { return iterator_->can_access_heap_objects_; }
#endif
...
Address CommonFrame::GetCallerStackPointer() const { return fp() + CommonFrameConstants::kCallerSPOffset; }
...
HeapObject EntryFrame::unchecked_code() const { return isolate()->builtins()->code(Builtin::kJSEntry); }

void EntryFrame::ComputeCallerState(State* state) const { GetCallerState(state); }
...
HeapObject ConstructEntryFrame::unchecked_code() const { return isolate()->builtins()->code(Builtin::kJSConstructEntry); }
...
void BuiltinExitFrame::Summarize(std::vector<FrameSummary>* frames) const
{
    ...
    FrameSummary::JavaScriptFrameSummary summary(isolate(), receiver(), function(), code.ToAbstractCode(), code_offset, IsConstructor(), *parameters);
...
bool BuiltinExitFrame::IsConstructor() const { return !new_target_slot_object().IsUndefined(isolate()); }

namespace {
void PrintIndex(StringStream* accumulator, StackFrame::PrintMode mode, int index) { accumulator->Add((mode == StackFrame::OVERVIEW) ? "%5d: " : "[%d]: ", index); }
...
Object CommonFrame::context() const { return ReadOnlyRoots(isolate()).undefined_value(); }
...
#endif
    state->sp = caller_sp();
    state->pc_address = ResolveReturnAddressLocation(reinterpret_cast<Address*>(ComputePCAddress(fp())));
    state->callee_fp = fp();
...
void VisitSpillSlot(Isolate* isolate, RootVisitor* v, FullObjectSlot spill_slot)
{
    ...
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        ...
        if (!HAS_SMI_TAG(value) && value <= 0xffffffff) {
            ...
            was_compressed = true;
            *spill_slot.location() = V8HeapCompressionScheme::DecompressTaggedPointer(cage_base, static_cast<Tagged_t>(value));
            if (DEBUG_BOOL) {
                ...
                HeapObject forwarded = map_word.IsForwardingAddress() ? map_word.ToForwardingAddress() : raw;
                bool is_self_forwarded = forwarded.map_word(cage_base, kRelaxedLoad).ptr() == forwarded.address();
                if (is_self_forwarded) {
                    ...
                    CHECK(BasicMemoryChunk::FromHeapObject(forwarded)->InNewLargeObjectSpace());
                } else {
        ...
        if (!HAS_SMI_TAG(compressed_value)) {
            ...
            *spill_slot.location() = V8HeapCompressionScheme::DecompressTaggedPointer(cage_base, compressed_value);
        }
    }
#endif
    ...
    if (was_compressed) {
        ...
        *spill_slot.location() = V8HeapCompressionScheme::CompressTagged(*spill_slot.location());
    }
#endif
}
...
SafepointEntry GetSafepointEntryFromCodeCache(Isolate* isolate, Address inner_pointer, InnerPointerToCodeCache::InnerPointerToCodeCacheEntry* entry)
{
    if (!entry->safepoint_entry.is_initialized()) {
        ...
        DCHECK_EQ(entry->safepoint_entry, entry->code.GetSafepointEntry(isolate, inner_pointer));
    }
...
MaglevSafepointEntry GetMaglevSafepointEntryFromCodeCache(Isolate* isolate, Address inner_pointer, InnerPointerToCodeCache::InnerPointerToCodeCacheEntry* entry)
{
    if (!entry->maglev_safepoint_entry.is_initialized()) {
        entry->maglev_safepoint_entry = entry->code.GetMaglevSafepointEntry(isolate, inner_pointer);
        DCHECK(entry->maglev_safepoint_entry.is_initialized());
    } else {
        DCHECK_EQ(entry->maglev_safepoint_entry, entry->code.GetMaglevSafepointEntry(isolate, inner_pointer));
    }
...
void WasmFrame::Iterate(RootVisitor* v) const
{
    ...
    // not be visited.
    static_assert(WasmExitFrameConstants::kFixedSlotCountFromFp == WasmFrameConstants::kFixedSlotCountFromFp + 1, "WasmExitFrame has one slot more than WasmFrame");
    ...
    FullObjectSlot parameters_limit(frame_header_base.address() - spill_slot_space);
...
void TypedFrame::Iterate(RootVisitor* v) const
{
    ...
    int spill_slots_size = entry->code.stack_slots() * kSystemPointerSize - (frame_header_size + StandardFrameConstants::kFixedFrameSizeAboveFp);
    ...
    // Visit pointer spill slots and locals.
    DCHECK_GE((entry->code.stack_slots() + kBitsPerByte) / kBitsPerByte, safepoint_entry.tagged_slots().size());
    VisitSpillSlots(isolate(), v, parameters_limit, safepoint_entry.tagged_slots());
...
void MaglevFrame::Iterate(RootVisitor* v) const
{
    ...
    DCHECK_EQ(entry->code.stack_slots(), StandardFrameConstants::kFixedSlotCount + maglev_safepoint_entry.num_tagged_slots() + maglev_safepoint_entry.num_untagged_slots());
    ...
    intptr_t expected_frame_size_excl_outgoing_params
        = StandardFrameConstants::kFixedFrameSizeFromFp + (spill_slot_count + maglev_safepoint_entry.num_pushed_registers()) * kSystemPointerSize;
    if (actual_frame_size < expected_frame_size_excl_outgoing_params) {
        ...
        if (v8_flags.maglev_ool_prologue) {
            ...
            DCHECK_EQ(isolate()->c_function(), Runtime::FunctionForId(Runtime::kStackGuardWithGap)->entry);
            ...
            DCHECK_EQ(actual_frame_size, StandardFrameConstants::kFixedFrameSizeFromFp + 2 * kSystemPointerSize);
            DCHECK_EQ(isolate()->c_function(), Runtime::FunctionForId(Runtime::kStackGuardWithGap)->entry);
            DCHECK_EQ(maglev_safepoint_entry.num_pushed_registers(), 0);
        }
        spill_slot_count = 0;
    ...
    FullObjectSlot parameters_base(&Memory<Address>(sp()));
    FullObjectSlot parameters_limit = frame_header_base - spill_slot_count - maglev_safepoint_entry.num_pushed_registers();
    v->VisitRootPointers(Root::kStackRoots, nullptr, parameters_base, parameters_limit);
...
bool CommonFrame::HasTaggedOutgoingParams(CodeLookupResult& code_lookup) const
{
    ...
    wasm::WasmCode* wasm_callee = wasm::GetWasmCodeManager()->LookupCode(callee_pc());
...
void CommonFrame::IterateTurbofanOptimizedFrame(RootVisitor* v) const
{
    ...
    int spill_slot_count = entry->code.stack_slots() - StandardFrameConstants::kFixedSlotCount;
...
void TurbofanStubWithContextFrame::Iterate(RootVisitor* v) const { return IterateTurbofanOptimizedFrame(v); }

void TurbofanFrame::Iterate(RootVisitor* v) const { return IterateTurbofanOptimizedFrame(v); }
...
void JavaScriptFrame::SetParameterValue(int index, Object value) const { Memory<Address>(GetParameterSlot(index)) = value.ptr(); }

bool JavaScriptFrame::IsConstructor() const { return IsConstructFrame(caller_fp()); }
...
HeapObject CommonFrameWithJSLinkage::unchecked_code() const { return function().code(); }
...
Address JavaScriptFrame::GetCallerStackPointer() const { return fp() + StandardFrameConstants::kCallerSPOffset; }

void JavaScriptFrame::GetFunctions(std::vector<SharedFunctionInfo>* functions) const
{
...
bool CommonFrameWithJSLinkage::IsConstructor() const { return IsConstructFrame(caller_fp()); }
...
JSFunction JavaScriptFrame::function() const { return JSFunction::cast(function_slot_object()); }
...
Script JavaScriptFrame::script() const { return Script::cast(function().shared().script()); }
...
void JavaScriptFrame::PrintFunctionAndOffset(JSFunction function, AbstractCode code, int code_offset, FILE* file, bool print_line_number)
{
...
void JavaScriptFrame::PrintTop(Isolate* isolate, FILE* file, bool print_args, bool print_line_number)
{
    ...
    while (!it.done()) {
        if (it.frame()->is_java_script()) {
            ...
            PrintFunctionAndOffset(function, abstract_code, code_offset, file, print_line_number);
            if (print_args) {
                // function arguments
...
// static
void JavaScriptFrame::CollectFunctionAndOffsetForICStats(JSFunction function, AbstractCode code, int code_offset)
{
...
Object CommonFrameWithJSLinkage::GetParameter(int index) const { return Object(Memory<Address>(GetParameterSlot(index))); }
...
int JavaScriptFrame::GetActualArgumentCount() const { return static_cast<int>(Memory<intptr_t>(fp() + StandardFrameConstants::kArgCOffset)) - kJSArgcReceiverSlots; }
...
int JavaScriptBuiltinContinuationFrame::ComputeParametersCount() const
{
    ...
    Object argc_object(Memory<Address>(fp() + BuiltinContinuationFrameConstants::kArgCOffset));
...
Object JavaScriptBuiltinContinuationFrame::context() const { return Object(Memory<Address>(fp() + BuiltinContinuationFrameConstants::kBuiltinContextOffset)); }

void JavaScriptBuiltinContinuationWithCatchFrame::SetException(Object exception)
{
    ...
    CHECK_EQ(ReadOnlyRoots(isolate()).the_hole_value(), Object(Memory<Address>(exception_argument_slot)));
    Memory<Address>(exception_argument_slot) = exception.ptr();
}

FrameSummary::JavaScriptFrameSummary::JavaScriptFrameSummary(
    Isolate* isolate, Object receiver, JSFunction function, AbstractCode abstract_code, int code_offset, bool is_constructor, FixedArray parameters)
...
bool FrameSummary::JavaScriptFrameSummary::is_subject_to_debugging() const { return function()->shared().IsSubjectToDebugging(); }

int FrameSummary::JavaScriptFrameSummary::SourcePosition() const { return abstract_code()->SourcePosition(isolate(), code_offset()); }

int FrameSummary::JavaScriptFrameSummary::SourceStatementPosition() const { return abstract_code()->SourceStatementPosition(isolate(), code_offset()); }

Handle<Object> FrameSummary::JavaScriptFrameSummary::script() const { return handle(function_->shared().script(), isolate()); }

Handle<Context> FrameSummary::JavaScriptFrameSummary::native_context() const { return handle(function_->native_context(), isolate()); }
...
Handle<StackFrameInfo> FrameSummary::JavaScriptFrameSummary::CreateStackFrameInfo() const
{
    ...
    if (function_name->length() == 0 && script->compilation_type() == Script::COMPILATION_TYPE_EVAL) {
...
#if V8_ENABLE_WEBASSEMBLY
FrameSummary::WasmFrameSummary::WasmFrameSummary(Isolate* isolate, Handle<WasmInstanceObject> instance, wasm::WasmCode* code, int code_offset, bool at_to_number_conversion)
...
Handle<Object> FrameSummary::WasmFrameSummary::receiver() const { return wasm_instance_->GetIsolate()->global_proxy(); }

uint32_t FrameSummary::WasmFrameSummary::function_index() const { return code()->index(); }

int FrameSummary::WasmFrameSummary::byte_offset() const { return code_->GetSourcePositionBefore(code_offset()); }

int FrameSummary::WasmFrameSummary::SourcePosition() const
{
    const wasm::WasmModule* module = wasm_instance()->module_object().module();
    return GetSourcePosition(module, function_index(), byte_offset(), at_to_number_conversion());
}

Handle<Script> FrameSummary::WasmFrameSummary::script() const { return handle(wasm_instance()->module_object().script(), wasm_instance()->GetIsolate()); }

Handle<Context> FrameSummary::WasmFrameSummary::native_context() const { return handle(wasm_instance()->native_context(), isolate()); }

Handle<StackFrameInfo> FrameSummary::WasmFrameSummary::CreateStackFrameInfo() const
{
    Handle<String> function_name = GetWasmFunctionDebugName(isolate(), wasm_instance(), function_index());
...
FrameSummary FrameSummary::GetBottom(const CommonFrame* frame) { return Get(frame, 0); }
...
void OptimizedFrame::Summarize(std::vector<FrameSummary>* frames) const
{
    ...
    if (deopt_index == SafepointEntry::kNoDeoptIndex) {
        ...
        if (code.is_maglevved()) {
            DCHECK(frames->empty());
            Handle<AbstractCode> abstract_code(AbstractCode::cast(function().shared().GetBytecodeArray(isolate())), isolate());
            Handle<FixedArray> params = GetParameters();
            FrameSummary::JavaScriptFrameSummary summary(isolate(), receiver(), function(), *abstract_code, kFunctionEntryBytecodeOffset, IsConstructor(), *params);
            frames->push_back(summary);
            return;
        }
    ...
    for (auto it = translated.begin(); it != translated.end(); it++) {
        if (it->kind() == TranslatedFrame::kUnoptimizedFunction || it->kind() == TranslatedFrame::kJavaScriptBuiltinContinuation
            ...
            if (it->kind() == TranslatedFrame::kJavaScriptBuiltinContinuation || it->kind() == TranslatedFrame::kJavaScriptBuiltinContinuationWithCatch) {
                code_offset = 0;
                abstract_code = ToAbstractCode(isolate()->builtins()->code_handle(Builtins::GetBuiltinFromBytecodeOffset(it->bytecode_offset())), isolate());
            } else {
                DCHECK_EQ(it->kind(), TranslatedFrame::kUnoptimizedFunction);
                code_offset = it->bytecode_offset().ToInt();
                abstract_code = handle(shared_info->abstract_code(isolate()), isolate());
            }
...
int OptimizedFrame::LookupExceptionHandlerInTable(int* data, HandlerTable::CatchPrediction* prediction)
{
...
int MaglevFrame::FindReturnPCForTrampoline(CodeT code, int trampoline_pc) const
{
...
int TurbofanFrame::FindReturnPCForTrampoline(CodeT code, int trampoline_pc) const
{
...
DeoptimizationData OptimizedFrame::GetDeoptimizationData(int* deopt_index) const
{
    DCHECK(is_optimized());
    ...
    if (!code.contains(isolate(), pc())) {
        CodeLookupResult lookup_result = isolate()->heap()->GcSafeFindCodeForInnerPointer(pc());
...
void OptimizedFrame::GetFunctions(std::vector<SharedFunctionInfo>* functions) const
{
    DCHECK(functions->empty());
    DCHECK(is_optimized());
    ...
    TranslationArrayIterator it(data.TranslationByteArray(), data.TranslationIndex(deopt_index).value());
    ...
    while (jsframe_count != 0) {
        ...
        if (opcode == TranslationOpcode::INTERPRETED_FRAME || opcode == TranslationOpcode::JAVA_SCRIPT_BUILTIN_CONTINUATION_FRAME
            || opcode == TranslationOpcode::JAVA_SCRIPT_BUILTIN_CONTINUATION_WITH_CATCH_FRAME) {
...
int OptimizedFrame::StackSlotOffsetRelativeToFp(int slot_index) { return StandardFrameConstants::kCallerSPOffset - ((slot_index + 1) * kSystemPointerSize); }
...
int UnoptimizedFrame::LookupExceptionHandlerInTable(int* context_register, HandlerTable::CatchPrediction* prediction)
{
...
{
    DCHECK(functions->empty());
    Handle<AbstractCode> abstract_code(AbstractCode::cast(GetBytecodeArray()), isolate());
    Handle<FixedArray> params = GetParameters();
...
void BaselineFrame::PatchContext(Context value) { base::Memory<Address>(fp() + BaselineFrameConstants::kContextOffset) = value.ptr(); }
...
int BuiltinFrame::ComputeParametersCount() const
{
    ...
    return Smi::ToInt(Object(base::Memory<Address>(fp() + offset))) - kJSArgcReceiverSlots;
}
...
void WasmFrame::Print(StringStream* accumulator, PrintMode mode, int index) const
{
    ...
    accumulator->Add("], function #%u ('%s'), pc=%p (+0x%x), pos=%d (+%d)\n", func_index, func_name, reinterpret_cast<void*>(pc()), static_cast<int>(pc() - instruction_start), pos,
        pos - func_code_offset);
    if (mode != OVERVIEW)
        accumulator->Add("\n");
}

wasm::WasmCode* WasmFrame::wasm_code() const { return wasm::GetWasmCodeManager()->LookupCode(pc()); }
...
wasm::NativeModule* WasmFrame::native_module() const { return module_object().native_module(); }

WasmModuleObject WasmFrame::module_object() const { return wasm_instance().module_object(); }
...
void WasmFrame::Summarize(std::vector<FrameSummary>* functions) const
{
    ...
    FrameSummary::WasmFrameSummary summary(isolate(), instance, code, offset, at_to_number_conversion());
    functions->push_back(summary);
}
...
bool WasmFrame::at_to_number_conversion() const
{
    ...
    wasm::WasmCode* code = callee_pc() != kNullAddress ? wasm::GetWasmCodeManager()->LookupCode(callee_pc()) : nullptr;
...
void JsToWasmFrame::Iterate(RootVisitor* v) const
{
    ...
    intptr_t scan_count = *reinterpret_cast<intptr_t*>(fp() + BuiltinWasmWrapperConstants::kGCScanSlotCountOffset);
...
void StackSwitchFrame::Iterate(RootVisitor* v) const
{
    ...
    FullObjectSlot suspender_slot(&Memory<Address>(fp() + BuiltinWasmWrapperConstants::kSuspenderOffset));
...
{
    Object func_index(Memory<Address>(sp() + WasmCompileLazyFrameConstants::kFunctionIndexOffset));
    return Smi::ToInt(func_index);
}

wasm::NativeModule* WasmCompileLazyFrame::GetNativeModule() const { return *reinterpret_cast<wasm::NativeModule**>(sp() + WasmCompileLazyFrameConstants::kNativeModuleOffset); }

FullObjectSlot WasmCompileLazyFrame::wasm_instance_slot() const { return FullObjectSlot(&Memory<Address>(sp() + WasmCompileLazyFrameConstants::kWasmInstanceOffset)); }

void WasmCompileLazyFrame::Iterate(RootVisitor* v) const
{
    FullObjectSlot spilled_instance_slot(&Memory<Address>(fp() + WasmCompileLazyFrameConstants::kInstanceSpillOffset));
    v->VisitRootPointer(Root::kStackRoots, "spilled wasm instance", spilled_instance_slot);
    v->VisitRootPointer(Root::kStackRoots, "wasm instance parameter", wasm_instance_slot());
    ...
    int num_int_params_in_registers = std::min(num_int_params, WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs);
    int num_ref_params_in_registers = std::min(num_ref_params, WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs - num_int_params_in_registers);

    for (int i = 0; i < num_ref_params_in_registers; ++i) {
        FullObjectSlot spill_slot(fp() + WasmCompileLazyFrameConstants::kParameterSpillsOffset[num_int_params_in_registers + i]);
...
void JavaScriptFrame::Print(StringStream* accumulator, PrintMode mode, int index) const
{
    ...
    if (script_obj.IsScript()) {
        ...
        if (is_interpreted()) {
            ...
            int source_pos = AbstractCode::cast(bytecodes).SourcePosition(isolate(), offset);
            int line = script.GetLineNumber(source_pos) + 1;
...
void EntryFrame::Iterate(RootVisitor* v) const { IteratePc(v, pc_address(), constant_pool_address(), LookupCodeT()); }
...
InnerPointerToCodeCache::InnerPointerToCodeCacheEntry* InnerPointerToCodeCache::GetCacheEntry(Address inner_pointer)
{
    ...
    if (entry->inner_pointer == inner_pointer) {
        if (DEBUG_BOOL) {
            CodeLookupResult lookup_result = isolate_->heap()->GcSafeFindCodeForInnerPointer(inner_pointer);
            CHECK(lookup_result.IsFound());
...
constexpr int TopOfStackRegisterPaddingSlots() { return ArgumentPaddingSlots(1); }
...
UnoptimizedFrameInfo::UnoptimizedFrameInfo(int parameters_count_with_receiver, int translation_height, bool is_topmost, bool pad_arguments, FrameInfoKind frame_info_kind)
{
...
// static
uint32_t UnoptimizedFrameInfo::GetStackSizeForAdditionalArguments(int parameters_count) { return (parameters_count + ArgumentPaddingSlots(parameters_count)) * kSystemPointerSize; }
...
ConstructStubFrameInfo::ConstructStubFrameInfo(int translation_height, bool is_topmost, FrameInfoKind frame_info_kind)
{
    ...
    const int adjusted_height = (is_topmost || frame_info_kind == FrameInfoKind::kConservative) ? parameters_count + argument_padding + kTheResult + kTopOfStackPadding
...
BuiltinContinuationFrameInfo::BuiltinContinuationFrameInfo(int translation_height, const CallInterfaceDescriptor& continuation_descriptor,
    const RegisterConfiguration* register_config, bool is_topmost, DeoptimizeKind deopt_kind, BuiltinContinuationMode continuation_mode, FrameInfoKind frame_info_kind)
{
    ...
    frame_size_in_bytes_ = kSystemPointerSize * (stack_parameter_count_ + stack_param_pad_count + allocatable_register_count + padding_slot_count + push_result_count)
