...
const uint8_t* DefaultEmbeddedBlobCode() { return v8_Default_embedded_blob_code_; }
uint32_t DefaultEmbeddedBlobCodeSize() { return v8_Default_embedded_blob_code_size_; }
const uint8_t* DefaultEmbeddedBlobData() { return v8_Default_embedded_blob_data_; }
uint32_t DefaultEmbeddedBlobDataSize() { return v8_Default_embedded_blob_data_size_; }
...
uint32_t StickyEmbeddedBlobCodeSize() { return sticky_embedded_blob_code_size_; }
const uint8_t* StickyEmbeddedBlobData() { return sticky_embedded_blob_data_; }
uint32_t StickyEmbeddedBlobDataSize() { return sticky_embedded_blob_data_size_; }
...
void FreeCurrentEmbeddedBlob()
{
    ...
    OffHeapInstructionStream::FreeOffHeapOffHeapInstructionStream(const_cast<uint8_t*>(Isolate::CurrentEmbeddedBlobCode()), Isolate::CurrentEmbeddedBlobCodeSize(),
        const_cast<uint8_t*>(Isolate::CurrentEmbeddedBlobData()), Isolate::CurrentEmbeddedBlobDataSize());
...
void Isolate::SetEmbeddedBlob(const uint8_t* code, uint32_t code_size, const uint8_t* data, uint32_t data_size)
{
    ...
    //sprintf_s(output, 399, "hash1: %p, hash2: %p, code_size:%d, data_size:%d, codeHash:%d, dataHash:%d\n",
    //  d.EmbeddedBlobDataHash(), d.CreateEmbeddedBlobDataHash(), code_size, data_size,
    //  RSHash((const char*)code, code_size), RSHash((const char*)data, data_size));
    // 
    //sprintf(output, "Isolate::SetEmbeddedBlob: %p, %d\n", d.data(), d.data_size());
    //OutputDebugStringA(output);
    //free(output);
    printf("Isolate::SetEmbeddedBlob: %p, %d\n", d.data(), d.data_size());
...
const uint8_t* Isolate::embedded_blob_code() const { return embedded_blob_code_; }
uint32_t Isolate::embedded_blob_code_size() const { return embedded_blob_code_size_; }
const uint8_t* Isolate::embedded_blob_data() const { return embedded_blob_data_; }
uint32_t Isolate::embedded_blob_data_size() const { return embedded_blob_data_size_; }

// static
const uint8_t* Isolate::CurrentEmbeddedBlobCode() { return current_embedded_blob_code_.load(std::memory_order_relaxed); }

// static
uint32_t Isolate::CurrentEmbeddedBlobCodeSize() { return current_embedded_blob_code_size_.load(std::memory_order_relaxed); }

// static
const uint8_t* Isolate::CurrentEmbeddedBlobData() { return current_embedded_blob_data_.load(std::memory_order_relaxed); }

// static
uint32_t Isolate::CurrentEmbeddedBlobDataSize() { return current_embedded_blob_data_size_.load(std::memory_order_relaxed); }
...
size_t Isolate::HashIsolateForEmbeddedBlob()
{
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        if (V8_REMOVE_BUILTINS_CODE_OBJECTS) {
            ...
            // remain the same.
            static_assert(CodeDataContainer::kCodePointerFieldsStrongEndOffset == CodeDataContainer::kCodeCageBaseUpper32BitsOffset);
            static_assert(CodeDataContainer::kCodeCageBaseUpper32BitsOffsetEnd + 1 == CodeDataContainer::kCodeEntryPointOffset);

            static_assert(CodeDataContainer::kCodeEntryPointOffsetEnd + 1 == CodeDataContainer::kFlagsOffset);
            static_assert(CodeDataContainer::kFlagsOffsetEnd + 1 == CodeDataContainer::kBuiltinIdOffset);
            static_assert(CodeDataContainer::kBuiltinIdOffsetEnd + 1 == CodeDataContainer::kKindSpecificFlagsOffset);
            static_assert(CodeDataContainer::kKindSpecificFlagsOffsetEnd + 1 == CodeDataContainer::kUnalignedSize);
            ...
            static_assert(Code::kInstructionSizeOffset == Code::kMainCageBaseUpper32BitsOffsetEnd + 1);
#else
            static_assert(Code::kInstructionSizeOffset == Code::kDataStart);
#endif // V8_EXTERNAL_CODE_SPACE
            static_assert(Code::kMetadataSizeOffset == Code::kInstructionSizeOffsetEnd + 1);
...
base::LazyMutex Isolate::process_wide_shared_isolate_mutex_ = LAZY_MUTEX_INITIALIZER;
...
void Isolate::InitializeOncePerProcess()
{
    ...
    bool expected = false;
    CHECK(isolate_key_created_.compare_exchange_strong(expected, true, std::memory_order_relaxed));
...
void Isolate::DisposeOncePerProcess()
{
    ...
    bool expected = true;
    CHECK(isolate_key_created_.compare_exchange_strong(expected, false, std::memory_order_relaxed));
    base::Thread::DeleteThreadLocalKey(per_isolate_thread_data_key_);
}

Address Isolate::get_address_from_id(IsolateAddressId id) { return isolate_addresses_[id]; }
...
void Isolate::Iterate(RootVisitor* v, ThreadLocalTop* thread)
{
    ...
    v->VisitRootPointer(Root::kStackRoots, nullptr, FullObjectSlot(&thread->pending_exception_));
    ...
    v->VisitRootPointer(Root::kStackRoots, nullptr, FullObjectSlot(&thread->scheduled_exception_));
...
void Isolate::RegisterTryCatchHandler(v8::TryCatch* that) { thread_local_top()->try_catch_handler_ = that; }
...
void Isolate::PushStackTraceAndDie(void* ptr1, void* ptr2, void* ptr3, void* ptr4)
{
    StackTraceFailureMessage message(this, StackTraceFailureMessage::kIncludeStackTrace, ptr1, ptr2, ptr3, ptr4);
    message.Print();
    base::OS::Abort();
}
...
public:
    ...
    void AppendAsyncFrame(Handle<JSGeneratorObject> generator_object)
    {
        ...
        Handle<BytecodeArray> code(function->shared().GetBytecodeArray(isolate_), isolate_);
        // The stored bytecode offset is relative to a different base than what
        // is used in the source position table, hence the subtraction.
        int offset = Smi::ToInt(generator_object->input_or_debug_pos()) - (BytecodeArray::kHeaderSize - kHeapObjectTag);
    ...
    void AppendPromiseCombinatorFrame(Handle<JSFunction> element_function, Handle<JSFunction> combinator)
    {
        if (!IsVisibleInStackTrace(combinator))
            return;
        int flags = CallSiteInfo::kIsAsync | CallSiteInfo::kIsSourcePositionComputed;

        Handle<Object> receiver(combinator->native_context().promise_function(), isolate_);
    ...
    void AppendJavaScriptFrame(FrameSummary::JavaScriptFrameSummary const& summary)
    {
        ...
        AppendFrame(summary.receiver(), function, summary.abstract_code(), summary.code_offset(), flags, summary.parameters());
    }
    ...
    void AppendWasmFrame(FrameSummary::WasmFrameSummary const& summary)
    {
        ...
        auto code = Managed<wasm::GlobalWasmCodeRef>::Allocate(isolate_, 0, summary.code(), instance->module_object().shared_native_module());
        AppendFrame(instance, handle(Smi::FromInt(summary.function_index()), isolate_), code, summary.code_offset(), flags, isolate_->factory()->empty_fixed_array());
    }
#endif // V8_ENABLE_WEBASSEMBLY

    bool Full() { return index_ >= limit_; }

    Handle<FixedArray> Build() { return FixedArray::ShrinkOrEmpty(isolate_, elements_, index_); }
...
private:
    ...
    // trace.
    bool IsVisibleInStackTrace(Handle<JSFunction> function) { return ShouldIncludeFrame(function) && IsNotHidden(function); }
    ...
    void AppendFrame(Handle<Object> receiver_or_instance, Handle<Object> function, Handle<HeapObject> code, int offset, int flags, Handle<FixedArray> parameters)
    {
        ...
        auto info = isolate_->factory()->NewCallSiteInfo(receiver_or_instance, function, code, offset, flags, parameters);
...
bool GetStackTraceLimit(Isolate* isolate, int* result)
{
    ...
    Handle<Object> stack_trace_limit = JSReceiver::GetDataProperty(isolate, error, key);
...
void CaptureAsyncStackTrace(Isolate* isolate, Handle<JSPromise> promise, CallSiteBuilder* builder)
{
    while (!builder->Full()) {
        ...
        if (!promise->reactions().IsPromiseReaction())
            return;
        Handle<PromiseReaction> reaction(PromiseReaction::cast(promise->reactions()), isolate);
        if (!reaction->next().IsSmi())
            return;

        // Check if the {reaction} has one of the known async function or
        // async generator continuations as its fulfill handler.
        if (IsBuiltinFunction(isolate, reaction->fulfill_handler(), Builtin::kAsyncFunctionAwaitResolveClosure)
            || IsBuiltinFunction(isolate, reaction->fulfill_handler(), Builtin::kAsyncGeneratorAwaitResolveClosure)
            || IsBuiltinFunction(isolate, reaction->fulfill_handler(), Builtin::kAsyncGeneratorYieldWithAwaitResolveClosure)) {
            // Now peek into the handlers' AwaitContext to get to
            // the JSGeneratorObject for the async function.
            Handle<Context> context(JSFunction::cast(reaction->fulfill_handler()).context(), isolate);
            Handle<JSGeneratorObject> generator_object(JSGeneratorObject::cast(context->extension()), isolate);
            ...
            if (generator_object->IsJSAsyncFunctionObject()) {
                ...
                if (async_generator_object->queue().IsUndefined(isolate))
                    return;
                Handle<AsyncGeneratorRequest> async_generator_request(AsyncGeneratorRequest::cast(async_generator_object->queue()), isolate);
                promise = handle(JSPromise::cast(async_generator_request->promise()), isolate);
            }
        } else if (IsBuiltinFunction(isolate, reaction->fulfill_handler(), Builtin::kPromiseAllResolveElementClosure)) {
            Handle<JSFunction> function(JSFunction::cast(reaction->fulfill_handler()), isolate);
            Handle<Context> context(function->context(), isolate);
            Handle<JSFunction> combinator(context->native_context().promise_all(), isolate);
            ...
            Handle<PromiseCapability> capability(PromiseCapability::cast(context->get(index)), isolate);
            ...
            Handle<JSFunction> function(JSFunction::cast(reaction->fulfill_handler()), isolate);
            Handle<Context> context(function->context(), isolate);
            Handle<JSFunction> combinator(context->native_context().promise_all_settled(), isolate);
            builder->AppendPromiseCombinatorFrame(function, combinator);
            ...
            Handle<JSFunction> function(JSFunction::cast(reaction->reject_handler()), isolate);
            Handle<Context> context(function->context(), isolate);
            Handle<JSFunction> combinator(context->native_context().promise_any(), isolate);
            builder->AppendPromiseCombinatorFrame(function, combinator);
            ...
            Handle<JSFunction> function(JSFunction::cast(reaction->fulfill_handler()), isolate);
            Handle<Context> context(function->context(), isolate);
            promise = handle(JSPromise::cast(context->get(PromiseBuiltins::kPromiseSlot)), isolate);
        } else {
...
void CaptureAsyncStackTrace(Isolate* isolate, CallSiteBuilder* builder)
{
    ...
    if (current_microtask->IsPromiseReactionJobTask()) {
        ...
        // Check if the {reaction} has one of the known async function or
        // async generator continuations as its fulfill handler.
        if (IsBuiltinFunction(isolate, promise_reaction_job_task->handler(), Builtin::kAsyncFunctionAwaitResolveClosure)
            || IsBuiltinFunction(isolate, promise_reaction_job_task->handler(), Builtin::kAsyncGeneratorAwaitResolveClosure)
            || IsBuiltinFunction(isolate, promise_reaction_job_task->handler(), Builtin::kAsyncGeneratorYieldWithAwaitResolveClosure)
            || IsBuiltinFunction(isolate, promise_reaction_job_task->handler(), Builtin::kAsyncFunctionAwaitRejectClosure)
            || IsBuiltinFunction(isolate, promise_reaction_job_task->handler(), Builtin::kAsyncGeneratorAwaitRejectClosure)) {
            // Now peek into the handlers' AwaitContext to get to
            // the JSGeneratorObject for the async function.
            Handle<Context> context(JSFunction::cast(promise_reaction_job_task->handler()).context(), isolate);
            Handle<JSGeneratorObject> generator_object(JSGeneratorObject::cast(context->extension()), isolate);
            if (generator_object->is_executing()) {
                if (generator_object->IsJSAsyncFunctionObject()) {
                    ...
                    if (!queue->IsUndefined(isolate)) {
                        Handle<AsyncGeneratorRequest> async_generator_request = Handle<AsyncGeneratorRequest>::cast(queue);
                        Handle<JSPromise> promise(JSPromise::cast(async_generator_request->promise()), isolate);
                        CaptureAsyncStackTrace(isolate, promise, builder);
                    }
                }
            }
...
MaybeHandle<JSObject> Isolate::CaptureAndSetErrorStack(Handle<JSObject> error_object, FrameSkipMode mode, Handle<Object> caller)
{
    ...
    if (capture_stack_trace_for_uncaught_exceptions_) {
        ...
        if (error_stack->IsUndefined(this) || (stack_trace_for_uncaught_exceptions_options_ & StackTrace::kExposeFramesAcrossSecurityOrigins)) {
            limit_or_stack_frame_infos = CaptureDetailedStackTrace(stack_trace_for_uncaught_exceptions_frame_limit_, stack_trace_for_uncaught_exceptions_options_);
        } else {
            int limit = stack_trace_limit > stack_trace_for_uncaught_exceptions_frame_limit_ ? -stack_trace_for_uncaught_exceptions_frame_limit_ : stack_trace_limit;
            limit_or_stack_frame_infos = handle(Smi::FromInt(limit), this);
        }
        error_stack = factory()->NewErrorStackData(error_stack, limit_or_stack_frame_infos);
    }

    RETURN_ON_EXCEPTION(
        this, JSObject::SetProperty(this, error_object, factory()->error_stack_symbol(), error_stack, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)), JSObject);
    return error_object;
}

Handle<FixedArray> Isolate::GetDetailedStackTrace(Handle<JSReceiver> error_object)
{
    Handle<Object> error_stack = JSReceiver::GetDataProperty(this, error_object, factory()->error_stack_symbol());
    if (!error_stack->IsErrorStackData()) {
        return Handle<FixedArray>();
    }
    Handle<ErrorStackData> error_stack_data = Handle<ErrorStackData>::cast(error_stack);
    ErrorStackData::EnsureStackFrameInfos(this, error_stack_data);
    if (!error_stack_data->limit_or_stack_frame_infos().IsFixedArray()) {
        return Handle<FixedArray>();
    }
    return handle(FixedArray::cast(error_stack_data->limit_or_stack_frame_infos()), this);
}

Handle<FixedArray> Isolate::GetSimpleStackTrace(Handle<JSReceiver> error_object)
{
    Handle<Object> error_stack = JSReceiver::GetDataProperty(this, error_object, factory()->error_stack_symbol());
...
public:
    ...
    Handle<FixedArray> Build() { return FixedArray::ShrinkOrEmpty(isolate_, frames_, index_); }
...
Handle<FixedArray> Isolate::CaptureDetailedStackTrace(int limit, StackTrace::StackTraceOptions options)
{
    TRACE_EVENT_BEGIN1(TRACE_DISABLED_BY_DEFAULT("v8.stack_trace"), __func__, "maxFrameCount", limit);
...
public:
    ...
    bool Visit(FrameSummary& summary)
    {
        ...
        Handle<Object> name_or_url_obj = handle(script->GetNameOrSourceURL(), isolate_);
        if (!name_or_url_obj->IsString())
            return true;
...
void Isolate::SetFailedAccessCheckCallback(v8::FailedAccessCheckCallback callback) { thread_local_top()->failed_access_check_callback_ = callback; }
...
bool Isolate::MayAccess(Handle<Context> accessing_context, Handle<JSObject> receiver)
{
...
Object Isolate::StackOverflow()
{
    ...
    Handle<Object> msg = factory()->NewStringFromAsciiChecked(MessageFormatter::TemplateString(MessageTemplate::kStackOverflow));
    Handle<Object> options = factory()->undefined_value();
    Handle<Object> no_caller;
    Handle<JSObject> exception;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(this, exception, ErrorUtils::Construct(this, fun, fun, msg, options, SKIP_NONE, no_caller, ErrorUtils::StackTraceCollection::kEnabled));
...
Object Isolate::ThrowAt(Handle<JSObject> exception, MessageLocation* location)
{
    ...
    Object::SetProperty(this, exception, key_start_pos, handle(Smi::FromInt(location->start_pos()), this), StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)).Check();

    Handle<Name> key_end_pos = factory()->error_end_pos_symbol();
    Object::SetProperty(this, exception, key_end_pos, handle(Smi::FromInt(location->end_pos()), this), StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)).Check();
...
Object Isolate::TerminateExecution() { return Throw(ReadOnlyRoots(this).termination_exception()); }
...
void ReportBootstrappingException(Handle<Object> exception, MessageLocation* location)
{
    ...
    if (exception->IsString() && location->script()->name().IsString()) {
        base::OS::PrintError("Extension or internal compilation error: %s in %s at line %d.\n", String::cast(*exception).ToCString().get(),
            String::cast(location->script()->name()).ToCString().get(), line_number);
    } else if (location->script()->name().IsString()) {
        base::OS::PrintError("Extension or internal compilation error in %s at line %d.\n", String::cast(location->script()->name()).ToCString().get(), line_number);
    ...
    if (location->script()->source().IsString()) {
        Handle<String> src(String::cast(location->script()->source()), location->script()->GetIsolate());
...
// initialize the line_ends array, so be careful when calling them.
            ...
            if ((false)) {
#endif
                PrintF(", %d:%d - %d:%d\n", Script::GetLineNumber(script, location->start_pos()) + 1, Script::GetColumnNumber(script, location->start_pos()),
                    Script::GetLineNumber(script, location->end_pos()) + 1, Script::GetColumnNumber(script, location->end_pos()));
...
public:
    SetThreadInWasmFlagScope() { DCHECK_IMPLIES(trap_handler::IsTrapHandlerEnabled(), !trap_handler::IsThreadInWasm()); }
...
Object Isolate::UnwindAndFindHandler()
{
    ...
    auto FoundHandler = [&](Context context, Address instruction_start, intptr_t handler_offset, Address constant_pool_address, Address handler_sp, Address handler_fp,
    ...
    if (v8_flags.experimental_wasm_stack_switching) {
        current_stack = WasmContinuationObject::cast(root(RootIndex::kActiveContinuation));
    }
#endif
    ...
    for (StackFrameIterator iter(this);; iter.Advance(), visited_frames++) {
        ...
        if (v8_flags.experimental_wasm_stack_switching && iter.done()) {
            ...
            wasm::StackMemory* parent_stack = Managed<wasm::StackMemory>::cast(current_stack.stack()).get().get();
            ...
            WasmSuspenderObject suspender = WasmSuspenderObject::cast(root(RootIndex::kActiveSuspender));
            ...
            uintptr_t limit = reinterpret_cast<uintptr_t>(parent_stack->jmpbuf()->stack_limit);
        ...
        if (debug()->ShouldRestartFrame(frame->id())) {
            ...
            return FoundHandler(Context(), code.InstructionStart(), 0, code.constant_pool(), kNullAddress, frame->fp(), visited_frames);
        }
        ...
        case StackFrame::CONSTRUCT_ENTRY: {
            ...
            return FoundHandler(Context(), code.InstructionStart(this, frame->pc()), table.LookupReturn(0), code.constant_pool(), handler->address() + StackHandlerConstants::kSize,
                0, visited_frames);
        }
        ...
        case StackFrame::WASM: {
            ...
            wasm::WasmCode* wasm_code = wasm::GetWasmCodeManager()->LookupCode(frame->pc());
            ...
            // Compute the stack pointer from the frame pointer. This ensures that
            // argument slots on the stack are dropped as returning would.
            Address return_sp = frame->fp() + StandardFrameConstants::kFixedFrameSizeAboveFp - wasm_code->stack_slots() * kSystemPointerSize;
        ...
        case StackFrame::STUB: {
            ...
            if (code.kind() != CodeKind::BUILTIN || !code.is_turbofanned() || !code.has_handler_table()) {
                break;
            }
        ...
        case StackFrame::BASELINE: {
            ...
            if (frame->is_baseline()) {
                ...
                return FoundHandler(Context(), code.InstructionStart(), pc_offset, code.constant_pool(), return_sp, sp_frame->fp(), visited_frames);
            } else {
                ...
                return FoundHandler(context, code.InstructionStart(), 0, code.constant_pool(), return_sp, frame->fp(), visited_frames - 1);
            }
        }
        ...
        case StackFrame::JAVA_SCRIPT_BUILTIN_CONTINUATION_WITH_CATCH: {
            ...
            return FoundHandler(Context(), code.InstructionStart(), 0, code.constant_pool(), return_sp, frame->fp(), visited_frames);
        }
        ...
        if (frame->is_turbofan()) {
            ...
            // marked for deopt.
            DCHECK_IMPLIES(removed, frame->LookupCodeT().ToCodeT().marked_for_deoptimization());
        }
    }
...
Isolate::CatchType Isolate::PredictExceptionCatcher()
{
    ...
    if (TopExceptionHandlerType(Object()) == ExceptionHandlerType::kExternalTryCatch) {
...
void Isolate::ScheduleThrow(Object exception)
{
    ...
    Throw(exception);
    PropagatePendingExceptionToExternalTryCatch(TopExceptionHandlerType(pending_exception()));
...
void Isolate::CancelScheduledExceptionFromTryCatch(v8::TryCatch* handler)
{
    ...
    if (reinterpret_cast<void*>(thread_local_top()->pending_message_.ptr()) == handler->message_obj_) {
...
{
    Handle<FixedArray> frames = CaptureSimpleStackTrace(this, FixedArray::kMaxLength, SKIP_NONE, factory()->undefined_value());
...
bool Isolate::ComputeLocation(MessageLocation* target)
{
    ...
    if (summary.AreSourcePositionsAvailable()) {
        ...
        *target = MessageLocation(Handle<Script>::cast(script), shared, summary.code_offset());
    }
    return true;
}
...
bool Isolate::ComputeLocationFromException(MessageLocation* target, Handle<Object> exception)
{
    ...
    Handle<Object> start_pos = JSReceiver::GetDataProperty(this, Handle<JSObject>::cast(exception), start_pos_symbol);
    if (!start_pos->IsSmi())
        return false;
    int start_pos_value = Handle<Smi>::cast(start_pos)->value();

    Handle<Name> end_pos_symbol = factory()->error_end_pos_symbol();
    Handle<Object> end_pos = JSReceiver::GetDataProperty(this, Handle<JSObject>::cast(exception), end_pos_symbol);
    if (!end_pos->IsSmi())
        return false;
    int end_pos_value = Handle<Smi>::cast(end_pos)->value();

    Handle<Name> script_symbol = factory()->error_script_symbol();
    Handle<Object> script = JSReceiver::GetDataProperty(this, Handle<JSObject>::cast(exception), script_symbol);
    if (!script->IsScript())
        return false;
...
bool Isolate::ComputeLocationFromSimpleStackTrace(MessageLocation* target, Handle<Object> exception)
{
    ...
    Handle<FixedArray> call_site_infos = GetSimpleStackTrace(Handle<JSReceiver>::cast(exception));
    for (int i = 0; i < call_site_infos->length(); ++i) {
        Handle<CallSiteInfo> call_site_info(CallSiteInfo::cast(call_site_infos->get(i)), this);
...
bool Isolate::ComputeLocationFromDetailedStackTrace(MessageLocation* target, Handle<Object> exception)
{
    ...
    Handle<FixedArray> stack_frame_infos = GetDetailedStackTrace(Handle<JSReceiver>::cast(exception));
    if (stack_frame_infos.is_null() || stack_frame_infos->length() == 0) {
        return false;
    }

    Handle<StackFrameInfo> info(StackFrameInfo::cast(stack_frame_infos->get(0)), this);
...
Handle<JSMessageObject> Isolate::CreateMessage(Handle<Object> exception, MessageLocation* location)
{
    ...
    if (capture_stack_trace_for_uncaught_exceptions_) {
        ...
        if (stack_trace_object.is_null()) {
            ...
            stack_trace_object = CaptureDetailedStackTrace(stack_trace_for_uncaught_exceptions_frame_limit_, stack_trace_for_uncaught_exceptions_options_);
        }
    }
    MessageLocation computed_location;
    if (location == nullptr
    ...
    return MessageHandler::MakeMessageObject(this, MessageTemplate::kUncaughtException, location, exception, stack_trace_object);
}
...
Handle<JSMessageObject> Isolate::CreateMessageFromException(Handle<Object> exception)
{
    ...
    return MessageHandler::MakeMessageObject(this, MessageTemplate::kPlaceholderOnly, location, exception, stack_trace_object);
}

Isolate::ExceptionHandlerType Isolate::TopExceptionHandlerType(Object exception)
{
...
std::vector<MemoryRange>* Isolate::GetCodePages() const { return code_pages_.load(std::memory_order_acquire); }

void Isolate::SetCodePages(std::vector<MemoryRange>* new_code_pages) { code_pages_.store(new_code_pages, std::memory_order_release); }
...
void Isolate::ReportPendingMessages()
{
    ...
    bool has_been_propagated = PropagatePendingExceptionToExternalTryCatch(top_handler);
...
void Isolate::PopPromise()
{
    if (!IsPromiseStackEmpty()) {
        debug()->thread_local_.promise_stack_ = PromiseOnStack::cast(debug()->thread_local_.promise_stack_).prev();
    }
}

bool Isolate::IsPromiseStackEmpty() const
{
    DCHECK_IMPLIES(!debug()->thread_local_.promise_stack_.IsSmi(), debug()->thread_local_.promise_stack_.IsPromiseOnStack());
...
bool PromiseIsRejectHandler(Isolate* isolate, Handle<JSReceiver> handler)
{
    ...
    Handle<Object> forwarding_handler = JSReceiver::GetDataProperty(isolate, handler, key);
    return forwarding_handler->IsUndefined(isolate);
}

bool PromiseHasUserDefinedRejectHandlerInternal(Isolate* isolate, Handle<JSPromise> promise)
{
    ...
    while (!current->IsSmi()) {
        ...
        if (!promise_or_capability->IsUndefined(isolate)) {
            if (!promise_or_capability->IsJSPromise()) {
                promise_or_capability = handle(Handle<PromiseCapability>::cast(promise_or_capability)->promise(), isolate);
            }
            if (promise_or_capability->IsJSPromise()) {
                promise = Handle<JSPromise>::cast(promise_or_capability);
                if (!reaction->reject_handler().IsUndefined(isolate)) {
                    Handle<JSReceiver> reject_handler(JSReceiver::cast(reaction->reject_handler()), isolate);
                    if (PromiseIsRejectHandler(isolate, reject_handler))
                        return true;
                }
                if (isolate->PromiseHasUserDefinedRejectHandler(promise))
                    return true;
            }
        }
...
bool Isolate::PromiseHasUserDefinedRejectHandler(Handle<JSPromise> promise)
{
    ...
    while (true) {
        ...
        Handle<Object> outer_promise_obj = JSObject::GetDataProperty(this, promise, key);
    ...
    while (!promises.empty()) {
        ...
        if (PromiseHasUserDefinedRejectHandlerInternal(this, promise))
            return true;
        promises.pop();
    }
    return false;
...
Handle<Object> Isolate::GetPromiseOnStackOnThrow()
{
    ...
    for (StackFrameIterator it(this); !it.done(); it.Advance()) {
        ...
        case HandlerTable::PROMISE: {
            ...
            if (promise_stack->IsPromiseOnStack() && PromiseOnStack::GetPromise(Handle<PromiseOnStack>::cast(promise_stack)).ToHandle(&promise)) {
                return promise;
            }
            return undefined;
        }
        ...
        case HandlerTable::ASYNC_AWAIT: {
            ...
            Handle<PromiseOnStack> promise_on_stack = Handle<PromiseOnStack>::cast(promise_stack);
            if (!PromiseOnStack::GetPromise(promise_on_stack).ToHandle(&retval)) {
                return retval;
            }
            if (retval->IsJSPromise()) {
                if (PromiseHasUserDefinedRejectHandler(Handle<JSPromise>::cast(retval))) {
                    return retval;
                }
            }
...
bool Isolate::get_capture_stack_trace_for_uncaught_exceptions() const { return capture_stack_trace_for_uncaught_exceptions_; }

void Isolate::SetAbortOnUncaughtExceptionCallback(v8::Isolate::AbortOnUncaughtExceptionCallback callback) { abort_on_uncaught_exception_callback_ = callback; }
...
Handle<Context> Isolate::GetIncumbentContext()
{
    ...
    if (top_backup_incumbent_scope()) {
        return Utils::OpenHandle(*top_backup_incumbent_scope()->backup_incumbent_context_);
    }
...
void Isolate::AddSharedWasmMemory(Handle<WasmMemoryObject> memory_object)
{
    ...
    shared_wasm_memories = WeakArrayList::AddToEnd(this, shared_wasm_memories, MaybeObjectHandle::Weak(memory_object));
...
private:
    ...
    {
        if (!v8_flags.trace_zone_stats && !(TracingFlags::zone_stats.load(std::memory_order_relaxed) & v8::tracing::TracingCategoryObserver::ENABLED_BY_TRACING)) {
...

        {
            ...
            if (V8_UNLIKELY(TracingFlags::zone_stats.load(std::memory_order_relaxed) & v8::tracing::TracingCategoryObserver::ENABLED_BY_TRACING)) {
                TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("v8.zone_stats"), "V8.Zone_Stats", TRACE_EVENT_SCOPE_THREAD, "stats", TRACE_STR_COPY(trace_str.c_str()));
            }
        }
...
namespace {
bool HasFlagThatRequiresSharedHeap() { return v8_flags.shared_string_table || v8_flags.harmony_struct; }
} // namespace
...
Isolate* Isolate::GetProcessWideSharedIsolate(bool* created_shared_isolate)
{
    if (!V8_CAN_CREATE_SHARED_HEAP_BOOL) {
        ...
        FATAL("Build configuration does not support creating shared heap. The RO "
            ...
            !COMPRESS_POINTERS_BOOL ? "NOT COMPRESSED" : (COMPRESS_POINTERS_IN_SHARED_CAGE_BOOL ? "COMPRESSED IN SHARED CAGE" : "COMPRESSED IN PER-ISOLATE CAGE"),
    ...
    if (process_wide_shared_isolate_ == nullptr) {
        ...
        size_t initial_shared_heap_size = static_cast<size_t>(v8_flags.initial_shared_heap_size) * MB;
        size_t max_shared_heap_size = static_cast<size_t>(v8_flags.max_shared_heap_size) * MB;
        if (initial_shared_heap_size != 0 && max_shared_heap_size != 0) {
            params.constraints.ConfigureDefaultsFromHeapSize(initial_shared_heap_size, max_shared_heap_size);
        } else {
        ...
        v8::Isolate::Initialize(reinterpret_cast<v8::Isolate*>(process_wide_shared_isolate_), params);
...
Isolate* Isolate::Allocate(bool is_shared)
{
    ...
    std::unique_ptr<IsolateAllocator> isolate_allocator = std::make_unique<IsolateAllocator>();
    // Construct Isolate object in the allocated memory.
    void* isolate_ptr = isolate_allocator->isolate_memory();
    Isolate* isolate = new (isolate_ptr) Isolate(std::move(isolate_allocator), is_shared);
...
void Isolate::Delete(Isolate* isolate)
{
    ...
    Isolate* saved_isolate = reinterpret_cast<Isolate*>(base::Thread::GetThreadLocal(isolate->isolate_key_));
...
void Isolate::SetUpFromReadOnlyArtifacts(std::shared_ptr<ReadOnlyArtifacts> artifacts, ReadOnlyHeap* ro_heap)
{
...
v8::PageAllocator* Isolate::page_allocator() const { return isolate_allocator_->page_allocator(); }

Isolate::Isolate(std::unique_ptr<i::IsolateAllocator> isolate_allocator, bool is_shared)
    ...
    ,
#endif
    next_module_async_evaluating_ordinal_(SourceTextModule::kFirstAsyncEvaluatingOrdinal)
...
void Isolate::CheckIsolateLayout()
{
    ...
    CHECK_EQ(static_cast<int>(OFFSET_OF(ExternalPointerTable, buffer_)), Internals::kExternalPointerTableBufferOffset);
...
{
    log_object_relocation_ = v8_flags.verify_predictable || v8_file_logger()->is_logging() || is_profiling() || v8_file_logger()->is_listening_to_code_events()
...
#if defined(V8_OS_WIN64)
    if (win64_unwindinfo::CanRegisterUnwindInfoForNonABICompliantCodeRange() && heap()->memory_allocator() && RequiresCodeRange()
...
bool Isolate::PropagatePendingExceptionToExternalTryCatch(ExceptionHandlerType top_handler)
{
    Object exception = pending_exception();
    ...
    if (!is_catchable_by_javascript(exception)) {
        ...
        DCHECK(pending_message().IsJSMessageObject() || pending_message().IsTheHole(this));
...
void CreateOffHeapTrampolines(Isolate* isolate)
{
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        Handle<CodeT> trampoline = isolate->factory()->NewOffHeapTrampolineFor(builtins->code_handle(builtin), instruction_start);
...
void Isolate::TearDownEmbeddedBlob()
{
    ...
    if (current_embedded_blob_refs_ == 0 && enable_embedded_blob_refcounting_) {
        ...
        OffHeapInstructionStream::FreeOffHeapOffHeapInstructionStream(
            const_cast<uint8_t*>(CurrentEmbeddedBlobCode()), embedded_blob_code_size(), const_cast<uint8_t*>(CurrentEmbeddedBlobData()), embedded_blob_data_size());
        ClearEmbeddedBlob();
    }
}

bool Isolate::InitWithoutSnapshot() { return Init(nullptr, nullptr, nullptr, false); }

bool Isolate::InitWithSnapshot(SnapshotData* startup_snapshot_data, SnapshotData* read_only_snapshot_data, SnapshotData* shared_heap_snapshot_data, bool can_rehash)
{
...
void Isolate::AddCrashKeysForIsolateAndHeapPointers()
{
    ...
    if (heap()->map_space()) {
        const uintptr_t map_space_firstpage_address = heap()->map_space()->FirstPageAddress();
        add_crash_key_callback_(v8::CrashKeyId::kMapSpaceFirstPageAddress, ToHexString(map_space_firstpage_address));
    }
    ...
    add_crash_key_callback_(v8::CrashKeyId::kSnapshotChecksumCalculated, ToHexString(v8_snapshot_checkum_calculated));
    const uint32_t v8_snapshot_checkum_expected = Snapshot::GetExpectedChecksum(data);
    add_crash_key_callback_(v8::CrashKeyId::kSnapshotChecksumExpected, ToHexString(v8_snapshot_checkum_expected));
}
...
void ResetBeforeGC(v8::Isolate* v8_isolate, v8::GCType gc_type, v8::GCCallbackFlags flags, void* data)
{
...
VirtualMemoryCage* Isolate::GetPtrComprCodeCageForTesting() { return V8_EXTERNAL_CODE_SPACE_BOOL ? heap_.code_range() : GetPtrComprCage(); }
...
#undef ASSIGN_ELEMENT
...
#ifdef V8_EXTERNAL_CODE_SPACE
    if (heap_.code_range()) {
        code_cage_base_ = ExternalCodeCompressionScheme::GetPtrComprCageBaseAddress(heap_.code_range()->base());
    } else {
        CHECK(jitless_);
        // In jitless mode the code space pages will be allocated in the main
        // pointer compression cage.
        code_cage_base_ = ExternalCodeCompressionScheme::GetPtrComprCageBaseAddress(cage_base());
    }
#endif // V8_EXTERNAL_CODE_SPACE
    ...
    if (owns_shareable_data()) {
        ...
        DCHECK(has_shared_heap());
        isolate_data_.shared_external_pointer_table_ = shared_heap_isolate()->isolate_data_.shared_external_pointer_table_;
    }
#endif // V8_COMPRESS_POINTERS
...
//     EmbeddedData d = EmbeddedData::FromBlob(this);
//     char* output = (char*)malloc(400);
//     sprintf_s(output, 399, "Isolate::Init: %x, %x\n", d.IsolateHash(), this->HashIsolateForEmbeddedBlob());
//     OutputDebugStringA(output);
//     free(output);
...
#if defined(V8_OS_WIN64)
    if (win64_unwindinfo::CanRegisterUnwindInfoForNonABICompliantCodeRange() && heap()->code_range()->AtomicIncrementUnwindInfoUseCount() == 0) {
        const base::AddressRegion& code_region = heap()->code_region();
        void* start = reinterpret_cast<void*>(code_region.begin());
    ...
    if (v8_flags.experimental_wasm_stack_switching) {
        std::unique_ptr<wasm::StackMemory> stack(wasm::StackMemory::GetCurrentStackView(this));
        this->wasm_stacks() = stack.get();
        if (v8_flags.trace_wasm_stack_switching) {
            PrintF("Set up native stack object (limit: %p, base: %p)\n", stack->jslimit(), reinterpret_cast<void*>(stack->base()));
        }
        HandleScope scope(this);
        Handle<WasmContinuationObject> continuation = WasmContinuationObject::New(this, std::move(stack), wasm::JumpBuffer::Active, AllocationType::kOld);
        heap()->roots_table().slot(RootIndex::kActiveContinuation).store(*continuation);
    }
#endif

    heap()->AddGCPrologueCallback(ResetBeforeGC, kGCTypeMarkSweepCompact, nullptr);

    // Isolate initialization allocates long living objects that should be
    // pretentured to old space.
    DCHECK_IMPLIES(heap()->new_space(), (heap()->new_space()->Size() == 0) && (heap()->gc_count() == 0));
...
std::unique_ptr<PersistentHandles> Isolate::NewPersistentHandles() { return std::make_unique<PersistentHandles>(this); }
...
bool Isolate::NeedsDetailedOptimizedCodeLineInfo() const { return NeedsSourcePositionsForProfiling() || detailed_source_positions_for_profiling(); }

bool Isolate::NeedsSourcePositionsForProfiling() const
{
    return
        // Static conditions.
        v8_flags.trace_deopt || v8_flags.trace_turbo || v8_flags.trace_turbo_graph || v8_flags.turbo_profiling || v8_flags.perf_prof || v8_flags.log_maps || v8_flags.log_ic ||
...
void Isolate::MaybeInitializeVectorListFromHeap()
{
...

    {
        ...
        for (HeapObject current_obj = heap_iterator.Next(); !current_obj.is_null(); current_obj = heap_iterator.Next()) {
...
Isolate::KnownPrototype Isolate::IsArrayOrObjectOrStringPrototype(Object object)
{
...
void Isolate::UpdateTypedArraySpeciesLookupChainProtectorOnSetPrototype(Handle<JSObject> object)
{
    ...
    if (object->IsTypedArrayConstructor() && Protectors::IsTypedArraySpeciesLookupChainIntact(this)) {
...
CodeLookupResult Isolate::FindCodeObject(Address a) { return heap()->GcSafeFindCodeForInnerPointer(a); }
...
Handle<Symbol> Isolate::SymbolFor(RootIndex dictionary_index, Handle<String> name, bool private_symbol)
{
    ...
    Handle<RegisteredSymbolTable> dictionary = Handle<RegisteredSymbolTable>::cast(root_handle(dictionary_index));
...
void Isolate::FireCallCompletedCallbackInternal(MicrotaskQueue* microtask_queue)
{
    ...
    bool perform_checkpoint = microtask_queue && microtask_queue->microtasks_policy() == v8::MicrotasksPolicy::kAuto;
...
{
    promise_hook_flags_ = (promise_hook_flags_ & PromiseHookFields::HasContextPromiseHook::kMask) | PromiseHookFields::HasIsolatePromiseHook::encode(promise_hook_)
        | PromiseHookFields::HasAsyncEventDelegate::encode(async_event_delegate_) | PromiseHookFields::IsDebugActive::encode(debug()->is_active());
...
MaybeHandle<JSPromise> NewRejectedPromise(Isolate* isolate, v8::Local<v8::Context> api_context, Handle<Object> exception)
{
    ...
    ASSIGN_RETURN_ON_SCHEDULED_EXCEPTION_VALUE(isolate, resolver, v8::Promise::Resolver::New(api_context), MaybeHandle<JSPromise>());

    RETURN_ON_SCHEDULED_EXCEPTION_VALUE(isolate, resolver->Reject(api_context, v8::Utils::ToLocal(exception)), MaybeHandle<JSPromise>());
...
MaybeHandle<JSPromise> Isolate::RunHostImportModuleDynamicallyCallback(
    MaybeHandle<Script> maybe_referrer, Handle<Object> specifier, MaybeHandle<Object> maybe_import_assertions_argument)
{
    v8::Local<v8::Context> api_context = v8::Utils::ToLocal(Handle<Context>::cast(native_context()));
    if (host_import_module_dynamically_with_import_assertions_callback_ == nullptr && host_import_module_dynamically_callback_ == nullptr) {
    ...
    if (!GetImportAssertionsFromArgument(maybe_import_assertions_argument).ToHandle(&import_assertions_array)) {
        Handle<Object> exception(pending_exception(), this);
        clear_pending_exception();
        return NewRejectedPromise(this, api_context, exception);
    }
    ...
    if (host_import_module_dynamically_callback_) {
        ASSIGN_RETURN_ON_SCHEDULED_EXCEPTION_VALUE(this, promise,
            host_import_module_dynamically_callback_(api_context, v8::Utils::ToLocal(host_defined_options), v8::Utils::ToLocal(resource_name), v8::Utils::ToLocal(specifier_str),
                ToApiHandle<v8::FixedArray>(import_assertions_array)),
            MaybeHandle<JSPromise>());
    } else {
        // TODO(cbruni, v8:12302): Avoid creating tempory ScriptOrModule objects.
        auto script_or_module = i::Handle<i::ScriptOrModule>::cast(this->factory()->NewStruct(i::SCRIPT_OR_MODULE_TYPE));
        ...
        ASSIGN_RETURN_ON_SCHEDULED_EXCEPTION_VALUE(this, promise,
            host_import_module_dynamically_with_import_assertions_callback_(
                api_context, v8::Utils::ToLocal(script_or_module), v8::Utils::ToLocal(specifier_str), ToApiHandle<v8::FixedArray>(import_assertions_array)),
            MaybeHandle<JSPromise>());
    }
    return v8::Utils::OpenHandle(*promise);
}

MaybeHandle<FixedArray> Isolate::GetImportAssertionsFromArgument(MaybeHandle<Object> maybe_import_assertions_argument)
{
    Handle<FixedArray> import_assertions_array = factory()->empty_fixed_array();
    Handle<Object> import_assertions_argument;
    if (!maybe_import_assertions_argument.ToHandle(&import_assertions_argument) || import_assertions_argument->IsUndefined()) {
    ...
    Handle<JSReceiver> import_assertions_argument_receiver = Handle<JSReceiver>::cast(import_assertions_argument);
    Handle<Name> key = factory()->assert_string();

    Handle<Object> import_assertions_object;
    if (!JSReceiver::GetProperty(this, import_assertions_argument_receiver, key).ToHandle(&import_assertions_object)) {
    ...
    if (import_assertions_object->IsUndefined())
        return import_assertions_array;

    if (!import_assertions_object->IsJSReceiver()) {
        this->Throw(*factory()->NewTypeError(MessageTemplate::kNonObjectAssertOption));
        return MaybeHandle<FixedArray>();
    }

    Handle<JSReceiver> import_assertions_object_receiver = Handle<JSReceiver>::cast(import_assertions_object);

    Handle<FixedArray> assertion_keys;
    if (!KeyAccumulator::GetKeys(this, import_assertions_object_receiver, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS, GetKeysConversion::kConvertToString)
    ...
    import_assertions_array = factory()->NewFixedArray(static_cast<int>(assertion_keys->length() * kAssertionEntrySizeForDynamicImport));
    for (int i = 0; i < assertion_keys->length(); i++) {
        ...
        if (!JSReceiver::GetProperty(this, import_assertions_object_receiver, assertion_key).ToHandle(&assertion_value)) {
            // This can happen if the property has a getter function that throws
            // an error.
            return MaybeHandle<FixedArray>();
        }
        ...
        if (!assertion_value->IsString()) {
            this->Throw(*factory()->NewTypeError(MessageTemplate::kNonStringImportAssertionValue));
            return MaybeHandle<FixedArray>();
        }

        import_assertions_array->set((i * kAssertionEntrySizeForDynamicImport), *assertion_key);
        import_assertions_array->set((i * kAssertionEntrySizeForDynamicImport) + 1, *assertion_value);
    }
...
MaybeHandle<JSObject> Isolate::RunHostInitializeImportMetaObjectCallback(Handle<SourceTextModule> module)
{
    ...
    if (host_initialize_import_meta_object_callback_ != nullptr) {
        ...
        host_initialize_import_meta_object_callback_(api_context, Utils::ToLocal(Handle<Module>::cast(module)), v8::Local<v8::Object>::Cast(v8::Utils::ToLocal(import_meta)));
...
void Isolate::SetHostInitializeImportMetaObjectCallback(HostInitializeImportMetaObjectCallback callback) { host_initialize_import_meta_object_callback_ = callback; }

void Isolate::SetHostCreateShadowRealmContextCallback(HostCreateShadowRealmContextCallback callback) { host_create_shadow_realm_context_callback_ = callback; }
...
MaybeHandle<NativeContext> Isolate::RunHostCreateShadowRealmContextCallback()
{
    ...
    ASSIGN_RETURN_ON_SCHEDULED_EXCEPTION_VALUE(this, shadow_realm_context, host_create_shadow_realm_context_callback_(api_context), MaybeHandle<NativeContext>());
...
MaybeHandle<Object> Isolate::RunPrepareStackTraceCallback(Handle<Context> context, Handle<JSObject> error, Handle<JSArray> sites)
{
    v8::Local<v8::Context> api_context = Utils::ToLocal(context);

    v8::Local<v8::Value> stack;
    ASSIGN_RETURN_ON_SCHEDULED_EXCEPTION_VALUE(this, stack, prepare_stack_trace_callback_(api_context, Utils::ToLocal(error), Utils::ToLocal(sites)), MaybeHandle<Object>());
...
{
    if (embedded_file_writer_ != nullptr) {
        embedded_file_writer_->PrepareBuiltinLabelInfoMap(heap()->construct_stub_create_deopt_pc_offset().value(), heap()->construct_stub_invoke_deopt_pc_offset().value());
    }
}
...
void Isolate::SetPrepareStackTraceCallback(PrepareStackTraceCallback callback) { prepare_stack_trace_callback_ = callback; }

bool Isolate::HasPrepareStackTraceCallback() const { return prepare_stack_trace_callback_ != nullptr; }
...
void Isolate::RunAtomicsWaitCallback(
    v8::Isolate::AtomicsWaitEvent event, Handle<JSArrayBuffer> array_buffer, size_t offset_in_bytes, int64_t value, double timeout_in_ms, AtomicsWaitWakeHandle* stop_handle)
{
    DCHECK(array_buffer->is_shared());
    if (atomics_wait_callback_ == nullptr)
        return;
    HandleScope handle_scope(this);
    atomics_wait_callback_(event, v8::Utils::ToLocalShared(array_buffer), offset_in_bytes, value, timeout_in_ms, reinterpret_cast<v8::Isolate::AtomicsWaitWakeHandle*>(stop_handle),
...
void Isolate::OnTerminationDuringRunMicrotasks()
{
    ...
    Handle<Microtask> current_microtask(Microtask::cast(heap()->current_microtask()), this);
    ...
    if (current_microtask->IsPromiseReactionJobTask()) {
        ...
        if (promise_or_capability->IsPromiseCapability()) {
            promise_or_capability = handle(Handle<PromiseCapability>::cast(promise_or_capability)->promise(), this);
        }
        if (promise_or_capability->IsJSPromise()) {
            OnPromiseAfter(Handle<JSPromise>::cast(promise_or_capability));
        }
    } else if (current_microtask->IsPromiseResolveThenableJobTask()) {
        Handle<PromiseResolveThenableJobTask> promise_resolve_thenable_job_task = Handle<PromiseResolveThenableJobTask>::cast(current_microtask);
...
void Isolate::SetPromiseRejectCallback(PromiseRejectCallback callback) { promise_reject_callback_ = callback; }
...
void Isolate::AddDetachedContext(Handle<Context> context)
{
    ...
    detached_contexts = WeakArrayList::AddToEnd(this, detached_contexts, MaybeObjectHandle(Smi::zero(), this), MaybeObjectHandle::Weak(context));
...
void Isolate::CheckDetachedContextsAfterGC()
{
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (!context->IsCleared()) {
            detached_contexts->Set(new_length, MaybeObject::FromSmi(Smi::FromInt(mark_sweeps + 1)));
...
void Isolate::IsolateInForegroundNotification() { is_isolate_in_background_ = false; }
...
bool StringEqualsLocales(Isolate* isolate, const std::string& str, Handle<Object> locales)
{
    if (locales->IsUndefined(isolate))
        return str == "";
    return Handle<String>::cast(locales)->IsEqualTo(base::VectorOf(str.c_str(), str.length()));
}
...
void Isolate::set_icu_object_in_cache(ICUObjectCacheType cache_type, Handle<Object> locales, std::shared_ptr<icu::UMemory> obj)
{
    icu_object_cache_[static_cast<int>(cache_type)] = { GetStringFromLocales(this, locales), std::move(obj) };
}

void Isolate::clear_cached_icu_object(ICUObjectCacheType cache_type) { icu_object_cache_[static_cast<int>(cache_type)] = ICUObjectCacheEntry {}; }
...
SaveContext::~SaveContext() { isolate_->set_context(context_.is_null() ? Context() : *context_); }

bool SaveContext::IsBelowFrame(CommonFrame* frame) { return (c_entry_fp_ == 0) || (c_entry_fp_ > frame->sp()); }

SaveAndSwitchContext::SaveAndSwitchContext(Isolate* isolate, Context new_context)
    : SaveContext(isolate)
{
...
void Isolate::AddCodeRange(Address begin, size_t length_in_bytes) { AddCodeMemoryRange(MemoryRange { reinterpret_cast<void*>(begin), length_in_bytes }); }

bool Isolate::RequiresCodeRange() const { return kPlatformRequiresCodeRange && !jitless_; }

v8::metrics::Recorder::ContextId Isolate::GetOrRegisterRecorderContextId(Handle<NativeContext> context)
{
...
LocalHeap* Isolate::main_thread_local_heap() { return main_thread_local_isolate()->heap(); }
...
ExternalPointerHandle Isolate::GetOrCreateWaiterQueueNodeExternalPointer()
{
    ...
    if (waiter_queue_node_external_pointer_handle_ != kNullExternalPointerHandle) {
        handle = waiter_queue_node_external_pointer_handle_;
    } else {
        handle = shared_external_pointer_table().AllocateAndInitializeEntry(this, kNullAddress, kWaiterQueueNodeTag);
...
void Isolate::LocalsBlockListCacheSet(Handle<ScopeInfo> scope_info, Handle<ScopeInfo> outer_scope_info, Handle<StringSet> locals_blocklist)
{
    ...
    if (heap()->locals_block_list_cache().IsEphemeronHashTable()) {
        cache = handle(EphemeronHashTable::cast(heap()->locals_block_list_cache()), this);
    } else {
...
Object Isolate::LocalsBlockListCacheGet(Handle<ScopeInfo> scope_info)
{
    ...
    Object maybe_value = EphemeronHashTable::cast(heap()->locals_block_list_cache()).Lookup(scope_info);
    if (maybe_value.IsTuple2())
        return Tuple2::cast(maybe_value).value2();
...
public:
    DefaultWasmAsyncResolvePromiseTask(
    ...
    void Run() override
    {
        ...
        MicrotasksScope microtasks_scope(isolate_, MicrotasksScope::kDoNotRunMicrotasks);
    ...
    v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Promise::Resolver> resolver, v8::Local<v8::Value> result, WasmAsyncSuccess success)
{
    ...
    // the result promise.
    V8::GetCurrentPlatform()->GetForegroundTaskRunner(isolate)->PostTask(std::make_unique<DefaultWasmAsyncResolvePromiseTask>(isolate, context, resolver, result, success));
}
