...
intptr_t S390Debugger::GetRegisterValue(int regnum) { return sim_->get_register(regnum); }

double S390Debugger::GetRegisterPairDoubleValue(int regnum) { return sim_->get_double_from_register_pair(regnum); }

double S390Debugger::GetFPDoubleRegisterValue(int regnum) { return sim_->get_fpr<double>(regnum); }

float S390Debugger::GetFPFloatRegisterValue(int regnum) { return sim_->get_fpr<float>(regnum); }
...
void Simulator::SoftwareInterrupt(Instruction* instr)
{
    ...
    case kCallRtRedirected: {
        ...
        *base::bit_cast<intptr_t*>(get_register(sp) + kStackFrameRASlot * kSystemPointerSize) = get_register(r14);
        ...
        int64_t saved_lr = *base::bit_cast<intptr_t*>(get_register(sp) + kStackFrameRASlot * kSystemPointerSize);
...
#define CASE(i, type)                                                                                                                                                              \
    case i: {                                                                                                                                                                      \
        FOR_EACH_LANE(j, type) { set_simd_register_by_lane<type>(r1, j, get_simd_register_by_lane<type>(r3, i2)); }                                                                \
        break;                                                                                                                                                                     \
    }
    switch (m4) {
        CASE(0, uint8_t);
...
#define CASE(i, type)                                                                                                                                                              \
    case i: {                                                                                                                                                                      \
        FOR_EACH_LANE(j, type) { set_simd_register_by_lane<type>(r1, j, *base::bit_cast<type*>(addr)); }                                                                           \
        break;                                                                                                                                                                     \
    }
    switch (m3) {
        CASE(0, uint8_t);
...
#define CASE(i, type)                                                                                                                                                              \
    case i: {                                                                                                                                                                      \
        FOR_EACH_LANE(j, type) { set_simd_register_by_lane<type>(r1, j, static_cast<type>(i2)); }                                                                                  \
        break;                                                                                                                                                                     \
    }
    switch (m3) {
        CASE(0, int8_t);
...
{
    FOR_EACH_LANE(i, S) { sim->set_simd_register_by_lane<S>(dst, i, base::bits::CountPopulation(sim->get_simd_register_by_lane<S>(src, i))); }
}
...
#define CASE(i, S, D)                                                                                                                                                              \
    case i: {                                                                                                                                                                      \
        FOR_EACH_LANE(index, S) { set_simd_register_by_lane<D>(r1, index, static_cast<D>(get_simd_register_by_lane<S>(r2, index))); }                                              \
        break;                                                                                                                                                                     \
    }
EVALUATE(VCDG)
{
...
template <class S, class D> void VectorUnpackLow(Simulator* sim, int dst, int src)
{
    ...
    FOR_EACH_LANE(i, D) { temps[i] = sim->get_simd_register_by_lane<S>(src, i, false); }
    FOR_EACH_LANE(i, D) { sim->set_simd_register_by_lane<D>(dst, i, temps[i], false); }
}
...
EVALUATE(EX)
{
    ...
    SixByteInstr the_instr = Instruction::InstructionBits(reinterpret_cast<const byte*>(b2_val + x2_val + d2_val));
    int inst_length = Instruction::InstructionLength(reinterpret_cast<const byte*>(b2_val + x2_val + d2_val));
    ...
    Instruction::SetInstructionBits<SixByteInstr>(reinterpret_cast<byte*>(addr), static_cast<SixByteInstr>(the_instr));
