...
void DefaultWasmAsyncResolvePromiseCallback(
    v8::Isolate* isolate, v8::Local<v8::Context> context, v8::Local<v8::Promise::Resolver> resolver, v8::Local<v8::Value> compilation_result, WasmAsyncSuccess success);
...
#define RETURN_VALUE_IF_SCHEDULED_EXCEPTION_DETECTOR(isolate, detector, value) RETURN_VALUE_IF_SCHEDULED_EXCEPTION(isolate, (detector.AcceptSideEffects(), value))
...
#define ISOLATE_INIT_LIST(V)                                                                                                                                                       \
    ...
    V(ModifyCodeGenerationFromStringsCallback, modify_code_gen_callback, nullptr)                                                                                                  \
    V(ModifyCodeGenerationFromStringsCallback2, modify_code_gen_callback2, nullptr)                                                                                                \
...
public:
    ...
    public:
        ...
        bool Matches(Isolate* isolate, ThreadId thread_id) const { return isolate_ == isolate && thread_id_ == thread_id; }
    ...
    void SetUpFromReadOnlyArtifacts(std::shared_ptr<ReadOnlyArtifacts> artifacts, ReadOnlyHeap* ro_heap);
    ...
    static PerIsolateThreadData* CurrentPerIsolateThreadData() { return reinterpret_cast<PerIsolateThreadData*>(base::Thread::GetThreadLocal(per_isolate_thread_data_key_)); }
    ...
    V8_INLINE static Isolate* TryGetCurrent()
    {
        ...
        return reinterpret_cast<Isolate*>(base::Thread::GetExistingThreadLocal(isolate_key_));
    }
    ...
    base::SharedMutex* feedback_vector_access() { return &feedback_vector_access_; }

    // Shared mutex for allowing thread-safe concurrent reads of
    // InternalizedStrings.
    base::SharedMutex* internalized_string_access() { return &internalized_string_access_; }

    // Shared mutex for allowing thread-safe concurrent reads of TransitionArrays
    // of kind kFullTransitionArray.
    base::SharedMutex* full_transition_array_access() { return &full_transition_array_access_; }

    // Shared mutex for allowing thread-safe concurrent reads of
    // SharedFunctionInfos.
    base::SharedMutex* shared_function_info_access() { return &shared_function_info_access_; }
    ...
    base::SharedMutex* boilerplate_migration_access() { return &boilerplate_migration_access_; }

    // The isolate's string table.
    StringTable* string_table() const { return string_table_.get(); }
    StringForwardingTable* string_forwarding_table() const { return string_forwarding_table_.get(); }
    ...
    inline void set_thread_id(ThreadId id) { thread_local_top()->thread_id_.store(id, std::memory_order_relaxed); }
    inline ThreadId thread_id() const { return thread_local_top()->thread_id_.load(std::memory_order_relaxed); }
    ...
    v8::TryCatch* try_catch_handler() { return thread_local_top()->try_catch_handler_; }
    ...
    static Address c_entry_fp(ThreadLocalTop* thread) { return thread->c_entry_fp_; }
    static Address handler(ThreadLocalTop* thread) { return thread->handler_; }
    Address c_function() { return thread_local_top()->c_function_; }

    inline Address* c_entry_fp_address() { return &thread_local_top()->c_entry_fp_; }
    static uint32_t c_entry_fp_offset() { return static_cast<uint32_t>(OFFSET_OF(Isolate, thread_local_top()->c_entry_fp_) - isolate_root_bias()); }
    inline Address* handler_address() { return &thread_local_top()->handler_; }
    inline Address* c_function_address() { return &thread_local_top()->c_function_; }
    ...
    void RegisterDeserializerFinished() { CHECK_GE(--num_active_deserializers_, 0); }
    bool has_active_deserializer() const { return num_active_deserializers_.load(std::memory_order_acquire) > 0; }
#else
    ...
    inline Address* js_entry_sp_address() { return &thread_local_top()->js_entry_sp_; }
    ...
    Handle<FixedArray> CaptureDetailedStackTrace(int limit, StackTrace::StackTraceOptions options);
    ...
    template <typename T> V8_WARN_UNUSED_RESULT MaybeHandle<T> Throw(Handle<Object> exception)
    {
        Throw(*exception);
        return MaybeHandle<T>();
    }

    template <typename T> V8_WARN_UNUSED_RESULT MaybeHandle<T> ThrowAt(Handle<JSObject> exception, MessageLocation* location)
    {
        ThrowAt(exception, location);
        return MaybeHandle<T>();
    }

    void FatalProcessOutOfHeapMemory(const char* location) { heap()->FatalProcessOutOfMemory(location); }

    void set_console_delegate(debug::ConsoleDelegate* delegate) { console_delegate_ = delegate; }
    ...
    enum CatchType { NOT_CAUGHT, CAUGHT_BY_JAVASCRIPT, CAUGHT_BY_EXTERNAL, CAUGHT_BY_PROMISE, CAUGHT_BY_ASYNC_AWAIT };
...
#undef GLOBAL_ACCESSOR
    ...
    bool detailed_source_positions_for_profiling() const { return detailed_source_positions_for_profiling_; }
...
#undef NATIVE_CONTEXT_FIELD_ACCESSOR
    ...
    const std::shared_ptr<metrics::Recorder>& metrics_recorder() { return metrics_recorder_; }
    ...
    static Isolate* FromHeap(const Heap* heap) { return reinterpret_cast<Isolate*>(reinterpret_cast<Address>(heap) - OFFSET_OF(Isolate, heap_)); }
    ...
    {
        DCHECK_IMPLIES(!COMPRESS_POINTERS_IN_ISOLATE_CAGE_BOOL && !COMPRESS_POINTERS_IN_SHARED_CAGE_BOOL, isolate_data()->cage_base() == kNullAddress);
    ...
    VirtualMemoryCage* GetPtrComprCage() { return isolate_allocator_->GetPtrComprCage(); }
    const VirtualMemoryCage* GetPtrComprCage() const { return isolate_allocator_->GetPtrComprCage(); }
    ...
    static size_t isolate_root_bias() { return OFFSET_OF(Isolate, isolate_data_) + IsolateData::kIsolateRootBias; }
    static Isolate* FromRootAddress(Address isolate_root) { return reinterpret_cast<Isolate*>(isolate_root - isolate_root_bias()); }
    ...
    base::AddressRegion root_register_addressable_region() const { return base::AddressRegion(reinterpret_cast<Address>(&isolate_data_), sizeof(IsolateData)); }

    Object root(RootIndex index) const { return Object(roots_table()[index]); }

    Handle<Object> root_handle(RootIndex index) { return Handle<Object>(&roots_table()[index]); }
    ...
    V8_INLINE Address* builtin_tier0_table() { return isolate_data_.builtin_tier0_table(); }
    ...
    void set_deoptimizer_lazy_throw(bool value) { deoptimizer_lazy_throw_ = value; }
    void InitializeThreadLocal();
    ThreadLocalTop* thread_local_top() { return &isolate_data_.thread_local_top_; }
    ThreadLocalTop const* thread_local_top() const { return &isolate_data_.thread_local_top_; }
    ...
    static uint32_t thread_in_wasm_flag_address_offset()
    {
        ...
        return static_cast<uint32_t>(OFFSET_OF(Isolate, thread_local_top()->thread_in_wasm_flag_address_) - isolate_root_bias());
    }

    THREAD_LOCAL_TOP_ADDRESS(Address, thread_in_wasm_flag_address)

    MaterializedObjectStore* materialized_object_store() const { return materialized_object_store_; }

    DescriptorLookupCache* descriptor_lookup_cache() const { return descriptor_lookup_cache_; }
    ...
    InnerPointerToCodeCache* inner_pointer_to_code_cache() { return inner_pointer_to_code_cache_; }
    ...
    unibrow::Mapping<unibrow::Ecma262UnCanonicalize>* jsregexp_uncanonicalize() { return &jsregexp_uncanonicalize_; }

    unibrow::Mapping<unibrow::CanonicalizationRange>* jsregexp_canonrange() { return &jsregexp_canonrange_; }

    unibrow::Mapping<unibrow::Ecma262Canonicalize>* regexp_macro_assembler_canonicalize() { return &regexp_macro_assembler_canonicalize_; }
#endif // !V8_INTL_SUPPORT
    ...
    size_t total_regexp_code_generated() const { return total_regexp_code_generated_; }
    ...
    bool is_profiling() const { return isolate_data_.is_profiling_.load(std::memory_order_relaxed); }
    ...
    bool snapshot_available() const { return snapshot_blob_ != nullptr && snapshot_blob_->raw_size != 0; }
    ...
    bool is_best_effort_code_coverage() const { return code_coverage_mode() == debug::CoverageMode::kBestEffort; }

    bool is_precise_count_code_coverage() const { return code_coverage_mode() == debug::CoverageMode::kPreciseCount; }

    bool is_precise_binary_code_coverage() const { return code_coverage_mode() == debug::CoverageMode::kPreciseBinary; }

    bool is_block_count_code_coverage() const { return code_coverage_mode() == debug::CoverageMode::kBlockCount; }

    bool is_block_binary_code_coverage() const { return code_coverage_mode() == debug::CoverageMode::kBlockBinary; }

    bool is_block_code_coverage() const { return is_block_count_code_coverage() || is_block_binary_code_coverage(); }

    bool is_binary_code_coverage() const { return is_precise_binary_code_coverage() || is_block_binary_code_coverage(); }

    bool is_count_code_coverage() const { return is_precise_count_code_coverage() || is_block_count_code_coverage(); }
    ...
    double time_millis_since_init() const { return heap_.MonotonicallyIncreasingTimeInMs() - time_millis_at_init_; }
    ...
    enum class ICUObjectCacheType { kDefaultCollator, kDefaultNumberFormat, kDefaultSimpleDateFormat, kDefaultSimpleDateFormatForTime, kDefaultSimpleDateFormatForDate };
    ...
    void set_icu_object_in_cache(ICUObjectCacheType cache_type, Handle<Object> locales, std::shared_ptr<icu::UMemory> obj);
    ...
    void UpdateNoElementsProtectorOnSetLength(Handle<JSObject> object) { UpdateNoElementsProtectorOnSetElement(object); }
    void UpdateNoElementsProtectorOnSetPrototype(Handle<JSObject> object) { UpdateNoElementsProtectorOnSetElement(object); }
    void UpdateTypedArraySpeciesLookupChainProtectorOnSetPrototype(Handle<JSObject> object);
    void UpdateNoElementsProtectorOnNormalizeElements(Handle<JSObject> object) { UpdateNoElementsProtectorOnSetElement(object); }
    ...
    PersistentHandlesList* persistent_handles_list() const { return persistent_handles_list_.get(); }
    ...
    bool was_locker_ever_used() const { return was_locker_ever_used_.load(std::memory_order_relaxed); }
    void set_was_locker_ever_used() { was_locker_ever_used_.store(true, std::memory_order_relaxed); }
    ...
    int GetNextUniqueSharedFunctionInfoId()
    {
        ...
        } while (!next_unique_sfi_id_.compare_exchange_weak(current_id, next_id, std::memory_order_relaxed));
        return current_id;
    }
#endif
    ...
    bool HasContextPromiseHooks() const { return PromiseHookFields::HasContextPromiseHook::decode(promise_hook_flags_); }

    Address promise_hook_flags_address() { return reinterpret_cast<Address>(&promise_hook_flags_); }

    Address promise_hook_address() { return reinterpret_cast<Address>(&promise_hook_); }

    Address async_event_delegate_address() { return reinterpret_cast<Address>(&async_event_delegate_); }

    Address javascript_execution_assert_address() { return reinterpret_cast<Address>(&javascript_execution_assert_); }

    void IncrementJavascriptExecutionCounter() { javascript_execution_counter_++; }

    Address handle_scope_implementer_address() { return reinterpret_cast<Address>(&handle_scope_implementer_); }

    void SetAtomicsWaitCallback(v8::Isolate::AtomicsWaitCallback callback, void* data);
    void RunAtomicsWaitCallback(
    ...
    bool IsGeneratingEmbeddedBuiltins() const { return builtins_constants_table_builder() != nullptr; }

    BuiltinsConstantsTableBuilder* builtins_constants_table_builder() const { return builtins_constants_table_builder_; }
    ...
    bool is_short_builtin_calls_enabled() const { return V8_SHORT_BUILTIN_CALLS_BOOL && is_short_builtin_calls_enabled_; }
    ...
    void set_array_buffer_allocator(v8::ArrayBuffer::Allocator* allocator) { array_buffer_allocator_ = allocator; }
    v8::ArrayBuffer::Allocator* array_buffer_allocator() const { return array_buffer_allocator_; }

    void set_array_buffer_allocator_shared(std::shared_ptr<v8::ArrayBuffer::Allocator> allocator) { array_buffer_allocator_shared_ = std::move(allocator); }
    std::shared_ptr<v8::ArrayBuffer::Allocator> array_buffer_allocator_shared() const { return array_buffer_allocator_shared_; }

    FutexWaitListNode* futex_wait_list_node() { return &futex_wait_list_node_; }

    CancelableTaskManager* cancelable_task_manager() { return cancelable_task_manager_; }

    const AstStringConstants* ast_string_constants() const { return ast_string_constants_; }

    interpreter::Interpreter* interpreter() const { return interpreter_; }

    compiler::PerIsolateCompilerCache* compiler_cache() const { return compiler_cache_; }
    ...
    LazyCompileDispatcher* lazy_compile_dispatcher() const { return lazy_compile_dispatcher_.get(); }
    ...
    void RegisterEmbeddedFileWriter(EmbeddedFileWriterInterface* writer) { embedded_file_writer_ = writer; }
    ...
    void set_code_coverage_mode(debug::CoverageMode coverage_mode) { code_coverage_mode_.store(coverage_mode, std::memory_order_relaxed); }
    debug::CoverageMode code_coverage_mode() const { return code_coverage_mode_.load(std::memory_order_relaxed); }
    ...
    void set_elements_deletion_counter(size_t value) { elements_deletion_counter_ = value; }

#if V8_ENABLE_WEBASSEMBLY
    void AddSharedWasmMemory(Handle<WasmMemoryObject> memory_object);
#endif // V8_ENABLE_WEBASSEMBLY

    const v8::Context::BackupIncumbentScope* top_backup_incumbent_scope() const { return top_backup_incumbent_scope_; }
    void set_top_backup_incumbent_scope(const v8::Context::BackupIncumbentScope* top_backup_incumbent_scope) { top_backup_incumbent_scope_ = top_backup_incumbent_scope; }
    ...
    v8::metrics::Recorder::ContextId GetOrRegisterRecorderContextId(Handle<NativeContext> context);
    ...
    LocalIsolate* main_thread_local_isolate() { return main_thread_local_isolate_.get(); }
    ...
    ExternalPointerTable& external_pointer_table() { return isolate_data_.external_pointer_table_; }

    const ExternalPointerTable& external_pointer_table() const { return isolate_data_.external_pointer_table_; }

    Address external_pointer_table_address() { return reinterpret_cast<Address>(&isolate_data_.external_pointer_table_); }

    ExternalPointerTable& shared_external_pointer_table() { return *isolate_data_.shared_external_pointer_table_; }

    const ExternalPointerTable& shared_external_pointer_table() const { return *isolate_data_.shared_external_pointer_table_; }

    Address shared_external_pointer_table_address_address() { return reinterpret_cast<Address>(&isolate_data_.shared_external_pointer_table_); }

    ExternalPointerHandle* GetWaiterQueueNodeExternalPointerHandleLocation() { return &waiter_queue_node_external_pointer_handle_; }
    ...
    Isolate* shared_space_isolate() const { return shared_space_isolate_.value(); }
    ...
    bool has_shared_heap() const { return v8_flags.shared_space ? shared_space_isolate() : shared_isolate(); }

    // Returns the isolate that owns the shared spaces.
    Isolate* shared_heap_isolate() const
    {
        DCHECK(has_shared_heap());
        Isolate* isolate = v8_flags.shared_space ? shared_space_isolate() : shared_isolate();
        DCHECK_NOT_NULL(isolate);
        return isolate;
    }

    bool is_shared_heap_isolate() const { return is_shared() || is_shared_space_isolate(); }
    ...
    bool OwnsStringTables() { return !v8_flags.shared_string_table || is_shared() || is_shared_space_isolate(); }
...
private:
    explicit Isolate(std::unique_ptr<IsolateAllocator> isolate_allocator, bool is_shared);
    ~Isolate();
    ...
    private:
        struct Hasher {
            std::size_t operator()(const ThreadId& t) const { return std::hash<int>()(t.ToInteger()); }
        };
    ...
    // then return true.
    bool PropagatePendingExceptionToExternalTryCatch(ExceptionHandlerType top_handler);

    bool HasIsolatePromiseHooks() const { return PromiseHookFields::HasIsolatePromiseHook::decode(promise_hook_flags_); }

    bool HasAsyncEventDelegate() const { return PromiseHookFields::HasAsyncEventDelegate::decode(promise_hook_flags_); }
    ...
    HostImportModuleDynamicallyWithImportAssertionsCallback host_import_module_dynamically_with_import_assertions_callback_ = nullptr;
    ...
    MaybeHandle<FixedArray> GetImportAssertionsFromArgument(MaybeHandle<Object> maybe_import_assertions_argument);
...
#undef ISOLATE_FIELD_OFFSET
    ...
    ExternalPointerHandle waiter_queue_node_external_pointer_handle_ = kNullExternalPointerHandle;
#endif
...
public:
    ...
    static bool TryLock(Isolate* isolate) { return isolate->break_access()->TryLock(); }
...
public:
    ...
    explicit StackTraceFailureMessage(
        Isolate* isolate, StackTraceMode mode, void* ptr1 = nullptr, void* ptr2 = nullptr, void* ptr3 = nullptr, void* ptr4 = nullptr, void* ptr5 = nullptr, void* ptr6 = nullptr);
