...
public:
    ...
    void RunInternal(double deadline_in_seconds) final { func_(deadline_in_seconds); }
...
std::unique_ptr<CancelableTask> MakeCancelableTask(Isolate* isolate, std::function<void()> func) { return std::make_unique<CancelableFuncTask>(isolate, std::move(func)); }
