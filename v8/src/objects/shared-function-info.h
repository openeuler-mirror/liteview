...
public:
    ...
    inline void set_child(int index, PreparseData value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    static int InnerOffset(int data_length) { return RoundUp(kDataStartOffset + data_length * kByteSize, kTaggedSize); }

    static int SizeFor(int data_length, int children_length) { return InnerOffset(data_length) + children_length * kTaggedSize; }
...
class UncompiledData : public TorqueGeneratedUncompiledData<UncompiledData, HeapObject> {
public:
...
class UncompiledDataWithPreparseDataAndJob : public TorqueGeneratedUncompiledDataWithPreparseDataAndJob<UncompiledDataWithPreparseDataAndJob, UncompiledDataWithPreparseData> {
public:
    class BodyDescriptor;
...
class InterpreterData : public TorqueGeneratedInterpreterData<InterpreterData, Struct> {
public:
...
public:
    ...
    V8_EXPORT_PRIVATE static constexpr Smi const kNoSharedNameSentinel = Smi::zero();
    ...
    template <typename IsolateT> inline AbstractCode abstract_code(IsolateT* isolate);

    // Set up the link between shared function info and the script. The shared
    // function info is added to the list on the script.
    V8_EXPORT_PRIVATE void SetScript(ReadOnlyRoots roots, HeapObject script_object, int function_literal_id, bool reset_preparsed_scope_data = true);
    ...
    inline void set_raw_scope_info(ScopeInfo scope_info, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);

    inline void SetScopeInfo(ScopeInfo scope_info, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    V8_EXPORT_PRIVATE void UpdateFromFunctionLiteralForLiveEdit(FunctionLiteral* lit);
...
public:
    ...
    template <typename IsolateT> inline BytecodeArray GetBytecodeArray(IsolateT* isolate) const;
    ...
    V8_EXPORT_PRIVATE WasmExportedFunctionData wasm_exported_function_data() const;
    ...
    inline UncompiledDataWithPreparseData uncompiled_data_with_preparse_data() const;
    inline void set_uncompiled_data_with_preparse_data(UncompiledDataWithPreparseData data);
...
public:
    ...
    V8_EXPORT_PRIVATE static void DiscardCompiled(Isolate* isolate, Handle<SharedFunctionInfo> shared_info);
    ...
    void DiscardCompiledMetadata(
        Isolate* isolate,
        std::function<void(HeapObject object, ObjectSlot slot, HeapObject target)> gc_notify_updated_slot = [](HeapObject object, ObjectSlot slot, HeapObject target) {});
    ...
    // literal.
    template <typename IsolateT> static void InitFromFunctionLiteral(IsolateT* isolate, Handle<SharedFunctionInfo> shared_info, FunctionLiteral* lit, bool is_toplevel);
    ...
    static_assert(FunctionSyntaxKind::kLastFunctionSyntaxKind <= FunctionSyntaxKindBits::kMax);
    ...
    static void InstallDebugBytecode(Handle<SharedFunctionInfo> shared, Isolate* isolate);
    // Removes the debug bytecode and restores the original bytecode to be
    // returned by following calls to GetActiveBytecodeArray.
    static void UninstallDebugBytecode(SharedFunctionInfo shared, Isolate* isolate);
...
public:
    ...
    inline IsCompiledScope(const SharedFunctionInfo shared, LocalIsolate* isolate);
