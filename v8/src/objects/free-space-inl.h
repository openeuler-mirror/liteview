...
{
    DCHECK(IsValid());
    return FreeSpace::unchecked_cast(TaggedField<Object, kNextOffset>::load(*this));
}
...
FreeSpace FreeSpace::unchecked_cast(const Object o) { return base::bit_cast<FreeSpace>(o); }

bool FreeSpace::IsValid()
{
    Heap* heap = GetHeapFromWritableObject(*this);
    Object free_space_map = Isolate::FromHeap(heap)->root(RootIndex::kFreeSpaceMap);
    CHECK_IMPLIES(!map_slot().contains_map_value(free_space_map.ptr()), !heap->deserialization_complete() && map_slot().contains_map_value(kNullAddress));
