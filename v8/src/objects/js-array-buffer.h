...
class JSArrayBuffer : public TorqueGeneratedJSArrayBuffer<JSArrayBuffer, JSObjectWithEmbedderSlots> {
public:
...
// restriction for the length.
    ...
    V8_EXPORT_PRIVATE void Setup(SharedFlag shared, ResizableFlag resizable, std::shared_ptr<BackingStore> backing_store);
...
public:
    ...
    size_t accounting_length() const { return accounting_length_.load(std::memory_order_relaxed); }

    void set_accounting_length(size_t accounting_length) { accounting_length_.store(accounting_length, std::memory_order_relaxed); }

    size_t ClearAccountingLength() { return accounting_length_.exchange(0, std::memory_order_relaxed); }

    std::shared_ptr<BackingStore> RemoveBackingStore() { return std::move(backing_store_); }

    void set_backing_store(std::shared_ptr<BackingStore> backing_store) { backing_store_ = std::move(backing_store); }
...
private:
    ...
    GcState young_gc_state() const { return young_gc_state_.load(std::memory_order_relaxed); }

    void set_young_gc_state(GcState value) { young_gc_state_.store(value, std::memory_order_relaxed); }
};

class JSArrayBufferView : public TorqueGeneratedJSArrayBufferView<JSArrayBufferView, JSObjectWithEmbedderSlots> {
public:
...
class JSDataView : public TorqueGeneratedJSDataView<JSDataView, JSArrayBufferView> {
public:
