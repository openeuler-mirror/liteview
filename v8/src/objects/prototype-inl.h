...
PrototypeIterator::PrototypeIterator(Isolate* isolate, JSReceiver receiver, WhereToStart where_to_start, WhereToEnd where_to_end)
    : isolate_(isolate)
    , object_(receiver)
    , where_to_end_(where_to_end)
    , is_at_end_(false)
...
PrototypeIterator::PrototypeIterator(Isolate* isolate, Map receiver_map, WhereToEnd where_to_end)
    : isolate_(isolate)
...
PrototypeIterator::PrototypeIterator(Isolate* isolate, Handle<Map> receiver_map, WhereToEnd where_to_end)
    : isolate_(isolate)
...
bool PrototypeIterator::HasAccess() const
{
    ...
    if (handle_->IsAccessCheckNeeded()) {
        return isolate_->MayAccess(handle(isolate_->context(), isolate_), Handle<JSObject>::cast(handle_));
    }
    return true;
}
...
void PrototypeIterator::AdvanceIgnoringProxies()
{
    ...
    is_at_end_ = prototype.IsNull(isolate_) || (where_to_end_ == END_AT_NON_HIDDEN && !map.IsJSGlobalProxyMap());
