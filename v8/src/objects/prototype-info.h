...
public:
    ...
    static inline void SetObjectCreateMap(Handle<PrototypeInfo> info, Handle<Map> map);
...
public:
    static Handle<WeakArrayList> Add(Isolate* isolate, Handle<WeakArrayList> array, Handle<Map> value, int* assigned_index);
    ...
    // must not cause GC.
    using CompactionCallback = void (*)(HeapObject object, int from_index, int to_index);
    static WeakArrayList Compact(Handle<WeakArrayList> array, Heap* heap, CompactionCallback callback, AllocationType allocation = AllocationType::kYoung);
