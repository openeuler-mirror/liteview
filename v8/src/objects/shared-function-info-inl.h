...
int PreparseData::inner_start_offset() const { return InnerOffset(data_length()); }

ObjectSlot PreparseData::inner_data_start() const { return RawField(inner_start_offset()); }
...
PreparseData PreparseData::get_child(int index) const { return PreparseData::cast(get_child_raw(index)); }
...
void PreparseData::set_child(int index, PreparseData value, WriteBarrierMode mode)
{
    DCHECK_LE(0, index);
    DCHECK_LT(index, this->children_length());
    int offset = inner_start_offset() + index * kTaggedSize;
...
RELEASE_ACQUIRE_ACCESSORS(SharedFunctionInfo, function_data, Object, kFunctionDataOffset)
RELEASE_ACQUIRE_ACCESSORS(SharedFunctionInfo, name_or_scope_info, Object, kNameOrScopeInfoOffset)
RELEASE_ACQUIRE_ACCESSORS(SharedFunctionInfo, script_or_debug_info, HeapObject, kScriptOrDebugInfoOffset)

RENAME_TORQUE_ACCESSORS(SharedFunctionInfo, raw_outer_scope_info_or_feedback_metadata, outer_scope_info_or_feedback_metadata, HeapObject)
DEF_ACQUIRE_GETTER(SharedFunctionInfo, raw_outer_scope_info_or_feedback_metadata, HeapObject)
{
    HeapObject value = TaggedField<HeapObject, kOuterScopeInfoOrFeedbackMetadataOffset>::Acquire_Load(cage_base, *this);
    return value;
}
...
int32_t SharedFunctionInfo::relaxed_flags() const { return flags(kRelaxedLoad); }
void SharedFunctionInfo::set_relaxed_flags(int32_t flags) { return set_flags(flags, kRelaxedStore); }
...
bool SharedFunctionInfo::is_script() const { return scope_info(kAcquireLoad).is_script_scope() && Script::cast(script()).compilation_type() == Script::COMPILATION_TYPE_HOST; }

bool SharedFunctionInfo::needs_script_context() const { return is_script() && scope_info(kAcquireLoad).ContextLocalCount() > 0; }

template <typename IsolateT> AbstractCode SharedFunctionInfo::abstract_code(IsolateT* isolate)
{
...
BIT_FIELD_ACCESSORS(SharedFunctionInfo, flags2, sparkplug_compiled, SharedFunctionInfo::SparkplugCompiledBit)
...
bool SharedFunctionInfo::optimization_disabled() const { return disabled_optimization_reason() != BailoutReason::kNoReason; }

BailoutReason SharedFunctionInfo::disabled_optimization_reason() const { return DisabledOptimizationReasonBits::decode(flags(kRelaxedLoad)); }
...
bool SharedFunctionInfo::is_wrapped() const { return syntax_kind() == FunctionSyntaxKind::kWrapped; }

bool SharedFunctionInfo::construct_as_builtin() const { return ConstructAsBuiltinBit::decode(flags(kRelaxedLoad)); }
...
void SharedFunctionInfo::clear_padding() { memset(reinterpret_cast<void*>(this->address() + kSize), 0, kAlignedSize - kSize); }
...
bool SharedFunctionInfo::IsDontAdaptArguments() const { return TorqueGeneratedClass::formal_parameter_count() == kDontAdaptArgumentsSentinel; }
...
DEF_GETTER(SharedFunctionInfo, scope_info, ScopeInfo) { return scope_info(cage_base, kAcquireLoad); }

void SharedFunctionInfo::SetScopeInfo(ScopeInfo scope_info, WriteBarrierMode mode)
{
...
void SharedFunctionInfo::set_raw_scope_info(ScopeInfo scope_info, WriteBarrierMode mode)
{
...
void SharedFunctionInfo::set_outer_scope_info(HeapObject value, WriteBarrierMode mode)
{
    DCHECK(!is_compiled());
...
bool SharedFunctionInfo::HasFeedbackMetadata() const { return raw_outer_scope_info_or_feedback_metadata().IsFeedbackMetadata(); }

bool SharedFunctionInfo::HasFeedbackMetadata(AcquireLoadTag tag) const { return raw_outer_scope_info_or_feedback_metadata(tag).IsFeedbackMetadata(); }
...
RELEASE_ACQUIRE_ACCESSORS_CHECKED2(SharedFunctionInfo, feedback_metadata, FeedbackMetadata, kOuterScopeInfoOrFeedbackMetadataOffset, HasFeedbackMetadata(kAcquireLoad),
    !HasFeedbackMetadata(kAcquireLoad) && value.IsFeedbackMetadata())

bool SharedFunctionInfo::is_compiled() const
{
    Object data = function_data(kAcquireLoad);
    return data != Smi::FromEnum(Builtin::kCompileLazy) && !data.IsUncompiledData();
}

template <typename IsolateT> IsCompiledScope SharedFunctionInfo::is_compiled_scope(IsolateT* isolate) const { return IsCompiledScope(*this, isolate); }

IsCompiledScope::IsCompiledScope(const SharedFunctionInfo shared, Isolate* isolate)
...
IsCompiledScope::IsCompiledScope(const SharedFunctionInfo shared, LocalIsolate* isolate)
    : is_compiled_(shared.is_compiled())
{
    if (shared.HasBaselineCode()) {
...
bool SharedFunctionInfo::has_simple_parameters() { return scope_info(kAcquireLoad).HasSimpleParameters(); }
...
bool SharedFunctionInfo::IsApiFunction() const { return function_data(kAcquireLoad).IsFunctionTemplateInfo(); }
...
DEF_GETTER(SharedFunctionInfo, HasBytecodeArray, bool)
{
    ...
    if (!data.IsHeapObject())
        return false;
    InstanceType instance_type = HeapObject::cast(data).map(cage_base).instance_type();
    return InstanceTypeChecker::IsBytecodeArray(instance_type) || InstanceTypeChecker::IsInterpreterData(instance_type) || InstanceTypeChecker::IsCodeT(instance_type);
}

template <typename IsolateT> BytecodeArray SharedFunctionInfo::GetBytecodeArray(IsolateT* isolate) const
{
    // TODO(ishell): access shared_function_info_access() via IsolateT.
    SharedMutexGuardIfOffThread<IsolateT, base::kShared> mutex_guard(GetIsolate()->shared_function_info_access(), isolate);
...
{
    DCHECK(function_data(kAcquireLoad) == Smi::FromEnum(Builtin::kCompileLazy) || HasUncompiledData());
    set_function_data(bytecode, kReleaseStore);
}

bool SharedFunctionInfo::ShouldFlushCode(base::EnumSet<CodeFlushMode> code_flush_mode)
{
    if (IsFlushingDisabled(code_flush_mode))
        return false;
    ...
    if (data.IsCodeT()) {
        ...
        if (!IsBaselineCodeFlushingEnabled(code_flush_mode))
            return false;
        data = baseline_code.bytecode_or_interpreter_data();
    ...
    if (IsStressFlushingEnabled(code_flush_mode))
        return true;
...
void SharedFunctionInfo::set_interpreter_data(InterpreterData interpreter_data)
{
...
void SharedFunctionInfo::set_baseline_code(CodeT baseline_code, ReleaseStoreTag tag, WriteBarrierMode mode)
{
    DCHECK_EQ(baseline_code.kind(), CodeKind::BASELINE);
...
{
    DCHECK(HasBaselineCode());
    set_function_data(baseline_code(kAcquireLoad).bytecode_or_interpreter_data(), kReleaseStore);
}

#if V8_ENABLE_WEBASSEMBLY
bool SharedFunctionInfo::HasAsmWasmData() const { return function_data(kAcquireLoad).IsAsmWasmData(); }

bool SharedFunctionInfo::HasWasmFunctionData() const { return function_data(kAcquireLoad).IsWasmFunctionData(); }

bool SharedFunctionInfo::HasWasmExportedFunctionData() const { return function_data(kAcquireLoad).IsWasmExportedFunctionData(); }

bool SharedFunctionInfo::HasWasmJSFunctionData() const { return function_data(kAcquireLoad).IsWasmJSFunctionData(); }

bool SharedFunctionInfo::HasWasmCapiFunctionData() const { return function_data(kAcquireLoad).IsWasmCapiFunctionData(); }

bool SharedFunctionInfo::HasWasmResumeData() const { return function_data(kAcquireLoad).IsWasmResumeData(); }
...
{
    DCHECK(function_data(kAcquireLoad) == Smi::FromEnum(Builtin::kCompileLazy) || HasUncompiledData() || HasAsmWasmData());
...
bool SharedFunctionInfo::HasBuiltinId() const { return function_data(kAcquireLoad).IsSmi(); }
...
void SharedFunctionInfo::set_builtin_id(Builtin builtin)
{
    ...
    set_function_data(Smi::FromInt(static_cast<int>(builtin)), kReleaseStore, SKIP_WRITE_BARRIER);
}

bool SharedFunctionInfo::HasUncompiledData() const { return function_data(kAcquireLoad).IsUncompiledData(); }
...
bool SharedFunctionInfo::HasUncompiledDataWithPreparseData() const { return function_data(kAcquireLoad).IsUncompiledDataWithPreparseData(); }

UncompiledDataWithPreparseData SharedFunctionInfo::uncompiled_data_with_preparse_data() const
{
    DCHECK(HasUncompiledDataWithPreparseData());
    return UncompiledDataWithPreparseData::cast(function_data(kAcquireLoad));
}

void SharedFunctionInfo::set_uncompiled_data_with_preparse_data(UncompiledDataWithPreparseData uncompiled_data_with_preparse_data)
{
...
bool SharedFunctionInfo::HasUncompiledDataWithoutPreparseData() const { return function_data(kAcquireLoad).IsUncompiledDataWithoutPreparseData(); }
...
void SharedFunctionInfo::ClearUncompiledDataJobPointer()
{
    ...
    if (uncompiled_data.IsUncompiledDataWithPreparseDataAndJob()) {
        UncompiledDataWithPreparseDataAndJob::cast(uncompiled_data).set_job(kNullAddress);
    } else if (uncompiled_data.IsUncompiledDataWithoutPreparseDataWithJob()) {
        UncompiledDataWithoutPreparseDataWithJob::cast(uncompiled_data).set_job(kNullAddress);
    }
}
...
void SharedFunctionInfo::ClearPreparseData()
{
    ...
    // Swap the map.
    data.set_map(GetReadOnlyRoots().uncompiled_data_without_preparse_data_map(), kReleaseStore);
...
void UncompiledData::InitAfterBytecodeFlush(
    String inferred_name, int start_position, int end_position, std::function<void(HeapObject object, ObjectSlot slot, HeapObject target)> gc_notify_updated_slot)
{
...
bool SharedFunctionInfo::is_repl_mode() const { return script().IsScript() && Script::cast(script()).is_repl_mode(); }

DEF_GETTER(SharedFunctionInfo, HasDebugInfo, bool) { return script_or_debug_info(cage_base, kAcquireLoad).IsDebugInfo(cage_base); }
...
bool SharedFunctionInfo::is_class_constructor() const { return IsClassConstructorBit::decode(flags(kRelaxedLoad)); }
