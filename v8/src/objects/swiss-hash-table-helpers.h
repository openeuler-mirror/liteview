...
public:
    ...
    int HighestBitSet() const { return (sizeof(T) * CHAR_BIT - base::bits::CountLeadingZeros(mask_) - 1) >> Shift; }
...
// present entry.
inline static swiss_table::ctrl_t H2(uint32_t hash) { return hash & ((1 << kH2Bits) - 1); }
...
struct GroupSse2Impl {
    ...
    {
        auto special = _mm_set1_epi8(kSentinel);
        return base::bits::CountTrailingZerosNonZero(_mm_movemask_epi8(_mm_cmpgt_epi8_fixed(special, ctrl)) + 1);
    }
...
struct GroupSse2Polyfill {
    ...
    // Returns a bitmask representing the positions of empty slots.
    BitMask<uint32_t, kWidth> MatchEmpty() const { return Match(static_cast<h2_t>(kEmpty)); }

    // Returns a bitmask representing the positions of empty or deleted slots.
    BitMask<uint32_t, kWidth> MatchEmptyOrDeleted() const { return BitMask<uint32_t, kWidth>(MatchEmptyOrDeletedMask()); }

    // Returns the number of trailing empty or deleted elements in the group.
    uint32_t CountLeadingEmptyOrDeleted() const { return base::bits::CountTrailingZerosNonZero(MatchEmptyOrDeletedMask() + 1); }
...
struct GroupPortableImpl {
    ...
    // Returns a bitmask representing the positions of empty slots.
    BitMask<uint64_t, kWidth, 3> MatchEmpty() const { return BitMask<uint64_t, kWidth, 3>((ctrl & (~ctrl << 6)) & kMsbs); }

    // Returns a bitmask representing the positions of empty or deleted slots.
    BitMask<uint64_t, kWidth, 3> MatchEmptyOrDeleted() const { return BitMask<uint64_t, kWidth, 3>((ctrl & (~ctrl << 7)) & kMsbs); }

    // Returns the number of trailing empty or deleted elements in the group.
    uint32_t CountLeadingEmptyOrDeleted() const
    {
        constexpr uint64_t gaps = 0x00FEFEFEFEFEFEFEULL;
        return (base::bits::CountTrailingZerosNonZero(((~ctrl & (ctrl >> 7)) | gaps) + 1) + 7) >> 3;
    }
