...
swiss_table::ctrl_t* SwissNameDictionary::CtrlTable() { return reinterpret_cast<ctrl_t*>(field_address(CtrlTableStartOffset(Capacity()))); }

uint8_t* SwissNameDictionary::PropertyDetailsTable() { return reinterpret_cast<uint8_t*>(field_address(PropertyDetailsTableStartOffset(Capacity()))); }

int SwissNameDictionary::Capacity() { return ReadField<int32_t>(CapacityOffset()); }
...
int SwissNameDictionary::NumberOfElements() { return GetMetaTableField(kMetaTableElementCountFieldIndex); }

int SwissNameDictionary::NumberOfDeletedElements() { return GetMetaTableField(kMetaTableDeletedElementCountFieldIndex); }

void SwissNameDictionary::SetNumberOfElements(int elements) { SetMetaTableField(kMetaTableElementCountFieldIndex, elements); }

void SwissNameDictionary::SetNumberOfDeletedElements(int deleted_elements) { SetMetaTableField(kMetaTableDeletedElementCountFieldIndex, deleted_elements); }

int SwissNameDictionary::UsedCapacity() { return NumberOfElements() + NumberOfDeletedElements(); }
...
// static
constexpr int SwissNameDictionary::DataTableSize(int capacity) { return capacity * kTaggedSize * kDataTableEntryCount; }
...
template <typename IsolateT> InternalIndex SwissNameDictionary::FindEntry(IsolateT* isolate, Object key)
{
...
template <typename IsolateT> InternalIndex SwissNameDictionary::FindEntry(IsolateT* isolate, Handle<Object> key) { return FindEntry(isolate, *key); }

Object SwissNameDictionary::LoadFromDataTable(int entry, int data_offset) { return LoadFromDataTable(GetPtrComprCageBase(*this), entry, data_offset); }

Object SwissNameDictionary::LoadFromDataTable(PtrComprCageBase cage_base, int entry, int data_offset)
{
    DCHECK_LT(static_cast<unsigned>(entry), static_cast<unsigned>(Capacity()));
...
void SwissNameDictionary::StoreToDataTable(int entry, int data_offset, Object data)
{
    DCHECK_LT(static_cast<unsigned>(entry), static_cast<unsigned>(Capacity()));
...
void SwissNameDictionary::StoreToDataTableNoBarrier(int entry, int data_offset, Object data)
{
    DCHECK_LT(static_cast<unsigned>(entry), static_cast<unsigned>(Capacity()));
...
void SwissNameDictionary::ValueAtPut(InternalIndex entry, Object value) { ValueAtPut(entry.as_int(), value); }
...
void SwissNameDictionary::DetailsAtPut(InternalIndex entry, PropertyDetails details) { DetailsAtPut(entry.as_int(), details); }

Object SwissNameDictionary::KeyAt(int entry) { return LoadFromDataTable(entry, kDataTableKeyEntryIndex); }

Object SwissNameDictionary::KeyAt(InternalIndex entry) { return KeyAt(entry.as_int()); }

Name SwissNameDictionary::NameAt(InternalIndex entry) { return Name::cast(KeyAt(entry)); }

// This version can be called on empty buckets.
Object SwissNameDictionary::ValueAtRaw(int entry) { return LoadFromDataTable(entry, kDataTableValueEntryIndex); }
...
PropertyDetails SwissNameDictionary::DetailsAt(InternalIndex entry) { return DetailsAt(entry.as_int()); }
...
// static
template <typename T> void SwissNameDictionary::SetMetaTableField(ByteArray meta_table, int field_index, int value)
{
    ...
    DCHECK_LT(meta_table.GetDataStartAddress() + field_index * sizeof(T), meta_table.GetDataEndAddress());
    T* raw_data = reinterpret_cast<T*>(meta_table.GetDataStartAddress());
    raw_data[field_index] = value;
}

// static
template <typename T> int SwissNameDictionary::GetMetaTableField(ByteArray meta_table, int field_index)
{
    static_assert((std::is_same<T, uint8_t>::value) || (std::is_same<T, uint16_t>::value) || (std::is_same<T, uint32_t>::value));
    DCHECK_LT(meta_table.GetDataStartAddress() + field_index * sizeof(T), meta_table.GetDataEndAddress());
    T* raw_data = reinterpret_cast<T*>(meta_table.GetDataStartAddress());
...
bool SwissNameDictionary::IsKey(ReadOnlyRoots roots, Object key_candidate) { return key_candidate != roots.the_hole_value(); }

bool SwissNameDictionary::ToKey(ReadOnlyRoots roots, int entry, Object* out_key)
{
...
bool SwissNameDictionary::ToKey(ReadOnlyRoots roots, InternalIndex entry, Object* out_key) { return ToKey(roots, entry.as_int(), out_key); }

// static
template <typename IsolateT>
Handle<SwissNameDictionary> SwissNameDictionary::Add(
    IsolateT* isolate, Handle<SwissNameDictionary> original_table, Handle<Name> key, Handle<Object> value, PropertyDetails details, InternalIndex* entry_out)
{
...
int SwissNameDictionary::AddInternal(Name key, Object value, PropertyDetails details)
{
...
template <typename IsolateT> void SwissNameDictionary::Initialize(IsolateT* isolate, ByteArray meta_table, int capacity)
{
    DCHECK(IsValidCapacity(capacity));
    DisallowHeapAllocation no_gc;
    ReadOnlyRoots roots(isolate);
...
bool SwissNameDictionary::IndexIterator::operator!=(const IndexIterator& b) const { return !(*this == b); }
...
SwissNameDictionary::IndexIterator SwissNameDictionary::IndexIterable::begin() { return IndexIterator(dict_, 0); }
...
SwissNameDictionary::IndexIterable SwissNameDictionary::IterateEntries() { return IterateEntriesOrdered(); }

void SwissNameDictionary::SetHash(int32_t hash) { WriteField(PrefixOffset(), hash); }
...
{
    int const_size = DataTableStartOffset() + ByteArray::kHeaderSize +
...
// static
constexpr int SwissNameDictionary::PrefixOffset() { return HeapObject::kHeaderSize; }

// static
constexpr int SwissNameDictionary::CapacityOffset() { return PrefixOffset() + sizeof(uint32_t); }

// static
constexpr int SwissNameDictionary::MetaTablePointerOffset() { return CapacityOffset() + sizeof(int32_t); }

// static
constexpr int SwissNameDictionary::DataTableStartOffset() { return MetaTablePointerOffset() + kTaggedSize; }

// static
constexpr int SwissNameDictionary::DataTableEndOffset(int capacity) { return CtrlTableStartOffset(capacity); }

// static
constexpr int SwissNameDictionary::CtrlTableStartOffset(int capacity) { return DataTableStartOffset() + DataTableSize(capacity); }

// static
constexpr int SwissNameDictionary::PropertyDetailsTableStartOffset(int capacity) { return CtrlTableStartOffset(capacity) + CtrlTableSize(capacity); }
...
ACCESSORS_CHECKED2(SwissNameDictionary, meta_table, ByteArray, MetaTablePointerOffset(), true, value.length() >= kMetaTableEnumerationDataStartIndex)
