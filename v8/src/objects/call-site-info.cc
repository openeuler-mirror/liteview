...
bool CallSiteInfo::IsToplevel() const { return receiver_or_instance().IsJSGlobalProxy() || receiver_or_instance().IsNullOrUndefined(); }
    ...
    if (script->has_eval_from_shared()) {
        ...
        if (eval_shared->script().IsScript()) {
            ...
            if (eval_script->compilation_type() == Script::COMPILATION_TYPE_EVAL) {
                ...
                Handle<String> str;
                ASSIGN_RETURN_ON_EXCEPTION(isolate, str, FormatEvalOrigin(isolate, eval_script), String);
                ...
                if (eval_script_name->IsString()) {
                    ...
                    if (Script::GetPositionInfo(eval_script, Script::GetEvalPosition(isolate, script), &info, Script::NO_OFFSET)) {
...
// static
Handle<PrimitiveHeapObject> CallSiteInfo::GetEvalOrigin(Handle<CallSiteInfo> info)
{
    auto isolate = info->GetIsolate();
    Handle<Script> script;
    if (!GetScript(isolate, info).ToHandle(&script) || script->compilation_type() != Script::COMPILATION_TYPE_EVAL) {
...
// static
Handle<PrimitiveHeapObject> CallSiteInfo::GetFunctionName(Handle<CallSiteInfo> info)
{
    Isolate* isolate = info->GetIsolate();
#if V8_ENABLE_WEBASSEMBLY
    if (info->IsWasm()) {
        Handle<WasmModuleObject> module_object(info->GetWasmInstance().module_object(), isolate);
...
#if V8_ENABLE_WEBASSEMBLY
    if (info->IsWasm()) {
        return GetWasmFunctionDebugName(isolate, handle(info->GetWasmInstance(), isolate), info->GetWasmFunctionIndex());
    }
#endif // V8_ENABLE_WEBASSEMBLY
    Handle<JSFunction> function(JSFunction::cast(info->function()), isolate);
    Handle<String> name = JSFunction::GetDebugName(function);
...
PrimitiveHeapObject InferMethodNameFromFastObject(Isolate* isolate, JSObject receiver, JSFunction fun, PrimitiveHeapObject name)
{
...
template <typename Dictionary> PrimitiveHeapObject InferMethodNameFromDictionary(Isolate* isolate, Dictionary dictionary, JSFunction fun, PrimitiveHeapObject name)
{
    ReadOnlyRoots roots(isolate);
    for (auto i : dictionary.IterateEntries()) {
        ...
        if (name != key) {
            name = name.IsUndefined(isolate) ? PrimitiveHeapObject::cast(key) : roots.null_value();
        }
    }
    return name;
}

PrimitiveHeapObject InferMethodName(Isolate* isolate, JSReceiver receiver, JSFunction fun)
{
    ...
    for (PrototypeIterator it(isolate, receiver, kStartAtReceiver); !it.IsAtEnd(); it.Advance()) {
        ...
        if (object.HasFastProperties()) {
            ...
            name = InferMethodNameFromDictionary(isolate, JSGlobalObject::cast(object).global_dictionary(kAcquireLoad), fun, name);
...
Handle<Object> CallSiteInfo::GetMethodName(Handle<CallSiteInfo> info)
{
    ...
    Handle<JSReceiver> receiver = JSReceiver::ToObject(isolate, receiver_or_instance).ToHandleChecked();
    Handle<JSFunction> function = handle(JSFunction::cast(info->function()), isolate);
...
Handle<Object> CallSiteInfo::GetTypeName(Handle<CallSiteInfo> info)
{
    ...
    Handle<JSReceiver> receiver = JSReceiver::ToObject(isolate, handle(info->receiver_or_instance(), isolate)).ToHandleChecked();
...
Handle<Object> CallSiteInfo::GetWasmModuleName(Handle<CallSiteInfo> info)
{
    ...
    if (info->IsWasm()) {
        Handle<String> name;
        auto module_object = handle(info->GetWasmInstance().module_object(), isolate);
...
// static
bool CallSiteInfo::ComputeLocation(Handle<CallSiteInfo> info, MessageLocation* location)
{
    Isolate* isolate = info->GetIsolate();
#if V8_ENABLE_WEBASSEMBLY
    if (info->IsWasm()) {
...
int CallSiteInfo::ComputeSourcePosition(Handle<CallSiteInfo> info, int offset)
{
    ...
    if (info->IsWasm()) {
        ...
        auto module = info->GetWasmInstance().module();
        uint32_t func_index = info->GetWasmFunctionIndex();
        return wasm::GetSourcePosition(module, func_index, byte_offset, info->IsAsmJsAtNumberConversion());
    }
#endif // V8_ENABLE_WEBASSEMBLY
    Handle<SharedFunctionInfo> shared(info->GetSharedFunctionInfo(), isolate);
    SharedFunctionInfo::EnsureSourcePositionsAvailable(isolate, shared);
    return AbstractCode::cast(info->code_object()).SourcePosition(isolate, offset);
}
...
// static
MaybeHandle<Script> CallSiteInfo::GetScript(Isolate* isolate, Handle<CallSiteInfo> info)
{
...
bool IsNonEmptyString(Handle<Object> object) { return (object->IsString() && String::cast(*object).length() > 0); }

void AppendFileLocation(Isolate* isolate, Handle<CallSiteInfo> frame, IncrementalStringBuilder* builder)
{
    ...
    if (!script_name_or_source_url->IsString() && frame->IsEval()) {
        builder->AppendString(Handle<String>::cast(CallSiteInfo::GetEvalOrigin(frame)));
...
void AppendMethodCall(Isolate* isolate, Handle<CallSiteInfo> frame, IncrementalStringBuilder* builder)
{
...
void SerializeJSStackFrame(Isolate* isolate, Handle<CallSiteInfo> frame, IncrementalStringBuilder* builder)
{
    Handle<Object> function_name = CallSiteInfo::GetFunctionName(frame);
    if (frame->IsAsync()) {
...
#if V8_ENABLE_WEBASSEMBLY
void SerializeWasmStackFrame(Isolate* isolate, Handle<CallSiteInfo> frame, IncrementalStringBuilder* builder)
{
...
void SerializeCallSiteInfo(Isolate* isolate, Handle<CallSiteInfo> frame, IncrementalStringBuilder* builder)
{
#if V8_ENABLE_WEBASSEMBLY
...
MaybeHandle<String> SerializeCallSiteInfo(Isolate* isolate, Handle<CallSiteInfo> frame)
{
    IncrementalStringBuilder builder(isolate);
