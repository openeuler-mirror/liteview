...
LookupIterator::LookupIterator(Isolate* isolate, Handle<Object> receiver, Handle<Name> name, Handle<Map> transition_map, PropertyDetails details, bool has_property)
...
template <bool is_element> void LookupIterator::NextInternal(Map map, JSReceiver holder)
{
    do {
...
// static
Handle<JSReceiver> LookupIterator::GetRootForNonJSReceiver(Isolate* isolate, Handle<Object> lookup_start_object, size_t index)
{
    // Strings are the only objects with properties (only elements) directly on
    // the wrapper. Hence we can skip generating the wrapper for all other cases.
    if (lookup_start_object->IsString(isolate) && index < static_cast<size_t>(String::cast(*lookup_start_object).length())) {
    ...
    Handle<HeapObject> root(lookup_start_object->GetPrototypeChainRootMap(isolate).prototype(isolate), isolate);
...
{
    if (receiver_->IsNumber(isolate_))
...
bool LookupIterator::HasAccess() const
{
    ...
    return isolate_->MayAccess(handle(isolate_->context(), isolate_), GetHolder<JSObject>());
}
...
// static
void LookupIterator::InternalUpdateProtector(Isolate* isolate, Handle<Object> receiver_generic, Handle<Name> name)
{
    if (isolate->bootstrapper()->IsActive())
        return;
    if (!receiver_generic->IsHeapObject())
        return;
    ...
    if (*name == roots.constructor_string()) {
        ...
        if (receiver->map(isolate).is_prototype_map()) {
            ...
            if (isolate->IsInAnyContext(*receiver, Context::INITIAL_ARRAY_PROTOTYPE_INDEX)) {
                if (!Protectors::IsArraySpeciesLookupChainIntact(isolate))
                    return;
        ...
        if (receiver->IsJSArrayIterator() || receiver->IsJSArrayIteratorPrototype()) {
        ...
        if (receiver->IsJSArray(isolate)) {
            if (!Protectors::IsArrayIteratorLookupChainIntact(isolate))
                return;
            Protectors::InvalidateArrayIteratorLookupChain(isolate);
        } else if (receiver->IsJSSet(isolate) || receiver->IsJSSetIterator() || receiver->IsJSSetIteratorPrototype() || receiver->IsJSSetPrototype()) {
            if (Protectors::IsSetIteratorLookupChainIntact(isolate)) {
                Protectors::InvalidateSetIteratorLookupChain(isolate);
            }
        } else if (receiver->IsJSMapIterator() || receiver->IsJSMapIteratorPrototype()) {
        ...
        } else if (isolate->IsInAnyContext(*receiver, Context::INITIAL_STRING_PROTOTYPE_INDEX)) {
        ...
        if (receiver->IsJSPromise(isolate) || receiver->IsJSObjectPrototype() || receiver->IsJSPromisePrototype()) {
...
void LookupIterator::PrepareForDataProperty(Handle<Object> value)
{
    ...
    if (holder->IsJSGlobalObject(isolate_)) {
        Handle<GlobalDictionary> dictionary(JSGlobalObject::cast(*holder).global_dictionary(isolate_, kAcquireLoad), isolate());
        Handle<PropertyCell> cell(dictionary->CellAt(isolate_, dictionary_entry()), isolate());
...
void LookupIterator::ReconfigureDataProperty(Handle<Object> value, PropertyAttributes attributes)
{
    ...
    if (IsElement(*holder)) {
        ...
        Handle<Map> new_map = MapUpdater::ReconfigureExistingProperty(isolate_, old_map, descriptor_number(), i::PropertyKind::kData, attributes, PropertyConstness::kMutable);
    ...
    if (!IsElement(*holder) && !holder_obj->HasFastProperties(isolate_)) {
        if (holder_obj->map(isolate_).is_prototype_map()
            && (((property_details_.attributes() & READ_ONLY) == 0 && (attributes & READ_ONLY) != 0) || (property_details_.attributes() & DONT_ENUM) != (attributes & DONT_ENUM))) {
        ...
        if (holder_obj->IsJSGlobalObject(isolate_)) {
            ...
            Handle<GlobalDictionary> dictionary(JSGlobalObject::cast(*holder_obj).global_dictionary(isolate_, kAcquireLoad), isolate());

            Handle<PropertyCell> cell = PropertyCell::PrepareForAndSetValue(isolate(), dictionary, dictionary_entry(), value, details);
            property_details_ = cell->property_details();
            DCHECK_EQ(cell->value(), *value);
        } else {
            PropertyDetails details(PropertyKind::kData, attributes, PropertyConstness::kMutable);
            if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
                Handle<SwissNameDictionary> dictionary(holder_obj->property_dictionary_swiss(isolate_), isolate());
                ...
                Handle<NameDictionary> dictionary(holder_obj->property_dictionary(isolate_), isolate());
...
void LookupIterator::PrepareTransitionToDataProperty(Handle<JSReceiver> receiver, Handle<Object> value, PropertyAttributes attributes, StoreOrigin store_origin)
{
    ...
    DCHECK(state_ != LookupIterator::ACCESSOR || (GetAccessors()->IsAccessorInfo(isolate_) && AccessorInfo::cast(*GetAccessors()).is_special_data_property()));
...
void LookupIterator::ApplyTransitionToDataProperty(Handle<JSReceiver> receiver)
{
    ...
    if (!receiver->IsJSProxy(isolate_)) {
        JSObject::MigrateToMap(isolate_, Handle<JSObject>::cast(receiver), transition);
    }
    ...
    if (simple_transition) {
        ...
        if (receiver->map(isolate_).is_prototype_map() && receiver->IsJSObject(isolate_)) {
            JSObject::InvalidatePrototypeChains(receiver->map(isolate_));
        }
        if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
...
void LookupIterator::Delete()
{
    ...
    if (IsElement(*holder)) {
        ...
        if (holder->HasFastProperties(isolate_)) {
            JSObject::NormalizeProperties(isolate_, Handle<JSObject>::cast(holder), mode, 0, "DeletingProperty");
            ReloadPropertyInformation<false>();
        }
...
void LookupIterator::TransitionToAccessorProperty(Handle<Object> getter, Handle<Object> setter, PropertyAttributes attributes)
{
    ...
    if (!IsElement(*receiver) && !receiver->map(isolate_).is_dictionary_map()) {
        ...
        Handle<Map> new_map = Map::TransitionToAccessorProperty(isolate_, old_map, name_, number_, getter, setter, attributes);
...
void LookupIterator::TransitionToAccessorPair(Handle<Object> pair, PropertyAttributes attributes)
{
    ...
    if (IsElement(*receiver)) {
        ...
        if (receiver->HasSlowArgumentsElements(isolate_)) {
            SloppyArgumentsElements parameter_map = SloppyArgumentsElements::cast(receiver->elements(isolate_));
...
bool LookupIterator::HolderIsReceiverOrHiddenPrototype() const
{
    ...
    if (!receiver_->IsJSGlobalProxy(isolate_))
        return false;
    return Handle<JSGlobalProxy>::cast(receiver_)->map(isolate_).prototype(isolate_) == *holder_;
}
...
bool LookupIterator::IsConstFieldValueEqualTo(Object value) const
{
    ...
    if (property_details_.representation().IsDouble()) {
        ...
        return Object::SameNumberValue(base::bit_cast<double>(bits), value.Number());
    } else {
        Object current_value = holder->RawFastPropertyAt(isolate_, field_index);
        if (current_value.IsUninitialized(isolate()) || current_value == value) {
            return true;
        }
        return current_value.IsNumber(isolate_) && value.IsNumber(isolate_) && Object::SameNumberValue(current_value.Number(), value.Number());
    }
}
...
void LookupIterator::WriteDataValue(Handle<Object> value, bool initializing_store)
{
    DCHECK_EQ(DATA, state_);
    ...
    if (IsElement(*holder)) {
        ...
        if (property_details_.location() == PropertyLocation::kField) {
            ...
            // equal to |value|.
            DCHECK_IMPLIES(!initializing_store && property_details_.constness() == PropertyConstness::kConst, IsConstFieldValueEqualTo(*value));
        ...
        // cell.
#ifdef DEBUG
        GlobalDictionary dictionary = JSGlobalObject::cast(*holder).global_dictionary(isolate_, kAcquireLoad);
        PropertyCell cell = dictionary.CellAt(isolate_, dictionary_entry());
        DCHECK(cell.value() == *value || (cell.value().IsString() && value->IsString() && String::cast(cell.value()).Equals(String::cast(*value))));
#endif // DEBUG
    } else {
        DCHECK_IMPLIES(holder->IsJSProxy(isolate_), name()->IsPrivate(isolate_));
        ...
        if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
            SwissNameDictionary dictionary = holder->property_dictionary_swiss(isolate_);
            dictionary.ValueAtPut(dictionary_entry(), *value);
        } else {
...
Handle<Object> LookupIterator::SwapDataValue(Handle<Object> value, SeqCstAccessTag tag)
{
    DCHECK_EQ(DATA, state_);
    // Currently only shared structs and arrays support sequentially consistent
    // access.
...
template <bool is_element> bool LookupIterator::SkipInterceptor(JSObject holder)
{
    InterceptorInfo info = GetInterceptor<is_element>(holder);
    if (!is_element && name_->IsSymbol(isolate_) && !info.can_intercept_symbols()) {
        return true;
    }
    if (info.non_masking()) {
...
{
    if (!holder.IsJSTypedArray(isolate_))
        return NOT_FOUND;
    if (IsElement())
        return INTEGER_INDEXED_EXOTIC;
    if (!name_->IsString(isolate_))
        return NOT_FOUND;
    return IsSpecialIndex(String::cast(*name_)) ? INTEGER_INDEXED_EXOTIC : NOT_FOUND;
}

namespace {

template <bool is_element> bool HasInterceptor(Map map, size_t index)
{
    if (is_element) {
...
template <bool is_element> LookupIterator::State LookupIterator::LookupInSpecialHolder(Map const map, JSReceiver const holder)
{
    ...
    case NOT_FOUND:
        ...
        if (map.is_access_check_needed()) {
            if (is_element || !name_->IsPrivate(isolate_) || name_->IsPrivateName(isolate_))
                return ACCESS_CHECK;
        }
        V8_FALLTHROUGH;
    case ACCESS_CHECK:
        if (check_interceptor() && HasInterceptor<is_element>(map, index_) && !SkipInterceptor<is_element>(JSObject::cast(holder))) {
            if (is_element || !name_->IsPrivate(isolate_))
                return INTERCEPTOR;
        }
        V8_FALLTHROUGH;
    case INTERCEPTOR:
        if (map.IsJSGlobalObjectMap() && !is_js_array_element(is_element)) {
            GlobalDictionary dict = JSGlobalObject::cast(holder).global_dictionary(isolate_, kAcquireLoad);
...
template <bool is_element> LookupIterator::State LookupIterator::LookupInRegularHolder(Map const map, JSReceiver const holder)
{
    DisallowGarbageCollection no_gc;
    ...
    if (is_element && IsElement(holder)) {
        ...
        if (number_.is_not_found()) {
            return holder.IsJSTypedArray(isolate_) ? INTEGER_INDEXED_EXOTIC : NOT_FOUND;
        }
...
Handle<InterceptorInfo> LookupIterator::GetInterceptorForFailedAccessCheck() const
{
    ...
    if (!access_check_info.is_null()) {
        ...
        Object interceptor = is_js_array_element(IsElement()) ? access_check_info.indexed_interceptor() : access_check_info.named_interceptor();
...
bool LookupIterator::TryLookupCachedProperty()
{
    ...
    return accessor_pair->IsAccessorPair(isolate_) && LookupCachedProperty(Handle<AccessorPair>::cast(accessor_pair));
}
...
bool LookupIterator::LookupCachedProperty(Handle<AccessorPair> accessor_pair)
{
    ...
    base::Optional<Name> maybe_name = FunctionTemplateInfo::TryGetCachedPropertyName(isolate(), getter);
...
// static
base::Optional<Object> ConcurrentLookupIterator::TryGetOwnCowElement(Isolate* isolate, FixedArray array_elements, ElementsKind elements_kind, int array_length, size_t index)
{
    DisallowGarbageCollection no_gc;
...
// static
ConcurrentLookupIterator::Result ConcurrentLookupIterator::TryGetOwnConstantElement(
    Object* result_out, Isolate* isolate, LocalIsolate* local_isolate, JSObject holder, FixedArrayBase elements, ElementsKind elements_kind, size_t index)
{
    DisallowGarbageCollection no_gc;
    ...
    if (IsFrozenElementsKind(elements_kind)) {
        ...
        *result_out = result;
        ...
        return ConcurrentLookupIterator::TryGetOwnChar(static_cast<String*>(result_out), isolate, local_isolate, wrapped_string, index);
    } else {
...
// static
ConcurrentLookupIterator::Result ConcurrentLookupIterator::TryGetOwnChar(String* result_out, Isolate* isolate, LocalIsolate* local_isolate, String string, size_t index)
{
    DisallowGarbageCollection no_gc;
    ...
    uint16_t charcode;
    {
        ...
        charcode = string.Get(static_cast<int>(index), PtrComprCageBase(isolate), access_guard);
    }

    if (charcode > unibrow::Latin1::kMaxChar)
        return kGaveUp;

    Object value = isolate->factory()->single_character_string_table()->get(charcode, kRelaxedLoad);
...
// static
base::Optional<PropertyCell> ConcurrentLookupIterator::TryGetPropertyCell(Isolate* isolate, LocalIsolate* local_isolate, Handle<JSGlobalObject> holder, Handle<Name> name)
{
    DisallowGarbageCollection no_gc;
    ...
    base::Optional<PropertyCell> cell = dict.TryFindPropertyCellForConcurrentLookupIterator(isolate, name, kRelaxedLoad);
    if (!cell.has_value())
        return {};

    if (cell->property_details(kAcquireLoad).kind() == PropertyKind::kAccessor) {
        Object maybe_accessor_pair = cell->value(kAcquireLoad);
        if (!maybe_accessor_pair.IsAccessorPair())
            return {};

        base::Optional<Name> maybe_cached_property_name
            = FunctionTemplateInfo::TryGetCachedPropertyName(isolate, AccessorPair::cast(maybe_accessor_pair).getter(isolate, kAcquireLoad));
        if (!maybe_cached_property_name.has_value())
            return {};

        cell = dict.TryFindPropertyCellForConcurrentLookupIterator(isolate, handle(*maybe_cached_property_name, local_isolate), kRelaxedLoad);
        if (!cell.has_value())
            return {};
