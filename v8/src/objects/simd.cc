...
inline int32_t reinterpret_vmaxvq_u64(uint64x2_t v) { return vmaxvq_u32(vreinterpretq_u32_u64(v)); }
#endif
...
#else
template <typename T> uintptr_t fast_search_avx(T* array, uintptr_t array_len, uintptr_t index, T search_element) { UNREACHABLE(); }
...
template <ArrayIndexOfIncludesKind kind> Address ArrayIndexOfIncludes(Address array_start, uintptr_t array_len, uintptr_t from_index, Address search_element)
{
    ...
    if constexpr (kind == ArrayIndexOfIncludesKind::DOUBLE) {
        ...
        double* array = static_cast<double*>(fixed_array.RawField(FixedDoubleArray::OffsetOfElementAt(0)).ToVoidPtr());
    ...
    if constexpr (kind == ArrayIndexOfIncludesKind::OBJECTORSMI) {
        ...
        Tagged_t* array = static_cast<Tagged_t*>(fixed_array.data_start().ToVoidPtr());
