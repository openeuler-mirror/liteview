...
Handle<String> String::SlowFlatten(Isolate* isolate, Handle<ConsString> cons, AllocationType allocation)
{
    ...
    if (cons->IsOneByteRepresentation()) {
        ...
        if (V8_UNLIKELY(v8_flags.always_use_string_forwarding_table && !cons->IsConsString())) {
...
template <class StringClass> void MigrateExternalStringResource(Isolate* isolate, ExternalString from, StringClass to)
{
    ...
    if (to_resource_address == kNullAddress) {
        ...
        isolate->heap()->UpdateExternalString(from, ExternalString::cast(from).ExternalPayloadSize(), 0);
...
void MigrateExternalString(Isolate* isolate, String string, String internalized)
{
    if (internalized.IsExternalOneByteString()) {
        MigrateExternalStringResource(isolate, ExternalString::cast(string), ExternalOneByteString::cast(internalized));
    } else if (internalized.IsExternalTwoByteString()) {
        MigrateExternalStringResource(isolate, ExternalString::cast(string), ExternalTwoByteString::cast(internalized));
    } else {
...
template <typename IsolateT> Map ComputeThinStringMap(IsolateT* isolate, StringShape from_string_shape, bool one_byte)
{
    ReadOnlyRoots roots(isolate);
    if (from_string_shape.IsShared()) {
        return one_byte ? roots.shared_thin_one_byte_string_map() : roots.shared_thin_string_map();
    }
    return one_byte ? roots.thin_one_byte_string_map() : roots.thin_string_map();
}

void InitExternalPointerFieldsDuringExternalization(String string, Map new_map, Isolate* isolate)
{
    string.InitExternalPointerField<kExternalStringResourceTag>(ExternalString::kResourceOffset, isolate, kNullAddress);
    bool is_uncached = (new_map.instance_type() & kUncachedExternalStringMask) == kUncachedExternalStringTag;
    if (!is_uncached) {
        string.InitExternalPointerField<kExternalStringResourceDataTag>(ExternalString::kResourceDataOffset, isolate, kNullAddress);
    }
}

} // namespace

template <typename IsolateT> void String::MakeThin(IsolateT* isolate, String internalized, UpdateInvalidatedObjectSize update_invalidated_object_size)
{
    DisallowGarbageCollection no_gc;
    ...
    Map target_map = ComputeThinStringMap(isolate, initial_shape, internalized.IsOneByteRepresentation());
    if (initial_shape.IsExternal()) {
        ...
        // main-thread.
        isolate->AsIsolate()->heap()->NotifyObjectLayoutChange(*this, no_gc, InvalidateRecordedSlots::kYes, ThinString::kSize);
    ...
    if (size_delta != 0) {
        if (!Heap::IsLargeObject(thin)) {
            isolate->heap()->NotifyObjectSizeChange(
                thin, old_size, ThinString::kSize, may_contain_recorded_slots ? ClearRecordedSlots::kYes : ClearRecordedSlots::kNo, update_invalidated_object_size);
        } else {
...
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::MakeThin(Isolate* isolate, String internalized, UpdateInvalidatedObjectSize update_invalidated_object_size);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::MakeThin(LocalIsolate* isolate, String internalized, UpdateInvalidatedObjectSize update_invalidated_object_size);
...
template <bool is_one_byte> Map ComputeExternalStringMap(Isolate* isolate, String string, int size)
{
    ReadOnlyRoots roots(isolate);
    ...
    if constexpr (is_one_byte) {
        ...
        if (size < ExternalString::kSizeOfAllExternalStrings) {
            if (is_internalized) {
                ...
                return is_shared ? roots.shared_uncached_external_string_map() : roots.uncached_external_string_map();
            }
        } else {
            if (is_internalized) {
                return roots.external_internalized_string_map();
            } else {
                return is_shared ? roots.shared_external_string_map() : roots.external_string_map();
            }
        }
    }
}
...
bool String::MakeExternal(v8::String::ExternalStringResource* resource)
{
    ...
    base::SharedMutexGuard<base::kExclusive> shared_mutex_guard(isolate->internalized_string_access());
    // Morph the string to an external string by replacing the map and
    // reinitializing the fields.  This won't work if the space the existing
    // string occupies is too small for a regular external string.  Instead, we
    // resort to an uncached external string instead, omitting the field caching
    ...
    if (has_pointers) {
        isolate->heap()->NotifyObjectLayoutChange(*this, no_gc, InvalidateRecordedSlots::kYes, new_size);
    }
...
String::FlatContent String::SlowGetFlatContent(const DisallowGarbageCollection& no_gc, const SharedStringAccessGuardIfNeeded& access_guard)
{
    ...
    return TryGetFlatContentFromDirectString(cage_base, no_gc, string, offset, length(), access_guard).value();
}

std::unique_ptr<char[]> String::ToCString(AllowNullsFlag allow_nulls, RobustnessFlag robust_flag, int offset, int length, int* length_return)
{
...
std::unique_ptr<char[]> String::ToCString(AllowNullsFlag allow_nulls, RobustnessFlag robust_flag, int* length_return)
{
    return ToCString(allow_nulls, robust_flag, 0, -1, length_return);
}

// static
template <typename sinkchar> void String::WriteToFlat(String source, sinkchar* sink, int start, int length)
{
    DCHECK(!SharedStringAccessGuardIfNeeded::IsNeeded(source));
    return WriteToFlat(source, sink, start, length, GetPtrComprCageBase(source), SharedStringAccessGuardIfNeeded::NotNeeded());
}

// static
template <typename sinkchar>
void String::WriteToFlat(String source, sinkchar* sink, int start, int length, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard)
{
    DisallowGarbageCollection no_gc;
    if (length == 0)
        return;
    while (true) {
        ...
        case kOneByteStringTag | kExternalStringTag:
            CopyChars(sink, ExternalOneByteString::cast(source).GetChars(cage_base) + start, length);
            return;
        case kTwoByteStringTag | kExternalStringTag:
            CopyChars(sink, ExternalTwoByteString::cast(source).GetChars(cage_base) + start, length);
            return;
        case kOneByteStringTag | kSeqStringTag:
            CopyChars(sink, SeqOneByteString::cast(source).GetChars(no_gc, access_guard) + start, length);
            return;
        case kTwoByteStringTag | kSeqStringTag:
            CopyChars(sink, SeqTwoByteString::cast(source).GetChars(no_gc, access_guard) + start, length);
            return;
        ...
        case kTwoByteStringTag | kConsStringTag: {
            ...
            if (second_length >= first_length) {
                ...
                if (first_length > 0) {
                    WriteToFlat(first, sink, start, first_length, cage_base, access_guard);
                ...
                if (second_length > 0) {
                    ...
                    if (second_length == 1) {
                        sink[boundary - start] = static_cast<sinkchar>(second.Get(0, cage_base, access_guard));
                    } else if (second.IsSeqOneByteString(cage_base)) {
                        CopyChars(sink + boundary - start, SeqOneByteString::cast(second).GetChars(no_gc, access_guard), second_length);
                    } else {
                        WriteToFlat(second, sink + boundary - start, 0, second_length, cage_base, access_guard);
                    }
...
template <typename SourceChar> static void CalculateLineEndsImpl(std::vector<int>* line_ends, base::Vector<const SourceChar> src, bool include_ending_line)
{
...
bool String::SlowEquals(String other, const SharedStringAccessGuardIfNeeded& access_guard) const
{
    DisallowGarbageCollection no_gc;
    ...
    if (this->Get(0, cage_base, access_guard) != other.Get(0, cage_base, access_guard))
        return false;

    if (IsSeqOneByteString() && other.IsSeqOneByteString()) {
        const uint8_t* str1 = SeqOneByteString::cast(*this).GetChars(no_gc, access_guard);
...
Object String::IndexOf(Isolate* isolate, Handle<Object> receiver, Handle<Object> search, Handle<Object> position)
{
    if (receiver->IsNullOrUndefined(isolate)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kCalledOnNullOrUndefined, isolate->factory()->NewStringFromAsciiChecked("String.prototype.indexOf")));
    }
...
MaybeHandle<String> String::GetSubstitution(Isolate* isolate, Match* match, Handle<String> replacement, int start_index)
{
    ...
    while (true) {
        ...
        case '9': {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate, capture, match->GetCapture(scaled_index, &capture_exists), String);
            if (capture_exists)
        ...
        case '<': { // $<name> - named capture
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate, capture, match->GetNamedCapture(capture_name, &capture_state), String);
...
Object String::LastIndexOf(Isolate* isolate, Handle<Object> receiver, Handle<Object> search, Handle<Object> position)
{
    if (receiver->IsNullOrUndefined(isolate)) {
...
{
    DCHECK(!SharedStringAccessGuardIfNeeded::IsNeeded(*this));
    return IsEqualToImpl<EqualityType::kPrefix>(str, GetPtrComprCageBase(*this), SharedStringAccessGuardIfNeeded::NotNeeded());
}

namespace {

template <typename Char> bool IsIdentifierVector(const base::Vector<Char>& vec)
{
    if (vec.empty()) {
        return false;
    }
...
uint32_t HashString(String string, size_t start, int length, uint64_t seed, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard)
{
    DisallowGarbageCollection no_gc;
    ...
    if (string.IsConsString(cage_base)) {
        ...
        String::WriteToFlat(string, buffer.get(), 0, length, cage_base, access_guard);
        chars = buffer.get();
    } else {
...
uint32_t String::ComputeAndSetRawHash(const SharedStringAccessGuardIfNeeded& access_guard)
{
    ...
    uint32_t raw_hash_field = shape.encoding_tag() == kOneByteStringTag ? HashString<uint8_t>(string, start, length(), seed, cage_base, access_guard)
                                                                        : HashString<uint16_t>(string, start, length(), seed, cage_base, access_guard);
...
uint16_t ConsString::Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const
{
...
uint16_t ThinString::Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const
{
    return actual(cage_base).Get(index, cage_base, access_guard);
}

uint16_t SlicedString::Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const
{
...
const byte* String::AddressOfCharacterAt(int start_index, const DisallowGarbageCollection& no_gc)
{
    DCHECK(IsFlat());
    String subject = *this;
    ...
    case kOneByteStringTag | kSeqStringTag:
        return reinterpret_cast<const byte*>(SeqOneByteString::cast(subject).GetChars(no_gc) + start_index);
    case kTwoByteStringTag | kSeqStringTag:
        return reinterpret_cast<const byte*>(SeqTwoByteString::cast(subject).GetChars(no_gc) + start_index);
    case kOneByteStringTag | kExternalStringTag:
        return reinterpret_cast<const byte*>(ExternalOneByteString::cast(subject).GetChars(cage_base) + start_index);
    case kTwoByteStringTag | kExternalStringTag:
        return reinterpret_cast<const byte*>(ExternalTwoByteString::cast(subject).GetChars(cage_base) + start_index);
    default:
        UNREACHABLE();
    }
}

template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::WriteToFlat(String source, uint16_t* sink, int from, int to);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::WriteToFlat(String source, uint8_t* sink, int from, int to);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::WriteToFlat(
    String source, uint16_t* sink, int from, int to, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded&);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void String::WriteToFlat(
    String source, uint8_t* sink, int from, int to, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded&);
