...
bool DebugInfo::IsEmpty() const { return flags(kRelaxedLoad) == kNone && debugger_hints() == 0; }

bool DebugInfo::HasBreakInfo() const { return (flags(kRelaxedLoad) & kHasBreakInfo) != 0; }

DebugInfo::ExecutionMode DebugInfo::DebugExecutionMode() const { return (flags(kRelaxedLoad) & kDebugExecutionMode) != 0 ? kSideEffects : kBreakpoints; }
...
void DebugInfo::ClearBreakInfo(Isolate* isolate)
{
    if (HasInstrumentedBytecodeArray()) {
        ...
        {
            RedirectActiveFunctions redirect_visitor(shared(), RedirectActiveFunctions::Mode::kUseOriginalBytecode);
...
bool DebugInfo::BreakAtEntry() const { return (flags(kRelaxedLoad) & kBreakAtEntry) != 0; }

bool DebugInfo::CanBreakAtEntry() const { return (flags(kRelaxedLoad) & kCanBreakAtEntry) != 0; }
...
bool DebugInfo::ClearBreakPoint(Isolate* isolate, Handle<DebugInfo> debug_info, Handle<BreakPoint> break_point)
{
    DCHECK(debug_info->HasBreakInfo());
    for (int i = 0; i < debug_info->break_points().length(); i++) {
        if (debug_info->break_points().get(i).IsUndefined(isolate))
            continue;
        Handle<BreakPointInfo> break_point_info = Handle<BreakPointInfo>(BreakPointInfo::cast(debug_info->break_points().get(i)), isolate);
...
void DebugInfo::SetBreakPoint(Isolate* isolate, Handle<DebugInfo> debug_info, int source_position, Handle<BreakPoint> break_point)
{
    DCHECK(debug_info->HasBreakInfo());
    Handle<Object> break_point_info(debug_info->GetBreakPointInfo(isolate, source_position), isolate);
    if (!break_point_info->IsUndefined(isolate)) {
        BreakPointInfo::SetBreakPoint(isolate, Handle<BreakPointInfo>::cast(break_point_info), break_point);
        return;
    }
    ...
    if (index == kNoBreakPointInfo) {
        ...
        Handle<FixedArray> new_break_points = isolate->factory()->NewFixedArray(old_break_points->length() + DebugInfo::kEstimatedNofBreakPointsInFunction);
...
Handle<Object> DebugInfo::GetBreakPoints(Isolate* isolate, int source_position)
{
    ...
    return Handle<Object>(BreakPointInfo::cast(break_point_info).break_points(), isolate);
}
...
Handle<Object> DebugInfo::FindBreakPointInfo(Isolate* isolate, Handle<DebugInfo> debug_info, Handle<BreakPoint> break_point)
{
    DCHECK(debug_info->HasBreakInfo());
    for (int i = 0; i < debug_info->break_points().length(); i++) {
        if (!debug_info->break_points().get(i).IsUndefined(isolate)) {
...
bool DebugInfo::HasCoverageInfo() const { return (flags(kRelaxedLoad) & kHasCoverageInfo) != 0; }
...
namespace {
bool IsEqual(BreakPoint break_point1, BreakPoint break_point2) { return break_point1.id() == break_point2.id(); }
} // namespace

// Remove the specified break point object.
void BreakPointInfo::ClearBreakPoint(Isolate* isolate, Handle<BreakPointInfo> break_point_info, Handle<BreakPoint> break_point)
{
    ...
    if (!break_point_info->break_points().IsFixedArray()) {
        if (IsEqual(BreakPoint::cast(break_point_info->break_points()), *break_point)) {
    ...
    Handle<FixedArray> old_array = Handle<FixedArray>(FixedArray::cast(break_point_info->break_points()), isolate);
...
void BreakPointInfo::SetBreakPoint(Isolate* isolate, Handle<BreakPointInfo> break_point_info, Handle<BreakPoint> break_point)
{
...
bool BreakPointInfo::HasBreakPoint(Isolate* isolate, Handle<BreakPointInfo> break_point_info, Handle<BreakPoint> break_point)
{
    // No break point.
    if (break_point_info->break_points().IsUndefined(isolate)) {
        return false;
    ...
    // Single break point.
    if (!break_point_info->break_points().IsFixedArray()) {
        return IsEqual(BreakPoint::cast(break_point_info->break_points()), *break_point);
    }
...
MaybeHandle<BreakPoint> BreakPointInfo::GetBreakPointById(Isolate* isolate, Handle<BreakPointInfo> break_point_info, int breakpoint_id)
{
    // No break point.
    if (break_point_info->break_points().IsUndefined(isolate)) {
...
void CoverageInfo::ResetBlockCount(int slot_index) { set_slots_block_count(slot_index, 0); }
...
int StackFrameInfo::GetSourcePosition(Handle<StackFrameInfo> info)
{
    ...
    Handle<SharedFunctionInfo> shared(SharedFunctionInfo::cast(info->shared_or_script()), isolate);
...
// static
void ErrorStackData::EnsureStackFrameInfos(Isolate* isolate, Handle<ErrorStackData> error_stack)
{
    ...
    Handle<FixedArray> stack_frame_infos = isolate->factory()->NewFixedArray(call_site_infos->length());
    int index = 0;
    for (int i = 0; i < call_site_infos->length(); ++i) {
        Handle<CallSiteInfo> call_site_info(CallSiteInfo::cast(call_site_infos->get(i)), isolate);
        ...
        Handle<StackFrameInfo> stack_frame_info = isolate->factory()->NewStackFrameInfo(
            script, CallSiteInfo::GetSourcePosition(call_site_info), CallSiteInfo::GetFunctionDebugName(call_site_info), call_site_info->IsConstructor());
        stack_frame_infos->set(index++, *stack_frame_info);
    }
    stack_frame_infos = FixedArray::ShrinkOrEmpty(isolate, stack_frame_infos, index);
    if (limit < 0 && -limit < index) {
        // Negative limit encodes cap to be applied to |stack_frame_infos|.
        stack_frame_infos = FixedArray::ShrinkOrEmpty(isolate, stack_frame_infos, -limit);
        ...
        call_site_infos = FixedArray::ShrinkOrEmpty(isolate, call_site_infos, limit);
...
// static
MaybeHandle<JSObject> PromiseOnStack::GetPromise(Handle<PromiseOnStack> promise_on_stack)
{
    HeapObject promise;
