...
UNumberFormatRoundingMode ToUNumberFormatRoundingMode(IntlRoundingMode rounding_mode)
{
...
bool IsAToZ(char ch) { return base::IsInRange(AsciiAlphaToLower(ch), 'a', 'z'); }
...
bool UseGroupingFromSkeleton(const icu::UnicodeString& skeleton) { return skeleton.indexOf("group-off") == -1; }
...
Handle<String> RoundingModeString(Isolate* isolate, const icu::UnicodeString& skeleton)
{
...
Handle<Object> RoundingIncrement(Isolate* isolate, const icu::UnicodeString& skeleton)
{
...
Handle<String> RoundingPriorityString(Isolate* isolate, const icu::UnicodeString& skeleton)
{
    int32_t found;
...
Handle<String> TrailingZeroDisplayString(Isolate* isolate, const icu::UnicodeString& skeleton)
{
    int32_t found;
...
icu::number::UnlocalizedNumberFormatter SetDigitOptionsToFormatterV2(const icu::number::UnlocalizedNumberFormatter& settings, const Intl::NumberFormatDigitOptions& digit_options)
{
    ...
    icu::number::Precision precision = (digit_options.minimum_significant_digits > 0)
        ? icu::number::Precision::minMaxSignificantDigits(digit_options.minimum_significant_digits, digit_options.maximum_significant_digits)
        : icu::number::Precision::minMaxFraction(digit_options.minimum_fraction_digits, digit_options.maximum_fraction_digits);

    return result.precision(precision);
}

icu::number::UnlocalizedNumberFormatter SetDigitOptionsToFormatterV3(const icu::number::UnlocalizedNumberFormatter& settings, const Intl::NumberFormatDigitOptions& digit_options,
    int rounding_increment, JSNumberFormat::ShowTrailingZeros trailing_zeros)
{
    icu::number::UnlocalizedNumberFormatter result = settings;
    if (digit_options.minimum_integer_digits > 1) {
        result = result.integerWidth(icu::number::IntegerWidth::zeroFillTo(digit_options.minimum_integer_digits));
    ...
    if (rounding_increment != 1) {
        precision = ::icu::number::Precision::incrementExact(rounding_increment, -digit_options.maximum_fraction_digits).withMinFraction(digit_options.minimum_fraction_digits);
    }
...
icu::number::UnlocalizedNumberFormatter JSNumberFormat::SetDigitOptionsToFormatter(const icu::number::UnlocalizedNumberFormatter& settings,
    const Intl::NumberFormatDigitOptions& digit_options, int rounding_increment, JSNumberFormat::ShowTrailingZeros trailing_zeros)
{
    if (v8_flags.harmony_intl_number_format_v3) {
        return SetDigitOptionsToFormatterV3(settings, digit_options, rounding_increment, trailing_zeros);
    } else {
...
Handle<JSObject> JSNumberFormat::ResolvedOptions(Isolate* isolate, Handle<JSNumberFormat> number_format)
{
    Factory* factory = isolate->factory();
    ...
    if (!currency_ustr.isEmpty()) {
        ...
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->currencyDisplay_string(), CurrencyDisplayString(isolate, skeleton), Just(kDontThrow)).FromJust());
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->currencySign_string(), CurrencySignString(isolate, skeleton), Just(kDontThrow)).FromJust());
    }
    ...
    if (style == Style::UNIT) {
        ...
        if (!unit.empty()) {
            CHECK(
                JSReceiver::CreateDataProperty(isolate, options, factory->unit_string(), isolate->factory()->NewStringFromAsciiChecked(unit.c_str()), Just(kDontThrow)).FromJust());
        }
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->unitDisplay_string(), UnitDisplayString(isolate, skeleton), Just(kDontThrow)).FromJust());
    }
    ...
    bool has_significant_digits = SignificantDigitsFromSkeleton(skeleton, &mnsd, &mxsd);
    if (has_significant_digits) {
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->minimumSignificantDigits_string(), factory->NewNumberFromInt(mnsd), Just(kDontThrow)).FromJust());
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->maximumSignificantDigits_string(), factory->NewNumberFromInt(mxsd), Just(kDontThrow)).FromJust());
    }
    if ((v8_flags.harmony_intl_number_format_v3 || !has_significant_digits)) {
        if (FractionDigitsFromSkeleton(skeleton, &mnfd, &mxfd)) {
            CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->minimumFractionDigits_string(), factory->NewNumberFromInt(mnfd), Just(kDontThrow)).FromJust());
            CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->maximumFractionDigits_string(), factory->NewNumberFromInt(mxfd), Just(kDontThrow)).FromJust());
        }
    }

    if (v8_flags.harmony_intl_number_format_v3) {
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->useGrouping_string(), UseGroupingFromSkeleton(isolate, skeleton), Just(kDontThrow)).FromJust());
    } else {
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->useGrouping_string(), factory->ToBoolean(UseGroupingFromSkeleton(skeleton)), Just(kDontThrow)).FromJust());
    }
    ...
    if (notation == Notation::COMPACT) {
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->compactDisplay_string(), CompactDisplayString(isolate, skeleton), Just(kDontThrow)).FromJust());
    }
    ...
    if (v8_flags.harmony_intl_number_format_v3) {
        ...
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->roundingIncrement_string(), RoundingIncrement(isolate, skeleton), Just(kDontThrow)).FromJust());
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->trailingZeroDisplay_string(), TrailingZeroDisplayString(isolate, skeleton), Just(kDontThrow)).FromJust());
        CHECK(JSReceiver::CreateDataProperty(isolate, options, factory->roundingPriority_string(), RoundingPriorityString(isolate, skeleton), Just(kDontThrow)).FromJust());
    }
    return options;
}
...
MaybeHandle<JSNumberFormat> JSNumberFormat::UnwrapNumberFormat(Isolate* isolate, Handle<JSReceiver> format_holder)
{
    ...
    Handle<JSFunction> constructor = Handle<JSFunction>(JSFunction::cast(native_context->intl_number_format_function()), isolate);
    Handle<Object> object;
    ASSIGN_RETURN_ON_EXCEPTION(isolate, object, Intl::LegacyUnwrapReceiver(isolate, format_holder, constructor, format_holder->IsJSNumberFormat()), JSNumberFormat);
    // 4. If ... or nf does not have an [[InitializedNumberFormat]] internal slot,
    // then
    if (!object->IsJSNumberFormat()) {
        // a. Throw a TypeError exception.
        THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kIncompatibleMethodReceiver, isolate->factory()->NewStringFromAsciiChecked("UnwrapNumberFormat")), JSNumberFormat);
    }
    // 5. Return nf.
...
{
    return value == 1 || value == 2 || value == 5 || value == 10 || value == 20 || value == 25 || value == 50 || value == 100 || value == 200 || value == 250 || value == 500
        || value == 1000 || value == 2000 || value == 2500 || value == 5000;
}
// static
MaybeHandle<JSNumberFormat> JSNumberFormat::New(Isolate* isolate, Handle<Map> map, Handle<Object> locales, Handle<Object> options_obj, const char* service)
{
    Factory* factory = isolate->factory();
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, options, CoerceOptionsToObject(isolate, options_obj, service), JSNumberFormat);
    ...
    Maybe<Intl::ResolvedLocale> maybe_resolve_locale = Intl::ResolveLocale(isolate, JSNumberFormat::GetAvailableLocales(), requested_locales, matcher, relevant_extension_keys);
    if (maybe_resolve_locale.IsNothing()) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kIcuError), JSNumberFormat);
    }
    ...
    Maybe<Style> maybe_style = GetStringOption<Style>(
        isolate, options, "style", service, { "decimal", "percent", "currency", "unit" }, { Style::DECIMAL, Style::PERCENT, Style::CURRENCY, Style::UNIT }, Style::DECIMAL);
    ...
    if (found_currency.FromJust()) {
        ...
        if (!IsWellFormedCurrencyCode(currency)) {
            THROW_NEW_ERROR(isolate,
                NewRangeError(MessageTemplate::kInvalid, factory->NewStringFromStaticChars("currency code"), factory->NewStringFromAsciiChecked(currency.c_str())), JSNumberFormat);
        }
    } else {
        // 7. If style is "currency" and currency is undefined, throw a TypeError
        // exception.
        if (style == Style::CURRENCY) {
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kCurrencyCode), JSNumberFormat);
        }
    }
    // 8. Let currencyDisplay be ? GetOption(options, "currencyDisplay",
    // "string", « "code",  "symbol", "name", "narrowSymbol" », "symbol").
    Maybe<CurrencyDisplay> maybe_currency_display = GetStringOption<CurrencyDisplay>(isolate, options, "currencyDisplay", service, { "code", "symbol", "name", "narrowSymbol" },
    ...
    if (found_unit.FromJust()) {
        ...
        if (maybe_wellformed_unit.IsNothing()) {
            THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kInvalidUnit, factory->NewStringFromAsciiChecked(service), factory->NewStringFromAsciiChecked(unit.c_str())),
                JSNumberFormat);
        }
        ...
        // exception.
        if (style == Style::UNIT) {
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kInvalidUnit, factory->NewStringFromAsciiChecked(service), factory->empty_string()), JSNumberFormat);
        }
    }
    ...
    Maybe<UnitDisplay> maybe_unit_display = GetStringOption<UnitDisplay>(
        isolate, options, "unitDisplay", service, { "short", "narrow", "long" }, { UnitDisplay::SHORT, UnitDisplay::NARROW, UnitDisplay::LONG }, UnitDisplay::SHORT);
    ...
    if (style == Style::CURRENCY) {
        ...
        if (!currency_ustr.isEmpty()) {
            Handle<String> currency_string;
            ASSIGN_RETURN_ON_EXCEPTION(isolate, currency_string, Intl::ToString(isolate, currency_ustr), JSNumberFormat);
    ...
    Maybe<Intl::NumberFormatDigitOptions> maybe_digit_options = Intl::SetNumberFormatDigitOptions(isolate, options, mnfd_default, mxfd_default, notation == Notation::COMPACT);
    ...
    if (v8_flags.harmony_intl_number_format_v3) {
        ...
        if (!IsValidRoundingIncrement(rounding_increment)) {
            THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kPropertyValueOutOfRange, factory->roundingIncrement_string()), JSNumberFormat);
        }
        if (rounding_increment != 1) {
            ...
            if (digit_options.rounding_type != Intl::RoundingType::kFractionDigits) {
                THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kBadRoundingType), JSNumberFormat);
            }
            ...
            if (digit_options.maximum_fraction_digits != digit_options.minimum_fraction_digits) {
                THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kMaximumFractionDigitsNotEqualMinimumFractionDigits), JSNumberFormat);
            }
        }
        ...
        Maybe<TrailingZeroDisplay> maybe_trailing_zero_display = GetStringOption<TrailingZeroDisplay>(isolate, options, "trailingZeroDisplay", service,
            { "auto", "stripIfInteger" }, { TrailingZeroDisplay::AUTO, TrailingZeroDisplay::STRIP_IF_INTEGER }, TrailingZeroDisplay::AUTO);
        MAYBE_RETURN(maybe_trailing_zero_display, MaybeHandle<JSNumberFormat>());
        TrailingZeroDisplay trailing_zero_display = maybe_trailing_zero_display.FromJust();

        // 27. Set numberFormat.[[TrailingZeroDisplay]] to trailingZeroDisplay.
        settings = SetDigitOptionsToFormatterV3(
            settings, digit_options, rounding_increment, trailing_zero_display == TrailingZeroDisplay::STRIP_IF_INTEGER ? ShowTrailingZeros::kHide : ShowTrailingZeros::kShow);
    } else {
    ...
    Maybe<CompactDisplay> maybe_compact_display
        = GetStringOption<CompactDisplay>(isolate, options, "compactDisplay", service, { "short", "long" }, { CompactDisplay::SHORT, CompactDisplay::LONG }, CompactDisplay::SHORT);
    ...
    if (!v8_flags.harmony_intl_number_format_v3) {
        ...
        Maybe<bool> found_use_grouping = GetBoolOption(isolate, options, "useGrouping", service, &use_grouping);
        ...
        settings = JSNumberFormat::SetDigitOptionsToFormatter(settings, digit_options, 1, ShowTrailingZeros::kShow);
    } else {
    ...
    if (v8_flags.harmony_intl_number_format_v3) {
        ...
        maybe_sign_display = GetStringOption<SignDisplay>(isolate, options, "signDisplay", service, { "auto", "never", "always", "exceptZero" },
            { SignDisplay::AUTO, SignDisplay::NEVER, SignDisplay::ALWAYS, SignDisplay::EXCEPT_ZERO }, SignDisplay::AUTO);
    }
    ...
    if (v8_flags.harmony_intl_number_format_v3) {
        ...
        // "halfExpand").
        Maybe<IntlRoundingMode> maybe_rounding_mode = GetStringOption<IntlRoundingMode>(isolate, options, "roundingMode", service,
            { "ceil", "floor", "expand", "trunc", "halfCeil", "halfFloor", "halfExpand", "halfTrunc", "halfEven" },
            { IntlRoundingMode::CEIL, IntlRoundingMode::FLOOR, IntlRoundingMode::EXPAND, IntlRoundingMode::TRUNC, IntlRoundingMode::HALF_CEIL, IntlRoundingMode::HALF_FLOOR,
                IntlRoundingMode::HALF_EXPAND, IntlRoundingMode::HALF_TRUNC, IntlRoundingMode::HALF_EVEN },
    ...
    Handle<Managed<icu::number::LocalizedNumberFormatter>> managed_number_formatter
        = Managed<icu::number::LocalizedNumberFormatter>::FromRawPtr(isolate, 0, new icu::number::LocalizedNumberFormatter(fmt));

    // Now all properties are ready, so we can allocate the result object.
    Handle<JSNumberFormat> number_format = Handle<JSNumberFormat>::cast(isolate->factory()->NewFastOrSlowJSObjectFromMap(map));
...
icu::number::FormattedNumber FormatDecimalString(Isolate* isolate, const icu::number::LocalizedNumberFormatter& number_format, Handle<String> string, UErrorCode& status)
{
...
Maybe<icu::number::FormattedNumber> IcuFormatNumber(Isolate* isolate, const icu::number::LocalizedNumberFormatter& number_format, Handle<Object> numeric_obj)
{
    ...
    if (numeric_obj->IsBigInt()) {
        ...
        if (v8_flags.harmony_intl_number_format_v3 && numeric_obj->IsString()) {
            ...
            Handle<String> string = String::Flatten(isolate, Handle<String>::cast(numeric_obj));
            DisallowGarbageCollection no_gc;
            const String::FlatContent& flat = string->GetFlatContent(no_gc);
            int32_t length = string->length();
            if (flat.IsOneByte()) {
                ...
                formatted = number_format.formatDecimal({ string->ToCString().get(), string->length() }, status);
            }
        } else {
            double number = numeric_obj->IsNaN() ? std::numeric_limits<double>::quiet_NaN() : numeric_obj->Number();
...
template <typename Char> std::pair<int, int> FindLeadingAndTrailingWhiteSpaceOrLineTerminator(const base::Vector<const Char>& src)
{
...
Maybe<IntlMathematicalValue> IntlMathematicalValue::From(Isolate* isolate, Handle<Object> value)
{
    ...
    if (value->IsJSReceiver()) {
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(
            isolate, prim_value, JSReceiver::ToPrimitive(isolate, Handle<JSReceiver>::cast(value), ToPrimitiveHint::kNumber), Nothing<IntlMathematicalValue>());
    } else {
        prim_value = value;
    }
    ...
    if (2 < string->length() && string->Get(0) == '0') {
        ...
        if (ch == 'b' || ch == 'B' || ch == 'o' || ch == 'O' || ch == 'x' || ch == 'X') {
            result.approx_ = StringToDouble(isolate, string, ALLOW_HEX | ALLOW_OCTAL | ALLOW_BINARY, 0);
...
namespace {
Maybe<int> ConstructParts(Isolate* isolate, const icu::FormattedValue& formatted, Handle<JSArray> result, int start_index, bool style_is_unit, bool is_nan, bool output_source)
{
    UErrorCode status = U_ZERO_ERROR;
...
MaybeHandle<T> PartitionNumberRangePattern(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> start, Handle<Object> end, const char* func_name)
{
    Factory* factory = isolate->factory();
...
MaybeHandle<JSArray> FormatToJSArray(Isolate* isolate, const icu::FormattedValue& formatted, const icu::number::LocalizedNumberFormatter& nfmt, bool is_nan, bool output_source)
{
    UErrorCode status = U_ZERO_ERROR;
    ...
    int format_to_parts;
    MAYBE_ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, format_to_parts, ConstructParts(isolate, formatted, result, 0, is_unit, is_nan, output_source), Handle<JSArray>());
    USE(format_to_parts);

    return result;
}

MaybeHandle<JSArray> FormatRangeToJSArray(Isolate* isolate, const icu::FormattedValue& formatted, const icu::number::LocalizedNumberFormatter& nfmt, bool is_nan)
{
...
Maybe<icu::number::LocalizedNumberRangeFormatter> JSNumberFormat::GetRangeFormatter(Isolate* isolate, String locale, const icu::number::LocalizedNumberFormatter& number_formatter)
{
    UErrorCode status = U_ZERO_ERROR;
    UParseError perror;
...
MaybeHandle<String> JSNumberFormat::FormatNumeric(Isolate* isolate, const icu::number::LocalizedNumberFormatter& number_format, Handle<Object> numeric_obj)
{
    ...
    return FormatToString(isolate, formatted, number_format, numeric_obj->IsNaN());
}

namespace {
MaybeHandle<String> NumberFormatFunctionV2(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> value)
{
    icu::number::LocalizedNumberFormatter* fmt = number_format->icu_number_formatter().raw();
    CHECK_NOT_NULL(fmt);
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, x, Object::ToNumeric(isolate, value), String);
    // 5. Return FormatNumeric(nf, x).
    return JSNumberFormat::FormatNumeric(isolate, *fmt, x);
}

MaybeHandle<String> NumberFormatFunctionV3(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> value)
{
    icu::number::LocalizedNumberFormatter* fmt = number_format->icu_number_formatter().raw();
    CHECK_NOT_NULL(fmt);
...
MaybeHandle<JSArray> FormatToPartsV2(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> numeric_obj)
{
    icu::number::LocalizedNumberFormatter* fmt = number_format->icu_number_formatter().raw();
    CHECK_NOT_NULL(fmt);
    ASSIGN_RETURN_ON_EXCEPTION(isolate, numeric_obj, Object::ToNumeric(isolate, numeric_obj), JSArray);

    Maybe<icu::number::FormattedNumber> maybe_formatted = IcuFormatNumber(isolate, *fmt, numeric_obj);
...
MaybeHandle<JSArray> FormatToPartsV3(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> numeric_obj)
{
    icu::number::LocalizedNumberFormatter* fmt = number_format->icu_number_formatter().raw();
    DCHECK_NOT_NULL(fmt);
...
MaybeHandle<String> JSNumberFormat::NumberFormatFunction(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> value)
{
    if (v8_flags.harmony_intl_number_format_v3) {
...
MaybeHandle<JSArray> JSNumberFormat::FormatToParts(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> numeric_obj)
{
    if (v8_flags.harmony_intl_number_format_v3) {
...
MaybeHandle<String> JSNumberFormat::FormatNumericRange(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> x_obj, Handle<Object> y_obj)
{
...
MaybeHandle<JSArray> JSNumberFormat::FormatNumericRangeToParts(Isolate* isolate, Handle<JSNumberFormat> number_format, Handle<Object> x_obj, Handle<Object> y_obj)
{
