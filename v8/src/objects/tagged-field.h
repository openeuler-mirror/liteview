...
public:
    static_assert(std::is_base_of<Object, T>::value || std::is_same<MapWord, T>::value || std::is_same<MaybeObject, T>::value, "T must be strong or weak tagged type or MapWord");
    ...
    // decompression.
    static constexpr bool kIsHeapObject = std::is_base_of<HeapObject, T>::value || std::is_same<MapWord, T>::value;

    static inline Address address(HeapObject host, int offset = 0);

    static inline T load(HeapObject host, int offset = 0);
    static inline T load(PtrComprCageBase cage_base, HeapObject host, int offset = 0);
    ...
    static inline T Relaxed_Load(PtrComprCageBase cage_base, HeapObject host, int offset = 0);
    ...
    static inline T Acquire_Load_No_Unpack(PtrComprCageBase cage_base, HeapObject host, int offset = 0);
    static inline T Acquire_Load(PtrComprCageBase cage_base, HeapObject host, int offset = 0);

    static inline T SeqCst_Load(HeapObject host, int offset = 0);
    static inline T SeqCst_Load(PtrComprCageBase cage_base, HeapObject host, int offset = 0);
    ...
    static inline T SeqCst_Swap(PtrComprCageBase cage_base, HeapObject host, int offset, T value);

    static inline Tagged_t Release_CompareAndSwap(HeapObject host, T old, T value);

    // Note: Use these *_Map_Word methods only when loading a MapWord from a
    // MapField.
    static inline T Relaxed_Load_Map_Word(PtrComprCageBase cage_base, HeapObject host);
