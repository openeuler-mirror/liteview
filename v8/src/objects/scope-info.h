...
public:
    ...
    static inline LocalNamesRange<ScopeInfo*> IterateLocalNames(ScopeInfo* scope_info, const DisallowGarbageCollection& no_gc);
    ...
    // is never 0).
    int ModuleIndex(String name, VariableMode* mode, InitializationFlag* init_flag, MaybeAssignedFlag* maybe_assigned_flag);
...
#ifdef DEBUG
    bool Equals(ScopeInfo other, bool ignore_position_and_module_info = false) const;
#endif
    ...
    V8_EXPORT_PRIVATE static Handle<ScopeInfo> RecreateWithBlockList(Isolate* isolate, Handle<ScopeInfo> original, Handle<StringSet> blocklist);
...
#define FIELD_ACCESSORS(name) inline int name() const;
...
private:
    ...
    void set(int index, Object value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    void CopyElements(Isolate* isolate, int dst_index, ScopeInfo src, int src_index, int len, WriteBarrierMode mode);
    ...
    // HeapObject header).
    static constexpr int OffsetOfElementAt(int index) { return HeapObject::kHeaderSize + index * kTaggedSize; }
    ...
    void ModuleVariable(int i, String* name, int* index, VariableMode* mode = nullptr, InitializationFlag* init_flag = nullptr, MaybeAssignedFlag* maybe_assigned_flag = nullptr);

    static const int kFunctionNameEntries = TorqueGeneratedFunctionVariableInfoOffsets::kSize / kTaggedSize;
    static const int kPositionInfoEntries = TorqueGeneratedPositionInfoOffsets::kSize / kTaggedSize;
