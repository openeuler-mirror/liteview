...
public:
    ...
    static Managed unchecked_cast(Object obj) { return base::bit_cast<Managed>(obj); }

    // Allocate a new {CppType} and wrap it in a {Managed<CppType>}.
    template <typename... Args> static Handle<Managed<CppType>> Allocate(Isolate* isolate, size_t estimated_size, Args&&... args);

    // Create a {Managed<CppType>} from an existing raw {CppType*}. The returned
    // object will now own the memory pointed to by {CppType}.
    static Handle<Managed<CppType>> FromRawPtr(Isolate* isolate, size_t estimated_size, CppType* ptr);
    ...
    static Handle<Managed<CppType>> FromUniquePtr(
        Isolate* isolate, size_t estimated_size, std::unique_ptr<CppType> unique_ptr, AllocationType allocation_type = AllocationType::kYoung);
