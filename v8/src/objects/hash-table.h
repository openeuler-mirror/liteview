...
template <typename Derived, typename Shape> class EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) HashTable : public HashTableBase {
public:
    using ShapeT = Shape;
    ...
    V8_WARN_UNUSED_RESULT static Handle<Derived> New(
        IsolateT* isolate, int at_least_space_for, AllocationType allocation = AllocationType::kYoung, MinimumCapacity capacity_option = USE_DEFAULT_MINIMUM_CAPACITY);
    ...
    inline bool ToKey(PtrComprCageBase cage_base, InternalIndex entry, Object* out_k);
    ...
    inline Object KeyAt(PtrComprCageBase cage_base, InternalIndex entry, RelaxedLoadTag tag);

    inline void SetKeyAt(InternalIndex entry, Object value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    static constexpr inline int EntryToIndex(InternalIndex entry) { return (entry.as_int() * kEntrySize) + kElementsStartIndex; }

    // Returns the entry for an index (of the key)
    static constexpr inline InternalIndex IndexToEntry(int index) { return InternalIndex((index - kElementsStartIndex) / kEntrySize); }

    // Returns the index for a slot address in the object.
    static constexpr inline int SlotToIndex(Address object, Address slot) { return static_cast<int>((slot - object - kHeaderSize) / kTaggedSize); }

    // Ensure enough space for n additional elements.
    template <typename IsolateT>
    V8_WARN_UNUSED_RESULT static Handle<Derived> EnsureCapacity(IsolateT* isolate, Handle<Derived> table, int n = 1, AllocationType allocation = AllocationType::kYoung);
...
private:
    ...
    static_assert(EntryToIndex(InternalIndex(kMaxRegularCapacity)) < kMaxRegularLength);
    ...
    // number _probe_.
    InternalIndex EntryForProbe(ReadOnlyRoots roots, Object k, int probe, InternalIndex expected);
...
#define EXTERN_DECLARE_HASH_TABLE(DERIVED, SHAPE)                                                                                                                                  \
    ...
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::New(LocalIsolate*, int, AllocationType, MinimumCapacity);                \
                                                                                                                                                                                   \
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::EnsureCapacity(Isolate*, Handle<DERIVED>, int, AllocationType);          \
    extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<DERIVED> HashTable<DERIVED, SHAPE>::EnsureCapacity(LocalIsolate*, Handle<DERIVED>, int, AllocationType);
...
public:
    ...
    static inline int EntryToValueIndex(InternalIndex entry) { return HashTable<Derived, Shape>::EntryToIndex(entry) + Shape::kEntryValueIndex; }
...
public:
    ...
    OBJECT_CONSTRUCTORS(ObjectHashTable, ObjectHashTableBase<ObjectHashTable, ObjectHashTableShape>);
};
...
protected:
    ...
    OBJECT_CONSTRUCTORS(EphemeronHashTable, ObjectHashTableBase<EphemeronHashTable, ObjectHashTableShape>);
};
...
public:
    ...
    OBJECT_CONSTRUCTORS(ObjectHashSet, HashTable<ObjectHashSet, ObjectHashSetShape>);
};
...
public:
    ...
    template <typename IsolateT> static Handle<NameToIndexHashTable> Add(IsolateT* isolate, Handle<NameToIndexHashTable> table, Handle<Name> key, int32_t value);
    ...
    OBJECT_CONSTRUCTORS(NameToIndexHashTable, HashTable<NameToIndexHashTable, NameToIndexShape>);

private:
    static inline int EntryToValueIndex(InternalIndex entry) { return EntryToIndex(entry) + NameToIndexShape::kEntryValueIndex; }
};
...
public:
    ...
    static Handle<RegisteredSymbolTable> Add(Isolate* isolate, Handle<RegisteredSymbolTable> table, Handle<String> key, Handle<Symbol>);

    DECL_CAST(RegisteredSymbolTable)
    DECL_PRINTER(RegisteredSymbolTable)
    OBJECT_CONSTRUCTORS(RegisteredSymbolTable, HashTable<RegisteredSymbolTable, RegisteredSymbolTableShape>);

private:
    static inline int EntryToValueIndex(InternalIndex entry) { return EntryToIndex(entry) + RegisteredSymbolTableShape::kEntryValueIndex; }
};
