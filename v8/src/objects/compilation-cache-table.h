...
public:
    static inline bool IsMatch(HashTableKey* key, Object value) { return key->IsMatch(value); }

    static inline uint32_t Hash(ReadOnlyRoots roots, HashTableKey* key) { return key->Hash(); }

    static inline uint32_t RegExpHash(String string, Smi flags);

    static inline uint32_t EvalHash(String source, SharedFunctionInfo shared, LanguageMode language_mode, int position);
...
public:
    ...
    inline InfoCellPair(Isolate* isolate, SharedFunctionInfo shared, FeedbackCell feedback_cell);
    ...
    bool has_feedback_cell() const { return !feedback_cell_.is_null() && is_compiled_scope_.is_compiled(); }
...
public:
    ...
    static CompilationCacheScriptLookupResult LookupScript(Handle<CompilationCacheTable> table, Handle<String> src, const ScriptDetails& script_details, Isolate* isolate);
    static Handle<CompilationCacheTable> PutScript(Handle<CompilationCacheTable> cache, Handle<String> src, Handle<SharedFunctionInfo> value, Isolate* isolate);
    ...
    // never needed later.
    static InfoCellPair LookupEval(
        Handle<CompilationCacheTable> table, Handle<String> src, Handle<SharedFunctionInfo> shared, Handle<Context> native_context, LanguageMode language_mode, int position);
    static Handle<CompilationCacheTable> PutEval(Handle<CompilationCacheTable> cache, Handle<String> src, Handle<SharedFunctionInfo> outer_info, Handle<SharedFunctionInfo> value,
        Handle<Context> native_context, Handle<FeedbackCell> feedback_cell, int position);
    ...
    inline void SetPrimaryValueAt(InternalIndex entry, Object value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    inline Object EvalFeedbackValueAt(InternalIndex entry);
    inline void SetEvalFeedbackValueAt(InternalIndex entry, Object value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
private:
    ...
    OBJECT_CONSTRUCTORS(CompilationCacheTable, HashTable<CompilationCacheTable, CompilationCacheShape>);
};
