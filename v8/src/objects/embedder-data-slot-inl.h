...
void EmbedderDataSlot::Initialize(Object initial_value)
{
    ...
    DCHECK(initial_value.IsSmi() || ReadOnlyHeap::Contains(HeapObject::cast(initial_value)));
...
Object EmbedderDataSlot::load_tagged() const { return ObjectSlot(address() + kTaggedPayloadOffset).Relaxed_Load(); }
...
// static
void EmbedderDataSlot::store_tagged(EmbedderDataArray array, int entry_index, Object value)
{
...
// static
void EmbedderDataSlot::store_tagged(JSObject object, int embedder_field_index, Object value)
{
...
bool EmbedderDataSlot::ToAlignedPointer(Isolate* isolate, void** out_pointer) const
{
...
bool EmbedderDataSlot::store_aligned_pointer(Isolate* isolate, void* ptr)
{
    ...
    WriteLazilyInitializedExternalPointerField<kEmbedderDataSlotPayloadTag>(address() + kExternalPointerOffset, isolate, value);
...
EmbedderDataSlot::RawData EmbedderDataSlot::load_raw(Isolate* isolate, const DisallowGarbageCollection& no_gc) const
{
    // We don't care about atomicity of access here because embedder slots
...
void EmbedderDataSlot::store_raw(Isolate* isolate, EmbedderDataSlot::RawData data, const DisallowGarbageCollection& no_gc) { gc_safe_store(isolate, data); }
...
// static
void EmbedderDataSlot::PopulateEmbedderDataSnapshot(Map map, JSObject js_object, int entry_index, EmbedderDataSlotSnapshot& snapshot)
{
#ifdef V8_COMPRESS_POINTERS
    ...
    const Address field_base = FIELD_ADDR(js_object, js_object.GetEmbedderFieldOffset(entry_index));
    ...
    reinterpret_cast<AtomicTagged_t*>(&snapshot)[index] = AsAtomicTagged::Relaxed_Load(reinterpret_cast<AtomicTagged_t*>(field_base + kTaggedPayloadOffset));
#ifdef V8_COMPRESS_POINTERS
    reinterpret_cast<AtomicTagged_t*>(&snapshot)[1 - index] = AsAtomicTagged::Relaxed_Load(reinterpret_cast<AtomicTagged_t*>(field_base + kRawPayloadOffset));
#endif // V8_COMPRESS_POINTERS
}
