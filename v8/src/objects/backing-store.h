...
public:
    ...
    static std::unique_ptr<BackingStore> AllocateWasmMemory(Isolate* isolate, size_t initial_pages, size_t maximum_pages, WasmMemoryFlag wasm_memory, SharedFlag shared);
    ...
    static std::unique_ptr<BackingStore> WrapAllocation(Isolate* isolate, void* allocation_base, size_t allocation_length, SharedFlag shared, bool free_on_destruct);
    ...
    size_t byte_length(std::memory_order memory_order = std::memory_order_relaxed) const { return byte_length_.load(memory_order); }
    ...
    bool CanReallocate() const { return !is_wasm_memory_ && !custom_deleter_ && !globally_registered_ && free_on_destruct_ && !is_resizable_by_js_ && buffer_start_ != nullptr; }
    ...
    base::Optional<size_t> GrowWasmMemoryInPlace(Isolate* isolate, size_t delta_pages, size_t max_pages);
    ...
    static void BroadcastSharedWasmMemoryGrow(Isolate* isolate, std::shared_ptr<BackingStore>);
...
private:
    ...
    BackingStore(void* buffer_start, size_t byte_length, size_t max_byte_length, size_t byte_capacity, SharedFlag shared, ResizableFlag resizable, bool is_wasm_memory,
...
private:
    ...
    static void BroadcastSharedWasmMemoryGrow(Isolate* isolate, std::shared_ptr<BackingStore> backing_store);
