...
public:
    ...
    void set_bytecode_and_trampoline(Isolate* isolate, Handle<ByteArray> bytecode);
    ...
    static constexpr Flag AsJSRegExpFlag(RegExpFlag f) { return static_cast<Flag>(f); }
    static constexpr Flags AsJSRegExpFlags(RegExpFlags f) { return Flags { static_cast<int>(f) }; }
    static constexpr RegExpFlags AsRegExpFlags(Flags f) { return RegExpFlags { static_cast<int>(f) }; }
    ...
    static base::Optional<RegExpFlag> FlagFromChar(char c)
    {
        ...
        if (f.value() == RegExpFlag::kUnicodeSets && !FLAG_harmony_regexp_unicode_sets) {
            return {};
        }
        return f;
    }
...
#undef V
    ...
    static base::Optional<Flags> FlagsFromString(Isolate* isolate, Handle<String> flags);
    ...
    static constexpr bool TypeSupportsCaptures(Type t) { return t == IRREGEXP || t == EXPERIMENTAL; }

    // Each capture (including the match itself) needs two registers.
    static constexpr int RegistersForCaptureCount(int count) { return (count + 1) * 2; }

    static constexpr int code_index(bool is_latin1) { return is_latin1 ? kIrregexpLatin1CodeIndex : kIrregexpUC16CodeIndex; }

    static constexpr int bytecode_index(bool is_latin1) { return is_latin1 ? kIrregexpLatin1BytecodeIndex : kIrregexpUC16BytecodeIndex; }
    ...
    static constexpr int kIrregexpLatin1BytecodeIndex = kIrregexpUC16CodeIndex + 1;
    static constexpr int kIrregexpUC16BytecodeIndex = kIrregexpLatin1BytecodeIndex + 1;
    // Maximal number of registers used by either Latin1 or UC16.
    // Only used to check that there is enough stack space
    static constexpr int kIrregexpMaxRegisterCountIndex = kIrregexpUC16BytecodeIndex + 1;
    // Number of captures in the compiled regexp.
    static constexpr int kIrregexpCaptureCountIndex = kIrregexpMaxRegisterCountIndex + 1;
    // Maps names of named capture groups (at indices 2i) to their corresponding
    // (1-based) capture group indices (at indices 2i + 1).
    static constexpr int kIrregexpCaptureNameMapIndex = kIrregexpCaptureCountIndex + 1;
    ...
    static constexpr int kIrregexpTicksUntilTierUpIndex = kIrregexpCaptureNameMapIndex + 1;
    ...
    // above to save space.
    static constexpr int kIrregexpBacktrackLimit = kIrregexpTicksUntilTierUpIndex + 1;
    ...
    static constexpr int Size() { return kHeaderSize + kInObjectFieldCount * kTaggedSize; }
...
public:
    static Handle<JSRegExpResultIndices> BuildIndices(Isolate* isolate, Handle<RegExpMatchInfo> match_info, Handle<Object> maybe_names);
