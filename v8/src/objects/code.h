...
public:
    ...
    inline void SetCodeAndEntryPoint(Isolate* isolate_for_sandbox, Code code, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    inline void SetEntryPointForOffHeapBuiltin(Isolate* isolate_for_sandbox, Address entry);
    ...
    inline void initialize_flags(CodeKind kind, Builtin builtin_id, bool is_turbofanned, bool is_off_heap_trampoline);
    ...
    V8_EXPORT_PRIVATE Address OffHeapInstructionStart(Isolate* isolate, Address pc) const;
    inline Address InstructionEnd(Isolate* isolate, Address pc) const;
    V8_EXPORT_PRIVATE Address OffHeapInstructionEnd(Isolate* isolate, Address pc) const;

    V8_EXPORT_PRIVATE bool OffHeapBuiltinContains(Isolate* isolate, Address pc) const;
...
#define CODE_DATA_FIELDS(V)                                                                                                                                                        \
    ...
    /* Raw data fields. */                                                                                                                                                         \
    V(kCodeCageBaseUpper32BitsOffset, V8_EXTERNAL_CODE_SPACE_BOOL ? kTaggedSize : 0)                                                                                               \
    V(kCodeEntryPointOffset, V8_EXTERNAL_CODE_SPACE_BOOL ? kSystemPointerSize : 0)                                                                                                 \
...
#ifdef V8_EXTERNAL_CODE_SPACE
    using ExternalCodeField = TaggedField<Object, kCodeOffset, ExternalCodeCompressionScheme>;
#endif
...
#undef FLAGS_BIT_FIELDS
    ...
    static_assert(!V8_EXTERNAL_CODE_SPACE_BOOL || (FLAGS_BIT_FIELDS_Ranges::kBitsCount <= FIELD_SIZE(CodeDataContainer::kFlagsOffset) * kBitsPerByte));
...
public:
    ...
    inline ByteArray SourcePositionTable(PtrComprCageBase cage_base, SharedFunctionInfo sfi) const;
    ...
    inline void initialize_flags(CodeKind kind, bool is_turbofanned, int stack_slots, bool is_off_heap_trampoline);
    ...
    static inline void CopyRelocInfoToByteArray(ByteArray dest, const CodeDesc& desc);

    inline uintptr_t GetBaselineStartPCForBytecodeOffset(int bytecode_offset, BytecodeArray bytecodes);

    inline uintptr_t GetBaselineEndPCForBytecodeOffset(int bytecode_offset, BytecodeArray bytecodes);
    ...
    inline uintptr_t GetBaselinePCForNextExecutedBytecode(int bytecode_offset, BytecodeArray bytecodes);

    inline int GetBytecodeOffsetForBaselinePC(Address baseline_pc, BytecodeArray bytecodes);
    ...
    static int SizeFor(int body_size) { return RoundUp(kHeaderSize + body_size, kCodeAlignment); }
...
#define CODE_FIELDS(V)                                                                                                                                                             \
    ...
    V(kDataStart, 0)                                                                                                                                                               \
    V(kMainCageBaseUpper32BitsOffset, V8_EXTERNAL_CODE_SPACE_BOOL ? kTaggedSize : 0)                                                                                               \
...
#undef CODE_FIELDS
    ...
    static constexpr int kHeaderPaddingSize = V8_EXTERNAL_CODE_SPACE_BOOL ? 4 : (COMPRESS_POINTERS_BOOL ? 8 : 20);
#elif V8_TARGET_ARCH_MIPS64
    static constexpr int kHeaderPaddingSize = 20;
#elif V8_TARGET_ARCH_LOONG64
    static constexpr int kHeaderPaddingSize = 20;
#elif V8_TARGET_ARCH_X64
    static constexpr int kHeaderPaddingSize = V8_EXTERNAL_CODE_SPACE_BOOL ? 4 : (COMPRESS_POINTERS_BOOL ? 8 : 52);
    ...
    static constexpr int kHeaderPaddingSize = V8_EMBEDDED_CONSTANT_POOL_BOOL ? (COMPRESS_POINTERS_BOOL ? 4 : 48) : (COMPRESS_POINTERS_BOOL ? 8 : 52);
...
#undef CODE_FLAGS_BIT_FIELDS
    ...
    static_assert(CODE_FLAGS_BIT_FIELDS_Ranges::kBitsCount <= FIELD_SIZE(kFlagsOffset) * kBitsPerByte);
...
#undef CODE_KIND_SPECIFIC_FLAGS_BIT_FIELDS
    ...
    static_assert(CODE_KIND_SPECIFIC_FLAGS_BIT_FIELDS_Ranges::kBitsCount <= FIELD_SIZE(CodeDataContainer::kKindSpecificFlagsOffset) * kBitsPerByte);
...
private:
    ...
    inline uintptr_t GetBaselinePCForBytecodeOffset(int bytecode_offset, BytecodeToPCPosition position, BytecodeArray bytecodes);
...
V8_EXPORT_PRIVATE Address OffHeapInstructionStart(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE Address OffHeapInstructionEnd(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapInstructionSize(HeapObject code, Builtin builtin);

V8_EXPORT_PRIVATE Address OffHeapMetadataStart(HeapObject code, Builtin builtin);
...
V8_EXPORT_PRIVATE Address OffHeapSafepointTableAddress(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapSafepointTableSize(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE Address OffHeapHandlerTableAddress(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapHandlerTableSize(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE Address OffHeapConstantPoolAddress(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapConstantPoolSize(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE Address OffHeapCodeCommentsAddress(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapCodeCommentsSize(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE Address OffHeapUnwindingInfoAddress(HeapObject code, Builtin builtin);
V8_EXPORT_PRIVATE int OffHeapUnwindingInfoSize(HeapObject code, Builtin builtin);
...
public:
    ...
    explicit CodeLookupResult(Code code)
        : code_(code)
    {
    }
    ...
    inline MaglevSafepointEntry GetMaglevSafepointEntry(Isolate* isolate, Address pc) const;
    ...
    bool operator!=(const CodeLookupResult& other) const { return !operator==(other); }
...
public:
    ...
    inline ByteArray SourcePositionTable(PtrComprCageBase cage_base, SharedFunctionInfo sfi);
    ...
    inline HandlerTable::CatchPrediction GetBuiltinCatchPrediction(PtrComprCageBase cage_base);
    ...
    constexpr bool operator==(AbstractCode other) const { return SafeEquals(other); }
    constexpr bool operator!=(AbstractCode other) const { return !SafeEquals(other); }
...
public:
    ...
    template <typename ObjectT> static bool MarkCodeForDeoptimization(ObjectT object, DependencyGroups groups);

    V8_EXPORT_PRIVATE static DependentCode empty_dependent_code(const ReadOnlyRoots& roots);
...
private:
    ...
    static void SetDependentCode(Handle<HeapObject> object, Handle<DependentCode> dep);

    static Handle<DependentCode> New(Isolate* isolate, DependencyGroups groups, Handle<Code> code);
    ...
    static constexpr int LengthFor(int number_of_entries) { return number_of_entries * kSlotsPerEntry; }
...
class BytecodeArray : public TorqueGeneratedBytecodeArray<BytecodeArray, FixedArrayBase> {
public:
    static constexpr int SizeFor(int length) { return OBJECT_POINTER_ALIGN(kHeaderSize + length); }
    ...
    static_assert(kBytecodeAgeOffset + kBytecodeAgeSize - 1 == kBytecodeAgeOffsetEnd);
...
#undef DECL_ENTRY_ACCESSORS
    ...
    static Handle<DeoptimizationData> New(Isolate* isolate, int deopt_entry_count, AllocationType allocation);
...
private:
    static int IndexForEntry(int i) { return kFirstDeoptEntryIndex + (i * kDeoptEntrySize); }
