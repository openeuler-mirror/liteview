...
JSAtomicsMutex::LockGuard::LockGuard(Isolate* isolate, Handle<JSAtomicsMutex> mutex)
    : isolate_(isolate)
    , mutex_(mutex)
{
...
void JSAtomicsMutex::Lock(Isolate* requester, Handle<JSAtomicsMutex> mutex)
{
    ...
    if (V8_UNLIKELY(!state->compare_exchange_weak(expected, kLockedUncontended, std::memory_order_acquire, std::memory_order_relaxed))) {
...
void JSAtomicsMutex::Unlock(Isolate* requester)
{
    ...
    if (V8_LIKELY(state->compare_exchange_strong(expected, kUnlocked, std::memory_order_release, std::memory_order_relaxed))) {
        return;
    }
    UnlockSlowPath(requester, state);
}

bool JSAtomicsMutex::IsHeld() { return AtomicStatePtr()->load(std::memory_order_relaxed) & kIsLockedBit; }
...
void JSAtomicsMutex::SetCurrentThreadAsOwner() { AtomicOwnerThreadIdPtr()->store(ThreadId::Current().ToInteger(), std::memory_order_relaxed); }
