...
public:
    bool is_null() const { return static_cast<Tagged_t>(ptr()) == static_cast<Tagged_t>(kNullAddress); }
    ...
    inline void set_map_no_write_barrier(Map value, RelaxedStoreTag = kRelaxedStore);
    ...
    inline bool release_compare_and_swap_map_word(MapWord old_map_word, MapWord new_map_word);

    // Initialize the map immediately after the object is allocated.
    // Do not use this outside Heap.
    inline void set_map_after_allocation(Map value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
#undef DECL_STRUCT_PREDICATE
    ...
    template <typename ObjectVisitor> inline void IterateFast(PtrComprCageBase cage_base, ObjectVisitor* v);

    template <typename ObjectVisitor> inline void IterateFast(Map map, int object_size, ObjectVisitor* v);
    ...
    template <typename ObjectVisitor> inline void IterateBodyFast(PtrComprCageBase cage_base, ObjectVisitor* v);

    template <typename ObjectVisitor> inline void IterateBodyFast(Map map, int object_size, ObjectVisitor* v);
...
private:
    ...
    template <EmitWriteBarrier emit_write_barrier, typename MemoryOrder> V8_INLINE void set_map(Map value, MemoryOrder order, VerificationMode mode);
};
