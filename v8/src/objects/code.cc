...
inline EmbeddedData EmbeddedDataWithMaybeRemappedEmbeddedBuiltins(HeapObject code)
{
    ...
    return (code_range && code_range->embedded_blob_code_copy() != nullptr) ? EmbeddedData::FromBlob(code_range) : EmbeddedData::FromBlob();
#else
...
{
    EmbeddedData d = EmbeddedDataWithMaybeRemappedEmbeddedBuiltins(code);
    return d.InstructionStartOfBuiltin(builtin) + d.InstructionSizeOfBuiltin(builtin);
}
...
void Code::FlushICache() const { FlushInstructionCache(raw_instruction_start(), raw_instruction_size()); }

void Code::CopyFromNoFlush(ByteArray reloc_info, Heap* heap, const CodeDesc& desc)
{
    // Copy code.
    static_assert(kOnHeapBodyIsContiguous);
    CopyBytes(reinterpret_cast<byte*>(raw_instruction_start()), desc.buffer, static_cast<size_t>(desc.instr_size));
    // TODO(jgruber,v8:11036): Merge with the above.
    CopyBytes(reinterpret_cast<byte*>(raw_instruction_start() + desc.instr_size), desc.unwinding_info, static_cast<size_t>(desc.unwinding_info_size));
...
void Code::RelocateFromDesc(ByteArray reloc_info, Heap* heap, const CodeDesc& desc)
{
    // Unbox handles and relocate.
    ...
    for (RelocIterator it(*this, reloc_info, mode_mask); !it.done(); it.next()) {
        ...
        if (RelocInfo::IsEmbeddedObjectMode(mode)) {
            ...
            it.rinfo()->set_target_object(heap, *p, UPDATE_WRITE_BARRIER, SKIP_ICACHE_FLUSH);
            ...
            it.rinfo()->set_target_address(code.raw_instruction_start(), UPDATE_WRITE_BARRIER, SKIP_ICACHE_FLUSH);
            ...
            Address p = heap->isolate()->builtin_entry_table()[Builtins::ToInt(builtin)];
            it.rinfo()->set_target_address(p, UPDATE_WRITE_BARRIER, SKIP_ICACHE_FLUSH);
...
SafepointEntry CodeDataContainer::GetSafepointEntry(Isolate* isolate, Address pc)
{
    DCHECK(!is_maglevved());
    SafepointTable table(isolate, pc, *this);
    return table.FindEntry(pc);
...
#ifdef V8_EXTERNAL_CODE_SPACE
MaglevSafepointEntry CodeDataContainer::GetMaglevSafepointEntry(Isolate* isolate, Address pc)
{
    DCHECK(is_maglevved());
    MaglevSafepointTable table(isolate, pc, *this);
    return table.FindEntry(pc);
...
#ifdef V8_EXTERNAL_CODE_SPACE
Address CodeDataContainer::OffHeapInstructionStart(Isolate* isolate, Address pc) const
{
    DCHECK(is_off_heap_trampoline());
    EmbeddedData d = EmbeddedData::GetEmbeddedDataForPC(isolate, pc);
    return d.InstructionStartOfBuiltin(builtin_id());
...
#ifdef V8_EXTERNAL_CODE_SPACE
Address CodeDataContainer::OffHeapInstructionEnd(Isolate* isolate, Address pc) const
{
    DCHECK(is_off_heap_trampoline());
    EmbeddedData d = EmbeddedData::GetEmbeddedDataForPC(isolate, pc);
    return d.InstructionEndOf(builtin_id());
...
#ifdef V8_EXTERNAL_CODE_SPACE
bool CodeDataContainer::OffHeapBuiltinContains(Isolate* isolate, Address pc) const
{
    DCHECK(is_off_heap_trampoline());
    EmbeddedData d = EmbeddedData::GetEmbeddedDataForPC(isolate, pc);
    return d.BuiltinContains(builtin_id(), pc);
...
int AbstractCode::SourcePosition(PtrComprCageBase cage_base, int offset)
{
    ...
    if (IsCode(cage_base))
        offset--;
    int position = 0;
    for (SourcePositionTableIterator iterator(source_position_table, SourcePositionTableIterator::kJavaScriptOnly, SourcePositionTableIterator::kDontSkipFunctionEntry);
...
// TODO(cbruni): Move to BytecodeArray
int AbstractCode::SourceStatementPosition(PtrComprCageBase cage_base, int offset)
{
    CHECK_NE(kind(cage_base), CodeKind::BASELINE);
    ...
    for (SourcePositionTableIterator it(SourcePositionTableInternal(cage_base)); !it.done(); it.Advance()) {
        if (it.is_statement()) {
...
bool Code::CanDeoptAt(Isolate* isolate, Address pc)
{
    ...
    for (int i = 0; i < deopt_data.DeoptCount(); i++) {
        ...
        if (address == pc && deopt_data.GetBytecodeOffset(i) != BytecodeOffset::None()) {
            return true;
        }
    }
    return false;
...
{
    static constexpr int kModeMask = RelocInfo::AllRealModesMask() & ~RelocInfo::ModeMask(RelocInfo::CONST_POOL) & ~RelocInfo::ModeMask(RelocInfo::OFF_HEAP_TARGET)
        & ~RelocInfo::ModeMask(RelocInfo::VENEER_POOL);
    static_assert(kModeMask
        == (RelocInfo::ModeMask(RelocInfo::CODE_TARGET) | RelocInfo::ModeMask(RelocInfo::RELATIVE_CODE_TARGET) | RelocInfo::ModeMask(RelocInfo::COMPRESSED_EMBEDDED_OBJECT)
            | RelocInfo::ModeMask(RelocInfo::FULL_EMBEDDED_OBJECT) | RelocInfo::ModeMask(RelocInfo::EXTERNAL_REFERENCE) | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE)
            | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE_ENCODED) | RelocInfo::ModeMask(RelocInfo::NEAR_BUILTIN_ENTRY) | RelocInfo::ModeMask(RelocInfo::WASM_CALL)
            | RelocInfo::ModeMask(RelocInfo::WASM_STUB_CALL)));

#if defined(V8_TARGET_ARCH_PPC) || defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_MIPS64)
    return RelocIterator(*this, kModeMask).done();
#elif defined(V8_TARGET_ARCH_X64) || defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_ARM) || defined(V8_TARGET_ARCH_S390) || defined(V8_TARGET_ARCH_IA32)                  \
...
Code::OptimizedCodeIterator::OptimizedCodeIterator(Isolate* isolate)
{
    ...
    next_context_ = list.IsUndefined(isolate_) ? NativeContext() : NativeContext::cast(list);
}
...
Code Code::OptimizedCodeIterator::Next()
{
    do {
        ...
        if (!current_code_.is_null()) {
            ...
            next_context_ = next_context.IsUndefined(isolate_) ? NativeContext() : NativeContext::cast(next_context);
        } else {
            // Exhausted contexts.
            return Code();
        }
        current_code_ = next.IsUndefined(isolate_) ? Code() : FromCodeT(CodeT::cast(next));
    } while (current_code_.is_null());
    DCHECK(CodeKindCanDeoptimize(current_code_.kind()));
    return current_code_;
}

Handle<DeoptimizationData> DeoptimizationData::New(Isolate* isolate, int deopt_entry_count, AllocationType allocation)
{
    return Handle<DeoptimizationData>::cast(isolate->factory()->NewFixedArray(LengthFor(deopt_entry_count), allocation));
}

Handle<DeoptimizationData> DeoptimizationData::Empty(Isolate* isolate) { return Handle<DeoptimizationData>::cast(isolate->factory()->empty_fixed_array()); }
...
void DeoptimizationData::DeoptimizationDataPrint(std::ostream& os)
{
    ...
    for (int i = 0; i < deopt_count; i++) {
        os << std::setw(6) << i << "  " << std::setw(15) << GetBytecodeOffset(i).ToInt() << "  "
#ifdef DEBUG
...
template <typename CodeOrCodeT> inline void DisassembleCodeRange(Isolate* isolate, std::ostream& os, CodeOrCodeT code, Address begin, size_t size, Address current_pc)
{
    Address end = begin + size;
    AllowHandleAllocation allow_handles;
    DisallowGarbageCollection no_gc;
    HandleScope handle_scope(isolate);
    Disassembler::Decode(isolate, os, reinterpret_cast<byte*>(begin), reinterpret_cast<byte*>(end), CodeReference(handle(code, isolate)), current_pc);
}

template <typename CodeOrCodeT> void Disassemble(const char* name, std::ostream& os, Isolate* isolate, CodeOrCodeT code, Address current_pc)
{
    ...
    os << "compiler = " << (code.is_turbofanned() ? "turbofan" : code.is_maglevved() ? "maglev" : kind == CodeKind::BASELINE ? "baseline" : "unknown") << "\n";
    ...
    if (code.IsCode() && code.is_off_heap_trampoline()) {
        ...
        DisassembleCodeRange(isolate, os, trampoline_code, trampoline_code.raw_instruction_start(), trampoline_size, current_pc);
        os << "\n";
    }

    {
        ...
        DisassembleCodeRange(isolate, os, code, code.InstructionStart(), code_size, current_pc);
    ...
    if (CodeKindCanDeoptimize(kind)) {
        DeoptimizationData data = DeoptimizationData::cast(code.deoptimization_data());
    ...
    if (code.has_unwinding_info()) {
        ...
        EhFrameDisassembler eh_frame_disassembler(reinterpret_cast<byte*>(code.unwinding_info_start()), reinterpret_cast<byte*>(code.unwinding_info_end()));
...
void Code::Disassemble(const char* name, std::ostream& os, Isolate* isolate, Address current_pc) { i::Disassemble(name, os, isolate, *this, current_pc); }

#ifdef V8_EXTERNAL_CODE_SPACE
void CodeDataContainer::Disassemble(const char* name, std::ostream& os, Isolate* isolate, Address current_pc) { i::Disassemble(name, os, isolate, *this, current_pc); }
#endif // V8_EXTERNAL_CODE_SPACE
...
void BytecodeArray::PrintJson(std::ostream& os)
{
    ...
    while (!iterator.done()) {
        ...
        interpreter::BytecodeDecoder::Decode(os, reinterpret_cast<byte*>(current_address), false);
...
void BytecodeArray::Disassemble(Handle<BytecodeArray> handle, std::ostream& os)
{
    ...
    while (!iterator.done()) {
        ...
        interpreter::BytecodeDecoder::Decode(os, reinterpret_cast<byte*>(current_address));
        if (interpreter::Bytecodes::IsJump(iterator.current_bytecode())) {
...
void BytecodeArray::CopyBytecodesTo(BytecodeArray to)
{
    ...
    CopyBytes(reinterpret_cast<byte*>(to.GetFirstBytecodeAddress()), reinterpret_cast<byte*>(from.GetFirstBytecodeAddress()), from.length());
}
...
void BytecodeArray::MakeOlder()
{
    ...
    if (age < v8_flags.bytecode_old_age) {
        ...
        base::AsAtomic16::Relaxed_CompareAndSwap(reinterpret_cast<base::Atomic16*>(age_addr), age, age + 1);
    }

    DCHECK_LE(bytecode_age(), v8_flags.bytecode_old_age);
}

bool BytecodeArray::IsOld() const { return bytecode_age() >= v8_flags.bytecode_old_age; }
...
void DependentCode::SetDependentCode(Handle<HeapObject> object, Handle<DependentCode> dep)
{
    if (object->IsMap()) {
...
void DependentCode::InstallDependency(Isolate* isolate, Handle<Code> code, Handle<HeapObject> object, DependencyGroups groups)
{
    if (V8_UNLIKELY(v8_flags.trace_compilation_dependencies)) {
        StdoutStream {} << "Installing dependency of [" << code->GetHeapObject() << "] on [" << object << "] in groups [";
...
Handle<DependentCode> DependentCode::InsertWeakCode(Isolate* isolate, Handle<DependentCode> entries, DependencyGroups groups, Handle<Code> code)
{
    ...
    MaybeObjectHandle code_slot(HeapObjectReference::Weak(ToCodeT(*code)), isolate);
    MaybeObjectHandle group_slot(MaybeObject::FromSmi(Smi::FromInt(groups)), isolate);
    entries = Handle<DependentCode>::cast(WeakArrayList::AddToEnd(isolate, entries, code_slot, group_slot));
    return entries;
}

Handle<DependentCode> DependentCode::New(Isolate* isolate, DependencyGroups groups, Handle<Code> code)
{
    Handle<DependentCode> result = Handle<DependentCode>::cast(isolate->factory()->NewWeakArrayList(LengthFor(1), AllocationType::kOld));
...
void DependentCode::IterateAndCompact(const IterateAndCompactFn& fn)
{
    ...
    while (i >= 0) {
        ...
        if (fn(CodeT::cast(obj->GetHeapObjectAssumeWeak()), static_cast<DependencyGroups>(Get(i + kGroupsSlotOffset).ToSmi().value()))) {
            len = FillEntryFromBack(i, len);
        }
...
bool DependentCode::MarkCodeForDeoptimization(DependentCode::DependencyGroups deopt_groups)
{
    DisallowGarbageCollection no_gc;
...
// static
DependentCode DependentCode::empty_dependent_code(const ReadOnlyRoots& roots) { return DependentCode::cast(roots.empty_weak_array_list()); }
