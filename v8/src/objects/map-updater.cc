...
V8_WARN_UNUSED_RESULT Handle<FieldType> GeneralizeFieldType(Representation rep1, Handle<FieldType> type1, Representation rep2, Handle<FieldType> type2, Isolate* isolate)
{
    ...
    // is "Any".
    if (Map::FieldTypeIsCleared(rep1, *type1) || Map::FieldTypeIsCleared(rep2, *type2)) {
...
void PrintGeneralization(Isolate* isolate, Handle<Map> map, FILE* file, const char* reason, InternalIndex modify_index, int split, int descriptors, bool descriptor_to_field,
    Representation old_representation, Representation new_representation, PropertyConstness old_constness, PropertyConstness new_constness, MaybeHandle<FieldType> old_field_type,
    MaybeHandle<Object> old_value, MaybeHandle<FieldType> new_field_type, MaybeHandle<Object> new_value)
{
    OFStream os(file);
    os << "[generalizing]";
    ...
    , is_transitionable_fast_elements_kind_(IsTransitionableFastElementsKind(new_elements_kind_))
{
    ...
    DCHECK(!old_map->FindRootMap(isolate).GetConstructor().IsFunctionTemplateInfo());
}

Name MapUpdater::GetKey(InternalIndex descriptor) const { return old_descriptors_->GetKey(descriptor); }
...
Handle<FieldType> MapUpdater::GetOrComputeFieldType(Handle<DescriptorArray> descriptors, InternalIndex descriptor, PropertyLocation location, Representation representation)
{
    // |location| is just a pre-fetched GetDetails(descriptor).location().
    ...
    if (location == PropertyLocation::kField) {
        ...
        return descriptors->GetStrongValue(descriptor).OptimalType(isolate_, representation);
    }
}
...
IntegrityLevelTransitionInfo DetectIntegrityLevelTransitions(Map map, Isolate* isolate, DisallowGarbageCollection* no_gc, ConcurrencyMode cmode)
{
...
// static
base::Optional<Map> MapUpdater::TryUpdateNoLock(Isolate* isolate, Map old_map, ConcurrencyMode cmode)
{
    ...
    Map result = root_map.TryReplayPropertyTransitions(isolate, info.integrity_level_source_map, cmode);
    if (result.is_null())
        return {};
...
void MapUpdater::GeneralizeField(Handle<Map> map, InternalIndex modify_index, PropertyConstness new_constness, Representation new_representation, Handle<FieldType> new_field_type)
{
...
{
    result_map_ = Map::Normalize(isolate_, old_map_, new_elements_kind_, CLEAR_INOBJECT_PROPERTIES, reason);
    state_ = kEnd;
    return state_; // Done.
}

// static
void MapUpdater::CompleteInobjectSlackTracking(Isolate* isolate, Map initial_map)
{
    ...
    if (slack != 0) {
        ...
        callback = [](Map map) { map.set_construction_counter(Map::kNoSlackTracking); };
    }
...
MapUpdater::State MapUpdater::TryReconfigureToDataFieldInplace()
{
    ...
    if (v8_flags.trace_generalization) {
        PrintGeneralization(isolate_, old_map_, stdout, "uninitialized field", modified_descriptor_, old_nof_, old_nof_, false, old_representation, new_representation_,
            old_details.constness(), new_constness_, handle(old_descriptors_->GetFieldType(modified_descriptor_), isolate_), MaybeHandle<Object>(), new_field_type_,
            MaybeHandle<Object>());
    }
    ...
    DCHECK(old_descriptors_->GetFieldType(modified_descriptor_).NowIs(new_field_type_));
...
MapUpdater::State MapUpdater::FindRootMap()
{
    ...
    if (root_map_->is_deprecated()) {
        state_ = kEnd;
        result_map_ = handle(JSFunction::cast(root_map_->GetConstructor()).initial_map(), isolate_);
...
MapUpdater::State MapUpdater::FindTargetMap()
{
    ...
    for (InternalIndex i : InternalIndex::Range(root_nof, old_nof_)) {
        ...
        if (!maybe_tmp_map.ToHandle(&tmp_map))
            break;
        Handle<DescriptorArray> tmp_descriptors(tmp_map->instance_descriptors(isolate_), isolate_);
    ...
    if (target_nof == old_nof_) {
        ...
        if (modified_descriptor_.is_found()) {
            DescriptorArray target_descriptors = target_map_->instance_descriptors(isolate_);
            ...
            if (new_location_ == PropertyLocation::kField) {
                ...
                DCHECK(new_field_type_->NowIs(target_descriptors.GetFieldType(modified_descriptor_)));
            } else {
                DCHECK(details.location() == PropertyLocation::kField || EqualImmutableValues(*new_value_, target_descriptors.GetStrongValue(modified_descriptor_)));
            }
        }
#endif
...
Handle<DescriptorArray> MapUpdater::BuildDescriptorArray()
{
    ...
    int target_nof = target_map_->NumberOfOwnDescriptors();
    Handle<DescriptorArray> target_descriptors(target_map_->instance_descriptors(isolate_), isolate_);
    ...
    for (InternalIndex i : InternalIndex::Range(root_nof)) {
        ...
        Descriptor d(handle(GetKey(i), isolate_), MaybeObjectHandle(old_descriptors_->GetValue(i), isolate_), old_details);
    ...
    for (InternalIndex i : InternalIndex::Range(root_nof, target_nof)) {
        ...
        if (next_location == PropertyLocation::kField) {
            ...
            Handle<FieldType> next_field_type = GeneralizeFieldType(old_details.representation(), old_field_type, next_representation, target_field_type, isolate_);

            Map::GeneralizeIfCanHaveTransitionableFastElementsKind(isolate_, instance_type, &next_representation, &next_field_type);

            MaybeObjectHandle wrapped_type(Map::WrapFieldType(isolate_, next_field_type));
            Descriptor d;
...
Handle<Map> MapUpdater::FindSplitMap(Handle<DescriptorArray> descriptors)
{
    ...
    for (InternalIndex i : InternalIndex::Range(root_nof, old_nof_)) {
        ...
        Map next = TransitionsAccessor(isolate_, current).SearchTransition(name, details.kind(), details.attributes());
        if (next.is_null())
            break;
...
MapUpdater::State MapUpdater::ConstructNewMap()
{
    ...
    MaybeHandle<Map> maybe_transition = TransitionsAccessor::SearchTransition(isolate_, split_map, GetKey(split_index), split_details.kind(), split_details.attributes());
    ...
    if (v8_flags.trace_generalization && modified_descriptor_.is_found()) {
        ...
        PrintGeneralization(isolate_, old_map_, stdout, "", modified_descriptor_, split_nof, old_nof_,
            old_details.location() == PropertyLocation::kDescriptor && new_location_ == PropertyLocation::kField, old_details.representation(), new_details.representation(),
            old_details.constness(), new_details.constness(), old_field_type, old_value, new_field_type, new_value);
    }
...
void PrintReconfiguration(Isolate* isolate, Handle<Map> map, FILE* file, InternalIndex modify_index, PropertyKind kind, PropertyAttributes attributes)
{
    OFStream os(file);
    os << "[reconfiguring]";
    Name name = map->instance_descriptors(isolate).GetKey(modify_index);
...
// static
void MapUpdater::UpdateFieldType(Isolate* isolate, Handle<Map> map, InternalIndex descriptor, Handle<Name> name, PropertyConstness new_constness, Representation new_representation,
    const MaybeObjectHandle& new_wrapped_type)
{
    ...
    while (!backlog.empty()) {
        ...
        if (new_constness != details.constness() || !new_representation.Equals(details.representation()) || descriptors.GetFieldType(descriptor) != *new_wrapped_type.object()) {
            Descriptor d = Descriptor::DataField(name, descriptors.GetFieldIndex(descriptor), details.attributes(), new_constness, new_representation, new_wrapped_type);
...
void MapUpdater::GeneralizeField(
    Isolate* isolate, Handle<Map> map, InternalIndex modify_index, PropertyConstness new_constness, Representation new_representation, Handle<FieldType> new_field_type)
{
    ...
    if (IsGeneralizableTo(new_constness, old_constness) && old_representation.Equals(new_representation) && !Map::FieldTypeIsCleared(new_representation, *new_field_type) &&
        // Checking old_field_type for being cleared is not necessary because
        // the NowIs check below would fail anyway in that case.
        new_field_type->NowIs(old_field_type)) {
        DCHECK(GeneralizeFieldType(old_representation, old_field_type, new_representation, new_field_type, isolate)->NowIs(old_field_type));
        return;
    }

    // Determine the field owner.
    Handle<Map> field_owner(map->FindFieldOwner(isolate, modify_index), isolate);
    Handle<DescriptorArray> descriptors(field_owner->instance_descriptors(isolate), isolate);
    ...
    UpdateFieldType(isolate, field_owner, modify_index, name, new_constness, new_representation, wrapped_type);
    ...
    if (v8_flags.trace_generalization) {
        PrintGeneralization(isolate, map, stdout, "field type generalization", modify_index, map->NumberOfOwnDescriptors(), map->NumberOfOwnDescriptors(), false,
            details.representation(), descriptors->GetDetails(modify_index).representation(), old_constness, new_constness, old_field_type, MaybeHandle<Object>(), new_field_type,
            MaybeHandle<Object>());
    }
}
