...
public:
    String original_string(PtrComprCageBase cage_base) const { return String::cast(OriginalStringObject(cage_base)); }

    String forward_string(PtrComprCageBase cage_base) const { return String::cast(ForwardStringObjectOrHash(cage_base)); }
    ...
    Object OriginalStringObject(PtrComprCageBase cage_base) const { return OriginalStringSlot().Acquire_Load(cage_base); }

    Object ForwardStringObjectOrHash(PtrComprCageBase cage_base) const { return ForwardStringOrHashSlot().Acquire_Load(cage_base); }

    Address ExternalResourceAddress() const { return base::AsAtomicPointer::Acquire_Load(&external_resource_); }

    void set_original_string(Object object) { OriginalStringSlot().Release_Store(object); }

    void set_forward_string(Object object) { ForwardStringOrHashSlot().Release_Store(object); }
    ...
    void set_external_resource(Address address) { base::AsAtomicPointer::Release_Store(&external_resource_, address); }

    inline void SetInternalized(String string, String forward_to);
    inline void SetExternal(String string, v8::String::ExternalStringResourceBase*, bool is_one_byte, uint32_t raw_hash);
...
private:
    OffHeapObjectSlot OriginalStringSlot() const { return OffHeapObjectSlot(&original_string_); }

    OffHeapObjectSlot ForwardStringOrHashSlot() const { return OffHeapObjectSlot(&forward_string_or_hash_); }
...
void StringForwardingTable::Record::SetInternalized(String string, String forward_to)
{
...
void StringForwardingTable::Record::SetExternal(String string, v8::String::ExternalStringResourceBase* resource, bool is_one_byte, uint32_t raw_hash)
{
    set_original_string(string);
...
void StringForwardingTable::Record::DisposeUnusedExternalResource(String original)
{
#ifdef DEBUG
    String stored_original = original_string(GetIsolateFromWritableObject(original));
    if (stored_original.IsThinString()) {
        stored_original = ThinString::cast(stored_original).actual();
    }
    DCHECK_EQ(original, stored_original);
#endif
    if (!original.IsExternalString())
        return;
    Address original_resource = ExternalString::cast(original).resource_as_address();
    bool is_one_byte;
    auto resource = external_resource(&is_one_byte);
...
uint32_t StringForwardingTable::BlockForIndex(int index, uint32_t* index_in_block)
{
    ...
    // of the first block.
    const uint32_t block_index = kBitsPerInt - base::bits::CountLeadingZeros(static_cast<uint32_t>(index + kInitialBlockSize)) - kInitialBlockSizeHighestBit - 1;
...
// static
uint32_t StringForwardingTable::CapacityForBlock(uint32_t block_index) { return 1u << (block_index + kInitialBlockSizeHighestBit); }
