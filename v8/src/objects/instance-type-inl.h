...
#define INSTANCE_TYPE_CHECKER(type, forinstancetype)                                                                                                                               \
    V8_INLINE constexpr bool Is##type(InstanceType instance_type) { return instance_type == forinstancetype; }
...
template <InstanceType lower_limit, InstanceType upper_limit> struct InstanceRangeChecker {
    static constexpr bool Check(InstanceType value) { return base::IsInRange(value, lower_limit, upper_limit); }
};
...
#define INSTANCE_TYPE_CHECKER_RANGE(type, first_instance_type, last_instance_type)                                                                                                 \
    V8_INLINE constexpr bool Is##type(InstanceType instance_type) { return InstanceRangeChecker<first_instance_type, last_instance_type>::Check(instance_type); }
INSTANCE_TYPE_CHECKERS_RANGE(INSTANCE_TYPE_CHECKER_RANGE)
#undef INSTANCE_TYPE_CHECKER_RANGE

V8_INLINE constexpr bool IsHeapObject(InstanceType instance_type) { return true; }
...
V8_INLINE constexpr bool IsExternalString(InstanceType instance_type) { return (instance_type & (kIsNotStringMask | kStringRepresentationMask)) == kExternalStringTag; }

V8_INLINE constexpr bool IsThinString(InstanceType instance_type) { return (instance_type & kStringRepresentationMask) == kThinStringTag; }

V8_INLINE constexpr bool IsAbstractCode(InstanceType instance_type)
{
    return IsBytecodeArray(instance_type) || IsCode(instance_type) || (V8_REMOVE_BUILTINS_CODE_OBJECTS && IsCodeDataContainer(instance_type));
}

V8_INLINE constexpr bool IsFreeSpaceOrFiller(InstanceType instance_type) { return instance_type == FREE_SPACE_TYPE || instance_type == FILLER_TYPE; }

V8_INLINE constexpr bool IsCodeT(InstanceType instance_type) { return instance_type == CODET_TYPE; }
