...
public:
    ...
    static inline Handle<Context> GetContext(Isolate* isolate, Handle<ScriptContextTable> table, int i);
    ...
    V8_WARN_UNUSED_RESULT
    V8_EXPORT_PRIVATE static Handle<ScriptContextTable> Extend(Isolate* isolate, Handle<ScriptContextTable> table, Handle<Context> script_context, bool ignore_duplicates = false);
...
public:
    ...
    V8_INLINE void set(int index, Object value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    // Accessors with acquire-release semantics.
    V8_INLINE Object get(int index, AcquireLoadTag) const;
    V8_INLINE Object get(PtrComprCageBase cage_base, int index, AcquireLoadTag) const;
    V8_INLINE void set(int index, Object value, WriteBarrierMode mode, ReleaseStoreTag);
    ...
    V8_INLINE static constexpr int SizeFor(int length)
    {
        ...
        // DCHECK_EQ(TorqueGeneratedContext::SizeFor(length), result); // weolar
        return result;
    }

    // Code Generation support.
    // Offset of the element from the beginning of object.
    V8_INLINE static constexpr int OffsetOfElementAt(int index) { return SizeFor(index); }
    // Offset of the element from the heap object pointer.
    V8_INLINE static constexpr int SlotOffset(int index) { return OffsetOfElementAt(index) - kHeapObjectTag; }
...
#undef NATIVE_CONTEXT_SLOT
    ...
    V8_EXPORT_PRIVATE void set_extension(HeapObject object, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
#undef NATIVE_CONTEXT_FIELD_ACCESSORS
    ...
    //    always ABSENT.
    static Handle<Object> Lookup(Handle<Context> context, Handle<String> name, ContextLookupFlags flags, int* index, PropertyAttributes* attributes, InitializationFlag* init_flag,
        VariableMode* variable_mode, bool* is_sloppy_function_name = nullptr);
...
private:
    ...
    inline void set_previous(Context context, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
public:
    ...
    inline void synchronized_set_script_context_table(ScriptContextTable script_context_table);
    ...
    JSGlobalObject global_object(AcquireLoadTag) { return Context::global_object(); }

    inline Map TypedArrayElementsKindToCtorMap(ElementsKind element_kind) const;

    inline Map TypedArrayElementsKindToRabGsabCtorMap(ElementsKind element_kind) const;
