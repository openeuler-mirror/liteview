...
Maybe<bool> JSReceiver::CheckPrivateNameStore(LookupIterator* it, bool is_define)
{
    ...
    Handle<String> name_string(String::cast(Handle<Symbol>::cast(it->GetName())->description()), isolate);
    for (; it->IsFound(); it->Next()) {
        switch (it->state()) {
        case LookupIterator::TRANSITION:
        case LookupIterator::INTERCEPTOR:
        ...
        case LookupIterator::ACCESS_CHECK:
            if (!it->HasAccess()) {
                isolate->ReportFailedAccessCheck(Handle<JSObject>::cast(it->GetReceiver()));
        ...
        case LookupIterator::DATA:
            if (is_define) {
                MessageTemplate message
                    = it->GetName()->IsPrivateBrand() ? MessageTemplate::kInvalidPrivateBrandReinitialization : MessageTemplate::kInvalidPrivateFieldReinitialization;
    ...
    DCHECK(!it->IsFound());
    if (!is_define) {
        RETURN_FAILURE(isolate, GetShouldThrow(isolate, Nothing<ShouldThrow>()), NewTypeError(MessageTemplate::kInvalidPrivateMemberWrite, name_string, it->GetReceiver()));
    }
    return Just(true);
}

// static
Maybe<bool> JSReceiver::CheckIfCanDefine(Isolate* isolate, LookupIterator* it, Handle<Object> value, Maybe<ShouldThrow> should_throw)
{
    if (it->IsFound()) {
    ...
    } else if (!JSObject::IsExtensible(Handle<JSObject>::cast(it->GetReceiver()))) {
...
bool HasExcludedProperty(const base::ScopedVector<Handle<Object>>* excluded_properties, Handle<Object> search_element)
{
...
V8_WARN_UNUSED_RESULT Maybe<bool> FastAssign(
    Handle<JSReceiver> target, Handle<Object> source, PropertiesEnumerationMode mode, const base::ScopedVector<Handle<Object>>* excluded_properties, bool use_set)
{
    ...
    while (true) {
        for (InternalIndex i : map->IterateOwnDescriptors()) {
            ...
            if (use_set) {
                ...
                CHECK(JSObject::CreateDataProperty(&it, prop_value, Just(kThrowOnError)).FromJust());
            }
        }
        if (mode == PropertiesEnumerationMode::kEnumerationOrder) {
...
Maybe<bool> JSReceiver::SetOrCopyDataProperties(
    Isolate* isolate, Handle<JSReceiver> target, Handle<Object> source, PropertiesEnumerationMode mode, const base::ScopedVector<Handle<Object>>* excluded_properties, bool use_set)
{
    Maybe<bool> fast_assign = FastAssign(target, source, mode, excluded_properties, use_set);
    ...
    if (!from->HasFastProperties() && target->HasFastProperties() && !target->IsJSGlobalProxy()) {
        ...
        if (from->IsJSGlobalObject()) {
            source_length = JSGlobalObject::cast(*from).global_dictionary(kAcquireLoad).NumberOfEnumerableProperties();
        ...
        if (source_length > kMaxNumberOfDescriptors) {
            JSObject::NormalizeProperties(isolate, Handle<JSObject>::cast(target), CLEAR_INOBJECT_PROPERTIES, source_length, "Copying data properties");
        }
    }
        ...
        if (found.FromJust() && desc.enumerable()) {
            ...
            if (use_set) {
                ...
                ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, status,
                    Runtime::SetObjectProperty(isolate, target, next_key, prop_value, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)), Nothing<bool>());
            } else {
                // 4a ii 2. Perform ! CreateDataProperty(target, nextKey, propValue).
                PropertyKey key(isolate, next_key);
...
std::pair<MaybeHandle<JSFunction>, Handle<String>> GetConstructorHelper(Isolate* isolate, Handle<JSReceiver> receiver)
{
    ...
    if (!receiver->IsJSProxy() && receiver->map().new_target_is_base() && !receiver->map().is_prototype_map()) {
        ...
        if (maybe_constructor->IsJSFunction()) {
            ...
            Handle<String> name = SharedFunctionInfo::DebugName(handle(constructor->shared(), isolate));
    ...
    for (PrototypeIterator it(isolate, receiver, kStartAtReceiver); !it.IsAtEnd(); it.AdvanceIgnoringProxies()) {
        ...
        if (maybe_to_string_tag->IsString()) {
            return std::make_pair(MaybeHandle<JSFunction>(), Handle<String>::cast(maybe_to_string_tag));
        }
        ...
        if (!receiver.is_identical_to(current)) {
            ...
            if (maybe_constructor->IsJSFunction()) {
                ...
                auto name = SharedFunctionInfo::DebugName(handle(constructor->shared(), isolate));
...
// static
MaybeHandle<JSFunction> JSReceiver::GetConstructor(Isolate* isolate, Handle<JSReceiver> receiver) { return GetConstructorHelper(isolate, receiver).first; }

// static
Handle<String> JSReceiver::GetConstructorName(Isolate* isolate, Handle<JSReceiver> receiver) { return GetConstructorHelper(isolate, receiver).second; }
...
MaybeHandle<NativeContext> JSReceiver::GetCreationContext()
{
    ...
    return function.has_context() ? Handle<NativeContext>(function.native_context(), receiver.GetIsolate()) : MaybeHandle<NativeContext>();
}

// static
MaybeHandle<NativeContext> JSReceiver::GetFunctionRealm(Handle<JSReceiver> receiver)
{
    ...
    do {
        ...
        if (current.IsJSProxy()) {
            ...
            if (proxy.IsRevoked()) {
                ...
                THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kProxyRevoked), NativeContext);
            }
...
// static
MaybeHandle<NativeContext> JSReceiver::GetContextForMicrotask(Handle<JSReceiver> receiver)
{
    Isolate* isolate = receiver->GetIsolate();
    while (receiver->IsJSBoundFunction() || receiver->IsJSProxy()) {
        if (receiver->IsJSBoundFunction()) {
            receiver = handle(Handle<JSBoundFunction>::cast(receiver)->bound_target_function(), isolate);
        } else {
...
int GetIdentityHashHelper(JSReceiver object)
{
    ...
    if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL && properties.IsSwissNameDictionary()) {
...
{
    DCHECK_IMPLIES(properties.IsPropertyArray() && PropertyArray::cast(properties).length() == 0, properties == GetReadOnlyRoots().empty_property_array());
    DisallowGarbageCollection no_gc;
...
void JSReceiver::DeleteNormalizedProperty(Handle<JSReceiver> object, InternalIndex entry)
{
    ...
    if (object->IsJSGlobalObject()) {
        ...
        Handle<GlobalDictionary> dictionary(JSGlobalObject::cast(*object).global_dictionary(kAcquireLoad), isolate);

        Handle<PropertyCell> cell(dictionary->CellAt(entry), isolate);

        Handle<GlobalDictionary> new_dictionary = GlobalDictionary::DeleteEntry(isolate, dictionary, entry);
        JSGlobalObject::cast(*object).set_global_dictionary(*new_dictionary, kReleaseStore);
...
Maybe<bool> JSReceiver::DeleteProperty(LookupIterator* it, LanguageMode language_mode)
{
    ...
    for (; it->IsFound(); it->Next()) {
        ...
        case LookupIterator::ACCESSOR: {
            ...
            if (!it->IsConfigurable() || (holder->IsJSTypedArray() && it->IsElement(*holder))) {
...
Maybe<bool> JSReceiver::DeleteElement(Handle<JSReceiver> object, uint32_t index, LanguageMode language_mode)
{
    LookupIterator it(object->GetIsolate(), object, index, object, LookupIterator::OWN);
    return DeleteProperty(&it, language_mode);
}

Maybe<bool> JSReceiver::DeleteProperty(Handle<JSReceiver> object, Handle<Name> name, LanguageMode language_mode)
{
    LookupIterator it(object->GetIsolate(), object, name, object, LookupIterator::OWN);
    return DeleteProperty(&it, language_mode);
}

Maybe<bool> JSReceiver::DeletePropertyOrElement(Handle<JSReceiver> object, Handle<Name> name, LanguageMode language_mode)
{
    Isolate* isolate = object->GetIsolate();
    PropertyKey key(isolate, name);
...
// ES6 19.1.2.4
// static
Object JSReceiver::DefineProperty(Isolate* isolate, Handle<Object> object, Handle<Object> key, Handle<Object> attributes)
{
    ...
    Maybe<bool> success = DefineOwnProperty(isolate, Handle<JSReceiver>::cast(object), key, &desc, Just(kThrowOnError));
...
MaybeHandle<Object> JSReceiver::DefineProperties(Isolate* isolate, Handle<Object> object, Handle<Object> properties)
{
    ...
    if (!object->IsJSReceiver()) {
        ...
        THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kCalledOnNonObject, fun_name), Object);
    }
    // 2. Let props be ToObject(Properties).
    // 3. ReturnIfAbrupt(props).
    Handle<JSReceiver> props;
    ASSIGN_RETURN_ON_EXCEPTION(isolate, props, Object::ToObject(isolate, properties), Object);

    // 4. Let keys be props.[[OwnPropertyKeys]]().
    // 5. ReturnIfAbrupt(keys).
    Handle<FixedArray> keys;
    ASSIGN_RETURN_ON_EXCEPTION(isolate, keys, KeyAccumulator::GetKeys(isolate, props, KeyCollectionMode::kOwnOnly, ALL_PROPERTIES), Object);
    ...
    for (int i = 0; i < keys->length(); ++i) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, desc_obj, Object::GetProperty(&it), Object);
    ...
    for (size_t i = 0; i < descriptors_index; ++i) {
        ...
        Maybe<bool> status = DefineOwnProperty(isolate, Handle<JSReceiver>::cast(object), desc->name(), desc, Just(kThrowOnError));
...
// static
Maybe<bool> JSReceiver::DefineOwnProperty(Isolate* isolate, Handle<JSReceiver> object, Handle<Object> key, PropertyDescriptor* desc, Maybe<ShouldThrow> should_throw)
{
    if (object->IsJSArray()) {
        return JSArray::DefineOwnProperty(isolate, Handle<JSArray>::cast(object), key, desc, should_throw);
    }
    if (object->IsJSProxy()) {
        return JSProxy::DefineOwnProperty(isolate, Handle<JSProxy>::cast(object), key, desc, should_throw);
    }
    if (object->IsJSTypedArray()) {
        return JSTypedArray::DefineOwnProperty(isolate, Handle<JSTypedArray>::cast(object), key, desc, should_throw);
    }
    if (object->IsJSModuleNamespace()) {
        return JSModuleNamespace::DefineOwnProperty(isolate, Handle<JSModuleNamespace>::cast(object), key, desc, should_throw);
    }
    ...
    return OrdinaryDefineOwnProperty(isolate, Handle<JSObject>::cast(object), key, desc, should_throw);
}

// static
Maybe<bool> JSReceiver::OrdinaryDefineOwnProperty(Isolate* isolate, Handle<JSObject> object, Handle<Object> key, PropertyDescriptor* desc, Maybe<ShouldThrow> should_throw)
{
...
Maybe<bool> JSReceiver::OrdinaryDefineOwnProperty(Isolate* isolate, Handle<JSObject> object, const PropertyKey& key, PropertyDescriptor* desc, Maybe<ShouldThrow> should_throw)
{
    LookupIterator it(isolate, object, key, LookupIterator::OWN);
...
MaybeHandle<Object> GetPropertyWithInterceptorInternal(LookupIterator* it, Handle<InterceptorInfo> interceptor, bool* done)
{
    *done = false;
    Isolate* isolate = it->isolate();
    ...
    RETURN_VALUE_IF_SCHEDULED_EXCEPTION_DETECTOR(isolate, args, MaybeHandle<Object>());
    if (result.is_null())
        return isolate->factory()->undefined_value();
    *done = true;
...
Maybe<PropertyAttributes> GetPropertyAttributesWithInterceptorInternal(LookupIterator* it, Handle<InterceptorInfo> interceptor)
{
    ...
    Handle<JSObject> holder = it->GetHolder<JSObject>();
    DCHECK_IMPLIES(!it->IsElement(*holder) && it->name()->IsSymbol(), interceptor->can_intercept_symbols());
    Handle<Object> receiver = it->GetReceiver();
    if (!receiver->IsJSReceiver()) {
    ...
    RETURN_VALUE_IF_SCHEDULED_EXCEPTION_DETECTOR(isolate, args, Nothing<PropertyAttributes>());
    return Just(ABSENT);
}

Maybe<bool> SetPropertyWithInterceptorInternal(LookupIterator* it, Handle<InterceptorInfo> interceptor, Maybe<ShouldThrow> should_throw, Handle<Object> value)
{
    Isolate* isolate = it->isolate();
    // Make sure that the top context does not change when doing callbacks or
    // interceptor calls.
    ...
    if (it->IsElement(*holder)) {
        ...
        result = !args.CallIndexedSetter(interceptor, it->array_index(), value).is_null();
    } else {
        result = !args.CallNamedSetter(interceptor, it->name(), value).is_null();
    }

    RETURN_VALUE_IF_SCHEDULED_EXCEPTION_DETECTOR(it->isolate(), args, Nothing<bool>());
    if (result)
        args.AcceptSideEffects();
    return Just(result);
}

Maybe<bool> DefinePropertyWithInterceptorInternal(LookupIterator* it, Handle<InterceptorInfo> interceptor, Maybe<ShouldThrow> should_throw, PropertyDescriptor* desc)
{
    Isolate* isolate = it->isolate();
    // Make sure that the top context does not change when doing callbacks or
    // interceptor calls.
    ...
    if (PropertyDescriptor::IsAccessorDescriptor(desc)) {
        descriptor.reset(new v8::PropertyDescriptor(v8::Utils::ToLocal(desc->get()), v8::Utils::ToLocal(desc->set())));
    ...
    PropertyCallbackArguments args(isolate, interceptor->data(), *receiver, *holder, should_throw);
    if (it->IsElement(*holder)) {
        result = !args.CallIndexedDefiner(interceptor, it->array_index(), *descriptor).is_null();
    } else {
...
// ES6 9.1.6.1
// static
Maybe<bool> JSReceiver::OrdinaryDefineOwnProperty(LookupIterator* it, PropertyDescriptor* desc, Maybe<ShouldThrow> should_throw)
{
    Isolate* isolate = it->isolate();
    ...
    for (; it->IsFound(); it->Next()) {
        if (it->state() == LookupIterator::INTERCEPTOR) {
            if (it->HolderIsReceiverOrHiddenPrototype()) {
                Maybe<bool> result = DefinePropertyWithInterceptorInternal(it, it->GetInterceptor(), should_throw, desc);
...
Maybe<bool> JSReceiver::ValidateAndApplyPropertyDescriptor(
    Isolate* isolate, LookupIterator* it, bool extensible, PropertyDescriptor* desc, PropertyDescriptor* current, Maybe<ShouldThrow> should_throw, Handle<Name> property_name)
{
    ...
    if (current->is_empty()) {
        ...
        if (!extensible) {
            RETURN_FAILURE(isolate, GetShouldThrow(isolate, should_throw), NewTypeError(MessageTemplate::kDefineDisallowed, it != nullptr ? it->GetName() : property_name));
        }
        ...
        if (!desc_is_accessor_descriptor) {
            ...
            if (it != nullptr) {
                ...
                if (!desc->has_configurable())
                    desc->set_configurable(false);
                Handle<Object> getter(desc->has_get() ? desc->get() : Handle<Object>::cast(isolate->factory()->null_value()));
                Handle<Object> setter(desc->has_set() ? desc->set() : Handle<Object>::cast(isolate->factory()->null_value()));
    ...
    if ((!desc->has_enumerable() || desc->enumerable() == current->enumerable()) && (!desc->has_configurable() || desc->configurable() == current->configurable())
        && (!desc->has_value() || (current->has_value() && current->value()->SameValue(*desc->value())))
        && (!desc->has_writable() || (current->has_writable() && current->writable() == desc->writable()))
        && (!desc->has_get() || (current->has_get() && current->get()->SameValue(*desc->get())))
        && (!desc->has_set() || (current->has_set() && current->set()->SameValue(*desc->set())))) {
        return Just(true);
    }
    ...
    if (!current->configurable()) {
        ...
        if (desc->has_configurable() && desc->configurable()) {
            RETURN_FAILURE(isolate, GetShouldThrow(isolate, should_throw), NewTypeError(MessageTemplate::kRedefineDisallowed, it != nullptr ? it->GetName() : property_name));
        }
    ...
    if (it != nullptr) {
        ...
        if (desc_is_data_descriptor || (desc_is_generic_descriptor && current_is_data_descriptor)) {
            ...
            Handle<Object> value(desc->has_value() ? desc->value() : current->has_value() ? current->value() : Handle<Object>::cast(isolate->factory()->undefined_value()));
            ...
            Handle<Object> getter(desc->has_get() ? desc->get() : current->has_get() ? current->get() : Handle<Object>::cast(isolate->factory()->null_value()));
            Handle<Object> setter(desc->has_set() ? desc->set() : current->has_set() ? current->set() : Handle<Object>::cast(isolate->factory()->null_value()));
...
// static
Maybe<bool> JSReceiver::CreateDataProperty(LookupIterator* it, Handle<Object> value, Maybe<ShouldThrow> should_throw)
{
    ...
    return JSReceiver::DefineOwnProperty(isolate, receiver, it->GetName(), &new_desc, should_throw);
}
...
Maybe<bool> JSReceiver::AddPrivateField(LookupIterator* it, Handle<Object> value, Maybe<ShouldThrow> should_throw)
{
    ...
    case LookupIterator::JSPROXY: {
        ...
        new_desc.set_enumerable(true);
        new_desc.set_configurable(true);
        return JSProxy::SetPrivateSymbol(isolate, Handle<JSProxy>::cast(receiver), symbol, &new_desc, should_throw);
    }
    case LookupIterator::WASM_OBJECT:
        RETURN_FAILURE(isolate, kThrowOnError, NewTypeError(MessageTemplate::kWasmObjectsAreOpaque));
    case LookupIterator::DATA:
...
Maybe<bool> GetPropertyDescriptorWithInterceptor(LookupIterator* it, PropertyDescriptor* desc)
{
    ...
    if (it->state() == LookupIterator::ACCESS_CHECK) {
        if (it->HasAccess()) {
            ...
            if (interceptor.is_null() && (!JSObject::AllCanRead(it) || it->state() != LookupIterator::INTERCEPTOR)) {
                it->Restart();
                return Just(false);
            }
        }
...
Maybe<bool> JSReceiver::GetOwnPropertyDescriptor(LookupIterator* it, PropertyDescriptor* desc)
{
    ...
    if (!is_accessor_pair) {
        ...
        Handle<NativeContext> native_context = it->GetHolder<JSReceiver>()->GetCreationContext().ToHandleChecked();
        // 6a. Set D.[[Get]] to the value of X's [[Get]] attribute.
        desc->set_get(AccessorPair::GetComponent(isolate, native_context, accessors, ACCESSOR_GETTER));
        // 6b. Set D.[[Set]] to the value of X's [[Set]] attribute.
        desc->set_set(AccessorPair::GetComponent(isolate, native_context, accessors, ACCESSOR_SETTER));
    }
...
Maybe<bool> JSReceiver::SetIntegrityLevel(Handle<JSReceiver> receiver, IntegrityLevel level, ShouldThrow should_throw)
{
    ...
    if (receiver->IsJSObject()) {
        ...
        if (!object->HasSloppyArgumentsElements() && !object->IsJSModuleNamespace()) { // Fast path.
            ...
            if (level == SEALED) {
                return JSObject::PreventExtensionsWithTransition<SEALED>(object, should_throw);
            } else {
                return JSObject::PreventExtensionsWithTransition<FROZEN>(object, should_throw);
            }
        }
    }
    ...
    MAYBE_RETURN(JSReceiver::PreventExtensions(receiver, should_throw), Nothing<bool>());
...
namespace {
Maybe<bool> GenericTestIntegrityLevel(Handle<JSReceiver> receiver, PropertyAttributes level)
{
    DCHECK(level == SEALED || level == FROZEN);
...
Maybe<bool> JSReceiver::TestIntegrityLevel(Handle<JSReceiver> receiver, IntegrityLevel level)
{
    if (!receiver->map().IsCustomElementsReceiverMap()) {
        return JSObject::TestIntegrityLevel(Handle<JSObject>::cast(receiver), level);
    }
    return GenericTestIntegrityLevel(receiver, level);
}

Maybe<bool> JSReceiver::PreventExtensions(Handle<JSReceiver> object, ShouldThrow should_throw)
{
    if (object->IsJSProxy()) {
        return JSProxy::PreventExtensions(Handle<JSProxy>::cast(object), should_throw);
    }
    if (object->IsWasmObject()) {
        RETURN_FAILURE(object->GetIsolate(), kThrowOnError, NewTypeError(MessageTemplate::kWasmObjectsAreOpaque));
    }
    DCHECK(object->IsJSObject());
    return JSObject::PreventExtensions(Handle<JSObject>::cast(object), should_throw);
}
...
MaybeHandle<Object> JSReceiver::ToPrimitive(Isolate* isolate, Handle<JSReceiver> receiver, ToPrimitiveHint hint)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, exotic_to_prim, Object::GetMethod(receiver, isolate->factory()->to_primitive_symbol()), Object);
    if (!exotic_to_prim->IsUndefined(isolate)) {
        Handle<Object> hint_string = isolate->factory()->ToPrimitiveHintString(hint);
        Handle<Object> result;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, result, Execution::Call(isolate, exotic_to_prim, receiver, 1, &hint_string), Object);
        if (result->IsPrimitive())
            return result;
        THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kCannotConvertToPrimitive), Object);
    }
...
MaybeHandle<Object> JSReceiver::OrdinaryToPrimitive(Isolate* isolate, Handle<JSReceiver> receiver, OrdinaryToPrimitiveHint hint)
{
    ...
    for (Handle<String> name : method_names) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, method, JSReceiver::GetProperty(isolate, receiver, name), Object);
        if (method->IsCallable()) {
            Handle<Object> result;
            ASSIGN_RETURN_ON_EXCEPTION(isolate, result, Execution::Call(isolate, method, receiver, 0, nullptr), Object);
            if (result->IsPrimitive())
                return result;
        }
    }
...
V8_WARN_UNUSED_RESULT Maybe<bool> FastGetOwnValuesOrEntries(Isolate* isolate, Handle<JSReceiver> receiver, bool get_entries, Handle<FixedArray>* result)
{
    ...
    if (object->elements() != ReadOnlyRoots(isolate).empty_fixed_array()) {
        MAYBE_RETURN(object->GetElementsAccessor()->CollectValuesOrEntries(isolate, object, values_or_entries, get_entries, &count, ENUMERABLE_STRINGS), Nothing<bool>());
    }
...
MaybeHandle<FixedArray> GetOwnValuesOrEntries(Isolate* isolate, Handle<JSReceiver> object, PropertyFilter filter, bool try_fast_path, bool get_entries)
{
    ...
    Handle<FixedArray> keys;
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(
        isolate, keys, KeyAccumulator::GetKeys(isolate, object, KeyCollectionMode::kOwnOnly, key_filter, GetKeysConversion::kConvertToString), MaybeHandle<FixedArray>());

    values_or_entries = isolate->factory()->NewFixedArray(keys->length());
    int length = 0;

    for (int i = 0; i < keys->length(); ++i) {
        Handle<Name> key = Handle<Name>::cast(handle(keys->get(isolate, i), isolate));
...
Maybe<bool> JSReceiver::SetPrototype(Isolate* isolate, Handle<JSReceiver> object, Handle<Object> value, bool from_javascript, ShouldThrow should_throw)
{
    ...
    if (object->IsJSProxy()) {
        return JSProxy::SetPrototype(isolate, Handle<JSProxy>::cast(object), value, from_javascript, should_throw);
    }
    return JSObject::SetPrototype(isolate, Handle<JSObject>::cast(object), value, from_javascript, should_throw);
}
...
bool JSReceiver::IsCodeLike(Isolate* isolate) const
{
    ...
    Object instance_template = JSFunction::cast(maybe_constructor).shared().get_api_func_data().GetInstanceTemplate();
    if (instance_template.IsUndefined(isolate))
        return false;
    return ObjectTemplateInfo::cast(instance_template).code_like();
}

// static
MaybeHandle<JSObject> JSObject::New(Handle<JSFunction> constructor, Handle<JSReceiver> new_target, Handle<AllocationSite> site)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, initial_map, JSFunction::GetDerivedMap(isolate, constructor, new_target), JSObject);
    int initial_capacity = V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL ? SwissNameDictionary::kInitialCapacity : NameDictionary::kInitialCapacity;
    Handle<JSObject> result = isolate->factory()->NewFastOrSlowJSObjectFromMap(initial_map, initial_capacity, AllocationType::kYoung, site);
    return result;
}
...
void JSObject::EnsureWritableFastElements(Handle<JSObject> object)
{
    ...
    Handle<FixedArray> writable_elems = isolate->factory()->CopyFixedArrayWithMap(elems, isolate->factory()->fixed_array_map());
...
MaybeHandle<Object> JSObject::GetPropertyWithFailedAccessCheck(LookupIterator* it)
{
    Isolate* isolate = it->isolate();
    ...
    if (interceptor.is_null()) {
        while (AllCanRead(it)) {
            ...
            bool done;
            Handle<Object> result;
            ASSIGN_RETURN_ON_EXCEPTION(isolate, result, GetPropertyWithInterceptor(it, &done), Object);
            if (done)
                return result;
        }
        ...
        Handle<Object> result;
        bool done;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, result, GetPropertyWithInterceptorInternal(it, interceptor, &done), Object);
        if (done)
            return result;
    }
...
Maybe<bool> JSObject::SetPropertyWithFailedAccessCheck(LookupIterator* it, Handle<Object> value, Maybe<ShouldThrow> should_throw)
{
    ...
    if (interceptor.is_null()) {
        ...
        Maybe<bool> result = SetPropertyWithInterceptorInternal(it, interceptor, should_throw, value);
        if (isolate->has_pending_exception())
            return Nothing<bool>();
        if (result.IsJust())
            return result;
...
void JSObject::SetNormalizedProperty(Handle<JSObject> object, Handle<Name> name, Handle<Object> value, PropertyDetails details)
{
    ...
    if (object->IsJSGlobalObject()) {
        ...
        if (entry.is_not_found()) {
            ...
            auto cell_type = value->IsUndefined(roots) ? PropertyCellType::kUndefined : PropertyCellType::kConstant;
...
void JSObject::SetNormalizedElement(Handle<JSObject> object, uint32_t index, Handle<Object> value, PropertyDetails details)
{
    ...
    Handle<NumberDictionary> dictionary = handle(NumberDictionary::cast(object->elements()), isolate);
...
void JSObject::JSObjectShortPrint(StringStream* accumulator)
{
    ...
    case JS_ARRAY_TYPE: {
        double length = JSArray::cast(*this).length().IsUndefined() ? 0 : JSArray::cast(*this).length().Number();
...
void JSObject::PrintElementsTransition(
    FILE* file, Handle<JSObject> object, ElementsKind from_kind, Handle<FixedArrayBase> from_elements, ElementsKind to_kind, Handle<FixedArrayBase> to_elements)
{
...
void JSObject::PrintInstanceMigration(FILE* file, Map original_map, Map new_map)
{
...
bool JSObject::IsUnmodifiedApiObject(FullObjectSlot o)
{
    ...
    if (!js_object.IsDroppableApiObject())
        return false;
...
// static
void JSObject::UpdatePrototypeUserRegistration(Handle<Map> old_map, Handle<Map> new_map, Isolate* isolate)
{
    ...
    if (v8_flags.trace_prototype_users) {
        PrintF("Moving prototype_info %p from map %p to map %p.\n", reinterpret_cast<void*>(new_map->prototype_info().ptr()), reinterpret_cast<void*>(old_map->ptr()),
    ...
    if (was_registered) {
        if (new_map->prototype_info().IsPrototypeInfo()) {
            ...
            PrototypeInfo::cast(new_map->prototype_info()).set_registry_slot(PrototypeInfo::UNREGISTERED);
        }
...
// static
void JSObject::NotifyMapChange(Handle<Map> old_map, Handle<Map> new_map, Isolate* isolate)
{
...
//     store.
void MigrateFastToFast(Isolate* isolate, Handle<JSObject> object, Handle<Map> new_map)
{
    ...
    if (new_map->GetBackPointer(isolate) == *old_map) {
        ...
        // double boxes).
        FieldIndex index = FieldIndex::ForDescriptor(isolate, *new_map, new_map->LastAdded());
    ...
    Handle<DescriptorArray> old_descriptors(old_map->instance_descriptors(isolate), isolate);
    Handle<DescriptorArray> new_descriptors(new_map->instance_descriptors(isolate), isolate);
    ...
    for (InternalIndex i : InternalIndex::Range(old_nof)) {
        ...
        if (old_details.location() == PropertyLocation::kDescriptor) {
            ...
            if (!old_representation.IsDouble() && representation.IsDouble()) {
                DCHECK_IMPLIES(old_representation.IsNone(), value->IsUninitialized(isolate));
...
void MigrateFastToSlow(Isolate* isolate, Handle<JSObject> object, Handle<Map> new_map, int expected_additional_properties)
{
...
void JSObject::MigrateToMap(Isolate* isolate, Handle<JSObject> object, Handle<Map> new_map, int expected_additional_properties)
{
...
void JSObject::ForceSetPrototype(Isolate* isolate, Handle<JSObject> object, Handle<HeapObject> proto)
{
...
Maybe<bool> JSObject::SetPropertyWithInterceptor(LookupIterator* it, Maybe<ShouldThrow> should_throw, Handle<Object> value)
{
    DCHECK_EQ(LookupIterator::INTERCEPTOR, it->state());
    return SetPropertyWithInterceptorInternal(it, it->GetInterceptor(), should_throw, value);
}

Handle<Map> JSObject::GetElementsTransitionMap(Handle<JSObject> object, ElementsKind to_kind)
{
...
void JSObject::AddProperty(Isolate* isolate, Handle<JSObject> object, Handle<Name> name, Handle<Object> value, PropertyAttributes attributes)
{
...
void JSObject::AddProperty(Isolate* isolate, Handle<JSObject> object, const char* name, Handle<Object> value, PropertyAttributes attributes)
{
...
Maybe<bool> JSObject::DefineOwnPropertyIgnoreAttributes(LookupIterator* it, Handle<Object> value, PropertyAttributes attributes, Maybe<ShouldThrow> should_throw,
    AccessorInfoHandling handling, EnforceDefineSemantics semantics, StoreOrigin store_origin)
{
    it->UpdateProtector();
    ...
    for (; it->IsFound(); it->Next()) {
        ...
        case LookupIterator::INTERCEPTOR: {
            ...
            if (semantics == EnforceDefineSemantics::kDefine) {
                ...
                result = DefinePropertyWithInterceptorInternal(it, it->GetInterceptor(), should_throw, &descriptor);
            } else {
                DCHECK_EQ(semantics, EnforceDefineSemantics::kSet);
                if (handling == DONT_FORCE_FIELD) {
                    result = JSObject::SetPropertyWithInterceptor(it, should_throw, value);
                }
            }
            if (result.IsNothing() || result.FromJust())
                return result;
            ...
            if (semantics == EnforceDefineSemantics::kDefine) {
                it->Restart();
                Maybe<bool> can_define = JSReceiver::CheckIfCanDefine(it->isolate(), it, value, should_throw);
        ...
        case LookupIterator::DATA: {
            ...
            DCHECK(!it->IsElement() || !Handle<JSObject>::cast(it->GetReceiver())->HasTypedArrayOrRabGsabTypedArrayElements());
...
MaybeHandle<Object> JSObject::DefinePropertyOrElementIgnoreAttributes(Handle<JSObject> object, Handle<Name> name, Handle<Object> value, PropertyAttributes attributes)
{
    Isolate* isolate = object->GetIsolate();
    PropertyKey key(isolate, name);
    LookupIterator it(isolate, object, key, object, LookupIterator::OWN);
...
Maybe<PropertyAttributes> JSObject::GetPropertyAttributesWithInterceptor(LookupIterator* it) { return GetPropertyAttributesWithInterceptorInternal(it, it->GetInterceptor()); }

void JSObject::NormalizeProperties(
    Isolate* isolate, Handle<JSObject> object, PropertyNormalizationMode mode, int expected_additional_properties, bool use_cache, const char* reason)
{
    if (!object->HasFastProperties())
        return;

    Handle<Map> map(object->map(), isolate);
    Handle<Map> new_map = Map::Normalize(isolate, map, map->elements_kind(), mode, use_cache, reason);

    JSObject::MigrateToMap(isolate, object, new_map, expected_additional_properties);
}

void JSObject::MigrateSlowToFast(Handle<JSObject> object, int unused_property_fields, const char* reason)
{
    if (object->HasFastProperties())
        return;
    ...
    Handle<DescriptorArray> descriptors = DescriptorArray::Allocate(isolate, number_of_elements, 0);
...
Handle<NumberDictionary> JSObject::NormalizeElements(Handle<JSObject> object)
{
    ...
    ElementsKind target_kind = is_sloppy_arguments ? SLOW_SLOPPY_ARGUMENTS_ELEMENTS : object->HasFastStringWrapperElements() ? SLOW_STRING_WRAPPER_ELEMENTS : DICTIONARY_ELEMENTS;
    ...
    if (is_sloppy_arguments) {
        SloppyArgumentsElements::cast(object->elements()).set_arguments(*dictionary);
    } else {
        object->set_elements(*dictionary);
    }
...
Maybe<bool> JSObject::DeletePropertyWithInterceptor(LookupIterator* it, ShouldThrow should_throw)
{
    Isolate* isolate = it->isolate();
    // Make sure that the top context does not change when doing callbacks or
    // interceptor calls.
...
Maybe<bool> JSObject::CreateDataProperty(LookupIterator* it, Handle<Object> value, Maybe<ShouldThrow> should_throw)
{
    ...
    RETURN_ON_EXCEPTION_VALUE(it->isolate(), DefineOwnPropertyIgnoreAttributes(it, value, NONE), Nothing<bool>());
...
template <typename Dictionary> bool TestDictionaryPropertiesIntegrityLevel(Dictionary dict, ReadOnlyRoots roots, PropertyAttributes level)
{
    DCHECK(level == SEALED || level == FROZEN);
...
bool TestElementsIntegrityLevel(JSObject object, PropertyAttributes level)
{
    ...
    if (IsDictionaryElementsKind(kind)) {
        return TestDictionaryPropertiesIntegrityLevel(NumberDictionary::cast(object.elements()), object.GetReadOnlyRoots(), level);
    }
...
Maybe<bool> JSObject::TestIntegrityLevel(Handle<JSObject> object, IntegrityLevel level)
{
...
Maybe<bool> JSObject::PreventExtensions(Handle<JSObject> object, ShouldThrow should_throw)
{
    Isolate* isolate = object->GetIsolate();
    ...
    if (object->IsAccessCheckNeeded() && !isolate->MayAccess(handle(isolate->context(), isolate), object)) {
        ...
        RETURN_VALUE_IF_SCHEDULED_EXCEPTION(isolate, Nothing<bool>());
        RETURN_FAILURE(isolate, should_throw, NewTypeError(MessageTemplate::kNoAccess));
    }
    ...
    if (object->IsJSGlobalProxy()) {
        ...
        return PreventExtensions(PrototypeIterator::GetCurrent<JSObject>(iter), should_throw);
    }
...
template <typename Dictionary> void JSObject::ApplyAttributesToDictionary(Isolate* isolate, ReadOnlyRoots roots, Handle<Dictionary> dictionary, const PropertyAttributes attributes)
{
...
template void JSObject::ApplyAttributesToDictionary(Isolate* isolate, ReadOnlyRoots roots, Handle<NumberDictionary> dictionary, const PropertyAttributes attributes);
...
Handle<NumberDictionary> CreateElementDictionary(Isolate* isolate, Handle<JSObject> object)
{
    ...
    if (!object->HasTypedArrayOrRabGsabTypedArrayElements() && !object->HasDictionaryElements() && !object->HasSlowStringWrapperElements()) {
        int length = object->IsJSArray() ? Smi::ToInt(Handle<JSArray>::cast(object)->length()) : object->elements().length();
...
template <PropertyAttributes attrs> Maybe<bool> JSObject::PreventExtensionsWithTransition(Handle<JSObject> object, ShouldThrow should_throw)
{
    ...
    if (object->IsJSGlobalProxy()) {
        ...
        if (iter.IsAtEnd())
            return Just(true);
        DCHECK(PrototypeIterator::GetCurrent(iter)->IsJSGlobalObject());
        return PreventExtensionsWithTransition<attrs>(PrototypeIterator::GetCurrent<JSObject>(iter), should_throw);
    }
    ...
    if (maybe_transition_map.ToHandle(&transition_map)) {
        ...
        Handle<Map> new_map = Map::CopyForPreventExtensions(isolate, old_map, attrs, transition_marker, "CopyForPreventExtensions");
        ...
        // TODO(adamk): Extend the NormalizedMapCache to handle non-extensible maps.
        Handle<Map> new_map = Map::Copy(isolate, handle(object->map(), isolate), "SlowCopyForPreventExtensions");
        new_map->set_is_extensible(false);
        new_element_dictionary = CreateElementDictionary(isolate, object);
...
Handle<Object> JSObject::FastPropertyAt(Isolate* isolate, Handle<JSObject> object, Representation representation, FieldIndex index)
{
    Handle<Object> raw_value(object->RawFastPropertyAt(index), isolate);
    return Object::WrapForRead(isolate, raw_value, representation);
}

Handle<Object> JSObject::FastPropertyAt(Isolate* isolate, Handle<JSObject> object, Representation representation, FieldIndex index, SeqCstAccessTag tag)
{
...
// static
Handle<Object> JSObject::DictionaryPropertyAt(Isolate* isolate, Handle<JSObject> object, InternalIndex dict_index)
{
...
// static
base::Optional<Object> JSObject::DictionaryPropertyAt(Handle<JSObject> object, InternalIndex dict_index, Heap* heap)
{
    Object backing_store = object->raw_properties_or_hash(kRelaxedLoad);
    if (!backing_store.IsHeapObject())
        return {};
    if (heap->IsPendingAllocation(HeapObject::cast(backing_store)))
        return {};

    base::Optional<Object> maybe_obj;
    if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
        if (!backing_store.IsSwissNameDictionary())
            return {};
        maybe_obj = SwissNameDictionary::cast(backing_store).TryValueAt(dict_index);
    } else {
        if (!backing_store.IsNameDictionary())
            return {};
...
bool JSObject::HasEnumerableElements()
{
    ...
    case SHARED_ARRAY_ELEMENTS: {
        int length = object.IsJSArray() ? Smi::ToInt(JSArray::cast(object).length()) : object.elements().length();
        return length > 0;
    }
    case HOLEY_SMI_ELEMENTS:
    ...
    case HOLEY_ELEMENTS: {
        ...
        int length = object.IsJSArray() ? Smi::ToInt(JSArray::cast(object).length()) : elements.length();
        Isolate* isolate = GetIsolate();
...
MaybeHandle<Object> JSObject::DefineAccessor(Handle<JSObject> object, Handle<Name> name, Handle<Object> getter, Handle<Object> setter, PropertyAttributes attributes)
{
    Isolate* isolate = object->GetIsolate();
...
MaybeHandle<Object> JSObject::DefineAccessor(LookupIterator* it, Handle<Object> getter, Handle<Object> setter, PropertyAttributes attributes)
{
    Isolate* isolate = it->isolate();
...
Object JSObject::SlowReverseLookup(Object value)
{
    if (HasFastProperties()) {
        ...
        return JSGlobalObject::cast(*this).global_dictionary(kAcquireLoad).SlowReverseLookup(value);
    } else if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
...
void JSObject::PrototypeRegistryCompactionCallback(HeapObject value, int old_index, int new_index)
{
...
void JSObject::MakePrototypesFast(Handle<Object> receiver, WhereToStart where_to_start, Isolate* isolate)
{
    if (!receiver->IsJSReceiver())
        return;
    for (PrototypeIterator iter(isolate, Handle<JSReceiver>::cast(receiver), where_to_start); !iter.IsAtEnd(); iter.Advance()) {
...
void JSObject::OptimizeAsPrototype(Handle<JSObject> object, bool enable_setup_mode)
{
    ...
    if (object->map(isolate).is_prototype_map()) {
        ...
        if (!V8_DICT_PROPERTY_CONST_TRACKING_BOOL && object->map(isolate).should_be_fast_prototype_map() && !object->HasFastProperties()) {
...
#ifdef DEBUG
    bool should_be_dictionary = V8_DICT_PROPERTY_CONST_TRACKING_BOOL && enable_setup_mode && !object->IsJSGlobalProxy() && !isolate->bootstrapper()->IsActive();
...
void JSObject::LazyRegisterPrototypeUser(Handle<Map> user, Isolate* isolate)
{
    ...
    for (PrototypeIterator iter(isolate, user); !iter.IsAtEnd(); iter.Advance()) {
        ...
        Handle<WeakArrayList> registry
            = maybe_registry->IsSmi() ? handle(ReadOnlyRoots(isolate->heap()).empty_weak_array_list(), isolate) : Handle<WeakArrayList>::cast(maybe_registry);
        int slot = 0;
...
bool JSObject::UnregisterPrototypeUser(Handle<Map> user, Isolate* isolate)
{
    ...
    Handle<PrototypeInfo> proto_info(PrototypeInfo::cast(maybe_proto_info), isolate);
    Handle<WeakArrayList> prototype_users(WeakArrayList::cast(proto_info->prototype_users()), isolate);
...
Maybe<bool> JSObject::SetPrototype(Isolate* isolate, Handle<JSObject> object, Handle<Object> value, bool from_javascript, ShouldThrow should_throw)
{
#ifdef DEBUG
    ...
    if (immutable_proto) {
        RETURN_FAILURE(isolate, should_throw, NewTypeError(MessageTemplate::kImmutablePrototypeSet, object));
    }
    ...
    if (value->IsJSReceiver()) {
        for (PrototypeIterator iter(isolate, JSReceiver::cast(*value), kStartAtReceiver); !iter.IsAtEnd(); iter.Advance()) {
    ...
    isolate->UpdateTypedArraySpeciesLookupChainProtectorOnSetPrototype(real_receiver);
...
static bool ShouldConvertToFastElements(JSObject object, NumberDictionary dictionary, uint32_t index, uint32_t* new_capacity)
{
...
Maybe<bool> JSObject::AddDataElement(Handle<JSObject> object, uint32_t index, Handle<Object> value, PropertyAttributes attributes)
{
    ...
    if (attributes != NONE) {
        ...
        kind = ShouldConvertToFastElements(*object, NumberDictionary::cast(elements), index, &new_capacity) ? BestFittingFastElementsKind(*object) : dictionary_kind;
    ...
    if (IsHoleyElementsKind(kind) || !object->IsJSArray(isolate) || index > old_length) {
...
template <AllocationSiteUpdateMode update_or_check> bool JSObject::UpdateAllocationSite(Handle<JSObject> object, ElementsKind to_kind)
{
    if (!object->IsJSArray())
        return false;
    ...
    Handle<AllocationSite> site;
    {
        ...
        AllocationMemento memento = pretunring_handler->FindAllocationMemento<PretenturingHandler::kForRuntime>(object->map(), *object);
        if (memento.is_null())
            return false;
...
template bool JSObject::UpdateAllocationSite<AllocationSiteUpdateMode::kCheckOnly>(Handle<JSObject> object, ElementsKind to_kind);
...
template <typename BackingStore> static int HoleyElementsUsage(JSObject object, BackingStore store)
{
    Isolate* isolate = object.GetIsolate();
    int limit = object.IsJSArray() ? Smi::ToInt(JSArray::cast(object).length()) : store.length();
    int used = 0;
    for (int i = 0; i < limit; ++i) {
...
int JSObject::GetFastElementsUsage()
{
    ...
    case PACKED_NONEXTENSIBLE_ELEMENTS:
    case SHARED_ARRAY_ELEMENTS:
        return IsJSArray() ? Smi::ToInt(JSArray::cast(*this).length()) : store.length();
    case FAST_SLOPPY_ARGUMENTS_ELEMENTS:
...
MaybeHandle<Object> JSObject::GetPropertyWithInterceptor(LookupIterator* it, bool* done)
{
    DCHECK_EQ(LookupIterator::INTERCEPTOR, it->state());
...
bool JSGlobalProxy::IsDetached() const { return native_context().IsNull(GetIsolate()); }

void JSGlobalObject::InvalidatePropertyCell(Handle<JSGlobalObject> global, Handle<Name> name)
{
...
MaybeHandle<JSDate> JSDate::New(Handle<JSFunction> constructor, Handle<JSReceiver> new_target, double tv)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, result, JSObject::New(constructor, new_target, Handle<AllocationSite>::null()), JSDate);
...
Address JSDate::GetField(Isolate* isolate, Address raw_object, Address smi_index)
{
    ...
    Smi index(smi_index);
    return JSDate::cast(object).DoGetField(isolate, static_cast<FieldIndex>(index.value())).ptr();
}
...
Object JSDate::GetUTCField(FieldIndex index, double value, DateCache* date_cache)
{
...
// static
void JSMessageObject::EnsureSourcePositionsAvailable(Isolate* isolate, Handle<JSMessageObject> message)
{
    if (!message->DidEnsureSourcePositionsAvailable()) {
        ...
        Handle<SharedFunctionInfo> shared_info(SharedFunctionInfo::cast(message->shared_info()), isolate);
...
int JSMessageObject::GetLineNumber() const
{
    ...
    if (!Script::GetPositionInfo(the_script, start_position(), &info, offset_flag)) {
        return Message::kNoLineNumberInfo;
    }
