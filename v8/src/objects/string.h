...
public:
    ...
    template <typename TDispatcher, typename TResult, typename... TArgs> inline TResult DispatchToSpecificType(String str, TArgs&&... args);
...
public:
    ...
    public:
        ...
        bool UsesSameString(const FlatContent& other) const { return onebyte_start == other.onebyte_start; }
    ...
    private:
        ...
        inline FlatContent(const uint8_t* start, int length, const DisallowGarbageCollection& no_gc);
        inline FlatContent(const base::uc16* start, int length, const DisallowGarbageCollection& no_gc);
    ...
    void MakeThin(IsolateT* isolate, String canonical, UpdateInvalidatedObjectSize update_invalidated_object_size = UpdateInvalidatedObjectSize::kYes);
    ...
    // read-only strings).
    template <typename Char> inline const Char* GetChars(PtrComprCageBase cage_base, const DisallowGarbageCollection& no_gc) const;

    // Get chars from sequential or external strings.
    template <typename Char>
    inline const Char* GetChars(PtrComprCageBase cage_base, const DisallowGarbageCollection& no_gc, const SharedStringAccessGuardIfNeeded& access_guard) const;

    // Returns the address of the character at an offset into this string.
    // Requires: this->IsFlat()
    const byte* AddressOfCharacterAt(int start_index, const DisallowGarbageCollection& no_gc);
    ...
    V8_INLINE uint16_t Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
    ...
    static Object IndexOf(Isolate* isolate, Handle<Object> receiver, Handle<Object> search, Handle<Object> position);
    ...
    static Object LastIndexOf(Isolate* isolate, Handle<Object> receiver, Handle<Object> search, Handle<Object> position);
    ...
    // replacement string.
    V8_WARN_UNUSED_RESULT static MaybeHandle<String> GetSubstitution(Isolate* isolate, Match* match, Handle<String> replacement, int start_index = 0);
    ...
    template <EqualityType kEqType = EqualityType::kWholeString, typename Char> inline bool IsEqualTo(base::Vector<const Char> str, LocalIsolate* isolate) const;
    ...
    V8_EXPORT_PRIVATE std::unique_ptr<char[]> ToCString(
        AllowNullsFlag allow_nulls = DISALLOW_NULLS, RobustnessFlag robustness_flag = FAST_STRING_TRAVERSAL, int* length_output = nullptr);
    ...
    V8_EXPORT_PRIVATE bool MakeExternal(v8::String::ExternalStringResource* resource);
    V8_EXPORT_PRIVATE bool MakeExternal(v8::String::ExternalOneByteStringResource* resource);
    ...
    static_assert(kMaxLength <= kSmiMaxValue, "String length must fit into a Smi");
    // - adding two string lengths must still fit into a 32-bit int without
    //   overflow
    static_assert(kMaxLength * 2 <= kMaxInt, "String::kMaxLength * 2 must fit into an int32");
    ...
    static_assert(kMaxLength * 2 + kHeaderSize <= kSmiMaxValue, "String object size in bytes must fit into a Smi");
    ...
    static_assert(kMaxLength * 2 + kHeaderSize <= kMaxInt, "String object size in bytes must fit into an int");
    ...
    template <typename sinkchar> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) static void WriteToFlat(String source, sinkchar* sink, int from, int to);
    template <typename sinkchar> static void WriteToFlat(String source, sinkchar* sink, int from, int to, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded&);

    static inline bool IsAscii(const char* chars, int length) { return IsAscii(reinterpret_cast<const uint8_t*>(chars), length); }

    static inline bool IsAscii(const uint8_t* chars, int length) { return NonAsciiStart(chars, length) >= length; }
    ...
    static inline bool IsOneByte(const base::uc16* chars, int length) { return NonOneByteStart(chars, length) >= length; }

    // May only be called when a SharedStringAccessGuard is not needed (i.e. on
    // the main thread or on read-only strings).
    template <class Visitor> static inline ConsString VisitFlat(Visitor* visitor, String string, int offset = 0);

    template <class Visitor> static inline ConsString VisitFlat(Visitor* visitor, String string, int offset, const SharedStringAccessGuardIfNeeded& access_guard);
...
private:
    ...
    V8_INLINE uint16_t GetImpl(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;

    // Implementation of the IsEqualTo() public methods. Do not use directly.
    template <EqualityType kEqType, typename Char>
    V8_INLINE bool IsEqualToImpl(base::Vector<const Char> str, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
    ...
    V8_EXPORT_PRIVATE V8_INLINE static base::Optional<FlatContent> TryGetFlatContentFromDirectString(
        PtrComprCageBase cage_base, const DisallowGarbageCollection& no_gc, String string, int offset, int length, const SharedStringAccessGuardIfNeeded&);
    ...
    V8_EXPORT_PRIVATE bool SlowEquals(String other, const SharedStringAccessGuardIfNeeded&) const;
...
// clang-format on
extern template EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) void String::WriteToFlat(String source, uint8_t* sink, int from, int to);
...
public:
    inline SubStringRange(String string, const DisallowGarbageCollection& no_gc, int first = 0, int length = -1);
    class iterator;
...
public:
    ...
    V8_WARN_UNUSED_RESULT static Handle<String> Truncate(Handle<SeqString> string, int new_length);
...
class InternalizedString : public TorqueGeneratedInternalizedString<InternalizedString, String> {
public:
...
class SeqOneByteString : public TorqueGeneratedSeqOneByteString<SeqOneByteString, SeqString> {
public:
    ...
    inline uint8_t Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
    inline void SeqOneByteStringSet(int index, uint16_t value);
    inline void SeqOneByteStringSetChars(int index, const uint8_t* string, int length);
    ...
    inline uint8_t* GetChars(const DisallowGarbageCollection& no_gc, const SharedStringAccessGuardIfNeeded& access_guard) const;
...
class SeqTwoByteString : public TorqueGeneratedSeqTwoByteString<SeqTwoByteString, SeqString> {
public:
    static const bool kHasOneByteEncoding = false;
    using Char = uint16_t;

    // Dispatched behavior.
    inline uint16_t Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
    ...
    // Get a pointer to the characters of the string.
    inline base::uc16* GetChars(const DisallowGarbageCollection& no_gc, const SharedStringAccessGuardIfNeeded& access_guard) const;
...
public:
    ...
    // Dispatched behavior.
    V8_EXPORT_PRIVATE uint16_t Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
...
public:
    inline void set_parent(String parent, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    // Dispatched behavior.
    V8_EXPORT_PRIVATE uint16_t Get(int index, PtrComprCageBase cage_base, const SharedStringAccessGuardIfNeeded& access_guard) const;
...
class ExternalString : public TorqueGeneratedExternalString<ExternalString, String> {
public:
...
// one-byte string.
class ExternalOneByteString : public TorqueGeneratedExternalOneByteString<ExternalOneByteString, ExternalString> {
public:
    static const bool kHasOneByteEncoding = true;
...
// encoded string.
class ExternalTwoByteString : public TorqueGeneratedExternalTwoByteString<ExternalTwoByteString, ExternalString> {
public:
    static const bool kHasOneByteEncoding = false;
...
public:
    ...
    template <typename Char> inline Char Get(int index) const;
    int length() const { return length_; }
...
public:
    ...
    inline explicit ConsStringIterator(ConsString cons_string, int offset = 0) { Reset(cons_string, offset); }
