...
Maybe<bool> InsertOptionsIntoLocale(Isolate* isolate, Handle<JSReceiver> options, icu::LocaleBuilder* builder)
{
    ...
    const std::array<OptionData, 6> kOptionToUnicodeTagMap
        = { { { "calendar", "ca", &empty_values, false }, { "collation", "co", &empty_values, false }, { "hourCycle", "hc", &hour_cycle_values, false },
            { "caseFirst", "kf", &case_first_values, false }, { "numeric", "kn", &empty_values, true }, { "numberingSystem", "nu", &empty_values, false } } };
    ...
    for (const auto& option_to_bcp47 : kOptionToUnicodeTagMap) {
        ...
        Maybe<bool> maybe_found = option_to_bcp47.is_bool_value ? GetBoolOption(isolate, options, option_to_bcp47.name, "locale", &value_bool)
        ...
        if (!uloc_toLegacyType(uloc_toLegacyKey(option_to_bcp47.key), value_str.get())) {
            return Just(false);
        }
...
Handle<Object> UnicodeKeywordValue(Isolate* isolate, Handle<JSLocale> locale, const char* key)
{
...
{
    return IsCheckRange(str, min, max, [](char c) -> bool { return base::IsInRange(c, 'a', 'z') || base::IsInRange(c, 'A', 'Z') || base::IsInRange(c, '0', '9'); });
}
...
bool IsDigitAlphanum3(const std::string& value) { return value.length() == 4 && base::IsInRange(value[0], '0', '9') && IsAlphanum(value.substr(1), 3, 3); }
...
bool IsExtensionSingleton(const std::string& value) { return IsAlphanum(value, 1, 1); }

int32_t weekdayFromEDaysOfWeek(icu::Calendar::EDaysOfWeek eDaysOfWeek) { return (eDaysOfWeek == icu::Calendar::SUNDAY) ? 7 : eDaysOfWeek - 1; }
...
bool JSLocale::Is3Alpha(const std::string& value) { return IsAlpha(value, 3, 3); }
...
MaybeHandle<JSLocale> JSLocale::New(Isolate* isolate, Handle<Map> map, Handle<String> locale_str, Handle<JSReceiver> options)
{
    ...
    if (!maybe_apply.FromJust()) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kLocaleBadParameters), JSLocale);
    }
    ...
    Handle<JSLocale> locale = Handle<JSLocale>::cast(isolate->factory()->NewFastOrSlowJSObjectFromMap(map));
...
MaybeHandle<JSLocale> Construct(Isolate* isolate, const icu::Locale& icu_locale)
{
    ...
    Handle<Map> map;
    ASSIGN_RETURN_ON_EXCEPTION(isolate, map, JSFunction::GetDerivedMap(isolate, constructor, constructor), JSLocale);
...
MaybeHandle<JSLocale> JSLocale::Maximize(Isolate* isolate, Handle<JSLocale> locale)
{
...
MaybeHandle<JSLocale> JSLocale::Minimize(Isolate* isolate, Handle<JSLocale> locale)
{
...
MaybeHandle<JSArray> JSLocale::Calendars(Isolate* isolate, Handle<JSLocale> locale)
{
    icu::Locale icu_locale(*(locale->icu_locale().raw()));
...
MaybeHandle<JSArray> JSLocale::Collations(Isolate* isolate, Handle<JSLocale> locale)
{
    icu::Locale icu_locale(*(locale->icu_locale().raw()));
    return GetKeywordValuesFromLocale<icu::Collator>(isolate, "collations", "co", icu_locale, Intl::RemoveCollation, true, false);
}

MaybeHandle<JSArray> JSLocale::HourCycles(Isolate* isolate, Handle<JSLocale> locale)
{
...
MaybeHandle<JSArray> JSLocale::NumberingSystems(Isolate* isolate, Handle<JSLocale> locale)
{
    ...
    Handle<String> str = factory->NewStringFromAsciiChecked(numbering_system.c_str());

    fixed_array->set(0, *str);
    return factory->NewJSArrayWithElements(fixed_array);
}

MaybeHandle<Object> JSLocale::TimeZones(Isolate* isolate, Handle<JSLocale> locale)
{
...
MaybeHandle<JSObject> JSLocale::TextInfo(Isolate* isolate, Handle<JSLocale> locale)
{
    // Let loc be the this value.
    ...
    UErrorCode status = U_ZERO_ERROR;
    ULayoutType orientation = uloc_getCharacterOrientation((locale->icu_locale().raw())->getName(), &status);
    if (U_FAILURE(status)) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kIcuError), JSObject);
    }
...
MaybeHandle<JSObject> JSLocale::WeekInfo(Isolate* isolate, Handle<JSLocale> locale)
{
    // Let loc be the this value.
...
Handle<Object> JSLocale::Calendar(Isolate* isolate, Handle<JSLocale> locale) { return UnicodeKeywordValue(isolate, locale, "ca"); }

Handle<Object> JSLocale::CaseFirst(Isolate* isolate, Handle<JSLocale> locale) { return UnicodeKeywordValue(isolate, locale, "kf"); }

Handle<Object> JSLocale::Collation(Isolate* isolate, Handle<JSLocale> locale) { return UnicodeKeywordValue(isolate, locale, "co"); }

Handle<Object> JSLocale::HourCycle(Isolate* isolate, Handle<JSLocale> locale) { return UnicodeKeywordValue(isolate, locale, "hc"); }
...
Handle<Object> JSLocale::NumberingSystem(Isolate* isolate, Handle<JSLocale> locale) { return UnicodeKeywordValue(isolate, locale, "nu"); }
