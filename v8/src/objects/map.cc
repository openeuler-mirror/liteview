...
// static
base::Optional<JSFunction> Map::GetConstructorFunction(Map map, Context native_context)
{
...
MaybeHandle<Map> Map::CopyWithField(Isolate* isolate, Handle<Map> map, Handle<Name> name, Handle<FieldType> type, PropertyAttributes attributes, PropertyConstness constness,
...
MaybeHandle<Map> Map::CopyWithConstant(Isolate* isolate, Handle<Map> map, Handle<Name> name, Handle<Object> constant, PropertyAttributes attributes, TransitionFlag flag)
{
    // Ensure the descriptor array does not get too big.
    if (map->NumberOfOwnDescriptors() >= kMaxNumberOfDescriptors) {
        return MaybeHandle<Map>();
...
bool Map::InstancesNeedRewriting(Map target, int target_number_of_fields, int target_inobject, int target_unused, int* old_number_of_fields, ConcurrencyMode cmode) const
{
    ...
    *old_number_of_fields = NumberOfFields(cmode);
    ...
    DescriptorArray old_desc = IsConcurrent(cmode) ? instance_descriptors(kAcquireLoad) : instance_descriptors();
    DescriptorArray new_desc = IsConcurrent(cmode) ? target.instance_descriptors(kAcquireLoad) : target.instance_descriptors();
...
{
    DescriptorArray descriptors = IsConcurrent(cmode) ? instance_descriptors(kAcquireLoad) : instance_descriptors();
    int result = 0;
    for (InternalIndex i : IterateOwnDescriptors()) {
...
void Map::ReplaceDescriptors(Isolate* isolate, DescriptorArray new_descriptors)
{
    ...
    if (NumberOfOwnDescriptors() == 0 || GetBackPointer(isolate).IsUndefined(isolate)) {
        return;
    }
...
{
    DisallowGarbageCollection no_gc;
    DCHECK_EQ(PropertyLocation::kField, instance_descriptors(isolate, kRelaxedLoad).GetDetails(descriptor).location());
    Map result = *this;
    while (true) {
...
Map SearchMigrationTarget(Isolate* isolate, Map old_map)
{
    ...
    for (InternalIndex i : old_map.IterateOwnDescriptors()) {
        ...
        if (old_details.location() == PropertyLocation::kField && old_details.kind() == PropertyKind::kData) {
...
MaybeHandle<Map> Map::TryUpdate(Isolate* isolate, Handle<Map> old_map)
{
    ...
    base::Optional<Map> new_map = MapUpdater::TryUpdateNoLock(isolate, *old_map, ConcurrencyMode::kSynchronous);
...
Map Map::TryReplayPropertyTransitions(Isolate* isolate, Map old_map, ConcurrencyMode cmode)
{
    DisallowGarbageCollection no_gc;
    ...
    DescriptorArray old_descriptors = old_map.instance_descriptors(isolate, kAcquireLoad);

    Map new_map = *this;
    for (InternalIndex i : InternalIndex::Range(root_nof, old_nof)) {
        PropertyDetails old_details = old_descriptors.GetDetails(i);
        Map transition = TransitionsAccessor(isolate, new_map, IsConcurrent(cmode)).SearchTransition(old_descriptors.GetKey(i), old_details.kind(), old_details.attributes());
        if (transition.is_null())
            return Map();
        new_map = transition;
        DescriptorArray new_descriptors = new_map.instance_descriptors(isolate, kAcquireLoad);
...
Map Map::FindElementsKindTransitionedMap(Isolate* isolate, MapHandles const& candidates, ConcurrencyMode cmode)
{
    DisallowGarbageCollection no_gc;
    ...
    if (IsTransitionableFastElementsKind(kind)) {
        ...
        for (root_map = root_map.ElementsTransitionMap(isolate, cmode); !root_map.is_null() && root_map.has_fast_elements();
            ...
            if (!HasElementsKind(candidates, root_map.elements_kind()))
                continue;
            Map current = root_map.TryReplayPropertyTransitions(isolate, *this, cmode);
            if (current.is_null())
                continue;
...
static Map FindClosestElementsTransition(Isolate* isolate, Map map, ElementsKind to_kind, ConcurrencyMode cmode)
{
    DisallowGarbageCollection no_gc;
...
Map Map::LookupElementsTransitionMap(Isolate* isolate, ElementsKind to_kind, ConcurrencyMode cmode)
{
...
Handle<Map> Map::TransitionElementsTo(Isolate* isolate, Handle<Map> map, ElementsKind to_kind)
{
    ...
    if (IsHoleyElementsKind(from_kind) && to_kind == GetPackedElementsKind(from_kind) && map->GetBackPointer().IsMap()
...
// static
base::Optional<Map> Map::TryAsElementsKind(Isolate* isolate, Handle<Map> map, ElementsKind kind, ConcurrencyMode cmode)
{
...
// static
Handle<Map> Map::AsElementsKind(Isolate* isolate, Handle<Map> map, ElementsKind kind)
{
...
int Map::NumberOfEnumerableProperties() const
{
    ...
    for (InternalIndex i : IterateOwnDescriptors()) {
        if ((int { descs.GetDetails(i).attributes() } & ONLY_ENUMERABLE) == 0 && !descs.GetKey(i).FilterKey(ENUMERABLE_STRINGS)) {
            result++;
        }
    }
    return result;
...
bool Map::OnlyHasSimpleProperties() const
{
    ...
    return !IsStringWrapperElementsKind(elements_kind()) && !IsSpecialReceiverMap() && !is_dictionary_map();
}
...
bool Map::MayHaveReadOnlyElementsInPrototypeChain(Isolate* isolate)
{
    for (PrototypeIterator iter(isolate, *this); !iter.IsAtEnd(); iter.Advance()) {
        ...
        if (iter.GetCurrent().map().IsCustomElementsReceiverMap())
            return true;
        ...
        if (IsSlowArgumentsElementsKind(elements_kind)) {
            SloppyArgumentsElements elements = SloppyArgumentsElements::cast(current.elements(isolate));
...
{
    Handle<Map> result = isolate->factory()->NewMap(src_handle->instance_type(), instance_size, TERMINAL_FAST_ELEMENTS_KIND, inobject_properties);
...
Handle<Map> Map::Normalize(Isolate* isolate, Handle<Map> fast_map, ElementsKind new_elements_kind, PropertyNormalizationMode mode, bool use_cache, const char* reason)
{
    DCHECK(!fast_map->is_dictionary_map());

    Handle<Object> maybe_cache(isolate->native_context()->normalized_map_cache(), isolate);
    ...
    Handle<Map> new_map;
    if (use_cache && cache->Get(fast_map, new_elements_kind, mode).ToHandle(&new_map)) {
#ifdef VERIFY_HEAP
        ...
        if (v8_flags.enable_slow_asserts) {
            ...
            DCHECK_EQ(0, memcmp(reinterpret_cast<void*>(fresh->address() + offset), reinterpret_cast<void*>(new_map->address() + offset), Map::kDependentCodeOffset - offset));
            ...
            if (new_map->is_prototype_map()) {
                ...
                DCHECK_EQ(fresh->raw_transitions(), MaybeObject::FromObject(Smi::zero()));
            }
...
void EnsureInitialMap(Isolate* isolate, Handle<Map> map)
{
    ...
    DCHECK((maybe_constructor.IsJSFunction() && *map == JSFunction::cast(maybe_constructor).initial_map()) ||
        ...
        *map == *isolate->generator_function_map() || *map == *isolate->generator_function_with_name_map() ||
        // AsyncFunction has Null as a constructor.
        *map == *isolate->async_function_map() || *map == *isolate->async_function_with_name_map());
#endif
...
{
    Handle<Map> result = RawCopy(isolate, map, map->instance_size(), map->IsJSObjectMap() ? map->GetInObjectProperties() : 0);
...
Handle<Map> Map::ShareDescriptor(Isolate* isolate, Handle<Map> map, Handle<DescriptorArray> descriptors, Descriptor* descriptor)
{
...
void Map::ConnectTransition(Isolate* isolate, Handle<Map> parent, Handle<Map> child, Handle<Name> name, SimpleTransitionFlag flag)
{
    DCHECK_IMPLIES(name->IsInterestingSymbol(), child->may_have_interesting_symbols());
    DCHECK_IMPLIES(parent->may_have_interesting_symbols(), child->may_have_interesting_symbols());
...
Handle<Map> Map::CopyReplaceDescriptors(
    Isolate* isolate, Handle<Map> map, Handle<DescriptorArray> descriptors, TransitionFlag flag, MaybeHandle<Name> maybe_name, const char* reason, SimpleTransitionFlag simple_flag)
{
...
// marking logic.
Handle<Map> Map::AddMissingTransitions(Isolate* isolate, Handle<Map> split_map, Handle<DescriptorArray> descriptors)
{
    DCHECK(descriptors->IsSortedNoDuplicates());
...
void Map::InstallDescriptors(Isolate* isolate, Handle<Map> parent, Handle<Map> child, InternalIndex new_descriptor, Handle<DescriptorArray> descriptors)
{
    DCHECK(descriptors->IsSortedNoDuplicates());
...
Handle<Map> Map::AsLanguageMode(Isolate* isolate, Handle<Map> initial_map, Handle<SharedFunctionInfo> shared_info)
{
    ...
    Handle<Map> function_map(Map::cast(isolate->native_context()->get(shared_info->function_map_index())), isolate);
    ...
    Handle<Map> map = Map::CopyInitialMap(isolate, function_map, initial_map->instance_size(), initial_map->GetInObjectProperties(), initial_map->UnusedPropertyFields());
...
Handle<Map> Map::CopyForElementsTransition(Isolate* isolate, Handle<Map> map)
{
    ...
    if (map->owns_descriptors()) {
        ...
        Handle<DescriptorArray> descriptors(map->instance_descriptors(isolate), isolate);
        int number_of_own_descriptors = map->NumberOfOwnDescriptors();
        Handle<DescriptorArray> new_descriptors = DescriptorArray::CopyUpTo(isolate, descriptors, number_of_own_descriptors);
...
Handle<Map> Map::Copy(Isolate* isolate, Handle<Map> map, const char* reason)
{
    ...
    int number_of_own_descriptors = map->NumberOfOwnDescriptors();
    Handle<DescriptorArray> new_descriptors = DescriptorArray::CopyUpTo(isolate, descriptors, number_of_own_descriptors);
    return CopyReplaceDescriptors(isolate, map, new_descriptors, OMIT_TRANSITION, MaybeHandle<Name>(), reason, SPECIAL_TRANSITION);
}
...
Handle<Map> Map::CopyForPreventExtensions(
    Isolate* isolate, Handle<Map> map, PropertyAttributes attrs_to_add, Handle<Symbol> transition_marker, const char* reason, bool old_map_is_dictionary_elements_kind)
{
    int num_descriptors = map->NumberOfOwnDescriptors();
    Handle<DescriptorArray> new_desc = DescriptorArray::CopyUpToAddAttributes(isolate, handle(map->instance_descriptors(isolate), isolate), num_descriptors, attrs_to_add);
...
bool CanHoldValue(DescriptorArray descriptors, InternalIndex descriptor, PropertyConstness constness, Object value)
{
    PropertyDetails details = descriptors.GetDetails(descriptor);
    if (details.location() == PropertyLocation::kField) {
        if (details.kind() == PropertyKind::kData) {
            return IsGeneralizableTo(constness, details.constness()) && value.FitsRepresentation(details.representation())
...
Handle<Map> UpdateDescriptorForValue(Isolate* isolate, Handle<Map> map, InternalIndex descriptor, PropertyConstness constness, Handle<Object> value)
{
...
// static
Handle<Map> Map::PrepareForDataProperty(Isolate* isolate, Handle<Map> map, InternalIndex descriptor, PropertyConstness constness, Handle<Object> value)
{
...
Handle<Map> Map::TransitionToDataProperty(
    Isolate* isolate, Handle<Map> map, Handle<Name> name, Handle<Object> value, PropertyAttributes attributes, PropertyConstness constness, StoreOrigin store_origin)
{
    RCS_SCOPE(isolate, map->IsDetached(isolate) ? RuntimeCallCounterId::kPrototypeMap_TransitionToDataProperty : RuntimeCallCounterId::kMap_TransitionToDataProperty);
    ...
    if (!maybe_map.ToHandle(&result)) {
        ...
        if (v8_flags.feedback_normalization && map->new_target_is_base() && maybe_constructor->IsJSFunction() && !JSFunction::cast(*maybe_constructor).shared().native()) {
...
Handle<Map> Map::TransitionToAccessorProperty(
    Isolate* isolate, Handle<Map> map, Handle<Name> name, InternalIndex descriptor, Handle<Object> getter, Handle<Object> setter, PropertyAttributes attributes)
{
    RCS_SCOPE(isolate, map->IsDetached(isolate) ? RuntimeCallCounterId::kPrototypeMap_TransitionToAccessorProperty : RuntimeCallCounterId::kMap_TransitionToAccessorProperty);
...
Handle<Map> Map::CopyAddDescriptor(Isolate* isolate, Handle<Map> map, Descriptor* descriptor, TransitionFlag flag)
{
    ...
    if (flag == INSERT_TRANSITION && map->owns_descriptors() && !map->GetBackPointer().IsUndefined(isolate) && TransitionsAccessor::CanHaveMoreTransitions(isolate, map)) {
...
{
    Handle<DescriptorArray> old_descriptors(map->instance_descriptors(isolate), isolate);
...
Handle<Map> Map::CopyReplaceDescriptor(
    Isolate* isolate, Handle<Map> map, Handle<DescriptorArray> descriptors, Descriptor* descriptor, InternalIndex insertion_index, TransitionFlag flag)
{
    ...
    Handle<DescriptorArray> new_descriptors = DescriptorArray::CopyUpTo(isolate, descriptors, map->NumberOfOwnDescriptors());

    new_descriptors->Replace(insertion_index, descriptor);

    SimpleTransitionFlag simple_flag = (insertion_index.as_int() == descriptors->number_of_descriptors() - 1) ? SIMPLE_PROPERTY_TRANSITION : PROPERTY_TRANSITION;
...
{
    return first.GetConstructor() == second.GetConstructor() && first.prototype() == second.prototype() && first.instance_type() == second.instance_type()
        && first.bit_field() == second.bit_field() && first.is_extensible() == second.is_extensible() && first.new_target_is_base() == second.new_target_is_base();
}

} // namespace

bool Map::EquivalentToForTransition(const Map other, ConcurrencyMode cmode) const
{
    ...
    if (InstanceTypeChecker::IsJSFunction(instance_type())) {
        ...
        DescriptorArray this_descriptors = IsConcurrent(cmode) ? instance_descriptors(kAcquireLoad) : instance_descriptors();
        DescriptorArray that_descriptors = IsConcurrent(cmode) ? other.instance_descriptors(kAcquireLoad) : other.instance_descriptors();
        return this_descriptors.IsEqualUpTo(that_descriptors, nof);
    }
    return true;
}

bool Map::EquivalentToForElementsKindTransition(const Map other, ConcurrencyMode cmode) const
{
...
bool Map::EquivalentToForNormalization(const Map other, ElementsKind elements_kind, PropertyNormalizationMode mode) const
{
    ...
    return CheckEquivalent(*this, other) && bit_field2() == adjusted_other_bit_field2 && GetInObjectProperties() == properties
...
int Map::ComputeMinObjectSlack(Isolate* isolate)
{
    ...
    TransitionsAccessor transitions(isolate, *this);
    TransitionsAccessor::TraverseCallback callback = [&](Map map) { slack = std::min(slack, map.UnusedPropertyFields()); };
    transitions.TraverseTransitionTree(callback);
    return slack;
}

void Map::SetInstanceDescriptors(Isolate* isolate, DescriptorArray descriptors, int number_of_own_descriptors)
{
...
// static
Handle<PrototypeInfo> Map::GetOrCreatePrototypeInfo(Handle<JSObject> prototype, Isolate* isolate)
{
...
// static
Handle<PrototypeInfo> Map::GetOrCreatePrototypeInfo(Handle<Map> prototype_map, Isolate* isolate)
{
...
// static
void Map::SetShouldBeFastPrototypeMap(Handle<Map> map, bool value, Isolate* isolate)
{
...
// static
Handle<Object> Map::GetOrCreatePrototypeChainValidityCell(Handle<Map> map, Isolate* isolate)
{
    ...
    Handle<Cell> cell = isolate->factory()->NewCell(handle(Smi::FromInt(Map::kPrototypeChainValid), isolate));
...
// static
void Map::SetPrototype(Isolate* isolate, Handle<Map> map, Handle<HeapObject> prototype, bool enable_prototype_setup_mode)
{
    ...
    if (prototype->IsJSObject()) {
        ...
        DCHECK(prototype->IsNull(isolate) || prototype->IsJSProxy() || prototype->IsWasmObject());
    }
...
Handle<Map> Map::TransitionToPrototype(Isolate* isolate, Handle<Map> map, Handle<HeapObject> prototype)
{
    Handle<Map> new_map = TransitionsAccessor::GetPrototypeTransition(isolate, map, prototype);
    if (new_map.is_null()) {
...
MaybeHandle<Map> NormalizedMapCache::Get(Handle<Map> fast_map, ElementsKind elements_kind, PropertyNormalizationMode mode)
{
    DisallowGarbageCollection no_gc;
    ...
    if (!normalized_map.EquivalentToForNormalization(*fast_map, elements_kind, mode)) {
        return MaybeHandle<Map>();
    }
...
void NormalizedMapCache::Set(Handle<Map> fast_map, Handle<Map> normalized_map)
{
    ...
    WeakFixedArray::Set(GetIndex(fast_map), HeapObjectReference::Weak(*normalized_map));
}
