...
public:
    ...
    inline bool HasElement(JSObject holder, uint32_t index, PropertyFilter filter = ALL_PROPERTIES);
    ...
    V8_WARN_UNUSED_RESULT virtual ExceptionStatus CollectElementIndices(Handle<JSObject> object, Handle<FixedArrayBase> backing_store, KeyAccumulator* keys) = 0;
    ...
    virtual Maybe<bool> CollectValuesOrEntries(
        Isolate* isolate, Handle<JSObject> object, Handle<FixedArray> values_or_entries, bool get_entries, int* nof_items, PropertyFilter filter = ALL_PROPERTIES)
        = 0;

    virtual MaybeHandle<FixedArray> PrependElementIndices(
        Isolate* isolate, Handle<JSObject> object, Handle<FixedArrayBase> backing_store, Handle<FixedArray> keys, GetKeysConversion convert, PropertyFilter filter = ALL_PROPERTIES)
        = 0;
    ...
    V8_WARN_UNUSED_RESULT virtual ExceptionStatus AddElementsToKeyAccumulator(Handle<JSObject> receiver, KeyAccumulator* accumulator, AddKeyConversion convert) = 0;
    ...
    virtual void Set(Handle<JSObject> holder, InternalIndex entry, Object value) = 0;

    // Currently only shared array elements support sequentially consistent
    // access.
    virtual void SetAtomic(Handle<JSObject> holder, InternalIndex entry, Object value, SeqCstAccessTag tag) = 0;

    // Currently only shared array elements support sequentially consistent
    // access.
    virtual Handle<Object> SwapAtomic(Isolate* isolate, Handle<JSObject> holder, InternalIndex entry, Object value, SeqCstAccessTag tag) = 0;

    V8_WARN_UNUSED_RESULT virtual Maybe<bool> Add(Handle<JSObject> object, uint32_t index, Handle<Object> value, PropertyAttributes attributes, uint32_t new_capacity) = 0;
    ...
    virtual Object CopyElements(Handle<Object> source, Handle<JSObject> destination, size_t length, size_t offset) = 0;
...
protected:
    ...
    virtual InternalIndex GetEntryForIndex(Isolate* isolate, JSObject holder, FixedArrayBase backing_store, size_t index) = 0;

    virtual PropertyDetails GetDetails(JSObject holder, InternalIndex entry) = 0;
    virtual void Reconfigure(Handle<JSObject> object, Handle<FixedArrayBase> backing_store, InternalIndex entry, Handle<Object> value, PropertyAttributes attributes) = 0;
    ...
    virtual void CopyElements(
        JSObject source_holder, uint32_t source_start, ElementsKind source_kind, Handle<FixedArrayBase> destination, uint32_t destination_start, int copy_size)
        = 0;
