...
// static
HeapObjectReference HeapObjectReference::From(Object object, HeapObjectReferenceType type)
{
    DCHECK(!object.IsSmi());
    DCHECK(!HasWeakHeapObjectTag(object));
    switch (type) {
...
// static
HeapObjectReference HeapObjectReference::ClearedValue(PtrComprCageBase cage_base)
{
    ...
    Address raw_value = V8HeapCompressionScheme::DecompressTaggedPointer(cage_base, kClearedWeakHeapObjectLower32);
#else
...
template <typename THeapObjectSlot> void HeapObjectReference::Update(THeapObjectSlot slot, HeapObject value)
{
    ...
    slot.store(HeapObjectReference(new_value | (old_value & kWeakHeapObjectMask)));
