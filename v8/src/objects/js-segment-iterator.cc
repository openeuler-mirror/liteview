...
Handle<String> JSSegmentIterator::GranularityAsString(Isolate* isolate) const { return JSSegmenter::GetGranularityString(isolate, granularity()); }

// ecma402 #sec-createsegmentiterator
MaybeHandle<JSSegmentIterator> JSSegmentIterator::Create(Isolate* isolate, icu::BreakIterator* break_iterator, JSSegmenter::Granularity granularity)
{
    ...
    Handle<Map> map = Handle<Map>(isolate->native_context()->intl_segment_iterator_map(), isolate);
    ...
    Handle<Managed<icu::UnicodeString>> unicode_string = Managed<icu::UnicodeString>::FromRawPtr(isolate, 0, string);
...
MaybeHandle<JSReceiver> JSSegmentIterator::Next(Isolate* isolate, Handle<JSSegmentIterator> segment_iterator)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(
        isolate, segment_data, JSSegments::CreateSegmentDataObject(isolate, segment_iterator->granularity(), icu_break_iterator, string, start_index, end_index), JSReceiver);
