...
public:
    ...
    static inline int ToInt(const Object object) { return Smi::cast(object).value(); }
    ...
    static inline Smi From31BitPattern(int value) { return Smi::FromInt((value << (32 - kSmiValueSize)) >> (32 - kSmiValueSize)); }

    template <typename E, typename = typename std::enable_if<std::is_enum<E>::value>::type> static inline Smi FromEnum(E value)
    {
    ...
    // usage.
    V8_EXPORT_PRIVATE static Address LexicographicCompare(Isolate* isolate, Smi x, Smi y);
    ...
    static constexpr Smi uninitialized_deserialization_value() { return Smi(kNullAddress); }
};
