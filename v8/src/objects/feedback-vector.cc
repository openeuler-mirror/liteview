...
std::ostream& operator<<(std::ostream& os, FeedbackSlotKind kind) { return os << FeedbackMetadata::Kind2String(kind); }
...
template <typename IsolateT> Handle<FeedbackMetadata> FeedbackMetadata::New(IsolateT* isolate, const FeedbackVectorSpec* spec)
{
    ...
    const int create_closure_slot_count = spec == nullptr ? 0 : spec->create_closure_slot_count();
...
// static
Handle<ClosureFeedbackCellArray> ClosureFeedbackCellArray::New(Isolate* isolate, Handle<SharedFunctionInfo> shared)
{
    Factory* factory = isolate->factory();

    int num_feedback_cells = shared->feedback_metadata().create_closure_slot_count();
    ...
    Isolate* isolate, Handle<SharedFunctionInfo> shared, Handle<ClosureFeedbackCellArray> closure_feedback_cell_array, IsCompiledScope* is_compiled_scope)
{
    ...
    Handle<FeedbackMetadata> feedback_metadata(shared->feedback_metadata(), isolate);
    const int slot_count = feedback_metadata->slot_count();

    Handle<FeedbackVector> vector = factory->NewFeedbackVector(shared, closure_feedback_cell_array);
    ...
    for (int i = 0; i < slot_count;) {
        ...
        case FeedbackSlotKind::kStoreGlobalStrict:
        case FeedbackSlotKind::kJumpLoop:
            vector->Set(slot, HeapObjectReference::ClearedValue(isolate), SKIP_WRITE_BARRIER);
            break;
        case FeedbackSlotKind::kForIn:
        case FeedbackSlotKind::kCompareOp:
        case FeedbackSlotKind::kBinaryOp:
...
Handle<FeedbackVector> NewFeedbackVectorForTesting(Isolate* isolate, const FeedbackVectorSpec* spec)
{
    Handle<FeedbackMetadata> metadata = FeedbackMetadata::New(isolate, spec);
    Handle<SharedFunctionInfo> shared = isolate->factory()->NewSharedFunctionInfoForBuiltin(isolate->factory()->empty_string(), Builtin::kIllegal);
    // Set the raw feedback metadata to circumvent checks that we are not
    // overwriting existing metadata.
    shared->set_raw_outer_scope_info_or_feedback_metadata(*metadata);
    Handle<ClosureFeedbackCellArray> closure_feedback_cell_array = ClosureFeedbackCellArray::New(isolate, shared);

    IsCompiledScope is_compiled_scope(shared->is_compiled_scope(isolate));
    return FeedbackVector::New(isolate, shared, closure_feedback_cell_array, &is_compiled_scope);
}
...
// static
void FeedbackVector::AddToVectorsForProfilingTools(Isolate* isolate, Handle<FeedbackVector> vector)
{
    DCHECK(!isolate->is_best_effort_code_coverage());
    if (!vector->shared_function_info().IsSubjectToDebugging())
        return;
    Handle<ArrayList> list = Handle<ArrayList>::cast(isolate->factory()->feedback_vectors_for_profiling_tools());
...
void FeedbackVector::SetOptimizedCode(CodeT code)
{
    ...
    DCHECK(!has_optimized_code() || optimized_code().marked_for_deoptimization() || (CodeKindCanTierUp(optimized_code().kind()) && optimized_code().kind() < code.kind())
...
void FeedbackVector::ClearOptimizedCode()
{
    ...
    set_maybe_optimized_code(HeapObjectReference::ClearedValue(GetIsolate()), kReleaseStore);
...
void FeedbackVector::reset_tiering_state() { set_tiering_state(TieringState::kNone); }
...
{
    set_flags(TieringStateBits::encode(TieringState::kNone) | LogNextExecutionBit::encode(false) | MaybeHasMaglevCodeBit::encode(false) | MaybeHasTurbofanCodeBit::encode(false)
        | OsrTieringStateBit::encode(TieringState::kNone) | MaybeHasOptimizedOsrCodeBit::encode(false));
}

TieringState FeedbackVector::osr_tiering_state() { return OsrTieringStateBit::decode(flags()); }
...
void FeedbackVector::EvictOptimizedCodeMarkedForDeoptimization(SharedFunctionInfo shared, const char* reason)
{
...
bool FeedbackVector::ClearSlots(Isolate* isolate, ClearBehavior behavior)
{
    if (!shared_function_info().HasFeedbackMetadata())
        return false;
    MaybeObject uninitialized_sentinel = MaybeObject::FromObject(FeedbackVector::RawUninitializedSentinel(isolate));
...
template <typename T> Handle<T> NexusConfig::NewHandle(T object) const
{
    if (mode() == Mode::MainThread) {
        return handle(object, isolate_);
    }
...
void NexusConfig::SetFeedbackPair(
...
std::pair<MaybeObject, MaybeObject> NexusConfig::GetFeedbackPair(FeedbackVector vector, FeedbackSlot slot) const
{
...
FeedbackNexus::FeedbackNexus(Handle<FeedbackVector> vector, FeedbackSlot slot)
    ...
    , slot_(slot)
    , config_(NexusConfig::FromMainThread(vector.is_null() ? nullptr : vector->GetIsolate()))
{
...
Handle<WeakFixedArray> FeedbackNexus::CreateArrayOfSize(int length)
{
    ...
    Handle<WeakFixedArray> array = GetIsolate()->factory()->NewWeakFixedArray(length);
    return array;
}
...
void FeedbackNexus::ConfigureUninitialized()
{
    ...
    case FeedbackSlotKind::kLoadGlobalNotInsideTypeof:
    case FeedbackSlotKind::kLoadGlobalInsideTypeof:
        SetFeedback(HeapObjectReference::ClearedValue(isolate), SKIP_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
        break;
    case FeedbackSlotKind::kCloneObject:
    case FeedbackSlotKind::kCall:
    ...
    case FeedbackSlotKind::kDefineKeyedOwnPropertyInLiteral:
        ...
        break;
    case FeedbackSlotKind::kJumpLoop:
        SetFeedback(HeapObjectReference::ClearedValue(isolate), SKIP_WRITE_BARRIER);
        break;
    default:
        UNREACHABLE();
    }
...
bool FeedbackNexus::ConfigureMegamorphic()
{
    ...
    if (GetFeedback() != sentinel) {
        SetFeedback(sentinel, SKIP_WRITE_BARRIER, HeapObjectReference::ClearedValue(isolate));
        return true;
    }
...
bool FeedbackNexus::ConfigureMegamorphic(IcCheckType property_type)
{
    DisallowGarbageCollection no_gc;
    MaybeObject sentinel = MegamorphicSentinel();
    MaybeObject maybe_extra = MaybeObject::FromSmi(Smi::FromInt(static_cast<int>(property_type)));
...
InlineCacheState FeedbackNexus::ic_state() const
{
    ...
    case FeedbackSlotKind::kCall: {
        ...
        if (feedback == MegamorphicSentinel()) {
            ...
            return InlineCacheState::MONOMORPHIC;
        } else if (feedback->GetHeapObjectIfStrong(&heap_object) && heap_object.IsAllocationSite()) {
            return InlineCacheState::MONOMORPHIC;
        }
...
void FeedbackNexus::ConfigurePropertyCellMode(Handle<PropertyCell> cell)
{
    ...
    SetFeedback(HeapObjectReference::Weak(*cell), UPDATE_WRITE_BARRIER, UninitializedSentinel(), SKIP_WRITE_BARRIER);
}
...
void FeedbackNexus::ConfigureHandlerMode(const MaybeObjectHandle& handler)
{
    ...
    SetFeedback(HeapObjectReference::ClearedValue(GetIsolate()), UPDATE_WRITE_BARRIER, *handler, UPDATE_WRITE_BARRIER);
}

void FeedbackNexus::ConfigureCloneObject(Handle<Map> source_map, Handle<Map> result_map)
{
    DCHECK(config()->can_write());
    Isolate* isolate = GetIsolate();
    ...
    case InlineCacheState::UNINITIALIZED:
        ...
        SetFeedback(HeapObjectReference::Weak(*source_map), UPDATE_WRITE_BARRIER, *result_map);
        break;
    case InlineCacheState::MONOMORPHIC:
        if (feedback.is_null() || feedback.is_identical_to(source_map) || Map::cast(*feedback).is_deprecated()) {
            SetFeedback(HeapObjectReference::Weak(*source_map), UPDATE_WRITE_BARRIER, *result_map);
        } else {
            // Transition to POLYMORPHIC.
            Handle<WeakFixedArray> array = CreateArrayOfSize(2 * kCloneObjectPolymorphicEntrySize);
            DisallowGarbageCollection no_gc;
            ...
            SetFeedback(raw_array, UPDATE_WRITE_BARRIER, HeapObjectReference::ClearedValue(isolate));
        }
        break;
    case InlineCacheState::POLYMORPHIC: {
        ...
        for (; i < array->length(); i += kCloneObjectPolymorphicEntrySize) {
            ...
            if (feedback_map->IsCleared())
                break;
            Handle<Map> cached_map(Map::cast(feedback_map->GetHeapObject()), isolate);
        ...
        if (i >= array->length()) {
            ...
            Handle<WeakFixedArray> new_array = CreateArrayOfSize(array->length() + kCloneObjectPolymorphicEntrySize);
...
void FeedbackNexus::ConfigureMonomorphic(Handle<Name> name, Handle<Map> receiver_map, const MaybeObjectHandle& handler)
{
    DCHECK(handler.is_null() || IC::IsHandler(*handler));
    if (kind() == FeedbackSlotKind::kDefineKeyedOwnPropertyInLiteral) {
        SetFeedback(HeapObjectReference::Weak(*receiver_map), UPDATE_WRITE_BARRIER, *name);
    } else {
        if (name.is_null()) {
            SetFeedback(HeapObjectReference::Weak(*receiver_map), UPDATE_WRITE_BARRIER, *handler);
        } else {
...
int FeedbackNexus::ExtractMapsAndFeedback(std::vector<MapAndFeedback>* maps_and_feedback) const
{
    DisallowGarbageCollection no_gc;
    int found = 0;
    ...
    for (FeedbackIterator it(this); !it.done(); it.Advance()) {
        ...
        if (!maybe_handler->IsCleared()) {
            DCHECK(IC::IsHandler(maybe_handler) || IsDefineKeyedOwnPropertyInLiteralKind(kind()));
...
KeyedAccessLoadMode FeedbackNexus::GetKeyedAccessLoadMode() const
{
    ...
    for (MapAndHandler map_and_handler : maps_and_handlers) {
        KeyedAccessLoadMode mode = LoadHandler::GetKeyedAccessLoadMode(*map_and_handler.second);
...
{
    DCHECK(IsKeyedStoreICKind(kind()) || IsStoreInArrayLiteralICKind(kind()) || IsDefineKeyedOwnPropertyInLiteralKind(kind()) || IsDefineKeyedOwnICKind(kind()));
    ...
    for (const MapAndHandler& map_and_handler : maps_and_handlers) {
        ...
        if (maybe_code_handler.object()->IsStoreHandler()) {
            ...
            if ((data_handler->smi_handler()).IsSmi()) {
                ...
                mode = StoreHandler::GetKeyedAccessStoreMode(MaybeObject::FromObject(data_handler->smi_handler()));
                if (mode != STANDARD_STORE)
                    return mode;
                continue;
            } else {
...
IcCheckType FeedbackNexus::GetKeyType() const
{
    ...
    if (feedback == MegamorphicSentinel()) {
        return static_cast<IcCheckType>(Smi::ToInt(pair.second->template cast<Object>()));
    }
    MaybeObject maybe_name = IsDefineKeyedOwnPropertyInLiteralKind(kind()) || IsDefineKeyedOwnICKind(kind()) ? pair.second : feedback;
    ...
    , state_(kOther)
{
    ...
    if ((feedback->GetHeapObjectIfStrong(&heap_object) && heap_object.IsWeakFixedArray()) || is_named_feedback) {
        ...
        if (is_named_feedback) {
            polymorphic_feedback_ = nexus->config()->NewHandle(WeakFixedArray::cast(pair.second->GetHeapObjectAssumeStrong()));
        } else {
            polymorphic_feedback_ = nexus->config()->NewHandle(WeakFixedArray::cast(heap_object));
        }
        AdvancePolymorphic();
