...
static inline bool IsShortcutCandidate(int type) { return ((type & kShortcutTypeMask) == kShortcutTypeTag); }
...
enum InstanceType : uint16_t {
    ...
    SHARED_UNCACHED_EXTERNAL_STRING_TYPE = UNCACHED_EXTERNAL_STRING_TYPE | kSharedStringTag,
    ...
    SHARED_THIN_ONE_BYTE_STRING_TYPE = THIN_ONE_BYTE_STRING_TYPE | kSharedStringTag,
...
#define CHECK_INSTANCE_TYPE(TYPE)                                                                                                                                                  \
    static_assert((TYPE >= FIRST_JS_RECEIVER_TYPE && TYPE <= LAST_SPECIAL_RECEIVER_TYPE)                                                                                           \
        == (IF_WASM(EXPAND, TYPE == WASM_STRUCT_TYPE || TYPE == WASM_ARRAY_TYPE ||) TYPE == JS_PROXY_TYPE || TYPE == JS_GLOBAL_OBJECT_TYPE || TYPE == JS_GLOBAL_PROXY_TYPE         \
            || TYPE == JS_MODULE_NAMESPACE_TYPE || TYPE == JS_SPECIAL_API_OBJECT_TYPE));                                                                                           \
    static_assert((TYPE >= FIRST_JS_RECEIVER_TYPE && TYPE <= LAST_CUSTOM_ELEMENTS_RECEIVER)                                                                                        \
        == (IF_WASM(EXPAND, TYPE == WASM_STRUCT_TYPE || TYPE == WASM_ARRAY_TYPE ||) TYPE == JS_PROXY_TYPE || TYPE == JS_GLOBAL_OBJECT_TYPE || TYPE == JS_GLOBAL_PROXY_TYPE         \
            || TYPE == JS_MODULE_NAMESPACE_TYPE || TYPE == JS_SPECIAL_API_OBJECT_TYPE || TYPE == JS_PRIMITIVE_WRAPPER_TYPE));
// clang-format on
...
#define IS_TYPE_FUNCTION_DECL(Type, ...) V8_INLINE constexpr bool Is##Type(InstanceType instance_type);
...
#define UNIQUE_LEAF_INSTANCE_TYPE_MAP_LIST_GENERATOR(V, _)                                                                                                                         \
    ...
    V(_, CachedTemplateObjectMap, cached_template_object_map, CachedTemplateObject)                                                                                                \
