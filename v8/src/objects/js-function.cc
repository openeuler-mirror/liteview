...
bool JSFunction::HasAvailableHigherTierCodeThan(CodeKind kind) const { return HasAvailableHigherTierCodeThanWithFilter(kind, kJSFunctionCodeKindsMask); }

bool JSFunction::HasAvailableHigherTierCodeThanWithFilter(CodeKind kind, CodeKinds filter_mask) const
{
    ...
    const CodeKinds masked_available_kinds = GetAvailableCodeKinds() & filter_mask;
...
base::Optional<CodeKind> JSFunction::GetActiveTier() const
{
    ...
    if (shared().HasAsmWasmData() || code().builtin_id() == Builtin::kInstantiateAsmJs) {
        return {};
    }
#endif // V8_ENABLE_WEBASSEMBLY

    CodeKind highest_tier;
    if (!HighestTierOf(GetAvailableCodeKinds(), &highest_tier))
        return {};

#ifdef DEBUG
    CHECK(highest_tier == CodeKind::TURBOFAN || highest_tier == CodeKind::BASELINE || highest_tier == CodeKind::MAGLEV || highest_tier == CodeKind::INTERPRETED_FUNCTION);

    if (highest_tier == CodeKind::INTERPRETED_FUNCTION) {
        CHECK(code().is_interpreter_trampoline_builtin() || (CodeKindIsOptimizedJSFunction(code().kind()) && code().marked_for_deoptimization())
            || (code().builtin_id() == Builtin::kCompileLazy && shared().HasBytecodeArray() && !shared().HasBaselineCode()));
    }
#endif // DEBUG
...
bool JSFunction::ActiveTierIsIgnition() const { return GetActiveTier() == CodeKind::INTERPRETED_FUNCTION; }

bool JSFunction::ActiveTierIsBaseline() const { return GetActiveTier() == CodeKind::BASELINE; }

bool JSFunction::ActiveTierIsMaglev() const { return GetActiveTier() == CodeKind::MAGLEV; }

bool JSFunction::ActiveTierIsTurbofan() const { return GetActiveTier() == CodeKind::TURBOFAN; }
...
void JSFunction::MarkForOptimization(Isolate* isolate, CodeKind target_kind, ConcurrencyMode mode)
{
    ...
    DCHECK(!is_compiled() || ActiveTierIsIgnition() || ActiveTierIsBaseline() || ActiveTierIsMaglev());
...
void JSFunction::SetInterruptBudget(Isolate* isolate) { raw_feedback_cell().set_interrupt_budget(TieringManager::InterruptBudgetFor(isolate, *this)); }
    ...
    Isolate* isolate, Handle<JSFunctionOrBoundFunctionOrWrappedFunction> function, Handle<JSReceiver> target, Handle<String> prefix, int arg_count)
{
    ...
    if (!target->IsJSFunction() || length_lookup.state() != LookupIterator::ACCESSOR || !length_lookup.GetAccessors().is_identical_to(function_length_accessor)) {
        ...
        if (attributes.FromJust() != ABSENT) {
            ...
            if (target_length->IsNumber()) {
                length = isolate->factory()->NewNumber(std::max(0.0, DoubleToInteger(target_length->Number()) - arg_count));
            }
        }
    ...
    if (!target->IsJSFunction() || name_lookup.state() != LookupIterator::ACCESSOR || !name_lookup.GetAccessors().is_identical_to(function_name_accessor)
        ...
        if (target_name->IsString()) {
            ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, name, Name::ToFunctionName(isolate, Handle<String>::cast(target_name)), Nothing<bool>());
            if (!prefix.is_null()) {
...
// static
MaybeHandle<String> JSBoundFunction::GetName(Isolate* isolate, Handle<JSBoundFunction> function)
{
    ...
    while (function->bound_target_function().IsJSBoundFunction()) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate, target_name, factory->NewConsString(prefix, target_name), String);
        function = handle(JSBoundFunction::cast(function->bound_target_function()), isolate);
    }
    if (function->bound_target_function().IsJSWrappedFunction()) {
        Handle<JSWrappedFunction> target(JSWrappedFunction::cast(function->bound_target_function()), isolate);
        Handle<String> name;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, name, JSWrappedFunction::GetName(isolate, target), String);
        return factory->NewConsString(target_name, name);
    }
    if (function->bound_target_function().IsJSFunction()) {
        Handle<JSFunction> target(JSFunction::cast(function->bound_target_function()), isolate);
...
// static
Maybe<int> JSBoundFunction::GetLength(Isolate* isolate, Handle<JSBoundFunction> function)
{
...
// static
MaybeHandle<String> JSWrappedFunction::GetName(Isolate* isolate, Handle<JSWrappedFunction> function)
{
    ...
    if (target->IsJSBoundFunction()) {
        return JSBoundFunction::GetName(isolate, handle(JSBoundFunction::cast(function->wrapped_target_function()), isolate));
    } else if (target->IsJSFunction()) {
        return JSFunction::GetName(isolate, handle(JSFunction::cast(function->wrapped_target_function()), isolate));
    }
    // This will omit the proper target name for bound JSProxies.
    return target_name;
}
...
// static
Maybe<int> JSWrappedFunction::GetLength(Isolate* isolate, Handle<JSWrappedFunction> function)
{
    STACK_CHECK(isolate, Nothing<int>());
    Handle<JSReceiver> target = handle(function->wrapped_target_function(), isolate);
    if (target->IsJSBoundFunction()) {
        return JSBoundFunction::GetLength(isolate, handle(JSBoundFunction::cast(function->wrapped_target_function()), isolate));
    }
    // All non JSFunction targets get a direct property and don't use this
    // accessor.
...
// static
MaybeHandle<Object> JSWrappedFunction::Create(Isolate* isolate, Handle<NativeContext> creation_context, Handle<JSReceiver> value)
{
    ...
    if (value->IsJSWrappedFunction()) {
        Handle<JSWrappedFunction> target_wrapped = Handle<JSWrappedFunction>::cast(value);
    ...
    if (is_abrupt.IsNothing()) {
        ...
        THROW_NEW_ERROR_RETURN_VALUE(isolate, NewError(Handle<JSFunction>(creation_context->type_error_function(), isolate), MessageTemplate::kCannotWrap), {});
    }
...
// static
Handle<String> JSFunction::GetName(Isolate* isolate, Handle<JSFunction> function)
{
...
// static
void JSFunction::EnsureClosureFeedbackCellArray(Handle<JSFunction> function, bool reset_budget_for_feedback_allocation)
{
    Isolate* const isolate = function->GetIsolate();
...
// static
void JSFunction::EnsureFeedbackVector(Isolate* isolate, Handle<JSFunction> function, IsCompiledScope* compiled_scope)
{
...
// static
void JSFunction::CreateAndAttachFeedbackVector(Isolate* isolate, Handle<JSFunction> function, IsCompiledScope* compiled_scope)
{
    DCHECK(compiled_scope->is_compiled());
    DCHECK(function->shared().HasFeedbackMetadata());
    ...
    Handle<FeedbackVector> feedback_vector = FeedbackVector::New(isolate, shared, closure_feedback_cell_array, compiled_scope);
...
// static
void JSFunction::InitializeFeedbackCell(Handle<JSFunction> function, IsCompiledScope* is_compiled_scope, bool reset_budget_for_feedback_allocation)
{
    Isolate* const isolate = function->GetIsolate();
#if V8_ENABLE_WEBASSEMBLY
    ...
    const bool needs_feedback_vector = !v8_flags.lazy_feedback_allocation || v8_flags.always_turbofan ||
        ...
        v8_flags.log_function_events || !isolate->is_best_effort_code_coverage() || function->shared().sparkplug_compiled();
    ...
    if (function->shared().sparkplug_compiled() && CanCompileWithBaseline(isolate, function->shared()) && function->ActiveTierIsIgnition()) {
...
void SetInstancePrototype(Isolate* isolate, Handle<JSFunction> function, Handle<JSReceiver> value)
{
...
void JSFunction::SetPrototype(Handle<JSFunction> function, Handle<Object> value)
{
...
void JSFunction::SetInitialMap(Isolate* isolate, Handle<JSFunction> function, Handle<Map> map, Handle<HeapObject> prototype)
{
    SetInitialMap(isolate, function, map, prototype, function);
}

void JSFunction::SetInitialMap(Isolate* isolate, Handle<JSFunction> function, Handle<Map> map, Handle<HeapObject> prototype, Handle<JSFunction> constructor)
{
    ...
    if (v8_flags.log_maps) {
        LOG(isolate, MapEvent("InitialMap", Handle<Map>(), map, "", SharedFunctionInfo::DebugName(handle(function->shared(), isolate))));
    }
}
...
void JSFunction::EnsureHasInitialMap(Handle<JSFunction> function)
{
    ...
    Handle<Map> map = isolate->factory()->NewMap(instance_type, instance_size, TERMINAL_FAST_ELEMENTS_KIND, inobject_properties);
...
bool FastInitializeDerivedMap(Isolate* isolate, Handle<JSFunction> new_target, Handle<JSFunction> constructor, Handle<Map> constructor_initial_map)
{
    ...
    int expected_nof_properties = std::max(static_cast<int>(constructor->shared().expected_nof_properties()), JSFunction::CalculateExpectedNofProperties(isolate, new_target));
...
MaybeHandle<Map> JSFunction::GetDerivedMap(Isolate* isolate, Handle<JSFunction> constructor, Handle<JSReceiver> new_target)
{
    ...
    if (new_target->IsJSFunction()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, prototype, JSReceiver::GetProperty(isolate, new_target, prototype_string), Map);
    ...
    if (!prototype->IsJSReceiver()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, context, JSReceiver::GetFunctionRealm(new_target), Map);
        DCHECK(context->IsNativeContext());
        Handle<Object> maybe_index = JSReceiver::GetDataProperty(isolate, constructor, isolate->factory()->native_context_index_symbol());
        int index = maybe_index->IsSmi() ? Smi::ToInt(*maybe_index) : Context::OBJECT_FUNCTION_INDEX;
        Handle<JSFunction> realm_constructor(JSFunction::cast(context->get(index)), isolate);
...
#define TYPED_ARRAY_CASE(Type, type, TYPE, ctype)                                                                                                                                  \
    static_assert(                                                                                                                                                                 \
        Context::TYPE##_ARRAY_FUN_INDEX == Context::FIRST_FIXED_TYPED_ARRAY_FUN_INDEX + ElementsKind::TYPE##_ELEMENTS - ElementsKind::FIRST_FIXED_TYPED_ARRAY_ELEMENTS_KIND);      \
...
MaybeHandle<Map> JSFunction::GetDerivedRabGsabMap(Isolate* isolate, Handle<JSFunction> constructor, Handle<JSReceiver> new_target)
{
...
std::unique_ptr<char[]> JSFunction::DebugNameCStr() { return shared().DebugNameCStr(); }

void JSFunction::PrintName(FILE* out) { PrintF(out, "%s", DebugNameCStr().get()); }
...
bool JSFunction::SetName(Handle<JSFunction> function, Handle<Name> name, Handle<String> prefix)
{
    Isolate* isolate = function->GetIsolate();
    ...
    if (prefix->length() > 0) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, function_name, builder.Finish(), false);
    }
    RETURN_ON_EXCEPTION_VALUE(isolate,
        JSObject::DefinePropertyOrElementIgnoreAttributes(function, isolate->factory()->name_string(), function_name, static_cast<PropertyAttributes>(DONT_ENUM | READ_ONLY)),
        false);
    return true;
}
...
Handle<String> NativeCodeFunctionSourceString(Handle<SharedFunctionInfo> shared_info)
{
...
Handle<String> JSFunction::ToString(Handle<JSFunction> function)
{
    ...
    Handle<Object> maybe_class_positions = JSReceiver::GetDataProperty(isolate, function, isolate->factory()->class_positions_symbol());
    if (maybe_class_positions->IsClassPositions()) {
        ...
        Handle<String> script_source(String::cast(Script::cast(shared_info->script()).source()), isolate);
    ...
    if (shared_info->HasWasmExportedFunctionData()) {
        Handle<WasmExportedFunctionData> function_data(shared_info->wasm_exported_function_data(), isolate);
        const wasm::WasmModule* module = function_data->instance().module();
        if (is_asmjs_module(module)) {
            std::pair<int, int> offsets = module->asm_js_offset_information->GetFunctionOffsets(declared_function_index(module, function_data->function_index()));
            Handle<String> source(String::cast(Script::cast(shared_info->script()).source()), isolate);
    ...
    return Handle<String>::cast(SharedFunctionInfo::GetSourceCodeHarmony(shared_info));
}
...
void JSFunction::CalculateInstanceSizeHelper(
    InstanceType instance_type, bool has_prototype_slot, int requested_embedder_fields, int requested_in_object_properties, int* instance_size, int* in_object_properties)
{
