...
Object CompilationCacheTable::PrimaryValueAt(InternalIndex entry) { return get(EntryToIndex(entry) + 1); }

void CompilationCacheTable::SetPrimaryValueAt(InternalIndex entry, Object value, WriteBarrierMode mode) { set(EntryToIndex(entry) + 1, value, mode); }
...
void CompilationCacheTable::SetEvalFeedbackValueAt(InternalIndex entry, Object value, WriteBarrierMode mode) { set(EntryToIndex(entry) + 2, value, mode); }
...
uint32_t CompilationCacheShape::RegExpHash(String string, Smi flags) { return string.EnsureHash() + flags.value(); }

uint32_t CompilationCacheShape::EvalHash(String source, SharedFunctionInfo shared, LanguageMode language_mode, int position)
{
...
uint32_t CompilationCacheShape::HashForObject(ReadOnlyRoots roots, Object object)
{
    ...
    if (object.IsWeakFixedArray()) {
        uint32_t result = static_cast<uint32_t>(Smi::ToInt(WeakFixedArray::cast(object).Get(ScriptCacheKey::kHash).ToSmi()));
        return result;
    }
    ...
    if (val.map() == roots.fixed_cow_array_map()) {
        ...
        return EvalHash(source, SharedFunctionInfo::cast(shared), language_mode, position);
    }
    ...
    return RegExpHash(String::cast(val.get(JSRegExp::kSourceIndex)), Smi::cast(val.get(JSRegExp::kFlagsIndex)));
}

InfoCellPair::InfoCellPair(Isolate* isolate, SharedFunctionInfo shared, FeedbackCell feedback_cell)
