...
public:
    ...
    Subclass operator+(int i) const { return Subclass(ptr_ + i * kSlotDataSize); }
    friend Subclass operator+(int i, const Subclass& slot) { return Subclass(slot.ptr_ + i * kSlotDataSize); }
...
public:
    ...
    Reference operator*() const { return Reference(SlotBase<UnalignedSlot<T>, T, 1>::address()); }
    Reference operator[](difference_type i) const { return Reference(SlotBase<UnalignedSlot<T>, T, 1>::address() + i * sizeof(T)); }

    friend void swap(Reference lhs, Reference rhs) { lhs.swap(rhs); }

    friend difference_type operator-(UnalignedSlot a, UnalignedSlot b) { return static_cast<int>(a.address() - b.address()) / sizeof(T); }
};
...
public:
    ...
    explicit ExternalPointerSlot(Address ptr)
        : SlotBase(ptr)
    {
    }
...
private:
#ifdef V8_ENABLE_SANDBOX
    inline const ExternalPointerTable& GetExternalPointerTableForTag(const Isolate* isolate, ExternalPointerTag tag);
    inline ExternalPointerTable& GetExternalPointerTableForTag(Isolate* isolate, ExternalPointerTag tag);
#endif // V8_ENABLE_SANDBOX
};
