...
void ValueSerializer::SetTreatArrayBufferViewsAsHostObjects(bool mode) { treat_array_buffer_views_as_host_objects_ = mode; }
...
void ValueSerializer::WriteUint32(uint32_t value) { WriteVarint<uint32_t>(value); }

void ValueSerializer::WriteUint64(uint64_t value) { WriteVarint<uint64_t>(value); }
...
void ValueSerializer::TransferArrayBuffer(uint32_t transfer_id, Handle<JSArrayBuffer> array_buffer)
{
...
Maybe<bool> ValueSerializer::WriteObject(Handle<Object> object)
{
    ...
    InstanceType instance_type = HeapObject::cast(*object).map(isolate_).instance_type();
    ...
    case JS_DATA_VIEW_TYPE: {
        ...
        if (!id_map_.Find(view) && !treat_array_buffer_views_as_host_objects_) {
            Handle<JSArrayBuffer> buffer(
                InstanceTypeChecker::IsJSTypedArray(instance_type) ? Handle<JSTypedArray>::cast(view)->GetBuffer() : handle(JSArrayBuffer::cast(view->buffer()), isolate_));
...
Maybe<bool> ValueSerializer::WriteJSReceiver(Handle<JSReceiver> receiver)
{
    ...
    *find_result.entry = id + 1;
    ...
    case JS_PRIMITIVE_WRAPPER_TYPE:
        return WriteJSPrimitiveWrapper(Handle<JSPrimitiveWrapper>::cast(receiver));
...
Maybe<bool> ValueSerializer::WriteJSObject(Handle<JSObject> object)
{
    ...
    for (InternalIndex i : map->IterateOwnDescriptors()) {
        ...
        if (V8_LIKELY(!map_changed && details.location() == PropertyLocation::kField)) {
            ...
            value = JSObject::FastPropertyAt(isolate_, object, details.representation(), field_index);
        } else {
    ...
    if (should_serialize_densely) {
        ...
        case PACKED_ELEMENTS: {
            ...
            for (; i < length; i++) {
                ...
                Handle<Object> element(FixedArray::cast(array->elements()).get(cage_base, i), isolate_);
        ...
        Handle<FixedArray> keys;
        if (!KeyAccumulator::GetKeys(isolate_, array, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS, GetKeysConversion::kKeepNumbers, false, true).ToHandle(&keys)) {
            return Nothing<bool>();
        }
...
Maybe<bool> ValueSerializer::WriteJSPrimitiveWrapper(Handle<JSPrimitiveWrapper> value)
{
    PtrComprCageBase cage_base(isolate_);
    {
        DisallowGarbageCollection no_gc;
...
Maybe<bool> ValueSerializer::WriteJSSharedStruct(Handle<JSSharedStruct> shared_struct)
{
    // TODO(v8:12547): Support copying serialization for shared structs as well.
    return WriteSharedObject(shared_struct);
}

Maybe<bool> ValueSerializer::WriteJSSharedArray(Handle<JSSharedArray> shared_array) { return WriteSharedObject(shared_array); }
...
Maybe<bool> ValueSerializer::WriteWasmModule(Handle<WasmModuleObject> object)
{
    ...
    Maybe<uint32_t> transfer_id
        = delegate_->GetWasmModuleTransferId(reinterpret_cast<v8::Isolate*>(isolate_), v8::Local<v8::WasmModuleObject>::Cast(Utils::ToLocal(Handle<JSObject>::cast(object))));
    RETURN_VALUE_IF_SCHEDULED_EXCEPTION(isolate_, Nothing<bool>());
    uint32_t id = 0;
...
Maybe<uint32_t> ValueSerializer::WriteJSObjectPropertiesSlow(Handle<JSObject> object, Handle<FixedArray> keys)
{
    uint32_t properties_written = 0;
...
Maybe<bool> ValueSerializer::ThrowDataCloneError(MessageTemplate template_index) { return ThrowDataCloneError(template_index, isolate_->factory()->empty_string()); }

Maybe<bool> ValueSerializer::ThrowDataCloneError(MessageTemplate index, Handle<Object> arg0)
{
...
bool ValueDeserializer::ReadUint32(uint32_t* value) { return ReadVarint<uint32_t>().To(value); }

bool ValueDeserializer::ReadUint64(uint64_t* value) { return ReadVarint<uint64_t>().To(value); }

bool ValueDeserializer::ReadDouble(double* value) { return ReadDouble().To(value); }
...
MaybeHandle<Object> ValueDeserializer::ReadObjectWrapper()
{
    ...
    if (result.is_null() && version_ == 13 && !isolate_->has_pending_exception()) {
...
MaybeHandle<Object> ValueDeserializer::ReadObject()
{
    ...
    if (result.is_null() && !suppress_deserialization_errors_ && !isolate_->has_pending_exception()) {
        isolate_->Throw(*isolate_->factory()->NewError(MessageTemplate::kDataCloneDeserializationError));
    }
...
bool ValueDeserializer::ReadExpectedString(Handle<String> expected)
{
    ...
    DCHECK_IMPLIES(!FLAG_fuzzing, byte_length <= static_cast<uint32_t>(std::numeric_limits<int32_t>::max()));
    if (!ReadRawBytes(byte_length).To(&bytes)) {
        position_ = original_position;
        return false;
    }
    ...
    if (tag == SerializationTag::kOneByteString && flat.IsOneByte()) {
        ...
        base::Vector<const base::uc16> chars = flat.ToUC16Vector();
        if (byte_length == static_cast<unsigned>(chars.length()) * sizeof(base::uc16) && memcmp(bytes.begin(), chars.begin(), byte_length) == 0) {
            return true;
        }
...
MaybeHandle<JSArray> ValueDeserializer::ReadDenseJSArray()
{
    ...
    uint32_t id = next_id_++;
    HandleScope scope(isolate_);
    Handle<JSArray> array = isolate_->factory()->NewJSArray(HOLEY_ELEMENTS, length, length, INITIALIZE_ARRAY_ELEMENTS_WITH_HOLE);
    AddObjectWithID(id, array);
...
MaybeHandle<JSPrimitiveWrapper> ValueDeserializer::ReadJSPrimitiveWrapper(SerializationTag tag)
{
    ...
    switch (tag) {
    case SerializationTag::kTrueObject:
        value = Handle<JSPrimitiveWrapper>::cast(isolate_->factory()->NewJSObject(isolate_->boolean_function()));
    ...
    case SerializationTag::kNumberObject: {
        ...
        value = Handle<JSPrimitiveWrapper>::cast(isolate_->factory()->NewJSObject(isolate_->number_function()));
    ...
    case SerializationTag::kBigIntObject: {
        ...
        if (!ReadBigInt().ToHandle(&bigint))
            return MaybeHandle<JSPrimitiveWrapper>();
        value = Handle<JSPrimitiveWrapper>::cast(isolate_->factory()->NewJSObject(isolate_->bigint_function()));
    ...
    case SerializationTag::kStringObject: {
        ...
        if (!ReadString().ToHandle(&string))
            return MaybeHandle<JSPrimitiveWrapper>();
        value = Handle<JSPrimitiveWrapper>::cast(isolate_->factory()->NewJSObject(isolate_->string_function()));
...
MaybeHandle<JSRegExp> ValueDeserializer::ReadJSRegExp()
{
    ...
    if ((raw_flags & bad_flags_mask) || !JSRegExp::New(isolate_, pattern, static_cast<JSRegExp::Flags>(raw_flags)).ToHandle(&regexp)) {
        return MaybeHandle<JSRegExp>();
    }
...
MaybeHandle<JSArrayBuffer> ValueDeserializer::ReadJSArrayBuffer(bool is_shared)
{
    uint32_t id = next_id_++;
    if (is_shared) {
        uint32_t clone_id;
    ...
    MaybeHandle<JSArrayBuffer> result = isolate_->factory()->NewJSArrayBufferAndBackingStore(byte_length, InitializedFlag::kUninitialized);
...
MaybeHandle<JSArrayBuffer> ValueDeserializer::ReadTransferredJSArrayBuffer()
{
    ...
    Handle<JSArrayBuffer> array_buffer(JSArrayBuffer::cast(transfer_map->ValueAt(index)), isolate_);
    AddObjectWithID(id, array_buffer);
    return array_buffer;
}

MaybeHandle<JSArrayBufferView> ValueDeserializer::ReadJSArrayBufferView(Handle<JSArrayBuffer> buffer)
{
    ...
    uint32_t flags = 0;
    if (!ReadVarint<uint8_t>().To(&tag) || !ReadVarint<uint32_t>().To(&byte_offset) || !ReadVarint<uint32_t>().To(&byte_length) || byte_offset > buffer_byte_length
...
#undef TYPED_ARRAY_CASE
    ...
    Handle<JSTypedArray> typed_array = isolate_->factory()->NewJSTypedArray(external_array_type, buffer, byte_offset, byte_length / element_size);
...
bool ValueDeserializer::ValidateAndSetJSArrayBufferViewFlags(JSArrayBufferView view, JSArrayBuffer buffer, uint32_t serialized_flags)
{
    bool is_length_tracking = JSArrayBufferViewIsLengthTracking::decode(serialized_flags);
    bool is_backed_by_rab = JSArrayBufferViewIsBackedByRab::decode(serialized_flags);
...
MaybeHandle<Object> ValueDeserializer::ReadJSError()
{
    ...
    while (!done) {
        ...
        case ErrorTag::kCause: {
            ...
            if (JSObject::DefinePropertyOrElementIgnoreAttributes(Handle<JSObject>::cast(options), isolate_->factory()->cause_string(), cause, DONT_ENUM).is_null()) {
                return MaybeHandle<JSObject>();
            }
            break;
        }
    ...
    if (!ErrorUtils::Construct(isolate_, constructor, constructor, message, options, SKIP_NONE, no_caller, ErrorUtils::StackTraceCollection::kDisabled).ToHandle(&error)) {
        return MaybeHandle<Object>();
    }
...
MaybeHandle<WasmMemoryObject> ValueDeserializer::ReadWasmMemory()
{
    ...
    Handle<WasmMemoryObject> result = WasmMemoryObject::New(isolate_, buffer, maximum_pages).ToHandleChecked();
        ...
        while (transitioning) {
            ...
            if (!expected_key.is_null() && ReadExpectedString(expected_key)) {
                ...
                if (key->IsString(isolate_)) {
                    ...
                    transitioning = TransitionsAccessor(isolate_, *map).FindTransitionToField(Handle<String>::cast(key)).ToHandle(&target);
                } else {
                    transitioning = false;
                }
            }
            ...
            if (transitioning) {
                ...
                if (value->FitsRepresentation(expected_representation)) {
                    if (expected_representation.IsHeapObject() && !target->instance_descriptors(isolate_).GetFieldType(descriptor).NowContains(value)) {
                    ...
                    DCHECK(target->instance_descriptors(isolate_).GetFieldType(descriptor).NowContains(value));
...
bool ValueDeserializer::HasObjectWithID(uint32_t id) { return id < static_cast<unsigned>(id_map_->length()) && !id_map_->get(id).IsTheHole(isolate_); }
...
void ValueDeserializer::AddObjectWithID(uint32_t id, Handle<JSReceiver> object)
{
    DCHECK(!HasObjectWithID(id));
