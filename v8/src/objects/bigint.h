...
#undef BIGINT_FIELDS
    ...
    static constexpr bool HasOptionalPadding() { return FIELD_SIZE(kOptionalPaddingOffset) > 0; }
...
public:
    ...
    inline static FreshlyAllocatedBigInt unchecked_cast(Object o) { return base::bit_cast<FreshlyAllocatedBigInt>(o); }
...
public:
    ...
    static MaybeHandle<BigInt> Exponentiate(Isolate* isolate, Handle<BigInt> base, Handle<BigInt> exponent);
    static MaybeHandle<BigInt> Multiply(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> Divide(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> Remainder(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    ...
    static MaybeHandle<BigInt> LeftShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> SignedRightShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> UnsignedRightShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    ...
    static MaybeHandle<BigInt> BitwiseAnd(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> BitwiseXor(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    static MaybeHandle<BigInt> BitwiseOr(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y);
    ...
    static Maybe<bool> EqualToString(Isolate* isolate, Handle<BigInt> x, Handle<String> y);
    static bool EqualToNumber(Handle<BigInt> x, Handle<Object> y);
    static Maybe<ComparisonResult> CompareToString(Isolate* isolate, Handle<BigInt> x, Handle<String> y);
    ...
    V8_EXPORT_PRIVATE static ComparisonResult CompareToDouble(Handle<BigInt> x, double y);
    ...
    static MaybeHandle<BigInt> FromWords64(Isolate* isolate, int sign_bit, int words64_count, const uint64_t* words);
    ...
    inline static int SizeFor(int length) { return kHeaderSize + length * kDigitSize; }

    static MaybeHandle<String> ToString(Isolate* isolate, Handle<BigInt> bigint, int radix = 10, ShouldThrow should_throw = kThrowOnError);
...
private:
    ...
    template <typename IsolateT> static MaybeHandle<BigInt> Allocate(IsolateT* isolate, bigint::FromStringAccumulator* accumulator, bool negative, AllocationType allocation);
