...
void PrintStatusTransition(Module module, Module::Status old_status)
{
    if (!v8_flags.trace_module_status)
        return;
    StdoutStream os;
    os << "Changing module status from " << old_status << " to " << module.status() << " for ";
...
void Module::RecordError(Isolate* isolate, Object error)
{
    ...
    DCHECK_IMPLIES(isolate->is_catchable_by_javascript(error), exception().IsTheHole(isolate));
...
void Module::ResetGraph(Isolate* isolate, Handle<Module> module)
{
    ...
    Handle<FixedArray> requested_modules = module->IsSourceTextModule() ? Handle<FixedArray>(SourceTextModule::cast(*module).requested_modules(), isolate) : Handle<FixedArray>();
...
void Module::Reset(Isolate* isolate, Handle<Module> module)
{
    ...
    const int export_count = module->IsSourceTextModule() ? SourceTextModule::cast(*module).regular_exports().length() : SyntheticModule::cast(*module).export_names().length();
...
MaybeHandle<Cell> Module::ResolveExport(
    Isolate* isolate, Handle<Module> module, Handle<String> module_specifier, Handle<String> export_name, MessageLocation loc, bool must_resolve, Module::ResolveSet* resolve_set)
{
    DCHECK_GE(module->status(), kPreLinking);
    DCHECK_NE(module->status(), kEvaluating);

    if (module->IsSourceTextModule()) {
        return SourceTextModule::ResolveExport(isolate, Handle<SourceTextModule>::cast(module), module_specifier, export_name, loc, must_resolve, resolve_set);
    } else {
        return SyntheticModule::ResolveExport(isolate, Handle<SyntheticModule>::cast(module), module_specifier, export_name, loc, must_resolve);
    }
}

bool Module::Instantiate(Isolate* isolate, Handle<Module> module, v8::Local<v8::Context> context, v8::Module::ResolveModuleCallback callback,
    DeprecatedResolveCallback callback_without_import_assertions)
{
    ...
    if (!PrepareInstantiate(isolate, module, context, callback, callback_without_import_assertions)) {
    ...
    DCHECK(module->status() == kLinked || module->status() == kEvaluated || module->status() == kErrored);
    DCHECK(stack.empty());
    return true;
}

bool Module::PrepareInstantiate(Isolate* isolate, Handle<Module> module, v8::Local<v8::Context> context, v8::Module::ResolveModuleCallback callback,
    DeprecatedResolveCallback callback_without_import_assertions)
{
    DCHECK_NE(module->status(), kEvaluating);
    ...
    if (module->IsSourceTextModule()) {
        return SourceTextModule::PrepareInstantiate(isolate, Handle<SourceTextModule>::cast(module), context, callback, callback_without_import_assertions);
    } else {
        return SyntheticModule::PrepareInstantiate(isolate, Handle<SyntheticModule>::cast(module), context);
    }
}
...
bool Module::FinishInstantiate(Isolate* isolate, Handle<Module> module, ZoneForwardList<Handle<SourceTextModule>>* stack, unsigned* dfs_index, Zone* zone)
{
    ...
    if (module->IsSourceTextModule()) {
        return SourceTextModule::FinishInstantiate(isolate, Handle<SourceTextModule>::cast(module), stack, dfs_index, zone);
    } else {
        return SyntheticModule::FinishInstantiate(isolate, Handle<SyntheticModule>::cast(module));
    }
}
...
MaybeHandle<Object> Module::Evaluate(Isolate* isolate, Handle<Module> module)
{
    ...
    if (module_status == kErrored) {
        ...
        if (module->top_level_capability().IsJSPromise()) {
            Handle<JSPromise> top_level_capability(JSPromise::cast(module->top_level_capability()), isolate);
    ...
    if (module->IsSourceTextModule()) {
        return SourceTextModule::Evaluate(isolate, Handle<SourceTextModule>::cast(module));
    } else {
        return SyntheticModule::Evaluate(isolate, Handle<SyntheticModule>::cast(module));
    }
}
...
Handle<JSModuleNamespace> Module::GetModuleNamespace(Isolate* isolate, Handle<Module> module)
{
    ...
    if (module->IsSourceTextModule()) {
        SourceTextModule::FetchStarExports(isolate, Handle<SourceTextModule>::cast(module), &zone, &visited);
    }
    ...
    std::sort(names.begin(), names.end(), [&isolate](Handle<String> a, Handle<String> b) { return String::Compare(isolate, a, b) == ComparisonResult::kLessThan; });
...
// static
Maybe<bool> JSModuleNamespace::DefineOwnProperty(Isolate* isolate, Handle<JSModuleNamespace> object, Handle<Object> key, PropertyDescriptor* desc, Maybe<ShouldThrow> should_throw)
{
    ...
    if (!has_own.FromJust() || (desc->has_configurable() && desc->configurable()) || (desc->has_enumerable() && !desc->enumerable())
        || PropertyDescriptor::IsAccessorDescriptor(desc) || (desc->has_writable() && !desc->writable()) || (desc->has_value() && !desc->value()->SameValue(*current.value()))) {
    ...
    do {
        ...
        for (int i = 0, length = requested_modules.length(); i < length; ++i) {
            ...
            if (descendant.IsSourceTextModule()) {
                ...
                if (!cycle)
                    worklist.push_back(SourceTextModule::cast(descendant));
            }
        }
