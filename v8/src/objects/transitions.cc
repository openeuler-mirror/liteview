...
// static
Map TransitionsAccessor::GetSimpleTransition(Isolate* isolate, Handle<Map> map)
{
...
// static
void TransitionsAccessor::Insert(Isolate* isolate, Handle<Map> map, Handle<Name> name, Handle<Map> target, SimpleTransitionFlag flag)
{
    ...
    if (encoding == kUninitialized || encoding == kMigrationTarget) {
        ...
        Handle<TransitionArray> result = isolate->factory()->NewTransitionArray(1, 0);
    ...
    if (encoding == kWeakRef) {
        ...
        Handle<TransitionArray> result = isolate->factory()->NewTransitionArray(1, 1);
        ...
        result->Set(0, GetSimpleTransitionKey(simple_transition), HeapObjectReference::Weak(simple_transition));

        // Search for the correct index to insert the new transition.
        int insertion_index;
        int index;
        if (flag == SPECIAL_TRANSITION) {
            index = result->SearchSpecial(Symbol::cast(*name), false, &insertion_index);
        } else {
        ...
        if (insertion_index == 0) {
            ...
            result->Set(1, GetSimpleTransitionKey(simple_transition), HeapObjectReference::Weak(simple_transition));
        }
    ...
    DCHECK_EQ(is_special_transition, IsSpecialTransition(ReadOnlyRoots(isolate), *name));
...

    {
        ...
        int index = is_special_transition ? array.SearchSpecial(Symbol::cast(*name), false, &insertion_index)
    ...
    Handle<TransitionArray> result = isolate->factory()->NewTransitionArray(new_nof, Map::SlackForArraySize(number_of_transitions, kMaxNumberOfTransitions));
...
Map TransitionsAccessor::SearchTransition(Name name, PropertyKind kind, PropertyAttributes attributes)
{
...
MaybeHandle<Map> TransitionsAccessor::FindTransitionToDataProperty(Handle<Name> name, RequestedLocation requested_location)
{
    DCHECK(name->IsUniqueName());
    DisallowGarbageCollection no_gc;
    ...
    if (requested_location == kFieldOnly && details.location() != PropertyLocation::kField) {
        return MaybeHandle<Map>();
    }
    return Handle<Map>(target, isolate_);
}

void TransitionsAccessor::ForEachTransitionTo(Name name, const ForEachTransitionCallback& callback, DisallowGarbageCollection* no_gc)
{
    DCHECK(name.IsUniqueName());
    switch (encoding()) {
    case kPrototypeInfo:
...
// static
bool TransitionsAccessor::CanHaveMoreTransitions(Isolate* isolate, Handle<Map> map)
{
...
// static
bool TransitionsAccessor::IsMatchingMap(Map target, Name name, PropertyKind kind, PropertyAttributes attributes)
{
    InternalIndex descriptor = target.LastAdded();
    DescriptorArray descriptors = target.instance_descriptors(kRelaxedLoad);
    Name key = descriptors.GetKey(descriptor);
...
// static
bool TransitionArray::CompactPrototypeTransitionArray(Isolate* isolate, WeakFixedArray array)
{
...
// static
Handle<WeakFixedArray> TransitionArray::GrowPrototypeTransitionArray(Handle<WeakFixedArray> array, int new_capacity, Isolate* isolate)
{
...
// static
void TransitionsAccessor::PutPrototypeTransition(Isolate* isolate, Handle<Map> map, Handle<Object> prototype, Handle<Map> target_map)
{
    ...
    base::SharedMutexGuard<base::kExclusive> scope(isolate->full_transition_array_access());
...
// static
Handle<Map> TransitionsAccessor::GetPrototypeTransition(Isolate* isolate, Handle<Map> map, Handle<Object> prototype)
{
    DisallowGarbageCollection no_gc;
...
// static
WeakFixedArray TransitionsAccessor::GetPrototypeTransitions(Isolate* isolate, Handle<Map> map)
{
    MaybeObject raw_transitions = map->raw_transitions(isolate, kAcquireLoad);
    if (GetEncoding(isolate, raw_transitions) != kFullTransitionArray) {
        return ReadOnlyRoots(isolate).empty_weak_fixed_array();
    }
    TransitionArray transition_array = GetTransitionArray(isolate, raw_transitions);
...
// static
void TransitionArray::SetNumberOfPrototypeTransitions(WeakFixedArray proto_transitions, int value)
{
    DCHECK_NE(proto_transitions.length(), 0);
    proto_transitions.Set(kProtoTransitionNumberOfEntriesOffset, MaybeObject::FromSmi(Smi::FromInt(value)));
}
...
// static
void TransitionsAccessor::SetMigrationTarget(Isolate* isolate, Handle<Map> map, Map migration_target)
{
    ...
    map->set_raw_transitions(MaybeObject::FromObject(migration_target), kReleaseStore);
}
...
// static
void TransitionsAccessor::ReplaceTransitions(Isolate* isolate, Handle<Map> map, MaybeObject new_transitions)
{
#if DEBUG
...
// static
void TransitionsAccessor::ReplaceTransitions(Isolate* isolate, Handle<Map> map, Handle<TransitionArray> new_transitions)
{
    ReplaceTransitions(isolate, map, MaybeObject::FromObject(*new_transitions));
}

// static
void TransitionsAccessor::SetPrototypeTransitions(Isolate* isolate, Handle<Map> map, Handle<WeakFixedArray> proto_transitions)
{
...
// static
void TransitionsAccessor::EnsureHasFullTransitionArray(Isolate* isolate, Handle<Map> map)
{
...
void TransitionsAccessor::TraverseTransitionTreeInternal(const TraverseCallback& callback, DisallowGarbageCollection* no_gc)
{
    ...
    while (!stack.empty()) {
        ...
        MaybeObject raw_transitions = current_map.raw_transitions(isolate_, kAcquireLoad);
        ...
        case kMigrationTarget:
            break;
        case kWeakRef: {
            stack.emplace_back(Map::cast(raw_transitions->GetHeapObjectAssumeWeak()));
            break;
        }
        case kFullTransitionArray: {
            TransitionArray transitions = TransitionArray::cast(raw_transitions->GetHeapObjectAssumeStrong());
...
#ifdef DEBUG
// static
void TransitionsAccessor::CheckNewTransitionsAreConsistent(Isolate* isolate, Handle<Map> map, Object transitions)
{
...
Map TransitionArray::SearchDetailsAndGetTarget(int transition, PropertyKind kind, PropertyAttributes attributes)
{
    int nof_transitions = number_of_transitions();
    DCHECK(transition < nof_transitions);
    Name key = GetKey(transition);
...
int TransitionArray::Search(PropertyKind kind, Name name, PropertyAttributes attributes, int* out_insertion_index)
{
...
Map TransitionArray::SearchAndGetTarget(PropertyKind kind, Name name, PropertyAttributes attributes)
{
...
void TransitionArray::ForEachTransitionTo(Name name, const ForEachTransitionCallback& callback)
{
    int transition = SearchName(name);
    if (transition == kNotFound)
        return;
...
bool TransitionsAccessor::HasIntegrityLevelTransitionTo(Map to, Symbol* out_symbol, PropertyAttributes* out_integrity_level)
{
