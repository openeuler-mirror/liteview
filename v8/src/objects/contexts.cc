...
void ScriptContextTable::AddLocalNamesFromContext(
    Isolate* isolate, Handle<ScriptContextTable> script_context_table, Handle<Context> script_context, bool ignore_duplicates, int script_context_index)
{
    ReadOnlyRoots roots(isolate);
    PtrComprCageBase cage_base(isolate);
    Handle<NameToIndexHashTable> names_table(script_context_table->names_to_context_index(cage_base), isolate);
...
Handle<ScriptContextTable> ScriptContextTable::Extend(Isolate* isolate, Handle<ScriptContextTable> table, Handle<Context> script_context, bool ignore_duplicates)
{
    ...
    ScriptContextTable::AddLocalNamesFromContext(isolate, result, script_context, ignore_duplicates, used);
...
JSGlobalObject Context::global_object() const { return JSGlobalObject::cast(native_context().extension()); }
...
JSGlobalProxy Context::global_proxy() const { return native_context().global_proxy_object(); }
...
static Maybe<bool> UnscopableLookup(LookupIterator* it, bool is_with_context)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(
        isolate, unscopables, JSReceiver::GetProperty(isolate, Handle<JSReceiver>::cast(it->GetReceiver()), isolate->factory()->unscopables_symbol()), Nothing<bool>());
    if (!unscopables->IsJSReceiver())
        return Just(true);
    Handle<Object> blocklist;
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, blocklist, JSReceiver::GetProperty(isolate, Handle<JSReceiver>::cast(unscopables), it->name()), Nothing<bool>());
...
// static
Handle<Object> Context::Lookup(Handle<Context> context, Handle<String> name, ContextLookupFlags flags, int* index, PropertyAttributes* attributes, InitializationFlag* init_flag,
    VariableMode* variable_mode, bool* is_sloppy_function_name)
{
    ...
    *variable_mode = VariableMode::kVar;
    ...
    do {
        ...
        DCHECK_IMPLIES(context->IsEvalContext() && context->has_extension(), context->extension().IsTheHole(isolate));
        if ((context->IsNativeContext() || context->IsWithContext() || context->IsFunctionContext() || context->IsBlockContext()) && context->has_extension()
        ...
        if (context->IsFunctionContext() || context->IsBlockContext() || context->IsScriptContext() || context->IsEvalContext() || context->IsModuleContext()
            ...
            if (slot_index >= 0) {
                ...
                if (scope_info.IsReplModeScope() && context->get(slot_index).IsTheHole(isolate)) {
            ...
            if (context->IsModuleContext()) {
                ...
                if (cell_index != 0) {
                    ...
                    *variable_mode = mode;
                    *init_flag = flag;
                    *attributes = SourceTextModuleDescriptor::GetCellIndexKind(cell_index) == SourceTextModuleDescriptor::kExport ? GetAttributesForMode(mode) : READ_ONLY;
            ...
            ScopeInfo scope_info = context->scope_info();
            CHECK_IMPLIES(v8_flags.experimental_reuse_locals_blocklists, !scope_info.HasLocalsBlockList());
            if (scope_info.HasLocalsBlockList() && scope_info.LocalsBlockList().Has(isolate, name)) {
                if (v8_flags.trace_contexts) {
        ...
        if (v8_flags.experimental_reuse_locals_blocklists && has_seen_debug_evaluate_context && isolate->heap()->locals_block_list_cache().IsEphemeronHashTable()) {
            Handle<ScopeInfo> scope_info = handle(context->scope_info(), isolate);
            Object maybe_outer_block_list = isolate->LocalsBlockListCacheGet(scope_info);
            if (maybe_outer_block_list.IsStringSet() && StringSet::cast(maybe_outer_block_list).Has(isolate, name)) {
                if (v8_flags.trace_contexts) {
                    PrintF(" - name is blocklisted. Aborting.\n");
                }
                break;
...
bool NativeContext::HasTemplateLiteralObject(JSArray array) { return array.map() == js_array_template_literal_object_map(); }
...
#define COMPARE_NAME(index, type, name)                                                                                                                                            \
    if (string->IsOneByteEqualTo(base::StaticCharVector(#name)))                                                                                                                   \
        return index;
...
int Context::IntrinsicIndexForName(const unsigned char* unsigned_string, int length)
{
...
// extensions.
bool IsContexExtensionTestObject(HeapObject extension) { return extension.IsInternalizedString() && String::cast(extension).length() == 1; }
} // namespace
...
void Context::VerifyExtensionSlot(HeapObject extension)
{
    ...
    if (extension.IsJSContextExtensionObject()) {
        ...
        CHECK(extension.IsJSReceiver() || (IsWithContext() && IsContexExtensionTestObject(extension)));
    } else if (IsNativeContext()) {
        CHECK(extension.IsJSGlobalObject() || IsContexExtensionTestObject(extension));
...
bool Context::IsBootstrappingOrValidParentContext(Object object, Context child)
{
...
void NativeContext::RunPromiseHook(PromiseHookType type, Handle<JSPromise> promise, Handle<Object> parent)
{
    ...
    Handle<Object> argv[2] = { Handle<Object>::cast(promise), parent };
