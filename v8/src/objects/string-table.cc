...
bool StringTableHasSufficientCapacityToAdd(int capacity, int number_of_elements, int number_of_deleted_elements, int number_of_additional_elements)
{
...
int ComputeStringTableCapacityWithShrink(int current_capacity, int at_least_room_for)
{
    ...
    if (new_capacity < kStringTableMinCapacity)
        return current_capacity;
    return new_capacity;
}

template <typename IsolateT, typename StringTableKey> bool KeyIsMatch(IsolateT* isolate, StringTableKey* key, String string)
{
...
public:
    ...
    OffHeapObjectSlot slot(InternalIndex index) const { return OffHeapObjectSlot(&elements_[index.as_uint32()]); }

    Object Get(PtrComprCageBase cage_base, InternalIndex index) const { return slot(index).Acquire_Load(cage_base); }

    void Set(InternalIndex index, String entry) { slot(index).Release_Store(entry); }

    void ElementAdded()
    {
        DCHECK_LT(number_of_elements_ + 1, capacity());
        DCHECK(StringTableHasSufficientCapacityToAdd(capacity(), number_of_elements(), number_of_deleted_elements(), 1));
    ...
    {
        DCHECK_LT(number_of_elements_ + 1, capacity());
        DCHECK(StringTableHasSufficientCapacityToAdd(capacity(), number_of_elements(), number_of_deleted_elements() - 1, 1));
    ...
    template <typename IsolateT, typename StringTableKey> InternalIndex FindEntry(IsolateT* isolate, StringTableKey* key, uint32_t hash) const;

    InternalIndex FindInsertionEntry(PtrComprCageBase cage_base, uint32_t hash) const;

    template <typename IsolateT, typename StringTableKey> InternalIndex FindEntryOrInsertionEntry(IsolateT* isolate, StringTableKey* key, uint32_t hash) const;

    // Helper method for StringTable::TryStringToIndexOrLookupExisting.
    template <typename Char> static Address TryStringToIndexOrLookupExisting(Isolate* isolate, String string, String source, size_t start);
...
private:
    ...
    // Returns probe entry.
    inline static InternalIndex FirstProbe(uint32_t hash, uint32_t size) { return InternalIndex(hash & (size - 1)); }

    inline static InternalIndex NextProbe(InternalIndex last, uint32_t number, uint32_t size) { return InternalIndex((last.as_uint32() + number) & (size - 1)); }
...
void* StringTable::Data::operator new(size_t size, int capacity)
{
    ...
    static_assert(offsetof(StringTable::Data, elements_) == sizeof(StringTable::Data) - sizeof(Tagged_t));
    // Make sure that elements_ is aligned when StringTable::Data is aligned.
    static_assert((alignof(StringTable::Data) + offsetof(StringTable::Data, elements_)) % kTaggedSize == 0);

    // Subtract 1 from capacity, as the member elements_ already supplies the
    // storage for the first element.
    return AlignedAllocWithRetry(size + (capacity - 1) * sizeof(Tagged_t), alignof(StringTable::Data));
}
...
std::unique_ptr<StringTable::Data> StringTable::Data::New(int capacity) { return std::unique_ptr<Data>(new (capacity) Data(capacity)); }
...
std::unique_ptr<StringTable::Data> StringTable::Data::Resize(PtrComprCageBase cage_base, std::unique_ptr<Data> data, int capacity)
{
    ...
    DCHECK(StringTableHasSufficientCapacityToAdd(new_data->capacity(), new_data->number_of_elements(), new_data->number_of_deleted_elements(), data->number_of_elements()));
    ...
    for (InternalIndex i : InternalIndex::Range(data->capacity())) {
        ...
        InternalIndex insertion_index = new_data->FindInsertionEntry(cage_base, hash);
...
template <typename IsolateT, typename StringTableKey> InternalIndex StringTable::Data::FindEntry(IsolateT* isolate, StringTableKey* key, uint32_t hash) const
{
    uint32_t count = 1;
    ...
    for (InternalIndex entry = FirstProbe(hash, capacity_);; entry = NextProbe(entry, count++, capacity_)) {
        ...
        if (element == deleted_element())
            continue;
        String string = String::cast(element);
        if (KeyIsMatch(isolate, key, string))
            return entry;
    }
}

InternalIndex StringTable::Data::FindInsertionEntry(PtrComprCageBase cage_base, uint32_t hash) const
{
    uint32_t count = 1;
    // EnsureCapacity will guarantee the hash table is never full.
    for (InternalIndex entry = FirstProbe(hash, capacity_);; entry = NextProbe(entry, count++, capacity_)) {
...
template <typename IsolateT, typename StringTableKey> InternalIndex StringTable::Data::FindEntryOrInsertionEntry(IsolateT* isolate, StringTableKey* key, uint32_t hash) const
{
...
void StringTable::Data::Print(PtrComprCageBase cage_base) const
{
    ...
    for (InternalIndex i : InternalIndex::Range(capacity_)) {
        os << "  " << i.as_uint32() << ": " << Brief(Get(cage_base, i)) << std::endl;
    }
    os << "}" << std::endl;
}
...
int StringTable::Capacity() const { return data_.load(std::memory_order_acquire)->capacity(); }
...
public:
        ...
        , string_(string)
    {
        ...
        DCHECK_IMPLIES(!v8_flags.shared_string_table, !string->IsInternalizedString());
    ...
    void PrepareForInsertion(Isolate* isolate)
    {
        ...
        if (can_avoid_copy && shape.IsExternalOneByte()) {
            ...
            string_ = isolate->factory()->InternalizeExternalString<ExternalOneByteString>(string_);
            ...
            // resource) immediately.
            DCHECK(!shape.IsShared());
            string_ = isolate->factory()->InternalizeExternalString<ExternalTwoByteString>(string_);
        } else {
            // Otherwise allocate a new internalized string.
            string_ = isolate->factory()->NewInternalizedStringImpl(string_, length(), raw_hash_field());
        }
    }
...
void SetInternalizedReference(Isolate* isolate, String string, String internalized)
{
...
Handle<String> StringTable::LookupString(Isolate* isolate, Handle<String> string)
{
    ...
    if (!result->IsInternalizedString()) {
        ...
        if (String::IsInternalizedForwardingIndex(raw_hash_field)) {
            ...
            result = handle(isolate->string_forwarding_table()->GetForwardString(isolate, index), isolate);
        } else {
...
template <typename StringTableKey, typename IsolateT> Handle<String> StringTable::LookupKey(IsolateT* isolate, StringTableKey* key)
{
    ...
    if (entry.is_found()) {
        Handle<String> result(String::cast(current_data->Get(isolate, entry)), isolate);
...
template Handle<String> StringTable::LookupKey(Isolate* isolate, OneByteStringKey* key);
template Handle<String> StringTable::LookupKey(Isolate* isolate, TwoByteStringKey* key);
template Handle<String> StringTable::LookupKey(Isolate* isolate, SeqOneByteSubStringKey* key);
template Handle<String> StringTable::LookupKey(Isolate* isolate, SeqTwoByteSubStringKey* key);

template Handle<String> StringTable::LookupKey(LocalIsolate* isolate, OneByteStringKey* key);
template Handle<String> StringTable::LookupKey(LocalIsolate* isolate, TwoByteStringKey* key);

template Handle<String> StringTable::LookupKey(Isolate* isolate, StringTableInsertionKey* key);
template Handle<String> StringTable::LookupKey(LocalIsolate* isolate, StringTableInsertionKey* key);
...
StringTable::Data* StringTable::EnsureCapacity(PtrComprCageBase cage_base, int additional_elements)
{
    ...
    int capacity_after_shrinking = ComputeStringTableCapacityWithShrink(current_capacity, current_nof + 1);

    int new_capacity = -1;
    if (capacity_after_shrinking < current_capacity) {
        DCHECK(StringTableHasSufficientCapacityToAdd(capacity_after_shrinking, current_nof, 0, 1));
        new_capacity = capacity_after_shrinking;
    } else if (!StringTableHasSufficientCapacityToAdd(current_capacity, current_nof, data->number_of_deleted_elements(), 1)) {
...
// static
template <typename Char> Address StringTable::Data::TryStringToIndexOrLookupExisting(Isolate* isolate, String string, String source, size_t start)
{
    ...
    if (Name::IsInternalizedForwardingIndex(raw_hash_field) && is_source_hash_usable) {
        ...
        String internalized = isolate->string_forwarding_table()->GetForwardString(isolate, index);
...
void StringTable::Print(PtrComprCageBase cage_base) const { data_.load(std::memory_order_acquire)->Print(cage_base); }

size_t StringTable::GetCurrentMemoryUsage() const { return sizeof(*this) + data_.load(std::memory_order_acquire)->GetCurrentMemoryUsage(); }
