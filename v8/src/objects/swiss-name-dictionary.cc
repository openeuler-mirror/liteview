...
// static
template <typename IsolateT> Handle<SwissNameDictionary> SwissNameDictionary::Rehash(IsolateT* isolate, Handle<SwissNameDictionary> table, int new_capacity)
{
    ...
    Handle<SwissNameDictionary> new_table
        = isolate->factory()->NewSwissNameDictionaryWithCapacity(new_capacity, Heap::InYoungGeneration(*table) ? AllocationType::kYoung : AllocationType::kOld);
...
Handle<SwissNameDictionary> SwissNameDictionary::ShallowCopy(Isolate* isolate, Handle<SwissNameDictionary> table)
{
    ...
    Handle<SwissNameDictionary> new_table
        = isolate->factory()->NewSwissNameDictionaryWithCapacity(capacity, Heap::InYoungGeneration(*table) ? AllocationType::kYoung : AllocationType::kOld);
    ...
    new_table->meta_table().copy_in(0, table->meta_table().GetDataStartAddress(), meta_table_used_bytes);
...
template <typename IsolateT> void SwissNameDictionary::Rehash(IsolateT* isolate)
{
    ...
    for (int enum_index = 0; enum_index < UsedCapacity(); ++enum_index) {
        ...
        data[data_index++] = Entry { Name::cast(key), ValueAtRaw(entry), DetailsAt(entry) };
    }
...
template V8_EXPORT_PRIVATE void SwissNameDictionary::Initialize(Isolate* isolate, ByteArray meta_table, int capacity);
template V8_EXPORT_PRIVATE void SwissNameDictionary::Initialize(LocalIsolate* isolate, ByteArray meta_table, int capacity);

template V8_EXPORT_PRIVATE Handle<SwissNameDictionary> SwissNameDictionary::Rehash(LocalIsolate* isolate, Handle<SwissNameDictionary> table, int new_capacity);
template V8_EXPORT_PRIVATE Handle<SwissNameDictionary> SwissNameDictionary::Rehash(Isolate* isolate, Handle<SwissNameDictionary> table, int new_capacity);
