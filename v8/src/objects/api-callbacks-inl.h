...
EXTERNAL_POINTER_ACCESSORS(AccessorInfo, maybe_redirected_getter, Address, kMaybeRedirectedGetterOffset, kAccessorInfoGetterTag)
EXTERNAL_POINTER_ACCESSORS(AccessorInfo, setter, Address, kSetterOffset, kAccessorInfoSetterTag)
...
bool AccessorInfo::has_getter() { return maybe_redirected_getter() != kNullAddress; }

bool AccessorInfo::has_setter() { return setter() != kNullAddress; }

BIT_FIELD_ACCESSORS(AccessorInfo, flags, all_can_read, AccessorInfo::AllCanReadBit)
BIT_FIELD_ACCESSORS(AccessorInfo, flags, all_can_write, AccessorInfo::AllCanWriteBit)
BIT_FIELD_ACCESSORS(AccessorInfo, flags, is_special_data_property, AccessorInfo::IsSpecialDataPropertyBit)
...
SideEffectType AccessorInfo::setter_side_effect_type() const { return SetterSideEffectTypeBits::decode(flags()); }
...
bool CallHandlerInfo::IsSideEffectFreeCallHandlerInfo() const
{
    ...
    DCHECK(map() == roots.side_effect_call_handler_info_map() || map() == roots.side_effect_free_call_handler_info_map()
...
void CallHandlerInfo::SetNextCallHasNoSideEffect() { set_map(GetReadOnlyRoots().next_call_side_effect_free_call_handler_info_map()); }
...
EXTERNAL_POINTER_ACCESSORS(CallHandlerInfo, maybe_redirected_callback, Address, kMaybeRedirectedCallbackOffset, kCallHandlerInfoCallbackTag)
...
void CallHandlerInfo::init_callback(i::Isolate* isolate, Address initial_value)
{
