...
int SearchLiteralsMapEntry(CompilationCacheTable cache, InternalIndex cache_entry, Context native_context)
{
...
void AddToFeedbackCellsMap(Handle<CompilationCacheTable> cache, InternalIndex cache_entry, Handle<Context> native_context, Handle<FeedbackCell> feedback_cell)
{
    ...
    if (!obj.IsWeakFixedArray() || WeakFixedArray::cast(obj).length() == 0) {
        ...
        if (entry >= 0) {
            ...
            old_literals_map->Set(entry + kLiteralLiteralsOffset, HeapObjectReference::Weak(*feedback_cell));
            return;
        }
    ...
    new_literals_map->Set(entry + kLiteralContextOffset, HeapObjectReference::Weak(*native_context));
    new_literals_map->Set(entry + kLiteralLiteralsOffset, HeapObjectReference::Weak(*feedback_cell));
...
FeedbackCell SearchLiteralsMap(CompilationCacheTable cache, InternalIndex cache_entry, Context native_context)
{
    FeedbackCell result;
    int entry = SearchLiteralsMapEntry(cache, cache_entry, native_context);
    if (entry >= 0) {
        WeakFixedArray literals_map = WeakFixedArray::cast(cache.EvalFeedbackValueAt(cache_entry));
...
public:
    ...
    bool IsMatch(Object obj) override
    {
        ...
        return string_->Equals(String::cast(val.get(JSRegExp::kSourceIndex))) && (flags_ == val.get(JSRegExp::kFlagsIndex));
    }
...
Smi ScriptHash(String source, MaybeHandle<Object> maybe_name, int line_offset, int column_offset, v8::ScriptOriginOptions origin_options, Isolate* isolate)
{
    DisallowGarbageCollection no_gc;
    size_t hash = base::hash_combine(source.EnsureHash());
    if (Handle<Object> name; maybe_name.ToHandle(&name) && name->IsString(isolate)) {
        hash = base::hash_combine(hash, String::cast(*name).EnsureHash(), line_offset, column_offset, origin_options.Flags());
    }
...
bool ScriptCacheKey::MatchesOrigin(Script script)
{
    ...
    FixedArray host_defined_options = FixedArray::cast(*maybe_host_defined_options);
...
ScriptCacheKey::ScriptCacheKey(Handle<String> source, const ScriptDetails* script_details, Isolate* isolate)
    : ScriptCacheKey(
        source, script_details->name_obj, script_details->line_offset, script_details->column_offset, script_details->origin_options, script_details->host_defined_options, isolate)
{
}
...
Handle<Object> ScriptCacheKey::AsHandle(Isolate* isolate, Handle<SharedFunctionInfo> shared)
{
    ...
    array->Set(kHash, MaybeObject::FromObject(Smi::FromInt(static_cast<int>(Hash()))));
    array->Set(kWeakScript, MaybeObject::MakeWeak(MaybeObject::FromObject(shared->script())));
    return array;
}
...
CompilationCacheScriptLookupResult CompilationCacheTable::LookupScript(
    Handle<CompilationCacheTable> table, Handle<String> src, const ScriptDetails& script_details, Isolate* isolate)
{
    ...
    Script script = Script::cast(WeakFixedArray::cast(key_in_table).Get(ScriptCacheKey::kWeakScript).GetHeapObjectAssumeWeak());
...
InfoCellPair CompilationCacheTable::LookupEval(
    Handle<CompilationCacheTable> table, Handle<String> src, Handle<SharedFunctionInfo> outer_info, Handle<Context> native_context, LanguageMode language_mode, int position)
{
    ...
    if (!table->KeyAt(entry).IsFixedArray())
        return empty_result;
    Object obj = table->PrimaryValueAt(entry);
    ...
    FeedbackCell feedback_cell = SearchLiteralsMap(*table, entry, *native_context);
    ...
    // pointers.
    {
        ...
        for (InternalIndex entry : cache->IterateEntries()) {
            ...
            if (!cache->ToKey(isolate, entry, &key))
                continue;
            if (WeakFixedArray::cast(key).Get(ScriptCacheKey::kWeakScript).IsCleared()) {
...
Handle<CompilationCacheTable> CompilationCacheTable::PutScript(Handle<CompilationCacheTable> cache, Handle<String> src, Handle<SharedFunctionInfo> value, Isolate* isolate)
{
    src = String::Flatten(isolate, src);
    ...
    ScriptCacheKey key(src, script_name, script->line_offset(), script->column_offset(), script->origin_options(), host_defined_options, isolate);
...
Handle<CompilationCacheTable> CompilationCacheTable::PutEval(Handle<CompilationCacheTable> cache, Handle<String> src, Handle<SharedFunctionInfo> outer_info,
    Handle<SharedFunctionInfo> value, Handle<Context> native_context, Handle<FeedbackCell> feedback_cell, int position)
{
    Isolate* isolate = native_context->GetIsolate();
    src = String::Flatten(isolate, src);
...
Handle<CompilationCacheTable> CompilationCacheTable::PutRegExp(
    Isolate* isolate, Handle<CompilationCacheTable> cache, Handle<String> src, JSRegExp::Flags flags, Handle<FixedArray> value)
{
    RegExpKey key(src, flags);
    cache = EnsureCapacity(isolate, cache);
    InternalIndex entry = cache->FindInsertionEntry(isolate, key.Hash());
