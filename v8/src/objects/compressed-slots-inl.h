...
void CompressedObjectSlot::store(Object value) const { *location() = TCompressionScheme::CompressTagged(value.ptr()); }
...
Object CompressedObjectSlot::Release_CompareAndSwap(Object old, Object target) const
{
...
void CompressedMaybeObjectSlot::store(MaybeObject value) const { *location() = TCompressionScheme::CompressTagged(value.ptr()); }
...
MaybeObject CompressedMaybeObjectSlot::Relaxed_Load(PtrComprCageBase cage_base) const
{
    AtomicTagged_t value = AsAtomicTagged::Relaxed_Load(location());
    return MaybeObject(TCompressionScheme::DecompressTaggedAny(cage_base, value));
}
...
void CompressedMaybeObjectSlot::Release_CompareAndSwap(MaybeObject old, MaybeObject target) const
{
    Tagged_t old_ptr = TCompressionScheme::CompressTagged(old.ptr());
    Tagged_t target_ptr = TCompressionScheme::CompressTagged(target.ptr());
...
{
    Tagged_t value = *location();
    return HeapObjectReference(TCompressionScheme::DecompressTaggedPointer(address(), value));
}

HeapObjectReference CompressedHeapObjectSlot::load(PtrComprCageBase cage_base) const
{
    Tagged_t value = *location();
    return HeapObjectReference(TCompressionScheme::DecompressTaggedPointer(cage_base, value));
}

void CompressedHeapObjectSlot::store(HeapObjectReference value) const { *location() = TCompressionScheme::CompressTagged(value.ptr()); }

HeapObject CompressedHeapObjectSlot::ToHeapObject() const
{
    Tagged_t value = *location();
    DCHECK(HAS_STRONG_HEAP_OBJECT_TAG(value));
    return HeapObject::cast(Object(TCompressionScheme::DecompressTaggedPointer(address(), value)));
}

void CompressedHeapObjectSlot::StoreHeapObject(HeapObject value) const { *location() = TCompressionScheme::CompressTagged(value.ptr()); }

//
// OffHeapCompressedObjectSlot implementation.
//

template <typename CompressionScheme> Object OffHeapCompressedObjectSlot<CompressionScheme>::load(PtrComprCageBase cage_base) const
{
    Tagged_t value = *TSlotBase::location();
    return Object(CompressionScheme::DecompressTaggedAny(cage_base, value));
}

template <typename CompressionScheme> void OffHeapCompressedObjectSlot<CompressionScheme>::store(Object value) const
{
    *TSlotBase::location() = CompressionScheme::CompressTagged(value.ptr());
}

template <typename CompressionScheme> Object OffHeapCompressedObjectSlot<CompressionScheme>::Relaxed_Load(PtrComprCageBase cage_base) const
{
    AtomicTagged_t value = AsAtomicTagged::Relaxed_Load(TSlotBase::location());
    return Object(CompressionScheme::DecompressTaggedAny(cage_base, value));
}

template <typename CompressionScheme> Object OffHeapCompressedObjectSlot<CompressionScheme>::Acquire_Load(PtrComprCageBase cage_base) const
{
    AtomicTagged_t value = AsAtomicTagged::Acquire_Load(TSlotBase::location());
    return Object(CompressionScheme::DecompressTaggedAny(cage_base, value));
}

template <typename CompressionScheme> void OffHeapCompressedObjectSlot<CompressionScheme>::Relaxed_Store(Object value) const
{
    Tagged_t ptr = CompressionScheme::CompressTagged(value.ptr());
    AsAtomicTagged::Relaxed_Store(TSlotBase::location(), ptr);
}

template <typename CompressionScheme> void OffHeapCompressedObjectSlot<CompressionScheme>::Release_Store(Object value) const
{
    Tagged_t ptr = CompressionScheme::CompressTagged(value.ptr());
    AsAtomicTagged::Release_Store(TSlotBase::location(), ptr);
}

template <typename CompressionScheme> void OffHeapCompressedObjectSlot<CompressionScheme>::Release_CompareAndSwap(Object old, Object target) const
{
