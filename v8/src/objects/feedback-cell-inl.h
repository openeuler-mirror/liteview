...
void FeedbackCell::clear_padding()
{
    ...
    memset(reinterpret_cast<byte*>(address() + FeedbackCell::kUnalignedSize), 0, FeedbackCell::kAlignedSize - FeedbackCell::kUnalignedSize);
}

void FeedbackCell::reset_feedback_vector(base::Optional<std::function<void(HeapObject object, ObjectSlot slot, HeapObject target)>> gc_notify_updated_slot)
{
    SetInitialInterruptBudget();
    if (value().IsUndefined() || value().IsClosureFeedbackCellArray())
        return;

    CHECK(value().IsFeedbackVector());
    ClosureFeedbackCellArray closure_feedback_cell_array = FeedbackVector::cast(value()).closure_feedback_cell_array();
...
void FeedbackCell::SetInitialInterruptBudget() { set_interrupt_budget(TieringManager::InitialInterruptBudget()); }
