...
class FixedArrayBase : public TorqueGeneratedFixedArrayBase<FixedArrayBase, HeapObject> {
public:
...
protected:
    ...
    inline FixedArrayBase(Address ptr, HeapObject::AllowInlineSmiStorage allow_smi);
};

// FixedArray describes fixed-sized arrays with element type Object.
class FixedArray : public TorqueGeneratedFixedArray<FixedArray, FixedArrayBase> {
public:
    ...
    V8_EXPORT_PRIVATE static Handle<FixedArray> SetAndGrow(Isolate* isolate, Handle<FixedArray> array, int index, Handle<Object> value);

    // Relaxed accessors.
    inline Object get(int index, RelaxedLoadTag) const;
    inline Object get(PtrComprCageBase cage_base, int index, RelaxedLoadTag) const;
    inline void set(int index, Object value, RelaxedStoreTag, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    inline void set(int index, Smi value, RelaxedStoreTag);

    // SeqCst accessors.
    inline Object get(int index, SeqCstAccessTag) const;
    inline Object get(PtrComprCageBase cage_base, int index, SeqCstAccessTag) const;
    inline void set(int index, Object value, SeqCstAccessTag, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    inline Object get(PtrComprCageBase cage_base, int index, AcquireLoadTag) const;
    inline void set(int index, Object value, ReleaseStoreTag, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    inline Object swap(int index, Object value, SeqCstAccessTag, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    ...
    inline void CopyElements(Isolate* isolate, int dst_index, FixedArray src, int src_index, int len, WriteBarrierMode mode);
    ...
    V8_EXPORT_PRIVATE void CopyTo(int pos, FixedArray dest, int dest_pos, int len) const;

    // Garbage collection support.
    static constexpr int SizeFor(int length) { return kHeaderSize + length * kTaggedSize; }
    ...
    static_assert(Internals::IsValidSmi(kMaxLength), "FixedArray maxLength not a Smi");
...
protected:
    ...
    static inline void NoWriteBarrierSet(FixedArray array, int index, Object value);
...
class FixedDoubleArray : public TorqueGeneratedFixedDoubleArray<FixedDoubleArray, FixedArrayBase> {
public:
    // Setter and getter for elements.
    inline double get_scalar(int index);
    inline uint64_t get_representation(int index);
    static inline Handle<Object> get(FixedDoubleArray array, int index, Isolate* isolate);
    ...
    // Garbage collection support.
    inline static int SizeFor(int length) { return kHeaderSize + length * kDoubleSize; }
    ...
    static_assert(Internals::IsValidSmi(kMaxLength), "FixedDoubleArray maxLength not a Smi");
...
// MaybeObject.
class WeakFixedArray : public TorqueGeneratedWeakFixedArray<WeakFixedArray, HeapObject> {
public:
    inline MaybeObject Get(int index) const;
    inline MaybeObject Get(PtrComprCageBase cage_base, int index) const;

    inline void Set(int index, MaybeObject value, WriteBarrierMode mode = WriteBarrierMode::UPDATE_WRITE_BARRIER);

    static inline Handle<WeakFixedArray> EnsureSpace(Isolate* isolate, Handle<WeakFixedArray> array, int length);
    ...
    inline void CopyElements(Isolate* isolate, int dst_index, WeakFixedArray src, int src_index, int len, WriteBarrierMode mode);
    ...
    static_assert(Internals::IsValidSmi(kMaxLength), "WeakFixedArray maxLength not a Smi");
...
public:
    ...
    V8_EXPORT_PRIVATE static Handle<WeakArrayList> AddToEnd(Isolate* isolate, Handle<WeakArrayList> array, const MaybeObjectHandle& value);

    // A version that adds to elements. This ensures that the elements are
    // inserted atomically w.r.t GC.
    V8_EXPORT_PRIVATE static Handle<WeakArrayList> AddToEnd(Isolate* isolate, Handle<WeakArrayList> array, const MaybeObjectHandle& value1, const MaybeObjectHandle& value2);
    ...
    static Handle<WeakArrayList> Append(Isolate* isolate, Handle<WeakArrayList> array, const MaybeObjectHandle& value, AllocationType allocation = AllocationType::kYoung);
    ...
    // instead.
    inline void Set(int index, MaybeObject value, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
    inline void Set(int index, Smi value);

    static constexpr int SizeForCapacity(int capacity) { return SizeFor(capacity); }

    static constexpr int CapacityForLength(int length) { return length + std::max(length / 2, 2); }

    // Gives access to raw memory which stores the array's data.
    inline MaybeObjectSlot data_start();

    inline void CopyElements(Isolate* isolate, int dst_index, WeakArrayList src, int src_index, int len, WriteBarrierMode mode);
...
private:
    static int OffsetOfElementAt(int index) { return kHeaderSize + index * kTaggedSize; }
...
public:
    V8_EXPORT_PRIVATE static Handle<ArrayList> Add(Isolate* isolate, Handle<ArrayList> array, Handle<Object> obj, AllocationType allocation = AllocationType::kYoung);
    V8_EXPORT_PRIVATE static Handle<ArrayList> Add(Isolate* isolate, Handle<ArrayList> array, Handle<Object> obj1, Handle<Object> obj2);
    V8_EXPORT_PRIVATE static Handle<ArrayList> Add(Isolate* isolate, Handle<ArrayList> array, Handle<Object> obj1, Smi obj2, Smi obj3, Smi obj4);
    ...
    inline void Set(int index, Object obj, WriteBarrierMode mode = UPDATE_WRITE_BARRIER);
...
template <SearchMode search_mode, typename T> inline int Search(T* array, Name name, int valid_entries = 0, int* out_insertion_index = nullptr, bool concurrent_search = false);
...
public:
    ...
    static int SizeFor(int length) { return OBJECT_POINTER_ALIGN(kHeaderSize + length); }
    ...
    static_assert(Internals::IsValidSmi(kMaxLength), "ByteArray maxLength not a Smi");
...
template <typename T> class FixedIntegerArray : public ByteArray {
    static_assert(std::is_integral<T>::value);

public:
    static Handle<FixedIntegerArray<T>> New(Isolate* isolate, int length, AllocationType allocation = AllocationType::kYoung);
    ...
    // Code Generation support.
    static constexpr int OffsetOfElementAt(int index) { return kHeaderSize + index * sizeof(T); }
...
template <class T> class PodArray : public ByteArray {
public:
    static Handle<PodArray<T>> New(Isolate* isolate, int length, AllocationType allocation = AllocationType::kYoung);
    void copy_out(int index, T* result, int length) { ByteArray::copy_out(index * sizeof(T), reinterpret_cast<byte*>(result), length * sizeof(T)); }

    void copy_in(int index, const T* buffer, int length) { ByteArray::copy_in(index * sizeof(T), reinterpret_cast<const byte*>(buffer), length * sizeof(T)); }
    ...
    bool matches(int offset, const T* buffer, int length)
    {
        ...
        return memcmp(GetDataStartAddress() + sizeof(T) * offset, buffer, length * sizeof(T)) == 0;
    }
...
class TemplateList : public TorqueGeneratedTemplateList<TemplateList, FixedArray> {
public:
    ...
    inline Object get(PtrComprCageBase cage_base, int index) const;
    inline void set(int index, Object value);
    static Handle<TemplateList> Add(Isolate* isolate, Handle<TemplateList> list, Handle<Object> value);
