...
Object PropertyArray::get(PtrComprCageBase cage_base, int index, SeqCstAccessTag tag) const
{
    DCHECK_LT(static_cast<unsigned>(index), static_cast<unsigned>(this->length(kAcquireLoad)));
    return TaggedField<Object>::SeqCst_Load(cage_base, *this, OffsetOfElementAt(index));
}
...
Object PropertyArray::Swap(PtrComprCageBase cage_base, int index, Object value, SeqCstAccessTag tag)
{
    DCHECK(IsPropertyArray());
    DCHECK_LT(static_cast<unsigned>(index), static_cast<unsigned>(this->length(kAcquireLoad)));
    DCHECK(value.IsShared());
    return TaggedField<Object>::SeqCst_Swap(cage_base, *this, OffsetOfElementAt(index), value);
    // JSSharedStructs are allocated in the shared old space, which is currently
    // collected by stopping the world, so the incremental write barrier is not
    // needed. They can only store Smis and other HeapObjects in the shared old
    // space, so the generational write barrier is also not needed.
...
int PropertyArray::length() const { return LengthField::decode(length_and_hash()); }
...
int PropertyArray::length(AcquireLoadTag) const { return LengthField::decode(length_and_hash(kAcquireLoad)); }
...
void PropertyArray::CopyElements(Isolate* isolate, int dst_index, PropertyArray src, int src_index, int len, WriteBarrierMode mode)
{
    if (len == 0)
        return;
