...
public:
    ...
    // Allocation helpers.
    template <typename IsolateT> static MaybeHandle<MutableBigInt> New(IsolateT* isolate, int length, AllocationType allocation = AllocationType::kYoung);
    ...
    static Handle<MutableBigInt> Copy(Isolate* isolate, Handle<BigIntBase> source);
    ...
    static MutableBigInt unchecked_cast(Object o) { return MutableBigInt(o.ptr()); }

    // Internal helpers.
    static MaybeHandle<MutableBigInt> AbsoluteAddOne(Isolate* isolate, Handle<BigIntBase> x, bool sign, MutableBigInt result_storage = MutableBigInt());
    static Handle<MutableBigInt> AbsoluteSubOne(Isolate* isolate, Handle<BigIntBase> x);
    ...
    static inline bool digit_ismax(digit_t x) { return static_cast<digit_t>(~x) == 0; }
...
{
    return bigint::Digits(reinterpret_cast<bigint::digit_t*>(bigint.ptr() + BigIntBase::kDigitsOffset - kHeapObjectTag), bigint.length());
}
bigint::Digits GetDigits(Handle<BigIntBase> bigint) { return GetDigits(*bigint); }

bigint::RWDigits GetRWDigits(MutableBigInt bigint)
{
    return bigint::RWDigits(reinterpret_cast<bigint::digit_t*>(bigint.ptr() + BigIntBase::kDigitsOffset - kHeapObjectTag), bigint.length());
}
bigint::RWDigits GetRWDigits(Handle<MutableBigInt> bigint) { return GetRWDigits(*bigint); }
...
template <typename IsolateT> MaybeHandle<MutableBigInt> MutableBigInt::New(IsolateT* isolate, int length, AllocationType allocation)
{
...
Handle<BigInt> MutableBigInt::NewFromDouble(Isolate* isolate, double value)
{
    ...
    int raw_exponent = static_cast<int>(double_bits >> base::Double::kPhysicalSignificandSize) & 0x7FF;
...
Handle<MutableBigInt> MutableBigInt::Copy(Isolate* isolate, Handle<BigIntBase> source)
{
    ...
    memcpy(reinterpret_cast<void*>(result->address() + BigIntBase::kHeaderSize), reinterpret_cast<void*>(source->address() + BigIntBase::kHeaderSize),
        BigInt::SizeFor(length) - BigIntBase::kHeaderSize);
    return result;
}

void MutableBigInt::InitializeDigits(int length, byte value) { memset(reinterpret_cast<void*>(ptr() + kDigitsOffset - kHeapObjectTag), value, length * kDigitSize); }
...
template <typename IsolateT> Handle<BigInt> BigInt::Zero(IsolateT* isolate, AllocationType allocation) { return MutableBigInt::Zero(isolate, allocation); }
...
MaybeHandle<BigInt> BigInt::Exponentiate(Isolate* isolate, Handle<BigInt> base, Handle<BigInt> exponent)
{
    // 1. If exponent is < 0, throw a RangeError exception.
    if (exponent->sign()) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kBigIntNegativeExponent), BigInt);
    }
...
MaybeHandle<BigInt> BigInt::Multiply(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    ...
    bigint::Status status = isolate->bigint_processor()->Multiply(GetRWDigits(result), GetDigits(x), GetDigits(y));
...
MaybeHandle<BigInt> BigInt::Divide(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    // 1. If y is 0n, throw a RangeError exception.
    if (y->is_zero()) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kBigIntDivZero), BigInt);
    }
    ...
    DisallowGarbageCollection no_gc;
    bigint::Status status = isolate->bigint_processor()->Divide(GetRWDigits(quotient), GetDigits(x), GetDigits(y));
    if (status == bigint::Status::kInterrupted) {
        AllowGarbageCollection terminating_anyway;
        isolate->TerminateExecution();
        return {};
...
MaybeHandle<BigInt> BigInt::Remainder(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    // 1. If y is 0n, throw a RangeError exception.
    if (y->is_zero()) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kBigIntDivZero), BigInt);
    ...
    DisallowGarbageCollection no_gc;
    bigint::Status status = isolate->bigint_processor()->Modulo(GetRWDigits(remainder), GetDigits(x), GetDigits(y));
    if (status == bigint::Status::kInterrupted) {
        AllowGarbageCollection terminating_anyway;
        isolate->TerminateExecution();
        return {};
...
MaybeHandle<BigInt> BigInt::Add(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    ...
    bool xsign = x->sign();
    bool ysign = y->sign();
    int result_length = bigint::AddSignedResultLength(x->length(), y->length(), xsign == ysign);
    Handle<MutableBigInt> result;
    if (!MutableBigInt::New(isolate, result_length).ToHandle(&result)) {
    ...
    DisallowGarbageCollection no_gc;
    bool result_sign = bigint::AddSigned(GetRWDigits(result), GetDigits(x), xsign, GetDigits(y), ysign);
...
MaybeHandle<BigInt> BigInt::Subtract(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    ...
    bool xsign = x->sign();
    bool ysign = y->sign();
    int result_length = bigint::SubtractSignedResultLength(x->length(), y->length(), xsign == ysign);
    Handle<MutableBigInt> result;
    if (!MutableBigInt::New(isolate, result_length).ToHandle(&result)) {
        // Allocation fails when {result_length} exceeds the max BigInt size.
        return {};
    ...
    DisallowGarbageCollection no_gc;
    bool result_sign = bigint::SubtractSigned(GetRWDigits(result), GetDigits(x), xsign, GetDigits(y), ysign);
    result->set_sign(result_sign);
    return MutableBigInt::MakeImmutable(result);
}

MaybeHandle<BigInt> BigInt::LeftShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
...
MaybeHandle<BigInt> BigInt::SignedRightShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    if (y->is_zero() || x->is_zero())
        return x;
    if (y->sign())
...
MaybeHandle<BigInt> BigInt::UnsignedRightShift(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
...
ComparisonResult UnequalSign(bool left_negative) { return left_negative ? ComparisonResult::kLessThan : ComparisonResult::kGreaterThan; }

// Produces result for |x| > |y|, with {both_negative} == sign(x) == sign(y);
ComparisonResult AbsoluteGreater(bool both_negative) { return both_negative ? ComparisonResult::kLessThan : ComparisonResult::kGreaterThan; }

// Produces result for |x| < |y|, with {both_negative} == sign(x) == sign(y).
ComparisonResult AbsoluteLess(bool both_negative) { return both_negative ? ComparisonResult::kGreaterThan : ComparisonResult::kLessThan; }
...
MaybeHandle<BigInt> BigInt::BitwiseAnd(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    bool x_sign = x->sign();
    bool y_sign = y->sign();
    Handle<MutableBigInt> result;
    if (!x_sign && !y_sign) {
        int result_length = bigint::BitwiseAnd_PosPos_ResultLength(x->length(), y->length());
        ...
        int result_length = bigint::BitwiseAnd_NegNeg_ResultLength(x->length(), y->length());
        if (!MutableBigInt::New(isolate, result_length).ToHandle(&result)) {
            return {};
        }
...
MaybeHandle<BigInt> BigInt::BitwiseXor(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    bool x_sign = x->sign();
    bool y_sign = y->sign();
    Handle<MutableBigInt> result;
    if (!x_sign && !y_sign) {
        int result_length = bigint::BitwiseXor_PosPos_ResultLength(x->length(), y->length());
        result = MutableBigInt::New(isolate, result_length).ToHandleChecked();
        bigint::BitwiseXor_PosPos(GetRWDigits(result), GetDigits(x), GetDigits(y));
        DCHECK(!result->sign());
    } else if (x_sign && y_sign) {
        int result_length = bigint::BitwiseXor_NegNeg_ResultLength(x->length(), y->length());
        result = MutableBigInt::New(isolate, result_length).ToHandleChecked();
        ...
        if (x_sign)
            std::swap(x, y);
        int result_length = bigint::BitwiseXor_PosNeg_ResultLength(x->length(), y->length());
        if (!MutableBigInt::New(isolate, result_length).ToHandle(&result)) {
            return {};
        }
...
MaybeHandle<BigInt> BigInt::BitwiseOr(Isolate* isolate, Handle<BigInt> x, Handle<BigInt> y)
{
    bool x_sign = x->sign();
    bool y_sign = y->sign();
    int result_length = bigint::BitwiseOrResultLength(x->length(), y->length());
    Handle<MutableBigInt> result = MutableBigInt::New(isolate, result_length).ToHandleChecked();
    if (!x_sign && !y_sign) {
...
Maybe<ComparisonResult> BigInt::CompareToString(Isolate* isolate, Handle<BigInt> x, Handle<String> y)
{
...
Maybe<bool> BigInt::EqualToString(Isolate* isolate, Handle<BigInt> x, Handle<String> y)
{
...
MaybeHandle<String> BigInt::ToString(Isolate* isolate, Handle<BigInt> bigint, int radix, ShouldThrow should_throw)
{
    if (bigint->is_zero()) {
    ...
    if (bigint->length() == 1 && radix == 10) {
        ...
        chars_allocated = bigint::ToStringResultLength(GetDigits(bigint), radix, sign);
        ...
        bigint::Status status = isolate->bigint_processor()->ToString(characters, &chars_written, GetDigits(bigint), radix, sign);
        if (status == bigint::Status::kInterrupted) {
            AllowGarbageCollection terminating_anyway;
            isolate->TerminateExecution();
            return {};
...
MaybeHandle<BigInt> BigInt::FromNumber(Isolate* isolate, Handle<Object> number)
{
    ...
    if (!std::isfinite(value) || (DoubleToInteger(value) != value)) {
        THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kBigIntFromNumber, number), BigInt);
    }
...
{
    if (obj->IsJSReceiver()) {
        ASSIGN_RETURN_ON_EXCEPTION(isolate, obj, JSReceiver::ToPrimitive(isolate, Handle<JSReceiver>::cast(obj), ToPrimitiveHint::kNumber), BigInt);
    }
    ...
    if (obj->IsString()) {
        ...
        if (!StringToBigInt(isolate, Handle<String>::cast(obj)).ToHandle(&n)) {
            if (isolate->has_pending_exception()) {
                ...
                THROW_NEW_ERROR(isolate, NewSyntaxError(MessageTemplate::kBigIntFromObject, str), BigInt);
            }
        }
        return n;
    }

    THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kBigIntFromObject, obj), BigInt);
}
...
MutableBigInt::Rounding MutableBigInt::DecideRounding(Handle<BigIntBase> x, int mantissa_bits_unset, int digit_index, uint64_t current_digit)
{
...
// modification.
MaybeHandle<MutableBigInt> MutableBigInt::AbsoluteAddOne(Isolate* isolate, Handle<BigIntBase> x, bool sign, MutableBigInt result_storage)
{
...
Handle<MutableBigInt> MutableBigInt::AbsoluteSubOne(Isolate* isolate, Handle<BigIntBase> x)
{
    DCHECK(!x->is_zero());
    int length = x->length();
    Handle<MutableBigInt> result = New(isolate, length).ToHandleChecked();
...
MaybeHandle<BigInt> MutableBigInt::LeftShiftByAbsolute(Isolate* isolate, Handle<BigIntBase> x, Handle<BigIntBase> y)
{
    ...
    const int result_length = bigint::LeftShift_ResultLength(x->length(), x->digit(x->length() - 1), shift);
...
Handle<BigInt> MutableBigInt::RightShiftByAbsolute(Isolate* isolate, Handle<BigIntBase> x, Handle<BigIntBase> y)
{
    ...
    const int result_length = bigint::RightShift_ResultLength(GetDigits(x), sign, shift, &state);
...
template <typename IsolateT> MaybeHandle<BigInt> BigInt::Allocate(IsolateT* isolate, bigint::FromStringAccumulator* accumulator, bool negative, AllocationType allocation)
{
    ...
    bigint::Status status = isolate->bigint_processor()->FromString(GetRWDigits(result), accumulator);
    if (status == bigint::Status::kInterrupted) {
        Terminate(isolate);
        return {};
    }
...
int BigInt::DigitsByteLengthForBitfield(uint32_t bitfield) { return LengthBits::decode(bitfield); }
...
Handle<BigInt> BigInt::AsIntN(Isolate* isolate, uint64_t n, Handle<BigInt> x)
{
    ...
    int needed_length = bigint::AsIntNResultLength(GetDigits(x), x->sign(), static_cast<int>(n));
    if (needed_length == -1)
        return x;
    Handle<MutableBigInt> result = MutableBigInt::New(isolate, needed_length).ToHandleChecked();
    bool negative = bigint::AsIntN(GetRWDigits(result), GetDigits(x), x->sign(), static_cast<int>(n));
    result->set_sign(negative);
    return MutableBigInt::MakeImmutable(result);
}
...
MaybeHandle<BigInt> BigInt::AsUintN(Isolate* isolate, uint64_t n, Handle<BigInt> x)
{
    ...
    if (x->sign()) {
        ...
        if (n >= kMaxLengthBits)
            return x;
        int result_length = bigint::AsUintN_Pos_ResultLength(GetDigits(x), static_cast<int>(n));
        if (result_length < 0)
            return x;
        result = MutableBigInt::New(isolate, result_length).ToHandleChecked();
...
MaybeHandle<BigInt> BigInt::FromWords64(Isolate* isolate, int sign_bit, int words64_count, const uint64_t* words)
{
...
void BigInt::ToWordsArray64(int* sign_bit, int* words64_count, uint64_t* words)
{
...
int32_t MutableBigInt_AbsoluteDivAndCanonicalize(Address result_addr, Address x_addr, Address y_addr)
{
    ...
    BigInt y = BigInt::cast(Object(y_addr));
    MutableBigInt result = MutableBigInt::cast(Object(result_addr));
    DCHECK_GE(result.length(), bigint::DivideResultLength(GetDigits(x), GetDigits(y)));
    ...
    bigint::Status status = isolate->bigint_processor()->Divide(GetRWDigits(result), GetDigits(x), GetDigits(y));
    if (status == bigint::Status::kInterrupted) {
        return 1;
    }
