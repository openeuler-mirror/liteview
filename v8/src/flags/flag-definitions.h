...
#define FLAG_FULL(ftype, ctype, nam, def, cmt) inline auto& FLAG_##nam = v8_flags.nam;
#define FLAG_READONLY(ftype, ctype, nam, def, cmt) inline auto constexpr& FLAG_##nam = v8_flags.nam;
...
#define FLAG_ALIAS(ftype, ctype, alias, nam) { Flag::TYPE_##ftype, #alias, &v8_flags.nam, &FLAGDEFAULT_##nam, "alias for --" #nam, false }, // NOLINT(whitespace/indent)

// We produce the code to set flags when it is implied by another flag.
#elif defined(FLAG_MODE_DEFINE_IMPLICATIONS)
#define DEFINE_VALUE_IMPLICATION(whenflag, thenflag, value) changed |= TriggerImplication(v8_flags.whenflag, #whenflag, &v8_flags.thenflag, value, false);

// A weak implication will be overwritten by a normal implication or by an
// explicit flag.
#define DEFINE_WEAK_VALUE_IMPLICATION(whenflag, thenflag, value) changed |= TriggerImplication(v8_flags.whenflag, #whenflag, &v8_flags.thenflag, value, true);

#define DEFINE_GENERIC_IMPLICATION(whenflag, statement)                                                                                                                            \
    if (v8_flags.whenflag)                                                                                                                                                         \
        statement;

#define DEFINE_NEG_VALUE_IMPLICATION(whenflag, thenflag, value) changed |= TriggerImplication(!v8_flags.whenflag, "!" #whenflag, &v8_flags.thenflag, value, false);
...
//  "armv8":       ARMv8 (including all of the above)
    ...
    || (defined(CAN_USE_ARMV8_INSTRUCTIONS) && defined(CAN_USE_ARMV7_INSTRUCTIONS) && defined(CAN_USE_SUDIV) && defined(CAN_USE_NEON) && defined(CAN_USE_VFP3_INSTRUCTIONS))
...
#define DEFINE_MAYBE_BOOL(nam, cmt) FLAG(MAYBE_BOOL, base::Optional<bool>, nam, base::nullopt, cmt)
...
#define HARMONY_STAGED_BASE(V)                                                                                                                                                     \
    V(harmony_rab_gsab, "harmony ResizableArrayBuffer / GrowableSharedArrayBuffer")                                                                                                \
...
#define FLAG_INPROGRESS_FEATURES(id, description) DEFINE_BOOL(id, false, "enable " #description " (in progress)")
...
DEFINE_BOOL(lite_mode, V8_LITE_BOOL, "enables trade-off of performance for memory savings")
...
DEFINE_NEG_IMPLICATION(enable_third_party_heap, parallel_compile_tasks_for_eager_toplevel)
...
DEFINE_BOOL_READONLY(enable_third_party_heap, V8_ENABLE_THIRD_PARTY_HEAP_BOOL, "Use third-party heap")
...
#endif
DEFINE_BOOL_READONLY(inner_pointer_resolution_osb, V8_ENABLE_INNER_POINTER_RESOLUTION_OSB_BOOL, "use object start bitmap for IPR")
...
#endif
DEFINE_BOOL_READONLY(inner_pointer_resolution_mb, V8_ENABLE_INNER_POINTER_RESOLUTION_MB_BOOL, "use marking bitmap for IPR")
...
DEFINE_BOOL(maglev_inlining, false, "enable inlining in the maglev optimizing compiler")
...
DEFINE_WEAK_IMPLICATION(future, merge_background_deserialized_script_with_compilation_cache)
...
// Flags for jitless
DEFINE_BOOL(jitless, V8_LITE_BOOL, "Disable runtime allocation of executable memory.")
...
DEFINE_INT(page_promotion_threshold, 70, "min percentage of live bytes on a page to enable fast evacuation")
...
DEFINE_BOOL(feedback_normalization, false, "feed back normalization to constructors")
...
DEFINE_INT(interrupt_budget_for_maglev, 40 * KB, "interrupt budget which should be used for the profiler counter")

// Tiering: Turbofan.
DEFINE_INT(interrupt_budget, 66 * KB, "interrupt budget which should be used for the profiler counter")
...
DEFINE_INT(bytecode_size_allowance_per_tick, 150,
    ...
    "bytecode.length/X")
DEFINE_INT(max_bytecode_size_for_early_opt, 81, "Maximum bytecode length for a function to be optimized on the first tick")
...
#if defined(V8_OS_DARWIN) && defined(V8_HOST_ARCH_ARM64) && !V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT
// M1 requires W^X.
DEFINE_BOOL_READONLY(concurrent_sparkplug, false, "compile Sparkplug code in a background thread")
#else
DEFINE_BOOL(concurrent_sparkplug, false, "compile Sparkplug code in a background thread")
...
#endif
DEFINE_UINT(concurrent_sparkplug_max_threads, 0, "max number of threads that concurrent Sparkplug can use (0 for unbounded)")
...
DEFINE_BOOL(trace_baseline_concurrent_compilation, false, "trace baseline concurrent compilation")
#undef FLAG
#define FLAG FLAG_FULL
...
DEFINE_BOOL(always_use_string_forwarding_table, false, "use string forwarding table instead of thin strings for all strings")
...
DEFINE_BOOL(write_code_using_rwx, true, "flip permissions to rwx to write page instead of rw")
...
DEFINE_WEAK_VALUE_IMPLICATION(stress_concurrent_inlining, interrupt_budget, 15 * KB)
...
DEFINE_BOOL(trace_heap_broker_memory, false, "trace the heap broker memory (refs analysis and zone numbers)")
...
DEFINE_BOOL(turbo_sp_frame_access, false, "use stack pointer-relative access to frame wherever possible")
...
#ifdef V8_USE_ZLIB
DEFINE_BOOL(turbo_compress_translation_arrays, false, "compress translation arrays (experimental)")
#else
DEFINE_BOOL_READONLY(turbo_compress_translation_arrays, false, "compress translation arrays (experimental)")
#endif // V8_USE_ZLIB
DEFINE_BOOL(turbo_inline_js_wasm_calls, true, "inline JS->Wasm calls")
DEFINE_BOOL(turbo_use_mid_tier_regalloc_for_huge_functions, true, "fall back to the mid-tier register allocator for huge functions")
DEFINE_BOOL(turbo_force_mid_tier_regalloc, false, "always use the mid-tier register allocator (for testing)")

DEFINE_BOOL(turbo_optimize_apply, true, "optimize Function.prototype.apply")

DEFINE_BOOL(turbo_collect_feedback_in_generic_lowering, true, "enable experimental feedback collection in generic lowering.")
DEFINE_BOOL(isolate_script_cache_ageing, true, "enable ageing of the isolate script cache.")
...
DEFINE_BOOL(turboshaft_wasm, false, "enable TurboFan's Turboshaft phases for wasm")
#ifdef DEBUG
...
DEFINE_BOOL(wasm_write_protect_code_memory, true, "write protect code memory on the wasm native heap with mprotect")
...
DEFINE_BOOL(wasm_native_module_cache_enabled, true, "enable the native module cache")
...
DEFINE_UINT(wasm_max_code_space, kMaxWasmCodeMB, "maximum committed code space for wasm (in MB)")
...
DEFINE_INT(wasm_tiering_budget, 1800000, "budget for dynamic tiering (rough approximation of bytes executed")
DEFINE_INT(wasm_caching_threshold, 1000000, "the amount of wasm top tier code that triggers the next caching event")
...
DEFINE_DEBUG_BOOL(trace_wasm_interpreter, false, "trace interpretation of wasm code")
...
DEFINE_INT(wasm_tier_mask_for_testing, 0, "bitmask of functions to compile with TurboFan instead of Liftoff")
...
#define DECL_WASM_FLAG(feat, desc, val) DEFINE_BOOL(experimental_wasm_##feat, val, "enable prototype " desc " for wasm")
...
DEFINE_IMPLICATION(experimental_wasm_stack_switching, experimental_wasm_type_reflection)
...
DEFINE_BOOL(wasm_inlining, false, "enable inlining of wasm functions into wasm functions (experimental)")
DEFINE_SIZE_T(wasm_inlining_budget, 9000, "maximum graph size (in TF nodes) that allows inlining more")
DEFINE_BOOL(wasm_speculative_inlining, false, "enable speculative inlining of call_ref targets (experimental)")
DEFINE_BOOL(trace_wasm_inlining, false, "trace wasm inlining")
DEFINE_BOOL(trace_wasm_speculative_inlining, false, "trace wasm speculative inlining")
...
DEFINE_BOOL(asm_wasm_lazy_compilation, false, "enable lazy compilation for asm-wasm modules")
DEFINE_IMPLICATION(validate_asm, asm_wasm_lazy_compilation)
DEFINE_BOOL(wasm_lazy_compilation, false, "enable lazy compilation for all wasm modules")
DEFINE_WEAK_IMPLICATION(future, wasm_lazy_compilation)
// Write protect code causes too much overhead for lazy compilation.
DEFINE_WEAK_NEG_IMPLICATION(wasm_lazy_compilation, wasm_write_protect_code_memory)
DEFINE_DEBUG_BOOL(trace_wasm_lazy_compilation, false, "trace lazy compilation of wasm functions")
DEFINE_BOOL(wasm_lazy_validation, false, "enable lazy validation for lazily compiled wasm functions")
...
DEFINE_BOOL(global_gc_scheduling, true, "enable GC scheduling based on global memory")
DEFINE_BOOL(gc_global, false, "always perform global GCs")
DEFINE_BOOL(shared_space, false, "Implement shared heap as shared space on a main isolate.")
...
DEFINE_BOOL(trace_idle_notification, false, "print one trace line following each idle notification")
DEFINE_BOOL(trace_idle_notification_verbose, false, "prints the heap state used by the idle notification")
...
DEFINE_BOOL(trace_allocations_origins, false,
...
DEFINE_BOOL(minor_mc_trace_fragmentation, false, "trace fragmentation after marking")
...
DEFINE_BOOL(incremental_marking_wrappers, true, "use incremental marking for marking wrappers")
...
DEFINE_INT(minor_mc_task_trigger, 80, "minormc task trigger in percent of the current heap limit")
DEFINE_BOOL(parallel_scavenge, true, "parallel scavenge")
DEFINE_BOOL(scavenge_task, true, "schedule scavenge tasks")
DEFINE_INT(scavenge_task_trigger, 80, "scavenge task trigger in percent of the current heap limit")
DEFINE_BOOL(scavenge_separate_stack_scanning, false, "use a separate phase for stack scanning in scavenge")
DEFINE_BOOL(trace_parallel_scavenge, false, "trace parallel scavenge")
DEFINE_BOOL(cppgc_young_generation, false, "run young generation garbage collections in Oilpan")
...
#endif
DEFINE_BOOL(concurrent_marking, V8_CONCURRENT_MARKING_BOOL, "use concurrent marking")
...
DEFINE_BOOL(parallel_marking, V8_CONCURRENT_MARKING_BOOL, "use parallel marking in atomic pause")
...
DEFINE_BOOL(track_retaining_path, false, "enable support for tracking retaining path")
...
DEFINE_BOOL(compact_maps, true, "Perform compaction on maps on full collections.")
...
DEFINE_BOOL(crash_on_aborted_evacuation, false, "crash when evacuation of page fails")
...
DEFINE_BOOL(merge_background_deserialized_script_with_compilation_cache, false,
...
DEFINE_BOOL(disable_old_api_accessors, false,
    ...
    "prototype chain")
DEFINE_BOOL(embedder_instance_types, false, "enable type checks based on instance types provided by the embedder")
...
DEFINE_BOOL(deopt_to_baseline, ENABLE_SPARKPLUG, "deoptimize to baseline code when available")
...
DEFINE_BOOL(parallel_compile_tasks_for_eager_toplevel, false, "spawn parallel compile tasks for eagerly compiled, top-level functions")
DEFINE_IMPLICATION(parallel_compile_tasks_for_eager_toplevel, lazy_compile_dispatcher)
DEFINE_BOOL(parallel_compile_tasks_for_lazy, false, "spawn parallel compile tasks for all lazily compiled functions")
...
DEFINE_BOOL(experimental_async_stack_tagging_api, true, "enable experimental async stacks tagging API")
DEFINE_BOOL(experimental_value_unavailable, false, "enable experimental <value unavailable> in scopes")
DEFINE_BOOL(experimental_reuse_locals_blocklists, false, "enable reuse of local blocklists across multiple debug-evaluates")

DEFINE_BOOL(live_edit_top_frame, true, "enable support for live-editing the top-most function on the stack")
DEFINE_BOOL(experimental_remove_internal_scopes_property, true, "don't report the artificial [[Scopes]] property for functions")
...
DEFINE_BOOL(clear_exceptions_on_js_entry, false, "clear pending exceptions when entering JavaScript")
...
DEFINE_INT(heap_snapshot_string_limit, 1024, "truncate strings to this length in the heap snapshot")
...
// v8.cc
DEFINE_BOOL(use_idle_notification, true, "Use idle notification to reduce memory footprint.")
// ic.cc
...
#if defined(V8_TARGET_ARCH_ARM64) || defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_RISCV64) || defined(V8_TARGET_ARCH_LOONG64)
...
DEFINE_BOOL(rehash_snapshot, true, "rehash strings from the snapshot to override the baked-in seed")
...
//
DEFINE_BOOL(trace_minor_mc_parallel_marking, false, "trace parallel marking for the young generation")
DEFINE_BOOL(minor_mc, false, "perform young generation mark compact GCs")
DEFINE_IMPLICATION(minor_mc, separate_gc_phases)

DEFINE_BOOL(concurrent_minor_mc_marking, false, "perform young generation marking concurrently")
DEFINE_NEG_NEG_IMPLICATION(concurrent_marking, concurrent_minor_mc_marking)

DEFINE_BOOL(concurrent_minor_mc_sweeping, false, "perform young generation sweeping concurrently")
