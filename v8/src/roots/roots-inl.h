...
template <typename T> bool RootsTable::IsRootHandle(Handle<T> handle, RootIndex* index) const
{
...
#define ROOT_ACCESSOR(Type, name, CamelName)                                                                                                                                       \
    ...
    Type ReadOnlyRoots::unchecked_##name() const { return Type::unchecked_cast(Object(*GetLocation(RootIndex::k##CamelName))); }                                                   \
...
Address ReadOnlyRoots::first_name_for_protector() const { return at(RootIndex::kFirstNameForProtector); }

Address ReadOnlyRoots::last_name_for_protector() const { return at(RootIndex::kLastNameForProtector); }

bool ReadOnlyRoots::IsNameForProtector(HeapObject object) const { return base::IsInRange(object.ptr(), first_name_for_protector(), last_name_for_protector()); }
...
void ReadOnlyRoots::VerifyNameForProtectorsPages() const
{
    ...
    CHECK_EQ(Page::FromAddress(first_name_for_protector()), Page::FromAddress(last_name_for_protector()));
}

Address ReadOnlyRoots::at(RootIndex root_index) const { return *GetLocation(root_index); }
