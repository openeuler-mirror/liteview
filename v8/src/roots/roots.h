...
#define STRONG_READ_ONLY_ROOT_LIST(V)                                                                                                                                              \
    ...
    V(Map, side_effect_free_call_handler_info_map, SideEffectFreeCallHandlerInfoMap)                                                                                               \
    V(Map, next_call_side_effect_free_call_handler_info_map, NextCallSideEffectFreeCallHandlerInfoMap)                                                                             \
    ...
    V(Map, external_one_byte_internalized_string_map, ExternalOneByteInternalizedStringMap)                                                                                        \
    V(Map, uncached_external_internalized_string_map, UncachedExternalInternalizedStringMap)                                                                                       \
    V(Map, uncached_external_one_byte_internalized_string_map, UncachedExternalOneByteInternalizedStringMap)                                                                       \
    ...
    V(ByteArray, off_heap_trampoline_relocation_info, OffHeapTrampolineRelocationInfo)                                                                                             \
    V(HeapObject, trampoline_trivial_code_data_container, TrampolineTrivialCodeDataContainer)                                                                                      \
    V(HeapObject, trampoline_promise_rejection_code_data_container, TrampolinePromiseRejectionCodeDataContainer)                                                                   \
...
#define STRONG_MUTABLE_IMMOVABLE_ROOT_LIST(V)                                                                                                                                      \
    ...
    V(SharedFunctionInfo, async_function_await_reject_shared_fun, AsyncFunctionAwaitRejectSharedFun)                                                                               \
    V(SharedFunctionInfo, async_function_await_resolve_shared_fun, AsyncFunctionAwaitResolveSharedFun)                                                                             \
    V(SharedFunctionInfo, async_generator_await_reject_shared_fun, AsyncGeneratorAwaitRejectSharedFun)                                                                             \
    V(SharedFunctionInfo, async_generator_await_resolve_shared_fun, AsyncGeneratorAwaitResolveSharedFun)                                                                           \
    V(SharedFunctionInfo, async_generator_yield_with_await_resolve_shared_fun, AsyncGeneratorYieldWithAwaitResolveSharedFun)                                                       \
    V(SharedFunctionInfo, async_generator_return_resolve_shared_fun, AsyncGeneratorReturnResolveSharedFun)                                                                         \
    V(SharedFunctionInfo, async_generator_return_closed_reject_shared_fun, AsyncGeneratorReturnClosedRejectSharedFun)                                                              \
    V(SharedFunctionInfo, async_generator_return_closed_resolve_shared_fun, AsyncGeneratorReturnClosedResolveSharedFun)                                                            \
    V(SharedFunctionInfo, async_iterator_value_unwrap_shared_fun, AsyncIteratorValueUnwrapSharedFun)                                                                               \
    V(SharedFunctionInfo, promise_all_resolve_element_shared_fun, PromiseAllResolveElementSharedFun)                                                                               \
    V(SharedFunctionInfo, promise_all_settled_resolve_element_shared_fun, PromiseAllSettledResolveElementSharedFun)                                                                \
    V(SharedFunctionInfo, promise_all_settled_reject_element_shared_fun, PromiseAllSettledRejectElementSharedFun)                                                                  \
    V(SharedFunctionInfo, promise_any_reject_element_shared_fun, PromiseAnyRejectElementSharedFun)                                                                                 \
    V(SharedFunctionInfo, promise_capability_default_reject_shared_fun, PromiseCapabilityDefaultRejectSharedFun)                                                                   \
    V(SharedFunctionInfo, promise_capability_default_resolve_shared_fun, PromiseCapabilityDefaultResolveSharedFun)                                                                 \
    V(SharedFunctionInfo, promise_catch_finally_shared_fun, PromiseCatchFinallySharedFun)                                                                                          \
    V(SharedFunctionInfo, promise_get_capabilities_executor_shared_fun, PromiseGetCapabilitiesExecutorSharedFun)                                                                   \
    V(SharedFunctionInfo, promise_then_finally_shared_fun, PromiseThenFinallySharedFun)                                                                                            \
    V(SharedFunctionInfo, promise_thrower_finally_shared_fun, PromiseThrowerFinallySharedFun)                                                                                      \
    V(SharedFunctionInfo, promise_value_thunk_finally_shared_fun, PromiseValueThunkFinallySharedFun)                                                                               \
    V(SharedFunctionInfo, proxy_revoke_shared_fun, ProxyRevokeSharedFun)                                                                                                           \
    V(SharedFunctionInfo, shadow_realm_import_value_fulfilled_sfi, ShadowRealmImportValueFulfilledSFI)                                                                             \
...
#define INTERNALIZED_STRING_ROOT_LIST(V) INTERNALIZED_STRING_LIST_GENERATOR(INTERNALIZED_STRING_LIST_ADAPTER, V)
...
public:
    ...
    template <typename T> bool IsRootHandle(Handle<T> handle, RootIndex* index) const;
    ...
    static constexpr int offset_of(RootIndex root_index) { return static_cast<int>(root_index) * kSystemPointerSize; }
...
private:
    FullObjectSlot begin() { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kFirstRoot)]); }
    FullObjectSlot end() { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kLastRoot) + 1]); }
    ...
    FullObjectSlot strong_or_read_only_roots_end() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kLastStrongOrReadOnlyRoot) + 1]); }

    // The read-only, strong and Smi roots as defined by these accessors are all
    // disjoint.
    FullObjectSlot read_only_roots_begin() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kFirstReadOnlyRoot)]); }
    FullObjectSlot read_only_roots_end() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kLastReadOnlyRoot) + 1]); }

    FullObjectSlot strong_roots_begin() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kFirstStrongRoot)]); }
    FullObjectSlot strong_roots_end() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kLastStrongRoot) + 1]); }

    FullObjectSlot smi_roots_begin() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kFirstSmiRoot)]); }
    FullObjectSlot smi_roots_end() const { return FullObjectSlot(&roots_[static_cast<size_t>(RootIndex::kLastSmiRoot) + 1]); }
