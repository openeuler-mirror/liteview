...
public:
    ...
    int ContextChainDepth(Scope* scope) { return scope_->ContextChainLength(scope); }
...
public:
    ...
    void Break(Statement* stmt) { PerformCommand(CMD_BREAK, stmt, kNoSourcePosition); }
    void Continue(Statement* stmt) { PerformCommand(CMD_CONTINUE, stmt, kNoSourcePosition); }
    void ReturnAccumulator(int source_position) { PerformCommand(CMD_RETURN, nullptr, source_position); }
    void AsyncReturnAccumulator(int source_position) { PerformCommand(CMD_ASYNC_RETURN, nullptr, source_position); }

    class DeferredCommands;

protected:
    enum Command { CMD_BREAK, CMD_CONTINUE, CMD_RETURN, CMD_ASYNC_RETURN, CMD_RETHROW };
    static constexpr bool CommandUsesAccumulator(Command command) { return command != CMD_BREAK && command != CMD_CONTINUE; }
...
public:
    ...
    void ApplyDeferredCommands()
    {
        ...
        if (deferred_.size() == 1) {
            ...
            builder()->LoadLiteral(Smi::FromInt(entry.token)).CompareReference(token_register_).JumpIfFalse(ToBooleanMode::kAlreadyBoolean, &fall_through);
            ...
            builder()->LoadAccumulatorWithRegister(token_register_).SwitchOnSmiNoFeedback(jump_table).Jump(&fall_through);
...
public:
    ...
    ~RegisterAllocationScope() { generator_->register_allocator()->ReleaseRegisters(outer_next_register_index_); }
...
public:
    ...
    ~ExpressionResultScope() { allocator_.generator()->set_execution_result(outer_); }
...
public:
    ...
    void set_then_labels(BytecodeLabels* then_labels) { then_labels_ = then_labels; }
    void set_else_labels(BytecodeLabels* else_labels) { else_labels_ = else_labels; }
    ...
    void set_fallthrough(TestFallthrough fallthrough) { fallthrough_ = fallthrough; }
...
public:
    template <typename IsolateT> Handle<FixedArray> AllocateDeclarations(UnoptimizedCompilationInfo* info, BytecodeGenerator* generator, Handle<Script> script, IsolateT* isolate)
    {
        ...
        if (info->scope()->is_module_scope()) {
            for (Declaration* decl : *info->scope()->declarations()) {
                ...
                if (decl->IsFunctionDeclaration()) {
                    ...
                    Handle<SharedFunctionInfo> sfi(Compiler::GetSharedFunctionInfo(f, script, isolate));
    ...
    void record_global_variable_declaration() { entry_slots_ += kGlobalVariableDeclarationSize; }
    void record_global_function_declaration() { entry_slots_ += kGlobalFunctionDeclarationSize; }
    void record_module_variable_declaration() { entry_slots_ += kModuleVariableDeclarationSize; }
    void record_module_function_declaration() { entry_slots_ += kModuleFunctionDeclarationSize; }
...
public:
    ...
    void Put(SlotKind slot_kind, Variable* variable, int slot_index) { PutImpl(slot_kind, 0, variable, slot_index); }
    void Put(SlotKind slot_kind, AstNode* node, int slot_index) { PutImpl(slot_kind, 0, node, slot_index); }
    void Put(SlotKind slot_kind, int variable_index, const AstRawString* name, int slot_index) { PutImpl(slot_kind, variable_index, name, slot_index); }
    void Put(SlotKind slot_kind, const AstRawString* name, int slot_index) { PutImpl(slot_kind, 0, name, slot_index); }

    int Get(SlotKind slot_kind, Variable* variable) const { return GetImpl(slot_kind, 0, variable); }
    int Get(SlotKind slot_kind, AstNode* node) const { return GetImpl(slot_kind, 0, node); }
    int Get(SlotKind slot_kind, int variable_index, const AstRawString* name) const { return GetImpl(slot_kind, variable_index, name); }
    int Get(SlotKind slot_kind, const AstRawString* name) const { return GetImpl(slot_kind, 0, name); }
...
public:
    ...
    ~OptionalChainNullLabelScope() { bytecode_generator_->optional_chaining_null_labels_ = prev_; }
...
public:
    ...
    const std::vector<std::pair<Literal*, Accessors<PropertyT>*>>& ordered_accessors() { return ordered_accessors_; }
...
BytecodeGenerator::BytecodeGenerator(LocalIsolate* local_isolate, Zone* compile_zone, UnoptimizedCompilationInfo* info, const AstStringConstants* ast_string_constants,
    std::vector<FunctionLiteral*>* eager_inner_literals, Handle<Script> script)
    : local_isolate_(local_isolate)
    , zone_(compile_zone)
    , builder_(zone(), info->num_parameters_including_this(), info->scope()->num_stack_slots(), info->feedback_vector_spec(), info->SourcePositionRecordingMode())
    , info_(info)
...
template <typename IsolateT> Handle<ByteArray> BytecodeGenerator::FinalizeSourcePositionTable(IsolateT* isolate)
{
    DCHECK_EQ(ThreadId::Current(), isolate->thread_id());
#ifdef DEBUG
    // Unoptimized compilation should be context-independent. Verify that we don't
    ...
    Handle<ByteArray> source_position_table = builder()->ToSourcePositionTable(isolate);
...
template Handle<ByteArray> BytecodeGenerator::FinalizeSourcePositionTable(Isolate* isolate);
template Handle<ByteArray> BytecodeGenerator::FinalizeSourcePositionTable(LocalIsolate* isolate);

#ifdef DEBUG
int BytecodeGenerator::CheckBytecodeMatches(BytecodeArray bytecode) { return builder()->CheckBytecodeMatches(bytecode); }
#endif
...
namespace {
bool NeedsContextInitialization(DeclarationScope* scope) { return scope->NeedsContext() && !scope->is_script_scope() && !scope->is_module_scope(); }
} // namespace
...
void BytecodeGenerator::VisitVariableDeclaration(VariableDeclaration* decl)
{
    ...
    case VariableLocation::CONTEXT:
        if (variable->binding_needs_init()) {
            ...
            builder()->LoadTheHole().StoreContextSlot(execution_context()->reg(), variable->index(), 0);
        }
        break;
...
void BytecodeGenerator::VisitFunctionDeclaration(FunctionDeclaration* decl)
{
    ...
    case VariableLocation::CONTEXT: {
        DCHECK_EQ(0, execution_context()->ContextChainDepth(variable->scope()));
        VisitFunctionLiteral(decl->fun());
        builder()->StoreContextSlot(execution_context()->reg(), variable->index(), 0);
        break;
    }
    case VariableLocation::LOOKUP: {
...
void BytecodeGenerator::VisitModuleNamespaceImports()
{
    ...
    for (auto entry : descriptor->namespace_imports()) {
        builder()->LoadLiteral(Smi::FromInt(entry->module_request)).StoreAccumulatorInRegister(module_request).CallRuntime(Runtime::kGetModuleNamespace, module_request);
...
void BytecodeGenerator::VisitStatements(const ZonePtrList<Statement>* statements)
{
...
void BytecodeGenerator::VisitSloppyBlockFunctionStatement(SloppyBlockFunctionStatement* stmt) { Visit(stmt->statement()); }
...
inline bool IsSpreadAcceptable(int spread, int ncases) { return spread < v8_flags.switch_table_spread_threshold * ncases; }
...
struct SwitchInfo {
    ...
    bool CaseExists(int j) { return covered_cases.find(j) != covered_cases.end(); }
    bool CaseExists(Expression* expr) { return IsSmiLiteralSwitchCaseValue(expr) ? CaseExists(ReduceToSmiSwitchCaseValue(expr)) : false; }
    ...
    int MinCase() { return covered_cases.size() == 0 ? INT_MAX : covered_cases.begin()->first; }
    int MaxCase() { return covered_cases.size() == 0 ? INT_MIN : covered_cases.rbegin()->first; }
    void Print()
    {
...
void BytecodeGenerator::VisitSwitchStatement(SwitchStatement* stmt)
{
    ...
    if (use_jump_table) {
        ...
        builder()->CompareOperation(Token::Value::GTE, r1, feedback_index(feedback_spec()->AddCompareICSlot()));
        ...
        builder()->StoreAccumulatorInRegister(r2);
        builder()->CompareOperation(Token::Value::LTE, r1, feedback_index(feedback_spec()->AddCompareICSlot()));

        switch_builder.JumpToFallThroughIfFalse();
        builder()->LoadAccumulatorWithRegister(r1);

        builder()->BinaryOperationSmiLiteral(Token::Value::BIT_OR, Smi::FromInt(0), feedback_index(feedback_spec()->AddBinaryOpICSlot()));

        builder()->StoreAccumulatorInRegister(r2);
        builder()->CompareOperation(Token::Value::EQ_STRICT, r1, feedback_index(feedback_spec()->AddCompareICSlot()));
    ...
    if (use_jumps) {
        ...
        for (int i = 0; i < clauses->length(); ++i) {
            ...
            if (clause->is_default()) {
                ...
                builder()->CompareOperation(Token::Value::EQ_STRICT, tag_holder, feedback_index(slot));
#ifdef DEBUG
    ...
    TryBodyFunc try_body_func, FinallyBodyFunc finally_body_func, HandlerTable::CatchPrediction catch_prediction, TryFinallyStatement* stmt_for_coverage)
{
    ...
    builder()->LoadTheHole().SetPendingMessage().StoreAccumulatorInRegister(message);
...
void BytecodeGenerator::VisitTryFinallyStatement(TryFinallyStatement* stmt)
{
    BuildTryFinally(
        ...
        [&]() { Visit(stmt->try_block()); },
        // Finally block.
        [&](Register body_continuation_token) { Visit(stmt->finally_block()); }, catch_prediction(), stmt);
}
...
void BytecodeGenerator::AddToEagerLiteralsIfEager(FunctionLiteral* literal)
{
    ...
    if (!script_.is_null() && literal->should_parallel_compile()) {
        ...
        if (!Script::FindSharedFunctionInfo(script_, local_isolate_, literal).ToHandle(&shared_info)) {
            shared_info = Compiler::GetSharedFunctionInfo(literal, script_, local_isolate_);
            info()->dispatcher()->Enqueue(local_isolate_, shared_info, info()->character_stream()->Clone());
        }
...
void BytecodeGenerator::BuildClassLiteral(ClassLiteral* expr, Register name)
{
    ...
    if (expr->scope()->brand() != nullptr) {
        ...
        const AstRawString* class_name = expr->scope()->class_variable() != nullptr ? expr->scope()->class_variable()->raw_name() : ast_string_constants()->anonymous_string();
            ...
            if (property->is_computed_name()) {
                ...
                if (property->is_static()) {
                    ...
                    builder()
                        ...
                        .CompareOperation(Token::Value::EQ_STRICT, key, feedback_index(slot))
    ...
    for (auto accessors : private_accessors.ordered_accessors()) {
        ...
        Variable* var = getter != nullptr ? getter->private_name_var() : setter->private_name_var();
        DCHECK_NOT_NULL(var);
        BuildVariableAssignment(var, Token::INIT, HoleCheckMode::kElided);
    }
    ...
    if (expr->instance_members_initializer_function() != nullptr) {
        ...
        builder()->LoadAccumulatorWithRegister(initializer).StoreClassFieldsInitializer(class_constructor, feedback_index(slot)).LoadAccumulatorWithRegister(class_constructor);
    }
    ...
    if (expr->static_initializer() != nullptr) {
        ...
        if (name.is_valid()) {
            ...
            builder()->LoadLiteral(ast_string_constants()->name_string()).StoreAccumulatorInRegister(key);
            ...
            builder()->LoadAccumulatorWithRegister(name).DefineKeyedOwnPropertyInLiteral(class_constructor, key, data_property_flags, feedback_index(slot));
        }
...
void BytecodeGenerator::VisitClassLiteral(ClassLiteral* expr) { VisitClassLiteral(expr, Register::invalid_value()); }
...
void BytecodeGenerator::VisitClassLiteral(ClassLiteral* expr, Register name)
{
    ...
    if (expr->scope()->NeedsContext()) {
        ...
        base::Optional<BytecodeSourceInfo> source_info = builder()->MaybePopSourcePosition(expr->scope()->start_position());
...
void BytecodeGenerator::BuildClassProperty(ClassLiteral::Property* property)
{
    ...
    DCHECK_IMPLIES(property->is_private(), property->kind() == ClassLiteral::Property::FIELD);
    builder()->SetExpressionPosition(property->key());

    bool is_literal_store = property->key()->IsPropertyName() && !property->is_computed_name() && !property->is_private();
    ...
    if (is_literal_store) {
        ...
        builder()->DefineKeyedOwnProperty(builder()->Receiver(), key, feedback_index(slot));
    }
}
...
void BytecodeGenerator::BuildPrivateBrandInitialization(Register receiver, Variable* brand)
{
    ...
    if (class_context) {
        ...
        FeedbackSlot slot = feedback_spec()->AddDefineKeyedOwnICSlot();
        builder()->StoreAccumulatorInRegister(brand_reg).LoadAccumulatorWithRegister(class_context->reg()).DefineKeyedOwnProperty(receiver, brand_reg, feedback_index(slot));
    } else {
...
void BytecodeGenerator::VisitObjectLiteral(ObjectLiteral* expr)
{
        ...
        case ObjectLiteral::Property::COMPUTED: {
            ...
            if (property->emit_store()) {
                ...
                if (key->IsStringLiteral()) {
                    ...
                    FeedbackSlot slot = feedback_spec()->AddDefineKeyedOwnICSlot();
                    builder()->DefineKeyedOwnProperty(literal, key_reg, feedback_index(slot));
                }
            } else {
    ...
    for (; property_index < expr->properties()->length(); property_index++) {
        ...
        case ObjectLiteral::Property::MATERIALIZED_LITERAL: {
            ...
            FeedbackSlot slot = feedback_spec()->AddDefineKeyedOwnPropertyInLiteralICSlot();
            builder()->LoadAccumulatorWithRegister(value).DefineKeyedOwnPropertyInLiteral(literal, key, data_property_flags, feedback_index(slot));
            break;
        }
        case ObjectLiteral::Property::GETTER:
        case ObjectLiteral::Property::SETTER: {
...
void BytecodeGenerator::BuildCreateArrayLiteral(const ZonePtrList<Expression>* elements, ArrayLiteral* expr)
{
    ...
    for (; current != end; ++current) {
        ...
        if (subexpr->IsSpread()) {
            ...
            if (current + 1 != end) {
                builder()->UnaryOperation(Token::INC, feedback_index(index_slot.Get())).StoreAccumulatorInRegister(index);
            }
        } else {
...
void BytecodeGenerator::BuildVariableLoad(Variable* variable, HoleCheckMode hole_check_mode, TypeofMode typeof_mode)
{
    ...
    case VariableLocation::CONTEXT: {
        ...
        builder()->LoadContextSlot(context_reg, variable->index(), depth, immutable);
        if (hole_check_mode == HoleCheckMode::kRequired) {
            BuildThrowIfHole(variable);
        }
    ...
    case VariableLocation::LOOKUP: {
        ...
        case VariableMode::kDynamicLocal: {
            ...
            builder()->LoadLookupContextSlot(variable->raw_name(), typeof_mode, local_variable->index(), depth);
            if (hole_check_mode == HoleCheckMode::kRequired) {
                BuildThrowIfHole(variable);
            }
            break;
...
void BytecodeGenerator::BuildAsyncReturn(int source_position)
{
    ...
    if (IsAsyncGeneratorFunction(info()->literal()->kind())) {
        ...
        DCHECK(IsAsyncFunction(info()->literal()->kind()) || IsAsyncModule(info()->literal()->kind()));
        RegisterList args = register_allocator()->NewRegisterList(2);
        builder()
            ->MoveRegister(generator_object(), args[0]) // generator
            .StoreAccumulatorInRegister(args[1]) // value
...
void BytecodeGenerator::BuildVariableAssignment(Variable* variable, Token::Value op, HoleCheckMode hole_check_mode, LookupHoistingMode lookup_hoisting_mode)
{
    ...
    case VariableLocation::CONTEXT: {
        ...
        if (hole_check_mode == HoleCheckMode::kRequired) {
            // Load destination to check for hole.
            Register value_temp = register_allocator()->NewRegister();
            builder()->StoreAccumulatorInRegister(value_temp).LoadContextSlot(context_reg, variable->index(), depth, BytecodeArrayBuilder::kMutableSlot);
...
// static
BytecodeGenerator::AssignmentLhsData BytecodeGenerator::AssignmentLhsData::PrivateMethodOrAccessor(AssignType type, Property* property, Register object, Register key)
{
...
void BytecodeGenerator::BuildDestructuringObjectAssignment(ObjectLiteral* pattern, Token::Value op, LookupHoistingMode lookup_hoisting_mode)
{
    ...
    if (pattern->properties()->is_empty() || (pattern->properties()->at(0)->is_computed_name() && pattern->properties()->at(0)->kind() != ObjectLiteralProperty::SPREAD)) {
...
void BytecodeGenerator::VisitCompoundAssignment(CompoundAssignment* expr)
{
    ...
    case KEYED_PROPERTY: {
        ...
        builder()->LoadAccumulatorWithRegister(lhs_data.key()).LoadKeyedProperty(lhs_data.object(), feedback_index(slot));
        break;
    }
    case NAMED_SUPER_PROPERTY: {
...
void BytecodeGenerator::BuildAwait(int position)
{
...

    {
        ...
        if (IsAsyncGeneratorFunction(function_kind())) {
            await_intrinsic_id = catch_prediction() == HandlerTable::ASYNC_AWAIT ? Runtime::kInlineAsyncGeneratorAwaitUncaught : Runtime::kInlineAsyncGeneratorAwaitCaught;
        } else {
            await_intrinsic_id = catch_prediction() == HandlerTable::ASYNC_AWAIT ? Runtime::kInlineAsyncFunctionAwaitUncaught : Runtime::kInlineAsyncFunctionAwaitCaught;
        }
        RegisterList args = register_allocator()->NewRegisterList(2);
...
void BytecodeGenerator::VisitPropertyLoad(Register obj, Property* property)
{
    ...
    case KEYED_PROPERTY: {
        ...
        builder()->SetExpressionPosition(property);
        builder()->LoadKeyedProperty(obj, feedback_index(feedback_spec()->AddKeyedLoadICSlot()));
        break;
    }
    case NAMED_SUPER_PROPERTY:
...
void BytecodeGenerator::VisitNamedSuperPropertyLoad(Property* property, Register opt_receiver_out)
{
    ...
    if (v8_flags.super_ic) {
        ...
        builder()->SetExpressionPosition(property);
        builder()->LoadLiteral(property->key()->AsLiteral()->AsRawPropertyName()).StoreAccumulatorInRegister(args[2]).CallRuntime(Runtime::kLoadFromSuper, args);
...
void BytecodeGenerator::VisitCallSuper(Call* expr)
{
    ...
    if (spread_position == Call::kHasNonFinalSpread) {
        ...
        if (omit_super_ctor) {
            BuildSuperCallOptimization(this_function, new_target, constructor_then_instance, &super_ctor_call_done);
        } else {
...
void BytecodeGenerator::BuildSuperCallOptimization(Register this_function, Register new_target, Register constructor_then_instance, BytecodeLabel* super_ctor_call_done)
{
...
void BytecodeGenerator::VisitNaryArithmeticExpression(NaryOperation* expr)
{
    ...
    for (size_t i = 0; i < expr->subsequent_length(); ++i) {
        ...
        if (expr->subsequent(i)->IsSmiLiteral()) {
            ...
            builder()->BinaryOperationSmiLiteral(expr->op(), expr->subsequent(i)->AsLiteral()->AsSmiLiteral(), feedback_index(feedback_spec()->AddBinaryOpICSlot()));
        } else {
            Register lhs = register_allocator()->NewRegister();
            builder()->StoreAccumulatorInRegister(lhs);
...
void BytecodeGenerator::VisitEmptyParentheses(EmptyParentheses* expr) { UNREACHABLE(); }
...
void BytecodeGenerator::VisitImportCallExpression(ImportCallExpression* expr)
{
    ...
    builder()->MoveRegister(Register::function_closure(), args[0]).CallRuntime(Runtime::kDynamicImportCall, args);
}
...
void BytecodeGenerator::BuildIteratorNext(const IteratorRecord& iterator, Register next_result)
{
    ...
    BytecodeLabel is_object;
    builder()->StoreAccumulatorInRegister(next_result).JumpIfJSReceiver(&is_object).CallRuntime(Runtime::kThrowIteratorResultNotAnObject, next_result).Bind(&is_object);
}
...
void BytecodeGenerator::VisitThisExpression(ThisExpression* expr) { BuildThisVariableLoad(); }
...
void BytecodeGenerator::VisitSuperPropertyReference(SuperPropertyReference* expr) { builder()->CallRuntime(Runtime::kThrowUnsupportedSuperError); }
...
void BytecodeGenerator::VisitLogicalTestSubExpression(Token::Value token, Expression* expr, BytecodeLabels* then_labels, BytecodeLabels* else_labels, int coverage_slot)
{
...
void BytecodeGenerator::BuildLocalActivationContextInitialization()
{
    ...
    if (scope->has_this_declaration() && scope->receiver()->IsContextSlot()) {
        ...
        builder()->LoadAccumulatorWithRegister(receiver).StoreContextSlot(execution_context()->reg(), variable->index(), 0);
    }
    ...
    for (int i = 0; i < num_parameters; i++) {
        ...
        // Context variable (at bottom of the context chain).
        DCHECK_EQ(0, scope->ContextChainLength(variable->scope()));
        builder()->LoadAccumulatorWithRegister(parameter).StoreContextSlot(execution_context()->reg(), variable->index(), 0);
    }
}
...
void BytecodeGenerator::BuildGeneratorObjectVariableInitialization()
{
    ...
    RegisterList args = register_allocator()->NewRegisterList(2);
    Runtime::FunctionId function_id
        = ((IsAsyncFunction(info()->literal()->kind()) && !IsAsyncGeneratorFunction(info()->literal()->kind())) || IsAsyncModule(info()->literal()->kind()))
...
{
    return (block_coverage_builder_ == nullptr) ? BlockCoverageBuilder::kNoCoverageArraySlot : block_coverage_builder_->AllocateNaryBlockCoverageSlot(node, index);
}
...
FunctionKind BytecodeGenerator::function_kind() const { return info()->literal()->kind(); }

LanguageMode BytecodeGenerator::language_mode() const { return current_scope()->language_mode(); }
...
FeedbackVectorSpec* BytecodeGenerator::feedback_spec() { return info()->feedback_vector_spec(); }
...
{
    FeedbackSlotCache::SlotKind slot_kind = is_strict(language_mode) ? FeedbackSlotCache::SlotKind::kStoreGlobalStrict : FeedbackSlotCache::SlotKind::kStoreGlobalSloppy;
    FeedbackSlot slot(feedback_slot_cache()->Get(slot_kind, variable));
    if (!slot.IsInvalid()) {
        return slot;
    }
...
FeedbackSlot BytecodeGenerator::GetCachedStoreICSlot(const Expression* expr, const AstRawString* name)
{
    ...
    FeedbackSlotCache::SlotKind slot_kind = is_strict(language_mode()) ? FeedbackSlotCache::SlotKind::kSetNamedStrict : FeedbackSlotCache::SlotKind::kSetNamedSloppy;
    if (!expr->IsVariableProxy()) {
        return feedback_spec()->AddStoreICSlot(language_mode());
    }
    const VariableProxy* proxy = expr->AsVariableProxy();
...
FeedbackSlot BytecodeGenerator::GetDummyCompareICSlot() { return dummy_feedback_slot_.Get(); }
