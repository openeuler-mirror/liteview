...
template <typename IsolateT> Handle<ByteArray> HandlerTableBuilder::ToHandlerTable(IsolateT* isolate)
{
    int handler_table_size = static_cast<int>(entries_.size());
    Handle<ByteArray> table_byte_array = isolate->factory()->NewByteArray(HandlerTable::LengthForRange(handler_table_size), AllocationType::kOld);
...
template Handle<ByteArray> HandlerTableBuilder::ToHandlerTable(Isolate* isolate);
template Handle<ByteArray> HandlerTableBuilder::ToHandlerTable(LocalIsolate* isolate);
...
void HandlerTableBuilder::SetPrediction(int handler_id, HandlerTable::CatchPrediction prediction) { entries_[handler_id].catch_prediction_ = prediction; }

void HandlerTableBuilder::SetContextRegister(int handler_id, Register reg) { entries_[handler_id].context = reg; }
