...
public:
    ...
    template <typename IsolateT> EXPORT_TEMPLATE_DECLARE(V8_EXPORT_PRIVATE) Handle<ByteArray> ToSourcePositionTable(IsolateT* isolate);
    ...
    // in the accumulator.
    BytecodeArrayBuilder& DefineKeyedOwnProperty(Register object, Register key, int feedback_slot);
    ...
    BytecodeRegisterOptimizer* GetRegisterOptimizer() { return register_optimizer_; }
...
#undef ENTRY_GETTER
    ...
    void SetStatementPosition(Statement* stmt) { SetStatementPosition(stmt->position()); }
    ...
    void SetExpressionPosition(Expression* expr) { SetExpressionPosition(expr->position()); }
    ...
    void SetExpressionAsStatementPosition(Expression* expr) { SetStatementPosition(expr->position()); }

    bool RemainderOfBlockIsDead() const { return bytecode_array_writer_.RemainderOfBlockIsDead(); }
    ...
    // Accessors
    BytecodeRegisterAllocator* register_allocator() { return &register_allocator_; }
    const BytecodeRegisterAllocator* register_allocator() const { return &register_allocator_; }
...
private:
    ...
    const FeedbackVectorSpec* feedback_vector_spec() const { return feedback_vector_spec_; }
...
#undef DECLARE_OPERAND_TYPE_INFO
    ...
    BytecodeArrayWriter* bytecode_array_writer() { return &bytecode_array_writer_; }
    ConstantArrayBuilder* constant_array_builder() { return &constant_array_builder_; }
    const ConstantArrayBuilder* constant_array_builder() const { return &constant_array_builder_; }
    HandlerTableBuilder* handler_table_builder() { return &handler_table_builder_; }
