...
public:
    ...
    void Run(JobDelegate* delegate) final { lazy_compile_dispatcher_->DoBackgroundWork(delegate); }
...
// gets a little messy.
void SetUncompiledDataJobPointer(LocalIsolate* isolate, Handle<SharedFunctionInfo> shared_info, Address job_address)
{
    ...
    case UNCOMPILED_DATA_WITH_PREPARSE_DATA_AND_JOB_TYPE:
        UncompiledDataWithPreparseDataAndJob::cast(uncompiled_data).set_job(job_address);
        break;
    case UNCOMPILED_DATA_WITHOUT_PREPARSE_DATA_WITH_JOB_TYPE:
        UncompiledDataWithoutPreparseDataWithJob::cast(uncompiled_data).set_job(job_address);
        break;
    ...
    case UNCOMPILED_DATA_WITH_PREPARSE_DATA_TYPE: {
        ...
        Handle<PreparseData> preparse_data(UncompiledDataWithPreparseData::cast(uncompiled_data).preparse_data(), isolate);
        Handle<UncompiledDataWithPreparseDataAndJob> new_uncompiled_data
            = isolate->factory()->NewUncompiledDataWithPreparseDataAndJob(inferred_name, uncompiled_data.start_position(), uncompiled_data.end_position(), preparse_data);
    ...
    case UNCOMPILED_DATA_WITHOUT_PREPARSE_DATA_TYPE: {
        ...
        Handle<UncompiledDataWithoutPreparseDataWithJob> new_uncompiled_data
            = isolate->factory()->NewUncompiledDataWithoutPreparseDataWithJob(inferred_name, uncompiled_data.start_position(), uncompiled_data.end_position());
...
bool LazyCompileDispatcher::IsEnqueued(Handle<SharedFunctionInfo> function) const
{
    Job* job = nullptr;
    Object function_data = function->function_data(kAcquireLoad);
    if (function_data.IsUncompiledDataWithPreparseDataAndJob()) {
        job = reinterpret_cast<Job*>(UncompiledDataWithPreparseDataAndJob::cast(function_data).job());
    } else if (function_data.IsUncompiledDataWithoutPreparseDataWithJob()) {
        job = reinterpret_cast<Job*>(UncompiledDataWithoutPreparseDataWithJob::cast(function_data).job());
    }
    return job != nullptr;
}
...
void LazyCompileDispatcher::WaitForJobIfRunningOnBackground(Job* job, const base::MutexGuard& lock)
{
    ...
    if (!job->is_running_on_background()) {
        if (job->state == Job::State::kPending) {
            ...
            // vector.
            pending_background_jobs_.erase(std::remove(pending_background_jobs_.begin(), pending_background_jobs_.end(), job));
            ...
            // vector.
            finalizable_jobs_.erase(std::remove(finalizable_jobs_.begin(), finalizable_jobs_.end(), job));
...
LazyCompileDispatcher::Job* LazyCompileDispatcher::GetJobFor(Handle<SharedFunctionInfo> shared, const base::MutexGuard&) const
{
    Object function_data = shared->function_data(kAcquireLoad);
    if (function_data.IsUncompiledDataWithPreparseDataAndJob()) {
        return reinterpret_cast<Job*>(UncompiledDataWithPreparseDataAndJob::cast(function_data).job());
    } else if (function_data.IsUncompiledDataWithoutPreparseDataWithJob()) {
        return reinterpret_cast<Job*>(UncompiledDataWithoutPreparseDataWithJob::cast(function_data).job());
    }
    return nullptr;
}
