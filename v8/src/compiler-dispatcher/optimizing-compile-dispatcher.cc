...
private:
    ...
    void RunInternal() override
    {
...

        {
            ...
            TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.OptimizeBackground", job, TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
...
void OptimizingCompileDispatcher::FlushInputQueue()
{
    ...
    while (input_queue_length_ > 0) {
        std::unique_ptr<TurbofanCompilationJob> job(input_queue_[InputQueueIndex(0)]);
        DCHECK_NOT_NULL(job);
        input_queue_shift_ = InputQueueIndex(1);
        input_queue_length_--;
...
void OptimizingCompileDispatcher::FlushQueues(BlockingBehavior blocking_behavior, bool restore_function_code)
{
    FlushInputQueue();
...
void OptimizingCompileDispatcher::QueueForOptimization(TurbofanCompilationJob* job)
{
    ...
    V8::GetCurrentPlatform()->CallOnWorkerThread(std::make_unique<CompileTask>(isolate_, this));
}
