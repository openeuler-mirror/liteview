...
#undef T
    ...
    // Predicates
    static bool IsKeyword(Value token) { return IsKeywordBits::decode(token_flags[token]); }

    static bool IsPropertyName(Value token) { return IsPropertyNameBits::decode(token_flags[token]); }

    V8_INLINE static bool IsValidIdentifier(Value token, LanguageMode language_mode, bool is_generator, bool disallow_await)
    {
        if (V8_LIKELY(base::IsInRange(token, IDENTIFIER, ASYNC)))
            return true;
        if (token == AWAIT)
    ...
    static bool IsCallable(Value token) { return base::IsInRange(token, SUPER, ESCAPED_STRICT_RESERVED_WORD); }

    static bool IsAutoSemicolon(Value token) { return base::IsInRange(token, SEMICOLON, EOS); }

    static bool IsAnyIdentifier(Value token) { return base::IsInRange(token, IDENTIFIER, ESCAPED_STRICT_RESERVED_WORD); }

    static bool IsStrictReservedWord(Value token) { return base::IsInRange(token, YIELD, ESCAPED_STRICT_RESERVED_WORD); }

    static bool IsLiteral(Value token) { return base::IsInRange(token, NULL_LITERAL, STRING); }

    static bool IsTemplate(Value token) { return base::IsInRange(token, TEMPLATE_SPAN, TEMPLATE_TAIL); }

    static bool IsMember(Value token) { return base::IsInRange(token, TEMPLATE_SPAN, LBRACK); }

    static bool IsProperty(Value token) { return base::IsInRange(token, PERIOD, LBRACK); }

    static bool IsPropertyOrCall(Value token) { return base::IsInRange(token, TEMPLATE_SPAN, LPAREN); }

    static bool IsArrowOrAssignmentOp(Value token) { return base::IsInRange(token, ARROW, ASSIGN_SUB); }

    static bool IsAssignmentOp(Value token) { return base::IsInRange(token, INIT, ASSIGN_SUB); }

    static bool IsLogicalAssignmentOp(Value token) { return base::IsInRange(token, ASSIGN_NULLISH, ASSIGN_AND); }
    ...
    static bool IsOrderedRelationalCompareOp(Value op) { return base::IsInRange(op, LT, GTE); }

    static bool IsEqualityOp(Value op) { return base::IsInRange(op, EQ, EQ_STRICT); }
    ...
    static bool IsBitOp(Value op) { return base::IsInRange(op, BIT_OR, SHR) || op == BIT_NOT; }
    ...
    static bool IsUnaryOrCountOp(Value op) { return base::IsInRange(op, ADD, DEC); }
