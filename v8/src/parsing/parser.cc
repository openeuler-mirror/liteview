...
FunctionLiteral* Parser::DefaultConstructor(const AstRawString* name, bool call_super, int pos, int end_pos)
{
...

    {
        ...
        if (call_super) {
            ...
            Variable* constructor_args = function_scope->DeclareParameter(constructor_args_name, VariableMode::kTemporary, is_optional, is_rest, ast_value_factory(), pos);

            Expression* call;
            {
                ScopedPtrList<Expression> args(pointer_buffer());
                Spread* spread_args = factory()->NewSpread(factory()->NewVariableProxy(constructor_args), pos, pos);
    ...
    FunctionLiteral* function_literal = factory()->NewFunctionLiteral(name, function_scope, body, expected_property_count, parameter_count, parameter_count,
        FunctionLiteral::kNoDuplicateParameters, FunctionSyntaxKind::kAnonymousExpression, default_eager_compile_hint(), pos, true, GetNextFunctionLiteralId());
...
Expression* Parser::NewSuperPropertyReference(Scope* home_object_scope, int pos)
{
    ...
    return factory()->NewSuperPropertyReference(home_object_scope->NewHomeObjectVariableProxy(factory(), home_object_name, pos), pos);
}
...
Expression* Parser::NewV8Intrinsic(const AstRawString* name, const ScopedPtrList<Expression>& args, int pos)
{
    ...
    if (function != nullptr) {
        ...
        DCHECK_EQ(Context::kNotFound, Context::IntrinsicIndexForName(name->raw_data(), name->length()));
...
Expression* Parser::NewV8RuntimeFunctionForFuzzing(const Runtime::Function* function, const ScopedPtrList<Expression>& args, int pos)
{
    ...
    // ignore this call.
    if (function == nullptr || !Runtime::IsAllowListedForFuzzing(function->function_id) || function->nargs > args.length()) {
...
Parser::Parser(LocalIsolate* local_isolate, ParseInfo* info, Handle<Script> script)
    : ParserBase<Parser>(info->zone(), &scanner_, info->stack_limit(), info->ast_value_factory(), info->pending_error_handler(), info->runtime_call_stats(), info->v8_file_logger(),
        info->flags(), true)
...
template <typename IsolateT> void Parser::DeserializeScopeChain(IsolateT* isolate, ParseInfo* info, MaybeHandle<ScopeInfo> maybe_outer_scope_info, Scope::DeserializationMode mode)
{
    InitializeEmptyScopeChain(info);
    Handle<ScopeInfo> outer_scope_info;
    if (maybe_outer_scope_info.ToHandle(&outer_scope_info)) {
        DCHECK_EQ(ThreadId::Current(), isolate->thread_id());
        original_scope_ = Scope::DeserializeScopeChain(isolate, zone(), *outer_scope_info, info->script_scope(), ast_value_factory(), mode);
        if (flags().is_eval() || IsArrowFunction(flags().function_kind())) {
            original_scope_->GetReceiverScope()->DeserializeReceiver(ast_value_factory());
        }
    }
}
...
template void Parser::DeserializeScopeChain(LocalIsolate* isolate, ParseInfo* info, MaybeHandle<ScopeInfo> maybe_outer_scope_info, Scope::DeserializationMode mode);
...
void Parser::ParseProgram(Isolate* isolate, Handle<Script> script, ParseInfo* info, MaybeHandle<ScopeInfo> maybe_outer_scope_info)
{
    ...
    FunctionLiteral* result = nullptr;
    {
        ...
        if (flags().is_module()) {
            ...
            if (IsAsyncModule(scope->function_kind())) {
                impl()->RewriteAsyncFunctionBody(&body, block, factory()->NewUndefinedLiteral(kNoSourcePosition));
            }
...
ZonePtrList<const AstRawString>* Parser::PrepareWrappedArguments(Isolate* isolate, ParseInfo* info, Zone* zone)
{
    ...
    for (int i = 0; i < arguments_length; i++) {
        const AstRawString* argument_string = ast_value_factory()->GetString(String::cast(arguments->get(i)), SharedStringAccessGuardIfNeeded(isolate));
...
void Parser::ParseREPLProgram(ParseInfo* info, ScopedPtrList<Statement>* body, DeclarationScope* scope)
{
    ...
    base::Optional<VariableProxy*> maybe_result = Rewriter::RewriteBody(info, scope, block->statements());
    ...
    impl()->RewriteAsyncFunctionBody(body, block, WrapREPLResult(result_value), REPLMode::kYes);
}
...
void Parser::ParseFunction(Isolate* isolate, ParseInfo* info, Handle<SharedFunctionInfo> shared_info)
{
    // It's OK to use the Isolate & counters here, since this function is only
    // called in the main thread.
    DCHECK(parsing_on_main_thread_);
    ...
    if (flags().function_kind() == FunctionKind::kClassMembersInitializerFunction && shared_info->HasOuterScopeInfo()
        && maybe_outer_scope_info.ToHandleChecked()->scope_type() == CLASS_SCOPE && maybe_outer_scope_info.ToHandleChecked()->StartPosition() == start_position) {
        Handle<ScopeInfo> outer_scope_info = maybe_outer_scope_info.ToHandleChecked();
        if (outer_scope_info->HasOuterScopeInfo()) {
            deserialize_start_scope = handle(outer_scope_info->OuterScopeInfo(), isolate);
        } else {
    ...
    DeserializeScopeChain(isolate, info, deserialize_start_scope, Scope::DeserializationMode::kIncludingVariables);
    ...
    if (V8_UNLIKELY(shared_info->private_name_lookup_skips_outer_class() && original_scope_->is_class_scope())) {
        ...
        result = DoParseDeserializedFunction(isolate, maybe_outer_scope_info, info, start_position, end_position, function_literal_id, info->function_name());
    } else {
        result = DoParseDeserializedFunction(isolate, maybe_outer_scope_info, info, start_position, end_position, function_literal_id, info->function_name());
    }
    MaybeProcessSourceRanges(info, result, stack_limit_);
    ...
    if (V8_UNLIKELY(v8_flags.log_function_events && result != nullptr)) {
        ...
        LOG(isolate,
            FunctionEvent(
                "parse-function", flags().script_id(), ms, function_scope->start_position(), function_scope->end_position(), function_name.get(), strlen(function_name.get())));
    }
}

FunctionLiteral* Parser::DoParseFunction(Isolate* isolate, ParseInfo* info, int start_position, int end_position, int function_literal_id, const AstRawString* raw_name)
{
    DCHECK_EQ(parsing_on_main_thread_, isolate != nullptr);
...

    {
        ...
        if (IsArrowFunction(kind)) {
            ...
            if (GetLastFunctionLiteralId() != function_literal_id - 1) {
                ...
                AstFunctionLiteralIdReindexer reindexer(stack_limit_, (function_literal_id - 1) - GetLastFunctionLiteralId());
            ...
            result = DefaultConstructor(raw_name, IsDerivedConstructor(kind), start_position, end_position);
        } else {
            ZonePtrList<const AstRawString>* arguments_for_wrapped_function = info->is_wrapped_as_function() ? PrepareWrappedArguments(isolate, info, zone()) : nullptr;
            result = ParseFunctionLiteral(raw_name, Scanner::Location::invalid(), kSkipFunctionNameCheck, kind, kNoSourcePosition, flags().function_syntax_kind(),
...
FunctionLiteral* Parser::DoParseDeserializedFunction(
    Isolate* isolate, MaybeHandle<ScopeInfo> maybe_outer_scope_info, ParseInfo* info, int start_position, int end_position, int function_literal_id, const AstRawString* raw_name)
{
    if (flags().function_kind() == FunctionKind::kClassMembersInitializerFunction) {
        return ParseClassForInstanceMemberInitialization(isolate, maybe_outer_scope_info, start_position, function_literal_id, end_position);
    }

    return DoParseFunction(isolate, info, start_position, end_position, function_literal_id, raw_name);
}

FunctionLiteral* Parser::ParseClassForInstanceMemberInitialization(
    Isolate* isolate, MaybeHandle<ScopeInfo> maybe_class_scope_info, int initializer_pos, int initializer_id, int initializer_end_pos)
{
    ...
    FunctionLiteral* initializer = literal->instance_members_initializer_function();

    // Reindex so that the function literal ids match.
    AstFunctionLiteralIdReindexer reindexer(stack_limit_, initializer_id - initializer->function_literal_id());
    ...
    bool needs_allocation_fixup = !maybe_class_scope_info.is_null() && maybe_class_scope_info.ToHandleChecked()->scope_type() == CLASS_SCOPE
        && maybe_class_scope_info.ToHandleChecked()->StartPosition() == class_token_pos;

    ClassScope* reparsed_scope = literal->scope();
    reparsed_scope->FinalizeReparsedClassScope(isolate, maybe_class_scope_info, ast_value_factory(), needs_allocation_fixup);
    original_scope_ = reparsed_scope;

    DCHECK_EQ(initializer->kind(), FunctionKind::kClassMembersInitializerFunction);
...
ImportAssertions* Parser::ParseImportAssertClause()
{
    ...
    if (scanner()->HasLineTerminatorBeforeNext() || !CheckContextualKeyword(ast_value_factory()->assert_string())) {
        return import_assertions;
    }
    ...
    while (peek() != Token::RBRACE) {
        ...
        auto result = import_assertions->insert(std::make_pair(attribute_key, std::make_pair(attribute_value, location)));
        if (!result.second) {
            // It is a syntax error if two AssertEntries have the same key.
            ReportMessageAt(location, MessageTemplate::kImportAssertionDuplicateKey, attribute_key);
            break;
        }
        ...
        if (CheckContextualKeyword(ast_value_factory()->from_string())) {
            ...
            if (export_data->is_empty()) {
                ...
                for (const ExportClauseData& data : *export_data) {
                    module()->AddExport(data.local_name, data.export_name, module_specifier, import_assertions, data.location, specifier_loc, zone());
                }
            }
        } else {
...
{
    Variable* var = DeclareVariable(proxy->raw_name(), kind, mode, Variable::DefaultInitializationFlag(mode), scope, was_added, proxy->position(), kNoSourcePosition);
    var->set_initializer_position(initializer_position);
    proxy->BindTo(var);
}

Variable* Parser::DeclareVariable(const AstRawString* name, VariableKind kind, VariableMode mode, InitializationFlag init, Scope* scope, bool* was_added, int begin, int end)
{
...
void Parser::Declare(Declaration* declaration, const AstRawString* name, VariableKind variable_kind, VariableMode mode, InitializationFlag init, Scope* scope, bool* was_added,
...
Statement* Parser::DeclareFunction(
    const AstRawString* variable_name, FunctionLiteral* function, VariableMode mode, VariableKind kind, int beg_pos, int end_pos, ZonePtrList<const AstRawString>* names)
{
...
Statement* Parser::DeclareNative(const AstRawString* name, int pos)
{
    ...
    return factory()->NewExpressionStatement(factory()->NewAssignment(Token::INIT, proxy, lit, kNoSourcePosition), pos);
}
...
Expression* Parser::RewriteReturn(Expression* return_value, int pos)
{
    if (IsDerivedConstructor(function_state_->kind())) {
        ...
        Assignment* assign = factory()->NewAssignment(Token::ASSIGN, factory()->NewVariableProxy(temp), return_value, pos);

        // temp === undefined
        Expression* is_undefined = factory()->NewCompareOperation(Token::EQ_STRICT, assign, factory()->NewUndefinedLiteral(kNoSourcePosition), pos);
        ...
        // ParseFunctionBody.
        return_value = factory()->NewConditional(is_undefined, factory()->ThisExpression(), factory()->NewVariableProxy(temp), pos);
    }
    return return_value;
}
...
Statement* Parser::RewriteSwitchStatement(SwitchStatement* switch_statement, Scope* scope)
{
    ...
    Assignment* tag_assign = factory()->NewAssignment(Token::ASSIGN, factory()->NewVariableProxy(tag_variable), tag, tag->position());
...
void Parser::InitializeVariables(ScopedPtrList<Statement>* statements, VariableKind kind, const DeclarationParsingResult::Declaration* declaration)
{
    ...
    Assignment* assignment = factory()->NewAssignment(Token::INIT, declaration->pattern, declaration->initializer, pos);
...
Statement* Parser::RewriteTryStatement(
    Block* try_block, Block* catch_block, const SourceRange& catch_range, Block* finally_block, const SourceRange& finally_range, const CatchInfo& catch_info, int pos)
{
...
void Parser::ParseAndRewriteGeneratorFunctionBody(int pos, FunctionKind kind, ScopedPtrList<Statement>* body)
{
...
void Parser::ParseAndRewriteAsyncGeneratorFunctionBody(int pos, FunctionKind kind, ScopedPtrList<Statement>* body)
{
    ...
    Block* try_block;
    {
        ScopedPtrList<Statement> statements(pointer_buffer());
        Expression* initial_yield = BuildInitialYield(pos, kind);
        statements.Add(factory()->NewExpressionStatement(initial_yield, kNoSourcePosition));
        ...
        statements.Add(factory()->NewSyntheticAsyncReturnStatement(factory()->NewUndefinedLiteral(kNoSourcePosition), kNoSourcePosition));
    ...
    Block* catch_block;
    {
        ...
        reject_args.Add(factory()->NewVariableProxy(function_state_->scope()->generator_object_var()));
        reject_args.Add(factory()->NewVariableProxy(catch_scope->catch_variable()));

        Expression* reject_call = factory()->NewCallRuntime(Runtime::kInlineAsyncGeneratorReject, reject_args, kNoSourcePosition);
        catch_block = IgnoreCompletion(factory()->NewReturnStatement(reject_call, kNoSourcePosition, kNoSourcePosition));
    }

    {
        ScopedPtrList<Statement> statements(pointer_buffer());
        TryStatement* try_catch = factory()->NewTryCatchStatementForAsyncAwait(try_block, catch_scope, catch_block, kNoSourcePosition);
    ...
    Expression* close_call;
    {
        ...
        VariableProxy* call_proxy = factory()->NewVariableProxy(function_state_->scope()->generator_object_var());
        close_args.Add(call_proxy);
        close_call = factory()->NewCallRuntime(Runtime::kInlineGeneratorClose, close_args, kNoSourcePosition);
    }

    Block* finally_block;
    {
        ScopedPtrList<Statement> statements(pointer_buffer());
        statements.Add(factory()->NewExpressionStatement(close_call, kNoSourcePosition));
        finally_block = factory()->NewBlock(false, statements);
    }

    body->Add(factory()->NewTryFinallyStatement(try_block, finally_block, kNoSourcePosition));
}
...
Block* Parser::RewriteForVarInLegacy(const ForInfo& for_info)
{
    ...
    if (!IsLexicalVariableMode(for_info.parsing_result.descriptor.mode) && decl.initializer != nullptr && decl.pattern->IsVariableProxy()) {
        ...
        init_block->statements()->Add(
            factory()->NewExpressionStatement(factory()->NewAssignment(Token::ASSIGN, single_var, decl.initializer, decl.value_beg_pos), kNoSourcePosition), zone());
        return init_block;
    }
    return nullptr;
}
    ...
    Block* inner_block = factory()->NewBlock(3, false);
    {
        ...
        if (next) {
            ...
            // Make statement: first = 0.
            {
                ...
                Assignment* assignment = factory()->NewAssignment(Token::ASSIGN, first_proxy, const0, kNoSourcePosition);
        ...
        Statement* compound_next_statement = nullptr;
        {
            ...
            for (int i = 0; i < for_info.bound_names.length(); i++) {
                ...
                Assignment* assignment = factory()->NewAssignment(Token::ASSIGN, temp_proxy, proxy, kNoSourcePosition);
                compound_next = factory()->NewBinaryOperation(Token::COMMA, compound_next, assignment, kNoSourcePosition);
            }
...
FunctionLiteral* Parser::ParseFunctionLiteral(const AstRawString* function_name, Scanner::Location function_name_location, FunctionNameValidity function_name_validity,
    FunctionKind kind, int function_token_pos, FunctionSyntaxKind function_syntax_kind, LanguageMode language_mode, ZonePtrList<const AstRawString>* arguments_for_wrapped_function)
{
    // Function ::
    ...
    FunctionLiteral::EagerCompileHint eager_compile_hint
        = function_state_->next_function_is_likely_called() || is_wrapped ? FunctionLiteral::kShouldEagerCompile : default_eager_compile_hint();
    ...
    bool should_post_parallel_task = can_post_parallel_task && !flags().is_reparse()
        && ((is_eager_top_level_function && flags().post_parallel_compile_tasks_for_eager_toplevel()) || (is_lazy && flags().post_parallel_compile_tasks_for_lazy()));
    ...
    bool did_preparse_successfully = should_preparse && SkipFunction(function_name, kind, function_syntax_kind, scope, &num_parameters, &function_length, &produced_preparse_data);
    ...
    if (!did_preparse_successfully) {
        ...
        ParseFunction(&body, function_name, pos, kind, function_syntax_kind, scope, &num_parameters, &function_length, &has_duplicate_parameters, &expected_property_count,
    ...
    FunctionLiteral::ParameterFlag duplicate_parameters = has_duplicate_parameters ? FunctionLiteral::kHasDuplicateParameters : FunctionLiteral::kNoDuplicateParameters;

    // Note that the FunctionLiteral needs to be created in the main Zone again.
    FunctionLiteral* function_literal = factory()->NewFunctionLiteral(function_name, scope, body, expected_property_count, num_parameters, function_length, duplicate_parameters,
        function_syntax_kind, eager_compile_hint, pos, true, function_literal_id, produced_preparse_data);
...
bool Parser::SkipFunction(const AstRawString* function_name, FunctionKind kind, FunctionSyntaxKind function_syntax_kind, DeclarationScope* function_scope, int* num_parameters,
    int* function_length, ProducedPreparseData** produced_preparse_data)
{
    ...
    DCHECK_IMPLIES(IsArrowFunction(kind), scanner()->current_token() == Token::ARROW);
    ...
    if (consumed_preparse_data_) {
            ...
            return true;
        {
            ...
            *produced_preparse_data = consumed_preparse_data_->GetDataForSkippableFunction(
                main_zone(), function_scope->start_position(), &end_position, num_parameters, function_length, &num_inner_functions, &uses_super_property, &language_mode);
        }
    ...
    PreParser::PreParseResult result = reusable_preparser()->PreParseFunction(function_name, kind, function_syntax_kind, function_scope, use_counts_, produced_preparse_data);
    ...
    for (auto parameter : parameters.params) {
        ...
        if (parameter->initializer() != nullptr) {
            ...
            auto condition = factory()->NewCompareOperation(
                Token::EQ_STRICT, factory()->NewVariableProxy(parameters.scope->parameter(index)), factory()->NewUndefinedLiteral(kNoSourcePosition), kNoSourcePosition);
...
Scope* Parser::NewHiddenCatchScope()
{
    ...
    bool was_added;
    catch_scope->DeclareLocal(ast_value_factory()->dot_catch_string(), VariableMode::kVar, NORMAL_VARIABLE, &was_added);
    DCHECK(was_added);
    catch_scope->set_is_hidden();
    return catch_scope;
}

Block* Parser::BuildRejectPromiseOnException(Block* inner_block, REPLMode repl_mode)
{
    // try {
    //   <inner_block>
    // } catch (.catch) {
    ...
    {
        ScopedPtrList<Expression> args(pointer_buffer());
        args.Add(factory()->NewVariableProxy(function_state_->scope()->generator_object_var()));
        args.Add(factory()->NewVariableProxy(catch_scope->catch_variable()));
        reject_promise = factory()->NewCallRuntime(Runtime::kInlineAsyncFunctionReject, args, kNoSourcePosition);
    }
    Block* catch_block = IgnoreCompletion(factory()->NewReturnStatement(reject_promise, kNoSourcePosition, kNoSourcePosition));
    ...
    TryStatement* try_catch_statement = repl_mode == REPLMode::kYes ? factory()->NewTryCatchStatementForReplAsyncAwait(inner_block, catch_scope, catch_block, kNoSourcePosition)
                                                                    : factory()->NewTryCatchStatementForAsyncAwait(inner_block, catch_scope, catch_block, kNoSourcePosition);
...
void Parser::ParseFunction(ScopedPtrList<Statement>* body, const AstRawString* function_name, int pos, FunctionKind kind, FunctionSyntaxKind function_syntax_kind,
    DeclarationScope* function_scope, int* num_parameters, int* function_length, bool* has_duplicate_parameters, int* expected_property_count, int* suspend_count,
        ...
        if (is_wrapped) {
            ...
            if (expected_parameters_end_pos != kNoSourcePosition) {
                ...
                if (position < expected_parameters_end_pos) {
                    ...
                    ReportMessageAt(Scanner::Location(expected_parameters_end_pos - 2, expected_parameters_end_pos), MessageTemplate::kUnexpectedEndOfArgString);
                    return;
                }
            }
            Expect(Token::RPAREN);
...
Variable* Parser::CreatePrivateNameVariable(ClassScope* scope, VariableMode mode, IsStaticFlag is_static_flag, const AstRawString* name)
{
    DCHECK_NOT_NULL(name);
    int begin = position();
...
void Parser::DeclarePublicClassField(ClassScope* scope, ClassLiteralProperty* property, bool is_static, bool is_computed_name, ClassInfo* class_info)
{
    ...
    if (is_computed_name) {
        ...
        Variable* computed_name_var = CreateSyntheticContextVariable(ClassFieldVariableName(ast_value_factory(), class_info->computed_field_count));
        property->set_computed_name_var(computed_name_var);
        class_info->public_members->Add(property, zone());
    }
}

void Parser::DeclarePrivateClassMember(
    ClassScope* scope, const AstRawString* property_name, ClassLiteralProperty* property, ClassLiteralProperty::Kind kind, bool is_static, ClassInfo* class_info)
{
    ...
    Variable* private_name_var = CreatePrivateNameVariable(scope, GetVariableMode(kind), is_static ? IsStaticFlag::kStatic : IsStaticFlag::kNotStatic, property_name);
...
FunctionLiteral* Parser::CreateInitializerFunction(const char* name, DeclarationScope* scope, Statement* initializer_stmt)
{
    DCHECK(IsClassMembersInitializerFunction(scope->function_kind()));
    // function() { .. class fields initializer .. }
    ScopedPtrList<Statement> statements(pointer_buffer());
    statements.Add(initializer_stmt);
    FunctionLiteral* result = factory()->NewFunctionLiteral(ast_value_factory()->GetOneByteString(name), scope, statements, 0, 0, 0, FunctionLiteral::kNoDuplicateParameters,
        FunctionSyntaxKind::kAccessorOrMethod, FunctionLiteral::kShouldEagerCompile, scope->start_position(), false, GetNextFunctionLiteralId());
#ifdef DEBUG
...
Expression* Parser::RewriteClassLiteral(ClassScope* block_scope, const AstRawString* name, ClassInfo* class_info, int pos, int end_pos)
{
    ...
    if (class_info->has_static_elements) {
        static_initializer = CreateInitializerFunction(
            "<static_initializer>", class_info->static_elements_scope, factory()->NewInitializeClassStaticElementsStatement(class_info->static_elements, kNoSourcePosition));
    }
    ...
    if (class_info->has_instance_members) {
        instance_members_initializer_function = CreateInitializerFunction(
            "<instance_members_initializer>", class_info->instance_members_scope, factory()->NewInitializeClassMembersStatement(class_info->instance_fields, kNoSourcePosition));
    ...
    ClassLiteral* class_literal = factory()->NewClassLiteral(block_scope, class_info->extends, class_info->constructor, class_info->public_members, class_info->private_members,
        static_initializer, instance_members_initializer_function, pos, end_pos, class_info->has_static_computed_names, class_info->is_anonymous, class_info->has_private_methods,
...
template <typename IsolateT> void Parser::HandleSourceURLComments(IsolateT* isolate, Handle<Script> script)
{
...
template void Parser::HandleSourceURLComments(Isolate* isolate, Handle<Script> script);
template void Parser::HandleSourceURLComments(LocalIsolate* isolate, Handle<Script> script);
...
void Parser::UpdateStatistics(Handle<Script> script, base::SmallVector<v8::Isolate::UseCounterFeature, 8>* use_counts, int* preparse_skipped)
{
    // Move statistics to Isolate.
    for (int feature = 0; feature < v8::Isolate::kUseCounterFeatureCount; ++feature) {
        if (use_counts_[feature] > 0) {
...
void Parser::ParseOnBackground(LocalIsolate* isolate, ParseInfo* info, int start_position, int end_position, int function_literal_id)
{
...
Parser::TemplateLiteralState Parser::OpenTemplateLiteral(int pos) { return zone()->New<TemplateLiteral>(zone(), pos); }
...
void Parser::AddTemplateExpression(TemplateLiteralState* state, Expression* expression) { (*state)->AddExpression(expression, zone()); }
...
ArrayLiteral* Parser::ArrayLiteralFromListWithSpread(const ScopedPtrList<Expression>& list)
{
    ...
    int first_spread = 0;
    for (; first_spread < list.length() && !list.at(first_spread)->IsSpread(); ++first_spread) { }
...
void Parser::RewriteAsyncFunctionBody(ScopedPtrList<Statement>* body, Block* block, Expression* return_value, REPLMode repl_mode)
{
    ...
    block->statements()->Add(factory()->NewSyntheticAsyncReturnStatement(return_value, return_value->position()), zone());
...
Statement* Parser::CheckCallable(Variable* var, Expression* error, int pos)
{
    ...
    {
        Expression* type_of = factory()->NewUnaryOperation(Token::TYPEOF, factory()->NewVariableProxy(var), nopos);
        Expression* function_literal = factory()->NewStringLiteral(ast_value_factory()->function_string(), nopos);
        Expression* condition = factory()->NewCompareOperation(Token::EQ_STRICT, type_of, function_literal, nopos);

        Statement* throw_call = factory()->NewExpressionStatement(error, pos);

        validate_var = factory()->NewIfStatement(condition, factory()->EmptyStatement(), throw_call, nopos);
    }
    return validate_var;
}
