...
template <typename T> class ZoneHandleSet final {
public:
    ZoneHandleSet()
        : data_(kEmptyTag)
    {
    ...
    bool contains(ZoneHandleSet<T> const& other) const
    {
        ...
        if ((other.data_ & kTagMask) == kSingletonTag) {
            return std::find(cached_list->begin(), cached_list->end(), other.singleton()) != cached_list->end();
        }
        DCHECK_EQ(kListTag, other.data_ & kTagMask);
        // TODO(bmeurer): Optimize this case.
        for (size_t i = 0; i < other.list()->size(); ++i) {
            if (std::find(cached_list->begin(), cached_list->end(), other.list()->at(i)) == cached_list->end()) {
                return false;
            }
        }
        return true;
    ...
    friend bool operator==(ZoneHandleSet<T> const& lhs, ZoneHandleSet<T> const& rhs)
    {
    ...
    friend bool operator!=(ZoneHandleSet<T> const& lhs, ZoneHandleSet<T> const& rhs) { return !(lhs == rhs); }

    friend size_t hash_value(ZoneHandleSet<T> const& set) { return static_cast<size_t>(set.data_); }
...
private:
    ...
    enum Tag : Address { kSingletonTag = 0, kEmptyTag = 1, kListTag = 2, kTagMask = 3 };
...
template <typename T> std::ostream& operator<<(std::ostream& os, ZoneHandleSet<T> set)
{
...
template <typename T> class ZoneHandleSet<T>::const_iterator {
public:
    ...
    bool operator==(const const_iterator& other) const { return set_ == other.set_ && current_ == other.current_; }
...
template <typename T> typename ZoneHandleSet<T>::const_iterator ZoneHandleSet<T>::begin() const { return ZoneHandleSet<T>::const_iterator(this, 0); }

template <typename T> typename ZoneHandleSet<T>::const_iterator ZoneHandleSet<T>::end() const { return ZoneHandleSet<T>::const_iterator(this, size()); }
