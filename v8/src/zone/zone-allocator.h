...
public:
    ...
    template <typename U> ZoneAllocator(const ZoneAllocator<U>& other) V8_NOEXCEPT : ZoneAllocator<T>(other.zone_)
    {
        // If we are going to allocate compressed pointers in the zone it must
        // support compression.
        DCHECK_IMPLIES(is_compressed_pointer<T>::value, zone_->supports_compression());
    ...
    size_t max_size() const { return std::numeric_limits<int>::max() / sizeof(T); }
    ...
    bool operator==(ZoneAllocator const& other) const { return zone_ == other.zone_; }
    bool operator!=(ZoneAllocator const& other) const { return zone_ != other.zone_; }
