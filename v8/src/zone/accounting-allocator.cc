...
VirtualMemory ReserveAddressSpace(v8::PageAllocator* platform_allocator)
{
    ...
    void* hint = reinterpret_cast<void*>(RoundDown(reinterpret_cast<uintptr_t>(platform_allocator->GetRandomMmapAddr()), ZoneCompression::kReservationAlignment));
    ...
    FATAL("Fatal process out of memory: Failed to reserve memory for compressed "
          "zones");
    UNREACHABLE();
}
...
AccountingAllocator::AccountingAllocator()
    : zone_backing_malloc_(V8::GetCurrentPlatform()->GetZoneBackingAllocator()->GetMallocFn())
    , zone_backing_free_(V8::GetCurrentPlatform()->GetZoneBackingAllocator()->GetFreeFn())
{
