...
public:
    ...
    bool supports_compression() const { return COMPRESS_ZONES_BOOL && supports_compression_; }
    ...
    template <typename T, typename TypeTag = T[]> T* NewArray(size_t length)
    {
    ...
    // When V8_ENABLE_PRECISE_ZONE_STATS is defined, the deallocated bytes are
    // associated with the provided TypeTag type.
    template <typename T, typename TypeTag = T[]> void DeleteArray(T* pointer, size_t length) { Delete<TypeTag>(pointer, length * sizeof(T)); }
...
public:
    ...
    template <typename T, typename TypeTag = T[]> V8_INLINE T* NewArray(size_t length) { return zone()->NewArray<T, TypeTag>(length); }
    template <typename T, typename TypeTag = T[]> V8_INLINE void DeleteArray(T* p, size_t length) { zone()->DeleteArray<T, TypeTag>(p, length); }
