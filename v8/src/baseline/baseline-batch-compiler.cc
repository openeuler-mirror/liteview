...
static bool CanCompileWithConcurrentBaseline(SharedFunctionInfo shared, Isolate* isolate) { return !shared.HasBaselineCode() && CanCompileWithBaseline(isolate, shared); }

class BaselineCompilerTask {
public:
    BaselineCompilerTask(Isolate* isolate, PersistentHandles* handles, SharedFunctionInfo sfi)
    ...
    void Compile(LocalIsolate* local_isolate)
    {
        ...
        maybe_code_ = local_isolate->heap()->NewPersistentMaybeHandle(compiler.Build(local_isolate));
        Handle<Code> code;
    ...
    void Install(Isolate* isolate)
    {
        ...
        if (shared_function_info_->script().IsScript()) {
            Compiler::LogFunctionCompilation(isolate, LogEventListener::CodeTag::kFunction, handle(Script::cast(shared_function_info_->script()), isolate), shared_function_info_,
                Handle<FeedbackVector>(), Handle<AbstractCode>::cast(code), CodeKind::BASELINE, time_taken_ms_);
        }
    }
...
public:
    BaselineBatchCompilerJob(Isolate* isolate, Handle<WeakFixedArray> task_queue, int batch_size)
    {
...
public:
    ...
    public:
        JobDispatcher(
            Isolate* isolate, LockedQueue<std::unique_ptr<BaselineBatchCompilerJob>>* incoming_queue, LockedQueue<std::unique_ptr<BaselineBatchCompilerJob>>* outcoming_queue)
            : isolate_(isolate)
...
void BaselineBatchCompiler::EnsureQueueCapacity()
{
    ...
    if (last_index_ >= compilation_queue_->length()) {
        Handle<WeakFixedArray> new_queue = isolate_->factory()->CopyWeakFixedArrayAndGrow(compilation_queue_, last_index_);
        GlobalHandles::Destroy(compilation_queue_.location());
...
bool BaselineBatchCompiler::MaybeCompileFunction(MaybeObject maybe_sfi)
{
    ...
    if (!maybe_sfi.GetHeapObjectIfWeak(&heapobj))
        return false;
    Handle<SharedFunctionInfo> shared = handle(SharedFunctionInfo::cast(heapobj), isolate_);
    // Skip functions where the bytecode has been flushed.
...
void BaselineBatchCompiler::EnqueueFunction(Handle<JSFunction> function) { UNREACHABLE(); }

void BaselineBatchCompiler::EnqueueSFI(SharedFunctionInfo shared) { UNREACHABLE(); }
