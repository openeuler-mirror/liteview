...
public:
    ...
    template <typename IsolateT> Handle<ByteArray> ToBytecodeOffsetTable(IsolateT* isolate);
...
private:
    ...
    void UpdateInterruptBudgetAndJumpToLabel(int weight, Label* label, Label* skip_interrupt_label);
    ...
    void UpdateMaxCallArgs(int max_call_args) { max_call_args_ = std::max(max_call_args_, max_call_args); }
...
#undef DECLARE_VISITOR
    ...
    Label* EnsureLabel(int i, MarkAsIndirectJumpTarget mark = MarkAsIndirectJumpTarget::kNo)
    {
