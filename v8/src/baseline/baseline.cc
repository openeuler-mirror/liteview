...
bool CanCompileWithBaseline(Isolate* isolate, SharedFunctionInfo shared)
{
    ...
    if (shared.HasDebugInfo() && shared.GetDebugInfo().HasInstrumentedBytecodeArray()) {
        return false;
    }
...
void EmitReturnBaseline(MacroAssembler* masm) { baseline::BaselineAssembler::EmitReturn(masm); }
...
bool CanCompileWithBaseline(Isolate* isolate, SharedFunctionInfo shared) { return false; }

MaybeHandle<Code> GenerateBaselineCode(Isolate* isolate, Handle<SharedFunctionInfo> shared) { UNREACHABLE(); }
