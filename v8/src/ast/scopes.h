...
public:
    ...
    void SetScopeName(const AstRawString* scope_name) { scope_name_ = scope_name; }
#endif
    ...
    bool must_use_preparsed_scope_data() const { return must_use_preparsed_scope_data_; }
    ...
    Variable* DeclareVariable(Declaration* declaration, const AstRawString* name, int pos, VariableMode mode, VariableKind kind, InitializationFlag init, bool* was_added,
    ...
    void set_start_position(int statement_pos) { start_position_ = statement_pos; }
    ...
    bool has_forced_context_allocation_for_parameters() const { return force_context_allocation_for_parameters_; }
    ...
    bool is_block_scope() const { return scope_type_ == BLOCK_SCOPE || scope_type_ == CLASS_SCOPE; }
    bool is_with_scope() const { return scope_type_ == WITH_SCOPE; }
    bool is_declaration_scope() const { return is_declaration_scope_; }
    bool is_class_scope() const { return scope_type_ == CLASS_SCOPE; }
    bool is_home_object_scope() const { return is_class_scope() || (is_block_scope() && is_block_scope_for_object_literal_); }
    ...
    bool private_name_lookup_skips_outer_class() const { return private_name_lookup_skips_outer_class_; }
    ...
    LanguageMode language_mode() const { return is_strict_ ? LanguageMode::kStrict : LanguageMode::kSloppy; }
    ...
    int ContextHeaderLength() const { return HasContextExtensionSlot() ? Context::MIN_CONTEXT_EXTENDED_SLOTS : Context::MIN_CONTEXT_SLOTS; }
    ...
    void set_deserialized_scope_uses_external_cache() { deserialized_scope_uses_external_cache_ = true; }
    bool deserialized_scope_uses_external_cache() const { return deserialized_scope_uses_external_cache_; }
    ...
    VariableProxy* NewHomeObjectVariableProxy(AstNodeFactory* factory, const AstRawString* name, int start_pos);
...
protected:
    ...
    void set_language_mode(LanguageMode language_mode) { is_strict_ = is_strict(language_mode); }

private:
    Variable* Declare(
        Zone* zone, const AstRawString* name, VariableMode mode, VariableKind kind, InitializationFlag initialization_flag, MaybeAssignedFlag maybe_assigned_flag, bool* was_added)
    {
    ...
    void CollectNonLocals(DeclarationScope* max_outer_scope, Isolate* isolate, Handle<StringSet>* non_locals);
    ...
    template <typename IsolateT> void AllocateScopeInfosRecursively(IsolateT* isolate, MaybeHandle<ScopeInfo> outer_scope);

    void AllocateDebuggerScopeInfos(Isolate* isolate, MaybeHandle<ScopeInfo> outer_scope);
...
public:
    ...
    bool is_arrow_scope() const { return is_function_scope() && IsArrowFunction(function_kind_); }
    ...
    bool sloppy_eval_can_extend_vars() const { return sloppy_eval_can_extend_vars_; }
...
#ifdef DEBUG
    void set_is_being_lazily_parsed(bool is_being_lazily_parsed) { is_being_lazily_parsed_ = is_being_lazily_parsed; }
    ...
    void set_has_checked_syntax(bool has_checked_syntax) { has_checked_syntax_ = has_checked_syntax; }
    bool has_checked_syntax() const { return has_checked_syntax_; }

    bool ShouldEagerCompile() const { return force_eager_compilation_ || should_eager_compile_; }
    ...
    bool should_ban_arguments() const { return IsClassMembersInitializerFunction(function_kind()); }
    ...
    Variable* rest_parameter() const { return has_rest_ ? params_[params_.length() - 1] : nullptr; }
    ...
    Variable* this_function_var() const
    {
        ...
        DCHECK(
            this_function == nullptr || (is_function_scope() && (IsClassConstructor(function_kind()) || IsConciseMethod(function_kind()) || IsAccessorFunction(function_kind()))));
    ...
    template <typename IsolateT> V8_EXPORT_PRIVATE static void AllocateScopeInfos(ParseInfo* info, IsolateT* isolate);

    Handle<StringSet> CollectNonLocals(Isolate* isolate, Handle<StringSet> non_locals);
    ...
    bool has_inferred_function_name() const { return has_inferred_function_name_; }
    ...
    bool needs_private_name_context_chain_recalc() const { return needs_private_name_context_chain_recalc_; }
    ...
    void set_class_scope_has_private_brand(bool value) { class_scope_has_private_brand_ = value; }
    bool class_scope_has_private_brand() const { return class_scope_has_private_brand_; }
...
public:
    ...
    Variable* brand() { return GetRareData() == nullptr ? nullptr : GetRareData()->brand; }

    Variable* class_variable() { return class_variable_; }

    V8_INLINE bool IsParsingHeritage() { return rare_data_and_is_parsing_heritage_.GetPayload(); }

    // Only maintained when the scope is parsed, not when the scope is
    // deserialized.
    bool has_static_private_methods() const { return has_static_private_methods_; }
    ...
    void set_should_save_class_variable_index() { should_save_class_variable_index_ = true; }
    ...
    void FinalizeReparsedClassScope(Isolate* isolate, MaybeHandle<ScopeInfo> outer_scope_info, AstValueFactory* ast_value_factory, bool needs_allocation_fixup);
#ifdef DEBUG
...
private:
    ...
    V8_INLINE RareData* GetRareData() { return rare_data_and_is_parsing_heritage_.GetPointer(); }
    V8_INLINE RareData* EnsureRareData()
    {
    ...
    V8_INLINE void SetIsParsingHeritage(bool v) { rare_data_and_is_parsing_heritage_.SetPayload(v); }
