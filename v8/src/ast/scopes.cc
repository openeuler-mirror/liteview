...
static_assert(sizeof(VariableMap) == (sizeof(void*) + 2 * sizeof(uint32_t) + sizeof(ZoneAllocationPolicy)), "Empty base optimization didn't kick in for VariableMap");
...
{
    bool was_added;
    Variable* home_object_variable = Declare(zone(), ast_value_factory->dot_home_object_string(), VariableMode::kConst, NORMAL_VARIABLE, InitializationFlag::kCreatedInitialized,
    ...
    , rare_data_and_is_parsing_heritage_(nullptr)
{
    ...
    if (scope_info->HasSavedClassVariable()) {
        ...
        Variable* var = DeclareClassVariable(ast_value_factory, ast_value_factory->GetString(name, SharedStringAccessGuardIfNeeded(isolate)), kNoSourcePosition);
...
void DeclarationScope::set_should_eager_compile() { should_eager_compile_ = !was_lazily_parsed_; }

#if V8_ENABLE_WEBASSEMBLY
void DeclarationScope::set_is_asm_module() { is_asm_module_ = true; }

bool Scope::IsAsmModule() const { return is_function_scope() && AsDeclarationScope()->is_asm_module(); }
...
Scope* Scope::DeserializeScopeChain(
    IsolateT* isolate, Zone* zone, ScopeInfo scope_info, DeclarationScope* script_scope, AstValueFactory* ast_value_factory, DeserializationMode deserialization_mode)
{
    ...
    while (!scope_info.is_null()) {
        if (scope_info.scope_type() == WITH_SCOPE) {
            ...
            if (scope_info.IsReplModeScope())
                script_scope->set_is_repl_mode_scope();
            ...
            outer_scope = zone->New<Scope>(zone, ast_value_factory->GetString(name, SharedStringAccessGuardIfNeeded(isolate)), maybe_assigned, handle(scope_info, isolate));
        }
        ...
        if (cache_scope_found) {
            ...
            cache_scope_found = outer_scope->is_declaration_scope() && !outer_scope->is_eval_scope();
        }
        ...
        scope_info = scope_info.HasOuterScopeInfo() ? scope_info.OuterScopeInfo() : ScopeInfo();
    }
...
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Scope* Scope::DeserializeScopeChain(
    Isolate* isolate, Zone* zone, ScopeInfo scope_info, DeclarationScope* script_scope, AstValueFactory* ast_value_factory, DeserializationMode deserialization_mode);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Scope* Scope::DeserializeScopeChain(
    LocalIsolate* isolate, Zone* zone, ScopeInfo scope_info, DeclarationScope* script_scope, AstValueFactory* ast_value_factory, DeserializationMode deserialization_mode);

#ifdef DEBUG
bool Scope::IsReparsedMemberInitializerScope() const
{
    return is_declaration_scope() && IsClassMembersInitializerFunction(AsDeclarationScope()->function_kind()) && outer_scope()->AsClassScope()->is_reparsed_class_scope();
}
#endif
...
void DeclarationScope::DeclareSloppyBlockFunction(SloppyBlockFunctionStatement* sloppy_block_function) { sloppy_block_functions_.Add(sloppy_block_function); }
...
void DeclarationScope::HoistSloppyBlockFunctions(AstNodeFactory* factory)
{
    ...
    for (SloppyBlockFunctionStatement* sloppy_block_function : sloppy_block_functions_) {
        ...
        if (factory) {
            ...
            Variable* var
                = DeclareVariable(declaration, name, pos, VariableMode::kVar, NORMAL_VARIABLE, Variable::DefaultInitializationFlag(VariableMode::kVar), &was_added, nullptr, &ok);
            DCHECK(ok);
...
bool DeclarationScope::Analyze(ParseInfo* info)
{
    ...
    DCHECK(scope->is_script_scope() || scope->outer_scope()->is_script_scope() || scope->IsReparsedMemberInitializerScope() || scope->outer_scope()->already_resolved_);
...
Variable* Scope::LookupInScopeInfo(const AstRawString* name, Scope* cache)
{
    ...
    DCHECK_IMPLIES(cache != this, cache->outer_scope()->deserialized_scope_uses_external_cache());
    DCHECK_NULL(cache->variables_.Lookup(name));
    ...
    Variable* var = cache->variables_.Declare(
        zone(), this, name, lookup_result.mode, NORMAL_VARIABLE, lookup_result.init_flag, lookup_result.maybe_assigned_flag, IsStaticFlag::kNotStatic, &was_added);
    DCHECK(was_added);
    var->AllocateTo(location, index);
    return var;
}

Variable* DeclarationScope::DeclareParameter(const AstRawString* name, VariableMode mode, bool is_optional, bool is_rest, AstValueFactory* ast_value_factory, int position)
{
    DCHECK(!already_resolved_);
...
Variable* Scope::DeclareLocal(const AstRawString* name, VariableMode mode, VariableKind kind, bool* was_added, InitializationFlag init_flag)
{
    ...
    DCHECK_IMPLIES(GetDeclarationScope()->is_being_lazily_parsed(), mode == VariableMode::kVar || mode == VariableMode::kLet || mode == VariableMode::kConst);
...
Variable* Scope::DeclareVariable(Declaration* declaration, const AstRawString* name, int pos, VariableMode mode, VariableKind kind, InitializationFlag init, bool* was_added,
...
Variable* DeclarationScope::DeclareDynamicGlobal(const AstRawString* name, VariableKind kind, Scope* cache)
{
    ...
    bool was_added;
    return cache->variables_.Declare(zone(), this, name, VariableMode::kDynamicGlobal, kind, kCreatedInitialized, kNotAssigned, IsStaticFlag::kNotStatic, &was_added);
    // TODO(neis): Mark variable as maybe-assigned?
}

bool Scope::RemoveUnresolved(VariableProxy* var) { return unresolved_list_.Remove(var); }
...
Variable* Scope::NewTemporary(const AstRawString* name) { return NewTemporary(name, kMaybeAssigned); }
...
bool Scope::ShouldBanArguments() { return GetReceiverScope()->should_ban_arguments(); }
...
namespace {
bool WasLazilyParsed(Scope* scope) { return scope->is_declaration_scope() && scope->AsDeclarationScope()->was_lazily_parsed(); }
...
bool Scope::IsConstructorScope() const { return is_declaration_scope() && IsClassConstructor(AsDeclarationScope()->function_kind()); }
...
void Scope::CollectNonLocals(DeclarationScope* max_outer_scope, Isolate* isolate, Handle<StringSet>* non_locals)
{
...
void Scope::AnalyzePartially(DeclarationScope* max_outer_scope, AstNodeFactory* ast_node_factory, UnresolvedList* new_unresolved_list, bool maybe_in_arrowhead)
{
    this->ForEach([max_outer_scope, ast_node_factory, new_unresolved_list, maybe_in_arrowhead](Scope* scope) {
        DCHECK_IMPLIES(scope->is_declaration_scope(), !scope->AsDeclarationScope()->was_lazily_parsed());
...
Handle<StringSet> DeclarationScope::CollectNonLocals(Isolate* isolate, Handle<StringSet> non_locals)
{
...
void DeclarationScope::AnalyzePartially(Parser* parser, AstNodeFactory* ast_node_factory, bool maybe_in_arrowhead)
{
    ...
    if (!IsArrowFunction(function_kind_)
        && (!outer_scope_->is_script_scope() || maybe_in_arrowhead || (preparse_data_builder_ != nullptr && preparse_data_builder_->HasInnerFunctions()))) {
...
void PrintName(const AstRawString* name) { PrintF("%.*s", name->length(), name->raw_data()); }
...
// static
template <Scope::ScopeLookupMode mode> Variable* Scope::Lookup(VariableProxy* proxy, Scope* scope, Scope* outer_scope_end, Scope* cache_scope, bool force_context_allocation)
{
    ...
    // continuing.
    if (mode == kDeserializedScope && scope->deserialized_scope_uses_external_cache()) {
    ...
    while (true) {
        ...
        if (mode == kDeserializedScope && V8_UNLIKELY(scope->is_debug_evaluate_scope_)) {
            DCHECK(scope->deserialized_scope_uses_external_cache() || scope == cache_scope);
        ...
        if (mode == kParsedScope) {
            ...
            var = scope->LookupInScopeInfo(proxy->raw_name(), external_cache ? cache_scope : scope);
        }
        ...
        if (mode == kParsedScope && !scope->scope_info_.is_null()) {
            ...
            return Lookup<kDeserializedScope>(proxy, scope, outer_scope_end, cache_scope);
        }
    }
...
Variable* Scope::LookupWith(VariableProxy* proxy, Scope* scope, Scope* outer_scope_end, Scope* cache_scope, bool force_context_allocation)
{
    ...
    Variable* var = scope->outer_scope_->scope_info_.is_null() ? Lookup<kParsedScope>(proxy, scope->outer_scope_, outer_scope_end, nullptr, force_context_allocation)
    ...
    Variable* dynamic = target_scope->NonLocal(proxy->raw_name(), VariableMode::kDynamic);
...
Variable* Scope::LookupSloppyEval(VariableProxy* proxy, Scope* scope, Scope* outer_scope_end, Scope* cache_scope, bool force_context_allocation)
{
    ...
    Scope* entry_cache = cache_scope == nullptr ? scope->outer_scope()->GetNonEvalDeclarationScope() : cache_scope;
    Variable* var = scope->outer_scope_->scope_info_.is_null() ? Lookup<kParsedScope>(proxy, scope->outer_scope_, outer_scope_end, nullptr, force_context_allocation)
...
void Scope::AllocateHeapSlot(Variable* var) { var->AllocateTo(VariableLocation::CONTEXT, num_heap_slots_++); }
...
void DeclarationScope::AllocateLocals()
{
    ...
    NullifyRareVariableIf(RareVariable::kThisFunction, [=](Variable* var) { return !MustAllocate(var); });
}
...
template <typename IsolateT> void Scope::AllocateScopeInfosRecursively(IsolateT* isolate, MaybeHandle<ScopeInfo> outer_scope)
{
    DCHECK(scope_info_.is_null());
...
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void Scope::AllocateScopeInfosRecursively<Isolate>(Isolate* isolate, MaybeHandle<ScopeInfo> outer_scope);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) void Scope::AllocateScopeInfosRecursively<LocalIsolate>(LocalIsolate* isolate, MaybeHandle<ScopeInfo> outer_scope);
...
// static
template <typename IsolateT> void DeclarationScope::AllocateScopeInfos(ParseInfo* info, IsolateT* isolate)
{
    DeclarationScope* scope = info->literal()->scope();
...
template V8_EXPORT_PRIVATE void DeclarationScope::AllocateScopeInfos(ParseInfo* info, Isolate* isolate);
template V8_EXPORT_PRIVATE void DeclarationScope::AllocateScopeInfos(ParseInfo* info, LocalIsolate* isolate);
...
VariableProxy* Scope::NewHomeObjectVariableProxy(AstNodeFactory* factory, const AstRawString* name, int start_pos)
{
    ...
    // innermost scope.
    VariableProxy* proxy = factory->NewVariableProxy(name, NORMAL_VARIABLE, start_pos);
    AddUnresolved(proxy);
    return proxy;
}
...
void ClassScope::FinalizeReparsedClassScope(Isolate* isolate, MaybeHandle<ScopeInfo> maybe_scope_info, AstValueFactory* ast_value_factory, bool needs_allocation_fixup)
{
    ...
    for (auto it : ScopeInfo::IterateLocalNames(scope_info)) {
        ...
        const AstRawString* string = ast_value_factory->GetString(it->name(), SharedStringAccessGuardIfNeeded(isolate));
        Variable* var = string->IsPrivateName() ? LookupLocalPrivateName(string) : LookupLocal(string);
        DCHECK_NOT_NULL(var);
...
Variable* ClassScope::DeclareClassVariable(AstValueFactory* ast_value_factory, const AstRawString* name, int class_token_pos)
{
    ...
    bool was_added;
    class_variable_ = Declare(zone(), name == nullptr ? ast_value_factory->dot_string() : name, VariableMode::kConst, NORMAL_VARIABLE, InitializationFlag::kNeedsInitialization,
