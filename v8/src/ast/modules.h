...
public:
    ...
    void AddImport(const AstRawString* import_name, const AstRawString* local_name, const AstRawString* module_request, const ImportAssertions* import_assertions,
        const Scanner::Location loc, const Scanner::Location specifier_loc, Zone* zone);

    // import * as x from "foo.js";
    void AddStarImport(const AstRawString* local_name, const AstRawString* module_request, const ImportAssertions* import_assertions, const Scanner::Location loc,
        const Scanner::Location specifier_loc, Zone* zone);

    // import "foo.js";
    // import {} from "foo.js";
    // export {} from "foo.js";  (sic!)
    void AddEmptyImport(const AstRawString* module_request, const ImportAssertions* import_assertions, const Scanner::Location specifier_loc, Zone* zone);
    ...
    void AddExport(const AstRawString* export_name, const AstRawString* import_name, const AstRawString* module_request, const ImportAssertions* import_assertions,
        const Scanner::Location loc, const Scanner::Location specifier_loc, Zone* zone);
    ...
    public:
        AstModuleRequest(const AstRawString* specifier, const ImportAssertions* import_assertions, int position, int index)
        ...
        const ImportAssertions* import_assertions() const { return import_assertions_; }
    ...
    // Namespace imports.
    const ZoneVector<const Entry*>& namespace_imports() const { return namespace_imports_; }
    ...
    const ZoneVector<const Entry*>& special_exports() const { return special_exports_; }
...
private:
    ...
    int AddModuleRequest(const AstRawString* specifier, const ImportAssertions* import_assertions, Scanner::Location specifier_loc, Zone* zone)
    {
        DCHECK_NOT_NULL(specifier);
        int module_requests_count = static_cast<int>(module_requests_.size());
        auto it = module_requests_.insert(zone->New<AstModuleRequest>(specifier, import_assertions, specifier_loc.beg_pos, module_requests_count)).first;
        return (*it)->index();
    }
};
