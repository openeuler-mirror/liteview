...
public:
    ...
    PlatformData()
        : profiled_thread_(OpenThread(THREAD_GET_CONTEXT | THREAD_SUSPEND_RESUME | THREAD_QUERY_INFORMATION, false, GetCurrentThreadId()))
    {
    }
...
class Sampler::PlatformData {
public:
    PlatformData() { zx_handle_duplicate(zx_thread_self(), ZX_RIGHT_SAME_RIGHTS, &profiled_thread_); }
    ~PlatformData()
    {
...
#if !(V8_OS_OPENBSD || (V8_OS_LINUX && (V8_HOST_ARCH_PPC || V8_HOST_ARCH_S390 || V8_HOST_ARCH_PPC64)))
