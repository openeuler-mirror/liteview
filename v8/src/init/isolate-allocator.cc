...
// not zero.
inline size_t GetIsolateRootBiasPageSize(v8::PageAllocator* platform_page_allocator) { return RoundUp(Isolate::isolate_root_bias(), platform_page_allocator->AllocatePageSize()); }
...
struct PtrComprCageReservationParams : public VirtualMemoryCage::ReservationParams {
    PtrComprCageReservationParams()
    {
        ...
        // cage base.
        const size_t kIsolateRootBiasPageSize = COMPRESS_POINTERS_IN_ISOLATE_CAGE_BOOL ? GetIsolateRootBiasPageSize(page_allocator) : 0;
        ...
        requested_start_hint = reinterpret_cast<Address>(page_allocator->GetRandomMmapAddr());
...
{
    if (std::shared_ptr<CodeRange> code_range = CodeRange::GetProcessWideCodeRange()) {
        code_range->Free();
    }
...
#endif
    if (!GetProcessWidePtrComprCage()->InitReservation(params, existing_reservation)) {
...
IsolateAllocator::IsolateAllocator()
{
    ...
    if (!isolate_ptr_compr_cage_.InitReservation(params)) {
        V8::FatalProcessOutOfMemory(nullptr, "Failed to reserve memory for Isolate V8 pointer compression cage");
    }
...
const VirtualMemoryCage* IsolateAllocator::GetPtrComprCage() const { return const_cast<IsolateAllocator*>(this)->GetPtrComprCage(); }
...
void IsolateAllocator::CommitPagesForIsolate()
{
    ...
    CHECK_GE(isolate_ptr_compr_cage_.reservation()->size(), kPtrComprCageReservationSize + GetIsolateRootBiasPageSize(platform_page_allocator));
    CHECK(isolate_ptr_compr_cage_.reservation()->InVM(isolate_root, kPtrComprCageReservationSize));
    ...
    // Inform the bounded page allocator about reserved pages.
    {
        ...
        CHECK(isolate_ptr_compr_cage_.page_allocator()->AllocatePagesAt(reserved_region_address, reserved_region_size, PageAllocator::Permission::kNoAccess));
    }
    ...
    // Commit pages where the Isolate will be stored.
    {
        ...
        // need.
        CHECK(isolate_ptr_compr_cage_.reservation()->SetPermissions(committed_region_address, committed_region_size, PageAllocator::kReadWrite));

        if (Heap::ShouldZapGarbage()) {
            MemsetPointer(reinterpret_cast<Address*>(committed_region_address), kZapValue, committed_region_size / kSystemPointerSize);
        }
    }
