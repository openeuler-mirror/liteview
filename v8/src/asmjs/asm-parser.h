...
#undef V
    ...
    public:
        ...
        inline void reuse(ZoneVector<T>* vec) { reusable_vectors_.emplace_back(std::move(*vec)); }
    ...
    inline bool Peek(AsmJsScanner::token_t token) { return scanner_.Token() == token; }

    inline bool PeekForZero() { return (scanner_.IsUnsigned() && scanner_.AsUnsigned() == 0); }
    ...
    void DeclareGlobal(VarInfo* info, bool mutable_variable, AsmType* type, ValueType vtype, WasmInitExpr init = WasmInitExpr());
