...
Type::Type(const Type& other) V8_NOEXCEPT : TypeBase(other), parent_(other.parent_), aliases_(), id_(TypeOracle::FreshTypeId()), constexpr_version_(other.constexpr_version_) { }
...
std::string Type::UnhandlifiedCppTypeName() const
{
    if (IsSubtypeOf(TypeOracle::GetSmiType()))
        return "int";
    if (this == TypeOracle::GetObjectType())
        return "Object";
...
std::string AbstractType::GetGeneratedTypeNameImpl() const
{
    ...
    if (base::Optional<const Type*> type_wrapped_in_lazy = Type::MatchUnaryGeneric(this, TypeOracle::GetLazyGeneric())) {
        DCHECK(!IsConstexpr());
...
std::string BitFieldStructType::ToExplicitString() const { return "bitfield struct " + name(); }
        ...
        for (const Field& field : aggregate_type->fields()) {
            ...
            if (i != field_names.end()) {
                ...
                if (i->second == this) {
                    ...
                    ReportError(aggregate_type_name, " '", name(), "' declares a field with the name '", field_name, "' that masks an inherited field from class '",
                        i->second->name(), "'");
                }
            }
...
std::string StructType::GetGeneratedTypeNameImpl() const { return generated_type_name_; }
...
// static
base::Optional<const Type*> Type::MatchUnaryGeneric(const Type* type, GenericType* generic)
{
...
ClassType::ClassType(
    const Type* parent, Namespace* nspace, const std::string& name, ClassFlags flags, const std::string& generates, const ClassDeclaration* decl, const TypeAlias* alias)
...
std::string ClassType::GetGeneratedTNodeTypeNameImpl() const { return generates_; }

std::string ClassType::GetGeneratedTypeNameImpl() const { return IsConstexpr() ? GetGeneratedTNodeTypeName() : "TNode<" + GetGeneratedTNodeTypeName() + ">"; }

std::string ClassType::ToExplicitString() const { return "class " + name(); }

bool ClassType::AllowInstantiation() const { return (!IsExtern() || nspace()->IsDefaultNamespace()) && !IsAbstract(); }
...
void ClassType::InitializeInstanceTypes(base::Optional<int> own, base::Optional<std::pair<int, int>> range) const
{
...
void ClassType::GenerateAccessors()
{
    ...
    for (size_t field_index = 0; field_index < fields_.size(); ++field_index) {
        ...
        Declarations::DeclareMacro(load_macro_name, true, base::nullopt, load_signature, load_body, base::nullopt);
        ...
        if (!field.const_qualified) {
            ...
            Declarations::DeclareMacro(store_macro_name, true, base::nullopt, store_signature, store_body, base::nullopt, false);
        }
    }
}
...
void ClassType::GenerateSliceAccessor(size_t field_index)
{
    ...
    signature.return_type = field.const_qualified ? TypeOracle::GetConstSliceType(field.name_and_type.type) : TypeOracle::GetMutableSliceType(field.name_and_type.type);
    ...
    if (field.offset.has_value()) {
        ...
        const Type* previous_slice_type
            = previous->const_qualified ? TypeOracle::GetConstSliceType(previous->name_and_type.type) : TypeOracle::GetMutableSliceType(previous->name_and_type.type);

        // %FieldSlice<ClassName, MutableSlice<PType>>(o, "p")
        Expression* previous_expression = MakeCallExpression(
            MakeIdentifierExpression({ "torque_internal" }, "%FieldSlice", { MakeNode<PrecomputedTypeExpression>(this), MakeNode<PrecomputedTypeExpression>(previous_slice_type) }),
    ...
    Expression* length_expression = MakeCallExpression(MakeIdentifierExpression({ "torque_internal" }, "%IndexedFieldLength", { MakeNode<PrecomputedTypeExpression>(this) }),
    ...
    IdentifierExpression* new_struct = MakeIdentifierExpression(
        { "torque_internal", "unsafe" }, field.const_qualified ? "NewConstSlice" : "NewMutableSlice", { MakeNode<PrecomputedTypeExpression>(field.name_and_type.type) });
    ...
    Macro* macro = Declarations::DeclareMacro(macro_name, true, base::nullopt, signature, block, base::nullopt);
...
namespace {
bool FirstTypeIsContext(const std::vector<const Type*> parameter_types) { return !parameter_types.empty() && parameter_types[0] == TypeOracle::GetContextType(); }
} // namespace

bool Signature::HasContextParameter() const { return FirstTypeIsContext(types()); }

bool BuiltinPointerType::HasContextParameter() const { return FirstTypeIsContext(parameter_types()); }
...
{
    return type == TypeOracle::GetUint32Type() || type == TypeOracle::GetUint31Type() || type == TypeOracle::GetUint16Type() || type == TypeOracle::GetUint8Type()
...
bool IsPointerSizeIntegralType(const Type* type) { return type->IsSubtypeOf(TypeOracle::GetUIntPtrType()) || type->IsSubtypeOf(TypeOracle::GetIntPtrType()); }
