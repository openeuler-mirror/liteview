...
public:
    ...
    template <class T> const T& Cast() const& { return value_->Cast<T>(); }
    template <class T> T& Cast() & { return value_->Cast<T>(); }
    template <class T> T&& Cast() && { return std::move(value_->Cast<T>()); }
...
public:
    ...
    template <class T> T NextAs() { return std::move(Next().Cast<T>()); }
...
inline base::Optional<ParseResult> DefaultAction(ParseResultIterator* child_results)
{
...
public:
    ...
    V8_EXPORT_PRIVATE base::Optional<ParseResult> RunAction(const Item* item, const LexerResult& tokens);
...
public:
    ...
    bool operator==(const Item& other) const { return rule_ == other.rule_ && mark_ == other.mark_ && start_ == other.start_ && pos_ == other.pos_; }

    friend size_t hash_value(const Item& i) { return base::hash_combine(i.rule_, i.mark_, i.start_, i.pos_); }
...
inline base::Optional<ParseResult> Symbol::RunAction(const Item* item, const LexerResult& tokens)
{
...
inline base::Optional<ParseResult> ParseTokens(Symbol* start, const LexerResult& tokens)
{
...
public:
    ...
    void SetWhitespace(PatternFunction whitespace) { match_whitespace_ = whitespace; }
...
protected:
    ...
    // result.
    static base::Optional<ParseResult> YieldMatchedInput(ParseResultIterator* child_results) { return ParseResult { child_results->matched_input().ToString() }; }

    // Create a new symbol to parse the given sequence of symbols.
    // At most one of the symbols can return a result.
    Symbol* Sequence(std::vector<Symbol*> symbols) { return NewSymbol({ Rule(std::move(symbols)) }); }

    template <class T, T value> static base::Optional<ParseResult> YieldIntegralConstant(ParseResultIterator* child_results) { return ParseResult { value }; }

    template <class T> static base::Optional<ParseResult> YieldDefaultValue(ParseResultIterator* child_results) { return ParseResult { T {} }; }

    template <class From, class To> static base::Optional<ParseResult> CastParseResult(ParseResultIterator* child_results)
    {
    ...
    template <class T, class Result = T> Symbol* TryOrDefault(Symbol* s) { return NewSymbol({ Rule({ s }, CastParseResult<Result, T>), Rule({}, YieldDefaultValue<T>) }); }

    template <class T> static base::Optional<ParseResult> MakeSingletonVector(ParseResultIterator* child_results)
    {
    ...
    template <class T> static base::Optional<ParseResult> MakeExtendedVector(ParseResultIterator* child_results)
    {
    ...
    template <class T> Symbol* NonemptyList(Symbol* element, base::Optional<Symbol*> separator = {})
    {
    ...
    template <class T> Symbol* List(Symbol* element, base::Optional<Symbol*> separator = {}) { return TryOrDefault<std::vector<T>>(NonemptyList<T>(element, separator)); }

    template <class T> Symbol* Optional(Symbol* x) { return TryOrDefault<base::Optional<T>, T>(x); }

    Symbol* CheckIf(Symbol* x) { return NewSymbol({ Rule({ x }, YieldIntegralConstant<bool, true>), Rule({}, YieldIntegralConstant<bool, false>) }); }
