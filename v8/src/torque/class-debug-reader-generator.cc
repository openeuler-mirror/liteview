...
public:
    ...
    bool operator==(const ValueTypeFieldIterator& other) const { return type_ == other.type_ && index_ == other.index_; }
    bool operator!=(const ValueTypeFieldIterator& other) const { return !(*this == other); }
...
public:
    ...
    bool IsTagged() const { return name_and_type_.type->IsSubtypeOf(TypeOracle::GetTaggedType()); }
    ...
    std::string GetOriginalType(TypeStorage storage) const
    {
        ...
        if (IsTagged()) {
            if (storage == kAsStoredInHeap && TargetArchitecture::ArePointersCompressed()) {
                return "v8::internal::TaggedValue";
            }
            base::Optional<const ClassType*> field_class_type = name_and_type_.type->ClassSupertype();
            return "v8::internal::" + (field_class_type.has_value() ? (*field_class_type)->GetGeneratedTNodeTypeName() : "Object");
        }
    ...
    std::string GetAddressGetter() { return "Get" + CamelifyString(name_and_type_.name) + "Address"; }
...
void GenerateFieldValueAccessor(const Field& field, const std::string& class_name, std::ostream& h_contents, std::ostream& cc_contents)
{
    ...
    cc_contents << "\nValue<" << field_value_type << "> Tq" << class_name << "::" << field_getter << "(d::MemoryAccessor accessor" << index_param << ") const {\n";
...
void GenerateGetPropsChunkForField(const Field& field, std::ostream& get_props_impl, std::string class_name)
{
    ...
    for (const auto& struct_field : ValueTypeFieldsRange(field.name_and_type.type)) {
        ...
        get_props_impl << "  " << struct_field_list << ".push_back(std::make_unique<StructProperty>(\"" << struct_field.name_and_type.name << "\", "
                       << struct_field_type.GetTypeString(kAsStoredInHeap) << ", " << struct_field_type.GetTypeString(kUncompressed) << ", " << struct_field.offset_bytes << ", "
    ...
    if (field.index) {
        ...
        get_props_impl << "  if (" << validity << " == d::MemoryAccessResult::kOk) {\n"
                       << "    result.push_back(std::make_unique<ObjectProperty>(\"" << field.name_and_type.name << "\", " << debug_field_type.GetTypeString(kAsStoredInHeap)
    ...
    get_props_impl << "  result.push_back(std::make_unique<ObjectProperty>(\"" << field.name_and_type.name << "\", " << debug_field_type.GetTypeString(kAsStoredInHeap) << ", "
                   << debug_field_type.GetTypeString(kUncompressed) << ", " << debug_field_type.GetAddressGetter() << "(), " << count_value << ", " << debug_field_type.GetSize()
...
void GenerateClassDebugReader(
    const ClassType& type, std::ostream& h_contents, std::ostream& cc_contents, std::ostream& visitor, std::ostream& class_names, std::unordered_set<const ClassType*>* done)
{
    ...
    if (super_type != nullptr) {
        GenerateClassDebugReader(*super_type, h_contents, cc_contents, visitor, class_names, done);
    }
...
void ImplementationVisitor::GenerateClassDebugReaders(const std::string& output_directory)
{
    ...
    h_contents << "// space and must be read using a callback function.\n\n";
    {
        ...
        for (const ClassType* type : TypeOracle::GetClasses()) {
            GenerateClassDebugReader(*type, h_contents, cc_contents, visitor, class_names, &done);
        }
