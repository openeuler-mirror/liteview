...
struct LineAndColumn {
    ...
    static LineAndColumn WithUnknownOffset(int line, int column) { return { kUnknownOffset, line, column }; }
    ...
    bool operator!=(const LineAndColumn& other) const { return !operator==(other); }
};
...
struct SourcePosition {
    ...
    bool CompareStartIgnoreColumn(const SourcePosition& pos) const { return start.line == pos.start.line && source == pos.source; }
    ...
    bool operator==(const SourcePosition& pos) const { return source == pos.source && start == pos.start && end == pos.end; }
