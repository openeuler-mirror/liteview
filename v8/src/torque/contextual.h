...
template <class Variable> V8_EXPORT_PRIVATE typename Variable::Scope** ContextualVariableTop();
...
template <class Derived, class VarType> class ContextualVariable {
public:
...
private:
    template <class T> friend V8_EXPORT_PRIVATE typename T::Scope** ContextualVariableTop();
...
#define DECLARE_CONTEXTUAL_VARIABLE(VarName, ...)                                                                                                                                  \
    struct VarName : v8::internal::torque::ContextualVariable<VarName, __VA_ARGS__> { }
//
...
#define DEFINE_CONTEXTUAL_VARIABLE(VarName)                                                                                                                                        \
    template <> V8_EXPORT_PRIVATE VarName::Scope** ContextualVariableTop<VarName>()                                                                                                \
    {                                                                                                                                                                              \
        ...
        do {                                                                                                                                                                       \
            ...
            if (0 == old_var) {                                                                                                                                                    \
                if (0 != slot)                                                                                                                                                     \
                    DebugBreak();                                                                                                                                                  \
                slot = TlsAlloc();                                                                                                                                                 \
                stub = malloc(sizeof(void*));                                                                                                                                      \
                TlsSetValue(slot, (LPVOID)stub);                                                                                                                                   \
                MB_InterlockedExchange((long volatile*)&(run_once), 1);                                                                                                              \
                break;                                                                                                                                                             \
            } else if (1 == old_var) {                                                                                                                                             \
                stub = (void*)TlsGetValue(slot);                                                                                                                                   \
                break;                                                                                                                                                             \
            } else if (2 == old_var) {                                                                                                                                             \
                continue;                                                                                                                                                          \
            } else {                                                                                                                                                               \
