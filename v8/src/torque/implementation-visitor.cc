...
{
    Signature signature { {}, base::nullopt, { {}, false }, 0, decl->type(), {}, false };
...
VisitResult ImplementationVisitor::InlineMacro(
    Macro* macro, base::Optional<LocationReference> this_reference, const std::vector<VisitResult>& arguments, const std::vector<Block*> label_blocks)
{
...
void ImplementationVisitor::VisitMacroCommon(Macro* macro)
{
    ...
    if (Method* method = Method::DynamicCast(macro)) {
        ...
        // errors.
        this_reference = (this_type->IsClassType()) ? LocationReference::Temporary(this_result, "this parameter") : LocationReference::VariableAccess(this_result);
    }
    ...
    for (size_t i = 0; i < label_blocks.size(); ++i) {
        ...
        assembler().Emit(GotoExternalInstruction { ExternalLabelName(label_info.name->value), label_parameter_variables });
    }
...
void ImplementationVisitor::Visit(TorqueMacro* macro) { VisitMacroCommon(macro); }
...
std::string AddParameter(
    size_t i, Builtin* builtin, Stack<std::string>* parameters, Stack<const Type*>* parameter_types, BlockBindings<LocalValue>* parameter_bindings, bool mark_as_used)
{
    ...
    Binding<LocalValue>* binding = parameter_bindings->Add(name, LocalValue { LocationReference::Temporary(VisitResult(type, range), "parameter " + name->value) }, mark_as_used);
    if (GlobalContext::collect_kythe_data()) {
        KytheData::AddBindingDefinition(binding);
    }
    ...
    if (builtin->IsVarArgsJavaScript() || builtin->IsFixedArgsJavaScript()) {
        if (builtin->IsVarArgsJavaScript()) {
            ...
            parameter_bindings.Add(
                *signature.arguments_variable, LocalValue { LocationReference::Temporary(VisitResult(arguments_type, range), "parameter " + *signature.arguments_variable) }, true);
        }
        ...
        for (size_t i = 0; i < signature.implicit_count; ++i) {
            ...
            if (std::find(expected_types.begin(), expected_types.end(), actual_type) == expected_types.end()) {
                Error("According to JavaScript calling convention, expected parameter ", param_name, " to have type ", PrintList(expected_types, " or "), " but found type ",
                    *actual_type)
                    .Position(param_pos);
            }
        }
...
const Type* ImplementationVisitor::Visit(VarDeclarationStatement* stmt, BlockBindings<LocalValue>* block_bindings)
{
    ...
    if (stmt->initializer) {
        ...
        if ((*type)->IsConstexpr() && !stmt->const_qualified) {
            Error("Use 'const' instead of 'let' for variable '", stmt->name->value, "' of constexpr type '", (*type)->ToString(), "'.").Position(stmt->name->pos).Throw();
        }
    ...
    LocationReference ref = stmt->const_qualified ? LocationReference::Temporary(*init_result, "const " + stmt->name->value) : LocationReference::VariableAccess(*init_result);
    block_bindings->Add(stmt->name, LocalValue { std::move(ref) });
    return TypeOracle::GetVoidType();
}

const Type* ImplementationVisitor::Visit(TailCallStatement* stmt) { return Visit(stmt->call, true).type(); }
...
VisitResult ImplementationVisitor::Visit(LogicalOrExpression* expr)
{
    ...
    if (left_result.type()->IsConstexprBool()) {
        ...
        return VisitResult(TypeOracle::GetConstexprBoolType(), std::string("(") + left_result.constexpr_value() + " || " + right_result.constexpr_value() + ")");
    }
...
#endif
    assembler().Emit(AbortInstruction { stmt->never_continues ? AbortInstruction::Kind::kUnreachable : AbortInstruction::Kind::kDebugBreak });
...
VisitResult ImplementationVisitor::Visit(TryLabelExpression* expr)
{
...

    {
        ...
        Binding<LocalLabel> label_binding { &LabelBindingsManager::Get(), expr->label_block->label, LocalLabel { label_block, parameter_types } };
...
VisitResult ImplementationVisitor::Visit(StatementExpression* expr) { return VisitResult { Visit(expr->statement), assembler().TopRange(0) }; }
...
LocationReference ImplementationVisitor::GenerateFieldReference(VisitResult object, const Field& field, const ClassType* class_type, bool treat_optional_as_indexed)
{
...
VisitResult ImplementationVisitor::Visit(NewExpression* expr)
{
    ...
    if (class_type->IsExtern()) {
        ...
        get_struct_map_arguments.parameters.push_back(VisitResult(TypeOracle::GetConstexprInstanceTypeType(), CapifyStringWithUnderscores(class_type->name()) + "_TYPE"));
    ...
    VisitResult allocate_result = GenerateCall(QualifiedName({ TORQUE_INTERNAL_NAMESPACE_STRING }, "AllocateFromNew"), allocate_arguments, { class_type }, false);
...
{
    base::Optional<Binding<LocalLabel>*> break_label = TryLookupLabel(kBreakLabelName);
    if (!break_label) {
...
{
    base::Optional<Binding<LocalLabel>*> continue_label = TryLookupLabel(kContinueLabelName);
    if (!continue_label) {
...
void FailCallableLookup(const std::string& reason, const QualifiedName& name, const TypeVector& parameter_types, const std::vector<Binding<LocalLabel>*>& labels,
    const std::vector<Signature>& candidates, const std::vector<std::pair<GenericCallable*, std::string>> inapplicable_generics)
{
    std::stringstream stream;
...
base::Optional<Binding<LocalValue>*> ImplementationVisitor::TryLookupLocalValue(const std::string& name) { return ValueBindingsManager::Get().TryLookup(name); }

base::Optional<Binding<LocalLabel>*> ImplementationVisitor::TryLookupLabel(const std::string& name) { return LabelBindingsManager::Get().TryLookup(name); }
    ...
    GenericCallable* generic, const TypeVector& explicit_specialization_types, const TypeVector& explicit_arguments)
{
    ...
    for (size_t i = 0; i < parameters.implicit_count; ++i) {
        base::Optional<Binding<LocalValue>*> val = TryLookupLocalValue(parameters.names[i]->value);
        all_arguments.push_back(val ? (*val)->GetLocationReference(*val).ReferencedType() : base::nullopt);
    }
...
Method* ImplementationVisitor::LookupMethod(const std::string& name, const AggregateType* receiver_type, const Arguments& arguments, const TypeVector& specialization_types)
{
...
LocationReference ImplementationVisitor::GenerateFieldAccess(
    LocationReference reference, const std::string& fieldname, bool ignore_stuct_field_constness, base::Optional<SourcePosition> pos)
{
    ...
    if (base::Optional<const Type*> referenced_type = reference.ReferencedType()) {
...
LocationReference ImplementationVisitor::GetLocationReference(IdentifierExpression* expr)
{
    ...
    if (expr->generic_arguments.size() != 0) {
        ...
        Callable* specialization = GetOrCreateSpecialization(SpecializationKey<GenericCallable> { generic, TypeVisitor::ComputeTypeVector(expr->generic_arguments) });
    ...
    if (auto* constant = NamespaceConstant::DynamicCast(value)) {
        ...
        if (constant->type()->IsConstexpr()) {
            return LocationReference::Temporary(VisitResult(constant->type(), constant->external_name() + "(state_)"), "namespace constant " + expr->name->value);
        }
...
void ImplementationVisitor::GenerateAssignToLocation(const LocationReference& reference, const VisitResult& assignment_value)
{
    if (reference.IsCallAccess()) {
        ...
        if (referenced_type == TypeOracle::GetFloat64OrHoleType()) {
            GenerateCall(QualifiedName({ TORQUE_INTERNAL_NAMESPACE_STRING }, "StoreFloat64OrHole"), Arguments { { reference.heap_reference(), assignment_value }, {} });
        } else if (auto struct_type = referenced_type->StructSupertype()) {
            ...
            for (const Field& field : (*struct_type)->fields()) {
                ...
                GenerateAssignToLocation(GenerateFieldAccess(reference, fieldname, ignore_stuct_field_constness), ProjectStructField(assignment_value, fieldname));
            }
        } else {
            GenerateCopy(reference.heap_reference());
...
VisitResult ImplementationVisitor::GeneratePointerCall(Expression* callee, const Arguments& arguments, bool is_tailcall)
{
    ...
    if (type->parameter_types().size() != parameter_types.size()) {
        std::stringstream stream;
        stream << "parameter count mismatch calling function pointer with Type: " << *type << " - expected " << std::to_string(type->parameter_types().size()) << ", found "
...
void ImplementationVisitor::AddCallParameter(Callable* callable, VisitResult parameter, const Type* parameter_type, std::vector<VisitResult>* converted_arguments,
...
VisitResult ImplementationVisitor::GenerateCall(
    Callable* callable, base::Optional<LocationReference> this_reference, Arguments arguments, const TypeVector& specialization_types, bool is_tailcall)
{
    ...
    for (size_t i = 0; i < callable->signature().implicit_count; ++i) {
        ...
        if (val) {
            ...
            VisitResult unititialized = VisitResult::TopTypeResult(
                "implicit parameter '" + implicit_name + "' is not defined when invoking " + callable->ReadableName() + " at " + PositionAsString(CurrentSourcePosition::Get()),
    ...
    for (; current < callable->signature().implicit_count; ++current) {
        AddCallParameter(
            callable, implicit_arguments[current], callable->signature().parameter_types.types[current], &converted_arguments, &argument_range, &constexpr_arguments, inline_macro);
    }
    ...
    if (auto* builtin = Builtin::DynamicCast(callable)) {
        ...
        if (return_type->IsConstexpr()) {
            ...
            base::Optional<Block*> catch_block = GetCatchBlock();
            assembler().Emit(CallCsaMacroInstruction { macro, constexpr_arguments, catch_block });
            GenerateCatchBlock(catch_block);
            ...
            for (size_t i = 0; i < label_count; ++i) {
                ...
                if (label->parameter_types.size() != callee_label_parameters) {
                    std::stringstream s;
                    s << "label " << label->name() << " doesn't have the right number of parameters (found " << std::to_string(label->parameter_types.size()) << " expected "
        ...
        if (intrinsic->ExternalName() == "%SizeOf") {
            ...
            // them.
            assembler().Emit(MakeLazyNodeInstruction { getter, return_type, constexpr_arguments_for_getter });
...
VisitResult ImplementationVisitor::GenerateCall(const QualifiedName& callable_name, Arguments arguments, const TypeVector& specialization_types, bool is_tailcall)
{
    Callable* callable = LookupCallable(callable_name, Declarations::Lookup(callable_name), arguments, specialization_types);
    return GenerateCall(callable, base::nullopt, arguments, specialization_types, is_tailcall);
}
...
StackRange ImplementationVisitor::GenerateLabelGoto(LocalLabel* label, base::Optional<StackRange> arguments)
{
...
std::string ImplementationVisitor::ExternalLabelName(const std::string& label_name) { return "label_" + label_name; }

std::string ImplementationVisitor::ExternalLabelParameterName(const std::string& label_name, size_t i) { return "label_" + label_name + "_parameter_" + std::to_string(i); }

std::string ImplementationVisitor::ExternalParameterName(const std::string& name) { return std::string("p_") + name; }
...
base::Optional<Block*> ImplementationVisitor::GetCatchBlock()
{
    ...
    if (base::Optional<Binding<LocalLabel>*> catch_handler = TryLookupLabel(kCatchLabelName)) {
        catch_block = assembler().NewBlock(base::nullopt, true);
    }
    return catch_block;
}

void ImplementationVisitor::GenerateCatchBlock(base::Optional<Block*> catch_block)
{
    if (catch_block) {
        base::Optional<Binding<LocalLabel>*> catch_handler = TryLookupLabel(kCatchLabelName);
...
void ImplementationVisitor::Visit(Declarable* declarable, base::Optional<SourceId> file)
{
...
bool IsPointerSection(FieldSectionType type) { return type == FieldSectionType::kWeakSection || type == FieldSectionType::kStrongSection; }
...
private:
    FieldSectionType GetSectionFor(const Field& f)
    {
        ...
        bool struct_contains_tagged_fields = (struct_contents & StructType::ClassificationFlag::kStrongTagged) || (struct_contents & StructType::ClassificationFlag::kWeakTagged);
        ...
        if ((field_type->IsSubtypeOf(TypeOracle::GetStrongTaggedType()) || struct_contents == StructType::ClassificationFlag::kStrongTagged) && !f.custom_weak_marking) {
...
public:
    ClassFieldOffsetGenerator(std::ostream& header, std::ostream& inline_header, const ClassType* type, std::string gen_name, const ClassType* parent)
        : FieldOffsetsGenerator(type)
        , hdr_(header)
        , inl_(inline_header)
        , previous_field_end_((parent && parent->IsShape()) ? "P::kSize" : "P::kHeaderSize")
        , gen_name_(gen_name)
    {
    }
    ...
    void WriteFieldOffsetGetter(const Field& f) override
    {
        ...
        getter.PrintDefinition(inl_, [&](std::ostream& stream) {
            ...
            stream << "  return static_cast<int>(std::get<1>(" << Callable::PrefixNameForCCOutput(type_->GetSliceMacroName(f)) << "(*static_cast<const D*>(this))));\n";
        });
    }
    void WriteMarker(const std::string& marker) override { hdr_ << "  static constexpr int " << marker << " = " << previous_field_end_ << ";\n"; }
...
public:
    ...
    const std::string template_decl() const { return "template <class D, class P>"; }
...
base::Optional<std::vector<Field>> GetOrderedUniqueIndexFields(const ClassType& type)
{
...
void CppClassGenerator::GenerateClass()
{
    ...
    if (!type_->IsShape()) {
        ...
        f.SetDescription("Alias for HeapObject::Is"s + name_ + "() that avoids inlining.");
        f.SetExport(true);
        ...
        hdr_ << "\n";
        f.PrintDefinition(impl_, [&](std::ostream& stream) { stream << "  return o.Is" << name_ << "();\n"; });
    }
    ...
    if (type_->ShouldGenerateFullClassDefinition()) {
        ...
        while (parent) {
            if ((*parent)->ShouldGenerateCppClassDefinitions() && !(*parent)->ShouldGenerateFullClassDefinition() && (*parent)->AttributedToFile() == type_->AttributedToFile()) {
...
void CppClassGenerator::GenerateCppObjectDefinitionAsserts()
{
    ...
    for (auto f : type_->fields()) {
        ...
        hdr_ << "  static_assert(" << field << " == D::" << field << ",\n"
             << "                \"Values of " << name_ << "::" << field << " defined in Torque and C++ do not match\");\n"
             << "  static_assert(StaticStringsEqual(\"" << type << "\", D::k" << CamelifyString(f.name_and_type.name) << "TqFieldType),\n"
             << "                \"Types of " << name_ << "::" << field << " specified in Torque and C++ do not match\");\n";
    }
...
{
    cpp::Class owner({ cpp::TemplateParameter("D"), cpp::TemplateParameter("P") }, gen_name_);
    ...
    f.PrintDeclaration(hdr_);
    f.PrintDefinition(inl_, [](std::ostream& stream) { stream << "    return D(object.ptr());\n"; });
    // V8_INLINE static D unchecked_cast(Object)
    f.SetName("unchecked_cast");
    f.PrintInlineDefinition(hdr_, [](std::ostream& stream) { stream << "    return base::bit_cast<D>(object);\n"; });
}
...
void CppClassGenerator::GenerateClassConstructors()
{
    ...
    hdr_ << "  inline explicit " << gen_name_ << "(Address ptr, HeapObject::AllowInlineSmiStorage allow_smi);\n";
    ...
    for (const TypeChecker& runtime_type : type->GetTypeCheckers()) {
        ...
        if (maybe_object) {
            ...
            if (strong && runtime_type.type == WEAK_HEAP_OBJECT) {
                ...
                type_check << "(" << (strong ? "!" : "") << value << ".IsWeak() && " << value << ".GetHeapObjectOrSmi().Is"
...
void GenerateBoundsDCheck(std::ostream& os, const std::string& index, const ClassType* type, const Field& f)
{
    ...
    if (base::Optional<NameAndType> array_length = ExtractSimpleFieldArraySize(*type, f.index->expr)) {
...
bool CanGenerateFieldAccessors(const Type* field_type)
{
    ...
    return field_type != TypeOracle::GetVoidType() && field_type != TypeOracle::GetFloat64OrHoleType() && !field_type->IsSubtypeOf(TypeOracle::GetExternalPointerType());
}
} // namespace
...
bool CppClassGenerator::CanContainHeapObjects(const Type* t) { return t->IsSubtypeOf(TypeOracle::GetTaggedType()) && !t->IsSubtypeOf(TypeOracle::GetSmiType()); }
...
void CppClassGenerator::EmitLoadFieldStatement(std::ostream& stream, const Field& class_field, std::vector<const Field*>& struct_fields)
{
    ...
    if (!field_type->IsSubtypeOf(TypeOracle::GetTaggedType())) {
        if (class_field.read_synchronization == FieldSynchronization::kAcquireRelease) {
            ReportError("Torque doesn't support @cppAcquireRead on untagged data");
        } else if (class_field.read_synchronization == FieldSynchronization::kRelaxed) {
...
void CppClassGenerator::EmitStoreFieldStatement(std::ostream& stream, const Field& class_field, std::vector<const Field*>& struct_fields)
{
    ...
    if (!field_type->IsSubtypeOf(TypeOracle::GetTaggedType())) {
        stream << "  this->template WriteField<" << type_name << ">(" << offset << ", value);\n";
    } else {
        bool strong_pointer = field_type->IsSubtypeOf(TypeOracle::GetObjectType());
        bool is_smi = field_type->IsSubtypeOf(TypeOracle::GetSmiType());
        const char* write_macro;
        if (!strong_pointer) {
            if (class_field.write_synchronization == FieldSynchronization::kAcquireRelease) {
        ...
        if (!is_smi) {
            const char* write_barrier = strong_pointer ? "CONDITIONAL_WRITE_BARRIER" : "CONDITIONAL_WEAK_WRITE_BARRIER";
            stream << "  " << write_barrier << "(*this, " << offset << ", value, mode);\n";
        }
    }
}
            ...
            if (type->ShouldGenerateFactoryFunction()) {
                ...
                for (const Field& f : type->ComputeAllFields()) {
                    if (f.name_and_type.name == "map")
                        continue;
                    if (!f.index) {
                        std::string type_string = f.name_and_type.type->HandlifiedCppTypeName();
                ...
                factory_impl << ");\n";
                factory_impl << "  Map map = factory()->read_only_roots()." << SnakeifyString(type->name()) << "_map();";
                factory_impl << "  HeapObject raw_object =\n";
                factory_impl << "    factory()->AllocateRawWithImmortalMap(size, "
                                "allocation_type, map);\n";
                factory_impl << "  " << type->UnhandlifiedCppTypeName() << " result = " << type->UnhandlifiedCppTypeName() << "::cast(raw_object);\n";
                ...
                factory_impl << "template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) " << return_type << "TorqueGeneratedFactory<Factory>::" << function_name << "("
                             << parameters.str() << ");\n";
                factory_impl << "template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) " << return_type << "TorqueGeneratedFactory<LocalFactory>::" << function_name << "("
                             << parameters.str() << ");\n";
...
namespace {
void GeneratePrintDefinitionsForClass(std::ostream& impl, const ClassType* type, const std::string& gen_name, const std::string& gen_name_T, const std::string template_params)
{
...
void ImplementationVisitor::GenerateBodyDescriptors(const std::string& output_directory)
{
    ...
    for (const ClassType* type : TypeOracle::GetClasses()) {
        ...
        base::Optional<ObjectSlotKind> array_slot_kind = type->ComputeArraySlotKind();
        ...
        if (type->size().SingleValue()) {
            ...
            h_contents << "    return " << name << "::unchecked_cast(raw_object).AllocatedSize();\n";
        }
        h_contents << "  }\n\n";
...
// both).
void GenerateFieldValueVerifier(
    const std::string& class_name, bool indexed, std::string offset, const Field& leaf_field, std::string indexed_field_size, std::ostream& cc_contents, bool is_map)
{
    ...
    if (is_map) {
        ...
        cc_contents << "    " << object_type << " " << value << " = TaggedField<" << object_type << ">::load(o, " << offset << ");\n";
    }

    // Call VerifyPointer or VerifyMaybeObjectPointer on it.
    cc_contents << "    " << object_type << "::" << verify_fn << "(isolate, " << value << ");\n";
...
void GenerateClassFieldVerifier(const std::string& class_name, const ClassType& class_type, const Field& f, std::ostream& h_contents, std::ostream& cc_contents)
{
    ...
    if (f.index) {
        ...
        cc_contents << "  std::tie(std::ignore, " << field_start_offset << ", " << length << ") = " << Callable::PrefixNameForCCOutput(class_type.GetSliceMacroName(f)) << "(o);\n";
...
void ImplementationVisitor::GenerateClassVerifiers(const std::string& output_directory)
{
    ...
    std::stringstream cc_contents;
    {
        ...
        for (const ClassType* type : TypeOracle::GetClasses()) {
            ...
            h_contents << "  static void " << method_name << "(" << name << " o, Isolate* isolate);\n";

            cc_contents << "void " << verifier_class << "::" << method_name << "(" << name << " o, Isolate* isolate) {\n";
...
void ImplementationVisitor::GenerateExportedMacrosAssembler(const std::string& output_directory)
{
    ...
    std::stringstream cc_contents;
    {
        ...
        for (auto& declarable : GlobalContext::AllDeclarables()) {
            ...
            cpp::Function f = GenerateFunction(&assembler, macro->ReadableName(), macro->signature(), macro->parameter_names(), false, &generated_parameter_names);
