...
class Block {
public:
    explicit Block(ControlFlowGraph* cfg, size_t id, base::Optional<Stack<const Type*>> input_types, bool is_deferred)
        : cfg_(cfg)
    ...
    bool IsComplete() const { return !instructions_.empty() && instructions_.back()->IsBlockTerminator(); }
    ...
    bool HasInputDefinitions() const { return input_definitions_ != base::nullopt; }
...
public:
    ...
    std::size_t ParameterCount() const { return start_ ? start_->InputTypes().Size() : 0; }
...
public:
    ...
    Block* NewBlock(base::Optional<Stack<const Type*>> input_types = base::nullopt, bool is_deferred = false) { return cfg_.NewBlock(std::move(input_types), is_deferred); }

    bool CurrentBlockIsComplete() const { return current_block_->IsComplete(); }
    bool CfgIsComplete() const
    {
        return std::all_of(cfg_.blocks().begin(), cfg_.blocks().end(), [this](Block* block) { return (cfg_.end() && *cfg_.end() == block) || block->IsComplete(); });
    }
    ...
    StackRange TopRange(size_t slot_count) const { return CurrentStack().TopRange(slot_count); }
