...
public:
    ...
    void PostNonNestableDelayedTask(std::unique_ptr<Task> task, double delay_in_seconds) override;
...
private:
    ...
    // holding the lock.
    void PostTaskLocked(std::unique_ptr<Task> task, Nestability nestability, const base::MutexGuard&);
    ...
    void PostDelayedTaskLocked(std::unique_ptr<Task> task, double delay_in_seconds, Nestability nestability, const base::MutexGuard&);

    // A caller of this function has to hold {lock_}. The {guard} parameter should
    // make sure that the caller is holding the lock.
    std::unique_ptr<Task> PopTaskFromDelayedQueueLocked(const base::MutexGuard&, Nestability* nestability);
    ...
    struct DelayedEntryCompare {
        bool operator()(const DelayedEntry& left, const DelayedEntry& right) const { return left.timeout_time > right.timeout_time; }
    };
