...
DefaultWorkerThreadsTaskRunner::DefaultWorkerThreadsTaskRunner(uint32_t thread_pool_size, TimeFunction time_function)
...
double DefaultWorkerThreadsTaskRunner::MonotonicallyIncreasingTime() { return time_function_(); }
...
void DefaultWorkerThreadsTaskRunner::PostDelayedTask(std::unique_ptr<Task> task, double delay_in_seconds)
{
    base::MutexGuard guard(&lock_);
    if (terminated_)
        return;
...
void DefaultWorkerThreadsTaskRunner::PostIdleTask(std::unique_ptr<IdleTask> task)
{
...
std::unique_ptr<Task> DefaultWorkerThreadsTaskRunner::GetNext() { return queue_.GetNext(); }
