...
class SharedToCounterMap : public base::TemplateHashMapImpl<SharedFunctionInfo, uint32_t, base::KeyEqualityMatcher<Object>, base::DefaultAllocationPolicy> {
public:
...
private:
    static uint32_t Hash(SharedFunctionInfo key) { return static_cast<uint32_t>(key.ptr()); }
...
public:
    ...
    bool HasNext() const { return read_index_ + 1 < static_cast<int>(function_->blocks.size()); }
...
bool HaveSameSourceRange(const CoverageBlock& lhs, const CoverageBlock& rhs) { return lhs.start == rhs.start && lhs.end == rhs.end; }
...
void CollectBlockCoverageInternal(CoverageFunction* function, SharedFunctionInfo info, debug::CoverageMode mode)
{
...
void CollectBlockCoverage(CoverageFunction* function, SharedFunctionInfo info, debug::CoverageMode mode)
{
...
void PrintBlockCoverage(const CoverageFunction* function, SharedFunctionInfo info, bool has_nonempty_source_range, bool function_is_relevant)
{
    DCHECK(v8_flags.trace_block_coverage);
    std::unique_ptr<char[]> function_name = function->name->ToCString(DISALLOW_NULLS, ROBUST_STRING_TRAVERSAL);
...
void CollectAndMaybeResetCounts(Isolate* isolate, SharedToCounterMap* counter_map, v8::debug::CoverageMode coverage_mode)
{
    ...
    case v8::debug::CoverageMode::kPreciseCount: {
        ...
        DCHECK(isolate->factory()->feedback_vectors_for_profiling_tools()->IsArrayList());
    ...
    case v8::debug::CoverageMode::kBestEffort: {
        ...
        for (HeapObject current_obj = heap_iterator.Next(); !current_obj.is_null(); current_obj = heap_iterator.Next()) {
            ...
            } else if (func.raw_feedback_cell().interrupt_budget() < v8_flags.interrupt_budget_for_feedback_allocation) {
...
std::unique_ptr<Coverage> Coverage::CollectBestEffort(Isolate* isolate) { return Collect(isolate, v8::debug::CoverageMode::kBestEffort); }
...
std::unique_ptr<Coverage> Coverage::Collect(Isolate* isolate, v8::debug::CoverageMode collectionMode)
{
    ...
    for (Script script = scriptIt.Next(); !script.is_null(); script = scriptIt.Next()) {
...
void Coverage::SelectMode(Isolate* isolate, debug::CoverageMode mode)
{
    ...
    case debug::CoverageMode::kPreciseCount: {
        ...
        {
            HeapObjectIterator heap_iterator(isolate->heap());
            for (HeapObject o = heap_iterator.Next(); !o.is_null(); o = heap_iterator.Next()) {
                if (o.IsJSFunction()) {
