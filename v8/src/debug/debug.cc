...
MaybeHandle<FixedArray> Debug::CheckBreakPointsForLocations(Handle<DebugInfo> debug_info, std::vector<BreakLocation>& break_locations, bool* has_break_points)
{
    ...
    for (size_t i = 0; i < break_locations.size(); i++) {
        ...
        if (!check_result.is_null()) {
            Handle<FixedArray> break_points_current_hit = check_result.ToHandleChecked();
...
JSGeneratorObject BreakLocation::GetGeneratorObjectForSuspendedFrame(JavaScriptFrame* frame) const
{
    DCHECK(IsSuspend());
    DCHECK_GE(generator_obj_reg_index_, 0);

    Object generator_obj = UnoptimizedFrame::cast(frame)->ReadInterpreterRegister(generator_obj_reg_index_);

    return JSGeneratorObject::cast(generator_obj);
}

int BreakLocation::BreakIndexFromCodeOffset(Handle<DebugInfo> debug_info, Handle<AbstractCode> abstract_code, int offset)
{
...
BreakIterator::BreakIterator(Handle<DebugInfo> debug_info)
    ...
    , break_index_(-1)
    , source_position_iterator_(debug_info->DebugBytecodeArray().SourcePositionTable())
{
...
{
    Handle<AbstractCode> code(AbstractCode::cast(debug_info_->DebugBytecodeArray()), isolate());
    DebugBreakType type = GetDebugBreakType();
...
void Debug::Iterate(RootVisitor* v, ThreadLocal* thread_local_data)
{
    ...
    v->VisitRootPointer(Root::kDebug, nullptr, FullObjectSlot(&thread_local_data->promise_stack_));
}
...
void Debug::OnInstrumentationBreak()
{
    ...
    debug_delegate_->BreakOnInstrumentation(v8::Utils::ToLocal(native_context), kInstrumentationId);
}
...
void Debug::Break(JavaScriptFrame* frame, Handle<JSFunction> break_target)
{
    ...
    if (!break_points_hit.is_null() || break_on_next_function_call() || scheduled_break_on_function_call()) {
        StepAction lastStepAction = last_step_action();
        DCHECK_IMPLIES(scheduled_break_on_function_call(), lastStepAction == StepNone);
        ...
        OnDebugBreak(!break_points_hit.is_null() ? break_points_hit.ToHandleChecked() : isolate_->factory()->empty_fixed_array(), lastStepAction, break_reasons);
        return;
    }
...
bool Debug::IsBreakOnInstrumentation(Handle<DebugInfo> debug_info, const BreakLocation& location)
{
    ...
    Handle<Object> break_points = debug_info->GetBreakPoints(isolate_, location.position());
    DCHECK(!break_points->IsUndefined(isolate_));
    if (!break_points->IsFixedArray()) {
        const Handle<BreakPoint> break_point = Handle<BreakPoint>::cast(break_points);
        return break_point->id() == kInstrumentationId;
    }

    Handle<FixedArray> array(FixedArray::cast(*break_points), isolate_);
    for (int i = 0; i < array->length(); ++i) {
        const Handle<BreakPoint> break_point = Handle<BreakPoint>::cast(handle(array->get(i), isolate_));
...
bool Debug::IsMutedAtCurrentLocation(JavaScriptFrame* frame)
{
    ...
    HandleScope scope(isolate_);
    bool has_break_points;
    MaybeHandle<FixedArray> checked = GetHitBreakpointsAtCurrentStatement(frame, &has_break_points);
    return has_break_points && checked.is_null();
}

MaybeHandle<FixedArray> Debug::GetHitBreakpointsAtCurrentStatement(JavaScriptFrame* frame, bool* has_break_points)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    FrameSummary summary = FrameSummary::GetTop(frame);
    ...
    return CheckBreakPointsForLocations(debug_info, break_locations, has_break_points);
}

// Check whether a single break point object is triggered.
bool Debug::CheckBreakPoint(Handle<BreakPoint> break_point, bool is_break_at_entry)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    HandleScope scope(isolate_);
...
bool Debug::SetBreakpoint(Handle<SharedFunctionInfo> shared, Handle<BreakPoint> break_point, int* source_position)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    HandleScope scope(isolate_);
...
bool Debug::SetBreakPointForScript(Handle<Script> script, Handle<String> condition, int* source_position, int* id)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
...
void Debug::ClearBreakPoint(Handle<BreakPoint> break_point)
{
    ...
    for (DebugInfoListNode* node = debug_info_list_; node != nullptr; node = node->next()) {
        if (!node->debug_info()->HasBreakInfo())
            continue;
        Handle<Object> result = DebugInfo::FindBreakPointInfo(isolate_, node->debug_info(), break_point);
        if (result->IsUndefined(isolate_))
            continue;
...
bool Debug::SetBreakpointForFunction(Handle<SharedFunctionInfo> shared, Handle<String> condition, int* id, BreakPointKind kind)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    if (shared->HasWasmExportedFunctionData()) {
        ...
        Handle<WasmInstanceObject> wasm_instance(shared->wasm_exported_function_data().instance(), isolate_);
        Handle<Script> script(Script::cast(wasm_instance->module_object().script()), isolate_);
...
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    Handle<BreakPoint> breakpoint = isolate_->factory()->NewBreakPoint(id, isolate_->factory()->empty_string());
...
void Debug::SetInstrumentationBreakpointForWasmScript(Handle<Script> script, int* id)
{
    ...
    *id = kInstrumentationId;
    ...
    Handle<BreakPoint> break_point = isolate_->factory()->NewBreakPoint(*id, isolate_->factory()->empty_string());
    RecordWasmScriptWithBreakpoints(script);
...
void Debug::RecordWasmScriptWithBreakpoints(Handle<Script> script)
{
    ...
    Handle<WeakArrayList> new_list = WeakArrayList::Append(isolate_, wasm_scripts_with_break_points_, MaybeObjectHandle { script });
...
MaybeHandle<FixedArray> Debug::GetHitBreakPoints(Handle<DebugInfo> debug_info, int position, bool* has_break_points)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    *has_break_points = false;
    for (int i = 0; i < num_objects; ++i) {
        Handle<BreakPoint> break_point = Handle<BreakPoint>::cast(handle(array->get(i), isolate_));
        *has_break_points |= break_point->id() != kInstrumentationId;
        if (CheckBreakPoint(break_point, is_break_at_entry)) {
...
void Debug::PrepareStepInSuspendedGenerator()
{
    ...
    UpdateHookOnFunctionCall();
    Handle<JSFunction> function(JSGeneratorObject::cast(thread_local_.suspended_generator_).function(), isolate_);
    FloodWithOneShot(Handle<SharedFunctionInfo>(function->shared(), isolate_));
    clear_suspended_generator();
}
...
void Debug::PrepareStep(StepAction step_action)
{
    ...
    if (frame->is_java_script()) {
        ...
        thread_local_.last_statement_position_ = summary.abstract_code()->SourceStatementPosition(isolate_, summary.code_offset());
    ...
    case StepOut: {
        ...
        if (!shared.is_null()) {
            ...
            if (IsAsyncFunction(shared->kind())) {
                ...
                Handle<JSReceiver> return_value(JSReceiver::cast(thread_local_.return_value_), isolate_);
                Handle<Object> awaited_by = JSReceiver::GetDataProperty(isolate_, return_value, isolate_->factory()->promise_awaited_by_symbol());
...
Handle<Object> Debug::GetSourceBreakLocations(Isolate* isolate, Handle<SharedFunctionInfo> shared)
{
        ...
        for (JavaScriptFrameIterator it(isolate, top); !it.done(); it.Advance()) {
            ...
            if (it.frame()->type() == StackFrame::BASELINE) {
                ...
                if (bytecode_offset == kFunctionEntryBytecodeOffset) {
                    advance = BUILTIN_CODE(isolate, BaselineOutOfLinePrologueDeopt)->InstructionStart();
                } else {
                    advance = BUILTIN_CODE(isolate, InterpreterEnterAtNextBytecode)->InstructionStart();
                }
                ...
                if (builtin == Builtin::kBaselineOrInterpreterEnterAtBytecode || builtin == Builtin::kBaselineOrInterpreterEnterAtNextBytecode) {
                    Address* pc_addr = frame->pc_address();
                    Builtin advance = builtin == Builtin::kBaselineOrInterpreterEnterAtBytecode ? Builtin::kInterpreterEnterAtBytecode : Builtin::kInterpreterEnterAtNextBytecode;
                    Address advance_pc = isolate->builtins()->code(advance).InstructionStart();
...
void Debug::PrepareFunctionForDebugExecution(Handle<SharedFunctionInfo> shared)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    if (debug_info->CanBreakAtEntry()) {
        ...
        RedirectActiveFunctions redirect_visitor(*shared, RedirectActiveFunctions::Mode::kUseDebugBytecode);
...
void Debug::InstallDebugBreakTrampoline()
{
    ...
    for (DebugInfoListNode* current = debug_info_list_; current != nullptr; current = current->next()) {
...
bool Debug::GetPossibleBreakpoints(Handle<Script> script, int start_position, int end_position, bool restrict_to_function, std::vector<BreakLocation>* locations)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
...
public:
    ...
    void NewCandidate(SharedFunctionInfo shared, JSFunction closure = JSFunction())
    {
...
namespace {
SharedFunctionInfo FindSharedFunctionInfoCandidate(int position, Handle<Script> script, Isolate* isolate)
{
    SharedFunctionInfoFinder finder(position);
    SharedFunctionInfo::ScriptIterator iterator(isolate, *script);
    for (SharedFunctionInfo info = iterator.Next(); !info.is_null(); info = iterator.Next()) {
...
Handle<SharedFunctionInfo> Debug::FindClosestSharedFunctionInfoFromPosition(int position, Handle<Script> script, Handle<SharedFunctionInfo> outer_shared)
{
    ...
    int closest_position = FindBreakablePosition(Handle<DebugInfo>(outer_shared->GetDebugInfo(), isolate_), position);
...
bool Debug::FindSharedFunctionInfosIntersectingRange(Handle<Script> script, int start_position, int end_position, std::vector<Handle<SharedFunctionInfo>>* intersecting_shared)
{
    ...
    while (true) {
        ...
        if (!triedTopLevelCompile && !candidateSubsumesRange && script->shared_function_info_count() > 0) {
            ...
            const bool topLevelInfoExists = maybeToplevel->GetHeapObject(&heap_object) && !heap_object.IsUndefined();
...
void Debug::CreateBreakInfo(Handle<SharedFunctionInfo> shared)
{
    ...
    Handle<FixedArray> break_points(factory->NewFixedArray(DebugInfo::kEstimatedNofBreakPointsInFunction));
...
Handle<DebugInfo> Debug::GetOrCreateDebugInfo(Handle<SharedFunctionInfo> shared)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    if (shared->HasDebugInfo())
...
void Debug::InstallCoverageInfo(Handle<SharedFunctionInfo> shared, Handle<CoverageInfo> coverage_info)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
...
{
    ClearAllDebugInfos([=](Handle<DebugInfo> info) { info->ClearCoverageInfo(isolate_); });
}

void Debug::ClearAllDebuggerHints()
{
    ClearAllDebugInfos([=](Handle<DebugInfo> info) { info->set_debugger_hints(0); });
}

void Debug::FindDebugInfo(Handle<DebugInfo> debug_info, DebugInfoListNode** prev, DebugInfoListNode** curr)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    HandleScope scope(isolate_);
    *prev = nullptr;
...
void Debug::FreeDebugInfoListNode(DebugInfoListNode* prev, DebugInfoListNode* node)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    Handle<DebugInfo> debug_info(node->debug_info());
    debug_info->shared().set_script_or_debug_info(debug_info->script(), kReleaseStore);
...
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    isolate_->heap()->CollectAllGarbage(Heap::kNoGCFlags, GarbageCollectionReason::kDebugger);
    Factory* factory = isolate_->factory();
    ...
    int length = 0;
    {
        ...
        for (Script script = iterator.Next(); !script.is_null(); script = iterator.Next()) {
...
base::Optional<Object> Debug::OnThrow(Handle<Object> exception)
{
    ...
    OnException(exception, maybe_promise, maybe_promise->IsJSPromise() ? v8::debug::kPromiseRejection : v8::debug::kException);
...
void Debug::OnPromiseReject(Handle<Object> promise, Handle<Object> value)
{
    ...
    if (!promise->IsJSObject() || JSReceiver::GetDataProperty(isolate_, Handle<JSObject>::cast(promise), key)->IsUndefined(isolate_)) {
...
void Debug::OnException(Handle<Object> exception, Handle<Object> promise, v8::debug::ExceptionType exception_type)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    if (!break_on_exception_ && !break_on_uncaught_exception_)
        return;
    ...
    if (promise->IsJSObject()) {
        ...
        Handle<Symbol> key = isolate_->factory()->promise_debug_marker_symbol();
        Object::SetProperty(isolate_, jsobject, key, key, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)).Assert();
...

    {
        ...
        Handle<Context> native_context(isolate_->native_context());
        debug_delegate_->ExceptionThrown(v8::Utils::ToLocal(native_context), v8::Utils::ToLocal(exception), v8::Utils::ToLocal(promise), uncaught, exception_type);
    }
}
...
void Debug::OnDebugBreak(Handle<FixedArray> break_points_hit, StepAction lastStepAction, v8::debug::BreakReasons break_reasons)
{
    ...
    }
    {
        ...
        debug_delegate_->BreakProgramRequested(v8::Utils::ToLocal(native_context), inspector_break_points_hit, break_reasons);
    }
}
...
bool Debug::IsBlackboxed(Handle<SharedFunctionInfo> shared)
{
    ...
    if (!debug_info->computed_debug_is_blackboxed()) {
        ...
        if (!is_blackboxed) {
            ...
            {
                RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebuggerCallback);
                is_blackboxed = debug_delegate_->IsFunctionBlackboxed(ToApiHandle<debug::Script>(script), start, end);
            }
        }
...
bool Debug::ShouldBeSkipped()
{
    ...
    {
        RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebuggerCallback);
        return debug_delegate_->ShouldBeSkipped(ToApiHandle<debug::Script>(script), line, column);
    }
}
...
void Debug::OnCompileError(Handle<Script> script) { ProcessCompileEvent(true, script); }

void Debug::OnAfterCompile(Handle<Script> script) { ProcessCompileEvent(false, script); }
...
void Debug::UpdateHookOnFunctionCall()
{
    ...
    hook_on_function_call_
        = thread_local_.last_step_action_ == StepInto || isolate_->debug_execution_mode() == DebugInfo::kSideEffects || thread_local_.break_on_next_function_call_;
}
...
void Debug::HandleDebugBreak(IgnoreBreakMode ignore_break_mode, v8::debug::BreakReasons break_reasons)
{
    ...
    MaybeHandle<FixedArray> break_points;
    {
        StackTraceFrameIterator it(isolate_);
        DCHECK(!it.done());
        JavaScriptFrame* frame = it.frame()->is_java_script() ? JavaScriptFrame::cast(it.frame()) : nullptr;
        if (frame && frame->function().IsJSFunction()) {
            ...
            if (function->shared().HasBreakInfo()) {
                ...
                bool has_break_points;
                break_points = CheckBreakPointsForLocations(debug_info, break_locations, &has_break_points);
...
void Debug::PrintBreakLocation()
{
    ...
    if (script_obj->IsScript()) {
        ...
        Handle<FixedArray> line_ends(FixedArray::cast(script->line_ends()), isolate_);
...
ReturnValueScope::~ReturnValueScope() { debug_->set_return_value(*return_value_); }
...
void Debug::StartSideEffectCheckMode()
{
    ...
    Handle<FixedArray> array(isolate_->native_context()->regexp_last_match_info(), isolate_);
...
bool Debug::PerformSideEffectCheck(Handle<JSFunction> function, Handle<Object> receiver)
{
    ...
    if (!function->is_compiled() && !Compiler::Compile(isolate_, function, Compiler::KEEP_EXCEPTION, &is_compiled_scope)) {
        return false;
    }
    DCHECK(is_compiled_scope.is_compiled());
    Handle<SharedFunctionInfo> shared(function->shared(), isolate_);
...
Handle<Object> Debug::return_value_handle() { return handle(thread_local_.return_value_, isolate_); }

bool Debug::PerformSideEffectCheckForCallback(Handle<Object> callback_info, Handle<Object> receiver, Debug::AccessorKind accessor_kind)
{
    RCS_SCOPE(isolate_, RuntimeCallCounterId::kDebugger);
    ...
    if (!callback_info.is_null()) {
        if (callback_info->IsAccessorInfo()) {
            ...
            switch (accessor_kind == kSetter ? info.setter_side_effect_type() : info.getter_side_effect_type()) {
