...
Handle<String> ToInternalString(StringBuilder& sb, Isolate* isolate) { return isolate->factory()->InternalizeString(base::VectorOf(sb.start(), sb.length())); }
...
Handle<Map> GetOrCreateDebugProxyMap(Isolate* isolate, DebugProxyId id, v8::Local<v8::FunctionTemplate> (*create_template_fn)(v8::Isolate*), bool make_non_extensible = true)
{
    ...
    auto fun = ApiNatives::InstantiateFunction(Utils::OpenHandle(*tmp)).ToHandleChecked();
...
template <typename T, DebugProxyId id, typename Provider> struct IndexedDebugProxy {
    ...
    template <typename V> static Isolate* GetIsolate(const PropertyCallbackInfo<V>& info) { return reinterpret_cast<Isolate*>(info.GetIsolate()); }

    template <typename V> static Handle<JSObject> GetHolder(const PropertyCallbackInfo<V>& info) { return Handle<JSObject>::cast(Utils::OpenHandle(*info.Holder())); }

    static Handle<Provider> GetProvider(Handle<JSObject> holder, Isolate* isolate) { return handle(Provider::cast(holder->GetEmbedderField(kProviderField)), isolate); }

    template <typename V> static Handle<Provider> GetProvider(const PropertyCallbackInfo<V>& info) { return GetProvider(GetHolder(info), GetIsolate(info)); }

    static void IndexedGetter(uint32_t index, const PropertyCallbackInfo<v8::Value>& info)
    {
    ...
    static void IndexedDescriptor(uint32_t index, const PropertyCallbackInfo<v8::Value>& info)
    {
        auto isolate = GetIsolate(info);
        auto provider = GetProvider(info);
        if (index < T::Count(isolate, provider)) {
    ...
    static void IndexedQuery(uint32_t index, const PropertyCallbackInfo<v8::Integer>& info)
    {
...
template <typename T, DebugProxyId id, typename Provider = WasmInstanceObject> struct NamedDebugProxy : IndexedDebugProxy<T, id, Provider> {
    ...
    static void IndexedEnumerator(const PropertyCallbackInfo<v8::Array>& info) { info.GetReturnValue().Set(v8::Array::New(info.GetIsolate())); }
    ...
    template <typename V> static base::Optional<uint32_t> FindName(Local<v8::Name> name, const PropertyCallbackInfo<V>& info)
    {
        if (!name->IsString())
            return {};
    ...
    static void NamedGetter(Local<v8::Name> name, const PropertyCallbackInfo<v8::Value>& info)
    {
        if (auto index = FindName(name, info))
            T::IndexedGetter(*index, info);
    }

    static void NamedQuery(Local<v8::Name> name, const PropertyCallbackInfo<v8::Integer>& info)
    {
        if (auto index = FindName(name, info))
            T::IndexedQuery(*index, info);
    }

    static void NamedDescriptor(Local<v8::Name> name, const PropertyCallbackInfo<v8::Value>& info)
    {
        if (auto index = FindName(name, info))
            T::IndexedDescriptor(*index, info);
    }
...
struct FunctionsProxy : NamedDebugProxy<FunctionsProxy, kFunctionsProxy> {
    ...
    static uint32_t Count(Isolate* isolate, Handle<WasmInstanceObject> instance) { return static_cast<uint32_t>(instance->module()->functions.size()); }

    static Handle<Object> Get(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index)
    {
        return handle(WasmInstanceObject::GetOrCreateWasmInternalFunction(isolate, instance, index)->external(), isolate);
    }

    static Handle<String> GetName(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index) { return GetWasmFunctionDebugName(isolate, instance, index); }
};
...
struct GlobalsProxy : NamedDebugProxy<GlobalsProxy, kGlobalsProxy> {
    ...
    static uint32_t Count(Isolate* isolate, Handle<WasmInstanceObject> instance) { return static_cast<uint32_t>(instance->module()->globals.size()); }

    static Handle<Object> Get(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index)
    {
        Handle<WasmModuleObject> module(instance->module_object(), isolate);
        return WasmValueObject::New(isolate, WasmInstanceObject::GetGlobalValue(instance, instance->module()->globals[index]), module);
    }

    static Handle<String> GetName(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index)
    {
...
struct MemoriesProxy : NamedDebugProxy<MemoriesProxy, kMemoriesProxy> {
    ...
    static uint32_t Count(Isolate* isolate, Handle<WasmInstanceObject> instance) { return instance->has_memory_object() ? 1 : 0; }

    static Handle<Object> Get(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index) { return handle(instance->memory_object(), isolate); }
...
struct TablesProxy : NamedDebugProxy<TablesProxy, kTablesProxy> {
    ...
    static uint32_t Count(Isolate* isolate, Handle<WasmInstanceObject> instance) { return instance->tables().length(); }

    static Handle<Object> Get(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t index) { return handle(instance->tables().get(index), isolate); }
...
struct LocalsProxy : NamedDebugProxy<LocalsProxy, kLocalsProxy, FixedArray> {
    ...
    static uint32_t Count(Isolate* isolate, Handle<FixedArray> values) { return values->length() - 2; }

    static Handle<Object> Get(Isolate* isolate, Handle<FixedArray> values, uint32_t index) { return handle(values->get(index), isolate); }

    static Handle<String> GetName(Isolate* isolate, Handle<FixedArray> values, uint32_t index)
    {
        uint32_t count = Count(isolate, values);
        auto native_module = WasmModuleObject::cast(values->get(count + 0)).native_module();
...
struct StackProxy : IndexedDebugProxy<StackProxy, kStackProxy, FixedArray> {
    ...
    {
        auto isolate = frame->isolate();
        auto debug_info = frame->wasm_instance().module_object().native_module()->GetDebugInfo();
    ...
    static uint32_t Count(Isolate* isolate, Handle<FixedArray> values) { return values->length(); }
...
Handle<FixedArray> GetOrCreateInstanceProxyCache(Isolate* isolate, Handle<WasmInstanceObject> instance)
{
    ...
    if (!Object::GetProperty(isolate, instance, symbol).ToHandle(&cache) || cache->IsUndefined(isolate)) {
    ...
    static MaybeHandle<Object> GetNamedProperty(Isolate* isolate, Handle<JSObject> receiver, Handle<String> name)
    {
        if (name->length() != 0 && name->Get(0) == '$') {
            ...
            for (auto delegate_name : kDelegateNames) {
                ...
                ASSIGN_RETURN_ON_EXCEPTION(isolate, delegate, JSObject::GetProperty(isolate, receiver, delegate_name), Object);
                if (!delegate->IsUndefined(isolate)) {
                    Handle<Object> value;
                    ASSIGN_RETURN_ON_EXCEPTION(isolate, value, Object::GetProperty(isolate, delegate, name), Object);
...
public:
    ...
    v8::Local<v8::Value> GetFunctionDebugName() override { return Utils::ToLocal(frame_->isolate()->factory()->empty_string()); }
    ...
    bool SetVariableValue(v8::Local<v8::String> name, v8::Local<v8::Value> value) override { return false; }
...
// static
Handle<WasmValueObject> WasmValueObject::New(Isolate* isolate, Handle<String> type, Handle<Object> value)
{
    auto maps = GetOrCreateDebugMaps(isolate);
    if (maps->is_the_hole(isolate, kWasmValueMapIndex)) {
        Handle<Map> map = isolate->factory()->NewMap(WASM_VALUE_OBJECT_TYPE, WasmValueObject::kSize, TERMINAL_FAST_ELEMENTS_KIND, 2);
        ...
        { // value
            Descriptor d = Descriptor::DataField(
                isolate, isolate->factory()->InternalizeString(base::StaticCharVector("value")), WasmValueObject::kValueIndex, FROZEN, Representation::Tagged());
            map->AppendDescriptor(isolate, &d);
        }
        map->set_is_extensible(false);
        maps->set(kWasmValueMapIndex, *map);
    }
    Handle<Map> value_map = handle(Map::cast(maps->get(kWasmValueMapIndex)), isolate);
    Handle<WasmValueObject> object = Handle<WasmValueObject>::cast(isolate->factory()->NewJSObjectFromMap(value_map));
...
struct StructProxy : NamedDebugProxy<StructProxy, kStructProxy, FixedArray> {
    ...
    static Handle<JSObject> Create(Isolate* isolate, Handle<WasmStruct> value, Handle<WasmModuleObject> module)
    {
    ...
    static uint32_t Count(Isolate* isolate, Handle<FixedArray> data) { return WasmStruct::cast(data->get(kObjectIndex)).type()->field_count(); }

    static Handle<Object> Get(Isolate* isolate, Handle<FixedArray> data, uint32_t index)
    {
        Handle<WasmStruct> obj(WasmStruct::cast(data->get(kObjectIndex)), isolate);
        Handle<WasmModuleObject> module(WasmModuleObject::cast(data->get(kModuleIndex)), isolate);
        return WasmValueObject::New(isolate, obj->GetFieldValue(index), module);
    }

    static Handle<String> GetName(Isolate* isolate, Handle<FixedArray> data, uint32_t index)
    {
        wasm::NativeModule* native_module = WasmModuleObject::cast(data->get(kModuleIndex)).native_module();
...
struct ArrayProxy : IndexedDebugProxy<ArrayProxy, kArrayProxy, FixedArray> {
    ...
    static Handle<JSObject> Create(Isolate* isolate, Handle<WasmArray> value, Handle<WasmModuleObject> module)
    {
        Handle<FixedArray> data = isolate->factory()->NewFixedArray(kLength);
        data->set(kObjectIndex, *value);
        data->set(kModuleIndex, *module);
    ...
    static uint32_t Count(Isolate* isolate, Handle<FixedArray> data) { return WasmArray::cast(data->get(kObjectIndex)).length(); }
...
Handle<WasmValueObject> WasmValueObject::New(Isolate* isolate, const wasm::WasmValue& value, Handle<WasmModuleObject> module_object)
{
    ...
    case wasm::kRef: {
        ...
        if (ref->IsWasmStruct()) {
            ...
            wasm::ValueType type = wasm::ValueType::FromIndex(wasm::ValueKind::kRef, type_info.type_index());
            Handle<WasmModuleObject> module(type_info.instance().module_object(), isolate);
            ...
            // available.
            if (module_object.is_null() && internal_fct->ref().IsWasmInstanceObject()) {
                module_object = handle(WasmInstanceObject::cast(internal_fct->ref()).module_object(), isolate);
            }
            t = GetRefTypeName(isolate, value.type(), module_object);
        } else if (ref->IsJSFunction() || ref->IsSmi() || ref->IsNull() || ref->IsString() || value.type().is_reference_to(wasm::HeapType::kExtern)
            ...
            int len = SNPrintF(error, "unimplemented object type: %d", HeapObject::cast(*ref).map().instance_type());
            t = GetRefTypeName(isolate, value.type(), module_object);
...
Handle<JSObject> GetWasmDebugProxy(WasmFrame* frame) { return ContextProxy::Create(frame); }

std::unique_ptr<debug::ScopeIterator> GetWasmScopeIterator(WasmFrame* frame) { return std::make_unique<DebugWasmScopeIterator>(frame); }

Handle<String> GetWasmFunctionDebugName(Isolate* isolate, Handle<WasmInstanceObject> instance, uint32_t func_index)
{
    Handle<WasmModuleObject> module_object(instance->module_object(), isolate);
    wasm::NamesProvider* names = module_object->native_module()->GetNamesProvider();
    StringBuilder sb;
    wasm::NamesProvider::FunctionNamesBehavior behavior = module_object->is_asm_js() ? wasm::NamesProvider::kWasmInternal : wasm::NamesProvider::kDevTools;
...
Handle<ArrayList> AddWasmInstanceObjectInternalProperties(Isolate* isolate, Handle<ArrayList> result, Handle<WasmInstanceObject> instance)
{
    ...
    if (FunctionsProxy::Count(isolate, instance) != 0) {
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[Functions]]"), GetOrCreateInstanceProxy<FunctionsProxy>(isolate, instance));
    }

    if (GlobalsProxy::Count(isolate, instance) != 0) {
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[Globals]]"), GetOrCreateInstanceProxy<GlobalsProxy>(isolate, instance));
    }

    if (MemoriesProxy::Count(isolate, instance) != 0) {
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[Memories]]"), GetOrCreateInstanceProxy<MemoriesProxy>(isolate, instance));
    }

    if (TablesProxy::Count(isolate, instance) != 0) {
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[Tables]]"), GetOrCreateInstanceProxy<TablesProxy>(isolate, instance));
    }
...
Handle<ArrayList> AddWasmTableObjectInternalProperties(Isolate* isolate, Handle<ArrayList> result, Handle<WasmTableObject> table)
{
    ...
    for (int i = 0; i < length; ++i) {
        ...
        if (table->instance().IsWasmInstanceObject()) {
            module = Handle<WasmModuleObject>(WasmInstanceObject::cast(table->instance()).module_object(), isolate);
        }
        Handle<Object> debug_value = WasmValueObject::New(isolate, wasm_value, module);
    ...
    Handle<String> entries_string = isolate->factory()->NewStringFromStaticChars("[[Entries]]");
