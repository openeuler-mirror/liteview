...
Handle<Name> DebugPropertyIterator::raw_name() const
{
    ...
    if (stage_ == kExoticIndices) {
        ...
        return Handle<Name>::cast(FixedArray::get(*current_keys_, static_cast<int>(current_key_index_), isolate_));
    }
}

v8::Local<v8::Name> DebugPropertyIterator::name() const { return Utils::ToLocal(raw_name()); }
...
bool DebugPropertyIterator::FillKeysForCurrentPrototypeAndStage()
{
    ...
    Handle<JSReceiver> receiver = PrototypeIterator::GetCurrent<JSReceiver>(prototype_iterator_);
    if (stage_ == kExoticIndices) {
        if (skip_indices_ || !receiver->IsJSTypedArray())
            return true;
        Handle<JSTypedArray> typed_array = Handle<JSTypedArray>::cast(receiver);
        current_keys_length_ = typed_array->WasDetached() ? 0 : typed_array->length();
        return true;
    }
    PropertyFilter filter = stage_ == kEnumerableStrings ? ENUMERABLE_STRINGS : ALL_PROPERTIES;
    if (KeyAccumulator::GetKeys(isolate_, receiver, KeyCollectionMode::kOwnOnly, filter, GetKeysConversion::kConvertToString, false, skip_indices_ || receiver->IsJSTypedArray())
...
bool DebugPropertyIterator::should_move_to_next_stage() const { return !is_done_ && current_key_index_ >= current_keys_length_; }
