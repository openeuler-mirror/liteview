...
namespace {
static MaybeHandle<SharedFunctionInfo> GetFunctionInfo(Isolate* isolate, Handle<String> source, REPLMode repl_mode)
{
    ...
    return Compiler::GetSharedFunctionInfoForScript(isolate, source, script_details, ScriptCompiler::kNoCompileOptions, ScriptCompiler::kNoCacheNoReason, NOT_NATIVES_CODE);
}
} // namespace
...
MaybeHandle<Object> DebugEvaluate::Global(Isolate* isolate, Handle<String> source, debug::EvaluateGlobalMode mode, REPLMode repl_mode)
{
    ...
    Handle<JSFunction> fun = Factory::JSFunctionBuilder { isolate, shared_info, context }.Build();

    return Global(isolate, fun, mode, repl_mode);
}

MaybeHandle<Object> DebugEvaluate::Global(Isolate* isolate, Handle<JSFunction> function, debug::EvaluateGlobalMode mode, REPLMode repl_mode)
{
    // Disable breaks in side-effect free mode.
    DisableBreak disable_break_scope(isolate->debug(), mode == debug::EvaluateGlobalMode::kDisableBreaks || mode == debug::EvaluateGlobalMode::kDisableBreaksAndThrowOnSideEffect);
    ...
    Handle<FixedArray> host_defined_options(Script::cast(function->shared().script()).host_defined_options(), isolate);
...
MaybeHandle<Object> DebugEvaluate::WithTopmostArguments(Isolate* isolate, Handle<String> source)
{
    ...
    Handle<Context> native_context(Context::cast(it.frame()->context()).native_context(), isolate);
    ...
    Isolate* isolate, Handle<SharedFunctionInfo> outer_info, Handle<Context> context, Handle<Object> receiver, Handle<String> source, bool throw_on_side_effect)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, eval_fun,
        Compiler::GetFunctionFromEval(
            source, outer_info, context, LanguageMode::kSloppy, NO_PARSE_RESTRICTION, kNoSourcePosition, kNoSourcePosition, kNoSourcePosition, ParsingWhileDebugging::kYes),
        Object);
...
Handle<SharedFunctionInfo> DebugEvaluate::ContextBuilder::outer_info() const { return handle(frame_inspector_.GetFunction()->shared(), isolate_); }
...
DebugEvaluate::ContextBuilder::ContextBuilder(Isolate* isolate, JavaScriptFrame* frame, int inlined_jsframe_index)
    ...
    , scope_iterator_(
          isolate, &frame_inspector_, v8_flags.experimental_reuse_locals_blocklists ? ScopeIterator::ReparseStrategy::kScriptIfNeeded : ScopeIterator::ReparseStrategy::kScript)
{
    ...
    for (; !scope_iterator_.Done(); scope_iterator_.Next()) {
        ...
        if (scope_iterator_.InInnerScope() && (scope_type == ScopeIterator::ScopeTypeLocal || scope_iterator_.DeclaresLocals(ScopeIterator::Mode::STACK))) {
    ...
    Handle<ScopeInfo> scope_info = evaluation_context_->IsNativeContext() ? Handle<ScopeInfo>::null() : handle(evaluation_context_->scope_info(), isolate);
    for (auto rit = context_chain_.rbegin(); rit != context_chain_.rend(); rit++) {
        ...
        if (v8_flags.experimental_reuse_locals_blocklists) {
            if (rit == context_chain_.rbegin()) {
                ...
                isolate_->LocalsBlockListCacheSet(scope_info, Handle<ScopeInfo>::null(), Handle<StringSet>::cast(block_list));
            }
        } else if (!element.blocklist.is_null()) {
            CHECK(!v8_flags.experimental_reuse_locals_blocklists);
            scope_info = ScopeInfo::RecreateWithBlockList(isolate, scope_info, element.blocklist);
        }
...
void DebugEvaluate::ContextBuilder::UpdateValues()
{
    ...
    for (ContextChainElement& element : context_chain_) {
        if (!element.materialized_object.is_null()) {
            Handle<FixedArray> keys = KeyAccumulator::GetKeys(isolate_, element.materialized_object, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS).ToHandleChecked();
...
// static
DebugInfo::SideEffectState DebugEvaluate::FunctionGetSideEffectState(Isolate* isolate, Handle<SharedFunctionInfo> info)
{
    if (v8_flags.trace_side_effect_free_debug_evaluate) {
    ...
    if (info->HasBytecodeArray()) {
        ...
        if (info->GetCode().is_builtin()) {
            return info->GetCode().builtin_id() == Builtin::kHandleApiCall ? DebugInfo::kHasNoSideEffect : DebugInfo::kHasSideEffects;
        }
    } else {
...
void DebugEvaluate::VerifyTransitiveBuiltins(Isolate* isolate)
{
    ...
    for (Builtin caller = Builtins::kFirst; caller <= Builtins::kLast; ++caller) {
        ...
        for (RelocIterator it(code, mode); !it.done(); it.next()) {
            ...
            CodeLookupResult lookup_result = isolate->heap()->GcSafeFindCodeForInnerPointer(rinfo->target_address());
    ...
    CHECK(!failed);
#if defined(V8_TARGET_ARCH_PPC) || defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_MIPS64) || defined(V8_TARGET_ARCH_RISCV32) || defined(V8_TARGET_ARCH_RISCV64)
