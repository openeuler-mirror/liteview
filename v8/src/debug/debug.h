...
private:
    ...
    static int BreakIndexFromCodeOffset(Handle<DebugInfo> debug_info, Handle<AbstractCode> abstract_code, int offset);
...
public:
    ...
    base::Optional<Object> OnThrow(Handle<Object> exception) V8_WARN_UNUSED_RESULT;
    ...
    bool SetBreakpoint(Handle<SharedFunctionInfo> shared, Handle<BreakPoint> break_point, int* source_position);
    ...
    bool SetBreakpointForFunction(Handle<SharedFunctionInfo> shared, Handle<String> condition, int* id, BreakPointKind kind = kRegular);
    ...
    MaybeHandle<FixedArray> GetHitBreakPoints(Handle<DebugInfo> debug_info, int position, bool* has_break_points);
    ...
    static Handle<Object> GetSourceBreakLocations(Isolate* isolate, Handle<SharedFunctionInfo> shared);
    ...
    inline bool in_debug_scope() const { return !!base::Relaxed_Load(&thread_local_.current_debug_scope_); }
    inline bool needs_check_on_function_call() const { return hook_on_function_call_; }
    ...
    Address hook_on_function_call_address() { return reinterpret_cast<Address>(&hook_on_function_call_); }

    Address suspended_generator_address() { return reinterpret_cast<Address>(&thread_local_.suspended_generator_); }

    StepAction last_step_action() { return thread_local_.last_step_action_; }
    bool break_on_next_function_call() const { return thread_local_.break_on_next_function_call_; }

    bool scheduled_break_on_function_call() const { return thread_local_.scheduled_break_on_next_function_call_; }

    bool IsRestartFrameScheduled() const { return thread_local_.restart_frame_id_ != StackFrameId::NO_ID; }
    bool ShouldRestartFrame(StackFrameId id) const { return IsRestartFrameScheduled() && thread_local_.restart_frame_id_ == id; }
    void clear_restart_frame()
    {
        thread_local_.restart_frame_id_ = StackFrameId::NO_ID;
        thread_local_.restart_inline_frame_index_ = -1;
    }
    int restart_inline_frame_index() const { return thread_local_.restart_inline_frame_index_; }
...
private:
    ...
    inline bool ignore_events() const { return is_suppressed_ || !is_active_ || isolate_->debug_execution_mode() == DebugInfo::kSideEffects; }

    void clear_suspended_generator() { thread_local_.suspended_generator_ = Smi::zero(); }

    bool has_suspended_generator() const { return thread_local_.suspended_generator_ != Smi::zero(); }

    bool IsExceptionBlackboxed(bool uncaught);

    void OnException(Handle<Object> exception, Handle<Object> promise, v8::debug::ExceptionType exception_type);
    ...
    MaybeHandle<FixedArray> GetHitBreakpointsAtCurrentStatement(JavaScriptFrame* frame, bool* hasBreakpoints);
    ...
    void FindDebugInfo(Handle<DebugInfo> debug_info, DebugInfoListNode** prev, DebugInfoListNode** curr);
...
public:
    ...
    ~DisableTemporaryObjectTracking() { debug_->SetTemporaryObjectTrackingDisabled(previous_tracking_disabled_); }
