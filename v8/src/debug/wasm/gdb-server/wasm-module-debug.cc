...
Handle<WasmInstanceObject> WasmModuleDebug::GetFirstWasmInstance()
{
    ...
    if (weak_instance_list->length() > 0) {
        ...
        if (maybe_instance->IsWeak()) {
            Handle<WasmInstanceObject> instance(WasmInstanceObject::cast(maybe_instance->GetHeapObjectAssumeWeak()), GetIsolate());
            return instance;
        }
    }
        ...
        case StackFrame::WASM: {
            ...
            for (size_t i = frames.size(); i-- != 0;) {
                ...
                if (summary.IsJavaScript()) {
                    FrameSummary::JavaScriptFrameSummary const& java_script = summary.AsJavaScript();
                    ...
                    offset = GetWasmFunctionOffset(wasm.wasm_instance()->module(), wasm.function_index()) + wasm.byte_offset();
                    script = wasm.script();
...
// static
std::vector<FrameSummary> WasmModuleDebug::FindWasmFrame(StackTraceFrameIterator* frame_it, uint32_t* frame_index)
{
    while (!frame_it->done()) {
        StackFrame* const frame = frame_it->frame();
        switch (frame->type()) {
