...
public:
    ScopeChainRetriever(DeclarationScope* scope, Handle<JSFunction> function, int position)
        : scope_(scope)
...
private:
    ...
    bool RetrieveClosureScope(Scope* scope)
    {
        ...
        if (!scope->is_class_scope() && break_scope_start_ == scope->start_position() && break_scope_end_ == scope->end_position()) {
    ...
    bool ContainsPosition(Scope* scope)
    {
        ...
        const bool position_fits_start = scope->is_class_scope() ? start <= position_ : start < position_;
...
void ScopeIterator::TryParseAndRetrieveScopes(ReparseStrategy strategy)
{
    ...
    if (strategy == ReparseStrategy::kScriptIfNeeded) {
        ...
        strategy = calculate_blocklists_ ? ReparseStrategy::kScript : ReparseStrategy::kFunctionLiteral;
    }
    ...
    if (scope_info->scope_type() == EVAL_SCOPE || script->is_wrapped()) {
        ...
        DCHECK(scope_info->scope_type() == SCRIPT_SCOPE || scope_info->scope_type() == FUNCTION_SCOPE);
    }
...
bool ScopeIterator::HasPositionInfo() { return InInnerScope() || !context_->IsNativeContext(); }
...
bool ScopeIterator::DeclaresLocals(Mode mode) const
{
    ...
    auto visitor = [&](Handle<String> name, Handle<Object> value, ScopeType scope_type) {
        declares_local = true;
        return true;
    };
...
bool ScopeIterator::HasContext() const { return !InInnerScope() || NeedsContext(); }
...
bool ScopeIterator::NeedsContext() const
{
    ...
    CHECK_IMPLIES(needs_context && current_scope_ == closure_scope_ && current_scope_->is_function_scope() && !function_->is_null(), function_->context() != *context_);
...
ScopeIterator::ScopeType ScopeIterator::Type() const
{
    ...
    if (InInnerScope()) {
        ...
        case SCRIPT_SCOPE:
            DCHECK_IMPLIES(NeedsContext(), context_->IsScriptContext() || context_->IsNativeContext());
...
Handle<JSObject> ScopeIterator::ScopeObject(Mode mode)
{
    ...
    auto visitor = [=](Handle<String> name, Handle<Object> value, ScopeType scope_type) {
        if (value->IsOptimizedOut(isolate_)) {
            ...
            if (scope_type == ScopeTypeScript && JSReceiver::HasOwnProperty(isolate_, scope, name).FromMaybe(true)) {
        ...
        JSObject::SetPropertyOrElement(isolate_, scope, name, value, Just(ShouldThrow::kDontThrow)).Check();
        return false;
    };
...
void ScopeIterator::DebugPrint()
{
    ...
    case ScopeIterator::ScopeTypeScript:
        os << "Script:\n";
        context_->global_object().native_context().script_context_table().Print(os);
        break;
...
void ScopeIterator::VisitScriptScope(const Visitor& visitor) const
{
    ...
    Handle<ScriptContextTable> script_contexts(global->native_context().script_context_table(), isolate_);

    // Skip the first script since that just declares 'this'.
    for (int context_index = 1; context_index < script_contexts->used(kAcquireLoad); context_index++) {
        Handle<Context> context = ScriptContextTable::GetContext(isolate_, script_contexts, context_index);
...
bool ScopeIterator::VisitContextLocals(const Visitor& visitor, Handle<ScopeInfo> scope_info, Handle<Context> context, ScopeType scope_type) const
{
    ...
    for (Variable* var : *current_scope_->locals()) {
        ...
        case VariableLocation::LOCAL:
            if (frame_inspector_ == nullptr) {
                ...
                } else if (IsLexicalVariableMode(var->mode()) && value->IsUndefined(isolate_) && GetSourcePosition() != kNoSourcePosition
...
void ScopeIterator::VisitLocalScope(const Visitor& visitor, Mode mode, ScopeType scope_type) const
{
    if (InInnerScope()) {
        ...
        if (mode == Mode::STACK && Type() == ScopeTypeLocal) {
            ...
            if (frame_inspector_ != nullptr && !closure_scope_->is_arrow_scope()
                && (closure_scope_->arguments() == nullptr || frame_inspector_->GetExpression(closure_scope_->arguments()->index())->IsOptimizedOut(isolate_))) {
    ...
    if (mode == Mode::ALL && HasContext()) {
        ...
        Handle<FixedArray> keys = KeyAccumulator::GetKeys(isolate_, extension, KeyCollectionMode::kOwnOnly, ENUMERABLE_STRINGS).ToHandleChecked();
...
bool ScopeIterator::SetLocalVariableValue(Handle<String> variable_name, Handle<Object> new_value)
{
        ...
        if (String::Equals(isolate_, var->name(), variable_name)) {
            ...
            case VariableLocation::PARAMETER: {
                ...
                if (frame_inspector_ == nullptr) {
                    ...
                    DCHECK(!generator_.is_null());
                    Handle<FixedArray> parameters_and_registers(generator_->parameters_and_registers(), isolate_);
                    DCHECK_LT(index, parameters_and_registers->length());
...
bool ScopeIterator::SetContextVariableValue(Handle<String> variable_name, Handle<Object> new_value)
{
...
bool ScopeIterator::SetModuleVariableValue(Handle<String> variable_name, Handle<Object> new_value)
{
    DisallowGarbageCollection no_gc;
    int cell_index;
    VariableMode mode;
...
bool ScopeIterator::SetScriptVariableValue(Handle<String> variable_name, Handle<Object> new_value)
{
    Handle<ScriptContextTable> script_contexts(context_->global_object().native_context().script_context_table(), isolate_);
    VariableLookupResult lookup_result;
    if (script_contexts->Lookup(variable_name, &lookup_result)) {
        Handle<Context> script_context = ScriptContextTable::GetContext(isolate_, script_contexts, lookup_result.context_index);
...
Handle<ScopeInfo> LocalBlocklistsCollector::FindScopeInfoForScope(Scope* scope) const
{
    ...
    for (SharedFunctionInfo info = iterator.Next(); !info.is_null(); info = iterator.Next()) {
        if (scope->start_position() == info.StartPosition() && scope->end_position() == info.EndPosition()) {
    ...
    while (scope_->outer_scope() && !context_->IsNativeContext()) {
        ...
        if (scope_->NeedsContext()) {
            if (!context_blocklist_.is_null()) {
                ...
                isolate_->LocalsBlockListCacheSet(handle(context_->scope_info(), isolate_), handle(context_->previous().scope_info(), isolate_), context_blocklist_);
                context_ = handle(context_->previous(), isolate_);
            }
...
void ScopeIterator::MaybeCollectAndStoreLocalBlocklists() const
{
    ...
    CHECK(v8_flags.experimental_reuse_locals_blocklists);
    DCHECK(isolate_->LocalsBlockListCacheGet(handle(function_->shared().scope_info(), isolate_)).IsTheHole());
