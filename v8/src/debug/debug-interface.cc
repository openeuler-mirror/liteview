...
i::Handle<i::String> GetBigIntStringPresentationHandle(i::Isolate* i_isolate, i::Handle<i::BigInt> i_bigint)
{
    ...
    if (radix == 16) {
        if (i_bigint->IsNegative()) {
            string_value
                = i_isolate->factory()
                      ->NewConsString(i_isolate->factory()->NewStringFromAsciiChecked("-0x"), i_isolate->factory()->NewProperSubString(string_value, 1, string_value->length() - 1))
                      .ToHandleChecked();
        } else {
...
Local<String> GetBigIntDescription(Isolate* isolate, Local<BigInt> bigint)
{
    ...
    i::Handle<i::String> description = i_isolate->factory()->NewConsString(string_value, i_isolate->factory()->LookupSingleCharacterStringFromCode('n')).ToHandleChecked();
...
Local<String> GetDateDescription(Local<Date> date)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    auto buffer = i::ToDateString(jsdate->value().Number(), i_isolate->date_cache(), i::ToDateStringMode::kLocalDateAndTime);
...
Local<String> GetFunctionDescription(Local<Function> function)
{
    ...
    if (receiver->IsJSBoundFunction()) {
        return Utils::ToLocal(i::JSBoundFunction::ToString(i::Handle<i::JSBoundFunction>::cast(receiver)));
    }
    if (receiver->IsJSFunction()) {
        ...
        if (js_function->shared().HasWasmExportedFunctionData()) {
            ...
            auto func_index = js_function->shared().wasm_exported_function_data().function_index();
            auto instance = i::handle(js_function->shared().wasm_exported_function_data().instance(), i_isolate);
            if (instance->module()->origin == i::wasm::kWasmOrigin) {
                ...
                auto debug_name = i::GetWasmFunctionDebugName(i_isolate, instance, func_index);
...
void CollectPrivateMethodsAndAccessorsFromContext(
    i::Isolate* isolate, i::Handle<i::Context> context, i::IsStaticFlag is_static_flag, std::vector<Local<Value>>* names_out, std::vector<Local<Value>>* values_out)
{
    ...
    for (auto it : i::ScopeInfo::IterateLocalNames(scope_info)) {
        ...
        if (!i::IsPrivateMethodOrAccessorVariableMode(mode) || flag != is_static_flag) {
            continue;
        }
        int context_index = scope_info->ContextHeaderLength() + it->index();
        i::Handle<i::Object> slot_value(context->get(context_index), isolate);
        DCHECK_IMPLIES(mode == i::VariableMode::kPrivateMethod, slot_value->IsJSFunction());
        DCHECK_IMPLIES(mode != i::VariableMode::kPrivateMethod, slot_value->IsAccessorPair());
...
bool GetPrivateMembers(Local<Context> context, Local<Object> object, std::vector<Local<Value>>* names_out, std::vector<Local<Value>>* values_out)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(
        isolate, keys, i::KeyAccumulator::GetKeys(isolate, receiver, i::KeyCollectionMode::kOwnOnly, key_filter, i::GetKeysConversion::kConvertToString), false);
    ...
    if (receiver->IsJSFunction()) {
        ...
        if (shared->is_class_constructor() && shared->has_static_private_methods_or_accessors()) {
            ...
            for (int j = 0; j < local_count; ++j) {
                ...
                i::IsStaticFlag is_static_flag = scope_info->ContextLocalIsStaticFlag(j);
                if (i::IsPrivateMethodOrAccessorVariableMode(mode) && is_static_flag == i::IsStaticFlag::kStatic) {
    ...
    if (has_static_private_methods_or_accessors) {
        i::Handle<i::Context> recevier_context(i::JSFunction::cast(*receiver).context(), isolate);
        CollectPrivateMethodsAndAccessorsFromContext(isolate, recevier_context, i::IsStaticFlag::kStatic, names_out, values_out);
    }
    ...
    for (int i = 0; i < keys->length(); ++i) {
        ...
        if (key->is_private_brand()) {
            ...
            i::Handle<i::Context> value_context(i::Context::cast(*value), isolate);
            CollectPrivateMethodsAndAccessorsFromContext(isolate, value_context, i::IsStaticFlag::kNotStatic, names_out, values_out);
        } else { // Private fields
            i::Handle<i::String> name(i::String::cast(i::Symbol::cast(*key).description()), isolate);
            names_out->push_back(Utils::ToLocal(name));
...
void ChangeBreakOnException(Isolate* isolate, ExceptionBreakState type)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    DCHECK_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i_isolate->debug()->ChangeBreakOnException(i::BreakException, type == BreakOnAnyException);
    i_isolate->debug()->ChangeBreakOnException(i::BreakUncaughtException, type != NoBreakOnException);
}
...
size_t ScriptSource::Length() const
{
    ...
    if (source->IsString())
        return i::Handle<i::String>::cast(source)->length();
    return Size();
}
...
Maybe<MemorySpan<const uint8_t>> ScriptSource::WasmBytecode() const
{
    ...
    base::Vector<const uint8_t> wire_bytes = i::Managed<i::wasm::NativeModule>::cast(*source).raw()->wire_bytes();
    return Just(MemorySpan<const uint8_t> { wire_bytes.begin(), wire_bytes.size() });
}
#endif // V8_ENABLE_WEBASSEMBLY

Isolate* Script::GetIsolate() const { return reinterpret_cast<Isolate*>(Utils::OpenHandle(this)->GetIsolate()); }

ScriptOriginOptions Script::OriginOptions() const { return Utils::OpenHandle(this)->origin_options(); }

bool Script::WasCompiled() const { return Utils::OpenHandle(this)->compilation_state() == i::Script::COMPILATION_STATE_COMPILED; }
...
int Script::StartColumn() const { return Utils::OpenHandle(this)->column_offset(); }
...
int Script::EndLine() const
{
    ...
    i::Script::GetPositionInfo(script, i::String::cast(script->source()).length(), &info, i::Script::WITH_OFFSET);
    return info.line;
}
...
#if V8_ENABLE_WEBASSEMBLY
    if (script->type() == i::Script::TYPE_WASM) {
        i::Handle<i::Object> wasm_native_module(script->wasm_managed_native_module(), isolate);
...
#if V8_ENABLE_WEBASSEMBLY
bool Script::IsWasm() const { return Utils::OpenHandle(this)->type() == i::Script::TYPE_WASM; }
#endif // V8_ENABLE_WEBASSEMBLY

bool Script::IsModule() const { return Utils::OpenHandle(this)->origin_options().IsModule(); }

namespace {

int GetSmiValue(i::Handle<i::FixedArray> array, int index) { return i::Smi::ToInt(array->get(index)); }

bool CompareBreakLocation(const i::BreakLocation& loc1, const i::BreakLocation& loc2) { return loc1.position() < loc2.position(); }
...
Maybe<int> Script::GetSourceOffset(const Location& location, GetSourceOffsetMode mode) const
{
    ...
    i::Handle<i::FixedArray> line_ends = i::Handle<i::FixedArray>::cast(i::handle(script->line_ends(), script->GetIsolate()));
    if (line < 0) {
...
{
    i::EmbedderStackStateScope stack_scope(reinterpret_cast<i::Isolate*>(isolate)->heap(), i::EmbedderStackStateScope::kImplicitThroughTask, embedder_stack_state);
...
MemorySpan<const char> WasmScript::ExternalSymbolsURL() const
{
    ...
    const i::wasm::WasmDebugSymbols& symbols = script->wasm_native_module()->module()->debug_symbols;
    if (symbols.external_url.is_empty())
        return {};

    internal::wasm::ModuleWireBytes wire_bytes(script->wasm_native_module()->wire_bytes());
    i::wasm::WasmName external_url = wire_bytes.GetNameOrNull(symbols.external_url);
...
uint32_t WasmScript::GetFunctionHash(int function_index)
{
    ...
    const i::wasm::WasmFunction& func = module->functions[function_index];
    i::wasm::ModuleWireBytes wire_bytes(native_module->wire_bytes());
    base::Vector<const i::byte> function_bytes = wire_bytes.GetFunctionBytes(&func);
...
void GetLoadedScripts(Isolate* v8_isolate, std::vector<v8::Global<Script>>& scripts)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(isolate);
    {
        ...
        for (i::Script script = iterator.Next(); !script.is_null(); script = iterator.Next()) {
#if V8_ENABLE_WEBASSEMBLY
            if (script.type() != i::Script::TYPE_NORMAL && script.type() != i::Script::TYPE_WASM) {
                continue;
            }
#else
...
MaybeLocal<UnboundScript> CompileInspectorScript(Isolate* v8_isolate, Local<String> source)
{
    ...
    i::Handle<i::SharedFunctionInfo> result;
    {
        ...
        i::MaybeHandle<i::SharedFunctionInfo> maybe_function_info = i::Compiler::GetSharedFunctionInfoForScriptWithCachedData(isolate, str, i::ScriptDetails(), cached_data,
            ScriptCompiler::kNoCompileOptions, ScriptCompiler::kNoCacheBecauseInspector, i::v8_flags.expose_inspector_scripts ? i::NOT_NATIVES_CODE : i::INSPECTOR_CODE);
...
void SetAsyncEventDelegate(Isolate* v8_isolate, AsyncEventDelegate* delegate) { reinterpret_cast<i::Isolate*>(v8_isolate)->set_async_event_delegate(delegate); }
...
void ResetBlackboxedStateCache(Isolate* v8_isolate, Local<Script> script)
{
    ...
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(isolate);
    i::DisallowGarbageCollection no_gc;
    i::SharedFunctionInfo::ScriptIterator iter(isolate, *Utils::OpenHandle(*script));
    for (i::SharedFunctionInfo info = iter.Next(); !info.is_null(); info = iter.Next()) {
...
void AccessorPair::CheckCast(Value* that)
{
    ...
    Utils::ApiCheck(obj->IsAccessorPair(), "v8::debug::AccessorPair::Cast", "Value is not a v8::debug::AccessorPair");
}
...
{
    i::Handle<i::Object> obj = Utils::OpenHandle(that);
    Utils::ApiCheck(obj->IsWasmValueObject(), "v8::debug::WasmValueObject::Cast", "Value is not a v8::debug::WasmValueObject");
}
...
{
    i::Handle<i::WasmValueObject> object = i::Handle<i::WasmValueObject>::cast(Utils::OpenHandle(this));
...
Local<Function> GetBuiltin(Isolate* v8_isolate, Builtin requested_builtin)
{
    ...
    i::Handle<i::SharedFunctionInfo> info = factory->NewSharedFunctionInfoForBuiltin(name, builtin);
...
ConsoleCallArguments::ConsoleCallArguments(const internal::BuiltinArguments& args)
    : v8::FunctionCallbackInfo<v8::Value>(nullptr,
        // Drop the first argument (receiver, i.e. the "console" object).
        args.length() > 1 ? args.address_of_first_argument() : nullptr, args.length() - 1)
{
}
...
Location GeneratorObject::SuspendedLocation()
{
    ...
    i::Script::GetPositionInfo(script, obj->source_position(), &info, i::Script::WITH_OFFSET);
    return Location(info.line, info.column);
}

bool GeneratorObject::IsSuspended() { return Utils::OpenHandle(this)->is_suspended(); }
...
MaybeLocal<Value> CallFunctionOn(Local<Context> context, Local<Function> function, Local<Value> recv, int argc, Local<Value> argv[], bool throw_on_side_effect)
{
    ...
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::Execution::Call(isolate, self, recv_obj, argc, args), &result);
    if (throw_on_side_effect) {
...
MaybeLocal<v8::Value> EvaluateGlobal(v8::Isolate* isolate, v8::Local<v8::String> source, EvaluateGlobalMode mode, bool repl)
{
    ...
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::DebugEvaluate::Global(i_isolate, Utils::OpenHandle(*source), mode, repl_mode), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}

v8::MaybeLocal<v8::Value> EvaluateGlobalForTesting(v8::Isolate* isolate, v8::Local<v8::Script> function, v8::debug::EvaluateGlobalMode mode, bool repl)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    PREPARE_FOR_DEBUG_INTERFACE_EXECUTION_WITH_ISOLATE(i_isolate, Value);
    i::REPLMode repl_mode = repl ? i::REPLMode::kYes : i::REPLMode::kNo;
    Local<Value> result;
    has_pending_exception = !ToLocal<Value>(i::DebugEvaluate::Global(i_isolate, Utils::OpenHandle(*function), mode, repl_mode), &result);
    RETURN_ON_FAILED_EXECUTION(Value);
    RETURN_ESCAPED(result);
}

void QueryObjects(v8::Local<v8::Context> v8_context, QueryObjectPredicate* predicate, std::vector<v8::Global<v8::Object>>* objects)
{
    i::Isolate* isolate = reinterpret_cast<i::Isolate*>(v8_context->GetIsolate());
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(isolate);
    isolate->heap_profiler()->QueryObjects(Utils::OpenHandle(*v8_context), predicate, objects);
}
...
void GlobalLexicalScopeNames(v8::Local<v8::Context> v8_context, std::vector<v8::Global<v8::String>>* names)
{
    ...
    i::Handle<i::ScriptContextTable> table(context->global_object().native_context().script_context_table(), isolate);
    for (int i = 0; i < table->used(kAcquireLoad); i++) {
        i::Handle<i::Context> script_context = i::ScriptContextTable::GetContext(isolate, table, i);
...
int64_t GetNextRandomInt64(v8::Isolate* v8_isolate) { return reinterpret_cast<i::Isolate*>(v8_isolate)->random_number_generator()->NextInt64(); }
...
bool SetFunctionBreakpoint(v8::Local<v8::Function> function, v8::Local<v8::String> condition, BreakpointId* id)
{
    ...
    i::Handle<i::String> condition_string = condition.IsEmpty() ? isolate->factory()->empty_string() : Utils::OpenHandle(*condition);
...
MaybeLocal<String> Coverage::FunctionData::Name() const { return ToApiHandle<String>(function_->name); }

size_t Coverage::FunctionData::BlockCount() const { return function_->blocks.size(); }

bool Coverage::FunctionData::HasBlockCoverage() const { return function_->has_block_coverage; }

Coverage::BlockData Coverage::FunctionData::GetBlockData(size_t i) const { return BlockData(&function_->blocks.at(i), coverage_); }

Local<Script> Coverage::ScriptData::GetScript() const { return ToApiHandle<Script>(script_->script); }

size_t Coverage::ScriptData::FunctionCount() const { return script_->functions.size(); }

Coverage::FunctionData Coverage::ScriptData::GetFunctionData(size_t i) const { return FunctionData(&script_->functions.at(i), coverage_); }
...
Coverage::ScriptData Coverage::GetScriptData(size_t i) const { return ScriptData(i, coverage_); }

Coverage Coverage::CollectPrecise(Isolate* isolate) { return Coverage(i::Coverage::CollectPrecise(reinterpret_cast<i::Isolate*>(isolate))); }

Coverage Coverage::CollectBestEffort(Isolate* isolate) { return Coverage(i::Coverage::CollectBestEffort(reinterpret_cast<i::Isolate*>(isolate))); }

void Coverage::SelectMode(Isolate* isolate, CoverageMode mode) { i::Coverage::SelectMode(reinterpret_cast<i::Isolate*>(isolate), mode); }
...
Local<EphemeronTable> EphemeronTable::Set(v8::Isolate* isolate, v8::Local<v8::Value> key, v8::Local<v8::Value> value)
{
    ...
    i::Handle<i::EphemeronHashTable> result(i::EphemeronHashTable::Put(self, internal_key, internal_value));
...
Local<EphemeronTable> EphemeronTable::New(v8::Isolate* isolate)
{
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    ENTER_V8_NO_SCRIPT_NO_EXCEPTION(i_isolate);
    i::Handle<i::EphemeronHashTable> table = i::EphemeronHashTable::New(i_isolate, 0);
    return ToApiHandle<EphemeronTable>(table);
}

EphemeronTable* EphemeronTable::Cast(v8::Value* value) { return static_cast<EphemeronTable*>(value); }
...
MaybeLocal<Message> GetMessageFromPromise(Local<Promise> p)
{
    ...
    if (!maybeMessage->IsJSMessageObject(isolate))
        return MaybeLocal<Message>();
    return ToApiHandle<Message>(i::Handle<i::JSMessageObject>::cast(maybeMessage));
}

bool isExperimentalAsyncStackTaggingApiEnabled() { return i::v8_flags.experimental_async_stack_tagging_api; }

bool isExperimentalRemoveInternalScopesPropertyEnabled() { return i::v8_flags.experimental_remove_internal_scopes_property; }
...
{
    internal::Isolate* isolate = reinterpret_cast<i::Isolate*>(object->GetIsolate());
    if (isolate->is_execution_terminating()) {
        return nullptr;
    }
    CallDepthScope<false> call_depth_scope(isolate, context);

    auto result = i::DebugPropertyIterator::Create(isolate, Utils::OpenHandle(*object), skip_indices);
    if (!result) {
...
Maybe<bool> DebugPropertyIterator::Advance()
{
    ...
    Local<v8::Context> context = Utils::ToLocal(handle(isolate_->context(), isolate_));
