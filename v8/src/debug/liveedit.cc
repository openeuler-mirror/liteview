...
bool CompareSubstrings(Handle<String> s1, int pos1, Handle<String> s2, int pos2, int len)
{
    for (int i = 0; i < len; i++) {
...
public:
    ...
    bool Equals(int index1, int index2) override { return s1_->Get(offset1_ + index1) == s2_->Get(offset2_ + index2); }
...
public:
    ...
    int length() { return ends_array_->length() + 1; }
...
private:
    ...
    int GetPosAfterNewLine(int index) { return Smi::ToInt(ends_array_->get(index)) + 1; }
};
...
public:
    ...
    void SetSubrange1(int offset, int len) override { subrange_offset1_ = offset; }
    void SetSubrange2(int offset, int len) override { subrange_offset2_ = offset; }
    ...
    for (const SourcePositionEvent& event : events) {
        ...
        case SourcePositionEvent::LITERAL_STARTS:
            literal_stack.push(std::make_pair(event.literal,
                FunctionLiteralChange(inside_diff ? kNoSourcePosition : event.literal->start_position() + delta, literal_stack.empty() ? nullptr : literal_stack.top().first)));
            break;
...
void MapLiterals(const FunctionLiteralChanges& changes, const std::vector<FunctionLiteral*>& new_literals, LiteralMap* unchanged, LiteralMap* changed)
{
    ...
    for (FunctionLiteral* literal : new_literals) {
        ...
        std::pair<int, int> key
            = literal->function_literal_id() == kFunctionLiteralIdTopLevel ? kTopLevelMarker : std::make_pair(literal->start_position(), literal->end_position());
    ...
    for (const auto& change_pair : changes) {
        ...
        std::pair<int, int> key
            = literal->function_literal_id() == kFunctionLiteralIdTopLevel ? kTopLevelMarker : std::make_pair(change.new_start_position, change.new_end_position);
...
public:
    void AddInterestingLiteral(int script_id, FunctionLiteral* literal) { map_.emplace(GetFuncId(script_id, literal), FunctionData { literal }); }
    ...
    bool Lookup(Handle<Script> script, FunctionLiteral* literal, FunctionData** data) { return Lookup(GetFuncId(script->id(), literal), data); }
...
bool CanPatchScript(const LiteralMap& changed, Handle<Script> script, Handle<Script> new_script, FunctionDataMap& function_data_map, bool allow_top_frame_live_editing,
...
void TranslateSourcePositionTable(Isolate* isolate, Handle<BytecodeArray> code, const std::vector<SourceChangeRange>& diffs)
{
    ...
    Handle<ByteArray> new_source_position_table(builder.ToSourcePositionTable(isolate));
...
void UpdatePositions(Isolate* isolate, Handle<SharedFunctionInfo> sfi, FunctionLiteral* new_function, const std::vector<SourceChangeRange>& diffs)
{
    sfi->UpdateFromFunctionLiteralForLiveEdit(new_function);
    if (sfi->HasBytecodeArray()) {
        TranslateSourcePositionTable(isolate, handle(sfi->GetBytecodeArray(isolate), isolate), diffs);
    }
}

#ifdef DEBUG
ScopeInfo FindOuterScopeInfoFromScriptSfi(Isolate* isolate, Handle<Script> script)
{
...
MaybeHandle<ScopeInfo> DetermineOuterScopeInfo(Isolate* isolate, Handle<Script> script)
{
...
void LiveEdit::PatchScript(Isolate* isolate, Handle<Script> script, Handle<String> new_source, bool preview, bool allow_top_frame_live_editing_param, debug::LiveEditResult* result)
{
    std::vector<SourceChangeRange> diffs;
    LiveEdit::CompareStrings(isolate, handle(String::cast(script->source()), isolate), new_source, &diffs);
    if (diffs.empty()) {
    ...
    const bool allow_top_frame_live_editing = allow_top_frame_live_editing_param && v8_flags.live_edit_top_frame;
    ...
    for (const auto& mapping : unchanged) {
        ...
        new_script->shared_function_infos().Set(mapping.second->function_literal_id(), HeapObjectReference::Weak(*sfi));
        ...
        for (int i = 0; i < constants.length(); ++i) {
            if (!constants.get(i).IsSharedFunctionInfo())
                continue;
            data = nullptr;
            if (!function_data_map.Lookup(SharedFunctionInfo::cast(constants.get(i)), &data)) {
                continue;
            }
    ...
    for (SharedFunctionInfo sfi = it.Next(); !sfi.is_null(); sfi = it.Next()) {
        ...
        for (int i = 0; i < constants.length(); ++i) {
            ...
            SharedFunctionInfo old_unchanged_inner_sfi = SharedFunctionInfo::cast(new_script->shared_function_infos().Get(unchanged_it->second)->GetHeapObject());
...
#ifdef DEBUG
    {
        ...
        for (SharedFunctionInfo sfi = script_it.Next(); !sfi.is_null(); sfi = script_it.Next()) {
            ...
            for (int i = 0; i < constants.length(); ++i) {
                ...
                DCHECK_EQ(inner_sfi, new_script->shared_function_infos().Get(inner_sfi.function_literal_id())->GetHeapObject());
            }
        }
    }
#endif
...
{
    auto it = std::lower_bound(diffs.begin(), diffs.end(), position, [](const SourceChangeRange& change, int position) { return change.end_position < position; });
