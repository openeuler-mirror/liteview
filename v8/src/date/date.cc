...
int DateCache::DaylightSavingsOffsetInMs(int64_t time_ms)
{
    ...
    int new_after_start_sec = before_->end_sec < kMaxEpochTimeInSec - kDefaultDSTDeltaInSec ? before_->end_sec + kDefaultDSTDeltaInSec : kMaxEpochTimeInSec;
...
void DateCache::ProbeDST(int time_sec)
{
    ...
    if (after == nullptr) {
        after = InvalidSegment(after_) && before != after_ ? after_ : LeastRecentlyUsedDST(before);
    }
...
DateBuffer ToDateString(double time_val, DateCache* date_cache, ToDateStringMode mode)
{
    ...
    case ToDateStringMode::kLocalDateAndTime:
        return FormatDate((year < 0) ? "%s %s %02d %05d %02d:%02d:%02d GMT%c%02d%02d (%s)" : "%s %s %02d %04d %02d:%02d:%02d GMT%c%02d%02d (%s)", kShortWeekDays[weekday],
            kShortMonths[month], day, year, hour, min, sec, (timezone_offset < 0) ? '-' : '+', timezone_hour, timezone_min, local_timezone);
    case ToDateStringMode::kUTCDateAndTime:
        return FormatDate(
            (year < 0) ? "%s, %02d %s %05d %02d:%02d:%02d GMT" : "%s, %02d %s %04d %02d:%02d:%02d GMT", kShortWeekDays[weekday], day, kShortMonths[month], year, hour, min, sec);
