...
public:
    ...
    static int TimeInDay(int64_t time_ms, int days) { return static_cast<int>(time_ms - days * kMsPerDay); }
    ...
    int LocalOffsetInMs(int64_t time, bool is_utc) { return GetLocalOffsetFromOS(time, is_utc); }
    ...
    int64_t ToLocal(int64_t time_ms) { return time_ms + LocalOffsetInMs(time_ms, true); }

    // ECMA 262 - ES#sec-utc-t
    // UTC(t) = t - LocalTZA(t, false)
    int64_t ToUTC(int64_t time_ms) { return time_ms - LocalOffsetInMs(time_ms, false); }
...
private:
    ...
    bool InvalidSegment(DST* segment) { return segment->start_sec > segment->end_sec; }
