...
template <typename C, typename P> inline size_t count_if(const C& container, const P& predicate) { return std::count_if(begin(container), end(container), predicate); }
...
template <typename C> inline bool all_equal(const C& container)
{
    ...
    const auto& value = *b;
    return std::all_of(++b, end(container), [&](const auto& v) { return v == value; });
}
