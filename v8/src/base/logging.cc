...
void SetPrintStackTrace(void (*print_stack_trace)()) { g_print_stack_trace = print_stack_trace; }

void SetDcheckFunction(void (*dcheck_function)(const char*, int, const char*)) { g_dcheck_function = dcheck_function ? dcheck_function : &DefaultDcheckHandler; }
...
#define DEFINE_PRINT_CHECK_OPERAND_CHAR(type)                                                                                                                                      \
    template <> std::string PrintCheckOperand<type>(type ch) { return PrettyPrintChar(ch); }                                                                                       \
    template <> std::string PrintCheckOperand<type*>(type * cstr) { return PrintCheckOperand<void*>(cstr); }                                                                       \
    template <> std::string PrintCheckOperand<const type*>(const type* cstr) { return PrintCheckOperand<const void*>(cstr); }
...
void V8_Dcheck(const char* file, int line, const char* message) { v8::base::g_dcheck_function(file, line, message); }
