...
public:
    ...
    void SetRandomMmapSeed(int64_t seed) override { return page_allocator_->SetRandomMmapSeed(seed); }

    void* GetRandomMmapAddr() override { return page_allocator_->GetRandomMmapAddr(); }
    ...
    bool SetPermissions(void* address, size_t size, PageAllocator::Permission access) override { return page_allocator_->SetPermissions(address, size, access); }

    bool RecommitPages(void* address, size_t size, PageAllocator::Permission access) override { return page_allocator_->RecommitPages(address, size, access); }

    bool DiscardSystemPages(void* address, size_t size) override { return page_allocator_->DiscardSystemPages(address, size); }

    bool DecommitPages(void* address, size_t size) override { return page_allocator_->DecommitPages(address, size); }
