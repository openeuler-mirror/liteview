...
#define CHECK_OP(name, op, lhs, rhs)                                                                                                                                               \
    do {                                                                                                                                                                           \
        if (std::string* _msg                                                                                                                                                      \
            = ::v8::base::Check##name##Impl<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type, typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>(             \
                (lhs), (rhs), #lhs " " #op " " #rhs)) {                                                                                                                            \
...
#define DCHECK_OP(name, op, lhs, rhs) \
    do { \
        if (::v8::base::Check##name##Impl<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type, typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>( \
                (lhs), (rhs), #lhs " " #op " " #rhs)) {    \
            V8_Dcheck(__FILE__, __LINE__, ""); \
        } \
    } while (false)

#define CHECK_OP(name, op, lhs, rhs) DCHECK_OP(name, op, lhs, rhs)
...
#define CHECK_OP(name, op, lhs, rhs)                                                                                                                                               \
    do {                                                                                                                                                                           \
        bool _cmp                                                                                                                                                                  \
            = ::v8::base::Cmp##name##Impl<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type, typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>((lhs), (rhs)); \
...
template <typename T> auto GetUnderlyingEnumTypeForPrinting(T val)
{
    ...
    // as a character.
    using int_t = typename std::conditional_t<sizeof(underlying_t) != 1, underlying_t, std::conditional_t<std::is_signed<underlying_t>::value, int16_t, uint16_t>>;
...
template <typename T>
typename std::enable_if<!std::is_function<typename std::remove_pointer<T>::type>::value && !std::is_enum<T>::value && has_output_operator<T, CheckMessageStream>::value,
    std::string>::type
...
template <typename T> typename std::enable_if<std::is_function<typename std::remove_pointer<T>::type>::value, std::string>::type PrintCheckOperand(T val)
{
    return PrintCheckOperand(reinterpret_cast<const void*>(val));
}

// Define PrintCheckOperand<T> for enums with an output operator.
template <typename T> typename std::enable_if<std::is_enum<T>::value && has_output_operator<T, CheckMessageStream>::value, std::string>::type PrintCheckOperand(T val)
{
...
template <typename T> typename std::enable_if<!has_output_operator<T, CheckMessageStream>::value && !std::is_enum<T>::value, std::string>::type PrintCheckOperand(T val)
{
    return "<unprintable>";
}
...
template <typename T> struct comparison_underlying_type {
    ...
    using type = typename std::conditional<std::is_same<type_or_bool, bool>::value, unsigned int, type_or_bool>::type;
};
...
template <typename Lhs, typename Rhs> struct is_signed_vs_unsigned {
    ...
    static constexpr bool value
        = std::is_integral<lhs_underlying>::value && std::is_integral<rhs_underlying>::value && std::is_signed<lhs_underlying>::value && std::is_unsigned<rhs_underlying>::value;
};
...
#define DEFINE_SIGNED_MISMATCH_COMP(CHECK, NAME, IMPL)                                                                                                                             \
    template <typename Lhs, typename Rhs> V8_INLINE constexpr typename std::enable_if<CHECK<Lhs, Rhs>::value, bool>::type Cmp##NAME##Impl(Lhs lhs, Rhs rhs) { return IMPL; }
...
#define DEFINE_CHECK_OP_IMPL(NAME, op)                                                                                                                                             \
    ...
    V8_INLINE constexpr typename std::enable_if<!is_signed_vs_unsigned<Lhs, Rhs>::value && !is_unsigned_vs_signed<Lhs, Rhs>::value, bool>::type Cmp##NAME##Impl(Lhs lhs, Rhs rhs)  \
    {                                                                                                                                                                              \
        return lhs op rhs;                                                                                                                                                         \
    }                                                                                                                                                                              \
...
template <typename Lhs, typename Rhs>                                                                                                                                          
V8_INLINE constexpr typename std::enable_if<!is_signed_vs_unsigned<Lhs, Rhs>::value && !is_unsigned_vs_signed<Lhs, Rhs>::value, bool>::type CmpLTImpl(Lhs lhs, Rhs rhs)
{                                                                                                                                                                              
    return lhs < rhs;
}

template <typename Lhs, typename Rhs> V8_INLINE constexpr std::string* CheckLTImpl22(char const* msg, Lhs lhs, Rhs rhs)
{                                                                                                                                                                              
    using LhsPassT = typename pass_value_or_ref<Lhs>::type;                                                                                                                    
    using RhsPassT = typename pass_value_or_ref<Rhs>::type;                                                                                                                    
    bool cmp = CmpLTImpl<LhsPassT, RhsPassT>(lhs, rhs);

    return V8_LIKELY(cmp) ? nullptr : MakeCheckOpString<LhsPassT, RhsPassT>(lhs, rhs, msg);                                                                                    
}

template <typename Lhs, typename Rhs> V8_INLINE constexpr std::string* CheckLTImpl33(
    char const* msg,
    Lhs lhs_nouse,
    char const* msg2,
    Lhs lhs, Rhs rhs)
...
// template <> V8_INLINE constexpr std::string* CheckLTImpl33<unsigned long, unsigned long>(
//     char const* msg,
//     unsigned long lhs_nouse,
//     char const* msg2,
//     unsigned long lhs, unsigned long rhs)
// {
//     bool cmp = (lhs < rhs);
//     return V8_LIKELY(cmp) ? nullptr : MakeCheckOpString<unsigned long, unsigned long>(lhs, rhs, msg);
// }
...
#define CHECK_LT(lhs, rhs) \
    do { \
        if (::v8::base::CheckLTImpl22<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type, typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>( \
                __FILE__, (lhs), (rhs))) {    \
            V8_Dcheck(__FILE__, __LINE__, ""); \
        } \
    } while (false)
...
#define DCHECK_LT(lhs, rhs) \
    do { \
        if (::v8::base::CheckLTImpl33<typename ::v8::base::pass_value_or_ref<decltype(lhs)>::type, typename ::v8::base::pass_value_or_ref<decltype(rhs)>::type>( \
                __FILE__, (lhs), \
                __FILE__, \
                (lhs), (rhs))) {    \
            V8_Dcheck(__FILE__, __LINE__, ""); \
