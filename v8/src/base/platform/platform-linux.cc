...
base::Optional<MemoryRegion> MemoryRegion::FromMapsLine(const char* line)
{
    ...
    if (sscanf(line, "%" V8PRIxPTR "-%" V8PRIxPTR " %4c %" V8PRIxPTR " %hhx:%hhx %" V8PRIdPTR " %n", &region.start, &region.end, region.permissions, &offset, &dev_major,
            &dev_minor, &inode, &path_index)
        < 7) {
        return base::nullopt;
    }
...
std::unique_ptr<std::vector<MemoryRegion>> ParseProcSelfMaps(FILE* fp, std::function<bool(const MemoryRegion&)> predicate, bool early_stopping)
{
    ...
    while (true) {
        ...
        base::Optional<MemoryRegion> region = MemoryRegion::FromMapsLine(line.get());
        if (!region) {
            break;
        }
...
// static
std::vector<OS::SharedLibraryAddress> OS::GetSharedLibraryAddresses() { return ::v8::base::GetSharedLibraryAddresses(nullptr); }
