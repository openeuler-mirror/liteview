...
SimpleTlsPointerXp<base::SharedMutex> single_held_shared_mutex;
SimpleTlsPointerXp<TSet> held_shared_mutexes;
...
bool SharedMutexNotHeld(SharedMutex* shared_mutex)
{
    ...
    return single_held_shared_mutex.Get() != shared_mutex && (!held_shared_mutexes.Get() || held_shared_mutexes->count(shared_mutex) == 0);
}
...
{
    DCHECK_NOT_NULL(shared_mutex);
    if (single_held_shared_mutex.Get()) {
        if (shared_mutex == single_held_shared_mutex.Get()) {
            return false;
        }
        DCHECK_NULL(held_shared_mutexes.Get());
        held_shared_mutexes = new TSet({ single_held_shared_mutex.Get(), shared_mutex });
        single_held_shared_mutex.Set(nullptr);
        return true;
    } else if (held_shared_mutexes.Get()) {
        return held_shared_mutexes->insert(shared_mutex).second;
    } else {
        DCHECK_NULL(single_held_shared_mutex.Get());
        single_held_shared_mutex.Set(shared_mutex);
        return true;
    }
}
...
{
    DCHECK_NOT_NULL(shared_mutex);
    if (single_held_shared_mutex.Get() == shared_mutex) {
        single_held_shared_mutex.Set(nullptr);
        return true;
    }
    if (held_shared_mutexes.Get() && held_shared_mutexes->erase(shared_mutex)) {
        if (held_shared_mutexes->empty()) {
            delete held_shared_mutexes.Get();
            held_shared_mutexes.Set(nullptr);
        }
        return true;
    }
    return false;
...
void Mutex::Lock()
{
    AcquireSRWLockExclusiveXp(V8ToWindowsType(&native_handle_));
    AssertUnheldAndMark();
}
...
{
    AssertHeldAndUnmark();
    ReleaseSRWLockExclusiveXp(V8ToWindowsType(&native_handle_));
}

bool Mutex::TryLock()
{
    if (!TryAcquireSRWLockExclusiveXp(V8ToWindowsType(&native_handle_))) {
        return false;
    }
    AssertUnheldAndMark();
    return true;
...
{
    DCHECK(TryHoldSharedMutex(this));
    AcquireSRWLockSharedXp(V8ToWindowsType(&native_handle_));
}
...
{
    DCHECK(TryReleaseSharedMutex(this));
    ReleaseSRWLockSharedXp(V8ToWindowsType(&native_handle_));
}
...
{
    DCHECK(SharedMutexNotHeld(this));
    bool result = TryAcquireSRWLockSharedXp(V8ToWindowsType(&native_handle_));
    if (result)
        DCHECK(TryHoldSharedMutex(this));
    return result;
}
...
{
    DCHECK(SharedMutexNotHeld(this));
    bool result = TryAcquireSRWLockExclusiveXp(V8ToWindowsType(&native_handle_));
    if (result)
        DCHECK(TryHoldSharedMutex(this));
    return result;
}
