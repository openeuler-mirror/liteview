...
// static
void OS::Free(void* address, const size_t size) { CHECK(SbMemoryUnmap(address, size)); }

// static
void OS::Release(void* address, size_t size) { CHECK(SbMemoryUnmap(address, size)); }
...
bool OS::SetPermissions(void* address, size_t size, MemoryPermission access)
{
    ...
    case OS::MemoryPermission::kReadExecute:
        ...
        new_protection = SbMemoryMapFlags(kSbMemoryMapProtectRead | kSbMemoryMapProtectExec);
#else
        UNREACHABLE();
#endif
        break;
...
bool OS::isDirectorySeparator(const char ch) { return ch == DirectorySeparator(); }
...
void OS::VPrint(const char* format, va_list args) { SbLogRawFormat(format, args); }
...
void OS::VFPrint(FILE* out, const char* format, va_list args) { SbLogRawFormat(format, args); }
...
void OS::StrNCpy(char* dest, int length, const char* src, size_t n) { SbStringCopy(dest, src, n); }
...
Thread::LocalStorageKey Thread::CreateThreadLocalKey() { return SbThreadCreateLocalKey(nullptr); }

void Thread::DeleteThreadLocalKey(LocalStorageKey key) { SbThreadDestroyLocalKey(key); }

void* Thread::GetThreadLocal(LocalStorageKey key) { return SbThreadGetLocalValue(key); }
...
public:
    const char* LocalTimezone(double time_ms) override { return SbTimeZoneGetName(); }
...
TimezoneCache* OS::CreateTimezoneCache() { return new StarboardDefaultTimezoneCache(); }
