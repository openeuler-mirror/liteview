...
#if !V8_OS_FUCHSIA
void OS::Initialize(bool hard_abort, const char* const gc_fake_mmap) { PosixInitializeCommon(hard_abort, gc_fake_mmap); }
#endif // !V8_OS_FUCHSIA
...
bool OS::DecommitPages(void* address, size_t size)
{
    ...
    void* ptr = mmap(address, size, PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    return ptr == address;
}
#endif // !defined(_AIX)
...
// static
void OS::FreeAddressSpaceReservation(AddressSpaceReservation reservation) { Free(reservation.base(), reservation.size()); }
...
void OS::Sleep(TimeDelta interval) { usleep(static_cast<useconds_t>(interval.InMicroseconds())); }
...
void OS::StrNCpy(char* dest, int length, const char* src, size_t n) { strncpy(dest, src, n); }
