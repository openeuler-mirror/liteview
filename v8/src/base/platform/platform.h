...
public:
    ...
    static std::vector<MemoryRange> GetFreeMemoryRangesWithin(Address boundary_start, Address boundary_end, size_t minimum_size, size_t alignment);
    ...
    {
#if (defined(V8_OS_MACOS) || defined(V8_OS_LINUX)) && !(defined(V8_TARGET_ARCH_PPC64) || defined(V8_TARGET_ARCH_S390X))
        return true;
#else
        return false;
#endif
...
private:
    ...
    V8_WARN_UNUSED_RESULT static void* AllocateShared(void* address, size_t size, OS::MemoryPermission access, PlatformSharedMemoryHandle handle, uint64_t offset);
    ...
    V8_WARN_UNUSED_RESULT static Optional<AddressSpaceReservation> CreateAddressSpaceReservation(void* hint, size_t size, size_t alignment, MemoryPermission max_permission);
...
public:
    ...
    V8_WARN_UNUSED_RESULT Optional<AddressSpaceReservation> CreateSubReservation(void* address, size_t size, OS::MemoryPermission max_permission);
...
public:
    ...
    static StackSlot GetRealStackAddressForSlot(StackSlot slot)
    {
        ...
        return real_frame ? (static_cast<char*>(real_frame) + kAsanRealFrameOffsetBytes) : slot;
#endif // V8_USE_ADDRESS_SANITIZER
        return slot;
    }
};
