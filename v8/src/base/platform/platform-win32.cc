...
Win32Time::Win32Time(double jstime) { t() = static_cast<int64_t>(jstime) * kTimeScaler + kTimeEpoc; }
...
double Win32Time::ToJSTime() { return static_cast<double>((t() - kTimeEpoc) / kTimeScaler); }
...
int64_t Win32Time::DaylightSavingsOffset(WindowsTimezoneCache* cache) { return InDST(cache) ? 60 * kMsPerMinute : 0; }
...
const char* WindowsTimezoneCache::LocalTimezone(double time) { return Win32Time(time).LocalTimezone(this); }
...
int OS::GetCurrentProcessId() { return static_cast<int>(::GetCurrentProcessId()); }

int OS::GetCurrentThreadId() { return static_cast<int>(::GetCurrentThreadId()); }
...
void OS::VPrint(const char* format, va_list args) { VPrintHelper(stdout, format, args); }
...
void OS::VFPrint(FILE* out, const char* format, va_list args) { VPrintHelper(out, format, args); }
...
void OS::VPrintError(const char* format, va_list args) { VPrintHelper(stderr, format, args); }
...
void OS::Initialize(bool hard_abort, const char* const gc_fake_mmap) { g_hard_abort = hard_abort; }
...
// static
void OS::FreeShared(void* address, size_t size) { CHECK(UnmapViewOfFile(address)); }
...
// static
bool OS::RecommitPages(void* address, size_t size, MemoryPermission access) { return SetPermissions(address, size, access); }
...
// static
bool OS::CanReserveAddressSpace() { return VirtualAlloc2 != nullptr && MapViewOfFile3 != nullptr && UnmapViewOfFile2 != nullptr; }

// static
Optional<AddressSpaceReservation> OS::CreateAddressSpaceReservation(void* hint, size_t size, size_t alignment, MemoryPermission max_permission)
{
...
// static
void OS::FreeAddressSpaceReservation(AddressSpaceReservation reservation) { OS::Free(reservation.base(), reservation.size()); }
...
void OS::Sleep(TimeDelta interval) { ::Sleep(static_cast<DWORD>(interval.InMilliseconds())); }
...
Optional<AddressSpaceReservation> AddressSpaceReservation::CreateSubReservation(void* address, size_t size, OS::MemoryPermission max_permission)
{
...
using DLL_FUNC_TYPE(SymLoadModule64) = DWORD64(__stdcall*)(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName, IN DWORD64 BaseOfDll, IN DWORD SizeOfDll);
using DLL_FUNC_TYPE(StackWalk64)
    = BOOL(__stdcall*)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME64 StackFrame, PVOID ContextRecord, PREAD_PROCESS_MEMORY_ROUTINE64 ReadMemoryRoutine,
...
#else // __MINGW32__
std::vector<OS::SharedLibraryAddress> OS::GetSharedLibraryAddresses() { return std::vector<OS::SharedLibraryAddress>(); }
...
void* Thread::GetThreadLocal(LocalStorageKey key) { return TlsGetValue(static_cast<DWORD>(key)); }
...
std::vector<OS::MemoryRange> OS::GetFreeMemoryRangesWithin(OS::Address boundary_start, OS::Address boundary_end, size_t minimum_size, size_t alignment)
{
