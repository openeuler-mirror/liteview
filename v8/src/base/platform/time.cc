...
int64_t ComputeThreadTicks()
{
    ...
    kern_return_t kr = thread_info(pthread_mach_thread_np(pthread_self()), THREAD_BASIC_INFO, reinterpret_cast<thread_info_t>(&thread_info_data), &thread_info_count);
...
std::ostream& operator<<(std::ostream& os, const Time& time) { return os << time.ToJsTime(); }
...
TimeDelta QPCValueToTimeDelta(LONGLONG qpc_value)
{
    ...
    return TimeDelta::FromMicroseconds((whole_seconds * TimeTicks::kMicrosecondsPerSecond) + ((leftover_ticks * TimeTicks::kMicrosecondsPerSecond) / g_qpc_ticks_per_second));
}
...
ThreadTicks ThreadTicks::GetForThread(const HANDLE& thread_handle)
{
    ...
    ::QueryThreadCycleTimeXp(thread_handle, &thread_cycle_time);
