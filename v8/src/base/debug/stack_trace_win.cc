...
#include "patch_code/dbghelp_xp.h" // SUPPORT_XP_CODE
...
bool InitializeSymbols()
{
    ...
    // not number of bytes!
    if (!SymGetSearchPathWXp(GetCurrentProcess(), symbols_path.get(), kSymbolsArraySize)) {
        g_init_error = GetLastError();
        return false;
    }
    ...
    if (!SymSetSearchPathWXp(GetCurrentProcess(), new_path.c_str())) {
        g_init_error = GetLastError();
        return false;
    }
...
void DisableSignalStackDump() { g_dump_stack_in_signal_handler = false; }
...
StackTrace::StackTrace(EXCEPTION_POINTERS* exception_pointers) { InitTrace(exception_pointers->ContextRecord); }
...
#error Unsupported Arch
    ...
    while (StackWalk64(machine_type, GetCurrentProcess(), GetCurrentThread(), &stack_frame, &context_copy, nullptr, &SymFunctionTableAccess64, &SymGetModuleBase64, nullptr)
