...
public:
    ...
    SmallVector(const SmallVector& other, const Allocator& allocator = Allocator()) V8_NOEXCEPT : allocator_(allocator) { *this = other; }
    SmallVector(SmallVector&& other, const Allocator& allocator = Allocator()) V8_NOEXCEPT : allocator_(allocator) { *this = std::move(other); }
    ...
    {
        T* end = end_;
        if (V8_UNLIKELY(end == end_of_storage_))
            end = Grow();
...
private:
    ...
    T* AllocateDynamicStorage(size_t number_of_elements) { return allocator_.allocate(number_of_elements); }
    ...
    const T* inline_storage_begin() const { return reinterpret_cast<const T*>(&inline_storage_); }
};
