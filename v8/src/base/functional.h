...
template <typename T, typename... Ts> V8_INLINE size_t hash_combine(T const& v, Ts const&... vs) { return hash_combine(hash_combine(vs...), hash<T>()(v)); }
...
V8_INLINE size_t hash_value(unsigned int v) { return hash_value_unsigned_impl(v); }
...
#define V8_BASE_HASH_VALUE_SIGNED(type)                                                                                                                                            \
    V8_INLINE size_t hash_value(signed type v) { return hash_value(base::bit_cast<unsigned type>(v)); }
...
template <typename T, size_t N> V8_INLINE size_t hash_value(const T (&v)[N]) { return hash_range(v, v + N); }

template <typename T, size_t N> V8_INLINE size_t hash_value(T (&v)[N]) { return hash_range(v, v + N); }

template <typename T> V8_INLINE size_t hash_value(T* const& v) { return hash_value(base::bit_cast<uintptr_t>(v)); }

template <typename T1, typename T2> V8_INLINE size_t hash_value(std::pair<T1, T2> const& v) { return hash_combine(v.first, v.second); }

template <typename... T, size_t... I> V8_INLINE size_t hash_value_impl(std::tuple<T...> const& v, std::index_sequence<I...>) { return hash_combine(std::get<I>(v)...); }

template <typename... T> V8_INLINE size_t hash_value(std::tuple<T...> const& v) { return hash_value_impl(v, std::make_index_sequence<sizeof...(T)>()); }

template <typename T, typename = std::enable_if_t<std::is_enum<T>::value>> V8_INLINE size_t hash_value(T v) { return hash_value(static_cast<std::underlying_type_t<T>>(v)); }
...
#define V8_BASE_HASH_SPECIALIZE(type)                                                                                                                                              \
    template <> struct hash<type> {                                                                                                                                                \
        V8_INLINE size_t operator()(type const v) const { return ::v8::base::hash_value(v); }                                                                                      \
    };
...
template <typename T> struct hash<T*> {
    V8_INLINE size_t operator()(T* const v) const { return ::v8::base::hash_value(v); }
};
...
#define V8_BASE_BIT_SPECIALIZE_BIT_CAST(type, btype)                                                                                                                               \
    template <> struct bit_equal_to<type> {                                                                                                                                        \
        V8_INLINE bool operator()(type lhs, type rhs) const { return base::bit_cast<btype>(lhs) == base::bit_cast<btype>(rhs); }                                                   \
    };                                                                                                                                                                             \
