...
template <typename T> struct FormattedStringPart {
    static_assert(sizeof(T) < 0, "unimplemented type, add specialization below if needed");
};

template <> struct FormattedStringPart<int> {
...
template <> struct FormattedStringPart<size_t> {
...
template <size_t N> struct FormattedStringPart<char[N]> {
...
public:
    ...
    V8_INLINE V8_WARN_UNUSED_RESULT std::array<char, kMaxLen> PrintToArray() const { return std::apply(impl::PrintFormattedStringToArray<kFormat, kMaxLen, Part<Ts>...>, parts_); }
