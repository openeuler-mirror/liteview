...
BoundedPageAllocator::BoundedPageAllocator(
    v8::PageAllocator* page_allocator, Address start, size_t size, size_t allocate_page_size, PageInitializationMode page_initialization_mode, PageFreeingMode page_freeing_mode)
...
BoundedPageAllocator::Address BoundedPageAllocator::begin() const { return region_allocator_.begin(); }
...
bool BoundedPageAllocator::ReserveForSharedMemoryMapping(void* ptr, size_t size)
{
    ...
    CHECK(page_allocator_->SetPermissions(ptr, size, PageAllocator::Permission::kNoAccess));
    return true;
}
...
bool BoundedPageAllocator::FreePages(void* raw_address, size_t size)
{
    ...
    if (page_initialization_mode_ == PageInitializationMode::kAllocatedPagesMustBeZeroInitialized) {
        ...
        if (page_freeing_mode_ == PageFreeingMode::kMakeInaccessible) {
            CHECK(page_allocator_->SetPermissions(raw_address, size, PageAllocator::kNoAccess));
        } else {
...
bool BoundedPageAllocator::ReleasePages(void* raw_address, size_t size, size_t new_size)
{
    ...
    if (page_initialization_mode_ == PageInitializationMode::kAllocatedPagesMustBeZeroInitialized) {
        ...
        DCHECK_EQ(page_initialization_mode_, PageInitializationMode::kAllocatedPagesCanBeUninitialized);
        if (page_freeing_mode_ == PageFreeingMode::kMakeInaccessible) {
            CHECK(page_allocator_->SetPermissions(free_address, free_size, PageAllocator::kNoAccess));
        } else {
            CHECK_EQ(page_freeing_mode_, PageFreeingMode::kDiscard);
...
bool BoundedPageAllocator::DiscardSystemPages(void* address, size_t size) { return page_allocator_->DiscardSystemPages(address, size); }

bool BoundedPageAllocator::DecommitPages(void* address, size_t size) { return page_allocator_->DecommitPages(address, size); }
