...
inline typename std::enable_if<std::is_same<decltype(std::declval<Function>()(0)), byte*>::value, void>::type VLQEncodeUnsigned(Function&& process_byte, uint32_t value)
{
...
template <typename Function>
inline typename std::enable_if<std::is_same<decltype(std::declval<Function>()(0)), byte*>::value, void>::type VLQEncode(Function&& process_byte, int32_t value)
{
...
template <typename A> inline void VLQEncode(std::vector<byte, A>* data, int32_t value)
{
    VLQEncode(
        [data](byte value) {
...
template <typename A> inline void VLQEncodeUnsigned(std::vector<byte, A>* data, uint32_t value)
{
    VLQEncodeUnsigned(
        [data](byte value) {
            data->push_back(value);
...
inline typename std::enable_if<std::is_same<decltype(std::declval<GetNextFunction>()()), byte>::value, uint32_t>::type VLQDecodeUnsigned(GetNextFunction&& get_next)
{
