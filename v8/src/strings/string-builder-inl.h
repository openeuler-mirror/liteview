...
public:
    ...
    template <int N> V8_INLINE void AppendCStringLiteral(const char (&literal)[N])
    {
        ...
        if (encoding_ == String::ONE_BYTE_ENCODING && CurrentPartCanFit(N)) {
            ...
            SeqOneByteString::cast(*current_part_).SeqOneByteStringSetChars(current_index_, chars, length);
    ...
    V8_INLINE void AppendCString(const base::uc16* s)
    {
        if (encoding_ == String::ONE_BYTE_ENCODING) {
            while (*s != '\0')
                Append<base::uc16, uint8_t>(*(s++));
        } else {
            while (*s != '\0')
                Append<base::uc16, base::uc16>(*(s++));
        }
    }
    ...
    V8_INLINE bool CurrentPartCanFit(int length) { return part_length_ - current_index_ > length; }
    ...
    public:
        NoExtend(String string, int offset, const DisallowGarbageCollection& no_gc)
        {
            DCHECK(string.IsSeqOneByteString() || string.IsSeqTwoByteString());
            if (sizeof(DestChar) == 1) {
                start_ = reinterpret_cast<DestChar*>(SeqOneByteString::cast(string).GetChars(no_gc) + offset);
            } else {
                start_ = reinterpret_cast<DestChar*>(SeqTwoByteString::cast(string).GetChars(no_gc) + offset);
            }
            cursor_ = start_;
#ifdef DEBUG
            string_ = string;
    ...
    template <typename DestChar> class NoExtendString : public NoExtend<DestChar> {
    public:
    ...
    public:
        NoExtendBuilder(IncrementalStringBuilder* builder, int required_length, const DisallowGarbageCollection& no_gc)
...
private:
    ...
    V8_INLINE void set_accumulator(Handle<String> string) { accumulator_.PatchValue(*string); }

    V8_INLINE Handle<String> current_part() { return current_part_; }

    V8_INLINE void set_current_part(Handle<String> string) { current_part_.PatchValue(*string); }
    ...
    void ShrinkCurrentPart()
    {
        ...
        set_current_part(SeqString::Truncate(Handle<SeqString>::cast(current_part()), current_index_));
    }
...
template <typename SrcChar, typename DestChar> void IncrementalStringBuilder::Append(SrcChar c)
{
    ...
    if (sizeof(DestChar) == 1) {
        DCHECK_EQ(String::ONE_BYTE_ENCODING, encoding_);
        SeqOneByteString::cast(*current_part_).SeqOneByteStringSet(current_index_++, c);
    } else {
        DCHECK_EQ(String::TWO_BYTE_ENCODING, encoding_);
        SeqTwoByteString::cast(*current_part_).SeqTwoByteStringSet(current_index_++, c);
    }
    if (current_index_ == part_length_)
        Extend();
    DCHECK(HasValidCurrentIndex());
