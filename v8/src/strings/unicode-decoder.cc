...
template <> struct DecoderTraits<Wtf8Decoder> {
    static bool IsInvalidSurrogatePair(uint32_t lead, uint32_t trail) { return unibrow::Utf16::IsSurrogatePair(lead, trail); }
...
Utf8DecoderBase<Decoder>::Utf8DecoderBase(const base::Vector<const uint8_t>& data)
...
template <class Decoder> template <typename Char> void Utf8DecoderBase<Decoder>::Decode(Char* out, const base::Vector<const uint8_t>& data)
{
    using Traits = DecoderTraits<Decoder>;
    DCHECK(!is_invalid());
...
#define DEFINE_UNICODE_DECODER(Decoder)                                                                                                                                            \
    template V8_EXPORT_PRIVATE Utf8DecoderBase<Decoder>::Utf8DecoderBase(const base::Vector<const uint8_t>& data);                                                                 \
    template V8_EXPORT_PRIVATE void Utf8DecoderBase<Decoder>::Decode(uint8_t* out, const base::Vector<const uint8_t>& data);                                                       \
    template V8_EXPORT_PRIVATE void Utf8DecoderBase<Decoder>::Decode(uint16_t* out, const base::Vector<const uint8_t>& data)
