...
void StringStream::Log(Isolate* isolate) { LOG(isolate, StringEvent("StackDump", buffer_)); }
...
Handle<String> StringStream::ToString(Isolate* isolate) { return isolate->factory()->NewStringFromUtf8(base::Vector<const char>(buffer_, length_)).ToHandleChecked(); }
...
#ifdef DEBUG
bool StringStream::IsMentionedObjectCacheClear(Isolate* isolate) { return object_print_mode_ == kPrintObjectConcise || isolate->string_stream_debug_object_cache()->size() == 0; }
#endif
...
void StringStream::PrintFunction(JSFunction fun, Object receiver) { PrintPrototype(fun, receiver); }
...
void StringStream::PrintPrototype(JSFunction fun, Object receiver)
{
    ...
    if (receiver.IsNullOrUndefined(isolate) || receiver.IsTheHole(isolate) || receiver.IsJSProxy()) {
        ...
        for (PrototypeIterator iter(isolate, JSObject::cast(receiver), kStartAtReceiver); !iter.IsAtEnd(); iter.Advance()) {
