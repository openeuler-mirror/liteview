...
template <typename sinkchar> void StringBuilderConcatHelper(String special, sinkchar* sink, FixedArray fixed_array, int array_length)
{
...
template void StringBuilderConcatHelper<uint8_t>(String special, uint8_t* sink, FixedArray fixed_array, int array_length);

template void StringBuilderConcatHelper<base::uc16>(String special, base::uc16* sink, FixedArray fixed_array, int array_length);

int StringBuilderConcatLength(int special_length, FixedArray fixed_array, int array_length, bool* one_byte)
{
    DisallowGarbageCollection no_gc;
    int position = 0;
    for (int i = 0; i < array_length; i++) {
...
ReplacementStringBuilder::ReplacementStringBuilder(Heap* heap, Handle<String> subject, int estimated_part_count)
    : heap_(heap)
...
void ReplacementStringBuilder::EnsureCapacity(int elements) { array_builder_.EnsureCapacity(Isolate::FromHeap(heap_), elements); }
...
MaybeHandle<String> ReplacementStringBuilder::ToString()
{
    ...
    if (is_one_byte_) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, seq, isolate->factory()->NewRawOneByteString(character_count_), String);
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, seq, isolate->factory()->NewRawTwoByteString(character_count_), String);
...
int IncrementalStringBuilder::Length() const { return accumulator_->length() + current_index_; }

bool IncrementalStringBuilder::HasValidCurrentIndex() const { return current_index_ < part_length_; }
...
bool IncrementalStringBuilder::CanAppendByCopy(Handle<String> string)
{
    ...
    return representation_ok && string->length() <= kMaxStringLengthForCopy && CurrentPartCanFit(string->length());
}
...
void IncrementalStringBuilder::AppendStringByCopy(Handle<String> string)
{
    ...
    Handle<SeqOneByteString> part = Handle<SeqOneByteString>::cast(current_part());
    {
        DisallowGarbageCollection no_gc;
        String::WriteToFlat(*string, part->GetChars(no_gc) + current_index_, 0, string->length());
    }
