...
// static
MergePointInterpreterFrameState* MergePointInterpreterFrameState::NewForCatchBlock(
    const MaglevCompilationUnit& unit, const compiler::BytecodeLivenessState* liveness, int handler_offset, interpreter::Register context_register, Graph* graph, bool is_inline)
{
    Zone* const zone = unit.zone();
    MergePointInterpreterFrameState* state = zone->New<MergePointInterpreterFrameState>(unit, 0, 0, nullptr, BasicBlockType::kExceptionHandlerStart, liveness);
    ...
    if (frame_state.liveness()->AccumulatorIsLive()) {
        frame_state.accumulator(unit) = state->NewExceptionPhi(zone, interpreter::Register::virtual_accumulator(), handler_offset);
    }
    frame_state.ForEachParameter(unit, [&](ValueNode*& entry, interpreter::Register reg) {
    ...
    frame_state.context(unit) = state->NewExceptionPhi(zone, context_register, handler_offset);
    frame_state.ForEachLocal(unit, [&](ValueNode*& entry, interpreter::Register reg) { entry = state->NewExceptionPhi(zone, reg, handler_offset); });
