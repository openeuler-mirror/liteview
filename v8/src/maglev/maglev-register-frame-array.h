...
public:
    explicit RegisterFrameArray(const MaglevCompilationUnit& info)
    {
        ...
        T* frame = info.zone()->NewArray<T>(last_local.index() - last_param.index() + 1);
    ...
    const T& operator[](interpreter::Register reg) const { return frame_start_[reg.index()]; }
...
private:
    ...
    T* data_begin(int parameter_count) const { return frame_start_ + interpreter::Register::FromParameterIndex(parameter_count - 1).index(); }
