...
public:
    ...
    void MarkInputUses(JumpLoop* node, const ProcessingState& state)
    {
        ...
        if (!loop_used_nodes.used_nodes.empty()) {
            ...
            base::Vector<Input> used_node_inputs = compilation_info_->zone()->NewVector<Input>(loop_used_nodes.used_nodes.size());
            int i = 0;
...
private:
    ...
    void MarkCheckpointNodes(NodeBase* node, const EagerDeoptInfo* deopt_info, LoopUsedNodes* loop_used_nodes, const ProcessingState& state)
    {
        int use_id = node->id();
        detail::DeepForEachInput(deopt_info, [&](ValueNode* node, interpreter::Register reg, InputLocation* input) { MarkUse(node, use_id, input, loop_used_nodes); });
    }
    void MarkCheckpointNodes(NodeBase* node, const LazyDeoptInfo* deopt_info, LoopUsedNodes* loop_used_nodes, const ProcessingState& state)
    {
        int use_id = node->id();
        detail::DeepForEachInput(deopt_info, [&](ValueNode* node, interpreter::Register reg, InputLocation* input) { MarkUse(node, use_id, input, loop_used_nodes); });
    }
...
public:
    explicit TranslationArrayProcessor(LocalIsolate* local_isolate, MaglevCompilationInfo* compilation_info)
    ...
    {
        translation_array_builder_.reset(new TranslationArrayBuilder(compilation_info_->zone()));
        deopt_literals_.reset(new IdentityMap<int, base::DefaultAllocationPolicy>(local_isolate_->heap()->heap()));

        tagged_slots_ = graph->tagged_stack_slots();
    }

    void PostProcessGraph(Graph* graph) { compilation_info_->set_translation_array_builder(std::move(translation_array_builder_), std::move(deopt_literals_)); }
    void PreProcessBasicBlock(BasicBlock* block) { }
...
private:
    void EmitDeoptFrame(const MaglevCompilationUnit& unit, const CheckpointedInterpreterState& state, const InputLocation* input_locations)
    {
        if (state.parent) {
        ...
        translation_array_builder().BeginInterpretedFrame(
            state.bytecode_position, GetDeoptLiteral(*unit.shared_function_info().object()), unit.register_count(), return_offset, return_count);

        EmitDeoptFrameValues(unit, state.register_frame, input_locations, interpreter::Register::invalid_value(), return_count);
    }
    ...
    void EmitEagerDeopt(EagerDeoptInfo* deopt_info)
    {
        ...
        deopt_info->translation_index = translation_array_builder().BeginTranslation(frame_count, jsframe_count, update_feedback_count);

        EmitDeoptFrame(deopt_info->unit, deopt_info->state, deopt_info->input_locations);
    }
    ...
    void EmitLazyDeopt(LazyDeoptInfo* deopt_info)
    {
        ...
        if (deopt_info->state.parent) {
            // Deopt input locations are in the order of deopt frame emission, so
            // update the pointer after emitting the parent frame.
            EmitDeoptFrame(*unit.caller(), *deopt_info->state.parent, input_locations);
        }
        ...
        int return_offset;
        if (deopt_info->result_location == interpreter::Register::virtual_accumulator()) {
            return_offset = 0;
            ...
            //   2 + 2 - 1 = 3
            return_offset = unit.register_count() + unit.parameter_count() - deopt_info->result_location.ToParameterIndex();
        } else {
            return_offset = unit.register_count() - deopt_info->result_location.index();
        }
        translation_array_builder().BeginInterpretedFrame(
            deopt_info->state.bytecode_position, GetDeoptLiteral(*unit.shared_function_info().object()), unit.register_count(), return_offset, deopt_info->result_size);

        EmitDeoptFrameValues(unit, deopt_info->state.register_frame, input_locations, deopt_info->result_location, deopt_info->result_size);
    }

    void EmitDeoptStoreRegister(const compiler::AllocatedOperand& operand, ValueRepresentation repr)
    {
        switch (repr) {
        ...
        case ValueRepresentation::kFloat64:
            translation_array_builder().StoreDoubleRegister(operand.GetDoubleRegister());
            break;
        }
    }

    void EmitDeoptStoreStackSlot(const compiler::AllocatedOperand& operand, ValueRepresentation repr)
    {
    ...
    void EmitDeoptFrameSingleValue(ValueNode* value, const InputLocation& input_location)
    {
        if (input_location.operand().IsConstant()) {
            translation_array_builder().StoreLiteral(GetDeoptLiteral(*value->Reify(local_isolate_)));
        } else {
    ...
    constexpr int DeoptStackSlotIndexFromFPOffset(int offset) { return 1 - offset / kSystemPointerSize; }
    ...
    inline constexpr int GetFramePointerOffsetForStackSlot(int index) { return StandardFrameConstants::kExpressionsOffset - index * kSystemPointerSize; }

    int DeoptStackSlotFromStackSlot(const compiler::AllocatedOperand& operand) { return DeoptStackSlotIndexFromFPOffset(GetFramePointerOffsetForStackSlot(operand)); }

    bool InReturnValues(interpreter::Register reg, interpreter::Register result_location, int result_size)
    {
    ...
    void EmitDeoptFrameValues(const MaglevCompilationUnit& compilation_unit, const CompactInterpreterFrameState* checkpoint_state, const InputLocation*& input_location,
        interpreter::Register result_location, int result_size)
    {
        // Closure
        if (compilation_unit.inlining_depth() == 0) {
            int closure_index = DeoptStackSlotIndexFromFPOffset(StandardFrameConstants::kFunctionOffset);
            translation_array_builder().StoreStackSlot(closure_index);
        } else {
            translation_array_builder().StoreLiteral(GetDeoptLiteral(*compilation_unit.function().object()));
        }
        ...
        // Accumulator
        {
            if (checkpoint_state->liveness()->AccumulatorIsLive() && !InReturnValues(interpreter::Register::virtual_accumulator(), result_location, result_size)) {
    ...
    TranslationArrayBuilder& translation_array_builder() { return *translation_array_builder_; }

    LocalIsolate* local_isolate_;
    MaglevCompilationInfo* compilation_info_;
    std::unique_ptr<TranslationArrayBuilder> translation_array_builder_;
    std::unique_ptr<IdentityMap<int, base::DefaultAllocationPolicy>> deopt_literals_;
    int tagged_slots_;
};

// static
void MaglevCompiler::Compile(LocalIsolate* local_isolate, MaglevCompilationInfo* compilation_info)
{
    compiler::UnparkedScopeIfNeeded unparked_scope(compilation_info->broker());

    // Build graph.
    if (v8_flags.print_maglev_code || v8_flags.code_comments || v8_flags.print_maglev_graph || v8_flags.trace_maglev_graph_building || v8_flags.trace_maglev_regalloc) {
        compilation_info->set_graph_labeller(new MaglevGraphLabeller());
    }

    if (v8_flags.print_maglev_code || v8_flags.print_maglev_graph || v8_flags.trace_maglev_graph_building || v8_flags.trace_maglev_regalloc) {
        MaglevCompilationUnit* top_level_unit = compilation_info->toplevel_compilation_unit();
        std::cout << "Compiling " << Brief(*top_level_unit->function().object()) << " with Maglev\n";
    ...
    {
        GraphMultiProcessor<UseMarkingProcessor, MaglevVregAllocator> processor(UseMarkingProcessor { compilation_info });
    ...
    StraightForwardRegisterAllocator allocator(compilation_info, graph_builder.graph());
    ...
    GraphProcessor<TranslationArrayProcessor> build_translation_array(local_isolate, compilation_info);
...
// static
MaybeHandle<CodeT> MaglevCompiler::GenerateCode(Isolate* isolate, MaglevCompilationInfo* compilation_info)
{
    ...
    Handle<Code> code;
    if (!MaglevCodeGenerator::Generate(isolate, compilation_info, graph).ToHandle(&code)) {
        compilation_info->toplevel_compilation_unit()->shared_function_info().object()->set_maglev_compilation_failed(true);
        return {};
    }
