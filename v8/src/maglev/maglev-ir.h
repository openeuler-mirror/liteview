...
inline std::ostream& operator<<(std::ostream& os, Opcode opcode) { return os << OpcodeToString(opcode); }
...
constexpr bool IsValueNode(Opcode opcode) { return kFirstValueNodeOpcode <= opcode && opcode <= kLastValueNodeOpcode; }
constexpr bool IsConstantNode(Opcode opcode) { return kFirstConstantNodeOpcode <= opcode && opcode <= kLastConstantNodeOpcode; }
constexpr bool IsGapMoveNode(Opcode opcode) { return kFirstGapMoveNodeOpcode <= opcode && opcode <= kLastGapMoveNodeOpcode; }
constexpr bool IsControlNode(Opcode opcode) { return kFirstControlNodeOpcode <= opcode && opcode <= kLastControlNodeOpcode; }
constexpr bool IsBranchControlNode(Opcode opcode) { return kFirstBranchControlNodeOpcode <= opcode && opcode <= kLastBranchControlNodeOpcode; }
constexpr bool IsConditionalControlNode(Opcode opcode) { return kFirstConditionalControlNodeOpcode <= opcode && opcode <= kLastConditionalControlNodeOpcode; }
constexpr bool IsUnconditionalControlNode(Opcode opcode) { return kFirstUnconditionalControlNodeOpcode <= opcode && opcode <= kLastUnconditionalControlNodeOpcode; }
constexpr bool IsTerminalControlNode(Opcode opcode) { return kFirstTerminalControlNodeOpcode <= opcode && opcode <= kLastTerminalControlNodeOpcode; }
...
public:
    ...
    constexpr bool can_eager_deopt() const { return kCanEagerDeoptBit::decode(bitfield_); }
    constexpr bool can_lazy_deopt() const { return kCanLazyDeoptBit::decode(bitfield_); }
    constexpr bool can_throw() const { return kCanThrowBit::decode(bitfield_) && can_lazy_deopt(); }
    ...
    constexpr bool non_memory_side_effects() const { return kNonMemorySideEffectsBit::decode(bitfield_); }
    constexpr ValueRepresentation value_representation() const { return kValueRepresentationBits::decode(bitfield_); }
    constexpr bool is_conversion() const { return kIsConversionBit::decode(bitfield_); }
    constexpr bool needs_register_snapshot() const { return kNeedsRegisterSnapshotBit::decode(bitfield_); }
    constexpr bool is_pure() const { return (bitfield_ & kPureMask) == kPureValue; }
    constexpr bool is_required_when_unused() const { return can_write() || non_memory_side_effects(); }

    constexpr OpProperties operator|(const OpProperties& that) { return OpProperties(bitfield_ | that.bitfield_); }

    static constexpr OpProperties Pure() { return OpProperties(kPureValue); }
    static constexpr OpProperties Call() { return OpProperties(kIsCallBit::encode(true)); }
    static constexpr OpProperties EagerDeopt() { return OpProperties(kCanEagerDeoptBit::encode(true)); }
    static constexpr OpProperties LazyDeopt() { return OpProperties(kCanLazyDeoptBit::encode(true)); }
    static constexpr OpProperties Throw() { return OpProperties(kCanThrowBit::encode(true)) | LazyDeopt(); }
    static constexpr OpProperties Reading() { return OpProperties(kCanReadBit::encode(true)); }
    static constexpr OpProperties Writing() { return OpProperties(kCanWriteBit::encode(true)); }
    static constexpr OpProperties NonMemorySideEffects() { return OpProperties(kNonMemorySideEffectsBit::encode(true)); }
    static constexpr OpProperties TaggedValue() { return OpProperties(kValueRepresentationBits::encode(ValueRepresentation::kTagged)); }
    static constexpr OpProperties Int32() { return OpProperties(kValueRepresentationBits::encode(ValueRepresentation::kInt32)); }
    static constexpr OpProperties Float64() { return OpProperties(kValueRepresentationBits::encode(ValueRepresentation::kFloat64)); }
    static constexpr OpProperties ConversionNode() { return OpProperties(kIsConversionBit::encode(true)); }
    static constexpr OpProperties NeedsRegisterSnapshot() { return OpProperties(kNeedsRegisterSnapshotBit::encode(true)); }
    ...
    static constexpr OpProperties GenericRuntimeOrBuiltinCall() { return Call() | NonMemorySideEffects() | LazyDeopt() | Throw(); }
    static constexpr OpProperties JSCall() { return Call() | NonMemorySideEffects() | LazyDeopt() | Throw(); }
    static constexpr OpProperties AnySideEffects() { return Reading() | Writing() | NonMemorySideEffects(); }
...
private:
    ...
    using kValueRepresentationBits = kNonMemorySideEffectsBit::Next<ValueRepresentation, 2>;
    using kIsConversionBit = kValueRepresentationBits::Next<bool, 1>;
    using kNeedsRegisterSnapshotBit = kIsConversionBit::Next<bool, 1>;

    static const uint32_t kPureMask = kCanReadBit::kMask | kCanWriteBit::kMask | kNonMemorySideEffectsBit::kMask;
    static const uint32_t kPureValue = kCanReadBit::encode(false) | kCanWriteBit::encode(false) | kNonMemorySideEffectsBit::encode(false);
...
public:
    ...
    void InjectLocation(compiler::InstructionOperand location) { operand_ = location; }
...
public:
    ...
    CheckpointedInterpreterState(BytecodeOffset bytecode_position, const CompactInterpreterFrameState* state, const CheckpointedInterpreterState* parent)
...
class DeoptInfo {
protected:
    DeoptInfo(Zone* zone, const MaglevCompilationUnit& compilation_unit, CheckpointedInterpreterState checkpoint);
...
public:
    EagerDeoptInfo(Zone* zone, const MaglevCompilationUnit& compilation_unit, CheckpointedInterpreterState checkpoint)
...
public:
    LazyDeoptInfo(Zone* zone, const MaglevCompilationUnit& compilation_unit, CheckpointedInterpreterState checkpoint)
        : DeoptInfo(zone, compilation_unit, checkpoint)
    {
    }
    ...
    interpreter::Register result_location = interpreter::Register::invalid_value();
    int result_size = 1;
};
...
public:
    ...
    bool HasExceptionHandler() { return pc_offset != kNoExceptionHandlerPCOffsetMarker; }
...
template <typename T> T* ObjectPtrBeforeAddress(void* address)
{
...
template <typename T> const T* ObjectPtrBeforeAddress(const void* address)
{
...
protected:
    ...
    template <class T, int size> using NextBitField = InputCountField::Next<T, size>;
...
public:
    ...
    static Derived* New(Zone* zone, const MaglevCompilationUnit& compilation_unit, CheckpointedInterpreterState checkpoint, Args&&... args)
    {
        Derived* node = New<Derived>(zone, std::forward<Args>(args)...);
        if constexpr (Derived::kProperties.can_eager_deopt()) {
            new (node->eager_deopt_info()) EagerDeoptInfo(zone, compilation_unit, checkpoint);
        } else {
            static_assert(Derived::kProperties.can_lazy_deopt());
            new (node->lazy_deopt_info()) LazyDeoptInfo(zone, compilation_unit, checkpoint);
        }
        return node;
    }
    ...
    constexpr OpProperties properties() const { return OpPropertiesField::decode(bitfield_); }
    ...
    template <class T> constexpr T* TryCast() { return Is<T>() ? static_cast<T*>(this) : nullptr; }
...
protected:
    ...
    const Input* input_base() const { return detail::ObjectPtrBeforeAddress<Input>(this); }
    Input* last_input() { return &input(input_count() - 1); }
    const Input* last_input() const { return &input(input_count() - 1); }

    Address last_input_address() const { return reinterpret_cast<Address>(last_input()); }

    void set_input(int index, ValueNode* node) { new (&input(index)) Input(node); }
    ...
    void RequireSpecificDoubleTemporary(DoubleRegister reg) { double_temporaries_.set(reg); }

private:
    template <class Derived, typename... Args> static Derived* Allocate(Zone* zone, size_t input_count, Args&&... args)
    {
        ...
        constexpr size_t size_before_inputs = RoundUp<alignof(Input)>((Derived::kProperties.can_throw() ? sizeof(ExceptionHandlerInfo) : 0)
            + (Derived::kProperties.needs_register_snapshot() ? sizeof(RegisterSnapshot) : 0) + (Derived::kProperties.can_eager_deopt() ? sizeof(EagerDeoptInfo) : 0)
    ...
    Address deopt_info_address() const
    {
        ...
        size_t extra = RoundUp<alignof(Input)>((properties().can_eager_deopt() ? sizeof(EagerDeoptInfo) : 0) + (properties().can_lazy_deopt() ? sizeof(LazyDeoptInfo) : 0));
    ...
    {
        size_t extra = RoundUp<alignof(Input)>((properties().needs_register_snapshot() ? sizeof(RegisterSnapshot) : 0));
    ...
    {
        size_t extra = RoundUp<alignof(Input)>((properties().can_throw() ? sizeof(ExceptionHandlerInfo) : 0));
...
template <class T> constexpr bool NodeBase::Is() const { return opcode() == opcode_of<T>; }

// Specialized sub-hierarchy type checks.
template <> constexpr bool NodeBase::Is<ValueNode>() const { return IsValueNode(opcode()); }
template <> constexpr bool NodeBase::Is<ControlNode>() const { return IsControlNode(opcode()); }
template <> constexpr bool NodeBase::Is<BranchControlNode>() const { return IsBranchControlNode(opcode()); }
template <> constexpr bool NodeBase::Is<ConditionalControlNode>() const { return IsConditionalControlNode(opcode()); }
template <> constexpr bool NodeBase::Is<UnconditionalControlNode>() const { return IsUnconditionalControlNode(opcode()); }
template <> constexpr bool NodeBase::Is<TerminalControlNode>() const { return IsTerminalControlNode(opcode()); }
...
public:
    ...
    constexpr bool use_double_register() const { return (properties().value_representation() == ValueRepresentation::kFloat64); }

    constexpr bool is_tagged() const { return (properties().value_representation() == ValueRepresentation::kTagged); }
...
template <class Derived> class NodeT : public Node {
...
protected:
    explicit NodeT(uint64_t bitfield)
        : Node(bitfield)
    {
...
template <size_t InputCount, class Derived> class FixedInputNodeT : public NodeT<Derived> {
...
public:
    ...
    auto end() { return std::make_reverse_iterator(&this->input(input_count() - 1)); }

protected:
    explicit FixedInputNodeT(uint64_t bitfield)
        : NodeT<Derived>(bitfield)
    {
        // DCHECK_EQ(NodeBase::input_count(), kInputCount);
        NodeBase* node = nullptr;
        DCHECK_EQ(node->input_count(), kInputCount);
    }
};

template <class Derived> class ValueNodeT : public ValueNode {
    static_assert(IsValueNode(opcode_of<Derived>));
...
protected:
    explicit ValueNodeT(uint64_t bitfield)
        : ValueNode(bitfield)
    {
        DCHECK_EQ(NodeBase::opcode(), opcode_of<Derived>);
    }
...
template <size_t InputCount, class Derived> class FixedInputValueNodeT : public ValueNodeT<Derived> {
    static constexpr size_t kInputCount = InputCount;
...
#define DEF_UNARY_WITH_FEEDBACK_NODE(Name) DEF_OPERATION_NODE(Generic##Name, UnaryWithFeedbackNode, Name)
#define DEF_BINARY_WITH_FEEDBACK_NODE(Name) DEF_OPERATION_NODE(Generic##Name, BinaryWithFeedbackNode, Name)
...
template <class Derived, Operation kOperation> class Int32CompareNode : public FixedInputValueNodeT<2, Derived> {
    using Base = FixedInputValueNodeT<2, Derived>;
...
protected:
    explicit Int32CompareNode(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
#define DEF_INT32_COMPARE_NODE(Name) DEF_OPERATION_NODE(Int32##Name, Int32CompareNode, Name)
...
template <class Derived, Operation kOperation> class Float64CompareNode : public FixedInputValueNodeT<2, Derived> {
    using Base = FixedInputValueNodeT<2, Derived>;
...
protected:
    explicit Float64CompareNode(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
#define DEF_FLOAT64_COMPARE_NODE(Name) DEF_OPERATION_NODE(Float64##Name, Float64CompareNode, Name)
...
public:
    explicit CheckedSmiTag(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit UnsafeSmiTag(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    ...
    bool ToBoolean(LocalIsolate* local_isolate) const { return value_ != 0.0 && !std::isnan(value_); }
...
public:
    explicit Float64Box(uint64_t bitfield)
        : Base(bitfield)
    {
    }

    static constexpr OpProperties kProperties = OpProperties::DeferredCall() | OpProperties::ConversionNode();
...
public:
    explicit CheckedFloat64Unbox(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit ToBooleanLogicalNot(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit TestUndetectable(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    ...
    int num_parameters_and_registers() const { return input_count() - kFixedInputCount; }
    Input& parameters_and_registers(int i) { return input(i + kFixedInputCount); }
    void set_parameters_and_registers(int i, ValueNode* node) { set_input(i + kFixedInputCount, node); }
...
public:
    explicit JumpLoopPrologue(uint64_t bitfield, int32_t loop_depth, FeedbackSlot feedback_slot, BytecodeOffset osr_offset, MaglevCompilationUnit* unit)
        : Base(bitfield)
    ...
    static constexpr OpProperties kProperties = OpProperties::NeedsRegisterSnapshot() | OpProperties::EagerDeopt();
...
public:
    explicit GetIterator(uint64_t bitfield, int load_slot, int call_slot, const compiler::FeedbackVectorRef& feedback)
        : Base(bitfield)
...
public:
    explicit ToObject(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit ToString(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
class GeneratorRestoreRegister : public FixedInputValueNodeT<1, GeneratorRestoreRegister> {
...
public:
    static constexpr RegList kAllowedRegisters = { kJavaScriptCallNewTargetRegister };
...
public:
    ...
    bool ToBoolean(LocalIsolate* local_isolate) const { return value_ != Smi::FromInt(0); }
...
public:
    ...
    bool ToBoolean(LocalIsolate* local_isolate) const { return object_.object()->BooleanValue(local_isolate); }
...
class CreateEmptyArrayLiteral : public FixedInputValueNodeT<0, CreateEmptyArrayLiteral> {
    using Base = FixedInputValueNodeT<0, CreateEmptyArrayLiteral>;

public:
    explicit CreateEmptyArrayLiteral(uint64_t bitfield, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , feedback_(feedback)
    {
    }
...
public:
    explicit CreateArrayLiteral(uint64_t bitfield, const compiler::HeapObjectRef& constant_elements, const compiler::FeedbackSource& feedback, int flags)
        : Base(bitfield)
    ...
    // The implementation currently calls runtime.
    static constexpr OpProperties kProperties = OpProperties::Call() | OpProperties::Throw() | OpProperties::LazyDeopt();
...
public:
    explicit CreateShallowArrayLiteral(uint64_t bitfield, const compiler::HeapObjectRef& constant_elements, const compiler::FeedbackSource& feedback, int flags)
        : Base(bitfield)
        , constant_elements_(constant_elements)
        , feedback_(feedback)
        , flags_(flags)
...
public:
    explicit CreateObjectLiteral(uint64_t bitfield, const compiler::ObjectBoilerplateDescriptionRef& boilerplate_descriptor, const compiler::FeedbackSource& feedback, int flags)
        : Base(bitfield)
    ...
    compiler::ObjectBoilerplateDescriptionRef boilerplate_descriptor() { return boilerplate_descriptor_; }
    compiler::FeedbackSource feedback() const { return feedback_; }
    int flags() const { return flags_; }
...
class CreateEmptyObjectLiteral : public FixedInputValueNodeT<0, CreateEmptyObjectLiteral> {
...
public:
    explicit CreateShallowObjectLiteral(
        uint64_t bitfield, const compiler::ObjectBoilerplateDescriptionRef& boilerplate_descriptor, const compiler::FeedbackSource& feedback, int flags)
        : Base(bitfield)
        , boilerplate_descriptor_(boilerplate_descriptor)
        , feedback_(feedback)
        , flags_(flags)
...
public:
    ...
    compiler::SharedFunctionInfoRef shared_function_info() const { return shared_function_info_; }
...
public:
    explicit CreateRegExpLiteral(uint64_t bitfield, const compiler::StringRef& pattern, const compiler::FeedbackSource& feedback, int flags)
        : Base(bitfield)
        , pattern_(pattern)
        , feedback_(feedback)
        , flags_(flags)
...
public:
    explicit CheckMaps(uint64_t bitfield, const compiler::MapRef& map, CheckType check_type)
        : Base(bitfield)
        , map_(map)
...
public:
    explicit CheckMapsWithMigration(uint64_t bitfield, const compiler::MapRef& map, CheckType check_type)
        : Base(bitfield)
        , map_(map)
        , check_type_(check_type)
    {
...
public:
    explicit CheckJSArrayBounds(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
class CheckJSObjectElementsBounds : public FixedInputNodeT<2, CheckJSObjectElementsBounds> {
    using Base = FixedInputNodeT<2, CheckJSObjectElementsBounds>;

public:
    explicit CheckJSObjectElementsBounds(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit CheckedInternalizedString(uint64_t bitfield, CheckType check_type = CheckType::kCheckHeapObject)
        : Base(bitfield)
        , check_type_(check_type)
    {
...
public:
    explicit CheckedObjectToIndex(uint64_t bitfield)
        : Base(bitfield)
    {
    }

    static constexpr OpProperties kProperties = OpProperties::EagerDeopt() | OpProperties::Int32() | OpProperties::DeferredCall() | OpProperties::ConversionNode();
...
public:
    explicit GetTemplateObject(uint64_t bitfield, const compiler::SharedFunctionInfoRef& shared_function_info, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , shared_function_info_(shared_function_info)
        , feedback_(feedback)
    {
    ...
    compiler::SharedFunctionInfoRef shared_function_info() { return shared_function_info_; }
    compiler::FeedbackSource feedback() const { return feedback_; }
...
public:
    ...
    static constexpr OpProperties kProperties = OpProperties::Reading() | OpProperties::Float64();
...
public:
    explicit LoadTaggedElement(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    explicit LoadDoubleElement(uint64_t bitfield)
        : Base(bitfield)
    {
    }
...
public:
    ...
    static constexpr OpProperties kProperties = OpProperties::Writing() | OpProperties::DeferredCall();
...
public:
    explicit LoadGlobal(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback, TypeofMode typeof_mode)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
...
public:
    explicit StoreGlobal(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
    {
...
public:
    explicit LoadNamedGeneric(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
    {
...
public:
    explicit LoadNamedFromSuperGeneric(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
    {
...
public:
    explicit SetNamedGeneric(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
    {
...
public:
    ...
    static constexpr OpProperties kProperties = OpProperties::Reading() | OpProperties::Int32();
...
public:
    explicit DefineNamedOwnGeneric(uint64_t bitfield, const compiler::NameRef& name, const compiler::FeedbackSource& feedback)
        : Base(bitfield)
        , name_(name)
        , feedback_(feedback)
    {
...
class DefineKeyedOwnGeneric : public FixedInputValueNodeT<4, DefineKeyedOwnGeneric> {
...
public:
    ...
    // This ctor is used when for variable input counts.
    // Inputs must be initialized manually.
    Call(uint64_t bitfield, ConvertReceiverMode mode, ValueNode* function, ValueNode* context)
        : Base(bitfield)
        , receiver_mode_(mode)
    {
    ...
    void set_arg(int i, ValueNode* node) { set_input(i + kFixedInputCount, node); }
...
public:
    ...
    void set_feedback(compiler::FeedbackSource& feedback, FeedbackSlotType slot_type)
    {
        feedback_ = feedback;
    ...
    int ReturnCount() const { return Builtins::CallInterfaceDescriptorFor(builtin_).GetReturnCount(); }
...
public:
    ...
    int ReturnCount() const { return Runtime::FunctionForId(function_id())->result_size; }
...
public:
    ...
    // This ctor is used when for variable input counts.
    // Inputs must be initialized manually.
    ConstructWithSpread(uint64_t bitfield, ValueNode* function, ValueNode* new_target, ValueNode* context)
        : Base(bitfield)
    {
        set_input(kFunctionIndex, function);
        set_input(kNewTargetIndex, new_target);
...
class IncreaseInterruptBudget : public FixedInputNodeT<0, IncreaseInterruptBudget> {
...
public:
    ...
    static constexpr OpProperties kProperties = OpProperties::Throw() | OpProperties::DeferredCall();
...
public:
    ...
    // of this node).
    ControlNode* next_post_dominating_hole() const { return next_post_dominating_hole_; }
...
template <class Derived> class UnconditionalControlNodeT : public UnconditionalControlNode {
...
public:
    ...
    void set_true_interrupt_correction(int interrupt_budget_correction) { if_true_.set_interrupt_budget_correction(interrupt_budget_correction); }
    void set_false_interrupt_correction(int interrupt_budget_correction) { if_false_.set_interrupt_budget_correction(interrupt_budget_correction); }
...
template <class Derived> class TerminalControlNodeT : public TerminalControlNode {
...
template <size_t InputCount, class Derived> class BranchControlNodeT : public BranchControlNode {
...
public:
    ...
    void set_used_nodes(base::Vector<Input> locations) { used_node_locations_ = locations; }
...
public:
    explicit JumpToInlined(uint64_t bitfield, BasicBlockRef* target_refs, MaglevCompilationUnit* unit)
        : Base(bitfield, target_refs)
        , unit_(unit)
    {
    }
...
public:
    explicit BranchIfRootConstant(uint64_t bitfield, BasicBlockRef* if_true_refs, BasicBlockRef* if_false_refs, RootIndex root_index)
...
public:
    explicit BranchIfToBooleanTrue(uint64_t bitfield, BasicBlockRef* if_true_refs, BasicBlockRef* if_false_refs)
        : Base(bitfield, if_true_refs, if_false_refs)
    {
    }
...
class BranchIfReferenceCompare : public BranchControlNodeT<2, BranchIfReferenceCompare> {
...
public:
    ...
    explicit BranchIfReferenceCompare(uint64_t bitfield, Operation operation, BasicBlockRef* if_true_refs, BasicBlockRef* if_false_refs)
        : Base(bitfield, if_true_refs, if_false_refs)
        , operation_(operation)
    {
    }
