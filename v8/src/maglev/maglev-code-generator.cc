...
template <> struct RegisterTHelper<DoubleRegister> {
    ...
    static constexpr DoubleRegList kAllocatableRegisters = kAllocatableDoubleRegisters;
};
...
template <typename RegisterT> class ParallelMoveResolver {
    static constexpr RegisterT kScratchRegT = RegisterTHelper<RegisterT>::kScratch;
...
public:
    ...
    void RecordMove(ValueNode* source_node, compiler::InstructionOperand source, compiler::AllocatedOperand target)
    {
    ...
    void RecordMove(ValueNode* source_node, compiler::InstructionOperand source, RegisterT target_reg) { RecordMoveToRegister(source_node, source, target_reg); }
...
private:
    ...
    struct GapMoveTargets {
        ...
        base::SmallVector<uint32_t, 1> stack_slots = base::SmallVector<uint32_t, 1> {};
        ...
        bool is_empty() const { return registers.is_empty() && stack_slots.empty(); }
    };
    ...
    void CheckNoExistingMoveToStackSlot(uint32_t target_slot)
    {
        for (Register reg : kAllocatableRegistersT) {
            ...
            if (std::any_of(stack_slots.begin(), stack_slots.end(), [&](uint32_t slot) { return slot == target_slot; })) {
    ...
    void RecordMoveToRegister(ValueNode* node, compiler::InstructionOperand source, RegisterT target_reg)
    {
        ...
        if (source.IsAnyRegister()) {
            ...
            uint32_t source_slot = masm_->GetFramePointerOffsetForStackSlot(compiler::AllocatedOperand::cast(source));
    ...
    void RecordMoveToStackSlot(ValueNode* node, compiler::InstructionOperand source, uint32_t target_slot)
    {
        ...
        if (source.IsAnyRegister()) {
            RegisterT source_reg = ToRegisterT<RegisterT>(source);
            moves_from_register_[source_reg.code()].stack_slots.push_back(target_slot);
        } else if (source.IsAnyStackSlot()) {
            uint32_t source_slot = masm_->GetFramePointerOffsetForStackSlot(compiler::AllocatedOperand::cast(source));
    ...
    GapMoveTargets PopTargets(RegisterT source_reg) { return std::exchange(moves_from_register_[source_reg.code()], GapMoveTargets {}); }
    ...
    void EmitMovesFromSource(RegisterT source_reg, const GapMoveTargets& targets)
    {
    ...
    void EmitMovesFromSource(uint32_t source_slot, const GapMoveTargets& targets)
    {
    ...
    // machine move.
    void EmitStackMove(uint32_t target_slot, Register source_reg) { __ movq(MemOperand(rbp, target_slot), source_reg); }
    void EmitStackMove(uint32_t target_slot, DoubleRegister source_reg) { __ Movsd(MemOperand(rbp, target_slot), source_reg); }
    void EmitStackMove(Register target_reg, uint32_t source_slot) { __ movq(target_reg, MemOperand(rbp, source_slot)); }
    void EmitStackMove(DoubleRegister target_reg, uint32_t source_slot) { __ Movsd(target_reg, MemOperand(rbp, source_slot)); }
...
private:
    ...
    void EmitTrampolineFor(NodeBase* node)
    {
        ...
        RecordMoves(deopt_info->unit, catch_block, deopt_info->state.register_frame, &direct_moves, &materialising_moves, &save_accumulator);
    ...
    void RecordMoves(const MaglevCompilationUnit& unit, BasicBlock* catch_block, const CompactInterpreterFrameState* register_frame, ParallelMoveResolver<Register>* direct_moves,
    ...
    void EmitPopMaterialisedResults(const MoveVector& moves, bool save_accumulator) const
    {
        if (moves.size() == 0)
            return;
...
public:
    ...
    void PreProcessGraph(Graph* graph)
    {
        ...
        if (v8_flags.maglev_ool_prologue) {
            ...
            // per Maglev code object on x64).
            {
                ...
                __ LoadFeedbackVectorFlagsAndJumpIfNeedsProcessing(flags, feedback_vector, CodeKind::MAGLEV, &flags_need_processing);
                __ jmp(&next);

                __ bind(&flags_need_processing);
                {
                    ASM_CODE_COMMENT_STRING(masm(), "Optimized marker check");
                    __ OptimizeCodeOrTailCallOptimizedCodeSlot(flags, feedback_vector, kJSFunctionRegister, JumpMode::kJump);
                    __ Trap();
                }
...

            {
                ...
                __ subq(kScratchRegister, Immediate(code_gen_state()->stack_slots() * kSystemPointerSize));
            ...
            if (graph->untagged_stack_slots() > 0) {
                ...
                __ subq(rsp, Immediate(graph->untagged_stack_slots() * kSystemPointerSize));
            }
        }
    }
    ...
    void PostProcessGraph(Graph*)
    {
        ...
        if (!v8_flags.maglev_ool_prologue) {
            ...
            // Push the frame size
            __ Push(Immediate(Smi::FromInt(code_gen_state()->stack_slots() * kSystemPointerSize)));
    ...
    template <typename NodeT> void Process(NodeT* node, const ProcessingState& state)
    {
        if (v8_flags.code_comments) {
            std::stringstream ss;
        ...
        if (v8_flags.debug_code) {
            ...
            __ cmpq(kScratchRegister, Immediate(code_gen_state()->stack_slots() * kSystemPointerSize + StandardFrameConstants::kFixedFrameSizeFromFp));
            ...
            if (value_node->is_spilled()) {
                ...
                if (!source.IsAnyStackSlot()) {
                    ...
                    if (source.IsRegister()) {
                        __ movq(masm()->GetStackSlot(value_node->spill_slot()), ToRegister(source));
                    } else {
                        __ Movsd(masm()->GetStackSlot(value_node->spill_slot()), ToDoubleRegister(source));
                    }
                } else {
        ...
        if (target->has_phi()) {
            ...
            for (Phi* phi : *phis) {
                ...
                if (!phi->has_valid_live_range()) {
                    if (v8_flags.code_comments) {
                        std::stringstream ss;
                        ss << "--   * " << phi->input(state.block()->predecessor_id()).operand() << " → " << target << " (n" << graph_labeller()->NodeId(phi) << ") [DEAD]";
                        __ RecordComment(ss.str());
                    }
                    continue;
                }
    ...
    MaglevCodeGenState* code_gen_state() const { return masm()->code_gen_state(); }
    MaglevGraphLabeller* graph_labeller() const { return code_gen_state()->graph_labeller(); }
    MaglevSafepointTableBuilder* safepoint_table_builder() const { return code_gen_state()->safepoint_table_builder(); }
...
public:
    static MaybeHandle<Code> Generate(Isolate* isolate, MaglevCompilationInfo* compilation_info, Graph* graph)
    {
        return MaglevCodeGeneratorImpl(isolate, compilation_info, graph).Generate();
    }

private:
    MaglevCodeGeneratorImpl(Isolate* isolate, MaglevCompilationInfo* compilation_info, Graph* graph)
    ...
    void EmitDeopts()
    {
        ...
        for (EagerDeoptInfo* deopt_info : code_gen_state_.eager_deopts()) {
            ...
            __ RecordDeoptReason(deopt_info->reason, 0, SourcePosition::Unknown(), deopt_index);
            __ bind(&deopt_info->deopt_entry_label);
            __ CallForDeoptimization(Builtin::kDeoptimizationEntry_Eager, deopt_index, &deopt_info->deopt_entry_label, DeoptimizeKind::kEager, nullptr, nullptr);
            deopt_index++;
        }
        ...
        for (LazyDeoptInfo* deopt_info : code_gen_state_.lazy_deopts()) {
            __ bind(&deopt_info->deopt_entry_label);
            __ CallForDeoptimization(Builtin::kDeoptimizationEntry_Lazy, deopt_index, &deopt_info->deopt_entry_label, DeoptimizeKind::kLazy, nullptr, nullptr);

            last_updated_safepoint
                = safepoint_table_builder_.UpdateDeoptimizationInfo(deopt_info->deopting_call_return_pc, deopt_info->deopt_entry_label.pos(), last_updated_safepoint, deopt_index);
            deopt_index++;
        }
    }
    ...
    {
        if (code_gen_state_.handlers().size() == 0)
            return;
    ...
    void EmitMetadata()
    {
        ...
        for (NodeBase* node : code_gen_state_.handlers()) {
            ...
            HandlerTable::EmitReturnEntry(masm(), info->pc_offset, info->trampoline_entry.pos());
        }
    }
    ...
    {
        CodeDesc desc;
        masm()->GetCode(isolate(), &desc, safepoint_table_builder(), handler_table_offset_);
    ...
    Handle<DeoptimizationData> GenerateDeoptimizationData()
    {
        ...
        Handle<TranslationArray> translation_array = code_gen_state_.compilation_info()->translation_array_builder().ToTranslationArray(isolate()->factory());
        {
            ...
            raw_data.SetSharedFunctionInfo(*code_gen_state_.compilation_info()->toplevel_compilation_unit()->shared_function_info().object());
        }

        IdentityMap<int, base::DefaultAllocationPolicy>& deopt_literals = code_gen_state_.compilation_info()->deopt_literals();
        Handle<DeoptimizationLiteralArray> literals = isolate()->factory()->NewDeoptimizationLiteralArray(deopt_literals.size() + 1);
        // TODO(leszeks): Fix with the real inlining positions.
        Handle<PodArray<InliningPosition>> inlining_positions = PodArray<InliningPosition>::New(isolate(), 0);
        DisallowGarbageCollection no_gc;

        auto raw_literals = *literals;
        auto raw_data = *data;
        IdentityMap<int, base::DefaultAllocationPolicy>::IteratableScope iterate(&deopt_literals);
        ...
        raw_literals.set(deopt_literals.size(), *code_gen_state_.compilation_info()->toplevel_compilation_unit()->bytecode().object());
    ...
    int stack_slot_count_with_fixed_frame() const { return stack_slot_count() + StandardFrameConstants::kFixedSlotCount; }

    Isolate* isolate() const { return code_gen_state_.isolate(); }
    MaglevAssembler* masm() { return &masm_; }
    MaglevSafepointTableBuilder* safepoint_table_builder() { return &safepoint_table_builder_; }
...
// static
MaybeHandle<Code> MaglevCodeGenerator::Generate(Isolate* isolate, MaglevCompilationInfo* compilation_info, Graph* graph)
{
