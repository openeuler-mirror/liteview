...
bool IsTargetOfNodeFallthrough(ControlNode* node, BasicBlock* target) { return node->id() + 1 == target->first_id(); }
...
template <typename RegisterT> void ClearDeadFallthroughRegisters(RegisterFrameState<RegisterT> registers, ConditionalControlNode* control_node, BasicBlock* target)
{
    ...
    for (block_it_ = graph_->begin(); block_it_ != graph_->end(); ++block_it_) {
        ...
        if (block->has_state()) {
            if (block->state()->is_exception_handler()) {
                ...
                ClearRegisterValues();
            } else if (block->state()->is_resumable_loop() && block->state()->predecessor_count() <= 1) {
        ...
        if (block->has_phi()) {
            ...
            if (block->is_exception_handler_block()) {
                ...
                if (phi->owner() == interpreter::Register::virtual_accumulator() && !phi->is_dead()) {
            ...
            for (Phi* phi : *block->phis()) {
                ...
                if (!general_registers_.UnblockedFreeIsEmpty()) {
                    compiler::AllocatedOperand allocation = general_registers_.AllocateRegister(phi);
...
void StraightForwardRegisterAllocator::UpdateUse(const EagerDeoptInfo& deopt_info)
{
    detail::DeepForEachInput(&deopt_info, [&](ValueNode* node, interpreter::Register reg, InputLocation* input) {
        if (v8_flags.trace_maglev_regalloc) {
...
void StraightForwardRegisterAllocator::UpdateUse(const LazyDeoptInfo& deopt_info)
{
    detail::DeepForEachInput(&deopt_info, [&](ValueNode* node, interpreter::Register reg, InputLocation* input) {
        if (v8_flags.trace_maglev_regalloc) {
            printing_visitor_->os() << "- using " << PrintNodeLabel(graph_labeller(), node) << "\n";
...
Register GetNodeResultRegister(Node* node)
{
    ...
    if (!value_node->result().operand().IsRegister())
        return Register::no_reg();
...
void StraightForwardRegisterAllocator::AllocateNode(Node* node)
{
    ...
    for (Input& input : *node) {
        if (v8_flags.trace_maglev_regalloc) {
            printing_visitor_->os() << "Using input " << PrintNodeLabel(graph_labeller(), input.node()) << "...\n";
        }
        UpdateUse(&input);
    }
    ...
    DCHECK_EQ(general_registers_.free() | (node->general_temporaries() - GetNodeResultRegister(node)), general_registers_.free());
...
template <typename RegisterT> void StraightForwardRegisterAllocator::DropRegisterValueAtEnd(RegisterT reg)
{
...
template <typename RegisterT> void StraightForwardRegisterAllocator::DropRegisterValue(RegisterFrameState<RegisterT>& registers, RegisterT reg)
{
...
void StraightForwardRegisterAllocator::DropRegisterValue(Register reg) { DropRegisterValue<Register>(general_registers_, reg); }

void StraightForwardRegisterAllocator::DropRegisterValue(DoubleRegister reg) { DropRegisterValue<DoubleRegister>(double_registers_, reg); }
...
void StraightForwardRegisterAllocator::AddMoveBeforeCurrentNode(ValueNode* node, compiler::InstructionOperand source, compiler::AllocatedOperand target)
{
    ...
    if (source.IsConstant()) {
        ...
        gap_move = Node::New<ConstantGapMove>(compilation_info_->zone(), {}, node, target);
    } else {
        if (v8_flags.trace_maglev_regalloc) {
            printing_visitor_->os() << "  gap move: " << target << " ← " << PrintNodeLabel(graph_labeller(), node) << ":" << source << std::endl;
        }
        gap_move = Node::New<GapMove>(compilation_info_->zone(), {}, compiler::AllocatedOperand::cast(source), target);
    }
...
void StraightForwardRegisterAllocator::AssignArbitraryRegisterInput(Input& input)
{
    ...
    if (v8_flags.trace_maglev_regalloc) {
        printing_visitor_->os() << "- " << PrintNodeLabel(graph_labeller(), input.node()) << " in " << location << "\n";
    }

    if (location.IsAnyRegister()) {
        compiler::AllocatedOperand location = node->use_double_register() ? double_registers_.ChooseInputRegister(node) : general_registers_.ChooseInputRegister(node);
...
void StraightForwardRegisterAllocator::AssignInputs(NodeBase* node)
{
    ...
    for (Input& input : *node)
        AssignArbitraryRegisterInput(input);
...
void StraightForwardRegisterAllocator::AllocateSpillSlot(ValueNode* node)
{
    ...
    if (!v8_flags.maglev_reuse_stack_slots || slots.free_slots.empty()) {
        ...
        auto it
            = std::upper_bound(slots.free_slots.begin(), slots.free_slots.end(), start, [](NodeIdT s, const SpillSlotInfo& slot_info) { return slot_info.freed_at_position < s; });
...
template <typename RegisterT> RegisterT StraightForwardRegisterAllocator::FreeUnblockedRegister()
{
    RegisterFrameState<RegisterT>& registers = GetRegisterFrameState<RegisterT>();
...
compiler::AllocatedOperand StraightForwardRegisterAllocator::AllocateRegister(ValueNode* node)
{
...
template <typename RegisterT> void StraightForwardRegisterAllocator::EnsureFreeRegisterAtEnd()
{
    RegisterFrameState<RegisterT>& registers = GetRegisterFrameState<RegisterT>();
...
template <typename RegisterT> compiler::AllocatedOperand StraightForwardRegisterAllocator::ForceAllocate(RegisterFrameState<RegisterT>& registers, RegisterT reg, ValueNode* node)
{
    DCHECK(!registers.is_blocked(reg));
    if (v8_flags.trace_maglev_regalloc) {
...
template <typename RegisterT> compiler::AllocatedOperand RegisterFrameState<RegisterT>::ChooseInputRegister(ValueNode* node)
{
    RegTList blocked = node->result_registers<RegisterT>() & blocked_;
    if (blocked.Count() > 0) {
        return compiler::AllocatedOperand(compiler::LocationOperand::REGISTER, node->GetMachineRepresentation(), blocked.first().code());
    }
    compiler::AllocatedOperand allocation = compiler::AllocatedOperand::cast(node->allocation());
...
template <typename RegisterT> compiler::AllocatedOperand RegisterFrameState<RegisterT>::AllocateRegister(ValueNode* node)
{
...
void StraightForwardRegisterAllocator::MergeRegisterValues(ControlNode* control, BasicBlock* target, int predecessor_id)
{
    ...
    auto merge = [&](auto& registers, auto reg, RegisterState& state) {
        ...
        if (node == nullptr && !incoming->is_loadable()) {
            ...
            if (v8_flags.trace_maglev_regalloc) {
                printing_visitor_->os() << "  " << reg << " - can't load incoming " << PrintNodeLabel(graph_labeller(), node) << ", bailing out\n";
            }
            return;
        }
        ...
        if (node == nullptr) {
            merge->operand(predecessor_id) = register_info;
            if (v8_flags.trace_maglev_regalloc) {
                printing_visitor_->os() << "  " << reg << " - new merge: loading new " << PrintNodeLabel(graph_labeller(), incoming) << " from " << register_info << " \n";
            }
        } else {
            merge->operand(predecessor_id) = node->allocation();
            if (v8_flags.trace_maglev_regalloc) {
