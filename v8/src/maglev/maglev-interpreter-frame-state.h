...
template <typename Value, typename MergeFunc> void DestructivelyIntersect(ZoneMap<ValueNode*, Value>& lhs_map, const ZoneMap<ValueNode*, Value>& rhs_map, MergeFunc&& func)
{
...
inline bool NodeTypeIsAnyHeapObject(NodeType type) { return static_cast<int>(type) & static_cast<int>(NodeType::kAnyHeapObject); }
inline bool NodeTypeIsString(NodeType type) { return type == NodeType::kString; }
inline bool NodeTypeIsSymbol(NodeType type) { return type == NodeType::kSymbol; }
...
struct NodeInfo {
    ...
    bool is_empty() { return type == NodeType::kUnknown && tagged_alternative == nullptr && int32_alternative == nullptr && float64_alternative == nullptr; }
    ...
    {
        type = static_cast<NodeType>(static_cast<int>(type) & static_cast<int>(other.type));
        tagged_alternative = tagged_alternative == other.tagged_alternative ? tagged_alternative : nullptr;
        int32_alternative = int32_alternative == other.int32_alternative ? int32_alternative : nullptr;
        float64_alternative = float64_alternative == other.float64_alternative ? float64_alternative : nullptr;
    }
};
...
struct KnownNodeAspects {
    ...
    void Merge(const KnownNodeAspects& other)
    {
        ...
        DestructivelyIntersect(stable_maps, other.stable_maps, [](compiler::MapRef lhs, compiler::MapRef rhs) { return lhs.equals(rhs); });
        DestructivelyIntersect(unstable_maps, other.unstable_maps, [](compiler::MapRef lhs, compiler::MapRef rhs) { return lhs.equals(rhs); });
    }
...
public:
    ...
    inline void CopyFrom(const MaglevCompilationUnit& info, const MergePointInterpreterFrameState& state);
    ...
    ValueNode* accumulator() const { return frame_[interpreter::Register::virtual_accumulator()]; }

    void set(interpreter::Register reg, ValueNode* value)
    {
        DCHECK_IMPLIES(reg.is_parameter(),
            reg == interpreter::Register::current_context() || reg == interpreter::Register::function_closure() || reg == interpreter::Register::virtual_accumulator()
    ...
    const KnownNodeAspects& known_node_aspects() const { return *known_node_aspects_; }
...
public:
    CompactInterpreterFrameState(const MaglevCompilationUnit& info, const compiler::BytecodeLivenessState* liveness)
        : live_registers_and_accumulator_(info.zone()->NewArray<ValueNode*>(SizeFor(info, liveness)))
        , liveness_(liveness)
    {
    }
    ...
    ValueNode*& context(const MaglevCompilationUnit& info) { return live_registers_and_accumulator_[info.parameter_count()]; }
    ValueNode* context(const MaglevCompilationUnit& info) const { return live_registers_and_accumulator_[info.parameter_count()]; }
    ...
    size_t size(const MaglevCompilationUnit& info) const { return SizeFor(info, liveness_); }
...
public:
    ...
    void CheckIsLoopPhiIfNeeded(const MaglevCompilationUnit& compilation_unit, int merge_offset, interpreter::Register reg, ValueNode* value)
    {
#ifdef DEBUG
        const auto& analysis = compilation_unit.bytecode_analysis();
        if (!analysis.IsLoopHeader(merge_offset))
            return;
        ...
        if (reg.is_parameter()) {
            ...
            DCHECK(analysis.GetInLivenessFor(merge_offset)->RegisterIsLive(reg.index()));
        ...
        BasicBlock* predecessor, const compiler::BytecodeLivenessState* liveness)
    {
        MergePointInterpreterFrameState* merge_state = info.zone()->New<MergePointInterpreterFrameState>(
            info, predecessor_count, 1, info.zone()->NewArray<BasicBlock*>(predecessor_count), BasicBlockType::kDefault, liveness);
        merge_state->frame_state_.ForEachValue(info, [&](ValueNode*& entry, interpreter::Register reg) { entry = state.get(reg); });
        merge_state->predecessors_[0] = predecessor;
        merge_state->known_node_aspects_ = state.known_node_aspects().Clone(info.zone());
        return merge_state;
    }

    static MergePointInterpreterFrameState* NewForLoop(const InterpreterFrameState& start_state, const MaglevCompilationUnit& info, int merge_offset, int predecessor_count,
        const compiler::BytecodeLivenessState* liveness, const compiler::LoopInfo* loop_info)
    {
        MergePointInterpreterFrameState* state = info.zone()->New<MergePointInterpreterFrameState>(
            info, predecessor_count, 0, info.zone()->NewArray<BasicBlock*>(predecessor_count), BasicBlockType::kLoopHeader, liveness);
        ...
        frame_state.context(info) = state->NewLoopPhi(info.zone(), interpreter::Register::current_context(), merge_offset);
    ...
    static MergePointInterpreterFrameState* NewForCatchBlock(const MaglevCompilationUnit& unit, const compiler::BytecodeLivenessState* liveness, int handler_offset,
        interpreter::Register context_register, Graph* graph, bool is_inline);

    // Merges an unmerged framestate with a possibly merged framestate into |this|
    // framestate.
    void Merge(MaglevCompilationUnit& compilation_unit, InterpreterFrameState& unmerged, BasicBlock* predecessor, int merge_offset)
    {
        ...
        frame_state_.ForEachValue(compilation_unit, [&](ValueNode*& value, interpreter::Register reg) {
            ...
            if (v8_flags.trace_maglev_graph_building) {
                std::cout << "  " << reg.ToString() << ": " << PrintNodeLabel(compilation_unit.graph_labeller(), value) << " <- "
                          << PrintNodeLabel(compilation_unit.graph_labeller(), unmerged.get(reg));
            }
            value = MergeValue(compilation_unit, reg, unmerged.known_node_aspects(), value, unmerged.get(reg), merge_offset);
            if (v8_flags.trace_maglev_graph_building) {
                std::cout << " => " << PrintNodeLabel(compilation_unit.graph_labeller(), value) << ": " << PrintNode(compilation_unit.graph_labeller(), value) << std::endl;
            }
        });
        ...
        if (known_node_aspects_ == nullptr) {
            ...
            known_node_aspects_ = unmerged.known_node_aspects().CloneWithoutUnstableMaps(compilation_unit.zone());
        } else {
    ...
    // Merges an unmerged framestate with a possibly merged framestate into |this|
    // framestate.
    void MergeLoop(MaglevCompilationUnit& compilation_unit, InterpreterFrameState& loop_end_state, BasicBlock* loop_end_block, int merge_offset)
    {
        ...
        frame_state_.ForEachValue(compilation_unit, [&](ValueNode* value, interpreter::Register reg) {
            ...
            if (v8_flags.trace_maglev_graph_building) {
                std::cout << "  " << reg.ToString() << ": " << PrintNodeLabel(compilation_unit.graph_labeller(), value) << " <- "
                          << PrintNodeLabel(compilation_unit.graph_labeller(), loop_end_state.get(reg));
            }
            MergeLoopValue(compilation_unit, reg, loop_end_state.known_node_aspects(), value, loop_end_state.get(reg), merge_offset);
            if (v8_flags.trace_maglev_graph_building) {
                std::cout << " => " << PrintNodeLabel(compilation_unit.graph_labeller(), value) << ": " << PrintNode(compilation_unit.graph_labeller(), value) << std::endl;
            }
        });
    ...
    // deopt).
    void MergeDead(const MaglevCompilationUnit& compilation_unit, int merge_offset)
    {
    ...
    void MergeDeadLoop(const MaglevCompilationUnit& compilation_unit, int merge_offset)
    {
        // This should be the last predecessor we try to merge.
        DCHECK_EQ(predecessors_so_far_, predecessor_count_ - 1);
        DCHECK(is_unmerged_loop());
    ...
    const CompactInterpreterFrameState& frame_state() const { return frame_state_; }
    ...
    bool is_loop() const { return basic_block_type_ == BasicBlockType::kLoopHeader; }

    bool is_exception_handler() const { return basic_block_type_ == BasicBlockType::kExceptionHandlerStart; }
...
private:
    friend void InterpreterFrameState::CopyFrom(const MaglevCompilationUnit& info, const MergePointInterpreterFrameState& state);

    template <typename T, typename... Args> friend T* Zone::New(Args&&... args);

    MergePointInterpreterFrameState(const MaglevCompilationUnit& info, int predecessor_count, int predecessors_so_far, BasicBlock** predecessors, BasicBlockType type,
    ...
    ValueNode* FromInt32ToTagged(MaglevCompilationUnit& compilation_unit, KnownNodeAspects& known_node_aspects, ValueNode* value)
    {
        DCHECK_EQ(value->properties().value_representation(), ValueRepresentation::kInt32);
        DCHECK(!value->properties().is_conversion());
#define IS_INT32_OP_NODE(Name) || value->Is<Name>()
        DCHECK(value->Is<Int32Constant>() || value->Is<StringLength>() INT32_OPERATIONS_NODE_LIST(IS_INT32_OP_NODE));
#undef IS_INT32_OP_NODE
        ...
        if (!node_info->tagged_alternative) {
            ...
            if (value->Is<StringLength>()) {
                ...
                tagged = Node::New<CheckedSmiTag, std::initializer_list<ValueNode*>>(compilation_unit.zone(), compilation_unit, value->eager_deopt_info()->state, { value });
            }
    ...
    ValueNode* FromFloat64ToTagged(MaglevCompilationUnit& compilation_unit, KnownNodeAspects& known_node_aspects, ValueNode* value)
    {
    ...
    ValueNode* EnsureTagged(MaglevCompilationUnit& compilation_unit, KnownNodeAspects& known_node_aspects, ValueNode* value)
    {
    ...
    ValueNode* MergeValue(
        MaglevCompilationUnit& compilation_unit, interpreter::Register owner, KnownNodeAspects& unmerged_aspects, ValueNode* merged, ValueNode* unmerged, int merge_offset)
    {
        ...
        result = Node::New<Phi>(compilation_unit.zone(), predecessor_count_, owner, merge_offset);
    ...
    void ReducePhiPredecessorCount(interpreter::Register owner, ValueNode* merged, int merge_offset)
    {
        // If the merged node is null, this is a pre-created loop header merge
    ...
    ValueNode* NewExceptionPhi(Zone* zone, interpreter::Register reg, int handler_offset)
    {
        DCHECK_EQ(predecessors_so_far_, 0);
...
void InterpreterFrameState::CopyFrom(const MaglevCompilationUnit& info, const MergePointInterpreterFrameState& state)
{
