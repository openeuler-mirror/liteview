...
void ExportedMaglevCompilationInfo::set_canonical_handles(std::unique_ptr<CanonicalHandlesMap>&& canonical_handles) { info_->set_canonical_handles(std::move(canonical_handles)); }

// static
std::unique_ptr<MaglevCompilationJob> MaglevCompilationJob::New(Isolate* isolate, Handle<JSFunction> function)
{
    auto info = maglev::MaglevCompilationInfo::New(isolate, function);
    return std::unique_ptr<MaglevCompilationJob>(new MaglevCompilationJob(std::move(info)));
}

MaglevCompilationJob::MaglevCompilationJob(std::unique_ptr<MaglevCompilationInfo>&& info)
...
Handle<JSFunction> MaglevCompilationJob::function() const { return info_->toplevel_compilation_unit()->function().object(); }

bool MaglevCompilationJob::specialize_to_function_context() const { return info_->specialize_to_function_context(); }
...
public:
    ...
    void Run(JobDelegate* delegate) override
    {
        ...
        while (!incoming_queue()->IsEmpty() && !delegate->ShouldYield()) {
            ...
            if (!incoming_queue()->Dequeue(&job))
                break;
            DCHECK_NOT_NULL(job);
            TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.MaglevBackground", job.get(), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);
    ...
    size_t GetMaxConcurrency(size_t) const override { return incoming_queue()->size(); }
    ...
    : isolate_(isolate)
{
    if (v8_flags.concurrent_recompilation && v8_flags.maglev) {
        job_handle_ = V8::GetCurrentPlatform()->PostJob(TaskPriority::kUserVisible, std::make_unique<JobTask>(this));
        DCHECK(is_enabled());
    } else {
        DCHECK(!is_enabled());
    }
...
void MaglevConcurrentDispatcher::FinalizeFinishedJobs()
{
    ...
    while (!outgoing_queue_.IsEmpty()) {
        ...
        TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.MaglevConcurrentFinalize", job.get(), TRACE_EVENT_FLAG_FLOW_IN);
