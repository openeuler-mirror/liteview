...
public:
    explicit MaglevGraphBuilder(LocalIsolate* local_isolate, MaglevCompilationUnit* compilation_unit, Graph* graph, MaglevGraphBuilder* parent = nullptr);
...
private:
    ...
    void ProcessMergePoint(int offset)
    {
        ...
        while (old_jump_targets != nullptr) {
            ...
            if (control->Is<ConditionalControlNode>()) {
                ...
                old_jump_targets = old_jump_targets->MoveToRefList(&jump_targets_[offset]);
            }
    ...
    // targetting it.
    bool IsOffsetAMergePoint(int offset) { return merge_states_[offset] != nullptr; }
    ...
    void VisitSingleBytecode()
    {
        ...
        if (V8_UNLIKELY(merge_state != nullptr)) {
            if (current_block_ != nullptr) {
                ...
                merge_state->Merge(*compilation_unit_, current_interpreter_frame_, predecessor, offset);
            }
            if (v8_flags.trace_maglev_graph_building) {
...
#undef DECLARE_VISITOR
    ...
    template <typename NodeT> NodeT* AddNode(NodeT* node)
    {
    ...
    template <typename NodeT, typename... Args> NodeT* AddNewNode(size_t input_count, Args&&... args)
    {
        return AddNode(CreateNewNode<NodeT>(input_count, std::forward<Args>(args)...));
    }
    ...
    template <typename NodeT, typename... Args> NodeT* CreateNewNodeHelper(Args&&... args)
    {
        if constexpr (NodeT::kProperties.can_eager_deopt()) {
            return NodeBase::New<NodeT>(zone(), *compilation_unit_, GetLatestCheckpointedState(), std::forward<Args>(args)...);
        } else if constexpr (NodeT::kProperties.can_lazy_deopt()) {
            return NodeBase::New<NodeT>(zone(), *compilation_unit_, GetCheckpointedStateForLazyDeopt(), std::forward<Args>(args)...);
        } else {
            return NodeBase::New<NodeT>(zone(), std::forward<Args>(args)...);
        }
    }

    template <typename NodeT, typename... Args> NodeT* CreateNewNode(Args&&... args)
    {
        ...
        if constexpr (NodeT::kProperties.can_throw()) {
            if (catch_block_stack_.size() > 0) {
                ...
                new (node->exception_handler_info()) ExceptionHandlerInfo(&jump_targets_[handler_offset]);
            } else {
    ...
    bool TrySpecializeLoadContextSlotToFunctionContext(ValueNode** context, size_t* depth, int slot_index, ContextSlotMutability slot_mutability);
    void BuildLoadContextSlot(ValueNode* context, size_t depth, int slot_index, ContextSlotMutability slot_mutability);
    ...
    template <Builtin kBuiltin> CallBuiltin* BuildCallBuiltin(std::initializer_list<ValueNode*> inputs)
    {
        ...
        if constexpr (Descriptor::HasContextParameter()) {
            call_builtin = CreateNewNode<CallBuiltin>(inputs.size() + 1, kBuiltin, GetContext());
        } else {
    ...
    CallBuiltin* BuildCallBuiltin(std::initializer_list<ValueNode*> inputs, compiler::FeedbackSource& feedback, CallBuiltin::FeedbackSlotType slot_type = CallBuiltin::kTaggedIndex)
    {
    ...
    CallRuntime* BuildCallRuntime(Runtime::FunctionId function_id, std::initializer_list<ValueNode*> inputs)
    {
        CallRuntime* call_runtime = CreateNewNode<CallRuntime>(inputs.size() + CallRuntime::kFixedInputCount, function_id, GetContext());
        int arg_index = 0;
        for (auto* input : inputs) {
    ...
    void Print(ValueNode* value) { BuildCallRuntime(Runtime::kDebugPrint, { value }); }
    ...
    ValueNode* GetClosure() const { return current_interpreter_frame_.get(interpreter::Register::function_closure()); }

    ValueNode* GetContext() const { return current_interpreter_frame_.get(interpreter::Register::current_context()); }

    void SetContext(ValueNode* context) { current_interpreter_frame_.set(interpreter::Register::current_context(), context); }

    FeedbackSlot GetSlotOperand(int operand_index) const { return iterator_.GetSlotOperand(operand_index); }

    uint32_t GetFlag8Operand(int operand_index) const { return iterator_.GetFlag8Operand(operand_index); }

    uint32_t GetFlag16Operand(int operand_index) const { return iterator_.GetFlag16Operand(operand_index); }

    template <class T, typename = std::enable_if_t<std::is_convertible<T*, Object*>::value>> typename compiler::ref_traits<T>::ref_type GetRefOperand(int operand_index)
    {
        // The BytecodeArray itself was fetched by using a barrier so all reads
        // from the constant pool are safe.
        return MakeRefAssumeMemoryFence(broker(), broker()->CanonicalPersistentHandle(Handle<T>::cast(iterator_.GetConstantForIndexOperand(operand_index, local_isolate()))));
    }
    ...
    RootConstant* GetBooleanConstant(bool value) { return GetRootConstant(value ? RootIndex::kTrueValue : RootIndex::kFalseValue); }
    ...
    ValueNode* GetInt32(interpreter::Register reg)
    {
        ...
        case ValueRepresentation::kFloat64: {
            NodeInfo* node_info = known_node_aspects().GetOrCreateInfoFor(value);
            if (node_info->int32_alternative == nullptr) {
                node_info->int32_alternative = AddNewNode<CheckedTruncateFloat64ToInt32>({ value });
            }
            return node_info->int32_alternative;
        }
        }
    ...
    ValueNode* GetFloat64(interpreter::Register reg)
    {
        ...
        case ValueRepresentation::kTagged: {
            ...
            if (node_info->float64_alternative == nullptr) {
                node_info->float64_alternative = AddNewNode<CheckedFloat64Unbox>({ value });
            }
        ...
        case ValueRepresentation::kInt32: {
            NodeInfo* node_info = known_node_aspects().GetOrCreateInfoFor(value);
            if (node_info->float64_alternative == nullptr) {
                node_info->float64_alternative = AddNewNode<ChangeInt32ToFloat64>({ value });
            }
            return node_info->float64_alternative;
        }
        case ValueRepresentation::kFloat64:
    ...
    ValueNode* GetAccumulatorTagged() { return GetTaggedValue(interpreter::Register::virtual_accumulator()); }

    ValueNode* GetAccumulatorInt32() { return GetInt32(interpreter::Register::virtual_accumulator()); }

    ValueNode* GetAccumulatorFloat64() { return GetFloat64(interpreter::Register::virtual_accumulator()); }
    ...
    ValueNode* LoadRegisterTagged(int operand_index) { return GetTaggedValue(iterator_.GetRegisterOperand(operand_index)); }

    ValueNode* LoadRegisterInt32(int operand_index) { return GetInt32(iterator_.GetRegisterOperand(operand_index)); }

    ValueNode* LoadRegisterFloat64(int operand_index) { return GetFloat64(iterator_.GetRegisterOperand(operand_index)); }

    ValueNode* LoadFixedArrayElement(ValueNode* node, int index) { return AddNewNode<LoadTaggedField>({ node }, FixedArray::OffsetOfElementAt(index)); }
    ...
    CheckpointedInterpreterState GetLatestCheckpointedState()
    {
        if (!latest_checkpointed_state_) {
            latest_checkpointed_state_.emplace(BytecodeOffset(iterator_.current_offset()),
                zone()->New<CompactInterpreterFrameState>(*compilation_unit_, GetInLiveness(), current_interpreter_frame_),
                parent_ == nullptr ? nullptr
                                   // TODO(leszeks): Don't always allocate for the parent state,
                                   // maybe cache it on the graph builder?
                                   : zone()->New<CheckpointedInterpreterState>(parent_->GetLatestCheckpointedState()));
        }
    ...
    {
        return CheckpointedInterpreterState(BytecodeOffset(iterator_.current_offset()),
            zone()->New<CompactInterpreterFrameState>(*compilation_unit_, GetOutLiveness(), current_interpreter_frame_),
            // TODO(leszeks): Support lazy deopts in inlined functions.
            nullptr);
    }

    template <typename NodeT> void MarkAsLazyDeoptResult(NodeT* value, interpreter::Register result_location, int result_size)
    {
        DCHECK_EQ(NodeT::kProperties.can_lazy_deopt(), value->properties().can_lazy_deopt());
    ...
    int next_offset() const { return iterator_.current_offset() + iterator_.current_bytecode_size(); }
    const compiler::BytecodeLivenessState* GetInLiveness() const { return GetInLivenessFor(iterator_.current_offset()); }
    const compiler::BytecodeLivenessState* GetInLivenessFor(int offset) const { return bytecode_analysis().GetInLivenessFor(offset); }
    const compiler::BytecodeLivenessState* GetOutLiveness() const { return GetOutLivenessFor(iterator_.current_offset()); }
    const compiler::BytecodeLivenessState* GetOutLivenessFor(int offset) const { return bytecode_analysis().GetOutLivenessFor(offset); }
    ...
    {
        ControlNode* control_node = CreateNewNode<ControlNodeT>(control_inputs, std::forward<Args>(args)...);
        ...
        if (has_graph_labeller()) {
            ...
            if (v8_flags.trace_maglev_graph_building) {
                ...
                std::cout << "  " << control_node << "  " << PrintNodeLabel(graph_labeller(), control_node) << ": " << PrintNode(graph_labeller(), control_node, kSkipTargets)
                          << std::endl;
            }
        }
        return block;
    ...
    void ResolveJumpsToBlockAtOffset(BasicBlock* block, int block_offset)
    {
        ...
        BasicBlockRef* jump_target_refs_head = jump_targets_[block_offset].SetToBlockAndReturnNext(block);
        while (jump_target_refs_head != nullptr) {
            ...
            interrupt_budget_correction = jump_target_refs_head->interrupt_budget_correction();
            jump_target_refs_head = jump_target_refs_head->SetToBlockAndReturnNext(block);
        }
    ...
    void InlineCallFromRegisters(int argc_count, ConvertReceiverMode receiver_mode, compiler::JSFunctionRef function);
    ...
    bool TryBuildPropertyCellAccess(const compiler::GlobalAccessFeedback& global_access_feedback);
    ...
    bool TryFoldLoadDictPrototypeConstant(compiler::PropertyAccessInfo access_info);
    bool TryFoldLoadConstantDataField(compiler::PropertyAccessInfo access_info);

    void BuildLoadField(compiler::PropertyAccessInfo access_info, ValueNode* lookup_start_object);
    bool TryBuildStoreField(compiler::PropertyAccessInfo access_info, ValueNode* receiver);
    bool TryBuildPropertyGetterCall(compiler::PropertyAccessInfo access_info, ValueNode* receiver);
    bool TryBuildPropertySetterCall(compiler::PropertyAccessInfo access_info, ValueNode* receiver, ValueNode* value);

    bool TryBuildPropertyLoad(ValueNode* receiver, ValueNode* lookup_start_object, compiler::PropertyAccessInfo const& access_info);
    bool TryBuildPropertyStore(ValueNode* receiver, compiler::PropertyAccessInfo const& access_info);
    bool TryBuildPropertyAccess(ValueNode* receiver, ValueNode* lookup_start_object, compiler::PropertyAccessInfo const& access_info, compiler::AccessMode access_mode);

    bool TryBuildNamedAccess(ValueNode* receiver, ValueNode* lookup_start_object, compiler::NamedAccessFeedback const& feedback, compiler::AccessMode access_mode);
    bool TryBuildElementAccess(ValueNode* object, ValueNode* index, compiler::ElementAccessFeedback const& feedback);
    ...
    template <Operation kOperation> ValueNode* AddNewInt32BinaryOperationNode(std::initializer_list<ValueNode*> inputs);
    template <Operation kOperation> ValueNode* AddNewFloat64BinaryOperationNode(std::initializer_list<ValueNode*> inputs);
    ...
    template <Operation kOperation> void BuildFloat64BinaryOperationNode();
    template <Operation kOperation> void BuildFloat64BinarySmiOperationNode();
    ...
    template <typename CompareControlNode> bool TryBuildCompareOperation(Operation operation, ValueNode* left, ValueNode* right);
    ...
    void BuildBranchIfRootConstant(ValueNode* node, JumpType jump_type, RootIndex root_index);
    ...
    const compiler::FeedbackVectorRef& feedback() const { return compilation_unit_->feedback(); }
    ...
    const FeedbackNexus FeedbackNexusForSlot(FeedbackSlot slot) const { return FeedbackNexus(feedback().object(), slot, broker()->feedback_nexus_config()); }
    const compiler::BytecodeArrayRef& bytecode() const { return compilation_unit_->bytecode(); }
    const compiler::BytecodeAnalysis& bytecode_analysis() const { return compilation_unit_->bytecode_analysis(); }
    ...
    bool has_graph_labeller() const { return compilation_unit_->has_graph_labeller(); }
    MaglevGraphLabeller* graph_labeller() const { return compilation_unit_->graph_labeller(); }
    KnownNodeAspects& known_node_aspects() { return current_interpreter_frame_.known_node_aspects(); }
