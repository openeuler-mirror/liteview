...
void PrintPaddedId(std::ostream& os, MaglevGraphLabeller* graph_labeller, NodeIdT max_node_id, NodeBase* node, std::string padding = " ", int padding_adjustement = 0)
{
...
void PrintPadding(std::ostream& os, int size) { os << std::setfill(' ') << std::setw(size) << ""; }
...
std::ostream& operator<<(std::ostream& os, const Connection& c) { return os << c.ToString(); }
...
void PrintVerticalArrows(
    std::ostream& os, const std::vector<BasicBlock*>& targets, std::set<size_t> arrows_starting_here = {}, std::set<BasicBlock*> targets_starting_here = {}, bool is_loop = false)
{
...
public:
    ...
    static MaglevPrintingVisitorOstream* cast(const std::unique_ptr<std::ostream>& os) { return static_cast<MaglevPrintingVisitorOstream*>(os.get()); }
...
template <typename NodeT> void PrintEagerDeopt(std::ostream& os, std::vector<BasicBlock*> targets, NodeT* node, MaglevGraphLabeller* graph_labeller, int max_node_id)
{
    ...
    bool first = true;
    int index = 0;
    deopt_info->state.register_frame->ForEachValue(deopt_info->unit, [&](ValueNode* node, interpreter::Register reg) {
        if (first) {
            first = false;
        } else {
            os << ", ";
        }
        os << reg.ToString() << ":" << PrintNodeLabel(graph_labeller, node) << ":" << deopt_info->input_locations[index].operand();
        index++;
    });
    os << "}\n";
}
...
#define CASE(Name)                                                                                                                                                                 \
    case Opcode::k##Name:                                                                                                                                                          \
        if constexpr (Name::kProperties.can_eager_deopt()) {                                                                                                                       \
            PrintEagerDeopt<Name>(os, targets, node->Cast<Name>(), graph_labeller, max_node_id);                                                                                   \
        }                                                                                                                                                                          \
        break;
        NODE_BASE_LIST(CASE)
#undef CASE
...
template <typename NodeT> void PrintLazyDeopt(std::ostream& os, std::vector<BasicBlock*> targets, NodeT* node, MaglevGraphLabeller* graph_labeller, int max_node_id)
{
    PrintVerticalArrows(os, targets);
    PrintPadding(os, graph_labeller, max_node_id, 0);
    ...
    deopt_info->state.register_frame->ForEachValue(deopt_info->unit, [&](ValueNode* node, interpreter::Register reg) {
        ...
        if (deopt_info->IsResultRegister(reg)) {
            ...
            os << PrintNodeLabel(graph_labeller, node) << ":" << deopt_info->input_locations[index].operand();
            index++;
        }
    });
    os << "}\n";
...
template <typename NodeT> void PrintExceptionHandlerPoint(std::ostream& os, std::vector<BasicBlock*> targets, NodeT* node, MaglevGraphLabeller* graph_labeller, int max_node_id)
{
...
void MaybePrintLazyDeoptOrExceptionHandler(std::ostream& os, std::vector<BasicBlock*> targets, NodeBase* node, MaglevGraphLabeller* graph_labeller, int max_node_id)
{
    switch (node->opcode()) {
#define CASE(Name)                                                                                                                                                                 \
    case Opcode::k##Name:                                                                                                                                                          \
...
void MaglevPrintingVisitor::Process(ControlNode* control_node, const ProcessingState& state)
{
    UNREACHABLE();
...
{
    GraphProcessor<MaglevPrintingVisitor> printer(compilation_info->graph_labeller(), os);
    printer.ProcessGraph(graph);
}

void PrintNode::Print(std::ostream& os) const { node_->Print(os, graph_labeller_, skip_targets_); }
...
void PrintNodeLabel::Print(std::ostream& os) const { graph_labeller_->PrintNodeLabel(os, node_); }
