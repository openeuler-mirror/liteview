...
void LogExecution(Isolate* isolate, Handle<JSFunction> function)
{
    ...
    LOG(isolate, FunctionEvent(event_name.c_str(), Script::cast(raw_sfi.script()).id(), 0, raw_sfi.StartPosition(), raw_sfi.EndPosition(), *name));
...
RUNTIME_FUNCTION(Runtime_HealOptimizedCodeSlot)
{
    ...
    function->feedback_vector().EvictOptimizedCodeMarkedForDeoptimization(function->shared(), "Runtime_HealOptimizedCodeSlot");
    return function->code();
}
...
bool DeoptExitIsInsideOsrLoop(Isolate* isolate, JSFunction function, BytecodeOffset deopt_exit_offset, BytecodeOffset osr_offset)
{
    DisallowGarbageCollection no_gc;
...
bool TryGetOptimizedOsrCode(Isolate* isolate, FeedbackVector vector, const interpreter::BytecodeArrayIterator& it, CodeT* code_out)
{
    base::Optional<CodeT> maybe_code = vector.GetOptimizedOsrCode(isolate, it.GetSlotOperand(2));
...
//  }  // The outermost loop
void DeoptAllOsrLoopsContainingDeoptExit(Isolate* isolate, JSFunction function, BytecodeOffset deopt_exit_offset)
{
    DisallowGarbageCollection no_gc;
    DCHECK(!deopt_exit_offset.IsNone());
...
RUNTIME_FUNCTION(Runtime_NotifyDeoptimized)
{
    ...
    } else if (DeoptExitIsInsideOsrLoop(isolate, *function, deopt_exit_offset, osr_offset)) {
        Deoptimizer::DeoptimizeFunction(*function, ToCodeT(*optimized_code));
    }
...
void GetOsrOffsetAndFunctionForOSR(Isolate* isolate, BytecodeOffset* osr_offset, Handle<JSFunction>* function)
{
    ...
    DCHECK_IMPLIES(frame->is_interpreted(), frame->LookupCodeT().is_interpreter_trampoline_builtin());
    DCHECK_IMPLIES(frame->is_baseline(), frame->LookupCodeT().kind() == CodeKind::BASELINE);
...
Object CompileOptimizedOSR(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset)
{
    const ConcurrencyMode mode = V8_LIKELY(isolate->concurrent_recompilation_enabled() && v8_flags.concurrent_osr) ? ConcurrencyMode::kConcurrent : ConcurrencyMode::kSynchronous;

    Handle<CodeT> result;
    if (!Compiler::CompileOptimizedOSR(isolate, function, osr_offset, mode).ToHandle(&result)) {
    ...
    if (function->feedback_vector().invocation_count() <= 1 && !IsNone(function->tiering_state()) && !IsInProgress(function->tiering_state())) {
...
static Object CompileGlobalEval(
    Isolate* isolate, Handle<i::Object> source_object, Handle<SharedFunctionInfo> outer_info, LanguageMode language_mode, int eval_scope_position, int eval_position)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, compiled,
        Compiler::GetFunctionFromEval(source.ToHandleChecked(), outer_info, context, language_mode, restriction, kNoSourcePosition, eval_scope_position, eval_position),
