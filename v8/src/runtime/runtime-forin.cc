...
MaybeHandle<HeapObject> Enumerate(Isolate* isolate, Handle<JSReceiver> receiver)
{
    ...
    if (!accumulator.is_receiver_simple_enum()) {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(
            isolate, keys, accumulator.GetKeys(accumulator.may_have_elements() ? GetKeysConversion::kConvertToString : GetKeysConversion::kNoNumbers), HeapObject);
    ...
    for (; it.IsFound(); it.Next()) {
        ...
        case LookupIterator::JSPROXY: {
            ...
            if (result.FromJust() == ABSENT) {
                ...
                ASSIGN_RETURN_ON_EXCEPTION(isolate, prototype, JSProxy::GetPrototype(proxy), Object);
                if (prototype->IsNull(isolate)) {
                    return isolate->factory()->undefined_value();
                }
                // We already have a stack-check in JSProxy::GetPrototype.
                return HasEnumerableProperty(isolate, Handle<JSReceiver>::cast(prototype), key);
        ...
        case LookupIterator::WASM_OBJECT:
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kWasmObjectsAreOpaque), Object);
