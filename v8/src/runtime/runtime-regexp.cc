...
int LookupNamedCapture(const std::function<bool(String)>& name_matches, FixedArray capture_name_map)
{
...
private:
    ...
    struct ReplacementPart {
        static inline ReplacementPart SubjectMatch() { return ReplacementPart(SUBJECT_CAPTURE, 0); }
        static inline ReplacementPart SubjectCapture(int capture_index) { return ReplacementPart(SUBJECT_CAPTURE, capture_index); }
        static inline ReplacementPart SubjectPrefix() { return ReplacementPart(SUBJECT_PREFIX, 0); }
        static inline ReplacementPart SubjectSuffix(int subject_length) { return ReplacementPart(SUBJECT_SUFFIX, subject_length); }
        static inline ReplacementPart ReplacementString() { return ReplacementPart(REPLACEMENT_STRING, 0); }
        static inline ReplacementPart EmptyReplacement() { return ReplacementPart(EMPTY_REPLACEMENT, 0); }
    ...
    template <typename Char> bool ParseReplacementPattern(base::Vector<Char> characters, FixedArray capture_name_map, int capture_count, int subject_length)
    {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (c == '$') {
                ...
                case '<': {
                    ...
                    const int capture_index = LookupNamedCapture([=](String capture_name) { return capture_name.IsEqualTo(requested_name); }, capture_name_map);
...
bool CompiledReplacement::Compile(Isolate* isolate, Handle<JSRegExp> regexp, Handle<String> replacement, int capture_count, int subject_length)
{
    {
...
void FindStringIndicesDispatch(Isolate* isolate, String subject, String pattern, std::vector<int>* indices, unsigned int limit)
{
    {
        DisallowGarbageCollection no_gc;
...
V8_WARN_UNUSED_RESULT static Object StringReplaceGlobalAtomRegExpWithString(
    Isolate* isolate, Handle<String> subject, Handle<JSRegExp> pattern_regexp, Handle<String> replacement, Handle<RegExpMatchInfo> last_match_info)
{
...
V8_WARN_UNUSED_RESULT static Object StringReplaceGlobalRegExpWithString(
    Isolate* isolate, Handle<String> subject, Handle<JSRegExp> regexp, Handle<String> replacement, Handle<RegExpMatchInfo> last_match_info)
{
    DCHECK(subject->IsFlat());
    DCHECK(replacement->IsFlat());
    ...
    const bool simple_replace = compiled_replacement.Compile(isolate, regexp, replacement, capture_count, subject_length);
...
V8_WARN_UNUSED_RESULT static Object StringReplaceGlobalRegExpWithEmptyString(
    Isolate* isolate, Handle<String> subject, Handle<JSRegExp> regexp, Handle<RegExpMatchInfo> last_match_info)
{
    DCHECK(subject->IsFlat());
...
RUNTIME_FUNCTION(Runtime_StringSplit)
{
    ...
    Handle<JSArray> result = isolate->factory()->NewJSArray(PACKED_ELEMENTS, part_count, part_count, INITIALIZE_ARRAY_ELEMENTS_WITH_HOLE);
    ...
    if (limit == 0xFFFFFFFFu) {
        if (result->HasObjectElements()) {
            RegExpResultsCache::Enter(isolate, subject, pattern, elements, isolate->factory()->empty_fixed_array(), RegExpResultsCache::STRING_SPLIT_SUBSTRINGS);
        }
    }
...
MaybeHandle<Object> RegExpExec(
    Isolate* isolate, Handle<JSRegExp> regexp, Handle<String> subject, int32_t index, Handle<RegExpMatchInfo> last_match_info, RegExp::ExecQuirks exec_quirks)
{
...
public:
    MatchInfoBackedMatch(Isolate* isolate, Handle<JSRegExp> regexp, Handle<String> subject, Handle<RegExpMatchInfo> match_info)
        : isolate_(isolate)
    ...
    Handle<String> GetMatch() override { return RegExpUtils::GenericCaptureGetter(isolate_, match_info_, 0, nullptr); }
    ...
    int CaptureCount() override { return match_info_->NumberOfCaptureRegisters() / 2; }
    ...
    MaybeHandle<String> GetNamedCapture(Handle<String> name, CaptureState* state) override
    {
        ...
        const int capture_index = LookupNamedCapture([=](String capture_name) { return capture_name.Equals(*name); }, *capture_name_map_);
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate_, capture_value, GetCapture(capture_index, &capture_exists), String);
...
public:
    VectorBackedMatch(Isolate* isolate, Handle<String> subject, Handle<String> match, int match_position, base::Vector<Handle<Object>> captures, Handle<Object> groups_obj)
        : isolate_(isolate)
        , match_(match)
    ...
    Handle<String> GetPrefix() override { return isolate_->factory()->NewSubString(subject_, 0, match_position_); }

    Handle<String> GetSuffix() override
    {
        const int match_end_position = match_position_ + match_->length();
        return isolate_->factory()->NewSubString(subject_, match_end_position, subject_->length());
    }
    ...
    MaybeHandle<String> GetNamedCapture(Handle<String> name, CaptureState* state) override
    {
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate_, capture_obj, Object::GetProperty(isolate_, groups_obj_, name), String);
        if (capture_obj->IsUndefined(isolate_)) {
            *state = UNMATCHED;
            return isolate_->factory()->empty_string();
        } else {
...
Handle<JSObject> ConstructNamedCaptureGroupsObject(Isolate* isolate, Handle<FixedArray> capture_map, const std::function<Object(int)>& f_get_capture)
{
    ...
    for (int i = 0; i < named_capture_count; i++) {
        ...
        Handle<String> capture_name(String::cast(capture_map->get(name_ix)), isolate);
...
static Object SearchRegExpMultiple(Isolate* isolate, Handle<String> subject, Handle<JSRegExp> regexp, Handle<RegExpMatchInfo> last_match_array, Handle<JSArray> result_array)
{
    ...
    if (subject_length > kMinLengthToCache) {
        ...
        Object cached_answer = RegExpResultsCache::Lookup(isolate->heap(), *subject, regexp->data(), &last_match_cache, RegExpResultsCache::REGEXP_MULTIPLE_INDICES);
        if (cached_answer.IsFixedArray()) {
            ...
            Handle<FixedArray> copied_fixed_array = isolate->factory()->CopyFixedArrayWithMap(cached_fixed_array, isolate->factory()->fixed_array_map());
            JSArray::SetContent(result_array, copied_fixed_array);
            RegExp::SetLastMatchInfo(isolate, last_match_array, subject, capture_count, last_match);
    ...
    Handle<FixedArray> result_elements(FixedArray::cast(result_array->elements()), isolate);
        ...
        match_end = current_match[1];
        {
            ...
            if (has_capture) {
                ...
                if (has_named_captures) {
                    ...
                    Handle<JSObject> groups = ConstructNamedCaptureGroupsObject(isolate, capture_map, [=](int ix) { return elements->get(ix); });
    ...
    if (match_start >= 0) {
        ...
        if (subject_length > kMinLengthToCache) {
            ...
            Handle<FixedArray> copied_fixed_array = isolate->factory()->CopyFixedArrayWithMap(result_fixed_array, isolate->factory()->fixed_array_map());
            RegExpResultsCache::Enter(isolate, subject, handle(regexp->data(), isolate), copied_fixed_array, last_match_cache, RegExpResultsCache::REGEXP_MULTIPLE_INDICES);
        }
...
V8_WARN_UNUSED_RESULT MaybeHandle<String> RegExpReplace(Isolate* isolate, Handle<JSRegExp> regexp, Handle<String> string, Handle<String> replace)
{
    ...
    if (!global) {
        ...
        if (sticky) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate, last_index_obj, Object::ToLength(isolate, last_index_obj), String);
        ...
        if (last_index <= static_cast<uint32_t>(string->length())) {
            ASSIGN_RETURN_ON_EXCEPTION(isolate, match_indices_obj, RegExp::Exec(isolate, regexp, string, last_index, last_match_info), String);
        }
        ...
        if (replace->length() > 0) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate, replacement, String::GetSubstitution(isolate, &m, replace), String);
        ...
        DCHECK(global);
        RETURN_ON_EXCEPTION(isolate, RegExpUtils::SetLastIndex(isolate, regexp, 0), String);
        ...
        if (replace->length() == 0) {
            if (string->IsOneByteRepresentation()) {
                Object result = StringReplaceGlobalRegExpWithEmptyString<SeqOneByteString>(isolate, string, regexp, last_match_info);
                return handle(String::cast(result), isolate);
            } else {
                Object result = StringReplaceGlobalRegExpWithEmptyString<SeqTwoByteString>(isolate, string, regexp, last_match_info);
                return handle(String::cast(result), isolate);
            }
        }

        Object result = StringReplaceGlobalRegExpWithString(isolate, string, regexp, replace, last_match_info);
        if (result.IsString()) {
            return handle(String::cast(result), isolate);
        } else {
...
RUNTIME_FUNCTION(Runtime_RegExpExecMultiple)
{
    ...
    if (regexp->capture_count() == 0) {
        result = SearchRegExpMultiple<false>(isolate, subject, regexp, last_match_info, result_array);
    } else {
        result = SearchRegExpMultiple<true>(isolate, subject, regexp, last_match_info, result_array);
    }
    DCHECK(RegExpUtils::IsUnmodifiedRegExp(isolate, regexp));
    return result;
}
...
V8_WARN_UNUSED_RESULT MaybeHandle<Object> ToUint32(Isolate* isolate, Handle<Object> object, uint32_t* out)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, number, Object::ToNumber(isolate, object), Object);
...
{
    return isolate->factory()->NewJSArrayWithElements(FixedArray::ShrinkOrEmpty(isolate, elems, num_elems));
}
...
RUNTIME_FUNCTION(Runtime_RegExpSplit)
{
    ...
    Handle<JSReceiver> splitter;
    {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, splitter_obj, Execution::New(isolate, ctor, argc, argv.begin()));
...
RUNTIME_FUNCTION(Runtime_RegExpReplaceRT)
{
    ...
    if (RegExpUtils::IsUnmodifiedRegExp(isolate, recv)) {
        ...
        Handle<Object> result;
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, result, RegExpReplace(isolate, Handle<JSRegExp>::cast(recv), string, replace));
