...
MaybeHandle<Object> Runtime::GetObjectProperty(Isolate* isolate, Handle<Object> lookup_start_object, Handle<Object> key, Handle<Object> receiver, bool* is_found)
{
    if (receiver.is_null()) {
    ...
    if (!it.IsFound() && key->IsSymbol() && Symbol::cast(*key).is_private_name()) {
        MessageTemplate message = Symbol::cast(*key).IsPrivateBrand() ? MessageTemplate::kInvalidPrivateBrandInstance : MessageTemplate::kInvalidPrivateMemberRead;
        THROW_NEW_ERROR(isolate, NewTypeError(message, key, lookup_start_object), Object);
    }
    return result;
}
...
MaybeHandle<Object> Runtime::HasProperty(Isolate* isolate, Handle<Object> object, Handle<Object> key)
{
    ...
    if (!object->IsJSReceiver()) {
        THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kInvalidInOperatorUse, key, object), Object);
    }
    Handle<JSReceiver> receiver = Handle<JSReceiver>::cast(object);

    // Convert the {key} to a name.
    Handle<Name> name;
    ASSIGN_RETURN_ON_EXCEPTION(isolate, name, Object::ToName(isolate, key), Object);
...
void GeneralizeAllTransitionsToFieldAsMutable(Isolate* isolate, Handle<Map> map, Handle<Name> name)
{
        ...
        transitions.ForEachTransitionTo(
            *name,
            [&](Map target) {
                ...
                if (details.kind() == PropertyKind::kData && details.constness() == PropertyConstness::kConst) {
                    target_maps[target_maps_count++] = handle(target, isolate);
                }
                DCHECK_IMPLIES(details.kind() == PropertyKind::kAccessor, details.constness() == PropertyConstness::kConst);
            },
            &no_gc);
    ...
    for (int i = 0; i < target_maps_count; i++) {
        ...
        PropertyDetails details = target->instance_descriptors(isolate).GetDetails(descriptor);
        Handle<FieldType> field_type(target->instance_descriptors(isolate).GetFieldType(descriptor), isolate);
        MapUpdater::GeneralizeField(isolate, target, descriptor, PropertyConstness::kMutable, details.representation(), field_type);
        DCHECK_EQ(PropertyConstness::kMutable, target->instance_descriptors(isolate).GetDetails(descriptor).constness());
    }
}

bool DeleteObjectPropertyFast(Isolate* isolate, Handle<JSReceiver> receiver, Handle<Object> raw_key)
{
    ...
    if (receiver_map->IsSpecialReceiverMap())
        return false;
    ...
    Handle<DescriptorArray> descriptors(receiver_map->instance_descriptors(isolate), isolate);
    ...
    if (parent_map->NumberOfOwnDescriptors() != nof - 1)
        return false;
    ...
    if (details.location() == PropertyLocation::kField) {
        ...
        // recorded slot.
        isolate->heap()->NotifyObjectLayoutChange(*receiver, no_gc, InvalidateRecordedSlots::kNo);
        FieldIndex index = FieldIndex::ForPropertyIndex(*receiver_map, details.field_index());
        ...
        if (!index.is_inobject() && index.outobject_array_index() == 0) {
            ...
            if (index.is_inobject()) {
                ...
                isolate->heap()->ClearRecordedSlot(*receiver, receiver->RawField(index.offset()));
            }
        }
    }
...
RUNTIME_FUNCTION(Runtime_ObjectGetOwnPropertyNames)
{
    ...
    Handle<FixedArray> keys;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, keys, KeyAccumulator::GetKeys(isolate, receiver, KeyCollectionMode::kOwnOnly, SKIP_SYMBOLS, GetKeysConversion::kConvertToString));
    return *keys;
}
...
RUNTIME_FUNCTION(Runtime_ObjectHasOwnProperty)
{
    ...
    if (object->IsJSModuleNamespace()) {
        ...
        Map map = js_obj->map();
        if (!map.IsJSGlobalProxyMap() && (key.is_element() && key.index() <= JSObject::kMaxElementIndex ? !map.has_indexed_interceptor() : !map.has_named_interceptor())) {
        ...
        LookupIterator it(isolate, object, key, Handle<JSProxy>::cast(object), LookupIterator::OWN);
        ...
        return isolate->heap()->ToBoolean(
            key.is_element() ? key.index() < static_cast<size_t>(String::cast(*object).length()) : key.name()->Equals(ReadOnlyRoots(isolate).length_string()));
...
RUNTIME_FUNCTION(Runtime_IsDictPropertyConstTrackingEnabled) { return isolate->heap()->ToBoolean(V8_DICT_PROPERTY_CONST_TRACKING_BOOL); }
...
RUNTIME_FUNCTION(Runtime_AddPrivateBrand)
{
    ...
    DCHECK_GE(depth, 0);
    for (; depth > 0; depth--) {
        context = handle(Context::cast(context->get(Context::PREVIOUS_INDEX)), isolate);
    }
    DCHECK_EQ(context->scope_info().scope_type(), ScopeType::CLASS_SCOPE);
    CHECK(Object::AddDataProperty(&it, context, attributes, Just(kDontThrow), StoreOrigin::kMaybeKeyed).FromJust());
    return *receiver;
}
...
RUNTIME_FUNCTION(Runtime_ObjectCreate)
{
    ...
    if (!prototype->IsNull(isolate) && !prototype->IsJSReceiver()) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kProtoObjectOrNull, prototype));
    }
...
MaybeHandle<Object> Runtime::SetObjectProperty(
    Isolate* isolate, Handle<Object> object, Handle<Object> key, Handle<Object> value, StoreOrigin store_origin, Maybe<ShouldThrow> should_throw)
{
    if (object->IsNullOrUndefined(isolate)) {
        MaybeHandle<String> maybe_property = Object::NoSideEffectsToMaybeString(isolate, key);
        Handle<String> property_name;
        if (maybe_property.ToHandle(&property_name)) {
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kNonObjectPropertyStoreWithProperty, object, property_name), Object);
        } else {
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kNonObjectPropertyStore, object), Object);
        }
    }
...
MaybeHandle<Object> Runtime::DefineObjectOwnProperty(Isolate* isolate, Handle<Object> object, Handle<Object> key, Handle<Object> value, StoreOrigin store_origin)
{
    if (object->IsNullOrUndefined(isolate)) {
        THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kNonObjectPropertyStore, key, object), Object);
    }
...
RUNTIME_FUNCTION(Runtime_ObjectIsExtensible)
{
    ...
    Maybe<bool> result = object->IsJSReceiver() ? JSReceiver::IsExtensible(Handle<JSReceiver>::cast(object)) : Just(false);
...
RUNTIME_FUNCTION(Runtime_JSReceiverPreventExtensionsThrow)
{
    ...
    MAYBE_RETURN(JSReceiver::PreventExtensions(Handle<JSReceiver>::cast(object), kThrowOnError), ReadOnlyRoots(isolate).exception());
    return *object;
}
...
RUNTIME_FUNCTION(Runtime_JSReceiverPreventExtensionsDontThrow)
{
    ...
    Maybe<bool> result = JSReceiver::PreventExtensions(Handle<JSReceiver>::cast(object), kDontThrow);
    MAYBE_RETURN(result, ReadOnlyRoots(isolate).exception());
...
RUNTIME_FUNCTION(Runtime_GetProperty)
{
    ...
    if (lookup_start_obj->IsJSObject()) {
        Handle<JSObject> lookup_start_object = Handle<JSObject>::cast(lookup_start_obj);
        if (!lookup_start_object->IsJSGlobalProxy() && !lookup_start_object->IsAccessCheckNeeded() && key_obj->IsName()) {
            ...
            if (lookup_start_object->IsJSGlobalObject()) {
                ...
                GlobalDictionary dictionary = JSGlobalObject::cast(*lookup_start_object).global_dictionary(kAcquireLoad);
                ...
                // Attempt dictionary lookup.
                if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
                    SwissNameDictionary dictionary = lookup_start_object->property_dictionary_swiss();
                    InternalIndex entry = dictionary.FindEntry(isolate, *key);
                    if (entry.is_found() && (dictionary.DetailsAt(entry).kind() == PropertyKind::kData)) {
                        return dictionary.ValueAt(entry);
                    }
                } else {
                    NameDictionary dictionary = lookup_start_object->property_dictionary();
                    InternalIndex entry = dictionary.FindEntry(isolate, key);
                    if ((entry.is_found()) && (dictionary.DetailsAt(entry).kind() == PropertyKind::kData)) {
                        return dictionary.ValueAt(entry);
                    }
...
RUNTIME_FUNCTION(Runtime_DefineKeyedOwnPropertyInLiteral_Simple)
{
    ...
    Maybe<bool> result = JSObject::DefineOwnPropertyIgnoreAttributes(&it, value, NONE, Just(kDontThrow));
...
// ES6 section 12.5.4.
Object DeleteProperty(Isolate* isolate, Handle<Object> object, Handle<Object> key, LanguageMode language_mode)
{
    Handle<JSReceiver> receiver;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, object));
...
RUNTIME_FUNCTION(Runtime_GetOwnPropertyKeys)
{
    ...
    Handle<FixedArray> keys;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, keys, KeyAccumulator::GetKeys(isolate, object, KeyCollectionMode::kOwnOnly, filter, GetKeysConversion::kConvertToString));
...
RUNTIME_FUNCTION(Runtime_ToFastProperties)
{
    ...
    if (object->IsJSObject() && !object->IsJSGlobalObject()) {
        JSObject::MigrateSlowToFast(Handle<JSObject>::cast(object), 0, "RuntimeToFastProperties");
    }
    return *object;
}
...
RUNTIME_FUNCTION(Runtime_GetDerivedMap)
{
    ...
    if (rab_gsab->IsTrue()) {
        RETURN_RESULT_OR_FAILURE(isolate, JSFunction::GetDerivedRabGsabMap(isolate, target, new_target));
    } else {
...
static bool IsValidAccessor(Isolate* isolate, Handle<Object> obj) { return obj->IsNullOrUndefined(isolate) || obj->IsCallable(); }
...
RUNTIME_FUNCTION(Runtime_DefineAccessorPropertyUnchecked)
{
    ...
    RETURN_FAILURE_ON_EXCEPTION(isolate, JSObject::DefineAccessor(obj, name, getter, setter, attrs));
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_DefineKeyedOwnPropertyInLiteral)
{
    ...
    if (!maybe_vector->IsUndefined()) {
        ...
        if (nexus.ic_state() == InlineCacheState::UNINITIALIZED) {
            if (name->IsUniqueName()) {
                nexus.ConfigureMonomorphic(name, handle(object->map(), isolate), MaybeObjectHandle());
            } else {
    ...
    PropertyAttributes attrs = (flags & DefineKeyedOwnPropertyInLiteralFlag::kDontEnum) ? PropertyAttributes::DONT_ENUM : PropertyAttributes::NONE;
    ...
    if (flags & DefineKeyedOwnPropertyInLiteralFlag::kSetFunctionName) {
        ...
        CHECK_IMPLIES(!IsClassConstructor(function->shared().kind()), *function_map == function->map());
    }
    ...
    CHECK(JSObject::DefineOwnPropertyIgnoreAttributes(&it, value, attrs, Just(kDontThrow)).IsJust());
...
RUNTIME_FUNCTION(Runtime_DefineGetterPropertyUnchecked)
{
    ...
    RETURN_FAILURE_ON_EXCEPTION(isolate, JSObject::DefineAccessor(object, name, getter, isolate->factory()->null_value(), attrs));
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_SetDataProperties)
{
    ...
    MAYBE_RETURN(JSReceiver::SetOrCopyDataProperties(isolate, target, source, PropertiesEnumerationMode::kEnumerationOrder), ReadOnlyRoots(isolate).exception());
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_CopyDataProperties)
{
    ...
    MAYBE_RETURN(
        JSReceiver::SetOrCopyDataProperties(isolate, target, source, PropertiesEnumerationMode::kPropertyAdditionOrder, nullptr, false), ReadOnlyRoots(isolate).exception());
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_CopyDataPropertiesWithExcludedPropertiesOnStack)
{
    ...
    base::ScopedVector<Handle<Object>> excluded_properties(excluded_property_count);
    ...
    MAYBE_RETURN(JSReceiver::SetOrCopyDataProperties(isolate, target, source, PropertiesEnumerationMode::kPropertyAdditionOrder, &excluded_properties, false),
...
RUNTIME_FUNCTION(Runtime_DefineSetterPropertyUnchecked)
{
    ...
    RETURN_FAILURE_ON_EXCEPTION(isolate, JSObject::DefineAccessor(object, name, isolate->factory()->null_value(), setter, attrs));
    return ReadOnlyRoots(isolate).undefined_value();
}
...
RUNTIME_FUNCTION(Runtime_CreateIterResultObject)
{
    ...
    return *isolate->factory()->NewJSIteratorResult(value, done->BooleanValue(isolate));
}
...
RUNTIME_FUNCTION(Runtime_CreateDataProperty)
{
    ...
    MAYBE_RETURN(JSReceiver::CreateDataProperty(&it, value, Just(kThrowOnError)), ReadOnlyRoots(isolate).exception());
    return *value;
}
