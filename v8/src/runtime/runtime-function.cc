...
RUNTIME_FUNCTION(Runtime_FunctionGetScriptSource)
{
    ...
    if (function->IsJSFunction()) {
        Handle<Object> script(Handle<JSFunction>::cast(function)->shared().script(), isolate);
        if (script->IsScript())
...
RUNTIME_FUNCTION(Runtime_FunctionGetSourceCode)
{
    ...
    Handle<JSReceiver> function = args.at<JSReceiver>(0);
    if (function->IsJSFunction()) {
        Handle<SharedFunctionInfo> shared(Handle<JSFunction>::cast(function)->shared(), isolate);
