...
V8_WARN_UNUSED_RESULT Object ReturnFuzzSafe(Object value, Isolate* isolate) { return v8_flags.correctness_fuzzer_suppressions ? ReadOnlyRoots(isolate).undefined_value() : value; }
...
bool IsAsmWasmFunction(Isolate* isolate, JSFunction function)
{
    ...
    return function.shared().HasAsmWasmData() || function.code().builtin_id() == Builtin::kInstantiateAsmJs;
#else
    return false;
#endif // V8_ENABLE_WEBASSEMBLY
}
...
RUNTIME_FUNCTION(Runtime_ConstructSlicedString)
{
    ...
    Handle<String> sliced_string = isolate->factory()->NewSubString(string, index, string->length());
...
RUNTIME_FUNCTION(Runtime_ConstructThinString)
{
    ...
    if (!string->IsConsString()) {
        const bool kIsOneByte = true;
        string = isolate->factory()->NewConsString(isolate->factory()->empty_string(), string, string->length(), kIsOneByte);
    }
...
template <CodeKind code_kind> bool CanOptimizeFunction(Handle<JSFunction> function, Isolate* isolate, IsCompiledScope* is_compiled_scope);

template <> bool CanOptimizeFunction<CodeKind::TURBOFAN>(Handle<JSFunction> function, Isolate* isolate, IsCompiledScope* is_compiled_scope)
{
    ...
    if (function->HasAvailableOptimizedCode() || function->HasAvailableCodeKind(kind)) {
        DCHECK(function->HasAttachedOptimizedCode() || function->ChecksTieringState());
        if (v8_flags.testing_d8_test_runner) {
...
#ifdef V8_ENABLE_MAGLEV
template <> bool CanOptimizeFunction<CodeKind::MAGLEV>(Handle<JSFunction> function, Isolate* isolate, IsCompiledScope* is_compiled_scope)
{
    if (!v8_flags.maglev)
        return false;
...
Object OptimizeFunctionOnNextCall(RuntimeArguments& args, Isolate* isolate)
{
    ...
    if (args.length() == 2) {
        ...
        if (!type->IsString())
            return CrashUnlessFuzzing(isolate);
        if (Handle<String>::cast(type)->IsOneByteEqualTo(base::StaticCharVector("concurrent")) && isolate->concurrent_recompilation_enabled()) {
...
bool EnsureFeedbackVector(Isolate* isolate, Handle<JSFunction> function)
{
    ...
    // If function isn't compiled, compile it now.
    IsCompiledScope is_compiled_scope(function->shared().is_compiled_scope(function->GetIsolate()));
    ...
    bool needs_compilation = !function->is_compiled() && !function->has_closure_feedback_cell_array();
    if (needs_compilation && !Compiler::Compile(isolate, function, Compiler::CLEAR_EXCEPTION, &is_compiled_scope)) {
        return false;
    }
...
RUNTIME_FUNCTION(Runtime_PrepareFunctionForOptimization)
{
    ...
    if (args.length() == 2) {
        ...
        if (sync->IsOneByteEqualTo(base::StaticCharVector("allow heuristic optimization"))) {
...
RUNTIME_FUNCTION(Runtime_OptimizeOsr)
{
    ...
    if (isolate->concurrent_recompilation_enabled() && v8_flags.concurrent_osr) {
        const BytecodeOffset osr_offset = OffsetOfNextJumpLoop(isolate, UnoptimizedFrame::cast(it.frame()));
        ...
        // Queue the job.
        auto unused_result = Compiler::CompileOptimizedOSR(isolate, function, osr_offset, ConcurrencyMode::kConcurrent);
        USE(unused_result);
...
RUNTIME_FUNCTION(Runtime_NeverOptimizeFunction)
{
    ...
    if (code_kind != CodeKind::INTERPRETED_FUNCTION && code_kind != CodeKind::BUILTIN) {
        return CrashUnlessFuzzing(isolate);
    }
...
static void ReturnNull(const v8::FunctionCallbackInfo<v8::Value>& args) { args.GetReturnValue().SetNull(); }
...
{
    double v1 = args[0]->NumberValue(args.GetIsolate()->GetCurrentContext()).ToChecked();
    double v2 = args[1]->NumberValue(args.GetIsolate()->GetCurrentContext()).ToChecked();
...
RUNTIME_FUNCTION(Runtime_GetCallable)
{
    ...
    Local<v8::Object> instance = t->GetFunction(v8_isolate->GetCurrentContext()).ToLocalChecked()->NewInstance(v8_isolate->GetCurrentContext()).ToLocalChecked();
...
int FixedArrayLenFromSize(int size) { return std::min({ (size - FixedArray::kHeaderSize) / kTaggedSize, FixedArray::kMaxRegularLength }); }
...
void FillUpOneNewSpacePage(Isolate* isolate, Heap* heap)
{
    ...
    DCHECK_IMPLIES(!space->IsInlineAllocationEnabled(), space->limit() == space->top());
    ...
    while (space_remaining > 0) {
        ...
        if (length > 0) {
            Handle<FixedArray> padding = isolate->factory()->NewFixedArray(length, AllocationType::kYoung);
            ...
            heap->CreateFillerObjectAt(*heap->new_space()->allocation_top_address(), space_remaining);
            break;
        }
    }
}
...
RUNTIME_FUNCTION(Runtime_DisassembleFunction)
{
    ...
    CHECK(func->is_compiled() || Compiler::Compile(isolate, func, Compiler::KEEP_EXCEPTION, &is_compiled_scope));
    StdoutStream os;
    func->code().Print(os);
    os << std::endl;
#endif // DEBUG
...
RUNTIME_FUNCTION(Runtime_PretenureAllocationSite)
{
    ...
    AllocationMemento memento = pretenuring_handler->FindAllocationMemento<PretenturingHandler::kForRuntime>(object.map(), object);
    if (memento.is_null())
        return ReturnFuzzSafe(ReadOnlyRoots(isolate).false_value(), isolate);
...
RUNTIME_FUNCTION(Runtime_EnableCodeLoggingForTesting)
{
    ...
    class NoopListener final : public LogEventListener {
        ...
        void RegExpCodeCreateEvent(Handle<AbstractCode> code, Handle<String> source) final { }
...
RUNTIME_FUNCTION(Runtime_NewRegExpWithBacktrackLimit)
{
    ...
    JSRegExp::Flags flags = JSRegExp::FlagsFromString(isolate, flags_string).value();
...
RUNTIME_FUNCTION(Runtime_IsSharedString)
{
    ...
    return isolate->heap()->ToBoolean(obj->IsString() && Handle<String>::cast(obj)->IsShared());
}
...
RUNTIME_FUNCTION(Runtime_IsInPlaceInternalizableString)
{
    ...
    DCHECK_EQ(1, args.length());
    Handle<HeapObject> obj = args.at<HeapObject>(0);
    return isolate->heap()->ToBoolean(obj->IsString() && String::IsInPlaceInternalizable(String::cast(*obj)));
}
...
RUNTIME_FUNCTION(Runtime_GetWeakCollectionSize)
{
    ...
    return Smi::FromInt(EphemeronHashTable::cast(collection->table()).NumberOfElements());
}
