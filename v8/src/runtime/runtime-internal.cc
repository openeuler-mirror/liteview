...
RUNTIME_FUNCTION(Runtime_ThrowTypeError) { THROW_ERROR(isolate, args, NewTypeError); }
...
RUNTIME_FUNCTION(Runtime_ThrowNoAccess)
{
    ...
    SaveAndSwitchContext save(isolate, impl->LastEnteredOrMicrotaskContext()->native_context());
...
RUNTIME_FUNCTION(Runtime_ThrowApplyNonFunction)
{
    ...
    THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kApplyNonFunction, object, type));
}
...
Object BytecodeBudgetInterruptWithStackCheck(Isolate* isolate, RuntimeArguments& args, CodeKind code_kind)
{
    HandleScope scope(isolate);
    DCHECK_EQ(1, args.length());
...
Object BytecodeBudgetInterrupt(Isolate* isolate, RuntimeArguments& args, CodeKind code_kind)
{
    HandleScope scope(isolate);
    DCHECK_EQ(1, args.length());
    Handle<JSFunction> function = args.at<JSFunction>(0);
...
RUNTIME_FUNCTION(Runtime_BytecodeBudgetInterruptWithStackCheck_Ignition) { return BytecodeBudgetInterruptWithStackCheck(isolate, args, CodeKind::INTERPRETED_FUNCTION); }

RUNTIME_FUNCTION(Runtime_BytecodeBudgetInterrupt_Ignition) { return BytecodeBudgetInterrupt(isolate, args, CodeKind::INTERPRETED_FUNCTION); }

RUNTIME_FUNCTION(Runtime_BytecodeBudgetInterruptWithStackCheck_Sparkplug) { return BytecodeBudgetInterruptWithStackCheck(isolate, args, CodeKind::BASELINE); }

RUNTIME_FUNCTION(Runtime_BytecodeBudgetInterrupt_Sparkplug) { return BytecodeBudgetInterrupt(isolate, args, CodeKind::BASELINE); }

RUNTIME_FUNCTION(Runtime_BytecodeBudgetInterruptWithStackCheck_Maglev) { return BytecodeBudgetInterruptWithStackCheck(isolate, args, CodeKind::MAGLEV); }
...
RUNTIME_FUNCTION(Runtime_AllocateInYoungGeneration)
{
    ...
    bool allow_large_object_allocation = AllowLargeObjectAllocationFlag::decode(flags);
...
RUNTIME_FUNCTION(Runtime_GetAndResetTurboProfilingData)
{
    ...
    Handle<String> result = isolate->factory()->NewStringFromAsciiChecked(stats_stream.str().c_str());
...
RUNTIME_FUNCTION(Runtime_CreateAsyncFromSyncIterator)
{
    ...
    return *isolate->factory()->NewJSAsyncFromSyncIterator(Handle<JSReceiver>::cast(sync_iterator), next);
}
...
RUNTIME_FUNCTION(Runtime_GetTemplateObject)
{
    ...
    Handle<TemplateObjectDescription> description = args.at<TemplateObjectDescription>(0);
...
RUNTIME_FUNCTION(Runtime_GetInitializerFunction)
{
    ...
    Handle<Object> initializer = JSReceiver::GetDataProperty(isolate, constructor, key);
    return *initializer;
}
