...
void ThrowRangeException(v8::Isolate* isolate, const char* message) { isolate->ThrowException(NewRangeException(isolate, message)); }
...
RUNTIME_FUNCTION(Runtime_WasmTraceExit)
{
    ...
    WasmFrame* frame = WasmFrame::cast(it.frame());
    int func_index = frame->function_index();
    const wasm::FunctionSig* sig = frame->wasm_instance().module()->functions[func_index].sig;
...
bool DisallowWasmCodegenFromStringsCallback(v8::Local<v8::Context> context, v8::Local<v8::String> source) { return false; }
...
RUNTIME_FUNCTION(Runtime_IsWasmCode)
{
    ...
    bool is_js_to_wasm = code.kind() == CodeKind::JS_TO_WASM_FUNCTION || (code.builtin_id() == Builtin::kGenericJSToWasmWrapper);
...
RUNTIME_FUNCTION(Runtime_SerializeWasmModule)
{
    ...
    Handle<JSArrayBuffer> array_buffer = isolate->factory()->NewJSArrayBufferAndBackingStore(byte_length, InitializedFlag::kUninitialized).ToHandleChecked();

    CHECK(wasm_serializer.SerializeNativeModule({ static_cast<uint8_t*>(array_buffer->backing_store()), byte_length }));
    return *array_buffer;
}
...
RUNTIME_FUNCTION(Runtime_DeserializeWasmModule)
{
    ...
    base::Vector<const uint8_t> wire_bytes_vec { reinterpret_cast<const uint8_t*>(wire_bytes_buffer->backing_store()) + wire_bytes->byte_offset(), wire_bytes->byte_length() };
...
RUNTIME_FUNCTION(Runtime_WasmGetNumberOfInstances)
{
    ...
    WeakArrayList weak_instance_list = module_obj->script().wasm_weak_instance_list();
...
RUNTIME_FUNCTION(Runtime_WasmNumCodeSpaces)
{
    ...
    if (argument->IsWasmInstanceObject()) {
        module = handle(Handle<WasmInstanceObject>::cast(argument)->module_object(), isolate);
    } else if (argument->IsWasmModuleObject()) {
        module = Handle<WasmModuleObject>::cast(argument);
    }
    size_t num_spaces = module->native_module()->GetNumberOfCodeSpacesForTesting();
...
RUNTIME_FUNCTION(Runtime_WasmTraceMemory)
{
    ...
    uint8_t* mem_start = reinterpret_cast<uint8_t*>(frame->wasm_instance().memory_object().array_buffer().backing_store());
    int func_index = frame->function_index();
    int pos = frame->position();
    wasm::ExecutionTier tier = frame->wasm_code()->is_liftoff() ? wasm::ExecutionTier::kLiftoff : wasm::ExecutionTier::kTurbofan;
