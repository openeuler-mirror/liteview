...
RUNTIME_FUNCTION(Runtime_ScheduleBreak)
{
    SealHandleScope shs(isolate);
    DCHECK_EQ(0, args.length());
    isolate->RequestInterrupt([](v8::Isolate* isolate, void*) { v8::debug::BreakRightNow(isolate, v8::debug::BreakReasons({ v8::debug::BreakReason::kScheduled })); }, nullptr);
    return ReadOnlyRoots(isolate).undefined_value();
}
...
template <class IteratorType> static Handle<ArrayList> AddIteratorInternalProperties(Isolate* isolate, Handle<ArrayList> result, Handle<IteratorType> iterator)
{
    ...
    result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[IteratorHasMore]]"), isolate->factory()->ToBoolean(iterator->HasMore()));
    ...
    result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[IteratorKind]]"), isolate->factory()->NewStringFromAsciiChecked(kind));
    return result;
}
...
MaybeHandle<JSArray> Runtime::GetInternalProperties(Isolate* isolate, Handle<Object> object)
{
    ...
    if (object->IsJSObject()) {
        PrototypeIterator iter(isolate, Handle<JSObject>::cast(object), kStartAtReceiver);
        if (iter.HasAccess()) {
            iter.Advance();
    ...
    if (object->IsJSBoundFunction()) {
        ...
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[TargetFunction]]"), handle(function->bound_target_function(), isolate));
        ...
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[GeneratorState]]"), isolate->factory()->NewStringFromAsciiChecked(status));
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[GeneratorFunction]]"), handle(generator->function(), isolate));
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[GeneratorReceiver]]"), handle(generator->receiver(), isolate));
        ...
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[IsRevoked]]"), isolate->factory()->ToBoolean(js_proxy->IsRevoked()));
...
#define TYPED_ARRAY_CASE(Type, type, TYPE, ctype)                                                                                                                                  \
    case kExternal##Type##Array: {                                                                                                                                                 \
        if ((byte_length % sizeof(ctype)) != 0)                                                                                                                                    \
            continue;                                                                                                                                                              \
        result = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromStaticChars("[[" #Type "Array]]"),                                                               \
            isolate->factory()->NewJSTypedArray(kExternal##Type##Array, js_array_buffer, 0, byte_length / sizeof(ctype)));                                                         \
        break;                                                                                                                                                                     \
    }
...
#undef TYPED_ARRAY_CASE
            ...
            result
                = ArrayList::Add(isolate, result, isolate->factory()->NewStringFromAsciiChecked("[[ArrayBufferByteLength]]"), isolate->factory()->NewNumberFromSize(byte_length));

            auto backing_store = js_array_buffer->GetBackingStore();
            Handle<Object> array_buffer_data = backing_store ? isolate->factory()->NewNumberFromUint(backing_store->id()) : isolate->factory()->null_value();
            ...
            Handle<Object> memory_object = JSObject::GetDataProperty(isolate, js_array_buffer, memory_symbol);
        ...
        result = AddWasmInstanceObjectInternalProperties(isolate, result, Handle<WasmInstanceObject>::cast(object));
    } else if (object->IsWasmModuleObject()) {
        result = AddWasmModuleObjectInternalProperties(isolate, result, Handle<WasmModuleObject>::cast(object));
    } else if (object->IsWasmTableObject()) {
        result = AddWasmTableObjectInternalProperties(isolate, result, Handle<WasmTableObject>::cast(object));
#endif // V8_ENABLE_WEBASSEMBLY
    }
    return isolate->factory()->NewJSArrayWithElements(ArrayList::Elements(isolate, result), PACKED_ELEMENTS);
}
...
RUNTIME_FUNCTION(Runtime_GetBreakLocations)
{
    ...
    return *isolate->factory()->NewJSArrayWithElements(Handle<FixedArray>::cast(break_locations));
}
...
RUNTIME_FUNCTION(Runtime_CollectGarbage)
{
    SealHandleScope shs(isolate);
    DCHECK_EQ(1, args.length());
    isolate->heap()->PreciseCollectAllGarbage(Heap::kNoGCFlags, GarbageCollectionReason::kRuntime);
    return ReadOnlyRoots(isolate).undefined_value();
}
...
Handle<Object> GetJSPositionInfo(Handle<Script> script, int position, Script::OffsetFlag offset_flag, Isolate* isolate)
{
    Script::PositionInfo info;
    ...
    Handle<String> sourceText
        = is_wasm_script ? isolate->factory()->empty_string() : isolate->factory()->NewSubString(handle(String::cast(script->source()), isolate), info.line_start, info.line_end);
...
Handle<Object> ScriptLocationFromLine(Isolate* isolate, Handle<Script> script, Handle<Object> opt_line, Handle<Object> opt_column, int32_t offset)
{
    ...
    return GetJSPositionInfo(script, line_position + column, Script::NO_OFFSET, isolate);
}
...
RUNTIME_FUNCTION(Runtime_DebugAsyncFunctionSuspended)
{
    ...
    if (isolate->debug()->is_active()) {
        Object::SetProperty(isolate, reject_handler, isolate->factory()->promise_forwarding_handler_symbol(), isolate->factory()->true_value(), StoreOrigin::kMaybeKeyed,
        ...
        Object::SetProperty(isolate, throwaway, isolate->factory()->promise_handled_by_symbol(), outer_promise, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)).Check();

        Object::SetProperty(isolate, promise, isolate->factory()->promise_awaited_by_symbol(), generator, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)).Check();
    }
...
RUNTIME_FUNCTION(Runtime_ProfileCreateSnapshotDataBlob)
{
    ...
    v8::StartupData blob = CreateSnapshotDataBlobInternal(v8::SnapshotCreator::FunctionCodeHandling::kClear, nullptr);
    delete[] blob.data;
