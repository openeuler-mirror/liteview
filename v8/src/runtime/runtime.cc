...
struct IntrinsicFunctionIdentifier {
    ...
    uint32_t Hash() { return StringHasher::HashSequentialString<uint8_t>(data_, length_, v8::internal::kZeroHashSeed); }
...
void InitializeIntrinsicFunctionNames()
{
    ...
    for (size_t i = 0; i < arraysize(kIntrinsicFunctions); ++i) {
        ...
        IntrinsicFunctionIdentifier* identifier = new IntrinsicFunctionIdentifier(reinterpret_cast<const unsigned char*>(function->name), static_cast<int>(strlen(function->name)));
...
bool Runtime::IsAllowListedForFuzzing(FunctionId id)
{
    ...
    case Runtime::kVerifyType:
        return !v8_flags.allow_natives_for_differential_fuzzing && !v8_flags.concurrent_recompilation;
...
std::ostream& operator<<(std::ostream& os, Runtime::FunctionId id) { return os << Runtime::FunctionForId(id)->name; }
