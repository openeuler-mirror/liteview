...
Object ThrowRedeclarationError(Isolate* isolate, Handle<String> name, RedeclarationType redeclaration_type)
{
    HandleScope scope(isolate);
...
Object DeclareGlobal(
    Isolate* isolate, Handle<JSGlobalObject> global, Handle<String> name, Handle<Object> value, PropertyAttributes attr, bool is_var, RedeclarationType redeclaration_type)
{
...
RUNTIME_FUNCTION(Runtime_DeclareModuleExports)
{
    ...
    if (closure->has_feedback_vector()) {
        closure_feedback_cell_array = Handle<ClosureFeedbackCellArray>(closure->feedback_vector().closure_feedback_cell_array(), isolate);
    } else {
        closure_feedback_cell_array = Handle<ClosureFeedbackCellArray>(closure->closure_feedback_cell_array(), isolate);
    }

    Handle<Context> context(isolate->context(), isolate);
    DCHECK(context->IsModuleContext());
    Handle<FixedArray> exports(SourceTextModule::cast(context->extension()).regular_exports(), isolate);
    ...
    FOR_WITH_HANDLE_SCOPE(isolate, int, i = 0, i, i < length, i++, {
        ...
        if (decl.IsSmi()) {
            ...
            Handle<SharedFunctionInfo> sfi(SharedFunctionInfo::cast(declarations->get(i)), isolate);
            int feedback_index = Smi::ToInt(declarations->get(++i));
            index = Smi::ToInt(declarations->get(++i));
            Handle<FeedbackCell> feedback_cell = closure_feedback_cell_array->GetFeedbackCell(feedback_index);
...
RUNTIME_FUNCTION(Runtime_DeclareGlobals)
{
    ...
    FOR_WITH_HANDLE_SCOPE(isolate, int, i = 0, i, i < length, i++, {
        ...
        if (is_var) {
            ...
            Handle<FeedbackCell> feedback_cell = closure_feedback_cell_array->GetFeedbackCell(index);
            value = Factory::JSFunctionBuilder(isolate, sfi, context).set_feedback_cell(feedback_cell).Build();
        }
        ...
        PropertyAttributes attr = script.compilation_type() == Script::COMPILATION_TYPE_EVAL ? NONE : DONT_DELETE;

        // ES#sec-globaldeclarationinstantiation 5.d:
        // If hasRestrictedGlobal is true, throw a SyntaxError exception.
        Object result = DeclareGlobal(isolate, global, name, value, attr, is_var, RedeclarationType::kSyntaxError);
...
Object DeclareEvalHelper(Isolate* isolate, Handle<String> name, Handle<Object> value)
{
    ...
    DCHECK(context->IsFunctionContext() || context->IsNativeContext() || context->IsScriptContext() || context->IsEvalContext()
    ...
    if (attributes != ABSENT && holder->IsJSGlobalObject()) {
        ...
        // If fnDefinable is false, throw a TypeError exception.
        return DeclareGlobal(isolate, Handle<JSGlobalObject>::cast(holder), name, value, NONE, is_var, RedeclarationType::kTypeError);
    }
    if (context->has_extension() && context->extension().IsJSGlobalObject()) {
        Handle<JSGlobalObject> global(JSGlobalObject::cast(context->extension()), isolate);
        ...
        Handle<JSGlobalObject> global(JSGlobalObject::cast(context->global_object()), isolate);
        return DeclareGlobal(isolate, global, name, value, NONE, is_var, RedeclarationType::kTypeError);
    }
...
std::unique_ptr<Handle<Object>[]> GetCallerArguments(Isolate* isolate, int* total_argc)
{
    ...
    if (functions.size() > 1) {
        ...
        *total_argc = argument_count;
        std::unique_ptr<Handle<Object>[]> param_data(NewArray<Handle<Object>>(*total_argc));
...
template <typename T> Handle<JSObject> NewSloppyArguments(Isolate* isolate, Handle<JSFunction> callee, T parameters, int argument_count)
{
    ...
    if (argument_count > 0) {
        if (parameter_count > 0) {
            ...
            // parameter map.
            Handle<Context> context(isolate->context(), isolate);
            Handle<FixedArray> arguments = isolate->factory()->NewFixedArray(argument_count, AllocationType::kYoung);

            Handle<SloppyArgumentsElements> parameter_map = isolate->factory()->NewSloppyArgumentsElements(mapped_count, context, arguments, AllocationType::kYoung);
            ...
            // special in any way.
            Handle<FixedArray> elements = isolate->factory()->NewFixedArray(argument_count, AllocationType::kYoung);
...
public:
    ...
    Object operator[](int index) { return *FullObjectSlot(parameters_ - (index + 1) * kSystemPointerSize); }
...
RUNTIME_FUNCTION(Runtime_NewStrictArguments)
{
    ...
    Handle<JSObject> result = isolate->factory()->NewArgumentsObject(callee, argument_count);
    if (argument_count) {
        Handle<FixedArray> array = isolate->factory()->NewFixedArray(argument_count);
...
RUNTIME_FUNCTION(Runtime_NewRestParameter)
{
    ...
    Handle<JSObject> result = isolate->factory()->NewJSArray(PACKED_ELEMENTS, num_elements, num_elements, DONT_INITIALIZE_ARRAY_ELEMENTS);
    {
        DisallowGarbageCollection no_gc;
...
MaybeHandle<Object> LoadLookupSlot(Isolate* isolate, Handle<String> name, ShouldThrow should_throw, Handle<Object>* receiver_return = nullptr)
{
    ...
    if (!holder.is_null() && holder->IsSourceTextModule()) {
        ...
        return SourceTextModule::LoadVariable(isolate, Handle<SourceTextModule>::cast(holder), index);
    }
    if (index != Context::kNotFound) {
    ...
    if (!holder.is_null()) {
        ...
        Handle<Object> value;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, value, Object::GetProperty(isolate, holder, name), Object);
        if (receiver_return) {
            *receiver_return = (holder->IsJSGlobalObject() || holder->IsJSContextExtensionObject()) ? Handle<Object>::cast(isolate->factory()->undefined_value()) : holder;
        }
        return value;
    }
...
RUNTIME_FUNCTION_RETURN_PAIR(Runtime_LoadLookupSlotForCall)
{
    ...
    ASSIGN_RETURN_ON_EXCEPTION_VALUE(isolate, value, LoadLookupSlot(isolate, name, kThrowOnError, &receiver), MakePair(ReadOnlyRoots(isolate).exception(), Object()));
...
MaybeHandle<Object> StoreLookupSlot(
    Isolate* isolate, Handle<Context> context, Handle<String> name, Handle<Object> value, LanguageMode language_mode, ContextLookupFlags context_lookup_flags = FOLLOW_CHAINS)
{
    int index;
    PropertyAttributes attributes;
    InitializationFlag flag;
    ...
    if (holder.is_null()) {
        ...
        if ((attributes & READ_ONLY) == 0) {
            SourceTextModule::StoreVariable(Handle<SourceTextModule>::cast(holder), index, value);
        } else {
            THROW_NEW_ERROR(isolate, NewTypeError(MessageTemplate::kConstAssign, name), Object);
        }
        return value;
    }
    ...
    ASSIGN_RETURN_ON_EXCEPTION(isolate, value, Object::SetProperty(isolate, object, name, value), Object);
    return value;
}
