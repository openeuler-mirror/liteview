...
WasmInstanceObject GetWasmInstanceOnStackTop(Isolate* isolate, std::initializer_list<StackFrame::Type> skipped_frame_types = { StackFrame::EXIT })
{
    return FrameFinder<WasmFrame>(isolate, skipped_frame_types).frame()->wasm_instance();
}

Context GetNativeContextFromWasmInstanceOnStackTop(Isolate* isolate) { return GetWasmInstanceOnStackTop(isolate).native_context(); }
...
Object ThrowWasmError(Isolate* isolate, MessageTemplate message, Handle<Object> arg0 = Handle<Object>())
{
    Handle<JSObject> error_obj = isolate->factory()->NewWasmRuntimeError(message, arg0);
...
RUNTIME_FUNCTION(Runtime_WasmJSToWasmObject)
{
    ...
    const wasm::WasmModule* module = raw_instance.IsWasmInstanceObject() ? WasmInstanceObject::cast(raw_instance).module() : nullptr;

    wasm::ValueType type = wasm::ValueType::FromRawBitField(raw_type);
    const char* error_message;

    Handle<Object> result;
    bool success = internal::wasm::JSToWasmObject(isolate, module, value, type, &error_message).ToHandle(&result);
    if (success)
        return *result;
...
RUNTIME_FUNCTION(Runtime_WasmMemoryGrow)
{
    ...
    int ret = WasmMemoryObject::Grow(isolate, handle(instance.memory_object(), isolate), delta_pages);
...
RUNTIME_FUNCTION(Runtime_WasmThrow)
{
    ...
    Handle<WasmExceptionPackage> exception = WasmExceptionPackage::New(isolate, tag, values);
...
RUNTIME_FUNCTION(Runtime_WasmCompileLazy)
{
    ...
    HandleScope scope(isolate);
    DCHECK_EQ(3, args.length());
    Handle<WasmInstanceObject> instance(WasmInstanceObject::cast(args[0]), isolate);
    ...
    *native_module_stack_slot = instance->module_object().native_module();
    ...
    if (!success) {
        wasm::ThrowLazyCompilationError(isolate, instance->module_object().native_module(), func_index);
        DCHECK(isolate->has_pending_exception());
...
namespace {
void ReplaceWrapper(Isolate* isolate, Handle<WasmInstanceObject> instance, int function_index, Handle<CodeT> wrapper_code)
{
    Handle<WasmInternalFunction> internal = WasmInstanceObject::GetWasmInternalFunction(isolate, instance, function_index).ToHandleChecked();
    Handle<WasmExternalFunction> exported_function = handle(WasmExternalFunction::cast(internal->external()), isolate);
    exported_function->set_code(*wrapper_code, kReleaseStore);
    WasmExportedFunctionData function_data = exported_function->shared().wasm_exported_function_data();
...
RUNTIME_FUNCTION(Runtime_WasmCompileWrapper)
{
    ...
    DCHECK_EQ(2, args.length());
    Handle<WasmInstanceObject> instance(WasmInstanceObject::cast(args[0]), isolate);
    Handle<WasmExportedFunctionData> function_data(WasmExportedFunctionData::cast(args[1]), isolate);
    DCHECK(isolate->context().is_null());
    isolate->set_context(instance->native_context());
    ...
    const uint32_t canonical_sig_index = module->isorecursive_canonical_type_ids[function.sig_index];
    ...
    if (WasmInstanceObject::GetWasmInternalFunction(isolate, instance, function_index).is_null()) {
        DCHECK_EQ(function_index, module->start_function_index);
        return ReadOnlyRoots(isolate).undefined_value();
    }

    Handle<CodeT> wrapper_code = wasm::JSToWasmWrapperCompilationUnit::CompileSpecificJSToWasmWrapper(isolate, sig, canonical_sig_index, module);
...
RUNTIME_FUNCTION(Runtime_WasmAtomicNotify)
{
    ...
    Handle<JSArrayBuffer> array_buffer { instance.memory_object().array_buffer(), isolate };
...
namespace {
Object ThrowTableOutOfBounds(Isolate* isolate, Handle<WasmInstanceObject> instance)
{
...
RUNTIME_FUNCTION(Runtime_WasmRefFunc)
{
    ...
    return *WasmInstanceObject::GetOrCreateWasmInternalFunction(isolate, instance, function_index);
}
...
RUNTIME_FUNCTION(Runtime_WasmFunctionTableGet)
{
    ...
    auto table = handle(WasmTableObject::cast(instance.tables().get(table_index)), isolate);
    // We only use the runtime call for lazily initialized function references.
    DCHECK(table->instance().IsUndefined() ? table->type() == wasm::kWasmFuncRef
                                           : IsSubtypeOf(table->type(), wasm::kWasmFuncRef, WasmInstanceObject::cast(table->instance()).module()));
...
RUNTIME_FUNCTION(Runtime_WasmTableInit)
{
    ...
    base::Optional<MessageTemplate> opt_error = WasmInstanceObject::InitTableEntries(isolate, instance, table_index, elem_segment_index, dst, src, count);
...
RUNTIME_FUNCTION(Runtime_WasmTableCopy)
{
    ...
    bool oob = !WasmInstanceObject::CopyTableEntries(isolate, instance, table_dst_index, table_src_index, dst, src, count);
    if (oob)
...
RUNTIME_FUNCTION(Runtime_WasmTableGrow)
{
    ...
    Handle<WasmTableObject> table(WasmTableObject::cast(instance.tables().get(table_index)), isolate);
...
bool ExecuteWasmDebugBreaks(Isolate* isolate, Handle<WasmInstanceObject> instance, WasmFrame* frame)
{
    ...
    if (script->break_on_entry()) {
        ...
        for (int i = 0; i < weak_instance_list.length(); ++i) {
            if (weak_instance_list.Get(i)->IsCleared())
                continue;
            i::WasmInstanceObject::cast(weak_instance_list.Get(i)->GetHeapObject()).set_break_on_entry(false);
        }
...
RUNTIME_FUNCTION(Runtime_WasmArrayNewSegment)
{
    ...
    if (type->element_type().is_numeric()) {
        ...
        if (!base::IsInBounds<uint32_t>(offset, length_in_bytes, instance->data_segment_sizes().get(segment_index))) {
        ...
        const wasm::WasmElemSegment* elem_segment = &instance->module()->elem_segments[segment_index];
        if (!base::IsInBounds<size_t>(offset, length, instance->dropped_elem_segments().get(segment_index) ? 0 : elem_segment->entries.size())) {
            return ThrowWasmError(isolate, MessageTemplate::kWasmTrapElementSegmentOutOfBounds);
        }

        Handle<Object> result = isolate->factory()->NewWasmArrayFromElementSegment(instance, elem_segment, offset, length, rtt);
        if (result->IsSmi()) {
            return ThrowWasmError(isolate, static_cast<MessageTemplate>(result->ToSmi().value()));
        } else {
            return *result;
        }
    }
...
{
    DisallowGarbageCollection no_gc;
    auto continuation = WasmContinuationObject::cast(isolate->root(RootIndex::kActiveContinuation));
...
RUNTIME_FUNCTION(Runtime_WasmAllocateSuspender)
{
    ...
    auto parent = handle(WasmContinuationObject::cast(isolate->root(RootIndex::kActiveContinuation)), isolate);
    Handle<WasmContinuationObject> target = WasmContinuationObject::New(isolate, wasm::JumpBuffer::Inactive, parent);
    auto target_stack = Managed<wasm::StackMemory>::cast(target->stack()).get().get();
    ...
    wasm::JumpBuffer* jmpbuf = reinterpret_cast<wasm::JumpBuffer*>(parent->ReadExternalPointerField<kWasmContinuationJmpbufTag>(WasmContinuationObject::kJmpbufOffset, isolate));
...
RUNTIME_FUNCTION(Runtime_WasmCreateResumePromise)
{
    ...
    i::Handle<i::Object> argv[] = { handle(suspender.resume(), isolate), handle(suspender.reject(), isolate) };
    i::Handle<i::Object> result;
    bool has_pending_exception = !i::Execution::CallBuiltin(isolate, isolate->promise_then(), promise, arraysize(argv), argv).ToHandle(&result);
...
#define RETURN_RESULT_OR_TRAP(call)                                                                                                                                                \
    do {                                                                                                                                                                           \
        ...
        if (!(call).ToHandle(&result)) {                                                                                                                                           \
            ...
            Handle<JSObject> exception(JSObject::cast(isolate->pending_exception()), isolate);                                                                                     \
...
RUNTIME_FUNCTION(Runtime_WasmStringNewWtf8)
{
    ...
    const base::Vector<const uint8_t> bytes { instance.memory_start() + offset, size };
    RETURN_RESULT_OR_TRAP(isolate->factory()->NewStringFromUtf8(bytes, utf8_variant));
}
...
RUNTIME_FUNCTION(Runtime_WasmStringNewWtf8Array)
{
    ...
    RETURN_RESULT_OR_TRAP(isolate->factory()->NewStringFromUtf8(array, start, end, utf8_variant));
}
...
RUNTIME_FUNCTION(Runtime_WasmStringConst)
{
    ...
    const wasm::WasmStringRefLiteral& literal = instance.module()->stringref_literals[index];
    const base::Vector<const uint8_t> module_bytes = instance.module_object().native_module()->wire_bytes();
    const base::Vector<const uint8_t> string_bytes = module_bytes.SubVector(literal.source.offset(), literal.source.offset() + literal.source.length());
...
bool HasUnpairedSurrogate(base::Vector<const base::uc16> wtf16) { return unibrow::Utf16::HasUnpairedSurrogate(wtf16.begin(), wtf16.size()); }
// TODO(12868): Consider unifying with api.cc:String::WriteUtf8.
template <typename T>
int EncodeWtf8(base::Vector<char> bytes, size_t offset, base::Vector<const T> wtf16, unibrow::Utf8Variant variant, MessageTemplate* message, MessageTemplate out_of_bounds)
{
...
Object EncodeWtf8(Isolate* isolate, unibrow::Utf8Variant variant, Handle<String> string, GetWritableBytes get_writable_bytes, size_t offset, MessageTemplate out_of_bounds_message)
{
    string = String::Flatten(isolate, string);
...
RUNTIME_FUNCTION(Runtime_WasmStringEncodeWtf8)
{
    ...
    auto utf8_variant = static_cast<unibrow::Utf8Variant>(utf8_variant_value);
    auto get_writable_bytes = [&](const DisallowGarbageCollection&) -> base::Vector<char> { return { memory_start, instance.memory_size() }; };
...
RUNTIME_FUNCTION(Runtime_WasmStringEncodeWtf8Array)
{
    ...
    DCHECK(utf8_variant_value <= static_cast<uint32_t>(unibrow::Utf8Variant::kLastUtf8Variant));
    auto utf8_variant = static_cast<unibrow::Utf8Variant>(utf8_variant_value);
    auto get_writable_bytes = [&](const DisallowGarbageCollection&) -> base::Vector<char> { return { reinterpret_cast<char*>(array->ElementAddress(0)), array->length() }; };
...
RUNTIME_FUNCTION(Runtime_WasmStringAsWtf8)
{
    ...
    auto get_writable_bytes = [&](const DisallowGarbageCollection&) -> base::Vector<char> {
        return { reinterpret_cast<char*>(array->GetDataStartAddress()), static_cast<size_t>(wtf8_length) };
    };
...
RUNTIME_FUNCTION(Runtime_WasmStringViewWtf8Encode)
{
    ...
    const byte* src = reinterpret_cast<const byte*>(array->GetDataStartAddress() + start);
