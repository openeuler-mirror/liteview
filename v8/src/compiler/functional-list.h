...
public:
    ...
    bool operator!=(const FunctionalList<A>& other) const { return !(*this == other); }

    bool TriviallyEquals(const FunctionalList<A>& other) const { return elements_ == other.elements_; }
    ...
    void PushFront(A a, Zone* zone) { elements_ = zone->New<Cons>(std::move(a), elements_); }
