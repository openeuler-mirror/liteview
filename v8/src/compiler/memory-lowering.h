...
public:
    ...
    public:
        ...
        static AllocationState const* Empty(Zone* zone) { return zone->New<AllocationState>(); }
        static AllocationState const* Closed(AllocationGroup* group, Node* effect, Zone* zone) { return zone->New<AllocationState>(group, effect); }
    ...
    MemoryLowering(
        JSGraph* jsgraph, Zone* zone, JSGraphAssembler* graph_assembler, AllocationFolding allocation_folding = AllocationFolding::kDontAllocationFolding,
    ...
    Reduction ReduceAllocateRaw(Node* node, AllocationType allocation_type, AllowLargeObjects allow_large_objects, AllocationState const** state);
