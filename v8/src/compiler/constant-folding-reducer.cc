...
bool IsAlreadyBeingFolded(Node* node)
{
    ...
    for (Edge edge : node->use_edges()) {
        if (NodeProperties::IsValueEdge(edge) && edge.from()->opcode() == IrOpcode::kFoldConstant) {
...
{
    if (!NodeProperties::IsConstant(node) && NodeProperties::IsTyped(node) && node->op()->HasProperty(Operator::kEliminatable) && node->opcode() != IrOpcode::kFinishRegion) {
        ...
        if (constant != nullptr) {
            ...
            if (!v8_flags.assert_types) {
                ...
                Node* fold_constant = jsgraph()->graph()->NewNode(jsgraph()->common()->FoldConstant(), node, constant);
