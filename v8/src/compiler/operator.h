...
#define OPERATOR_PROPERTY_LIST(V)                                                                                                                                                  \
    ...
    // Constructor.
    Operator(
        Opcode opcode, Properties properties, const char* mnemonic, size_t value_in, size_t effect_in, size_t control_in, size_t value_out, size_t effect_out, size_t control_out);
    ...
    // can be merged.
    virtual bool Equals(const Operator* that) const { return this->opcode() == that->opcode(); }
    ...
    bool HasProperty(Property property) const { return (properties() & property) == property; }
    ...
    static size_t ZeroIfEliminatable(Properties properties) { return (properties & kEliminatable) == kEliminatable ? 0 : 1; }

    static size_t ZeroIfNoThrow(Properties properties) { return (properties & kNoThrow) == kNoThrow ? 0 : 2; }

    static size_t ZeroIfPure(Properties properties) { return (properties & kPure) == kPure ? 0 : 1; }
...
public:
    Operator1(Opcode opcode, Properties properties, const char* mnemonic, size_t value_in, size_t effect_in, size_t control_in, size_t value_out, size_t effect_out,
        size_t control_out, T parameter, Pred const& pred = Pred(), Hash const& hash = Hash())
    ...
    size_t HashCode() const final { return base::hash_combine(this->opcode(), this->hash_(this->parameter())); }
    ...
    virtual void PrintParameter(std::ostream& os, PrintVerbosity verbose) const { os << "[" << parameter() << "]"; }
...
template <typename T> inline T const& OpParameter(const Operator* op) { return reinterpret_cast<const Operator1<T, OpEqualTo<T>, OpHash<T>>*>(op)->parameter(); }
...
template <class T> struct OpEqualTo<Handle<T>> : public Handle<T>::equal_to { };
template <class T> struct OpHash<Handle<T>> : public Handle<T>::hash { };
