...
private:
    ...
    int num_nodes() { return static_cast<int>(loop_tree_->node_to_loop_num_.size()); }
    ...
    bool IsLoopHeaderNode(Node* node) { return node->opcode() == IrOpcode::kLoop || NodeProperties::IsPhi(node); }

    bool IsLoopExitNode(Node* node) { return node->opcode() == IrOpcode::kLoopExit || node->opcode() == IrOpcode::kLoopExitValue || node->opcode() == IrOpcode::kLoopExitEffect; }
...
#if V8_ENABLE_WEBASSEMBLY
// static
ZoneUnorderedSet<Node*>* LoopFinder::FindSmallInnermostLoopFromHeader(Node* loop_header, AllNodes& all_nodes, Zone* zone, size_t max_size, bool calls_are_large)
{
...
#define ENQUEUE_USES(use_name, condition)                                                                                                                                          \
    ...
    while (!queue.empty()) {
        ...
        case IrOpcode::kCall: {
            ...
            intptr_t info = OpParameter<RelocatablePtrConstantInfo>(callee->op()).value();
            using WasmCode = v8::internal::wasm::WasmCode;
            constexpr intptr_t unrollable_builtins[] = { // Exists in every stack check.
                WasmCode::kWasmStackGuard,
                // Fast table operations.
                WasmCode::kWasmTableGet, WasmCode::kWasmTableSet, WasmCode::kWasmTableGetFuncRef, WasmCode::kWasmTableSetFuncRef, WasmCode::kWasmTableGrow,
                // Atomics.
                WasmCode::kWasmAtomicNotify, WasmCode::kWasmI32AtomicWait32, WasmCode::kWasmI32AtomicWait64, WasmCode::kWasmI64AtomicWait32, WasmCode::kWasmI64AtomicWait64,
                // Exceptions.
                WasmCode::kWasmAllocateFixedArray, WasmCode::kWasmThrow, WasmCode::kWasmRethrow, WasmCode::kWasmRethrowExplicitContext,
                // Fast wasm-gc operations.
                WasmCode::kWasmRefFunc
            };
            if (std::count(unrollable_builtins, unrollable_builtins + arraysize(unrollable_builtins), info) == 0) {
                return nullptr;
            }
            ENQUEUE_USES(use, true)
            break;
