...
public:
    ...
    Node* target() const { return call_->InputAt(JSCallOrConstructNode::TargetIndex()); }

    Node* receiver() const { return JSCallNode { call_ }.receiver(); }

    Node* new_target() const { return JSConstructNode { call_ }.new_target(); }

    FrameState frame_state() const { return FrameState { NodeProperties::GetFrameStateInput(call_) }; }

    int argument_count() const { return (call_->opcode() == IrOpcode::kJSCall) ? JSCallNode { call_ }.ArgumentCount() : JSConstructNode { call_ }.ArgumentCount(); }
    ...
    Node* call, Node* new_target, Node* context, Node* frame_state, StartNode start, Node* end, Node* exception_target, const NodeVector& uncaught_subcalls)
{
    ...
    return InlineCall(call, new_target, context, frame_state, start, end, exception_target, uncaught_subcalls, static_cast<int>(n.Parameters().signature()->parameter_count()));
}
#endif // V8_ENABLE_WEBASSEMBLY

Reduction JSInliner::InlineCall(
    Node* call, Node* new_target, Node* context, Node* frame_state, StartNode start, Node* end, Node* exception_target, const NodeVector& uncaught_subcalls, int argument_count)
{
...
FrameState JSInliner::CreateArtificialFrameState(
    Node* node, FrameState outer_frame_state, int parameter_count, BytecodeOffset bailout_id, FrameStateType frame_state_type, SharedFunctionInfoRef shared, Node* context)
{
    const int parameter_count_with_receiver = parameter_count + JSCallOrConstructNode::kReceiverOrNewTargetInputCount;
    const FrameStateFunctionInfo* state_info = common()->CreateFrameStateFunctionInfo(frame_state_type, parameter_count_with_receiver, 0, shared.object());
    ...
    return FrameState { graph()->NewNode(op, params_node, node0, node0, context, node->InputAt(JSCallOrConstructNode::TargetIndex()), outer_frame_state) };
}
...
base::Optional<SharedFunctionInfoRef> JSInliner::DetermineCallTarget(Node* node)
{
...
Reduction JSInliner::ReduceJSWasmCall(Node* node)
{
    ...
    size_t subgraph_min_node_id;
    {
        ...
        Node* continuation_frame_state = CreateJSWasmCallBuiltinContinuationFrameState(jsgraph(), n.context(), n.frame_state(), wasm_call_params.signature());
        ...
        BuildInlinedJSToWasmWrapper(graph()->zone(), jsgraph(), wasm_call_params.signature(), wasm_call_params.module(), isolate(), source_positions_,
            StubCallMode::kCallBuiltinPointer, wasm::WasmFeatures::FromFlags(), continuation_frame_state);
    ...
    return InlineJSWasmCall(node, new_target, context, frame_state, start, end, exception_target, uncaught_subcalls);
}
#endif // V8_ENABLE_WEBASSEMBLY
...
Reduction JSInliner::ReduceJSCall(Node* node)
{
    ...
    SharedFunctionInfo::Inlineability inlineability = shared_info->GetInlineability();
    ...
    Node* end;
    {
        ...
        }
        {
            ...
            BuildGraphFromBytecode(broker(), zone(), *shared_info, feedback_cell, BytecodeOffset::None(), jsgraph(), frequency, source_positions_, node_origins_, inlining_id,
    ...
    if (node->opcode() == IrOpcode::kJSConstruct) {
        ...
        if (NeedsImplicitReceiver(*shared_info)) {
            ...
            Node* frame_state_inside = CreateArtificialFrameState(
                node, frame_state, n.ArgumentCount(), BytecodeOffset::ConstructStubCreate(), FrameStateType::kConstructStub, *shared_info, caller_context);
            ...
            branch_is_receiver_false = graph()->NewNode(
                javascript()->CallRuntime(Runtime::kThrowConstructorReturnedNonObject), caller_context, NodeProperties::GetFrameStateInput(node), node, branch_is_receiver_false);
            ...
            NodeProperties::MergeControlToEnd(graph(), common(), branch_is_receiver_false);
        ...
        frame_state
            = CreateArtificialFrameState(node, frame_state, n.ArgumentCount(), BytecodeOffset::ConstructStubInvoke(), FrameStateType::kConstructStub, *shared_info, caller_context);
    }
    ...
    if (node->opcode() == IrOpcode::kJSCall && is_sloppy(shared_info->language_mode()) && !shared_info->native()) {
        ...
        if (NodeProperties::CanBePrimitive(broker(), call.receiver(), effect)) {
            ...
            Node* global_proxy = jsgraph()->Constant(broker()->target_native_context().global_proxy_object());
            Node* receiver = effect = graph()->NewNode(simplified()->ConvertReceiver(p.convert_mode()), call.receiver(), global_proxy, effect, start);
    ...
    int parameter_count = shared_info->internal_formal_parameter_count_without_receiver();
    DCHECK_EQ(parameter_count, start.FormalParameterCountWithoutReceiver());
    if (call.argument_count() != parameter_count) {
        frame_state = CreateArtificialFrameState(node, frame_state, call.argument_count(), BytecodeOffset::None(), FrameStateType::kInlinedExtraArguments, *shared_info);
    }
...
JSOperatorBuilder* JSInliner::javascript() const { return jsgraph()->javascript(); }

CommonOperatorBuilder* JSInliner::common() const { return jsgraph()->common(); }

SimplifiedOperatorBuilder* JSInliner::simplified() const { return jsgraph()->simplified(); }
