...
Node* LinearScheduler::GetEarlySchedulePosition(Node* node)
{
    ...
    while (!stack.empty()) {
        ...
        if (NodeProperties::IsPhi(top.node)) {
            ...
            if (top.input_index == top.node->InputCount()) {
                ...
                if (input_early_schedule_position != nullptr) {
                    if (top.early_schedule_position == nullptr || GetControlLevel(top.early_schedule_position) < GetControlLevel(input_early_schedule_position)) {
        ...
        if (!stack.empty()) {
            ...
            if (use.early_schedule_position == nullptr || GetControlLevel(use.early_schedule_position) < GetControlLevel(top.early_schedule_position)) {
