...
void PeelWasmLoop(Node* loop_node, ZoneUnorderedSet<Node*>* loop, Graph* graph, CommonOperatorBuilder* common, Zone* tmp_zone, SourcePositionTable* source_positions,
    ...
    for (Node* node : loop_node->uses()) {
        ...
        for (Edge use_edge : node->use_edges()) {
            ...
            if (loop->count(use) == 1) {
                ...
                const Operator* phi_operator = use->opcode() == IrOpcode::kLoopExitEffect ? common->EffectPhi(2) : common->Phi(LoopExitValueRepresentationOf(use->op()), 2);
