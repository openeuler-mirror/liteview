...
void PipelineStatistics::CommonStats::Begin(PipelineStatistics* pipeline_stats)
{
    DCHECK(!scope_);
...
void PipelineStatistics::CommonStats::End(PipelineStatistics* pipeline_stats, CompilationStatistics::BasicStats* diff)
{
    DCHECK(scope_);
...
PipelineStatistics::PipelineStatistics(OptimizedCompilationInfo* info, std::shared_ptr<CompilationStatistics> compilation_stats, ZoneStats* zone_stats)
...
void PipelineStatistics::BeginPhaseKind(const char* phase_kind_name)
{
    ...
    if (InPhaseKind())
        EndPhaseKind();
    TRACE_EVENT_BEGIN1(kTraceCategory, phase_kind_name, "kind", CodeKindToString(code_kind_));
...
void PipelineStatistics::EndPhaseKind()
{
    ...
    TRACE_EVENT_END2(kTraceCategory, phase_kind_name_, "kind", CodeKindToString(code_kind_), "stats", TRACE_STR_COPY(diff.AsJSON().c_str()));
}

void PipelineStatistics::BeginPhase(const char* phase_name)
{
    TRACE_EVENT_BEGIN1(kTraceCategory, phase_name, "kind", CodeKindToString(code_kind_));
    DCHECK(InPhaseKind());
...
void PipelineStatistics::EndPhase()
{
    ...
    TRACE_EVENT_END2(kTraceCategory, phase_name_, "kind", CodeKindToString(code_kind_), "stats", TRACE_STR_COPY(diff.AsJSON().c_str()));
}
