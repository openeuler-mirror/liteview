...
WasmGCOperatorReducer::WasmGCOperatorReducer(Editor* editor, Zone* temp_zone_, MachineGraph* mcgraph, const wasm::WasmModule* module)
...
namespace {
bool InDeadBranch(Node* node) { return node->opcode() == IrOpcode::kDead || NodeProperties::GetType(node).AsWasm().type.is_bottom(); }
...
Reduction WasmGCOperatorReducer::UpdateNodeAndAliasesTypes(Node* state_owner, ControlPathTypes parent_state, Node* node, wasm::TypeInModule type, bool in_new_block)
{
...
Reduction WasmGCOperatorReducer::ReduceStart(Node* node) { return UpdateStates(node, ControlPathTypes(zone())); }

wasm::TypeInModule WasmGCOperatorReducer::ObjectTypeFromContext(Node* object, Node* control)
{
    if (InDeadBranch(object))
        return {};
    ...
    return type_from_state.IsSet() ? wasm::Intersection(type_from_node, type_from_state.type) : type_from_node;
}
...
Reduction WasmGCOperatorReducer::ReduceIf(Node* node, bool condition)
{
    ...
    case IrOpcode::kWasmTypeCheck: {
        ...
        wasm::ValueType rtt_type = wasm::ValueType::RefNull(NodeProperties::GetType(rtt).AsWasm().type.ref_index());

        // TODO(manoskouk): Think about {module_} below if we have cross-module
        // inlining.
        wasm::TypeInModule new_type = wasm::Intersection(object_type, { rtt_type, module_ });
...
Reduction WasmGCOperatorReducer::ReduceCheckNull(Node* node)
{
    ...
    // Optimize the check away if the argument is known to be non-null.
    if (object_type.type.is_non_nullable()) {
        ReplaceWithValue(node, gasm_.Int32Constant(node->opcode() == IrOpcode::kIsNull ? 0 : 1));
        node->Kill();
    ...
    if (object->opcode() == IrOpcode::kNull) {
        ReplaceWithValue(node, gasm_.Int32Constant(node->opcode() == IrOpcode::kIsNull ? 1 : 0));
        node->Kill();
        return Replace(object); // Irrelevant replacement.
    }
...
Reduction WasmGCOperatorReducer::ReduceWasmTypeCast(Node* node)
{
    ...
    if (wasm::HeapTypesUnrelated(object_type.type.heap_type(), wasm::HeapType(rtt_type.type.ref_index()), object_type.module, rtt_type.module)) {
        ...
        Node* non_trapping_condition = object_type.type.is_nullable() ? gasm_.IsNull(object) : gasm_.Int32Constant(0);
    ...
    if (!object_type.type.is_nullable()) {
        ...
        NodeProperties::ChangeOp(node,
            gasm_.simplified()->WasmTypeCast({ false, // object_can_be_null
                false, // null_succeeds
                rtt_depth }));
    }
...
Reduction WasmGCOperatorReducer::ReduceWasmTypeCheck(Node* node)
{
    ...
    if (wasm::IsHeapSubtypeOf(object_type.type.heap_type(), wasm::HeapType(rtt_type.type.ref_index()), object_type.module, rtt_type.module)) {
        bool null_succeeds = OpParameter<WasmTypeCheckConfig>(node->op()).null_succeeds;
        // Type cast will fail only on null.
        gasm_.InitializeEffectControl(effect, control);
        Node* condition = SetType(object_type.type.is_nullable() && !null_succeeds ? gasm_.IsNotNull(object) : gasm_.Int32Constant(1), wasm::kWasmI32);
    ...
    if (!object_type.type.is_nullable()) {
        uint8_t rtt_depth = OpParameter<WasmTypeCheckConfig>(node->op()).rtt_depth;
        NodeProperties::ChangeOp(node,
            gasm_.simplified()->WasmTypeCheck({ false, // object_can_be_null
                false, // null_succeeds
                rtt_depth }));
    }
