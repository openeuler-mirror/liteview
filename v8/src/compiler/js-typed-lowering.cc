...
public:
    ...
    bool BothInputsMaybe(Type t) { return left_type().Maybe(t) && right_type().Maybe(t); }

    bool OneInputCannotBe(Type t) { return !left_type().Maybe(t) || !right_type().Maybe(t); }

    bool NeitherInputCanBe(Type t) { return !left_type().Maybe(t) && !right_type().Maybe(t); }
...
private:
    ...
    void update_effect(Node* effect) { NodeProperties::ReplaceEffectInput(node_, effect); }
};
...
JSTypedLowering::JSTypedLowering(Editor* editor, JSGraph* jsgraph, JSHeapBroker* broker, Zone* zone)
    ...
    , jsgraph_(jsgraph)
    , broker_(broker)
    , empty_string_type_(Type::Constant(broker, factory()->empty_string(), graph()->zone()))
    , pointer_comparable_type_(Type::Union(Type::Oddball(), Type::Union(Type::SymbolOrReceiver(), empty_string_type_, graph()->zone()), graph()->zone()))
...
Reduction JSTypedLowering::ReduceJSAdd(Node* node)
{
    ...
    if (r.BothInputsAre(Type::String())) {
        ...
        if (string_length_protector.value().AsSmi() == Protectors::kProtectorValid) {
            ...
            // more truncations.
            length = effect = graph()->NewNode(simplified()->CheckBounds(FeedbackSource()), length, jsgraph()->Constant(String::kMaxLength + 1), effect, control);
        } else {
            // Check if we would overflow the allowed maximum string length.
            Node* check = graph()->NewNode(simplified()->NumberLessThanOrEqual(), length, jsgraph()->Constant(String::kMaxLength));
            Node* branch = graph()->NewNode(common()->Branch(BranchHint::kTrue), check, control);
            Node* if_false = graph()->NewNode(common()->IfFalse(), branch);
            Node* efalse = effect;
            {
                // Throw a RangeError in case of overflow.
                Node* vfalse = efalse = if_false = graph()->NewNode(javascript()->CallRuntime(Runtime::kThrowInvalidStringLength), context, frame_state, efalse, if_false);
        ...
        Operator const* const op = r.ShouldCreateConsString() ? simplified()->NewConsString() : simplified()->StringConcat();
    ...
    if (r.OneInputIs(Type::String())) {
        ...
        auto call_descriptor
            = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNeedsFrameState, properties);
    ...
    if (input_type.Is(type_cache_->kIntegerOrMinusZero)) {
        if (input_type.IsNone() || input_type.Max() <= 0.0) {
            ...
            if (input_type.Max() > kMaxSafeInteger) {
                input = graph()->NewNode(simplified()->NumberMin(), jsgraph()->Constant(kMaxSafeInteger), input);
            }
        }
        ReplaceWithValue(node, input);
        return Replace(input);
...
Reduction JSTypedLowering::ReduceJSToStringInput(Node* input)
{
    ...
    if (input_type.Is(Type::Boolean())) {
        return Replace(graph()->NewNode(
            common()->Select(MachineRepresentation::kTagged), input, jsgraph()->HeapConstant(factory()->true_string()), jsgraph()->HeapConstant(factory()->false_string())));
    }
    if (input_type.Is(Type::Undefined())) {
...
Reduction JSTypedLowering::ReduceJSToObject(Node* node)
{
    ...
    Node* rfalse;
    {
        ...
        rfalse = efalse = if_false = graph()->NewNode(common()->Call(call_descriptor), jsgraph()->HeapConstant(callable.code()), receiver, context, frame_state, efalse, if_false);
    }
...
Reduction JSTypedLowering::ReduceJSHasInPrototypeChain(Node* node)
{
    ...
    Node* check1 = graph()->NewNode(simplified()->NumberLessThanOrEqual(), value_instance_type, jsgraph()->Constant(LAST_SPECIAL_RECEIVER_TYPE));
    ...
    Node* check10 = graph()->NewNode(simplified()->NumberLessThan(), value_instance_type, jsgraph()->Constant(FIRST_JS_RECEIVER_TYPE));
    ...
    Node* vfalse1;
    {
        ...
        vfalse1 = efalse1 = if_false1 = graph()->NewNode(javascript()->CallRuntime(Runtime::kHasInPrototypeChain), value, prototype, context, frame_state, efalse1, if_false1);
...
Reduction JSTypedLowering::ReduceJSHasContextExtension(Node* node)
{
    ...
    for (size_t i = 0; i < depth; ++i) {
        context = effect = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForContextSlotKnownPointer(Context::PREVIOUS_INDEX)), context, effect, control);
    }
    Node* const scope_info = effect = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForContextSlot(Context::SCOPE_INFO_INDEX)), context, effect, control);
    Node* scope_info_flags = effect = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForScopeInfoFlags()), scope_info, effect, control);
    Node* flags_masked = graph()->NewNode(simplified()->NumberBitwiseAnd(), scope_info_flags, jsgraph()->SmiConstant(ScopeInfo::HasContextExtensionSlotBit::kMask));
    Node* no_extension = graph()->NewNode(simplified()->NumberEqual(), flags_masked, jsgraph()->SmiConstant(0));
    Node* has_extension = graph()->NewNode(simplified()->BooleanNot(), no_extension);
...
void ReduceBuiltin(JSGraph* jsgraph, Node* node, Builtin builtin, int arity, CallDescriptor::Flags flags)
{
    ...
    Node* stub = jsgraph->CEntryStubConstant(1, SaveFPRegsMode::kIgnore, ArgvMode::kStack, has_builtin_exit_frame);
...
Reduction JSTypedLowering::ReduceJSConstructForwardVarargs(Node* node)
{
    ...
    if (target_type.IsHeapConstant() && target_type.AsHeapConstant()->Ref().IsJSFunction()) {
        ...
        node->InsertInput(graph()->zone(), 0, jsgraph()->HeapConstant(callable.code()));
        node->InsertInput(graph()->zone(), 3, jsgraph()->Constant(JSParameterCount(arity)));
        node->InsertInput(graph()->zone(), 4, jsgraph()->Constant(start_index));
        node->InsertInput(graph()->zone(), 5, jsgraph()->UndefinedConstant());
        NodeProperties::ChangeOp(node, common()->Call(Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), arity + 1, CallDescriptor::kNeedsFrameState)));
        return Changed(node);
    }
...
Reduction JSTypedLowering::ReduceJSConstruct(Node* node)
{
    ...
    if (target_type.IsHeapConstant() && target_type.AsHeapConstant()->Ref().IsJSFunction()) {
        ...
        Callable callable = Builtins::CallableFor(isolate(), function.shared().construct_as_builtin() ? Builtin::kJSBuiltinsConstructStub : Builtin::kJSConstructStubGeneric);
        ...
        NodeProperties::ChangeOp(node, common()->Call(Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), 1 + arity, CallDescriptor::kNeedsFrameState)));
        return Changed(node);
    }
...
Reduction JSTypedLowering::ReduceJSCallForwardVarargs(Node* node)
{
    ...
    if (target_type.Is(Type::CallableFunction())) {
        ...
        node->InsertInput(graph()->zone(), 0, jsgraph()->HeapConstant(callable.code()));
        node->InsertInput(graph()->zone(), 2, jsgraph()->Constant(JSParameterCount(arity)));
...
Reduction JSTypedLowering::ReduceJSCall(Node* node)
{
    ...
    if (shared.has_value()) {
        ...
        if (is_sloppy(shared->language_mode()) && !shared->native() && !receiver_type.Is(Type::Receiver())) {
            if (!function.has_value() || !function->native_context().equals(broker()->target_native_context())) {
                return NoChange();
            }
            Node* global_proxy = jsgraph()->Constant(function->native_context().global_proxy_object());
            receiver = effect = graph()->NewNode(simplified()->ConvertReceiver(convert_mode), receiver, global_proxy, effect, control);
        ...
        if (formal_count > arity) {
            ...
            node->InsertInput(graph()->zone(), formal_count + 3, jsgraph()->Constant(JSParameterCount(arity)));
            NodeProperties::ChangeOp(node, common()->Call(Linkage::GetJSCallDescriptor(graph()->zone(), false, 1 + formal_count, flags | CallDescriptor::kCanUseRoots)));
            ...
            node->InsertInput(graph()->zone(), arity + 3, jsgraph()->Constant(JSParameterCount(arity)));
            NodeProperties::ChangeOp(node, common()->Call(Linkage::GetJSCallDescriptor(graph()->zone(), false, 1 + arity, flags | CallDescriptor::kCanUseRoots)));
        }
        return Changed(node);
    }
...
Reduction JSTypedLowering::ReduceJSForInNext(Node* node)
{
    ...
    case ForInMode::kGeneric: {
        ...
        Node* key = effect = graph()->NewNode(simplified()->LoadElement(AccessBuilder::ForJSForInCacheArrayElement(n.Parameters().mode())), cache_array, index, effect, control);
        ...
        Node* vfalse;
        {
            ...
            auto call_descriptor
                = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNeedsFrameState);
            vfalse = efalse = if_false
                = graph()->NewNode(common()->Call(call_descriptor), jsgraph()->HeapConstant(callable.code()), key, receiver, context, frame_state, effect, if_false);
...
Reduction JSTypedLowering::ReduceJSForInPrepare(Node* node)
{
    ...
    case ForInMode::kUseEnumCacheKeysAndIndices: {
        ...
        effect = graph()->NewNode(simplified()->CheckMaps(CheckMapsFlag::kNone, ZoneHandleSet<Map>(factory()->meta_map())), enumerator, effect, control);
        ...
        cache_length = graph()->NewNode(simplified()->NumberBitwiseAnd(), bit_field3, jsgraph()->Constant(Map::Bits3::EnumLengthBits::kMask));
        break;
    }
    case ForInMode::kGeneric: {
        // Check if the {enumerator} is a Map or a FixedArray.
        Node* check = effect = graph()->NewNode(simplified()->CompareMaps(ZoneHandleSet<Map>(factory()->meta_map())), enumerator, effect, control);
        Node* branch = graph()->NewNode(common()->Branch(BranchHint::kTrue), check, control);
        ...
        {
            // Load the enum cache from the {enumerator} map.
            Node* descriptor_array = etrue = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForMapDescriptors()), enumerator, etrue, if_true);
            Node* enum_cache = etrue = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForDescriptorArrayEnumCache()), descriptor_array, etrue, if_true);
            cache_array_true = etrue = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForEnumCacheKeys()), enum_cache, etrue, if_true);

            // Load the enum length of the {enumerator} map.
            Node* bit_field3 = etrue = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForMapBitField3()), enumerator, etrue, if_true);
            static_assert(Map::Bits3::EnumLengthBits::kShift == 0);
            cache_length_true = graph()->NewNode(simplified()->NumberBitwiseAnd(), bit_field3, jsgraph()->Constant(Map::Bits3::EnumLengthBits::kMask));
        }
        ...
        Node* cache_length_false;
        {
            ...
            cache_length_false = efalse = graph()->NewNode(simplified()->LoadField(AccessBuilder::ForFixedArrayLength()), cache_array_false, efalse, if_false);
        }
...
Reduction JSTypedLowering::ReduceObjectIsArray(Node* node)
{
    ...
    check = graph()->NewNode(simplified()->NumberEqual(), value_instance_type, jsgraph()->Constant(JS_ARRAY_TYPE));
    ...
    check = graph()->NewNode(simplified()->NumberEqual(), value_instance_type, jsgraph()->Constant(JS_PROXY_TYPE));
    control = graph()->NewNode(common()->Branch(BranchHint::kFalse), check, control);
...
CompilationDependencies* JSTypedLowering::dependencies() const { return broker()->dependencies(); }

Isolate* JSTypedLowering::isolate() const { return jsgraph()->isolate(); }

JSOperatorBuilder* JSTypedLowering::javascript() const { return jsgraph()->javascript(); }

CommonOperatorBuilder* JSTypedLowering::common() const { return jsgraph()->common(); }

SimplifiedOperatorBuilder* JSTypedLowering::simplified() const { return jsgraph()->simplified(); }
