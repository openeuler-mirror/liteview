...
public:
    ...
    bool IsYoungGenerationAllocation() const { return allocation() == AllocationType::kYoung; }
...
MemoryLowering::MemoryLowering(JSGraph* jsgraph, Zone* zone, JSGraphAssembler* graph_assembler, AllocationFolding allocation_folding, WriteBarrierAssertFailedCallback callback,
...
void MemoryLowering::EnsureAllocateOperator()
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph_zone(), descriptor, descriptor.GetStackParameterCount(), CallDescriptor::kCanUseRoots, Operator::kNoThrow, mode);
...
Node* MemoryLowering::GetWasmInstanceNode()
{
    ...
    for (Node* use : graph()->start()->uses()) {
        if (use->opcode() == IrOpcode::kParameter && ParameterIndexOf(use->op()) == wasm::kWasmInstanceParameterIndex) {
...
Reduction MemoryLowering::ReduceAllocateRaw(Node* node, AllocationType allocation_type, AllowLargeObjects allow_large_objects, AllocationState const** state_ptr)
{
    ...
    if (isolate_ != nullptr) {
        ...
        int top_address_offset
            = allocation_type == AllocationType::kYoung ? WasmInstanceObject::kNewAllocationTopAddressOffset : WasmInstanceObject::kOldAllocationTopAddressOffset;
        int limit_address_offset
            = allocation_type == AllocationType::kYoung ? WasmInstanceObject::kNewAllocationLimitAddressOffset : WasmInstanceObject::kOldAllocationLimitAddressOffset;
...
Reduction MemoryLowering::ReduceLoadFromObject(Node* node)
{
    ...
    const Operator* load_op = ElementSizeInBytes(rep) > kTaggedSize && !machine()->UnalignedLoadSupported(machine_type.representation()) ? machine()->UnalignedLoad(machine_type)
...
Reduction MemoryLowering::ReduceLoadExternalPointerField(Node* node)
{
    ...
    if (IsSandboxedExternalPointerType(tag)) {
        ...
        Node* table_address = IsSharedExternalPointerType(tag) ? __ Load(
                                  MachineType::Pointer(), __ ExternalConstant(ExternalReference::shared_external_pointer_table_address_address(isolate())), __ IntPtrConstant(0))
                                                               : __ ExternalConstant(ExternalReference::external_pointer_table_address(isolate()));
        Node* table = __ Load(MachineType::Pointer(), table_address, Internals::kExternalPointerTableBufferOffset);
...
Reduction MemoryLowering::ReduceLoadBoundedSize(Node* node)
{
    ...
    const Operator* load_op
        = !machine()->UnalignedLoadSupported(MachineRepresentation::kWord64) ? machine()->UnalignedLoad(MachineType::Uint64()) : machine()->Load(MachineType::Uint64());
    NodeProperties::ChangeOp(node, load_op);
...
Reduction MemoryLowering::ReduceStoreToObject(Node* node, AllocationState const* state)
{
    ...
    const Operator* store_op = ElementSizeInBytes(rep) > kTaggedSize && !machine()->UnalignedStoreSupported(rep) ? machine()->UnalignedStore(rep) : machine()->Store(store_rep);
...
Reduction MemoryLowering::ReduceAllocateRaw(Node* node)
{
    ...
    return ReduceAllocateRaw(node, allocation.allocation_type(), allocation.allow_large_objects(), nullptr);
}

WriteBarrierKind MemoryLowering::ComputeWriteBarrierKind(Node* node, Node* object, Node* value, AllocationState const* state, WriteBarrierKind write_barrier_kind)
{
...
void MemoryLowering::AllocationGroup::Add(Node* node) { node_ids_.insert(node->id()); }
...
bool MemoryLowering::AllocationState::IsYoungGenerationAllocation() const { return group() && group()->IsYoungGenerationAllocation(); }
