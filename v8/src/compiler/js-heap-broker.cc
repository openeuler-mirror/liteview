...
void JSHeapBroker::SetPersistentAndCopyCanonicalHandlesForTesting(std::unique_ptr<PersistentHandles> persistent_handles, std::unique_ptr<CanonicalHandlesMap> canonical_handles)
{
    set_persistent_handles(std::move(persistent_handles));
    CopyCanonicalHandlesForTesting(std::move(canonical_handles));
}

void JSHeapBroker::CopyCanonicalHandlesForTesting(std::unique_ptr<CanonicalHandlesMap> canonical_handles)
{
    DCHECK_NULL(canonical_handles_);
    canonical_handles_ = std::make_unique<CanonicalHandlesMap>(isolate_->heap(), ZoneAllocationPolicy(zone()));
...
void JSHeapBroker::SetTargetNativeContextRef(Handle<NativeContext> native_context)
{
    DCHECK((mode() == kDisabled && !target_native_context_.has_value()) || (mode() == kSerializing && target_native_context_->object().is_identical_to(native_context)));
...
void JSHeapBroker::CollectArrayAndObjectPrototypes()
{
    ...
    while (!maybe_context.IsUndefined(isolate())) {
        ...
        array_and_object_prototypes_.emplace(JSObject::cast(object_prot), isolate());
...
bool JSHeapBroker::IsArrayOrObjectPrototype(const JSObjectRef& object) const { return IsArrayOrObjectPrototype(object.object()); }

bool JSHeapBroker::IsArrayOrObjectPrototype(Handle<JSObject> object) const
{
    if (mode() == kDisabled) {
        return isolate()->IsInAnyContext(*object, Context::INITIAL_ARRAY_PROTOTYPE_INDEX) || isolate()->IsInAnyContext(*object, Context::INITIAL_OBJECT_PROTOTYPE_INDEX);
    }
    CHECK(!array_and_object_prototypes_.empty());
    return array_and_object_prototypes_.find(object) != array_and_object_prototypes_.end();
}

ObjectData* JSHeapBroker::TryGetOrCreateData(Object object, GetOrCreateDataFlags flags) { return TryGetOrCreateData(CanonicalPersistentHandle(object), flags); }
...
ObjectData* JSHeapBroker::GetOrCreateData(Object object, GetOrCreateDataFlags flags) { return GetOrCreateData(CanonicalPersistentHandle(object), flags); }
...
bool JSHeapBroker::ObjectMayBeUninitialized(Handle<Object> object) const { return ObjectMayBeUninitialized(*object); }
...
bool JSHeapBroker::ObjectMayBeUninitialized(HeapObject object) const { return !IsMainThread() && isolate()->heap()->IsPendingAllocation(object); }
...
KeyedAccessMode ElementAccessFeedback::keyed_mode() const { return keyed_mode_; }

ZoneVector<ElementAccessFeedback::TransitionGroup> const& ElementAccessFeedback::transition_groups() const { return transition_groups_; }
...
ElementAccessFeedback const& ElementAccessFeedback::Refine(JSHeapBroker* broker, ZoneVector<MapRef> const& inferred_maps) const
{
    ...
    for (auto const& group : transition_groups()) {
        ...
        bool const keep_target = inferred.find(target) != inferred.end() || new_group.size() > 1;
        if (keep_target) {
        ...
        if (!new_group.empty()) {
            DCHECK(new_group.size() == 1 || new_group.front().equals(target.object()));
...
bool GlobalAccessFeedback::IsMegamorphic() const { return !cell_or_context_.has_value(); }
bool GlobalAccessFeedback::IsPropertyCell() const { return cell_or_context_.has_value() && cell_or_context_->IsPropertyCell(); }
bool GlobalAccessFeedback::IsScriptContextSlot() const { return cell_or_context_.has_value() && cell_or_context_->IsContext(); }
...
bool KeyedAccessMode::IsLoad() const { return access_mode_ == AccessMode::kLoad || access_mode_ == AccessMode::kHas; }
bool KeyedAccessMode::IsStore() const { return access_mode_ == AccessMode::kStore || access_mode_ == AccessMode::kDefine || access_mode_ == AccessMode::kStoreInLiteral; }
...
bool ElementAccessFeedback::HasOnlyStringMaps(JSHeapBroker* broker) const
{
    for (auto const& group : transition_groups()) {
        for (Handle<Map> map : group) {
            ...
            // vector.
            if (!MakeRefAssumeMemoryFence(broker, map).IsStringMap())
                return false;
        }
    }
    return true;
...
NamedAccessFeedback::NamedAccessFeedback(NameRef const& name, ZoneVector<MapRef> const& maps, FeedbackSlotKind slot_kind)
    : ProcessedFeedback(kNamedAccess, slot_kind)
    , name_(name)
    , maps_(maps)
{
    DCHECK(IsLoadICKind(slot_kind) || IsSetNamedICKind(slot_kind) || IsDefineNamedOwnICKind(slot_kind) || IsKeyedLoadICKind(slot_kind) || IsKeyedHasICKind(slot_kind)
        || IsKeyedStoreICKind(slot_kind) || IsStoreInArrayLiteralICKind(slot_kind) || IsDefineKeyedOwnPropertyInLiteralKind(slot_kind) || IsDefineKeyedOwnICKind(slot_kind));
}
...
const ProcessedFeedback& JSHeapBroker::NewInsufficientFeedback(FeedbackSlotKind kind) const { return *zone()->New<InsufficientFeedback>(kind); }

ProcessedFeedback const& JSHeapBroker::ReadFeedbackForPropertyAccess(FeedbackSource const& source, AccessMode mode, base::Optional<NameRef> static_name)
{
    FeedbackNexus nexus(source.vector, source.slot, feedback_nexus_config());
    FeedbackSlotKind kind = nexus.kind();
    ...
    ZoneVector<MapRef> maps(zone());
    {
        ...
        for (const MapAndHandler& map_and_handler : maps_and_handlers_unfiltered) {
            ...
            if (map.is_deprecated()) {
                ...
                base::Optional<Map> maybe_map = MapUpdater::TryUpdateNoLock(isolate(), *map.object(), ConcurrencyMode::kConcurrent);
    ...
    base::Optional<NameRef> name = static_name.has_value() ? static_name : GetNameFeedback(nexus);
    ...
    if (nexus.ic_state() == InlineCacheState::MEGADOM) {
        ...
        if (!maybe_handler.is_null()) {
            Handle<MegaDomHandler> handler = Handle<MegaDomHandler>::cast(maybe_handler.object());
            if (!handler->accessor(kAcquireLoad)->IsCleared()) {
                FunctionTemplateInfoRef info = MakeRefAssumeMemoryFence(this, FunctionTemplateInfo::cast(handler->accessor(kAcquireLoad).GetHeapObject()));
...
ProcessedFeedback const& JSHeapBroker::ReadFeedbackForGlobalAccess(FeedbackSource const& source)
{
    FeedbackNexus nexus(source.vector, source.slot, feedback_nexus_config());
    ...
    if (feedback_value->IsSmi()) {
        ...
        ContextRef context = MakeRefAssumeMemoryFence(this, target_native_context().script_context_table().object()->get_context(script_context_index, kAcquireLoad));
    ...
    return *zone()->New<GlobalAccessFeedback>(MakeRefAssumeMemoryFence(this, Handle<PropertyCell>::cast(feedback_value)), nexus.kind());
}
...
ProcessedFeedback const& JSHeapBroker::ReadFeedbackForArrayOrObjectLiteral(FeedbackSource const& source)
{
    ...
    AllocationSiteRef site = MakeRefAssumeMemoryFence(this, AllocationSite::cast(object));
...
ProcessedFeedback const& JSHeapBroker::ReadFeedbackForRegExpLiteral(FeedbackSource const& source)
{
    ...
    RegExpBoilerplateDescriptionRef boilerplate = MakeRefAssumeMemoryFence(this, RegExpBoilerplateDescription::cast(object));
...
ProcessedFeedback const& JSHeapBroker::GetFeedbackForPropertyAccess(FeedbackSource const& source, AccessMode mode, base::Optional<NameRef> static_name)
{
    if (HasFeedback(source))
        return GetFeedback(source);
...
ElementAccessFeedback const& JSHeapBroker::ProcessFeedbackMapsForElementAccess(ZoneVector<MapRef>& maps, KeyedAccessMode const& keyed_mode, FeedbackSlotKind slot_kind)
{
    DCHECK(!maps.empty());
    ...
    struct HandleLess {
        bool operator()(Handle<Map> x, Handle<Map> y) const { return x.address() < y.address(); }
    };
    ...
    for (const MapRef& map : maps) {
        ...
        if (!map.is_stable()) {
            ...
            transition_target = map.object()->FindElementsKindTransitionedMap(isolate(), possible_transition_targets, ConcurrencyMode::kConcurrent);
        }
...
void ElementAccessFeedback::AddGroup(TransitionGroup&& group)
{
    ...
    for (Handle<Map> map : group) {
        ...
        for (TransitionGroup const& some_group : transition_groups()) {
            count += std::count_if(some_group.begin(), some_group.end(), [&](Handle<Map> some_map) { return some_map.equals(map); });
        }
        CHECK_EQ(count, 1);
    }
#endif
...
base::Optional<NameRef> JSHeapBroker::GetNameFeedback(FeedbackNexus const& nexus)
{
...
PropertyAccessInfo JSHeapBroker::GetPropertyAccessInfo(MapRef map, NameRef name, AccessMode access_mode, CompilationDependencies* dependencies)
{
