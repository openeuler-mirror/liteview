...
// static
void NodeProperties::RemoveNonValueInputs(Node* node) { node->TrimInputCount(node->op()->ValueInputCount()); }
...
// static
base::Optional<MapRef> NodeProperties::GetJSCreateMap(JSHeapBroker* broker, Node* receiver)
{
    ...
    if (mtarget.HasResolvedValue() && mnewtarget.HasResolvedValue() && mnewtarget.Ref(broker).IsJSFunction()) {
        ...
        if (newtarget.map().has_prototype_slot() && newtarget.has_initial_map(broker->dependencies())) {
...
ZoneRefUnorderedSet<MapRef> ToRefSet(JSHeapBroker* broker, const ZoneHandleSet<Map>& handles)
{
    ZoneRefUnorderedSet<MapRef> refs = ZoneRefUnorderedSet<MapRef>(broker->zone());
...
// static
NodeProperties::InferMapsResult NodeProperties::InferMapsUnsafe(JSHeapBroker* broker, Node* receiver, Effect effect, ZoneRefUnorderedSet<MapRef>* maps_out)
{
    ...
    while (true) {
        ...
        case IrOpcode::kCheckMaps: {
            Node* const object = GetValueInput(effect, 0);
            if (IsSame(receiver, object)) {
                *maps_out = ToRefSet(broker, CheckMapsParametersOf(effect->op()).maps());
                return result;
            }
            break;
        }
        ...
        case IrOpcode::kJSCreatePromise: {
            if (IsSame(receiver, effect)) {
                *maps_out = RefSetOf(broker, broker->target_native_context().promise_function().initial_map(broker->dependencies()));
                return result;
            }
            break;
        }
...
// static
Type NodeProperties::GetTypeOrAny(const Node* node) { return IsTyped(node) ? node->type() : Type::Any(); }
