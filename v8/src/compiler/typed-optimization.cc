...
TypedOptimization::TypedOptimization(Editor* editor, CompilationDependencies* dependencies, JSGraph* jsgraph, JSHeapBroker* broker)
    ...
    , jsgraph_(jsgraph)
    , broker_(broker)
    , true_type_(Type::Constant(broker, factory()->true_value(), graph()->zone()))
    , false_type_(Type::Constant(broker, factory()->false_value(), graph()->zone()))
...
base::Optional<MapRef> GetStableMapFromObjectType(JSHeapBroker* broker, Type object_type)
{
...
Reduction TypedOptimization::ReduceMaybeGrowFastElements(Node* node)
{
    ...
    if (!index_type.IsNone() && !length_type.IsNone() && index_type.Max() < length_type.Min()) {
        Node* check_bounds = graph()->NewNode(simplified()->CheckBounds(FeedbackSource {}, CheckBoundsFlag::kAbortOnOutOfBounds), index, length, effect, control);
...
Reduction TypedOptimization::ReduceCheckBounds(Node* node)
{
    ...
    if (p.flags() & CheckBoundsFlag::kConvertStringAndMinusZero && !input_type.Maybe(Type::String()) && !input_type.Maybe(Type::MinusZero())) {
        NodeProperties::ChangeOp(node, simplified()->CheckBounds(p.check_parameters().feedback(), p.flags().without(CheckBoundsFlag::kConvertStringAndMinusZero)));
        return Changed(node);
    }
    return NoChange();
}
...
Reduction TypedOptimization::ReduceReferenceEqual(Node* node)
{
    ...
    if (rhs_type.Is(Type::Boolean()) && rhs_type.IsHeapConstant() && lhs_type.Is(Type::Boolean())) {
        base::Optional<bool> maybe_result = rhs_type.AsHeapConstant()->Ref().TryGetBooleanValue();
...
Reduction TypedOptimization::TryReduceStringComparisonOfStringFromSingleCharCodeToConstant(Node* comparison, const StringRef& string, bool inverted)
{
...
Reduction TypedOptimization::TryReduceStringComparisonOfStringFromSingleCharCode(Node* comparison, Node* from_char_code, Type constant_type, bool inverted)
{
    ...
    if (!from_char_code_repl_type.Is(type_cache_->kUint16)) {
        ...
        from_char_code_repl = graph()->NewNode(simplified()->NumberBitwiseAnd(), from_char_code_repl, jsgraph()->Constant(std::numeric_limits<uint16_t>::max()));
    }
    if (!string.GetFirstChar().has_value())
        return NoChange();
    ...
    if (lhs->opcode() == IrOpcode::kStringFromSingleCharCode) {
        if (rhs->opcode() == IrOpcode::kStringFromSingleCharCode) {
            ...
            if (!left_type.Is(type_cache_->kUint16)) {
                ...
                left = graph()->NewNode(simplified()->NumberBitwiseAnd(), left, jsgraph()->Constant(std::numeric_limits<uint16_t>::max()));
            }
            if (!right_type.Is(type_cache_->kUint16)) {
                // Convert to signed int32 to satisfy type of {NumberBitwiseAnd}.
                right = graph()->NewNode(simplified()->NumberToInt32(), right);
                right = graph()->NewNode(simplified()->NumberBitwiseAnd(), right, jsgraph()->Constant(std::numeric_limits<uint16_t>::max()));
            }
...
Reduction TypedOptimization::ReduceTypeOf(Node* node)
{
    ...
    if (type.Is(Type::Boolean())) {
        ...
        return Replace(jsgraph()->Constant(MakeRef(broker(), f->function_string())));
    }
    return NoChange();
}
...
bool NeitherCanBe(Type t1, Type t2, Type t3) { return !t1.Maybe(t3) && !t2.Maybe(t3); }
...
Factory* TypedOptimization::factory() const { return jsgraph()->isolate()->factory(); }

Graph* TypedOptimization::graph() const { return jsgraph()->graph(); }

SimplifiedOperatorBuilder* TypedOptimization::simplified() const { return jsgraph()->simplified(); }
