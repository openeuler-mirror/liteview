...
void LoopVariableOptimizer::VisitLoopExit(Node* node) { return TakeConditionsFromFirstControl(node); }
...
void LoopVariableOptimizer::TakeConditionsFromFirstControl(Node* node) { limits_.Set(node, limits_.Get(NodeProperties::GetControlInput(node, 0))); }
...
InductionVariable* LoopVariableOptimizer::TryGetInductionVariable(Node* phi)
{
    ...
    } else if (arith->opcode() == IrOpcode::kJSSubtract || arith->opcode() == IrOpcode::kNumberSubtract || arith->opcode() == IrOpcode::kSpeculativeNumberSubtract
