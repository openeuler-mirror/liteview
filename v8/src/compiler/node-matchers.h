...
struct NodeMatcher {
    ...
    bool HasProperty(Operator::Property property) const { return op()->HasProperty(property); }
...
template <typename T, IrOpcode::Value kOpcode> struct IntMatcher final : public ValueMatcher<T, kOpcode> {
    ...
    bool Is(const T& value) const { return this->HasResolvedValue() && this->ResolvedValue() == value; }
    bool IsInRange(const T& low, const T& high) const { return this->HasResolvedValue() && base::IsInRange(this->ResolvedValue(), low, high); }
    bool IsMultipleOf(T n) const { return this->HasResolvedValue() && (this->ResolvedValue() % n) == 0; }
    bool IsPowerOf2() const { return this->HasResolvedValue() && this->ResolvedValue() > 0 && (this->ResolvedValue() & (this->ResolvedValue() - 1)) == 0; }
    ...
    bool IsNegative() const { return this->HasResolvedValue() && this->ResolvedValue() < 0; }
};
...
template <typename T, IrOpcode::Value kOpcode> struct FloatMatcher final : public ValueMatcher<T, kOpcode> {
    ...
    bool Is(const T& value) const { return this->HasResolvedValue() && this->ResolvedValue() == value; }
    bool IsInRange(const T& low, const T& high) const { return this->HasResolvedValue() && low <= this->ResolvedValue() && this->ResolvedValue() <= high; }
    bool IsMinusZero() const { return this->Is(0.0) && std::signbit(this->ResolvedValue()); }
    bool IsNegative() const { return this->HasResolvedValue() && this->ResolvedValue() < 0.0; }
    bool IsNaN() const { return this->HasResolvedValue() && std::isnan(this->ResolvedValue()); }
    bool IsZero() const { return this->Is(0.0) && !std::signbit(this->ResolvedValue()); }
    bool IsNormal() const { return this->HasResolvedValue() && std::isnormal(this->ResolvedValue()); }
    bool IsInteger() const { return this->HasResolvedValue() && std::nearbyint(this->ResolvedValue()) == this->ResolvedValue(); }
...
template <IrOpcode::Value kHeapConstantOpcode> struct HeapObjectMatcherImpl final : public ValueMatcher<Handle<HeapObject>, kHeapConstantOpcode> {
    explicit HeapObjectMatcherImpl(Node* node)
        : ValueMatcher<Handle<HeapObject>, kHeapConstantOpcode>(node)
    {
    }

    bool Is(Handle<HeapObject> const& value) const { return this->HasResolvedValue() && this->ResolvedValue().address() == value.address(); }
...
struct ExternalReferenceMatcher final : public ValueMatcher<ExternalReference, IrOpcode::kExternalConstant> {
    ...
    bool Is(const ExternalReference& value) const { return this->HasResolvedValue() && this->ResolvedValue() == value; }
};
...
template <typename Left, typename Right> struct BinopMatcher : public NodeMatcher {
    ...
    bool IsFoldable() const { return left().HasResolvedValue() && right().HasResolvedValue(); }
...
using CompressedHeapObjectBinopMatcher = BinopMatcher<CompressedHeapObjectMatcher, CompressedHeapObjectMatcher>;
...
struct V8_EXPORT_PRIVATE DiamondMatcher : public NON_EXPORTED_BASE(NodeMatcher) {
    ...
    bool IfProjectionsAreOwned() const { return if_true_->OwnedBy(node()) && if_false_->OwnedBy(node()); }
...
struct LoadTransformMatcher : ValueMatcher<LoadTransformParameters, IrOpcode::kLoadTransform> {
    ...
    bool Is(LoadTransformation t) { return HasResolvedValue() && ResolvedValue().transformation == t; }
};
