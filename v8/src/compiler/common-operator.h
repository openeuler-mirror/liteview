...
public:
    ...
    public:
        ...
        Node* Get(Node* empty_value) const { return IsReal() ? GetReal() : empty_value; }
...
public:
    ...
    ZoneVector<MachineType> const* machine_types() const { return machine_types_; }
...
public:
    explicit SLVerifierHintParameters(const Operator* semantics, base::Optional<Type> override_output_type)
    ...
    const base::Optional<Type>& override_output_type() const { return override_output_type_; }
...
public:
    ...
    const Operator* SLVerifierHint(const Operator* semantics, const base::Optional<Type>& override_output_type);
...
public:
    ...
    const FrameStateInfo& frame_state_info() const { return FrameStateInfoOf(node()->op()); }
...
public:
    ...
    static constexpr int OutputArityForFormalParameterCount(int argc)
    {
        ...
        static_assert(kClosure + kNewTarget + kArgCount + kContext == kExtraOutputCount);
    ...
    int NewTargetParameterIndex() const { return Linkage::GetJSCallNewTargetParamIndex(FormalParameterCount()); }
    int ArgCountParameterIndex() const { return Linkage::GetJSCallArgCountParamIndex(FormalParameterCount()); }
    int ContextParameterIndex() const { return Linkage::GetJSCallContextParamIndex(FormalParameterCount()); }
    ...
    int LastParameterIndex_MaybeNonStandardLayout() const { return ContextParameterIndex_MaybeNonStandardLayout(); }
    ...
    int NewTargetOutputIndex() const
    {
        ...
        DCHECK_EQ(Linkage::GetJSCallNewTargetParamIndex(FormalParameterCount()) + 1, node()->op()->ValueOutputCount() - 3);
    ...
    int ArgCountOutputIndex() const
    {
        ...
        // at -1).
        // TODO(jgruber): Consider starting at 0.
        DCHECK_EQ(Linkage::GetJSCallArgCountParamIndex(FormalParameterCount()) + 1, node()->op()->ValueOutputCount() - 2);
        return node()->op()->ValueOutputCount() - 2;
    }
    int ContextOutputIndex() const
    {
        ...
        // at -1).
        // TODO(jgruber): Consider starting at 0.
        DCHECK_EQ(Linkage::GetJSCallContextParamIndex(FormalParameterCount()) + 1, node()->op()->ValueOutputCount() - 1);
