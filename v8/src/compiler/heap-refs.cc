...
bool IsReadOnlyHeapObjectForCompiler(PtrComprCageBase cage_base, HeapObject object)
{
    ...
    // builtins.
    return (object.IsCode(cage_base) && Code::cast(object).is_builtin()) || ReadOnlyHeap::Contains(object);
}
...
public:
...
#endif // DEBUG
    {
        ...
        *storage = this;
        ...
        CHECK_IMPLIES(broker->mode() == JSHeapBroker::kDisabled || broker->mode() == JSHeapBroker::kSerializing, isolate->handle_scope_data()->canonical_scope != nullptr);
        ...
        CHECK_IMPLIES(kind == kUnserializedReadOnlyHeapObject, object->IsHeapObject() && IsReadOnlyHeapObjectForCompiler(isolate, HeapObject::cast(*object)));
    }
...
#undef DECLARE_AS
    ...
    bool should_access_heap() const { return kind_ == kUnserializedHeapObject || kind_ == kNeverSerializedHeapObject || kind_ == kUnserializedReadOnlyHeapObject; }
...
public:
    HeapObjectData(JSHeapBroker* broker, ObjectData** storage, Handle<HeapObject> object, ObjectDataKind kind);
...
public:
    PropertyCellData(JSHeapBroker* broker, ObjectData** storage, Handle<PropertyCell> object, ObjectDataKind kind);
...
ZoneVector<Address> GetCFunctions(FixedArray function_overloads, Zone* zone)
{
    ...
    for (int i = 0; i < len; i++) {
        c_functions[i] = v8::ToCData<Address>(function_overloads.get(FunctionTemplateInfo::kFunctionOverloadEntrySize * i));
    }
    return c_functions;
}

ZoneVector<const CFunctionInfo*> GetCSignatures(FixedArray function_overloads, Zone* zone)
{
    const int len = function_overloads.length() / FunctionTemplateInfo::kFunctionOverloadEntrySize;
    ZoneVector<const CFunctionInfo*> c_signatures = ZoneVector<const CFunctionInfo*>(len, zone);
    for (int i = 0; i < len; i++) {
        c_signatures[i] = v8::ToCData<const CFunctionInfo*>(function_overloads.get(FunctionTemplateInfo::kFunctionOverloadEntrySize * i + 1));
    }
    return c_signatures;
}
...
PropertyCellData::PropertyCellData(JSHeapBroker* broker, ObjectData** storage, Handle<PropertyCell> object, ObjectDataKind kind)
...
public:
    JSReceiverData(JSHeapBroker* broker, ObjectData** storage, Handle<JSReceiver> object, ObjectDataKind kind)
        : HeapObjectData(broker, storage, object, kind)
    {
    }
};
...
public:
    JSObjectData(JSHeapBroker* broker, ObjectData** storage, Handle<JSObject> object, ObjectDataKind kind)
...
base::Optional<ObjectRef> GetOwnFastDataPropertyFromHeap(JSHeapBroker* broker, JSObjectRef holder, Representation representation, FieldIndex field_index)
{
    base::Optional<Object> constant;
    {
        ...
        if (field_index.is_inobject()) {
            ...
            Object raw_properties_or_hash = holder.object()->raw_properties_or_hash(cage_base, kRelaxedLoad);
        ...
        if (!constant->FitsRepresentation(representation, false)) {
            const char* repString = constant->IsSmi() ? "Smi" : constant->IsHeapNumber() ? "HeapNumber" : "HeapObject";
    ...
    Handle<Object> possibly_wrapped = Object::WrapForRead<AllocationType::kOld>(broker->local_isolate_or_isolate(), value, representation);
...
base::Optional<ObjectRef> GetOwnDictionaryPropertyFromHeap(JSHeapBroker* broker, Handle<JSObject> receiver, InternalIndex dict_index)
{
    Handle<Object> constant;
    {
        ...
        // object.
        base::Optional<Object> maybe_constant = JSObject::DictionaryPropertyAt(receiver, dict_index, broker->isolate()->heap());
...
public:
    JSTypedArrayData(JSHeapBroker* broker, ObjectData** storage, Handle<JSTypedArray> object, ObjectDataKind kind)
...
public:
    JSDataViewData(JSHeapBroker* broker, ObjectData** storage, Handle<JSDataView> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
    }
};
...
public:
    JSBoundFunctionData(JSHeapBroker* broker, ObjectData** storage, Handle<JSBoundFunction> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
    }
};
...
public:
    JSFunctionData(JSHeapBroker* broker, ObjectData** storage, Handle<JSFunction> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
        Cache(broker);
    }
    ...
    bool has_used_field(UsedField used_field) const { return (used_fields_ & used_field) != 0; }
...
public:
    BigIntData(JSHeapBroker* broker, ObjectData** storage, Handle<BigInt> object, ObjectDataKind kind)
        : HeapObjectData(broker, storage, object, kind)
...
public:
    MapData(JSHeapBroker* broker, ObjectData** storage, Handle<Map> object, ObjectDataKind kind);
    ...
    bool is_abandoned_prototype_map() const { return is_abandoned_prototype_map_; }
...
int InstanceSizeWithMinSlack(JSHeapBroker* broker, MapRef map)
{
...

    {
        ...
        TransitionsAccessor(broker->isolate(), *map.object(), kConcurrentAccess).TraverseTransitionTree([&](Map m) { maps.push_back(broker->CanonicalPersistentHandle(m)); });
    }
...
void JSFunctionData::Cache(JSHeapBroker* broker)
{
    ...
    if (function->has_prototype_slot()) {
        ...
        if (has_initial_map_) {
            ...
            instance_prototype_ = MakeRefAssumeMemoryFence(broker, Handle<Map>::cast(initial_map_->object())->prototype()).data();
        } else if (prototype_or_initial_map_->IsHeapObject() && !Handle<HeapObject>::cast(prototype_or_initial_map_->object())->IsTheHole()) {
            has_instance_prototype_ = true;
...
bool JSFunctionData::IsConsistentWithHeapState(JSHeapBroker* broker) const
{
    ...
    if (has_initial_map()) {
        ...
        if (has_used_field(kInitialMapInstanceSizeWithMinSlack) && initial_map_instance_size_with_min_slack_ != f->ComputeInstanceSizeWithMinSlack(f->GetIsolate())) {
...
HeapObjectData::HeapObjectData(JSHeapBroker* broker, ObjectData** storage, Handle<HeapObject> object, ObjectDataKind kind)
...
base::Optional<bool> HeapObjectData::TryGetBooleanValue(JSHeapBroker* broker) const
{
    // Keep in sync with Object::BooleanValue.
    auto result = TryGetBooleanValueImpl(broker);
    DCHECK_IMPLIES(broker->IsMainThread() && result.has_value(), result.value() == object()->BooleanValue(broker->isolate()));
    return result;
}

base::Optional<bool> HeapObjectData::TryGetBooleanValueImpl(JSHeapBroker* broker) const
{
    DisallowGarbageCollection no_gc;
    Object o = *object();
    Isolate* isolate = broker->isolate();
...
bool IsReadOnlyLengthDescriptor(Isolate* isolate, Handle<Map> jsarray_map)
{
    ...
    DescriptorArray descriptors = jsarray_map->instance_descriptors(isolate, kRelaxedLoad);
...
{
    return map->instance_type() == JS_ARRAY_TYPE && IsFastElementsKind(map->elements_kind()) && map->prototype().IsJSArray()
        && broker->IsArrayOrObjectPrototype(broker->CanonicalPersistentHandle(JSArray::cast(map->prototype())));
}
...
MapData::MapData(JSHeapBroker* broker, ObjectData** storage, Handle<Map> object, ObjectDataKind kind)
    : HeapObjectData(broker, storage, object, kind)
{
    ...
    in_object_properties_ = object->IsJSObjectMap() ? object->GetInObjectProperties() : 0;
}

class FixedArrayBaseData : public HeapObjectData {
public:
    FixedArrayBaseData(JSHeapBroker* broker, ObjectData** storage, Handle<FixedArrayBase> object, ObjectDataKind kind)
        : HeapObjectData(broker, storage, object, kind)
...
public:
    FixedArrayData(JSHeapBroker* broker, ObjectData** storage, Handle<FixedArray> object, ObjectDataKind kind)
...
public:
    ScriptContextTableData(JSHeapBroker* broker, ObjectData** storage, Handle<ScriptContextTable> object, ObjectDataKind kind)
...
public:
    JSArrayData(JSHeapBroker* broker, ObjectData** storage, Handle<JSArray> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
    }
};
...
public:
    JSGlobalObjectData(JSHeapBroker* broker, ObjectData** storage, Handle<JSGlobalObject> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
    }
};
...
public:
    JSGlobalProxyData(JSHeapBroker* broker, ObjectData** storage, Handle<JSGlobalProxy> object, ObjectDataKind kind)
        : JSObjectData(broker, storage, object, kind)
    {
    }
};
...
bool ObjectRef::equals(const ObjectRef& other) const { return data_ == other.data_; }
...
ObjectData* JSHeapBroker::TryGetOrCreateData(Handle<Object> object, GetOrCreateDataFlags flags)
{
    ...
    if (mode() == JSHeapBroker::kDisabled) {
        ...
        if (*storage == nullptr) {
            entry->value = zone()->New<ObjectData>(this, storage, object, object->IsSmi() ? kSmi : kUnserializedHeapObject);
        }
        return *storage;
    }
    ...
    if ((flags & kAssumeMemoryFence) == 0 && ObjectMayBeUninitialized(HeapObject::cast(*object))) {
...
#define CREATE_DATA(Name)                                                                                                                                                          \
    if (object->Is##Name()) {                                                                                                                                                      \
        entry = refs_->LookupOrInsert(object.address());                                                                                                                           \
        object_data = zone()->New<ref_traits<Name>::data_type>(this, &entry->value, Handle<Name>::cast(object), ObjectDataKindFor(ref_traits<Name>::ref_serialization_kind));      \
...
#define DEF_TESTER(Type, ...)                                                                                                                                                      \
    bool MapRef::Is##Type##Map() const { return InstanceTypeChecker::Is##Type(instance_type()); }
...
bool MapRef::CanInlineElementAccess() const
{
    ...
    if (IsFastElementsKind(kind))
        return true;
    if (IsSharedArrayElementsKind(kind))
        return true;
    if (IsTypedArrayElementsKind(kind) && kind != BIGUINT64_ELEMENTS && kind != BIGINT64_ELEMENTS) {
        return true;
    }
    return false;
}
...
base::Optional<MapRef> MapRef::AsElementsKind(ElementsKind kind) const
{
    ...
    base::Optional<Map> maybe_result = Map::TryAsElementsKind(broker()->isolate(), object(), kind, ConcurrencyMode::kConcurrent);
    ...
    if (equals(native_context.GetInitialJSArrayMap(current_kind))) {
        CHECK_EQ(Map::TryAsElementsKind(broker()->isolate(), object(), kind, ConcurrencyMode::kConcurrent).value(), *native_context.GetInitialJSArrayMap(kind).object());
    }
#endif // DEBUG
...
bool MapRef::HasOnlyStablePrototypesWithFastElements(ZoneVector<MapRef>* prototype_maps)
{
    DCHECK_NOT_NULL(prototype_maps);
    MapRef prototype_map = prototype().map();
    while (prototype_map.oddball_type() != OddballType::kNull) {
        if (!prototype_map.IsJSObjectMap() || !prototype_map.is_stable() || !IsFastElementsKind(prototype_map.elements_kind())) {
            return false;
        }
...
bool MapRef::supports_fast_array_iteration() const { return SupportsFastArrayIteration(broker(), object()); }

bool MapRef::supports_fast_array_resize() const { return SupportsFastArrayResize(broker(), object()); }

namespace {

void RecordConsistentJSFunctionViewDependencyIfNeeded(const JSHeapBroker* broker, const JSFunctionRef& ref, JSFunctionData* data, JSFunctionData::UsedField used_field)
{
...
base::Optional<FeedbackVectorRef> JSFunctionRef::feedback_vector(CompilationDependencies* dependencies) const { return raw_feedback_cell(dependencies).feedback_vector(); }

int JSFunctionRef::InitialMapInstanceSizeWithMinSlack(CompilationDependencies* dependencies) const
{
...
OddballType MapRef::oddball_type() const
{
    ...
    DCHECK(equals(MakeRef(broker(), f->termination_exception_map())) || equals(MakeRef(broker(), f->arguments_marker_map())) || equals(MakeRef(broker(), f->optimized_out_map()))
        || equals(MakeRef(broker(), f->stale_register_map())));
    return OddballType::kOther;
}

FeedbackCellRef FeedbackVectorRef::GetClosureFeedbackCell(int index) const { return MakeRefAssumeMemoryFence(broker(), object()->closure_feedback_cell(index)); }

base::Optional<ObjectRef> JSObjectRef::raw_properties_or_hash() const { return TryMakeRef(broker(), object()->raw_properties_or_hash()); }

base::Optional<ObjectRef> JSObjectRef::RawInobjectPropertyAt(FieldIndex index) const
{
    ...
    Handle<Object> value;
    {
        ...
        base::Optional<Object> maybe_value = object()->RawInobjectPropertyAt(cage_base, current_map, index);
...
bool JSObjectRef::IsElementsTenured(const FixedArrayBaseRef& elements) { return !ObjectInYoungGeneration(*elements.object()); }
...
int MapRef::GetInObjectPropertyOffset(int i) const { return object()->GetInObjectPropertyOffset(i); }

PropertyDetails MapRef::GetPropertyDetails(InternalIndex descriptor_index) const
{
    CHECK_LT(descriptor_index.as_int(), NumberOfOwnDescriptors());
...
bool MapRef::IsPrimitiveMap() const { return instance_type() <= LAST_PRIMITIVE_HEAP_OBJECT_TYPE; }
...
MapRef MapRef::FindFieldOwner(InternalIndex descriptor_index) const
{
    ...
    return MakeRefAssumeMemoryFence(broker(), object()->FindFieldOwner(broker()->isolate(), descriptor_index));
}

base::Optional<ObjectRef> StringRef::GetCharAsStringOrUndefined(uint32_t index) const
{
    String maybe_char;
...
bool StringRef::SupportedStringKind() const { return IsInternalizedString() || object()->IsThinString(); }

bool StringRef::IsContentAccessible() const { return data_->kind() != kNeverSerializedHeapObject || SupportedStringKind(); }
...
int ArrayBoilerplateDescriptionRef::constants_elements_length() const { return object()->constant_elements().length(); }
...
Handle<ByteArray> BytecodeArrayRef::SourcePositionTable() const { return broker()->CanonicalPersistentHandle(object()->SourcePositionTable()); }

Address BytecodeArrayRef::handler_table_address() const { return reinterpret_cast<Address>(object()->handler_table().GetDataStartAddress()); }

int BytecodeArrayRef::handler_table_size() const { return object()->handler_table().length(); }
...
#define HEAP_ACCESSOR_B(holder, field, name, BitField)                                                                                                                             \
    typename BitField::FieldType holder##Ref::name() const { return object()->name(); }

ObjectRef AllocationSiteRef::nested_site() const { return MakeRefAssumeMemoryFence(broker(), object()->nested_site()); }
...
int BytecodeArrayRef::register_count() const { return object()->register_count(); }
int BytecodeArrayRef::parameter_count() const { return object()->parameter_count(); }
interpreter::Register BytecodeArrayRef::incoming_new_target_or_generator_register() const { return object()->incoming_new_target_or_generator_register(); }

BIMODAL_ACCESSOR(HeapObject, Map, map)

HEAP_ACCESSOR_C(HeapNumber, double, value)

uint64_t HeapNumberRef::value_as_bits() const { return object()->value_as_bits(kRelaxedLoad); }
...
bool FunctionTemplateInfoRef::is_signature_undefined() const { return object()->signature().IsUndefined(broker()->isolate()); }
...
HolderLookupResult FunctionTemplateInfoRef::LookupHolderOfExpectedType(MapRef receiver_map)
{
...
ObjectRef CallHandlerInfoRef::data() const { return MakeRefAssumeMemoryFence(broker(), object()->data()); }
...
ScopeInfoRef ScopeInfoRef::OuterScopeInfo() const { return MakeRefAssumeMemoryFence(broker(), object()->OuterScopeInfo()); }
...
SharedFunctionInfo::Inlineability SharedFunctionInfoRef::GetInlineability() const
{
...
base::Optional<ObjectRef> MapRef::GetStrongValue(InternalIndex descriptor_index) const
{
    CHECK_LT(descriptor_index.as_int(), NumberOfOwnDescriptors());
    return instance_descriptors().GetStrongValue(descriptor_index);
}

DescriptorArrayRef MapRef::instance_descriptors() const { return MakeRefAssumeMemoryFence(broker(), object()->instance_descriptors(broker()->isolate(), kAcquireLoad)); }

HeapObjectRef MapRef::prototype() const { return MakeRefAssumeMemoryFence(broker(), HeapObject::cast(object()->prototype())); }

MapRef MapRef::FindRootMap() const
{
    // TODO(solanes, v8:7790): Consider caching the result of the root map.
    return MakeRefAssumeMemoryFence(broker(), object()->FindRootMap(broker()->isolate()));
}
...
{
    // Immutable after initialization.
    return MakeRefAssumeMemoryFence(broker(), HeapObject::cast(object()->GetBackPointer()));
}
...
bool MapRef::IsInobjectSlackTrackingInProgress() const { return object()->IsInobjectSlackTrackingInProgress(); }

int MapRef::constructor_function_index() const { return object()->GetConstructorFunctionIndex(); }
...
int MapRef::GetInObjectPropertiesStartInWords() const { return object()->GetInObjectPropertiesStartInWords(); }
...
bool StringRef::IsExternalString() const { return object()->IsExternalString(); }

Address CallHandlerInfoRef::callback() const { return object()->callback(); }

ZoneVector<Address> FunctionTemplateInfoRef::c_functions() const { return GetCFunctions(FixedArray::cast(object()->GetCFunctionOverloads()), broker()->zone()); }

ZoneVector<const CFunctionInfo*> FunctionTemplateInfoRef::c_signatures() const { return GetCSignatures(FixedArray::cast(object()->GetCFunctionOverloads()), broker()->zone()); }
...
MapRef NativeContextRef::GetFunctionMapFromIndex(int index) const
{
    ...
    return MakeRefAssumeMemoryFence(broker(), Map::cast(object()->get(index, kAcquireLoad)));
}
...
#define DEF_NATIVE_CONTEXT_ACCESSOR(ResultType, Name)                                                                                                                              \
    ResultType##Ref NativeContextRef::Name() const { return MakeRefAssumeMemoryFence(broker(), ResultType::cast(object()->Name(kAcquireLoad))); }
BROKER_NATIVE_CONTEXT_FIELDS(DEF_NATIVE_CONTEXT_ACCESSOR)
#undef DEF_NATIVE_CONTEXT_ACCESSOR

base::Optional<JSFunctionRef> NativeContextRef::GetConstructorFunction(const MapRef& map) const
{
...
bool ObjectRef::IsTheHole() const { return IsHeapObject() && AsHeapObject().map().oddball_type() == OddballType::kHole; }
...
Maybe<double> ObjectRef::OddballToNumber() const
{
    ...
    case OddballType::kBoolean: {
        ObjectRef true_ref = MakeRef<Object>(broker(), broker()->isolate()->factory()->true_value());
...
bool ObjectRef::should_access_heap() const { return data()->should_access_heap(); }

base::Optional<ObjectRef> JSObjectRef::GetOwnConstantElement(const FixedArrayBaseRef& elements_ref, uint32_t index, CompilationDependencies* dependencies) const
{
    base::Optional<Object> maybe_element = GetOwnConstantElementFromHeap(*elements_ref.object(), map().elements_kind(), index);
...
base::Optional<Object> JSObjectRef::GetOwnConstantElementFromHeap(FixedArrayBase elements, ElementsKind elements_kind, uint32_t index) const
{
    ...
    //   of `length` below.
    if (holder->IsJSArray()) {
        Object array_length_obj = JSArray::cast(*holder).length(broker()->isolate(), kRelaxedLoad);
    ...
    auto result = ConcurrentLookupIterator::TryGetOwnConstantElement(&maybe_element, broker()->isolate(), broker()->local_isolate(), *holder, elements, elements_kind, index);
...
base::Optional<ObjectRef> JSObjectRef::GetOwnFastDataProperty(Representation field_representation, FieldIndex index, CompilationDependencies* dependencies) const
{
    base::Optional<ObjectRef> result = GetOwnFastDataPropertyFromHeap(broker(), *this, field_representation, index);
    if (result.has_value()) {
        dependencies->DependOnOwnConstantDataProperty(*this, map(), field_representation, index, *result);
    }
    return result;
}

base::Optional<ObjectRef> JSObjectRef::GetOwnDictionaryProperty(InternalIndex index, CompilationDependencies* dependencies) const
{
    CHECK(index.is_found());
    base::Optional<ObjectRef> result = GetOwnDictionaryPropertyFromHeap(broker(), object(), index);
    if (result.has_value()) {
...
base::Optional<ObjectRef> JSArrayRef::length_unsafe() const { return TryMakeRef(broker(), object()->length(broker()->isolate(), kRelaxedLoad)); }

base::Optional<ObjectRef> JSArrayRef::GetOwnCowElement(FixedArrayBaseRef elements_ref, uint32_t index) const
{
    ...
    if (!elements_ref.map().IsFixedCowArrayMap())
        return {};
...
base::Optional<CellRef> SourceTextModuleRef::GetCell(int cell_index) const { return TryMakeRef(broker(), object()->GetCell(cell_index)); }

base::Optional<ObjectRef> SourceTextModuleRef::import_meta() const { return TryMakeRef(broker(), object()->import_meta(kAcquireLoad)); }
...
HeapObjectType HeapObjectRef::GetHeapObjectType() const
{
    if (data_->should_access_heap()) {
        ...
        return HeapObjectType(map.instance_type(), flags, GetOddballType(broker()->isolate(), map));
    }
    HeapObjectType::Flags flags(0);
    if (map().is_undetectable())
        flags |= HeapObjectType::kUndetectable;
...
base::Optional<FixedArrayBaseRef> JSObjectRef::elements(RelaxedLoadTag tag) const { return TryMakeRef(broker(), object()->elements(tag)); }
...
PropertyDetails DescriptorArrayRef::GetPropertyDetails(InternalIndex descriptor_index) const { return object()->GetDetails(descriptor_index); }

NameRef DescriptorArrayRef::GetPropertyKey(InternalIndex descriptor_index) const
{
...
base::Optional<ObjectRef> DescriptorArrayRef::GetStrongValue(InternalIndex descriptor_index) const
{
...
base::Optional<SharedFunctionInfoRef> FeedbackCellRef::shared_function_info() const
{
...
#define DEF_OBJECT_GETTER(T)                                                                                                                                                       \
    Handle<T> T##Ref::object() const { return Handle<T>(reinterpret_cast<Address*>(data_->object().address())); }
#else
#define DEF_OBJECT_GETTER(T)                                                                                                                                                       \
    Handle<T> T##Ref::object() const { return Handle<T>(reinterpret_cast<Address*>(data_->object().address())); }
#endif // DEBUG
...
template <class T> typename TinyRef<T>::RefType TinyRef<T>::AsRef(JSHeapBroker* broker) const
{
    if (data_->kind() == kUnserializedHeapObject && broker->mode() != JSHeapBroker::kDisabled) {
        // Gotta reconstruct to avoid returning a stale unserialized ref.
        return MakeRefAssumeMemoryFence<T>(broker, Handle<T>::cast(data_->object()));
    }
    return TryMakeRef<T>(broker, data_).value();
}

template <class T> Handle<T> TinyRef<T>::object() const { return Handle<T>::cast(data_->object()); }
...
#define JSFUNCTION_BIMODAL_ACCESSOR_WITH_DEP(Result, Name, UsedField)                                                                                                              \
    Result##Ref JSFunctionRef::Name(CompilationDependencies* dependencies) const                                                                                                   \
    {                                                                                                                                                                              \
        IF_ACCESS_FROM_HEAP(Result, Name);                                                                                                                                         \
...
JSFUNCTION_BIMODAL_ACCESSOR_WITH_DEP(Object, instance_prototype, JSFunctionData::kInstancePrototype)
...
NativeContextRef JSFunctionRef::native_context() const { return MakeRefAssumeMemoryFence(broker(), context().object()->native_context()); }

base::Optional<FunctionTemplateInfoRef> SharedFunctionInfoRef::function_template_info() const
{
...
int SharedFunctionInfoRef::context_header_size() const { return object()->scope_info().ContextHeaderLength(); }

int SharedFunctionInfoRef::context_parameters_start() const { return object()->scope_info().ParametersStartIndex(); }

ScopeInfoRef SharedFunctionInfoRef::scope_info() const { return MakeRefAssumeMemoryFence(broker(), object()->scope_info(kAcquireLoad)); }
...
base::Optional<MapRef> JSObjectRef::GetObjectCreateMap() const
{
    ...
    Handle<Object> maybe_proto_info = broker()->CanonicalPersistentHandle(map_handle->prototype_info(kAcquireLoad));
    if (!maybe_proto_info->IsPrototypeInfo())
        return {};

    MaybeObject maybe_object_create_map = Handle<PrototypeInfo>::cast(maybe_proto_info)->object_create_map(kAcquireLoad);
    if (!maybe_object_create_map->IsWeak())
        return {};

    return MapRef(broker(), broker()->GetOrCreateData(maybe_object_create_map->GetHeapObjectAssumeWeak(), kAssumeMemoryFence));
}
...
base::Optional<PropertyCellRef> JSGlobalObjectRef::GetPropertyCell(NameRef const& name) const
{
    base::Optional<PropertyCell> maybe_cell = ConcurrentLookupIterator::TryGetPropertyCell(
        broker()->isolate(), broker()->local_isolate_or_isolate(), broker()->target_native_context().global_object().object(), name.object());
...
unsigned CodeRef::GetInlinedBytecodeSize() const { return GetInlinedBytecodeSizeImpl(*object()); }
