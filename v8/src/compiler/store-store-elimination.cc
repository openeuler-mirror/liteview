...
#ifdef DEBUG
#define DCHECK_EXTRA(condition, fmt, ...) CHECK_EXTRA(condition, fmt, ##__VA_ARGS__)
#else
...
public:
    ...
    UnobservablesSet& operator=(const UnobservablesSet& other) V8_NOEXCEPT = default;

    // Computes the intersection of two UnobservablesSets. If one of the sets is
    // empty, will return empty.
    UnobservablesSet Intersect(const UnobservablesSet& other, const UnobservablesSet& empty, Zone* zone) const;
    ...
    bool IsEmpty() const
    {
        if (IsUnvisited())
            return true;
        return unobservable_->begin() == unobservable_->end() && gc_observable_->begin() == gc_observable_->end();
    }
    ...
    bool IsUnobservable(NodeId node, StoreOffset offset, bool maybe_initializing_or_transitioning) const
    {
    ...
    bool IsGCObservable(NodeId node, StoreOffset offset) const { return gc_observable_ != nullptr && gc_observable_->Get(offset).Get(node); }
    ...
    bool operator==(const UnobservablesSet& other) const
    {
        if (IsUnvisited() || other.IsUnvisited()) {
            ...
            return *unobservable_ == *(other.unobservable_) && *gc_observable_ == *other.gc_observable_;
        }
    }

    bool operator!=(const UnobservablesSet& other) const { return !(*this == other); }

private:
    UnobservablesSet() = default;
    explicit UnobservablesSet(const SetT* unobservable, const SetT* gc_observable)
        : unobservable_(unobservable)
        , gc_observable_(gc_observable)
    {
    }

    static SetT* NewSet(Zone* zone) { return zone->New<UnobservablesSet::SetT>(zone, InnerSetT(zone)); }
...
public:
    ...
    RedundantStoreFinder(JSGraph* js_graph, TickCounter* tick_counter, Zone* temp_zone)
        : jsgraph_(js_graph)
        , tick_counter_(tick_counter)
        , temp_zone_(temp_zone)
        , revisit_(temp_zone)
        , in_revisit_(js_graph->graph()->NodeCount(), temp_zone)
        , unobservable_(js_graph->graph()->NodeCount(), UnobservablesSet::Unvisited(), temp_zone)
...
void RedundantStoreFinder::Find()
{
    ...
    for (Node* node : all.reachable) {
        if (node->op()->opcode() == IrOpcode::kStoreField) {
            DCHECK_EXTRA(HasBeenVisited(node), "#%d:%s", node->id(), node->op()->mnemonic());
        }
    }
#endif
}
...
bool RedundantStoreFinder::HasBeenVisited(Node* node) { return !unobservable_for_id(node->id()).IsUnvisited(); }

UnobservablesSet RedundantStoreFinder::RecomputeSet(Node* node, const UnobservablesSet& uses)
{
    ...
    case IrOpcode::kStoreField: {
        ...
        const bool is_not_observable = uses.IsUnobservable(stored_to->id(), offset, access.maybe_initializing_or_transitioning_store);

        if (is_not_observable) {
            TRACE("  #%d is StoreField[+%d,%s](#%d), unobservable", node->id(), offset, MachineReprToString(access.machine_type.representation()), stored_to->id());
            to_remove().insert(node);
            return uses;
        } else {
            const bool is_gc_observable = access.maybe_initializing_or_transitioning_store && uses.IsGCObservable(stored_to->id(), offset);
            ...
            TRACE("  #%d is StoreField[+%d,%s](#%d), observable%s, recording in set", node->id(), offset, MachineReprToString(access.machine_type.representation()),
    ...
    case IrOpcode::kLoadField: {
        ...
        TRACE("  #%d is LoadField[+%d,%s](#%d), removing all offsets [+%d] from "
              "set",
            node->id(), offset, MachineReprToString(access.machine_type.representation()), loaded_from->id(), offset);
    ...
    case IrOpcode::kAllocateRaw: {
        ...
        // stores.
        TRACE("  #%d is Allocate or AllocateRaw, marking recorded offsets as "
              "observable by GC",
            node->id());
    ...
    default:
        if (CannotObserveStoreField(node)) {
            TRACE("  #%d:%s can observe nothing, set stays unchanged", node->id(), node->op()->mnemonic());
            return uses;
        } else {
            TRACE("  #%d:%s might observe anything, recording empty set", node->id(), node->op()->mnemonic());
...
bool RedundantStoreFinder::CannotObserveStoreField(Node* node)
{
    ...
    constexpr uint8_t cannot_observe = Operator::kNoRead | Operator::kNoWrite | Operator::kNoThrow | Operator::kNoDeopt;
    return ((node->op()->properties() & cannot_observe) == cannot_observe) || opcode == IrOpcode::kLoadElement || opcode == IrOpcode::kLoad || opcode == IrOpcode::kLoadImmutable
        || opcode == IrOpcode::kStore || opcode == IrOpcode::kStoreElement || opcode == IrOpcode::kBitcastWordToTagged || opcode == IrOpcode::kBitcastTaggedToWord;
}
...
void RedundantStoreFinder::VisitEffectfulNode(Node* node)
{
    ...
    bool cur_set_changed = stores_for_node.IsUnvisited() || stores_for_node != before_set;
    if (!cur_set_changed) {
        ...
        for (int i = 0; i < node->op()->EffectInputCount(); i++) {
            ...
            TRACE("    marking #%d:%s for revisit", input->id(), input->op()->mnemonic());
...
UnobservablesSet RedundantStoreFinder::RecomputeUseIntersection(Node* node)
{
    ...
    if (node->op()->EffectOutputCount() == 0) {
        ...
        DCHECK_EXTRA(
            opcode == IrOpcode::kReturn || opcode == IrOpcode::kTerminate || opcode == IrOpcode::kDeoptimize || opcode == IrOpcode::kThrow || opcode == IrOpcode::kTailCall,
    ...
    for (Edge edge : node->use_edges()) {
        ...
        if (first) {
            ...
            cur_set = cur_set.Intersect(new_set, unobservables_visited_empty_, temp_zone());
        }
...
UnobservablesSet UnobservablesSet::VisitedEmpty(Zone* zone) { return UnobservablesSet(NewSet(zone), NewSet(zone)); }

UnobservablesSet UnobservablesSet::Intersect(const UnobservablesSet& other, const UnobservablesSet& empty, Zone* zone) const
{
    ...
    for (auto [offset, inner_unobservable1, inner_unobservable2] : unobservable_->Zip(*other.unobservable_)) {
        for (auto [node, value1, value2] : inner_unobservable1.Zip(inner_unobservable2)) {
            if (value1 && value2) {
                new_unobservable->Modify(offset, [node = node](InnerSetT* inner) { inner->Set(node, true); });
            }
        }
    }

    for (auto [offset, inner_gc_observable1, inner_gc_observable2] : gc_observable_->Zip(*other.gc_observable_)) {
        for (auto [node, value, other_value] : inner_gc_observable1.Zip(inner_gc_observable2)) {
            if ((value || unobservable_->Get(offset).Get(node)) && (other_value || other.unobservable_->Get(offset).Get(node))) {
                new_gc_observable->Modify(offset, [node = node](InnerSetT* inner) { inner->Set(node, true); });
            }
        }
    }
...
UnobservablesSet UnobservablesSet::Add(NodeId node, StoreOffset offset, Zone* zone) const
{
    if (unobservable_->Get(offset).Get(node))
        return *this;

    UnobservablesSet::SetT* new_unobservable = zone->New<SetT>(*unobservable_);
    new_unobservable->Modify(offset, [node](auto inner_set) { inner_set->Set(node, true); });

    return UnobservablesSet(new_unobservable, gc_observable_);
}

UnobservablesSet UnobservablesSet::RemoveSameOffset(StoreOffset offset, Zone* zone) const
{
    UnobservablesSet::SetT* new_unobservable = zone->New<SetT>(*unobservable_);
...
UnobservablesSet UnobservablesSet::MarkGCObservable(Zone* zone) const
{
    ...
    for (auto [offset, inner_unobservable] : *unobservable_) {
        new_gc_observable->Modify(offset, [inner_unobservable = inner_unobservable](InnerSetT* inner_gc_observable) {
...
// static
void StoreStoreElimination::Run(JSGraph* js_graph, TickCounter* tick_counter, Zone* temp_zone)
{
    ...
    for (Node* node : finder.to_remove_const()) {
        ...
        NodeProperties::ReplaceUses(node, nullptr, previous_effect, nullptr, nullptr);
        node->Kill();
    }
}
