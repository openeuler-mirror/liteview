...
public:
    ...
    template <typename T> static bool IsWordNAnd(const T& x) { return x.IsWord32And(); }
    template <typename T> static bool IsWordNShl(const T& x) { return x.IsWord32Shl(); }
    template <typename T> static bool IsWordNShr(const T& x) { return x.IsWord32Shr(); }
    template <typename T> static bool IsWordNSar(const T& x) { return x.IsWord32Sar(); }
    static bool IsWordNSarShiftOutZeros(const Operator* op) { return op->opcode() == IrOpcode::kWord32Sar && OpParameter<ShiftKind>(op) == ShiftKind::kShiftOutZeros; }
    template <typename T> static bool IsWordNXor(const T& x) { return x.IsWord32Xor(); }
    template <typename T> static bool IsIntNAdd(const T& x) { return x.IsInt32Add(); }
    template <typename T> static bool IsIntNMul(const T& x) { return x.IsInt32Mul(); }

    const Operator* IntNAdd(MachineOperatorBuilder* machine) { return machine->Int32Add(); }
...
public:
    ...
    template <typename T> static bool IsWordNAnd(const T& x) { return x.IsWord64And(); }
    template <typename T> static bool IsWordNShl(const T& x) { return x.IsWord64Shl(); }
    template <typename T> static bool IsWordNShr(const T& x) { return x.IsWord64Shr(); }
    template <typename T> static bool IsWordNSar(const T& x) { return x.IsWord64Sar(); }
    static bool IsWordNSarShiftOutZeros(const Operator* op) { return op->opcode() == IrOpcode::kWord64Sar && OpParameter<ShiftKind>(op) == ShiftKind::kShiftOutZeros; }
    template <typename T> static bool IsWordNXor(const T& x) { return x.IsWord64Xor(); }
    template <typename T> static bool IsIntNAdd(const T& x) { return x.IsInt64Add(); }
    template <typename T> static bool IsIntNMul(const T& x) { return x.IsInt64Mul(); }

    static const Operator* IntNAdd(MachineOperatorBuilder* machine) { return machine->Int64Add(); }
...
MachineOperatorReducer::MachineOperatorReducer(Editor* editor, MachineGraph* mcgraph, bool allow_signalling_nan)
...
Node* MachineOperatorReducer::Float32Constant(float value) { return graph()->NewNode(common()->Float32Constant(value)); }

Node* MachineOperatorReducer::Float64Constant(double value) { return mcgraph()->Float64Constant(value); }

Node* MachineOperatorReducer::Int32Constant(int32_t value) { return mcgraph()->Int32Constant(value); }

Node* MachineOperatorReducer::Int64Constant(int64_t value) { return graph()->NewNode(common()->Int64Constant(value)); }

Node* MachineOperatorReducer::Float64Mul(Node* lhs, Node* rhs) { return graph()->NewNode(machine()->Float64Mul(), lhs, rhs); }
...
Node* MachineOperatorReducer::Word32Equal(Node* lhs, Node* rhs) { return graph()->NewNode(machine()->Word32Equal(), lhs, rhs); }
...
Node* MachineOperatorReducer::Int32Mul(Node* lhs, Node* rhs) { return graph()->NewNode(machine()->Int32Mul(), lhs, rhs); }
...
Reduction MachineOperatorReducer::Reduce(Node* node)
{
    ...
    case IrOpcode::kFloat32Sub: {
        ...
        if (allow_signalling_nan_ && m.right().Is(0) && (std::copysign(1.0, m.right().ResolvedValue()) > 0)) {
    ...
    case IrOpcode::kFloat64Sub: {
        Float64BinopMatcher m(node);
        if (allow_signalling_nan_ && m.right().Is(0) && (base::Double(m.right().ResolvedValue()).Sign() > 0)) {
            return Replace(m.left().node()); // x - 0 => x
        }
        if (m.right().IsNaN()) { // x - NaN => NaN
            return ReplaceFloat64(SilenceNaN(m.right().ResolvedValue()));
            ...
            if (ElementSizeLog2Of(load_rep.representation()) == 2) {
                ...
                if (value_edges == 1) {
                    ...
                    NodeProperties::ChangeOp(
                        input, input->opcode() == IrOpcode::kLoad ? machine()->Load(LoadRepresentation::Int32()) : machine()->LoadImmutable(LoadRepresentation::Int32()));
...
Reduction MachineOperatorReducer::ReduceStore(Node* node)
{
    ...
    case IrOpcode::kWord32Sar: {
        ...
        if (m.left().IsWord32Shl()
            && ((rep == MachineRepresentation::kWord8 && m.right().IsInRange(1, 24)) || (rep == MachineRepresentation::kWord16 && m.right().IsInRange(1, 16)))) {
            Int32BinopMatcher mleft(m.left().node());
...
bool CanTruncate(int64_t value) { return value >= std::numeric_limits<int32_t>::min() && value <= std::numeric_limits<int32_t>::max(); }
...
template <typename WordNAdapter> Reduction MachineOperatorReducer::ReduceWordNAnd(Node* node)
{
    ...
    if (m.right().Is(1)) {
        ...
        while (left->opcode() == IrOpcode::kTruncateInt64ToInt32 || left->opcode() == IrOpcode::kChangeInt32ToInt64 || left->opcode() == IrOpcode::kChangeUint32ToUint64) {
            left = left->InputAt(0);
        }
...
private:
    template <typename WordNAdapter> static base::Optional<BitfieldCheck> TryDetectShiftAndMaskOneBit(Node* node)
    {
...
Reduction MachineOperatorReducer::ReduceWord32Equal(Node* node)
{
    ...
    if (m.right().HasResolvedValue()) {
        ...
        if (m.left().IsTruncateInt64ToInt32()) {
            replacements
                = ReduceWordEqualForConstantRhs<Word64Adapter, uint32_t>(NodeProperties::GetValueInput(m.left().node(), 0), static_cast<uint32_t>(m.right().ResolvedValue()));
        } else {
...
Reduction MachineOperatorReducer::ReduceFloat64InsertLowWord32(Node* node)
{
    ...
    if (mlhs.HasResolvedValue() && mrhs.HasResolvedValue()) {
        return ReplaceFloat64(base::bit_cast<double>((base::bit_cast<uint64_t>(mlhs.ResolvedValue()) & uint64_t { 0xFFFFFFFF00000000 }) | mrhs.ResolvedValue()));
    }
    return NoChange();
}
...
Reduction MachineOperatorReducer::ReduceFloat64InsertHighWord32(Node* node)
{
    ...
    if (mlhs.HasResolvedValue() && mrhs.HasResolvedValue()) {
        return ReplaceFloat64(
            base::bit_cast<double>((base::bit_cast<uint64_t>(mlhs.ResolvedValue()) & uint64_t { 0xFFFFFFFF }) | (static_cast<uint64_t>(mrhs.ResolvedValue()) << 32)));
    }
    return NoChange();
}
...
Reduction MachineOperatorReducer::ReduceFloat64Compare(Node* node)
{
    ...
    } else if ((m.left().IsChangeFloat32ToFloat64() && m.right().IsChangeFloat32ToFloat64()) || (m.left().IsChangeFloat32ToFloat64() && IsFloat64RepresentableAsFloat32(m.right()))
    ...
    if (auto simplified = TrySimplifyCompareZero(cond)) {
        ...
        if (simplified->is_inverted) {
            ...
            case IrOpcode::kBranch:
                ...
                break;
            case IrOpcode::kTrapIf:
                NodeProperties::ChangeOp(node, common()->TrapUnless(TrapIdOf(node->op())));
                break;
            case IrOpcode::kTrapUnless:
                NodeProperties::ChangeOp(node, common()->TrapIf(TrapIdOf(node->op())));
                break;
            case IrOpcode::kDeoptimizeIf: {
...
{
    DCHECK(node->opcode() == IrOpcode::kBranch || node->opcode() == IrOpcode::kDeoptimizeIf || node->opcode() == IrOpcode::kDeoptimizeUnless || node->opcode() == IrOpcode::kTrapIf
...
template <typename WordNAdapter> base::Optional<Node*> MachineOperatorReducer::ReduceConditionalN(Node* node)
{
    NodeMatcher condition(NodeProperties::GetValueInput(node, 0));
...
base::Optional<std::pair<Node*, uintN_t>> MachineOperatorReducer::ReduceWordEqualForConstantRhs(Node* lhs, uintN_t rhs)
{
...
CommonOperatorBuilder* MachineOperatorReducer::common() const { return mcgraph()->common(); }

MachineOperatorBuilder* MachineOperatorReducer::machine() const { return mcgraph()->machine(); }
