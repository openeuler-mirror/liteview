...
public:
    ...
    template <typename T> Node* PointerConstant(T* value) { return PointerConstant(base::bit_cast<intptr_t>(value)); }
    ...
    Node* Dead() { return Dead_ ? Dead_ : Dead_ = graph_->NewNode(common_->Dead()); }

    // Store and retrieve call count information.
    void StoreCallCount(NodeId call_id, int count) { call_counts_.Put(call_id, count); }
    ...
    void ReserveCallCounts(size_t num_call_instructions) { call_counts_.Reserve(num_call_instructions); }
