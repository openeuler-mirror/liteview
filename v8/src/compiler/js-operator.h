...
public:
    ...
    bool operator==(CallFrequency const& that) const { return base::bit_cast<uint32_t>(this->value_) == base::bit_cast<uint32_t>(that.value_); }
    bool operator!=(CallFrequency const& that) const { return !(*this == that); }

    friend size_t hash_value(CallFrequency const& f) { return base::bit_cast<uint32_t>(f.value_); }
...
public:
    ...
    bool operator==(ConstructForwardVarargsParameters const& that) const { return this->bit_field_ == that.bit_field_; }
    bool operator!=(ConstructForwardVarargsParameters const& that) const { return !(*this == that); }

private:
    friend size_t hash_value(ConstructForwardVarargsParameters const& p) { return p.bit_field_; }
...
public:
    ...
    int arity_without_implicit_args() const { return static_cast<int>(arity_ - kExtraConstructInputCount); }
...
public:
    ...
    bool operator==(CallForwardVarargsParameters const& that) const { return this->bit_field_ == that.bit_field_; }
    bool operator!=(CallForwardVarargsParameters const& that) const { return !(*this == that); }

private:
    friend size_t hash_value(CallForwardVarargsParameters const& p) { return p.bit_field_; }
...
public:
    ...
    CallParameters(size_t arity, CallFrequency const& frequency, FeedbackSource const& feedback, ConvertReceiverMode convert_mode, SpeculationMode speculation_mode,
    ...
    size_t arity() const { return ArityField::decode(bit_field_); }
    int arity_without_implicit_args() const { return static_cast<int>(arity() - kExtraCallInputCount); }

    CallFrequency const& frequency() const { return frequency_; }
    ConvertReceiverMode convert_mode() const { return ConvertReceiverModeField::decode(bit_field_); }
    FeedbackSource const& feedback() const { return feedback_; }

    SpeculationMode speculation_mode() const { return SpeculationModeField::decode(bit_field_); }

    CallFeedbackRelation feedback_relation() const { return CallFeedbackRelationField::decode(bit_field_); }

    bool operator==(CallParameters const& that) const { return this->bit_field_ == that.bit_field_ && this->frequency_ == that.frequency_ && this->feedback_ == that.feedback_; }
...
public:
    CreateFunctionContextParameters(const ScopeInfoRef& scope_info, int slot_count, ScopeType scope_type)
    ...
    ScopeInfoRef scope_info(JSHeapBroker* broker) const { return scope_info_.AsRef(broker); }
...
public:
    DefineNamedOwnPropertyParameters(const NameRef& name, FeedbackSource const& feedback)
        : name_(name)
        , feedback_(feedback)
    {
    }
...
public:
    NamedAccess(LanguageMode language_mode, const NameRef& name, FeedbackSource const& feedback)
        : name_(name)
        , feedback_(feedback)
...
public:
    LoadGlobalParameters(const NameRef& name, const FeedbackSource& feedback, TypeofMode typeof_mode)
        : name_(name)
        , feedback_(feedback)
...
public:
    StoreGlobalParameters(LanguageMode language_mode, const FeedbackSource& feedback, const NameRef& name)
        : language_mode_(language_mode)
        , name_(name)
        , feedback_(feedback)
    {
...
public:
    ...
    size_t arity() const { return arity_; }
    base::Optional<AllocationSiteRef> site(JSHeapBroker* broker) const { return AllocationSiteTinyRef::AsOptionalRef(broker, site_); }
...
public:
    CreateClosureParameters(const SharedFunctionInfoRef& shared_info, const CodeTRef& code, AllocationType allocation)
    ...
    SharedFunctionInfoRef shared_info(JSHeapBroker* broker) const { return shared_info_.AsRef(broker); }
...
public:
    GetTemplateObjectParameters(const TemplateObjectDescriptionRef& description, const SharedFunctionInfoRef& shared, FeedbackSource const& feedback)
    ...
    TemplateObjectDescriptionRef description(JSHeapBroker* broker) const { return description_.AsRef(broker); }
    SharedFunctionInfoRef shared(JSHeapBroker* broker) const { return shared_.AsRef(broker); }
    FeedbackSource const& feedback() const { return feedback_; }
...
public:
    CreateLiteralParameters(const HeapObjectRef& constant, FeedbackSource const& feedback, int length, int flags)
        : constant_(constant)
        , feedback_(feedback)
        , length_(length)
        , flags_(flags)
    ...
    HeapObjectRef constant(JSHeapBroker* broker) const { return constant_.AsRef(broker); }
    FeedbackSource const& feedback() const { return feedback_; }
...
public:
    explicit JSWasmCallParameters(const wasm::WasmModule* module, const wasm::FunctionSig* signature, FeedbackSource const& feedback)
        : module_(module)
...
ScopeInfoRef ScopeInfoOf(JSHeapBroker* broker, const Operator* op) V8_WARN_UNUSED_RESULT;
...
V8_EXPORT_PRIVATE std::ostream& operator<<(std::ostream&, ScopeInfoTinyRef const&);
...
public:
    ...
    const Operator* CreateArray(size_t arity, base::Optional<AllocationSiteRef> site);
    ...
    const Operator* CreateClosure(const SharedFunctionInfoRef& shared_info, const CodeTRef& code, AllocationType allocation = AllocationType::kYoung);
    ...
    const Operator* CreateLiteralArray(const ArrayBoilerplateDescriptionRef& constant, FeedbackSource const& feedback, int literal_flags, int number_of_elements);
    const Operator* CreateEmptyLiteralArray(FeedbackSource const& feedback);
    const Operator* CreateArrayFromIterable();
    const Operator* CreateEmptyLiteralObject();
    const Operator* CreateLiteralObject(const ObjectBoilerplateDescriptionRef& constant, FeedbackSource const& feedback, int literal_flags, int number_of_properties);
    const Operator* CloneObject(FeedbackSource const& feedback, int literal_flags);
    const Operator* CreateLiteralRegExp(const StringRef& constant_pattern, FeedbackSource const& feedback, int literal_flags);

    const Operator* GetTemplateObject(const TemplateObjectDescriptionRef& description, const SharedFunctionInfoRef& shared, FeedbackSource const& feedback);
...
#if V8_ENABLE_WEBASSEMBLY
    const Operator* CallWasm(const wasm::WasmModule* wasm_module, const wasm::FunctionSig* wasm_signature, FeedbackSource const& feedback);
#endif // V8_ENABLE_WEBASSEMBLY
    ...
    const Operator* LoadNamed(const NameRef& name, FeedbackSource const& feedback);
    const Operator* LoadNamedFromSuper(const NameRef& name, FeedbackSource const& feedback);
    ...
    const Operator* SetNamedProperty(LanguageMode language_mode, const NameRef& name, FeedbackSource const& feedback);

    const Operator* DefineNamedOwnProperty(const NameRef& name, FeedbackSource const& feedback);
    ...
    const Operator* LoadGlobal(const NameRef& name, const FeedbackSource& feedback, TypeofMode typeof_mode = TypeofMode::kNotInside);
    const Operator* StoreGlobal(LanguageMode language_mode, const NameRef& name, const FeedbackSource& feedback);
    ...
    const Operator* CreateFunctionContext(const ScopeInfoRef& scope_info, int slot_count, ScopeType scope_type);
...
public:
    ...
    FrameState frame_state() const { return FrameState { NodeProperties::GetFrameStateInput(node()) }; }
};

#define DEFINE_INPUT_ACCESSORS(Name, name, TheIndex, Type)                                                                                                                         \
    static constexpr int Name##Index() { return TheIndex; }                                                                                                                        \
    TNode<Type> name() const { return TNode<Type>::UncheckedCast(NodeProperties::GetValueInput(node(), TheIndex)); }
...
public:
    ...
    const FeedbackParameter& Parameters() const { return FeedbackParameterOf(node()->op()); }
...
public:
    ...
    const GetIteratorParameters& Parameters() const { return GetIteratorParametersOf(node()->op()); }
...
public:
    ...
    const CloneObjectParameters& Parameters() const { return CloneObjectParametersOf(node()->op()); }
...
public:
    ...
    const GetTemplateObjectParameters& Parameters() const { return GetTemplateObjectParametersOf(node()->op()); }
...
public:
    explicit constexpr JSCreateLiteralOpNode(Node* node)
        : JSNodeWrapperBase(node)
    {
        DCHECK(node->opcode() == IrOpcode::kJSCreateLiteralArray || node->opcode() == IrOpcode::kJSCreateLiteralObject || node->opcode() == IrOpcode::kJSCreateLiteralRegExp);
    }

    const CreateLiteralParameters& Parameters() const { return CreateLiteralParametersOf(node()->op()); }
...
public:
    ...
    const PropertyAccess& Parameters() const { return PropertyAccessOf(node()->op()); }
...
#undef INPUTS
    ...
    static constexpr int ArityForArgc(int parameters) { return parameters + kExtraInputCount; }

    static constexpr int FirstArgumentIndex() { return ReceiverOrNewTargetIndex() + 1; }
    ...
    TNode<Object> ArgumentOr(int i, Node* default_value) const { return i < ArgumentCount() ? Argument(i) : TNode<Object>::UncheckedCast(default_value); }
    TNode<Object> ArgumentOrUndefined(int i, JSGraph* jsgraph) const { return ArgumentOr(i, js_node_wrapper_utils::UndefinedConstant(jsgraph)); }
    ...
    int FeedbackVectorIndex() const { return FeedbackVectorIndexForArgc(ArgumentCount()); }
    TNode<HeapObject> feedback_vector() const { return TNode<HeapObject>::UncheckedCast(NodeProperties::GetValueInput(node(), FeedbackVectorIndex())); }
...
private:
    static constexpr bool IsValidNode(Node* node)
    {
        return node->opcode() == IrOpcode::kJSCall || node->opcode() == IrOpcode::kJSCallWithArrayLike || node->opcode() == IrOpcode::kJSCallWithSpread
            || node->opcode() == IrOpcode::kJSConstruct || node->opcode() == IrOpcode::kJSConstructWithArrayLike || node->opcode() == IrOpcode::kJSConstructWithSpread
#if V8_ENABLE_WEBASSEMBLY
...
template <int kOpcode> bool IsExpectedOpcode(int opcode) { return opcode == kOpcode; }

template <int kOpcode1, int kOpcode2, int... kOpcodes> bool IsExpectedOpcode(int opcode) { return opcode == kOpcode1 || IsExpectedOpcode<kOpcode2, kOpcodes...>(opcode); }
...
public:
    ...
    const CallParameters& Parameters() const { return CallParametersOf(node()->op()); }
...
public:
    ...
    const JSWasmCallParameters& Parameters() const { return OpParameter<JSWasmCallParameters>(node()->op()); }
...
public:
    ...
    const ConstructParameters& Parameters() const { return ConstructParametersOf(node()->op()); }
...
public:
    ...
    const DefineNamedOwnPropertyParameters& Parameters() const { return DefineNamedOwnPropertyParametersOf(node()->op()); }
...
public:
    ...
    const StoreGlobalParameters& Parameters() const { return StoreGlobalParametersOf(node()->op()); }
...
public:
    ...
    const LoadGlobalParameters& Parameters() const { return LoadGlobalParametersOf(node()->op()); }
...
public:
    ...
    const CreateClosureParameters& Parameters() const { return CreateClosureParametersOf(node()->op()); }
...
public:
    ...
    const ForInParameters& Parameters() const { return ForInParametersOf(node()->op()); }
