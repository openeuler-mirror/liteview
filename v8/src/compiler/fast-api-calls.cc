...
OverloadsResolutionResult ResolveOverloads(Zone* zone, const FastApiCallFunctionVector& candidates, unsigned int arg_count)
{
...
public:
    FastApiCallBuilder(Isolate* isolate, Graph* graph, GraphAssembler* graph_assembler, const GetParameter& get_parameter, const ConvertReturnValue& convert_return_value,
    ...
    const CallDescriptor* call_descriptor, int inputs_size, Node** inputs, Node* target, const CFunctionInfo* c_signature, int c_arg_count, Node* stack_slot)
{
    ...
    Node* target_address = __ ExternalConstant(ExternalReference::fast_api_call_target_address(isolate()));
    __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), target_address, 0, target);

    // Disable JS execution
    Node* javascript_execution_assert = __ ExternalConstant(ExternalReference::javascript_execution_assert(isolate()));
    static_assert(sizeof(bool) == 1, "Wrong assumption about boolean size.");

    if (v8_flags.debug_code) {
        auto do_store = __ MakeLabel();
        Node* old_scope_value = __ Load(MachineType::Int8(), javascript_execution_assert, 0);
    ...
    __ Store(StoreRepresentation(MachineRepresentation::kWord8, kNoWriteBarrier), javascript_execution_assert, 0, __ Int32Constant(0));
    ...
    __ Store(StoreRepresentation(MachineRepresentation::kWord8, kNoWriteBarrier), javascript_execution_assert, 0, __ Int32Constant(1));
...
Node* FastApiCallBuilder::Build(const FastApiCallFunctionVector& c_functions, const CFunctionInfo* c_signature, Node* data_argument)
{
    ...
    if (c_functions.size() == 1) {
        ...
        overloads_resolution_result = ResolveOverloads(graph()->zone(), c_functions, c_arg_count);
    ...
    int extra_input_count = FastApiCallNode::kEffectAndControlInputCount + (c_signature->HasOptions() ? 1 : 0);

    Node** const inputs = graph()->zone()->NewArray<Node*>(kFastTargetAddressInputCount + c_arg_count + extra_input_count);
    ...
    inputs[kFastTargetAddressInputIndex] = (c_functions.size() == 1) ? __ ExternalConstant(ExternalReference::Create(c_functions[0].address, ref_type)) : nullptr;
    ...
    if (c_signature->HasOptions()) {
        ...
        __ Store(StoreRepresentation(MachineRepresentation::kWord32, kNoWriteBarrier), stack_slot, static_cast<int>(offsetof(v8::FastApiCallbackOptions, fallback)),
            __ Int32Constant(0));

        Node* data_stack_slot = __ StackSlot(sizeof(uintptr_t), alignof(uintptr_t));
        __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), data_stack_slot, 0, data_argument);

        __ Store(
            StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), stack_slot, static_cast<int>(offsetof(v8::FastApiCallbackOptions, data)), data_stack_slot);
    ...
    CallDescriptor* call_descriptor = Linkage::GetSimplifiedCDescriptor(graph()->zone(), builder.Build());
    ...
    if (c_signature->HasOptions()) {
        ...
        Node* load = __ Load(MachineType::Int32(), stack_slot, static_cast<int>(offsetof(v8::FastApiCallbackOptions, fallback)));
...
Node* BuildFastApiCall(Isolate* isolate, Graph* graph, GraphAssembler* graph_assembler, const FastApiCallFunctionVector& c_functions, const CFunctionInfo* c_signature,
    Node* data_argument, const GetParameter& get_parameter, const ConvertReturnValue& convert_return_value, const InitializeOptions& initialize_options,
