...
void GraphReducer::AddReducer(Reducer* reducer) { reducers_.push_back(reducer); }
        ...
        if (i != skip) {
            ...
            if (!reduction.Changed()) {
                ...
                if (v8_flags.trace_turbo_reduction) {
                    ...
                    // supporting --no-local-heaps --no-concurrent-inlining.
                    AllowHandleDereference allow_deref;
                    StdoutStream {} << "- Replacement of #" << *node << " with #" << *(reduction.replacement()) << " by reducer " << (*i)->reducer_name() << std::endl;
                }
                return reduction;
            }
        }
...
void GraphReducer::Replace(Node* node, Node* replacement) { Replace(node, replacement, std::numeric_limits<NodeId>::max()); }
