...
BasicBlockProfilerData* BasicBlockInstrumentor::Instrument(OptimizedCompilationInfo* info, Graph* graph, Schedule* schedule, Isolate* isolate)
{
    ...
    for (BasicBlockVector::iterator it = blocks->begin(); block_number < n_blocks; ++it, ++block_number) {
        ...
        Node* store = graph->NewNode(
            machine.Store(StoreRepresentation(MachineRepresentation::kWord32, kNoWriteBarrier)), counters_array, offset_to_counter, saturated_inc, graph->start(), graph->start());
        ...
        // count.
        if (block->control() == BasicBlock::kBranch && block->successors()[0]->rpo_number() != static_cast<int32_t>(n_blocks)
            && block->successors()[1]->rpo_number() != static_cast<int32_t>(n_blocks)) {
