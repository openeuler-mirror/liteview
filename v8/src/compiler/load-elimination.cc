...
bool MustAlias(Node* a, Node* b) { return ResolveRenames(a) == ResolveRenames(b); }
...
void LoadElimination::AbstractElements::Print() const
{
    for (Element const& element : elements_) {
        if (element.object) {
            PrintF("    #%d:%s @ #%d:%s -> #%d:%s\n", element.object->id(), element.object->op()->mnemonic(), element.index->id(), element.index->op()->mnemonic(),
                element.value->id(), element.value->op()->mnemonic());
        }
    }
}
...
LoadElimination::AbstractMaps::AbstractMaps(Node* object, ZoneHandleSet<Map> maps, Zone* zone)
    : info_for_node_(zone)
{
...
bool LoadElimination::AbstractMaps::Lookup(Node* object, ZoneHandleSet<Map>* object_maps) const
{
...
LoadElimination::AbstractMaps const* LoadElimination::AbstractMaps::Extend(Node* object, ZoneHandleSet<Map> maps, Zone* zone) const
{
...
bool LoadElimination::AbstractState::LookupMaps(Node* object, ZoneHandleSet<Map>* object_map) const { return this->maps_ && this->maps_->Lookup(object, object_map); }

LoadElimination::AbstractState const* LoadElimination::AbstractState::SetMaps(Node* object, ZoneHandleSet<Map> maps, Zone* zone) const
{
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::AddElement(Node* object, Node* index, Node* value, MachineRepresentation representation, Zone* zone) const
{
    AbstractState* that = zone->New<AbstractState>(*this);
    if (that->elements_) {
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::AddField(Node* object, IndexRange index_range, LoadElimination::FieldInfo info, Zone* zone) const
{
    AbstractState* that = zone->New<AbstractState>(*this);
    AbstractFields& fields = info.const_field_info.IsConst() ? that->const_fields_ : that->fields_;
...
LoadElimination::AbstractState const* LoadElimination::AbstractState::KillField(const AliasStateInfo& alias_info, IndexRange index_range, MaybeHandle<Name> name, Zone* zone) const
{
    AbstractState* that = nullptr;
    for (int index : index_range) {
...
Reduction LoadElimination::ReduceMaybeGrowFastElements(Node* node)
{
    ...
    if (params.mode() == GrowFastElementsMode::kDoubleElements) {
        ...
        state = state->SetMaps(node, ZoneHandleSet<Map>(factory()->fixed_double_array_map()), zone());
    } else {
...
Reduction LoadElimination::ReduceStoreField(Node* node, FieldAccess const& access)
{
    ...
    if (access.offset == HeapObject::kMapOffset && access.base_is_tagged == kTaggedBase) {
        ...
        if (new_value_type.IsHeapConstant()) {
            ...
            ZoneHandleSet<Map> object_maps(new_value_type.AsHeapConstant()->Ref().AsMap().object());
            state = state->SetMaps(object, object_maps, zone());
        }
    } else {
        IndexRange field_index = FieldIndexOf(access);
...
Reduction LoadElimination::ReduceStart(Node* node) { return UpdateState(node, empty_state()); }
...
LoadElimination::AbstractState const* LoadElimination::ComputeLoopStateForStoreField(Node* current, LoadElimination::AbstractState const* state, FieldAccess const& access) const
{
...
CommonOperatorBuilder* LoadElimination::common() const { return jsgraph()->common(); }
