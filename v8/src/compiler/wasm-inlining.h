...
public:
    WasmInliner(Editor* editor, wasm::CompilationEnv* env, uint32_t function_index, SourcePositionTable* source_positions, NodeOriginTable* node_origins, MachineGraph* mcgraph,
        const wasm::WireBytesStorage* wire_bytes, std::vector<WasmLoopInfo>* loop_infos, const char* debug_name)
    ...
    static bool graph_size_allows_inlining(size_t graph_size) { return graph_size < v8_flags.wasm_inlining_budget; }
...
private:
    ...
    struct LexicographicOrdering {
        ...
        {
            if (c1.call_count > c2.call_count)
                return false;
            if (c2.call_count > c1.call_count)
                return true;
    ...
    void InlineCall(Node* call, Node* callee_start, Node* callee_end, const wasm::FunctionSig* inlinee_sig, size_t subgraph_min_node_id);
