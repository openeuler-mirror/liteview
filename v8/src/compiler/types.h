...
#define PROPER_ATOMIC_BITSET_TYPE_HIGH_LIST(V) V(SandboxedPointer, uint64_t { 1 } << 32)
...
#define PROPER_BITSET_TYPE_LIST(V)                                                                                                                                                 \
    ...
    V(NumberOrUndefinedOrNullOrBoolean, kNumber | kNullOrUndefined | kBoolean)                                                                                                     \
...
#undef DECLARE_TYPE
    ...
    static bool Is(bitset bits1, bitset bits2) { return (bits1 | bits2) == bits2; }

    static double Min(bitset);
    static double Max(bitset);

    static bitset Glb(double min, double max);
    static bitset Lub(HeapObjectType const& type) { return Lub<HeapObjectType>(type); }
...
private:
    ...
    template <typename MapRefLike> static bitset Lub(MapRefLike const& map);
};
...
private:
    ...
    static RangeType* New(double min, double max, Zone* zone) { return New(Limits(min, max), zone); }
...
public:
    static WasmType* New(wasm::ValueType value_type, const wasm::WasmModule* module, Zone* zone) { return zone->New<WasmType>(value_type, module); }
...
#undef DEFINE_TYPE_CONSTRUCTOR
    ...
    static Type For(MapRef const& type) { return NewBitset(BitsetType::ExpandInternals(BitsetType::Lub(type))); }

    // Predicates.
    bool IsNone() const { return payload_ == None().payload_; }
    bool IsInvalid() const { return payload_ == uint64_t { 0 }; }

    bool Is(Type that) const { return payload_ == that.payload_ || this->SlowIs(that); }
    ...
    bool IsOtherNumberConstant() const { return IsKind(TypeBase::kOtherNumberConstant); }
    ...
    bool IsSingleton() const
    {
        if (IsNone())
            return false;
        return Is(Type::Null()) || Is(Type::Undefined()) || Is(Type::MinusZero()) || Is(Type::NaN()) || Is(Type::Hole()) || IsHeapConstant()
...
private:
    ...
    const TypeBase* ToTypeBase() const { return reinterpret_cast<TypeBase*>(payload_); }
...
private:
    ...
    static OtherNumberConstantType* New(double value, Zone* zone) { return zone->New<OtherNumberConstantType>(value); }
...
private:
    ...
    static HeapConstantType* New(const HeapObjectRef& heap_ref, BitsetType::bitset bitset, Zone* zone) { return zone->New<HeapConstantType>(bitset, heap_ref); }
...
private:
    ...
    static TupleType* New(int length, Zone* zone) { return zone->New<TupleType>(length, zone); }
};
...
private:
    ...
    static UnionType* New(int length, Zone* zone) { return zone->New<UnionType>(length, zone); }
