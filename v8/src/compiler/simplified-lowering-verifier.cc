...
{
    if (t1.IsLessGeneralThan(t2))
        return t1;
    CHECK(t2.IsLessGeneralThan(t1));
    return t2;
}

Truncation LeastGeneralTruncation(const Truncation& t1, const Truncation& t2, const Truncation& t3) { return LeastGeneralTruncation(LeastGeneralTruncation(t1, t2), t3); }
...
bool IsModuloTruncation(const Truncation& truncation) { return truncation.IsUsedAsWord32() || truncation.IsUsedAsWord64() || Truncation::Any().IsLessGeneralThan(truncation); }
...
void SimplifiedLoweringVerifier::VisitNode(Node* node, OperationTyper& op_typer)
{
    ...
    case IrOpcode::kInt32Add: {
        Type output_type = op_typer.NumberAdd(InputType(node, 0), InputType(node, 1));
        Truncation output_trunc = LeastGeneralTruncation(InputTruncation(node, 0), InputTruncation(node, 1), Truncation::Word32());
        CHECK(IsModuloTruncation(output_trunc));
        CheckAndSet(node, output_type, output_trunc);
        break;
    }
    case IrOpcode::kInt32Sub: {
        Type output_type = op_typer.NumberSubtract(InputType(node, 0), InputType(node, 1));
        Truncation output_trunc = LeastGeneralTruncation(InputTruncation(node, 0), InputTruncation(node, 1), Truncation::Word32());
        CHECK(IsModuloTruncation(output_trunc));
        CheckAndSet(node, output_type, output_trunc);
        break;
    ...
    case IrOpcode::kChangeFloat64ToInt64: {
        Truncation output_trunc = LeastGeneralTruncation(InputTruncation(node, 0), Truncation::Word64());
    ...
    case IrOpcode::kInt64Add: {
        ...
        if (left_type.Is(Type::BigInt()) && right_type.Is(Type::BigInt())) {
        ...
        } else if (left_type.Is(Type::Number()) && right_type.Is(Type::Number())) {
            ...
            FATAL("SimplifiedLoweringVerifierError: invalid combination of input "
                  "types "
                  "%s and %s for node #%d:%s",
                left_str.str().c_str(), right_str.str().c_str(), node->id(), node->op()->mnemonic());
        }

        Truncation output_trunc = LeastGeneralTruncation(InputTruncation(node, 0), InputTruncation(node, 1), Truncation::Word64());
        CHECK(IsModuloTruncation(output_trunc));
        CheckAndSet(node, output_type, output_trunc);
        break;
    }
    ...
    case IrOpcode::kTypedStateValues: {
        ...
        for (int i = 0; i < static_cast<int>(machine_types->size()); ++i) {
            ...
            CHECK(IsNonTruncatingMachineTypeFor(machine_types->at(i), InputType(node, i)));
        }
        break;
    }
