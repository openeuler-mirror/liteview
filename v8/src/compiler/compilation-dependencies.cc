...
#undef V
    ...
    const char* ToString() const { return CompilationDependencyKindToString(kind); }
...
size_t CompilationDependencies::CompilationDependencyHash::operator()(const CompilationDependency* dep) const { return base::hash_combine(dep->kind, dep->Hash()); }
...
public:
    ...
    void InstallAll(Isolate* isolate, Handle<Code> code)
    {
        ...
        for (const auto& o_and_g : deps_) {
            DependentCode::InstallDependency(isolate, code, o_and_g.first, o_and_g.second);
        }
    }
    ...
    void InstallAllPredictable(Isolate* isolate, Handle<Code> code)
    {
        ...
        using HandleAndGroup = std::pair<Handle<HeapObject>, DependentCode::DependencyGroups>;
        std::vector<HandleAndGroup> entries(deps_.begin(), deps_.end());

        std::sort(entries.begin(), entries.end(), [](const HandleAndGroup& lhs, const HandleAndGroup& rhs) { return lhs.first->ptr() < rhs.first->ptr(); });
...
private:
    struct HandleHash {
        size_t operator()(const Handle<HeapObject>& x) const { return static_cast<size_t>(x->ptr()); }
    };
    struct HandleEqual {
        bool operator()(const Handle<HeapObject>& lhs, const Handle<HeapObject>& rhs) const { return lhs.is_identical_to(rhs); }
    };
    ZoneUnorderedMap<Handle<HeapObject>, DependentCode::DependencyGroups, HandleHash, HandleEqual> deps_;
    const DisallowGarbageCollection no_gc_;
};

class InitialMapDependency final : public CompilationDependency {
public:
    InitialMapDependency(JSHeapBroker* broker, const JSFunctionRef& function, const MapRef& initial_map)
...
public:
    PrototypePropertyDependency(JSHeapBroker* broker, const JSFunctionRef& function, const ObjectRef& prototype)
        ...
        , prototype_(prototype)
    {
        ...
        DCHECK(function_.instance_prototype(broker->dependencies()).equals(prototype_));
    }
...
private:
    ...
    MaybeHandle<JSObject> GetHolderIfValid() const
    {
        ...
        auto try_load = [&](auto dictionary) -> ValidationResult {
            ...
            if (kind_ == PropertyKind::kAccessor) {
                ...
                // getter.
                value = AccessorPair::cast(dictionary_value).get(AccessorComponent::ACCESSOR_GETTER);
            } else {
        ...
        while (prototype.IsJSObject()) {
            ...
            ValidationResult result = V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL ? try_load(object.property_dictionary_swiss()) : try_load(object.property_dictionary());
    ...
    bool Equals(const CompilationDependency* that) const override
    {
        ...
        return receiver_map_.equals(zat->receiver_map_) && property_name_.equals(zat->property_name_) && constant_.equals(zat->constant_) && kind_ == zat->kind_;
    }
...
public:
    OwnConstantDataPropertyDependency(JSHeapBroker* broker, const JSObjectRef& holder, const MapRef& map, Representation representation, FieldIndex index, const ObjectRef& value)
    ...
    bool IsValid() const override
    {
        ...
        if (representation_.IsDouble()) {
            ...
            if (!current_value.IsHeapNumber() || !used_value.IsHeapNumber()
                || HeapNumber::cast(current_value).value_as_bits(kRelaxedLoad) != HeapNumber::cast(used_value).value_as_bits(kRelaxedLoad)) {
...
private:
    ...
    {
        ObjectRef::Hash h;
        return base::hash_combine(h(holder_), h(map_), representation_.kind(), index_.bit_field(), h(value_));
    }

    bool Equals(const CompilationDependency* that) const override
    {
        const OwnConstantDataPropertyDependency* const zat = that->AsOwnConstantDataProperty();
        return holder_.equals(zat->holder_) && map_.equals(zat->map_) && representation_.Equals(zat->representation_) && index_ == zat->index_ && value_.equals(zat->value_);
    }
...
public:
    OwnConstantDictionaryPropertyDependency(JSHeapBroker* broker, const JSObjectRef& holder, InternalIndex index, const ObjectRef& value)
    ...
    bool IsValid() const override
    {
        ...
        base::Optional<Object> maybe_value = JSObject::DictionaryPropertyAt(holder_.object(), index_, broker_->isolate()->heap());

        if (!maybe_value) {
            TRACE_BROKER_MISSING(broker_, holder_.object() << "has a value that might not safe to read at index " << index_.as_int());
            return false;
        }

        if (*maybe_value != *value_.object()) {
            TRACE_BROKER_MISSING(broker_, "Constant property value changed in " << holder_.object() << " at InternalIndex " << index_.as_int());
            return false;
        }
        return true;
    }
...
public:
    ...
    bool IsValid() const override { return function_.IsConsistentWithHeapState(); }
...
public:
    PretenureModeDependency(const AllocationSiteRef& site, AllocationType allocation)
    ...
    bool IsValid() const override { return allocation_ == site_.object()->GetAllocationType(); }
    void Install(PendingDependencies* deps) const override
    {
        SLOW_DCHECK(IsValid());
...
public:
    FieldRepresentationDependency(const MapRef& map, InternalIndex descriptor, Representation representation)
    ...
    bool IsValid() const override
    {
        ...
        if (map_.object()->is_deprecated())
            return false;
        return representation_.Equals(map_.object()->instance_descriptors(map_.isolate()).GetDetails(descriptor_).representation());
    }
    ...
    void Install(PendingDependencies* deps) const override
    {
        ...
        Handle<Map> owner(map_.object()->FindFieldOwner(isolate, descriptor_), isolate);
    ...
    bool DependsOn(const Handle<Map>& receiver_map) const { return map_.object().equals(receiver_map); }
...
public:
    FieldTypeDependency(const MapRef& map, InternalIndex descriptor, const ObjectRef& type)
    ...
    bool IsValid() const override
    {
        ...
        if (map_.object()->is_deprecated())
            return false;
        return *type_.object() == map_.object()->instance_descriptors(map_.isolate()).GetFieldType(descriptor_);
    }
...
public:
    ...
    bool IsValid() const override
    {
        ...
        if (map_.object()->is_deprecated())
            return false;
        return PropertyConstness::kConst == map_.object()->instance_descriptors(map_.isolate()).GetDetails(descriptor_).constness();
    }
...
public:
    GlobalPropertyDependency(const PropertyCellRef& cell, PropertyCellType type, bool read_only)
...
public:
    explicit ObjectSlotValueDependency(const HeapObjectRef& object, int offset, const ObjectRef& value)
    ...
    bool IsValid() const override
    {
        ...
        Object current_value = offset_ == HeapObject::kMapOffset ? object_->map() : TaggedField<Object>::Relaxed_Load(cage_base, *object_, offset_);
        return *value_ == current_value;
    }
    void Install(PendingDependencies* deps) const override { }

private:
    size_t Hash() const override { return base::hash_combine(object_.address(), offset_, value_.address()); }
...
public:
    OwnConstantElementDependency(const JSObjectRef& holder, uint32_t index, const ObjectRef& element)
    ...
    bool IsValid() const override
    {
        ...
        base::Optional<Object> maybe_element = holder_.GetOwnConstantElementFromHeap(holder.elements(), holder.GetElementsKind(), index_);
...
public:
    InitialMapInstanceSizePredictionDependency(const JSFunctionRef& function, int instance_size)
    ...
    bool IsValid() const override
    {
        ...
        if (!function_.object()->has_initial_map())
            return false;
        int instance_size = function_.object()->ComputeInstanceSizeWithMinSlack(function_.isolate());
...
MapRef CompilationDependencies::DependOnInitialMap(const JSFunctionRef& function)
{
...
ObjectRef CompilationDependencies::DependOnPrototypeProperty(const JSFunctionRef& function)
{
...
void CompilationDependencies::DependOnConstantInDictionaryPrototypeChain(const MapRef& receiver_map, const NameRef& property_name, const ObjectRef& constant, PropertyKind kind)
{
    RecordDependency(zone_->New<ConstantInDictionaryPrototypeChainDependency>(receiver_map, property_name, constant, kind));
}

AllocationType CompilationDependencies::DependOnPretenureMode(const AllocationSiteRef& site)
{
...
PropertyConstness CompilationDependencies::DependOnFieldConstness(const MapRef& map, InternalIndex descriptor)
{
...
void CompilationDependencies::DependOnGlobalProperty(const PropertyCellRef& cell)
{
...
bool CompilationDependencies::DependOnProtector(const PropertyCellRef& cell)
{
    ...
    if (cell.value().AsSmi() != Protectors::kProtectorValid)
        return false;
    RecordDependency(zone_->New<ProtectorDependency>(cell));
    return true;
}

bool CompilationDependencies::DependOnMegaDOMProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->mega_dom_protector())); }
...
bool CompilationDependencies::DependOnArrayIteratorProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->array_iterator_protector())); }

bool CompilationDependencies::DependOnArraySpeciesProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->array_species_protector())); }

bool CompilationDependencies::DependOnNoElementsProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->no_elements_protector())); }

bool CompilationDependencies::DependOnPromiseHookProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->promise_hook_protector())); }

bool CompilationDependencies::DependOnPromiseSpeciesProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->promise_species_protector())); }

bool CompilationDependencies::DependOnPromiseThenProtector() { return DependOnProtector(MakeRef(broker_, broker_->isolate()->factory()->promise_then_protector())); }

void CompilationDependencies::DependOnElementsKind(const AllocationSiteRef& site)
{
    ElementsKind kind = site.PointsToLiteral() ? site.boilerplate().value().map().elements_kind() : site.GetElementsKind();
    if (AllocationSite::ShouldTrack(kind)) {
        RecordDependency(zone_->New<ElementsKindDependency>(site, kind));
    }
}

void CompilationDependencies::DependOnObjectSlotValue(const HeapObjectRef& object, int offset, const ObjectRef& value)
{
    RecordDependency(zone_->New<ObjectSlotValueDependency>(object, offset, value));
}

void CompilationDependencies::DependOnOwnConstantElement(const JSObjectRef& holder, uint32_t index, const ObjectRef& element)
{
    RecordDependency(zone_->New<OwnConstantElementDependency>(holder, index, element));
}

void CompilationDependencies::DependOnOwnConstantDataProperty(const JSObjectRef& holder, const MapRef& map, Representation representation, FieldIndex index, const ObjectRef& value)
{
    RecordDependency(zone_->New<OwnConstantDataPropertyDependency>(broker_, holder, map, representation, index, value));
}

void CompilationDependencies::DependOnOwnConstantDictionaryProperty(const JSObjectRef& holder, InternalIndex index, const ObjectRef& value)
{
    RecordDependency(zone_->New<OwnConstantDictionaryPropertyDependency>(broker_, holder, index, value));
}

V8_INLINE void TraceInvalidCompilationDependency(const CompilationDependency* d)
{
...
bool CompilationDependencies::Commit(Handle<Code> code)
{
    ...
    if (v8_flags.stress_gc_during_compilation) {
        broker_->isolate()->heap()->PreciseCollectAllGarbage(Heap::kForcedGC, GarbageCollectionReason::kTesting, kNoGCCallbackFlags);
    }
#ifdef DEBUG
    for (auto dep : dependencies_) {
        CHECK_IMPLIES(!dep->IsValid(), dep->IsPretenureMode() || dep->IsConsistentJSFunctionView());
    }
#endif
...
void CompilationDependencies::DependOnStablePrototypeChains(ZoneVector<MapRef> const& receiver_maps, WhereToStart start, base::Optional<JSObjectRef> last_prototype)
{
...
void CompilationDependencies::DependOnStablePrototypeChain(MapRef receiver_map, WhereToStart start, base::Optional<JSObjectRef> last_prototype)
{
    if (receiver_map.IsPrimitiveMap()) {
        // Perform the implicit ToObject for primitives here.
        // Implemented according to ES6 section 7.3.2 GetV (V, P).
        // Note: Keep sync'd with AccessInfoFactory::ComputePropertyAccessInfo.
        base::Optional<JSFunctionRef> constructor = broker_->target_native_context().GetConstructorFunction(receiver_map);
...
void CompilationDependencies::DependOnElementsKinds(const AllocationSiteRef& site)
{
...
void CompilationDependencies::DependOnConsistentJSFunctionView(const JSFunctionRef& function) { RecordDependency(zone_->New<ConsistentJSFunctionViewDependency>(function)); }
...
SlackTrackingPrediction CompilationDependencies::DependOnInitialMapInstanceSizePrediction(const JSFunctionRef& function)
{
...
CompilationDependency const* CompilationDependencies::TransitionDependencyOffTheRecord(const MapRef& target_map) const
{
...
CompilationDependency const* CompilationDependencies::FieldRepresentationDependencyOffTheRecord(const MapRef& map, InternalIndex descriptor, Representation representation) const
{
    return zone_->New<FieldRepresentationDependency>(map, descriptor, representation);
}

CompilationDependency const* CompilationDependencies::FieldTypeDependencyOffTheRecord(const MapRef& map, InternalIndex descriptor, const ObjectRef& type) const
{
