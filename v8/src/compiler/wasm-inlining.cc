...
Reduction WasmInliner::ReduceCall(Node* call)
{
    ...
    if (v8_flags.liftoff && v8_flags.wasm_speculative_inlining && wire_byte_size >= 12 && call_count < min_count_for_inlining) {
...
{
    if (WasmInliner::graph_size_allows_inlining(current_graph_size + candidate_size)) {
        return true;
    }
    // For truly tiny functions, let's be a bit more generous.
    return candidate_size <= 12 && WasmInliner::graph_size_allows_inlining(current_graph_size - 100);
}
...
void WasmInliner::Trace(const CandidateInfo& candidate, const char* decision)
{
    TRACE("  [function %d: considering candidate {@%d, index=%d, count=%d, "
          "size=%d}: %s]\n",
        function_index_, candidate.node->id(), candidate.inlinee_index, candidate.call_count, candidate.wire_byte_size, decision);
}
...
void WasmInliner::Finalize()
{
    ...
    while (!inlining_candidates_.empty()) {
        ...
        const wasm::FunctionSig* lowered_sig = mcgraph_->machine()->Is64() ? inlinee->sig : GetI32Sig(zone(), inlinee->sig);

        DCHECK_EQ(lowered_sig->parameter_count(), call->op()->ValueInputCount() - 2);
#if DEBUG
        ...
        for (uint32_t i = 0; i < lowered_sig->parameter_count(); i++) {
            ...
            CHECK(IsSubtypeOf(param_type.type, lowered_sig->GetParam(i), param_type.module, module()));
        }
#endif
        ...
        Node* inlinee_start;
        Node* inlinee_end;
        const wasm::FunctionBody inlinee_body(inlinee->sig, inlinee->code.offset(), function_bytes.begin(), function_bytes.end());
        WasmGraphBuilder builder(env_, zone(), mcgraph_, inlinee_body.sig, source_positions_);
        {
            Graph::SubgraphScope scope(graph());
            wasm::DecodeResult result = wasm::BuildTFGraph(zone()->allocator(), env_->enabled_features, module(), &builder, &detected, inlinee_body, &inlinee_loop_infos,
                node_origins_, candidate.inlinee_index, NodeProperties::IsExceptionalCall(call) ? wasm::kInlinedHandledCall : wasm::kInlinedNonHandledCall);
            if (result.ok()) {
        ...
        if (call->opcode() == IrOpcode::kCall) {
            InlineCall(call, inlinee_start, inlinee_end, lowered_sig, subgraph_min_node_id);
        } else {
            InlineTailCall(call, inlinee_start, inlinee_end);
        }
        call->Kill();
        loop_infos_->insert(loop_infos_->end(), inlinee_loop_infos.begin(), inlinee_loop_infos.end());
    ...
    for (Node* use : if_exception->uses()) {
        ...
        if (use->opcode() == IrOpcode::kLoopExit) {
            for (Node* loop_exit_use : use->uses()) {
                if (loop_exit_use->opcode() != IrOpcode::kLoopExitEffect && loop_exit_use->opcode() != IrOpcode::kLoopExitValue) {
...
void WasmInliner::InlineCall(Node* call, Node* callee_start, Node* callee_end, const wasm::FunctionSig* inlinee_sig, size_t subgraph_min_node_id)
{
    ...
    if (NodeProperties::IsExceptionalCall(call, &handler)) {
        ...
        for (Node* node : subgraph_nodes.reachable) {
            if (node->id() >= subgraph_min_node_id && !node->op()->HasProperty(Operator::kNoThrow)) {
        ...
        case IrOpcode::kTailCall: {
            ...
            if (return_arity == 1) {
                ...
                for (int i = 0; i < return_arity; i++) {
                    return_inputs.push_back(graph()->NewNode(common()->Projection(i), input, input));
                }
            }

            // Add effect and control inputs.
            return_inputs.push_back(input->op()->EffectOutputCount() > 0 ? input : NodeProperties::GetEffectInput(input));
            return_inputs.push_back(input->op()->ControlOutputCount() > 0 ? input : NodeProperties::GetControlInput(input));
    ...
    if (handler_count > 0) {
        Node* control_output = graph()->NewNode(common()->Merge(handler_count), handler_count, dangling_handlers.data());
        ...
        for (Node* control : dangling_handlers) {
            if (control->opcode() == IrOpcode::kIfException) {
                ...
                effects.push_back(graph()->NewNode(common()->LoopExitEffect(), if_exception, control));
                values.push_back(graph()->NewNode(common()->LoopExitValue(MachineRepresentation::kTagged), if_exception, control));
            }
        }
