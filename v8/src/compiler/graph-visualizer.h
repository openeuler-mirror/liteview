...
V8_INLINE V8_EXPORT_PRIVATE SourcePositionAsJSON AsJSON(const SourcePosition& sp) { return SourcePositionAsJSON(sp); }
...
V8_INLINE V8_EXPORT_PRIVATE NodeOriginAsJSON AsJSON(const NodeOrigin& no) { return NodeOriginAsJSON(no); }
...
void JsonPrintFunctionSource(
    std::ostream& os, int source_id, std::unique_ptr<char[]> function_name, Handle<Script> script, Isolate* isolate, Handle<SharedFunctionInfo> shared, bool with_key = false);
...
V8_INLINE V8_EXPORT_PRIVATE GraphAsJSON AsJSON(const Graph& g, SourcePositionTable* p, NodeOriginTable* o) { return GraphAsJSON(g, p, o); }
