...
base::Optional<ContextRef> GetSpecializationContext(JSHeapBroker* broker, Node* node, size_t* distance, Maybe<OuterContext> maybe_outer)
{
    switch (node->opcode()) {
...
base::Optional<ContextRef> GetModuleContext(JSHeapBroker* broker, Node* node, Maybe<OuterContext> maybe_context)
{
...
{
    base::Optional<ContextRef> maybe_context = GetModuleContext(broker(), node, outer());
    ...
    if (!module.has_value())
        return NoChange();
    base::Optional<ObjectRef> import_meta = module->AsSourceTextModule().import_meta();
...
Isolate* JSContextSpecialization::isolate() const { return jsgraph()->isolate(); }
