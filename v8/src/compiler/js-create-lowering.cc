...
Reduction JSCreateLowering::ReduceJSCreateArguments(Node* node)
{
    ...
    if (frame_state.outer_frame_state()->opcode() != IrOpcode::kFrameState) {
        ...
        case CreateArgumentsType::kMappedArguments: {
            ...
            Node* const arguments_map = jsgraph()->Constant(has_aliased_arguments ? native_context().fast_aliased_arguments_map() : native_context().sloppy_arguments_map());
        ...
        case CreateArgumentsType::kUnmappedArguments: {
            ...
            Node* const elements = effect = graph()->NewNode(
                simplified()->NewArgumentsElements(CreateArgumentsType::kUnmappedArguments, shared.internal_formal_parameter_count_without_receiver()), arguments_length, effect);
            // Load the arguments object map.
        ...
        case CreateArgumentsType::kRestParameter: {
            ...
            // Allocate the elements backing store.
            Node* const elements = effect = graph()->NewNode(
                simplified()->NewArgumentsElements(CreateArgumentsType::kRestParameter, shared.internal_formal_parameter_count_without_receiver()), arguments_length, effect);
    ...
    case CreateArgumentsType::kRestParameter: {
        ...
        a.Store(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), jsgraph()->EmptyFixedArrayConstant());
        a.Store(AccessBuilder::ForJSObjectElements(), elements);
        a.Store(AccessBuilder::ForJSArrayLength(PACKED_ELEMENTS), jsgraph()->Constant(length));
        RelaxControls(node);
        a.FinishAndChange(node);
        return Changed(node);
    }
...
Reduction JSCreateLowering::ReduceJSCreateGeneratorObject(Node* node)
{
    ...
    if (closure_type.IsHeapConstant()) {
        ...
        if (!js_function.has_initial_map(dependencies()))
            return NoChange();
        ...
        int length = parameter_count_no_receiver + shared.GetBytecodeArray().register_count();
        ...
        a.Store(AccessBuilder::ForJSGeneratorObjectResumeMode(), jsgraph()->Constant(JSGeneratorObject::kNext));
        a.Store(AccessBuilder::ForJSGeneratorObjectContinuation(), jsgraph()->Constant(JSGeneratorObject::kGeneratorExecuting));
...
Reduction JSCreateLowering::ReduceNewArray(
    Node* node, Node* length, MapRef initial_map, ElementsKind elements_kind, AllocationType allocation, const SlackTrackingPrediction& slack_tracking_prediction)
{
    ...
    base::Optional<MapRef> maybe_initial_map = initial_map.AsElementsKind(GetHoleyElementsKind(elements_kind));
    ...
    length = effect = graph()->NewNode(simplified()->CheckBounds(FeedbackSource()), length, jsgraph()->Constant(JSArray::kInitialMaxFastElementArray), effect, control);

    // Construct elements and properties for the resulting JSArray.
    Node* elements = effect
        = graph()->NewNode(IsDoubleElementsKind(initial_map.elements_kind()) ? simplified()->NewDoubleElements(allocation) : simplified()->NewSmiOrObjectElements(allocation),
...
Reduction JSCreateLowering::ReduceNewArray(
    Node* node, Node* length, int capacity, MapRef initial_map, ElementsKind elements_kind, AllocationType allocation, const SlackTrackingPrediction& slack_tracking_prediction)
{
    ...
    base::Optional<MapRef> maybe_initial_map = initial_map.AsElementsKind(elements_kind);
    if (!maybe_initial_map.has_value())
        return NoChange();
    initial_map = maybe_initial_map.value();
...
Reduction JSCreateLowering::ReduceNewArray(
    Node* node, std::vector<Node*> values, MapRef initial_map, ElementsKind elements_kind, AllocationType allocation, const SlackTrackingPrediction& slack_tracking_prediction)
{
    DCHECK_EQ(IrOpcode::kJSCreateArray, node->opcode());
    Node* effect = NodeProperties::GetEffectInput(node);
    Node* control = NodeProperties::GetControlInput(node);
...
Reduction JSCreateLowering::ReduceJSCreateArrayIterator(Node* node)
{
    ...
    a.Store(AccessBuilder::ForMap(), native_context().initial_array_iterator_map());
    a.Store(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), jsgraph()->EmptyFixedArrayConstant());
    a.Store(AccessBuilder::ForJSObjectElements(), jsgraph()->EmptyFixedArrayConstant());
    ...
    a.Store(AccessBuilder::ForJSArrayIteratorKind(), jsgraph()->Constant(static_cast<int>(p.kind())));
    RelaxControls(node);
    a.FinishAndChange(node);
    return Changed(node);
}
...
Reduction JSCreateLowering::ReduceJSCreateAsyncFunctionObject(Node* node)
{
    ...
    a.Store(AccessBuilder::ForMap(), native_context().async_function_object_map());
    a.Store(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), jsgraph()->EmptyFixedArrayConstant());
    a.Store(AccessBuilder::ForJSObjectElements(), jsgraph()->EmptyFixedArrayConstant());
    a.Store(AccessBuilder::ForJSGeneratorObjectContext(), context);
    a.Store(AccessBuilder::ForJSGeneratorObjectFunction(), closure);
...
MapRef MapForCollectionIterationKind(const NativeContextRef& native_context, CollectionKind collection_kind, IterationKind iteration_kind)
{
...
Reduction JSCreateLowering::ReduceJSCreateCollectionIterator(Node* node)
{
    ...
    a.Store(AccessBuilder::ForMap(), MapForCollectionIterationKind(native_context(), p.collection_kind(), p.iteration_kind()));
    a.Store(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), jsgraph()->EmptyFixedArrayConstant());
    a.Store(AccessBuilder::ForJSObjectElements(), jsgraph()->EmptyFixedArrayConstant());
...
Reduction JSCreateLowering::ReduceJSCreateClosure(Node* node)
{
    ...
    if (!feedback_cell.map().equals(MakeRef(broker(), factory()->many_closures_cell_map()))) {
        return NoChange();
    }
...
Reduction JSCreateLowering::ReduceJSCreateLiteralArrayOrObject(Node* node)
{
    ...
    if (!feedback.IsInsufficient()) {
        ...
        base::Optional<Node*> maybe_value = TryAllocateFastLiteral(effect, control, *site.boilerplate(), allocation, kMaxFastLiteralDepth, &max_properties);
...
Reduction JSCreateLowering::ReduceJSCreateFunctionContext(Node* node)
{
    ...
    if (slot_count < kFunctionContextAllocationLimit) {
        ...
        case EVAL_SCOPE:
            ...
            break;
        case FUNCTION_SCOPE:
            a.AllocateContext(context_length, native_context().function_context_map());
            break;
        default:
            UNREACHABLE();
        }
...
Reduction JSCreateLowering::ReduceJSCreateWithContext(Node* node)
{
    ...
    a.AllocateContext(Context::MIN_CONTEXT_EXTENDED_SLOTS, native_context().with_context_map());
    a.Store(AccessBuilder::ForContextSlot(Context::SCOPE_INFO_INDEX), scope_info);
    a.Store(AccessBuilder::ForContextSlot(Context::PREVIOUS_INDEX), context);
...
Reduction JSCreateLowering::ReduceJSCreateCatchContext(Node* node)
{
    ...
    AllocationBuilder a(jsgraph(), effect, control);
    static_assert(Context::MIN_CONTEXT_SLOTS == 2); // Ensure fully covered.
    a.AllocateContext(Context::MIN_CONTEXT_SLOTS + 1, native_context().catch_context_map());
    a.Store(AccessBuilder::ForContextSlot(Context::SCOPE_INFO_INDEX), scope_info);
    a.Store(AccessBuilder::ForContextSlot(Context::PREVIOUS_INDEX), context);
...
base::Optional<MapRef> GetObjectCreateMap(JSHeapBroker* broker, HeapObjectRef prototype)
{
    MapRef standard_map = broker->target_native_context().object_function().initial_map(broker->dependencies());
...
Node* JSCreateLowering::TryAllocateArguments(Node* effect, Node* control, FrameState frame_state)
{
    ...
    for (int i = 0; i < argument_count; ++i, ++parameters_it) {
        ...
        ab.Store(AccessBuilder::ForFixedArrayElement(), jsgraph()->Constant(i), parameters_it.node());
    }
    return ab.Finish();
}
...
Node* JSCreateLowering::TryAllocateAliasedArguments(
    Node* effect, Node* control, FrameState frame_state, Node* context, const SharedFunctionInfoRef& shared, bool* has_aliased_arguments)
{
    FrameStateInfo state_info = frame_state.frame_state_info();
    int argument_count = state_info.parameter_count() - 1; // Minus receiver.
    if (argument_count == 0)
    ...
    *has_aliased_arguments = true;
    ...
    MapRef sloppy_arguments_elements_map = MakeRef(broker(), factory()->sloppy_arguments_elements_map());
    AllocationBuilder ab(jsgraph(), effect, control);
    ...
    for (int i = 0; i < mapped_count; ++i) {
        ab.Store(AccessBuilder::ForFixedArrayElement(), jsgraph()->Constant(i), jsgraph()->TheHoleConstant());
    }
    ...
    for (int i = 0; i < mapped_count; ++i) {
        ...
        a.Store(AccessBuilder::ForSloppyArgumentsElementsMappedEntry(), jsgraph()->Constant(i), jsgraph()->Constant(idx));
    }
    return a.Finish();
}
...
Node* JSCreateLowering::TryAllocateAliasedArguments(
    Node* effect, Node* control, Node* context, Node* arguments_length, const SharedFunctionInfoRef& shared, bool* has_aliased_arguments)
{
    ...
    *has_aliased_arguments = true;
    ...
    Node* arguments = effect = graph()->NewNode(simplified()->NewArgumentsElements(CreateArgumentsType::kMappedArguments, mapped_count), arguments_length, effect);
    ...
    for (int i = 0; i < mapped_count; ++i) {
        int idx = shared.context_parameters_start() + parameter_count - 1 - i;
        Node* value = graph()->NewNode(common()->Select(MachineRepresentation::kTagged), graph()->NewNode(simplified()->NumberLessThan(), jsgraph()->Constant(i), arguments_length),
            jsgraph()->Constant(idx), jsgraph()->TheHoleConstant());
        a.Store(AccessBuilder::ForSloppyArgumentsElementsMappedEntry(), jsgraph()->Constant(i), value);
    }
    return a.Finish();
}
...
base::Optional<Node*> JSCreateLowering::TryAllocateFastLiteral(Node* effect, Node* control, JSObjectRef boilerplate, AllocationType allocation, int max_depth, int* max_properties)
{
    ...
    {
        base::Optional<MapRef> current_boilerplate_map = boilerplate.map_direct_read();
    ...
    if (boilerplate.map().elements_kind() == DICTIONARY_ELEMENTS || boilerplate.map().is_dictionary_map() || !boilerplate.raw_properties_or_hash().has_value()) {
        return {};
    }
    {
        ObjectRef properties = *boilerplate.raw_properties_or_hash();
        bool const empty = properties.IsSmi() || properties.equals(MakeRef<Object>(broker(), factory()->empty_fixed_array()))
            || properties.equals(MakeRef<Object>(broker(), factory()->empty_property_array()));
        if (!empty)
            return {};
    }
    ...
    for (InternalIndex i : InternalIndex::Range(boilerplate_nof)) {
        ...
        FieldAccess access = { kTaggedBase, index.offset(), property_name.object(), MaybeHandle<Map>(), Type::Any(), MachineType::AnyTagged(), kFullWriteBarrier,
        ...
        base::Optional<ObjectRef> maybe_boilerplate_value = boilerplate.RawInobjectPropertyAt(index);
        ...
        ObjectRef uninitialized_oddball = MakeRef<HeapObject>(broker(), factory()->uninitialized_value());
        if (boilerplate_value.equals(uninitialized_oddball) || (boilerplate_value.IsHeapNumber() && boilerplate_value.AsHeapNumber().value_as_bits() == kHoleNanInt64)) {
        ...
        if (boilerplate_value.IsJSObject()) {
            ...
            base::Optional<Node*> maybe_value = TryAllocateFastLiteral(effect, control, boilerplate_object, allocation, max_depth - 1, max_properties);
            if (!maybe_value.has_value())
                return {};
            ...
            builder.Store(AccessBuilder::ForMap(), MakeRef(broker(), factory()->heap_number_map()));
            builder.Store(AccessBuilder::ForHeapNumberValue(), jsgraph()->Constant(number));
            ...
            DCHECK_IMPLIES(property_details.representation().IsSmi() && !boilerplate_value.IsSmi(), boilerplate_value.equals(uninitialized_oddball));
    ...
    base::Optional<Node*> maybe_elements = TryAllocateFastLiteralElements(effect, control, boilerplate, allocation, max_depth, max_properties);
    ...
    AllocationBuilder builder(jsgraph(), effect, control);
    builder.Allocate(boilerplate_map.instance_size(), allocation, Type::For(boilerplate_map));
    ...
    if (boilerplate.IsJSArray()) {
        ...
        builder.Store(AccessBuilder::ForJSArrayLength(boilerplate_array.map().elements_kind()), boilerplate_array.GetBoilerplateLength());
    }
    ...
    Node* effect, Node* control, JSObjectRef boilerplate, AllocationType allocation, int max_depth, int* max_properties)
{
    ...
    base::Optional<FixedArrayBaseRef> maybe_boilerplate_elements = boilerplate.elements(kRelaxedLoad);
    ...
    if (boilerplate_elements.IsFixedDoubleArray()) {
        ...
        for (int i = 0; i < elements_length; ++i) {
            ...
            elements_values[i] = value.is_hole_nan() ? jsgraph()->TheHoleConstant() : jsgraph()->Constant(value.get_scalar());
        }
    } else {
        ...
        for (int i = 0; i < elements_length; ++i) {
            ...
            if (element_value->IsJSObject()) {
                base::Optional<Node*> object = TryAllocateFastLiteral(effect, control, element_value->AsJSObject(), allocation, max_depth - 1, max_properties);
                if (!object.has_value())
                    return {};
    ...
    ElementAccess const access = boilerplate_elements.IsFixedDoubleArray() ? AccessBuilder::ForFixedDoubleArrayElement() : AccessBuilder::ForFixedArrayElement();
    for (int i = 0; i < elements_length; ++i) {
...
{
    MapRef initial_map = native_context().regexp_function().initial_map(dependencies());
    ...
    AllocationBuilder builder(jsgraph(), effect, control);
    builder.Allocate(JSRegExp::Size(), AllocationType::kYoung, Type::For(initial_map));
...
Factory* JSCreateLowering::factory() const { return jsgraph()->isolate()->factory(); }

Graph* JSCreateLowering::graph() const { return jsgraph()->graph(); }

CommonOperatorBuilder* JSCreateLowering::common() const { return jsgraph()->common(); }

SimplifiedOperatorBuilder* JSCreateLowering::simplified() const { return jsgraph()->simplified(); }

NativeContextRef JSCreateLowering::native_context() const { return broker()->target_native_context(); }
