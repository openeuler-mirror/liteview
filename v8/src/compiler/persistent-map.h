...
template <class Key, class Value, class Hasher> struct may_be_unequal<PersistentMap<Key, Value, Hasher>> {
    bool operator()(const PersistentMap<Key, Value, Hasher>& a, const PersistentMap<Key, Value, Hasher>& b) { return a.tree_ != b.tree_; }
};
...
template <class Key, class Value, class Hasher> struct PersistentMap<Key, Value, Hasher>::FocusedTree {
    ...
    {
        DCHECK(i < length);
        return reinterpret_cast<const FocusedTree**>(reinterpret_cast<byte*>(this) + offsetof(FocusedTree, path_array))[i];
    }
    const FocusedTree* path(int i) const
    {
        DCHECK(i < length);
        return reinterpret_cast<const FocusedTree* const*>(reinterpret_cast<const byte*>(this) + offsetof(FocusedTree, path_array))[i];
    }
};
...
public:
    ...
    bool operator==(HashValue other) const { return bits_ == other.bits_; }
    bool operator!=(HashValue other) const { return bits_ != other.bits_; }
    HashValue operator^(HashValue other) const { return HashValue(bits_ ^ other.bits_); }
...
public:
    ...
    bool operator!=(const double_iterator& other) { return first_ != other.first_ || second_ != other.second_; }
...
template <class Key, class Value, class Hasher> template <class F> void PersistentMap<Key, Value, Hasher>::Modify(Key key, F f)
{
    ...
    FocusedTree* tree = new (zone_->Allocate<FocusedTree>(size)) FocusedTree { KeyValue(std::move(key), std::move(new_value)), static_cast<int8_t>(length), key_hash, more, {} };
...
template <class Key, class Value, class Hasher> const typename PersistentMap<Key, Value, Hasher>::FocusedTree* PersistentMap<Key, Value, Hasher>::FindHash(HashValue hash) const
{
