...
CallDescriptor::Flags FrameStateFlagForCall(Node* node) { return OperatorProperties::HasFrameStateInput(node->op()) ? CallDescriptor::kNeedsFrameState : CallDescriptor::kNoFlags; }
...
#define REPLACE_STUB_CALL(Name)                                                                                                                                                    \
    void JSGenericLowering::LowerJS##Name(Node* node) { ReplaceWithBuiltinCall(node, Builtin::k##Name); }
...
#define DEF_UNARY_LOWERING(Name)                                                                                                                                                   \
    void JSGenericLowering::LowerJS##Name(Node* node) { ReplaceUnaryOpWithBuiltinCall(node, Builtin::k##Name, Builtin::k##Name##_WithFeedback); }
...
#define DEF_BINARY_LOWERING(Name)                                                                                                                                                  \
    void JSGenericLowering::LowerJS##Name(Node* node) { ReplaceBinaryOpWithBuiltinCall(node, Builtin::k##Name, Builtin::k##Name##_WithFeedback); }
// Binary ops.
...
bool ShouldUseMegamorphicLoadBuiltin(FeedbackSource const& source, base::Optional<NameRef> name, JSHeapBroker* broker)
{
    ProcessedFeedback const& feedback = broker->GetFeedbackForPropertyAccess(source, AccessMode::kLoad, name);
...
void JSGenericLowering::LowerJSLoadProperty(Node* node)
{
    ...
    if (outer_state->opcode() != IrOpcode::kFrameState) {
        ...
        n->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node, ShouldUseMegamorphicLoadBuiltin(p.feedback(), {}, broker()) ? Builtin::kKeyedLoadICTrampoline_Megamorphic : Builtin::kKeyedLoadICTrampoline);
    } else {
        n->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node, ShouldUseMegamorphicLoadBuiltin(p.feedback(), {}, broker()) ? Builtin::kKeyedLoadIC_Megamorphic : Builtin::kKeyedLoadIC);
    }
}
...
void JSGenericLowering::LowerJSLoadNamed(Node* node)
{
    ...
    if (!p.feedback().IsValid()) {
        ...
        node->InsertInput(zone(), 1, jsgraph()->Constant(p.name(broker())));
        node->InsertInput(zone(), 2, jsgraph()->TaggedIndexConstant(p.feedback().index()));
        ReplaceWithBuiltinCall(node, ShouldUseMegamorphicLoadBuiltin(p.feedback(), p.name(broker()), broker()) ? Builtin::kLoadIC_Megamorphic : Builtin::kLoadIC);
    }
}
...
void JSGenericLowering::LowerJSDeleteProperty(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kDeleteProperty); }
...
void JSGenericLowering::LowerJSFindNonDefaultConstructorOrConstruct(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kFindNonDefaultConstructorOrConstruct); }

void JSGenericLowering::LowerJSHasInPrototypeChain(Node* node) { ReplaceWithRuntimeCall(node, Runtime::kHasInPrototypeChain); }

void JSGenericLowering::LowerJSOrdinaryHasInstance(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kOrdinaryHasInstance); }
...
void JSGenericLowering::LowerJSCreate(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kFastNewObject); }
...
void JSGenericLowering::LowerJSCreateArray(Node* node)
{
    ...
    Node* type_info = site.has_value() ? jsgraph()->Constant(site.value()) : jsgraph()->UndefinedConstant();
...
void JSGenericLowering::LowerJSCreateObject(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kCreateObjectWithoutProperties); }

void JSGenericLowering::LowerJSParseInt(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kParseInt); }

void JSGenericLowering::LowerJSRegExpTest(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kRegExpPrototypeTestFast); }
...
void JSGenericLowering::LowerJSCreateIterResultObject(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kCreateIterResultObject); }
...
void JSGenericLowering::LowerJSCreateTypedArray(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kCreateTypedArray); }
...
void JSGenericLowering::LowerJSCreateEmptyLiteralObject(Node* node) { ReplaceWithBuiltinCall(node, Builtin::kCreateEmptyLiteralObject); }
...
void JSGenericLowering::LowerJSGetImportMeta(Node* node) { ReplaceWithRuntimeCall(node, Runtime::kGetImportMetaObject); }
...
void JSGenericLowering::LowerJSStackCheck(Node* node)
{
    ...
    Node* limit = effect = graph()->NewNode(
        machine()->Load(MachineType::Pointer()), jsgraph()->ExternalConstant(ExternalReference::address_of_jslimit(isolate())), jsgraph()->IntPtrConstant(0), effect, control);

    StackCheckKind stack_check_kind = StackCheckKindOfJSStackCheck(node->op());
    Node* check = effect = graph()->NewNode(machine()->StackPointerGreaterThan(stack_check_kind), limit, effect);
...
void JSGenericLowering::LowerJSDebugger(Node* node) { ReplaceWithRuntimeCall(node, Runtime::kHandleDebuggerStatement); }
...
CommonOperatorBuilder* JSGenericLowering::common() const { return jsgraph()->common(); }

MachineOperatorBuilder* JSGenericLowering::machine() const { return jsgraph()->machine(); }
