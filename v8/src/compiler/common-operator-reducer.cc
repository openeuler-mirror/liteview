...
CommonOperatorReducer::CommonOperatorReducer(
    Editor* editor, Graph* graph, JSHeapBroker* broker, CommonOperatorBuilder* common, MachineOperatorBuilder* machine, Zone* temp_zone, BranchSemantics branch_semantics)
...
Reduction CommonOperatorReducer::ReduceBranch(Node* node)
{
    ...
    if (cond->opcode() == IrOpcode::kBooleanNot
        || (cond->opcode() == IrOpcode::kSelect && DecideCondition(cond->InputAt(1)) == Decision::kFalse && DecideCondition(cond->InputAt(2)) == Decision::kTrue)) {
...
Reduction CommonOperatorReducer::ReduceDeoptimizeConditional(Node* node)
{
    ...
    if (condition->opcode() == IrOpcode::kBooleanNot) {
        ...
        NodeProperties::ChangeOp(node, condition_is_true ? common()->DeoptimizeIf(p.reason(), p.feedback()) : common()->DeoptimizeUnless(p.reason(), p.feedback()));
        return Changed(node);
    }
...
Reduction CommonOperatorReducer::ReduceMerge(Node* node)
{
    ...
    if (node->InputCount() == 2) {
        ...
        if (if_true->opcode() == IrOpcode::kIfTrue && if_false->opcode() == IrOpcode::kIfFalse && if_true->InputAt(0) == if_false->InputAt(0) && if_true->OwnedBy(node)
