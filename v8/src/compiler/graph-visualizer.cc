...
void JsonPrintBytecodeSource(std::ostream& os, int source_id, std::unique_ptr<char[]> function_name, Handle<BytecodeArray> bytecode_array)
{
    os << "\"" << source_id << "\" : {";
...
void JsonPrintFunctionSource(
    std::ostream& os, int source_id, std::unique_ptr<char[]> function_name, Handle<Script> script, Isolate* isolate, Handle<SharedFunctionInfo> shared, bool with_key)
{
    if (with_key)
        os << "\"" << source_id << "\" : ";
...
void JsonPrintAllSourceWithPositions(std::ostream& os, OptimizedCompilationInfo* info, Isolate* isolate)
{
    ...
    Handle<Script> script
        = (info->shared_info().is_null() || info->shared_info()->script() == Object()) ? Handle<Script>() : handle(Script::cast(info->shared_info()->script()), isolate);
    JsonPrintFunctionSource(
        os, -1, info->shared_info().is_null() ? std::unique_ptr<char[]>(new char[1] { 0 }) : info->shared_info()->DebugNameCStr(), script, isolate, info->shared_info(), true);
    const auto& inlined = info->inlined_functions();
    SourceIdAssigner id_assigner(info->inlined_functions().size());
    for (unsigned id = 0; id < inlined.size(); id++) {
        os << ", ";
        ...
        JsonPrintFunctionSource(os, source_id, shared->DebugNameCStr(), handle(Script::cast(shared->script()), isolate), isolate, shared, true);
    }
    os << "}, ";
    os << "\"inlinings\" : {";
...
std::unique_ptr<char[]> GetVisualizerLogFileName(OptimizedCompilationInfo* info, const char* optional_base_dir, const char* phase, const char* suffix)
{
    ...
    if (v8_flags.trace_file_names && info->has_shared_info() && info->shared_info()->script().IsScript()) {
...
static const char* SafeMnemonic(Node* node) { return node == nullptr ? "null" : node->op()->mnemonic(); }
...
void JSONGraphWriter::PrintNode(Node* node, bool is_live)
{
    ...
    os_ << ",\"opinfo\":\"" << node->op()->ValueInputCount() << " v " << node->op()->EffectInputCount() << " eff " << node->op()->ControlInputCount() << " ctrl in, "
        << node->op()->ValueOutputCount() << " v " << node->op()->EffectOutputCount() << " eff " << node->op()->ControlOutputCount() << " ctrl out\"";
...
public:
    ...
    void PrintLiveRanges(const char* phase, const TopTierRegisterAllocationData* data);
...
void GraphC1Visualizer::PrintCompilation(const OptimizedCompilationInfo* info)
{
    ...
    PrintLongProperty("date", static_cast<int64_t>(V8::GetCurrentPlatform()->CurrentClockTimeMillis()));
}
...
void GraphC1Visualizer::PrintSchedule(const char* phase, const Schedule* schedule, const SourcePositionTable* positions, const InstructionSequence* instructions)
{
    Tag tag(this, "cfg");
...
void GraphC1Visualizer::PrintLiveRanges(const char* phase, const TopTierRegisterAllocationData* data)
{
...
void GraphC1Visualizer::PrintLiveRange(const LiveRange* range, const char* type, int vreg)
{
    if (range != nullptr && !range->IsEmpty()) {
        ...
        for (const UseInterval* interval = range->first_interval(); interval != nullptr; interval = interval->next()) {
...
std::ostream& operator<<(std::ostream& os, const AsC1VRegisterAllocationData& ac)
{
    ...
    if (ac.data_->type() == RegisterAllocationData::kTopTier) {
        AccountingAllocator allocator;
        Zone tmp_zone(&allocator, ZONE_NAME);
        GraphC1Visualizer(os, &tmp_zone).PrintLiveRanges(ac.phase_, TopTierRegisterAllocationData::cast(ac.data_));
    }
    return os;
}
...
std::ostream& operator<<(std::ostream& os, const LiveRangeAsJSON& live_range_json)
{
    ...
    os << "],\"uses\":[";
    first = true;
    for (UsePosition* current_pos = range.first_pos(); current_pos != nullptr; current_pos = current_pos->next()) {
        if (first) {
            first = false;
        } else {
            os << ",";
...
{
    if (ac.data_.type() == RegisterAllocationData::kTopTier) {
        const TopTierRegisterAllocationData& ac_data = TopTierRegisterAllocationData::cast(ac.data_);
