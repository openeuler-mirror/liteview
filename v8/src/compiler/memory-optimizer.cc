...
MemoryOptimizer::MemoryOptimizer(JSGraph* jsgraph, Zone* zone, MemoryLowering::AllocationFolding allocation_folding, const char* function_debug_name, TickCounter* tick_counter)
    : graph_assembler_(jsgraph, zone)
    , memory_lowering_(jsgraph, zone, &graph_assembler_, allocation_folding, WriteBarrierAssertFailed, function_debug_name)
    , jsgraph_(jsgraph)
...
void MemoryOptimizer::VisitAllocateRaw(Node* node, AllocationState const* state)
{
    ...
    Reduction reduction = memory_lowering()->ReduceAllocateRaw(node, allocation_type, allocation.allow_large_objects(), &state);
...
void MemoryOptimizer::VisitLoadFromObject(Node* node, AllocationState const* state)
{
...
void MemoryOptimizer::VisitStoreToObject(Node* node, AllocationState const* state)
{
...
void MemoryOptimizer::VisitStoreElement(Node* node, AllocationState const* state)
{
...
void MemoryOptimizer::VisitOtherEffect(Node* node, AllocationState const* state) { EnqueueUses(node, state); }
...
void MemoryOptimizer::EnqueueUse(Node* node, int index, AllocationState const* state)
{
