...
void BranchElimination::SimplifyBranchCondition(Node* branch)
{
  ...
  //                                   second_true  second_false
    ...
    for (int i = 0; i != input_count; ++i) {
        ...
        if (phase_ == kEARLY) {
            phi_inputs.emplace_back(condition_value ? jsgraph()->TrueConstant() : jsgraph()->FalseConstant());
        } else {
            phi_inputs.emplace_back(condition_value ? graph->NewNode(jsgraph()->common()->Int32Constant(1)) : graph->NewNode(jsgraph()->common()->Int32Constant(0)));
        }
    }
    phi_inputs.emplace_back(merge);
    Node* new_phi
        = graph->NewNode(common()->Phi(phase_ == kEARLY ? MachineRepresentation::kTagged : MachineRepresentation::kWord32, input_count), input_count + 1, &phi_inputs.at(0));
...
Reduction BranchElimination::ReduceStart(Node* node) { return UpdateStates(node, ControlPathConditions(zone())); }
...
CommonOperatorBuilder* BranchElimination::common() const { return jsgraph()->common(); }
