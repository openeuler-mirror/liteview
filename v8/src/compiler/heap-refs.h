...
template <class T> class TinyRef {
private:
    using RefType = typename ref_traits<T>::ref_type;

public:
    explicit TinyRef(const RefType& ref)
        : TinyRef(ref.data_)
    {
    }
    RefType AsRef(JSHeapBroker* broker) const;
    static base::Optional<RefType> AsOptionalRef(JSHeapBroker* broker, base::Optional<TinyRef<T>> ref)
    {
        if (!ref.has_value())
            return {};
...
private:
    explicit TinyRef(ObjectData* data)
        : data_(data)
    {
...
#undef HEAP_AS_METHOD_DECL
    ...
    struct Hash {
        size_t operator()(const ObjectRef& ref) const { return base::hash_combine(ref.object().address()); }
    };
    struct Equal {
        bool operator()(const ObjectRef& lhs, const ObjectRef& rhs) const { return lhs.equals(rhs); }
    };
    struct Less {
        bool operator()(const ObjectRef& lhs, const ObjectRef& rhs) const { return lhs.data_ < rhs.data_; }
    };
...
private:
    ...
    template <class T> friend class TinyRef;
...
template <class T> using ZoneRefUnorderedSet = ZoneUnorderedSet<T, ObjectRef::Hash, ObjectRef::Equal>;

template <class K, class V> using ZoneRefMap = ZoneMap<K, V, ObjectRef::Less>;
...
public:
    ...
    base::Optional<ObjectRef> GetOwnConstantElement(const FixedArrayBaseRef& elements_ref, uint32_t index, CompilationDependencies* dependencies) const;
    ...
    base::Optional<Object> GetOwnConstantElementFromHeap(FixedArrayBase elements, ElementsKind elements_kind, uint32_t index) const;
    ...
    base::Optional<ObjectRef> GetOwnFastDataProperty(Representation field_representation, FieldIndex index, CompilationDependencies* dependencies) const;

    // Return the value of the dictionary property at {index} in the dictionary
    // if {index} is known to be an own data property of the object.
    base::Optional<ObjectRef> GetOwnDictionaryProperty(InternalIndex index, CompilationDependencies* dependencies) const;
...
public:
    ...
    bool PrototypeRequiresRuntimeLookup(CompilationDependencies* dependencies) const;
    ...
    int InitialMapInstanceSizeWithMinSlack(CompilationDependencies* dependencies) const;
    FeedbackCellRef raw_feedback_cell(CompilationDependencies* dependencies) const;
    base::Optional<FeedbackVectorRef> feedback_vector(CompilationDependencies* dependencies) const;
};
...
public:
    ...
    base::Optional<ObjectRef> GetStrongValue(InternalIndex descriptor_index) const;
};
...
#undef DEF_TESTER
    ...
    bool HasOnlyStablePrototypesWithFastElements(ZoneVector<MapRef>* prototype_maps);
    ...
    base::Optional<ObjectRef> GetStrongValue(InternalIndex descriptor_number) const;
...
struct HolderLookupResult {
    HolderLookupResult(CallOptimization::HolderLookup lookup_ = CallOptimization::kHolderNotFound, base::Optional<JSObjectRef> holder_ = base::nullopt)
        : lookup(lookup_)
        , holder(holder_)
    {
    }
...
public:
    ...
    base::Optional<ObjectRef> GetOwnCowElement(FixedArrayBaseRef elements_ref, uint32_t index) const;
...
#undef DECL_ACCESSOR
    ...
    bool IsInlineable() const { return GetInlineability() == SharedFunctionInfo::kIsInlineable; }
};
