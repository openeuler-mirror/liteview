    ...
    , instance_node_(nullptr)
{
    ...
    for (Node* start_use : mcgraph->graph()->start()->uses()) {
        if (start_use->opcode() == IrOpcode::kParameter && ParameterIndexOf(start_use->op()) == 0) {
...
{
    Node* isolate_root = gasm_.LoadImmutable(MachineType::Pointer(), instance_node_, WasmInstanceObject::kIsolateRootOffset - kHeapObjectTag);
    return gasm_.LoadImmutable(MachineType::Pointer(), isolate_root, IsolateData::root_slot_offset(index));
}
...
Reduction WasmGCLowering::ReduceWasmTypeCheck(Node* node)
{
    ...
    if (object_can_be_null) {
        ...
        gasm_.GotoIf(gasm_.TaggedEqual(object, Null()), &end_label, BranchHint::kFalse, gasm_.Int32Constant(kResult));
    }
    ...
    Node* maybe_match
        = gasm_.LoadImmutableFromObject(MachineType::TaggedPointer(), type_info, wasm::ObjectAccess::ToTagged(WasmTypeInfo::kSupertypesOffset + kTaggedSize * rtt_depth));
...
Reduction WasmGCLowering::ReduceWasmTypeCast(Node* node)
{
    ...
    if (object_can_be_null) {
        gasm_.GotoIf(gasm_.TaggedEqual(object, Null()), &end_label, BranchHint::kFalse);
    }
...
Reduction WasmGCLowering::ReduceIsNotNull(Node* node)
{
    ...
    Node* object = NodeProperties::GetValueInput(node, 0);
    return Replace(gasm_.Word32Equal(gasm_.TaggedEqual(object, Null()), gasm_.Int32Constant(0)));
}
...
Reduction WasmGCLowering::ReduceRttCanon(Node* node)
{
    ...
    Node* maps_list = gasm_.LoadImmutable(MachineType::TaggedPointer(), instance_node_, WasmInstanceObject::kManagedObjectMapsOffset - kHeapObjectTag);
...
Reduction WasmGCLowering::ReduceWasmExternInternalize(Node* node)
{
    ...
    if (!v8_flags.wasm_gc_js_interop) {
        Node* context = gasm_.LoadImmutable(MachineType::TaggedPointer(), instance_node_, WasmInstanceObject::kNativeContextOffset - kHeapObjectTag);
        Node* obj = gasm_.CallBuiltin(Builtin::kWasmGetOwnProperty, Operator::kEliminatable, object, RootNode(RootIndex::kwasm_wrapped_object_symbol), context);
        ...
        Node* is_undefined = gasm_.TaggedEqual(obj, RootNode(RootIndex::kUndefinedValue));
...
Reduction WasmGCLowering::ReduceWasmExternExternalize(Node* node)
{
    ...
    if (!v8_flags.wasm_gc_js_interop) {
        ...
        gasm_.Bind(&wrap);
        Node* context = gasm_.LoadImmutable(MachineType::TaggedPointer(), instance_node_, WasmInstanceObject::kNativeContextOffset - kHeapObjectTag);
        Node* wrapped = gasm_.CallBuiltin(Builtin::kWasmAllocateObjectWrapper, Operator::kEliminatable, object, context);
