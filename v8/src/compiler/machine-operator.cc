...
bool operator==(StoreRepresentation lhs, StoreRepresentation rhs) { return lhs.representation() == rhs.representation() && lhs.write_barrier_kind() == rhs.write_barrier_kind(); }

bool operator!=(StoreRepresentation lhs, StoreRepresentation rhs) { return !(lhs == rhs); }

size_t hash_value(StoreRepresentation rep) { return base::hash_combine(rep.representation(), rep.write_barrier_kind()); }

std::ostream& operator<<(std::ostream& os, StoreRepresentation rep) { return os << rep.representation() << ", " << rep.write_barrier_kind(); }
...
bool operator!=(AtomicStoreParameters lhs, AtomicStoreParameters rhs) { return !(lhs == rhs); }

size_t hash_value(AtomicStoreParameters params) { return base::hash_combine(hash_value(params.store_representation()), params.order(), params.kind()); }

std::ostream& operator<<(std::ostream& os, AtomicStoreParameters params) { return os << params.store_representation() << ", " << params.order(); }
...
bool operator!=(AtomicLoadParameters lhs, AtomicLoadParameters rhs) { return !(lhs == rhs); }

size_t hash_value(AtomicLoadParameters params) { return base::hash_combine(params.representation(), params.order(), params.kind()); }

std::ostream& operator<<(std::ostream& os, AtomicLoadParameters params) { return os << params.representation() << ", " << params.order(); }

bool operator==(AtomicOpParameters lhs, AtomicOpParameters rhs) { return lhs.type() == rhs.type() && lhs.kind() == rhs.kind(); }

bool operator!=(AtomicOpParameters lhs, AtomicOpParameters rhs) { return !(lhs == rhs); }

size_t hash_value(AtomicOpParameters params) { return base::hash_combine(params.type(), params.kind()); }

std::ostream& operator<<(std::ostream& os, AtomicOpParameters params) { return os << params.type() << ", " << params.kind(); }
...
size_t hash_value(LoadTransformParameters params) { return base::hash_combine(params.kind, params.transformation); }

std::ostream& operator<<(std::ostream& os, LoadTransformParameters params) { return os << "(" << params.kind << " " << params.transformation << ")"; }
...
bool operator==(LoadTransformParameters lhs, LoadTransformParameters rhs) { return lhs.transformation == rhs.transformation && lhs.kind == rhs.kind; }

bool operator!=(LoadTransformParameters lhs, LoadTransformParameters rhs) { return !(lhs == rhs); }

size_t hash_value(LoadLaneParameters params) { return base::hash_combine(params.kind, params.rep, params.laneidx); }
...
bool operator==(LoadLaneParameters lhs, LoadLaneParameters rhs) { return lhs.kind == rhs.kind && lhs.rep == rhs.rep && lhs.laneidx == rhs.laneidx; }

LoadRepresentation LoadRepresentationOf(Operator const* op)
{
    DCHECK(IrOpcode::kLoad == op->opcode() || IrOpcode::kProtectedLoad == op->opcode() || IrOpcode::kUnalignedLoad == op->opcode() || IrOpcode::kLoadImmutable == op->opcode());
...
size_t hash_value(StoreLaneParameters params) { return base::hash_combine(params.kind, params.rep, params.laneidx); }
...
bool operator==(StoreLaneParameters lhs, StoreLaneParameters rhs) { return lhs.kind == rhs.kind && lhs.rep == rhs.rep && lhs.laneidx == rhs.laneidx; }

bool operator==(StackSlotRepresentation lhs, StackSlotRepresentation rhs) { return lhs.size() == rhs.size() && lhs.alignment() == rhs.alignment(); }

bool operator!=(StackSlotRepresentation lhs, StackSlotRepresentation rhs) { return !(lhs == rhs); }

size_t hash_value(StackSlotRepresentation rep) { return base::hash_combine(rep.size(), rep.alignment()); }

std::ostream& operator<<(std::ostream& os, StackSlotRepresentation rep) { return os << rep.size() << ", " << rep.alignment(); }
...
#define STACK_SLOT_CACHED_SIZES_ALIGNMENTS_LIST(V) V(4, 0) V(8, 0) V(16, 0) V(4, 4) V(8, 8) V(16, 16)

struct StackSlotOperator : public Operator1<StackSlotRepresentation> {
    explicit StackSlotOperator(int size, int alignment)
        : Operator1<StackSlotRepresentation>(IrOpcode::kStackSlot, Operator::kNoDeopt | Operator::kNoThrow, "StackSlot", 0, 0, 0, 1, 0, 0, StackSlotRepresentation(size, alignment))
    {
    }
};
...
#define PURE(Name, properties, value_input_count, control_input_count, output_count)                                                                                               \
    ...
    struct ArchitectureDefaultTruncateFloat32ToUint32Operator final : public Operator1<TruncateKind> {
        ArchitectureDefaultTruncateFloat32ToUint32Operator()
            : Operator1<TruncateKind>(IrOpcode::kTruncateFloat32ToUint32, Operator::kPure, "TruncateFloat32ToUint32", 1, 0, 0, 1, 0, 0, TruncateKind::kArchitectureDefault)
        {
        }
    };
    ...
    struct SetOverflowToMinTruncateFloat32ToUint32Operator final : public Operator1<TruncateKind> {
        SetOverflowToMinTruncateFloat32ToUint32Operator()
            : Operator1<TruncateKind>(IrOpcode::kTruncateFloat32ToUint32, Operator::kPure, "TruncateFloat32ToUint32", 1, 0, 0, 1, 0, 0, TruncateKind::kSetOverflowToMin)
        {
        }
    };
    ...
    struct ArchitectureDefaultTruncateFloat32ToInt32Operator final : public Operator1<TruncateKind> {
        ArchitectureDefaultTruncateFloat32ToInt32Operator()
            : Operator1<TruncateKind>(IrOpcode::kTruncateFloat32ToInt32, Operator::kPure, "TruncateFloat32ToInt32", 1, 0, 0, 1, 0, 0, TruncateKind::kArchitectureDefault)
        {
        }
    };
    ...
    struct SetOverflowToMinTruncateFloat32ToInt32Operator final : public Operator1<TruncateKind> {
        SetOverflowToMinTruncateFloat32ToInt32Operator()
            : Operator1<TruncateKind>(IrOpcode::kTruncateFloat32ToInt32, Operator::kPure, "TruncateFloat32ToInt32", 1, 0, 0, 1, 0, 0, TruncateKind::kSetOverflowToMin)
        {
        }
    };
...
#define LOAD(Type)                                                                                                                                                                 \
    ...
    struct ProtectedLoad##Type##Operator final : public Operator1<LoadRepresentation> {                                                                                            \
        ProtectedLoad##Type##Operator()                                                                                                                                            \
            : Operator1<LoadRepresentation>(IrOpcode::kProtectedLoad, Operator::kNoDeopt | Operator::kNoThrow, "ProtectedLoad", 2, 1, 1, 1, 1, 0, MachineType::Type())             \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
    ...
    struct KIND##LoadTransform##TYPE##Operator final : public Operator1<LoadTransformParameters> {                                                                                 \
        KIND##LoadTransform##TYPE##Operator()                                                                                                                                      \
            : Operator1<LoadTransformParameters>(IrOpcode::kLoadTransform,                                                                                                         \
                MemoryAccessKind::k##KIND == MemoryAccessKind::kProtected ? Operator::kNoDeopt | Operator::kNoThrow : Operator::kEliminatable, #KIND "LoadTransform", 2, 1, 1, 1,  \
                1, 0, LoadTransformParameters { MemoryAccessKind::k##KIND, LoadTransformation::k##TYPE })                                                                          \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
...
#define STACKSLOT(Size, Alignment)                                                                                                                                                 \
    struct StackSlotOfSize##Size##OfAlignment##Alignment##Operator final : public StackSlotOperator {                                                                              \
        StackSlotOfSize##Size##OfAlignment##Alignment##Operator()                                                                                                                  \
            : StackSlotOperator(Size, Alignment)                                                                                                                                   \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
    StackSlotOfSize##Size##OfAlignment##Alignment##Operator kStackSlotOfSize##Size##OfAlignment##Alignment;
    ...
    struct UnalignedStore##Type##Operator final : public Operator1<UnalignedStoreRepresentation> {                                                                                 \
        UnalignedStore##Type##Operator()                                                                                                                                           \
            : Operator1<UnalignedStoreRepresentation>(                                                                                                                             \
                IrOpcode::kUnalignedStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "UnalignedStore", 3, 1, 1, 0, 1, 0, MachineRepresentation::Type)           \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
    struct ProtectedStore##Type##Operator : public Operator1<StoreRepresentation> {                                                                                                \
        explicit ProtectedStore##Type##Operator()                                                                                                                                  \
            : Operator1<StoreRepresentation>(IrOpcode::kProtectedStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "ProtectedStore", 3, 1, 1, 0, 1, 0,           \
                StoreRepresentation(MachineRepresentation::Type, kNoWriteBarrier))                                                                                                 \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
...
#define ATOMIC_STORE_WITH_KIND(Type, Kind)                                                                                                                                         \
    struct Word32SeqCstStore##Type##Kind##Operator : public Operator1<AtomicStoreParameters> {                                                                                     \
        Word32SeqCstStore##Type##Kind##Operator()                                                                                                                                  \
            : Operator1<AtomicStoreParameters>(IrOpcode::kWord32AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word32AtomicStore", 3, 1, 1, 0, 1, 0,   \
                AtomicStoreParameters(MachineRepresentation::Type, kNoWriteBarrier, AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))                                        \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
...
#define ATOMIC_STORE_WITH_KIND(Type, Kind)                                                                                                                                         \
    struct Word64SeqCstStore##Type##Kind##Operator : public Operator1<AtomicStoreParameters> {                                                                                     \
        Word64SeqCstStore##Type##Kind##Operator()                                                                                                                                  \
            : Operator1<AtomicStoreParameters>(IrOpcode::kWord64AtomicStore, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "Word64AtomicStore", 3, 1, 1, 0, 1, 0,   \
                AtomicStoreParameters(MachineRepresentation::Type, kNoWriteBarrier, AtomicMemoryOrder::kSeqCst, MemoryAccessKind::k##Kind))                                        \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
    ...
    struct op##type##kind##Operator : public Operator1<AtomicOpParameters> {                                                                                                       \
        op##type##kind##Operator()                                                                                                                                                 \
            : Operator1<AtomicOpParameters>(                                                                                                                                       \
                IrOpcode::k##op, Operator::kNoDeopt | Operator::kNoThrow, #op, 3, 1, 1, 1, 1, 0, AtomicOpParameters(MachineType::type(), MemoryAccessKind::k##kind))               \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
...
#define ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Kind)                                                                                                                              \
    struct Word32AtomicCompareExchange##Type##Kind##Operator : public Operator1<AtomicOpParameters> {                                                                              \
        Word32AtomicCompareExchange##Type##Kind##Operator()                                                                                                                        \
            : Operator1<AtomicOpParameters>(IrOpcode::kWord32AtomicCompareExchange, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicCompareExchange", 4, 1, 1, 1, 1, 0,      \
...
#define ATOMIC_COMPARE_EXCHANGE_WITH_KIND(Type, Kind)                                                                                                                              \
    struct Word64AtomicCompareExchange##Type##Kind##Operator : public Operator1<AtomicOpParameters> {                                                                              \
        Word64AtomicCompareExchange##Type##Kind##Operator()                                                                                                                        \
            : Operator1<AtomicOpParameters>(IrOpcode::kWord64AtomicCompareExchange, Operator::kNoDeopt | Operator::kNoThrow, "Word64AtomicCompareExchange", 4, 1, 1, 1, 1, 0,      \
                AtomicOpParameters(MachineType::Type(), MemoryAccessKind::k##Kind))                                                                                                \
        {                                                                                                                                                                          \
        }                                                                                                                                                                          \
    };                                                                                                                                                                             \
...
#undef ATOMIC_COMPARE_EXCHANGE
    ...
    struct Word32SeqCstPairStoreOperator : public Operator1<AtomicMemoryOrder> {
        Word32SeqCstPairStoreOperator()
            : Operator1<AtomicMemoryOrder>(
                IrOpcode::kWord32AtomicPairStore, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairStore", 4, 1, 1, 0, 1, 0, AtomicMemoryOrder::kSeqCst)
        {
        }
    };
...
#define PURE(Name, properties, value_input_count, control_input_count, output_count)                                                                                               \
    const OptionalOperator MachineOperatorBuilder::Name() { return OptionalOperator(flags_ & k##Name, &cache_.k##Name); }
PURE_OPTIONAL_OP_LIST(PURE)
#undef PURE
...
#define LOAD_LANE_KIND(TYPE, KIND, LANEIDX)                                                                                                                                        \
    if (kind == MemoryAccessKind::k##KIND && rep == MachineType::TYPE() && laneidx == LANEIDX) {                                                                                   \
        return zone_->New<Operator1<LoadLaneParameters>>(IrOpcode::kLoadLane,                                                                                                      \
            MemoryAccessKind::k##KIND == MemoryAccessKind::kProtected ? Operator::kNoDeopt | Operator::kNoThrow : Operator::kEliminatable, "LoadLane", 3, 1, 1, 1, 1, 0,           \
            LoadLaneParameters { MemoryAccessKind::k##KIND, LoadRepresentation::TYPE(), LANEIDX });                                                                                \
    }
...
#define STORE_LANE_KIND(REP, KIND, LANEIDX)                                                                                                                                        \
    if (kind == MemoryAccessKind::k##KIND && rep == MachineRepresentation::REP && laneidx == LANEIDX) {                                                                            \
        return zone_->New<Operator1<StoreLaneParameters>>(IrOpcode::kStoreLane, Operator::kNoDeopt | Operator::kNoRead | Operator::kNoThrow, "StoreLane", 3, 1, 1, 0, 1, 0,        \
            StoreLaneParameters { MemoryAccessKind::k##KIND, MachineRepresentation::REP, LANEIDX });                                                                               \
    }
...
const Operator* MachineOperatorBuilder::StackSlot(MachineRepresentation rep, int alignment) { return StackSlot(1 << ElementSizeLog2Of(rep), alignment); }
...
const Operator* MachineOperatorBuilder::BitcastWordToTagged() { return &cache_.kBitcastWordToTagged; }

const Operator* MachineOperatorBuilder::BitcastTaggedToWord() { return &cache_.kBitcastTaggedToWord; }

const Operator* MachineOperatorBuilder::BitcastMaybeObjectToWord() { return &cache_.kBitcastMaybeObjectToWord; }

const Operator* MachineOperatorBuilder::AbortCSADcheck() { return &cache_.kAbortCSADcheck; }

const Operator* MachineOperatorBuilder::DebugBreak() { return &cache_.kDebugBreak; }

const Operator* MachineOperatorBuilder::Comment(const char* msg) { return zone_->New<CommentOperator>(msg); }
...
#define LOAD(Type)                                                                                                                                                                 \
    if (params.representation() == MachineType::Type()) {                                                                                                                          \
        return zone_->New<Operator1<AtomicLoadParameters>>(IrOpcode::kWord32AtomicLoad, Operator::kNoProperties, "Word32AtomicLoad", 2, 1, 1, 1, 1, 0, params);                    \
    }
...
#define CACHED_STORE_WITH_KIND(kRep, Kind)                                                                                                                                         \
    if (params.representation() == MachineRepresentation::kRep && params.order() == AtomicMemoryOrder::kSeqCst && params.kind() == MemoryAccessKind::k##Kind) {                    \
...
#define LOAD(Type)                                                                                                                                                                 \
    if (params.representation() == MachineType::Type()) {                                                                                                                          \
        return zone_->New<Operator1<AtomicLoadParameters>>(IrOpcode::kWord64AtomicLoad, Operator::kNoProperties, "Word64AtomicLoad", 2, 1, 1, 1, 1, 0, params);                    \
    }
...
const Operator* MachineOperatorBuilder::Word32AtomicPairLoad(AtomicMemoryOrder order)
{
    ...
    return zone_->New<Operator1<AtomicMemoryOrder>>(IrOpcode::kWord32AtomicPairLoad, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairLoad", 2, 1, 1, 2, 1, 0, order);
}
...
const Operator* MachineOperatorBuilder::Word32AtomicPairStore(AtomicMemoryOrder order)
{
    ...
    return zone_->New<Operator1<AtomicMemoryOrder>>(IrOpcode::kWord32AtomicPairStore, Operator::kNoDeopt | Operator::kNoThrow, "Word32AtomicPairStore", 4, 1, 1, 0, 1, 0, order);
}

const Operator* MachineOperatorBuilder::Word32AtomicPairAdd() { return &cache_.kWord32AtomicPairAdd; }

const Operator* MachineOperatorBuilder::Word32AtomicPairSub() { return &cache_.kWord32AtomicPairSub; }

const Operator* MachineOperatorBuilder::Word32AtomicPairAnd() { return &cache_.kWord32AtomicPairAnd; }

const Operator* MachineOperatorBuilder::Word32AtomicPairOr() { return &cache_.kWord32AtomicPairOr; }

const Operator* MachineOperatorBuilder::Word32AtomicPairXor() { return &cache_.kWord32AtomicPairXor; }

const Operator* MachineOperatorBuilder::Word32AtomicPairExchange() { return &cache_.kWord32AtomicPairExchange; }

const Operator* MachineOperatorBuilder::Word32AtomicPairCompareExchange() { return &cache_.kWord32AtomicPairCompareExchange; }
...
#define EXTRACT_LANE_OP(Type, Sign, lane_count)                                                                                                                                    \
    ...
    {                                                                                                                                                                              \
        ...
        return zone_->New<Operator1<int32_t>>(IrOpcode::k##Type##ExtractLane##Sign, Operator::kPure, "" #Type "ExtractLane" #Sign, 1, 0, 0, 1, 0, 0, lane_index);                  \
    }
...
bool operator==(S128ImmediateParameter const& lhs, S128ImmediateParameter const& rhs) { return (lhs.immediate() == rhs.immediate()); }

bool operator!=(S128ImmediateParameter const& lhs, S128ImmediateParameter const& rhs) { return !(lhs == rhs); }

size_t hash_value(S128ImmediateParameter const& p) { return base::hash_range(p.immediate().begin(), p.immediate().end()); }
...
{
    return zone_->New<Operator1<S128ImmediateParameter>>(IrOpcode::kI8x16Shuffle, Operator::kPure, "Shuffle", 2, 0, 0, 1, 0, 0, S128ImmediateParameter(shuffle));
}
