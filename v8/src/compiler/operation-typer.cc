    ...
    , cache_(TypeCache::Get())
{
    ...
    singleton_empty_string_ = Type::Constant(broker, factory->empty_string(), zone);
    ...
    falsish_ = Type::Union(
        Type::Undetectable(), Type::Union(Type::Union(singleton_false_, cache_->kZeroish, zone), Type::Union(singleton_empty_string_, Type::Hole(), zone), zone), zone);
    truish_ = Type::Union(singleton_true_, Type::Union(Type::DetectableReceiver(), Type::Symbol(), zone), zone);
}

Type OperationTyper::Merge(Type left, Type right) { return Type::Union(left, right, zone()); }
...
Type OperationTyper::SpeculativeBigIntAsIntN(Type) { return Type::SignedBigInt64(); }

Type OperationTyper::SpeculativeBigIntAsUintN(Type) { return Type::UnsignedBigInt64(); }
...
Type OperationTyper::NumberMultiply(Type lhs, Type rhs)
{
    ...
    bool maybe_nan = lhs.Maybe(Type::NaN()) || rhs.Maybe(Type::NaN()) || (lhs.Maybe(cache_->kZeroish) && (rhs.Min() == -V8_INFINITY || rhs.Max() == V8_INFINITY))
    ...
    bool maybe_minuszero
        = lhs.Maybe(Type::MinusZero()) || rhs.Maybe(Type::MinusZero()) || (lhs.Maybe(cache_->kZeroish) && rhs.Min() < 0.0) || (rhs.Maybe(cache_->kZeroish) && lhs.Min() < 0.0);
    if (lhs.Maybe(Type::MinusZero())) {
        lhs = Type::Union(lhs, cache_->kSingletonZero, zone());
        lhs = Type::Intersect(lhs, Type::PlainNumber(), zone());
    }
...
Type OperationTyper::CheckNumber(Type type) { return Type::Intersect(type, Type::Number(), zone()); }

Type OperationTyper::TypeTypeGuard(const Operator* sigma_op, Type input) { return Type::Intersect(input, TypeGuardTypeOf(sigma_op), zone()); }
