...
Int64Lowering::Int64Lowering(Graph* graph, MachineOperatorBuilder* machine, CommonOperatorBuilder* common, SimplifiedOperatorBuilder* simplified, Zone* zone,
    const wasm::WasmModule* module, Signature<MachineRepresentation>* signature, std::unique_ptr<Int64LoweringSpecialCase> special_case)
    : graph_(graph)
    , machine_(machine)
    , common_(common)
...
int GetReturnCountAfterLowering(const CallDescriptor* call_descriptor) { return GetReturnIndexAfterLowering(call_descriptor, static_cast<int>(call_descriptor->ReturnCount())); }
...
void Int64Lowering::LowerNode(Node* node)
{
    ...
    case IrOpcode::kLoadFromObject: {
        ...
        LowerLoadOperator(node, access.machine_type.representation(), simplified()->LoadFromObject(ObjectAccess(MachineType::Int32(), access.write_barrier_kind)));
        break;
    }
    case IrOpcode::kLoadImmutableFromObject: {
        ObjectAccess access = ObjectAccessOf(node->op());
        LowerLoadOperator(node, access.machine_type.representation(), simplified()->LoadImmutableFromObject(ObjectAccess(MachineType::Int32(), access.write_barrier_kind)));
        break;
    }
    case IrOpcode::kStore: {
        StoreRepresentation store_rep = StoreRepresentationOf(node->op());
        LowerStoreOperator(node, store_rep.representation(), machine()->Store(StoreRepresentation(MachineRepresentation::kWord32, store_rep.write_barrier_kind())));
        break;
    }
    ...
    case IrOpcode::kStoreToObject: {
        ObjectAccess access = ObjectAccessOf(node->op());
        LowerStoreOperator(node, access.machine_type.representation(), simplified()->StoreToObject(ObjectAccess(MachineType::Int32(), access.write_barrier_kind)));
        break;
    }
    case IrOpcode::kInitializeImmutableInObject: {
        ObjectAccess access = ObjectAccessOf(node->op());
        LowerStoreOperator(node, access.machine_type.representation(), simplified()->InitializeImmutableInObject(ObjectAccess(MachineType::Int32(), access.write_barrier_kind)));
        break;
    }
    ...
    case IrOpcode::kParameter: {
        ...
        if (GetParameterCountAfterLowering(signature()) != param_count) {
            ...
            if (signature()->GetParam(old_index) == MachineRepresentation::kWord64) {
                SetInt32Type(node);
                Node* high_node = SetInt32Type(graph()->NewNode(common()->Parameter(new_index + 1), graph()->start()));
                ReplaceNode(node, node, high_node);
            }
        }
        break;
            ...
            if (return_arity == 1) {
                ...
                for (size_t old_index = 0, new_index = 0; old_index < return_arity; ++old_index, ++new_index) {
                    ...
                    if (call_descriptor->GetReturnType(old_index).representation() == MachineRepresentation::kWord64) {
                        Node* high_node = SetInt32Type(graph()->NewNode(common()->Projection(new_index + 1), node, graph()->start()));
    ...
    case IrOpcode::kWord64And: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->Word32And(), GetReplacementLow(left), GetReplacementLow(right)));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32And(), GetReplacementHigh(left), GetReplacementHigh(right)));
        ReplaceNode(node, low_node, high_node);
        break;
    }
    ...
    case IrOpcode::kWord64Or: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->Word32Or(), GetReplacementLow(left), GetReplacementLow(right)));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32Or(), GetReplacementHigh(left), GetReplacementHigh(right)));
        ReplaceNode(node, low_node, high_node);
        break;
    }
    case IrOpcode::kWord64Xor: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->Word32Xor(), GetReplacementLow(left), GetReplacementLow(right)));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32Xor(), GetReplacementHigh(left), GetReplacementHigh(right)));
        ReplaceNode(node, low_node, high_node);
        break;
    }
    ...
    case IrOpcode::kChangeInt32ToInt64: {
        ...
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32Sar(), input, graph()->NewNode(common()->Int32Constant(31))));
    ...
    case IrOpcode::kBitcastFloat64ToInt64: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->Float64ExtractLowWord32(), input));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Float64ExtractHighWord32(), input));
        ReplaceNode(node, low_node, high_node);
        break;
    }
    ...
    case IrOpcode::kWord64RorLowerable: {
        ...
        if (m.HasResolvedValue()) {
            ...
            if (shift_value == 0) {
                ...
                Node* low_node
                    = SetInt32Type(graph()->NewNode(machine()->Word32Or(), graph()->NewNode(op1, low_input, masked_shift), graph()->NewNode(op2, high_input, inv_shift)));
                Node* high_node
                    = SetInt32Type(graph()->NewNode(machine()->Word32Or(), graph()->NewNode(op1, high_input, masked_shift), graph()->NewNode(op2, low_input, inv_shift)));
                ReplaceNode(node, low_node, high_node);
            }
        } else {
            ...
            bool is_ror = node->opcode() == IrOpcode::kWord64RorLowerable;
            Node* inv_mask = is_ror ? graph()->NewNode(machine()->Word32Xor(), graph()->NewNode(machine()->Word32Shr(), graph()->NewNode(common()->Int32Constant(-1)), safe_shift),
            ...
            Node* low_node = SetInt32Type(
                graph()->NewNode(machine()->Word32Or(), graph()->NewNode(machine()->Word32And(), rotate_low, mask1), graph()->NewNode(machine()->Word32And(), rotate_high, mask2)));
            Node* high_node = SetInt32Type(
                graph()->NewNode(machine()->Word32Or(), graph()->NewNode(machine()->Word32And(), rotate_high, mask1), graph()->NewNode(machine()->Word32And(), rotate_low, mask2)));
            ReplaceNode(node, low_node, high_node);
        }
        break;
    }
    case IrOpcode::kWord64ClzLowerable: {
        ...
        Node* low_node = d.Phi(MachineRepresentation::kWord32,
            graph()->NewNode(machine()->Int32Add(), graph()->NewNode(machine()->Word32Clz(), GetReplacementLow(input)), graph()->NewNode(common()->Int32Constant(32))),
            graph()->NewNode(machine()->Word32Clz(), GetReplacementHigh(input)));
        ReplaceNode(node, SetInt32Type(low_node), SetInt32Type(graph()->NewNode(common()->Int32Constant(0))));
        break;
    }
    case IrOpcode::kWord64CtzLowerable: {
        ...
        Node* low_node = d.Phi(MachineRepresentation::kWord32,
            graph()->NewNode(machine()->Int32Add(), graph()->NewNode(machine()->Word32Ctz().op(), GetReplacementHigh(input)), graph()->NewNode(common()->Int32Constant(32))),
    ...
    case IrOpcode::kLoopExitValue: {
        ...
        if (rep == MachineRepresentation::kWord64) {
            Node* low_node = SetInt32Type(graph()->NewNode(common()->LoopExitValue(MachineRepresentation::kWord32), GetReplacementLow(node->InputAt(0)), node->InputAt(1)));
            Node* high_node = SetInt32Type(graph()->NewNode(common()->LoopExitValue(MachineRepresentation::kWord32), GetReplacementHigh(node->InputAt(0)), node->InputAt(1)));
            ReplaceNode(node, low_node, high_node);
        } else {
            DefaultLowering(node);
        }
    ...
    case IrOpcode::kWord64ReverseBytes: {
        Node* input = node->InputAt(0);
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->Word32ReverseBytes(), GetReplacementHigh(input)));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32ReverseBytes(), GetReplacementLow(input)));
        ReplaceNode(node, low_node, high_node);
        break;
    }
    case IrOpcode::kSignExtendWord8ToInt64: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->SignExtendWord8ToInt32(), input));
        // We use SAR to preserve the sign in the high word.
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32Sar(), low_node, graph()->NewNode(common()->Int32Constant(31))));
        ReplaceNode(node, low_node, high_node);
        node->NullAllInputs();
        break;
    }
    case IrOpcode::kSignExtendWord16ToInt64: {
        ...
        // Sign extend low node to Int32
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->SignExtendWord16ToInt32(), input));
        // We use SAR to preserve the sign in the high word.
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->Word32Sar(), low_node, graph()->NewNode(common()->Int32Constant(31))));
        ReplaceNode(node, low_node, high_node);
        node->NullAllInputs();
    ...
    case IrOpcode::kWord64AtomicLoad: {
        ...
        if (params.representation() == MachineType::Uint64()) {
            ...
            ReplaceNode(node, SetInt32Type(node), SetInt32Type(graph()->NewNode(common()->Int32Constant(0))));
        }
        break;
    }
...
#undef ATOMIC_CASE
    ...
    case IrOpcode::kI64x2ExtractLane: {
        ...
        Node* low_node = SetInt32Type(graph()->NewNode(machine()->I32x4ExtractLane(lane * 2), input));
        Node* high_node = SetInt32Type(graph()->NewNode(machine()->I32x4ExtractLane(lane * 2 + 1), input));
        ReplaceNode(node, low_node, high_node);
        break;
    }
...
bool Int64Lowering::HasReplacementLow(Node* node) { return replacements_[node->id()].low != nullptr; }
...
bool Int64Lowering::HasReplacementHigh(Node* node) { return replacements_[node->id()].high != nullptr; }
