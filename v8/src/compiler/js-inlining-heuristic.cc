...
namespace {
bool IsSmall(int const size) { return size <= v8_flags.max_inlined_bytecode_size_small; }

bool CanConsiderForInlining(JSHeapBroker* broker, FeedbackCellRef const& feedback_cell)
{
    base::Optional<FeedbackVectorRef> feedback_vector = feedback_cell.feedback_vector();
    ...
    base::Optional<FeedbackVectorRef> feedback_vector_again = feedback_cell.feedback_vector();
...
{
    FeedbackCellRef feedback_cell = function.raw_feedback_cell(broker->dependencies());
    bool const result = CanConsiderForInlining(broker, feedback_cell);
    if (result) {
        CHECK(function.shared().equals(feedback_cell.shared_function_info().value()));
    }
    return result;
}
...
Reduction JSInliningHeuristic::Reduce(Node* node)
{
    ...
    for (int i = 0; i < candidate.num_functions; ++i) {
        ...
        SharedFunctionInfoRef shared = candidate.functions[i].has_value() ? candidate.functions[i].value().shared() : candidate.shared_info.value();
        ...
        // this case.
        CHECK_IMPLIES(candidate.can_inline_function[i], shared.IsInlineable() || shared.GetInlineability() == SharedFunctionInfo::kHasOptimizationDisabled);
...
bool JSInliningHeuristic::TryReuseDispatch(Node* node, Node* callee, Node** if_successes, Node** calls, Node** inputs, int input_count)
{
    ...
    for (int i = 0; i < num_calls; ++i) {
        ...
        if (checkpoint) {
            ...
            FrameState new_checkpoint_state = DuplicateFrameStateAndRename(FrameState { checkpoint_state }, callee, target, (i == num_calls - 1) ? kChangeInPlace : kCloneState);
...
void JSInliningHeuristic::CreateOrReuseDispatch(Node* node, Node* callee, Candidate const& candidate, Node** if_successes, Node** calls, Node** inputs, int input_count)
{
...
Reduction JSInliningHeuristic::InlineCandidate(Candidate const& candidate, bool small_function)
{
    ...
    // Create the appropriate control flow to dispatch to the cloned calls.
    CreateOrReuseDispatch(node, callee, candidate, if_successes, calls, inputs, input_count);
...
void JSInliningHeuristic::PrintCandidates()
{
    ...
    for (const Candidate& candidate : candidates_) {
        ...
        for (int i = 0; i < candidate.num_functions; ++i) {
            SharedFunctionInfoRef shared = candidate.functions[i].has_value() ? candidate.functions[i]->shared() : candidate.shared_info.value();
            os << "  - target: " << shared;
            if (candidate.bytecode[i].has_value()) {
                os << ", bytecode size: " << candidate.bytecode[i]->length();
                if (candidate.functions[i].has_value()) {
                    JSFunctionRef function = candidate.functions[i].value();
                    unsigned inlined_bytecode_size = function.code().GetInlinedBytecodeSize();
...
CompilationDependencies* JSInliningHeuristic::dependencies() const { return broker()->dependencies(); }

CommonOperatorBuilder* JSInliningHeuristic::common() const { return jsgraph()->common(); }

SimplifiedOperatorBuilder* JSInliningHeuristic::simplified() const { return jsgraph()->simplified(); }
