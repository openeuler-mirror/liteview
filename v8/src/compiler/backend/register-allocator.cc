...
const InstructionBlock* GetInstructionBlock(const InstructionSequence* code, LifetimePosition pos) { return code->GetInstructionBlock(pos.ToInstructionIndex()); }

Instruction* GetLastInstruction(InstructionSequence* code, const InstructionBlock* block) { return code->InstructionAt(block->last_instruction_index()); }
...
LiveRangeBound* LiveRangeBoundArray::Find(const LifetimePosition position) const
{
    size_t left_index = 0;
...
{
    LifetimePosition succ_start = LifetimePosition::GapFromInstructionIndex(succ->first_instruction_index());
    return Find(succ_start);
}

bool LiveRangeBoundArray::FindConnectableSubranges(const InstructionBlock* block, const InstructionBlock* pred, FindResult* result) const
{
    LifetimePosition pred_end = LifetimePosition::InstructionFromInstructionIndex(pred->last_instruction_index());
...
LiveRangeFinder::LiveRangeFinder(const TopTierRegisterAllocationData* data, Zone* zone)
    : data_(data)
...
bool UsePosition::HintRegister(int* register_code) const
{
    ...
    case UsePositionHintType::kPhi: {
        TopTierRegisterAllocationData::PhiMapValue* phi = reinterpret_cast<TopTierRegisterAllocationData::PhiMapValue*>(hint_);
...
void LiveRange::VerifyIntervals() const
{
    ...
    for (UseInterval* interval = first_interval()->next(); interval != nullptr; interval = interval->next()) {
...
UsePosition* LiveRange::FirstHintPosition(int* register_index)
{
    ...
    for (; pos != nullptr; pos = pos->next()) {
        ...
        needs_revisit = needs_revisit || pos->hint_type() == UsePositionHintType::kPhi || pos->hint_type() == UsePositionHintType::kUsePos;
    }
    if (!needs_revisit) {
...
UsePosition* LiveRange::PreviousUsePositionRegisterIsBeneficial(LifetimePosition start) const
{
...
{
    UsePosition* pos = NextUsePosition(start);
    while (pos != nullptr && pos->type() != UsePositionType::kRequiresRegister && !pos->SpillDetrimental()) {
        pos = pos->next();
    }
    return pos;
}
...
UseInterval* LiveRange::FirstSearchIntervalForPosition(LifetimePosition position) const
{
...
void LiveRange::AdvanceLastProcessedMarker(UseInterval* to_start_of, LifetimePosition but_not_past) const
{
    if (to_start_of == nullptr)
        return;
    if (to_start_of->start() > but_not_past)
        return;
    LifetimePosition start = current_interval_ == nullptr ? LifetimePosition::Invalid() : current_interval_->start();
...
UsePosition* LiveRange::DetachAt(LifetimePosition position, LiveRange* result, Zone* zone, HintConnectionOption connect_hints)
{
    ...
    result->last_interval_ = (last_interval_ == before) ? after // Only interval in the range after split.
...
bool LiveRange::Covers(LifetimePosition position) const
{
    ...
    for (UseInterval* interval = start_search; interval != nullptr; interval = interval->next()) {
...
void LiveRange::Print(bool with_children) const { Print(RegisterConfiguration::Default(), with_children); }
...
void TopLevelLiveRange::CommitSpillMoves(TopTierRegisterAllocationData* data, const InstructionOperand& op)
{
...
void TopLevelLiveRange::FilterSpillMoves(TopTierRegisterAllocationData* data, const InstructionOperand& op)
{
    DCHECK_IMPLIES(op.IsConstant(), GetSpillMoveInsertionLocations(data) == nullptr);
...
void TopLevelLiveRange::EnsureInterval(LifetimePosition start, LifetimePosition end, Zone* zone, bool trace_alloc)
{
...
void TopLevelLiveRange::AddUseInterval(LifetimePosition start, LifetimePosition end, Zone* zone, bool trace_alloc)
{
...
static bool AreUseIntervalsIntersecting(UseInterval* interval1, UseInterval* interval2)
{
...
void LinearScanAllocator::PrintRangeRow(std::ostream& os, const TopLevelLiveRange* toplevel)
{
    ...
    for (const LiveRange* range = toplevel; range != nullptr; range = range->next()) {
        for (UseInterval* interval = range->first_interval(); interval != nullptr; interval = interval->next()) {
...
bool SpillRange::TryMerge(SpillRange* other)
{
    ...
    live_ranges().insert(live_ranges().end(), other->live_ranges().begin(), other->live_ranges().end());
...
TopTierRegisterAllocationData::PhiMapValue::PhiMapValue(PhiInstruction* phi, const InstructionBlock* block, Zone* zone)
    : phi_(phi)
    , block_(block)
...
void TopTierRegisterAllocationData::PhiMapValue::AddOperand(InstructionOperand* operand) { incoming_operands_.push_back(operand); }

void TopTierRegisterAllocationData::PhiMapValue::CommitAssignment(const InstructionOperand& assigned)
{
...
TopTierRegisterAllocationData::TopTierRegisterAllocationData(
    const RegisterConfiguration* config, Zone* zone, Frame* frame, InstructionSequence* code, RegisterAllocationFlags flags, TickCounter* tick_counter, const char* debug_name)
    ...
    , live_ranges_(code->VirtualRegisterCount() * 2, nullptr, allocation_zone())
...
MoveOperands* TopTierRegisterAllocationData::AddGapMove(int index, Instruction::GapPosition position, const InstructionOperand& from, const InstructionOperand& to)
{
...
MachineRepresentation TopTierRegisterAllocationData::RepresentationFor(int virtual_register)
{
    DCHECK_LT(virtual_register, code()->VirtualRegisterCount());
    return code()->GetRepresentation(virtual_register);
}

TopLevelLiveRange* TopTierRegisterAllocationData::GetOrCreateLiveRangeFor(int index)
{
...
TopLevelLiveRange* TopTierRegisterAllocationData::NewLiveRange(int index, MachineRepresentation rep) { return allocation_zone()->New<TopLevelLiveRange>(index, rep); }

TopTierRegisterAllocationData::PhiMapValue* TopTierRegisterAllocationData::InitializePhiMap(const InstructionBlock* block, PhiInstruction* phi)
{
    TopTierRegisterAllocationData::PhiMapValue* map_value = allocation_zone()->New<TopTierRegisterAllocationData::PhiMapValue>(phi, block, allocation_zone());
...
TopTierRegisterAllocationData::PhiMapValue* TopTierRegisterAllocationData::GetPhiMapValueFor(int virtual_register)
{
...
TopTierRegisterAllocationData::PhiMapValue* TopTierRegisterAllocationData::GetPhiMapValueFor(TopLevelLiveRange* top_range) { return GetPhiMapValueFor(top_range->vreg()); }
...
bool TopTierRegisterAllocationData::RangesDefinedInDeferredStayInDeferred()
{
    ...
    for (const TopLevelLiveRange* range : live_ranges()) {
        ...
        for (const UseInterval* i = range->first_interval(); i != nullptr; i = i->next()) {
...
SpillRange* TopTierRegisterAllocationData::AssignSpillRangeToLiveRange(TopLevelLiveRange* range, SpillMode spill_mode)
{
...
void TopTierRegisterAllocationData::MarkFixedUse(MachineRepresentation rep, int index)
{
    switch (rep) {
...
bool TopTierRegisterAllocationData::HasFixedUse(MachineRepresentation rep, int index)
{
    switch (rep) {
    case MachineRepresentation::kFloat32:
    case MachineRepresentation::kSimd128:
...
void TopTierRegisterAllocationData::MarkAllocated(MachineRepresentation rep, int index)
{
    switch (rep) {
    case MachineRepresentation::kFloat32:
    case MachineRepresentation::kSimd128:
...
bool TopTierRegisterAllocationData::IsBlockBoundary(LifetimePosition pos) const
{
...
void ConstraintBuilder::MeetConstraintsBefore(int instr_index)
{
    ...
    for (size_t i = 0; i < second->InputCount(); i++) {
        ...
        if (cur_input->HasSlotPolicy()) {
            TopLevelLiveRange* range = data()->GetOrCreateLiveRangeFor(cur_input->virtual_register());
        ...
        *cur_input = UnallocatedOperand(*cur_input, second_output->virtual_register());
        ...
        if (code()->IsReference(input_vreg) && !code()->IsReference(output_vreg)) {
            if (second->HasReferenceMap()) {
                TopTierRegisterAllocationData::DelayedReference delayed_reference = { second->reference_map(), &gap_move->source() };
...
void ConstraintBuilder::ResolvePhis(const InstructionBlock* block)
{
    for (PhiInstruction* phi : block->phis()) {
        ...
        TopTierRegisterAllocationData::PhiMapValue* map_value = data()->InitializePhiMap(block, phi);
...
LiveRangeBuilder::LiveRangeBuilder(TopTierRegisterAllocationData* data, Zone* local_zone)
    : data_(data)
    , phi_hints_(local_zone)
{
}

SparseBitVector* LiveRangeBuilder::ComputeLiveOut(const InstructionBlock* block, TopTierRegisterAllocationData* data)
{
...
void LiveRangeBuilder::AddInitialIntervals(const InstructionBlock* block, SparseBitVector* live_out)
{
    ...
    for (int operand_index : *live_out) {
        ...
        range->AddUseInterval(start, end, allocation_zone(), data()->is_trace_alloc());
    }
}
...
TopLevelLiveRange* LiveRangeBuilder::LiveRangeFor(InstructionOperand* operand, SpillMode spill_mode)
{
    if (operand->IsUnallocated()) {
        return data()->GetOrCreateLiveRangeFor(UnallocatedOperand::cast(operand)->virtual_register());
    } else if (operand->IsConstant()) {
        return data()->GetOrCreateLiveRangeFor(ConstantOperand::cast(operand)->virtual_register());
...
UsePosition* LiveRangeBuilder::Define(LifetimePosition position, InstructionOperand* operand, void* hint, UsePositionHintType hint_type, SpillMode spill_mode)
{
    ...
    if (range->IsEmpty() || range->Start() > position) {
        ...
        range->AddUseInterval(position, position.NextStart(), allocation_zone(), data()->is_trace_alloc());
    ...
    LifetimePosition block_start, LifetimePosition position, InstructionOperand* operand, void* hint, UsePositionHintType hint_type, SpillMode spill_mode)
{
    ...
    range->AddUseInterval(block_start, position, allocation_zone(), data()->is_trace_alloc());
    return use_pos;
}
    ...
    for (int index = block->last_instruction_index(); index >= block_start; index--) {
        ...
        if (instr->ClobbersRegisters()) {
            for (int i = 0; i < config()->num_allocatable_general_registers(); ++i) {
                ...
                range->AddUseInterval(curr_position, curr_position.End(), allocation_zone(), data()->is_trace_alloc());
            }
        }
...
void LiveRangeBuilder::ProcessLoopHeader(const InstructionBlock* block, SparseBitVector* live)
{
    ...
    for (int operand_index : *live) {
        TopLevelLiveRange* range = data()->GetOrCreateLiveRangeFor(operand_index);
        range->EnsureInterval(start, end, allocation_zone(), data()->is_trace_alloc());
    }
...
void LiveRangeBuilder::BuildLiveRanges()
{
    ...
    for (TopLevelLiveRange* range : data()->live_ranges()) {
        ...
        if (range->has_slot_use() && range->HasNoSpillType()) {
            SpillMode spill_mode = range->slot_use_kind() == TopLevelLiveRange::SlotUseKind::kDeferredSlotUse ? SpillMode::kSpillDeferred : SpillMode::kSpillAtDefinition;
        ...
        // operand assigned.
        if (range->HasSpillOperand() && range->GetSpillOperand()->IsConstant()) {
            for (UsePosition* pos = range->first_pos(); pos != nullptr; pos = pos->next()) {
...
bool LiveRangeBuilder::IntervalStartsAtBlockBoundary(const UseInterval* interval) const
{
    LifetimePosition start = interval->start();
...
bool LiveRangeBuilder::IntervalPredecessorsCoveredByRange(const UseInterval* interval, const TopLevelLiveRange* range) const
{
    LifetimePosition start = interval->start();
    int instruction_index = start.ToInstructionIndex();
    const InstructionBlock* block = data()->code()->GetInstructionBlock(instruction_index);
...
bool LiveRangeBuilder::NextIntervalStartsInDifferentBlocks(const UseInterval* interval) const
{
        ...
        for (auto phi : block->phis()) {
            ...
            for (auto input : phi->operands()) {
                ...
                if (input_bundle != nullptr) {
                    TRACE("Merge\n");
                    LiveRangeBundle* merged = LiveRangeBundle::TryMerge(out, input_bundle, data()->is_trace_alloc());
                    if (merged != nullptr) {
...
LiveRangeBundle* LiveRangeBundle::TryMerge(LiveRangeBundle* lhs, LiveRangeBundle* rhs, bool trace_alloc)
{
    if (rhs == lhs)
        return lhs;
    ...
    while (iter1 != lhs->uses_.end() && iter2 != rhs->uses_.end()) {
        if (iter1->start >= iter2->end) {
            ...
            TRACE_COND(trace_alloc, "No merge %d:%d %d:%d\n", iter1->start, iter1->end, iter2->start, iter2->end);
            return nullptr;
        }
    }
...
RegisterAllocator::RegisterAllocator(TopTierRegisterAllocationData* data, RegisterKind kind)
    : data_(data)
    , mode_(kind)
    ...
    *begin_spill_out = range;
    ...
    while (loop_header != nullptr) {
        ...
        if (live_at_header != nullptr && !live_at_header->spilled()) {
            for (LiveRange* check_use = live_at_header; check_use != nullptr && check_use->Start() < pos; check_use = check_use->next()) {
...
LinearScanAllocator::LinearScanAllocator(TopTierRegisterAllocationData* data, RegisterKind kind, Zone* local_zone)
...
void LinearScanAllocator::SpillNotLiveRanges(RangeWithRegisterSet* to_be_live, LifetimePosition position, SpillMode spill_mode)
{
...
{
    using SmallRangeVector = base::SmallVector<TopLevelLiveRange*, RegisterConfiguration::kMaxRegisters>;
    ...
    return left_used.size() > right_used.size() ? current_block->predecessors()[0] : current_block->predecessors()[1];
}

bool LinearScanAllocator::CheckConflict(MachineRepresentation rep, int reg, RangeWithRegisterSet* to_be_live)
{
    for (RangeWithRegister range_with_reg : *to_be_live) {
        if (data()->config()->AreAliases(range_with_reg.range->representation(), range_with_reg.expected_register, rep, reg)) {
            return true;
        }
    }
    return false;
...
void LinearScanAllocator::ComputeStateFromManyPredecessors(InstructionBlock* current_block, RangeWithRegisterSet* to_be_live)
{
    struct Vote {
        size_t count;
    ...
    struct TopLevelLiveRangeComparator {
        bool operator()(const TopLevelLiveRange* lhs, const TopLevelLiveRange* rhs) const { return lhs->vreg() < rhs->vreg(); }
    };
    ZoneMap<TopLevelLiveRange*, Vote, TopLevelLiveRangeComparator> counts(data()->allocation_zone());
    ...
    auto assign_to_live = [this, counts, majority](std::function<bool(TopLevelLiveRange*)> filter, RangeWithRegisterSet* to_be_live, bool* taken_registers) {
        ...
        for (const auto& val : counts) {
            ...
            if (val.second.count >= majority) {
                ...
                if (check_aliasing && (rep == MachineRepresentation::kFloat32 || rep == MachineRepresentation::kSimd128 || rep == MachineRepresentation::kSimd256))
                ...
                TRACE("Reset %d as live due vote %zu in %s\n", val.first->TopLevel()->vreg(), val.second.count, RegisterName(reg));
            }
        }
    };
    ...
    if (spill_mode == SpillMode::kSpillDeferred) {
        ...
        auto add_to_inactive = [this, max](LiveRange* range) {
            ...
            for (auto active : active_live_ranges()) {
                split_conflicting(range, active, [this](LiveRange* updated) { next_active_ranges_change_ = std::min(updated->End(), next_active_ranges_change_); });
            }
            for (int reg = 0; reg < num_registers(); ++reg) {
                ...
                for (auto inactive : inactive_live_ranges(reg)) {
                    if (inactive->NextStart() > max)
                        break;
                    split_conflicting(range, inactive, [this](LiveRange* updated) { next_inactive_ranges_change_ = std::min(updated->End(), next_inactive_ranges_change_); });
                }
            }
        };
    ...
    while (!unhandled_live_ranges().empty() || last_block < max_blocks) {
        ...
        if (position >= next_block_boundary) {
            ...
            if (!fallthrough) {
                ...
                auto pick_state_from = [this, current_block](RpoNumber pred, RangeWithRegisterSet* to_be_live) -> bool {
        ...
        for (int reg = 0; reg < num_registers(); ++reg) {
            ...
            for (auto it = inactive_live_ranges(reg).begin(); it != inactive_live_ranges(reg).end();) {
                ...
                if (cur_inactive->End() <= position) {
                    ...
                    next_inactive_ranges_change_ = std::min(next_inactive_ranges_change_, cur_inactive->NextStartAfter(position));
                    it = inactive_live_ranges(reg).erase(it);
...
void LinearScanAllocator::FindFreeRegistersForRange(LiveRange* range, base::Vector<LifetimePosition> positions)
{
    ...
    for (LiveRange* cur_active : active_live_ranges()) {
        ...
        if (kFPAliasing != AliasingKind::kCombine || !check_fp_aliasing()) {
            ...
            TRACE("Register %s is free until pos %d (1) due to %d\n", RegisterName(cur_reg), LifetimePosition::GapFromInstructionIndex(0).value(), cur_active->TopLevel()->vreg());
        } else {
            int alias_base_index = -1;
    ...
    for (int cur_reg = 0; cur_reg < num_regs; ++cur_reg) {
        for (LiveRange* cur_inactive : inactive_live_ranges(cur_reg)) {
            ...
            if ((kFPAliasing != AliasingKind::kCombine || !check_fp_aliasing()) && (positions[cur_reg] <= cur_inactive->NextStart() || range->End() <= cur_inactive->NextStart())) {
                break;
            }
...
bool LinearScanAllocator::TryAllocatePreferredReg(LiveRange* current, const base::Vector<LifetimePosition>& free_until_pos)
{
    int hint_register;
...
int LinearScanAllocator::PickRegisterThatIsAvailableLongest(LiveRange* current, int hint_reg, const base::Vector<LifetimePosition>& free_until_pos)
{
...
bool LinearScanAllocator::TryAllocateFreeReg(LiveRange* current, const base::Vector<LifetimePosition>& free_until_pos)
{
...
void LinearScanAllocator::SplitAndSpillIntersecting(LiveRange* current, SpillMode spill_mode)
{
    ...
    for (int cur_reg = 0; cur_reg < num_registers(); ++cur_reg) {
        ...
        for (auto it = inactive_live_ranges(cur_reg).begin(); it != inactive_live_ranges(cur_reg).end();) {
            LiveRange* range = *it;
            if (kFPAliasing == AliasingKind::kCombine && check_fp_aliasing() && !data()->config()->AreAliases(current->representation(), reg, range->representation(), cur_reg)) {
                ++it;
                continue;
            }
            DCHECK(range->End() > current->Start());
...
bool LinearScanAllocator::TryReuseSpillForPhi(TopLevelLiveRange* range)
{
    ...
    TopTierRegisterAllocationData::PhiMapValue* phi_map_value = data()->GetPhiMapValueFor(range);
...
void LinearScanAllocator::SpillBetweenUntil(LiveRange* range, LifetimePosition start, LifetimePosition until, LifetimePosition end, SpillMode spill_mode)
{
    ...
    if (second_part->Start() < end) {
        ...
        if (GetInstructionBlock(data()->code(), second_part->Start())->IsDeferred()) {
            ...
            TRACE("Setting control flow hint for %d:%d to %s\n", third_part->TopLevel()->vreg(), third_part->relative_id(), RegisterName(range->controlflow_hint()));
    ...
    for (auto range : data()->live_ranges()) {
        ...
        if (range != nullptr && range->IsSpilledOnlyInDeferredBlocks(data())) {
            ...
            if (GetInstructionBlock(data()->code(), range->Start())->IsDeferred()) {
                ...
                range->TransitionRangeToDeferredSpill(data()->allocation_zone(), max_blocks);
            }
        }
    }
}
...
ReferenceMapPopulator::ReferenceMapPopulator(TopTierRegisterAllocationData* data)
    : data_(data)
{
}
...
void ReferenceMapPopulator::PopulateReferenceMaps()
{
    ...
    for (TopTierRegisterAllocationData::DelayedReference& delayed_reference : data()->delayed_references()) {
    ...
    // phase.
    std::sort(data()->live_ranges().begin(), data()->live_ranges().end(), [](TopLevelLiveRange* a, TopLevelLiveRange* b) {
        if (!a || a->IsEmpty())
            return false;
        if (!b || b->IsEmpty())
            return true;
    ...
    for (TopLevelLiveRange* range : data()->live_ranges()) {
        ...
        for (auto it = first_it; it != reference_maps->end(); ++it) {
            ...
            // the spill position.
            int spill_index = range->IsSpilledOnlyInDeferredBlocks(data()) || range->LateSpillingSelected() ? cur->Start().ToInstructionIndex() : range->spill_start_index();
            ...
            for (const RpoNumber& pred : block->predecessors()) {
                ...
                if (!pred_op.IsAnyRegister() && cur_op.IsAnyRegister()) {
                    ...
                    if (current->End() < block_end && (successor == nullptr || successor->spilled())) {
                        ...
                        bool uses_reg = false;
                        for (const UsePosition* use = current->NextUsePosition(block_start); use != nullptr; use = use->next()) {
                    ...
                    if (current->TopLevel()->IsSpilledOnlyInDeferredBlocks(data()) && pred_block->IsDeferred()) {
                        ...
                        TRACE("Adding B%d to list of spill blocks for %d\n", pred_block->rpo_number().ToInt(), current->TopLevel()->vreg());
                        current->TopLevel()->GetListOfBlocksRequiringSpillOperands(data())->Add(pred_block->rpo_number().ToInt());
                    }
                }
                int move_loc = ResolveControlFlow(block, cur_op, pred_block, pred_op);
                USE(move_loc);
                DCHECK_IMPLIES(result.cur_cover_->TopLevel()->IsSpilledOnlyInDeferredBlocks(data()) && !(pred_op.IsAnyRegister() && cur_op.IsAnyRegister()) && move_loc != -1,
...
int LiveRangeConnector::ResolveControlFlow(const InstructionBlock* block, const InstructionOperand& cur_op, const InstructionBlock* pred, const InstructionOperand& pred_op)
{
...
void LiveRangeConnector::ConnectRanges(Zone* local_zone)
{
    ...
    for (TopLevelLiveRange* top_range : data()->live_ranges()) {
        ...
        for (LiveRange* second_range = first_range->next(); second_range != nullptr; first_range = second_range, second_range = second_range->next()) {
            ...
            DCHECK_IMPLIES(connect_spilled && !(prev_operand.IsAnyRegister() && cur_operand.IsAnyRegister()), code()->GetInstructionBlock(gap_index)->IsDeferred());
...
void LiveRangeConnector::CommitSpillsInDeferredBlocks(TopLevelLiveRange* range, LiveRangeBoundArray* array, Zone* temp_zone)
{
    ...
    for (const LiveRange* child = range; child != nullptr; child = child->next()) {
        for (const UsePosition* pos = child->first_pos(); pos != nullptr; pos = pos->next()) {
