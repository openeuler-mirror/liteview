...
MoveOperands* Split(MoveOperands* move, MachineRepresentation smaller_rep, ParallelMove* moves)
{
    ...
    for (int i = 1; i < aliases; ++i) {
        src_index += src_step;
        dst_index += dst_step;
        moves->AddMove(AllocatedOperand(src_kind, smaller_rep, src_index), AllocatedOperand(dst_kind, smaller_rep, dst_index));
    }
...
void GapResolver::Resolve(ParallelMove* moves)
{
    ...
    for (size_t i = 0; i < nmoves;) {
        ...
        if (kFPAliasing == AliasingKind::kCombine && move->destination().IsFPRegister()) {
            fp_reps |= RepresentationBit(LocationOperand::cast(move->destination()).representation());
        }
    }
...
void GapResolver::PerformMove(ParallelMove* moves, MoveOperands* move)
{
    ...
    if (cycle->size() == 2) {
        ...
        MachineRepresentation rep = LocationOperand::cast(move->source()).representation();
        MachineRepresentation rep2 = LocationOperand::cast(move2->source()).representation();
        if (rep == rep2) {
    ...
    for (auto* unblocked_move : *cycle) {
        assembler_->AssembleMove(&unblocked_move->source(), &unblocked_move->destination());
        unblocked_move->Eliminate();
    }
    assembler_->MoveTempLocationTo(&destination, rep);
}

base::Optional<std::vector<MoveOperands*>> GapResolver::PerformMoveHelper(ParallelMove* moves, MoveOperands* move)
{
    ...
    const bool is_fp_loc_move = kFPAliasing == AliasingKind::kCombine && destination.IsFPLocationOperand();
