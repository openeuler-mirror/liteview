...
public:
    ...
    // virtual register.
    void DefineAsUnallocatedOperand(int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block, bool is_exceptional_call_output);
    void DefineAsFixedSpillOperand(
        AllocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block, bool is_exceptional_call_output);
    void DefineAsConstantOperand(ConstantOperand* operand, MachineRepresentation rep, int instr_index, bool is_deferred_block);
    void DefineAsPhi(int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block);

    // Spill an operand that is assigned to this virtual register.
    void SpillOperand(InstructionOperand* operand, int instr_index, bool has_constant_policy, MidTierRegisterAllocationData* data);

    // Emit gap moves to / from the spill slot.
    void EmitGapMoveToInputFromSpillSlot(InstructionOperand to_operand, int instr_index, MidTierRegisterAllocationData* data);
    void EmitGapMoveFromOutputToSpillSlot(InstructionOperand from_operand, const InstructionBlock* current_block, int instr_index, MidTierRegisterAllocationData* data);
    void EmitGapMoveToSpillSlot(InstructionOperand from_operand, int instr_index, MidTierRegisterAllocationData* data);

    // Adds pending spills for deferred-blocks.
    void AddDeferredSpillUse(int instr_index, MidTierRegisterAllocationData* data);
    void AddDeferredSpillOutput(AllocatedOperand allocated_op, int instr_index, MidTierRegisterAllocationData* data);
    ...
    bool HasPendingSpillOperand() const { return HasSpillOperand() && spill_operand_->IsPending(); }
    bool HasAllocatedSpillOperand() const { return HasSpillOperand() && spill_operand_->IsAllocated(); }
    bool HasConstantSpillOperand() const { return HasSpillOperand() && spill_operand_->IsConstant(); }
    ...
    {
        if (HasConstantSpillOperand())
            return;
        needs_spill_at_output_ = true;
        if (HasSpillRange())
            spill_range()->ClearDeferredBlockSpills();
    }
    ...
    bool IsSpilledAt(int instr_index, MidTierRegisterAllocationData* data)
    {
        ...
        if (NeedsSpillAtOutput() || HasConstantSpillOperand())
            return true;
    ...
    bool is_defined_in_deferred_block() const { return is_defined_in_deferred_block_; }
    bool is_exceptional_call_output() const { return is_exceptional_call_output_; }

    struct DeferredSpillSlotOutput {
    public:
        explicit DeferredSpillSlotOutput(int instr, AllocatedOperand op, const SparseBitVector* blocks)
            : instr_index(instr)
            , operand(op)
            , live_blocks(blocks)
        {
    ...
    public:
        ...
        SpillRange(int definition_instr_index, const InstructionBlock* definition_block, MidTierRegisterAllocationData* data)
        ...
        SpillRange(const InstructionBlock* phi_block, MidTierRegisterAllocationData* data)
            : live_range_(phi_block->first_instruction_index(), phi_block->first_instruction_index())
        ...
        {
            if (!live_range_.Contains(instr_index))
                return false;

            int block_rpo = block->rpo_number().ToInt();
            if (!live_blocks_->Contains(block_rpo))
                return false;
        ...
        bool HasDeferredBlockSpills() const { return deferred_spill_outputs_ != nullptr; }
...
private:
    void Initialize(int virtual_register, MachineRepresentation rep, InstructionOperand* spill_operand, int instr_index, bool is_phi, bool is_constant,
        bool is_defined_in_deferred_block, bool is_exceptional_call_output);
    ...
    bool TrySpillOnEntryToDeferred(MidTierRegisterAllocationData* data, const InstructionBlock* block);
...
public:
    ...
    explicit BlockState(Zone* zone)
        : dominated_blocks_(zone)
    {
    }
    ...
    DeferredBlocksRegion* deferred_blocks_region() const { return deferred_blocks_region_; }
    ...
    bool is_deferred_block_boundary() const { return is_deferred_block_boundary_; }
...
public:
    ...
    {
        if (spilled_vregs_.count(vreg) != 0)
            return true;
        if (is_frozen_)
            return false;
...
void BlockState::set_register_in_state(RegisterState* register_state, RegisterKind kind)
{
    switch (kind) {
    case RegisterKind::kGeneral:
...
void VirtualRegisterData::SpillRange::AddDeferredSpillOutput(AllocatedOperand allocated_op, int instr_index, MidTierRegisterAllocationData* data)
{
    if (deferred_spill_outputs_ == nullptr) {
        Zone* zone = data->allocation_zone();
        deferred_spill_outputs_ = zone->New<ZoneVector<DeferredSpillSlotOutput>>(zone);
    }
    ...
    const SparseBitVector* deferred_blocks = block_state.deferred_blocks_region()->blocks_covered();
    deferred_spill_outputs_->emplace_back(instr_index, allocated_op, deferred_blocks);
}

MoveOperands* MidTierRegisterAllocationData::AddGapMove(int instr_index, Instruction::GapPosition position, const InstructionOperand& from, const InstructionOperand& to)
{
...
MoveOperands* MidTierRegisterAllocationData::AddPendingOperandGapMove(int instr_index, Instruction::GapPosition position)
{
    return AddGapMove(instr_index, position, PendingOperand(), PendingOperand());
}

BlockState& MidTierRegisterAllocationData::block_state(RpoNumber rpo_number) { return block_states_[rpo_number.ToInt()]; }

const InstructionBlock* MidTierRegisterAllocationData::GetBlock(RpoNumber rpo_number) { return code()->InstructionBlockAt(rpo_number); }

const InstructionBlock* MidTierRegisterAllocationData::GetBlock(int instr_index) { return code()->InstructionAt(instr_index)->block(); }

const SparseBitVector* MidTierRegisterAllocationData::GetBlocksDominatedBy(const InstructionBlock* block) { return block_state(block->rpo_number()).dominated_blocks(); }
...
public:
    ...
    explicit RegisterIndex(int index)
        : index_(index)
    {
    }
    ...
    {
        if (kFPAliasing != AliasingKind::kCombine || rep != MachineRepresentation::kSimd128) {
            return 1ull << ToInt();
        } else {
    ...
    bool operator==(const RegisterIndex& rhs) const { return index_ == rhs.index_; }
    bool operator!=(const RegisterIndex& rhs) const { return index_ != rhs.index_; }
...
VirtualRegisterData& MidTierRegisterAllocationData::VirtualRegisterDataFor(int virtual_register)
{
...
void VirtualRegisterData::Initialize(int virtual_register, MachineRepresentation rep, InstructionOperand* spill_operand, int instr_index, bool is_phi, bool is_constant,
    bool is_defined_in_deferred_block, bool is_exceptional_call_output)
{
...
void VirtualRegisterData::DefineAsConstantOperand(ConstantOperand* operand, MachineRepresentation rep, int instr_index, bool is_deferred_block)
{
    Initialize(operand->virtual_register(), rep, operand, instr_index, false, true, is_deferred_block, false);
}

void VirtualRegisterData::DefineAsFixedSpillOperand(
    AllocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block, bool is_exceptional_call_output)
{
    Initialize(virtual_register, rep, operand, instr_index, false, false, is_deferred_block, is_exceptional_call_output);
}

void VirtualRegisterData::DefineAsUnallocatedOperand(int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block, bool is_exceptional_call_output)
{
    Initialize(virtual_register, rep, nullptr, instr_index, false, false, is_deferred_block, is_exceptional_call_output);
}

void VirtualRegisterData::DefineAsPhi(int virtual_register, MachineRepresentation rep, int instr_index, bool is_deferred_block)
{
    Initialize(virtual_register, rep, nullptr, instr_index, true, false, is_deferred_block, false);
}

void VirtualRegisterData::EnsureSpillRange(MidTierRegisterAllocationData* data)
{
    ...
    const InstructionBlock* definition_block = data->GetBlock(output_instr_index_);
    if (is_phi()) {
        // Define a spill slot that is defined for the phi's range.
        spill_range_ = data->allocation_zone()->New<SpillRange>(definition_block, data);
    } else {
        if (is_exceptional_call_output()) {
            ...
            DCHECK_EQ(output_instr_index_, definition_block->last_instruction_index() - 1);
        ...
        spill_range_ = data->allocation_zone()->New<SpillRange>(output_instr_index_ + 1, definition_block, data);
    }
    data->spilled_virtual_registers().Add(vreg());
}

void VirtualRegisterData::AddSpillUse(int instr_index, MidTierRegisterAllocationData* data)
{
    if (HasConstantSpillOperand())
        return;
...
void VirtualRegisterData::AddDeferredSpillUse(int instr_index, MidTierRegisterAllocationData* data)
{
...
bool VirtualRegisterData::TrySpillOnEntryToDeferred(MidTierRegisterAllocationData* data, const InstructionBlock* block)
{
    BlockState& block_state = data->block_state(block->rpo_number());
    if (!NeedsSpillAtOutput() && block->IsDeferred() && !is_defined_in_deferred_block() && !is_constant()) {
        return block_state.deferred_blocks_region()->TryDeferSpillOutputUntilEntry(vreg());
    }
    return false;
}

void VirtualRegisterData::AddDeferredSpillOutput(AllocatedOperand allocated_op, int instr_index, MidTierRegisterAllocationData* data)
{
...
void VirtualRegisterData::SpillOperand(InstructionOperand* operand, int instr_index, bool has_constant_policy, MidTierRegisterAllocationData* data)
{
...
bool VirtualRegisterData::NeedsSpillAtDeferredBlocks() const { return HasSpillRange() && spill_range()->HasDeferredBlockSpills(); }

void VirtualRegisterData::EmitDeferredSpillOutputs(MidTierRegisterAllocationData* data)
{
    DCHECK(NeedsSpillAtDeferredBlocks());
    for (auto deferred_spill : *spill_range()->deferred_spill_outputs()) {
        EmitGapMoveToSpillSlot(deferred_spill.operand, deferred_spill.instr_index, data);
    }
}

void VirtualRegisterData::EmitGapMoveToInputFromSpillSlot(InstructionOperand to_operand, int instr_index, MidTierRegisterAllocationData* data)
{
    AddSpillUse(instr_index, data);
    DCHECK(!to_operand.IsPending());
    if (HasAllocatedSpillOperand() || HasConstantSpillOperand()) {
        data->AddGapMove(instr_index, Instruction::END, *spill_operand(), to_operand);
    } else {
        MoveOperands* move_ops = data->AddPendingOperandGapMove(instr_index, Instruction::END);
        AddPendingSpillOperand(PendingOperand::cast(&move_ops->source()));
        InstructionOperand::ReplaceWith(&move_ops->destination(), &to_operand);
    }
}

void VirtualRegisterData::EmitGapMoveToSpillSlot(InstructionOperand from_operand, int instr_index, MidTierRegisterAllocationData* data)
{
    AddSpillUse(instr_index, data);
    if (HasAllocatedSpillOperand() || HasConstantSpillOperand()) {
        data->AddGapMove(instr_index, Instruction::START, from_operand, *spill_operand());
    } else {
        MoveOperands* move_ops = data->AddPendingOperandGapMove(instr_index, Instruction::START);
...
void VirtualRegisterData::EmitGapMoveFromOutputToSpillSlot(
    InstructionOperand from_operand, const InstructionBlock* current_block, int instr_index, MidTierRegisterAllocationData* data)
{
    ...
    if (instr_index == current_block->last_instruction_index()) {
        ...
        for (const RpoNumber& succ : current_block->successors()) {
            ...
            EmitGapMoveToSpillSlot(from_operand, successor->first_instruction_index(), data);
        }
    } else {
...
void VirtualRegisterData::AllocatePendingSpillOperand(const AllocatedOperand& allocated)
{
...
public:
    static RegisterState* New(RegisterKind kind, int num_allocatable_registers, Zone* zone) { return zone->New<RegisterState>(kind, num_allocatable_registers, zone); }
    ...
    void Commit(RegisterIndex reg, AllocatedOperand allocated, InstructionOperand* operand, MidTierRegisterAllocationData* data);

    // Spill the contents of |reg| for an instruction in |current_block| using
    // the |allocated| operand to commit the spill gap move.
    void Spill(RegisterIndex reg, AllocatedOperand allocated, const InstructionBlock* current_block, MidTierRegisterAllocationData* data);
    ...
    void SpillForDeferred(RegisterIndex reg, AllocatedOperand allocated, int instr_index, MidTierRegisterAllocationData* data);
    ...
    void MoveToSpillSlotOnDeferred(RegisterIndex reg, int virtual_register, int instr_index, MidTierRegisterAllocationData* data);
    ...
    void AllocateUse(RegisterIndex reg, int virtual_register, InstructionOperand* operand, int instr_index, MidTierRegisterAllocationData* data);
    ...
    void AllocatePendingUse(RegisterIndex reg, int virtual_register, InstructionOperand* operand, bool can_be_constant, int instr_index);
    ...
    RegisterIndex::Iterator end() const { return RegisterIndex::Iterator(num_allocatable_registers()); }
...
private:
    ...
    public:
        ...
        void Commit(AllocatedOperand allocated_operand, MidTierRegisterAllocationData* data);
        void Spill(AllocatedOperand allocated_op, const InstructionBlock* current_block, MidTierRegisterAllocationData* data);
        void Use(int virtual_register, int instr_index);
        void PendingUse(InstructionOperand* operand, int virtual_register, bool can_be_constant, int instr_index);
        void SpillForDeferred(AllocatedOperand allocated, int instr_index, MidTierRegisterAllocationData* data);
        void MoveToSpillSlotOnDeferred(int virtual_register, int instr_index, MidTierRegisterAllocationData* data);
        ...
        bool has_deferred_block_spills() const { return deferred_block_spills_.has_value(); }
        ...
        bool was_spilled_while_shared() const { return is_shared() && !is_allocated(); }

        bool is_allocated() const { return virtual_register_ != InstructionOperand::kInvalidVirtualRegister; }
    ...
    private:
        ...
        void SpillPhiGapMove(AllocatedOperand allocated_op, const InstructionBlock* block, MidTierRegisterAllocationData* data);
    ...
    int num_allocatable_registers() const { return static_cast<int>(register_data_.size()); }
...
void RegisterState::Register::PendingUse(InstructionOperand* operand, int virtual_register, bool can_be_constant, int instr_index)
{
...
void RegisterState::Register::AddDeferredBlockSpill(int instr_index, bool on_exit, Zone* zone)
{
    DCHECK(is_allocated());
...
void RegisterState::Register::Commit(AllocatedOperand allocated_op, MidTierRegisterAllocationData* data)
{
    DCHECK(is_allocated());
    DCHECK_GT(num_commits_required_, 0);
    ...
    if (--num_commits_required_ == 0) {
        ...
        VirtualRegisterData& vreg_data = data->VirtualRegisterDataFor(virtual_register());
        ...
        if (has_deferred_block_spills()) {
            for (DeferredBlockSpill& spill : *deferred_block_spills_) {
                if (spill.on_deferred_exit) {
                    vreg_data.EmitGapMoveToInputFromSpillSlot(allocated_op, spill.instr_index, data);
                } else if (!vreg_data.NeedsSpillAtOutput()) {
                    vreg_data.AddDeferredSpillOutput(allocated_op, spill.instr_index, data);
                }
            }
        }
...
void RegisterState::Register::Spill(AllocatedOperand allocated_op, const InstructionBlock* current_block, MidTierRegisterAllocationData* data)
{
    VirtualRegisterData& vreg_data = data->VirtualRegisterDataFor(virtual_register());
    ...
    if (needs_gap_move_on_spill()) {
        vreg_data.EmitGapMoveToInputFromSpillSlot(allocated_op, last_use_instr_index(), data);
    }
...
void RegisterState::Register::SpillPhiGapMove(AllocatedOperand allocated_op, const InstructionBlock* current_block, MidTierRegisterAllocationData* data)
{
    DCHECK_EQ(current_block->SuccessorCount(), 1);
    const InstructionBlock* phi_block = data->GetBlock(current_block->successors()[0]);
    ...
    for (RpoNumber predecessor : phi_block->predecessors()) {
        ...
        if (predecessor > current_block->rpo_number()) {
            ...
            vreg_data.EmitGapMoveToSpillSlot(allocated_op, predecessor_block->last_instruction_index(), data);
        }
    }
}

void RegisterState::Register::SpillPendingUses(MidTierRegisterAllocationData* data)
{
    VirtualRegisterData& vreg_data = data->VirtualRegisterDataFor(virtual_register());
    PendingOperand* pending_use = pending_uses();
    while (pending_use) {
        ...
        PendingOperand* next = pending_use->next();
        vreg_data.SpillOperand(pending_use, last_use_instr_index(), pending_uses_can_use_constant_, data);
        pending_use = next;
    }
    pending_uses_ = nullptr;
}

void RegisterState::Register::SpillForDeferred(AllocatedOperand allocated, int instr_index, MidTierRegisterAllocationData* data)
{
    DCHECK(is_allocated());
    DCHECK(is_shared());
    ...
    data->VirtualRegisterDataFor(virtual_register()).AddDeferredSpillUse(instr_index, data);
    AddDeferredBlockSpill(instr_index, true, data->allocation_zone());
    Commit(allocated, data);
}

void RegisterState::Register::MoveToSpillSlotOnDeferred(int virtual_register, int instr_index, MidTierRegisterAllocationData* data)
{
    DCHECK(!was_spilled_while_shared());
    if (!is_allocated()) {
        virtual_register_ = virtual_register;
...
RegisterState::RegisterState(RegisterKind kind, int num_allocatable_registers, Zone* zone)
    : register_data_(num_allocatable_registers, zone)
    , zone_(zone)
{
}

RegisterState::RegisterState(const RegisterState& other) V8_NOEXCEPT : register_data_(other.register_data_.begin(), other.register_data_.end(), other.zone_), zone_(other.zone_) { }
...
void RegisterState::Commit(RegisterIndex reg, AllocatedOperand allocated, InstructionOperand* operand, MidTierRegisterAllocationData* data)
{
...
void RegisterState::Spill(RegisterIndex reg, AllocatedOperand allocated, const InstructionBlock* current_block, MidTierRegisterAllocationData* data)
{
    DCHECK(IsAllocated(reg));
...
void RegisterState::SpillForDeferred(RegisterIndex reg, AllocatedOperand allocated, int instr_index, MidTierRegisterAllocationData* data)
{
    DCHECK(IsAllocated(reg));
...
void RegisterState::MoveToSpillSlotOnDeferred(RegisterIndex reg, int virtual_register, int instr_index, MidTierRegisterAllocationData* data)
{
    EnsureRegisterData(reg);
    reg_data(reg).MoveToSpillSlotOnDeferred(virtual_register, instr_index, data);
}

void RegisterState::AllocateUse(RegisterIndex reg, int virtual_register, InstructionOperand* operand, int instr_index, MidTierRegisterAllocationData* data)
{
    EnsureRegisterData(reg);
    reg_data(reg).Use(virtual_register, instr_index);
}

void RegisterState::AllocatePendingUse(RegisterIndex reg, int virtual_register, InstructionOperand* operand, bool can_be_constant, int instr_index)
{
    EnsureRegisterData(reg);
    reg_data(reg).PendingUse(operand, virtual_register, can_be_constant, instr_index);
}
...
bool RegisterState::IsShared(RegisterIndex reg) { return HasRegisterData(reg) && reg_data(reg).is_shared(); }

bool RegisterState::IsAllocated(RegisterIndex reg) { return HasRegisterData(reg) && reg_data(reg).is_allocated(); }
...
void RegisterState::CopyFrom(RegisterIndex reg, RegisterState* source) { register_data_[reg.ToInt()] = source->register_data_[reg.ToInt()]; }

bool RegisterState::Equals(RegisterIndex reg, RegisterState* other) { return register_data_[reg.ToInt()] == other->register_data_[reg.ToInt()]; }
...
RegisterState* RegisterState::Clone() { return zone_->New<RegisterState>(*this); }
...
public:
    ...
    bool operator==(const RegisterBitVector& other) const { return bits_ == other.bits_; }

    bool Contains(RegisterIndex reg, MachineRepresentation rep) const { return bits_ & reg.ToBit(rep); }

    RegisterIndex GetFirstSet() const { return RegisterIndex(base::bits::CountTrailingZeros(bits_)); }
    ...
    void Add(RegisterIndex reg, MachineRepresentation rep) { bits_ |= reg.ToBit(rep); }

    void Clear(RegisterIndex reg, MachineRepresentation rep) { bits_ &= ~reg.ToBit(rep); }

    RegisterBitVector Union(const RegisterBitVector& other) { return RegisterBitVector(bits_ | other.bits_); }
...
private:
    ...
    explicit RegisterBitVector(uintptr_t bits)
        : bits_(bits)
    {
    }

    static_assert(RegisterConfiguration::kMaxRegisters <= sizeof(uintptr_t) * 8, "Maximum registers must fit in uintptr_t bitmap");
    uintptr_t bits_;
};

std::ostream& operator<<(std::ostream& os, RegisterBitVector register_bits) { return os << std::hex << register_bits.bits_ << std::dec; }
...
public:
    SinglePassRegisterAllocator(RegisterKind kind, MidTierRegisterAllocationData* data);
    ...
    void AllocateConstantOutput(ConstantOperand* operand, VirtualRegisterData& vreg, int instr_index);
    void AllocateOutput(UnallocatedOperand* operand, VirtualRegisterData& vreg, int instr_index);
    void AllocateInput(UnallocatedOperand* operand, VirtualRegisterData& vreg, int instr_index);
    void AllocateSameInputOutput(UnallocatedOperand* output, UnallocatedOperand* input, VirtualRegisterData& output_vreg, VirtualRegisterData& input_vreg, int instr_index);
    void AllocateGapMoveInput(UnallocatedOperand* operand, VirtualRegisterData& vreg, int instr_index);
    void AllocateTemp(UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index);
    void AllocatePhi(VirtualRegisterData& virtual_register, const InstructionBlock* block);
    void AllocatePhiGapMove(VirtualRegisterData& to_vreg, VirtualRegisterData& from_vreg, int instr_index);

    // Reserve any fixed registers for the operands on an instruction before doing
    // allocation on the operands.
    void ReserveFixedInputRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index);
    void ReserveFixedTempRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index);
    void ReserveFixedOutputRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index);
    ...
    void AllocateDeferredBlockSpillOutput(int instr_index, RpoNumber deferred_block, VirtualRegisterData& virtual_register);
...
private:
    ...
    void SpillRegisterAtMerge(RegisterState* reg_state, RegisterIndex reg, MachineRepresentation rep);

    // Introduce a gap move to move |virtual_register| from reg |from| to reg |to|
    // on entry to a |successor| block.
    void MoveRegisterOnMerge(RegisterIndex from, RegisterIndex to, VirtualRegisterData& virtual_register, RpoNumber successor, RegisterState* succ_state);
    ...
    // |pos| use position.
    void AllocateUse(RegisterIndex reg, VirtualRegisterData& virtual_register, InstructionOperand* operand, int instr_index, UsePosition pos);
    ...
    // |instr_index|.
    void AllocatePendingUse(RegisterIndex reg, VirtualRegisterData& virtual_register, InstructionOperand* operand, bool can_be_constant, int instr_index);
    ...
    void AllocateUseWithMove(RegisterIndex reg, VirtualRegisterData& virtual_register, UnallocatedOperand* operand, int instr_index, UsePosition pos);

    void CommitRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep, InstructionOperand* operand, UsePosition pos);
    void SpillRegister(RegisterIndex reg);
    void SpillRegisterAndPotentialSimdSibling(RegisterIndex reg, MachineRepresentation rep);
    ...
    // |virtual_register|.
    AllocatedOperand AllocatedOperandForReg(RegisterIndex reg, MachineRepresentation rep);

    void ReserveFixedRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index, UsePosition pos);
    RegisterIndex AllocateOutput(UnallocatedOperand* operand, VirtualRegisterData& vreg_data, int instr_index, UsePosition pos);
    void EmitGapMoveFromOutput(InstructionOperand from, InstructionOperand to, int instr_index);
    ...
    V8_INLINE RegisterIndex ChooseRegisterFor(MachineRepresentation rep, UsePosition pos, bool must_use_register);
    V8_INLINE RegisterIndex ChooseFreeRegister(MachineRepresentation rep, UsePosition pos);
    V8_INLINE RegisterIndex ChooseFreeRegister(const RegisterBitVector& allocated_regs, MachineRepresentation rep);
    V8_INLINE RegisterIndex ChooseRegisterToSpill(MachineRepresentation rep, UsePosition pos);

    // Assign, free and mark use's of |reg| for a |virtual_register| at use
    // position |pos|.
    V8_INLINE void AssignRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep, UsePosition pos);
    V8_INLINE void FreeRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep);
    V8_INLINE void MarkRegisterUse(RegisterIndex reg, MachineRepresentation rep, UsePosition pos);
    ...
    bool VirtualRegisterIsUnallocatedOrInReg(int virtual_register, RegisterIndex reg);
    ...
    RegisterIndex simdSibling(RegisterIndex reg) const
    {
        ...
        DCHECK_EQ(2,
            data_->config()->GetAliases(MachineRepresentation::kSimd128, ToRegCode(reg, MachineRepresentation::kSimd128), MachineRepresentation::kFloat64, &double_reg_base_code));
        DCHECK_EQ(reg, FromRegCode(double_reg_base_code, MachineRepresentation::kFloat64));
        DCHECK_EQ(sibling, FromRegCode(double_reg_base_code + 1, MachineRepresentation::kFloat64));
#endif // DEBUG
        return sibling;
    }
    ...
    VirtualRegisterData& VirtualRegisterDataFor(int virtual_register) const { return data_->VirtualRegisterDataFor(virtual_register); }
...
SinglePassRegisterAllocator::SinglePassRegisterAllocator(RegisterKind kind, MidTierRegisterAllocationData* data)
    : virtual_register_to_reg_(data->code()->VirtualRegisterCount(), data->allocation_zone())
    , register_state_(nullptr)
    , current_block_(nullptr)
    , kind_(kind)
    , num_allocatable_registers_(GetAllocatableRegisterCount(data->config(), kind))
    , reg_code_to_index_(GetRegisterCount(data->config(), kind), data->allocation_zone())
    , index_to_reg_code_(GetAllocatableRegisterCodes(data->config(), kind))
    , assigned_registers_(data->code_zone()->New<BitVector>(GetRegisterCount(data->config(), kind), data->code_zone()))
    , data_(data)
    ...
    , same_input_output_registers_bits_()
{
    ...
    if (kFPAliasing == AliasingKind::kCombine && kind == RegisterKind::kDouble) {
        ...
        //  Float registers.
        float32_reg_code_to_index_.emplace(config->num_float_registers(), data->allocation_zone());
        index_to_float32_reg_code_.emplace(num_allocatable_registers_, -1, data->allocation_zone());
        for (int i = 0; i < config->num_allocatable_float_registers(); i++) {
            ...
            if (reg_code % 2 != 0)
                continue;
            int double_reg_base_code;
            CHECK_EQ(1, config->GetAliases(MachineRepresentation::kFloat32, reg_code, MachineRepresentation::kFloat64, &double_reg_base_code));
        ...
        //  Simd128 registers.
        simd128_reg_code_to_index_.emplace(config->num_simd128_registers(), data->allocation_zone());
        index_to_simd128_reg_code_.emplace(num_allocatable_registers_, -1, data->allocation_zone());
        for (int i = 0; i < config->num_allocatable_simd128_registers(); i++) {
            int reg_code = config->allocatable_simd128_codes()[i];
            int double_reg_base_code;
            CHECK_EQ(2, config->GetAliases(MachineRepresentation::kSimd128, reg_code, MachineRepresentation::kFloat64, &double_reg_base_code));
            RegisterIndex double_reg { reg_code_to_index_[double_reg_base_code] };
            // We later rely on the fact that the two aliasing double registers are at
            // consecutive indexes.
            DCHECK_EQ(double_reg.ToInt() + 1, reg_code_to_index_[double_reg_base_code + 1].ToInt());
...
int SinglePassRegisterAllocator::VirtualRegisterForRegister(RegisterIndex reg) { return register_state_->VirtualRegisterForRegister(reg); }

RegisterIndex SinglePassRegisterAllocator::RegisterForVirtualRegister(int virtual_register)
{
...
void SinglePassRegisterAllocator::EndBlock(const InstructionBlock* block)
{
    ...
    if (block->PredecessorCount() > 1) {
        register_state_->AddSharedUses(static_cast<int>(block->PredecessorCount()) - 1);
    }
    ...
    while (!allocated_registers_bits_.IsEmpty()) {
        ...
        VirtualRegisterData& vreg_data = data_->VirtualRegisterDataFor(VirtualRegisterForRegister(reg));
...
void SinglePassRegisterAllocator::MergeStateFrom(const InstructionBlock::Successors& successors)
{
    for (RpoNumber successor : successors) {
        ...
        if (register_state_ == nullptr) {
            ...
            for (RegisterIndex reg : *successor_registers) {
                ...
                if (!successor_registers->IsAllocated(reg))
                    continue;

                int virtual_register = successor_registers->VirtualRegisterForRegister(reg);
                ...
                if (processed_regs.Contains(reg, rep))
                    continue;
                ...
                if (kFPAliasing == AliasingKind::kCombine && (rep == MachineRepresentation::kFloat64 || rep == MachineRepresentation::kFloat32)) {
                    int simd_reg_code;
                    CHECK_EQ(1, data_->config()->GetAliases(rep, ToRegCode(reg, rep), MachineRepresentation::kSimd128, &simd_reg_code));
                    // Sanity check: The SIMD reg code should be the shifted FP reg code.
                    DCHECK_EQ(simd_reg_code, ToRegCode(reg, rep) >> (rep == MachineRepresentation::kFloat64 ? 1 : 2));
                    RegisterIndex simd_reg = FromRegCode(simd_reg_code, MachineRepresentation::kSimd128);
                    reg_in_use |= simd_reg.is_valid() && register_state_->IsAllocated(simd_reg)
                        && VirtualRegisterDataFor(VirtualRegisterForRegister(simd_reg)).rep() == MachineRepresentation::kSimd128;
                }
                ...
                if (!new_reg.is_valid()) {
                    new_reg = ChooseFreeRegister(allocated_registers_bits_.Union(succ_allocated_regs), rep);
                } else if (new_reg != reg) {
                ...
                if (new_reg.is_valid()) {
                    MoveRegisterOnMerge(new_reg, reg, vreg_data, successor, successor_registers);
...
RegisterBitVector SinglePassRegisterAllocator::GetAllocatedRegBitVector(RegisterState* reg_state)
{
    RegisterBitVector allocated_regs;
    for (RegisterIndex reg : *reg_state) {
        if (reg_state->IsAllocated(reg)) {
            VirtualRegisterData virtual_register = VirtualRegisterDataFor(reg_state->VirtualRegisterForRegister(reg));
...
void SinglePassRegisterAllocator::SpillRegisterAtMerge(RegisterState* reg_state, RegisterIndex reg, MachineRepresentation rep)
{
    DCHECK_NE(reg_state, register_state_);
    if (reg_state->IsAllocated(reg)) {
        int virtual_register = reg_state->VirtualRegisterForRegister(reg);
        VirtualRegisterData& vreg_data = data_->VirtualRegisterDataFor(virtual_register);
    ...
    if (kFPAliasing == AliasingKind::kCombine && rep == MachineRepresentation::kSimd128) {
        ...
        if (reg_state->IsAllocated(sibling)) {
            ...
            VirtualRegisterData& vreg_data = data_->VirtualRegisterDataFor(virtual_register);
            AllocatedOperand allocated = AllocatedOperandForReg(sibling, vreg_data.rep());
    ...
    if (kFPAliasing == AliasingKind::kCombine && (rep == MachineRepresentation::kFloat64 || rep == MachineRepresentation::kFloat32)) {
        ...
        DCHECK_EQ(simd_reg_code, ToRegCode(reg, rep) >> (rep == MachineRepresentation::kFloat64 ? 1 : 2));
        RegisterIndex simd_reg = FromRegCode(simd_reg_code, MachineRepresentation::kSimd128);
        DCHECK(!simd_reg.is_valid() || simd_reg == reg || simdSibling(simd_reg) == reg);
        if (simd_reg.is_valid() && reg_state->IsAllocated(simd_reg)) {
            ...
            if (vreg_data.rep() == MachineRepresentation::kSimd128) {
                AllocatedOperand allocated = AllocatedOperandForReg(simd_reg, vreg_data.rep());
...
void SinglePassRegisterAllocator::MoveRegisterOnMerge(RegisterIndex from, RegisterIndex to, VirtualRegisterData& virtual_register, RpoNumber successor, RegisterState* succ_state)
{
    int instr_index = data_->GetBlock(successor)->first_instruction_index();
    MoveOperands* move = data_->AddPendingOperandGapMove(instr_index, Instruction::START);
    succ_state->Commit(to, AllocatedOperandForReg(to, virtual_register.rep()), &move->destination(), data_);
    AllocatePendingUse(from, virtual_register, &move->source(), true, instr_index);
}
...
void SinglePassRegisterAllocator::UpdateVirtualRegisterState()
{
    ...
    for (RegisterIndex reg : *register_state_) {
        ...
        if (virtual_register != InstructionOperand::kInvalidVirtualRegister) {
            MachineRepresentation rep = data_->VirtualRegisterDataFor(virtual_register).rep();
            AssignRegister(reg, virtual_register, rep, UsePosition::kNone);
        }
    }
    CheckConsistency();
...
void SinglePassRegisterAllocator::CheckConsistency()
{
    ...
    for (RegisterIndex reg : *register_state_) {
        if (!register_state_->IsAllocated(reg))
            continue;
        int virtual_register = VirtualRegisterForRegister(reg);
        // reg <-> vreg linking is consistent.
...
RegisterIndex SinglePassRegisterAllocator::FromRegCode(int reg_code, MachineRepresentation rep) const
{
    if (kFPAliasing == AliasingKind::kCombine && kind() == RegisterKind::kDouble) {
...
int SinglePassRegisterAllocator::ToRegCode(RegisterIndex reg, MachineRepresentation rep) const
{
    if (kFPAliasing == AliasingKind::kCombine && kind() == RegisterKind::kDouble) {
        if (rep == MachineRepresentation::kFloat32) {
...
bool SinglePassRegisterAllocator::VirtualRegisterIsUnallocatedOrInReg(int virtual_register, RegisterIndex reg)
{
    RegisterIndex existing_reg = RegisterForVirtualRegister(virtual_register);
    return !existing_reg.is_valid() || existing_reg == reg;
}

bool SinglePassRegisterAllocator::IsFreeOrSameVirtualRegister(RegisterIndex reg, int virtual_register)
{
    int allocated_vreg = VirtualRegisterForRegister(reg);
    return allocated_vreg == InstructionOperand::kInvalidVirtualRegister || allocated_vreg == virtual_register;
}

void SinglePassRegisterAllocator::EmitGapMoveFromOutput(InstructionOperand from, InstructionOperand to, int instr_index)
{
    ...
    if (instr_index == block->last_instruction_index()) {
        ...
        for (const RpoNumber& succ : block->successors()) {
            ...
            DCHECK_EQ(1, successor->PredecessorCount());
            data_->AddGapMove(successor->first_instruction_index(), Instruction::START, from, to);
        }
    } else {
...
void SinglePassRegisterAllocator::AssignRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep, UsePosition pos)
{
...
void SinglePassRegisterAllocator::MarkRegisterUse(RegisterIndex reg, MachineRepresentation rep, UsePosition pos)
{
...
void SinglePassRegisterAllocator::FreeRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep)
{
...
RegisterIndex SinglePassRegisterAllocator::ChooseRegisterFor(VirtualRegisterData& virtual_register, int instr_index, UsePosition pos, bool must_use_register)
{
    ...
    if (!reg.is_valid() && (must_use_register || !virtual_register.IsSpilledAt(instr_index, data_))) {
        reg = ChooseRegisterFor(rep, pos, must_use_register);
    } else if (reg.is_valid() && same_input_output_registers_bits_.Contains(reg, rep) && pos != UsePosition::kStart) {
...
RegisterIndex SinglePassRegisterAllocator::ChooseRegisterFor(MachineRepresentation rep, UsePosition pos, bool must_use_register)
{
    DCHECK_NE(pos, UsePosition::kNone);
...
bool SinglePassRegisterAllocator::IsValidForRep(RegisterIndex reg, MachineRepresentation rep)
{
    if (kFPAliasing != AliasingKind::kCombine || kind() == RegisterKind::kGeneral) {
        return true;
    } else {
        switch (rep) {
...
RegisterIndex SinglePassRegisterAllocator::ChooseFreeRegister(MachineRepresentation rep, UsePosition pos)
{
    // Take the first free, non-blocked register, if available.
    // TODO(rmcilroy): Consider a better heuristic.
    RegisterBitVector allocated_or_in_use = InUseBitmap(pos).Union(allocated_registers_bits_);
    return ChooseFreeRegister(allocated_or_in_use, rep);
}

RegisterIndex SinglePassRegisterAllocator::ChooseFreeRegister(const RegisterBitVector& allocated_regs, MachineRepresentation rep)
{
...
RegisterIndex SinglePassRegisterAllocator::ChooseRegisterToSpill(MachineRepresentation rep, UsePosition pos)
{
    ...
    for (RegisterIndex reg : *register_state_) {
        ...
        if (!IsValidForRep(reg, rep) || in_use.Contains(reg, rep))
            continue;
        // With non-simple FP aliasing, a SIMD register might block more than one FP
        // register.
        DCHECK_IMPLIES(kFPAliasing != AliasingKind::kCombine, register_state_->IsAllocated(reg));
        if (kFPAliasing == AliasingKind::kCombine && !register_state_->IsAllocated(reg))
            continue;

        VirtualRegisterData& vreg_data = VirtualRegisterDataFor(VirtualRegisterForRegister(reg));
        if ((!pending_only_use && register_state_->HasPendingUsesOnly(reg)) || (!already_spilled && vreg_data.HasSpillOperand())
...
void SinglePassRegisterAllocator::CommitRegister(RegisterIndex reg, int virtual_register, MachineRepresentation rep, InstructionOperand* operand, UsePosition pos)
{
...
void SinglePassRegisterAllocator::SpillRegisterAndPotentialSimdSibling(RegisterIndex reg, MachineRepresentation rep)
{
    SpillRegister(reg);
...
void SinglePassRegisterAllocator::SpillRegisterForVirtualRegister(int virtual_register)
{
    DCHECK_NE(virtual_register, InstructionOperand::kInvalidVirtualRegister);
...
void SinglePassRegisterAllocator::SpillRegisterForDeferred(RegisterIndex reg, int instr_index)
{
    // Committing the output operation, and mark the register use in this
    // instruction, then mark it as free going forward.
    if (register_state_->IsAllocated(reg) && register_state_->IsShared(reg)) {
        VirtualRegisterData& virtual_register = data_->VirtualRegisterDataFor(VirtualRegisterForRegister(reg));
        AllocatedOperand allocated = AllocatedOperandForReg(reg, virtual_register.rep());
...
void SinglePassRegisterAllocator::AllocateDeferredBlockSpillOutput(int instr_index, RpoNumber deferred_block, VirtualRegisterData& virtual_register)
{
    DCHECK(data_->GetBlock(deferred_block)->IsDeferred());
    DCHECK(virtual_register.HasSpillRange());
    if (!virtual_register.NeedsSpillAtOutput() && !DefinedAfter(virtual_register.vreg(), instr_index, UsePosition::kEnd)) {
        ...
        RegisterIndex reg = RegisterForVirtualRegister(virtual_register.vreg());
        if (reg.is_valid()) {
            int deferred_block_start = data_->GetBlock(deferred_block)->first_instruction_index();
            register_state_->MoveToSpillSlotOnDeferred(reg, virtual_register.vreg(), deferred_block_start, data_);
            return;
        } else {
...
AllocatedOperand SinglePassRegisterAllocator::AllocatedOperandForReg(RegisterIndex reg, MachineRepresentation rep)
{
    return AllocatedOperand(AllocatedOperand::REGISTER, rep, ToRegCode(reg, rep));
}

void SinglePassRegisterAllocator::AllocateUse(RegisterIndex reg, VirtualRegisterData& virtual_register, InstructionOperand* operand, int instr_index, UsePosition pos)
{
    DCHECK(IsFreeOrSameVirtualRegister(reg, virtual_register.vreg()));

    AllocatedOperand allocated = AllocatedOperandForReg(reg, virtual_register.rep());
    register_state_->Commit(reg, allocated, operand, data_);
    register_state_->AllocateUse(reg, virtual_register.vreg(), operand, instr_index, data_);
    AssignRegister(reg, virtual_register.vreg(), virtual_register.rep(), pos);
    CheckConsistency();
}

void SinglePassRegisterAllocator::AllocatePendingUse(RegisterIndex reg, VirtualRegisterData& virtual_register, InstructionOperand* operand, bool can_be_constant, int instr_index)
{
    DCHECK(IsFreeOrSameVirtualRegister(reg, virtual_register.vreg()));

    register_state_->AllocatePendingUse(reg, virtual_register.vreg(), operand, can_be_constant, instr_index);
    ...
    AssignRegister(reg, virtual_register.vreg(), virtual_register.rep(), UsePosition::kNone);
    CheckConsistency();
}

void SinglePassRegisterAllocator::AllocateUseWithMove(RegisterIndex reg, VirtualRegisterData& virtual_register, UnallocatedOperand* operand, int instr_index, UsePosition pos)
{
    AllocatedOperand to = AllocatedOperandForReg(reg, virtual_register.rep());
    UnallocatedOperand from = UnallocatedOperand(UnallocatedOperand::REGISTER_OR_SLOT_OR_CONSTANT, virtual_register.vreg());
...
void SinglePassRegisterAllocator::AllocateInput(UnallocatedOperand* operand, VirtualRegisterData& virtual_register, int instr_index)
{
    EnsureRegisterState();
    ...
    if (operand->HasFixedSlotPolicy()) {
        ...
        UnallocatedOperand input_copy(UnallocatedOperand::REGISTER_OR_SLOT_OR_CONSTANT, virtual_register.vreg());
        AllocatedOperand allocated = AllocatedOperand(AllocatedOperand::STACK_SLOT, virtual_register.rep(), operand->fixed_slot_index());
        InstructionOperand::ReplaceWith(operand, &allocated);
        MoveOperands* move_op = data_->AddGapMove(instr_index, Instruction::END, input_copy, *operand);
    ...
    UsePosition pos = operand->IsUsedAtStart() ? UsePosition::kStart : UsePosition::kAll;
    if (operand->HasFixedRegisterPolicy() || operand->HasFixedFPRegisterPolicy()) {
        ...
        RegisterIndex reg = FromRegCode(operand->fixed_register_index(), virtual_register.rep());
        ...
        RegisterIndex reg = ChooseRegisterFor(virtual_register, instr_index, pos, must_use_register);

        if (!reg.is_valid()) {
            // The register will have been spilled at this use.
            virtual_register.SpillOperand(operand, instr_index, operand->HasRegisterOrSlotOrConstantPolicy(), data_);
        } else if (!must_use_register) {
            // We might later dedice to spill this register; allocate a pending use.
            AllocatePendingUse(reg, virtual_register, operand, operand->HasRegisterOrSlotOrConstantPolicy(), instr_index);
        } else if (VirtualRegisterIsUnallocatedOrInReg(virtual_register.vreg(), reg)) {
...
void SinglePassRegisterAllocator::AllocateGapMoveInput(UnallocatedOperand* operand, VirtualRegisterData& vreg_data, int instr_index)
{
    EnsureRegisterState();
    // Gap move inputs should be unconstrained.
    DCHECK(operand->HasRegisterOrSlotOrConstantPolicy());
    RegisterIndex reg = ChooseRegisterFor(vreg_data, instr_index, UsePosition::kStart, false);
    if (reg.is_valid()) {
...
void SinglePassRegisterAllocator::AllocateConstantOutput(ConstantOperand* operand, VirtualRegisterData& vreg_data, int instr_index)
{
    EnsureRegisterState();
    ...
    if (vreg_data.NeedsSpillAtOutput()) {
        vreg_data.EmitGapMoveFromOutputToSpillSlot(*operand, current_block_, instr_index, data_);
    }
}

void SinglePassRegisterAllocator::AllocateOutput(UnallocatedOperand* operand, VirtualRegisterData& vreg_data, int instr_index)
{
    AllocateOutput(operand, vreg_data, instr_index, UsePosition::kEnd);
}

RegisterIndex SinglePassRegisterAllocator::AllocateOutput(UnallocatedOperand* operand, VirtualRegisterData& vreg_data, int instr_index, UsePosition pos)
{
    EnsureRegisterState();
    ...
    if (operand->HasSlotPolicy() || operand->HasFixedSlotPolicy()) {
        ...
        reg = ChooseRegisterFor(vreg_data, instr_index, pos, operand->HasRegisterPolicy());
    }
    ...
    if (!reg.is_valid()) {
        ...
        if (!VirtualRegisterIsUnallocatedOrInReg(virtual_register, reg)) {
            ...
            CommitRegister(existing_reg, vreg_data.vreg(), vreg_data.rep(), &move_output_to, UsePosition::kNone);
        }
        ...
        if (vreg_data.NeedsSpillAtOutput()) {
            vreg_data.EmitGapMoveFromOutputToSpillSlot(*AllocatedOperand::cast(operand), current_block_, instr_index, data_);
...
void SinglePassRegisterAllocator::AllocateSameInputOutput(
    UnallocatedOperand* output, UnallocatedOperand* input, VirtualRegisterData& output_vreg_data, VirtualRegisterData& input_vreg_data, int instr_index)
{
    EnsureRegisterState();
    ...
    RegisterIndex reg = AllocateOutput(output, output_vreg_data, instr_index, UsePosition::kAll);

    if (reg.is_valid()) {
        // Replace the input operand with an unallocated fixed register policy for
        // the same register.
        UnallocatedOperand::ExtendedPolicy policy = kind() == RegisterKind::kGeneral ? UnallocatedOperand::FIXED_REGISTER : UnallocatedOperand::FIXED_FP_REGISTER;
        ...
        UnallocatedOperand unconstrained_input(UnallocatedOperand::REGISTER_OR_SLOT_OR_CONSTANT, input_vreg);
        MoveOperands* move_ops = data_->AddGapMove(instr_index, Instruction::END, unconstrained_input, PendingOperand());
        output_vreg_data.SpillOperand(&move_ops->destination(), instr_index, true, data_);
    }
}

void SinglePassRegisterAllocator::AllocateTemp(UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index)
{
    EnsureRegisterState();
    RegisterIndex reg;
    ...
    if (operand->HasSlotPolicy()) {
        reg = RegisterIndex::Invalid();
    } else if (operand->HasFixedRegisterPolicy() || operand->HasFixedFPRegisterPolicy()) {
        reg = FromRegCode(operand->fixed_register_index(), rep);
    } else {
        reg = ChooseRegisterFor(rep, UsePosition::kAll, operand->HasRegisterPolicy());
    }

    if (reg.is_valid()) {
        DCHECK(virtual_register == InstructionOperand::kInvalidVirtualRegister || VirtualRegisterIsUnallocatedOrInReg(virtual_register, reg));
        CommitRegister(reg, virtual_register, rep, operand, UsePosition::kAll);
    } else {
        VirtualRegisterData& vreg_data = VirtualRegisterDataFor(virtual_register);
        vreg_data.SpillOperand(operand, instr_index, operand->HasRegisterOrSlotOrConstantPolicy(), data_);
    }
}

bool SinglePassRegisterAllocator::DefinedAfter(int virtual_register, int instr_index, UsePosition pos)
{
    if (virtual_register == InstructionOperand::kInvalidVirtualRegister)
        return false;
    int defined_at = VirtualRegisterDataFor(virtual_register).output_instr_index();
    return defined_at > instr_index || (defined_at == instr_index && pos == UsePosition::kStart);
}

void SinglePassRegisterAllocator::ReserveFixedInputRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index)
{
    ReserveFixedRegister(operand, virtual_register, rep, instr_index, operand->IsUsedAtStart() ? UsePosition::kStart : UsePosition::kAll);
}

void SinglePassRegisterAllocator::ReserveFixedTempRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index)
{
    ReserveFixedRegister(operand, virtual_register, rep, instr_index, UsePosition::kAll);
}

void SinglePassRegisterAllocator::ReserveFixedOutputRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index)
{
    ReserveFixedRegister(operand, virtual_register, rep, instr_index, UsePosition::kEnd);
}

void SinglePassRegisterAllocator::ReserveFixedRegister(const UnallocatedOperand* operand, int virtual_register, MachineRepresentation rep, int instr_index, UsePosition pos)
{
    EnsureRegisterState();
    int reg_code = operand->fixed_register_index();
    RegisterIndex reg = FromRegCode(reg_code, rep);
    if (!IsFreeOrSameVirtualRegister(reg, virtual_register) && !DefinedAfter(virtual_register, instr_index, pos)) {
    ...
    if (kFPAliasing == AliasingKind::kCombine && rep == MachineRepresentation::kSimd128) {
        if (register_state_->IsAllocated(simdSibling(reg)) && !DefinedAfter(virtual_register, instr_index, pos)) {
            SpillRegister(simdSibling(reg));
        }
    }
    ...
    if (kFPAliasing == AliasingKind::kCombine && (rep == MachineRepresentation::kFloat64 || rep == MachineRepresentation::kFloat32)) {
        int simd_reg_code;
        CHECK_EQ(1, data_->config()->GetAliases(rep, reg_code, MachineRepresentation::kSimd128, &simd_reg_code));
        // Sanity check: The SIMD register code should be the shifted {reg_code}.
        DCHECK_EQ(simd_reg_code, reg_code >> (rep == MachineRepresentation::kFloat64 ? 1 : 2));
        RegisterIndex simd_reg = FromRegCode(simd_reg_code, MachineRepresentation::kSimd128);
        DCHECK(simd_reg == reg || simdSibling(simd_reg) == reg);
        int allocated_vreg = VirtualRegisterForRegister(simd_reg);
        if (simd_reg != reg && allocated_vreg != InstructionOperand::kInvalidVirtualRegister && VirtualRegisterDataFor(allocated_vreg).rep() == MachineRepresentation::kSimd128
...
void SinglePassRegisterAllocator::AllocatePhiGapMove(VirtualRegisterData& to_vreg, VirtualRegisterData& from_vreg, int instr_index)
{
    EnsureRegisterState();
    ...
    if (to_register.is_valid() && !from_register.is_valid()) {
        ...
        CommitRegister(to_register, to_vreg.vreg(), to_vreg.rep(), &operand, UsePosition::kAll);
        AllocateUse(to_register, from_vreg, &operand, instr_index, UsePosition::kAll);
    } else {
        // Otherwise add a gap move.
        MoveOperands* move = data_->AddPendingOperandGapMove(instr_index, Instruction::END);
        ...
        if (to_register.is_valid()) {
            CommitRegister(to_register, to_vreg.vreg(), to_vreg.rep(), to_operand, UsePosition::kAll);
        } else {
            to_vreg.SpillOperand(to_operand, instr_index, true, data_);
        }

        // The from side is unconstrained.
        UnallocatedOperand unconstrained_input(UnallocatedOperand::REGISTER_OR_SLOT_OR_CONSTANT, from_vreg.vreg());
        InstructionOperand::ReplaceWith(from_operand, &unconstrained_input);
    }
}

void SinglePassRegisterAllocator::AllocatePhi(VirtualRegisterData& virtual_register, const InstructionBlock* block)
{
...
void SinglePassRegisterAllocator::EnsureRegisterState()
{
    if (V8_UNLIKELY(!register_state_)) {
        register_state_ = RegisterState::New(kind(), num_allocatable_registers_, data_->allocation_zone());
    }
}
...
private:
    ...
    bool IsDeferredBlockBoundary(const ZoneVector<RpoNumber>& blocks) { return blocks.size() == 1 && !data_->GetBlock(blocks[0])->IsDeferred(); }
...
MidTierOutputProcessor::MidTierOutputProcessor(MidTierRegisterAllocationData* data)
    : data_(data)
...
void MidTierOutputProcessor::PopulateDeferredBlockRegion(RpoNumber initial_block)
{
    DeferredBlocksRegion* deferred_blocks_region = zone()->New<DeferredBlocksRegion>(zone());
    ...
    while (!deferred_blocks_worklist_.empty()) {
        ...
        if (IsDeferredBlockBoundary(curr_block->predecessors())) {
            ...
            data_->block_state(curr_block->predecessors()[0]).MarkAsDeferredBlockBoundary();
        } else {
...
void MidTierOutputProcessor::InitializeBlockState(const InstructionBlock* block)
{
    ...
    if (block->dominator().IsValid()) {
        ...
        dominator_block_state.dominated_blocks()->Union(*block_state.dominated_blocks());
    } else {
...
void MidTierOutputProcessor::DefineOutputs(const InstructionBlock* block)
{
    ...
    for (int index = block->last_instruction_index(); index >= block_start; index--) {
        ...
        for (size_t i = 0; i < instr->OutputCount(); i++) {
            ...
            if (output->IsConstant()) {
                ...
                VirtualRegisterDataFor(virtual_register).DefineAsConstantOperand(constant_operand, rep, index, is_deferred);
            } else {
                DCHECK(output->IsUnallocated());
                UnallocatedOperand* unallocated_operand = UnallocatedOperand::cast(output);
                int virtual_register = unallocated_operand->virtual_register();
                MachineRepresentation rep = RepresentationFor(virtual_register);
                bool is_exceptional_call_output = instr->IsCallWithDescriptorFlags() && instr->HasCallDescriptorFlag(CallDescriptor::kHasExceptionHandler);
                if (unallocated_operand->HasFixedSlotPolicy()) {
                    ...
                    AllocatedOperand* fixed_spill_operand = AllocatedOperand::New(zone(), AllocatedOperand::STACK_SLOT, rep, unallocated_operand->fixed_slot_index());
                    VirtualRegisterDataFor(virtual_register).DefineAsFixedSpillOperand(fixed_spill_operand, virtual_register, rep, index, is_deferred, is_exceptional_call_output);
                } else {
                    VirtualRegisterDataFor(virtual_register).DefineAsUnallocatedOperand(virtual_register, rep, index, is_deferred, is_exceptional_call_output);
                }
            }
        }
    ...
    for (PhiInstruction* phi : block->phis()) {
        ...
        MachineRepresentation rep = RepresentationFor(virtual_register);
        VirtualRegisterDataFor(virtual_register).DefineAsPhi(virtual_register, rep, block->first_instruction_index(), is_deferred);
    }
}
...
void DefineOutputs(MidTierRegisterAllocationData* data)
{
    ...
    for (const InstructionBlock* block : base::Reversed(data->code()->instruction_blocks())) {
...
public:
    ...
    SinglePassRegisterAllocator& general_reg_allocator() { return general_reg_allocator_; }
    SinglePassRegisterAllocator& double_reg_allocator() { return double_reg_allocator_; }
...
MidTierRegisterAllocator::MidTierRegisterAllocator(MidTierRegisterAllocationData* data)
    : data_(data)
...
void MidTierRegisterAllocator::AllocateRegisters(const InstructionBlock* block)
{
    RpoNumber block_rpo = block->rpo_number();
    bool is_deferred_block_boundary = data_->block_state(block_rpo).is_deferred_block_boundary();
    ...
    if (is_deferred_block_boundary && !block->IsDeferred()) {
        for (RpoNumber successor : block->successors()) {
            if (!data_->GetBlock(successor)->IsDeferred())
                continue;
            DCHECK_GT(successor, block_rpo);
            DeferredBlocksRegion* deferred_region = data_->block_state(successor).deferred_blocks_region();
            ...
            for (const int virtual_register : *deferred_region) {
                VirtualRegisterData& vreg_data = VirtualRegisterDataFor(virtual_register);
                AllocatorFor(vreg_data.rep()).AllocateDeferredBlockSpillOutput(block->last_instruction_index(), successor, vreg_data);
            }
        }
    }
    ...
    // to the start.
    int block_start = block->first_instruction_index();
    for (int instr_index = block->last_instruction_index(); instr_index >= block_start; instr_index--) {
        Instruction* instr = code()->InstructionAt(instr_index);
        ...
        for (size_t i = 0; i < instr->OutputCount(); i++) {
            ...
            if (output->IsConstant()) {
                ConstantOperand* constant_operand = ConstantOperand::cast(output);
                VirtualRegisterData& vreg_data = VirtualRegisterDataFor(constant_operand->virtual_register());
                AllocatorFor(vreg_data.rep()).AllocateConstantOutput(constant_operand, vreg_data, instr_index);
            } else {
                UnallocatedOperand* unallocated_output = UnallocatedOperand::cast(output);
                VirtualRegisterData& output_vreg_data = VirtualRegisterDataFor(unallocated_output->virtual_register());

                if (unallocated_output->HasSameAsInputPolicy()) {
                    DCHECK_EQ(i, 0);
                    UnallocatedOperand* unallocated_input = UnallocatedOperand::cast(instr->InputAt(unallocated_output->input_index()));
                    VirtualRegisterData& input_vreg_data = VirtualRegisterDataFor(unallocated_input->virtual_register());
                    DCHECK_EQ(AllocatorFor(output_vreg_data.rep()).kind(), AllocatorFor(input_vreg_data.rep()).kind());
                    AllocatorFor(output_vreg_data.rep()).AllocateSameInputOutput(unallocated_output, unallocated_input, output_vreg_data, input_vreg_data, instr_index);
                } else {
                    AllocatorFor(output_vreg_data.rep()).AllocateOutput(unallocated_output, output_vreg_data, instr_index);
                }
            }
        }
        ...
        for (size_t i = 0; i < instr->TempCount(); i++) {
            ...
            MachineRepresentation rep
                = virtual_register == InstructionOperand::kInvalidVirtualRegister ? InstructionSequence::DefaultRepresentation() : code()->GetRepresentation(virtual_register);
        ...
        for (size_t i = 0; i < instr->InputCount(); i++) {
            if (!instr->InputAt(i)->IsUnallocated())
                continue;
            UnallocatedOperand* input = UnallocatedOperand::cast(instr->InputAt(i));
            if (input->IsUsedAtStart())
                continue;
            VirtualRegisterData& vreg_data = VirtualRegisterDataFor(input->virtual_register());
            AllocatorFor(vreg_data.rep()).AllocateInput(input, vreg_data, instr_index);
        }
        ...
        if (moves != nullptr) {
            for (MoveOperands* move : *moves) {
                ...
                if (move->source().IsUnallocated()) {
                    UnallocatedOperand* source = UnallocatedOperand::cast(&move->source());
                    VirtualRegisterData& vreg_data = VirtualRegisterDataFor(source->virtual_register());
                    AllocatorFor(vreg_data.rep()).AllocateGapMoveInput(source, vreg_data, instr_index);
                }
            }
        }
...
SinglePassRegisterAllocator& MidTierRegisterAllocator::AllocatorFor(MachineRepresentation rep) { return IsFloatingPoint(rep) ? double_reg_allocator_ : general_reg_allocator_; }

bool MidTierRegisterAllocator::IsFixedRegisterPolicy(const UnallocatedOperand* operand) { return operand->HasFixedRegisterPolicy() || operand->HasFixedFPRegisterPolicy(); }

void MidTierRegisterAllocator::ReserveFixedRegisters(int instr_index)
{
    Instruction* instr = code()->InstructionAt(instr_index);
    for (size_t i = 0; i < instr->OutputCount(); i++) {
        if (!instr->OutputAt(i)->IsUnallocated())
            continue;
        const UnallocatedOperand* operand = UnallocatedOperand::cast(instr->OutputAt(i));
        if (operand->HasSameAsInputPolicy()) {
            ...
            operand = UnallocatedOperand::cast(instr->InputAt(operand->input_index()));
        }
        if (IsFixedRegisterPolicy(operand)) {
            VirtualRegisterData& vreg_data = VirtualRegisterDataFor(operand->virtual_register());
            AllocatorFor(vreg_data.rep()).ReserveFixedOutputRegister(operand, vreg_data.vreg(), vreg_data.rep(), instr_index);
        }
    }
    for (size_t i = 0; i < instr->TempCount(); i++) {
        if (!instr->TempAt(i)->IsUnallocated())
            continue;
        const UnallocatedOperand* operand = UnallocatedOperand::cast(instr->TempAt(i));
        if (IsFixedRegisterPolicy(operand)) {
            int virtual_register = operand->virtual_register();
            MachineRepresentation rep
                = virtual_register == InstructionOperand::kInvalidVirtualRegister ? InstructionSequence::DefaultRepresentation() : code()->GetRepresentation(virtual_register);
            AllocatorFor(rep).ReserveFixedTempRegister(operand, virtual_register, rep, instr_index);
        }
    }
    for (size_t i = 0; i < instr->InputCount(); i++) {
        if (!instr->InputAt(i)->IsUnallocated())
            continue;
        const UnallocatedOperand* operand = UnallocatedOperand::cast(instr->InputAt(i));
        if (IsFixedRegisterPolicy(operand)) {
            VirtualRegisterData& vreg_data = VirtualRegisterDataFor(operand->virtual_register());
            AllocatorFor(vreg_data.rep()).ReserveFixedInputRegister(operand, vreg_data.vreg(), vreg_data.rep(), instr_index);
        }
    }
}

void MidTierRegisterAllocator::AllocatePhiGapMoves(const InstructionBlock* block)
{
    int successors_phi_index = data_->block_state(block->rpo_number()).successors_phi_index();

    // If successors_phi_index is -1 there are no phi's in the successor.
    if (successors_phi_index == -1)
        return;
    ...
    for (PhiInstruction* phi : successor->phis()) {
        VirtualRegisterData& to_vreg = VirtualRegisterDataFor(phi->virtual_register());
        VirtualRegisterData& from_vreg = VirtualRegisterDataFor(phi->operands()[successors_phi_index]);

        AllocatorFor(to_vreg.rep()).AllocatePhiGapMove(to_vreg, from_vreg, instr_index);
    }
}
...
{
    for (PhiInstruction* phi : block->phis()) {
        VirtualRegisterData& virtual_register = VirtualRegisterDataFor(phi->virtual_register());
...
void MidTierRegisterAllocator::UpdateSpillRangesForLoops()
{
    ...
    for (InstructionBlock* block : code()->instruction_blocks()) {
        if (block->IsLoopHeader()) {
            RpoNumber last_loop_block = RpoNumber::FromInt(block->loop_end().ToInt() - 1);
            int last_loop_instr = data_->GetBlock(last_loop_block)->last_instruction_index();
            ...
            for (int vreg : data_->spilled_virtual_registers()) {
                ...
                if (vreg_data.HasSpillRange() && vreg_data.spill_range()->IsLiveAt(block->first_instruction_index(), block)) {
...
void AllocateRegisters(MidTierRegisterAllocationData* data)
{
    ...
    for (InstructionBlock* block : base::Reversed(data->code()->instruction_blocks())) {
        data->tick_counter()->TickAndMaybeEnterSafepoint();
    ...
    data->frame()->SetAllocatedRegisters(allocator.general_reg_allocator().assigned_registers());
    data->frame()->SetAllocatedDoubleRegisters(allocator.double_reg_allocator().assigned_registers());
}
...
public:
    ...
    MidTierSpillSlotAllocator& operator=(const MidTierSpillSlotAllocator&) = delete;
...
public:
    ...
    AllocatedOperand ToOperand(MachineRepresentation rep) const { return AllocatedOperand(AllocatedOperand::STACK_SLOT, rep, stack_slot_); }
...
bool MidTierSpillSlotAllocator::OrderByLastUse::operator()(const SpillSlot* a, const SpillSlot* b) const { return a->last_use() > b->last_use(); }

MidTierSpillSlotAllocator::MidTierSpillSlotAllocator(MidTierRegisterAllocationData* data)
    : data_(data)
...
void MidTierSpillSlotAllocator::AdvanceTo(int instr_index)
{
    ...
    while (!allocated_slots_.empty() && instr_index > allocated_slots_.top()->last_use()) {
...
MidTierSpillSlotAllocator::SpillSlot* MidTierSpillSlotAllocator::GetFreeSpillSlot(int byte_width)
{
...
void MidTierSpillSlotAllocator::Allocate(VirtualRegisterData* virtual_register)
{
...
void AllocateSpillSlots(MidTierRegisterAllocationData* data)
{
    ...
    std::sort(spilled.begin(), spilled.end(),
        [](const VirtualRegisterData* a, const VirtualRegisterData* b) { return a->spill_range()->live_range().start() < b->spill_range()->live_range().start(); });
...
public:
    ...
    MidTierReferenceMapPopulator& operator=(const MidTierReferenceMapPopulator&) = delete;
...
MidTierReferenceMapPopulator::MidTierReferenceMapPopulator(MidTierRegisterAllocationData* data)
    : data_(data)
{
}

void MidTierReferenceMapPopulator::RecordReferences(const VirtualRegisterData& virtual_register)
{
    if (!virtual_register.HasAllocatedSpillOperand())
        return;
    if (!code()->IsReference(virtual_register.vreg()))
        return;

    VirtualRegisterData::SpillRange* spill_range = virtual_register.spill_range();
    Range& live_range = spill_range->live_range();
    AllocatedOperand allocated = *AllocatedOperand::cast(virtual_register.spill_operand());
