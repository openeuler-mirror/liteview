...
size_t OperandCount(const Instruction* instr) { return instr->InputCount() + instr->OutputCount() + instr->TempCount(); }
...
BlockAssessments* RegisterAllocatorVerifier::CreateForBlock(const InstructionBlock* block)
{
    ...
    BlockAssessments* ret = zone()->New<BlockAssessments>(zone(), spill_slot_delta());
        ...
        for (int instr_index = block->code_start(); instr_index < block->code_end(); ++instr_index) {
            ...
            for (size_t i = 0; i < instr->OutputCount(); ++i, ++count) {
                ...
                if (op_constraints[count].type_ == kRegisterAndSlot) {
                    ...
                    const AllocatedOperand* stack_op = AllocatedOperand::New(zone(), LocationOperand::LocationKind::STACK_SLOT, rep, op_constraints[i].spilled_slot_);
