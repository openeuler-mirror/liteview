...
CodeGenerator::CodeGenerator(Zone* codegen_zone, Frame* frame, Linkage* linkage, InstructionSequence* instructions, OptimizedCompilationInfo* info, Isolate* isolate,
    base::Optional<OsrHelper> osr_helper, int start_source_position, JumpOptimizationInfo* jump_opt, const AssemblerOptions& options, wasm::AssemblerBufferCache* buffer_cache,
    ...
    , info_(info)
    , labels_(codegen_zone->NewArray<Label>(instructions->InstructionBlockCount()))
    , current_block_(RpoNumber::Invalid())
    , start_source_position_(start_source_position)
    , current_source_position_(SourcePosition::Unknown())
    , tasm_(isolate, options, CodeObjectRequired::kNo,
#if V8_ENABLE_WEBASSEMBLY
          buffer_cache ? buffer_cache->GetAssemblerBuffer(AssemblerBase::kDefaultBufferSize) :
#endif // V8_ENABLE_WEBASSEMBLY
...
void CodeGenerator::AddProtectedInstructionLanding(uint32_t instr_offset, uint32_t landing_offset)
{
#if V8_ENABLE_WEBASSEMBLY
...
uint32_t CodeGenerator::GetStackCheckOffset()
{
    ...
    int32_t optimized_frame_height = frame()->GetTotalFrameSlotCount() * kSystemPointerSize;
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleDeoptimizerCall(DeoptimizationExit* exit)
{
    ...
    if (info()->source_positions()) {
        tasm()->RecordDeoptReason(deoptimization_reason, exit->node_id(), exit->pos(), deoptimization_id);
    }
    ...
    tasm()->CallForDeoptimization(target, deoptimization_id, exit->label(), deopt_kind, exit->continue_label(), jump_deoptimization_entry_label);
...
void CodeGenerator::MaybeEmitOutOfLineConstantPool() { tasm()->MaybeEmitOutOfLineConstantPool(); }
...
void CodeGenerator::AssembleCode()
{
    ...
    if (!handlers_.empty()) {
        ...
        for (size_t i = 0; i < handlers_.size(); ++i) {
            HandlerTable::EmitReturnEntry(tasm(), handlers_[i].pc_offset, handlers_[i].handler->pos());
        }
    }
...
base::OwnedVector<byte> CodeGenerator::GetSourcePositionTable() { return source_position_table_builder_.ToSourcePositionTableVector(); }

base::OwnedVector<byte> CodeGenerator::GetProtectedInstructionsData()
{
#if V8_ENABLE_WEBASSEMBLY
    return base::OwnedVector<byte>::Of(base::Vector<byte>::cast(base::VectorOf(protected_instructions_)));
#else
    return {};
#endif // V8_ENABLE_WEBASSEMBLY
}
...
MaybeHandle<Code> CodeGenerator::FinalizeCode()
{
    ...
    MaybeHandle<Code> maybe_code = Factory::CodeBuilder(isolate(), desc, info()->code_kind())
    ...
    LOG_CODE_EVENT(isolate(), CodeLinePosInfoRecordEvent(code->raw_instruction_start(), *source_positions, JitCodeEvent::JIT_CODE));
...
RpoNumber CodeGenerator::ComputeBranchInfo(BranchInfo* branch, Instruction* instr)
{
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleInstruction(int instruction_index, const InstructionBlock* block)
{
    ...
    DCHECK_IMPLIES(block->must_deconstruct_frame(), instr != instructions()->InstructionAt(block->last_instruction_index()) || instr->IsRet() || instr->IsJump());
...
void CodeGenerator::AssembleSourcePosition(SourcePosition source_position)
{
    ...
    if (!source_position.IsKnown())
        return;
    source_position_table_builder_.AddPosition(tasm()->pc_offset(), source_position, false);
    if (v8_flags.code_comments) {
        OptimizedCompilationInfo* info = this->info();
        if (!info->IsOptimizing()) {
#if V8_ENABLE_WEBASSEMBLY
        ...
        if (info->trace_turbo_json() || !tasm()->isolate() || tasm()->isolate()->concurrent_recompilation_enabled()) {
...
Handle<PodArray<InliningPosition>> CreateInliningPositions(OptimizedCompilationInfo* info, Isolate* isolate)
{
    const OptimizedCompilationInfo::InlinedFunctionList& inlined_functions = info->inlined_functions();
    Handle<PodArray<InliningPosition>> inl_positions = PodArray<InliningPosition>::New(isolate, static_cast<int>(inlined_functions.size()), AllocationType::kOld);
...
Handle<DeoptimizationData> CodeGenerator::GenerateDeoptimizationData()
{
    ...
    Handle<TranslationArray> translation_array = translations_.ToTranslationArray(isolate()->factory());
    ...
    Handle<DeoptimizationLiteralArray> literals = isolate()->factory()->NewDeoptimizationLiteralArray(static_cast<int>(deoptimization_literals_.size()));
...
void CodeGenerator::RecordCallPosition(Instruction* instr)
{
    ...
    if (instr->HasCallDescriptorFlag(CallDescriptor::kHasExceptionHandler)) {
        ...
        handlers_.push_back({ GetLabel(handler_rpo), tasm()->pc_offset_for_safepoint() });
    }
...
void CodeGenerator::BuildTranslationForFrameStateDescriptor(FrameStateDescriptor* descriptor, InstructionOperandIterator* iter, OutputFrameStateCombine state_combine)
{
    ...
    const int shared_info_id = DefineDeoptimizationLiteral(DeoptimizationLiteral(shared_info));
    ...
    case FrameStateType::kUnoptimizedFunction: {
        ...
        translations_.BeginInterpretedFrame(bailout_id, shared_info_id, height, return_offset, return_count);
        break;
    }
    ...
    Instruction* instr, int pc_offset, size_t frame_state_offset, size_t immediate_args_count, OutputFrameStateCombine state_combine)
{
    ...
    const int translation_index
        = translations_.BeginTranslation(static_cast<int>(descriptor->GetFrameCount()), static_cast<int>(descriptor->GetJSFrameCount()), update_feedback_count);
    ...
    DeoptimizationExit* const exit = zone()->New<DeoptimizationExit>(current_source_position_, descriptor->bailout_id(), translation_index, pc_offset, entry.kind(), entry.reason(),
#ifdef DEBUG
        entry.node_id());
#else // DEBUG
        0);
    ...
    if (op->IsStackSlot()) {
        ...
        case Constant::kInt64:
            ...
            if (type == MachineType::SignedBigInt64()) {
                ...
                CHECK_EQ(constant.ToInt64(), static_cast<int64_t>(static_cast<double>(constant.ToInt64())));
...
void CodeGenerator::MarkLazyDeoptSite() { last_lazy_deopt_pc_ = tasm()->pc_offset(); }
