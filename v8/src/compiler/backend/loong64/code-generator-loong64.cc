...
#define TRACE_UNIMPL() PrintF("UNIMPLEMENTED code_generator_loong64: %s at line %d\n", __FUNCTION__, __LINE__)
...
public:
    ...
    void Generate() final
    {
        __ CheckPageFlag(value_, MemoryChunk::kPointersToHereAreInterestingOrInSharedHeapMask, eq, exit());
...
void CodeGenerator::BailoutIfDeoptimized()
{
    ...
    __ Jump(BUILTIN_CODE(isolate(), CompileLazyDeoptimizedCode), RelocInfo::CODE_TARGET, ne, scratch, Operand(zero_reg));
}
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleArchInstruction(Instruction* instr)
{
    ...
    case kArchAtomicStoreWithWriteBarrier: {
        ...
        auto ool = zone()->New<OutOfLineRecordWrite>(this, object, offset, value, mode, DetermineStubCallMode());
    ...
    if (required_slots > 0) {
        ...
        if (info()->IsWasm() && required_slots * kSystemPointerSize > 4 * KB) {
            ...
            if (required_slots * kSystemPointerSize < v8_flags.stack_size * KB) {
                UseScratchRegisterScope temps(tasm());
                Register scratch = temps.Acquire();
                __ Ld_d(scratch, FieldMemOperand(kWasmInstanceRegister, WasmInstanceObject::kRealStackLimitAddressOffset));
                __ Ld_d(scratch, MemOperand(scratch, 0));
                __ Add_d(scratch, scratch, Operand(required_slots * kSystemPointerSize));
