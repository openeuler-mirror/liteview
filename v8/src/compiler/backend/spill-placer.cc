...
SpillPlacer::SpillPlacer(LiveRangeFinder* finder, TopTierRegisterAllocationData* data, Zone* zone)
    : finder_(finder)
    , data_(data)
    , zone_(zone)
{
...
void SpillPlacer::Add(TopLevelLiveRange* range)
{
    ...
    for (const LiveRange* child = range; child != nullptr; child = child->next()) {
        if (child->spilled()) {
            ...
            for (UseInterval* interval = child->first_interval(); interval != nullptr; interval = interval->next()) {
...
public:
    ...
    void SetSpillRequired(uint64_t mask) { UpdateValuesToState<kSpillRequired>(mask); }
    uint64_t SpillRequiredInNonDeferredSuccessor() const { return GetValuesInState<kSpillRequiredInNonDeferredSuccessor>(); }
    void SetSpillRequiredInNonDeferredSuccessor(uint64_t mask) { UpdateValuesToState<kSpillRequiredInNonDeferredSuccessor>(mask); }
    uint64_t SpillRequiredInDeferredSuccessor() const { return GetValuesInState<kSpillRequiredInDeferredSuccessor>(); }
    void SetSpillRequiredInDeferredSuccessor(uint64_t mask) { UpdateValuesToState<kSpillRequiredInDeferredSuccessor>(mask); }
...
private:
    ...
    template <bool set_ones> static uint64_t UpdateBitDataWithMask(uint64_t data, uint64_t mask) { return set_ones ? data | mask : data & ~mask; }
...
int SpillPlacer::GetOrCreateIndexForLatestVreg(int vreg)
{
    ...
    if (!IsLatestVreg(vreg)) {
        if (vreg_numbers_ == nullptr) {
            ...
            entries_ = zone_->NewArray<Entry>(data()->code()->instruction_blocks().size());
...
void SpillPlacer::CommitSpill(int vreg, InstructionBlock* predecessor, InstructionBlock* successor)
{
    ...
    data()->AddGapMove(successor->first_instruction_index(), Instruction::GapPosition::START, pred_op, top->GetSpillRangeOperand());
