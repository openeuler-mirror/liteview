...
struct GapJumpRecord {
    ...
    struct RpoNumberHash {
        std::size_t operator()(const RpoNumber& key) const { return std::hash<int>()(key.ToInt()); }
    };
...
void JumpThreading::ApplyForwarding(Zone* local_zone, ZoneVector<RpoNumber> const& result, InstructionSequence* code)
{
    ...
    for (auto const block : code->ao_blocks()) {
        ...
        for (int i = block->code_start(); i < block->code_end(); ++i) {
            ...
            } else if (instr->arch_opcode() == kArchJmp || instr->arch_opcode() == kArchRet) {
                if (skip[block_num]) {
