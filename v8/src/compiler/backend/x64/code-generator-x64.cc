...
public:
    ...
    Immediate InputImmediate(size_t index) { return ToImmediate(instr_->InputAt(index)); }

    Operand InputOperand(size_t index, int extra = 0) { return ToOperand(instr_->InputAt(index), extra); }
    ...
    Operand MemoryOperand(size_t first_input = 0) { return MemoryOperand(&first_input); }
};

namespace {

bool HasAddressingMode(Instruction* instr) { return instr->addressing_mode() != kMode_None; }

bool HasImmediateInput(Instruction* instr, size_t index) { return instr->InputAt(index)->IsImmediate(); }

bool HasRegisterInput(Instruction* instr, size_t index) { return instr->InputAt(index)->IsRegister(); }
...
public:
    OutOfLineRecordWrite(CodeGenerator* gen, Register object, Operand operand, Register value, Register scratch0, Register scratch1, RecordWriteMode mode, StubCallMode stub_mode)
        : OutOfLineCode(gen)
        , object_(object)
        , operand_(operand)
        , value_(value)
    ...
    void Generate() final
    {
        ...
        __ CheckPageFlag(value_, scratch0_, MemoryChunk::kPointersToHereAreInterestingOrInSharedHeapMask, zero, exit());
...
template <std::memory_order order> int EmitStore(TurboAssembler* tasm, Operand operand, Register value, MachineRepresentation rep)
{
...
template <std::memory_order order> int EmitStore(TurboAssembler* tasm, Operand operand, Immediate value, MachineRepresentation rep);

template <> int EmitStore<std::memory_order_relaxed>(TurboAssembler* tasm, Operand operand, Immediate value, MachineRepresentation rep)
{
...
private:
    void GenerateCallToTrap(TrapId trap_id)
    {
        if (!gen_->wasm_runtime_exception_support()) {
            ...
            size_t pop_size = call_descriptor->ParameterSlotCount() * kSystemPointerSize;
...
void EmitOOLTrapIfNeeded(Zone* zone, CodeGenerator* codegen, InstructionCode opcode, Instruction* instr, int pc)
{
...
void EmitOOLTrapIfNeeded(Zone* zone, CodeGenerator* codegen, InstructionCode opcode, Instruction* instr, int pc) { DCHECK_NE(kMemoryAccessProtected, instr->memory_access_mode()); }

#endif // V8_ENABLE_WEBASSEMBLY

#ifdef V8_IS_TSAN
void EmitMemoryProbeForTrapHandlerIfNeeded(TurboAssembler* tasm, Register scratch, Operand operand, StubCallMode mode, int size)
{
...
public:
    ...
    void Generate() final
    {
        ...
        if (stub_mode_ == StubCallMode::kCallWasmRuntimeStub) {
            ...
            // Just encode the stub index. This will be patched when the code
            // is added to the native module and copied into wasm code space.
            tasm()->CallTSANStoreStub(scratch0_, value_, save_fp_mode, size_, StubCallMode::kCallWasmRuntimeStub, memory_order_);
            return;
        }
#endif // V8_ENABLE_WEBASSEMBLY

        tasm()->CallTSANStoreStub(scratch0_, value_, save_fp_mode, size_, StubCallMode::kCallBuiltinPointer, memory_order_);
    }
...
void EmitTSANStoreOOL(
    Zone* zone, CodeGenerator* codegen, TurboAssembler* tasm, Operand operand, Register value_reg, X64OperandConverter& i, StubCallMode mode, int size, std::memory_order order)
{
...
template <std::memory_order order> Register GetTSANValueRegister(TurboAssembler* tasm, Register value, X64OperandConverter& i, MachineRepresentation rep)
{
...
template <std::memory_order order> Register GetTSANValueRegister(TurboAssembler* tasm, Immediate value, X64OperandConverter& i, MachineRepresentation rep);

template <> Register GetTSANValueRegister<std::memory_order_relaxed>(TurboAssembler* tasm, Immediate value, X64OperandConverter& i, MachineRepresentation rep)
{
    Register value_reg = i.TempRegister(1);
    tasm->movq(value_reg, value);
    if (rep == MachineRepresentation::kSandboxedPointer) {
...
void EmitTSANAwareStore(Zone* zone, CodeGenerator* codegen, TurboAssembler* tasm, Operand operand, ValueT value, X64OperandConverter& i, StubCallMode stub_call_mode,
    MachineRepresentation rep, Instruction* instr)
{
    ...
    if (codegen->code_kind() != CodeKind::FOR_TESTING) {
        if (instr->HasMemoryAccessMode()) {
            EmitOOLTrapIfNeeded(zone, codegen, instr->opcode(), instr, tasm->pc_offset());
        }
        int size = ElementSizeInBytes(rep);
        EmitMemoryProbeForTrapHandlerIfNeeded(tasm, i.TempRegister(0), operand, stub_call_mode, size);
        Register value_reg = GetTSANValueRegister<order>(tasm, value, i, rep);
        EmitTSANStoreOOL(zone, codegen, tasm, operand, value_reg, i, stub_call_mode, size, order);
    } else {
        int store_instr_offset = EmitStore<order>(tasm, operand, value, rep);
        if (instr->HasMemoryAccessMode()) {
            EmitOOLTrapIfNeeded(zone, codegen, instr->opcode(), instr, store_instr_offset);
        }
    }
}
...
void EmitTSANRelaxedLoadOOLIfNeeded(Zone* zone, CodeGenerator* codegen, TurboAssembler* tasm, Operand operand, X64OperandConverter& i, StubCallMode mode, int size)
{
    // The FOR_TESTING code doesn't initialize the root register. We can't call
    // the TSAN builtin since we need to load the external reference through the
    // root register.
    ...
    do {                                                                                                                                                                           \
        if (HasAddressingMode(instr)) {                                                                                                                                            \
            ...
            if (HasImmediateInput(instr, 1)) {                                                                                                                                     \
                ...
                if (HasRegisterInput(instr, 0)) {                                                                                                                                  \
                    if (right.value() == 0 && (FlagsConditionField::decode(opcode) == kEqual || FlagsConditionField::decode(opcode) == kNotEqual)) {                               \
...
#define ASSEMBLE_SIMD_SHIFT(opcode, width)                                                                                                                                         \
    do {                                                                                                                                                                           \
        ...
        if (HasImmediateInput(instr, 1)) {                                                                                                                                         \
            if (CpuFeatures::IsSupported(AVX)) {                                                                                                                                   \
                CpuFeatureScope avx_scope(tasm(), AVX);                                                                                                                            \
                __ v##opcode(dst, i.InputSimd128Register(0), byte { i.InputInt##width(1) });                                                                                       \
            } else {                                                                                                                                                               \
                DCHECK_EQ(dst, i.InputSimd128Register(0));                                                                                                                         \
...
#define ASSEMBLE_SEQ_CST_STORE(rep)                                                                                                                                                \
    do {                                                                                                                                                                           \
        ...
        EmitTSANAwareStore<std::memory_order_seq_cst>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), rep, instr);                                               \
    } while (false)
...
void AdjustStackPointerForTailCall(
    Instruction* instr, TurboAssembler* assembler, Linkage* linkage, OptimizedCompilationInfo* info, FrameAccessState* state, int new_slot_above_sp, bool allow_shrinkage = true)
{
...
void SetupSimdImmediateInRegister(TurboAssembler* assembler, uint32_t* imms, XMMRegister reg)
{
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleArchInstruction(Instruction* instr)
{
    ...
    case kArchCallJSFunction: {
        ...
        __ LoadTaggedPointerField(rcx, FieldOperand(func, JSFunction::kCodeOffset));
    ...
    case kArchAbortCSADcheck:
        DCHECK(i.InputRegister(0) == rdx);
        {
            ...
            __ Call(BUILTIN_CODE(isolate(), AbortCSADcheck), RelocInfo::CODE_TARGET);
        }
        __ int3();
        unwinding_info_writer_.MarkBlockWillExit();
        break;
    ...
    case kArchAtomicStoreWithWriteBarrier: {
        ...
        if (arch_opcode == kArchStoreWithWriteBarrier) {
            EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kTagged, instr);
        } else {
            DCHECK_EQ(arch_opcode, kArchAtomicStoreWithWriteBarrier);
            EmitTSANAwareStore<std::memory_order_seq_cst>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kTagged, instr);
        }
    ...
    case kX64Movb: {
        ...
        if (HasImmediateInput(instr, index)) {
            ...
            EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord8, instr);
        } else {
            Register value(i.InputRegister(index));
            EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord8, instr);
        }
    ...
    case kX64Movw: {
        ...
        if (HasImmediateInput(instr, index)) {
            ...
            EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord16, instr);
        } else {
            Register value(i.InputRegister(index));
            EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord16, instr);
        }
    ...
    case kX64Movl:
        if (instr->HasOutput()) {
            ...
            if (HasAddressingMode(instr)) {
                ...
                EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, tasm(), address, i, DetermineStubCallMode(), kInt32Size);
            } else {
                if (HasRegisterInput(instr, 0)) {
            ...
            if (HasImmediateInput(instr, index)) {
                ...
                EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord32, instr);
            } else {
                Register value(i.InputRegister(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord32, instr);
            }
    ...
    case kX64MovqDecompressTaggedSigned: {
        ...
        EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, tasm(), address, i, DetermineStubCallMode(), kTaggedSize);
        break;
    }
    ...
    case kX64MovqDecodeSandboxedPointer: {
        ...
        EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, tasm(), address, i, DetermineStubCallMode(), kSystemPointerSize);
        break;
    }
    case kX64MovqEncodeSandboxedPointer: {
        ...
        Register value(i.InputRegister(index));
        EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kSandboxedPointer, instr);
        break;
    }
    case kX64Movq:
        if (instr->HasOutput()) {
            ...
            EmitTSANRelaxedLoadOOLIfNeeded(zone(), this, tasm(), address, i, DetermineStubCallMode(), kInt64Size);
        } else {
            size_t index = 0;
            Operand operand = i.MemoryOperand(&index);
            if (HasImmediateInput(instr, index)) {
                Immediate value(i.InputImmediate(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord64, instr);
            } else {
                Register value(i.InputRegister(index));
                EmitTSANAwareStore<std::memory_order_relaxed>(zone(), this, tasm(), operand, value, i, DetermineStubCallMode(), MachineRepresentation::kWord64, instr);
            }
    ...
    case kX64I8x16Swizzle: {
        __ I8x16Swizzle(i.OutputSimd128Register(), i.InputSimd128Register(0), i.InputSimd128Register(1), kScratchDoubleReg, kScratchRegister, MiscField::decode(instr->opcode()));
        break;
    }
    ...
    case kX64I32x4TruncF64x2UZero: {
        __ I32x4TruncF64x2UZero(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister, kScratchDoubleReg);
        break;
    }
    case kX64I32x4TruncF32x4U: {
        __ I32x4TruncF32x4U(i.OutputSimd128Register(), i.InputSimd128Register(0), kScratchRegister, kScratchDoubleReg);
        break;
    }
    case kX64Cvttps2dq: {
...
void CodeGenerator::AssembleArchJumpRegardlessOfAssemblyOrder(RpoNumber target) { __ jmp(GetLabel(target)); }
    ...
    if (required_slots > 0) {
        ...
        if (info()->IsWasm() && required_slots * kSystemPointerSize > 4 * KB) {
            ...
            if (required_slots * kSystemPointerSize < v8_flags.stack_size * KB) {
                __ movq(kScratchRegister, FieldOperand(kWasmInstanceRegister, WasmInstanceObject::kRealStackLimitAddressOffset));
            ...
            __ near_call(wasm::WasmCode::kWasmStackOverflow, RelocInfo::WASM_STUB_CALL);
...
void CodeGenerator::SetPendingMove(MoveOperands* move)
{
    ...
    if (move_type == MoveType::kConstantToStack) {
        ...
        if (move->destination().IsStackSlot() && (RelocInfo::IsWasmReference(src.rmode()) || (src.type() != Constant::kInt32 && src.type() != Constant::kInt64))) {
