...
enum class DeoptimizationLiteralKind { kObject, kNumber, kSignedBigInt64, kUnsignedBigInt64, kInvalid };
...
public:
    ...
    void Validate() const { CHECK_NE(kind_, DeoptimizationLiteralKind::kInvalid); }
...
public:
    explicit CodeGenerator(Zone* codegen_zone, Frame* frame, Linkage* linkage, InstructionSequence* instructions, OptimizedCompilationInfo* info, Isolate* isolate,
    ...
    SourcePosition start_source_position() const { return start_source_position_; }
    ...
    const ZoneVector<TurbolizerInstructionStartInfo>& instr_starts() const { return instr_starts_; }

    const TurbolizerCodeOffsetsInfo& offsets_info() const { return offsets_info_; }
