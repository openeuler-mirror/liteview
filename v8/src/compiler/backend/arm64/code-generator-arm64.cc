...
public:
    ...
    DoubleRegister InputFloat32Register(size_t index) { return InputDoubleRegister(index).S(); }

    DoubleRegister InputFloat64Register(size_t index) { return InputDoubleRegister(index); }

    DoubleRegister InputSimd128Register(size_t index) { return InputDoubleRegister(index).Q(); }
    ...
    Register InputRegister32(size_t index) { return ToRegister(instr_->InputAt(index)).W(); }
    ...
    Operand InputOperand(size_t index) { return ToOperand(instr_->InputAt(index)); }

    Operand InputOperand64(size_t index) { return InputOperand(index); }

    Operand InputOperand32(size_t index) { return ToOperand32(instr_->InputAt(index)); }

    Register OutputRegister64() { return OutputRegister(); }

    Register OutputRegister32() { return OutputRegister().W(); }

    Register TempRegister32(size_t index) { return ToRegister(instr_->TempAt(index)).W(); }
...
public:
    OutOfLineRecordWrite(
        CodeGenerator* gen, Register object, Operand offset, Register value, RecordWriteMode mode, StubCallMode stub_mode, UnwindingInfoWriter* unwinding_info_writer)
        : OutOfLineCode(gen)
        , object_(object)
        , offset_(offset)
        , value_(value)
    ...
    void Generate() final
    {
        ...
        __ CheckPageFlag(value_, MemoryChunk::kPointersToHereAreInterestingOrInSharedHeapMask, ne, exit());
...
template <typename Fn> void EmitFpOrNeonUnop(TurboAssembler* tasm, Fn fn, Instruction* instr, Arm64OperandConverter i, VectorFormat scalar, VectorFormat vector)
{
...
#define ASSEMBLE_ATOMIC_COMPARE_EXCHANGE_INTEGER(load_instr, store_instr, ext, reg)                                                                                                \
    do {                                                                                                                                                                           \
...
void CodeGenerator::BailoutIfDeoptimized()
{
    ...
    __ LoadTaggedPointerField(scratch, MemOperand(kJavaScriptCallCodeStartRegister, offset));
    __ Ldr(scratch.W(), FieldMemOperand(scratch, CodeDataContainer::kKindSpecificFlagsOffset));
    Label not_deoptimized;
    __ Tbz(scratch.W(), Code::kMarkedForDeoptimizationBit, &not_deoptimized);
    __ Jump(BUILTIN_CODE(isolate(), CompileLazyDeoptimizedCode), RelocInfo::CODE_TARGET);
...
CodeGenerator::CodeGenResult CodeGenerator::AssembleArchInstruction(Instruction* instr)
{
    ...
    case kArchTruncateDoubleToI:
        __ TruncateDoubleToI(
            isolate(), zone(), i.OutputRegister(), i.InputDoubleRegister(0), DetermineStubCallMode(), frame_access_state()->has_frame() ? kLRHasBeenSaved : kLRHasNotBeenSaved);
    ...
    case kArchStoreWithWriteBarrier: {
        ...
        __ CheckPageFlag(object, MemoryChunk::kPointersFromHereAreInterestingMask, eq, ool->entry());
        __ Bind(ool->exit());
        break;
    }
    ...
    case kArm64And:
        if (FlagsModeField::decode(opcode) != kFlags_none) {
            ...
            DCHECK(FlagsConditionField::decode(opcode) == kEqual || FlagsConditionField::decode(opcode) == kNotEqual || FlagsConditionField::decode(opcode) == kPositiveOrZero
...
void CodeGenerator::AssembleArchSelect(Instruction* instr, FlagsCondition condition)
{
    ...
    if (rep == MachineRepresentation::kFloat32) {
        __ Fcsel(i.OutputFloat32Register(), i.InputFloat32Register(true_value_index), i.InputFloat32Register(false_value_index), cc);
    } else {
        DCHECK_EQ(rep, MachineRepresentation::kFloat64);
        __ Fcsel(i.OutputFloat64Register(), i.InputFloat64Register(true_value_index), i.InputFloat64Register(false_value_index), cc);
    }
}
...
void CodeGenerator::AssembleArchTableSwitch(Instruction* instr)
{
    ...
    __ Br(temp);
    {
        TurboAssembler::BlockPoolsScope block_pools(tasm(), case_count * kInstrSize);
        __ Bind(&table);
...
void CodeGenerator::AssembleConstructFrame()
{
    ...
    if (frame_access_state()->has_frame()) {
        ...
        if (info()->is_osr()) {
            ...
            required_slots -= unoptimized_frame_slots - TurboAssembler::kExtraSlotClaimedByPrologue;
        }
        ...
        case CallDescriptor::kCallWasmCapiFunction: {
            ...
            __ Mov(scratch, StackFrame::TypeToMarker(info()->GetOutputStackFrameType()));
            __ Push(scratch, kWasmInstanceRegister);
            int extra_slots = call_descriptor->kind() == CallDescriptor::kCallWasmImportWrapper ? 0 // Import wrapper: none.
                                                                                                : 1; // C-API function: PC.
...
void CodeGenerator::SetPendingMove(MoveOperands* move)
{
    ...
    if (move_type == MoveType::kStackToStack) {
        ...
        if ((src.IsImmediateOffset() && !tasm()->IsImmLSScaled(src_offset, src_size) && !tasm()->IsImmLSUnscaled(src_offset))
            || (dst.IsImmediateOffset() && !tasm()->IsImmLSScaled(dst_offset, dst_size) && !tasm()->IsImmLSUnscaled(dst_offset))) {
            Register temp = temps.AcquireX();
            move_cycle_.scratch_regs.set(temp);
        }
    }
