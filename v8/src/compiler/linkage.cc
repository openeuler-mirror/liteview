...
inline LinkageLocation regloc(Register reg, MachineType type) { return LinkageLocation::ForRegister(reg.code(), type); }

inline LinkageLocation regloc(DoubleRegister reg, MachineType type) { return LinkageLocation::ForRegister(reg.code(), type); }
...
std::ostream& operator<<(std::ostream& os, const CallDescriptor& d)
{
    ...
    return os << d.kind() << ":" << d.debug_name() << ":r" << d.ReturnCount() << "s" << d.ParameterSlotCount() << "i" << d.InputCount() << "f" << d.FrameStateCount();
}
...
CallDescriptor* Linkage::ComputeIncoming(Zone* zone, OptimizedCompilationInfo* info)
{
    ...
    if (!info->closure().is_null()) {
        ...
        return GetJSCallDescriptor(zone, info->is_osr(), shared.internal_formal_parameter_count_with_receiver(), CallDescriptor::kCanUseRoots);
    }
...
CallDescriptor* Linkage::GetRuntimeCallDescriptor(Zone* zone, Runtime::FunctionId function_id, int js_parameter_count, Operator::Properties properties, CallDescriptor::Flags flags)
{
...
CallDescriptor* Linkage::GetCEntryStubCallDescriptor(
    Zone* zone, int return_count, int js_parameter_count, const char* debug_name, Operator::Properties properties, CallDescriptor::Flags flags, StackArgumentOrder stack_order)
{
...
CallDescriptor* Linkage::GetJSCallDescriptor(Zone* zone, bool is_osr, int js_parameter_count, CallDescriptor::Flags flags)
{
    ...
    LinkageLocation target_loc = is_osr ? LinkageLocation::ForSavedCallerFunction() : regloc(kJSFunctionRegister, MachineType::AnyTagged());
    return zone->New<CallDescriptor>( // --
...
CallDescriptor* Linkage::GetStubCallDescriptor(
    Zone* zone, const CallInterfaceDescriptor& descriptor, int stack_parameter_count, CallDescriptor::Flags flags, Operator::Properties properties, StubCallMode stub_mode)
{
    ...
    // Add returns.
    static constexpr Register return_registers[] = { kReturnRegister0, kReturnRegister1, kReturnRegister2 };
    ...
    for (int i = 0; i < js_parameter_count; i++) {
        if (i < register_parameter_count) {
            ...
            locations.AddParam(LinkageLocation::ForCallerFrameSlot(stack_slot, i < descriptor.GetParameterCount() ? descriptor.GetParameterType(i) : MachineType::AnyTagged()));
        }
    }
    // Add context.
    if (context_count) {
...
LinkageLocation Linkage::GetParameterSecondaryLocation(int index) const
{
    ...
    if (incoming_->IsWasmFunctionCall()) {
        ...
        return LinkageLocation::ForCalleeFrameSlot(kWasmInstanceSlot, MachineType::AnyTagged());
    }
#endif // V8_ENABLE_WEBASSEMBLY
    UNREACHABLE();
}
