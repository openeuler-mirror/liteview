...
struct ScheduleBuilder {
    ...
    const size_t node_count_estimate = static_cast<size_t>(1.1 * input_graph.op_id_count());
    Schedule* const schedule = graph_zone->New<Schedule>(graph_zone, node_count_estimate);
    compiler::Graph* const tf_graph = graph_zone->New<compiler::Graph>(graph_zone);
    compiler::MachineOperatorBuilder machine { graph_zone, MachineType::PointerRepresentation(), InstructionSelector::SupportedMachineOperatorFlags(),
    ...
    Node* MakeNode(const Operator* op, std::initializer_list<Node*> inputs) { return MakeNode(op, base::VectorOf(inputs)); }
    Node* AddNode(const Operator* op, base::Vector<Node* const> inputs);
    Node* AddNode(const Operator* op, std::initializer_list<Node*> inputs) { return AddNode(op, base::VectorOf(inputs)); }
    Node* GetNode(OpIndex i) { return nodes[i.id()]; }
    BasicBlock* GetBlock(const Block& block) { return blocks[block.index().id()]; }
    Node* IntPtrConstant(intptr_t value) { return AddNode(machine.Is64() ? common.Int64Constant(value) : common.Int32Constant(base::checked_cast<int32_t>(value)), {}); }
    Node* IntPtrAdd(Node* a, Node* b) { return AddNode(machine.Is64() ? machine.Int64Add() : machine.Int32Add(), { a, b }); }
    Node* IntPtrShl(Node* a, Node* b) { return AddNode(machine.Is64() ? machine.Word64Shl() : machine.Word32Shl(), { a, b }); }
    Node* RelocatableIntPtrConstant(intptr_t value, RelocInfo::Mode mode)
    {
        return AddNode(machine.Is64() ? common.RelocatableInt64Constant(value, mode) : common.RelocatableInt32Constant(base::checked_cast<int32_t>(value), mode), {});
    }
...
Node* ScheduleBuilder::ProcessOperation(const ChangeOp& op)
{
    ...
    case Kind::kSignedToFloat:
        ...
        } else if (op.from == WordRepresentation::Word32() && op.to == FloatRepresentation::Float64()) {
            o = machine.ChangeInt32ToFloat64();
        } else {
            UNIMPLEMENTED();
        }
...
Node* ScheduleBuilder::ProcessOperation(const Float64InsertWord32Op& op)
{
    ...
    case Float64InsertWord32Op::Kind::kHighHalf:
        return AddNode(machine.Float64InsertHighWord32(), { GetNode(op.float64()), GetNode(op.word32()) });
    case Float64InsertWord32Op::Kind::kLowHalf:
        return AddNode(machine.Float64InsertLowWord32(), { GetNode(op.float64()), GetNode(op.word32()) });
    }
}
Node* ScheduleBuilder::ProcessOperation(const TaggedBitcastOp& op)
{
    const Operator* o;
    if (op.from == RegisterRepresentation::Tagged() && op.to == RegisterRepresentation::PointerSized()) {
        o = machine.BitcastTaggedToWord();
    } else if (op.from == RegisterRepresentation::PointerSized() && op.to == RegisterRepresentation::Tagged()) {
...
{
    const Operator* o = op.rep == WordRepresentation::Word32() ? machine.Word32Select().op() : machine.Word64Select().op();
    return AddNode(o, { GetNode(op.condition()), GetNode(op.left()), GetNode(op.right()) });
}

Node* ScheduleBuilder::ProcessOperation(const PendingLoopPhiOp& op) { UNREACHABLE(); }
...
Node* ScheduleBuilder::ProcessOperation(const LoadOp& op)
{
    ...
    return AddNode(op.kind == LoadOp::Kind::kRawAligned ? machine.Load(op.loaded_rep.ToMachineType())
            : op.kind == LoadOp::Kind::kRawUnaligned    ? machine.UnalignedLoad(op.loaded_rep.ToMachineType())
...
Node* ScheduleBuilder::ProcessOperation(const IndexedLoadOp& op)
{
    ...
    return AddNode(op.kind == LoadOp::Kind::kRawAligned ? machine.Load(loaded_rep)
            : op.kind == LoadOp::Kind::kRawUnaligned    ? machine.UnalignedLoad(loaded_rep)
                                                        : machine.ProtectedLoad(loaded_rep),
        { base, index });
}
Node* ScheduleBuilder::ProcessOperation(const ProtectedLoadOp& op)
{
    return AddNode(machine.ProtectedLoad(op.loaded_rep.ToMachineType()), { GetNode(op.base()), GetNode(op.index()) });
}
...
{
    return AddNode(machine.ProtectedStore(op.stored_rep.ToMachineType().representation()), { GetNode(op.base()), GetNode(op.index()), GetNode(op.value()) });
}
Node* ScheduleBuilder::ProcessOperation(const RetainOp& op) { return AddNode(common.Retain(), { GetNode(op.retained()) }); }
...
Node* ScheduleBuilder::ProcessOperation(const StackPointerGreaterThanOp& op) { return AddNode(machine.StackPointerGreaterThan(op.kind), { GetNode(op.stack_limit()) }); }
Node* ScheduleBuilder::ProcessOperation(const StackSlotOp& op) { return AddNode(machine.StackSlot(op.size, op.alignment), {}); }
...
Node* ScheduleBuilder::ProcessOperation(const DeoptimizeIfOp& op)
{
    ...
    Node* frame_state = GetNode(op.frame_state());
    const Operator* o
        = op.negated ? common.DeoptimizeUnless(op.parameters->reason(), op.parameters->feedback()) : common.DeoptimizeIf(op.parameters->reason(), op.parameters->feedback());
    return AddNode(o, { condition, frame_state });
}
Node* ScheduleBuilder::ProcessOperation(const TrapIfOp& op)
{
    Node* condition = GetNode(op.condition());
    const Operator* o = op.negated ? common.TrapUnless(op.trap_id) : common.TrapIf(op.trap_id);
...
Node* ScheduleBuilder::ProcessOperation(const ProjectionOp& op) { return AddNode(common.Projection(op.index), { GetNode(op.input()) }); }
...
Node* ScheduleBuilder::BuildStateValues(FrameStateData::Iterator* it, int32_t size)
{
    ...
    return AddNode(common.TypedStateValues(graph_zone->New<ZoneVector<MachineType>>(types.begin(), types.end(), graph_zone), SparseInputMask(input_mask)), base::VectorOf(inputs));
}
...
{
    Node* branch = MakeNode(common.Branch(BranchHint::kNone), { GetNode(op.condition()) });
...
RecreateScheduleResult RecreateSchedule(
    const Graph& graph, CallDescriptor* call_descriptor, Zone* graph_zone, Zone* phase_zone, SourcePositionTable* source_positions, NodeOriginTable* origins)
{
    ScheduleBuilder builder { graph, call_descriptor, graph_zone, phase_zone, source_positions, origins };
    return builder.Run();
}
