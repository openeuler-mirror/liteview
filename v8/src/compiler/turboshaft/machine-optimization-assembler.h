...
class MachineOptimizationAssembler : public AssemblerInterface<MachineOptimizationAssembler<Base, signalling_nan_possible>, Base> {
public:
    template <class T> bool Is(OpIndex op) { return Base::template Is<T>(op); }
    template <class T> const T& Cast(OpIndex op) { return Base::template Cast<T>(op); }
    template <class T> const T* TryCast(OpIndex op) { return Base::template TryCast<T>(op); }
    using Base::Get;
    using Base::graph;

    MachineOptimizationAssembler(Graph* graph, Zone* phase_zone)
        : AssemblerInterface<MachineOptimizationAssembler, Base>(graph, phase_zone)
    {
    }

    OpIndex Change(OpIndex input, ChangeOp::Kind kind, ChangeOp::Assumption assumption, RegisterRepresentation from, RegisterRepresentation to)
    {
        ...
        if (uint64_t value; from.IsWord() && this->MatchWordConstant(input, WordRepresentation(from), &value)) {
            if (kind == Kind::kSignExtend && from == WordRepresentation::Word32() && to == WordRepresentation::Word64()) {
                return this->Word64Constant(int64_t { static_cast<int32_t>(value) });
            }
            if (kind == any_of(Kind::kZeroExtend, Kind::kBitcast) && from == WordRepresentation::Word32() && to == WordRepresentation::Word64()) {
                return this->Word64Constant(uint64_t { static_cast<uint32_t>(value) });
            }
            if (kind == Kind::kBitcast && from == WordRepresentation::Word32() && to == RegisterRepresentation::Float32()) {
                return this->Float32Constant(base::bit_cast<float>(static_cast<uint32_t>(value)));
            }
            if (kind == Kind::kBitcast && from == WordRepresentation::Word64() && to == RegisterRepresentation::Float64()) {
                return this->Float64Constant(base::bit_cast<double>(value));
            }
            if (kind == Kind::kSignedToFloat && from == WordRepresentation::Word32() && to == RegisterRepresentation::Float64()) {
                return this->Float64Constant(static_cast<double>(static_cast<int32_t>(value)));
            }
            if (kind == Kind::kSignedToFloat && from == WordRepresentation::Word64() && to == RegisterRepresentation::Float64()) {
                return this->Float64Constant(static_cast<double>(static_cast<int64_t>(value)));
            }
            if (kind == Kind::kUnsignedToFloat && from == WordRepresentation::Word32() && to == RegisterRepresentation::Float64()) {
                return this->Float64Constant(static_cast<double>(static_cast<uint32_t>(value)));
            }
        }
        if (float value; from == RegisterRepresentation::Float32() && this->MatchFloat32Constant(input, &value)) {
        ...
        if (double value; from == RegisterRepresentation::Float64() && this->MatchFloat64Constant(input, &value)) {
    ...
    OpIndex Float64InsertWord32(OpIndex float64, OpIndex word32, Float64InsertWord32Op::Kind kind)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        double f;
        uint32_t w;
        if (this->MatchFloat64Constant(float64, &f) && this->MatchWord32Constant(word32, &w)) {
            uint64_t float_as_word = base::bit_cast<uint64_t>(f);
            switch (kind) {
            case Float64InsertWord32Op::Kind::kLowHalf:
                return this->Float64Constant(base::bit_cast<double>((float_as_word& uint64_t { 0xFFFFFFFF00000000 }) | w));
            case Float64InsertWord32Op::Kind::kHighHalf:
                return this->Float64Constant(base::bit_cast<double>((float_as_word& uint64_t { 0xFFFFFFFF }) | (uint64_t { w } << 32)));
            }
        }
        return Base::Float64InsertWord32(float64, word32, kind);
    }

    OpIndex TaggedBitcast(OpIndex input, RegisterRepresentation from, RegisterRepresentation to)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        if (auto* input_bitcast = TryCast<TaggedBitcastOp>(input)) {
            if (all_of(input_bitcast->to, from) == RegisterRepresentation::PointerSized() && all_of(input_bitcast->from, to) == RegisterRepresentation::Tagged()) {
    ...
    OpIndex FloatUnary(OpIndex input, FloatUnaryOp::Kind kind, FloatRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
            return Base::FloatUnary(input, kind, rep);
        }
        if (float k; rep == FloatRepresentation::Float32() && this->MatchFloat32Constant(input, &k)) {
            if (std::isnan(k)) {
        ...
        } else if (double k; rep == FloatRepresentation::Float64() && this->MatchFloat64Constant(input, &k)) {
            if (std::isnan(k)) {
    ...
    OpIndex WordUnary(OpIndex input, WordUnaryOp::Kind kind, WordRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        if (uint32_t k; rep == WordRepresentation::Word32() && this->MatchWord32Constant(input, &k)) {
            switch (kind) {
        ...
        } else if (uint64_t k; rep == WordRepresentation::Word64() && this->MatchWord64Constant(input, &k)) {
            switch (kind) {
            case WordUnaryOp::Kind::kReverseBytes:
                return this->Word64Constant(base::bits::ReverseBytes(k));
            case WordUnaryOp::Kind::kCountLeadingZeros:
                return this->Word64Constant(uint64_t { base::bits::CountLeadingZeros(k) });
            case WordUnaryOp::Kind::kCountTrailingZeros:
                return this->Word64Constant(uint64_t { base::bits::CountTrailingZeros(k) });
            case WordUnaryOp::Kind::kPopCount:
    ...
    OpIndex FloatBinop(OpIndex lhs, OpIndex rhs, FloatBinopOp::Kind kind, FloatRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        if (FloatBinopOp::IsCommutative(kind) && Is<ConstantOp>(lhs) && !Is<ConstantOp>(rhs)) {
            return FloatBinop(rhs, lhs, kind, rep);
        }

        // constant folding
        if (float k1, k2; rep == FloatRepresentation::Float32() && this->MatchFloat32Constant(lhs, &k1) && this->MatchFloat32Constant(rhs, &k2)) {
            switch (kind) {
            case Kind::kAdd:
        ...
        if (double k1, k2; rep == FloatRepresentation::Float64() && this->MatchFloat64Constant(lhs, &k1) && this->MatchFloat64Constant(rhs, &k2)) {
            switch (kind) {
            case Kind::kAdd:
        ...
        if (Is<ConstantOp>(rhs)) {
            ...
            if (kind == Kind::kDiv) {
                ...
                if (rep == FloatRepresentation::Float32()) {
                    if (float k; this->MatchFloat32Constant(rhs, &k) && std::isnormal(k) && k != 0 && std::isfinite(k) && base::bits::IsPowerOfTwo(base::Double(k).Significand())) {
                        return this->FloatMul(lhs, this->FloatConstant(1.0 / k, rep), rep);
                    }
                } else {
                    DCHECK_EQ(rep, FloatRepresentation::Float64());
                    if (double k;
                        this->MatchFloat64Constant(rhs, &k) && std::isnormal(k) && k != 0 && std::isfinite(k) && base::bits::IsPowerOfTwo(base::Double(k).Significand())) {
                        return this->FloatMul(lhs, this->FloatConstant(1.0 / k, rep), rep);
                    }
                }
            }
            ...
            if (kind == Kind::kPower) {
                ...
                if (this->MatchFloat(rhs, 0.5)) {
                    ...
                    this->Branch(this->FloatLessThanOrEqual(lhs, this->FloatConstant(-V8_INFINITY, rep), rep), if_neg_infinity, otherwise);
        ...
        if (!signalling_nan_possible && kind == Kind::kSub && this->MatchFloat(lhs, -0.0)) {
            // -0.0 - round_down(-0.0 - y) => round_up(y)
            if (OpIndex a, b, c; FloatUnaryOp::IsSupported(FloatUnaryOp::Kind::kRoundUp, rep) && this->MatchFloatRoundDown(rhs, &a, rep) && this->MatchFloatSub(a, &b, &c, rep)
                && this->MatchFloat(b, -0.0)) {
    ...
    OpIndex WordBinop(OpIndex left, OpIndex right, WordBinopOp::Kind kind, WordRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
        ...
        // Place constant on the right for commutative operators.
        if (WordBinopOp::IsCommutative(kind) && Is<ConstantOp>(left) && !Is<ConstantOp>(right)) {
            return WordBinop(right, left, kind, rep);
        }
        // constant folding
        if (uint64_t k1, k2; this->MatchWordConstant(left, rep, &k1) && this->MatchWordConstant(right, rep, &k2)) {
            switch (kind) {
            case Kind::kAdd:
            ...
            case Kind::kSignedMulOverflownBits:
                return this->WordConstant(is_64 ? base::bits::SignedMulHigh64(static_cast<int64_t>(k1), static_cast<int64_t>(k2))
                                                : base::bits::SignedMulHigh32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)),
                    rep);
            case Kind::kUnsignedMulOverflownBits:
                return this->WordConstant(is_64 ? base::bits::UnsignedMulHigh64(k1, k2) : base::bits::UnsignedMulHigh32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            case Kind::kSignedDiv:
                return this->WordConstant(is_64 ? base::bits::SignedDiv64(k1, k2) : base::bits::SignedDiv32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)), rep);
            case Kind::kUnsignedDiv:
                return this->WordConstant(is_64 ? base::bits::UnsignedDiv64(k1, k2) : base::bits::UnsignedDiv32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            case Kind::kSignedMod:
                return this->WordConstant(is_64 ? base::bits::SignedMod64(k1, k2) : base::bits::SignedMod32(static_cast<int32_t>(k1), static_cast<int32_t>(k2)), rep);
            case Kind::kUnsignedMod:
                return this->WordConstant(is_64 ? base::bits::UnsignedMod64(k1, k2) : base::bits::UnsignedMod32(static_cast<uint32_t>(k1), static_cast<uint32_t>(k2)), rep);
            }
        }
        ...
        if (uint64_t right_value; this->MatchWordConstant(right, rep, &right_value)) {
            ...
            if (OpIndex a, k1; WordBinopOp::IsAssociative(kind) && this->MatchWordBinop(left, &a, &k1, kind, rep) && Is<ConstantOp>(k1)) {
                OpIndex k2 = right;
            ...
            case Kind::kSub:
                // left - k  => left + -k
                return WordBinop(left, this->WordConstant(-right_value, rep), Kind::kAdd, rep);
            case Kind::kAdd:
                // left + 0  =>  left
                if (right_value == 0) {
                    return left;
            ...
            case Kind::kBitwiseXor:
                ...
                // (x ^ -1) ^ -1  =>  x
                {
                    OpIndex x, y;
                    int64_t k;
                    if (right_value_signed == -1 && this->MatchBitwiseAnd(left, &x, &y, rep) && this->MatchWordConstant(y, rep, &k) && k == -1) {
                        return x;
                    }
                }
                break;
            case Kind::kBitwiseOr:
                ...
                // This case can be constructed by UpdateWord and UpdateWord32 in CSA.
                {
                    ...
                    if (this->MatchBitwiseAnd(left, &x, &y, rep) && this->MatchWordConstant(y, rep, &k1) && (k1 | k2) == rep.MaxUnsignedValue()) {
            ...
            case Kind::kMul:
                ...
                if (base::bits::IsPowerOfTwo(right_value)) {
                    OpIndex shift_amount = this->WordConstant(base::bits::WhichPowerOfTwo(right_value), rep);
            ...
            case Kind::kBitwiseAnd:
                ...
                if (right_value == 1) {
                    // (x + x) & 1  =>  0
                    OpIndex left_ignore_extensions = IsWord32ConvertedToWord64(left) ? UndoWord32ToWord64Conversion(left) : left;
                    if (OpIndex a, b; this->MatchWordAdd(left_ignore_extensions, &a, &b, WordRepresentation::Word32()) && a == b) {
                        return this->WordConstant(0, rep);
                    }
            ...
            case WordBinopOp::Kind::kSignedMod:
                ...
                if (base::bits::IsPowerOfTwo(right_value_signed)) {
                    ...
                    OpIndex m = this->ShiftRightLogical(this->ShiftRightArithmetic(left, bits - 1, rep), bits - n, rep);
                    return this->WordSub(this->WordBitwiseAnd(this->WordAdd(left, m, rep), this->WordConstant(right_value_signed - 1, rep), rep), m, rep);
                }
                // The `IntDiv` with a constant right-hand side will be turned into a
                // multiplication, avoiding the expensive integer division.
                return this->WordSub(left, this->WordMul(this->IntDiv(left, right, rep), right, rep), rep);
            case WordBinopOp::Kind::kUnsignedMod:
                ...
                // x % 2^n => x & (2^n - 1)
                if (base::bits::IsPowerOfTwo(right_value)) {
                    return this->WordBitwiseAnd(left, this->WordConstant(right_value - 1, rep), rep);
                }
                // The `UintDiv` with a constant right-hand side will be turned into a
                // multiplication, avoiding the expensive integer division.
                return this->WordSub(left, this->WordMul(right, this->UintDiv(left, right, rep), rep), rep);
        ...
        // 0 / right  =>  0
        // 0 % right  =>  0
        if (this->MatchZero(left) && kind == any_of(Kind::kSignedDiv, Kind::kUnsignedDiv, Kind::kUnsignedMod, Kind::kSignedMod)) {
            return this->WordConstant(0, rep);
        }
    ...
    base::Optional<OpIndex> TryReduceToRor(OpIndex left, OpIndex right, WordBinopOp::Kind kind, WordRepresentation rep)
    {
        ...
        if (!(kind == any_of(WordBinopOp::Kind::kBitwiseAnd, WordBinopOp::Kind::kBitwiseXor))) {
            return {};
        }
        ...
        OpIndex amount;
        uint64_t k;
        if (OpIndex a, b; this->MatchWordSub(high->right(), &a, &b, rep) && this->MatchWordConstant(a, rep, &k) && b == low->right() && k == rep.bit_width()) {
            amount = b;
        } else if (OpIndex a, b; this->MatchWordSub(low->right(), &a, &b, rep) && a == high->right() && this->MatchWordConstant(b, rep, &k) && k == rep.bit_width()) {
            amount = low->right();
        } else if (uint64_t k1, k2;
                   this->MatchWordConstant(high->right(), rep, &k1) && this->MatchWordConstant(low->right(), rep, &k2) && k1 + k2 == rep.bit_width() && k1 >= 0 && k2 >= 0) {
            if (k1 == 0 || k2 == 0) {
    ...
    OpIndex OverflowCheckedBinop(OpIndex left, OpIndex right, OverflowCheckedBinopOp::Kind kind, WordRepresentation rep)
    {
        if (ShouldSkipOptimizationStep()) {
            return Base::OverflowCheckedBinop(left, right, kind, rep);
        }
        using Kind = OverflowCheckedBinopOp::Kind;
        if (OverflowCheckedBinopOp::IsCommutative(kind) && Is<ConstantOp>(left) && !Is<ConstantOp>(right)) {
        ...
        // constant folding
        if (rep == WordRepresentation::Word32()) {
            if (int32_t k1, k2; this->MatchWord32Constant(left, &k1) && this->MatchWord32Constant(right, &k2)) {
                bool overflow;
                int32_t res;
                switch (kind) {
                ...
                return this->Tuple(this->Word32Constant(res), this->Word32Constant(overflow));
            }
        } else {
            DCHECK_EQ(rep, WordRepresentation::Word64());
            if (int64_t k1, k2; this->MatchWord64Constant(left, &k1) && this->MatchWord64Constant(right, &k2)) {
                bool overflow;
                int64_t res;
                switch (kind) {
                case OverflowCheckedBinopOp::Kind::kSignedAdd:
                ...
                return this->Tuple(this->Word64Constant(res), this->Word32Constant(overflow));
            }
        }

        // left + 0  =>  (left, false)
        // left - 0  =>  (left, false)
        if (kind == any_of(Kind::kSignedAdd, Kind::kSignedSub) && this->MatchZero(right)) {
        ...
        if (kind == Kind::kSignedMul) {
            if (int64_t k; this->MatchWordConstant(right, rep, &k)) {
                // left * 0  =>  (0, false)
                if (k == 0) {
                    return this->Tuple(this->WordConstant(0, rep), this->Word32Constant(false));
                }
                // left * 1  =>  (left, false)
                if (k == 1) {
                ...
                // left * -1  =>  0 - left
                if (k == -1) {
                    return this->IntSubCheckOverflow(this->WordConstant(0, rep), left, rep);
                }
                // left * 2  =>  left + left
                if (k == 2) {
        ...
        if (Is<ConstantOp>(right)) {
            if (Is<ConstantOp>(left)) {
                ...
                case RegisterRepresentation::Word64(): {
                    if (uint64_t k1, k2; this->MatchWordConstant(left, WordRepresentation(rep), &k1) && this->MatchWordConstant(right, WordRepresentation(rep), &k2)) {
                        return this->Word32Constant(k1 == k2);
                    }
                    break;
                }
                case RegisterRepresentation::Float32(): {
                    if (float k1, k2; this->MatchFloat32Constant(left, &k1) && this->MatchFloat32Constant(right, &k2)) {
                        return this->Word32Constant(k1 == k2);
                    }
                    break;
                }
                case RegisterRepresentation::Float64(): {
                    if (double k1, k2; this->MatchFloat64Constant(left, &k1) && this->MatchFloat64Constant(right, &k2)) {
                        return this->Word32Constant(k1 == k2);
                    }
                    break;
                }
            ...
            if (rep.IsWord()) {
                ...
                // x - y == 0  =>  x == y
                if (OpIndex x, y; this->MatchWordSub(left, &x, &y, rep_w) && this->MatchZero(right)) {
                ...
                }
                {
                    ...
                    int shift_amount;
                    uint64_t mask, k;
                    if (this->MatchBitwiseAnd(left, &shift, &mask_op, rep_w) && this->MatchConstantRightShift(shift, &x, rep_w, &shift_amount)
                        && this->MatchWordConstant(mask_op, rep_w, &mask) && this->MatchWordConstant(right, rep_w, &k) && mask <= rep.MaxUnsignedValue() >> shift_amount
                        && k <= rep.MaxUnsignedValue() >> shift_amount) {
                        return Equal(this->WordBitwiseAnd(x, this->Word64Constant(mask << shift_amount), rep_w), this->Word64Constant(k << shift_amount), rep_w);
                    }
                }
                {
                    ...
                    uint16_t k1;
                    int64_t k2;
                    if (this->MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1) && this->MatchWordConstant(right, rep_w, &k2)
                        && CountLeadingSignBits(k2, rep_w) > k1 && Get(left).saturated_use_count == 0) {
                        return this->Equal(x, this->WordConstant(base::bits::Unsigned(k2) << k1, rep_w), rep_w);
                    }
                }
            }
        }
    ...
    OpIndex Comparison(OpIndex left, OpIndex right, ComparisonOp::Kind kind, RegisterRepresentation rep)
    {
        ...
        if (Is<ConstantOp>(right) && Is<ConstantOp>(left)) {
            ...
            case RegisterRepresentation::Word64(): {
                if (kind == any_of(Kind::kSignedLessThan, Kind::kSignedLessThanOrEqual)) {
                    if (int64_t k1, k2; this->MatchWordConstant(left, WordRepresentation(rep), &k1) && this->MatchWordConstant(right, WordRepresentation(rep), &k2)) {
                        switch (kind) {
        ...
        if (rep == RegisterRepresentation::Float64() && IsFloat32ConvertedToFloat64(left) && IsFloat32ConvertedToFloat64(right)) {
            return this->Comparison(UndoFloat32ToFloat64Conversion(left), UndoFloat32ToFloat64Conversion(right), kind, RegisterRepresentation::Float32());
        }
        if (rep.IsWord()) {
            WordRepresentation rep_w { rep };
            if (kind == Kind::kUnsignedLessThanOrEqual) {
                ...
                // x <= MaxUint  =>  true
                if (uint64_t k; this->MatchWordConstant(right, rep_w, &k) && k == rep.MaxUnsignedValue()) {
                    return this->Word32Constant(1);
                }
            }
            if (kind == Kind::kUnsignedLessThan) {
                ...
                // MaxUint < x  =>  true
                if (uint64_t k; this->MatchWordConstant(left, rep_w, &k) && k == rep.MaxUnsignedValue()) {
                    return this->Word32Constant(0);
                }
            }
            {
                ...
                OpIndex x, y;
                uint16_t k1, k2;
                if (this->MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1) && this->MatchConstantShiftRightArithmeticShiftOutZeros(right, &y, rep_w, &k2)
                    && k1 == k2) {
            ...
            }
            {
                ...
                uint16_t k1;
                int64_t k2;
                if (this->MatchConstantShiftRightArithmeticShiftOutZeros(left, &x, rep_w, &k1) && this->MatchWordConstant(right, rep_w, &k2) && CountLeadingSignBits(k2, rep_w) > k1
                    && Get(left).saturated_use_count == 0) {
                    return this->Comparison(x, this->WordConstant(base::bits::Unsigned(k2) << k1, rep_w), kind, rep_w);
                }
                // k2 </<= (x >> k1)  =>  (k2 << k1) </<= x  if shifts reversible
                // Only perform the transformation if the shift is not used yet, to
                // avoid keeping both the shift and x alive.
                if (this->MatchConstantShiftRightArithmeticShiftOutZeros(right, &x, rep_w, &k1) && this->MatchWordConstant(left, rep_w, &k2) && CountLeadingSignBits(k2, rep_w) > k1
                    && Get(right).saturated_use_count == 0) {
                    return this->Comparison(this->WordConstant(base::bits::Unsigned(k2) << k1, rep_w), x, kind, rep_w);
                }
            }
                ...
                if (IsWord32ConvertedToWord64(left, &left_sign_extended) && IsWord32ConvertedToWord64(right, &right_sign_extended)) {
                    if (left_sign_extended != true && right_sign_extended != true) {
                        ...
                        return this->Comparison(
                            UndoWord32ToWord64Conversion(left), UndoWord32ToWord64Conversion(right), ComparisonOp::SetSigned(kind, false), WordRepresentation::Word32());
                        ...
                        return this->Comparison(UndoWord32ToWord64Conversion(left), UndoWord32ToWord64Conversion(right), kind, WordRepresentation::Word32());
                    }
                }
            }
        }
        ...
        if (this->MatchWordConstant(left, rep, &c_unsigned, &c_signed)) {
            if (uint32_t amount; this->MatchWord32Constant(right, &amount)) {
                ...
                case Kind::kRotateRight:
                    if (rep == WordRepresentation::Word32()) {
                        return this->WordConstant(base::bits::RotateRight32(static_cast<uint32_t>(c_unsigned), amount), rep);
                    } else {
                        return this->WordConstant(base::bits::RotateRight64(c_unsigned, amount), rep);
                    }
                case Kind::kRotateLeft:
                    if (rep == WordRepresentation::Word32()) {
                        return this->WordConstant(base::bits::RotateLeft32(static_cast<uint32_t>(c_unsigned), amount), rep);
                    } else {
                        return this->WordConstant(base::bits::RotateLeft64(c_unsigned, amount), rep);
                    }
                }
            }
        }
        if (int32_t amount; this->MatchWord32Constant(right, &amount) && 0 <= amount && amount < rep.bit_width()) {
            if (amount == 0) {
                return left;
            }
            if (kind == Kind::kShiftLeft) {
                ...
                int k;
                OpIndex x;
                if (this->MatchConstantShift(left, &x, Kind::kShiftRightArithmeticShiftOutZeros, rep, &k)) {
                    int32_t l = amount;
                    if (k == l) {
                        return x;
                    } else if (k > l) {
                        return this->ShiftRightArithmeticShiftOutZeros(x, this->Word32Constant(k - l), rep);
                    } else if (k < l) {
                ...
                if (this->MatchConstantRightShift(left, &x, rep, &k) && k == amount) {
                    return this->WordBitwiseAnd(x, this->WordConstant(rep.MaxUnsignedValue() << k, rep), rep);
                }
            }
            if (kind == any_of(Kind::kShiftRightArithmetic, Kind::kShiftRightArithmeticShiftOutZeros)) {
                ...
                // (x << k) >> k
                if (this->MatchConstantShift(left, &x, ShiftOp::Kind::kShiftLeft, rep, &left_shift_amount) && amount == left_shift_amount) {
            ...
            if (rep == WordRepresentation::Word32() && SupportedOperations::word32_shift_is_safe()) {
                ...
                if (OpIndex a, b; this->MatchBitwiseAnd(right, &a, &b, WordRepresentation::Word32())) {
                    static_assert(0x1f == WordRepresentation::Word32().bit_width() - 1);
                    if (uint32_t b_value; this->MatchWord32Constant(b, &b_value) && b_value == 0x1f) {
    ...
    OpIndex DeoptimizeIf(OpIndex condition, OpIndex frame_state, bool negated, const DeoptimizeParameters* parameters)
    {
        if (ShouldSkipOptimizationStep()) {
    ...
    OpIndex Store(OpIndex base, OpIndex value, StoreOp::Kind kind, MemoryRepresentation stored_rep, WriteBarrierKind write_barrier, int32_t offset)
    {
        if (ShouldSkipOptimizationStep()) {
            return Base::Store(base, value, kind, stored_rep, write_barrier, offset);
        }
        return IndexedStore(base, OpIndex::Invalid(), value, kind, stored_rep, write_barrier, offset, 0);
    }

    OpIndex IndexedStore(OpIndex base, OpIndex index, OpIndex value, IndexedStoreOp::Kind kind, MemoryRepresentation stored_rep, WriteBarrierKind write_barrier, int32_t offset,
        uint8_t element_scale)
    {
        ...
        if (index.valid()) {
            return Base::IndexedStore(base, index, value, kind, stored_rep, write_barrier, offset, element_scale);
        } else {
            return Base::Store(base, value, kind, stored_rep, write_barrier, offset);
        }
    }

    OpIndex Load(OpIndex base, LoadOp::Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep, int32_t offset)
    {
        if (ShouldSkipOptimizationStep())
            return Base::Load(base, kind, loaded_rep, result_rep, offset);
        return IndexedLoad(base, OpIndex::Invalid(), kind, loaded_rep, result_rep, offset, 0);
    }

    OpIndex IndexedLoad(
        OpIndex base, OpIndex index, IndexedLoadOp::Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep, int32_t offset, uint8_t element_scale)
    {
        while (true) {
            if (ShouldSkipOptimizationStep())
                break;
            ...
            if (kind != IndexedLoadOp::Kind::kTaggedBase && !index.valid()) {
                if (OpIndex left, right; this->MatchWordAdd(base, &left, &right, WordRepresentation::PointerSized()) && TryAdjustOffset(&offset, Get(right), element_scale)) {
                    base = left;
                    continue;
                }
            }
        ...
        if (index.valid()) {
            return Base::IndexedLoad(base, index, kind, loaded_rep, result_rep, offset, element_scale);
        } else {
            return Base::Load(base, kind, loaded_rep, result_rep, offset);
        }
    }
...
private:
    bool TryAdjustOffset(int32_t* offset, const Operation& maybe_constant, uint8_t element_scale)
    {
    ...
    {
        uint64_t diff;
        if (!this->MatchWordConstant(maybe_constant, WordRepresentation::PointerSized(), &diff)) {
            return false;
        }
        DCHECK_LT(*element_scale, WordRepresentation::PointerSized().bit_width());
        if (diff < (WordRepresentation::PointerSized().bit_width() - uint64_t { *element_scale })) {
            *element_scale += diff;
            return true;
        }
        return false;
    ...
    OpIndex ReduceMemoryIndex(OpIndex index, int32_t* offset, uint8_t* element_scale)
    {
        while (index.valid()) {
            ...
            if (TryAdjustOffset(offset, index_op, *element_scale)) {
                ...
                *element_scale = 0;
                ...
                if (binary_op->kind == WordBinopOp::Kind::kAdd && TryAdjustOffset(offset, Get(binary_op->right()), *element_scale)) {
    ...
    {
        if (OpIndex input; this->MatchChange(value, &input, ChangeOp::Kind::kFloatConversion, RegisterRepresentation::Float32(), RegisterRepresentation::Float64())) {
            return true;
        }
        if (double c; this->MatchFloat64Constant(value, &c) && DoubleToFloat32(c) == c) {
            return true;
        }
        return false;
    }
    ...
    bool IsBit(OpIndex value) { return Is<EqualOp>(value) || Is<ComparisonOp>(value); }
    ...
    bool IsWord32ConvertedToWord64(OpIndex value, base::Optional<bool>* sign_extended = nullptr)
    {
        ...
        if (int64_t c; this->MatchWord64Constant(value, &c) && c >= std::numeric_limits<int32_t>::min()) {
            if (c < 0) {
                if (sign_extended)
                    *sign_extended = true;
                return true;
    ...
    OpIndex ReduceWithTruncation(OpIndex value, uint64_t truncation_mask, WordRepresentation rep)
    {
        { // Remove bitwise-and with a mask whose zero-bits are not observed.
            OpIndex input, mask;
            uint64_t mask_value;
            if (this->MatchBitwiseAnd(value, &input, &mask, rep) && this->MatchWordConstant(mask, rep, &mask_value)) {
        ...
        }
        {
            ...
            if (this->MatchConstantShift(value, &left_shift, &right_shift_kind, &rep, &right_shift_amount) && ShiftOp::IsRightShift(right_shift_kind)
                && this->MatchConstantShift(left_shift, &left_shift_input, ShiftOp::Kind::kShiftLeft, rep, &left_shift_amount)
                ...
                if (left_shift_amount == right_shift_amount) {
                    ...
                    OpIndex shift_amount = this->WordConstant(right_shift_amount - left_shift_amount, rep);
                    return this->Shift(left_shift_input, shift_amount, right_shift_kind, rep);
                } else if (left_shift_amount > right_shift_amount) {
                    OpIndex shift_amount = this->WordConstant(left_shift_amount - right_shift_amount, rep);
                    return this->Shift(left_shift_input, shift_amount, ShiftOp::Kind::kShiftLeft, rep);
                }
            }
        }
        return value;
    ...
    OpIndex ReduceSignedDiv(OpIndex left, int64_t right, WordRepresentation rep)
    {
        ...
        if (right == rep.MinSignedValue()) {
            return this->ChangeUint32ToUint64(this->Equal(left, this->WordConstant(right, rep), rep));
        }
        // left / -right  => -(left / right)
        if (right < 0) {
            DCHECK_NE(right, rep.MinSignedValue());
            return this->WordSub(this->WordConstant(0, rep), ReduceSignedDiv(left, Abs(right), rep), rep);
        }
        ...
        if (base::bits::IsPowerOfTwo(right)) {
            ...
            DCHECK_GT(shift, 0);
            if (shift > 1) {
                quotient = this->ShiftRightArithmetic(quotient, rep.bit_width() - 1, rep);
            }
            quotient = this->ShiftRightArithmetic(quotient, rep.bit_width() - shift, rep);
        ...
        auto LowerToMul = [this, left](auto right, WordRepresentation rep) {
            ...
            OpIndex quotient = this->IntMulOverflownBits(left, this->WordConstant(magic.multiplier, rep), rep);
            if (magic.multiplier < 0) {
                quotient = this->WordAdd(quotient, left, rep);
            }
            OpIndex sign_bit = this->ShiftRightLogical(left, rep.bit_width() - 1, rep);
            return this->WordAdd(this->ShiftRightArithmetic(quotient, magic.shift, rep), sign_bit, rep);
        };
        if (rep == WordRepresentation::Word32()) {
    ...
    OpIndex ReduceUnsignedDiv(OpIndex left, uint64_t right, WordRepresentation rep)
    {
        ...
        // left / 2^k  => left >> k
        if (base::bits::IsPowerOfTwo(right)) {
            return this->ShiftRightLogical(left, base::bits::WhichPowerOfTwo(right), rep);
        }
        DCHECK_GT(right, 0);
        ...
        auto LowerToMul = [this, left, shift](auto right, WordRepresentation rep) {
            ...
            OpIndex quotient = this->UintMulOverflownBits(left, this->WordConstant(mag.multiplier, rep), rep);
            if (mag.add) {
                DCHECK_GE(mag.shift, 1);
                // quotient = (((left - quotient) >> 1) + quotient) >> (mag.shift -
                // 1)
                quotient = this->ShiftRightLogical(this->WordAdd(this->ShiftRightLogical(this->WordSub(left, quotient, rep), 1, rep), quotient, rep), mag.shift - 1, rep);
            } else {
    ...
    OpIndex ReduceBranchCondition(OpIndex condition, bool* negated)
    {
        while (true) {
            ...
            if (OpIndex left, right; this->MatchEqual(condition, &left, &right, WordRepresentation::Word32()) && this->MatchZero(right)) {
                condition = left;
                *negated = !*negated;
                continue;
            }
            // x - y  =>  x == y with flipped branches
            if (OpIndex left, right; this->MatchWordSub(condition, &left, &right, WordRepresentation::Word32())) {
                condition = this->Word32Equal(left, right);
                *negated = !*negated;
                continue;
            }
            // x & (1 << k) == (1 << k)  =>  x & (1 << k)
            if (OpIndex left, right; this->MatchEqual(condition, &left, &right, WordRepresentation::Word32())) {
                OpIndex x, mask;
                uint32_t k1, k2;
                if (this->MatchBitwiseAnd(left, &x, &mask, WordRepresentation::Word32()) && this->MatchWord32Constant(mask, &k1) && this->MatchWord32Constant(right, &k2)
                    && k1 == k2 && base::bits::IsPowerOfTwo(k1)) {
                    condition = left;
                    continue;
                }
            }
    ...
    uint16_t CountLeadingSignBits(int64_t c, WordRepresentation rep) { return base::bits::CountLeadingSignBits(c) - (64 - rep.bit_width()); }
};
