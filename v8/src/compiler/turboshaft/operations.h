...
constexpr std::underlying_type_t<Opcode> OpcodeIndex(Opcode x) { return static_cast<std::underlying_type_t<Opcode>>(x); }
...
public:
    ...
    bool IsTurbofanNodeId() const { return offset_ % sizeof(OperationStorageSlot) == kTurbofanNodeIdFlag; }
...
struct OpProperties {
    ...
    const bool is_pure = !(can_read || can_write || can_abort || is_block_terminator);
    const bool is_required_when_unused = can_write || can_abort || is_block_terminator;
    // Nodes that don't read, write and aren't block terminators can be eliminated
    // via value numbering.
    const bool can_be_eliminated = !(can_read || can_write || is_block_terminator);

    constexpr OpProperties(bool can_read, bool can_write, bool can_abort, bool is_block_terminator)
        : can_read(can_read)
        , can_write(can_write)
        , can_abort(can_abort)
    ...
    static constexpr OpProperties Reading() { return { true, false, false, false }; }
    static constexpr OpProperties Writing() { return { false, true, false, false }; }
    static constexpr OpProperties CanAbort() { return { false, false, true, false }; }
    static constexpr OpProperties AnySideEffects() { return { true, true, true, false }; }
    static constexpr OpProperties BlockTerminator() { return { false, false, false, true }; }
    static constexpr OpProperties BlockTerminatorWithAnySideEffect() { return { true, true, true, true }; }
    static constexpr OpProperties ReadingAndCanAbort() { return { true, false, true, false }; }
    static constexpr OpProperties WritingAndCanAbort() { return { false, true, true, false }; }
    bool operator==(const OpProperties& other) const
    {
        return can_read == other.can_read && can_write == other.can_write && can_abort == other.can_abort && is_block_terminator == other.is_block_terminator;
    }
};
...
struct alignas(OpIndex) Operation {
    ...
    static constexpr uint8_t kUnknownUseCount = std::numeric_limits<uint8_t>::max();
    ...
    size_t StorageSlotCount() const { return StorageSlotCount(opcode, input_count); }

    template <class Op> bool Is() const { return opcode == Op::opcode; }
    template <class Op> Op& Cast()
    {
        DCHECK(Is<Op>());
        return *static_cast<Op*>(this);
    }
    template <class Op> const Op& Cast() const
    {
        DCHECK(Is<Op>());
        return *static_cast<const Op*>(this);
    }
    template <class Op> const Op* TryCast() const
    {
        if (!Is<Op>())
            return nullptr;
        return static_cast<const Op*>(this);
    }
    template <class Op> Op* TryCast()
    {
        if (!Is<Op>())
            return nullptr;
        return static_cast<Op*>(this);
...
inline std::ostream& operator<<(std::ostream& os, const Operation& op) { return os << OperationPrintStyle { op }; }
...
template <class Derived> struct OperationT : Operation {
    ...
    base::Vector<OpIndex> inputs() { return { reinterpret_cast<OpIndex*>(reinterpret_cast<char*>(this) + sizeof(Derived)), derived_this().input_count }; }
    base::Vector<const OpIndex> inputs() const { return { reinterpret_cast<const OpIndex*>(reinterpret_cast<const char*>(this) + sizeof(Derived)), derived_this().input_count }; }
    ...
    template <class... Args> static Derived& New(Graph* graph, base::Vector<const OpIndex> inputs, Args... args) { return New(graph, inputs.size(), inputs, args...); }
    ...
    bool operator==(const Base& other) const { return derived_this().inputs() == other.derived_this().inputs() && derived_this().options() == other.derived_this().options(); }
    size_t hash_value() const { return fast_hash_combine(opcode, derived_this().inputs(), derived_this().options()); }
...
struct OverflowCheckedBinopOp : FixedArityOperationT<2, OverflowCheckedBinopOp> {
    ...
    OverflowCheckedBinopOp(OpIndex left, OpIndex right, Kind kind, WordRepresentation rep)
        : Base(left, right)
        , kind(kind)
        , rep(rep)
    {
...
struct EqualOp : FixedArityOperationT<2, EqualOp> {
    ...
    EqualOp(OpIndex left, OpIndex right, RegisterRepresentation rep)
        ...
        , rep(rep)
    {
        DCHECK(rep == any_of(RegisterRepresentation::Word32(), RegisterRepresentation::Word64(), RegisterRepresentation::Float32(), RegisterRepresentation::Float64()));
    }
    auto options() const { return std::tuple { rep }; }
};

struct ComparisonOp : FixedArityOperationT<2, ComparisonOp> {
    enum class Kind : uint8_t { kSignedLessThan, kSignedLessThanOrEqual, kUnsignedLessThan, kUnsignedLessThanOrEqual };
    Kind kind;
    RegisterRepresentation rep;
    ...
    ComparisonOp(OpIndex left, OpIndex right, Kind kind, RegisterRepresentation rep)
        : Base(left, right)
        , kind(kind)
        , rep(rep)
    {
        DCHECK_EQ(rep, any_of(RegisterRepresentation::Word32(), RegisterRepresentation::Word64(), RegisterRepresentation::Float32(), RegisterRepresentation::Float64()));
        DCHECK_IMPLIES(rep == any_of(RegisterRepresentation::Float32(), RegisterRepresentation::Float64()), kind == any_of(Kind::kSignedLessThan, Kind::kSignedLessThanOrEqual));
    }
    auto options() const { return std::tuple { kind, rep }; }
...
struct ChangeOp : FixedArityOperationT<1, ChangeOp> {
    ...
    static bool IsReversible(Kind kind, Assumption assumption, RegisterRepresentation from, RegisterRepresentation to, Kind reverse_kind, bool signalling_nan_possible)
    {
        ...
        case Kind::kFloatConversion:
            return from == RegisterRepresentation::Float32() && to == RegisterRepresentation::Float64() && reverse_kind == Kind::kFloatConversion && !signalling_nan_possible;
    ...
    bool IsReversibleBy(Kind reverse_kind, bool signalling_nan_possible) const { return IsReversible(kind, assumption, from, to, reverse_kind, signalling_nan_possible); }

    static constexpr OpProperties properties = OpProperties::Pure();

    OpIndex input() const { return Base::input(0); }

    ChangeOp(OpIndex input, Kind kind, Assumption assumption, RegisterRepresentation from, RegisterRepresentation to)
        : Base(input)
        , kind(kind)
        , assumption(assumption)
        , from(from)
...
struct TaggedBitcastOp : FixedArityOperationT<1, TaggedBitcastOp> {
    ...
    TaggedBitcastOp(OpIndex input, RegisterRepresentation from, RegisterRepresentation to)
        : Base(input)
        , from(from)
        , to(to)
    {
        DCHECK((from == RegisterRepresentation::PointerSized() && to == RegisterRepresentation::Tagged())
            || (from == RegisterRepresentation::Tagged() && to == RegisterRepresentation::PointerSized()));
    }
...
struct SelectOp : FixedArityOperationT<3, SelectOp> {
    ...
    SelectOp(OpIndex condition, OpIndex left, OpIndex right, WordRepresentation rep)
        : Base(condition, left, right)
        , rep(rep)
    {
        DCHECK(rep == WordRepresentation::Word32() ? SupportedOperations::word32_select() : SupportedOperations::word64_select());
    }
    auto options() const { return std::tuple { rep }; }
};
...
struct PendingLoopPhiOp : FixedArityOperationT<1, PendingLoopPhiOp> {
    ...
    PendingLoopPhiOp(OpIndex first, RegisterRepresentation rep, OpIndex old_backedge_index)
        : Base(first)
        , rep(rep)
    ...
    PendingLoopPhiOp(OpIndex first, RegisterRepresentation rep, Node* old_backedge_node)
        : Base(first)
        , rep(rep)
...
struct ConstantOp : FixedArityOperationT<0, ConstantOp> {
    ...
    union Storage {
        ...
        Storage(double constant)
            : float64(constant)
        {
        }
        Storage(float constant)
            : float32(constant)
        {
        }
        ...
        Storage(Handle<HeapObject> constant)
            : handle(constant)
        {
        }
    } storage;
    ...
    {
        DCHECK(kind == Kind::kWord32 || kind == Kind::kWord64 || kind == Kind::kRelocatableWasmCall || kind == Kind::kRelocatableWasmStubCall);
    ...
    bool operator==(const ConstantOp& other) const
    {
        ...
        case Kind::kFloat32:
            ...
            return base::bit_cast<uint32_t>(storage.float32) == base::bit_cast<uint32_t>(other.storage.float32)
                || (std::isnan(storage.float32) && std::isnan(other.storage.float32));
        case Kind::kFloat64:
        case Kind::kNumber:
            ...
            return base::bit_cast<uint64_t>(storage.float64) == base::bit_cast<uint64_t>(other.storage.float64)
                || (std::isnan(storage.float64) && std::isnan(other.storage.float64));
        case Kind::kExternal:
...
struct LoadOp : FixedArityOperationT<1, LoadOp> {
    ...
    LoadOp(OpIndex base, Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep, int32_t offset)
        : Base(base)
        , kind(kind)
        ...
        , offset(offset)
    {
        DCHECK(loaded_rep.ToRegisterRepresentation() == result_rep || (loaded_rep.IsTagged() && result_rep == RegisterRepresentation::Compressed()));
    }
    void PrintOptions(std::ostream& os) const;
    auto options() const { return std::tuple { kind, loaded_rep, result_rep, offset }; }
};
...
struct IndexedLoadOp : FixedArityOperationT<2, IndexedLoadOp> {
    ...
    IndexedLoadOp(OpIndex base, OpIndex index, Kind kind, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep, int32_t offset, uint8_t element_size_log2)
        : Base(base, index)
        , kind(kind)
        , loaded_rep(loaded_rep)
        , result_rep(result_rep)
    ...
    void PrintOptions(std::ostream& os) const;
    auto options() const { return std::tuple { kind, loaded_rep, offset, element_size_log2 }; }
};
...
struct ProtectedLoadOp : FixedArityOperationT<2, ProtectedLoadOp> {
    ...
    ProtectedLoadOp(OpIndex base, OpIndex index, MemoryRepresentation loaded_rep, RegisterRepresentation result_rep)
        : Base(base, index)
        , loaded_rep(loaded_rep)
        , result_rep(result_rep)
    {
...
struct StoreOp : FixedArityOperationT<2, StoreOp> {
    ...
    StoreOp(OpIndex base, OpIndex value, Kind kind, MemoryRepresentation stored_rep, WriteBarrierKind write_barrier, int32_t offset)
        : Base(base, value)
        , kind(kind)
    ...
    void PrintOptions(std::ostream& os) const;
    auto options() const { return std::tuple { kind, stored_rep, write_barrier, offset }; }
};
...
struct IndexedStoreOp : FixedArityOperationT<3, IndexedStoreOp> {
    ...
    IndexedStoreOp(
        OpIndex base, OpIndex index, OpIndex value, Kind kind, MemoryRepresentation stored_rep, WriteBarrierKind write_barrier, int32_t offset, uint8_t element_size_log2)
    ...
    void PrintOptions(std::ostream& os) const;
    auto options() const { return std::tuple { kind, stored_rep, write_barrier, offset, element_size_log2 }; }
};
...
struct ProtectedStoreOp : FixedArityOperationT<3, ProtectedStoreOp> {
    ...
    ProtectedStoreOp(OpIndex base, OpIndex index, OpIndex value, MemoryRepresentation stored_rep)
        : Base(base, index, value)
        , stored_rep(stored_rep)
    {
    }
...
struct RetainOp : FixedArityOperationT<1, RetainOp> {
    ...
    explicit RetainOp(OpIndex retained)
        : Base(retained)
    {
    }
...
struct StackSlotOp : FixedArityOperationT<0, StackSlotOp> {
    ...
    StackSlotOp(int size, int alignment)
        : size(size)
        , alignment(alignment)
    {
    }
...
struct DeoptimizeIfOp : FixedArityOperationT<2, DeoptimizeIfOp> {
    ...
    DeoptimizeIfOp(OpIndex condition, OpIndex frame_state, bool negated, const DeoptimizeParameters* parameters)
...
struct CallOp : OperationT<CallOp> {
    ...
    base::Vector<const OpIndex> arguments() const { return inputs().SubVector(1, input_count); }

    CallOp(OpIndex callee, base::Vector<const OpIndex> arguments, const CallDescriptor* descriptor)
    ...
    static CallOp& New(Graph* graph, OpIndex callee, base::Vector<const OpIndex> arguments, const CallDescriptor* descriptor)
    {
...
struct TailCallOp : OperationT<TailCallOp> {
    ...
    static constexpr OpProperties properties = OpProperties::BlockTerminatorWithAnySideEffect();

    OpIndex callee() const { return input(0); }
    base::Vector<const OpIndex> arguments() const { return inputs().SubVector(1, input_count); }

    TailCallOp(OpIndex callee, base::Vector<const OpIndex> arguments, const CallDescriptor* descriptor)
        : Base(1 + arguments.size())
        , descriptor(descriptor)
    {
        base::Vector<OpIndex> inputs = this->inputs();
    ...
    static TailCallOp& New(Graph* graph, OpIndex callee, base::Vector<const OpIndex> arguments, const CallDescriptor* descriptor)
    {
        return Base::New(graph, 1 + arguments.size(), callee, arguments, descriptor);
    }
    auto options() const { return std::tuple { descriptor }; }
...
struct ReturnOp : OperationT<ReturnOp> {
    ...
    base::Vector<const OpIndex> return_values() const { return inputs().SubVector(1, input_count); }
    ...
    static ReturnOp& New(Graph* graph, OpIndex pop_count, base::Vector<const OpIndex> return_values)
    {
...
struct GotoOp : FixedArityOperationT<0, GotoOp> {
    ...
    explicit GotoOp(Block* destination)
        : Base()
...
struct CatchExceptionOp : FixedArityOperationT<1, CatchExceptionOp> {
    ...
    explicit CatchExceptionOp(OpIndex call, Block* if_success, Block* if_exception)
        : Base(call)
...
struct SwitchOp : FixedArityOperationT<1, SwitchOp> {
    struct Case {
        ...
        bool operator==(const Case& other) const { return value == other.value && destination == other.destination; }
    };
...
template <> struct fast_hash<SwitchOp::Case> {
    size_t operator()(SwitchOp::Case v) { return fast_hash_combine(v.value, v.destination); }
};
...
struct TupleOp : OperationT<TupleOp> {
    ...
    explicit TupleOp(base::Vector<const OpIndex> inputs)
        : Base(inputs)
    {
    }
    auto options() const { return std::tuple {}; }
...
struct ProjectionOp : FixedArityOperationT<1, ProjectionOp> {
    ...
    ProjectionOp(OpIndex input, uint16_t index)
        : Base(input)
        , index(index)
    {
    }
...
static constexpr OpProperties kOperationPropertiesTable[kNumberOfOpcodes] = { TURBOSHAFT_OPERATION_LIST(OPERATION_PROPERTIES_CASE) };
...
template <class Op> const Opcode OperationT<Op>::opcode = operation_to_opcode_map<Op>::value;

template <class Op, class = void> struct static_operation_input_count : std::integral_constant<uint32_t, 0> { };
template <class Op> struct static_operation_input_count<Op, std::void_t<decltype(Op::inputs)>> : std::integral_constant<uint32_t, sizeof(Op::inputs) / sizeof(OpIndex)> { };
...
inline const OpProperties& Operation::properties() const { return kOperationPropertiesTable[OpcodeIndex(opcode)]; }
