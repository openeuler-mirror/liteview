...
struct DecompressionAnalyzer : AnalyzerBase {
    ...
    bool NeedsDecompression(const Operation& op) { return NeedsDecompression(graph.Index(op)); }
    bool MarkAsNeedsDecompression(OpIndex op) { return (needs_decompression[op] = true); }

    template <bool is_loop> void ProcessBlock(const Block& block, uint32_t* next_block_id)
    {
        for (const Operation& op : base::Reversed(graph.operations(block))) {
            if (is_loop && op.Is<PhiOp>() && NeedsDecompression(op)) {
                ...
                if (!NeedsDecompression(phi.input(1))) {
                    ...
                    *next_block_id = std::max<uint32_t>(*next_block_id, backedge->index().id());
                }
            }
            ProcessOperation(op);
        }
