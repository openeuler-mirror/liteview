...
private:
    ...
    void FixLoopPhis(Block* loop, Block* backedge)
    {
        ...
        for (Operation& op : assembler.graph().operations(*loop)) {
            if (!op.Is<PendingLoopPhiOp>())
                continue;
            auto& pending_phi = op.Cast<PendingLoopPhiOp>();
            assembler.graph().Replace<PhiOp>(assembler.graph().Index(pending_phi), base::VectorOf({ pending_phi.first(), Map(pending_phi.old_backedge_node) }), pending_phi.rep);
        }
    }
    ...
    void BuildFrameStateData(FrameStateData::Builder* builder, FrameState frame_state)
    {
    ...
    OpIndex Process(Node* node, BasicBlock* block, const base::SmallVector<int, 16>& predecessor_permutation);
};
...
base::Optional<BailoutReason> GraphBuilder::Run()
{
    ...
    for (BasicBlock* block : *schedule.rpo_order()) {
        ...
        if (!assembler.Bind(target_block))
            continue;
        ...
        std::sort(
            predecessor_permutation.begin(), predecessor_permutation.end(), [&](size_t i, size_t j) { return predecessors[i]->rpo_number() < predecessors[j]->rpo_number(); });
        ...
        case BasicBlock::kCall: {
            ...
            OpIndex catch_exception = assembler.CatchException(Map(call), if_success, if_exception);
    ...
    if (source_positions->IsEnabled()) {
        for (OpIndex index : assembler.graph().AllOperationIndices()) {
            compiler::NodeId origin = assembler.graph().operation_origins()[index].DecodeTurbofanNodeId();
            assembler.graph().source_positions()[index] = source_positions->GetSourcePosition(origin);
        }
    }
...
OpIndex GraphBuilder::Process(Node* node, BasicBlock* block, const base::SmallVector<int, 16>& predecessor_permutation)
{
    ...
    case IrOpcode::kPhi: {
        ...
        if (assembler.current_block()->IsLoop()) {
            ...
            return assembler.PendingLoopPhi(Map(node->InputAt(0)), rep, node->InputAt(1));
        } else {
    ...
    case IrOpcode::kInt64Constant:
        return assembler.Word64Constant(static_cast<uint64_t>(OpParameter<int64_t>(op)));
    case IrOpcode::kInt32Constant:
        return assembler.Word32Constant(static_cast<uint32_t>(OpParameter<int32_t>(op)));
    ...
    case IrOpcode::kRelocatableInt64Constant:
        return assembler.RelocatableConstant(OpParameter<RelocatablePtrConstantInfo>(op).value(), OpParameter<RelocatablePtrConstantInfo>(op).rmode());
...
#undef BINOP_CASE
    ...
    case IrOpcode::kWord32Sar: {
        ...
        return assembler.Shift(Map(node->InputAt(0)), Map(node->InputAt(1)), kind, rep);
    }
...
#undef UNARY_CASE
    ...
    case IrOpcode::kTruncateFloat32ToInt32:
        ...
        case TruncateKind::kArchitectureDefault:
            return assembler.TruncateFloat32ToInt32OverflowUndefined(Map(node->InputAt(0)));
        case TruncateKind::kSetOverflowToMin:
            return assembler.TruncateFloat32ToInt32OverflowToMin(Map(node->InputAt(0)));
        }
    case IrOpcode::kTruncateFloat32ToUint32:
        switch (OpParameter<TruncateKind>(node->op())) {
        case TruncateKind::kArchitectureDefault:
            return assembler.TruncateFloat32ToUint32OverflowUndefined(Map(node->InputAt(0)));
        case TruncateKind::kSetOverflowToMin:
            return assembler.TruncateFloat32ToUint32OverflowToMin(Map(node->InputAt(0)));
        }
    case IrOpcode::kTruncateFloat64ToInt64:
        switch (OpParameter<TruncateKind>(node->op())) {
        case TruncateKind::kArchitectureDefault:
            return assembler.TruncateFloat64ToInt64OverflowUndefined(Map(node->InputAt(0)));
        case TruncateKind::kSetOverflowToMin:
            return assembler.TruncateFloat64ToInt64OverflowToMin(Map(node->InputAt(0)));
        }
    case IrOpcode::kFloat64InsertLowWord32:
        return assembler.Float64InsertWord32(Map(node->InputAt(0)), Map(node->InputAt(1)), Float64InsertWord32Op::Kind::kLowHalf);
    case IrOpcode::kFloat64InsertHighWord32:
        return assembler.Float64InsertWord32(Map(node->InputAt(0)), Map(node->InputAt(1)), Float64InsertWord32Op::Kind::kHighHalf);

    case IrOpcode::kBitcastTaggedToWord:
        return assembler.TaggedBitcast(Map(node->InputAt(0)), RegisterRepresentation::Tagged(), RegisterRepresentation::PointerSized());
    case IrOpcode::kBitcastWordToTagged:
        return assembler.TaggedBitcast(Map(node->InputAt(0)), RegisterRepresentation::PointerSized(), RegisterRepresentation::Tagged());

    case IrOpcode::kWord32Select:
        return assembler.Word32Select(Map(node->InputAt(0)), Map(node->InputAt(1)), Map(node->InputAt(2)));
    case IrOpcode::kWord64Select:
        return assembler.Word64Select(Map(node->InputAt(0)), Map(node->InputAt(1)), Map(node->InputAt(2)));
    ...
    case IrOpcode::kUnalignedLoad: {
        ...
        LoadOp::Kind kind = opcode == IrOpcode::kUnalignedLoad ? LoadOp::Kind::kRawUnaligned : LoadOp::Kind::kRawAligned;
        ...
        if (index->opcode() == IrOpcode::kInt64Constant) {
            ...
            if (base::IsValueInRangeForNumericType<int32_t>(offset)) {
                return assembler.Load(Map(base), kind, loaded_rep, result_rep, static_cast<int32_t>(offset));
            }
        }
        int32_t offset = 0;
        uint8_t element_size_log2 = 0;
        return assembler.IndexedLoad(Map(base), Map(index), kind, loaded_rep, result_rep, offset, element_size_log2);
    }
    case IrOpcode::kProtectedLoad: {
        MemoryRepresentation loaded_rep = MemoryRepresentation::FromMachineType(LoadRepresentationOf(op));
        RegisterRepresentation result_rep = loaded_rep.ToRegisterRepresentation();
        return assembler.ProtectedLoad(Map(node->InputAt(0)), Map(node->InputAt(1)), loaded_rep, result_rep);
    }

    case IrOpcode::kStore:
    case IrOpcode::kUnalignedStore: {
        bool aligned = opcode != IrOpcode::kUnalignedStore;
        StoreRepresentation store_rep = aligned ? StoreRepresentationOf(op) : StoreRepresentation(UnalignedStoreRepresentationOf(op), WriteBarrierKind::kNoWriteBarrier);
        StoreOp::Kind kind = opcode == IrOpcode::kStore ? StoreOp::Kind::kRawAligned : StoreOp::Kind::kRawUnaligned;
        ...
        if (index->opcode() == IrOpcode::kInt32Constant) {
            ...
            return assembler.Store(
                Map(base), Map(value), kind, MemoryRepresentation::FromMachineRepresentation(store_rep.representation()), store_rep.write_barrier_kind(), offset);
        }
        if (index->opcode() == IrOpcode::kInt64Constant) {
            int64_t offset = OpParameter<int64_t>(index->op());
            if (base::IsValueInRangeForNumericType<int32_t>(offset)) {
                return assembler.Store(Map(base), Map(value), kind, MemoryRepresentation::FromMachineRepresentation(store_rep.representation()), store_rep.write_barrier_kind(),
                    static_cast<int32_t>(offset));
            }
        }
        int32_t offset = 0;
        uint8_t element_size_log2 = 0;
        return assembler.IndexedStore(Map(base), Map(index), Map(value), kind, MemoryRepresentation::FromMachineRepresentation(store_rep.representation()),
            store_rep.write_barrier_kind(), offset, element_size_log2);
    }
    case IrOpcode::kProtectedStore: {
        return assembler.ProtectedStore(
            Map(node->InputAt(0)), Map(node->InputAt(1)), Map(node->InputAt(2)), MemoryRepresentation::FromMachineRepresentation(OpParameter<MachineRepresentation>(node->op())));
    }
    ...
    case IrOpcode::kStackPointerGreaterThan:
        return assembler.StackPointerGreaterThan(Map(node->InputAt(0)), StackCheckKindOf(op));
    ...
    case IrOpcode::kLoadParentFramePointer:
        return assembler.FrameConstant(FrameConstantOp::Kind::kParentFramePointer);

    case IrOpcode::kStackSlot:
        return assembler.StackSlot(StackSlotRepresentationOf(op).size(), StackSlotRepresentationOf(op).alignment());

    case IrOpcode::kBranch:
        DCHECK_EQ(block->SuccessorCount(), 2);
        return assembler.Branch(Map(node->InputAt(0)), Map(block->SuccessorAt(0)), Map(block->SuccessorAt(1)));
    ...
    case IrOpcode::kSwitch: {
        ...
        return assembler.Switch(Map(node->InputAt(0)), graph_zone->CloneVector(base::VectorOf(cases)), Map(default_branch));
    }
    ...
    case IrOpcode::kCall: {
        ...
        OpIndex call = assembler.Call(callee, base::VectorOf(arguments), call_descriptor);
        if (!call_descriptor->NeedsFrameState())
            return call;
        FrameState frame_state { node->InputAt(static_cast<int>(call_descriptor->InputCount())) };
    ...
    case IrOpcode::kTailCall: {
        ...
        return assembler.TailCall(callee, base::VectorOf(arguments), call_descriptor);
    }
    ...
    case IrOpcode::kFrameState: {
        ...
        return assembler.FrameState(builder.Inputs(), builder.inlined(), builder.AllocateFrameStateData(frame_state.frame_state_info(), graph_zone));
    }
    ...
    case IrOpcode::kDeoptimizeUnless: {
        ...
        return assembler.DeoptimizeIf(condition, frame_state, negated, &DeoptimizeParametersOf(op));
    }
...
base::Optional<BailoutReason> BuildGraph(Schedule* schedule, Zone* graph_zone, Zone* phase_zone, Graph* graph, SourcePositionTable* source_positions, NodeOriginTable* origins)
{
    GraphBuilder builder { graph_zone, phase_zone, *schedule, Assembler(graph, phase_zone), source_positions, origins };
    return builder.Run();
}
