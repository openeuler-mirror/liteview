...
struct AnalyzerBase {
    ...
    bool OpIsUsed(OpIndex i) const
    {
        ...
        return op.saturated_use_count > 0 || op.properties().is_required_when_unused;
    }
...
struct LivenessAnalyzer : AnalyzerBase {
    ...
    LivenessAnalyzer(const Graph& graph, Zone* phase_zone)
        ...
        , op_used(graph.op_id_count(), false)
    {
    }
    ...
    void Run()
    {
        for (uint32_t unprocessed_count = graph.block_count(); unprocessed_count > 0;) {
    ...
    template <bool is_loop> void ProcessBlock(const Block& block, uint32_t* unprocessed_count)
    {
...
template <class Analyzer, class Assembler> class OptimizationPhase {
private:
    struct Impl;

public:
    static void Run(Graph* input, Zone* phase_zone, NodeOriginTable* origins, VisitOrder visit_order = VisitOrder::kAsEmitted)
    {
    ...
    static void RunWithoutTracing(Graph* input, Zone* phase_zone, VisitOrder visit_order = VisitOrder::kAsEmitted)
    {
...
template <class Analyzer, class Assembler> struct OptimizationPhase<Analyzer, Assembler>::Impl {
    Graph& input_graph;
    Zone* phase_zone;
    ...
    std::vector<OpIndex> op_mapping { input_graph.op_id_count(), OpIndex::Invalid() };

    // `trace_reduction` is a template parameter to avoid paying for tracing at
    // runtime.
    template <bool trace_reduction> void Run()
    {
        analyzer.Run();

        for (const Block& input_block : input_graph.blocks()) {
            block_mapping[input_block.index().id()] = assembler.NewBlock(input_block.kind());
        }
        ...
        if (!input_graph.source_positions().empty()) {
            for (OpIndex index : assembler.graph().AllOperationIndices()) {
                ...
                assembler.graph().source_positions()[index] = input_graph.source_positions()[origin];
            }
        }
        if (origins) {
            for (OpIndex index : assembler.graph().AllOperationIndices()) {
    ...
    template <bool trace_reduction> void RunAsEmittedOrder()
    {
        for (const Block& input_block : input_graph.blocks()) {
    ...
    template <bool trace_reduction> void RunDominatorOrder()
    {
    ...
    template <bool trace_reduction> void VisitBlock(const Block& input_block)
    {
        ...
        if (!assembler.Bind(MapToNewGraph(input_block.index()))) {
            ...
            const Operation& last_op = *base::Reversed(input_graph.operations(input_block)).begin();
        ...
        for (OpIndex index : input_graph.OperationIndices(input_block)) {
            ...
            if (input_block.IsLoop() && op.Is<PhiOp>()) {
                const PhiOp& phi = op.Cast<PhiOp>();
                new_index = assembler.PendingLoopPhi(MapToNewGraph(phi.inputs()[0]), phi.rep, phi.inputs()[1]);
                if constexpr (trace_reduction) {
...
#undef EMIT_INSTR_CASE
    ...
    {
        std::cout << "╭── o" << index.id() << ": " << PaddingSpace { 5 - CountDecimalDigits(index.id()) } << OperationPrintStyle { input_graph.Get(index), "#o" } << "\n";
    }
    ...
    void TraceReductionResult(OpIndex first_output_index, OpIndex new_index)
    {
        ...
        for (const Operation& op : assembler.graph().operations(first_output_index, assembler.graph().next_operation_index())) {
            ...
            std::cout << prefix << " n" << index.id() << ": " << PaddingSpace { 5 - CountDecimalDigits(index.id()) } << OperationPrintStyle { assembler.graph().Get(index), "#n" }
                      << "\n";
        }
        std::cout << "\n";
    }
    ...
    OpIndex ReduceCatchException(const CatchExceptionOp& op)
    {
        ...
        return assembler.CatchException(MapToNewGraph(op.call()), if_success, if_exception);
    }
    OpIndex ReduceSwitch(const SwitchOp& op)
    {
        ...
        return assembler.Switch(MapToNewGraph(op.input()), assembler.graph_zone()->CloneVector(base::VectorOf(cases)), MapToNewGraph(op.default_case->index()));
    }
    OpIndex ReducePhi(const PhiOp& op)
    {
        ...
        if (new_pred != nullptr) {
            ...
            for (old_pred = current_input_block->LastPredecessor(); old_pred != nullptr; old_pred = old_pred->NeighboringPredecessor()) {
                old_pred_vec.push_back(old_pred->index().id());
                // Checking that predecessors are indeed sorted.
                DCHECK_IMPLIES(old_pred->NeighboringPredecessor() != nullptr, old_pred->index().id() > old_pred->NeighboringPredecessor()->index().id());
            }
            ...
            new_inputs.clear();
            for (new_pred = assembler.current_block()->LastPredecessor(); new_pred != nullptr; new_pred = new_pred->NeighboringPredecessor()) {
                ...
                auto lower = std::lower_bound(old_pred_vec.begin(), old_pred_vec.end(), origin->index().id());
                DCHECK_NE(lower, old_pred_vec.end());
                new_inputs.push_back(MapToNewGraph(old_inputs[lower - old_pred_vec.begin()]));
            }
        }
    ...
    OpIndex ReduceReturn(const ReturnOp& op)
    {
        ...
        return assembler.Return(MapToNewGraph(op.pop_count()), base::VectorOf(return_values));
    }
    OpIndex ReduceOverflowCheckedBinop(const OverflowCheckedBinopOp& op)
    {
        return assembler.OverflowCheckedBinop(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.kind, op.rep);
    }
    OpIndex ReduceWordUnary(const WordUnaryOp& op) { return assembler.WordUnary(MapToNewGraph(op.input()), op.kind, op.rep); }
    OpIndex ReduceFloatUnary(const FloatUnaryOp& op) { return assembler.FloatUnary(MapToNewGraph(op.input()), op.kind, op.rep); }
    OpIndex ReduceShift(const ShiftOp& op) { return assembler.Shift(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.kind, op.rep); }
    OpIndex ReduceEqual(const EqualOp& op) { return assembler.Equal(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.rep); }
    OpIndex ReduceComparison(const ComparisonOp& op) { return assembler.Comparison(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.kind, op.rep); }
    OpIndex ReduceChange(const ChangeOp& op) { return assembler.Change(MapToNewGraph(op.input()), op.kind, op.assumption, op.from, op.to); }
    OpIndex ReduceTryChange(const TryChangeOp& op) { return assembler.TryChange(MapToNewGraph(op.input()), op.kind, op.from, op.to); }

    OpIndex ReduceFloat64InsertWord32(const Float64InsertWord32Op& op) { return assembler.Float64InsertWord32(MapToNewGraph(op.float64()), MapToNewGraph(op.word32()), op.kind); }
    OpIndex ReduceTaggedBitcast(const TaggedBitcastOp& op) { return assembler.TaggedBitcast(MapToNewGraph(op.input()), op.from, op.to); }
    OpIndex ReduceSelect(const SelectOp& op) { return assembler.Select(MapToNewGraph(op.condition()), MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.rep); }
    OpIndex ReduceConstant(const ConstantOp& op) { return assembler.Constant(op.kind, op.storage); }
    OpIndex ReduceLoad(const LoadOp& op) { return assembler.Load(MapToNewGraph(op.base()), op.kind, op.loaded_rep, op.result_rep, op.offset); }
    OpIndex ReduceIndexedLoad(const IndexedLoadOp& op)
    {
        return assembler.IndexedLoad(MapToNewGraph(op.base()), MapToNewGraph(op.index()), op.kind, op.loaded_rep, op.result_rep, op.offset, op.element_size_log2);
    }
    OpIndex ReduceProtectedLoad(const ProtectedLoadOp& op) { return assembler.ProtectedLoad(MapToNewGraph(op.base()), MapToNewGraph(op.index()), op.loaded_rep, op.result_rep); }
    OpIndex ReduceStore(const StoreOp& op) { return assembler.Store(MapToNewGraph(op.base()), MapToNewGraph(op.value()), op.kind, op.stored_rep, op.write_barrier, op.offset); }
    OpIndex ReduceIndexedStore(const IndexedStoreOp& op)
    {
        return assembler.IndexedStore(
            MapToNewGraph(op.base()), MapToNewGraph(op.index()), MapToNewGraph(op.value()), op.kind, op.stored_rep, op.write_barrier, op.offset, op.element_size_log2);
    }
    OpIndex ReduceProtectedStore(const ProtectedStoreOp& op)
    {
        return assembler.ProtectedStore(MapToNewGraph(op.base()), MapToNewGraph(op.index()), MapToNewGraph(op.value()), op.stored_rep);
    }
    OpIndex ReduceRetain(const RetainOp& op) { return assembler.Retain(MapToNewGraph(op.retained())); }
    OpIndex ReduceParameter(const ParameterOp& op) { return assembler.Parameter(op.parameter_index, op.debug_name); }
    OpIndex ReduceOsrValue(const OsrValueOp& op) { return assembler.OsrValue(op.index); }
    OpIndex ReduceStackPointerGreaterThan(const StackPointerGreaterThanOp& op) { return assembler.StackPointerGreaterThan(MapToNewGraph(op.stack_limit()), op.kind); }
    OpIndex ReduceStackSlot(const StackSlotOp& op) { return assembler.StackSlot(op.size, op.alignment); }
    OpIndex ReduceFrameConstant(const FrameConstantOp& op) { return assembler.FrameConstant(op.kind); }
    OpIndex ReduceCheckLazyDeopt(const CheckLazyDeoptOp& op) { return assembler.CheckLazyDeopt(MapToNewGraph(op.call()), MapToNewGraph(op.frame_state())); }
    OpIndex ReduceDeoptimize(const DeoptimizeOp& op) { return assembler.Deoptimize(MapToNewGraph(op.frame_state()), op.parameters); }
    OpIndex ReduceDeoptimizeIf(const DeoptimizeIfOp& op)
    {
        return assembler.DeoptimizeIf(MapToNewGraph(op.condition()), MapToNewGraph(op.frame_state()), op.negated, op.parameters);
    }
    OpIndex ReduceTrapIf(const TrapIfOp& op) { return assembler.TrapIf(MapToNewGraph(op.condition()), op.negated, op.trap_id); }
    OpIndex ReduceTuple(const TupleOp& op) { return assembler.Tuple(base::VectorOf(MapToNewGraph<4>(op.inputs()))); }
    OpIndex ReduceProjection(const ProjectionOp& op) { return assembler.Projection(MapToNewGraph(op.input()), op.index); }
    OpIndex ReduceWordBinop(const WordBinopOp& op) { return assembler.WordBinop(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.kind, op.rep); }
    OpIndex ReduceFloatBinop(const FloatBinopOp& op) { return assembler.FloatBinop(MapToNewGraph(op.left()), MapToNewGraph(op.right()), op.kind, op.rep); }
    OpIndex ReduceUnreachable(const UnreachableOp& op) { return assembler.Unreachable(); }
        ...
        for (Operation& op : assembler.graph().operations(*loop)) {
            if (auto* pending_phi = op.TryCast<PendingLoopPhiOp>()) {
                assembler.graph().template Replace<PhiOp>(
                    assembler.graph().Index(*pending_phi), base::VectorOf({ pending_phi->first(), MapToNewGraph(pending_phi->old_backedge_index) }), pending_phi->rep);
            }
        }
    }
};
