...
public:
    ...
    {
        table_ = phase_zone->NewVector<Entry>(base::bits::RoundUpToPowerOfTwo(std::max<size_t>(128, graph->op_id_capacity() / 2)), Entry());
        entry_count_ = 0;
        mask_ = table_.size() - 1;
    }

#define EMIT_OP(Name)                                                                                                                                                              \
    template <class... Args> OpIndex Name(Args... args)                                                                                                                            \
    {                                                                                                                                                                              \
...
private:
    ...
    {
        if constexpr (!Op::properties.can_be_eliminated || std::is_same<Op, PendingLoopPhiOp>::value) {
            return op_idx;
        }
        RehashIfNeeded();
        ...
        for (size_t i = start_index;; i = NextEntryIndex(i)) {
            ...
            if (entry.hash == 0) {
                ...
                table_[i] = Entry { op_idx, current_block()->index(), hash, depths_heads_.back() };
            ...
            if (entry.hash == hash) {
                ...
                if (entry_op.Is<Op>() && (!same_block_only || entry.block == current_block()->index()) && entry_op.Cast<Op>() == op) {
                    graph().RemoveLast();
                    return entry.value;
                }
            }
    ...
    void RehashIfNeeded()
    {
        if (V8_LIKELY(table_.size() - (table_.size() / 4) > entry_count_))
            return;
        base::Vector<Entry> new_table = table_ = phase_zone()->NewVector<Entry>(table_.size() * 2, Entry());
    ...
    Entry* NextEntry(Entry* entry) { return V8_LIKELY(entry + 1 < table_.end()) ? entry + 1 : &table_[0]; }
    Entry* PrevEntry(Entry* entry) { return V8_LIKELY(entry > table_.begin()) ? entry - 1 : table_.end() - 1; }
