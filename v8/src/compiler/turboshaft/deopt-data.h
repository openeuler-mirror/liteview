...
struct FrameStateData {
    ...
    public:
        ...
        void AddUnusedRegister() { instructions_.push_back(Instr::kUnusedRegister); }
        ...
        void AddArgumentsLength() { instructions_.push_back(Instr::kArgumentsLength); }

        const FrameStateData* AllocateFrameStateData(const FrameStateInfo& frame_state_info, Zone* zone)
        {
            return zone->New<FrameStateData>(FrameStateData { frame_state_info, zone->CloneVector(base::VectorOf(instructions_)), zone->CloneVector(base::VectorOf(machine_types_)),
    ...
    Iterator iterator(base::Vector<const OpIndex> state_values) const { return Iterator { instructions, machine_types, int_operands, state_values }; }
