...
public:
    ...
    public:
        ...
        ~ReplaceScope()
        {
            ...
            buffer_->operation_sizes_[OpIndex(replaced_.offset() + static_cast<uint32_t>(old_slot_count_) * sizeof(OperationStorageSlot)).id() - 1] = old_slot_count_;
        }
        ...
        : zone_(zone)
    {
        ...
        operation_sizes_ = zone_->NewArray<uint16_t>((initial_capacity + 1) / kSlotsPerId);
    ...
    OpIndex Index(const Operation& op) const { return Index(reinterpret_cast<const OperationStorageSlot*>(&op)); }
    ...
    void Grow(size_t min_capacity)
    {
        ...
        OperationStorageSlot* new_buffer = zone_->NewArray<OperationStorageSlot>(new_capacity);
        memcpy(new_buffer, begin_, size * sizeof(OperationStorageSlot));

        uint16_t* new_operation_sizes = zone_->NewArray<uint16_t>(new_capacity / kSlotsPerId);
...
public:
    ...
    bool Contains(OpIndex op_idx) const { return begin_ <= op_idx && op_idx < end_; }
...
public:
    ...
    OperationStorageSlot* Allocate(size_t slot_count) { return operations_.Allocate(slot_count); }
    ...
    template <class Op, class... Args> V8_INLINE OpIndex Add(Args... args)
    {
    ...
    V8_INLINE Block* NewBlock(Block::Kind kind)
    {
        if (V8_UNLIKELY(next_block_ == all_blocks_.size())) {
            ...
            base::Vector<Block> blocks = graph_zone_->NewVector<Block>(new_block_count, Block(kind));
    ...
    V8_INLINE bool Add(Block* block)
    {
        ...
        if (!bound_blocks_.empty() && !block->HasPredecessors())
            return false;
        bool deferred = true;
        for (Block* pred = block->last_predecessor_; pred != nullptr; pred = pred->neighboring_predecessor_) {
            if (!pred->IsDeferred()) {
                deferred = false;
                break;
            }
    ...
    uint32_t block_count() const { return static_cast<uint32_t>(bound_blocks_.size()); }
    uint32_t op_id_count() const { return (operations_.size() + (kSlotsPerId - 1)) / kSlotsPerId; }
    uint32_t op_id_capacity() const { return operations_.capacity() / kSlotsPerId; }

    class OpIndexIterator : public base::iterator<std::bidirectional_iterator_tag, OpIndex> {
    public:
    ...
    base::iterator_range<MutableOperationIterator> AllOperations() { return operations(operations_.BeginIndex(), operations_.EndIndex()); }
    base::iterator_range<ConstOperationIterator> AllOperations() const { return operations(operations_.BeginIndex(), operations_.EndIndex()); }

    base::iterator_range<OpIndexIterator> AllOperationIndices() const { return OperationIndices(operations_.BeginIndex(), operations_.EndIndex()); }

    base::iterator_range<MutableOperationIterator> operations(const Block& block) { return operations(block.begin_, block.end_); }
    base::iterator_range<ConstOperationIterator> operations(const Block& block) const { return operations(block.begin_, block.end_); }

    base::iterator_range<OpIndexIterator> OperationIndices(const Block& block) const { return OperationIndices(block.begin_, block.end_); }
    ...
    const GrowingSidetable<SourcePosition>& source_positions() const { return source_positions_; }
    GrowingSidetable<SourcePosition>& source_positions() { return source_positions_; }

    const GrowingSidetable<OpIndex>& operation_origins() const { return operation_origins_; }
...
V8_INLINE OperationStorageSlot* AllocateOpStorage(Graph* graph, size_t slot_count) { return graph->Allocate(slot_count); }
