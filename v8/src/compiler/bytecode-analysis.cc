...
void BytecodeLoopAssignments::Union(const BytecodeLoopAssignments& other) { bit_vector_->Union(*other.bit_vector_); }
...
ResumeJumpTarget ResumeJumpTarget::Leaf(int suspend_id, int target_offset) { return ResumeJumpTarget(suspend_id, target_offset, target_offset); }
    ...
    Handle<BytecodeArray> bytecode_array, const BytecodeLivenessMap& liveness_map, Zone* zone)
{
    ...
    // unconditional jump).
    if (next_bytecode_in_liveness != nullptr && !Bytecodes::IsUnconditionalJump(bytecode) && !Bytecodes::Returns(bytecode) && !Bytecodes::UnconditionallyThrows(bytecode)) {
        if (IsFirstUpdate) {
    ...
    if (!interpreter::Bytecodes::IsWithoutExternalSideEffects(bytecode)) {
        ...
        int handler_offset = table.LookupRange(iterator.current_offset(), &handler_context, nullptr);
...
void UpdateOutLiveness(Bytecode bytecode, BytecodeLiveness& liveness, BytecodeLivenessState* next_bytecode_in_liveness, const interpreter::BytecodeArrayIterator& iterator,
    Handle<BytecodeArray> bytecode_array, const BytecodeLivenessMap& liveness_map, Zone* zone)
{
    switch (bytecode) {
...
template <bool IsFirstUpdate = false>
void UpdateLiveness(Bytecode bytecode, BytecodeLiveness& liveness, BytecodeLivenessState** next_bytecode_in_liveness, const interpreter::BytecodeArrayIterator& iterator,
    Handle<BytecodeArray> bytecode_array, const BytecodeLivenessMap& liveness_map, Zone* zone)
{
    switch (bytecode) {
...
void BytecodeAnalysis::Analyze()
{
    ...
    for (iterator.GoToEnd(); iterator.IsValid(); --iterator) {
        ...
        bool pop_current_loop = loop_stack_.size() > 1 && (bytecode != Bytecode::kJumpLoop || iterator.GetJumpTargetOffset() == current_offset);
        ...
        if (pop_current_loop) {
            ...
            if (current_offset == current_loop.header_offset) {
                ...
                if (loop_stack_.size() > 1) {
                    ...
                    for (const auto& target : current_loop_info->resume_jump_targets()) {
                        resume_jump_targets_.push_back(ResumeJumpTarget::AtLoopHeader(current_offset, target));
                    }
                }
            }
            ...
            int suspend_id = iterator.GetUnsignedImmediateOperand(3);
            int resume_offset = current_offset + iterator.current_bytecode_size();
            resume_jump_targets_.push_back(ResumeJumpTarget::Leaf(suspend_id, resume_offset));
        }

        if (analyze_liveness_) {
            BytecodeLiveness& liveness = liveness_map().InsertNewLiveness(current_offset);
            UpdateLiveness<true>(bytecode, liveness, &next_bytecode_in_liveness, iterator, bytecode_array(), liveness_map(), zone());
        }
    }
    ...
    for (int loop_end_index : loop_end_index_queue_) {
        ...
        for (; iterator.current_offset() > header_offset; --iterator) {
            ...
            UpdateLiveness(bytecode, liveness, &next_bytecode_in_liveness, iterator, bytecode_array(), liveness_map(), zone());
        }
        // Now we are at the loop header. Since the in-liveness of the header
        // can't change, we need only to update the out-liveness.
        UpdateOutLiveness(iterator.current_bytecode(), header_liveness, next_bytecode_in_liveness, iterator, bytecode_array(), liveness_map(), zone());
    }
    ...
    if (generator_switch_index != -1) {
        ...
        int current_offset = iterator.current_offset();
        BytecodeLiveness& switch_liveness = liveness_map().GetLiveness(current_offset);

        bool any_changed = false;
        for (interpreter::JumpTableTargetOffset entry : iterator.GetJumpTableTargetOffsets()) {
            if (switch_liveness.out->UnionIsChanged(*liveness_map().GetInLiveness(entry.target_offset))) {
                any_changed = true;
            }
        }
        ...
        if (any_changed) {
            ...
            UpdateInLiveness(Bytecode::kSwitchOnGeneratorState, switch_liveness.in, iterator);
            next_bytecode_in_liveness = switch_liveness.in;
            for (--iterator; iterator.IsValid(); --iterator) {
                Bytecode bytecode = iterator.current_bytecode();
                BytecodeLiveness& liveness = liveness_map().GetLiveness(iterator.current_offset());
...
void BytecodeAnalysis::PushLoop(int loop_header, int loop_end)
{
    ...
    auto it = header_to_info_.insert({ loop_header, LoopInfo(parent_offset, bytecode_array_->parameter_count(), bytecode_array_->register_count(), zone_) });
...
std::ostream& BytecodeAnalysis::PrintLivenessTo(std::ostream& os) const
{
    ...
    for (; !iterator.done(); iterator.Advance()) {
        ...
        const BytecodeLivenessState* out_liveness = GetOutLivenessFor(current_offset);
...
bool BytecodeAnalysis::ResumeJumpTargetsAreValid()
{
    ...
    // Check top-level.
    if (!ResumeJumpTargetLeavesResolveSuspendIds(-1, resume_jump_targets(), &unresolved_suspend_ids)) {
        valid = false;
    }
    // Check loops.
    for (const std::pair<const int, LoopInfo>& loop_info : header_to_info_) {
    ...
    for (const ResumeJumpTarget& target : resume_jump_targets) {
        ...
        if (target.is_leaf()) {
            ...
            if (target.target_offset() != expected_target) {
                ...
                interpreter::BytecodeArrayIterator iterator(bytecode_array(), target.target_offset());
...
bool BytecodeAnalysis::LivenessIsValid()
{
    ...
    BytecodeLivenessState previous_liveness(bytecode_array()->register_count(), zone());

    int invalid_offset = -1;
    int which_invalid = -1;
    BytecodeLivenessState invalid_liveness(bytecode_array()->register_count(), zone());
    ...
    for (iterator.GoToEnd(); iterator.IsValid(); --iterator) {
        ...
        UpdateOutLiveness(bytecode, liveness, next_bytecode_in_liveness, iterator, bytecode_array(), liveness_map(), zone());
