...
Scheduler::Scheduler(Zone* zone, Graph* graph, Schedule* schedule, Flags flags, size_t node_count_hint, TickCounter* tick_counter, const ProfileDataFromFile* profile_data)
    : zone_(zone)
    , graph_(graph)
    , schedule_(schedule)
    , flags_(flags)
...
Scheduler::SchedulerData* Scheduler::GetData(Node* node) { return &node_data_[node->id()]; }
...
Scheduler::Placement Scheduler::GetPlacement(Node* node) { return GetData(node)->placement_; }
...
void Scheduler::IncrementUnscheduledUseCount(Node* node, Node* from)
{
    ...
    if (v8_flags.trace_turbo_scheduler) {
        TRACE("  Use count of #%d:%s (used by #%d:%s)++ = %d\n", node->id(), node->op()->mnemonic(), from->id(), from->op()->mnemonic(), GetData(node)->unscheduled_count_);
    }
}
...
#undef CONNECT_BLOCK_JS_CASE
    ...
    void ConnectBranch(Node* branch)
    {
        ...
        if (v8_flags.warn_about_builtin_profile_data && hint_from_profile != BranchHint::kNone && BranchHintOf(branch->op()) != BranchHint::kNone
    ...
    bool IsFinalMerge(Node* node) { return (node->opcode() == IrOpcode::kMerge && node == scheduler_->graph_->end()->InputAt(0)); }
...
private:
    ...
    static void SetLoopNumber(BasicBlock* block, int loop_number) { return block->set_loop_number(loop_number); }
    static bool HasLoopNumber(BasicBlock* block) { return block->loop_number() >= 0; }
...
private:
    ...
    void VisitInputs(Node* node)
    {
        ...
        base::Optional<int> coupled_control_edge = scheduler_->GetCoupledControlEdge(node);
...
private:
    ...
    BasicBlock* FindPredecessorBlock(Node* node) { return scheduler_->control_flow_builder_->FindPredecessorBlock(node); }
    ...
    void ScheduleFloatingControl(BasicBlock* block, Node* node) { scheduler_->FuseFloatingControl(block, node); }
