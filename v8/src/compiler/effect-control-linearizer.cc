...
public:
    EffectControlLinearizer(JSGraph* js_graph, Schedule* schedule, JSGraphAssembler* graph_assembler, Zone* temp_zone, SourcePositionTable* source_positions,
        NodeOriginTable* node_origins, MaintainSchedule maintain_schedule, JSHeapBroker* broker)
        : js_graph_(js_graph)
        , schedule_(schedule)
...
private:
    ...
    Node* AdaptFastCallTypedArrayArgument(Node* node, ElementsKind expected_elements_kind, GraphAssemblerLabel<0>* bailout);
    ...
    AdaptOverloadedFastCallResult AdaptOverloadedFastCallArgument(
        Node* node, const FastApiCallFunctionVector& c_functions, const fast_api_call::OverloadsResolutionResult& overloads_resolution_result, GraphAssemblerLabel<0>* if_error);
    ...
    Node* BuildCheckedFloat64ToInt32(CheckForMinusZeroMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state);
    Node* BuildCheckedFloat64ToInt64(CheckForMinusZeroMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state);
    Node* BuildCheckedFloat64ToIndex(const FeedbackSource& feedback, Node* value, Node* frame_state);
    Node* BuildCheckedHeapNumberOrOddballToFloat64(CheckTaggedInputMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state);
    Node* BuildReverseBytes(ExternalArrayType type, Node* value);
    Node* BuildFloat64RoundDown(Node* value);
    Node* BuildFloat64RoundTruncate(Node* input);
    template <size_t VarCount, size_t VarCount2> void SmiTagOrOverflow(Node* value, GraphAssemblerLabel<VarCount>* if_overflow, GraphAssemblerLabel<VarCount2>* done);
    Node* SmiTagOrDeopt(Node* value, const CheckParameters& params, Node* frame_state);
    ...
    template <typename... Args> Node* CallBuiltin(Builtin builtin, Operator::Properties properties, Args...);
    ...
    void TransitionElementsTo(Node* node, Node* array, ElementsKind from, ElementsKind to);
    ...
    // fails.
    void MigrateInstanceOrDeopt(Node* value, Node* value_map, Node* frame_state, FeedbackSource const& feedback_source, DeoptimizeReason reason);
    // Tries to migrate |value| if its map |value_map| is deprecated, but doesn't
    // deopt on failure.
    void TryMigrateInstance(Node* value, Node* value_map);

    bool should_maintain_schedule() const { return maintain_schedule_ == MaintainSchedule::kMaintain; }
    ...
    SimplifiedOperatorBuilder* simplified() const { return js_graph_->simplified(); }
...
public:
    ...
    BlockEffectControlData& For(BasicBlock* from, BasicBlock* to) { return map_[std::make_pair(from->id().ToInt(), to->id().ToInt())]; }

    const BlockEffectControlData& For(BasicBlock* from, BasicBlock* to) const { return map_.at(std::make_pair(from->id().ToInt(), to->id().ToInt())); }
...
void UpdateEffectPhi(Node* node, BasicBlock* block, BlockEffectControlMap* block_effects)
{
    // Update all inputs to an effect phi with the effects from the given
    // block->effect map.
    DCHECK_EQ(IrOpcode::kEffectPhi, node->opcode());
    DCHECK_EQ(static_cast<size_t>(node->op()->EffectInputCount()), block->PredecessorCount());
...
void UpdateBlockControl(BasicBlock* block, BlockEffectControlMap* block_effects)
{
    ...
    for (int i = 0; i < control->op()->ControlInputCount(); i++) {
        ...
        if (input != block_effect.current_control) {
            NodeProperties::ReplaceControlInput(control, block_effect.current_control, i);
        }
    }
}
...
void TryCloneBranch(Node* node, BasicBlock* block, Zone* temp_zone, Graph* graph, CommonOperatorBuilder* common, BlockEffectControlMap* block_effects,
    SourcePositionTable* source_positions, NodeOriginTable* node_origins)
{
    ...
    SourcePositionTable::Scope scope(source_positions, source_positions->GetSourcePosition(node));
...
void EffectControlLinearizer::Run()
{
    ...
    for (BasicBlock* block : *(schedule()->rpo_order())) {
        ...
        if (effect == nullptr) {
            ...
            if (block == schedule()->start()) {
                ...
                if (effect == nullptr) {
                    ...
                    effect = graph()->NewNode(common()->EffectPhi(static_cast<int>(block->PredecessorCount())), static_cast<int>(inputs_buffer.size()), &(inputs_buffer.front()));
                    gasm()->AddNode(effect);
        ...
        if (!should_maintain_schedule() && block->control() == BasicBlock::kBranch) {
            TryCloneBranch(block->control_input(), block, temp_zone(), graph(), common(), &block_effects, source_positions_, node_origins_);
        }
    ...
    for (const PendingEffectPhi& pending_effect_phi : pending_effect_phis) {
        UpdateEffectPhi(pending_effect_phi.effect_phi, pending_effect_phi.block, &block_effects);
    }
...
bool EffectControlLinearizer::TryWireInStateEffect(Node* node, Node* frame_state)
{
    Node* result = nullptr;
    switch (node->opcode()) {
    ...
    case IrOpcode::kCheckedTaggedSignedToInt32:
        if (frame_state == nullptr) {
            FATAL("No frame state (zapped by #%d: %s)", frame_state_zapper_->id(), frame_state_zapper_->op()->mnemonic());
        }
    ...
    if ((result ? 1 : 0) != node->op()->ValueOutputCount()) {
        FATAL("Effect control linearizer lowering of '%s':"
              " value output count does not agree.",
            node->op()->mnemonic());
    }

    NodeProperties::ReplaceUses(node, result, gasm()->effect(), gasm()->control());
    return true;
}
...
Node* EffectControlLinearizer::ChangeFloat64ToTagged(Node* value, CheckForMinusZeroMode mode)
{
    ...
    __ GotoIf(__ Float64Equal(value, __ ChangeInt32ToFloat64(value32)), &if_int32);
    ...
    __ Bind(&if_int32);
    {
        if (mode == CheckForMinusZeroMode::kCheckForMinusZero) {
            ...
            __ Bind(&if_zero);
            {
                ...
                __ GotoIf(__ Int32LessThan(__ Float64ExtractHighWord32(value), zero), &if_heapnumber);
                __ Goto(&if_smi);
            }
...
Node* EffectControlLinearizer::LowerChangeInt64ToTagged(Node* node)
{
    ...
    __ GotoIfNot(__ Word64Equal(__ ChangeInt32ToInt64(value32), value), &if_not_in_smi_range);
...
void EffectControlLinearizer::TruncateTaggedPointerToBit(Node* node, GraphAssemblerLabel<1>* done)
{
    Node* value = node->InputAt(0);
    ...
    __ GotoIfNot(__ Word32Equal(__ Word32And(value_map_bitfield, __ Int32Constant(Map::Bits1::IsUndetectableBit::kMask)), zero), done, zero);

    // Check if {value} is a HeapNumber.
    __ GotoIf(__ TaggedEqual(value_map, __ HeapNumberMapConstant()), &if_heapnumber);
    ...
    __ Bind(&if_bigint);
    {
        ...
        Node* length_is_zero = __ Word32Equal(__ Word32And(bitfield, __ Int32Constant(BigInt::LengthBits::kMask)), __ Int32Constant(0));
...
Node* EffectControlLinearizer::LowerTruncateTaggedToBit(Node* node)
{
    ...
    __ Bind(&if_smi);
    {
        ...
        __ Goto(&done, __ Word32Equal(__ TaggedEqual(value, __ SmiConstant(0)), __ Int32Constant(0)));
    }
...
Node* EffectControlLinearizer::LowerChangeTaggedToFloat64(Node* node) { return LowerTruncateTaggedToFloat64(node); }
...
Node* EffectControlLinearizer::LowerCheckClosure(Node* node, Node* frame_state)
{
    ...
    Node* check_instance_type
        = __ Uint32LessThanOrEqual(__ Int32Sub(value_instance_type, __ Int32Constant(FIRST_JS_FUNCTION_TYPE)), __ Int32Constant(LAST_JS_FUNCTION_TYPE - FIRST_JS_FUNCTION_TYPE));
    __ DeoptimizeIfNot(DeoptimizeReason::kWrongCallTarget, FeedbackSource(), check_instance_type, frame_state);

    // Check that the {value}s feedback vector cell matches the one
    // we recorded before.
    Node* value_cell = __ LoadField(AccessBuilder::ForJSFunctionFeedbackCell(), value);
    Node* check_cell = __ WordEqual(value_cell, __ HeapConstant(feedback_cell));
    __ DeoptimizeIfNot(DeoptimizeReason::kWrongFeedbackCell, FeedbackSource(), check_cell, frame_state);
    return value;
}

void EffectControlLinearizer::MigrateInstanceOrDeopt(Node* value, Node* value_map, Node* frame_state, FeedbackSource const& feedback_source, DeoptimizeReason reason)
{
    ...
    Node* is_not_deprecated = __ Word32Equal(__ Word32And(bitfield3, __ Int32Constant(Map::Bits3::IsDeprecatedBit::kMask)), __ Int32Constant(0));
    __ DeoptimizeIf(reason, feedback_source, is_not_deprecated, frame_state);
    Operator::Properties properties = Operator::kNoDeopt | Operator::kNoThrow;
    Runtime::FunctionId id = Runtime::kTryMigrateInstance;
    auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 1, properties, CallDescriptor::kNoFlags);
    Node* result = __ Call(call_descriptor, __ CEntryStubConstant(1), value, __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(1), __ NoContextConstant());
    Node* check = ObjectIsSmi(result);
    __ DeoptimizeIf(DeoptimizeReason::kInstanceMigrationFailed, feedback_source, check, frame_state);
}
...
void EffectControlLinearizer::LowerCheckMaps(Node* node, Node* frame_state)
{
    ...
    if (p.flags() & CheckMapsFlag::kTryMigrateInstance) {
        ...
        __ Bind(&migrate);
        MigrateInstanceOrDeopt(value, value_map, frame_state, p.feedback(), DeoptimizeReason::kWrongMap);
        ...
        for (size_t i = 0; i < map_count; ++i) {
            ...
            Node* check = __ TaggedEqual(value_map, map);
            if (i == map_count - 1) {
                __ DeoptimizeIfNot(DeoptimizeReason::kWrongMap, p.feedback(), check, frame_state);
            } else {
                auto next_map = __ MakeLabel();
                __ BranchWithCriticalSafetyCheck(check, &done, &next_map);
                __ Bind(&next_map);
...
void EffectControlLinearizer::TryMigrateInstance(Node* value, Node* value_map)
{
    ...
    Runtime::FunctionId id = Runtime::kTryMigrateInstance;
    auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 1, properties, CallDescriptor::kNoFlags);
    __ Call(call_descriptor, __ CEntryStubConstant(1), value, __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(1), __ NoContextConstant());
    __ Goto(&done);
    __ Bind(&done);
}
...
Node* EffectControlLinearizer::LowerCheckReceiver(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    ...
    Node* check = __ Uint32LessThanOrEqual(__ Uint32Constant(FIRST_JS_RECEIVER_TYPE), value_instance_type);
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAJavaScriptObject, FeedbackSource(), check, frame_state);
    return value;
}

Node* EffectControlLinearizer::LowerCheckReceiverOrNullOrUndefined(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    ...
    static_assert(LAST_TYPE == LAST_JS_RECEIVER_TYPE);
    Node* check0 = __ Uint32LessThanOrEqual(__ Uint32Constant(ODDBALL_TYPE), value_instance_type);
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAJavaScriptObjectOrNullOrUndefined, FeedbackSource(), check0, frame_state);

    // Rule out booleans.
    Node* check1 = __ TaggedEqual(value_map, __ BooleanMapConstant());
    __ DeoptimizeIf(DeoptimizeReason::kNotAJavaScriptObjectOrNullOrUndefined, FeedbackSource(), check1, frame_state);
    return value;
}
...
Node* EffectControlLinearizer::LowerCheckSymbol(Node* node, Node* frame_state)
{
    ...
    Node* check = __ TaggedEqual(value_map, __ HeapConstant(factory()->symbol_map()));
...
Node* EffectControlLinearizer::LowerCheckString(Node* node, Node* frame_state)
{
    ...
    Node* check = __ Uint32LessThan(value_instance_type, __ Uint32Constant(FIRST_NONSTRING_TYPE));
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAString, params.feedback(), check, frame_state);
    return value;
}

Node* EffectControlLinearizer::LowerCheckInternalizedString(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    ...
    Node* check = __ Word32Equal(__ Word32And(value_instance_type, __ Int32Constant(kIsNotStringMask | kIsNotInternalizedMask)), __ Int32Constant(kInternalizedTag));
    __ DeoptimizeIfNot(DeoptimizeReason::kWrongInstanceType, FeedbackSource(), check, frame_state);
...
Node* EffectControlLinearizer::LowerStringConcat(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags,
        Operator::kNoDeopt | Operator::kNoWrite | Operator::kNoThrow);

    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());

    return value;
}

Node* EffectControlLinearizer::LowerCheckedInt32Add(Node* node, Node* frame_state)
{
    ...
    __ DeoptimizeIf(DeoptimizeReason::kOverflow, FeedbackSource(), check, frame_state);
    return __ Projection(0, value);
}

Node* EffectControlLinearizer::LowerCheckedInt32Sub(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
...
Node* EffectControlLinearizer::LowerCheckedInt32Div(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
    Node* zero = __ Int32Constant(0);
    ...
    if (m.IsPowerOf2()) {
        ...
        __ DeoptimizeIfNot(DeoptimizeReason::kLostPrecision, FeedbackSource(), check, frame_state);
        ...
        __ Bind(&if_rhs_negative);
        {
            ...
            __ DeoptimizeIf(DeoptimizeReason::kDivisionByZero, FeedbackSource(), check_rhs_zero, frame_state);

            // Check if {lhs} is zero, as that would produce minus zero.
            Node* check_lhs_zero = __ Word32Equal(lhs, zero);
            __ DeoptimizeIf(DeoptimizeReason::kMinusZero, FeedbackSource(), check_lhs_zero, frame_state);
            ...
            __ Bind(&if_lhs_minint);
            {
                ...
                __ DeoptimizeIf(DeoptimizeReason::kOverflow, FeedbackSource(), check_rhs_minusone, frame_state);
...
void EffectControlLinearizer::SmiTagOrOverflow(Node* value, GraphAssemblerLabel<VarCount>* if_overflow, GraphAssemblerLabel<VarCount2>* done)
{
    DCHECK(SmiValuesAre31Bits());
...
Node* EffectControlLinearizer::SmiTagOrDeopt(Node* value, const CheckParameters& params, Node* frame_state)
{
    DCHECK(SmiValuesAre31Bits());
    ...
    __ DeoptimizeIf(DeoptimizeReason::kLostPrecision, params.feedback(), check, frame_state);
...
Node* EffectControlLinearizer::BuildUint32Mod(Node* lhs, Node* rhs)
{
    ...
    __ GotoIf(__ Word32Equal(__ Word32And(rhs, msk), __ Int32Constant(0)), &if_rhs_power_of_two);
    {
...
Node* EffectControlLinearizer::LowerCheckedInt32Mod(Node* node, Node* frame_state)
{
    ...
    __ Bind(&if_rhs_not_positive);
    {
        ...
        __ DeoptimizeIf(DeoptimizeReason::kDivisionByZero, FeedbackSource(), __ Word32Equal(vtrue0, zero), frame_state);
    ...
    __ Bind(&if_lhs_negative);
    {
        ...
        __ DeoptimizeIf(DeoptimizeReason::kMinusZero, FeedbackSource(), __ Word32Equal(res, zero), frame_state);
...
Node* EffectControlLinearizer::LowerCheckedUint32Div(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
    Node* zero = __ Int32Constant(0);
    ...
    if (m.IsPowerOf2()) {
        ...
        __ DeoptimizeIf(DeoptimizeReason::kDivisionByZero, FeedbackSource(), check, frame_state);
...
Node* EffectControlLinearizer::LowerCheckedUint32Mod(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
...
Node* EffectControlLinearizer::LowerCheckedInt32Mul(Node* node, Node* frame_state)
{
    CheckForMinusZeroMode mode = CheckMinusZeroModeOf(node->op());
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
    ...
    if (mode == CheckForMinusZeroMode::kCheckForMinusZero) {
        ...
        __ DeoptimizeIf(DeoptimizeReason::kMinusZero, FeedbackSource(), check_or, frame_state);
        __ Goto(&check_done);
...
Node* EffectControlLinearizer::LowerCheckedInt32ToTaggedSigned(Node* node, Node* frame_state)
{
    DCHECK(SmiValuesAre31Bits());
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckedInt64ToInt32(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckedInt64ToTaggedSigned(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckedUint32Bounds(Node* node, Node* frame_state)
{
...
Node* EffectControlLinearizer::LowerCheckedUint32ToInt32(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
    Node* unsafe = __ Int32LessThan(value, __ Int32Constant(0));
    __ DeoptimizeIf(DeoptimizeReason::kLostPrecision, params.feedback(), unsafe, frame_state);
    return value;
}

Node* EffectControlLinearizer::LowerCheckedUint32ToTaggedSigned(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
    Node* check = __ Uint32LessThanOrEqual(value, SmiMaxValueConstant());
...
Node* EffectControlLinearizer::LowerCheckedUint64Bounds(Node* node, Node* frame_state)
{
...
Node* EffectControlLinearizer::LowerCheckedUint64ToInt32(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckedUint64ToInt64(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());

    Node* check = __ Uint64LessThanOrEqual(value, __ Uint64Constant(std::numeric_limits<int64_t>::max()));
    __ DeoptimizeIfNot(DeoptimizeReason::kLostPrecision, params.feedback(), check, frame_state);
    return value;
}

Node* EffectControlLinearizer::LowerCheckedUint64ToTaggedSigned(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::BuildCheckedFloat64ToInt32(CheckForMinusZeroMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state)
{
    Node* value32 = __ RoundFloat64ToInt32(value);
    Node* check_same = __ Float64Equal(value, __ ChangeInt32ToFloat64(value32));
    __ DeoptimizeIfNot(DeoptimizeReason::kLostPrecisionOrNaN, feedback, check_same, frame_state);
    ...
    if (mode == CheckForMinusZeroMode::kCheckForMinusZero) {
        ...
        __ Bind(&if_zero);
        // In case of 0, we need to check the high bits for the IEEE -0 pattern.
        Node* check_negative = __ Int32LessThan(__ Float64ExtractHighWord32(value), __ Int32Constant(0));
        __ DeoptimizeIf(DeoptimizeReason::kMinusZero, feedback, check_negative, frame_state);
        __ Goto(&check_done);
...
Node* EffectControlLinearizer::BuildCheckedFloat64ToIndex(const FeedbackSource& feedback, Node* value, Node* frame_state)
{
    if (machine()->Is64()) {
        Node* value64 = __ TruncateFloat64ToInt64(value, TruncateKind::kArchitectureDefault);
        ...
        __ DeoptimizeIfNot(DeoptimizeReason::kLostPrecisionOrNaN, feedback, check_same, frame_state);
        Node* check_max = __ IntLessThan(value64, __ Int64Constant(kMaxSafeInteger));
        __ DeoptimizeIfNot(DeoptimizeReason::kNotAnArrayIndex, feedback, check_max, frame_state);
        Node* check_min = __ IntLessThan(__ Int64Constant(-kMaxSafeInteger), value64);
        __ DeoptimizeIfNot(DeoptimizeReason::kNotAnArrayIndex, feedback, check_min, frame_state);
        return value64;
    } else {
        Node* value32 = __ RoundFloat64ToInt32(value);
        Node* check_same = __ Float64Equal(value, __ ChangeInt32ToFloat64(value32));
...
Node* EffectControlLinearizer::LowerCheckedFloat64ToInt32(Node* node, Node* frame_state)
{
    const CheckMinusZeroParameters& params = CheckMinusZeroParametersOf(node->op());
    Node* value = node->InputAt(0);
    return BuildCheckedFloat64ToInt32(params.mode(), params.feedback(), value, frame_state);
}

Node* EffectControlLinearizer::BuildCheckedFloat64ToInt64(CheckForMinusZeroMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state)
{
    Node* value64 = __ TruncateFloat64ToInt64(value, TruncateKind::kSetOverflowToMin);
    Node* check_same = __ Float64Equal(value, __ ChangeInt64ToFloat64(value64));
    __ DeoptimizeIfNot(DeoptimizeReason::kLostPrecisionOrNaN, feedback, check_same, frame_state);
...
Node* EffectControlLinearizer::LowerCheckedFloat64ToInt64(Node* node, Node* frame_state)
{
    const CheckMinusZeroParameters& params = CheckMinusZeroParametersOf(node->op());
    Node* value = node->InputAt(0);
    return BuildCheckedFloat64ToInt64(params.mode(), params.feedback(), value, frame_state);
}

Node* EffectControlLinearizer::LowerCheckedTaggedSignedToInt32(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
    Node* check = ObjectIsSmi(value);
...
Node* EffectControlLinearizer::LowerCheckedTaggedToArrayIndex(Node* node, Node* frame_state)
{
    ...
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    Node* is_string = __ Uint32LessThan(value_instance_type, __ Uint32Constant(FIRST_NONSTRING_TYPE));
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAString, params.feedback(), is_string, frame_state);
    ...
    Node* string_to_array_index_function = __ ExternalConstant(ExternalReference::string_to_array_index_function());
    auto call_descriptor = Linkage::GetSimplifiedCDescriptor(graph()->zone(), builder.Build());
    Node* index = __ Call(common()->Call(call_descriptor), string_to_array_index_function, value);

    __ DeoptimizeIf(DeoptimizeReason::kNotAnArrayIndex, params.feedback(), __ Word32Equal(index, __ Int32Constant(-1)), frame_state);
...
Node* EffectControlLinearizer::LowerCheckedTaggedToInt32(Node* node, Node* frame_state)
{
    const CheckMinusZeroParameters& params = CheckMinusZeroParametersOf(node->op());
    Node* value = node->InputAt(0);
    ...
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAHeapNumber, params.feedback(), check_map, frame_state);
    Node* vfalse = __ LoadField(AccessBuilder::ForHeapNumberValue(), value);
...
Node* EffectControlLinearizer::LowerCheckedTaggedToInt64(Node* node, Node* frame_state)
{
    const CheckMinusZeroParameters& params = CheckMinusZeroParametersOf(node->op());
    Node* value = node->InputAt(0);
    ...
    __ DeoptimizeIfNot(DeoptimizeReason::kNotAHeapNumber, params.feedback(), check_map, frame_state);
    Node* vfalse = __ LoadField(AccessBuilder::ForHeapNumberValue(), value);
    vfalse = BuildCheckedFloat64ToInt64(params.mode(), params.feedback(), vfalse, frame_state);
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::BuildCheckedHeapNumberOrOddballToFloat64(CheckTaggedInputMode mode, const FeedbackSource& feedback, Node* value, Node* frame_state)
{
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* check_number = __ TaggedEqual(value_map, __ HeapNumberMapConstant());
    switch (mode) {
    case CheckTaggedInputMode::kNumber: {
        __ DeoptimizeIfNot(DeoptimizeReason::kNotAHeapNumber, feedback, check_number, frame_state);
        break;
    }
    case CheckTaggedInputMode::kNumberOrBoolean: {
        ...
        __ DeoptimizeIfNot(DeoptimizeReason::kNotANumberOrBoolean, feedback, __ TaggedEqual(value_map, __ BooleanMapConstant()), frame_state);
        STATIC_ASSERT_FIELD_OFFSETS_EQUAL(HeapNumber::kValueOffset, Oddball::kToNumberRawOffset);
        __ Goto(&check_done);
...
Node* EffectControlLinearizer::LowerCheckedTaggedToFloat64(Node* node, Node* frame_state)
{
    ...
    Node* number = BuildCheckedHeapNumberOrOddballToFloat64(p.mode(), p.feedback(), value, frame_state);
    __ Goto(&done, number);
...
Node* EffectControlLinearizer::LowerCheckedTaggedToTaggedSigned(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckedTaggedToTaggedPointer(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    const CheckParameters& params = CheckParametersOf(node->op());
...
Node* EffectControlLinearizer::LowerCheckBigInt(Node* node, Node* frame_state)
{
    ...
    __ DeoptimizeIf(DeoptimizeReason::kSmi, params.feedback(), smi_check, frame_state);

    // Check for BigInt.
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* bi_check = __ TaggedEqual(value_map, __ BigIntMapConstant());
    __ DeoptimizeIfNot(DeoptimizeReason::kWrongInstanceType, params.feedback(), bi_check, frame_state);

    return value;
}

Node* EffectControlLinearizer::LowerCheckBigInt64(Node* node, Node* frame_state)
{
    DCHECK(machine()->Is64());
    ...
    __ Bind(&if_not_zero);
    {
        ...
        Node* length = __ Word32And(bitfield, __ Int32Constant(BigInt::LengthBits::kMask));
        __ DeoptimizeIfNot(
            DeoptimizeReason::kWrongInstanceType, params.feedback(), __ Word32Equal(length, __ Int32Constant(uint32_t { 1 } << BigInt::LengthBits::kShift)), frame_state);

        Node* lsd = __ LoadField(AccessBuilder::ForBigIntLeastSignificantDigit64(), value);
        // Accepted small BigInts are in the range [-2^63 + 1, 2^63 - 1].
        // Excluding -2^63 from the range makes the check simpler and faster.
        Node* bi64_check = __ Uint64LessThanOrEqual(lsd, __ Int64Constant(std::numeric_limits<int64_t>::max()));
        __ DeoptimizeIfNot(DeoptimizeReason::kWrongInstanceType, params.feedback(), bi64_check, frame_state);
        __ Goto(&done);
    }
...
Node* EffectControlLinearizer::LowerCheckedBigInt64Add(Node* node, Node* frame_state)
{
    DCHECK(machine()->Is64());
...
Node* EffectControlLinearizer::LowerChangeInt64ToBigInt(Node* node)
{
    ...
    __ GotoIf(__ Word64Equal(value, __ IntPtrConstant(0)), &done, BuildAllocateBigInt(nullptr, nullptr));

    // Shift sign bit into BigInt's sign bit position.
    Node* sign = __ Word64Shr(value, __ IntPtrConstant(63 - BigInt::SignBits::kShift));
    Node* bitfield = __ Word32Or(__ Int32Constant(BigInt::LengthBits::encode(1)), sign);
...
Node* EffectControlLinearizer::LowerTruncateBigIntToWord64(Node* node)
{
    ...
    {
        Node* lsd = __ LoadField(AccessBuilder::ForBigIntLeastSignificantDigit64(), value);
        Node* sign = __ Word32And(bitfield, __ Int32Constant(BigInt::SignBits::kMask));
...
Node* EffectControlLinearizer::LowerCheckedTruncateTaggedToWord32(Node* node, Node* frame_state)
{
    ...
    // to int32.
    __ Bind(&if_not_smi);
    Node* number = BuildCheckedHeapNumberOrOddballToFloat64(params.mode(), params.feedback(), value, frame_state);
...
Node* EffectControlLinearizer::LowerNumberToString(Node* node)
{
    ...
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kNumberToString);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), argument, __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerObjectIsArrayBufferView(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    Node* vfalse = __ Uint32LessThan(__ Int32Sub(value_instance_type, __ Int32Constant(FIRST_JS_ARRAY_BUFFER_VIEW_TYPE)),
        __ Int32Constant(LAST_JS_ARRAY_BUFFER_VIEW_TYPE - FIRST_JS_ARRAY_BUFFER_VIEW_TYPE + 1));
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::LowerObjectIsCallable(Node* node)
{
    ...
    Node* vfalse = __ Word32Equal(__ Int32Constant(Map::Bits1::IsCallableBit::kMask), __ Word32And(value_bit_field, __ Int32Constant(Map::Bits1::IsCallableBit::kMask)));
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::LowerObjectIsConstructor(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_bit_field = __ LoadField(AccessBuilder::ForMapBitField(), value_map);
    Node* vfalse = __ Word32Equal(__ Int32Constant(Map::Bits1::IsConstructorBit::kMask), __ Word32And(value_bit_field, __ Int32Constant(Map::Bits1::IsConstructorBit::kMask)));
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::LowerObjectIsDetectableCallable(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_bit_field = __ LoadField(AccessBuilder::ForMapBitField(), value_map);
    Node* vfalse = __ Word32Equal(__ Int32Constant(Map::Bits1::IsCallableBit::kMask),
        __ Word32And(value_bit_field, __ Int32Constant((Map::Bits1::IsCallableBit::kMask) | (Map::Bits1::IsUndetectableBit::kMask))));
    __ Goto(&done, vfalse);
...
{
    Node* value = node->InputAt(0);
    Node* check = __ Word32Equal(__ Float64ExtractHighWord32(value), __ Int32Constant(kHoleNanUpper32));
    return check;
}
...
Node* EffectControlLinearizer::LowerObjectIsFiniteNumber(Node* node)
{
    ...
    __ GotoIfNot(__ TaggedEqual(value_map, __ HeapNumberMapConstant()), &done, zero);
...
Node* EffectControlLinearizer::LowerNumberIsSafeInteger(Node* node)
{
    ...
    Node* in_range = __ Float64LessThanOrEqual(__ Float64Abs(trunc), __ Float64Constant(kMaxSafeInteger));
...
Node* EffectControlLinearizer::LowerObjectIsMinusZero(Node* node)
{
    ...
    if (machine()->Is64()) {
        ...
        __ GotoIfNot(__ Word32Equal(value_lo, __ Int32Constant(kMinusZeroLoBits)), &done, zero);
        Node* value_hi = __ Float64ExtractHighWord32(value_value);
        __ Goto(&done, __ Word32Equal(value_hi, __ Int32Constant(kMinusZeroHiBits)));
    }
...
Node* EffectControlLinearizer::LowerNumberIsMinusZero(Node* node)
{
    ...
    if (machine()->Is64()) {
        ...
        __ GotoIfNot(__ Word32Equal(value_lo, __ Int32Constant(kMinusZeroLoBits)), &done, __ Int32Constant(0));
...
Node* EffectControlLinearizer::LowerObjectIsNaN(Node* node)
{
    ...
    Node* value_value = __ LoadField(AccessBuilder::ForHeapNumberValue(), value);
    __ Goto(&done, __ Word32Equal(__ Float64Equal(value_value, value_value), zero));
...
Node* EffectControlLinearizer::LowerObjectIsNonCallable(Node* node)
{
    ...
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    static_assert(LAST_TYPE == LAST_JS_RECEIVER_TYPE);
    Node* check1 = __ Uint32LessThanOrEqual(__ Uint32Constant(FIRST_JS_RECEIVER_TYPE), value_instance_type);
    __ GotoIfNot(check1, &if_primitive);

    Node* value_bit_field = __ LoadField(AccessBuilder::ForMapBitField(), value_map);
    Node* check2 = __ Word32Equal(__ Int32Constant(0), __ Word32And(value_bit_field, __ Int32Constant(Map::Bits1::IsCallableBit::kMask)));
    __ Goto(&done, check2);
...
Node* EffectControlLinearizer::LowerObjectIsReceiver(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    Node* result = __ Uint32LessThanOrEqual(__ Uint32Constant(FIRST_JS_RECEIVER_TYPE), value_instance_type);
    __ Goto(&done, result);
...
Node* EffectControlLinearizer::LowerObjectIsString(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    Node* vfalse = __ Uint32LessThan(value_instance_type, __ Uint32Constant(FIRST_NONSTRING_TYPE));
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::LowerObjectIsUndetectable(Node* node)
{
    ...
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
    Node* value_bit_field = __ LoadField(AccessBuilder::ForMapBitField(), value_map);
    Node* vfalse = __ Word32Equal(__ Word32Equal(__ Int32Constant(0), __ Word32And(value_bit_field, __ Int32Constant(Map::Bits1::IsUndetectableBit::kMask))), __ Int32Constant(0));
    __ Goto(&done, vfalse);
...
Node* EffectControlLinearizer::LowerTypeOf(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), obj, __ NoContextConstant());
}

Node* EffectControlLinearizer::LowerToBoolean(Node* node)
{
    Node* obj = node->InputAt(0);
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kToBoolean);
    Operator::Properties const properties = Operator::kEliminatable;
    CallDescriptor::Flags const flags = CallDescriptor::kNoAllocate;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
...
{
    Node* arguments_length = ChangeIntPtrToSmi(__ Load(MachineType::Pointer(), __ LoadFramePointer(), __ IntPtrConstant(StandardFrameConstants::kArgCOffset)));
    arguments_length = __ SmiSub(arguments_length, __ SmiConstant(kJSArgcReceiverSlots));
...
Node* EffectControlLinearizer::LowerRestLength(Node* node)
{
    ...
    Node* arguments_length = ChangeIntPtrToSmi(__ Load(MachineType::Pointer(), frame, __ IntPtrConstant(StandardFrameConstants::kArgCOffset)));
    arguments_length = __ SmiSub(arguments_length, __ SmiConstant(kJSArgcReceiverSlots));
    Node* rest_length = __ SmiSub(arguments_length, __ SmiConstant(formal_parameter_count));
    __ GotoIf(__ SmiLessThan(rest_length, __ SmiConstant(0)), &done, __ SmiConstant(0));
...
Node* EffectControlLinearizer::LowerNewDoubleElements(Node* node)
{
    ...
    __ GotoIf(zero_length, &done, __ HeapConstant(factory()->empty_fixed_array()));

    // Compute the effective size of the backing store.
    Node* size = __ IntAdd(__ WordShl(length, __ IntPtrConstant(kDoubleSizeLog2)), __ IntPtrConstant(FixedDoubleArray::kHeaderSize));

    // Allocate the result and initialize the header.
    Node* result = __ Allocate(allocation, size);
    __ StoreField(AccessBuilder::ForMap(), result, __ FixedDoubleArrayMapConstant());
    __ StoreField(AccessBuilder::ForFixedArrayLength(), result, ChangeIntPtrToSmi(length));
    ...
    __ Bind(&loop);
    {
        ...
        ElementAccess const access = { kTaggedBase, FixedDoubleArray::kHeaderSize, Type::NumberOrHole(), MachineType::Float64(), kNoWriteBarrier };
...
Node* EffectControlLinearizer::LowerNewSmiOrObjectElements(Node* node)
{
    ...
    // Compute the effective size of the backing store.
    Node* size = __ IntAdd(__ WordShl(length, __ IntPtrConstant(kTaggedSizeLog2)), __ IntPtrConstant(FixedArray::kHeaderSize));
    ...
    __ Bind(&loop);
    {
        ...
        ElementAccess const access = { kTaggedBase, FixedArray::kHeaderSize, Type::Any(), MachineType::AnyTagged(), kNoWriteBarrier };
        __ StoreElement(access, result, index, the_hole);
...
{
    const NewArgumentsElementsParameters& parameters = NewArgumentsElementsParametersOf(node->op());
    ...
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), frame, __ IntPtrConstant(parameters.formal_parameter_count()), arguments_count);
}
...
Node* EffectControlLinearizer::LowerNewConsString(Node* node)
{
    ...
    __ Branch(__ Word32Equal(encoding, __ Int32Constant(kTwoByteStringTag)), &if_twobyte, &if_onebyte);
    __ Bind(&if_onebyte);
    ...
    __ StoreField(AccessBuilder::ForNameRawHashField(), result, __ Int32Constant(Name::kEmptyHashField));
...
Node* EffectControlLinearizer::LowerSameValue(Node* node)
{
    ...
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kSameValue);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerSameValueNumbersOnly(Node* node)
{
    ...
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kSameValueNumbersOnly);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
...
Node* EffectControlLinearizer::LowerStringToNumber(Node* node)
{
    ...
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kStringToNumber);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), string, __ NoContextConstant());
}

Node* EffectControlLinearizer::StringCharCodeAt(Node* receiver, Node* position)
{
    // We need a loop here to properly deal with indirect strings
    // (SlicedString, ConsString and ThinString).
    auto loop = __ MakeLoopLabel(MachineRepresentation::kTagged, MachineType::PointerRepresentation());
    auto loop_next = __ MakeLabel(MachineRepresentation::kTagged, MachineType::PointerRepresentation());
    ...
    __ Bind(&loop);
    {
        ...
        Node* receiver_representation = __ Word32And(receiver_instance_type, __ Int32Constant(kStringRepresentationMask));
        ...
        __ Branch(__ Int32LessThanOrEqual(receiver_representation, __ Int32Constant(kConsStringTag)), &if_lessthanoreq_cons, &if_greaterthan_cons);

        __ Bind(&if_lessthanoreq_cons);
        {
            __ Branch(__ Word32Equal(receiver_representation, __ Int32Constant(kConsStringTag)), &if_consstring, &if_seqstring);
        }

        __ Bind(&if_greaterthan_cons);
        {
            __ GotoIf(__ Word32Equal(receiver_representation, __ Int32Constant(kThinStringTag)), &if_thinstring);
            __ GotoIf(__ Word32Equal(receiver_representation, __ Int32Constant(kExternalStringTag)), &if_externalstring);
            __ Branch(__ Word32Equal(receiver_representation, __ Int32Constant(kSlicedStringTag)), &if_slicedstring, &if_runtime);
        }

        __ Bind(&if_seqstring);
        {
            Node* receiver_is_onebyte = __ Word32Equal(
                __ Word32Equal(__ Word32And(receiver_instance_type, __ Int32Constant(kStringEncodingMask)), __ Int32Constant(kTwoByteStringTag)), __ Int32Constant(0));
        ...
        __ Bind(&if_consstring);
        {
            ...
            __ GotoIfNot(__ TaggedEqual(receiver_second, __ EmptyStringConstant()), &if_runtime);
        ...
        __ Bind(&if_externalstring);
        {
            ...
            __ GotoIf(
                __ Word32Equal(__ Word32And(receiver_instance_type, __ Int32Constant(kUncachedExternalStringMask)), __ Int32Constant(kUncachedExternalStringTag)), &if_runtime);

            Node* receiver_data = __ LoadField(AccessBuilder::ForExternalStringResourceData(), receiver);

            auto if_onebyte = __ MakeLabel();
            auto if_twobyte = __ MakeLabel();
            __ Branch(__ Word32Equal(__ Word32And(receiver_instance_type, __ Int32Constant(kStringEncodingMask)), __ Int32Constant(kTwoByteStringTag)), &if_twobyte, &if_onebyte);
            ...
            __ Bind(&if_twobyte);
            {
                Node* result = __ Load(MachineType::Uint16(), receiver_data, __ WordShl(position, __ IntPtrConstant(1)));
                __ Goto(&loop_done, result);
            }
        }
        ...
        __ Bind(&if_slicedstring);
        {
            ...
            __ Goto(&loop_next, receiver_parent, __ IntAdd(position, ChangeSmiToIntPtr(receiver_offset)));
        }
        ...
        __ Bind(&if_runtime);
        {
            ...
            auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 2, properties, CallDescriptor::kNoFlags);
            Node* result = __ Call(call_descriptor, __ CEntryStubConstant(1), receiver, ChangeIntPtrToSmi(position), __ ExternalConstant(ExternalReference::Create(id)),
...
Node* EffectControlLinearizer::LowerStringCodePointAt(Node* node)
{
    ...
    __ GotoIfNot(__ Word32Equal(__ Word32And(first_code_unit, __ Int32Constant(0xFC00)), __ Int32Constant(0xD800)), &return_result, BranchHint::kFalse, first_code_unit);

    auto length = __ LoadField(AccessBuilder::ForStringLength(), receiver);
    auto next_index = __ IntAdd(position, __ IntPtrConstant(1));
    __ GotoIfNot(__ IntLessThan(next_index, length), &return_result, first_code_unit);
    Node* second_code_unit = StringCharCodeAt(receiver, next_index);
    __ GotoIfNot(__ Word32Equal(__ Word32And(second_code_unit, __ Int32Constant(0xFC00)), __ Int32Constant(0xDC00)), &return_result, first_code_unit);

    auto surrogate_offset = __ Int32Constant(0x10000 - (0xD800 << 10) - 0xDC00);
    auto result = __ Int32Add(__ Word32Shl(first_code_unit, __ Int32Constant(10)), __ Int32Add(second_code_unit, surrogate_offset));
...
Node* EffectControlLinearizer::LoadFromSeqString(Node* receiver, Node* position, Node* is_one_byte)
{
    auto one_byte_load = __ MakeLabel();
    auto done = __ MakeLabel(MachineRepresentation::kWord32);
    __ GotoIf(is_one_byte, &one_byte_load);
    Node* two_byte_result = __ LoadElement(AccessBuilder::ForSeqTwoByteStringCharacter(), receiver, position);
    __ Goto(&done, two_byte_result);

    __ Bind(&one_byte_load);
    Node* one_byte_element = __ LoadElement(AccessBuilder::ForSeqOneByteStringCharacter(), receiver, position);
...
Node* EffectControlLinearizer::LowerStringFromSingleCharCode(Node* node)
{
    ...
    Node* check1 = __ Uint32LessThanOrEqual(code, __ Uint32Constant(String::kMaxOneByteCharCode));
    __ GotoIfNot(check1, &if_not_one_byte);
    {
        ...
        // table.
        Node* entry = __ LoadElement(AccessBuilder::ForFixedArrayElement(), table, index);
    ...
    __ Bind(&if_not_one_byte);
    {
        ...
        Node* vfalse1 = __ Allocate(AllocationType::kYoung, __ IntPtrConstant(SeqTwoByteString::SizeFor(1)));
        __ StoreField(AccessBuilder::ForMap(), vfalse1, __ HeapConstant(factory()->string_map()));
        __ StoreField(AccessBuilder::ForNameRawHashField(), vfalse1, __ Int32Constant(Name::kEmptyHashField));
        __ StoreField(AccessBuilder::ForStringLength(), vfalse1, __ Int32Constant(1));
        __ Store(StoreRepresentation(MachineRepresentation::kWord16, kNoWriteBarrier), vfalse1, __ IntPtrConstant(SeqTwoByteString::kHeaderSize - kHeapObjectTag), code);
...
Node* EffectControlLinearizer::LowerStringToLowerCaseIntl(Node* node)
{
    ...
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kStringToLowerCaseIntl);
    Operator::Properties properties = Operator::kNoDeopt | Operator::kNoThrow;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), receiver, __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerStringToUpperCaseIntl(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 1, properties, CallDescriptor::kNoFlags);
    return __ Call(call_descriptor, __ CEntryStubConstant(1), receiver, __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(1), __ NoContextConstant());
}

#else

Node* EffectControlLinearizer::LowerStringToLowerCaseIntl(Node* node) { UNREACHABLE(); }

Node* EffectControlLinearizer::LowerStringToUpperCaseIntl(Node* node) { UNREACHABLE(); }
...
Node* EffectControlLinearizer::LowerStringFromSingleCodePoint(Node* node)
{
...

    {
        ...
        __ Bind(&if_not_one_byte);
        {
            ...
            Node* vfalse1 = __ Allocate(AllocationType::kYoung, __ IntPtrConstant(SeqTwoByteString::SizeFor(1)));
            __ StoreField(AccessBuilder::ForMap(), vfalse1, __ HeapConstant(factory()->string_map()));
            __ StoreField(AccessBuilder::ForNameRawHashField(), vfalse1, __ IntPtrConstant(Name::kEmptyHashField));
            __ StoreField(AccessBuilder::ForStringLength(), vfalse1, __ Int32Constant(1));
            __ Store(StoreRepresentation(MachineRepresentation::kWord16, kNoWriteBarrier), vfalse1, __ IntPtrConstant(SeqTwoByteString::kHeaderSize - kHeapObjectTag), code);
            __ Goto(&done, vfalse1);
        }
    ...
    // Generate surrogate pair string
    {
        ...
        Node* trail = __ Int32Add(__ Word32And(code, __ Int32Constant(0x3FF)), __ Int32Constant(0xDC00));
        ...
        // Allocate a new SeqTwoByteString for {code}.
        Node* vfalse0 = __ Allocate(AllocationType::kYoung, __ IntPtrConstant(SeqTwoByteString::SizeFor(2)));
        __ StoreField(AccessBuilder::ForMap(), vfalse0, __ HeapConstant(factory()->string_map()));
        __ StoreField(AccessBuilder::ForNameRawHashField(), vfalse0, __ Int32Constant(Name::kEmptyHashField));
        __ StoreField(AccessBuilder::ForStringLength(), vfalse0, __ Int32Constant(2));
        __ Store(StoreRepresentation(MachineRepresentation::kWord32, kNoWriteBarrier), vfalse0, __ IntPtrConstant(SeqTwoByteString::kHeaderSize - kHeapObjectTag), code);
...
Node* EffectControlLinearizer::LowerStringIndexOf(Node* node)
{
    ...
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), subject, search_string, position, __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerStringFromCodePointAt(Node* node)
{
    ...
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kStringFromCodePointAt);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), string, index, __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerStringComparison(Callable const& callable, Node* node)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);
...
Node* EffectControlLinearizer::LowerStringSubstring(Node* node)
{
    ...
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kStringSubstring);
    Operator::Properties properties = Operator::kEliminatable;
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), receiver, start, end, __ NoContextConstant());
}

Node* EffectControlLinearizer::LowerStringEqual(Node* node) { return LowerStringComparison(Builtins::CallableFor(isolate(), Builtin::kStringEqual), node); }

Node* EffectControlLinearizer::LowerStringLessThan(Node* node) { return LowerStringComparison(Builtins::CallableFor(isolate(), Builtin::kStringLessThan), node); }

Node* EffectControlLinearizer::LowerStringLessThanOrEqual(Node* node) { return LowerStringComparison(Builtins::CallableFor(isolate(), Builtin::kStringLessThanOrEqual), node); }
...
Node* EffectControlLinearizer::LowerBigIntAdd(Node* node, Node* frame_state)
{
    ...
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntAddNoThrow);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());

    // Check for exception sentinel: Smi is returned to signal BigIntTooBig.
    __ DeoptimizeIf(DeoptimizeReason::kBigIntTooBig, FeedbackSource {}, ObjectIsSmi(value), frame_state);

    return value;
}

Node* EffectControlLinearizer::LowerBigIntSubtract(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);

    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntSubtractNoThrow);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
...
Node* EffectControlLinearizer::LowerBigIntMultiply(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);

    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntMultiplyNoThrow);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
    ...
    __ Bind(&if_termreq);
    {
        ...
        auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 0, Operator::kNoDeopt, CallDescriptor::kNeedsFrameState);
        __ Call(call_descriptor, __ CEntryStubConstant(1), __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(0), __ NoContextConstant(), frame_state);
        __ Goto(&done);
    }
...
Node* EffectControlLinearizer::LowerBigIntDivide(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);

    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntDivideNoThrow);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
    ...
    __ DeoptimizeIf(DeoptimizeReason::kDivisionByZero, FeedbackSource {}, ObjectIsSmi(value), frame_state);
...
Node* EffectControlLinearizer::LowerBigIntBitwiseAnd(Node* node, Node* frame_state)
{
    Node* lhs = node->InputAt(0);
    Node* rhs = node->InputAt(1);

    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntBitwiseAndNoThrow);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), lhs, rhs, __ NoContextConstant());
...
{
    Callable const callable = Builtins::CallableFor(isolate(), Builtin::kBigIntUnaryMinus);
    auto call_descriptor = Linkage::GetStubCallDescriptor(
        graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, Operator::kFoldable | Operator::kNoThrow);
    Node* value = __ Call(call_descriptor, __ HeapConstant(callable.code()), node->InputAt(0), __ NoContextConstant());

    return value;
}

Node* EffectControlLinearizer::LowerCheckFloat64Hole(Node* node, Node* frame_state)
{
...
Node* EffectControlLinearizer::LowerCheckNotTaggedHole(Node* node, Node* frame_state)
{
    Node* value = node->InputAt(0);
    Node* check = __ TaggedEqual(value, __ TheHoleConstant());
    __ DeoptimizeIf(DeoptimizeReason::kHole, FeedbackSource(), check, frame_state);
    return value;
}
...
void EffectControlLinearizer::LowerCheckEqualsInternalizedString(Node* node, Node* frame_state)
{
    ...
    __ Bind(&if_notsame);
    {
        ...
        __ DeoptimizeIf(DeoptimizeReason::kWrongName, FeedbackSource(), ObjectIsSmi(val), frame_state);
        ...
        __ GotoIf(__ Word32Equal(val_instance_type, __ Int32Constant(THIN_ONE_BYTE_STRING_TYPE)), &if_thinstring);
        __ Branch(__ Word32Equal(val_instance_type, __ Int32Constant(THIN_STRING_TYPE)), &if_thinstring, &if_notthinstring);
        ...
        __ Bind(&if_notthinstring);
        {
            ...
            __ DeoptimizeIfNot(DeoptimizeReason::kWrongName, FeedbackSource(),
                __ Word32Equal(__ Word32And(val_instance_type, __ Int32Constant(kIsNotStringMask | kIsNotInternalizedMask)), __ Int32Constant(kStringTag | kNotInternalizedTag)),
                frame_state);
            ...
            Node* try_string_to_index_or_lookup_existing = __ ExternalConstant(ExternalReference::try_string_to_index_or_lookup_existing());
            Node* const isolate_ptr = __ ExternalConstant(ExternalReference::isolate_address(isolate()));
            auto call_descriptor = Linkage::GetSimplifiedCDescriptor(graph()->zone(), builder.Build());
            Node* val_internalized = __ Call(common()->Call(call_descriptor), try_string_to_index_or_lookup_existing, isolate_ptr, val);

            // Now see if the results match.
            __ DeoptimizeIfNot(DeoptimizeReason::kWrongName, FeedbackSource(), __ TaggedEqual(exp, val_internalized), frame_state);
            __ Goto(&if_same);
        }
        ...
        __ Bind(&if_thinstring);
        {
            ...
            __ DeoptimizeIfNot(DeoptimizeReason::kWrongName, FeedbackSource(), __ TaggedEqual(exp, val_actual), frame_state);
            __ Goto(&if_same);
        }
    }
...
void EffectControlLinearizer::LowerCheckEqualsSymbol(Node* node, Node* frame_state)
{
    Node* exp = node->InputAt(0);
    Node* val = node->InputAt(1);
    Node* check = __ TaggedEqual(exp, val);
    __ DeoptimizeIfNot(DeoptimizeReason::kWrongName, FeedbackSource(), check, frame_state);
}
...
Node* EffectControlLinearizer::ChangeTaggedInt32ToSmi(Node* value)
{
    ...
    // important.
    return COMPRESS_POINTERS_BOOL ? __ BitcastWord32ToWord64(value) : ChangeInt32ToIntPtr(value);
}
...
Node* EffectControlLinearizer::ChangeUint32ToSmi(Node* value)
{
    ...
    if (machine()->Is64() && SmiValuesAre31Bits()) {
        ...
        // In pointer compression, we smi-corrupt. Then, the upper bits are not
        // important.
        return COMPRESS_POINTERS_BOOL ? __ BitcastWord32ToWord64(smi_value) : __ ChangeUint32ToUint64(smi_value);
    } else {
...
Node* EffectControlLinearizer::ChangeSmiToIntPtr(Node* value)
{
    if (machine()->Is64() && SmiValuesAre31Bits()) {
        ...
        return __ WordSarShiftOutZeros(__ ChangeInt32ToInt64(__ TruncateInt64ToInt32(value)), SmiShiftBitsConstant());
    }
...
Node* EffectControlLinearizer::ChangeSmiToInt32(Node* value)
{
    // Do shift on 32bit values if Smis are stored in the lower word.
    if (machine()->Is64() && SmiValuesAre31Bits()) {
        return __ Word32SarShiftOutZeros(__ TruncateInt64ToInt32(value), SmiShiftBitsConstant());
    }
    if (machine()->Is64()) {
...
Node* EffectControlLinearizer::ObjectIsSmi(Node* value) { return __ Word32Equal(__ Word32And(value, __ Int32Constant(kSmiTagMask)), __ Int32Constant(kSmiTag)); }

Node* EffectControlLinearizer::SmiMaxValueConstant() { return __ Int32Constant(Smi::kMaxValue); }
...
Node* EffectControlLinearizer::LowerPlainPrimitiveToWord32(Node* node)
{
    ...
    __ Bind(&if_not_smi);
    Node* to_number = __ PlainPrimitiveToNumber(TNode<Object>::UncheckedCast(value));
...
Node* EffectControlLinearizer::LowerEnsureWritableFastElements(Node* node)
{
    ...
    Operator::Properties properties = Operator::kEliminatable;
    Callable callable = Builtins::CallableFor(isolate(), Builtin::kCopyFastSmiOrObjectElements);
    CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
    Node* result = __ Call(call_descriptor, __ HeapConstant(callable.code()), object, __ NoContextConstant());
    __ Goto(&done, result);
...
Node* EffectControlLinearizer::LowerMaybeGrowFastElements(Node* node, Node* frame_state)
{
    ...
    Operator::Properties properties = Operator::kEliminatable;
    Callable callable = (params.mode() == GrowFastElementsMode::kDoubleElements) ? Builtins::CallableFor(isolate(), Builtin::kGrowFastDoubleElements)
                                                                                 : Builtins::CallableFor(isolate(), Builtin::kGrowFastSmiOrObjectElements);
    CallDescriptor::Flags call_flags = CallDescriptor::kNoFlags;
    auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), call_flags, properties);
    Node* new_elements = __ Call(call_descriptor, __ HeapConstant(callable.code()), object, ChangeInt32ToSmi(index), __ NoContextConstant());

    // Ensure that we were able to grow the {elements}.
    __ DeoptimizeIf(DeoptimizeReason::kCouldNotGrowElements, params.feedback(), ObjectIsSmi(new_elements), frame_state);
...
void EffectControlLinearizer::LowerTransitionElementsKind(Node* node)
{
    ...
    case ElementsTransition::kSlowTransition: {
        ...
        auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 2, properties, CallDescriptor::kNoFlags);
        __ Call(call_descriptor, __ CEntryStubConstant(1), object, target_map, __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(2), __ NoContextConstant());
        break;
    }
    }
    __ Goto(&done);
...
Node* EffectControlLinearizer::AdaptFastCallTypedArrayArgument(Node* node, ElementsKind expected_elements_kind, GraphAssemblerLabel<0>* bailout)
{
    Node* value_map = __ LoadField(AccessBuilder::ForMap(), node);
    Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
    Node* value_is_typed_array = __ Word32Equal(value_instance_type, __ Int32Constant(JS_TYPED_ARRAY_TYPE));
    ...
    Node* value_is_expected_elements_kind = __ Word32Equal(kind, __ Int32Constant(expected_elements_kind));
    __ GotoIfNot(value_is_expected_elements_kind, bailout);

    Node* buffer = __ LoadField(AccessBuilder::ForJSArrayBufferViewBuffer(), node);
    Node* buffer_bit_field = __ LoadField(AccessBuilder::ForJSArrayBufferBitField(), buffer);

    // Go to the slow path if the {buffer} was detached.
    Node* buffer_is_not_detached = __ Word32Equal(__ Word32And(buffer_bit_field, __ Int32Constant(JSArrayBuffer::WasDetachedBit::kMask)), __ ZeroConstant());
    __ GotoIfNot(buffer_is_not_detached, bailout);

    // Go to the slow path if the {buffer} is shared.
    Node* buffer_is_not_shared = __ Word32Equal(__ Word32And(buffer_bit_field, __ Int32Constant(JSArrayBuffer::IsSharedBit::kMask)), __ ZeroConstant());
    __ GotoIfNot(buffer_is_not_shared, bailout);

    // Unpack the store and length, and store them to a struct
    // FastApiTypedArray.
    Node* external_pointer = __ LoadField(AccessBuilder::ForJSTypedArrayExternalPointer(), node);
    ...
    Node* base_pointer = __ LoadField(AccessBuilder::ForJSTypedArrayBasePointer(), node);
    if (JSTypedArray::kMaxSizeInHeap == 0) {
        base_pointer = jsgraph()->ZeroConstant();
    }
    Node* data_ptr = BuildTypedArrayDataPointer(base_pointer, external_pointer);
    Node* length_in_bytes = __ LoadField(AccessBuilder::ForJSTypedArrayLength(), node);
    ...
    __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), stack_slot, 0, length_in_bytes);
    __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), stack_slot, sizeof(size_t), data_ptr);
...
Node* EffectControlLinearizer::ClampFastCallArgument(Node* input, CTypeInfo::Type scalar_type)
{
    Node* min = nullptr;
    Node* max = nullptr;
    switch (scalar_type) {
    ...
    Node* clamped = graph()->NewNode(common()->Select(MachineRepresentation::kFloat64), graph()->NewNode(machine()->Float64LessThan(), min, input),
        graph()->NewNode(common()->Select(MachineRepresentation::kFloat64), graph()->NewNode(machine()->Float64LessThan(), input, max), input, max), min);

    Node* rounded = graph()->NewNode(machine()->Float64RoundTiesEven().placeholder(), clamped);
...
Node* EffectControlLinearizer::AdaptFastCallArgument(Node* node, CTypeInfo arg_type, GraphAssemblerLabel<0>* if_error)
{
    ...
    case CTypeInfo::SequenceType::kScalar: {
        ...
        if (flags & uint8_t(CTypeInfo::Flags::kEnforceRangeBit)) {
            ...
            case CTypeInfo::Type::kV8Value: {
                Node* stack_slot = __ StackSlot(kSize, kAlign);
                __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), stack_slot, 0, node);
    ...
    case CTypeInfo::SequenceType::kIsSequence: {
        ...
        Node* value_map = __ LoadField(AccessBuilder::ForMap(), node);
        Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
        Node* value_is_js_array = __ Word32Equal(value_instance_type, __ Int32Constant(JS_ARRAY_TYPE));
    ...
    case CTypeInfo::SequenceType::kIsTypedArray: {
        ...
        return AdaptFastCallTypedArrayArgument(node, fast_api_call::GetTypedArrayElementsKind(arg_type.GetType()), if_error);
    }
    default: {
        UNREACHABLE();
    }
...
EffectControlLinearizer::AdaptOverloadedFastCallResult EffectControlLinearizer::AdaptOverloadedFastCallArgument(
    Node* node, const FastApiCallFunctionVector& c_functions, const fast_api_call::OverloadsResolutionResult& overloads_resolution_result, GraphAssemblerLabel<0>* if_error)
{
    auto merge = __ MakeLabel(MachineRepresentation::kTagged, MachineRepresentation::kTagged);
    ...
    for (size_t func_index = 0; func_index < c_functions.size(); func_index++) {
        ...
        CTypeInfo arg_type = c_signature->ArgumentInfo(overloads_resolution_result.distinguishable_arg_index);
        ...
        case CTypeInfo::SequenceType::kIsSequence: {
            ...
            Node* target_address = __ ExternalConstant(ExternalReference::Create(c_functions[func_index].address, ref_type));
        ...
        case CTypeInfo::SequenceType::kIsTypedArray: {
            ...
            Node* stack_slot = AdaptFastCallTypedArrayArgument(node, fast_api_call::GetTypedArrayElementsKind(overloads_resolution_result.element_type), &next);
            Node* target_address = __ ExternalConstant(ExternalReference::Create(c_functions[func_index].address, ref_type));
            __ Goto(&merge, target_address, stack_slot);
            break;
        }
...
Node* EffectControlLinearizer::GenerateSlowApiCall(Node* node)
{
    ...
    Node** const slow_inputs = graph()->zone()->NewArray<Node*>(n.SlowCallArgumentCount() + FastApiCallNode::kEffectAndControlInputCount);

    int fast_call_params = c_arg_count;
    CHECK_EQ(node->op()->ValueInputCount() - fast_call_params, n.SlowCallArgumentCount());
    int index = 0;
    ...
    Node* slow_call_result = __ Call(params.descriptor(), index + FastApiCallNode::kEffectAndControlInputCount, slow_inputs);
...
Node* EffectControlLinearizer::LowerFastApiCall(Node* node)
{
    ...
    CHECK_EQ(FastApiCallNode::ArityForArgc(c_arg_count, js_arg_count), value_input_count);

    return fast_api_call::BuildFastApiCall(
        isolate(), graph(), gasm(), c_functions, c_signature, n.SlowCallArgument(FastApiCallNode::kSlowCallDataArgumentIndex),
        // Load and convert parameters to be passed to C function
        [this, node, c_signature, c_functions](int param_index, fast_api_call::OverloadsResolutionResult& overloads_resolution_result, GraphAssemblerLabel<0>* if_error) {
            Node* value = NodeProperties::GetValueInput(node, param_index);
            if (param_index == overloads_resolution_result.distinguishable_arg_index) {
                ...
                // argument.
                AdaptOverloadedFastCallResult nodes = AdaptOverloadedFastCallArgument(value, c_functions, overloads_resolution_result, if_error);
        ...
        [this](const CFunctionInfo* c_signature, Node* c_call_result) -> Node* {
            ...
            case CTypeInfo::Type::kBool:
                ...
                return ChangeBitToTagged(__ Word32And(c_call_result, __ Int32Constant(0xFF)));
            case CTypeInfo::Type::kInt32:
            ...
            case CTypeInfo::Type::kUint64:
                UNREACHABLE();
            case CTypeInfo::Type::kFloat32:
                return ChangeFloat64ToTagged(__ ChangeFloat32ToFloat64(c_call_result), CheckForMinusZeroMode::kCheckForMinusZero);
            case CTypeInfo::Type::kFloat64:
                return ChangeFloat64ToTagged(c_call_result, CheckForMinusZeroMode::kCheckForMinusZero);
            case CTypeInfo::Type::kV8Value:
            case CTypeInfo::Type::kApiObject:
            case CTypeInfo::Type::kUint8:
                UNREACHABLE();
            case CTypeInfo::Type::kAny:
                return ChangeFloat64ToTagged(__ ChangeInt64ToFloat64(c_call_result), CheckForMinusZeroMode::kCheckForMinusZero);
            }
        },
        // Initialize js-specific callback options.
        [this](Node* options_stack_slot) {
            __ Store(StoreRepresentation(MachineType::PointerRepresentation(), kNoWriteBarrier), options_stack_slot,
                static_cast<int>(offsetof(v8::FastApiCallbackOptions, wasm_memory)), __ IntPtrConstant(0));
        },
...
Node* EffectControlLinearizer::LowerLoadFieldByIndex(Node* node)
{
    ...
    // to the left by one in the code below.
    {
        ...
        {
            Node* offset = __ IntAdd(__ WordShl(index, __ IntPtrConstant(kTaggedSizeLog2 - 1)), __ IntPtrConstant(JSObject::kHeaderSize - kHeapObjectTag));
        ...
        {
            Node* properties = __ LoadField(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), object);
            Node* offset = __ IntAdd(
                __ WordShl(__ IntSub(zero, index), __ IntPtrConstant(kTaggedSizeLog2 - 1)), __ IntPtrConstant((FixedArray::kHeaderSize - kTaggedSize) - kHeapObjectTag));
    ...
    __ Bind(&if_double);
    {
        ...
        // The field is located in the {object} itself.
        {
            Node* offset = __ IntAdd(__ WordShl(index, __ IntPtrConstant(kTaggedSizeLog2)), __ IntPtrConstant(JSObject::kHeaderSize - kHeapObjectTag));
            Node* field = __ Load(MachineType::AnyTagged(), object, offset);
            __ Goto(&loaded_field, field);
        }
        ...
        __ Bind(&if_outofobject);
        {
            Node* properties = __ LoadField(AccessBuilder::ForJSObjectPropertiesOrHashKnownPointer(), object);
            Node* offset
                = __ IntAdd(__ WordShl(__ IntSub(zero, index), __ IntPtrConstant(kTaggedSizeLog2)), __ IntPtrConstant((FixedArray::kHeaderSize - kTaggedSize) - kHeapObjectTag));
            Node* field = __ Load(MachineType::AnyTagged(), properties, offset);
            __ Goto(&loaded_field, field);
        }
    }
    ...
    __ Bind(&loaded_field);
    {
        ...
        __ GotoIfNot(__ TaggedEqual(field_map, __ HeapNumberMapConstant()), &done, field);
...
Node* EffectControlLinearizer::BuildReverseBytes(ExternalArrayType type, Node* value)
{
    switch (type) {
...
void EffectControlLinearizer::LowerStoreDataViewElement(Node* node)
{
    ...
    __ Bind(&done);
    __ StoreUnaligned(machine_type.representation(), storage, index, done.PhiAt(0));
}
...
Node* EffectControlLinearizer::BuildTypedArrayDataPointer(Node* base, Node* external)
{
...
Node* EffectControlLinearizer::LowerLoadTypedElement(Node* node)
{
    ...
    return __ LoadElement(AccessBuilder::ForTypedArrayElement(array_type, true), data_ptr, index);
}
...
void EffectControlLinearizer::LowerStoreTypedElement(Node* node)
{
    ...
    // Perform the actual typed element access.
    __ StoreElement(AccessBuilder::ForTypedArrayElement(array_type, true), data_ptr, index, value);
}

void EffectControlLinearizer::TransitionElementsTo(Node* node, Node* array, ElementsKind from, ElementsKind to)
{
    DCHECK(IsMoreGeneralElementsKindTransition(from, to));
    DCHECK(to == HOLEY_ELEMENTS || to == HOLEY_DOUBLE_ELEMENTS);

    Handle<Map> target(to == HOLEY_ELEMENTS ? FastMapParameterOf(node->op()) : DoubleMapParameterOf(node->op()));
    ...
    if (IsSimpleMapChangeTransition(from, to)) {
        ...
        Runtime::FunctionId id = Runtime::kTransitionElementsKind;
        auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 2, properties, CallDescriptor::kNoFlags);
        __ Call(call_descriptor, __ CEntryStubConstant(1), array, target_map, __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(2), __ NoContextConstant());
    }
}

Node* EffectControlLinearizer::IsElementsKindGreaterThan(Node* kind, ElementsKind reference_kind)
{
...
void EffectControlLinearizer::LowerTransitionAndStoreElement(Node* node)
{
    ...
    {
        __ GotoIfNot(IsElementsKindGreaterThan(kind, HOLEY_SMI_ELEMENTS), &transition_smi_array);
        __ GotoIfNot(IsElementsKindGreaterThan(kind, HOLEY_ELEMENTS), &do_store, kind);
    ...
    __ Bind(&transition_smi_array); // deferred code.
    {
        ...
        __ GotoIfNot(check, &if_value_not_heap_number);
        {
            ...
            TransitionElementsTo(node, array, HOLEY_SMI_ELEMENTS, HOLEY_DOUBLE_ELEMENTS);
    ...
    auto done = __ MakeLabel();
    __ GotoIf(IsElementsKindGreaterThan(kind, HOLEY_ELEMENTS), &if_kind_is_double);
    {
        // Our ElementsKind is HOLEY_SMI_ELEMENTS or HOLEY_ELEMENTS.
        __ StoreElement(AccessBuilder::ForFixedArrayElement(HOLEY_ELEMENTS), elements, index, value);
        __ Goto(&done);
    }
    __ Bind(&if_kind_is_double);
    {
        ...
        __ GotoIfNot(ObjectIsSmi(value), &do_double_store);
        {
            ...
            __ StoreElement(AccessBuilder::ForFixedDoubleArrayElement(), elements, index, float_value);
            __ Goto(&done);
        }
        __ Bind(&do_double_store);
        {
            Node* float_value = __ LoadField(AccessBuilder::ForHeapNumberValue(), value);
            __ StoreElement(AccessBuilder::ForFixedDoubleArrayElement(), elements, index, __ Float64SilenceNaN(float_value));
            __ Goto(&done);
        }
    }
...
void EffectControlLinearizer::LowerTransitionAndStoreNumberElement(Node* node)
{
    ...
    auto transition_smi_array = __ MakeDeferredLabel();
    {
        ...
        __ GotoIf(__ Word32Equal(kind, __ Int32Constant(HOLEY_DOUBLE_ELEMENTS)), &do_store);
    ...
    Node* elements = __ LoadField(AccessBuilder::ForJSObjectElements(), array);
    __ StoreElement(AccessBuilder::ForFixedDoubleArrayElement(), elements, index, __ Float64SilenceNaN(value));
}

void EffectControlLinearizer::LowerTransitionAndStoreNonNumberElement(Node* node)
{
    Node* array = node->InputAt(0);
    Node* index = node->InputAt(1);
    Node* value = node->InputAt(2);
    ...
    {
        __ GotoIfNot(IsElementsKindGreaterThan(kind, HOLEY_SMI_ELEMENTS), &transition_smi_array);
        __ GotoIf(IsElementsKindGreaterThan(kind, HOLEY_ELEMENTS), &transition_double_to_fast);
        __ Goto(&do_store);
    }
...
void EffectControlLinearizer::LowerRuntimeAbort(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 1, properties, CallDescriptor::kNoFlags);
    __ Call(call_descriptor, __ CEntryStubConstant(1), __ SmiConstant(static_cast<int>(reason)), __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(1),
        __ NoContextConstant());
}

template <typename... Args> Node* EffectControlLinearizer::CallBuiltin(Builtin builtin, Operator::Properties properties, Args... args)
{
    Callable const callable = Builtins::CallableFor(isolate(), builtin);
    auto call_descriptor
        = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), CallDescriptor::kNoFlags, properties);
    return __ Call(call_descriptor, __ HeapConstant(callable.code()), args..., __ NoContextConstant());
}
...
Node* EffectControlLinearizer::LowerAssertType(Node* node)
{
    ...
    CallBuiltin(Builtin::kCheckTurbofanType, node->op()->properties(), input, allocated_type, __ SmiConstant(node->id()));
    return input;
}
...
Node* EffectControlLinearizer::LowerFoldConstant(Node* node)
{
    ...
    CallBuiltin(Builtin::kCheckSameObject, node->op()->properties(), original, constant);
    return constant;
}
...
Node* EffectControlLinearizer::LowerConvertReceiver(Node* node)
{
    ...
    case ConvertReceiverMode::kNotNullOrUndefined: {
        ...
        Node* value_map = __ LoadField(AccessBuilder::ForMap(), value);
        Node* value_instance_type = __ LoadField(AccessBuilder::ForMapInstanceType(), value_map);
        Node* check = __ Uint32LessThan(value_instance_type, __ Uint32Constant(FIRST_JS_RECEIVER_TYPE));
        ...
        CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
        auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
        Node* native_context = __ LoadField(AccessBuilder::ForJSGlobalProxyNativeContext(), global_proxy);
        Node* result = __ Call(call_descriptor, __ HeapConstant(callable.code()), value, native_context);
    ...
    case ConvertReceiverMode::kAny: {
        ...
        // Wrap the primitive {value} into a JSPrimitiveWrapper.
        __ Bind(&convert_to_object);
        __ GotoIf(__ TaggedEqual(value, __ UndefinedConstant()), &convert_global_proxy);
        __ GotoIf(__ TaggedEqual(value, __ NullConstant()), &convert_global_proxy);
        Operator::Properties properties = Operator::kEliminatable;
        Callable callable = Builtins::CallableFor(isolate(), Builtin::kToObject);
        CallDescriptor::Flags flags = CallDescriptor::kNoFlags;
        auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
...
Node* EffectControlLinearizer::LowerFindOrderedHashMapEntry(Node* node)
{
    ...
    {
        Callable const callable = Builtins::CallableFor(isolate(), Builtin::kFindOrderedHashMapEntry);
        Operator::Properties const properties = node->op()->properties();
        CallDescriptor::Flags const flags = CallDescriptor::kNoFlags;
        auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
        return __ Call(call_descriptor, __ HeapConstant(callable.code()), table, key, __ NoContextConstant());
    }
}
...
Node* EffectControlLinearizer::LowerFindOrderedHashSetEntry(Node* node)
{
    ...
    {
        Callable const callable = Builtins::CallableFor(isolate(), Builtin::kFindOrderedHashSetEntry);
        Operator::Properties const properties = node->op()->properties();
        CallDescriptor::Flags const flags = CallDescriptor::kNoFlags;
        auto call_descriptor = Linkage::GetStubCallDescriptor(graph()->zone(), callable.descriptor(), callable.descriptor().GetStackParameterCount(), flags, properties);
        return __ Call(call_descriptor, __ HeapConstant(callable.code()), table, key, __ NoContextConstant());
...
Node* EffectControlLinearizer::ComputeUnseededHash(Node* value)
{
    ...
    value = __ Int32Add(__ Word32Xor(value, __ Int32Constant(0xFFFFFFFF)), __ Word32Shl(value, __ Int32Constant(15)));
...
Node* EffectControlLinearizer::LowerFindOrderedHashMapEntryForInt32Key(Node* node)
{
    Node* table = NodeProperties::GetValueInput(node, 0);
    Node* key = NodeProperties::GetValueInput(node, 1);
    ...
    Node* number_of_buckets = ChangeSmiToIntPtr(__ LoadField(AccessBuilder::ForOrderedHashMapOrSetNumberOfBuckets(), table));
    hash = __ WordAnd(hash, __ IntSub(number_of_buckets, __ IntPtrConstant(1)));
    Node* first_entry = ChangeSmiToIntPtr(__ Load(MachineType::TaggedSigned(), table,
        __ IntAdd(__ WordShl(hash, __ IntPtrConstant(kTaggedSizeLog2)), __ IntPtrConstant(OrderedHashMap::HashTableStartOffset() - kHeapObjectTag))));
    ...
    __ Bind(&loop);
    {
        ...
        Node* check = __ IntPtrEqual(entry, __ IntPtrConstant(OrderedHashMap::kNotFound));
        __ GotoIf(check, &done, entry);
        entry = __ IntAdd(__ IntMul(entry, __ IntPtrConstant(OrderedHashMap::kEntrySize)), number_of_buckets);

        Node* candidate_key = __ Load(MachineType::AnyTagged(), table,
            __ IntAdd(__ WordShl(entry, __ IntPtrConstant(kTaggedSizeLog2)), __ IntPtrConstant(OrderedHashMap::HashTableStartOffset() - kHeapObjectTag)));
        ...
        __ Branch(__ Word32Equal(ChangeSmiToInt32(candidate_key), key), &if_match, &if_notmatch);

        __ Bind(&if_notsmi);
        __ GotoIfNot(__ TaggedEqual(__ LoadField(AccessBuilder::ForMap(), candidate_key), __ HeapNumberMapConstant()), &if_notmatch);
        __ Branch(__ Float64Equal(__ LoadField(AccessBuilder::ForHeapNumberValue(), candidate_key), __ ChangeInt32ToFloat64(key)), &if_match, &if_notmatch);
        ...
        __ Bind(&if_notmatch);
        {
            Node* next_entry = ChangeSmiToIntPtr(__ Load(MachineType::TaggedSigned(), table,
...
Node* EffectControlLinearizer::LowerDateNow(Node* node)
{
    ...
    auto call_descriptor = Linkage::GetRuntimeCallDescriptor(graph()->zone(), id, 0, properties, CallDescriptor::kNoFlags);
    return __ Call(call_descriptor, __ CEntryStubConstant(1), __ ExternalConstant(ExternalReference::Create(id)), __ Int32Constant(0), __ NoContextConstant());
}
...
{
    return __ Word32Equal(__ Word32And(TruncateWordToInt32(__ BitcastTaggedToWordForTagAndSmiBits(value)), __ Int32Constant(kHeapObjectTagMask)), __ Int32Constant(kHeapObjectTag));
}

Node* EffectControlLinearizer::MakeWeakForComparison(Node* heap_object)
{
    // TODO(gsathya): Specialize this for pointer compression.
    return __ BitcastWordToTagged(__ WordOr(__ BitcastTaggedToWord(heap_object), __ IntPtrConstant(kWeakHeapObjectTag)));
}

Node* EffectControlLinearizer::BuildStrongReferenceFromWeakReference(Node* maybe_object)
{
    return __ BitcastWordToTagged(__ WordAnd(__ BitcastMaybeObjectToWord(maybe_object), __ IntPtrConstant(~kWeakHeapObjectMask)));
}

Node* EffectControlLinearizer::BuildIsWeakReferenceTo(Node* maybe_object, Node* value)
{
    if (COMPRESS_POINTERS_BOOL) {
        return __ Word32Equal(__ Word32And(TruncateWordToInt32(__ BitcastMaybeObjectToWord(maybe_object)), __ Uint32Constant(~static_cast<uint32_t>(kWeakHeapObjectMask))),
            TruncateWordToInt32(__ BitcastTaggedToWord(value)));
    } else {
        return __ WordEqual(__ WordAnd(__ BitcastMaybeObjectToWord(maybe_object), __ IntPtrConstant(~kWeakHeapObjectMask)), __ BitcastTaggedToWord(value));
    }
}

Node* EffectControlLinearizer::BuildIsClearedWeakReference(Node* maybe_object)
{
    return __ Word32Equal(TruncateWordToInt32(__ BitcastMaybeObjectToWord(maybe_object)), __ Int32Constant(kClearedWeakHeapObjectLower32));
}

// Pass {bitfield} = {digit} = nullptr to construct the canoncial 0n BigInt.
Node* EffectControlLinearizer::BuildAllocateBigInt(Node* bitfield, Node* digit)
{
    DCHECK(machine()->Is64());
    ...
    Node* result = __ Allocate(AllocationType::kYoung, __ IntPtrConstant(BigInt::SizeFor(digit ? 1 : 0)));
    __ StoreField(AccessBuilder::ForMap(), result, map);
    __ StoreField(AccessBuilder::ForBigIntBitfield(), result, bitfield ? bitfield : __ Int32Constant(zero_bitfield));

    // BigInts have no padding on 64 bit architectures with pointer compression.
    if (BigInt::HasOptionalPadding()) {
        __ StoreField(AccessBuilder::ForBigIntOptionalPadding(), result, __ IntPtrConstant(0));
    }
    if (digit) {
        __ StoreField(AccessBuilder::ForBigIntLeastSignificantDigit64(), result, digit);
    }
    return result;
}
...
void LinearizeEffectControl(JSGraph* graph, Schedule* schedule, Zone* temp_zone, SourcePositionTable* source_positions, NodeOriginTable* node_origins, JSHeapBroker* broker)
{
    JSGraphAssembler graph_assembler_(graph, temp_zone);
    EffectControlLinearizer linearizer(graph, schedule, &graph_assembler_, temp_zone, source_positions, node_origins, MaintainSchedule::kDiscard, broker);
    linearizer.Run();
}
