...
AllocationTraceNode* AllocationTraceTree::AddPathFromEnd(const base::Vector<unsigned>& path)
{
    AllocationTraceNode* node = root();
    for (unsigned* entry = path.begin() + path.length() - 1; entry != path.begin() - 1; --entry) {
...
void AddressToTraceMap::Clear() { ranges_.clear(); }
...
void AllocationTracker::AllocationEvent(Address addr, int size)
{
    ...
    while (!it.done() && length < kMaxAllocationTraceLength) {
        ...
        SnapshotObjectId id = ids_->FindOrAddEntry(shared.address(), shared.Size(), false);
...
static uint32_t SnapshotObjectIdHash(SnapshotObjectId id) { return ComputeUnseededHash(static_cast<uint32_t>(id)); }

unsigned AllocationTracker::AddFunctionInfo(SharedFunctionInfo shared, SnapshotObjectId id)
{
    ...
    if (entry->value == nullptr) {
        ...
        if (shared.script().IsScript()) {
            ...
            unresolved_locations_.push_back(new UnresolvedLocation(script, shared.StartPosition(), info));
        }
...
AllocationTracker::UnresolvedLocation::UnresolvedLocation(Script script, int start, FunctionInfo* info)
    : start_position_(start)
    , info_(info)
{
    script_ = script.GetIsolate()->global_handles()->Create(script);
    GlobalHandles::MakeWeak(script_.location(), this, &HandleWeakScript, v8::WeakCallbackType::kParameter);
}
...
void AllocationTracker::UnresolvedLocation::HandleWeakScript(const v8::WeakCallbackInfo<void>& data)
{
