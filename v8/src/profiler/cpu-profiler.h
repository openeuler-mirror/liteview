...
public:
    explicit CodeEventsContainer(CodeEventRecord::Type type = CodeEventRecord::Type::kNoEvent) { generic.type = type; }
    union {
...
private:
    ...
    void set_processor(ProfilerEventsProcessor* processor) { processor_ = processor; }
...
public:
    ...
    CpuProfiler(Isolate* isolate, CpuProfilingNamingMode naming_mode, CpuProfilingLoggingMode logging_mode, CpuProfilesCollection* profiles, Symbolizer* test_symbolizer,
        ProfilerEventsProcessor* test_processor, ProfilerCodeObserver* test_code_observer);
    ...
    CpuProfilingResult StartProfiling(String title, CpuProfilingOptions options = {}, std::unique_ptr<DiscardedSamplesDelegate> delegate = nullptr);
    ...
    ProfilerListener* profiler_listener_for_test() const { return profiler_listener_.get(); }
