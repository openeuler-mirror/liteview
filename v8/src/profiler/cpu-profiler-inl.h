...
void CodeCreateEventRecord::UpdateCodeMap(CodeMap* code_map) { code_map->AddCode(instruction_start, entry, instruction_size); }

void CodeMoveEventRecord::UpdateCodeMap(CodeMap* code_map) { code_map->MoveCode(from_instruction_start, to_instruction_start); }
...
void ReportBuiltinEventRecord::UpdateCodeMap(CodeMap* code_map)
{
    ...
    if (builtin == Builtin::kGenericJSToWasmWrapper) {
        ...
        entry = code_map->code_entries().Create(LogEventListener::CodeTag::kBuiltin, Builtins::name(builtin));
...
void SamplingEventsProcessor::FinishTickSample() { ticks_buffer_.FinishEnqueue(); }
