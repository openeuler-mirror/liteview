...
public:
    ...
    v8::AllocationProfile::Node* GetRootNode() override { return nodes_.size() == 0 ? nullptr : &nodes_.front(); }

    const std::vector<v8::AllocationProfile::Sample>& GetSamples() override { return samples_; }
...
public:
    ...
    public:
        ...
        AllocationNode* AddChildNode(FunctionId id, std::unique_ptr<AllocationNode> node) { return children_.emplace(id, std::move(node)).first->second.get(); }
