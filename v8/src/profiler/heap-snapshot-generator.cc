...
private:
    using UnorderedHeapObjectSet = std::unordered_set<HeapObject, Object::Hasher, Object::KeyEqualSafe>;
...
void HeapEntry::SetNamedReference(HeapGraphEdge::Type type, const char* name, HeapEntry* entry, HeapSnapshotGenerator* generator, ReferenceVerification verification)
{
    ++children_count_;
...
void HeapEntry::SetNamedAutoIndexReference(
    HeapGraphEdge::Type type, const char* description, HeapEntry* child, StringsStorage* names, HeapSnapshotGenerator* generator, ReferenceVerification verification)
{
...
void HeapSnapshot::RememberLastJSObjectId() { max_snapshot_js_object_id_ = profiler_->heap_object_map()->last_assigned_id(); }
...
void HeapSnapshot::AddLocation(HeapEntry* entry, int scriptId, int line, int col) { locations_.emplace_back(entry->index(), scriptId, line, col); }
...
bool HeapObjectsMap::MoveObject(Address from, Address to, int object_size)
{
    ...
    if (from_value == nullptr) {
        ...
        if (v8_flags.heap_profiler_trace_objects) {
            PrintF("Move object from %p to %p old size %6d new size %6d\n", reinterpret_cast<void*>(from), reinterpret_cast<void*>(to), entries_.at(from_entry_info_index).size,
                object_size);
        }
...
void HeapObjectsMap::UpdateObjectSize(Address addr, int size) { FindOrAddEntry(addr, size, false); }
...
SnapshotObjectId HeapObjectsMap::FindOrAddEntry(Address addr, unsigned int size, bool accessed)
{
    DCHECK(static_cast<uint32_t>(entries_.size()) > entries_map_.occupancy());
...
void HeapObjectsMap::UpdateHeapObjectsMap()
{
    ...
    heap_->PreciseCollectAllGarbage(Heap::kNoGCFlags, GarbageCollectionReason::kHeapProfiler);
...
HeapEntry* V8HeapExplorer::AllocateEntry(HeapThing ptr) { return AddEntry(HeapObject::cast(Object(reinterpret_cast<Address>(ptr)))); }
...
void V8HeapExplorer::ExtractLocationForJSFunction(HeapEntry* entry, JSFunction func)
{
...
HeapEntry* V8HeapExplorer::AddEntry(HeapObject object)
{
    ...
    if (InstanceTypeChecker::IsJSObject(instance_type)) {
        ...
        const char* name = names_->GetName(GetConstructorName(heap_->isolate(), JSObject::cast(object)));
        ...
        if (string.IsConsString(cage_base)) {
            ...
            return AddEntry(object, HeapEntry::kString, names_->GetName(String::cast(object)));
        }
    ...
    } else if (InstanceTypeChecker::IsCode(instance_type) || InstanceTypeChecker::IsCodeDataContainer(instance_type)) {
        ...
        return AddEntry(object, HeapEntry::kCode, name.IsString() ? names_->GetName(String::cast(name)) : "");
...
HeapEntry* V8HeapExplorer::AddEntry(HeapObject object, HeapEntry::Type type, const char* name)
{
...
HeapEntry::Type V8HeapExplorer::GetSystemEntryType(HeapObject object)
{
    ...
    if (InstanceTypeChecker::IsAllocationSite(type) || InstanceTypeChecker::IsArrayBoilerplateDescription(type) || InstanceTypeChecker::IsBytecodeArray(type)
        || InstanceTypeChecker::IsClosureFeedbackCellArray(type) || InstanceTypeChecker::IsCodeDataContainer(type) || InstanceTypeChecker::IsFeedbackCell(type)
        || InstanceTypeChecker::IsFeedbackMetadata(type) || InstanceTypeChecker::IsFeedbackVector(type) || InstanceTypeChecker::IsInterpreterData(type)
    ...
    // shapes.
    if ((InstanceTypeChecker::IsMap(type) && !BasicMemoryChunk::FromHeapObject(object)->InReadOnlySpace()) || InstanceTypeChecker::IsDescriptorArray(type)
...
public:
    IndexedReferencesExtractor(V8HeapExplorer* generator, HeapObject parent_obj, HeapEntry* parent)
    ...
    void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) override { VisitPointers(host, MaybeObjectSlot(start), MaybeObjectSlot(end)); }
    void VisitMapPointer(HeapObject object) override { VisitSlotImpl(cage_base(), object.map_slot()); }
...
private:
    template <typename TSlot> V8_INLINE void VisitSlotImpl(PtrComprCageBase cage_base, TSlot slot)
    {
        int field_index = static_cast<int>(MaybeObjectSlot(slot.address()) - parent_start_);
#ifdef V8_TARGET_BIG_ENDIAN
...
void V8HeapExplorer::ExtractReferences(HeapEntry* entry, HeapObject obj)
{
    if (obj.IsJSGlobalProxy()) {
        ...
        ExtractArrayBoilerplateDescriptionReferences(entry, ArrayBoilerplateDescription::cast(obj));
    } else if (obj.IsRegExpBoilerplateDescription()) {
        ExtractRegExpBoilerplateDescriptionReferences(entry, RegExpBoilerplateDescription::cast(obj));
        ...
        ExtractWeakArrayReferences(WeakFixedArray::kHeaderSize, entry, WeakFixedArray::cast(obj));
    } else if (obj.IsWeakArrayList()) {
        ExtractWeakArrayReferences(WeakArrayList::kHeaderSize, entry, WeakArrayList::cast(obj));
...
void V8HeapExplorer::ExtractJSGlobalProxyReferences(HeapEntry* entry, JSGlobalProxy proxy)
{
    SetInternalReference(entry, "native_context", proxy.native_context(), JSGlobalProxy::kNativeContextOffset);
}

void V8HeapExplorer::ExtractJSObjectReferences(HeapEntry* entry, JSObject js_obj)
{
    ...
    if (obj.IsJSBoundFunction()) {
        ...
        SetInternalReference(entry, "native_context", global_obj.native_context(), JSGlobalObject::kNativeContextOffset);
        SetInternalReference(entry, "global_proxy", global_obj.global_proxy(), JSGlobalObject::kGlobalProxyOffset);
        static_assert(JSGlobalObject::kHeaderSize - JSObject::kHeaderSize == 2 * kTaggedSize);
...
void V8HeapExplorer::ExtractStringReferences(HeapEntry* entry, String string)
{
    if (string.IsConsString()) {
        ...
        SetInternalReference(entry, "second", cs.second(), ConsString::kSecondOffset);
    } else if (string.IsSlicedString()) {
        SlicedString ss = SlicedString::cast(string);
        SetInternalReference(entry, "parent", ss.parent(), SlicedString::kParentOffset);
    } else if (string.IsThinString()) {
        ThinString ts = ThinString::cast(string);
        SetInternalReference(entry, "actual", ts.actual(), ThinString::kActualOffset);
    }
}

void V8HeapExplorer::ExtractSymbolReferences(HeapEntry* entry, Symbol symbol) { SetInternalReference(entry, "name", symbol.description(), Symbol::kDescriptionOffset); }

void V8HeapExplorer::ExtractJSCollectionReferences(HeapEntry* entry, JSCollection collection)
{
    SetInternalReference(entry, "table", collection.table(), JSCollection::kTableOffset);
}

void V8HeapExplorer::ExtractJSWeakCollectionReferences(HeapEntry* entry, JSWeakCollection obj)
{
    SetInternalReference(entry, "table", obj.table(), JSWeakCollection::kTableOffset);
}

void V8HeapExplorer::ExtractEphemeronHashTableReferences(HeapEntry* entry, EphemeronHashTable table)
{
    for (InternalIndex i : table.IterateEntries()) {
        ...
        if (key_entry && value_entry && !key.IsUndefined()) {
            const char* edge_name = names_->GetFormatted("part of key (%s @%u) -> value (%s @%u) pair in WeakMap (table @%u)", key_entry->name(), key_entry->id(),
                value_entry->name(), value_entry->id(), table_entry->id());
...
void V8HeapExplorer::ExtractContextReferences(HeapEntry* entry, Context context)
{
    ...
    if (context.IsNativeContext()) {
        ...
        SetWeakReference(
            entry, "optimized_code_list", context.get(Context::OPTIMIZED_CODE_LIST), Context::OffsetOfElementAt(Context::OPTIMIZED_CODE_LIST), HeapEntry::kCustomWeakPointer);
        SetWeakReference(
            entry, "deoptimized_code_list", context.get(Context::DEOPTIMIZED_CODE_LIST), Context::OffsetOfElementAt(Context::DEOPTIMIZED_CODE_LIST), HeapEntry::kCustomWeakPointer);
...
void V8HeapExplorer::ExtractSharedFunctionInfoReferences(HeapEntry* entry, SharedFunctionInfo shared)
{
    ...
    HeapObject maybe_code_obj = V8_REMOVE_BUILTINS_CODE_OBJECTS && code.is_off_heap_trampoline() ? HeapObject::cast(code) : FromCodeT(code);
    if (name[0] != '\0') {
        TagObject(maybe_code_obj, names_->GetFormatted("(code for %s)", name.get()));
    } else {
        TagObject(maybe_code_obj, names_->GetFormatted("(%s code)", CodeKindToString(code.kind())));
    }
    ...
    SetInternalReference(entry, "script_or_debug_info", shared.script_or_debug_info(kAcquireLoad), SharedFunctionInfo::kScriptOrDebugInfoOffset);
    SetInternalReference(entry, "function_data", shared.function_data(kAcquireLoad), SharedFunctionInfo::kFunctionDataOffset);
    SetInternalReference(
        entry, "raw_outer_scope_info_or_feedback_metadata", shared.raw_outer_scope_info_or_feedback_metadata(), SharedFunctionInfo::kOuterScopeInfoOrFeedbackMetadataOffset);
}
...
void V8HeapExplorer::ExtractScriptReferences(HeapEntry* entry, Script script)
{
    ...
    TagObject(script.shared_function_infos(), "(shared function infos)", HeapEntry::kCode);
    TagObject(script.host_defined_options(), "(host-defined options)", HeapEntry::kCode);
}

void V8HeapExplorer::ExtractAccessorInfoReferences(HeapEntry* entry, AccessorInfo accessor_info)
{
...
void V8HeapExplorer::ExtractAccessorPairReferences(HeapEntry* entry, AccessorPair accessors)
{
...
void V8HeapExplorer::ExtractJSWeakRefReferences(HeapEntry* entry, JSWeakRef js_weak_ref) { SetWeakReference(entry, "target", js_weak_ref.target(), JSWeakRef::kTargetOffset); }

void V8HeapExplorer::ExtractWeakCellReferences(HeapEntry* entry, WeakCell weak_cell)
{
...
void V8HeapExplorer::ExtractCodeReferences(HeapEntry* entry, Code code)
{
    ...
    SetInternalReference(entry, "relocation_info", code.relocation_info(), Code::kRelocationInfoOffset);
    ...
    if (code.kind() == CodeKind::BASELINE) {
        ...
        DeoptimizationData deoptimization_data = DeoptimizationData::cast(code.deoptimization_data());
        ...
        if (deoptimization_data.length() > 0) {
            TagObject(deoptimization_data.TranslationByteArray(), "(code deopt data)", HeapEntry::kCode);
...
void V8HeapExplorer::ExtractCellReferences(HeapEntry* entry, Cell cell) { SetInternalReference(entry, "value", cell.value(), Cell::kValueOffset); }

void V8HeapExplorer::ExtractFeedbackCellReferences(HeapEntry* entry, FeedbackCell feedback_cell)
{
    TagObject(feedback_cell, "(feedback cell)");
    SetInternalReference(entry, "value", feedback_cell.value(), FeedbackCell::kValueOffset);
}

void V8HeapExplorer::ExtractPropertyCellReferences(HeapEntry* entry, PropertyCell cell)
{
...
void V8HeapExplorer::ExtractPrototypeInfoReferences(HeapEntry* entry, PrototypeInfo info)
{
...
void V8HeapExplorer::ExtractAllocationSiteReferences(HeapEntry* entry, AllocationSite site)
{
...
void V8HeapExplorer::ExtractArrayBoilerplateDescriptionReferences(HeapEntry* entry, ArrayBoilerplateDescription value)
{
...
void V8HeapExplorer::ExtractRegExpBoilerplateDescriptionReferences(HeapEntry* entry, RegExpBoilerplateDescription value)
{
...
public:
    ...
    HeapEntry* AllocateEntry(HeapThing ptr) override { return explorer_->AddEntry(reinterpret_cast<Address>(ptr), HeapEntry::kNative, "system / JSArrayBufferData", size_); }
...
void V8HeapExplorer::ExtractJSArrayBufferReferences(HeapEntry* entry, JSArrayBuffer buffer)
{
...
void V8HeapExplorer::ExtractJSPromiseReferences(HeapEntry* entry, JSPromise promise)
{
    SetInternalReference(entry, "reactions_or_result", promise.reactions_or_result(), JSPromise::kReactionsOrResultOffset);
}

void V8HeapExplorer::ExtractJSGeneratorObjectReferences(HeapEntry* entry, JSGeneratorObject generator)
{
...
void V8HeapExplorer::ExtractFixedArrayReferences(HeapEntry* entry, FixedArray array)
{
...
void V8HeapExplorer::ExtractBytecodeArrayReferences(HeapEntry* entry, BytecodeArray bytecode)
{
    ...
    TagObject(bytecode.source_position_table(kAcquireLoad), "(source position table)", HeapEntry::kCode);
}

void V8HeapExplorer::ExtractScopeInfoReferences(HeapEntry* entry, ScopeInfo info)
{
...
void V8HeapExplorer::ExtractFeedbackVectorReferences(HeapEntry* entry, FeedbackVector feedback_vector)
{
    ...
    for (int i = 0; i < feedback_vector.length(); ++i) {
        ...
        HeapObject entry;
        if (maybe_entry.GetHeapObjectIfStrong(&entry) && (entry.map(isolate()).instance_type() == WEAK_FIXED_ARRAY_TYPE || entry.IsFixedArrayExact())) {
            TagObject(entry, "(feedback)", HeapEntry::kCode);
        }
    }
}

void V8HeapExplorer::ExtractDescriptorArrayReferences(HeapEntry* entry, DescriptorArray array)
{
...
void V8HeapExplorer::ExtractEnumCacheReferences(HeapEntry* entry, EnumCache cache)
{
    TagObject(cache.keys(), "(enum cache)", HeapEntry::kObjectShape);
    TagObject(cache.indices(), "(enum cache)", HeapEntry::kObjectShape);
}

void V8HeapExplorer::ExtractTransitionArrayReferences(HeapEntry* entry, TransitionArray transitions)
{
...
template <typename T> void V8HeapExplorer::ExtractWeakArrayReferences(int header_size, HeapEntry* entry, T array)
{
...
void V8HeapExplorer::ExtractPropertyReferences(JSObject js_obj, HeapEntry* entry)
{
    ...
    if (js_obj.HasFastProperties()) {
        ...
        GlobalDictionary dictionary = JSGlobalObject::cast(js_obj).global_dictionary(kAcquireLoad);
        ReadOnlyRoots roots(isolate);
        ...
        for (InternalIndex i : dictionary.IterateEntries()) {
            ...
            SetDataOrAccessorPropertyReference(details.kind(), entry, Name::cast(k), value);
        }
    } else {
...
void V8HeapExplorer::ExtractAccessorPairProperty(HeapEntry* entry, Name key, Object callback_obj, int field_offset)
{
...
void V8HeapExplorer::ExtractElementReferences(JSObject js_obj, HeapEntry* entry)
{
    ...
    if (js_obj.HasObjectElements()) {
        ...
        int length = js_obj.IsJSArray() ? Smi::ToInt(JSArray::cast(js_obj).length()) : elements.length();
...
JSFunction V8HeapExplorer::GetConstructor(Isolate* isolate, JSReceiver receiver)
{
    DisallowGarbageCollection no_gc;
    HandleScope scope(isolate);
...
public:
    ...
    void VisitRootPointer(Root root, const char* description, FullObjectSlot object) override
    {
                ...
                if (deopt_data.length() > 0) {
                    ...
                    for (int i = 0; i < literals_length; ++i) {
                        ...
                        if (maybe_literal.GetHeapObject(&heap_literal)) {
                            VisitRootPointer(Root::kStackRoots, nullptr, FullObjectSlot(&heap_literal));
                        }
                    }
                }
            }
...
bool V8HeapExplorer::IterateAndExtractReferences(HeapSnapshotGenerator* generator)
{
    ...
    for (HeapObject obj = iterator.Next(); !obj.is_null(); obj = iterator.Next(), progress_->ProgressStep()) {
        if (interrupted)
            continue;
        ...
        if (v8_flags.heap_snapshot_verify && !BasicMemoryChunk::FromHeapObject(obj)->InReadOnlySpace()) {
...
bool V8HeapExplorer::IsEssentialObject(Object object)
{
    ...
    if (V8_EXTERNAL_CODE_SPACE_BOOL && IsCodeSpaceObject(HeapObject::cast(object))) {
        return true;
    }
    Isolate* isolate = heap_->isolate();
    ReadOnlyRoots roots(isolate);
    return !object.IsOddball(isolate) && object != roots.empty_byte_array() && object != roots.empty_fixed_array() && object != roots.empty_weak_fixed_array()
        && object != roots.empty_descriptor_array() && object != roots.fixed_array_map() && object != roots.cell_map() && object != roots.global_property_cell_map()
        && object != roots.shared_function_info_map() && object != roots.free_space_map() && object != roots.one_pointer_filler_map() && object != roots.two_pointer_filler_map();
}

bool V8HeapExplorer::IsEssentialHiddenReference(Object parent, int field_offset)
{
    ...
    if (parent.IsContext() && field_offset == Context::OffsetOfElementAt(Context::NEXT_CONTEXT_LINK))
        return false;
    if (parent.IsJSFinalizationRegistry() && field_offset == JSFinalizationRegistry::kNextDirtyOffset)
        return false;
    return true;
}

void V8HeapExplorer::SetContextReference(HeapEntry* parent_entry, String reference_name, Object child_obj, int field_offset)
{
...
void V8HeapExplorer::SetNativeBindReference(HeapEntry* parent_entry, const char* reference_name, Object child_obj)
{
    HeapEntry* child_entry = GetEntry(child_obj);
    if (child_entry == nullptr)
        return;
...
void V8HeapExplorer::SetElementReference(HeapEntry* parent_entry, int index, Object child_obj)
{
    HeapEntry* child_entry = GetEntry(child_obj);
    if (child_entry == nullptr)
        return;
...
void V8HeapExplorer::SetInternalReference(HeapEntry* parent_entry, const char* reference_name, Object child_obj, int field_offset)
{
...
void V8HeapExplorer::SetInternalReference(HeapEntry* parent_entry, int index, Object child_obj, int field_offset)
{
    if (!IsEssentialObject(child_obj)) {
        return;
    }
...
void V8HeapExplorer::SetHiddenReference(HeapObject parent_obj, HeapEntry* parent_entry, int index, Object child_obj, int field_offset)
{
...
void V8HeapExplorer::SetWeakReference(HeapEntry* parent_entry, const char* reference_name, Object child_obj, int field_offset, HeapEntry::ReferenceVerification verification)
{
    if (!IsEssentialObject(child_obj)) {
        return;
    }
...
void V8HeapExplorer::SetWeakReference(HeapEntry* parent_entry, int index, Object child_obj, base::Optional<int> field_offset)
{
    if (!IsEssentialObject(child_obj)) {
        return;
    }
...
void V8HeapExplorer::SetDataOrAccessorPropertyReference(
    PropertyKind kind, HeapEntry* parent_entry, Name reference_name, Object child_obj, const char* name_format_string, int field_offset)
{
...
void V8HeapExplorer::SetPropertyReference(HeapEntry* parent_entry, Name reference_name, Object child_obj, const char* name_format_string, int field_offset)
{
    HeapEntry* child_entry = GetEntry(child_obj);
    if (child_entry == nullptr)
        return;
    HeapGraphEdge::Type type = reference_name.IsSymbol() || String::cast(reference_name).length() > 0 ? HeapGraphEdge::kProperty : HeapGraphEdge::kInternal;
    const char* name = name_format_string != nullptr && reference_name.IsString()
        ? names_->GetFormatted(name_format_string, String::cast(reference_name).ToCString(DISALLOW_NULLS, ROBUST_STRING_TRAVERSAL).get())
...
void V8HeapExplorer::SetRootGcRootsReference() { snapshot_->root()->SetIndexedAutoIndexReference(HeapGraphEdge::kElement, snapshot_->gc_roots(), generator_); }
...
void V8HeapExplorer::SetGcRootsReference(Root root) { snapshot_->gc_roots()->SetIndexedAutoIndexReference(HeapGraphEdge::kElement, snapshot_->gc_subroot(root), generator_); }

void V8HeapExplorer::SetGcSubrootReference(Root root, const char* description, bool is_weak, Object child_obj)
{
    if (child_obj.IsSmi()) {
...
void V8HeapExplorer::TagObject(Object obj, const char* tag, base::Optional<HeapEntry::Type> type)
{
...
void V8HeapExplorer::RecursivelyTagConstantPool(Object obj, const char* tag, HeapEntry::Type type, int recursion_limit)
{
    --recursion_limit;
    ...
    } else if (obj.IsNameDictionary(isolate()) || obj.IsNumberDictionary(isolate())) {
...
public:
    explicit GlobalObjectsEnumerator(Isolate* isolate)
        : isolate_(isolate)
    {
    }

    void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override { VisitRootPointersImpl(root, description, start, end); }

    void VisitRootPointers(Root root, const char* description, OffHeapObjectSlot start, OffHeapObjectSlot end) override { VisitRootPointersImpl(root, description, start, end); }
...
private:
    template <typename TSlot> void VisitRootPointersImpl(Root root, const char* description, TSlot start, TSlot end)
    {
        for (TSlot p = start; p < end; ++p) {
            ...
            if (!o.IsNativeContext(isolate_))
                continue;
            JSObject proxy = Context::cast(o).global_proxy();
            if (!proxy.IsJSGlobalProxy(isolate_))
                continue;
            Object global = proxy.map(isolate_).prototype(isolate_);
            if (!global.IsJSGlobalObject(isolate_))
                continue;
...
void V8HeapExplorer::CollectGlobalObjectsTags()
{
    ...
    for (int i = 0, l = enumerator.count(); i < l; ++i) {
        ...
        const char* tag = global_object_name_resolver_->GetName(Utils::ToLocal(Handle<JSObject>::cast(obj)));
        if (tag) {
...
void V8HeapExplorer::MakeGlobalObjectTagMap(const SafepointScope& safepoint_scope)
{
...
public:
    ...
    void AddEdge(Node* from, Node* to, const char* name) final { edges_.push_back({ from, to, name }); }
...
HeapEntry::Type EmbedderGraphNodeType(EmbedderGraphImpl::Node* node) { return node->IsRootNode() ? HeapEntry::kSynthetic : HeapEntry::kNative; }
...
HeapEntry* EmbedderGraphEntriesAllocator::AllocateEntry(HeapThing ptr)
{
    ...
    SnapshotObjectId id = (lookup_address) ? heap_object_map_->FindOrAddEntry(lookup_address, 0) : static_cast<SnapshotObjectId>(reinterpret_cast<uintptr_t>(node) << 1);
...
HeapEntry* NativeObjectsExplorer::EntryForEmbedderGraphNode(EmbedderGraphImpl::Node* node)
{
    ...
    // Node is V8NodeImpl.
    Object object = static_cast<EmbedderGraphImpl::V8NodeImpl*>(node)->GetObject();
    if (object.IsSmi())
        return nullptr;
    auto* entry = generator_->FindEntry(reinterpret_cast<void*>(Object::cast(object).ptr()));
    return entry;
}
...
HeapSnapshotGenerator::HeapSnapshotGenerator(HeapSnapshot* snapshot, v8::ActivityControl* control, v8::HeapProfiler::ObjectNameResolver* resolver, Heap* heap)
    : snapshot_(snapshot)
    , control_(control)
...
bool HeapSnapshotGenerator::FillReferences() { return v8_heap_explorer_.IterateAndExtractReferences(this) && dom_explorer_.IterateAndExtractReferences(this); }
...
template <typename T> static int utoa_impl(T value, const base::Vector<char>& buffer, int buffer_pos)
{
...
template <typename T> static int utoa(T value, const base::Vector<char>& buffer, int buffer_pos)
{
...
void HeapSnapshotJSONSerializer::SerializeNode(const HeapEntry* entry)
{
    ...
    static const int kBufferSize
        = 5 * MaxDecimalDigitsIn<sizeof(unsigned)>::kUnsigned + MaxDecimalDigitsIn<sizeof(size_t)>::kUnsigned + MaxDecimalDigitsIn<sizeof(uint8_t)>::kUnsigned + 7 + 1 + 1;
    base::EmbeddedVector<char, kBufferSize> buffer;
    int buffer_pos = 0;
...
#define JSON_S(s) "\"" s "\""
    writer_
        ->AddString(
            JSON_O(
                JSON_S("node_fields") ":" JSON_A(JSON_S("type") "," JSON_S("name") "," JSON_S("id") "," JSON_S("self_size") "," JSON_S("edge_count") "," JSON_S("trace_node_id") "," JSON_S("detachedness")) "," JSON_S("node_types") ":" JSON_A(JSON_A(JSON_S("hidden") "," JSON_S("array") "," JSON_S("string") "," JSON_S("object") "," JSON_S("code") "," JSON_S("closure") "," JSON_S("regexp") "," JSON_S("number") "," JSON_S("native") "," JSON_S("synthetic") "," JSON_S("concatenated string") "," JSON_S("sliced string") "," JSON_S("symbol") "," JSON_S("bigint") "," JSON_S("object shape")) "," JSON_S("string") "," JSON_S("number") "," JSON_S("number") "," JSON_S("number") "," JSON_S("number") "," JSON_S("number")) "," JSON_S("edge_fields") ":" JSON_A(JSON_S("type") "," JSON_S("name_or_index") "," JSON_S("to_node")) "," JSON_S("edge_types") ":" JSON_A(
                    JSON_A(
                        JSON_S("context") "," JSON_S("element") "," JSON_S("property") "," JSON_S("internal") "," JSON_S("hidden") "," JSON_S(
                            "shortcut") "," JSON_S("weak")) "," JSON_S("string_or_number") "," JSON_S("node")) "," JSON_S("trace_function_info_fields") ":" JSON_A(JSON_S("function"
                                                                                                                                                                          "_id") "," JSON_S("name") "," JSON_S("script_name") "," JSON_S("script_id") "," JSON_S("line") "," JSON_S("column")) "," JSON_S("trace_node_fields") ":" JSON_A(JSON_S("id") "," JSON_S("function_info_index") "," JSON_S("count") "," JSON_S("size") "," JSON_S("children")) "," JSON_S("sample_fields") ":" JSON_A(JSON_S("timestamp_us") "," JSON_S("last_assigned_id")) "," JSON_S("location_fields") ":" JSON_A(JSON_S("object_index") "," JSON_S("script_id") "," JSON_S("line") "," JSON_S("column"))));
// clang-format on
#undef JSON_S
#undef JSON_O
#undef JSON_A
...
static int SerializePosition(int position, const base::Vector<char>& buffer, int buffer_pos)
{
    if (position == -1) {
        buffer[buffer_pos++] = '0';
    } else {
...
void HeapSnapshotJSONSerializer::SerializeSamples()
{
    ...
    const int kBufferSize = MaxDecimalDigitsIn<sizeof(base::TimeDelta().InMicroseconds())>::kUnsigned + MaxDecimalDigitsIn<sizeof(samples[0].id)>::kUnsigned + 2 + 1 + 1;
    base::EmbeddedVector<char, kBufferSize> buffer;
    int i = 0;
...
void HeapSnapshotJSONSerializer::SerializeLocation(const SourceLocation& location)
{
