...
bool StringsStorage::StringsMatch(void* key1, void* key2) { return strcmp(reinterpret_cast<char*>(key1), reinterpret_cast<char*>(key2)) == 0; }
...
const char* StringsStorage::GetSymbol(Symbol sym)
{
    ...
    int length = std::min(v8_flags.heap_snapshot_string_limit.value(), description.length());
    auto data = description.ToCString(DISALLOW_NULLS, ROBUST_STRING_TRAVERSAL, 0, length, &length);
...
const char* StringsStorage::GetName(Name name)
{
    if (name.IsString()) {
        ...
        int length = std::min(v8_flags.heap_snapshot_string_limit.value(), str.length());
        int actual_length = 0;
        std::unique_ptr<char[]> data = str.ToCString(DISALLOW_NULLS, ROBUST_STRING_TRAVERSAL, 0, length, &actual_length);
...
const char* StringsStorage::GetName(int index) { return GetFormatted("%d", index); }
...
{
    uint32_t raw_hash_field = StringHasher::HashSequentialString(str, len, kZeroHashSeed);
...
size_t StringsStorage::GetStringCountForTesting() const { return names_.occupancy(); }
