...
void Decoder::PrintDRegister(int reg) { Print(RegisterName(DoubleRegister::from_code(reg))); }

// Print the Simd128 register name according to the active name converter.
void Decoder::PrintVectorRegister(int reg) { Print(RegisterName(Simd128Register::from_code(reg))); }
...
void Decoder::DecodeExtP(Instruction* instr)
{
    ...
    case PLOAD_STORE_MLS: {
        ...
        Instruction* next_instr = base::bit_cast<Instruction*>(base::bit_cast<intptr_t>(instr) + kInstrSize);
