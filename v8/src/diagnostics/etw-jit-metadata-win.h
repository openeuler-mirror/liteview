...
template <size_t count, typename idx = std::make_index_sequence<count>> constexpr auto MakeStrBytes(char const (&s)[count]) { return str_bytes<count> { s, idx {} }; }
...
template <typename T1> constexpr auto JoinFields(T1 field) { return field; }
...
inline ULONG LogEvent(uint64_t regHandle, const EVENT_DESCRIPTOR* event_descriptor, EVENT_DATA_DESCRIPTOR* data_descriptor, ULONG desc_count)
{
    ...
    return EventWriteTransferXp(regHandle, event_descriptor, NULL /* ActivityId */, NULL /* RelatedActivityId */, desc_count, data_descriptor);
}

// This template is called by the provider implementation
template <typename T, typename... Fs> void LogEventData(const TraceLoggingHProvider provider, const EVENT_DESCRIPTOR* event_descriptor, T* meta, const Fs&... fields)
{
