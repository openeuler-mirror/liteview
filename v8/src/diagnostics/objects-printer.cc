...
void PrintHeapObjectHeaderWithoutMap(HeapObject object, std::ostream& os, const char* id)
{
...
template <typename T> void PrintDictionaryContents(std::ostream& os, T dict)
{
...
void FreeSpace::FreeSpacePrint(std::ostream& os) { os << "free space, size " << Size() << "\n"; }
...
bool JSObject::PrintProperties(std::ostream& os)
{
    if (HasFastProperties()) {
        ...
        PrintDictionaryContents(os, JSGlobalObject::cast(*this).global_dictionary(kAcquireLoad));
...
template <class T> bool IsTheHoleAt(T array, int index) { return false; }

template <> bool IsTheHoleAt(FixedDoubleArray array, int index) { return array.is_the_hole(index); }

template <class T> double GetScalarElement(T array, int index)
{
...
template <class T> void DoPrintElements(std::ostream& os, Object object, int length)
{
...
template <typename T> void PrintFixedArrayElements(std::ostream& os, T array)
{
...
void PrintSloppyArgumentElements(std::ostream& os, ElementsKind kind, SloppyArgumentsElements elements)
{
...
void PrintEmbedderData(Isolate* isolate, std::ostream& os, EmbedderDataSlot slot)
{
    DisallowGarbageCollection no_gc;
...
#define PRINT_ELEMENTS(Type, type, TYPE, elementType)                                                                                                                              \
    case TYPE##_ELEMENTS: {                                                                                                                                                        \
        ...
        const elementType* data_ptr = static_cast<const elementType*>(JSTypedArray::cast(*this).DataPtr());                                                                        \
...
#undef PRINT_ELEMENTS
    ...
    case SLOW_SLOPPY_ARGUMENTS_ELEMENTS:
        PrintSloppyArgumentElements(os, map().elements_kind(), SloppyArgumentsElements::cast(elements()));
        break;
...
static void JSObjectPrintHeader(std::ostream& os, JSObject obj, const char* id)
{
...
static void JSObjectPrintBody(std::ostream& os, JSObject obj, bool print_elements = true)
{
    os << "\n - properties: ";
    ...
    if (print_elements) {
        size_t length = obj.IsJSTypedArray() ? JSTypedArray::cast(obj).GetLength() : obj.elements().length();
        if (length > 0)
...
void JSGeneratorObject::JSGeneratorObjectPrint(std::ostream& os)
{
    ...
    if (is_suspended()) {
        ...
        if (fun_info.HasSourceCode()) {
            ...
            String script_name = script.name().IsString() ? String::cast(script.name()) : GetReadOnlyRoots().empty_string();
...
void DescriptorArray::DescriptorArrayPrint(std::ostream& os)
{
    ...
    os << "\n - raw marked descriptors: mc epoch " << NumberOfMarkedDescriptors::Epoch::decode(raw_marked) << ", marked " << NumberOfMarkedDescriptors::Marked::decode(raw_marked);
    PrintDescriptors(os);
}

namespace {
void PrintFixedArrayWithHeader(std::ostream& os, FixedArray array, const char* type)
{
...
void ObjectBoilerplateDescription::ObjectBoilerplateDescriptionPrint(std::ostream& os) { PrintFixedArrayWithHeader(os, *this, "ObjectBoilerplateDescription"); }
...
void FixedArray::FixedArrayPrint(std::ostream& os) { PrintFixedArrayWithHeader(os, *this, "FixedArray"); }
...
void AccessorInfo::AccessorInfoPrint(std::ostream& os)
{
    ...
    if (USE_SIMULATOR_BOOL) {
        os << "\n - maybe_redirected_getter: " << reinterpret_cast<void*>(maybe_redirected_getter());
    }
...
namespace {
void PrintContextWithHeader(std::ostream& os, Context context, const char* type)
{
...
void Context::ContextPrint(std::ostream& os) { PrintContextWithHeader(os, *this, "Context"); }
...
void print_hex_byte(std::ostream& os, int value) { os << "0x" << std::setfill('0') << std::setw(2) << std::right << std::hex << (value & 0xff) << std::setfill(' '); }
...
void ClosureFeedbackCellArray::ClosureFeedbackCellArrayPrint(std::ostream& os) { PrintFixedArrayWithHeader(os, *this, "ClosureFeedbackCellArray"); }
...
void FeedbackNexus::Print(std::ostream& os)
{
    ...
    case FeedbackSlotKind::kLoadProperty: {
        ...
        if (ic_state() == InlineCacheState::MONOMORPHIC) {
            ...
            WeakFixedArray array = WeakFixedArray::cast(GetFeedback().GetHeapObject());
...
void JSAsyncFunctionObject::JSAsyncFunctionObjectPrint(std::ostream& os) { JSGeneratorObjectPrint(os); }

void JSAsyncGeneratorObject::JSAsyncGeneratorObjectPrint(std::ostream& os) { JSGeneratorObjectPrint(os); }

void JSArgumentsObject::JSArgumentsObjectPrint(std::ostream& os) { JSObjectPrint(os); }
...
void JSDate::JSDatePrint(std::ostream& os)
{
    ...
    if (!year().IsSmi()) {
        ...
        SNPrintF(buf, "\n - time = %s %04d/%02d/%02d %02d:%02d:%02d\n", weekdays[weekday().IsSmi() ? Smi::ToInt(weekday()) + 1 : 0], year().IsSmi() ? Smi::ToInt(year()) : -1,
            month().IsSmi() ? Smi::ToInt(month()) : -1, day().IsSmi() ? Smi::ToInt(day()) : -1, hour().IsSmi() ? Smi::ToInt(hour()) : -1, min().IsSmi() ? Smi::ToInt(min()) : -1,
...
void JSSetIterator::JSSetIteratorPrint(std::ostream& os) { JSCollectionIteratorPrint(os, "JSSetIterator"); }

void JSMapIterator::JSMapIteratorPrint(std::ostream& os) { JSCollectionIteratorPrint(os, "JSMapIterator"); }
...
void JSFunction::JSFunctionPrint(std::ostream& os)
{
    ...
    os << "\n - formal_parameter_count: " << shared().internal_formal_parameter_count_without_receiver();
    os << "\n - kind: " << shared().kind();
    os << "\n - context: " << Brief(context());
    os << "\n - code: " << Brief(code());
...
void SharedFunctionInfo::PrintSourceCode(std::ostream& os)
{
    if (HasSourceCode()) {
        ...
        std::unique_ptr<char[]> source_string = source.ToCString(DISALLOW_NULLS, FAST_STRING_TRAVERSAL, start, length, nullptr);
...
void SharedFunctionInfo::SharedFunctionInfoPrint(std::ostream& os)
{
    ...
    os << "\n - formal_parameter_count: " << internal_formal_parameter_count_without_receiver();
...
{
    PrintHeader(os, "Code");
    os << "\n - code_data_container: " << Brief(code_data_container(kAcquireLoad));
    if (is_builtin()) {
...
void CodeDataContainer::CodeDataContainerPrint(std::ostream& os)
{
    ...
    os << "\n - code: " << Brief(raw_code());
    os << "\n - code_entry_point: " << reinterpret_cast<void*>(code_entry_point());
...
void WasmStruct::WasmStructPrint(std::ostream& os)
{
    ...
    for (uint32_t i = 0; i < struct_type->field_count(); i++) {
        ...
        case wasm::kRtt: {
            ...
            Address obj = V8HeapCompressionScheme::DecompressTaggedPointer(address(), raw);
#else
            Address obj = raw;
#endif
            os << Brief(Object(obj));
...
#define PRINT_OPTIONAL_WASM_INSTANCE_FIELD(name, convert)                                                                                                                          \
    ...
    PRINT_WASM_INSTANCE_FIELD(isorecursive_canonical_types, reinterpret_cast<const uint32_t*>);
...
void WasmIndirectFunctionTable::WasmIndirectFunctionTablePrint(std::ostream& os)
{
    ...
    if (has_managed_native_allocations()) {
        os << "\n - managed_native_allocations: " << Brief(managed_native_allocations());
    }
    os << "\n - refs: " << Brief(refs());
    os << "\n";
}
...
void CallHandlerInfo::CallHandlerInfoPrint(std::ostream& os)
{
    ...
    if (USE_SIMULATOR_BOOL) {
        os << "\n - maybe_redirected_callback: " << reinterpret_cast<void*>(maybe_redirected_callback());
    }
    os << "\n - data: " << Brief(data());
    os << "\n - side_effect_free: " << (IsSideEffectFreeCallHandlerInfo() ? "true" : "false");
    os << "\n";
}
...
void Script::ScriptPrint(std::ostream& os)
{
    ...
    if (!is_wasm) {
        if (has_eval_from_shared()) {
            ...
            os << "\n - shared function info table: " << Brief(shared_function_info_table());
        }
...
namespace {
void PrintScopeInfoList(ScopeInfo scope_info, std::ostream& os, const char* list_name, int length)
{
    DisallowGarbageCollection no_gc;
    if (length <= 0)
        return;
...
void HeapNumber::HeapNumberShortPrint(std::ostream& os)
{
    ...
    double val = value();
    if (val == DoubleToInteger(val) && val >= static_cast<double>(kMinSafeInteger) && val <= static_cast<double>(kMaxSafeInteger)) {
...
int Name::NameShortPrint(base::Vector<char> str)
{
    if (this->IsString()) {
        ...
        if (s.description().IsUndefined()) {
            ...
            return SNPrintF(str, "<%s>", String::cast(s.description()).ToCString().get());
        }
    }
}
...
// static
void TransitionsAccessor::PrintOneTransition(std::ostream& os, Name key, Map target)
{
    os << "\n     ";
#ifdef OBJECT_PRINT
    key.NamePrint(os);
...
void TransitionsAccessor::PrintTransitionTree(std::ostream& os, int level, DisallowGarbageCollection* no_gc)
{
    ...
    for (int i = 0; i < num_transitions; i++) {
        Name key = GetKey(i);
        Map target = GetTarget(i);
        os << std::endl << "  " << level << "/" << i << ":" << std::setw(level * 2 + 2) << " ";
        std::stringstream ss;
        ss << Brief(target);
...
inline i::Object GetObjectFromRaw(void* object)
{
    ...
    if (RoundDown<i::kPtrComprCageBaseAlignment>(object_ptr) == i::kNullAddress) {
        ...
        object_ptr = i::V8HeapCompressionScheme::DecompressTaggedAny(isolate, static_cast<i::Tagged_t>(object_ptr));
    }
#endif
...
V8_EXPORT_PRIVATE extern i::Object _v8_internal_Get_Object(void* object) { return GetObjectFromRaw(object); }

V8_DONT_STRIP_SYMBOL
V8_EXPORT_PRIVATE extern void _v8_internal_Print_Object(void* object) { GetObjectFromRaw(object).Print(); }
...
V8_EXPORT_PRIVATE extern void _v8_internal_Print_Code(void* object)
{
    ...
    i::CodeLookupResult lookup_result = isolate->heap()->GcSafeFindCodeForInnerPointerForPrinting(address);
    if (!lookup_result.IsFound()) {
        i::PrintF("%p is not within the current isolate's code, read_only or embedded "
                  "spaces\n",
            object);
        return;
    }
