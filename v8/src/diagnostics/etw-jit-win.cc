...
private:
    ...
    bool HasPendingLog() const { return pending_log_task_id_ != CancelableTaskManager::kInvalidTaskId; }
    void RemovePendingLog() { pending_log_task_id_ = CancelableTaskManager::kInvalidTaskId; }
    ...
    void EnqueueEnableLog()
    {
        ...
        auto taskrunner = V8::GetCurrentPlatform()->GetForegroundTaskRunner(v8_isolate);
    ...
    void EnqueueDisableLog()
    {
        ...
        auto v8_isolate = reinterpret_cast<v8::Isolate*>(isolate_);
        Isolate* isolate = isolate_;
        auto task = MakeCancelableTask(isolate_, [isolate] { DisableLog(isolate); });
        pending_log_task_id_ = task->id();
        auto taskrunner = V8::GetCurrentPlatform()->GetForegroundTaskRunner(v8_isolate);
        taskrunner->PostTask(std::move(task));
    }

    bool IsScriptLoaded(int script_id) const { return loaded_scripts_ids_.find(script_id) != loaded_scripts_ids_.end(); }
    ...
    CancelableTaskManager::Id pending_log_task_id_ = CancelableTaskManager::kInvalidTaskId;
};
...
IsolateLoadScriptData& IsolateLoadScriptData::GetData(Isolate* isolate) { return isolate_map.Pointer()->at(isolate); }
...
void IsolateLoadScriptData::EnableLog(Isolate* isolate)
{
    ...
    isolate->v8_file_logger()->SetEtwCodeEventHandler(has_pending_log ? kJitCodeEventEnumExisting : kJitCodeEventDefault);
}
...
{
    base::MutexGuard guard(isolates_mutex.Pointer());
    std::for_each(isolate_map.Pointer()->begin(), isolate_map.Pointer()->end(), [etw_enabled](auto& pair) {
...
void EnableETWLog(Isolate* isolate) { IsolateLoadScriptData::EnableLog(isolate); }

// TODO(v8/11911): UnboundScript::GetLineNumber should be replaced
SharedFunctionInfo GetSharedFunctionInfo(const JitCodeEvent* event) { return event->script.IsEmpty() ? SharedFunctionInfo() : *Utils::OpenHandle(*event->script); }

int GetScriptLineNumber(const JitCodeEvent* event)
{
    auto sfi = GetSharedFunctionInfo(event);
    return sfi.is_null() ? -1 // invalid sentinel number
                         : Script::cast(sfi.script()).GetLineNumber(sfi.StartPosition()) + 1;
}
...
int GetScriptColumnNumber(const JitCodeEvent* event)
{
    auto sfi = GetSharedFunctionInfo(event);
    return sfi.is_null() ? -1 // invalid sentinel number
                         : Script::cast(sfi.script()).GetColumnNumber(sfi.StartPosition()) + 1;
}
...
std::wstring GetScriptMethodNameFromSharedFunctionInfo(const SharedFunctionInfo& sfi)
{
    ...
    PEVENT_FILTER_DESCRIPTOR /* filter_data */, PVOID /* callback_context */)
{
    ...
    bool is_etw_enabled_now
        = is_enabled && level >= kTraceLevel && (match_any_keyword & kJScriptRuntimeKeyword) && ((match_all_keyword & kJScriptRuntimeKeyword) == match_all_keyword);

    UpdateETWEnabled(is_etw_enabled_now);
}

inline ULONG EventWriteTransferXp(REGHANDLE RegHandle,
...
{
    typedef ULONG(WINAPI* PFN_EventWriteTransfer)(REGHANDLE RegHandle,
        PCEVENT_DESCRIPTOR EventDescriptor,
        LPCGUID ActivityId,
        LPCGUID RelatedActivityId,
        ULONG UserDataCount,
        PEVENT_DATA_DESCRIPTOR UserData);
    static PFN_EventWriteTransfer s_EventWriteTransfer = (PFN_EventWriteTransfer)-1;

    if ((PFN_EventWriteTransfer)-1 == s_EventWriteTransfer) {
      HMODULE handle = LoadLibraryW(L"advapi32.dll");
      s_EventWriteTransfer = (PFN_EventWriteTransfer)(GetProcAddress(handle, "EventWriteTransfer"));
    }
    if (s_EventWriteTransfer)
        return s_EventWriteTransfer(RegHandle, EventDescriptor, ActivityId, RelatedActivityId, UserDataCount, UserData);
...
void AddIsolate(Isolate* isolate) { IsolateLoadScriptData::AddIsolate(isolate); }

void RemoveIsolate(Isolate* isolate) { IsolateLoadScriptData::RemoveIsolate(isolate); }
...
void EventHandler(const JitCodeEvent* event)
{
    ...
    if (!script.IsEmpty()) {
        ...
        if (IsolateLoadScriptData::MaybeAddLoadedScript(isolate, script_id)) {
            ...
            constexpr static auto source_load_event_fields = EventFields(
                "SourceLoad", Field("SourceID", TlgInUINT64), Field("ScriptContextID", TlgInPOINTER), Field("SourceFlags", TlgInUINT32), Field("Url", TlgInUNICODESTRING));
    ...
    constexpr static auto method_load_event_fields = EventFields("MethodLoad", Field("ScriptContextID", TlgInPOINTER), Field("MethodStartAddress", TlgInPOINTER),
        Field("MethodSize", TlgInUINT64), Field("MethodID", TlgInUINT32), Field("MethodFlags", TlgInUINT16), Field("MethodAddressRangeID", TlgInUINT16),
        Field("SourceID", TlgInUINT64), Field("Line", TlgInUINT32), Field("Column", TlgInUINT32), Field("MethodName", TlgInUNICODESTRING));
    LogEventData(g_v8Provider, &method_load_event_meta, &method_load_event_fields, script_context, event->code_start, (uint64_t)event->code_len,
        ...
        (uint64_t)script_id, (uint32_t)GetScriptLineNumber(event), (uint32_t)GetScriptColumnNumber(event), method_name);
}
