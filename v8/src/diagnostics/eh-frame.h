...
public:
    ...
    void IncreaseBaseAddressOffset(int base_delta) { SetBaseAddressOffset(base_offset_ + base_delta); }
    ...
    void RecordRegisterSavedToStack(Register name, int offset) { RecordRegisterSavedToStack(RegisterToDwarfCode(name), offset); }
...
private:
    ...
    void WriteOpcode(EhFrameConstants::DwarfOpcodes opcode) { WriteByte(static_cast<byte>(opcode)); }
    void WriteBytes(const byte* start, int size) { eh_frame_buffer_.insert(eh_frame_buffer_.end(), start, start + size); }
    void WriteInt16(uint16_t value) { WriteBytes(reinterpret_cast<const byte*>(&value), sizeof(value)); }
    void WriteInt32(uint32_t value) { WriteBytes(reinterpret_cast<const byte*>(&value), sizeof(value)); }
    ...
    int GetProcedureAddressOffset() const { return fde_offset() + EhFrameConstants::kProcedureAddressOffsetInFde; }

    int GetProcedureSizeOffset() const { return fde_offset() + EhFrameConstants::kProcedureSizeOffsetInFde; }

    int eh_frame_offset() const { return static_cast<int>(eh_frame_buffer_.size()); }
...
public:
    ...
    EhFrameConstants::DwarfOpcodes GetNextOpcode() { return static_cast<EhFrameConstants::DwarfOpcodes>(GetNextByte()); }
    ...
    const void* current_address() const { return reinterpret_cast<const void*>(next_); }
...
private:
    static void DumpDwarfDirectives(std::ostream& stream, const byte* start, const byte* end);
