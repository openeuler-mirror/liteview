...
public:
    ...
    public:
        ...
        void set(const T& value) { base::WriteUnalignedValue(w_->AddressAt<T>(offset_), value); }
    ...
    template <typename T> Slot<T> CreateSlotHere() { return CreateSlotsHere<T>(1); }
...
public:
    enum Type { S_REGULAR = 0x0u, S_ATTR_COALESCED = 0xBu, S_ATTR_SOME_INSTRUCTIONS = 0x400u, S_ATTR_DEBUG = 0x02000000u, S_ATTR_PURE_INSTRUCTIONS = 0x80000000u };
...
private:
    ...
    void WriteHeader(Writer* w)
    {
        ...
        const uint8_t ident[16] = { 0x7F, 'E', 'L', 'F', 1, 2, 1, 3, 0, 0, 0, 0, 0, 0, 0, 0 };
#else
#error Unsupported target architecture.
#endif
...
#error Unsupported target architecture.
    ...
    int SectionHeaderPosition(uint32_t section_index) { return sizeof(ELFHeader) + sizeof(ELFSection::Header) * section_index; }
...
public:
    void SetPosition(intptr_t pc, int pos, bool is_statement) { AddPCInfo(PCInfo(pc, pos, is_statement)); }
...
public:
    ...
    CodeDescription(const char* name, base::AddressRegion region, SharedFunctionInfo shared, LineInfo* lineinfo, bool is_function)
        : name_(name)
        , shared_info_(shared)
        , lineinfo_(lineinfo)
    ...
    bool has_script() { return !shared_info_.is_null() && shared_info_.script().IsScript(); }
...
static JITCodeEntry* CreateELFObject(CodeDescription* desc, Isolate* isolate)
{
    ...
    size_t text_section_index = elf.AddSection(
        zone.New<FullHeaderELFSection>(".text", ELFSection::TYPE_NOBITS, kCodeAlignment, desc->CodeStart(), 0, desc->CodeSize(), ELFSection::FLAG_ALLOC | ELFSection::FLAG_EXEC));
...
static base::Optional<std::pair<CodeMap::iterator, CodeMap::iterator>> GetOverlappingRegions(CodeMap* map, const base::AddressRegion region)
{
...
static void AddJITCodeEntry(CodeMap* map, const base::AddressRegion region, JITCodeEntry* entry, bool dump_if_enabled, const char* name_hint)
{
    ...
    if (v8_flags.gdbjit_dump && dump_if_enabled) {
        ...
        WriteBytes(file_name, reinterpret_cast<byte*>(entry->symfile_addr_), static_cast<int>(entry->symfile_size_));
    }
#endif
...
static void AddCode(const char* name, base::AddressRegion region, SharedFunctionInfo shared, LineInfo* lineinfo, Isolate* isolate, bool is_function)
{
...
void EventHandler(const v8::JitCodeEvent* event)
{
    ...
    case v8::JitCodeEvent::CODE_ADDED: {
        ...
        SharedFunctionInfo shared = event->script.IsEmpty() ? SharedFunctionInfo() : *Utils::OpenHandle(*event->script);
        ...
        if (event->code_type == v8::JitCodeEvent::JIT_CODE) {
            CodeLookupResult lookup_result = isolate->heap()->GcSafeFindCodeForInnerPointer(addr);
    ...
    case v8::JitCodeEvent::CODE_ADD_LINE_POS_INFO: {
        ...
        line_info->SetPosition(
            static_cast<intptr_t>(event->line_info.offset), static_cast<int>(event->line_info.pos), event->line_info.position_type == v8::JitCodeEvent::STATEMENT_POSITION);
        break;
    }
