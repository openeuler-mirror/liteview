...
#define USE_TORQUE_VERIFIER(Class)                                                                                                                                                 \
    void Class::Class##Verify(Isolate* isolate) { TorqueGeneratedClassVerifiers::Class##Verify(*this, isolate); }
...
void TaggedIndex::TaggedIndexVerify(Isolate* isolate) { CHECK(IsTaggedIndex()); }
...
#undef STRING_TYPE_CASE
    ...
    case OBJECT_BOILERPLATE_DESCRIPTION_TYPE:
        ObjectBoilerplateDescription::cast(*this).ObjectBoilerplateDescriptionVerify(isolate);
        break;
    // FixedArray types
...
void Map::MapVerify(Isolate* isolate)
{
    ...
    if (IsContextMap()) {
        ...
        // null.
        CHECK(native_context_or_null().IsNull() || native_context().IsNativeContext());
    } else {
    ...
    if (IsJSObjectMap()) {
        ...
        if (IsJSSharedStructMap() || IsJSSharedArrayMap() || IsJSAtomicsMutex() || IsJSAtomicsCondition()) {
            CHECK(InSharedHeap());
            CHECK(GetBackPointer().IsUndefined(isolate));
            Object maybe_cell = prototype_validity_cell(kRelaxedLoad);
            if (maybe_cell.IsCell())
                CHECK(maybe_cell.InSharedHeap());
            CHECK(!is_extensible());
    ...
    if (is_prototype_map()) {
        DCHECK(prototype_info() == Smi::zero() || prototype_info().IsPrototypeInfo());
    }
}
    ...
    if (number_of_all_descriptors() == 0) {
        ...
        for (InternalIndex descriptor : InternalIndex::Range(number_of_descriptors())) {
            ...
            if (details.location() == PropertyLocation::kField) {
                ...
                CHECK(value == MaybeObject::FromObject(FieldType::None()) || value == MaybeObject::FromObject(FieldType::Any()) || value->IsCleared()
...
namespace {
void SloppyArgumentsElementsVerify(Isolate* isolate, SloppyArgumentsElements elements, JSObject holder)
{
...
void JSArgumentsObject::JSArgumentsObjectVerify(Isolate* isolate)
{
    ...
    if (IsSloppyArgumentsElementsKind(GetElementsKind())) {
        SloppyArgumentsElementsVerify(isolate, SloppyArgumentsElements::cast(elements()), *this);
    }
    if (isolate->IsInAnyContext(map(), Context::SLOPPY_ARGUMENTS_MAP_INDEX) || isolate->IsInAnyContext(map(), Context::SLOW_ALIASED_ARGUMENTS_MAP_INDEX)
        || isolate->IsInAnyContext(map(), Context::FAST_ALIASED_ARGUMENTS_MAP_INDEX)) {
    ...
    } else if (isolate->IsInAnyContext(map(), Context::STRICT_ARGUMENTS_MAP_INDEX)) {
        VerifyObjectField(isolate, JSStrictArgumentsObject::kLengthOffset);
    }
}

void JSAsyncFunctionObject::JSAsyncFunctionObjectVerify(Isolate* isolate) { TorqueGeneratedClassVerifiers::JSAsyncFunctionObjectVerify(*this, isolate); }

void JSAsyncGeneratorObject::JSAsyncGeneratorObjectVerify(Isolate* isolate) { TorqueGeneratedClassVerifiers::JSAsyncGeneratorObjectVerify(*this, isolate); }
...
void ConsString::ConsStringVerify(Isolate* isolate)
{
    ...
    if (IsFlat(isolate)) {
        ...
        CHECK(first().IsSeqString() || first().IsExternalString() || first().IsThinString());
    }
}
...
void SharedFunctionInfo::SharedFunctionInfoVerify(LocalIsolate* isolate) { this->SharedFunctionInfoVerify(ReadOnlyRoots(isolate)); }
...
void SharedFunctionInfo::SharedFunctionInfoVerify(ReadOnlyRoots roots)
{
...

    {
        ...
        CHECK(script.IsUndefined(roots) || script.IsScript() || script.IsDebugInfo());
    }

    if (!is_compiled()) {
        CHECK(!HasFeedbackMetadata());
        CHECK(outer_scope_info().IsScopeInfo() || outer_scope_info().IsTheHole(roots));
    ...
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        if (raw_code() != Smi::zero()) {
            ...
            if (V8_REMOVE_BUILTINS_CODE_OBJECTS) {
                ...
                CHECK_IMPLIES(isolate->embedded_blob_code() && is_off_heap_trampoline(), builtin_id() == Builtin::kInterpreterEntryTrampoline);
            }
            ...
            if (V8_SHORT_BUILTIN_CALLS_BOOL) {
                if (code.InstructionStart() == code_entry_point()) {
                    ...
                    // CodeDataContainer.
                    CodeLookupResult lookup_result = isolate->heap()->GcSafeFindCodeForInnerPointer(code_entry_point());
...
{
    CHECK(IsAligned(InstructionSize(), static_cast<unsigned>(Code::kMetadataAlignment)));
    ...
    CHECK_IMPLIES(!ReadOnlyHeap::Contains(*this), IsAligned(InstructionStart(), kCodeAlignment));
#endif // !defined(_MSC_VER) || defined(__clang__)
    CHECK_IMPLIES(!ReadOnlyHeap::Contains(*this), IsAligned(raw_instruction_start(), kCodeAlignment));
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
    ...
    CHECK(V8_ENABLE_THIRD_PARTY_HEAP_BOOL || CodeSize() <= MemoryChunkLayout::MaxRegularCodeObjectSize() || isolate->heap()->InSpace(*this, CODE_LO_SPACE));
...
void JSArray::JSArrayVerify(Isolate* isolate)
{
    ...
    if (length().IsSmi() && (HasFastElements() || HasAnyNonextensibleElements())) {
        if (elements().length() > 0) {
            CHECK_IMPLIES(HasDoubleElements(), elements().IsFixedDoubleArray());
            CHECK_IMPLIES(HasSmiOrObjectElements() || HasAnyNonextensibleElements(), elements().IsFixedArray());
        }
...
void WeakCell::WeakCellVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(unregister_token().IsUndefined(isolate), key_list_prev().IsUndefined(isolate));
    CHECK_IMPLIES(unregister_token().IsUndefined(isolate), key_list_next().IsUndefined(isolate));
    ...
    CHECK(finalization_registry().IsUndefined(isolate) || finalization_registry().IsJSFinalizationRegistry());
}
...
void SwissNameDictionary::SwissNameDictionaryVerify(Isolate* isolate) { this->SwissNameDictionaryVerify(isolate, false); }
...
void JSRegExp::JSRegExpVerify(Isolate* isolate)
{
    ...
    case JSRegExp::EXPERIMENTAL: {
        ...
        if (is_compiled) {
            CHECK_EQ(CodeT::cast(latin1_code).builtin_id(), Builtin::kRegExpExperimentalTrampoline);
        ...
        CHECK_EQ(arr.get(JSRegExp::kIrregexpMaxRegisterCountIndex), uninitialized);
        CHECK(arr.get(JSRegExp::kIrregexpCaptureCountIndex).IsSmi());
        CHECK_GE(Smi::ToInt(arr.get(JSRegExp::kIrregexpCaptureCountIndex)), 0);
        CHECK_EQ(arr.get(JSRegExp::kIrregexpTicksUntilTierUpIndex), uninitialized);
    ...
    case JSRegExp::IRREGEXP: {
        ...
        CHECK((one_byte_data.IsSmi() && Smi::ToInt(one_byte_data) == JSRegExp::kUninitializedValue) || one_byte_data.IsCodeT());
        Object uc16_data = arr.get(JSRegExp::kIrregexpUC16CodeIndex);
        CHECK((uc16_data.IsSmi() && Smi::ToInt(uc16_data) == JSRegExp::kUninitializedValue) || uc16_data.IsCodeT());

        Object one_byte_bytecode = arr.get(JSRegExp::kIrregexpLatin1BytecodeIndex);
        // Smi : Not compiled yet (-1).
        // ByteArray: Bytecode to interpret regexp.
        CHECK((one_byte_bytecode.IsSmi() && Smi::ToInt(one_byte_bytecode) == JSRegExp::kUninitializedValue) || (can_be_interpreted && one_byte_bytecode.IsByteArray()));
        Object uc16_bytecode = arr.get(JSRegExp::kIrregexpUC16BytecodeIndex);
        CHECK((uc16_bytecode.IsSmi() && Smi::ToInt(uc16_bytecode) == JSRegExp::kUninitializedValue) || (can_be_interpreted && uc16_bytecode.IsByteArray()));
...
void JSDataView::JSDataViewVerify(Isolate* isolate)
{
    ...
    if (!WasDetached()) {
        CHECK_EQ(reinterpret_cast<uint8_t*>(JSArrayBuffer::cast(buffer()).backing_store()) + byte_offset(), data_pointer());
    }
}
...
void SourceTextModuleInfoEntry::SourceTextModuleInfoEntryVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(export_name().IsString() && import_name().IsString(), local_name().IsUndefined(isolate));
}
...
void Module::ModuleVerify(Isolate* isolate)
{
    ...
    if (!(status() == kErrored || status() == kEvaluating || status() == kEvaluated)) {
...
void ModuleRequest::ModuleRequestVerify(Isolate* isolate)
{
    ...
    CHECK_EQ(0, import_assertions().length() % ModuleRequest::kAssertionEntrySize);

    for (int i = 0; i < import_assertions().length(); i += ModuleRequest::kAssertionEntrySize) {
...
void ObjectBoilerplateDescription::ObjectBoilerplateDescriptionVerify(Isolate* isolate)
{
    ...
    CHECK_GE(this->length(), ObjectBoilerplateDescription::kDescriptionStartIndex);
...
void WasmInstanceObject::WasmInstanceObjectVerify(Isolate* isolate)
{
    ...
    for (int offset = kHeaderSize; offset < kEndOfStrongFieldsOffset; offset += kTaggedSize) {
...
void WasmExportedFunctionData::WasmExportedFunctionDataVerify(Isolate* isolate)
{
    ...
    CHECK(wrapper_code().kind() == CodeKind::JS_TO_WASM_FUNCTION || wrapper_code().kind() == CodeKind::C_WASM_ENTRY
        || (wrapper_code().is_builtin()
            && (wrapper_code().builtin_id() == Builtin::kGenericJSToWasmWrapper || wrapper_code().builtin_id() == Builtin::kWasmReturnPromiseOnSuspend)));
}
...
void DataHandler::DataHandlerVerify(Isolate* isolate)
{
    ...
    CHECK_IMPLIES(!smi_handler().IsSmi(), IsStoreHandler() && smi_handler().IsCodeT());
...
void CallHandlerInfo::CallHandlerInfoVerify(Isolate* isolate)
{
    ...
    CHECK(map() == ReadOnlyRoots(isolate).side_effect_call_handler_info_map() || map() == ReadOnlyRoots(isolate).side_effect_free_call_handler_info_map()
        || map() == ReadOnlyRoots(isolate).next_call_side_effect_free_call_handler_info_map());
}
...
void AllocationSite::AllocationSiteVerify(Isolate* isolate)
{
    ...
    CHECK(transition_info_or_boilerplate().IsSmi() || transition_info_or_boilerplate().IsJSObject());
...
void FunctionTemplateRareData::FunctionTemplateRareDataVerify(Isolate* isolate) { CHECK(c_function_overloads().IsFixedArray() || c_function_overloads().IsUndefined(isolate)); }

void StackFrameInfo::StackFrameInfoVerify(Isolate* isolate) { TorqueGeneratedClassVerifiers::StackFrameInfoVerify(*this, isolate); }

void ErrorStackData::ErrorStackDataVerify(Isolate* isolate)
{
    TorqueGeneratedClassVerifiers::ErrorStackDataVerify(*this, isolate);
    CHECK_IMPLIES(!call_site_infos_or_formatted_stack().IsFixedArray(), limit_or_stack_frame_infos().IsFixedArray());
}
...
void JSObject::IncrementSpillStatistics(Isolate* isolate, SpillInformation* info)
{
    ...
    if (HasFastProperties()) {
        ...
        GlobalDictionary dict = JSGlobalObject::cast(*this).global_dictionary(kAcquireLoad);
...
void JSObject::SpillInformation::Print()
{
    ...
    PrintF("    - fast properties (#%d): %d (used) %d (unused)\n", number_of_objects_with_fast_properties_, number_of_fast_used_fields_, number_of_fast_unused_fields_);

    PrintF("    - slow properties (#%d): %d (used) %d (unused)\n", number_of_objects_ - number_of_objects_with_fast_properties_, number_of_slow_used_properties_,
        number_of_slow_unused_properties_);

    PrintF("    - fast elements (#%d): %d (used) %d (unused)\n", number_of_objects_with_fast_elements_, number_of_fast_used_elements_, number_of_fast_unused_elements_);
...
static bool CheckOneBackPointer(Map current_map, Object target) { return !target.IsMap() || Map::cast(target).GetBackPointer() == current_map; }
