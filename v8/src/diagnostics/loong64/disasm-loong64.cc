...
void Decoder::PrintPCOffs16(Instruction* instr)
{
    ...
    out_buffer_pos_ += base::SNPrintF(out_buffer_ + out_buffer_pos_, "%s", converter_.NameOfAddress(reinterpret_cast<byte*>(instr) + target));
}
...
const char* NameConverter::NameOfByteCPURegister(int reg) const { UNREACHABLE(); }
