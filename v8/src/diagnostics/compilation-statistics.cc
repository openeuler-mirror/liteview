...
#define MEMBER(s) QUOTE(s) << ":"
    ...
    stream << DICT(MEMBER("function_name") << QUOTE(function_name_) << "," MEMBER("total_allocated_bytes") << total_allocated_bytes_ << "," MEMBER("max_allocated_bytes")
                                           << max_allocated_bytes_ << "," MEMBER("absolute_max_allocated_bytes") << absolute_max_allocated_bytes_);
...
static void WriteLine(std::ostream& os, bool machine_format, const char* name, const CompilationStatistics::BasicStats& stats, const CompilationStatistics::BasicStats& total_stats)
{
    ...
    if (machine_format) {
        base::OS::SNPrintF(buffer, kBufferSize, "\"%s_time\"=%.3f\n\"%s_space\"=%zu", name, ms, name, stats.total_allocated_bytes_);
        os << buffer;
    } else {
        base::OS::SNPrintF(buffer, kBufferSize, "%34s %10.3f (%5.1f%%)  %10zu (%5.1f%%) %10zu %10zu", name, ms, percent, stats.total_allocated_bytes_, size_percent,
    ...
    for (const auto& phase_kind_it : sorted_phase_kinds) {
        ...
        if (!ps.machine_output) {
            for (const auto& phase_it : sorted_phases) {
                ...
                WriteLine(os, ps.machine_output, phase_name.c_str(), phase_stats, s.total_stats_);
            }
            WritePhaseKindBreak(os);
        }
        const auto& phase_kind_stats = phase_kind_it->second;
        WriteLine(os, ps.machine_output, phase_kind_name.c_str(), phase_kind_stats, s.total_stats_);
        os << std::endl;
    }
