...
void Decoder::PrintRegister(int reg) { Print(converter_.NameOfCPURegister(reg)); }

void Decoder::PrintVRegister(int reg) { Print(v8::internal::VRegisters::Name(reg)); }
...
void Decoder::PrintFPURegister(int freg) { Print(converter_.NameOfXMMRegister(freg)); }
...
void Decoder::PrintTarget(Instruction* instr)
{
    if (Assembler::IsJalr(instr->InstructionBits())) {
        if (Assembler::IsAuipc((instr - 4)->InstructionBits()) && (instr - 4)->RdValue() == instr->Rs1Value()) {
            ...
            const char* target = converter_.NameOfAddress(reinterpret_cast<byte*>(instr - 4) + imm);
...
const char* NameConverter::NameOfCPURegister(int reg) const { return v8::internal::Registers::Name(reg); }

const char* NameConverter::NameOfXMMRegister(int reg) const { return v8::internal::FPURegisters::Name(reg); }
...
int Disassembler::ConstantPoolSizeAt(byte* instruction) { return v8::internal::Assembler::ConstantPoolSizeAt(reinterpret_cast<v8::internal::Instruction*>(instruction)); }
