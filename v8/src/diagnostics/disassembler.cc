...
const char* V8NameConverter::NameOfAddress(byte* pc) const
{
    if (!code_.is_null()) {
        ...
        if (auto* wasm_code = wasm::GetWasmCodeManager()->LookupCode(reinterpret_cast<Address>(pc))) {
...
static void PrintRelocInfo(std::ostringstream& out, Isolate* isolate, const ExternalReferenceEncoder* ref_encoder, std::ostream& os, CodeReference host, RelocInfo* relocinfo,
    bool first_reloc_info = true)
{
    ...
    if (rmode == RelocInfo::DEOPT_SCRIPT_OFFSET) {
        ...
        const char* reference_name = ref_encoder ? ref_encoder->NameOfAddress(isolate, address) : ExternalReferenceTable::NameOfIsolateIndependentAddress(address);
        out << "    ;; external reference (" << reference_name << ")";
    } else if (RelocInfo::IsCodeTargetMode(rmode)) {
        out << "    ;; code:";
        CodeT code = isolate->heap()->GcSafeFindCodeForInnerPointer(relocinfo->target_address()).ToCodeT();
        ...
        const char* runtime_stub_name = GetRuntimeStubName(host.as_wasm_code()->native_module()->GetRuntimeStubId(relocinfo->wasm_stub_call_address()));
...
static int DecodeIt(
    Isolate* isolate, ExternalReferenceEncoder* ref_encoder, std::ostream& os, CodeReference code, const V8NameConverter& converter, byte* begin, byte* end, Address current_pc)
{
    CHECK(!code.is_null());
    ...
    while (pc < end) {
        ...
        if (decoding_constant_pool) {
            ...
            if (num_const >= 0) {
                ...
                constants = num_const;
                pc += 4;
            } else if (!rit.done() && rit.rinfo()->pc() == reinterpret_cast<Address>(pc)
                && (rit.rinfo()->rmode() == RelocInfo::INTERNAL_REFERENCE || rit.rinfo()->rmode() == RelocInfo::LITERAL_CONSTANT)) {
                // raw pointer embedded in code stream, e.g., jump table
                byte* ptr = base::ReadUnalignedValue<byte*>(reinterpret_cast<Address>(pc));
                if (RelocInfo::IsInternalReference(rit.rinfo()->rmode())) {
                    ...
                    const char* kType = RelocInfo::IsLiteralConstant(rit.rinfo()->rmode()) ? "    literal constant" : "embedded data object";
                    SNPrintF(decode_buffer, "%08" V8PRIxPTR "       %s 0x%08" V8PRIxPTR, reinterpret_cast<intptr_t>(ptr), kType, reinterpret_cast<intptr_t>(ptr));
                }
                pc += sizeof(ptr);
            } else {
                decode_buffer[0] = '\0';
        ...
        for (size_t i = 0; i < pcs.size(); i++) {
            ...
            RelocInfo relocinfo(pcs[i], rmodes[i], datas[i], code_pointer, constant_pool);
        ...
        if (pcs.empty() && !code.is_null() && !decoding_constant_pool) {
            RelocInfo dummy_rinfo(reinterpret_cast<Address>(prev_pc), RelocInfo::NO_INFO, 0, Code());
...
int Disassembler::Decode(Isolate* isolate, std::ostream& os, byte* begin, byte* end, CodeReference code, Address current_pc)
{
    ...
    if (isolate) {
        ...
        return DecodeIt(isolate, &ref_encoder, os, code, v8NameConverter, begin, end, current_pc);
    } else {
        // No isolate => isolate-independent code. Only V8 External references
        // available.
        return DecodeIt(nullptr, nullptr, os, code, v8NameConverter, begin, end, current_pc);
    }
}

#else // ENABLE_DISASSEMBLER

int Disassembler::Decode(Isolate* isolate, std::ostream& os, byte* begin, byte* end, CodeReference code, Address current_pc) { return 0; }
