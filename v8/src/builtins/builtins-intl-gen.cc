...
public:
    ...
    TNode<IntPtrT> PointerToSeqStringData(TNode<String> seq_string)
    {
        ...
        return IntPtrAdd(BitcastTaggedToWord(seq_string), IntPtrConstant(SeqOneByteString::kHeaderSize - kHeapObjectTag));
    }
};
...
TF_BUILTIN(StringToLowerCaseIntl, IntlBuiltinsAssembler)
{
    ...
    CSA_DCHECK(this, Word32BinaryNot(IsIndirectStringInstanceType(instance_type)));
    ...
    BIND(&runtime);
    {
        const TNode<Object> result = CallRuntime(Runtime::kStringToLowerCaseIntl, NoContextConstant(), string);
        Return(result);
    }
}
...
{
    return CodeStubAssembler::AllocateJSArray(PACKED_ELEMENTS, LoadJSArrayElementsMap(PACKED_ELEMENTS, LoadNativeContext(context)), IntPtrConstant(0), SmiConstant(0));
}

TF_BUILTIN(ListFormatPrototypeFormat, IntlBuiltinsAssembler)
{
    ListFormatCommon(
        Parameter<Context>(Descriptor::kContext), UncheckedParameter<Int32T>(Descriptor::kJSActualArgumentsCount), Runtime::kFormatList, "Intl.ListFormat.prototype.format");
}
