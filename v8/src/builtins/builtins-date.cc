...
Object SetLocalDateValue(Isolate* isolate, Handle<JSDate> date, double time_val)
{
...
BUILTIN(DateUTC)
{
    ...
    double const day = MakeDay(year, month, date);
    double const time = MakeTime(hours, minutes, seconds, ms);
    return *isolate->factory()->NewNumber(DateCache::TimeClip(MakeDate(day, time)));
}
...
BUILTIN(DatePrototypeToDateString)
{
    ...
    DateBuffer buffer = ToDateString(date->value().Number(), isolate->date_cache(), ToDateStringMode::kLocalDate);
    RETURN_RESULT_OR_FAILURE(isolate, isolate->factory()->NewStringFromUtf8(base::VectorOf(buffer)));
}
...
BUILTIN(DatePrototypeToString)
{
    ...
    DateBuffer buffer = ToDateString(date->value().Number(), isolate->date_cache(), ToDateStringMode::kLocalDateAndTime);
    RETURN_RESULT_OR_FAILURE(isolate, isolate->factory()->NewStringFromUtf8(base::VectorOf(buffer)));
}
...
BUILTIN(DatePrototypeToTimeString)
{
    ...
    DateBuffer buffer = ToDateString(date->value().Number(), isolate->date_cache(), ToDateStringMode::kLocalTime);
    RETURN_RESULT_OR_FAILURE(isolate, isolate->factory()->NewStringFromUtf8(base::VectorOf(buffer)));
}
...
BUILTIN(DatePrototypeToUTCString)
{
    ...
    DateBuffer buffer = ToDateString(date->value().Number(), isolate->date_cache(), ToDateStringMode::kUTCDateAndTime);
    RETURN_RESULT_OR_FAILURE(isolate, isolate->factory()->NewStringFromUtf8(base::VectorOf(buffer)));
}
...
BUILTIN(DatePrototypeToTemporalInstant)
{
    ...
    Handle<BigInt> t;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, t, BigInt::FromNumber(isolate, Handle<Object>(date->value(), isolate)));
