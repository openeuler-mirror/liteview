    ...
    TNode<NativeContext> caller_context, TNode<NativeContext> eval_context, TNode<String> specifier, TNode<String> export_name)
{
    ...
    const TNode<Map> function_map = CAST(LoadContextElement(caller_context, Context::STRICT_FUNCTION_WITHOUT_PROTOTYPE_MAP_INDEX));
    const TNode<SharedFunctionInfo> info = ShadowRealmImportValueFulfilledSFIConstant();

    return AllocateFunctionWithMapAndContext(function_map, info, function_context);
}
...
TF_BUILTIN(CallWrappedFunction, ShadowRealmBuiltinsAssembler)
{
    ...
    BuildFastLoop<IntPtrT>(
        ...
        [&](TNode<IntPtrT> index) {
            ...
            // Create wrapped value in the target realm.
            TNode<Object> wrapped_value = CallBuiltin(Builtin::kShadowRealmGetWrappedValue, caller_context, target_context, caller_context, args.AtIndex(index));
    ...
    TNode<Object> result;
    {
        ...
        result = CallStub(callable, target_context, target, args_count, argc, wrapped_args);
    }
    ...
    ThrowTypeError(context, MessageTemplate::kCallShadowRealmFunctionThrown, var_exception.value());
...
TNode<Object> ShadowRealmBuiltinsAssembler::ImportValue(TNode<NativeContext> caller_context, TNode<NativeContext> eval_context, TNode<String> specifier, TNode<String> export_name)
{
