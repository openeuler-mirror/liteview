...
TNode<Object> IteratorBuiltinsAssembler::GetIteratorMethod(TNode<Context> context, TNode<Object> object) { return GetProperty(context, object, factory()->iterator_symbol()); }
...
TNode<JSReceiver> IteratorBuiltinsAssembler::IteratorStep(
    TNode<Context> context, const IteratorRecord& iterator, Label* if_done, base::Optional<TNode<Map>> fast_iterator_result_map)
{
    ...
    if (fast_iterator_result_map) {
        ...
        TNode<Object> done = LoadObjectField(heap_object_result, JSIteratorResult::kDoneOffset);
    ...
    // Generic iterator result case:
    {
        ...
        // IteratorComplete
        // 2. Return ToBoolean(? Get(iterResult, "done")).
        TNode<Object> done = GetProperty(context, heap_object_result, factory()->done_string());
        BranchIfToBooleanIsTrue(done, if_done, &return_result);
    }
...
TNode<Object> IteratorBuiltinsAssembler::IteratorValue(TNode<Context> context, TNode<JSReceiver> result, base::Optional<TNode<Map>> fast_iterator_result_map)
{
    Label exit(this);
...
void IteratorBuiltinsAssembler::Iterate(
    TNode<Context> context, TNode<Object> iterable, std::function<void(TNode<Object>)> func, std::initializer_list<compiler::CodeAssemblerVariable*> merged_variables)
{
...
TF_BUILTIN(IterableToFixedArrayForWasm, IteratorBuiltinsAssembler)
{
    ...
    GotoIf(WordEqual(SmiUntag(expected_length), values.var_length()->value()), &done);
...
void IteratorBuiltinsAssembler::FastIterableToList(TNode<Context> context, TNode<Object> iterable, TVariable<JSArray>* var_result, Label* slow)
{
    ...
    *var_result = CAST(CallBuiltin(Builtin::kCloneFastJSArrayFillingHoles, context, iterable));
    ...
    BIND(&check_string);
    {
        ...
        GotoIf(IntPtrGreaterThan(length, IntPtrConstant(JSArray::kMaxFastArrayLength)), slow);
