...
TNode<FixedArray> GrowableFixedArray::ToFixedArray() { return ResizeFixedArray(length(), length()); }
...
TNode<FixedArray> GrowableFixedArray::ResizeFixedArray(const TNode<IntPtrT> element_count, const TNode<IntPtrT> new_capacity)
{
    ...
    TNode<FixedArray> to_array = CAST(ExtractFixedArray(
        from_array, base::Optional<TNode<IntPtrT>>(base::nullopt), base::Optional<TNode<IntPtrT>>(element_count), base::Optional<TNode<IntPtrT>>(new_capacity), flags));
