...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ JumpIfIsInRange(
        t2, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor), &not_create_implicit_receiver);
...
static void GetSharedFunctionInfoBytecodeOrBaseline(MacroAssembler* masm, Register sfi_data, Register scratch1, Label* is_baseline)
{
    ...
    __ Ld_d(sfi_data, FieldMemOperand(sfi_data, InterpreterData::kBytecodeArrayOffset));
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm)
{
    ...
    __ Ld_d(t1, FieldMemOperand(a1, JSGeneratorObject::kParametersAndRegistersOffset));
    {
        ...
        __ bind(&done_loop);
        // Push receiver.
        __ Ld_d(kScratchReg, FieldMemOperand(a1, JSGeneratorObject::kReceiverOffset));
        __ Push(kScratchReg);
    }
    ...
    if (v8_flags.debug_code) {
        ...
        __ Assert(eq, AbortReason::kMissingBytecodeArray, a3, Operand(BYTECODE_ARRAY_TYPE));
        __ bind(&is_baseline);
    }
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ Ld_w(params_size, FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));
...
static void AdvanceBytecodeOffsetOrReturn(
    MacroAssembler* masm, Register bytecode_array, Register bytecode_offset, Register bytecode, Register scratch1, Register scratch2, Register scratch3, Label* if_return)
{
...
namespace {
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array) { __ St_h(zero_reg, FieldMemOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset)); }
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm)
{
    ...
    __ Ld_d(feedback_vector, FieldMemOperand(closure, JSFunction::kFeedbackCellOffset));
    __ Ld_d(feedback_vector, FieldMemOperand(feedback_vector, Cell::kValueOffset));
    {
        UseScratchRegisterScope temps(masm);
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    __ Ld_d(kScratchReg, FieldMemOperand(closure, JSFunction::kSharedFunctionInfoOffset));
    __ Ld_d(kInterpreterBytecodeArrayRegister, FieldMemOperand(kScratchReg, SharedFunctionInfo::kFunctionDataOffset));

    Label is_baseline;
    GetSharedFunctionInfoBytecodeOrBaseline(masm, kInterpreterBytecodeArrayRegister, kScratchReg, &is_baseline);
    ...
    // Increment invocation count for the function.
    __ Ld_w(a4, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
    __ Add_w(a4, a4, Operand(1));
    __ St_w(a4, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
    ...
    Label stack_overflow;
    {
        ...
        __ Ld_w(a4, FieldMemOperand(kInterpreterBytecodeArrayRegister, BytecodeArray::kFrameSizeOffset));
    ...
    __ Alsl_d(kScratchReg, a7, kInterpreterDispatchTableRegister, kPointerSizeLog2, t7);
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm)
{
    ...
    __ Ld_d(t0, FieldMemOperand(t0, InterpreterData::kInterpreterTrampolineOffset));
...
void Builtins::Generate_InterpreterEnterAtNextBytecode(MacroAssembler* masm)
{
    ...
    __ Branch(&function_entry_bytecode, eq, kInterpreterBytecodeOffsetRegister, Operand(BytecodeArray::kHeaderSize - kHeapObjectTag + kFunctionEntryBytecodeOffset));
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
    ...
    if (with_result) {
        if (java_script_builtin) {
            ...
            __ St_d(a0, MemOperand(sp, config->num_allocatable_general_registers() * kPointerSize + BuiltinContinuationFrameConstants::kFixedFrameSize));
        }
    }
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ Ld_d(a1, MemOperand(maybe_target_code, Code::kDeoptimizationDataOrInterpreterDataOffset - kHeapObjectTag));
    ...
    Generate_OSREntry(masm, maybe_target_code, Operand(Code::kHeaderSize - kHeapObjectTag));
}
} // namespace
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET, ls, scratch, Operand(LAST_CALLABLE_JS_FUNCTION_TYPE - FIRST_CALLABLE_JS_FUNCTION_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), CallBoundFunction), RelocInfo::CODE_TARGET, eq, instance_type, Operand(JS_BOUND_FUNCTION_TYPE));
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), CallProxy), RelocInfo::CODE_TARGET, eq, instance_type, Operand(JS_PROXY_TYPE));

    // Check if target is a wrapped function and call CallWrappedFunction external
    // builtin
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWrappedFunction), RelocInfo::CODE_TARGET, eq, instance_type, Operand(JS_WRAPPED_FUNCTION_TYPE));
...
void Builtins::Generate_ConstructBoundFunction(MacroAssembler* masm)
{
    ...
    // Patch new.target to [[BoundTargetFunction]] if new.target equals target.
    {
        ...
        __ Ld_d(a3, FieldMemOperand(a1, JSBoundFunction::kBoundTargetFunctionOffset));
        __ bind(&skip_load);
    }
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET, ls, scratch, Operand(LAST_JS_FUNCTION_TYPE - FIRST_JS_FUNCTION_TYPE));

    // Only dispatch to bound functions after checking whether they are
    // constructors.
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructBoundFunction), RelocInfo::CODE_TARGET, eq, instance_type, Operand(JS_BOUND_FUNCTION_TYPE));
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    constexpr RegList kSavedGpRegs = ([]() constexpr {
        ...
        // + instance
        CHECK_EQ(WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs + 1, saved_gp_regs.Count());
    ...
    __ Ld_d(t8, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
...
void Builtins::Generate_GenericJSToWasmWrapper(MacroAssembler* masm) { __ Trap(); }
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, SaveFPRegsMode save_doubles, ArgvMode argv_mode, bool builtin_exit_frame)
{
    ...
    FrameScope scope(masm, StackFrame::MANUAL);
    __ EnterExitFrame(save_doubles == SaveFPRegsMode::kSave, 0, builtin_exit_frame ? StackFrame::BUILTIN_EXIT : StackFrame::EXIT);
    ...
    // Clear c_entry_fp, like we do in `LeaveExitFrame`.
    {
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.Acquire();
        __ li(scratch, ExternalReference::Create(IsolateAddressId::kCEntryFPAddress, masm->isolate()));
...
void Builtins::Generate_DoubleToI(MacroAssembler* masm)
{
    ...
    __ And(scratch, scratch, kFCSRExceptionCauseMask ^ kFCSRDivideByZeroCauseMask);
    ...
    __ Ld_w(input_low, MemOperand(sp, kArgumentOffset + Register::kMantissaOffset));
    __ Ld_w(input_high, MemOperand(sp, kArgumentOffset + Register::kExponentOffset));

    Label normal_exponent;
    // Extract the biased exponent in result.
    __ bstrpick_w(result_reg, input_high, HeapNumber::kExponentShift + HeapNumber::kExponentBits - 1, HeapNumber::kExponentShift);
    ...
    __ Sub_w(result_reg, result_reg, Operand(HeapNumber::kExponentBias + HeapNumber::kMantissaBits + 31));
    ...
    // Calculate shift.
    __ Add_w(scratch, result_reg, Operand(kShiftBase + HeapNumber::kMantissaBits));
...
void CallApiFunctionAndReturn(
    MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, int stack_space, MemOperand* stack_space_operand, MemOperand return_value_operand)
{
    ...
    __ Ld_b(t7, __ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), t7));
    ...
    if (v8_flags.debug_code) {
        ...
        __ Check(eq, AbortReason::kUnexpectedLevelAfterReturnFromApiCall, a1, Operand(s2));
    }
    ...
    __ LeaveExitFrame(kDontSaveDoubles, s0, NO_EMIT_RETURN, kRegisterContainsSlotCount);
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    DCHECK(!AreAliased(api_function_address, argc, call_data, holder, scratch, base));
    ...
    // on the stack).
    __ Add_d(scratch, scratch, Operand((FCA::kArgsLength + 1) * kSystemPointerSize));
    ...
    MemOperand return_value_operand(fp, (kStackSlotsAboveFCA + FCA::kReturnValueOffset) * kPointerSize);

    static constexpr int kUseStackSpaceOperand = 0;
    MemOperand stack_space_operand(sp, 4 * kPointerSize);

    AllowExternalCallThatCantCauseGC scope(masm);
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, kUseStackSpaceOperand, &stack_space_operand, return_value_operand);
}
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    __ LoadRoot(scratch, RootIndex::kUndefinedValue);
    __ St_d(scratch, MemOperand(sp, (PCA::kReturnValueOffset + 1) * kPointerSize));
    __ St_d(scratch, MemOperand(sp, (PCA::kReturnValueDefaultValueIndex + 1) * kPointerSize));
    __ li(scratch, ExternalReference::isolate_address(masm->isolate()));
    ...
    __ St_d(zero_reg, MemOperand(sp, (PCA::kShouldThrowOnErrorIndex + 1) * kPointerSize));
    ...
    __ Ld_d(api_function_address, FieldMemOperand(callback, AccessorInfo::kMaybeRedirectedGetterOffset));

    // +3 is to skip prolog, return address and name handle.
    MemOperand return_value_operand(fp, (PropertyCallbackArguments::kReturnValueOffset + 3) * kPointerSize);
    MemOperand* const kUseStackSpaceConstant = nullptr;
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, kStackUnwindSpace, kUseStackSpaceConstant, return_value_operand);
}
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    const int kSavedRegistersAreaSize = (kNumberOfRegisters * kPointerSize) + kDoubleRegsSize;
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ Ld_d(code_obj, FieldMemOperand(closure, JSFunction::kSharedFunctionInfoOffset));
    __ Ld_d(code_obj, FieldMemOperand(code_obj, SharedFunctionInfo::kFunctionDataOffset));
    ...
    __ SmiUntag(kInterpreterBytecodeOffsetRegister, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
    // Replace BytecodeOffset with the feedback vector.
    __ St_d(feedback_vector, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
    ...
    if (is_osr) {
        ...
        __ Ld_d(kInterpreterBytecodeArrayRegister, MemOperand(fp, InterpreterFrameConstants::kBytecodeArrayFromFp));
        ResetBytecodeAge(masm, kInterpreterBytecodeArrayRegister);
        Generate_OSREntry(masm, code_obj, Operand(Code::kHeaderSize - kHeapObjectTag));
    } else {
