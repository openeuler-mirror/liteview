...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ JumpIfIsInRange(
        r4, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor), &not_create_implicit_receiver);
...
constexpr int kPushedStackSpace = kNumCalleeSaved * kPointerSize - kPointerSize /* FP */ + kNumDoubleCalleeSaved * kDoubleSize
    + 5 * kPointerSize /* r5, r6, r7, fp, lr */ + EntryFrameConstants::kCallerFPOffset;

// Assert that the EntryFrameConstants are in sync with the builtin.
static_assert(kPushedStackSpace == EntryFrameConstants::kDirectCallerSPOffset + 3 * kPointerSize /* r5, r6, r7*/ + EntryFrameConstants::kCallerFPOffset,
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type, Builtin entry_trampoline)
{
    ...
    __ add(sp, sp, Operand(-EntryFrameConstants::kCallerFPOffset - kSystemPointerSize /* already popped one */));
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ ldr(params_size, FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));
...
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array, Register scratch)
{
    ...
    __ strh(scratch, FieldMemOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset));
}
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ and_(scratch, scratch, Operand(FeedbackVector::MaybeHasOptimizedOsrCodeBit::kMask));
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    __ ldr(kInterpreterBytecodeArrayRegister, FieldMemOperand(r4, SharedFunctionInfo::kFunctionDataOffset));

    Label is_baseline;
    GetSharedFunctionInfoBytecodeOrBaseline(masm, kInterpreterBytecodeArrayRegister, r8, &is_baseline);
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    __ SmiTag(kWasmCompileLazyFuncIndexRegister);
    {
        ...
        CHECK_EQ(gp_regs.Count(), WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs + 1);
        CHECK_EQ(highest_fp_reg.code() - lowest_fp_reg.code() + 1, WasmCompileLazyFrameConstants::kNumberOfSavedFpParamRegs);
        ...
        __ ldr(r9, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
        __ add(r8, r8, r9);
    }
...
void Builtins::Generate_WasmDebugBreak(MacroAssembler* masm)
{
    HardAbortScope hard_abort(masm); // Avoid calls to Abort.
    {
        ...
        static_assert(WasmDebugBreakFrameConstants::kPushedFpRegs.Count() == last.code() - first.code() + 1, "All registers in the range from first to last have to be set");
...
int AddressOffset(ExternalReference ref0, ExternalReference ref1) { return ref0.address() - ref1.address(); }
    ...
    MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, int stack_space, MemOperand* stack_space_operand, MemOperand return_value_operand)
{
    ...
    __ ldrb(r8, __ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), r8));
    __ cmp(r8, Operand(0));
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    __ mov(scratch, Operand((FCA::kArgsLength + 1 /* receiver */) * kPointerSize));
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    static constexpr int kDoubleRegsOffset = FrameDescription::double_registers_offset();
    {
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.Acquire();
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ ldr(code_obj, FieldMemOperand(code_obj, SharedFunctionInfo::kFunctionDataOffset));
