...
template <typename T> static constexpr T FirstFromVarArgs(T x, ...) noexcept { return x; }
...
#define DEF_ENUM(Name, ...) k##Name,
    BUILTIN_LIST(DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM, DEF_ENUM)
#undef DEF_ENUM
...
#define ADD_ONE(Name, ...) +1
    static constexpr int kBuiltinCount = 0 BUILTIN_LIST(ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE, ADD_ONE);
...
#undef ADD_ONE
    ...
    static constexpr bool IsBuiltinId(Builtin builtin) { return builtin != Builtin::kNoBuiltinId; }
    ...
    static constexpr bool IsTier0(Builtin builtin) { return builtin <= kLastTier0 && IsBuiltinId(builtin); }
    ...
    Handle<CodeT> NonPrimitiveToPrimitive(ToPrimitiveHint hint = ToPrimitiveHint::kDefault);
    ...
    static constexpr bool AllBuiltinsAreIsolateIndependent() { return kAllBuiltinsAreIsolateIndependent; }
    ...
    template <typename CodeOrCodeT> static bool IsIsolateIndependentBuiltin(CodeOrCodeT code)
    {
    ...
    static Handle<Code> GenerateOffHeapTrampolineFor(Isolate* isolate, Address off_heap_entry, int32_t kind_specific_flags, bool generate_jump_to_instruction_stream);
...
private:
    ...
    static void Generate_CallOrConstructVarargs(MacroAssembler* masm, Handle<CodeT> code);
    static void Generate_CallOrConstructForwardVarargs(MacroAssembler* masm, CallOrConstructMode mode, Handle<CodeT> code);
...
#define DECLARE_TF(Name, ...) static void Generate_##Name(compiler::CodeAssemblerState* state);
    ...
    BUILTIN_LIST(IGNORE_BUILTIN, DECLARE_TF, DECLARE_TF, DECLARE_TF, DECLARE_TF, IGNORE_BUILTIN, DECLARE_ASM)
...
V8_INLINE constexpr bool IsInterpreterTrampolineBuiltin(Builtin builtin_id)
{
    ...
    return builtin_id != Builtin::kNoBuiltinId
        && (builtin_id == Builtin::kInterpreterEntryTrampoline || builtin_id == Builtin::kInterpreterEnterAtBytecode || builtin_id == Builtin::kInterpreterEnterAtNextBytecode);
}
