...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm)
{
...

    {
        ...
        __ mov(ebx, FieldOperand(edx, JSGeneratorObject::kParametersAndRegistersOffset));
        {
            ...
            __ dec(ecx);
            __ j(less, &done_loop);
            __ Push(FieldOperand(ebx, ecx, times_tagged_size, FixedArray::kHeaderSize));
            __ jmp(&loop);
            __ bind(&done_loop);
        }
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ mov(params_size, FieldOperand(params_size, BytecodeArray::kParameterSizeOffset));
    ...
    __ lea(actual_params_size, Operand(actual_params_size, times_system_pointer_size, 0));
...
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array) { __ mov_w(FieldOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset), Immediate(0)); }
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ and_(scratch, Immediate(FeedbackVector::MaybeHasOptimizedOsrCodeBit::kMask));
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    Register flags = ecx;
    __ LoadFeedbackVectorFlagsAndJumpIfNeedsProcessing(flags, xmm1, CodeKind::INTERPRETED_FUNCTION, &flags_need_processing);
    ...
    __ mov(eax, FieldOperand(edi, JSFunction::kSharedFunctionInfoOffset));
    __ mov(kInterpreterBytecodeArrayRegister, FieldOperand(eax, SharedFunctionInfo::kFunctionDataOffset));
    ...
    Label do_return;
    __ Push(eax);
    AdvanceBytecodeOffsetOrReturn(masm, kInterpreterBytecodeArrayRegister, kInterpreterBytecodeOffsetRegister, ecx, kInterpreterDispatchTableRegister, eax, &do_return);
    __ Pop(eax);
    __ jmp(&do_dispatch);
    ...
    __ mov(Operand(ebp, InterpreterFrameConstants::kBytecodeOffsetFromFp), Immediate(Smi::FromInt(BytecodeArray::kHeaderSize - kHeapObjectTag + kFunctionEntryBytecodeOffset)));
...
void Builtins::Generate_InterpreterPushArgsThenConstructImpl(MacroAssembler* masm, InterpreterPushArgsMode mode)
{
    ...
    Generate_InterpreterPushZeroAndArgsAndReturnAddress(masm, eax, ecx, edx, edi, InterpreterPushArgsThenConstructDescriptor::GetStackParameterCount(), &stack_overflow);
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm)
{
    ...
    __ mov(scratch, FieldOperand(scratch, SharedFunctionInfo::kFunctionDataOffset));
    __ Push(eax);
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
    ...
    if (with_result && java_script_builtin) {
        ...
        __ movd(Operand(esp, eax, times_system_pointer_size, BuiltinContinuationFrameConstants::kFixedFrameSize - kJSArgcReceiverSlots * kSystemPointerSize), xmm0);
    }
...
void Builtins::Generate_CallOrConstructVarargs(MacroAssembler* masm, Handle<Code> code)
{
    ...
    // Push additional arguments onto the stack.
    {
        ...
        __ mov(edi, FieldOperand(kArgumentsList, eax, times_tagged_size, FixedArray::kHeaderSize));
...
void Generate_PushBoundArguments(MacroAssembler* masm)
{
    ...
    __ j(zero, &no_bound_arguments);
    {
        ...
        // Push [[BoundArguments]] to the stack.
        {
            ...
            __ bind(&loop);
            __ dec(edx);
            __ mov(esi, FieldOperand(ecx, edx, times_tagged_size, FixedArray::kHeaderSize));
            __ push(esi);
            __ j(greater, &loop);
        }
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET);

    __ bind(&non_callable_jsfunction);
    __ cmpw(instance_type, Immediate(JS_BOUND_FUNCTION_TYPE));
    __ j(not_equal, &non_jsboundfunction);
    __ Jump(BUILTIN_CODE(masm->isolate(), CallBoundFunction), RelocInfo::CODE_TARGET);
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWrappedFunction), RelocInfo::CODE_TARGET);
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET);
    ...
    __ j(not_equal, &non_jsboundfunction);
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructBoundFunction), RelocInfo::CODE_TARGET);
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    __ SmiTag(kWasmCompileLazyFuncIndexRegister);
    {
        ...
        __ add(edi, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
    }
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, SaveFPRegsMode save_doubles, ArgvMode argv_mode, bool builtin_exit_frame)
{
    ...
    DCHECK(!AreAliased(kRuntimeCallArgCountRegister, kRuntimeCallArgvRegister, kRuntimeCallFunctionRegister, kContextRegister, kJSFunctionRegister, kRootRegister));
    ...
    ExternalReference find_handler = ExternalReference::Create(Runtime::kUnwindAndFindExceptionHandler);
    {
        ...
        __ Move(esi, Immediate(ExternalReference::isolate_address(masm->isolate())));
...
Operand ApiParameterOperand(int index) { return Operand(esp, index * kSystemPointerSize); }
...
void CallApiFunctionAndReturn(MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, Operand thunk_last_arg, int stack_space, Operand* stack_space_operand,
    Operand return_value_operand)
{
    ...
    __ cmpb(__ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), eax), Immediate(0));
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    // on the stack).
    __ lea(scratch, Operand(scratch, (FCA::kArgsLength + 1) * kSystemPointerSize));
    ...
    __ lea(scratch, Operand(argc, times_system_pointer_size, (FCA::kArgsLength + 1 /* receiver */) * kSystemPointerSize));
    ...
    Operand return_value_operand(ebp, (kStackSlotsAboveFCA + FCA::kReturnValueOffset) * kSystemPointerSize);

    static constexpr int kUseStackSpaceOperand = 0;
    Operand stack_space_operand = ApiParameterOperand(kApiArgc + 3);
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, ApiParameterOperand(1), kUseStackSpaceOperand, &stack_space_operand, return_value_operand);
}
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    __ mov(function_address, FieldOperand(callback, AccessorInfo::kMaybeRedirectedGetterOffset));
    // +3 is to skip prolog, return address and name handle.
    Operand return_value_operand(ebp, (PropertyCallbackArguments::kReturnValueOffset + 3) * kSystemPointerSize);
    Operand* const kUseStackSpaceConstant = nullptr;
    CallApiFunctionAndReturn(masm, function_address, thunk_ref, thunk_last_arg, kStackUnwindSpace, kUseStackSpaceConstant, return_value_operand);
}
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    __ Move(Operand(esp, 4 * kSystemPointerSize), Immediate(ExternalReference::isolate_address(masm->isolate())));
    {
    ...
    __ mov_b(__ ExternalReferenceAsOperand(ExternalReference::stack_is_iterable_address(isolate), edx), Immediate(0));
    ...
    __ mov_b(__ ExternalReferenceAsOperand(ExternalReference::stack_is_iterable_address(isolate), edx), Immediate(1));
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ mov(code_obj, FieldOperand(code_obj, SharedFunctionInfo::kFunctionDataOffset));
    ...
    __ mov(MemOperand(ebp, InterpreterFrameConstants::kBytecodeOffsetFromFp), feedback_vector);
    ...
    __ lea(code_obj, FieldOperand(code_obj, kReturnRegister0, times_1, Code::kHeaderSize));
    __ pop(kInterpreterAccumulatorRegister);
