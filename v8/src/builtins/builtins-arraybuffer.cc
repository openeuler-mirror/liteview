...
#define CHECK_SHARED(expected, name, method)                                                                                                                                       \
    if (name->is_shared() != expected) {                                                                                                                                           \
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kIncompatibleMethodReceiver, isolate->factory()->NewStringFromAsciiChecked(method), name));          \
    }
...
Object ConstructBuffer(Isolate* isolate, Handle<JSFunction> target, Handle<JSReceiver> new_target, Handle<Object> length, Handle<Object> max_length, InitializedFlag initialized)
{
    ...
    if (resizable == ResizableFlag::kNotResizable) {
        ...
        if (!TryNumberToSize(*max_length, &max_byte_length) || max_byte_length > JSArrayBuffer::kMaxByteLength || max_byte_length > JSTypedArray::kMaxLength) {
        ...
        MAYBE_RETURN(JSArrayBuffer::GetResizableBackingStorePageConfiguration(isolate, byte_length, max_byte_length, kThrowOnError, &page_size, &initial_pages, &max_pages),
            ReadOnlyRoots(isolate).exception());

        backing_store
            = BackingStore::TryAllocateAndPartiallyCommitMemory(isolate, byte_length, max_byte_length, page_size, initial_pages, max_pages, WasmMemoryFlag::kNotWasm, shared);
    }
    if (!backing_store) {
...
static Object SliceHelper(BuiltinArguments args, Isolate* isolate, const char* kMethodName, bool is_shared)
{
    HandleScope scope(isolate);
    ...
    double const first = (relative_start->Number() < 0) ? std::max(len + relative_start->Number(), 0.0) : std::min(relative_start->Number(), len);
    ...
    Handle<Object> ctor;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, ctor, Object::SpeciesConstructor(isolate, Handle<JSReceiver>::cast(args.receiver()), constructor_fun));
    ...
    Handle<JSReceiver> new_;
    {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, new_obj, Execution::New(isolate, ctor, argc, argv.begin()));
    ...
    if (!new_->IsJSArrayBuffer()) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kIncompatibleMethodReceiver, isolate->factory()->NewStringFromAsciiChecked(kMethodName), new_));
    }
...
static Object ResizeHelper(BuiltinArguments args, Isolate* isolate, const char* kMethodName, bool is_shared)
{
    HandleScope scope(isolate);
    ...
    if (!TryNumberToSize(*number_new_byte_length, &new_byte_length)) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kInvalidArrayBufferResizeLength, isolate->factory()->NewStringFromAsciiChecked(kMethodName)));
    }
...
BUILTIN(SharedArrayBufferPrototypeGetByteLength)
{
    ...
    DCHECK_EQ(array_buffer->max_byte_length(), array_buffer->GetBackingStore()->max_byte_length());
...
BUILTIN(ArrayBufferPrototypeTransfer)
{
    ...
    if (new_byte_length == 0) {
        ...
        // 14. Return new.
        return *isolate->factory()->NewJSArrayBufferAndBackingStore(0, InitializedFlag::kUninitialized).ToHandleChecked();
    }

    // Case 2: We can reuse the same BackingStore.
    auto from_backing_store = array_buffer->GetBackingStore();
    if (from_backing_store && !from_backing_store->is_resizable_by_js() && (new_byte_length == array_buffer->GetByteLength() || from_backing_store->CanReallocate())) {
        // Reallocate covers steps 6-12.
        if (new_byte_length != array_buffer->GetByteLength() && !from_backing_store->Reallocate(isolate, new_byte_length)) {
            THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewRangeError(MessageTemplate::kArrayBufferAllocationFailed));
        }
    ...
    MaybeHandle<JSArrayBuffer> result = isolate->factory()->NewJSArrayBufferAndBackingStore(new_byte_length, InitializedFlag::kUninitialized);
