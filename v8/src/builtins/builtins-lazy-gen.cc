...
void LazyBuiltinsAssembler::GenerateTailCallToJSCode(TNode<CodeT> code, TNode<JSFunction> function)
{
...
void LazyBuiltinsAssembler::MaybeTailCallOptimizedCodeSlot(TNode<JSFunction> function, TNode<FeedbackVector> feedback_vector)
{
    ...
    GotoIfNot(
        IsSetWord32(flags, FeedbackVector::kFlagsHasAnyOptimizedCode | FeedbackVector::kFlagsTieringStateIsAnyRequested | FeedbackVector::kFlagsLogNextExecution), &fallthrough);
    ...
    BIND(&may_have_optimized_code);
    {
        ...
        TNode<CodeT> optimized_code = CAST(GetHeapObjectAssumeWeak(maybe_optimized_code_entry, &heal_optimized_code_slot));
...
void LazyBuiltinsAssembler::CompileLazy(TNode<JSFunction> function)
{
    ...
    TNode<CodeT> sfi_code = GetSharedFunctionInfoCode(shared, &sfi_data_type, &compile_function);
    ...
    CSA_DCHECK(this, TaggedNotEqual(sfi_code, HeapConstant(BUILTIN_CODE(isolate(), CompileLazy))));
    ...
    code = Select<CodeT>(
        ...
        [=]() { return CAST(CallRuntime(Runtime::kInstallBaselineCode, Parameter<Context>(Descriptor::kContext), function)); });
    Goto(&tailcall_code);
