...
void Generate_JSBuiltinsConstructStubHelper(MacroAssembler* masm)
{
    ...
    __ DropArguments(kScratchReg, MacroAssembler::kCountIsSmi, MacroAssembler::kCountIncludesReceiver, kScratchReg);
    __ Ret();
}
...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    // -----------------------------------
    {
        ...
        __ JumpIfIsInRange(
            func_info, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor), &not_create_implicit_receiver);
...
static void GetSharedFunctionInfoBytecodeOrBaseline(MacroAssembler* masm, Register sfi_data, Register scratch1, Label* is_baseline)
{
    ...
    __ Branch(&done, ne, scratch1, Operand(INTERPRETER_DATA_TYPE), Label::Distance::kNear);
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ Lw(params_size, FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));
...
namespace {
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array) { __ Sh(zero_reg, FieldMemOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset)); }
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm)
{
    ...
    __ LoadWord(feedback_vector, FieldMemOperand(closure, JSFunction::kFeedbackCellOffset));
    __ LoadWord(feedback_vector, FieldMemOperand(feedback_vector, Cell::kValueOffset));
    {
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    __ Branch(&push_stack_frame, ne, a4, Operand(FEEDBACK_VECTOR_TYPE), Label::Distance::kNear);
    ...
    __ bind(&is_baseline);
    {
        ...
        // allocate it.
        __ LoadTaggedPointerField(scratch, FieldMemOperand(feedback_vector, HeapObject::kMapOffset));
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
    ...
    if (with_result) {
        if (java_script_builtin) {
            ...
            __ StoreWord(a0, MemOperand(sp, config->num_allocatable_general_registers() * kSystemPointerSize + BuiltinContinuationFrameConstants::kFixedFrameSize));
        }
    }
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ LoadTaggedPointerField(a1, MemOperand(a0, Code::kDeoptimizationDataOrInterpreterDataOffset - kHeapObjectTag));
...
void Builtins::Generate_FunctionPrototypeApply(MacroAssembler* masm)
{
    ...
    __ Branch(&no_arguments, eq, arg_array, Operand(undefined_value), Label::Distance::kNear);
...
void Builtins::Generate_CallFunction(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ And(kScratchReg, a3, Operand(SharedFunctionInfo::IsClassConstructorBit::kMask));
    ...
    __ Branch(&done_convert, ne, kScratchReg, Operand(zero_reg));
    {
        ...
        if (mode == ConvertReceiverMode::kNullOrUndefined) {
            ...
            __ Branch(&done_convert, Ugreater_equal, a4, Operand(FIRST_JS_RECEIVER_TYPE));
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    temps.Include(t1, t2);
    temps.Include(t4);
    Register map = temps.Acquire(), type = temps.Acquire(), range = temps.Acquire();
    __ JumpIfSmi(a1, &non_callable);
    __ LoadMap(map, a1);
    __ GetInstanceTypeRange(map, type, FIRST_CALLABLE_JS_FUNCTION_TYPE, range);
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET, Uless_equal, range, Operand(LAST_CALLABLE_JS_FUNCTION_TYPE - FIRST_CALLABLE_JS_FUNCTION_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), CallBoundFunction), RelocInfo::CODE_TARGET, eq, type, Operand(JS_BOUND_FUNCTION_TYPE));
    Register scratch = map;
    // Check if target has a [[Call]] internal method.
    __ Lbu(scratch, FieldMemOperand(map, Map::kBitFieldOffset));
    __ And(scratch, scratch, Operand(Map::Bits1::IsCallableBit::kMask));
    __ Branch(&non_callable, eq, scratch, Operand(zero_reg), Label::Distance::kNear);

    __ Jump(BUILTIN_CODE(masm->isolate(), CallProxy), RelocInfo::CODE_TARGET, eq, type, Operand(JS_PROXY_TYPE));

    // Check if target is a wrapped function and call CallWrappedFunction external
    // builtin
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWrappedFunction), RelocInfo::CODE_TARGET, eq, type, Operand(JS_WRAPPED_FUNCTION_TYPE));
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET, Uless_equal, range, Operand(LAST_JS_FUNCTION_TYPE - FIRST_JS_FUNCTION_TYPE));

    // Only dispatch to bound functions after checking whether they are
    // constructors.
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructBoundFunction), RelocInfo::CODE_TARGET, eq, scratch, Operand(JS_BOUND_FUNCTION_TYPE));

    // Only dispatch to proxies after checking whether they are constructors.
    __ Branch(&non_proxy, ne, scratch, Operand(JS_PROXY_TYPE), Label::Distance::kNear);
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    __ LoadWord(kScratchReg, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
    ...
    MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, int stack_space, MemOperand* stack_space_operand, MemOperand return_value_operand)
{
    ...
    Label profiler_enabled, end_profiler_check;
    {
        ...
        __ Branch(&profiler_enabled, ne, scratch, Operand(zero_reg), Label::Distance::kNear);
    ...
    __ LeaveExitFrame(kDontSaveDoubles, s3, NO_EMIT_RETURN, kRegisterContainsSlotCount);

    // Check if the function scheduled an exception.
    __ LoadRoot(a4, RootIndex::kTheHoleValue);
    __ li(kScratchReg, ExternalReference::scheduled_exception_address(isolate));
    __ LoadWord(a5, MemOperand(kScratchReg));
    __ Branch(&promote_scheduled_exception, ne, a4, Operand(a5), Label::Distance::kNear);
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    // on the stack).
    __ AddWord(scratch, scratch, Operand((FCA::kArgsLength + 1) * kSystemPointerSize));
    __ StoreWord(scratch, MemOperand(sp, 2 * kSystemPointerSize));
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    __ StoreWord(receiver, MemOperand(sp, (PCA::kThisIndex + 1) * kSystemPointerSize));
    __ LoadAnyTaggedField(scratch, FieldMemOperand(callback, AccessorInfo::kDataOffset));
    __ StoreWord(scratch, MemOperand(sp, (PCA::kDataIndex + 1) * kSystemPointerSize));
    __ LoadRoot(scratch, RootIndex::kUndefinedValue);
    __ StoreWord(scratch, MemOperand(sp, (PCA::kReturnValueOffset + 1) * kSystemPointerSize));
    __ StoreWord(scratch, MemOperand(sp, (PCA::kReturnValueDefaultValueIndex + 1) * kSystemPointerSize));
    __ li(scratch, ExternalReference::isolate_address(masm->isolate()));
    __ StoreWord(scratch, MemOperand(sp, (PCA::kIsolateIndex + 1) * kSystemPointerSize));
    __ StoreWord(holder, MemOperand(sp, (PCA::kHolderIndex + 1) * kSystemPointerSize));
    // should_throw_on_error -> false
    DCHECK_EQ(0, Smi::zero().ptr());
    __ StoreWord(zero_reg, MemOperand(sp, (PCA::kShouldThrowOnErrorIndex + 1) * kSystemPointerSize));
    ...
    __ LoadWord(api_function_address, FieldMemOperand(callback, AccessorInfo::kMaybeRedirectedGetterOffset));
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    if (!is_osr) {
        ...
        Register scratch = temps.Acquire();
        __ GetObjectType(code_obj, scratch, scratch);
        __ Assert(eq, AbortReason::kExpectedBaselineData, scratch, Operand(CODET_TYPE));
    }
    if (v8_flags.debug_code) {
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.Acquire();
    ...
    __ SmiUntag(kInterpreterBytecodeOffsetRegister, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
    // Replace BytecodeOffset with the feedback vector.
    __ StoreWord(feedback_vector, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
