...
TNode<Object> ArrayBuiltinsAssembler::TypedArrayMapProcessor(TNode<Object> k_value, TNode<UintPtrT> k)
{
    ...
    EmitElementStore(CAST(a()), k_number, num_value, source_elements_kind_, KeyedAccessStoreMode::STANDARD_STORE, &detached, context());
    Goto(&done);
...
void ArrayBuiltinsAssembler::InitIteratingArrayBuiltinBody(TNode<Context> context, TNode<Object> receiver, TNode<Object> callbackfn, TNode<Object> this_arg, TNode<IntPtrT> argc)
{
    context_ = context;
    receiver_ = receiver;
...
#undef ELEMENTS_KIND
    ...
    for (auto it = labels.begin(); it != labels.end(); ++i, ++it) {
        ...
        if (is_rab_gsab) {
            source_elements_kind_ = GetCorrespondingNonRabGsabElementsKind(source_elements_kind_);
        }
        VisitAllTypedArrayElements(array_buffer, processor, direction, typed_array, is_rab_gsab);
        ReturnFromBuiltin(a_.value());
    }
}

void ArrayBuiltinsAssembler::VisitAllTypedArrayElements(
    TNode<JSArrayBuffer> array_buffer, const CallResultProcessor& processor, ForEachDirection direction, TNode<JSTypedArray> typed_array, bool can_shrink)
{
    ...
    BuildFastLoop<UintPtrT>(
        ...
        [&](TNode<UintPtrT> index) {
            ...
            }
            {
                ...
                value = LoadFixedTypedArrayElementAsTagged(data_ptr, index, source_elements_kind_);
                Goto(&process);
            }
...
TF_BUILTIN(ArrayPrototypePop, CodeStubAssembler)
{
    ...
    BIND(&fast);
    {
        ...
        GotoIf(IntPtrLessThan(IntPtrAdd(IntPtrAdd(new_length, new_length), IntPtrConstant(JSObject::kMinAddedElementsCapacity)), capacity), &runtime);

        StoreObjectFieldNoWriteBarrier(array_receiver, JSArray::kLengthOffset, SmiTag(new_length));
...

        {
            ...
            TNode<Float64T> value = LoadFixedDoubleArrayElement(elements_known_double_array, new_length, &return_undefined);
        ...
        BIND(&fast_elements);
        {
            ...
            TNode<Object> value = LoadFixedArrayElement(elements_known_fixed_array, new_length);
            StoreFixedArrayElement(elements_known_fixed_array, new_length, TheHoleConstant());
    ...
    BIND(&runtime);
    {
        ...
        TailCallBuiltin(Builtin::kArrayPop, context, target, UndefinedConstant(), argc);
    }
}
...
TF_BUILTIN(ArrayPrototypePush, CodeStubAssembler)
{
    ...
    BIND(&runtime);
    {
        ...
        // fast path.
        TNode<JSFunction> target = LoadTargetFromFrame();
        TailCallBuiltin(Builtin::kArrayPush, context, target, UndefinedConstant(), argc);
    }
}
...
TF_BUILTIN(CloneFastJSArrayFillingHoles, ArrayBuiltinsAssembler)
{
    ...
    Return(CloneFastJSArray(context, array, base::nullopt, HoleConversionMode::kConvertToUndefined));
}
...
TF_BUILTIN(TypedArrayPrototypeMap, ArrayBuiltinsAssembler)
{
    ...
    GenerateIteratingTypedArrayBuiltinBody("%TypedArray%.prototype.map", &ArrayBuiltinsAssembler::TypedArrayMapResultGenerator, &ArrayBuiltinsAssembler::TypedArrayMapProcessor);
}
...
void ArrayIncludesIndexofAssembler::Generate(SearchVariant variant, TNode<IntPtrT> argc, TNode<Context> context)
{
    ...
    BIND(&if_smi);
    {
        Callable callable = Builtins::CallableFor(isolate(), (variant == kIncludes) ? Builtin::kArrayIncludesSmi : Builtin::kArrayIndexOfSmi);
        TNode<Object> result = CallStub(callable, context, elements, search_element, array_length, SmiTag(index_var.value()));
    ...
    {
        Callable callable
            = (variant == kIncludes) ? Builtins::CallableFor(isolate(), Builtin::kArrayIncludesSmiOrObject) : Builtins::CallableFor(isolate(), Builtin::kArrayIndexOfSmiOrObject);
        TNode<Object> result = CallStub(callable, context, elements, search_element, array_length, SmiTag(index_var.value()));
        args.PopAndReturn(result);
    }
    ...
    {
        Callable callable = (variant == kIncludes) ? Builtins::CallableFor(isolate(), Builtin::kArrayIncludesPackedDoubles)
                                                   : Builtins::CallableFor(isolate(), Builtin::kArrayIndexOfPackedDoubles);
        TNode<Object> result = CallStub(callable, context, elements, search_element, array_length, SmiTag(index_var.value()));
        args.PopAndReturn(result);
    }
    ...
    {
        Callable callable
            = (variant == kIncludes) ? Builtins::CallableFor(isolate(), Builtin::kArrayIncludesHoleyDoubles) : Builtins::CallableFor(isolate(), Builtin::kArrayIndexOfHoleyDoubles);
        TNode<Object> result = CallStub(callable, context, elements, search_element, array_length, SmiTag(index_var.value()));
        args.PopAndReturn(result);
    }
    ...
    TNode<Smi> array_length, TNode<Smi> from_index, SimpleElementKind array_kind)
{
    ...
    Label ident_loop(this, &index_var), heap_num_loop(this, &search_num), string_loop(this), bigint_loop(this, &index_var), undef_loop(this, &index_var), not_smi(this),
    ...
    if (kCanVectorize) {
        ...
        TNode<IntPtrT> result = UncheckedCast<IntPtrT>(CallCFunction(simd_function, MachineType::UintPtr(), std::make_pair(MachineType::TaggedPointer(), elements),
            std::make_pair(MachineType::UintPtr(), array_length_untagged), std::make_pair(MachineType::UintPtr(), index_var.value()),
    ...
    BIND(&string_loop);
    {
        ...
        string_asm.StringEqual_Core(search_element_string, search_type, CAST(element_k), element_k_type, search_length, &return_found, &continue_loop, &runtime);
        BIND(&runtime);
...
TF_BUILTIN(ArrayIteratorPrototypeNext, CodeStubAssembler)
{
    ...
    BIND(&if_array);
    {
        ...
        GotoIf(
            Word32Equal(LoadAndUntagToWord32ObjectField(iterator, JSArrayIterator::kKindOffset), Int32Constant(static_cast<int>(IterationKind::kKeys))), &allocate_iterator_result);
...
public:
    ...
    TNode<Number> FlattenIntoArray(TNode<Context> context, TNode<JSReceiver> target, TNode<JSReceiver> source, TNode<Number> source_length, TNode<Number> start,
        TNode<Number> depth, base::Optional<TNode<HeapObject>> mapper_function = base::nullopt, base::Optional<TNode<Object>> this_arg = base::nullopt)
    {
        ...
        BIND(&loop);
        {
            ...
            CSA_DCHECK(this, SmiGreaterThanOrEqual(CAST(source_index), SmiConstant(0)));
            const TNode<Oddball> exists = HasProperty(context, source, source_index, kHasProperty);
            ...
            GotoIfNot(IsTrue(exists), &next);
            {
                ...
                TNode<Object> element_maybe_smi = GetProperty(context, source, source_index);

                // ii. If mapperFunction is present, then
                if (mapper_function) {
                    CSA_DCHECK(this, Word32Or(IsUndefined(mapper_function.value()), IsCallable(mapper_function.value())));
                    DCHECK(this_arg.has_value());

                    // 1. Set element to ? Call(mapperFunction, thisArg , « element,
                    //                          sourceIndex, source »).
                    element_maybe_smi = Call(context, mapper_function.value(), this_arg.value(), element_maybe_smi, source_index, source);
                }

                // iii. Let shouldFlatten be false.
                Label if_flatten_array(this), if_flatten_proxy(this, Label::kDeferred), if_noflatten(this);
                // iv. If depth > 0, then
                ...
                Branch(IsTrue(CallRuntime(Runtime::kArrayIsArray, context, element)), &if_flatten_proxy, &if_noflatten);
                ...
                BIND(&if_flatten_array);
                {
                    ...
                    const TNode<Object> element_length = LoadObjectField(element, JSArray::kLengthOffset);

                    // 2. Set targetIndex to ? FlattenIntoArray(target, element,
                    //                                          elementLen, targetIndex,
                    //                                          depth - 1).
                    var_target_index = CAST(CallBuiltin(Builtin::kFlattenIntoArray, context, target, element, element_length, target_index, NumberDec(depth)));
                    Goto(&next);
                }
                ...
                BIND(&if_flatten_proxy);
                {
                    ...
                    // 1. Let elementLen be ? ToLength(? Get(element, "length")).
                    const TNode<Number> element_length = ToLength_Inline(context, GetProperty(context, element, LengthStringConstant()));
                ...
                BIND(&if_noflatten);
                {
                    ...
                    GotoIfNumberGreaterThanOrEqual(target_index, NumberConstant(kMaxSafeInteger), &throw_error);

                    // 2. Perform ? CreateDataPropertyOrThrow(target,
                    //                                        ! ToString(targetIndex),
                    //                                        element).
                    CallRuntime(Runtime::kCreateDataProperty, context, target, target_index, element);
                    ...
                    BIND(&throw_error);
                    ThrowTypeError(context, MessageTemplate::kFlattenPastSafeLength, source_length, target_index);
                }
            }
            BIND(&next);
...
TF_BUILTIN(FlatMapIntoArray, ArrayFlattenAssembler)
{
    ...
    Return(FlattenIntoArray(context, target, source, source_length, start, depth, mapper_function, this_arg));
}
...
TF_BUILTIN(ArrayPrototypeFlat, CodeStubAssembler)
{
    ...
    const TNode<Number> source_length = ToLength_Inline(context, GetProperty(context, o, LengthStringConstant()));
    ...
    const TNode<JSReceiver> constructor = CAST(CallRuntime(Runtime::kArraySpeciesConstructor, context, o));
    const TNode<JSReceiver> a = Construct(context, constructor, SmiConstant(0));

    // 6. Perform ? FlattenIntoArray(A, O, sourceLen, 0, depthNum).
    CallBuiltin(Builtin::kFlattenIntoArray, context, a, o, source_length, SmiConstant(0), var_depth_num.value());
...
TF_BUILTIN(ArrayPrototypeFlatMap, CodeStubAssembler)
{
    ...
    CallBuiltin(Builtin::kFlatMapIntoArray, context, a, o, source_length, SmiConstant(0), SmiConstant(1), mapper_function, t);
...
void ArrayBuiltinsAssembler::CreateArrayDispatchNoArgument(
    TNode<Context> context, TNode<JSFunction> target, TNode<Int32T> argc, AllocationSiteOverrideMode mode, base::Optional<TNode<AllocationSite>> allocation_site)
{
    ...
    BIND(&smi_size);
    {
        ...
        BIND(&small_smi_size);
        {
            TNode<JSArray> array = AllocateJSArray(elements_kind, array_map, array_size_smi, array_size_smi,
                mode == DONT_TRACK_ALLOCATION_SITE ? base::Optional<TNode<AllocationSite>>(base::nullopt) : CAST(allocation_site));
            Return(array);
        }
    }
...
void ArrayBuiltinsAssembler::GenerateArrayNoArgumentConstructor(ElementsKind kind, AllocationSiteOverrideMode mode)
{
    ...
    base::Optional<TNode<AllocationSite>> allocation_site
        = track_allocation_site ? Parameter<AllocationSite>(Descriptor::kAllocationSite) : base::Optional<TNode<AllocationSite>>(base::nullopt);
...
#define GENERATE_ARRAY_CTOR(name, kind_camel, kind_caps, mode_camel, mode_caps)                                                                                                    \
    TF_BUILTIN(Array##name##Constructor_##kind_camel##_##mode_camel, ArrayBuiltinsAssembler) { GenerateArray##name##Constructor(kind_caps, mode_caps); }
