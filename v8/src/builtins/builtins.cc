...
#define DECL_CPP(Name, ...) { #Name, Builtins::CPP, { FUNCTION_ADDR(Builtin_##Name) } },
...
const BuiltinMetadata builtin_metadata[] = { BUILTIN_LIST(DECL_CPP, DECL_TFJ, DECL_TFC, DECL_TFS, DECL_TFH, DECL_BCH, DECL_ASM) };
#undef DECL_CPP
#undef DECL_TFJ
#undef DECL_TFC
#undef DECL_TFS
...
#define CASE_OTHER(Name, ...)                                                                                                                                                      \
    case Builtin::k##Name: {                                                                                                                                                       \
        ...
        BUILTIN_LIST(IGNORE_BUILTIN, IGNORE_BUILTIN, CASE_OTHER, CASE_OTHER, CASE_OTHER, IGNORE_BUILTIN, CASE_OTHER)
#undef CASE_OTHER
    default:
...
void Builtins::PrintBuiltinSize()
{
    ...
    for (Builtin builtin = Builtins::kFirst; builtin <= Builtins::kLast; ++builtin) {
        ...
        PrintF(stdout, "%s Builtin, %s, %d\n", kind, builtin_name, code.InstructionSize());
    }
}
...
// static
bool Builtins::IsBuiltin(const Code code) { return Builtins::IsBuiltinId(code.builtin_id()); }
...
void Builtins::InitializeIsolateDataTables(Isolate* isolate)
{
    ...
    for (Builtin i = Builtins::kFirst; i <= Builtins::kLast; ++i) {
        ...
        isolate_data->builtin_entry_table()[ToInt(i)] = embedded_data.InstructionStartOfBuiltin(i);
    }
...
{
    if (!isolate->v8_file_logger()->is_listening_to_code_events() && !isolate->is_profiling()) {
...
public:
    explicit OffHeapTrampolineGenerator(Isolate* isolate)
        : isolate_(isolate)
        , masm_(isolate, AssemblerOptions::DefaultForOffHeapTrampoline(isolate), CodeObjectRequired::kYes, ExternalAssemblerBuffer(buffer_, kBufferSize))
    {
    }
...
// static
Handle<Code> Builtins::GenerateOffHeapTrampolineFor(Isolate* isolate, Address off_heap_entry, int32_t kind_specific_flags, bool generate_jump_to_instruction_stream)
{
    ...
    CodeDesc desc = generator.Generate(off_heap_entry, generate_jump_to_instruction_stream ? TrampolineType::kJump : TrampolineType::kAbort);
...
// static
Handle<ByteArray> Builtins::GenerateOffHeapTrampolineRelocInfo(Isolate* isolate)
{
    OffHeapTrampolineGenerator generator(isolate);
    ...
    Handle<ByteArray> reloc_info = isolate->factory()->NewByteArray(desc.reloc_size, AllocationType::kReadOnly);
...
Handle<Code> Builtins::CreateInterpreterEntryTrampolineForProfiling(Isolate* isolate)
{
    ...
    int kind_specific_flags;
    {
        ...
        kind_specific_flags = CodeDataContainerFromCodeT(code).kind_specific_flags(kRelaxedLoad);
    }
...
// static
bool Builtins::IsCpp(Builtin builtin) { return Builtins::KindOf(builtin) == CPP; }

// static
bool Builtins::AllowDynamicFunction(Isolate* isolate, Handle<JSFunction> target, Handle<JSObject> target_global_proxy)
{
