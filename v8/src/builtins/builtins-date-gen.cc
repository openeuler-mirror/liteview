...
void DateBuiltinsAssembler::Generate_DatePrototype_GetField(TNode<Context> context, TNode<Object> receiver, int field_index)
{
    ...
    GotoIfNot(InstanceTypeEqual(receiver_instance_type, JS_DATE_TYPE), &receiver_not_date);
    ...
    if (field_index == JSDate::kDateValue) {
        ...
        if (field_index < JSDate::kFirstUncachedField) {
            ...
            Return(LoadObjectField(date_receiver, JSDate::kValueOffset + field_index * kTaggedSize));
...
TF_BUILTIN(DatePrototypeToPrimitive, CodeStubAssembler)
{
    ...
    GotoIf(TaggedEqual(CallBuiltin(Builtin::kStringEqual, context, hint, number_string), TrueConstant()), &hint_is_number);
    GotoIf(TaggedEqual(CallBuiltin(Builtin::kStringEqual, context, hint, default_string), TrueConstant()), &hint_is_string);
    GotoIf(TaggedEqual(CallBuiltin(Builtin::kStringEqual, context, hint, string_string), TrueConstant()), &hint_is_string);
    Goto(&hint_is_invalid);

    // Use the OrdinaryToPrimitive builtin to convert to a Number.
    BIND(&hint_is_number);
    {
        Callable callable = CodeFactory::OrdinaryToPrimitive(isolate(), OrdinaryToPrimitiveHint::kNumber);
    ...
    BIND(&hint_is_string);
    {
        Callable callable = CodeFactory::OrdinaryToPrimitive(isolate(), OrdinaryToPrimitiveHint::kString);
        TNode<Object> result = CallStub(callable, context, receiver);
        Return(result);
    }
