...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ SmiUntagUnsigned(rax, Operand(rbp, ConstructFrameConstants::kLengthOffset));
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type, Builtin entry_trampoline)
{
    ...
    ExternalReference pending_exception = ExternalReference::Create(IsolateAddressId::kPendingExceptionAddress, masm->isolate());
...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm)
{
    ...
    __ movzxwq(rcx, FieldOperand(rcx, SharedFunctionInfo::kFormalParameterCountOffset));
    ...
    if (v8_flags.debug_code) {
        ...
        GetSharedFunctionInfoBytecodeOrBaseline(masm, rcx, kScratchRegister, &is_baseline);
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ movl(params_size, FieldOperand(params_size, BytecodeArray::kParameterSizeOffset));
    ...
    __ leaq(actual_params_size, Operand(actual_params_size, times_system_pointer_size, 0));
...
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array) { __ movw(FieldOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset), Immediate(0)); }
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ andb(scratch, Immediate(FeedbackVector::MaybeHasOptimizedOsrCodeBit::kMask));
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    Label is_baseline;
    GetSharedFunctionInfoBytecodeOrBaseline(masm, kInterpreterBytecodeArrayRegister, kScratchRegister, &is_baseline);
    ...
    __ Move(Operand(rbp, InterpreterFrameConstants::kBytecodeOffsetFromFp), Smi::FromInt(BytecodeArray::kHeaderSize - kHeapObjectTag + kFunctionEntryBytecodeOffset));
...
void Builtins::Generate_InterpreterPushArgsThenCallImpl(MacroAssembler* masm, ConvertReceiverMode receiver_mode, InterpreterPushArgsMode mode)
{
    ...
    if (mode == InterpreterPushArgsMode::kWithFinalSpread) {
        ...
        __ Jump(masm->isolate()->builtins()->Call(receiver_mode), RelocInfo::CODE_TARGET);
    }
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm)
{
    ...
    __ movq(rbx, __ ExternalReferenceAsOperand(ExternalReference::address_of_interpreter_entry_trampoline_instruction_start(masm->isolate()), kScratchRegister));
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm)
{
    ...
    FrameScope frame_scope(masm, StackFrame::MANUAL);
    {
        ...
        __ Push(descriptor.GetRegisterParameter(BaselineOutOfLinePrologueDescriptor::kJavaScriptCallArgCount)); // Actual argument
                                                                                                                // count.
    ...
    __ bind(&flags_need_processing);
    {
        ...
        __ Drop(1);
        __ OptimizeCodeOrTailCallOptimizedCodeSlot(flags, feedback_vector, closure, JumpMode::kPushAndReturn);
        __ Trap();
    }
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
    ...
    if (with_result && java_script_builtin) {
        ...
        __ movq(Operand(rsp, rax, times_system_pointer_size, BuiltinContinuationFrameConstants::kFixedFrameSize - kJSArgcReceiverSlots * kSystemPointerSize), kScratchRegister);
    }
    ...
    __ j(not_zero, &done_convert);
    {
        ...
        if (mode == ConvertReceiverMode::kNullOrUndefined) {
            ...
            if (mode != ConvertReceiverMode::kNotNullOrUndefined) {
                ...
                __ JumpIfRoot(rcx, RootIndex::kUndefinedValue, &convert_global_proxy, Label::kNear);
                __ JumpIfNotRoot(rcx, RootIndex::kNullValue, &convert_to_object, Label::kNear);
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET, below_equal);

    __ cmpw(instance_type, Immediate(JS_BOUND_FUNCTION_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), CallBoundFunction), RelocInfo::CODE_TARGET, equal);
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), CallProxy), RelocInfo::CODE_TARGET, equal);

    // Check if target is a wrapped function and call CallWrappedFunction external
    // builtin
    __ cmpw(instance_type, Immediate(JS_WRAPPED_FUNCTION_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWrappedFunction), RelocInfo::CODE_TARGET, equal);
...
void Builtins::Generate_ConstructFunction(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), JSBuiltinsConstructStub), RelocInfo::CODE_TARGET, not_zero);
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET, below_equal);

    // Only dispatch to bound functions after checking whether they are
    // constructors.
    __ cmpw(instance_type, Immediate(JS_BOUND_FUNCTION_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructBoundFunction), RelocInfo::CODE_TARGET, equal);

    // Only dispatch to proxies after checking whether they are constructors.
    __ cmpw(instance_type, Immediate(JS_PROXY_TYPE));
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructProxy), RelocInfo::CODE_TARGET, equal);
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ LoadTaggedPointerField(deopt_data, FieldOperand(rax, Code::kDeoptimizationDataOrInterpreterDataOffset));

    // Load the OSR entrypoint offset from the deoptimization data.
    __ SmiUntagField(rbx, FieldOperand(deopt_data, FixedArray::OffsetOfElementAt(DeoptimizationData::kOsrPcOffsetIndex)));
...
{
    using D = i::CallInterfaceDescriptorFor<Builtin::kMaglevOutOfLinePrologue>::type;
    ...
    // before deoptimizing.
    {
        static constexpr int kCodeStartToCodeDataContainerOffset = Code::kCodeDataContainerOffset - Code::kHeaderSize;
        __ LoadTaggedPointerField(scratch0, Operand(kJavaScriptCallCodeStartRegister, kCodeStartToCodeDataContainerOffset));
        __ testl(FieldOperand(scratch0, CodeDataContainer::kKindSpecificFlagsOffset), Immediate(1 << Code::kMarkedForDeoptimizationBit));
    ...
    {
        __ LoadTaggedPointerField(feedback_vector, FieldOperand(kJSFunctionRegister, JSFunction::kFeedbackCellOffset));
        __ LoadTaggedPointerField(feedback_vector, FieldOperand(feedback_vector, Cell::kValueOffset));
        __ AssertFeedbackVector(feedback_vector);

        __ LoadFeedbackVectorFlagsAndJumpIfNeedsProcessing(flags, feedback_vector, CodeKind::MAGLEV, &optimize);
    }
    ...
    __ bind(&deoptimize);
    {
        ...
        __ Move(scratch0, BUILTIN_CODE(masm->isolate(), CompileLazyDeoptimizedCode));
    ...
    __ bind(&optimize);
    {
        ...
        __ OptimizeCodeOrTailCallOptimizedCodeSlot(flags, feedback_vector, kJSFunctionRegister, JumpMode::kPushAndReturn);
        __ Trap();
    }
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
...

    {
        ...
        static_assert(WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs + 1 == arraysize(wasm::kGpParamRegisters), "frame size mismatch");
        for (Register reg : wasm::kGpParamRegisters) {
            __ Push(reg);
        }
        static_assert(WasmCompileLazyFrameConstants::kNumberOfSavedFpParamRegs == arraysize(wasm::kFpParamRegisters), "frame size mismatch");
        ...
        __ addq(r15, MemOperand(kWasmInstanceRegister, wasm::ObjectAccess::ToTagged(WasmInstanceObject::kJumpTableStartOffset)));
    }
...
void LoadJumpBuffer(MacroAssembler* masm, Register jmpbuf, bool load_pc)
{
    ...
    SwitchStackState(masm, jmpbuf, wasm::JumpBuffer::Inactive, wasm::JumpBuffer::Active);
    if (load_pc) {
...
void AllocateSuspender(MacroAssembler* masm, Register function_data, Register wasm_instance)
{
    MemOperand GCScanSlotPlace = MemOperand(rbp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset);
...
void LoadTargetJumpBuffer(MacroAssembler* masm, Register target_continuation)
{
    ...
    __ LoadExternalPointerField(target_jmpbuf, FieldOperand(target_continuation, WasmContinuationObject::kJmpbufOffset), kWasmContinuationJmpbufTag, kScratchRegister);
    MemOperand GCScanSlotPlace = MemOperand(rbp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset);
...
void ReloadParentContinuation(MacroAssembler* masm, Register wasm_instance, Register return_reg, Register tmp1, Register tmp2)
{
    ...
    jmpbuf = parent;
    __ LoadExternalPointerField(jmpbuf, FieldOperand(parent, WasmContinuationObject::kJmpbufOffset), kWasmContinuationJmpbufTag, tmp1);
...
void LoadValueTypesArray(MacroAssembler* masm, Register function_data, Register valuetypes_array_ptr, Register return_count, Register param_count)
{
    ...
    __ LoadExternalPointerField(signature, FieldOperand(function_data, WasmExportedFunctionData::kSigOffset), kWasmExportedFunctionDataSignatureTag, kScratchRegister);
...
void GenericJSToWasmWrapperHelper(MacroAssembler* masm, bool stack_switch)
{
    ...
    // for all references.
    constexpr int kHasRefTypesOffset = kValueTypesArrayStartOffset - kSystemPointerSize;
    ...
    if (!stack_switch) {
        ...
        // -------------------------------------------
        __ SmiAddConstant(MemOperand(function_data, WasmExportedFunctionData::kWrapperBudgetOffset - kHeapObjectTag), Smi::FromInt(-1));
    ...
    constexpr int kIntegerSectionStartOffset = kLastSpillOffset - kSystemPointerSize;
    // For Integer section.
    ...
    __ Move(current_param, kFPOnStackSize + kPCOnStackSize + kReceiverOnStackSize);
    __ movq(param_limit, param_count);
    __ shlq(param_limit, Immediate(kSystemPointerSizeLog2));
    __ addq(param_limit, Immediate(kFPOnStackSize + kPCOnStackSize + kReceiverOnStackSize));
    ...
    __ LoadExternalPointerField(function_entry, FieldOperand(function_entry, WasmInternalFunction::kCallTargetOffset), kWasmInternalFunctionCallTargetTag, scratch);
    function_data = no_reg;
    scratch = no_reg;
    ...
    PrepareForBuiltinCall(masm, MemOperand(rbp, kGCScanSlotCountOffset), kBuiltinCallGCScanSlotCount, current_param, param_limit, current_int_param_slot, current_float_param_slot,
    ...
    __ movq(MemOperand(rbp, kHasRefTypesOffset), Immediate(1));
    RestoreAfterBuiltinCall(masm, function_data, wasm_instance, valuetypes_array_ptr, current_float_param_slot, current_int_param_slot, param_limit, current_param, original_fp);
...
void Builtins::Generate_WasmSuspend(MacroAssembler* masm)
{
    ...
    __ subq(rsp, Immediate(-(BuiltinWasmWrapperConstants::kGCScanSlotCountOffset - TypedFrameConstants::kFixedFrameSizeFromFp)));
    ...
    SwitchStackState(masm, jmpbuf, wasm::JumpBuffer::Active, wasm::JumpBuffer::Inactive);
...
void Generate_WasmResumeHelper(MacroAssembler* masm, wasm::OnResume on_resume)
{
    ...
    __ LoadRoot(kScratchRegister, RootIndex::kUndefinedValue);
    __ movq(MemOperand(rbp, BuiltinWasmWrapperConstants::kSuspenderOffset), kScratchRegister);
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, SaveFPRegsMode save_doubles, ArgvMode argv_mode, bool builtin_exit_frame)
{
    ...
    if (argv_mode == ArgvMode::kRegister) {
        ...
        __ EnterExitFrame(arg_stack_space, save_doubles == SaveFPRegsMode::kSave, builtin_exit_frame ? StackFrame::BUILTIN_EXIT : StackFrame::EXIT);
    }
    ...
    if (v8_flags.debug_code) {
        ...
        ExternalReference pending_exception_address = ExternalReference::Create(IsolateAddressId::kPendingExceptionAddress, masm->isolate());
    ...
    __ LeaveExitFrame(save_doubles == SaveFPRegsMode::kSave, argv_mode == ArgvMode::kStack);
    __ ret(0);
    ...
    ExternalReference num_frames_above_pending_handler_address = ExternalReference::Create(IsolateAddressId::kNumFramesAbovePendingHandlerAddress, masm->isolate());
    ...
    ExternalReference c_entry_fp_address = ExternalReference::Create(IsolateAddressId::kCEntryFPAddress, masm->isolate());
...
void CallApiFunctionAndReturn(MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, Register thunk_last_arg, int stack_space, Operand* stack_space_operand,
    Operand return_value_operand)
{
    ...
    const int kLimitOffset = Offset(ExternalReference::handle_scope_limit_address(isolate), next_address);
    const int kLevelOffset = Offset(ExternalReference::handle_scope_level_address(isolate), next_address);
    ...
    __ cmpb(__ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), rax), Immediate(0));
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    DCHECK(!AreAliased(api_function_address, argc, holder, call_data, kScratchRegister));
    ...
    // on the stack).
    __ leaq(scratch, Operand(scratch, (FCA::kArgsLength + 1) * kSystemPointerSize));
    ...
    __ leaq(kScratchRegister, Operand(argc, times_system_pointer_size, (FCA::kArgsLength + 1 /* receiver */) * kSystemPointerSize));
    ...
    Operand return_value_operand(rbp, (kStackSlotsAboveFCA + FCA::kReturnValueOffset) * kSystemPointerSize);

    static constexpr int kUseStackSpaceOperand = 0;
    Operand stack_space_operand = StackSpaceOperand(3);
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, callback_arg, kUseStackSpaceOperand, &stack_space_operand, return_value_operand);
}
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    __ Push(receiver);
    __ PushTaggedAnyField(FieldOperand(callback, AccessorInfo::kDataOffset), decompr_scratch1);
    __ LoadRoot(kScratchRegister, RootIndex::kUndefinedValue);
    ...
    __ PushTaggedPointerField(FieldOperand(callback, AccessorInfo::kNameOffset), decompr_scratch1);
    ...
    __ LoadExternalPointerField(api_function_address, FieldOperand(callback, AccessorInfo::kMaybeRedirectedGetterOffset), kAccessorInfoGetterTag, kScratchRegister);

    // +3 is to skip prolog, return address and name handle.
    Operand return_value_operand(rbp, (PropertyCallbackArguments::kReturnValueOffset + 3) * kSystemPointerSize);
    Operand* const kUseStackSpaceConstant = nullptr;
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, getter_arg, kStackUnwindSpace, kUseStackSpaceConstant, return_value_operand);
}
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    // FrameDescription.
    static constexpr int kDoubleRegsSize = kDoubleSize * XMMRegister::kNumRegisters;
    ...
    __ movb(__ ExternalReferenceAsOperand(ExternalReference::stack_is_iterable_address(isolate)), Immediate(0));
    ...
    __ movb(__ ExternalReferenceAsOperand(ExternalReference::stack_is_iterable_address(isolate)), Immediate(1));
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ movq(MemOperand(rbp, InterpreterFrameConstants::kBytecodeOffsetFromFp), feedback_vector);
    ...
    __ leaq(code_obj, FieldOperand(code_obj, kReturnRegister0, times_1, Code::kHeaderSize));
