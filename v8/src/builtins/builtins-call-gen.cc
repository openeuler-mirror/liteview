...
void Builtins::Generate_CallFunction_ReceiverIsNullOrUndefined(MacroAssembler* masm) { Generate_CallFunction(masm, ConvertReceiverMode::kNullOrUndefined); }

void Builtins::Generate_CallFunction_ReceiverIsNotNullOrUndefined(MacroAssembler* masm) { Generate_CallFunction(masm, ConvertReceiverMode::kNotNullOrUndefined); }

void Builtins::Generate_CallFunction_ReceiverIsAny(MacroAssembler* masm) { Generate_CallFunction(masm, ConvertReceiverMode::kAny); }

void Builtins::Generate_CallBoundFunction(MacroAssembler* masm) { Generate_CallBoundFunctionImpl(masm); }

void Builtins::Generate_Call_ReceiverIsNullOrUndefined(MacroAssembler* masm) { Generate_Call(masm, ConvertReceiverMode::kNullOrUndefined); }

void Builtins::Generate_Call_ReceiverIsNotNullOrUndefined(MacroAssembler* masm) { Generate_Call(masm, ConvertReceiverMode::kNotNullOrUndefined); }

void Builtins::Generate_Call_ReceiverIsAny(MacroAssembler* masm) { Generate_Call(masm, ConvertReceiverMode::kAny); }

void Builtins::Generate_CallVarargs(MacroAssembler* masm) { Generate_CallOrConstructVarargs(masm, masm->isolate()->builtins()->Call()); }

void Builtins::Generate_CallForwardVarargs(MacroAssembler* masm) { Generate_CallOrConstructForwardVarargs(masm, CallOrConstructMode::kCall, masm->isolate()->builtins()->Call()); }

void Builtins::Generate_CallFunctionForwardVarargs(MacroAssembler* masm)
{
    Generate_CallOrConstructForwardVarargs(masm, CallOrConstructMode::kCall, masm->isolate()->builtins()->CallFunction());
}
...
TF_BUILTIN(Call_ReceiverIsNotNullOrUndefined_Baseline_Compact, CallOrConstructBuiltinsAssembler) { CallReceiver<Descriptor>(Builtin::kCall_ReceiverIsNotNullOrUndefined); }
...
TF_BUILTIN(Call_ReceiverIsAny_Baseline_Compact, CallOrConstructBuiltinsAssembler) { CallReceiver<Descriptor>(Builtin::kCall_ReceiverIsAny); }
...
void CallOrConstructBuiltinsAssembler::CallOrConstructWithArrayLike(
    TNode<Object> target, base::Optional<TNode<Object>> new_target, TNode<Object> arguments_list, TNode<Context> context)
{
    ...
    BIND(&if_runtime);
    {
        ...
        var_length = LoadAndUntagToWord32ObjectField(var_elements.value(), FixedArray::kLengthOffset);
        Goto(&if_done);
    }
    ...
    BIND(&if_done);
    {
        ...
        BIND(&if_not_double);
        {
            if (!new_target) {
                ...
                TailCallStub(callable, context, target, *new_target, args_count, length, elements);
            }
        }
...
void CallOrConstructBuiltinsAssembler::CallOrConstructDoubleVarargs(TNode<Object> target, base::Optional<TNode<Object>> new_target, TNode<FixedDoubleArray> elements,
    TNode<Int32T> length, TNode<Int32T> args_count, TNode<Context> context, TNode<Int32T> kind)
{
    ...
    TNode<FixedArray> new_elements = CAST(AllocateFixedArray(new_kind, intptr_length, AllocationFlag::kAllowLargeObjectAllocation));
    ...
    if (!new_target) {
        ...
        Callable callable = CodeFactory::ConstructVarargs(isolate());
        TailCallStub(callable, context, target, *new_target, args_count, length, new_elements);
    }
}

void CallOrConstructBuiltinsAssembler::CallOrConstructWithSpread(
    TNode<Object> target, base::Optional<TNode<Object>> new_target, TNode<Object> spread, TNode<Int32T> args_count, TNode<Context> context)
{
    ...
    BIND(&if_generic);
    {
        Label if_iterator_fn_not_callable(this, Label::kDeferred), if_iterator_is_null_or_undefined(this, Label::kDeferred), throw_spread_error(this, Label::kDeferred);
...
template <class Descriptor> void CallOrConstructBuiltinsAssembler::CallReceiver(Builtin id, base::Optional<TNode<Object>> receiver)
{
...
template <class Descriptor> void CallOrConstructBuiltinsAssembler::CallReceiver(Builtin id, TNode<Int32T> argc, TNode<UintPtrT> slot, base::Optional<TNode<Object>> maybe_receiver)
{
    auto target = Parameter<Object>(Descriptor::kFunction);
...
TNode<JSReceiver> CallOrConstructBuiltinsAssembler::GetCompatibleReceiver(TNode<JSReceiver> receiver, TNode<HeapObject> signature, TNode<Context> context)
{
    ...
    BIND(&holder_loop);
    {
        ...
        BIND(&template_from_closure);
        {
            ...
            TNode<Object> template_data = LoadObjectField(template_shared, SharedFunctionInfo::kFunctionDataOffset);
...
void CallOrConstructBuiltinsAssembler::CallFunctionTemplate(
    CallFunctionTemplateMode mode, TNode<FunctionTemplateInfo> function_template_info, TNode<IntPtrT> argc, TNode<Context> context)
{
    CodeStubArguments args(this, argc);
    ...
    if (mode == CallFunctionTemplateMode::kCheckAccess || mode == CallFunctionTemplateMode::kCheckAccessAndCompatibleReceiver) {
        ...
        TNode<IntPtrT> function_template_info_flags = LoadAndUntagObjectField(function_template_info, FunctionTemplateInfo::kFlagOffset);
        Branch(IsSetWord(function_template_info_flags, 1 << FunctionTemplateInfo::AcceptAnyReceiverBit::kShift), &receiver_done, &receiver_needs_access_check);
    ...
    TNode<CallHandlerInfo> call_handler_info = LoadObjectField<CallHandlerInfo>(function_template_info, FunctionTemplateInfo::kCallCodeOffset);
    ...
    TailCallStub(CodeFactory::CallApiCallback(isolate()), context, callback, args.GetLengthWithoutReceiver(), call_data, holder);
}
...
TF_BUILTIN(CallFunctionTemplate_CheckAccess, CallOrConstructBuiltinsAssembler)
{
    ...
    CallFunctionTemplate(CallFunctionTemplateMode::kCheckAccess, function_template_info, argc, context);
}
...
TF_BUILTIN(CallFunctionTemplate_CheckCompatibleReceiver, CallOrConstructBuiltinsAssembler)
{
    ...
    auto function_template_info = UncheckedParameter<FunctionTemplateInfo>(Descriptor::kFunctionTemplateInfo);
    auto argc = UncheckedParameter<IntPtrT>(Descriptor::kArgumentsCount);
    CallFunctionTemplate(CallFunctionTemplateMode::kCheckCompatibleReceiver, function_template_info, argc, context);
}
...
TF_BUILTIN(CallFunctionTemplate_CheckAccessAndCompatibleReceiver, CallOrConstructBuiltinsAssembler)
{
    ...
    auto function_template_info = UncheckedParameter<FunctionTemplateInfo>(Descriptor::kFunctionTemplateInfo);
    auto argc = UncheckedParameter<IntPtrT>(Descriptor::kArgumentsCount);
    CallFunctionTemplate(CallFunctionTemplateMode::kCheckAccessAndCompatibleReceiver, function_template_info, argc, context);
}
