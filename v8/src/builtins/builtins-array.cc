...
inline bool IsJSArrayFastElementMovingAllowed(Isolate* isolate, JSArray receiver) { return JSObject::PrototypeHasNoElements(isolate, receiver); }

inline bool HasSimpleElements(JSObject current) { return !current.map().IsCustomElementsReceiverMap() && !current.GetElementsAccessor()->HasAccessors(current); }
...
V8_WARN_UNUSED_RESULT MaybeHandle<Object> SetLengthProperty(Isolate* isolate, Handle<JSReceiver> receiver, double length)
{
    ...
    return Object::SetProperty(
        isolate, receiver, isolate->factory()->length_string(), isolate->factory()->NewNumber(length), StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError));
}

V8_WARN_UNUSED_RESULT Object GenericArrayFill(Isolate* isolate, Handle<JSReceiver> receiver, Handle<Object> value, double start, double end)
{
...
namespace {
V8_WARN_UNUSED_RESULT Object GenericArrayPush(Isolate* isolate, BuiltinArguments* args)
{
    // 1. Let O be ? ToObject(this value).
    Handle<JSReceiver> receiver;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, args->receiver()));
    ...
    if (arg_count > kMaxSafeInteger - length) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kPushPastSafeLength, isolate->factory()->NewNumberFromInt(arg_count), raw_length_number));
    }
...
V8_WARN_UNUSED_RESULT Object GenericArrayPop(Isolate* isolate, BuiltinArguments* args)
{
    // 1. Let O be ? ToObject(this value).
    Handle<JSReceiver> receiver;
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, receiver, Object::ToObject(isolate, args->receiver()));
    ...
    if (length == 0) {
        ...
        RETURN_FAILURE_ON_EXCEPTION(isolate,
            Object::SetProperty(
                isolate, receiver, isolate->factory()->length_string(), Handle<Smi>(Smi::zero(), isolate), StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)));
    ...
    MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(receiver, index, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());
...
V8_WARN_UNUSED_RESULT bool CanUseFastArrayShift(Isolate* isolate, Handle<JSReceiver> receiver)
{
    ...
    if (!EnsureJSArrayWithWritableFastElements(isolate, receiver, nullptr, 0, 0) || !IsJSArrayFastElementMovingAllowed(isolate, JSArray::cast(*receiver))) {
        return false;
    }
...
V8_WARN_UNUSED_RESULT Object GenericArrayShift(Isolate* isolate, Handle<JSReceiver> receiver, double length)
{
    ...
    while (k < length) {
        ...
        if (from_present) {
            ...
            MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(receiver, to, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());
        }
    ...
    MAYBE_RETURN(JSReceiver::DeletePropertyOrElement(receiver, new_length, LanguageMode::kStrict), ReadOnlyRoots(isolate).exception());
...
public:
    ArrayConcatVisitor(Isolate* isolate, Handle<HeapObject> storage, bool fast_elements)
        : isolate_(isolate)
        , storage_(isolate->global_handles()->Create(*storage))
        , index_offset_(0u)
        , bit_field_(FastElementsField::encode(fast_elements) | ExceedsLimitField::encode(false) | IsFixedArrayField::encode(storage->IsFixedArray(isolate))
              | HasSimpleElementsField::encode(storage->IsFixedArray(isolate) ||
                  // Don't take fast path for storages that might have
                  // side effects when storing to them.
                  (!storage->map(isolate).IsCustomElementsReceiverMap() && !storage->IsJSTypedArray(isolate))))
    {
    ...
    V8_WARN_UNUSED_RESULT bool visit(uint32_t i, Handle<Object> elm)
    {
        ...
        if (!is_fixed_array()) {
            ...
            MAYBE_RETURN(JSReceiver::CreateDataProperty(&it, elm, Just(kThrowOnError)), false);
            return true;
        }
    ...
    bool exceeds_array_limit() const { return ExceedsLimitField::decode(bit_field_); }
    ...
    Handle<JSArray> ToArray()
    {
        ...
        Handle<Map> map = JSObject::GetElementsTransitionMap(array, fast_elements() ? HOLEY_ELEMENTS : DICTIONARY_ELEMENTS);
        {
            DisallowGarbageCollection no_gc;
            auto raw = *array;
    ...
    V8_WARN_UNUSED_RESULT MaybeHandle<JSReceiver> ToJSReceiver()
    {
        ...
        Handle<Object> length = isolate_->factory()->NewNumber(static_cast<double>(index_offset_));
        RETURN_ON_EXCEPTION(
            isolate_, Object::SetProperty(isolate_, result, isolate_->factory()->length_string(), length, StoreOrigin::kMaybeKeyed, Just(ShouldThrow::kThrowOnError)), JSReceiver);
        return result;
    }
    bool has_simple_elements() const { return HasSimpleElementsField::decode(bit_field_); }
...
private:
    ...
    void set_fast_elements(bool fast) { bit_field_ = FastElementsField::update(bit_field_, fast); }
    void set_exceeds_array_limit(bool exceeds) { bit_field_ = ExceedsLimitField::update(bit_field_, exceeds); }
...
#undef TYPED_ARRAY_CASE
    ...
    case SLOW_STRING_WRAPPER_ELEMENTS: {
        ...
        Handle<JSPrimitiveWrapper> js_value = Handle<JSPrimitiveWrapper>::cast(object);
...
static Maybe<bool> IsConcatSpreadable(Isolate* isolate, Handle<Object> obj)
{
    ...
    if (!obj->IsJSReceiver())
        return Just(false);
    if (!Protectors::IsIsConcatSpreadableLookupChainIntact(isolate) || JSReceiver::cast(*obj).HasProxyInPrototype(isolate)) {
...
Object Slow_ArrayConcat(BuiltinArguments* args, Handle<Object> species, Isolate* isolate)
{
    ...
    FOR_WITH_HANDLE_SCOPE(isolate, int, i = 0, i, i < argument_count, i++, {
        ...
        if (obj->IsJSArray()) {
            ...
            if (obj->IsHeapObject()) {
                kind = GetMoreGeneralElementsKind(kind, obj->IsNumber() ? PACKED_DOUBLE_ELEMENTS : PACKED_ELEMENTS);
            }
            length_estimate = 1;
            element_estimate = 1;
        }
    ...
    int result_len = 0;
    {
        ...
        for (int i = 0; i < n_arguments; i++) {
            ...
            if (FixedDoubleArray::kMaxLength < result_len || FixedArray::kMaxLength < result_len) {
                ...
                THROW_NEW_ERROR(isolate, NewRangeError(MessageTemplate::kInvalidArrayLength), JSArray);
            }
        }
    }
...
BUILTIN(ArrayConcat)
{
    ...
    BuiltinArguments::ChangeValueScope set_receiver_value_scope(isolate, &args, BuiltinArguments::kReceiverOffset, *receiver);

    Handle<JSArray> result_array;

    // Avoid a real species read to avoid extra lookups to the array constructor
    if (V8_LIKELY(receiver->IsJSArray() && Handle<JSArray>::cast(receiver)->HasArrayPrototype(isolate) && Protectors::IsArraySpeciesLookupChainIntact(isolate))) {
...
inline Handle<OrderedHashMap> AddValueToKeyedGroup(Isolate* isolate, Handle<OrderedHashMap> groups, Handle<Object> key, Handle<Object> value)
{
    ...
    Handle<ArrayList> array = Handle<ArrayList>(ArrayList::cast(groups->ValueAt(entry)), isolate);
    array = ArrayList::Add(isolate, array, value);
...
inline bool IsFastArray(Handle<JSReceiver> object)
{
    ...
    if (array->map().prototype() != context.get(Context::INITIAL_ARRAY_PROTOTYPE_INDEX)) {
        return false;
    }
...
inline bool CheckArrayMapNotModified(Handle<JSArray> array, Handle<Map> original_map)
{
...
inline MaybeHandle<OrderedHashMap> GenericArrayGroup(
    Isolate* isolate, Handle<JSReceiver> O, Handle<Object> callbackfn, Handle<Object> thisArg, Handle<OrderedHashMap> groups, double initialK, double len)
{
    // 6. Repeat, while k < len
    for (double k = initialK; k < len; ++k) {
        // 6a. Let Pk be ! ToString(𝔽(k)).
        Handle<Name> Pk;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, Pk, Object::ToName(isolate, isolate->factory()->NewNumber(k)), OrderedHashMap);
        // 6b. Let kValue be ? Get(O, Pk).
        Handle<Object> kValue;
        ASSIGN_RETURN_ON_EXCEPTION(isolate, kValue, Object::GetPropertyOrElement(isolate, O, Pk), OrderedHashMap);
        ...
        ASSIGN_RETURN_ON_EXCEPTION(isolate, propertyKey, Execution::Call(isolate, callbackfn, thisArg, 3, argv), OrderedHashMap);
        ...
        if (mode == ArrayGroupMode::kToMap) {
            ...
            ASSIGN_RETURN_ON_EXCEPTION(isolate, propertyKeyName, Object::ToName(isolate, propertyKey), OrderedHashMap);
...
inline MaybeHandle<OrderedHashMap> FastArrayGroup(
    Isolate* isolate, Handle<JSArray> array, Handle<Object> callbackfn, Handle<Object> thisArg, Handle<OrderedHashMap> groups, double len, ElementsKind* result_elements_kind)
{
    ...
    for (InternalIndex k : InternalIndex::Range(uint_len)) {
        if (!CheckArrayMapNotModified(array, original_map) || k.as_uint32() >= static_cast<uint32_t>(array->length().Number())) {
            return GenericArrayGroup<mode>(isolate, array, callbackfn, thisArg, groups, k.as_uint32(), len);
        }
        // 6a. Let Pk be ! ToString(𝔽(k)).
        // 6b. Let kValue be ? Get(O, Pk).
        ...
        // 6c. Let key be ? Call(callbackfn, thisArg, « kValue, 𝔽(k), O »).
        Handle<Object> propertyKey;
        Handle<Object> argv[] = { kValue, isolate->factory()->NewNumber(k.as_uint32()), array };
        ASSIGN_RETURN_ON_EXCEPTION(isolate, propertyKey, Execution::Call(isolate, callbackfn, thisArg, 3, argv), OrderedHashMap);
...
BUILTIN(ArrayPrototypeGroup)
{
    ...
    Handle<JSReceiver> O;
    // 1. Let O be ? ToObject(this value).
    ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, O, Object::ToObject(isolate, args.receiver(), kMethodName));

    // 2. Let len be ? LengthOfArrayLike(O).
    double len;
    MAYBE_ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, len, GetLengthProperty(isolate, O));

    // 3. If IsCallable(callbackfn) is false, throw a TypeError exception.
    Handle<Object> callbackfn = args.atOrUndefined(isolate, 1);
    if (!callbackfn->IsCallable()) {
        THROW_NEW_ERROR_RETURN_FAILURE(isolate, NewTypeError(MessageTemplate::kCalledNonCallable, callbackfn));
    }
    ...
    if (IsFastArray(O)) {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, groups, FastArrayGroup<ArrayGroupMode::kToObject>(isolate, array, callbackfn, thisArg, groups, len, &result_elements_kind));
    } else {
        // 4. Let k be 0.
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, groups, GenericArrayGroup<ArrayGroupMode::kToObject>(isolate, O, callbackfn, thisArg, groups, 0, len));
    }
    ...
    for (InternalIndex entry : groups->IterateEntries()) {
        ...
        Handle<ArrayList> array_list = Handle<ArrayList>(ArrayList::cast(groups->ValueAt(entry)), isolate);
        Handle<FixedArray> elements = ArrayList::Elements(isolate, array_list);
        Handle<JSArray> array = isolate->factory()->NewJSArrayWithElements(elements, result_elements_kind, array_list->Length());

        // 8b. Perform ! CreateDataPropertyOrThrow(obj, g.[[Key]], elements).
        JSReceiver::CreateDataProperty(isolate, obj, key, array, Just(kThrowOnError)).Check();
    }
...
BUILTIN(ArrayPrototypeGroupToMap)
{
    ...
    if (IsFastArray(O)) {
        ...
        // 4. Let k be 0.
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, groups, GenericArrayGroup<ArrayGroupMode::kToMap>(isolate, O, callbackfn, thisArg, groups, 0, len));
    }
    ...
    for (InternalIndex entry : groups->IterateEntries()) {
        ...
        map_table = OrderedHashMap::Add(isolate, map_table, key, array).ToHandleChecked();
    }
