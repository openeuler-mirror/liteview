...
static V8_INLINE bool CheckForName(Isolate* isolate, Handle<Name> name, Handle<String> property_name, int offset, FieldIndex::Encoding encoding, FieldIndex* index)
{
...
bool Accessors::IsJSObjectFieldAccessor(Isolate* isolate, Handle<Map> map, Handle<Name> name, FieldIndex* index)
{
    ...
    default:
        if (map->instance_type() < FIRST_NONSTRING_TYPE) {
            return CheckForName(isolate, name, isolate->factory()->length_string(), String::kLengthOffset, FieldIndex::kWord32, index);
        }
...
V8_WARN_UNUSED_RESULT MaybeHandle<Object> Accessors::ReplaceAccessorWithDataProperty(
    Isolate* isolate, Handle<Object> receiver, Handle<JSObject> holder, Handle<Name> name, Handle<Object> value)
{
...
void Accessors::ArrayLengthSetter(v8::Local<v8::Name> name, v8::Local<v8::Value> val, const v8::PropertyCallbackInfo<v8::Boolean>& info)
{
    ...
    DCHECK(Utils::OpenHandle(*name)->SameValue(ReadOnlyRoots(isolate).length_string()));
    ...
    if (!was_readonly && V8_UNLIKELY(JSArray::HasReadOnlyLength(array))) {
        ...
        if (length == array->length().Number()) {
            ...
            Factory* factory = isolate->factory();
            isolate->Throw(*factory->NewTypeError(MessageTemplate::kStrictReadOnlyProperty, Utils::OpenHandle(*name), i::Object::TypeOf(isolate, object), object));
            isolate->OptionalRescheduleException(false);
        } else {
...
Handle<AccessorInfo> Accessors::MakeArrayLengthInfo(Isolate* isolate) { return MakeAccessor(isolate, isolate->factory()->length_string(), &ArrayLengthGetter, &ArrayLengthSetter); }

//
// Accessors::SharedArrayLength
//

void Accessors::SharedArrayLengthGetter(v8::Local<v8::Name> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    i::Isolate* isolate = reinterpret_cast<i::Isolate*>(info.GetIsolate());
    DisallowGarbageCollection no_gc;
    HandleScope scope(isolate);
...
{
    return MakeAccessor(isolate, isolate->factory()->length_string(), &SharedArrayLengthGetter, nullptr);
}
...
void Accessors::ModuleNamespaceEntryGetter(v8::Local<v8::Name> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    ...
    if (!holder.GetExport(isolate, Handle<String>::cast(Utils::OpenHandle(*name))).ToHandle(&result)) {
        isolate->OptionalRescheduleException(false);
    } else {
...
void Accessors::StringLengthGetter(v8::Local<v8::Name> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    ...
    if (!value.IsString()) {
        ...
        value = JSPrimitiveWrapper::cast(*Utils::OpenHandle(*info.Holder())).value();
    }
    Object result = Smi::FromInt(String::cast(value).length());
    info.GetReturnValue().Set(Utils::ToLocal(Handle<Object>(result, isolate)));
}

Handle<AccessorInfo> Accessors::MakeStringLengthInfo(Isolate* isolate) { return MakeAccessor(isolate, isolate->factory()->length_string(), &StringLengthGetter, nullptr); }
...
Handle<JSObject> ArgumentsForInlinedFunction(JavaScriptFrame* frame, int inlined_frame_index)
{
...
Handle<JSObject> GetFrameArguments(Isolate* isolate, JavaScriptFrameIterator* it, int function_index)
{
...
static inline bool AllowAccessToFunction(Context current_context, JSFunction function) { return current_context.HasSameSecurityTokenAs(function.context()); }
...
Handle<AccessorInfo> Accessors::MakeFunctionCallerInfo(Isolate* isolate) { return MakeAccessor(isolate, isolate->factory()->caller_string(), &FunctionCallerGetter, nullptr); }
...
void Accessors::WrappedFunctionLengthGetter(v8::Local<v8::Name> name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    ...
    RCS_SCOPE(isolate, RuntimeCallCounterId::kBoundFunctionLengthGetter);
    HandleScope scope(isolate);
    Handle<JSWrappedFunction> function = Handle<JSWrappedFunction>::cast(Utils::OpenHandle(*info.Holder()));
...
void Accessors::ErrorStackGetter(v8::Local<v8::Name> key, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    Isolate* isolate = reinterpret_cast<Isolate*>(info.GetIsolate());
    HandleScope scope(isolate);
    Handle<Object> formatted_stack;
    Handle<JSObject> error_object = Handle<JSObject>::cast(Utils::OpenHandle(*info.Holder()));
    if (!ErrorUtils::GetFormattedStack(isolate, error_object).ToHandle(&formatted_stack)) {
        isolate->OptionalRescheduleException(false);
        return;
    }
...
void Accessors::ErrorStackSetter(v8::Local<v8::Name> name, v8::Local<v8::Value> value, const v8::PropertyCallbackInfo<v8::Boolean>& info)
{
    Isolate* isolate = reinterpret_cast<Isolate*>(info.GetIsolate());
    HandleScope scope(isolate);
    Handle<JSObject> error_object = Handle<JSObject>::cast(Utils::OpenHandle(*info.Holder()));
    ErrorUtils::SetFormattedStack(isolate, error_object, Utils::OpenHandle(*value));
}

Handle<AccessorInfo> Accessors::MakeErrorStackInfo(Isolate* isolate) { return MakeAccessor(isolate, isolate->factory()->stack_string(), &ErrorStackGetter, &ErrorStackSetter); }
