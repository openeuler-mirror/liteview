...
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array, Register scratch)
{
    DCHECK(!AreAliased(bytecode_array, scratch));
    __ mov(r0, Operand(0));
    __ StoreU16(r0, FieldMemOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset), scratch);
}
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ AndP(scratch, scratch, Operand(FeedbackVector::MaybeHasOptimizedOsrCodeBit::kMask));
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    __ StoreU64(feedback_vector, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ LoadTaggedPointerField(r3, FieldMemOperand(r2, Code::kDeoptimizationDataOrInterpreterDataOffset));
...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ JumpIfIsInRange(r6, static_cast<uint8_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint8_t>(FunctionKind::kDerivedConstructor), &not_create_implicit_receiver);
...
constexpr int kPushedStackSpace
    = (kNumCalleeSaved + 2) * kSystemPointerSize + kNumCalleeSavedDoubles * kDoubleSize + 5 * kSystemPointerSize + EntryFrameConstants::kCallerFPOffset - kSystemPointerSize;
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type, Builtin entry_trampoline)
{
    ...
    __ Move(r1, ExternalReference::Create(IsolateAddressId::kCEntryFPAddress, masm->isolate()));
    ...
    __ lay(fp, MemOperand(sp, -EntryFrameConstants::kCallerFPOffset + kSystemPointerSize));
    pushed_stack_space += EntryFrameConstants::kCallerFPOffset - kSystemPointerSize;
    ...
    __ Move(scrach, ExternalReference::Create(IsolateAddressId::kPendingExceptionAddress, masm->isolate()));
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ LoadU32(params_size, FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));

    Register actual_params_size = scratch2;
    // Compute the size of the actual parameters + receiver (in bytes).
    __ LoadU64(actual_params_size, MemOperand(fp, StandardFrameConstants::kArgCOffset));
    __ ShiftLeftU64(actual_params_size, actual_params_size, Operand(kSystemPointerSizeLog2));
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm)
{
    ...
    // Increment invocation count for the function.
    {
        ...
        __ LoadU64(invocation_count, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
        __ AddU64(invocation_count, Operand(1));
        __ StoreU64(invocation_count, FieldMemOperand(feedback_vector, FeedbackVector::kInvocationCountOffset));
    }
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    Label is_baseline;
    GetSharedFunctionInfoBytecodeOrBaseline(masm, kInterpreterBytecodeArrayRegister, ip, &is_baseline);

    // The bytecode array could have been flushed from the shared function info,
    // if so, call into CompileLazy.
    Label compile_lazy;
    __ CompareObjectType(kInterpreterBytecodeArrayRegister, r6, no_reg, BYTECODE_ARRAY_TYPE);
    __ bne(&compile_lazy);
...
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
    ...
    if (with_result) {
        if (java_script_builtin) {
            ...
            __ StoreU64(r2, MemOperand(sp, config->num_allocatable_general_registers() * kSystemPointerSize + BuiltinContinuationFrameConstants::kFixedFrameSize));
        }
    }
...
void Builtins::Generate_CallOrConstructVarargs(MacroAssembler* masm, Handle<Code> code)
{
    ...
    // Push arguments onto the stack (thisArgument is already on the stack).
    {
        ...
        __ CmpS64(r6, Operand::Zero());
        __ beq(&no_args);
        __ AddS64(r4, r4, Operand(FixedArray::kHeaderSize - kHeapObjectTag - kTaggedSize));
        __ mov(r1, r6);
        __ bind(&loop);
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET, le);
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET, le);
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
...

    {
        ...
        CHECK_EQ(WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs + 1, gp_regs.Count());
        CHECK_EQ(WasmCompileLazyFrameConstants::kNumberOfSavedFpParamRegs, fp_regs.Count());
        ...
        __ LoadU64(r0, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
        __ AddS64(ip, ip, r0);
    }
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, SaveFPRegsMode save_doubles, ArgvMode argv_mode, bool builtin_exit_frame)
{
    ...
    __ EnterExitFrame(save_doubles == SaveFPRegsMode::kSave, arg_stack_space, builtin_exit_frame ? StackFrame::BUILTIN_EXIT : StackFrame::EXIT);
...
static int AddressOffset(ExternalReference ref0, ExternalReference ref1) { return ref0.address() - ref1.address(); }
    ...
    MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, int stack_space, MemOperand* stack_space_operand, MemOperand return_value_operand)
{
    ...
    __ LoadU8(scratch, __ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), scratch));
    __ CmpS64(scratch, Operand::Zero());
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    __ StoreU64(scratch, MemOperand(sp, (kStackFrameExtraParamSlot + 1) * kSystemPointerSize));

    // FunctionCallbackInfo::values_ (points at the first varargs argument passed
    // on the stack).
    __ AddS64(scratch, scratch, Operand((FCA::kArgsLength + 1) * kSystemPointerSize));
    __ StoreU64(scratch, MemOperand(sp, (kStackFrameExtraParamSlot + 2) * kSystemPointerSize));

    // FunctionCallbackInfo::length_.
    __ StoreU32(argc, MemOperand(sp, (kStackFrameExtraParamSlot + 3) * kSystemPointerSize));
    ...
    __ StoreU64(scratch, MemOperand(sp, (kStackFrameExtraParamSlot + 4) * kSystemPointerSize));

    // v8::InvocationCallback's argument.
    __ lay(r2, MemOperand(sp, (kStackFrameExtraParamSlot + 1) * kSystemPointerSize));
    ...
    MemOperand stack_space_operand(sp, (kStackFrameExtraParamSlot + 4) * kSystemPointerSize);
