...
protected:
    ...
    // supported.
    void AddToDictionaryIf(
        TNode<BoolT> condition, TNode<Context> context, TNode<Object> object, TNode<HeapObject> name_dictionary, Handle<Name> name, TNode<Object> value, Label* bailout);
...
protected:
    ...
    TNode<Uint32T> LoadPropertyKind(TNode<Uint32T> details) { return DecodeWord32<PropertyDetails::KindField>(details); }
...
void ObjectBuiltinsAssembler::ReturnToStringFormat(TNode<Context> context, TNode<String> string)
{
    ...
    Return(CallStub(callable, context, CallStub(callable, context, lhs, string), rhs));
}
...
TNode<BoolT> ObjectEntriesValuesBuiltinsAssembler::IsPropertyKindData(TNode<Uint32T> kind) { return Word32Equal(kind, Int32Constant(static_cast<int>(PropertyKind::kData))); }
    ...
    TNode<Context> context, TNode<JSObject> object, Label* if_call_runtime_with_fast_path, Label* if_no_properties, CollectType collect_type)
{
    ...
    BIND(&if_has_enum_cache);
    {
        ...
        TNode<FixedArray> values_or_entries = CAST(AllocateFixedArray(PACKED_ELEMENTS, object_enum_length, AllocationFlag::kAllowLargeObjectAllocation));
        ...
        BIND(&loop);
        {
            ...
            if (collect_type == CollectType::kEntries) {
                ...
                std::tie(array, elements) = AllocateUninitializedJSArrayWithElements(PACKED_ELEMENTS, array_map, SmiConstant(2), base::nullopt, IntPtrConstant(2));
...
TF_BUILTIN(ObjectAssign, ObjectBuiltinsAssembler)
{
    ...
    GotoIf(UintPtrLessThanOrEqual(args.GetLengthWithoutReceiver(), IntPtrConstant(1)), &done);

    // 3. Let sources be the List of argument values starting with the
    //    second argument.
    // 4. For each element nextSource of sources, in ascending index order,
    args.ForEach([=](TNode<Object> next_source) { CallBuiltin(Builtin::kSetDataProperties, context, to, next_source); }, IntPtrConstant(1));
    Goto(&done);
...
TF_BUILTIN(ObjectKeys, ObjectBuiltinsAssembler)
{
    ...
    BIND(&if_fast);
    {
        ...
        std::tie(array, elements) = AllocateUninitializedJSArrayWithElements(PACKED_ELEMENTS, array_map, array_length, base::nullopt, object_enum_length_intptr);
...
TF_BUILTIN(ObjectHasOwn, ObjectBuiltinsAssembler)
{
    ...
    Return(CallBuiltin(Builtin::kObjectPrototypeHasOwnProperty, context, target, new_target, JSParameterCount(1), object, key));
}
...
TF_BUILTIN(ObjectGetOwnPropertyNames, ObjectBuiltinsAssembler)
{
    ...
    TVARIABLE(Smi, var_length);
    TVARIABLE(FixedArrayBase, var_elements);
    Label if_empty(this, Label::kDeferred), if_empty_elements(this), if_fast(this), try_fast(this, Label::kDeferred), if_slow(this, Label::kDeferred), if_join(this);
...
{
    Label checkstringtag(this), if_arguments(this), if_array(this), if_boolean(this), if_date(this), if_error(this), if_function(this), if_number(this, Label::kDeferred),
        if_object(this), if_primitive(this), if_proxy(this, Label::kDeferred), if_regexp(this), if_string(this), if_symbol(this, Label::kDeferred), if_value(this),
    ...
    const struct {
        InstanceType value;
        Label* label;
    } kJumpTable[] = { { JS_OBJECT_TYPE, &if_object }, { JS_ARRAY_TYPE, &if_array }, { JS_REG_EXP_TYPE, &if_regexp }, { JS_ARGUMENTS_OBJECT_TYPE, &if_arguments },
        { JS_DATE_TYPE, &if_date }, { JS_API_OBJECT_TYPE, &if_object }, { JS_SPECIAL_API_OBJECT_TYPE, &if_object }, { JS_PROXY_TYPE, &if_proxy }, { JS_ERROR_TYPE, &if_error },
    ...
    BIND(&if_proxy);
    {
        ...
        TNode<HeapObject> handler = CAST(LoadObjectField(TNode<JSProxy>::UncheckedCast(receiver), JSProxy::kHandlerOffset));
        ...
        TNode<Object> receiver_is_array = CallRuntime(Runtime::kArrayIsArray, context, receiver);
        TNode<String> builtin_tag = Select<String>(
            IsTrue(receiver_is_array), [=] { return ArrayStringConstant(); },
            [=] {
                return Select<String>(
                    IsCallableMap(receiver_map), [=] { return FunctionStringConstant(); }, [=] { return ObjectStringConstant(); });
            });

        // Lookup the @@toStringTag property on the {receiver}.
        TVARIABLE(Object, var_tag, GetProperty(context, receiver, isolate()->factory()->to_string_tag_symbol()));
    ...
    BIND(&checkstringtag);
    {
        ...
        Label loop(this, &var_holder), return_default(this), return_generic(this, Label::kDeferred);
        Goto(&loop);
        BIND(&loop);
        {
            ...
            GotoIf(IsSetWord32<Map::Bits3::MayHaveInterestingSymbolsBit>(holder_bit_field3), &return_generic);
        ...
        BIND(&return_generic);
        {
            TNode<Object> tag = GetProperty(context, ToObject(context, receiver), ToStringTagSymbolConstant());
    ...
    BIND(&no_properties);
    {
        ...
        BIND(&null_proto);
        {
            ...
            if (V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL) {
                new_properties = AllocateSwissNameDictionary(SwissNameDictionary::kInitialCapacity);
            } else {
                new_properties = AllocateNameDictionary(NameDictionary::kInitialCapacity);
            }
        ...
        BIND(&non_null_proto);
        {
            ...
            TNode<MaybeObject> maybe_map = LoadMaybeWeakObjectField(prototype_info, PrototypeInfo::kObjectCreateMapOffset);
...
TF_BUILTIN(CreateGeneratorObject, ObjectBuiltinsAssembler)
{
    ...
    TNode<IntPtrT> size = IntPtrAdd(WordSar(frame_size, IntPtrConstant(kTaggedSizeLog2)), formal_parameter_count);
    TNode<FixedArrayBase> parameters_and_registers = AllocateFixedArray(HOLEY_ELEMENTS, size, AllocationFlag::kAllowLargeObjectAllocation);
    FillFixedArrayWithValue(HOLEY_ELEMENTS, parameters_and_registers, IntPtrConstant(0), size, RootIndex::kUndefinedValue);
    // TODO(cbruni): support start_offset to avoid double initialization.
    TNode<JSObject> result = AllocateJSObjectFromMap(map, base::nullopt, base::nullopt, AllocationFlag::kNone, kWithSlackTracking);
...
TF_BUILTIN(ObjectGetOwnPropertyDescriptor, ObjectBuiltinsAssembler)
{
    ...
    Label if_keyisindex(this), if_iskeyunique(this), call_runtime(this, Label::kDeferred), return_undefined(this, Label::kDeferred), if_notunique_name(this);
    ...
    GotoIf(IsSpecialReceiverInstanceType(instance_type), &call_runtime);
    {
        ...
        TryToName(key, &if_keyisindex, &var_index, &if_iskeyunique, &var_name, &call_runtime, &if_notunique_name);

        BIND(&if_notunique_name);
        {
            Label not_in_string_table(this);
            TryInternalizeString(CAST(key), &if_keyisindex, &var_index, &if_iskeyunique, &var_name, &not_in_string_table, &call_runtime);
        ...
        BIND(&if_iskeyunique);
        {
            ...
            TryGetOwnProperty(context, object, object, map, instance_type, var_name.value(), &if_found_value, &var_value, &var_details, &var_raw_value, &return_empty,
                &if_not_found, kReturnAccessorPair);

            BIND(&if_found_value);
            // 4. Return FromPropertyDetails(desc).
            TNode<JSObject> js_desc = FromPropertyDetails(context, var_value.value(), var_details.value(), &call_runtime);
    ...
    BIND(&call_runtime);
    {
        TNode<Object> desc = CallRuntime(Runtime::kGetOwnPropertyDescriptor, context, object, key);
    ...
    TNode<BoolT> condition, TNode<Context> context, TNode<Object> object, TNode<HeapObject> name_dictionary, Handle<Name> name, TNode<Object> value, Label* bailout)
{
    ...
    Add<PropertyDictionary>(CAST(name_dictionary), HeapConstant(name), value, bailout);
    Goto(&done);
...
TNode<JSObject> ObjectBuiltinsAssembler::FromPropertyDescriptor(TNode<Context> context, TNode<PropertyDescriptorObject> desc)
{
    ...
    {
        js_descriptor
            = ConstructAccessorDescriptor(context, LoadObjectField(desc, PropertyDescriptorObject::kGetOffset), LoadObjectField(desc, PropertyDescriptorObject::kSetOffset),
                IsSetWord32<PropertyDescriptorObject::IsEnumerableBit>(flags), IsSetWord32<PropertyDescriptorObject::IsConfigurableBit>(flags));
        Goto(&return_desc);
    }

    BIND(&if_data_desc);
    {
        js_descriptor = ConstructDataDescriptor(context, LoadObjectField(desc, PropertyDescriptorObject::kValueOffset), IsSetWord32<PropertyDescriptorObject::IsWritableBit>(flags),
            IsSetWord32<PropertyDescriptorObject::IsEnumerableBit>(flags), IsSetWord32<PropertyDescriptorObject::IsConfigurableBit>(flags));
        Goto(&return_desc);
    }
    ...
    BIND(&if_generic_desc);
    {
        ...
        TNode<HeapObject> properties = V8_ENABLE_SWISS_NAME_DICTIONARY_BOOL ? TNode<HeapObject>(AllocateSwissNameDictionary(6)) : AllocateNameDictionary(6);
