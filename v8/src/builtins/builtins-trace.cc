        ...
        : buf_(data_)
    {
        ...
        if (string->IsOneByteRepresentation()) {
            ...
            if (len > 0) {
                ...
                memcpy(buf_, Handle<SeqOneByteString>::cast(string)->GetChars(no_gc), len);
            }
        } else {
...
BUILTIN(Trace)
{
    ...
    if (!data_arg->IsUndefined(isolate)) {
        ...
        ASSIGN_RETURN_FAILURE_ON_EXCEPTION(isolate, arg_json, JsonStringify(isolate, data_arg, isolate->factory()->undefined_value(), isolate->factory()->undefined_value()));
        num_args++;
    }
    ...
    uint64_t arg_value;
    if (num_args) {
        std::unique_ptr<JsonTraceValue> traced_value(new JsonTraceValue(isolate, Handle<String>::cast(arg_json)));
        tracing::SetTraceValue(std::move(traced_value), &arg_type, &arg_value);
    }

    TRACE_EVENT_API_ADD_TRACE_EVENT(phase, category_group_enabled, *name, tracing::kGlobalScope, id, tracing::kNoId, num_args, &arg_name, &arg_type, &arg_value, flags);
#endif // !defined(V8_USE_PERFETTO)
