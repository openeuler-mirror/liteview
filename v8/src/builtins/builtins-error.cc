...
BUILTIN(ErrorCaptureStackTrace)
{
    ...
    RETURN_FAILURE_ON_EXCEPTION(isolate, isolate->CaptureAndSetErrorStack(object, mode, caller));
    ...
    if (!JSObject::IsExtensible(object)) {
        return isolate->Throw(*isolate->factory()->NewTypeError(MessageTemplate::kDefineDisallowed, name));
    }

    RETURN_FAILURE_ON_EXCEPTION(isolate, JSObject::SetAccessor(object, name, error_stack, DONT_ENUM));
