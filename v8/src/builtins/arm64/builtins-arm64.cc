...
void Builtins::Generate_JSConstructStubGeneric(MacroAssembler* masm)
{
    ...
    __ JumpIfIsInRange(
        w4, static_cast<uint32_t>(FunctionKind::kDefaultDerivedConstructor), static_cast<uint32_t>(FunctionKind::kDerivedConstructor), &not_create_implicit_receiver);
    ...
    __ JumpIfObjectType(x0, x4, x5, FIRST_JS_RECEIVER_TYPE, &leave_and_return, ge);
    __ B(&use_receiver);
...
void Builtins::Generate_JSBuiltinsConstructStub(MacroAssembler* masm) { Generate_JSBuiltinsConstructStubHelper(masm); }
...
static void AssertCodeTIsBaselineAllowClobber(MacroAssembler* masm, Register code, Register scratch)
{
...
static void AssertCodeTIsBaseline(MacroAssembler* masm, Register code, Register scratch)
{
...
static void GetSharedFunctionInfoBytecodeOrBaseline(MacroAssembler* masm, Register sfi_data, Register scratch1, Label* is_baseline)
{
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type, Builtin entry_trampoline)
{
    ...
    // that.
    {
        ...
        __ Mov(x10, ExternalReference::Create(IsolateAddressId::kPendingExceptionAddress, masm->isolate()));
    }
    ...
    Handle<CodeT> trampoline_code = masm->isolate()->builtins()->code_handle(entry_trampoline);
...
void Builtins::Generate_JSEntry(MacroAssembler* masm) { Generate_JSEntryVariant(masm, StackFrame::ENTRY, Builtin::kJSEntryTrampoline); }

void Builtins::Generate_JSConstructEntry(MacroAssembler* masm) { Generate_JSEntryVariant(masm, StackFrame::CONSTRUCT_ENTRY, Builtin::kJSConstructEntryTrampoline); }

void Builtins::Generate_JSRunMicrotasksEntry(MacroAssembler* masm) { Generate_JSEntryVariant(masm, StackFrame::ENTRY, Builtin::kRunMicrotasksTrampoline); }
...
static void Generate_JSEntryTrampolineHelper(MacroAssembler* masm, bool is_construct)
{
...

    {
        ...
        Handle<CodeT> builtin = is_construct ? BUILTIN_CODE(masm->isolate(), Construct) : masm->isolate()->builtins()->Call();
...
void Builtins::Generate_JSEntryTrampoline(MacroAssembler* masm) { Generate_JSEntryTrampolineHelper(masm, false); }

void Builtins::Generate_JSConstructEntryTrampoline(MacroAssembler* masm) { Generate_JSEntryTrampolineHelper(masm, true); }
...
static void LeaveInterpreterFrame(MacroAssembler* masm, Register scratch1, Register scratch2)
{
    ...
    __ Ldr(params_size.W(), FieldMemOperand(params_size, BytecodeArray::kParameterSizeOffset));
...
void ResetBytecodeAge(MacroAssembler* masm, Register bytecode_array) { __ Strh(wzr, FieldMemOperand(bytecode_array, BytecodeArray::kBytecodeAgeOffset)); }
...
void ResetFeedbackVectorOsrUrgency(MacroAssembler* masm, Register feedback_vector, Register scratch)
{
    ...
    __ And(scratch, scratch, Operand(FeedbackVector::MaybeHasOptimizedOsrCodeBit::kMask));
...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm, InterpreterEntryTrampolineMode mode)
{
    ...
    Label is_baseline;
    GetSharedFunctionInfoBytecodeOrBaseline(masm, kInterpreterBytecodeArrayRegister, x11, &is_baseline);
...
void Builtins::Generate_InterpreterPushArgsThenCallImpl(MacroAssembler* masm, ConvertReceiverMode receiver_mode, InterpreterPushArgsMode mode)
{
    ...
    // Call the target.
    if (mode == InterpreterPushArgsMode::kWithFinalSpread) {
        __ Jump(BUILTIN_CODE(masm->isolate(), CallWithSpread), RelocInfo::CODE_TARGET);
    } else {
        __ Jump(masm->isolate()->builtins()->Call(ConvertReceiverMode::kAny), RelocInfo::CODE_TARGET);
    }
}
...
void Builtins::Generate_InterpreterPushArgsThenConstructImpl(MacroAssembler* masm, InterpreterPushArgsMode mode)
{
    ...
    if (mode == InterpreterPushArgsMode::kArrayFunction) {
        ...
        __ Jump(BUILTIN_CODE(masm->isolate(), ArrayConstructorImpl), RelocInfo::CODE_TARGET);
    } else if (mode == InterpreterPushArgsMode::kWithFinalSpread) {
        // Call the constructor with x0, x1, and x3 unmodified.
        __ Jump(BUILTIN_CODE(masm->isolate(), ConstructWithSpread), RelocInfo::CODE_TARGET);
    } else {
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm)
{
    ...
    Smi interpreter_entry_return_pc_offset(masm->isolate()->heap()->interpreter_entry_return_pc_offset());
...
void Builtins::Generate_InterpreterEnterAtBytecode(MacroAssembler* masm) { Generate_InterpreterEnterBytecode(masm); }

namespace {
void Generate_ContinueToBuiltinHelper(MacroAssembler* masm, bool java_script_builtin, bool with_result)
{
...
void Builtins::Generate_ContinueToCodeStubBuiltin(MacroAssembler* masm) { Generate_ContinueToBuiltinHelper(masm, false, false); }

void Builtins::Generate_ContinueToCodeStubBuiltinWithResult(MacroAssembler* masm) { Generate_ContinueToBuiltinHelper(masm, false, true); }

void Builtins::Generate_ContinueToJavaScriptBuiltin(MacroAssembler* masm) { Generate_ContinueToBuiltinHelper(masm, true, false); }

void Builtins::Generate_ContinueToJavaScriptBuiltinWithResult(MacroAssembler* masm) { Generate_ContinueToBuiltinHelper(masm, true, true); }
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source, Register maybe_target_code)
{
    ...
    __ LoadTaggedPointerField(x1, FieldMemOperand(x0, Code::kDeoptimizationDataOrInterpreterDataOffset));

    // Load the OSR entrypoint offset from the deoptimization data.
    // <osr_offset> = <deopt_data>[#header_size + #osr_pc_offset]
    __ SmiUntagField(x1, FieldMemOperand(x1, FixedArray::OffsetOfElementAt(DeoptimizationData::kOsrPcOffsetIndex)));
...
void Builtins::Generate_FunctionPrototypeApply(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWithArrayLike), RelocInfo::CODE_TARGET);
...
void Builtins::Generate_FunctionPrototypeCall(MacroAssembler* masm)
{
    ...
    // That is if padding exists or not.
    {
        ...
        __ CopyDoubleWords(copy_to, copy_from, count, TurboAssembler::kSrcLessThanDst);
        __ Drop(2);
    }
...
void Builtins::Generate_ReflectConstruct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructWithArrayLike), RelocInfo::CODE_TARGET);
}
...
void Builtins::Generate_CallOrConstructVarargs(MacroAssembler* masm, Handle<CodeT> code)
{
    // ----------- S t a t e -------------
    //  -- x1 : target
...
// static
void Builtins::Generate_CallOrConstructForwardVarargs(MacroAssembler* masm, CallOrConstructMode mode, Handle<CodeT> code)
{
    // ----------- S t a t e -------------
    //  -- x0 : the number of arguments
    ...
    __ TestAndBranchIfAnySet(w3, SharedFunctionInfo::IsNativeBit::kMask | SharedFunctionInfo::IsStrictBit::kMask, &done_convert);
    {
        ...
        if (mode == ConvertReceiverMode::kNullOrUndefined) {
            ...
            __ Bind(&convert_to_object);
            {
                ...
                __ Mov(x0, x3);
                __ Call(BUILTIN_CODE(masm->isolate(), ToObject), RelocInfo::CODE_TARGET);
                __ Mov(x3, x0);
...
void Generate_PushBoundArguments(MacroAssembler* masm)
{
    ...
    __ SmiUntagField(bound_argc, FieldMemOperand(bound_argv, FixedArray::kLengthOffset));
...
void Builtins::Generate_CallBoundFunctionImpl(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), Call_ReceiverIsAny), RelocInfo::CODE_TARGET);
}
...
void Builtins::Generate_Call(MacroAssembler* masm, ConvertReceiverMode mode)
{
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(mode), RelocInfo::CODE_TARGET, ls);
    __ Cmp(instance_type, JS_BOUND_FUNCTION_TYPE);
    __ Jump(BUILTIN_CODE(masm->isolate(), CallBoundFunction), RelocInfo::CODE_TARGET, eq);
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), CallWrappedFunction), RelocInfo::CODE_TARGET, eq);
    ...
    __ Jump(masm->isolate()->builtins()->CallFunction(ConvertReceiverMode::kNotNullOrUndefined), RelocInfo::CODE_TARGET);
...
void Builtins::Generate_ConstructFunction(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), JSBuiltinsConstructStub), RelocInfo::CODE_TARGET);
...
void Builtins::Generate_Construct(MacroAssembler* masm)
{
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructFunction), RelocInfo::CODE_TARGET, ls);

    // Only dispatch to bound functions after checking whether they are
    // constructors.
    __ Cmp(instance_type, JS_BOUND_FUNCTION_TYPE);
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructBoundFunction), RelocInfo::CODE_TARGET, eq);
    ...
    __ Jump(BUILTIN_CODE(masm->isolate(), ConstructedNonConstructable), RelocInfo::CODE_TARGET);
}
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm)
{
    ...
    constexpr RegList kSavedGpRegs = ([]() constexpr {
        ...
        // + instance + alignment
        CHECK_EQ(WasmCompileLazyFrameConstants::kNumberOfSavedGpParamRegs + 2, saved_gp_regs.Count());
    ...
    constexpr DoubleRegList kSavedFpRegs = ([]() constexpr {
        ...
        CHECK_EQ(WasmCompileLazyFrameConstants::kNumberOfSavedFpParamRegs, saved_fp_regs.Count());
    ...
    __ ldr(temp, MemOperand(kWasmInstanceRegister, WasmInstanceObject::kJumpTableStartOffset - kHeapObjectTag));
...
void PrepareForBuiltinCall(MacroAssembler* masm, MemOperand GCScanSlotPlace, const int GCScanSlotCount, Register current_param, Register param_limit,
    Register current_int_param_slot, Register current_float_param_slot, Register valuetypes_array_ptr, Register wasm_instance, Register function_data, Register original_fp)
{
    UseScratchRegisterScope temps(masm);
    ...
    __ Stp(current_param, param_limit, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
    __ Stp(current_int_param_slot, current_float_param_slot, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
    __ Stp(valuetypes_array_ptr, original_fp, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
    __ Stp(wasm_instance, function_data, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
...
void RestoreAfterBuiltinCall(MacroAssembler* masm, Register function_data, Register wasm_instance, Register valuetypes_array_ptr, Register current_float_param_slot,
    Register current_int_param_slot, Register param_limit, Register current_param, Register original_fp)
{
    // Pop and load values from the stack in order into the registers after
    // builtin calls for the GenericJSToWasmWrapper.
    __ Ldp(wasm_instance, function_data, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
    __ Ldp(valuetypes_array_ptr, original_fp, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
    __ Ldp(current_int_param_slot, current_float_param_slot, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
    __ Ldp(current_param, param_limit, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
}
...
void LoadJumpBuffer(MacroAssembler* masm, Register jmpbuf, bool load_pc, Register tmp)
{
    __ Ldr(tmp, MemOperand(jmpbuf, wasm::kJmpBufSpOffset));
    __ Mov(sp, tmp);
    __ Ldr(fp, MemOperand(jmpbuf, wasm::kJmpBufFpOffset));
    SwitchStackState(masm, jmpbuf, tmp, wasm::JumpBuffer::Inactive, wasm::JumpBuffer::Active);
    if (load_pc) {
...
void AllocateSuspender(MacroAssembler* masm, Register function_data, Register wasm_instance, Register tmp)
{
    __ Mov(tmp, 2);
    __ Str(tmp, MemOperand(fp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset));
    __ Stp(wasm_instance, function_data, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
...
void LoadTargetJumpBuffer(MacroAssembler* masm, Register target_continuation, Register tmp)
{
    ...
    __ Str(xzr, MemOperand(fp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset));
    // Switch stack!
    LoadJumpBuffer(masm, target_jmpbuf, false, tmp);
}

void ReloadParentContinuation(MacroAssembler* masm, Register wasm_instance, Register return_reg, Register tmp1, Register tmp2)
{
    ...
    __ Mov(tmp1, 1);
    __ Str(tmp1, MemOperand(fp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset));
    __ Stp(wasm_instance, return_reg, MemOperand(sp, -2 * kSystemPointerSize, PreIndex)); // Spill.
    __ Move(kContextRegister, Smi::zero());
    __ CallRuntime(Runtime::kWasmSyncStackLimit);
    __ Ldp(wasm_instance, return_reg, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
}
...
void LoadValueTypesArray(MacroAssembler* masm, Register function_data, Register valuetypes_array_ptr, Register return_count, Register param_count)
{
    Register signature = valuetypes_array_ptr;
    __ LoadExternalPointerField(signature, FieldMemOperand(function_data, WasmExportedFunctionData::kSigOffset), kWasmExportedFunctionDataSignatureTag);
...
public:
    ...
    void Reserve(const Register& reg1, const Register& reg2, const Register& reg3 = NoReg, const Register& reg4 = NoReg, const Register& reg5 = NoReg, const Register& reg6 = NoReg)
    {
        Reserve(reg1);
        Reserve(reg2);
        Reserve(reg3);
    ...
    bool IsUsed(const Register& reg) { return initial_.IncludesAliasOf(reg) && !available_.IncludesAliasOf(reg); }

    void ResetExcept(const Register& reg1 = NoReg, const Register& reg2 = NoReg, const Register& reg3 = NoReg, const Register& reg4 = NoReg, const Register& reg5 = NoReg,
...
void GenericJSToWasmWrapperHelper(MacroAssembler* masm, bool stack_switch)
{
    ...
    // -------------------------------------------
    constexpr int kGCScanSlotCountOffset = BuiltinWasmWrapperConstants::kGCScanSlotCountOffset;
    // The number of parameters passed to this function.
    constexpr int kInParamCountOffset = BuiltinWasmWrapperConstants::kInParamCountOffset;
    // The number of parameters according to the signature.
    constexpr int kParamCountOffset = BuiltinWasmWrapperConstants::kParamCountOffset;
    constexpr int kSuspenderOffset = BuiltinWasmWrapperConstants::kSuspenderOffset;
    constexpr int kReturnCountOffset = kSuspenderOffset - kSystemPointerSize;
    constexpr int kValueTypesArrayStartOffset = kReturnCountOffset - kSystemPointerSize;
    ...
    constexpr int kRefParamsCountOffset = kValueTypesArrayStartOffset - kSystemPointerSize;
    // We set and use this slot only when moving parameters into the parameter
    // registers (so no GC scan is needed).
    constexpr int kFunctionDataOffset = kRefParamsCountOffset - kSystemPointerSize;
    constexpr int kLastSpillOffset = kFunctionDataOffset;
    constexpr int kNumSpillSlots = (-TypedFrameConstants::kFixedFrameSizeFromFp - kLastSpillOffset) >> kSystemPointerSizeLog2;
    ...
    if (!stack_switch) {
        ...
        // -------------------------------------------
        MemOperand budget_loc = FieldMemOperand(function_data, WasmExportedFunctionData::kWrapperBudgetOffset);
    ...
    // -------------------------------------------
    LoadValueTypesArray(masm, function_data, valuetypes_array_ptr, return_count, param_count);
    ...
    __ Sub(current_float_param_slot, current_int_param_slot, Operand(param_count, LSL, kSystemPointerSizeLog2));
    ...
    __ Add(param_ptr, original_fp, kFPOnStackSize + kPCOnStackSize + kReceiverOnStackSize);
    DEFINE_REG(param_limit);
    __ Add(param_limit, param_ptr, Operand(param_count, LSL, kSystemPointerSizeLog2));
    ...
    __ Str(param, MemOperand(current_int_param_slot, -kSystemPointerSize, PostIndex));
    ...
    PrepareForBuiltinCall(masm, MemOperand(fp, kGCScanSlotCountOffset), kBuiltinCallGCScanSlotCount, param_ptr, param_limit, current_int_param_slot, current_float_param_slot,
        valuetypes_array_ptr, wasm_instance, function_data, original_fp);
    ...
    RestoreAfterBuiltinCall(masm, function_data, wasm_instance, valuetypes_array_ptr, current_float_param_slot, current_int_param_slot, param_limit, param_ptr, original_fp);
    __ jmp(&param_conversion_done);
    ...
    __ Call(BUILTIN_CODE(masm->isolate(), WasmTaggedToFloat64), RelocInfo::CODE_TARGET);
    RestoreAfterBuiltinCall(masm, function_data, wasm_instance, valuetypes_array_ptr, current_float_param_slot, current_int_param_slot, param_limit, param_ptr, original_fp);
    __ Str(kFPReturnRegister0, MemOperand(current_float_param_slot, -kSystemPointerSize, PostIndex));
    __ jmp(&param_conversion_done);
    ...
    regs.ResetExcept(valuetypes_array_ptr, param_count, current_int_param_slot, current_float_param_slot, wasm_instance, original_fp);
    ...
    constexpr int kIntegerSectionStartOffset = kLastSpillOffset - kGapSlotSize - kSystemPointerSize;
    DEFINE_REG(start_int_section);
    __ Add(start_int_section, fp, kIntegerSectionStartOffset);

    DEFINE_REG(start_float_section);
    __ Sub(start_float_section, start_int_section, Operand(param_count, LSL, kSystemPointerSizeLog2));
    ...
    __ Add(current_int_param_slot, current_int_param_slot, Immediate(kSystemPointerSize));
    __ Add(current_float_param_slot, current_float_param_slot, Immediate(kSystemPointerSize));
    ...
    ASSIGN_REG(param);
    __ Ldr(param, MemOperand(current_int_param_slot, kSystemPointerSize, PostIndex));
    ...
    __ Ldr(param, MemOperand(current_float_param_slot, kSystemPointerSize, PostIndex));
    __ Str(param, MemOperand(args_pointer, -kSystemPointerSize, PostIndex));
    __ jmp(&loop_through_valuetypes);
    ...
    __ LoadExternalPointerField(function_entry, FieldMemOperand(function_entry, WasmInternalFunction::kCallTargetOffset), kWasmInternalFunctionCallTargetTag);
    ...
    __ Call(BUILTIN_CODE(masm->isolate(), WasmFuncRefToJS), RelocInfo::CODE_TARGET);
    __ jmp(&return_done);
...
void Builtins::Generate_GenericJSToWasmWrapper(MacroAssembler* masm) { GenericJSToWasmWrapperHelper(masm, false); }

void Builtins::Generate_WasmReturnPromiseOnSuspend(MacroAssembler* masm) { GenericJSToWasmWrapperHelper(masm, true); }
...
void Builtins::Generate_WasmSuspend(MacroAssembler* masm)
{
    ...
    __ Sub(sp, sp, RoundUp(-(BuiltinWasmWrapperConstants::kGCScanSlotCountOffset - TypedFrameConstants::kFixedFrameSizeFromFp), 16));
    ...
    __ Str(undefined, MemOperand(fp, BuiltinWasmWrapperConstants::kSuspenderOffset));
    ...
    SwitchStackState(masm, jmpbuf, scratch, wasm::JumpBuffer::Active, wasm::JumpBuffer::Inactive);
    ...
    // Load jump buffer.
    // -------------------------------------------
    MemOperand GCScanSlotPlace = MemOperand(fp, BuiltinWasmWrapperConstants::kGCScanSlotCountOffset);
    ASSIGN_REG(scratch);
    __ Mov(scratch, 2);
    __ Str(scratch, GCScanSlotPlace);
    __ Stp(caller, promise, MemOperand(sp, -2 * kSystemPointerSize, PreIndex));
    __ Move(kContextRegister, Smi::zero());
    __ CallRuntime(Runtime::kWasmSyncStackLimit);
    __ Ldp(caller, promise, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
    ASSIGN_REG(jmpbuf);
...
void Generate_WasmResumeHelper(MacroAssembler* masm, wasm::OnResume on_resume)
{
    ...
    DEFINE_REG(scratch);
    __ LoadRoot(scratch, RootIndex::kUndefinedValue);
    __ Str(scratch, MemOperand(fp, BuiltinWasmWrapperConstants::kSuspenderOffset));
    ...
    __ Move(kContextRegister, Smi::zero());
    __ CallRuntime(Runtime::kWasmSyncStackLimit);
    __ Ldp(target_continuation, scratch, MemOperand(sp, 2 * kSystemPointerSize, PostIndex));
...
void Builtins::Generate_WasmResume(MacroAssembler* masm) { Generate_WasmResumeHelper(masm, wasm::OnResume::kContinue); }

void Builtins::Generate_WasmReject(MacroAssembler* masm) { Generate_WasmResumeHelper(masm, wasm::OnResume::kThrow); }
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size, SaveFPRegsMode save_doubles, ArgvMode argv_mode, bool builtin_exit_frame)
{
    ...
    FrameScope scope(masm, StackFrame::MANUAL);
    __ EnterExitFrame(save_doubles == SaveFPRegsMode::kSave, x10, extra_stack_space, builtin_exit_frame ? StackFrame::BUILTIN_EXIT : StackFrame::EXIT);
    ...
    ExternalReference pending_handler_context_address = ExternalReference::Create(IsolateAddressId::kPendingHandlerContextAddress, masm->isolate());
    ExternalReference pending_handler_entrypoint_address = ExternalReference::Create(IsolateAddressId::kPendingHandlerEntrypointAddress, masm->isolate());
    ExternalReference pending_handler_fp_address = ExternalReference::Create(IsolateAddressId::kPendingHandlerFPAddress, masm->isolate());
    ExternalReference pending_handler_sp_address = ExternalReference::Create(IsolateAddressId::kPendingHandlerSPAddress, masm->isolate());
...

    {
        ...
        UseScratchRegisterScope temps(masm);
        Register scratch = temps.AcquireX();
        __ Mov(scratch, ExternalReference::Create(IsolateAddressId::kCEntryFPAddress, masm->isolate()));
...
int AddressOffset(ExternalReference ref0, ExternalReference ref1) { return static_cast<int>(ref0.address() - ref1.address()); }
...
void CallApiFunctionAndReturn(MacroAssembler* masm, Register function_address, ExternalReference thunk_ref, int stack_space, MemOperand* stack_space_operand, int spill_offset,
    MemOperand return_value_operand)
{
    ...
    const int kLimitOffset = AddressOffset(ExternalReference::handle_scope_limit_address(isolate), next_address);
    const int kLevelOffset = AddressOffset(ExternalReference::handle_scope_level_address(isolate), next_address);
    ...
    __ Ldrb(w10, __ ExternalReferenceAsOperand(ExternalReference::is_profiling_address(isolate), x10));
...
void Builtins::Generate_CallApiCallback(MacroAssembler* masm)
{
    ...
    FrameScope frame_scope(masm, StackFrame::MANUAL);
    __ EnterExitFrame(kDontSaveDoubles, x10, kApiStackSpace + kCallApiFunctionSpillSpace);
    ...
    MemOperand return_value_operand(fp, (kStackSlotsAboveFCA + FCA::kReturnValueOffset) * kSystemPointerSize);
    ...
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, kUseStackSpaceOperand, &stack_space_operand, kSpillOffset, return_value_operand);
}
...
void Builtins::Generate_CallApiGetter(MacroAssembler* masm)
{
    ...
    Register name = x7;
    DCHECK(!AreAliased(receiver, holder, callback, data, undef, isolate_address, name));
    ...
    static_assert(kStackUnwindSpace % 2 == 0, "slots must be a multiple of 2 for stack pointer alignment");
    ...
    MemOperand return_value_operand(fp, (PropertyCallbackArguments::kReturnValueOffset + 3) * kSystemPointerSize);
    MemOperand* const kUseStackSpaceConstant = nullptr;
    CallApiFunctionAndReturn(masm, api_function_address, thunk_ref, kStackUnwindSpace, kUseStackSpaceConstant, spill_offset, return_value_operand);
}
...
void CopyRegListToFrame(MacroAssembler* masm, const Register& dst, int dst_offset, const CPURegList& reg_list, const Register& temp0, const Register& temp1, int src_offset = 0)
{
    ASM_CODE_COMMENT(masm);
...
void Generate_DeoptimizationEntry(MacroAssembler* masm, DeoptimizeKind deopt_kind)
{
    ...
    CPURegList saved_double_registers(kDRegSizeInBits, DoubleRegList::FromBits(RegisterConfiguration::Default()->allocatable_double_codes_mask()));
    ...
    const int kSavedRegistersAreaSize = (saved_registers.Count() * kXRegSize) + (saved_double_registers.Count() * kDRegSize);
    ...
    CopyRegListToFrame(masm, x1, FrameDescription::double_registers_offset(), saved_double_registers, x2, x3, kDoubleRegistersOffset);
    ...
    RestoreRegList(masm, saved_double_registers, x1, FrameDescription::double_registers_offset());
...
void Builtins::Generate_DeoptimizationEntry_Eager(MacroAssembler* masm) { Generate_DeoptimizationEntry(masm, DeoptimizeKind::kEager); }

void Builtins::Generate_DeoptimizationEntry_Lazy(MacroAssembler* masm) { Generate_DeoptimizationEntry(masm, DeoptimizeKind::kLazy); }
...
void Generate_BaselineOrInterpreterEntry(MacroAssembler* masm, bool next_bytecode, bool is_osr = false)
{
    ...
    if (!is_osr) {
        ...
        Builtin builtin_id = next_bytecode ? Builtin::kInterpreterEnterAtNextBytecode : Builtin::kInterpreterEnterAtBytecode;
        __ Jump(masm->isolate()->builtins()->code_handle(builtin_id), RelocInfo::CODE_TARGET);
    ...
    __ Str(feedback_vector, MemOperand(fp, InterpreterFrameConstants::kBytecodeOffsetFromFp));
...
void Builtins::Generate_BaselineOrInterpreterEnterAtBytecode(MacroAssembler* masm) { Generate_BaselineOrInterpreterEntry(masm, false); }

void Builtins::Generate_BaselineOrInterpreterEnterAtNextBytecode(MacroAssembler* masm) { Generate_BaselineOrInterpreterEntry(masm, true); }

void Builtins::Generate_InterpreterOnStackReplacement_ToBaseline(MacroAssembler* masm) { Generate_BaselineOrInterpreterEntry(masm, false, true); }
