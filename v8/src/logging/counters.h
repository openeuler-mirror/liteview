...
public:
    ...
    void SetCreateHistogramFunction(CreateHistogramCallback f) { create_histogram_function_ = f; }

    // Register an application-defined function to add a sample
    // to a histogram created with CreateHistogram function.
    void SetAddHistogramSampleFunction(AddHistogramSampleCallback f) { add_histogram_sample_function_ = f; }
...
public:
    ...
    void Increment(int value = 1) { GetPtr()->fetch_add(value, std::memory_order_relaxed); }

    void Decrement(int value = 1) { GetPtr()->fetch_sub(value, std::memory_order_relaxed); }
...
public:
    ...
    void AssertReportsToCounters(Counters* expected_counters) { DCHECK_EQ(counters_, expected_counters); }
...
private:
    ...
    inline void Leave(NestedTimedHistogramScope* previous) { current_ = previous; }
...
public:
    ...
    void SetAddHistogramSampleFunction(AddHistogramSampleCallback f) { stats_table_.SetAddHistogramSampleFunction(f); }
...
#define COUNTER_ID(name, caption) k_##name,
                        STATS_COUNTER_LIST_1(COUNTER_ID) STATS_COUNTER_LIST_2(COUNTER_ID) STATS_COUNTER_NATIVE_CODE_LIST(COUNTER_ID)
#undef COUNTER_ID
...
#undef COUNTER_ID
#define COUNTER_ID(name) kCountOfFIXED_ARRAY__##name, kSizeOfFIXED_ARRAY__##name,
...
#undef COUNTER_ID
    ...
    WorkerThreadRuntimeCallStats* worker_thread_runtime_call_stats() { return &worker_thread_runtime_call_stats_; }
#else // V8_RUNTIME_CALL_STATS
    RuntimeCallStats* runtime_call_stats() { return nullptr; }

    WorkerThreadRuntimeCallStats* worker_thread_runtime_call_stats() { return nullptr; }
...
private:
    ...
    int* FindLocation(const char* name) { return stats_table_.FindLocation(name); }

    void* CreateHistogram(const char* name, int min, int max, size_t buckets) { return stats_table_.CreateHistogram(name, min, max, buckets); }

    void AddHistogramSample(void* histogram, int sample) { stats_table_.AddHistogramSample(histogram, sample); }
