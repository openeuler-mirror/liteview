...
// static
bool LogFile::IsLoggingToConsole(std::string file_name) { return file_name.compare(LogFile::kLogToConsole) == 0; }

// static
bool LogFile::IsLoggingToTemporaryFile(std::string file_name) { return file_name.compare(LogFile::kLogToTemporaryFile) == 0; }
...
void LogFile::MessageBuilder::AppendString(String str, base::Optional<int> length_limit)
{
    if (str.is_null())
        return;
...
void LogFile::MessageBuilder::AppendSymbolNameDetails(String str, bool show_impl_info)
{
    if (str.is_null())
        return;
...
void LogFile::MessageBuilder::AppendRawString(const char* str) { log_->os_ << str; }
...
template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <String>(String string)
{
    this->AppendString(string);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <Symbol>(Symbol symbol)
{
    this->AppendSymbolName(symbol);
    return *this;
}

template <> LogFile::MessageBuilder& LogFile::MessageBuilder::operator<< <Name>(Name name)
{
    if (name.IsString()) {
