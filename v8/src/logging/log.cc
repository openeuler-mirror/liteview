...
const char* ComputeMarker(SharedFunctionInfo shared, AbstractCode code)
{
    ...
    // "interpreted" marker.
    if (v8_flags.interpreted_frames_native_stack && kind == CodeKind::BUILTIN && !code.is_off_heap_trampoline(cage_base)) {
...
#if V8_ENABLE_WEBASSEMBLY
void CodeEventLogger::CodeCreateEvent(CodeTag tag, const wasm::WasmCode* code, wasm::WasmName name, const char* source_url, int /*code_offset*/, int /*script_id*/)
{
    DCHECK(is_listening_to_code_events());
    name_buffer_->Init(tag);
    DCHECK(!name.empty());
...
void CodeEventLogger::RegExpCodeCreateEvent(Handle<AbstractCode> code, Handle<String> source)
{
    DCHECK(is_listening_to_code_events());
...
private:
    void LogRecordedBuffer(Handle<AbstractCode> code, MaybeHandle<SharedFunctionInfo> maybe_shared, const char* name, int length) override;
#if V8_ENABLE_WEBASSEMBLY
...
void LinuxPerfBasicLogger::LogRecordedBuffer(Handle<AbstractCode> code, MaybeHandle<SharedFunctionInfo>, const char* name, int length)
{
...
void ExternalLogEventListener::CodeCreateEvent(CodeTag tag, Handle<AbstractCode> code, Handle<SharedFunctionInfo> shared, Handle<Name> source, int line, int column)
{
...
#if V8_ENABLE_WEBASSEMBLY
void ExternalLogEventListener::CodeCreateEvent(CodeTag tag, const wasm::WasmCode* code, wasm::WasmName name, const char* source_url, int code_offset, int script_id)
{
    // TODO(mmarchini): handle later
}
#endif // V8_ENABLE_WEBASSEMBLY

void ExternalLogEventListener::RegExpCodeCreateEvent(Handle<AbstractCode> code, Handle<String> source)
{
    PtrComprCageBase cage_base(isolate_);
    CodeEvent code_event;
    code_event.code_start_address = static_cast<uintptr_t>(code->InstructionStart(cage_base));
...
void ExternalLogEventListener::CodeMoveEvent(AbstractCode from, AbstractCode to)
{
    PtrComprCageBase cage_base(isolate_);
    CodeEvent code_event;
    code_event.previous_code_start_address = static_cast<uintptr_t>(from.InstructionStart(cage_base));
    code_event.code_start_address = static_cast<uintptr_t>(to.InstructionStart(cage_base));
...
void LowLevelLogger::LogRecordedBuffer(Handle<AbstractCode> code, MaybeHandle<SharedFunctionInfo>, const char* name, int length)
{
    PtrComprCageBase cage_base(isolate_);
...
void JitLogger::LogRecordedBuffer(Handle<AbstractCode> code, MaybeHandle<SharedFunctionInfo> maybe_shared, const char* name, int length)
{
    PtrComprCageBase cage_base(isolate_);
    JitCodeEvent event;
    ...
    if (maybe_shared.ToHandle(&shared) && shared->script(cage_base).IsScript(cage_base)) {
...
void JitLogger::CodeMoveEvent(AbstractCode from, AbstractCode to)
{
    ...
    event.code_type = from.IsCode(cage_base) ? JitCodeEvent::JIT_CODE : JitCodeEvent::BYTE_CODE;
    event.code_start = reinterpret_cast<void*>(from.InstructionStart(cage_base));
    event.code_len = from.InstructionSize(cage_base);
    event.new_code_start = reinterpret_cast<void*>(to.InstructionStart(cage_base));
    event.isolate = reinterpret_cast<v8::Isolate*>(isolate_);

    code_event_handler_(&event);
}

void JitLogger::AddCodeLinePosInfoEvent(void* jit_handler_data, int pc_offset, int position, JitCodeEvent::PositionType position_type, JitCodeEvent::CodeType code_type)
{
    JitCodeEvent event;
...
public:
    ...
    void SampleStack(const v8::RegisterState& state) override
    {
        ...
        if (isolate->was_locker_ever_used() && (!isolate->thread_manager()->IsLockedByThread(perThreadData_->thread_id()) || perThreadData_->thread_state() != nullptr))
            return;
...
void V8FileLogger::RemoveLogEventListener(LogEventListener* listener) { isolate_->logger()->RemoveListener(listener); }
...
void V8FileLogger::SharedLibraryEvent(const std::string& library_path, uintptr_t start, uintptr_t end, intptr_t aslr_slide)
{
    if (!v8_flags.prof_cpp)
        return;
    MSG_BUILDER();
    msg << "shared-library" << kNext << library_path.c_str() << kNext << reinterpret_cast<void*>(start) << kNext << reinterpret_cast<void*>(end) << kNext << aslr_slide;
    msg.WriteToLogFile();
}
...
void AppendCodeCreateHeader(Isolate* isolate, LogFile::MessageBuilder& msg, LogEventListener::CodeTag tag, AbstractCode code, uint64_t time)
{
    PtrComprCageBase cage_base(isolate);
    AppendCodeCreateHeader(msg, tag, code.kind(cage_base), reinterpret_cast<uint8_t*>(code.InstructionStart(cage_base)), code.InstructionSize(cage_base), time);
}
...
void V8FileLogger::LogSourceCodeInformation(Handle<AbstractCode> code, Handle<SharedFunctionInfo> shared)
{
    PtrComprCageBase cage_base(isolate_);
    Object script_object = shared->script(cage_base);
    if (!script_object.IsScript(cage_base))
        return;
    ...
    if (!v8_flags.log_source_position)
        return;
    MSG_BUILDER();
    msg << "code-source-info" << V8FileLogger::kNext << reinterpret_cast<void*>(code->InstructionStart(cage_base)) << V8FileLogger::kNext << script.id() << V8FileLogger::kNext
        << shared->StartPosition() << V8FileLogger::kNext << shared->EndPosition() << V8FileLogger::kNext;
    // TODO(v8:11429): Clean-up baseline-replated code in source position
    // iteration.
    bool hasInlined = false;
    if (code->kind(cage_base) != CodeKind::BASELINE) {
        SourcePositionTableIterator iterator(code->SourcePositionTable(cage_base, *shared));
    ...
    int maxInlinedId = -1;
    if (hasInlined) {
        PodArray<InliningPosition> inlining_positions = DeoptimizationData::cast(Handle<Code>::cast(code)->deoptimization_data()).InliningPositions();
    ...
    msg << V8FileLogger::kNext;
    if (hasInlined) {
        DeoptimizationData deopt_data = DeoptimizationData::cast(Handle<Code>::cast(code)->deoptimization_data());
        msg << std::hex;
...
void V8FileLogger::LogCodeDisassemble(Handle<AbstractCode> code)
{
    ...
    PtrComprCageBase cage_base(isolate_);
    MSG_BUILDER();
    msg << "code-disassemble" << V8FileLogger::kNext << reinterpret_cast<void*>(code->InstructionStart(cage_base)) << V8FileLogger::kNext << CodeKindToString(code->kind(cage_base))
        << V8FileLogger::kNext;
    {
...
#endif
        } else if (V8_REMOVE_BUILTINS_CODE_OBJECTS && code->IsCodeDataContainer(cage_base)) {
#ifdef ENABLE_DISASSEMBLER
...
void V8FileLogger::CodeCreateEvent(CodeTag tag, Handle<AbstractCode> code, Handle<SharedFunctionInfo> shared, Handle<Name> script_name)
{
    ...
    if (!v8_flags.log_code)
        return;
    if (*code == AbstractCode::cast(isolate_->builtins()->code(Builtin::kCompileLazy))) {
        return;
    }
    {
        MSG_BUILDER();
...
void V8FileLogger::FeedbackVectorEvent(FeedbackVector vector, AbstractCode code)
{
...
void V8FileLogger::CodeCreateEvent(CodeTag tag, Handle<AbstractCode> code, Handle<SharedFunctionInfo> shared, Handle<Name> script_name, int line, int column)
{
        ...
        return;
    {
        MSG_BUILDER();
        AppendCodeCreateHeader(isolate_, msg, tag, *code, Time());
        msg << shared->DebugNameCStr().get() << " " << *script_name << ":" << line << ":" << column << kNext << reinterpret_cast<void*>(shared->address()) << kNext
...
#if V8_ENABLE_WEBASSEMBLY
void V8FileLogger::CodeCreateEvent(CodeTag tag, const wasm::WasmCode* code, wasm::WasmName name, const char* /*source_url*/, int /*code_offset*/, int /*script_id*/)
{
    if (!is_listening_to_code_events())
        return;
    if (!v8_flags.log_code)
...
void V8FileLogger::CallbackEventInternal(const char* prefix, Handle<Name> name, Address entry_point)
{
    if (!v8_flags.log_code)
        return;
    MSG_BUILDER();
    msg << Event::kCodeCreation << kNext << CodeTag::kCallback << kNext << -2 << kNext << Time() << kNext << reinterpret_cast<void*>(entry_point) << kNext << 1 << kNext << prefix
        << *name;
    msg.WriteToLogFile();
}

void V8FileLogger::CallbackEvent(Handle<Name> name, Address entry_point) { CallbackEventInternal("", name, entry_point); }

void V8FileLogger::GetterCallbackEvent(Handle<Name> name, Address entry_point) { CallbackEventInternal("get ", name, entry_point); }

void V8FileLogger::SetterCallbackEvent(Handle<Name> name, Address entry_point) { CallbackEventInternal("set ", name, entry_point); }

void V8FileLogger::RegExpCodeCreateEvent(Handle<AbstractCode> code, Handle<String> source)
{
    if (!is_listening_to_code_events())
        return;
    if (!v8_flags.log_code)
...
void V8FileLogger::CodeMoveEvent(AbstractCode from, AbstractCode to)
{
    if (!is_listening_to_code_events())
        return;
    PtrComprCageBase cage_base(isolate_);
    MoveEventInternal(Event::kCodeMove, from.InstructionStart(cage_base), to.InstructionStart(cage_base));
}
...
void V8FileLogger::ProcessDeoptEvent(Handle<Code> code, SourcePosition position, const char* kind, const char* reason)
{
    MSG_BUILDER();
    msg << Event::kCodeDeopt << kNext << Time() << kNext << code->CodeSize() << kNext << reinterpret_cast<void*>(code->InstructionStart());
...
void CodeLinePosEvent(JitLogger& jit_logger, Address code_start, SourcePositionTableIterator& iter, JitCodeEvent::CodeType code_type)
{
    ...
    for (; !iter.done(); iter.Advance()) {
        if (iter.is_statement()) {
            jit_logger.AddCodeLinePosInfoEvent(jit_handler_data, iter.code_offset(), iter.source_position().ScriptOffset(), JitCodeEvent::STATEMENT_POSITION, code_type);
        }
...
void V8FileLogger::CodeLinePosInfoRecordEvent(Address code_start, ByteArray source_position_table, JitCodeEvent::CodeType code_type)
{
    if (!jit_logger_)
        return;
...
#if V8_ENABLE_WEBASSEMBLY
void V8FileLogger::WasmCodeLinePosInfoRecordEvent(Address code_start, base::Vector<const byte> source_position_table)
{
    if (!jit_logger_)
        return;
    SourcePositionTableIterator iter(source_position_table);
...
namespace {
void AppendFunctionMessage(LogFile::MessageBuilder& msg, const char* reason, int script_id, double time_delta, int start_position, int end_position, uint64_t time)
{
    msg << "function" << V8FileLogger::kNext << reason << V8FileLogger::kNext << script_id << V8FileLogger::kNext << start_position << V8FileLogger::kNext << end_position
        << V8FileLogger::kNext;
...
void V8FileLogger::FunctionEvent(const char* reason, int script_id, double time_delta, int start_position, int end_position, String function_name)
{
...
void V8FileLogger::FunctionEvent(
    const char* reason, int script_id, double time_delta, int start_position, int end_position, const char* function_name, size_t function_name_length, bool is_one_byte)
{
    if (!v8_flags.log_function_events)
        return;
    MSG_BUILDER();
...
void V8FileLogger::CompilationCacheEvent(const char* action, const char* cache_type, SharedFunctionInfo sfi)
{
    if (!v8_flags.log_function_events)
        return;
    MSG_BUILDER();
...
void V8FileLogger::ICEvent(const char* type, bool keyed, Handle<Map> map, Handle<Object> key, char old_state, char new_state, const char* modifier, const char* slow_stub_reason)
{
    if (!v8_flags.log_ic)
        return;
    int line;
    ...
    if (keyed)
        msg << "Keyed";
    msg << type << kNext << reinterpret_cast<void*>(pc) << kNext << Time() << kNext << line << kNext << column << kNext << old_state << kNext << new_state << kNext
...
void V8FileLogger::MapEvent(const char* type, Handle<Map> from, Handle<Map> to, const char* reason, Handle<HeapObject> name_or_sfi)
{
    ...
    msg << "map" << kNext << type << kNext << Time() << kNext << AsHex::Address(from.is_null() ? kNullAddress : from->ptr()) << kNext
        << AsHex::Address(to.is_null() ? kNullAddress : to->ptr()) << kNext << AsHex::Address(pc) << kNext << line << kNext << column << kNext << reason << kNext;
...
static std::vector<std::pair<Handle<SharedFunctionInfo>, Handle<AbstractCode>>> EnumerateCompiledFunctions(Heap* heap)
{
    ...
    auto hash = [](const std::pair<SharedFunctionInfo, AbstractCode>& p) { return base::hash_combine(p.first.address(), p.second.address()); };
    std::unordered_set<std::pair<SharedFunctionInfo, AbstractCode>, decltype(hash)> seen(8, hash);
    ...
    for (HeapObject obj = iterator.Next(); !obj.is_null(); obj = iterator.Next()) {
        if (obj.IsSharedFunctionInfo()) {
            ...
            if (function.HasAttachedOptimizedCode() && Script::cast(function.shared().script()).HasValidSource()) {
                record(function.shared(), AbstractCode::cast(FromCodeT(function.code())));
            }
        }
    }

    Script::Iterator script_iterator(heap->isolate());
    for (Script script = script_iterator.Next(); !script.is_null(); script = script_iterator.Next()) {
        if (!script.HasValidSource())
            continue;

        SharedFunctionInfo::ScriptIterator sfi_iterator(heap->isolate(), script);
        for (SharedFunctionInfo sfi = sfi_iterator.Next(); !sfi.is_null(); sfi = sfi_iterator.Next()) {
            if (sfi.is_compiled()) {
                record(sfi, AbstractCode::cast(sfi.abstract_code(isolate)));
            }
        }
...
void V8FileLogger::LogExistingFunction(Handle<SharedFunctionInfo> shared, Handle<AbstractCode> code) { existing_code_logger_.LogExistingFunction(shared, code); }

void V8FileLogger::LogCompiledFunctions() { existing_code_logger_.LogCompiledFunctions(); }
    ...
    for (const char* p = file_name; *p; p++) {
        ...
        if (*p == '%') {
            ...
            case 't':
                ...
                os << static_cast<int64_t>(V8::GetCurrentPlatform()->CurrentClockTimeMillis());
                break;
            case '%':
...
void ExistingCodeLogger::LogCodeObjects()
{
    ...
    for (HeapObject obj = iterator.Next(); !obj.is_null(); obj = iterator.Next()) {
        ...
        if (V8_REMOVE_BUILTINS_CODE_OBJECTS) {
            ...
            if (InstanceTypeChecker::IsCodeT(instance_type) || InstanceTypeChecker::IsBytecodeArray(instance_type)) {
...
void ExistingCodeLogger::LogCompiledFunctions()
{
    ...
    for (auto& pair : compiled_funcs) {
        ...
        if (shared->HasInterpreterData()) {
            LogExistingFunction(shared, Handle<AbstractCode>(AbstractCode::cast(FromCodeT(shared->InterpreterTrampoline())), isolate_));
        }
        if (shared->HasBaselineCode()) {
            LogExistingFunction(shared, Handle<AbstractCode>(AbstractCode::cast(FromCodeT(shared->baseline_code(kAcquireLoad))), isolate_));
        }
    ...
    if (shared->script().IsScript()) {
        ...
        if (script->name().IsString()) {
            ...
            CALL_CODE_EVENT_HANDLER(
                CodeCreateEvent(V8FileLogger::ToNativeByScript(tag, *script), code, shared, ReadOnlyRoots(isolate_).empty_string_handle(), line_num, column_num))
        }
    } else if (shared->IsApiFunction()) {
        // API function.
        Handle<FunctionTemplateInfo> fun_data = handle(shared->get_api_func_data(), isolate_);
        ...
        if (!raw_call_data.IsUndefined(isolate_)) {
            ...
            for (int i = 0; i < c_functions_count; i++) {
                CALL_CODE_EVENT_HANDLER(CallbackEvent(fun_name, fun_data->GetCFunction(i)))
            }
        }
    }
}
