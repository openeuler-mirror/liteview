...
void StatsTable::SetCounterFunction(CounterLookupCallback f) { lookup_function_ = f; }
...
void* Histogram::CreateHistogram() const { return counters_->CreateHistogram(name_, min_, max_, num_buckets_); }
...
void TimedHistogram::RecordAbandon(base::ElapsedTimer* timer, Isolate* isolate)
{
    if (Enabled()) {
        ...
        timer->Stop();
        int64_t sample = resolution_ == TimedHistogramResolution::MICROSECOND ? base::TimeDelta::Max().InMicroseconds() : base::TimeDelta::Max().InMilliseconds();
...
{
    //static thread_local base::LazyInstance<std::unordered_map<const TimedHistogram*, bool>>::type active_timer = LAZY_INSTANCE_INITIALIZER;
...
#define HR(name, caption, min, max, num_buckets) { &Counters::name##_, #caption, min, max, num_buckets },
        HISTOGRAM_RANGE_LIST(HR)
#undef HR
    };
    for (const auto& histogram : kHistograms) {
        (this->*histogram.member).Initialize(histogram.caption, histogram.min, histogram.max, histogram.num_buckets, this);
    }
...
#define HT(name, caption, max, res) { &Counters::name##_, #caption, max, TimedHistogramResolution::res },
        NESTED_TIMED_HISTOGRAM_LIST(HT) NESTED_TIMED_HISTOGRAM_LIST_SLOW(HT)
#undef HT
    };
    for (const auto& timer : kNestedTimedHistograms) {
        (this->*timer.member).Initialize(timer.caption, 0, timer.max, timer.res, DefaultTimedHistogramNumBuckets, this);
    }
...
#undef AHT
    ...
    for (const auto& aht : kAggregatableHistogramTimers) {
        (this->*aht.member).Initialize(aht.caption, 0, 10000000, DefaultTimedHistogramNumBuckets, this);
    }
...
#undef HP
    ...
    for (const auto& percentage : kHistogramPercentages) {
        (this->*percentage.member).Initialize(percentage.caption, 0, 101, 100, this);
    }
...
#undef HM
    ...
    for (const auto& histogram : kLegacyMemoryHistograms) {
        (this->*histogram.member).Initialize(histogram.caption, 1000, 500000, 50, this);
    }
...
#define FIXED_ARRAY_INSTANCE_TYPE_SC(name) COUNT_AND_SIZE_SC(FIXED_ARRAY_##name)
        ...
        // clang-format on
        STATS_COUNTER_LIST_1(BARE_SC) STATS_COUNTER_LIST_2(BARE_SC) STATS_COUNTER_NATIVE_CODE_LIST(BARE_SC) INSTANCE_TYPE_LIST(COUNT_AND_SIZE_SC) CODE_KIND_LIST(CODE_KIND_SC)
