...
public:
    ...
    void LogExistingFunction(Handle<SharedFunctionInfo> shared, Handle<AbstractCode> code, LogEventListener::CodeTag tag = LogEventListener::CodeTag::kFunction);
...
public:
    enum class ScriptEventType { kReserveId, kCreate, kDeserialize, kBackgroundCompile, kStreamingCompile };
    ...
    void FunctionEvent(const char* reason, int script_id, double time_delta_ms, int start_position, int end_position, String function_name);
    ...
    void RegExpCodeCreateEvent(Handle<AbstractCode> code, Handle<String> source) override;
    ...
    void ProcessDeoptEvent(Handle<Code> code, SourcePosition position, const char* kind, const char* reason);

    // Emits a code line info record event.
    void CodeLinePosInfoRecordEvent(Address code_start, ByteArray source_position_table, JitCodeEvent::CodeType code_type);
#if V8_ENABLE_WEBASSEMBLY
    void WasmCodeLinePosInfoRecordEvent(Address code_start, base::Vector<const byte> source_position_table);
#endif // V8_ENABLE_WEBASSEMBLY

    void CodeNameEvent(Address addr, int pos, const char* code_name);

    void ICEvent(const char* type, bool keyed, Handle<Map> map, Handle<Object> key, char old_state, char new_state, const char* modifier, const char* slow_stub_reason);
    ...
    V8_INLINE static void CallEventLoggerInternal(Isolate* isolate, const char* name, v8::LogEventStatus se, bool expose_to_api)
    {
...
private:
    ...
    void CallbackEventInternal(const char* prefix, Handle<Name> name, Address entry_point);
    ...
    void LogSourceCodeInformation(Handle<AbstractCode> code, Handle<SharedFunctionInfo> shared);
    void LogCodeDisassemble(Handle<AbstractCode> code);

    void WriteApiSecurityCheck();
    void WriteApiNamedPropertyAccess(const char* tag, JSObject holder, Object name);
    void WriteApiIndexedPropertyAccess(const char* tag, JSObject holder, uint32_t index);
...
#define V(TimerName, expose)                                                                                                                                                       \
    ...
    public:                                                                                                                                                                        \
        static const char* name(void* unused = nullptr) { return "V8." #TimerName; }                                                                                               \
...
private:
    ...
    virtual void LogRecordedBuffer(Handle<AbstractCode> code, MaybeHandle<SharedFunctionInfo> maybe_shared, const char* name, int length) = 0;
#if V8_ENABLE_WEBASSEMBLY
