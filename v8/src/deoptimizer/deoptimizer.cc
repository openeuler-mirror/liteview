...
private:
    ...
    {
        if (trace_scope_ != nullptr) {
            PrintF(trace_scope_->file(), "    " V8PRIxPTR_FMT ": [top + %3d] <- " V8PRIxPTR_FMT " ;  %s", output_address(top_offset_), top_offset_, value, debug_hint);
        }
    }
    ...
    void DebugPrintOutputPc(intptr_t value, const char* debug_hint = "")
    {
        ...
        if (trace_scope_ != nullptr) {
            PrintF(trace_scope_->file(), "    " V8PRIxPTR_FMT ": [top + %3d] <- " V8PRIxPTR_FMT " (signed) " V8PRIxPTR_FMT " (unsigned) ;  %s", output_address(top_offset_),
                top_offset_, value, PointerAuthentication::StripPAC(value), debug_hint);
        }
#else
        DebugPrintOutputValue(value, debug_hint);
#endif
    ...
    void DebugPrintOutputObject(Object obj, unsigned output_offset, const char* debug_hint = "")
    {
        if (trace_scope_ != nullptr) {
            PrintF(trace_scope_->file(), "    " V8PRIxPTR_FMT ": [top + %3d] <- ", output_address(output_offset), output_offset);
            if (obj.IsSmi()) {
                PrintF(trace_scope_->file(), V8PRIxPTR_FMT " <Smi %d>", obj.ptr(), Smi::cast(obj).value());
            } else {
...
Code Deoptimizer::FindDeoptimizingCode(Address addr)
{
    if (function_.IsHeapObject()) {
        ...
        while (!element.IsUndefined(isolate)) {
            ...
            if (code.contains(isolate, addr))
                return FromCodeT(code);
...
public:
    explicit ActivationsFinder(std::set<CodeT>* codes, CodeT topmost_optimized_code, bool safe_to_deopt_topmost_optimized_code)
        : codes_(codes)
    {
#ifdef DEBUG
            ...
            if (it.frame()->is_optimized()) {
                ...
                if (CodeKindCanDeoptimize(code.kind()) && code.marked_for_deoptimization()) {
                    ...
                    if (code.is_maglevved()) {
                        MaglevSafepointEntry safepoint = code.GetMaglevSafepointEntry(isolate, it.frame()->pc());
                        trampoline_pc = safepoint.trampoline_pc();
                    } else {
                        SafepointEntry safepoint = code.GetSafepointEntry(isolate, it.frame()->pc());
                        trampoline_pc = safepoint.trampoline_pc();
                    }
...
void Deoptimizer::DeoptimizeMarkedCodeForContext(NativeContext native_context)
{
    ...
    while (!element.IsUndefined(isolate)) {
        ...
        if (code.marked_for_deoptimization()) {
            ...
            CodeTPageHeaderModificationScope rwx_write_scope("Storing a CodeT object triggers marking barrier which requires "
    ...
    ActivationsFinder visitor(&codes, topmost_optimized_code, safe_to_deopt_topmost_optimized_code);
...
void Deoptimizer::DeoptimizeFunction(JSFunction function, CodeT code)
{
    ...
    if (CodeKindCanDeoptimize(code.kind())) {
        ...
        function.feedback_vector().EvictOptimizedCodeMarkedForDeoptimization(function.shared(), "unlinking code marked for deopt");
        DeoptimizeMarkedCodeForContext(function.native_context());
    }
}

void Deoptimizer::ComputeOutputFrames(Deoptimizer* deoptimizer) { deoptimizer->DoComputeOutputFrames(); }
...
Deoptimizer::Deoptimizer(Isolate* isolate, JSFunction function, DeoptimizeKind kind, Address from, int fp_to_sp_delta)
    : isolate_(isolate)
    , function_(function)
    ...
    , trace_scope_(v8_flags.trace_deopt || v8_flags.log_deopt ? new CodeTracer::Scope(isolate->GetCodeTracer()) : nullptr)
{
    ...
    const int parameter_count = function.shared().internal_formal_parameter_count_with_receiver();
    ...
    Address deopt_start = compiled_code_.raw_instruction_start() + deopt_data.DeoptExitStart().value();
...
Handle<JSFunction> Deoptimizer::function() const { return Handle<JSFunction>(function_, isolate()); }

Handle<Code> Deoptimizer::compiled_code() const { return Handle<Code>(compiled_code_, isolate()); }
...
bool Deoptimizer::IsDeoptimizationEntry(Isolate* isolate, Address addr, DeoptimizeKind* type_out)
{
...
void Deoptimizer::TraceMarkForDeoptimization(Code code, const char* reason)
{
    ...
    if (maybe_data == ReadOnlyRoots(isolate).empty_fixed_array())
        return;
    ...
    no_gc.Release();
    {
        ...
        PROFILE(isolate, CodeDependencyChangeEvent(handle(code, isolate), handle(SharedFunctionInfo::cast(deopt_data.SharedFunctionInfo()), isolate), reason));
    }
}

// static
void Deoptimizer::TraceEvictFromOptimizedCodeCache(SharedFunctionInfo sfi, const char* reason)
{
...
void Deoptimizer::DoComputeOutputFrames()
{
    ...
    BytecodeOffset bytecode_offset = input_data.GetBytecodeOffset(deopt_exit_index_);
    ...
    translated_state_.Init(isolate_, input_->GetFramePointerAddress(), stack_fp_, &state_iterator, input_data.LiteralArray(), input_->GetRegisterValues(), trace_file,
        function_.IsHeapObject() ? function_.shared().internal_formal_parameter_count_without_receiver() : 0, actual_argument_count_ - kJSArgcReceiverSlots);
    ...
    for (size_t i = 0; i < count; ++i, ++frame_index) {
        ...
        case TranslatedFrame::kJavaScriptBuiltinContinuationWithCatch:
            DoComputeBuiltinContinuation(
                translated_frame, frame_index, handle_exception ? BuiltinContinuationMode::JAVASCRIPT_HANDLE_EXCEPTION : BuiltinContinuationMode::JAVASCRIPT_WITH_CATCH);
            break;
...
void Deoptimizer::DoComputeUnoptimizedFrame(TranslatedFrame* translated_frame, int frame_index, bool goto_catch_handler)
{
    ...
    const int parameters_count = shared.internal_formal_parameter_count_with_receiver();
    ...
    BytecodeArray bytecode_array = shared.HasBreakInfo() ? shared.GetDebugInfo().DebugBytecodeArray() : shared.GetBytecodeArray(isolate());

    // Allocate and store the output frame description.
    FrameDescription* output_frame = new (output_frame_size) FrameDescription(output_frame_size, parameters_count, isolate());
    ...
    const bool deopt_to_baseline = shared.HasBaselineCode() && v8_flags.deopt_to_baseline;
    const bool restart_frame = goto_catch_handler && is_restart_frame();
    CodeT dispatch_builtin = builtins->code(DispatchBuiltinFor(deopt_to_baseline, advance_bc, restart_frame));
    ...
    const intptr_t pc = static_cast<intptr_t>(dispatch_builtin.InstructionStart());
    if (is_topmost) {
    ...
    if (V8_EMBEDDED_CONSTANT_POOL_BOOL) {
        ...
        if (is_topmost) {
            Register constant_pool_reg = UnoptimizedFrame::constant_pool_pointer_register();
    ...
    if (is_topmost) {
        ...
        output_frame->SetContinuation(static_cast<intptr_t>(continuation.InstructionStart()));
    }
}
...
void Deoptimizer::DoComputeInlinedExtraArguments(TranslatedFrame* translated_frame, int frame_index)
{
    ...
    if (verbose_tracing_enabled()) {
        PrintF(trace_scope_->file(), "  translating arguments adaptor => variable_size=%d\n", output_frame_size);
    }

    // Allocate and store the output frame description.
    FrameDescription* output_frame = new (output_frame_size) FrameDescription(output_frame_size, JSParameterCount(argument_count_without_receiver), isolate());
    // The top address of the frame is computed from the previous frame's top and
    // this frame's size.
...
void Deoptimizer::DoComputeConstructStubFrame(TranslatedFrame* translated_frame, int frame_index)
{
    TranslatedFrame::iterator value_iterator = translated_frame->begin();
    const bool is_topmost = (output_count_ - 1 == frame_index);
    ...
    if (verbose_tracing_enabled()) {
        PrintF(trace_scope()->file(),
            ...
            bytecode_offset.ToInt(), bytecode_offset == BytecodeOffset::ConstructStubCreate() ? "create" : "invoke", frame_info.frame_size_in_bytes_without_fixed(),
            output_frame_size);
    }
    ...
    CHECK(bytecode_offset == BytecodeOffset::ConstructStubCreate() || bytecode_offset == BytecodeOffset::ConstructStubInvoke());
    const char* debug_hint = bytecode_offset == BytecodeOffset::ConstructStubCreate() ? "new target\n" : "allocated receiver\n";
    ...
    const int pc_offset = bytecode_offset == BytecodeOffset::ConstructStubCreate() ? isolate_->heap()->construct_stub_create_deopt_pc_offset().value()
...
#if V8_ENABLE_WEBASSEMBLY
TranslatedValue Deoptimizer::TranslatedValueForWasmReturnKind(base::Optional<wasm::ValueKind> wasm_call_return_kind)
{
    if (wasm_call_return_kind) {
        switch (wasm_call_return_kind.value()) {
        case wasm::kI32:
            return TranslatedValue::NewInt32(&translated_state_, (int32_t)input_->GetRegister(kReturnRegister0.code()));
        case wasm::kI64:
            return TranslatedValue::NewInt64ToBigInt(&translated_state_, (int64_t)input_->GetRegister(kReturnRegister0.code()));
        case wasm::kF32:
...
void Deoptimizer::DoComputeBuiltinContinuation(TranslatedFrame* translated_frame, int frame_index, BuiltinContinuationMode mode)
{
    ...
    const int parameters_count = translated_frame->height();
    BuiltinContinuationFrameInfo frame_info = BuiltinContinuationFrameInfo::Precise(parameters_count, continuation_descriptor, config, is_topmost, deopt_kind_, mode);
    ...
    FrameDescription* output_frame = new (output_frame_size) FrameDescription(output_frame_size, frame_info.stack_parameter_count(), isolate());
    output_[frame_index] = output_frame;
    FrameWriter frame_writer(this, output_frame, verbose_trace_scope());
    ...
    if (mode == BuiltinContinuationMode::STUB) {
        DCHECK_EQ(Builtins::CallInterfaceDescriptorFor(builtin).GetStackArgumentOrder(), StackArgumentOrder::kDefault);
        ...
        case BuiltinContinuationMode::JAVASCRIPT_HANDLE_EXCEPTION: {
            intptr_t accumulator_value = input_->GetRegister(kInterpreterAccumulatorRegister.code());
            frame_writer.PushRawObject(Object(accumulator_value), "exception (from accumulator)\n");
        } break;
        }
    ...
    if (is_topmost) {
        // Only the pc of the topmost frame needs to be signed since it is
        // authenticated at the end of the DeoptimizationEntry builtin.
        const intptr_t top_most_pc
            = PointerAuthentication::SignAndCheckPC(isolate(), static_cast<intptr_t>(continue_to_builtin.InstructionStart()), frame_writer.frame()->GetTop());
        output_frame->SetPc(top_most_pc);
    } else {
        output_frame->SetPc(static_cast<intptr_t>(continue_to_builtin.InstructionStart()));
    }
...
void Deoptimizer::MaterializeHeapObjects()
{
    ...
    if (v8_flags.deopt_every_n_times > 0) {
        ...
        isolate_->heap()->CollectAllGarbage(Heap::kNoGCFlags, GarbageCollectionReason::kTesting);
    }
    ...
    for (auto& materialization : values_to_materialize_) {
        ...
        if (verbose_tracing_enabled()) {
            PrintF(
                trace_scope()->file(), "Materialization [" V8PRIxPTR_FMT "] <- " V8PRIxPTR_FMT " ;  ", static_cast<intptr_t>(materialization.output_slot_address_), value->ptr());
...
void Deoptimizer::QueueValueForMaterialization(Address output_address, Object obj, const TranslatedFrame::iterator& iterator)
{
...
// static
int Deoptimizer::ComputeSourcePositionFromBytecodeArray(Isolate* isolate, SharedFunctionInfo shared, BytecodeOffset bytecode_offset)
{
    DCHECK(shared.HasBytecodeArray());
    return AbstractCode::cast(shared.GetBytecodeArray(isolate)).SourcePosition(isolate, bytecode_offset.ToInt());
}
