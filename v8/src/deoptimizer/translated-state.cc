...
void TranslationArrayPrintSingleFrame(std::ostream& os, TranslationArray translation_array, int translation_index, DeoptimizationLiteralArray literal_array)
{
    ...
    os << "  " << TranslationOpcodeToString(opcode) << " {frame count=" << frame_count << ", js frame count=" << jsframe_count
    ...
    while (iterator.HasNext()) {
        ...
        case TranslationOpcode::INTERPRETED_FRAME: {
            ...
            os << "{bytecode_offset=" << bytecode_offset << ", function=" << SharedFunctionInfo::cast(shared_info).DebugNameCStr().get() << ", height=" << height << ", retval=@"
        ...
        case TranslationOpcode::CONSTRUCT_STUB_FRAME: {
            ...
            Object shared_info = literal_array.get(shared_info_id);
            unsigned height = iterator.Next();
            os << "{bailout_id=" << bailout_id << ", function=" << SharedFunctionInfo::cast(shared_info).DebugNameCStr().get() << ", height=" << height << "}";
            break;
        }
        ...
        case TranslationOpcode::INLINED_EXTRA_ARGUMENTS: {
            ...
            Object shared_info = literal_array.get(shared_info_id);
            unsigned height = iterator.Next();
            os << "{function=" << SharedFunctionInfo::cast(shared_info).DebugNameCStr().get() << ", height=" << height << "}";
            break;
        }
...
// static
TranslatedValue TranslatedValue::NewTagged(TranslatedState* container, Object literal)
{
...
// static
TranslatedValue TranslatedValue::NewInvalid(TranslatedState* container) { return TranslatedValue(container, kInvalid); }
...
Object TranslatedValue::GetRawValue() const
{
    ...
    if (materialization_state() == kFinished) {
        int smi;
        if (storage_->IsHeapNumber() && DoubleToSmiInteger(storage_->Number(), &smi)) {
...
Handle<Object> TranslatedValue::GetValue()
{
    ...
    if (value->IsSmi()) {
        ...
        set_initialized_storage(isolate()->factory()->NewHeapNumber(value->Number()));
        return value;
    }
...
Float64 TranslatedState::GetDoubleSlot(Address fp, int slot_offset) { return Float64::FromBits(GetUInt64Slot(fp, slot_offset)); }

void TranslatedValue::Handlify()
{
    if (kind() == kTagged && raw_literal().IsHeapObject()) {
        set_initialized_storage(Handle<HeapObject>(HeapObject::cast(raw_literal()), isolate()));
        raw_literal_ = Object();
    }
}

TranslatedFrame TranslatedFrame::UnoptimizedFrame(BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height, int return_value_offset, int return_value_count)
{
    TranslatedFrame frame(kUnoptimizedFunction, shared_info, height, return_value_offset, return_value_count);
    frame.bytecode_offset_ = bytecode_offset;
    return frame;
}

TranslatedFrame TranslatedFrame::InlinedExtraArguments(SharedFunctionInfo shared_info, int height) { return TranslatedFrame(kInlinedExtraArguments, shared_info, height); }

TranslatedFrame TranslatedFrame::ConstructStubFrame(BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height)
{
...
TranslatedFrame TranslatedFrame::BuiltinContinuationFrame(BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height)
{
...
TranslatedFrame TranslatedFrame::JSToWasmBuiltinContinuationFrame(
    BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height, base::Optional<wasm::ValueKind> return_kind)
{
...
TranslatedFrame TranslatedFrame::JavaScriptBuiltinContinuationFrame(BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height)
{
...
TranslatedFrame TranslatedFrame::JavaScriptBuiltinContinuationWithCatchFrame(BytecodeOffset bytecode_offset, SharedFunctionInfo shared_info, int height)
{
...
int TranslatedFrame::GetValueCount()
{
    ...
    case kUnoptimizedFunction: {
        int parameter_count = raw_shared_info_.internal_formal_parameter_count_with_receiver();
...
void TranslatedFrame::Handlify()
{
    if (!raw_shared_info_.is_null()) {
        shared_info_ = Handle<SharedFunctionInfo>(raw_shared_info_, raw_shared_info_.GetIsolate());
...
TranslatedFrame TranslatedState::CreateNextTranslatedFrame(TranslationArrayIterator* iterator, DeoptimizationLiteralArray literal_array, Address fp, FILE* trace_file)
{
    TranslationOpcode opcode = TranslationOpcodeFromInt(iterator->NextUnsigned());
    switch (opcode) {
    case TranslationOpcode::INTERPRETED_FRAME: {
        ...
        if (trace_file != nullptr) {
            ...
            int arg_count = shared_info.internal_formal_parameter_count_with_receiver();
            PrintF(trace_file,
        ...
        return TranslatedFrame::UnoptimizedFrame(bytecode_offset, shared_info, height, return_value_offset, return_value_count);
    }
    ...
    case TranslationOpcode::CONSTRUCT_STUB_FRAME: {
        ...
        return TranslatedFrame::ConstructStubFrame(bytecode_offset, shared_info, height);
    }
    ...
    case TranslationOpcode::JS_TO_WASM_BUILTIN_CONTINUATION_FRAME: {
        ...
        if (trace_file != nullptr) {
            ...
            PrintF(trace_file, " => bailout_id=%d, height=%d return_type=%d; inputs:\n", bailout_id.ToInt(), height, return_kind.has_value() ? return_kind.value() : -1);
        }
...
void TranslatedState::CreateArgumentsElementsTranslatedValues(int frame_index, Address input_frame_pointer, CreateArgumentsType type, FILE* trace_file)
{
    ...
    frame.Add(TranslatedValue::NewDeferredObject(this, length + FixedArray::kHeaderSize / kTaggedSize, object_index));
...
int TranslatedState::CreateNextTranslatedValue(
    int frame_index, TranslationArrayIterator* iterator, DeoptimizationLiteralArray literal_array, Address fp, RegisterValues* registers, FILE* trace_file)
{
    disasm::NameConverter converter;
    ...
    case TranslationOpcode::SIGNED_BIGINT64_STACK_SLOT: {
        ...
        uint64_t value = GetUInt64Slot(fp, slot_offset);
        if (trace_file != nullptr) {
            PrintF(trace_file, "%" V8PRIdPTR " ; (signed bigint64) [fp %c %3d] ", static_cast<intptr_t>(value), slot_offset < 0 ? '-' : '+', std::abs(slot_offset));
        }
        TranslatedValue translated_value = TranslatedValue::NewInt64ToBigInt(this, value);
        frame.Add(translated_value);
        return translated_value.GetChildrenCount();
    ...
    case TranslationOpcode::UNSIGNED_BIGINT64_STACK_SLOT: {
        ...
        uint64_t value = GetUInt64Slot(fp, slot_offset);
        if (trace_file != nullptr) {
            PrintF(trace_file, "%" V8PRIdPTR " ; (unsigned bigint64) [fp %c %3d] ", static_cast<intptr_t>(value), slot_offset < 0 ? '-' : '+', std::abs(slot_offset));
        }
        TranslatedValue translated_value = TranslatedValue::NewUint64ToBigInt(this, value);
        frame.Add(translated_value);
        return translated_value.GetChildrenCount();
    ...
    case TranslationOpcode::LITERAL: {
        ...
        if (trace_file != nullptr) {
            PrintF(trace_file, V8PRIxPTR_FMT " ; (literal %2d) ", value.ptr(), literal_index);
...
Address TranslatedState::DecompressIfNeeded(intptr_t value)
{
    if (COMPRESS_POINTERS_BOOL) {
        return V8HeapCompressionScheme::DecompressTaggedAny(isolate(), static_cast<uint32_t>(value));
    } else {
        return value;
    }
}
    ...
    : purpose_(kFrameInspection)
{
    ...
    DeoptimizationData data = static_cast<const OptimizedFrame*>(frame)->GetDeoptimizationData(&deopt_index);
    ...
    Init(frame->isolate(), frame->fp(), frame->fp(), &it, data.LiteralArray(), nullptr /* registers */, nullptr /* trace file */,
        frame->function().shared().internal_formal_parameter_count_without_receiver(), actual_argc);
}

void TranslatedState::Init(Isolate* isolate, Address input_frame_pointer, Address stack_frame_pointer, TranslationArrayIterator* iterator, DeoptimizationLiteralArray literal_array,
    RegisterValues* registers, FILE* trace_file, int formal_parameter_count, int actual_argument_count)
{
    ...
    for (int frame_index = 0; frame_index < count; frame_index++) {
        ...
        frames_.push_back(CreateNextTranslatedFrame(iterator, literal_array, input_frame_pointer, trace_file));
    ...
    CHECK(!iterator->HasNext() || TranslationOpcodeFromInt(iterator->NextUnsigned()) == TranslationOpcode::BEGIN);
}
...
void TranslatedState::MaterializeFixedDoubleArray(TranslatedFrame* frame, int* value_index, TranslatedValue* slot, Handle<Map> map)
{
    int length = frame->values_[*value_index].GetSmiValue();
    (*value_index)++;
    Handle<FixedDoubleArray> array = Handle<FixedDoubleArray>::cast(isolate()->factory()->NewFixedDoubleArray(length));
    CHECK_GT(length, 0);
...
void TranslatedState::EnsureCapturedObjectAllocatedAt(int object_index, std::stack<int>* worklist)
{
    ...
    default:
        ...
        value_index++, remaining_children_count--;
        if (elements_slot->kind() == TranslatedValue::kCapturedObject || !map->IsJSArrayMap()) {
...
void TranslatedValue::ReplaceElementsArrayWithCopy()
{
    ...
    if (elements->IsFixedDoubleArray()) {
        ...
        set_storage(isolate()->factory()->CopyFixedDoubleArray(Handle<FixedDoubleArray>::cast(elements)));
    } else if (!elements->IsCowArray()) {
        set_storage(isolate()->factory()->CopyFixedArray(Handle<FixedArray>::cast(elements)));
    }
}
...
void TranslatedState::EnsurePropertiesAllocatedAndMarked(TranslatedValue* properties_slot, Handle<Map> map)
{
...
void TranslatedState::EnsureJSObjectAllocated(TranslatedValue* slot, Handle<Map> map)
{
    CHECK(map->IsJSObjectMap());
...
void TranslatedState::InitializeJSObjectAt(TranslatedFrame* frame, int* value_index, TranslatedValue* slot, Handle<Map> map, const DisallowGarbageCollection& no_gc)
{
    ...
    isolate()->heap()->NotifyObjectLayoutChange(*object_storage, no_gc, InvalidateRecordedSlots::kNo);
    ...
    for (int i = 2; i < children_count; i++) {
        ...
        if (marker == kStoreHeapObject) {
            ...
            DCHECK_IMPLIES(field_value->IsHeapNumber(), !IsSmiDouble(field_value->Number()));
            WRITE_FIELD(*object_storage, offset, *field_value);
            WRITE_BARRIER(*object_storage, offset, *field_value);
        }
    }
...
void TranslatedState::InitializeObjectWithTaggedFieldsAt(TranslatedFrame* frame, int* value_index, TranslatedValue* slot, Handle<Map> map, const DisallowGarbageCollection& no_gc)
{
    Handle<HeapObject> object_storage = Handle<HeapObject>::cast(slot->storage_);
    int children_count = slot->GetChildrenCount();
...
void TranslatedState::StoreMaterializedValuesAndDeopt(JavaScriptFrame* frame)
{
    ...
    for (int i = 0; i < length; i++) {
        ...
        Handle<Object> previous_value(previously_materialized_objects->get(i), isolate_);
        ...
        if (value.is_identical_to(marker)) {
            ...
            if (*previous_value == *marker) {
                ...
                CHECK(*previous_value == *value || (previous_value->IsHeapNumber() && value->IsSmi() && previous_value->Number() == value->Number()));
            }
        }
    }
    ...
    if (new_store && value_changed) {
        ...
        Deoptimizer::DeoptimizeFunction(frame->function(), frame->LookupCodeT().ToCodeT());
    }
}
...
void TranslatedState::ReadUpdateFeedback(TranslationArrayIterator* iterator, DeoptimizationLiteralArray literal_array, FILE* trace_file)
{
    CHECK_EQ(TranslationOpcode::UPDATE_FEEDBACK, TranslationOpcodeFromInt(iterator->NextUnsigned()));
