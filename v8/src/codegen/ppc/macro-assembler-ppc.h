...
public:
...
#endif
        const Register dst, const DoubleRegister double_dst, FPRoundingMode rounding_mode = kRoundToZero);
    ...
    void DropArguments(Register count, ArgumentsCountType type, ArgumentsCountMode mode);
    void DropArgumentsAndPushNewReceiver(Register argc, Register receiver, ArgumentsCountType type, ArgumentsCountMode mode);
    ...
    void LoadRoot(Register destination, RootIndex index) final { LoadRoot(destination, index, al); }
    ...
    void CanonicalizeNaN(const DoubleRegister value) { CanonicalizeNaN(value, value); }
    ...
    void SmiTag(Register dst, Register src, RCBit rc = LeaveRC) { ShiftLeftU64(dst, src, Operand(kSmiShift), rc); }

    // Abort execution if argument is a smi, enabled via --debug-code.
    void AssertNotSmi(Register object) NOOP_UNLESS_DEBUG_CODE void AssertSmi(Register object) NOOP_UNLESS_DEBUG_CODE
...
#else
            rlwinm(dst, src, rotate, kBitsPerSystemPointer - width, kBitsPerSystemPointer - 1, rc);
#endif
        }
    }

    inline void ExtractBit(Register dst, Register src, uint32_t bitNumber, RCBit rc = LeaveRC, bool test = false) { ExtractBitRange(dst, src, bitNumber, bitNumber, rc, test); }
    ...
    inline void TestBit(Register value, int bitNumber, Register scratch = r0) { ExtractBitRange(scratch, value, bitNumber, bitNumber, SetRC, true); }

    // Test consecutive bit range in value.  Range is defined by mask.
    inline void TestBitMask(Register value, uintptr_t mask, Register scratch = r0) { ExtractBitMask(scratch, value, mask, SetRC, true); }
    // Test consecutive bit range in value.  Range is defined by
    // rangeStart - rangeEnd.
    inline void TestBitRange(Register value, int rangeStart, int rangeEnd, Register scratch = r0) { ExtractBitRange(scratch, value, rangeStart, rangeEnd, SetRC, true); }

    inline void TestIfSmi(Register value, Register scratch) { TestBitRange(value, kSmiTagSize - 1, 0, scratch); }
...
#else
    inline void TestIfInt32(Register hi_word, Register lo_word, Register scratch, CRegister cr = cr7)
    {
        // High bits must be identical to fit into an 32-bit integer
...
#undef SIMD_UNOP_LIST
    ...
    void I64x2Abs(Simd128Register dst, Simd128Register src, Simd128Register scratch);
    void I32x4Abs(Simd128Register dst, Simd128Register src, Simd128Register scratch);
    void I16x8Abs(Simd128Register dst, Simd128Register src, Simd128Register scratch);
    void I16x8Neg(Simd128Register dst, Simd128Register src, Simd128Register scratch);
    void I8x16Abs(Simd128Register dst, Simd128Register src, Simd128Register scratch);
    void I8x16Neg(Simd128Register dst, Simd128Register src, Simd128Register scratch);
...
public:
    ...
    // arguments order.
    void LoadReceiver(Register dest, Register argc) { LoadU64(dest, MemOperand(sp, 0)); }

    void StoreReceiver(Register rec, Register argc, Register scratch) { StoreU64(rec, MemOperand(sp, 0)); }
    ...
    void RecordWriteField(
        Register object, int offset, Register value, Register slot_address, LinkRegisterStatus lr_status, SaveFPRegsMode save_fp, SmiCheck smi_check = SmiCheck::kInline);
...
#define SmiWordOffset(offset) offset
    ...
    template <typename Field> void DecodeField(Register reg, RCBit rc = LeaveRC) { DecodeField<Field>(reg, reg, rc); }

    void TestCodeTIsMarkedForDeoptimization(Register codet, Register scratch1, Register scratch2);
