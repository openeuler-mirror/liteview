...
#if !(V8_HOST_ARCH_PPC || V8_HOST_ARCH_PPC64) || !V8_TARGET_ARCH_PPC64 || V8_TARGET_LITTLE_ENDIAN || (defined(_CALL_ELF) && _CALL_ELF == 2)
...
#define DECLARE_STATIC_ACCESSOR(Name) DECLARE_STATIC_TYPED_ACCESSOR(int, Name)
    ...
    inline uint32_t BitField(int hi, int lo) const { return InstructionBits() & (((2 << (hi - lo)) - 1) << lo); }
    ...
    // Read a bit field out of the instruction bits.
    static inline uint32_t BitField(Instr instr, int hi, int lo) { return instr & (((2 << (hi - lo)) - 1) << lo); }
    ...
    inline uint32_t OpcodeField() const { return static_cast<Opcode>(BitField(31, 26)); }
    inline uint32_t PrefixOpcodeField() const { return static_cast<Opcode>(BitField(31, 25)); }
