...
inline MemOperand FieldMemOperand(Register object, int offset) { return MemOperand(object, offset - kHeapObjectTag); }
...
#define DECLARE_BRANCH_PROTOTYPES(Name)                                                                                                                                            \
    ...
    void Branch(Label* target, Condition cond, Register r1, const Operand& r2, Label::Distance near_jump = Label::kFar);
    void Branch(int32_t target, Condition cond, Register r1, const Operand& r2, Label::Distance near_jump = Label::kFar);
...
#undef COND_ARGS
    ...
    inline void li(Register rd, intptr_t j, LiFlags mode = OPTIMIZE_SIZE) { li(rd, Operand(j), mode); }

    inline void Move(Register output, MemOperand operand) { LoadWord(output, operand); }
...
#define COND_ARGS Condition cond = al, Register rs = zero_reg, const Operand &rt = Operand(zero_reg)
    ...
    void Pop(uint32_t count = 1) { AddWord(sp, sp, Operand(count * kSystemPointerSize)); }
...
#undef DEFINE_INSTRUCTION3
    ...
    inline void PrepareCEntryFunction(const ExternalReference& ref) { li(a1, ref); }
...
#undef COND_ARGS
    ...
    // Change endianness
    void ByteSwap(Register dest, Register src, int operand_size, Register scratch);
    ...
    inline void ExtractLowWordFromF64(Register dst_low, FPURegister src) { fmv_x_w(dst_low, src); }

    // Insert low-word from GPR (src_high) to the low-half of FPR (dst)
    void InsertLowWordF64(FPURegister dst, Register src_low);

    void LoadFPRImmediate(FPURegister dst, float imm) { LoadFPRImmediate(dst, base::bit_cast<uint32_t>(imm)); }
    void LoadFPRImmediate(FPURegister dst, double imm) { LoadFPRImmediate(dst, base::bit_cast<uint64_t>(imm)); }
    ...
    void JumpIfEqual(Register a, int32_t b, Label* dest) { Branch(dest, eq, a, Operand(b)); }

    void JumpIfLessThan(Register a, int32_t b, Label* dest) { Branch(dest, lt, a, Operand(b)); }
    ...
    // simplify builtins.
    inline void LoadTaggedPointerField(const Register& destination, const MemOperand& field_operand) { Lw(destination, field_operand); }
    inline void LoadAnyTaggedField(const Register& destination, const MemOperand& field_operand) { Lw(destination, field_operand); }
    inline void LoadTaggedSignedField(const Register& destination, const MemOperand& field_operand) { Lw(destination, field_operand); }

    inline void SmiUntagField(Register dst, const MemOperand& src) { SmiUntag(dst, src); }

    // Compresses and stores tagged value to given on-heap location.
    void StoreTaggedField(const Register& value, const MemOperand& dst_field_operand) { Sw(value, dst_field_operand); }
#endif
...
public:
    ...
    // arguments order.
    void LoadReceiver(Register dest, Register argc) { LoadWord(dest, MemOperand(sp, 0)); }

    void StoreReceiver(Register rec, Register argc, Register scratch) { StoreWord(rec, MemOperand(sp, 0)); }
    ...
    inline void SmiTst(Register value, Register scratch) { And(scratch, value, Operand(kSmiTagMask)); }

    enum ArgumentsCountMode { kCountIncludesReceiver, kCountExcludesReceiver };
    enum ArgumentsCountType { kCountIsInteger, kCountIsSmi, kCountIsBytes };
    void DropArguments(Register count, ArgumentsCountType type, ArgumentsCountMode mode, Register scratch = no_reg);
    void DropArgumentsAndPushNewReceiver(Register argc, Register receiver, ArgumentsCountType type, ArgumentsCountMode mode, Register scratch = no_reg);
    ...
    template <typename Field> void DecodeField(Register dst, Register src) { ExtractBits(dst, src, Field::kShift, Field::kSize); }
