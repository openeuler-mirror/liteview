...
public:
    ...
    {
        return this->SafepointEntryBase::operator==(other) && tagged_register_indexes_ == other.tagged_register_indexes_ && tagged_slots_ == other.tagged_slots_;
    }
...
public:
    ...
    int byte_size() const { return kHeaderSize + length_ * (entry_size() + tagged_slots_bytes()); }
...
private:
    ...
    int tagged_slots_bytes() const { return TaggedSlotsBytesField::decode(entry_configuration_); }
...
private:
    ...
    int min_stack_index() const { return min_stack_index_ == std::numeric_limits<int>::max() ? 0 : min_stack_index_; }
