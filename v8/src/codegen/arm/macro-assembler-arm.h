...
// register's value, in the version that takes a register.
    ...
    void VFPCanonicalizeNaN(const DwVfpRegister value, const Condition cond = al) { VFPCanonicalizeNaN(value, value, cond); }
    ...
    void SmiUntag(Register reg, SBit s = LeaveCC) { mov(reg, Operand::SmiUntag(reg), s); }
    void SmiUntag(Register dst, Register src, SBit s = LeaveCC) { mov(dst, Operand::SmiUntag(src), s); }
...
public:
    ...
    // arguments order.
    MemOperand ReceiverOperand(Register argc) { return MemOperand(sp, 0); }

    // Tiering support.
    void AssertFeedbackVector(Register object) NOOP_UNLESS_DEBUG_CODE void ReplaceClosureCodeWithOptimizedCode(Register optimized_code, Register closure);
