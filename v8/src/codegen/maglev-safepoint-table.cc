...
MaglevSafepointTable::MaglevSafepointTable(Isolate* isolate, Address pc, Code code)
    : MaglevSafepointTable(code.InstructionStart(isolate, pc), code.SafepointTableAddress())
{
    DCHECK(code.is_maglevved());
}

#ifdef V8_EXTERNAL_CODE_SPACE
MaglevSafepointTable::MaglevSafepointTable(Isolate* isolate, Address pc, CodeDataContainer code)
    : MaglevSafepointTable(code.InstructionStart(isolate, pc), code.SafepointTableAddress())
{
    DCHECK(code.is_maglevved());
}
...
MaglevSafepointEntry MaglevSafepointTable::FindEntry(Address pc) const
{
    ...
    return MaglevSafepointEntry(pc_offset, deopt_index, num_tagged_slots_, num_untagged_slots_, num_pushed_registers, tagged_register_indexes, trampoline_pc);
}

void MaglevSafepointTable::Print(std::ostream& os) const
{
    os << "Safepoints (entries = " << length_ << ", byte size = " << byte_size() << ", tagged slots = " << num_tagged_slots_ << ", untagged slots = " << num_untagged_slots_
       << ")\n";
    ...
    for (int index = 0; index < length_; index++) {
        ...
        os << "  num pushed registers: " << static_cast<int>(entry.num_pushed_registers());
...
void MaglevSafepointTableBuilder::Emit(Assembler* assembler)
{
    ...
    uint32_t entry_configuration = MaglevSafepointTable::HasDeoptDataField::encode(has_deopt_data) | MaglevSafepointTable::RegisterIndexesSizeField::encode(register_indexes_size)
        | MaglevSafepointTable::PcSizeField::encode(pc_size) | MaglevSafepointTable::DeoptIndexSizeField::encode(deopt_index_size);
