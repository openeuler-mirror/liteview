...
void TurboAssembler::LoadRootRelative(Register destination, int32_t offset) { movq(destination, Operand(kRootRegister, offset)); }
...
void TurboAssembler::LoadTaggedPointerField(Register destination, Operand field_operand)
{
...
void TurboAssembler::LoadTaggedPointerField(TaggedRegister destination, Operand field_operand)
{
    if (COMPRESS_POINTERS_BOOL) {
...
void TurboAssembler::LoadAnyTaggedField(Register destination, Operand field_operand)
{
    if (COMPRESS_POINTERS_BOOL) {
...
void TurboAssembler::LoadAnyTaggedField(TaggedRegister destination, Operand field_operand)
{
    if (COMPRESS_POINTERS_BOOL) {
        movl(destination.reg(), field_operand);
    } else {
...
void TurboAssembler::PushTaggedPointerField(Operand field_operand, Register scratch)
{
    if (COMPRESS_POINTERS_BOOL) {
...
void TurboAssembler::PushTaggedAnyField(Operand field_operand, Register scratch)
{
    if (COMPRESS_POINTERS_BOOL) {
        DCHECK(!field_operand.AddressUsesRegister(scratch));
...
void TurboAssembler::SmiUntagField(Register dst, Operand src) { SmiUntag(dst, src); }

void TurboAssembler::SmiUntagFieldUnsigned(Register dst, Operand src) { SmiUntagUnsigned(dst, src); }

void TurboAssembler::StoreTaggedField(Operand dst_field_operand, Immediate value)
{
    if (COMPRESS_POINTERS_BOOL) {
...
void TurboAssembler::StoreTaggedField(Operand dst_field_operand, Register value)
{
    if (COMPRESS_POINTERS_BOOL) {
        movl(dst_field_operand, value);
    } else {
...
void TurboAssembler::StoreTaggedSignedField(Operand dst_field_operand, Smi value)
{
...
void TurboAssembler::DecompressTaggedPointer(Register destination, Operand field_operand)
{
    ASM_CODE_COMMENT(this);
    movl(destination, field_operand);
...
void TurboAssembler::LoadExternalPointerField(Register destination, Operand field_operand, ExternalPointerTag tag, Register scratch, IsolateRootLocation isolateRootLocation)
{
    ...
    if (IsSandboxedExternalPointerType(tag)) {
        ...
        if (isolateRootLocation == IsolateRootLocation::kInRootRegister) {
            DCHECK(root_array_available_);
            movq(scratch, Operand(kRootRegister, IsolateData::external_pointer_table_offset() + Internals::kExternalPointerTableBufferOffset));
        } else {
            DCHECK(isolateRootLocation == IsolateRootLocation::kInScratchRegister);
            movq(scratch, Operand(scratch, IsolateData::external_pointer_table_offset() + Internals::kExternalPointerTableBufferOffset));
        }
        movl(destination, field_operand);
...
#ifdef V8_IS_TSAN
void TurboAssembler::CallTSANStoreStub(Register address, Register value, SaveFPRegsMode fp_mode, int size, StubCallMode mode, std::memory_order order)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::CallTSANRelaxedLoadStub(Register address, SaveFPRegsMode fp_mode, int size, StubCallMode mode)
{
...
void MacroAssembler::RecordWrite(Register object, Register slot_address, Register value, SaveFPRegsMode fp_mode, SmiCheck smi_check)
{
    ...
    CheckPageFlag(value,
        ...
        MemoryChunk::kPointersToHereAreInterestingOrInSharedHeapMask, zero, &done, Label::kNear);
...
void MacroAssembler::ReplaceClosureCodeWithOptimizedCode(Register optimized_code, Register closure, Register scratch1, Register slot_address)
{
    ...
    StoreTaggedField(FieldOperand(closure, JSFunction::kCodeOffset), optimized_code);
...
void MacroAssembler::OptimizeCodeOrTailCallOptimizedCodeSlot(Register flags, Register feedback_vector, Register closure, JumpMode jump_mode)
{
    ASM_CODE_COMMENT(this);
...
namespace {
template <typename OperandOrXMMRegister, bool is_double> void ConvertFloatToUint64(TurboAssembler* tasm, Register dst, OperandOrXMMRegister src, Label* fail)
{
    Label success;
...
template <typename OperandOrXMMRegister, bool is_double> void ConvertFloatToUint32(TurboAssembler* tasm, Register dst, OperandOrXMMRegister src, Label* fail)
{
    Label success;
    // There does not exist a native float-to-uint instruction, so we have to use
    // a float-to-int, and postprocess the result.
...
void TurboAssembler::Cvttsd2uiq(Register dst, Operand src, Label* fail) { ConvertFloatToUint64<Operand, true>(this, dst, src, fail); }

void TurboAssembler::Cvttsd2uiq(Register dst, XMMRegister src, Label* fail) { ConvertFloatToUint64<XMMRegister, true>(this, dst, src, fail); }

void TurboAssembler::Cvttsd2ui(Register dst, Operand src, Label* fail) { ConvertFloatToUint32<Operand, true>(this, dst, src, fail); }

void TurboAssembler::Cvttsd2ui(Register dst, XMMRegister src, Label* fail) { ConvertFloatToUint32<XMMRegister, true>(this, dst, src, fail); }

void TurboAssembler::Cvttss2uiq(Register dst, Operand src, Label* fail) { ConvertFloatToUint64<Operand, false>(this, dst, src, fail); }

void TurboAssembler::Cvttss2uiq(Register dst, XMMRegister src, Label* fail) { ConvertFloatToUint64<XMMRegister, false>(this, dst, src, fail); }

void TurboAssembler::Cvttss2ui(Register dst, Operand src, Label* fail) { ConvertFloatToUint32<Operand, false>(this, dst, src, fail); }

void TurboAssembler::Cvttss2ui(Register dst, XMMRegister src, Label* fail) { ConvertFloatToUint32<XMMRegister, false>(this, dst, src, fail); }
...
void TurboAssembler::JumpIfNotSmi(Register src, Label* on_not_smi, Label::Distance near_jump)
{
    Condition smi = CheckSmi(src);
    j(NegateCondition(smi), on_not_smi, near_jump);
}

void TurboAssembler::JumpIfNotSmi(Operand src, Label* on_not_smi, Label::Distance near_jump)
{
    Condition smi = CheckSmi(src);
    j(NegateCondition(smi), on_not_smi, near_jump);
}
...
void MacroAssembler::Cmp(Register dst, Handle<Object> source)
{
    if (source->IsSmi()) {
        ...
        EmbeddedObjectIndex index = AddEmbeddedObject(Handle<HeapObject>::cast(source));
...
void MacroAssembler::Cmp(Operand dst, Handle<Object> source)
{
    if (source->IsSmi()) {
        ...
        Move(kScratchRegister, Handle<HeapObject>::cast(source), COMPRESS_POINTERS_BOOL ? RelocInfo::COMPRESSED_EMBEDDED_OBJECT : RelocInfo::FULL_EMBEDDED_OBJECT);
        cmp_tagged(dst, kScratchRegister);
    }
}
...
void TurboAssembler::Move(Register result, Handle<HeapObject> object, RelocInfo::Mode rmode)
{
    // TODO(jgruber,v8:8887): Also consider a root-relative load when generating
    // non-isolate-independent code. In many cases it might be cheaper than
    // embedding the relocatable value.
...
void TurboAssembler::Move(Operand dst, Handle<HeapObject> object, RelocInfo::Mode rmode)
{
...
void TurboAssembler::Jump(Handle<CodeT> code_object, RelocInfo::Mode rmode, Condition cc)
{
    DCHECK_IMPLIES(options().isolate_independent_code, Builtins::IsIsolateIndependentBuiltin(*code_object));
    Builtin builtin = Builtin::kNoBuiltinId;
    if (isolate()->builtins()->IsBuiltinHandle(code_object, &builtin)) {
...
Operand TurboAssembler::EntryFromBuiltinIndexAsOperand(Register builtin_index)
{
    if (SmiValuesAre32Bits()) {
        ...
        return Operand(kRootRegister, builtin_index, times_system_pointer_size, IsolateData::builtin_entry_table_offset());
    } else {
        DCHECK(SmiValuesAre31Bits());
...
void TurboAssembler::CallBuiltinByIndex(Register builtin_index) { Call(EntryFromBuiltinIndexAsOperand(builtin_index)); }
...
void TurboAssembler::LoadCodeObjectEntry(Register destination, Register code_object)
{
    ...
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        movq(destination, FieldOperand(code_object, CodeDataContainer::kCodeEntryPointOffset));
        return;
    }
    ...
    if (options().isolate_independent_code) {
        ...
        testl(FieldOperand(code_object, Code::kFlagsOffset), Immediate(Code::IsOffHeapTrampoline::kMask));
        ...
        movq(destination, Operand(kRootRegister, destination, times_system_pointer_size, IsolateData::builtin_entry_table_offset()));
...
void TurboAssembler::LoadCodeDataContainerEntry(Register destination, Register code_data_container_object)
{
    ...
    movq(destination, FieldOperand(code_data_container_object, CodeDataContainer::kCodeEntryPointOffset));
}
...
void TurboAssembler::LoadCodeDataContainerCodeNonBuiltin(Register destination, Register code_data_container_object)
{
    ...
    movq(destination, FieldOperand(code_data_container_object, CodeDataContainer::kCodeOffset));
}
...
void TurboAssembler::CodeDataContainerFromCodeT(Register destination, Register codet)
{
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        Move(destination, codet);
    } else {
        LoadTaggedPointerField(destination, FieldOperand(codet, Code::kCodeDataContainerOffset));
    }
}
...
namespace {
template <typename Op> void PinsrdPreSse41Helper(TurboAssembler* tasm, XMMRegister dst, Op src, uint8_t imm8, uint32_t* load_pc_offset)
{
    tasm->Movd(kScratchDoubleReg, src);
    if (load_pc_offset)
        *load_pc_offset = tasm->pc_offset();
    if (imm8 == 1) {
...
void TurboAssembler::PinsrdPreSse41(XMMRegister dst, Register src, uint8_t imm8, uint32_t* load_pc_offset) { PinsrdPreSse41Helper(this, dst, src, imm8, load_pc_offset); }

void TurboAssembler::PinsrdPreSse41(XMMRegister dst, Operand src, uint8_t imm8, uint32_t* load_pc_offset) { PinsrdPreSse41Helper(this, dst, src, imm8, load_pc_offset); }

void TurboAssembler::Pinsrq(XMMRegister dst, XMMRegister src1, Register src2, uint8_t imm8, uint32_t* load_pc_offset)
{
    PinsrHelper(this, &Assembler::vpinsrq, &Assembler::pinsrq, dst, src1, src2, imm8, load_pc_offset, { SSE4_1 });
}

void TurboAssembler::Pinsrq(XMMRegister dst, XMMRegister src1, Operand src2, uint8_t imm8, uint32_t* load_pc_offset)
{
    PinsrHelper(this, &Assembler::vpinsrq, &Assembler::pinsrq, dst, src1, src2, imm8, load_pc_offset, { SSE4_1 });
}
...
void TurboAssembler::IncsspqIfSupported(Register number_of_words, Register scratch)
{
...
void MacroAssembler::CmpInstanceType(Register map, InstanceType type) { cmpw(FieldOperand(map, Map::kInstanceTypeOffset), Immediate(type)); }
...
{
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        testl(FieldOperand(codet, CodeDataContainer::kKindSpecificFlagsOffset), Immediate(1 << Code::kMarkedForDeoptimizationBit));
    } else {
        LoadTaggedPointerField(scratch, FieldOperand(codet, Code::kCodeDataContainerOffset));
        testl(FieldOperand(scratch, CodeDataContainer::kKindSpecificFlagsOffset), Immediate(1 << Code::kMarkedForDeoptimizationBit));
    }
}

Immediate MacroAssembler::ClearedValue() const { return Immediate(static_cast<int32_t>(HeapObjectReference::ClearedValue(isolate()).ptr())); }
...
{
    ASM_CODE_COMMENT(this);
    LoadTaggedPointerField(rbx, FieldOperand(function, JSFunction::kSharedFunctionInfoOffset));
...
void MacroAssembler::InvokeFunction(Register function, Register new_target, Register expected_parameter_count, Register actual_parameter_count, InvokeType type)
{
    ...
    LoadTaggedPointerField(rsi, FieldOperand(function, JSFunction::kContextOffset));
...
void MacroAssembler::EnterExitFrameEpilogue(int arg_stack_space, bool save_doubles)
{
    ASM_CODE_COMMENT(this);
#ifdef V8_TARGET_OS_WIN
...
void MacroAssembler::EnterExitFrame(int arg_stack_space, bool save_doubles, StackFrame::Type frame_type)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::CallCFunction(Register function, int num_arguments)
{
    ...
    if (root_array_available()) {
        movq(Operand(kRootRegister, IsolateData::fast_c_call_caller_pc_offset()), kScratchRegister);
        movq(Operand(kRootRegister, IsolateData::fast_c_call_caller_fp_offset()), rbp);
    } else {
        ...
        movq(ExternalReferenceAsOperand(ExternalReference::fast_c_call_caller_pc_address(isolate()), scratch), kScratchRegister);
        movq(ExternalReferenceAsOperand(ExternalReference::fast_c_call_caller_fp_address(isolate())), rbp);
    ...
    if (root_array_available()) {
        movq(Operand(kRootRegister, IsolateData::fast_c_call_caller_fp_offset()), Immediate(0));
    } else {
        DCHECK_NOT_NULL(isolate());
        movq(ExternalReferenceAsOperand(ExternalReference::fast_c_call_caller_fp_address(isolate())), Immediate(0));
    }
...
void TurboAssembler::CheckPageFlag(Register object, Register scratch, int mask, Condition cc, Label* condition_met, Label::Distance condition_met_distance)
{
    ...
    if (mask < (1 << kBitsPerByte)) {
        testb(Operand(scratch, BasicMemoryChunk::kFlagsOffset), Immediate(static_cast<uint8_t>(mask)));
    } else {
...
void TurboAssembler::BailoutIfDeoptimized(Register scratch)
{
    ...
    testl(FieldOperand(scratch, CodeDataContainer::kKindSpecificFlagsOffset), Immediate(1 << Code::kMarkedForDeoptimizationBit));
    Jump(BUILTIN_CODE(isolate(), CompileLazyDeoptimizedCode), RelocInfo::CODE_TARGET, not_zero);
}
