...
{
    printf("SSE3=%d SSSE3=%d SSE4_1=%d SSE4_2=%d SAHF=%d AVX=%d AVX2=%d FMA3=%d "
           "BMI1=%d "
           "BMI2=%d "
           "LZCNT=%d "
           "POPCNT=%d ATOM=%d\n",
        CpuFeatures::IsSupported(SSE3), CpuFeatures::IsSupported(SSSE3), CpuFeatures::IsSupported(SSE4_1), CpuFeatures::IsSupported(SSE4_2), CpuFeatures::IsSupported(SAHF),
        CpuFeatures::IsSupported(AVX), CpuFeatures::IsSupported(AVX2), CpuFeatures::IsSupported(FMA3), CpuFeatures::IsSupported(BMI1), CpuFeatures::IsSupported(BMI2),
...
Operand::Operand(Operand operand, int32_t offset)
{
    ...
    if (mode == 0x80 || is_baseless) {
        ...
        disp_value = ReadUnalignedValue<int32_t>(reinterpret_cast<Address>(&operand.data().buf[disp_offset]));
    } else if (mode == 0x40) {
    ...
    if (!is_int8(disp_value) || is_baseless) {
        ...
        WriteUnalignedValue(reinterpret_cast<Address>(&data_.buf[disp_offset]), disp_value);
...
bool ConstPool::IsMoveRipRelative(Address instr) { return (ReadUnalignedValue<uint32_t>(instr) & kMoveRipRelativeMask) == kMoveRipRelativeInstr; }

void ConstPool::Clear() { entries_.clear(); }

void ConstPool::PatchEntries()
{
    for (EntryMap::iterator iter = entries_.begin(); iter != entries_.end(); iter = entries_.upper_bound(iter->first)) {
...
void Assembler::GetCode(Isolate* isolate, CodeDesc* desc, SafepointTableBuilderBase* safepoint_table_builder, int handler_table_offset)
{
...
void Assembler::emit_operand(int code, Operand adr)
{
    ...
    *pc_++ = adr.data().buf[0] | code << 3;
    ...
    if (adr.data().buf[0] == 5) {
        DCHECK_EQ(9u, length);
        Label* label = ReadUnalignedValue<Label*>(reinterpret_cast<Address>(&adr.data().buf[1]));
        if (label->is_bound()) {
...
void Assembler::arithmetic_op(byte opcode, Register reg, Register rm_reg, int size)
{
    EnsureSpace ensure_space(this);
...
void Assembler::immediate_arithmetic_op(byte subcode, Register dst, Immediate src, int size)
{
    EnsureSpace ensure_space(this);
    emit_rex(dst, size);
...
void Assembler::immediate_arithmetic_op(byte subcode, Operand dst, Immediate src, int size)
{
    EnsureSpace ensure_space(this);
    emit_rex(dst, size);
    if (is_int8(src.value_) && RelocInfo::IsNoInfo(src.rmode_)) {
...
void Assembler::immediate_arithmetic_op_16(byte subcode, Register dst, Immediate src)
{
    EnsureSpace ensure_space(this);
...
void Assembler::immediate_arithmetic_op_16(byte subcode, Operand dst, Immediate src)
{
    EnsureSpace ensure_space(this);
    emit(0x66); // Operand size override prefix.
    emit_optional_rex_32(dst);
...
void Assembler::immediate_arithmetic_op_8(byte subcode, Operand dst, Immediate src)
{
    EnsureSpace ensure_space(this);
    emit_optional_rex_32(dst);
...
void Assembler::immediate_arithmetic_op_8(byte subcode, Register dst, Immediate src)
{
    EnsureSpace ensure_space(this);
...
void Assembler::load_rax(ExternalReference ref) { load_rax(ref.address(), RelocInfo::EXTERNAL_REFERENCE); }
...
void Assembler::store_rax(ExternalReference ref) { store_rax(ref.address(), RelocInfo::EXTERNAL_REFERENCE); }
...
void Assembler::testb(Operand op, Register reg) { emit_test(op, reg, sizeof(int8_t)); }

void Assembler::testw(Register dst, Register src) { emit_test(dst, src, sizeof(uint16_t)); }

void Assembler::testw(Register reg, Immediate mask) { emit_test(reg, mask, sizeof(int16_t)); }

void Assembler::testw(Operand op, Immediate mask) { emit_test(op, mask, sizeof(int16_t)); }

void Assembler::testw(Operand op, Register reg) { emit_test(op, reg, sizeof(int16_t)); }
...
void Assembler::pinsrd(XMMRegister dst, Register src, uint8_t imm8) { sse4_instr(dst, src, 0x66, 0x0F, 0x3A, 0x22, imm8); }
...
void Assembler::pinsrb(XMMRegister dst, Register src, uint8_t imm8) { sse4_instr(dst, src, 0x66, 0x0F, 0x3A, 0x20, imm8); }
...
void Assembler::cvtdq2pd(XMMRegister dst, XMMRegister src) { sse2_instr(dst, src, 0xF3, 0x0F, 0xE6); }
...
void Assembler::fma_instr(byte op, XMMRegister dst, XMMRegister src1, XMMRegister src2, VectorLength l, SIMDPrefix pp, LeadingOpcode m, VexW w)
{
...
void Assembler::fma_instr(byte op, XMMRegister dst, XMMRegister src1, Operand src2, VectorLength l, SIMDPrefix pp, LeadingOpcode m, VexW w)
{
    DCHECK(IsEnabled(FMA3));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(dst, src1, src2, l, pp, m, w);
...
void Assembler::vinstr(byte op, XMMRegister dst, XMMRegister src1, XMMRegister src2, SIMDPrefix pp, LeadingOpcode m, VexW w, CpuFeature feature)
{
...
void Assembler::vinstr(byte op, XMMRegister dst, XMMRegister src1, Operand src2, SIMDPrefix pp, LeadingOpcode m, VexW w, CpuFeature feature)
{
    DCHECK(IsEnabled(feature));
    DCHECK(feature == AVX || feature == AVX2);
    EnsureSpace ensure_space(this);
...
template <typename Reg1, typename Reg2, typename Op> void Assembler::vinstr(byte op, Reg1 dst, Reg2 src1, Op src2, SIMDPrefix pp, LeadingOpcode m, VexW w, CpuFeature feature)
{
    DCHECK(IsEnabled(feature));
    DCHECK(feature == AVX || feature == AVX2);
    EnsureSpace ensure_space(this);
...
void Assembler::vps(byte op, XMMRegister dst, XMMRegister src1, XMMRegister src2)
{
    DCHECK(IsEnabled(AVX));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(dst, src1, src2, kL128, kNoPrefix, k0F, kWIG);
...
void Assembler::vps(byte op, YMMRegister dst, YMMRegister src1, YMMRegister src2)
{
    DCHECK(IsEnabled(AVX));
    EnsureSpace ensure_space(this);
...
void Assembler::vps(byte op, XMMRegister dst, XMMRegister src1, XMMRegister src2, byte imm8)
{
    DCHECK(IsEnabled(AVX));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(dst, src1, src2, kL128, kNoPrefix, k0F, kWIG);
...
void Assembler::vps(byte op, YMMRegister dst, YMMRegister src1, YMMRegister src2, byte imm8)
{
    DCHECK(IsEnabled(AVX));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(dst, src1, src2, kL256, kNoPrefix, k0F, kWIG);
...
#define VPD(DSTRegister, SRCRegister, length)                                                                                                                                      \
    void Assembler::vpd(byte op, DSTRegister dst, SRCRegister src1, SRCRegister src2)                                                                                              \
    {                                                                                                                                                                              \
        DCHECK(IsEnabled(AVX));                                                                                                                                                    \
        EnsureSpace ensure_space(this);                                                                                                                                            \
        ...
        emit_sse_operand(dst, src2);                                                                                                                                               \
                                                                                                                                                                                   ...
                                                                                                                                                                                   \
    void Assembler::vpd(byte op, DSTRegister dst, SRCRegister src1, Operand src2)                                                                                                  \
    {                                                                                                                                                                              \
        DCHECK(IsEnabled(AVX));                                                                                                                                                    \
        EnsureSpace ensure_space(this);                                                                                                                                            \
        emit_vex_prefix(dst, src1, src2, k##length, k66, k0F, kWIG);                                                                                                               \
...
void Assembler::vss(byte op, XMMRegister dst, XMMRegister src1, XMMRegister src2)
{
    DCHECK(IsEnabled(AVX));
    EnsureSpace ensure_space(this);
...
void Assembler::bmi2q(SIMDPrefix pp, byte op, Register reg, Register vreg, Register rm)
{
...
void Assembler::bmi2q(SIMDPrefix pp, byte op, Register reg, Register vreg, Operand rm)
{
    DCHECK(IsEnabled(BMI2));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(reg, vreg, rm, kLZ, pp, k0F38, kW1);
...
void Assembler::bmi2l(SIMDPrefix pp, byte op, Register reg, Register vreg, Register rm)
{
    DCHECK(IsEnabled(BMI2));
    EnsureSpace ensure_space(this);
...
void Assembler::bmi2l(SIMDPrefix pp, byte op, Register reg, Register vreg, Operand rm)
{
    DCHECK(IsEnabled(BMI2));
    EnsureSpace ensure_space(this);
    emit_vex_prefix(reg, vreg, rm, kLZ, pp, k0F38, kW0);
...
void Assembler::sse_instr(XMMRegister dst, XMMRegister src, byte escape, byte opcode)
{
    EnsureSpace ensure_space(this);
    emit_optional_rex_32(dst, src);
    emit(escape);
...
void Assembler::sse_instr(XMMRegister dst, Operand src, byte escape, byte opcode)
{
    EnsureSpace ensure_space(this);
    emit_optional_rex_32(dst, src);
    emit(escape);
...
void Assembler::sse2_instr(XMMRegister dst, XMMRegister src, byte prefix, byte escape, byte opcode)
{
    EnsureSpace ensure_space(this);
    emit(prefix);
    emit_optional_rex_32(dst, src);
...
void Assembler::sse2_instr(XMMRegister dst, Operand src, byte prefix, byte escape, byte opcode)
{
    EnsureSpace ensure_space(this);
    emit(prefix);
    emit_optional_rex_32(dst, src);
...
void Assembler::ssse3_instr(XMMRegister dst, XMMRegister src, byte prefix, byte escape1, byte escape2, byte opcode)
{
...
void Assembler::ssse3_instr(XMMRegister dst, Operand src, byte prefix, byte escape1, byte escape2, byte opcode)
{
    DCHECK(IsEnabled(SSSE3));
    EnsureSpace ensure_space(this);
    emit(prefix);
...
void Assembler::sse4_instr(XMMRegister dst, Register src, byte prefix, byte escape1, byte escape2, byte opcode, int8_t imm8)
{
    DCHECK(is_uint8(imm8));
    DCHECK(IsEnabled(SSE4_1));
    EnsureSpace ensure_space(this);
...
void Assembler::sse4_instr(XMMRegister dst, XMMRegister src, byte prefix, byte escape1, byte escape2, byte opcode)
{
    DCHECK(IsEnabled(SSE4_1));
    EnsureSpace ensure_space(this);
    emit(prefix);
...
void Assembler::sse4_instr(XMMRegister dst, Operand src, byte prefix, byte escape1, byte escape2, byte opcode)
{
    DCHECK(IsEnabled(SSE4_1));
    EnsureSpace ensure_space(this);
    emit(prefix);
...
void Assembler::sse4_instr(Register dst, XMMRegister src, byte prefix, byte escape1, byte escape2, byte opcode, int8_t imm8)
{
    DCHECK(is_uint8(imm8));
    DCHECK(IsEnabled(SSE4_1));
    EnsureSpace ensure_space(this);
...
void Assembler::sse4_instr(Operand dst, XMMRegister src, byte prefix, byte escape1, byte escape2, byte opcode, int8_t imm8)
{
    DCHECK(is_uint8(imm8));
    DCHECK(IsEnabled(SSE4_1));
    EnsureSpace ensure_space(this);
...
void Assembler::sse4_2_instr(XMMRegister dst, XMMRegister src, byte prefix, byte escape1, byte escape2, byte opcode)
{
...
void Assembler::sse4_2_instr(XMMRegister dst, Operand src, byte prefix, byte escape1, byte escape2, byte opcode)
{
    DCHECK(IsEnabled(SSE4_2));
    EnsureSpace ensure_space(this);
    emit(prefix);
...
void Assembler::emit_sse_operand(Register reg, Operand adr) { emit_operand(reg, adr); }

void Assembler::emit_sse_operand(XMMRegister dst, XMMRegister src) { emit(0xC0 | (dst.low_bits() << 3) | src.low_bits()); }

void Assembler::emit_sse_operand(XMMRegister dst, Register src) { emit(0xC0 | (dst.low_bits() << 3) | src.low_bits()); }

void Assembler::emit_sse_operand(Register dst, XMMRegister src) { emit(0xC0 | (dst.low_bits() << 3) | src.low_bits()); }

void Assembler::emit_sse_operand(XMMRegister dst) { emit(0xD8 | dst.low_bits()); }
...
const int RelocInfo::kApplyMask = RelocInfo::ModeMask(RelocInfo::CODE_TARGET) | RelocInfo::ModeMask(RelocInfo::NEAR_BUILTIN_ENTRY)
    | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE) | RelocInfo::ModeMask(RelocInfo::WASM_CALL);
