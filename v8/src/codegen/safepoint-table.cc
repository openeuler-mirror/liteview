...
SafepointTable::SafepointTable(Isolate* isolate, Address pc, Code code)
    : SafepointTable(code.InstructionStart(isolate, pc), code.SafepointTableAddress())
{
}

#ifdef V8_EXTERNAL_CODE_SPACE
SafepointTable::SafepointTable(Isolate* isolate, Address pc, CodeDataContainer code)
    : SafepointTable(code.InstructionStart(isolate, pc), code.SafepointTableAddress())
{
}
...
SafepointTableBuilder::Safepoint SafepointTableBuilder::DefineSafepoint(Assembler* assembler)
{
...
void SafepointTableBuilder::Emit(Assembler* assembler, int tagged_slots_size)
{
    ...
    uint32_t entry_configuration = SafepointTable::HasDeoptDataField::encode(has_deopt_data) | SafepointTable::RegisterIndexesSizeField::encode(register_indexes_size)
        | SafepointTable::PcSizeField::encode(pc_size) | SafepointTable::DeoptIndexSizeField::encode(deopt_index_size)
...
void SafepointTableBuilder::RemoveDuplicates()
{
    ...
    for (auto it = entries_.begin(), end = entries_.end(); it != end; ++remaining_it, ++remaining) {
        if (remaining_it != it)
            *remaining_it = *it;
