...
public:
    ...
    template <typename Op> void Pshufb(XMMRegister dst, Op mask) { Pshufb(dst, dst, mask); }
...
#define AVX_OP_SSE3(macro_name, name)                                                                                                                                              \
    template <typename Dst, typename Arg, typename... Args> void macro_name(Dst dst, Arg arg, Args... args)                                                                        \
    {                                                                                                                                                                              \
        AvxHelper<Dst, Arg, Args...> { this, base::Optional<CpuFeature>(SSE3) }.template emit<&Assembler::v##name, &Assembler::name>(dst, arg, args...);                           \
    }

#define AVX_OP_SSSE3(macro_name, name)                                                                                                                                             \
    template <typename Dst, typename Arg, typename... Args> void macro_name(Dst dst, Arg arg, Args... args)                                                                        \
    {                                                                                                                                                                              \
        AvxHelper<Dst, Arg, Args...> { this, base::Optional<CpuFeature>(SSSE3) }.template emit<&Assembler::v##name, &Assembler::name>(dst, arg, args...);                          \
    }

#define AVX_OP_SSE4_1(macro_name, name)                                                                                                                                            \
    template <typename Dst, typename Arg, typename... Args> void macro_name(Dst dst, Arg arg, Args... args)                                                                        \
    {                                                                                                                                                                              \
        AvxHelper<Dst, Arg, Args...> { this, base::Optional<CpuFeature>(SSE4_1) }.template emit<&Assembler::v##name, &Assembler::name>(dst, arg, args...);                         \
    }

#define AVX_OP_SSE4_2(macro_name, name)                                                                                                                                            \
    template <typename Dst, typename Arg, typename... Args> void macro_name(Dst dst, Arg arg, Args... args)                                                                        \
    {                                                                                                                                                                              \
        AvxHelper<Dst, Arg, Args...> { this, base::Optional<CpuFeature>(SSE4_2) }.template emit<&Assembler::v##name, &Assembler::name>(dst, arg, args...);                         \
    }
...
protected:
    ...
    void PinsrHelper(Assembler* assm, AvxFn<Op> avx, NoAvxFn<Op> noavx, XMMRegister dst, XMMRegister src1, Op src2, uint8_t imm8, uint32_t* load_pc_offset = nullptr,
        base::Optional<CpuFeature> feature = base::nullopt)
    {
        if (CpuFeatures::IsSupported(AVX)) {
            CpuFeatureScope scope(assm, AVX);
...
template <typename Impl> class V8_EXPORT_PRIVATE SharedTurboAssemblerBase : public SharedTurboAssembler {
    using SharedTurboAssembler::SharedTurboAssembler;

public:
    void Abspd(XMMRegister dst, XMMRegister src, Register tmp) { FloatUnop(dst, src, tmp, &SharedTurboAssembler::Andps, ExternalReference::address_of_double_abs_constant()); }

    void Absps(XMMRegister dst, XMMRegister src, Register tmp) { FloatUnop(dst, src, tmp, &SharedTurboAssembler::Andps, ExternalReference::address_of_float_abs_constant()); }

    void Negpd(XMMRegister dst, XMMRegister src, Register tmp) { FloatUnop(dst, src, tmp, &SharedTurboAssembler::Xorps, ExternalReference::address_of_double_neg_constant()); }

    void Negps(XMMRegister dst, XMMRegister src, Register tmp) { FloatUnop(dst, src, tmp, &SharedTurboAssembler::Xorps, ExternalReference::address_of_float_neg_constant()); }
#undef FLOAT_UNOP
    ...
    template <typename Op> void Pinsrd(XMMRegister dst, XMMRegister src1, Op src2, uint8_t imm8, uint32_t* load_pc_offset = nullptr)
    {
        if (CpuFeatures::IsSupported(SSE4_1)) {
            PinsrHelper(this, &Assembler::vpinsrd, &Assembler::pinsrd, dst, src1, src2, imm8, load_pc_offset, base::Optional<CpuFeature>(SSE4_1));
        } else {
            if (dst != src1) {
                movaps(dst, src1);
            }
    ...
    template <typename Op> void Pinsrd(XMMRegister dst, Op src, uint8_t imm8, uint32_t* load_pc_offset = nullptr) { Pinsrd(dst, dst, src, imm8, load_pc_offset); }
    ...
    void I32x4TruncF64x2UZero(XMMRegister dst, XMMRegister src, Register tmp, XMMRegister scratch)
    {
        ...
        // Add to special double where significant bits == uint32.
        Addpd(dst, dst, ExternalReferenceAsOperand(ExternalReference::address_of_wasm_double_2_power_52(), tmp));
        // Extract low 32 bits of each double's significand, zero top lanes.
        // dst = [dst[0], dst[2], 0, 0]
        Shufps(dst, dst, scratch, 0x88);
    }

    void I32x4TruncF32x4U(XMMRegister dst, XMMRegister src, Register scratch, XMMRegister tmp)
    {
        ASM_CODE_COMMENT(this);
        Operand int32_overflow_op = ExternalReferenceAsOperand(ExternalReference::address_of_wasm_int32_overflow_as_float(), scratch);
        if (CpuFeatures::IsSupported(AVX)) {
            CpuFeatureScope avx_scope(this, AVX);
...
private:
    ...
    Operand ExternalReferenceAsOperand(ExternalReference reference, Register scratch) { return impl()->ExternalReferenceAsOperand(reference, scratch); }

    using FloatInstruction = void (SharedTurboAssembler::*)(XMMRegister, XMMRegister, Operand);
