...
public:
    ...
    void CompareF32(FPURegister cmp1, FPURegister cmp2, FPUCondition cc, CFRegister cd = FCC0) { CompareF(cmp1, cmp2, cc, cd, true); }

    void CompareIsNanF32(FPURegister cmp1, FPURegister cmp2, CFRegister cd = FCC0) { CompareIsNanF(cmp1, cmp2, cd, true); }

    void CompareF64(FPURegister cmp1, FPURegister cmp2, FPUCondition cc, CFRegister cd = FCC0) { CompareF(cmp1, cmp2, cc, cd, false); }

    void CompareIsNanF64(FPURegister cmp1, FPURegister cmp2, CFRegister cd = FCC0) { CompareIsNanF(cmp1, cmp2, cd, false); }
    ...
    inline void li(Register rd, int64_t j, LiFlags mode = OPTIMIZE_SIZE) { li(rd, Operand(j), mode); }
    inline void li(Register rd, int32_t j, LiFlags mode = OPTIMIZE_SIZE) { li(rd, Operand(static_cast<int64_t>(j)), mode); }
    ...
    inline void Move(Register output, MemOperand operand) { Ld_d(output, operand); }
...
#define DEFINE_INSTRUCTION(instr)                                                                                                                                                  \
    ...
    void instr(Register rd, Register rj, Register rk) { instr(rd, rj, Operand(rk)); }                                                                                              \
...
#undef COND_ARGS
    ...
    void LoadZeroOnCondition(Register rd, Register rj, const Operand& rk, Condition cond);
    ...
    inline void FmoveLow(Register dst_low, FPURegister src) { movfr2gr_s(dst_low, src); }
    ...
    void Move(FPURegister dst, float imm) { Move(dst, base::bit_cast<uint32_t>(imm)); }
    void Move(FPURegister dst, double imm) { Move(dst, base::bit_cast<uint64_t>(imm)); }
...
public:
    ...
    void LoadReceiver(Register dest, Register argc) { Ld_d(dest, MemOperand(sp, 0)); }

    void StoreReceiver(Register rec, Register argc, Register scratch) { St_d(rec, MemOperand(sp, 0)); }
    ...
    void TestCodeTIsMarkedForDeoptimizationAndJump(Register codet, Register scratch, Condition cond, Label* target);
    ...
    template <typename Field> void DecodeField(Register dst, Register src) { Bstrpick_d(dst, src, Field::kShift + Field::kSize - 1, Field::kShift); }
