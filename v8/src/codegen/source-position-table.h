...
public:
    ...
    explicit SourcePositionTableIterator(
        Handle<ByteArray> byte_array, IterationFilter iteration_filter = kJavaScriptOnly, FunctionEntryFilter function_entry_filter = kSkipFunctionEntry);
    ...
    // scope around.
    explicit SourcePositionTableIterator(ByteArray byte_array, IterationFilter iteration_filter = kJavaScriptOnly, FunctionEntryFilter function_entry_filter = kSkipFunctionEntry);
    ...
    explicit SourcePositionTableIterator(
        base::Vector<const byte> bytes, IterationFilter iteration_filter = kJavaScriptOnly, FunctionEntryFilter function_entry_filter = kSkipFunctionEntry);
    ...
    IndexAndPositionState GetState() const { return { index_, current_, iteration_filter_, function_entry_filter_ }; }
