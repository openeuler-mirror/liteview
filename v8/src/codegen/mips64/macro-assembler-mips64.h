...
#define DECLARE_NORELOC_PROTOTYPE(Name, target_type)                                                                                                                               \
    ...
    inline void Name(BranchDelaySlot bd, target_type target) { Name(target, bd); }                                                                                                 \
    void Name(target_type target, COND_TYPED_ARGS, BranchDelaySlot bd = PROTECT);                                                                                                  \
    inline void Name(BranchDelaySlot bd, target_type target, COND_TYPED_ARGS) { Name(target, COND_ARGS, bd); }
...
#undef COND_ARGS
    ...
    void CompareF32(FPUCondition cc, FPURegister cmp1, FPURegister cmp2) { CompareF(S, cc, cmp1, cmp2); }

    void CompareIsNanF32(FPURegister cmp1, FPURegister cmp2) { CompareIsNanF(S, cmp1, cmp2); }

    void CompareF64(FPUCondition cc, FPURegister cmp1, FPURegister cmp2) { CompareF(D, cc, cmp1, cmp2); }

    void CompareIsNanF64(FPURegister cmp1, FPURegister cmp2) { CompareIsNanF(D, cmp1, cmp2); }
...
#define COND_ARGS Condition cond = al, Register rs = zero_reg, const Operand &rt = Operand(zero_reg), BranchDelaySlot bd = PROTECT
    ...
    void Ret(COND_ARGS);
    inline void Ret(BranchDelaySlot bd, Condition cond = al, Register rs = zero_reg, const Operand& rt = Operand(zero_reg)) { Ret(cond, rs, rt, bd); }
...
#undef COND_ARGS
    ...
    inline void FmoveHigh(Register dst_high, FPURegister src) { mfhc1(dst_high, src); }

    inline void FmoveHigh(FPURegister dst, Register src_high) { mthc1(src_high, dst); }

    inline void FmoveLow(Register dst_low, FPURegister src) { mfc1(dst_low, src); }
...
public:
    ...
    // arguments order.
    void LoadReceiver(Register dest, Register argc) { Ld(dest, MemOperand(sp, 0)); }

    void StoreReceiver(Register rec, Register argc, Register scratch) { Sd(rec, MemOperand(sp, 0)); }
    ...
    template <typename Field> void DecodeField(Register dst, Register src) { Ext(dst, src, Field::kShift, Field::kSize); }
