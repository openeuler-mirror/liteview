...
public:
    ...
    // Iteration support.
    base::iterator_range<const T*> parameters() const { return { reps_ + return_count_, reps_ + return_count_ + parameter_count_ }; }
    base::iterator_range<const T*> returns() const { return { reps_, reps_ + return_count_ }; }
    base::iterator_range<const T*> all() const { return { reps_, reps_ + return_count_ + parameter_count_ }; }
    ...
    public:
        Builder(Zone* zone, size_t return_count, size_t parameter_count)
            ...
            , rcursor_(0)
            , pcursor_(0)
            , buffer_(zone->NewArray<T>(static_cast<int>(return_count + parameter_count)))
        {
        }
...
template <typename T> size_t hash_value(const Signature<T>& sig)
{
    ...
    for (T rep : sig.all())
        seed = base::hash_combine(seed, base::hash<T> {}(rep));
    return seed;
}
