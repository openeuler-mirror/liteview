...
template <typename T> void EncodeInt(ZoneVector<byte>* bytes, T value)
{
...
template <typename T> T DecodeInt(base::Vector<const byte> bytes, int* index)
{
    byte current;
    int shift = 0;
    T decoded = 0;
...
void DecodeEntry(base::Vector<const byte> bytes, int* index, PositionTableEntry* entry)
{
...
base::Vector<const byte> VectorFromByteArray(ByteArray byte_array) { return base::Vector<const byte>(byte_array.GetDataStartAddress(), byte_array.length()); }
...
template <typename IsolateT> Handle<ByteArray> SourcePositionTableBuilder::ToSourcePositionTable(IsolateT* isolate)
{
    if (bytes_.empty())
...
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Handle<ByteArray> SourcePositionTableBuilder::ToSourcePositionTable(Isolate* isolate);
template EXPORT_TEMPLATE_DEFINE(V8_EXPORT_PRIVATE) Handle<ByteArray> SourcePositionTableBuilder::ToSourcePositionTable(LocalIsolate* isolate);

base::OwnedVector<byte> SourcePositionTableBuilder::ToSourcePositionTableVector()
{
    if (bytes_.empty())
        return base::OwnedVector<byte>();
    DCHECK(!Omit());
...
SourcePositionTableIterator::SourcePositionTableIterator(ByteArray byte_array, IterationFilter iteration_filter, FunctionEntryFilter function_entry_filter)
...
SourcePositionTableIterator::SourcePositionTableIterator(Handle<ByteArray> byte_array, IterationFilter iteration_filter, FunctionEntryFilter function_entry_filter)
    : table_(byte_array)
    , iteration_filter_(iteration_filter)
    , function_entry_filter_(function_entry_filter)
{
...
SourcePositionTableIterator::SourcePositionTableIterator(base::Vector<const byte> bytes, IterationFilter iteration_filter, FunctionEntryFilter function_entry_filter)
    : raw_table_(bytes)
    , iteration_filter_(iteration_filter)
    , function_entry_filter_(function_entry_filter)
{
...
{
    base::Vector<const byte> bytes = table_.is_null() ? raw_table_ : VectorFromByteArray(*table_);
    DCHECK(!done());
    ...
    while (!done() && !filter_satisfied) {
        if (index_ >= bytes.length()) {
            ...
            filter_satisfied = (iteration_filter_ == kAll) || (iteration_filter_ == kJavaScriptOnly && p.IsJavaScript()) || (iteration_filter_ == kExternalOnly && p.IsExternal());
        }
    }
}
