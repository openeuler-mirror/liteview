...
CPURegList TurboAssembler::DefaultFPTmpList() { return CPURegList(fp_scratch1, fp_scratch2); }
...
void TurboAssembler::LogicalMacro(const Register& rd, const Register& rn, const Operand& operand, LogicalOp op)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::Mov(const Register& rd, const Operand& operand, DiscardMoveMode discard_mode)
{
    DCHECK(allow_macro_instructions());
    DCHECK(!rd.IsZero());
...
void TurboAssembler::Mov(const Register& rd, Smi smi) { return Mov(rd, Operand(smi)); }
...
void TurboAssembler::Movi(const VRegister& vd, uint64_t imm, Shift shift, int shift_amount)
{
    DCHECK(allow_macro_instructions());
...
bool TurboAssembler::IsImmMovn(uint64_t imm, unsigned reg_size) { return IsImmMovz(~imm, reg_size); }

void TurboAssembler::ConditionalCompareMacro(const Register& rn, const Operand& operand, StatusFlags nzcv, Condition cond, ConditionalCompareOp op)
{
...
void TurboAssembler::Csel(const Register& rd, const Register& rn, const Operand& operand, Condition cond)
{
    DCHECK(allow_macro_instructions());
    DCHECK(!rd.IsZero());
    DCHECK((cond != al) && (cond != nv));
...
bool TurboAssembler::TryOneInstrMoveImmediate(const Register& dst, int64_t imm)
{
    unsigned n, imm_s, imm_r;
...
Operand TurboAssembler::MoveImmediateForShiftedOp(const Register& dst, int64_t imm, PreShiftImmMode mode)
{
    int reg_size = dst.SizeInBits();
...
void TurboAssembler::AddSubMacro(const Register& rd, const Register& rn, const Operand& operand, FlagsUpdate S, AddSubOp op)
{
...
void TurboAssembler::AddSubWithCarryMacro(const Register& rd, const Register& rn, const Operand& operand, FlagsUpdate S, AddSubWithCarryOp op)
{
...
void TurboAssembler::LoadStoreMacro(const CPURegister& rt, const MemOperand& addr, LoadStoreOp op)
{
...
void TurboAssembler::LoadStorePairMacro(const CPURegister& rt, const CPURegister& rt2, const MemOperand& addr, LoadStorePairOp op)
{
...
bool TurboAssembler::NeedExtraInstructionsOrRegisterBranch(Label* label, ImmBranchType b_type)
{
    ...
    if (!need_longer_range && !label->is_bound()) {
        ...
        unresolved_branches_.insert(std::pair<int, FarBranchInfo>(max_reachable_pc, FarBranchInfo(pc_offset(), label)));
...
void TurboAssembler::Abs(const Register& rd, const Register& rm, Label* is_not_representable, Label* is_representable)
{
    DCHECK(allow_macro_instructions());
...
void TurboAssembler::Push(const CPURegister& src0, const CPURegister& src1, const CPURegister& src2, const CPURegister& src3, const CPURegister& src4, const CPURegister& src5,
...
void TurboAssembler::Pop(const CPURegister& dst0, const CPURegister& dst1, const CPURegister& dst2, const CPURegister& dst3, const CPURegister& dst4, const CPURegister& dst5,
...
void TurboAssembler::PushHelper(int count, int size, const CPURegister& src0, const CPURegister& src1, const CPURegister& src2, const CPURegister& src3)
{
...
void TurboAssembler::PopHelper(int count, int size, const CPURegister& dst0, const CPURegister& dst1, const CPURegister& dst2, const CPURegister& dst3)
{
    // Ensure that we don't unintentially modify scratch or debug registers.
    InstructionAccurateScope scope(this);
...
void TurboAssembler::PokePair(const CPURegister& src1, const CPURegister& src2, int offset)
{
...
void TailCallOptimizedCodeSlot(MacroAssembler* masm, Register optimized_code_entry, Register scratch)
{
    ...
    __ JumpIfCodeTIsMarkedForDeoptimization(optimized_code_entry, scratch, &heal_optimized_code_slot);
...
void MacroAssembler::GenerateTailCallToReturnedCode(Runtime::FunctionId function_id)
{
    ...
    // -----------------------------------
    {
        ...
        Push(kJavaScriptCallTargetRegister, kJavaScriptCallNewTargetRegister, kJavaScriptCallArgCountRegister, padreg);
        ...
        Pop(padreg, kJavaScriptCallArgCountRegister, kJavaScriptCallNewTargetRegister, kJavaScriptCallTargetRegister);
...
void TurboAssembler::CopySlots(Register dst, Register src, Register slot_count)
{
...
void TurboAssembler::CopyDoubleWords(Register dst, Register src, Register count, CopyDoubleWordsMode mode)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::SlotAddress(Register dst, int slot_offset) { Add(dst, sp, slot_offset << kSystemPointerSizeLog2); }

void TurboAssembler::SlotAddress(Register dst, Register slot_offset) { Add(dst, sp, Operand(slot_offset, LSL, kSystemPointerSizeLog2)); }
...
void MacroAssembler::CallRuntime(const Runtime::Function* f, int num_arguments, SaveFPRegsMode save_doubles)
{
    ...
    Handle<CodeT> code = CodeFactory::CEntry(isolate(), f->result_size, save_doubles);
...
void MacroAssembler::JumpToExternalReference(const ExternalReference& builtin, bool builtin_exit_frame)
{
    ASM_CODE_COMMENT(this);
    Mov(x1, builtin);
    Handle<CodeT> code = CodeFactory::CEntry(isolate(), 1, SaveFPRegsMode::kIgnore, ArgvMode::kStack, builtin_exit_frame);
...
void TurboAssembler::CallCFunction(ExternalReference function, int num_of_reg_args) { CallCFunction(function, num_of_reg_args, 0); }

void TurboAssembler::CallCFunction(ExternalReference function, int num_of_reg_args, int num_of_double_args)
{
    ASM_CODE_COMMENT(this);
    UseScratchRegisterScope temps(this);
    Register temp = temps.AcquireX();
...
void TurboAssembler::CallCFunction(Register function, int num_of_reg_args, int num_of_double_args)
{
    ASM_CODE_COMMENT(this);
    ...
    if (root_array_available()) {
        Str(pc_scratch, MemOperand(kRootRegister, IsolateData::fast_c_call_caller_pc_offset()));
        Str(fp, MemOperand(kRootRegister, IsolateData::fast_c_call_caller_fp_offset()));
    } else {
        DCHECK_NOT_NULL(isolate());
        Mov(addr_scratch, ExternalReference::fast_c_call_caller_pc_address(isolate()));
        Str(pc_scratch, MemOperand(addr_scratch));
        Mov(addr_scratch, ExternalReference::fast_c_call_caller_fp_address(isolate()));
    ...
    if (root_array_available()) {
        Str(xzr, MemOperand(kRootRegister, IsolateData::fast_c_call_caller_fp_offset()));
    } else {
        DCHECK_NOT_NULL(isolate());
...
void TurboAssembler::JumpHelper(int64_t offset, RelocInfo::Mode rmode, Condition cond)
{
    if (cond == nv)
        return;
    Label done;
...
//   other type of call.
int64_t TurboAssembler::CalculateTargetOffset(Address target, RelocInfo::Mode rmode, byte* pc)
{
...
void TurboAssembler::Jump(Handle<CodeT> code, RelocInfo::Mode rmode, Condition cond)
{
...
void TurboAssembler::LoadEntryFromBuiltinIndex(Register builtin_index)
{
    ...
    if (SmiValuesAre32Bits()) {
        ...
        if (COMPRESS_POINTERS_BOOL) {
            Add(builtin_index, kRootRegister, Operand(builtin_index.W(), SXTW, kSystemPointerSizeLog2 - kSmiShift));
        } else {
            Add(builtin_index, kRootRegister, Operand(builtin_index, LSL, kSystemPointerSizeLog2 - kSmiShift));
        }
        Ldr(builtin_index, MemOperand(builtin_index, IsolateData::builtin_entry_table_offset()));
    }
}

void TurboAssembler::LoadEntryFromBuiltin(Builtin builtin, Register destination) { Ldr(destination, EntryFromBuiltinAsOperand(builtin)); }
...
void TurboAssembler::LoadCodeObjectEntry(Register destination, Register code_object)
{
    ...
    if (options().isolate_independent_code) {
        ...
        TestAndBranchIfAnySet(scratch.W(), Code::IsOffHeapTrampoline::kMask, &if_code_is_off_heap);
        ...
        Add(destination, kRootRegister, Operand(scratch, LSL, kSystemPointerSizeLog2));
...
void TurboAssembler::LoadCodeDataContainerEntry(Register destination, Register code_data_container_object)
{
    ...
    Ldr(destination, FieldMemOperand(code_data_container_object, CodeDataContainer::kCodeEntryPointOffset));
}

void TurboAssembler::LoadCodeDataContainerCodeNonBuiltin(Register destination, Register code_data_container_object)
{
    ASM_CODE_COMMENT(this);
    CHECK(V8_EXTERNAL_CODE_SPACE_BOOL);
    ...
    Ldr(destination, FieldMemOperand(code_data_container_object, CodeDataContainer::kCodeOffset));
}
...
bool TurboAssembler::IsNearCallOffset(int64_t offset) { return is_int26(offset); }
...
void MacroAssembler::InvokePrologue(Register formal_parameter_count, Register actual_argument_count, Label* done, InvokeType type)
{
    ASM_CODE_COMMENT(this);
...
{
    if (V8_EXTERNAL_CODE_SPACE_BOOL) {
        Ldr(scratch.W(), FieldMemOperand(codet, CodeDataContainer::kKindSpecificFlagsOffset));
        ...
        Ldr(scratch.W(), FieldMemOperand(scratch, CodeDataContainer::kKindSpecificFlagsOffset));
        Tbnz(scratch.W(), Code::kMarkedForDeoptimizationBit, if_marked_for_deoptimization);
    }
}
...
Operand MacroAssembler::ReceiverOperand(Register arg_count) { return Operand(0); }
...
void TurboAssembler::TryConvertDoubleToInt64(Register result, DoubleRegister double_input, Label* done)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::EnterFrame(StackFrame::Type type)
{
    ...
    if (StackFrame::IsJavaScript(type)) {
        ...
        if (type == StackFrame::CONSTRUCT) {
            fourth_reg = cp;
#if V8_ENABLE_WEBASSEMBLY
        } else if (type == StackFrame::WASM || type == StackFrame::WASM_COMPILE_LAZY || type == StackFrame::WASM_EXIT) {
...
void MacroAssembler::EnterExitFrame(bool save_doubles, const Register& scratch, int extra_space, StackFrame::Type frame_type)
{
    ASM_CODE_COMMENT(this);
...
void MacroAssembler::LeaveExitFrame(bool restore_doubles, const Register& scratch, const Register& scratch2)
{
    ASM_CODE_COMMENT(this);
    if (restore_doubles) {
...
void TurboAssembler::AtomicDecompressTaggedPointer(const Register& destination, const Register& base, const Register& index, const Register& temp)
{
    ASM_CODE_COMMENT(this);
    Add(temp, base, index);
    Ldar(destination.W(), temp);
...
void TurboAssembler::AtomicDecompressAnyTagged(const Register& destination, const Register& base, const Register& index, const Register& temp)
{
    ASM_CODE_COMMENT(this);
    Add(temp, base, index);
    Ldar(destination.W(), temp);
...
void TurboAssembler::LoadExternalPointerField(Register destination, MemOperand field_operand, ExternalPointerTag tag, Register isolate_root)
{
    ...
    if (IsSandboxedExternalPointerType(tag)) {
        ...
        Ldr(external_table, MemOperand(isolate_root, IsolateData::external_pointer_table_offset() + Internals::kExternalPointerTableBufferOffset));
        Ldr(destination.W(), field_operand);
...
void MacroAssembler::RecordWrite(Register object, Operand offset, Register value, LinkRegisterStatus lr_status, SaveFPRegsMode fp_mode, SmiCheck smi_check)
{
    ...
    CheckPageFlag(value, MemoryChunk::kPointersToHereAreInterestingOrInSharedHeapMask, ne, &done);

    CheckPageFlag(object, MemoryChunk::kPointersFromHereAreInterestingMask, ne, &done);
...
void TurboAssembler::PrintfNoPreserve(const char* format, const CPURegister& arg0, const CPURegister& arg1, const CPURegister& arg2, const CPURegister& arg3)
{
    ASM_CODE_COMMENT(this);
...
void TurboAssembler::Printf(const char* format, CPURegister arg0, CPURegister arg1, CPURegister arg2, CPURegister arg3)
{
    ASM_CODE_COMMENT(this);
...
{
    static_assert(StandardFrameConstants::kCallerFPOffset + kSystemPointerSize == StandardFrameConstants::kCallerPCOffset, "Offsets must be consecutive for ldp!");
#ifdef V8_ENABLE_CONTROL_FLOW_INTEGRITY
