...
public:
    ...
    inline static void deserialization_set_special_target_at(Address location, Code code, Address target);
    ...
    // current position.
    uint64_t InstructionsGeneratedSince(const Label* label) { return SizeOfCodeGeneratedSince(label) / kInstrSize; }
    ...
    bool is_veneer_pool_blocked() const { return veneer_pool_blocked_nesting_ > 0; }
    ...
    Instruction* InstructionAt(ptrdiff_t offset) const { return reinterpret_cast<Instruction*>(buffer_start_ + offset); }

    ptrdiff_t InstructionOffset(Instruction* instr) const { return reinterpret_cast<byte*>(instr) - buffer_start_; }
    ...
    bool ShouldEmitVeneer(int max_reachable_pc, size_t margin) { return max_reachable_pc < MaxPCOffsetAfterVeneerPoolIfEmittedNow(margin); }
    bool ShouldEmitVeneers(size_t margin = kVeneerDistanceMargin) { return ShouldEmitVeneer(unresolved_branches_first_limit(), margin); }
...
#if defined(V8_OS_WIN)
    win64_unwindinfo::XdataEncoder* GetXdataEncoder() { return xdata_encoder_.get(); }
...
public:
    ...
    static size_t GetCheckConstPoolIntervalForTesting() { return ConstantPool::kCheckInterval; }

    static size_t GetApproxMaxDistToConstPoolForTesting() { return ConstantPool::kApproxDistToPool64; }
#endif
...
public:
    ...
    PatchingAssembler(const AssemblerOptions& options, byte* start, unsigned count)
        : Assembler(options, ExternalAssemblerBuffer(start, count * kInstrSize + kGap))
