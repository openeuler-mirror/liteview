...
const char* const ExternalReferenceTable::ref_name_[ExternalReferenceTable::kSize] = {
    ...
    // Accessors:
    ACCESSOR_INFO_LIST_GENERATOR(ADD_ACCESSOR_INFO_NAME, /* not used */) ACCESSOR_GETTER_LIST(ADD_ACCESSOR_GETTER_NAME) ACCESSOR_SETTER_LIST(ADD_ACCESSOR_SETTER_NAME)
    ...
    "Load StubCache::primary_->value", "Load StubCache::primary_->map", "Load StubCache::secondary_->key", "Load StubCache::secondary_->value", "Load StubCache::secondary_->map",
    "Store StubCache::primary_->key", "Store StubCache::primary_->value", "Store StubCache::primary_->map", "Store StubCache::secondary_->key",
    "Store StubCache::secondary_->value", "Store StubCache::secondary_->map",
...
namespace {
static Address ref_addr_isolate_independent_[ExternalReferenceTable::kSizeIsolateIndependent] = { 0 };
} // namespace
...
const char* ExternalReferenceTable::NameOfIsolateIndependentAddress(Address address)
{
...
void ExternalReferenceTable::AddIsolateIndependent(Address address, int* index) { ref_addr_isolate_independent_[(*index)++] = address; }

void ExternalReferenceTable::AddIsolateIndependentReferences(int* index)
{
    CHECK_EQ(kSpecialReferenceCount, *index);

#define ADD_EXTERNAL_REFERENCE(name, desc) AddIsolateIndependent(ExternalReference::name().address(), index);
...
void ExternalReferenceTable::CopyIsolateIndependentReferences(int* index)
{
    ...
    std::copy(ref_addr_isolate_independent_, ref_addr_isolate_independent_ + kSizeIsolateIndependent, ref_addr_);
...
#undef ACCESSOR_SETTER_DECLARATION
    ...
    CHECK_EQ(kSpecialReferenceCount + kExternalReferenceCountIsolateIndependent + kBuiltinsReferenceCount + kRuntimeReferenceCount + kAccessorReferenceCount, *index);
}
...
void ExternalReferenceTable::AddStubCache(Isolate* isolate, int* index)
{
    ...
    Add(store_stub_cache->value_reference(StubCache::kSecondary).address(), index);
...
#undef SC
    ...
    CHECK_EQ(kSizeIsolateIndependent + kExternalReferenceCountIsolateDependent + kIsolateAddressReferenceCount + kStubCacheReferenceCount + kStatsCountersReferenceCount, *index);
    CHECK_EQ(kSize, *index);
}
