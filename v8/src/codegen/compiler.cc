...
void SetTieringState(JSFunction function, BytecodeOffset osr_offset, TieringState value)
{
    if (IsOSR(osr_offset)) {
...
public:
    static void TraceStartBaselineCompile(Isolate* isolate, Handle<SharedFunctionInfo> shared)
    {
    ...
    static void TraceStartMaglevCompile(Isolate* isolate, Handle<JSFunction> function, ConcurrencyMode mode)
    {
    ...
    static void TraceOptimizeOSRStarted(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, ConcurrencyMode mode)
    {
    ...
    static void TraceOptimizeOSRFinished(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset)
    {
        if (!v8_flags.trace_osr)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceOptimizeOSRAvailable(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, ConcurrencyMode mode)
    {
        if (!v8_flags.trace_osr)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceOptimizeOSRUnavailable(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, ConcurrencyMode mode)
    {
        if (!v8_flags.trace_osr)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceFinishBaselineCompile(Isolate* isolate, Handle<SharedFunctionInfo> shared, double ms_timetaken)
    {
        if (!v8_flags.trace_baseline)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceFinishMaglevCompile(Isolate* isolate, Handle<JSFunction> function, double ms_prepare, double ms_optimize, double ms_codegen)
    {
        if (!v8_flags.trace_opt)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
        PrintTracePrefix(scope, "completed compiling", function, CodeKind::MAGLEV);
        PrintF(scope.file(), " - took %0.3f, %0.3f, %0.3f ms", ms_prepare, ms_optimize, ms_codegen);
        PrintTraceSuffix(scope);
    }

    static void TraceAbortedMaglevCompile(Isolate* isolate, Handle<JSFunction> function, BailoutReason bailout_reason)
    {
        if (!v8_flags.trace_opt)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceOptimizedCodeCacheHit(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, CodeKind code_kind)
    {
        if (!v8_flags.trace_opt)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceOptimizeForAlwaysOpt(Isolate* isolate, Handle<JSFunction> function, CodeKind code_kind)
    {
        if (!v8_flags.trace_opt)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
    ...
    static void TraceMarkForAlwaysOpt(Isolate* isolate, Handle<JSFunction> function)
    {
        if (!v8_flags.trace_opt)
            return;
        CodeTracer::Scope scope(isolate->GetCodeTracer());
...
private:
    ...
    static void PrintTracePrefix(const CodeTracer::Scope& scope, const char* header, Handle<JSFunction> function, CodeKind code_kind)
    {
    ...
    static void PrintTracePrefix(const CodeTracer::Scope& scope, const char* header, Handle<SharedFunctionInfo> shared, CodeKind code_kind)
    {
        PrintF(scope.file(), "[%s ", header);
    ...
    static void PrintTraceSuffix(const CodeTracer::Scope& scope) { PrintF(scope.file(), "]\n"); }
};

} // namespace

// static
void Compiler::LogFunctionCompilation(Isolate* isolate, LogEventListener::CodeTag code_type, Handle<Script> script, Handle<SharedFunctionInfo> shared,
    Handle<FeedbackVector> vector, Handle<AbstractCode> abstract_code, CodeKind kind, double time_taken_ms)
{
    DCHECK_NE(*abstract_code, ToAbstractCode(*BUILTIN_CODE(isolate, CompileLazy)));

    // Log the code generation. If source information is available include
    // script name and line number. Check explicitly whether logging is
    // enabled as finding the line number is not free.
    if (!isolate->v8_file_logger()->is_listening_to_code_events() && !isolate->is_profiling() && !v8_flags.log_function_events
    ...
    Handle<String> script_name(script->name().IsString() ? String::cast(script->name()) : ReadOnlyRoots(isolate).empty_string(), isolate);
...
ScriptOriginOptions OriginOptionsForEval(Object script, ParsingWhileDebugging parsing_while_debugging)
{
...
namespace {
void LogUnoptimizedCompilation(
    Isolate* isolate, Handle<SharedFunctionInfo> shared, LogEventListener::CodeTag code_type, base::TimeDelta time_taken_to_execute, base::TimeDelta time_taken_to_finalize)
{
    Handle<AbstractCode> abstract_code;
    if (shared->HasBytecodeArray()) {
        abstract_code = handle(AbstractCode::cast(shared->GetBytecodeArray(isolate)), isolate);
    } else {
#if V8_ENABLE_WEBASSEMBLY
        DCHECK(shared->HasAsmWasmData());
        abstract_code = ToAbstractCode(BUILTIN_CODE(isolate, InstantiateAsmJs), isolate);
#else
        UNREACHABLE();
    ...
    Compiler::LogFunctionCompilation(isolate, code_type, script, shared, Handle<FeedbackVector>(), abstract_code, CodeKind::INTERPRETED_FUNCTION, time_taken_ms);
}
...
void TurbofanCompilationJob::RecordCompilationStats(ConcurrencyMode mode, Isolate* isolate) const
{
    ...
    if (base::TimeTicks::IsHighResolution()) {
        ...
        if (compilation_info()->is_osr()) {
            ...
            counters->turbofan_osr_total_time()->AddSample(static_cast<int>(ElapsedTime().InMicroseconds()));
        } else {
            ...
            counters->turbofan_optimize_total_time()->AddSample(static_cast<int>(ElapsedTime().InMicroseconds()));
            ...
            case ConcurrencyMode::kConcurrent:
                ...
                counters->turbofan_optimize_concurrent_total_time()->AddSample(static_cast<int>(ElapsedTime().InMicroseconds()));
                break;
            case ConcurrencyMode::kSynchronous:
                counters->turbofan_optimize_non_concurrent_total_time()->AddSample(static_cast<int>(ElapsedTime().InMicroseconds()));
...
void TurbofanCompilationJob::RecordFunctionCompilation(LogEventListener::CodeTag code_type, Isolate* isolate) const
{
    ...
    Handle<Script> script(Script::cast(compilation_info()->shared_info()->script()), isolate);
    Handle<FeedbackVector> feedback_vector(compilation_info()->closure()->feedback_vector(), isolate);
    Compiler::LogFunctionCompilation(isolate, code_type, script, compilation_info()->shared_info(), feedback_vector, abstract_code, compilation_info()->code_kind(), time_taken_ms);
}
...
void InstallInterpreterTrampolineCopy(Isolate* isolate, Handle<SharedFunctionInfo> shared_info, LogEventListener::CodeTag log_tag)
{
    ...
    Handle<BytecodeArray> bytecode_array(shared_info->GetBytecodeArray(isolate), isolate);

    Handle<Code> code = Builtins::CreateInterpreterEntryTrampolineForProfiling(isolate);

    Handle<InterpreterData> interpreter_data = Handle<InterpreterData>::cast(isolate->factory()->NewStruct(INTERPRETER_DATA_TYPE, AllocationType::kOld));
    ...
    Handle<String> script_name = handle(script->name().IsString() ? String::cast(script->name()) : ReadOnlyRoots(isolate).empty_string(), isolate);
    PROFILE(isolate, CodeCreateEvent(log_tag, abstract_code, shared_info, script_name, line_num, column_num));
}

template <typename IsolateT> void InstallUnoptimizedCode(UnoptimizedCompilationInfo* compilation_info, Handle<SharedFunctionInfo> shared_info, IsolateT* isolate)
{
    if (compilation_info->has_bytecode_array()) {
        ...
        Handle<FeedbackMetadata> feedback_metadata = FeedbackMetadata::New(isolate, compilation_info->feedback_vector_spec());
...
template <typename IsolateT> void EnsureSharedFunctionInfosArrayOnScript(Handle<Script> script, ParseInfo* parse_info, IsolateT* isolate)
{
    DCHECK(parse_info->flags().is_toplevel());
    if (script->shared_function_info_count() > 0) {
        DCHECK_LE(script->shared_function_info_count(), script->shared_function_infos().length());
        DCHECK_EQ(script->shared_function_info_count(), parse_info->max_function_literal_id() + 1);
        return;
    }
    Handle<WeakFixedArray> infos(isolate->factory()->NewWeakFixedArray(parse_info->max_function_literal_id() + 1, AllocationType::kOld));
    script->set_shared_function_infos(*infos);
}

void UpdateSharedFunctionFlagsAfterCompilation(FunctionLiteral* literal, SharedFunctionInfo shared_info)
{
...
CompilationJob::Status FinalizeSingleUnoptimizedCompilationJob(
    UnoptimizedCompilationJob* job, Handle<SharedFunctionInfo> shared_info, IsolateT* isolate, FinalizeUnoptimizedCompilationDataList* finalize_unoptimized_compilation_data_list)
{
    ...
    if (status == CompilationJob::SUCCEEDED) {
        ...
        finalize_unoptimized_compilation_data_list->emplace_back(isolate, shared_info, coverage_info, job->time_taken_to_execute(), job->time_taken_to_finalize());
    }
...
#endif
    std::unique_ptr<UnoptimizedCompilationJob> job(interpreter::Interpreter::NewCompilationJob(parse_info, literal, script, allocator, eager_inner_literals, local_isolate));
...
template <typename IsolateT>
bool IterativelyExecuteAndFinalizeUnoptimizedCompilationJobs(IsolateT* isolate, Handle<SharedFunctionInfo> outer_shared_info, Handle<Script> script, ParseInfo* parse_info,
    AccountingAllocator* allocator, IsCompiledScope* is_compiled_scope, FinalizeUnoptimizedCompilationDataList* finalize_unoptimized_compilation_data_list,
    DeferredFinalizationJobDataList* jobs_to_retry_finalization_on_main_thread)
{
    ...
    while (!functions_to_compile.empty()) {
        ...
        if (is_first) {
            ...
            DCHECK_EQ(literal->function_literal_id(), outer_shared_info->function_literal_id());
...
bool FinalizeDeferredUnoptimizedCompilationJobs(Isolate* isolate, Handle<Script> script, DeferredFinalizationJobDataList* deferred_jobs,
...
public:
    static V8_WARN_UNUSED_RESULT MaybeHandle<CodeT> Get(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, CodeKind code_kind)
    {
        ...
        if (IsOSR(osr_offset)) {
            ...
            base::Optional<CodeT> maybe_code = feedback_vector.GetOptimizedOsrCode(isolate, it.GetSlotOperand(2));
            if (maybe_code.has_value())
                code = maybe_code.value();
        } else {
            feedback_vector.EvictOptimizedCodeMarkedForDeoptimization(shared, "OptimizedCodeCache::Get");
    ...
    static void Insert(Isolate* isolate, JSFunction function, BytecodeOffset osr_offset, CodeT code, bool is_function_context_specializing)
    {
        ...
        if (is_function_context_specializing) {
            ...
            if (feedback_vector.has_optimized_code() && feedback_vector.optimized_code().kind() == code.kind()) {
...
bool CompileTurbofan_NotConcurrent(Isolate* isolate, TurbofanCompilationJob* job)
{
    ...
    OptimizedCodeCache::Insert(
        isolate, *compilation_info->closure(), compilation_info->osr_offset(), ToCodeT(*compilation_info->code()), compilation_info->function_context_specializing());
...
bool CompileTurbofan_Concurrent(Isolate* isolate, std::unique_ptr<TurbofanCompilationJob> job)
{
    ...
    TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.OptimizeConcurrentPrepare", job.get(), TRACE_EVENT_FLAG_FLOW_OUT);
    ...
    SetTieringState(*function, compilation_info->osr_offset(), TieringState::kInProgress);
...
MaybeHandle<CodeT> CompileTurbofan(
    Isolate* isolate, Handle<JSFunction> function, Handle<SharedFunctionInfo> shared, ConcurrencyMode mode, BytecodeOffset osr_offset, CompileResultBehavior result_behavior)
{
    ...
    std::unique_ptr<TurbofanCompilationJob> job(compiler::Pipeline::NewCompilationJob(isolate, function, CodeKind::TURBOFAN, has_script, osr_offset));
...
void RecordMaglevFunctionCompilation(Isolate* isolate, Handle<JSFunction> function)
{
    ...
    Handle<AbstractCode> abstract_code(AbstractCode::cast(FromCodeT(function->code(cage_base))), isolate);
    ...
    Compiler::LogFunctionCompilation(isolate, LogEventListener::CodeTag::kFunction, script, shared, feedback_vector, abstract_code, abstract_code->kind(cage_base), time_taken_ms);
}
#endif // V8_ENABLE_MAGLEV

MaybeHandle<CodeT> CompileMaglev(Isolate* isolate, Handle<JSFunction> function, ConcurrencyMode mode, BytecodeOffset osr_offset, CompileResultBehavior result_behavior)
{
#ifdef V8_ENABLE_MAGLEV
...

    {
        TRACE_EVENT_WITH_FLOW0(
            TRACE_DISABLED_BY_DEFAULT("v8.compile"), IsSynchronous(mode) ? "V8.MaglevPrepare" : "V8.MaglevConcurrentPrepare", job.get(), TRACE_EVENT_FLAG_FLOW_OUT);
...
MaybeHandle<CodeT> GetOrCompileOptimized(Isolate* isolate, Handle<JSFunction> function, ConcurrencyMode mode, CodeKind code_kind,
...
void SpawnDuplicateConcurrentJobForStressTesting(Isolate* isolate, Handle<JSFunction> function, ConcurrencyMode mode, CodeKind code_kind)
{
    ...
    CompileResultBehavior result_behavior = v8_flags.stress_concurrent_inlining_attach_code ? CompileResultBehavior::kDefault : CompileResultBehavior::kDiscardForTesting;
...
template <typename IsolateT> bool PreparePendingException(IsolateT* isolate, ParseInfo* parse_info)
{
...
bool FailWithPreparedPendingException(
    Isolate* isolate, Handle<Script> script, const PendingCompilationErrorHandler* pending_error_handler, Compiler::ClearExceptionFlag flag = Compiler::KEEP_EXCEPTION)
{
...
bool FailWithPendingException(Isolate* isolate, Handle<Script> script, ParseInfo* parse_info, Compiler::ClearExceptionFlag flag)
{
    PreparePendingException(isolate, parse_info);
    return FailWithPreparedPendingException(isolate, script, parse_info->pending_error_handler(), flag);
}
    ...
    const FinalizeUnoptimizedCompilationDataList& finalize_unoptimized_compilation_data_list)
{
    ...
    bool need_source_positions = v8_flags.stress_lazy_source_positions || (!flags.collect_source_positions() && isolate->NeedsSourcePositionsForProfiling());
...
void FinalizeUnoptimizedScriptCompilation(Isolate* isolate, Handle<Script> script, const UnoptimizedCompileFlags& flags, const UnoptimizedCompileState* compile_state,
    const FinalizeUnoptimizedCompilationDataList& finalize_unoptimized_compilation_data_list)
{
...
Handle<SharedFunctionInfo> GetOrCreateTopLevelSharedFunctionInfo(ParseInfo* parse_info, Handle<Script> script, Isolate* isolate, IsCompiledScope* is_compiled_scope)
{
    EnsureSharedFunctionInfosArrayOnScript(script, parse_info, isolate);
    ...
    ParseInfo* parse_info, Handle<Script> script, MaybeHandle<ScopeInfo> maybe_outer_scope_info, Isolate* isolate, IsCompiledScope* is_compiled_scope)
{
    ...
    if (parse_info->literal() == nullptr && !parsing::ParseProgram(parse_info, script, maybe_outer_scope_info, isolate, parsing::ReportStatisticsMode::kYes)) {
        FailWithPendingException(isolate, script, parse_info, Compiler::ClearExceptionFlag::KEEP_EXCEPTION);
    ...
    if (!IterativelyExecuteAndFinalizeUnoptimizedCompilationJobs(
            isolate, shared_info, script, parse_info, isolate->allocator(), is_compiled_scope, &finalize_unoptimized_compilation_data_list, nullptr)) {
        FailWithPendingException(isolate, script, parse_info, Compiler::ClearExceptionFlag::KEEP_EXCEPTION);
        return MaybeHandle<SharedFunctionInfo>();
    }
...
CompilationHandleScope::~CompilationHandleScope() { info_->set_persistent_handles(persistent_.Detach()); }

FinalizeUnoptimizedCompilationData::FinalizeUnoptimizedCompilationData(LocalIsolate* isolate, Handle<SharedFunctionInfo> function_handle, MaybeHandle<CoverageInfo> coverage_info,
...
DeferredFinalizationJobData::DeferredFinalizationJobData(LocalIsolate* isolate, Handle<SharedFunctionInfo> function_handle, std::unique_ptr<UnoptimizedCompilationJob> job)
    : function_handle_(isolate->heap()->NewPersistentHandle(function_handle))
    , job_(std::move(job))
{
}
...
BackgroundCompileTask::BackgroundCompileTask(ScriptStreamingData* streamed_data, Isolate* isolate, ScriptType type, ScriptCompiler::CompileOptions options)
    ...
    , flags_(UnoptimizedCompileFlags::ForToplevelCompile(
          isolate, true, construct_language_mode(v8_flags.use_strict), REPLMode::kNo, type, options != ScriptCompiler::CompileOptions::kEagerCompile && v8_flags.lazy_streaming))
...
void SetScriptFieldsFromDetails(Isolate* isolate, Script script, ScriptDetails script_details, DisallowGarbageCollection* no_gc)
{
    ...
    if (script_details.source_map_url.ToHandle(&source_map_url) && script.source_mapping_url(isolate).IsUndefined(isolate)) {
        ...
        for (MaybeObjectSlot current = start; current != end; ++current) {
            ...
            if (maybe_obj.GetHeapObject(&obj)) {
                if (obj.IsSharedFunctionInfo()) {
                    CHECK((current_object_kind_ == kConstantPool && !is_weak) || (current_object_kind_ == kScriptSfiList && is_weak));
                } else if (obj.IsScript()) {
                    CHECK(host.IsSharedFunctionInfo() && current == MaybeObjectSlot(host.address() + SharedFunctionInfo::kScriptOrDebugInfoOffset));
                } else if (obj.IsFixedArray() && current_object_kind_ == kConstantPool) {
...
void BackgroundCompileTask::Run(LocalIsolate* isolate, ReusableUnoptimizedCompileState* reusable_state)
{
    ...
    if (toplevel_script_compilation) {
        ...
        Handle<Script> script
            = info.CreateScript(isolate, isolate->factory()->empty_string(), kNullMaybeHandle, ScriptOriginOptions(false, false, false, info.flags().is_module()));
        ...
        Handle<SharedFunctionInfo> shared_info = input_shared_info_.ToHandleChecked();
        script_ = isolate->heap()->NewPersistentHandle(Script::cast(shared_info->script()));
        ...
        if (shared_info->HasUncompiledDataWithPreparseData()) {
            info.set_consumed_preparse_data(ConsumedPreparseData::For(isolate, handle(shared_info->uncompiled_data_with_preparse_data().preparse_data(isolate), isolate)));
        }
    }
    ...
    parser.ParseOnBackground(isolate, &info, start_position_, end_position_, function_literal_id_);
    ...
    if (info.literal() != nullptr) {
        ...
        if (toplevel_script_compilation) {
            ...
            shared_info = isolate->factory()->CloneSharedFunctionInfo(input_shared_info_.ToHandleChecked());
        }

        if (IterativelyExecuteAndFinalizeUnoptimizedCompilationJobs(isolate, shared_info, script_, &info, reusable_state->allocator(), &is_compiled_scope_,
...
public:
    explicit ConstantPoolPointerForwarder(PtrComprCageBase cage_base, LocalHeap* local_heap)
        : cage_base_(cage_base)
        , local_heap_(local_heap)
    {
    }

    void AddBytecodeArray(BytecodeArray bytecode_array) { bytecode_arrays_to_update_.push_back(handle(bytecode_array, local_heap_)); }

    void Forward(SharedFunctionInfo from, SharedFunctionInfo to) { forwarding_table_[from.function_literal_id()] = handle(to, local_heap_); }
    ...
    void IterateConstantPool(FixedArray constant_pool)
    {
        for (int i = 0, length = constant_pool.length(); i < length; ++i) {
            ...
            if (heap_obj.IsFixedArray(cage_base_)) {
                ...
                auto it = forwarding_table_.find(SharedFunctionInfo::cast(heap_obj).function_literal_id());
...
BackgroundMergeTask::~BackgroundMergeTask() { DCHECK(!HasPendingForegroundWork()); }
...
void BackgroundMergeTask::BeginMergeInBackground(LocalIsolate* isolate, Handle<Script> new_script)
{
    ...
    {
        DisallowGarbageCollection no_gc;
        MaybeObject maybe_old_toplevel_sfi = old_script->shared_function_infos().Get(kFunctionLiteralIdTopLevel);
        if (maybe_old_toplevel_sfi.IsWeak()) {
            SharedFunctionInfo old_toplevel_sfi = SharedFunctionInfo::cast(maybe_old_toplevel_sfi.GetHeapObjectAssumeWeak());
    ...
    CHECK_EQ(old_script->shared_function_infos().length(), new_script->shared_function_infos().length());
    for (int i = 0; i < old_script->shared_function_infos().length(); ++i) {
        ...
        if (maybe_new_sfi.IsWeak()) {
            SharedFunctionInfo new_sfi = SharedFunctionInfo::cast(maybe_new_sfi.GetHeapObjectAssumeWeak());
            ...
            if (maybe_old_sfi.IsWeak()) {
                ...
                SharedFunctionInfo old_sfi = SharedFunctionInfo::cast(maybe_old_sfi.GetHeapObjectAssumeWeak());
                ...
                if (new_sfi.is_compiled()) {
                    if (old_sfi.is_compiled()) {
                        ...
                        new_compiled_data_for_cached_sfis_.push_back(
                            { local_heap->NewPersistentHandle(old_sfi), local_heap->NewPersistentHandle(function_data), local_heap->NewPersistentHandle(feedback_metadata) });
...
Handle<SharedFunctionInfo> BackgroundMergeTask::CompleteMergeInForeground(Isolate* isolate, Handle<Script> new_script)
{
    DCHECK_EQ(state_, kPendingForegroundWork);

    HandleScope handle_scope(isolate);
    ConstantPoolPointerForwarder forwarder(isolate, isolate->main_thread_local_heap());

    Handle<Script> old_script = cached_script_.ToHandleChecked();

    for (const auto& new_compiled_data : new_compiled_data_for_cached_sfis_) {
        if (!new_compiled_data.cached_sfi->is_compiled()) {
            new_compiled_data.cached_sfi->set_function_data(*new_compiled_data.function_data, kReleaseStore);
            new_compiled_data.cached_sfi->set_feedback_metadata(*new_compiled_data.feedback_metadata, kReleaseStore);
        }
    }
    for (Handle<SharedFunctionInfo> new_sfi : used_new_sfis_) {
        DisallowGarbageCollection no_gc;
        DCHECK_GE(new_sfi->function_literal_id(), 0);
        MaybeObject maybe_old_sfi = old_script->shared_function_infos().Get(new_sfi->function_literal_id());
        if (maybe_old_sfi.IsWeak()) {
            ...
            old_script->shared_function_infos().Set(new_sfi->function_literal_id(), MaybeObject::MakeWeak(MaybeObject::FromObject(*new_sfi)));
        }
    }
    ...
    MaybeObject maybe_toplevel_sfi = old_script->shared_function_infos().Get(kFunctionLiteralIdTopLevel);
    CHECK(maybe_toplevel_sfi.IsWeak());
    Handle<SharedFunctionInfo> result = handle(SharedFunctionInfo::cast(maybe_toplevel_sfi.GetHeapObjectAssumeWeak()), isolate);
...
MaybeHandle<SharedFunctionInfo> BackgroundCompileTask::FinalizeScript(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details)
{
    ...
    if (!maybe_result.is_null() && background_merge_task_.HasPendingForegroundWork()) {
        DCHECK(flags().is_toplevel());
        Handle<SharedFunctionInfo> result = background_merge_task_.CompleteMergeInForeground(isolate, script);
        maybe_result = result;
        script = handle(Script::cast(result->script()), isolate);
        DCHECK(script->source().StrictEquals(*source));
        DCHECK(isolate->factory()->script_list()->Contains(MaybeObject::MakeWeak(MaybeObject::FromObject(*script))));
    } else {
        ...
        scripts = WeakArrayList::Append(isolate, scripts, MaybeObjectHandle::Weak(script));
    ...
    if (!maybe_result.ToHandle(&result)) {
        FailWithPreparedPendingException(isolate, script, compile_state_.pending_error_handler());
...
bool BackgroundCompileTask::FinalizeFunction(Isolate* isolate, Compiler::ClearExceptionFlag flag)
{
    ...
    MaybeHandle<SharedFunctionInfo> maybe_result;
    Handle<SharedFunctionInfo> input_shared_info = input_shared_info_.ToHandleChecked();
    ...
    Handle<SharedFunctionInfo> result;
    if (!maybe_result.ToHandle(&result)) {
        FailWithPreparedPendingException(isolate, script_, compile_state_.pending_error_handler(), flag);
        return false;
    }
...
void BackgroundCompileTask::SourceTextAvailable(Isolate* isolate, Handle<String> source_text, const ScriptDetails& script_details)
{
    DCHECK_EQ(isolate, isolate_for_local_isolate_);
...
bool BackgroundCompileTask::ShouldMergeWithExistingScript() const
{
    ...
    return background_merge_task_.HasPendingBackgroundWork() && jobs_to_retry_finalization_on_main_thread_.empty();
}
...
MaybeHandle<SharedFunctionInfo> BackgroundDeserializeTask::Finish(Isolate* isolate, Handle<String> source, ScriptOriginOptions origin_options)
{
    return CodeSerializer::FinishOffThreadDeserialize(isolate, std::move(off_thread_data_), &cached_data_, source, origin_options, &background_merge_task_);
}
...
bool Compiler::CollectSourcePositions(Isolate* isolate, Handle<SharedFunctionInfo> shared_info)
{
    ...
    std::unique_ptr<UnoptimizedCompilationJob> job;
    {
        job = interpreter::Interpreter::NewSourcePositionCollectionJob(&parse_info, parse_info.literal(), bytecode, isolate->allocator(), isolate->main_thread_local_isolate());
    ...
    if (shared_info->HasDebugInfo() && shared_info->GetDebugInfo().HasInstrumentedBytecodeArray()) {
        ByteArray source_position_table = job->compilation_info()->bytecode_array()->SourcePositionTable();
        shared_info->GetActiveBytecodeArray().set_source_position_table(source_position_table, kReleaseStore);
    }
...
bool Compiler::Compile(
    Isolate* isolate, Handle<SharedFunctionInfo> shared_info, ClearExceptionFlag flag, IsCompiledScope* is_compiled_scope, CreateSourcePositions create_source_positions_flag)
{
    ...
    if (shared_info->HasUncompiledDataWithPreparseData()) {
        parse_info.set_consumed_preparse_data(ConsumedPreparseData::For(isolate, handle(shared_info->uncompiled_data_with_preparse_data().preparse_data(), isolate)));
    }
...
// static
bool Compiler::CompileSharedWithBaseline(Isolate* isolate, Handle<SharedFunctionInfo> shared, Compiler::ClearExceptionFlag flag, IsCompiledScope* is_compiled_scope)
{
    ...
    if (shared->script().IsScript()) {
        LogFunctionCompilation(isolate, LogEventListener::CodeTag::kFunction, handle(Script::cast(shared->script()), isolate), shared, Handle<FeedbackVector>(),
...
// static
bool Compiler::CompileBaseline(Isolate* isolate, Handle<JSFunction> function, ClearExceptionFlag flag, IsCompiledScope* is_compiled_scope)
{
...
// static
MaybeHandle<SharedFunctionInfo> Compiler::CompileForLiveEdit(ParseInfo* parse_info, Handle<Script> script, MaybeHandle<ScopeInfo> outer_scope_info, Isolate* isolate)
{
...
// static
MaybeHandle<JSFunction> Compiler::GetFunctionFromEval(Handle<String> source, Handle<SharedFunctionInfo> outer_info, Handle<Context> context, LanguageMode language_mode,
    ParseRestriction restriction, int parameters_end_pos, int eval_scope_position, int eval_position, ParsingWhileDebugging parsing_while_debugging)
{
    ...
    InfoCellPair eval_result = compilation_cache->LookupEval(source, outer_info, context, language_mode, eval_scope_position);
    ...
    if (eval_result.has_shared()) {
        ...
        UnoptimizedCompileFlags flags = UnoptimizedCompileFlags::ForToplevelCompile(isolate, true, language_mode, REPLMode::kNo, ScriptType::kClassic, v8_flags.lazy_eval);
    ...
    if (eval_result.has_shared()) {
        if (eval_result.has_feedback_cell()) {
            result = Factory::JSFunctionBuilder { isolate, shared_info, context }.set_feedback_cell(feedback_cell).set_allocation_type(AllocationType::kYoung).Build();
        } else {
            ...
            if (allow_eval_cache) {
                ...
                Handle<FeedbackCell> new_feedback_cell(result->raw_feedback_cell(), isolate);
                compilation_cache->PutEval(source, outer_info, context, shared_info, new_feedback_cell, eval_scope_position);
            }
        }
    } else {
        result = Factory::JSFunctionBuilder { isolate, shared_info, context }.set_allocation_type(AllocationType::kYoung).Build();
...
bool CodeGenerationFromStringsAllowed(Isolate* isolate, Handle<Context> context, Handle<String> source)
{
    ...
    ExternalCallbackScope external_callback(isolate, reinterpret_cast<Address>(callback));
...
bool ModifyCodeGenerationFromStrings(Isolate* isolate, Handle<Context> context, Handle<i::Object>* source, bool is_code_like)
{
    DCHECK(isolate->modify_code_gen_callback() || isolate->modify_code_gen_callback2());
    DCHECK(source);
    ...
    RCS_SCOPE(isolate, RuntimeCallCounterId::kCodeGenerationFromStringsCallbacks);
    ModifyCodeGenerationFromStringsResult result = isolate->modify_code_gen_callback()
        ? isolate->modify_code_gen_callback()(v8::Utils::ToLocal(context), v8::Utils::ToLocal(*source))
        : isolate->modify_code_gen_callback2()(v8::Utils::ToLocal(context), v8::Utils::ToLocal(*source), is_code_like);
...
// static
std::pair<MaybeHandle<String>, bool> Compiler::ValidateDynamicCompilationSource(Isolate* isolate, Handle<Context> context, Handle<i::Object> original_source, bool is_code_like)
{
    ...
    // the same.
    if (!context->allow_code_gen_from_strings().IsFalse(isolate) && original_source->IsString()) {
    ...
    if (isolate->modify_code_gen_callback() || isolate->modify_code_gen_callback2()) {
    ...
    if (!context->allow_code_gen_from_strings().IsFalse(isolate) && original_source->IsCodeLike(isolate)) {
...
// static
MaybeHandle<JSFunction> Compiler::GetFunctionFromValidatedString(Handle<Context> context, MaybeHandle<String> source, ParseRestriction restriction, int parameters_end_pos)
{
    ...
    if (source.is_null()) {
        ...
        THROW_NEW_ERROR(isolate, NewEvalError(MessageTemplate::kCodeGenFromStrings, error_message), JSFunction);
    }
    ...
    return Compiler::GetFunctionFromEval(
        source.ToHandleChecked(), outer_info, native_context, LanguageMode::kSloppy, restriction, parameters_end_pos, eval_scope_position, eval_position);
}

// static
MaybeHandle<JSFunction> Compiler::GetFunctionFromString(Handle<Context> context, Handle<Object> source, ParseRestriction restriction, int parameters_end_pos, bool is_code_like)
{
    Isolate* const isolate = context->GetIsolate();
    MaybeHandle<String> validated_source = ValidateDynamicCompilationSource(isolate, context, source, is_code_like).first;
    return GetFunctionFromValidatedString(context, validated_source, restriction, parameters_end_pos);
}
...
public:
    ...
    explicit ScriptCompileTimerScope(Isolate* isolate, ScriptCompiler::NoCacheReason no_cache_reason)
        : isolate_(isolate)
    ...
    void set_consuming_code_cache_failed() { consuming_code_cache_failed_ = true; }
    ...
    MaybeHandle<FixedArray> maybe_wrapped_arguments = kNullMaybeHandle)
{
    ...
    Handle<Script> script = parse_info->CreateScript(isolate, source, maybe_wrapped_arguments, script_details.origin_options, natives);
    DisallowGarbageCollection no_gc;
    SetScriptFieldsFromDetails(isolate, *script, script_details, &no_gc);
    LOG(isolate, ScriptDetails(*script));
    return script;
...
MaybeHandle<SharedFunctionInfo> CompileScriptOnMainThread(const UnoptimizedCompileFlags flags, Handle<String> source, const ScriptDetails& script_details, NativesFlag natives,
    v8::Extension* extension, Isolate* isolate, MaybeHandle<Script> maybe_script, IsCompiledScope* is_compiled_scope)
{
    UnoptimizedCompileState compile_state;
    ReusableUnoptimizedCompileState reusable_state(isolate);
    ParseInfo parse_info(isolate, flags, &compile_state, &reusable_state);
...
public:
    StressBackgroundCompileThread(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details)
        : base::Thread(base::Thread::Options("StressBackgroundCompileThread", 2 * i::MB))
        , source_(source)
        , streamed_source_(std::make_unique<SourceStream>(source, isolate), v8::ScriptCompiler::StreamedSource::UTF8)
    {
        ScriptType type = script_details.origin_options.IsModule() ? ScriptType::kModule : ScriptType::kClassic;
        data()->task = std::make_unique<i::BackgroundCompileTask>(data(), isolate, type, ScriptCompiler::CompileOptions::kNoCompileOptions);
...
private:
    ...
    public:
        SourceStream(Handle<String> source, Isolate* isolate)
            : done_(false)
        {
            source_buffer_ = source->ToCString(ALLOW_NULLS, FAST_STRING_TRAVERSAL, &source_length_);
        }
...
bool CanBackgroundCompile(const ScriptDetails& script_details, v8::Extension* extension, ScriptCompiler::CompileOptions compile_options, NativesFlag natives)
{
    ...
    return !script_details.origin_options.IsModule() && !extension && script_details.repl_mode == REPLMode::kNo && compile_options == ScriptCompiler::kNoCompileOptions
        && natives == NOT_NATIVES_CODE;
}

bool CompilationExceptionIsRangeError(Isolate* isolate, Handle<Object> obj)
{
    if (!obj->IsJSError(isolate))
        return false;
    ...
    Handle<String> source, const ScriptDetails& script_details, Isolate* isolate, IsCompiledScope* is_compiled_scope)
{
    ...
    // In parallel, compile on the main thread to flush out any data races.
    {
        ...
        main_thread_maybe_result
            = CompileScriptOnMainThread(flags_copy, source, script_details, NOT_NATIVES_CODE, nullptr, isolate, MaybeHandle<Script>(), &inner_is_compiled_scope);
        if (main_thread_maybe_result.is_null()) {
            // Assume all range errors are stack overflows.
            main_thread_had_stack_overflow = CompilationExceptionIsRangeError(isolate, handle(isolate->pending_exception(), isolate));
            isolate->clear_pending_exception();
        }
    }
    ...
    MaybeHandle<SharedFunctionInfo> maybe_result = Compiler::GetSharedFunctionInfoForStreamedScript(isolate, source, script_details, background_compile_thread.data());
...
MaybeHandle<SharedFunctionInfo> GetSharedFunctionInfoForScriptImpl(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details, v8::Extension* extension,
    AlignedCachedData* cached_data, BackgroundDeserializeTask* deserialize_task, ScriptCompiler::CompileOptions compile_options, ScriptCompiler::NoCacheReason no_cache_reason,
    NativesFlag natives)
{
    ...
    if (V8_UNLIKELY(v8_flags.experimental_web_snapshots
            && (source->IsExternalOneByteString() || source->IsSeqOneByteString() || source->IsExternalTwoByteString() || source->IsSeqTwoByteString()) && source->length() > 4)) {
        ...
        for (size_t i = 0; i < sizeof(WebSnapshotSerializerDeserializer::kMagicNumber); ++i) {
            if (source->Get(static_cast<int>(i)) != WebSnapshotSerializerDeserializer::kMagicNumber[i]) {
                magic_matches = false;
                break;
            }
        }
        if (magic_matches) {
            return Compiler::GetSharedFunctionInfoForWebSnapshot(isolate, source, script_details.name_obj);
        }
    }
    ...
    if (use_compilation_cache) {
        ...
        if (!maybe_result.is_null()) {
            ...
            if (deserialize_task) {
                ...
                maybe_result = deserialize_task->Finish(isolate, source, script_details.origin_options);
                ...
                maybe_result = CodeSerializer::Deserialize(isolate, cached_data, source, script_details.origin_options);
    ...
    if (maybe_result.is_null()) {
        ...
        if (v8_flags.stress_background_compile && CanBackgroundCompile(script_details, extension, compile_options, natives)) {
            ...
            UnoptimizedCompileFlags flags = UnoptimizedCompileFlags::ForToplevelCompile(isolate, natives == NOT_NATIVES_CODE, language_mode, script_details.repl_mode,
                script_details.origin_options.IsModule() ? ScriptType::kModule : ScriptType::kClassic, v8_flags.lazy);
...
MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScript(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details,
    ScriptCompiler::CompileOptions compile_options, ScriptCompiler::NoCacheReason no_cache_reason, NativesFlag natives)
{
    return GetSharedFunctionInfoForScriptImpl(isolate, source, script_details, nullptr, nullptr, nullptr, compile_options, no_cache_reason, natives);
}

MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScriptWithExtension(
    Isolate* isolate, Handle<String> source, const ScriptDetails& script_details, v8::Extension* extension, ScriptCompiler::CompileOptions compile_options, NativesFlag natives)
{
    return GetSharedFunctionInfoForScriptImpl(isolate, source, script_details, extension, nullptr, nullptr, compile_options, ScriptCompiler::kNoCacheBecauseV8Extension, natives);
}

MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScriptWithCachedData(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details,
    AlignedCachedData* cached_data, ScriptCompiler::CompileOptions compile_options, ScriptCompiler::NoCacheReason no_cache_reason, NativesFlag natives)
{
    return GetSharedFunctionInfoForScriptImpl(isolate, source, script_details, nullptr, cached_data, nullptr, compile_options, no_cache_reason, natives);
}

MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForScriptWithDeserializeTask(Isolate* isolate, Handle<String> source, const ScriptDetails& script_details,
    BackgroundDeserializeTask* deserialize_task, ScriptCompiler::CompileOptions compile_options, ScriptCompiler::NoCacheReason no_cache_reason, NativesFlag natives)
{
    return GetSharedFunctionInfoForScriptImpl(isolate, source, script_details, nullptr, nullptr, deserialize_task, compile_options, no_cache_reason, natives);
}

// static
MaybeHandle<JSFunction> Compiler::GetWrappedFunction(Handle<String> source, Handle<FixedArray> arguments, Handle<Context> context, const ScriptDetails& script_details,
    AlignedCachedData* cached_data, v8::ScriptCompiler::CompileOptions compile_options, v8::ScriptCompiler::NoCacheReason no_cache_reason)
{
    ...
    if (!maybe_result.ToHandle(&wrapped)) {
        UnoptimizedCompileFlags flags = UnoptimizedCompileFlags::ForToplevelCompile(isolate, true, language_mode, script_details.repl_mode, ScriptType::kClassic, v8_flags.lazy);
        ...
        script = NewScript(isolate, &parse_info, source, script_details, NOT_NATIVES_CODE, arguments);
        ...
        for (SharedFunctionInfo info = infos.Next(); !info.is_null(); info = infos.Next()) {
            if (info.is_wrapped()) {
    ...
    return Factory::JSFunctionBuilder { isolate, wrapped, context }.set_allocation_type(AllocationType::kYoung).Build();
}

// static
MaybeHandle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForStreamedScript(
    Isolate* isolate, Handle<String> source, const ScriptDetails& script_details, ScriptStreamingData* streaming_data)
{
    DCHECK(!script_details.origin_options.IsWasm());

    ScriptCompileTimerScope compile_timer(isolate, ScriptCompiler::kNoCacheBecauseStreamingSource);
    PostponeInterruptsScope postpone(isolate);
...
// static
Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfoForWebSnapshot(Isolate* isolate, Handle<String> source, MaybeHandle<Object> maybe_script_name)
{
    ...
    Handle<WeakFixedArray> shared_function_infos = isolate->factory()->NewWeakFixedArray(1, AllocationType::kOld);
    ...
    Handle<SharedFunctionInfo> shared = isolate->factory()->NewSharedFunctionInfoForWebSnapshot();
    shared->SetScript(isolate->factory()->read_only_roots(), *script, 0, false);
    return shared;
}

// static
template <typename IsolateT> Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfo(FunctionLiteral* literal, Handle<Script> script, IsolateT* isolate)
{
    ...
    if (maybe_existing.ToHandle(&existing)) {
        ...
        if (literal->produced_preparse_data() != nullptr && existing->HasUncompiledDataWithoutPreparseData()) {
            Handle<UncompiledData> existing_uncompiled_data = handle(existing->uncompiled_data(), isolate);
            ...
            Handle<UncompiledData> new_uncompiled_data = isolate->factory()->NewUncompiledDataWithPreparseData(
...
template Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfo(FunctionLiteral* literal, Handle<Script> script, Isolate* isolate);
template Handle<SharedFunctionInfo> Compiler::GetSharedFunctionInfo(FunctionLiteral* literal, Handle<Script> script, LocalIsolate* isolate);

// static
MaybeHandle<CodeT> Compiler::CompileOptimizedOSR(Isolate* isolate, Handle<JSFunction> function, BytecodeOffset osr_offset, ConcurrencyMode mode)
{
    DCHECK(IsOSR(osr_offset));
    ...
    MaybeHandle<CodeT> result = GetOrCompileOptimized(isolate, function, mode, CodeKind::TURBOFAN, osr_offset);
...
// static
void Compiler::DisposeTurbofanCompilationJob(TurbofanCompilationJob* job, bool restore_function_code)
{
...
void Compiler::FinalizeTurbofanCompilationJob(TurbofanCompilationJob* job, Isolate* isolate)
{
    ...
    TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "V8.OptimizeConcurrentFinalize", job, TRACE_EVENT_FLAG_FLOW_IN);
...
void Compiler::FinalizeMaglevCompilationJob(maglev::MaglevCompilationJob* job, Isolate* isolate)
{
    ...
    if (status == CompilationJob::SUCCEEDED) {
        ...
        OptimizedCodeCache::Insert(isolate, *function, BytecodeOffset::None(), function->code(), job->specialize_to_function_context());
        ...
        CompilerTracer::TraceFinishMaglevCompile(isolate, function, ms_prepare, ms_optimize, ms_codegen);
    }
#endif
}
...
void Compiler::PostInstantiation(Handle<JSFunction> function)
{
    ...
    if (is_compiled_scope.is_compiled() && shared->HasBytecodeArray()) {
        ...
        if (function->has_feedback_vector()) {
            ...
            function->feedback_vector().EvictOptimizedCodeMarkedForDeoptimization(*shared, "new function from shared function info");
            ...
            if (!code.is_null()) {
                ...
                static_assert(FeedbackVector::kFeedbackVectorMaybeOptimizedCodeIsStoreRelease);
                function->set_code(code);
            }
        }

        if (v8_flags.always_turbofan && shared->allows_lazy_compilation() && !shared->optimization_disabled() && !function->HasAvailableOptimizedCode()) {
            CompilerTracer::TraceMarkForAlwaysOpt(isolate, function);
            JSFunction::EnsureFeedbackVector(isolate, function, &is_compiled_scope);
            function->MarkForOptimization(isolate, CodeKind::TURBOFAN, ConcurrencyMode::kSynchronous);
        }
    }
