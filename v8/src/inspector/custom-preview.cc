...
void reportError(v8::Local<v8::Context> context, const v8::TryCatch& tryCatch)
{
    ...
    storage->addMessage(V8ConsoleMessage::createForConsoleAPI(
        context, contextId, groupId, inspector, inspector->client()->currentTimeMS(), ConsoleAPIType::kError, arguments, String16(), nullptr));
}
...
bool substituteObjectTags(int sessionId, const String16& groupName, v8::Local<v8::Context> context, v8::Local<v8::Array> jsonML, int maxDepth)
{
    ...
    if (jsonML->Length() == 2 && firstValue->IsString() && firstValue.As<v8::String>()->StringEquals(objectLiteral)) {
        ...
        protocol::Response response = injectedScript->wrapObject(originValue, groupName, WrapMode::kNoPreview, configValue, maxDepth - 1, &wrapper);
...
void bodyCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
{
    ...
    if (jsonML->Length()
        && !substituteObjectTags(sessionIdValue.As<v8::Int32>()->Value(), toProtocolString(isolate, groupNameValue.As<v8::String>()), context, jsonML, kMaxCustomPreviewDepth)) {
        return;
    }
...
void generateCustomPreview(
    int sessionId, const String16& groupName, v8::Local<v8::Object> object, v8::MaybeLocal<v8::Value> maybeConfig, int maxDepth, std::unique_ptr<CustomPreview>* preview)
{
