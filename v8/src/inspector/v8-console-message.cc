...
void V8ConsoleMessage::reportToFrontend(protocol::Console::Frontend* frontend) const
{
    ...
    std::unique_ptr<protocol::Console::ConsoleMessage> result
        = protocol::Console::ConsoleMessage::create().setSource(protocol::Console::ConsoleMessage::SourceEnum::ConsoleApi).setLevel(level).setText(m_message).build();
    if (m_lineNumber)
...
void V8ConsoleMessage::reportToFrontend(protocol::Runtime::Frontend* frontend, V8InspectorSessionImpl* session, bool generatePreview) const
{
    ...
    if (m_origin == V8MessageOrigin::kConsole) {
        ...
        frontend->consoleAPICalled(consoleAPITypeValue(m_type), std::move(arguments), m_contextId, m_timestamp, std::move(stackTrace), std::move(consoleContext));
        return;
    }
    UNREACHABLE();
}
...
// static
std::unique_ptr<V8ConsoleMessage> V8ConsoleMessage::createForConsoleAPI(v8::Local<v8::Context> v8Context, int contextId, int groupId, V8InspectorImpl* inspector, double timestamp,
    ConsoleAPIType type, const std::vector<v8::Local<v8::Value>>& arguments, const String16& consoleContext, std::unique_ptr<V8StackTraceImpl> stackTrace)
{
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        std::unique_ptr<v8::Global<v8::Value>> argument(new v8::Global<v8::Value>(isolate, arguments.at(i)));
    ...
    for (size_t i = 0, num_args = arguments.size(); i < num_args; ++i) {
        ...
        message->m_message += V8ValueStringBuilder::toString(arguments[i], v8Context);
    }
    ...
    if (type != ConsoleAPIType::kClear) {
        inspector->client()->consoleAPIMessage(
            groupId, clientLevel, toStringView(message->m_message), toStringView(message->m_url), message->m_lineNumber, message->m_columnNumber, message->m_stackTrace.get());
    }
...
// static
std::unique_ptr<V8ConsoleMessage> V8ConsoleMessage::createForException(double timestamp, const String16& detailedMessage, const String16& url, unsigned lineNumber,
    unsigned columnNumber, std::unique_ptr<V8StackTraceImpl> stackTrace, int scriptId, v8::Isolate* isolate, const String16& message, int contextId, v8::Local<v8::Value> exception,
...
int V8ConsoleMessageStorage::count(int contextId, const String16& id) { return ++m_data[contextId].m_count[id]; }

void V8ConsoleMessageStorage::time(int contextId, const String16& id) { m_data[contextId].m_time[id] = m_inspector->client()->currentTimeMS(); }
