...
RegExpNode* RegExpText::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return compiler->zone()->New<TextNode>(elements(), compiler->read_backward(), on_success); }
...
constexpr UC16Range ToUC16Range(base::uc16 from, base::uc16 to) { return (static_cast<uint32_t>(from) << 16) | to; }
constexpr base::uc16 ExtractFrom(UC16Range r) { return static_cast<base::uc16>(r >> 16); }
constexpr base::uc16 ExtractTo(UC16Range r) { return static_cast<base::uc16>(r); }
...
void AddNonBmpSurrogatePairs(RegExpCompiler* compiler, ChoiceNode* result, RegExpNode* on_success, UnicodeRangeSplitter* splitter)
{
    ...
    for (const auto& it : grouped_by_leading) {
        ...
        result->AddAlternative(GuardedAlternative(TextNode::CreateForSurrogatePair(zone, leading_range, trailing_ranges, compiler->read_backward(), on_success)));
    }
}
...
RegExpNode* RegExpClassSetExpression::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return ToCharacterClass(compiler->zone())->ToNode(compiler, on_success); }
...
int CompareCaseInsensitive(const icu::UnicodeString& a, const icu::UnicodeString& b) { return a.caseCompare(b, U_FOLD_CASE_DEFAULT); }
...
bool CharAtEquals(bool ignore_case, int index, const RegExpAtom* a, const RegExpAtom* b) { return Equals(ignore_case, a->data().at(index), b->data().at(index)); }
...
RegExpNode* RegExpQuantifier::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return ToNode(min(), max(), is_greedy(), body(), compiler, on_success); }

namespace {
// Desugar \b to (?<=\w)(?=\W)|(?<=\W)(?=\w) and
//         \B to (?<=\w)(?=\w)|(?<=\W)(?=\W)
RegExpNode* BoundaryAssertionAsLookaround(RegExpCompiler* compiler, RegExpNode* on_success, RegExpAssertion::Type type, RegExpFlags flags)
{
...
RegExpNode* RegExpAssertion::ToNode(RegExpCompiler* compiler, RegExpNode* on_success)
{
    ...
    case Type::BOUNDARY:
        return NeedsUnicodeCaseEquivalents(compiler->flags()) ? BoundaryAssertionAsLookaround(compiler, on_success, Type::BOUNDARY, compiler->flags())
                                                              : AssertionNode::AtBoundary(on_success);
    case Type::NON_BOUNDARY:
        return NeedsUnicodeCaseEquivalents(compiler->flags()) ? BoundaryAssertionAsLookaround(compiler, on_success, Type::NON_BOUNDARY, compiler->flags())
    ...
    case Type::END_OF_LINE: {
        ...
        TextNode* newline_matcher = zone->New<TextNode>(newline_atom, false,
            ActionNode::PositiveSubmatchSuccess(stack_pointer_register, position_register,
                0, // No captures inside.
                -1, // Ignored if no captures.
                on_success));
        // Create an end-of-input matcher.
        RegExpNode* end_of_line = ActionNode::BeginPositiveSubmatch(stack_pointer_register, position_register, newline_matcher);
...
{
    return compiler->zone()->New<BackReferenceNode>(RegExpCapture::StartRegister(index()), RegExpCapture::EndRegister(index()), flags_, compiler->read_backward(), on_success);
}

RegExpNode* RegExpEmpty::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return on_success; }

RegExpNode* RegExpGroup::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return body_->ToNode(compiler, on_success); }
...
{
    if (is_positive_) {
        on_match_success_ = ActionNode::PositiveSubmatchSuccess(stack_pointer_register, position_register, capture_register_count, capture_register_start, on_success_);
    } else {
...
{
    if (is_positive_) {
        return ActionNode::BeginPositiveSubmatch(stack_pointer_register_, position_register_, match);
    } else {
        Zone* zone = on_success_->zone();
...
RegExpNode* RegExpCapture::ToNode(RegExpCompiler* compiler, RegExpNode* on_success) { return ToNode(body(), index(), compiler, on_success); }
...
void CharacterRange::AddClassEscape(StandardCharacterSet standard_character_set, ZoneList<CharacterRange>* ranges, bool add_unicode_case_equivalents, Zone* zone)
{
...
void CharacterRange::AddCaseEquivalents(Isolate* isolate, Zone* zone, ZoneList<CharacterRange>* ranges, bool is_one_byte)
{
    ...
    for (int i = 0; i < range_count; i++) {
        ...
        if (is_one_byte && !RangeContainsLatin1Equivalents(range)) {
            if (from > kMaxOneByteCharCode)
                continue;
            if (to > kMaxOneByteCharCode)
                to = kMaxOneByteCharCode;
        }
        others.add(from, to);
    }
...
{
    ZoneList<CharacterRange>* result_ranges = zone->template New<ZoneList<CharacterRange>>(2, zone);
    ZoneList<CharacterRange>* temp_ranges = zone->template New<ZoneList<CharacterRange>>(2, zone);
    ComputeCharacterRanges(this, result_ranges, temp_ranges, zone);
    return zone->template New<RegExpClassRanges>(zone, result_ranges);
}

// static
void RegExpClassSetExpression::ComputeCharacterRanges(RegExpTree* root, ZoneList<CharacterRange>* result_ranges, ZoneList<CharacterRange>* temp_ranges, Zone* zone)
{
    ...
    case OperationType::kUnion: {
        ZoneList<CharacterRange>* op_ranges = zone->template New<ZoneList<CharacterRange>>(2, zone);
    ...
    case OperationType::kIntersection: {
        ZoneList<CharacterRange>* op_ranges = zone->template New<ZoneList<CharacterRange>>(2, zone);
        ComputeCharacterRanges(node->operands()->at(0), op_ranges, temp_ranges, zone);
        result_ranges->AddAll(*op_ranges, zone);
        op_ranges->Rewind(0);
        for (int i = 1; i < node->operands()->length(); i++) {
            ComputeCharacterRanges(node->operands()->at(i), op_ranges, temp_ranges, zone);
    ...
    case OperationType::kSubtraction: {
        ...
        ZoneList<CharacterRange>* lhs_range
            = node->operands()->at(0)->IsClassRanges() ? node->operands()->at(0)->AsClassRanges()->ranges(zone) : node->operands()->at(0)->AsClassSetExpression()->ranges_;
        ZoneList<CharacterRange>* rhs_union = zone->template New<ZoneList<CharacterRange>>(2, zone);
        for (int i = 1; i < node->operands()->length(); i++) {
            ZoneList<CharacterRange>* op_range
                = node->operands()->at(i)->IsClassRanges() ? node->operands()->at(i)->AsClassRanges()->ranges(zone) : node->operands()->at(i)->AsClassSetExpression()->ranges_;
            rhs_union->AddAll(*op_range, zone);
        }
        CharacterRange::Canonicalize(rhs_union);
        ZoneList<CharacterRange>* ranges_check = zone->template New<ZoneList<CharacterRange>>(2, zone);
        ...
        ZoneList<CharacterRange>* rhs_union_negated = zone->template New<ZoneList<CharacterRange>>(rhs_union->length(), zone);
        CharacterRange::Negate(rhs_union, rhs_union_negated, zone);
        ranges_check->Rewind(0);
        CharacterRange::Intersect(lhs_range, rhs_union_negated, ranges_check, zone);
        DCHECK(CharacterRange::Equals(result_ranges, ranges_check));
#endif
        break;
    }
    ...
    node->ranges_ = zone->template New<ZoneList<CharacterRange>>(*result_ranges, zone);
#endif
}
...
public:
    ...
    ~RegExpExpansionLimiter() { compiler_->set_current_expansion_factor(saved_expansion_factor_); }
