...
void RegExpAtom::AppendToText(RegExpText* text, Zone* zone) { text->AddElement(TextElement::Atom(this), zone); }

void RegExpClassRanges::AppendToText(RegExpText* text, Zone* zone) { text->AddElement(TextElement::ClassRanges(this), zone); }
...
TextElement TextElement::Atom(RegExpAtom* atom) { return TextElement(ATOM, atom); }

TextElement TextElement::ClassRanges(RegExpClassRanges* class_ranges) { return TextElement(CLASS_RANGES, class_ranges); }
...
RegExpCompiler::CompilationResult RegExpCompiler::Assemble(Isolate* isolate, RegExpMacroAssembler* macro_assembler, RegExpNode* start, int capture_count, Handle<String> pattern)
{
...
void Trace::RestoreAffectedRegisters(RegExpMacroAssembler* assembler, int max_register, const DynamicBitSet& registers_to_pop, const DynamicBitSet& registers_to_clear)
{
...
void Trace::PerformDeferredActions(
    RegExpMacroAssembler* assembler, int max_register, const DynamicBitSet& affected_registers, DynamicBitSet* registers_to_pop, DynamicBitSet* registers_to_clear, Zone* zone)
{
...
ActionNode* ActionNode::BeginPositiveSubmatch(int stack_reg, int position_reg, RegExpNode* on_success)
{
    ActionNode* result = on_success->zone()->New<ActionNode>(BEGIN_POSITIVE_SUBMATCH, on_success);
...
int GetCaseIndependentLetters(Isolate* isolate, base::uc16 character, bool one_byte_subject, unibrow::uchar* letters, int letter_length)
{
#ifdef V8_INTL_SUPPORT
...
void EmitUseLookupTable(RegExpMacroAssembler* masm, ZoneList<base::uc32>* ranges, uint32_t start_index, uint32_t end_index, base::uc32 min_char, Label* fall_through,
...
void CutOutRange(RegExpMacroAssembler* masm, ZoneList<base::uc32>* ranges, uint32_t start_index, uint32_t end_index, uint32_t cut_index, Label* even_label, Label* odd_label)
{
...
// with recursion.
void SplitSearchSpace(ZoneList<base::uc32>* ranges, uint32_t start_index, uint32_t end_index, uint32_t* new_start_index, uint32_t* new_end_index, base::uc32* border)
{
    static const uint32_t kSize = RegExpMacroAssembler::kTableSize;
    static const uint32_t kMask = RegExpMacroAssembler::kTableMask;
...
void GenerateBranches(RegExpMacroAssembler* masm, ZoneList<base::uc32>* ranges, uint32_t start_index, uint32_t end_index, base::uc32 min_char, base::uc32 max_char,
...
void EmitClassRanges(RegExpMacroAssembler* macro_assembler, RegExpClassRanges* cr, bool one_byte, Label* on_failure, int cp_offset, bool check_offset, bool preloaded, Zone* zone)
{
...
bool RegExpNode::KeepRecursing(RegExpCompiler* compiler) { return !compiler->limiting_recursion() && compiler->recursion_depth() <= RegExpCompiler::kMaxRecursion; }
...
int RegExpNode::EatsAtLeast(bool not_at_start) { return not_at_start ? eats_at_least_.eats_at_least_from_not_start : eats_at_least_.eats_at_least_from_possibly_start; }
...
EatsAtLeastInfo LoopChoiceNode::EatsAtLeastFromLoopEntry()
{
    ...
    uint8_t loop_body_from_not_start = base::saturated_cast<uint8_t>(loop_node_->EatsAtLeast(true) - continue_node_->EatsAtLeast(true));
    uint8_t loop_body_from_possibly_start = base::saturated_cast<uint8_t>(loop_node_->EatsAtLeast(false) - continue_node_->EatsAtLeast(true));
    ...
    if (loop_iterations > 0 && loop_body_from_possibly_start > 0) {
        ...
        result.eats_at_least_from_possibly_start
            = base::saturated_cast<uint8_t>(loop_body_from_possibly_start + (loop_iterations - 1) * loop_body_from_not_start + continue_node_->EatsAtLeast(true));
    } else {
        ...
        if (elm.text_type() == TextElement::ATOM) {
            ...
            for (int i = 0; i < characters && i < quarks.length(); i++) {
                ...
                if (IsIgnoreCase(compiler->flags())) {
                    unibrow::uchar chars[4];
                    int length = GetCaseIndependentLetters(isolate, c, compiler->one_byte(), chars, 4);
                    if (length == 0) {
...
RegExpNode* LoopChoiceNode::FilterOneByte(int depth, RegExpFlags flags)
{
        ...
        return this;
    {
        ...
        RegExpNode* continue_replacement = continue_node_->FilterOneByte(depth - 1, flags);
...
RegExpNode* ChoiceNode::FilterOneByte(int depth, RegExpFlags flags)
{
    ...
    for (int i = 0; i < choice_count; i++) {
        GuardedAlternative alternative = alternatives_->at(i);
        RegExpNode* replacement = alternative.node()->FilterOneByte(depth - 1, flags);
    ...
    for (int i = 0; i < choice_count; i++) {
        RegExpNode* replacement = alternatives_->at(i).node()->FilterOneByte(depth - 1, flags);
        if (replacement != nullptr) {
            alternatives_->at(i).set_node(replacement);
...
RegExpNode* NegativeLookaroundChoiceNode::FilterOneByte(int depth, RegExpFlags flags)
{
    if (info()->replacement_calculated)
        return replacement();
    if (depth < 0)
...
{
    if (assembler->CheckSpecialClassRanges(fall_through_on_word ? StandardCharacterSet::kWord : StandardCharacterSet::kNotWord, fall_through_on_word ? non_word : word)) {
...
void TextNode::TextEmitPass(RegExpCompiler* compiler, TextEmitPassType pass, bool preloaded, Trace* trace, bool first_element_checked, int* checked_up_to)
{
    ...
    for (int i = preloaded ? 0 : element_count - 1; i >= 0; i--) {
        ...
        if (elm.text_type() == TextElement::ATOM) {
            if (SkipPass(pass, IsIgnoreCase(compiler->flags())))
                continue;
            base::Vector<const base::uc16> quarks = elm.atom()->data();
...
void TextNode::Emit(RegExpCompiler* compiler, Trace* trace)
{
    ...
    for (int pass = kFirstRealPass; pass <= kLastPass; pass++) {
        TextEmitPass(compiler, static_cast<TextEmitPassType>(pass), false, trace, first_elt_done, &bound_checked_to);
    }
...
void BoyerMoorePositionInfo::Set(int character) { SetInterval(Interval(character, character)); }
...
int BoyerMooreLookahead::GetSkipTable(int min_lookahead, int max_lookahead, Handle<ByteArray> boolean_skip_table)
{
    const int kSkipArrayEntry = 0;
    const int kDontSkipArrayEntry = 1;

    std::memset(boolean_skip_table->GetDataStartAddress(), kSkipArrayEntry, boolean_skip_table->length());
...
void ChoiceNode::EmitChoices(RegExpCompiler* compiler, AlternativeGenerationList* alt_gens, int first_choice, Trace* trace, PreloadState* preload)
{
    ...
    for (int i = first_choice; i < choice_count; i++) {
        ...
        if (compiler->optimize() && try_to_emit_quick_check_for_alternative(i == 0)
            && alternative.node()->EmitQuickCheck(
                compiler, trace, &new_trace, preload->preload_has_checked_bounds_, &alt_gen->possible_success, &alt_gen->quick_check_details, fall_through_on_failure, this)) {
...
public:
    ...
    static void VisitLoopChoiceContinueNode(LoopChoiceNode* that) { that->info()->AddFromFollowing(that->continue_node()->info()); }

    static void VisitLoopChoiceLoopNode(LoopChoiceNode* that) { that->info()->AddFromFollowing(that->loop_node()->info()); }

    static void VisitNegativeLookaroundChoiceLookaroundNode(NegativeLookaroundChoiceNode* that) { VisitChoice(that, NegativeLookaroundChoiceNode::kLookaroundIndex); }

    static void VisitNegativeLookaroundChoiceContinueNode(NegativeLookaroundChoiceNode* that) { VisitChoice(that, NegativeLookaroundChoiceNode::kContinueIndex); }
...
public:
    ...
    static void VisitNegativeLookaroundChoiceContinueNode(NegativeLookaroundChoiceNode* that) { that->set_eats_at_least_info(*that->continue_node()->eats_at_least_info()); }
        ...
        if (text.text_type() == TextElement::ATOM) {
            ...
            for (int j = 0; j < atom->length(); j++, offset++) {
                ...
                if (IsIgnoreCase(bm->compiler()->flags())) {
                    unibrow::uchar chars[4];
                    int length = GetCaseIndependentLetters(isolate, character, bm->max_char() == String::kMaxOneByteCharCode, chars, 4);
...
RegExpNode* RegExpCompiler::PreprocessRegExp(RegExpCompileData* data, RegExpFlags flags, bool is_one_byte)
{
    ...
    if (!data->tree->IsAnchoredAtStart() && !IsSticky(flags)) {
        ...
        if (data->contains_anchor) {
            ...
            first_step_node->AddAlternative(GuardedAlternative(zone()->New<TextNode>(zone()->New<RegExpClassRanges>(StandardCharacterSet::kEverything), false, loop_node)));
