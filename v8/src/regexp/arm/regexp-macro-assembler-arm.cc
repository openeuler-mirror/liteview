...
int RegExpMacroAssemblerARM::stack_limit_slack() { return RegExpStack::kStackLimitSlack; }
...
void RegExpMacroAssemblerARM::CheckGreedyLoop(Label* on_equal)
{
    ...
    __ add(backtrack_stackpointer(), backtrack_stackpointer(), Operand(kPointerSize), LeaveCC, eq);
    BranchOrBacktrack(eq, on_equal);
}
...
void RegExpMacroAssemblerARM::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
    ...
    if (mode_ != LATIN1 || kTableMask != String::kMaxOneByteCharCode) {
        ...
        __ add(r1, current_character(), Operand(ByteArray::kHeaderSize - kHeapObjectTag));
    }
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerARM::Implementation() { return kARMImplementation; }

void RegExpMacroAssemblerARM::PopCurrentPosition() { Pop(current_input_offset()); }
...
void RegExpMacroAssemblerARM::PushCurrentPosition() { Push(current_input_offset()); }
...
void RegExpMacroAssemblerARM::ReadCurrentPositionFromRegister(int reg) { __ ldr(current_input_offset(), register_location(reg)); }
...
int RegExpMacroAssemblerARM::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame)
{
    Code re_code = Code::cast(Object(raw_code));
    return NativeRegExpMacroAssembler::CheckStackGuardState(frame_entry<Isolate*>(re_frame, kIsolate), frame_entry<int>(re_frame, kStartIndex),
        static_cast<RegExp::CallOrigin>(frame_entry<int>(re_frame, kDirectCall)), return_address, re_code, frame_entry_address<Address>(re_frame, kInputString),
...
void RegExpMacroAssemblerARM::SafeCall(Label* to, Condition cond) { __ bl(to, cond); }
...
void RegExpMacroAssemblerARM::Push(Register source)
{
    ...
    __ str(source, MemOperand(backtrack_stackpointer(), kPointerSize, NegPreIndex));
}

void RegExpMacroAssemblerARM::Pop(Register target)
{
    DCHECK(target != backtrack_stackpointer());
    __ ldr(target, MemOperand(backtrack_stackpointer(), kPointerSize, PostIndex));
}
