...
int RegExpMacroAssemblerRISCV::stack_limit_slack() { return RegExpStack::kStackLimitSlack; }
...
void RegExpMacroAssemblerRISCV::CheckCharacter(uint32_t c, Label* on_equal) { BranchOrBacktrack(on_equal, eq, current_character(), Operand(c)); }

void RegExpMacroAssemblerRISCV::CheckCharacterGT(base::uc16 limit, Label* on_greater) { BranchOrBacktrack(on_greater, gt, current_character(), Operand(limit)); }
...
void RegExpMacroAssemblerRISCV::CheckCharacterLT(base::uc16 limit, Label* on_less) { BranchOrBacktrack(on_less, lt, current_character(), Operand(limit)); }
...
void RegExpMacroAssemblerRISCV::CheckNotCharacter(uint32_t c, Label* on_not_equal) { BranchOrBacktrack(on_not_equal, ne, current_character(), Operand(c)); }
        ...
        if (success_label_.is_linked()) {
            ...
            if (global()) {
                ...
                if (global_with_zero_length_check()) {
                    ...
                    __ Branch(&load_char_start_regexp, ne, current_input_offset(), Operand(s3));
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerRISCV::Implementation() { return kRISCVImplementation; }

void RegExpMacroAssemblerRISCV::PopCurrentPosition() { Pop(current_input_offset()); }
...
void RegExpMacroAssemblerRISCV::PushCurrentPosition() { Push(current_input_offset()); }
...
void RegExpMacroAssemblerRISCV::ReadCurrentPositionFromRegister(int reg) { __ LoadWord(current_input_offset(), register_location(reg)); }
...
int64_t RegExpMacroAssemblerRISCV::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame)
{
...
void RegExpMacroAssemblerRISCV::SafeCall(Label* to, Condition cond, Register rs, const Operand& rt) { __ BranchAndLink(to, cond, rs, rt); }
