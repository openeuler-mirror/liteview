...
private:
    ...
    int TryOptimizeSequence(const byte* bytecode, int bytecode_length, int start_pc);
    ...
    void EmitOptimization(int start_pc, const byte* bytecode, const BytecodeSequenceNode& last_node);
    ...
    void EmitArgument(int start_pc, const byte* bytecode, BytecodeArgumentMapping arg);
    int pc() const;
    Zone* zone() const;
...
template <typename T> T GetValue(const byte* buffer, int pos)
{
...
bool BytecodeSequenceNode::IsSequence() const { return bytecode_replacement_ != kDummyBytecode; }

int BytecodeSequenceNode::SequenceLength() const { return start_offset_ + RegExpBytecodeLength(bytecode_); }

int BytecodeSequenceNode::OptimizedBytecode() const { return bytecode_replacement_; }
...
bool BytecodeSequenceNode::HasIgnoredArguments() const { return argument_ignored_ != nullptr; }
...
bool RegExpBytecodePeephole::OptimizeBytecode(const byte* bytecode, int length)
{
    int old_pc = 0;
...
void RegExpBytecodePeephole::CopyOptimizedBytecode(byte* to_address) const { MemCopy(to_address, &(*optimized_bytecode_buffer_.begin()), Length()); }
...
int RegExpBytecodePeephole::TryOptimizeSequence(const byte* bytecode, int bytecode_length, int start_pc)
{
...
void RegExpBytecodePeephole::EmitOptimization(int start_pc, const byte* bytecode, const BytecodeSequenceNode& last_node)
{
#ifdef DEBUG
    ...
    if (jump_destination_candidate != jump_usage_counts_.end() && jump_candidate_destination < start_pc + last_node.SequenceLength()) {
        ...
        for (auto jump_iter = jump_edges_.lower_bound(preserve_from); jump_iter != jump_edges_.end() && jump_iter->first /* jump source */ < start_pc + last_node.SequenceLength();
             ++jump_iter) {
...
void RegExpBytecodePeephole::CopyRangeToOutput(const byte* orig_bytecode, int start, int length)
{
    DCHECK(optimized_bytecode_buffer_.begin() + pc() == optimized_bytecode_buffer_.end());
...
void RegExpBytecodePeephole::EmitArgument(int start_pc, const byte* bytecode, BytecodeArgumentMapping arg)
{
...
Handle<ByteArray> RegExpBytecodePeepholeOptimization::OptimizeBytecode(
    Isolate* isolate, Zone* zone, Handle<String> source, const byte* bytecode, int length, const ZoneUnorderedMap<int, int>& jump_edges)
{
    ...
    if (did_optimize && v8_flags.trace_regexp_peephole_optimization) {
        ...
        RegExpBytecodeDisassemble(array->GetDataStartAddress(), peephole.Length(), source->ToCString().get());
    }
