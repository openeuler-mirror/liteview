...
int RegExpMacroAssemblerIA32::stack_limit_slack() { return RegExpStack::kStackLimitSlack; }
...
void RegExpMacroAssemblerIA32::CallIsCharacterInRangeArray(const ZoneList<CharacterRange>* ranges)
{
    ...
    __ mov(Operand(esp, 2 * kSystemPointerSize), Immediate(ExternalReference::isolate_address(isolate())));
...
void RegExpMacroAssemblerIA32::CheckBitInTable(Handle<ByteArray> table, Label* on_bit_set)
{
    ...
    __ cmpb(FieldOperand(eax, index, times_1, ByteArray::kHeaderSize), Immediate(0));
...
RegExpMacroAssembler::IrregexpImplementation RegExpMacroAssemblerIA32::Implementation() { return kIA32Implementation; }
...
void RegExpMacroAssemblerIA32::PushCurrentPosition() { Push(edi); }
...
void RegExpMacroAssemblerIA32::ReadCurrentPositionFromRegister(int reg) { __ mov(edi, register_location(reg)); }
...
Operand RegExpMacroAssemblerIA32::StaticVariable(const ExternalReference& ext) { return Operand(ext.address(), RelocInfo::EXTERNAL_REFERENCE); }
...
int RegExpMacroAssemblerIA32::CheckStackGuardState(Address* return_address, Address raw_code, Address re_frame)
{
...
void RegExpMacroAssemblerIA32::BranchOrBacktrack(Condition condition, Label* to) { __ j(condition, to ? to : &backtrack_label_); }
