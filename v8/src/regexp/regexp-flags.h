...
#define V(Lower, Camel, ...)                                                                                                                                                       \
    constexpr bool Is##Camel(RegExpFlags f) { return (f & RegExpFlag::k##Camel) != 0; }
REGEXP_FLAG_LIST(V)
#undef V

constexpr bool IsEitherUnicode(RegExpFlags f) { return IsUnicode(f) || IsUnicodeSets(f); }

// clang-format on
#define V(Lower, Camel, LowerCamel, Char, Bit) c == Char ? RegExpFlag::k##Camel:
constexpr base::Optional<RegExpFlag> TryRegExpFlagFromChar(char c) { return REGEXP_FLAG_LIST(V) base::Optional<RegExpFlag> {}; }
#undef V
// clang-format on
