...
public:
    ...
    void NewAlternative(); // '|'
    bool AddQuantifierToAtom(int min, int max, RegExpQuantifier::QuantifierType type);
    void FlushText();
    RegExpTree* ToRegExp();
...
private:
    ...
    using SmallRegExpTreeVector = base::SmallVector<RegExpTree*, 8, ZoneAllocator<RegExpTree*>>;
    SmallRegExpTreeVector terms_;
    SmallRegExpTreeVector text_;
    SmallRegExpTreeVector alternatives_;
#ifdef DEBUG
    enum { ADD_NONE, ADD_CHAR, ADD_TERM, ADD_ASSERT, ADD_ATOM } last_added_ = ADD_NONE;
...
private:
    ...
    bool AddPropertyClassRange(ZoneList<CharacterRange>* add_to, bool negate, const ZoneVector<char>& name_1, const ZoneVector<char>& name_2);
    ...
    bool TryParseCharacterClassEscape(base::uc32 next, InClassEscapeState in_class_escape_state, ZoneList<CharacterRange>* ranges, Zone* zone, bool add_unicode_case_equivalents);
    ...
    RegExpTree* ParseClassSetOperand(const RegExpBuilder* builder, ClassSetOperandType* type_out, ZoneList<CharacterRange>* ranges);
    ...
    RegExpTree* ParseClassUnion(const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand, ClassSetOperandType first_operand_type, ZoneList<CharacterRange>* ranges);
    RegExpTree* ParseClassIntersection(const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand);
    RegExpTree* ParseClassSubtraction(const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand);
    ...
    bool IsUnicodeMode() const
    {
        ...
        // TODO(v8:11935): Change permalink once proposal is in stage 4.
        // https://arai-a.github.io/ecma262-compare/snapshot.html?pr=2418#sec-parsepattern
        return IsUnicode(top_level_flags_) || IsUnicodeSets(top_level_flags_) || force_unicode_;
    }
...
RegExpParserImpl<CharT>::RegExpParserImpl(const CharT* input, int input_length, RegExpFlags flags, uintptr_t stack_limit, Zone* zone, const DisallowGarbageCollection& no_gc)
    : zone_(zone)
    , captures_(nullptr)
...
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseDisjunction()
{
    ...
    RegExpParserState initial_state(nullptr, INITIAL, RegExpLookaround::LOOKAHEAD, 0, nullptr, top_level_flags_, zone());
    ...
    while (true) {
        ...
        case ')': {
            ...
            if (group_type == CAPTURE) {
                if (state->IsNamedCapture()) {
                    CreateNamedCaptureAtIndex(state->capture_name(), capture_index CHECK_FAILED);
                }
                ...
                body = zone()->template New<RegExpLookaround>(body, is_positive, end_capture_index - capture_index, capture_index, state->lookaround_type());
            }
        ...
        case '^': {
            Advance();
            builder->AddAssertion(zone()->template New<RegExpAssertion>(builder->multiline() ? RegExpAssertion::Type::START_OF_LINE : RegExpAssertion::Type::START_OF_INPUT));
            set_contains_anchor();
            continue;
        }
        case '$': {
        ...
        case '\\':
            ...
            case '9': {
                ...
                if (is_backref) {
                    if (state->IsInsideCaptureGroup(index)) {
                        ...
                        RegExpTree* atom = zone()->template New<RegExpBackReference>(capture, builder->flags());
            ...
            case 'P': {
                ...
                bool parsed_character_class_escape = TryParseCharacterClassEscape(next, InClassEscapeState::kNotInClass, ranges, zone(), add_unicode_case_equivalents CHECK_FAILED);
...
template <class CharT> RegExpParserState* RegExpParserImpl<CharT>::ParseOpenParenthesis(RegExpParserState* state)
{
    ...
    return zone()->template New<RegExpParserState>(state, subexpr_type, lookaround_type, captures_started_, capture_name, state->builder()->flags(), zone());
}
...
template <class CharT> bool RegExpParserImpl<CharT>::CreateNamedCaptureAtIndex(const ZoneVector<base::uc16>* name, int index)
{
    ...
    if (named_captures_ == nullptr) {
        named_captures_ = zone_->template New<ZoneSet<RegExpCapture*, RegExpCaptureNameLess>>(zone());
    } else {
...
template <class CharT> bool RegExpParserImpl<CharT>::ParseNamedBackReference(RegExpBuilder* builder, RegExpParserState* state)
{
    ...
    if (state->IsInsideCaptureGroup(name)) {
        ...
        RegExpBackReference* atom = zone()->template New<RegExpBackReference>(builder->flags());
        atom->set_name(name);
...
template <class CharT> ZoneVector<RegExpCapture*>* RegExpParserImpl<CharT>::GetNamedCaptures() const
{
    if (named_captures_ == nullptr || named_captures_->empty()) {
        return nullptr;
    }

    return zone()->template New<ZoneVector<RegExpCapture*>>(named_captures_->begin(), named_captures_->end(), zone());
}
...
bool LookupPropertyValueName(UProperty property, const char* property_value_name, bool negate, bool needs_case_folding, ZoneList<CharacterRange>* result, Zone* zone)
{
    ...
    if (success) {
        ...
        for (int i = 0; i < set.getRangeCount(); i++) {
            result->Add(CharacterRange::Range(set.getRangeStart(i), set.getRangeEnd(i)), zone);
        }
    }
    return success;
}

template <size_t N> inline bool NameEquals(const char* name, const char (&literal)[N]) { return strncmp(name, literal, N + 1) == 0; }

bool LookupSpecialPropertyValueName(const char* name, ZoneList<CharacterRange>* result, bool negate, bool needs_case_folding, Zone* zone)
{
    if (NameEquals(name, "Any")) {
        ...
        return LookupPropertyValueName(UCHAR_GENERAL_CATEGORY, "Unassigned", !negate, needs_case_folding, result, zone);
    } else {
        return false;
    }
    return true;
...
template <class CharT>
bool RegExpParserImpl<CharT>::AddPropertyClassRange(ZoneList<CharacterRange>* add_to, bool negate, const ZoneVector<char>& name_1, const ZoneVector<char>& name_2)
{
    ...
    if (name_2.empty()) {
        ...
        if (LookupPropertyValueName(UCHAR_GENERAL_CATEGORY_MASK, name, negate, needs_case_folding, add_to, zone())) {
            return true;
        }
        // Interpret "Any", "ASCII", and "Assigned".
        if (LookupSpecialPropertyValueName(name, add_to, negate, needs_case_folding, zone())) {
            return true;
        }
        ...
        if (!IsExactPropertyAlias(name, property))
            return false;
        return LookupPropertyValueName(property, negate ? "N" : "Y", false, needs_case_folding, add_to, zone());
    } else {
        ...
        return LookupPropertyValueName(property, value_name, negate, needs_case_folding, add_to, zone());
    }
}

#else // V8_INTL_SUPPORT

template <class CharT> bool RegExpParserImpl<CharT>::ParsePropertyClassName(ZoneVector<char>* name_1, ZoneVector<char>* name_2) { return false; }
...
template <class CharT>
void RegExpParserImpl<CharT>::ParseClassEscape(ZoneList<CharacterRange>* ranges, Zone* zone, bool add_unicode_case_equivalents, base::uc32* char_out, bool* is_class_escape)
{
    ...
    *is_class_escape = TryParseCharacterClassEscape(next, kInClassEscape, ranges, zone, add_unicode_case_equivalents);
    if (*is_class_escape)
        return;
...
bool RegExpParserImpl<CharT>::TryParseCharacterClassEscape(
    base::uc32 next, InClassEscapeState in_class_escape_state, ZoneList<CharacterRange>* ranges, Zone* zone, bool add_unicode_case_equivalents)
{
    DCHECK_EQ(current(), '\\');
    DCHECK_EQ(Next(), next);
    ...
    case 'p':
    case 'P': {
        OutputDebugStringA("TryParseCharacterClassEscape\n");
        if (!IsUnicodeMode())
            return false;
        bool negate = next == 'P';
        Advance(2);
...
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseClassStringDisjunction()
{
    DCHECK(unicode_sets());
    DCHECK_EQ(current(), '\\');
    DCHECK_EQ(Next(), 'q');
...
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseClassSetOperand(const RegExpBuilder* builder, ClassSetOperandType* type_out)
{
    ...
    RegExpTree* tree = ParseClassSetOperand(builder, type_out, ranges CHECK_FAILED);
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kClassSetCharacter, ranges->length() == 1);
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kClassSetCharacter, tree == nullptr);
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kCharacterClassEscape, !ranges->is_empty());
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kCharacterClassEscape, tree == nullptr);
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kClassStringDisjunction, ranges->is_empty());
    DCHECK_IMPLIES(*type_out == ClassSetOperandType::kClassStringDisjunction, tree->IsAtom() || tree->IsAlternative());
...
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseClassSetOperand(const RegExpBuilder* builder, ClassSetOperandType* type_out, ZoneList<CharacterRange>* ranges)
{
    DCHECK(unicode_sets());
    const base::uc32 c = current();
    if (c == '\\') {
        ...
        if (TryParseCharacterClassEscape(next, kInClassEscape, ranges, zone(), add_unicode_case_equivalents)) {
...
RegExpTree* RegExpParserImpl<CharT>::ParseClassUnion(
    const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand, ClassSetOperandType first_operand_type, ZoneList<CharacterRange>* ranges)
{
    DCHECK(unicode_sets());
    ...
    while (has_more() && current() != ']') {
        if (current() == '-') {
            ...
            DCHECK_NE(current(), '-');
            RegExpTree* operand = ParseClassSetOperand(builder, &last_type, ranges CHECK_FAILED);
            if (operand != nullptr) {
                ...
                if (!ranges->is_empty()) {
                    ...
                    operands->Add(zone()->template New<RegExpClassRanges>(zone(), ranges), zone());
    ...
    DCHECK_EQ(current(), ']');
    Advance();
    return zone()->template New<RegExpClassSetExpression>(RegExpClassSetExpression::OperationType::kUnion, is_negated, operands);
}

// TODO(v8:11935): Change permalink once proposal is in stage 4.
// https://arai-a.github.io/ecma262-compare/snapshot.html?pr=2418#prod-ClassIntersection
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseClassIntersection(const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand)
{
    DCHECK(unicode_sets());
    DCHECK(current() == '&' && Next() == '&');
    ZoneList<RegExpTree*>* operands = zone()->template New<ZoneList<RegExpTree*>>(2, zone());
    ...
    DCHECK_EQ(current(), ']');
    Advance();
    return zone()->template New<RegExpClassSetExpression>(RegExpClassSetExpression::OperationType::kIntersection, is_negated, operands);
}

// TODO(v8:11935): Change permalink once proposal is in stage 4.
// https://arai-a.github.io/ecma262-compare/snapshot.html?pr=2418#prod-ClassSubtraction
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseClassSubtraction(const RegExpBuilder* builder, bool is_negated, RegExpTree* first_operand)
{
    DCHECK(unicode_sets());
    DCHECK(current() == '-' && Next() == '-');
    ZoneList<RegExpTree*>* operands = zone()->template New<ZoneList<RegExpTree*>>(2, zone());
    ...
    DCHECK_EQ(current(), ']');
    Advance();
    return zone()->template New<RegExpClassSetExpression>(RegExpClassSetExpression::OperationType::kSubtraction, is_negated, operands);
}
...
template <class CharT> RegExpTree* RegExpParserImpl<CharT>::ParseCharacterClass(const RegExpBuilder* builder)
{
    ...
    if (!unicode_sets()) {
        ...
        RegExpTree* operand = ParseClassSetOperand(builder, &operand_type, ranges CHECK_FAILED);
        switch (current()) {
        case '-':
            if (Next() == '-') {
                if (operand == nullptr) {
...
void RegExpBuilder::AddClassRangesForDesugaring(base::uc32 c) { AddTerm(zone()->New<RegExpClassRanges>(zone(), CharacterRange::List(zone(), CharacterRange::Singleton(c)))); }
...
bool RegExpBuilder::AddQuantifierToAtom(int min, int max, RegExpQuantifier::QuantifierType quantifier_type)
{
    FlushPendingSurrogate();
    if (pending_empty_) {
        pending_empty_ = false;
    ...
    terms_.emplace_back(zone()->New<RegExpQuantifier>(min, max, quantifier_type, atom));
    LAST(ADD_TERM);
    return true;
}
...
// static
bool RegExpParser::ParseRegExpFromHeapString(Isolate* isolate, Zone* zone, Handle<String> input, RegExpFlags flags, RegExpCompileData* result)
{
...
// static
template <class CharT>
bool RegExpParser::VerifyRegExpSyntax(
    Zone* zone, uintptr_t stack_limit, const CharT* input, int input_length, RegExpFlags flags, RegExpCompileData* result, const DisallowGarbageCollection& no_gc)
{
    return RegExpParserImpl<CharT> { input, input_length, flags, stack_limit, zone, no_gc }.Parse(result);
}

template bool RegExpParser::VerifyRegExpSyntax<uint8_t>(Zone*, uintptr_t, const uint8_t*, int, RegExpFlags, RegExpCompileData*, const DisallowGarbageCollection&);
