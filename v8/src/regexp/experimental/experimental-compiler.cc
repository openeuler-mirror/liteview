...
private:
    ...
    static bool AreSuitableFlags(RegExpFlags flags)
    {
        ...
        // future.
        static constexpr RegExpFlags kAllowedFlags = RegExpFlag::kGlobal | RegExpFlag::kSticky | RegExpFlag::kMultiline | RegExpFlag::kDotAll | RegExpFlag::kLinear;
    ...
    void* VisitClassRanges(RegExpClassRanges* node, void*) override { return nullptr; }
    ...
    void* VisitAssertion(RegExpAssertion* node, void*) override { return nullptr; }
...
bool ExperimentalRegExpCompiler::CanBeHandled(RegExpTree* tree, RegExpFlags flags, int capture_count) { return CanBeHandledVisitor::Check(tree, flags, capture_count); }
...
public:
    ...
    void Assertion(RegExpAssertion::Type t) { code_.Add(RegExpInstruction::Assertion(t), zone_); }

    void ClearRegister(int32_t register_index) { code_.Add(RegExpInstruction::ClearRegister(register_index), zone_); }

    void ConsumeRange(base::uc16 from, base::uc16 to) { code_.Add(RegExpInstruction::ConsumeRange(from, to), zone_); }

    void ConsumeAnyChar() { code_.Add(RegExpInstruction::ConsumeAnyChar(), zone_); }

    void Fork(Label& target) { LabelledInstrImpl(RegExpInstruction::Opcode::FORK, target); }

    void Jmp(Label& target) { LabelledInstrImpl(RegExpInstruction::Opcode::JMP, target); }

    void SetRegisterToCp(int32_t register_index) { code_.Add(RegExpInstruction::SetRegisterToCp(register_index), zone_); }
    ...
    void Bind(Label& target)
    {
        ...
        while (target.unbound_patch_list_begin_ != -1) {
            ...
            DCHECK(inst.opcode == RegExpInstruction::FORK || inst.opcode == RegExpInstruction::JMP);
...
private:
    ...
    void* VisitBackReference(RegExpBackReference* node, void*) override { UNREACHABLE(); }
...
ZoneList<RegExpInstruction> ExperimentalRegExpCompiler::Compile(RegExpTree* tree, RegExpFlags flags, Zone* zone) { return CompileVisitor::Compile(tree, flags, zone); }
