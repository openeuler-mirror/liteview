...
void CodeObjectRegistry::RegisterNewlyAllocatedCodeObject(Address code)
{
    ...
    if (is_sorted_) {
        is_sorted_ = (code_object_registry_.empty() || code_object_registry_.back() < code);
    }
...
bool CodeObjectRegistry::Contains(Address object) const
{
    ...
    return (std::binary_search(code_object_registry_.begin(), code_object_registry_.end(), object));
}

Address CodeObjectRegistry::GetCodeObjectStartFromInnerAddress(Address address) const
{
    base::RecursiveMutexGuard guard(&code_object_registry_mutex_);
    if (!is_sorted_) {
        std::sort(code_object_registry_.begin(), code_object_registry_.end());
    ...
    // one.
    auto it = std::upper_bound(code_object_registry_.begin(), code_object_registry_.end(), address);
