...
public:
    ReadOnlyPage(Heap* heap, BaseSpace* space, size_t chunk_size, Address area_start, Address area_end, VirtualMemory reservation);
    ...
    Address GetAreaStart() const { return address() + MemoryChunkLayout::ObjectStartOffsetInMemoryChunk(RO_SPACE); }
...
public:
    ...
    virtual void Initialize(Isolate* isolate, std::vector<ReadOnlyPage*>&& pages, const AllocationStats& stats) = 0;
    ...
    void set_shared_read_only_space(std::unique_ptr<SharedReadOnlySpace> shared_space) { shared_read_only_space_ = std::move(shared_space); }
    SharedReadOnlySpace* shared_read_only_space() { return shared_read_only_space_.get(); }
...
public:
    ...
    void Initialize(Isolate* isolate, std::vector<ReadOnlyPage*>&& pages, const AllocationStats& stats) override;
...
private:
    ...
    std::unique_ptr<v8::PageAllocator::SharedMemoryMapping> RemapPageTo(size_t i, Address new_address, ReadOnlyPage*& new_page);

    static constexpr size_t kReadOnlyRootsCount = static_cast<size_t>(RootIndex::kReadOnlyRootsCount);
...
public:
    ...
    void DetachPagesAndAddToArtifacts(std::shared_ptr<ReadOnlyArtifacts> artifacts);
...
public:
    ...
    SharedReadOnlySpace(Heap* heap, PointerCompressedReadOnlyArtifacts* artifacts);
    SharedReadOnlySpace(
