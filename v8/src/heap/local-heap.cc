...
SimpleTlsPointerXp<LocalHeap> current_local_heap;
} // namespace

LocalHeap* LocalHeap::Current() { return current_local_heap.Get(); }
    ...
    , persistent_handles_(std::move(persistent_handles))
{
        ...
        if (!is_main_thread()) {
            ...
            if (heap_->incremental_marking()->IsMarking()) {
                marking_barrier_->Activate(
                    heap_->incremental_marking()->IsCompacting(), heap_->incremental_marking()->IsMinorMarking() ? MarkingBarrierType::kMinor : MarkingBarrierType::kMajor);
            }
        }
    });
    ...
    DCHECK_NULL(current_local_heap.Get());
    if (!is_main_thread())
        current_local_heap.Set(this);
}
...
LocalHeap::~LocalHeap()
{
    ...
    heap_->safepoint()->RemoveLocalHeap(this, [this] {
        ...
        if (!is_main_thread()) {
            CodePageHeaderModificationScope rwx_write_scope("Publishing of marking barrier results for Code space pages requires "
    ...
    if (!is_main_thread()) {
        DCHECK_EQ(current_local_heap.Get(), this);
        current_local_heap.Set(nullptr);
    }
...
void LocalHeap::SetUp()
{
    ...
    old_space_allocator_ = std::make_unique<ConcurrentAllocator>(this, heap_->old_space());

    DCHECK_NULL(code_space_allocator_);
    code_space_allocator_ = std::make_unique<ConcurrentAllocator>(this, heap_->code_space());

    DCHECK_NULL(shared_old_space_allocator_);
    if (heap_->isolate()->has_shared_heap()) {
        shared_old_space_allocator_ = std::make_unique<ConcurrentAllocator>(this, heap_->shared_allocation_space());
    }
...
{
    if (!persistent_handles_) {
        persistent_handles_.reset(heap_->isolate()->NewPersistentHandles().release());
        persistent_handles_->Attach(this);
    }
}
...
#ifdef DEBUG
bool LocalHeap::ContainsPersistentHandle(Address* location) { return persistent_handles_ ? persistent_handles_->Contains(location) : false; }

bool LocalHeap::ContainsLocalHandle(Address* location) { return handles_ ? handles_->Contains(location) : false; }
...
void LocalHeap::EnsureParkedBeforeDestruction() { DCHECK_IMPLIES(!is_main_thread(), IsParked()); }
...
Address LocalHeap::PerformCollectionAndAllocateAgain(int object_size, AllocationType type, AllocationOrigin origin, AllocationAlignment alignment)
{
    ...
    if (v8_flags.trace_gc) {
        heap_->isolate()->PrintWithTimestamp("Background allocation failure: "
                                             "allocations=%d"
...
void LocalHeap::AddGCEpilogueCallback(GCEpilogueCallback* callback, void* data, GCType gc_type)
{
    DCHECK(!IsParked());
    gc_epilogue_callbacks_.Add(callback, LocalIsolate::FromHeap(this), gc_type, data);
}
...
void LocalHeap::InvokeGCEpilogueCallbacksInSafepoint(GCType gc_type, GCCallbackFlags flags) { gc_epilogue_callbacks_.Invoke(gc_type, flags); }

void LocalHeap::NotifyObjectSizeChange(
    HeapObject object, int old_size, int new_size, ClearRecordedSlots clear_recorded_slots, UpdateInvalidatedObjectSize update_invalidated_object_size)
{
    heap()->NotifyObjectSizeChange(object, old_size, new_size, clear_recorded_slots, update_invalidated_object_size);
}
