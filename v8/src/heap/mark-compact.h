...
template <LiveObjectIterationMode mode> class LiveObjectRange {
public:
    class iterator {
    public:
        ...
        bool operator==(iterator other) const { return current_object_ == other.current_object_; }

        bool operator!=(iterator other) const { return !(*this == other); }

        value_type operator*() { return std::make_pair(current_object_, current_size_); }
...
public:
    ...
    // an object.
    template <class Visitor, typename MarkingState> static bool VisitBlackObjects(MemoryChunk* chunk, MarkingState* state, Visitor* visitor, HeapObject* failed_object);

    // Visits black objects on a MemoryChunk. The visitor is not allowed to fail
    // visitation for an object.
    template <class Visitor, typename MarkingState> static void VisitBlackObjectsNoFail(MemoryChunk* chunk, MarkingState* state, Visitor* visitor);

    template <typename MarkingState> static void RecomputeLiveBytes(MemoryChunk* chunk, MarkingState* state);
};
...
template <typename MarkingState> class MainMarkingVisitor final : public MarkingVisitorBase<MainMarkingVisitor<MarkingState>, MarkingState> {
public:
    MainMarkingVisitor(MarkingState* marking_state, MarkingWorklists::Local* local_marking_worklists, WeakObjects::Local* local_weak_objects, Heap* heap,
        unsigned mark_compact_epoch, base::EnumSet<CodeFlushMode> code_flush_mode, bool embedder_tracing_enabled, bool should_keep_ages_unchanged)
        : MarkingVisitorBase<MainMarkingVisitor<MarkingState>, MarkingState>(
            local_marking_worklists, local_weak_objects, heap, mark_compact_epoch, code_flush_mode, embedder_tracing_enabled, should_keep_ages_unchanged)
...
private:
    ...
    TraceRetainingPathMode retaining_path_mode() { return (V8_UNLIKELY(v8_flags.track_retaining_path)) ? TraceRetainingPathMode::kEnabled : TraceRetainingPathMode::kDisabled; }

    MarkingState* const marking_state_;

    friend class MarkingVisitorBase<MainMarkingVisitor<MarkingState>, MarkingState>;
};

class YoungGenerationMainMarkingVisitor final : public YoungGenerationMarkingVisitorBase<YoungGenerationMainMarkingVisitor, MarkingState> {
public:
    YoungGenerationMainMarkingVisitor(Isolate* isolate, MarkingState* marking_state, MarkingWorklists::Local* worklists_local);
...
private:
    ...
    friend class YoungGenerationMarkingVisitorBase<YoungGenerationMainMarkingVisitor, MarkingState>;
};
...
public:
    ...
    MarkingWorklists::Local* local_marking_worklists() { return local_marking_worklists_.get(); }
    ...
    virtual std::pair<size_t, size_t> ProcessMarkingWorklist(size_t bytes_to_process) = 0;
...
public:
    ...
    static MarkCompactCollector* From(CollectorBase* collector) { return static_cast<MarkCompactCollector*>(collector); }

    std::pair<size_t, size_t> ProcessMarkingWorklist(size_t bytes_to_process) final;

    std::pair<size_t, size_t> ProcessMarkingWorklist(size_t bytes_to_process, MarkingWorklistProcessingMode mode);
    ...
    static inline bool IsOnEvacuationCandidate(Object obj) { return Page::FromAddress(obj.ptr())->IsEvacuationCandidate(); }
    ...
    static bool ShouldRecordRelocSlot(Code host, RelocInfo* rinfo, HeapObject target);
    static RecordRelocSlotInfo ProcessRelocInfo(Code host, RelocInfo* rinfo, HeapObject target);

    static void RecordRelocSlot(Code host, RelocInfo* rinfo, HeapObject target);
    V8_INLINE static void RecordSlot(HeapObject object, ObjectSlot slot, HeapObject target);
    V8_INLINE static void RecordSlot(HeapObject object, HeapObjectSlot slot, HeapObject target);
    V8_INLINE static void RecordSlot(MemoryChunk* source_page, HeapObjectSlot slot, HeapObject target);
    ...
    base::EnumSet<CodeFlushMode> code_flush_mode() const { return code_flush_mode_; }
    ...
    std::unique_ptr<UpdatingItem> CreateRememberedSetUpdatingItem(MemoryChunk* chunk, RememberedSetUpdatingMode updating_mode);
...
private:
    ...
    void MarkRoots(RootVisitor* root_visitor, ObjectVisitor* custom_root_body_visitor);
    ...
    bool CompactTransitionArray(Map map, TransitionArray transitions, DescriptorArray descriptors);
    bool TransitionArrayNeedsCompaction(TransitionArray transitions, int num_transitions);
    ...
    void ReportAbortedEvacuationCandidateDueToOOM(Address failed_start, Page* page);
    void ReportAbortedEvacuationCandidateDueToFlags(Address failed_start, Page* page);
...
public:
    ...
    static MinorMarkCompactCollector* From(CollectorBase* collector) { return static_cast<MinorMarkCompactCollector*>(collector); }
...
private:
    ...
    void MarkLiveObjects();
    void MarkRootSetInParallel(RootMarkingVisitor* root_visitor, bool was_marked_incrementally);
