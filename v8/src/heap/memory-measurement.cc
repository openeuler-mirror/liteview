...
public:
    ...
    void AddTotal(size_t estimate, size_t lower_bound, size_t upper_bound) { AddProperty(result_, factory_->total_string(), NewResult(estimate, lower_bound, upper_bound)); }
...
private:
    ...
    Handle<Object> NewNumber(size_t value) { return factory_->NewNumberFromSize(value); }
    Handle<JSObject> NewJSObject() { return factory_->NewJSObject(isolate_->object_function()); }
    ...
    void AddProperty(Handle<JSObject> object, Handle<String> name, Handle<Object> value) { JSObject::AddProperty(isolate_, object, name, value, NONE); }
    Isolate* isolate_;
    Factory* factory_;
...
public:
    MeasureMemoryDelegate(Isolate* isolate, Handle<NativeContext> context, Handle<JSPromise> promise, v8::MeasureMemoryMode mode);
...
MeasureMemoryDelegate::MeasureMemoryDelegate(Isolate* isolate, Handle<NativeContext> context, Handle<JSPromise> promise, v8::MeasureMemoryMode mode)
    : isolate_(isolate)
    , mode_(mode)
{
...
{
    Handle<NativeContext> native_context = Handle<NativeContext>::cast(Utils::OpenHandle(*context));
    return context_->security_token() == native_context->security_token();
}

void MeasureMemoryDelegate::MeasurementComplete(const std::vector<std::pair<v8::Local<v8::Context>, size_t>>& context_sizes_in_bytes, size_t shared_size)
{
    v8::Local<v8::Context> v8_context = Utils::Convert<HeapObject, v8::Context>(context_);
...
bool MemoryMeasurement::EnqueueRequest(std::unique_ptr<v8::MeasureMemoryDelegate> delegate, v8::MeasureMemoryExecution execution, const std::vector<Handle<NativeContext>> contexts)
{
    int length = static_cast<int>(contexts.size());
    Handle<WeakFixedArray> weak_contexts = isolate_->factory()->NewWeakFixedArray(length);
    for (int i = 0; i < length; ++i) {
    ...
    Request request = { std::move(delegate), global_weak_contexts, std::vector<size_t>(length), 0u, {} };
    request.timer.Start();
...
void MemoryMeasurement::ScheduleReportingTask()
{
    ...
    auto taskrunner = V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate_));
    ...
    auto task = MakeCancelableTask(isolate_, [this, execution] {
        ...
        if (v8_flags.incremental_marking) {
            if (heap->incremental_marking()->IsStopped()) {
                heap->StartIncrementalMarking(Heap::kNoGCFlags, GarbageCollectionReason::kMeasureMemory);
            } else {
                if (execution == v8::MeasureMemoryExecution::kEager) {
...
int MemoryMeasurement::NextGCTaskDelayInSeconds() { return kGCTaskDelayInSeconds + random_number_generator_.NextInt(kGCTaskDelayInSeconds); }
...
void MemoryMeasurement::ReportResults()
{
    while (!done_.empty()) {
        ...
        for (int i = 0; i < request.contexts->length(); i++) {
            ...
            v8::Local<v8::Context> context = Utils::Convert<HeapObject, v8::Context>(handle(raw_context, isolate_));
...
std::unique_ptr<v8::MeasureMemoryDelegate> MemoryMeasurement::DefaultDelegate(
    Isolate* isolate, Handle<NativeContext> context, Handle<JSPromise> promise, v8::MeasureMemoryMode mode)
{
    return std::make_unique<MeasureMemoryDelegate>(isolate, context, promise, mode);
}

bool NativeContextInferrer::InferForContext(Isolate* isolate, Context context, Address* native_context)
{
    PtrComprCageBase cage_base(isolate);
    Map context_map = context.map(cage_base, kAcquireLoad);
    Object maybe_native_context = TaggedField<Object, Map::kConstructorOrBackPointerOrNativeContextOffset>::Acquire_Load(cage_base, context_map);
...
bool NativeContextInferrer::InferForJSFunction(Isolate* isolate, JSFunction function, Address* native_context)
{
    Object maybe_context = TaggedField<Object, JSFunction::kContextOffset>::Acquire_Load(isolate, function);
...
bool NativeContextInferrer::InferForJSObject(Isolate* isolate, Map map, JSObject object, Address* native_context)
{
    if (map.instance_type() == JS_GLOBAL_OBJECT_TYPE) {
        Object maybe_context = JSGlobalObject::cast(object).native_context_unchecked(isolate);
    ...
    if (maybe_constructor.IsJSFunction()) {
        return InferForJSFunction(isolate, JSFunction::cast(maybe_constructor), native_context);
    }
    return false;
}
...
void NativeContextStats::IncrementExternalSize(Address context, Map map, HeapObject object)
{
