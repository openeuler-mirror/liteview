...
class V8_EXPORT_PRIVATE DefaultEmbedderRootsHandler final : public EmbedderRootsHandler {
public:
...
public:
    ...
    static V8_INLINE bool ExtractWrappableInfo(Isolate*, JSObject, const WrapperDescriptor&, WrapperInfo*);
    static V8_INLINE bool ExtractWrappableInfo(Isolate*, const WrapperDescriptor&, const EmbedderDataSlot& type_slot, const EmbedderDataSlot& instance_slot, WrapperInfo*);
    ...
    bool ShouldFinalizeIncrementalMarking()
    {
        ...
        if (!SupportsIncrementalEmbedderSteps())
            return true;
    ...
    bool SupportsIncrementalEmbedderSteps() const
    {
        ...
        return cpp_heap_ ? v8_flags.cppheap_incremental_marking : v8_flags.incremental_marking_wrappers;
    }

    void SetEmbedderWorklistEmpty(bool is_empty) { embedder_worklist_empty_ = is_empty; }

    void IncreaseAllocatedSize(size_t bytes)
    {
        remote_stats_.used_size.fetch_add(bytes, std::memory_order_relaxed);
        remote_stats_.allocated_size += bytes;
        if (remote_stats_.allocated_size > remote_stats_.allocated_size_limit_for_check) {
            StartIncrementalMarkingIfNeeded();
            remote_stats_.allocated_size_limit_for_check = remote_stats_.allocated_size + kEmbedderAllocatedThreshold;
        }
    }
    ...
    size_t used_size() const { return remote_stats_.used_size.load(std::memory_order_relaxed); }
    ...
    DefaultEmbedderRootsHandler& default_embedder_roots_handler() { return default_embedder_roots_handler_; }

    EmbedderHeapTracer::EmbedderStackState embedder_stack_state() const { return embedder_stack_state_; }
...
private:
    ...
    static constexpr WrapperDescriptor::InternalFieldIndex kDefaultWrapperTypeEmbedderIndex = 0;
    static constexpr WrapperDescriptor::InternalFieldIndex kDefaultWrapperInstanceEmbedderIndex = 1;
    ...
    EmbedderHeapTracer::EmbedderStackState embedder_stack_state_ = EmbedderHeapTracer::EmbedderStackState::kMayContainHeapPointers;
