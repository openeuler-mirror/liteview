...
void IncrementalMarkingJob::ScheduleTask()
{
    ...
    if (is_task_pending_ || heap_->IsTearingDown() || !v8_flags.incremental_marking_task) {
        return;
    }
    ...
    const auto stack_state = taskrunner->NonNestableTasksEnabled() ? StackState::kNoHeapPointers : StackState::kMayContainHeapPointers;
...
void IncrementalMarkingJob::Task::RunInternal()
{
    ...
    EmbedderStackStateScope scope(heap, EmbedderStackStateScope::kImplicitThroughTask, stack_state_);

    heap->tracer()->RecordTimeToIncrementalMarkingTask(heap->MonotonicallyIncreasingTimeInMs() - job_->scheduled_time_);
