...
void PretenturingHandler::MergeAllocationSitePretenuringFeedback(const PretenuringFeedbackMap& local_pretenuring_feedback)
{
...
bool PretenturingHandler::DeoptMaybeTenuredAllocationSites() const
{
    ...
    return new_space && new_space->IsAtMaximumCapacity() && !heap_->MaximumSizeMinorGC();
}

namespace {

inline bool MakePretenureDecision(AllocationSite site, AllocationSite::PretenureDecision current_decision, double ratio, bool maximum_size_scavenge)
{
...
inline bool DigestPretenuringFeedback(Isolate* isolate, AllocationSite site, bool maximum_size_scavenge)
{
    bool deopt = false;
    ...
    if (minimum_mementos_created) {
        deopt = MakePretenureDecision(site, current_decision, ratio, maximum_size_scavenge);
    }
...
void PretenturingHandler::RemoveAllocationSitePretenuringFeedback(AllocationSite site) { global_pretenuring_feedback_.erase(site); }
    ...
    if (v8_flags.allocation_site_pretenuring) {
        ...
        for (auto& site_and_count : global_pretenuring_feedback_) {
            ...
            if (found_count > 0) {
                ...
                if (DigestPretenuringFeedback(heap_->isolate(), site, maximum_size_scavenge)) {
        ...
        if (deopt_maybe_tenured) {
            heap_->ForeachAllocationSite(heap_->allocation_sites_list(), [&allocation_sites, &trigger_deoptimization](AllocationSite site) {
                DCHECK(site.IsAllocationSite());
                allocation_sites++;
        ...
        if (v8_flags.trace_pretenuring_statistics && (allocation_mementos_found > 0 || tenure_decisions > 0 || dont_tenure_decisions > 0)) {
            PrintIsolate(heap_->isolate(),
                "pretenuring: deopt_maybe_tenured=%d visited_sites=%d "
                "active_sites=%d "
                "mementos=%d tenured=%d not_tenured=%d\n",
                deopt_maybe_tenured ? 1 : 0, allocation_sites, active_allocation_sites, allocation_mementos_found, tenure_decisions, dont_tenure_decisions);
        }
...
void PretenturingHandler::PretenureAllocationSiteOnNextCollection(AllocationSite site)
{
    if (!allocation_sites_to_pretenure_) {
