...
Address ObjectStartBitmap::FindBasePtrImpl(Address maybe_inner_ptr) const
{
    ...
    uint32_t byte = load(cell_index) & static_cast<uint32_t>((1 << (bit + 1)) - 1);
...
void ObjectStartBitmap::store(size_t cell_index, uint32_t value) { object_start_bit_map_[cell_index] = value; }

uint32_t ObjectStartBitmap::load(size_t cell_index) const { return object_start_bit_map_[cell_index]; }

void ObjectStartBitmap::ObjectStartIndexAndBit(Address base_ptr, size_t* cell_index, size_t* bit) const
{
...
Address ObjectStartBitmap::StartIndexToAddress(size_t object_start_index) const { return offset() + (kAllocationGranularity * object_start_index); }
...
template <typename Callback> inline void ObjectStartBitmap::Iterate(Callback callback) const
{
    for (size_t cell_index = 0; cell_index < kReservedForBitmap; cell_index++) {
        ...
        while (value != 0) {
            const int trailing_zeroes = v8::base::bits::CountTrailingZerosNonZero(value);
...
void ObjectStartBitmap::Clear() { std::fill(object_start_bit_map_.begin(), object_start_bit_map_.end(), 0); }
...
void ObjectStartBitmap::Verify() const
{
    ...
    Iterate([&next_object_in_page, cage](Address next_object_in_bitmap) {
        while (next_object_in_page != next_object_in_bitmap) {
            ...
            next_object_in_page += HeapObject::FromAddress(next_object_in_page).Size(cage);
        }
    });
}
#endif // VERIFY_HEAP
