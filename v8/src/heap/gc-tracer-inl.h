    ...
    , start_time_(tracer_->MonotonicallyIncreasingTimeInMs())
{
    ...
    if (thread_kind_ == ThreadKind::kMain) {
        DCHECK(tracer_->heap_->IsMainThread() || tracer_->heap_->IsSharedMainThread());
...
GCTracer::Scope::~Scope()
{
    ...
    if (thread_kind_ == ThreadKind::kMain) {
        ...
        if (scope_ == ScopeId::MC_INCREMENTAL || scope_ == ScopeId::MC_INCREMENTAL_START || scope_ == ScopeId::MC_INCREMENTAL_FINALIZE) {
            ...
            long_task_stats->gc_full_incremental_wall_clock_duration_us += static_cast<int64_t>(duration_ms * base::Time::kMicrosecondsPerMillisecond);
        }
    }
...
constexpr bool GCTracer::Event::IsYoungGenerationEvent(Type type)
{
    ...
    return type == SCAVENGER || type == MINOR_MARK_COMPACTOR || type == INCREMENTAL_MINOR_MARK_COMPACTOR;
}

CollectionEpoch GCTracer::CurrentEpoch(Scope::ScopeId id) const { return Scope::NeedsYoungEpoch(id) ? epoch_young_ : epoch_full_; }

#ifdef DEBUG
bool GCTracer::IsInObservablePause() const { return 0.0 < start_of_observable_pause_; }

bool GCTracer::IsInAtomicPause() const { return current_.state == Event::State::ATOMIC; }

bool GCTracer::IsConsistentWithCollector(GarbageCollector collector) const
{
    return (collector == GarbageCollector::SCAVENGER && current_.type == Event::SCAVENGER)
        || (collector == GarbageCollector::MINOR_MARK_COMPACTOR && (current_.type == Event::MINOR_MARK_COMPACTOR || current_.type == Event::INCREMENTAL_MINOR_MARK_COMPACTOR))
        || (collector == GarbageCollector::MARK_COMPACTOR && (current_.type == Event::MARK_COMPACTOR || current_.type == Event::INCREMENTAL_MARK_COMPACTOR));
}

bool GCTracer::IsSweepingInProgress() const
{
    return (current_.type == Event::MARK_COMPACTOR || current_.type == Event::INCREMENTAL_MARK_COMPACTOR) && current_.state == Event::State::SWEEPING;
}
#endif
...
constexpr const GCTracer::IncrementalMarkingInfos& GCTracer::incremental_scope(Scope::ScopeId id) const { return incremental_scopes_[Scope::IncrementalOffset(id)]; }
...
#ifdef V8_RUNTIME_CALL_STATS
WorkerThreadRuntimeCallStats* GCTracer::worker_thread_runtime_call_stats() { return heap_->isolate_->counters()->worker_thread_runtime_call_stats(); }
...
double GCTracer::MonotonicallyIncreasingTimeInMs()
{
    if (V8_UNLIKELY(v8_flags.predictable)) {
        ...
        return base::TimeTicks::Now().ToInternalValue() / static_cast<double>(base::Time::kMicrosecondsPerMillisecond);
    }
}
