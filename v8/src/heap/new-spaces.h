...
public:
    ...
    Page* first_page() final { return reinterpret_cast<Page*>(memory_chunk_list_.front()); }
    Page* last_page() final { return reinterpret_cast<Page*>(memory_chunk_list_.back()); }

    const Page* first_page() const final { return reinterpret_cast<const Page*>(memory_chunk_list_.front()); }
    const Page* last_page() const final { return reinterpret_cast<const Page*>(memory_chunk_list_.back()); }
...
public:
    ...
    bool IsAtMaximumCapacity() const { return TotalCapacity() == MaximumCapacity(); }

    size_t ExternalBackingStoreOverallBytes() const
    {
        size_t result = 0;
        for (int i = 0; i < ExternalBackingStoreType::kNumTypes; i++) {
            result += ExternalBackingStoreBytes(static_cast<ExternalBackingStoreType>(i));
        }
        return result;
    }
    ...
    void VerifyImpl(Isolate* isolate, const Page* current_page, Address current_address) const;
#endif
...
public:
    ...
    SemiSpaceNewSpace(Heap* heap, size_t initial_semispace_capacity, size_t max_semispace_capacity, LinearAllocationArea& allocation_info);
    ...
    size_t Size() const final
    {
        ...
        return (to_space_.current_capacity() - Page::kPageSize) / Page::kPageSize * MemoryChunkLayout::AllocatableMemoryInDataPage()
    ...
    size_t Capacity() const final
    {
        ...
        return (to_space_.target_capacity() / Page::kPageSize) * MemoryChunkLayout::AllocatableMemoryInDataPage();
    }
    ...
    // combined.
    size_t CommittedMemory() const final { return from_space_.CommittedMemory() + to_space_.CommittedMemory(); }

    size_t MaximumCommittedMemory() const final { return from_space_.MaximumCommittedMemory() + to_space_.MaximumCommittedMemory(); }
    ...
    Address first_allocatable_address() const final { return to_space_.space_start(); }
...
private:
    ...
    bool EnsureAllocation(int size_in_bytes, AllocationAlignment alignment, AllocationOrigin origin, int* out_max_aligned_size) final;
...
public:
    ...
    // from OS.
    explicit PagedSpaceForNewSpace(Heap* heap, size_t initial_capacity, size_t max_capacity, AllocationCounter& allocation_counter, LinearAllocationArea& allocation_info,
    ...
    // Return the address of the first allocatable address in the active
    // semispace. This may be the address where the first object resides.
    Address first_allocatable_address() const { return first_page()->area_start(); }
...
public:
    ...
    PagedNewSpace(Heap* heap, size_t initial_capacity, size_t max_capacity, LinearAllocationArea& allocation_info);

    ~PagedNewSpace() final;

    bool ContainsSlow(Address a) const final { return paged_space_.ContainsSlow(a); }
    ...
    size_t CommittedMemory() const final { return paged_space_.CommittedMemory(); }

    size_t MaximumCommittedMemory() const final { return paged_space_.MaximumCommittedMemory(); }

    // Approximate amount of physical memory committed for this space.
    size_t CommittedPhysicalMemory() const final { return paged_space_.CommittedPhysicalMemory(); }

    // Return the available bytes without growing.
    size_t Available() const final { return paged_space_.Available() + limit() - top(); }

    size_t ExternalBackingStoreBytes(ExternalBackingStoreType type) const final { return paged_space_.ExternalBackingStoreBytes(type); }

    size_t AllocatedSinceLastGC() const final { return paged_space_.AllocatedSinceLastGC(); }

    // Return the maximum capacity of the space.
    size_t MaximumCapacity() const final { return paged_space_.MaximumCapacity(); }
    ...
    // Return the address of the first allocatable address in the active
    // semispace. This may be the address where the first object resides.
    Address first_allocatable_address() const final { return paged_space_.first_allocatable_address(); }
    ...
    // allocation_info_.limit to be lower than the actual limit and and increasing
    // it in steps to guarantee that the observers are notified periodically.
    void UpdateInlineAllocationLimit(size_t size_in_bytes) final { paged_space_.UpdateInlineAllocationLimit(size_in_bytes); }
    ...
    void FreeLinearAllocationArea() final { paged_space_.FreeLinearAllocationArea(); }
    ...
    std::unique_ptr<ObjectIterator> GetObjectIterator(Heap* heap) final { return paged_space_.GetObjectIterator(heap); }
    ...
    bool IsPromotionCandidate(const MemoryChunk* page) const final { return true; }

    bool EnsureCurrentCapacity() final { return paged_space_.EnsureCurrentCapacity(); }

    void MakeLinearAllocationAreaIterable() final { paged_space_.MakeLinearAllocationAreaIterable(); }
...
#ifdef V8_ENABLE_INNER_POINTER_RESOLUTION_OSB
    void ClearUnusedObjectStartBitmaps() override { paged_space_.ClearUnusedObjectStartBitmaps(); }
#endif // V8_ENABLE_INNER_POINTER_RESOLUTION_OSB
...
private:
    bool EnsureAllocation(int size_in_bytes, AllocationAlignment alignment, AllocationOrigin origin, int* out_max_aligned_size) final
    {
        return paged_space_.EnsureAllocation(size_in_bytes, alignment, origin, out_max_aligned_size);
    }
