...
template <typename T> T ForwardingAddress(T heap_obj)
{
...
bool Heap::IsMainThread() const { return isolate()->thread_id() == ThreadId::Current(); }
...
int64_t Heap::update_external_memory(int64_t delta) { return external_memory_.Update(delta); }

PagedSpace* Heap::space_for_maps() { return V8_LIKELY(map_space_) ? static_cast<PagedSpace*>(map_space_) : static_cast<PagedSpace*>(old_space_); }

ConcurrentAllocator* Heap::concurrent_allocator_for_maps() { return V8_LIKELY(shared_map_allocator_) ? shared_map_allocator_.get() : shared_space_allocator_.get(); }

RootsTable& Heap::roots_table() { return isolate()->roots_table(); }

#define ROOT_ACCESSOR(Type, name, CamelName)                                                                                                                                       \
    Type Heap::name() { return Type::cast(Object(roots_table()[RootIndex::k##CamelName])); }
MUTABLE_ROOT_LIST(ROOT_ACCESSOR)
#undef ROOT_ACCESSOR

FixedArray Heap::single_character_string_table() { return FixedArray::cast(Object(roots_table()[RootIndex::kSingleCharacterStringTable])); }
...
#define ROOT_ACCESSOR(type, name, CamelName)                                                                                                                                       \
    ...
    {                                                                                                                                                                              \
        ...
        DCHECK_IMPLIES(RootsTable::IsImmortalImmovable(RootIndex::k##CamelName), IsImmovable(HeapObject::cast(value)));                                                            \
        roots_table()[RootIndex::k##CamelName] = value.ptr();                                                                                                                      \
    }
ROOT_LIST(ROOT_ACCESSOR)
#undef ROOT_ACCESSOR

void Heap::SetRootMaterializedObjects(FixedArray objects) { roots_table()[RootIndex::kMaterializedObjects] = objects.ptr(); }

void Heap::SetRootScriptList(Object value) { roots_table()[RootIndex::kScriptList] = value.ptr(); }

void Heap::SetMessageListeners(TemplateList value) { roots_table()[RootIndex::kMessageListeners] = value.ptr(); }
...
{
    DCHECK(idx == OLD_SPACE || idx == CODE_SPACE || idx == MAP_SPACE || idx == SHARED_SPACE);
    return static_cast<PagedSpace*>(space_[idx].get());
}

Space* Heap::space(int idx) { return space_[idx].get(); }

Address* Heap::NewSpaceAllocationTopAddress() { return new_space_ ? new_space_->allocation_top_address() : nullptr; }

Address* Heap::NewSpaceAllocationLimitAddress() { return new_space_ ? new_space_->allocation_limit_address() : nullptr; }

Address* Heap::OldSpaceAllocationTopAddress() { return old_space_->allocation_top_address(); }

Address* Heap::OldSpaceAllocationLimitAddress() { return old_space_->allocation_limit_address(); }
...
Address Heap::code_range_base() { return code_range_ ? code_range_->base() : kNullAddress; }

int Heap::MaxRegularHeapObjectSize(AllocationType allocation)
{
    if (!V8_ENABLE_THIRD_PARTY_HEAP_BOOL && (allocation == AllocationType::kCode)) {
...
Address Heap::NewSpaceTop() { return new_space_ ? new_space_->top() : kNullAddress; }
...
bool Heap::InYoungGeneration(HeapObject heap_object)
{
    if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL)
        return false;
    bool result = BasicMemoryChunk::FromHeapObject(heap_object)->InYoungGeneration();
#ifdef DEBUG
...
{
    HeapObject heap_object;
    return object->GetHeapObject(&heap_object) && heap_object.InSharedWritableHeap();
}
...
// static
bool Heap::InFromPage(HeapObject heap_object) { return BasicMemoryChunk::FromHeapObject(heap_object)->IsFromPage(); }
...
// static
bool Heap::InToPage(HeapObject heap_object) { return BasicMemoryChunk::FromHeapObject(heap_object)->IsToPage(); }

bool Heap::InOldSpace(Object object)
{
    if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL) {
        return object.IsHeapObject() && third_party_heap::Heap::InOldSpace(object.ptr());
    }
...
bool Heap::IsPendingAllocationInternal(HeapObject object)
{
    ...
    case NEW_SPACE: {
        base::SharedMutexGuard<base::kShared> guard(new_space_->linear_area_lock());
    ...
    case MAP_SPACE: {
        ...
        base::SharedMutexGuard<base::kShared> guard(paged_space->linear_area_lock());
...
bool Heap::IsPendingAllocation(Object object) { return object.IsHeapObject() && IsPendingAllocation(HeapObject::cast(object)); }
...
int Heap::NextScriptId()
{
    ...
    do {
        ...
        last_id = Smi::cast(last_script_id_slot.Relaxed_CompareAndSwap(last_id, new_id));
...
int Heap::MaxNumberToStringCacheSize() const
{
    ...
    number_string_cache_size = std::max(static_cast<size_t>(kInitialNumberStringCacheSize * 2), std::min(static_cast<size_t>(0x4000), number_string_cache_size));
...
bool Heap::HasDirtyJSFinalizationRegistries() { return !dirty_js_finalization_registries_list().IsUndefined(isolate()); }
...
AlwaysAllocateScope::~AlwaysAllocateScope() { heap_->always_allocate_scope_count_--; }
...
void Heap::IncrementCodePageCollectionMemoryModificationScopeDepth()
{
    ...
    DCHECK_LE(local_heap->code_page_collection_memory_modification_scope_depth_, kMaxCodePageCollectionMemoryModificationScopeDepth);
#endif
}
...
PagedNewSpace* Heap::paged_new_space() const { return PagedNewSpace::From(new_space()); }
...
CodePageCollectionMemoryModificationScope::CodePageCollectionMemoryModificationScope(Heap* heap)
    :
#if V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT || V8_HEAP_USE_PKU_JIT_WRITE_PROTECT
...
CodePageMemoryModificationScope::CodePageMemoryModificationScope(BasicMemoryChunk* chunk)
    :
#if V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT || V8_HEAP_USE_PKU_JIT_WRITE_PROTECT
    rwx_write_scope_("A part of CodePageMemoryModificationScope")
    ,
