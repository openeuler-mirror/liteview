...
public:
    FieldStatsCollector(Heap* heap, size_t* tagged_fields_count, size_t* embedder_fields_count, size_t* inobject_smi_fields_count, size_t* boxed_double_fields_count,
    ...
    void RecordStats(HeapObject host)
    {
        ...
        if (host.IsJSObject(cage_base())) {
            ...
            *boxed_double_fields_count_ += 1;
            ...
            int string_data = SeqString::cast(host).length(kAcquireLoad) * (String::cast(host).IsOneByteRepresentation() ? 1 : 2) / kTaggedSize;
    ...
    void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) override { *tagged_fields_count_ += (end - start); }
    void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) override { *tagged_fields_count_ += (end - start); }

    V8_INLINE void VisitCodePointer(HeapObject host, CodeObjectSlot slot) override
    {
    ...
    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) override { *tagged_fields_count_ += 1; }
...
private:
    ...
    std::unordered_map<Map, JSObjectFieldStats, Object::Hasher> field_stats_cache_;
...
FieldStatsCollector::JSObjectFieldStats FieldStatsCollector::GetInobjectFieldStats(Map map)
{
...
V8_NOINLINE static void DumpJSONArray(std::stringstream& stream, size_t* array, const int len) { stream << PrintCollection(base::Vector<size_t>(array, len)); }

void ObjectStats::PrintKeyAndId(const char* key, int gc_count) { PrintF("\"isolate\": \"%p\", \"id\": %d, \"key\": \"%s\", ", reinterpret_cast<void*>(isolate()), gc_count, key); }
...
public:
    ...
    void CollectStatistics(HeapObject obj, Phase phase, CollectFieldStats collect_field_stats);
...
private:
    ...
    bool RecordVirtualObjectStats(
        HeapObject parent, HeapObject obj, ObjectStats::VirtualInstanceType type, size_t size, size_t over_allocated, CowMode check_cow_array = kCheckCow);
    ...
    bool RecordSimpleVirtualObjectStats(HeapObject parent, HeapObject obj, ObjectStats::VirtualInstanceType type);
    // For HashTable it is possible to compute over allocated memory.
    template <typename Derived, typename Shape>
    void RecordHashTableVirtualObjectStats(HeapObject parent, HashTable<Derived, Shape> hash_table, ObjectStats::VirtualInstanceType type);
    ...
    void RecordObjectStats(HeapObject obj, InstanceType type, size_t size, size_t over_allocated = ObjectStats::kNoOverAllocation);

    // Specific recursion into constant pool or embedded code objects. Records
    // FixedArrays and Tuple2.
    void RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(HeapObject parent, HeapObject object, ObjectStats::VirtualInstanceType type);
    ...
    void RecordVirtualArrayBoilerplateDescription(ArrayBoilerplateDescription description);

    PtrComprCageBase cage_base() const { return field_stats_collector_.cage_base(); }

    Heap* const heap_;
    ObjectStats* const stats_;
    NonAtomicMarkingState* const marking_state_;
    std::unordered_set<HeapObject, Object::Hasher, Object::KeyEqualSafe> virtual_objects_;
...
ObjectStatsCollectorImpl::ObjectStatsCollectorImpl(Heap* heap, ObjectStats* stats)
    ...
    , field_stats_collector_(heap_, &stats->tagged_fields_count_, &stats->embedder_fields_count_, &stats->inobject_smi_fields_count_, &stats->boxed_double_fields_count_,
          &stats->string_data_count_, &stats->raw_fields_count_)
{
}

bool ObjectStatsCollectorImpl::ShouldRecordObject(HeapObject obj, CowMode check_cow_array)
{
...
template <typename Derived, typename Shape>
void ObjectStatsCollectorImpl::RecordHashTableVirtualObjectStats(HeapObject parent, HashTable<Derived, Shape> hash_table, ObjectStats::VirtualInstanceType type)
{
...
bool ObjectStatsCollectorImpl::RecordSimpleVirtualObjectStats(HeapObject parent, HeapObject obj, ObjectStats::VirtualInstanceType type)
{
...
bool ObjectStatsCollectorImpl::RecordVirtualObjectStats(
    HeapObject parent, HeapObject obj, ObjectStats::VirtualInstanceType type, size_t size, size_t over_allocated, CowMode check_cow_array)
{
...
void ObjectStatsCollectorImpl::RecordVirtualAllocationSiteDetails(AllocationSite site)
{
...
void ObjectStatsCollectorImpl::RecordVirtualFunctionTemplateInfoDetails(FunctionTemplateInfo fti)
{
    ...
    if (!call_code.IsUndefined(isolate())) {
        RecordSimpleVirtualObjectStats(fti, CallHandlerInfo::cast(call_code), ObjectStats::FUNCTION_TEMPLATE_INFO_ENTRIES_TYPE);
    }
    if (!fti.GetInstanceCallHandler().IsUndefined(isolate())) {
        RecordSimpleVirtualObjectStats(fti, CallHandlerInfo::cast(fti.GetInstanceCallHandler()), ObjectStats::FUNCTION_TEMPLATE_INFO_ENTRIES_TYPE);
    }
}

void ObjectStatsCollectorImpl::RecordVirtualJSGlobalObjectDetails(JSGlobalObject object)
{
    // Properties.
...
void ObjectStatsCollectorImpl::RecordVirtualJSObjectDetails(JSObject object)
{
    ...
    if (object.HasFastProperties()) {
        ...
        if (properties != ReadOnlyRoots(heap_).empty_property_array()) {
            ...
            RecordVirtualObjectStats(object, properties, object.map().is_prototype_map() ? ObjectStats::PROTOTYPE_PROPERTY_ARRAY_TYPE : ObjectStats::OBJECT_PROPERTY_ARRAY_TYPE,
                properties.Size(), over_allocated);
        }
    } else {
        NameDictionary properties = object.property_dictionary();
        RecordHashTableVirtualObjectStats(
            object, properties, object.map().is_prototype_map() ? ObjectStats::PROTOTYPE_PROPERTY_DICTIONARY_TYPE : ObjectStats::OBJECT_PROPERTY_DICTIONARY_TYPE);
    }
    ...
    if (object.HasDictionaryElements()) {
        RecordHashTableVirtualObjectStats(
            object, NumberDictionary::cast(elements), object.IsJSArray() ? ObjectStats::ARRAY_DICTIONARY_ELEMENTS_TYPE : ObjectStats::OBJECT_DICTIONARY_ELEMENTS_TYPE);
    ...
    if (object.IsJSCollection()) {
        ...
        if (!maybe_table.IsUndefined(isolate())) {
            ...
            RecordSimpleVirtualObjectStats(object, HeapObject::cast(maybe_table), ObjectStats::JS_COLLECTION_TABLE_TYPE);
        }
    }
}

static ObjectStats::VirtualInstanceType GetFeedbackSlotType(MaybeObject maybe_obj, FeedbackSlotKind kind, Isolate* isolate)
{
...
void ObjectStatsCollectorImpl::RecordVirtualFeedbackVectorDetails(FeedbackVector vector)
{
    ...
    while (it.HasNext()) {
        ...
        for (int i = 0; i < it.entry_size(); i++) {
            ...
            if (raw_object->GetHeapObject(&object)) {
                if (object.IsCell(cage_base()) || object.IsWeakFixedArray(cage_base())) {
...
void ObjectStatsCollectorImpl::RecordVirtualFixedArrayDetails(FixedArray array)
{
    if (IsCowArray(array)) {
...
void ObjectStatsCollectorImpl::CollectStatistics(HeapObject obj, Phase phase, CollectFieldStats collect_field_stats)
{
    ...
    case kPhase1:
        if (InstanceTypeChecker::IsFeedbackVector(instance_type)) {
            ...
            RecordVirtualFunctionTemplateInfoDetails(FunctionTemplateInfo::cast(obj));
            ...
            RecordVirtualArrayBoilerplateDescription(ArrayBoilerplateDescription::cast(obj));
...
void ObjectStatsCollectorImpl::CollectGlobalStatistics()
{
    ...
    // WeakArrayList.
    RecordSimpleVirtualObjectStats(HeapObject(), WeakArrayList::cast(heap_->script_list()), ObjectStats::SCRIPT_LIST_TYPE);
}

void ObjectStatsCollectorImpl::RecordObjectStats(HeapObject obj, InstanceType type, size_t size, size_t over_allocated)
{
    if (virtual_objects_.find(obj) == virtual_objects_.end()) {
...
bool ObjectStatsCollectorImpl::IsCowArray(FixedArrayBase array) { return array.map(cage_base()) == ReadOnlyRoots(heap_).fixed_cow_array_map(); }
    ...
    if (map.is_prototype_map()) {
        if (map.prototype_info().IsPrototypeInfo(cage_base())) {
            ...
            if (users.IsWeakFixedArray(cage_base())) {
                RecordSimpleVirtualObjectStats(map, WeakArrayList::cast(users), ObjectStats::PROTOTYPE_USERS_TYPE);
            }
        }
    }
}
...
{
    RecordSimpleVirtualObjectStats(script, script.shared_function_infos(), ObjectStats::SCRIPT_SHARED_FUNCTION_INFOS_TYPE);
    ...
    if (raw_source.IsExternalString(cage_base())) {
        ...
        RecordExternalResourceStats(
            resource, string.IsOneByteRepresentation() ? ObjectStats::SCRIPT_SOURCE_EXTERNAL_ONE_BYTE_TYPE : ObjectStats::SCRIPT_SOURCE_EXTERNAL_TWO_BYTE_TYPE, off_heap_size);
    } else if (raw_source.IsString(cage_base())) {
        String source = String::cast(raw_source);
        RecordSimpleVirtualObjectStats(
            script, source, source.IsOneByteRepresentation() ? ObjectStats::SCRIPT_SOURCE_NON_EXTERNAL_ONE_BYTE_TYPE : ObjectStats::SCRIPT_SOURCE_NON_EXTERNAL_TWO_BYTE_TYPE);
    }
}

void ObjectStatsCollectorImpl::RecordVirtualExternalStringDetails(ExternalString string)
{
    ...
    RecordExternalResourceStats(resource,
        string.IsOneByteRepresentation(cage_base()) ? ObjectStats::STRING_EXTERNAL_RESOURCE_ONE_BYTE_TYPE : ObjectStats::STRING_EXTERNAL_RESOURCE_TWO_BYTE_TYPE, off_heap_size);
}

void ObjectStatsCollectorImpl::RecordVirtualSharedFunctionInfoDetails(SharedFunctionInfo info)
{
...
void ObjectStatsCollectorImpl::RecordVirtualArrayBoilerplateDescription(ArrayBoilerplateDescription description)
{
    RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(description, description.constant_elements(), ObjectStats::ARRAY_BOILERPLATE_DESCRIPTION_ELEMENTS_TYPE);
}

void ObjectStatsCollectorImpl::RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(HeapObject parent, HeapObject object, ObjectStats::VirtualInstanceType type)
{
    ...
    if (object.IsFixedArrayExact(cage_base())) {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            if (!entry.IsHeapObject())
                continue;
            RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(array, HeapObject::cast(entry), type);
        }
    }
}

void ObjectStatsCollectorImpl::RecordVirtualBytecodeArrayDetails(BytecodeArray bytecode)
{
    ...
    for (int i = 0; i < constant_pool.length(); i++) {
        ...
        if (entry.IsFixedArrayExact(cage_base())) {
            RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(constant_pool, HeapObject::cast(entry), ObjectStats::EMBEDDED_OBJECT_TYPE);
        }
    }
...
{
    RecordSimpleVirtualObjectStats(HeapObject(), code, CodeKindToVirtualInstanceType(code.kind()));
    RecordSimpleVirtualObjectStats(code, code.relocation_info(), ObjectStats::RELOC_INFO_TYPE);
    if (CodeKindIsOptimizedJSFunction(code.kind())) {
        ...
        if (source_position_table.IsHeapObject()) {
            RecordSimpleVirtualObjectStats(code, HeapObject::cast(source_position_table), ObjectStats::SOURCE_POSITION_TABLE_TYPE);
        }
        RecordSimpleVirtualObjectStats(code, code.deoptimization_data(), ObjectStats::DEOPTIMIZATION_DATA_TYPE);
    ...
    for (RelocIterator it(code, mode_mask); !it.done(); it.next()) {
        ...
        if (target.IsFixedArrayExact(cage_base())) {
            RecordVirtualObjectsForConstantPoolOrEmbeddedObjects(code, HeapObject::cast(target), ObjectStats::EMBEDDED_OBJECT_TYPE);
        }
    }
}
...
void ObjectStatsCollectorImpl::RecordVirtualContext(Context context)
{
    if (context.IsNativeContext()) {
        ...
        if (context.retained_maps().IsWeakArrayList(cage_base())) {
            RecordSimpleVirtualObjectStats(context, WeakArrayList::cast(context.retained_maps()), ObjectStats::RETAINED_MAPS_TYPE);
        }
