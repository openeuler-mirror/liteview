...
#ifdef V8_COMPRESS_POINTERS
    V8HeapCompressionScheme::ProcessIntermediatePointers(isolate_, address, [this](Address ptr) { VisitConservativelyIfPointer(ptr); });
#endif // V8_COMPRESS_POINTERS
}
...
void ConservativeStackVisitor::VisitConservativelyIfPointer(Address address)
{
    ...
    base_ptr = isolate_->heap()->mark_compact_collector()->FindBasePtrForMarking(address);
#else
#error "Some inner pointer resolution mechanism is needed"
    ...
    Object root = obj;
    delegate_->VisitRootPointer(Root::kHandleScope, nullptr, FullObjectSlot(&root));
