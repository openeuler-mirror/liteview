...
public:
    ...
    static void ActivateAll(Heap* heap, bool is_compacting, MarkingBarrierType marking_barrier_type);
    ...
    bool is_minor() const { return marking_barrier_type_ == MarkingBarrierType::kMinor; }
...
private:
    ...
    template <typename TSlot> inline void MarkRange(HeapObject value, TSlot start, TSlot end);

    bool is_major() const { return marking_barrier_type_ == MarkingBarrierType::kMajor; }
    ...
    std::unordered_map<MemoryChunk*, std::unique_ptr<TypedSlots>, MemoryChunk::Hasher> typed_slots_map_;
