...
public:
    ...
    struct Hasher {
        size_t operator()(const BasicMemoryChunk* const chunk) const { return reinterpret_cast<size_t>(chunk) >> kPageSizeBits; }
    };
    ...
    static constexpr MainThreadFlags kPointersToHereAreInterestingOrInSharedHeapMask = MainThreadFlags(POINTERS_TO_HERE_ARE_INTERESTING) | MainThreadFlags(IN_SHARED_HEAP);
    ...
    BasicMemoryChunk(Heap* heap, BaseSpace* space, size_t chunk_size, Address area_start, Address area_end, VirtualMemory reservation);
    ...
    inline size_t Offset(Address a) const { return static_cast<size_t>(a - address()); }

    // Some callers rely on the fact that this can operate on both
    // tagged and aligned object addresses.
    inline uint32_t AddressToMarkbitIndex(Address addr) const { return static_cast<uint32_t>(addr - this->address()) >> kTaggedSizeLog2; }

    inline Address MarkbitIndexToAddress(uint32_t index) const { return this->address() + (index << kTaggedSizeLog2); }
    ...
    size_t area_size() const { return static_cast<size_t>(area_end() - area_start()); }
    ...
    void ClearFlag(Flag flag) { main_thread_flags_ = main_thread_flags_.without(flag); }
    ...
    void SetFlags(MainThreadFlags flags, MainThreadFlags mask = kAllFlagsMask) { main_thread_flags_ = (main_thread_flags_ & ~mask) | (flags & mask); }
...
public:
    ...
    bool CanAllocate() const { return !IsEvacuationCandidate() && !IsFlagSet(NEVER_ALLOCATE_ON_PAGE); }
    ...
    Executability executable() const { return IsFlagSet(IS_EXECUTABLE) ? EXECUTABLE : NOT_EXECUTABLE; }
    ...
    bool InYoungGeneration() const { return (GetFlags() & kIsInYoungGenerationMask) != 0; }
    bool InNewSpace() const { return InYoungGeneration() && !IsLargePage(); }
    bool InNewLargeObjectSpace() const { return InYoungGeneration() && IsLargePage(); }
    ...
    bool Contains(Address addr) const { return addr >= area_start() && addr < area_end(); }
    ...
    bool ContainsLimit(Address addr) const { return addr >= area_start() && addr <= area_end(); }
    ...
    static const intptr_t kMarkingBitmapOffset = MemoryChunkLayout::kMarkingBitmapOffset;
    static const size_t kHeaderSize = MemoryChunkLayout::kBasicMemoryChunkHeaderSize;
    ...
    template <AccessMode mode> ConcurrentBitmap<mode>* marking_bitmap() const
    {
        return static_cast<ConcurrentBitmap<mode>*>(Bitmap::FromAddress(address() + kMarkingBitmapOffset));
    }
