...
void IsolateSafepoint::InitiateGlobalSafepointScopeRaw(Isolate* initiator, PerClientSafepointData* client_data)
{
    ...
    if (isolate() != initiator) {
        ...
        // wake it up.
        V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate()))->PostTask(std::make_unique<GlobalSafepointInterruptTask>(heap_));
...
void IsolateSafepoint::WaitUntilRunningThreadsInSafepoint(const PerClientSafepointData* client_data) { barrier_.WaitUntilRunningThreadsInSafepoint(client_data->running()); }
...
Isolate* IsolateSafepoint::shared_heap_isolate() const { return isolate()->shared_heap_isolate(); }

SafepointScope::SafepointScope(Heap* heap)
...
{
    DCHECK_WITH_MSG(clients_head_ == nullptr,
...
void GlobalSafepoint::EnterGlobalSafepointScope(Isolate* initiator)
{
    ...
    if (shared_heap_isolate_->is_shared()) {
        ...
        CHECK(shared_heap_isolate_->heap()->safepoint()->local_heaps_mutex_.TryLock());
...
GlobalSafepointScope::GlobalSafepointScope(Isolate* initiator)
    ...
    , shared_heap_isolate_(initiator->has_shared_heap() ? initiator->shared_heap_isolate() : nullptr)
{
...
{
    if (shared_heap_isolate_) {
        shared_heap_isolate_->global_safepoint()->LeaveGlobalSafepointScope(initiator_);
    } else {
