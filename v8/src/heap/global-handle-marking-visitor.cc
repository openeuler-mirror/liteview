...
GlobalHandleMarkingVisitor::GlobalHandleMarkingVisitor(Heap& heap, MarkingWorklists::Local& local_marking_worklist)
    : heap_(heap)
    , marking_state_(*heap_.marking_state())
    , local_marking_worklist_(local_marking_worklist)
    , traced_node_bounds_(heap.isolate()->global_handles()->GetTracedNodeBounds())
{
}
...
{
    const auto upper_it
        = std::upper_bound(traced_node_bounds_.begin(), traced_node_bounds_.end(), address, [](const void* needle, const auto& pair) { return needle < pair.first; });
    ...
    if (address < bounds->second) {
        auto object = GlobalHandles::MarkTracedConservatively(
            const_cast<Address*>(reinterpret_cast<const Address*>(address)), const_cast<Address*>(reinterpret_cast<const Address*>(bounds->first)));
