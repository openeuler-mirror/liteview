...
PagedSpaceObjectIterator::PagedSpaceObjectIterator(Heap* heap, const PagedSpaceBase* space, const Page* page)
    : cur_addr_(kNullAddress)
    , cur_end_(kNullAddress)
    , space_(space)
    , page_range_(page)
...
PagedSpaceObjectIterator::PagedSpaceObjectIterator(Heap* heap, const PagedSpace* space, const Page* page, Address start_address)
...
{
    heap->MakeHeapIterable();
    DCHECK_IMPLIES(space->IsInlineAllocationEnabled(), !page->Contains(space->top()));
...
PagedSpaceBase::PagedSpaceBase(Heap* heap, AllocationSpace space, Executability executable, FreeList* free_list, AllocationCounter& allocation_counter,
    LinearAllocationArea& allocation_info_, LinearAreaOriginalData& linear_area_original_data, CompactionSpaceKind compaction_space_kind)
    : SpaceWithLinearArea(heap, space, free_list, allocation_counter, allocation_info_, linear_area_original_data)
...
void PagedSpaceBase::RefillFreeList()
{
    ...
    DCHECK(identity() == OLD_SPACE || identity() == CODE_SPACE || identity() == MAP_SPACE || identity() == NEW_SPACE || identity() == SHARED_SPACE);
...

    {
        ...
        while ((p = sweeper->GetSweptPageSafe(this)) != nullptr) {
            ...
            if (p->IsFlagSet(Page::NEVER_ALLOCATE_ON_PAGE)) {
                p->ForAllFreeListCategories([this](FreeListCategory* category) { category->Reset(free_list()); });
            }
            ...
            if (is_compaction_space() && (added > kCompactionMemoryWanted))
                break;
        }
    }
}
...
void PagedSpaceBase::MergeCompactionSpace(CompactionSpace* other)
{
    ...
    for (int i = static_cast<int>(AllocationOrigin::kFirstAllocationOrigin); i <= static_cast<int>(AllocationOrigin::kLastAllocationOrigin); i++) {
    ...
    for (auto it = other->begin(); it != other->end();) {
        ...
        other->RemovePage(p);
        AddPage(p);
        DCHECK_IMPLIES(!p->IsFlagSet(Page::NEVER_ALLOCATE_ON_PAGE), p->AvailableInFreeList() == p->AvailableInFreeListFromAllocatedBytes());
...
size_t PagedSpaceBase::CommittedPhysicalMemory() const
{
    ...
    CodePageHeaderModificationScope rwx_write_scope("Updating high water mark for Code pages requires write access to "
...
void PagedSpaceBase::SetTopAndLimit(Address top, Address limit)
{
    ...
    if (!is_compaction_space())
        optional_guard.emplace(linear_area_lock());
...
{
    Page* page = heap()->memory_allocator()->AllocatePage(MemoryAllocator::AllocationMode::kRegular, this, executable());
    if (page == nullptr)
        return nullptr;
    ConcurrentAllocationMutex guard(this);
    AddPage(page);
    Free(page->area_start(), page->area_size(), SpaceAccountingMode::kSpaceAccounted);
    return page;
}

base::Optional<std::pair<Address, size_t>> PagedSpaceBase::TryExpandBackground(size_t size_in_bytes)
{
    DCHECK_NE(NEW_SPACE, identity());
    Page* page = heap()->memory_allocator()->AllocatePage(MemoryAllocator::AllocationMode::kRegular, this, executable());
    if (page == nullptr)
    ...
    Free(page->area_start() + size_in_bytes, page->area_size() - size_in_bytes, SpaceAccountingMode::kSpaceAccounted);
...
void PagedSpaceBase::DecreaseLimit(Address new_limit)
{
    ...
    if (new_limit != old_limit) {
        ...
        Free(new_limit, old_limit - new_limit, SpaceAccountingMode::kSpaceAccounted);
        if (heap()->incremental_marking()->black_allocation() && identity() != NEW_SPACE) {
            Page::FromAllocationAreaAddress(new_limit)->DestroyBlackArea(new_limit, old_limit);
        }
    }
}
...
void PagedSpaceBase::MarkLinearAllocationAreaBlack()
{
    ...
    if (current_top != kNullAddress && current_top != current_limit) {
        Page::FromAllocationAreaAddress(current_top)->CreateBlackArea(current_top, current_limit);
    }
}
...
void PagedSpaceBase::UnmarkLinearAllocationArea()
{
    ...
    Address current_limit = limit();
    if (current_top != kNullAddress && current_top != current_limit) {
        Page::FromAllocationAreaAddress(current_top)->DestroyBlackArea(current_top, current_limit);
    }
}
...
void PagedSpaceBase::MakeLinearAllocationAreaIterable()
{
    ...
    if (current_top != kNullAddress && current_top != current_limit) {
        ...
        heap_->CreateFillerObjectAt(current_top, static_cast<int>(current_limit - current_top));
    }
}
...
{
    return is_compaction_space ? UnprotectMemoryOrigin::kMaybeOffMainThread : UnprotectMemoryOrigin::kMainThread;
}
...
void PagedSpaceBase::FreeLinearAllocationArea()
{
    ...
    if (identity() != NEW_SPACE && current_top != current_limit && heap()->incremental_marking()->black_allocation()) {
        Page::FromAddress(current_top)->DestroyBlackArea(current_top, current_limit);
    }
    ...
    if (identity() == CODE_SPACE) {
        heap()->UnprotectAndRegisterMemoryChunk(MemoryChunk::FromAddress(current_top), GetUnprotectMemoryOrigin(is_compaction_space()));
    }

    DCHECK_IMPLIES(current_limit - current_top >= 2 * kTaggedSize, heap()->marking_state()->IsWhite(HeapObject::FromAddress(current_top)));
    Free(current_top, current_limit - current_top, SpaceAccountingMode::kSpaceAccounted);
}
...
void PagedSpaceBase::ReleasePage(Page* page)
{
    ...
    heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kConcurrently, page);
}
...
std::unique_ptr<ObjectIterator> PagedSpaceBase::GetObjectIterator(Heap* heap) { return std::unique_ptr<ObjectIterator>(new PagedSpaceObjectIterator(heap, this)); }

bool PagedSpaceBase::TryAllocationFromFreeListMain(size_t size_in_bytes, AllocationOrigin origin)
{
    ConcurrentAllocationMutex guard(this);
    ...
    if (limit != end) {
        if (identity() == CODE_SPACE) {
            heap()->UnprotectAndRegisterMemoryChunk(page, GetUnprotectMemoryOrigin(is_compaction_space()));
        }
...
base::Optional<std::pair<Address, size_t>> PagedSpaceBase::TryAllocationFromFreeListBackground(size_t min_size_in_bytes, size_t max_size_in_bytes, AllocationOrigin origin)
{
    base::MutexGuard lock(&space_mutex_);
    DCHECK_LE(min_size_in_bytes, max_size_in_bytes);
    DCHECK(identity() == OLD_SPACE || identity() == CODE_SPACE || identity() == MAP_SPACE || identity() == SHARED_SPACE);

    size_t new_node_size = 0;
    FreeSpace new_node = free_list_->Allocate(min_size_in_bytes, &new_node_size, origin);
    if (new_node.is_null())
        return {};
    ...
    if (limit != end) {
        if (identity() == CODE_SPACE) {
            heap()->UnprotectAndRegisterMemoryChunk(page, UnprotectMemoryOrigin::kMaybeOffMainThread);
        }
        Free(limit, end - limit, SpaceAccountingMode::kSpaceAccounted);
    }
    AddRangeToActiveSystemPages(page, start, limit);
...
void PagedSpaceBase::Verify(Isolate* isolate, ObjectVisitor* visitor) const
{
    ...
    for (const Page* page : *this) {
        ...
        for (HeapObject object = it.Next(); !object.is_null(); object = it.Next()) {
            ...
            CHECK(ReadOnlyHeap::Contains(map) || isolate->heap()->space_for_maps()->Contains(map));
            ...
            if (object.IsExternalString(cage_base)) {
                ...
                external_page_bytes[ExternalBackingStoreType::kExternalString] += payload_size;
            }
        }
        for (int i = 0; i < kNumTypes; i++) {
            ExternalBackingStoreType t = static_cast<ExternalBackingStoreType>(i);
    ...
    for (int i = 0; i < kNumTypes; i++) {
        if (i == ExternalBackingStoreType::kArrayBuffer)
            continue;
        ExternalBackingStoreType t = static_cast<ExternalBackingStoreType>(i);
...
void PagedSpaceBase::VerifyCountersBeforeConcurrentSweeping() const
{
    ...
    auto marking_state = heap()->non_atomic_marking_state();
    for (const Page* page : *this) {
        size_t page_allocated = page->SweepingDone() ? page->allocated_bytes() : static_cast<size_t>(marking_state->live_bytes(page));
        total_capacity += page->area_size();
...
bool CompactionSpace::RefillLabMain(int size_in_bytes, AllocationOrigin origin) { return RawRefillLabMain(size_in_bytes, origin); }
...
bool PagedSpaceBase::TryExpand(int size_in_bytes, AllocationOrigin origin)
{
    ...
    return TryAllocationFromFreeListMain(static_cast<size_t>(size_in_bytes), origin);
}

bool PagedSpaceBase::RawRefillLabMain(int size_in_bytes, AllocationOrigin origin)
{
    // Allocation in this space has failed.
    DCHECK_GE(size_in_bytes, 0);
    const int kMaxPagesToSweep = 1;

    if (TryAllocationFromFreeListMain(size_in_bytes, origin))
        return true;
    ...
    const bool is_main_thread = heap()->IsMainThread() || heap()->IsSharedMainThread();
    const auto sweeping_scope_id = is_main_thread ? GCTracer::Scope::MC_SWEEP : GCTracer::Scope::MC_BACKGROUND_SWEEPING;
    ...
    if (heap()->sweeping_in_progress()) {
        ...
        if (TryAllocationFromFreeListMain(static_cast<size_t>(size_in_bytes), origin))
            return true;

        {
            TRACE_GC_EPOCH(heap()->tracer(), sweeping_scope_id, sweeping_scope_kind);
            if (ContributeToSweepingMain(size_in_bytes, kMaxPagesToSweep, size_in_bytes, origin))
                return true;
        }
    }
    ...
    if (heap()->ShouldExpandOldGenerationOnSlowAllocation(heap()->main_thread_local_heap()) && heap()->CanExpandOldGeneration(AreaSize())) {
...
bool PagedSpaceBase::ContributeToSweepingMain(int required_freed_bytes, int max_pages, int size_in_bytes, AllocationOrigin origin)
{
    ...
    // final atomic pause.
    Sweeper::SweepingMode sweeping_mode = is_compaction_space() ? Sweeper::SweepingMode::kEagerDuringGC : Sweeper::SweepingMode::kLazyOrConcurrent;

    if (heap()->sweeping_in_progress()) {
        heap()->sweeper()->ParallelSweepSpace(identity(), sweeping_mode, required_freed_bytes, max_pages);
        RefillFreeList();
...
void PagedSpaceBase::AddRangeToActiveSystemPages(Page* page, Address start, Address end)
{
    DCHECK_LE(page->address(), start);
    DCHECK_LT(start, end);
    DCHECK_LE(end, page->address() + Page::kPageSize);

    const size_t added_pages = page->active_system_pages()->Add(start - page->address(), end - page->address(), MemoryAllocator::GetCommitPageSizeBits());

    IncrementCommittedPhysicalMemory(added_pages * MemoryAllocator::GetCommitPageSize());
}

void PagedSpaceBase::ReduceActiveSystemPages(Page* page, ActiveSystemPages active_system_pages)
{
...
size_t PagedSpaceBase::RelinkFreeListCategories(Page* page)
{
    ...
    DCHECK_IMPLIES(!page->IsFlagSet(Page::NEVER_ALLOCATE_ON_PAGE), page->AvailableInFreeList() == page->AvailableInFreeListFromAllocatedBytes());
    return added;
}
