...
public:
    ...
    private:
        ...
        void PerformFreeMemoryOnQueuedChunks(FreeMode mode, JobDelegate* delegate = nullptr);

        void PerformFreeMemoryOnQueuedNonRegularChunks(JobDelegate* delegate = nullptr);
    ...
    V8_EXPORT_PRIVATE static base::AddressRegion ComputeDiscardMemoryArea(Address addr, size_t size);

    V8_EXPORT_PRIVATE MemoryAllocator(Isolate* isolate, v8::PageAllocator* code_page_allocator, size_t max_capacity);
    ...
    V8_EXPORT_PRIVATE Page* AllocatePage(MemoryAllocator::AllocationMode alloc_mode, Space* space, Executability executable);

    V8_EXPORT_PRIVATE LargePage* AllocateLargePage(LargeObjectSpace* space, size_t object_size, Executability executable);
    ...
    V8_EXPORT_PRIVATE void Free(MemoryAllocator::FreeMode mode, MemoryChunk* chunk);
    ...
    V8_INLINE bool IsOutsideAllocatedSpace(Address address) const { return address < lowest_ever_allocated_ || address >= highest_ever_allocated_; }
    ...
    // executability.
    v8::PageAllocator* page_allocator(Executability executable) { return executable == EXECUTABLE ? code_page_allocator_ : data_page_allocator_; }
...
private:
    ...
    V8_WARN_UNUSED_RESULT base::Optional<MemoryChunkAllocationResult> AllocateUninitializedChunk(BaseSpace* space, size_t area_size, Executability executable, PageSize page_size);

    // Internal raw allocation method that allocates an aligned MemoryChunk and
    // sets the right memory permissions.
    Address AllocateAlignedMemory(size_t chunk_size, size_t area_size, size_t alignment, Executability executable, void* hint, VirtualMemory* controller);
    ...
    // permissions).
    V8_WARN_UNUSED_RESULT bool SetPermissionsOnExecutableMemoryChunk(VirtualMemory* vm, Address start, size_t area_size, size_t reserved_size);
    ...
    base::Optional<MemoryChunkAllocationResult> AllocateUninitializedPageFromPool(Space* space);
    ...
    Page* InitializePagesInChunk(int chunk_id, int pages_in_chunk, PagedSpace* space);
    ...
    void UpdateAllocatedSpaceLimits(Address low, Address high)
    {
        ...
        while ((low < ptr) && !lowest_ever_allocated_.compare_exchange_weak(ptr, low, std::memory_order_acq_rel)) { }
        ptr = highest_ever_allocated_.load(std::memory_order_relaxed);
        while ((high > ptr) && !highest_ever_allocated_.compare_exchange_weak(ptr, high, std::memory_order_acq_rel)) { }
    }
    ...
    void UnregisterBasicMemoryChunk(BasicMemoryChunk* chunk, Executability executable = NOT_EXECUTABLE);
