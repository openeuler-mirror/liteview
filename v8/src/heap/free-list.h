...
public:
    ...
    void Free(Address address, size_t size_in_bytes, FreeMode mode, FreeList* owner);

    // Performs a single try to pick a node of at least |minimum_size| from the
    // category. Stores the actual size in |node_size|. Returns nullptr if no
    // node is found.
    V8_EXPORT_PRIVATE FreeSpace PickNodeFromList(size_t minimum_size, size_t* node_size);
...
public:
    ...
    virtual V8_WARN_UNUSED_RESULT FreeSpace Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) = 0;
...
protected:
    ...
    // than minimum_size.
    FreeSpace TryFindNodeIn(FreeListCategoryType type, size_t minimum_size, size_t* node_size);

    // Searches a given |type| for a node of at least |minimum_size|.
    FreeSpace SearchForNodeInList(FreeListCategoryType type, size_t minimum_size, size_t* node_size);
    ...
    FreeListCategory* top(FreeListCategoryType type) const { return categories_[type]; }
...
public:
    size_t GuaranteedAllocatable(size_t maximum_freed) final { FATAL("NoFreeList can't be used as a standard FreeList. "); }
    size_t Free(Address start, size_t size_in_bytes, FreeMode mode) final { FATAL("NoFreeList can't be used as a standard FreeList."); }
    V8_WARN_UNUSED_RESULT FreeSpace Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) final { FATAL("NoFreeList can't be used as a standard FreeList."); }
    Page* GetPageForSize(size_t size_in_bytes) final { FATAL("NoFreeList can't be used as a standard FreeList."); }

private:
    FreeListCategoryType SelectFreeListCategoryType(size_t size_in_bytes) final { FATAL("NoFreeList can't be used as a standard FreeList."); }
};
...
public:
    ...
    V8_WARN_UNUSED_RESULT FreeSpace Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) override;
...
class V8_EXPORT_PRIVATE FreeListManyCachedFastPathForNewSpace : public FreeListManyCachedFastPath {
public:
    V8_WARN_UNUSED_RESULT FreeSpace Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) override;
...
class V8_EXPORT_PRIVATE FreeListManyCachedOriginForNewSpace : public FreeListManyCachedFastPathForNewSpace {
public:
    V8_WARN_UNUSED_RESULT FreeSpace Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) override;
};
