...
CodePageCollectionMemoryModificationScopeForTesting::CodePageCollectionMemoryModificationScopeForTesting(Heap* heap)
    : CodePageCollectionMemoryModificationScope(heap)
{
}

CodePageCollectionMemoryModificationScopeForTesting::~CodePageCollectionMemoryModificationScopeForTesting() = default;

#ifdef V8_ENABLE_THIRD_PARTY_HEAP
Isolate* Heap::GetIsolateFromWritableObject(HeapObject object) { return reinterpret_cast<Isolate*>(third_party_heap::Heap::GetIsolate(object.address())); }
#endif

// These are outside the Heap class so they can be forward-declared
// in heap-write-barrier-inl.h.
bool Heap_PageFlagsAreConsistent(HeapObject object) { return Heap::PageFlagsAreConsistent(object); }

void Heap_CombinedGenerationalAndSharedBarrierSlow(HeapObject object, Address slot, HeapObject value) { Heap::CombinedGenerationalAndSharedBarrierSlow(object, slot, value); }

void Heap_CombinedGenerationalAndSharedEphemeronBarrierSlow(EphemeronHashTable table, Address slot, HeapObject value)
{
    Heap::CombinedGenerationalAndSharedEphemeronBarrierSlow(table, slot, value);
}

void Heap_GenerationalBarrierForCodeSlow(Code host, RelocInfo* rinfo, HeapObject object) { Heap::GenerationalBarrierForCodeSlow(host, rinfo, object); }
...
void Heap::SetBasicBlockProfilingData(Handle<ArrayList> list) { set_basic_block_profiling_data(*list); }
...
public:
    ...
    void Step(int bytes_allocated, Address, size_t) override { heap_->ScheduleScavengeTaskIfNeeded(); }
    ...
    void Step(int bytes_allocated, Address, size_t) override
    {
        if (v8_flags.concurrent_minor_mc_marking) {
            if (heap_->incremental_marking()->IsMinorMarking()) {
                heap_->concurrent_marking()->RescheduleJobIfNeeded(GarbageCollector::MINOR_MARK_COMPACTOR);
            }
        }
...
size_t Heap::MaxReserved()
{
    ...
    return static_cast<size_t>(2 * max_semi_space_size_ + kMaxNewLargeObjectSpaceSize + max_old_generation_size());
}

size_t Heap::YoungGenerationSizeFromOldGenerationSize(size_t old_generation)
{
    // Compute the semi space size and cap it.
    size_t ratio = old_generation <= kOldGenerationLowMemory ? kOldGenerationToSemiSpaceRatioLowMemory : kOldGenerationToSemiSpaceRatio;
...
size_t Heap::MinYoungGenerationSize() { return YoungGenerationSizeFromSemiSpaceSize(kMinSemiSpaceSize); }
...
size_t Heap::AllocatorLimitOnMaxOldGenerationSize()
{
    ...
    return kPtrComprCageReservationSize - YoungGenerationSizeFromSemiSpaceSize(kMaxSemiSpaceSize) - RoundUp(sizeof(Isolate), size_t { 1 } << kPageSizeBits);
#else
...
size_t Heap::MaxOldGenerationSize(uint64_t physical_memory)
{
    ...
    if (v8_flags.huge_max_old_generation_size && x64_bit && (physical_memory + 512 * MB) / GB >= 16) {
...
size_t Heap::YoungGenerationSizeFromSemiSpaceSize(size_t semi_space_size) { return semi_space_size * (2 + kNewLargeObjectSpaceToSemiSpaceRatio); }

size_t Heap::SemiSpaceSizeFromYoungGenerationSize(size_t young_generation_size) { return young_generation_size / (2 + kNewLargeObjectSpaceToSemiSpaceRatio); }
...
bool Heap::CanExpandOldGenerationBackground(LocalHeap* local_heap, size_t size)
{
    if (force_oom_)
        return false;
    ...
    return gc_state() == TEAR_DOWN || IsMainThreadParked(local_heap) || memory_allocator()->Size() + size <= MaxReserved();
}
...
GarbageCollector Heap::SelectGarbageCollector(AllocationSpace space, GarbageCollectionReason gc_reason, const char** reason)
{
...
void Heap::SetGCState(HeapState state) { gc_state_.store(state, std::memory_order_relaxed); }

bool Heap::IsGCWithStack() const { return local_embedder_heap_tracer()->embedder_stack_state() == cppgc::EmbedderStackState::kMayContainHeapPointers; }
...
void Heap::PrintShortHeapStatistics()
{
    ...
    PrintIsolate(isolate_,
        ...
        ", available: %6zu KB"
        ", committed: %6zu KB\n",
        NewSpaceSize() / KB, new_space_->Available() / KB, new_space_->CommittedMemory() / KB);
    PrintIsolate(isolate_,
    ...
    PrintIsolate(isolate_,
        ...
        ", available: %6zu KB"
        ", committed: %6zu KB\n",
        old_space_->SizeOfObjects() / KB, old_space_->Available() / KB, old_space_->CommittedMemory() / KB);
    PrintIsolate(isolate_,
        "Code space,             used: %6zu KB"
        ", available: %6zu KB"
        ", committed: %6zu KB\n",
        code_space_->SizeOfObjects() / KB, code_space_->Available() / KB, code_space_->CommittedMemory() / KB);
    if (map_space()) {
        PrintIsolate(isolate_,
            "Map space,              used: %6zu KB"
            ", available: %6zu KB"
            ", committed: %6zu KB\n",
            map_space_->SizeOfObjects() / KB, map_space_->Available() / KB, map_space_->CommittedMemory() / KB);
    }
    PrintIsolate(isolate_,
    ...
    PrintIsolate(isolate_,
        ...
        ", available: %6zu KB"
        ", committed: %6zu KB\n",
        (this->SizeOfObjects() + ro_space->Size()) / KB, (this->Available()) / KB, (this->CommittedMemory() + ro_space->CommittedMemory()) / KB);
    PrintIsolate(isolate_, "Unmapper buffering %zu chunks of committed: %6zu KB\n", memory_allocator()->unmapper()->NumberOfCommittedChunks(), CommittedMemoryOfUnmapper() / KB);
...
void Heap::PrintFreeListsStats()
{
    ...
    std::ostringstream out_str;
    for (int cat = kFirstCategory; cat <= old_space()->free_list()->last_category(); cat++) {
        out_str << "[" << cat << ": " << categories_lengths[cat] << " || " << std::fixed << std::setprecision(2) << static_cast<double>(categories_sums[cat]) / KB << " KB]"
                << (cat == old_space()->free_list()->last_category() ? "\n" : ", ");
    }
    PrintIsolate(isolate_, "%s", out_str.str().c_str());
}
...
#define MEMBER(s) QUOTE(s) << ":"
    ...
    auto SpaceStatistics = [this](int space_index) {
        ...
        stream << DICT(MEMBER("name") << QUOTE(BaseSpace::GetSpaceName(static_cast<AllocationSpace>(space_index))) << "," MEMBER("size") << space_stats.space_size()
                                      << "," MEMBER("used_size") << space_stats.space_used_size() << "," MEMBER("available_size") << space_stats.space_available_size()
                                      << "," MEMBER("physical_size") << space_stats.physical_space_size());
        return stream.str();
    };

    stream << DICT(MEMBER("isolate") << QUOTE(reinterpret_cast<void*>(isolate())) << "," MEMBER("id") << gc_count() << "," MEMBER("time_ms") << isolate()->time_millis_since_init()
                                     << "," MEMBER("total_heap_size") << stats.total_heap_size() << "," MEMBER("total_heap_size_executable") << stats.total_heap_size_executable()
                                     << "," MEMBER("total_physical_size") << stats.total_physical_size() << "," MEMBER("total_available_size") << stats.total_available_size()
                                     ...
                                     << LIST(SpaceStatistics(RO_SPACE) << "," << SpaceStatistics(NEW_SPACE) << "," << SpaceStatistics(OLD_SPACE) << ","
                                                                       << SpaceStatistics(CODE_SPACE) << "," << SpaceStatistics(MAP_SPACE) << "," << SpaceStatistics(LO_SPACE)
                                                                       << "," << SpaceStatistics(CODE_LO_SPACE) << "," << SpaceStatistics(NEW_LO_SPACE)));
...
{
    for (int i = 0; i < static_cast<int>(v8::Isolate::kUseCounterFeatureCount); ++i) {
        isolate()->CountUsage(static_cast<v8::Isolate::UseCounterFeature>(i), deferred_counters_[i]);
...
public:
    static bool IsNeeded() { return v8_flags.verify_predictable || v8_flags.fuzzer_gc_analysis || (v8_flags.trace_allocation_stack_interval > 0); }
...
void Heap::AddRetainingPathTarget(Handle<HeapObject> object, RetainingPathOption option)
{
    if (!v8_flags.track_retaining_path) {
        ...
        array = WeakArrayList::AddToEnd(isolate(), array, MaybeObjectHandle::Weak(object));
...
bool Heap::IsRetainingPathTarget(HeapObject object, RetainingPathOption* option)
{
...
void Heap::PrintRetainingPath(HeapObject target, RetainingPathOption option)
{
    ...
    for (auto node : retaining_path) {
        ...
        PrintF("\n");
        PrintF("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n");
        PrintF("Distance from root %d%s: ", distance, node_ephemeron ? " (ephemeron)" : "");
...
void UpdateRetainersMapAfterScavenge(std::unordered_map<HeapObject, HeapObject, Object::Hasher>* map)
{
...
void Heap::IncrementDeferredCount(v8::Isolate::UseCounterFeature feature) { deferred_counters_[feature]++; }
...
void Heap::GarbageCollectionPrologue(GarbageCollectionReason gc_reason, const v8::GCCallbackFlags gc_callback_flags)
{
    ...
    is_current_gc_forced_ = gc_callback_flags & v8::kGCCallbackFlagForced || current_gc_flags_ & kForcedGC || force_gc_on_next_allocation_;
...
void Heap::UpdateNewSpaceAllocationCounter() { new_space_allocation_counter_ = NewSpaceAllocationCounter(); }

size_t Heap::NewSpaceAllocationCounter() { return new_space_allocation_counter_ + (new_space_ ? new_space()->AllocatedSinceLastGC() : 0); }
...
size_t Heap::TotalGlobalHandlesSize() { return isolate_->global_handles()->TotalSize(); }

size_t Heap::UsedGlobalHandlesSize() { return isolate_->global_handles()->UsedSize(); }
...
void Heap::DeoptMarkedAllocationSites()
{
    ...
    ForeachAllocationSite(allocation_sites_list(), [](AllocationSite site) {
        if (site.deopt_dependent_code()) {
            DependentCode::MarkCodeForDeoptimization(site, DependentCode::kAllocationSiteTenuringChangedGroup);
...
void Heap::GarbageCollectionEpilogueInSafepoint(GarbageCollector collector)
{
    ...
    safepoint()->IterateLocalHeaps(
        [this, collector](LocalHeap* local_heap) { local_heap->InvokeGCEpilogueCallbacksInSafepoint(GetGCTypeFromGarbageCollector(collector), current_gc_callback_flags_); });
...
#define UPDATE_FRAGMENTATION_FOR_SPACE(space)                                                                                                                                      \
    if (space()->CommittedMemory() > 0) {                                                                                                                                          \
        isolate_->counters()->external_fragmentation_##space()->AddSample(static_cast<int>(100 - (space()->SizeOfObjects() * 100.0) / space()->CommittedMemory()));                \
    }
...
#undef UPDATE_COUNTERS_AND_FRAGMENTATION_FOR_SPACE
    ...
    for (SpaceIterator it(this); it.HasNext();) {
        ...
        for (MemoryChunk* chunk = space->first_page(); chunk != space->last_page(); chunk = chunk->list_node().next())
    ...
    DCHECK_IMPLIES(collector == GarbageCollector::MARK_COMPACTOR, !memory_allocator()->unmapper()->IsRunning());
...
void Heap::GarbageCollectionEpilogue(GarbageCollector collector)
{
    ...
    if (v8_flags.track_retaining_path && collector == GarbageCollector::MARK_COMPACTOR) {
        retainer_.clear();
    ...
    if (CommittedMemory() > 0) {
        ...
        if (map_space()) {
            isolate_->counters()->heap_sample_map_space_committed()->AddSample(static_cast<int>(map_space()->CommittedMemory() / KB));
        }
...
bool GCCallbacksScope::CheckReenter() const { return heap_->gc_callbacks_depth_ == 1; }
...
size_t Heap::MinorMCTaskTriggerSize() const { return new_space()->Capacity() * v8_flags.minor_mc_task_trigger / 100; }

void Heap::StartMinorMCIncrementalMarkingIfNeeded()
{
    if (v8_flags.concurrent_minor_mc_marking && !IsTearingDown() && !incremental_marking()->IsMarking() && incremental_marking()->CanBeStarted() && V8_LIKELY(!v8_flags.gc_global)
        && (new_space()->Size() >= MinorMCTaskTriggerSize())) {
        StartIncrementalMarking(Heap::kNoGCFlags, GarbageCollectionReason::kTask, kNoGCCallbackFlags, GarbageCollector::MINOR_MARK_COMPACTOR);
    }
}

void Heap::CollectAllGarbage(int flags, GarbageCollectionReason gc_reason, const v8::GCCallbackFlags gc_callback_flags)
{
...
void Heap::CollectAllAvailableGarbage(GarbageCollectionReason gc_reason)
{
    ...
    set_current_gc_flags(kReduceMemoryFootprintMask | (gc_reason == GarbageCollectionReason::kLowMemoryNotification ? kForcedGC : 0));
    isolate_->compilation_cache()->Clear();
    const int kMaxNumberOfAttempts = 7;
    const int kMinNumberOfAttempts = 2;
    for (int attempt = 0; attempt < kMaxNumberOfAttempts; attempt++) {
        if (!CollectGarbage(OLD_SPACE, gc_reason, kNoGCCallbackFlags) && attempt + 1 >= kMinNumberOfAttempts) {
            break;
        }
    }
...
void Heap::PreciseCollectAllGarbage(int flags, GarbageCollectionReason gc_reason, const GCCallbackFlags gc_callback_flags)
{
...
void Heap::ReportExternalMemoryPressure()
{
    ...
    TRACE_EVENT2("devtools.timeline,v8", "V8.ExternalMemoryPressure", "external_memory_mb", static_cast<int>((current - baseline) / MB), "external_memory_limit_mb",
        static_cast<int>((limit - baseline) / MB));
    if (current > baseline + external_memory_hard_limit()) {
        CollectAllGarbage(kReduceMemoryFootprintMask, GarbageCollectionReason::kExternalMemoryPressure,
    ...
    if (incremental_marking()->IsStopped()) {
        if (incremental_marking()->CanBeStarted()) {
            ...
            CollectAllGarbage(i::Heap::kNoGCFlags, GarbageCollectionReason::kExternalMemoryPressure, kGCCallbackFlagsForExternalMemory);
        }
    } else {
...
Heap::DevToolsTraceEventScope::~DevToolsTraceEventScope() { TRACE_EVENT_END1("devtools.timeline,v8", event_name_, "usedHeapSizeAfter", heap_->SizeOfObjects()); }
...
bool Heap::CollectGarbage(AllocationSpace space, GarbageCollectionReason gc_reason, const v8::GCCallbackFlags gc_callback_flags)
{
    ...
    GCType gc_type = GetGCTypeFromGarbageCollector(collector);
    {
        ...
        EmbedderStackStateScope embedder_scope(this, EmbedderStackStateScope::kExplicitInvocation, StackState::kMayContainHeapPointers);
    ...
    size_t committed_memory_before = collector == GarbageCollector::MARK_COMPACTOR ? CommittedOldGenerationMemory() : 0;
    {
        ...
        DevToolsTraceEventScope devtools_trace_event_scope(this, IsYoungGenerationCollector(collector) ? "MinorGC" : "MajorGC", GarbageCollectionReasonToString(gc_reason));
        ...
        if (collector == GarbageCollector::MARK_COMPACTOR && cpp_heap()) {
            ...
            static_cast<v8::internal::CppHeap*>(cpp_heap())->SetStackEndOfCurrentGC(v8::base::Stack::GetCurrentStackPosition());
        }
        ...
        GarbageCollectionPrologue(gc_reason, gc_callback_flags);
        {
            ...
            base::Optional<OptionalTimedHistogramScope> histogram_timer_priority_scope;
            ...
            if (record_gc_phases_info.type_priority_timer()) {
                OptionalTimedHistogramScopeMode mode
                    = isolate_->IsMemorySavingsModeActive() ? OptionalTimedHistogramScopeMode::DONT_TAKE_TIME : OptionalTimedHistogramScopeMode::TAKE_TIME;
                histogram_timer_priority_scope.emplace(record_gc_phases_info.type_priority_timer(), isolate_, mode);
            }

            if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL) {
                tp_heap_->CollectGarbage();
            } else {
                freed_global_handles += PerformGarbageCollection(collector, gc_reason, collector_reason, gc_callback_flags);
            }
        ...
        if (collector == GarbageCollector::MARK_COMPACTOR) {
            ...
            if (memory_reducer_ != nullptr) {
                ...
                event.next_gc_likely_to_collect_more = (committed_memory_before > committed_memory_after + MB) || HasHighFragmentation(used_memory_after, committed_memory_after);
                event.committed_memory = committed_memory_after;
                memory_reducer_->NotifyMarkCompact(event);
            }
            if (initial_max_old_generation_size_ < max_old_generation_size() && used_memory_after < initial_max_old_generation_size_threshold_) {
...

    {
        ...
        gc_post_processing_depth_++;
        {
            AllowGarbageCollection allow_gc;
            AllowJavascriptExecution allow_js(isolate());
            isolate_->global_handles()->PostGarbageCollectionProcessing(collector, gc_callback_flags);
        }
        gc_post_processing_depth_--;
    }
    ...
    if (collector == GarbageCollector::MARK_COMPACTOR && (gc_callback_flags & (kGCCallbackFlagForced | kGCCallbackFlagCollectAllAvailableGarbage)) != 0) {
    ...
    if (IsYoungGenerationCollector(collector)) {
        StartIncrementalMarkingIfAllocationLimitIsReached(GCFlagsForIncrementalMarking(), kGCCallbackScheduleIdleGarbageCollection);
    }
...
void Heap::StartIncrementalMarking(int gc_flags, GarbageCollectionReason gc_reason, GCCallbackFlags gc_callback_flags, GarbageCollector collector)
{
...
void Heap::StartIncrementalMarkingIfAllocationLimitIsReached(int gc_flags, const GCCallbackFlags gc_callback_flags)
{
    ...
    if (incremental_marking()->IsStopped()) {
        ...
        case IncrementalMarkingLimit::kHardLimit:
            StartIncrementalMarking(gc_flags,
                OldGenerationSpaceAvailable() <= NewSpaceCapacity() ? GarbageCollectionReason::kAllocationLimit : GarbageCollectionReason::kGlobalAllocationLimit,
                gc_callback_flags);
            break;
...
{
    if (incremental_marking()->IsMarking() || !incremental_marking()->CanBeStarted()) {
        return;
    }
...
void Heap::StartIdleIncrementalMarking(GarbageCollectionReason gc_reason, const GCCallbackFlags gc_callback_flags)
{
    StartIncrementalMarking(kReduceMemoryFootprintMask, gc_reason, gc_callback_flags);
}

void Heap::MoveRange(HeapObject dst_object, const ObjectSlot dst_slot, const ObjectSlot src_slot, int len, WriteBarrierMode mode)
{
    DCHECK_NE(len, 0);
...
template void Heap::CopyRange<ObjectSlot>(HeapObject dst_object, ObjectSlot dst_slot, ObjectSlot src_slot, int len, WriteBarrierMode mode);
template void Heap::CopyRange<MaybeObjectSlot>(HeapObject dst_object, MaybeObjectSlot dst_slot, MaybeObjectSlot src_slot, int len, WriteBarrierMode mode);

template <typename TSlot> void Heap::CopyRange(HeapObject dst_object, const TSlot dst_slot, const TSlot src_slot, int len, WriteBarrierMode mode)
{
    DCHECK_NE(len, 0);
...
bool Heap::CollectionRequested() { return collection_barrier_->WasGCRequested(); }
...
void Heap::EnsureWasmCanonicalRttsSize(int length)
{
    ...
    if (length <= current_rtts->length())
        return;
    Handle<WeakArrayList> new_rtts = WeakArrayList::EnsureSpace(isolate(), current_rtts, length, AllocationType::kOld);
    ...
    Handle<WeakArrayList> current_wrappers = handle(js_to_wasm_wrappers(), isolate_);
    if (required_wrapper_length <= current_wrappers->length())
        return;
    Handle<WeakArrayList> new_wrappers = WeakArrayList::EnsureSpace(isolate(), current_wrappers, required_wrapper_length, AllocationType::kOld);
...
size_t Heap::PerformGarbageCollection(GarbageCollector collector, GarbageCollectionReason gc_reason, const char* collector_reason, const v8::GCCallbackFlags gc_callback_flags)
{
    ...
    if (!Heap::IsYoungGenerationCollector(collector) && incremental_marking_->IsMarking()) {
    ...
    size_t start_young_generation_size = NewSpaceSize() + (new_lo_space() ? new_lo_space()->SizeOfObjects() : 0);
    ...
    if (cpp_heap() && IsYoungGenerationCollector(collector)) {
        ...
        CppHeap::From(cpp_heap())->RunMinorGCIfNeeded(with_stack ? CppHeap::StackState::kMayContainHeapPointers : CppHeap::StackState::kNoHeapPointers);
    }
...
bool Heap::CollectGarbageShared(LocalHeap* local_heap, GarbageCollectionReason gc_reason)
{
    ...
    if (v8_flags.shared_space) {
        ...
        isolate()->shared_isolate()->heap()->PerformSharedGarbageCollection(isolate(), gc_reason);
        return true;
    }
}
...
void Heap::PerformSharedGarbageCollection(Isolate* initiator, GarbageCollectionReason gc_reason)
{
    DCHECK(IsShared());
    ...
    isolate()->global_safepoint()->IterateClientIsolates([](Isolate* client) {
        ...
        if (v8_flags.concurrent_marking && client->heap()->incremental_marking()->IsMarking()) {
...
void Heap::EnsureSweepingCompletedForObject(HeapObject object)
{
    ...
    if (basic_chunk->InReadOnlySpace())
        return;
...
{
    if (!((collector == GarbageCollector::MARK_COMPACTOR) || (HasLowYoungGenerationAllocationRate() && old_generation_size_configured_))) {
        return;
    }

    double v8_gc_speed = tracer()->CombinedMarkCompactSpeedInBytesPerMillisecond();
    double v8_mutator_speed = tracer()->CurrentOldGenerationAllocationThroughputInBytesPerMillisecond();
    double v8_growing_factor = MemoryController<V8HeapTrait>::GrowingFactor(this, max_old_generation_size(), v8_gc_speed, v8_mutator_speed);
    ...
    if (UseGlobalMemoryScheduling()) {
        ...
        double embedder_speed = tracer()->CurrentEmbedderAllocationThroughputInBytesPerMillisecond();
        double embedder_growing_factor = (embedder_gc_speed > 0 && embedder_speed > 0)
            ? MemoryController<GlobalMemoryTrait>::GrowingFactor(this, max_global_memory_size_, embedder_gc_speed, embedder_speed)
            : 0;
    ...
    if (collector == GarbageCollector::MARK_COMPACTOR) {
        ...
        set_old_generation_allocation_limit(MemoryController<V8HeapTrait>::CalculateAllocationLimit(
            this, old_gen_size, min_old_generation_size_, max_old_generation_size(), new_space_capacity, v8_growing_factor, mode));
        if (UseGlobalMemoryScheduling()) {
            DCHECK_GT(global_growing_factor, 0);
            global_allocation_limit_ = MemoryController<GlobalMemoryTrait>::CalculateAllocationLimit(
                this, GlobalSizeOfObjects(), min_global_memory_size_, max_global_memory_size_, new_space_capacity, global_growing_factor, mode);
        }
        ...
        size_t new_old_generation_limit = MemoryController<V8HeapTrait>::CalculateAllocationLimit(
            this, old_gen_size, min_old_generation_size_, max_old_generation_size(), new_space_capacity, v8_growing_factor, mode);
        ...
        if (UseGlobalMemoryScheduling()) {
            DCHECK_GT(global_growing_factor, 0);
            size_t new_global_limit = MemoryController<GlobalMemoryTrait>::CalculateAllocationLimit(
                this, GlobalSizeOfObjects(), min_global_memory_size_, max_global_memory_size_, new_space_capacity, global_growing_factor, mode);
...
void Heap::MarkCompact()
{
    ...
    OptionalAlwaysAllocateScope always_allocate_shared_heap(isolate()->shared_isolate() ? isolate()->shared_isolate()->heap() : nullptr);
...
{
    if (new_space_->TotalCapacity() < new_space_->MaximumCapacity() && survived_since_last_expansion_ > new_space_->TotalCapacity()) {
...
void Heap::UnprotectAndRegisterMemoryChunk(MemoryChunk* chunk, UnprotectMemoryOrigin origin)
{
    if (!write_protect_code_memory())
        return;
    ...
    if (code_space_memory_modification_scope_depth_ > 0)
        return;

    LocalHeap* local_heap = isolate()->CurrentLocalHeap();
    DCHECK_GT(local_heap->code_page_collection_memory_modification_scope_depth_, 0);
...
{
    safepoint()->IterateLocalHeaps([chunk](LocalHeap* local_heap) { local_heap->unprotected_memory_chunks_.erase(chunk); });
}

void Heap::UnprotectAndRegisterMemoryChunk(HeapObject object, UnprotectMemoryOrigin origin) { UnprotectAndRegisterMemoryChunk(MemoryChunk::FromHeapObject(object), origin); }
...
void Heap::UpdateExternalString(String string, size_t old_payload, size_t new_payload)
{
...
String Heap::UpdateYoungReferenceInExternalStringTableEntry(Heap* heap, FullObjectSlot p)
{
    // This is only used for Scavenger.
    DCHECK(!v8_flags.minor_mc);
...
void Heap::ExternalStringTable::VerifyYoung()
{
    ...
    for (std::map<MemoryChunk*, size_t>::iterator it = size_map.begin(); it != size_map.end(); it++)
...
{
    if (!young_strings_.empty()) {
        v->VisitRootPointers(Root::kExternalStringsTable, nullptr, FullObjectSlot(young_strings_.data()), FullObjectSlot(young_strings_.data() + young_strings_.size()));
    }
}
...
void Heap::ExternalStringTable::IterateAll(RootVisitor* v)
{
    ...
    if (!old_strings_.empty()) {
        v->VisitRootPointers(Root::kExternalStringsTable, nullptr, FullObjectSlot(old_strings_.data()), FullObjectSlot(old_strings_.data() + old_strings_.size()));
    }
}

void Heap::UpdateYoungReferencesInExternalStringTable(ExternalStringTableUpdaterCallback updater_func) { external_string_table_.UpdateYoungReferences(updater_func); }
...
void Heap::UpdateReferencesInExternalStringTable(ExternalStringTableUpdaterCallback updater_func) { external_string_table_.UpdateReferences(updater_func); }
...
{
    Object head = VisitWeakList<JSFinalizationRegistry>(this, dirty_js_finalization_registries_list(), retainer);
...
void Heap::ForeachAllocationSite(Object list, const std::function<void(AllocationSite)>& visitor)
{
...
void Heap::ResetAllAllocationSitesDependentCode(AllocationType allocation)
{
    ...
    ForeachAllocationSite(allocation_sites_list(), [&marked, allocation, this](AllocationSite site) {
    ...
    public:
        ...
        void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override
        {
            for (FullObjectSlot p = start; p < end; ++p) {
                ...
                visitor_->VisitExternalString(Utils::ToLocal(Handle<String>(String::cast(*p), isolate_)));
            }
        }
...
static_assert(!USE_ALLOCATION_ALIGNMENT_BOOL || (HeapNumber::kValueOffset & kDoubleAlignmentMask) == kTaggedSize);
...
size_t Heap::GetCodeRangeReservedAreaSize() { return CodeRange::GetWritableReservedAreaSize(); }
...
HeapObject Heap::AlignWithFiller(HeapObject object, int object_size, int allocation_size, AllocationAlignment alignment)
{
...
void* Heap::AllocateExternalBackingStore(const std::function<void*(size_t)>& allocate, size_t byte_length)
{
    if (!always_allocate() && new_space()) {
        ...
        if (new_space_backing_store_bytes >= 2 * kMaxSemiSpaceSize && new_space_backing_store_bytes >= byte_length) {
...
void Heap::ConfigureInitialOldGenerationSize()
{
    if (!old_generation_size_configured_ && tracer()->SurvivalEventsRecorded()) {
        ...
        const size_t new_old_generation_allocation_limit = std::max(OldGenerationSizeOfObjects() + minimum_growing_step,
            static_cast<size_t>(static_cast<double>(old_generation_allocation_limit()) * (tracer()->AverageSurvivalRatio() / 100)));
        if (new_old_generation_allocation_limit < old_generation_allocation_limit()) {
        ...
        if (UseGlobalMemoryScheduling()) {
            const size_t new_global_memory_limit = std::max(
                GlobalSizeOfObjects() + minimum_growing_step, static_cast<size_t>(static_cast<double>(global_allocation_limit_) * (tracer()->AverageSurvivalRatio() / 100)));
...
void CreateFillerObjectAtImpl(Heap* heap, Address addr, int size, ClearFreedMemoryMode clear_memory_mode)
{
    if (size == 0)
        return;
    DCHECK_IMPLIES(V8_COMPRESS_POINTERS_8GB_BOOL, IsAligned(addr, kObjectAlignment8GbHeap));
    ...
    if (size == kTaggedSize) {
        filler.set_map_after_allocation(roots.unchecked_one_pointer_filler_map(), SKIP_WRITE_BARRIER);
    } else if (size == 2 * kTaggedSize) {
        filler.set_map_after_allocation(roots.unchecked_two_pointer_filler_map(), SKIP_WRITE_BARRIER);
        ...
        filler.set_map_after_allocation(roots.unchecked_free_space_map(), SKIP_WRITE_BARRIER);
        FreeSpace::cast(filler).set_size(size, kRelaxedStore);
        if (clear_memory_mode == ClearFreedMemoryMode::kClearFreedMemory) {
            MemsetTagged(ObjectSlot(addr) + 2, Object(kClearedFreeMemoryValue), (size / kTaggedSize) - 2);
        }
    }

    // At this point, we may be deserializing the heap from a snapshot, and
    // none of the maps have been created yet and are nullptr.
    DCHECK((filler.map_slot().contains_map_value(kNullAddress) && !heap->deserialization_complete()) || filler.map(heap->isolate()).IsMap());
}
...
void Heap::CreateFillerObjectAtBackground(Address addr, int size)
{
    ...
    CreateFillerObjectAtRaw(addr, size, ClearFreedMemoryMode::kDontClearFreedMemory, ClearRecordedSlots::kNo, VerifyNoSlotsRecorded::kNo);
}
...
{
    CreateFillerObjectAtRaw(addr, size, ClearFreedMemoryMode::kDontClearFreedMemory, ClearRecordedSlots::kNo, VerifyNoSlotsRecorded::kYes);
}

void Heap::CreateFillerObjectAtRaw(
    Address addr, int size, ClearFreedMemoryMode clear_memory_mode, ClearRecordedSlots clear_slots_mode, VerifyNoSlotsRecorded verify_no_slots_recorded)
{
...
{
    if (V8_ENABLE_THIRD_PARTY_HEAP_BOOL)
        return third_party_heap::Heap::InLargeObjectSpace(object.address()) || third_party_heap::Heap::InSpace(object.address(), CODE_LO_SPACE);
...
void Heap::OnMoveEvent(HeapObject source, HeapObject target, int size_in_bytes)
{
    HeapProfiler* heap_profiler = isolate_->heap_profiler();
    if (heap_profiler->is_tracking_object_moves()) {
        heap_profiler->ObjectMoveEvent(source.address(), target.address(), size_in_bytes);
    }
...
FixedArrayBase Heap::LeftTrimFixedArray(FixedArrayBase object, int elements_to_trim)
{
    ...
    incremental_marking()->NotifyLeftTrimming(object, HeapObject::FromAddress(new_start));
    ...
    // we still do it.
    CreateFillerObjectAtRaw(old_start, bytes_to_trim, ClearFreedMemoryMode::kClearFreedMemory, MayContainRecordedSlots(object) ? ClearRecordedSlots::kYes : ClearRecordedSlots::kNo,
        VerifyNoSlotsRecorded::kYes);

    // Initialize header of the trimmed array. Since left trimming is only
    // performed on pages which are not concurrently swept creating a filler
    // object does not require synchronization.
    RELAXED_WRITE_FIELD(object, bytes_to_trim, Object(MapWord::FromMap(map).ptr()));
...
void Heap::RightTrimWeakFixedArray(WeakFixedArray object, int elements_to_trim)
{
...
template <typename T> void Heap::CreateFillerForArray(T object, int elements_to_trim, int bytes_to_trim)
{
    ...
    if (!IsLargeObject(object)) {
        ...
        if (incremental_marking()->black_allocation() && marking_state()->IsBlackOrGrey(filler)) {
            Page* page = Page::FromAddress(new_end);
            marking_state()->bitmap(page)->ClearRange(page->AddressToMarkbitIndex(new_end), page->AddressToMarkbitIndex(new_end + bytes_to_trim));
        }
    } else if (clear_slots) {
        // Large objects are not swept, so it is not necessary to clear the
        // recorded slot.
        MemsetTagged(ObjectSlot(new_end), Object(kClearedFreeMemoryValue), (old_end - new_end) / kTaggedSize);
    }
...
void Heap::MakeHeapIterable()
{
    ...
    safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->MakeLinearAllocationAreaIterable(); });
...
{
    safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->FreeLinearAllocationArea(); });

    if (isolate()->is_shared_space_isolate()) {
        isolate()->global_safepoint()->IterateClientIsolates([](Isolate* client) { client->heap()->FreeSharedLinearAllocationAreas(); });
    }
    ...
    if (new_space())
        new_space()->FreeLinearAllocationArea();
}

void Heap::FreeSharedLinearAllocationAreas()
{
    if (!isolate()->has_shared_heap())
        return;
    safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->FreeSharedLinearAllocationArea(); });
...
void Heap::MarkSharedLinearAllocationAreasBlack()
{
    ...
    safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->MarkSharedLinearAllocationAreaBlack(); });
...
bool Heap::HasLowYoungGenerationAllocationRate()
{
    double mu = ComputeMutatorUtilization(
        "Young generation", tracer()->NewSpaceAllocationThroughputInBytesPerMillisecond(), tracer()->ScavengeSpeedInBytesPerMillisecond(kForSurvivedObjects));
...
{
    double mu = ComputeMutatorUtilization(
        "Old generation", tracer()->OldGenerationAllocationThroughputInBytesPerMillisecond(), tracer()->CombinedMarkCompactSpeedInBytesPerMillisecond());
...
bool Heap::HasLowEmbedderAllocationRate()
{
    ...
    DCHECK_NOT_NULL(local_embedder_heap_tracer());
    double mu = ComputeMutatorUtilization("Embedder", tracer()->CurrentEmbedderAllocationThroughputInBytesPerMillisecond(), tracer()->EmbedderSpeedInBytesPerMillisecond());
    const double kHighMutatorUtilization = 0.993;
    return mu > kHighMutatorUtilization;
}

bool Heap::HasLowAllocationRate() { return HasLowYoungGenerationAllocationRate() && HasLowOldGenerationAllocationRate() && HasLowEmbedderAllocationRate(); }
...
bool Heap::ShouldOptimizeForMemoryUsage()
{
    ...
    return v8_flags.optimize_for_size || isolate()->IsIsolateInBackground() || isolate()->IsMemorySavingsModeActive() || HighMemoryPressure()
...
bool Heap::ShouldReduceNewSpaceSize() const
{
    ...
    const double allocation_throughput = tracer_->CurrentAllocationThroughputInBytesPerMillisecond();

    return ShouldReduceMemory() || ((allocation_throughput != 0) && (allocation_throughput < kLowAllocationThroughput));
}

void Heap::ReduceNewSpaceSize()
{
    if (!ShouldReduceNewSpaceSize())
        return;
...
size_t Heap::NewSpaceCapacity() { return new_space() ? new_space()->Capacity() : 0; }

void Heap::FinalizeIncrementalMarkingIfComplete(GarbageCollectionReason gc_reason)
{
...
void Heap::NotifyObjectLayoutChange(HeapObject object, const DisallowGarbageCollection&, InvalidateRecordedSlots invalidate_recorded_slots, int new_size)
{
    if (invalidate_recorded_slots == InvalidateRecordedSlots::kYes) {
        ...
        if (incremental_marking()->IsMarking()) {
            ...
            if (may_contain_recorded_slots && incremental_marking()->IsCompacting()) {
                MemoryChunk::FromHeapObject(object)->RegisterObjectWithInvalidatedSlots<OLD_TO_OLD>(object, new_size);
            }
        }

        if (may_contain_recorded_slots) {
            MemoryChunk::FromHeapObject(object)->RegisterObjectWithInvalidatedSlots<OLD_TO_NEW>(object, new_size);
            MemoryChunk::FromHeapObject(object)->RegisterObjectWithInvalidatedSlots<OLD_TO_SHARED>(object, new_size);
        }
    }
#ifdef VERIFY_HEAP
    if (v8_flags.verify_heap) {
...
void Heap::NotifyObjectSizeChange(
    HeapObject object, int old_size, int new_size, ClearRecordedSlots clear_recorded_slots, enum UpdateInvalidatedObjectSize update_invalidated_object_size)
{
    ...
    DCHECK_IMPLIES(!is_main_thread, update_invalidated_object_size == UpdateInvalidatedObjectSize::kNo);
    ...
    if (update_invalidated_object_size == UpdateInvalidatedObjectSize::kYes) {
        ...
        if (is_main_thread) {
            ...
            DCHECK(!chunk->RegisteredObjectWithInvalidatedSlots<OLD_TO_NEW>(object));
            DCHECK(!chunk->RegisteredObjectWithInvalidatedSlots<OLD_TO_SHARED>(object));
            DCHECK_IMPLIES(incremental_marking()->IsCompacting(), !chunk->RegisteredObjectWithInvalidatedSlots<OLD_TO_OLD>(object));
        }
#endif
    }
    ...
    CreateFillerObjectAtRaw(filler, filler_size, clear_memory_mode, clear_recorded_slots, verify_no_slots_recorded);
}
...
void Heap::UpdateInvalidatedObjectSize(HeapObject object, int new_size)
{
    ...
    if (incremental_marking()->IsCompacting() || gc_state() == MARK_COMPACT) {
        MemoryChunk::FromHeapObject(object)->UpdateInvalidatedObjectSize<OLD_TO_OLD>(object, new_size);
    }

    MemoryChunk::FromHeapObject(object)->UpdateInvalidatedObjectSize<OLD_TO_NEW>(object, new_size);
    MemoryChunk::FromHeapObject(object)->UpdateInvalidatedObjectSize<OLD_TO_SHARED>(object, new_size);
}
...
bool Heap::PerformIdleTimeAction(GCIdleTimeAction action, GCIdleTimeHeapState heap_state, double deadline_in_ms)
{
    bool result = false;
    switch (action) {
...
void Heap::IdleNotificationEpilogue(GCIdleTimeAction action, GCIdleTimeHeapState heap_state, double start_ms, double deadline_in_ms)
{
    const double idle_time_in_ms = deadline_in_ms - start_ms;
    const double deadline_difference = deadline_in_ms - MonotonicallyIncreasingTimeInMs();

    if (v8_flags.trace_idle_notification) {
        isolate_->PrintWithTimestamp("Idle notification: requested idle time %.2f ms, used idle time %.2f "
                                     "ms, deadline usage %.2f ms [",
            idle_time_in_ms, idle_time_in_ms - deadline_difference, deadline_difference);
        switch (action) {
        case GCIdleTimeAction::kDone:
            PrintF("done");
            break;
...
double Heap::MonotonicallyIncreasingTimeInMs() const { return V8::GetCurrentPlatform()->MonotonicallyIncreasingTime() * static_cast<double>(base::Time::kMillisecondsPerSecond); }
...
bool Heap::IdleNotification(double deadline_in_seconds)
{
    ...
    NestedTimedHistogramScope idle_notification_scope(isolate_->counters()->gc_idle_notification());
    TRACE_EVENT0("v8", "V8.GCIdleNotification");
    double start_ms = MonotonicallyIncreasingTimeInMs();
    double idle_time_in_ms = deadline_in_ms - start_ms;

    tracer()->SampleAllocation(start_ms, NewSpaceAllocationCounter(), OldGenerationAllocationCounter(), EmbedderAllocationCounter());
...
void Heap::CheckMemoryPressure()
{
    ...
    if (memory_pressure_level == MemoryPressureLevel::kCritical) {
        ...
        if (v8_flags.incremental_marking && incremental_marking()->IsStopped()) {
            TRACE_EVENT0("devtools.timeline,v8", "V8.CheckMemoryPressure");
            StartIncrementalMarking(kReduceMemoryFootprintMask, GarbageCollectionReason::kMemoryPressure);
        }
    }
}
...
void Heap::CollectGarbageOnMemoryPressure()
{
    ...
    int64_t potential_garbage = (CommittedMemory() - SizeOfObjects()) + external_memory_.total();
...
void Heap::MemoryPressureNotification(MemoryPressureLevel level, bool is_isolate_locked)
{
    ...
    if ((previous != MemoryPressureLevel::kCritical && level == MemoryPressureLevel::kCritical)
        ...
        if (is_isolate_locked) {
            ...
            auto taskrunner = V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate()));
...
void Heap::AppendArrayBufferExtension(JSArrayBuffer object, ArrayBufferExtension* extension)
{
    // ArrayBufferSweeper is managing all counters and updating Heap counters.
    array_buffer_sweeper_->Append(object, extension);
}

void Heap::DetachArrayBufferExtension(JSArrayBuffer object, ArrayBufferExtension* extension)
{
    // ArrayBufferSweeper is managing all counters and updating Heap counters.
    return array_buffer_sweeper_->Detach(object, extension);
}

void Heap::AutomaticallyRestoreInitialHeapLimit(double threshold_percent) { initial_max_old_generation_size_threshold_ = initial_max_old_generation_size_ * threshold_percent; }
...
bool Heap::InvokeNearHeapLimitCallback()
{
    if (near_heap_limit_callbacks_.size() > 0) {
        ...
        if (heap_limit > max_old_generation_size()) {
            set_max_old_generation_size(std::min(heap_limit, AllocatorLimitOnMaxOldGenerationSize()));
            return true;
        }
    }
    return false;
...
bool Heap::MeasureMemory(std::unique_ptr<v8::MeasureMemoryDelegate> delegate, v8::MeasureMemoryExecution execution)
{
    ...
    for (auto& current : contexts) {
        if (delegate->ShouldMeasure(v8::Utils::ToLocal(Handle<Context>::cast(current)))) {
...
std::unique_ptr<v8::MeasureMemoryDelegate> Heap::MeasureMemoryDelegate(Handle<NativeContext> context, Handle<JSPromise> promise, v8::MeasureMemoryMode mode)
{
    return i::MemoryMeasurement::DefaultDelegate(isolate_, context, promise, mode);
}
...
const char* Heap::GarbageCollectionReasonToString(GarbageCollectionReason gc_reason)
{
    switch (gc_reason) {
...
bool Heap::Contains(HeapObject value) const
{
    ...
    return (new_space_ && new_space_->Contains(value)) || old_space_->Contains(value) || code_space_->Contains(value) || (map_space_ && map_space_->Contains(value))
        || (shared_space_ && shared_space_->Contains(value)) || lo_space_->Contains(value) || code_lo_space_->Contains(value) || (new_lo_space_ && new_lo_space_->Contains(value))
...
bool Heap::SharedHeapContains(HeapObject value) const
{
    if (shared_allocation_space_) {
        ...
        if (shared_lo_allocation_space_->Contains(value))
            return true;
        if (shared_map_allocation_space_ && shared_map_allocation_space_->Contains(value))
            return true;
    }
...
bool Heap::ShouldMarkSharedHeap() const { return isolate()->is_shared() || isolate()->is_shared_space_isolate(); }
...
void Heap::RegisterCodeObject(Handle<Code> code)
{
    ...
    if (!V8_ENABLE_THIRD_PARTY_HEAP_BOOL && code_space()->Contains(addr)) {
        MemoryChunk::FromHeapObject(*code)->GetCodeObjectRegistry()->RegisterNewlyAllocatedCodeObject(addr);
    }
}
...
public:
    ...
    void VisitRootPointer(Root root, const char* description, FullObjectSlot p) override { FixHandle(p); }
...
private:
    inline void FixHandle(FullObjectSlot p)
    {
        ...
        if (!current.map_word(cage_base(), kRelaxedLoad).IsForwardingAddress() && current.IsFreeSpaceOrFiller(cage_base())) {
#ifdef DEBUG
            // We need to find a FixedArrayBase map after walking the fillers.
            while (!current.map_word(cage_base(), kRelaxedLoad).IsForwardingAddress() && current.IsFreeSpaceOrFiller(cage_base())) {
            ...
            DCHECK(current.map_word(cage_base(), kRelaxedLoad).IsForwardingAddress() || current.IsFixedArrayBase(cage_base()));
#endif // DEBUG
            p.store(Smi::zero());
        }
    }
...
public:
    ...
    void Synchronize(VisitorSynchronization::SyncTag tag) final { actual_visitor_->Synchronize(tag); }

private:
    void MaybeForwardSlot(Root root, const char* description, FullObjectSlot slot)
    {
        Object object = *slot;
...
void Heap::IterateRootsIncludingClients(RootVisitor* v, base::EnumSet<SkipRoot> options)
{
    ...
    if (isolate()->is_shared_heap_isolate()) {
        ...
        isolate()->global_safepoint()->IterateClientIsolates([v = &client_root_visitor, options](Isolate* client) { client->heap()->IterateRoots(v, options); });
    }
}
...
void Heap::IterateRootsFromStackIncludingClient(RootVisitor* v)
{
    ...
    if (isolate()->is_shared_heap_isolate()) {
        ClientRootVisitor client_root_visitor(v);
        isolate()->global_safepoint()->IterateClientIsolates([v = &client_root_visitor](Isolate* client) { client->heap()->IterateStackRoots(v); });
    }
}

void Heap::IterateWeakGlobalHandles(RootVisitor* v) { isolate_->global_handles()->IterateWeakRoots(v); }
...
void Heap::ConfigureHeap(const v8::ResourceConstraints& constraints)
{
    ...
    // Initialize max_old_generation_size_ and max_global_memory_.
    {
        ...
        max_old_generation_size = RoundDown<Page::kPageSize>(max_old_generation_size);
    ...
    // Initialize initial_semispace_size_.
    {
        ...
        initial_semispace_size_ = RoundDown<Page::kPageSize>(initial_semispace_size_);
    }
    ...
    // Initialize initial_old_space_size_.
    {
        ...
        if (v8_flags.initial_heap_size > 0) {
            ...
            initial_old_generation_size_ = initial_heap_size > young_generation_size ? initial_heap_size - young_generation_size : 0;
            old_generation_size_configured_ = true;
        }
        if (v8_flags.initial_old_space_size > 0) {
            initial_old_generation_size_ = static_cast<size_t>(v8_flags.initial_old_space_size) * MB;
            old_generation_size_configured_ = true;
        }
        initial_old_generation_size_ = std::min(initial_old_generation_size_, max_old_generation_size() / 2);
        initial_old_generation_size_ = RoundDown<Page::kPageSize>(initial_old_generation_size_);
    }
    ...
    set_old_generation_allocation_limit(initial_old_generation_size_);
    global_allocation_limit_ = GlobalMemorySizeFromV8Size(old_generation_allocation_limit());
    initial_max_old_generation_size_ = max_old_generation_size();

    // We rely on being able to allocate new arrays in paged spaces.
    DCHECK(
        kMaxRegularHeapObjectSize >= (JSArray::kHeaderSize + FixedArray::SizeFor(JSArray::kInitialMaxFastElementArray) + ALIGN_TO_ALLOCATION_ALIGNMENT(AllocationMemento::kSize)));
...
size_t Heap::EmbedderSizeOfObjects() const { return local_embedder_heap_tracer() ? local_embedder_heap_tracer()->used_size() : 0; }

size_t Heap::GlobalSizeOfObjects() { return OldGenerationSizeOfObjects() + EmbedderSizeOfObjects(); }

uint64_t Heap::AllocatedExternalMemorySinceMarkCompact() { return external_memory_.AllocatedSinceMarkCompact(); }
...
bool Heap::AllocationLimitOvershotByLargeMargin()
{
    ...
    uint64_t size_now = OldGenerationSizeOfObjects() + AllocatedExternalMemorySinceMarkCompact();

    const size_t v8_overshoot = old_generation_allocation_limit() < size_now ? size_now - old_generation_allocation_limit() : 0;
    const size_t global_overshoot = global_allocation_limit_ < GlobalSizeOfObjects() ? GlobalSizeOfObjects() - global_allocation_limit_ : 0;
    ...
    const size_t v8_margin = std::min(std::max(old_generation_allocation_limit() / 2, kMarginForSmallHeaps), (max_old_generation_size() - old_generation_allocation_limit()) / 2);
    const size_t global_margin = std::min(std::max(global_allocation_limit_ / 2, kMarginForSmallHeaps), (max_global_memory_size_ - global_allocation_limit_) / 2);
...
{
    return isolate()->rail_mode() == PERFORMANCE_LOAD && !AllocationLimitOvershotByLargeMargin()
        && MonotonicallyIncreasingTimeInMs() < isolate()->LoadStartTimeMs() + kMaxLoadTimeMs;
}
...
double Heap::PercentToOldGenerationLimit()
{
    ...
    double size_now = OldGenerationSizeOfObjects() + AllocatedExternalMemorySinceMarkCompact();
...
Heap::IncrementalMarkingLimit Heap::IncrementalMarkingLimitReached()
{
        ...
        if (current_percent > 0) {
            ...
            if (v8_flags.fuzzer_gc_analysis) {
                ...
                if (current_percent < 100) {
                    max_marking_limit_reached_ = std::max<double>(max_marking_limit_reached_, current_percent);
                }
            } else if (current_percent >= stress_marking_percentage_.load(std::memory_order_relaxed)) {
                stress_marking_percentage_.store(NextStressMarkingLimit(), std::memory_order_relaxed);
                return IncrementalMarkingLimit::kHardLimit;
            }
        }
    }
    ...
    const base::Optional<size_t> global_memory_available = GlobalMemoryAvailable();

    if (old_generation_space_available > NewSpaceCapacity() && (!global_memory_available || global_memory_available > NewSpaceCapacity())) {
        if (local_embedder_heap_tracer()->InUse() && !old_generation_size_configured_ && gc_count_ == 0) {
...
bool Heap::ShouldStressCompaction() const { return v8_flags.stress_compaction && (gc_count_ & 1) != 0; }
...
void Heap::SetUp(LocalHeap* main_thread_local_heap)
{
    ...
    if (isolate_->RequiresCodeRange() || code_range_size_ != 0) {
        ...
        if (COMPRESS_POINTERS_IN_SHARED_CAGE_BOOL) {
            ...
            code_range_ = CodeRange::EnsureProcessWideCodeRange(isolate_->page_allocator(), requested_size);
        } else {
            code_range_ = std::make_shared<CodeRange>();
            if (!code_range_->InitReservation(isolate_->page_allocator(), requested_size)) {
    ...
    memory_allocator_.reset(new MemoryAllocator(isolate_, code_page_allocator, MaxReserved()));
    ...
    if (V8_UNLIKELY(v8_flags.trace_gc_heap_layout)) {
        ...
        if (V8_UNLIKELY(!v8_flags.trace_gc_heap_layout_ignore_minor_gc)) {
            gc_type = static_cast<v8::GCType>(gc_type | kGCTypeScavenge | kGCTypeMinorMarkCompact);
        }
...
void Heap::SetUpSpaces(LinearAllocationArea& new_allocation_info, LinearAllocationArea& old_allocation_info)
{
    ...
    if (has_young_gen) {
        if (v8_flags.minor_mc) {
            space_[NEW_SPACE] = std::make_unique<PagedNewSpace>(this, initial_semispace_size_, max_semi_space_size_, new_allocation_info);
        } else {
            space_[NEW_SPACE] = std::make_unique<SemiSpaceNewSpace>(this, initial_semispace_size_, max_semi_space_size_, new_allocation_info);
        }
    ...
    embedder_roots_handler_ = &local_embedder_heap_tracer()->default_embedder_roots_handler();
    ...
    if (new_space()) {
        if (v8_flags.concurrent_minor_mc_marking) {
            ...
            minor_mc_task_observer_.reset(new MinorMCTaskObserver(this, MinorMCTaskObserver::kStepSize));
            ...
            scavenge_task_observer_.reset(new ScavengeTaskObserver(this, ScavengeJob::kStepSize));
            new_space()->AddAllocationObserver(scavenge_task_observer_.get());
        }
    }

    SetGetExternallyAllocatedMemoryInBytesCallback(DefaultGetExternallyAllocatedMemoryInBytesCallback);

    if (v8_flags.stress_marking > 0) {
        stress_marking_percentage_ = NextStressMarkingLimit();
        stress_marking_observer_ = new StressMarkingObserver(this);
        AddAllocationObserversToAllSpaces(stress_marking_observer_, stress_marking_observer_);
    }
    ...
    if (isolate()->shared_space_isolate()) {
        ...
        shared_space_allocator_ = std::make_unique<ConcurrentAllocator>(main_thread_local_heap(), heap->shared_space_);
        ...
        shared_space_allocator_ = std::make_unique<ConcurrentAllocator>(main_thread_local_heap(), shared_heap->old_space());

        if (shared_heap->map_space()) {
            shared_map_allocator_ = std::make_unique<ConcurrentAllocator>(main_thread_local_heap(), shared_heap->map_space());
        }
...
void Heap::InitializeHashSeed()
{
    ...
    ReadOnlyRoots(this).hash_seed().copy_in(0, reinterpret_cast<byte*>(&new_hash_seed), kInt64Size);
}
...
void Heap::NotifyOldGenerationExpansion(AllocationSpace space, MemoryChunk* chunk)
{
    ...
    if (memory_reducer() != nullptr && old_generation_capacity_after_bootstrap_ && ms_count_ == 0
        && OldGenerationCapacity() >= old_generation_capacity_after_bootstrap_ + kMemoryReducerActivationThreshold && v8_flags.memory_reducer_for_small_heaps) {
        MemoryReducer::Event event;
        event.type = MemoryReducer::kPossibleGarbage;
        event.time_ms = MonotonicallyIncreasingTimeInMs();
        memory_reducer()->NotifyPossibleGarbage(event);
...
EmbedderHeapTracer* Heap::GetEmbedderHeapTracer() const { return local_embedder_heap_tracer()->remote_tracer(); }
...
void Heap::SetEmbedderRootsHandler(EmbedderRootsHandler* handler) { embedder_roots_handler_ = handler; }

EmbedderRootsHandler* Heap::GetEmbedderRootsHandler() const { return embedder_roots_handler_; }
...
void Heap::SetStackStart(void* stack_start) { stack_->SetStackStart(stack_start); }
...
void Heap::RegisterExternallyReferencedObject(Address* location)
{
    ...
    DCHECK(incremental_marking()->IsMarking() || mark_compact_collector()->in_use());
...
void Heap::TearDown()
{
    ...
    if (v8_flags.stress_marking > 0) {
        RemoveAllocationObserversFromAllSpaces(stress_marking_observer_, stress_marking_observer_);
    ...
    {
        CodePageHeaderModificationScope rwx_write_scope("Deletion of CODE_SPACE and CODE_LO_SPACE requires write access to "
                                                        "Code page headers");
...
void Heap::RemoveGCPrologueCallback(v8::Isolate::GCCallbackWithData callback, void* data) { gc_prologue_callbacks_.Remove(callback, data); }
...
void Heap::RemoveGCEpilogueCallback(v8::Isolate::GCCallbackWithData callback, void* data) { gc_epilogue_callbacks_.Remove(callback, data); }
...
Handle<WeakArrayList> CompactWeakArrayList(Heap* heap, Handle<WeakArrayList> array, AllocationType allocation)
{
    ...
    Handle<WeakArrayList> new_array = WeakArrayList::EnsureSpace(heap->isolate(), handle(ReadOnlyRoots(heap).empty_weak_array_list(), heap->isolate()), new_length, allocation);
...
void Heap::CompactWeakArrayLists()
{
    ...
    for (auto& prototype_info : prototype_infos) {
        Handle<WeakArrayList> array(WeakArrayList::cast(prototype_info->prototype_users()), isolate());
        DCHECK(InOldSpace(*array) || *array == ReadOnlyRoots(this).empty_weak_array_list());
        WeakArrayList new_array = PrototypeUsers::Compact(array, this, JSObject::PrototypeRegistryCompactionCallback, AllocationType::kOld);
...
void Heap::AddRetainedMap(Handle<NativeContext> context, Handle<Map> map)
{
    ...
    Handle<WeakArrayList> array(WeakArrayList::cast(context->retained_maps()), isolate());
    if (array->IsFull()) {
        CompactRetainedMaps(*array);
    }
    array = WeakArrayList::AddToEnd(isolate(), array, MaybeObjectHandle::Weak(map), MaybeObjectHandle(Smi::FromInt(v8_flags.retain_maps_for_n_gc), isolate()));
...
void Heap::FatalProcessOutOfMemory(const char* location) { V8::FatalProcessOutOfMemory(isolate(), location, V8::kHeapOOM); }
...
public:
    ...
    ~CheckHandleCountVisitor() override { CHECK_GT(HandleScope::kCheckHandleThreshold, handle_count_); }
    void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override { handle_count_ += end - start; }
...
void Heap::VerifyClearedSlot(HeapObject object, ObjectSlot slot)
{
    ...
    CHECK_IMPLIES(RememberedSet<OLD_TO_NEW>::Contains(page, slot.address()), page->RegisteredObjectWithInvalidatedSlots<OLD_TO_NEW>(object));
    CHECK_IMPLIES(RememberedSet<OLD_TO_OLD>::Contains(page, slot.address()), page->RegisteredObjectWithInvalidatedSlots<OLD_TO_OLD>(object));
#endif
}
...
PagedSpace* PagedSpaceIterator::Next()
{
    ...
    if (counter_ > LAST_GROWABLE_PAGED_SPACE)
        return nullptr;
...
private:
    ...
    static constexpr intptr_t kLogicalChunkAlignment = (static_cast<uintptr_t>(1) << kPageSizeBits);

    static constexpr intptr_t kLogicalChunkAlignmentMask = kLogicalChunkAlignment - 1;
    ...
    public:
        ...
        void VisitMapPointer(HeapObject object) override { MarkHeapObject(Map::unchecked_cast(object.map(cage_base()))); }
        void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) override { MarkPointers(MaybeObjectSlot(start), MaybeObjectSlot(end)); }

        void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) final { MarkPointers(start, end); }
        ...
        void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) final { MarkHeapObject(rinfo->target_object(cage_base())); }

        void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override { MarkPointersImpl(start, end); }
        void VisitRootPointers(Root root, const char* description, OffHeapObjectSlot start, OffHeapObjectSlot end) override { MarkPointersImpl(start, end); }
    ...
    private:
        void MarkPointers(MaybeObjectSlot start, MaybeObjectSlot end) { MarkPointersImpl(start, end); }
    ...
    std::unordered_map<Address, std::unordered_set<HeapObject, Object::Hasher>*> reachable_;
};
...
size_t Heap::YoungArrayBufferBytes() { return array_buffer_sweeper()->YoungBytes(); }

size_t Heap::OldArrayBufferBytes() { return array_buffer_sweeper()->OldBytes(); }
...
void Heap::SetBuiltinsConstantsTable(FixedArray cache) { set_builtins_constants_table(cache); }

void Heap::SetDetachedContexts(WeakArrayList detached_contexts) { set_detached_contexts(detached_contexts); }
...
void Heap::EnqueueDirtyJSFinalizationRegistry(
    JSFinalizationRegistry finalization_registry, std::function<void(HeapObject object, ObjectSlot slot, Object target)> gc_notify_updated_slot)
{
    // Add a FinalizationRegistry to the tail of the dirty list.
    DCHECK(!HasDirtyJSFinalizationRegistries() || dirty_js_finalization_registries_list().IsJSFinalizationRegistry());
    ...
    if (dirty_js_finalization_registries_list_tail().IsUndefined(isolate())) {
        ...
        JSFinalizationRegistry tail = JSFinalizationRegistry::cast(dirty_js_finalization_registries_list_tail());
...
MaybeHandle<JSFinalizationRegistry> Heap::DequeueDirtyJSFinalizationRegistry()
{
    ...
    if (HasDirtyJSFinalizationRegistries()) {
        Handle<JSFinalizationRegistry> head(JSFinalizationRegistry::cast(dirty_js_finalization_registries_list()), isolate());
...
void Heap::RemoveDirtyFinalizationRegistriesOnContext(NativeContext context)
{
    ...
    while (!current.IsUndefined(isolate)) {
        JSFinalizationRegistry finalization_registry = JSFinalizationRegistry::cast(current);
        if (finalization_registry.native_context() == context) {
            if (prev.IsUndefined(isolate)) {
                ...
                JSFinalizationRegistry::cast(prev).set_next_dirty(finalization_registry.next_dirty());
            }
...
void Heap::ClearKeptObjects() { set_weak_refs_keep_during_job(ReadOnlyRoots(isolate()).undefined_value()); }

size_t Heap::NumberOfTrackedHeapObjectTypes() { return ObjectStats::OBJECT_STATS_COUNT; }
...
void VerifyPointersVisitor::VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) { VerifyPointers(host, MaybeObjectSlot(start), MaybeObjectSlot(end)); }

void VerifyPointersVisitor::VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) { VerifyPointers(host, start, end); }

void VerifyPointersVisitor::VisitCodePointer(HeapObject host, CodeObjectSlot slot)
{
    CHECK(V8_EXTERNAL_CODE_SPACE_BOOL);
    Object maybe_code = slot.load(code_cage_base());
    HeapObject code;
...
void VerifyPointersVisitor::VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) { VerifyPointersImpl(start, end); }

void VerifyPointersVisitor::VisitRootPointers(Root root, const char* description, OffHeapObjectSlot start, OffHeapObjectSlot end) { VerifyPointersImpl(start, end); }
...
template <typename TSlot> void VerifyPointersVisitor::VerifyPointersImpl(TSlot start, TSlot end)
{
    for (TSlot slot = start; slot < end; ++slot) {
        ...
        if (object.GetHeapObject(&heap_object)) {
            ...
            CHECK(object.IsSmi() || object.IsCleared() || MapWord::IsPacked(object.ptr()));
        }
    }
}

void VerifyPointersVisitor::VerifyPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end)
{
    ...
    DCHECK_EQ(ObjectFields::kMaybePointers, Map::ObjectFieldsFrom(host.map(cage_base()).visitor_id()));
...
void VerifyPointersVisitor::VisitEmbeddedPointer(Code host, RelocInfo* rinfo) { VerifyHeapObjectImpl(rinfo->target_object(cage_base())); }
...
size_t Heap::EmbedderAllocationCounter() const { return local_embedder_heap_tracer() ? local_embedder_heap_tracer()->allocated_size() : 0; }
...
CodeLookupResult Heap::GcSafeCastToCode(HeapObject object, Address inner_pointer)
{
...
bool Heap::GcSafeCodeContains(Code code, Address addr)
{
    ...
    Builtin maybe_builtin = OffHeapInstructionStream::TryLookupCode(isolate(), addr);
    if (Builtins::IsBuiltinId(maybe_builtin) && code.builtin_id() == maybe_builtin) {
        return true;
    }
...
CodeLookupResult Heap::GcSafeFindCodeForInnerPointer(Address inner_pointer, bool die_on_unsuccessful_lookup)
{
    ...
    if (V8_LIKELY(code_space()->Contains(inner_pointer))) {
        ...
        Address start = page->GetCodeObjectRegistry()->GetCodeObjectStartFromInnerAddress(inner_pointer);
        return GcSafeCastToCode(HeapObject::FromAddress(start), inner_pointer);
    }

    if (!die_on_unsuccessful_lookup)
        return {};
    ...
    void* code_range_embedded_blob_code_copy = code_range ? code_range->embedded_blob_code_copy() : nullptr;
    Address flags = (isolate()->is_short_builtin_calls_enabled() ? 1 : 0) | (code_range ? 2 : 0) | static_cast<Address>(max_old_generation_size());

    isolate()->PushParamsAndDie(reinterpret_cast<void*>(inner_pointer), const_cast<uint8_t*>(isolate()->embedded_blob_code()),
        const_cast<uint8_t*>(Isolate::CurrentEmbeddedBlobCode()), code_range_embedded_blob_code_copy, reinterpret_cast<void*>(Isolate::CurrentEmbeddedBlobCodeSize()),
        reinterpret_cast<void*>(flags));

    UNREACHABLE();
}

CodeLookupResult Heap::GcSafeFindCodeForInnerPointerForPrinting(Address inner_pointer)
{
    if (InSpaceSlow(inner_pointer, i::CODE_SPACE) || InSpaceSlow(inner_pointer, i::CODE_LO_SPACE) || i::OffHeapInstructionStream::PcIsOffHeap(isolate(), inner_pointer)) {
        CodeLookupResult result = GcSafeFindCodeForInnerPointer(inner_pointer, false);
        if (result.IsFound())
            return result;
    }
    ...
    if (read_only_space()->ContainsSlow(inner_pointer)) {
        ...
        for (HeapObject object = iterator.Next(); !object.is_null(); object = iterator.Next()) {
            ...
            if (inner_pointer >= code.address() && inner_pointer < code.address() + code.Size()) {
                return CodeLookupResult { code };
            }
        }
    }
...
void Heap::CombinedGenerationalAndSharedBarrierSlow(HeapObject object, Address slot, HeapObject value)
{
...
void Heap::CombinedGenerationalAndSharedEphemeronBarrierSlow(EphemeronHashTable table, Address slot, HeapObject value)
{
    MemoryChunk* value_chunk = MemoryChunk::FromHeapObject(value);
...
void Heap::GenerationalBarrierSlow(HeapObject object, Address slot, HeapObject value)
{
    MemoryChunk* chunk = MemoryChunk::FromHeapObject(object);
    RememberedSet<OLD_TO_NEW>::Insert<AccessMode::NON_ATOMIC>(chunk, slot);
}
...
void Heap::EphemeronKeyWriteBarrierFromCode(Address raw_object, Address key_slot_address, Isolate* isolate)
{
    EphemeronHashTable table = EphemeronHashTable::cast(Object(raw_object));
    ObjectSlot key_slot(key_slot_address);
    CombinedEphemeronWriteBarrier(table, key_slot, *key_slot, UPDATE_WRITE_BARRIER);
}
...
template <int kModeMask, typename TSlot> void Heap::WriteBarrierForRangeImpl(MemoryChunk* source_page, HeapObject object, TSlot start_slot, TSlot end_slot)
{
    ...
    CodeTPageHeaderModificationScope rwx_write_scope("Marking CodeT objects might require write access to the CodeT page "
                                                     "header");
    ...
    for (TSlot slot = start_slot; slot < end_slot; ++slot) {
        ...
        if (kModeMask & kDoGenerationalOrShared) {
            if (Heap::InYoungGeneration(value_heap_object)) {
                RememberedSet<OLD_TO_NEW>::Insert<AccessMode::NON_ATOMIC>(source_page, slot.address());
            } else if (value_heap_object.InSharedWritableHeap()) {
                RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::ATOMIC>(source_page, slot.address());
            }
        }

        if ((kModeMask & kDoMarking) && marking_barrier->MarkValue(object, value_heap_object)) {
            if (kModeMask & kDoEvacuationSlotRecording) {
                collector->RecordSlot(source_page, HeapObjectSlot(slot), value_heap_object);
            }
        }
    }
}
...
template <typename TSlot> void Heap::WriteBarrierForRange(HeapObject object, TSlot start_slot, TSlot end_slot)
{
    ...
    case kDoGenerationalOrShared:
        return WriteBarrierForRangeImpl<kDoGenerationalOrShared>(source_page, object, start_slot, end_slot);
    // Marking, no evacuation slot recording.
    case kDoMarking:
        return WriteBarrierForRangeImpl<kDoMarking>(source_page, object, start_slot, end_slot);
    // Marking with evacuation slot recording.
    case kDoMarking | kDoEvacuationSlotRecording:
        return WriteBarrierForRangeImpl<kDoMarking | kDoEvacuationSlotRecording>(source_page, object, start_slot, end_slot);

    // Generational and marking, no evacuation slot recording.
    case kDoGenerationalOrShared | kDoMarking:
        return WriteBarrierForRangeImpl<kDoGenerationalOrShared | kDoMarking>(source_page, object, start_slot, end_slot);
    ...
    case kDoGenerationalOrShared | kDoMarking | kDoEvacuationSlotRecording:
        return WriteBarrierForRangeImpl<kDoGenerationalOrShared | kDoMarking | kDoEvacuationSlotRecording>(source_page, object, start_slot, end_slot);
...
void Heap::GenerationalBarrierForCodeSlow(Code host, RelocInfo* rinfo, HeapObject object)
{
    DCHECK(InYoungGeneration(object));
    const MarkCompactCollector::RecordRelocSlotInfo info = MarkCompactCollector::ProcessRelocInfo(host, rinfo, object);

    RememberedSet<OLD_TO_NEW>::InsertTyped(info.memory_chunk, info.slot_type, info.offset);
}
...
bool Heap::PageFlagsAreConsistent(HeapObject object)
{
    ...
    CHECK_EQ(chunk->IsFlagSet(MemoryChunk::INCREMENTAL_MARKING), slim_chunk->IsMarking());

    AllocationSpace identity = chunk->owner()->identity();

    // Generation consistency.
    CHECK_EQ(identity == NEW_SPACE || identity == NEW_LO_SPACE, slim_chunk->InYoungGeneration());
...
bool Heap::IsStressingScavenge() { return v8_flags.stress_scavenge > 0 && new_space(); }

void Heap::SetIsMarkingFlag(bool value) { isolate()->isolate_data()->is_marking_flag_ = value; }

uint8_t* Heap::IsMarkingFlagAddress() { return &isolate()->isolate_data()->is_marking_flag_; }

void Heap::SetIsMinorMarkingFlag(bool value) { isolate()->isolate_data()->is_minor_marking_flag_ = value; }

uint8_t* Heap::IsMinorMarkingFlagAddress() { return &isolate()->isolate_data()->is_minor_marking_flag_; }
...
Address* StrongRootBlockAllocator::allocate(size_t n)
{
    ...
    *header = heap_->RegisterStrongRoots("StrongRootBlockAllocator", FullObjectSlot(ret), FullObjectSlot(ret + n));
...
#ifdef V8_ENABLE_ALLOCATION_TIMEOUT
void Heap::set_allocation_timeout(int allocation_timeout) { heap_allocator_.SetAllocationTimeout(allocation_timeout); }
#endif // V8_ENABLE_ALLOCATION_TIMEOUT

void Heap::FinishSweepingIfOutOfWork()
{
    if (sweeper()->sweeping_in_progress() && v8_flags.concurrent_sweeping && !sweeper()->AreSweeperTasksRunning()) {
...
void Heap::EnsureSweepingCompleted(SweepingForcedFinalizationMode mode)
{
    if (sweeper()->sweeping_in_progress()) {
        TRACE_GC_EPOCH(tracer(), GCTracer::Scope::MC_COMPLETE_SWEEPING, ThreadKind::kMain);

        sweeper()->EnsureCompleted();
        old_space()->RefillFreeList();
        {
            CodePageHeaderModificationScope rwx_write_scope("Updating per-page stats stored in page headers requires write "
...
EmbedderStackStateScope::EmbedderStackStateScope(Heap* heap, Origin origin, StackState stack_state)
...
// static
EmbedderStackStateScope EmbedderStackStateScope::ExplicitScopeForTesting(LocalEmbedderHeapTracer* local_tracer, StackState stack_state)
{
    return EmbedderStackStateScope(local_tracer, stack_state);
}

EmbedderStackStateScope::EmbedderStackStateScope(LocalEmbedderHeapTracer* local_tracer, StackState stack_state)
...
EmbedderStackStateScope::~EmbedderStackStateScope() { local_tracer_->embedder_stack_state_ = old_stack_state_; }
