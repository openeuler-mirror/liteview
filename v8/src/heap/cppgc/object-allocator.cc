...
void* ObjectAllocator::OutOfLineAllocateImpl(NormalPageSpace& space, size_t size, AlignVal alignment, GCInfoIndex gcinfo)
{
    ...
    if (size >= kLargeObjectSizeThreshold) {
        ...
        if (!result) {
            ...
            config.free_memory_handling = GCConfig::FreeMemoryHandling::kDiscardWherePossible;
    ...
    void* result = (dynamic_alignment == kAllocationGranularity) ? AllocateObjectOnSpace(space, size, gcinfo) : AllocateObjectOnSpace(space, size, alignment, gcinfo);
    CHECK(result);
    return result;
}
...
bool ObjectAllocator::TryRefillLinearAllocationBuffer(NormalPageSpace& space, size_t size)
{
    ...
    if (sweeper.SweepForAllocationIfRunning(&space, size, v8::base::TimeDelta::FromMicroseconds(500)) && TryRefillLinearAllocationBufferFromFreeList(space, size)) {
        return true;
    }
...
bool ObjectAllocator::in_disallow_gc_scope() const { return raw_heap_.heap()->in_disallow_gc_scope(); }
