...
public:
    ...
    bool Contains(ConstAddress addr) const { return (reinterpret_cast<uintptr_t>(addr) - reinterpret_cast<uintptr_t>(base_)) < size_; }

    bool Contains(const MemoryRegion& other) const { return base_ <= other.base() && other.end() <= end(); }
...
public:
    ...
    const PageMemory GetPageMemory(size_t index) const
    {
        ...
        return PageMemory(MemoryRegion(reserved_region().base() + kPageSize * index, kPageSize),
            MemoryRegion(reserved_region().base() + kPageSize * index + kGuardPageSize, kPageSize - 2 * kGuardPageSize));
    }
...
private:
    ...
    size_t GetIndex(ConstAddress address) const { return static_cast<size_t>(address - reserved_region().base()) >> kPageSizeLog2; }
...
public:
    PageBackend(PageAllocator& normal_page_allocator, PageAllocator& large_page_allocator, FatalOutOfMemoryHandler&);
    ~PageBackend();
...
// granularity.
inline bool SupportsCommittingGuardPages(PageAllocator& allocator) { return kGuardPageSize != 0 && kGuardPageSize % allocator.CommitPageSize() == 0; }
...
Address PageMemoryRegion::Lookup(ConstAddress address) const
{
    ...
    return is_large() ? static_cast<const LargePageMemoryRegion*>(this)->Lookup(address) : static_cast<const NormalPageMemoryRegion*>(this)->Lookup(address);
}
