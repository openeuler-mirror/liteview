...
const size_t IncrementalMarkingSchedule::kMinimumMarkedBytesPerIncrementalStep = 64 * kKB;
...
void IncrementalMarkingSchedule::UpdateMutatorThreadMarkedBytes(size_t overall_marked_bytes) { mutator_thread_marked_bytes_ = overall_marked_bytes; }

void IncrementalMarkingSchedule::AddConcurrentlyMarkedBytes(size_t marked_bytes) { concurrently_marked_bytes_.fetch_add(marked_bytes, std::memory_order_relaxed); }

size_t IncrementalMarkingSchedule::GetOverallMarkedBytes() const { return mutator_thread_marked_bytes_ + GetConcurrentlyMarkedBytes(); }

size_t IncrementalMarkingSchedule::GetConcurrentlyMarkedBytes() const { return concurrently_marked_bytes_.load(std::memory_order_relaxed); }

double IncrementalMarkingSchedule::GetElapsedTimeInMs(v8::base::TimeTicks start_time)
{
...
size_t IncrementalMarkingSchedule::GetNextIncrementalStepDuration(size_t estimated_live_bytes)
{
    ...
    DCHECK(!incremental_marking_start_time_.IsNull());
    double elapsed_time_in_ms = GetElapsedTimeInMs(incremental_marking_start_time_);
    size_t actual_marked_bytes = GetOverallMarkedBytes();
    size_t expected_marked_bytes = std::ceil(estimated_live_bytes * elapsed_time_in_ms / kEstimatedMarkingTimeMs);
    ...
    return std::max(kMinimumMarkedBytesPerIncrementalStep, expected_marked_bytes - actual_marked_bytes);
}
...
bool IncrementalMarkingSchedule::ShouldFlushEphemeronPairs()
{
    ...
    if (GetOverallMarkedBytes() < (ephemeron_pairs_flushing_ratio_target * last_estimated_live_bytes_))
        return false;
    ephemeron_pairs_flushing_ratio_target += kEphemeronPairsFlushingRatioIncrements;
    return true;
}
