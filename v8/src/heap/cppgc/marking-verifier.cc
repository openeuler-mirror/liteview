...
void VerificationState::VerifyMarked(const void* base_object_payload) const
{
    ...
    if (!child_header.IsMarked()) {
        FATAL("MarkingVerifier: Encountered unmarked object.\n"
              ...
              "#   %s (%p)\n"
              "#     \\-> %s (%p)",
            parent_ ? parent_->GetName().value : "Stack", parent_ ? parent_->ObjectStart() : nullptr, child_header.GetName().value, child_header.ObjectStart());
    }
}

MarkingVerifierBase::MarkingVerifierBase(HeapBase& heap, CollectionType collection_type, VerificationState& verification_state, std::unique_ptr<cppgc::Visitor> visitor)
...
void MarkingVerifierBase::Run(StackState stack_state, uintptr_t stack_end, v8::base::Optional<size_t> expected_marked_bytes)
{
...
public:
    ...
    void Visit(const void*, TraceDescriptor desc) final { state_.VerifyMarked(desc.base_object_payload); }
