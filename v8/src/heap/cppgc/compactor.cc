...
public:
    ...
    void RelocateObject(const NormalPage* page, const Address header, size_t size)
    {
        ...
        if (V8_LIKELY(compact_frontier != header)) {
            ...
            movable_references_.Relocate(header + sizeof(HeapObjectHeader), compact_frontier + sizeof(HeapObjectHeader));
        }
...
Compactor::CompactableSpaceHandling Compactor::CompactSpacesIfEnabled()
{
    ...
    for (NormalPageSpace* space : compactable_spaces_) {
        CompactSpace(space, movable_references, young_gen_enabled ? StickyBits::kEnabled : StickyBits::kDisabled);
    }
