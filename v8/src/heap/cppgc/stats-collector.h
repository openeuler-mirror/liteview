...
#undef CPPGC_DECLARE_ENUM
    ...
    struct Event final {
        ...
        v8::base::Atomic32 concurrent_scope_data[kNumHistogramConcurrentScopeIds] { 0 };
...
#undef CPPGC_CASE
    ...
    public:
            ...
            , scope_id_(scope_id)
        {
            ...
            DCHECK_LT(static_cast<int>(scope_id_), scope_category == kMutatorThread ? static_cast<int>(kNumScopeIds) : static_cast<int>(kNumConcurrentScopeIds));
            DCHECK_NE(static_cast<int>(scope_id_), scope_category == kMutatorThread ? static_cast<int>(kNumHistogramScopeIds) : static_cast<int>(kNumHistogramConcurrentScopeIds));
            StartTrace(args...);
        }
        ...
        void DecreaseStartTimeForTesting(v8::base::TimeDelta delta) { start_time_ -= delta; }
...
public:
    ...
    void SetMetricRecorder(std::unique_ptr<MetricRecorder> histogram_recorder) { metric_recorder_ = std::move(histogram_recorder); }

    MetricRecorder* GetMetricRecorder() const { return metric_recorder_.get(); }

private:
    enum class GarbageCollectionState : uint8_t { kNotRunning, kMarking, kSweeping };
...
void StatsCollector::InternalScope<trace_category, scope_category>::StartTrace(Args... args)
{
    ...
    DCHECK_IMPLIES(static_cast<int>(scope_id_) < (scope_category == kMutatorThread ? static_cast<int>(kNumHistogramScopeIds) : static_cast<int>(kNumHistogramConcurrentScopeIds)),
        trace_category == StatsCollector::TraceCategory::kEnabled);
    if (trace_category == StatsCollector::TraceCategory::kEnabled)
        StartTraceImpl(args...);
}

template <StatsCollector::TraceCategory trace_category, StatsCollector::ScopeContext scope_category> void StatsCollector::InternalScope<trace_category, scope_category>::StopTrace()
{
    if (trace_category == StatsCollector::TraceCategory::kEnabled)
        StopTraceImpl();
}
...
void StatsCollector::InternalScope<trace_category, scope_category>::IncreaseScopeTime()
{
    ...
    if (static_cast<int>(scope_id_) >= (scope_category == kMutatorThread ? static_cast<int>(kNumHistogramScopeIds) : static_cast<int>(kNumHistogramConcurrentScopeIds)))
        return;
    ...
    v8::base::Relaxed_AtomicIncrement(&stats_collector_->current_.concurrent_scope_data[scope_id_], static_cast<Atomic32>(us));
}
