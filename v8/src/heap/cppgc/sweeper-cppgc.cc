...
public:
    ...
    bool Check() { return (++count_ % kInterval == 0) && (end_ < v8::base::TimeTicks::Now()); }
...
class ObjectStartBitmapVerifier : private HeapVisitor<ObjectStartBitmapVerifier> {
...
template <typename FinalizationBuilder> typename FinalizationBuilder::ResultType SweepNormalPage(NormalPage* page, PageAllocator& page_allocator, StickyBits sticky_bits)
{
    ...
    if (start_of_gap != page->PayloadStart() && start_of_gap != page->PayloadEnd()) {
...
public:
    SweepFinalizer(cppgc::Platform* platform, FreeMemoryHandling free_memory_handling)
        : platform_(platform)
    ...
    bool FinalizeSpaceWithDeadline(SpaceState* space_state, v8::base::TimeTicks deadline)
    {
        DCHECK(platform_);
    ...
    void FinalizePage(SpaceState::SweptPageState* page_state)
    {
        ...
        largest_new_free_list_entry_ = std::max(page_state->largest_new_free_list_entry, largest_new_free_list_entry_);
    ...
    size_t largest_new_free_list_entry() const { return largest_new_free_list_entry_; }
...
public:
    MutatorThreadSweeper(HeapBase* heap, SpaceStates* states, cppgc::Platform* platform, FreeMemoryHandling free_memory_handling)
        : states_(states)
        , platform_(platform)
        , free_memory_handling_(free_memory_handling)
        , sticky_bits_(heap->generational_gc_supported() ? StickyBits::kEnabled : StickyBits::kDisabled)
    {
    }
    ...
    bool SweepWithDeadline(v8::base::TimeDelta max_duration, MutatorThreadSweepingMode sweeping_mode)
    {
        DCHECK(platform_);
        for (SpaceState& state : *states_) {
...
private:
    ...
    bool VisitNormalPage(NormalPage& page)
    {
        ...
        if (result.is_empty) {
            ...
            page.space().AddPage(&page);
            largest_new_free_list_entry_ = std::max(result.largest_new_free_list_entry, largest_new_free_list_entry_);
        }
        return true;
    }
...
public:
    ConcurrentSweepTask(HeapBase& heap, SpaceStates* states, Platform* platform, FreeMemoryHandling free_memory_handling)
        : heap_(heap)
        , states_(states)
        , platform_(platform)
        , free_memory_handling_(free_memory_handling)
    ...
    size_t GetMaxConcurrency(size_t /* active_worker_count */) const final { return is_completed_.load(std::memory_order_relaxed) ? 0 : 1; }
...
private:
    bool VisitNormalPage(NormalPage& page)
    {
        ...
        SpaceState::SweptPageState sweep_result = (free_memory_handling_ == FreeMemoryHandling::kDiscardWherePossible)
            ? SweepNormalPage<DeferredFinalizationBuilder<DiscardingFreeHandler>>(&page, *platform_->GetPageAllocator(), sticky_bits_)
            : SweepNormalPage<DeferredFinalizationBuilder<RegularFreeHandler>>(&page, *platform_->GetPageAllocator(), sticky_bits_);
    ...
    bool VisitLargePage(LargePage& page)
    {
        ...
        state.swept_unfinalized_pages.Push({ &page, std::move(unfinalized_objects), {}, {}, true });
        return true;
    }
...
public:
    PrepareForSweepVisitor(SpaceStates* states, CompactableSpaceHandling compactable_space_handling)
        : states_(states)
...
private:
    void ExtractPages(BaseSpace& space)
    {
        ...
        (*states_)[space.index()].unswept_pages.Insert(space_pages.begin(), space_pages.end());
    }
...
public:
    ...
    void Start(SweepingConfig config, cppgc::Platform* platform)
    {
        ...
        PrepareForSweepVisitor(&space_states_, config.compactable_space_handling).Run(heap_);
    ...
    bool SweepForAllocationIfRunning(NormalPageSpace* space, size_t size, v8::base::TimeDelta max_duration)
    {
        if (!is_in_progress_)
            return false;
        ...
        // moment.
        if (space_state.swept_unfinalized_pages.IsEmpty() && space_state.unswept_pages.IsEmpty()) {
            return false;
        }

        StatsCollector::EnabledScope stats_scope(stats_collector_, StatsCollector::kIncrementalSweep);
        StatsCollector::EnabledScope inner_scope(stats_collector_, StatsCollector::kSweepOnAllocation);
        ...
        }
        {
            ...
            MutatorThreadSweeper sweeper(heap_.heap(), &space_states_, platform_, config_.free_memory_handling);
    ...
    bool FinishIfRunning()
    {
        ...
        {
            StatsCollector::EnabledScope stats_scope(stats_collector_, StatsCollector::kIncrementalSweep);
            StatsCollector::EnabledScope inner_scope(stats_collector_, StatsCollector::kSweepFinalize);
            if (concurrent_sweeper_handle_ && concurrent_sweeper_handle_->IsValid() && concurrent_sweeper_handle_->UpdatePriorityEnabled()) {
                concurrent_sweeper_handle_->UpdatePriority(cppgc::TaskPriority::kUserBlocking);
            }
            Finish();
        }
        NotifyDone();
    ...
    bool IsConcurrentSweepingDone() const { return !concurrent_sweeper_handle_ || (concurrent_sweeper_handle_->IsValid() && !concurrent_sweeper_handle_->IsActive()); }

    void FinishIfOutOfWork()
    {
        if (is_in_progress_ && !is_sweeping_on_mutator_thread_ && concurrent_sweeper_handle_ && concurrent_sweeper_handle_->IsValid() && !concurrent_sweeper_handle_->IsActive()) {
            ...
            // sweeping though.
            DCHECK(std::all_of(space_states_.begin(), space_states_.end(), [](const SpaceState& state) { return state.unswept_pages.IsEmpty(); }));
            ...
            MutatorThreadSweeper sweeper(heap_.heap(), &space_states_, platform_, config_.free_memory_handling);
            if (sweeper.SweepWithDeadline(v8::base::TimeDelta::FromMilliseconds(2), MutatorThreadSweepingMode::kAll)) {
                FinalizeSweep();
            }
        }
        NotifyDoneIfNeeded();
    ...
    bool IsSweepingOnMutatorThread() const { return is_sweeping_on_mutator_thread_; }
    ...
    {
        DCHECK_EQ(mutator_thread_sweeping_observers_.end(), std::find(mutator_thread_sweeping_observers_.begin(), mutator_thread_sweeping_observers_.end(), observer));
...
private:
    ...
    private:
        void Run() override
        {
            ...
            if (!sweeper_.PerformSweepOnMutatorThread(v8::base::TimeDelta::FromMilliseconds(5), StatsCollector::kSweepInTask,
                    sweeper_.IsConcurrentSweepingDone() ? MutatorThreadSweepingMode::kAll : MutatorThreadSweepingMode::kOnlyFinalizers)) {
    ...
    void ScheduleIncrementalSweeping()
    {
        ...
        incremental_sweeper_handle_ = IncrementalSweepTask::Post(*this, runner.get());
    }
    ...
    void ScheduleConcurrentSweeping()
    {
        ...
        if (config_.sweeping_type != SweepingConfig::SweepingType::kIncrementalAndConcurrent)
            return;

        concurrent_sweeper_handle_
            = platform_->PostJob(cppgc::TaskPriority::kUserVisible, std::make_unique<ConcurrentSweepTask>(*heap_.heap(), &space_states_, platform_, config_.free_memory_handling));
    }
...
Sweeper::Sweeper(HeapBase& heap)
    : heap_(heap)
    , impl_(std::make_unique<SweeperImpl>(heap.raw_heap(), heap.stats_collector()))
{
}

Sweeper::~Sweeper() = default;

void Sweeper::Start(SweepingConfig config) { impl_->Start(config, heap_.platform()); }

bool Sweeper::FinishIfRunning() { return impl_->FinishIfRunning(); }
void Sweeper::FinishIfOutOfWork() { impl_->FinishIfOutOfWork(); }
void Sweeper::WaitForConcurrentSweepingForTesting() { impl_->WaitForConcurrentSweepingForTesting(); }
void Sweeper::NotifyDoneIfNeeded() { impl_->NotifyDoneIfNeeded(); }
bool Sweeper::SweepForAllocationIfRunning(NormalPageSpace* space, size_t size, v8::base::TimeDelta max_duration)
{
    return impl_->SweepForAllocationIfRunning(space, size, max_duration);
}
bool Sweeper::IsSweepingOnMutatorThread() const { return impl_->IsSweepingOnMutatorThread(); }

bool Sweeper::IsSweepingInProgress() const { return impl_->IsSweepingInProgress(); }
...
Sweeper::SweepingOnMutatorThreadObserver::~SweepingOnMutatorThreadObserver() { sweeper_.impl_->RemoveMutatorThreadSweepingObserver(this); }
