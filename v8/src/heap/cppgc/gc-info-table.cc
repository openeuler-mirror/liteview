...
GCInfoTable::GCInfoTable(PageAllocator& page_allocator, FatalOutOfMemoryHandler& oom_handler)
    ...
    , table_(static_cast<decltype(table_)>(page_allocator_.AllocatePages(nullptr, MaxTableSize(), page_allocator_.AllocatePageSize(), PageAllocator::kNoAccess)))
...
GCInfoTable::~GCInfoTable() { page_allocator_.ReleasePages(const_cast<GCInfo*>(table_), MaxTableSize(), 0); }

size_t GCInfoTable::MaxTableSize() const { return RoundUp(GCInfoTable::kMaxIndex * kEntrySize, page_allocator_.AllocatePageSize()); }
