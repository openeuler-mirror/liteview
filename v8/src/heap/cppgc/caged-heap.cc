...
static_assert(api_constants::kCagedHeapReservationSize == kCagedHeapReservationSize);
...
{
    DCHECK_EQ(0u, kCagedHeapReservationSize % platform_allocator.AllocatePageSize());
    ...
    for (size_t i = 0; i < kAllocationTries; ++i) {
        ...
        static constexpr size_t kTryReserveAlignment = 2 * kCagedHeapReservationAlignment;
#else // !defined(CPPGC_POINTER_COMPRESSION)
        static constexpr size_t kTryReserveSize = kCagedHeapReservationSize;
        static constexpr size_t kTryReserveAlignment = kCagedHeapReservationAlignment;
...
// static
void CagedHeap::InitializeIfNeeded(PageAllocator& platform_allocator) { static v8::base::LeakyObject<CagedHeap> caged_heap(platform_allocator); }
    ...
    : reserved_area_(ReserveCagedHeap(platform_allocator))
{
    ...
    if (!platform_allocator.SetPermissions(cage_start, RoundUp(sizeof(CagedHeapLocalData), platform_allocator.CommitPageSize()), PageAllocator::kReadWrite)) {
        GetGlobalOOMHandler()("Oilpan: CagedHeap commit CageHeapLocalData.");
    }

    const CagedAddress caged_heap_start = RoundUp(reinterpret_cast<CagedAddress>(cage_start) + sizeof(CagedHeapLocalData), kPageSize);
    const size_t local_data_size_with_padding = caged_heap_start - reinterpret_cast<CagedAddress>(cage_start);

    page_bounded_allocator_ = std::make_unique<v8::base::BoundedPageAllocator>(&platform_allocator, caged_heap_start, kCagedHeapReservationSize - local_data_size_with_padding,
        kPageSize, v8::base::PageInitializationMode::kAllocatedPagesMustBeZeroInitialized, v8::base::PageFreeingMode::kMakeInaccessible);
