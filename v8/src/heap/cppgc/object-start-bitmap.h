...
public:
    ...
    static constexpr size_t MaxEntries() { return kReservedForBitmap * kBitsPerCell; }
...
ObjectStartBitmap::ObjectStartBitmap(Address offset)
    : offset_(offset)
{
    Clear();
...
template <AccessMode mode> HeapObjectHeader* ObjectStartBitmap::FindHeader(ConstAddress address_maybe_pointing_to_the_middle_of_object) const
{
    ...
    size_t object_offset = address_maybe_pointing_to_the_middle_of_object - offset_;
...
template <typename Callback> inline void ObjectStartBitmap::Iterate(Callback callback) const
{
    for (size_t cell_index = 0; cell_index < kReservedForBitmap; cell_index++) {
        ...
        while (value) {
            ...
            const Address object_address = offset_ + (kAllocationGranularity * object_start_number);
