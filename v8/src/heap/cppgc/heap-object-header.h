...
private:
    ...
    template <AccessMode mode, EncodedHalf part, std::memory_order memory_order = std::memory_order_seq_cst> inline void StoreEncoded(uint16_t bits, uint16_t mask);
...
template <AccessMode mode> bool HeapObjectHeader::IsLargeObject() const { return AllocatedSize<mode>() == kLargeObjectSizeInHeader; }
...
void HeapObjectHeader::MarkAsFullyConstructed() { MakeGarbageCollectedTraitInternal::MarkObjectAsFullyConstructed(ObjectStart()); }
...
template <AccessMode mode> bool HeapObjectHeader::IsYoung() const { return !IsMarked<mode>(); }

template <AccessMode mode> bool HeapObjectHeader::IsFree() const { return GetGCInfoIndex<mode>() == kFreeListGCInfoIndex; }
...
#if defined(CPPGC_CAGED_HEAP)
void HeapObjectHeader::SetNextUnfinalized(HeapObjectHeader* next) { next_unfinalized_ = CagedHeap::OffsetFromAddress<uint32_t>(next); }

HeapObjectHeader* HeapObjectHeader::GetNextUnfinalized(uintptr_t cage_base) const
{
    DCHECK(cage_base);
    DCHECK_EQ(0u, CagedHeap::OffsetFromAddress(reinterpret_cast<void*>(cage_base)));
    return next_unfinalized_ ? reinterpret_cast<HeapObjectHeader*>(cage_base + next_unfinalized_) : nullptr;
}
#endif // defined(CPPGC_CAGED_HEAP)
