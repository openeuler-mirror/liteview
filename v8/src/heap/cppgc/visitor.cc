...
void TraceConservatively(ConservativeTracingVisitor* conservative_visitor, const HeapObjectHeader& header)
{
    ...
    for (size_t i = 0; i < (object_view.Size() / sizeof(uintptr_t)); ++i) {
        ...
        if (maybe_full_ptr > SentinelPointer::kSentinelValue)
            conservative_visitor->TraceConservativelyIfNeeded(reinterpret_cast<Address>(maybe_full_ptr));
        ...
        auto decompressed_high = reinterpret_cast<Address>(CompressedPointer::Decompress(static_cast<uint32_t>(maybe_full_ptr >> (sizeof(uint32_t) * CHAR_BIT))));
...
void ConservativeTracingVisitor::TryTracePointerConservatively(Address pointer)
{
#if defined(CPPGC_CAGED_HEAP)
    // TODO(chromium:1056170): Add support for SIMD in stack scanning.
    if (V8_LIKELY(!CagedHeapBase::IsWithinCage(pointer)))
        return;
#endif // defined(CPPGC_CAGED_HEAP)

    const BasePage* page = reinterpret_cast<const BasePage*>(page_backend_.Lookup(const_cast<ConstAddress>(pointer)));
...
void ConservativeTracingVisitor::TraceConservativelyIfNeeded(const void* address)
{
    ...
    auto decompressed_high = static_cast<Address>(CompressedPointer::Decompress(static_cast<uint32_t>(reinterpret_cast<uintptr_t>(pointer) >> (sizeof(uint32_t) * CHAR_BIT))));
    ...
    auto intermediate_decompressed_high = reinterpret_cast<Address>(static_cast<uint32_t>(reinterpret_cast<uintptr_t>(pointer) >> (sizeof(uint32_t) * CHAR_BIT)) | base);
