...
public:
    ...
    bool ConcurrentSweepSpace(AllocationSpace identity, JobDelegate* delegate)
    {
        while (!delegate->ShouldYield()) {
            ...
            if (page == nullptr)
                return true;
            sweeper_->ParallelSweepPage(page, identity, &local_pretenuring_feedback_, SweepingMode::kLazyOrConcurrent);
        }
        return false;
    }

    PretenturingHandler::PretenuringFeedbackMap* local_pretenuring_feedback() { return &local_pretenuring_feedback_; }
...
public:
    SweeperJob(Isolate* isolate, Sweeper* sweeper, std::vector<ConcurrentSweeper>* concurrent_sweepers)
        : sweeper_(sweeper)
    ...
    void Run(JobDelegate* delegate) final
    {
        ...
        if (delegate->IsJoiningThread()) {
            TRACE_GC(tracer_, sweeper_->current_collector_ == GarbageCollector::MINOR_MARK_COMPACTOR ? GCTracer::Scope::MINOR_MC_SWEEP : GCTracer::Scope::MC_SWEEP);
            RunImpl(delegate);
        } else {
            TRACE_GC_EPOCH(tracer_,
                sweeper_->current_collector_ == GarbageCollector::MINOR_MARK_COMPACTOR ? GCTracer::Scope::MINOR_MC_BACKGROUND_SWEEPING : GCTracer::Scope::MC_BACKGROUND_SWEEPING,
    ...
    size_t GetMaxConcurrency(size_t worker_count) const override
    {
        ...
        return std::min<size_t>(concurrent_sweepers_->size(), worker_count + (sweeper_->ConcurrentSweepingPageCount() + kPagePerTask - 1) / kPagePerTask);
    }
...
private:
    void RunImpl(JobDelegate* delegate)
    {
        ...
        for (int i = 0; i < kNumberOfSweepingSpaces; i++) {
            const AllocationSpace space_id = static_cast<AllocationSpace>(FIRST_SWEEPABLE_SPACE + ((i + offset) % kNumberOfSweepingSpaces));
            DCHECK(IsValidSweepingSpace(space_id));
            if (!sweeper.ConcurrentSweepSpace(space_id, delegate))
                return;
        }
    }
...
Sweeper::PauseScope::PauseScope(Sweeper* sweeper)
    : sweeper_(sweeper)
{
...
Sweeper::FilterSweepingPagesScope::FilterSweepingPagesScope(Sweeper* sweeper, const PauseScope& pause_scope)
    : sweeper_(sweeper)
...
void Sweeper::StartSweeping(GarbageCollector collector)
{
    ...
    ForAllSweepingSpaces([this](AllocationSpace space) {
        ...
        std::sort(sweeping_list_[space_index].begin(), sweeping_list_[space_index].end(),
            [marking_state = marking_state_](Page* a, Page* b) { return marking_state->live_bytes(a) > marking_state->live_bytes(b); });
    });
}
...
int Sweeper::NumberOfConcurrentSweepers() const
{
    ...
    return std::min(Sweeper::kMaxSweeperTasks, V8::GetCurrentPlatform()->NumberOfWorkerThreads() + 1);
}
...
void Sweeper::StartSweeperTasks()
{
    ...
    if (v8_flags.concurrent_sweeping && sweeping_in_progress_ && !heap_->delay_sweeper_tasks_for_testing_) {
        ...
        job_handle_ = V8::GetCurrentPlatform()->PostJob(TaskPriority::kUserVisible, std::make_unique<SweeperJob>(heap_->isolate(), this, &concurrent_sweepers_));
    }
}
...
void Sweeper::EnsureCompleted(SweepingMode sweeping_mode)
{
    ...
    // here.
    ForAllSweepingSpaces([this, sweeping_mode](AllocationSpace space) { ParallelSweepSpace(space, sweeping_mode, 0); });

    if (job_handle_ && job_handle_->IsValid())
        job_handle_->Join();

    ForAllSweepingSpaces([this](AllocationSpace space) { CHECK(sweeping_list_[GetSweepSpaceIndex(space)].empty()); });
    ...
    for (ConcurrentSweeper& concurrent_sweeper : concurrent_sweepers_) {
        pretenuring_handler_->MergeAllocationSitePretenuringFeedback(*concurrent_sweeper.local_pretenuring_feedback());
    }
...
bool Sweeper::AreSweeperTasksRunning() { return job_handle_ && job_handle_->IsValid() && job_handle_->IsActive(); }

V8_INLINE size_t Sweeper::FreeAndProcessFreedMemory(Address free_start, Address free_end, Page* page, Space* space, FreeSpaceTreatmentMode free_space_treatment_mode)
{
    ...
    freed_bytes = reinterpret_cast<PagedSpaceBase*>(space)->UnaccountedFree(free_start, size);
    if (should_reduce_memory_)
        page->DiscardUnusedMemory(free_start, size);

    return freed_bytes;
}

V8_INLINE void Sweeper::CleanupRememberedSetEntriesForFreedMemory(Address free_start, Address free_end, Page* page, bool record_free_ranges,
    TypedSlotSet::FreeRangesMap* free_ranges_map, SweepingMode sweeping_mode, InvalidatedSlotsCleanup* invalidated_old_to_new_cleanup,
    InvalidatedSlotsCleanup* invalidated_old_to_shared_cleanup)
{
    ...
    if (record_free_ranges) {
        free_ranges_map->insert(std::pair<uint32_t, uint32_t>(static_cast<uint32_t>(free_start - page->address()), static_cast<uint32_t>(free_end - page->address())));
    }
...
void Sweeper::CleanupTypedSlotsInFreeMemory(Page* page, const TypedSlotSet::FreeRangesMap& free_ranges_map, SweepingMode sweeping_mode)
{
    if (sweeping_mode == SweepingMode::kEagerDuringGC) {
...
void Sweeper::ClearMarkBitsAndHandleLivenessStatistics(Page* page, size_t live_bytes)
{
...
int Sweeper::RawSweep(Page* p, FreeSpaceTreatmentMode free_space_treatment_mode, SweepingMode sweeping_mode, const base::MutexGuard& page_guard,
    PretenturingHandler::PretenuringFeedbackMap* local_pretenuring_feedback)
{
    ...
    if (should_reduce_memory_) {
        ...
        active_system_pages_after_sweeping->Init(MemoryChunkLayout::kMemoryChunkHeaderSize, MemoryAllocator::GetCommitPageSizeBits(), Page::kPageSize);
    }
    ...
    bool record_free_ranges
        = p->typed_slot_set<OLD_TO_NEW>() != nullptr || p->typed_slot_set<OLD_TO_OLD>() != nullptr || p->typed_slot_set<OLD_TO_SHARED>() != nullptr || DEBUG_BOOL;
    ...
    // in free memory.
    InvalidatedSlotsCleanup invalidated_old_to_new_cleanup = InvalidatedSlotsCleanup::NoCleanup(p);
    InvalidatedSlotsCleanup invalidated_old_to_shared_cleanup = InvalidatedSlotsCleanup::NoCleanup(p);
    if (sweeping_mode == SweepingMode::kEagerDuringGC) {
    ...
    for (auto object_and_size : LiveObjectRange<kBlackObjects>(p, marking_state_->bitmap(p))) {
        ...
        if (free_end != free_start) {
            max_freed_bytes = std::max(max_freed_bytes, FreeAndProcessFreedMemory(free_start, free_end, p, space, free_space_treatment_mode));
            CleanupRememberedSetEntriesForFreedMemory(
                free_start, free_end, p, record_free_ranges, &free_ranges_map, sweeping_mode, &invalidated_old_to_new_cleanup, &invalidated_old_to_shared_cleanup);
        }
        ...
        if (p->InYoungGeneration()) {
            pretenuring_handler_->UpdateAllocationSite(map, object, local_pretenuring_feedback);
        }

        if (active_system_pages_after_sweeping) {
            active_system_pages_after_sweeping->Add(free_end - p->address(), free_start - p->address(), MemoryAllocator::GetCommitPageSizeBits());
        }
    ...
    return static_cast<int>(p->owner()->free_list()->GuaranteedAllocatable(max_freed_bytes));
}

size_t Sweeper::ConcurrentSweepingPageCount()
{
    base::MutexGuard guard(&mutex_);
    return sweeping_list_[GetSweepSpaceIndex(OLD_SPACE)].size() + sweeping_list_[GetSweepSpaceIndex(MAP_SPACE)].size()
        + (v8_flags.minor_mc ? sweeping_list_[GetSweepSpaceIndex(NEW_SPACE)].size() : 0);
}

int Sweeper::ParallelSweepSpace(AllocationSpace identity, SweepingMode sweeping_mode, int required_freed_bytes, int max_pages)
{
    int max_freed = 0;
    int pages_freed = 0;
    Page* page = nullptr;
    while ((page = GetSweepingPageSafe(identity)) != nullptr) {
        int freed = ParallelSweepPage(page, identity, &local_pretenuring_feedback_, sweeping_mode);
        ++pages_freed;
...
int Sweeper::ParallelSweepPage(Page* page, AllocationSpace identity, PretenturingHandler::PretenuringFeedbackMap* local_pretenuring_feedback, SweepingMode sweeping_mode)
{
    ...
    int max_freed = 0;
    {
        ...
        DCHECK_EQ(Page::ConcurrentSweepingState::kPending, page->concurrent_sweeping_state());
        page->set_concurrent_sweeping_state(Page::ConcurrentSweepingState::kInProgress);
        const FreeSpaceTreatmentMode free_space_treatment_mode = Heap::ShouldZapGarbage() ? FreeSpaceTreatmentMode::kZapFreeSpace : FreeSpaceTreatmentMode::kIgnoreFreeSpace;
        max_freed = RawSweep(page, free_space_treatment_mode, sweeping_mode, guard, local_pretenuring_feedback);
        DCHECK(page->SweepingDone());
    }
...
{
    if (!sweeping_in_progress() || page->SweepingDone())
        return;
    ...
    if (IsValidSweepingSpace(space)) {
        if (TryRemoveSweepingPageSafe(space, page)) {
            ...
            ParallelSweepPage(page, space, &local_pretenuring_feedback_, SweepingMode::kLazyOrConcurrent);
        } else {
...
void Sweeper::AddPage(AllocationSpace space, Page* page, Sweeper::AddPageMode mode)
{
...
void Sweeper::AddPageImpl(AllocationSpace space, Page* page, Sweeper::AddPageMode mode)
{
    base::MutexGuard guard(&mutex_);
    DCHECK(IsValidSweepingSpace(space));
    DCHECK(!v8_flags.concurrent_sweeping || !job_handle_ || !job_handle_->IsValid());
...
{
#ifdef DEBUG
    DCHECK_GE(page->area_size(), static_cast<size_t>(marking_state_->live_bytes(page)));
    DCHECK_EQ(Page::ConcurrentSweepingState::kDone, page->concurrent_sweeping_state());
