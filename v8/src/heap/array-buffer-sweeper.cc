...
void ArrayBufferSweeper::EnsureFinished()
{
    ...
    TryAbortResult abort_result = heap_->isolate()->cancelable_task_manager()->TryAbort(job_->id_);
...
void ArrayBufferSweeper::RequestSweep(SweepingType type)
{
    ...
    Prepare(type);
    if (!heap_->IsTearingDown() && !heap_->ShouldReduceMemory() && v8_flags.concurrent_array_buffer_sweeping) {
        auto task = MakeCancelableTask(heap_->isolate(), [this, type] {
            GCTracer::Scope::ScopeId scope_id
                = type == SweepingType::kYoung ? GCTracer::Scope::BACKGROUND_YOUNG_ARRAY_BUFFER_SWEEP : GCTracer::Scope::BACKGROUND_FULL_ARRAY_BUFFER_SWEEP;
...
void ArrayBufferSweeper::Prepare(SweepingType type)
{
    ...
    case SweepingType::kYoung: {
        job_ = std::make_unique<SweepingJob>(std::move(young_), ArrayBufferList(), type);
        young_ = ArrayBufferList();
    } break;
    case SweepingType::kFull: {
        job_ = std::make_unique<SweepingJob>(std::move(young_), std::move(old_), type);
        young_ = ArrayBufferList();
...
void ArrayBufferSweeper::Finalize()
{
    ...
    const size_t freed_bytes = job_->freed_bytes_.exchange(0, std::memory_order_relaxed);
...
void ArrayBufferSweeper::Append(JSArrayBuffer object, ArrayBufferExtension* extension)
{
...
void ArrayBufferSweeper::Detach(JSArrayBuffer object, ArrayBufferExtension* extension)
{
...
void ArrayBufferSweeper::IncrementExternalMemoryCounters(size_t bytes)
{
    ...
    reinterpret_cast<v8::Isolate*>(heap_->isolate())->AdjustAmountOfExternalAllocatedMemory(static_cast<int64_t>(bytes));
}
...
ArrayBufferList ArrayBufferSweeper::SweepingJob::SweepListFull(ArrayBufferList* list)
{
    ArrayBufferExtension* current = list->head_;
