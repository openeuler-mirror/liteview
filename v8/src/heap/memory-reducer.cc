...
MemoryReducer::MemoryReducer(Heap* heap)
    : heap_(heap)
    , taskrunner_(V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(heap->isolate())))
...
void MemoryReducer::TimerTask::RunInternal()
{
    ...
    heap->tracer()->SampleAllocation(time_ms, heap->NewSpaceAllocationCounter(), heap->OldGenerationAllocationCounter(), heap->EmbedderAllocationCounter());
    ...
    event.can_start_incremental_gc = heap->incremental_marking()->IsStopped() && (heap->incremental_marking()->CanBeStarted() || optimize_for_memory);
...
void MemoryReducer::NotifyTimer(const Event& event)
{
    ...
    if (state_.action == kRun) {
        ...
        heap()->StartIdleIncrementalMarking(GarbageCollectionReason::kMemoryReducer, kGCCallbackFlagCollectAllExternalMemory);
...
void MemoryReducer::NotifyMarkCompact(const Event& event)
{
    ...
    if (old_action == kRun) {
        if (v8_flags.trace_gc_verbose) {
            heap()->isolate()->PrintWithTimestamp("Memory reducer: finished GC #%d (%s)\n", state_.started_gcs, state_.action == kWait ? "will do more" : "done");
        }
    }
}
...
bool MemoryReducer::WatchdogGC(const State& state, const Event& event) { return state.last_gc_time_ms != 0 && event.time_ms > state.last_gc_time_ms + kWatchdogDelayMs; }
    ...
    case kDone:
        if (event.type == kTimer) {
            ...
            if (event.committed_memory
                < std::max(static_cast<size_t>(state.committed_memory_at_last_run * kCommittedMemoryFactor), state.committed_memory_at_last_run + kCommittedMemoryDelta)) {
                return state;
            } else {
                return State(kWait, 0, event.time_ms + kLongDelayMs, event.time_ms, 0);
            }
        } else {
            DCHECK_EQ(kPossibleGarbage, event.type);
            return State(kWait, 0, event.time_ms + v8_flags.gc_memory_reducer_start_delay_ms, state.last_gc_time_ms, 0);
        }
    case kWait:
        switch (event.type) {
        ...
        case kTimer:
            if (state.started_gcs >= kMaxNumberOfGCs) {
                return State(kDone, kMaxNumberOfGCs, 0.0, state.last_gc_time_ms, event.committed_memory);
            } else if (event.can_start_incremental_gc && (event.should_start_incremental_gc || WatchdogGC(state, event))) {
                if (state.next_gc_start_ms <= event.time_ms) {
                    return State(kRun, state.started_gcs + 1, 0.0, state.last_gc_time_ms, 0);
                } else {
                    return state;
                }
            } else {
                return State(kWait, state.started_gcs, event.time_ms + kLongDelayMs, state.last_gc_time_ms, 0);
            }
        case kMarkCompact:
            return State(kWait, state.started_gcs, event.time_ms + kLongDelayMs, event.time_ms, 0);
        }
    case kRun:
        if (event.type != kMarkCompact) {
            ...
            if (state.started_gcs < kMaxNumberOfGCs && (event.next_gc_likely_to_collect_more || state.started_gcs == 1)) {
                return State(kWait, state.started_gcs, event.time_ms + kShortDelayMs, event.time_ms, 0);
            } else {
                return State(kDone, kMaxNumberOfGCs, 0.0, event.time_ms, event.committed_memory);
            }
        }
    }
    UNREACHABLE();
