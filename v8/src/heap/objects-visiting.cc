...
static bool MustRecordSlots(Heap* heap) { return heap->gc_state() == Heap::MARK_COMPACT && heap->mark_compact_collector()->is_compacting(); }
        ...
        if (retained != Object()) {
            if (head == undefined) {
                ...
                if (record_slots) {
                    ...
                    MarkCompactCollector::RecordSlot(slot_holder, slot, HeapObject::cast(retained));
                }
            }
...
template <class T> static void ClearWeakList(Heap* heap, Object list)
{
...
template <> struct WeakListVisitor<CodeT> {
    static void SetWeakNext(CodeT code, Object next) { CodeDataContainerFromCodeT(code).set_next_code_link(next, UPDATE_WRITE_BARRIER); }

    static Object WeakNext(CodeT code) { return CodeDataContainerFromCodeT(code).next_code_link(); }

    static HeapObject WeakNextHolder(CodeT code) { return CodeDataContainerFromCodeT(code); }
...
template <> struct WeakListVisitor<Context> {
    static void SetWeakNext(Context context, Object next) { context.set(Context::NEXT_CONTEXT_LINK, next, UPDATE_WRITE_BARRIER); }

    static Object WeakNext(Context context) { return context.next_context_link(); }

    static HeapObject WeakNextHolder(Context context) { return context; }

    static int WeakNextOffset() { return FixedArray::SizeFor(Context::NEXT_CONTEXT_LINK); }
    ...
    static void VisitLiveObject(Heap* heap, Context context, WeakObjectRetainer* retainer)
    {
        if (heap->gc_state() == Heap::MARK_COMPACT) {
            ...
            for (int idx = Context::FIRST_WEAK_SLOT; idx < Context::NATIVE_CONTEXT_SLOTS; ++idx) {
                ...
                MarkCompactCollector::RecordSlot(context, slot, HeapObject::cast(*slot));
            }
            ...
            DoWeakList<CodeT>(heap, context, retainer, Context::DEOPTIMIZED_CODE_LIST);
        }
    }

    template <class T> static void DoWeakList(Heap* heap, Context context, WeakObjectRetainer* retainer, int index)
    {
        ...
        if (MustRecordSlots(heap)) {
            ...
            heap->mark_compact_collector()->RecordSlot(context, head_slot, HeapObject::cast(list_head));
        }
    }
...
template <> struct WeakListVisitor<AllocationSite> {
    static void SetWeakNext(AllocationSite obj, Object next) { obj.set_weak_next(next, UPDATE_WRITE_BARRIER); }
...
template <> struct WeakListVisitor<JSFinalizationRegistry> {
    static void SetWeakNext(JSFinalizationRegistry obj, HeapObject next) { obj.set_next_dirty(next, UPDATE_WRITE_BARRIER); }

    static Object WeakNext(JSFinalizationRegistry obj) { return obj.next_dirty(); }

    static HeapObject WeakNextHolder(JSFinalizationRegistry obj) { return obj; }

    static int WeakNextOffset() { return JSFinalizationRegistry::kNextDirtyOffset; }

    static void VisitLiveObject(Heap* heap, JSFinalizationRegistry obj, WeakObjectRetainer*) { heap->set_dirty_js_finalization_registries_list_tail(obj); }
...
template Object VisitWeakList<JSFinalizationRegistry>(Heap* heap, Object list, WeakObjectRetainer* retainer);
} // namespace internal
} // namespace v8
