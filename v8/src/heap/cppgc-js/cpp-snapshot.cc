...
public:
    ...
    void SetDetachedness(Detachedness detachedness) { detachedness_ = detachedness; }
...
public:
    ...
    const HeapObjectHeader* header() const { return static_cast<const HeapObjectHeader*>(key_); }
    ...
    void AddEagerEphemeronEdge(const void* value, cppgc::TraceCallback callback) { eager_ephemeron_edges_.insert({ value, callback }); }
...
public:
    bool StateExists(const void* key) const { return states_.find(key) != states_.end(); }
    ...
    State& GetExistingState(const HeapObjectHeader& header) const { return static_cast<State&>(GetExistingState(&header)); }
...
void* ExtractEmbedderDataBackref(Isolate* isolate, v8::Local<v8::Value> v8_value)
{
    ...
    if (!v8_object->IsJSObject() || !JSObject::cast(*v8_object).MayHaveEmbedderFields())
        return nullptr;

    JSObject js_object = JSObject::cast(*v8_object);
    return LocalEmbedderHeapTracer::VerboseWrapperInfo(isolate->heap()->local_embedder_heap_tracer()->ExtractWrapperInfo(isolate, js_object)).instance();
}
...
public:
    ...
    {
        return static_cast<EmbedderNode*>(graph_.AddNode(std::unique_ptr<v8::EmbedderGraph::Node> { new EmbedderNode(header.GetName(), header.AllocatedSize()) }));
    }
    ...
    {
        DCHECK(parent.IsVisibleNotDependent());
        v8::Local<v8::Value> v8_value = ref.Get(reinterpret_cast<v8::Isolate*>(cpp_heap_.isolate()));
        if (!v8_value.IsEmpty()) {
            if (!parent.get_node()) {
                parent.set_node(AddNode(*parent.header()));
            }
            ...
            // has a specific name.
            if (!ref.WrapperClassId() || !edge_name.empty())
                return;

            void* back_reference_object = ExtractEmbedderDataBackref(reinterpret_cast<v8::internal::Isolate*>(cpp_heap_.isolate()), v8_value);
            if (back_reference_object) {
                ...
                if (profiler->HasGetDetachednessCallback()) {
                    back_state.get_node()->SetDetachedness(profiler->GetDetachedness(v8_value, ref.WrapperClassId()));
                }
            }
        }
    }
...
public:
    ...
    RootState& ParentAsRootState() const { return static_cast<RootState&>(parent_); }
...
public:
    ...
    // JS handling.
    void Visit(const TracedReferenceBase& ref) final { graph_builder_.VisitForVisibility(parent_scope_.ParentAsRegularState(), ref); }
...
public:
    ...
    // JS handling.
    void Visit(const TracedReferenceBase& ref) final { graph_builder_.AddEdge(parent_scope_.ParentAsRegularState(), ref, edge_name_); }

    void set_edge_name(std::string edge_name) { edge_name_ = std::move(edge_name); }
...
void CppGraphBuilderImpl::VisitEphemeronWithNonGarbageCollectedValueForVisibility(const HeapObjectHeader& key, const void* value, cppgc::TraceDescriptor value_desc)
{
...
void CppGraphBuilderImpl::Run()
{
    ...
    {
        ParentScope parent_scope(states_.CreateRootState(AddRootNode("C++ cross-thread roots")));
        GraphBuildingRootVisitor root_object_visitor(*this, parent_scope);
