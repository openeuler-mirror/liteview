...
public:
    using EmbedderDataSnapshot = std::pair<EmbedderDataSlot::EmbedderDataSlotSnapshot, EmbedderDataSlot::EmbedderDataSlotSnapshot>;

    CppMarkingState(Isolate* isolate, const WrapperDescriptor& wrapper_descriptor, cppgc::internal::MarkingStateBase& main_thread_marking_state)
        : isolate_(isolate)
    ...
    CppMarkingState(Isolate* isolate, const WrapperDescriptor& wrapper_descriptor, std::unique_ptr<cppgc::internal::MarkingStateBase> concurrent_marking_state)
        : isolate_(isolate)
        , wrapper_descriptor_(wrapper_descriptor)
    ...
    inline void MarkAndPush(const EmbedderDataSlot type_slot, const EmbedderDataSlot instance_slot);

    bool IsLocalEmpty() { return marking_state_.marking_worklist().IsLocalEmpty(); }
