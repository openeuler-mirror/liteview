...
{
    if (IsCommitted()) {
        const int expected_pages = static_cast<int>(target_capacity_ / Page::kPageSize);
        ...
        while (current_page) {
            ...
            heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kConcurrentlyAndPool, current_page);
...
bool SemiSpace::Commit()
{
    ...
    for (int pages_added = 0; pages_added < num_pages; pages_added++) {
        ...
        // old pages.
        Page* new_page = heap()->memory_allocator()->AllocatePage(MemoryAllocator::AllocationMode::kUsePool, this, NOT_EXECUTABLE);
        if (new_page == nullptr) {
            if (pages_added)
...
void SemiSpace::Uncommit()
{
    ...
    while (!memory_chunk_list_.Empty()) {
        ...
        heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kConcurrentlyAndPool, chunk);
    }
    current_page_ = nullptr;
    current_capacity_ = 0;
    size_t removed_page_size = static_cast<size_t>(actual_pages * Page::kPageSize);
...
void SemiSpace::RewindPages(int num_pages)
{
    ...
    while (num_pages > 0) {
        ...
        heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kConcurrentlyAndPool, last);
        num_pages--;
    }
}
...
void SemiSpace::FixPagesFlags(Page::MainThreadFlags flags, Page::MainThreadFlags mask)
{
    for (Page* page : *this) {
        page->set_owner(this);
...
void SemiSpace::Swap(SemiSpace* from, SemiSpace* to)
{
    ...
    std::swap(from->external_backing_store_bytes_, to->external_backing_store_bytes_);
...
void SemiSpace::Verify() const
{
    ...
    for (const Page* page : *this) {
        ...
        CHECK(page->IsFlagSet(is_from_space ? MemoryChunk::FROM_PAGE : MemoryChunk::TO_PAGE));
        CHECK(!page->IsFlagSet(is_from_space ? MemoryChunk::TO_PAGE : MemoryChunk::FROM_PAGE));
...
NewSpace::NewSpace(Heap* heap, LinearAllocationArea& allocation_info)
    : SpaceWithLinearArea(heap, NEW_SPACE, new NoFreeList(), allocation_counter_, allocation_info, linear_area_original_data_)
{
}

void NewSpace::ResetParkedAllocationBuffers() { parked_allocation_buffers_.clear(); }
...
void NewSpace::VerifyTop() const
{
    ...
    DCHECK_GE(linear_area_original_data_.get_original_top_acquire(), allocation_info_.start());

    // Ensure that limit() is <= original_limit_.
    DCHECK_LE(allocation_info_.limit(), linear_area_original_data_.get_original_limit_relaxed());
}
#endif // DEBUG
...
void NewSpace::VerifyImpl(Isolate* isolate, const Page* current_page, Address current_address) const
{
    ...
    while (true) {
        if (!Page::IsAlignedToPageSize(current_address)) {
            ...
            if (object.IsExternalString(cage_base)) {
                ...
                external_space_bytes[ExternalBackingStoreType::kExternalString] += string_size;
            }
...
SemiSpaceNewSpace::SemiSpaceNewSpace(Heap* heap, size_t initial_semispace_capacity, size_t max_semispace_capacity, LinearAllocationArea& allocation_info)
...
{
    Address new_limit = ComputeLimit(top(), to_space_.page_high(), ALIGN_TO_ALLOCATION_ALIGNMENT(min_size));
    ...
    heap()->CreateFillerObjectAt(limit(), static_cast<int>(to_space_.page_high() - limit()));
...
void SemiSpaceNewSpace::VerifyTop() const
{
    ...
    DCHECK_EQ(linear_area_original_data_.get_original_limit_relaxed(), to_space_.page_high());
}
#endif // DEBUG
...
void SemiSpaceNewSpace::MakeUnusedPagesInToSpaceIterable()
{
    ...
    heap()->CreateFillerObjectAt(limit(), static_cast<int>((*it)->area_end() - limit()));
...
bool SemiSpaceNewSpace::ShouldBePromoted(Address address) const
{
    ...
    return page->IsFlagSet(MemoryChunk::NEW_SPACE_BELOW_AGE_MARK) && (!page->ContainsLimit(current_age_mark) || address < current_age_mark);
}

std::unique_ptr<ObjectIterator> SemiSpaceNewSpace::GetObjectIterator(Heap* heap) { return std::unique_ptr<ObjectIterator>(new SemiSpaceObjectIterator(this)); }

bool SemiSpaceNewSpace::ContainsSlow(Address a) const { return from_space_.ContainsSlow(a) || to_space_.ContainsSlow(a); }
...
void SemiSpaceNewSpace::ZapUnusedMemory()
{
    if (!IsFromSpaceCommitted())
        return;
    for (Page* page : PageRange(from_space().first_page(), nullptr)) {
        heap_->memory_allocator()->ZapBlock(page->area_start(), page->HighWaterMark() - page->area_start(), heap_->ZapValue());
    }
}
...
bool SemiSpaceNewSpace::IsPromotionCandidate(const MemoryChunk* page) const { return !page->Contains(age_mark()); }
...
PagedSpaceForNewSpace::PagedSpaceForNewSpace(Heap* heap, size_t initial_capacity, size_t max_capacity, AllocationCounter& allocation_counter, LinearAllocationArea& allocation_info,
    LinearAreaOriginalData& linear_area_original_data)
    : PagedSpaceBase(
        heap, NEW_SPACE, NOT_EXECUTABLE, FreeList::CreateFreeListForNewSpace(), allocation_counter, allocation_info, linear_area_original_data, CompactionSpaceKind::kNone)
...
void PagedSpaceForNewSpace::Grow()
{
    ...
    DCHECK(TotalCapacity() < MaximumCapacity());
    target_capacity_ = std::min(MaximumCapacity(), RoundUp(static_cast<size_t>(v8_flags.semi_space_growth_factor) * TotalCapacity(), Page::kPageSize));
...
bool PagedSpaceForNewSpace::StartShrinking()
{
    ...
    size_t new_target_capacity = RoundUp(std::max(initial_capacity_, 2 * Size()), Page::kPageSize);
...
void PagedSpaceForNewSpace::UpdateInlineAllocationLimit(size_t size_in_bytes) { PagedSpaceBase::UpdateInlineAllocationLimit(size_in_bytes); }
...
#ifdef VERIFY_HEAP
void PagedSpaceForNewSpace::Verify(Isolate* isolate, ObjectVisitor* visitor) const
{
...
bool PagedSpaceForNewSpace::ShouldReleasePage() const { return current_capacity_ > target_capacity_; }

// -----------------------------------------------------------------------------
// PagedNewSpace implementation

PagedNewSpace::PagedNewSpace(Heap* heap, size_t initial_capacity, size_t max_capacity, LinearAllocationArea& allocation_info)
    : NewSpace(heap, allocation_info)
    , paged_space_(heap, initial_capacity, max_capacity, allocation_counter_, allocation_info_, linear_area_original_data_)
{
}
