...
void PretenturingHandler::UpdateAllocationSite(Map map, HeapObject object, PretenuringFeedbackMap* pretenuring_feedback)
{
    DCHECK_NE(pretenuring_feedback, &global_pretenuring_feedback_);
#ifdef DEBUG
    BasicMemoryChunk* chunk = BasicMemoryChunk::FromHeapObject(object);
    DCHECK_IMPLIES(chunk->IsToPage(), v8_flags.minor_mc || chunk->IsFlagSet(MemoryChunk::PAGE_NEW_NEW_PROMOTION));
    DCHECK_IMPLIES(!chunk->InYoungGeneration(), chunk->IsFlagSet(MemoryChunk::PAGE_NEW_OLD_PROMOTION));
#endif
    if (!v8_flags.allocation_site_pretenuring || !AllocationSite::CanTrack(map.instance_type())) {
        return;
    }
...
template <PretenturingHandler::FindMementoMode mode> AllocationMemento PretenturingHandler::FindAllocationMemento(Map map, HeapObject object)
{
    Address object_address = object.address();
    Address memento_address = object_address + ALIGN_TO_ALLOCATION_ALIGNMENT(object.SizeFromMap(map));
    ...
    if (!candidate_map_slot.contains_map_value(ReadOnlyRoots(heap_).allocation_memento_map().ptr())) {
        return AllocationMemento();
    }
    ...
    case kForRuntime:
        ...
        DCHECK(memento_address >= heap_->new_space()->limit() || memento_address + ALIGN_TO_ALLOCATION_ALIGNMENT(AllocationMemento::kSize) <= top);
