...
template <typename Trait> double MemoryController<Trait>::GrowingFactor(Heap* heap, size_t max_heap_size, double gc_speed, double mutator_speed)
{
    const double max_factor = MaxGrowingFactor(max_heap_size);
    const double factor = DynamicGrowingFactor(gc_speed, mutator_speed, max_factor);
...
template <typename Trait> double MemoryController<Trait>::MaxGrowingFactor(size_t max_heap_size)
{
    ...
    double factor = (max_size - Trait::kMinSize) * (kMaxSmallFactor - kMinSmallFactor) / (Trait::kMaxSize - Trait::kMinSize) + kMinSmallFactor;
    return factor;
}
...
size_t MemoryController<Trait>::CalculateAllocationLimit(
    Heap* heap, size_t current_size, size_t min_size, size_t max_size, size_t new_space_capacity, double factor, Heap::HeapGrowingMode growing_mode)
{
    switch (growing_mode) {
    ...
    const uint64_t limit
        = std::max(static_cast<uint64_t>(current_size * factor), static_cast<uint64_t>(current_size) + MinimumAllocationLimitGrowingStep(growing_mode)) + new_space_capacity;
    ...
    const size_t result = static_cast<size_t>(std::min(limit_above_min_size, halfway_to_the_max));
    if (v8_flags.trace_gc_verbose) {
        Isolate::FromHeap(heap)->PrintWithTimestamp("[%s] Limit: old size: %zu KB, new limit: %zu KB (%.1f)\n", Trait::kName, current_size / KB, result / KB, factor);
    }
    return result;
}
