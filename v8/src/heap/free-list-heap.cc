...
FreeSpace FreeListCategory::PickNodeFromList(size_t minimum_size, size_t* node_size)
{
    FreeSpace node = top();
    DCHECK(!node.is_null());
...
FreeSpace FreeListCategory::SearchForNodeInList(size_t minimum_size, size_t* node_size)
{
    FreeSpace prev_non_evac_node;
    for (FreeSpace cur_node = top(); !cur_node.is_null(); cur_node = cur_node.next()) {
        ...
        if (size >= minimum_size) {
            ...
            if (!prev_non_evac_node.is_null()) {
                ...
                if (chunk->owner_identity() == CODE_SPACE) {
                    chunk->heap()->UnprotectAndRegisterMemoryChunk(chunk, UnprotectMemoryOrigin::kMaybeOffMainThread);
                }
...
void FreeListCategory::Free(Address start, size_t size_in_bytes, FreeMode mode, FreeList* owner)
{
...
FreeList* FreeList::CreateFreeListForNewSpace() { return new FreeListManyCachedOriginForNewSpace(); }

FreeSpace FreeList::TryFindNodeIn(FreeListCategoryType type, size_t minimum_size, size_t* node_size)
{
...
FreeSpace FreeList::SearchForNodeInList(FreeListCategoryType type, size_t minimum_size, size_t* node_size)
{
...
size_t FreeListManyCached::Free(Address start, size_t size_in_bytes, FreeMode mode)
{
    Page* page = Page::FromAddress(start);
    page->DecreaseAllocatedBytes(size_in_bytes);
...
FreeSpace FreeListManyCached::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin)
{
    USE(origin);
    DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
FreeSpace FreeListManyCachedFastPath::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin)
{
    USE(origin);
    DCHECK_GE(kMaxBlockSize, size_in_bytes);
    FreeSpace node;
...
FreeSpace FreeListManyCachedFastPathForNewSpace::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin)
{
    FreeSpace node = FreeListManyCachedFastPath::Allocate(size_in_bytes, node_size, origin);
    if (!node.is_null())
        return node;
...
FreeSpace FreeListManyCachedOrigin::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin)
{
...
FreeSpace FreeListManyCachedOriginForNewSpace::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin)
{
    if (origin == AllocationOrigin::kGC) {
        return FreeListManyCached::Allocate(size_in_bytes, node_size, origin);
    } else {
        return FreeListManyCachedFastPathForNewSpace::Allocate(size_in_bytes, node_size, origin);
    }
}
...
size_t FreeListCategory::SumFreeList()
{
    ...
    while (!cur.is_null()) {
        ...
        DCHECK(cur.map_slot().contains_map_value(Page::FromHeapObject(cur)->heap()->isolate()->root(RootIndex::kFreeSpaceMap).ptr()));
