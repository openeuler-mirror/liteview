...
void IncrementalMarking::Observer::Step(int bytes_allocated, Address addr, size_t size)
{
...
void IncrementalMarking::MarkBlackAndVisitObjectDueToLayoutChange(HeapObject obj)
{
...
void IncrementalMarking::NotifyLeftTrimming(HeapObject from, HeapObject to)
{
    ...
    if (black_allocation() && Marking::IsBlack<AccessMode::ATOMIC>(new_mark_bit)) {
...
bool IncrementalMarking::CanBeStarted() const
{
    ...
    //   4) not a shared heap.
    return v8_flags.incremental_marking && heap_->gc_state() == Heap::NOT_IN_GC && heap_->deserialization_complete() && !heap_->isolate()->serializer_enabled()
        && !heap_->IsShared();
}
...
void IncrementalMarking::Start(GarbageCollector garbage_collector, GarbageCollectionReason gc_reason)
{
    ...
    if (v8_flags.trace_incremental_marking) {
        ...
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Start (%s): (size/limit/slack) v8: %zuMB / %zuMB "
                                              "/ %zuMB global: %zuMB / %zuMB / %zuMB\n",
            Heap::GarbageCollectionReasonToString(gc_reason), old_generation_size_mb, old_generation_limit_mb,
            old_generation_size_mb > old_generation_limit_mb ? 0 : old_generation_limit_mb - old_generation_size_mb, global_size_mb, global_limit_mb,
            global_size_mb > global_limit_mb ? 0 : global_limit_mb - global_size_mb);
    }
    ...
    const auto scope_id = is_major ? GCTracer::Scope::MC_INCREMENTAL_START : GCTracer::Scope::MINOR_MC_INCREMENTAL_START;
    TRACE_EVENT1("v8", is_major ? "V8.GCIncrementalMarkingStart" : "V8.GCMinorIncrementalMarkingStart", "epoch", heap_->tracer()->CurrentEpoch(scope_id));
    ...
    if (is_major) {
        ...
        StartMarkingMajor();
        heap_->AddAllocationObserversToAllSpaces(&old_generation_observer_, &new_generation_observer_);
...
class IncrementalMarking::IncrementalMarkingRootMarkingVisitor final : public RootVisitor {
public:
...
void IncrementalMarking::MarkRoots()
{
    ...
    CodePageHeaderModificationScope rwx_write_scope("Marking of builtins table entries require write access to Code page "
                                                    "header");
    if (IsMajorMarking()) {
        heap_->IterateRoots(&visitor, base::EnumSet<SkipRoot> { SkipRoot::kStack, SkipRoot::kMainThreadHandles, SkipRoot::kWeak });
    } else {
        heap_->IterateRoots(&visitor,
            base::EnumSet<SkipRoot> {
                SkipRoot::kStack, SkipRoot::kMainThreadHandles, SkipRoot::kWeak, SkipRoot::kExternalStringTable, SkipRoot::kGlobalHandles, SkipRoot::kOldGeneration });

        heap()->isolate()->global_handles()->IterateYoungStrongAndDependentRoots(&visitor);

        std::vector<PageMarkingItem> marking_items;
        RememberedSet<OLD_TO_NEW>::IterateMemoryChunks(heap_, [&marking_items](MemoryChunk* chunk) { marking_items.emplace_back(chunk); });

        V8::GetCurrentPlatform()
            ->CreateJob(v8::TaskPriority::kUserBlocking,
                std::make_unique<YoungGenerationMarkingJob>(
                    heap_->isolate(), heap_, minor_collector_->marking_worklists(), std::move(marking_items), YoungMarkingJobType::kIncremental))
            ->Join();
    }
}
...
void IncrementalMarking::StartMarkingMajor()
{
    if (heap_->isolate()->serializer_enabled()) {
        ...
        if (v8_flags.trace_incremental_marking) {
            heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Start delayed - serializer\n");
        }
        return;
    }
    if (v8_flags.trace_incremental_marking) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Start marking\n");
    }
    ...
    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_INCREMENTAL_EMBEDDER_PROLOGUE);
    ...
    MarkingBarrier::ActivateAll(heap(), is_compacting_, MarkingBarrierType::kMajor);
...
void IncrementalMarking::StartMarkingMinor()
{
    ...
    if (v8_flags.trace_incremental_marking) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] (MinorMC) Start marking\n");
    }
    ...
    if (v8_flags.concurrent_marking && !heap_->IsTearingDown()) {
        heap_->concurrent_marking()->ScheduleJob(GarbageCollector::MINOR_MARK_COMPACTOR);
    }

    if (v8_flags.trace_incremental_marking) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] (MinorMC) Running\n");
    }
}
...
void IncrementalMarking::StartBlackAllocation()
{
    ...
    {
        CodePageHeaderModificationScope rwx_write_scope("Marking Code objects requires write access to the Code page header");
    ...
    if (heap()->isolate()->is_shared_heap_isolate()) {
        ...
        heap()->isolate()->global_safepoint()->IterateClientIsolates([](Isolate* client) { client->heap()->MarkSharedLinearAllocationAreasBlack(); });
    }
    heap()->safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->MarkLinearAllocationAreaBlack(); });
    if (v8_flags.trace_incremental_marking) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Black allocation started\n");
    }
}
...
void IncrementalMarking::PauseBlackAllocation()
{
    ...
    if (heap()->isolate()->is_shared_heap_isolate()) {
        DCHECK_EQ(heap()->shared_space()->top(), kNullAddress);
        heap()->isolate()->global_safepoint()->IterateClientIsolates([](Isolate* client) { client->heap()->UnmarkSharedLinearAllocationAreas(); });
    }
    heap()->safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->UnmarkLinearAllocationArea(); });
    if (v8_flags.trace_incremental_marking) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Black allocation paused\n");
    }
...
void IncrementalMarking::FinishBlackAllocation()
{
    if (black_allocation_) {
        black_allocation_ = false;
        if (v8_flags.trace_incremental_marking) {
            heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Black allocation finished\n");
        }
    }
}
...
void IncrementalMarking::UpdateMarkingWorklistAfterYoungGenGC()
{
    ...
    major_collector_->marking_worklists()->Update([this, marking_state, cage_base, filler_map](HeapObject obj, HeapObject* out) -> bool {
        ...
        if (Heap::InFromPage(obj)) {
            ...
            *out = dest;
            ...
            // new space.
            DCHECK(Heap::IsLargeObject(obj) || Page::FromHeapObject(obj)->IsFlagSet(Page::PAGE_NEW_NEW_PROMOTION));
            DCHECK_IMPLIES(v8_flags.minor_mc, !Page::FromHeapObject(obj)->IsFlagSet(Page::PAGE_NEW_NEW_PROMOTION));
            DCHECK_IMPLIES(v8_flags.minor_mc, !obj.map_word(cage_base, kRelaxedLoad).IsForwardingAddress());
            ...
            *out = obj;
            ...
            if (!Heap::IsLargeObject(obj) && Page::FromHeapObject(obj)->IsFlagSet(Page::PAGE_NEW_OLD_PROMOTION)) {
                if (marking_state->IsWhite(obj)) {
                    return false;
                }
                *out = obj;
            ...
            DCHECK_IMPLIES(marking_state->IsWhite(obj), obj.IsFreeSpaceOrFiller(cage_base));
...
void IncrementalMarking::EmbedderStep(double expected_duration_ms, double* duration_ms)
{
    DCHECK(IsMarking());
    if (!heap_->local_embedder_heap_tracer()->SupportsIncrementalEmbedderSteps()) {
        *duration_ms = 0.0;
        return;
    }
...
bool IncrementalMarking::Stop()
{
    ...
    if (v8_flags.trace_incremental_marking) {
        ...
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Stopping: old generation %dMB, limit %dMB, "
                                              "overshoot %dMB\n",
            old_generation_size_mb, old_generation_limit_mb, std::max(0, old_generation_size_mb - old_generation_limit_mb));
    }
...
{
    const double recorded_time_to_marking_task = heap_->tracer()->AverageTimeToIncrementalMarkingTask();
    const double current_time_to_marking_task = incremental_marking_job_.CurrentTimeToTask();
    if (recorded_time_to_marking_task == 0.0)
        return 0.0;
...
bool IncrementalMarking::ShouldWaitForTask()
{
    ...
    if (v8_flags.trace_incremental_marking && wait_for_task) {
        heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Delaying GC via stack guard. time left: "
                                              "%fms\n",
...
bool IncrementalMarking::TryInitializeTaskTimeout()
{
    ...
    const double overshoot_ms = std::max(kMinOvershootMs, (now - start_time_ms_) * kAllowedOvershoot);
    const double time_to_marking_task = CurrentTimeToMarkingTask();

    if (time_to_marking_task == 0.0 || time_to_marking_task > overshoot_ms) {
        if (v8_flags.trace_incremental_marking) {
            heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Not delaying marking completion. time to "
        ...
        if (v8_flags.trace_incremental_marking) {
            heap()->isolate()->PrintWithTimestamp("[IncrementalMarking] Delaying GC via stack guard. time to task: "
                                                  "%fms "
...
void IncrementalMarking::FastForwardSchedule()
{
    ...
    if (scheduled_bytes_to_mark_ < bytes_marked_) {
        ...
        if (v8_flags.trace_incremental_marking) {
            heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Fast-forwarded schedule\n");
        }
    }
}
...
void IncrementalMarking::ScheduleBytesToMarkBasedOnTime(double time_ms)
{
    ...
    if (schedule_update_time_ms_ + kMinTimeBetweenScheduleInMs > time_ms)
        return;
    double delta_ms = std::min(time_ms - schedule_update_time_ms_, kTargetMarkingWallTimeInMs);
    ...
    if (v8_flags.trace_incremental_marking) {
        heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Scheduled %zuKB to mark based on time delta "
                                             "%.1fms\n",
...
void IncrementalMarking::AdvanceAndFinalizeIfComplete()
{
    ...
    heap()->FinalizeIncrementalMarkingIfComplete(GarbageCollectionReason::kFinalizeMarkingViaTask);
}
...
void IncrementalMarking::AdvanceAndFinalizeIfNecessary()
{
    ...
    if (collection_requested_via_stack_guard_) {
        heap()->FinalizeIncrementalMarkingIfComplete(GarbageCollectionReason::kFinalizeMarkingViaStackGuard);
    }
}

void IncrementalMarking::AdvanceForTesting(double max_step_size_in_ms) { Step(max_step_size_in_ms, StepOrigin::kV8); }
...
bool IncrementalMarking::ShouldFinalize() const
{
    ...
    return heap()->mark_compact_collector()->local_marking_worklists()->IsEmpty() && heap()->local_embedder_heap_tracer()->ShouldFinalizeIncrementalMarking();
}
...
size_t IncrementalMarking::StepSizeToMakeProgress()
{
    ...
    return std::min(std::max({ initial_old_generation_size_ / kTargetStepCount, IncrementalMarking::kMinStepSizeInBytes }), kMaxStepSizeInByte);
}
...
void IncrementalMarking::ScheduleBytesToMarkBasedOnAllocation()
{
    ...
    if (v8_flags.trace_incremental_marking) {
        heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Scheduled %zuKB to mark based on allocation "
...
void IncrementalMarking::FetchBytesMarkedConcurrently()
{
    if (v8_flags.concurrent_marking) {
        ...
        if (v8_flags.trace_incremental_marking) {
            heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Marked %zuKB on background threads\n", heap_->concurrent_marking()->TotalMarkedBytes() / KB);
        }
    }
}
...
size_t IncrementalMarking::ComputeStepSizeInBytes(StepOrigin step_origin)
{
    ...
    if (v8_flags.trace_incremental_marking) {
        if (scheduled_bytes_to_mark_ > bytes_marked_) {
            heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Marker is %zuKB behind schedule\n", (scheduled_bytes_to_mark_ - bytes_marked_) / KB);
        } else {
            heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Marker is %zuKB ahead of schedule\n", (bytes_marked_ - scheduled_bytes_to_mark_) / KB);
        }
    }
...
void IncrementalMarking::Step(double max_step_size_in_ms, StepOrigin step_origin)
{
    NestedTimedHistogramScope incremental_marking_scope(heap_->isolate()->counters()->gc_incremental_marking());
    TRACE_EVENT1("v8", "V8.GCIncrementalMarking", "epoch", heap_->tracer()->CurrentEpoch(GCTracer::Scope::MC_INCREMENTAL));
    TRACE_GC_EPOCH(heap_->tracer(), GCTracer::Scope::MC_INCREMENTAL, ThreadKind::kMain);
    DCHECK(IsMajorMarking());
...
// Only print marking worklist in debug mode to save ~40KB of code size.
#ifdef DEBUG
    if (v8_flags.trace_incremental_marking && v8_flags.trace_concurrent_marking && v8_flags.trace_gc_verbose) {
        major_collector_->marking_worklists()->Print();
    }
#endif
    if (v8_flags.trace_incremental_marking) {
        heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Marking speed %.fKB/ms\n", heap()->tracer()->IncrementalMarkingSpeedInBytesPerMillisecond());
    }
    ...
    const double marking_speed = heap()->tracer()->IncrementalMarkingSpeedInBytesPerMillisecond();
    size_t max_step_size = GCIdleTimeHandler::EstimateMarkingStepSize(max_step_size_in_ms, marking_speed);
    bytes_to_process = std::min(ComputeStepSizeInBytes(step_origin), max_step_size);
    ...
    std::tie(v8_bytes_processed, std::ignore) = major_collector_->ProcessMarkingWorklist(bytes_to_process);
    if (heap_->local_embedder_heap_tracer()->InUse()) {
        embedder_deadline = std::min(max_step_size_in_ms, static_cast<double>(bytes_to_process) / marking_speed);
    ...
    if (v8_flags.trace_incremental_marking) {
        heap_->isolate()->PrintWithTimestamp("[IncrementalMarking] Step %s V8: %zuKB (%zuKB), embedder: %fms "
                                             "(%fms) "
                                             "in %.1f\n",
            step_origin == StepOrigin::kV8 ? "in v8" : "in task", v8_bytes_processed / KB, bytes_to_process / KB, embedder_duration, embedder_deadline, current_time - start);
    }
}
