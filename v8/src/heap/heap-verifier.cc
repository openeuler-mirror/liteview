...
void HeapVerification::Verify()
{
    ...
    if (!isolate()->context().is_null() && !isolate()->raw_native_context().is_null()) {
        Object normalized_map_cache = isolate()->raw_native_context().normalized_map_cache();

        if (normalized_map_cache.IsNormalizedMapCache()) {
            NormalizedMapCache::cast(normalized_map_cache).NormalizedMapCacheVerify(isolate());
        }
    }
...
public:
    SlotVerifyingVisitor(Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed)
    ...
    void VisitCodePointer(HeapObject host, CodeObjectSlot slot) override
    {
        ...
        if (ShouldHaveBeenRecorded(host, MaybeObject::FromObject(slot.load(code_cage_base())))) {
            CHECK_GT(untyped_->count(slot.address()), 0);
        }
    }
    ...
    void VisitCodeTarget(Code host, RelocInfo* rinfo) override
    {
        ...
        if (ShouldHaveBeenRecorded(host, MaybeObject::FromObject(target))) {
            CHECK(InTypedSet(SlotType::kCodeEntry, rinfo->pc()) || (rinfo->IsInConstantPool() && InTypedSet(SlotType::kConstPoolCodeEntry, rinfo->constant_pool_entry_address())));
        }
    }
...
protected:
    bool InUntypedSet(ObjectSlot slot) { return untyped_->count(slot.address()) > 0; }

private:
    bool InTypedSet(SlotType type, Address slot) { return typed_->count(std::make_pair(type, slot)) > 0; }
...
public:
    OldToNewSlotVerifyingVisitor(Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed, EphemeronRememberedSet* ephemeron_remembered_set)
    ...
    {
        DCHECK_IMPLIES(target->IsStrongOrWeak() && Heap::InYoungGeneration(target), Heap::InToPage(target));
        return target->IsStrongOrWeak() && Heap::InYoungGeneration(target) && !Heap::InYoungGeneration(host);
    }

    void VisitEphemeron(HeapObject host, int index, ObjectSlot key, ObjectSlot target) override
    {
...
public:
    OldToSharedSlotVerifyingVisitor(Isolate* isolate, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed)
        : SlotVerifyingVisitor(isolate, untyped, typed)
    {
    }
    ...
    bool ShouldHaveBeenRecorded(HeapObject host, MaybeObject target) override
    {
        return target->IsStrongOrWeak() && Heap::InSharedWritableHeap(target) && !Heap::InYoungGeneration(host) && !host.InSharedWritableHeap();
    }
};

template <RememberedSetType direction> void CollectSlots(MemoryChunk* chunk, Address start, Address end, std::set<Address>* untyped, std::set<std::pair<SlotType, Address>>* typed)
{
...
void HeapVerifier::VerifyRememberedSetFor(Heap* heap, HeapObject object)
{
    ...
    base::LockGuard<base::Mutex, base::NullBehavior::kIgnoreIfNull> lock_guard(chunk->mutex());
    ...
    if (!Heap::InYoungGeneration(object)) {
        ...
        OldToNewSlotVerifyingVisitor old_to_new_visitor(heap->isolate(), &old_to_new, &typed_old_to_new, &heap->ephemeron_remembered_set_);
        ...
        OldToSharedSlotVerifyingVisitor old_to_shared_visitor(heap->isolate(), &old_to_shared, &typed_old_to_shared);
...
// static
void HeapVerifier::VerifyObjectLayoutChange(Heap* heap, HeapObject object, Map new_map)
{
...
// static
void HeapVerifier::VerifySafeMapTransition(Heap* heap, HeapObject object, Map new_map)
{
    PtrComprCageBase cage_base(heap->isolate());
    ...
    if (object.IsString(cage_base)
        && (new_map == ReadOnlyRoots(heap).thin_string_map() || new_map == ReadOnlyRoots(heap).thin_one_byte_string_map() || new_map == ReadOnlyRoots(heap).shared_thin_string_map()
    ...
    if (v8_flags.shared_string_table && object.IsString(cage_base) && InstanceTypeChecker::IsInternalizedString(new_map.instance_type())) {
...
#ifdef V8_EXTERNAL_CODE_SPACE
    DCHECK_EQ(new_visitor.number_of_code_slots(), old_visitor.number_of_code_slots());
