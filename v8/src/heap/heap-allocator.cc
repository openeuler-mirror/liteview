...
HeapAllocator::HeapAllocator(Heap* heap)
    : heap_(heap)
{
}
...
void HeapAllocator::Setup()
{
    ...
    space_for_maps_ = spaces_[MAP_SPACE] ? static_cast<PagedSpace*>(spaces_[MAP_SPACE]) : static_cast<PagedSpace*>(spaces_[OLD_SPACE]);

    shared_old_allocator_ = heap_->shared_space_allocator_.get();
    shared_map_allocator_ = heap_->shared_map_allocator_ ? heap_->shared_map_allocator_.get() : shared_old_allocator_;
    shared_lo_space_ = heap_->shared_lo_allocation_space();
}

void HeapAllocator::SetReadOnlySpace(ReadOnlySpace* read_only_space) { read_only_space_ = read_only_space; }
...
AllocationResult HeapAllocator::AllocateRawLargeInternal(int size_in_bytes, AllocationType allocation, AllocationOrigin origin, AllocationAlignment alignment)
{
    ...
    case AllocationType::kSharedOld:
        return shared_lo_space()->AllocateRawBackground(heap_->main_thread_local_heap(), size_in_bytes);
...
AllocationResult HeapAllocator::AllocateRawWithLightRetrySlowPath(int size, AllocationType allocation, AllocationOrigin origin, AllocationAlignment alignment)
{
    ...
    for (int i = 0; i < 2; i++) {
        if (IsSharedAllocationType(allocation)) {
            heap_->CollectGarbageShared(heap_->main_thread_local_heap(), GarbageCollectionReason::kAllocationFailure);
        } else {
...
AllocationResult HeapAllocator::AllocateRawWithRetryOrFailSlowPath(int size, AllocationType allocation, AllocationOrigin origin, AllocationAlignment alignment)
{
    ...
    if (IsSharedAllocationType(allocation)) {
        ...
        AlwaysAllocateScope shared_scope(heap_->isolate()->shared_heap_isolate()->heap());
...
// static
void HeapAllocator::InitializeOncePerProcess() { SetAllocationGcInterval(v8_flags.gc_interval); }

// static
void HeapAllocator::SetAllocationGcInterval(int allocation_gc_interval) { allocation_gc_interval_.store(allocation_gc_interval, std::memory_order_relaxed); }
...
void HeapAllocator::UpdateAllocationTimeout()
{
    if (v8_flags.random_gc_interval > 0) {
        const int new_timeout = allocation_timeout_ <= 0 ? heap_->isolate()->fuzzer_rng()->NextInt(v8_flags.random_gc_interval + 1) : allocation_timeout_;
