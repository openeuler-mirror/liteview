...
public:
    ...
    V8_INLINE void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) final { VisitPointersImpl(host, start, end); }

    V8_INLINE void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) final { VisitPointersImpl(host, start, end); }
    ...
    inline void VisitEphemeron(HeapObject obj, int entry, ObjectSlot key, ObjectSlot value) override
    {
        ...
        if (ObjectInYoungGeneration(*key)) {
            ...
            scavenger_->RememberPromotedEphemeron(EphemeronHashTable::unchecked_cast(obj), entry);
        } else {
...
private:
    template <typename TSlot> V8_INLINE void VisitPointersImpl(HeapObject host, TSlot start, TSlot end)
    {
    ...
    template <typename THeapObjectSlot> V8_INLINE void HandleSlot(HeapObject host, THeapObjectSlot slot, HeapObject target)
    {
        ...
        if (Heap::InFromPage(target)) {
            ...
            if (result == KEEP_SLOT) {
                ...
                RememberedSet<OLD_TO_NEW>::Insert<AccessMode::ATOMIC>(chunk, slot.address());
            }
            ...
            RememberedSet<OLD_TO_OLD>::Insert<AccessMode::ATOMIC>(MemoryChunk::FromHeapObject(host), slot.address());
        }

        if (target.InSharedWritableHeap()) {
            DCHECK(!scavenger_->heap()->IsShared());
            MemoryChunk* chunk = MemoryChunk::FromHeapObject(host);
            RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::ATOMIC>(chunk, slot.address());
        }
    }
...
V8_INLINE bool IsUnscavengedHeapObject(Heap* heap, Object object) { return Heap::InFromPage(object) && !HeapObject::cast(object).map_word(kRelaxedLoad).IsForwardingAddress(); }

// Same as IsUnscavengedHeapObject() above but specialized for HeapObjects.
V8_INLINE bool IsUnscavengedHeapObject(Heap* heap, HeapObject heap_object) { return Heap::InFromPage(heap_object) && !heap_object.map_word(kRelaxedLoad).IsForwardingAddress(); }

bool IsUnscavengedHeapObjectSlot(Heap* heap, FullObjectSlot p) { return IsUnscavengedHeapObject(heap, *p); }

} // namespace

ScavengerCollector::JobTask::JobTask(ScavengerCollector* outer, std::vector<std::unique_ptr<Scavenger>>* scavengers,
    std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> memory_chunks, Scavenger::CopiedList* copied_list, Scavenger::PromotionList* promotion_list)
    : outer_(outer)
...
void ScavengerCollector::JobTask::Run(JobDelegate* delegate)
{
    ...
    if (delegate->IsJoiningThread()) {
        ...
        TRACE_GC_EPOCH(outer_->heap_->tracer(), GCTracer::Scope::SCAVENGER_BACKGROUND_SCAVENGE_PARALLEL, ThreadKind::kBackground);
        ProcessItems(delegate, scavenger);
    }
}
...
size_t ScavengerCollector::JobTask::GetMaxConcurrency(size_t worker_count) const
{
    ...
    return std::min<size_t>(
        scavengers_->size(), std::max<size_t>(remaining_memory_chunks_.load(std::memory_order_relaxed), worker_count + copied_list_->Size() + promotion_list_->Size()));
}
...
void ScavengerCollector::JobTask::ProcessItems(JobDelegate* delegate, Scavenger* scavenger)
{
    ...
    if (v8_flags.trace_parallel_scavenge) {
        PrintIsolate(outer_->heap_->isolate(), "scavenge[%p]: time=%.2f copied=%zu promoted=%zu\n", static_cast<void*>(this), scavenging_time, scavenger->bytes_copied(),
            scavenger->bytes_promoted());
    }
}
...
public:
    void VisitRootPointer(Root root, const char* description, FullObjectSlot p) final { UpdatePointer(p); }
...
private:
    void UpdatePointer(FullObjectSlot p)
    {
        ...
        CHECK_IMPLIES(Heap::InYoungGeneration(dest), Heap::InToPage(dest) || Heap::IsLargeObject(dest));
    }
};
...
public:
    ...
    ~ScopedFullHeapCrashKey() { isolate_->AddCrashKey(v8::CrashKeyId::kDumpType, ""); }
...
void ScavengerCollector::CollectGarbage()
{
...

    {
        ...
        filter_scope.FilterOldSpaceSweepingPages([](Page* page) { return !page->ContainsSlots<OLD_TO_NEW>(); });

        const bool is_logging = isolate_->log_object_relocation();
        for (int i = 0; i < num_scavenge_tasks; ++i) {
            scavengers.emplace_back(new Scavenger(this, heap_, is_logging, &empty_chunks, &copied_list, &promotion_list, &ephemeron_table_list, i));
        }

        std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> memory_chunks;
        RememberedSet<OLD_TO_NEW>::IterateMemoryChunks(heap_, [&memory_chunks](MemoryChunk* chunk) { memory_chunks.emplace_back(ParallelWorkItem {}, chunk); });
...

        {
            ...
            isolate_->global_handles()->ComputeWeaknessForYoungObjects(&JSObject::IsUnmodifiedApiObject);
        }
        {
            // Copy roots.
            ...
            base::EnumSet<SkipRoot> options({ SkipRoot::kExternalStringTable, SkipRoot::kGlobalHandles, SkipRoot::kOldGeneration });
        ...
        }
        {
            ...
            V8::GetCurrentPlatform()
                ->CreateJob(v8::TaskPriority::kUserBlocking, std::make_unique<JobTask>(this, &scavengers, std::move(memory_chunks), &copied_list, &promotion_list))
                ->Join();
        ...
        if (V8_UNLIKELY(v8_flags.scavenge_separate_stack_scanning)) {
            IterateStackAndScavenge(&root_scavenge_visitor, &scavengers, kMainThreadId);
            DCHECK(copied_list.IsEmpty());
            DCHECK(promotion_list.IsEmpty());
        }
    ...
    if (v8_flags.concurrent_marking) {
        ...
        for (Page* p : PageRange(semi_space_new_space->from_space().first_page(), nullptr)) {
...
#ifdef DEBUG
        RememberedSet<OLD_TO_NEW>::IterateMemoryChunks(heap_, [](MemoryChunk* chunk) { DCHECK(chunk->possibly_empty_buckets()->IsEmpty()); });
#endif
    }
...
void ScavengerCollector::IterateStackAndScavenge(
    ...
    RootScavengeVisitor* root_scavenge_visitor, std::vector<std::unique_ptr<Scavenger>>* scavengers, int main_thread_id)
{
    ...
    for (auto& scavenger : *scavengers) {
        survived_bytes_before += scavenger->bytes_copied() + scavenger->bytes_promoted();
    }
    heap_->IterateStackRoots(root_scavenge_visitor);
    (*scavengers)[main_thread_id]->Process();
    size_t survived_bytes_after = 0;
    for (auto& scavenger : *scavengers) {
        survived_bytes_after += scavenger->bytes_copied() + scavenger->bytes_promoted();
    }
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "V8.GCScavengerStackScanning", "survived_bytes_before", survived_bytes_before, "survived_bytes_after", survived_bytes_after);
...
void ScavengerCollector::HandleSurvivingNewLargeObjects()
{
    ...
    for (SurvivingNewLargeObjectMapEntry update_info : surviving_new_large_objects_) {
        ...
        if (is_compacting && marking_state->IsBlack(object) && MarkCompactCollector::IsOnEvacuationCandidate(map)) {
            RememberedSet<OLD_TO_OLD>::Insert<AccessMode::ATOMIC>(MemoryChunk::FromHeapObject(object), object.map_slot().address());
        }
...
int ScavengerCollector::NumberOfScavengeTasks()
{
    ...
    if (!heap_->CanPromoteYoungAndExpandOldGeneration(static_cast<size_t>(tasks * Page::kPageSize))) {
...
Scavenger::Scavenger(ScavengerCollector* collector, Heap* heap, bool is_logging, EmptyChunksList* empty_chunks, CopiedList* copied_list, PromotionList* promotion_list,
    ...
    , is_compacting_including_map_space_(is_compacting_ && v8_flags.compact_maps)
    , shared_string_table_(shared_old_allocator_.get() != nullptr)
    , mark_shared_heap_(heap->isolate()->is_shared_space_isolate())
{
}

void Scavenger::IterateAndScavengePromotedObject(HeapObject target, Map map, int size)
{
    ...
    // its slots.
    const bool record_slots = is_compacting_ && heap()->atomic_marking_state()->IsBlack(target);
...
void Scavenger::AddPageToSweeperIfNecessary(MemoryChunk* page)
{
    ...
    if ((space == OLD_SPACE) && !page->SweepingDone()) {
        heap()->sweeper()->AddPage(space, reinterpret_cast<Page*>(page), Sweeper::READD_TEMPORARY_REMOVED_PAGE);
    }
}
...
void Scavenger::ScavengePage(MemoryChunk* page)
{
    ...
    if (page->slot_set<OLD_TO_NEW, AccessMode::ATOMIC>() != nullptr) {
        InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToNew(page, InvalidatedSlotsFilter::LivenessCheck::kNo);
        RememberedSet<OLD_TO_NEW>::IterateAndTrackEmptyBuckets(
            page,
            [this, page, record_old_to_shared_slots, &filter](MaybeObjectSlot slot) {
    ...
    RememberedSet<OLD_TO_NEW>::IterateTyped(page, [=](SlotType slot_type, Address slot_address) {
        return UpdateTypedSlotHelper::UpdateTypedSlot(heap_, slot_type, slot_address, [this, page, slot_type, slot_address, record_old_to_shared_slots](FullMaybeObjectSlot slot) {
...
void Scavenger::Process(JobDelegate* delegate)
{
    ...
    do {
        ...
        while (promotion_list_local_.ShouldEagerlyProcessPromotionList() && copied_list_local_.Pop(&object_and_size)) {
...
void ScavengerCollector::ProcessWeakReferences(EphemeronTableList* ephemeron_table_list)
{
...
void Scavenger::Finalize()
{
    ...
    for (auto it = ephemeron_remembered_set_.begin(); it != ephemeron_remembered_set_.end(); ++it) {
        auto insert_result = heap()->ephemeron_remembered_set_.insert({ it->first, std::unordered_set<int>() });
...
void Scavenger::AddEphemeronHashTable(EphemeronHashTable table) { ephemeron_table_list_local_.Push(table); }

template <typename TSlot> void Scavenger::CheckOldToNewSlotForSharedUntyped(MemoryChunk* chunk, TSlot slot)
{
    MaybeObject object = *slot;
    HeapObject heap_object;

    if (object.GetHeapObject(&heap_object) && heap_object.InSharedWritableHeap()) {
        RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::ATOMIC>(chunk, slot.address());
    }
}

void Scavenger::CheckOldToNewSlotForSharedTyped(MemoryChunk* chunk, SlotType slot_type, Address slot_address)
{
    HeapObject heap_object = UpdateTypedSlotHelper::GetTargetObject(chunk->heap(), slot_type, slot_address);

#if DEBUG
    UpdateTypedSlotHelper::UpdateTypedSlot(chunk->heap(), slot_type, slot_address, [heap_object](FullMaybeObjectSlot slot) {
