...
inline BytesAndDuration MakeBytesAndDuration(uint64_t bytes, double duration) { return std::make_pair(bytes, duration); }
...
#undef DEFINE_SCOPE
    ...
    public:
        ...
        Event(Type type, State state, GarbageCollectionReason gc_reason, const char* collector_reason);
        ...
        IncrementalMarkingInfos incremental_scopes[Scope::NUMBER_OF_INCREMENTAL_SCOPES];
    };
    ...
    static double CombineSpeedsInBytesPerMillisecond(double default_speed, double optional_speed);
    ...
    void SampleAllocation(double current_ms, size_t new_space_counter_bytes, size_t old_generation_counter_bytes, size_t embedder_counter_bytes);
    ...
    // Returns 0 if no allocation events have been recorded.
    double EmbedderAllocationThroughputInBytesPerMillisecond(double time_ms = 0) const;
    ...
    bool IsCurrentGCDueToAllocationFailure() const { return current_.gc_reason == GarbageCollectionReason::kAllocationFailure; }
...
private:
    ...
    V8_INLINE constexpr const IncrementalMarkingInfos& incremental_scope(Scope::ScopeId id) const;
    ...
    static double AverageSpeed(const base::RingBuffer<BytesAndDuration>& buffer, const BytesAndDuration& initial, double time_ms);

    void ResetForTesting();
    void ResetIncrementalMarkingCounters();
    void RecordIncrementalMarkingSpeed(size_t bytes, double duration);
    void RecordMutatorUtilization(double mark_compactor_end_time, double mark_compactor_duration);
    ...
    IncrementalMarkingInfos incremental_scopes_[Scope::NUMBER_OF_INCREMENTAL_SCOPES];
