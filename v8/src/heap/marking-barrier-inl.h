...
bool MarkingBarrier::MarkValue(HeapObject host, HeapObject value)
{
    ...
    // filler map.
    DCHECK(!marking_state_.IsImpossible(host) || value == ReadOnlyRoots(heap_->isolate()).one_pointer_filler_map());
...
template <typename TSlot> inline void MarkingBarrier::MarkRange(HeapObject host, TSlot start, TSlot end)
{
