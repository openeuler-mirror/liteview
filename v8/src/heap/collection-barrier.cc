...
bool CollectionBarrier::WasGCRequested() { return collection_requested_.load(); }
...
void CollectionBarrier::ActivateStackGuardAndPostTask()
{
    ...
    V8::GetCurrentPlatform()->GetForegroundTaskRunner(reinterpret_cast<v8::Isolate*>(isolate))->PostTask(std::make_unique<BackgroundCollectionInterruptTask>(heap_));
}
...
void CollectionBarrier::StopTimeToCollectionTimer()
{
    if (collection_requested_.load()) {
        ...
        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "V8.GC.TimeToCollectionOnBackground", TRACE_EVENT_SCOPE_THREAD, "duration", delta.InMillisecondsF());
