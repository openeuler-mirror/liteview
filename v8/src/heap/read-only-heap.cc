...
// remain.
base::LazyInstance<std::weak_ptr<ReadOnlyArtifacts>>::type read_only_artifacts_ = LAZY_INSTANCE_INITIALIZER;
...
{
    static bool shared_memory_allocation_supported = GetPlatformPageAllocator()->CanAllocateSharedPages();
...
void ReadOnlyHeap::SetUp(Isolate* isolate, SnapshotData* read_only_snapshot_data, bool can_rehash)
{
    ...
    if (IsReadOnlySpaceShared()) {
        ...
        if (read_only_snapshot_data != nullptr) {
            ...
            std::shared_ptr<ReadOnlyArtifacts> artifacts = read_only_artifacts_.Get().lock();
            if (!artifacts) {
...
// static
ReadOnlyHeap* ReadOnlyHeap::CreateInitalHeapForBootstrapping(Isolate* isolate, std::shared_ptr<ReadOnlyArtifacts> artifacts)
{
    ...
    if (COMPRESS_POINTERS_IN_ISOLATE_CAGE_BOOL) {
        ...
        std::unique_ptr<SoleReadOnlyHeap> sole_ro_heap(new SoleReadOnlyHeap(ro_space));
...
bool ReadOnlyHeap::read_only_object_cache_is_initialized() const { return read_only_object_cache_.size() > 0; }

size_t ReadOnlyHeap::read_only_object_cache_size() const { return read_only_object_cache_.size(); }
...
ReadOnlyHeapObjectIterator::ReadOnlyHeapObjectIterator(const ReadOnlySpace* ro_space)
    : ro_space_(ro_space)
    , current_page_(V8_ENABLE_THIRD_PARTY_HEAP_BOOL ? std::vector<ReadOnlyPage*>::iterator() : ro_space->pages().begin())
    , current_addr_(V8_ENABLE_THIRD_PARTY_HEAP_BOOL ? Address() : (*current_page_)->GetAreaStart())
{
}
...
HeapObject ReadOnlyHeapObjectIterator::Next()
{
    ...
    for (;;) {
        Address end = current_page->address() + current_page->area_size() + MemoryChunkLayout::ObjectStartOffsetInMemoryChunk(RO_SPACE);
        ...
        if (current_addr_ == ro_space_->top() && current_addr_ != ro_space_->limit()) {
