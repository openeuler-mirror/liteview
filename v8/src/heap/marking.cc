...
template <> bool ConcurrentBitmap<AccessMode::NON_ATOMIC>::AllBitsSetInRange(uint32_t start_index, uint32_t end_index)
{
...
template <> bool ConcurrentBitmap<AccessMode::NON_ATOMIC>::AllBitsClearInRange(uint32_t start_index, uint32_t end_index)
{
    if (start_index >= end_index)
        return true;
    end_index--;
...
public:
    ...
    {
        if (seq_length > 0) {
            PrintF("%zu: %dx%zu\n", seq_start, seq_type == 0 ? 0 : 1, seq_length * Bitmap::kBitsPerCell);
            seq_length = 0;
        }
    }
...
template <> bool ConcurrentBitmap<AccessMode::NON_ATOMIC>::IsClean()
{
    for (size_t i = 0; i < CellsCount(); i++) {
        if (cells()[i] != 0) {
            return false;
