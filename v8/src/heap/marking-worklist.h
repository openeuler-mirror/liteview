...
public:
    ...
    const std::vector<ContextWorklistPair>& context_worklists() const { return context_worklists_; }
...
public:
    ...
    inline bool ExtractWrapper(Map map, JSObject object, WrapperSnapshot& snapshot);
    ...
    CppMarkingState* cpp_marking_state() const { return cpp_marking_state_.get(); }
...
private:
    ...
    const std::unordered_map<Address, std::unique_ptr<MarkingWorklist::Local>> worklist_by_context_;
