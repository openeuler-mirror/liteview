...
enum class SkipRoot { kExternalStringTable, kGlobalHandles, kOldGeneration, kStack, kMainThreadHandles, kUnserializable, kWeak };
...
using EphemeronRememberedSet = std::unordered_map<EphemeronHashTable, std::unordered_set<int>, Object::Hasher>;
...
public:
    ...
    enum HeapState { NOT_IN_GC, SCAVENGE, MARK_COMPACT, MINOR_MARK_COMPACT, TEAR_DOWN };
    ...
    public:
        ...
        {
            const int64_t amount = total_.fetch_add(delta, std::memory_order_relaxed) + delta;
    ...
    private:
        void set_total(int64_t value) { total_.store(value, std::memory_order_relaxed); }

        void set_limit(int64_t value) { limit_.store(value, std::memory_order_relaxed); }

        void set_low_since_mark_compact(int64_t value) { low_since_mark_compact_.store(value, std::memory_order_relaxed); }
    ...
    static constexpr size_t kOldGenerationToSemiSpaceRatio = 128 * kHeapLimitMultiplier / kPointerMultiplier;
    ...
    static constexpr size_t kMaxSemiSpaceSize = kHugePageSize * 16 * kPointerMultiplier;
#else
    ...
    static uintptr_t ZapValue() { return v8_flags.clear_free_memory ? kClearedFreeMemoryValue : kZapValue; }

    static inline bool IsYoungGenerationCollector(GarbageCollector collector)
    {
        return collector == GarbageCollector::SCAVENGER || collector == GarbageCollector::MINOR_MARK_COMPACTOR;
    }

    static inline GarbageCollector YoungGenerationCollector() { return (v8_flags.minor_mc) ? GarbageCollector::MINOR_MARK_COMPACTOR : GarbageCollector::SCAVENGER; }
    ...
    template <typename TSlot> V8_EXPORT_PRIVATE void WriteBarrierForRange(HeapObject object, TSlot start, TSlot end);

    // Implements slow path of both generational & shared heap barrier.
    V8_EXPORT_PRIVATE static void CombinedGenerationalAndSharedBarrierSlow(HeapObject object, Address slot, HeapObject value);
    V8_EXPORT_PRIVATE static void CombinedGenerationalAndSharedEphemeronBarrierSlow(EphemeronHashTable table, Address slot, HeapObject value);

    V8_EXPORT_PRIVATE static void GenerationalBarrierSlow(HeapObject object, Address slot, HeapObject value);

    V8_EXPORT_PRIVATE static void SharedHeapBarrierSlow(HeapObject object, Address slot);
    V8_EXPORT_PRIVATE inline void RecordEphemeronKeyWrite(EphemeronHashTable table, Address key_slot);
    V8_EXPORT_PRIVATE static void EphemeronKeyWriteBarrierFromCode(Address raw_object, Address address, Isolate* isolate);
    V8_EXPORT_PRIVATE static void GenerationalBarrierForCodeSlow(Code host, RelocInfo* rinfo, HeapObject value);
    ...
    V8_EXPORT_PRIVATE void MoveRange(HeapObject dst_object, ObjectSlot dst_slot, ObjectSlot src_slot, int len, WriteBarrierMode mode);
    ...
    template <typename TSlot> void CopyRange(HeapObject dst_object, TSlot dst_slot, TSlot src_slot, int len, WriteBarrierMode mode);
    ...
    template <typename T> void CreateFillerForArray(T object, int elements_to_trim, int bytes_to_trim);
    ...
    V8_EXPORT_PRIVATE FixedArrayBase LeftTrimFixedArray(FixedArrayBase obj, int elements_to_trim);

    // Trim the given array from the right.
    V8_EXPORT_PRIVATE void RightTrimFixedArray(FixedArrayBase obj, int elements_to_trim);
    ...
    void set_native_contexts_list(Object object) { native_contexts_list_.store(object.ptr(), std::memory_order_release); }

    Object native_contexts_list() const { return Object(native_contexts_list_.load(std::memory_order_acquire)); }

    void set_allocation_sites_list(Object object) { allocation_sites_list_ = object; }
    Object allocation_sites_list() { return allocation_sites_list_; }

    void set_dirty_js_finalization_registries_list(Object object) { dirty_js_finalization_registries_list_ = object; }
    Object dirty_js_finalization_registries_list() { return dirty_js_finalization_registries_list_; }
    void set_dirty_js_finalization_registries_list_tail(Object object) { dirty_js_finalization_registries_list_tail_ = object; }
    Object dirty_js_finalization_registries_list_tail() { return dirty_js_finalization_registries_list_tail_; }

    // Used in CreateAllocationSiteStub and the (de)serializer.
    Address allocation_sites_list_address() { return reinterpret_cast<Address>(&allocation_sites_list_); }
    ...
    uintptr_t code_space_memory_modification_scope_depth() { return code_space_memory_modification_scope_depth_; }

    void increment_code_space_memory_modification_scope_depth() { code_space_memory_modification_scope_depth_++; }

    void decrement_code_space_memory_modification_scope_depth() { code_space_memory_modification_scope_depth_--; }

    void UnprotectAndRegisterMemoryChunk(MemoryChunk* chunk, UnprotectMemoryOrigin origin);
    V8_EXPORT_PRIVATE void UnprotectAndRegisterMemoryChunk(HeapObject object, UnprotectMemoryOrigin origin);
    ...
    inline HeapState gc_state() const { return gc_state_.load(std::memory_order_relaxed); }
    void SetGCState(HeapState state);
    bool IsTearingDown() const { return gc_state() == TEAR_DOWN; }
    bool force_oom() const { return force_oom_; }

    bool ignore_local_gc_requests() const { return ignore_local_gc_requests_depth_ > 0; }
    ...
    V8_EXPORT_PRIVATE void MemoryPressureNotification(MemoryPressureLevel level, bool is_isolate_locked);
    ...
    void AppendArrayBufferExtension(JSArrayBuffer object, ArrayBufferExtension* extension);
    void DetachArrayBufferExtension(JSArrayBuffer object, ArrayBufferExtension* extension);
    ...
    std::unique_ptr<v8::MeasureMemoryDelegate> MeasureMemoryDelegate(Handle<NativeContext> context, Handle<JSPromise> promise, v8::MeasureMemoryMode mode);
    ...
    uint64_t backing_store_bytes() const { return backing_store_bytes_.load(std::memory_order_relaxed); }

    void CompactWeakArrayLists();

    V8_EXPORT_PRIVATE void AddRetainedMap(Handle<NativeContext> context, Handle<Map> map);
    ...
    bool HighMemoryPressure() { return memory_pressure_level_.load(std::memory_order_relaxed) != MemoryPressureLevel::kNone; }
    ...
    void RestoreHeapLimit(size_t heap_limit)
    {
        ...
        set_max_old_generation_size(std::min(max_old_generation_size(), std::max(heap_limit, min_limit)));
    }
    ...
    PagedSpace* shared_allocation_space() const { return shared_allocation_space_; }
    OldLargeObjectSpace* shared_lo_allocation_space() const { return shared_lo_allocation_space_; }
    ...
    const MemoryAllocator* memory_allocator() const { return memory_allocator_.get(); }
    ...
    MarkCompactCollector* mark_compact_collector() { return mark_compact_collector_.get(); }

    MinorMarkCompactCollector* minor_mark_compact_collector() { return minor_mark_compact_collector_.get(); }

    Sweeper* sweeper() { return sweeper_.get(); }

    ArrayBufferSweeper* array_buffer_sweeper() { return array_buffer_sweeper_.get(); }
...
#undef ROOT_ACCESSOR
    ...
    void set_is_finalization_registry_cleanup_task_posted(bool posted) { is_finalization_registry_cleanup_task_posted_ = posted; }

    bool is_finalization_registry_cleanup_task_posted() { return is_finalization_registry_cleanup_task_posted_; }
    ...
    V8_EXPORT_PRIVATE void PreciseCollectAllGarbage(int flags, GarbageCollectionReason gc_reason, const GCCallbackFlags gc_callback_flags = kNoGCCallbackFlags);
    ...
    void SetGetExternallyAllocatedMemoryInBytesCallback(GetExternallyAllocatedMemoryInBytesCallback callback) { external_memory_callback_ = callback; }
    ...
    int GCFlagsForIncrementalMarking() { return ShouldOptimizeForMemoryUsage() ? kReduceMemoryFootprintMask : kNoGCFlags; }

    // Start incremental marking and ensure that idle time handler can perform
    // incremental steps.
    V8_EXPORT_PRIVATE void StartIdleIncrementalMarking(GarbageCollectionReason gc_reason, GCCallbackFlags gc_callback_flags = GCCallbackFlags::kNoGCCallbackFlags);

    // Starts incremental marking assuming incremental marking is currently
    // stopped.
    V8_EXPORT_PRIVATE void StartIncrementalMarking(int gc_flags, GarbageCollectionReason gc_reason, GCCallbackFlags gc_callback_flags = GCCallbackFlags::kNoGCCallbackFlags,
        GarbageCollector collector = GarbageCollector::MARK_COMPACTOR);

    V8_EXPORT_PRIVATE void StartIncrementalMarkingIfAllocationLimitIsReached(int gc_flags, GCCallbackFlags gc_callback_flags = GCCallbackFlags::kNoGCCallbackFlags);
    ...
    IncrementalMarking* incremental_marking() const { return incremental_marking_.get(); }

    // ===========================================================================
    // Concurrent marking API. ===================================================
    // ===========================================================================

    ConcurrentMarking* concurrent_marking() const { return concurrent_marking_.get(); }
    ...
    // manually.
    void NotifyObjectLayoutChange(HeapObject object, const DisallowGarbageCollection&, InvalidateRecordedSlots invalidate_recorded_slots, int new_size = 0);
    ...
    // in that area.
    void NotifyObjectSizeChange(HeapObject, int old_size, int new_size, ClearRecordedSlots clear_recorded_slots,
        UpdateInvalidatedObjectSize update_invalidated_object_size = UpdateInvalidatedObjectSize::kYes);
    ...
    LocalEmbedderHeapTracer* local_embedder_heap_tracer() const { return local_embedder_heap_tracer_.get(); }
    ...
    V8_EXPORT_PRIVATE void UpdateExternalString(String string, size_t old_payload, size_t new_payload);

    // Finalizes an external string by deleting the associated external
    // data and clearing the resource pointer.
    inline void FinalizeExternalString(String string);

    static String UpdateYoungReferenceInExternalStringTableEntry(Heap* heap, FullObjectSlot pointer);
    ...
    inline void IncrementPromotedObjectsSize(size_t object_size) { promoted_objects_size_ += object_size; }
    inline size_t promoted_objects_size() { return promoted_objects_size_; }

    inline void IncrementNewSpaceSurvivingObjectSize(size_t object_size) { new_space_surviving_object_size_ += object_size; }
    inline size_t new_space_surviving_object_size() { return new_space_surviving_object_size_; }

    inline size_t SurvivedYoungObjectSize() { return promoted_objects_size_ + new_space_surviving_object_size_; }

    inline void IncrementNodesDiedInNewSpace(int count) { nodes_died_in_new_space_ += count; }

    inline void IncrementNodesCopiedInNewSpace() { nodes_copied_in_new_space_++; }

    inline void IncrementNodesPromoted() { nodes_promoted_++; }

    inline void IncrementYoungSurvivorsCounter(size_t survived) { survived_since_last_expansion_ += survived; }
    ...
    void set_new_space_allocation_counter(size_t new_value) { new_space_allocation_counter_ = new_value; }
    ...
    size_t OldGenerationAllocationCounter() { return old_generation_allocation_counter_at_last_gc_ + PromotedSinceLastGC(); }

    size_t EmbedderAllocationCounter() const;

    // This should be used only for testing.
    void set_old_generation_allocation_counter_at_last_gc(size_t new_value) { old_generation_allocation_counter_at_last_gc_ = new_value; }
    ...
    GarbageCollector current_or_last_garbage_collector() const { return current_or_last_garbage_collector_; }
    ...
    bool ShouldCurrentGCKeepAgesUnchanged() const { return is_current_gc_forced_ || is_current_gc_for_heap_profiler_; }
    ...
    V8_EXPORT_PRIVATE HeapObject PrecedeWithFiller(HeapObject object, int filler_size);
    ...
    bool has_heap_object_allocation_tracker() const { return !allocation_trackers_.empty(); }
    ...
    CodeLookupResult GcSafeFindCodeForInnerPointer(Address inner_pointer, bool die_on_unsuccessful_lookup = true);
    ...
    CodeLookupResult GcSafeFindCodeForInnerPointerForPrinting(Address inner_pointer);
    ...
    static const char* GarbageCollectionReasonToString(GarbageCollectionReason gc_reason);
    ...
    inline bool ShouldReduceMemory() const { return (current_gc_flags_ & kReduceMemoryFootprintMask) != 0; }

    MarkingState* marking_state() { return &marking_state_; }

    NonAtomicMarkingState* non_atomic_marking_state() { return &non_atomic_marking_state_; }
...
private:
    ...
    using ExternalStringTableUpdaterCallback = String (*)(Heap* heap, FullObjectSlot pointer);
    ...
    static bool IsRegularObjectAllocation(AllocationType allocation) { return AllocationType::kYoung == allocation || AllocationType::kOld == allocation; }

    static size_t DefaultGetExternallyAllocatedMemoryInBytesCallback() { return 0; }
...
#undef ROOT_ACCESSOR
    ...
    GarbageCollector SelectGarbageCollector(AllocationSpace space, GarbageCollectionReason gc_reason, const char** reason);
    ...
    size_t PerformGarbageCollection(
        GarbageCollector collector, GarbageCollectionReason gc_reason, const char* collector_reason, const GCCallbackFlags gc_callback_flags = kNoGCCallbackFlags);

    // Performs garbage collection in the shared heap.
    void PerformSharedGarbageCollection(Isolate* initiator, GarbageCollectionReason gc_reason);
    ...
    V8_EXPORT_PRIVATE void ZapCodeObject(Address start_address, int size_in_bytes);
    ...
    template <int kModeMask, typename TSlot> V8_INLINE void WriteBarrierForRangeImpl(MemoryChunk* source_page, HeapObject object, TSlot start_slot, TSlot end_slot);
    ...
    bool PerformIdleTimeAction(GCIdleTimeAction action, GCIdleTimeHeapState heap_state, double deadline_in_ms);

    void IdleNotificationEpilogue(GCIdleTimeAction action, GCIdleTimeHeapState heap_state, double start_ms, double deadline_in_ms);
    ...
    {
        uint64_t bytes = OldGenerationSizeOfObjects() + AllocatedExternalMemorySinceMarkCompact();
    ...
    size_t old_generation_allocation_limit() const { return old_generation_allocation_limit_.load(std::memory_order_relaxed); }

    void set_old_generation_allocation_limit(size_t newlimit) { old_generation_allocation_limit_.store(newlimit, std::memory_order_relaxed); }

    size_t global_allocation_limit() const { return global_allocation_limit_; }

    size_t max_old_generation_size() { return max_old_generation_size_.load(std::memory_order_relaxed); }

    void set_max_old_generation_size(size_t value) { max_old_generation_size_.store(value, std::memory_order_relaxed); }

    bool always_allocate() { return always_allocate_scope_count_ != 0; }

    V8_EXPORT_PRIVATE bool CanExpandOldGenerationBackground(LocalHeap* local_heap, size_t size);

    bool ShouldExpandOldGenerationOnSlowAllocation(LocalHeap* local_heap = nullptr);
    ...
    bool UseGlobalMemoryScheduling() const { return v8_flags.global_gc_scheduling && local_embedder_heap_tracer(); }
    ...
    V8_WARN_UNUSED_RESULT AllocationResult AllocateMap(
        InstanceType instance_type, int instance_size, ElementsKind elements_kind = TERMINAL_FAST_ELEMENTS_KIND, int inobject_properties = 0);
    ...
    V8_WARN_UNUSED_RESULT V8_INLINE HeapObject AllocateRawWith(
    ...
    V8_WARN_UNUSED_RESULT AllocationResult Allocate(Handle<Map> map, AllocationType allocation);
    ...
    void set_force_gc_on_next_allocation() { force_gc_on_next_allocation_ = true; }
    ...
    AllocationType allocation_type_for_in_place_internalizable_strings() const { return allocation_type_for_in_place_internalizable_strings_; }
    ...
    std::unordered_map<HeapObject, HeapObject, Object::Hasher> ephemeron_retainer_;
    ...
    template <typename ConcreteVisitor, typename MarkingState> friend class MarkingVisitorBase;
...
class V8_EXPORT_PRIVATE V8_NODISCARD CodePageCollectionMemoryModificationScopeForTesting : public CodePageCollectionMemoryModificationScope {
public:
    V8_NOINLINE explicit CodePageCollectionMemoryModificationScopeForTesting(Heap* heap);
...
class VerifyPointersVisitor : public ObjectVisitorWithCageBases, public RootVisitor {
public:
...
protected:
    ...
    virtual void VerifyPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end);
...
public:
    ...
    explicit StrongRootBlockAllocator(Heap* heap)
        : heap_(heap)
    {
    }
...
template <> struct StrongRootBlockAllocator::rebind<Address> {
...
template <class U> struct StrongRootBlockAllocator::rebind {
...
public:
    ...
    static EmbedderStackStateScope ExplicitScopeForTesting(LocalEmbedderHeapTracer* local_tracer, StackState stack_state);

    EmbedderStackStateScope(Heap* heap, Origin origin, StackState stack_state);
    ~EmbedderStackStateScope();

private:
    EmbedderStackStateScope(LocalEmbedderHeapTracer* local_tracer, StackState stack_state);
...
public:
    explicit EvacuationScope(Heap* heap)
        : heap_(heap)
    {
