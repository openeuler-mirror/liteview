...
MemoryAllocator::MemoryAllocator(Isolate* isolate, v8::PageAllocator* code_page_allocator, size_t capacity)
    : isolate_(isolate)
...
public:
    ...
    void Run(JobDelegate* delegate) override
    {
        if (delegate->IsJoiningThread()) {
            ...
            TRACE_GC1(tracer_, GCTracer::Scope::BACKGROUND_UNMAPPER, ThreadKind::kBackground);
            RunImpl(delegate);
        }
    }
    ...
    size_t GetMaxConcurrency(size_t worker_count) const override
    {
        ...
        return std::min<size_t>(kMaxUnmapperTasks, worker_count + (unmapper_->NumberOfCommittedChunks() + kTaskPerChunk - 1) / kTaskPerChunk);
    }

private:
    void RunImpl(JobDelegate* delegate)
    {
        unmapper_->PerformFreeMemoryOnQueuedChunks(FreeMode::kUncommitPooled, delegate);
...
void MemoryAllocator::Unmapper::FreeQueuedChunks()
{
    ...
    if (!heap_->IsTearingDown() && v8_flags.concurrent_sweeping) {
        if (job_handle_ && job_handle_->IsValid()) {
            ...
            job_handle_ = V8::GetCurrentPlatform()->PostJob(TaskPriority::kUserVisible, std::make_unique<UnmapFreeMemoryJob>(heap_->isolate(), this));
            if (v8_flags.trace_unmapper) {
...
void MemoryAllocator::Unmapper::CancelAndWaitForPendingTasks()
{
    ...
    if (v8_flags.trace_unmapper) {
        PrintIsolate(heap_->isolate(), "Unmapper::CancelAndWaitForPendingTasks: no tasks remaining\n");
    }
}
...
void MemoryAllocator::Unmapper::PerformFreeMemoryOnQueuedNonRegularChunks(JobDelegate* delegate)
{
...
void MemoryAllocator::Unmapper::PerformFreeMemoryOnQueuedChunks(MemoryAllocator::Unmapper::FreeMode mode, JobDelegate* delegate)
{
    MemoryChunk* chunk = nullptr;
    if (v8_flags.trace_unmapper) {
        PrintIsolate(heap_->isolate(), "Unmapper::PerformFreeMemoryOnQueuedChunks: %d queued chunks\n", NumberOfChunks());
    }
    // Regular chunks.
    while ((chunk = GetMemoryChunkSafe(ChunkQueueType::kRegular)) != nullptr) {
        ...
        allocator_->PerformFreeMemory(chunk);
        if (pooled)
            AddMemoryChunkSafe(ChunkQueueType::kPooled, chunk);
        if (delegate && delegate->ShouldYield())
            return;
    }
...
size_t MemoryAllocator::Unmapper::NumberOfCommittedChunks()
{
    ...
    return chunks_[ChunkQueueType::kRegular].size() + chunks_[ChunkQueueType::kNonRegular].size();
}
...
bool MemoryAllocator::Unmapper::IsRunning() const { return job_handle_ && job_handle_->IsValid(); }
...
void MemoryAllocator::FreeMemoryRegion(v8::PageAllocator* page_allocator, Address base, size_t size) { FreePages(page_allocator, reinterpret_cast<void*>(base), size); }

Address MemoryAllocator::AllocateAlignedMemory(size_t chunk_size, size_t area_size, size_t alignment, Executability executable, void* hint, VirtualMemory* controller)
{
    ...
    if (executable == EXECUTABLE) {
        ...
        if (!SetPermissionsOnExecutableMemoryChunk(&reservation, base, aligned_area_size, chunk_size)) {
            return HandleAllocationFailure(executable);
        }
    } else {
        ...
        const size_t commit_size = ::RoundUp(MemoryChunkLayout::ObjectStartOffsetInDataPage() + area_size, GetCommitPageSize());

        if (reservation.SetPermissions(base, commit_size, PageAllocator::kReadWrite)) {
...
Address MemoryAllocator::HandleAllocationFailure(Executability executable)
{
    ...
    if (!heap->deserialization_complete()) {
        heap->FatalProcessOutOfMemory(
            executable == EXECUTABLE ? "Executable MemoryChunk allocation failed during deserialization." : "MemoryChunk allocation failed during deserialization.");
    }
    return kNullAddress;
}

size_t MemoryAllocator::ComputeChunkSize(size_t area_size, Executability executable)
{
    if (executable == EXECUTABLE) {
        //
        //             Executable
        ...
        return ::RoundUp(MemoryChunkLayout::ObjectStartOffsetInCodePage() + area_size + MemoryChunkLayout::CodePageGuardSize(), GetCommitPageSize());
    }
    ...
    return ::RoundUp(MemoryChunkLayout::ObjectStartOffsetInDataPage() + area_size, GetCommitPageSize());
}

base::Optional<MemoryAllocator::MemoryChunkAllocationResult> MemoryAllocator::AllocateUninitializedChunk(
    BaseSpace* space, size_t area_size, Executability executable, PageSize page_size)
{
...
#else
    void* address_hint = AlignedAddress(isolate_->heap()->GetRandomMmapAddr(), MemoryChunk::kAlignment);
#endif
    ...
    Address base = AllocateAlignedMemory(chunk_size, area_size, MemoryChunk::kAlignment, executable, address_hint, &reservation);
    ...
    if (Heap::ShouldZapGarbage()) {
        if (executable == EXECUTABLE) {
            ...
            // Now zap object area.
            ZapBlock(base + MemoryChunkLayout::ObjectStartOffsetInCodePage(), area_size, kZapValue);
        } else {
            DCHECK_EQ(executable, NOT_EXECUTABLE);
            // Zap both page header and object area at once. No guard page in-between.
            ZapBlock(base, MemoryChunkLayout::ObjectStartOffsetInDataPage() + area_size, kZapValue);
        }
    }
...
void MemoryAllocator::PartialFreeMemory(BasicMemoryChunk* chunk, Address start_free, size_t bytes_to_free, Address new_area_end)
{
    ...
    if (chunk->IsFlagSet(MemoryChunk::IS_EXECUTABLE)) {
        ...
        DCHECK_EQ(chunk->address() + chunk->size(), chunk->area_end() + MemoryChunkLayout::CodePageGuardSize());
        ...
        if (V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT && !isolate_->jitless()) {
            ...
            reservation->SetPermissions(chunk->area_end(), page_size, PageAllocator::kNoAccess);
        }
    }
...
void MemoryAllocator::UnregisterSharedBasicMemoryChunk(BasicMemoryChunk* chunk)
{
    VirtualMemory* reservation = chunk->reserved_memory();
...
void MemoryAllocator::UnregisterBasicMemoryChunk(BasicMemoryChunk* chunk, Executability executable)
{
...
#endif // DEBUG
        chunk->heap()->UnregisterUnprotectedMemoryChunk(static_cast<MemoryChunk*>(chunk));
    }
    chunk->SetFlag(MemoryChunk::UNREGISTERED);
}

void MemoryAllocator::UnregisterMemoryChunk(MemoryChunk* chunk) { UnregisterBasicMemoryChunk(chunk, chunk->executable()); }
...
void MemoryAllocator::FreeReadOnlyPage(ReadOnlyPage* chunk)
{
    ...
    if (reservation->IsReserved()) {
        ...
        FreeMemoryRegion(allocator, chunk->address(), RoundUp(chunk->size(), allocator->AllocatePageSize()));
    }
}
...
void MemoryAllocator::PreFreeMemory(MemoryChunk* chunk)
{
    ...
    isolate_->heap()->RememberUnmappedPage(reinterpret_cast<Address>(chunk), chunk->IsEvacuationCandidate());
...
void MemoryAllocator::PerformFreeMemory(MemoryChunk* chunk)
{
    ...
    if (chunk->executable() == EXECUTABLE) {
        rwx_write_scope.emplace("We are going to modify the chunk's header, so ensure we have write "
...
void MemoryAllocator::FreePooledChunk(MemoryChunk* chunk)
{
    ...
    FreeMemoryRegion(data_page_allocator(), chunk->address(), static_cast<size_t>(MemoryChunk::kPageSize));
}

Page* MemoryAllocator::AllocatePage(MemoryAllocator::AllocationMode alloc_mode, Space* space, Executability executable)
{
    size_t size = MemoryChunkLayout::AllocatableMemoryInMemoryChunk(space->identity());
    base::Optional<MemoryChunkAllocationResult> chunk_info;
    if (alloc_mode == AllocationMode::kUsePool) {
        DCHECK_EQ(size, static_cast<size_t>(MemoryChunkLayout::AllocatableMemoryInMemoryChunk(space->identity())));
        DCHECK_EQ(executable, NOT_EXECUTABLE);
    ...
    Page* page
        = new (chunk_info->start) Page(isolate_->heap(), space, chunk_info->size, chunk_info->area_start, chunk_info->area_end, std::move(chunk_info->reservation), executable);

#ifdef DEBUG
    if (page->executable())
        RegisterExecutableMemoryChunk(page);
#endif // DEBUG
...
ReadOnlyPage* MemoryAllocator::AllocateReadOnlyPage(ReadOnlySpace* space)
{
    ...
    base::Optional<MemoryChunkAllocationResult> chunk_info = AllocateUninitializedChunk(space, size, NOT_EXECUTABLE, PageSize::kRegular);
    if (!chunk_info)
        return nullptr;
    return new (chunk_info->start) ReadOnlyPage(isolate_->heap(), space, chunk_info->size, chunk_info->area_start, chunk_info->area_end, std::move(chunk_info->reservation));
}

std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping> MemoryAllocator::RemapSharedPage(::v8::PageAllocator::SharedMemory* shared_memory, Address new_address)
{
    return shared_memory->RemapTo(reinterpret_cast<void*>(new_address));
}

LargePage* MemoryAllocator::AllocateLargePage(LargeObjectSpace* space, size_t object_size, Executability executable)
{
    base::Optional<MemoryChunkAllocationResult> chunk_info = AllocateUninitializedChunk(space, object_size, executable, PageSize::kLarge);

    if (!chunk_info)
        return nullptr;

    LargePage* page = new (chunk_info->start)
        LargePage(isolate_->heap(), space, chunk_info->size, chunk_info->area_start, chunk_info->area_end, std::move(chunk_info->reservation), executable);
...
base::Optional<MemoryAllocator::MemoryChunkAllocationResult> MemoryAllocator::AllocateUninitializedPageFromPool(Space* space)
{
...
void MemoryAllocator::ZapBlock(Address start, size_t size, uintptr_t zap_value)
{
    DCHECK(IsAligned(start, kTaggedSize));
    DCHECK(IsAligned(size, kTaggedSize));
    MemsetTagged(ObjectSlot(start), Object(static_cast<Address>(zap_value)), size >> kTaggedSizeLog2);
}
...
base::AddressRegion MemoryAllocator::ComputeDiscardMemoryArea(Address addr, size_t size)
{
    size_t page_size = GetCommitPageSize();
    ...
    if (discardable_start >= discardable_end)
        return base::AddressRegion(0, 0);
    return base::AddressRegion(discardable_start, discardable_end - discardable_start);
}

bool MemoryAllocator::SetPermissionsOnExecutableMemoryChunk(VirtualMemory* vm, Address start, size_t area_size, size_t chunk_size)
{
    ...
    if (V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT && !jitless) {
        ...
        if (vm->RecommitPages(start, pre_guard_offset, PageAllocator::kReadWriteExecute)) {
            ...
            if (vm->DiscardSystemPages(pre_guard_page, page_size)) {
                ...
                if (vm->RecommitPages(code_area, area_size, PageAllocator::kReadWriteExecute)) {
        ...
        if (vm->SetPermissions(start, pre_guard_offset, PageAllocator::kReadWrite)) {
            // Create the pre-code guard page, following the header.
            if (vm->SetPermissions(pre_guard_page, page_size, PageAllocator::kNoAccess)) {
                // Commit the executable code body.
                ...
                if (!jitless && RwxMemoryWriteScope::IsSupported()) {
                    ...
                    set_permission_successed = base::MemoryProtectionKey::SetPermissionsAndKey(
                        code_page_allocator_, region, PageAllocator::kReadWriteExecute, RwxMemoryWriteScope::memory_protection_key());
                } else
#endif
                {
                    set_permission_successed = vm->SetPermissions(code_area, area_size, jitless ? PageAllocator::kReadWrite : MemoryChunk::GetCodeModificationPermission());
                }
                if (set_permission_successed) {
                    // Create the post-code guard page.
                    if (vm->SetPermissions(post_guard_page, page_size, PageAllocator::kNoAccess)) {
                        UpdateAllocatedSpaceLimits(start, code_area + area_size);
                        return true;
                    }
...
const MemoryChunk* MemoryAllocator::LookupChunkContainingAddress(Address addr) const
{
    ...
    if (auto it = normal_pages_.find(static_cast<Page*>(chunk)); it != normal_pages_.end()) {
        // The chunk is a normal page.
        DCHECK_LE(chunk->address(), addr);
        if (chunk->Contains(addr))
            return *it;
    } else if (auto it = large_pages_.upper_bound(static_cast<LargePage*>(chunk)); it != large_pages_.begin()) {
