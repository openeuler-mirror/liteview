...
LargePage::LargePage(Heap* heap, BaseSpace* space, size_t chunk_size, Address area_start, Address area_end, VirtualMemory reservation, Executability executable)
    : MemoryChunk(heap, space, chunk_size, area_start, area_end, std::move(reservation), executable, PageSize::kLarge)
{
...
LargePage* LargePage::Initialize(Heap* heap, MemoryChunk* chunk, Executability executable)
{
...
{
    RememberedSet<OLD_TO_NEW>::RemoveRange(this, free_start, area_end(), SlotSet::FREE_EMPTY_BUCKETS);
...
LargeObjectSpaceObjectIterator::LargeObjectSpaceObjectIterator(LargeObjectSpace* space) { current_ = space->first_page(); }
...
void LargeObjectSpace::TearDown()
{
    while (!memory_chunk_list_.Empty()) {
        ...
        LOG(heap()->isolate(), DeleteEvent("LargeObjectChunk", reinterpret_cast<void*>(page->address())));
...
AllocationResult OldLargeObjectSpace::AllocateRaw(int object_size) { return AllocateRaw(object_size, NOT_EXECUTABLE); }

AllocationResult OldLargeObjectSpace::AllocateRaw(int object_size, Executability executable)
{
    object_size = ALIGN_TO_ALLOCATION_ALIGNMENT(object_size);
    DCHECK(!v8_flags.enable_third_party_heap);
    // Check if we want to force a GC before growing the old space further.
    // If so, fail the allocation.
    if (!heap()->CanExpandOldGeneration(object_size) || !heap()->ShouldExpandOldGenerationOnSlowAllocation(heap()->main_thread_local_heap())) {
    ...
    DCHECK_IMPLIES(heap()->incremental_marking()->black_allocation(), heap()->marking_state()->IsBlack(object));
...
AllocationResult OldLargeObjectSpace::AllocateRawBackground(LocalHeap* local_heap, int object_size) { return AllocateRawBackground(local_heap, object_size, NOT_EXECUTABLE); }

AllocationResult OldLargeObjectSpace::AllocateRawBackground(LocalHeap* local_heap, int object_size, Executability executable)
{
    object_size = ALIGN_TO_ALLOCATION_ALIGNMENT(object_size);
    DCHECK(!v8_flags.enable_third_party_heap);
    // Check if we want to force a GC before growing the old space further.
    // If so, fail the allocation.
    if (!heap()->CanExpandOldGenerationBackground(local_heap, object_size) || !heap()->ShouldExpandOldGenerationOnSlowAllocation(local_heap)) {
        return AllocationResult::Failure();
    }
...
LargePage* LargeObjectSpace::AllocateLargePage(int object_size, Executability executable)
{
    LargePage* page = heap()->memory_allocator()->AllocateLargePage(this, object_size, executable);
    if (page == nullptr)
        return nullptr;
...
{
    for (Address current = reinterpret_cast<Address>(page); current < reinterpret_cast<Address>(page) + page->size(); current += MemoryChunk::kPageSize) {
...
{
    for (Address current = page->address(); current < reinterpret_cast<Address>(page) + page->size(); current += MemoryChunk::kPageSize) {
...
void LargeObjectSpace::AddPage(LargePage* page, size_t object_size)
{
    ...
    page->set_owner(this);
    page->SetOldGenerationPageFlags(!is_off_thread() && heap()->incremental_marking()->IsMarking());
...
// supported.
Address GetEndOfPayload(LargePage* page, Address object_address, size_t object_size)
{
    if (page->executable() == EXECUTABLE) {
        return kNullAddress;
    }
    const size_t used_committed_size = ::RoundUp((object_address - page->address()) + object_size, MemoryAllocator::GetCommitPageSize());
    return (used_committed_size < page->size()) ? page->address() + used_committed_size : kNullAddress;
}

} // namespace

void LargeObjectSpace::ShrinkPageToObjectSize(LargePage* page, HeapObject object, size_t object_size)
{
#ifdef DEBUG
    PtrComprCageBase cage_base(heap()->isolate());
    ...
    if (free_start != kNullAddress) {
        ...
        heap()->memory_allocator()->PartialFreeMemory(page, free_start, bytes_to_free, page->area_start() + object_size);
        size_ -= bytes_to_free;
...
std::unique_ptr<ObjectIterator> LargeObjectSpace::GetObjectIterator(Heap* heap) { return std::unique_ptr<ObjectIterator>(new LargeObjectSpaceObjectIterator(this)); }
...
void LargeObjectSpace::Verify(Isolate* isolate)
{
    ...
    for (LargePage* chunk = first_page(); chunk != nullptr; chunk = chunk->next_page()) {
...
size_t NewLargeObjectSpace::Available() const { return capacity_ - SizeOfObjects(); }
...
void NewLargeObjectSpace::FreeDeadObjects(const std::function<bool(HeapObject)>& is_dead)
{
...
void NewLargeObjectSpace::SetCapacity(size_t capacity) { capacity_ = std::max(capacity, SizeOfObjects()); }
...
AllocationResult CodeLargeObjectSpace::AllocateRawBackground(LocalHeap* local_heap, int object_size)
{
    DCHECK(!v8_flags.enable_third_party_heap);
    return OldLargeObjectSpace::AllocateRawBackground(local_heap, object_size, EXECUTABLE);
}
...
AllocationResult SharedLargeObjectSpace::AllocateRawBackground(LocalHeap* local_heap, int object_size)
{
    DCHECK(!v8_flags.enable_third_party_heap);
    return OldLargeObjectSpace::AllocateRawBackground(local_heap, object_size, NOT_EXECUTABLE);
}
