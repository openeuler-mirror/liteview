...
extern "C" void PushAllRegistersAndIterateStack(void*, void*, void*);

namespace heap {
namespace base {

using IterateStackCallback = void (*)(const Stack*, StackVisitor*, intptr_t*);
//extern "C" void PushAllRegistersAndIterateStack(const Stack*, StackVisitor*, IterateStackCallback);
//
// extern "C" void PushAllRegistersAndIterateStack(const Stack*, StackVisitor*, IterateStackCallback)
// {
//   *(int*)1 = 1;
...
Stack::Stack(const void* stack_start)
    : stack_start_(stack_start)
{
}

void Stack::SetStackStart(const void* stack_start) { stack_start_ = stack_start; }
...
bool Stack::IsOnStack(void* slot) const
{
    ...
    if (__asan_addr_is_in_fake_stack(__asan_get_current_fake_stack(), reinterpret_cast<void*>(slot), nullptr, nullptr)) {
        return true;
    }
    ...
    if (__builtin___get_unsafe_stack_top() >= slot && slot >= __builtin___get_unsafe_stack_ptr()) {
        return true;
    }
#endif // __has_feature(safe_stack)
#endif // defined(__has_feature)
    return v8::base::Stack::GetCurrentStackPosition() <= slot && slot <= stack_start_;
}
...
DISABLE_TSAN
void IterateAsanFakeFrameIfNecessary(StackVisitor* visitor, void* asan_fake_stack, const void* stack_start, const void* stack_end, void* address)
{
    ...
    if (asan_fake_stack) {
        ...
        void* fake_frame_end;
        void* real_stack_frame = __asan_addr_is_in_fake_stack(asan_fake_stack, address, &fake_frame_begin, &fake_frame_end);
        if (real_stack_frame) {
            ...
            if (stack_start >= real_stack_frame && real_stack_frame >= stack_end) {
                ...
                for (void** current = reinterpret_cast<void**>(fake_frame_begin); current < fake_frame_end; ++current) {
                    void* addr = *current;
                    if (addr == nullptr)
                        continue;
...
void IterateUnsafeStackIfNecessary(StackVisitor* visitor)
{
    ...
    CHECK_EQ(0u, reinterpret_cast<uintptr_t>(stack_end) & (kSafeStackAlignmentBytes - 1));
    CHECK_EQ(0u, reinterpret_cast<uintptr_t>(stack_start) & (kSafeStackAlignmentBytes - 1));
...
// other thread may use a lock to synchronize the access.
DISABLE_TSAN
void IteratePointersImpl(const Stack* stack, StackVisitor* visitor, intptr_t* stack_end)
{
#ifdef V8_USE_ADDRESS_SANITIZER
    ...
    for (; current < stack->stack_start(); ++current) {
        ...
        visitor->VisitPointer(address);
#ifdef V8_USE_ADDRESS_SANITIZER
        IterateAsanFakeFrameIfNecessary(visitor, asan_fake_stack, stack->stack_start(), stack_end, address);
#endif // V8_USE_ADDRESS_SANITIZER
    }
}
...
{
    DCHECK_NOT_NULL(stack_start_);
    PushAllRegistersAndIterateStack((void*)this, (void*)visitor, (void*)(&IteratePointersImpl));
...
void Stack::IteratePointersUnsafe(StackVisitor* visitor, uintptr_t stack_end) const { IteratePointersImpl(this, visitor, reinterpret_cast<intptr_t*>(stack_end)); }
