...
public:
    ...
    constexpr static size_t BucketsForSize(size_t size) { return (size + (SlotGranularity * kBitsPerBucket) - 1) / (SlotGranularity * kBitsPerBucket); }
    ...
    {
        return Iterate(chunk_start, start_bucket, end_bucket, callback, [this, mode](size_t bucket_index) {
    ...
    public:
        ...
        void ClearCellBits(int cell_index, uint32_t mask) { v8::base::AsAtomic32::SetBits(cell(cell_index), 0u, mask); }

        void StoreCell(int cell_index, uint32_t value) { v8::base::AsAtomic32::Release_Store(cell(cell_index), value); }
...
protected:
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> Bucket* LoadBucket(size_t bucket_index) { return LoadBucket(bucket(bucket_index)); }
    ...
    template <AccessMode access_mode = AccessMode::ATOMIC> void StoreBucket(size_t bucket_index, Bucket* value) { StoreBucket(bucket(bucket_index), value); }
