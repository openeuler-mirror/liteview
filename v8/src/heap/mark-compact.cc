...
protected:
    ...
    virtual ConcurrentBitmap<AccessMode::NON_ATOMIC>* bitmap(const MemoryChunk* chunk) = 0;
    ...
    void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) override { VerifyPointers(start, end); }

    void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) override { VerifyPointers(start, end); }
    ...
    void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override { VerifyRootPointers(start, end); }

    void VisitMapPointer(HeapObject object) override { VerifyMap(object.map(cage_base())); }
...
void MarkingVerifier::VerifyRoots() { heap_->IterateRootsIncludingClients(this, base::EnumSet<SkipRoot> { SkipRoot::kWeak }); }

void MarkingVerifier::VerifyMarkingOnPage(const Page* page, Address start, Address end)
{
    Address next_object_must_be_here_or_later = start;

    for (auto object_and_size : LiveObjectRange<kAllLiveObjects>(page, bitmap(page))) {
        ...
        CHECK(bitmap(page)->AllBitsSetInRange(page->AddressToMarkbitIndex(current), page->AddressToMarkbitIndex(next_object_must_be_here_or_later))
            || bitmap(page)->AllBitsClearInRange(page->AddressToMarkbitIndex(current + kTaggedSize * 2), page->AddressToMarkbitIndex(next_object_must_be_here_or_later)));
...
void MarkingVerifier::VerifyMarking(NewSpace* space)
{
    ...
    CHECK_EQ(space->first_allocatable_address(), space->first_page()->area_start());
...
protected:
    ConcurrentBitmap<AccessMode::NON_ATOMIC>* bitmap(const MemoryChunk* chunk) override { return marking_state_->bitmap(chunk); }

    bool IsBlack(HeapObject object) override { return marking_state_->IsBlack(object); }

    void VerifyMap(Map map) override { VerifyHeapObjectImpl(map); }

    void VerifyPointers(ObjectSlot start, ObjectSlot end) override { VerifyPointersImpl(start, end); }

    void VerifyPointers(MaybeObjectSlot start, MaybeObjectSlot end) override { VerifyPointersImpl(start, end); }
    ...
    void VerifyRootPointers(FullObjectSlot start, FullObjectSlot end) override { VerifyPointersImpl(start, end); }
...
private:
    ...
    V8_INLINE bool ShouldVerifyObject(HeapObject heap_object)
    {
        ...
        return heap_->isolate()->is_shared_heap_isolate() ? in_shared_heap : !in_shared_heap;
    }
...
int NumberOfParallelCompactionTasks(Heap* heap)
{
    ...
    if (!heap->CanPromoteYoungAndExpandOldGeneration(static_cast<size_t>(tasks * Page::kPageSize))) {
...
bool CollectorBase::IsMajorMC() { return !heap_->IsYoungGenerationCollector(garbage_collector_); }
...
void CollectorBase::StartSweepSpace(PagedSpace* space)
{
        ...
        if (non_atomic_marking_state()->live_bytes(p) == 0) {
            if (unused_page_present) {
                if (v8_flags.gc_verbose) {
                    PrintIsolate(isolate(), "sweeping: released page: %p", static_cast<void*>(p));
                }
                space->ReleasePage(p);
                continue;
            }
    ...
    if (v8_flags.gc_verbose) {
        PrintIsolate(isolate(), "sweeping: space=%s initialized_for_sweeping=%d", space->name(), will_be_swept);
    }
}
...
void CollectorBase::StartSweepNewSpace()
{
    ...
    if (v8_flags.gc_verbose) {
        PrintIsolate(isolate(), "sweeping: space=%s initialized_for_sweeping=%d", paged_space->name(), will_be_swept);
    }
}
...
void CollectorBase::SweepLargeSpace(LargeObjectSpace* space)
{
    ...
    for (auto it = space->begin(); it != space->end();) {
        ...
        if (!marking_state->IsBlack(object)) {
            ...
            heap()->memory_allocator()->Free(MemoryAllocator::FreeMode::kConcurrently, current);
...
void MarkCompactCollector::AddEvacuationCandidate(Page* p)
{
    ...
    if (v8_flags.trace_evacuation_candidates) {
        PrintIsolate(isolate(), "Evacuation candidate: Free bytes: %6zu. Free Lists length: %4d.\n", p->area_size() - p->allocated_bytes(), p->FreeListsLength());
    }
...
static void TraceFragmentation(PagedSpace* space)
{
    ...
    PrintF("[%s]: %d pages, %d (%.1f%%) free\n", space->name(), number_of_pages, static_cast<int>(free), static_cast<double>(free) * 100 / reserved);
}
...
bool MarkCompactCollector::StartCompaction(StartCompactionMode mode)
{
    ...
    if (!v8_flags.compact || (mode == StartCompactionMode::kAtomic && heap()->IsGCWithStack() && !v8_flags.compact_with_stack)
        || (v8_flags.gc_experiment_less_compaction && !heap_->ShouldReduceMemory())) {
        return false;
    }
    ...
    if (v8_flags.compact_code_space && (!heap()->IsGCWithStack() || v8_flags.compact_code_space_with_stack)) {
...
{
    std::vector<Address> contexts = heap()->memory_measurement()->StartProcessing();
    ...
    local_marking_worklists_ = std::make_unique<MarkingWorklists::Local>(
        marking_worklists(), cpp_heap ? cpp_heap->CreateCppMarkingStateForMutatorThread() : MarkingWorklists::Local::kNoCppMarkingState);
    local_weak_objects_ = std::make_unique<WeakObjects::Local>(weak_objects());
    marking_visitor_ = std::make_unique<MarkingVisitor>(marking_state(), local_marking_worklists(), local_weak_objects_.get(), heap_, epoch(), code_flush_mode(),
        heap_->local_embedder_heap_tracer()->InUse(), heap_->ShouldCurrentGCKeepAgesUnchanged());
...
void MarkCompactCollector::VerifyMarkbitsAreDirty(ReadOnlySpace* space)
{
    ...
    for (HeapObject object = iterator.Next(); !object.is_null(); object = iterator.Next()) {
...
{
    DCHECK(space->identity() == OLD_SPACE || space->identity() == CODE_SPACE || space->identity() == MAP_SPACE || space->identity() == SHARED_SPACE);

    int number_of_pages = space->CountTotalPages();
    size_t area_size = space->AreaSize();

    const bool in_standard_path
        = !(v8_flags.manual_evacuation_candidates_selection || v8_flags.stress_compaction_random || v8_flags.stress_compaction || v8_flags.compact_on_every_full_gc);
    ...
    CodePageHeaderModificationScope rwx_write_scope("Modification of Code page header flags requires write access");

    DCHECK(!sweeper()->sweeping_in_progress());
    Page* owner_of_linear_allocation_area = space->top() == space->limit() ? nullptr : Page::FromAllocationAreaAddress(space->top());
    for (Page* p : *space) {
        if (p->NeverEvacuate() || (p == owner_of_linear_allocation_area) || !p->CanAllocate())
            continue;

        if (p->IsPinned()) {
            DCHECK(!p->IsFlagSet(MemoryChunk::FORCE_EVACUATION_CANDIDATE_FOR_TESTING));
            continue;
        }
    ...
    if (v8_flags.manual_evacuation_candidates_selection) {
        ...
        for (uint64_t i : isolate()->fuzzer_rng()->NextSample(pages.size(), pages_to_mark_count)) {
            candidate_count++;
            total_live_bytes += pages[i].first;
        ...
        for (size_t i = 0; i < pages.size(); i++) {
            ...
            if (v8_flags.trace_fragmentation_verbose) {
                PrintIsolate(isolate(),
                    ...
                    space->name(), (area_size - live_bytes) / KB, free_bytes_threshold / KB, target_fragmentation_percent, total_live_bytes / KB, max_evacuated_bytes / KB);
            }
        }
...
{
    if (compacting_) {
        CodePageHeaderModificationScope rwx_write_scope("Changing Code page flags and remembered sets require write access "
                                                        "to the page header");
...
void MarkCompactCollector::FinishConcurrentMarking()
{
    ...
    DCHECK_EQ(heap()->concurrent_marking()->garbage_collector(), GarbageCollector::MARK_COMPACTOR);
    if (v8_flags.parallel_marking || v8_flags.concurrent_marking) {
        heap()->concurrent_marking()->Join();
        heap()->concurrent_marking()->FlushMemoryChunkData(non_atomic_marking_state());
...
void MarkCompactCollector::VerifyMarking()
{
    ...
    if (v8_flags.verify_heap) {
        ...
        if (heap()->shared_space())
            heap()->shared_space()->VerifyLiveBytes();
...
{
    {
        ...
        if (v8_flags.minor_mc && heap()->new_space()) {
            ...
            GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_FINISH_NEW, ThreadKind::kMain);
            sweeper()->ParallelSweepSpace(NEW_SPACE, Sweeper::SweepingMode::kEagerDuringGC, 0);
...
void MarkCompactCollector::SweepArrayBufferExtensions()
{
    ...
    heap_->array_buffer_sweeper()->RequestSweep(ArrayBufferSweeper::SweepingType::kFull);
}
                ...
                if (deopt_data.length() > 0) {
                    ...
                    for (int i = 0; i < literals_length; ++i) {
                        ...
                        if (maybe_literal.GetHeapObject(&heap_literal)) {
                            MarkObjectByPointer(Root::kStackRoots, FullObjectSlot(&heap_literal));
                        }
                    }
                }
            }
...
public:
    ...
    void VisitPointer(HeapObject host, ObjectSlot p) final { MarkObject(host, p.load(cage_base())); }

    void VisitMapPointer(HeapObject host) final { MarkObject(host, host.map(cage_base())); }
    ...
    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) override { MarkObject(host, rinfo->target_object(cage_base())); }
...
class MarkCompactCollector::SharedHeapObjectVisitor final : public ObjectVisitorWithCageBases {
public:
    ...
    void VisitPointer(HeapObject host, ObjectSlot p) final { CheckForSharedObject(host, p, p.load(cage_base())); }
    ...
    void VisitMapPointer(HeapObject host) final { CheckForSharedObject(host, host.map_slot(), host.map(cage_base())); }
    ...
    void VisitCodePointer(HeapObject host, CodeObjectSlot slot) override { UNREACHABLE(); }
    ...
    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) override { UNREACHABLE(); }

private:
    V8_INLINE void CheckForSharedObject(HeapObject host, ObjectSlot slot, Object object)
    {
        ...
        RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::NON_ATOMIC>(host_chunk, slot.address());
...
public:
    ...
    void VisitRootPointers(Root root, const char* description, FullObjectSlot start, FullObjectSlot end) override { UNREACHABLE(); }
...
public:
    explicit MarkExternalPointerFromExternalStringTable(ExternalPointerTable* shared_table)
...
private:
    ...
    public:
        ...
        void VisitExternalPointer(HeapObject host, ExternalPointerSlot slot, ExternalPointerTag tag) override
        {
        ...
        void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) override { UNREACHABLE(); }
        void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) override { UNREACHABLE(); }
        void VisitCodePointer(HeapObject host, CodeObjectSlot slot) override { UNREACHABLE(); }
        void VisitCodeTarget(Code host, RelocInfo* rinfo) override { UNREACHABLE(); }
        void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) override { UNREACHABLE(); }
...
public:
    explicit RecordMigratedSlotVisitor(Heap* heap, EphemeronRememberedSet* ephemeron_remembered_set)
    ...
    {
        DCHECK(!HasWeakHeapObjectTag(p.load(cage_base())));
        RecordMigratedSlot(host, MaybeObject::FromObject(p.load(cage_base())), p.address());
    }

    inline void VisitMapPointer(HeapObject host) final { VisitPointer(host, host.map_slot()); }
    ...
    inline void VisitPointers(HeapObject host, ObjectSlot start, ObjectSlot end) final
    {
        while (start < end) {
    ...
    inline void VisitPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) final
    {
        while (start < end) {
            VisitPointer(host, start);
            ++start;
    ...
    inline void VisitEphemeron(HeapObject host, int index, ObjectSlot key, ObjectSlot value) override
    {
    ...
    inline void VisitExternalPointer(HeapObject host, ExternalPointerSlot slot, ExternalPointerTag tag) final { }

    virtual void MarkArrayBufferExtensionPromoted(HeapObject object) { }

protected:
    inline virtual void RecordMigratedSlot(HeapObject host, MaybeObject value, Address slot)
    {
        if (value->IsStrongOrWeak()) {
            BasicMemoryChunk* p = BasicMemoryChunk::FromAddress(value.ptr());
            if (p->InYoungGeneration()) {
                DCHECK_IMPLIES(p->IsToPage(), v8_flags.minor_mc || p->IsFlagSet(Page::PAGE_NEW_NEW_PROMOTION) || p->IsLargePage());
                ...
                if (V8_EXTERNAL_CODE_SPACE_BOOL && p->IsFlagSet(MemoryChunk::IS_EXECUTABLE)) {
                    RememberedSet<OLD_TO_CODE>::Insert<AccessMode::NON_ATOMIC>(MemoryChunk::FromHeapObject(host), slot);
                } else {
                    RememberedSet<OLD_TO_OLD>::Insert<AccessMode::NON_ATOMIC>(MemoryChunk::FromHeapObject(host), slot);
                }
            } else if (p->InSharedHeap() && !host.InSharedWritableHeap()) {
                RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::NON_ATOMIC>(MemoryChunk::FromHeapObject(host), slot);
            }
        }
    }
...
public:
    ...
    inline void Move(AllocationSpace dest, HeapObject src, HeapObject dst, int size) final
    {
        if (dest == CODE_SPACE || (dest == OLD_SPACE && dst.IsBytecodeArray())) {
            PROFILE(heap_->isolate(), CodeMoveEvent(AbstractCode::cast(src), AbstractCode::cast(dst)));
        }
...
#if DEBUG
    void DisableAbortEvacuationAtAddress(MemoryChunk* chunk) { abort_evacuation_at_address_ = chunk->area_end(); }
    ...
    void SetUpAbortEvacuationAtAddress(MemoryChunk* chunk)
    {
        if (v8_flags.stress_compaction || v8_flags.stress_compaction_random) {
            ...
            if (heap_->isolate()->fuzzer_rng()->NextDouble() < kFraction) {
                const double abort_evacuation_percentage = heap_->isolate()->fuzzer_rng()->NextDouble();
...
protected:
    ...
    using MigrateFunction = void (*)(EvacuateVisitorBase* base, HeapObject dst, HeapObject src, int size, AllocationSpace dest);

    template <MigrationMode mode> static void RawMigrateObject(EvacuateVisitorBase* base, HeapObject dst, HeapObject src, int size, AllocationSpace dest)
    {
    ...
    EvacuateVisitorBase(Heap* heap, EvacuationAllocator* local_allocator, ConcurrentAllocator* shared_old_allocator, RecordMigratedSlotVisitor* record_visitor)
        : heap_(heap)
    ...
    inline bool TryEvacuateObject(AllocationSpace target_space, HeapObject object, int size, HeapObject* target_object)
    {
#if DEBUG
        DCHECK_LE(abort_evacuation_at_address_, MemoryChunk::FromHeapObject(object)->area_end());
        DCHECK_GE(abort_evacuation_at_address_, MemoryChunk::FromHeapObject(object)->area_start());
        ...
        if (target_space == OLD_SPACE && ShouldPromoteIntoSharedHeap(map)) {
            ...
            allocation = shared_old_allocator_->AllocateRaw(size, alignment, AllocationOrigin::kGC);
        } else {
            allocation = local_allocator_->Allocate(target_space, size, AllocationOrigin::kGC, alignment);
        }
        if (allocation.To(target_object)) {
            MigrateObject(*target_object, object, size, target_space);
            if (target_space == CODE_SPACE)
                MemoryChunk::FromHeapObject(*target_object)->GetCodeObjectRegistry()->RegisterNewlyAllocatedCodeObject((*target_object).address());
            return true;
        }
        return false;
    }
    ...
    inline void ExecuteMigrationObservers(AllocationSpace dest, HeapObject src, HeapObject dst, int size)
    {
    ...
    inline void MigrateObject(HeapObject dst, HeapObject src, int size, AllocationSpace dest) { migration_function_(this, dst, src, size, dest); }
...
public:
    explicit EvacuateNewSpaceVisitor(Heap* heap, EvacuationAllocator* local_allocator, ConcurrentAllocator* shared_old_allocator, RecordMigratedSlotVisitor* record_visitor,
        PretenturingHandler::PretenuringFeedbackMap* local_pretenuring_feedback, AlwaysPromoteYoung always_promote_young)
        : EvacuateVisitorBase(heap, local_allocator, shared_old_allocator, record_visitor)
    ...
    inline bool Visit(HeapObject object, int size) override
    {
        ...
        if (always_promote_young_ == AlwaysPromoteYoung::kYes) {
            pretenuring_handler_->UpdateAllocationSite(object.map(), object, local_pretenuring_feedback_);
        ...
        if (heap_->new_space()->ShouldBePromoted(object.address()) && TryEvacuateObject(OLD_SPACE, object, size, &target_object)) {
...
private:
    inline bool TryEvacuateWithoutCopy(HeapObject object)
    {
        ...
        if (map.visitor_id() == kVisitThinString) {
            ...
            if (MarkCompactCollector::IsOnEvacuationCandidate(actual))
                return false;
            object.set_map_word(MapWord::FromForwardingAddress(actual), kRelaxedStore);
            return true;
        }
    ...
    inline AllocationSpace AllocateTargetObject(HeapObject old_object, int size, HeapObject* target_object)
    {
        AllocationAlignment alignment = HeapObject::RequiredAlignment(old_object.map());
        AllocationSpace space_allocated_in = NEW_SPACE;
        AllocationResult allocation = local_allocator_->Allocate(NEW_SPACE, size, AllocationOrigin::kGC, alignment);
    ...
    {
        AllocationResult allocation = local_allocator_->Allocate(OLD_SPACE, size_in_bytes, AllocationOrigin::kGC, alignment);
        if (allocation.IsFailure()) {
...
template <PageEvacuationMode mode> class EvacuateNewSpacePageVisitor final : public HeapObjectVisitor {
public:
    explicit EvacuateNewSpacePageVisitor(Heap* heap, RecordMigratedSlotVisitor* record_visitor, PretenturingHandler::PretenuringFeedbackMap* local_pretenuring_feedback)
        : heap_(heap)
        , record_visitor_(record_visitor)
        , moved_bytes_(0)
        , pretenuring_handler_(heap_->pretenuring_handler())
...
public:
    EvacuateOldSpaceVisitor(Heap* heap, EvacuationAllocator* local_allocator, ConcurrentAllocator* shared_old_allocator, RecordMigratedSlotVisitor* record_visitor)
        : EvacuateVisitorBase(heap, local_allocator, shared_old_allocator, record_visitor)
    {
    }
    ...
    {
        HeapObject target_object;
        if (TryEvacuateObject(Page::FromHeapObject(object)->owner_identity(), object, size, &target_object)) {
...
bool MarkCompactCollector::IsUnmarkedHeapObject(Heap* heap, FullObjectSlot p)
{
    ...
    if (!o.IsHeapObject())
        return false;
    HeapObject heap_object = HeapObject::cast(o);
    return heap->mark_compact_collector()->non_atomic_marking_state()->IsWhite(heap_object);
}

void MarkCompactCollector::MarkRoots(RootVisitor* root_visitor, ObjectVisitor* custom_root_body_visitor)
{
    // Mark the heap roots including global variables, stack variables,
    // etc., and all objects reachable from them.
    heap()->IterateRootsIncludingClients(root_visitor, base::EnumSet<SkipRoot> { SkipRoot::kWeak, SkipRoot::kStack });
    ...
    if (isolate()->is_shared_heap_isolate()) {
        isolate()->global_safepoint()->IterateClientIsolates([this, custom_root_body_visitor](Isolate* client) { ProcessTopOptimizedFrame(custom_root_body_visitor, client); });
    }
    ...
    if (!heap_->cpp_heap() && heap_->local_embedder_heap_tracer()->InUse()) {
        ...
        if (stack.stack_start() && heap_->local_embedder_heap_tracer()->embedder_stack_state() == cppgc::EmbedderStackState::kMayContainHeapPointers) {
            GlobalHandleMarkingVisitor global_handles_marker(*heap_, *local_marking_worklists_);
...
Address FindPreviousObjectForConservativeMarking(const Page* page, Address maybe_inner_ptr)
{
    ...
    cell &= ((~static_cast<MarkBit::CellType>(0)) >> (Bitmap::kBitsPerCell - Bitmap::IndexInCell(index) - 1));
    ...
    const uint32_t leftmost_ones = base::bits::CountLeadingZeros(~(cell << leading_zeros));
    const uint32_t index_of_last_leftmost_one = Bitmap::kBitsPerCell - leading_zeros - leftmost_ones;

    // If the leftmost sequence of set bits does not reach the start of the cell,
    // we found it.
    if (index_of_last_leftmost_one > 0) {
        return page->MarkbitIndexToAddress(cell_index * Bitmap::kBitsPerCell + index_of_last_leftmost_one);
    }
    ...
    const uint32_t index_of_last_leading_one = Bitmap::kBitsPerCell - leading_ones;
    DCHECK_LT(0, index_of_last_leading_one);
    return page->MarkbitIndexToAddress(cell_index * Bitmap::kBitsPerCell + index_of_last_leading_one);
}
...
Address MarkCompactCollector::FindBasePtrForMarking(Address maybe_inner_ptr)
{
    ...
    const MemoryChunk* chunk = heap()->memory_allocator()->LookupChunkContainingAddress(maybe_inner_ptr);
    if (chunk == nullptr)
        return kNullAddress;
    DCHECK(chunk->Contains(maybe_inner_ptr));
    // If it is contained in a large page, we want to mark the only object on it.
    if (chunk->IsLargePage())
        return chunk->area_start();
    ...
    Address base_ptr = FindPreviousObjectForConservativeMarking(page, maybe_inner_ptr);
...
void MarkCompactCollector::MarkRootsFromStack(RootVisitor* root_visitor) { heap()->IterateRootsFromStackIncludingClient(root_visitor); }
...
void MarkCompactCollector::MarkObjectsFromClientHeaps()
{
    ...
    isolate()->global_safepoint()->IterateClientIsolates([collector = this](Isolate* client) {
        if (client->is_shared_heap_isolate())
            return;
...
void MarkCompactCollector::MarkObjectsFromClientHeap(Isolate* client)
{
    ...
    if (heap->new_space()) {
        std::unique_ptr<ObjectIterator> iterator = heap->new_space()->GetObjectIterator(heap);
    ...
    if (heap->new_lo_space()) {
        std::unique_ptr<ObjectIterator> iterator = heap->new_lo_space()->GetObjectIterator(heap);
        for (HeapObject obj = iterator->Next(); !obj.is_null(); obj = iterator->Next()) {
            obj.IterateFast(cage_base, &visitor);
        }
    }
    ...
    for (MemoryChunk* chunk = chunk_iterator.next(); chunk; chunk = chunk_iterator.next()) {
        InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToShared(chunk, InvalidatedSlotsFilter::LivenessCheck::kNo);
        RememberedSet<OLD_TO_SHARED>::Iterate(
            ...
            [collector = this, cage_base, &filter](MaybeObjectSlot slot) {
                ...
                if (obj.GetHeapObject(&heap_object) && heap_object.InSharedWritableHeap()) {
        ...
        RememberedSet<OLD_TO_SHARED>::IterateTyped(chunk, [collector = this, heap](SlotType slot_type, Address slot) {
            HeapObject heap_object = UpdateTypedSlotHelper::GetTargetObject(heap, slot_type, slot);
            if (heap_object.InSharedWritableHeap()) {
                collector->MarkRootObject(Root::kClientHeap, heap_object);
                return KEEP_SLOT;
            } else {
    ...
    // condition variables.
    ExternalPointerHandle* handle_location = client->GetWaiterQueueNodeExternalPointerHandleLocation();
    ExternalPointerTable& table = client->shared_external_pointer_table();
    ExternalPointerHandle handle = base::AsAtomic32::Relaxed_Load(handle_location);
    if (handle) {
...
#ifdef V8_ENABLE_SANDBOX
    if (IsSandboxedExternalPointerType(kExternalStringResourceTag) || IsSandboxedExternalPointerType(kExternalStringResourceDataTag)) {
...
void MarkCompactCollector::VisitObject(HeapObject obj) { marking_visitor_->Visit(obj.map(), obj); }
...
bool MarkCompactCollector::MarkTransitiveClosureUntilFixpoint()
{
    ...
    do {
        ...
        {
            TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_MARK_WEAK_CLOSURE_EPHEMERON_MARKING);
        ...
        ++iterations;
    } while (another_ephemeron_iteration_main_thread || heap()->concurrent_marking()->another_ephemeron_iteration() || !local_marking_worklists()->IsEmpty()
        || !local_marking_worklists()->IsWrapperEmpty() || !heap()->local_embedder_heap_tracer()->IsRemoteTracingDone());
...
{
    TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_MARK_WEAK_CLOSURE_EPHEMERON_LINEAR);
    ...
    while (work_to_do) {
...

        {
            ...
            // newly_discovered.
            ProcessMarkingWorklist(0, MarkingWorklistProcessingMode::kTrackNewlyDiscoveredObjects);
        }
        ...
        work_to_do = !local_marking_worklists()->IsEmpty() || !local_marking_worklists()->IsWrapperEmpty() || !heap()->local_embedder_heap_tracer()->IsRemoteTracingDone();
...
void MarkCompactCollector::PerformWrapperTracing()
{
    if (heap_->local_embedder_heap_tracer()->InUse()) {
        ...
        if (local_marking_worklists()->PublishWrapper()) {
            ...
            LocalEmbedderHeapTracer::ProcessingScope scope(heap_->local_embedder_heap_tracer());
            HeapObject object;
            while (local_marking_worklists()->PopWrapper(&object)) {
                scope.TracePossibleWrapper(JSObject::cast(object));
            }
        }
        heap_->local_embedder_heap_tracer()->Trace(std::numeric_limits<double>::infinity());
    }
}

std::pair<size_t, size_t> MarkCompactCollector::ProcessMarkingWorklist(size_t bytes_to_process)
{
    return ProcessMarkingWorklist(bytes_to_process, MarkingWorklistProcessingMode::kDefault);
}

std::pair<size_t, size_t> MarkCompactCollector::ProcessMarkingWorklist(size_t bytes_to_process, MarkingWorklistProcessingMode mode)
{
    HeapObject object;
    ...
    CodePageHeaderModificationScope rwx_write_scope("Marking of Code objects require write access to Code page headers");
    ...
    while (local_marking_worklists()->Pop(&object) || local_marking_worklists()->PopOnHold(&object)) {
        ...
        if (object.IsFreeSpaceOrFiller(cage_base)) {
            ...
            DCHECK_IMPLIES(object.map(cage_base) == ReadOnlyRoots(isolate).one_pointer_filler_map(), marking_state()->IsBlack(object));
            // Other fillers may be black or grey depending on the color of the object
            // that was trimmed.
            DCHECK_IMPLIES(object.map(cage_base) != ReadOnlyRoots(isolate).one_pointer_filler_map(), marking_state()->IsBlackOrGrey(object));
            continue;
        }
        ...
        if (is_per_context_mode) {
            native_context_stats_.IncrementSize(local_marking_worklists()->Context(), map, object, visited_size);
        }
...
{
    for (StackFrameIterator it(isolate, isolate->thread_local_top()); !it.done(); it.Advance()) {
...
void MarkCompactCollector::RecordObjectStats()
{
    ...
    ObjectStatsCollector collector(heap(), heap()->live_object_stats_.get(), heap()->dead_object_stats_.get());
    collector.Collect();
    if (V8_UNLIKELY(TracingFlags::gc_stats.load(std::memory_order_relaxed) & v8::tracing::TracingCategoryObserver::ENABLED_BY_TRACING)) {
        ...
        TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("v8.gc_stats"), "V8.GC_Objects_Stats", TRACE_EVENT_SCOPE_THREAD, "live", TRACE_STR_COPY(live.str().c_str()), "dead",
...
void MarkCompactCollector::RetainMaps()
{
    ...
    // aggressively.
    const bool should_retain_maps = !heap()->ShouldReduceMemory() && v8_flags.retain_maps_for_n_gc != 0;
...
void MarkCompactCollector::MarkLiveObjects()
{
    ...
    if (v8_flags.parallel_marking) {
        ...
        {
            TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_MARK_FULL_CLOSURE_PARALLEL_JOIN);
...
public:
    ...
    void Add(std::unique_ptr<ClearingItem> item) { items_.push_back(std::move(item)); }
...
public:
    ...
    void Run(JobDelegate* delegate) final
    {
        if (isolate_->OwnsStringTables()) {
            TRACE_GC1(isolate_->heap()->tracer(), GCTracer::Scope::MC_CLEAR_STRING_TABLE, delegate->IsJoiningThread() ? ThreadKind::kMain : ThreadKind::kBackground);
...
private:
    ...
    void TryExternalize(String original_string, StringForwardingTable::Record* record)
    {
    ...
    void TryInternalize(String original_string, StringForwardingTable::Record* record)
    {
        if (original_string.IsInternalizedString())
            return;
        ...
        // compaction.
        ObjectSlot slot = ThinString::cast(original_string).RawField(ThinString::kActualOffset);
...
void MarkCompactCollector::ClearNonLiveReferences()
{
    ...
    if (isolate()->OwnsStringTables()) {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_CLEAR_STRING_FORWARDING_TABLE);
    ...
    auto clearing_job_handle = V8::GetCurrentPlatform()->PostJob(TaskPriority::kUserBlocking, std::move(clearing_job));
...

    {
        ...
        // CPU profiler.
        heap()->isolate()->global_handles()->IterateWeakRootsForPhantomHandles(&IsUnmarkedHeapObject);
    }
...
#ifdef V8_ENABLE_SANDBOX
    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_SWEEP_EXTERNAL_POINTER_TABLE);
...
void MarkCompactCollector::ClearPotentialSimpleMapTransition(Map dead_target)
{
    ...
    if (potential_parent.IsMap()) {
        ...
        if (non_atomic_marking_state()->IsBlackOrGrey(parent) && TransitionsAccessor(isolate(), parent).HasSimpleTransitionTo(dead_target)) {
            ClearPotentialSimpleMapTransition(parent, dead_target);
        }
    }
}

void MarkCompactCollector::ClearPotentialSimpleMapTransition(Map map, Map dead_target)
{
...
void MarkCompactCollector::FlushBytecodeFromSFI(SharedFunctionInfo shared_info)
{
    ...
    shared_info.DiscardCompiledMetadata(isolate(), [](HeapObject object, ObjectSlot slot, HeapObject target) { RecordSlot(object, slot, target); });
    ...
    compiled_data.set_map_after_allocation(ReadOnlyRoots(heap()).uncompiled_data_without_preparse_data_map(), SKIP_WRITE_BARRIER);
    ...
    if (!heap()->IsLargeObject(compiled_data)) {
        ...
        heap()->CreateFillerObjectAt(compiled_data.address() + aligned_filler_offset, compiled_data_size - aligned_filler_offset);
    }
    ...
    uncompiled_data.InitAfterBytecodeFlush(
        inferred_name, start_position, end_position, [](HeapObject object, ObjectSlot slot, HeapObject target) { RecordSlot(object, slot, target); });

    // Mark the uncompiled data as black, and ensure all fields have already been
    // marked.
    DCHECK(!ShouldMarkObject(inferred_name) || marking_state()->IsBlackOrGrey(inferred_name));
...
void MarkCompactCollector::ProcessOldCodeCandidates()
{
    ...
    while (local_weak_objects()->code_flushing_candidates_local.Pop(&flushing_candidate)) {
        bool is_bytecode_live = non_atomic_marking_state()->IsBlackOrGrey(flushing_candidate.GetBytecodeArray(isolate()));
        if (v8_flags.flush_baseline_code && flushing_candidate.HasBaselineCode()) {
            CodeT baseline_codet = CodeT::cast(flushing_candidate.function_data(kAcquireLoad));
            ...
            if (non_atomic_marking_state()->IsBlackOrGrey(baseline_code)) {
                ...
                flushing_candidate.set_function_data(baseline_code.bytecode_or_interpreter_data(), kReleaseStore);
            }
        }
        ...
        if (!is_bytecode_live) {
            ...
            DCHECK(v8_flags.flush_baseline_code || !flushing_candidate.HasBaselineCode());
...
void MarkCompactCollector::ClearFlushedJsFunctions()
{
    ...
    while (local_weak_objects()->flushed_js_functions_local.Pop(&flushed_js_function)) {
        auto gc_notify_updated_slot = [](HeapObject object, ObjectSlot slot, Object target) { RecordSlot(object, slot, HeapObject::cast(target)); };
    ...
    while (local_weak_objects()->transition_arrays_local.Pop(&array)) {
        ...
        if (num_transitions > 0) {
            ...
            if (array.GetTargetIfExists(0, isolate(), &map)) {
                ...
                DescriptorArray descriptors = parent_is_alive ? parent.instance_descriptors(isolate()) : DescriptorArray();
...
bool MarkCompactCollector::TransitionArrayNeedsCompaction(TransitionArray transitions, int num_transitions)
{
    for (int i = 0; i < num_transitions; ++i) {
        ...
        if (raw_target.IsSmi()) {
            ...
            for (int j = 0; j < num_transitions; ++j) {
                DCHECK_IMPLIES(!transitions.GetRawTarget(j).IsSmi(), !non_atomic_marking_state()->IsWhite(transitions.GetTarget(j)));
            }
#endif
            return false;
        } else if (non_atomic_marking_state()->IsWhite(TransitionsAccessor::GetTargetFromRaw(raw_target))) {
#ifdef DEBUG
            // Targets can only be dead iff this array is fully deserialized.
            for (int j = 0; j < num_transitions; ++j) {
...
bool MarkCompactCollector::CompactTransitionArray(Map map, TransitionArray transitions, DescriptorArray descriptors)
{
    DCHECK(!map.is_prototype_map());
    ...
    for (int i = 0; i < num_transitions; ++i) {
        ...
        if (non_atomic_marking_state()->IsWhite(target)) {
            if (!descriptors.is_null() && target.instance_descriptors(isolate()) == descriptors) {
    ...
    if (trim > 0) {
        heap_->RightTrimWeakFixedArray(transitions, trim * TransitionArray::kEntrySize);
        transitions.SetNumberOfTransitions(transition_index);
    }
    return descriptors_owner_died;
}

void MarkCompactCollector::RightTrimDescriptorArray(DescriptorArray array, int descriptors_to_trim)
{
    ...
    if (V8_COMPRESS_POINTERS_8GB_BOOL) {
        ...
        if (aligned_start < aligned_end) {
            heap()->CreateFillerObjectAt(aligned_start, static_cast<int>(aligned_end - aligned_start));
        }
        if (Heap::ShouldZapGarbage()) {
            Address zap_end = std::min(aligned_start, end);
            MemsetTagged(ObjectSlot(start), Object(static_cast<Address>(kZapValue)), (zap_end - start) >> kTaggedSizeLog2);
        }
    } else {
...
void MarkCompactCollector::TrimDescriptorArray(Map map, DescriptorArray descriptors)
{
    int number_of_own_descriptors = map.NumberOfOwnDescriptors();
...
void MarkCompactCollector::ClearWeakCollections()
{
        ...
        for (InternalIndex i : table.IterateEntries()) {
            ...
            if (v8_flags.verify_heap) {
                ...
                if (value.IsHeapObject()) {
                    ...
                    CHECK_IMPLIES(!ShouldMarkObject(key) || non_atomic_marking_state()->IsBlackOrGrey(key),
                        !ShouldMarkObject(heap_object) || non_atomic_marking_state()->IsBlackOrGrey(heap_object));
                }
            }
#endif
    ...
    for (auto it = heap_->ephemeron_remembered_set_.begin(); it != heap_->ephemeron_remembered_set_.end();) {
...
void MarkCompactCollector::ClearJSWeakRefs()
{
    ...
    while (local_weak_objects()->weak_cells_local.Pop(&weak_cell)) {
        auto gc_notify_updated_slot = [](HeapObject object, ObjectSlot slot, Object target) {
        ...
        if (!non_atomic_marking_state()->IsBlackOrGrey(target)) {
            ...
            JSFinalizationRegistry finalization_registry = JSFinalizationRegistry::cast(weak_cell.finalization_registry());
        ...
        if (!non_atomic_marking_state()->IsBlackOrGrey(unregister_token)) {
            ...
            // slots.
            JSFinalizationRegistry finalization_registry = JSFinalizationRegistry::cast(weak_cell.finalization_registry());
            finalization_registry.RemoveUnregisterToken(unregister_token, isolate(), JSFinalizationRegistry::kKeepMatchedCellsInRegistry, gc_notify_updated_slot);
        } else {
...
bool MarkCompactCollector::IsOnEvacuationCandidate(MaybeObject obj) { return Page::FromAddress(obj.ptr())->IsEvacuationCandidate(); }

// static
bool MarkCompactCollector::ShouldRecordRelocSlot(Code host, RelocInfo* rinfo, HeapObject target)
{
...
// static
MarkCompactCollector::RecordRelocSlotInfo MarkCompactCollector::ProcessRelocInfo(Code host, RelocInfo* rinfo, HeapObject target)
{
    DCHECK_EQ(host, rinfo->host());
...
// static
void MarkCompactCollector::RecordRelocSlot(Code host, RelocInfo* rinfo, HeapObject target)
{
    ...
    RememberedSet<OLD_TO_OLD>::InsertTyped(info.memory_chunk, info.slot_type, info.offset);
}
...
template <typename TSlot, HeapObjectReferenceType reference_type> typename TSlot::TObject MakeSlotValue(HeapObject heap_object);

template <> Object MakeSlotValue<ObjectSlot, HeapObjectReferenceType::STRONG>(HeapObject heap_object) { return heap_object; }

template <> MaybeObject MakeSlotValue<MaybeObjectSlot, HeapObjectReferenceType::STRONG>(HeapObject heap_object) { return HeapObjectReference::Strong(heap_object); }

template <> MaybeObject MakeSlotValue<MaybeObjectSlot, HeapObjectReferenceType::WEAK>(HeapObject heap_object) { return HeapObjectReference::Weak(heap_object); }

template <> Object MakeSlotValue<OffHeapObjectSlot, HeapObjectReferenceType::STRONG>(HeapObject heap_object) { return heap_object; }

#ifdef V8_COMPRESS_POINTERS
template <> Object MakeSlotValue<FullObjectSlot, HeapObjectReferenceType::STRONG>(HeapObject heap_object) { return heap_object; }

template <> MaybeObject MakeSlotValue<FullMaybeObjectSlot, HeapObjectReferenceType::STRONG>(HeapObject heap_object) { return HeapObjectReference::Strong(heap_object); }
...
template <AccessMode access_mode, HeapObjectReferenceType reference_type, typename TSlot>
static inline void UpdateSlot(PtrComprCageBase cage_base, TSlot slot, typename TSlot::TObject old, HeapObject heap_obj)
{
    static_assert(std::is_same<TSlot, FullObjectSlot>::value || std::is_same<TSlot, ObjectSlot>::value || std::is_same<TSlot, FullMaybeObjectSlot>::value
            || std::is_same<TSlot, MaybeObjectSlot>::value || std::is_same<TSlot, OffHeapObjectSlot>::value,
    ...
    if (map_word.IsForwardingAddress()) {
        ...
        PtrComprCageBase host_cage_base = V8_EXTERNAL_CODE_SPACE_BOOL ? GetPtrComprCageBase(heap_obj) : cage_base;
        typename TSlot::TObject target = MakeSlotValue<TSlot, reference_type>(map_word.ToForwardingAddress(host_cage_base));
...
template <AccessMode access_mode, typename TSlot> static inline void UpdateSlot(PtrComprCageBase cage_base, TSlot slot)
{
    typename TSlot::TObject obj = slot.Relaxed_Load(cage_base);
    HeapObject heap_obj;
    if (TSlot::kCanBeWeak && obj->GetHeapObjectIfWeak(&heap_obj)) {
        UpdateSlot<access_mode, HeapObjectReferenceType::WEAK>(cage_base, slot, obj, heap_obj);
    } else if (obj->GetHeapObjectIfStrong(&heap_obj)) {
        UpdateSlot<access_mode, HeapObjectReferenceType::STRONG>(cage_base, slot, obj, heap_obj);
    }
}

static inline SlotCallbackResult UpdateOldToSharedSlot(PtrComprCageBase cage_base, MaybeObjectSlot slot)
{
    MaybeObject obj = slot.Relaxed_Load(cage_base);
    HeapObject heap_obj;
    ...
    if (obj.GetHeapObject(&heap_obj)) {
        if (obj.IsWeak()) {
            UpdateSlot<AccessMode::NON_ATOMIC, HeapObjectReferenceType::WEAK>(cage_base, slot, obj, heap_obj);
        } else {
            UpdateSlot<AccessMode::NON_ATOMIC, HeapObjectReferenceType::STRONG>(cage_base, slot, obj, heap_obj);
        }
...
template <AccessMode access_mode, typename TSlot> static inline void UpdateStrongSlot(PtrComprCageBase cage_base, TSlot slot)
{
    typename TSlot::TObject obj = slot.Relaxed_Load(cage_base);
...
template <AccessMode access_mode> static inline void UpdateStrongCodeSlot(HeapObject host, PtrComprCageBase cage_base, PtrComprCageBase code_cage_base, CodeObjectSlot slot)
{
    ...
    if (obj.GetHeapObject(&heap_obj)) {
        ...
        CodeDataContainer code_data_container = CodeDataContainer::cast(HeapObject::FromAddress(slot.address() - CodeDataContainer::kCodeOffset));
...
public:
    ...
    void VisitPointer(HeapObject host, ObjectSlot p) override { UpdateStrongSlotInternal(cage_base(), p); }

    void VisitPointer(HeapObject host, MaybeObjectSlot p) override { UpdateSlotInternal(cage_base(), p); }
    ...
    {
        CHECK(V8_EXTERNAL_CODE_SPACE_BOOL);
        UpdateStrongCodeSlot<AccessMode::NON_ATOMIC>(host, cage_base(), code_cage_base(), slot);
    }
...
private:
    static inline void UpdateRootSlotInternal(PtrComprCageBase cage_base, FullObjectSlot slot) { UpdateStrongSlot<AccessMode::NON_ATOMIC>(cage_base, slot); }

    static inline void UpdateRootSlotInternal(PtrComprCageBase cage_base, OffHeapObjectSlot slot) { UpdateStrongSlot<AccessMode::NON_ATOMIC>(cage_base, slot); }

    static inline void UpdateStrongMaybeObjectSlotInternal(PtrComprCageBase cage_base, MaybeObjectSlot slot) { UpdateStrongSlot<AccessMode::NON_ATOMIC>(cage_base, slot); }

    static inline void UpdateStrongSlotInternal(PtrComprCageBase cage_base, ObjectSlot slot) { UpdateStrongSlot<AccessMode::NON_ATOMIC>(cage_base, slot); }

    static inline void UpdateSlotInternal(PtrComprCageBase cage_base, MaybeObjectSlot slot) { UpdateSlot<AccessMode::NON_ATOMIC>(cage_base, slot); }
};

static String UpdateReferenceInExternalStringTableEntry(Heap* heap, FullObjectSlot p)
{
    ...
    if (map_word.IsForwardingAddress()) {
        ...
        if (new_string.IsExternalString()) {
            MemoryChunk::MoveExternalBackingStoreBytes(
                ExternalBackingStoreType::kExternalString, Page::FromAddress((*p).ptr()), Page::FromHeapObject(new_string), ExternalString::cast(new_string).ExternalPayloadSize());
        }
        return new_string;
    }
...
void MarkCompactCollector::EvacuateEpilogue()
{
    ...
    while (chunk_iterator.HasNext()) {
        ...
        if (id == SHARED_SPACE || id == SHARED_LO_SPACE || id == NEW_SPACE || id == NEW_LO_SPACE || isolate()->is_shared()) {
...
public:
    ...
    // evacuation.
    static intptr_t NewSpacePageEvacuationThreshold() { return v8_flags.page_promotion_threshold * MemoryChunkLayout::AllocatableMemoryInDataPage() / 100; }

    Evacuator(Heap* heap, RecordMigratedSlotVisitor* record_visitor, EvacuationAllocator* local_allocator, AlwaysPromoteYoung always_promote_young)
        : heap_(heap)
        , local_pretenuring_feedback_(PretenturingHandler::kInitialFeedbackCapacity)
        , shared_old_allocator_(CreateSharedOldAllocator(heap_))
        , new_space_visitor_(heap_, local_allocator, shared_old_allocator_.get(), record_visitor, &local_pretenuring_feedback_, always_promote_young)
        , new_to_new_page_visitor_(heap_, record_visitor, &local_pretenuring_feedback_)
        , new_to_old_page_visitor_(heap_, record_visitor, &local_pretenuring_feedback_)
        ,

        old_space_visitor_(heap_, local_allocator, shared_old_allocator_.get(), record_visitor)
        , local_allocator_(local_allocator)
        , duration_(0.0)
        , bytes_compacted_(0)
    {
...
protected:
    ...
    virtual bool RawEvacuatePage(MemoryChunk* chunk, intptr_t* saved_live_bytes) = 0;
...
void Evacuator::EvacuatePage(MemoryChunk* chunk)
{
    ...
    if (v8_flags.trace_evacuation) {
        PrintIsolate(heap()->isolate(),
            ...
            static_cast<void*>(this), static_cast<void*>(chunk), chunk->InNewSpace(),
            chunk->IsFlagSet(Page::PAGE_NEW_OLD_PROMOTION) || chunk->IsFlagSet(Page::PAGE_NEW_NEW_PROMOTION), chunk->IsFlagSet(MemoryChunk::IS_EXECUTABLE),
            heap()->new_space()->IsPromotionCandidate(chunk), saved_live_bytes, evacuation_time, success);
    }
}
...
void Evacuator::Finalize()
{
    ...
    heap()->IncrementNewSpaceSurvivingObjectSize(new_space_visitor_.semispace_copied_size() + new_to_new_page_visitor_.moved_bytes());
...
public:
    explicit FullEvacuator(Heap* heap)
        : Evacuator(heap, &record_visitor_, &local_allocator_, AlwaysPromoteYoung::kYes)
    ...
    GCTracer::Scope::ScopeId GetTracingScope() override { return GCTracer::Scope::MC_EVACUATE_COPY_PARALLEL; }

    void Finalize() override
    {
        Evacuator::Finalize();

        for (auto it = ephemeron_remembered_set_.begin(); it != ephemeron_remembered_set_.end(); ++it) {
...
bool FullEvacuator::RawEvacuatePage(MemoryChunk* chunk, intptr_t* live_bytes)
{
    ...
    *live_bytes = marking_state->live_bytes(chunk);
    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "FullEvacuator::RawEvacuatePage", "evacuation_mode", EvacuationModeName(evacuation_mode), "live_bytes", *live_bytes);
...
#endif // DEBUG
        LiveObjectVisitor::VisitBlackObjectsNoFail(chunk, marking_state, &new_space_visitor_);
        marking_state->ClearLiveness(chunk);
        break;
    case kPageNewToOld:
        LiveObjectVisitor::VisitBlackObjectsNoFail(chunk, marking_state, &new_to_old_page_visitor_);
        new_to_old_page_visitor_.account_moved_bytes(marking_state->live_bytes(chunk));
        break;
    case kPageNewToNew:
        DCHECK(!v8_flags.minor_mc);
        LiveObjectVisitor::VisitBlackObjectsNoFail(chunk, marking_state, &new_to_new_page_visitor_);
        new_to_new_page_visitor_.account_moved_bytes(marking_state->live_bytes(chunk));
        break;
    case kObjectsOldToOld: {
        RwxMemoryWriteScope rwx_write_scope("Evacuation of objects in Code space requires write access for the "
                                            "current worker thread.");
#if DEBUG
        old_space_visitor_.SetUpAbortEvacuationAtAddress(chunk);
#endif // DEBUG
        const bool success = LiveObjectVisitor::VisitBlackObjects(chunk, marking_state, &old_space_visitor_, &failed_object);
        if (success) {
            marking_state->ClearLiveness(chunk);
        } else {
            if (v8_flags.crash_on_aborted_evacuation) {
                ...
                heap_->mark_compact_collector()->ReportAbortedEvacuationCandidateDueToOOM(failed_object.address(), static_cast<Page*>(chunk));
                return false;
            }
        }
        break;
...
public:
    PageEvacuationJob(Isolate* isolate, std::vector<std::unique_ptr<Evacuator>>* evacuators, std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> evacuation_items)
    ...
    void Run(JobDelegate* delegate) override
    {
        ...
        if (delegate->IsJoiningThread()) {
            ...
            TRACE_GC_EPOCH(tracer_, evacuator->GetBackgroundTracingScope(), ThreadKind::kBackground);
            ProcessItems(delegate, evacuator);
        }
    }
    ...
    size_t GetMaxConcurrency(size_t worker_count) const override
    {
        ...
        const size_t wanted_num_workers = (remaining_evacuation_items_.load(std::memory_order_relaxed) + kItemsPerWorker - 1) / kItemsPerWorker;
...
size_t CreateAndExecuteEvacuationTasks(Heap* heap, std::vector<std::pair<ParallelWorkItem, MemoryChunk*>> evacuation_items, MigrationObserver* migration_observer)
{
    ...
    V8::GetCurrentPlatform()->CreateJob(v8::TaskPriority::kUserBlocking, std::make_unique<PageEvacuationJob>(heap->isolate(), &evacuators, std::move(evacuation_items)))->Join();
...
bool ShouldMovePage(Page* p, intptr_t live_bytes, intptr_t wasted_bytes, MemoryReductionMode memory_reduction_mode, AlwaysPromoteYoung always_promote_young,
    PromoteUnusablePages promote_unusable_pages)
{
    Heap* heap = p->heap();
    return v8_flags.page_promotion && (memory_reduction_mode == MemoryReductionMode::kNone) && !p->NeverEvacuate()
        && ((live_bytes + wasted_bytes > Evacuator::NewSpacePageEvacuationThreshold()) || (promote_unusable_pages == PromoteUnusablePages::kYes && !p->WasUsedForAllocation()))
        && (always_promote_young == AlwaysPromoteYoung::kYes || heap->new_space()->IsPromotionCandidate(p)) && heap->CanExpandOldGeneration(live_bytes);
}
...
void TraceEvacuation(Isolate* isolate, size_t pages_count, size_t wanted_num_tasks, size_t live_bytes, size_t aborted_pages)
{
    ...
    PrintIsolate(isolate,
        ...
        isolate->time_millis_since_init(), v8_flags.parallel_compaction ? "yes" : "no", pages_count, wanted_num_tasks, V8::GetCurrentPlatform()->NumberOfWorkerThreads() + 1,
        live_bytes, isolate->heap()->tracer()->CompactionSpeedInBytesPerMillisecond(), aborted_pages);
}
...
void MarkCompactCollector::EvacuatePagesInParallel()
{
    ...
    bool force_page_promotion = heap()->IsGCWithStack() && !v8_flags.compact_with_stack;
    for (Page* page : new_space_evacuation_pages_) {
        intptr_t live_bytes_on_page = non_atomic_marking_state()->live_bytes(page);
        DCHECK_LT(0, live_bytes_on_page);
        live_bytes += live_bytes_on_page;
        MemoryReductionMode memory_reduction_mode = heap()->ShouldReduceMemory() ? MemoryReductionMode::kShouldReduceMemory : MemoryReductionMode::kNone;
        if (ShouldMovePage(page, live_bytes_on_page, 0, memory_reduction_mode, AlwaysPromoteYoung::kYes, PromoteUnusablePages::kNo) || force_page_promotion) {
            ...
            heap()->old_space()->DecreaseAllocatedBytes(page->allocated_bytes(), page);
        }
        evacuation_items.emplace_back(ParallelWorkItem {}, page);
    }

    if (heap()->IsGCWithStack()) {
        if (!v8_flags.compact_with_stack || !v8_flags.compact_code_space_with_stack) {
            for (Page* page : old_space_evacuation_pages_) {
                if (!v8_flags.compact_with_stack || page->owner_identity() == CODE_SPACE) {
    ...
    if (!evacuation_items.empty()) {
        ...
        wanted_num_tasks = CreateAndExecuteEvacuationTasks<FullEvacuator>(heap(), std::move(evacuation_items), nullptr);
    }

    const size_t aborted_pages = PostProcessAbortedEvacuationCandidates();

    if (v8_flags.trace_evacuation) {
        TraceEvacuation(isolate(), pages_count, wanted_num_tasks, live_bytes, aborted_pages);
    }
}
...
void MarkCompactCollector::RecordLiveSlotsOnPage(Page* page)
{
    ...
    LiveObjectVisitor::VisitBlackObjectsNoFail(page, non_atomic_marking_state(), &visitor);
}

template <class Visitor, typename MarkingState>
bool LiveObjectVisitor::VisitBlackObjects(MemoryChunk* chunk, MarkingState* marking_state, Visitor* visitor, HeapObject* failed_object)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "LiveObjectVisitor::VisitBlackObjects");
    for (auto object_and_size : LiveObjectRange<kBlackObjects>(chunk, marking_state->bitmap(chunk))) {
...
template <class Visitor, typename MarkingState> void LiveObjectVisitor::VisitBlackObjectsNoFail(MemoryChunk* chunk, MarkingState* marking_state, Visitor* visitor)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "LiveObjectVisitor::VisitBlackObjectsNoFail");
    if (chunk->IsLargePage()) {
...
template <typename MarkingState> void LiveObjectVisitor::RecomputeLiveBytes(MemoryChunk* chunk, MarkingState* marking_state)
{
    int new_live_size = 0;
    for (auto object_and_size : LiveObjectRange<kAllLiveObjects>(chunk, marking_state->bitmap(chunk))) {
...
public:
    explicit PointersUpdatingJob(
        Isolate* isolate, std::vector<std::unique_ptr<UpdatingItem>> updating_items, GCTracer::Scope::ScopeId scope, GCTracer::Scope::ScopeId background_scope)
...
template <typename MarkingState> class ToSpaceUpdatingItem : public UpdatingItem {
public:
    explicit ToSpaceUpdatingItem(Heap* heap, MemoryChunk* chunk, Address start, Address end, MarkingState* marking_state)
        : heap_(heap)
        , chunk_(chunk)
        , start_(start)
        , end_(end)
...
private:
    ...
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "ToSpaceUpdatingItem::ProcessVisitAll");
    ...
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "ToSpaceUpdatingItem::ProcessVisitLive");
        // For young generation evacuations we want to visit grey objects, for
        // full MC, we need to visit black objects.
        PointersUpdatingVisitor visitor(heap_);
        for (auto object_and_size : LiveObjectRange<kAllLiveObjects>(chunk_, marking_state_->bitmap(chunk_))) {
...
template <typename MarkingState, GarbageCollector collector> class RememberedSetUpdatingItem : public UpdatingItem {
public:
    explicit RememberedSetUpdatingItem(Heap* heap, MarkingState* marking_state, MemoryChunk* chunk, RememberedSetUpdatingMode updating_mode)
        : heap_(heap)
        , marking_state_(marking_state)
        , chunk_(chunk)
        , updating_mode_(updating_mode)
        , record_old_to_shared_slots_(heap->isolate()->has_shared_heap() && !chunk->InSharedHeap())
    {
    }
...
private:
    template <typename TSlot> inline void CheckSlotForOldToSharedUntyped(PtrComprCageBase cage_base, MemoryChunk* chunk, TSlot slot)
    {
        HeapObject heap_object;
        ...
        if (heap_object.InSharedWritableHeap()) {
            RememberedSet<OLD_TO_SHARED>::Insert<AccessMode::NON_ATOMIC>(chunk, slot.address());
        }
    }

    inline void CheckSlotForOldToSharedTyped(MemoryChunk* chunk, SlotType slot_type, Address addr)
    {
        HeapObject heap_object = UpdateTypedSlotHelper::GetTargetObject(chunk->heap(), slot_type, addr);

#if DEBUG
        UpdateTypedSlotHelper::UpdateTypedSlot(chunk->heap(), slot_type, addr, [heap_object](FullMaybeObjectSlot slot) {
        ...
        if (heap_object.InSharedWritableHeap()) {
            ...
            DCHECK_LT(offset, static_cast<uintptr_t>(TypedSlotSet::kMaxOffset));
            RememberedSet<OLD_TO_SHARED>::InsertTyped(chunk, slot_type, static_cast<uint32_t>(offset));
        }
    }

    template <typename TSlot> inline SlotCallbackResult CheckAndUpdateOldToNewSlot(TSlot slot)
    {
        static_assert(
            std::is_same<TSlot, FullMaybeObjectSlot>::value || std::is_same<TSlot, MaybeObjectSlot>::value, "Only FullMaybeObjectSlot and MaybeObjectSlot are expected here");
        ...
        if (Heap::InFromPage(heap_object)) {
            DCHECK_IMPLIES(v8_flags.minor_mc, Page::FromHeapObject(heap_object)->IsLargePage());
            MapWord map_word = heap_object.map_word(kRelaxedLoad);
            if (map_word.IsForwardingAddress()) {
                HeapObjectReference::Update(THeapObjectSlot(slot), map_word.ToForwardingAddress());
            }
            ...
            if (Page::FromHeapObject(heap_object)->IsFlagSet(Page::PAGE_NEW_NEW_PROMOTION)) {
                if (marking_state_->IsBlack(heap_object)) {
                    return KEEP_SLOT;
                } else {
                    return REMOVE_SLOT;
    ...
    void UpdateUntypedPointers()
    {
        ...
        if (chunk_->slot_set<OLD_TO_NEW, AccessMode::NON_ATOMIC>() != nullptr) {
            ...
            // objects as well.
            InvalidatedSlotsFilter::LivenessCheck liveness_check = updating_mode_ == RememberedSetUpdatingMode::ALL && !chunk_->SweepingDone()
                ? InvalidatedSlotsFilter::LivenessCheck::kYes
                : InvalidatedSlotsFilter::LivenessCheck::kNo;
            InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToNew(chunk_, liveness_check);
        ...
        if ((updating_mode_ == RememberedSetUpdatingMode::ALL) && (chunk_->slot_set<OLD_TO_OLD, AccessMode::NON_ATOMIC>() != nullptr)) {
            InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToOld(chunk_, InvalidatedSlotsFilter::LivenessCheck::kNo);
        ...
        if ((updating_mode_ == RememberedSetUpdatingMode::ALL) && chunk_->invalidated_slots<OLD_TO_OLD>() != nullptr) {
            // The invalidated slots are not needed after old-to-old slots were
            // processed.
            chunk_->ReleaseInvalidatedSlots<OLD_TO_OLD>();
        }
        if (V8_EXTERNAL_CODE_SPACE_BOOL) {
            if ((updating_mode_ == RememberedSetUpdatingMode::ALL) && (chunk_->slot_set<OLD_TO_CODE, AccessMode::NON_ATOMIC>() != nullptr)) {
                ...
                RememberedSet<OLD_TO_CODE>::Iterate(
                    ...
                    [=](MaybeObjectSlot slot) {
                        HeapObject host = HeapObject::FromAddress(slot.address() - CodeDataContainer::kCodeOffset);
                        DCHECK(host.IsCodeDataContainer(cage_base));
                        UpdateStrongCodeSlot<AccessMode::NON_ATOMIC>(host, cage_base, code_cage_base, CodeObjectSlot(slot.address()));
                        // Always keep slot since all slots are dropped at once after
                        // iteration.
                        return KEEP_SLOT;
                    },
        ...
        if (updating_mode_ == RememberedSetUpdatingMode::ALL) {
            if (chunk_->slot_set<OLD_TO_SHARED, AccessMode::NON_ATOMIC>()) {
                ...
                InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToShared(chunk_, InvalidatedSlotsFilter::LivenessCheck::kNo);
                RememberedSet<OLD_TO_SHARED>::Iterate(
                    chunk_, [&filter](MaybeObjectSlot slot) { return filter.IsValid(slot.address()) ? KEEP_SLOT : REMOVE_SLOT; }, SlotSet::FREE_EMPTY_BUCKETS);
            }
    ...
    void UpdateTypedPointers()
    {
        if (chunk_->typed_slot_set<OLD_TO_NEW, AccessMode::NON_ATOMIC>() != nullptr) {
            ...
            const auto check_and_update_old_to_new_slot_fn = [this](FullMaybeObjectSlot slot) { return CheckAndUpdateOldToNewSlot(slot); };
            RememberedSet<OLD_TO_NEW>::IterateTyped(chunk_, [this, &check_and_update_old_to_new_slot_fn](SlotType slot_type, Address slot) {
                SlotCallbackResult result = UpdateTypedSlotHelper::UpdateTypedSlot(heap_, slot_type, slot, check_and_update_old_to_new_slot_fn);
                // A new space string might have been promoted into the shared heap
                // during GC.
                if (record_old_to_shared_slots_) {
        ...
        if ((updating_mode_ == RememberedSetUpdatingMode::ALL) && (chunk_->typed_slot_set<OLD_TO_OLD, AccessMode::NON_ATOMIC>() != nullptr)) {
            CHECK_NE(chunk_->owner(), heap_->map_space());
            RememberedSet<OLD_TO_OLD>::IterateTyped(chunk_, [this](SlotType slot_type, Address slot) {
                // Using UpdateStrongSlot is OK here, because there are no weak
                // typed slots.
                PtrComprCageBase cage_base = heap_->isolate();
                SlotCallbackResult result = UpdateTypedSlotHelper::UpdateTypedSlot(heap_, slot_type, slot, [cage_base](FullMaybeObjectSlot slot) {
...
std::unique_ptr<UpdatingItem> MarkCompactCollector::CreateRememberedSetUpdatingItem(MemoryChunk* chunk, RememberedSetUpdatingMode updating_mode)
{
    return std::make_unique<RememberedSetUpdatingItem<NonAtomicMarkingState, GarbageCollector::MARK_COMPACTOR>>(heap(), non_atomic_marking_state(), chunk, updating_mode);
}

namespace {
template <typename IterateableSpace, typename Collector>
int CollectRememberedSetUpdatingItems(Collector* collector, std::vector<std::unique_ptr<UpdatingItem>>* items, IterateableSpace* space, RememberedSetUpdatingMode mode)
{
    int pages = 0;
    for (MemoryChunk* chunk : *space) {
        ...
        const bool contains_old_to_code_slots = V8_EXTERNAL_CODE_SPACE_BOOL && chunk->slot_set<OLD_TO_CODE>() != nullptr;
        const bool contains_old_to_new_slots = chunk->slot_set<OLD_TO_NEW>() != nullptr || chunk->typed_slot_set<OLD_TO_NEW>() != nullptr;
        const bool contains_old_to_shared_slots = chunk->slot_set<OLD_TO_SHARED>() != nullptr || chunk->typed_slot_set<OLD_TO_SHARED>() != nullptr;
        const bool contains_old_to_old_invalidated_slots = chunk->invalidated_slots<OLD_TO_OLD>() != nullptr;
        const bool contains_old_to_new_invalidated_slots = chunk->invalidated_slots<OLD_TO_NEW>() != nullptr;
        const bool contains_old_to_shared_invalidated_slots = chunk->invalidated_slots<OLD_TO_SHARED>() != nullptr;
        if (!contains_old_to_new_slots && !contains_old_to_old_slots && !contains_old_to_old_invalidated_slots && !contains_old_to_new_invalidated_slots
            && !contains_old_to_code_slots && !contains_old_to_shared_slots && !contains_old_to_shared_invalidated_slots)
            continue;
        if (mode == RememberedSetUpdatingMode::ALL || contains_old_to_new_slots || contains_old_to_old_invalidated_slots || contains_old_to_new_invalidated_slots) {
            items->emplace_back(collector->CreateRememberedSetUpdatingItem(chunk, mode));
            pages++;
        }
    }
    return pages;
...
void MarkCompactCollector::UpdatePointersAfterEvacuation()
{
    ...
    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_TO_NEW_ROOTS);
        // The external string table is updated at the end.
        PointersUpdatingVisitor updating_visitor(heap());
        heap_->IterateRootsIncludingClients(&updating_visitor, base::EnumSet<SkipRoot> { SkipRoot::kExternalStringTable });
    }

    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_CLIENT_HEAPS);
        UpdatePointersInClientHeaps();
    }

    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_SLOTS_MAIN);
        std::vector<std::unique_ptr<UpdatingItem>> updating_items;

        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->old_space(), RememberedSetUpdatingMode::ALL);
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->code_space(), RememberedSetUpdatingMode::ALL);
        if (heap()->shared_space()) {
            CollectRememberedSetUpdatingItems(this, &updating_items, heap()->shared_space(), RememberedSetUpdatingMode::ALL);
        }
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->lo_space(), RememberedSetUpdatingMode::ALL);
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->code_lo_space(), RememberedSetUpdatingMode::ALL);
        if (heap()->shared_lo_space()) {
            CollectRememberedSetUpdatingItems(this, &updating_items, heap()->shared_lo_space(), RememberedSetUpdatingMode::ALL);
        }
        if (heap()->map_space()) {
            CollectRememberedSetUpdatingItems(this, &updating_items, heap()->map_space(), RememberedSetUpdatingMode::ALL);
        }
        ...
        updating_items.push_back(std::make_unique<EphemeronTableUpdatingItem>(heap()));

        V8::GetCurrentPlatform()
            ->CreateJob(v8::TaskPriority::kUserBlocking,
                std::make_unique<PointersUpdatingJob>(
                    isolate(), std::move(updating_items), GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_PARALLEL, GCTracer::Scope::MC_BACKGROUND_EVACUATE_UPDATE_POINTERS))
            ->Join();
    }

    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MC_EVACUATE_UPDATE_POINTERS_WEAK);
...
void MarkCompactCollector::UpdatePointersInClientHeap(Isolate* client)
{
    ...
    while (chunk_iterator.HasNext()) {
        ...
        DCHECK_NULL(chunk->invalidated_slots<OLD_TO_SHARED>());
        RememberedSet<OLD_TO_SHARED>::Iterate(
            chunk, [cage_base](MaybeObjectSlot slot) { return UpdateOldToSharedSlot(cage_base, slot); }, SlotSet::FREE_EMPTY_BUCKETS);

        if (chunk->InYoungGeneration())
            chunk->ReleaseSlotSet<OLD_TO_SHARED>();

        RememberedSet<OLD_TO_SHARED>::IterateTyped(chunk, [this](SlotType slot_type, Address slot) {
            // Using UpdateStrongSlot is OK here, because there are no weak
            // typed slots.
            PtrComprCageBase cage_base = heap_->isolate();
            return UpdateTypedSlotHelper::UpdateTypedSlot(heap_, slot_type, slot, [cage_base](FullMaybeObjectSlot slot) { return UpdateStrongOldToSharedSlot(cage_base, slot); });
        });
        if (chunk->InYoungGeneration())
...
void MarkCompactCollector::ReportAbortedEvacuationCandidateDueToOOM(Address failed_start, Page* page)
{
    base::MutexGuard guard(&mutex_);
    aborted_evacuation_candidates_due_to_oom_.push_back(std::make_pair(failed_start, page));
}

void MarkCompactCollector::ReportAbortedEvacuationCandidateDueToFlags(Address failed_start, Page* page)
{
...
void ReRecordPage(Heap* heap, Address failed_start, Page* page)
{
    ...
    marking_state->bitmap(page)->ClearRange(page->AddressToMarkbitIndex(page->area_start()), page->AddressToMarkbitIndex(failed_start));

    // Remove outdated slots.
    RememberedSet<OLD_TO_NEW>::RemoveRange(page, page->address(), failed_start, SlotSet::FREE_EMPTY_BUCKETS);
    RememberedSet<OLD_TO_NEW>::RemoveRangeTyped(page, page->address(), failed_start);

    RememberedSet<OLD_TO_SHARED>::RemoveRange(page, page->address(), failed_start, SlotSet::FREE_EMPTY_BUCKETS);
    RememberedSet<OLD_TO_SHARED>::RemoveRangeTyped(page, page->address(), failed_start);

    // Remove invalidated slots.
    if (failed_start > page->area_start()) {
        InvalidatedSlotsCleanup old_to_new_cleanup = InvalidatedSlotsCleanup::OldToNew(page);
        old_to_new_cleanup.Free(page->area_start(), failed_start);

        InvalidatedSlotsCleanup old_to_shared_cleanup = InvalidatedSlotsCleanup::OldToShared(page);
    ...
    LiveObjectVisitor::VisitBlackObjectsNoFail(page, marking_state, &record_visitor);
...
{
    CHECK_IMPLIES(v8_flags.crash_on_aborted_evacuation, aborted_evacuation_candidates_due_to_oom_.empty());
...
void MarkCompactCollector::Sweep()
{
    ...
    {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_LO, ThreadKind::kMain);
        SweepLargeSpace(heap()->lo_space());
    }
    {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_CODE_LO, ThreadKind::kMain);
        SweepLargeSpace(heap()->code_lo_space());
    }
    {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_OLD, ThreadKind::kMain);
        StartSweepSpace(heap()->old_space());
    }
    {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_CODE, ThreadKind::kMain);
        StartSweepSpace(heap()->code_space());
    }
    if (heap()->map_space()) {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_MAP, ThreadKind::kMain);
    ...
    if (v8_flags.minor_mc && heap()->new_space()) {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MC_SWEEP_NEW, ThreadKind::kMain);
        StartSweepNewSpace();
    }
...
protected:
    ...
    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) override { VerifyHeapObjectImpl(rinfo->target_object(cage_base())); }
    void VerifyRootPointers(FullObjectSlot start, FullObjectSlot end) override { VerifyPointersImpl(start, end); }

private:
    V8_INLINE void VerifyHeapObjectImpl(HeapObject heap_object) { CHECK_IMPLIES(Heap::InYoungGeneration(heap_object), IsBlack(heap_object)); }
...
bool IsUnmarkedObjectForYoungGeneration(Heap* heap, FullObjectSlot p)
{
    ...
    return Heap::InYoungGeneration(*p) && !heap->non_atomic_marking_state()->IsBlack(HeapObject::cast(*p));
}

} // namespace

YoungGenerationMainMarkingVisitor::YoungGenerationMainMarkingVisitor(Isolate* isolate, MarkingState* marking_state, MarkingWorklists::Local* worklists_local)
    : YoungGenerationMarkingVisitorBase<YoungGenerationMainMarkingVisitor, MarkingState>(isolate, worklists_local)
    , marking_state_(marking_state)
{
}

bool YoungGenerationMainMarkingVisitor::ShouldVisit(HeapObject object) { return marking_state_->GreyToBlack(object); }
...
void MinorMarkCompactCollector::FinishConcurrentMarking()
{
    if (v8_flags.concurrent_marking) {
        DCHECK_EQ(heap()->concurrent_marking()->garbage_collector(), GarbageCollector::MINOR_MARK_COMPACTOR);
...
std::pair<size_t, size_t> MinorMarkCompactCollector::ProcessMarkingWorklist(size_t bytes_to_process)
{
...
void MinorMarkCompactCollector::VisitObject(HeapObject obj) { main_marking_visitor_->Visit(obj.map(), obj); }

void MinorMarkCompactCollector::SweepArrayBufferExtensions()
{
    TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_FINISH_SWEEP_ARRAY_BUFFERS);
...
public:
    YoungGenerationMigrationObserver(Heap* heap, MarkCompactCollector* mark_compact_collector)
        : MigrationObserver(heap)
...
class YoungGenerationRecordMigratedSlotVisitor final : public RecordMigratedSlotVisitor {
public:
    ...
    void VisitEmbeddedPointer(Code host, RelocInfo* rinfo) final { UNREACHABLE(); }
...
private:
    ...
    // full collector.
    inline bool IsLive(HeapObject object) { return heap_->non_atomic_marking_state()->IsBlack(object); }

    inline void RecordMigratedSlot(HeapObject host, MaybeObject value, Address slot) final
    {
        if (value->IsStrongOrWeak()) {
            BasicMemoryChunk* p = BasicMemoryChunk::FromAddress(value.ptr());
            if (p->InYoungGeneration()) {
...
{
    TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_EVACUATE_UPDATE_POINTERS);

    std::vector<std::unique_ptr<UpdatingItem>> updating_items;

    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_EVACUATE_UPDATE_POINTERS_SLOTS);
        // Create batches of global handles.
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->old_space(), RememberedSetUpdatingMode::OLD_TO_NEW_ONLY);
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->code_space(), RememberedSetUpdatingMode::OLD_TO_NEW_ONLY);
        if (heap()->map_space()) {
            CollectRememberedSetUpdatingItems(this, &updating_items, heap()->map_space(), RememberedSetUpdatingMode::OLD_TO_NEW_ONLY);
        }
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->lo_space(), RememberedSetUpdatingMode::OLD_TO_NEW_ONLY);
        CollectRememberedSetUpdatingItems(this, &updating_items, heap()->code_lo_space(), RememberedSetUpdatingMode::OLD_TO_NEW_ONLY);

        V8::GetCurrentPlatform()
            ->CreateJob(v8::TaskPriority::kUserBlocking,
                std::make_unique<PointersUpdatingJob>(isolate(), std::move(updating_items), GCTracer::Scope::MINOR_MC_EVACUATE_UPDATE_POINTERS_PARALLEL,
                    GCTracer::Scope::MINOR_MC_BACKGROUND_EVACUATE_UPDATE_POINTERS))
            ->Join();
    }

    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_EVACUATE_UPDATE_POINTERS_WEAK);

        // Update pointers from external string table.
        heap()->UpdateYoungReferencesInExternalStringTable(&UpdateReferenceInExternalStringTableEntry);
    }
}
...
public:
    ...
    void VisitRootPointer(Root root, const char* description, FullObjectSlot p) final { MarkObjectByPointer(p); }
...
{
    local_marking_worklists_ = std::make_unique<MarkingWorklists::Local>(&marking_worklists_);
    main_marking_visitor_ = std::make_unique<YoungGenerationMainMarkingVisitor>(heap()->isolate(), marking_state(), local_marking_worklists());
...
{
    {
        ...
        {
            // Keep new space sweeping atomic.
            GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MINOR_MC_SWEEP_FINISH_NEW, ThreadKind::kMain);
...
void MinorMarkCompactCollector::MakeIterable(Page* p, FreeSpaceTreatmentMode free_space_mode)
{
    CHECK(!p->IsLargePage());
    ...
    for (auto object_and_size : LiveObjectRange<kBlackObjects>(p, marking_state()->bitmap(p))) {
        HeapObject const object = object_and_size.first;
        ...
        if (free_end != free_start) {
            ...
            heap()->non_atomic_marking_state()->bitmap(p)->ClearRange(p->AddressToMarkbitIndex(free_start), p->AddressToMarkbitIndex(free_end));
    ...
    if (free_start != p->area_end()) {
        ...
        heap()->non_atomic_marking_state()->bitmap(p)->ClearRange(p->AddressToMarkbitIndex(free_start), p->AddressToMarkbitIndex(p->area_end()));
        if (free_space_mode == FreeSpaceTreatmentMode::kZapFreeSpace) {
            ZapCode(free_start, size);
        }
        p->heap()->CreateFillerObjectAt(free_start, static_cast<int>(size));
...
void MinorMarkCompactCollector::EvacuateEpilogue() { heap()->new_space()->EvacuateEpilogue(); }

std::unique_ptr<UpdatingItem> MinorMarkCompactCollector::CreateRememberedSetUpdatingItem(MemoryChunk* chunk, RememberedSetUpdatingMode updating_mode)
{
    return std::make_unique<RememberedSetUpdatingItem<NonAtomicMarkingState, GarbageCollector::MINOR_MARK_COMPACTOR>>(heap(), non_atomic_marking_state(), chunk, updating_mode);
}
...
public:
    YoungGenerationMarkingTask(Isolate* isolate, Heap* heap, MarkingWorklists* global_worklists)
        : marking_worklists_local_(std::make_unique<MarkingWorklists::Local>(global_worklists))
    ...
    {
        HeapObject object;
        while (marking_worklists_local_->Pop(&object) || marking_worklists_local_->PopOnHold(&object)) {
    ...
    MarkingWorklists::Local* marking_worklists_local() { return marking_worklists_local_.get(); }
...
{
    InvalidatedSlotsFilter filter = InvalidatedSlotsFilter::OldToNew(chunk_, InvalidatedSlotsFilter::LivenessCheck::kNo);
...
{
    RememberedSet<OLD_TO_NEW>::IterateTyped(chunk_, [=](SlotType slot_type, Address slot) {
        return UpdateTypedSlotHelper::UpdateTypedSlot(heap(), slot_type, slot, [this, task](FullMaybeObjectSlot slot) { return CheckAndMarkObject(task, slot); });
    });
}

template <typename TSlot> V8_INLINE SlotCallbackResult PageMarkingItem::CheckAndMarkObject(YoungGenerationMarkingTask* task, TSlot slot)
{
    static_assert(std::is_same<TSlot, FullMaybeObjectSlot>::value || std::is_same<TSlot, MaybeObjectSlot>::value, "Only FullMaybeObjectSlot and MaybeObjectSlot are expected here");
...
void YoungGenerationMarkingJob::Run(JobDelegate* delegate)
{
    if (delegate->IsJoiningThread()) {
        ...
        TRACE_GC_EPOCH(heap_->tracer(), GCTracer::Scope::MINOR_MC_BACKGROUND_MARKING, ThreadKind::kBackground);
        ProcessItems(delegate);
    }
}
...
size_t YoungGenerationMarkingJob::GetMaxConcurrency(size_t worker_count) const
{
    ...
    if (!incremental()) {
        num_tasks = std::max((items + 1) / kPagesPerTask,
            global_worklists_->shared()->Size() + global_worklists_->on_hold()->Size()); // TODO(v8:13012): If this is used with concurrent
    ...
    return std::min<size_t>(num_tasks, MinorMarkCompactCollector::kMaxParallelTasks);
}
...
void YoungGenerationMarkingJob::ProcessMarkingItems(YoungGenerationMarkingTask* task)
{
    ...
    while (remaining_marking_items_.load(std::memory_order_relaxed) > 0) {
        ...
        for (size_t i = *index; i < marking_items_.size(); ++i) {
            ...
            if (remaining_marking_items_.fetch_sub(1, std::memory_order_relaxed) <= 1) {
                return;
            }
        }
    }
...
void MinorMarkCompactCollector::MarkRootSetInParallel(RootMarkingVisitor* root_visitor, bool was_marked_incrementally)
{
    {
        ...
        // Seed the root set (roots + old->new set).
        {
            ...
            isolate()->global_handles()->ComputeWeaknessForYoungObjects(&JSObject::IsUnmodifiedApiObject);
            // MinorMC treats all weak roots except for global handles as strong.
            // That is why we don't set skip_weak = true here and instead visit
            // global handles separately.
            heap()->IterateRoots(root_visitor, base::EnumSet<SkipRoot> { SkipRoot::kExternalStringTable, SkipRoot::kGlobalHandles, SkipRoot::kOldGeneration });
            isolate()->global_handles()->IterateYoungStrongAndDependentRoots(root_visitor);

            if (!was_marked_incrementally) {
                // Create items for each page.
                RememberedSet<OLD_TO_NEW>::IterateMemoryChunks(heap(), [&marking_items](MemoryChunk* chunk) { marking_items.emplace_back(chunk); });
            }
        }
        ...
        // Add tasks and run in parallel.
        {
            ...
            V8::GetCurrentPlatform()
                ->CreateJob(v8::TaskPriority::kUserBlocking,
                    std::make_unique<YoungGenerationMarkingJob>(isolate(), heap(), marking_worklists(), std::move(marking_items), YoungMarkingJobType::kAtomic))
                ->Join();
...
void MinorMarkCompactCollector::MarkLiveObjects()
{
    ...
    bool was_marked_incrementally = false;
    {
        TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_MARK_FINISH_INCREMENTAL);
        if (heap_->incremental_marking()->Stop()) {
            MarkingBarrier::PublishAll(heap());
...

    {
        ...
        isolate()->global_handles()->ProcessWeakYoungObjects(&root_visitor, &IsUnmarkedObjectForYoungGeneration);
        DrainMarkingWorklist();
    }
...
void MinorMarkCompactCollector::TraceFragmentation()
{
    ...
    for (Page* p : PageRange(new_space->first_allocatable_address(), new_space->top())) {
        Address free_start = p->area_start();
        for (auto object_and_size : LiveObjectRange<kBlackObjects>(p, non_atomic_marking_state()->bitmap(p))) {
            HeapObject const object = object_and_size.first;
            Address free_end = object.address();
            if (free_end != free_start) {
        ...
        size_t area_end = p->Contains(new_space->top()) ? new_space->top() : p->area_end();
...
public:
    explicit YoungGenerationEvacuator(Heap* heap)
        : Evacuator(heap, &record_visitor_, &local_allocator_, AlwaysPromoteYoung::kNo)
        , record_visitor_(heap_)
        , local_allocator_(heap_, CompactionSpaceKind::kCompactionSpaceForMinorMarkCompact)
        , collector_(heap_->minor_mark_compact_collector())
    {
    }

    GCTracer::Scope::ScopeId GetBackgroundTracingScope() override { return GCTracer::Scope::MINOR_MC_BACKGROUND_EVACUATE_COPY; }

    GCTracer::Scope::ScopeId GetTracingScope() override { return GCTracer::Scope::MINOR_MC_EVACUATE_COPY_PARALLEL; }
...
bool YoungGenerationEvacuator::RawEvacuatePage(MemoryChunk* chunk, intptr_t* live_bytes)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("v8.gc"), "YoungGenerationEvacuator::RawEvacuatePage");
    NonAtomicMarkingState* marking_state = heap_->non_atomic_marking_state();
    *live_bytes = marking_state->live_bytes(chunk);
    ...
    if (!chunk->IsLargePage()) {
        if (heap()->ShouldZapGarbage()) {
            collector_->MakeIterable(static_cast<Page*>(chunk), FreeSpaceTreatmentMode::kZapFreeSpace);
            ...
            // updating.
            collector_->MakeIterable(static_cast<Page*>(chunk), FreeSpaceTreatmentMode::kIgnoreFreeSpace);
        }
    }
...
void MinorMarkCompactCollector::EvacuatePagesInParallel()
{
    ...
    for (Page* page : new_space_evacuation_pages_) {
        ...
        DCHECK_LT(0, live_bytes_on_page);
        live_bytes += live_bytes_on_page;
        if (ShouldMovePage(page, live_bytes_on_page, page->wasted_memory(), MemoryReductionMode::kNone, AlwaysPromoteYoung::kNo,
                heap()->tracer()->IsCurrentGCDueToAllocationFailure() ? PromoteUnusablePages::kYes : PromoteUnusablePages::kNo)) {
            EvacuateNewSpacePageVisitor<NEW_TO_OLD>::Move(page);
            evacuation_items.emplace_back(ParallelWorkItem {}, page);
        }
    }
    ...
    // Promote young generation large objects.
    for (auto it = heap()->new_lo_space()->begin(); it != heap()->new_lo_space()->end();) {
        LargePage* current = *it;
        it++;
        HeapObject object = current->GetObject();
    ...
    YoungGenerationMigrationObserver observer(heap(), heap()->mark_compact_collector());
    const auto pages_count = evacuation_items.size();
    const auto wanted_num_tasks = CreateAndExecuteEvacuationTasks<YoungGenerationEvacuator>(heap(), std::move(evacuation_items), &observer);
...
void MinorMarkCompactCollector::Sweep()
{
    ...
    TRACE_GC(heap()->tracer(), GCTracer::Scope::MINOR_MC_SWEEP);
    {
        GCTracer::Scope sweep_scope(heap()->tracer(), GCTracer::Scope::MINOR_MC_SWEEP_NEW, ThreadKind::kMain);
        StartSweepNewSpace();
    }
