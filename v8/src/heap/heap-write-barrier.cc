...
SimpleTlsPointerXp<MarkingBarrier> current_marking_barrier;
} // namespace

MarkingBarrier* WriteBarrier::CurrentMarkingBarrier(Heap* heap)
{
    return current_marking_barrier.Get() ? current_marking_barrier.Get() : heap->main_thread_local_heap()->marking_barrier();
}

void WriteBarrier::SetForThread(MarkingBarrier* marking_barrier)
{
    DCHECK_NULL(current_marking_barrier.Get());
    current_marking_barrier = marking_barrier;
}

void WriteBarrier::ClearForThread(MarkingBarrier* marking_barrier)
{
    DCHECK_EQ(current_marking_barrier.Get(), marking_barrier);
    current_marking_barrier = nullptr;
}

void WriteBarrier::MarkingSlow(Heap* heap, HeapObject host, HeapObjectSlot slot, HeapObject value)
{
...
// static
void WriteBarrier::MarkingSlowFromGlobalHandle(Heap* heap, HeapObject value) { heap->main_thread_local_heap()->marking_barrier()->WriteWithoutHost(value); }
...
void WriteBarrier::MarkingSlow(Heap* heap, Code host, RelocInfo* reloc_info, HeapObject value)
{
    MarkingBarrier* marking_barrier = CurrentMarkingBarrier(heap);
    marking_barrier->Write(host, reloc_info, value);
}

void WriteBarrier::SharedSlow(Heap* heap, Code host, RelocInfo* reloc_info, HeapObject value)
{
    ...
    RememberedSet<OLD_TO_SHARED>::InsertTyped(info.memory_chunk, info.slot_type, info.offset);
}

void WriteBarrier::MarkingSlow(Heap* heap, JSArrayBuffer host, ArrayBufferExtension* extension)
{
    MarkingBarrier* marking_barrier = CurrentMarkingBarrier(heap);
    marking_barrier->Write(host, extension);
}

void WriteBarrier::MarkingSlow(Heap* heap, DescriptorArray descriptor_array, int number_of_own_descriptors)
{
    MarkingBarrier* marking_barrier = CurrentMarkingBarrier(heap);
