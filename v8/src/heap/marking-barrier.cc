...
void MarkingBarrier::Write(HeapObject host, HeapObjectSlot slot, HeapObject value)
{
...
void MarkingBarrier::WriteWithoutHost(HeapObject value)
{
    ...
    if (is_minor() && !Heap::InYoungGeneration(value))
        return;
...
void MarkingBarrier::Write(JSArrayBuffer host, ArrayBufferExtension* extension)
{
    DCHECK(IsCurrentMarkingBarrier());
...
void MarkingBarrier::Write(DescriptorArray descriptor_array, int number_of_own_descriptors)
{
    DCHECK(IsCurrentMarkingBarrier());
    DCHECK(IsReadOnlyHeapObject(descriptor_array.map()));

    if (is_minor() && !heap_->InYoungGeneration(descriptor_array))
        return;
    ...
    if (!marking_state_.IsBlack(descriptor_array)) {
        ...
        MarkRange(descriptor_array, descriptor_array.GetFirstPointerSlot(), descriptor_array.GetDescriptorSlot(0));
    }
    ...
    const int16_t old_marked = is_minor() ? 0 : descriptor_array.UpdateNumberOfMarkedDescriptors(major_collector_->epoch(), number_of_own_descriptors);
    if (old_marked < number_of_own_descriptors) {
        ...
        MarkRange(
            descriptor_array, MaybeObjectSlot(descriptor_array.GetDescriptorSlot(old_marked)), MaybeObjectSlot(descriptor_array.GetDescriptorSlot(number_of_own_descriptors)));
    }
}

void MarkingBarrier::RecordRelocSlot(Code host, RelocInfo* rinfo, HeapObject target)
{
    DCHECK(IsCurrentMarkingBarrier());
...
// static
void MarkingBarrier::ActivateAll(Heap* heap, bool is_compacting, MarkingBarrierType marking_barrier_type)
{
    heap->safepoint()->IterateLocalHeaps(
        [is_compacting, marking_barrier_type](LocalHeap* local_heap) { local_heap->marking_barrier()->Activate(is_compacting, marking_barrier_type); });
}
...
{
    heap->safepoint()->IterateLocalHeaps([](LocalHeap* local_heap) { local_heap->marking_barrier()->Publish(); });
}
...
void MarkingBarrier::Publish()
{
    if (is_activated_) {
        ...
        if (!typed_slots_map_.empty()) {
            optional_rwx_write_scope.emplace("Merging typed slots may require allocating a new typed slot set.");
        }
...
void MarkingBarrier::Activate(bool is_compacting, MarkingBarrierType marking_barrier_type)
{
...
bool MarkingBarrier::IsCurrentMarkingBarrier() { return WriteBarrier::CurrentMarkingBarrier(heap_) == this; }
