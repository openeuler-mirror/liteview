...
base::LazyMutex process_wide_code_range_creation_mutex_ = LAZY_MUTEX_INITIALIZER;
...
// remain.
base::LazyInstance<std::weak_ptr<CodeRange>>::type process_wide_code_range_ = LAZY_INSTANCE_INITIALIZER;
...
Address CodeRangeAddressHint::GetAddressHint(size_t code_range_size, size_t alignment)
{
    ...
    if (it == recently_freed_.end() || it->second.empty()) {
        if (V8_ENABLE_NEAR_CODE_RANGE_BOOL && !preferred_region.is_empty()) {
            auto memory_ranges = base::OS::GetFreeMemoryRangesWithin(preferred_region.begin(), preferred_region.end(), code_range_size, alignment);
...
// static
size_t CodeRange::GetWritableReservedAreaSize() { return kReservedCodeRangePages * MemoryAllocator::GetCommitPageSize(); }

bool CodeRange::InitReservation(v8::PageAllocator* page_allocator, size_t requested)
{
    ...
    //    big aligment.
    const size_t base_alignment = V8_EXTERNAL_CODE_SPACE_BOOL ? base::bits::RoundUpToPowerOfTwo(requested) : VirtualMemoryCage::ReservationParams::kAnyBaseAlignment;
    ...
    params.requested_start_hint = GetCodeRangeAddressHint()->GetAddressHint(requested, allocate_page_size);
    params.jit = v8_flags.jitless ? JitPermission::kNoJit : JitPermission::kMapAsJittable;
    ...
    CHECK_EQ(ExternalCodeCompressionScheme::GetPtrComprCageBaseAddress(base), ExternalCodeCompressionScheme::GetPtrComprCageBaseAddress(last));
    ...
    // for details.
    if (reserved_area > 0) {
        if (!reservation()->SetPermissions(reservation()->address(), reserved_area, PageAllocator::kReadWrite)) {
            return false;
        }
    }
    if (V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT && params.jit == JitPermission::kMapAsJittable) {
        void* base = reinterpret_cast<void*>(page_allocator_->begin());
        size_t size = page_allocator_->size();
        CHECK(params.page_allocator->SetPermissions(base, size, PageAllocator::kReadWriteExecute));
...
uint8_t* CodeRange::RemapEmbeddedBuiltins(Isolate* isolate, const uint8_t* embedded_blob_code, size_t embedded_blob_code_size)
{
    ...
    embedded_blob_code_copy = reinterpret_cast<uint8_t*>(page_allocator()->AllocatePages(hint, allocate_code_size, kAllocatePageSize, PageAllocator::kNoAccess));
    ...
    if (code_region.size() > max_pc_relative_code_range) {
        ...
        if (code_region.contains(unreachable_start)) {
            ...
            void* result = page_allocator()->AllocatePages(reinterpret_cast<void*>(unreachable_start), unreachable_size, kAllocatePageSize, PageAllocator::kNoAccess);
...
// static
std::shared_ptr<CodeRange> CodeRange::EnsureProcessWideCodeRange(v8::PageAllocator* page_allocator, size_t requested_size)
{
...
// static
std::shared_ptr<CodeRange> CodeRange::GetProcessWideCodeRange() { return process_wide_code_range_.Get().lock(); }
