...
size_t MemoryChunkLayout::CodePageGuardStartOffset()
{
    ...
    return ::RoundUp(MemoryChunk::kHeaderSize + Bitmap::kSize, MemoryAllocator::GetCommitPageSize());
}

size_t MemoryChunkLayout::CodePageGuardSize() { return MemoryAllocator::GetCommitPageSize(); }
...
intptr_t MemoryChunkLayout::ObjectEndOffsetInCodePage()
{
    ...
    // non-writable.
    return MemoryChunk::kPageSize - static_cast<int>(MemoryAllocator::GetCommitPageSize());
}
...
intptr_t MemoryChunkLayout::ObjectStartOffsetInDataPage() { return RoundUp(MemoryChunk::kHeaderSize + Bitmap::kSize, ALIGN_TO_ALLOCATION_ALIGNMENT(kDoubleSize)); }

size_t MemoryChunkLayout::ObjectStartOffsetInMemoryChunk(AllocationSpace space)
{
...
size_t MemoryChunkLayout::AllocatableMemoryInMemoryChunk(AllocationSpace space)
{
    if (space == CODE_SPACE) {
