...
inline std::unordered_map<Address, std::unique_ptr<MarkingWorklist::Local>> GetLocalPerContextMarkingWorklists(bool is_per_context_mode, MarkingWorklists* global)
{
    if (!is_per_context_mode)
        return {};

    std::unordered_map<Address, std::unique_ptr<MarkingWorklist::Local>> worklist_by_context;
    worklist_by_context.reserve(global->context_worklists().size());
    for (auto& cw : global->context_worklists()) {
        worklist_by_context[cw.context] = std::make_unique<MarkingWorklist::Local>(*cw.worklist.get());
    }
...
MarkingWorklists::Local::Local(MarkingWorklists* global, std::unique_ptr<CppMarkingState> cpp_marking_state)
    ...
    , worklist_by_context_(GetLocalPerContextMarkingWorklists(is_per_context_mode_, global))
    , other_(*global->other())
...
bool MarkingWorklists::Local::IsEmpty()
{
    ...
    for (auto& cw : worklist_by_context_) {
        if (cw.first != active_context_ && !(cw.second->IsLocalEmpty() && cw.second->IsGlobalEmpty())) {
...
Address MarkingWorklists::Local::SwitchToSharedForTesting() { return SwitchToContext(kSharedContext); }
