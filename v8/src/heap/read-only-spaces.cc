...
void ReadOnlyArtifacts::set_read_only_heap(std::unique_ptr<ReadOnlyHeap> read_only_heap) { read_only_heap_ = std::move(read_only_heap); }
...
ReadOnlyHeap* SingleCopyReadOnlyArtifacts::GetReadOnlyHeapForIsolate(Isolate* isolate) { return read_only_heap(); }

void SingleCopyReadOnlyArtifacts::Initialize(Isolate* isolate, std::vector<ReadOnlyPage*>&& pages, const AllocationStats& stats)
{
    ...
    // shared cage.
    page_allocator_ = COMPRESS_POINTERS_IN_SHARED_CAGE_BOOL ? isolate->page_allocator() : GetPlatformPageAllocator();
    pages_ = std::move(pages);
    set_accounting_stats(stats);
    set_shared_read_only_space(std::make_unique<SharedReadOnlySpace>(isolate->heap(), this));
}

void SingleCopyReadOnlyArtifacts::ReinstallReadOnlySpace(Isolate* isolate) { isolate->heap()->ReplaceReadOnlySpace(shared_read_only_space()); }

void SingleCopyReadOnlyArtifacts::VerifyHeapAndSpaceRelationships(Isolate* isolate)
{
...
{
    auto isolate_ro_roots = isolate->roots_table().read_only_roots_begin().location();
...
{
    auto isolate_ro_roots = isolate->roots_table().read_only_roots_begin().location();
    CopyAndRebaseRoots(read_only_roots_, isolate_ro_roots, isolate->isolate_root());
}

SharedReadOnlySpace* PointerCompressedReadOnlyArtifacts::CreateReadOnlySpace(Isolate* isolate)
{
    ...
    for (size_t i = 0; i < pages_.size(); ++i) {
        ...
        bool success = isolate->heap()->memory_allocator()->data_page_allocator()->ReserveForSharedMemoryMapping(reinterpret_cast<void*>(new_address), page->size());
        CHECK(success);
    ...
    auto* shared_read_only_space = new SharedReadOnlySpace(isolate->heap(), std::move(pages), std::move(mappings), std::move(new_stats));
    return shared_read_only_space;
}

ReadOnlyHeap* PointerCompressedReadOnlyArtifacts::GetReadOnlyHeapForIsolate(Isolate* isolate)
{
    ...
    for (Object original_object : original_cache) {
        ...
        Address new_address = isolate_root + V8HeapCompressionScheme::CompressTagged(original_address);
...
std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping> PointerCompressedReadOnlyArtifacts::RemapPageTo(size_t i, Address new_address, ReadOnlyPage*& new_page)
{
    std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping> mapping = shared_memory_[i]->RemapTo(reinterpret_cast<void*>(new_address));
    if (mapping) {
...
void PointerCompressedReadOnlyArtifacts::Initialize(Isolate* isolate, std::vector<ReadOnlyPage*>&& pages, const AllocationStats& stats)
{
    DCHECK(ReadOnlyHeap::IsReadOnlySpaceShared());
    DCHECK(pages_.empty());
    DCHECK(!pages.empty());
    ...
    for (const ReadOnlyPage* page : pages) {
        ...
        Tagged_t compressed_address = V8HeapCompressionScheme::CompressTagged(page->address());
...
void PointerCompressedReadOnlyArtifacts::ReinstallReadOnlySpace(Isolate* isolate)
{
    ...
    auto* ro_heap = new ReadOnlyHeap(isolate->read_only_heap(), isolate->heap()->read_only_space());
    isolate->set_read_only_heap(ro_heap);

    DCHECK_NE(*isolate->roots_table().read_only_roots_begin().location(), 0);
}

void PointerCompressedReadOnlyArtifacts::VerifyHeapAndSpaceRelationships(Isolate* isolate)
{
    ...
    DCHECK_EQ(read_only_heap()->read_only_space(), shared_read_only_space());
    DCHECK_EQ(isolate->read_only_heap()->read_only_space(), isolate->heap()->read_only_space());
}
...
void ReadOnlySpace::DetachPagesAndAddToArtifacts(std::shared_ptr<ReadOnlyArtifacts> artifacts)
{
    DCHECK(ReadOnlyHeap::IsReadOnlySpaceShared());
    ...
    Seal(COMPRESS_POINTERS_IN_ISOLATE_CAGE_BOOL ? SealMode::kDetachFromHeap : SealMode::kDetachFromHeapAndUnregisterMemory);
    artifacts->Initialize(heap->isolate(), std::move(pages_), accounting_stats_);
}

ReadOnlyPage::ReadOnlyPage(Heap* heap, BaseSpace* space, size_t chunk_size, Address area_start, Address area_end, VirtualMemory reservation)
...
void ReadOnlySpace::SetPermissionsForPages(MemoryAllocator* memory_allocator, PageAllocator::Permission access)
{
    for (BasicMemoryChunk* chunk : pages_) {
        ...
        v8::PageAllocator* page_allocator = memory_allocator->page_allocator(NOT_EXECUTABLE);
        CHECK(SetPermissions(page_allocator, chunk->address(), chunk->size(), access));
    }
}
...
public:
    ReadOnlySpaceObjectIterator(const Heap* heap, const ReadOnlySpace* space, BasicMemoryChunk* chunk)
...
protected:
    void VerifyPointers(HeapObject host, MaybeObjectSlot start, MaybeObjectSlot end) override
    {
        if (!host.is_null()) {
...
void ReadOnlySpace::FreeLinearAllocationArea()
{
    ...
    heap()->marking_state()->bitmap(page)->ClearRange(page->AddressToMarkbitIndex(top_), page->AddressToMarkbitIndex(limit_));
...
void ReadOnlySpace::EnsureSpaceForAllocation(int size_in_bytes)
{
    ...
    BasicMemoryChunk* chunk = heap()->memory_allocator()->AllocateReadOnlyPage(this);
    capacity_ += AreaSize();
    ...
    heap()->CreateFillerObjectAt(chunk->area_start(), static_cast<int>(chunk->area_size()));
...
HeapObject ReadOnlySpace::TryAllocateLinearlyAligned(int size_in_bytes, AllocationAlignment alignment)
{
...
{
    AllocationResult result = USE_ALLOCATION_ALIGNMENT_BOOL && alignment != kTaggedAligned ? AllocateRawAligned(size_in_bytes, alignment) : AllocateRawUnaligned(size_in_bytes);
    HeapObject heap_obj;
...
size_t ReadOnlyPage::ShrinkToHighWaterMark()
{
    ...
    if (unused > 0) {
        ...
        heap()->memory_allocator()->PartialFreeMemory(this, address() + size() - unused, unused, area_end() - unused);
        if (filler.address() != area_end()) {
            CHECK(filler.IsFreeSpaceOrFiller());
...
SharedReadOnlySpace::SharedReadOnlySpace(Heap* heap, PointerCompressedReadOnlyArtifacts* artifacts)
...
SharedReadOnlySpace::SharedReadOnlySpace(
    Heap* heap, std::vector<ReadOnlyPage*>&& new_pages, std::vector<std::unique_ptr<::v8::PageAllocator::SharedMemoryMapping>>&& mappings, AllocationStats&& new_stats)
    : SharedReadOnlySpace(heap)
{
    DCHECK(V8_SHARED_RO_HEAP_BOOL);
    DCHECK(COMPRESS_POINTERS_BOOL);
...
SharedReadOnlySpace::SharedReadOnlySpace(Heap* heap, SingleCopyReadOnlyArtifacts* artifacts)
    : SharedReadOnlySpace(heap)
{
