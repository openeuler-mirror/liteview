...
public:
    ...
    PagedSpaceObjectIterator(Heap* heap, const PagedSpaceBase* space, const Page* page);
    PagedSpaceObjectIterator(Heap* heap, const PagedSpace* space, const Page* page, Address start_address);
...
public:
    ...
    PagedSpaceBase(Heap* heap, AllocationSpace id, Executability executable, FreeList* free_list, AllocationCounter& allocation_counter, LinearAllocationArea& allocation_info,
        LinearAreaOriginalData& linear_area_original_data, CompactionSpaceKind compaction_space_kind = CompactionSpaceKind::kNone);
    ...
    // thread.
    V8_WARN_UNUSED_RESULT base::Optional<std::pair<Address, size_t>> RawAllocateBackground(
    ...
    void DecreaseAllocatedBytes(size_t bytes, Page* page) { accounting_stats_.DecreaseAllocatedBytes(bytes, page); }
    void IncreaseAllocatedBytes(size_t bytes, Page* page) { accounting_stats_.IncreaseAllocatedBytes(bytes, page); }
    void DecreaseCapacity(size_t bytes) { accounting_stats_.DecreaseCapacity(bytes); }
    void IncreaseCapacity(size_t bytes) { accounting_stats_.IncreaseCapacity(bytes); }
    ...
    bool is_compaction_space() const { return compaction_space_kind_ != CompactionSpaceKind::kNone; }

    CompactionSpaceKind compaction_space_kind() const { return compaction_space_kind_; }
    ...
    Page* first_page() override { return reinterpret_cast<Page*>(memory_chunk_list_.front()); }
    const Page* first_page() const override { return reinterpret_cast<const Page*>(memory_chunk_list_.front()); }

    Page* last_page() override { return reinterpret_cast<Page*>(memory_chunk_list_.back()); }
    const Page* last_page() const override { return reinterpret_cast<const Page*>(memory_chunk_list_.back()); }
    ...
    void ReduceActiveSystemPages(Page* page, ActiveSystemPages active_system_pages);

    // Allocates memory with the given size constraints from the space's free
    // list.
    V8_WARN_UNUSED_RESULT base::Optional<std::pair<Address, size_t>> TryAllocationFromFreeListBackground(
        size_t min_size_in_bytes, size_t max_size_in_bytes, AllocationOrigin origin);
    ...
    base::Optional<std::pair<Address, size_t>> TryExpandBackground(size_t size_in_bytes);
...
private:
    ...
    bool SupportsAllocationObserver() const override { return !is_compaction_space(); }
...
protected:
    ...
    bool EnsureAllocation(int size_in_bytes, AllocationAlignment alignment, AllocationOrigin origin, int* out_max_aligned_size) override;

    V8_WARN_UNUSED_RESULT bool TryAllocationFromFreeListMain(size_t size_in_bytes, AllocationOrigin origin);

    V8_WARN_UNUSED_RESULT bool ContributeToSweepingMain(int required_freed_bytes, int max_pages, int size_in_bytes, AllocationOrigin origin);
    ...
    // collecting garbage.
    V8_WARN_UNUSED_RESULT virtual bool RefillLabMain(int size_in_bytes, AllocationOrigin origin);
    ...
    V8_WARN_UNUSED_RESULT bool RawRefillLabMain(int size_in_bytes, AllocationOrigin origin);

    V8_WARN_UNUSED_RESULT bool TryExpand(int size_in_bytes, AllocationOrigin origin);

    size_t committed_physical_memory() const { return committed_physical_memory_.load(std::memory_order_relaxed); }
...
public:
    // Creates a space with an id.
    PagedSpace(Heap* heap, AllocationSpace id, Executability executable, FreeList* free_list, LinearAllocationArea& allocation_info,
        CompactionSpaceKind compaction_space_kind = CompactionSpaceKind::kNone)
        : PagedSpaceBase(heap, id, executable, free_list, allocation_counter_, allocation_info, linear_area_original_data_, compaction_space_kind)
    {
    }
...
public:
    CompactionSpace(Heap* heap, AllocationSpace id, Executability executable, CompactionSpaceKind compaction_space_kind)
        : PagedSpace(heap, id, executable, FreeList::CreateFreeList(), allocation_info_, compaction_space_kind)
    {
...
protected:
    V8_WARN_UNUSED_RESULT bool RefillLabMain(int size_in_bytes, AllocationOrigin origin) override;
...
public:
    explicit CompactionSpaceCollection(Heap* heap, CompactionSpaceKind compaction_space_kind)
        : old_space_(heap, OLD_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind)
        , map_space_(heap, MAP_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind)
        , code_space_(heap, CODE_SPACE, Executability::EXECUTABLE, compaction_space_kind)
        , shared_space_(heap, SHARED_SPACE, Executability::NOT_EXECUTABLE, compaction_space_kind)
    {
    }
...
public:
    ...
    explicit OldSpace(Heap* heap, LinearAllocationArea& allocation_info)
        : PagedSpace(heap, OLD_SPACE, NOT_EXECUTABLE, FreeList::CreateFreeList(), allocation_info)
    {
    }

    static bool IsAtPageStart(Address addr) { return static_cast<intptr_t>(addr & kPageAlignmentMask) == MemoryChunkLayout::ObjectStartOffsetInDataPage(); }
...
public:
    ...
    explicit CodeSpace(Heap* heap)
        : PagedSpace(heap, CODE_SPACE, EXECUTABLE, FreeList::CreateFreeList(), paged_allocation_info_)
    {
    }
...
public:
    ...
    explicit MapSpace(Heap* heap)
        : PagedSpace(heap, MAP_SPACE, NOT_EXECUTABLE, FreeList::CreateFreeList(), paged_allocation_info_)
    {
    }
...
public:
    ...
    explicit SharedSpace(Heap* heap)
        : PagedSpace(heap, SHARED_SPACE, NOT_EXECUTABLE, FreeList::CreateFreeList(), allocation_info)
    {
    }
...
private:
    enum State { kOldSpaceState, kMapState, kCodeState, kLargeObjectState, kCodeLargeObjectState, kFinishedState };
    Heap* const heap_;
    State state_;
