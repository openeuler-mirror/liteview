...
StackMemory::~StackMemory()
{
    ...
    if (owned_)
        allocator->DecommitPages(limit_, size_);
    ...
    , owned_(true)
{
    ...
    limit_ = static_cast<byte*>(allocator->AllocatePages(nullptr, size_, allocator->AllocatePageSize(), PageAllocator::kReadWrite));
    if (v8_flags.trace_wasm_stack_switching) {
        PrintF("Allocate stack #%d (limit: %p, base: %p)\n", id_, limit_, limit_ + size_);
    }
}
