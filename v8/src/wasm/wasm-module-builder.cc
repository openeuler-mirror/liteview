...
void FixupSection(ZoneBuffer* buffer, size_t start) { buffer->patch_u32v(start, static_cast<uint32_t>(buffer->offset() - start - kPaddedVarInt32Size)); }
...
void WasmFunctionBuilder::EmitGetLocal(uint32_t local_index) { EmitWithU32V(kExprLocalGet, local_index); }

void WasmFunctionBuilder::EmitSetLocal(uint32_t local_index) { EmitWithU32V(kExprLocalSet, local_index); }

void WasmFunctionBuilder::EmitTeeLocal(uint32_t local_index) { EmitWithU32V(kExprLocalTee, local_index); }

void WasmFunctionBuilder::EmitCode(const byte* code, uint32_t code_size) { body_.write(code, code_size); }
...
void WasmFunctionBuilder::EmitWithU8U8(WasmOpcode opcode, const byte imm1, const byte imm2)
{
    body_.write_u8(opcode);
    body_.write_u8(imm1);
    body_.write_u8(imm2);
...
void WasmFunctionBuilder::EmitValueType(ValueType type) { WriteValueType(&body_, type); }

void WasmFunctionBuilder::EmitI32Const(int32_t value) { EmitWithI32V(kExprI32Const, value); }
...
void WasmFunctionBuilder::SetName(base::Vector<const char> name) { name_ = name; }
...
void WasmFunctionBuilder::WriteSignature(ZoneBuffer* buffer) const { buffer->write_u32v(signature_index_); }
...
void WasmModuleBuilder::AddDataSegment(const byte* data, uint32_t size, uint32_t dest)
{
...
uint32_t WasmModuleBuilder::ForceAddSignature(const FunctionSig* sig, uint32_t supertype)
{
...
uint32_t WasmModuleBuilder::AddSignature(const FunctionSig* sig, uint32_t supertype)
{
...
uint32_t WasmModuleBuilder::AddStructType(StructType* type, uint32_t supertype)
{
    uint32_t index = static_cast<uint32_t>(types_.size());
...
uint32_t WasmModuleBuilder::AddTable(ValueType type, uint32_t min_size, uint32_t max_size)
{
    tables_.push_back({ type, min_size, max_size, true, {} });
    return static_cast<uint32_t>(tables_.size() - 1);
}

uint32_t WasmModuleBuilder::AddTable(ValueType type, uint32_t min_size, uint32_t max_size, WasmInitExpr init)
{
    tables_.push_back({ type, min_size, max_size, true, init });
    return static_cast<uint32_t>(tables_.size() - 1);
}

void WasmModuleBuilder::AddElementSegment(WasmElemSegment segment) { element_segments_.push_back(std::move(segment)); }

void WasmModuleBuilder::SetIndirectFunction(uint32_t table_index, uint32_t index_in_table, uint32_t direct_function_index, WasmElemSegment::FunctionIndexingMode indexing_mode)
{
...
uint32_t WasmModuleBuilder::AddImport(base::Vector<const char> name, FunctionSig* sig, base::Vector<const char> module)
{
...
void WasmModuleBuilder::MarkStartFunction(WasmFunctionBuilder* function) { start_function_index_ = function->func_index(); }
...
#endif
    exports_.push_back({ name, kExternalFunction, import_index - static_cast<int>(function_imports_.size()) });
}
...
void WasmModuleBuilder::SetMinMemorySize(uint32_t value) { min_memory_size_ = value; }
...
namespace {
void WriteInitializerExpressionWithEnd(ZoneBuffer* buffer, const WasmInitExpr& init, ValueType type)
{
    switch (init.kind()) {
    ...
    case WasmInitExpr::kI31New:
        WriteInitializerExpressionWithEnd(buffer, (*init.operands())[0], kWasmI32);
...
void WriteInitializerExpression(ZoneBuffer* buffer, const WasmInitExpr& init, ValueType type)
{
...
void WasmModuleBuilder::WriteTo(ZoneBuffer* buffer) const
{
    ...
    if (tables_.size() > 0) {
        ...
        for (const WasmTable& table : tables_) {
            ...
            if (table.has_maximum)
                buffer->write_size(table.max_size);
    ...
    // == Emit memory declaration ================================================
    {
        ...
        if (has_shared_memory_) {
            buffer->write_u8(has_max_memory_size_ ? kSharedWithMaximum : kSharedNoMaximum);
        } else {
            ...
            for (const WasmElemSegment::Entry entry : segment.entries) {
                uint8_t opcode = entry.kind == WasmElemSegment::Entry::kGlobalGetEntry ? kExprGlobalGet
                    : entry.kind == WasmElemSegment::Entry::kRefFuncEntry              ? kExprRefFunc
                                                                                       : kExprRefNull;
                bool needs_function_offset = segment.indexing_mode == WasmElemSegment::kRelativeToDeclaredFunctions && entry.kind == WasmElemSegment::Entry::kRefFuncEntry;
