...
public:
    ...
    virtual bool ProcessModuleHeader(base::Vector<const uint8_t> bytes, uint32_t offset) = 0;
    ...
    virtual void ProcessFunctionBody(base::Vector<const uint8_t> bytes, uint32_t offset) = 0;
...
public:
    ...
    void SetMoreFunctionsCanBeSerializedCallback(MoreFunctionsCanBeSerializedCallback callback) { more_functions_can_be_serialized_callback_ = std::move(callback); }
    ...
    void SetCompiledModuleBytes(base::Vector<const uint8_t> bytes) { compiled_module_bytes_ = bytes; }
    ...
    void SetUrl(base::Vector<const char> url) { url_->assign(url.begin(), url.size()); }
    ...
    static std::unique_ptr<StreamingDecoder> CreateSyncStreamingDecoder(
        Isolate* isolate, const WasmFeatures& enabled, Handle<Context> context, const char* api_method_name_for_errors, std::shared_ptr<CompilationResultResolver> resolver);
