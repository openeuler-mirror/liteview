...
public:
    struct CacheKey {
        CacheKey(const compiler::WasmImportCallKind& kind, uint32_t canonical_type_index, int expected_arity, Suspend suspend)
            : kind(kind)
            , canonical_type_index(canonical_type_index)
            , expected_arity(expected_arity == kDontAdaptArgumentsSentinel ? 0 : expected_arity)
            , suspend(suspend)
        {
        }
        ...
        {
            return kind == rhs.kind && canonical_type_index == rhs.canonical_type_index && expected_arity == rhs.expected_arity && suspend == rhs.suspend;
        }
    ...
    class CacheKeyHash {
    public:
        size_t operator()(const CacheKey& key) const { return base::hash_combine(static_cast<uint8_t>(key.kind), key.canonical_type_index, key.expected_arity); }
    };
    ...
    V8_EXPORT_PRIVATE WasmCode* Get(compiler::WasmImportCallKind kind, uint32_t canonical_type_index, int expected_arity, Suspend suspend) const;
    // Thread-safe. Returns nullptr if the key doesn't exist in the map.
    WasmCode* MaybeGet(compiler::WasmImportCallKind kind, uint32_t canonical_type_index, int expected_arity, Suspend suspend) const;
