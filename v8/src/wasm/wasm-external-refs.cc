...
void f32_trunc_wrapper(Address data) { WriteUnalignedValue<float>(data, truncf(ReadUnalignedValue<float>(data))); }

void f32_floor_wrapper(Address data) { WriteUnalignedValue<float>(data, floorf(ReadUnalignedValue<float>(data))); }

void f32_ceil_wrapper(Address data) { WriteUnalignedValue<float>(data, ceilf(ReadUnalignedValue<float>(data))); }
...
void f64_trunc_wrapper(Address data) { WriteUnalignedValue<double>(data, trunc(ReadUnalignedValue<double>(data))); }

void f64_floor_wrapper(Address data) { WriteUnalignedValue<double>(data, floor(ReadUnalignedValue<double>(data))); }

void f64_ceil_wrapper(Address data) { WriteUnalignedValue<double>(data, ceil(ReadUnalignedValue<double>(data))); }
...
uint32_t word32_ctz_wrapper(Address data) { return base::bits::CountTrailingZeros(ReadUnalignedValue<uint32_t>(data)); }

uint32_t word64_ctz_wrapper(Address data) { return base::bits::CountTrailingZeros(ReadUnalignedValue<uint64_t>(data)); }

uint32_t word32_popcnt_wrapper(Address data) { return base::bits::CountPopulation(ReadUnalignedValue<uint32_t>(data)); }

uint32_t word64_popcnt_wrapper(Address data) { return base::bits::CountPopulation(ReadUnalignedValue<uint64_t>(data)); }
...
void f64x2_ceil_wrapper(Address data) { simd_float_round_wrapper<double, &ceil>(data); }

void f64x2_floor_wrapper(Address data) { simd_float_round_wrapper<double, &floor>(data); }

void f64x2_trunc_wrapper(Address data) { simd_float_round_wrapper<double, &trunc>(data); }

void f64x2_nearest_int_wrapper(Address data) { simd_float_round_wrapper<double, &nearbyint>(data); }

void f32x4_ceil_wrapper(Address data) { simd_float_round_wrapper<float, &ceilf>(data); }

void f32x4_floor_wrapper(Address data) { simd_float_round_wrapper<float, &floorf>(data); }

void f32x4_trunc_wrapper(Address data) { simd_float_round_wrapper<float, &truncf>(data); }

void f32x4_nearest_int_wrapper(Address data) { simd_float_round_wrapper<float, &nearbyintf>(data); }
...
inline byte* EffectiveAddress(WasmInstanceObject instance, uintptr_t index) { return instance.memory_start() + index; }
...
int32_t memory_init_wrapper(Address data)
{
    ...
    byte* seg_start = reinterpret_cast<byte*>(instance.data_segment_starts().get(seg_index));
...
int32_t memory_copy_wrapper(Address data)
{
    ...
    if (!base::IsInBounds<uint64_t>(dst, size, mem_size))
        return kOutOfBounds;
    if (!base::IsInBounds<uint64_t>(src, size, mem_size))
        return kOutOfBounds;

    // Use std::memmove, because the ranges can overlap.
    std::memmove(EffectiveAddress(instance, dst), EffectiveAddress(instance, src), size);
    return kSuccess;
}
...
int32_t memory_fill_wrapper(Address data)
{
    ...
    WasmInstanceObject instance = WasmInstanceObject::cast(raw_instance);
    uintptr_t dst = ReadAndIncrementOffset<uintptr_t>(data, &offset);
    uint8_t value = static_cast<uint8_t>(ReadAndIncrementOffset<uint32_t>(data, &offset));
    uintptr_t size = ReadAndIncrementOffset<uintptr_t>(data, &offset);
...
inline void* ArrayElementAddress(WasmArray array, uint32_t index, int element_size_bytes) { return ArrayElementAddress(array.ptr(), index, element_size_bytes); }
} // namespace

void array_copy_wrapper(Address raw_instance, Address raw_dst_array, uint32_t dst_index, Address raw_src_array, uint32_t src_index, uint32_t length)
{
    DCHECK_GT(length, 0);
    ThreadNotInWasmScope thread_not_in_wasm_scope;
    DisallowGarbageCollection no_gc;
    ...
    if (element_type.is_reference()) {
        WasmInstanceObject instance = WasmInstanceObject::cast(Object(raw_instance));
...
void array_fill_with_zeroes_wrapper(Address raw_array, uint32_t length, uint32_t element_size_bytes)
{
    ThreadNotInWasmScope thread_not_in_wasm_scope;
    DisallowGarbageCollection no_gc;
    std::memset(ArrayElementAddress(raw_array, 0, element_size_bytes), 0, length * element_size_bytes);
}

static WasmTrapCallbackForTesting wasm_trap_callback_for_testing = nullptr;

void set_trap_callback_for_testing(WasmTrapCallbackForTesting callback) { wasm_trap_callback_for_testing = callback; }
