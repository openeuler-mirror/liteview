...
public:
    LogCodesTask(base::Mutex* mutex, LogCodesTask** task_slot, Isolate* isolate, WasmEngine* engine)
        : mutex_(mutex)
...
public:
    explicit WeakScriptHandle(Handle<Script> script)
...
std::shared_ptr<NativeModule> NativeModuleCache::MaybeGetNativeModule(ModuleOrigin origin, base::Vector<const uint8_t> wire_bytes)
{
    if (!v8_flags.wasm_native_module_cache_enabled)
        return nullptr;
...
std::shared_ptr<NativeModule> NativeModuleCache::Update(std::shared_ptr<NativeModule> native_module, bool error)
{
    ...
    if (!error) {
        ...
        // the map.
        auto p = map_.emplace(key, base::Optional<std::weak_ptr<NativeModule>>(native_module));
        USE(p);
        DCHECK(p.second);
    }
    cache_cv_.NotifyAll();
...
bool WasmEngine::SyncValidate(Isolate* isolate, const WasmFeatures& enabled, const ModuleWireBytes& bytes, std::string* error_message)
{
    TRACE_EVENT0("v8.wasm", "wasm.SyncValidate");
    // TODO(titzer): remove dependency on the isolate.
    if (bytes.start() == nullptr || bytes.length() == 0) {
        if (error_message)
            *error_message = "empty module wire bytes";
        return false;
    }
    auto result = DecodeWasmModule(enabled, bytes.start(), bytes.end(), true, kWasmOrigin, isolate->counters(), isolate->metrics_recorder(),
        isolate->GetOrRegisterRecorderContextId(isolate->native_context()), DecodingMethod::kSync, allocator());
...
MaybeHandle<AsmWasmData> WasmEngine::SyncCompileTranslatedAsmJs(Isolate* isolate, ErrorThrower* thrower, const ModuleWireBytes& bytes,
    base::Vector<const byte> asm_js_offset_table_bytes, Handle<HeapNumber> uses_bitset, LanguageMode language_mode)
{
    ...
    ModuleResult result = DecodeWasmModule(
        WasmFeatures::ForAsmjs(), bytes.start(), bytes.end(), false, origin, isolate->counters(), isolate->metrics_recorder(), context_id, DecodingMethod::kSync, allocator());
    if (result.failed()) {
    ...
    std::shared_ptr<NativeModule> native_module = CompileToNativeModule(isolate, WasmFeatures::ForAsmjs(), thrower, std::move(result).value(), bytes, compilation_id, context_id);
    if (!native_module)
        return {};
...
Handle<WasmModuleObject> WasmEngine::FinalizeTranslatedAsmJs(Isolate* isolate, Handle<AsmWasmData> asm_wasm_data, Handle<Script> script)
{
...
MaybeHandle<WasmModuleObject> WasmEngine::SyncCompile(Isolate* isolate, const WasmFeatures& enabled, ErrorThrower* thrower, const ModuleWireBytes& bytes)
{
    int compilation_id = next_compilation_id_.fetch_add(1);
    ...
    {
        ModuleResult result = DecodeWasmModule(
            enabled, bytes.start(), bytes.end(), false, kWasmOrigin, isolate->counters(), isolate->metrics_recorder(), context_id, DecodingMethod::kSync, allocator());
        if (result.failed()) {
    ...
    // Transfer ownership of the WasmModule to the {Managed<WasmModule>} generated
    // in {CompileToNativeModule}.
    std::shared_ptr<NativeModule> native_module = CompileToNativeModule(isolate, enabled, thrower, std::move(module), bytes, compilation_id, context_id);
    if (!native_module)
        return {};
    ...
    Handle<Script> script = GetOrCreateScript(isolate, native_module, kNoSourceUrl);
...
void WasmEngine::AsyncInstantiate(Isolate* isolate, std::unique_ptr<InstantiationResultResolver> resolver, Handle<WasmModuleObject> module_object, MaybeHandle<JSReceiver> imports)
{
...
void WasmEngine::AsyncCompile(Isolate* isolate, const WasmFeatures& enabled, std::shared_ptr<CompilationResultResolver> resolver, const ModuleWireBytes& bytes, bool is_shared,
    const char* api_method_name_for_errors)
{
    ...
    if (v8_flags.wasm_test_streaming) {
        std::shared_ptr<StreamingDecoder> streaming_decoder
            = StartStreamingCompilation(isolate, enabled, handle(isolate->context(), isolate), api_method_name_for_errors, std::move(resolver));
    ...
    AsyncCompileJob* job = CreateAsyncCompileJob(
        isolate, enabled, std::move(copy), bytes.length(), handle(isolate->context(), isolate), api_method_name_for_errors, std::move(resolver), compilation_id);
    job->Start();
}

std::shared_ptr<StreamingDecoder> WasmEngine::StartStreamingCompilation(
    Isolate* isolate, const WasmFeatures& enabled, Handle<Context> context, const char* api_method_name, std::shared_ptr<CompilationResultResolver> resolver)
{
    int compilation_id = next_compilation_id_.fetch_add(1);
    TRACE_EVENT1("v8.wasm", "wasm.StartStreamingCompilation", "id", compilation_id);
    if (v8_flags.wasm_async_compilation) {
        AsyncCompileJob* job = CreateAsyncCompileJob(isolate, enabled, std::unique_ptr<byte[]>(nullptr), 0, context, api_method_name, std::move(resolver), compilation_id);
        return job->CreateStreamingDecoder();
    }
    return StreamingDecoder::CreateSyncStreamingDecoder(isolate, enabled, context, api_method_name, std::move(resolver));
}

void WasmEngine::CompileFunction(Isolate* isolate, NativeModule* native_module, uint32_t function_index, ExecutionTier tier)
{
    // Note we assume that "one-off" compilations can discard detected features.
    WasmFeatures detected = WasmFeatures::None();
    WasmCompilationUnit::CompileWasmFunction(isolate, native_module, &detected, &native_module->module()->functions[function_index], tier);
}
...
std::shared_ptr<NativeModule> WasmEngine::ExportNativeModule(Handle<WasmModuleObject> module_object) { return module_object->shared_native_module(); }
...
Handle<Script> CreateWasmScript(Isolate* isolate, std::shared_ptr<NativeModule> native_module, base::Vector<const char> source_url)
{
    ...
    if (debug_symbols.type == WasmDebugSymbols::Type::SourceMap && !debug_symbols.external_url.is_empty()) {
        base::Vector<const char> external_url = ModuleWireBytes(wire_bytes).GetNameOrNull(debug_symbols.external_url);
    ...
    script->set_wasm_weak_instance_list(ReadOnlyRoots(isolate).empty_weak_array_list());
    return script;
}
} // namespace

Handle<WasmModuleObject> WasmEngine::ImportNativeModule(Isolate* isolate, std::shared_ptr<NativeModule> shared_native_module, base::Vector<const char> source_url)
{
    NativeModule* native_module = shared_native_module.get();
    ModuleWireBytes wire_bytes(native_module->wire_bytes());
    Handle<Script> script = GetOrCreateScript(isolate, shared_native_module, source_url);
...
AsyncCompileJob* WasmEngine::CreateAsyncCompileJob(Isolate* isolate, const WasmFeatures& enabled, std::unique_ptr<byte[]> bytes_copy, size_t length, Handle<Context> context,
    const char* api_method_name, std::shared_ptr<CompilationResultResolver> resolver, int compilation_id)
{
    Handle<Context> incumbent_context = isolate->GetIncumbentContext();
    AsyncCompileJob* job = new AsyncCompileJob(isolate, enabled, std::move(bytes_copy), length, context, incumbent_context, api_method_name, std::move(resolver), compilation_id);
...
OperationsBarrier::Token WasmEngine::StartWrapperCompilation(Isolate* isolate)
{
    ...
    if (isolate_info_it == isolates_.end())
        return {};
...
void WasmEngine::LogCode(base::Vector<WasmCode*> code_vec)
{
    ...
    for (Isolate* isolate : native_modules_[native_module]->isolates) {
        ...
        if (info->log_codes_task == nullptr) {
            auto new_task = std::make_unique<LogCodesTask>(&mutex_, &info->log_codes_task, isolate, this);
...
std::shared_ptr<NativeModule> WasmEngine::NewNativeModule(Isolate* isolate, const WasmFeatures& enabled, std::shared_ptr<const WasmModule> module, size_t code_size_estimate)
{
#ifdef V8_ENABLE_WASM_GDB_REMOTE_DEBUGGING
    ...
    std::shared_ptr<NativeModule> native_module = GetWasmCodeManager()->NewNativeModule(isolate, enabled, code_size_estimate, std::move(module));
    base::MutexGuard lock(&mutex_);
    ...
    auto pair = native_modules_.insert(std::make_pair(native_module.get(), std::make_unique<NativeModuleInfo>(native_module)));
...
std::shared_ptr<NativeModule> WasmEngine::MaybeGetNativeModule(ModuleOrigin origin, base::Vector<const uint8_t> wire_bytes, Isolate* isolate)
{
    TRACE_EVENT1("v8.wasm", "wasm.GetNativeModuleFromCache", "wire_bytes", wire_bytes.size());
    std::shared_ptr<NativeModule> native_module = native_module_cache_.MaybeGetNativeModule(origin, wire_bytes);
...
bool WasmEngine::UpdateNativeModuleCache(bool error, std::shared_ptr<NativeModule>* native_module, Isolate* isolate)
{
    ...
    *native_module = native_module_cache_.Update(*native_module, error);
    ...
    // Potentially recompile the module for tier down, after releasing the mutex.
    if (recompile_module)
        native_module->get()->RecompileForTiering();
    return false;
}
...
void WasmEngine::StreamingCompilationFailed(size_t prefix_hash) { native_module_cache_.StreamingCompilationFailed(prefix_hash); }
...
namespace {
void ReportLiveCodeFromFrameForGC(StackFrame* frame, std::unordered_set<wasm::WasmCode*>& live_wasm_code)
{
    ...
    if (WasmFrame::cast(frame)->wasm_code()->for_debugging()) {
        Address osr_target = base::Memory<Address>(WasmFrame::cast(frame)->fp() - kOSRTargetOffset);
        if (osr_target) {
...
bool WasmEngine::AddPotentiallyDeadCode(WasmCode* code)
{
    ...
    if (v8_flags.wasm_code_gc) {
        ...
        if (new_potentially_dead_code_size_ > dead_code_limit) {
            bool inc_gc_count = info->num_code_gcs_triggered < std::numeric_limits<int8_t>::max();
            if (current_gc_info_ == nullptr) {
                if (inc_gc_count)
                    ++info->num_code_gcs_triggered;
...
std::shared_ptr<OperationsBarrier> WasmEngine::GetBarrierForBackgroundCompile() { return operations_barrier_; }
...
void WasmEngine::SampleThrowEvent(Isolate* isolate)
{
    ...
    throw_count = std::min(throw_count + 1, isolate->counters()->wasm_throw_count()->max());
    isolate->counters()->wasm_throw_count()->AddSample(throw_count);
    SampleExceptionEvent(&isolate_info->throw_timer, isolate->counters()->wasm_time_between_throws());
}
...
void WasmEngine::SampleRethrowEvent(Isolate* isolate)
{
    ...
    // To avoid an int overflow, clip the count to the histogram's max value.
    rethrow_count = std::min(rethrow_count + 1, isolate->counters()->wasm_rethrow_count()->max());
    isolate->counters()->wasm_rethrow_count()->AddSample(rethrow_count);
    SampleExceptionEvent(&isolate_info->rethrow_timer, isolate->counters()->wasm_time_between_rethrows());
}
...
void WasmEngine::SampleCatchEvent(Isolate* isolate)
{
    ...
    // To avoid an int overflow, clip the count to the histogram's max value.
    catch_count = std::min(catch_count + 1, isolate->counters()->wasm_catch_count()->max());
    isolate->counters()->wasm_catch_count()->AddSample(catch_count);
    SampleExceptionEvent(&isolate_info->catch_timer, isolate->counters()->wasm_time_between_catch());
}
...
void WasmEngine::TriggerGC(int8_t gc_sequence_index)
{
    ...
    TRACE_CODE_GC("Starting GC (nr %d). Number of potentially dead code objects: %zu\n", current_gc_info_->gc_sequence_index, current_gc_info_->dead_code.size());
...
uint32_t max_table_init_entries() { return std::min(uint32_t { kV8MaxWasmTableInitEntries }, v8_flags.wasm_max_table_size.value()); }
