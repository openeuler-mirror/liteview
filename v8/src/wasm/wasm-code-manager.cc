...
#include "patch_code/simple_tls.h" // SUPPORT_XP_CODE
...
base::AddressRegion DisjointAllocationPool::Allocate(size_t size) { return AllocateInRegion(size, { kNullAddress, std::numeric_limits<size_t>::max() }); }
...
Address WasmCode::handler_table() const { return instruction_start() + handler_table_offset_; }
...
Address WasmCode::code_comments() const { return instruction_start() + code_comments_offset_; }
...
std::unique_ptr<const byte[]> WasmCode::ConcatenateBytes(std::initializer_list<base::Vector<const byte>> vectors)
{
    size_t total_size = 0;
...
bool WasmCode::ShouldBeLogged(Isolate* isolate)
{
    ...
    return isolate->v8_file_logger()->is_listening_to_code_events() || isolate->logger()->is_listening_to_code_events() || isolate->is_profiling();
}
...
std::string WasmCode::DebugName() const
{
    ...
    if (kind() == kWasmToJsWrapper) {
        ...
        size_t sig_length = PrintSignature(base::VectorOf(&name_buffer[prefix_len], kMaxSigLength), sig);
...
void WasmCode::LogCode(Isolate* isolate, const char* source_url, int script_id) const
{
    ...
    if (!source_map && debug_symbols.type == WasmDebugSymbols::Type::SourceMap && !debug_symbols.external_url.is_empty() && load_wasm_source_map) {
        WasmName external_url = wire_bytes.GetNameOrNull(debug_symbols.external_url);
...
void WasmCode::Disassemble(const char* name, std::ostream& os, Address current_pc) const
{
    ...
    if (protected_instructions_size_ > 0) {
        ...
        for (auto& data : protected_instructions()) {
            os << std::setw(10) << std::hex << data.instr_offset << std::setw(10) << std::hex << data.landing_offset << "\n";
        }
        os << "\n";
    }
    ...
    if (!source_positions().empty()) {
        ...
        for (SourcePositionTableIterator it(source_positions()); !it.done(); it.Advance()) {
            os << std::setw(10) << std::hex << it.code_offset() << std::dec << std::setw(10) << it.source_position().ScriptOffset() << (it.is_statement() ? "  statement" : "")
               << "\n";
        }
        os << "\n";
    }
    ...
    os << "Instructions (size = " << instruction_size << ", " << static_cast<void*>(instructions().begin()) << "-" << static_cast<void*>(instructions().begin() + instruction_size)
       << ")\n";
...
int WasmCode::GetSourcePositionBefore(int offset)
{
    ...
    for (SourcePositionTableIterator iterator(source_positions()); !iterator.done() && iterator.code_offset() < offset; iterator.Advance()) {
...
WasmCodeAllocator::WasmCodeAllocator(std::shared_ptr<Counters> async_counters)
    : protect_code_memory_(!V8_HAS_PTHREAD_JIT_WRITE_PROTECT && v8_flags.wasm_write_protect_code_memory && !WasmCodeManager::MemoryProtectionKeysEnabled())
    , async_counters_(std::move(async_counters))
{
    owned_code_space_.reserve(4);
}

WasmCodeAllocator::~WasmCodeAllocator() { GetWasmCodeManager()->FreeNativeModule(base::VectorOf(owned_code_space_), committed_code_space()); }
...
int NumWasmFunctionsInFarJumpTable(uint32_t num_declared_functions) { return NativeModule::kNeedsFarJumpsBetweenCodeSpaces ? static_cast<int>(num_declared_functions) : 0; }
...
size_t OverheadPerCodeSpace(uint32_t num_declared_functions)
{
    ...
    overhead += RoundUp<kCodeAlignment>(JumpTableAssembler::SizeForNumberOfFarJumpSlots(WasmCode::kRuntimeStubCount, NumWasmFunctionsInFarJumpTable(num_declared_functions)));
...
size_t ReservationSize(size_t code_size_estimate, int num_declared_functions, size_t total_reserved)
{
    ...
    size_t reserve_size = std::min(WasmCodeAllocator::kMaxCodeSpaceSize, suggested_size);
...
public:
    CheckWritableMemoryRegions(std::set<base::AddressRegion, base::AddressRegion::StartAddressLess>& writable_memory, base::AddressRegion new_region, size_t& new_writable_memory)
        : writable_memory_(writable_memory)
        , new_region_(new_region)
        , new_writable_memory_(new_writable_memory)
        , old_writable_size_(
              std::accumulate(writable_memory_.begin(), writable_memory_.end(), size_t { 0 }, [](size_t old, base::AddressRegion region) { return old + region.size(); }))
    {
    }
    ...
    ~CheckWritableMemoryRegions()
    {
        ...
        DCHECK(std::any_of(writable_memory_.begin(), writable_memory_.end(), [this](auto region) { return region.contains(new_region_); }));

        // The new total size of writable memory must have increased by
        // {new_writable_memory}.
        size_t total_writable_size = std::accumulate(writable_memory_.begin(), writable_memory_.end(), size_t { 0 }, [](size_t old, auto region) { return old + region.size(); });
        DCHECK_EQ(old_writable_size_ + new_writable_memory_, total_writable_size);

        // There are no empty regions.
        DCHECK(std::none_of(writable_memory_.begin(), writable_memory_.end(), [](auto region) { return region.is_empty(); }));
        ...
        USE(std::accumulate(writable_memory_.begin(), writable_memory_.end(), Address { 0 }, [](Address previous_end, auto region) {
...
private:
    const std::set<base::AddressRegion, base::AddressRegion::StartAddressLess>& writable_memory_;
...
class CheckWritableMemoryRegions {
public:
    template <typename... Args> explicit CheckWritableMemoryRegions(Args...) { }
};
#endif // !DEBUG
...
base::Vector<byte> WasmCodeAllocator::AllocateForCode(NativeModule* native_module, size_t size) { return AllocateForCodeInRegion(native_module, size, kUnrestrictedRegion); }

base::Vector<byte> WasmCodeAllocator::AllocateForCodeInRegion(NativeModule* native_module, size_t size, base::AddressRegion region)
{
    DCHECK_LT(0, size);
    ...
    if (V8_UNLIKELY(code_space.is_empty())) {
        ...
        Address hint = owned_code_space_.empty() ? kNullAddress : owned_code_space_.back().end();

        size_t total_reserved = 0;
        for (auto& vmem : owned_code_space_)
            total_reserved += vmem.size();
        size_t reserve_size = ReservationSize(size, native_module->module()->num_declared_functions, total_reserved);
        VirtualMemory new_mem = code_manager->TryAllocate(reserve_size, reinterpret_cast<void*>(hint));
    ...
    if (commit_start < commit_end) {
        ...
        if (protect_code_memory_) {
            ...
            InsertIntoWritableRegions({ commit_start, commit_end - commit_start }, false);
        }
    }
...
void WasmCodeAllocator::RemoveWriter()
{
    ...
    for (base::AddressRegion writable : writable_memory_) {
        for (base::AddressRegion split_range : SplitRangeByReservationsIfNeeded(writable, owned_code_space_)) {
            TRACE_HEAP("Set 0x%" V8PRIxPTR ":0x%" V8PRIxPTR " to RX\n", split_range.begin(), split_range.end());
            CHECK(SetPermissions(page_allocator, split_range.begin(), split_range.size(), PageAllocator::kReadExecute));
        }
    }
...
size_t WasmCodeAllocator::GetNumCodeSpaces() const { return owned_code_space_.size(); }

void WasmCodeAllocator::InsertIntoWritableRegions(base::AddressRegion region, bool switch_to_writable)
{
    size_t new_writable_memory = 0;

    CheckWritableMemoryRegions check_on_return { writable_memory_, region, new_writable_memory };

    v8::PageAllocator* page_allocator = GetPlatformPageAllocator();
    // Subroutine to make a non-writable region writable (if {switch_to_writable}
    // is {true}) and insert it into {writable_memory_}.
    auto make_writable = [&](decltype(writable_memory_)::iterator insert_pos, base::AddressRegion region) {
        new_writable_memory += region.size();
        if (switch_to_writable) {
            for (base::AddressRegion split_range : SplitRangeByReservationsIfNeeded(region, owned_code_space_)) {
                TRACE_HEAP("Set 0x%" V8PRIxPTR ":0x%" V8PRIxPTR " to RWX\n", split_range.begin(), split_range.end());
                CHECK(SetPermissions(page_allocator, split_range.begin(), split_range.size(), PageAllocator::kReadWriteExecute));
            }
        }
        ...
        if (insert_pos != writable_memory_.end() && region.end() == insert_pos->begin()) {
    ...
    for (;; ++it) {
        ...
        if (it->end() <= region.begin())
            continue; // Continue after {it}.
        base::AddressRegion overlap = it->GetOverlap(region);
        DCHECK(!overlap.is_empty());
        if (overlap.begin() == region.begin()) {
            if (overlap.end() == region.end())
                return; // Fully contained already.
        ...
        base::AddressRegion split = { region.begin(), overlap.begin() - region.begin() };
...
BoundsCheckStrategy GetBoundsChecks(const WasmModule* module)
{
    if (!v8_flags.wasm_bounds_checks)
        return kNoBoundsChecks;
    if (v8_flags.wasm_enforce_bounds_checks)
...
NativeModule::NativeModule(const WasmFeatures& enabled, DynamicTiering dynamic_tiering, VirtualMemory code_space, std::shared_ptr<const WasmModule> module,
    std::shared_ptr<Counters> async_counters, std::shared_ptr<NativeModule>* shared_this)
    : engine_scope_(GetWasmEngine()->GetBarrierForBackgroundCompile()->TryLock())
    , code_allocator_(async_counters)
    , enabled_features_(enabled)
    , module_(std::move(module))
    , import_wrapper_cache_(std::unique_ptr<WasmImportWrapperCache>(new WasmImportWrapperCache()))
    , bounds_checks_(GetBoundsChecks(module_.get()))
{
    ...
    compilation_state_ = CompilationState::New(*shared_this, std::move(async_counters), dynamic_tiering);
    compilation_state_->InitCompileJob();
    DCHECK_NOT_NULL(module_);
    if (module_->num_declared_functions > 0) {
        code_table_ = std::make_unique<WasmCode*[]>(module_->num_declared_functions);
        tiering_budgets_ = std::make_unique<uint32_t[]>(module_->num_declared_functions);
...
void NativeModule::ReserveCodeTableForTesting(uint32_t max_functions)
{
    ...
    main_jump_table_ = CreateEmptyJumpTableInRegionLocked(JumpTableAssembler::SizeForNumberOfSlots(max_functions), single_code_space_region);
...
void NativeModule::LogWasmCodes(Isolate* isolate, Script script)
{
    ...
    std::unique_ptr<char[]> source_url = url_obj.IsString() ? String::cast(url_obj).ToCString() : nullptr;
...
{
    return { module(), bounds_checks_, kRuntimeExceptionSupport, enabled_features_, compilation_state()->dynamic_tiering() };
}
...
WasmCode* NativeModule::AddCodeForTesting(Handle<Code> code)
{
    ...
    if (relocation_size > 0) {
        reloc_info = base::OwnedVector<byte>::Of(base::Vector<byte> { code->relocation_start(), relocation_size });
    }
    Handle<ByteArray> source_pos_table(code->source_position_table(), code->GetIsolate());
    base::OwnedVector<byte> source_pos = base::OwnedVector<byte>::NewForOverwrite(source_pos_table->length());
    ...
    base::Vector<const byte> instructions(reinterpret_cast<byte*>(code->raw_body_start()), static_cast<size_t>(code->raw_body_size()));
    ...
    intptr_t delta = reinterpret_cast<Address>(dst_code_bytes.begin()) - code->raw_instruction_start();
    ...
    for (RelocIterator it(dst_code_bytes, reloc_info.as_vector(), constant_pool_start, mode_mask); !it.done(); it.next(), orig_it.next()) {
        RelocInfo::Mode mode = it.rinfo()->rmode();
        if (RelocInfo::IsWasmStubCall(mode)) {
            ...
            Address entry = GetNearRuntimeStubEntry(static_cast<WasmCode::RuntimeStubId>(stub_call_tag), jump_tables_ref);
...
void NativeModule::UseLazyStub(uint32_t func_index)
{
    ...
    if (!lazy_compile_table_) {
        ...
        WasmCodeRefScope code_ref_scope;
        lazy_compile_table_ = CreateEmptyJumpTableLocked(JumpTableAssembler::SizeForNumberOfLazyFunctions(num_slots));
        Address compile_lazy_address
            = GetNearRuntimeStubEntry(WasmCode::kWasmCompileLazy, FindJumpTablesForRegionLocked(base::AddressRegionOf(lazy_compile_table_->instructions())));
        JumpTableAssembler::GenerateLazyCompileTable(lazy_compile_table_->instruction_start(), num_slots, module_->num_imported_functions, compile_lazy_address);
    }
...
std::unique_ptr<WasmCode> NativeModule::AddCode(int index, const CodeDesc& desc, int stack_slots, uint32_t tagged_parameter_slots,
    base::Vector<const byte> protected_instructions_data, base::Vector<const byte> source_position_table, WasmCode::Kind kind, ExecutionTier tier, ForDebugging for_debugging)
{
    ...
    return AddCodeWithCodeSpace(
        index, desc, stack_slots, tagged_parameter_slots, protected_instructions_data, source_position_table, kind, tier, for_debugging, code_space, jump_table_ref);
}

std::unique_ptr<WasmCode> NativeModule::AddCodeWithCodeSpace(int index, const CodeDesc& desc, int stack_slots, uint32_t tagged_parameter_slots,
    base::Vector<const byte> protected_instructions_data, base::Vector<const byte> source_position_table, WasmCode::Kind kind, ExecutionTier tier, ForDebugging for_debugging,
    base::Vector<uint8_t> dst_code_bytes, const JumpTablesRef& jump_tables)
{
    base::Vector<byte> reloc_info { desc.buffer + desc.buffer_size - desc.reloc_size, static_cast<size_t>(desc.reloc_size) };
    ...
    memcpy(dst_code_bytes.begin(), desc.buffer, static_cast<size_t>(desc.instr_size));
    ...
    for (RelocIterator it(dst_code_bytes, reloc_info, constant_pool_start, mode_mask); !it.done(); it.next()) {
        RelocInfo::Mode mode = it.rinfo()->rmode();
        if (RelocInfo::IsWasmCall(mode)) {
            ...
            DCHECK_LT(stub_call_tag, WasmCode::kRuntimeStubCount);
            Address entry = GetNearRuntimeStubEntry(static_cast<WasmCode::RuntimeStubId>(stub_call_tag), jump_tables);
            it.rinfo()->set_wasm_stub_call_address(entry, SKIP_ICACHE_FLUSH);
        } else {
            it.rinfo()->apply(delta);
        }
    ...
    std::unique_ptr<WasmCode> code { new WasmCode { this, index, dst_code_bytes, stack_slots, tagged_parameter_slots, safepoint_table_offset, handler_table_offset,
        constant_pool_offset, code_comments_offset, instr_size, protected_instructions_data, reloc_info, source_position_table, kind, tier, for_debugging } };
...
WasmCode* NativeModule::PublishCodeLocked(std::unique_ptr<WasmCode> owned_code)
{
    ...
    if (V8_UNLIKELY(cached_code_))
        InsertToCodeCache(code);
    ...
    // with a higher tier.
    static_assert(kForDebugging > kNoDebugging && kWithBreakpoints > kForDebugging, "for_debugging is ordered");
    const bool update_code_table =
        ...
        && (!prior_code
            || (tiering_state_ == kTieredDown
                    ...
                    : (prior_code->tier() < code->tier() || (prior_code->for_debugging() && !code->for_debugging()))));
...
std::unique_ptr<WasmCode> NativeModule::AddDeserializedCode(int index, base::Vector<byte> instructions, int stack_slots, uint32_t tagged_parameter_slots,
    ...
    base::Vector<const byte> protected_instructions_data, base::Vector<const byte> reloc_info, base::Vector<const byte> source_position_table, WasmCode::Kind kind,
    ExecutionTier tier)
{
    UpdateCodeSize(instructions.size(), tier, kNoDebugging);

    return std::unique_ptr<WasmCode> { new WasmCode { this, index, instructions, stack_slots, tagged_parameter_slots, safepoint_table_offset, handler_table_offset,
        constant_pool_offset, code_comments_offset, unpadded_binary_size, protected_instructions_data, reloc_info, source_position_table, kind, tier, kNoDebugging } };
}
...
void NativeModule::SetWasmSourceMap(std::unique_ptr<WasmModuleSourceMap> source_map) { source_map_ = std::move(source_map); }

WasmModuleSourceMap* NativeModule::GetWasmSourceMap() const { return source_map_.get(); }

WasmCode* NativeModule::CreateEmptyJumpTableLocked(int jump_table_size) { return CreateEmptyJumpTableInRegionLocked(jump_table_size, kUnrestrictedRegion); }

WasmCode* NativeModule::CreateEmptyJumpTableInRegionLocked(int jump_table_size, base::AddressRegion region)
{
    allocation_mutex_.AssertHeld();
...
void NativeModule::PatchJumpTableLocked(const CodeSpaceData& code_space_data, uint32_t slot_index, Address target)
{
    allocation_mutex_.AssertHeld();
    ...
    if (code_space_data.jump_table->instructions().end() == code_space_data.far_jump_table->instructions().begin()) {
        base::Vector<uint8_t> jump_tables_space = base::VectorOf(
            code_space_data.jump_table->instructions().begin(), code_space_data.jump_table->instructions().size() + code_space_data.far_jump_table->instructions().size());
        code_allocator_.MakeWritable(AddressRegionOf(jump_tables_space));
    } else {
        code_allocator_.MakeWritable(AddressRegionOf(code_space_data.jump_table->instructions()));
        code_allocator_.MakeWritable(AddressRegionOf(code_space_data.far_jump_table->instructions()));
    }
    ...
    uint32_t far_jump_table_offset = JumpTableAssembler::FarJumpSlotIndexToOffset(WasmCode::kRuntimeStubCount + slot_index);
    ...
    JumpTableAssembler::PatchJumpTableSlot(jump_table_slot, far_jump_table_slot, target);
}
...
void NativeModule::AddCodeSpaceLocked(base::AddressRegion region)
{
    ...
    if (WasmCodeManager::CanRegisterUnwindInfoForNonABICompliantCodeRange()) {
        ...
        DCHECK_LT(0, size);
        base::Vector<byte> padding = code_allocator_.AllocateForCodeInRegion(this, size, region);
    ...
    if (needs_jump_table) {
        jump_table = CreateEmptyJumpTableInRegionLocked(JumpTableAssembler::SizeForNumberOfSlots(num_wasm_functions), region);
    ...
    if (needs_far_jump_table) {
        ...
        far_jump_table = CreateEmptyJumpTableInRegionLocked(
            JumpTableAssembler::SizeForNumberOfFarJumpSlots(WasmCode::kRuntimeStubCount, NumWasmFunctionsInFarJumpTable(num_function_slots)), region);
...
#define RUNTIME_STUB_TRAP(Name) RUNTIME_STUB(ThrowWasm##Name)
        Builtin stub_names[WasmCode::kRuntimeStubCount] = { WASM_RUNTIME_STUB_LIST(RUNTIME_STUB, RUNTIME_STUB_TRAP) };
#undef RUNTIME_STUB
#undef RUNTIME_STUB_TRAP
        ...
        JumpTableAssembler::GenerateFarJumpTable(far_jump_table->instruction_start(), builtin_addresses, WasmCode::kRuntimeStubCount, num_function_slots);
    }
    ...
    if (jump_table && !is_first_code_space) {
        ...
        for (uint32_t slot_index = 0; slot_index < num_wasm_functions; ++slot_index) {
            if (code_table_[slot_index]) {
                PatchJumpTableLocked(new_code_space_data, slot_index, code_table_[slot_index]->instruction_start());
            } else if (lazy_compile_table_) {
                Address lazy_compile_target = lazy_compile_table_->instruction_start() + JumpTableAssembler::LazyCompileSlotIndexToOffset(slot_index);
                PatchJumpTableLocked(new_code_space_data, slot_index, lazy_compile_target);
            }
        }
    }
}
...
public:
    ...
    base::Vector<const uint8_t> GetCode(WireBytesRef ref) const final { return std::atomic_load(&wire_bytes_)->as_vector().SubVector(ref.offset(), ref.end_offset()); }

    base::Optional<ModuleWireBytes> GetModuleBytes() const final { return base::Optional<ModuleWireBytes>(std::atomic_load(&wire_bytes_)->as_vector()); }
...
void NativeModule::InsertToCodeCache(WasmCode* code)
{
    ...
    // stepping).
    if (code->tier() == ExecutionTier::kLiftoff && code->for_debugging() != kForDebugging) {
        return;
    }
...
NativeModule::JumpTablesRef NativeModule::FindJumpTablesForRegionLocked(base::AddressRegion code_region) const
{
    ...
    auto jump_table_usable = [code_region](const WasmCode* jump_table) {
        ...
        size_t max_distance
            = std::max(code_region.end() > table_start ? code_region.end() - table_start : 0, table_end > code_region.begin() ? table_end - code_region.begin() : 0);
    ...
    for (auto& code_space_data : code_space_data_) {
        ...
        return { code_space_data.jump_table ? code_space_data.jump_table->instruction_start() : kNullAddress, code_space_data.far_jump_table->instruction_start() };
    }
    return {};
}
...
Address NativeModule::GetNearRuntimeStubEntry(WasmCode::RuntimeStubId index, const JumpTablesRef& jump_tables) const
{
    DCHECK(jump_tables.is_valid());
...
WasmCode::RuntimeStubId NativeModule::GetRuntimeStubId(Address target) const
{
    ...
    for (auto& code_space_data : code_space_data_) {
        if (code_space_data.far_jump_table != nullptr && code_space_data.far_jump_table->contains(target)) {
            ...
            if (index >= WasmCode::kRuntimeStubCount)
                continue;
...
void WasmCodeManager::Commit(base::AddressRegion region)
{
    ...
    if (MemoryProtectionKeysEnabled()) {
        ...
        TRACE_HEAP("Setting rwx permissions and memory protection key for 0x%" PRIxPTR ":0x%" PRIxPTR "\n", region.begin(), region.end());
        success = base::MemoryProtectionKey::SetPermissionsAndKey(GetPlatformPageAllocator(), region, permission, RwxMemoryWriteScope::memory_protection_key());
#else
        UNREACHABLE();
#endif // V8_HAS_PKU_JIT_WRITE_PROTECT
    } else {
        TRACE_HEAP("Setting rwx permissions for 0x%" PRIxPTR ":0x%" PRIxPTR "\n", region.begin(), region.end());
        success = SetPermissions(GetPlatformPageAllocator(), region.begin(), region.size(), permission);
    }
...
void WasmCodeManager::Decommit(base::AddressRegion region)
{
    ...
    CHECK(allocator->DecommitPages(reinterpret_cast<void*>(region.begin()), region.size()));
}
...
VirtualMemory WasmCodeManager::TryAllocate(size_t size, void* hint)
{
    ...
    VirtualMemory mem(page_allocator, size, hint, allocate_page_size, JitPermission::kMapAsJittable);
    if (!mem.IsReserved())
        return {};
...
// static
size_t WasmCodeManager::EstimateLiftoffCodeSize(int body_size) { return kLiftoffFunctionOverhead + kCodeAlignment / 2 + body_size * kLiftoffCodeSizeMultiplier; }
...
// static
size_t WasmCodeManager::EstimateNativeModuleCodeSize(int num_functions, int num_imported_functions, int code_section_length, bool include_liftoff, DynamicTiering dynamic_tiering)
{
...
size_t WasmCodeManager::EstimateNativeModuleMetaDataSize(const WasmModule* module)
{
    ...
    size_t far_jump_table_size
        = RoundUp<kCodeAlignment>(JumpTableAssembler::SizeForNumberOfFarJumpSlots(WasmCode::kRuntimeStubCount, NumWasmFunctionsInFarJumpTable(num_wasm_functions)));
...
// static
bool WasmCodeManager::MemoryProtectionKeysEnabled() { return HasMemoryProtectionKeySupport() && v8_flags.wasm_memory_protection_keys; }
...
base::AddressRegion WasmCodeManager::AllocateAssemblerBufferSpace(int size)
{
    ...
    if (MemoryProtectionKeysEnabled()) {
        ...
        void* mapped = AllocatePages(page_allocator, nullptr, size, page_size, PageAllocator::kNoAccess);
        if (V8_UNLIKELY(!mapped)) {
            auto oom_detail = base::FormattedString {} << "cannot allocate " << size << " more bytes for assembler buffers";
            V8::FatalProcessOutOfMemory(nullptr, "Allocate protected assembler buffer space", oom_detail.PrintToArray().data());
            UNREACHABLE();
        }
        auto region = base::AddressRegionOf(reinterpret_cast<uint8_t*>(mapped), size);
        CHECK(base::MemoryProtectionKey::SetPermissionsAndKey(page_allocator, region, PageAllocator::kReadWrite, RwxMemoryWriteScope::memory_protection_key()));
        return region;
    }
#endif // V8_HAS_PKU_JIT_WRITE_PROTECT
...
std::shared_ptr<NativeModule> WasmCodeManager::NewNativeModule(Isolate* isolate, const WasmFeatures& enabled, size_t code_size_estimate, std::shared_ptr<const WasmModule> module)
{
    ...
    new NativeModule(enabled, DynamicTiering { v8_flags.wasm_dynamic_tiering.value() }, std::move(code_space), std::move(module), isolate->async_counters(), &ret);
...
void NativeModule::SampleCodeSize(Counters* counters, NativeModule::CodeSamplingTime sampling_time) const
{
    size_t code_size = sampling_time == kSampling ? code_allocator_.committed_code_space() : code_allocator_.generated_code_size();
...
std::unique_ptr<WasmCode> NativeModule::AddCompiledCode(WasmCompilationResult result)
{
    std::vector<std::unique_ptr<WasmCode>> code = AddCompiledCode({ &result, 1 });
    return std::move(code[0]);
}

std::vector<std::unique_ptr<WasmCode>> NativeModule::AddCompiledCode(base::Vector<WasmCompilationResult> results)
{
    ...
    for (auto& result : results) {
        ...
        generated_code.emplace_back(
            AddCodeWithCodeSpace(result.func_index, result.code_desc, result.frame_slot_count, result.tagged_parameter_slots, result.protected_instructions_data.as_vector(),
                result.source_positions.as_vector(), GetCodeKind(result), result.result_tier, result.for_debugging, this_code_space, jump_tables));
    }
...
void NativeModule::RecompileForTiering()
{
    ...
    TieringState current_state;
    {
        ...
        // from now on.
        if (!cached_code_) {
            cached_code_ = std::make_unique<std::map<std::pair<ExecutionTier, int>, WasmCode*>>();
...
std::vector<int> NativeModule::FindFunctionsToRecompile(TieringState new_tiering_state)
{
    WasmCodeRefScope code_ref_scope;
    base::RecursiveMutexGuard guard(&allocation_mutex_);
    ...
    for (int slot_index = 0; slot_index < declared; ++slot_index) {
        ...
        bool code_is_good = tier_down ? old_code && old_code->for_debugging() : old_code && old_code->tier() == ExecutionTier::kTurbofan;
        if (code_is_good)
            continue;
        DCHECK_NOT_NULL(cached_code_);
        auto cache_it = cached_code_->find(std::make_pair(tier_down ? ExecutionTier::kLiftoff : ExecutionTier::kTurbofan, function_index));
...
SimpleTlsPointerXp<WasmCodeRefScope> current_code_refs_scope;
} // namespace
...
#define RUNTIME_STUB_NAME_TRAP(Name) Builtin::kThrowWasm##Name,
    constexpr Builtin builtin_names[] = { WASM_RUNTIME_STUB_LIST(RUNTIME_STUB_NAME, RUNTIME_STUB_NAME_TRAP) };
...
#define RUNTIME_STUB_NAME_TRAP(Name) "ThrowWasm" #Name,
    constexpr const char* runtime_stub_names[] = { WASM_RUNTIME_STUB_LIST(RUNTIME_STUB_NAME, RUNTIME_STUB_NAME_TRAP) "<unknown>" };
#undef RUNTIME_STUB_NAME
#undef RUNTIME_STUB_NAME_TRAP
    static_assert(arraysize(runtime_stub_names) == WasmCode::kRuntimeStubCount + 1);
