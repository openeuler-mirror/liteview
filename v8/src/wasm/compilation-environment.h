...
enum RuntimeExceptionSupport : bool { kRuntimeExceptionSupport = true, kNoRuntimeExceptionSupport = false };
...
struct CompilationEnv {
    ...
    static constexpr uintptr_t MinPages(const WasmModule* module)
    {
        if (!module)
            return 0;
        const uintptr_t platform_max_pages = module->is_memory64 ? kV8MaxWasmMemory64Pages : kV8MaxWasmMemory32Pages;
...
enum class CompilationEvent : uint8_t { kFinishedBaselineCompilation, kFinishedExportWrappers, kFinishedCompilationChunk, kFailedCompilation, kFinishedRecompilation };
...
public:
    ...
    virtual ReleaseAfterFinalEvent release_after_final_event() { return kReleaseAfterFinalEvent; }
};
...
private:
    ...
    // certain scopes.
    static std::unique_ptr<CompilationState> New(const std::shared_ptr<NativeModule>&, std::shared_ptr<Counters>, DynamicTiering dynamic_tiering);
};
