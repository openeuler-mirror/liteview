...
enum WasmOpcodeSig : byte { kSigEnum_None, FOREACH_SIGNATURE(DECLARE_SIG_ENUM) };
...
#define DECLARE_SIG(name, ...)                                                                                                                                                     \
    ...
    constexpr FunctionSig kSig_##name(kReturnsCount_##name, static_cast<int>(arraysize(kTypes_##name)) - 1, kTypes_##name + (1 - kReturnsCount_##name));
FOREACH_SIGNATURE(DECLARE_SIG)
#undef DECLARE_SIG

#define DECLARE_SIG_ENTRY(name, ...) &kSig_##name,
constexpr const FunctionSig* kCachedSigs[] = { nullptr, FOREACH_SIGNATURE(DECLARE_SIG_ENTRY) };
...
const char* WasmOpcodes::TrapReasonMessage(TrapReason reason) { return MessageFormatter::TemplateString(TrapReasonToMessageId(reason)); }
