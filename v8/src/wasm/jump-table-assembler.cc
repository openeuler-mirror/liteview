...
{
    intptr_t displacement = static_cast<intptr_t>(reinterpret_cast<byte*>(target) - pc_ - kNearJmpInstrSize);
...
void JumpTableAssembler::EmitFarJumpSlot(Address target) { jmp(target, RelocInfo::NO_INFO); }

// static
void JumpTableAssembler::PatchFarJumpSlot(Address slot, Address target) { UNREACHABLE(); }
...
bool JumpTableAssembler::EmitJumpSlot(Address target)
{
    if (!TurboAssembler::IsNearCallOffset((reinterpret_cast<byte*>(target) - pc_) / kInstrSize)) {
        return false;
    }
...
void JumpTableAssembler::PatchFarJumpSlot(Address slot, Address target)
{
    ...
    int kTargetOffset = ENABLE_CONTROL_FLOW_INTEGRITY_BOOL ? 4 * kInstrSize : 2 * kInstrSize;
    // The slot needs to be pointer-size aligned so we can atomically update it.
...
void JumpTableAssembler::EmitFarJumpSlot(Address target) { JumpToOffHeapInstructionStream(target); }
