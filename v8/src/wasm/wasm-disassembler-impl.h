...
V8_EXPORT_PRIVATE void PrintSignatureOneLine(
    StringBuilder& out, const FunctionSig* sig, uint32_t func_index, NamesProvider* names, bool param_names, IndexAsComment indices_as_comments = NamesProvider::kDontPrintIndex);
...
// template <Decoder::kFullValidation>
// class WasmDecoder;

class V8_EXPORT_PRIVATE FunctionBodyDisassembler : public WasmDecoder<Decoder::kFullValidation> {
public:
    static constexpr Decoder::ValidateFlag validate = Decoder::kFullValidation;
    enum FunctionHeader : bool { kSkipHeader = false, kPrintHeader = true };

    FunctionBodyDisassembler(Zone* zone, const WasmModule* module, uint32_t func_index, WasmFeatures* detected, const FunctionSig* sig, const byte* start, const byte* end,
        uint32_t offset, NamesProvider* names)
        : WasmDecoder<validate>(zone, module, WasmFeatures::All(), detected, sig, start, end, offset)
...
protected:
    ...
    LabelInfo& label_info(int depth) { return label_stack_[label_stack_.size() - 1 - depth]; }
