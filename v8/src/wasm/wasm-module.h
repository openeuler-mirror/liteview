...
struct WasmTag {
    explicit WasmTag(const WasmTagSig* sig)
        : sig(sig)
    {
    }
...
struct WasmElemSegment {
    ...
    // Construct an active segment.
    WasmElemSegment(ValueType type, uint32_t table_index, ConstantExpression offset, ElementType element_type)
...
#define SELECT_WASM_COUNTER(counters, origin, prefix, suffix) ((origin) == kWasmOrigin ? (counters)->prefix##_wasm_##suffix() : (counters)->prefix##_asm_##suffix())
...
public:
    WireBytesRef LookupFunctionName(const ModuleWireBytes& wire_bytes, uint32_t function_index);
...
struct TypeDefinition {
    ...
    bool operator!=(const TypeDefinition& other) const { return !(*this == other); }
...
struct V8_EXPORT_PRIVATE WasmModule {
    ...
    bool has_signature(uint32_t index) const { return index < types.size() && types[index].kind == TypeDefinition::kFunction; }
    ...
    bool has_struct(uint32_t index) const { return index < types.size() && types[index].kind == TypeDefinition::kStruct; }
    ...
    bool has_array(uint32_t index) const { return index < types.size() && types[index].kind == TypeDefinition::kArray; }
    ...
    bool has_supertype(uint32_t index) const { return supertype(index) != kNoSuperType; }
...
inline bool is_asmjs_module(const WasmModule* module) { return module->origin != kWasmOrigin; }
...
V8_EXPORT_PRIVATE int GetSubtypingDepth(const WasmModule* module, uint32_t type_index);
...
struct V8_EXPORT_PRIVATE ModuleWireBytes {
    ...
    base::Vector<const byte> GetFunctionBytes(const WasmFunction* function) const { return module_bytes_.SubVector(function->code.offset(), function->code.end_offset()); }
...
V8_EXPORT_PRIVATE bool IsWasmCodegenAllowed(Isolate* isolate, Handle<Context> context);
V8_EXPORT_PRIVATE Handle<String> ErrorStringForCodegen(Isolate* isolate, Handle<Context> context);

Handle<JSObject> GetTypeForFunction(Isolate* isolate, const FunctionSig* sig, bool for_exception = false);
...
Handle<JSArray> GetCustomSections(Isolate* isolate, Handle<WasmModuleObject> module, Handle<String> name, ErrorThrower* thrower);
...
size_t PrintSignature(base::Vector<char> buffer, const wasm::FunctionSig*, char delimiter = ':');
