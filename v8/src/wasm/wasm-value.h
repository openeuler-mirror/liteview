...
#define DEFINE_SIMD_TYPE_SPECIFIC_METHODS(cType, sType, name, size)                                                                                                                \
    explicit Simd128(sType val) { base::WriteUnalignedValue<sType>(reinterpret_cast<Address>(val_), val); }                                                                        \
    sType to_##name() const { return base::ReadUnalignedValue<sType>(reinterpret_cast<Address>(val_)); }
    FOREACH_SIMD_TYPE(DEFINE_SIMD_TYPE_SPECIFIC_METHODS)
#undef DEFINE_SIMD_TYPE_SPECIFIC_METHODS

    explicit Simd128(byte* bytes) { memcpy(static_cast<void*>(val_), reinterpret_cast<void*>(bytes), kSimd128Size); }
...
#define DECLARE_CAST(cType, sType, name, size)                                                                                                                                     \
    template <> inline sType Simd128::to() const { return to_##name(); }
...
#define DEFINE_TYPE_SPECIFIC_METHODS(name, localtype, ctype)                                                                                                                       \
    ...
    ctype to_##name##_unchecked() const { return base::ReadUnalignedValue<ctype>(reinterpret_cast<Address>(bit_pattern_)); }

    FOREACH_PRIMITIVE_WASMVAL_TYPE(DEFINE_TYPE_SPECIFIC_METHODS)
#undef DEFINE_TYPE_SPECIFIC_METHODS

    WasmValue(byte* raw_bytes, ValueType type)
        : type_(type)
        , bit_pattern_ {}
    {
    ...
    WasmValue(Handle<Object> ref, ValueType type)
        : type_(type)
        , bit_pattern_ {}
    {
...
#define DECLARE_CAST(name, localtype, ctype, ...)                                                                                                                                  \
    template <> inline ctype WasmValue::to_unchecked() const { return to_##name##_unchecked(); }                                                                                   \
    template <> inline ctype WasmValue::to() const { return to_##name(); }
