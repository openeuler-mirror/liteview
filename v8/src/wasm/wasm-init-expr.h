...
public:
    ...
    static WasmInitExpr StructNew(uint32_t index, ZoneVector<WasmInitExpr>* elements)
    {
    ...
    static WasmInitExpr ArrayNewFixed(uint32_t index, ZoneVector<WasmInitExpr>* elements)
    {
    ...
    {
        WasmInitExpr expr(kI31New, zone->New<ZoneVector<WasmInitExpr>>(std::initializer_list<WasmInitExpr> { value }, zone));
        return expr;
    }
    ...
    bool operator==(const WasmInitExpr& other) const
    {
        ...
        case kI31New: {
            ...
            return (immediate().i32_const & mask) == (other.immediate().i32_const & mask);
        }
        }
    }

    V8_INLINE bool operator!=(const WasmInitExpr& other) const { return !(*this == other); }

    ValueType type(const WasmModule* module, const WasmFeatures& enabled_features) const;
