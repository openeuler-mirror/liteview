...
static constexpr LoadType GetLoadType(WasmOpcode opcode)
{
    ...
    constexpr LoadType kLoadTypes[]
        = { LoadType::kI32Load, LoadType::kI64Load, LoadType::kF32Load, LoadType::kF64Load, LoadType::kI32Load8S, LoadType::kI32Load8U, LoadType::kI32Load16S,
              LoadType::kI32Load16U, LoadType::kI64Load8S, LoadType::kI64Load8U, LoadType::kI64Load16S, LoadType::kI64Load16U, LoadType::kI64Load32S, LoadType::kI64Load32U };
...
static constexpr StoreType GetStoreType(WasmOpcode opcode)
{
    ...
    constexpr StoreType kStoreTypes[] = { StoreType::kI32Store, StoreType::kI64Store, StoreType::kF32Store, StoreType::kF64Store, StoreType::kI32Store8, StoreType::kI32Store16,
...
template <Decoder::ValidateFlag validate, typename... Args> void DecodeError(Decoder* decoder, const byte* pc, const char* str, Args&&... args)
{
    CHECK(validate == Decoder::kFullValidation || validate == Decoder::kBooleanValidation);
...
template <Decoder::ValidateFlag validate> void DecodeError(Decoder* decoder, const byte* pc, const char* str)
{
    CHECK(validate == Decoder::kFullValidation || validate == Decoder::kBooleanValidation);
    if (validate == Decoder::kBooleanValidation) {
        decoder->MarkError();
...
template <Decoder::ValidateFlag validate, typename... Args> void DecodeError(Decoder* decoder, const char* str, Args&&... args)
{
    CHECK(validate == Decoder::kFullValidation || validate == Decoder::kBooleanValidation);
    static_assert(sizeof...(Args) > 0);
    if (validate == Decoder::kBooleanValidation) {
...
template <Decoder::ValidateFlag validate> void DecodeError(Decoder* decoder, const char* str)
{
    CHECK(validate == Decoder::kFullValidation || validate == Decoder::kBooleanValidation);
    if (validate == Decoder::kBooleanValidation) {
        decoder->MarkError();
...
// type capacity.
template <Decoder::ValidateFlag validate> HeapType read_heap_type(Decoder* decoder, const byte* pc, uint32_t* const length, const WasmModule* module, const WasmFeatures& enabled)
{
    ...
    if (heap_index < 0) {
        ...
        if (!VALIDATE(heap_index >= min_1_byte_leb128)) {
            DecodeError<validate>(decoder, pc, "Unknown heap type %" PRId64, heap_index);
        ...
        case kNoFuncCode:
            if (!VALIDATE(enabled.has_gc())) {
                DecodeError<validate>(decoder, pc, "invalid heap type '%s', enable with --experimental-wasm-gc", HeapType::from_code(code).name().c_str());
            }
            V8_FALLTHROUGH;
        case kExternRefCode:
        case kFuncRefCode:
        ...
        if (!VALIDATE(module == nullptr || type_index < module->types.capacity())) {
            DecodeError<validate>(decoder, pc, "Type index %u is out of bounds", type_index);
        }
...
// kNoValidate.
template <Decoder::ValidateFlag validate> ValueType read_value_type(Decoder* decoder, const byte* pc, uint32_t* const length, const WasmModule* module, const WasmFeatures& enabled)
{
    *length = 1;
    ...
    case kNoFuncCode:
        if (!VALIDATE(enabled.has_gc())) {
            DecodeError<validate>(decoder, pc, "invalid value type '%sref', enable with --experimental-wasm-gc", HeapType::from_code(code).name().c_str());
            return kWasmBottom;
        }
        V8_FALLTHROUGH;
    case kExternRefCode:
    ...
    case kRefNullCode: {
        ...
        HeapType heap_type = read_heap_type<validate>(decoder, pc + 1, length, module, enabled);
        *length += 1;
        return heap_type.is_bottom() ? kWasmBottom : ValueType::RefMaybeNull(heap_type, nullability);
    }
    case kS128Code: {
        if (!VALIDATE(enabled.has_simd())) {
            DecodeError<validate>(decoder, pc, "invalid value type 's128', enable with --experimental-wasm-simd");
            return kWasmBottom;
        }
...
template <Decoder::ValidateFlag validate> struct ImmI32Immediate {
    int32_t value;
    uint32_t length;
    ImmI32Immediate(Decoder* decoder, const byte* pc) { value = decoder->read_i32v<validate>(pc, &length, "immi32"); }
};

template <Decoder::ValidateFlag validate> struct ImmI64Immediate {
    int64_t value;
    uint32_t length;
    ImmI64Immediate(Decoder* decoder, const byte* pc) { value = decoder->read_i64v<validate>(pc, &length, "immi64"); }
};

template <Decoder::ValidateFlag validate> struct ImmF32Immediate {
    float value;
    uint32_t length = 4;
...
template <Decoder::ValidateFlag validate> struct ImmF64Immediate {
    double value;
    uint32_t length = 8;
...
template <Decoder::ValidateFlag validate> struct MemoryIndexImmediate {
    uint8_t index = 0;
    uint32_t length = 1;
    MemoryIndexImmediate(Decoder* decoder, const byte* pc) { index = decoder->read_u8<validate>(pc, "memory index"); }
};

// Parent class for all Immediates which read a u32v index value in their
// constructor.
template <Decoder::ValidateFlag validate> struct IndexImmediate {
    uint32_t index;
    uint32_t length;

    IndexImmediate(Decoder* decoder, const byte* pc, const char* name) { index = decoder->read_u32v<validate>(pc, &length, name); }
};

template <Decoder::ValidateFlag validate> struct TagIndexImmediate : public IndexImmediate<validate> {
...
template <Decoder::ValidateFlag validate> struct GlobalIndexImmediate : public IndexImmediate<validate> {
...
template <Decoder::ValidateFlag validate> struct SigIndexImmediate : public IndexImmediate<validate> {
...
template <Decoder::ValidateFlag validate> struct StructIndexImmediate : public IndexImmediate<validate> {
...
template <Decoder::ValidateFlag validate> struct ArrayIndexImmediate : public IndexImmediate<validate> {
...
template <Decoder::ValidateFlag validate> struct CallFunctionImmediate : public IndexImmediate<validate> {
    const FunctionSig* sig = nullptr;
...
template <Decoder::ValidateFlag validate> struct SelectTypeImmediate {
    uint32_t length;
    ValueType type;

    SelectTypeImmediate(const WasmFeatures& enabled, Decoder* decoder, const byte* pc, const WasmModule* module)
    {
        uint8_t num_types = decoder->read_u32v<validate>(pc, &length, "number of select types");
        if (!VALIDATE(num_types == 1)) {
            DecodeError<validate>(decoder, pc, "Invalid number of types. Select accepts exactly one type");
            return;
        }
        uint32_t type_length;
        type = value_type_reader::read_value_type<validate>(decoder, pc + length, &type_length, module, enabled);
        length += type_length;
    }
};

template <Decoder::ValidateFlag validate> struct BlockTypeImmediate {
    uint32_t length = 1;
    ValueType type = kWasmVoid;
    uint32_t sig_index = 0;
    const FunctionSig* sig = nullptr;

    BlockTypeImmediate(const WasmFeatures& enabled, Decoder* decoder, const byte* pc, const WasmModule* module)
    {
        int64_t block_type = decoder->read_i33v<validate>(pc, &length, "block type");
        if (block_type < 0) {
            ...
            if (!VALIDATE(block_type >= min_1_byte_leb128)) {
                DecodeError<validate>(decoder, pc, "invalid block type %" PRId64, block_type);
                return;
            }
            if (static_cast<ValueTypeCode>(block_type & 0x7F) == kVoidCode)
                return;
            type = value_type_reader::read_value_type<validate>(decoder, pc, &length, module, enabled);
        } else {
            type = kWasmBottom;
...
template <Decoder::ValidateFlag validate> struct BranchDepthImmediate {
    uint32_t depth;
    uint32_t length;
    BranchDepthImmediate(Decoder* decoder, const byte* pc) { depth = decoder->read_u32v<validate>(pc, &length, "branch depth"); }
};

template <Decoder::ValidateFlag validate> struct FieldImmediate {
...
template <Decoder::ValidateFlag validate> struct CallIndirectImmediate {
...
template <Decoder::ValidateFlag validate> struct BranchTableImmediate {
...
template <Decoder::ValidateFlag validate> class BranchTableIterator {
public:
...
template <Decoder::ValidateFlag validate, DecodingMode decoding_mode = kFunctionBody> class WasmDecoder;

template <Decoder::ValidateFlag validate> struct MemoryAccessImmediate {
    uint32_t alignment;
    uint64_t offset;
    uint32_t length = 0;
    MemoryAccessImmediate(Decoder* decoder, const byte* pc, uint32_t max_alignment, bool is_memory64)
    {
        uint32_t alignment_length;
        alignment = decoder->read_u32v<validate>(pc, &alignment_length, "alignment");
        ...
        offset = is_memory64 ? decoder->read_u64v<validate>(pc + alignment_length, &offset_length, "offset")
                             : decoder->read_u32v<validate>(pc + alignment_length, &offset_length, "offset");
        length = alignment_length + offset_length;
    }
};

// Immediate for SIMD lane operations.
template <Decoder::ValidateFlag validate> struct SimdLaneImmediate {
    uint8_t lane;
    uint32_t length = 1;

    SimdLaneImmediate(Decoder* decoder, const byte* pc) { lane = decoder->read_u8<validate>(pc, "lane"); }
};

// Immediate for SIMD S8x16 shuffle operations.
template <Decoder::ValidateFlag validate> struct Simd128Immediate {
...
template <Decoder::ValidateFlag validate> struct MemoryInitImmediate {
...
template <Decoder::ValidateFlag validate> struct MemoryCopyImmediate {
...
template <Decoder::ValidateFlag validate> struct TableInitImmediate {
...
template <Decoder::ValidateFlag validate> struct TableCopyImmediate {
...
template <Decoder::ValidateFlag validate> struct HeapTypeImmediate {
    uint32_t length = 1;
    HeapType type;
    HeapTypeImmediate(const WasmFeatures& enabled, Decoder* decoder, const byte* pc, const WasmModule* module)
        : type(value_type_reader::read_heap_type<validate>(decoder, pc, &length, module, enabled))
    {
    }
};

template <Decoder::ValidateFlag validate> struct StringConstImmediate {
    uint32_t index;
    uint32_t length;
    StringConstImmediate(Decoder* decoder, const byte* pc) { index = decoder->read_u32v<validate>(pc, &length, "stringref literal index"); }
};

template <Decoder::ValidateFlag validate> struct PcForErrors {
    explicit PcForErrors(const byte* /* pc */) { }

    const byte* pc() const { return nullptr; }
};

template <> struct PcForErrors<Decoder::kFullValidation> {
...
template <Decoder::ValidateFlag validate> struct ValueBase : public PcForErrors<validate> {
    ValueType type = kWasmVoid;
...
template <typename Value, Decoder::ValidateFlag validate> struct ControlBase : public PcForErrors<validate> {
    ...
    ControlBase(ControlKind kind, uint32_t stack_depth, uint32_t init_stack_depth, const uint8_t* pc, Reachability reachability)
        : PcForErrors<validate>(pc)
        , kind(kind)
    ...
    Reachability innerReachability() const { return reachability == kReachable ? kReachable : kSpecOnlyReachable; }
    ...
    bool is_try() const { return is_incomplete_try() || is_try_catch() || is_try_catchall(); }

    Merge<Value>* br_merge() { return is_loop() ? &this->start_merge : &this->end_merge; }
};
...
#define INTERFACE_CONSTANT_FUNCTIONS(F) /*       force 80 columns           */                                                                                                     \
    ...
    F(StructNew, const StructIndexImmediate<validate>& imm, const Value& rtt, const Value args[], Value* result)                                                                   \
    F(StructNewDefault, const StructIndexImmediate<validate>& imm, const Value& rtt, Value* result)                                                                                \
    F(ArrayNew, const ArrayIndexImmediate<validate>& imm, const Value& length, const Value& initial_value, const Value& rtt, Value* result)                                        \
    F(ArrayNewDefault, const ArrayIndexImmediate<validate>& imm, const Value& length, const Value& rtt, Value* result)                                                             \
    F(ArrayNewFixed, const ArrayIndexImmediate<validate>& imm, const base::Vector<Value>& elements, const Value& rtt, Value* result)                                               \
    F(ArrayNewSegment, const ArrayIndexImmediate<validate>& array_imm, const IndexImmediate<validate>& data_segment, const Value& offset, const Value& length, const Value& rtt,   \
        Value* result)                                                                                                                                                             \
...
#define INTERFACE_NON_CONSTANT_FUNCTIONS(F) /*       force 80 columns       */                                                                                                     \
    ...
    F(LocalTee, const Value& value, Value* result, const IndexImmediate<validate>& imm)                                                                                            \
    F(GlobalSet, const Value& value, const GlobalIndexImmediate<validate>& imm)                                                                                                    \
    F(TableGet, const Value& index, Value* result, const IndexImmediate<validate>& imm)                                                                                            \
    F(TableSet, const Value& index, const Value& value, const IndexImmediate<validate>& imm)                                                                                       \
    ...
    F(LoadMem, LoadType type, const MemoryAccessImmediate<validate>& imm, const Value& index, Value* result)                                                                       \
    F(LoadTransform, LoadType type, LoadTransformationKind transform, const MemoryAccessImmediate<validate>& imm, const Value& index, Value* result)                               \
    F(LoadLane, LoadType type, const Value& value, const Value& index, const MemoryAccessImmediate<validate>& imm, const uint8_t laneidx, Value* result)                           \
    F(StoreMem, StoreType type, const MemoryAccessImmediate<validate>& imm, const Value& index, const Value& value)                                                                \
    F(StoreLane, StoreType type, const MemoryAccessImmediate<validate>& imm, const Value& index, const Value& value, const uint8_t laneidx)                                        \
    F(CurrentMemoryPages, Value* result)                                                                                                                                           \
    F(MemoryGrow, const Value& value, Value* result)                                                                                                                               \
    F(CallDirect, const CallFunctionImmediate<validate>& imm, const Value args[], Value returns[])                                                                                 \
    F(CallIndirect, const Value& index, const CallIndirectImmediate<validate>& imm, const Value args[], Value returns[])                                                           \
    F(CallRef, const Value& func_ref, const FunctionSig* sig, uint32_t sig_index, const Value args[], const Value returns[])                                                       \
    F(ReturnCallRef, const Value& func_ref, const FunctionSig* sig, uint32_t sig_index, const Value args[])                                                                        \
    F(ReturnCall, const CallFunctionImmediate<validate>& imm, const Value args[])                                                                                                  \
    F(ReturnCallIndirect, const Value& index, const CallIndirectImmediate<validate>& imm, const Value args[])                                                                      \
    ...
    F(SimdLaneOp, WasmOpcode opcode, const SimdLaneImmediate<validate>& imm, const base::Vector<Value> inputs, Value* result)                                                      \
    F(S128Const, const Simd128Immediate<validate>& imm, Value* result)                                                                                                             \
    F(Simd8x16ShuffleOp, const Simd128Immediate<validate>& imm, const Value& input0, const Value& input1, Value* result)                                                           \
    F(Throw, const TagIndexImmediate<validate>& imm, const base::Vector<Value>& args)                                                                                              \
    F(Rethrow, Control* block)                                                                                                                                                     \
    F(CatchException, const TagIndexImmediate<validate>& imm, Control* block, base::Vector<Value> caught_values)                                                                   \
    F(Delegate, uint32_t depth, Control* block)                                                                                                                                    \
    F(CatchAll, Control* block)                                                                                                                                                    \
    F(AtomicOp, WasmOpcode opcode, base::Vector<Value> args, const MemoryAccessImmediate<validate>& imm, Value* result)                                                            \
    F(AtomicFence)                                                                                                                                                                 \
    F(MemoryInit, const MemoryInitImmediate<validate>& imm, const Value& dst, const Value& src, const Value& size)                                                                 \
    F(DataDrop, const IndexImmediate<validate>& imm)                                                                                                                               \
    F(MemoryCopy, const MemoryCopyImmediate<validate>& imm, const Value& dst, const Value& src, const Value& size)                                                                 \
    F(MemoryFill, const MemoryIndexImmediate<validate>& imm, const Value& dst, const Value& value, const Value& size)                                                              \
    F(TableInit, const TableInitImmediate<validate>& imm, base::Vector<Value> args)                                                                                                \
    F(ElemDrop, const IndexImmediate<validate>& imm)                                                                                                                               \
    F(TableCopy, const TableCopyImmediate<validate>& imm, base::Vector<Value> args)                                                                                                \
    F(TableGrow, const IndexImmediate<validate>& imm, const Value& value, const Value& delta, Value* result)                                                                       \
    F(TableSize, const IndexImmediate<validate>& imm, Value* result)                                                                                                               \
    F(TableFill, const IndexImmediate<validate>& imm, const Value& start, const Value& value, const Value& count)                                                                  \
    F(StructGet, const Value& struct_object, const FieldImmediate<validate>& field, bool is_signed, Value* result)                                                                 \
    F(StructSet, const Value& struct_object, const FieldImmediate<validate>& field, const Value& field_value)                                                                      \
    F(ArrayGet, const Value& array_obj, const ArrayIndexImmediate<validate>& imm, const Value& index, bool is_signed, Value* result)                                               \
    F(ArraySet, const Value& array_obj, const ArrayIndexImmediate<validate>& imm, const Value& index, const Value& value)                                                          \
    F(ArrayLen, const Value& array_obj, Value* result)                                                                                                                             \
    F(ArrayCopy, const Value& src, const Value& src_index, const Value& dst, const Value& dst_index, const Value& length)                                                          \
    F(I31GetS, const Value& input, Value* result)                                                                                                                                  \
    F(I31GetU, const Value& input, Value* result)                                                                                                                                  \
    F(RefTest, const Value& obj, const Value& rtt, Value* result, bool null_succeeds)                                                                                              \
    ...
    F(BrOnCast, const Value& obj, const Value& rtt, Value* result_on_branch, uint32_t depth)                                                                                       \
    F(BrOnCastFail, const Value& obj, const Value& rtt, Value* result_on_fallthrough, uint32_t depth)                                                                              \
    ...
    F(BrOnNonArray, const Value& object, Value* value_on_fallthrough, uint32_t br_depth)                                                                                           \
    F(StringNewWtf8, const MemoryIndexImmediate<validate>& memory, const unibrow::Utf8Variant variant, const Value& offset, const Value& size, Value* result)                      \
    F(StringNewWtf8Array, const unibrow::Utf8Variant variant, const Value& array, const Value& start, const Value& end, Value* result)                                             \
    F(StringNewWtf16, const MemoryIndexImmediate<validate>& memory, const Value& offset, const Value& size, Value* result)                                                         \
    ...
    F(StringEncodeWtf8, const MemoryIndexImmediate<validate>& memory, const unibrow::Utf8Variant variant, const Value& str, const Value& address, Value* result)                   \
    ...
    F(StringEncodeWtf16, const MemoryIndexImmediate<validate>& memory, const Value& str, const Value& address, Value* result)                                                      \
    ...
    F(StringViewWtf8Encode, const MemoryIndexImmediate<validate>& memory, const unibrow::Utf8Variant variant, const Value& view, const Value& addr, const Value& pos,              \
    ...
    F(StringViewWtf16Encode, const MemoryIndexImmediate<validate>& memory, const Value& view, const Value& addr, const Value& pos, const Value& codeunits, Value* result)          \
...
// lengths, etc.
template <Decoder::ValidateFlag validate, DecodingMode decoding_mode> class WasmDecoder : public Decoder {
public:
    WasmDecoder(Zone* zone, const WasmModule* module, const WasmFeatures& enabled, WasmFeatures* detected, const FunctionSig* sig, const byte* start, const byte* end,
    ...
    void DecodeLocals(const byte* pc, uint32_t* total_length)
    {
        ...
        *total_length += length;
        ...
        for (uint32_t entry = 0; entry < entries; ++entry) {
            if (!VALIDATE(more())) {
                return DecodeError(end(), "expected more local decls but reached end of input");
            }

            uint32_t count = read_u32v<validate>(pc + *total_length, &length, "local count");
            if (!VALIDATE(ok())) {
            ...
            *total_length += length;
            ...
            ValueType type = value_type_reader::read_value_type<validate>(this, pc + *total_length, &length, this->module_, enabled_);
            if (!VALIDATE(ok()))
                return;
            *total_length += length;
    ...
    // {validate} flag.
    template <typename... Args> void DecodeError(Args... args) { wasm::DecodeError<validate>(this, std::forward<Args>(args)...); }
    ...
    // the instance cache.
    static BitVector* AnalyzeLoopAssignment(WasmDecoder* decoder, const byte* pc, uint32_t locals_count, Zone* zone)
    {
        if (pc >= decoder->end())
            return nullptr;
        if (*pc != kExprLoop)
    ...
    bool Validate(const byte* pc, GlobalIndexImmediate<validate>& imm)
    {
        ...
        if (decoding_mode == kConstantExpression) {
            ...
            if (!VALIDATE(imm.global->imported || this->enabled_.has_gc())) {
                this->DecodeError(pc, "non-imported globals cannot be used in constant expressions");
                return false;
            }
        }
    ...
    {
        if (!ValidateSignature(pc, imm.sig_imm))
            return false;
    ...
    bool Validate(const byte* pc, BranchDepthImmediate<validate>& imm, size_t control_depth)
    {
    ...
    bool Validate(const byte* pc, MemoryInitImmediate<validate>& imm) { return ValidateDataSegment(pc, imm.data_segment) && Validate(pc + imm.data_segment.length, imm.memory); }

    bool Validate(const byte* pc, MemoryCopyImmediate<validate>& imm) { return Validate(pc, imm.memory_src) && Validate(pc + imm.memory_src.length, imm.memory_dst); }
    ...
    bool Validate(const byte* pc, TableInitImmediate<validate>& imm)
    {
        ...
        if (!VALIDATE(IsSubtypeOf(elem_type, module_->tables[imm.table.index].type, module_))) {
    ...
    {
        if (!ValidateTable(pc, imm.table_src))
            return false;
        if (!ValidateTable(pc + imm.table_src.length, imm.table_dst))
            return false;
        ValueType src_type = module_->tables[imm.table_src.index].type;
        if (!VALIDATE(IsSubtypeOf(src_type, module_->tables[imm.table_dst.index].type, module_))) {
    ...
    template <class ImmediateObserver = EmptyImmediateObserver> static uint32_t OpcodeLength(WasmDecoder* decoder, const byte* pc, ImmediateObserver* io = nullptr)
    {
        WasmOpcode opcode = static_cast<WasmOpcode>(*pc);
        ...
        case kExprLoop:
        case kExprBlock: {
            BlockTypeImmediate<validate> imm(WasmFeatures::All(), decoder, pc + 1, nullptr);
            if (io)
                io->BlockType(imm);
            return 1 + imm.length;
        }
        ...
        case kExprSelectWithType: {
            SelectTypeImmediate<validate> imm(WasmFeatures::All(), decoder, pc + 1, nullptr);
            if (io)
                io->SelectType(imm);
            return 1 + imm.length;
        }
        ...
        case kExprF64Const:
            ...
            return 9;
        case kExprRefNull: {
            HeapTypeImmediate<validate> imm(WasmFeatures::All(), decoder, pc + 1, nullptr);
            if (io)
                io->HeapType(imm);
            return 1 + imm.length;
        }
...
#define DECLARE_OPCODE_CASE(name, ...) case kExpr##name:
            ...
            {
                MemoryAccessImmediate<validate> imm(decoder, pc + 1, UINT32_MAX, kConservativelyAssumeMemory64);
                if (io)
                    io->MemoryAccess(imm);
                return 1 + imm.length;
            }
        ...
        case kNumericPrefix: {
            ...
            case kExprDataDrop: {
                IndexImmediate<validate> imm(decoder, pc + length, "data segment index");
                if (io)
                    io->DataSegmentIndex(imm);
                return length + imm.length;
            }
            ...
            case kExprElemDrop: {
                IndexImmediate<validate> imm(decoder, pc + length, "element segment index");
                if (io)
                    io->ElemSegmentIndex(imm);
                return length + imm.length;
            }
            case kExprTableCopy: {
        ...
        case kAtomicPrefix: {
            uint32_t length = 0;
            opcode = decoder->read_prefixed_opcode<validate>(pc, &length, "atomic_index");
            switch (opcode) {
                FOREACH_ATOMIC_OPCODE(DECLARE_OPCODE_CASE)
                {
                    MemoryAccessImmediate<validate> imm(decoder, pc + length, UINT32_MAX, kConservativelyAssumeMemory64);
                    if (io)
                        io->MemoryAccess(imm);
                    return length + imm.length;
                }
        ...
        case kGCPrefix: {
            uint32_t length = 0;
            opcode = decoder->read_prefixed_opcode<validate>(pc, &length, "gc_index");
            switch (opcode) {
            case kExprStructNew:
            ...
            case kExprArrayNewFixed: {
                ...
                IndexImmediate<validate> length_imm(decoder, pc + length + array_imm.length, "array length");
                if (io)
                    io->TypeIndex(array_imm);
                if (io)
                    io->Length(length_imm);
                return length + array_imm.length + length_imm.length;
            }
            case kExprArrayCopy: {
                ArrayIndexImmediate<validate> dst_imm(decoder, pc + length);
                ArrayIndexImmediate<validate> src_imm(decoder, pc + length + dst_imm.length);
                if (io)
                    io->ArrayCopy(dst_imm, src_imm);
                return length + dst_imm.length + src_imm.length;
            }
            case kExprArrayNewData:
            case kExprArrayNewElem: {
                ArrayIndexImmediate<validate> array_imm(decoder, pc + length);
                IndexImmediate<validate> data_imm(decoder, pc + length + array_imm.length, "segment index");
                if (io)
                    io->TypeIndex(array_imm);
                if (io)
                    io->DataSegmentIndex(data_imm);
            ...
            case kExprRefTestNull: {
                HeapTypeImmediate<validate> imm(WasmFeatures::All(), decoder, pc + length, nullptr);
                if (io)
                    io->HeapType(imm);
                return length + imm.length;
            }
            ...
            case kExprBrOnCastFail: {
                ...
                IndexImmediate<validate> index(decoder, pc + length + branch.length, "type index");
                if (io)
                    io->BranchDepth(branch);
                if (io)
                    io->TypeIndex(index);
...
template <Decoder::ValidateFlag validate, typename Interface, DecodingMode decoding_mode = kFunctionBody> class WasmFullDecoder : public WasmDecoder<validate, decoding_mode> {
...
public:
    ...
    WasmFullDecoder(Zone* zone, const WasmModule* module, const WasmFeatures& enabled, WasmFeatures* detected, const FunctionBody& body, InterfaceArgs&&... interface_args)
        : WasmDecoder<validate, decoding_mode>(zone, module, enabled, detected, body.sig, body.start, body.end, body.offset)
    ...
    bool TraceFailed()
    {
        if (this->error_.offset()) {
            TRACE("wasm-error module+%-6d func+%d: %s\n\n", this->error_.offset(), this->GetBufferRelativeOffset(this->error_.offset()), this->error_.message().c_str());
        } else {
    ...
    uint32_t control_depth() const { return static_cast<uint32_t>(control_.size()); }
    ...
    uint32_t control_depth_of_current_catch() const { return control_depth() - 1 - current_catch(); }
    ...
    uint32_t pc_relative_offset() const { return this->pc_offset() - locals_offset_; }
    ...
    uint32_t locals_initialization_stack_depth() const { return static_cast<uint32_t>(locals_initializers_stack_.size()); }
    ...
    void InitializeInitializedLocalsTracking(int non_defaultable_locals)
    {
        ...
        for (size_t i = num_params; i < this->num_locals_; i++) {
            if (this->local_types_[i].is_defaultable())
                initialized_locals_[i] = true;
        }
        if (non_defaultable_locals == 0)
            return;
    ...
    void DecodeFunctionBody()
    {
        ...
        // Set up initial function block.
        {
            ...
            control_.emplace_back(kControlBlock, kStackDepth, kInitStackDepth, this->pc_, kReachable);
            Control* c = &control_.back();
            if (decoding_mode == kFunctionBody) {
                InitMerge(&c->start_merge, 0, [](uint32_t) -> Value { UNREACHABLE(); });
                InitMerge(&c->end_merge, static_cast<uint32_t>(this->sig_->return_count()), [&](uint32_t i) { return Value { this->pc_, this->sig_->GetReturn(i) }; });
            } else {
...
private:
    ...
    static Value UnreachableValue(const uint8_t* pc) { return Value { pc, kWasmBottom }; }

    bool CheckSimdFeatureFlagOpcode(WasmOpcode opcode)
    {
        if (!v8_flags.experimental_wasm_relaxed_simd && WasmOpcodes::IsRelaxedSimdOpcode(opcode)) {
            this->DecodeError("simd opcode not available, enable with --experimental-relaxed-simd");
            return false;
        }
    ...
    MemoryAccessImmediate<validate> MakeMemoryAccessImmediate(uint32_t pc_offset, uint32_t max_alignment)
    {
        return MemoryAccessImmediate<validate>(this, this->pc_ + pc_offset, max_alignment, this->module_->is_memory64);
    }
...
#undef BUILD_SIMPLE_OPCODE
    ...
    DECODE(Block)
    {
        BlockTypeImmediate<validate> imm(this->enabled_, this, this->pc_ + 1, this->module_);
    ...
    DECODE(BrOnNull)
    {
        ...
        Control* c = control_at(imm.depth);
        if (!VALIDATE(TypeCheckBranch<true>(c, 1)))
            return 0;
    ...
    DECODE(BrOnNonNull)
    {
        ...
        Control* c = control_at(imm.depth);
        if (!VALIDATE(TypeCheckBranch<true>(c, 0)))
            return 0;
        switch (ref_object.type.kind()) {
        case kBottom:
    ...
    {
        this->detected_->Add(kFeature_reftypes);
        SelectTypeImmediate<validate> imm(this->enabled_, this, this->pc_ + 1, this->module_);
        if (this->failed())
            return 0;
        Value cond = Peek(0, 2, kWasmI32);
    ...
    DECODE(Br)
    {
        ...
        if (!this->Validate(this->pc_ + 1, imm, control_.size()))
            return 0;
        Control* c = control_at(imm.depth);
        if (!VALIDATE(TypeCheckBranch<false>(c, 0)))
            return 0;
        if (V8_LIKELY(current_code_reachable_and_ok_)) {
            CALL_INTERFACE(BrOrRet, imm.depth, 0);
            c->br_merge()->reached = true;
    ...
    DECODE(BrTable)
    {
        ...
        while (iterator.has_next()) {
            ...
            if (validate) {
                ...
                if (!VALIDATE(TypeCheckBranch<false>(control_at(target), 1)))
                    return 0;
            }
        }
    ...
    DECODE(Return) { return DoReturn<kNonStrictCounting, kReturnMerge>() ? 1 : 0; }
    ...
    {
        this->detected_->Add(kFeature_reftypes);
        HeapTypeImmediate<validate> imm(this->enabled_, this, this->pc_ + 1, this->module_);
        if (!VALIDATE(this->ok()))
            return 0;
    ...
    DECODE(RefFunc)
    {
        ...
        if (!this->ValidateFunction(this->pc_ + 1, imm))
            return 0;
        HeapType heap_type(this->enabled_.has_typed_funcref() ? this->module_->functions[imm.index].sig_index : HeapType::kFunc);
    ...
    DECODE(TableGet)
    {
        ...
        if (!this->ValidateTable(this->pc_ + 1, imm))
            return 0;
    ...
    DECODE(CallFunction)
    {
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(CallDirect, imm, args.begin(), returns.begin());
        DropArgs(imm.sig);
        PushReturns(returns);
        return 1 + imm.length;
    }
    ...
    DECODE(CallIndirect)
    {
        ...
        if (!this->Validate(this->pc_ + 1, imm))
            return 0;
        Value index = Peek(0, static_cast<int>(imm.sig->parameter_count()), kWasmI32);
        ArgVector args = PeekArgs(imm.sig, 1);
        ReturnVector returns = CreateReturnValues(imm.sig);
        CALL_INTERFACE_IF_OK_AND_REACHABLE(CallIndirect, index, imm, args.begin(), returns.begin());
        Drop(index);
        DropArgs(imm.sig);
        PushReturns(returns);
        return 1 + imm.length;
    ...
    DECODE(ReturnCallIndirect)
    {
        ...
        Value index = Peek(0, 0, kWasmI32);
        ArgVector args = PeekArgs(imm.sig, 1);
        CALL_INTERFACE_IF_OK_AND_REACHABLE(ReturnCallIndirect, index, imm, args.begin());
        Drop(index);
        DropArgs(imm.sig);
        EndControl();
        return 1 + imm.length;
    ...
    DECODE(CallRefDeprecated)
    {
        ...
        if (!VALIDATE(func_type.is_object_reference() && func_type.has_index() && this->module_->has_signature(func_type.ref_index()))) {
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(CallRef, func_ref, sig, func_type.ref_index(), args.begin(), returns.begin());
        Drop(func_ref);
        DropArgs(sig);
        PushReturns(returns);
        return 1;
    ...
    DECODE(CallRef)
    {
        ...
        ArgVector args = PeekArgs(imm.sig, 1);
        ReturnVector returns = CreateReturnValues(imm.sig);
        CALL_INTERFACE_IF_OK_AND_REACHABLE(CallRef, func_ref, imm.sig, imm.index, args.begin(), returns.begin());
        Drop(func_ref);
        DropArgs(imm.sig);
        PushReturns(returns);
        return 1 + imm.length;
    ...
    DECODE(ReturnCallRef)
    {
        ...
        Value func_ref = Peek(0, 0, ValueType::RefNull(imm.index));
        ArgVector args = PeekArgs(imm.sig, 1);
        CALL_INTERFACE_IF_OK_AND_REACHABLE(ReturnCallRef, func_ref, imm.sig, imm.index, args.begin());
        Drop(func_ref);
        DropArgs(imm.sig);
        EndControl();
        return 1 + imm.length;
    ...
    DECODE(Numeric)
    {
        ...
        WasmOpcode full_opcode = this->template read_prefixed_opcode<validate>(this->pc_, &opcode_length, "numeric index");
    ...
    DECODE(Simd)
    {
        ...
        uint32_t opcode_length = 0;
        WasmOpcode full_opcode = this->template read_prefixed_opcode<validate>(this->pc_, &opcode_length);
        if (!VALIDATE(this->ok()))
            return 0;
        trace_msg->AppendOpcode(full_opcode);
    ...
    DECODE(Atomic)
    {
        ...
        uint32_t opcode_length = 0;
        WasmOpcode full_opcode = this->template read_prefixed_opcode<validate>(this->pc_, &opcode_length, "atomic index");
        trace_msg->AppendOpcode(full_opcode);
    ...
    {
        uint32_t opcode_length = 0;
        WasmOpcode full_opcode = this->template read_prefixed_opcode<validate>(this->pc_, &opcode_length, "gc index");
        trace_msg->AppendOpcode(full_opcode);
...
#undef DECODE_IMPL2
    ...
    template <typename func> void InitMerge(Merge<Value>* merge, uint32_t arity, func get_val)
    {
        merge->arity = arity;
        if (arity == 1) {
    ...
    void SetBlockType(Control* c, BlockTypeImmediate<validate>& imm, Value* args)
    {
        const byte* pc = this->pc_;
        InitMerge(&c->end_merge, imm.out_arity(), [pc, &imm](uint32_t i) { return Value { pc, imm.out_type(i) }; });
    ...
    V8_INLINE int EnsureStackArguments(int count)
    {
        ...
        if (V8_LIKELY(stack_size() >= count + limit))
            return 0;
    ...
    V8_INLINE void DropArgs(const StructType* type) { Drop(static_cast<int>(type->field_count())); }
    ...
    Control* PushControl(ControlKind kind, uint32_t drop_values)
    {
        ...
        uint32_t stack_depth = stack_size() >= drop_values ? stack_size() - drop_values : 0;
        ...
        control_.emplace_back(kind, stack_depth, init_stack_depth, this->pc_, reachability);
    ...
    {
        MemoryAccessImmediate<validate> imm = MakeMemoryAccessImmediate(prefix_len, type.size_log_2());
    ...
    int DecodeLoadTransformMem(LoadType type, LoadTransformationKind transform, uint32_t opcode_length)
    {
        ...
        MemoryAccessImmediate<validate> imm = MakeMemoryAccessImmediate(opcode_length, max_alignment);
    ...
    {
        MemoryAccessImmediate<validate> mem_imm = MakeMemoryAccessImmediate(opcode_length, type.size_log_2());
        if (!this->Validate(this->pc_ + opcode_length, mem_imm))
            return 0;
        SimdLaneImmediate<validate> lane_imm(this, this->pc_ + opcode_length + mem_imm.length);
    ...
    {
        MemoryAccessImmediate<validate> imm = MakeMemoryAccessImmediate(prefix_len, store.size_log_2());
        if (!this->Validate(this->pc_ + prefix_len, imm))
            return 0;
    ...
    uint32_t SimdExtractLane(WasmOpcode opcode, ValueType type, uint32_t opcode_length)
    {
        ...
        if (this->Validate(this->pc_ + opcode_length, opcode, imm)) {
            ...
            Value result = CreateValue(type);
            CALL_INTERFACE_IF_OK_AND_REACHABLE(SimdLaneOp, opcode, imm, base::ArrayVector(inputs), &result);
            Drop(1);
            Push(result);
        }
        return opcode_length + imm.length;
    ...
    uint32_t DecodeSimdOpcode(WasmOpcode opcode, uint32_t opcode_length)
    {
        ...
        default: {
            ...
            if (sig->return_count() == 0) {
                CALL_INTERFACE_IF_OK_AND_REACHABLE(SimdOp, opcode, base::VectorOf(args), nullptr);
                DropArgs(sig);
            } else {
                ReturnVector results = CreateReturnValues(sig);
                CALL_INTERFACE_IF_OK_AND_REACHABLE(SimdOp, opcode, base::VectorOf(args), results.begin());
                DropArgs(sig);
                PushReturns(results);
            }
    ...
    {
        bool types_unrelated = !IsSubtypeOf(ValueType::Ref(expected_type), obj.type, this->module_) && !IsSubtypeOf(obj.type, ValueType::RefNull(expected_type), this->module_);
    ...
    bool TypeCheckAlwaysFails(Value obj, Value rtt)
    {
        ...
        return TypeCheckAlwaysFails(obj, HeapType(rtt.type.ref_index()), kNullSucceeds);
    }

    // Checks if {obj} is a subtype of type, thus checking will always
    // succeed.
    bool TypeCheckAlwaysSucceeds(Value obj, HeapType type) { return IsSubtypeOf(obj.type, ValueType::RefNull(type), this->module_); }
    bool TypeCheckAlwaysSucceeds(Value obj, Value rtt) { return TypeCheckAlwaysSucceeds(obj, HeapType(rtt.type.ref_index())); }
...
#define NON_CONST_ONLY                                                                                                                                                             \
    ...
    int DecodeGCOpcode(WasmOpcode opcode, uint32_t opcode_length)
    {
        ...
        case kExprStructNew: {
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(StructNew, imm, rtt, args.begin(), &value);
            Drop(rtt);
            ...
            if (validate) {
                for (uint32_t i = 0; i < imm.struct_type->field_count(); i++) {
                    if (!VALIDATE(imm.struct_type->mutability(i))) {
                        this->DecodeError("%s: struct_type %d has immutable field %d", WasmOpcodes::OpcodeName(opcode), imm.index, i);
                        return 0;
                    }
                    ValueType ftype = imm.struct_type->field(i);
                    if (!VALIDATE(ftype.is_defaultable())) {
                        this->DecodeError("%s: struct type %d has field %d of non-defaultable type %s", WasmOpcodes::OpcodeName(opcode), imm.index, i, ftype.name().c_str());
                        return 0;
                    }
                }
            }
        ...
        case kExprStructGet: {
            ...
            Value struct_obj = Peek(0, 0, ValueType::RefNull(field.struct_imm.index));
        ...
        case kExprStructSet: {
            ...
            Value field_value = Peek(0, 1, struct_type->field(field.field_imm.index).Unpacked());
            Value struct_obj = Peek(1, 0, ValueType::RefNull(field.struct_imm.index));
        ...
        case kExprArrayNew: {
            ...
            Value initial_value = Peek(2, 0, imm.array_type->element_type().Unpacked());
            Value value = CreateValue(ValueType::Ref(imm.index));
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayNew, imm, length, initial_value, rtt, &value);
        ...
        case kExprArrayNewDefault: {
            ...
            if (!VALIDATE(imm.array_type->mutability())) {
                this->DecodeError("%s: array type %d is immutable", WasmOpcodes::OpcodeName(opcode), imm.index);
                return 0;
            }
            if (!VALIDATE(imm.array_type->element_type().is_defaultable())) {
                this->DecodeError(
                    "%s: array type %d has non-defaultable element type %s", WasmOpcodes::OpcodeName(opcode), imm.index, imm.array_type->element_type().name().c_str());
                return 0;
            }
            ValueType rtt_type = ValueType::Rtt(imm.index);
            Value rtt = CreateValue(rtt_type);
            ...
            Value value = CreateValue(ValueType::Ref(imm.index));
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayNewDefault, imm, length, rtt, &value);
            Drop(2); // rtt, length
            Push(value);
            return opcode_length + imm.length;
        }
        case kExprArrayNewData: {
            ArrayIndexImmediate<validate> array_imm(this, this->pc_ + opcode_length);
...
#endif
            const byte* data_index_pc = this->pc_ + opcode_length + array_imm.length;
            IndexImmediate<validate> data_segment(this, data_index_pc, "data segment");
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayNewSegment, array_imm, data_segment, offset, length, rtt, &array);
        ...
        case kExprArrayNewElem: {
            ...
            const byte* elem_index_pc = this->pc_ + opcode_length + array_imm.length;
            IndexImmediate<validate> elem_segment(this, elem_index_pc, "data segment");
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayNewSegment, array_imm, elem_segment, offset, length, rtt, &array);
            Drop(3); // rtt, length, offset
            Push(array);
        ...
        case kExprArrayCopy: {
            ...
            ArrayIndexImmediate<validate> src_imm(this, this->pc_ + opcode_length + dst_imm.length);
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayCopy, dst, dst_index, src, src_index, length);
            Drop(5);
        ...
        case kExprArrayNewFixed: {
            ...
            if (!this->Validate(this->pc_ + opcode_length, array_imm))
                return 0;
            IndexImmediate<validate> length_imm(this, this->pc_ + opcode_length + array_imm.length, "array.new_fixed length");
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(ArrayNewFixed, array_imm, elements, rtt, &result);
            Drop(elem_count + 1);
            Push(result);
        ...
        case kExprRefTest: {
            NON_CONST_ONLY
            HeapTypeImmediate<validate> imm(this->enabled_, this, this->pc_ + opcode_length, this->module_);
            if (!VALIDATE(this->ok()))
                return 0;
            ...
            if (imm.type.is_index()) {
                ...
                CALL_INTERFACE_IF_OK_AND_REACHABLE(RttCanon, imm.type.ref_index(), &rtt.value());
                Push(rtt.value());
            }
            ...
            if (!VALIDATE((obj.type.is_object_reference() && IsSameTypeHierarchy(obj.type.heap_type(), target_type, this->module_)) || obj.type.is_bottom())) {
                this->DecodeError(obj.pc(),
                    ...
                    SafeOpcodeNameAt(obj.pc()), obj.type.name().c_str(), target_type.name().c_str());
                return 0;
            }
            ...
            if (V8_LIKELY(current_code_reachable_and_ok_)) {
                ...
                if (V8_UNLIKELY(TypeCheckAlwaysSucceeds(obj, target_type))) {
                    ...
                    if (rtt.has_value()) {
                        ...
                        CALL_INTERFACE(RefTestAbstract, obj, target_type, &value, null_succeeds);
                    }
                }
            }
        ...
        case kExprRefTestDeprecated: {
            NON_CONST_ONLY
            IndexImmediate<validate> imm(this, this->pc_ + opcode_length, "type index");
            if (!this->ValidateType(this->pc_ + opcode_length, imm))
                return 0;
            opcode_length += imm.length;
            ...
            Value obj = Peek(1);
            Value value = CreateValue(kWasmI32);
            if (!VALIDATE(IsSubtypeOf(obj.type, kWasmFuncRef, this->module_) || IsSubtypeOf(obj.type, kWasmDataRef, this->module_) || obj.type.is_bottom())) {
        ...
        case kExprRefCastNop: {
            ...
            if (!VALIDATE(this->enabled_.has_ref_cast_nop())) {
                this->DecodeError("Invalid opcode 0xfb48 (enable with "
            ...
            Value value = CreateValue(ValueType::RefMaybeNull(imm.index, obj.type.is_bottom() ? kNonNullable : obj.type.nullability()));
        ...
        case kExprBrOnCast: {
            NON_CONST_ONLY
            BranchDepthImmediate<validate> branch_depth(this, this->pc_ + opcode_length);
            ...
            if (V8_LIKELY(current_code_reachable_and_ok_)) {
                ...
                if (V8_UNLIKELY(TypeCheckAlwaysSucceeds(obj, rtt))) {
                    ...
                    CALL_INTERFACE(BrOnCast, obj, rtt, value_on_branch, branch_depth.depth);
                    c->br_merge()->reached = true;
                }
        ...
        case kExprBrOnCastFail: {
            ...
            Control* c = control_at(branch_depth.depth);
            if (c->br_merge()->arity == 0) {
                this->DecodeError("br_on_cast_fail must target a branch of arity at least 1");
                return 0;
            }
            ...
            if (V8_LIKELY(current_code_reachable_and_ok_)) {
                ...
                if (V8_UNLIKELY(TypeCheckAlwaysFails(obj, rtt))) {
                    ...
                    CALL_INTERFACE(BrOnCastFail, obj, rtt, &result_on_fallthrough, branch_depth.depth);
                    c->br_merge()->reached = true;
                }
            }
...
#define ABSTRACT_TYPE_CHECK(h_type)                                                                                                                                                \
    case kExprRefIs##h_type: {                                                                                                                                                     \
        ...
        Value result = CreateValue(kWasmI32);                                                                                                                                      \
        if (V8_LIKELY(current_code_reachable_and_ok_)) {                                                                                                                           \
            if (IsHeapSubtypeOf(arg.type.heap_type(), HeapType(HeapType::k##h_type), this->module_)) {                                                                             \
            ...
            } else if (!IsHeapSubtypeOf(HeapType(HeapType::k##h_type), arg.type.heap_type(), this->module_)) {                                                                     \
                CALL_INTERFACE(Drop);                                                                                                                                              \
...
#define ABSTRACT_TYPE_CAST(h_type)                                                                                                                                                 \
    case kExprRefAs##h_type: {                                                                                                                                                     \
        NON_CONST_ONLY                                                                                                                                                             \
        Value arg = Peek(0, 0, kWasmAnyRef);                                                                                                                                       \
        ValueType non_nullable_abstract_type = ValueType::Ref(HeapType::k##h_type);                                                                                                \
...
#undef ABSTRACT_TYPE_CAST
        ...
        case kExprBrOnI31: {
            ...
            Control* c = control_at(branch_depth.depth);
            if (c->br_merge()->arity == 0) {
                this->DecodeError("%s must target a branch of arity at least 1", SafeOpcodeNameAt(this->pc_));
                return 0;
            }
            ...
            Drop(obj);
            HeapType::Representation heap_type = opcode == kExprBrOnData ? HeapType::kData : opcode == kExprBrOnArray ? HeapType::kArray : HeapType::kI31;
        ...
        case kExprBrOnNonI31: {
            ...
            Value obj = Peek(0, 0, kWasmAnyRef);
            HeapType::Representation heap_type = opcode == kExprBrOnNonData ? HeapType::kData : opcode == kExprBrOnNonArray ? HeapType::kArray : HeapType::kI31;
            Value value_on_fallthrough = CreateValue(ValueType::Ref(heap_type));

            if (V8_LIKELY(current_code_reachable_and_ok_)) {
                if (opcode == kExprBrOnNonData) {
                    CALL_INTERFACE(BrOnNonData, obj, &value_on_fallthrough, branch_depth.depth);
                } else if (opcode == kExprBrOnNonArray) {
                    CALL_INTERFACE(BrOnNonArray, obj, &value_on_fallthrough, branch_depth.depth);
                } else {
                    CALL_INTERFACE(BrOnNonI31, obj, &value_on_fallthrough, branch_depth.depth);
                }
                c->br_merge()->reached = true;
            }
            Drop(obj);
        ...
        case kExprExternInternalize: {
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(UnOp, kExprExternInternalize, extern_val, &intern_val);
            Drop(extern_val);
            Push(intern_val);
            return opcode_length;
        }
        case kExprExternExternalize: {
            ...
            CALL_INTERFACE_IF_OK_AND_REACHABLE(UnOp, kExprExternExternalize, val, &extern_val);
            Drop(val);
            Push(extern_val);
            return opcode_length;
        }
    ...
    int DecodeStringNewWtf8(unibrow::Utf8Variant variant, uint32_t opcode_length)
    {
        ...
        if (!this->Validate(this->pc_ + opcode_length, memory))
            return 0;
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(StringNewWtf8, memory, variant, offset, size, &result);
        Drop(2);
        Push(result);
    ...
    int DecodeStringEncodeWtf8(unibrow::Utf8Variant variant, uint32_t opcode_length)
    {
        ...
        Value result = CreateValue(kWasmI32);
        CALL_INTERFACE_IF_OK_AND_REACHABLE(StringEncodeWtf8, memory, variant, str, addr, &result);
        Drop(2);
        Push(result);
        return opcode_length + memory.length;
    }
    ...
    int DecodeStringViewWtf8Encode(unibrow::Utf8Variant variant, uint32_t opcode_length)
    {
        ...
        CALL_INTERFACE_IF_OK_AND_REACHABLE(StringViewWtf8Encode, memory, variant, view, addr, pos, bytes, &next_pos, &bytes_out);
        Drop(4);
        Push(next_pos);
        Push(bytes_out);
        return opcode_length + memory.length;
...
#undef CASE_ATOMIC_OP
        case kExprAtomicFence: {
            byte zero = this->template read_u8<validate>(this->pc_ + opcode_length, "zero");
            if (!VALIDATE(zero == 0)) {
        ...
        MemoryAccessImmediate<validate> imm = MakeMemoryAccessImmediate(opcode_length, ElementSizeLog2Of(memtype.representation()));
        if (!this->Validate(this->pc_ + opcode_length, imm))
            return false;
        ...
        if (ret_type == kWasmVoid) {
            CALL_INTERFACE_IF_OK_AND_REACHABLE(AtomicOp, opcode, base::VectorOf(args), imm, nullptr);
            DropArgs(sig);
        } else {
            Value result = CreateValue(GetReturnType(sig));
            CALL_INTERFACE_IF_OK_AND_REACHABLE(AtomicOp, opcode, base::VectorOf(args), imm, &result);
            DropArgs(sig);
            Push(result);
        }
        return opcode_length + imm.length;
    ...
    unsigned DecodeNumericOpcode(WasmOpcode opcode, uint32_t opcode_length)
    {
        ...
        case kExprDataDrop: {
            IndexImmediate<validate> imm(this, this->pc_ + opcode_length, "data segment index");
        ...
        case kExprTableInit: {
            ...
            if (!this->Validate(this->pc_ + opcode_length, imm))
                return 0;
            ArgVector args = PeekArgs(sig);
            CALL_INTERFACE_IF_OK_AND_REACHABLE(TableInit, imm, base::VectorOf(args));
            DropArgs(sig);
            return opcode_length + imm.length;
        }
        case kExprElemDrop: {
            IndexImmediate<validate> imm(this, this->pc_ + opcode_length, "element segment index");
        ...
        case kExprTableCopy: {
            ...
            if (!this->Validate(this->pc_ + opcode_length, imm))
                return 0;
            ArgVector args = PeekArgs(sig);
            CALL_INTERFACE_IF_OK_AND_REACHABLE(TableCopy, imm, base::VectorOf(args));
            DropArgs(sig);
            return opcode_length + imm.length;
        }
        case kExprTableGrow: {
            IndexImmediate<validate> imm(this, this->pc_ + opcode_length, "table index");
            if (!this->ValidateTable(this->pc_ + opcode_length, imm))
                return 0;
    ...
    {
        if (V8_LIKELY(stack_capacity_end_ - stack_end_ >= slots_needed))
            return;
    ...
    {
        size_t new_stack_capacity = std::max(size_t { 8 }, base::bits::RoundUpToPowerOfTwo(stack_size() + slots_needed));
        Value* new_stack = this->zone()->template NewArray<Value>(new_stack_capacity);
        if (stack_) {
    ...
    V8_INLINE ReturnVector CreateReturnValues(const FunctionSig* sig)
    {
        ...
        std::transform(sig->returns().begin(), sig->returns().end(), values.begin(), [this](ValueType type) { return CreateValue(type); });
        return values;
    }
    ...
    {
        this->DecodeError(val.pc(), "%s[%d] expected %s, found %s of type %s", SafeOpcodeNameAt(this->pc_), index, expected, SafeOpcodeNameAt(val.pc()), val.type.name().c_str());
    }

    V8_NOINLINE void PopTypeError(int index, Value val, std::string expected) { PopTypeError(index, val, expected.c_str()); }

    V8_NOINLINE void PopTypeError(int index, Value val, ValueType expected) { PopTypeError(index, val, ("type " + expected.name()).c_str()); }
    ...
    V8_INLINE Value Peek(int depth, int index, ValueType expected)
    {
        ...
        if (!VALIDATE(IsSubtypeOf(val.type, expected, this->module_) || val.type == kWasmBottom || expected == kWasmBottom)) {
    ...
    Value PeekPackedArray(uint32_t stack_depth, uint32_t operand_index, ValueType expected_element_type, WasmArrayAccess access)
    {
        ...
        PopTypeError(operand_index, array, (std::string("array of ") + (access == WasmArrayAccess::kWrite ? "mutable " : "") + expected_element_type.name()).c_str());
        return array;
    }

    V8_INLINE void ValidateArgType(ArgVector args, int index, ValueType expected)
    {
    ...
    template <StackElementsCountMode strict_count, bool push_branch_values, MergeType merge_type> bool TypeCheckStackAgainstMerge(uint32_t drop_values, Merge<Value>* merge)
    {
        ...
        if (V8_LIKELY(decoding_mode == kConstantExpression || !control_.back().unreachable())) {
            if (V8_UNLIKELY(strict_count ? actual != drop_values + arity : actual < drop_values + arity)) {
                this->DecodeError("expected %u elements on the stack for %s, found %u", arity, merge_description, actual >= drop_values ? actual - drop_values : 0);
                return false;
            }
        ...
        if (push_branch_values) {
            uint32_t inserted_value_count = static_cast<uint32_t>(EnsureStackArguments(drop_values + arity));
    ...
    {
        if (!VALIDATE((TypeCheckStackAgainstMerge<strict_count, false, merge_type>(0, &control_.front().end_merge)))) {
            return false;
        }
        DCHECK_IMPLIES(current_code_reachable_and_ok_, stack_size() >= this->sig_->return_count());
    ...
    {
        static_assert(validate, "Call this function only within VALIDATE");
        return TypeCheckStackAgainstMerge<kStrictCounting, true, kFallthroughMerge>(0, &control_.back().end_merge);
    }
    ...
    template <bool push_branch_values> bool TypeCheckBranch(Control* c, uint32_t drop_values)
    {
        static_assert(validate, "Call this function only within VALIDATE");
        return TypeCheckStackAgainstMerge<kNonStrictCounting, push_branch_values, kBranchMerge>(drop_values, c->br_merge());
    }
...
#define DEFINE_SIMPLE_SIG_OPERATOR(sig, ...)                                                                                                                                       \
    int BuildSimpleOperator_##sig(WasmOpcode opcode) { return BuildSimpleOperator(opcode, __VA_ARGS__); }
