...
public:
    ...
    explicit Decoder(const base::Vector<const byte> bytes, uint32_t buffer_offset = 0)
    ...
    template <ValidateFlag validate> uint8_t read_u8(const byte* pc, const char* msg = "expected 1 byte") { return read_little_endian<uint8_t, validate>(pc, msg); }

    // Reads a 16-bit unsigned integer (little endian).
    template <ValidateFlag validate> uint16_t read_u16(const byte* pc, const char* msg = "expected 2 bytes") { return read_little_endian<uint16_t, validate>(pc, msg); }

    // Reads a 32-bit unsigned integer (little endian).
    template <ValidateFlag validate> uint32_t read_u32(const byte* pc, const char* msg = "expected 4 bytes") { return read_little_endian<uint32_t, validate>(pc, msg); }

    // Reads a 64-bit unsigned integer (little endian).
    template <ValidateFlag validate> uint64_t read_u64(const byte* pc, const char* msg = "expected 8 bytes") { return read_little_endian<uint64_t, validate>(pc, msg); }

    // Reads a variable-length unsigned integer (little endian).
    template <ValidateFlag validate> uint32_t read_u32v(const byte* pc, uint32_t* length, const char* name = "LEB32")
    {
        return read_leb<uint32_t, validate, kNoTrace>(pc, length, name);
    }

    // Reads a variable-length signed integer (little endian).
    template <ValidateFlag validate> int32_t read_i32v(const byte* pc, uint32_t* length, const char* name = "signed LEB32")
    {
        return read_leb<int32_t, validate, kNoTrace>(pc, length, name);
    }

    // Reads a variable-length unsigned integer (little endian).
    template <ValidateFlag validate> uint64_t read_u64v(const byte* pc, uint32_t* length, const char* name = "LEB64")
    {
        return read_leb<uint64_t, validate, kNoTrace>(pc, length, name);
    }

    // Reads a variable-length signed integer (little endian).
    template <ValidateFlag validate> int64_t read_i64v(const byte* pc, uint32_t* length, const char* name = "signed LEB64")
    {
        return read_leb<int64_t, validate, kNoTrace>(pc, length, name);
    }

    // Reads a variable-length 33-bit signed integer (little endian).
    template <ValidateFlag validate> int64_t read_i33v(const byte* pc, uint32_t* length, const char* name = "signed LEB33")
    {
        return read_leb<int64_t, validate, kNoTrace, 33>(pc, length, name);
    }

    // Convenient overload for callers who don't care about length.
    template <ValidateFlag validate> WasmOpcode read_prefixed_opcode(const byte* pc)
    {
        uint32_t len;
    ...
    template <ValidateFlag validate> WasmOpcode read_prefixed_opcode(const byte* pc, uint32_t* length, const char* name = "prefixed opcode")
    {
        uint32_t index;
        ...
        *length += 1; // Prefix byte.
        ...
        if (index > 0xff)
            return static_cast<WasmOpcode>((*pc) << 12 | index);

        return static_cast<WasmOpcode>((*pc) << 8 | index);
    }

    // Reads a 8-bit unsigned integer (byte) and advances {pc_}.
    uint8_t consume_u8(const char* name = "uint8_t") { return consume_little_endian<uint8_t, kTrace>(name); }
    template <class Tracer> uint8_t consume_u8(const char* name, Tracer& tracer)
    {
    ...
    uint16_t consume_u16(const char* name = "uint16_t") { return consume_little_endian<uint16_t, kTrace>(name); }

    // Reads a single 32-bit unsigned integer (little endian) and advances {pc_}.
    template <class Tracer> uint32_t consume_u32(const char* name, Tracer& tracer)
    {
    ...
    {
        uint32_t length = 0;
        uint32_t result = read_leb<uint32_t, kFullValidation, kTrace>(pc_, &length, name);
        pc_ += length;
        return result;
    }
    template <class Tracer> uint32_t consume_u32v(const char* name, Tracer& tracer)
    {
        uint32_t length = 0;
        uint32_t result = read_leb<uint32_t, kFullValidation, kNoTrace>(pc_, &length, name);
    ...
    {
        uint32_t length = 0;
        int32_t result = read_leb<int32_t, kFullValidation, kTrace>(pc_, &length, name);
        pc_ += length;
        return result;
    }

    // Reads a LEB128 variable-length unsigned 64-bit integer and advances {pc_}.
    template <class Tracer> uint64_t consume_u64v(const char* name, Tracer& tracer)
    {
        uint32_t length = 0;
        uint64_t result = read_leb<uint64_t, kFullValidation, kNoTrace>(pc_, &length, name);
        tracer.Bytes(pc_, length);
        tracer.Description(name);
        pc_ += length;
        return result;
    ...
    {
        uint32_t length = 0;
        int64_t result = read_leb<int64_t, kFullValidation, kTrace>(pc_, &length, name);
        pc_ += length;
        return result;
    }
    ...
    template <class Tracer> void consume_bytes(uint32_t size, const char* name, Tracer& tracer)
    {
    ...
    void V8_NOINLINE error(const uint8_t* pc, const char* msg) { errorf(pc_offset(pc), "%s", msg); }
    void V8_NOINLINE error(uint32_t offset, const char* msg) { errorf(offset, "%s", msg); }
    ...
    void V8_NOINLINE PRINTF_FORMAT(3, 4) errorf(uint32_t offset, const char* format, ...)
    {
        va_list args;
        va_start(args, format);
    ...
    void V8_NOINLINE PRINTF_FORMAT(3, 4) errorf(const uint8_t* pc, const char* format, ...)
    {
        va_list args;
        va_start(args, format);
    ...
    void Reset(base::Vector<const uint8_t> bytes, uint32_t buffer_offset = 0) { Reset(bytes.begin(), bytes.end(), buffer_offset); }
    ...
    uint32_t V8_INLINE position() const { return static_cast<uint32_t>(pc_ - start_); }
...
private:
    ...
    template <typename IntType, ValidateFlag validate> IntType read_little_endian(const byte* pc, const char* msg)
    {
        if (!validate) {
    ...
    template <typename IntType, ValidateFlag validate, TraceFlag trace, size_t size_in_bits = 8 * sizeof(IntType)>
    V8_INLINE IntType read_leb(const byte* pc, uint32_t* length, const char* name = "varint")
    {
        ...
        return read_leb_slowpath<IntType, validate, trace, size_in_bits>(pc, length, name);
    }

    template <typename IntType, ValidateFlag validate, TraceFlag trace, size_t size_in_bits = 8 * sizeof(IntType)>
    V8_NOINLINE IntType read_leb_slowpath(const byte* pc, uint32_t* length, const char* name)
    {
        // Create an unrolled LEB decoding function per integer type.
        return read_leb_tail<IntType, validate, trace, size_in_bits, 0>(pc, length, name, 0);
    }

    template <typename IntType, ValidateFlag validate, TraceFlag trace, size_t size_in_bits, int byte_index>
    V8_INLINE IntType read_leb_tail(const byte* pc, uint32_t* length, const char* name, IntType result)
    {
        ...
        if (!is_last_byte && (b & 0x80)) {
            ...
            return read_leb_tail<IntType, validate, trace, size_in_bits, next_byte_index>(pc + 1, length, name, result);
        }
