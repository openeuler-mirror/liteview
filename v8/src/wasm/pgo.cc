...
private:
    ...
    {
        std::unordered_map<uint32_t, FunctionTypeFeedback>& feedback_for_function = module_->type_feedback.feedback_for_function;
...
void RestoreProfileData(WasmModule* module, base::Vector<uint8_t> profile_data)
{
...
void DumpProfileToFile(const WasmModule* module, base::Vector<const uint8_t> wire_bytes)
{
...
void LoadProfileFromFile(WasmModule* module, base::Vector<const uint8_t> wire_bytes)
{
    CHECK(!wire_bytes.empty());
    // File are named `profile-wasm-<hash>`.
    // We use the same hash as for reported scripts, to make it easier to
    ...
    DCHECK_EQ(profile_data.as_vector(), ProfileGenerator { module }.GetProfileData().as_vector());
}
