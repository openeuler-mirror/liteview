...
public:
    ...
    std::shared_ptr<NativeModule> MaybeGetNativeModule(ModuleOrigin origin, base::Vector<const uint8_t> wire_bytes);
...
public:
    ...
    // validation error.
    bool SyncValidate(Isolate* isolate, const WasmFeatures& enabled, const ModuleWireBytes& bytes, std::string* error_message = nullptr);

    // Synchronously compiles the given bytes that represent a translated
    // asm.js module.
    MaybeHandle<AsmWasmData> SyncCompileTranslatedAsmJs(Isolate* isolate, ErrorThrower* thrower, const ModuleWireBytes& bytes, base::Vector<const byte> asm_js_offset_table_bytes,
    ...
    void CompileFunction(Isolate* isolate, NativeModule* native_module, uint32_t function_index, ExecutionTier tier);
    ...
    std::shared_ptr<NativeModule> NewNativeModule(Isolate* isolate, const WasmFeatures& enabled_features, std::shared_ptr<const WasmModule> module, size_t code_size_estimate);
    ...
    std::shared_ptr<NativeModule> MaybeGetNativeModule(ModuleOrigin origin, base::Vector<const uint8_t> wire_bytes, Isolate* isolate);
    ...
    bool UpdateNativeModuleCache(bool error, std::shared_ptr<NativeModule>* native_module, Isolate* isolate);
...
private:
    ...
    AsyncCompileJob* CreateAsyncCompileJob(Isolate* isolate, const WasmFeatures& enabled, std::unique_ptr<byte[]> bytes_copy, size_t length, Handle<Context> context,
