...
//     nullptr;
SimpleTlsPointerXp<NativeModule> CodeSpaceWriteScope::current_native_module_;
...
CodeSpaceWriteScope::CodeSpaceWriteScope(NativeModule* native_module)
    : previous_native_module_(current_native_module_.Get())
{
    if (!native_module) {
    ...
    if (previous_native_module_ == native_module)
        return;
...
{
    if (previous_native_module_ == current_native_module_.Get())
        return;
    if (previous_native_module_ == nullptr || SwitchingPerNativeModule()) {
        SetExecutable();
    }
...
// static
void CodeSpaceWriteScope::SetExecutable() { RwxMemoryWriteScope::SetExecutable(); }
...
// static
bool CodeSpaceWriteScope::SwitchingPerNativeModule() { return !WasmCodeManager::MemoryProtectionKeysEnabled() && v8_flags.wasm_write_protect_code_memory; }
