...
void RecompileNativeModule(NativeModule* native_module, TieringState new_tiering_state);
...
V8_EXPORT_PRIVATE void TriggerTierUp(WasmInstanceObject instance, int func_index);
// Synchronous version of the above.
void TierUpNowForTesting(Isolate* isolate, WasmInstanceObject instance, int func_index);

template <typename Key, typename KeyInfo, typename Hash> class WrapperQueue {
public:
    ...
    // successful.
    // Not thread-safe.
    bool insert(const Key& key, KeyInfo key_info) { return queue_.insert({ key, key_info }).second; }
...
public:
    AsyncCompileJob(Isolate* isolate, const WasmFeatures& enabled_features, std::unique_ptr<byte[]> bytes_copy, size_t length, Handle<Context> context,
...
private:
    ...
    V8_WARN_UNUSED_RESULT bool DecrementAndCheckFinisherCount(FinishingComponent component)
    {
        ...
        if (streaming_until_finished_timer_.IsStarted()) {
            ...
            int elapsed = static_cast<int>(streaming_until_finished_timer_.Elapsed().InMilliseconds());
            if (component == kStreamingDecoder) {
                isolate_->counters()->wasm_compilation_until_streaming_finished()->AddSample(elapsed);
            } else {
                isolate_->counters()->wasm_streaming_until_compilation_finished()->AddSample(elapsed);
            }
        }
