...
class StackTransferRecipe {
    ...
    struct RegisterLoad {
        ...
        static RegisterLoad Stack(int32_t offset, ValueKind kind) { return { kStack, kind, offset }; }
        static RegisterLoad HalfStack(int32_t offset, RegPairHalf half) { return { half == kLowWord ? kLowHalfStack : kHighHalfStack, kI32, offset }; }
...
public:
    ...
    V8_INLINE void LoadIntoRegister(LiftoffRegister dst, const LiftoffAssembler::VarState& src, uint32_t src_offset)
    {
        switch (src.loc()) {
        case VarState::kStack:
    ...
    void LoadI64HalfIntoRegister(LiftoffRegister dst, const LiftoffAssembler::VarState& src, int offset, RegPairHalf half)
    {
    ...
    void LoadStackSlot(LiftoffRegister dst, uint32_t stack_offset, ValueKind kind)
    {
...
private:
    ...
    RegisterMove* register_move(LiftoffRegister reg) { return reinterpret_cast<RegisterMove*>(&register_moves_) + reg.liftoff_code(); }
    RegisterLoad* register_load(LiftoffRegister reg) { return reinterpret_cast<RegisterLoad*>(&register_loads_) + reg.liftoff_code(); }
    int* src_reg_use_count(LiftoffRegister reg) { return src_reg_use_count_ + reg.liftoff_code(); }
...
void InitMergeRegion(LiftoffAssembler::CacheState* state, const VarState* source, VarState* target, uint32_t count, MergeKeepStackSlots keep_stack_slots,
    MergeAllowConstants allow_constants, MergeAllowRegisters allow_registers, ReuseRegisters reuse_registers, LiftoffRegList used_regs)
{
...
void LiftoffAssembler::CacheState::InitMerge(const CacheState& source, uint32_t num_locals, uint32_t arity, uint32_t stack_depth)
{
    ...
    if (allow_registers) {
        for (auto& src : base::VectorOf(source_begin + stack_base + discarded, arity)) {
            if (src.is_reg())
                used_regs.set(src.reg());
        }
    }
    ...
    InitMergeRegion(
        this, source_begin + stack_base + discarded, target_begin + stack_base, arity, keep_merge_stack_slots, kConstantsNotAllowed, allow_registers, kNoReuseRegisters, used_regs);
    // Shift spill offsets down to keep slots contiguous.
    int offset = stack_base == 0 ? StaticStackFrameSize() : source.stack_state[stack_base - 1].offset();
    ...
    InitMergeRegion(this, source_begin, target_begin, num_locals, kKeepStackSlots, kConstantsNotAllowed, kRegistersAllowed, kNoReuseRegisters, used_regs);
    ...
    InitMergeRegion(this, source_begin + num_locals, target_begin + num_locals, stack_depth, kKeepStackSlots, kConstantsAllowed, kRegistersAllowed, kReuseRegisters, used_regs);
}
...
LiftoffAssembler::LiftoffAssembler(std::unique_ptr<AssemblerBuffer> buffer)
    : TurboAssembler(nullptr, DefaultLiftoffOptions(), CodeObjectRequired::kNo, std::move(buffer))
{
...
LiftoffRegister LiftoffAssembler::LoadToRegister(VarState slot, LiftoffRegList pinned)
{
    if (slot.is_reg())
        return slot.reg();
...
LiftoffRegister LiftoffAssembler::LoadToRegister(VarState slot, LiftoffRegister reg)
{
    if (slot.is_const()) {
...
LiftoffRegister LiftoffAssembler::LoadI64HalfIntoRegister(VarState slot, RegPairHalf half)
{
    if (slot.is_reg()) {
...
LiftoffRegister LiftoffAssembler::PeekToRegister(int index, LiftoffRegList pinned)
{
...
void LiftoffAssembler::MergeFullStackWith(CacheState& target, const CacheState& source)
{
    ...
    for (uint32_t i = 0, e = source.stack_height(); i < e; ++i) {
        ...
        DCHECK(!SlotInterference(target.stack_state[i], base::VectorOf(source.stack_state.data() + i + 1, source.stack_height() - i - 1)));
    }
...
void LiftoffAssembler::MergeStackWith(CacheState& target, uint32_t arity, JumpDirection jump_direction)
{
    ...
    for (uint32_t i = 0; i < target_stack_base; ++i) {
        transfers.TransferStackSlot(target.stack_state[i], cache_state_.stack_state[i]);
    ...
    for (uint32_t i = 0; i < arity; ++i) {
        transfers.TransferStackSlot(target.stack_state[target_stack_base + i], cache_state_.stack_state[stack_base + i]);
        DCHECK(!SlotInterference(target.stack_state[target_stack_base + i], base::VectorOf(cache_state_.stack_state.data() + stack_base + i + 1, arity - i - 1)));
    }
    ...
    for (auto tuple : { std::make_tuple(&reload_instance, cache_state_.cached_instance, &target.cached_instance),
             std::make_tuple(&reload_mem_start, cache_state_.cached_mem_start, &target.cached_mem_start) }) {
        ...
        // needs to be done.
        if (src_reg == *dst_reg || *dst_reg == no_reg)
            continue;
        ...
        if (jump_direction == kForwardJump) {
            ...
            transfers.MoveRegister(LiftoffRegister { *dst_reg }, LiftoffRegister { src_reg }, kPointerKind);
        } else {
    ...
    if (reload_mem_start) {
        ...
        LoadFromInstance(target.cached_mem_start, instance, ObjectAccess::ToTagged(WasmInstanceObject::kMemoryStartOffset), sizeof(size_t));
...
void PrepareStackTransfers(const ValueKindSig* sig, compiler::CallDescriptor* call_descriptor, const VarState* slots, LiftoffStackSlots* stack_slots,
    StackTransferRecipe* stack_transfers, LiftoffRegList* param_regs)
{
    ...
    for (uint32_t i = num_params; i > 0; --i) {
        ...
        for (int lowered_idx = 0; lowered_idx < num_lowered_params; ++lowered_idx) {
            ...
            if (loc.IsRegister()) {
                ...
                param_regs->set(reg);
                if (is_gp_pair) {
                    stack_transfers->LoadI64HalfIntoRegister(reg, slot, stack_offset, half);
                } else {
...
void LiftoffAssembler::PrepareBuiltinCall(const ValueKindSig* sig, compiler::CallDescriptor* call_descriptor, std::initializer_list<VarState> params)
{
    ...
    PrepareStackTransfers(sig, call_descriptor, params.begin(), &stack_slots, &stack_transfers, &param_regs);
    SpillAllRegisters();
...
void LiftoffAssembler::PrepareCall(const ValueKindSig* sig, compiler::CallDescriptor* call_descriptor, Register* target, Register* target_instance)
{
    uint32_t num_params = static_cast<uint32_t>(sig->parameter_count());
    ...
    cache_state_.ClearAllCacheRegisters();
    for (VarState* it = cache_state_.stack_state.end() - 1 - num_params; it >= cache_state_.stack_state.begin() && !cache_state_.used_registers.is_empty(); --it) {
        if (!it->is_reg())
            continue;
    ...
    if (target_instance && *target_instance != instance_reg) {
        stack_transfers.MoveRegister(LiftoffRegister(instance_reg), LiftoffRegister(*target_instance), kPointerKind);
    }

    int param_slots = static_cast<int>(call_descriptor->ParameterSlotCount());
    if (num_params) {
        uint32_t param_base = cache_state_.stack_height() - num_params;
        PrepareStackTransfers(sig, call_descriptor, &cache_state_.stack_state[param_base], &stack_slots, &stack_transfers, &param_regs);
    }
    ...
    if (target && param_regs.has(LiftoffRegister(*target))) {
        ...
        if (!free_regs.is_empty()) {
            ...
            stack_transfers.MoveRegister(new_target, LiftoffRegister(*target), kPointerKind);
            *target = new_target.gp();
        } else {
            stack_slots.Add(VarState(kPointerKind, LiftoffRegister(*target), 0), param_slots);
            param_slots++;
            *target = no_reg;
        }
    }
...
void LiftoffAssembler::FinishCall(const ValueKindSig* sig, compiler::CallDescriptor* call_descriptor)
{
    ...
    for (ValueKind return_kind : sig->returns()) {
        ...
        for (int pair_idx = 0; pair_idx < num_lowered_params; ++pair_idx) {
            compiler::LinkageLocation loc = call_descriptor->GetReturnLocation(call_desc_return_idx++);
            if (loc.IsRegister()) {
                DCHECK(!loc.IsAnyRegister());
...
void LiftoffAssembler::MoveToReturnLocations(const FunctionSig* sig, compiler::CallDescriptor* descriptor)
{
    ...
    if (sig->return_count() == 1) {
        ...
        stack_transfers.LoadIntoRegister(return_reg, cache_state_.stack_state.back(), cache_state_.stack_state.back().offset());
        return;
    }
    ...
    for (size_t i = 0; i < sig->return_count(); ++i) {
        ...
        for (int pair_idx = 0; pair_idx < num_lowered_params; ++pair_idx) {
            compiler::LinkageLocation loc = descriptor->GetReturnLocation(call_desc_return_idx++);
            if (loc.IsCallerFrameSlot()) {
                RegPairHalf half = pair_idx == 0 ? kLowWord : kHighWord;
                VarState& slot = slots[i];
                LiftoffRegister reg = needs_gp_pair ? LoadI64HalfIntoRegister(slot, half) : LoadToRegister(slot, {});
    ...
    for (size_t i = 0; i < sig->return_count(); ++i) {
        ...
        for (int pair_idx = 0; pair_idx < num_lowered_params; ++pair_idx) {
            ...
            if (loc.IsRegister()) {
                ...
                VarState& slot = slots[i];
                if (needs_gp_pair) {
                    stack_transfers.LoadI64HalfIntoRegister(reg, slot, slot.offset(), half);
                } else {
