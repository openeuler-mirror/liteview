...
#define WASM_INSTANCE_OBJECT_FIELD_OFFSET(name) ObjectAccess::ToTagged(WasmInstanceObject::k##name##Offset)
...
#define WASM_INSTANCE_OBJECT_FIELD_SIZE(name) FIELD_SIZE(WasmInstanceObject::k##name##Offset)

#define LOAD_INSTANCE_FIELD(dst, name, load_size, pinned)                                                                                                                          \
    __ LoadFromInstance(                                                                                                                                                           \
        dst, LoadInstanceIntoRegister(pinned, dst), WASM_INSTANCE_OBJECT_FIELD_OFFSET(name), assert_field_size<WASM_INSTANCE_OBJECT_FIELD_SIZE(name), load_size>::size);

#define LOAD_TAGGED_PTR_INSTANCE_FIELD(dst, name, pinned)                                                                                                                          \
    static_assert(WASM_INSTANCE_OBJECT_FIELD_SIZE(name) == kTaggedSize, "field in WasmInstance does not have the expected size");                                                  \
    __ LoadTaggedPointerFromInstance(dst, LoadInstanceIntoRegister(pinned, dst), WASM_INSTANCE_OBJECT_FIELD_OFFSET(name));
...
public:
    ...
    public:
        ...
        Entry ToTableEntry() { return Entry { pc_offset_, stack_height_, std::move(changed_values_) }; }
...
private:
    static std::vector<Value> GetChangedStackValues(std::vector<Value>& last_values, base::Vector<DebugSideTable::Entry::Value> values)
    {
...
#if V8_TARGET_ARCH_MIPS64 || V8_TARGET_ARCH_S390X || V8_TARGET_ARCH_PPC || V8_TARGET_ARCH_PPC64 || V8_TARGET_ARCH_LOONG64
    return;
#endif
...
#define LIST_FEATURE(name, ...) kFeature_##name,
    constexpr WasmFeatures kExperimentalFeatures { FOREACH_WASM_EXPERIMENTAL_FEATURE_FLAG(LIST_FEATURE) };
#undef LIST_FEATURE
...
public:
    ...
    struct OutOfLineCode {
        ...
        // Named constructors:
        static OutOfLineCode Trap(WasmCode::RuntimeStubId s, WasmCodePosition pos, SpilledRegistersForInspection* spilled_registers, OutOfLineSafepointInfo* safepoint_info,
            uint32_t pc, DebugSideTableBuilder::EntryBuilder* debug_sidetable_entry_builder)
        {
            DCHECK_LT(0, pos);
            return {
                {}, // label
        ...
        static OutOfLineCode StackCheck(WasmCodePosition pos, LiftoffRegList regs_to_save, Register cached_instance, SpilledRegistersForInspection* spilled_regs,
        ...
        static OutOfLineCode TierupCheck(WasmCodePosition pos, LiftoffRegList regs_to_save, Register cached_instance, SpilledRegistersForInspection* spilled_regs,
            OutOfLineSafepointInfo* safepoint_info, DebugSideTableBuilder::EntryBuilder* debug_sidetable_entry_builder)
        {
            return {
                {}, // label
    ...
    LiftoffCompiler(compiler::CallDescriptor* call_descriptor, CompilationEnv* env, Zone* compilation_zone, std::unique_ptr<AssemblerBuffer> buffer,
        DebugSideTableBuilder* debug_sidetable_builder, const LiftoffOptions& options)
        : asm_(std::move(buffer))
        , descriptor_(GetLoweredCallDescriptor(compilation_zone, call_descriptor))
        , env_(env)
    ...
    void GetCode(CodeDesc* desc) { asm_.GetCode(nullptr, desc, &safepoint_table_builder_, handler_table_offset_); }

    std::unique_ptr<AssemblerBuffer> ReleaseBuffer() { return asm_.ReleaseBuffer(); }

    base::OwnedVector<uint8_t> GetSourcePositionTable() { return source_position_table_builder_.ToSourcePositionTableVector(); }
    ...
    uint32_t GetTotalFrameSlotCountForGC() const { return __ GetTotalFrameSlotCountForGC(); }
    ...
    private:
        ...
        void NextLocation() { location_ = compiler_->descriptor_->GetInputLocation(input_idx_++); }
        ...
        ValueKind reg_kind_;
        RegClass rc_;
        compiler::LinkageLocation location_ { compiler::LinkageLocation::ForAnyRegister() };
    ...
    void StackCheck(FullDecoder* decoder, WasmCodePosition position)
    {
        ...
        OutOfLineSafepointInfo* safepoint_info = compilation_zone_->New<OutOfLineSafepointInfo>(compilation_zone_);
        ...
        out_of_line_code_.push_back(
            OutOfLineCode::StackCheck(position, regs_to_save, __ cache_state()->cached_instance, spilled_regs, safepoint_info, RegisterOOLDebugSideTableEntry(decoder)));
        OutOfLineCode& ool = out_of_line_code_.back();
        __ StackCheck(ool.label.get(), limit_address);
        __ bind(ool.continuation.get());
    }

    void TierupCheck(FullDecoder* decoder, WasmCodePosition position, int budget_used, Register tmp1, Register tmp2)
    {
        ...
        if (for_debugging_ != kNoDebugging)
            return;
        ...
        constexpr int kArrayOffset = WASM_INSTANCE_OBJECT_FIELD_OFFSET(TieringBudgetArray);
        static_assert(WASM_INSTANCE_OBJECT_FIELD_SIZE(TieringBudgetArray) == kArraySize);
        Register array_reg = tmp1; // Overwriting {instance}.
        __ LoadFromInstance(array_reg, instance, kArrayOffset, kArraySize);
        uint32_t offset = kInt32Size * declared_function_index(env_->module, func_index_);
        ...
        out_of_line_code_.push_back(
            OutOfLineCode::TierupCheck(position, regs_to_save, __ cache_state()->cached_instance, spilled_regs, safepoint_info, RegisterOOLDebugSideTableEntry(decoder)));
        OutOfLineCode& ool = out_of_line_code_.back();
        FREEZE_STATE(trapping);
        __ emit_i32_subi_jump_negative(budget_reg.gp(), budget_used, ool.label.get(), trapping);
    ...
    {
        return env_->dynamic_tiering && for_debugging_ == kNoDebugging && (v8_flags.wasm_tier_up_filter == -1 || v8_flags.wasm_tier_up_filter == func_index_);
    }
    ...
    void StartFunctionBody(FullDecoder* decoder, Control* block)
    {
        ...
        DCHECK_EQ(kWasmInstanceRegister, Register::from_code(descriptor_->GetInputLocation(kInstanceParameterIndex).AsRegister()));
        ...
        if (v8_flags.wasm_speculative_inlining) {
            ...
            __ LoadTaggedPointerFromInstance(tmp.gp(), kWasmInstanceRegister, WASM_INSTANCE_OBJECT_FIELD_OFFSET(FeedbackVectors));
            __ LoadTaggedPointer(tmp.gp(), tmp.gp(), no_reg, wasm::ObjectAccess::ElementOffsetInTaggedFixedArray(declared_func_index));
            ...
            if (has_refs) {
                ...
                for (uint32_t local_index = num_params; local_index < __ num_locals(); ++local_index) {
                    ...
                    if (is_reference(kind)) {
                        __ Spill(__ cache_state()->stack_state[local_index].offset(), LiftoffRegister(null_ref_reg), kind);
                    }
                }
            }
        } else {
    ...
    {
        CODE_COMMENT((std::string("OOL: ") + GetRuntimeStubName(ool->stub)).c_str());
        ...
        // do not set {pc}.
        DCHECK_IMPLIES(ool->stub != WasmCode::kThrowWasmTrapMemOutOfBounds, ool->pc == 0);

        if (env_->bounds_checks == kTrapHandler && ool->pc != 0) {
            uint32_t pc = static_cast<uint32_t>(__ pc_offset());
            DCHECK_EQ(pc, __ pc_offset());
            protected_instructions_.emplace_back(trap_handler::ProtectedInstructionData { ool->pc, pc });
        }
        ...
        if (ool->safepoint_info) {
            ...
            __ RecordSpillsInSafepoint(safepoint, gp_regs, ool->safepoint_info->spills, index);
        }
    ...
    void FinishFunction(FullDecoder* decoder)
    {
        ...
        __ PatchPrepareStackFrame(pc_offset_stack_frame_construction_, &safepoint_table_builder_);
        __ FinishCode();
        ...
        if (v8_flags.wasm_speculative_inlining && !encountered_call_instructions_.empty()) {
            ...
            base::OwnedVector<uint32_t>& call_targets = type_feedback.feedback_for_function[func_index_].call_targets;
    ...
    void NextInstruction(FullDecoder* decoder, WasmOpcode opcode)
    {
        ...
        CODE_COMMENT(WasmOpcodes::OpcodeName(WasmOpcodes::IsPrefixOpcode(opcode) ? decoder->read_prefixed_opcode<Decoder::kFullValidation>(decoder->pc()) : opcode));
    }
    ...
    LiftoffRegister GetExceptionProperty(LiftoffAssembler::VarState& exception, RootIndex root_index)
    {
        ...
        CallRuntimeStub(
            WasmCode::kWasmGetOwnProperty, MakeSig::Returns(kPointerKind).Params(kPointerKind, kPointerKind, kPointerKind), { exception, tag_symbol, context }, kNoSourcePosition);

        return LiftoffRegister(kReturnRegister0);
    }

    void CatchException(FullDecoder* decoder, const TagIndexImmediate<validate>& imm, Control* block, base::Vector<Value> values)
    {
        ...
        CODE_COMMENT("compare tags");
        {
            FREEZE_STATE(frozen);
            Label caught;
            __ emit_cond_jump(kEqual, &caught, kI32, imm_tag, caught_tag.gp(), frozen);
    ...
    void Rethrow(FullDecoder* decoder, const LiftoffAssembler::VarState& exception)
    {
        DCHECK_EQ(exception.kind(), kRef);
        CallRuntimeStub(WasmCode::kWasmRethrow, MakeSig::Params(kPointerKind), { exception }, decoder->position());
    }
        ...
        if (block->try_info->catch_reached) {
            ...
            if (depth == decoder->control_depth() - 1) {
                ...
                if (!target->try_info->catch_reached) {
                    target->try_info->catch_state.InitMerge(*__ cache_state(), __ num_locals(), 1, target->stack_depth + target->num_exceptions);
    ...
    void JumpIfFalse(FullDecoder* decoder, Label* false_dst, std::unique_ptr<FreezeCacheState>& will_freeze)
    {
        LiftoffCondition cond = test_and_reset_outstanding_op(kExprI32Eqz) ? kNotEqualZero : kEqualZero;

        if (!has_outstanding_op()) {
            // Unary comparison.
            Register value = __ PopToRegister().gp();
            will_freeze.reset(new FreezeCacheState(asm_));
            __ emit_cond_jump(cond, false_dst, kI32, value, no_reg, *will_freeze.get());
            return;
        }
        ...
        if (lhs_slot.is_const()) {
            ...
            will_freeze.reset(new FreezeCacheState(asm_));
            __ emit_i32_cond_jumpi(Flip(cond), false_dst, rhs, lhs_imm, *will_freeze.get());
            return;
        }
    ...
    void FallThruTo(FullDecoder* decoder, Control* c)
    {
        if (!c->end_merge.reached) {
            c->label_state.InitMerge(*__ cache_state(), __ num_locals(), c->end_merge.arity, c->stack_depth + c->num_exceptions);
        }
    ...
    void FinishOneArmedIf(FullDecoder* decoder, Control* c)
    {
        ...
        if (c->end_merge.reached) {
            ...
            c->label_state.InitMerge(c->else_state->state, __ num_locals(), c->start_merge.arity, c->stack_depth + c->num_exceptions);
            __ MergeFullStackWith(c->label_state, *__ cache_state());
            __ emit_jump(c->label.get());
            // Merge the else state into the end state. Set this state as the current
            // state first so helper functions know which registers are in use.
    ...
    void GenerateCCall(const LiftoffRegister* result_regs, const ValueKindSig* sig, ValueKind out_argument_kind, const LiftoffRegister* arg_regs, ExternalReference ext_ref)
    {
        ...
        __ CallC(sig, arg_regs, result_regs, out_argument_kind, stack_bytes, ext_ref);
    }

    template <typename EmitFn, typename... Args> typename std::enable_if<!std::is_member_function_pointer<EmitFn>::value>::type CallEmitFn(EmitFn fn, Args... args) { fn(args...); }

    template <typename EmitFn, typename... Args> typename std::enable_if<std::is_member_function_pointer<EmitFn>::value>::type CallEmitFn(EmitFn fn, Args... args)
    {
    ...
    // unchanged.
    template <typename T> typename std::conditional<std::is_same<LiftoffRegister, T>::value, AssemblerRegisterConverter, T>::type ConvertAssemblerArg(T t) { return { t }; }
    ...
    template <typename EmitFn, typename ArgType> EmitFnWithFirstArg<EmitFn, ArgType> BindFirst(EmitFn fn, ArgType arg) { return { fn, arg }; }
    ...
    template <ValueKind kind> void EmitFloatUnOpWithCFallback(bool (LiftoffAssembler::*emit_fn)(DoubleRegister, DoubleRegister), ExternalReference (*fallback_fn)())
    {
    ...
    void EmitTypeConversion(FullDecoder* decoder, WasmOpcode opcode, ExternalReference (*fallback_fn)())
    {
        ...
        Label* trap = can_trap ? AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapFloatUnrepresentable) : nullptr;
...
#define CASE_TYPE_CONVERSION(opcode, dst_kind, src_kind, ext_ref, can_trap)                                                                                                        \
    case kExpr##opcode:                                                                                                                                                            \
        ...
        case kExprI32Popcnt:
            return EmitUnOp<kI32, kI32>([=](LiftoffRegister dst, LiftoffRegister src) {
                if (__ emit_i32_popcnt(dst.gp(), src.gp()))
                    return;
                auto sig = MakeSig::Returns(kI32).Params(kI32);
                GenerateCCall(&dst, &sig, kVoid, &src, ExternalReference::wasm_word32_popcnt());
            });
        case kExprI64Popcnt:
            return EmitUnOp<kI64, kI64>([=](LiftoffRegister dst, LiftoffRegister src) {
                ...
                GenerateCCall(&c_call_dst, &sig, kVoid, &src, ExternalReference::wasm_word64_popcnt());
                // Now zero-extend the result to i64.
                __ emit_type_conversion(kExprI64UConvertI32, dst, c_call_dst, nullptr);
            });
        case kExprRefIsNull:
        ...
        case kExprRefAsNonNull: {
            ...
            __ emit_ptrsize_set_cond(opcode == kExprRefIsNull ? kEqual : kUnequal, dst.gp(), ref, null);
        ...
        case kExprExternInternalize:
            if (!v8_flags.wasm_gc_js_interop) {
                ...
                LiftoffRegister context_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
                LOAD_TAGGED_PTR_INSTANCE_FIELD(context_reg.gp(), NativeContext, pinned);
                LiftoffAssembler::VarState& extern_value = __ cache_state()->stack_state.back();

                LiftoffAssembler::VarState context(kPointerKind, context_reg, 0);

                CallRuntimeStub(
                    WasmCode::kWasmExternInternalize, MakeSig::Returns(kPointerKind).Params(kPointerKind, kPointerKind), { extern_value, context }, decoder->position());
                __ DropValues(1);
        ...
        case kExprExternExternalize:
            if (!v8_flags.wasm_gc_js_interop) {
                ...
                LiftoffRegister context_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
                LOAD_TAGGED_PTR_INSTANCE_FIELD(context_reg.gp(), NativeContext, pinned);
                LiftoffAssembler::VarState& value = __ cache_state()->stack_state.back();

                LiftoffAssembler::VarState context(kPointerKind, context_reg, 0);

                CallRuntimeStub(WasmCode::kWasmExternExternalize, MakeSig::Returns(kPointerKind).Params(kPointerKind, kPointerKind), { value, context }, decoder->position());
                __ DropValues(1);
                __ PushRegister(kRefNull, LiftoffRegister(kReturnRegister0));
            }
            return;
...
#undef CASE_TYPE_CONVERSION
    ...
    template <ValueKind src_kind, ValueKind result_kind, bool swap_lhs_rhs = false, ValueKind result_lane_kind = kVoid, typename EmitFn> void EmitBinOp(EmitFn fn)
    {
        static constexpr RegClass src_rc = reg_class_for(src_kind);
        static constexpr RegClass result_rc = reg_class_for(result_kind);
    ...
    void EmitDivOrRem64CCall(LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs, ExternalReference ext_ref, Label* trap_by_zero, Label* trap_unrepresentable = nullptr)
    {
        // Cannot emit native instructions, build C call.
        LiftoffRegister ret = __ GetUnusedRegister(kGpReg, LiftoffRegList { dst });
        LiftoffRegister tmp = __ GetUnusedRegister(kGpReg, LiftoffRegList { dst, ret });
        ...
        if (trap_unrepresentable) {
            ...
            __ emit_cond_jump(kEqual, trap_unrepresentable, kI32, ret.gp(), tmp.gp(), trapping);
        }
    }
...
#define CASE_I64_SHIFTOP(opcode, fn)                                                                                                                                               \
    case kExpr##opcode:                                                                                                                                                            \
        return EmitBinOpImm<kI64, kI64>(                                                                                                                                           \
            [=](LiftoffRegister dst, LiftoffRegister src, LiftoffRegister amount) { __ emit_##fn(dst, src, amount.is_gp_pair() ? amount.low_gp() : amount.gp()); },                \
            &LiftoffAssembler::emit_##fn##i);
#define CASE_CCALL_BINOP(opcode, kind, ext_ref_fn)                                                                                                                                 \
    case kExpr##opcode:                                                                                                                                                            \
        return EmitBinOp<k##kind, k##kind>([=](LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs) {                                                                    \
        ...
        case kExprI64Ne:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kUnequal));
        case kExprI64LtS:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kSignedLessThan));
        case kExprI64LtU:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kUnsignedLessThan));
        case kExprI64GtS:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kSignedGreaterThan));
        case kExprI64GtU:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kUnsignedGreaterThan));
        case kExprI64LeS:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kSignedLessEqual));
        case kExprI64LeU:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kUnsignedLessEqual));
        case kExprI64GeS:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kSignedGreaterEqual));
        case kExprI64GeU:
            return EmitBinOp<kI64, kI32>(BindFirst(&LiftoffAssembler::emit_i64_set_cond, kUnsignedGreaterEqual));
        case kExprF32Eq:
            return EmitBinOp<kF32, kI32>(BindFirst(&LiftoffAssembler::emit_f32_set_cond, kEqual));
        case kExprF32Ne:
            return EmitBinOp<kF32, kI32>(BindFirst(&LiftoffAssembler::emit_f32_set_cond, kUnequal));
        case kExprF32Lt:
        ...
        case kExprF32Le:
            return EmitBinOp<kF32, kI32>(BindFirst(&LiftoffAssembler::emit_f32_set_cond, kUnsignedLessEqual));
        case kExprF32Ge:
            return EmitBinOp<kF32, kI32>(BindFirst(&LiftoffAssembler::emit_f32_set_cond, kUnsignedGreaterEqual));
        case kExprF64Eq:
            return EmitBinOp<kF64, kI32>(BindFirst(&LiftoffAssembler::emit_f64_set_cond, kEqual));
        case kExprF64Ne:
            return EmitBinOp<kF64, kI32>(BindFirst(&LiftoffAssembler::emit_f64_set_cond, kUnequal));
        case kExprF64Lt:
        ...
        case kExprF64Le:
            return EmitBinOp<kF64, kI32>(BindFirst(&LiftoffAssembler::emit_f64_set_cond, kUnsignedLessEqual));
        case kExprF64Ge:
            return EmitBinOp<kF64, kI32>(BindFirst(&LiftoffAssembler::emit_f64_set_cond, kUnsignedGreaterEqual));
        case kExprI32Shl:
        ...
        case kExprI32DivU:
            return EmitBinOp<kI32, kI32>([this, decoder](LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs) {
                Label* div_by_zero = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapDivByZero);
                __ emit_i32_divu(dst.gp(), lhs.gp(), rhs.gp(), div_by_zero);
            });
        case kExprI32RemS:
            return EmitBinOp<kI32, kI32>([this, decoder](LiftoffRegister dst, LiftoffRegister lhs, LiftoffRegister rhs) {
                Label* rem_by_zero = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapRemByZero);
...
#undef CASE_CCALL_BINOP
    ...
    void I32Const(FullDecoder* decoder, Value* result, int32_t value) { __ PushConstant(kI32, value); }
    ...
    void RefFunc(FullDecoder* decoder, uint32_t function_index, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmRefFunc, MakeSig::Returns(kRef).Params(kI32), { func_index_var }, decoder->position());
    ...
    void LocalGet(FullDecoder* decoder, Value* result, const IndexImmediate<validate>& imm)
    {
    ...
    void LocalSetFromStackSlot(LiftoffAssembler::VarState* dst_slot, uint32_t local_index)
    {
    ...
    void LocalSet(FullDecoder* decoder, const Value& value, const IndexImmediate<validate>& imm) { LocalSet(imm.index, false); }

    void LocalTee(FullDecoder* decoder, const Value& value, Value* result, const IndexImmediate<validate>& imm) { LocalSet(imm.index, true); }
    ...
    Register GetGlobalBaseAndOffset(const WasmGlobal* global, LiftoffRegList* pinned, uint32_t* offset)
    {
        ...
        if (global->mutability && global->imported) {
            ...
            __ Load(LiftoffRegister(addr), addr, no_reg, field_offset, kPointerLoadType);
            *offset = 0;
        } else {
    ...
    void GlobalGet(FullDecoder* decoder, Value* result, const GlobalIndexImmediate<validate>& imm)
    {
    ...
    void GlobalSet(FullDecoder* decoder, const Value&, const GlobalIndexImmediate<validate>& imm)
    {
    ...
    void TableGet(FullDecoder* decoder, const Value&, Value*, const IndexImmediate<validate>& imm)
    {
        LiftoffRegList pinned;
        ...
        auto stub = is_funcref ? WasmCode::kWasmTableGetFuncRef : WasmCode::kWasmTableGet;

        CallRuntimeStub(stub, MakeSig::Returns(type.kind()).Params(kI32, kI32), { table_index, index }, decoder->position());
    ...
    void TableSet(FullDecoder* decoder, const Value&, const Value&, const IndexImmediate<validate>& imm)
    {
        LiftoffRegList pinned;
        ...
        ValueType type = env_->module->tables[imm.index].type;
        bool is_funcref = IsSubtypeOf(type, kWasmFuncRef, env_->module);
        auto stub = is_funcref ? WasmCode::kWasmTableSetFuncRef : WasmCode::kWasmTableSet;

        CallRuntimeStub(stub, MakeSig::Params(kI32, kI32, type.kind()), { table_index, index, value }, decoder->position());
...
#undef RUNTIME_STUB_FOR_TRAP
    ...
    {
        Label* trap_label = AddOutOfLineTrap(decoder, GetRuntimeStubIdForTrapReason(reason));
    ...
    void AssertNull(FullDecoder* decoder, const Value& arg, Value* result)
    {
        LiftoffRegList pinned;
        LiftoffRegister obj = pinned.set(__ PopToRegister(pinned));
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapIllegalCast);
        LiftoffRegister null = __ GetUnusedRegister(kGpReg, pinned);
        LoadNullValue(null.gp(), pinned);
        {
            FREEZE_STATE(trapping);
            __ emit_cond_jump(kUnequal, trap_label, kRefNull, obj.gp(), null.gp(), trapping);
        }
        __ PushRegister(kRefNull, obj);
    }

    void NopForTestingUnsupportedInLiftoff(FullDecoder* decoder) { unsupported(decoder, kOtherReason, "testing opcode"); }
    ...
    void Select(FullDecoder* decoder, const Value& cond, const Value& fval, const Value& tval, Value* result)
    {
        ...
        DCHECK(CheckCompatibleStackSlotTypes(kind, __ cache_state()->stack_state.end()[-2].kind()));
    ...
    // isn't a loop.
    void BrImpl(FullDecoder* decoder, Control* target, Register tmp1, Register tmp2)
    {
        if (dynamic_tiering()) {
            if (target->is_loop()) {
                ...
                TierupCheck(decoder, decoder->position(), jump_distance + kTierUpCheckCost, tmp1, tmp2);
            } else {
        ...
        if (!target->br_merge()->reached) {
            target->label_state.InitMerge(*__ cache_state(), __ num_locals(), target->br_merge()->arity, target->stack_depth + target->num_exceptions);
        }
        __ MergeStackWith(target->label_state, target->br_merge()->arity, target->is_loop() ? LiftoffAssembler::kBackwardJump : LiftoffAssembler::kForwardJump);
    ...
    void BrOrRet(FullDecoder* decoder, uint32_t depth, uint32_t /* drop_values */)
    {
        ...
        if (NeedsTierupCheck(decoder, depth))
            AllocateTempRegisters(temps);
        BrOrRetImpl(decoder, depth, temps.tmp1, temps.tmp2);
    }

    void BrOrRetImpl(FullDecoder* decoder, uint32_t depth, Register tmp1, Register tmp2)
    {
        if (depth == decoder->control_depth() - 1) {
            ReturnImpl(decoder, tmp1, tmp2);
        } else {
    ...
    // stack transfer code.
    void GenerateBrCase(FullDecoder* decoder, uint32_t br_depth, std::map<uint32_t, MovableLabel>* br_targets, Register tmp1, Register tmp2)
    {
    ...
    void GenerateBrTable(FullDecoder* decoder, LiftoffRegister tmp, LiftoffRegister value, uint32_t min, uint32_t max, BranchTableIterator<validate>* table_iterator,
        std::map<uint32_t, MovableLabel>* br_targets, Register tmp1, Register tmp2, const FreezeCacheState& frozen)
    {
        DCHECK_LT(min, max);
        // Check base case.
        if (max == min + 1) {
        ...
        __ emit_cond_jump(kUnsignedGreaterEqual, &upper_half, kI32, value.gp(), tmp.gp(), frozen);
        // Emit br table for lower half:
        GenerateBrTable(decoder, tmp, value, min, split, table_iterator, br_targets, tmp1, tmp2, frozen);
        __ bind(&upper_half);
        // table_iterator will trigger a DCHECK if we don't stop decoding now.
        if (did_bailout())
            return;
        // Emit br table for upper half:
        GenerateBrTable(decoder, tmp, value, split, max, table_iterator, br_targets, tmp1, tmp2, frozen);
    }

    void BrTable(FullDecoder* decoder, const BranchTableImmediate<validate>& imm, const Value& key)
    {
        LiftoffRegList pinned;
        ...
        if (dynamic_tiering()) {
            ...
            while (table_iterator.has_next()) {
                ...
                if (depth == decoder->control_depth() - 1 || decoder->control_at(depth)->is_loop()) {
                    need_temps = true;
                    break;
                }
            }
...

        {
            ...
            uint32_t sample_depth = decoder->read_u32v<Decoder::kNoValidation>(imm.table, &ignored_length, "first depth");
        ...
        if (imm.table_count > 0) {
            ...
            FREEZE_STATE(frozen);
            Label case_default;
            __ emit_cond_jump(kUnsignedGreaterEqual, &case_default, kI32, value.gp(), tmp.gp(), frozen);

            GenerateBrTable(decoder, tmp, value, 0, imm.table_count, &table_iterator, &br_targets, tmp1, tmp2, frozen);
    ...
    SpilledRegistersForInspection* GetSpilledRegistersForInspection()
    {
        ...
        auto* spilled = compilation_zone_->New<SpilledRegistersForInspection>(compilation_zone_);
    ...
    Label* AddOutOfLineTrap(FullDecoder* decoder, WasmCode::RuntimeStubId stub, uint32_t pc = 0)
    {
        ...
        if (V8_UNLIKELY(for_debugging_)) {
            ...
            safepoint_info = compilation_zone_->New<OutOfLineSafepointInfo>(compilation_zone_);
            __ cache_state()->GetTaggedSlotsForOOLCode(&safepoint_info->slots, &safepoint_info->spills, LiftoffAssembler::CacheState::SpillLocation::kStackSlots);
        }
        out_of_line_code_.push_back(OutOfLineCode::Trap(
            stub, decoder->position(), V8_UNLIKELY(for_debugging_) ? GetSpilledRegistersForInspection() : nullptr, safepoint_info, pc, RegisterOOLDebugSideTableEntry(decoder)));
    ...
    Register BoundsCheckMem(FullDecoder* decoder, uint32_t access_size, uint64_t offset, LiftoffRegister index, LiftoffRegList pinned, ForceCheck force_check)
    {
        const bool statically_oob = !base::IsInBounds<uintptr_t>(offset, access_size, env_->max_memory_size);
        ...
        DCHECK_IMPLIES(env_->module->is_memory64, env_->bounds_checks == kExplicitBoundsChecks);
        if (!force_check && !statically_oob && env_->bounds_checks == kTrapHandler) {
        ...
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapMemOutOfBounds, 0);
        ...
        if (!env_->module->is_memory64) {
            ...
            FREEZE_STATE(trapping);
            __ emit_cond_jump(kNotEqualZero, trap_label, kI32, index.high_gp(), no_reg, trapping);
        }
        ...
        if (end_offset > env_->min_memory_size) {
            __ emit_cond_jump(kUnsignedGreaterEqual, trap_label, kPointerKind, end_offset_reg.gp(), mem_size.gp(), trapping);
        }
        ...
        __ emit_cond_jump(kUnsignedGreaterEqual, trap_label, kPointerKind, index_ptrsize, effective_size_reg.gp(), trapping);
        return index_ptrsize;
    }
    ...
    void AlignmentCheckMem(FullDecoder* decoder, uint32_t access_size, uintptr_t offset, Register index, LiftoffRegList pinned)
    {
        ...
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapUnalignedAccess, 0);
    ...
    bool IndexStaticallyInBounds(const LiftoffAssembler::VarState& index_slot, int access_size, uintptr_t* offset)
    {
        ...
        if (effective_offset < index // overflow
            || !base::IsInBounds<uintptr_t>(effective_offset, access_size, env_->min_memory_size)) {
            return false;
        }
    ...
    void LoadMem(FullDecoder* decoder, LoadType type, const MemoryAccessImmediate<validate>& imm, const Value& index_val, Value* result)
    {
        ...
        if (IndexStaticallyInBounds(index_slot, type.size(), &offset)) {
            ...
            index = BoundsCheckMem(decoder, type.size(), offset, full_index, {}, kDontForceCheck);
            if (index == no_reg)
                return;
            ...
            if (env_->bounds_checks == kTrapHandler) {
                AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapMemOutOfBounds, protected_load_pc);
            }
            __ PushRegister(kind, value);
        }
    ...
    void LoadTransform(FullDecoder* decoder, LoadType type, LoadTransformationKind transform, const MemoryAccessImmediate<validate>& imm, const Value& index_val, Value* result)
    {
        ...
        Register index = BoundsCheckMem(decoder, access_size, imm.offset, full_index, {}, kDontForceCheck);
        if (index == no_reg)
            return;
    ...
    void LoadLane(FullDecoder* decoder, LoadType type, const Value& _value, const Value& _index, const MemoryAccessImmediate<validate>& imm, const uint8_t laneidx, Value* _result)
    {
        ...
        LiftoffRegister value = pinned.set(__ PopToRegister());
        LiftoffRegister full_index = __ PopToRegister();
        Register index = BoundsCheckMem(decoder, type.size(), imm.offset, full_index, pinned, kDontForceCheck);
        if (index == no_reg)
            return;
        ...
        __ LoadLane(result, value, addr, index, offset, type, laneidx, &protected_load_pc);
        if (env_->bounds_checks == kTrapHandler) {
            AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapMemOutOfBounds, protected_load_pc);
        }
    ...
    void StoreMem(FullDecoder* decoder, StoreType type, const MemoryAccessImmediate<validate>& imm, const Value& index_val, const Value& value_val)
    {
        ValueKind kind = type.value_type().kind();
        ...
        if (IndexStaticallyInBounds(index_slot, type.size(), &offset)) {
            ...
            index = BoundsCheckMem(decoder, type.size(), imm.offset, full_index, pinned, kDontForceCheck);
            if (index == no_reg)
                return;
            ...
            if (env_->bounds_checks == kTrapHandler) {
                AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapMemOutOfBounds, protected_store_pc);
            }
        }
    ...
    void StoreLane(FullDecoder* decoder, StoreType type, const MemoryAccessImmediate<validate>& imm, const Value& _index, const Value& _value, const uint8_t lane)
    {
    ...
    void MemoryGrow(FullDecoder* decoder, const Value& value, Value* result_val)
    {
        ...
        if (env_->module->is_memory64) {
            ...
            if (kNeedI64RegPair) {
                ...
                __ emit_cond_jump(kUnequal, &done, kI32, input.high_gp(), no_reg, all_spilled_anyway);
                input = input.low();
            } else {
                LiftoffRegister high_word = __ GetUnusedRegister(kGpReg, pinned);
                __ emit_i64_shri(high_word, input, 32);
                FREEZE_STATE(all_spilled_anyway);
                __ emit_cond_jump(kUnequal, &done, kI32, high_word.gp(), no_reg, all_spilled_anyway);
            }
        }
        ...
        if (input.gp() != param_reg)
            __ Move(param_reg, input.gp(), kI32);
    ...
    base::OwnedVector<DebugSideTable::Entry::Value> GetCurrentDebugSideTableEntries(FullDecoder* decoder, DebugSideTableBuilder::AssumeSpilling assume_spilling)
    {
        ...
        DCHECK_LE(stack_state.size() - num_exceptions_, decoder->num_locals() + decoder->stack_size());
        int index = 0;
        ...
        for (int j = decoder->control_depth() - 1; j >= 0; j--) {
            ...
            for (; index < end_index; ++index) {
                ...
                value.index = index;
                ValueType type = index < static_cast<int>(__ num_locals()) ? decoder->local_type(index)
    ...
    void CallDirect(FullDecoder* decoder, const CallFunctionImmediate<validate>& imm, const Value args[], Value[]) { CallDirect(decoder, imm, args, nullptr, kNoTailCall); }

    void CallIndirect(FullDecoder* decoder, const Value& index_val, const CallIndirectImmediate<validate>& imm, const Value args[], Value returns[])
    {
        CallIndirect(decoder, index_val, imm, kNoTailCall);
    }

    void CallRef(FullDecoder* decoder, const Value& func_ref, const FunctionSig* sig, uint32_t sig_index, const Value args[], Value returns[])
    {
        CallRef(decoder, func_ref.type, sig, kNoTailCall);
    }

    void ReturnCall(FullDecoder* decoder, const CallFunctionImmediate<validate>& imm, const Value args[])
    {
        TierupCheckOnTailCall(decoder);
        CallDirect(decoder, imm, args, nullptr, kTailCall);
    }

    void ReturnCallIndirect(FullDecoder* decoder, const Value& index_val, const CallIndirectImmediate<validate>& imm, const Value args[])
    {
        TierupCheckOnTailCall(decoder);
        CallIndirect(decoder, index_val, imm, kTailCall);
    }

    void ReturnCallRef(FullDecoder* decoder, const Value& func_ref, const FunctionSig* sig, uint32_t sig_index, const Value args[])
    {
        TierupCheckOnTailCall(decoder);
    ...
    void BrOnNull(FullDecoder* decoder, const Value& ref_object, uint32_t depth, bool pass_null_along_branch, Value* /* result_on_fallthrough */)
    {
        ...
        Register tmp = NeedsTierupCheck(decoder, depth) ? pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp() : no_reg;
        LoadNullValue(null, pinned);
        {
            FREEZE_STATE(frozen);
            __ emit_cond_jump(kUnequal, &cont_false, ref_object.type.kind(), ref.gp(), null, frozen);
    ...
    template <ValueKind src_kind, ValueKind result_kind, ValueKind result_lane_kind = kVoid, typename EmitFn>
    void EmitTerOp(EmitFn fn, LiftoffRegister dst, LiftoffRegister src1, LiftoffRegister src2, LiftoffRegister src3)
    {
    ...
    template <ValueKind src_kind, ValueKind result_kind, ValueKind result_lane_kind = kVoid, typename EmitFn> void EmitTerOp(EmitFn fn)
    {
        ...
        LiftoffRegister dst = src_rc == result_rc ? __ GetUnusedRegister(result_rc, { src3 }, LiftoffRegList { src1, src2 }) : __ GetUnusedRegister(result_rc, {});
    ...
    void EmitRelaxedLaneSelect()
    {
        ...
        if (!CpuFeatures::IsSupported(AVX)) {
            ...
            EmitTerOp<kS128, kS128>(&LiftoffAssembler::emit_s128_relaxed_laneselect, src2, src1, src2, mask);
            return;
        }
#endif
        LiftoffRegList pinned;
        ...
        EmitTerOp<kS128, kS128>(&LiftoffAssembler::emit_s128_relaxed_laneselect, dst, src1, src2, mask);
    }
    ...
    template <typename EmitFn> void EmitSimdFmaOp(EmitFn emit_fn)
    {
        LiftoffRegList pinned;
    ...
    void SimdOp(FullDecoder* decoder, WasmOpcode opcode, base::Vector<Value> args, Value* result)
    {
        if (!CpuFeatures::SupportsWasmSimd128()) {
    ...
    template <ValueKind src_kind, ValueKind result_kind, typename EmitFn> void EmitSimdExtractLaneOp(EmitFn fn, const SimdLaneImmediate<validate>& imm)
    {
        static constexpr RegClass src_rc = reg_class_for(src_kind);
        static constexpr RegClass result_rc = reg_class_for(result_kind);
        LiftoffRegister lhs = __ PopToRegister();
    ...
    template <ValueKind src2_kind, typename EmitFn> void EmitSimdReplaceLaneOp(EmitFn fn, const SimdLaneImmediate<validate>& imm)
    {
        ...
        LiftoffRegister dst
            = (src2_rc == result_rc || pin_src2) ? __ GetUnusedRegister(result_rc, { src1 }, LiftoffRegList { src2 }) : __ GetUnusedRegister(result_rc, { src1 }, {});
        fn(dst, src1, src2, imm.lane);
        __ PushRegister(kS128, dst);
    }

    void SimdLaneOp(FullDecoder* decoder, WasmOpcode opcode, const SimdLaneImmediate<validate>& imm, const base::Vector<Value> inputs, Value* result)
    {
        if (!CpuFeatures::SupportsWasmSimd128()) {
            return unsupported(decoder, kSimd, "simd");
        }
...
#define CASE_SIMD_EXTRACT_LANE_OP(opcode, kind, fn)                                                                                                                                \
    case wasm::kExpr##opcode:                                                                                                                                                      \
        EmitSimdExtractLaneOp<kS128, k##kind>([=](LiftoffRegister dst, LiftoffRegister lhs, uint8_t imm_lane_idx) { __ emit_##fn(dst, lhs, imm_lane_idx); }, imm);                 \
        break;
...
#define CASE_SIMD_REPLACE_LANE_OP(opcode, kind, fn)                                                                                                                                \
    case wasm::kExpr##opcode:                                                                                                                                                      \
        EmitSimdReplaceLaneOp<k##kind>(                                                                                                                                            \
            [=](LiftoffRegister dst, LiftoffRegister src1, LiftoffRegister src2, uint8_t imm_lane_idx) { __ emit_##fn(dst, src1, src2, imm_lane_idx); }, imm);                     \
        break;
...
#undef CASE_SIMD_REPLACE_LANE_OP
    ...
    void S128Const(FullDecoder* decoder, const Simd128Immediate<validate>& imm, Value* result)
    {
        if (!CpuFeatures::SupportsWasmSimd128()) {
            return unsupported(decoder, kSimd, "simd");
        }
    ...
    void Simd8x16ShuffleOp(FullDecoder* decoder, const Simd128Immediate<validate>& imm, const Value& input0, const Value& input1, Value* result)
    {
        if (!CpuFeatures::SupportsWasmSimd128()) {
            return unsupported(decoder, kSimd, "simd");
        }
    ...
    void Store32BitExceptionValue(Register values_array, int* index_in_array, Register value, LiftoffRegList pinned)
    {
        ...
        ToSmi(tmp_reg.gp());
        __ StoreTaggedPointer(values_array, no_reg, wasm::ObjectAccess::ElementOffsetInTaggedFixedArray(*index_in_array), tmp_reg, pinned, LiftoffAssembler::kSkipWriteBarrier);
    ...
    void StoreExceptionValue(ValueType type, Register values_array, int* index_in_array, LiftoffRegList pinned)
    {
        ...
        case wasm::kRtt: {
            --(*index_in_array);
            __ StoreTaggedPointer(values_array, no_reg, wasm::ObjectAccess::ElementOffsetInTaggedFixedArray(*index_in_array), value, pinned);
            break;
        }
        case wasm::kI8:
        case wasm::kI16:
    ...
    void GetExceptionValues(FullDecoder* decoder, LiftoffAssembler::VarState& exception_var, const WasmTag* tag)
    {
        LiftoffRegList pinned;
    ...
    void EmitLandingPad(FullDecoder* decoder, int handler_offset)
    {
        ...
        if (!current_try->try_info->catch_reached) {
            current_try->try_info->catch_state.InitMerge(*__ cache_state(), __ num_locals(), 1, current_try->stack_depth + current_try->num_exceptions);
    ...
    void Throw(FullDecoder* decoder, const TagIndexImmediate<validate>& imm, const base::Vector<Value>& /* args */)
    {
        LiftoffRegList pinned;
        ...
        CallRuntimeStub(WasmCode::kWasmAllocateFixedArray, MakeSig::Returns(kPointerKind).Params(kPointerKind),
            { LiftoffAssembler::VarState { kSmiKind, LiftoffRegister { encoded_size_reg }, 0 } }, decoder->position());
        MaybeOSR();
        ...
        CallRuntimeStub(WasmCode::kWasmThrow, MakeSig::Params(kPointerKind, kPointerKind),
            { LiftoffAssembler::VarState { kPointerKind, exception_tag, 0 }, LiftoffAssembler::VarState { kPointerKind, values_array, 0 } }, decoder->position());
    ...
    void AtomicStoreMem(FullDecoder* decoder, StoreType type, const MemoryAccessImmediate<validate>& imm)
    {
        LiftoffRegList pinned;
        LiftoffRegister value = pinned.set(__ PopToRegister());
        LiftoffRegister full_index = __ PopToRegister(pinned);
        Register index = BoundsCheckMem(decoder, type.size(), imm.offset, full_index, pinned, kDoForceCheck);
        if (index == no_reg)
            return;
    ...
    void AtomicLoadMem(FullDecoder* decoder, LoadType type, const MemoryAccessImmediate<validate>& imm)
    {
        ValueKind kind = type.value_type().kind();
        LiftoffRegister full_index = __ PopToRegister();
        Register index = BoundsCheckMem(decoder, type.size(), imm.offset, full_index, {}, kDoForceCheck);
        if (index == no_reg)
            return;
    ...
    void AtomicBinop(FullDecoder* decoder, StoreType type, const MemoryAccessImmediate<validate>& imm,
        void (LiftoffAssembler::*emit_fn)(Register, Register, uintptr_t, LiftoffRegister, LiftoffRegister, StoreType))
    {
    ...
    void AtomicCompareExchange(FullDecoder* decoder, StoreType type, const MemoryAccessImmediate<validate>& imm)
    {
#ifdef V8_TARGET_ARCH_IA32
        ...
        // assembler now.
        __ AtomicCompareExchange(addr, no_reg, offset, expected, new_value, result, type);
        ...
        __ AtomicCompareExchange(addr, index, offset, expected, new_value, result, type);
        __ PushRegister(result_kind, result);
#endif
    }

    void CallRuntimeStub(WasmCode::RuntimeStubId stub_id, const ValueKindSig& sig, std::initializer_list<LiftoffAssembler::VarState> params, int position)
    {
        CODE_COMMENT((std::string { "call builtin: " } + GetRuntimeStubName(stub_id)).c_str());
        auto interface_descriptor = Builtins::CallInterfaceDescriptorFor(RuntimeStubIdToBuiltinName(stub_id));
        auto* call_descriptor = compiler::Linkage::GetStubCallDescriptor(compilation_zone_, // zone
    ...
    void AtomicWait(FullDecoder* decoder, ValueKind kind, const MemoryAccessImmediate<validate>& imm)
    {
        LiftoffRegister full_index = __ PeekToRegister(2, {});
        Register index_reg = BoundsCheckMem(decoder, value_kind_size(kind), imm.offset, full_index, {}, kDoForceCheck);
        if (index_reg == no_reg)
            return;
        LiftoffRegList pinned { index_reg };
        AlignmentCheckMem(decoder, value_kind_size(kind), imm.offset, index_reg, pinned);

        uintptr_t offset = imm.offset;
        Register index_plus_offset = __ cache_state()->is_used(LiftoffRegister(index_reg)) ? pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp() : index_reg;
        ...
        LiftoffAssembler::VarState timeout = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState expected_value = __ cache_state()->stack_state.end()[-2];
        ...
        static constexpr WasmCode::RuntimeStubId kTargets[2][2] { // 64 bit systems (kNeedI64RegPair == false):
        ...
        CallRuntimeStub(target, MakeSig::Params(kPointerKind, kind, kI64), { index, expected_value, timeout }, decoder->position());
        // Pop parameters from the value stack.
        __ DropValues(3);
    ...
    void AtomicNotify(FullDecoder* decoder, const MemoryAccessImmediate<validate>& imm)
    {
        LiftoffRegister full_index = __ PeekToRegister(1, {});
        Register index_reg = BoundsCheckMem(decoder, kInt32Size, imm.offset, full_index, {}, kDoForceCheck);
        if (index_reg == no_reg)
            return;
        LiftoffRegList pinned { index_reg };
        ...
        CallRuntimeStub(WasmCode::kWasmAtomicNotify, MakeSig::Returns(kI32).Params(kPointerKind, kI32), { index, count }, decoder->position());
        // Pop parameters from the value stack.
        __ DropValues(2);
...
#define ATOMIC_COMPARE_EXCHANGE_LIST(V)                                                                                                                                            \
    ...
    void AtomicOp(FullDecoder* decoder, WasmOpcode opcode, base::Vector<Value> args, const MemoryAccessImmediate<validate>& imm, Value* result)
    {
        switch (opcode) {
...
#undef ATOMIC_COMPARE_EXCHANGE_LIST
    ...
    LiftoffRegister PopMemTypeToRegister(FullDecoder* decoder, Register* high_word, LiftoffRegList* pinned)
    {
        ...
        if (*high_word == no_reg) {
            ...
            // stack.
            *high_word = pinned->has(reg.high()) ? __ GetUnusedRegister(kGpReg, *pinned).gp() : __ GetUnusedRegister(kGpReg, { reg.high() }, *pinned).gp();
    ...
    void MemoryInit(FullDecoder* decoder, const MemoryInitImmediate<validate>& imm, const Value&, const Value&, const Value&)
    {
        ...
        LiftoffRegister dst = PopMemTypeToRegister(decoder, &mem_offsets_high_word, &pinned);
        ...
        // correctly.
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapMemOutOfBounds);
        if (mem_offsets_high_word != no_reg) {
            // If any high word has bits set, jump to the OOB trap.
            FREEZE_STATE(trapping);
            __ emit_cond_jump(kNotEqualZero, trap_label, kI32, mem_offsets_high_word, no_reg, trapping);
            pinned.clear(mem_offsets_high_word);
        }

        LiftoffRegister segment_index = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        __ LoadConstant(segment_index, WasmValue(imm.data_segment.index));

        auto sig = MakeSig::Returns(kI32).Params(kPointerKind, kPointerKind, kI32, kI32, kI32);
        LiftoffRegister args[] = { LiftoffRegister(instance), dst, src, segment_index, size };
        // We don't need the instance anymore after the call. We can use the
        // register for the result.
        LiftoffRegister result(instance);
        GenerateCCall(&result, &sig, kVoid, args, ExternalReference::wasm_memory_init());
        FREEZE_STATE(trapping);
    ...
    void MemoryCopy(FullDecoder* decoder, const MemoryCopyImmediate<validate>& imm, const Value&, const Value&, const Value&)
    {
        Register mem_offsets_high_word = no_reg;
        LiftoffRegList pinned;
        LiftoffRegister size = pinned.set(PopMemTypeToRegister(decoder, &mem_offsets_high_word, &pinned));
        LiftoffRegister src = pinned.set(PopMemTypeToRegister(decoder, &mem_offsets_high_word, &pinned));
        LiftoffRegister dst = pinned.set(PopMemTypeToRegister(decoder, &mem_offsets_high_word, &pinned));
        ...
        auto sig = MakeSig::Returns(kI32).Params(kPointerKind, kPointerKind, kPointerKind, kPointerKind);
        LiftoffRegister args[] = { LiftoffRegister(instance), dst, src, size };
        // We don't need the instance anymore after the call. We can use the
        // register for the result.
        LiftoffRegister result(instance);
        GenerateCCall(&result, &sig, kVoid, args, ExternalReference::wasm_memory_copy());
        FREEZE_STATE(trapping);
        __ emit_cond_jump(kEqual, trap_label, kI32, result.gp(), no_reg, trapping);
    }

    void MemoryFill(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const Value&, const Value&, const Value&)
    {
        Register mem_offsets_high_word = no_reg;
        LiftoffRegList pinned;
        LiftoffRegister size = pinned.set(PopMemTypeToRegister(decoder, &mem_offsets_high_word, &pinned));
        ...
        auto sig = MakeSig::Returns(kI32).Params(kPointerKind, kPointerKind, kI32, kPointerKind);
        LiftoffRegister args[] = { LiftoffRegister(instance), dst, value, size };
        // We don't need the instance anymore after the call. We can use the
        // register for the result.
        LiftoffRegister result(instance);
        GenerateCCall(&result, &sig, kVoid, args, ExternalReference::wasm_memory_fill());
        FREEZE_STATE(trapping);
        __ emit_cond_jump(kEqual, trap_label, kI32, result.gp(), no_reg, trapping);
    }
    ...
    void TableInit(FullDecoder* decoder, const TableInitImmediate<validate>& imm, base::Vector<Value> args)
    {
        LiftoffRegList pinned;
        LiftoffRegister table_index_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        ...
        LiftoffAssembler::VarState segment_index(kPointerKind, segment_index_reg, 0);
        ...
        CallRuntimeStub(WasmCode::kWasmTableInit, MakeSig::Params(kI32, kI32, kI32, kSmiKind, kSmiKind), { dst, src, size, table_index, segment_index }, decoder->position());
    ...
    {
        LiftoffRegList pinned;
        Register dropped_elem_segments = pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp();
        LOAD_TAGGED_PTR_INSTANCE_FIELD(dropped_elem_segments, DroppedElemSegments, pinned);

        LiftoffRegister seg_index = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        __ LoadConstant(seg_index, WasmValue(wasm::ObjectAccess::ElementOffsetInTaggedFixedUInt8Array(imm.index)));
        ...
        __ Store(dropped_elem_segments, seg_index.gp(), 0, one_reg, StoreType::kI32Store8, pinned);
    }

    void TableCopy(FullDecoder* decoder, const TableCopyImmediate<validate>& imm, base::Vector<Value> args)
    {
        LiftoffRegList pinned;

        LiftoffRegister table_dst_index_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        LoadSmi(table_dst_index_reg, imm.table_dst.index);
        LiftoffAssembler::VarState table_dst_index(kPointerKind, table_dst_index_reg, 0);

        LiftoffRegister table_src_index_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        LoadSmi(table_src_index_reg, imm.table_src.index);
        LiftoffAssembler::VarState table_src_index(kPointerKind, table_src_index_reg, 0);

        LiftoffAssembler::VarState size = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState src = __ cache_state()->stack_state.end()[-2];
        LiftoffAssembler::VarState dst = __ cache_state()->stack_state.end()[-3];

        CallRuntimeStub(WasmCode::kWasmTableCopy, MakeSig::Params(kI32, kI32, kI32, kSmiKind, kSmiKind), { dst, src, size, table_dst_index, table_src_index }, decoder->position());
    ...
    void TableGrow(FullDecoder* decoder, const IndexImmediate<validate>& imm, const Value&, const Value&, Value* result)
    {
        LiftoffRegList pinned;
        ...
        CallRuntimeStub(WasmCode::kWasmTableGrow, MakeSig::Returns(kSmiKind).Params(kSmiKind, kI32, kTaggedKind), { table_index, delta, value }, decoder->position());
    ...
    void TableSize(FullDecoder* decoder, const IndexImmediate<validate>& imm, Value*)
    {
    ...
    void TableFill(FullDecoder* decoder, const IndexImmediate<validate>& imm, const Value&, const Value&, const Value&)
    {
        LiftoffRegList pinned;
        ...
        CallRuntimeStub(WasmCode::kWasmTableFill, MakeSig::Params(kSmiKind, kI32, kI32, kTaggedKind), { table_index, start, count, value }, decoder->position());
    ...
    void StructNew(FullDecoder* decoder, const StructIndexImmediate<validate>& imm, const Value& rtt, bool initial_values_on_stack)
    {
        LiftoffRegList pinned;
        LiftoffRegister instance_size = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        LiftoffAssembler::VarState instance_size_state(kI32, instance_size, 0);
        LiftoffAssembler::VarState rtt_value = __ cache_state()->stack_state.end()[-1];

        __ LoadConstant(instance_size, WasmValue(static_cast<int32_t>(WasmStruct::Size(imm.struct_type))));
        CallRuntimeStub(WasmCode::kWasmAllocateStructWithRtt, MakeSig::Returns(kRef).Params(rtt.type.kind(), kI32), { rtt_value, instance_size_state }, decoder->position());
        // Drop the RTT.
        __ cache_state()->stack_state.pop_back(1);
        ...
        for (uint32_t i = imm.struct_type->field_count(); i > 0;) {
            ...
            LiftoffRegister value = pinned.set(initial_values_on_stack ? __ PopToRegister(pinned) : __ GetUnusedRegister(reg_class_for(field_kind), pinned));
            if (!initial_values_on_stack) {
                if (!CheckSupportedType(decoder, field_kind, "default value"))
                    return;
    ...
    void StructNew(FullDecoder* decoder, const StructIndexImmediate<validate>& imm, const Value& rtt, const Value args[], Value* result) { StructNew(decoder, imm, rtt, true); }

    void StructNewDefault(FullDecoder* decoder, const StructIndexImmediate<validate>& imm, const Value& rtt, Value* result) { StructNew(decoder, imm, rtt, false); }

    void StructGet(FullDecoder* decoder, const Value& struct_obj, const FieldImmediate<validate>& field, bool is_signed, Value* result)
    {
        ...
        LoadObjectField(value, obj.gp(), no_reg, offset, field_kind, is_signed, pinned);
        __ PushRegister(unpacked(field_kind), value);
    }

    void StructSet(FullDecoder* decoder, const Value& struct_obj, const FieldImmediate<validate>& field, const Value& field_value)
    {
        const StructType* struct_type = field.struct_imm.struct_type;
        ValueKind field_kind = struct_type->field(field.field_imm.index).kind();
        int offset = StructFieldOffset(struct_type, field.field_imm.index);
    ...
    void ArrayNew(FullDecoder* decoder, const ArrayIndexImmediate<validate>& imm, ValueKind rtt_kind, bool initial_value_on_stack)
    {
        // Max length check.
        {
            LiftoffRegister length = __ LoadToRegister(__ cache_state()->stack_state.end()[-2], {});
            Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapArrayTooLarge);
            FREEZE_STATE(trapping);
        ...
        // Allocate the array.
        {
            ...
            LiftoffAssembler::VarState rtt_var = __ cache_state()->stack_state.end()[-1];
            LiftoffAssembler::VarState length_var = __ cache_state()->stack_state.end()[-2];
            __ LoadConstant(elem_size_reg, WasmValue(elem_size));
            LiftoffAssembler::VarState elem_size_var(kI32, elem_size_reg, 0);
            CallRuntimeStub(
                WasmCode::kWasmAllocateArray_Uninitialized, MakeSig::Returns(kRef).Params(rtt_kind, kI32, kI32), { rtt_var, length_var, elem_size_var }, decoder->position());
            // Drop the RTT.
            __ cache_state()->stack_state.pop_back(1);
        }
        ...
        __ LoadConstant(offset, WasmValue(wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize)));
        ...
        __ bind(&loop);
        {
            ...
            __ emit_cond_jump(kUnsignedGreaterEqual, &done, kI32, offset.gp(), end_offset.gp(), in_this_case_its_fine);
        }
    ...
    void ArrayNew(FullDecoder* decoder, const ArrayIndexImmediate<validate>& imm, const Value& length_value, const Value& initial_value, const Value& rtt, Value* result)
    {
        ArrayNew(decoder, imm, rtt.type.kind(), true);
    }

    void ArrayNewDefault(FullDecoder* decoder, const ArrayIndexImmediate<validate>& imm, const Value& length, const Value& rtt, Value* result)
    {
        ArrayNew(decoder, imm, rtt.type.kind(), false);
    }

    void ArrayGet(FullDecoder* decoder, const Value& array_obj, const ArrayIndexImmediate<validate>& imm, const Value& index_val, bool is_signed, Value* result)
    {
        LiftoffRegList pinned;
        ...
        LoadObjectField(value, array.gp(), index.gp(), wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize), elem_kind, is_signed, pinned);
        __ PushRegister(unpacked(elem_kind), value);
    }

    void ArraySet(FullDecoder* decoder, const Value& array_obj, const ArrayIndexImmediate<validate>& imm, const Value& index_val, const Value& value_val)
    {
        LiftoffRegList pinned;
        LiftoffRegister value = pinned.set(__ PopToRegister(pinned));
        ...
        StoreObjectField(array.gp(), index.gp(), wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize), value, pinned, elem_kind);
    }
    ...
    void ArrayCopy(FullDecoder* decoder, const Value& dst, const Value& dst_index, const Value& src, const Value& src_index, const Value& length)
    {
        // TODO(7748): Unify implementation with TF: Implement this with
        // GenerateCCall. Remove runtime function and builtin in wasm.tq.
        CallRuntimeStub(v8_flags.experimental_wasm_skip_bounds_checks ? WasmCode::kWasmArrayCopy : WasmCode::kWasmArrayCopyWithChecks,
            MakeSig::Params(kI32, kI32, kI32, kRefNull, kRefNull),
            // Builtin parameter order:
            // [dst_index, src_index, length, dst, src].
            { __ cache_state()->stack_state.end()[-4], __ cache_state()->stack_state.end()[-2], __ cache_state()->stack_state.end()[-1], __ cache_state()->stack_state.end()[-5],
                __ cache_state()->stack_state.end()[-3] },
            decoder->position());
        __ cache_state()->stack_state.pop_back(5);
    }

    void ArrayNewFixed(FullDecoder* decoder, const ArrayIndexImmediate<validate>& imm, const base::Vector<Value>& elements, const Value& rtt, Value* result)
    {
        ...
        {
            LiftoffRegList pinned;
            LiftoffRegister elem_size_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));

            __ LoadConstant(elem_size_reg, WasmValue(value_kind_size(elem_kind)));
            LiftoffAssembler::VarState elem_size_var(kI32, elem_size_reg, 0);

            LiftoffRegister length_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
            __ LoadConstant(length_reg, WasmValue(static_cast<int32_t>(elements.size())));
        ...
        for (int i = static_cast<int>(elements.size()) - 1; i >= 0; i--) {
            ...
            LiftoffRegister offset_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
            __ LoadConstant(offset_reg, WasmValue(i << value_kind_size_log2(elem_kind)));
            StoreObjectField(array.gp(), offset_reg.gp(), wasm::ObjectAccess::ToTagged(WasmArray::kHeaderSize), element, pinned, elem_kind);
        }
    ...
    void ArrayNewSegment(FullDecoder* decoder, const ArrayIndexImmediate<validate>& array_imm, const IndexImmediate<validate>& data_segment, const Value& /* offset */,
        const Value& /* length */, const Value& /* rtt */, Value* /* result */)
    {
        LiftoffRegList pinned;
        LiftoffRegister data_segment_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        __ LoadConstant(data_segment_reg, WasmValue(static_cast<int32_t>(data_segment.index)));
        LiftoffAssembler::VarState data_segment_var(kI32, data_segment_reg, 0);

        CallRuntimeStub(WasmCode::kWasmArrayNewSegment, MakeSig::Returns(kRef).Params(kI32, kI32, kI32, kRtt),
            {
                data_segment_var,
    ...
    void RttCanon(FullDecoder* decoder, uint32_t type_index, Value* result)
    {
        ...
        __ LoadTaggedPointer(rtt.gp(), rtt.gp(), no_reg, wasm::ObjectAccess::ElementOffsetInTaggedFixedArray(type_index));
    ...
    void SubtypeCheck(const WasmModule* module, Register obj_reg, ValueType obj_type, Register rtt_reg, ValueType rtt_type, Register scratch_null, Register scratch2,
        Label* no_match, NullSucceeds null_succeeds, const FreezeCacheState& frozen)
    {
        Label match;
        ...
        if (rtt_depth >= kMinimumSupertypeArraySize) {
            ...
            __ emit_i32_cond_jumpi(kUnsignedLessEqual, no_match, list_length.gp(), rtt_depth, frozen);
        }
        ...
        __ emit_cond_jump(kUnequal, no_match, rtt_type.kind(), tmp1, rtt_reg, frozen);

        // Fall through to {match}.
        __ bind(&match);
    }

    void RefTest(FullDecoder* decoder, const Value& obj, const Value& rtt, Value* /* result_val */, bool null_succeeds)
    {
        ...
        if (obj.type.is_nullable())
            LoadNullValue(scratch_null, pinned);

        {
            FREEZE_STATE(frozen);
            SubtypeCheck(
                decoder->module_, obj_reg.gp(), obj.type, rtt_reg.gp(), rtt.type, scratch_null, result.gp(), &return_false, null_succeeds ? kNullSucceeds : kNullFails, frozen);
    ...
    void RefCast(FullDecoder* decoder, const Value& obj, const Value& rtt, Value* result)
    {
        ...
        {
            FREEZE_STATE(frozen);
            SubtypeCheck(decoder->module_, obj_reg.gp(), obj.type, rtt_reg.gp(), rtt.type, scratch_null, scratch2, trap_label, kNullSucceeds, frozen);
        }
        __ PushRegister(obj.type.kind(), obj_reg);
    }

    void BrOnCast(FullDecoder* decoder, const Value& obj, const Value& rtt, Value* /* result_on_branch */, uint32_t depth)
    {
        // Avoid having sequences of branches do duplicate work.
        if (depth != decoder->control_depth() - 1) {
            __ PrepareForBranch(decoder->control_at(depth)->br_merge()->arity, {});
        ...
        SubtypeCheck(decoder->module_, obj_reg.gp(), obj.type, rtt_reg.gp(), rtt.type, scratch_null, scratch2, &cont_false, kNullFails, frozen);
    ...
    void BrOnCastFail(FullDecoder* decoder, const Value& obj, const Value& rtt, Value* /* result_on_fallthrough */, uint32_t depth)
    {
        // Avoid having sequences of branches do duplicate work.
        if (depth != decoder->control_depth() - 1) {
            __ PrepareForBranch(decoder->control_at(depth)->br_merge()->arity, {});
        ...
        SubtypeCheck(decoder->module_, obj_reg.gp(), obj.type, rtt_reg.gp(), rtt.type, scratch_null, scratch2, &cont_branch, kNullFails, frozen);
    ...
    void LoadInstanceType(TypeCheck& check, const FreezeCacheState& frozen, Label* on_smi)
    {
        ...
        __ Load(LiftoffRegister(check.instance_type()), check.instance_type(), no_reg, wasm::ObjectAccess::ToTagged(Map::kInstanceTypeOffset), LoadType::kI32Load16U);
    }
    ...
    void ArrayCheck(TypeCheck& check, const FreezeCacheState& frozen)
    {
        ...
        __ emit_i32_cond_jumpi(kUnequal, check.no_match, check.instance_type(), WASM_ARRAY_TYPE, frozen);
    }

    void I31Check(TypeCheck& check, const FreezeCacheState& frozen) { __ emit_smi_check(check.obj_reg, check.no_match, LiftoffAssembler::kJumpOnNotSmi, frozen); }
    ...
    void RefIsData(FullDecoder* /* decoder */, const Value& object, Value* /* result_val */, bool null_succeeds = false)
    {
        AbstractTypeCheck<&LiftoffCompiler::DataCheck>(object, null_succeeds);
    }

    void RefIsEq(FullDecoder* /* decoder */, const Value& object, Value* /* result_val */, bool null_succeeds)
    {
        AbstractTypeCheck<&LiftoffCompiler::EqCheck>(object, null_succeeds);
    }

    void RefIsArray(FullDecoder* /* decoder */, const Value& object, Value* /* result_val */, bool null_succeeds = false)
    {
        AbstractTypeCheck<&LiftoffCompiler::ArrayCheck>(object, null_succeeds);
    }

    void RefIsI31(FullDecoder* decoder, const Value& object, Value* /* result */, bool null_succeeds = false)
    {
        AbstractTypeCheck<&LiftoffCompiler::I31Check>(object, null_succeeds);
    }

    template <TypeChecker type_checker> void AbstractTypeCast(const Value& object, FullDecoder* decoder, ValueKind result_kind)
    {
    ...
    void RefAsData(FullDecoder* decoder, const Value& object, Value* /* result */) { AbstractTypeCast<&LiftoffCompiler::DataCheck>(object, decoder, kRef); }

    void RefAsI31(FullDecoder* decoder, const Value& object, Value* result) { AbstractTypeCast<&LiftoffCompiler::I31Check>(object, decoder, kRef); }

    void RefAsArray(FullDecoder* decoder, const Value& object, Value* result) { AbstractTypeCast<&LiftoffCompiler::ArrayCheck>(object, decoder, kRef); }

    template <TypeChecker type_checker> void BrOnAbstractType(const Value& object, FullDecoder* decoder, uint32_t br_depth)
    {
        bool null_succeeds = false; // TODO(mliedtke): Use parameter.
        // Avoid having sequences of branches do duplicate work.
    ...
    template <TypeChecker type_checker> void BrOnNonAbstractType(const Value& object, FullDecoder* decoder, uint32_t br_depth)
    {
        bool null_succeeds = false; // TODO(mliedtke): Use parameter.
        // Avoid having sequences of branches do duplicate work.
        if (br_depth != decoder->control_depth() - 1) {
    ...
    void BrOnData(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnAbstractType<&LiftoffCompiler::DataCheck>(object, decoder, br_depth);
    }

    void BrOnI31(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnAbstractType<&LiftoffCompiler::I31Check>(object, decoder, br_depth);
    }

    void BrOnArray(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnAbstractType<&LiftoffCompiler::ArrayCheck>(object, decoder, br_depth);
    }

    void BrOnNonData(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnNonAbstractType<&LiftoffCompiler::DataCheck>(object, decoder, br_depth);
    }

    void BrOnNonI31(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnNonAbstractType<&LiftoffCompiler::I31Check>(object, decoder, br_depth);
    }

    void BrOnNonArray(FullDecoder* decoder, const Value& object, Value* /* value_on_branch */, uint32_t br_depth)
    {
        BrOnNonAbstractType<&LiftoffCompiler::ArrayCheck>(object, decoder, br_depth);
    }

    void StringNewWtf8(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const unibrow::Utf8Variant variant, const Value& offset, const Value& size, Value* result)
    {
        LiftoffRegList pinned;
        ...
        CallRuntimeStub(WasmCode::kWasmStringNewWtf8, MakeSig::Returns(kRef).Params(kI32, kI32, kSmiKind, kSmiKind),
            {
    ...
    void StringNewWtf8Array(FullDecoder* decoder, const unibrow::Utf8Variant variant, const Value& array, const Value& start, const Value& end, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringNewWtf8Array, MakeSig::Returns(kRef).Params(kI32, kI32, kRef, kSmiKind),
            {
    ...
    void StringNewWtf16(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const Value& offset, const Value& size, Value* result)
    {
        LiftoffRegList pinned;
        LiftoffRegister memory_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        ...
        CallRuntimeStub(WasmCode::kWasmStringNewWtf16, MakeSig::Returns(kRef).Params(kI32, kI32, kI32),
            {
                memory_var,
                __ cache_state()->stack_state.end()[-2], // offset
                __ cache_state()->stack_state.end()[-1] // size
    ...
    void StringNewWtf16Array(FullDecoder* decoder, const Value& array, const Value& start, const Value& end, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringNewWtf16Array, MakeSig::Returns(kRef).Params(kRef, kI32, kI32),
            {
                array_var,
                __ cache_state()->stack_state.end()[-2], // start
                __ cache_state()->stack_state.end()[-1], // end
    ...
    void StringConst(FullDecoder* decoder, const StringConstImmediate<validate>& imm, Value* result)
    {
        LiftoffRegList pinned;
        LiftoffRegister index_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        __ LoadConstant(index_reg, WasmValue(static_cast<int32_t>(imm.index)));
        LiftoffAssembler::VarState index_var(kI32, index_reg, 0);

        CallRuntimeStub(WasmCode::kWasmStringConst, MakeSig::Returns(kRef).Params(kI32),
            {
                index_var,
            },
            decoder->position());
    ...
    void StringMeasureWtf16(FullDecoder* decoder, const Value& str, Value* result)
    {
        ...
        LoadObjectField(value, string_reg.gp(), no_reg, wasm::ObjectAccess::ToTagged(String::kLengthOffset), ValueKind::kI32, false /* is_signed */, pinned);
        __ PushRegister(kI32, value);
    }

    void StringEncodeWtf8(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const unibrow::Utf8Variant variant, const Value& str, const Value& offset, Value* result)
    {
        LiftoffRegList pinned;

        LiftoffAssembler::VarState& offset_var = __ cache_state()->stack_state.end()[-1];
        ...
        CallRuntimeStub(WasmCode::kWasmStringEncodeWtf8, MakeSig::Returns(kI32).Params(kRef, kI32, kSmiKind, kSmiKind),
            {
                string_var,
                offset_var,
                memory_var,
    ...
    void StringEncodeWtf8Array(FullDecoder* decoder, const unibrow::Utf8Variant variant, const Value& str, const Value& array, const Value& start, Value* result)
    {
        ...
        LiftoffAssembler::VarState& start_var = __ cache_state()->stack_state.end()[-1];

        LiftoffRegister variant_reg = pinned.set(__ GetUnusedRegister(kGpReg, pinned));
        LoadSmi(variant_reg, static_cast<int32_t>(variant));
        LiftoffAssembler::VarState variant_var(kSmiKind, variant_reg, 0);

        CallRuntimeStub(WasmCode::kWasmStringEncodeWtf8Array, MakeSig::Returns(kI32).Params(kRef, kRef, kI32, kSmiKind),
            {
                string_var,
                array_var,
                start_var,
    ...
    void StringEncodeWtf16(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const Value& str, const Value& offset, Value* result)
    {
        LiftoffRegList pinned;
        ...
        CallRuntimeStub(WasmCode::kWasmStringEncodeWtf16, MakeSig::Returns(kI32).Params(kRef, kI32, kSmiKind),
            {
                string_var,
                offset_var,
                memory_var,
    ...
    void StringEncodeWtf16Array(FullDecoder* decoder, const Value& str, const Value& array, const Value& start, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringEncodeWtf16Array, MakeSig::Returns(kI32).Params(kRef, kRef, kI32),
            {
                string_var,
                array_var,
                start_var,
    ...
    void StringConcat(FullDecoder* decoder, const Value& head, const Value& tail, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringConcat, MakeSig::Returns(kRef).Params(kRef, kRef),
            {
                head_var,
                tail_var,
            },
    ...
    void StringEq(FullDecoder* decoder, const Value& a, const Value& b, Value* result)
    {
...

        {
            ...
            __ emit_cond_jump(LiftoffCondition::kEqual, &done, kRefNull, a_reg.gp(), b_reg.gp(), frozen);

            // Otherwise if either operand is null, result is 0.
            if (check_for_null) {
                __ LoadConstant(result_reg, WasmValue(int32_t { 0 }));
                if (a.type.is_nullable()) {
                    __ emit_cond_jump(LiftoffCondition::kEqual, &done, kRefNull, a_reg.gp(), null.gp(), frozen);
                }
                if (b.type.is_nullable()) {
                    __ emit_cond_jump(LiftoffCondition::kEqual, &done, kRefNull, b_reg.gp(), null.gp(), frozen);
                }
            }
        ...
        CallRuntimeStub(WasmCode::kWasmStringEqual, MakeSig::Returns(kI32).Params(kRef, kRef),
            {
                a_var,
                b_var,
            },
    ...
    void StringIsUSVSequence(FullDecoder* decoder, const Value& str, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringIsUSVSequence, MakeSig::Returns(kI32).Params(kRef),
            {
                str_var,
            },
            decoder->position());
    ...
    void StringAsWtf8(FullDecoder* decoder, const Value& str, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringAsWtf8, MakeSig::Returns(kRef).Params(kRef),
            {
                str_var,
            },
            decoder->position());
    ...
    void StringViewWtf8Advance(FullDecoder* decoder, const Value& view, const Value& pos, const Value& bytes, Value* result)
    {
        ...
        LiftoffAssembler::VarState& bytes_var = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState& pos_var = __ cache_state()->stack_state.end()[-2];
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewWtf8Advance, MakeSig::Returns(kI32).Params(kRef, kI32, kI32),
            {
                view_var,
                pos_var,
                bytes_var,
    ...
    void StringViewWtf8Encode(FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const unibrow::Utf8Variant variant, const Value& view, const Value& addr,
        const Value& pos, const Value& bytes, Value* next_pos, Value* bytes_written)
    {
        ...
        LiftoffAssembler::VarState& bytes_var = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState& pos_var = __ cache_state()->stack_state.end()[-2];
        LiftoffAssembler::VarState& addr_var = __ cache_state()->stack_state.end()[-3];
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewWtf8Encode, MakeSig::Returns(kI32, kI32).Params(kI32, kI32, kI32, kRef, kSmiKind, kSmiKind),
            {
                addr_var,
                pos_var,
                bytes_var,
    ...
    void StringViewWtf8Slice(FullDecoder* decoder, const Value& view, const Value& start, const Value& end, Value* result)
    {
        ...
        LiftoffAssembler::VarState& end_var = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState& start_var = __ cache_state()->stack_state.end()[-2];

        LiftoffRegister view_reg = pinned.set(__ LoadToRegister(__ cache_state()->stack_state.end()[-3], pinned));
        MaybeEmitNullCheck(decoder, view_reg.gp(), pinned, view.type);
        LiftoffAssembler::VarState view_var(kRef, view_reg, 0);

        CallRuntimeStub(WasmCode::kWasmStringViewWtf8Slice, MakeSig::Returns(kRef).Params(kRef, kI32, kI32),
            {
                view_var,
                start_var,
                end_var,
    ...
    void StringAsWtf16(FullDecoder* decoder, const Value& str, Value* result) { RefAsNonNull(decoder, str, result); }
    ...
    void StringViewWtf16GetCodeUnit(FullDecoder* decoder, const Value& view, const Value& pos, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewWtf16GetCodeUnit, MakeSig::Returns(kI32).Params(kRef, kI32),
            {
                view_var,
                pos_var,
            },
    ...
    void StringViewWtf16Encode(
        FullDecoder* decoder, const MemoryIndexImmediate<validate>& imm, const Value& view, const Value& offset, const Value& pos, const Value& codeunits, Value* result)
    {
        LiftoffRegList pinned;

        LiftoffAssembler::VarState& codeunits_var = __ cache_state()->stack_state.end()[-1];
        LiftoffAssembler::VarState& pos_var = __ cache_state()->stack_state.end()[-2];
        LiftoffAssembler::VarState& offset_var = __ cache_state()->stack_state.end()[-3];
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewWtf16Encode, MakeSig::Returns(kI32).Params(kI32, kI32, kI32, kRef, kSmiKind),
            {
                offset_var,
                pos_var,
                codeunits_var,
    ...
    void StringViewWtf16Slice(FullDecoder* decoder, const Value& view, const Value& start, const Value& end, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewWtf16Slice, MakeSig::Returns(kRef).Params(kRef, kI32, kI32),
            {
                view_var,
                start_var,
                end_var,
    ...
    void StringAsIter(FullDecoder* decoder, const Value& str, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringAsIter, MakeSig::Returns(kRef).Params(kRef),
            {
                str_var,
            },
            decoder->position());
    ...
    void StringViewIterNext(FullDecoder* decoder, const Value& view, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewIterNext, MakeSig::Returns(kI32).Params(kRef),
            {
                view_var,
            },
            decoder->position());
    ...
    void StringViewIterAdvance(FullDecoder* decoder, const Value& view, const Value& codepoints, Value* result)
    {
        ...
        LiftoffAssembler::VarState& codepoints_var = __ cache_state()->stack_state.end()[-1];

        LiftoffRegister view_reg = pinned.set(__ LoadToRegister(__ cache_state()->stack_state.end()[-2], pinned));
        MaybeEmitNullCheck(decoder, view_reg.gp(), pinned, view.type);
        LiftoffAssembler::VarState view_var(kRef, view_reg, 0);

        CallRuntimeStub(WasmCode::kWasmStringViewIterAdvance, MakeSig::Returns(kI32).Params(kRef, kI32),
            {
                view_var,
                codepoints_var,
            },
    ...
    void StringViewIterRewind(FullDecoder* decoder, const Value& view, const Value& codepoints, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewIterRewind, MakeSig::Returns(kI32).Params(kRef, kI32),
            {
                view_var,
                codepoints_var,
            },
    ...
    void StringViewIterSlice(FullDecoder* decoder, const Value& view, const Value& codepoints, Value* result)
    {
        ...
        CallRuntimeStub(WasmCode::kWasmStringViewIterSlice, MakeSig::Returns(kRef).Params(kRef, kI32),
            {
                view_var,
                codepoints_var,
            },
...
private:
    void CallDirect(FullDecoder* decoder, const CallFunctionImmediate<validate>& imm, const Value args[], Value returns[], TailCall tail_call)
    {
        ...
        if (imm.index < env_->module->num_imported_functions) {
            ...
            LOAD_TAGGED_PTR_INSTANCE_FIELD(imported_targets, ImportedFunctionTargets, pinned);
            __ Load(LiftoffRegister(target), imported_targets, no_reg, wasm::ObjectAccess::ElementOffsetInTaggedFixedAddressArray(imm.index), kPointerLoadType);

            Register imported_function_refs = tmp;
            LOAD_TAGGED_PTR_INSTANCE_FIELD(imported_function_refs, ImportedFunctionRefs, pinned);
            Register imported_function_ref = tmp;
            __ LoadTaggedPointer(imported_function_ref, imported_function_refs, no_reg, ObjectAccess::ElementOffsetInTaggedFixedArray(imm.index));
    ...
    void CallIndirect(FullDecoder* decoder, const Value& index_val, const CallIndirectImmediate<validate>& imm, TailCall tail_call)
    {
        MostlySmallValueKindSig sig(compilation_zone_, imm.sig);
        for (ValueKind ret : sig.returns()) {
            if (!CheckSupportedType(decoder, ret, "return"))
        ...
        if (imm.table_imm.index != 0) {
            Register indirect_function_tables = pinned.set(__ GetUnusedRegister(kGpReg, pinned)).gp();
            LOAD_TAGGED_PTR_INSTANCE_FIELD(indirect_function_tables, IndirectFunctionTables, pinned);

            indirect_function_table = indirect_function_tables;
            __ LoadTaggedPointer(indirect_function_table, indirect_function_tables, no_reg, ObjectAccess::ElementOffsetInTaggedFixedArray(imm.table_imm.index));
        }

        // Bounds check against the table size.
        Label* invalid_func_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapTableOutOfBounds);

        // Compare against table size stored in
        // {instance->indirect_function_table_size}.
        if (imm.table_imm.index == 0) {
            LOAD_INSTANCE_FIELD(tmp_const, IndirectFunctionTableSize, kUInt32Size, pinned);
        } else {
            __ Load(LiftoffRegister(tmp_const), indirect_function_table, no_reg, wasm::ObjectAccess::ToTagged(WasmIndirectFunctionTable::kSizeOffset), LoadType::kI32Load);
        }
        {
            FREEZE_STATE(trapping);
            __ emit_cond_jump(kUnsignedGreaterEqual, invalid_func_label, kI32, index, tmp_const, trapping);
        }

        CODE_COMMENT("Check indirect call signature");
        // Load the signature from {instance->ift_sig_ids[key]}
        if (imm.table_imm.index == 0) {
            LOAD_INSTANCE_FIELD(table, IndirectFunctionTableSigIds, kSystemPointerSize, pinned);
        } else {
            __ Load(LiftoffRegister(table), indirect_function_table, no_reg, wasm::ObjectAccess::ToTagged(WasmIndirectFunctionTable::kSigIdsOffset), kPointerLoadType);
        }
        static_assert((1 << 2) == kInt32Size);
        __ Load(LiftoffRegister(scratch), table, index, 0, LoadType::kI32Load, nullptr, false, false, true);

        // Compare against expected signature.
        LOAD_INSTANCE_FIELD(tmp_const, IsorecursiveCanonicalTypes, kSystemPointerSize, pinned);
        __ Load(LiftoffRegister(tmp_const), tmp_const, no_reg, imm.sig_imm.index * kInt32Size, LoadType::kI32Load);

        Label* sig_mismatch_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapFuncSigMismatch);
        __ DropValues(1);
        {
            FREEZE_STATE(trapping);
            __ emit_cond_jump(kUnequal, sig_mismatch_label, kPointerKind, scratch, tmp_const, trapping);
        }
        ...
        if (imm.table_imm.index == 0) {
            ...
            __ LoadTaggedPointer(table, indirect_function_table, no_reg, wasm::ObjectAccess::ToTagged(WasmIndirectFunctionTable::kRefsOffset));
        }
        __ LoadTaggedPointer(tmp_const, table, index, ObjectAccess::ElementOffsetInTaggedFixedArray(0), true);

        Register* explicit_instance = &tmp_const;

        // Load the target from {instance->ift_targets[key]}
        if (imm.table_imm.index == 0) {
            LOAD_INSTANCE_FIELD(table, IndirectFunctionTableTargets, kSystemPointerSize, pinned);
        } else {
            __ Load(LiftoffRegister(table), indirect_function_table, no_reg, wasm::ObjectAccess::ToTagged(WasmIndirectFunctionTable::kTargetsOffset), kPointerLoadType);
        }
        __ Load(LiftoffRegister(scratch), table, index, 0, kPointerLoadType, nullptr, false, false, true);
    ...
    void CallRef(FullDecoder* decoder, ValueType func_ref_type, const FunctionSig* type_sig, TailCall tail_call)
    {
        ...
        compiler::CallDescriptor* call_descriptor = compiler::GetWasmCallDescriptor(compilation_zone_, type_sig);
        call_descriptor = GetLoweredCallDescriptor(compilation_zone_, call_descriptor);
        ...
        if (v8_flags.wasm_speculative_inlining) {
            ...
            size_t vector_slot = encountered_call_instructions_.size() * 2;
            encountered_call_instructions_.push_back(FunctionTypeFeedback::kNonDirectCall);
            ...
            CallRuntimeStub(WasmCode::kCallRefIC, MakeSig::Returns(kPointerKind, kPointerKind).Params(kPointerKind, kIntPtrKind, kPointerKind),
            ...
            __ LoadTaggedPointer(instance.gp(), func_ref.gp(), no_reg, wasm::ObjectAccess::ToTagged(WasmInternalFunction::kRefOffset));

#ifdef V8_ENABLE_SANDBOX
            LOAD_INSTANCE_FIELD(temp.gp(), IsolateRoot, kSystemPointerSize, pinned);
            __ LoadExternalPointer(target.gp(), func_ref.gp(), WasmInternalFunction::kCallTargetOffset, kWasmInternalFunctionCallTargetTag, temp.gp());
#else
            __ Load(target, func_ref.gp(), no_reg, wasm::ObjectAccess::ToTagged(WasmInternalFunction::kCallTargetOffset), kPointerLoadType);
#endif
            ...
            __ emit_cond_jump(kUnequal, &perform_call, kRef, target.gp(), null_address.gp(), frozen);
            // The cached target can only be null for WasmJSFunctions.
            __ LoadTaggedPointer(target.gp(), func_ref.gp(), no_reg, wasm::ObjectAccess::ToTagged(WasmInternalFunction::kCodeOffset));
#ifdef V8_EXTERNAL_CODE_SPACE
            __ LoadCodeDataContainerEntry(target.gp(), target.gp());
#else
            __ emit_ptrsize_addi(target.gp(), target.gp(), wasm::ObjectAccess::ToTagged(Code::kHeaderSize));
#endif
    ...
    void LoadNullValue(Register null, LiftoffRegList pinned)
    {
        ...
        __ LoadFullPointer(null, null, IsolateData::root_slot_offset(RootIndex::kNullValue));
    }
    ...
    void MaybeEmitNullCheck(FullDecoder* decoder, Register object, LiftoffRegList pinned, ValueType type)
    {
        if (v8_flags.experimental_wasm_skip_null_checks || !type.is_nullable())
            return;
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapNullDereference);
        LiftoffRegister null = __ GetUnusedRegister(kGpReg, pinned);
        LoadNullValue(null.gp(), pinned);
        FREEZE_STATE(trapping);
        __ emit_cond_jump(LiftoffCondition::kEqual, trap_label, kRefNull, object, null.gp(), trapping);
    }

    void BoundsCheckArray(FullDecoder* decoder, LiftoffRegister array, LiftoffRegister index, LiftoffRegList pinned)
    {
        if (V8_UNLIKELY(v8_flags.experimental_wasm_skip_bounds_checks))
            return;
        Label* trap_label = AddOutOfLineTrap(decoder, WasmCode::kThrowWasmTrapArrayOutOfBounds);
        ...
        FREEZE_STATE(trapping);
        __ emit_cond_jump(LiftoffCondition::kUnsignedGreaterEqual, trap_label, kI32, index.gp(), length.gp(), trapping);
    }

    int StructFieldOffset(const StructType* struct_type, int field_index) { return wasm::ObjectAccess::ToTagged(WasmStruct::kHeaderSize + struct_type->field_offset(field_index)); }

    void LoadObjectField(LiftoffRegister dst, Register src, Register offset_reg, int offset, ValueKind kind, bool is_signed, LiftoffRegList pinned)
    {
        if (is_reference(kind)) {
    ...
    void StoreObjectField(Register obj, Register offset_reg, int offset, LiftoffRegister value, LiftoffRegList pinned, ValueKind kind)
    {
        if (is_reference(kind)) {
    ...
    void SetDefaultValue(LiftoffRegister reg, ValueKind kind, LiftoffRegList pinned)
    {
    ...
    bool has_outstanding_op() const { return outstanding_op_ != kNoOutstandingOp; }
    ...
    Register LoadInstanceIntoRegister(LiftoffRegList pinned, Register fallback)
    {
        Register instance = __ cache_state()->cached_instance;
        if (instance == no_reg) {
            instance = __ cache_state()->TrySetCachedInstanceRegister(pinned | LiftoffRegList { fallback });
            if (instance == no_reg)
                instance = fallback;
            __ LoadInstanceFromFrame(instance);
        }
...
std::unique_ptr<AssemblerBuffer> NewLiftoffAssemblerBuffer(AssemblerBufferCache* assembler_buffer_cache, int func_body_size)
{
    ...
    return assembler_buffer_cache ? assembler_buffer_cache->GetAssemblerBuffer(initial_buffer_size) : NewAssemblerBuffer(initial_buffer_size);
}
...
WasmCompilationResult ExecuteLiftoffCompilation(CompilationEnv* env, const FunctionBody& func_body, const LiftoffOptions& compiler_options)
{
    ...
    WasmFullDecoder<Decoder::kBooleanValidation, LiftoffCompiler> decoder(&zone, env->module, env->enabled_features,
        ...
        NewLiftoffAssemblerBuffer(compiler_options.assembler_buffer_cache, func_body_size), debug_sidetable_builder.get(), compiler_options);
    decoder.Decode();
    ...
    if (V8_UNLIKELY(v8_flags.trace_wasm_compilation_times)) {
        ...
        StdoutStream {} << "Compiled function " << reinterpret_cast<const void*>(env->module) << "#" << compiler_options.func_index << " using Liftoff, took "
                        << time.InMilliseconds() << " ms and " << zone.allocation_size() << " bytes; bodysize " << func_body_size << " codesize " << codesize << std::endl;
    }
...
std::unique_ptr<DebugSideTable> GenerateLiftoffDebugSideTable(const WasmCode* code)
{
    ...
    base::Vector<const byte> function_bytes = wire_bytes.GetFunctionBytes(function);
    CompilationEnv env = native_module->CreateCompilationEnv();
    FunctionBody func_body { function->sig, 0, function_bytes.begin(), function_bytes.end() };
    ...
    WasmFullDecoder<Decoder::kBooleanValidation, LiftoffCompiler> decoder(&zone, native_module->module(), env.enabled_features, &detected, func_body, call_descriptor, &env, &zone,
        NewAssemblerBuffer(AssemblerBase::kDefaultBufferSize), &debug_sidetable_builder,
