...
void SanitizeUnicodeName(StringBuilder& out, const byte* utf8_src, size_t length)
{
...
void NamesProvider::WriteRef(StringBuilder& out, WireBytesRef ref) { out.write(wire_bytes_.begin() + ref.offset(), ref.length()); }
...
void NamesProvider::PrintElementSegmentName(StringBuilder& out, uint32_t element_segment_index)
{
    DecodeNamesIfNotYetDone();
...
void NamesProvider::PrintDataSegmentName(StringBuilder& out, uint32_t data_segment_index)
{
    DecodeNamesIfNotYetDone();
