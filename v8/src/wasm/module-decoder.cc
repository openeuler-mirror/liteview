...
public:
    ...
    ModuleDecoderImpl(const WasmFeatures& enabled, const byte* module_start, const byte* module_end, ModuleOrigin origin)
        : ModuleDecoderTemplate<NoTracer>(enabled, module_start, module_end, origin, no_tracer_)
    {
    }
...
ModuleResult DecodeWasmModule(const WasmFeatures& enabled, const byte* module_start, const byte* module_end, bool validate_functions, ModuleOrigin origin, Counters* counters,
    std::shared_ptr<metrics::Recorder> metrics_recorder, v8::metrics::Recorder::ContextId context_id, DecodingMethod decoding_method, AccountingAllocator* allocator)
{
    size_t size = module_end - module_start;
    CHECK_LE(module_start, module_end);
    size_t max_size = max_module_size();
    if (size > max_size) {
        return ModuleResult { WasmError { 0, "size > maximum module size (%zu): %zu", max_size, size } };
    }
    ...
    ModuleResult result = decoder.DecodeModule(counters, allocator, validate_functions);

    // Record event metrics.
    metrics_event.wall_clock_duration_in_us = timer.Elapsed().InMicroseconds();
    timer.Stop();
    if (!thread_ticks.IsNull()) {
        metrics_event.cpu_duration_in_us = (base::ThreadTicks::Now() - thread_ticks).InMicroseconds();
    }
...
ModuleResult DecodeWasmModuleForDisassembler(const byte* module_start, const byte* module_end, AccountingAllocator* allocator)
{
    constexpr bool validate_functions = false;
    ModuleDecoderImpl decoder(WasmFeatures::All(), module_start, module_end, kWasmOrigin);
...
const std::shared_ptr<WasmModule>& ModuleDecoder::shared_module() const { return impl_->shared_module(); }

void ModuleDecoder::StartDecoding(
    Counters* counters, std::shared_ptr<metrics::Recorder> metrics_recorder, v8::metrics::Recorder::ContextId context_id, AccountingAllocator* allocator, ModuleOrigin origin)
{
    DCHECK_NULL(impl_);
...
void ModuleDecoder::DecodeModuleHeader(base::Vector<const uint8_t> bytes, uint32_t offset) { impl_->DecodeModuleHeader(bytes, offset); }

void ModuleDecoder::DecodeSection(SectionCode section_code, base::Vector<const uint8_t> bytes, uint32_t offset) { impl_->DecodeSection(section_code, bytes, offset); }

void ModuleDecoder::DecodeFunctionBody(uint32_t index, uint32_t length, uint32_t offset, bool validate_functions)
{
    impl_->DecodeFunctionBody(index, length, offset, validate_functions);
}

void ModuleDecoder::StartCodeSection(WireBytesRef section_bytes) { impl_->StartCodeSection(section_bytes); }

bool ModuleDecoder::CheckFunctionsCount(uint32_t functions_count, uint32_t error_offset) { return impl_->CheckFunctionsCount(functions_count, error_offset); }
...
Result<const FunctionSig*> DecodeWasmSignatureForTesting(const WasmFeatures& enabled, Zone* zone, const byte* start, const byte* end)
{
...
ConstantExpression DecodeWasmInitExprForTesting(const WasmFeatures& enabled, const byte* start, const byte* end, ValueType expected)
{
    ModuleDecoderImpl decoder(enabled, start, end, kWasmOrigin);
...
FunctionResult DecodeWasmFunctionForTesting(
    const WasmFeatures& enabled, Zone* zone, const ModuleWireBytes& wire_bytes, const WasmModule* module, const byte* function_start, const byte* function_end, Counters* counters)
{
    size_t size = function_end - function_start;
    CHECK_LE(function_start, function_end);
    if (size > kV8MaxWasmFunctionSize) {
        return FunctionResult { WasmError { 0, "size > maximum function size (%zu): %zu", kV8MaxWasmFunctionSize, size } };
    }
...
std::vector<CustomSectionOffset> DecodeCustomSections(const byte* start, const byte* end)
{
...
void DecodeNameMap(NameMap& target, Decoder& decoder, EmptyNames empty_names = kSkipEmptyNames)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (index > NameMap::kMaxKey)
            continue;
        if (empty_names == kSkipEmptyNames && name.is_empty())
            continue;
...
void DecodeFunctionNames(const byte* module_start, const byte* module_end, NameMap& names)
{
