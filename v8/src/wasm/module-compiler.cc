...
//----
class std_shared_mutex {
public:
    std_shared_mutex() = default;
    ~std_shared_mutex() = default;

    v8::base::SharedMutex mutex_;
};

class std_shared_lock {
public:
    std_shared_lock(std_shared_mutex& mutex)
    {
        mutex_ = &mutex;
        mutex_->mutex_.LockShared();
    }

    ~std_shared_lock()
    {
        mutex_->mutex_.UnlockShared();
    }
private:
    std_shared_mutex* mutex_;
};

class std_unique_lock {
public:
    std_unique_lock(std_shared_mutex& mutex)
    {
        mutex_ = &mutex;
        mutex_->mutex_.LockExclusive();
    }
    ~std_unique_lock()
    {
        mutex_->mutex_.UnlockExclusive();
    }
private:
    std_shared_mutex* mutex_;
};
...
enum CompileBaselineOnly : bool { kBaselineOnly = true, kBaselineOrTopTier = false };
...
public:
    ...
    Queue* GetQueueForTask(int task_id)
    {
        ...
        {
            /*std::shared_lock<std::shared_mutex>*/std_shared_lock queues_guard { queues_mutex_ };
        ...
        /*std::unique_lock<std::shared_mutex>*/std_unique_lock queues_guard { queues_mutex_ };
    ...
    base::Optional<WasmCompilationUnit> GetNextUnit(Queue* queue, CompileBaselineOnly baseline_only)
    {
        ...
        for (int tier = GetLowestTierWithUnits(); tier <= max_tier; ++tier) {
            if (auto unit = GetNextUnitOfTier(queue, tier)) {
                size_t old_units_count = num_units_[tier].fetch_sub(1, std::memory_order_relaxed);
    ...
    void AddUnits(base::Vector<WasmCompilationUnit> baseline_units, base::Vector<WasmCompilationUnit> top_tier_units, const WasmModule* module)
    {
        ...
        for (auto pair : { std::make_pair(int { kBaseline }, baseline_units), std::make_pair(int { kTopTier }, top_tier_units) }) {
            int tier = pair.first;
...
private:
    ...
    struct BigUnit {
        ...
        bool operator<(const BigUnit& other) const { return func_size < other.func_size; }
    };
    ...
    struct TopTierPriorityUnit {
        ...
        bool operator<(const TopTierPriorityUnit& other) const { return priority < other.priority; }
    };
    ...
    base::Optional<WasmCompilationUnit> GetNextUnitOfTier(Queue* public_queue, int tier)
    {
        ...
        // stolen units.
        {
            /*std::shared_lock<std::shared_mutex>*/std_shared_lock guard { queues_mutex_ };
    ...
    base::Optional<WasmCompilationUnit> StealUnitsAndGetFirst(QueueImpl* queue, int steal_from_task_id, int wanted_tier)
    {
    ...
    // Hold a shared lock on {queues_mutex_} when calling this method.
    base::Optional<WasmCompilationUnit> StealTopTierPriorityUnit(QueueImpl* queue, int steal_from_task_id)
    {
        auto* steal_queue = queues_[steal_from_task_id].get();
        // Cannot steal from own queue.
        if (steal_queue == queue)
    ...
    /*std::shared_mutex*/std_shared_mutex queues_mutex_;
...
public:
    ...
    void InitializeCompilationProgress(int num_import_wrappers, int num_export_wrappers);
    ...
    // otherwise.
    void InitializeRecompilation(TieringState new_tiering_state, std::unique_ptr<CompilationEventCallback> recompilation_finished_callback);
    ...
    void CommitCompilationUnits(base::Vector<WasmCompilationUnit> baseline_units, base::Vector<WasmCompilationUnit> top_tier_units,
        base::Vector<std::shared_ptr<JSToWasmWrapperCompilationUnit>> js_to_wasm_wrapper_units);
    ...
    base::Optional<WasmCompilationUnit> GetNextCompilationUnit(CompilationUnitQueues::Queue*, CompileBaselineOnly);

    std::shared_ptr<JSToWasmWrapperCompilationUnit> GetNextJSToWasmWrapperCompilationUnit();
    ...
    void SchedulePublishCompilationResults(std::vector<std::unique_ptr<WasmCode>> unpublished_code);
    ...
    bool failed() const { return compile_failed_.load(std::memory_order_relaxed); }

    bool baseline_compilation_finished() const
    {
        base::MutexGuard guard(&callbacks_mutex_);
        return outstanding_baseline_units_ == 0 && outstanding_export_wrappers_ == 0;
    }
    ...
    std::weak_ptr<NativeModule> const native_module_weak() const { return native_module_weak_; }

private:
    // Returns the potentially-updated {function_progress}.
    uint8_t AddCompilationUnitInternal(CompilationUnitBuilder* builder, int function_index, uint8_t function_progress);
    ...
    std::vector<std::shared_ptr<JSToWasmWrapperCompilationUnit>> js_to_wasm_wrapper_units_;
...
CompilationStateImpl* Impl(CompilationState* compilation_state) { return reinterpret_cast<CompilationStateImpl*>(compilation_state); }
const CompilationStateImpl* Impl(const CompilationState* compilation_state) { return reinterpret_cast<const CompilationStateImpl*>(compilation_state); }
...
bool BackgroundCompileScope::cancelled() const { return native_module_ == nullptr || Impl(native_module_->compilation_state())->cancelled(); }

void UpdateFeatureUseCounts(Isolate* isolate, const WasmFeatures& detected)
{
    using Feature = v8::Isolate::UseCounterFeature;
    constexpr static std::pair<WasmFeature, Feature> kUseCounters[] = { { kFeature_reftypes, Feature::kWasmRefTypes }, { kFeature_simd, Feature::kWasmSimdOpcodes },
        { kFeature_threads, Feature::kWasmThreadOpcodes }, { kFeature_eh, Feature::kWasmExceptionHandling } };

    for (auto& feature : kUseCounters) {
        if (detected.contains(feature.first))
            isolate->CountUsage(feature.second);
    }
}
...
void CompilationState::CancelCompilation() { Impl(this)->CancelCompilation(CompilationStateImpl::kCancelUnconditionally); }

void CompilationState::CancelInitialCompilation() { Impl(this)->CancelCompilation(CompilationStateImpl::kCancelInitialCompilation); }

void CompilationState::SetError() { Impl(this)->SetError(); }

void CompilationState::SetWireBytesStorage(std::shared_ptr<WireBytesStorage> wire_bytes_storage) { Impl(this)->SetWireBytesStorage(std::move(wire_bytes_storage)); }

std::shared_ptr<WireBytesStorage> CompilationState::GetWireBytesStorage() const { return Impl(this)->GetWireBytesStorage(); }

void CompilationState::AddCallback(std::unique_ptr<CompilationEventCallback> callback) { return Impl(this)->AddCallback(std::move(callback)); }
...
bool CompilationState::baseline_compilation_finished() const { return Impl(this)->baseline_compilation_finished(); }

bool CompilationState::recompilation_finished() const { return Impl(this)->recompilation_finished(); }

void CompilationState::set_compilation_id(int compilation_id) { Impl(this)->set_compilation_id(compilation_id); }

DynamicTiering CompilationState::dynamic_tiering() const { return Impl(this)->dynamic_tiering(); }

// static
std::unique_ptr<CompilationState> CompilationState::New(
    const std::shared_ptr<NativeModule>& native_module, std::shared_ptr<Counters> async_counters, DynamicTiering dynamic_tiering)
{
    return std::unique_ptr<CompilationState>(reinterpret_cast<CompilationState*>(new CompilationStateImpl(std::move(native_module), std::move(async_counters), dynamic_tiering)));
}
...
CompileStrategy GetCompileStrategy(const WasmModule* module, const WasmFeatures& enabled_features, uint32_t func_index, bool lazy_module)
{
    if (lazy_module)
...
ExecutionTierPair GetDefaultTiersPerModule(NativeModule* native_module, DynamicTiering dynamic_tiering, bool lazy_module)
{
...
ExecutionTierPair GetLazyCompilationTiers(NativeModule* native_module, uint32_t func_index)
{
    ...
    ExecutionTierPair tiers = GetDefaultTiersPerModule(native_module, dynamic_tiering, kNotLazy);
...
public:
    ...
    void AddImportUnit(uint32_t func_index)
    {
        ...
        baseline_units_.emplace_back(func_index, ExecutionTier::kNone, kNoDebugging);
    }

    void AddJSToWasmWrapperUnit(std::shared_ptr<JSToWasmWrapperCompilationUnit> unit) { js_to_wasm_wrapper_units_.emplace_back(std::move(unit)); }

    void AddBaselineUnit(int func_index, ExecutionTier tier) { baseline_units_.emplace_back(func_index, tier, kNoDebugging); }

    void AddTopTierUnit(int func_index, ExecutionTier tier) { tiering_units_.emplace_back(func_index, tier, kNoDebugging); }

    void AddDebugUnit(int func_index) { baseline_units_.emplace_back(func_index, ExecutionTier::kLiftoff, kForDebugging); }

    void AddRecompilationUnit(int func_index, ExecutionTier tier)
    {
        // For recompilation, just treat all units like baseline units.
        baseline_units_.emplace_back(func_index, tier, tier == ExecutionTier::kLiftoff ? kForDebugging : kNoDebugging);
    }

    bool Commit()
    {
        if (baseline_units_.empty() && tiering_units_.empty() && js_to_wasm_wrapper_units_.empty()) {
            return false;
        }
        compilation_state()->CommitCompilationUnits(base::VectorOf(baseline_units_), base::VectorOf(tiering_units_), base::VectorOf(js_to_wasm_wrapper_units_));
        Clear();
        return true;
    }
...
private:
    CompilationStateImpl* compilation_state() const { return Impl(native_module_->compilation_state()); }
...
WasmError GetWasmErrorWithName(ModuleWireBytes wire_bytes, const WasmFunction* func, const WasmModule* module, WasmError error)
{
    WasmName name = wire_bytes.GetNameOrNull(func, module);
    if (name.begin() == nullptr) {
        return WasmError(error.offset(), "Compiling function #%d failed: %s", func->func_index, error.message().c_str());
    } else {
        TruncatedUserString<> truncated_name(name);
        return WasmError(error.offset(), "Compiling function #%d:\"%.*s\" failed: %s", func->func_index, truncated_name.length(), truncated_name.start(), error.message().c_str());
    }
}

void SetCompileError(ErrorThrower* thrower, ModuleWireBytes wire_bytes, const WasmFunction* func, const WasmModule* module, WasmError error)
{
    thrower->CompileFailed(GetWasmErrorWithName(std::move(wire_bytes), func, module, std::move(error)));
}

DecodeResult ValidateSingleFunction(const WasmModule* module, int func_index, base::Vector<const uint8_t> code, AccountingAllocator* allocator, WasmFeatures enabled_features)
{
    const WasmFunction* func = &module->functions[func_index];
    FunctionBody body { func->sig, func->code.offset(), code.begin(), code.end() };
    WasmFeatures detected_features;
    return ValidateFunctionBody(allocator, enabled_features, module, &detected_features, body);
}
...
void ValidateSequentially(const WasmModule* module, NativeModule* native_module, Counters* counters, AccountingAllocator* allocator, ErrorThrower* thrower, bool lazy_module,
    OnlyLazyFunctions only_lazy_functions = kAllFunctions)
{
    ...
    for (uint32_t func_index = start; func_index < end; func_index++) {
        ...
        if (only_lazy_functions) {
            ...
            if (strategy != CompileStrategy::kLazy && strategy != CompileStrategy::kLazyBaselineEagerTopTier) {
                continue;
            }
        }
        ...
        DecodeResult result = ValidateSingleFunction(module, func_index, code, allocator, enabled_features);
        if (result.failed()) {
...
bool IsLazyModule(const WasmModule* module) { return v8_flags.wasm_lazy_compilation || (v8_flags.asm_wasm_lazy_compilation && is_asmjs_module(module)); }
...
bool CompileLazy(Isolate* isolate, Handle<WasmInstanceObject> instance, int func_index)
{
    ...
    base::ThreadTicks thread_ticks = base::ThreadTicks::IsSupported() ? base::ThreadTicks::Now() : base::ThreadTicks();
    ...
    WasmCompilationUnit baseline_unit { func_index, tiers.baseline_tier, kNoDebugging };
    ...
    WasmFeatures detected_features;
    WasmCompilationResult result = baseline_unit.ExecuteCompilation(&env, compilation_state->GetWireBytesStorage().get(), counters, assembler_buffer_cache, &detected_features);
    compilation_state->OnCompilationStopped(detected_features);
    if (!thread_ticks.IsNull()) {
        native_module->UpdateCPUDuration((base::ThreadTicks::Now() - thread_ticks).InMicroseconds(), tiers.baseline_tier);
    }
    ...
    WasmCode* code;
    {
        ...
        code = native_module->PublishCode(native_module->AddCompiledCode(std::move(result)));
    }
    ...
    if (WasmCode::ShouldBeLogged(isolate)) {
        ...
        std::unique_ptr<char[]> url = url_obj.IsString() ? String::cast(url_obj).ToCString() : nullptr;
    ...
    if (GetCompileStrategy(module, native_module->enabled_features(), func_index, lazy_module) == CompileStrategy::kLazy && tiers.baseline_tier < tiers.top_tier) {
    ...
    if (feedback_vector_slots > 0) {
        ...
        Handle<FixedArray> vector = isolate->factory()->NewFixedArrayWithZeroes(feedback_vector_slots);
        instance->feedback_vectors().set(declared_function_index(module, func_index), *vector);
    }
...
void ThrowLazyCompilationError(Isolate* isolate, const NativeModule* native_module, int func_index)
{
    ...
    auto enabled_features = native_module->enabled_features();
    DecodeResult decode_result = ValidateSingleFunction(module, func_index, code, engine->allocator(), enabled_features);

    CHECK(decode_result.failed());
    wasm::ErrorThrower thrower(isolate, nullptr);
    SetCompileError(&thrower, ModuleWireBytes(native_module->wire_bytes()), func, module, decode_result.error());
}

class TransitiveTypeFeedbackProcessor {
public:
    static void Process(WasmInstanceObject instance, int func_index) { TransitiveTypeFeedbackProcessor { instance, func_index }.ProcessQueue(); }
    ...
    void EnqueueCallees(const std::vector<CallSiteFeedback>& feedback)
    {
        for (size_t i = 0; i < feedback.size(); i++) {
            ...
            for (int j = 0; j < csf.num_cases(); j++) {
                ...
                if (existing != feedback_for_function_.end() && existing->second.feedback_vector.size() > 0) {
                    continue;
                }
                queue_.insert(func);
            }
...
public:
    FeedbackMaker(WasmInstanceObject instance, int func_index, int num_calls)
        : instance_(instance)
        , num_imported_functions_(static_cast<int>(instance.module()->num_imported_functions))
    ...
    {
        if (!maybe_function.IsWasmInternalFunction())
            return;
        ...
        WasmExportedFunction target = WasmExportedFunction::cast(function.external());
        if (target.instance() != instance_)
            return;
        if (target.function_index() < num_imported_functions_)
            return;
...
void TransitiveTypeFeedbackProcessor::ProcessFunction(int func_index)
{
    ...
    if (!maybe_feedback.IsFixedArray())
        return;
    FixedArray feedback = FixedArray::cast(maybe_feedback);
    base::Vector<uint32_t> call_direct_targets = module_->type_feedback.feedback_for_function[func_index].call_targets.as_vector();
...
void TriggerTierUp(WasmInstanceObject instance, int func_index)
{
    ...
    CompilationStateImpl* compilation_state = Impl(native_module->compilation_state());
    WasmCompilationUnit tiering_unit { func_index, ExecutionTier::kTurbofan, kNoDebugging };

    const WasmModule* module = native_module->module();
    int priority;
    {
        base::MutexGuard mutex_guard(&module->type_feedback.mutex);
        int array_index = wasm::declared_function_index(instance.module(), func_index);
...
void TierUpNowForTesting(Isolate* isolate, WasmInstanceObject instance, int func_index)
{
    if (v8_flags.wasm_speculative_inlining) {
        TransitiveTypeFeedbackProcessor::Process(instance, func_index);
    }
    auto* native_module = instance.module_object().native_module();
    wasm::GetWasmEngine()->CompileFunction(isolate, native_module, func_index, wasm::ExecutionTier::kTurbofan);
...
CompilationExecutionResult ExecuteJSToWasmWrapperCompilationUnits(std::weak_ptr<NativeModule> native_module, JobDelegate* delegate)
{
...

    {
        ...
        if (compile_scope.cancelled())
            return kYield;
        wrapper_unit = compile_scope.compilation_state()->GetNextJSToWasmWrapperCompilationUnit();
        if (!wrapper_unit)
            return kNoMoreUnits;
        isolate = wrapper_unit->isolate();
        wrapper_compilation_token = wasm::GetWasmEngine()->StartWrapperCompilation(isolate);
        if (!wrapper_compilation_token)
            return kNoMoreUnits;
    }
    ...
    while (true) {
        ...
        if (compile_scope.cancelled())
            return kYield;
        if (yield || !(wrapper_unit = compile_scope.compilation_state()->GetNextJSToWasmWrapperCompilationUnit())) {
            compile_scope.compilation_state()->OnFinishedJSToWasmWrapperUnits(num_processed_wrappers);
...
CompilationExecutionResult ExecuteCompilationUnits(std::weak_ptr<NativeModule> native_module, Counters* counters, JobDelegate* delegate, CompileBaselineOnly baseline_only)
{
    ...
    // triggered.
    if (ExecuteJSToWasmWrapperCompilationUnits(native_module, delegate) == kYield) {
        return kYield;
    }
    ...
    // compilation unit.
    {
        ...
        unit = compile_scope.compilation_state()->GetNextCompilationUnit(queue, baseline_only);
        if (!unit)
            return kNoMoreUnits;
    }
    ...
    while (true) {
        ...
        while (unit->tier() == current_tier) {
            ...
            WasmCompilationResult result = unit->ExecuteCompilation(&env.value(), wire_bytes.get(), counters, assembler_buffer_cache, &detected_features);
            ...
            if (yield || !(unit = compile_scope.compilation_state()->GetNextCompilationUnit(queue, baseline_only))) {
                if (!thread_ticks.IsNull()) {
                    compile_scope.native_module()->UpdateCPUDuration((base::ThreadTicks::Now() - thread_ticks).InMicroseconds(), current_tier);
                }
                std::vector<std::unique_ptr<WasmCode>> unpublished_code = compile_scope.native_module()->AddCompiledCode(base::VectorOf(std::move(results_to_publish)));
                results_to_publish.clear();
                compile_scope.compilation_state()->SchedulePublishCompilationResults(std::move(unpublished_code));
                compile_scope.compilation_state()->OnCompilationStopped(detected_features);
                return yield ? kYield : kNoMoreUnits;
            }
            ...
            if (batch_full || liftoff_finished) {
                if (!thread_ticks.IsNull()) {
                    ...
                    compile_scope.native_module()->UpdateCPUDuration((thread_ticks_now - thread_ticks).InMicroseconds(), current_tier);
...
int AddExportWrapperUnits(Isolate* isolate, NativeModule* native_module, CompilationUnitBuilder* builder)
{
    ...
    for (auto exp : native_module->module()->export_table) {
        ...
        uint32_t canonical_type_index = native_module->module()->isorecursive_canonical_type_ids[function.sig_index];
        JSToWasmWrapperKey key(function.imported, canonical_type_index);
        if (keys.insert(key).second) {
            auto unit = std::make_shared<JSToWasmWrapperCompilationUnit>(isolate, function.sig, canonical_type_index, native_module->module(), function.imported,
                native_module->enabled_features(), JSToWasmWrapperCompilationUnit::kAllowGeneric);
...
// Returns the number of units added.
int AddImportWrapperUnits(NativeModule* native_module, CompilationUnitBuilder* builder)
{
    std::unordered_set<WasmImportWrapperCache::CacheKey, WasmImportWrapperCache::CacheKeyHash> keys;
    ...
    for (int func_index = 0; func_index < num_imported_functions; func_index++) {
        const WasmFunction& function = native_module->module()->functions[func_index];
        if (!IsJSCompatibleSignature(function.sig, native_module->module(), native_module->enabled_features())) {
            continue;
        }
        uint32_t canonical_type_index = native_module->module()->isorecursive_canonical_type_ids[function.sig_index];
        WasmImportWrapperCache::CacheKey key(compiler::kDefaultImportCallKind, canonical_type_index, static_cast<int>(function.sig->parameter_count()), kNoSuspend);
...
void InitializeLazyCompilation(NativeModule* native_module)
{
    ...
    for (uint32_t func_index = start; func_index < end; func_index++) {
        CompileStrategy strategy = GetCompileStrategy(module, native_module->enabled_features(), func_index, lazy_module);
        if (strategy == CompileStrategy::kLazy || strategy == CompileStrategy::kLazyBaselineEagerTopTier) {
...
std::unique_ptr<CompilationUnitBuilder> InitializeCompilation(Isolate* isolate, NativeModule* native_module)
{
    ...
    int num_export_wrappers = AddExportWrapperUnits(isolate, native_module, builder.get());
    compilation_state->InitializeCompilationProgress(num_import_wrappers, num_export_wrappers);
    return builder;
}

bool MayCompriseLazyFunctions(const WasmModule* module, const WasmFeatures& enabled_features, bool lazy_module)
{
    if (lazy_module || enabled_features.has_compilation_hints())
        return true;
#ifdef ENABLE_SLOW_DCHECKS
    int start = module->num_imported_functions;
    int end = start + module->num_declared_functions;
    for (int func_index = start; func_index < end; func_index++) {
        SLOW_DCHECK(GetCompileStrategy(module, enabled_features, func_index, lazy_module) != CompileStrategy::kLazy);
    }
#endif
    return false;
}
...
public:
    ...
    explicit CompilationTimeCallback(std::shared_ptr<Counters> async_counters, std::shared_ptr<metrics::Recorder> metrics_recorder, v8::metrics::Recorder::ContextId context_id,
        ...
        if (compilation_event == CompilationEvent::kFinishedBaselineCompilation) {
            ...
            if (compile_mode_ != kSynchronous) {
                TimedHistogram* histogram
                    = compile_mode_ == kAsync ? async_counters_->wasm_async_compile_wasm_module_time() : async_counters_->wasm_streaming_compile_wasm_module_time();
...
void CompileNativeModule(
    Isolate* isolate, v8::metrics::Recorder::ContextId context_id, ErrorThrower* thrower, const WasmModule* wasm_module, std::shared_ptr<NativeModule> native_module)
{
    CHECK(!v8_flags.jitless);
    ModuleWireBytes wire_bytes(native_module->wire_bytes());
    const bool lazy_module = IsLazyModule(wasm_module);
    if (!v8_flags.wasm_lazy_validation && wasm_module->origin == kWasmOrigin && MayCompriseLazyFunctions(wasm_module, native_module->enabled_features(), lazy_module)) {
        ...
        ValidateSequentially(wasm_module, native_module.get(), isolate->counters(), isolate->allocator(), thrower, lazy_module, kOnlyLazyFunctions);
    ...
    if (base::TimeTicks::IsHighResolution()) {
        compilation_state->AddCallback(
            std::make_unique<CompilationTimeCallback>(isolate->async_counters(), isolate->metrics_recorder(), context_id, native_module, CompilationTimeCallback::kSynchronous));
    }

    // Initialize the compilation units and kick off background compile tasks.
    std::unique_ptr<CompilationUnitBuilder> builder = InitializeCompilation(isolate, native_module.get());
    compilation_state->InitializeCompilationUnits(std::move(builder));

    compilation_state->WaitForCompilationEvent(CompilationEvent::kFinishedExportWrappers);

    if (compilation_state->failed()) {
        DCHECK_IMPLIES(lazy_module, !v8_flags.wasm_lazy_validation);
        ValidateSequentially(wasm_module, native_module.get(), isolate->counters(), isolate->allocator(), thrower, lazy_module);
        CHECK(thrower->error());
        return;
    }
...
public:
    explicit BackgroundCompileJob(std::weak_ptr<NativeModule> native_module, std::shared_ptr<Counters> async_counters)
        : native_module_(std::move(native_module))
    ...
    void Run(JobDelegate* delegate) override
    {
        ...
        if (!engine_scope)
            return;
        ExecuteCompilationUnits(native_module_, async_counters_.get(), delegate, kBaselineOrTopTier);
    }
    ...
    size_t GetMaxConcurrency(size_t worker_count) const override
    {
        ...
        return std::min(static_cast<size_t>(v8_flags.wasm_num_compilation_tasks), worker_count + compile_scope.compilation_state()->NumOutstandingCompilations());
    }
...
std::shared_ptr<NativeModule> CompileToNativeModule(Isolate* isolate, const WasmFeatures& enabled, ErrorThrower* thrower, std::shared_ptr<const WasmModule> module,
    const ModuleWireBytes& wire_bytes, int compilation_id, v8::metrics::Recorder::ContextId context_id)
{
    ...
    std::shared_ptr<NativeModule> native_module = engine->MaybeGetNativeModule(wasm_module->origin, wire_bytes_copy.as_vector(), isolate);
    if (native_module) {
    ...
    if (base::TimeTicks::IsHighResolution()) {
        wasm_compile_module_time_scope.emplace(SELECT_WASM_COUNTER(isolate->counters(), wasm_module->origin, wasm_compile, module_time));
    }
    ...
    size_t code_size_estimate = wasm::WasmCodeManager::EstimateNativeModuleCodeSize(module.get(), include_liftoff, DynamicTiering { v8_flags.wasm_dynamic_tiering.value() });
    ...
    bool cache_hit = !engine->UpdateNativeModuleCache(thrower->error(), &native_module, isolate);
    if (thrower->error())
        return {};
...
void RecompileNativeModule(NativeModule* native_module, TieringState tiering_state)
{
    ...
    public:
        explicit RecompilationFinishedCallback(std::shared_ptr<base::Semaphore> recompilation_finished_semaphore)
            : recompilation_finished_semaphore_(std::move(recompilation_finished_semaphore))
        {
        }
    ...
    // The callback captures a shared ptr to the semaphore.
    // Initialize the compilation units and kick off background compile tasks.
    compilation_state->InitializeRecompilation(tiering_state, std::make_unique<RecompilationFinishedCallback>(recompilation_finished_semaphore));

    constexpr JobDelegate* kNoDelegate = nullptr;
    ExecuteCompilationUnits(compilation_state->native_module_weak(), compilation_state->counters(), kNoDelegate, kBaselineOnly);
    recompilation_finished_semaphore->Wait();
    DCHECK(!compilation_state->failed());
}

AsyncCompileJob::AsyncCompileJob(Isolate* isolate, const WasmFeatures& enabled, std::unique_ptr<byte[]> bytes_copy, size_t length, Handle<Context> context,
    Handle<Context> incumbent_context, const char* api_method_name, std::shared_ptr<CompilationResultResolver> resolver, int compilation_id)
    : isolate_(isolate)
...
public:
    explicit AsyncStreamingProcessor(AsyncCompileJob* job, std::shared_ptr<Counters> counters, AccountingAllocator* allocator);

    ~AsyncStreamingProcessor() override;

    bool ProcessModuleHeader(base::Vector<const uint8_t> bytes, uint32_t offset) override;

    bool ProcessSection(SectionCode section_code, base::Vector<const uint8_t> bytes, uint32_t offset) override;

    bool ProcessCodeSectionHeader(
        int num_functions, uint32_t functions_mismatch_error_offset, std::shared_ptr<WireBytesStorage>, int code_section_start, int code_section_length) override;

    void ProcessFunctionBody(base::Vector<const uint8_t> bytes, uint32_t offset) override;
...
private:
    ...
    void FinishAsyncCompileJobWithError(const WasmError&, ErrorLocation error_location = kErrorInSection);
...
void AsyncCompileJob::CreateNativeModule(std::shared_ptr<const WasmModule> module, size_t code_size_estimate)
{
    ...
    native_module_ = GetWasmEngine()->NewNativeModule(isolate_, enabled_features_, std::move(module), code_size_estimate);
...
{
    native_module_ = GetWasmEngine()->MaybeGetNativeModule(module->origin, wire_bytes_.module_bytes(), isolate_);
...
void AsyncCompileJob::PrepareRuntimeObjects()
{
    ...
    Handle<WasmModuleObject> module_object = WasmModuleObject::New(isolate_, native_module_, script);
...
void AsyncCompileJob::FinishCompile(bool is_after_cache_hit)
{
    ...
    if (script->type() == Script::TYPE_WASM && module->debug_symbols.type == WasmDebugSymbols::Type::SourceMap && !module->debug_symbols.external_url.is_empty()) {
        ModuleWireBytes wire_bytes(module_object_->native_module()->wire_bytes());
        MaybeHandle<String> src_map_str = isolate_->factory()->NewStringFromUtf8(wire_bytes.GetNameOrNull(module->debug_symbols.external_url), AllocationType::kOld);
...
void AsyncCompileJob::DecodeFailed(const WasmError& error)
{
    ...
    std::shared_ptr<AsyncCompileJob> job = GetWasmEngine()->RemoveCompileJob(this);
...
void AsyncCompileJob::AsyncCompileFailed()
{
    ...
    ValidateSequentially(native_module_->module(), native_module_.get(), isolate_->counters(), isolate_->allocator(), &thrower, lazy_module);
    DCHECK(thrower.error());
    // {job} keeps the {this} pointer alive.
    std::shared_ptr<AsyncCompileJob> job = GetWasmEngine()->RemoveCompileJob(this);
    resolver_->OnCompilationFailed(thrower.Reify());
    ...
    void call(CompilationEvent event) override
    {
        ...
        case CompilationEvent::kFinishedBaselineCompilation:
            ...
            if (job_->DecrementAndCheckFinisherCount(kCompilation)) {
                ...
                bool cache_hit = !GetWasmEngine()->UpdateNativeModuleCache(false, &native_module, job_->isolate_);
                DCHECK_EQ(cache_hit, native_module != job_->native_module_);
                job_->DoSync<CompileFinished>(cache_hit ? std::move(native_module) : nullptr);
            }
            break;
        ...
        case CompilationEvent::kFailedCompilation:
            DCHECK(!last_event_.has_value() || last_event_ == CompilationEvent::kFinishedExportWrappers);
            if (job_->DecrementAndCheckFinisherCount(kCompilation)) {
                ...
                std::shared_ptr<NativeModule> native_module = job_->native_module_;
                GetWasmEngine()->UpdateNativeModuleCache(true, &native_module, job_->isolate_);
...
template <typename Step, typename... Args> void AsyncCompileJob::NextStep(Args&&... args) { step_.reset(new Step(std::forward<Args>(args)...)); }
...
public:
    ...
    void RunInBackground(AsyncCompileJob* job) override
    {
        ModuleResult result;
        {
            ...
            result = DecodeWasmModule(enabled_features, job->wire_bytes_.start(), job->wire_bytes_.end(), false, kWasmOrigin, counters_, metrics_recorder_, job->context_id(),
            ...
            if (!v8_flags.wasm_lazy_validation && result.ok()) {
                ...
                if (MayCompriseLazyFunctions(module, enabled_features, lazy_module)) {
                    ...
                    for (int func_index = start; func_index < end; func_index++) {
                        const WasmFunction* func = &module->functions[func_index];
                        base::Vector<const uint8_t> code = job->wire_bytes_.GetFunctionBytes(func);

                        CompileStrategy strategy = GetCompileStrategy(module, enabled_features, func_index, lazy_module);
                        bool validate_lazily_compiled_function = strategy == CompileStrategy::kLazy || strategy == CompileStrategy::kLazyBaselineEagerTopTier;
                        if (validate_lazily_compiled_function) {
                            DecodeResult function_result = ValidateSingleFunction(module, func_index, code, allocator, enabled_features);
                            if (function_result.failed()) {
                                WasmError error = function_result.error();
                                WasmError error_with_name = GetWasmErrorWithName(job->wire_bytes_, func, module, std::move(error));
        ...
        if (result.failed()) {
            ...
            job->DoSync<PrepareAndStartCompile>(std::move(module), true, code_size_estimate);
        }
    }
...
public:
    PrepareAndStartCompile(std::shared_ptr<const WasmModule> module, bool start_compilation, size_t code_size_estimate)
...
private:
    void RunInForeground(AsyncCompileJob* job) override
    {
        ...
        if (base::TimeTicks::IsHighResolution()) {
            ...
            compilation_state->AddCallback(
                std::make_unique<CompilationTimeCallback>(job->isolate_->async_counters(), job->isolate_->metrics_recorder(), job->context_id_, job->native_module_, compile_mode));
        }

        if (start_compilation_) {
            std::unique_ptr<CompilationUnitBuilder> builder = InitializeCompilation(job->isolate(), job->native_module_.get());
            compilation_state->InitializeCompilationUnits(std::move(builder));
...
private:
    void RunInForeground(AsyncCompileJob* job) override
    {
        ...
        if (cached_native_module_) {
            ...
            job->native_module_->SampleCodeSize(job->isolate_->counters(), NativeModule::kAfterBaseline);
        }
...
AsyncStreamingProcessor::AsyncStreamingProcessor(AsyncCompileJob* job, std::shared_ptr<Counters> async_counters, AccountingAllocator* allocator)
...
void AsyncStreamingProcessor::FinishAsyncCompileJobWithError(const WasmError& error, ErrorLocation error_location)
{
    ...
    if (job_->native_module_) {
        CompilationStateImpl* impl = Impl(job_->native_module_->compilation_state());
        ...
        if (error_location == kErrorInSection) {
            job_->DoSync<AsyncCompileJob::DecodeFail, AsyncCompileJob::kUseExistingForegroundTask>(error);
        }
...
bool AsyncStreamingProcessor::ProcessModuleHeader(base::Vector<const uint8_t> bytes, uint32_t offset)
{
    TRACE_STREAMING("Process module header...\n");
    decoder_.StartDecoding(job_->isolate()->counters(), job_->isolate()->metrics_recorder(), job_->context_id(), GetWasmEngine()->allocator());
...
bool AsyncStreamingProcessor::ProcessCodeSectionHeader(
    int num_functions, uint32_t functions_mismatch_error_offset, std::shared_ptr<WireBytesStorage> wire_bytes_storage, int code_section_start, int code_section_length)
{
    ...
    size_t code_size_estimate
        = wasm::WasmCodeManager::EstimateNativeModuleCodeSize(num_functions, num_imported_functions, code_section_length, include_liftoff, job_->dynamic_tiering_);
    job_->DoImmediately<AsyncCompileJob::PrepareAndStartCompile>(decoder_.shared_module(), false, code_size_estimate);
    ...
    compilation_unit_builder_ = InitializeCompilation(job_->isolate(), job_->native_module_.get());
    return true;
}

// Process a function body.
void AsyncStreamingProcessor::ProcessFunctionBody(base::Vector<const uint8_t> bytes, uint32_t offset)
{
    ...
    decoder_.DecodeFunctionBody(func_index, static_cast<uint32_t>(bytes.length()), offset, false);
    ...
    CompileStrategy strategy = GetCompileStrategy(module, enabled_features, func_index, lazy_module);
    bool validate_lazily_compiled_function = !v8_flags.wasm_lazy_validation && (strategy == CompileStrategy::kLazy || strategy == CompileStrategy::kLazyBaselineEagerTopTier);
    if (validate_lazily_compiled_function) {
        ...
        DecodeResult result = ValidateSingleFunction(module, func_index, bytes, allocator_, enabled_features);
...
void AsyncStreamingProcessor::OnFinishedStream(base::OwnedVector<uint8_t> bytes)
{
    ...
    if (prefix_cache_hit_) {
        ...
        const bool include_liftoff = v8_flags.liftoff;
        size_t code_size_estimate = wasm::WasmCodeManager::EstimateNativeModuleCodeSize(result.value().get(), include_liftoff, job_->dynamic_tiering_);
        job_->DoSync<AsyncCompileJob::PrepareAndStartCompile>(std::move(result).value(), true, code_size_estimate);
        return;
    }
    ...
    if (!has_code_section) {
        ...
        cache_hit = job_->GetOrCreateNativeModule(std::move(result).value(), kCodeSizeEstimate);
    } else {
        job_->native_module_->SetWireBytes({ std::move(job_->bytes_copy_), job_->wire_bytes_.length() });
    }
    const bool needs_finish = job_->DecrementAndCheckFinisherCount(AsyncCompileJob::kStreamingDecoder);
    DCHECK_IMPLIES(!has_code_section, needs_finish);
    if (needs_finish) {
        const bool failed = job_->native_module_->compilation_state()->failed();
        if (!cache_hit) {
            cache_hit = !GetWasmEngine()->UpdateNativeModuleCache(failed, &job_->native_module_, job_->isolate_);
        }
        if (failed) {
...
bool AsyncStreamingProcessor::Deserialize(base::Vector<const uint8_t> module_bytes, base::Vector<const uint8_t> wire_bytes)
{
    ...
    MaybeHandle<WasmModuleObject> result = DeserializeNativeModule(job_->isolate_, module_bytes, wire_bytes, base::VectorOf(job_->stream_->url()));
...
CompilationStateImpl::CompilationStateImpl(const std::shared_ptr<NativeModule>& native_module, std::shared_ptr<Counters> async_counters, DynamicTiering dynamic_tiering)
...
void CompilationStateImpl::InitCompileJob()
{
    ...
    compile_job_ = V8::GetCurrentPlatform()->CreateJob(TaskPriority::kUserVisible, std::make_unique<BackgroundCompileJob>(native_module_weak_, async_counters_));
}
...
bool CompilationStateImpl::cancelled() const { return compile_cancelled_.load(std::memory_order_relaxed); }
...
void CompilationStateImpl::InitializeCompilationProgress(int num_import_wrappers, int num_export_wrappers)
{
    DCHECK(!failed());
    ...
    const ExecutionTierPair default_tiers = GetDefaultTiersPerModule(native_module_, dynamic_tiering_, IsLazyModule(module));
    const uint8_t default_progress
        = RequiredBaselineTierField::encode(default_tiers.baseline_tier) | RequiredTopTierField::encode(default_tiers.top_tier) | ReachedTierField::encode(ExecutionTier::kNone);
...
uint8_t CompilationStateImpl::AddCompilationUnitInternal(CompilationUnitBuilder* builder, int function_index, uint8_t function_progress)
{
    ...
    return CompilationStateImpl::RequiredBaselineTierField::encode(required_baseline_tier) | CompilationStateImpl::RequiredTopTierField::encode(required_top_tier)
...
void CompilationStateImpl::InitializeCompilationUnits(std::unique_ptr<CompilationUnitBuilder> builder)
{
    ...
    if (native_module_->IsTieredDown()) {
        ...
        for (size_t i = 0; i < compilation_progress_.size(); ++i) {
            ...
            int func_index = offset + static_cast<int>(i);
            compilation_progress_[i] = AddCompilationUnitInternal(builder.get(), func_index, function_progress);
        }
    }
    builder->Commit();
}
...
void CompilationStateImpl::AddCompilationUnit(CompilationUnitBuilder* builder, int func_index)
{
    ...
    uint8_t updated_function_progress = AddCompilationUnitInternal(builder, func_index, function_progress);
...
void CompilationStateImpl::InitializeCompilationProgressAfterDeserialization(base::Vector<const int> lazy_functions, base::Vector<const int> eager_functions)
{
    ...
    }
    {
        ...
        constexpr uint8_t kProgressForLazyFunctions
            = RequiredBaselineTierField::encode(ExecutionTier::kNone) | RequiredTopTierField::encode(ExecutionTier::kNone) | ReachedTierField::encode(ExecutionTier::kNone);
        ...
        constexpr bool kNotLazy = false;
        ExecutionTierPair default_tiers = GetDefaultTiersPerModule(native_module_, dynamic_tiering_, kNotLazy);
        uint8_t progress_for_eager_functions = RequiredBaselineTierField::encode(default_tiers.baseline_tier) | RequiredTopTierField::encode(default_tiers.top_tier)
...
void CompilationStateImpl::InitializeRecompilation(TieringState new_tiering_state, std::unique_ptr<CompilationEventCallback> recompilation_finished_callback)
{
    DCHECK(!failed());
    ...
    while (outstanding_recompilation_functions_ > 0 && compilation_unit_queues_.GetTotalSize() > 0) {
        guard.reset();
        constexpr JobDelegate* kNoDelegate = nullptr;
        ExecuteCompilationUnits(native_module_weak_, async_counters_.get(), kNoDelegate, kBaselineOrTopTier);
    ...
    std::vector<int> recompile_function_indexes = native_module_->FindFunctionsToRecompile(new_tiering_state);
    ...
    if (compilation_progress_.size() > 0) {
        ...
        DCHECK_GE(module->num_declared_functions, recompile_function_indexes.size());
        outstanding_recompilation_functions_ = static_cast<int>(recompile_function_indexes.size());
        ...
        auto new_tier = new_tiering_state == kTieredDown ? ExecutionTier::kLiftoff : ExecutionTier::kTurbofan;
    ...
    if (outstanding_recompilation_functions_ == 0) {
        TriggerCallbacks(base::EnumSet<CompilationEvent>({ CompilationEvent::kFinishedRecompilation }));
    }
...
void CompilationStateImpl::AddCallback(std::unique_ptr<CompilationEventCallback> callback)
{
    ...
    for (auto event : { CompilationEvent::kFinishedExportWrappers, CompilationEvent::kFinishedBaselineCompilation, CompilationEvent::kFailedCompilation }) {
    ...
    base::Vector<std::shared_ptr<JSToWasmWrapperCompilationUnit>> js_to_wasm_wrapper_units)
{
    if (!js_to_wasm_wrapper_units.empty()) {
        ...
        js_to_wasm_wrapper_units_.insert(js_to_wasm_wrapper_units_.end(), js_to_wasm_wrapper_units.begin(), js_to_wasm_wrapper_units.end());
        // Use release semantics such that updates to {js_to_wasm_wrapper_units_}
        // are available to other threads doing an acquire load.
        outstanding_js_to_wasm_wrappers_.store(js_to_wasm_wrapper_units.size(), std::memory_order_release);
    }
...
void CompilationStateImpl::CommitTopTierCompilationUnit(WasmCompilationUnit unit) { CommitCompilationUnits({}, { &unit, 1 }, {}); }
...
std::shared_ptr<JSToWasmWrapperCompilationUnit> CompilationStateImpl::GetNextJSToWasmWrapperCompilationUnit()
{
    size_t outstanding_units = outstanding_js_to_wasm_wrappers_.load(std::memory_order_relaxed);
    // Use acquire semantics such that initialization of
    // {js_to_wasm_wrapper_units_} is available.
    while (outstanding_units && !outstanding_js_to_wasm_wrappers_.compare_exchange_weak(outstanding_units, outstanding_units - 1, std::memory_order_acquire)) {
...
void CompilationStateImpl::FinalizeJSToWasmWrappers(Isolate* isolate, const WasmModule* module)
{
    ...
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("v8.wasm.detailed"), "wasm.FinalizeJSToWasmWrappers", "wrappers", js_to_wasm_wrapper_units_.size());

    isolate->heap()->EnsureWasmCanonicalRttsSize(module->MaxCanonicalTypeIndex() + 1);
    ...
    for (auto& unit : js_to_wasm_wrapper_units_) {
        ...
        uint32_t index = GetExportWrapperIndex(unit->canonical_sig_index(), unit->is_import());
        isolate->heap()->js_to_wasm_wrappers().Set(index, MaybeObject::FromObject(*code));
        RecordStats(*code, isolate->counters());
    }
}

CompilationUnitQueues::Queue* CompilationStateImpl::GetQueueForCompileTask(int task_id) { return compilation_unit_queues_.GetQueueForTask(task_id); }

base::Optional<WasmCompilationUnit> CompilationStateImpl::GetNextCompilationUnit(CompilationUnitQueues::Queue* queue, CompileBaselineOnly baseline_only)
{
    ...
    for (size_t i = 0; i < code_vector.size(); i++) {
        ...
        if (code->index() < static_cast<int>(native_module_->num_imported_functions())) {
            ...
            if (V8_UNLIKELY(MissingRecompilationField::decode(function_progress))) {
                ...
                // non-debugging units.
                bool matches = tiering_state_ == kTieredDown ? code->for_debugging() : code->tier() == ExecutionTier::kTurbofan;
                if (matches) {
                    outstanding_recompilation_functions_--;
                    compilation_progress_[slot_index] = MissingRecompilationField::update(compilation_progress_[slot_index], false);
                    if (outstanding_recompilation_functions_ == 0) {
...
void CompilationStateImpl::TriggerCallbacks(base::EnumSet<CompilationEvent> triggered_events)
{
    ...
    finished_events_ |= triggered_events - CompilationEvent::kFinishedRecompilation - CompilationEvent::kFinishedCompilationChunk;

    for (auto event :
        { std::make_pair(CompilationEvent::kFailedCompilation, "wasm.CompilationFailed"), std::make_pair(CompilationEvent::kFinishedExportWrappers, "wasm.ExportWrappersFinished"),
            ...
            std::make_pair(CompilationEvent::kFinishedRecompilation, "wasm.RecompilationFinished") }) {
    ...
    if (outstanding_baseline_units_ == 0 && outstanding_export_wrappers_ == 0 && outstanding_recompilation_functions_ == 0) {
...
void CompilationStateImpl::PublishCompilationResults(std::vector<std::unique_ptr<WasmCode>> unpublished_code)
{
    ...
    for (const auto& code : unpublished_code) {
        ...
        if (func_index < num_imported_functions) {
            const WasmFunction& function = native_module_->module()->functions[func_index];
            uint32_t canonical_type_index = native_module_->module()->isorecursive_canonical_type_ids[function.sig_index];
            WasmImportWrapperCache::CacheKey key(compiler::kDefaultImportCallKind, canonical_type_index, static_cast<int>(function.sig->parameter_count()), kNoSuspend);
...
void CompilationStateImpl::PublishCode(base::Vector<std::unique_ptr<WasmCode>> code)
{
    WasmCodeRefScope code_ref_scope;
...
void CompilationStateImpl::SchedulePublishCompilationResults(std::vector<std::unique_ptr<WasmCode>> unpublished_code)
{
    {
...
{
    size_t outstanding_wrappers = outstanding_js_to_wasm_wrappers_.load(std::memory_order_relaxed);
...
void CompilationStateImpl::WaitForCompilationEvent(CompilationEvent expect_event)
{
    ...
    public:
        WaitForCompilationEventCallback(std::shared_ptr<base::Semaphore> semaphore, std::shared_ptr<std::atomic<bool>> done, base::EnumSet<CompilationEvent> events)
    ...
    {
        base::MutexGuard callbacks_guard(&callbacks_mutex_);
        if (finished_events_.contains_any(events))
            return;
        callbacks_.emplace_back(std::make_unique<WaitForCompilationEventCallback>(semaphore, done, events));
    }
    ...
    public:
        ...
        bool ShouldYield() override { return done_->load(std::memory_order_relaxed); }
    ...
    ExecuteCompilationUnits(native_module_weak_, async_counters_.get(), &delegate, kBaselineOnly);
    semaphore->Wait();
}

namespace {
using JSToWasmWrapperQueue = WrapperQueue<JSToWasmWrapperKey, std::nullptr_t, base::hash<JSToWasmWrapperKey>>;
using JSToWasmWrapperUnitMap = std::unordered_map<JSToWasmWrapperKey, std::unique_ptr<JSToWasmWrapperCompilationUnit>, base::hash<JSToWasmWrapperKey>>;

class CompileJSToWasmWrapperJob final : public JobTask {
public:
    CompileJSToWasmWrapperJob(JSToWasmWrapperQueue* queue, JSToWasmWrapperUnitMap* compilation_units)
        : queue_(queue)
    ...
    void Run(JobDelegate* delegate) override
    {
        while (base::Optional<std::pair<JSToWasmWrapperKey, std::nullptr_t>> key = queue_->pop()) {
            JSToWasmWrapperCompilationUnit* unit = (*compilation_units_)[key->first].get();
            unit->Execute();
    ...
    size_t GetMaxConcurrency(size_t /* worker_count */) const override
    {
        ...
        return std::min(static_cast<size_t>(v8_flags.wasm_num_compilation_tasks), outstanding_units_.load(std::memory_order_relaxed));
    }
...
void CompileJsToWasmWrappers(Isolate* isolate, const WasmModule* module)
{
    ...
    for (auto exp : module->export_table) {
        ...
        uint32_t canonical_type_index = module->isorecursive_canonical_type_ids[function.sig_index];
        int wrapper_index = GetExportWrapperIndex(canonical_type_index, function.imported);
        auto existing_wrapper = isolate->heap()->js_to_wasm_wrappers().Get(wrapper_index);
        if (existing_wrapper.IsStrongOrWeak() && !existing_wrapper.GetHeapObject().IsUndefined()) {
            continue;
        }
        ...
        if (queue.insert(key, nullptr)) {
            auto unit = std::make_unique<JSToWasmWrapperCompilationUnit>(
                isolate, function.sig, canonical_type_index, module, function.imported, enabled_features, JSToWasmWrapperCompilationUnit::kAllowGeneric);
...

    {
        ...
        TRACE_EVENT1("v8.wasm", "wasm.JsToWasmWrapperCompilation", "num_wrappers", compilation_units.size());
        auto job = std::make_unique<CompileJSToWasmWrapperJob>(&queue, &compilation_units);
        if (v8_flags.wasm_num_compilation_tasks > 0) {
            auto job_handle = V8::GetCurrentPlatform()->CreateJob(TaskPriority::kUserVisible, std::move(job));
    ...
    for (auto& pair : compilation_units) {
        ...
        isolate->heap()->js_to_wasm_wrappers().Set(wrapper_index, HeapObjectReference::Strong(*code));
        RecordStats(*code, isolate->counters());
    }
}

WasmCode* CompileImportWrapper(NativeModule* native_module, Counters* counters, compiler::WasmImportCallKind kind, const FunctionSig* sig, uint32_t canonical_type_index,
    int expected_arity, Suspend suspend, WasmImportWrapperCache::ModificationScope* cache_scope)
{
    ...
    WasmCodeRefScope code_ref_scope;
    CompilationEnv env = native_module->CreateCompilationEnv();
    WasmCompilationResult result = compiler::CompileWasmImportCallWrapper(&env, kind, sig, source_positions, expected_arity, suspend);
    WasmCode* published_code;
    {
        CodeSpaceWriteScope code_space_write_scope(native_module);
        std::unique_ptr<WasmCode> wasm_code = native_module->AddCode(result.func_index, result.code_desc, result.frame_slot_count, result.tagged_parameter_slots,
            result.protected_instructions_data.as_vector(), result.source_positions.as_vector(), GetCodeKind(result), ExecutionTier::kNone, kNoDebugging);
        published_code = native_module->PublishCode(std::move(wasm_code));
    }
    (*cache_scope)[key] = published_code;
    published_code->IncRef();
    counters->wasm_generated_code_size()->Increment(published_code->instructions().length());
