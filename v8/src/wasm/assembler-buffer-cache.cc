...
public:
    ...
    uint8_t* start() const override { return reinterpret_cast<uint8_t*>(region_.begin()); }

    int size() const override { return static_cast<int>(region_.size()); }

    std::unique_ptr<AssemblerBuffer> Grow(int new_size) override { return cache_->GetAssemblerBuffer(new_size); }
...
std::unique_ptr<AssemblerBuffer> AssemblerBufferCache::GetAssemblerBuffer(int size)
{
    DCHECK_LT(0, size);
    base::AddressRegion region = available_memory_.Allocate(size);
    if (region.is_empty()) {
        static constexpr int kMinimumReservation = 64 * KB;
        int minimum_allocation = std::max(kMinimumReservation, std::max(total_allocated_ / 4, size));
        base::AddressRegion new_space = GetWasmCodeManager()->AllocateAssemblerBufferSpace(minimum_allocation);
...
void AssemblerBufferCache::Return(base::AddressRegion region) { available_memory_.Merge(region); }
