...
public:
    ...
    void InitializerExpression(const byte* start, const byte* end, ValueType expected_type) { }
...
template <class Tracer> inline WireBytesRef consume_string(Decoder* decoder, unibrow::Utf8Variant grammar, const char* name, Tracer& tracer)
{
...
template <class Tracer> inline WireBytesRef consume_utf8_string(Decoder* decoder, const char* name, Tracer& tracer)
{
    return consume_string(decoder, unibrow::Utf8Variant::kUtf8, name, tracer);
}

template <class Tracer> inline SectionCode IdentifyUnknownSectionInternal(Decoder* decoder, Tracer& tracer)
{
    ...
    TRACE("  +%d  section name        : \"%.*s\"\n", static_cast<int>(section_name_start - decoder->start()), string.length() < 20 ? string.length() : 20, section_name_start);
    ...
    static constexpr SpecialSectionPair kSpecialSections[] { { base::StaticCharVector(kNameString), kNameSectionCode },
        ...
        { base::StaticCharVector(kDebugInfoString), kDebugInfoSectionCode }, { base::StaticCharVector(kExternalDebugInfoString), kExternalDebugInfoSectionCode } };
...
template <class Tracer> class WasmSectionIterator {
public:
    ...
    uint32_t section_length() const { return static_cast<uint32_t>(section_end_ - section_start_); }

    base::Vector<const uint8_t> payload() const { return { payload_start_, payload_length() }; }

    const byte* payload_start() const { return payload_start_; }

    uint32_t payload_length() const { return static_cast<uint32_t>(section_end_ - payload_start_); }
...
template <class T> WasmSectionIterator(Decoder*, T&) -> WasmSectionIterator<T>;

// The main logic for decoding the bytes of a module.
template <class Tracer> class ModuleDecoderTemplate : public Decoder {
public:
    explicit ModuleDecoderTemplate(const WasmFeatures& enabled, ModuleOrigin origin, Tracer& tracer)
    ...
    ModuleDecoderTemplate(const WasmFeatures& enabled, const byte* module_start, const byte* module_end, ModuleOrigin origin, Tracer& tracer)
    ...
    void StartDecoding(Counters* counters, AccountingAllocator* allocator)
    {
        ...
        module_.reset(new WasmModule(std::make_unique<Zone>(allocator, "signatures")));
...
#undef BYTES
    ...
    void DecodeSection(SectionCode section_code, base::Vector<const uint8_t> bytes, uint32_t offset, bool validate_functions = true)
    {
        if (failed())
            return;
        Reset(bytes, offset);
        ...
        case kTagSectionCode:
            if (enabled_features_.has_eh()) {
                ...
                errorf(pc(), "unexpected section <%s> (enable with --experimental-wasm-eh)", SectionName(section_code));
            }
            break;
        case kStringRefSectionCode:
    ...
    bool check_supertype(uint32_t supertype)
    {
        if (V8_UNLIKELY(supertype >= module_->types.size())) {
            errorf(pc(), "type %zu: forward-declared supertype %d", module_->types.size(), supertype);
            return false;
        }
        return true;
    }
    ...
    TypeDefinition consume_subtype_definition()
    {
        ...
        if (kind == kWasmSubtypeCode) {
            ...
            uint32_t supertype = supertype_count == 1 ? consume_u32v("supertype", tracer_) : kNoSuperType;
            if (supertype_count == 1) {
                tracer_.Description(supertype);
                tracer_.NextLine();
            }
            if (!check_supertype(supertype))
                return {};
    ...
    void DecodeTypeSection()
    {
        ...
        if (!enabled_features_.has_gc()) {
            ...
            for (uint32_t i = 0; i < types_count; ++i) {
                ...
                case kWasmRecursiveTypeGroupCode:
                    errorf("Unknown type code 0x%02x, enable with --experimental-wasm-gc", opcode);
                    return;
                default:
                    errorf("Expected signature definition 0x%02x, got 0x%02x", kWasmFunctionTypeCode, opcode);
                    return;
                }
            }
            return;
        ...
        for (uint32_t i = 0; ok() && i < module_->types.size(); ++i) {
            ...
            if (explicit_super == kNoSuperType)
                continue;
    ...
    void DecodeImportSection()
    {
        ...
        for (uint32_t i = 0; ok() && i < import_table_count; ++i) {
            ...
            import->kind = static_cast<ImportExportKindCode>(consume_u8("kind: ", tracer_));
            ...
            case kExternalTable: {
                ...
                consume_resizable_limits("element count", "elements", std::numeric_limits<uint32_t>::max(), &table->initial_size, &table->has_maximum_size,
            ...
            case kExternalMemory: {
                ...
                if (!AddMemory(module_.get()))
                    break;
                uint8_t flags = validate_memory_flags(&module_->has_shared_memory, &module_->is_memory64);
                uint32_t max_pages = module_->is_memory64 ? kSpecMaxMemory64Pages : kSpecMaxMemory32Pages;
                consume_resizable_limits("memory", "pages", max_pages, &module_->initial_pages, &module_->has_maximum_pages, max_pages, &module_->maximum_pages, flags);
                break;
            }
            case kExternalGlobal: {
                // ===== Imported global =============================================
    ...
    void DecodeTableSection()
    {
        ...
        for (uint32_t i = 0; ok() && i < table_count; i++) {
            ...
            if (enabled_features_.has_typed_funcref() && read_u8<Decoder::kFullValidation>(pc(), "table-with-initializer byte") == 0x40) {
            ...
            consume_resizable_limits("table elements", "elements", std::numeric_limits<uint32_t>::max(), &table->initial_size, &table->has_maximum_size,
                std::numeric_limits<uint32_t>::max(), &table->maximum_size, flags);
    ...
    void DecodeExportSection()
    {
        ...
        for (uint32_t i = 0; ok() && i < export_table_count; ++i) {
            ...
            const byte* pos = pc();
            exp->kind = static_cast<ImportExportKindCode>(consume_u8("kind: ", tracer_));
    ...
    void DecodeElementSection()
    {
        ...
        for (uint32_t i = 0; i < segment_count; ++i) {
            ...
            for (uint32_t j = 0; j < num_elem; j++) {
                ConstantExpression entry = segment.element_type == WasmElemSegment::kExpressionElements ? consume_init_expr(module_.get(), segment.type)
                                                                                                        : ConstantExpression::RefFunc(consume_element_func_index(segment.type));
                if (failed())
                    return;
    ...
    void DecodeFunctionBody(uint32_t index, uint32_t length, uint32_t offset, bool validate_functions)
    {
        ...
        if (validate_functions) {
            ...
            ValidateFunctionBody(module_->signature_zone->allocator(), index + module_->num_imported_functions, bytes, module_.get(), function);
        }
    }
        ...
        if (!has_seen_unordered_section(kNameSectionCode)) {
            ...
            while (inner.ok() && inner.more()) {
                ...
                if (name_type == NameSectionKindCode::kModuleCode) {
                    WireBytesRef name = consume_string(&inner, unibrow::Utf8Variant::kLossyUtf8, "module name", tracing_already_done);
    ...
    void DecodeSourceMappingURLSection()
    {
        ...
        if (inner.ok() && module_->debug_symbols.type != WasmDebugSymbols::Type::SourceMap) {
    ...
    ModuleResult DecodeModule(Counters* counters, AccountingAllocator* allocator, bool validate_functions = true)
    {
        ...
        while (ok()) {
            ...
            if (section_iter.section_code() != SectionCode::kUnknownSectionCode) {
                DecodeSection(section_iter.section_code(), section_iter.payload(), offset, validate_functions);
            }
    ...
    FunctionResult DecodeSingleFunctionForTesting(Zone* zone, const ModuleWireBytes& wire_bytes, const WasmModule* module)
    {
        pc_ = start_;
    ...
    ConstantExpression DecodeInitExprForTesting(ValueType expected) { return consume_init_expr(module_.get(), expected); }
...
private:
    ...
    static_assert(sizeof(ModuleDecoderTemplate::next_ordered_section_) == sizeof(SectionCode), "type mismatch");
    uint32_t seen_unordered_sections_ = 0;
    static_assert(kBitsPerByte * sizeof(ModuleDecoderTemplate::seen_unordered_sections_) > kLastKnownModuleSection, "not enough bits");
    ...
    bool has_seen_unordered_section(SectionCode section_code) { return seen_unordered_sections_ & (1 << section_code); }

    void set_seen_unordered_section(SectionCode section_code) { seen_unordered_sections_ |= 1 << section_code; }

    uint32_t off(const byte* ptr) { return static_cast<uint32_t>(ptr - start_) + buffer_offset_; }
    ...
    void ValidateFunctionBody(AccountingAllocator* allocator, uint32_t func_num, const ModuleWireBytes& wire_bytes, const WasmModule* module, WasmFunction* function)
    {
        if (v8_flags.trace_wasm_decoder) {
            WasmFunctionName func_name(function, wire_bytes.GetNameOrNull(function, module));
            StdoutStream {} << "Verifying wasm function " << func_name << std::endl;
        }
        FunctionBody body
            = { function->sig, function->code.offset(), start_ + GetBufferRelativeOffset(function->code.offset()), start_ + GetBufferRelativeOffset(function->code.end_offset()) };

        WasmFeatures unused_detected_features = WasmFeatures::None();
        DecodeResult result = wasm::ValidateFunctionBody(allocator, enabled_features_, module, &unused_detected_features, body);
        ...
        if (result.failed() && error_.empty()) {
            ...
            error_msg << "in function " << func_name << ": " << result.error().message();
    ...
    uint32_t consume_sig_index(WasmModule* module, const FunctionSig** sig)
    {
        ...
        if (!module->has_signature(sig_index)) {
            errorf(pos, "signature index %u out of bounds (%d signatures)", sig_index, static_cast<int>(module->types.size()));
            *sig = nullptr;
            return 0;
        }
    ...
    uint32_t consume_func_index(WasmModule* module, WasmFunction** func) { return consume_index("function", &module->functions, func); }

    uint32_t consume_global_index(WasmModule* module, WasmGlobal** global) { return consume_index("global", &module->globals, global); }

    uint32_t consume_table_index(WasmModule* module, WasmTable** table) { return consume_index("table", &module->tables, table); }

    uint32_t consume_tag_index(WasmModule* module, WasmTag** tag) { return consume_index("tag", &module->tags, tag); }
    ...
    void consume_resizable_limits(
        const char* name, const char* units, uint32_t max_initial, uint32_t* initial, bool* has_max, uint32_t max_maximum, uint32_t* maximum, uint8_t flags)
    {
        const byte* pos = pc();
        ...
        const bool is_memory64 = flags == kMemory64NoMaximum || flags == kMemory64WithMaximum;
        uint64_t initial_64 = is_memory64 ? consume_u64v("initial size", tracer_) : consume_u32v("initial size", tracer_);
        ...
        *initial = static_cast<uint32_t>(initial_64);
        ...
        if (flags & 1) {
            *has_max = true;
            pos = pc();
            uint64_t maximum_64 = is_memory64 ? consume_u64v("maximum size", tracer_) : consume_u32v("maximum size", tracer_);
...
#define TYPE_CHECK(found)                                                                                                                                                          \
    if (V8_UNLIKELY(!IsSubtypeOf(found, expected, module_.get()))) {                                                                                                               \
        ...
        case kExprI32Const: {
            int32_t value = read_i32v<kFullValidation>(pc() + 1, &length, "i32.const");
        ...
        case kExprRefFunc: {
            uint32_t index = read_u32v<kFullValidation>(pc() + 1, &length, "ref.func");
            if (V8_UNLIKELY(failed()))
                return {};
            if (V8_LIKELY(lookahead(1 + length, kExprEnd))) {
                ...
                ValueType type = enabled_features_.has_typed_funcref() ? ValueType::Ref(module_->functions[index].sig_index) : kWasmFuncRef;
                TYPE_CHECK(type)
        ...
        case kExprRefNull: {
            HeapType type = value_type_reader::read_heap_type<kFullValidation>(this, pc() + 1, &length, module_.get(), enabled_features_);
            if (V8_UNLIKELY(failed()))
                return {};
            if (V8_LIKELY(lookahead(1 + length, kExprEnd))) {
...
#undef TYPE_CHECK
        ...
        WasmFullDecoder<Decoder::kFullValidation, ConstantExpressionInterface, kConstantExpression> decoder(&init_expr_zone_, module, enabled_features_, &detected, body, module);
        ...
        return ConstantExpression::WireBytes(offset, static_cast<uint32_t>(decoder.end() - decoder.start()));
    }
    ...
    ValueType consume_value_type()
    {
        ...
        ValueType result
            = value_type_reader::read_value_type<kFullValidation>(this, pc_, &type_length, module_.get(), origin_ == kWasmOrigin ? enabled_features_ : WasmFeatures::None());
    ...
    {
        uint32_t type_length;
        HeapType result = value_type_reader::read_heap_type<kFullValidation>(this, pc_, &type_length, module_.get(), enabled_features_);
        tracer_.Bytes(pc_, type_length);
        tracer_.Description(result);
    ...
    WasmElemSegment consume_element_segment_header()
    {
        ...
        constexpr uint8_t kFullMask = kNonActiveMask | kHasTableIndexOrIsDeclarativeMask | kExpressionsAsElementsMask;
    ...
    tracer_.Description(status == WasmElemSegment::kStatusActive ? "active" :
        ...
        WasmElemSegment::ElementType element_type = flag & kExpressionsAsElementsMask ? WasmElemSegment::kExpressionElements : WasmElemSegment::kFunctionIndexElements;
        ...
        if (element_type == WasmElemSegment::kExpressionElements) {
            ...
            if (!is_active) {
                ...
                // functions.)
                if (V8_UNLIKELY(!IsSubtypeOf(table_type, kWasmFuncRef, this->module_.get()))) {
                    errorf(pos,
                        "An active element segment with function indices as elements "
                        "must reference a table of %s. Instead, table %u of type %s "
                        "is referenced.",
                        enabled_features_.has_typed_funcref() ? "a subtype of type funcref" : "type funcref", table_index, table_type.name().c_str());
                    return {};
                }
            }
        }
    ...
    void consume_data_segment_header(bool* is_active, uint32_t* index, ConstantExpression* offset)
    {
        const byte* pos = pc();
        uint32_t flag = consume_u32v("flag: ", tracer_);
        ...
        if (flag != SegmentFlags::kActiveNoIndex && flag != SegmentFlags::kPassive && flag != SegmentFlags::kActiveWithIndex) {
