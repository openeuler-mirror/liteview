...
void Disassemble(const WasmModule* module, ModuleWireBytes wire_bytes, NamesProvider* names, v8::debug::DisassemblyCollector* collector, std::vector<int>* function_body_offsets)
{
    ...
    ModuleDisassembler md(out, module, names, wire_bytes, &allocator, function_body_offsets);
    md.PrintModule({ 0, 2 });
...
void PrintSignatureOneLine(StringBuilder& out, const FunctionSig* sig, uint32_t func_index, NamesProvider* names, bool param_names, IndexAsComment indices_as_comments)
{
    if (param_names) {
...
void FunctionBodyDisassembler::DecodeAsWat(MultiLineStringBuilder& out, Indentation indentation, FunctionHeader include_header, uint32_t* first_instruction_offset)
{
    out_ = &out;
    ...
    while (pc_ < end_) {
        ...
        out << indentation;
        if (opcode == kExprElse || opcode == kExprCatch || opcode == kExprCatchAll || opcode == kExprBlock || opcode == kExprIf || opcode == kExprLoop || opcode == kExprTry) {
            indentation.increase();
        }
...
template <Decoder::ValidateFlag validate> class ImmediatesPrinter {
public:
    ...
    void BranchDepth(BranchDepthImmediate<validate>& imm) { PrintDepthAsLabel(imm.depth); }
    ...
    void SimdLane(SimdLaneImmediate<validate>& imm) { out_ << " " << uint32_t { imm.lane }; }
...
public:
    ...
    void CollectOffsets(const WasmModule* module, const byte* start, const byte* end, AccountingAllocator* allocator)
    {
        ...
        OffsetsCollectingDecoder decoder(WasmFeatures::All(), start, end, kWasmOrigin, *this);
    ...
    , function_body_offsets_(function_body_offsets)
{
    if (function_body_offsets != nullptr) {
        offsets_->CollectOffsets(module, wire_bytes_.start(), wire_bytes_.end(), allocator);
    }
}
...
void ModuleDisassembler::PrintModule(Indentation indentation)
{
    ...
    // V. Memories
    static_assert(kV8MaxWasmMemories == 1, "Code below needs updating for multi-memory");
    ...
    for (uint32_t i = module_->num_imported_functions; i < module_->functions.size(); i++) {
        ...
        FunctionBodyDisassembler d(&zone_, module_, i, &detected, func->sig, code.begin(), code.end(), func->code.offset(), names_);
...
void ModuleDisassembler::PrintInitExpression(const ConstantExpression& init, ValueType expected_type)
{
    ...
    case ConstantExpression::kWireBytesRef:
        ...
        WasmFeatures detected;
        FunctionBodyDisassembler d(&zone_, module_, 0, &detected, &sig, start, end, ref.offset(), names_);
...
{
    for (const byte* ptr = start_ + ref.offset(); ptr < start_ + ref.end_offset(); ptr++) {
        byte b = *ptr;
        if (b < 32 || b >= 127 || b == '"' || b == '\\') {
