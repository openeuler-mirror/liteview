...
public:
    ...
    base::Vector<byte> current_buffer() const { return { current_location(), current_size() }; }
    ...
    void WriteVector(const base::Vector<const byte> v)
    {
        ...
        if (v8_flags.trace_wasm_serialization) {
            StdoutStream {} << "wrote vector of " << v.size() << " elements" << std::endl;
        }
    }
...
public:
    ...
    size_t current_size() const { return end_ - pos_; }
    base::Vector<const byte> current_buffer() const { return { current_location(), current_size() }; }
...
void SetWasmCalleeTag(RelocInfo* rinfo, uint32_t tag)
{
    ...
    if (instr->IsLdrLiteralX()) {
        ...
        instr->SetBranchImmTarget(reinterpret_cast<Instruction*>(rinfo->pc() + tag * kInstrSize));
    }
#else
...
NativeModuleSerializer::NativeModuleSerializer(const NativeModule* module, base::Vector<WasmCode* const> code_table)
...
size_t NativeModuleSerializer::MeasureCode(const WasmCode* code) const
{
    ...
    return kCodeHeaderSize + code->instructions().size() + code->reloc_info().size() + code->source_positions().size() + code->protected_instructions_data().size();
}
...
void NativeModuleSerializer::WriteCode(const WasmCode* code, Writer* writer)
{
    ...
    if (code->tier() != ExecutionTier::kTurbofan) {
        ...
        uint32_t budget = native_module->tiering_budget_array()[declared_function_index(native_module->module(), code->index())];
...
#if V8_TARGET_ARCH_MIPS64 || V8_TARGET_ARCH_ARM || V8_TARGET_ARCH_PPC || V8_TARGET_ARCH_PPC64 || V8_TARGET_ARCH_S390X || V8_TARGET_ARCH_RISCV32 || V8_TARGET_ARCH_RISCV64
    ...
    // Relocate the code.
    int mask = RelocInfo::ModeMask(RelocInfo::WASM_CALL) | RelocInfo::ModeMask(RelocInfo::WASM_STUB_CALL) | RelocInfo::ModeMask(RelocInfo::EXTERNAL_REFERENCE)
        | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE) | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE_ENCODED);
    RelocIterator orig_iter(code->instructions(), code->reloc_info(), code->constant_pool(), mask);
    for (RelocIterator iter({ code_start, code->instructions().size() }, code->reloc_info(), reinterpret_cast<Address>(code_start) + code->constant_pool_offset(), mask);
...
public:
    ...
    base::Vector<const int> lazy_functions() { return base::VectorOf(lazy_functions_); }

    base::Vector<const int> eager_functions() { return base::VectorOf(eager_functions_); }
...
bool NativeModuleDeserializer::Read(Reader* reader)
{
    ...
    std::unique_ptr<JobHandle> job_handle = V8::GetCurrentPlatform()->CreateJob(TaskPriority::kUserVisible, std::make_unique<DeserializeCodeTask>(this, &reloc_queue));
...
void NativeModuleDeserializer::ReadHeader(Reader* reader) { remaining_code_size_ = reader->Read<size_t>(); }
...
DeserializationUnit NativeModuleDeserializer::ReadCode(int fn_index, Reader* reader)
{
    ...
    if (current_code_space_.size() < static_cast<size_t>(code_size)) {
        ...
        constexpr size_t kMaxReservation = RoundUp<kCodeAlignment>(WasmCodeAllocator::kMaxCodeSpaceSize * 9 / 10);
    ...
    auto protected_instructions = reader->ReadVector<byte>(protected_instructions_size);
    ...
    unit.code = native_module_->AddDeserializedCode(fn_index, instructions, stack_slot_count, tagged_parameter_slots, safepoint_table_offset, handler_table_offset,
        constant_pool_offset, code_comment_offset, unpadded_binary_size, protected_instructions, reloc_info, source_pos, kind, tier);
...
{
    memcpy(unit.code->instructions().begin(), unit.src_code_buffer.begin(), unit.src_code_buffer.size());

    // Relocate the code.
    int mask = RelocInfo::ModeMask(RelocInfo::WASM_CALL) | RelocInfo::ModeMask(RelocInfo::WASM_STUB_CALL) | RelocInfo::ModeMask(RelocInfo::EXTERNAL_REFERENCE)
        | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE) | RelocInfo::ModeMask(RelocInfo::INTERNAL_REFERENCE_ENCODED);
    for (RelocIterator iter(unit.code->instructions(), unit.code->reloc_info(), unit.code->constant_pool(), mask); !iter.done(); iter.next()) {
        ...
        case RelocInfo::WASM_STUB_CALL: {
            ...
            Address target = native_module_->GetNearRuntimeStubEntry(static_cast<WasmCode::RuntimeStubId>(tag), unit.jump_tables);
...
MaybeHandle<WasmModuleObject> DeserializeNativeModule(Isolate* isolate, base::Vector<const byte> data, base::Vector<const byte> wire_bytes_vec, base::Vector<const char> source_url)
{
    ...
    ModuleResult decode_result = DecodeWasmModule(enabled_features, owned_wire_bytes.start(), owned_wire_bytes.end(), false, i::wasm::kWasmOrigin, isolate->counters(),
        isolate->metrics_recorder(), isolate->GetOrRegisterRecorderContextId(isolate->native_context()), DecodingMethod::kDeserialize, wasm_engine->allocator());
    if (decode_result.failed())
        return {};
    std::shared_ptr<WasmModule> module = std::move(decode_result).value();
    CHECK_NOT_NULL(module);

    auto shared_native_module = wasm_engine->MaybeGetNativeModule(module->origin, owned_wire_bytes.as_vector(), isolate);
    if (shared_native_module == nullptr) {
        ...
        shared_native_module = wasm_engine->NewNativeModule(isolate, enabled_features, std::move(module), code_size_estimate);
    ...
    Handle<Script> script = wasm_engine->GetOrCreateScript(isolate, shared_native_module, source_url);
