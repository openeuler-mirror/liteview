...
public:
    WasmStreamingImpl(Isolate* isolate, const char* api_method_name, std::shared_ptr<internal::wasm::CompilationResultResolver> resolver)
        : isolate_(isolate)
        , resolver_(std::move(resolver))
    {
        i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate_);
        auto enabled_features = i::wasm::WasmFeatures::FromIsolate(i_isolate);
        streaming_decoder_ = i::wasm::GetWasmEngine()->StartStreamingCompilation(i_isolate, enabled_features, handle(i_isolate->context(), i_isolate), api_method_name, resolver_);
    }

    void OnBytesReceived(const uint8_t* bytes, size_t size) { streaming_decoder_->OnBytesReceived(base::VectorOf(bytes, size)); }
    void Finish(bool can_use_compiled_module) { streaming_decoder_->Finish(can_use_compiled_module); }
    ...
    {
        if (!i::wasm::IsSupportedVersion(bytes))
            return false;
...
i::Handle<i::String> v8_str(i::Isolate* isolate, const char* str) { return isolate->factory()->NewStringFromAsciiChecked(str); }
Local<String> v8_str(Isolate* isolate, const char* str) { return Utils::ToLocal(v8_str(reinterpret_cast<i::Isolate*>(isolate), str)); }

#define GET_FIRST_ARGUMENT_AS(Type)                                                                                                                                                \
    i::MaybeHandle<i::Wasm##Type##Object> GetFirstArgumentAs##Type(const v8::FunctionCallbackInfo<v8::Value>& args, ErrorThrower* thrower)                                         \
    {                                                                                                                                                                              \
...
i::wasm::ModuleWireBytes GetFirstArgumentAsBytes(const v8::FunctionCallbackInfo<v8::Value>& args, ErrorThrower* thrower, bool* is_shared)
{
    ...
    if (length > max_length) {
        thrower->RangeError("buffer source exceeds maximum size of %zu (is %zu)", max_length, length);
    }
    if (thrower->error())
...
public:
    ...
    void OnCompilationSucceeded(i::Handle<i::WasmModuleObject> result) override
    {
        ...
        CHECK(callback);
        callback(isolate_, context_.Get(isolate_), promise_resolver_.Get(isolate_), Utils::ToLocal(i::Handle<i::Object>::cast(result)), WasmAsyncSuccess::kSuccess);
    }
...
public:
    ...
    void OnInstantiationSucceeded(i::Handle<i::WasmInstanceObject> instance) override
    {
        ...
        auto callback = reinterpret_cast<i::Isolate*>(isolate_)->wasm_async_resolve_promise_callback();
        CHECK(callback);
        callback(isolate_, context_.Get(isolate_), promise_resolver_.Get(isolate_), Utils::ToLocal(i::Handle<i::Object>::cast(instance)), WasmAsyncSuccess::kSuccess);
    }
...
public:
    ...
    void OnInstantiationSucceeded(i::Handle<i::WasmInstanceObject> instance) override
    {
        ...
        result->CreateDataProperty(context, v8_str(isolate_, "module"), module_.Get(isolate_)).Check();
        result->CreateDataProperty(context, v8_str(isolate_, "instance"), Utils::ToLocal(i::Handle<i::Object>::cast(instance))).Check();

        auto callback = reinterpret_cast<i::Isolate*>(isolate_)->wasm_async_resolve_promise_callback();
        CHECK(callback);
        callback(isolate_, context, promise_resolver_.Get(isolate_), result, WasmAsyncSuccess::kSuccess);
    }
...
public:
    ...
    void OnCompilationSucceeded(i::Handle<i::WasmModuleObject> result) override
    {
        ...
        i::wasm::GetWasmEngine()->AsyncInstantiate(reinterpret_cast<i::Isolate*>(isolate_),
            std::make_unique<InstantiateBytesResultResolver>(isolate_, context_.Get(isolate_), promise_resolver_.Get(isolate_), Utils::ToLocal(i::Handle<i::Object>::cast(result))),
...
std::string ToString(const i::Handle<i::String> name) { return std::string("Property '") + name->ToCString().get() + "'"; }

// Web IDL: '[EnforceRange] unsigned long'
// Previously called ToNonWrappingUint32 in the draft WebAssembly JS spec.
// https://heycam.github.io/webidl/#EnforceRange
template <typename T> bool EnforceUint32(T argument_name, Local<v8::Value> v, Local<Context> context, ErrorThrower* thrower, uint32_t* res)
{
...
void WebAssemblyCompile(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!i::wasm::IsWasmCodegenAllowed(i_isolate, native_context)) {
        i::Handle<i::String> error = i::wasm::ErrorStringForCodegen(i_isolate, native_context);
    ...
    auto enabled_features = i::wasm::WasmFeatures::FromIsolate(i_isolate);
    i::wasm::GetWasmEngine()->AsyncCompile(i_isolate, enabled_features, std::move(resolver), bytes, is_shared, kAPIMethodName);
}

void WasmStreamingCallbackForTesting(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
...
void WasmStreamingPromiseFailedCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    std::shared_ptr<v8::WasmStreaming> streaming = v8::WasmStreaming::Unpack(args.GetIsolate(), args.Data());
...
void WebAssemblyCompileStreaming(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
    ...
    i::Handle<i::Managed<WasmStreaming>> data
        = i::Managed<WasmStreaming>::Allocate(i_isolate, 0, std::make_unique<WasmStreaming::WasmStreamingImpl>(isolate, kAPIMethodName, resolver));

    DCHECK_NOT_NULL(i_isolate->wasm_streaming_callback());
    ASSIGN(v8::Function, compile_callback, v8::Function::New(context, i_isolate->wasm_streaming_callback(), Utils::ToLocal(i::Handle<i::Object>::cast(data)), 1));
    ASSIGN(v8::Function, reject_callback, v8::Function::New(context, WasmStreamingPromiseFailedCallback, Utils::ToLocal(i::Handle<i::Object>::cast(data)), 1));
...
void WebAssemblyValidate(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (is_shared) {
        ...
        validated = i::wasm::GetWasmEngine()->SyncValidate(i_isolate, enabled_features, bytes_copy);
    } else {
        // The wire bytes are not shared, OK to use them directly.
        validated = i::wasm::GetWasmEngine()->SyncValidate(i_isolate, enabled_features, bytes);
    }
...
void WebAssemblyModule(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (is_shared) {
        ...
        memcpy(copy.get(), bytes.start(), bytes.length());
        i::wasm::ModuleWireBytes bytes_copy(copy.get(), copy.get() + bytes.length());
        maybe_module_obj = i::wasm::GetWasmEngine()->SyncCompile(i_isolate, enabled_features, &thrower, bytes_copy);
    } else {
        // The wire bytes are not shared, OK to use them directly.
        maybe_module_obj = i::wasm::GetWasmEngine()->SyncCompile(i_isolate, enabled_features, &thrower, bytes);
    }
    ...
    if (!TransferPrototype(i_isolate, module_obj, Utils::OpenHandle(*args.This()))) {
        return;
    }
...
void WebAssemblyModuleCustomSections(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    HandleScope scope(args.GetIsolate());
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    ScheduledErrorThrower thrower(i_isolate, "WebAssembly.Module.customSections()");
    ...
    if (!maybe_name.ToHandle(&name))
        return;
    auto custom_sections = i::wasm::GetCustomSections(i_isolate, maybe_module.ToHandleChecked(), i::Handle<i::String>::cast(name), &thrower);
    if (thrower.error())
        return;
...
void WebAssemblyInstance(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    i::MaybeHandle<i::JSObject> maybe_instance_obj;
    {
        ...
        i::MaybeHandle<i::WasmModuleObject> maybe_module = GetFirstArgumentAsModule(args, &thrower);
        if (thrower.error())
            return;
        ...
        maybe_instance_obj = i::wasm::GetWasmEngine()->SyncInstantiate(i_isolate, &thrower, module_obj, ImportsAsMaybeReceiver(ffi), i::MaybeHandle<i::JSArrayBuffer>());
    }
    ...
    // point, so we must overwrite that with the correct prototype for {Foo}.
    if (!TransferPrototype(i_isolate, instance_obj, Utils::OpenHandle(*args.This()))) {
        return;
    }
...
void WebAssemblyInstantiateStreaming(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    i_isolate->CountUsage(v8::Isolate::UseCounterFeature::kWebAssemblyInstantiation);
    ...
    // embedder.
    i::Handle<i::Managed<WasmStreaming>> data
        = i::Managed<WasmStreaming>::Allocate(i_isolate, 0, std::make_unique<WasmStreaming::WasmStreamingImpl>(isolate, kAPIMethodName, compilation_resolver));
...
void WebAssemblyInstantiate(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    std::shared_ptr<i::wasm::CompilationResultResolver> compilation_resolver(new AsyncInstantiateCompileResultResolver(isolate, context, promise_resolver, ffi));
    ...
    // Asynchronous compilation handles copying wire bytes if necessary.
    auto enabled_features = i::wasm::WasmFeatures::FromIsolate(i_isolate);
    i::wasm::GetWasmEngine()->AsyncCompile(i_isolate, enabled_features, std::move(compilation_resolver), bytes, is_shared, kAPIMethodName);
}

bool GetIntegerProperty(v8::Isolate* isolate, ErrorThrower* thrower, Local<Context> context, v8::Local<v8::Value> value, i::Handle<i::String> property_name, int64_t* result,
    int64_t lower_bound, uint64_t upper_bound)
{
    ...
    if (number < lower_bound) {
        thrower->RangeError("Property '%s': value %" PRIu32 " is below the lower bound %" PRIx64, property_name->ToCString().get(), number, lower_bound);
        return false;
    }
    if (number > upper_bound) {
        thrower->RangeError("Property '%s': value %" PRIu32 " is above the upper bound %" PRIu64, property_name->ToCString().get(), number, upper_bound);
        return false;
    }
...
bool GetOptionalIntegerProperty(v8::Isolate* isolate, ErrorThrower* thrower, Local<Context> context, Local<v8::Object> object, Local<String> property, bool* has_property,
    int64_t* result, int64_t lower_bound, uint64_t upper_bound)
{
    ...
    return GetIntegerProperty(isolate, thrower, context, value, property_name, result, lower_bound, upper_bound);
}
...
bool GetInitialOrMinimumProperty(
    v8::Isolate* isolate, ErrorThrower* thrower, Local<Context> context, Local<v8::Object> object, int64_t* result, int64_t lower_bound, uint64_t upper_bound)
{
    bool has_initial = false;
    if (!GetOptionalIntegerProperty(isolate, thrower, context, object, v8_str(isolate, "initial"), &has_initial, result, lower_bound, upper_bound)) {
        return false;
    }
    auto enabled_features = i::wasm::WasmFeatures::FromIsolate(reinterpret_cast<i::Isolate*>(isolate));
    if (enabled_features.has_type_reflection()) {
        bool has_minimum = false;
        int64_t minimum = 0;
        if (!GetOptionalIntegerProperty(isolate, thrower, context, object, v8_str(isolate, "minimum"), &has_minimum, &minimum, lower_bound, upper_bound)) {
            return false;
        }
...
namespace {
i::Handle<i::Object> DefaultReferenceValue(i::Isolate* isolate, i::wasm::ValueType type)
{
...
void WebAssemblyTable(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    // The descriptor's 'element'.
    {
        ...
        } else if (enabled_features.has_type_reflection() && string->StringEquals(v8_str(isolate, "funcref"))) {
        ...
        } else if (enabled_features.has_stringref() && string->StringEquals(v8_str(isolate, "stringref"))) {
            type = i::wasm::kWasmStringRef;
        } else if (enabled_features.has_gc() && string->StringEquals(v8_str(isolate, "anyref"))) {
            type = i::wasm::kWasmAnyRef;
        } else if (enabled_features.has_gc() && string->StringEquals(v8_str(isolate, "eqref"))) {
            type = i::wasm::kWasmEqRef;
        } else if (enabled_features.has_gc() && string->StringEquals(v8_str(isolate, "dataref"))) {
            type = i::wasm::kWasmDataRef;
        } else if (enabled_features.has_gc() && string->StringEquals(v8_str(isolate, "arrayref"))) {
    ...
    if (!GetOptionalIntegerProperty(isolate, &thrower, context, descriptor, v8_str(isolate, "maximum"), &has_maximum, &maximum, initial, std::numeric_limits<uint32_t>::max())) {
        return;
    }

    i::Handle<i::FixedArray> fixed_array;
    i::Handle<i::WasmTableObject> table_obj = i::WasmTableObject::New(i_isolate, i::Handle<i::WasmInstanceObject>(), type, static_cast<uint32_t>(initial), has_maximum,
        static_cast<uint32_t>(maximum), &fixed_array, DefaultReferenceValue(i_isolate, type));
    ...
    // point, so we must overwrite that with the correct prototype for {Foo}.
    if (!TransferPrototype(i_isolate, table_obj, Utils::OpenHandle(*args.This()))) {
        return;
    }
...
void WebAssemblyMemory(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    // code once it is.
    int64_t initial = 0;
    if (!GetInitialOrMinimumProperty(isolate, &thrower, context, descriptor, &initial, 0, i::wasm::kSpecMaxMemory32Pages)) {
        return;
    }
    // The descriptor's 'maximum'.
    int64_t maximum = i::WasmMemoryObject::kNoMaximum;
    if (!GetOptionalIntegerProperty(isolate, &thrower, context, descriptor, v8_str(isolate, "maximum"), nullptr, &maximum, initial, i::wasm::kSpecMaxMemory32Pages)) {
        return;
    }
    ...
    if (enabled_features.has_threads()) {
        ...
        if (maybe_value.ToLocal(&value)) {
            shared = value->BooleanValue(isolate) ? i::SharedFlag::kShared : i::SharedFlag::kNotShared;
        } else {
            DCHECK(i_isolate->has_scheduled_exception());
            return;
        }
    ...
    if (!i::WasmMemoryObject::New(i_isolate, static_cast<int>(initial), static_cast<int>(maximum), shared).ToHandle(&memory_obj)) {
    ...
    // point, so we must overwrite that with the correct prototype for {Foo}.
    if (!TransferPrototype(i_isolate, memory_obj, Utils::OpenHandle(*args.This()))) {
        return;
    }

    if (shared == i::SharedFlag::kShared) {
        i::Handle<i::JSArrayBuffer> buffer(i::Handle<i::WasmMemoryObject>::cast(memory_obj)->array_buffer(), i_isolate);
...
bool GetValueType(Isolate* isolate, MaybeLocal<Value> maybe, Local<Context> context, i::wasm::ValueType* type, i::wasm::WasmFeatures enabled_features)
{
    v8::Local<v8::Value> value;
    if (!maybe.ToLocal(&value))
        return false;
...
void WebAssemblyGlobal(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    i::MaybeHandle<i::WasmGlobalObject> maybe_global_obj
        = i::WasmGlobalObject::New(i_isolate, i::Handle<i::WasmInstanceObject>(), i::MaybeHandle<i::JSArrayBuffer>(), i::MaybeHandle<i::FixedArray>(), type, offset, is_mutable);
    ...
    // point, so we must overwrite that with the correct prototype for {Foo}.
    if (!TransferPrototype(i_isolate, global_obj, Utils::OpenHandle(*args.This()))) {
        return;
    }
    ...
    case i::wasm::kRefNull: {
        ...
        i::Handle<i::Object> value_handle = (args.Length() < 2) ? i_isolate->factory()->null_value() : Utils::OpenHandle(*value);
        const char* error_message;
        if (!i::wasm::JSToWasmObject(i_isolate, nullptr, value_handle, type, &error_message).ToHandle(&value_handle)) {
...
void WebAssemblyTag(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    for (uint32_t i = 0; i < parameters_len; ++i) {
        ...
        if (!GetValueType(isolate, maybe, context, &type, enabled_features) || type == i::wasm::kWasmVoid) {
    ...
    i::Handle<i::JSObject> tag_object = i::WasmTagObject::New(i_isolate, &sig, tag);
...
void WebAssemblySuspender(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    // point, so we must overwrite that with the correct prototype for {Foo}.
    if (!TransferPrototype(i_isolate, suspender, Utils::OpenHandle(*args.This()))) {
        return;
    }
...
uint32_t GetEncodedSize(i::Handle<i::WasmTagObject> tag_object)
{
    ...
    i::wasm::WasmTagSig sig { 0, static_cast<size_t>(serialized_sig.length()), reinterpret_cast<i::wasm::ValueType*>(serialized_sig.GetDataStartAddress()) };
    i::wasm::WasmTag tag(&sig);
    return i::WasmExceptionPackage::GetEncodedSize(&tag);
}

void EncodeExceptionValues(
    v8::Isolate* isolate, i::Handle<i::PodArray<i::wasm::ValueType>> signature, const Local<Value>& arg, ScheduledErrorThrower* thrower, i::Handle<i::FixedArray> values_out)
{
    Local<Context> context = isolate->GetCurrentContext();
    uint32_t index = 0;
    if (!arg->IsObject()) {
...
void WebAssemblyException(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    i::Handle<i::WasmTagObject> tag_object = i::Handle<i::WasmTagObject>::cast(arg0);
    i::Handle<i::WasmExceptionTag> tag(i::WasmExceptionTag::cast(tag_object->tag()), i_isolate);
    ...
    i::Handle<i::FixedArray> values = i::Handle<i::FixedArray>::cast(i::WasmExceptionPackage::GetExceptionValues(i_isolate, runtime_exception));
    i::Handle<i::PodArray<i::wasm::ValueType>> signature(tag_object->serialized_signature(), i_isolate);
    ...
    if (args[2]->IsObject()) {
        ...
        v8::MaybeLocal<v8::Value> maybe_trace_stack = trace_stack_obj->Get(context, trace_stack_key);
        v8::Local<Value> trace_stack_value;
        if (maybe_trace_stack.ToLocal(&trace_stack_value) && trace_stack_value->BooleanValue(isolate)) {
            auto caller = Utils::OpenHandle(*args.NewTarget());
            i_isolate->CaptureAndSetErrorStack(runtime_exception, i::SKIP_NONE, caller);
            i::Handle<i::AccessorInfo> error_stack = i_isolate->factory()->error_stack_accessor();
            i::Handle<i::Name> name(i::Name::cast(error_stack->name()), i_isolate);
            i::JSObject::SetAccessor(runtime_exception, name, error_stack, i::DONT_ENUM).Assert();
        }
    }

    args.GetReturnValue().Set(Utils::ToLocal(i::Handle<i::Object>::cast(runtime_exception)));
}

namespace {
bool HasJSPromiseIntegrationFlag(Isolate* isolate, Local<Object> usage_obj, ErrorThrower* thrower, const char* flag_name)
{
    Local<Context> context = isolate->GetCurrentContext();
    ...
    if (maybe_flag.ToLocal(&flag_value) && !flag_value->IsUndefined() && flag_value->ToString(context).ToLocal(&flag_value_str)) {
        if (!flag_value_str->StringEquals(first_str) && !flag_value_str->StringEquals(last_str) && !flag_value_str->StringEquals(none_str)) {
            thrower->TypeError("JS Promise Integration: Expected suspender "
...
bool IsSuspendingSignature(const i::wasm::FunctionSig* inner_sig, const i::wasm::FunctionSig* outer_sig)
{
    ...
    if (outer_sig->GetParam(0) != i::wasm::kWasmExternRef)
        return false;
    for (size_t i = 1; i < outer_sig->parameter_count(); ++i) {
        if (outer_sig->GetParam(i) != inner_sig->GetParam(i - 1))
            return false;
    }
    for (size_t i = 0; i < outer_sig->return_count(); ++i) {
        if (outer_sig->GetReturn(i) != inner_sig->GetReturn(i))
            return false;
    }
    return true;
}
...
bool IsPromisingSignature(const i::wasm::FunctionSig* inner_sig, const i::wasm::FunctionSig* outer_sig)
{
    ...
    if (inner_sig->GetParam(0) != i::wasm::kWasmExternRef)
        return false;
    for (size_t i = 0; i < outer_sig->parameter_count(); ++i) {
        if (outer_sig->GetParam(i) != inner_sig->GetParam(i + 1))
            return false;
    }
    if (outer_sig->GetReturn(0) != i::wasm::kWasmExternRef)
        return false;
    return true;
}
...
void WebAssemblyFunction(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    v8::MaybeLocal<v8::Value> results_maybe = function_type->Get(context, results_key);
    v8::Local<v8::Value> results_value;
    if (!results_maybe.ToLocal(&results_value))
        return;
    ...
    i::Handle<i::JSReceiver> callable = Utils::OpenHandle(*args[1].As<Function>());
    ...
    if (i::v8_flags.experimental_wasm_stack_switching) {
        ...
        if (!args[2]->IsNullOrUndefined() && !args[2]->IsObject()) {
            thrower.TypeError("Expected argument 3 to be an object with a "
                              "'suspending' or 'promising' property");
            return;
        }
        if (args[2]->IsObject()) {
            Local<Object> usage_obj = Local<Object>::Cast(args[2]);
            if (HasJSPromiseIntegrationFlag(isolate, usage_obj, &thrower, "suspending")) {
                suspend = i::wasm::kSuspend;
            }
            if (HasJSPromiseIntegrationFlag(isolate, usage_obj, &thrower, "promising")) {
                promise = i::wasm::kPromise;
            }
        }
    }

    bool is_wasm_exported_function = i::WasmExportedFunction::IsWasmExportedFunction(*callable);
    ...
    if (is_wasm_exported_function && !suspend && !promise) {
        ...
        thrower.TypeError("The signature of Argument 1 (a WebAssembly function) does "
    ...
    if (is_wasm_exported_function && promise) {
        ...
        i::Handle<i::WasmInstanceObject> instance(i::WasmInstanceObject::cast(data.internal().ref()), i_isolate);
        int func_index = data.function_index();
        i::Handle<i::CodeT> wrapper = BUILTIN_CODE(i_isolate, WasmReturnPromiseOnSuspend);
        i::Handle<i::JSFunction> result = i::WasmExportedFunction::New(i_isolate, instance, func_index, static_cast<int>(data.sig()->parameter_count()), wrapper);
    ...
    if (is_wasm_js_function && suspend) {
        ...
        const i::wasm::FunctionSig* inner_sig = wasm_js_function.GetSignature(&zone);
...
void WebAssemblyFunctionType(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (i::WasmExportedFunction::IsWasmExportedFunction(*arg0)) {
        auto wasm_exported_function = i::Handle<i::WasmExportedFunction>::cast(arg0);
        auto sfi = handle(wasm_exported_function->shared(), i_isolate);
        i::Handle<i::WasmExportedFunctionData> data = handle(sfi->wasm_exported_function_data(), i_isolate);
...
void WebAssemblyInstanceGetExports(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
...
void WebAssemblyTableGetLength(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
    ...
    args.GetReturnValue().Set(v8::Number::New(isolate, receiver->current_length()));
}
...
void WebAssemblyTableGrow(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    int old_size = i::WasmTableObject::Grow(i_isolate, receiver, grow_by, init_value);
    if (old_size < 0) {
...
namespace {
void WasmObjectToJSReturnValue(v8::ReturnValue<v8::Value>& return_value, i::Handle<i::Object> value, i::wasm::HeapType::Representation repr, const i::wasm::WasmModule* module,
    i::Isolate* isolate, ScheduledErrorThrower* thrower)
{
    switch (repr) {
    case i::wasm::HeapType::kExtern:
    case i::wasm::HeapType::kString:
    ...
    case i::wasm::HeapType::kFunc: {
        if (!value->IsNull()) {
            ...
            value = handle(i::Handle<i::WasmInternalFunction>::cast(value)->external(), isolate);
        }
        return_value.Set(Utils::ToLocal(value));
        return;
    }
    ...
    case i::wasm::HeapType::kAny: {
        if (!i::v8_flags.wasm_gc_js_interop && value->IsWasmObject()) {
            ...
            i::Handle<i::JSObject> wrapper = isolate->factory()->NewJSObject(isolate->object_function());
            i::JSObject::AddProperty(isolate, wrapper, isolate->factory()->wasm_wrapped_object_symbol(), value, i::NONE);
            value = wrapper;
        }
        return_value.Set(Utils::ToLocal(value));
        return;
...
void WebAssemblyTableGet(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    v8::ReturnValue<v8::Value> return_value = args.GetReturnValue();
    const i::wasm::WasmModule* module = receiver->instance().IsWasmInstanceObject() ? i::WasmInstanceObject::cast(receiver->instance()).module() : nullptr;
    WasmObjectToJSReturnValue(return_value, result, receiver->type().heap_representation(), module, i_isolate, &thrower);
}
...
void WebAssemblyTableType(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    auto type = i::wasm::GetTypeForTable(i_isolate, table->type(), table->current_length(), max_size);
    args.GetReturnValue().Set(Utils::ToLocal(type));
}
...
void WebAssemblyMemoryGetBuffer(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
    ...
    i::Handle<i::JSArrayBuffer> buffer(i::JSArrayBuffer::cast(*buffer_obj), i_isolate);
    if (buffer->is_shared()) {
...
void WebAssemblyExceptionGetArg(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    HandleScope scope(isolate);
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    ...
    i::MaybeHandle<i::WasmTagObject> maybe_tag = GetFirstArgumentAsTag(args, &thrower);
    if (thrower.error())
        return;
...
void WebAssemblyGlobalGetValueCommon(const v8::FunctionCallbackInfo<v8::Value>& args, const char* name)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
    ...
    case i::wasm::kRefNull: {
        const i::wasm::WasmModule* module = receiver->instance().IsWasmInstanceObject() ? i::WasmInstanceObject::cast(receiver->instance()).module() : nullptr;
        WasmObjectToJSReturnValue(return_value, receiver->GetRef(), receiver->type().heap_representation(), module, i_isolate, &thrower);
        break;
    }
    case i::wasm::kRtt:
    case i::wasm::kI8:
...
void WebAssemblyGlobalValueOf(const v8::FunctionCallbackInfo<v8::Value>& args) { return WebAssemblyGlobalGetValueCommon(args, "WebAssembly.Global.valueOf()"); }

// get WebAssembly.Global.value -> num
void WebAssemblyGlobalGetValue(const v8::FunctionCallbackInfo<v8::Value>& args) { return WebAssemblyGlobalGetValueCommon(args, "get WebAssembly.Global.value"); }

// set WebAssembly.Global.value(num)
void WebAssemblyGlobalSetValue(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    v8::Isolate* isolate = args.GetIsolate();
    i::Isolate* i_isolate = reinterpret_cast<i::Isolate*>(isolate);
    HandleScope scope(isolate);
...
Handle<JSFunction> CreateFunc(Isolate* isolate, Handle<String> name, FunctionCallback func, bool has_prototype, SideEffectType side_effect_type = SideEffectType::kHasSideEffect)
{
...
Handle<String> GetterName(Isolate* isolate, Handle<String> name) { return Name::ToFunctionName(isolate, name, isolate->factory()->get_string()).ToHandleChecked(); }
...
Handle<String> SetterName(Isolate* isolate, Handle<String> name) { return Name::ToFunctionName(isolate, name, isolate->factory()->set_string()).ToHandleChecked(); }
...
void SetDummyInstanceTemplate(Isolate* isolate, Handle<JSFunction> fun)
{
    ...
    FunctionTemplateInfo::SetInstanceTemplate(isolate, handle(fun->shared().get_api_func_data(), isolate), instance_template);
}

Handle<JSObject> SetupConstructor(Isolate* isolate, Handle<JSFunction> constructor, InstanceType instance_type, int instance_size, const char* name = nullptr)
{
    SetDummyInstanceTemplate(isolate, constructor);
    JSFunction::EnsureHasInitialMap(constructor);
    Handle<JSObject> proto(JSObject::cast(constructor->instance_prototype()), isolate);
...
void WasmJs::Install(Isolate* isolate, bool exposed_on_global_object)
{
    ...
    JSObject::AddProperty(isolate, webassembly, factory->to_string_tag_symbol(), name, ro_attributes);
    ...
    if (v8_flags.experimental_wasm_gc) {
        SimpleInstallFunction(isolate, webassembly, "experimentalConvertArrayToString", Builtin::kExperimentalWasmConvertArrayToString, 0, true);
        SimpleInstallFunction(isolate, webassembly, "experimentalConvertStringToArray", Builtin::kExperimentalWasmConvertStringToArray, 0, true);
    }
    ...
    SetupConstructor(isolate, module_constructor, i::WASM_MODULE_OBJECT_TYPE, WasmModuleObject::kHeaderSize, "WebAssembly.Module");
    context->set_wasm_module_constructor(*module_constructor);
    InstallFunc(isolate, module_constructor, "imports", WebAssemblyModuleImports, 1, false, NONE, SideEffectType::kHasNoSideEffect);
    InstallFunc(isolate, module_constructor, "exports", WebAssemblyModuleExports, 1, false, NONE, SideEffectType::kHasNoSideEffect);
    InstallFunc(isolate, module_constructor, "customSections", WebAssemblyModuleCustomSections, 2, false, NONE, SideEffectType::kHasNoSideEffect);

    // Setup Instance
    Handle<JSFunction> instance_constructor = InstallConstructorFunc(isolate, webassembly, "Instance", WebAssemblyInstance);
    Handle<JSObject> instance_proto = SetupConstructor(isolate, instance_constructor, i::WASM_INSTANCE_OBJECT_TYPE, WasmInstanceObject::kHeaderSize, "WebAssembly.Instance");
    context->set_wasm_instance_constructor(*instance_constructor);
    InstallGetter(isolate, instance_proto, "exports", WebAssemblyInstanceGetExports);
    ...
    Handle<JSObject> table_proto = SetupConstructor(isolate, table_constructor, i::WASM_TABLE_OBJECT_TYPE, WasmTableObject::kHeaderSize, "WebAssembly.Table");
    context->set_wasm_table_constructor(*table_constructor);
    InstallGetter(isolate, table_proto, "length", WebAssemblyTableGetLength);
    InstallFunc(isolate, table_proto, "grow", WebAssemblyTableGrow, 1);
    InstallFunc(isolate, table_proto, "set", WebAssemblyTableSet, 1);
    InstallFunc(isolate, table_proto, "get", WebAssemblyTableGet, 1, false, NONE, SideEffectType::kHasNoSideEffect);
    if (enabled_features.has_type_reflection()) {
        InstallFunc(isolate, table_proto, "type", WebAssemblyTableType, 0, false, NONE, SideEffectType::kHasNoSideEffect);
    }

    // Setup Memory
    Handle<JSFunction> memory_constructor = InstallConstructorFunc(isolate, webassembly, "Memory", WebAssemblyMemory);
    Handle<JSObject> memory_proto = SetupConstructor(isolate, memory_constructor, i::WASM_MEMORY_OBJECT_TYPE, WasmMemoryObject::kHeaderSize, "WebAssembly.Memory");
    context->set_wasm_memory_constructor(*memory_constructor);
    InstallFunc(isolate, memory_proto, "grow", WebAssemblyMemoryGrow, 1);
    InstallGetter(isolate, memory_proto, "buffer", WebAssemblyMemoryGetBuffer);
    if (enabled_features.has_type_reflection()) {
        InstallFunc(isolate, memory_proto, "type", WebAssemblyMemoryType, 0, false, NONE, SideEffectType::kHasNoSideEffect);
    }

    // Setup Global
    Handle<JSFunction> global_constructor = InstallConstructorFunc(isolate, webassembly, "Global", WebAssemblyGlobal);
    Handle<JSObject> global_proto = SetupConstructor(isolate, global_constructor, i::WASM_GLOBAL_OBJECT_TYPE, WasmGlobalObject::kHeaderSize, "WebAssembly.Global");
    context->set_wasm_global_constructor(*global_constructor);
    InstallFunc(isolate, global_proto, "valueOf", WebAssemblyGlobalValueOf, 0, false, NONE, SideEffectType::kHasNoSideEffect);
    InstallGetterSetter(isolate, global_proto, "value", WebAssemblyGlobalGetValue, WebAssemblyGlobalSetValue);
    if (enabled_features.has_type_reflection()) {
        InstallFunc(isolate, global_proto, "type", WebAssemblyGlobalType, 0, false, NONE, SideEffectType::kHasNoSideEffect);
    }

    // Setup Exception
    if (enabled_features.has_eh()) {
        Handle<JSFunction> tag_constructor = InstallConstructorFunc(isolate, webassembly, "Tag", WebAssemblyTag);
        Handle<JSObject> tag_proto = SetupConstructor(isolate, tag_constructor, i::WASM_TAG_OBJECT_TYPE, WasmTagObject::kHeaderSize, "WebAssembly.Tag");
        ...
        Handle<JSFunction> exception_constructor = InstallConstructorFunc(isolate, webassembly, "Exception", WebAssemblyException);
        SetDummyInstanceTemplate(isolate, exception_constructor);
        Handle<JSObject> exception_proto
            = SetupConstructor(isolate, exception_constructor, i::WASM_EXCEPTION_PACKAGE_TYPE, WasmExceptionPackage::kHeaderSize, "WebAssembly.Exception");
        InstallFunc(isolate, exception_proto, "getArg", WebAssemblyExceptionGetArg, 2);
    ...
    if (enabled_features.has_stack_switching()) {
        Handle<JSFunction> suspender_constructor = InstallConstructorFunc(isolate, webassembly, "Suspender", WebAssemblySuspender);
        context->set_wasm_suspender_constructor(*suspender_constructor);
        SetupConstructor(isolate, suspender_constructor, i::WASM_SUSPENDER_OBJECT_TYPE, WasmSuspenderObject::kHeaderSize, "WebAssembly.Suspender");
    }
    ...
    if (enabled_features.has_type_reflection()) {
        ...
        Handle<JSObject> function_proto(JSObject::cast(function_constructor->instance_prototype()), isolate);
        Handle<Map> function_map = isolate->factory()->CreateSloppyFunctionMap(FUNCTION_WITHOUT_PROTOTYPE, MaybeHandle<JSFunction>());
        ...
        InstallFunc(isolate, function_constructor, "type", WebAssemblyFunctionType, 1);
    ...
    // Setup errors
    Handle<JSFunction> compile_error(isolate->native_context()->wasm_compile_error_function(), isolate);
    JSObject::AddProperty(isolate, webassembly, isolate->factory()->CompileError_string(), compile_error, DONT_ENUM);
    Handle<JSFunction> link_error(isolate->native_context()->wasm_link_error_function(), isolate);
    JSObject::AddProperty(isolate, webassembly, isolate->factory()->LinkError_string(), link_error, DONT_ENUM);
    Handle<JSFunction> runtime_error(isolate->native_context()->wasm_runtime_error_function(), isolate);
    JSObject::AddProperty(isolate, webassembly, isolate->factory()->RuntimeError_string(), runtime_error, DONT_ENUM);
}

// static
void WasmJs::InstallConditionalFeatures(Isolate* isolate, Handle<Context> context)
{
    ...
    if (enabled_features.has_eh()) {
        ...
        MaybeHandle<Object> maybe_webassembly = JSObject::GetProperty(isolate, global, "WebAssembly");
        Handle<Object> webassembly_obj;
        if (!maybe_webassembly.ToHandle(&webassembly_obj) || !webassembly_obj->IsJSObject()) {
        ...
        //    has been completed.
        if (JSObject::HasOwnProperty(isolate, webassembly, tag_name).FromMaybe(true)) {
            // Existing property, or exception.
            return;
        }

        bool has_prototype = true;
        Handle<JSFunction> tag_constructor = CreateFunc(isolate, tag_name, WebAssemblyTag, has_prototype, SideEffectType::kHasNoSideEffect);
        ...
        Maybe<bool> result = JSObject::DefineOwnPropertyIgnoreAttributes(&it, tag_constructor, DONT_ENUM, Just(kDontThrow));
