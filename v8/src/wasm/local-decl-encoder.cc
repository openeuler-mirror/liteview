...
void LocalDeclEncoder::Prepend(Zone* zone, const byte** start, const byte** end) const
{
...
size_t LocalDeclEncoder::Size() const
{
    ...
    for (auto p : local_decls) {
        size += LEBHelper::sizeof_u32v(p.first) + // number of locals
            1 + // Opcode
            (p.second.encoding_needs_heap_type() ? LEBHelper::sizeof_i32v(p.second.heap_type().code()) : 0)
