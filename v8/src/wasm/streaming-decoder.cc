...
private:
    ...
    public:
        ...
        SectionCode section_code() const { return static_cast<SectionCode>(bytes_.start()[0]); }
    ...
    std::unique_ptr<DecodingState> Error(std::string message) { return Error(WasmError { module_offset_ - 1, std::move(message) }); }
...
{
    if (deserializing()) {
        wire_bytes_for_deserializing_.insert(wire_bytes_for_deserializing_.end(), bytes.begin(), bytes.end());
        return;
    }
...
void AsyncStreamingDecoder::Finish(bool can_use_compiled_module)
{
    ...
    if (deserializing()) {
        base::Vector<const uint8_t> wire_bytes = base::VectorOf(wire_bytes_for_deserializing_);
        // Try to deserialize the module from wire bytes and module bytes.
        if (can_use_compiled_module && processor_->Deserialize(compiled_module_bytes_, wire_bytes))
            return;
    ...
    base::OwnedVector<uint8_t> bytes = base::OwnedVector<uint8_t>::NewForOverwrite(total_size_);
...
public:
    ...
    ReleaseAfterFinalEvent release_after_final_event() override { return kKeepAfterFinalEvent; }
...
public:
    ...
    base::Vector<uint8_t> buffer() override { return base::ArrayVector(byte_buffer_); }
...
public:
    explicit DecodeFunctionBody(SectionBuffer* section_buffer, size_t buffer_offset, size_t function_body_length, size_t num_remaining_functions, uint32_t module_offset)
        : section_buffer_(section_buffer)
        , buffer_offset_(buffer_offset)
...
std::unique_ptr<AsyncStreamingDecoder::DecodingState> AsyncStreamingDecoder::DecodeVarInt32::Next(AsyncStreamingDecoder* streaming)
{
    ...
    if (value_ > max_value_) {
        ...
        oss << "The value " << value_ << " for " << field_name_ << " exceeds the maximum allowed value of " << max_value_;
...
std::unique_ptr<AsyncStreamingDecoder::DecodingState> AsyncStreamingDecoder::DecodeFunctionLength::NextWithValue(AsyncStreamingDecoder* streaming)
{
    ...
    return std::make_unique<DecodeFunctionBody>(section_buffer_, buffer_offset_ + bytes_consumed_, value_, num_remaining_functions_, streaming->module_offset());
}
...
AsyncStreamingDecoder::SectionBuffer* AsyncStreamingDecoder::CreateNewBuffer(uint32_t module_offset, uint8_t section_id, size_t length, base::Vector<const uint8_t> length_bytes)
{
