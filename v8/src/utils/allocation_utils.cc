...
public:
    ...
    void SetPageAllocatorForTesting(PageAllocator* allocator) { page_allocator_ = allocator; }
...
size_t AllocatePageSize() { return GetPlatformPageAllocator()->AllocatePageSize(); }

size_t CommitPageSize() { return GetPlatformPageAllocator()->CommitPageSize(); }

void* GetRandomMmapAddr() { return GetPlatformPageAllocator()->GetRandomMmapAddr(); }
...
void OnCriticalMemoryPressure() { V8::GetCurrentPlatform()->OnCriticalMemoryPressure(); }

VirtualMemory::VirtualMemory() = default;

VirtualMemory::VirtualMemory(v8::PageAllocator* page_allocator, size_t size, void* hint, size_t alignment, JitPermission jit)
    : page_allocator_(page_allocator)
{
    ...
    PageAllocator::Permission permissions = jit == JitPermission::kMapAsJittable ? PageAllocator::kNoAccessWillJitLater : PageAllocator::kNoAccess;
...
VirtualMemoryCage::VirtualMemoryCage(VirtualMemoryCage&& other) V8_NOEXCEPT { *this = std::move(other); }
...
namespace {
inline Address VirtualMemoryCageStart(Address reservation_start, const VirtualMemoryCage::ReservationParams& params)
{
    return RoundUp(reservation_start + params.base_bias_size, params.base_alignment) - params.base_bias_size;
}
} // namespace
...
bool VirtualMemoryCage::InitReservation(const ReservationParams& params, base::AddressRegion existing_reservation)
{
    ...
    CHECK(params.base_alignment == ReservationParams::kAnyBaseAlignment
        || (IsAligned(params.base_alignment, allocate_page_size) && IsAligned(params.base_bias_size, allocate_page_size)));
    ...
    if (!existing_reservation.is_empty()) {
    ...
    } else if (params.base_alignment == ReservationParams::kAnyBaseAlignment || params.base_bias_size == 0) {
        ...
        // due to OOM.
        Address hint = RoundDown(params.requested_start_hint, RoundUp(params.base_alignment, allocate_page_size));
        VirtualMemory reservation(params.page_allocator, params.reservation_size, reinterpret_cast<void*>(hint), params.base_alignment, params.jit);
        ...
        Address hint = RoundDown(params.requested_start_hint + bias_size, RoundUp(params.base_alignment, allocate_page_size)) - bias_size;
        ...
        for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
            ...
            VirtualMemory padded_reservation(params.page_allocator, params.reservation_size * 2, reinterpret_cast<void*>(hint), 1, params.jit);
            if (!padded_reservation.IsReserved())
                return false;

            // Find properly aligned sub-region inside the reservation.
            Address address = VirtualMemoryCageStart(padded_reservation.address(), params);
            ...
            if (overreserve) {
                ...
                VirtualMemory reservation(params.page_allocator, params.reservation_size, reinterpret_cast<void*>(address), 1, params.jit);
                if (!reservation.IsReserved())
                    return false;

                // The reservation could still be somewhere else but we can accept it
                // if it has the required alignment.
                Address start_address = VirtualMemoryCageStart(reservation.address(), params);
    ...
    const size_t allocatable_size = RoundDown(params.reservation_size - (allocatable_base - base_) - params.base_bias_size, params.page_size);
    size_ = allocatable_base + allocatable_size - base_;

    const base::PageFreeingMode page_freeing_mode = V8_HEAP_USE_PTHREAD_JIT_WRITE_PROTECT && params.jit == JitPermission::kMapAsJittable
    ...
    page_allocator_ = std::make_unique<base::BoundedPageAllocator>(
        params.page_allocator, allocatable_base, allocatable_size, params.page_size, base::PageInitializationMode::kAllocatedPagesCanBeUninitialized, page_freeing_mode);
    return true;
}
