...
template <typename T> void DeleteArray(T* array) { delete[] array; }
...
public:
    template <typename T, typename TypeTag = T[]> V8_INLINE T* NewArray(size_t length) { return static_cast<T*>(Malloced::operator new(length * sizeof(T))); }
    template <typename T, typename TypeTag = T[]> V8_INLINE void DeleteArray(T* p, size_t length) { Malloced::operator delete(p); }
};
...
public:
    ...
    V8_EXPORT_PRIVATE VirtualMemory(v8::PageAllocator* page_allocator, size_t size, void* hint, size_t alignment = 1, JitPermission jit = JitPermission::kNoJit);
    ...
    V8_EXPORT_PRIVATE bool SetPermissions(Address address, size_t size, PageAllocator::Permission access);
    ...
    V8_EXPORT_PRIVATE bool RecommitPages(Address address, size_t size, PageAllocator::Permission access);
    ...
    bool InVM(Address address, size_t size) const { return region_.contains(address, size); }
...
public:
    ...
    base::BoundedPageAllocator* page_allocator() const { return page_allocator_.get(); }
