...
public:
    ...
    IdentityMapFindResult<V> FindOrInsert(Handle<Object> key) { return FindOrInsert(*key); }
    ...
    V* Find(Object key) const { return reinterpret_cast<V*>(FindEntry(key.ptr())); }

    // Insert the value for the given key. The key must not have previously
    // existed.
    void Insert(Handle<Object> key, V v) { Insert(*key, v); }
    void Insert(Object key, V v) { *reinterpret_cast<V*>(InsertEntry(key.ptr())) = v; }

    bool Delete(Handle<Object> key, V* deleted_value) { return Delete(*key, deleted_value); }
    ...
    public:
        ...
        V* entry() const { return reinterpret_cast<V*>(map_->EntryAtIndex(index_)); }
...
protected:
    ...
    uintptr_t* NewPointerArray(size_t length) override { return allocator_.template NewArray<uintptr_t, Buffer>(length); }
    void DeletePointerArray(uintptr_t* array, size_t length) override { allocator_.template DeleteArray<uintptr_t, Buffer>(array, length); }
