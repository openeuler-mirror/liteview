...
public:
    ...
    public:
        ...
        bool operator==(const Iterator& other) const { return segment_ == other.segment_ && bit_in_segment_ == other.bit_in_segment_; }
    ...
    void Union(const SparseBitVector& other)
    {
        ...
        for (const Segment* other_segment = &other.first_segment_; other_segment; other_segment = other_segment->next) {
            ...
            if (segment && segment->offset == other_segment->offset) {
                std::transform(std::begin(segment->words), std::end(segment->words), std::begin(other_segment->words), std::begin(segment->words), std::bit_or<uintptr_t> {});
            } else {
