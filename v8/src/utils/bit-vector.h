...
public:
    ...
    bool IsEmpty() const { return std::all_of(data_begin_, data_end_, std::logical_not<uintptr_t> {}); }

    bool Equals(const BitVector& other) const { return std::equal(data_begin_, data_end_, other.data_begin_); }
...
private:
    ...
    V8_INLINE static uintptr_t bit(int index) { return uintptr_t { 1 } << (index & (kDataBits - 1)); }
...
public:
    ...
    bool Equals(const GrowableBitVector& other) const { return length() == other.length() && bits_.Equals(other.bits_); }
