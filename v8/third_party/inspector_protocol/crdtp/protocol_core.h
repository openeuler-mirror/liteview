...
public:
    ...
    template <typename T> void AddField(span<char> name, const T& field) { serializer_.AddField(name, field); }
...
template <typename T> struct ProtocolTypeTraits<std::unique_ptr<std::vector<T>>> {
    ...
    static void Serialize(const std::unique_ptr<std::vector<T>>& value, std::vector<uint8_t>* bytes) { ProtocolTypeTraits<std::vector<T>>::Serialize(*value, bytes); }
};
...
template <typename T> struct ProtocolTypeTraits<detail::ValueMaybe<T>> {
    ...
    static void Serialize(const detail::ValueMaybe<T>& value, std::vector<uint8_t>* bytes) { ProtocolTypeTraits<T>::Serialize(value.fromJust(), bytes); }
};
...
template <typename T> struct ProtocolTypeTraits<detail::PtrMaybe<T>> {
    ...
    static void Serialize(const detail::PtrMaybe<T>& value, std::vector<uint8_t>* bytes) { ProtocolTypeTraits<T>::Serialize(*value.fromJust(), bytes); }
};
...
public:
    ...
    [[nodiscard]] static bool Deserialize(DeserializerState* state, T* value) { return T::deserializer_descriptor().Deserialize(state, value); }
...
template <typename T> struct ProtocolTypeTraits<T, typename std::enable_if<std::is_base_of<ProtocolObject<T>, T>::value>::type> {
    static bool Deserialize(DeserializerState* state, T* value) { return T::Deserialize(state, value); }

    static void Serialize(const T& value, std::vector<uint8_t>* bytes) { value.AppendSerialized(bytes); }
};
...
template <typename T> struct ProtocolTypeTraits<std::unique_ptr<T>, typename std::enable_if<std::is_base_of<ProtocolObject<T>, T>::value>::type> {
    ...
    static void Serialize(const std::unique_ptr<T>& value, std::vector<uint8_t>* bytes) { ProtocolTypeTraits<T>::Serialize(*value, bytes); }
};
...
#define V8_CRDTP_DESERIALIZE_FILED_IMPL(name, field, is_optional)                                                                                                                  \
    {                                                                                                                                                                              \
        MakeSpan(name), is_optional,                                                                                                                                               \
            [](DeserializerState* __state, void* __obj) -> bool { return ProtocolTypeTraits<decltype(field)>::Deserialize(__state, &static_cast<ProtocolType*>(__obj)->field); }   \
    }
