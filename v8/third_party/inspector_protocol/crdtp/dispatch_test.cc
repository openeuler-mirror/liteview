...
TEST(DispatchableTest, DuplicateMapKey)
{
    for (const std::string& json :
        { "{\"id\":42,\"id\":42}", "{\"params\":null,\"params\":null}", "{\"method\":\"foo\",\"method\":\"foo\"}", "{\"sessionId\":\"42\",\"sessionId\":\"42\"}" }) {
...
private:
    void SendProtocolResponse(int call_id, std::unique_ptr<Serializable> message) override { cbor_ = message->Serialize(); }

    void SendProtocolNotification(std::unique_ptr<Serializable> message) override { cbor_ = message->Serialize(); }
...
public:
    ...
    std::vector<std::string> DispatchedCommands() const { return dispatched_commands_; }
