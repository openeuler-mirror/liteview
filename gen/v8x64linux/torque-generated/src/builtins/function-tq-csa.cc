...
TF_BUILTIN(FastFunctionPrototypeBind, CodeStubAssembler)
{
    ...
    if (block10.is_used()) {
        ...
        CheckAccessor_0(state_, TNode<Context> { parameter0 }, TNode<DescriptorArray> { tmp15 },
            JSFunctionOrBoundFunctionOrWrappedFunction::kLengthDescriptorIndex, TNode<Name> { tmp16 }, &label17);
        ca_.Goto(&block11);
        if (label17.is_used()) {
            ca_.Bind(&label17);
            ca_.Goto(&block12);
    ...
    if (block11.is_used()) {
        ...
        CheckAccessor_0(state_, TNode<Context> { parameter0 }, TNode<DescriptorArray> { tmp15 },
            JSFunctionOrBoundFunctionOrWrappedFunction::kNameDescriptorIndex, TNode<Name> { tmp18 }, &label19);
        ca_.Goto(&block13);
        if (label19.is_used()) {
            ca_.Bind(&label19);
            ca_.Goto(&block14);
    ...
    if (block23.is_used()) {
        ...
        tmp44 = CodeStubAssembler(state_).GetArgumentValue(
