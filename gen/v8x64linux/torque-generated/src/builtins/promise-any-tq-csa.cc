    ...
    TNode<Smi> p_index, TNode<NativeContext> p_nativeContext)
{
    ...
    if (block0.is_used()) {
        ...
        tmp5 = CodeStubAssembler(state_).AllocateFunctionWithMapAndContext(
            TNode<Map> { tmp3 }, TNode<SharedFunctionInfo> { tmp4 }, TNode<Context> { p_rejectElementContext });
...
TF_BUILTIN(PromiseAnyRejectElementClosure, CodeStubAssembler)
{
    ...
    if (block13.is_used()) {
        ...
        std::tie(tmp10, tmp11)
            = ContextSlot_PromiseAnyRejectElementContext_PromiseAnyRejectElementContext_FixedArray_0(state_, TNode<Context> { tmp2 }, TNode<IntPtrT> { tmp9 })
                  .Flatten();
        ...
        std::tie(tmp14, tmp15)
            = ContextSlot_PromiseAnyRejectElementContext_PromiseAnyRejectElementContext_Smi_0(state_, TNode<Context> { tmp2 }, TNode<IntPtrT> { tmp13 })
                  .Flatten();
    ...
    if (block19.is_used()) {
        ...
        std::tie(tmp29, tmp30)
            = ContextSlot_PromiseAnyRejectElementContext_PromiseAnyRejectElementContext_FixedArray_0(state_, TNode<Context> { tmp2 }, TNode<IntPtrT> { tmp28 })
                  .Flatten();
    ...
    if (block29.is_used()) {
        ...
        std::tie(tmp50, tmp51) = ContextSlot_PromiseAnyRejectElementContext_PromiseAnyRejectElementContext_PromiseCapability_0(
            state_, TNode<Context> { tmp2 }, TNode<IntPtrT> { tmp49 })
                                     .Flatten();
