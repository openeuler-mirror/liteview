...
TF_BUILTIN(ArrayFindIndexLoopEagerDeoptContinuation, CodeStubAssembler)
{
    ...
    if (block15.is_used()) {
        ca_.Bind(&block15);
        tmp8 = ca_.CallStub<Number>(
            Builtins::CallableFor(ca_.isolate(), Builtin::kArrayFindIndexLoopContinuation), parameter0, tmp0, tmp2, parameter3, tmp0, tmp4, tmp6);
...
TF_BUILTIN(ArrayPrototypeFindIndex, CodeStubAssembler)
{
    ...
    if (block10.is_used()) {
        ca_.Bind(&block10);
        tmp14 = ca_.CallStub<Number>(
            Builtins::CallableFor(ca_.isolate(), Builtin::kArrayFindIndexLoopContinuation), parameter0, tmp1, tmp7, tmp10, tmp1, tmp13.value(), tmp2);
