...
TF_BUILTIN(ProxyGetProperty, CodeStubAssembler)
{
    ...
    if (block24.is_used()) {
        ...
        tmp10 = ca_.CallStub<Object>(
            Builtins::CallableFor(ca_.isolate(), Builtin::kGetPropertyWithReceiver), parameter0, tmp6, parameter2, parameter3, parameter4);
    ...
    if (block23.is_used()) {
        ca_.Bind(&block23);
        tmp11 = CodeStubAssembler(state_).Call(TNode<Context> { parameter0 }, TNode<Object> { tmp8 }, TNode<Object> { ca_.UncheckedCast<JSReceiver>(tmp1) },
        ...
        ProxiesCodeStubAssembler(state_).CheckGetSetTrapResult(TNode<Context> { parameter0 }, TNode<JSReceiver> { tmp6 }, TNode<JSProxy> { parameter1 },
            TNode<Name> { parameter2 }, TNode<Object> { tmp11 }, JSProxy::AccessKind::kGet);
