...
(function() {
  ...
  AudioProcessingSettings.prototype.initDefaults_ = function() {
    ...
    this.experimentalAutomaticGainControl = false;
  ...
  AudioProcessingSettings.decode = function(decoder) {
    ...
    val.experimentalAutomaticGainControl = (packed >> 4) & 1 ? true : false;
    val.highPassFilter = (packed >> 5) & 1 ? true : false;
    val.multiChannelCaptureProcessing = (packed >> 6) & 1 ? true : false;
    val.stereoMirroring = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.forceApmCreation = (packed >> 0) & 1 ? true : false;
    decoder.skip(1);
    decoder.skip(1);
    decoder.skip(1);
    decoder.skip(1);
  ...
  AudioProcessingSettings.encode = function(encoder, val) {
    ...
    packed |= (val.experimentalAutomaticGainControl & 1) << 4
    packed |= (val.highPassFilter & 1) << 5
    packed |= (val.multiChannelCaptureProcessing & 1) << 6
    packed |= (val.stereoMirroring & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.forceApmCreation & 1) << 0
    encoder.writeUint8(packed);
    encoder.skip(1);
    encoder.skip(1);
    encoder.skip(1);
  ...
  var kAudioProcessorControls_GetStats_Name = 1715307670;
  var kAudioProcessorControls_SetPreferredNumCaptureChannels_Name = 1278966770;
