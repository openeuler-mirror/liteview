// third_party/blink/public/mojom/frame/lifecycle.mojom-shared.cc is auto generated by mojom_bindings_generator.py, do not edit

// Copyright 2016 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "third_party/blink/public/mojom/frame/lifecycle.mojom-shared.h"

// Used to support stream output operator for enums.
// TODO(dcheng): Consider omitting this somehow if not needed.
#include <ostream>
#include <utility>

#include "base/strings/stringprintf.h"
#include "mojo/public/cpp/bindings/lib/validate_params.h"
#include "mojo/public/cpp/bindings/lib/validation_errors.h"
#include "mojo/public/cpp/bindings/lib/validation_util.h"
#include "third_party/perfetto/include/perfetto/tracing/traced_value.h"

#include "third_party/blink/public/mojom/frame/lifecycle.mojom-params-data.h"
namespace blink {
namespace mojom {

static NOINLINE const char* FrameVisibilityToStringHelper(FrameVisibility value)
{
    // Defined in a helper function to ensure that Clang generates a lookup table.
    switch (value) {
    case FrameVisibility::kRenderedOutOfViewport:
        return "kRenderedOutOfViewport";
    case FrameVisibility::kRenderedInViewport:
        return "kRenderedInViewport";
    case FrameVisibility::kNotRendered:
        return "kNotRendered";
    default:
        return nullptr;
    }
}

std::string FrameVisibilityToString(FrameVisibility value)
{
    const char* str = FrameVisibilityToStringHelper(value);
    if (!str) {
        return base::StringPrintf("Unknown FrameVisibility value: %i", static_cast<int32_t>(value));
    }
    return str;
}

std::ostream& operator<<(std::ostream& os, FrameVisibility value) { return os << FrameVisibilityToString(value); }

static NOINLINE const char* FrameLifecycleStateToStringHelper(FrameLifecycleState value)
{
    // Defined in a helper function to ensure that Clang generates a lookup table.
    switch (value) {
    case FrameLifecycleState::kRunning:
        return "kRunning";
    case FrameLifecycleState::kPaused:
        return "kPaused";
    case FrameLifecycleState::kFrozen:
        return "kFrozen";
    case FrameLifecycleState::kFrozenAutoResumeMedia:
        return "kFrozenAutoResumeMedia";
    default:
        return nullptr;
    }
}

std::string FrameLifecycleStateToString(FrameLifecycleState value)
{
    const char* str = FrameLifecycleStateToStringHelper(value);
    if (!str) {
        return base::StringPrintf("Unknown FrameLifecycleState value: %i", static_cast<int32_t>(value));
    }
    return str;
}

std::ostream& operator<<(std::ostream& os, FrameLifecycleState value) { return os << FrameLifecycleStateToString(value); }

namespace internal {

} // namespace internal
} // namespace mojom
} // namespace blink

namespace perfetto {

// static
void TraceFormatTraits<::blink::mojom::FrameVisibility>::WriteIntoTrace(perfetto::TracedValue context, ::blink::mojom::FrameVisibility value)
{
    return std::move(context).WriteString(::blink::mojom::FrameVisibilityToString(value));
}

} // namespace perfetto

namespace perfetto {

// static
void TraceFormatTraits<::blink::mojom::FrameLifecycleState>::WriteIntoTrace(perfetto::TracedValue context, ::blink::mojom::FrameLifecycleState value)
{
    return std::move(context).WriteString(::blink::mojom::FrameLifecycleStateToString(value));
}

} // namespace perfetto
