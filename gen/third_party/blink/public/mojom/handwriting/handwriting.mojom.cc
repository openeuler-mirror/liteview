...
HandwritingHints::HandwritingHints(
    const std::string& recognition_type_in, const std::string& input_type_in, const absl::optional<std::string>& text_context_in, uint32_t alternatives_in)
...
private:
    ...
    void Run(absl::optional<std::vector<HandwritingPredictionPtr>> in_prediction);
};
...
void HandwritingRecognizer_GetPrediction_ProxyToResponder::Run(absl::optional<std::vector<HandwritingPredictionPtr>> in_prediction)
{
#if BUILDFLAG(MOJO_TRACE_ENABLED)
    TRACE_EVENT1("mojom", "Send reply handwriting::mojom::HandwritingRecognizer::GetPrediction", "async_response_parameters", [&](perfetto::TracedValue context) {
        auto dict = std::move(context).WriteDictionary();
        perfetto::WriteIntoTracedValueWithFallback(dict.AddItem("prediction"), in_prediction, "<value of type absl::optional<std::vector<HandwritingPredictionPtr>>>");
    });
#endif
    ...
    std::vector<HandwritingStrokePtr> strokes, HandwritingHintsPtr hints, absl::optional<std::vector<HandwritingPredictionPtr>>* out_prediction)
{
    ...
    proxy_->GetPrediction(std::move(strokes), std::move(hints),
        base::BindOnce(
            [](base::RunLoop* loop, absl::optional<std::vector<HandwritingPredictionPtr>>* out_prediction, absl::optional<std::vector<HandwritingPredictionPtr>> prediction) {
...
absl::optional<std::vector<HandwritingPredictionPtr>> HandwritingRecognizerAsyncWaiter::GetPrediction(std::vector<HandwritingStrokePtr> strokes, HandwritingHintsPtr hints)
{
