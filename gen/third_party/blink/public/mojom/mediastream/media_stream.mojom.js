...
(function() {
  ...
  StreamControls.decode = function(decoder) {
    ...
    val.excludeSystemAudio = (packed >> 2) & 1 ? true : false;
    val.excludeSelfBrowserSurface = (packed >> 3) & 1 ? true : false;
    val.requestPanTiltZoomPermission = (packed >> 4) & 1 ? true : false;
    val.requestAllScreens = (packed >> 5) & 1 ? true : false;
    val.dynamicSurfaceSwitchingRequested = (packed >> 6) & 1 ? true : false;
    decoder.skip(1);
    decoder.skip(1);
    decoder.skip(1);
  ...
  StreamControls.encode = function(encoder, val) {
    ...
    packed |= (val.excludeSystemAudio & 1) << 2
    packed |= (val.excludeSelfBrowserSurface & 1) << 3
    packed |= (val.requestPanTiltZoomPermission & 1) << 4
    packed |= (val.requestAllScreens & 1) << 5
    packed |= (val.dynamicSurfaceSwitchingRequested & 1) << 6
    encoder.writeUint8(packed);
    encoder.skip(1);
    encoder.skip(1);
    encoder.skip(1);
