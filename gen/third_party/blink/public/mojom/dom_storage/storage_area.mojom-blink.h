...
// with not having this data in traces there.
    ...
    virtual void SetStorageAreaMap(void* area_map) = 0;
...
//
//   void KeyChanged(const WTF::Vector<uint8_t>& key, const WTF::Vector<uint8_t>& new_value, const absl::optional<WTF::Vector<uint8_t>>& old_value, const WTF::String& source)
//   final;
//
...
//
//   void Put(const WTF::Vector<uint8_t>& key, const WTF::Vector<uint8_t>& value, const absl::optional<WTF::Vector<uint8_t>>& client_old_value, const WTF::String& source,
//   PutCallback callback) final;
//
...
public:
    ...
    template <typename... Args> static KeyValuePtr New(Args&&... args) { return KeyValuePtr(absl::in_place, std::forward<Args>(args)...); }

    template <typename U> static KeyValuePtr From(const U& u) { return mojo::TypeConverter<KeyValuePtr, U>::Convert(u); }

    template <typename U> U To() const { return mojo::TypeConverter<U, KeyValue>::Convert(*this); }
    KeyValue()
        : key()
        , value()
    {
    ...
    template <typename T, KeyValue::EnableIfSame<T>* = nullptr> bool operator==(const T& rhs) const { return Equals(rhs); }
    template <typename UserType> static WTF::Vector<uint8_t> Serialize(UserType* input) { return mojo::internal::SerializeImpl<KeyValue::DataView, WTF::Vector<uint8_t>>(input); }

    template <typename UserType> static mojo::Message SerializeAsMessage(UserType* input) { return mojo::internal::SerializeAsMessageImpl<KeyValue::DataView>(input); }
    ...
    {
        return mojo::Message(std::make_unique<internal::KeyValue_UnserializedMessageContext<UserType, KeyValue::DataView>>(0, 0, std::move(input)), MOJO_CREATE_MESSAGE_FLAG_NONE);
    }
    ...
    {
        return KeyValue::Deserialize(input.size() == 0 ? nullptr : &input.front(), input.size(), output);
    }
...
template <typename T, KeyValue::EnableIfSame<T>* = nullptr> bool operator<=(const T& lhs, const T& rhs) { return !(rhs < lhs); }

template <typename T, KeyValue::EnableIfSame<T>* = nullptr> bool operator>(const T& lhs, const T& rhs) { return rhs < lhs; }

template <typename T, KeyValue::EnableIfSame<T>* = nullptr> bool operator>=(const T& lhs, const T& rhs) { return !(lhs < rhs); }

template <typename StructPtrType> KeyValuePtr KeyValue::Clone() const { return New(mojo::Clone(key), mojo::Clone(value)); }
...
template <> struct PLATFORM_EXPORT StructTraits<::blink::mojom::blink::KeyValue::DataView, ::blink::mojom::blink::KeyValuePtr> {
    ...
    static const decltype(::blink::mojom::blink::KeyValue::key)& key(const ::blink::mojom::blink::KeyValuePtr& input) { return input->key; }

    static const decltype(::blink::mojom::blink::KeyValue::value)& value(const ::blink::mojom::blink::KeyValuePtr& input) { return input->value; }
