...
// with not having this data in traces there.
    ...
    virtual void Put(const std::vector<uint8_t>& key, const std::vector<uint8_t>& value, const absl::optional<std::vector<uint8_t>>& client_old_value, const std::string& source,
...
//
//   void KeyChanged(const std::vector<uint8_t>& key, const std::vector<uint8_t>& new_value, const absl::optional<std::vector<uint8_t>>& old_value, const std::string& source)
//   final;
//
...
//
//   void Put(const std::vector<uint8_t>& key, const std::vector<uint8_t>& value, const absl::optional<std::vector<uint8_t>>& client_old_value, const std::string& source,
...
public:
    ...
    template <typename UserType> static std::vector<uint8_t> Serialize(UserType* input) { return mojo::internal::SerializeImpl<KeyValue::DataView, std::vector<uint8_t>>(input); }
...
template <> struct BLINK_COMMON_EXPORT StructTraits<::blink::mojom::KeyValue::DataView, ::blink::mojom::KeyValuePtr> {
    ...
    static const decltype(::blink::mojom::KeyValue::key)& key(const ::blink::mojom::KeyValuePtr& input) { return input->key; }

    static const decltype(::blink::mojom::KeyValue::value)& value(const ::blink::mojom::KeyValuePtr& input) { return input->value; }
