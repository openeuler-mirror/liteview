...
//
//   void StartDetection(::mojo::PendingRemote<OomInterventionHost> host, DetectionArgsPtr detection_args, bool renderer_pause_enabled, bool navigate_ads_enabled, bool
//   purge_v8_memory_enabled) final;
//
//  private:
//   mojo::MessageReceiverWithResponder* receiver_;
// };
...
public:
    ...
    template <typename... Args> static DetectionArgsPtr New(Args&&... args) { return DetectionArgsPtr(absl::in_place, std::forward<Args>(args)...); }

    template <typename U> static DetectionArgsPtr From(const U& u) { return mojo::TypeConverter<DetectionArgsPtr, U>::Convert(u); }

    template <typename U> U To() const { return mojo::TypeConverter<U, DetectionArgs>::Convert(*this); }
    DetectionArgs()
    ...
    DetectionArgs(uint64_t blink_workload_threshold_in, uint64_t private_footprint_threshold_in, uint64_t swap_threshold_in, uint64_t virtual_memory_thresold_in)
    ...
    template <typename T, DetectionArgs::EnableIfSame<T>* = nullptr> bool operator==(const T& rhs) const { return Equals(rhs); }
    ...
    template <typename UserType> static mojo::Message SerializeAsMessage(UserType* input) { return mojo::internal::SerializeAsMessageImpl<DetectionArgs::DataView>(input); }
    ...
    {
        return mojo::Message(
            std::make_unique<internal::DetectionArgs_UnserializedMessageContext<UserType, DetectionArgs::DataView>>(0, 0, std::move(input)), MOJO_CREATE_MESSAGE_FLAG_NONE);
    }
    ...
    {
        return DetectionArgs::Deserialize(input.size() == 0 ? nullptr : &input.front(), input.size(), output);
    }
...
template <typename T, DetectionArgs::EnableIfSame<T>* = nullptr> bool operator<=(const T& lhs, const T& rhs) { return !(rhs < lhs); }

template <typename T, DetectionArgs::EnableIfSame<T>* = nullptr> bool operator>(const T& lhs, const T& rhs) { return rhs < lhs; }

template <typename T, DetectionArgs::EnableIfSame<T>* = nullptr> bool operator>=(const T& lhs, const T& rhs) { return !(lhs < rhs); }
template <typename StructPtrType> DetectionArgsPtr DetectionArgs::Clone() const
{
    return New(mojo::Clone(blink_workload_threshold), mojo::Clone(private_footprint_threshold), mojo::Clone(swap_threshold), mojo::Clone(virtual_memory_thresold));
}
...
template <> struct PLATFORM_EXPORT StructTraits<::blink::mojom::blink::DetectionArgs::DataView, ::blink::mojom::blink::DetectionArgsPtr> {
    ...
    static decltype(::blink::mojom::blink::DetectionArgs::blink_workload_threshold) blink_workload_threshold(const ::blink::mojom::blink::DetectionArgsPtr& input)
    {
        return input->blink_workload_threshold;
    }

    static decltype(::blink::mojom::blink::DetectionArgs::private_footprint_threshold) private_footprint_threshold(const ::blink::mojom::blink::DetectionArgsPtr& input)
    {
        return input->private_footprint_threshold;
    }

    static decltype(::blink::mojom::blink::DetectionArgs::swap_threshold) swap_threshold(const ::blink::mojom::blink::DetectionArgsPtr& input) { return input->swap_threshold; }
