...
void DidClearDocumentOfWindowObjectImpl(LocalFrame* param_local_frame)
{
    ...
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->DidClearDocumentOfWindowObject(param_local_frame); });
    }
}
...
void WillModifyDOMAttrImpl(Element* param_element, const AtomicString& old_value, const AtomicString& new_value)
{
    ...
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->WillModifyDOMAttr(param_element, old_value, new_value); });
    }
}
...
void DidCreateCanvasContextImpl(Document* param_document)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidCreateCanvasContext(); });
    }
}
...
void DidFireWebGLErrorImpl(Element* param_element, const String& error_name)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidFireWebGLError(error_name); });
    }
}
...
void DidFireWebGLWarningImpl(Element* param_element)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidFireWebGLWarning(); });
    }
}
...
void DidFireWebGLErrorOrWarningImpl(Element* param_element, const String& message)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidFireWebGLErrorOrWarning(message); });
    }
}
...
void ApplyHardwareConcurrencyOverrideImpl(CoreProbeSink* param_core_probe_sink, unsigned int& hardware_concurrency)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorEmulationAgents()) {
        probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) { agent->ApplyHardwareConcurrencyOverride(hardware_concurrency); });
    }
}
...
void ApplyUserAgentMetadataOverrideImpl(CoreProbeSink* param_core_probe_sink, absl::optional<blink::UserAgentMetadata>* ua_metadata_override)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorEmulationAgents()) {
        probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) { agent->ApplyUserAgentMetadataOverride(ua_metadata_override); });
    }
}

void DidBlockRequestImpl(CoreProbeSink* param_core_probe_sink, const ResourceRequest& param_resource_request, DocumentLoader* param_document_loader, const KURL& fetch_context_url,
    const ResourceLoaderOptions& param_resource_loader_options, ResourceRequestBlockedReason param_resource_request_blocked_reason, ResourceType param_resource_type)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
            agent->DidBlockRequest(
                param_resource_request, param_document_loader, fetch_context_url, param_resource_loader_options, param_resource_request_blocked_reason, param_resource_type);
        });
    }
}
    ...
    ResourceLoaderOptions& param_resource_loader_options, ResourceType param_resource_type)
{
    ...
    if (probe_sink->HasInspectorEmulationAgents()) {
        probe_sink->InspectorEmulationAgents().ForEachAgent(
            [&](InspectorEmulationAgent* agent) { agent->PrepareRequest(param_document_loader, param_resource_request, param_resource_loader_options, param_resource_type); });
    }
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->PrepareRequest(param_document_loader, param_resource_request, param_resource_loader_options, param_resource_type); });
    }
}

void WillSendRequestImpl(CoreProbeSink* param_core_probe_sink, DocumentLoader* param_document_loader, const KURL& fetch_context_url, const ResourceRequest& param_resource_request,
    const ResourceResponse& redirect_response, const ResourceLoaderOptions& param_resource_loader_options, ResourceType param_resource_type,
    RenderBlockingBehavior param_render_blocking_behavior, base::TimeTicks timestamp)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) {
            agent->WillSendRequest(param_document_loader, fetch_context_url, param_resource_request, redirect_response, param_resource_loader_options, param_resource_type,
                param_render_blocking_behavior, timestamp);
        });
    }
    ...
    const AtomicString& http_method, EncodedFormData* param_encoded_form_data)
{
    ...
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->WillSendNavigationRequest(identifier, param_document_loader, param_kurl, http_method, param_encoded_form_data); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
        probe_sink->InspectorTraceEventss().ForEachAgent(
            [&](InspectorTraceEvents* agent) { agent->WillSendNavigationRequest(identifier, param_document_loader, param_kurl, http_method, param_encoded_form_data); });
    }
}
...
void MarkResourceAsCachedImpl(LocalFrame* param_local_frame, DocumentLoader* param_document_loader, uint64_t identifier)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->MarkResourceAsCached(param_document_loader, identifier); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
    ...
    const ResourceResponse& param_resource_response, const Resource* param_resource)
{
    ...
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->DidReceiveResourceResponse(identifier, param_document_loader, param_resource_response, param_resource); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
        probe_sink->InspectorTraceEventss().ForEachAgent(
            [&](InspectorTraceEvents* agent) { agent->DidReceiveResourceResponse(identifier, param_document_loader, param_resource_response, param_resource); });
    }
}
...
void DidReceiveDataImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, const char* data, uint64_t data_length)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidReceiveData(identifier, param_document_loader, data, data_length); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
        probe_sink->InspectorTraceEventss().ForEachAgent([&](InspectorTraceEvents* agent) { agent->DidReceiveData(identifier, param_document_loader, data, data_length); });
    }
}
...
void DidReceiveBlobImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, BlobDataHandle* param_blob_data_handle)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidReceiveBlob(identifier, param_document_loader, param_blob_data_handle); });
    }
}
...
void DidReceiveEncodedDataLengthImpl(CoreProbeSink* param_core_probe_sink, DocumentLoader* loader, uint64_t identifier, size_t encoded_data_length)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidReceiveEncodedDataLength(loader, identifier, encoded_data_length); });
    }
}
...
void DidFinishLoadingImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, base::TimeTicks finish_time,
    int64_t encoded_data_length, int64_t decoded_body_length, bool should_report_corb_blocking)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
    if (!probe_sink)
        return;
    ...
    const ResourceResponse& param_resource_response, Resource* param_resource)
{
    ...
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->DidReceiveCorsRedirectResponse(identifier, param_document_loader, param_resource_response, param_resource); });
    }
}
...
void DidFailLoadingImpl(CoreProbeSink* param_core_probe_sink, uint64_t identifier, DocumentLoader* param_document_loader, const ResourceError& param_resource_error,
    const base::UnguessableToken& param_base)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_core_probe_sink);
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorIssueReporters()) {
        probe_sink->InspectorIssueReporters().ForEachAgent(
            [&](InspectorIssueReporter* agent) { agent->DidFailLoading(param_core_probe_sink, identifier, param_document_loader, param_resource_error, param_base); });
    }
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->DidFailLoading(param_core_probe_sink, identifier, param_document_loader, param_resource_error, param_base); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
        probe_sink->InspectorTraceEventss().ForEachAgent(
            [&](InspectorTraceEvents* agent) { agent->DidFailLoading(param_core_probe_sink, identifier, param_document_loader, param_resource_error, param_base); });
    }
}
    ...
    ExecutionContext* param_execution_context, uint64_t identifier, const AtomicString& event_name, const AtomicString& event_id, const String& data)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->WillDispatchEventSourceEvent(identifier, event_name, event_id, data); });
    }
}
...
void ScriptExecutionBlockedByCSPImpl(ExecutionContext* param_execution_context, const String& directive_text)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->ScriptExecutionBlockedByCSP(directive_text); });
    }
}
...
void FrameAttachedToParentImpl(LocalFrame* param_local_frame, const absl::optional<AdScriptIdentifier>& ad_script_on_stack)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_local_frame);
    if (!probe_sink)
        return;
...
void FrameDetachedFromParentImpl(LocalFrame* param_local_frame, FrameDetachType param_frame_detach_type)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->FrameDetachedFromParent(param_local_frame, param_frame_detach_type); });
    }
}
...
void WillCommitLoadImpl(LocalFrame* param_local_frame, DocumentLoader* param_document_loader)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorEmulationAgents()) {
        probe_sink->InspectorEmulationAgents().ForEachAgent([&](InspectorEmulationAgent* agent) { agent->WillCommitLoad(param_local_frame, param_document_loader); });
    }
    if (probe_sink->HasInspectorPageAgents()) {
...
void DidCommitLoadImpl(LocalFrame* param_local_frame, DocumentLoader* param_document_loader)
{
    ...
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidCommitLoad(param_local_frame, param_document_loader); });
    }
}
...
void DidNavigateWithinDocumentImpl(LocalFrame* param_local_frame)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->DidNavigateWithinDocument(param_local_frame); });
    }
}
...
void FrameOwnerContentUpdatedImpl(LocalFrame* param_local_frame, HTMLFrameOwnerElement* param_html_frame_owner_element)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMAgents()) {
        probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) { agent->FrameOwnerContentUpdated(param_local_frame, param_html_frame_owner_element); });
    }
}
...
void FrameStartedLoadingImpl(LocalFrame* param_local_frame)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->FrameStartedLoading(param_local_frame); });
    }
    if (probe_sink->HasInspectorTraceEventss()) {
...
void FrameScheduledNavigationImpl(LocalFrame* param_local_frame, const KURL& url, base::TimeDelta delay, ClientNavigationReason reason)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->FrameScheduledNavigation(param_local_frame, url, delay, reason); });
    }
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->FrameScheduledNavigation(param_local_frame, url, delay, reason); });
    }
}
...
void WillCreateWebSocketImpl(
    ExecutionContext* param_execution_context, uint64_t identifier, const KURL& request_url, const String& protocol, absl::optional<base::UnguessableToken>* devtools_token)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
    if (!probe_sink)
        return;
...
void DidReceiveWebSocketHandshakeResponseImpl(ExecutionContext* param_execution_context, uint64_t identifier, network::mojom::blink::WebSocketHandshakeRequest* request,
...
void DidReceiveWebSocketMessageImpl(ExecutionContext* param_execution_context, uint64_t identifier, int opcode, bool masked, const Vector<base::span<const char>>& data)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidReceiveWebSocketMessage(identifier, opcode, masked, data); });
    }
}
...
void DidSendWebSocketMessageImpl(ExecutionContext* param_execution_context, uint64_t identifier, int opcode, bool masked, const char* payload, size_t payload_length)
{
    ...
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent(
            [&](InspectorNetworkAgent* agent) { agent->DidSendWebSocketMessage(identifier, opcode, masked, payload, payload_length); });
    }
}
...
void DidReceiveWebSocketMessageErrorImpl(ExecutionContext* param_execution_context, uint64_t identifier, const String& error_message)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->DidReceiveWebSocketMessageError(identifier, error_message); });
    }
}
...
void WebTransportCreatedImpl(ExecutionContext* param_execution_context, uint64_t transport_id, const KURL& request_url)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorNetworkAgents()) {
        probe_sink->InspectorNetworkAgents().ForEachAgent([&](InspectorNetworkAgent* agent) { agent->WebTransportCreated(param_execution_context, transport_id, request_url); });
    }
}
...
void AnimationPlayStateChangedImpl(Document* param_document, Animation* param_animation, Animation::AnimationPlayState old_play_state, Animation::AnimationPlayState new_play_state)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorAnimationAgents()) {
        probe_sink->InspectorAnimationAgents().ForEachAgent(
            [&](InspectorAnimationAgent* agent) { agent->AnimationPlayStateChanged(param_animation, old_play_state, new_play_state); });
    }
}
...
void BreakableLocationImpl(ExecutionContext* context, const char* name)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->BreakableLocation(name); });
    }
}
...
ExecuteScript::ExecuteScript(ExecutionContext* context, const v8::Local<v8::Context>& v8_context, const String& script_url, int script_id)
    : context(context)
...
CallFunction::CallFunction(ExecutionContext* context, const v8::Local<v8::Context>& v8_context, v8::Local<v8::Function> function, int depth)
    : context(context)
    , v8_context(v8_context)
    , function(function)
    , depth(depth)
...
void ConsoleTimeStampImpl(ExecutionContext* param_execution_context, const String& title)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPerformanceAgents()) {
        probe_sink->InspectorPerformanceAgents().ForEachAgent([&](InspectorPerformanceAgent* agent) { agent->ConsoleTimeStamp(title); });
    }
}
...
void LifecycleEventImpl(LocalFrame* param_local_frame, DocumentLoader* param_document_loader, const char* name, double timestamp)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->LifecycleEvent(param_local_frame, param_document_loader, name, timestamp); });
    }
}
...
void DidCreateAudioContextImpl(Document* param_document)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidCreateAudioContext(); });
    }
}
...
void DidCloseAudioContextImpl(Document* param_document)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidCloseAudioContext(); });
    }
}
...
void DidResumeAudioContextImpl(Document* param_document)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidResumeAudioContext(); });
    }
}
...
void DidSuspendAudioContextImpl(Document* param_document)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->DidSuspendAudioContext(); });
    }
}
...
void ApplyCompilationModeOverrideImpl(ExecutionContext* param_execution_context, const ClassicScript& param_classic_script, v8::ScriptCompiler::CachedData** cached_data,
    v8::ScriptCompiler::CompileOptions* param_v8)
{
    CoreProbeSink* probe_sink = ToCoreProbeSink(param_execution_context);
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorPageAgents()) {
        probe_sink->InspectorPageAgents().ForEachAgent([&](InspectorPageAgent* agent) { agent->ApplyCompilationModeOverride(param_classic_script, cached_data, param_v8); });
    }
}
...
void PortalRemoteFrameCreatedImpl(Document* param_document, HTMLPortalElement* portal_element)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMAgents()) {
        probe_sink->InspectorDOMAgents().ForEachAgent([&](InspectorDOMAgent* agent) { agent->PortalRemoteFrameCreated(portal_element); });
    }
}
...
void OnContentSecurityPolicyViolationImpl(ExecutionContext* context, const blink::ContentSecurityPolicyViolationType violationType)
{
    ...
    if (!probe_sink)
        return;
    if (probe_sink->HasInspectorDOMDebuggerAgents()) {
        probe_sink->InspectorDOMDebuggerAgents().ForEachAgent([&](InspectorDOMDebuggerAgent* agent) { agent->OnContentSecurityPolicyViolation(violationType); });
    }
}
