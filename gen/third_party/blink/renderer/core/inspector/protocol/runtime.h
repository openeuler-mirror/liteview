...
template <typename T> struct ProtocolTypeTraits<std::unique_ptr<T>, typename std::enable_if<std::is_base_of<v8_inspector::protocol::Exported, T>::value>::type> {
...
public:
    static std::unique_ptr<ImportedValue> fromExported(const Exported* value) { return std::unique_ptr<ImportedValue>(new ImportedValue(value)); }

    void AppendSerialized(std::vector<uint8_t>* output) const override { m_exported->AppendSerialized(output); }

    std::unique_ptr<Value> clone() const override { return std::unique_ptr<Value>(new ImportedValue(m_exported)); }
...
template <> struct ValueConversions<v8_inspector::protocol::Runtime::API::RemoteObject> {
    ...
    static std::unique_ptr<protocol::Value> toValue(const v8_inspector::protocol::Runtime::API::RemoteObject* exported) { return ImportedValue::fromExported(exported); }
};
template <> struct ValueConversions<v8_inspector::protocol::Runtime::API::StackTrace> {
    ...
    static std::unique_ptr<protocol::Value> toValue(const v8_inspector::protocol::Runtime::API::StackTrace* exported) { return ImportedValue::fromExported(exported); }
};
} // namespace blink
} // namespace protocol
