...
template <typename T> struct ProtocolTypeTraits<T, typename std::enable_if<std::is_base_of<blink::protocol::Value, T>::value>::type> {
    static void Serialize(const blink::protocol::Value& value, std::vector<uint8_t>* bytes) { value.AppendSerialized(bytes); }
};
...
template <> struct ProtocolTypeTraits<blink::protocol::Object> {
    static void Serialize(const blink::protocol::Object& value, std::vector<uint8_t>* bytes) { value.AppendSerialized(bytes); }
};
