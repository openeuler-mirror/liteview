...
#define SETTINGS_INITIALIZER_LIST                                                                                                                                                  \
    , network_quiet_timeout_(0.5), accelerated_2d_canvas_msaa_sample_count_(0), accessibility_font_scale_factor_(1.0),                                                             \
        autoplay_policy_(AutoplayPolicy::Type::kNoUserGestureRequired), available_hover_types_(ui::HOVER_TYPE_NONE), available_pointer_types_(ui::POINTER_TYPE_NONE),              \
        default_fixed_font_size_(0), default_font_size_(0), device_scale_adjustment_(1.0), display_mode_override_(blink::mojom::DisplayMode::kUndefined),                          \
        editing_behavior_type_(EditingBehaviorTypeForPlatform()), image_animation_policy_(mojom::blink::ImageAnimationPolicy::kImageAnimationPolicyAllowed),                       \
        lazy_frame_loading_distance_threshold_px_2_g_(6000), lazy_frame_loading_distance_threshold_px_3_g_(3500), lazy_frame_loading_distance_threshold_px_4_g_(2500),             \
        lazy_frame_loading_distance_threshold_px_offline_(8000), lazy_frame_loading_distance_threshold_px_slow_2_g_(8000),                                                         \
        lazy_frame_loading_distance_threshold_px_unknown_(4000), lazy_image_loading_distance_threshold_px_2_g_(6000), lazy_image_loading_distance_threshold_px_3_g_(2500),         \
        lazy_image_loading_distance_threshold_px_4_g_(1250), lazy_image_loading_distance_threshold_px_offline_(8000), lazy_image_loading_distance_threshold_px_slow_2_g_(8000),    \
        lazy_image_loading_distance_threshold_px_unknown_(3000), low_priority_iframes_threshold_(WebEffectiveConnectionType::kTypeUnknown), max_touch_points_(0),                  \
        media_type_override_(""), minimum_font_size_(0), minimum_logical_font_size_(0), navigation_controls_(NavigationControls::kNone),                                           \
        parser_scripting_flag_policy_(ParserScriptingFlagPolicy::kOnlyIfScriptIsEnabled), password_echo_duration_in_seconds_(1),                                                   \
        preferred_color_scheme_(mojom::blink::PreferredColorScheme::kLight), preferred_contrast_(mojom::blink::PreferredContrast::kNoPreference),                                  \
        primary_hover_type_(mojom::blink::HoverType::kHoverNone), primary_pointer_type_(mojom::blink::PointerType::kPointerNone),                                                  \
        selection_strategy_(SelectionStrategy::kCharacter), text_track_kind_user_preference_(TextTrackKindUserPreference::kDefault), text_track_margin_percentage_(0),             \
        v8_cache_options_(mojom::blink::V8CacheOptions::kDefault), validation_message_timer_magnification_(50), viewport_style_(mojom::blink::ViewportStyle::kDefault),            \
        dom_paste_allowed_(false), dont_send_key_events_to_javascript_(false), highlight_ads_(false), web_xr_immersive_ar_allowed_(true), accelerated_compositing_enabled_(false), \
        accessibility_always_show_focus_(false), accessibility_include_svg_g_element_(false), accessibility_password_values_enabled_(false),                                       \
        allow_custom_scrollbar_in_main_frame_(true), allow_file_access_from_file_urls_(true), allow_geolocation_on_insecure_origins_(false),                                       \
        allow_non_empty_navigator_plugins_(false), allow_running_of_insecure_content_(true), allow_scripts_to_close_windows_(false), allow_universal_access_from_file_urls_(true), \
        always_show_context_menu_on_touch_(true), antialiased_2d_canvas_enabled_(true), antialiased_clips_2d_canvas_enabled_(true), aria_modal_prunes_ax_tree_(false),             \
        barrel_button_for_drag_enabled_(false), bypass_csp_(false), caret_browsing_enabled_(false), cookie_enabled_(true), disable_reading_from_canvas_(false),                    \
        disallow_fetch_for_doc_written_scripts_in_main_frame_(false), disallow_fetch_for_doc_written_scripts_in_main_frame_if_effectively_2_g_(false),                             \
        disallow_fetch_for_doc_written_scripts_in_main_frame_on_slow_connections_(true), dns_prefetching_enabled_(false), do_html_preload_scanning_(true),                         \
        do_not_update_selection_on_mutating_selection_range_(false), downloadable_binary_fonts_enabled_(true), embedded_media_experience_enabled_(false),                          \
        force_android_overlay_scrollbar_(false), force_dark_mode_enabled_(false), force_main_world_initialization_(false),                                                         \
        force_touch_event_feature_detection_for_inspector_(false), force_zero_layout_height_(false), fullscreen_supported_(true), hide_download_ui_(false),                        \
        hide_scrollbars_(false), hyperlink_auditing_enabled_(false), ignore_main_frame_overflow_hidden_quirk_(false), images_enabled_(true), immersive_mode_enabled_(false),       \
        inline_text_box_accessibility_enabled_(false), java_script_can_access_clipboard_(false), lazy_load_enabled_(true), load_with_overview_mode_(true),                         \
        loads_images_automatically_(false), local_storage_enabled_(false), log_dns_prefetch_and_preconnect_(false), log_preload_(false), main_frame_clips_content_(true),          \
        main_frame_resizes_are_orientation_changes_(false), media_controls_enabled_(true), mock_gesture_tap_highlights_enabled_(false),                                            \
        multi_target_tap_notification_enabled_(true), navigate_on_drag_drop_(true), password_echo_enabled_(false), picture_in_picture_enabled_(true), plugins_enabled_(false),     \
        prefer_compositing_to_lcd_text_enabled_(false), prefer_hidden_volume_controls_(false), prefers_reduced_motion_(false), presentation_receiver_(false),                      \
        presentation_requires_user_gesture_(true), report_screen_size_in_physical_pixels_quirk_(false), rubber_banding_on_compositor_thread_(false), script_enabled_(false),       \
        scroll_animator_enabled_(true), select_trailing_whitespace_enabled_(kDefaultSelectTrailingWhitespaceEnabled), selection_clipboard_buffer_available_(false),                \
        selection_includes_alt_image_text_(false), should_clear_document_background_(true), should_print_backgrounds_(false), should_protect_against_ipc_flooding_(true),          \
        should_reuse_global_for_unowned_main_frame_(false), show_context_menu_on_mouse_up_(false), shrinks_viewport_content_to_fit_(false), smart_insert_delete_enabled_(false),   \
        smooth_scroll_for_find_enabled_(false), spatial_navigation_enabled_(false), spell_check_enabled_by_default_(true), strict_mixed_content_checking_(false),                  \
        strict_mixed_content_checking_for_plugin_(false), strict_powerful_feature_restrictions_(false), strictly_block_blockable_mixed_content_(false),                            \
        supports_multiple_windows_(true), sync_xhr_in_documents_enabled_(true), target_blank_implies_no_opener_enabled_will_be_removed_(true), text_areas_are_resizable_(false),   \
        text_autosizing_enabled_(false), threaded_scrolling_enabled_(true), touch_drag_drop_enabled_(false), touch_drag_end_context_menu_(false), touch_editing_enabled_(false),   \
        use_ax_menu_list_(true), use_legacy_background_size_shorthand_behavior_(false), use_wide_viewport_(true), viewport_enabled_(false), viewport_meta_enabled_(false),         \
        viewport_meta_merge_content_quirk_(false), viewport_meta_zero_values_quirk_(false), web_gl_1_enabled_(true), webgl2_enabled_(true),                                        \
        web_gl_errors_to_console_enabled_(true), web_security_enabled_(true), wide_viewport_quirk_enabled_(false) // End of SETTINGS_INITIALIZER_LIST.
