...
CORE_EXPORT void ApplyUserAgentMetadataOverrideImpl(CoreProbeSink*, absl::optional<blink::UserAgentMetadata>*);
inline void ApplyUserAgentMetadataOverride(CoreProbeSink* param_core_probe_sink, absl::optional<blink::UserAgentMetadata>* ua_metadata_override)
{
    if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorEmulationAgent))
        return;
...
inline void DidBlockRequest(CoreProbeSink* param_core_probe_sink, const ResourceRequest& param_resource_request, DocumentLoader* param_document_loader,
    const KURL& fetch_context_url, const ResourceLoaderOptions& param_resource_loader_options, ResourceRequestBlockedReason param_resource_request_blocked_reason,
    ResourceType param_resource_type)
{
...
CORE_EXPORT void WillSendRequestImpl(CoreProbeSink*, DocumentLoader*, const KURL&, const ResourceRequest&, const ResourceResponse&, const ResourceLoaderOptions&, ResourceType,
    RenderBlockingBehavior, base::TimeTicks);
inline void WillSendRequest(CoreProbeSink* param_core_probe_sink, DocumentLoader* param_document_loader, const KURL& fetch_context_url,
    ...
    ResourceType param_resource_type, RenderBlockingBehavior param_render_blocking_behavior, base::TimeTicks timestamp)
{
    ...
    WillSendRequestImpl(param_core_probe_sink, param_document_loader, fetch_context_url, param_resource_request, redirect_response, param_resource_loader_options,
        param_resource_type, param_render_blocking_behavior, timestamp);
}
...
CORE_EXPORT void DidFinishLoadingImpl(CoreProbeSink*, uint64_t, DocumentLoader*, base::TimeTicks, int64_t, int64_t, bool);
...
CORE_EXPORT void FrameAttachedToParentImpl(LocalFrame*, const absl::optional<AdScriptIdentifier>&);
inline void FrameAttachedToParent(LocalFrame* param_local_frame, const absl::optional<AdScriptIdentifier>& ad_script_on_stack)
{
    if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorPageAgent))
        return;
...
{
    if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorPageAgent | CoreProbeSink::kInspectorTraceEvents))
        return;
...
CORE_EXPORT void WillCreateWebSocketImpl(ExecutionContext*, uint64_t, const KURL&, const String&, absl::optional<base::UnguessableToken>*);
inline void WillCreateWebSocket(
...
inline void DidReceiveWebSocketHandshakeResponse(ExecutionContext* param_execution_context, uint64_t identifier, network::mojom::blink::WebSocketHandshakeRequest* request,
...
inline void AnimationPlayStateChanged(
    Document* param_document, Animation* param_animation, Animation::AnimationPlayState old_play_state, Animation::AnimationPlayState new_play_state)
{
    if (!CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorAnimationAgent))
        return;
...
public:
    explicit ExecuteScript(ExecutionContext*, const v8::Local<v8::Context>&, const String&, int);
    ~ExecuteScript();
    CoreProbeSink* probe_sink = nullptr;
...
public:
    explicit CallFunction(ExecutionContext*, const v8::Local<v8::Context>&, v8::Local<v8::Function>, int = 0);
    ~CallFunction();
    CoreProbeSink* probe_sink = nullptr;
    ExecutionContext* context;
    const v8::Local<v8::Context>& v8_context;
...
inline void ApplyCompilationModeOverride(ExecutionContext* param_execution_context, const ClassicScript& param_classic_script, v8::ScriptCompiler::CachedData** cached_data,
