...
public:
    ...
    static ComputedEffectTiming* Create(v8::Isolate* isolate) { return MakeGarbageCollected<ComputedEffectTiming>(isolate); }
    ...
    bool hasCurrentIterationNonNull() const { return hasCurrentIteration() && currentIteration().has_value(); }
    double currentIterationNonNull() const
    {
        DCHECK(hasCurrentIterationNonNull());
        return currentIteration().value();
    }
    bool hasProgressNonNull() const { return hasProgress() && progress().has_value(); }
