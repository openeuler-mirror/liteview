// Copyright 2019 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// DO NOT EDIT: This file is auto-generated by
// //third_party/blink/renderer/bindings/scripts/generate_bindings.py
//
// Use the GN flag `blink_enable_generated_code_formatting=true` to enable
// formatting of the generated files.

#include "third_party/blink/renderer/bindings/core/v8/v8_union_unsignedlong_unsignedlongsequence.h"

#include "third_party/blink/renderer/bindings/core/v8/generated_code_helper.h"
#include "third_party/blink/renderer/bindings/core/v8/native_value_traits_impl.h"
#include "third_party/blink/renderer/bindings/core/v8/to_v8_traits.h"
#include "third_party/blink/renderer/platform/bindings/exception_state.h"

namespace blink {

V8UnionUnsignedLongOrUnsignedLongSequence* V8UnionUnsignedLongOrUnsignedLongSequence::Create(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ExceptionState& exception_state)
{
    if (v8_value->IsArray()) {
        auto&& blink_value = NativeValueTraits<IDLSequence<IDLUnsignedLong>>::NativeValue(isolate, v8_value, exception_state);
        if (UNLIKELY(exception_state.HadException())) {
            return nullptr;
        }
        return MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(std::move(blink_value));
    }
    if (v8_value->IsObject()) {
        ScriptIterator script_iterator = ScriptIterator::FromIterable(isolate, v8_value.As<v8::Object>(), exception_state);
        if (UNLIKELY(exception_state.HadException())) {
            return nullptr;
        }
        if (!script_iterator.IsNull()) {
            auto&& blink_value = bindings::CreateIDLSequenceFromIterator<IDLUnsignedLong>(isolate, std::move(script_iterator), exception_state);
            if (UNLIKELY(exception_state.HadException())) {
                return nullptr;
            }
            return MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(std::move(blink_value));
        }
    }
    if (v8_value->IsNumber()) {
        auto&& blink_value = NativeValueTraits<IDLUnsignedLong>::NativeValue(isolate, v8_value, exception_state);
        if (UNLIKELY(exception_state.HadException())) {
            return nullptr;
        }
        return MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(blink_value);
    }
    {
        auto&& blink_value = NativeValueTraits<IDLUnsignedLong>::NativeValue(isolate, v8_value, exception_state);
        if (UNLIKELY(exception_state.HadException())) {
            return nullptr;
        }
        return MakeGarbageCollected<V8UnionUnsignedLongOrUnsignedLongSequence>(blink_value);
    }
}

V8UnionUnsignedLongOrUnsignedLongSequence::V8UnionUnsignedLongOrUnsignedLongSequence(const Vector<uint32_t>& value)
    : content_type_(ContentType::kUnsignedLongSequence)
    , member_unsigned_long_sequence_(value)
{
}

V8UnionUnsignedLongOrUnsignedLongSequence::V8UnionUnsignedLongOrUnsignedLongSequence(Vector<uint32_t>&& value)
    : content_type_(ContentType::kUnsignedLongSequence)
    , member_unsigned_long_sequence_(std::move(value))
{
}

void V8UnionUnsignedLongOrUnsignedLongSequence::Set(const Vector<uint32_t>& value)
{
    Clear();
    member_unsigned_long_sequence_ = value;
    content_type_ = ContentType::kUnsignedLongSequence;
}

void V8UnionUnsignedLongOrUnsignedLongSequence::Set(Vector<uint32_t>&& value)
{
    Clear();
    member_unsigned_long_sequence_ = std::move(value);
    content_type_ = ContentType::kUnsignedLongSequence;
}

v8::MaybeLocal<v8::Value> V8UnionUnsignedLongOrUnsignedLongSequence::ToV8Value(ScriptState* script_state) const
{
    switch (content_type_) {
    case ContentType::kUnsignedLong: {
        return ToV8Traits<IDLUnsignedLong>::ToV8(script_state, member_unsigned_long_);
    }
    case ContentType::kUnsignedLongSequence: {
        return ToV8Traits<IDLSequence<IDLUnsignedLong>>::ToV8(script_state, member_unsigned_long_sequence_);
    }
    }

    NOTREACHED();
    return v8::MaybeLocal<v8::Value>();
}

void V8UnionUnsignedLongOrUnsignedLongSequence::Trace(Visitor* visitor) const
{
    TraceIfNeeded<uint32_t>::Trace(visitor, member_unsigned_long_);
    TraceIfNeeded<Vector<uint32_t>>::Trace(visitor, member_unsigned_long_sequence_);
    bindings::UnionBase::Trace(visitor);
}

void V8UnionUnsignedLongOrUnsignedLongSequence::Clear()
{
    member_unsigned_long_ = 0;
    member_unsigned_long_sequence_.clear();
}

} // namespace blink
