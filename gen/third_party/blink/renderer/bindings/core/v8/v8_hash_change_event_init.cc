...
bool HashChangeEventInit::FillV8ObjectWithMembers(ScriptState* script_state, v8::Local<v8::Object> v8_dictionary) const
{
    ...
    if (hasNewURL()) {
        if (!ToV8Traits<IDLString>::ToV8(script_state, member_new_url_).ToLocal(&v8_value)) {
            return false;
        }
    ...
    if (hasOldURL()) {
        if (!ToV8Traits<IDLString>::ToV8(script_state, member_old_url_).ToLocal(&v8_value)) {
            return false;
        }
...
void HashChangeEventInit::FillMembersFromV8Object(v8::Isolate* isolate, v8::Local<v8::Object> v8_dictionary, ExceptionState& exception_state)
{
    ...
    exception_context_scope.ChangePropertyNameAsOptimizationHack("newURL");
    constexpr bool is_optional = false;
    v8::Local<v8::Context> current_context = isolate->GetCurrentContext();
    const auto& v8_own_member_names = GetV8OwnMemberNames(isolate);
    v8::TryCatch try_block(isolate);
    if (!bindings::GetDictionaryMemberFromV8Object<IDLString, is_optional>(
            isolate, current_context, v8_dictionary, v8_own_member_names[0].Get(isolate), has_new_url_, member_new_url_, try_block, exception_state)) {
        return;
    }
    exception_context_scope.ChangePropertyNameAsOptimizationHack("oldURL");
    if (!bindings::GetDictionaryMemberFromV8Object<IDLString, is_optional>(
            isolate, current_context, v8_dictionary, v8_own_member_names[1].Get(isolate), has_old_url_, member_old_url_, try_block, exception_state)) {
        return;
    }
}
