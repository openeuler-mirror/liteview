...
bool PushSubscriptionChangeEventInit::FillV8ObjectWithMembers(ScriptState* script_state, v8::Local<v8::Object> v8_dictionary) const
{
    ...
    if (hasNewSubscription()) {
        if (!ToV8Traits<PushSubscription>::ToV8(script_state, member_new_subscription_).ToLocal(&v8_value)) {
            return false;
        }
    ...
    if (hasOldSubscription()) {
        if (!ToV8Traits<PushSubscription>::ToV8(script_state, member_old_subscription_).ToLocal(&v8_value)) {
            return false;
        }
...
void PushSubscriptionChangeEventInit::FillMembersFromV8Object(v8::Isolate* isolate, v8::Local<v8::Object> v8_dictionary, ExceptionState& exception_state)
{
    ...
    exception_context_scope.ChangePropertyNameAsOptimizationHack("newSubscription");
    constexpr bool is_optional = false;
    v8::Local<v8::Context> current_context = isolate->GetCurrentContext();
    const auto& v8_own_member_names = GetV8OwnMemberNames(isolate);
    v8::TryCatch try_block(isolate);
    if (!bindings::GetDictionaryMemberFromV8Object<PushSubscription, is_optional>(
            isolate, current_context, v8_dictionary, v8_own_member_names[0].Get(isolate), has_new_subscription_, member_new_subscription_, try_block, exception_state)) {
        return;
    }
    exception_context_scope.ChangePropertyNameAsOptimizationHack("oldSubscription");
    if (!bindings::GetDictionaryMemberFromV8Object<PushSubscription, is_optional>(
            isolate, current_context, v8_dictionary, v8_own_member_names[1].Get(isolate), has_old_subscription_, member_old_subscription_, try_block, exception_state)) {
        return;
    }
}
