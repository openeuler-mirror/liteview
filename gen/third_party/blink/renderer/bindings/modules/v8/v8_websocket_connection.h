// Copyright 2019 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// DO NOT EDIT: This file is auto-generated by
// //third_party/blink/renderer/bindings/scripts/generate_bindings.py
//
// Use the GN flag `blink_enable_generated_code_formatting=true` to enable
// formatting of the generated files.

#ifndef THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_WEBSOCKET_CONNECTION_H_
#define THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_WEBSOCKET_CONNECTION_H_

#include "base/containers/span.h"
#include "third_party/blink/renderer/modules/modules_export.h"
#include "third_party/blink/renderer/platform/bindings/dictionary_base.h"
#include "third_party/blink/renderer/platform/heap/member.h"
#include "third_party/blink/renderer/platform/wtf/text/wtf_string.h"

namespace blink {

class ExceptionState;
class ReadableStream;
class WritableStream;

class MODULES_EXPORT WebSocketConnection : public bindings::DictionaryBase {

public:
    static WebSocketConnection* Create() { return MakeGarbageCollected<WebSocketConnection>(); }
    static WebSocketConnection* Create(v8::Isolate* isolate) { return MakeGarbageCollected<WebSocketConnection>(isolate); }
    static WebSocketConnection* Create(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, ExceptionState& exception_state);

    explicit WebSocketConnection();
    explicit WebSocketConnection(v8::Isolate* isolate);

    bool hasExtensions() const { return has_extensions_; }
    const String& extensions() const
    {
        DCHECK(hasExtensions());
        return member_extensions_;
    }
    String getExtensionsOr(const String& fallback_value) const;
    String getExtensionsOr(String&& fallback_value) const;
    void setExtensions(const String& value);
    void setExtensions(String&& value);

    bool hasProtocol() const { return has_protocol_; }
    const String& protocol() const
    {
        DCHECK(hasProtocol());
        return member_protocol_;
    }
    String getProtocolOr(const String& fallback_value) const;
    String getProtocolOr(String&& fallback_value) const;
    void setProtocol(const String& value);
    void setProtocol(String&& value);

    bool hasReadable() const { return has_readable_; }
    ReadableStream* readable() const
    {
        DCHECK(hasReadable());
        return member_readable_;
    }
    ReadableStream* getReadableOr(ReadableStream* fallback_value) const
    {
        if (!hasReadable()) {
            return fallback_value;
        }
        return member_readable_;
    }
    void setReadable(ReadableStream* value)
    {
        member_readable_ = value;
        has_readable_ = true;
        DCHECK(member_readable_);
    }

    bool hasWritable() const { return has_writable_; }
    WritableStream* writable() const
    {
        DCHECK(hasWritable());
        return member_writable_;
    }
    WritableStream* getWritableOr(WritableStream* fallback_value) const
    {
        if (!hasWritable()) {
            return fallback_value;
        }
        return member_writable_;
    }
    void setWritable(WritableStream* value)
    {
        member_writable_ = value;
        has_writable_ = true;
        DCHECK(member_writable_);
    }
    void Trace(Visitor* visitor) const override;

protected:
    bool FillV8ObjectWithMembers(ScriptState* script_state, v8::Local<v8::Object> v8_dictionary) const override;

    void FillMembersFromV8Object(v8::Isolate* isolate, v8::Local<v8::Object> v8_dictionary, ExceptionState& exception_state);

private:
    static const base::span<const v8::Eternal<v8::Name>> GetV8OwnMemberNames(v8::Isolate* isolate);

    bool has_extensions_ = false;
    bool has_protocol_ = false;
    bool has_readable_ = false;
    bool has_writable_ = false;

    String member_extensions_;
    String member_protocol_;
    Member<ReadableStream> member_readable_;
    Member<WritableStream> member_writable_;
};
} // namespace blink

#endif // THIRD_PARTY_BLINK_RENDERER_BINDINGS_MODULES_V8_V8_WEBSOCKET_CONNECTION_H_
