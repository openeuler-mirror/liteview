...
public:
    ...
    static WriteParams* Create(v8::Isolate* isolate) { return MakeGarbageCollected<WriteParams>(isolate); }
    ...
    V8WriteCommandType type() const { return member_type_; }
    void setType(V8WriteCommandType value) { member_type_ = value; }
    void setType(V8WriteCommandType::Enum value) { member_type_ = V8WriteCommandType(value); }
    // Obsolete accessor functions
    bool hasPositionNonNull() const { return hasPosition() && position().has_value(); }
    uint64_t positionNonNull() const
    {
        DCHECK(hasPositionNonNull());
        return position().value();
    }
    bool hasSizeNonNull() const { return hasSize() && size().has_value(); }
    uint64_t sizeNonNull() const
    {
        DCHECK(hasSizeNonNull());
        return size().value();
    }
    void setType(const String& value) { member_type_ = V8WriteCommandType::Create(value).value(); }
