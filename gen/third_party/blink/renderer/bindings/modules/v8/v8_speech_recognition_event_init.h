...
public:
    ...
    static SpeechRecognitionEventInit* Create(v8::Isolate* isolate) { return MakeGarbageCollected<SpeechRecognitionEventInit>(isolate); }
    ...
    bool hasResultIndex() const { return true; }
    uint32_t resultIndex() const { return member_result_index_; }
    void setResultIndex(uint32_t value) { member_result_index_ = value; }

    bool hasResults() const { return has_results_; }
    SpeechRecognitionResultList* results() const
    {
        DCHECK(hasResults());
        return member_results_;
    }
