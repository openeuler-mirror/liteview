...
(function() {
  ...
  URLResponseHead.validate = function(messageValidator, offset) {
    ...
    err = messageValidator.validateStructPointer(offset + codec.kStructHeaderSize + 112, proxy_resolver$.ProxyServer, false);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  URLResponseHead.encodedSize = codec.kStructHeaderSize + 232;
  ...
  URLResponseHead.decode = function(decoder) {
    ...
    packed = decoder.readUint8();
    val.isLegacyTlsVersion = (packed >> 0) & 1 ? true : false;
    val.hasRangeRequested = (packed >> 1) & 1 ? true : false;
    val.timingAllowPassed = (packed >> 2) & 1 ? true : false;
    val.hasAuthorizationCoveredByWildcardOnPreflight = (packed >> 3) & 1 ? true : false;
    val.requestIncludeCredentials = (packed >> 4) & 1 ? true : false;
    val.hasPartitionedCookie = (packed >> 5) & 1 ? true : false;
    decoder.skip(1);
    val.contentLength =
  ...
  URLResponseHead.encode = function(encoder, val) {
    ...
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.isLegacyTlsVersion & 1) << 0
    packed |= (val.hasRangeRequested & 1) << 1
    packed |= (val.timingAllowPassed & 1) << 2
    packed |= (val.hasAuthorizationCoveredByWildcardOnPreflight & 1) << 3
    packed |= (val.requestIncludeCredentials & 1) << 4
    packed |= (val.hasPartitionedCookie & 1) << 5
    encoder.writeUint8(packed);
    encoder.skip(1);
