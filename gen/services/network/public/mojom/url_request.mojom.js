...
(function() {
  ...
  URLRequest.validate = function(messageValidator, offset) {
    ...
    err = messageValidator.validateStructPointer(offset + codec.kStructHeaderSize + 216, NetLogSource, true);
    if (err !== validator.validationError.NONE)
        return err;
    // validate URLRequest.netLogReferenceInfo
    err = messageValidator.validateStructPointer(offset + codec.kStructHeaderSize + 224, NetLogSource, true);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  URLRequest.encodedSize = codec.kStructHeaderSize + 232;
  ...
  URLRequest.decode = function(decoder) {
    ...
    val.skipServiceWorker = (packed >> 2) & 1 ? true : false;
    val.corbDetachable = (packed >> 3) & 1 ? true : false;
    val.keepalive = (packed >> 4) & 1 ? true : false;
    val.hasUserGesture = (packed >> 5) & 1 ? true : false;
    val.enableLoadTiming = (packed >> 6) & 1 ? true : false;
    val.enableUploadProgress = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.doNotPromptForLogin = (packed >> 0) & 1 ? true : false;
    val.isOutermostMainFrame = (packed >> 1) & 1 ? true : false;
    val.upgradeIfInsecure = (packed >> 2) & 1 ? true : false;
    val.isRevalidating = (packed >> 3) & 1 ? true : false;
    val.isFetchLikeApi = (packed >> 4) & 1 ? true : false;
    val.isFavicon = (packed >> 5) & 1 ? true : false;
    decoder.skip(1);
    decoder.skip(1);
    val.referrerPolicy =
  ...
  URLRequest.encode = function(encoder, val) {
    ...
    packed |= (val.skipServiceWorker & 1) << 2
    packed |= (val.corbDetachable & 1) << 3
    packed |= (val.keepalive & 1) << 4
    packed |= (val.hasUserGesture & 1) << 5
    packed |= (val.enableLoadTiming & 1) << 6
    packed |= (val.enableUploadProgress & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.doNotPromptForLogin & 1) << 0
    packed |= (val.isOutermostMainFrame & 1) << 1
    packed |= (val.upgradeIfInsecure & 1) << 2
    packed |= (val.isRevalidating & 1) << 3
    packed |= (val.isFetchLikeApi & 1) << 4
    packed |= (val.isFavicon & 1) << 5
    encoder.writeUint8(packed);
    encoder.skip(1);
    encoder.skip(1);
