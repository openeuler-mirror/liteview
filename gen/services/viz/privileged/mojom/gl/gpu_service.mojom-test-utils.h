...
public:
    ...
    void EstablishGpuChannel(int32_t client_id, uint64_t client_tracing_id, bool is_gpu_host, ::mojo::ScopedMessagePipeHandle* out_channel_handle, ::gpu::GPUInfo* out_gpu_info,
        ::gpu::GpuFeatureInfo* out_gpu_feature_info);

    void RegisterDCOMPSurfaceHandle(::mojo::PlatformHandle surface_handle, absl::optional<::base::UnguessableToken>* out_token);
    absl::optional<::base::UnguessableToken> RegisterDCOMPSurfaceHandle(::mojo::PlatformHandle surface_handle);
    void CreateGpuMemoryBuffer(::gfx::GpuMemoryBufferId id, const ::gfx::Size& size, ::gfx::BufferFormat format, ::gfx::BufferUsage usage, int32_t client_id,
    ...
    void GetPeakMemoryUsage(uint32_t sequence_num, uint64_t* out_memory_usage, base::flat_map<::gpu::GpuPeakMemoryAllocationSource, uint64_t>* out_memory_per_allocation_source);

    void RequestDXGIInfo(::gfx::mojom::DXGIInfoPtr* out_dxgi_info);
    ::gfx::mojom::DXGIInfoPtr RequestDXGIInfo();
    void GetDawnInfo(std::vector<std::string>* out_dawn_info_list);
    std::vector<std::string> GetDawnInfo();
