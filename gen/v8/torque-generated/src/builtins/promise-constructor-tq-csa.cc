    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<NativeContext> p_nativeContext, TNode<Object> p_promiseFun, TNode<Object> p_executor)
{
    ...
    if (block0.is_used()) {
        ...
        BranchIfAccessCheckFailed_0(
            state_, TNode<Context> { p_context }, TNode<NativeContext> { p_nativeContext }, TNode<Object> { p_promiseFun }, TNode<Object> { p_executor }, &label0);
        ca_.Goto(&block4);
        if (label0.is_used()) {
            ca_.Bind(&label0);
            ca_.Goto(&block5);
...
TF_BUILTIN(PromiseConstructor, CodeStubAssembler)
{
    ...
    if (block11.is_used()) {
        ca_.Bind(&block11);
        tmp18 = True_0(state_);
        std::tie(tmp19, tmp20) = CreatePromiseResolvingFunctions_0(
            state_, TNode<Context> { parameter0 }, TNode<JSPromise> { phi_bb9_5 }, TNode<Oddball> { tmp18 }, TNode<NativeContext> { parameter0 })
                                     .Flatten();
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch26__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp25
                = CodeStubAssembler(state_).Call(TNode<Context> { parameter0 }, TNode<Object> { tmp21 }, TNode<Object> { tmp24 }, TNode<Object> { tmp19 }, TNode<Object> { tmp20 });
        }
...
TF_BUILTIN(PromisePrototypeCatch, CodeStubAssembler)
{
    ...
    if (block1.is_used()) {
        ...
        tmp3 = InvokeThen_1(
            state_, TNode<Context> { parameter0 }, TNode<NativeContext> { tmp1 }, TNode<Object> { parameter1 }, TNode<Object> { tmp2 }, TNode<Object> { parameter2 });
