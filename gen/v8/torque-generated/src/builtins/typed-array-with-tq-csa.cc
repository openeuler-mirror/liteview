...
TF_BUILTIN(TypedArrayPrototypeWith, CodeStubAssembler)
{
    ...
    if (block30.is_used()) {
        ...
        tmp37 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallableFor(ca_.isolate(), ExampleBuiltinForTorqueFunctionPointerType(3)).descriptor(), tmp36, parameter0, tmp17, tmp10, phi_bb13_8));
    ...
    if (block40.is_used()) {
        ...
        tmp43 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallableFor(ca_.isolate(), ExampleBuiltinForTorqueFunctionPointerType(3)).descriptor(), tmp36, parameter0, tmp17, phi_bb40_20, tmp42));
        tmp44 = kStoreFailureArrayDetached_0(state_);
        tmp45 = CodeStubAssembler(state_).SmiEqual(TNode<Smi> { tmp43 }, TNode<Smi> { tmp44 });
        ca_.Branch(tmp45, &block46, std::vector<compiler::Node*> { phi_bb40_20, phi_bb40_20, phi_bb40_20 }, &block47,
            std::vector<compiler::Node*> { phi_bb40_20, phi_bb40_20, phi_bb40_20 });
    }
