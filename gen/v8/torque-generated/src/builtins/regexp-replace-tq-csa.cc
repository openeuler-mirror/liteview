    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<FixedArray> p_matchesElements, TNode<IntPtrT> p_matchesLength, TNode<JSReceiver> p_replaceFn)
{
    ...
    if (block16.is_used()) {
        ...
        tmp18 = CodeStubAssembler(state_).Call(
            TNode<Context> { p_context }, TNode<Object> { tmp15 }, TNode<Object> { tmp16 }, TNode<Object> { p_replaceFn }, TNode<Object> { tmp17 }, TNode<Object> { tmp13 });
    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string, TNode<JSReceiver> p_replaceFn)
{
    ...
    if (block8.is_used()) {
        ca_.Bind(&block8);
        RegExpReplaceCallableNoExplicitCaptures_0(
            state_, TNode<Context> { p_context }, TNode<FixedArray> { tmp18 }, TNode<IntPtrT> { tmp15 }, TNode<String> { p_string }, TNode<JSReceiver> { p_replaceFn });
        ca_.Goto(&block10);
    }
