...
TF_BUILTIN(GetIteratorWithFeedback, CodeStubAssembler)
{
    ...
    if (block11.is_used()) {
        ...
        tmp3
            = ca_.CallStub<Object>(Builtins::CallableFor(ca_.isolate(), Builtin::kLoadIC), parameter0, parameter1, tmp2, parameter2, ca_.UncheckedCast<FeedbackVector>(parameter4));
...
TF_BUILTIN(CallIteratorWithFeedback, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        CollectCallFeedback_1(state_, TNode<Object> { parameter2 }, std::function<TNode<Object>()> { tmp2 }, TNode<Context> { parameter0 }, TNode<HeapObject> { parameter4 },
            TNode<UintPtrT> { tmp1 });
