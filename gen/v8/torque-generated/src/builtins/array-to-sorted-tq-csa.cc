...
TNode<JSArray> CopyWorkArrayToNewFastJSArray_0(
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<SortState> p_sortState, ElementsKind p_elementsKind, TNode<Smi> p_numberOfNonUndefined)
{
    ...
    if (block0.is_used()) {
        ...
        CodeStubAssembler(state_).CopyElements(
            p_elementsKind, TNode<FixedArrayBase> { tmp4 }, TNode<IntPtrT> { tmp8 }, TNode<FixedArrayBase> { tmp6 }, TNode<IntPtrT> { tmp9 }, TNode<IntPtrT> { tmp7 });
    ...
    if (block21.is_used()) {
        ...
        ca_.Branch(
            tmp17, &block29, std::vector<compiler::Node*> { phi_bb21_6, phi_bb21_6, phi_bb21_6 }, &block30, std::vector<compiler::Node*> { phi_bb21_6, phi_bb21_6, phi_bb21_6 });
    }
