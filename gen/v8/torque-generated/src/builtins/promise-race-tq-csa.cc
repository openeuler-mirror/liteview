...
TF_BUILTIN(PromiseRace, CodeStubAssembler)
{
    ...
    if (block22.is_used()) {
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch39__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp37 = IteratorBuiltinsAssembler(state_).IteratorStep(
                TNode<Context> { parameter0 }, TorqueStructIteratorRecord { TNode<JSReceiver> { tmp20 }, TNode<Object> { tmp21 } }, TNode<Map> { tmp29 }, &label38);
        }
    ...
    if (block30.is_used()) {
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch59__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp58
                = CodeStubAssembler(state_).Call(TNode<Context> { parameter0 }, TNode<Object> { tmp49 }, TNode<Object> { tmp45 }, TNode<Object> { tmp52 }, TNode<Object> { tmp55 });
        }
