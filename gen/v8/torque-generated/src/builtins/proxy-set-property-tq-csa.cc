...
TF_BUILTIN(ProxySetProperty, CodeStubAssembler)
{
    ...
    if (block30.is_used()) {
        ca_.Bind(&block30);
        ProxiesCodeStubAssembler(state_).CheckGetSetTrapResult(TNode<Context> { parameter0 }, TNode<JSReceiver> { tmp9 }, TNode<JSProxy> { parameter1 }, TNode<Name> { parameter2 },
            TNode<Object> { parameter3 }, JSProxy::AccessKind::kSet);
        CodeStubAssembler(state_).Return(parameter3);
    }
