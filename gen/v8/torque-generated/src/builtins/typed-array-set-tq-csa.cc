...
TF_BUILTIN(TypedArrayPrototypeSet, CodeStubAssembler)
{
    ...
    if (block18.is_used()) {
        ...
        tmp17 = CodeStubAssembler(state_).GetArgumentValue(TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
                                                               TNode<IntPtrT> { torque_arguments.length }, TNode<IntPtrT> { torque_arguments.actual_count } },
            TNode<IntPtrT> { tmp16 });
    ...
    if (block23.is_used()) {
        ...
        TypedArrayPrototypeSetArray_0(state_, TNode<Context> { parameter0 }, TNode<Object> { parameter1 }, TNode<JSTypedArray> { tmp0 }, TNode<UintPtrT> { tmp14 },
            TNode<Object> { tmp17 }, TNode<UintPtrT> { phi_bb23_8 }, TNode<BoolT> { phi_bb23_7 }, &label20);
    ...
    if (block24.is_used()) {
        ...
        TypedArrayPrototypeSetTypedArray_0(state_, TNode<Context> { parameter0 }, TNode<Object> { parameter1 },
            ...
            TorqueStructAttachedJSTypedArrayAndLength_0 { TNode<JSTypedArray> { tmp21 }, TNode<UintPtrT> { tmp22 } }, TNode<UintPtrT> { phi_bb24_8 }, TNode<BoolT> { phi_bb24_7 },
            &label24);
...
void TypedArrayPrototypeSetTypedArray_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_receiver,
    TorqueStructAttachedJSTypedArrayAndLength_0 p_attachedTargetAndLength, TorqueStructAttachedJSTypedArrayAndLength_0 p_attachedSourceAndLength, TNode<UintPtrT> p_targetOffset,
    TNode<BoolT> p_targetOffsetOverflowed, compiler::CodeAssemblerLabel* label_IfOffsetOutOfBounds)
{
    compiler::CodeAssembler ca_(state_);
    compiler::CodeAssembler::SourcePositionScope pos_scope(&ca_);
    compiler::CodeAssemblerParameterizedLabel<> block0(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
    ...
    if (block4.is_used()) {
        ...
        CheckIntegerIndexAdditionOverflow_0(
            state_, TNode<UintPtrT> { p_attachedSourceAndLength.length }, TNode<UintPtrT> { p_targetOffset }, TNode<UintPtrT> { p_attachedTargetAndLength.length }, &label0);
        ca_.Goto(&block5);
        if (label0.is_used()) {
            ca_.Bind(&label0);
            ca_.Goto(&block6);
    ...
    if (block11.is_used()) {
        ...
        ca_.Branch(tmp12, &block38, std::vector<compiler::Node*> {}, &block39, std::vector<compiler::Node*> {});
    }
    ...
    if (block31.is_used()) {
        ...
        ca_.Branch(tmp32, &block34, std::vector<compiler::Node*> {}, &block35, std::vector<compiler::Node*> {});
    }
    ...
    if (block35.is_used()) {
        ...
        ca_.Goto(&block36, tmp36);
    }

    TNode<BoolT> phi_bb36_19;
    if (block36.is_used()) {
        ca_.Bind(&block36, &phi_bb36_19);
        ca_.Branch(phi_bb36_19, &block32, std::vector<compiler::Node*> {}, &block33, std::vector<compiler::Node*> {});
    }

    TNode<RawPtrT> tmp37;
    if (block32.is_used()) {
        ca_.Bind(&block32);
        tmp37 = CodeStubAssembler(state_).LoadJSTypedArrayDataPtr(TNode<JSTypedArray> { p_attachedSourceAndLength.array });
        TypedArrayBuiltinsAssembler(state_).CallCRelaxedMemmove(TNode<RawPtrT> { tmp29 }, TNode<RawPtrT> { tmp37 }, TNode<UintPtrT> { tmp20 });
        ca_.Goto(&block37);
    }
    ...
    if (block33.is_used()) {
        ca_.Bind(&block33);
        tmp38 = CodeStubAssembler(state_).LoadJSTypedArrayDataPtr(TNode<JSTypedArray> { p_attachedSourceAndLength.array });
        TypedArrayBuiltinsAssembler(state_).CallCMemmove(TNode<RawPtrT> { tmp29 }, TNode<RawPtrT> { tmp38 }, TNode<UintPtrT> { tmp20 });
        ca_.Goto(&block37);
    }
    ...
    if (block39.is_used()) {
        ca_.Bind(&block39);
        tmp39 = FromConstexpr_uintptr_constexpr_IntegerLiteral_0(state_, IntegerLiteral(false, 0x0ull));
        tmp40 = CodeStubAssembler(state_).WordEqual(TNode<UintPtrT> { p_attachedSourceAndLength.length }, TNode<UintPtrT> { tmp39 });
