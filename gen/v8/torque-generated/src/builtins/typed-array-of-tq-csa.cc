...
TF_BUILTIN(TypedArrayOf, CodeStubAssembler)
{
    ...
    if (block7.is_used()) {
        ...
        tmp13 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallableFor(ca_.isolate(), ExampleBuiltinForTorqueFunctionPointerType(3)).descriptor(), tmp8, parameter0, tmp4, phi_bb7_12, tmp12));
        tmp14 = kStoreFailureArrayDetached_0(state_);
        tmp15 = CodeStubAssembler(state_).SmiEqual(TNode<Smi> { tmp13 }, TNode<Smi> { tmp14 });
        ca_.Branch(
            tmp15, &block12, std::vector<compiler::Node*> { phi_bb7_12, phi_bb7_12, phi_bb7_12 }, &block13, std::vector<compiler::Node*> { phi_bb7_12, phi_bb7_12, phi_bb7_12 });
    }
