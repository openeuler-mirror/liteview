...
TF_BUILTIN(TypedArrayPrototypeToReversed, CodeStubAssembler)
{
    ...
    if (block1.is_used()) {
        ...
        tmp12 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallableFor(ca_.isolate(), ExampleBuiltinForTorqueFunctionPointerType(1)).descriptor(), tmp4, TNode<Object>(), tmp1, tmp11));
        tmp13 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallableFor(ca_.isolate(), ExampleBuiltinForTorqueFunctionPointerType(2)).descriptor(), tmp5, parameter0, tmp2, phi_bb1_12, tmp12));
