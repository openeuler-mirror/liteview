...
TF_BUILTIN(ObjectFromEntries, CodeStubAssembler)
{
    ...
    if (block17.is_used()) {
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch15__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp13 = IteratorBuiltinsAssembler(state_).IteratorStep(
                TNode<Context> { parameter0 }, TorqueStructIteratorRecord { TNode<JSReceiver> { tmp7 }, TNode<Object> { tmp8 } }, TNode<Map> { tmp6 }, &label14);
        }
