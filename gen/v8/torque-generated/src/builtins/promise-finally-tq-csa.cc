...
TF_BUILTIN(PromiseValueThunkFinally, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        std::tie(tmp2, tmp3)
            = ContextSlot_PromiseValueThunkOrReasonContext_PromiseValueThunkOrReasonContext_JSAny_0(state_, TNode<Context> { tmp0 }, TNode<IntPtrT> { tmp1 }).Flatten();
...
TorqueStructPromiseFinallyFunctions_0 CreatePromiseFinallyFunctions_0(
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<NativeContext> p_nativeContext, TNode<JSReceiver> p_onFinally, TNode<JSReceiver> p_constructor)
{
    compiler::CodeAssembler ca_(state_);
    compiler::CodeAssembler::SourcePositionScope pos_scope(&ca_);
    compiler::CodeAssemblerParameterizedLabel<> block0(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
...
TF_BUILTIN(PromisePrototypeFinally, CodeStubAssembler)
{
    ...
    if (block18.is_used()) {
        ca_.Bind(&block18);
        std::tie(tmp20, tmp21)
            = CreatePromiseFinallyFunctions_0(state_, TNode<Context> { parameter0 }, TNode<NativeContext> { tmp3 }, TNode<JSReceiver> { tmp18 }, TNode<JSReceiver> { phi_bb8_6 })
                  .Flatten();
    ...
    if (block16.is_used()) {
        ...
        tmp22 = InvokeThen_1(
            state_, TNode<Context> { parameter0 }, TNode<NativeContext> { tmp3 }, TNode<Object> { parameter1 }, TNode<Object> { phi_bb16_8 }, TNode<Object> { phi_bb16_9 });
    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<IntPtrT> p_index)
{
    ...
    if (block6.is_used()) {
        ...
        std::tie(tmp10, tmp11)
            = ReferenceCast_JSAny_Object_0(state_, TorqueStructReference_Object_0 { TNode<Object> { tmp8 }, TNode<IntPtrT> { tmp9 }, TorqueStructUnsafe_0 {} }).Flatten();
        ca_.Goto(&block10);
    }
