    ...
    TNode<Smi> p_newLenSmi, TNode<Smi> p_actualStartSmi, TNode<Smi> p_insertCountSmi, TNode<Smi> p_actualDeleteCountSmi)
{
    ...
    if (block9.is_used()) {
        ...
        CodeStubAssembler(state_).CopyElements(
            p_kind, TNode<FixedArrayBase> { tmp4 }, TNode<IntPtrT> { tmp15 }, TNode<FixedArrayBase> { tmp21 }, TNode<IntPtrT> { tmp19 }, TNode<IntPtrT> { tmp16 });
        ca_.Goto(&block10);
    }
    ...
    compiler::CodeAssemblerLabel* label_Slow)
{
    ...
    if (block17.is_used()) {
        ca_.Bind(&block17);
        tmp19 = CopyFastPackedArrayForToSpliced_0(state_, TNode<Context> { p_context }, ElementsKind::PACKED_SMI_ELEMENTS, TNode<JSArray> { tmp6 }, TNode<Smi> { tmp0 },
    ...
    if (block20.is_used()) {
        ca_.Bind(&block20);
        tmp22 = CopyFastPackedArrayForToSpliced_0(state_, TNode<Context> { p_context }, ElementsKind::PACKED_ELEMENTS, TNode<JSArray> { tmp6 }, TNode<Smi> { tmp0 },
            TNode<Smi> { tmp2 }, TNode<Smi> { p_insertCount }, TNode<Smi> { tmp4 });
    ...
    if (block21.is_used()) {
        ca_.Bind(&block21);
        tmp23 = CopyFastPackedArrayForToSpliced_0(state_, TNode<Context> { p_context }, ElementsKind::PACKED_DOUBLE_ELEMENTS, TNode<JSArray> { tmp6 }, TNode<Smi> { tmp0 },
            TNode<Smi> { tmp2 }, TNode<Smi> { p_insertCount }, TNode<Smi> { tmp4 });
...
TF_BUILTIN(ArrayPrototypeToSpliced, CodeStubAssembler)
{
    ...
    if (block18.is_used()) {
        ...
        tmp39 = TryFastArrayToSpliced_0(state_, TNode<Context> { parameter0 },
            ...
            TNode<JSReceiver> { tmp4 }, TNode<Number> { tmp5 }, TNode<Number> { tmp30 }, TNode<Number> { phi_bb3_11 }, TNode<Smi> { phi_bb18_12 }, TNode<Number> { phi_bb18_13 },
            &label40);
