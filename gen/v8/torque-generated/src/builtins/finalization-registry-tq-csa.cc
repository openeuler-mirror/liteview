...
TF_BUILTIN(FinalizationRegistryConstructor, CodeStubAssembler)
{
    ...
    if (block5.is_used()) {
        ...
        tmp15 = ca_.UncheckedCast<Uint32T>(
            CodeStubAssembler(state_).UpdateWord32<base::BitField<bool, 0, 1, uint32_t>>(ca_.UncheckedCast<Word32T>(tmp13), ca_.UncheckedCast<Uint32T>(tmp14), true));
...
TF_BUILTIN(FinalizationRegistryRegister, CodeStubAssembler)
{
    ...
    if (block3.is_used()) {
        ...
        tmp4 = CodeStubAssembler(state_).GetArgumentValue(TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
    ...
    if (block10.is_used()) {
        ...
        tmp13 = CodeStubAssembler(state_).GetArgumentValue(TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
                                                               TNode<IntPtrT> { torque_arguments.length }, TNode<IntPtrT> { torque_arguments.actual_count } },
            TNode<IntPtrT> { tmp12 });
