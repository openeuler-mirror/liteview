    ...
    compiler::CodeAssemblerLabel* label_IfSlow)
{
    ...
    if (block22.is_used()) {
        ...
        ca_.Branch(tmp27, &block23, std::vector<compiler::Node*> {}, &block24, std::vector<compiler::Node*> {});
    }
...
TF_BUILTIN(TypedArrayPrototypeSlice, CodeStubAssembler)
{
    ...
    if (block3.is_used()) {
        ...
        tmp9 = CodeStubAssembler(state_).GetArgumentValue(TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
                                                              TNode<IntPtrT> { torque_arguments.length }, TNode<IntPtrT> { torque_arguments.actual_count } },
            TNode<IntPtrT> { tmp8 });
    ...
    if (block22.is_used()) {
        ca_.Bind(&block22);
        SlowCopy_0(
            state_, TNode<Context> { parameter0 }, TNode<JSTypedArray> { tmp1 }, TNode<JSTypedArray> { tmp18 }, TNode<UintPtrT> { phi_bb3_9 }, TNode<UintPtrT> { phi_bb20_11 });
        ca_.Goto(&block11);
    }
