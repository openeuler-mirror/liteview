...
void Method_JSPromise_SetStatus_0(compiler::CodeAssemblerState* state_, TNode<JSPromise> p_this, Promise::PromiseState p_status)
{
    ...
    if (block0.is_used()) {
        ...
        tmp3 = ca_.UncheckedCast<Smi>(ca_.BitcastWordToTaggedSigned(CodeStubAssembler(state_).UpdateWord32InWord<base::BitField<Promise::PromiseState, 1, 2, uintptr_t>>(
...
TNode<BoolT> Method_JSPromise_HasHandler_0(compiler::CodeAssemblerState* state_, TNode<JSPromise> p_this)
{
    ...
    if (block0.is_used()) {
        ...
        tmp0 = FromConstexpr_intptr_constexpr_int31_0(state_, 16);
        tmp1 = CodeStubAssembler(state_).LoadReference<Smi>(CodeStubAssembler::Reference { p_this, tmp0 });
        tmp2 = ca_.UncheckedCast<BoolT>(
            CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 3, 1, uintptr_t>>(ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp1))));
        ca_.Goto(&block2);
    }
