...
(function() {
  ...
  WebGPUAdapterName.kCompat = 1;
  WebGPUAdapterName.kSwiftShader = 2;
  WebGPUAdapterName.MIN_VALUE = 0;
  WebGPUAdapterName.MAX_VALUE = 2;
  ...
  GpuPreferences.validate = function(messageValidator, offset) {
    ...
    err = messageValidator.validateEnum(offset + codec.kStructHeaderSize + 52, DawnBackendValidationLevel);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  GpuPreferences.decode = function(decoder) {
    ...
    val.watchdogStartsBackgrounded = (packed >> 6) & 1 ? true : false;
    val.enableVulkanProtectedMemory = (packed >> 7) & 1 ? true : false;
    ...
    packed = decoder.readUint8();
    val.disableVulkanSurface = (packed >> 0) & 1 ? true : false;
    val.disableVulkanFallbackToGlForTesting = (packed >> 1) & 1 ? true : false;
    val.enableMetal = (packed >> 2) & 1 ? true : false;
    val.enableGpuBenchmarkingExtension = (packed >> 3) & 1 ? true : false;
    val.enableWebgpu = (packed >> 4) & 1 ? true : false;
    val.enableUnsafeWebgpu = (packed >> 5) & 1 ? true : false;
  ...
  GpuPreferences.encode = function(encoder, val) {
    ...
    packed |= (val.watchdogStartsBackgrounded & 1) << 6
    packed |= (val.enableVulkanProtectedMemory & 1) << 7
    encoder.writeUint8(packed);
    ...
    packed = 0;
    packed |= (val.disableVulkanSurface & 1) << 0
    packed |= (val.disableVulkanFallbackToGlForTesting & 1) << 1
    packed |= (val.enableMetal & 1) << 2
    packed |= (val.enableGpuBenchmarkingExtension & 1) << 3
    packed |= (val.enableWebgpu & 1) << 4
    packed |= (val.enableUnsafeWebgpu & 1) << 5
