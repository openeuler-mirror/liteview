    ...
    ElementsKind p_elementsKind, TNode<Smi> p_numberOfNonUndefined)
{
    ...
    if (block0.is_used()) {
        ...
        tmp3 = CodeStubAssembler(state_).AllocateFixedArray(
            p_elementsKind, TNode<IntPtrT> { tmp2 }, CodeStubAssembler::AllocationFlag::kAllowLargeObjectAllocation);
...
TF_BUILTIN(ArrayTimSortIntoCopy, CodeStubAssembler)
{
    ...
    if (block7.is_used()) {
        ...
        ca_.Branch(tmp21, &block15, std::vector<compiler::Node*> { phi_bb7_4, phi_bb7_4, phi_bb7_4 }, &block16,
            std::vector<compiler::Node*> { phi_bb7_4, phi_bb7_4, phi_bb7_4 });
    }
    ...
    if (block8.is_used()) {
        ...
        tmp31 = CopyWorkArrayToNewFastJSArray_0(
    ...
    if (block4.is_used()) {
        ca_.Bind(&block4);
        tmp32 = CopyWorkArrayToNewFastJSArray_0(
    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o, compiler::CodeAssemblerLabel* label_CastError)
{
    ...
    if (block3.is_used()) {
        ca_.Bind(&block3);
        compiler::CodeAssemblerLabel label3(&ca_);
        tmp2 = Cast_Undefined_OR_CallableApiObject_OR_CallableJSProxy_OR_JSWrappedFunction_OR_JSBoundFunction_OR_JSFunction_0(
