...
(function() {
  ...
  FilePath.encode = function(encoder, val) {
    ...
    encoder.encodeArrayPointer(codec.Uint16, val.path);
  };
  function RelativeFilePath(values) {
    this.initDefaults_();
    this.initFields_(values);
  }
  RelativeFilePath.prototype.initDefaults_ = function() {
    this.path = null;
  };
  RelativeFilePath.prototype.initFields_ = function(fields) {
    for(var field in fields) {
        if (this.hasOwnProperty(field))
          this[field] = fields[field];
    }
  ...
  RelativeFilePath.validate = function(messageValidator, offset) {
    var err;
    err = messageValidator.validateStructHeader(offset, codec.kStructHeaderSize);
    if (err !== validator.validationError.NONE)
        return err;
    ...
    if (err !== validator.validationError.NONE)
        return err;
    // validate RelativeFilePath.path
    err = messageValidator.validateArrayPointer(offset + codec.kStructHeaderSize + 0, 2, codec.Uint16, false, [0], 0);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  RelativeFilePath.encodedSize = codec.kStructHeaderSize + 8;

  RelativeFilePath.decode = function(decoder) {
    var packed;
    var val = new RelativeFilePath();
    var numberOfBytes = decoder.readUint32();
    var version = decoder.readUint32();
    val.path =
        decoder.decodeArrayPointer(codec.Uint16);
  ...
  RelativeFilePath.encode = function(encoder, val) {
    var packed;
    encoder.writeUint32(RelativeFilePath.encodedSize);
    encoder.writeUint32(0);
    encoder.encodeArrayPointer(codec.Uint16, val.path);
  };
