...
class std_mutex {
public:
    std_mutex() { ::InitializeCriticalSection(&m_mu); }
    void lock() { ::EnterCriticalSection(&m_mu); }
    void unlock() { ::LeaveCriticalSection(&m_mu); }

    CRITICAL_SECTION_XP* getLock() { return &m_mu; }
...
template <typename... Args> void std_call_once(Args&&... args) { std::call_once(std::forward<Args>(args)...); }
