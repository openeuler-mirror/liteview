...
{
    typedef HRESULT(__stdcall * PFN_UiaRaiseAutomationPropertyChangedEvent)(IRawElementProviderSimple * pProvider, PROPERTYID id, VARIANT oldValue, VARIANT newValue);
    ...
    if (!s_is_init) {
        HMODULE mod_handle = LoadLibraryW(L"UIAUTOMATIONCORE.dll");
        if (mod_handle)
            s_UiaRaiseAutomationPropertyChangedEvent = (PFN_UiaRaiseAutomationPropertyChangedEvent)GetProcAddress(mod_handle, "UiaRaiseAutomationPropertyChangedEvent");
        s_is_init = TRUE;
    }
...
inline HRESULT UiaRaiseStructureChangedEventXp(IRawElementProviderSimple* pProvider, StructureChangeType structureChangeType, int* pRuntimeId, int cRuntimeIdLen)
{
