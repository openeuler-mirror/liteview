...
{
    typedef BOOL(__stdcall * PFN_GetProductInfo)(DWORD dwOSMajorVersion, DWORD dwOSMinorVersion, DWORD dwSpMajorVersion, DWORD dwSpMinorVersion, PDWORD pdwReturnedProductType);
...
inline LONG QueryDisplayConfigXp(UINT32 flags, UINT32* numPathArrayElements, void* pathArray, UINT32* numModeInfoArrayElements, void* modeInfoArray, void* currentTopologyId)
{
...
inline HRESULT SetCurrentProcessExplicitAppUserModelIDXp(PCWSTR AppID)
{
    ...
    if (!s_is_init) {
        ...
        s_SetCurrentProcessExplicitAppUserModelIDXp = (PFN_SetCurrentProcessExplicitAppUserModelID)GetProcAddress(handle, "SetCurrentProcessExplicitAppUserModelID");
        s_is_init = TRUE;
    }
...
inline HRESULT GetCurrentProcessExplicitAppUserModelIDXp(PWSTR* AppID)
{
    ...
    if (!s_is_init) {
        HMODULE handle = GetModuleHandle(L"shell32.dll");
        s_GetCurrentProcessExplicitAppUserModelID = (PFN_GetCurrentProcessExplicitAppUserModelID)(GetProcAddress(handle, "GetCurrentProcessExplicitAppUserModelID"));
        s_is_init = TRUE;
    }
...
{
    typedef BOOL(WINAPI * PFN_SetFileInformationByHandle)(HANDLE file_handle, FILE_INFO_BY_HANDLE_CLASS file_information_class, LPVOID file_information, DWORD buffer_size);
    static PFN_SetFileInformationByHandle s_SetFileInformationByHandle;
    static BOOL s_is_init = FALSE;

    typedef LONG(WINAPI * PFN_NtSetInformationFile)(HANDLE FileHandle, XP_IO_STATUS_BLOCK * IoStatusBlock, PVOID FileInformation, ULONG Length, DWORD FileInformationClass);
