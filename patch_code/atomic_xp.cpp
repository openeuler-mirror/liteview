...
#if defined(_MSC_VER) //|| defined(__ARMEL__) 
...
#if defined(__AARCH64EL__) || defined(_M_ARM64) || defined(_M_X64) || defined(__x86_64__)
    ...
    do {
        ...
        if (2 == size) {
            ...
            *(int32_t*)ret = (int32_t)retv;
            ...
            int64_t retv = _InterlockedExchange64((long long volatile*)ptr, *(int64_t*)val);
#else
            int64_t retv = _InterlockedExchange64((long volatile*)ptr, *(int64_t*)val);
#endif
            *(int64_t*)ret = (int64_t)retv;
            return;
        }
#else
#error "32 bit not impl !"
#endif
    } while (false);
