...
public:
    ...
    MojoResult CreateTrap(MojoHandle* handle) { return MojoCreateTrap(&Notify, nullptr, handle); }
...
private:
    ...
    public:
        ...
        void SetCancelCallback(base::OnceClosure cancel_callback) { cancel_callback_ = std::move(cancel_callback); }
    ...
    static void Notify(const MojoTrapEvent* event) { reinterpret_cast<NotificationContext*>(event->trigger_context)->Notify(*event); }
};
...
void ExpectNoNotification(const MojoTrapEvent* event) { NOTREACHED(); }

void ExpectOnlyCancel(const MojoTrapEvent* event) { EXPECT_EQ(event->result, MOJO_RESULT_CANCELLED); }
...
TEST_F(TrapTest, InvalidArguments)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoAddTrigger(t, buffer_handle, MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, 0, nullptr));
...
TEST_F(TrapTest, TrapDataPipeConsumerReadable)
{
    ...
    MojoHandle t;
    EXPECT_EQ(MOJO_RESULT_OK, helper.CreateTrap(&t));
    EXPECT_EQ(MOJO_RESULT_OK, MojoAddTrigger(t, consumer, MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, readable_consumer_context, nullptr));
    EXPECT_EQ(MOJO_RESULT_OK, MojoArmTrap(t, nullptr, nullptr, nullptr));
...
TEST_F(TrapTest, TrapDataPipeConsumerNewDataReadable)
{
    ...
    MojoHandle t;
    EXPECT_EQ(MOJO_RESULT_OK, helper.CreateTrap(&t));
    EXPECT_EQ(MOJO_RESULT_OK, MojoAddTrigger(t, consumer, MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, new_data_context, nullptr));
    EXPECT_EQ(MOJO_RESULT_OK, MojoArmTrap(t, nullptr, nullptr, nullptr));
...
TEST_F(TrapTest, TrapDataPipeProducerWritable)
{
    ...
    MojoHandle t;
    EXPECT_EQ(MOJO_RESULT_OK, helper.CreateTrap(&t));
    EXPECT_EQ(MOJO_RESULT_OK, MojoAddTrigger(t, producer, MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, writable_producer_context, nullptr));
...
MojoHandle RandomHandle(MojoHandle* handles, size_t size) { return handles[base::RandInt(0, static_cast<int>(size) - 1)]; }
