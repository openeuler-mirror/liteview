...
MojoResult MojoWriteMessage(MojoHandle pipe, const void* bytes, uint32_t num_bytes, const MojoHandle* handles, uint32_t num_handles, MojoWriteMessageFlags flags)
{
...
DEFINE_TEST_CLIENT_WITH_PIPE(EchoEcho, MultiprocessMessagePipeTest, h)
{
    ...
    for (;; rv = (rv + 1) % 100) {
        ...
        CHECK_EQ(MojoWriteMessage(h, write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    }
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, Basic)
{
    int exit_code = RunTestClientAndGetExitCode("EchoEcho", [&](MojoHandle h) {
        ...
        CHECK_EQ(MojoWriteMessage(h, quitquitquit.data(), static_cast<uint32_t>(quitquitquit.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    });
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, QueueMessages)
{
    ...
    int exit_code = RunTestClientAndGetExitCode("EchoEcho", [&](MojoHandle h) {
        for (size_t i = 0; i < kNumMessages; i++) {
            ...
            ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
        }
        ...
        const std::string quitquitquit("quitquitquit");
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, quitquitquit.data(), static_cast<uint32_t>(quitquitquit.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
...
TEST_F(MultiprocessMessagePipeTest, SharedBufferPassing)
{
    RunTestClient("CheckSharedBuffer", [&](MojoHandle h) {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, &go1[0], static_cast<uint32_t>(go1.size()), &handles[0], std::size(handles), MOJO_WRITE_MESSAGE_FLAG_NONE));
...
TEST_P(MultiprocessMessagePipeTestWithPipeCount, PlatformHandlePassing)
{
    ...
    RunTestClient("CheckPlatformHandleFile", [&](MojoHandle h) {
        ...
        ASSERT_EQ(
            MOJO_RESULT_OK, MojoWriteMessage(h, message, static_cast<uint32_t>(strlen(message)), &handles[0], static_cast<uint32_t>(handles.size()), MOJO_WRITE_MESSAGE_FLAG_NONE));
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckMessagePipe, MultiprocessMessagePipeTest, h)
{
    ...
    CHECK_EQ(MojoWriteMessage(handles[0], write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    MojoClose(handles[0]);
    return 0;
}
...
DEFINE_TEST_CLIENT_WITH_PIPE(DataPipeConsumer, MultiprocessMessagePipeTest, h)
{
    ...
    CHECK_EQ(hss.satisfiable_signals,
        MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE | MOJO_HANDLE_SIGNAL_QUOTA_EXCEEDED);
