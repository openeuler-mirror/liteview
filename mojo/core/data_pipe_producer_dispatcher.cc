...
{
    scoped_refptr<DataPipeProducerDispatcher> producer = new DataPipeProducerDispatcher(node_controller, control_port, std::move(shared_ring_buffer), options, pipe_id);
...
Dispatcher::Type DataPipeProducerDispatcher::GetType() const { return Type::DATA_PIPE_PRODUCER; }
...
MojoResult DataPipeProducerDispatcher::EndWriteData(uint32_t num_bytes_written)
{
    ...
    if (num_bytes_written > available_capacity_ || num_bytes_written % options_.element_num_bytes != 0 || write_offset_ + num_bytes_written > options_.capacity_num_bytes) {
    ...
    const void* data, size_t num_bytes, const ports::PortName* ports, size_t num_ports, PlatformHandle* handles, size_t num_handles)
{
    ...
    scoped_refptr<DataPipeProducerDispatcher> dispatcher = new DataPipeProducerDispatcher(node_controller, port, std::move(ring_buffer), state->options, state->pipe_id);
...
DataPipeProducerDispatcher::DataPipeProducerDispatcher(NodeController* node_controller, const ports::PortRef& control_port, base::UnsafeSharedMemoryRegion shared_ring_buffer,
...
DataPipeProducerDispatcher::~DataPipeProducerDispatcher() { DCHECK(is_closed_ && !in_transit_ && !shared_ring_buffer_.IsValid() && !ring_buffer_mapping_.IsValid()); }
    ...
    if (rv != ports::OK || !port_status.receiving_messages) {
        ...
        do {
            ...
            if (message_event) {
                ...
                if (!base::CheckAdd(available_capacity_, m->num_bytes).AssignIfValid(&new_available_capacity) || new_available_capacity > options_.capacity_num_bytes) {
