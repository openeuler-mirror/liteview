...
public:
    ...
    {
        if (base::StartsWith(test_info.name(), kManualTestPrefix, base::CompareCase::SENSITIVE) && !base::CommandLine::ForCurrentProcess()->HasSwitch(kRunManualTestsFlag)) {
            GTEST_SKIP();
        }
    }
};
...
public:
    ...
    void OnTestEnd(const testing::TestInfo& test_info) override { mojo_support_.reset(); }
