...
public:
    ...
    static MojoMessageHandle MakeMessageHandle(std::unique_ptr<TestMessageBase> message)
    {
        ...
        rv = MojoSetMessageContext(
            handle, reinterpret_cast<uintptr_t>(message.release()), &TestMessageBase::SerializeMessageContext, &TestMessageBase::DestroyMessageContext, nullptr);
        DCHECK_EQ(MOJO_RESULT_OK, rv);
...
private:
    static void SerializeMessageContext(MojoMessageHandle message_handle, uintptr_t context)
    {
        ...
        MojoResult rv = MojoAppendMessageData(
            message_handle, base::checked_cast<uint32_t>(num_bytes), handles.data(), base::checked_cast<uint32_t>(num_handles), &options, &buffer, &buffer_size);
        DCHECK_EQ(MOJO_RESULT_OK, rv);
    ...
    static void DestroyMessageContext(uintptr_t context) { delete reinterpret_cast<TestMessageBase*>(context); }
};
...
private:
    // TestMessageBase:
    void GetSerializedSize(size_t* num_bytes, size_t* num_handles) override { NOTREACHED(); }
...
public:
    ...
    void AddMessagePipe(mojo::ScopedMessagePipeHandle handle) { handles_.emplace_back(std::move(handle)); }
...
private:
    ...
    void SerializePayload(void* buffer) override { std::copy(contents_.begin(), contents_.end(), static_cast<char*>(buffer)); }
...
TEST_F(MessageTest, DropUnreadLocalMessageWithContext)
{
    ...
    auto message = std::make_unique<SimpleMessage>(kTestMessageWithContext1, base::BindOnce([](bool* was_destroyed) { *was_destroyed = true; }, &message_was_destroyed));
...
TEST_F(MessageTest, GetMessageDataWithHandles)
{
    ...
    void* buffer;
    uint32_t buffer_size;
    ASSERT_EQ(MOJO_RESULT_OK, MojoAppendMessageData(message_handle, static_cast<uint32_t>(kTestMessage.size()), h, 2, &append_data_options, &buffer, &buffer_size));
...
{
    bool message_was_destroyed = false;
    std::unique_ptr<TestMessageBase> message = std::make_unique<NeverSerializedMessage>(base::BindOnce([](bool* was_destroyed) { *was_destroyed = true; }, &message_was_destroyed));
...
TEST_F(MessageTest, ForceSerializeMessageWithContext)
{
    ...
    message = std::make_unique<SimpleMessage>(kTestMessageWithContext1, base::BindOnce([](bool* was_destroyed) { *was_destroyed = true; }, &message_was_destroyed));
    MessagePipe pipe1;
