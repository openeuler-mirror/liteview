    ...
    void** out_user_payload)
{
    ...
    if (*out_message) {
        ...
        original_payload_size = original_message->payload_size() - (static_cast<char*>(original_user_payload) - static_cast<char*>(original_message->mutable_payload()));
    }
    ...
    if (num_new_dispatchers > 0) {
        ...
        for (size_t i = 0; i < num_new_dispatchers; ++i) {
            ...
            if (g_always_fail_handle_serialization
                || !d->EndSerialize(static_cast<void*>(new_dispatcher_data), event->ports() + port_index, !handles.empty() ? handles.data() + handle_index : nullptr)) {
                fail = true;
                break;
            }
...
void IncrementMessageCount() { g_message_count.fetch_add(1, std::memory_order_relaxed); }

void DecrementMessageCount() { g_message_count.fetch_add(-1, std::memory_order_relaxed); }

class MessageMemoryDumpProvider : public base::trace_event::MemoryDumpProvider {
public:
    MessageMemoryDumpProvider() { base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "MojoMessages", nullptr); }
    ...
    ~MessageMemoryDumpProvider() override { base::trace_event::MemoryDumpManager::GetInstance()->UnregisterDumpProvider(this); }
...
private:
    ...
    bool OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd) override
    {
        ...
        dump->AddScalar(
            base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, g_message_count.load(std::memory_order_relaxed));
        return true;
    }
};

void EnsureMemoryDumpProviderExists() { [[maybe_unused]] static base::NoDestructor<MessageMemoryDumpProvider> provider; }
    ...
    uint32_t num_bytes, const Dispatcher::DispatcherInTransit* dispatchers, uint32_t num_dispatchers, std::unique_ptr<ports::UserMessageEvent>* out_event)
{
    ...
    MojoResult rv = CreateOrExtendSerializedEventMessage(event.get(), num_bytes, num_bytes, dispatchers, num_dispatchers, &channel_message, &header, &header_size, &user_payload);
...
MojoResult UserMessageImpl::AppendData(uint32_t additional_payload_size, const MojoHandle* handles, uint32_t num_handles)
{
    ...
    if (!IsSerialized()) {
        ...
        Channel::MessagePtr channel_message;
        MojoResult rv = CreateOrExtendSerializedEventMessage(message_event_, additional_payload_size, std::max(additional_payload_size, kMinimumPayloadBufferSize),
...
MojoResult UserMessageImpl::CommitSize()
{
    ...
    if (!pending_handle_attachments_.empty()) {
        CreateOrExtendSerializedEventMessage(message_event_, user_payload_size_, user_payload_size_, pending_handle_attachments_.data(), pending_handle_attachments_.size(),
...
MojoResult UserMessageImpl::ExtractSerializedHandles(ExtractBadHandlePolicy bad_handle_policy, MojoHandle* handles)
{
    ...
    for (size_t i = 0; i < header->num_dispatchers; ++i) {
        ...
        dispatchers[i].dispatcher
            = Dispatcher::Deserialize(type, dispatcher_data, dh.num_bytes, message_event_->ports() + port_index, dh.num_ports, out_handles, dh.num_platform_handles);
...
// static
void UserMessageImpl::FailHandleSerializationForTesting(bool fail) { g_always_fail_handle_serialization = fail; }
