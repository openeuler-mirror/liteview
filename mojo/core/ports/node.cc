...
void GenerateRandomPortName(PortName* name) { *name = g_name_generator.Get().GenerateRandomPortName(); }
...
int Node::InitializePort(const PortRef& port_ref, const NodeName& peer_node_name, const PortName& peer_port_name, const NodeName& prev_node_name, const PortName& prev_port_name)
{
    {
    ...
    *message = nullptr;
    ...
    ScopedEvent ack_event;
    {
        ...
        if (*message && (*message)->sequence_num() == port->sequence_num_to_acknowledge) {
            peer_node_name = port->peer_node_name;
            ack_event
                = std::make_unique<UserMessageReadAckEvent>(port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, port->sequence_num_to_acknowledge);
        }
        if (*message) {
...
int Node::SetAcknowledgeRequestInterval(const PortRef& port_ref, uint64_t sequence_num_acknowledge_interval)
{
    ...
    delegate_->ForwardEvent(peer_node_name, std::make_unique<UserMessageReadAckRequestEvent>(peer_port_name, port_ref.name(), sequence_num, sequence_num_to_request_ack));
    return OK;
}
...
int Node::AcceptEvent(const NodeName& from_node, ScopedEvent event)
{
    ...
    // Before processing the event, verify the sender and sequence number.
    {
        ...
        if (!port->IsNextEvent(from_node, *event)) {
            DVLOG(2) << "Buffering event (type " << event->type() << "): " << event->from_port() << "@" << from_node << " => " << port_ref.name() << "@" << name_
                     << " seq nr: " << event->control_sequence_num() << " / " << port->next_control_sequence_num_to_receive << ", want " << port->prev_port_name << "@"
                     << port->prev_node_name;
    ...
    while (true) {
        ...
        NodeName next_from_node;
        {
            ...
            if (next_event) {
                DVLOG(2) << "Handling buffered event (type " << next_event->type() << "): " << next_event->from_port() << "@" << next_from_node << " => " << port_ref.name() << "@"
                         << name_ << " seq nr: " << next_event->control_sequence_num() << " / " << port->next_control_sequence_num_to_receive;
            }
        }
        if (!next_event)
            break;
...
int Node::MergePorts(const PortRef& port_ref, const NodeName& destination_node_name, const PortName& destination_port_name)
{
    ...
    delegate_->ForwardEvent(pending_update_event.receiver,
        std::make_unique<UpdatePreviousPeerEvent>(
            pending_update_event.port, pending_update_event.from_port, pending_update_event.sequence_num, pending_update_event.new_prev_node, pending_update_event.new_prev_port));
#endif
    ...
    delegate_->ForwardEvent(
        destination_node_name, std::make_unique<MergePortEvent>(destination_port_name, kInvalidPortName, kInvalidSequenceNum, new_port_name, new_port_descriptor));
    return OK;
}
...
int Node::OnPortAccepted(const PortRef& port_ref, std::unique_ptr<PortAcceptedEvent> event)
{
    ...
    {
        SinglePortLocker locker(&port_ref);
        DVLOG(2) << "PortAccepted at " << port_ref.name() << "@" << name_ << " pointing to " << locker.port()->peer_port_name << "@" << locker.port()->peer_node_name;
    }
#endif
...
int Node::OnObserveProxy(const PortRef& port_ref, std::unique_ptr<ObserveProxyEvent> event)
{
    ...
    DVLOG(2) << "ObserveProxy at " << port_ref.name() << "@" << name_ << ", proxy at " << event->proxy_port_name() << "@" << event->proxy_node_name() << " pointing to "
             << event->proxy_target_port_name() << "@" << event->proxy_target_node_name();
    ...
    NodeName event_target_node;
    {
        ...
        if (port->peer_node_name == event->proxy_node_name() && port->peer_port_name == event->proxy_port_name()) {
            if (port->state == Port::kReceiving) {
                ...
                event_to_forward = std::make_unique<ObserveProxyAckEvent>(event->proxy_port_name(), port_ref.name(), sequence_num, port->next_sequence_num_to_send - 1);
                peer_changed = true;
                DVLOG(2) << "Forwarding ObserveProxyAck from " << event->port_name() << "@" << name_ << " to " << event->proxy_port_name() << "@" << event_target_node;
            } else {
                ...
                port->send_on_proxy_removal = std::make_unique<std::pair<NodeName, ScopedEvent>>(
                    event->proxy_node_name(), std::make_unique<ObserveProxyAckEvent>(event->proxy_port_name(), port_ref.name(), kInvalidSequenceNum, kInvalidSequenceNum));
            }
        } else {
...
int Node::OnObserveClosure(const PortRef& port_ref, std::unique_ptr<ObserveClosureEvent> event)
{
    ...
    bool erase_port = false;
    {
        ...
        DVLOG(2) << "ObserveClosure at " << port_ref.name() << "@" << name_ << " (state=" << port->state << ") pointing to " << port->peer_port_name << "@" << port->peer_node_name
    ...
    std::unique_ptr<Event> event_to_send;
    {
        ...
        if (port->state == Port::kProxying) {
            ...
            if (current_sequence_num >= event->sequence_num_to_acknowledge()) {
                ...
                event_to_send = std::make_unique<UserMessageReadAckEvent>(port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, current_sequence_num);
    ...
    if (port_ref.is_valid()) {
        ...
        if (port->sequence_num_acknowledge_interval && !port->peer_closed) {
            ...
            ack_request_event = std::make_unique<UserMessageReadAckRequestEvent>(
                port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, port->last_sequence_num_acknowledged + port->sequence_num_acknowledge_interval);
...
int Node::MergePortsInternal(const PortRef& port0_ref, const PortRef& port1_ref, bool allow_close_on_bad_state)
{
    ...
    uint64_t original_sequence_number[2];
    {
        ...
        if (port0->state != Port::kReceiving || port1->state != Port::kReceiving || (port0->peer_node_name == name_ && port0->peer_port_name == port1_ref.name())
...
void Node::ConvertToProxy(Port* port, const NodeName& to_node_name, PortName* port_name, Event::PortDescriptor* port_descriptor, PendingUpdatePreviousPeer* pending_update)
{
...
int Node::AcceptPort(const PortName& port_name, const Event::PortDescriptor& port_descriptor)
{
    ...
    delegate_->ForwardEvent(port_descriptor.referring_node_name, std::make_unique<PortAcceptedEvent>(port_descriptor.referring_port_name, kInvalidPortName, kInvalidSequenceNum));
    return OK;
}
...
void Node::InitiateProxyRemoval(const PortRef& port_ref)
{
    ...
    delegate_->ForwardEvent(
        peer_node_name, std::make_unique<ObserveProxyEvent>(peer_port_name, port_ref.name(), sequence_num, name_, port_ref.name(), peer_node_name, peer_port_name));
}
...
void Node::DestroyAllPortsWithPeer(const NodeName& node_name, const PortName& port_name)
{
    ...
    for (const auto& proxy_name : dead_proxies_to_broadcast) {
        ...
        delegate_->BroadcastEvent(
            std::make_unique<ObserveProxyEvent>(kInvalidPortName, kInvalidPortName, kInvalidSequenceNum, name_, proxy_name, kInvalidNodeName, kInvalidPortName));
...
void Node::MaybeForwardAckRequest(const PortRef& port_ref)
{
    ...
    ScopedEvent ack_request_event;
    {
        ...
        peer_node_name = port->peer_node_name;
        ack_request_event
            = std::make_unique<UserMessageReadAckRequestEvent>(port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, port->sequence_num_to_acknowledge);
...
void Node::MaybeResendAck(const PortRef& port_ref)
{
    ...
    ScopedEvent ack_event;
    {
        ...
        peer_node_name = port->peer_node_name;
        ack_event = std::make_unique<UserMessageReadAckEvent>(port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, last_sequence_num_read);
    }
