...
#pragma pack(pop)
    ...
    template <typename T> static std::unique_ptr<T> Cast(ScopedEvent* event) { return base::WrapUnique(static_cast<T*>(event->release())); }
    ...
    void set_control_sequence_num(uint64_t control_sequence_num) { control_sequence_num_ = control_sequence_num; }
...
public:
    ObserveProxyEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const NodeName& proxy_node_name, const PortName& proxy_port_name,
        const NodeName& proxy_target_node_name, const PortName& proxy_target_port_name);
    ...
    const NodeName& proxy_target_node_name() const { return proxy_target_node_name_; }
    const PortName& proxy_target_port_name() const { return proxy_target_port_name_; }
...
public:
    ...
    uint64_t last_sequence_num() const { return last_sequence_num_; }
    void set_last_sequence_num(uint64_t last_sequence_num) { last_sequence_num_ = last_sequence_num; }
...
public:
    MergePortEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const PortName& new_port_name, const PortDescriptor& new_port_descriptor);
    ...
    const PortDescriptor& new_port_descriptor() const { return new_port_descriptor_; }
...
public:
    ...
    uint64_t sequence_num_acknowledged() const { return sequence_num_acknowledged_; }
...
public:
    UpdatePreviousPeerEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const NodeName& new_node_name, const PortName& new_port_name);
