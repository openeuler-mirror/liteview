...
bool MessageEquals(const ScopedMessage& message, const base::StringPiece& s) { return message->GetMessage<TestMessage>()->payload() == s; }
...
public:
    ...
    int SendStringMessage(const PortRef& port, const std::string& s) { return node_.SendUserMessage(port, NewUserMessageEvent(s, 0)); }
    ...
    int SendStringMessageWithPort(const PortRef& port, const std::string& s, const PortRef& sent_port) { return SendStringMessageWithPort(port, s, sent_port.name()); }
    ...
    bool ReadMessage(const PortRef& port, ScopedMessage* message) { return node_.GetMessage(port, message, nullptr) == OK && *message; }
    ...
    void BroadcastEvent(ScopedEvent event) override { router_->BroadcastEvent(this, std::move(event)); }
