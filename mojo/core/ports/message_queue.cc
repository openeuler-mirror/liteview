...
inline bool operator<(const std::unique_ptr<UserMessageEvent>& a, const std::unique_ptr<UserMessageEvent>& b) { return a->sequence_num() > b->sequence_num(); }
...
bool MessageQueue::HasNextMessage() const { return !heap_.empty() && heap_[0]->sequence_num() == next_sequence_num_; }
