...
{
    scoped_refptr<DataPipeConsumerDispatcher> consumer = new DataPipeConsumerDispatcher(node_controller, control_port, std::move(shared_ring_buffer), options, pipe_id);
...
Dispatcher::Type DataPipeConsumerDispatcher::GetType() const { return Type::DATA_PIPE_CONSUMER; }
    ...
    const void* data, size_t num_bytes, const ports::PortName* ports, size_t num_ports, PlatformHandle* handles, size_t num_handles)
{
    ...
    scoped_refptr<DataPipeConsumerDispatcher> dispatcher = new DataPipeConsumerDispatcher(node_controller, port, std::move(ring_buffer), state->options, state->pipe_id);
...
DataPipeConsumerDispatcher::DataPipeConsumerDispatcher(NodeController* node_controller, const ports::PortRef& control_port, base::UnsafeSharedMemoryRegion shared_ring_buffer,
...
DataPipeConsumerDispatcher::~DataPipeConsumerDispatcher() { DCHECK(is_closed_ && !shared_ring_buffer_.IsValid() && !ring_buffer_mapping_.IsValid() && !in_transit_); }
    ...
    if (rv != ports::OK || !port_status.receiving_messages) {
        ...
        do {
            ...
            if (message_event) {
                ...
                DVLOG(1) << "Data pipe consumer " << pipe_id_ << " is aware that " << m->num_bytes << " bytes were written. [control_port=" << control_port_.name() << "]";
