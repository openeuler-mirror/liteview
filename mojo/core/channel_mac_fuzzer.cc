...
public:
    ...
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner() { return io_task_executor_.task_runner(); }
...
DEFINE_BINARY_PROTO_FUZZER(mojo_fuzzer::ChannelMac proto)
{
    ...
    if (!proto.fuzz_handshake()) {
        peer_channel = mojo::core::Channel::Create(
            &delegate, mojo::core::ConnectionParams(std::move(other_endpoint)), mojo::core::Channel::HandlePolicy::kAcceptHandles, environment.io_task_runner());
        peer_channel->Start();
    }
