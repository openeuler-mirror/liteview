...
Watch::Watch(
    const scoped_refptr<WatcherDispatcher>& watcher, const scoped_refptr<Dispatcher>& dispatcher, uintptr_t context, MojoHandleSignals signals, MojoTriggerCondition condition)
    : watcher_(watcher)
...
void Watch::Cancel() { RequestContext::current()->AddWatchCancelFinalizer(this); }
...
#if DCHECK_IS_ON()
void Watch::AssertWatcherLockAcquired() const { watcher_->lock_.AssertAcquired(); }
#endif
