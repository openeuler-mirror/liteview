...
struct UpgradeOfferMessage {
    ...
    static bool IsValidVersion(int version) { return (version == kEventFdNotifier || version == kEventFdZeroWakeNotifier); }

    int version = kDefaultVersion;
    int num_pages = kDefaultPages;
};

constexpr size_t RoundUpToWordBoundary(size_t size) { return base::bits::AlignUp(size, sizeof(void*)); }
...
public:
    ...
    base::ScopedFD take_dup() { return base::ScopedFD(HANDLE_EINTR(dup(fd_.get()))); }
...
ChannelLinux::ChannelLinux(Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
...
bool ChannelLinux::OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    ...
    case Message::MessageType::UPGRADE_OFFER: {
        ...
        if (msg->version == UpgradeOfferMessage::kEventFdNotifier || msg->version == UpgradeOfferMessage::kEventFdZeroWakeNotifier) {
            ...
            read_notifier = EventFDNotifier::CreateReadNotifier(handles[1].TakeFD(), base::BindRepeating(&ChannelLinux::SharedMemReadReady, this), io_task_runner_, zero_on_wake);
        }
    ...
    if (read_buffer_->TryLockForReading()) {
        ...
        do {
            ...
            while (bytes_read - data_offset > 0) {
                ...
                DispatchResult result = TryDispatchMessage(base::make_span(reinterpret_cast<char*>(read_buf_.data() + data_offset), bytes_read - data_offset), &read_size_hint);
...
void ChannelLinux::StartOnIOThread() { ChannelPosix::StartOnIOThread(); }
