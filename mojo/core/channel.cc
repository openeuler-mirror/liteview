...
static_assert(
    offsetof(Channel::Message::LegacyHeader, num_bytes) == offsetof(Channel::Message::Header, num_bytes), "num_bytes should be at the same offset in both Header structs.");
...
struct IpczMessage : public Channel::Message {
    ...
    void SetHandles(std::vector<PlatformHandleInTransit>) override { NOTREACHED(); }
    std::vector<PlatformHandleInTransit> TakeHandles() override { return std::move(handles_); }
...
struct TrivialMessage : public Channel::Message {
    ...
    void* mutable_data() const override { return const_cast<uint8_t*>(&data_[0]); }
...
// static
Channel::MessagePtr Channel::Message::CreateMessage(size_t payload_size, size_t max_handles) { return CreateMessage(payload_size, payload_size, max_handles); }
...
// static
void Channel::set_use_trivial_messages(bool use_trivial_messages) { g_use_trivial_messages = use_trivial_messages; }
...
size_t Channel::Message::num_handles() const { return is_legacy_message() ? legacy_header()->num_handles : header()->num_handles; }

bool Channel::Message::has_handles() const { return (is_legacy_message() ? legacy_header()->num_handles : header()->num_handles) > 0; }

bool Channel::Message::is_legacy_message() const { return legacy_header()->message_type == MessageType::NORMAL_LEGACY; }

Channel::Message::LegacyHeader* Channel::Message::legacy_header() const { return reinterpret_cast<LegacyHeader*>(mutable_data()); }
...
std::vector<PlatformHandleInTransit> ComplexMessage::TakeHandles() { return std::move(handle_vector_); }

size_t ComplexMessage::NumHandlesForTransit() const { return handle_vector_.size(); }
...
void TrivialMessage::SetHandles(std::vector<PlatformHandle> new_handles) { CHECK(new_handles.empty()); }

void TrivialMessage::SetHandles(std::vector<PlatformHandleInTransit> new_handles) { CHECK(new_handles.empty()); }

std::vector<PlatformHandleInTransit> TrivialMessage::TakeHandles() { return std::vector<PlatformHandleInTransit>(); }

size_t TrivialMessage::NumHandlesForTransit() const { return 0; }
...
public:
    ...
    const char* occupied_bytes() const { return data_.get() + num_discarded_bytes_; }

    size_t num_occupied_bytes() const { return num_occupied_bytes_ - num_discarded_bytes_; }
...
bool Channel::Delegate::IsIpczTransport() const { return false; }
...
bool Channel::OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles) { return false; }
...
// static
MOJO_SYSTEM_IMPL_EXPORT bool Channel::SupportsChannelUpgrade() { return false; }
