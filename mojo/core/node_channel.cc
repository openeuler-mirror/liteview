...
template <typename DataType> Channel::MessagePtr CreateMessage(MessageType type, size_t payload_size, size_t num_handles, DataType** out_data, size_t capacity = 0)
{
...
void NodeChannel::SendChannelMessage(Channel::MessagePtr message) { WriteChannelMessage(std::move(message)); }
...
NodeChannel::~NodeChannel() { ShutDown(); }
    ...
    case MessageType::RELAY_EVENT_MESSAGE: {
        ...
        if (GetMessagePayload(payload, payload_size, &data)) {
            ...
            Channel::MessagePtr message
                = Channel::Message::Deserialize(message_start, payload_size - sizeof(Header) - sizeof(data), Channel::HandlePolicy::kAcceptHandles, from_process);
            if (!message) {
...
uint64_t NodeChannel::RemoteCapabilities() const { return remote_capabilities_; }

bool NodeChannel::HasRemoteCapability(const uint64_t capability) const { return (remote_capabilities_ & capability) == capability; }

void NodeChannel::SetRemoteCapabilities(const uint64_t capabilities) { remote_capabilities_ |= capabilities; }

uint64_t NodeChannel::LocalCapabilities() const { return local_capabilities_; }

bool NodeChannel::HasLocalCapability(const uint64_t capability) const { return (local_capabilities_ & capability) == capability; }
