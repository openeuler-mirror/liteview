...
#if BUILDFLAG(IS_NACL)
template <typename T> void GenerateRandomName(T* out) { base::RandBytes(out, sizeof(T)); }
#else
template <typename T> void GenerateRandomName(T* out) { crypto::RandBytes(out, sizeof(T)); }
#endif
...
ports::ScopedEvent DeserializeEventMessage(const ports::NodeName& from_node, Channel::MessagePtr channel_message)
{
    ...
    auto message = UserMessageImpl::CreateFromChannelMessage(message_event.get(), std::move(channel_message), static_cast<uint8_t*>(data) + event_size, size - event_size);
    if (!message)
        return nullptr;
...
private:
    ...
    ~ThreadDestructionObserver() override { base::CurrentThread::Get()->RemoveDestructionObserver(this); }
...
ConnectionParams CreateSyncNodeConnectionParams(
    const base::Process& target_process, ConnectionParams connection_params, const ProcessErrorCallback& process_error_callback, Channel::HandlePolicy& handle_policy)
{
    ConnectionParams node_connection_params;
    const bool is_untrusted_process = connection_params.is_untrusted_process();

    // BrokerHost owns itself.
    BrokerHost* broker_host = new BrokerHost(target_process.IsValid() ? target_process.Duplicate() : base::Process(), std::move(connection_params), process_error_callback);
    ...
    const std::vector<std::pair<std::string, ports::PortRef>>& attached_ports, const ProcessErrorCallback& process_error_callback)
{
    ...
    io_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&NodeController::SendBrokerClientInvitationOnIOThread, base::Unretained(this), std::move(target_process), std::move(connection_params), temporary_node_name,
...
void NodeController::AcceptBrokerClientInvitation(ConnectionParams connection_params)
{
    ...
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&NodeController::AcceptBrokerClientInvitationOnIOThread, base::Unretained(this), std::move(connection_params), std::move(broker_host_handle)));
}
...
{
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&NodeController::ConnectIsolatedOnIOThread, base::Unretained(this), std::move(connection_params), port, std::string(connection_name)));
}

void NodeController::SetPortObserver(const ports::PortRef& port, scoped_refptr<PortObserver> observer) { node_->SetUserData(port, std::move(observer)); }
...
int NodeController::SendUserMessage(const ports::PortRef& port, std::unique_ptr<ports::UserMessageEvent> message) { return node_->SendUserMessage(port, std::move(message)); }
...
int NodeController::MergeLocalPorts(const ports::PortRef& port0, const ports::PortRef& port1) { return node_->MergeLocalPorts(port0, port1); }
...
// static
void NodeController::DeserializeMessageAsEventForFuzzer(Channel::MessagePtr message) { DeserializeEventMessage(ports::NodeName(), std::move(message)); }
    ...
    base::Process target_process, ConnectionParams connection_params, ports::NodeName temporary_node_name, const ProcessErrorCallback& process_error_callback)
{
    ...
    scoped_refptr<NodeChannel> channel = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, process_error_callback);
#endif // !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA)
...
void NodeController::AcceptBrokerClientInvitationOnIOThread(ConnectionParams connection_params, absl::optional<PlatformHandle> broker_host_handle)
{
...

    {
        ...
        bootstrap_inviter_channel_ = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, ProcessErrorCallback());
...
void NodeController::OnAddBrokerClient(const ports::NodeName& from_node, const ports::NodeName& client_name, base::ProcessHandle process_handle)
{
    ...
    scoped_refptr<NodeChannel> client = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, ProcessErrorCallback());
...
void NodeController::OnAcceptBrokerClient(const ports::NodeName& from_node, const ports::NodeName& broker_name, PlatformHandle broker_channel, const uint64_t broker_capabilities)
{
    if (GetConfiguration().is_broker_process) {
    ...
    if (broker_name == inviter_name) {
        ...
        broker = NodeChannel::Create(
            this, ConnectionParams(PlatformChannelEndpoint(std::move(broker_channel))), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, ProcessErrorCallback());
...
void NodeController::OnIntroduce(const ports::NodeName& from_node, const ports::NodeName& name, PlatformHandle channel_handle, const uint64_t remote_capabilities)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    scoped_refptr<NodeChannel> channel
        = NodeChannel::Create(this, ConnectionParams(PlatformChannelEndpoint(std::move(channel_handle))), kPeerHandlePolicy, io_task_runner_, ProcessErrorCallback());
...
#if BUILDFLAG(IS_WIN)
void NodeController::OnRelayEventMessage(const ports::NodeName& from_node, base::ProcessHandle from_process, const ports::NodeName& destination, Channel::MessagePtr message)
{
...
void NodeController::OnAcceptPeer(const ports::NodeName& from_node, const ports::NodeName& token, const ports::NodeName& peer_name, const ports::PortName& port_name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
...
void NodeController::DestroyOnIOThreadShutdown() { destroy_on_io_thread_shutdown_ = true; }
