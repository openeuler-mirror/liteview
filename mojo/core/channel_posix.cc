...
public:
    ...
    const void* data() const { return static_cast<const char*>(message_->data()) + offset_; }
    ...
    std::vector<PlatformHandleInTransit> TakeHandles() { return std::move(handles_); }

    void SetHandles(std::vector<PlatformHandleInTransit> handles) { handles_ = std::move(handles); }

    size_t num_handles_sent() { return num_handles_sent_; }

    void set_num_handles_sent(size_t num_handles_sent) { num_handles_sent_ = num_handles_sent; }

    size_t num_handles_remaining() const { return handles_.size() - num_handles_sent_; }
...
ChannelPosix::ChannelPosix(Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
...
bool ChannelPosix::GetReadPlatformHandles(
    const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size, std::vector<PlatformHandle>* handles, bool* deferred)
{
...
void ChannelPosix::WaitForWriteOnIOThreadNoLock()
{
    ...
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
        pending_write_ = true;
        base::CurrentIOThread::Get()->WatchFileDescriptor(socket_.get(), false /* persistent */, base::MessagePumpForIO::WATCH_WRITE, write_watcher_.get(), this);
    } else {
...
void ChannelPosix::RejectUpgradeOffer() { Write(Message::CreateMessage(0, 0, Message::MessageType::UPGRADE_REJECT)); }

void ChannelPosix::AcceptUpgradeOffer() { Write(Message::CreateMessage(0, 0, Message::MessageType::UPGRADE_ACCEPT)); }
...
#if !BUILDFLAG(IS_NACL)
// static
void Channel::set_posix_use_writev(bool use_writev) { g_use_writev = use_writev; }
#endif // !BUILDFLAG(IS_NACL)
...
#if (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID))
// static
bool Channel::SupportsChannelUpgrade() { return ChannelLinux::KernelSupportsUpgradeRequirements() && ChannelLinux::UpgradesEnabled(); }
