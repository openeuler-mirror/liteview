...
public:
    ...
    char* GetReadBufferTest(size_t* buffer_capacity) { return GetReadBuffer(buffer_capacity); }

    bool OnReadCompleteTest(size_t bytes_read, size_t* next_read_size_hint) { return OnReadComplete(bytes_read, next_read_size_hint); }

    MOCK_METHOD7(GetReadPlatformHandles,
        bool(const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size, std::vector<PlatformHandle>* handles,
            bool* deferred));
...
Channel::MessagePtr CreateDefaultMessage(bool legacy_message)
{
    ...
    Channel::MessagePtr message
        = Channel::Message::CreateMessage(payload_size, 0, legacy_message ? Channel::Message::MessageType::NORMAL_LEGACY : Channel::Message::MessageType::NORMAL);
...
public:
    ChannelTestShutdownAndWriteDelegate(PlatformChannelEndpoint endpoint, scoped_refptr<base::SingleThreadTaskRunner> task_runner, scoped_refptr<Channel> client_channel,
        std::unique_ptr<base::Thread> client_thread, base::RepeatingClosure quit_closure)
...
public:
    ...
    // Channel::Delegate:
    void OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles) override { ++num_messages_; }
...
TEST(ChannelTest, RejectHandles)
{
    ...
    scoped_refptr<Channel> receiver
        = Channel::Create(&receiver_delegate, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());
    receiver->Start();

    RejectHandlesDelegate sender_delegate;
    scoped_refptr<Channel> sender
        = Channel::Create(&sender_delegate, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());
    sender->Start();
...
TEST(ChannelTest, PeerStressTest)
{
    ...
    scoped_refptr<Channel> channel_a
        = Channel::Create(&delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());

    CountingChannelDelegate delegate_b(quit_when_both_channels_received_final_message);
    scoped_refptr<Channel> channel_b
        = Channel::Create(&delegate_b, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kRejectHandles, peer_thread.task_runner());
...
public:
    ...
    void set_on_message(base::OnceClosure on_message) { on_message_ = std::move(on_message); }

    void set_on_error(base::OnceClosure on_error) { on_error_ = std::move(on_error); }
...
TEST(ChannelTest, MessageSizeTest)
{
    ...
    scoped_refptr<Channel> receiver
        = Channel::Create(&receiver_delegate, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    receiver->Start();

    MockChannelDelegate sender_delegate;
    scoped_refptr<Channel> sender
        = Channel::Create(&sender_delegate, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    sender->Start();
...
TEST(ChannelTest, SendToDeadMachPortName)
{
    ...
    scoped_refptr<Channel> channel_a
        = Channel::Create(&delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    channel_a->Start();

    // Channel B gets the receive right.
    MockChannelDelegate delegate_b;
    scoped_refptr<Channel> channel_b
        = Channel::Create(&delegate_b, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kAcceptHandles, peer_thread.task_runner());
    channel_b->Start();
...
TEST(ChannelTest, ShutDownStress)
{
    ...
    CallbackChannelDelegate delegate_a;
    scoped_refptr<Channel> channel_a
        = Channel::Create(&delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, task_environment.GetMainThreadTaskRunner());
    channel_a->Start();
