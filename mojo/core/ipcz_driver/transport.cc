...
void EncodeHandle(PlatformHandle& handle, const base::Process& remote_process, HandleOwner handle_owner, HANDLE& out_handle)
{
    ...
    BOOL result = ::DuplicateHandle(::GetCurrentProcess(), handle.ReleaseHandle(), remote_process.Handle(), &out_handle, 0, FALSE, DUPLICATE_SAME_ACCESS | DUPLICATE_CLOSE_SOURCE);
    DCHECK(result);
}
...
// static
void Transport::SetIOTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> runner) { GetIOTaskRunnerStorage() = std::move(runner); }

// static
const scoped_refptr<base::SingleThreadTaskRunner>& Transport::GetIOTaskRunner() { return GetIOTaskRunnerStorage(); }
...
void Transport::Close() { Deactivate(); }

bool Transport::IsSerializable() const { return true; }
...
scoped_refptr<Transport> Transport::Deserialize(Transport& from_transport, base::span<const uint8_t> data, base::span<PlatformHandle> handles)
{
    ...
    return Create({ .source = from_transport.source_type(), .destination = header.destination_type }, PlatformChannelEndpoint(std::move(handles[0])), std::move(process));
}

bool Transport::IsIpczTransport() const { return true; }
...
void Transport::OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    ...
    const IpczResult result
        = activity_handler_(ipcz_transport_, static_cast<const uint8_t*>(payload), payload_size, driver_handles.data(), driver_handles.size(), IPCZ_NO_FLAGS, nullptr);
    if (result != IPCZ_RESULT_OK && result != IPCZ_RESULT_UNIMPLEMENTED) {
        OnChannelError(Channel::Error::kReceivedMalformedData);
    }
}

void Transport::OnChannelError(Channel::Error error) { activity_handler_(ipcz_transport_, nullptr, 0, nullptr, 0, IPCZ_TRANSPORT_ACTIVITY_ERROR, nullptr); }
