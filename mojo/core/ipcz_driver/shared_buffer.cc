...
// an access mode.
base::subtle::ScopedPlatformSharedMemoryHandle CreateRegionHandleFromPlatformHandles(base::span<PlatformHandle> handles, base::subtle::PlatformSharedMemoryRegion::Mode mode)
{
    if (handles.empty()) {
        return {};
    }
...
scoped_refptr<SharedBuffer> SharedBuffer::CreateForMojoWrapper(
    base::span<const MojoPlatformHandle> mojo_platform_handles, uint32_t size, const MojoSharedBufferGuid& mojo_guid, MojoPlatformSharedMemoryRegionAccessMode access)
{
...
void SharedBuffer::Close() { region_ = {}; }

bool SharedBuffer::IsSerializable() const { return true; }
