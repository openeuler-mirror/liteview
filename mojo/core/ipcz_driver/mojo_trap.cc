...
struct MojoTrap::Trigger : public base::RefCountedThreadSafe<Trigger> {
    ...
    static Trigger& FromEvent(const IpczTrapEvent& event) { return *reinterpret_cast<Trigger*>(event.context); }

    bool is_for_data_producer() const { return data_pipe && data_pipe->byte_capacity() > 0; }
    bool is_for_data_consumer() const { return data_pipe && data_pipe->byte_capacity() == 0; }
...
MojoResult MojoTrap::AddTrigger(MojoHandle handle, MojoHandleSignals signals, MojoTriggerCondition condition, uintptr_t trigger_context)
{
    ...
    IpczPortalStatus status = { sizeof(status) };
    {
        ...
        IpczResult result = GetIpczAPI().Trap(handle, &removal_conditions, &TrapRemovalEventHandler, trigger->ipcz_context(), IPCZ_NO_FLAGS, nullptr, nullptr, nullptr);
...
// static
void MojoTrap::TrapEventHandler(const IpczTrapEvent* event) { Trigger::FromEvent(*event).mojo_trap->HandleEvent(*event); }
...
IpczResult MojoTrap::ArmTrigger(Trigger& trigger, IpczTrapConditionFlags* satisfied_flags, IpczPortalStatus* status)
{
    ...
    trigger.AddRef();
    IpczResult result = GetIpczAPI().Trap(trigger.handle, &trigger.conditions, &TrapEventHandler, trigger.ipcz_context(), IPCZ_NO_FLAGS, nullptr, satisfied_flags, status);
    if (result == IPCZ_RESULT_OK) {
        trigger.armed = true;
    } else {
