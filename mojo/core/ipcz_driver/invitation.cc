...
IpczDriverHandle CreateTransportForMojoEndpoint(Transport::EndpointTypes endpoint_types, const MojoInvitationTransportEndpoint& endpoint, bool leak_channel_on_shutdown,
    base::Process remote_process = base::Process(), MojoProcessErrorHandler error_handler = nullptr, uintptr_t error_handler_context = 0)
{
...
// static
void Invitation::SetDefaultProcessErrorHandler(MojoDefaultProcessErrorHandler handler) { g_default_process_error_handler = handler; }
...
MojoResult Invitation::Send(const MojoPlatformProcessHandle* process_handle, const MojoInvitationTransportEndpoint* transport_endpoint, MojoProcessErrorHandler error_handler,
    uintptr_t error_handler_context, const MojoSendInvitationOptions* options)
{
    ...
    IpczDriverHandle transport = CreateTransportForMojoEndpoint(
        { .source = config.is_broker ? Transport::kBroker : Transport::kNonBroker, .destination = is_isolated ? Transport::kBroker : Transport::kNonBroker }, *transport_endpoint,
