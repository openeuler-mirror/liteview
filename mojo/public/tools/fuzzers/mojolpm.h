...
#if MOJOLPM_DBG
template <typename T> std::string type_name() { return std::string(__PRETTY_FUNCTION__).substr(38, strlen(__PRETTY_FUNCTION__) - 39); }
#endif
...
template <typename T> Context::Storage::Storage(T&& value) { wrapper_ = std::make_unique<StorageWrapper<T>>(std::forward<T>(value)); }
...
template <typename T> TypeId Context::Storage::StorageWrapper<T>::type() const { return type_id<T>(); }

template <typename T> T& Context::Storage::StorageWrapper<T>::value() { return value_; }

template <typename T> const T& Context::Storage::StorageWrapper<T>::value() const { return value_; }
...
template <typename T> void Context::StorageTraits<std::unique_ptr<::mojo::AssociatedReceiver<T>>>::OnInstanceAdded(uint32_t id)
{
    ...
    CHECK(instance);
    // Unretained is safe here since context_ owns instance.
    (*instance)->set_disconnect_handler(base::BindOnce(&Context::RemoveInstance<std::unique_ptr<::mojo::AssociatedReceiver<T>>>, base::Unretained(context_), id));
}
...
template <typename T> uint32_t Context::AddInstance(T instance) { return AddInstance(1, std::move(instance)); }
...
template <typename T> uint32_t NextId() { return GetContext()->NextId<T>(); }
...
template <typename ProtoTestcase, typename ProtoAction, int kMaxActionCount, int kMaxActionSize>
void Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::Run(scoped_refptr<base::TaskRunner> fuzzer_task_runner, base::OnceClosure done_closure)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsFinished()) {
        ...
        auto run_closure
            = base::BindOnce(&Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::Run, base::Unretained(this), fuzzer_task_runner, std::move(done_closure));
