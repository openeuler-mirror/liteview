...
public:
    ...
    Message(uint32_t name, uint32_t flags, size_t payload_size, size_t payload_interface_id_count, MojoCreateMessageFlags create_message_flags, std::vector<ScopedHandle>* handles);
    ...
    // Access the header.
    const internal::MessageHeader* header() const { return reinterpret_cast<const internal::MessageHeader*>(data()); }
    internal::MessageHeader* header() { return reinterpret_cast<internal::MessageHeader*>(mutable_data()); }
    ...
    void set_request_id(uint64_t request_id) { header_v1()->request_id = request_id; }

    void set_trace_nonce(uint32_t trace_nonce) { header()->trace_nonce = trace_nonce; }
    ...
    const std::vector<ScopedInterfaceEndpointHandle>* associated_endpoint_handles() const { return &associated_endpoint_handles_; }
    std::vector<ScopedInterfaceEndpointHandle>* mutable_associated_endpoint_handles() { return &associated_endpoint_handles_; }
    ...
    void set_receiver_connection_group(const ConnectionGroup::Ref* ref) { receiver_connection_group_ = ref; }
    const ConnectionGroup::Ref* receiver_connection_group() const { return receiver_connection_group_; }
    ...
    void set_heap_profiler_tag(const char* heap_profiler_tag) { heap_profiler_tag_ = heap_profiler_tag; }
    ...
    void set_interface_name(const char* interface_name) { interface_name_ = interface_name; }
