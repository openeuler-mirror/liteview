...
private:
    // mojom::TestService1:
    void GetIdentity(GetIdentityCallback callback) override { std::move(callback).Run(1); }
...
private:
    // mojom::TestService2:
    void GetIdentity(GetIdentityCallback callback) override { std::move(callback).Run(2); }
...
auto RunTestService1(PendingReceiver<mojom::TestService1> receiver) { return std::make_unique<TestService1Impl>(std::move(receiver)); }

auto RunTestService2(PendingReceiver<mojom::TestService2> receiver) { return std::make_unique<TestService2Impl>(std::move(receiver)); }
