...
template <> struct TypeConverter<test::RectPtr, RedmondRect> {
    static test::RectPtr Convert(const RedmondRect& input) { return test::Rect::New(input.left, input.top, input.right - input.left, input.bottom - input.top); }
};
...
template <> struct TypeConverter<test::NamedRegionPtr, RedmondNamedRegion> {
    static test::NamedRegionPtr Convert(const RedmondNamedRegion& input) { return test::NamedRegion::New(input.name, ConvertTo<std::vector<test::RectPtr>>(input.rects)); }
};
