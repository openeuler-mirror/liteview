...
private:
    ...
    void GetIntValue(GetIntValueCallback callback) override { std::move(callback).Run(int_value_); }
...
private:
    void Echo(PodUnionPtr in, EchoCallback callback) override { std::move(callback).Run(std::move(in)); }
};
...
public:
    ...
    void ExchangeWithBoolDefault(mojom::ExtensibleTestUnionV1WithBoolDefaultPtr u, ExchangeWithBoolDefaultCallback callback) override { std::move(callback).Run(std::move(u)); }
    void ExchangeWithIntDefault(mojom::ExtensibleTestUnionV1WithIntDefaultPtr u, ExchangeWithIntDefaultCallback callback) override { std::move(callback).Run(std::move(u)); }
