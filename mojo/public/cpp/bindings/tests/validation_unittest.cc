...
std::string GetPath(const std::string& root, const std::string& suffix) { return "mojo/public/interfaces/bindings/tests/data/validation/" + root + suffix; }
...
public:
    ...
    bool Accept(Message* message) override { return first_validator_->Accept(message) && second_validator_->Accept(message); }
...
public:
    ...
    ScopedMessagePipeHandle testee_endpoint() { return std::move(testee_endpoint_); }
...
private:
    ...
    public:
        ...
        bool Accept(Message* message) override { return connector_.Accept(message); }
...
public:
    ...
    void Method0(BasicStructPtr param0, Method0Callback callback) override { std::move(callback).Run(std::vector<uint8_t>()); }
};
...
TEST_F(ValidationTest, InputParser)
{
    ...
    {
        const char* error_inputs[] = { "/ hello world", "[u1]x", "[u2]-1000", "[u1]0x100", "[s2]-0x8001", "[b]1", "[b]1111111k", "[dist4]unmatched", "[anchr]hello [dist8]hello",
            "[dist4]a [dist4]a [anchr]a", "[dist4]a [anchr]a [dist4]a [anchr]a", "0 [handles]50", nullptr };
...
TEST_F(ValidationTest, Conformance)
{
    ...
    validators.SetValidator(std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<ConformanceTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, AssociatedConformace)
{
    ...
    validators.SetValidator(
        std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<AssociatedConformanceTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, BoundsCheck)
{
    DummyMessageReceiver dummy_receiver;
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<BoundsCheckTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, ResponseConformance)
{
    DummyMessageReceiver dummy_receiver;
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<ConformanceTestInterface::ResponseValidator_>()));
...
TEST_F(ValidationTest, ResponseBoundsCheck)
{
    DummyMessageReceiver dummy_receiver;
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<BoundsCheckTestInterface::ResponseValidator_>()));
