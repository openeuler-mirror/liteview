...
RectPtr MakeRect(int32_t factor = 1) { return Rect::New(1 * factor, 2 * factor, 10 * factor, 20 * factor); }
...
TEST_F(StructTest, Versioning_OldToNew)
{
    ...
    {
        MessagePipe pipe;
        MultiVersionStructV7Ptr input(MultiVersionStructV7::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }, std::move(pipe.handle0), false));
