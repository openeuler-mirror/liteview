...
public:
    ...
    void Bind(PendingReceiver<TestBadMessages> receiver) { receiver_.Bind(std::move(receiver)); }

    ReportBadMessageCallback& bad_message_callback() { return bad_message_callback_; }
...
private:
    ...
    void RequestResponse(RequestResponseCallback callback) override { std::move(callback).Run(); }
    ...
    void RequestResponseSync(RequestResponseSyncCallback callback) override { std::move(callback).Run(); }
...
public:
    ...
    void TearDown() override { mojo::SetDefaultProcessErrorHandler(base::NullCallback()); }

    TestBadMessages* remote() { return remote_.get(); }

    TestBadMessagesImpl* impl() { return &impl_; }

    void SetErrorHandler(base::OnceClosure handler) { error_handler_ = std::move(handler); }
