...
public:
    ...
    template <typename Func> void set_ping_handler(Func handler) { ping_handler_ = base::BindLambdaForTesting(handler); }

    using EchoHandler = base::RepeatingCallback<void(int32_t, base::OnceCallback<void(int32_t)>)>;
    template <typename Func> void set_echo_handler(Func handler) { echo_handler_ = base::BindLambdaForTesting(handler); }

    using AsyncEchoHandler = base::RepeatingCallback<void(int32_t, base::OnceCallback<void(int32_t)>)>;
    template <typename Func> void set_async_echo_handler(Func handler) { async_echo_handler_ = base::BindLambdaForTesting(handler); }

    using SendRemoteHandler = base::RepeatingCallback<void(PendingAssociatedRemote<TestSync>)>;
    template <typename Func> void set_send_remote_handler(Func handler) { send_remote_handler_ = base::BindLambdaForTesting(handler); }

    using SendReceiverHandler = base::RepeatingCallback<void(PendingAssociatedReceiver<TestSync>)>;
    template <typename Func> void set_send_receiver_handler(Func handler) { send_receiver_handler_ = base::BindLambdaForTesting(handler); }
    ...
    void SendRemoteImpl(PendingAssociatedRemote<TestSync> remote) { send_remote_handler_.Run(std::move(remote)); }
    void SendReceiverImpl(PendingAssociatedReceiver<TestSync> receiver) { send_receiver_handler_.Run(std::move(receiver)); }
...
public:
    ...
    void Echo(int32_t value, EchoCallback callback) override { EchoImpl(value, std::move(callback)); }
    void AsyncEcho(int32_t value, AsyncEchoCallback callback) override { AsyncEchoImpl(value, std::move(callback)); }
...
public:
    ...
    void Echo(int32_t value, EchoCallback callback) override { EchoImpl(value, std::move(callback)); }
    void AsyncEcho(int32_t value, AsyncEchoCallback callback) override { AsyncEchoImpl(value, std::move(callback)); }
    void SendRemote(PendingAssociatedRemote<TestSync> remote) override { SendRemoteImpl(std::move(remote)); }
    void SendReceiver(PendingAssociatedReceiver<TestSync> receiver) override { SendReceiverImpl(std::move(receiver)); }
...
public:
    ...
    Interface* operator->() { return shared_remote_ ? shared_remote_.get() : remote_.get(); }
...
public:
    ...
    void SetUp() override { BindingsTestBase::SetupSerializationBehavior(TypeParam::kSerializationMode); }
};
...
protected:
    ...
    base::OnceClosure DoneClosure() { return base::BindOnce(&SequencedTaskRunnerTestBase::Done, base::Unretained(this)); }
...
using InterfaceTypes = testing::Types<TestParams<TestSync, true, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSync, false, BindingsTestSerializationMode::kSerializeBeforeSend>, TestParams<TestSyncPrimary, true, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSyncPrimary, false, BindingsTestSerializationMode::kSerializeBeforeSend>, TestParams<TestSync, true, BindingsTestSerializationMode::kSerializeBeforeDispatch>,
    ...
    TestParams<TestSyncPrimary, false, BindingsTestSerializationMode::kSerializeBeforeDispatch>, TestParams<TestSync, true, BindingsTestSerializationMode::kNeverSerialize>,
    TestParams<TestSync, false, BindingsTestSerializationMode::kNeverSerialize>, TestParams<TestSyncPrimary, true, BindingsTestSerializationMode::kNeverSerialize>,
...
TYPED_TEST(SyncMethodCommonTest, BasicSyncCalls)
{
    ...
    service_sequence.task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestSyncServiceSequence<Interface>::SetUp, base::Unretained(&service_sequence), std::move(receiver)));
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(SyncMethodOnSequenceCommonTest, QueuedMessagesProcessedBeforeErrorNotification)
{
    ...
    void Run() override
    {
        this->impl_->set_async_echo_handler([this](int32_t value, TestSync::AsyncEchoCallback callback) { OnAsyncEchoReachedService(value, std::move(callback)); });
...
private:
    ...
    void SimplePing(SimplePingCallback callback) override { std::move(callback).Run(); }

    void SimplePingNoInterrupt(SimplePingNoInterruptCallback callback) override { std::move(callback).Run(); }
...
public:
    SyncInterruptTest()
    {
        ...
        receiver_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                [](PendingReceiver<mojom::Pinger> receiver, PendingReceiver<mojom::SimplePinger> simple_receiver, PendingReceiver<mojom::Pinger> shared_receiver) {
    ...
    mojom::Pinger& shared_associated_pinger() { return *shared_associated_pinger_.get(); }
...
public:
    ...
    void SetCallHandler(base::OnceClosure call_handler) { call_handler_ = std::move(call_handler); }
...
public:
    void SetUp() override { mojo::SyncCallRestrictions::DisableSyncCallInterrupts(); }

    void TearDown() override { mojo::SyncCallRestrictions::EnableSyncCallInterruptsForTesting(); }
};
...
TEST_P(DisableSyncInterruptTest, SharedRemoteNoInterruptWhenDisabled)
{
    ...
    base::SequenceBound<SyncService> background_service { background_service_thread.task_runner(), caller.BindNewPipeAndPassReceiver(background_client_thread.task_runner()) };
...
void NoSyncImpl::BindNoSync(PendingAssociatedReceiver<mojom::NoSync> receiver) { associated_no_sync_ = std::make_unique<NoSyncImpl>(std::move(receiver)); }

void NoSyncImpl::BindOneSync(PendingAssociatedReceiver<mojom::OneSync> receiver) { associated_one_sync_ = std::make_unique<OneSyncImpl>(std::move(receiver)); }
...
void OneSyncImpl::SyncMethod(MethodCallback callback) { std::move(callback).Run(); }

void OneSyncImpl::BindNoSync(PendingAssociatedReceiver<mojom::NoSync> receiver) { associated_no_sync_ = std::make_unique<NoSyncImpl>(std::move(receiver)); }

void OneSyncImpl::BindOneSync(PendingAssociatedReceiver<mojom::OneSync> receiver) { associated_one_sync_ = std::make_unique<OneSyncImpl>(std::move(receiver)); }
