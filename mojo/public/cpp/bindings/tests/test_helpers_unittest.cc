...
public:
    ...
    // test::EchoService:
    void Echo(const std::string& message, EchoCallback callback) override { std::move(callback).Run(message); }
...
public:
    ...
    void BounceOne(ScopedMessagePipeHandle one, BounceOneCallback callback) override { std::move(callback).Run(std::move(one)); }

    void BounceTwo(ScopedMessagePipeHandle one, ScopedMessagePipeHandle two, BounceTwoCallback callback) override { std::move(callback).Run(std::move(one), std::move(two)); }
