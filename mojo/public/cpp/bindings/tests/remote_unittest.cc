...
public:
    ...
    void set_disconnect_handler(base::OnceClosure closure) { calculator_.set_disconnect_handler(std::move(closure)); }

    void Add(double value, base::OnceClosure closure) { calculator_->Add(value, base::BindOnce(&MathCalculatorUI::Output, base::Unretained(this), std::move(closure))); }

    void Multiply(double value, base::OnceClosure closure) { calculator_->Multiply(value, base::BindOnce(&MathCalculatorUI::Output, base::Unretained(this), std::move(closure))); }
...
private:
    ...
    void GetInteger(GetIntegerCallback callback) override { std::move(callback).Run(integer_, sample::Enum::VALUE); }
...
TEST_P(EndToEndRemoteTest, EndToEnd) { RunTest(base::ThreadTaskRunnerHandle::Get()); }

TEST_P(EndToEndRemoteTest, EndToEndOnSequence) { RunTest(base::ThreadPool::CreateSequencedTaskRunner({})); }
...
public:
    ...
    void Clear(ClearCallback callback) override { std::move(callback).Run(total_); }
...
public:
    ...
    void Bind(PendingReceiver<C> receiver) { receiver_.Bind(std::move(receiver)); }
...
public:
    ...
    void Bind(PendingReceiver<B> receiver) { receiver_.Bind(std::move(receiver)); }

private:
    void GetC(PendingReceiver<C> receiver) override { c_.Bind(std::move(receiver)); }
...
private:
    void GetB(PendingReceiver<B> receiver) override { b_.Bind(std::move(receiver)); }
...
void Fail() { FAIL() << "Unexpected connection error"; }
...
public:
    ...
    void set_quit_callback(base::OnceClosure callback) { quit_callback_ = std::move(callback); }

    // mojom::SequenceChecker:
    void Bind(PendingAssociatedReceiver<mojom::SequenceChecker> receiver) override { receivers_.Add(this, std::move(receiver)); }

    void AddClient(PendingAssociatedRemote<mojom::SequenceCheckerClient> client) override { clients_.Add(std::move(client)); }
...
TEST_P(RemoteTest, SharedRemotePassAssociatedEndpointsEarly)
{
    ...
    other_thread_task_runner->PostTask(FROM_HERE,
        base::BindOnce([](PendingReceiver<mojom::SequenceChecker> receiver) { MakeSelfOwnedReceiver(std::make_unique<SequenceCheckerImpl>(), std::move(receiver)); },
...
TEST_P(RemoteTest, SharedRemoteSyncOnlyBlocksCallingSequence)
{
    ...
    bound_task_runner->PostTask(FROM_HERE,
        base::BindOnce([](PendingReceiver<mojom::SharedRemoteSyncTest> receiver) { MakeSelfOwnedReceiver(std::make_unique<SharedRemoteSyncTestImpl>(), std::move(receiver)); },
            std::move(receiver)));
...
extern "C" void HandleDumpWithoutCrashing() { *dump_without_crashing_flag = true; }
...
public:
    ...
    void ProcessData(const std::vector<uint8_t>& data, ProcessDataCallback callback) override { std::move(callback).Run(data.size()); }

    void ProcessLotsOfData(const std::vector<uint8_t>& data, ProcessLotsOfDataCallback callback) override { std::move(callback).Run(data.size()); }

    void GetLotsOfData(uint64_t data_size, GetLotsOfDataCallback callback) override { std::move(callback).Run(std::vector<uint8_t>(data_size)); }
