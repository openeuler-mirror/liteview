...
public:
    ...
    // EchoService:
    void Echo(const std::string& test_data, EchoCallback callback) override { std::move(callback).Run(test_data); }
...
public:
    void RunTestOnTaskRunner(base::TaskRunner* runner, MojoHandle client_mp, const std::string& test_name)
    {
        if (runner == base::ThreadTaskRunnerHandle::Get().get()) {
            ...
            runner->PostTaskAndReply(FROM_HERE, base::BindOnce(&MojoE2EPerftest::RunTests, base::Unretained(this), client_mp, test_name), run_loop.QuitClosure());
            run_loop.Run();
        }
    }
