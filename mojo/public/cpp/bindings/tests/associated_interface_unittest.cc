...
public:
    ...
    void set_notify_send_method_called(base::RepeatingCallback<void(int32_t)> callback) { notify_send_method_called_ = std::move(callback); }
...
public:
    ...
    template <typename T>
    PendingAssociatedRemote<T> EmulatePassingAssociatedRemote(PendingAssociatedRemote<T> remote, scoped_refptr<MultiplexRouter> source, scoped_refptr<MultiplexRouter> target)
    {
    ...
    void CreateIntegerSenderWithExistingRouters(scoped_refptr<MultiplexRouter> router0, PendingAssociatedRemote<IntegerSender>* remote0, scoped_refptr<MultiplexRouter> router1,
...
public:
    ...
    void SetUp(PendingAssociatedReceiver<IntegerSender> receiver0, PendingAssociatedReceiver<IntegerSender> receiver1, size_t expected_calls, base::OnceClosure notify_finish)
    {
        CHECK(task_runner()->RunsTasksInCurrentSequence());
...
TEST_F(AssociatedInterfaceTest, MultiThreadAccess)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        base::RunLoop run_loop2;
        receivers[i].task_runner()->PostTaskAndReply(FROM_HERE, base::BindOnce(&TestReceiver::TearDown, base::Unretained(&receivers[i])), run_loop2.QuitClosure());
        run_loop2.Run();
    }
...
TEST_F(AssociatedInterfaceTest, FIFO)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        senders[i].task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&TestSender::SetUp, base::Unretained(&senders[i]), std::move(pending_remotes[i]), base::Unretained(&senders[(i + 1) % 4]), kMaxValue));
    }
...
public:
    ...
    void set_ping_handler(base::RepeatingClosure handler) { ping_handler_ = std::move(handler); }
...
public:
    ...
    std::vector<std::unique_ptr<PingServiceImpl>>& ping_services() { return ping_services_; }
...
public:
    ...
    static std::unique_ptr<CallbackFilter> Wrap(base::RepeatingClosure callback) { return std::make_unique<CallbackFilter>(std::move(callback)); }
