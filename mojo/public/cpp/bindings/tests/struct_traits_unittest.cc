...
template <typename T> base::OnceCallback<void(const T&)> ExpectResult(const T& r, base::OnceClosure callback) { return base::BindOnce(&DoExpectResult<T>, r, std::move(callback)); }

template <typename T> void DoFail(const std::string& reason, const T&) { EXPECT_TRUE(false) << reason; }

template <typename T> base::OnceCallback<void(const T&)> Fail(const std::string& reason) { return base::BindOnce(&DoFail<T>, reason); }

template <typename T> void ExpectError(Remote<T>* proxy, base::OnceClosure callback) { proxy->set_disconnect_handler(std::move(callback)); }
...
public:
    ...
    void GetLargestRect(GetLargestRectCallback callback) override { std::move(callback).Run(largest_rect_); }

    void PassSharedRect(const SharedRect& r, PassSharedRectCallback callback) override { std::move(callback).Run(r); }
...
protected:
    void BindToChromiumService(PendingReceiver<RectService> receiver) { chromium_receivers_.Add(&chromium_service_, std::move(receiver)); }
    ...
    void BindToBlinkService(PendingReceiver<blink::RectService> receiver) { blink_receivers_.Add(&blink_service_, std::move(receiver)); }
    void BindToBlinkService(PendingReceiver<RectService> receiver) { blink_receivers_.Add(&blink_service_, ConvertPendingReceiver<blink::RectService>(std::move(receiver))); }
...
private:
    // TraitsTestService:
    void EchoStructWithTraits(const StructWithTraitsImpl& s, EchoStructWithTraitsCallback callback) override { std::move(callback).Run(s); }

    void EchoTrivialStructWithTraits(TrivialStructWithTraitsImpl s, EchoTrivialStructWithTraitsCallback callback) override { std::move(callback).Run(s); }

    void EchoMoveOnlyStructWithTraits(MoveOnlyStructWithTraitsImpl s, EchoMoveOnlyStructWithTraitsCallback callback) override { std::move(callback).Run(std::move(s)); }
    ...
    void EchoEnumWithTraits(EnumWithTraitsImpl e, EchoEnumWithTraitsCallback callback) override { std::move(callback).Run(e); }

    void EchoStructWithTraitsForUniquePtr(std::unique_ptr<int> e, EchoStructWithTraitsForUniquePtrCallback callback) override { std::move(callback).Run(std::move(e)); }
    ...
    void EchoUnionWithTraits(std::unique_ptr<test::UnionWithTraitsBase> u, EchoUnionWithTraitsCallback callback) override { std::move(callback).Run(std::move(u)); }
...
TEST_F(StructTraitsTest, EchoEnumWithTraits)
{
    ...
    proxy->EchoEnumWithTraits(EnumWithTraitsImpl::CUSTOM_VALUE_1, base::BindOnce(&ExpectEnumWithTraits, EnumWithTraitsImpl::CUSTOM_VALUE_1, loop.QuitClosure()));
    loop.Run();
}
...
TEST_F(StructTraitsTest, TypemapUniquePtr)
{
    ...
    {
        base::RunLoop loop;
        proxy->EchoStructWithTraitsForUniquePtr(std::make_unique<int>(12345), base::BindOnce(&ExpectUniquePtr, std::make_unique<int>(12345), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
