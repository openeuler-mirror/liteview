...
public:
    ...
    void ShutDown(base::OnceClosure callback) { task_runner_->PostTask(FROM_HERE, base::BindOnce(&KeyValueStoreImpl::ShutDownOnTaskRunner, this, std::move(callback))); }
...
private:
    ...
    public:
        ...
        void Put(const std::string& key, const std::string& value) override { key_value_store_->StoreValue(key, value); }

    private:
        void BindOnTaskRunner(PendingReceiver<mojom::Writer> receiver) { receiver_ = std::make_unique<Receiver<mojom::Writer>>(this, std::move(receiver)); }
    ...
    void BindOnTaskRunner(PendingReceiver<mojom::KeyValueStore> receiver) { receiver_ = std::make_unique<Receiver<mojom::KeyValueStore>>(this, std::move(receiver)); }
    ...
    void SetClient(PendingRemote<mojom::KeyValueStoreClient> client) override { client_.Bind(std::move(client)); }
...
public:
    FlushAsyncTest() { key_value_store_->Bind(remote_key_value_store_.BindNewPipeAndPassReceiver()); }
    ...
    Remote<mojom::KeyValueStore>& key_value_store() { return remote_key_value_store_; }
...
public:
    ...
    void set_snapshot_taken_callback(base::RepeatingClosure callback) { snapshot_taken_callback_ = std::move(callback); }
