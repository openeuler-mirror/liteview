...
public:
    ...
    bool RunsTasksInCurrentSequence() const override { return base::PlatformThread::CurrentRef() == thread_id_; }
...
public:
    ...
    explicit IntegerSenderConnectionImpl(
        PendingReceiver<IntegerSenderConnection> receiver, scoped_refptr<base::SequencedTaskRunner> runner, scoped_refptr<base::SequencedTaskRunner> sender_runner)
        : receiver_(this, std::move(receiver), std::move(runner))
    ...
    void set_get_sender_notification(base::OnceClosure notification) { get_sender_notification_ = std::move(notification); }
    ...
    void AsyncGetSender(AsyncGetSenderCallback callback) override { NOTREACHED(); }
...
protected:
    void SetUp() override
    {
        ...
        connection_impl_ = std::make_unique<IntegerSenderConnectionImpl>(std::move(connection_receiver), connection_receiver_task_runner_, sender_receiver_task_runner_);
