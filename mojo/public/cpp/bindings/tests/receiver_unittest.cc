...
private:
    ...
    void Frobinate(sample::FooPtr foo, BazOptions options, PendingRemote<sample::Port> port, FrobinateCallback callback) override { std::move(callback).Run(1); }
...
private:
    ...
    void GetInteger(GetIntegerCallback callback) override { std::move(callback).Run(1, sample::Enum::VALUE); }
...
public:
    ...
    void DidDispatchOrReject(Message* message, bool accepted) override { did_dispatch_callback_.Run(); }
...
public:
    ...
    void SyncCall(SyncCallCallback callback) override { std::move(callback).Run(); }
...
public:
    ...
    void set_disconnect_handler(base::OnceClosure callback) { receiver_.set_disconnect_handler(std::move(callback)); }
