...
template <typename ReceiverSetType, typename ContextType> base::RepeatingClosure ExpectContext(const ReceiverSetType* receiver_set, ContextType expected_context)
{
    return base::BindRepeating(&ExpectContextHelper<ReceiverSetType, ContextType>, receiver_set, expected_context);
}

template <typename ReceiverSetType, typename ContextType> void ExpectMutableContextHelper(ReceiverSetType* receiver_set, ContextType expected_context, ContextType new_context)
{
    {
...
template <typename ReceiverSetType> void ReportBadMessageHelper(ReceiverSetType* receiver_set, const std::string& error) { receiver_set->ReportBadMessage(error); }
...
public:
    ...
    void set_ping_handler(const base::RepeatingClosure& handler) { ping_handler_ = handler; }
...
public:
    ...
    void set_new_ping_handler(base::RepeatingClosure handler) { new_ping_handler_ = std::move(handler); }

    void set_ping_handler(base::RepeatingClosure handler) { ping_handler_ = std::move(handler); }

    AssociatedReceiverSet<PingService, int>& ping_receivers() { return ping_receivers_; }
