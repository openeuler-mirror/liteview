...
//     }
    ...
    template <typename T,
        std::enable_if_t<std::is_same<PendingReceiver<Interface>, std::invoke_result_t<decltype(&PendingReceiverConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    ...
    // entangled Remote.
    bool is_valid() const { return /*state_.raw_ptr_pipe.get();*/ state_.pipe.is_valid(); }
...
//         DCHECK_EQ(MOJO_RESULT_OK, result);
    ...
    [[nodiscard]] ScopedMessagePipeHandle PassPipe() { return std::move(state_.pipe); }
//     void** PassPtrAddr()
//     {
...
public:
    template <typename Interface> operator PendingReceiver<Interface>() const { return PendingReceiver<Interface>(); }
};
...
using PendingAssociatedReceiver = PendingReceiver<Interface>;
