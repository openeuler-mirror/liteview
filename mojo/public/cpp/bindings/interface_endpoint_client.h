...
public:
    ...
    InterfaceEndpointClient(ScopedInterfaceEndpointHandle handle, MessageReceiverWithResponderStatus* receiver, std::unique_ptr<MessageReceiver> payload_validator,
        base::span<const uint32_t> sync_method_ordinals, scoped_refptr<base::SequencedTaskRunner> task_runner, uint32_t interface_version, const char* interface_name,
    ...
    unsigned int GetNumUnackedMessagesForTesting() const { return num_unacked_messages_; }
    ...
    MessageToMethodInfoCallback method_info_callback() const { return method_info_callback_; }
    MessageToMethodNameCallback method_name_callback() const { return method_name_callback_; }

#if DCHECK_IS_ON()
    void SetNextCallLocation(const base::Location& location) { next_call_location_ = location; }
#endif
    ...
    base::span<const uint32_t> sync_method_ordinals() const { return sync_method_ordinals_; }
