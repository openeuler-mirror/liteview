...
public:
    ...
    // bound and callable.
    bool is_bound() const { return remote_impl_.is_bound(); }
    ...
    // weolar: no use
    void set_connection_error_handler_is_sync() { remote_impl_.set_connection_error_handler_is_sync(); }
...
//     }
    ...
    // or |RequireVersion()|.
    uint32_t version() const { return remote_impl_.version(); }
...
//         return PendingRemote<Interface>();
    ...
    void QueryVersion(base::OnceCallback<void(uint32_t)> callback) { remote_impl_.QueryVersion(std::move(callback)); }
    ...
    // immediately.
    void RequireVersion(uint32_t version) { remote_impl_.RequireVersion(version); }
    ...
    // example usage.
    void PauseReceiverUntilFlushCompletes(PendingFlush flush) { remote_impl_.PauseReceiverUntilFlushCompletes(std::move(flush)); }
    ...
    void FlushAsyncWithFlusher(AsyncFlusher flusher) { remote_impl_.FlushAsync(std::move(flusher)); }
    ...
    // stimulus.
    void FlushForTesting() { remote_impl_.FlushForTesting(); }

    // Same as |FlushForTesting()| but will call |callback| when the flush is
    // complete.
    void FlushAsyncForTesting(base::OnceClosure callback) { remote_impl_.FlushAsyncForTesting(std::move(callback)); }

    // Returns the number of unacknowledged messages sent by this Remote. Only
    // non-zero when |set_idle_handler()| has been called.
    unsigned int GetNumUnackedMessagesForTesting() const { return remote_impl_.GetNumUnackedMessagesForTesting(); }

    scoped_refptr<base::SequencedTaskRunner> runner() { return remote_impl_.runner(); } // weolar

    // DO NOT USE. Exposed only for internal use and for testing.
    internal::RemoteImpl<Interface>* internal_state() { return &remote_impl_; }
...
using AssociatedRemote = Remote<Interface>;
#endif
