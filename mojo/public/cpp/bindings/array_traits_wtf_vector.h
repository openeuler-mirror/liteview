...
template <typename U, WTF::wtf_size_t InlineCapacity> struct ArrayTraits<WTF::Vector<U, InlineCapacity>> {
    ...
    static size_t GetSize(const WTF::Vector<U, InlineCapacity>& input) { return input.size(); }

    static U* GetData(WTF::Vector<U, InlineCapacity>& input) { return input.data(); }

    static const U* GetData(const WTF::Vector<U, InlineCapacity>& input) { return input.data(); }

    static U& GetAt(WTF::Vector<U, InlineCapacity>& input, size_t index) { return input[static_cast<wtf_size_t>(index)]; }

    static const U& GetAt(const WTF::Vector<U, InlineCapacity>& input, size_t index) { return input[static_cast<wtf_size_t>(index)]; }
