...
//     }
    ...
    template <typename T,
        std::enable_if_t<std::is_same<PendingRemote<Interface>, std::invoke_result_t<decltype(&PendingRemoteConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    ...
    template <typename T,
        std::enable_if_t<
        std::is_same<PendingRemote<Interface>, std::invoke_result_t<decltype(&PendingAssociatedRemoteConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    PendingRemote(T&& other)
...
//     }
    ...
    ScopedInterfaceEndpointHandle PassHandle() { return PassPipe(); }
#endif
...
public:
    template <typename Interface> operator PendingRemote<Interface>() const { return PendingRemote<Interface>(); }
};
...
using PendingAssociatedRemote = PendingRemote<Interface>;
