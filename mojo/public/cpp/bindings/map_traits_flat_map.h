...
template <typename K, typename V, typename Compare> struct MapTraits<base::flat_map<K, V, Compare>> {
    ...
    static size_t GetSize(const base::flat_map<K, V, Compare>& input) { return input.size(); }

    static ConstIterator GetBegin(const base::flat_map<K, V, Compare>& input) { return input.begin(); }
    static Iterator GetBegin(base::flat_map<K, V, Compare>& input) { return input.begin(); }
    ...
    static void SetToEmpty(base::flat_map<K, V, Compare>* output) { output->clear(); }
};
