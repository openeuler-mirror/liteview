...
template <typename T, size_t Extent> struct ArrayTraits<base::span<T, Extent>> {
    ...
    static T& GetAt(base::span<T>& input, size_t index) { return input.data()[index]; }

    static const T& GetAt(const base::span<T>& input, size_t index) { return input.data()[index]; }
