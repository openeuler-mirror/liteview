...
public:
    ...
    // returning.
    Connector(ScopedMessagePipeHandle message_pipe, ConnectorConfig config, scoped_refptr<base::SequencedTaskRunner> runner, const char* interface_name = "unknown interface");
    ...
    void set_force_immediate_dispatch(bool force) { force_immediate_dispatch_ = force; }
    ...
    bool during_sync_handle_watcher_callback() const { return sync_handle_watcher_callback_count_ > 0; }
...
private:
    ...
    bool should_dispatch_messages_immediately() const { return force_immediate_dispatch_ || during_sync_handle_watcher_callback(); }

    base::OnceClosure connection_error_handler_;

    // ---mojo lite:
    static void OnConnectorAcceptMsg(base::WeakPtr<mojo::Connector> connector, MojoHandle handle, size_t id, mojo::MessageReceiver* incomingReceiver, std::vector<uint8_t>* data);
