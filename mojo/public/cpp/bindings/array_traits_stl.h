...
template <typename T> struct ArrayTraits<std::unordered_set<T>> {
    ...
    static size_t GetSize(const std::unordered_set<T>& input) { return input.size(); }

    static ConstIterator GetBegin(const std::unordered_set<T>& input) { return input.begin(); }
...
template <typename T> struct ArrayTraits<std::vector<T>> {
    ...
    static typename std::vector<T>::reference GetAt(std::vector<T>& input, size_t index) { return input[index]; }

    static typename std::vector<T>::const_reference GetAt(const std::vector<T>& input, size_t index) { return input[index]; }
...
template <typename T> struct ArrayTraits<std::set<T>> {
    ...
    static ConstIterator GetBegin(const std::set<T>& input) { return input.begin(); }
    static void AdvanceIterator(ConstIterator& iterator) { ++iterator; }
    static const T& GetValue(ConstIterator& iterator) { return *iterator; }
};
...
template <typename T> struct ArrayTraits<base::flat_set<T>> {
    ...
    static ConstIterator GetBegin(const base::flat_set<T>& input) { return input.begin(); }
    static void AdvanceIterator(ConstIterator& iterator) { ++iterator; }
    static const T& GetValue(ConstIterator& iterator) { return *iterator; }
};
...
template <typename K, typename V> MapValuesArrayView<K, V> MapValuesToArray(const std::map<K, V>& map) { return MapValuesArrayView<K, V>(map); }
...
template <typename K, typename V> struct ArrayTraits<MapValuesArrayView<K, V>> {
    ...
    static size_t GetSize(const MapValuesArrayView<K, V>& input) { return input.map.size(); }
    static ConstIterator GetBegin(const MapValuesArrayView<K, V>& input) { return input.map.begin(); }
    static void AdvanceIterator(ConstIterator& iterator) { ++iterator; }
...
template <typename T, size_t N> struct ArrayTraits<std::array<T, N>> {
    ...
    static const T& GetAt(const std::array<T, N>& input, size_t index) { return input[index]; }
    static T& GetAt(std::array<T, N>& input, size_t index) { return input[index]; }
