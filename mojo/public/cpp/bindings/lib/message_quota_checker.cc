...
NOINLINE void MaybeDumpWithoutCrashing(size_t total_quota_used, absl::optional<size_t> message_pipe_quota_used, int64_t seconds_since_construction, double average_write_rate,
    uint64_t messages_enqueued, uint64_t messages_dequeued, uint64_t messages_written)
{
...
int64_t ToSamplingInterval(base::TimeTicks when) { return (when - base::TimeTicks::UnixEpoch()).IntDiv(MessageQuotaChecker::DecayingRateAverage::kSamplingInterval); }
...
// static
MessageQuotaChecker::Configuration MessageQuotaChecker::GetConfigurationForTesting() { return GetConfiguration(); }

// static
scoped_refptr<MessageQuotaChecker> MessageQuotaChecker::MaybeCreateForTesting(const Configuration& config) { return MaybeCreateImpl(config); }
...
MessageQuotaChecker::DecayingRateAverage::DecayingRateAverage() { events_sampling_interval_ = ToSamplingInterval(base::TimeTicks::Now()); }
...
// static
base::TimeTicks MessageQuotaChecker::DecayingRateAverage::GetNextSamplingIntervalForTesting(base::TimeTicks when) { return FromSamplingInterval(ToSamplingInterval(when) + 1); }
