...
public:
    ...
    void Start() { runner_for_pauses_->PostTask(FROM_HERE, base::BindOnce(&RandomMojoDelays::PauseRandomBindingStateBases, base::Unretained(this))); }
    ...
    void AddBindingStateBase(scoped_refptr<base::SequencedTaskRunner> runner, base::WeakPtr<BindingStateBase> binding_state_base)
    {
        runner_for_pauses_->PostTask(
            FROM_HERE, base::BindOnce(&RandomMojoDelays::AddBindingStateBaseInternal, base::Unretained(this), std::move(runner), std::move(binding_state_base)));
    }
...
private:
    ...
    void ResumeFrozenBindingStateBasesOnTaskRunner(BindingList binding_state_bases, BindingList paused_binding_state_bases)
    {
        ...
        if (!paused_binding_state_bases.empty()) {
            ...
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
                base::BindOnce(
                    &RandomMojoDelays::ResumeFrozenBindingStateBasesOnTaskRunner, base::Unretained(this), std::move(binding_state_bases), std::move(paused_binding_state_bases)),
                kMillisecondsToResume);
            return;
        }
        // Re-attach the bindings to the global map for future pausing.
        runner_for_pauses_->PostTask(
            FROM_HERE, base::BindOnce(&RandomMojoDelays::AddBindingStateBaseList, base::Unretained(this), base::SequencedTaskRunnerHandle::Get(), std::move(binding_state_bases)));
    }
    ...
    void PauseRandomBindingStateBasesOnTaskRunner(BindingList binding_state_bases)
    {
        ...
        if (paused_binding_state_bases.size() > 0) {
            ...
            runner_for_pauses_->PostTask(FROM_HERE,
                base::BindOnce(&RandomMojoDelays::AddBindingStateBaseList, base::Unretained(this), base::SequencedTaskRunnerHandle::Get(), std::move(binding_state_bases)));
        }
    }
    ...
    void PauseRandomBindingStateBases()
    {
        ...
        while (map_it != binding_state_base_map_.end()) {
            ...
            map_it->first->PostTask(FROM_HERE, base::BindOnce(&RandomMojoDelays::PauseRandomBindingStateBasesOnTaskRunner, base::Unretained(this), std::move(map_it->second)));
...
void BeginRandomMojoDelays() { internal::GetRandomMojoDelays().Start(); }
