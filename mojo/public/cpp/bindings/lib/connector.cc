...
public:
    RunLoopNestingObserver() { base::RunLoop::AddNestingObserverOnCurrentThread(this); }
    ...
    ~RunLoopNestingObserver() override { base::RunLoop::RemoveNestingObserverOnCurrentThread(this); }
...
bool Connector::SimulateReadMessage(ScopedMessageHandle message) { return DispatchMessage(std::move(message)); }

void Connector::OnWatcherHandleReady(MojoResult result) { OnHandleReadyInternal(result); }
...
void Connector::WaitToReadMore()
{
    ...
    MojoResult rv = handle_watcher_->Watch(message_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE, base::BindRepeating(&Connector::OnWatcherHandleReady, base::Unretained(this)));
...
MojoResult Connector::ReadMessage(ScopedMessageHandle& message) { return ReadMessageNew(message_pipe_.get(), &message, MOJO_READ_MESSAGE_FLAG_NONE); }
