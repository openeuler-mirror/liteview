...
public:
    ...
    // such errors.
    ValidationContext(const void* data, size_t data_num_bytes, size_t num_handles, size_t num_associated_endpoint_handles, Message* message = nullptr, const char* description = "",
        int stack_depth = 0, ValidatorType validator_type = kUnspecifiedValidator);
    ...
    [[nodiscard]] bool ExceedsMaxDepth() { return stack_depth_ > kMaxRecursionDepth; }
...
private:
    bool InternalIsValidRange(uintptr_t begin, uintptr_t end) const { return end > begin && begin >= data_begin_ && end <= data_end_; }
