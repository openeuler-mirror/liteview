...
bool Buffer::AttachHandles(std::vector<ScopedHandle>* handles)
{
    ...
    uint32_t new_size = 0;
    MojoResult rv = MojoAppendMessageData(message_.value(), 0, reinterpret_cast<MojoHandle*>(handles->data()), static_cast<uint32_t>(handles->size()), nullptr, &data_, &new_size);
