...
public:
    ...
    const absl::optional<DisconnectReason>& disconnect_reason() const { return disconnect_reason_; }
    ...
    bool SyncWatchExclusive(uint64_t request_id) override { return router_->ExclusiveSyncWaitForReply(id_, request_id); }
...
scoped_refptr<MultiplexRouter> MultiplexRouter::Create(
    ScopedMessagePipeHandle message_pipe, Config config, bool set_interface_id_namespace_bit, scoped_refptr<base::SequencedTaskRunner> runner, const char* primary_interface_name)
{
    return base::MakeRefCounted<MultiplexRouter>(base::PassKey<MultiplexRouter>(), std::move(message_pipe), config, set_interface_id_namespace_bit, runner, primary_interface_name);
}
...
void MultiplexRouter::SetIncomingMessageFilter(std::unique_ptr<MessageFilter> filter) { dispatcher_.SetFilter(std::move(filter)); }

void MultiplexRouter::SetConnectionGroup(ConnectionGroup::Ref ref) { connector_.SetConnectionGroup(std::move(ref)); }
...
void MultiplexRouter::PauseIncomingMethodCallProcessing() { PauseInternal(/*must_resume_manually=*/true); }
...
void MultiplexRouter::FlushAsync(AsyncFlusher flusher) { control_message_proxy_.FlushAsync(std::move(flusher)); }

void MultiplexRouter::PausePeerUntilFlushCompletes(PendingFlush flush) { control_message_proxy_.PausePeerUntilFlushCompletes(std::move(flush)); }
...
void MultiplexRouter::EnableBatchDispatch() { connector_.set_force_immediate_dispatch(true); }
...
bool MultiplexRouter::Accept(Message* message)
{
    ...
    ClientCallBehavior client_call_behavior = connector_.during_sync_handle_watcher_callback() ? ALLOW_DIRECT_CLIENT_CALLS_FOR_SYNC_MESSAGES : ALLOW_DIRECT_CLIENT_CALLS;
...
bool MultiplexRouter::ProcessIncomingMessage(MessageWrapper* message_wrapper, ClientCallBehavior client_call_behavior, base::SequencedTaskRunner* current_task_runner)
{
    DCHECK(!current_task_runner || current_task_runner->RunsTasksInCurrentSequence());
    DCHECK(!paused_);
