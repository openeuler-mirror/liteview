...
void ReportValidationError(ValidationContext* context, ValidationError error, const char* description)
{
    ...
    if (description) {
        ...
        if (context->message()) {
            context->message()->NotifyBadMessage(base::StringPrintf("Validation failed for %s [%s]", context->GetFullDescription().c_str(), ValidationErrorToString(error)));
        }
    }
}
...
ScopedSuppressValidationErrorLoggingForTests ::~ScopedSuppressValidationErrorLoggingForTests() { g_suppress_logging = was_suppressed_; }
...
void RecordInvalidStringDeserialization() { base::UmaHistogramBoolean("Mojo.InvalidUTF8String", false); }
