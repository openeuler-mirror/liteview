...
public:
    ...
    void SetAssociationEventHandler(AssociationEventCallback handler)
    {
        ...
        if (!pending_association_) {
            ...
            runner_->PostTask(FROM_HERE, base::BindOnce(&ScopedInterfaceEndpointHandle::State::RunAssociationEventHandler, this, runner_, PEER_CLOSED_BEFORE_ASSOCIATION));
        }
    }
...
ScopedInterfaceEndpointHandle::~ScopedInterfaceEndpointHandle() { state_->Close(absl::nullopt); }
...
bool ScopedInterfaceEndpointHandle::is_valid() const { return state_->is_valid(); }

bool ScopedInterfaceEndpointHandle::pending_association() const { return state_->pending_association(); }

InterfaceId ScopedInterfaceEndpointHandle::id() const { return state_->id(); }

AssociatedGroupController* ScopedInterfaceEndpointHandle::group_controller() const { return state_->group_controller(); }

const absl::optional<DisconnectReason>& ScopedInterfaceEndpointHandle::disconnect_reason() const { return state_->disconnect_reason(); }

void ScopedInterfaceEndpointHandle::SetAssociationEventHandler(AssociationEventCallback handler) { state_->SetAssociationEventHandler(std::move(handler)); }

void ScopedInterfaceEndpointHandle::reset() { ResetInternal(absl::nullopt); }
