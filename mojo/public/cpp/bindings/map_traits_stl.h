...
template <typename K, typename V, typename Compare> struct MapTraits<std::map<K, V, Compare>> {
    ...
    static size_t GetSize(const std::map<K, V, Compare>& input) { return input.size(); }

    static ConstIterator GetBegin(const std::map<K, V, Compare>& input) { return input.begin(); }
    static Iterator GetBegin(std::map<K, V, Compare>& input) { return input.begin(); }
...
template <typename K, typename V> struct MapTraits<std::unordered_map<K, V>> {
    ...
    static size_t GetSize(const std::unordered_map<K, V>& input) { return input.size(); }

    static ConstIterator GetBegin(const std::unordered_map<K, V>& input) { return input.begin(); }
    static Iterator GetBegin(std::unordered_map<K, V>& input) { return input.begin(); }
...
template <typename K, typename V> struct MapTraits<std::vector<std::pair<K, V>>> {
    ...
    static ConstIterator GetBegin(const Container& input) { return input.begin(); }

    static void AdvanceIterator(ConstIterator& iterator) { iterator++; }

    static const K& GetKey(const ConstIterator& iterator) { return iterator->first; }

    static const V& GetValue(const ConstIterator& iterator) { return iterator->second; }
};
