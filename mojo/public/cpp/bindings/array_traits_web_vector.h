...
template <typename U> struct ArrayTraits<blink::WebVector<U>> {
    ...
    static size_t GetSize(const blink::WebVector<U>& input) { return input.size(); }

    static U* GetData(blink::WebVector<U>& input) { return input.Data(); }

    static const U* GetData(const blink::WebVector<U>& input) { return input.Data(); }

    static U& GetAt(blink::WebVector<U>& input, size_t index) { return input[index]; }

    static const U& GetAt(const blink::WebVector<U>& input, size_t index) { return input[index]; }
