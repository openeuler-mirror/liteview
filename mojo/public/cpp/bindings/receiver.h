...
public:
    ...
    bool is_bound() const { return receiver_impl_.is_bound(); }
    ...
    void set_disconnect_handler(base::OnceClosure handler) { receiver_impl_.set_connection_error_handler(std::move(handler)); }
    ...
    void set_connection_error_handler_is_sync() { receiver_impl_.set_connection_error_handler_is_sync(); } 
    ...
    void reset() { receiver_impl_.Close(); }

    // Similar to the method above, but also specifies a disconnect reason.
    void ResetWithReason(uint32_t custom_reason_code, base::StringPiece description) { receiver_impl_.CloseWithReason(custom_reason_code, description); }
...
//     }
    ...
    void Resume() { receiver_impl_.ResumeIncomingMethodCallProcessing(); }

    // Blocks the calling thread until a new message arrives and is dispatched
    // to the bound implementation.
    bool WaitForIncomingCall() { return receiver_impl_.WaitForIncomingMethodCall(); }
    ...
    // example usage.
    void PauseRemoteCallbacksUntilFlushCompletes(PendingFlush flush) { receiver_impl_.PauseRemoteCallbacksUntilFlushCompletes(std::move(flush)); }
    ...
    void FlushAsyncWithFlusher(AsyncFlusher flusher) { receiver_impl_.FlushAsync(std::move(flusher)); }
    ...
    void FlushForTesting() { receiver_impl_.FlushForTesting(); }

    // Exposed for testing, should not generally be used.
    void EnableTestingMode() { receiver_impl_.EnableTestingMode(); }
    ...
    // possible.
    [[nodiscard]] ImplPointerType SwapImplForTesting(ImplPointerType new_impl) { return receiver_impl_.SwapImplForTesting(std::move(new_impl)); }
    ...
    void ReportBadMessage(base::StringPiece error) { GetBadMessageCallback().Run(error); }
    ...
    ReportBadMessageCallback GetBadMessageCallback() { return receiver_impl_.GetBadMessageCallback(); }

    // DO NOT USE. Exposed only for internal use and for testing.
    internal::ReceiverImpl<Interface, ImplRefTraits>* internal_state() { return &receiver_impl_; }
...
using AssociatedReceiver = Receiver<Interface, ImplRefTraits>;
#endif
