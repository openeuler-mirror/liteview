...
{
    MojoResult rv = high_watcher_.Watch(handle, signals, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, base::BindRepeating(&HandleSignalTracker::OnNotify, base::Unretained(this)));
    DCHECK_EQ(MOJO_RESULT_OK, rv);

    rv = low_watcher_.Watch(handle, signals, MOJO_TRIGGER_CONDITION_SIGNALS_UNSATISFIED, base::BindRepeating(&HandleSignalTracker::OnNotify, base::Unretained(this)));
    DCHECK_EQ(MOJO_RESULT_OK, rv);
