...
public:
    ...
    // |signals| on |handle|.
    static scoped_refptr<Context> Create(base::WeakPtr<SimpleWatcher> watcher, scoped_refptr<base::SequencedTaskRunner> task_runner, TrapHandle trap_handle, Handle handle,
        MojoHandleSignals signals, MojoTriggerCondition condition, int watch_id, MojoResult* result, const char* handler_tag)
    {
...
private:
    ...
    void Notify(MojoResult result, MojoHandleSignalsState signals_state, MojoTrapEventFlags flags)
    {
        ...
        if (!(flags & MOJO_TRAP_EVENT_FLAG_WITHIN_API_CALL) && task_runner_->RunsTasksInCurrentSequence() && weak_watcher_ && weak_watcher_->is_default_task_runner_) {
...
SimpleWatcher::SimpleWatcher(const base::Location& from_here, ArmingPolicy arming_policy, scoped_refptr<base::SequencedTaskRunner> runner, const char* handler_tag)
