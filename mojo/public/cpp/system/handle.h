...
public:
    ...
    bool operator==(const ScopedHandleBase& other) const { return handle_.value() == other.get().value(); }
...
template <typename HandleType> inline ScopedHandleBase<HandleType> MakeScopedHandle(HandleType handle) { return ScopedHandleBase<HandleType>(handle); }
...
inline MojoResult CloseRaw(Handle handle) { return MojoClose(handle.value()); }
