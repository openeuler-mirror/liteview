...
public:
    ...
    SimpleWatcher(const base::Location& from_here, ArmingPolicy arming_policy, scoped_refptr<base::SequencedTaskRunner> runner = base::SequencedTaskRunnerHandle::Get(),
...
private:
    ...
    static void DiscardReadyState(const ReadyCallback& callback, MojoResult result, const HandleSignalsState& state) { callback.Run(result); }
