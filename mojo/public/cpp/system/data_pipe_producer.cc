...
private:
    ...
    void StartOnSequence(std::unique_ptr<DataSource> data_source)
    {
        ...
        if (producer_handle_.is_valid()) {
            ...
            watcher_->Watch(producer_handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&SequenceState::OnHandleReady, this));
        }
    }
...
void DataPipeProducer::InitializeNewRequest(CompletionCallback callback)
{
    ...
    sequence_state_ = new SequenceState(std::move(producer_), file_task_runner, base::BindOnce(&DataPipeProducer::OnWriteComplete, weak_factory_.GetWeakPtr(), std::move(callback)),
