...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::DictionaryValueDataView, base::Value::Dict> {
    static const base::Value::Dict& storage(const base::Value::Dict& in) { return in; }
...
template <> struct MapTraits<base::Value> {
    ...
    static const Value& GetValue(const Iterator& iterator) { return iterator->second; }
};

template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::DeprecatedDictionaryValueDataView, base::Value> {
    ...
    static bool Read(mojo_base::mojom::DeprecatedDictionaryValueDataView data, base::Value* value);
};
...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) UnionTraits<mojo_base::mojom::ValueDataView, base::Value> {
    ...
    static double double_value(const base::Value& value) { return value.GetDouble(); }

    static base::StringPiece string_value(const base::Value& value) { return value.GetString(); }

    static const std::vector<uint8_t>& binary_value(const base::Value& value) { return value.GetBlob(); }

    static const base::Value::Dict& dictionary_value(const base::Value& value) { return value.GetDict(); }

    static const base::Value::List& list_value(const base::Value& value) { return value.GetList(); }
