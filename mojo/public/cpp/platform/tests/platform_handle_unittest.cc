...
// Let |handle| retain ownership.
    ...
    std::string GetSharedMemoryContents(const PlatformHandle& handle)
    {
        ...
        auto generic_region
            = base::subtle::PlatformSharedMemoryRegion::Take(std::move(region_handle), base::subtle::PlatformSharedMemoryRegion::Mode::kUnsafe, kTestData.size(), shm_guid_);
...
TEST_P(PlatformHandleTest, BasicConstruction) { EXPECT_EQ(kTestData, GetObjectContents(test_handle())); }
