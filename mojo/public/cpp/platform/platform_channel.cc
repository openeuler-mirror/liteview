...
void CreateChannel(PlatformHandle* local_endpoint, PlatformHandle* remote_endpoint)
{
    ...
    *remote_endpoint = PlatformHandle(base::win::ScopedHandle(::CreateFileW(pipe_name.c_str(), kDesiredAccess, 0, &security_attributes, OPEN_EXISTING, kFlags, nullptr)));
...
void PlatformChannel::PrepareToPassRemoteEndpoint(HandlePassingInfo* info, std::string* value) { remote_endpoint_.PrepareToPass(*info, *value); }

void PlatformChannel::PrepareToPassRemoteEndpoint(HandlePassingInfo* info, base::CommandLine* command_line) { remote_endpoint_.PrepareToPass(*info, *command_line); }

void PlatformChannel::PrepareToPassRemoteEndpoint(base::LaunchOptions* options, base::CommandLine* command_line) { remote_endpoint_.PrepareToPass(*options, *command_line); }

void PlatformChannel::RemoteProcessLaunchAttempted() { remote_endpoint_.ProcessLaunchAttempted(); }

// static
PlatformChannelEndpoint PlatformChannel::RecoverPassedEndpointFromString(base::StringPiece value) { return PlatformChannelEndpoint::RecoverFromString(value); }
...
// static
bool PlatformChannel::CommandLineHasPassedEndpoint(const base::CommandLine& command_line) { return command_line.HasSwitch(kHandleSwitch); }
