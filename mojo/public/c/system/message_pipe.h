...
MOJO_SYSTEM_EXPORT MojoResult MojoSetMessageContext(MojoMessageHandle message, uintptr_t context, MojoMessageContextSerializer serializer, MojoMessageContextDestructor destructor,
...
MOJO_SYSTEM_EXPORT MojoResult MojoNotifyBadMessage(MojoMessageHandle message, const char* error, uint32_t error_num_bytes, const struct MojoNotifyBadMessageOptions* options);
...
#define MOJO_RECORD_CALL(handle, from) \
    MojoRecordCall(handle, _ReturnAddress(), from);
#else
#define MOJO_RECORD_CALL(handle, from) 
#endif
//---------
