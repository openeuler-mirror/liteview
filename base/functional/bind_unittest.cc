...
struct RepeatingTestConfig {
    ...
    template <typename F, typename... Args> static CallbackType<internal::MakeUnboundRunType<F, Args...>> Bind(F&& f, Args&&... args)
    {
...
TYPED_TEST(BindVariantsTest, ArgumentBinding)
{
    ...
    EXPECT_EQ(incomplete_ptr, TypeParam::Bind(&PolymorphicIdentity<IncompleteType*>, incomplete_ptr).Run());
...
TEST_F(BindTest, BindMoveOnlyVector)
{
    ...
    base::RepeatingCallback<MoveOnlyVector()> bound_cb = base::BindRepeating(&AcceptAndReturnMoveOnlyVector, Passed(&v));
    ...
    base::RepeatingCallback<MoveOnlyVector(MoveOnlyVector)> unbound_cb = base::BindRepeating(&AcceptAndReturnMoveOnlyVector);
...
TEST_F(BindTest, CapturelessLambda)
{
    ...
    EXPECT_TRUE((std::is_same<char(int, double), internal::ExtractCallableRunType<decltype(h)>>::value));

    EXPECT_EQ(42, BindRepeating([] { return 42; }).Run());
    EXPECT_EQ(42, BindRepeating([](int i) { return i * 7; }, 6).Run());

    int x = 1;
    base::RepeatingCallback<void(int)> cb = BindRepeating([](int* x, int i) { *x *= i; }, Unretained(&x));
    cb.Run(6);
    EXPECT_EQ(6, x);
    cb.Run(7);
    EXPECT_EQ(42, x);
...
TEST_F(BindTest, OnceCallback)
{
    ...
    static_assert(std::is_constructible<RepeatingClosure, const RepeatingClosure&>::value, "RepeatingClosure should be copyable.");
    static_assert(std::is_assignable<RepeatingClosure, const RepeatingClosure&>::value, "RepeatingClosure should be copy-assignable.");

    // Move constructor and assignment of RepeatingCallback.
    static_assert(std::is_constructible<RepeatingClosure, RepeatingClosure&&>::value, "RepeatingClosure should be movable.");
    static_assert(std::is_assignable<RepeatingClosure, RepeatingClosure&&>::value, "RepeatingClosure should be move-assignable");

    // Conversions from OnceCallback to RepeatingCallback.
    static_assert(!std::is_constructible<RepeatingClosure, const OnceClosure&>::value, "OnceClosure should not be convertible to RepeatingClosure.");
    static_assert(!std::is_assignable<RepeatingClosure, const OnceClosure&>::value, "OnceClosure should not be convertible to RepeatingClosure.");

    // Destructive conversions from OnceCallback to RepeatingCallback.
    static_assert(!std::is_constructible<RepeatingClosure, OnceClosure&&>::value, "OnceClosure should not be convertible to RepeatingClosure.");
    static_assert(!std::is_assignable<RepeatingClosure, OnceClosure&&>::value, "OnceClosure should not be convertible to RepeatingClosure.");

    // Copy constructor and assignment of OnceCallback.
    static_assert(!std::is_constructible<OnceClosure, const OnceClosure&>::value, "OnceClosure should not be copyable.");
    static_assert(!std::is_assignable<OnceClosure, const OnceClosure&>::value, "OnceClosure should not be copy-assignable");

    // Move constructor and assignment of OnceCallback.
    static_assert(std::is_constructible<OnceClosure, OnceClosure&&>::value, "OnceClosure should be movable.");
    static_assert(std::is_assignable<OnceClosure, OnceClosure&&>::value, "OnceClosure should be move-assignable.");

    // Conversions from RepeatingCallback to OnceCallback.
    static_assert(std::is_constructible<OnceClosure, const RepeatingClosure&>::value, "RepeatingClosure should be convertible to OnceClosure.");
    static_assert(std::is_assignable<OnceClosure, const RepeatingClosure&>::value, "RepeatingClosure should be convertible to OnceClosure.");

    // Destructive conversions from RepeatingCallback to OnceCallback.
    static_assert(std::is_constructible<OnceClosure, RepeatingClosure&&>::value, "RepeatingClosure should be convertible to OnceClosure.");
    static_assert(std::is_assignable<OnceClosure, RepeatingClosure&&>::value, "RepeatingClosure should be covretible to OnceClosure.");
...
TEST_F(BindTest, WindowsCallingConventions)
{
    ...
    MethodHolder obj;
    auto stdcall_method_cb = BindRepeating(&MethodHolder::Func, base::Unretained(&obj), 1);
    EXPECT_EQ(1, stdcall_method_cb.Run());

    const MethodHolder const_obj;
    auto stdcall_const_method_cb = BindRepeating(&MethodHolder::ConstFunc, base::Unretained(&const_obj), 1);
...
TEST_F(BindTest, BindNoexcept)
{
    ...
    EXPECT_EQ(42, base::BindOnce(&BindTest::NoexceptMethod, base::Unretained(this)).Run());
    EXPECT_EQ(42, base::BindOnce(&BindTest::ConstNoexceptMethod, base::Unretained(this)).Run());
...
public:
    ...
    void TearDown() override
    {
        ...
        allocator::InstallUnretainedDanglingRawPtrChecks();
        allocator_.root()->PurgeMemory(partition_alloc::PurgeFlags::kDecommitEmptySlotSpans | partition_alloc::PurgeFlags::kDiscardUnusedSystemPages);
    ...
    // root.
    template <typename T, typename... Args> raw_ptr<T> Alloc(Args&&... args)
    {
        void* ptr = allocator_.root()->Alloc(sizeof(T), "");
        T* p = new (reinterpret_cast<T*>(ptr)) T(std::forward<Args>(args)...);
        return raw_ptr<T>(p);
    }
    template <typename T> void Free(raw_ptr<T>& ptr)
    {
...
TEST_F(BindUnretainedDanglingTest, UnretainedWeakReceiverValidNoDangling)
{
    ...
    auto callback = base::BindOnce(&ClassWithWeakPtr::RawPtrArg, r->GetWeakPtr(), base::Unretained(p));
