...
template <typename Functor, typename... Args> inline OnceCallback<internal::MakeUnboundRunType<Functor, Args...>> BindOnce(Functor&& functor, Args&&... args)
{
    static_assert(
        !internal::IsOnceCallback<std::decay_t<Functor>>() || (std::is_rvalue_reference<Functor&&>() && !std::is_const<std::remove_reference_t<Functor>>()),
        "BindOnce requires non-const rvalue for OnceCallback binding."
        " I.e.: base::BindOnce(std::move(callback)).");
    static_assert(std::conjunction<internal::AssertBindArgIsNotBasePassed<std::decay_t<Args>>...>::value,
        "Use std::move() instead of base::Passed() with base::BindOnce()");

    return internal::BindImpl<OnceCallback>(std::forward<Functor>(functor), std::forward<Args>(args)...);
}

// Bind as RepeatingCallback.
template <typename Functor, typename... Args>
inline RepeatingCallback<internal::MakeUnboundRunType<Functor, Args...>> BindRepeating(Functor&& functor, Args&&... args)
{
    static_assert(!internal::IsOnceCallback<std::decay_t<Functor>>(), "BindRepeating cannot bind OnceCallback. Use BindOnce with std::move().");

    return internal::BindImpl<RepeatingCallback>(std::forward<Functor>(functor), std::forward<Args>(args)...);
}
...
// pointer as receiver.
template <typename T> inline internal::UnretainedWrapper<T> Unretained(T* o)
{
    return internal::UnretainedWrapper<T>(o);
}

template <typename T, typename I> inline internal::UnretainedWrapper<T> Unretained(const raw_ptr<T, I>& o)
{
    return internal::UnretainedWrapper<T>(o);
}

template <typename T, typename I> inline internal::UnretainedWrapper<T> Unretained(raw_ptr<T, I>&& o)
{
    return internal::UnretainedWrapper<T>(std::move(o));
}

template <typename T, typename I> inline auto Unretained(const raw_ref<T, I>& o)
{
    return internal::UnretainedRefWrapper(o);
}

template <typename T, typename I> inline auto Unretained(raw_ref<T, I>&& o)
{
...
// the one above.
template <typename T> inline internal::UnretainedWrapper<T, DisableDanglingPtrDetection> UnsafeDangling(T* o)
{
    return internal::UnretainedWrapper<T, DisableDanglingPtrDetection>(o);
}

template <typename T, typename I> internal::UnretainedWrapper<T, DisableDanglingPtrDetection> UnsafeDangling(const raw_ptr<T, I>& o)
{
    return internal::UnretainedWrapper<T, DisableDanglingPtrDetection>(o);
}

template <typename T, typename I> internal::UnretainedWrapper<T, DisableDanglingPtrDetection> UnsafeDangling(raw_ptr<T, I>&& o)
{
    return internal::UnretainedWrapper<T, DisableDanglingPtrDetection>(std::move(o));
}
...
template <typename T> inline internal::UnretainedWrapper<T, DanglingUntriaged> UnsafeDanglingUntriaged(T* o)
{
    return internal::UnretainedWrapper<T, DanglingUntriaged>(o);
}

template <typename T, typename I> internal::UnretainedWrapper<T, DanglingUntriaged> UnsafeDanglingUntriaged(const raw_ptr<T, I>& o)
{
    return internal::UnretainedWrapper<T, DanglingUntriaged>(o);
}

template <typename T, typename I> internal::UnretainedWrapper<T, DanglingUntriaged> UnsafeDanglingUntriaged(raw_ptr<T, I>&& o)
{
...
template <typename T, std::enable_if_t<!std::is_lvalue_reference_v<T>>* = nullptr> inline internal::PassedWrapper<T> Passed(T&& scoper)
{
...
template <typename R, typename... Args> base::mac::ScopedBlock<R (^)(Args...)> RetainBlock(R (^block)(Args...))
{
    return base::mac::ScopedBlock<R (^)(Args...)>(block, base::scoped_policy::RETAIN);
}
