...
private:
    template <typename Functor, typename FunctorReturnType = typename internal::BindTypeHelper<Functor>::ReturnType,
        typename FunctorArgsAsTypeList = typename internal::BindTypeHelper<Functor>::RunParamsList>
    using EnableIfCompatible
        = std::enable_if_t<std::is_convertible_v<FunctorReturnType, R> && std::is_same_v<FunctorArgsAsTypeList, internal::TypeList<Args...>>>;
...
public:
    ...
    absl::FunctionRef<R(Args...)> ToAbsl() const
    {
        return wrapped_func_ref_;
    }

    // In Chrome, converting to `absl::FunctionRef` should be explicitly done
    // through `ToAbsl()`.
    template <typename Signature> operator absl::FunctionRef<Signature>() = delete;
