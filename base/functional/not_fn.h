...
template <typename F> struct NotFnImpl {
    F f;

    template <typename... Args> constexpr decltype(auto) operator()(Args&&... args) & noexcept
    {
...
template <typename F> constexpr internal::NotFnImpl<std::decay_t<F>> not_fn(F&& f)
{
