...
private:
    ...
    static bool IsCancelled(const internal::BindStateBase*, internal::BindStateBase::CancellationQueryMode mode)
    {
        switch (mode) {
...
TEST_F(CallbackTest, Types)
{
    ...
    static_assert(std::is_same<bool(float, short), RepeatingCallbackT::RunType>::value, "");
}
...
private:
    ...
    static int Outer(std::string* s, std::unique_ptr<int> a, std::unique_ptr<int> b)
    {
        *s += "Outer";
        *s += base::NumberToString(*a) + base::NumberToString(*b);
        return *a + *b;
    }
    template <typename R2 = R, std::enable_if_t<!std::is_void<R2>::value, int> = 0> static int Outer(std::string* s, int a, int b)
    {
        *s += "Outer";
        *s += base::NumberToString(a) + base::NumberToString(b);
        return a + b;
    }
    template <typename R2 = R, std::enable_if_t<!std::is_void<R2>::value, int> = 0> static int Outer(std::string* s)
    {
        *s += "Outer";
        *s += "None";
        return 99;
    ...
    template <typename R2 = R, std::enable_if_t<std::is_void<R2>::value, int> = 0>
    static void Outer(std::string* s, std::unique_ptr<int> a, std::unique_ptr<int> b)
    {
        *s += "Outer";
        *s += base::NumberToString(*a) + base::NumberToString(*b);
    }
    template <typename R2 = R, std::enable_if_t<std::is_void<R2>::value, int> = 0> static void Outer(std::string* s, int a, int b)
    {
        *s += "Outer";
        *s += base::NumberToString(a) + base::NumberToString(b);
    }
    template <typename R2 = R, std::enable_if_t<std::is_void<R2>::value, int> = 0> static void Outer(std::string* s)
    {
        *s += "Outer";
        *s += "None";
    }

    template <typename OuterR, typename InnerR, std::enable_if_t<!std::is_void<OuterR>::value, int> = 0,
    ...
    template <typename OuterR, typename InnerR, std::enable_if_t<std::is_void<OuterR>::value, int> = 0, std::enable_if_t<!std::is_void<InnerR>::value, int> = 0>
