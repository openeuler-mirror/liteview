...
public:
    ...
    using DeprecatedConstListView = CheckedContiguousConstRange<DeprecatedListStorage>;
    ...
    public:
        ...
        template <typename T> Value* Set(StringPiece, const T*) = delete;
        ...
        template <typename T> Value* SetByDottedPath(StringPiece, const T*) = delete;
    ...
    // DEPRECATED: prefer `Value::List::Append()`.
    void Append(bool value);
    template <typename T> void Append(const T* ptr) = delete;
    // DEPRECATED: prefer `Value::List::Append()`.
    void Append(int value);
    // DEPRECATED: prefer `Value::List::Append()`.
    void Append(double value);
    ...
    const Value* FindPathOfType(std::initializer_list<StringPiece> path, Type type) const;
...
protected:
    ...
    {
        return GetDict().storage_;
    }
    LegacyDictStorage& dict()
    {
        return GetDict().storage_;
    }
    const ListStorage& list() const
    {
        return GetList().storage_;
    }
    ListStorage& list()
    {
        return GetList().storage_;
...
private:
    using ViewType = absl::variant<absl::monostate, bool, int, Value::DoubleStorage, StringPiece, std::reference_wrapper<const Value::BlobStorage>,
...
BASE_EXPORT inline std::ostream& operator<<(std::ostream& out, const DictionaryValue& value)
{
    return out << static_cast<const Value&>(value);
}

BASE_EXPORT inline std::ostream& operator<<(std::ostream& out, const ListValue& value)
{
    return out << static_cast<const Value&>(value);
}
