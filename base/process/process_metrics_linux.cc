...
bool ReadProcFileToTrimmedStringPairs(pid_t pid, StringPiece filename, StringPairs* key_value_pairs)
{
...
size_t ReadProcStatusAndGetFieldAsSizeT(pid_t pid, StringPiece field)
{
    ...
    for (const auto& pair : pairs) {
        ...
        std::vector<StringPiece> split_value_str = SplitStringPiece(value_str, " ", TRIM_WHITESPACE, SPLIT_WANT_ALL);
...
bool ReadProcStatusAndGetFieldAsUint64(pid_t pid, StringPiece field, uint64_t* result)
{
    StringPairs pairs;
    if (!ReadProcFileToTrimmedStringPairs(pid, "status", &pairs))
        return false;
...
{
    return internal::GetProcStatsFieldAsInt64(proc_stats, internal::VM_UTIME) + internal::GetProcStatsFieldAsInt64(proc_stats, internal::VM_STIME);
}
...
{
    FilePath time_in_state_path = internal::GetProcPidDir(GetCurrentProcId()).Append("task").Append(NumberToString(GetCurrentProcId())).Append("time_in_state");
    std::string contents;
...
bool ProcessMetrics::GetCumulativeCPUUsagePerThread(CPUUsagePerThread& cpu_per_thread)
{
    ...
    internal::ForEachProcessTask(process_, [&cpu_per_thread](PlatformThreadId tid, const FilePath& task_path) {
        ...
        TimeDelta thread_time = internal::ClockTicksToTimeDelta(ParseTotalCPUTimeFromStats(proc_stats));
...
bool ProcessMetrics::GetPerThreadCumulativeCPUTimeInState(TimeInStatePerThread& time_in_state_per_thread)
{
    ...
    bool success = false;
    internal::ForEachProcessTask(process_, [&time_in_state_per_thread, &success, this](PlatformThreadId tid, const FilePath& task_path) {
...
size_t GetSystemCommitCharge()
{
    ...
    if (!GetSystemMemoryInfo(&meminfo))
        return 0;
    return checked_cast<size_t>(meminfo.total - meminfo.free - meminfo.buffers - meminfo.cached);
}
...
bool ProcessMetrics::ParseProcTimeInState(const std::string& content, PlatformThreadId tid, TimeInStatePerThread& time_in_state_per_thread)
{
        ...
        if (!isdigit(next_char)) {
            ...
            if (time > 0) {
                time_in_state_per_thread.push_back(
                    { tid, current_core_type, current_core_index, frequency, internal::ClockTicksToTimeDelta(checked_cast<int64_t>(time)) });
            }
        } else {
...
bool ParseProcVmstat(StringPiece vmstat_data, VmStatInfo* vmstat)
{
    ...
    for (const StringPiece& line : SplitStringPiece(vmstat_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
        std::vector<StringPiece> tokens = SplitStringPiece(line, " ", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
        if (tokens.size() != 2)
            continue;
...
bool GetSystemDiskInfo(SystemDiskInfo* diskinfo)
{
    ...
    std::vector<StringPiece> diskinfo_lines = SplitStringPiece(diskinfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
    ...
    for (const StringPiece& line : diskinfo_lines) {
        std::vector<StringPiece> disk_fields = SplitStringPiece(line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
bool ParseZramMmStat(StringPiece mm_stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<StringPiece> tokens = SplitStringPiece(mm_stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 7) {
...
bool ParseZramStat(StringPiece stat_data, SwapInfo* swap_info)
{
    ...
    std::vector<StringPiece> tokens = SplitStringPiece(stat_data, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() < 11) {
...
int ProcessMetrics::GetIdleWakeupsPerSecond()
{
    ...
    return ReadProcStatusAndGetFieldAsUint64(process_, kSwitchStat, &num_switches) ? CalculateIdleWakeupsPerSecond(num_switches) : 0;
}
#endif // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_AIX)
