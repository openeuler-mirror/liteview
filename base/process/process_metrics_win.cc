...
inline BOOL QueryProcessCycleTimeXp(HANDLE ProcessHandle, PULONG64 CycleTime)
{
    typedef BOOL(__stdcall * PFN_QueryProcessCycleTime)(HANDLE ProcessHandle, PULONG64 CycleTime);
    static PFN_QueryProcessCycleTime s_QueryProcessCycleTime = (PFN_QueryProcessCycleTime)-1;

    if ((PFN_QueryProcessCycleTime)-1 == s_QueryProcessCycleTime) {
        HMODULE hModule = GetModuleHandle(L"kernel32.dll");
        if (hModule)
            s_QueryProcessCycleTime = (PFN_QueryProcessCycleTime)GetProcAddress(hModule, "QueryProcessCycleTime");
    }

    if (s_QueryProcessCycleTime)
        return s_QueryProcessCycleTime(ProcessHandle, CycleTime);

    return FALSE;
}
#else
#define QueryProcessCycleTimeXp QueryProcessCycleTime
#endif
...
TimeDelta ProcessMetrics::GetPreciseCumulativeCPUUsage()
{
    ...
    if (!QueryProcessCycleTimeXp(process_.get(), &process_cycle_time)) {
        // NOTREACHED();
        return TimeDelta();
    }
...
ProcessMetrics::ProcessMetrics(ProcessHandle process)
{
    if (process) {
        ...
        BOOL result = ::DuplicateHandle(::GetCurrentProcess(), process, ::GetCurrentProcess(), &duplicate_handle, PROCESS_QUERY_INFORMATION, FALSE, 0);
        DPCHECK(result);
...
size_t GetSystemCommitCharge()
{
    ...
    if (!GetPerformanceInfoXp(&info, sizeof(info))) {
...
Value SystemPerformanceInfo::ToValue() const
{
    ...
    result.SetDoubleKey("read_transfer_count", strict_cast<double>(read_transfer_count));
    result.SetDoubleKey("write_transfer_count", strict_cast<double>(write_transfer_count));
    result.SetDoubleKey("other_transfer_count", strict_cast<double>(other_transfer_count));
    result.SetDoubleKey("read_operation_count", strict_cast<double>(read_operation_count));
    result.SetDoubleKey("write_operation_count", strict_cast<double>(write_operation_count));
    result.SetDoubleKey("other_operation_count", strict_cast<double>(other_operation_count));
    result.SetDoubleKey("pagefile_pages_written", strict_cast<double>(pagefile_pages_written));
    result.SetDoubleKey("pagefile_pages_write_ios", strict_cast<double>(pagefile_pages_write_ios));
...
BASE_EXPORT bool GetSystemPerformanceInfo(SystemPerformanceInfo* info)
{
    static const auto query_system_information_ptr
        = reinterpret_cast<decltype(&::NtQuerySystemInformation)>(GetProcAddress(GetModuleHandle(L"ntdll.dll"), "NtQuerySystemInformation"));
    ...
    SYSTEM_PERFORMANCE_INFORMATION counters = {};
    {
        ...
        if (query_system_information_ptr(::SystemPerformanceInformation, &counters, sizeof(SYSTEM_PERFORMANCE_INFORMATION), nullptr) != STATUS_SUCCESS) {
            return false;
        }
    }
