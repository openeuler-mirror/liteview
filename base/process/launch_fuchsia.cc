...
Process LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options)
{
    ...
    std::unique_ptr<char*[]> new_environ = internal::AlterEnvironment(old_environ, environ_modifications);
    ...
    zx_status_t status = fdio_spawn_etc(job->get(), spawn_flags, argv_cstr[0], argv_cstr.data(), new_environ.get(), spawn_actions.size(), spawn_actions.data(),
