...
std::string GetProcessCpuCgroup(const base::Process& process)
{
    ...
    std::vector<base::StringPiece> lines = SplitStringPiece(proc, "\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    for (const auto& line : lines) {
        std::vector<base::StringPiece> fields = SplitStringPiece(line, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        if (fields.size() != 3U) {
            continue;
        }
...
#if !BUILDFLAG(                                                                                                                                                \
    IS_ANDROID) // Cannot read other processes' creation time on                                                                                    \
    // Android.
