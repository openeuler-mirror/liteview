...
#if !BUILDFLAG(IS_OPENBSD) && BUILDFLAG(USE_ALLOCATOR_SHIM) && !defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
...
TEST_F(OutOfMemoryHandledTest, UncheckedCalloc)
{
    ...
    EXPECT_TRUE(base::UncheckedCalloc(kSafeCallocItems, kSafeCallocSize, &value_));
    EXPECT_TRUE(value_ != nullptr);
