...
BASE_FEATURE(kMacAllowBackgroundingProcesses, "MacAllowBackgroundingProcesses", FEATURE_DISABLED_BY_DEFAULT);
...
bool Process::IsProcessBackgrounded(PortProvider* port_provider) const
{
    ...
    MACH_LOG_IF(ERROR, result != KERN_SUCCESS, result) << "task_policy_get TASK_CATEGORY_POLICY";
...
bool Process::SetProcessBackgrounded(PortProvider* port_provider, bool background)
{
    DCHECK(IsValid());
    ...
    category_policy.role = background ? TASK_BACKGROUND_APPLICATION : TASK_FOREGROUND_APPLICATION;
    kern_return_t result = task_policy_set(task_port, TASK_CATEGORY_POLICY, reinterpret_cast<task_policy_t>(&category_policy), TASK_CATEGORY_POLICY_COUNT);
