...
struct kernel_sigaction {
    raw_ptr<void> k_sa_handler; // For this usage it only needs to be a generic pointer.
    unsigned long k_sa_flags;
    raw_ptr<void> k_sa_restorer; // For this usage it only needs to be a generic pointer.
...
CloneAndLongjmpInChild(int flags, pid_t* ptid, pid_t* ctid, jmp_buf* env)
{
    ...
    || defined(ARCH_CPU_PPC64_FAMILY) || defined(ARCH_CPU_LOONG_FAMILY) || defined(ARCH_CPU_RISCV_FAMILY)
