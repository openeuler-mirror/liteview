...
int responsibility_spawnattrs_setdisclaim(posix_spawnattr_t attrs, int disclaim) API_AVAILABLE(macosx(10.14));
} // extern "C"
...
Process LaunchProcess(const std::vector<std::string>& argv, const LaunchOptions& options)
{
    ...
    if (!options.environment.empty()) {
        owned_environ = internal::AlterEnvironment(new_environ, options.environment);
    ...
    pid_t pid;
    {
        ...
        AutoLockMaybe rendezvous_lock(has_mach_ports_for_rendezvous ? &MachPortRendezvousServer::GetInstance()->GetLock() : nullptr);
        ...
        if (has_mach_ports_for_rendezvous) {
            if (rv == 0) {
                ...
                MachPortRendezvousServer::GetInstance()->RegisterPortsForPid(pid, options.mach_ports_for_rendezvous);
            } else {
