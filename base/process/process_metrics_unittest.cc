...
TEST_F(SystemMetricsTest, TestNoNegativeCpuUsage)
{
    ...
    std::unique_ptr<ProcessMetrics> metrics(ProcessMetrics::CreateProcessMetrics(handle));
...
TEST(ProcessMetricsTest, ParseProcTimeInState)
{
    ...
    const char kStatThread123[] = "cpu0\n"
                                  "100000 4\n"
                                  "200000 5\n"
                                  "300000 0\n"
                                  "cpu4\n"
                                  "400000 3\n"
                                  "500000 2\n";
    EXPECT_TRUE(metrics->ParseProcTimeInState(kStatThread123, 123, time_in_state));
    ...
    const char kStatThread456[] = "cpu0\n"
                                  "\n" // extra empty line is fine.
                                  "1000000 10"; // missing "\n" at end is fine.
    EXPECT_TRUE(metrics->ParseProcTimeInState(kStatThread456, 456, time_in_state));
    ...
    EXPECT_FALSE(metrics->ParseProcTimeInState("100000 5\n" // no header
                                               "200000 6\n",
        123, time_in_state));
    EXPECT_FALSE(metrics->ParseProcTimeInState("cpu0\n"
                                               "100000\n", // no time value
        123, time_in_state));
    EXPECT_FALSE(metrics->ParseProcTimeInState("header0\n" // invalid header / line
                                               "100000 5\n",
        123, time_in_state));
    EXPECT_FALSE(metrics->ParseProcTimeInState("cpu0\n"
                                               "100000 5\n"
...
TEST(ProcessMetricsTest, GetChildOpenFdCount)
{
    ...
    Process child = SpawnMultiProcessTestChild(ChildMainString, child_command_line, LaunchOptions());
...
TEST(ProcessMetricsTestLinux, GetPerThreadCumulativeCPUTimeInState)
{
    ...
    std::string contents;
    {
        FilePath time_in_state_path = FilePath("/proc").Append(NumberToString(handle)).Append("task").Append(NumberToString(handle)).Append("time_in_state");
        expect_success = ReadFileToString(time_in_state_path, &contents) && StartsWith(contents, "cpu", CompareCase::SENSITIVE);
    }

    ProcessMetrics::TimeInStatePerThread prev_thread_times;
    EXPECT_EQ(metrics->GetPerThreadCumulativeCPUTimeInState(prev_thread_times), expect_success) << "time_in_state example contents: \n" << contents;
    ...
    EXPECT_EQ(metrics->GetPerThreadCumulativeCPUTimeInState(current_thread_times), expect_success);

    // Reported times should not decrease.
    for (const auto& entry : current_thread_times) {
        auto prev_it = ranges::find_if(prev_thread_times, [&entry](const ProcessMetrics::ThreadTimeInState& prev_entry) {
            return entry.thread_id == prev_entry.thread_id && entry.core_type == prev_entry.core_type
