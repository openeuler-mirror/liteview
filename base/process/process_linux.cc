...
struct CGroups {
    ...
    CGroups()
    {
        ...
        enabled = GetFileSystemType(foreground_file, &foreground_type) && GetFileSystemType(background_file, &background_type)
...
bool Process::SetProcessBackgrounded(bool background)
{
    ...
    if (CGroups::Get().enabled) {
        ...
        const FilePath file = background ? CGroups::Get().background_file : CGroups::Get().GetForegroundCgroupFile(unique_token_);
...
bool IsProcessBackgroundedCGroup(const StringPiece& cgroup_contents)
{
    ...
    std::vector<StringPiece> lines = SplitStringPiece(cgroup_contents, "\n", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
ProcessId Process::GetPidInNamespace() const
{
    ...
    for (const auto& pair : pairs) {
        ...
        if (key == "NSpid") {
            std::vector<StringPiece> split_value_str = SplitStringPiece(value_str, "\t", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
