...
TEST_F(ProcessUtilTest, HandleTransfersOverrideClones)
{
    ...
    options.paths_to_transfer.push_back(
        { FilePath(kPersistedDataDirectoryPath), OpenDirectoryHandle(FilePath(tmpdir_with_staged.GetPath())).TakeChannel().release() });
...
MULTIPROCESS_TEST_MAIN(ChildVerifiesCetDisabled)
{
    auto get_process_mitigation_policy
        = reinterpret_cast<decltype(&GetProcessMitigationPolicy)>(::GetProcAddress(::GetModuleHandleW(L"kernel32.dll"), "GetProcessMitigationPolicy"));
    ...
    if (get_process_mitigation_policy(GetCurrentProcess(), ProcessUserShadowStackPolicy, &policy, sizeof(policy))) {
