...
{
    absl::optional<ScopedSafearray::LockScope<ElementVartype>> lock_scope = safearray.CreateLockScope<ElementVartype>();
...
template <VARTYPE ElementVartype> SAFEARRAY* VariantVector::CreateAndPopulateSafearray()
{
    ...
    if (!scoped_safearray.Get()) {
        constexpr size_t kElementSize = sizeof(typename internal::VariantUtil<ElementVartype>::Type);
        base::TerminateBecauseOutOfMemory(sizeof(SAFEARRAY) + (Size() * kElementSize));
    }

    absl::optional<ScopedSafearray::LockScope<ElementVartype>> lock_scope = scoped_safearray.CreateLockScope<ElementVartype>();
    DCHECK(lock_scope);
