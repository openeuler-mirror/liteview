...
absl::optional<std::vector<char>> GetTokenInfo(HANDLE token, TOKEN_INFORMATION_CLASS info_class)
{
...
template <typename T> absl::optional<T> GetTokenInfoFixed(HANDLE token, TOKEN_INFORMATION_CLASS info_class)
{
    T result;
    DWORD size = sizeof(T);
...
template <typename T> T* GetType(absl::optional<std::vector<char>>& info)
{
    DCHECK(info);
...
{
    absl::optional<TOKEN_STATISTICS> value = GetTokenInfoFixed<TOKEN_STATISTICS>(token, TokenStatistics);
    if (!value)
        return {};
    return *value;
}
...
AccessToken::Dacl::Dacl(const ACL* dacl)
    : acl_buffer_(reinterpret_cast<const char*>(dacl), reinterpret_cast<const char*>(dacl) + dacl->AclSize)
{
}
...
{
    HANDLE new_token;
    if (!::DuplicateHandle(::GetCurrentProcess(), token, ::GetCurrentProcess(), &new_token, TOKEN_QUERY, FALSE, 0)) {
        return absl::nullopt;
    }
    return AccessToken(new_token);
}

absl::optional<AccessToken> AccessToken::FromProcess(HANDLE process, bool impersonation)
{
...
absl::optional<AccessToken> AccessToken::FromCurrentProcess(bool impersonation)
{
    return FromProcess(::GetCurrentProcess(), impersonation);
}

absl::optional<AccessToken> AccessToken::FromThread(HANDLE thread, bool open_as_self)
{
    HANDLE token;
...
{
    absl::optional<std::vector<char>> buffer = GetTokenInfo(token_.get(), TokenUser);
...
{
    absl::optional<std::vector<char>> buffer = GetTokenInfo(token_.get(), TokenOwner);
    return UnwrapSid(Sid::FromPSID(GetType<TOKEN_OWNER>(buffer)->Owner));
}

Sid AccessToken::PrimaryGroup() const
{
    absl::optional<std::vector<char>> buffer = GetTokenInfo(token_.get(), TokenPrimaryGroup);
...
{
    absl::optional<std::vector<char>> buffer = GetTokenInfo(token_.get(), TokenIntegrityLevel);
    if (!buffer)
        return MAXDWORD;
...
{
    absl::optional<DWORD> value = GetTokenInfoFixed<DWORD>(token_.get(), TokenSessionId);
    if (!value)
        return MAXDWORD;
    return *value;
}
...
{
    absl::optional<DWORD> value = GetTokenInfoFixed<DWORD>(token_.get(), TokenIsAppContainer);
    if (!value)
        return false;
    return !!*value;
}
...
{
    absl::optional<std::vector<char>> buffer = GetTokenInfo(token_.get(), TokenAppContainerSid);
    if (!buffer)
        return absl::nullopt;
...
{
    absl::optional<TOKEN_LINKED_TOKEN> value = GetTokenInfoFixed<TOKEN_LINKED_TOKEN>(token_.get(), TokenLinkedToken);
    if (!value)
        return absl::nullopt;
...
{
    absl::optional<std::vector<char>> dacl_buffer = GetTokenInfo(token_.get(), TokenDefaultDacl);
    if (!dacl_buffer)
        return absl::nullopt;
...
{
    absl::optional<std::vector<char>> privileges = GetTokenInfo(token_.get(), TokenPrivileges);
    if (!privileges)
        return {};
...
{
    absl::optional<TOKEN_ELEVATION> value = GetTokenInfoFixed<TOKEN_ELEVATION>(token_.get(), TokenElevation);
    if (!value)
        return false;
