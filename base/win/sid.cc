...
absl::optional<Sid> FromSubAuthorities(PSID_IDENTIFIER_AUTHORITY identifier_authority, BYTE sub_authority_count, PDWORD sub_authorities)
{
...
template <typename T> absl::optional<std::vector<Sid>> FromVector(const std::vector<T>& values, absl::optional<Sid> (*create_sid)(T))
{
...
absl::optional<Sid> Sid::FromKnownCapability(WellKnownCapability capability)
{
    ...
    DWORD sub_authorities[] = { SECURITY_CAPABILITY_BASE_RID, static_cast<DWORD>(*capability_rid) };
    return FromSubAuthorities(&capability_authority, std::size(sub_authorities), sub_authorities);
}
...
absl::optional<Sid> Sid::FromNamedCapability(const wchar_t* capability_name)
{
    ...
    if (!derive_capability_sids(capability_name, &capability_groups, &capability_group_count, &capability_sids, &capability_sid_count)) {
        return absl::nullopt;
    }
...
absl::optional<Sid> Sid::FromKnownSid(WellKnownSid type)
{
    if (type == WellKnownSid::kAllRestrictedApplicationPackages) {
        ...
        DWORD sub_authorities[] = { SECURITY_APP_PACKAGE_BASE_RID, SECURITY_BUILTIN_PACKAGE_ANY_RESTRICTED_PACKAGE };
...
absl::optional<std::vector<Sid>> Sid::FromSddlStringVector(const std::vector<const wchar_t*>& sddl_sids)
{
    return FromVector(sddl_sids, Sid::FromSddlString);
}

absl::optional<std::vector<Sid>> Sid::FromNamedCapabilityVector(const std::vector<const wchar_t*>& capability_names)
{
    return FromVector(capability_names, Sid::FromNamedCapability);
}

absl::optional<std::vector<Sid>> Sid::FromKnownCapabilityVector(const std::vector<WellKnownCapability>& capabilities)
{
    return FromVector(capabilities, Sid::FromKnownCapability);
}

absl::optional<std::vector<Sid>> Sid::FromKnownSidVector(const std::vector<WellKnownSid>& sids)
{
