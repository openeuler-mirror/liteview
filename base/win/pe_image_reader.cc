...
const IMAGE_FILE_HEADER* PeImageReader::GetCoffFileHeader()
{
    ...
    return reinterpret_cast<const IMAGE_FILE_HEADER*>(image_data_ + GetDosHeader()->e_lfanew + sizeof(DWORD));
}

const uint8_t* PeImageReader::GetOptionalHeaderData(size_t* optional_header_size)
{
...
const IMAGE_DEBUG_DIRECTORY* PeImageReader::GetDebugEntry(size_t index, const uint8_t** raw_data, size_t* raw_data_size)
{
    DCHECK_LT(index, GetNumberOfDebugEntries());

    // Get the debug directory.
    size_t debug_directory_size = 0;
    const IMAGE_DEBUG_DIRECTORY* entries = reinterpret_cast<const IMAGE_DEBUG_DIRECTORY*>(GetImageData(IMAGE_DIRECTORY_ENTRY_DEBUG, &debug_directory_size));
    if (!entries)
        return nullptr;
...
bool PeImageReader::EnumCertificates(EnumCertificatesCallback callback, void* context)
{
    ...
    while (data_size) {
        ...
        if (kWinCertificateSize > data_size || kWinCertificateSize > win_certificate->dwLength || win_certificate->dwLength > data_size) {
            return false;
        }
        if (!(*callback)(win_certificate->wRevision, win_certificate->wCertificateType, &win_certificate->bCertificate[0],
...
bool PeImageReader::ValidateOptionalHeader()
{
    ...
    if (*optional_header_magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER32>>(image_data_ + optional_header_offset);
    } else if (*optional_header_magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
        optional_header = std::make_unique<OptionalHeaderImpl<IMAGE_OPTIONAL_HEADER64>>(image_data_ + optional_header_offset);
    } else {
        return false;
    }
...
bool PeImageReader::ValidateSectionHeaders()
{
    ...
    if (!GetStructureAt(static_cast<size_t>(first_section_header - image_data_), number_of_sections * sizeof(IMAGE_SECTION_HEADER), &first_section_header)) {
        return false;
    }
...
const uint8_t* PeImageReader::GetImageData(size_t index, size_t* data_length)
{
    ...
    if (index == IMAGE_DIRECTORY_ENTRY_SECURITY) {
        ...
        if (entry->VirtualAddress > image_size_ || image_size_ - entry->VirtualAddress < entry->Size) {
            return nullptr;
        }
