...
template <typename T>
using AsyncResultsT
    = std::conditional_t<std::is_convertible<AsyncAbiT<T>, IUnknown*>::value, Microsoft::WRL::ComPtr<std::remove_pointer_t<AsyncAbiT<T>>>, AsyncAbiT<T>>;
...
template <typename T> HRESULT PostAsyncOperationCompletedHandler(IAsyncOperationT<T>* async_operation, IAsyncOperationCompletedHandlerT<T> completed_handler)
{
    ...
    auto internal_completed_handler
        = base::BindOnce([](IAsyncOperationT<T>* async_operation, AsyncStatus async_status) -> AsyncResult {
              ...
              .Then(base::BindPostTask(base::SequencedTaskRunnerHandle::Get(),
                  base::BindOnce([](IAsyncOperationCompletedHandlerT<T> completed_handler,
                                     AsyncResult async_result) { std::move(completed_handler).Run(async_result.first.Get(), async_result.second); },
