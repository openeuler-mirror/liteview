...
public:
    ...
    // required.
    static absl::optional<AccessToken> FromProcess(HANDLE process, bool impersonation = false);
    ...
    // requires an impersonation token. To duplicate TOKEN_DUPLICATE access is
    // required.
    static absl::optional<AccessToken> FromCurrentProcess(bool impersonation = false);
    ...
    static absl::optional<AccessToken> FromThread(HANDLE thread, bool open_as_self = true);
    ...
    // If the thread isn't impersonating it will return an empty value and the
    // Win32 last error code will be ERROR_NO_TOKEN.
    static absl::optional<AccessToken> FromCurrentThread(bool open_as_self = true);
