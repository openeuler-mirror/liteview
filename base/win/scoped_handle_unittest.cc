...
bool DoDeathTestsWork()
{
    ...
    if (os_info->version() <= base::win::Version::WIN7 && os_info->IsWowDisabled()) {
        return false;
    }
...
TEST_F(ScopedHandleDeathTest, HandleVerifierTrackedHasBeenClosed)
{
    ...
    NtCloseFunc ntclose = reinterpret_cast<NtCloseFunc>(GetProcAddress(GetModuleHandle(L"ntdll.dll"), "NtClose"));
