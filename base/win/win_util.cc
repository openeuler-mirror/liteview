...
#include "patch_code/propidl_xp.h" // SUPPORT_XP_CODE
...
{
    return PowerDeterminePlatformRoleExXp(POWER_PLATFORM_ROLE_V2);
}
...
bool SetProcessDpiAwarenessWrapper(PROCESS_DPI_AWARENESS value)
{
    ...
    static const auto set_process_dpi_awareness_func
        = reinterpret_cast<decltype(&::SetProcessDpiAwareness)>(GetUser32FunctionPointer("SetProcessDpiAwarenessInternal"));
    if (set_process_dpi_awareness_func) {
        HRESULT hr = set_process_dpi_awareness_func(value);
        if (SUCCEEDED(hr))
            return true;
        DLOG_IF(ERROR, hr == E_ACCESSDENIED) << "Access denied error from SetProcessDpiAwarenessInternal. "
...
bool IsWindows10OrGreaterTabletMode(HWND hwnd)
{
    if (GetVersion() >= Version::WIN11) {
        ...
        base::win::RegKey registry_key(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\PriorityControl", KEY_READ);
        DWORD slate_mode = 0;
    ...
    HRESULT hr = win::RoGetActivationFactory(view_settings_guid.get(), IID_PPV_ARGS(&view_settings_interop));
    if (FAILED(hr))
        return false;
...
{
    ScopedPropVariant property_value;
    if (FAILED(InitPropVariantFromCLSID_XP(property_clsid_value, property_value.Receive()))) {
        return false;
    }
...
bool IsUser32AndGdi32Available()
{
    static auto is_user32_and_gdi32_available = []() {
        ...
        GetProcessMitigationPolicyType get_process_mitigation_policy_func
            = reinterpret_cast<GetProcessMitigationPolicyType>(GetProcAddress(GetModuleHandle(L"kernel32.dll"), "GetProcessMitigationPolicy"));

        if (!get_process_mitigation_policy_func)
            return true;

        PROCESS_MITIGATION_SYSTEM_CALL_DISABLE_POLICY policy = {};
        if (get_process_mitigation_policy_func(GetCurrentProcess(), ProcessSystemCallDisablePolicy, &policy, sizeof(policy))) {
...
bool GetLoadedModulesSnapshot(HANDLE process, std::vector<HMODULE>* snapshot)
{
    ...
    do {
        ...
        if (!::EnumProcessModulesXp(process, &(*snapshot)[0], static_cast<DWORD>(snapshot->size() * sizeof(HMODULE)), &bytes_required)) {
...
bool IsProcessPerMonitorDpiAware()
{
    ...
    if (per_monitor_dpi_aware == PerMonitorDpiAware::UNKNOWN) {
        ...
        if (shcore_dll) {
            auto get_process_dpi_awareness_func = reinterpret_cast<decltype(::GetProcessDpiAwareness)*>(::GetProcAddress(shcore_dll, "GetProcessDpiAwareness"));
...
void EnableHighDPISupport()
{
    ...
    PROCESS_DPI_AWARENESS process_dpi_awareness = GetVersion() >= Version::WIN10 ? PROCESS_PER_MONITOR_DPI_AWARE : PROCESS_SYSTEM_DPI_AWARE;
...
bool IsAppVerifierEnabled(const std::wstring& process_name)
{
    ...
    return key.Open(HKEY_LOCAL_MACHINE,
