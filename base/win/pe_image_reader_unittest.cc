...
protected:
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(image_reader_.Initialize(data_file_.data(), data_file_.length()));
    }
...
TEST_P(PeImageReaderTest, GetOptionalHeaderData)
{
    ...
    const uint8_t* optional_header_data = image_reader_.GetOptionalHeaderData(&optional_header_size);
...
TEST_P(PeImageReaderTest, GetDebugEntry)
{
    ...
    for (size_t i = 0; i < number_of_debug_entries; ++i) {
        ...
        const IMAGE_DEBUG_DIRECTORY* entry = image_reader_.GetDebugEntry(i, &raw_data, &raw_data_size);
...
public:
    ...
    static bool OnCertificateCallback(
        uint16_t revision, uint16_t certificate_type, const uint8_t* certificate_data, size_t certificate_data_size, void* context)
    {
        return reinterpret_cast<CertificateReceiver*>(context)->OnCertificate(revision, certificate_type, certificate_data, certificate_data_size);
    }

protected:
    CertificateReceiver() = default;
    virtual ~CertificateReceiver() = default;
    virtual bool OnCertificate(uint16_t revision, uint16_t certificate_type, const uint8_t* certificate_data, size_t certificate_data_size) = 0;
};
...
TEST_P(PeImageReaderCertificateTest, EnumCertificates)
{
    ...
    if (expected_data_->num_signers) {
        EXPECT_CALL(receiver, OnCertificate(WIN_CERT_REVISION_2_0, WIN_CERT_TYPE_PKCS_SIGNED_DATA, NotNull(), Gt(0U)))
