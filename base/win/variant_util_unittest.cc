...
template <VARTYPE ElementVartype> static bool TestIsConvertibleTo(const std::set<VARTYPE>& allowed_vartypes)
{
    for (VARTYPE vartype : kSupportedVartypes) {
        if (VariantUtil<ElementVartype>::IsConvertibleTo(vartype) != base::Contains(allowed_vartypes, vartype)) {
            return false;
        }
    }
    return true;
...
{
    for (VARTYPE vartype : kSupportedVartypes) {
        if (VariantUtil<ElementVartype>::IsConvertibleFrom(vartype) != base::Contains(allowed_vartypes, vartype)) {
            return false;
        }
    }
    return true;
