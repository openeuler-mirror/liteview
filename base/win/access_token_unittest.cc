...
void CheckTokenError(const absl::optional<AccessToken>& token, DWORD expected_error)
{
...
{
    AccessToken::Group group(*Sid::FromKnownSid(WellKnownSid::kWorld), attributes);
...
void CompareAppContainer(const Sid& package_sid, const std::vector<Sid>& caps)
{
    ...
    absl::optional<AccessToken> ac_token = AccessToken::FromToken(scoped_tmp_token.get());
...
TEST(AccessTokenTest, FromToken)
{
    ...
    absl::optional<AccessToken> token = AccessToken::FromToken(atl_token.GetHandle());
    ASSERT_TRUE(token);
    ...
    ASSERT_FALSE(atl_token.GetTokenId(&temp_luid));
    absl::optional<AccessToken> token2 = AccessToken::FromToken(atl_token.GetHandle());
    ASSERT_TRUE(token2);
...
TEST(AccessTokenTest, FromProcess)
{
    ...
    CompareTokens(*token, atl_token);
    absl::optional<AccessToken> imp_token = AccessToken::FromProcess(process.get(), true);
...
TEST(AccessTokenTest, FromThread)
{
    ...
    absl::optional<AccessToken> id_token = AccessToken::FromThread(thread.get(), true);
    atl_id_token.Revert();
...
TEST(AccessTokenTest, FromEffective)
{
    // Make sure we have no impersonation token before starting.
    ::RevertToSelf();
    absl::optional<base::win::AccessToken> primary_token = AccessToken::FromEffective();
...
TEST(AccessTokenTest, Restricted)
{
    ...
    absl::optional<AccessToken> restricted = AccessToken::FromToken(atl_restricted.GetHandle());
...
TEST(AccessTokenTest, AppContainer)
{
    if (GetVersion() < Version::WIN8)
        return;
    absl::optional<Sid> package_sid = Sid::FromSddlString(L"S-1-15-2-1-2-3-4-5-6-7");
    ASSERT_TRUE(package_sid);
    absl::optional<std::vector<Sid>> caps = Sid::FromKnownCapabilityVector({ WellKnownCapability::kInternetClient, WellKnownCapability::kDocumentsLibrary });
    ASSERT_TRUE(caps);
...
TEST(AccessTokenTest, Anonymous)
{
    ...
    ::RevertToSelf();
    ASSERT_TRUE(result);
    absl::optional<AccessToken> anon_token = AccessToken::FromToken(atl_anon_token.GetHandle());
