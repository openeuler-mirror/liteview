...
using StorageType = std::conditional_t<std::is_convertible<AbiType<TComplex>, IUnknown*>::value,
    Microsoft::WRL::ComPtr<std::remove_pointer_t<AbiType<TComplex>>>, AbiType<TComplex>>;

// Similar to StorageType, but returns a absl::optional in case underlying Abi
// type is not a pointer to IUnknown.
template <typename TComplex>
using OptionalStorageType = std::conditional_t<std::is_convertible<AbiType<TComplex>, IUnknown*>::value,
    Microsoft::WRL::ComPtr<std::remove_pointer_t<AbiType<TComplex>>>, absl::optional<AbiType<TComplex>>>;
...
template <typename T> HRESULT CopyTo(const absl::optional<T>& value, T* ptr)
{
    *ptr = *value;
    return S_OK;
}
