...
bool EqualSid(const absl::optional<Sid>& sid, const absl::optional<Sid>& compare_sid)
{
    if (!sid || !compare_sid)
        return false;
...
bool TestSidVector(absl::optional<std::vector<Sid>> sids, const std::vector<const wchar_t*> sddl)
{
    if (!sids)
        return false;
    if (sids->size() != sddl.size())
        return false;
    return std::equal(sids->begin(), sids->end(), sddl.begin(), [](const Sid& sid, const wchar_t* sddl) { return EqualSid(sid, sddl); });
}
...
TEST(SidTest, NamedCapability)
{
    ...
    const NamedCapabilityTestEntry capabilities[] = { { L"internetClient", L"S-1-15-3-1" }, { L"internetClientServer", L"S-1-15-3-2" },
        { L"registryRead",
        ...
        { L"enterpriseAuthentication", L"S-1-15-3-8" }, { L"privateNetworkClientServer", L"S-1-15-3-3" } };

    for (auto capability : capabilities) {
        EXPECT_TRUE(EqualSid(Sid::FromNamedCapability(capability.capability_name), capability.sddl_sid)) << "Named Capability: " << capability.sddl_sid;
    }
}
...
TEST(SidTest, KnownSids)
{
    ...
    EXPECT_TRUE(EqualSid(Sid::FromKnownSid(WellKnownSid::kAllApplicationPackages), ::WinBuiltinAnyPackageSid));
...
TEST(SidTest, FromNamedCapabilityVector)
{
    if (GetVersion() < Version::WIN10_RS2)
        return;
    std::vector<const wchar_t*> capabilities
        = { L"internetClient", L"internetClientServer", L"registryRead", L"lpacCryptoServices", L"enterpriseAuthentication", L"privateNetworkClientServer" };
    std::vector<const wchar_t*> sddl_caps = { L"S-1-15-3-1", L"S-1-15-3-2",
        ...
        L"S-1-15-3-8", L"S-1-15-3-3" };
    ASSERT_TRUE(TestSidVector(Sid::FromNamedCapabilityVector(capabilities), sddl_caps));
