...
#pragma pack(pop)
        ...
        DWORD os_type = 0;
        ::GetProductInfoXp(version_info.dwMajorVersion, version_info.dwMinorVersion, 0, 0, &os_type);
...
OSInfo::VersionNumber OSInfo::Kernel32VersionNumber() const
{
    ...
    static VersionNumber version = { 0 };
    if (version.major == 0) {
        version.major = Kernel32BaseVersion().components()[0];
        version.minor = Kernel32BaseVersion().components()[1];
        version.build = Kernel32BaseVersion().components()[2];
        version.patch = Kernel32BaseVersion().components()[3];
    }
    return version;
}
