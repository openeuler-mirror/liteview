...
public:
    ...
    static absl::optional<Sid> FromNamedCapability(const wchar_t* capability_name);

    // Create a Sid from a known capability enumeration value. The Sids
    // match with the list defined in Windows 8.
    static absl::optional<Sid> FromKnownCapability(WellKnownCapability capability);
    ...
    static absl::optional<std::vector<Sid>> FromSddlStringVector(const std::vector<const wchar_t*>& sddl_sids);

    // Create a vector of SIDs from a vector of capability names.
    static absl::optional<std::vector<Sid>> FromNamedCapabilityVector(const std::vector<const wchar_t*>& capability_names);

    // Create a vector of SIDs from a vector of well-known capability.
    static absl::optional<std::vector<Sid>> FromKnownCapabilityVector(const std::vector<WellKnownCapability>& capabilities);

    // Create a vector of SIDs from a vector of well-known sids.
    static absl::optional<std::vector<Sid>> FromKnownSidVector(const std::vector<WellKnownSid>& sids);
