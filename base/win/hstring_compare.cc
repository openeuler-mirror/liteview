...
HRESULT HStringCompare(HSTRING string1, HSTRING string2, INT32* result)
{
    ...
    static const auto compare_string_func = []() -> CompareStringFunc {
        ...
        NativeLibrary combase_module = PinSystemLibrary(FILE_PATH_LITERAL("combase.dll"), &load_error);
        if (load_error.code)
            return nullptr;

        return reinterpret_cast<CompareStringFunc>(GetFunctionPointerFromNativeLibrary(combase_module, "WindowsCompareStringOrdinal"));
    }();
