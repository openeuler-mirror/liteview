...
{
    int count = 0;
    CancelableRepeatingClosure cancelable(base::BindRepeating(&Increment, base::Unretained(&count)));
...
TEST(CancelableCallbackTest, CancelDropsCallback)
{
    ...
    CancelableOnceClosure cancelable(base::BindOnce(RefCountedParam, ref_counted));
...
TEST(CancelableCallbackTest, Reset)
{
    ...
    cancelable.Reset(base::BindRepeating(&IncrementBy, base::Unretained(&count), 3));
    EXPECT_FALSE(cancelable.IsCancelled());
...
TEST(CancelableCallbackTest, MoveOnlyType)
{
    ...
    int result = 0;
    CancelableRepeatingCallback<void(std::unique_ptr<int>)> cb(base::BindRepeating(&OnMoveOnlyReceived, base::Unretained(&result)));
