...
public:
    using RequestGlobalDumpFunction = RepeatingCallback<void(MemoryDumpType, MemoryDumpLevelOfDetail)>;
    ...
    void CreateProcessDump(const MemoryDumpRequestArgs& args, ProcessMemoryDumpCallback callback);
...
private:
    ...
    struct ProcessMemoryDumpAsyncState {
        ProcessMemoryDumpAsyncState(MemoryDumpRequestArgs req_args, const MemoryDumpProviderInfo::OrderedSet& dump_providers,
        ...
        ProcessMemoryDumpAsyncState& operator=(const ProcessMemoryDumpAsyncState&) = delete;
        ~ProcessMemoryDumpAsyncState();
    ...
    scoped_refptr<base::SequencedTaskRunner> GetOrCreateBgTaskRunnerLocked() EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    // at the end.
    void ContinueAsyncProcessDump(ProcessMemoryDumpAsyncState* owned_pmd_async_state);

    // Invokes OnMemoryDump() of the given MDP. Should be called on the MDP task
    // runner.
    void InvokeOnMemoryDump(MemoryDumpProviderInfo* mdpinfo, ProcessMemoryDump* pmd);

    void FinishAsyncProcessDump(std::unique_ptr<ProcessMemoryDumpAsyncState> pmd_async_state);
    ...
    void UnregisterDumpProviderInternal(MemoryDumpProvider* mdp, bool take_mdp_ownership_and_delete_async);
