        ...
        ProcessMemoryDumpCallback callback = BindOnce(
            [](bool* curried_success, OnceClosure curried_quit_closure, uint64_t curried_expected_guid, bool success, uint64_t dump_guid,
                ...
                *curried_success = success;
                ...
                ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(curried_quit_closure));
            },
...
TEST_F(MemoryDumpManagerTest, SingleDumper)
{
    ...
    for (int i = 0; i < 3; ++i) {
        EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::EXPLICITLY_TRIGGERED, MemoryDumpLevelOfDetail::DETAILED, MemoryDumpDeterminism::NONE));
    }
    DisableTracing();
...
TEST_F(MemoryDumpManagerTest, CheckMemoryDumpArgs)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::EXPLICITLY_TRIGGERED, MemoryDumpLevelOfDetail::LIGHT, MemoryDumpDeterminism::NONE));
    DisableTracing();
    mdm_->UnregisterDumpProvider(&mdp);
}
...
TEST_F(MemoryDumpManagerTest, CheckMemoryDumpArgsDeterministic)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::EXPLICITLY_TRIGGERED, MemoryDumpLevelOfDetail::DETAILED, MemoryDumpDeterminism::FORCE_GC));
    DisableTracing();
    mdm_->UnregisterDumpProvider(&mdp);
...
TEST_F(MemoryDumpManagerTest, TearDownThreadWhileDumping)
{
    ...
    for (const std::unique_ptr<MockMemoryDumpProvider>& mdp : mdps) {
        ...
        scoped_refptr<SequencedTaskRunner> main_runner = SequencedTaskRunnerHandle::Get();
...
TEST_F(MemoryDumpManagerTest, BackgroundAllowlisting)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::SUMMARY_ONLY, MemoryDumpLevelOfDetail::BACKGROUND, MemoryDumpDeterminism::NONE));
    DisableTracing();
}
...
TEST_F(MemoryDumpManagerTest, NoStackOverflowWithTooManyMDPs)
{
    ...
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::EXPLICITLY_TRIGGERED, MemoryDumpLevelOfDetail::DETAILED, MemoryDumpDeterminism::NONE));
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::EXPLICITLY_TRIGGERED, MemoryDumpLevelOfDetail::BACKGROUND, MemoryDumpDeterminism::NONE));
    EXPECT_TRUE(RequestProcessDumpAndWait(MemoryDumpType::SUMMARY_ONLY, MemoryDumpLevelOfDetail::BACKGROUND, MemoryDumpDeterminism::NONE));
}
