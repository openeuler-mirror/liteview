...
void WriteToATrace(int fd, const char* buffer, size_t size)
{
    ...
    while (total_written < size) {
        ssize_t written = HANDLE_EINTR(write(fd, buffer + total_written, size - total_written));
        if (written <= 0)
            break;
    ...
    if (total_written < size && errno != EBADF) {
        PLOG(WARNING) << "Failed to write buffer '" << std::string(buffer, size) << "' to trace_marker";
    }
}

void WriteEvent(char phase, const char* category_group, const char* name, unsigned long long id, const TraceArguments& args, unsigned int flags)
{
...
int OpenATraceMarkerFile(int mode)
{
    ...
    if (fd == -1) {
        PLOG(WARNING) << "Couldn't open " << kATraceMarkerFile << " or " << kLegacyATraceMarkerFile;
        return -1;
    }
    return fd;
}
...
void TraceEvent::SendToATrace()
{
    ...
    case TRACE_EVENT_PHASE_COMPLETE:
        WriteEvent(duration_.ToInternalValue() == -1 ? 'B' : 'E', category_group, name_, id_, args_, flags_);
        break;
    ...
    case TRACE_EVENT_PHASE_COUNTER:
        for (size_t i = 0; i < arg_size() && arg_name(i); ++i) {
            ...
            std::string out = base::StringPrintf("C|%d|%s-%s", getpid(), name_, arg_name(i));
            if (flags_ & TRACE_EVENT_FLAG_HAS_ID)
                StringAppendF(&out, "-%" PRIx64, static_cast<uint64_t>(id_));
            StringAppendF(&out, "|%d|%s", static_cast<int>(arg_value(i).as_int), category_group);
            WriteToATrace(g_atrace_fd, out.c_str(), out.size());
        }
        break;
...
void TraceLog::AddClockSyncMetadataEvent()
{
    ...
    std::string marker = StringPrintf("trace_event_clock_sync: parent_ts=%f\n", now_in_seconds);
