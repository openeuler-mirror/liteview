...
std::atomic<AllocationContextTracker::CaptureMode> AllocationContextTracker::capture_mode_ { AllocationContextTracker::CaptureMode::DISABLED };
...
const char* GetAndLeakThreadName()
{
    ...
    snprintf(name, sizeof(name), "%lu", static_cast<unsigned long>(PlatformThread::CurrentId()));
    return strdup(name);
}
...
// than our backtrace capacity.
        ...
        static_assert(std::size(frames) >= Backtrace::kMaxFrameCount, "not requesting enough frames to fill Backtrace");
        size_t frame_count = CFIBacktraceAndroid::GetInitializedInstance()->Unwind(frames, std::size(frames));
#elif BUILDFLAG(CAN_UNWIND_WITH_FRAME_POINTERS)
        const void* frames[Backtrace::kMaxFrameCount + 1];
        static_assert(std::size(frames) >= Backtrace::kMaxFrameCount, "not requesting enough frames to fill Backtrace");
        size_t frame_count = debug::TraceStackFramePointers(frames, std::size(frames), 1 /* exclude this function from the trace */);
#else
        ...
        ptrdiff_t starting_frame_index = base::checked_cast<ptrdiff_t>(frame_count);
    ...
    ctx->backtrace.frame_count = static_cast<size_t>(backtrace - std::begin(ctx->backtrace.frames));
