...
public:
    ...
    const Value* FindNamePhaseKeyValue(const char* name, const char* phase, const char* key, const char* value);
    ...
    {
        TraceLog::GetInstance()->SetDisabled(TraceLog::RECORDING_MODE | TraceLog::FILTERING_MODE);
    ...
    WaitableEvent* flush_complete_event, const scoped_refptr<base::RefCountedString>& events_str, bool has_more_events)
{
    ...
    absl::optional<Value> root = base::JSONReader::Read(json_output_.json_output, JSON_PARSE_RFC | JSON_ALLOW_CONTROL_CHARS);
...
static bool IsAllKeyValueInDict(const JsonKeyValue* key_values, const Value* dict)
{
...
const Value* TraceEventTestFixture::FindMatchingTraceEntry(const JsonKeyValue* key_values)
{
    // Scan all items
...
const Value* TraceEventTestFixture::FindNamePhase(const char* name, const char* phase)
{
    JsonKeyValue key_values[] = { { "name", name, IS_EQUAL }, { "ph", phase, IS_EQUAL }, { nullptr, nullptr, IS_EQUAL } };
    return FindMatchingTraceEntry(key_values);
}

const Value* TraceEventTestFixture::FindNamePhaseKeyValue(const char* name, const char* phase, const char* key, const char* value)
{
...
const Value* FindTraceEntry(const Value& trace_parsed, const char* string_to_match, const Value* match_after_this_item = nullptr)
{
    // Scan all items
...
std::vector<const Value*> FindTraceEntries(const Value& trace_parsed, const char* string_to_match)
{
...
void ValidateInstantEventPresentOnEveryThread(const Value& trace_parsed, int num_threads, int num_events)
{
...
TEST_F(TraceEventTestFixture, EnabledObserverDoesntFireOnSecondEnable)
{
    ...
    EXPECT_CALL(observer, OnTraceLogEnabled()).Times(0);
    EXPECT_CALL(observer, OnTraceLogDisabled()).Times(0);
    TraceLog::GetInstance()->SetEnabled(TraceConfig(kRecordAllCategoryFilter, ""), TraceLog::RECORDING_MODE);
    testing::Mock::VerifyAndClear(&observer);
    EXPECT_TRUE(TraceLog::GetInstance()->IsEnabled());
...
TEST_F(TraceEventTestFixture, TestTraceFlush)
{
    ...
    do {
        max_traces *= 2;
        TraceLog::GetInstance()->SetEnabled(TraceConfig(), TraceLog::RECORDING_MODE);
...
TEST_F(TraceEventTestFixture, Categories)
{
    ...
    Clear();
    TraceLog::GetInstance()->SetEnabled(TraceConfig("-test_inc_wildcard_*", ""), TraceLog::RECORDING_MODE);
    TRACE_EVENT_INSTANT0("test_inc_wildcard_abc", "not_inc", TRACE_EVENT_SCOPE_THREAD);
    TRACE_EVENT_INSTANT0("test_inc_wildcard_", "not_inc", TRACE_EVENT_SCOPE_THREAD);
...
TEST_F(TraceEventTestFixture, ThreadNames)
{
    ...
    for (int i = 0; i < kNumThreads; i++) {
        ...
        threads[i]->task_runner()->PostTask(FROM_HERE, base::BindOnce(&TraceManyInstantEvents, i, kNumEvents, task_complete_events[i]));
    }
    for (int i = 0; i < kNumThreads; i++) {
        task_complete_events[i]->Wait();
    }
    ...
    std::vector<const Value*> items = FindTraceEntries(trace_parsed_, "thread_name");
    for (const Value* item : items) {
        ...
        for (int j = 0; j < kNumThreads; j++) {
            ...
            EXPECT_EQ(*item->FindIntKey("pid"), static_cast<int>(base::GetCurrentProcId()));
...
TEST_F(TraceEventTestFixture, NormallyNoDeepCopy)
{
    ...
    BeginTrace();
    TRACE_EVENT_INSTANT0("category", name_string.c_str(), TRACE_EVENT_SCOPE_THREAD);
...
TEST_F(TraceEventTestFixture, TraceCategoriesAfterNestedEnable)
{
    ...
    EXPECT_STREQ("", trace_log->GetCurrentTraceConfig().ToCategoryFilterString().c_str());
    trace_log->SetDisabled();
    trace_log->SetDisabled();
    trace_log->SetDisabled();
    ...
    EXPECT_FALSE(*trace_log->GetCategoryGroupEnabled("foo"));
    EXPECT_STREQ("-foo,-bar", trace_log->GetCurrentTraceConfig().ToCategoryFilterString().c_str());
    trace_log->SetDisabled();
    trace_log->SetDisabled();

    // Make sure disabled categories aren't cleared if we set in the second.
    trace_log->SetEnabled(TraceConfig("disabled-by-default-cc,foo", ""), TraceLog::RECORDING_MODE);
    EXPECT_FALSE(*trace_log->GetCategoryGroupEnabled("bar"));
    trace_log->SetEnabled(TraceConfig("disabled-by-default-gpu", ""), TraceLog::RECORDING_MODE);
    ...
    EXPECT_TRUE(*trace_log->GetCategoryGroupEnabled("bar"));
    EXPECT_STREQ("disabled-by-default-cc,disabled-by-default-gpu", trace_log->GetCurrentTraceConfig().ToCategoryFilterString().c_str());
    trace_log->SetDisabled();
    trace_log->SetDisabled();
}
#endif // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
...
bool IsTraceEventArgsAllowlisted(const char* category_group_name, const char* event_name, ArgumentNameFilterPredicate* arg_filter)
{
    if (base::MatchPattern(category_group_name, "toplevel") && base::MatchPattern(event_name, "*")) {
        return true;
    }

    if (base::MatchPattern(category_group_name, "benchmark") && base::MatchPattern(event_name, "granularly_allowed")) {
...
{
    TraceLog::GetInstance()->SetArgumentFilterPredicate(base::BindRepeating(&IsTraceEventArgsAllowlisted));

    TraceLog::GetInstance()->SetEnabled(TraceConfig(kRecordAllCategoryFilter, "enable-argument-filter"), TraceLog::RECORDING_MODE);

    TRACE_EVENT1("toplevel", "event1", "int_one", 1);
    TRACE_EVENT1("Testing", "event2", "int_two", 1);

    TRACE_EVENT2("benchmark", "granularly_allowed", "granular_arg_allowed", "allowed_value", "granular_arg_disallowed", "disallowed_value");
    ...
    EXPECT_EQ(*args_dict->FindStringKey("granular_arg_disallowed"), "__stripped__");
}
#endif // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
...
{
    TraceLog* trace_log = TraceLog::GetInstance();
    trace_log->SetEnabled(TraceConfig(kRecordAllCategoryFilter, ""), TraceLog::RECORDING_MODE);
    trace_log->logged_events_.reset(TraceBuffer::CreateTraceBufferVectorOfSize(100));
    do {
        TRACE_EVENT_BEGIN_WITH_ID_TID_AND_TIMESTAMP0("test_all", "with_timestamp", 0, 0, TimeTicks::Now());
        TRACE_EVENT_END_WITH_ID_TID_AND_TIMESTAMP0("test_all", "with_timestamp", 0, 0, TimeTicks::Now());
    ...
    trace_full_metadata = FindTraceEntry(trace_parsed_, "overflowed_at_ts");

    EXPECT_TRUE(trace_full_metadata);
    EXPECT_EQ(*trace_full_metadata->FindStringKey("ph"), "M");
    absl::optional<double> maybe_buffer_limit_reached_timestamp = trace_full_metadata->FindDoublePath("args.overflowed_at_ts");

    EXPECT_EQ(*maybe_buffer_limit_reached_timestamp, static_cast<double>(trace_log->buffer_limit_reached_timestamp_.ToInternalValue()));
    ...
    absl::optional<double> maybe_last_trace_event_timestamp = last_trace_event.FindDoubleKey("ts");
    EXPECT_TRUE(maybe_last_trace_event_timestamp.has_value());
    EXPECT_LE(maybe_last_trace_event_timestamp.value(), maybe_buffer_limit_reached_timestamp.value());
    EXPECT_LE(maybe_buffer_limit_reached_timestamp.value(), trace_log->OffsetNow().ToInternalValue());
}

TEST_F(TraceEventTestFixture, TraceBufferRingBufferGetReturnChunk)
{
    TraceLog::GetInstance()->SetEnabled(TraceConfig(kRecordAllCategoryFilter, RECORD_CONTINUOUSLY), TraceLog::RECORDING_MODE);
    ...
    std::unique_ptr<TraceBufferChunk*[]> chunks(new TraceBufferChunk*[num_chunks]);
    for (size_t i = 0; i < num_chunks; ++i) {
        ...
        EXPECT_EQ((i + 1) * TraceBufferChunk::kTraceBufferChunkSize, buffer->Size());
...
TEST_F(TraceEventTestFixture, TraceBufferRingBufferHalfIteration)
{
    ...
    std::unique_ptr<TraceBufferChunk*[]> chunks(new TraceBufferChunk*[half_chunks]);
...
{
    TraceLog::GetInstance()->SetEnabled(TraceConfig(kRecordAllCategoryFilter, RECORD_AS_MUCH_AS_POSSIBLE), TraceLog::RECORDING_MODE);
    TraceBuffer* buffer = TraceLog::GetInstance()->trace_buffer();
...
TEST_F(TraceEventTestFixture, EchoToConsole)
{
    ...
    EXPECT_NE(std::string::npos, g_log_buffer->find("| | duration1[test_b1]\x1b"));
...
TEST_F(TraceEventTestFixture, TimeOffset)
{
    ...
    double end_time = static_cast<double>((TimeTicks::Now() - time_offset).ToInternalValue());
...
TEST_F(TraceEventTestFixture, TraceFilteringMode)
{
    ...
    TraceLog::GetInstance()->SetEnabled(TraceConfig(config_json), TraceLog::FILTERING_MODE);
    ...
    TraceLog::GetInstance()->SetEnabled(TraceConfig("", ""), TraceLog::RECORDING_MODE);
    EXPECT_EQ(TraceLog::RECORDING_MODE | TraceLog::FILTERING_MODE, TraceLog::GetInstance()->enabled_modes());
    {
...
TEST_F(TraceEventTestFixture, EventFiltering)
{
    ...
    TraceLog::GetInstance()->SetEnabled(trace_config, TraceLog::RECORDING_MODE | TraceLog::FILTERING_MODE);
...
TEST_F(TraceEventTestFixture, ContextLambda)
{
    ...
    EXPECT_EQ(*args_dict->FindStringKey("arg"), "Unsupported (crbug.com/1225176)");
#endif
}
