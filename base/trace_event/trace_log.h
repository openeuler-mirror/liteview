...
public:
    ...
    bool ShouldAddAfterUpdatingState(
    ...
    void EndFilteredEvent(const unsigned char* category_group_enabled, const char* name, TraceEventHandle handle);
    ...
    typedef std::unique_ptr<TraceEventFilter> (*FilterFactoryForTesting)(const std::string& /* predicate_name */);
...
private:
    ...
    std::vector<EnabledStateObserver*> enabled_state_observers_ GUARDED_BY(observers_lock_);
    ...
    std::vector<IncrementalStateObserver*> incremental_state_observers_ GUARDED_BY(observers_lock_);
