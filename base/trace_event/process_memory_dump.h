...
public:
    ...
    static absl::optional<size_t> CountResidentBytes(void* start_address, size_t mapped_size);

    // The same as above, but the given mapped range should belong to the
    // shared_memory's mapped region.
    static absl::optional<size_t> CountResidentBytesInSharedMemory(void* start_address, size_t mapped_size);
#endif
    ...
    const AllocatorDumpsMap& allocator_dumps() const
    {
    ...
    const MemoryDumpArgs& dump_args() const
    {
        return dump_args_;
    }
...
private:
    ...
    // different processes.
    const UnguessableToken& process_token() const
    {
