...
void TraceConfigCategoryFilter::InitializeFromString(const StringPiece& category_filter_string)
{
...
{
    const Value* included_category_list = dict.FindListKey(kIncludedCategoriesParam);
    if (included_category_list)
        SetCategoriesFromIncludedList(*included_category_list);
    const Value* excluded_category_list = dict.FindListKey(kExcludedCategoriesParam);
    if (excluded_category_list)
        SetCategoriesFromExcludedList(*excluded_category_list);
}

bool TraceConfigCategoryFilter::IsCategoryGroupEnabled(const StringPiece& category_group_name) const
{
...
bool TraceConfigCategoryFilter::IsCategoryEnabled(const StringPiece& category_name) const
{
...
void TraceConfigCategoryFilter::SetCategoriesFromIncludedList(const Value& included_list)
{
    included_categories_.clear();
...
void TraceConfigCategoryFilter::SetCategoriesFromExcludedList(const Value& excluded_list)
{
    excluded_categories_.clear();
