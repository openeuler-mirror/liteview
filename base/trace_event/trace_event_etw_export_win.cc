...
static void __stdcall EtwEnableCallback(LPCGUID SourceId, ULONG ControlCode, UCHAR Level, ULONGLONG MatchAnyKeyword, ULONGLONG MatchAllKeyword,
    PEVENT_FILTER_DESCRIPTOR FilterData, PVOID CallbackContext)
{
    ...
    DVLOG(1) << "ETW Keyword"
             << " Bits enabled in global context: " << std::hex << MatchAnyKeyword << " Bits enabled in our code: " << std::hex << g_callback_match_any_keyword;
...
TraceEventETWExport::TraceEventETWExport()
{
    ...
    static const GUID Chrome_GUID = { 0xD2D578D9, 0x2936, 0x45B6, { 0xA0, 0x9F, 0x30, 0xE3, 0x27, 0x15, 0xF4, 0x2D } };

    etw_provider_ = std::make_unique<TlmProvider>("Google.Chrome", Chrome_GUID, &EtwEnableCallback);
    ...
    // for keyword use.
    static_assert(ARRAYSIZE(kFilteredEventGroupNames) <= kMaxNumberOfGroupNames, "Exceeded max ETW keyword bits");
}
...
// static
uint64_t TraceEventETWExport::CategoryGroupToKeyword(const uint8_t* category_state)
{
    uint64_t keyword = 0;
    ...
    if (instance->categories_status_[kFilteredEventGroupNames[kOtherEventsGroupNameIndex]]) {
        keyword |= instance->categories_keyword_[kFilteredEventGroupNames[kOtherEventsGroupNameIndex]];
    }
    if (instance->categories_status_[kFilteredEventGroupNames[kDisabledOtherEventsGroupNameIndex]]) {
        keyword |= instance->categories_keyword_[kFilteredEventGroupNames[kDisabledOtherEventsGroupNameIndex]];
    }
    ...
    CStringTokenizer category_group_tokens(category_group_name.begin(), category_group_name.end(), ",");
...
// static
void TraceEventETWExport::AddEvent(char phase, const unsigned char* category_group_enabled, const char* name, unsigned long long id, const TraceArguments* args)
{
    // We bail early in case exporting is disabled or no consumer is listening.
    auto* instance = GetInstance();
    uint64_t keyword = CategoryGroupToKeyword(category_group_enabled);
    if (!instance || !instance->etw_provider_->IsEnabled(TRACE_LEVEL_NONE, keyword)) {
        return;
    }
    ...
    if (num_args == 0) {
        ...
        instance->etw_provider_->WriteEvent(name, TlmEventDescriptor(0, keyword), TlmMbcsStringField("Phase", phase_string),
            TlmMbcsStringField((args->names()[0]), (arg_values_string[0].c_str())), TlmMbcsStringField((args->names()[1]), (arg_values_string[1].c_str())));
    } else {
        NOTREACHED();
    }
}
...
// static
bool TraceEventETWExport::IsCategoryGroupEnabled(StringPiece category_group_name)
{
