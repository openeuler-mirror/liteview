...
TEST_F(MemoryDumpSchedulerTest, StartStopQuickly)
{
    ...
    detailed_config.triggers.push_back({ MemoryDumpLevelOfDetail::DETAILED, kPeriodMs });
    detailed_config.callback = BindRepeating(&CallbackWrapper::OnTick, Unretained(&on_tick_));

    testing::InSequence sequence;
    EXPECT_CALL(on_tick_, OnTick(MemoryDumpLevelOfDetail::LIGHT)).Times(AtMost(kQuickIterations));
    EXPECT_CALL(on_tick_, OnTick(MemoryDumpLevelOfDetail::DETAILED)).Times(kDetailedTicks - 1);
    EXPECT_CALL(on_tick_, OnTick(MemoryDumpLevelOfDetail::DETAILED)).WillRepeatedly(Invoke([this](MemoryDumpLevelOfDetail) { this->evt_.Signal(); }));
