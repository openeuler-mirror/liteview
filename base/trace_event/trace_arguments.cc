...
static_assert(std::is_pod<TraceValue>::value, "TraceValue must be plain-old-data type for performance reasons!");
...
void TraceValue::Append(unsigned char type, bool as_json, std::string* out) const
{
    ...
    case TRACE_VALUE_TYPE_POINTER: {
        ...
        StringAppendF(out, format_string, static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this->as_pointer)));
    } break;
    case TRACE_VALUE_TYPE_STRING:
    case TRACE_VALUE_TYPE_COPY_STRING:
        if (as_json)
