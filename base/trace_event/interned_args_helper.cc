...
{
    const auto build_id = InternedBuildId::Get(ctx, base::TransformModuleIDToBreakpadFormat(module->GetId()));
...
// static
absl::optional<size_t> InternedUnsymbolizedSourceLocation::Get(perfetto::EventContext* ctx, uintptr_t address)
{
    auto* index_for_field = GetOrCreateIndexForField(ctx->GetIncrementalState());
    const base::ModuleCache::Module* module = index_for_field->module_cache_.GetModuleForAddress(address);
    if (!module) {
        return absl::nullopt;
    }
    size_t iid;
