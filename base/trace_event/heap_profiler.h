...
#define INTERNAL_HEAP_PROFILER_UID(name_prefix) INTERNAL_HEAP_PROFILER_UID2(name_prefix, __LINE__)
...
#define HEAP_PROFILER_SCOPED_IGNORE trace_event_internal::HeapProfilerScopedIgnore INTERNAL_HEAP_PROFILER_UID(scoped_ignore)
...
public:
    inline HeapProfilerScopedIgnore()
    {
        using base::trace_event::AllocationContextTracker;
        if (UNLIKELY(AllocationContextTracker::capture_mode() != AllocationContextTracker::CaptureMode::DISABLED)) {
            AllocationContextTracker::GetInstanceForCurrentThread()->begin_ignore_scope();
        }
    }
    inline ~HeapProfilerScopedIgnore()
    {
