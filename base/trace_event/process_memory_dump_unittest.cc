...
const char* const kTestDumpNameAllowlist[] = { "Allowlisted/TestName", "Allowlisted/TestName_0x?", "Allowlisted/0x?/TestName", "Allowlisted/0x?", nullptr };
...
TEST(ProcessMemoryDumpTest, MoveConstructor)
{
    ...
    EXPECT_EQ(MemoryDumpLevelOfDetail::DETAILED, pmd2.dump_args().level_of_detail);
...
TEST(ProcessMemoryDumpTest, MAYBE_CountResidentBytes)
{
    ...
    absl::optional<size_t> res1 = ProcessMemoryDump::CountResidentBytes(memory1, size1);
    ...
    absl::optional<size_t> res2 = ProcessMemoryDump::CountResidentBytes(memory2, kVeryLargeMemorySize);
...
TEST(ProcessMemoryDumpTest, MAYBE_CountResidentBytesInSharedMemory)
{
    ...
    // Allocate few page of dirty memory and check if it is resident.
    {
        ...
        absl::optional<size_t> res1 = ProcessMemoryDump::CountResidentBytesInSharedMemory(mapping.memory(), mapping.mapped_size());
        ASSERT_TRUE(res1.has_value());
    ...
    // Allocate a large memory segment (> 8Mib).
    {
        ...
        absl::optional<size_t> res2 = ProcessMemoryDump::CountResidentBytesInSharedMemory(mapping.memory(), mapping.mapped_size());
        ASSERT_TRUE(res2.has_value());
        ASSERT_EQ(res2.value(), kVeryLargeMemorySize);
    }
    ...
    // Allocate a large memory segment, but touch about half of all pages.
    {
        ...
        absl::optional<size_t> res3 = ProcessMemoryDump::CountResidentBytesInSharedMemory(mapping.memory(), mapping.mapped_size());
