...
std::set<MemoryDumpLevelOfDetail> GetDefaultAllowedMemoryDumpModes()
{
    ...
    for (uint32_t mode = static_cast<uint32_t>(MemoryDumpLevelOfDetail::FIRST); mode <= static_cast<uint32_t>(MemoryDumpLevelOfDetail::LAST); mode++) {
...
void TraceConfig::EventFilterConfig::InitializeFromConfigDict(const Value& event_filter)
{
...
bool TraceConfig::EventFilterConfig::IsCategoryGroupEnabled(const StringPiece& category_group_name) const
{
...
TraceConfig::TraceConfig(StringPiece category_filter_string, StringPiece trace_options_string)
{
    InitializeFromStrings(category_filter_string, trace_options_string);
}

TraceConfig::TraceConfig(StringPiece category_filter_string, TraceRecordMode record_mode)
{
...
bool TraceConfig::IsCategoryGroupEnabled(const StringPiece& category_group_name) const
{
...
void TraceConfig::InitializeFromConfigDict(const Value& dict)
{
    ...
    trace_buffer_size_in_events_ = base::saturated_cast<size_t>(dict.FindIntKey(kTraceBufferSizeInEvents).value_or(0));
    trace_buffer_size_in_kb_ = base::saturated_cast<size_t>(dict.FindIntKey(kTraceBufferSizeInKb).value_or(0));

    enable_systrace_ = dict.FindBoolKey(kEnableSystraceParam).value_or(false);
    enable_argument_filter_ = dict.FindBoolKey(kEnableArgumentFilterParam).value_or(false);
    enable_event_package_name_filter_ = dict.FindBoolKey(kEnableEventPackageNameFilterParam).value_or(false);
...
void TraceConfig::InitializeFromStrings(StringPiece category_filter_string, StringPiece trace_options_string)
{
...
void TraceConfig::SetMemoryDumpConfigFromConfigDict(const Value& memory_dump_config)
{
    // Set allowed dump modes.
    memory_dump_config_.allowed_dump_modes.clear();
    const Value* allowed_modes_list = memory_dump_config.FindListKey(kAllowedDumpModesParam);
    ...
    if (trigger_list) {
        for (const Value& trigger : trigger_list->GetList()) {
            ...
            if (!interval) {
                ...
                const std::string* trigger_type_str = trigger.FindStringKey(kTriggerTypeParam);
            ...
            const std::string* level_of_detail_str = trigger.FindStringKey(kTriggerModeParam);
    ...
    const Value* heap_profiler_options = memory_dump_config.FindDictKey(kHeapProfilerOptions);
    if (heap_profiler_options) {
        absl::optional<int> min_size_bytes = heap_profiler_options->FindIntKey(kBreakdownThresholdBytes);
...
void TraceConfig::SetHistogramNamesFromConfigList(const Value& histogram_names)
{
    histogram_names_.clear();
...
void TraceConfig::SetEventFiltersFromConfigList(const Value& category_event_filters)
{
    event_filters_.clear();
    ...
    for (const Value& event_filter : category_event_filters.GetList()) {
        ...
        const std::string* predicate_name = event_filter.FindStringKey(kFilterPredicateParam);
...
Value TraceConfig::ToValue() const
{
    ...
    if (category_filter_.IsCategoryEnabled(MemoryDumpManager::kTraceCategory)) {
        ...
        if (memory_dump_config_.heap_profiler_options.breakdown_threshold_bytes != MemoryDumpConfig::HeapProfiler::kDefaultBreakdownThresholdBytes) {
            ...
            options.SetIntKey(kBreakdownThresholdBytes, base::checked_cast<int>(memory_dump_config_.heap_profiler_options.breakdown_threshold_bytes));
