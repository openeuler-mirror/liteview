...
public:
    ...
    //
    template <typename T, class = void> struct Helper { };

    template <typename T> struct HasHelperSupport {
    private:
        using Yes = char[1];
        using No = char[2];

        template <typename V> static Yes& check_support(decltype(TraceValue::Helper<typename InnerType<V>::type>::kType, int()));
        template <typename V> static No& check_support(...);
    ...
    template <typename T, class = void> struct TypeFor;

    template <typename T> struct TypeFor<T, typename std::enable_if<HasHelperSupport<typename InnerType<T>::type>::value>::type> {
    ...
    template <typename T,
        class = std::enable_if_t<
            HasHelperSupport<typename InnerType<T>::type>::value
            // || perfetto::internal::has_traced_value_support<typename InnerType<T>::type>::value // weolar
        > >
    struct TypeCheck {
    ...
    template <class T> typename std::enable_if<HasHelperSupport<typename InnerType<T>::type>::value>::type Init(T&& value)
    {
        using ValueType = typename InnerType<T>::type;
    ...
    //   template <class T>
    //   typename std::enable_if<!HasHelperSupport<typename InnerType<T>::type>::value && perfetto::internal::has_traced_value_support<typename InnerType<T>::type>::value>::type
    //   Init(T&& value) {
...
template <typename T> struct TraceValue::Helper<T, typename std::enable_if<std::is_integral<T>::value || std::is_enum<T>::value>::type> {
    static constexpr unsigned char kType = std::is_signed<T>::value ? TRACE_VALUE_TYPE_INT : TRACE_VALUE_TYPE_UINT;
...
template <typename T> struct TraceValue::Helper<T, typename std::enable_if<std::is_floating_point<T>::value>::type> {
...
template <> struct TraceValue::Helper<bool> {
    static constexpr unsigned char kType = TRACE_VALUE_TYPE_BOOL;
    static inline void SetValue(TraceValue* v, bool value)
    {
        v->as_bool = value;
    }
};
...
template <> struct TraceValue::Helper<const void*> {
...
template <> struct TraceValue::Helper<void*> {
    static constexpr unsigned char kType = TRACE_VALUE_TYPE_POINTER;
...
template <> struct TraceValue::Helper<const char*> {
...
template <> struct TraceValue::Helper<std::string> {
...
struct TraceValue::Helper<std::unique_ptr<CONVERTABLE_TYPE>,
    typename std::enable_if<std::is_convertible<CONVERTABLE_TYPE*, ConvertableToTraceFormat*>::value>::type> {
    static constexpr unsigned char kType = TRACE_VALUE_TYPE_CONVERTABLE;
    static inline void SetValue(TraceValue* v, std::unique_ptr<CONVERTABLE_TYPE> value)
    {
...
struct TraceValue::Helper<T,
    typename std::enable_if<std::is_same<T, base::Time>::value || std::is_same<T, base::TimeTicks>::value || std::is_same<T, base::ThreadTicks>::value>::type> {
...
template <> struct TraceValue::Helper<TraceStringWithCopy> {
    static constexpr unsigned char kType = TRACE_VALUE_TYPE_COPY_STRING;
...
public:
    ...
    explicit StringStorage(size_t alloc_size)
    {
        Reset(alloc_size);
    }
    ...
    {
        return data_ ? data_->chars : nullptr;
    }
    constexpr char* data()
    {
        return data_ ? data_->chars : nullptr;
...
public:
    ...
    template <typename T1, typename T2, class = decltype(TraceValue::TypeCheck<T1>::value && TraceValue::TypeCheck<T2>::value)>
    ...
    void CopyStringsTo(StringStorage* storage, bool copy_all_strings, const char** extra_string1, const char** extra_string2);
