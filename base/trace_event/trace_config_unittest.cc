...
TEST(TraceConfigTest, TraceConfigFromDict)
{
    ...
    absl::optional<Value> default_value = JSONReader::Read(kDefaultTraceConfigString);
    ...
    absl::optional<Value> custom_value = JSONReader::Read(kCustomTraceConfigString);
...
TEST(TraceConfigTest, TraceConfigFromMemoryConfigString)
{
    ...
    EXPECT_EQ(MemoryDumpLevelOfDetail::LIGHT, tc1.memory_dump_config().triggers[0].level_of_detail);

    EXPECT_EQ(2000u, tc1.memory_dump_config().triggers[1].min_time_between_dumps_ms);
    EXPECT_EQ(MemoryDumpLevelOfDetail::DETAILED, tc1.memory_dump_config().triggers[1].level_of_detail);
    ...
    EXPECT_EQ(MemoryDumpLevelOfDetail::BACKGROUND, tc3.memory_dump_config().triggers[0].level_of_detail);
}
