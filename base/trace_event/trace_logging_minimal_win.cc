...
#include "patch_code/winevt_xp.h" // SUPPORT_XP_CODE
...
TlmProvider::TlmProvider(const char* provider_name, const GUID& provider_guid, PENABLECALLBACK enable_callback, void* enable_callback_context) noexcept
{
    ULONG status = Register(provider_name, provider_guid, enable_callback, enable_callback_context);
...
uint16_t TlmProvider::AppendNameToMetadata(char* metadata, uint16_t metadata_size, uint16_t metadata_index, const char* name) const noexcept
{
...
void TlmProvider::Unregister() noexcept
{
    ...
    ULONG status = EventUnregisterXp(reg_handle_);
    LOG_IF(ERROR, status != ERROR_SUCCESS) << "Provider unregistration failure";
    reg_handle_ = 0;
    level_plus1_ = 0;
}

ULONG TlmProvider::Register(const char* provider_name, const GUID& provider_guid, PENABLECALLBACK enable_callback, void* enable_callback_context) noexcept
{
    ...
    *reinterpret_cast<uint16_t*>(provider_metadata_) = provider_metadata_size_;
    ...
    ULONG status = EventRegisterXp(&provider_guid, StaticEnableCallback, this, &reg_handle_);
    ...
    status = TLM_EVENT_SET_INFORMATION(reg_handle_, EventProviderSetTraits, provider_metadata_, provider_metadata_size_);

#elif TLM_HAVE_EVENT_SET_INFORMATION == 2

    HMODULE eventing_lib;
    if (GetModuleHandleExW(0, L"api-ms-win-eventing-provider-l1-1-0.dll", &eventing_lib) || GetModuleHandleExW(0, L"advapi32.dll", &eventing_lib)) {
        typedef ULONG(WINAPI * PFEventSetInformation)(
            REGHANDLE reg_handle, EVENT_INFO_CLASS information_class, PVOID event_information, ULONG information_length);
        PFEventSetInformation event_set_information_ptr
            = reinterpret_cast<decltype(&::EventSetInformation)>(GetProcAddress(eventing_lib, "EventSetInformation"));
        if (event_set_information_ptr) {
            // Best-effort, ignore failure.
            status = event_set_information_ptr(reg_handle_, EventProviderSetTraits, provider_metadata_, provider_metadata_size_);
        }
    ...
    PEVENT_FILTER_DESCRIPTOR filter_data, PVOID callback_context)
{
    ...
    case EVENT_CONTROL_CODE_ENABLE_PROVIDER:
        pProvider->level_plus1_ = level != 0 ? static_cast<unsigned>(level) + 1u : 256u;
    ...
    if (pProvider->enable_callback_) {
        pProvider->enable_callback_(source_id, is_enabled, level, match_any_keyword, match_all_keyword, filter_data, pProvider->enable_callback_context_);
    }
}

uint16_t TlmProvider::EventBegin(char* metadata, const char* event_name) const noexcept
{
...
char TlmProvider::EventAddField(char* metadata, uint16_t* metadata_index, uint8_t in_type, uint8_t out_type, const char* field_name) const noexcept
{
    ...
    const EVENT_DESCRIPTOR& event_descriptor) const noexcept
{
    ...
    *reinterpret_cast<uint16_t*>(metadata) = meta_data_index;
    ...
    return EventWriteXp(reg_handle_, &event_descriptor, descriptors_index, descriptors);
}
...
{
    EventDataDescCreateXp(&descriptors[0], value_, base::checked_cast<ULONG>(strlen(value_) + 1));
}
