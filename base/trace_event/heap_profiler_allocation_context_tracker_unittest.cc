...
template <size_t N> void AssertBacktraceEquals(const StackFrame (&expected_backtrace)[N])
{
    AllocationContext ctx;
    ASSERT_TRUE(AllocationContextTracker::GetInstanceForCurrentThread()->GetContextSnapshot(&ctx));
    ...
    // string comparison.
    for (; actual != actual_bottom && expected != expected_bottom; actual++, expected++)
...
public:
    void SetUp() override
    {
        AllocationContextTracker::SetCaptureMode(AllocationContextTracker::CaptureMode::NATIVE_STACK);
        AllocationContextTracker::SetCurrentThreadName(kThreadName);
    }

    void TearDown() override
    {
        AllocationContextTracker::SetCaptureMode(AllocationContextTracker::CaptureMode::DISABLED);
    }
};
...
TEST_F(AllocationContextTrackerTest, IgnoreAllocationTest)
{
    ...
    AllocationContext ctx;
    ASSERT_FALSE(AllocationContextTracker::GetInstanceForCurrentThread()->GetContextSnapshot(&ctx));
}
