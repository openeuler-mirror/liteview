...
// static
absl::optional<size_t> ProcessMemoryDump::CountResidentBytes(void* start_address, size_t mapped_size)
{
...
#if BUILDFLAG(IS_WIN)
    std::unique_ptr<PSAPI_WORKING_SET_EX_INFORMATION[]> vec(new PSAPI_WORKING_SET_EX_INFORMATION[max_vec_size]);
    ...
    while (offset < mapped_size) {
        ...
        DWORD vec_size = static_cast<DWORD>(page_count * sizeof(PSAPI_WORKING_SET_EX_INFORMATION));
        failure = !QueryWorkingSetExXp(GetCurrentProcess(), vec.get(), vec_size);
        ...
        do {
            result =
#if BUILDFLAG(IS_AIX)
                mincore(reinterpret_cast<char*>(chunk_start), chunk_size, reinterpret_cast<char*>(vec.get()));
#else
...
// static
absl::optional<size_t> ProcessMemoryDump::CountResidentBytesInSharedMemory(void* start_address, size_t mapped_size)
{
...
MemoryAllocatorDump* ProcessMemoryDump::AddAllocatorDumpInternal(std::unique_ptr<MemoryAllocatorDump> mad)
{
    ...
    // given.
    if (dump_args_.level_of_detail == MemoryDumpLevelOfDetail::BACKGROUND && !IsMemoryAllocatorDumpNameInAllowlist(mad->absolute_name())) {
...
bool ProcessMemoryDump::MemoryAllocatorDumpEdge::operator==(const MemoryAllocatorDumpEdge& other) const
{
    return source == other.source && target == other.target && importance == other.importance && overridable == other.overridable;
}
