...
template <typename Function> void ForEachCategoryFilter(const unsigned char* category_group_enabled, Function filter_fn)
{
...
void OnAddLegacyTraceEvent(TraceEvent* trace_event, bool thread_will_flush, base::trace_event::TraceEventHandle* handle)
{
    ...
    auto write_args = [trace_event](perfetto::EventContext ctx) {
        ...
        if (!id_flags && perfetto::internal::TrackEventLegacy::PhaseToType(trace_event->phase()) != perfetto::protos::pbzero::TrackEvent::TYPE_UNSPECIFIED) {
            return;
        }
    ...
    if (trace_event->thread_id() && trace_event->thread_id() != static_cast<int>(base::PlatformThread::CurrentId())) {
...
public:
    ...
    perfetto::trace_processor::util::Status AppendString(const std::string& string) override
    {
        ...
        } else if (buffer_->data().empty() && !strncmp(string.c_str(), kJsonJoiner, strlen(kJsonJoiner))) {
    ...
    , generation_(trace_log->generation())
{
    ...
    MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "ThreadLocalEventBuffer", ThreadTaskRunnerHandle::Get());
...
void TraceLog::InitializeThreadLocalEventBufferIfSupported()
{
    ...
    if (thread_blocks_message_loop_.Get() || !CurrentThread::IsSet() || !ThreadTaskRunnerHandle::IsSet()) {
        return;
    }
...
const unsigned char* TraceLog::GetCategoryGroupEnabled(const char* category_group)
{
    ...
    if (!category) {
        ...
        // at the same time.
        auto category_initializer = [](TraceCategory* category) { TraceLog::GetInstance()->UpdateCategoryState(category); };
        AutoLock lock(tracelog->lock_);
        CategoryRegistry::GetOrCreateCategoryLocked(category_group, category_initializer, &category);
    }
...
#else // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
    return CategoryRegistry::GetCategoryByStatePtr(category_group_enabled)->name();
#endif // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
}
...
void TraceLog::UpdateCategoryState(TraceCategory* category)
{
    ...
    if (enabled_modes_ & RECORDING_MODE && trace_config_.IsCategoryGroupEnabled(category->name())) {
    ...
    if (enabled_modes_ & RECORDING_MODE && category == CategoryRegistry::kCategoryMetadata) {
        state_flags |= TraceCategory::ENABLED_FOR_RECORDING;
    }
...
void TraceLog::CreateFiltersForTraceConfig()
{
    ...
    for (auto& filter_config : enabled_event_filters_) {
        if (GetCategoryGroupFilters().size() >= MAX_TRACE_EVENT_FILTERS) {
            NOTREACHED() << "Too many trace event filters installed in the current session";
            break;
        }
...
void TraceLog::SetEnabled(const TraceConfig& trace_config, uint8_t modes_to_enable)
{
    ...
    if (new_options != old_options || (trace_config_.GetTraceBufferSizeInEvents() && !already_recording)) {
...
void TraceLog::SetDisabledWhileLocked(uint8_t modes_to_disable)
{
    ...
    bool is_recording_mode_disabled = (enabled_modes_ & RECORDING_MODE) && (modes_to_disable & RECORDING_MODE);
...
void TraceLog::FlushInternal(const TraceLog::OutputCallback& cb, bool use_worker_thread, bool discard_events)
{
    ...
    if (convert_to_json) {
        ...
        json_output_writer_.reset(new JsonStringOutputWriter(use_worker_thread ? ThreadTaskRunnerHandle::Get() : nullptr, cb));
    } else {
    ...
    std::vector<scoped_refptr<SingleThreadTaskRunner>> task_runners;
    {
        AutoLock lock(lock_);
        DCHECK(!flush_task_runner_);
        flush_task_runner_ = SequencedTaskRunnerHandle::IsSet() ? SequencedTaskRunnerHandle::Get() : nullptr;
    ...
    if (!task_runners.empty()) {
        for (auto& task_runner : task_runners) {
            task_runner->PostTask(FROM_HERE, BindOnce(&TraceLog::FlushCurrentThread, Unretained(this), gen, discard_events));
        }
        flush_task_runner_->PostDelayedTask(
            FROM_HERE, BindOnce(&TraceLog::OnFlushTimeout, Unretained(this), gen, discard_events), Milliseconds(kThreadFlushTimeoutMs));
        return;
    }
...
void TraceLog::OnTraceData(const char* data, size_t size, bool has_more)
{
    ...
    auto status = perfetto::trace_processor::json::ExportJson(trace_processor_.get(), json_output_writer_.get());
    DCHECK(status.ok()) << status.message();
    ...
    std::unique_ptr<TraceBuffer> logged_events, const OutputCallback& flush_output_callback, const ArgumentFilterPredicate& argument_filter_predicate)
{
    ...
    while (const TraceBufferChunk* chunk = logged_events->NextChunk()) {
        for (size_t j = 0; j < chunk->size(); ++j) {
            ...
            chunk->GetEventAt(j)->AppendAsJSON(&(json_events_str_ptr->data()), argument_filter_predicate);
        }
    }
...
bool TraceLog::ShouldAddAfterUpdatingState(
    char phase, const unsigned char* category_group_enabled, const char* name, uint64_t id, PlatformThreadId thread_id, TraceArguments* args)
{
    ...
    if (thread_id == PlatformThread::CurrentId()) {
        ...
        if (new_name != current_thread_name->Get() && new_name && *new_name) {
            ...
            if (existing_name == thread_names_.end()) {
                ...
                // new name.
                std::vector<StringPiece> existing_names = base::SplitStringPiece(existing_name->second, ",", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    if (*category_group_enabled & TraceCategory::ENABLED_FOR_ETW_EXPORT) {
        ...
        name = name ? name : "";
        TraceEventETWExport::AddEvent(phase, category_group_enabled, name, id, args);
    }
    ...
    unsigned int flags) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    if (!ShouldAddAfterUpdatingState(phase, category_group_enabled, name, id, thread_id, args)) {
        return handle;
    }
    ...
    if (*category_group_enabled & TraceCategory::ENABLED_FOR_FILTERING) {
        ...
        ForEachCategoryFilter(category_group_enabled, [&new_trace_event, &disabled_by_filters](TraceEventFilter* trace_event_filter) {
    ...
    if ((*category_group_enabled & TraceCategory::ENABLED_FOR_RECORDING) && !disabled_by_filters) {
...
void TraceLog::EndFilteredEvent(const unsigned char* category_group_enabled, const char* name, TraceEventHandle handle)
{
    const char* category_name = GetCategoryGroupName(category_group_enabled);
    ForEachCategoryFilter(
        category_group_enabled, [name, category_name](TraceEventFilter* trace_event_filter) { trace_event_filter->EndEvent(category_name, name); });
}
...
tracing::PerfettoPlatform* TraceLog::GetOrCreatePerfettoPlatform()
{
    if (!perfetto_platform_) {
        perfetto_platform_.reset(new tracing::PerfettoPlatform(tracing::PerfettoPlatform::TaskRunnerType::kBuiltin));
    }
...
#if !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
ScopedTraceBinaryEfficient::ScopedTraceBinaryEfficient(const char* category_group, const char* name)
{
    // The single atom works because for now the category_group can only be "gpu".
    DCHECK_EQ(strcmp(category_group, "gpu"), 0);
    static TRACE_EVENT_API_ATOMIC_WORD atomic = 0;
    INTERNAL_TRACE_EVENT_GET_CATEGORY_INFO_CUSTOM_VARIABLES(category_group, atomic, category_group_enabled_);
    name_ = name;
    if (*category_group_enabled_) {
        event_handle_ = TRACE_EVENT_API_ADD_TRACE_EVENT_WITH_THREAD_ID_AND_TIMESTAMP(TRACE_EVENT_PHASE_COMPLETE, category_group_enabled_, name,
            trace_event_internal::kGlobalScope, // scope
            trace_event_internal::kNoId, // id
...
{
    if (*category_group_enabled_) {
        TRACE_EVENT_API_UPDATE_TRACE_EVENT_DURATION(category_group_enabled_, name_, event_handle_);
    }
}
#endif // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
