...
TEST(TraceEventArgumentTest, SetTracedValueArgBasicTypes)
{
    ...
    SetTracedValueArg(raw_value, "my_string", std::string("wonderful_") + std::string("world"));
...
public:
    std::string ToString()
    {
        return "don't return me, not const";
    }
};
...
public:
    ConfusingToStringAPI ToString() const
    {
        return ConfusingToStringAPI();
    }
};
...
TEST(TraceEventConversionHelperTest, UseOstreamOperator)
{
    ...
    EXPECT_EQ(internal::OstreamValueToString((void*)0x123), ValueToString((void*)0x123));
}
