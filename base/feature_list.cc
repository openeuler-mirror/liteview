...
BASE_FEATURE(kCacheFeatureOverrideState, "CacheFeatureOverrideState", base::FEATURE_ENABLED_BY_DEFAULT);
...
struct FeatureEntry {
    ...
    bool GetFeatureAndTrialName(StringPiece* feature_name, StringPiece* trial_name) const
    {
...
bool SplitIntoTwo(StringPiece text, StringPiece separator, StringPiece* first, std::string* second)
{
    std::vector<StringPiece> parts = SplitStringPiece(text, separator, TRIM_WHITESPACE, SPLIT_WANT_ALL);
    if (parts.size() == 2) {
...
void FeatureList::InitializeFromCommandLine(const std::string& enable_features, const std::string& disable_features)
{
    DCHECK(!initialized_);
...
void FeatureList::InitializeFromSharedMemory(PersistentMemoryAllocator* allocator)
{
    DCHECK(!initialized_);
...
{
    auto it = overrides_.find(feature_name);
    return it != overrides_.end() && !it->second.overridden_by_field_trial && it->second.overridden_state == state;
}
...
// static
std::vector<StringPiece> FeatureList::SplitFeatureListString(StringPiece input)
{
...
bool FeatureList::ParseEnableFeatureString(
    StringPiece enable_feature, std::string* feature_name, std::string* study_name, std::string* group_name, std::string* params)
{
    StringPiece first;
...
// static
bool FeatureList::InitializeInstance(const std::string& enable_features, const std::string& disable_features)
{
    return InitializeInstance(enable_features, disable_features, std::vector<FeatureOverrideInfo>());
}

// static
bool FeatureList::InitializeInstance(
    const std::string& enable_features, const std::string& disable_features, const std::vector<FeatureOverrideInfo>& extra_overrides)
{
...
void FeatureList::SetInstance(std::unique_ptr<FeatureList> instance)
{
    ...
    g_cache_override_state = base::FeatureList::IsEnabled(kCacheFeatureOverrideState);
...
absl::optional<bool> FeatureList::IsFeatureEnabledIfOverridden(const Feature& feature) const
{
    OverrideState overridden_state = GetOverrideState(feature);
...
FeatureList::OverrideState FeatureList::GetOverrideStateByFeatureName(StringPiece feature_name) const
{
    DCHECK(initialized_);
...
const base::FeatureList::OverrideEntry* FeatureList::GetOverrideEntryByFeatureName(StringPiece name) const
{
    DCHECK(initialized_);
...
FieldTrial* FeatureList::GetAssociatedFieldTrialByFeatureName(StringPiece name) const
{
    DCHECK(initialized_);
...
FieldTrial* FeatureList::GetEnabledFieldTrialByFeatureName(StringPiece name) const
{
    DCHECK(initialized_);
...
void FeatureList::RegisterOverride(StringPiece feature_name, OverrideState overridden_state, FieldTrial* field_trial)
{
    DCHECK(!initialized_);
...
FeatureList::OverrideState FeatureList::Accessor::GetOverrideStateByFeatureName(StringPiece feature_name)
{
    return feature_list_->GetOverrideStateByFeatureName(feature_name);
}

bool FeatureList::Accessor::GetParamsByFeatureName(StringPiece feature_name, std::map<std::string, std::string>* params)
{
