...
#if defined(ARCH_CPU_ARM_FAMILY) && (BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
StringPairs::const_iterator FindFirstProcCpuKey(const StringPairs& pairs, StringPiece key)
{
    return ranges::find_if(pairs, [key](const StringPairs::value_type& pair) { return TrimWhitespaceASCII(pair.first, base::TRIM_ALL) == key; });
}
...
void CPU::Initialize(bool require_branding)
{
    ...
    static_assert(kVendorNameSize < std::size(cpu_string), "cpu_string too small");
    ...
    static_assert(kParameterSize * sizeof(cpu_info) + 1 == std::size(cpu_string), "cpu_string has wrong size");
...
constexpr char kTimeInStatePath[] = "/sys/devices/system/cpu/cpu%" PRIuS "/cpufreq/stats/time_in_state";
constexpr char kPhysicalPackageIdPath[] = "/sys/devices/system/cpu/cpu%" PRIuS "/topology/physical_package_id";
constexpr char kCoreIdleStateTimePath[] = "/sys/devices/system/cpu/cpu%" PRIuS "/cpuidle/state%d/time";
...
bool SupportsTimeInState()
{
    ...
    FilePath time_in_state_path(StringPrintf(kTimeInStatePath, /*core_index=*/size_t { 0 }));
    ScopedFILE file_stream(OpenFile(time_in_state_path, "rb"));
    return static_cast<bool>(file_stream);
}

bool ParseTimeInState(const std::string& content, CPU::CoreType core_type, size_t core_index, CPU::TimeInState& time_in_state)
{
    ...
    for (size_t pos = 0; pos <= max_pos;) {
        ...
        // frequency).
        if (time > 0) {
            time_in_state.push_back({ core_type, core_index, frequency, internal::ClockTicksToTimeDelta(time) });
        }
...
bool SupportsCoreIdleTimes()
{
    ...
    FilePath idle_state0_path(StringPrintf(kCoreIdleStateTimePath, /*core_index=*/size_t { 0 }, /*idle_state=*/0));
...
std::vector<CPU::CoreType> GuessCoreTypes()
{
    ...
    const char kCPUMaxFreqPath[] = "/sys/devices/system/cpu/cpu%" PRIuS "/cpufreq/cpuinfo_max_freq";
    ...
    for (size_t core_index = 0; core_index < num_cpus; ++core_index) {
        ...
        if (num_frequencies == 1u) {
            ...
            if (it != frequencies_mhz.end()) {
                ...
                case 0:
                    core_type = num_frequencies == 2u ? CPU::CoreType::kBigLittle_Little : CPU::CoreType::kBigLittleBigger_Little;
                    break;
                case 1:
                    core_type = num_frequencies == 2u ? CPU::CoreType::kBigLittle_Big : CPU::CoreType::kBigLittleBigger_Big;
                    break;
                case 2:
...
bool CPU::GetTimeInState(TimeInState& time_in_state)
{
    ...
    static NoDestructor<std::vector<size_t>> kFirstCoresIndexes([]() {
        ...
        for (size_t core_index = 0; core_index < static_cast<size_t>(SysInfo::NumberOfProcessors()); core_index++) {
            ...
            int package_id;
            base::StringPiece trimmed = base::TrimWhitespaceASCII(package_id_str, base::TrimPositions::TRIM_ALL);
    ...
    for (size_t cluster_core_index : *kFirstCoresIndexes) {
        ...
        if (!ParseTimeInState(buffer, kCoreTypes[cluster_core_index], cluster_core_index, time_in_state)) {
            return false;
        }
    }
