...
private:
    ...
    struct ClientData {
        ClientData(ScopedDispatchObject<dispatch_source_t> exit_watcher, MachPortsForRendezvous ports);
    ...
    std::unique_ptr<uint8_t[]> CreateReplyMessage(mach_port_t reply_port, const MachPortsForRendezvous& ports);
...
public:
    ...
    // call from any thread. DCHECKs if the right referenced by |key| is not a
    // receive right.
    mac::ScopedMachReceiveRight TakeReceiveRight(MachPortsForRendezvous::key_type key);
...
private:
    ...
    bool SendRequest(mac::ScopedMachSendRight server_port) EXCLUSIVE_LOCKS_REQUIRED(lock_);
