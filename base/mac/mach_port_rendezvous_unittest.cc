...
MULTIPROCESS_TEST_MAIN(TakeSendRight)
{
    ...
    mac::ScopedMachSendRight port = rendezvous_client->TakeSendRight(kTestPortKey);
    CHECK(port.is_valid());
...
TEST_F(MachPortRendezvousServerTest, SendRight)
{
    ...
    kern_return_t kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, mac::ScopedMachReceiveRight::Receiver(port).get());
