...
{
    return sizeof(mach_msg_base_t) + (num_ports * sizeof(mach_msg_port_descriptor_t)) + (num_ports * sizeof(MachPortsForRendezvous::key_type));
}
...
MachRendezvousPort::MachRendezvousPort(mac::ScopedMachReceiveRight receive_right)
...
void MachPortRendezvousServer::RegisterPortsForPid(pid_t pid, const MachPortsForRendezvous& ports)
{
    ...
    auto it = client_data_.emplace(pid, ClientData { std::move(exit_watcher), ports });
    DCHECK(it.second);
}

MachPortRendezvousServer::ClientData::ClientData(ScopedDispatchObject<dispatch_source_t> exit_watcher, MachPortsForRendezvous ports)
...
{
    std::string bootstrap_name = StringPrintf(kBootstrapNameFormat, mac::BaseBundleID(), getpid());
    kern_return_t kr = bootstrap_check_in(bootstrap_port, bootstrap_name.c_str(), mac::ScopedMachReceiveRight::Receiver(server_port_).get());
    BOOTSTRAP_CHECK(kr == KERN_SUCCESS, kr) << "bootstrap_check_in " << bootstrap_name;

    dispatch_source_ = std::make_unique<DispatchSourceMach>(bootstrap_name.c_str(), server_port_.get(), ^{
        HandleRequest();
    });
...
void MachPortRendezvousServer::HandleRequest()
{
    ...
    if (request.msgh_id != kMachRendezvousMsgIdRequest || request.msgh_size != sizeof(mach_msg_header_t)) {
    ...
    std::unique_ptr<uint8_t[]> response = CreateReplyMessage(request.msgh_remote_port, ports_to_send);
...
std::unique_ptr<uint8_t[]> MachPortRendezvousServer::CreateReplyMessage(mach_port_t reply_port, const MachPortsForRendezvous& ports)
{
...
mac::ScopedMachSendRight MachPortRendezvousClient::TakeSendRight(MachPortsForRendezvous::key_type key)
{
...
mac::ScopedMachReceiveRight MachPortRendezvousClient::TakeReceiveRight(MachPortsForRendezvous::key_type key)
{
    MachRendezvousPort port = PortForKey(key);
...
bool MachPortRendezvousClient::AcquirePorts()
{
    ...
    kern_return_t kr = bootstrap_look_up(bootstrap_port, const_cast<char*>(bootstrap_name.c_str()), mac::ScopedMachSendRight::Receiver(server_port).get());
    if (kr != KERN_SUCCESS) {
...
bool MachPortRendezvousClient::SendRequest(mac::ScopedMachSendRight server_port)
{
    const size_t buffer_size = CalculateResponseSize(kMaximumRendezvousPorts) + sizeof(mach_msg_trailer_t);
    auto buffer = std::make_unique<uint8_t[]>(buffer_size);
    BufferIterator<uint8_t> iterator(buffer.get(), buffer_size);
    ...
    kern_return_t mr = mach_msg(&message->header, MACH_SEND_MSG | MACH_RCV_MSG, message->header.msgh_size, checked_cast<mach_msg_size_t>(buffer_size),
