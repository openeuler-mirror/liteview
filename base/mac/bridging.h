...
inline BASE_EXPORT NSFont* _Nullable CFToNSOwnershipCast(CTFontRef CF_CONSUMED _Nullable cf_val)
{
    NSFont* ns_val = (__bridge_transfer NSFont*)cf_val;
    DCHECK(!cf_val || CTFontGetTypeID() == CFGetTypeID(cf_val) || (_CFIsObjC(CTFontGetTypeID(), cf_val) && [ns_val isKindOfClass:[NSFont class]]));
    return ns_val;
}

inline BASE_EXPORT CF_RETURNS_RETAINED _Nullable CTFontRef NSToCFOwnershipCast(NSFont* _Nullable ns_val)
{
    CTFontRef cf_val = (__bridge_retained CTFontRef)ns_val;
    DCHECK(!cf_val || CTFontGetTypeID() == CFGetTypeID(cf_val) || [ns_val isKindOfClass:[NSFont class]]);
    return cf_val;
}
