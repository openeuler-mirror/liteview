...
void ListenFilteredByPid(SimpleTestLogListener& listener)
{
    ...
    std::unique_ptr<fuchsia::logger::LogFilterOptions> options = std::make_unique<fuchsia::logger::LogFilterOptions>();
    options->filter_by_pid = true;
    options->pid = Process::Current().Pid();
    options->min_severity = fuchsia::logger::LogLevelFilter::INFO;
    fuchsia::logger::LogPtr log = ComponentContextForProcess()->svc()->Connect<fuchsia::logger::Log>();
...
TEST(FuchsiaLoggingTest, SystemLogging)
{
    ...
    absl::optional<fuchsia::logger::LogMessage> logged_message = listener.RunUntilMessageReceived(kLogMessage);

    ASSERT_TRUE(logged_message.has_value());
    EXPECT_EQ(logged_message->severity, static_cast<int32_t>(fuchsia::logger::LogLevelFilter::ERROR));
...
TEST(FuchsiaLoggingTest, SystemLoggingMultipleTags)
{
    ...
    ScopedFxLogger logger = ScopedFxLogger::CreateFromLogSink(ComponentContextForProcess()->svc()->Connect<fuchsia::logger::LogSink>(), kTags);
    logger.LogMessage("", 0, kLogMessage, FUCHSIA_LOG_ERROR);

    absl::optional<fuchsia::logger::LogMessage> logged_message = listener.RunUntilMessageReceived(kLogMessage);

    ASSERT_TRUE(logged_message.has_value());
    auto tags = std::vector<StringPiece>(logged_message->tags.begin(), logged_message->tags.end());
