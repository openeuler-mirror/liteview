...
// static
ScopedFxLogger ScopedFxLogger::CreateForProcess(std::vector<base::StringPiece> tags)
{
    ...
    zx_status_t status = fdio_service_connect("/svc/fuchsia.logger.LogSink", log_sink.NewRequest().TakeChannel().release());
    if (status != ZX_OK) {
...
// static
ScopedFxLogger ScopedFxLogger::CreateFromLogSink(fuchsia::logger::LogSinkHandle log_sink_handle, std::vector<base::StringPiece> tags)
{
    // CHECK()ing or LOG()ing inside this function is safe, since it is only
    // called to initialize logging, not during individual logging operations.
...
void ScopedFxLogger::LogMessage(base::StringPiece file, uint32_t line_number, base::StringPiece msg, FuchsiaLogSeverity severity)
{
    if (!socket_.is_valid())
        return;
    ...
    buffer.BeginRecord(severity, cpp17::string_view(file.data(), file.size()), line_number, cpp17::string_view(msg.data(), msg.size()), false, socket_.borrow(),
...
ScopedFxLogger::ScopedFxLogger(std::vector<base::StringPiece> tags, zx::socket socket)
