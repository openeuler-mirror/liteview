...
TEST_F(ScopedServiceBindingTest, ConnectTwiceNewName)
{
    ...
    ScopedServiceBinding<testfidl::TestInterface> new_service_binding(outgoing_directory_.get(), &test_service_, kInterfaceName);

    testfidl::TestInterfacePtr stub, stub2;
    public_service_directory_->Connect(kInterfaceName, stub.NewRequest().TakeChannel());
    public_service_directory_->Connect(kInterfaceName, stub2.NewRequest().TakeChannel());
    VerifyTestInterface(&stub, ZX_OK);
    VerifyTestInterface(&stub2, ZX_OK);
}
...
{
    const char kInterfaceName[] = "fuchsia.TestInterface2";
    auto service_binding_new_name_
        = std::make_unique<ScopedSingleClientServiceBinding<testfidl::TestInterface>>(outgoing_directory_.get(), &test_service_, kInterfaceName);
...
TEST_F(ScopedServiceBindingTest, SingleClientPreferNew)
{
    ...
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kPreferNew> binding(outgoing_directory_.get(), &test_service_);

    // Connect the first client, and verify that it is functional.
    auto existing_client = public_service_directory_->Connect<testfidl::TestInterface>();
    ...
    auto new_client = public_service_directory_->Connect<testfidl::TestInterface>();
...
TEST_F(ScopedServiceBindingTest, SingleClientPreferExisting)
{
    ...
    service_binding_ = nullptr;
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kPreferExisting> binding(outgoing_directory_.get(), &test_service_);
...
TEST_F(ScopedServiceBindingTest, SingleClientDefaultIsPreferNew)
{
    // Teardown the default multi-client binding and create a prefer-new one.
    service_binding_ = nullptr;
    ScopedSingleClientServiceBinding<testfidl::TestInterface> binding(outgoing_directory_.get(), &test_service_);
...
TEST_F(ScopedServiceBindingTest, ConnectDebugService)
{
    ...
    ScopedServiceBinding<testfidl::TestInterface> debug_service_binding(outgoing_directory_->debug_dir(), &test_service_);

    auto debug_stub = debug_service_directory_->Connect<testfidl::TestInterface>();
    VerifyTestInterface(&debug_stub, ZX_OK);

    auto release_stub = public_service_directory_->Connect<testfidl::TestInterface>();
...
{
    service_binding_.reset();
    ScopedSingleClientServiceBinding<testfidl::TestInterface> single_service_binding(outgoing_directory_.get(), &test_service_);

    base::RunLoop run_loop;
    single_service_binding.SetOnLastClientCallback(run_loop.QuitClosure());

    auto current_client = public_service_directory_->Connect<testfidl::TestInterface>();
...
TEST_F(ScopedServiceBindingTest, ConnectOnce_OnlyFirstConnectionSucceeds)
{
    ...
    service_binding_ = nullptr;
    ScopedSingleClientServiceBinding<testfidl::TestInterface, ScopedServiceBindingPolicy::kConnectOnce> binding(outgoing_directory_.get(), &test_service_);
...
public:
    MultiUseBindingTest()
    {
        service_binding_->SetOnLastClientCallback(BindRepeating(&MultiUseBindingTest::OnLastClient, Unretained(this)));
    }
