...
TEST_F(TimeZoneDataTest, CompareSystemRevisionWithExpected)
{
    ...
    std::string expected;
    ASSERT_TRUE(base::ReadFileToString(base::FilePath(kRevisionFilePath), &expected));
    std::string actual;
...
TEST_F(TimeZoneDataTest, DoesNotCrashWithInvalidPath)
{
    ...
    std::string actual;
    GetActualRevision(&actual);
    EXPECT_TRUE(base::StartsWith(actual, "20", base::CompareCase::INSENSITIVE_ASCII)) << "Got version: " << actual;
}
