...
class FakeHardwareInfoProduct : public fuchsia::hwinfo::testing::Product_TestBase {
public:
    FakeHardwareInfoProduct(const base::StringPiece model, const base::StringPiece manufacturer, sys::OutgoingDirectory* outgoing_services)
        : model_(model)
...
protected:
    ...
    fuchsia::hwinfo::ProductInfo GetProductInfoViaTask()
    {
        ...
        thread_.task_runner()->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&GetProductInfo),
            base::BindOnce(
                [](base::RunLoop& run_loop, fuchsia::hwinfo::ProductInfo& product_info, fuchsia::hwinfo::ProductInfo result) {
