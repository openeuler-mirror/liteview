...
absl::optional<std::u16string> ReadUTF8FromVMOAsUTF16(const fuchsia::mem::Buffer& buffer)
{
    absl::optional<std::string> output_utf8 = StringFromMemBuffer(buffer);
    if (!output_utf8)
        return absl::nullopt;
    std::u16string output;
    return UTF8ToUTF16(&output_utf8->front(), output_utf8->size(), &output) ? absl::optional<std::u16string>(std::move(output)) : absl::nullopt;
}
...
fuchsia::mem::Buffer MemBufferFromString16(StringPiece16 data, StringPiece name)
{
    return MemBufferFromString(StringPiece(reinterpret_cast<const char*>(data.data()), data.size() * sizeof(char16_t)), name);
}
...
absl::optional<std::string> StringFromMemBuffer(const fuchsia::mem::Buffer& buffer)
{
    std::string result;
...
fuchsia::mem::Buffer CloneBuffer(const fuchsia::mem::Buffer& buffer, StringPiece name)
{
    fuchsia::mem::Buffer output;
