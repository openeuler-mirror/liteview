...
TestComponentContextForProcess::TestComponentContextForProcess(InitialState initial_state)
{
    ...
    old_context_ = ReplaceComponentContextForProcessForTest(
        std::make_unique<sys::ComponentContext>(std::move(incoming_services), published_root_directory.NewRequest().TakeChannel()));
    ...
    published_services_ = std::make_unique<sys::ServiceDirectory>(std::move(published_services));
}
...
void TestComponentContextForProcess::AddService(const base::StringPiece service)
{
    zx_status_t status = context_services_->AddService(service);
    ZX_CHECK(status == ZX_OK, status) << "AddService(" << service << ") failed";
}

void TestComponentContextForProcess::AddServices(base::span<const base::StringPiece> services)
{
