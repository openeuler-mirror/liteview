...
class TestComponentContextForProcessTest : public testing::Test, public testfidl::TestInterface {
public:
    ...
    {
        return VerifyTestInterface(ComponentContextForProcess()->svc()->Connect<testfidl::TestInterface>());
    }

    bool HasPublishedTestInterface()
    {
        return VerifyTestInterface(test_context_.published_services()->Connect<testfidl::TestInterface>());
    }
...
protected:
    bool VerifyTestInterface(testfidl::TestInterfacePtr test_interface)
    {
        ...
        RunLoop wait_loop;
        test_interface.set_error_handler([quit_loop = wait_loop.QuitClosure(), &have_interface](zx_status_t status) {
            ZX_CHECK(status == ZX_ERR_PEER_CLOSED, status);
            have_interface = false;
            quit_loop.Run();
        });
        test_interface->Add(45, 6, [quit_loop = wait_loop.QuitClosure(), &have_interface](int32_t result) {
...
TEST_F(TestComponentContextForProcessTest, InjectTestInterface)
{
    ...
    base::ScopedServiceBinding<testfidl::TestInterface> service_binding(test_context_.additional_services(), this);
...
TEST_F(TestComponentContextForProcessTest, PublishTestInterface)
{
    ...
    base::ScopedServiceBinding<testfidl::TestInterface> service_binding(ComponentContextForProcess()->outgoing().get(), this);
...
TEST_F(TestComponentContextForProcessTest, ProvideSystemService)
{
    ...
    RunLoop wait_loop;
    auto loader = ComponentContextForProcess()->svc()->Connect<::fuchsia::sys::Loader>();
    loader.set_error_handler([quit_loop = wait_loop.QuitClosure()](zx_status_t status) {
        ZX_LOG(ERROR, status);
        ADD_FAILURE();
        quit_loop.Run();
    });
    ...
    const char kComponentUrl[] = "fuchsia-pkg://fuchsia.com/base_unittests#meta/base_unittests.cm";
    loader->LoadUrl(kComponentUrl, [quit_loop = wait_loop.QuitClosure(), expected_path = kComponentUrl](::fuchsia::sys::PackagePtr package) {
