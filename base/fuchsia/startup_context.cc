...
StartupContext::StartupContext(fuchsia::sys::StartupInfo startup_info)
{
    ...
    CHECK_EQ(startup_info.flat_namespace.directories.size(), startup_info.flat_namespace.paths.size());
    ...
    for (size_t i = 0; i < startup_info.flat_namespace.paths.size(); ++i) {
        if (startup_info.flat_namespace.paths[i] == kServiceDirectoryPath) {
            incoming_services = std::make_unique<sys::ServiceDirectory>(std::move(startup_info.flat_namespace.directories[i]));
            break;
        }
    }
    ...
    if (!incoming_services && startup_info.launch_info.flat_namespace) {
        ...
        for (size_t i = 0; i < startup_info.launch_info.flat_namespace->paths.size(); ++i) {
            if (startup_info.launch_info.flat_namespace->paths[i] == kServiceDirectoryPath) {
                incoming_services = std::make_unique<sys::ServiceDirectory>(std::move(startup_info.launch_info.flat_namespace->directories[i]));
                break;
            }
        }
    }
    ...
    if (!incoming_services && startup_info.launch_info.additional_services) {
        ...
        // it.
        additional_services_.Bind(std::move(startup_info.launch_info.additional_services->provider));
        ...
        for (auto& name : startup_info.launch_info.additional_services->names) {
            zx_status_t status = additional_services_directory_->AddPublicService(
                std::make_unique<vfs::Service>(
                    [this, name](zx::channel channel, async_dispatcher_t* dispatcher) { additional_services_->ConnectToService(name, std::move(channel)); }),
                name);
            ZX_CHECK(status == ZX_OK, status) << "AddPublicService(" << name << ") failed";
        }
        ...
        additional_services_directory_->GetOrCreateDirectory("svc")->Serve(
            fuchsia::io::OpenFlags::RIGHT_READABLE | fuchsia::io::OpenFlags::RIGHT_WRITABLE, incoming_directory.NewRequest().TakeChannel());
        incoming_services = std::make_unique<sys::ServiceDirectory>(std::move(incoming_directory));
    }
    ...
    if (!incoming_services) {
        ...
        incoming_services = std::make_unique<sys::ServiceDirectory>(std::move(dummy_directory));
    }

    component_context_ = std::make_unique<sys::ComponentContext>(std::move(incoming_services));
    outgoing_directory_request_ = std::move(startup_info.launch_info.directory_request);
}
