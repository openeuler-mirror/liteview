...
{
    base::ScopedCFTypeRef<CFTimeZoneRef> time_zone(is_local ? CFTimeZoneCopySystem() : CFTimeZoneCreateWithTimeIntervalFromGMT(kCFAllocatorDefault, 0));
    base::ScopedCFTypeRef<CFCalendarRef> gregorian(CFCalendarCreateWithIdentifier(kCFAllocatorDefault, kCFGregorianCalendar));
    ...
    CFCalendarComposeAbsoluteTime(gregorian, &absolute_time, "yMdHmsS", exploded.year, exploded.month, exploded.day_of_month, exploded.hour, exploded.minute,
    ...
    double microseconds = (seconds * kMicrosecondsPerSecond) + kTimeTToMicrosecondsOffset;
...
void Time::Explode(bool is_local, Exploded* exploded) const
{
    ...
    CFAbsoluteTime seconds = ((us_ - microsecond - kTimeTToMicrosecondsOffset) / kMicrosecondsPerSecond) - kCFAbsoluteTimeIntervalSince1970;
    ...
    CFCalendarDecomposeAbsoluteTime(
        gregorian, seconds, "yMdHmsE", &exploded->year, &exploded->month, &exploded->day_of_month, &exploded->hour, &exploded->minute, &second, &day_of_week);
    ...
    exploded->millisecond = (microsecond >= 0) ? microsecond / kMicrosecondsPerMillisecond : ((microsecond + 1) / kMicrosecondsPerMillisecond - 1);
}
