...
constexpr absl::optional<ParsedDecimal> ConsumeDurationNumber(StringPiece& number_string)
{
    ParsedDecimal res;
    ...
    return int_part_empty && res.frac_scale == 1 ? absl::nullopt : absl::make_optional(res);
}
...
absl::optional<TimeDelta> TimeDeltaFromString(StringPiece duration_string)
{
    ...
    while (!duration_string.empty()) {
        absl::optional<ParsedDecimal> number_opt = ConsumeDurationNumber(duration_string);
