...
absl::optional<base::TimeDelta> GetTimeZoneOffsetAtTime(const char* timezone_id, Time time)
{
...
{
    constexpr Time kMaxMinusOne = Time::FromInternalValue(std::numeric_limits<int64_t>::max() - 1);
...
TEST_F(TimeTest, LocalExplodedIsLocaleIndependent)
{
    ...
    absl::optional<TimeDelta> expected_delta = GetTimeZoneOffsetAtTime(kBangkokTimeZoneId, time);
...
template <class Any> std::string AnyToString(Any any)
{
...
TEST(TimeDelta, MinConversions)
{
    ...
    EXPECT_EQ(kMin.InMillisecondsRoundedUp(), std::numeric_limits<int64_t>::min());
}
...
TEST(TimeDelta, Overflows)
{
    ...
    EXPECT_EQ((kLargeDelta + kOneSecond).InMillisecondsF(), std::numeric_limits<double>::infinity());
    EXPECT_EQ((kLargeDelta + kOneSecond).InMicrosecondsF(), std::numeric_limits<double>::infinity());
...
TEST(TimeTicksLogging, FortyYearsLater)
{
    ...
    EXPECT_EQ("1262304000000000 bogo-microseconds", AnyToString(forty_years_later));
}
