...
scoped_refptr<FieldTrial> CreateFieldTrial(const std::string& trial_name, int total_probability, const std::string& default_group_name)
{
    return FieldTrialList::FactoryGetFieldTrial(
        trial_name, total_probability, default_group_name, base::FieldTrialList::GetEntropyProviderForSessionRandomization());
}
...
public:
    ...
    void OnFieldTrialGroupFinalized(const std::string& trial, const std::string& group) override
    {
        trial_name_ = trial;
        group_name_ = group;
    }
...
TEST_F(FieldTrialTest, AbsoluteProbabilities)
{
    ...
    for (int i = 1; i < 250; ++i) {
        ...
        scoped_refptr<FieldTrial> trial_true = CreateFieldTrial(always_true, 10, default_always_true);
        const std::string winner = "TheWinner";
        trial_true->AppendGroup(winner, 10);

        EXPECT_EQ(winner, trial_true->group_name());

        scoped_refptr<FieldTrial> trial_false = CreateFieldTrial(always_false, 10, default_always_false);
...
TEST_F(FieldTrialTest, FiftyFiftyProbability)
{
    ...
    do {
        ...
        scoped_refptr<FieldTrial> trial = CreateFieldTrial(name, 2, default_group_name);
...
TEST_F(FieldTrialTest, MiddleProbabilities)
{
    ...
    for (int i = 1; i < 250; ++i) {
        ...
        scoped_refptr<FieldTrial> trial = CreateFieldTrial(name, 10, default_group_name);
...
TEST_F(FieldTrialTest, OneWinner)
{
    ...
    scoped_refptr<FieldTrial> trial = CreateFieldTrial(name, group_count, default_group_name);
...
{
    FieldTrial::ActiveGroups active_groups;
    FieldTrialList::GetActiveFieldTrialGroupsFromString("*A/X/B/Y/*C/Z", &active_groups);
...
TEST_F(FieldTrialListTest, TestCopyFieldTrialStateToFlags)
{
    ...
    feature_list->RegisterFieldTrialOverride("MyFeature", FeatureList::OVERRIDE_ENABLE_FEATURE, trial);
    ...
    FieldTrialList::PopulateLaunchOptionsWithFieldTrialState(&command_line, &launch_options);
...
{
    std::string serialized = CommandLine::ForCurrentProcess()->GetSwitchValueASCII("field_trials");
    std::string guid_string = CommandLine::ForCurrentProcess()->GetSwitchValueASCII("guid");
    ...
    base::ReadOnlySharedMemoryRegion deserialized = FieldTrialList::DeserializeSharedMemoryRegionMetadata(serialized, fd);
...
TEST_F(FieldTrialListTest, SerializeSharedMemoryRegionMetadata)
{
    ...
    std::string serialized = FieldTrialList::SerializeSharedMemoryRegionMetadata(shm.region, &options);
    ...
    Process process = SpawnMultiProcessTestChild("SerializeSharedMemoryRegionMetadata", cmd_line, options);
