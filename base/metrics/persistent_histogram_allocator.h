...
public:
    ...
    PersistentSampleMapRecords* UseSampleMapRecords(uint64_t id, const void* user);
...
private:
    ...
    PersistentSampleMapRecords* GetSampleMapRecordsWhileLocked(uint64_t id) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::map<uint64_t, std::unique_ptr<PersistentSampleMapRecords>> sample_records_ GUARDED_BY(lock_);
...
public:
    ...
    // get recorded again.
    void MergeHistogramFinalDeltaToStatisticsRecorder(const HistogramBase* histogram);
...
public:
    ...
    static void CreateWithPersistentMemory(void* base, size_t size, size_t page_size, uint64_t id, StringPiece name);
    ...
    static bool CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, StringPiece name, bool exclusive_write = false);
    ...
    // |dir|.
    static bool CreateWithActiveFileInDir(const FilePath& dir, size_t size, uint64_t id, StringPiece name);
    ...
    static FilePath ConstructFilePathForActiveFile(const FilePath& dir, StringPiece name);

    // Like above but with timestamp and pid for use in upload directories.
    static FilePath ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name, base::Time stamp, ProcessId pid);

    // Override that uses the current time stamp and current process id.
    static FilePath ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name);
    ...
    static void CreateWithSharedMemoryRegion(const WritableSharedMemoryRegion& region);
