...
#define kPersistentHistogramsFeatureValue FEATURE_DISABLED_BY_DEFAULT
#else
#define kPersistentHistogramsFeatureValue FEATURE_ENABLED_BY_DEFAULT
#endif // BUILDFLAG(IS_FUCHSIA)
BASE_FEATURE(kPersistentHistogramsFeature, "PersistentHistograms", kPersistentHistogramsFeatureValue);
...
PersistentSampleMapRecords* PersistentSparseHistogramDataManager::UseSampleMapRecords(uint64_t id, const void* user)
{
    base::AutoLock auto_lock(lock_);
    return GetSampleMapRecordsWhileLocked(id)->Acquire(user);
}

PersistentSampleMapRecords* PersistentSparseHistogramDataManager::GetSampleMapRecordsWhileLocked(uint64_t id)
{
...
bool PersistentSparseHistogramDataManager::LoadRecords(PersistentSampleMapRecords* sample_map_records)
{
    ...
    if (!sample_map_records->found_.empty()) {
        ...
        sample_map_records->records_.insert(sample_map_records->records_.end(), sample_map_records->found_.begin(), sample_map_records->found_.end());
    ...
    for (int count = 0; !found || count < kMinimumNumberToLoad; ++count) {
        ...
        uint64_t found_id;
        PersistentMemoryAllocator::Reference ref = PersistentSampleMap::GetNextPersistentRecord(record_iterator_, &found_id);
...
PersistentSampleMapRecords::PersistentSampleMapRecords(PersistentSparseHistogramDataManager* data_manager, uint64_t sample_map_id)
...
PersistentSampleMapRecords* PersistentSampleMapRecords::Acquire(const void* user)
{
    DCHECK(!user_);
    user_ = user;
    seen_ = 0;
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::AllocateHistogram(
    HistogramType histogram_type, const std::string& name, int minimum, int maximum, const BucketRanges* bucket_ranges, int32_t flags, Reference* ref_ptr)
{
    ...
    PersistentHistogramData* histogram_data = memory_allocator_->New<PersistentHistogramData>(offsetof(PersistentHistogramData, name) + name.length() + 1);
    if (histogram_data) {
...
void PersistentHistogramAllocator::MergeHistogramDeltaToStatisticsRecorder(HistogramBase* histogram)
{
    DCHECK(histogram);
...
void PersistentHistogramAllocator::MergeHistogramFinalDeltaToStatisticsRecorder(const HistogramBase* histogram)
{
    DCHECK(histogram);
...
PersistentSampleMapRecords* PersistentHistogramAllocator::UseSampleMapRecords(uint64_t id, const void* user)
{
...
std::unique_ptr<HistogramBase> PersistentHistogramAllocator::CreateHistogram(PersistentHistogramData* histogram_data_ptr)
{
    ...
    HistogramBase::Sample* ranges_data
        = memory_allocator_->GetAsArray<HistogramBase::Sample>(histogram_ranges_ref, kTypeIdRangesArray, PersistentMemoryAllocator::kSizeAny);
    ...
    DCHECK_EQ(created_ranges->range(histogram_bucket_count - 1), histogram_maximum);
    const BucketRanges* ranges;
    if (ranges_manager_) {
        ranges = ranges_manager_->RegisterOrDeleteDuplicateRanges(created_ranges.release());
    } else {
    ...
    if (counts_bytes == 0 || (counts_ref != 0 && memory_allocator_->GetAllocSize(counts_ref) < counts_bytes)) {
        return nullptr;
    }
    ...
    DelayedPersistentAllocation counts_data(memory_allocator_.get(), &histogram_data_ptr->counts_ref, kTypeIdCountsArray, counts_bytes, false);
...
// static
void GlobalHistogramAllocator::CreateWithPersistentMemory(void* base, size_t size, size_t page_size, uint64_t id, StringPiece name)
{
    Set(WrapUnique(new GlobalHistogramAllocator(std::make_unique<PersistentMemoryAllocator>(base, size, page_size, id, name, false))));
}

// static
void GlobalHistogramAllocator::CreateWithLocalMemory(size_t size, uint64_t id, StringPiece name)
{
    Set(WrapUnique(new GlobalHistogramAllocator(std::make_unique<LocalPersistentMemoryAllocator>(size, id, name))));
}

#if !BUILDFLAG(IS_NACL)
// static
bool GlobalHistogramAllocator::CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, StringPiece name, bool exclusive_write)
{
    ...
    Set(WrapUnique(new GlobalHistogramAllocator(std::make_unique<FilePersistentMemoryAllocator>(std::move(mmfile), 0, id, name, false))));
...
bool GlobalHistogramAllocator::CreateWithActiveFile(
    const FilePath& base_path, const FilePath& active_path, const FilePath& spare_path, size_t size, uint64_t id, StringPiece name)
{
...
// static
bool GlobalHistogramAllocator::CreateWithActiveFileInDir(const FilePath& dir, size_t size, uint64_t id, StringPiece name)
{
...
// static
FilePath GlobalHistogramAllocator::ConstructFilePath(const FilePath& dir, StringPiece name)
{
    return dir.AppendASCII(name).AddExtension(PersistentMemoryAllocator::kFileExtension);
}

// static
FilePath GlobalHistogramAllocator::ConstructFilePathForActiveFile(const FilePath& dir, StringPiece name)
{
    return ConstructFilePath(dir, std::string(name) + "-active");
}

// static
FilePath GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name, base::Time stamp, ProcessId pid)
{
...
// static
FilePath GlobalHistogramAllocator::ConstructFilePathForUploadDir(const FilePath& dir, StringPiece name)
{
...
bool GlobalHistogramAllocator::ParseFilePath(const FilePath& path, std::string* out_name, Time* out_stamp, ProcessId* out_pid)
{
    ...
    std::vector<base::StringPiece> parts = base::SplitStringPiece(filename, "-.", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    if (parts.size() != 4)
        return false;
...
// static
void GlobalHistogramAllocator::CreateWithSharedMemoryRegion(const WritableSharedMemoryRegion& region)
{
    ...
    Set(WrapUnique(new GlobalHistogramAllocator(std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(mapping), 0, StringPiece()))));
}

// static
void GlobalHistogramAllocator::Set(std::unique_ptr<GlobalHistogramAllocator> allocator)
{
    // Releasing or changing an allocator is extremely dangerous because it
    // likely has histograms stored within it. If the backing memory is also
    // also released, future accesses to those histograms will seg-fault.
    CHECK(!subtle::NoBarrier_Load(&g_histogram_allocator));
    subtle::Release_Store(&g_histogram_allocator, reinterpret_cast<intptr_t>(allocator.release()));
...
// static
std::unique_ptr<GlobalHistogramAllocator> GlobalHistogramAllocator::ReleaseForTesting()
{
