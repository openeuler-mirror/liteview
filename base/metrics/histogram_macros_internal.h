...
template <typename Enum, typename SFINAE = void> struct EnumSizeTraits {
...
template <typename Enum> struct EnumSizeTraits<Enum, std::enable_if_t<std::is_enum<decltype(Enum::kMaxValue)>::value>> {
    static constexpr Enum Count()
    {
        // If you're getting
...
#define INTERNAL_HISTOGRAM_EXACT_LINEAR_WITH_FLAG(name, sample, boundary, flag)                                                                                \
    do {                                                                                                                                                       \
        static_assert(!std::is_enum<std::decay_t<decltype(sample)>>::value, "|sample| should not be an enum type!");                                           \
        static_assert(!std::is_enum<std::decay_t<decltype(boundary)>>::value, "|boundary| should not be an enum type!");                                       \
...
#define INTERNAL_HISTOGRAM_ENUMERATION_WITH_FLAG(name, sample, boundary, flag)                                                                                 \
    do {                                                                                                                                                       \
        ...
        static_assert(!std::is_enum<decayed_boundary>::value || std::is_enum<decayed_sample>::value, "Unexpected: |boundary| is enum, but |sample| is not.");  \
        static_assert(!std::is_enum<decayed_sample>::value || !std::is_enum<decayed_boundary>::value || std::is_same<decayed_sample, decayed_boundary>::value, \
...
#define INTERNAL_HISTOGRAM_SCALED_ENUMERATION_WITH_FLAG(name, sample, count, scale, flag)                                                                      \
    do {                                                                                                                                                       \
        using decayed_sample = std::decay<decltype(sample)>::type;                                                                                             \
        static_assert(std::is_enum<decayed_sample>::value, "Unexpected: |sample| is not at enum.");                                                            \
