...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Iterator::GetNext(uint32_t* type_return)
{
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Iterator::GetNextOfType(uint32_t type_match)
{
    Reference ref;
    uint32_t type_found;
...
PersistentMemoryAllocator::PersistentMemoryAllocator(void* base, size_t size, size_t page_size, uint64_t id, base::StringPiece name, bool readonly)
    : PersistentMemoryAllocator(Memory(base, MEM_EXTERNAL), size, page_size, id, name, readonly)
{
}

PersistentMemoryAllocator::PersistentMemoryAllocator(Memory memory, size_t size, size_t page_size, uint64_t id, base::StringPiece name, bool readonly)
    ...
    , errors_histogram_(nullptr)
{
    ...
    if (shared_meta()->cookie != kGlobalCookie) {
        ...
        if (shared_meta()->size == 0 || shared_meta()->version != kGlobalVersion || shared_meta()->freeptr.load(std::memory_order_relaxed) == 0
            || shared_meta()->tailptr == 0 || shared_meta()->queue.cookie == 0 || shared_meta()->queue.next.load(std::memory_order_relaxed) == 0) {
            SetCorrupt();
        }
        if (!readonly) {
...
void PersistentMemoryAllocator::CreateTrackingHistograms(base::StringPiece name)
{
  ...
  allocs_histogram_ = Histogram::FactoryGet(
    ...
    errors_histogram_ = SparseHistogram::FactoryGet("UMA.PersistentAllocator." + name_string + ".Errors", HistogramBase::kUmaTargetedHistogramFlag);
}
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::Allocate(size_t req_size, uint32_t type_id)
{
...
PersistentMemoryAllocator::Reference PersistentMemoryAllocator::AllocateImpl(size_t req_size, uint32_t type_id)
{
    DCHECK(!readonly_);
...
const volatile PersistentMemoryAllocator::BlockHeader* PersistentMemoryAllocator::GetBlock(
    Reference ref, uint32_t type_id, size_t size, bool queue_ok, bool free_ok) const
{
...
const volatile void* PersistentMemoryAllocator::GetBlockData(Reference ref, uint32_t type_id, size_t size) const
{
    DCHECK(size > 0);
...
LocalPersistentMemoryAllocator::LocalPersistentMemoryAllocator(size_t size, uint64_t id, base::StringPiece name)
...
// static
PersistentMemoryAllocator::Memory LocalPersistentMemoryAllocator::AllocateLocalMemory(size_t size)
{
    void* address;
    ...
    UmaHistogramSparse("UMA.LocalPersistentMemoryAllocator.Failures.Win", static_cast<int>(::GetLastError()));
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
...
WritableSharedPersistentMemoryAllocator::WritableSharedPersistentMemoryAllocator(base::WritableSharedMemoryMapping memory, uint64_t id, base::StringPiece name)
    : PersistentMemoryAllocator(Memory(memory.memory(), MEM_SHARED), memory.size(), 0, id, name, false)
...
ReadOnlySharedPersistentMemoryAllocator::ReadOnlySharedPersistentMemoryAllocator(base::ReadOnlySharedMemoryMapping memory, uint64_t id, base::StringPiece name)
    : PersistentMemoryAllocator(Memory(const_cast<void*>(memory.memory()), MEM_SHARED), memory.size(), 0, id, name, true)
    , shared_memory_(std::move(memory))
{
}
...
FilePersistentMemoryAllocator::FilePersistentMemoryAllocator(
    std::unique_ptr<MemoryMappedFile> file, size_t max_size, uint64_t id, base::StringPiece name, bool read_only)
    : PersistentMemoryAllocator(Memory(const_cast<uint8_t*>(file->data()), MEM_FILE), max_size != 0 ? max_size : file->length(), 0, id, name, read_only)
...
// static
bool FilePersistentMemoryAllocator::IsFileAcceptable(const MemoryMappedFile& file, bool read_only)
{
...
DelayedPersistentAllocation::DelayedPersistentAllocation(
    PersistentMemoryAllocator* allocator, std::atomic<Reference>* ref, uint32_t type, size_t size, bool make_iterable)
    : DelayedPersistentAllocation(allocator, ref, type, size, 0, make_iterable)
{
}

DelayedPersistentAllocation::DelayedPersistentAllocation(
    PersistentMemoryAllocator* allocator, std::atomic<Reference>* ref, uint32_t type, size_t size, size_t offset, bool make_iterable)
    : allocator_(allocator)
    , type_(type)
