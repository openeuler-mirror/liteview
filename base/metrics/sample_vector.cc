...
std::unique_ptr<SampleCountIterator> SampleVectorBase::Iterator() const
{
    ...
    if (sample.count != 0) {
        return std::make_unique<SingleSampleIterator>(
            bucket_ranges_->range(sample.bucket), bucket_ranges_->range(sample.bucket + 1), sample.count, sample.bucket);
    }
...
bool SampleVectorBase::AddSubtractImpl(SampleCountIterator* iter, HistogramSamples::Operator op)
{
    ...
    while (true) {
        ...
        if (min != bucket_ranges_->range(dest_index) || max != bucket_ranges_->range(dest_index + 1)) {
            NOTREACHED() << "sample=" << min << "," << max << "; range=" << bucket_ranges_->range(dest_index) << "," << bucket_ranges_->range(dest_index + 1);
            return false;
        }

        // Sample's bucket matches exactly. Adjust count.
        subtle::NoBarrier_AtomicIncrement(&counts()[dest_index], op == HistogramSamples::ADD ? count : -count);
...
std::string SampleVector::GetAsciiHeader(StringPiece histogram_name, int32_t flags) const
{
    Count sample_count = TotalCount();
    std::string output;
    StringAppendF(&output, "Histogram: %.*s recorded %d samples", static_cast<int>(histogram_name.size()), histogram_name.data(), sample_count);
    if (sample_count == 0) {
        DCHECK_EQ(sum(), 0);
    } else {
...
HistogramBase::AtomicCount* PersistentSampleVector::CreateCountsStorageWhileLocked()
{
...
SampleVectorIterator::SampleVectorIterator(const std::vector<HistogramBase::AtomicCount>* counts, const BucketRanges* bucket_ranges)
    : counts_(&(*counts)[0])
...
SampleVectorIterator::SampleVectorIterator(const HistogramBase::AtomicCount* counts, size_t counts_size, const BucketRanges* bucket_ranges)
    : counts_(counts)
...
void SampleVectorIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const
{
    DCHECK(!Done());
    if (min != nullptr)
