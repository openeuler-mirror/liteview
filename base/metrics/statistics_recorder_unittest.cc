...
TEST_P(StatisticsRecorderTest, FindHistogram)
{
    ...
    if (use_persistent_histogram_allocator_) {
        GlobalHistogramAllocator::CreateWithPersistentMemory(
            const_cast<void*>(old_global_allocator_->data()), old_global_allocator_->length(), 0, old_global_allocator_->Id(), old_global_allocator_->Name());
    }
...
TEST_P(StatisticsRecorderTest, AddHistogramCallbackBeforeHistogramRegistration)
{
    ...
    EXPECT_EQ(histogram->flags() & base::HistogramBase::kCallbackExists, base::HistogramBase::kCallbackExists);
    EXPECT_TRUE(base::StatisticsRecorder::have_active_callbacks());
}
...
public:
    explicit TestHistogramProvider(std::unique_ptr<PersistentHistogramAllocator> allocator)
