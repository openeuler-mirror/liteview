...
public:
    ...
    double GetEntropyForTrial(StringPiece trial_name, uint32_t randomization_seed) const override
    {
        return RandDouble();
    }
};
...
void WriteStringPair(Pickle* pickle, const StringPiece& string1, const StringPiece& string2)
{
...
bool ParseFieldTrialsString(const std::string& trials_string, std::vector<FieldTrial::State>* entries)
{
...
bool DeserializeGUIDFromStringPieces(StringPiece first, StringPiece second, UnguessableToken* guid)
{
    uint64_t high = 0;
    uint64_t low = 0;
...
bool FieldTrial::FieldTrialEntry::GetTrialAndGroupName(StringPiece* trial_name, StringPiece* group_name) const
{
...
bool FieldTrial::FieldTrialEntry::ReadStringPair(PickleIterator* iter, StringPiece* trial_name, StringPiece* group_name) const
{
...
// static
FieldTrial* FieldTrial::CreateSimulatedFieldTrial(StringPiece trial_name, Probability total_probability, StringPiece default_group_name, double entropy_value)
{
    return new FieldTrial(trial_name, total_probability, default_group_name, entropy_value);
}

FieldTrial::FieldTrial(StringPiece trial_name, const Probability total_probability, StringPiece default_group_name, double entropy_value)
...
// static
FieldTrial* FieldTrialList::FactoryGetFieldTrial(StringPiece trial_name, FieldTrial::Probability total_probability, StringPiece default_group_name,
    const FieldTrial::EntropyProvider& entropy_provider, uint32_t randomization_seed)
{
    ...
    FieldTrial* field_trial = new FieldTrial(trial_name, total_probability, default_group_name, entropy_value);
...
// static
void FieldTrialList::GetActiveFieldTrialGroupsFromString(const std::string& trials_string, FieldTrial::ActiveGroups* active_groups)
{
...
// static
void FieldTrialList::GetInitiallyActiveFieldTrials(const CommandLine& command_line, FieldTrial::ActiveGroups* active_groups)
{
    DCHECK(global_);
    DCHECK(global_->create_trials_from_command_line_called_);

    if (!global_->field_trial_allocator_) {
        GetActiveFieldTrialGroupsFromString(command_line.GetSwitchValueASCII(switches::kForceFieldTrials), active_groups);
        return;
    }
    ...
    while ((entry = mem_iter.GetNextOfObject<FieldTrial::FieldTrialEntry>()) != nullptr) {
        ...
        if (subtle::NoBarrier_Load(&entry->activated) && entry->GetTrialAndGroupName(&trial_name, &group_name)) {
            FieldTrial::ActiveGroup group;
...
// static
void FieldTrialList::CreateTrialsFromCommandLine(const CommandLine& cmd_line, uint32_t fd_key)
{
...
// static
void FieldTrialList::CreateFeaturesFromCommandLine(const CommandLine& command_line, FeatureList* feature_list)
{
...
#if !BUILDFLAG(IS_IOS)
// static
void FieldTrialList::PopulateLaunchOptionsWithFieldTrialState(CommandLine* command_line, LaunchOptions* launch_options)
{
    DCHECK(command_line);
    ...
    std::string switch_value = SerializeSharedMemoryRegionMetadata(global_->readonly_allocator_region_, launch_options);
...
// static
FieldTrial* FieldTrialList::CreateFieldTrial(StringPiece name, StringPiece group_name)
{
    DCHECK(global_);
...
void FieldTrialList::NotifyFieldTrialGroupSelection(FieldTrial* field_trial)
{
    ...
    for (Observer* observer : local_observers) {
        observer->OnFieldTrialGroupFinalized(field_trial->trial_name(), field_trial->group_name_internal());
    }
...
bool FieldTrialList::GetParamsFromSharedMemory(FieldTrial* field_trial, std::map<std::string, std::string>* params)
{
    ...
    const FieldTrial::FieldTrialEntry* entry = global_->field_trial_allocator_->GetAsObject<FieldTrial::FieldTrialEntry>(field_trial->ref_);
...
// static
const FieldTrial::EntropyProvider& FieldTrialList::GetEntropyProviderForSessionRandomization()
{
    static const base::NoDestructor<SessionEntropyProvider> session_entropy_provider;
...
// static
std::string FieldTrialList::SerializeSharedMemoryRegionMetadata(const ReadOnlySharedMemoryRegion& shm, LaunchOptions* launch_options)
{
    DCHECK(launch_options);
    ...
    launch_options->mach_ports_for_rendezvous.emplace(kFieldTrialRendezvousKey, MachRendezvousPort(shm.GetPlatformHandle(), MACH_MSG_TYPE_COPY_SEND));
    ...
    zx::vmo transfer_vmo;
    zx_status_t status
        = shm.GetPlatformHandle()->duplicate(ZX_RIGHT_READ | ZX_RIGHT_MAP | ZX_RIGHT_TRANSFER | ZX_RIGHT_GET_PROPERTY | ZX_RIGHT_DUPLICATE, &transfer_vmo);
    ...
    uint32_t handle_id = LaunchOptions::AddHandleToTransfer(&launch_options->handles_to_transfer, transfer_vmo.release());
    ss << handle_id << ",";
...
// static
ReadOnlySharedMemoryRegion FieldTrialList::DeserializeSharedMemoryRegionMetadata(const std::string& switch_value, int fd)
{
    // Format: "handle,[irp],guid-high,guid-low,size".
    std::vector<StringPiece> tokens = SplitStringPiece(switch_value, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    ...
    if (!StringToInt(tokens[0], &field_trial_handle))
        ...
        // bool is_xp = win::GetVersion() <= win::Version::XP;
#endif
    ...
    if (tokens[1] == "p"
        ...
        DuplicateHandle(parent_handle, handle, GetCurrentProcess(), &handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    ...
    if (!rendezvous)
        return ReadOnlySharedMemoryRegion();
    mac::ScopedMachSendRight scoped_handle = rendezvous->TakeSendRight(static_cast<MachPortsForRendezvous::key_type>(field_trial_handle));
    ...
    zx::vmo scoped_handle(zx_take_startup_handle(checked_cast<uint32_t>(field_trial_handle)));
...
#error Unsupported OS
    ...
    auto platform_handle = subtle::PlatformSharedMemoryRegion::Take(
        std::move(scoped_handle), subtle::PlatformSharedMemoryRegion::Mode::kReadOnly, static_cast<size_t>(size), guid);
    return ReadOnlySharedMemoryRegion::Deserialize(std::move(platform_handle));
}

// static
bool FieldTrialList::CreateTrialsFromSwitchValue(const std::string& switch_value, uint32_t fd_key)
{
    int fd = -1;
    ...
    ReadOnlySharedMemoryRegion shm = DeserializeSharedMemoryRegionMetadata(switch_value, fd);
    if (!shm.IsValid())
        return false;
