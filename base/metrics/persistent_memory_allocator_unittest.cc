...
public:
    ...
    void SetUp() override
    {
        ...
        allocator_ = std::make_unique<PersistentMemoryAllocator>(mem_segment_.get(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, TEST_ID, TEST_NAME, false);
    }
...
TEST_F(PersistentMemoryAllocatorTest, AllocateAndIterate)
{
    ...
    EXPECT_GT(sizeof(TestObject1) + kAllocAlignment, allocator_->GetAllocSize(block1));
    ...
    EXPECT_GT(sizeof(TestObject2) + kAllocAlignment, allocator_->GetAllocSize(block2));
    ...
    std::unique_ptr<PersistentMemoryAllocator> allocator2(new PersistentMemoryAllocator(mem_segment_.get(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, 0, "", false));
    ...
    std::unique_ptr<const PersistentMemoryAllocator> allocator3(
        new PersistentMemoryAllocator(mem_segment_.get(), TEST_MEMORY_SIZE, TEST_MEMORY_PAGE, 0, "", true));
...
TEST_F(PersistentMemoryAllocatorTest, DelayedAllocationTest)
{
    ...
    EXPECT_EQ(allocator_->GetAsReference(mem1, 1001), ref1.load(std::memory_order_relaxed));
    ...
    void* mem2a = da2a.Get();
    ASSERT_TRUE(mem2a);
    EXPECT_EQ(allocator_->GetAsReference(mem2a, 2002), ref2.load(std::memory_order_relaxed));
    EXPECT_NE(0U, iter.GetNext(&type));
    ...
    ASSERT_TRUE(mem2b);
    EXPECT_EQ(0U, iter.GetNext(&type));
    EXPECT_EQ(reinterpret_cast<uintptr_t>(mem2a) + 5, reinterpret_cast<uintptr_t>(mem2b));
}
...
TEST(FilePersistentMemoryAllocatorTest, ExtendTest)
{
    ...
    // Map it as an extendable read/write file and append to it.
    {
        ...
        FilePersistentMemoryAllocator allocator(std::move(mmfile), region.size, 0, "", false);
...
TEST(FilePersistentMemoryAllocatorTest, AcceptableTest)
{
    ...
    for (size_t filesize = minsize; filesize > 0; --filesize) {
        ...
        if (FilePersistentMemoryAllocator::IsFileAcceptable(*mmfile, read_only)) {
            ...
            FilePersistentMemoryAllocator allocator(std::move(mmfile), 0, 0, "", read_only);
            // Also make sure that iteration doesn't crash.
            PersistentMemoryAllocator::Iterator iter(&allocator);
            uint32_t type_id;
            Reference ref;
            while ((ref = iter.GetNext(&type_id)) != 0) {
                const char* data = allocator.GetAsArray<char>(ref, 0, PersistentMemoryAllocator::kSizeAny);
