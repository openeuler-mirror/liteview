...
private:
    ...
    std::string GetAsciiHeader(StringPiece histogram_name, int32_t flags) const override;
...
public:
    SampleVectorIterator(const std::vector<HistogramBase::AtomicCount>* counts, const BucketRanges* bucket_ranges);
    SampleVectorIterator(const HistogramBase::AtomicCount* counts, size_t counts_size, const BucketRanges* bucket_ranges);
