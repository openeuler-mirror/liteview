...
public:
    ...
    union BASE_EXPORT AtomicSingleSample {
        ...
        AtomicSingleSample(subtle::Atomic32 rhs)
            : as_atomic(rhs)
        {
        }
    ...
    base::Value::Dict ToGraphDict(StringPiece histogram_name, int32_t flags) const;
...
protected:
    ...
    virtual std::string GetAsciiHeader(StringPiece histogram_name, int32_t flags) const;
...
public:
    ...
    // Requires: !Done();
    virtual void Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const = 0;
    static_assert(
...
public:
    ...
    SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count, size_t bucket_index);
