...
protected:
    ...
    void CreatePersistentHistogramAllocator()
    {
        ...
        GlobalHistogramAllocator::CreateWithPersistentMemory(allocator_memory_.get(), kAllocatorMemorySize, 0, 0, "PersistentHistogramAllocatorTest");
...
TEST_F(PersistentHistogramAllocatorTest, CreateAndIterate)
{
    ...
    PersistentHistogramAllocator recovery(std::make_unique<PersistentMemoryAllocator>(allocator_memory_.get(), kAllocatorMemorySize, 0, 0, "", false));
...
TEST_F(PersistentHistogramAllocatorTest, StatisticsRecorderMerge)
{
    ...
    std::unique_ptr<GlobalHistogramAllocator> old_allocator = GlobalHistogramAllocator::ReleaseForTesting();
    ...
    // become unmanaged.
    std::unique_ptr<GlobalHistogramAllocator> new_allocator = GlobalHistogramAllocator::ReleaseForTesting();
    local_sr.reset();
    ...
    PersistentHistogramAllocator recovery1(std::make_unique<PersistentMemoryAllocator>(
        const_cast<void*>(new_allocator->memory_allocator()->data()), new_allocator->memory_allocator()->size(), 0, 0, "", false));
    ...
    // Do another merge.
    PersistentHistogramAllocator recovery2(std::make_unique<PersistentMemoryAllocator>(
        const_cast<void*>(new_allocator->memory_allocator()->data()), new_allocator->memory_allocator()->size(), 0, 0, "", false));
...
TEST_F(PersistentHistogramAllocatorTest, CustomRangesManager)
{
    ...
    // Create a "recovery" allocator using the same memory as the local one.
    PersistentHistogramAllocator recovery(std::make_unique<PersistentMemoryAllocator>(
        const_cast<void*>(new_allocator->memory_allocator()->data()), new_allocator->memory_allocator()->size(), 0, 0, "", false));
