...
void PersistentSampleMapIterator::Get(Sample* min, int64_t* max, Count* count) const
{
    DCHECK(!Done());
    if (min)
        *min = iter_->first;
...
// static
PersistentMemoryAllocator::Reference PersistentSampleMap::GetNextPersistentRecord(PersistentMemoryAllocator::Iterator& iterator, uint64_t* sample_map_id)
{
...
Count* PersistentSampleMap::ImportSamples(Sample until_value, bool import_everything)
{
