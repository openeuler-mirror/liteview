...
public:
    ...
    void Next() override;
    void Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const override;
...
void SampleCountPickleIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const
{
    DCHECK(!Done());
    *min = min_;
    *max = max_;
...
HistogramSamples::SingleSample HistogramSamples::AtomicSingleSample::Extract(bool disable)
{
    AtomicSingleSample single_sample = subtle::NoBarrier_AtomicExchange(&as_atomic, disable ? kDisabledSingleSample : 0);
    if (single_sample.as_atomic == kDisabledSingleSample)
        single_sample.as_atomic = 0;
    return single_sample.as_parts;
}
...
base::Value::Dict HistogramSamples::ToGraphDict(StringPiece histogram_name, int32_t flags) const
{
...
std::string HistogramSamples::GetAsciiHeader(StringPiece histogram_name, int32_t flags) const
{
    std::string output;
    StringAppendF(&output, "Histogram: %.*s recorded %d samples", static_cast<int>(histogram_name.size()), histogram_name.data(), TotalCount());
    if (flags)
...
void HistogramSamples::WriteAsciiBucketGraph(double x_count, int line_length, std::string* output) const
{
...
SingleSampleIterator::SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count)
    : SingleSampleIterator(min, max, count, kSizeMax)
{
}

SingleSampleIterator::SingleSampleIterator(HistogramBase::Sample min, int64_t max, HistogramBase::Count count, size_t bucket_index)
    : min_(min)
    , max_(max)
...
void SingleSampleIterator::Get(HistogramBase::Sample* min, int64_t* max, HistogramBase::Count* count) const
{
    DCHECK(!Done());
    if (min != nullptr)
        *min = min_;
