...
public:
    ...
    template <typename Function> void AsOtherProcess(ProcessId pid, Function function)
    {
        std::unique_ptr<GlobalActivityTracker> old_global = GlobalActivityTracker::ReleaseForTesting();
        ...
        std::unique_ptr<PersistentMemoryAllocator> new_allocator(
            std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(old_allocator->data()), old_allocator->size(), 0, 0, "", false));
        GlobalActivityTracker::CreateWithAllocator(std::move(new_allocator), 3, pid);
...
public:
    SimpleActivityThread(const std::string& name, const void* source, Activity::Type activity, const ActivityData& data)
    ...
    void Run() override
    {
        ThreadActivityTracker::ActivityId id = GlobalActivityTracker::Get()->GetOrCreateTrackerForCurrentThread()->PushActivity(source_, activity_, data_);
...
TEST_F(ActivityAnalyzerTest, MAYBE_GlobalAnalyzerConstruction)
{
    ...
    PersistentMemoryAllocator* allocator = GlobalActivityTracker::Get()->allocator();
    GlobalActivityAnalyzer analyzer(std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(allocator->data()), allocator->size(), 0, 0, "", true));
    ...
    SimpleActivityThread t2("t2", nullptr, Activity::ACT_TASK, ActivityData::ForTask(11));
    t2.Start();
    t2.WaitReady();
    ...
    const ActivityUserData::Snapshot& data_snapshot = analyzer.GetProcessDataSnapshot(pid);
...
{
    base::MappedReadOnlyRegion shm = base::ReadOnlySharedMemoryRegion::Create(kMemorySize);
    ...
    GlobalActivityTracker::CreateWithSharedMemory(std::move(rw_mapping), 0, "", 3);
    GlobalActivityTracker::Get()->process_data().SetString("foo", "bar");

    std::unique_ptr<GlobalActivityAnalyzer> analyzer = GlobalActivityAnalyzer::CreateWithSharedMemory(std::move(ro_mapping));
...
TEST_F(ActivityAnalyzerTest, UserDataSnapshotTest)
{
    ...
    GlobalActivityAnalyzer global_analyzer(
        std::make_unique<PersistentMemoryAllocator>(const_cast<void*>(allocator->data()), allocator->size(), 0, 0, "", true));
...

    {
...

        {
            ...
            const ThreadActivityAnalyzer::Snapshot& analyzer_snapshot = analyzer.activity_snapshot();
            ASSERT_EQ(2U, analyzer_snapshot.user_data_stack.size());
            const ActivityUserData::Snapshot& user_data = analyzer_snapshot.user_data_stack.at(1);
            ...
            EXPECT_EQ(strlen(string2b), user_data.at("sref2").GetStringReference().size());
        }
        ...
        const ActivityUserData::Snapshot& user_data = analyzer_snapshot.user_data_stack.at(0);
        EXPECT_EQ(8U, user_data.size());
        ...
        EXPECT_EQ(strlen(string1b), user_data.at("sref1").GetStringReference().size());
    }
...
TEST_F(ActivityAnalyzerTest, GlobalUserDataTest)
{
    ...
    const ActivityUserData::Snapshot& snapshot = global_analyzer.GetProcessDataSnapshot(pid);
...
TEST_F(ActivityAnalyzerTest, GlobalMultiProcess)
{
    ...
    GlobalActivityTracker::CreateWithLocalMemory(kMemorySize, 0, "", 3, kProcessIdA);
    GlobalActivityTracker* global = GlobalActivityTracker::Get();
    PersistentMemoryAllocator* allocator = global->allocator();
    ...
    ActivityUserData::GetOwningProcessId(GlobalActivityTracker::Get()->process_data().GetBaseAddress(), &process_id, &create_stamp);
    ASSERT_EQ(kProcessIdA, process_id);

    GlobalActivityTracker::Get()->process_data().SetInt("pid", global->process_id());
    ...
    AsOtherProcess(kProcessIdB, [&global, kProcessIdB]() {
        ...
        GlobalActivityTracker::Get()->process_data().SetInt("pid", GlobalActivityTracker::Get()->process_id());
    });
    ...
    const ActivityUserData::Snapshot& pdata1 = analyzer.GetProcessDataSnapshot(pid1);
    const ActivityUserData::Snapshot& pdata2 = analyzer.GetProcessDataSnapshot(pid2);
