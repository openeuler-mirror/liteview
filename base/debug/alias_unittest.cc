...
{
    std::unique_ptr<std::string> input = std::make_unique<std::string>("string contents");
    ...
    static_assert(sizeof(copy1) == 100, "Verification that copy1 has expected size");
    ...
    static_assert(sizeof(copy2) == 3, "Verification that copy2 has expected size");
