...
BASE_EXPORT void SetCrashKeyString(CrashKeyString* crash_key, base::StringPiece value);
...
#define SCOPED_CRASH_KEY_BOOL(category, name, data)                                                                                                            \
    static_assert(std::is_same<std::decay_t<decltype(data)>, bool>::value, "SCOPED_CRASH_KEY_BOOL must be passed a boolean value.");                           \
