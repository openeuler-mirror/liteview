...
public:
    ...
    class ThreadKey {
    public:
        ThreadKey(ProcessId pid, int64_t tid)
            : pid_(pid)
            , tid_(tid)
        {
        }
    ...
    ThreadActivityAnalyzer(PersistentMemoryAllocator* allocator, PersistentMemoryAllocator::Reference reference);
    ...
    const Snapshot& activity_snapshot()
    {
...
public:
    ...
    static std::unique_ptr<GlobalActivityAnalyzer> CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator);

#if !BUILDFLAG(IS_NACL)
    // Creates a global analyzer using the contents of a file given in
    // |file_path|.
    static std::unique_ptr<GlobalActivityAnalyzer> CreateWithFile(const FilePath& file_path);
#endif // !BUILDFLAG(IS_NACL)

    // Like above but accesses an allocator in a mapped shared-memory segment.
    static std::unique_ptr<GlobalActivityAnalyzer> CreateWithSharedMemory(base::ReadOnlySharedMemoryMapping mapping);
    ...
    ActivityUserData::Snapshot GetUserDataSnapshot(ProcessId pid, uint32_t ref, uint32_t id);
