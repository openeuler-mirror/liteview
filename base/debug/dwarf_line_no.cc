...
void SerializeLineNumberInfoToString(int fd, const LineNumberInfo& info, char* out, size_t out_size)
{
    ...
    out[out_pos - 1] = ':';
    char* tmp = internal::itoa_r(static_cast<intptr_t>(info.line), out + out_pos, out_size - out_pos, 10, 0);
    out_pos += strlen(tmp) + 1;
    out[out_pos - 1] = ':';
    tmp = internal::itoa_r(static_cast<intptr_t>(info.column), out + out_pos, out_size - out_pos, 10, 0);
    out_pos += strlen(tmp) + 1;
}
...
bool GetDwarfSourceLineNumber(void* pc, uintptr_t cu_offset, char* out, size_t out_size)
{
...
void GetDwarfCompileUnitOffsets(void* const* trace, uint64_t* cu_offsets, size_t num_frames)
{
    // Ensure `cu_offsets` always has a known state.
    memset(cu_offsets, 0, sizeof(uint64_t) * num_frames);

    FrameInfo* frame_info = static_cast<FrameInfo*>(alloca(sizeof(FrameInfo) * num_frames));
