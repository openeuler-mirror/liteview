...
void SymbolMap::Populate()
{
    ...
    while (lmap != nullptr) {
        ...
        for (const Elf64_Phdr& phdr : GetElfProgramHeaders(next_entry.addr)) {
            ...
            Segment segment;
            segment.addr = reinterpret_cast<const char*>(next_entry.addr) + phdr.p_vaddr;
        ...
        absl::optional<StringPiece> elf_library_name = ReadElfLibraryName(next_entry.addr);
        if (elf_library_name) {
            strlcpy(next_entry.name, elf_library_name->data(), elf_library_name->size() + 1);
        } else {
            StringPiece link_map_name(lmap->l_name[0] ? lmap->l_name : "<executable>");
...
void StackTrace::OutputToStreamWithPrefix(std::ostream* os, const char* prefix_string) const
{
    ...
    for (const SymbolMap::Module& entry : map.GetModules()) {
        *os << "{{{module:" << module_id << ":" << entry.name << ":elf:" << entry.build_id << "}}}\n";
