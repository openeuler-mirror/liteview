...
PersistentMemoryAllocator::Reference AllocateFrom(PersistentMemoryAllocator* allocator, uint32_t from_type, size_t size, uint32_t to_type)
{
...
// static
bool OwningProcess::GetOwningProcessId(const void* memory, ProcessId* out_id, int64_t* out_stamp)
{
...
#if BUILDFLAG(IS_WIN)
    thread_ref.as_tid = ::GetThreadIdXp(handle.platform_handle());
...
ActivityTrackerMemoryAllocator::ActivityTrackerMemoryAllocator(
    PersistentMemoryAllocator* allocator, uint32_t object_type, uint32_t object_free_type, size_t object_size, size_t cache_size, bool make_iterable)
...
ActivityTrackerMemoryAllocator::Reference ActivityTrackerMemoryAllocator::GetObjectReference()
{
...
// static
void Activity::FillFrom(Activity* activity, const void* program_counter, const void* origin, Type type, const ActivityData& data)
{
    ...
    , orig_create_stamp(0)
{
    ...
    *const_cast<ProcessId*>(&orig_process_id) = static_cast<ProcessId>(header_->owner.process_id);
...
bool ActivityUserData::CreateSnapshot(Snapshot* output_snapshot) const
{
    ...
    for (const auto& entry : values_) {
        ...
        case RAW_VALUE:
        case STRING_VALUE:
            value.long_value_ = std::string(reinterpret_cast<char*>(entry.second.memory.get()), size);
            break;
        case RAW_VALUE_REFERENCE:
        case STRING_VALUE_REFERENCE: {
            ReferenceRecord* ref = reinterpret_cast<ReferenceRecord*>(entry.second.memory.get());
            value.ref_value_ = StringPiece(reinterpret_cast<char*>(static_cast<uintptr_t>(ref->address)), static_cast<size_t>(ref->size));
        } break;
        case BOOL_VALUE:
        case CHAR_VALUE:
            value.short_value_ = static_cast<uint64_t>(reinterpret_cast<std::atomic<char>*>(entry.second.memory.get())->load(std::memory_order_relaxed));
            break;
        case SIGNED_VALUE:
        case UNSIGNED_VALUE:
            value.short_value_ = reinterpret_cast<std::atomic<uint64_t>*>(entry.second.memory.get())->load(std::memory_order_relaxed);
            break;
...
void ActivityUserData::SetOwningProcessIdForTesting(ProcessId pid, int64_t stamp)
{
    if (!header_)
        return;
...
// static
bool ActivityUserData::GetOwningProcessId(const void* memory, ProcessId* out_id, int64_t* out_stamp)
{
...
void* ActivityUserData::Set(StringPiece name, ValueType type, const void* memory, size_t size)
{
    ...
    if (existing != values_.end()) {
        ...
        size_t name_extent = bits::AlignUp(sizeof(FieldHeader) + name_size, kMemoryAlignment) - sizeof(FieldHeader);
        ...
        // into a uint16_t.
        size_t full_size
            = std::min({ base_size + value_extent, available_, bits::AlignDown(size_t { std::numeric_limits<uint16_t>::max() }, kMemoryAlignment) });
...
void ActivityUserData::SetReference(StringPiece name, ValueType type, const void* memory, size_t size)
{
    ReferenceRecord rec;
...
void ActivityUserData::ImportExistingData() const
{
    ...
    while (available_ > sizeof(FieldHeader)) {
        ...
        size_t value_offset = bits::AlignUp(sizeof(FieldHeader) + header->name_size, kMemoryAlignment);
        if (header->record_size == value_offset && header->value_size.load(std::memory_order_relaxed) == 1) {
            value_offset -= 1;
        }
    ...
    if (header_->owner.data_id.load(std::memory_order_acquire) != orig_data_id || static_cast<ProcessId>(header_->owner.process_id) != orig_process_id
...
struct ThreadActivityTracker::Header {
    ...
    static constexpr size_t kExpectedInstanceSize = OwningProcess::kExpectedInstanceSize + Activity::kExpectedInstanceSize + 72;
...
ThreadActivityTracker::ScopedActivity::ScopedActivity(
    ThreadActivityTracker* tracker, const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data)
    : tracker_(tracker)
{
    if (tracker_)
...
void ThreadActivityTracker::ScopedActivity::ChangeTypeAndData(Activity::Type type, const ActivityData& data)
{
    if (tracker_)
    ...
    stack_slots_(static_cast<uint32_t>((size - sizeof(Header)) / sizeof(Activity)))
{
    ...
    static_assert(sizeof(header_->thread_ref) == sizeof(header_->thread_ref.as_id), "PlatformThreadHandle::Handle is too big to hold in 64-bit ID");
    ...
    // architectures.
    static_assert(offsetof(Activity, data) % sizeof(uint64_t) == 0, "ActivityData.data is not 64-bit aligned");
...
ThreadActivityTracker::ActivityId ThreadActivityTracker::PushActivity(
    const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data)
{
...
void ThreadActivityTracker::ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data)
{
    ...
    if (id < stack_slots_) {
        ...
        if (type != Activity::ACT_NULL) {
            DCHECK_EQ(activity->activity_type & Activity::ACT_CATEGORY_MASK, type & Activity::ACT_CATEGORY_MASK);
            activity->activity_type = type;
        }
...
std::unique_ptr<ActivityUserData> ThreadActivityTracker::GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator)
{
...
void ThreadActivityTracker::ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator)
{
    // User-data is only stored for activities actually held in the stack.
...
void ThreadActivityTracker::RecordExceptionActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data)
{
    // A thread-checker creates a lock to check the thread-id which means
    // re-entry into this code if lock acquisitions are being tracked.
    DCHECK(CalledOnValidThread());
...
{
    if (header_->owner.data_id.load(std::memory_order_acquire) == 0 || header_->owner.process_id == 0 || header_->thread_ref.as_id == 0
        || header_->start_time == 0 || header_->start_ticks == 0 || header_->stack_slots != stack_slots_
...
bool ThreadActivityTracker::CreateSnapshot(Snapshot* output_snapshot) const
{
    ...
    for (int attempt = 0; attempt < kMaxAttempts; ++attempt) {
        ...
        const auto starting_process_id = static_cast<ProcessId>(header_->owner.process_id);
        ...
        output_snapshot->thread_name = std::string(header_->thread_name, sizeof(header_->thread_name) - 1);
        output_snapshot->create_stamp = header_->owner.create_stamp;
        output_snapshot->thread_id = header_->thread_ref.as_id;
        output_snapshot->process_id = static_cast<ProcessId>(header_->owner.process_id);
        ...
        if (header_->owner.data_id.load(std::memory_order_seq_cst) != starting_id || output_snapshot->create_stamp != starting_create_stamp
            || output_snapshot->process_id != starting_process_id || output_snapshot->thread_id != starting_thread_id) {
            continue;
        }
        ...
        for (Activity& activity : output_snapshot->activity_stack) {
            activity.time_internal = WallTimeFromTickTime(start_ticks, activity.time_internal, start_time).ToInternalValue();
        }
        output_snapshot->last_exception.time_internal
            = WallTimeFromTickTime(start_ticks, output_snapshot->last_exception.time_internal, start_time).ToInternalValue();
...
void ThreadActivityTracker::SetOwningProcessIdForTesting(ProcessId pid, int64_t stamp)
{
    header_->owner.SetOwningProcessIdForTesting(pid, stamp);
}

// static
bool ThreadActivityTracker::GetOwningProcessId(const void* memory, ProcessId* out_id, int64_t* out_stamp)
{
...
std::unique_ptr<ActivityUserData> ThreadActivityTracker::CreateUserDataForActivity(Activity* activity, ActivityTrackerMemoryAllocator* allocator)
{
    ...
    if (memory) {
        std::unique_ptr<ActivityUserData> user_data = std::make_unique<ActivityUserData>(memory, kUserDataSize);
...
GlobalActivityTracker::ModuleInfo& GlobalActivityTracker::ModuleInfo::operator=(ModuleInfo&& rhs) = default;
GlobalActivityTracker::ModuleInfo& GlobalActivityTracker::ModuleInfo::operator=(const ModuleInfo& rhs) = default;

GlobalActivityTracker::ModuleInfoRecord::ModuleInfoRecord() = default;
GlobalActivityTracker::ModuleInfoRecord::~ModuleInfoRecord() = default;

bool GlobalActivityTracker::ModuleInfoRecord::DecodeTo(GlobalActivityTracker::ModuleInfo* info, size_t record_size) const
{
    ...
    if ((current_changes & kModuleInformationChanging) != 0 || changes.load(std::memory_order_seq_cst) != current_changes) {
...
GlobalActivityTracker::ModuleInfoRecord* GlobalActivityTracker::ModuleInfoRecord::CreateFrom(
    const GlobalActivityTracker::ModuleInfo& info, PersistentMemoryAllocator* allocator)
{
    Pickle pickler;
...
bool GlobalActivityTracker::ModuleInfoRecord::UpdateFrom(const GlobalActivityTracker::ModuleInfo& info)
{
    ...
    if ((old_changes & kModuleInformationChanging) != 0
        || !changes.compare_exchange_strong(old_changes, new_changes, std::memory_order_acquire, std::memory_order_acquire)) {
    ...
    bool success = changes.compare_exchange_strong(new_changes, old_changes + 1, std::memory_order_release, std::memory_order_relaxed);
    DCHECK(success);
    return true;
}

GlobalActivityTracker::ScopedThreadActivity::ScopedThreadActivity(
    const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data, bool lock_allowed)
    : ThreadActivityTracker::ScopedActivity(GetOrCreateTracker(lock_allowed), program_counter, origin, type, data)
{
}
...
GlobalActivityTracker::ThreadSafeUserData::ThreadSafeUserData(void* memory, size_t size, ProcessId pid)
...
void* GlobalActivityTracker::ThreadSafeUserData::Set(StringPiece name, ValueType type, const void* memory, size_t size)
{
    AutoLock lock(data_lock_);
    return ActivityUserData::Set(name, type, memory, size);
}

GlobalActivityTracker::ManagedActivityTracker::ManagedActivityTracker(PersistentMemoryAllocator::Reference mem_reference, void* base, size_t size)
...
void GlobalActivityTracker::CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, ProcessId process_id)
{
    // There's no need to do anything with the result. It is self-managing.
    GlobalActivityTracker* global_tracker = new GlobalActivityTracker(std::move(allocator), stack_depth, process_id);
...
// static
bool GlobalActivityTracker::CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, StringPiece name, int stack_depth)
{
    ...
    bool success = mapped_file->Initialize(File(file_path, File::FLAG_CREATE_ALWAYS | File::FLAG_READ | File::FLAG_WRITE | File::FLAG_WIN_SHARE_DELETE),
        { 0, size }, MemoryMappedFile::READ_WRITE_EXTEND);
    if (!success)
        return false;
    if (!FilePersistentMemoryAllocator::IsFileAcceptable(*mapped_file, false))
        return false;
    CreateWithAllocator(std::make_unique<FilePersistentMemoryAllocator>(std::move(mapped_file), size, id, name, false), stack_depth, 0);
    return true;
}
#endif // !BUILDFLAG(IS_NACL)

// static
bool GlobalActivityTracker::CreateWithLocalMemory(size_t size, uint64_t id, StringPiece name, int stack_depth, ProcessId process_id)
{
    CreateWithAllocator(std::make_unique<LocalPersistentMemoryAllocator>(size, id, name), stack_depth, process_id);
    return true;
}

// static
bool GlobalActivityTracker::CreateWithSharedMemory(base::WritableSharedMemoryMapping mapping, uint64_t id, StringPiece name, int stack_depth)
{
    if (!mapping.IsValid() || !WritableSharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(mapping)) {
        return false;
    }
    CreateWithAllocator(std::make_unique<WritableSharedPersistentMemoryAllocator>(std::move(mapping), id, name), stack_depth, 0);
    return true;
}

// static
void GlobalActivityTracker::SetForTesting(std::unique_ptr<GlobalActivityTracker> tracker)
{
...
// static
std::unique_ptr<GlobalActivityTracker> GlobalActivityTracker::ReleaseForTesting()
{
...
ThreadActivityTracker* GlobalActivityTracker::CreateTrackerForCurrentThread()
{
    ...
    auto tracker = std::make_unique<ManagedActivityTracker>(mem_reference, mem_base, stack_memory_size_);
...
void GlobalActivityTracker::SetBackgroundTaskRunner(const scoped_refptr<SequencedTaskRunner>& runner)
{
    AutoLock lock(global_tracker_lock_);
    background_task_runner_ = std::move(runner);
}

void GlobalActivityTracker::SetProcessExitCallback(ProcessExitCallback callback)
{
    AutoLock lock(global_tracker_lock_);
    process_exit_callback_ = callback;
}

void GlobalActivityTracker::RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& cmd)
{
...
void GlobalActivityTracker::RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& exe, const FilePath::StringType& args)
{
    if (exe.find(FILE_PATH_LITERAL(" "))) {
        RecordProcessLaunch(process_id, FilePath::StringType(FILE_PATH_LITERAL("\"")) + exe + FILE_PATH_LITERAL("\" ") + args);
    } else {
        RecordProcessLaunch(process_id, exe + FILE_PATH_LITERAL(' ') + args);
    }
}

void GlobalActivityTracker::RecordProcessExit(ProcessId process_id, int exit_code)
{
    DCHECK_NE(GetProcessId(), process_id);
    DCHECK_NE(ProcessId { 0 }, process_id);
    ...
    if (task_runner && !task_runner->RunsTasksInCurrentSequence()) {
        task_runner->PostTask(
            FROM_HERE, BindOnce(&GlobalActivityTracker::CleanupAfterProcess, Unretained(this), process_id, now_stamp, exit_code, std::move(command_line)));
        return;
    }

    CleanupAfterProcess(process_id, now_stamp, exit_code, std::move(command_line));
}
...
void GlobalActivityTracker::CleanupAfterProcess(ProcessId process_id, int64_t exit_stamp, int exit_code, std::string&& command_line)
{
    ...
    if (process_exit_callback) {
        ...
        while ((ref = iter.GetNextOfType(kTypeIdProcessDataRecord)) != 0) {
            const void* memory = allocator_->GetAsArray<char>(ref, kTypeIdProcessDataRecord, PersistentMemoryAllocator::kSizeAny);
            if (!memory)
                continue;
            ProcessId found_id;
            ...
            if (ActivityUserData::GetOwningProcessId(memory, &found_id, &create_stamp)) {
                if (found_id == process_id && create_stamp < exit_stamp) {
                    const ActivityUserData process_data(const_cast<void*>(memory), allocator_->GetAllocSize(ref));
        ...
        process_exit_callback.Run(process_id, exit_stamp, exit_code, exit_phase, std::move(command_line), std::move(process_data_snapshot));
    }
    ...
    while ((ref = iter.GetNext(&type)) != 0) {
        ...
        case ModuleInfoRecord::kPersistentTypeId: {
            const void* memory = allocator_->GetAsArray<char>(ref, type, PersistentMemoryAllocator::kSizeAny);
            if (!memory)
                continue;
            ProcessId found_id;
            int64_t create_stamp;
...
void GlobalActivityTracker::RecordLogMessage(StringPiece message)
{
    ...
    PersistentMemoryAllocator::Reference ref = allocator_->Allocate(message.size() + 1, kTypeIdGlobalLogMessage);
...
void GlobalActivityTracker::RecordException(const void* pc, const void* origin, uint32_t code)
{
...
GlobalActivityTracker::GlobalActivityTracker(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, ProcessId process_id)
    ...
    , thread_tracker_allocator_(allocator_.get(), kTypeIdActivityTracker, kTypeIdActivityTrackerFree, stack_memory_size_, kCachedThreadMemories,
          /*make_iterable=*/true)
    , user_data_allocator_(allocator_.get(), kTypeIdUserDataRecord, kTypeIdUserDataRecordFree, kUserDataSize, kCachedUserDataMemories,
          /*make_iterable=*/true)
    , process_data_(allocator_->GetAsArray<char>(AllocateFrom(allocator_.get(), kTypeIdProcessDataRecordFree, kProcessDataSize, kTypeIdProcessDataRecord),
                        kTypeIdProcessDataRecord, kProcessDataSize),
          kProcessDataSize, process_id_)
{
    ...
    allocator_->MakeIterable(allocator_->GetAsReference(process_data_.GetBaseAddress(), kTypeIdProcessDataRecord));
...
void GlobalActivityTracker::ReturnTrackerMemory(ManagedActivityTracker* tracker)
{
...
void GlobalActivityTracker::RecordExceptionImpl(const void* pc, const void* origin, uint32_t code)
{
    ...
    tracker->RecordExceptionActivity(pc, origin, Activity::ACT_EXCEPTION, ActivityData::ForException(code));
}

ScopedActivity::ScopedActivity(const void* program_counter, uint8_t action, uint32_t id, int32_t info)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, nullptr, static_cast<Activity::Type>(Activity::ACT_GENERIC | action),
...
{
    DCHECK_EQ(0, action & Activity::ACT_CATEGORY_MASK);
    ChangeTypeAndData(static_cast<Activity::Type>(Activity::ACT_GENERIC | action), kNullActivityData);
}
...
{
    DCHECK_EQ(0, action & Activity::ACT_CATEGORY_MASK);
    ChangeTypeAndData(static_cast<Activity::Type>(Activity::ACT_GENERIC | action), ActivityData::ForGeneric(id_, info));
}

ScopedTaskRunActivity::ScopedTaskRunActivity(const void* program_counter, const base::PendingTask& task)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, task.posted_from.program_counter(), Activity::ACT_TASK_RUN,
        ActivityData::ForTask(static_cast<uint64_t>(task.sequence_num)),
        /*lock_allowed=*/true)
{
}

ScopedLockAcquireActivity::ScopedLockAcquireActivity(const void* program_counter, const base::internal::LockImpl* lock)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, nullptr, Activity::ACT_LOCK_ACQUIRE, ActivityData::ForLock(lock),
        /*lock_allowed=*/false)
{
}

ScopedEventWaitActivity::ScopedEventWaitActivity(const void* program_counter, const base::WaitableEvent* event)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, nullptr, Activity::ACT_EVENT_WAIT, ActivityData::ForEvent(event),
        /*lock_allowed=*/true)
{
}

ScopedThreadJoinActivity::ScopedThreadJoinActivity(const void* program_counter, const base::PlatformThreadHandle* thread)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, nullptr, Activity::ACT_THREAD_JOIN, ActivityData::ForThread(*thread),
        /*lock_allowed=*/true)
{
}

#if !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_IOS)
ScopedProcessWaitActivity::ScopedProcessWaitActivity(const void* program_counter, const base::Process* process)
    : GlobalActivityTracker::ScopedThreadActivity(program_counter, nullptr, Activity::ACT_PROCESS_WAIT, ActivityData::ForProcess(process->Pid()),
        /*lock_allowed=*/true)
{
}
#endif
