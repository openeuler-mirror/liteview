...
struct OwningProcess {
    ...
    static bool GetOwningProcessId(const void* memory, ProcessId* out_id, int64_t* out_stamp);
...
public:
    ...
    ActivityTrackerMemoryAllocator(const ActivityTrackerMemoryAllocator&) = delete;
    ActivityTrackerMemoryAllocator& operator=(const ActivityTrackerMemoryAllocator&) = delete;
    ...
    template <typename T> T* GetAsObject(Reference ref)
    {
        return allocator_->GetAsObject<T>(ref);
    }

    // Similar to GetAsObject() but converts references to arrays of objects.
    template <typename T> T* GetAsArray(Reference ref, size_t count)
    {
...
struct Activity {
    ...
    static void FillFrom(Activity* activity, const void* program_counter, const void* origin, Type type, const ActivityData& data);
};
...
protected:
    virtual void* Set(StringPiece name, ValueType type, const void* memory, size_t size);
...
private:
    ...
    struct ValueInfo {
        ...
        raw_ptr<std::atomic<uint16_t>> size_ptr; // Address of the actual size of value.
        size_t extent; // The total storage of the value,
    }; // typically rounded up for alignment.

    void SetReference(StringPiece name, ValueType type, const void* memory, size_t size);
...
public:
    ...
    public:
        ScopedActivity(ThreadActivityTracker* tracker, const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data);
    ...
    ActivityId PushActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data);

    // An inlined version of the above that gets the program counter where it
    // is called.
    ALWAYS_INLINE
    ActivityId PushActivity(const void* origin, Activity::Type type, const ActivityData& data)
    {
    ...
    void ChangeActivity(ActivityId id, Activity::Type type, const ActivityData& data);
    ...
    std::unique_ptr<ActivityUserData> GetUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator);
    ...
    void ReleaseUserData(ActivityId id, ActivityTrackerMemoryAllocator* allocator);

    // Save an exception. |origin| is the location of the exception.
    void RecordExceptionActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data);
...
private:
    ...
    std::unique_ptr<ActivityUserData> CreateUserDataForActivity(Activity* activity, ActivityTrackerMemoryAllocator* allocator);
...
public:
    ...
    using ProcessExitCallback = RepeatingCallback<void(ProcessId process_id, int64_t exit_stamp, int exit_code, ProcessPhase exit_phase,
        std::string&& command_line, ActivityUserData::Snapshot&& process_data)>;
    ...
    class BASE_EXPORT ScopedThreadActivity : public ThreadActivityTracker::ScopedActivity {
    public:
        ScopedThreadActivity(const void* program_counter, const void* origin, Activity::Type type, const ActivityData& data, bool lock_allowed);
    ...
    // purposes.
    static void CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, ProcessId process_id);
    ...
    static bool CreateWithFile(const FilePath& file_path, size_t size, uint64_t id, StringPiece name, int stack_depth);
    ...
    static bool CreateWithLocalMemory(size_t size, uint64_t id, StringPiece name, int stack_depth, ProcessId process_id);
    ...
    static bool CreateWithSharedMemory(base::WritableSharedMemoryMapping mapping, uint64_t id, StringPiece name, int stack_depth);
    ...
    void SetBackgroundTaskRunner(const scoped_refptr<SequencedTaskRunner>& runner);
    ...
    void RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& cmd);
    void RecordProcessLaunch(ProcessId process_id, const FilePath::StringType& exe, const FilePath::StringType& args);
    void RecordProcessExit(ProcessId process_id, int exit_code);
    static void RecordProcessLaunchIfEnabled(ProcessId process_id, const FilePath::StringType& cmd)
    {
        GlobalActivityTracker* tracker = Get();
        if (tracker)
            tracker->RecordProcessLaunch(process_id, cmd);
    }
    static void RecordProcessLaunchIfEnabled(ProcessId process_id, const FilePath::StringType& exe, const FilePath::StringType& args)
    {
        GlobalActivityTracker* tracker = Get();
        if (tracker)
...
private:
    ...
    private:
        void* Set(StringPiece name, ValueType type, const void* memory, size_t size) override;
    ...
    struct BASE_EXPORT ModuleInfoRecord {
        ...
        bool DecodeTo(GlobalActivityTracker::ModuleInfo* info, size_t record_size) const;
        static ModuleInfoRecord* CreateFrom(const GlobalActivityTracker::ModuleInfo& info, PersistentMemoryAllocator* allocator);
    ...
    public:
        ManagedActivityTracker(PersistentMemoryAllocator::Reference mem_reference, void* base, size_t size);
    ...
    // The |process_id| can be zero to get it from the OS but is taken for testing
    // purposes.
    GlobalActivityTracker(std::unique_ptr<PersistentMemoryAllocator> allocator, int stack_depth, ProcessId process_id);
    ...
    void CleanupAfterProcess(ProcessId process_id, int64_t exit_stamp, int exit_code, std::string&& command_line);
    ...
    ActivityTrackerMemoryAllocator thread_tracker_allocator_ GUARDED_BY(thread_tracker_allocator_lock_);
    ...
    std::map<ProcessId, std::string> known_processes_ GUARDED_BY(global_tracker_lock_);
...
class BASE_EXPORT ScopedActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
...
class BASE_EXPORT ScopedTaskRunActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
    ALWAYS_INLINE
...
class BASE_EXPORT ScopedLockAcquireActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
    ALWAYS_INLINE
    ...
    ScopedLockAcquireActivity& operator=(const ScopedLockAcquireActivity&) = delete;

private:
    ScopedLockAcquireActivity(const void* program_counter, const base::internal::LockImpl* lock);
};

class BASE_EXPORT ScopedEventWaitActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
    ALWAYS_INLINE
...
class BASE_EXPORT ScopedThreadJoinActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
    ALWAYS_INLINE
...
class BASE_EXPORT ScopedProcessWaitActivity : public GlobalActivityTracker::ScopedThreadActivity {
public:
    ALWAYS_INLINE
    ...
    ScopedProcessWaitActivity& operator=(const ScopedProcessWaitActivity&) = delete;
