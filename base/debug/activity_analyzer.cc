...
{
    UmaHistogramEnumeration("ActivityTracker.Collect.AnalyzerCreationError", error, kAnalyzerCreationErrorMax);
}
...
ThreadActivityAnalyzer::ThreadActivityAnalyzer(PersistentMemoryAllocator* allocator, PersistentMemoryAllocator::Reference reference)
    : ThreadActivityAnalyzer(allocator->GetAsArray<char>(reference, GlobalActivityTracker::kTypeIdActivityTracker, PersistentMemoryAllocator::kSizeAny),
...
void ThreadActivityAnalyzer::AddGlobalInformation(GlobalActivityAnalyzer* global)
{
    if (!IsValid())
        return;
    ...
    for (auto& activity : activity_snapshot_.activity_stack) {
        ...
        // or id is not valid.
        activity_snapshot_.user_data_stack.push_back(global->GetUserDataSnapshot(activity_snapshot_.process_id, activity.user_data_ref, activity.user_data_id));
    }
}

GlobalActivityAnalyzer::GlobalActivityAnalyzer(std::unique_ptr<PersistentMemoryAllocator> allocator)
...
// static
std::unique_ptr<GlobalActivityAnalyzer> GlobalActivityAnalyzer::CreateWithAllocator(std::unique_ptr<PersistentMemoryAllocator> allocator)
{
...
// static
std::unique_ptr<GlobalActivityAnalyzer> GlobalActivityAnalyzer::CreateWithFile(const FilePath& file_path)
{
    ...
    return CreateWithAllocator(std::make_unique<FilePersistentMemoryAllocator>(std::move(mmfile), 0, 0, StringPiece(), /*readonly=*/true));
}
#endif // !BUILDFLAG(IS_NACL)

// static
std::unique_ptr<GlobalActivityAnalyzer> GlobalActivityAnalyzer::CreateWithSharedMemory(base::ReadOnlySharedMemoryMapping mapping)
{
    if (!mapping.IsValid() || !ReadOnlySharedPersistentMemoryAllocator::IsSharedMemoryAcceptable(mapping)) {
        return nullptr;
    }
    return CreateWithAllocator(std::make_unique<ReadOnlySharedPersistentMemoryAllocator>(std::move(mapping), 0, StringPiece()));
}
...
ThreadActivityAnalyzer* GlobalActivityAnalyzer::GetFirstAnalyzer(ProcessId pid)
{
    ...
    if (analyzers_iterator_->second->GetProcessId(&create_stamp) == pid && create_stamp <= analysis_stamp_) {
...
ThreadActivityAnalyzer* GlobalActivityAnalyzer::GetNextAnalyzer()
{
    ...
    do {
        ...
        if (analyzers_iterator_ == analyzers_.end())
            return nullptr;
    } while (analyzers_iterator_->second->GetProcessId(&create_stamp) != analyzers_iterator_pid_ || create_stamp > analysis_stamp_);
    return analyzers_iterator_->second.get();
}

ThreadActivityAnalyzer* GlobalActivityAnalyzer::GetAnalyzerForThread(const ThreadKey& key)
{
...
ActivityUserData::Snapshot GlobalActivityAnalyzer::GetUserDataSnapshot(ProcessId pid, uint32_t ref, uint32_t id)
{
    ActivityUserData::Snapshot snapshot;

    void* memory = allocator_->GetAsArray<char>(ref, GlobalActivityTracker::kTypeIdUserDataRecord, PersistentMemoryAllocator::kSizeAny);
    if (memory) {
        ...
        int64_t create_stamp;
        if (!ActivityUserData::GetOwningProcessId(memory, &process_id, &create_stamp) || process_id != pid || user_data.id() != id) {
...
const ActivityUserData::Snapshot& GlobalActivityAnalyzer::GetProcessDataSnapshot(ProcessId pid)
{
...
std::vector<std::string> GlobalActivityAnalyzer::GetLogMessages()
{
    ...
    while ((ref = iter.GetNextOfType(GlobalActivityTracker::kTypeIdGlobalLogMessage)) != 0) {
        const char* message = allocator_->GetAsArray<char>(ref, GlobalActivityTracker::kTypeIdGlobalLogMessage, PersistentMemoryAllocator::kSizeAny);
        if (message)
...
std::vector<GlobalActivityTracker::ModuleInfo> GlobalActivityAnalyzer::GetModules(ProcessId pid)
{
    ...
    while ((record = iter.GetNextOfObject<GlobalActivityTracker::ModuleInfoRecord>()) != nullptr) {
        ProcessId process_id;
        int64_t create_stamp;
        if (!OwningProcess::GetOwningProcessId(&record->owner, &process_id, &create_stamp) || pid != process_id || create_stamp > analysis_stamp_) {
            continue;
        }
...
GlobalActivityAnalyzer::ProgramLocation GlobalActivityAnalyzer::GetProgramLocationFromAddress(uint64_t address)
{
...
void GlobalActivityAnalyzer::PrepareAllAnalyzers()
{
    ...
    for (PersistentMemoryAllocator::Reference memory_ref : memory_references_) {
        ...
        // is checked below.
        void* const base = allocator_->GetAsArray<char>(memory_ref, PersistentMemoryAllocator::kTypeIdAny, PersistentMemoryAllocator::kSizeAny);
        ...
        case GlobalActivityTracker::kTypeIdActivityTracker: {
            ...
            std::unique_ptr<ThreadActivityAnalyzer> analyzer(new ThreadActivityAnalyzer(base, size));
            if (!analyzer->IsValid())
                continue;
