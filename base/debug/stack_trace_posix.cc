...
void ProcessBacktrace(void* const* trace, size_t size, const char* prefix_string, BacktraceOutputHandler* handler)
{
#if defined(OS_OHOS)
    printf("ProcessBacktrace not impl\n");
    *(int*)1 = 1;
#else
...
#if defined(USE_SYMBOLIZE)
#if BUILDFLAG(ENABLE_STACK_TRACE_LINE_NUMBERS)
    uint64_t* cu_offsets = static_cast<uint64_t*>(alloca(sizeof(uint64_t) * size));
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        if (google::Symbolize(address, buf, sizeof(buf))) {
            ...
            if (GetDwarfSourceLineNumber(address, cu_offsets[i], &buf[0], sizeof(buf))) {
                handler->HandleOutput(" [");
                handler->HandleOutput(buf);
                handler->HandleOutput("]");
            }
...
private:
    ...
    // IMPORTANT: This function must be async-signal-safe because it can be
    // called from a signal handler (symbolizing stack frames for a crash).
    static int OpenObjectFileContainingPc(uint64_t pc, uint64_t& start_address, uint64_t& base_address, char* file_path, size_t file_path_size)
    {
...
size_t CollectStackTrace(void** trace, size_t count)
{
    ...
    return base::debug::TraceStackFramePointers(const_cast<const void**>(trace), count, 0);
#elif !defined(__UCLIBC__) && !defined(_AIX)
    // Though the backtrace API man page does not list any possible negative
    // return values, we take no chance.
#if defined(OS_OHOS)
    printf("CollectStackTrace not impl\n");
    *(int*)1 = 1;
    return 0;
#else
    return base::saturated_cast<size_t>(backtrace(trace, base::saturated_cast<int>(count)));
#endif
#else
    return 0;
#endif
