...
template <typename ProcessType> absl::optional<base::debug::GdiHandleCounts> CollectGdiHandleCountsImpl(DWORD pid)
{
...
void NOINLINE CrashIfPagefileUsageTooLarge(const PROCESS_MEMORY_COUNTERS_EX& pmc)
{
    CHECK_LE(pmc.PagefileUsage, kLotsOfMemory);
}

void NOINLINE CrashIfPrivateUsageTooLarge(const PROCESS_MEMORY_COUNTERS_EX& pmc)
{
    CHECK_LE(pmc.PrivateUsage, kLotsOfMemory);
}

void NOINLINE CrashIfCannotAllocateSmallBitmap(BITMAPINFOHEADER* header, HANDLE shared_section)
{
...
{
    pmc->cb = sizeof(*pmc);
    CHECK(GetProcessMemoryInfoXp(GetCurrentProcess(), reinterpret_cast<PROCESS_MEMORY_COUNTERS*>(pmc), sizeof(*pmc)));
}
...
void CollectGDIUsageAndDie(BITMAPINFOHEADER* header, HANDLE shared_section)
{
    ...
    absl::optional<GdiHandleCounts> optional_handle_counts = CollectGdiHandleCounts(GetCurrentProcessId());
...
{
    absl::optional<GdiHandleCounts> handle_counts = CollectGdiHandleCounts(GetCurrentProcessId());
