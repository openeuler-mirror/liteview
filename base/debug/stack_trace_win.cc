...
bool InitializeSymbols()
{
    ...
    // not number of bytes!
    if (!SymGetSearchPathWXp(GetCurrentProcess(), symbols_path, kSymbolsArraySize)) {
        g_init_error = GetLastError();
        DLOG(WARNING) << "SymGetSearchPath failed: " << g_init_error;
        return false;
    }

    std::wstring new_path = StringPrintf(L"%ls;%ls", symbols_path, GetExePath().DirName().value().c_str());
...
public:
    ...
    void OutputTraceToStream(const void* const* trace, size_t count, std::ostream* os, const char* prefix_string)
    {
        AutoLock lock(lock_);
...
void StackTrace::OutputToStreamWithPrefix(std::ostream* os, const char* prefix_string) const
{
    DebugBreak();
