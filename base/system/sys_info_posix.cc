...
bool IsStatsZeroIfUnlimited(const base::FilePath& path)
{
    ...
    switch (stats.f_type) {
    case /*TMPFS_MAGIC*/0x01021994:
    case /*static_cast<int>*/(/*HUGETLBFS_MAGIC*/0x958458f6):
    case /*static_cast<int>*/(/*RAMFS_MAGIC*/0x858458f6):
        return true;
    }
    return false;
}
...
int NumberOfProcessors()
{
    ...
    // physical cores.
    if (base::FeatureList::IsEnabled(base::kNumberOfCoresWithCpuSecurityMitigation) && is_cpu_security_mitigation_enabled) {
...
absl::optional<int> NumberOfPhysicalProcessors()
{
    ...
    if (sysctlbyname("hw.physicalcpu_max", &sysctl_value, &length, nullptr, 0) == 0) {
...
#if BUILDFLAG(IS_MAC)
BASE_FEATURE(kNumberOfCoresWithCpuSecurityMitigation, "NumberOfCoresWithCpuSecurityMitigation", base::FEATURE_DISABLED_BY_DEFAULT);
