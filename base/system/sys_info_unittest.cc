...
TEST_F(SysInfoTest, NumProcsWithSecurityMitigationEnabled)
{
    ...
    EXPECT_EQ(internal::NumberOfProcessors(), internal::NumberOfPhysicalProcessors());
...
TEST_F(SysInfoTest, HardwareModelNameFormatMacAndiOS)
{
    ...
    std::vector<StringPiece> mainPieces = SplitStringPiece(hardware_model, "()", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    ASSERT_EQ(3u, mainPieces.size()) << hardware_model;
    std::vector<StringPiece> modelPieces = SplitStringPiece(mainPieces[1], ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
    ...
    // a number.
    std::vector<StringPiece> pieces = SplitStringPiece(hardware_model, ",", KEEP_WHITESPACE, SPLIT_WANT_ALL);
...
TEST_F(SysInfoTest, GetHardwareInfo)
{
    ...
    auto callback = base::BindOnce(
        [](absl::optional<SysInfo::HardwareInfo>* target_info, SysInfo::HardwareInfo info) { *target_info = std::move(info); }, &hardware_info);
...
TEST_F(SysInfoTest, GoogleChromeOSLsbReleaseTime)
{
    ...
    EXPECT_DOUBLE_EQ(lsb_release_time.ToDoubleT(), parsed_lsb_release_time.ToDoubleT());
}
