...
uint64_t SysInfo::AmountOfPhysicalMemory()
{
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableLowEndDeviceMode)) {
        ...
        constexpr uint64_t kSimulatedMemoryForEnableLowEndDeviceMode = 512 * 1024 * 1024;
...
bool DetectLowEndDevice()
{
    ...
    return ram_size_mb > 0 && static_cast<uint64_t>(ram_size_mb) <= kLowMemoryDeviceThresholdMB;
}
