...
template <typename T, typename CharT = typename T::value_type> int CompareCaseInsensitiveASCIIT(T a, T b)
{
...
template <bool (*Validator)(base_icu::UChar32)> inline bool DoIsStringUTF8(StringPiece str)
{
...
// char16_t.
template <typename list_type, typename T, typename CharT = typename T::value_type> static std::basic_string<CharT> JoinStringT(list_type parts, T sep)
{
    if (std::empty(parts))
...
template <typename T, typename CharT = typename T::value_type>
std::basic_string<CharT> DoReplaceStringPlaceholders(T format_string, const std::vector<std::basic_string<CharT>>& subst, std::vector<size_t>* offsets)
{
        ...
        if ('$' == *i) {
            if (i + 1 != format_string.end()) {
                ...
                if ('$' == *i) {
                    ...
                    if (*i < '1' || *i > '9') {
                        DLOG(ERROR) << "Invalid placeholder: $" << std::basic_string<CharT>(1, *i);
                        continue;
                    }
...
template <typename CHAR> size_t lcpyT(CHAR* dst, const CHAR* src, size_t dst_size)
{
