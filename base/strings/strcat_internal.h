...
template <typename CharT> auto Resize(std::basic_string<CharT>& str, size_t total_size, priority_tag<1>) -> decltype(str.__resize_default_init(total_size))
{
...
template <typename CharT> void Resize(std::basic_string<CharT>& str, size_t total_size, priority_tag<0>)
{
