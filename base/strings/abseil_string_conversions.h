...
BASE_EXPORT std::vector<absl::string_view> StringPiecesToStringViews(span<const StringPiece> pieces);

// Converts `views` to string pieces, pointing to the same piece of memory.
BASE_EXPORT std::vector<StringPiece> StringViewsToStringPieces(span<const absl::string_view> views);
