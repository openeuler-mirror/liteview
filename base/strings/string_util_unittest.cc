...
TEST(StringUtilTest, as_wcstr)
{
    ...
    static_assert(std::is_same<wchar_t*, decltype(as_writable_wcstr(rw_buffer))>::value, "");
    EXPECT_EQ(static_cast<void*>(rw_buffer), as_writable_wcstr(rw_buffer));

    std::u16string rw_str(10, '\0');
    static_assert(std::is_same<wchar_t*, decltype(as_writable_wcstr(rw_str))>::value, "");
    EXPECT_EQ(static_cast<const void*>(rw_str.data()), as_writable_wcstr(rw_str));

    const char16_t ro_buffer[10] = {};
    static_assert(std::is_same<const wchar_t*, decltype(as_wcstr(ro_buffer))>::value, "");
...
TEST(StringUtilTest, as_u16cstr)
{
    ...
    static_assert(std::is_same<char16_t*, decltype(as_writable_u16cstr(rw_buffer))>::value, "");
    EXPECT_EQ(static_cast<void*>(rw_buffer), as_writable_u16cstr(rw_buffer));

    std::wstring rw_str(10, '\0');
    static_assert(std::is_same<char16_t*, decltype(as_writable_u16cstr(rw_str))>::value, "");
    EXPECT_EQ(static_cast<const void*>(rw_str.data()), as_writable_u16cstr(rw_str));

    const wchar_t ro_buffer[10] = {};
    static_assert(std::is_same<const char16_t*, decltype(as_u16cstr(ro_buffer))>::value, "");
    EXPECT_EQ(static_cast<const void*>(ro_buffer), as_u16cstr(ro_buffer));

    const std::wstring ro_str(10, '\0');
    static_assert(std::is_same<const char16_t*, decltype(as_u16cstr(ro_str))>::value, "");
    EXPECT_EQ(static_cast<const void*>(ro_str.data()), as_u16cstr(ro_str));

    WStringPiece piece = ro_buffer;
    static_assert(std::is_same<const char16_t*, decltype(as_u16cstr(piece))>::value, "");
        ...
        for (size_t offset = 0; offset < 4; ++offset) {
            for (size_t len = 0, max_len = string_length - offset; len < max_len; ++len) {
                ...
                for (size_t char_pos = offset; char_pos < len; ++char_pos) {
                    ...
                    EXPECT_FALSE(IsStringASCII(StringPiece16(char16_ascii + offset, len)));
                    char16_ascii[char_pos] &= ~0x80;
...
TEST(StringUtilTest, JoinStringInitializerList16)
{
    ...
    const std::u16string kSpace = u" ";
    EXPECT_EQ(u"a|b|c|| ", JoinString({ kA, kB, kC, StringPiece16(), kSpace }, u"|"));
...
TEST(StringUtilTest, StreamableToString)
{
    ...
    EXPECT_EQ(StreamableToString(StreamableTestEnum::kGreeting, " ", StreamableTestEnum::kLocation), "hello world");
    EXPECT_EQ(StreamableToString("42 in hex is ", std::hex, 42), "42 in hex is 2a");
}
