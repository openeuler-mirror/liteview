...
template <typename T, typename CharT = typename T::value_type> size_t findT(T self, T s, size_t pos)
{
    if (pos > self.size())
        return BasicStringPiece<CharT>::npos;

    typename BasicStringPiece<CharT>::const_iterator result = std::search(self.begin() + pos, self.end(), s.begin(), s.end());
...
template <typename T, typename CharT = typename T::value_type> size_t rfindT(T self, T s, size_t pos)
{
    if (self.size() < s.size())
        return BasicStringPiece<CharT>::npos;
    ...
    typename BasicStringPiece<CharT>::const_iterator last = self.begin() + std::min(self.size() - s.size(), pos) + s.size();
    typename BasicStringPiece<CharT>::const_iterator result = std::find_end(self.begin(), last, s.begin(), s.end());
    return result != last ? static_cast<size_t>(result - self.begin()) : BasicStringPiece<CharT>::npos;
}
...
template <typename T, typename CharT = typename T::value_type> size_t find_first_ofT(T self, T s, size_t pos)
{
    // Use the faster std::find() if searching for a single character.
    typename BasicStringPiece<CharT>::const_iterator found
        = s.size() == 1 ? std::find(self.begin() + pos, self.end(), s[0]) : std::find_first_of(self.begin() + pos, self.end(), s.begin(), s.end());
    if (found == self.end())
        return BasicStringPiece<CharT>::npos;
...
// Generic brute-force version.
template <typename T, typename CharT = typename T::value_type> size_t find_first_not_ofT(T self, T s, size_t pos)
{
    if (self.size() == 0)
        return BasicStringPiece<CharT>::npos;
...
// Generic brute-force version.
template <typename T, typename CharT = typename T::value_type> size_t find_last_ofT(T self, T s, size_t pos)
{
    if (self.size() == 0)
        return BasicStringPiece<CharT>::npos;
...
// Generic brute-force version.
template <typename T, typename CharT = typename T::value_type> size_t find_last_not_ofT(T self, T s, size_t pos)
{
    if (self.size() == 0)
        return StringPiece::npos;
