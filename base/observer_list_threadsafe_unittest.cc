...
public:
    AddRemoveThread(ObserverListThreadSafe<Foo>* list, bool notify)
        ...
        , do_notifies_(notify)
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&AddRemoveThread::AddTask, weak_factory_.GetWeakPtr()));
    }
    ...
    void AddTask()
    {
        ...
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&AddRemoveThread::AddTask, weak_factory_.GetWeakPtr()));
    }
...
static void ThreadSafeObserverHarness(int num_threads, bool cross_thread_notifies)
{
    test::TaskEnvironment task_environment;
    ...
    for (int index = 0; index < num_threads; index++) {
        threaded_observer.push_back(std::make_unique<AddRemoveThread>(observer_list.get(), cross_thread_notifies));
    }
