...
bool AreSearchFlagsAvailable()
{
    ...
    // method.
    static const auto add_dll_dir_func = reinterpret_cast<decltype(::AddDllDirectory)*>(GetProcAddress(GetModuleHandle(L"kernel32.dll"), "AddDllDirectory"));
    return !!add_dll_dir_func;
}

// A helper method to encode the library loading result to enum
// LoadLibraryResult.
LoadLibraryResult GetLoadLibraryResult(bool are_search_flags_available, bool has_load_library_succeeded)
{
...
NativeLibrary LoadNativeLibraryHelper(const FilePath& library_path, NativeLibraryLoadError* error)
{
    ...
    if (are_search_flags_available) {
        ...
        // directory.
        module = ::LoadLibraryExW(library_path.value().c_str(), nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
...
NativeLibrary LoadSystemLibraryHelper(const FilePath& library_path, NativeLibraryLoadError* error)
{
    ...
    if (!module_found) {
        ...
        DWORD flags = are_search_flags_available ? LOAD_LIBRARY_SEARCH_SYSTEM32 : LOAD_WITH_ALTERED_SEARCH_PATH;
