...
void XmlUnitTestResultPrinter::AddLink(const std::string& name, const std::string& url)
{
    ...
    fprintf(output_file_.get(),
        ...
        "link_name=\"%s\">%s</link>\n",
        info->name(), info->test_case_name(), name.c_str(), escaped_url.c_str());
    fflush(output_file_);
}
...
void XmlUnitTestResultPrinter::OnTestCaseStart(const testing::TestCase& test_case)
{
...
void XmlUnitTestResultPrinter::OnTestEnd(const testing::TestInfo& test_info)
{
    fprintf(output_file_.get(),
        ...
        test_info.name(), static_cast<double>(test_info.result()->elapsed_time()) / Time::kMillisecondsPerSecond, test_info.test_case_name(),
        TimeToISO8601(Time::FromJavaTime(test_info.result()->start_timestamp())).c_str());
...
void XmlUnitTestResultPrinter::OnTestCaseEnd(const testing::TestCase& test_case)
{
    fprintf(output_file_.get(), "  </testsuite>\n");
    fflush(output_file_);
}
