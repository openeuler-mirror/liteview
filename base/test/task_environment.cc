...
std::unique_ptr<sequence_manager::SequenceManager> CreateSequenceManagerForMainThreadType(TaskEnvironment::MainThreadType main_thread_type)
{
    ...
    return sequence_manager::CreateSequenceManagerOnCurrentThreadWithPump(
        MessagePump::Create(type), base::sequence_manager::SequenceManager::Settings::Builder().SetMessagePumpType(type).Build());
}
...
public:
    ...
    bool MaybeFastForwardToWakeUp(absl::optional<sequence_manager::WakeUp> next_wake_up, bool quit_when_idle_requested) override
    {
        if (quit_when_idle_requested)
            return false;

        return FastForwardToNextTaskOrCap(next_wake_up, TimeTicks::Max()) == NextTaskSource::kMainThreadHasWork;
    }
    ...
    NextTaskSource FastForwardToNextTaskOrCap(absl::optional<sequence_manager::WakeUp> next_main_thread_wake_up, TimeTicks fast_forward_cap)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
TaskEnvironment::TaskEnvironment(TimeSource time_source, MainThreadType main_thread_type, ThreadPoolExecutionMode thread_pool_execution_mode,
    ThreadingMode threading_mode, ThreadPoolCOMEnvironment thread_pool_com_environment, bool subclass_creates_default_taskrunner, trait_helpers::NotATraitTag)
    ...
    , sequence_manager_(CreateSequenceManagerForMainThreadType(main_thread_type))
    ...
    , time_overrides_(time_source == TimeSource::MOCK_TIME
              ? std::make_unique<subtle::ScopedTimeClockOverrides>(&MockTimeDomain::GetTime, &MockTimeDomain::GetTimeTicks, nullptr)
              : nullptr)
...
void TaskEnvironment::FastForwardBy(TimeDelta delta)
{
    ...
    do {
        ...
        mock_time_domain_->FastForwardToNextTaskOrCap(sequence_manager_->GetNextDelayedWakeUp(), fast_forward_until) != MockTimeDomain::NextTaskSource::kNone);
...
TimeDelta TaskEnvironment::NextMainThreadPendingTaskDelay() const
{
    ...
    if (!sequence_manager_->IsIdleForTesting())
        return TimeDelta();
    absl::optional<sequence_manager::WakeUp> wake_up = sequence_manager_->GetNextDelayedWakeUp();
