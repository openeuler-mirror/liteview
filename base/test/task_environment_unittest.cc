...
TEST_F(TaskEnvironmentTest, SupportsFileDescriptorWatcherOnIOMockTimeMainThread)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&]() {
        int64_t x = 1;
...
TEST_F(TaskEnvironmentTest, FastForwardAdvancesTickClock)
{
    ...
    constexpr base::TimeDelta kShortTaskDelay = Days(1);
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::DoNothing(), kShortTaskDelay);

    constexpr base::TimeDelta kLongTaskDelay = Days(7);
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::DoNothing(), kLongTaskDelay);
...
TEST_F(TaskEnvironmentTest, AdvanceClockDoesNotRunTasks)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::DoNothing(), kTaskDelay);
...
TEST_F(TaskEnvironmentTest, FastForwardOnlyAdvancesWhenIdle)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { EXPECT_EQ(start_time, base::TimeTicks::Now()); }));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&]() { EXPECT_EQ(start_time + kDelay, base::TimeTicks::Now()); }), kDelay);
...
TEST_F(TaskEnvironmentTest, FastForwardZero)
{
    ...
    for (int i = 0; i < 1000; ++i) {
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { run_count.fetch_add(1, std::memory_order_relaxed); }));
...
TEST_F(TaskEnvironmentTest, NestedFastForwardBy)
{
    ...
    post_fast_forwarding_task = BindLambdaForTesting([&]() {
        if (max_nesting_level < 5) {
            ++max_nesting_level;
            ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_fast_forwarding_task, kDelayPerTask);
...
TEST_F(TaskEnvironmentTest, NestedRunInFastForwardBy)
{
    ...
    post_and_runloop_task = BindLambdaForTesting([&]() {
        ...
        if (run_loops.size() < 4U) {
            ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_and_runloop_task, kDelayPerTask);
...
TEST_F(TaskEnvironmentTest, MultiThreadedMockTime)
{
    ...
    post_main_thread_delayed_task = BindLambdaForTesting([&]() {
        ...
        if (last_main_thread_ticks < task_environment.NowTicks() && task_environment.NowTicks() < end_time) {
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
        }
    ...
    post_thread_pool_delayed_task = BindLambdaForTesting([&]() {
        ...
        if (last_thread_pool_ticks < task_environment.NowTicks() && task_environment.NowTicks() < end_time) {
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
            SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_thread_pool_delayed_task, kOneMs);
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, post_main_thread_delayed_task, kOneMs);
...
TEST_F(TaskEnvironmentTest, MultiThreadedFastForwardBy)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Seconds(5));
...
TEST_F(TaskEnvironmentTest, MultiThreadedMockTimeAndThreadPoolQueuedMode)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { count += 1; }));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&]() {
        count += 2;
        run_loop.Quit();
    }),
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { count += 2048; }));
...
TEST_F(TaskEnvironmentTest, DescribeCurrentTasksHasPendingMainThreadTasks)
{
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOG_INFO, _, _, _, HasSubstr("ThreadPool currently running tasks"))).WillOnce(Return(true));
    // The pending task posted above to the main thread is logged.
    EXPECT_CALL(mock_log, Log(::logging::LOG_INFO, _, _, _, HasSubstr("task_environment_unittest.cc"))).WillOnce(Return(true));
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOG_INFO, _, _, _, HasSubstr("\"immediate_work_queue_size\":0"))).WillOnce(Return(true));
    task_environment.DescribeCurrentTasks();
}
...
TEST_F(TaskEnvironmentTest, DescribeCurrentTasksHasThreadPoolTasks)
{
    ...
    EXPECT_CALL(mock_log, Log(::logging::LOG_INFO, _, _, _, Not(HasSubstr("task_environment_unittest.cc")))).WillOnce(Return(true));
...
TEST_F(TaskEnvironmentTest, Basic)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 256; }, Unretained(&counter)), Seconds(3));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 64; }, Unretained(&counter)), Seconds(1));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 1024; }, Unretained(&counter)), Minutes(20));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([](int* counter) { *counter += 4096; }, Unretained(&counter)), Days(20));
...
TEST_F(TaskEnvironmentTest, RunLoopDriveable)
{
    ...
    int counter = 0;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1; }, Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 32; }, Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 256; }, Unretained(&counter)), Seconds(3));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 64; }, Unretained(&counter)), Seconds(1));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 1024; }, Unretained(&counter)), Minutes(20));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 4096; }, Unretained(&counter)), Days(20));
    ...
    {
        RunLoop run_loop;
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), Seconds(1));
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 8192; }, Unretained(&counter)), Seconds(1));
    ...
    {
        RunLoop run_loop;
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitWhenIdleClosure(), Seconds(5));
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindOnce([](int* counter) { *counter += 16384; }, Unretained(&counter)), Seconds(5));
    ...
    RunLoop run_loop;
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitWhenIdleClosure(), Days(50));
...
TEST_F(TaskEnvironmentTest, RunLoopGetsTurnAfterYieldingToPool)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Seconds(1));
...
TEST_F(TaskEnvironmentTest, CancelPendingTask)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, task1.callback(), Seconds(1));
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, task2.callback(), Seconds(1));
    task2.Cancel();
    EXPECT_EQ(0u, task_environment.GetPendingMainThreadTaskCount());

    CancelableRepeatingClosure task3(BindRepeating([]() {}));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, task3.callback(), Seconds(1));
    task3.Cancel();
    EXPECT_EQ(TimeDelta::Max(), task_environment.NextMainThreadPendingTaskDelay());

    CancelableRepeatingClosure task4(BindRepeating([]() {}));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, task4.callback(), Seconds(1));
    task4.Cancel();
    EXPECT_TRUE(task_environment.MainThreadIsIdle());
}
...
TEST_F(TaskEnvironmentTest, NoFastForwardToCancelledTask)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, task.callback(), Seconds(1));
    EXPECT_EQ(Seconds(1), task_environment.NextMainThreadPendingTaskDelay());
    task.Cancel();
    task_environment.FastForwardUntilNoTasksRemain();
...
TEST_F(TaskEnvironmentTest, NextTaskIsDelayed)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce([]() {}), Seconds(2));
    EXPECT_TRUE(task_environment.NextTaskIsDelayed());
    task_environment.FastForwardUntilNoTasksRemain();
    EXPECT_FALSE(task_environment.NextTaskIsDelayed());
...
TEST_F(TaskEnvironmentTest, SingleThread)
{
    ...
    bool ran = false;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() { ran = true; }));
    RunLoop().RunUntilIdle();
    EXPECT_TRUE(ran);
...
TEST_F(TaskEnvironmentTest, SingleThreadMockTime)
{
    ...
    int counter = 0;
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, base::BindLambdaForTesting([&]() { counter += 1; }), kDelay);
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() { counter += 2; }));
