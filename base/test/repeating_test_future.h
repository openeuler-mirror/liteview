...
public:
    ...
    template <typename T = TupleType, internal::EnableIfSingleValue<T> = true> FirstType Take()
    {
    ...
    //
    // Will DCHECK if a timeout happens.
    template <typename T = TupleType, internal::EnableIfMultiValue<T> = true> TupleType Take()
    {
        return TakeTuple();
    }
