...
public:
    ...
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindLambdaForTesting(lambda));
    }

    void RunLater(base::OnceClosure callable)
    {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, std::move(callable));
    }

    void PostDelayedTask(base::OnceClosure callable, base::TimeDelta delay)
    {
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, std::move(callable), delay);
    }
...
TEST_F(TestFutureTest, WaitShouldBlockUntilValueArrives)
{
    ...
    PostDelayedTask(base::BindOnce(future.GetCallback(), expected_value), base::Milliseconds(1));
...
TEST_F(TestFutureTest, WaitShouldReturnTrueWhenValueArrives)
{
    ...
    PostDelayedTask(base::BindOnce(future.GetCallback(), kAnyValue), base::Milliseconds(1));
...
TEST_F(TestFutureTest, ShouldOnlyAllowSetValueToBeCalledOnce)
{
    ...
    EXPECT_DCHECK_DEATH_WITH(future.SetValue(kOtherValue), "The value of a TestFuture can only be set once.");
}
...
TEST_F(TestFutureTest, ShouldAllowReferenceArgumentsForCallback)
{
    ...
    base::OnceCallback<void(const int&)> callback = future.GetCallback<const int&>();
...
TEST_F(TestFutureTest, ShouldReturnTupleValue)
{
    ...
    RunLater(base::BindOnce(future.GetCallback(), expected_int_value, expected_string_value));
...
TEST_F(TestFutureTest, ShouldAllowReferenceArgumentsForMultiArgumentCallback)
{
    ...
    base::OnceCallback<void(int, const std::string&)> callback = future.GetCallback<int, const std::string&>();
    RunLater(base::BindOnce(std::move(callback), expected_int_value, expected_string_value));
