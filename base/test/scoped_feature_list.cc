...
bool ContainsFeature(const std::vector<ScopedFeatureList::FeatureWithStudyGroup>& feature_vector, StringPiece feature_name)
{
...
ScopedFeatureList::FeatureAndParams::FeatureAndParams(const Feature& feature, const FieldTrialParams& params)
    : feature(feature)
    , params(params)
{
}
...
ScopedFeatureList::FeatureAndParams::FeatureAndParams(const FeatureAndParams& other) = default;
...
void ScopedFeatureList::InitWithFeaturesImpl(const std::vector<FeatureRef>& enabled_features, const std::vector<FeatureAndParams>& enabled_features_and_params,
    const std::vector<FeatureRef>& disabled_features, bool keep_existing_states)
{
    DCHECK(!init_called_);
...
void ScopedFeatureList::InitWithFeaturesAndParameters(const std::vector<FeatureAndParams>& enabled_features, const std::vector<FeatureRef>& disabled_features)
{
    InitWithFeaturesImpl({}, enabled_features, disabled_features);
}
...
void ScopedFeatureList::InitWithMergedFeatures(Features&& merged_features, bool create_associated_field_trials, bool keep_existing_states)
{
    ...
    for (const auto& feature : merged_features.enabled_feature_list) {
        ...
        EraseIf(all_states, [feature](const auto& state) { return state.trial_name == feature.StudyNameOrDefault(); });
