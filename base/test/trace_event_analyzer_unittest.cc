...
TEST_F(TraceEventAnalyzerTest, BeginEndDuration)
{
    ...
    TraceEventVector found;
    analyzer->FindEvents(Query::MatchBeginWithEnd() && Query::EventDuration() > Query::Int(static_cast<int>(duration_cutoff_us))
...
TEST_F(TraceEventAnalyzerTest, ComplexArgument)
{
    ...
    EXPECT_EQ(absl::optional<std::string>("value"), base::OptionalFromPtr(arg.FindString("property")));
}
