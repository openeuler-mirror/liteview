...
ScopedRunLoopTimeout::ScopedRunLoopTimeout(const Location& timeout_enabled_from_here, TimeDelta timeout, RepeatingCallback<std::string()> on_timeout_log)
    ...
    if (g_add_gtest_failure_on_timeout) {
        run_timeout_.on_timeout = BindRepeating(
            [](const Location& timeout_enabled_from_here, RepeatingCallback<std::string()> on_timeout_log, const Location& run_from_here) {
                GTEST_FAIL_AT(run_from_here.file_name(), run_from_here.line_number()) << TimeoutMessage(on_timeout_log, timeout_enabled_from_here);
            },
            timeout_enabled_from_here, std::move(on_timeout_log));
    } else {
        run_timeout_.on_timeout = BindRepeating(
            [](const Location& timeout_enabled_from_here, RepeatingCallback<std::string()> on_timeout_log, const Location& run_from_here) {
                std::string message = TimeoutMessage(on_timeout_log, timeout_enabled_from_here);
