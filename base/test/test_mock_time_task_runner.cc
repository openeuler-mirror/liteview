    ...
    , mock_clock_(this)
{
    if (type == Type::kBoundToThread) {
        ...
        thread_task_runner_handle_ = std::make_unique<ThreadTaskRunnerHandle>(proxy_task_runner_);
    }
}
...
void TestMockTimeTaskRunner::ProcessTasksNoLaterThan(TimeDelta max_delta, int limit)
{
    ...
    if (!ThreadTaskRunnerHandle::IsSet() || ThreadTaskRunnerHandle::Get() != proxy_task_runner_.get()) {
