...
template <typename Callback, typename Tuple, size_t... Is> decltype(auto) RunImpl(Callback&& cb, Tuple&& tuple, std::index_sequence<Is...>)
{
    return std::forward<Callback>(cb).Run(std::get<Is>(std::forward<Tuple>(tuple))...);
}
...
{
    return RunImpl(std::forward<Callback>(cb), std::forward<Tuple>(tuple), std::make_index_sequence<std::tuple_size<std::remove_reference_t<Tuple>>::value>());
}
...
template <size_t I, typename Tuple, std::enable_if_t<std::is_copy_constructible<Tuple>::value, int> = 0> auto RunOnceCallbackImpl(Tuple&& tuple)
{
    return [tuple = std::forward<Tuple>(tuple)](auto&&... args) -> decltype(auto) { return RunImpl(std::move(internal::get<I>(args...)), tuple); };
}
...
template <size_t I, typename Tuple, std::enable_if_t<!std::is_copy_constructible<Tuple>::value, int> = 0> auto RunOnceCallbackImpl(Tuple&& tuple)
{
    ...
    auto tuple_ptr = base::MakeRefCounted<base::RefCountedData<Tuple>>(std::forward<Tuple>(tuple));
    return [tuple_ptr = std::move(tuple_ptr)](auto&&... args) mutable -> decltype(auto) {
        ...
        // crash.
        return RunImpl(std::move(internal::get<I>(args...)), std::move(std::exchange(tuple_ptr, nullptr)->data));
    };
}
...
template <size_t I, typename Tuple> auto RunRepeatingCallbackImpl(Tuple&& tuple)
{
    return [tuple = std::forward<Tuple>(tuple)](auto&&... args) -> decltype(auto) { return RunImpl(internal::get<I>(args...), tuple); };
}
...
inline auto RunOnceClosure(base::OnceClosure cb)
{
    ...
    scoped_refptr<RefCountedOnceClosure> copyable_cb = base::MakeRefCounted<RefCountedOnceClosure>(std::move(cb));
...
{
    return [](auto&&... args) -> decltype(auto) { return internal::get<I>(args...).Run(); };
}

template <size_t I> auto RunOnceClosure()
{
    return [](auto&&... args) -> decltype(auto) { return std::move(internal::get<I>(args...)).Run(); };
}
...
{
    return internal::RunOnceCallbackImpl<I>(std::make_tuple(std::forward<RunArgs>(run_args)...));
}

template <size_t I, typename... RunArgs> auto RunCallback(RunArgs&&... run_args)
{
    return internal::RunRepeatingCallbackImpl<I>(std::make_tuple(std::forward<RunArgs>(run_args)...));
}
