...
void HistogramTester::ExpectUniqueSample(StringPiece name, HistogramBase::Sample sample, HistogramBase::Count expected_bucket_count) const
{
...
void HistogramTester::ExpectUniqueTimeSample(StringPiece name, TimeDelta sample, HistogramBase::Count expected_bucket_count) const
{
    ExpectUniqueSample(name, sample.InMilliseconds(), expected_bucket_count);
}

void HistogramTester::ExpectBucketCount(StringPiece name, HistogramBase::Sample sample, HistogramBase::Count expected_count) const
{
    HistogramBase* histogram = StatisticsRecorder::FindHistogram(name);
    if (histogram) {
        std::unique_ptr<HistogramSamples> samples = histogram->SnapshotSamples();
...
void HistogramTester::ExpectTotalCount(StringPiece name, HistogramBase::Count count) const
{
    HistogramBase* histogram = StatisticsRecorder::FindHistogram(name);
    if (histogram) {
        std::unique_ptr<HistogramSamples> samples = histogram->SnapshotSamples();
...
void HistogramTester::ExpectTimeBucketCount(StringPiece name, TimeDelta sample, HistogramBase::Count count) const
{
...
HistogramBase::Count HistogramTester::GetBucketCount(StringPiece name, HistogramBase::Sample sample) const
{
    HistogramBase* histogram = StatisticsRecorder::FindHistogram(name);
...
void HistogramTester::GetBucketCountForSamples(
    StringPiece name, HistogramBase::Sample sample, const HistogramSamples& samples, HistogramBase::Count* count) const
{
...
HistogramTester::CountsMap HistogramTester::GetTotalCountsForPrefix(StringPiece prefix) const
{
    EXPECT_TRUE(prefix.find('.') != StringPiece::npos) << "|prefix| ought to contain at least one period, to avoid matching too"
                                                       << " many histograms.";
...
std::unique_ptr<HistogramSamples> HistogramTester::GetHistogramSamplesSinceCreation(StringPiece histogram_name) const
{
...
void HistogramTester::CheckBucketCount(
    StringPiece name, HistogramBase::Sample sample, HistogramBase::Count expected_count, const HistogramSamples& samples) const
{
    int actual_count;
...
int HistogramTester::GetTotalCountForSamples(StringPiece name, const HistogramSamples& samples) const
{
...
void HistogramTester::CheckTotalCount(StringPiece name, HistogramBase::Count expected_count, const HistogramSamples& samples) const
{
