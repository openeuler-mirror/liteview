...
public:
    ...
    void ExpectUniqueSample(StringPiece name, HistogramBase::Sample sample, HistogramBase::Count expected_bucket_count) const;
    template <typename T> void ExpectUniqueSample(StringPiece name, T sample, HistogramBase::Count expected_bucket_count) const
    {
        ExpectUniqueSample(name, static_cast<HistogramBase::Sample>(sample), expected_bucket_count);
    }
    void ExpectUniqueTimeSample(StringPiece name, TimeDelta sample, HistogramBase::Count expected_bucket_count) const;
    ...
    // object was constructed.
    void ExpectBucketCount(StringPiece name, HistogramBase::Sample sample, HistogramBase::Count expected_count) const;
    template <typename T> void ExpectBucketCount(StringPiece name, T sample, HistogramBase::Count expected_count) const
    {
        ExpectBucketCount(name, static_cast<HistogramBase::Sample>(sample), expected_count);
    }
    ...
    void ExpectTimeBucketCount(StringPiece name, TimeDelta sample, HistogramBase::Count count) const;
    ...
    HistogramBase::Count GetBucketCount(StringPiece name, HistogramBase::Sample sample) const;
    template <typename T> HistogramBase::Count GetBucketCount(StringPiece name, T sample) const
    {
    ...
    std::unique_ptr<HistogramSamples> GetHistogramSamplesSinceCreation(StringPiece histogram_name) const;
...
private:
    ...
    void CheckBucketCount(StringPiece name, HistogramBase::Sample sample, Histogram::Count expected_count, const HistogramSamples& samples) const;
    ...
    // taken for |name|.
    int GetTotalCountForSamples(StringPiece name, const HistogramSamples& samples) const;
    ...
    void CheckTotalCount(StringPiece name, Histogram::Count expected_count, const HistogramSamples& samples) const;
    ...
    void GetBucketCountForSamples(StringPiece name, HistogramBase::Sample sample, const HistogramSamples& samples, HistogramBase::Count* count) const;
...
struct Bucket {
    ...
    template <typename MetricEnum, typename = std::enable_if_t<std::is_enum_v<MetricEnum>>>
