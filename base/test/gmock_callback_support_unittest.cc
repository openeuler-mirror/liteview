...
TEST(GmockCallbackSupportTest, RunOnceCallbackTwice)
{
    ...
    EXPECT_CALL(check, Call).WillRepeatedly(RunOnceCallback<0>(std::ref(src), &dst));
...
TEST(GmockCallbackSupportTest, RunOnceCallbackMultipleTimesWithMoveOnlyArgCrashes)
{
    MockFunction<void(TestOnceCallbackMove)> check;
    auto val = std::make_unique<int>(42);
    int dst = 0;
    EXPECT_CALL(check, Call).WillRepeatedly(RunOnceCallback<0>(std::move(val), &dst));
