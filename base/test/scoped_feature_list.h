...
public:
    ...
    struct FeatureAndParams {
        FeatureAndParams(const Feature& feature ABSL_ATTRIBUTE_LIFETIME_BOUND, const FieldTrialParams& params);
        ~FeatureAndParams();
    ...
    // Note: This creates a scoped global field trial list if there is not
    // currently one.
    void InitWithFeaturesAndParameters(const std::vector<FeatureAndParams>& enabled_features, const std::vector<FeatureRef>& disabled_features);
...
private:
    ...
    // empty).
    void InitWithFeaturesImpl(const std::vector<FeatureRef>& enabled_features, const std::vector<FeatureAndParams>& enabled_features_and_params,
