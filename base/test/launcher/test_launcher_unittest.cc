...
// at |key| value.
bool ValidateStringList(const absl::optional<Value::Dict>& root, const std::string& key, std::vector<const char*> values)
{
...
TEST_F(ResultWatcherTest, PollCompletesQuickly)
{
    ...
    ASSERT_TRUE(AppendToFile(result_file,
        StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeToISO8601(Time::Now()).c_str(), "\" />\n",
...
TEST_F(ResultWatcherTest, PollCompletesSlowly)
{
    ...
    FilePath result_file = CreateResultFile();
    ASSERT_TRUE(
        AppendToFile(result_file, StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeToISO8601(Time::Now()).c_str(), "\" />\n" })));
...
TEST_F(ResultWatcherTest, PollWithClockJumpBackward)
{
    ...
    ASSERT_TRUE(AppendToFile(result_file,
        StrCat({ "    <x-teststart name=\"B\" classname=\"A\" timestamp=\"", TimeToISO8601(time_before_change).c_str(), "\" />\n",
            "    <testcase name=\"B\" status=\"run\" time=\"0.500\" "
            "classname=\"A\" timestamp=\"",
...
void MatchesFatalMessagesTest()
{
    ...
    logging::SetLogMessageHandler([](int severity, const char* file, int line, size_t start, const std::string& str) -> bool {
        *log_string = str;
        return true;
    });
