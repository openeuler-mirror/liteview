    ...
    RepeatingClosure timeout_callback, OnceClosure gtest_init)
{
    ...
    if (CommandLine::ForCurrentProcess()->HasSwitch(kGTestHelpFlag) || CommandLine::ForCurrentProcess()->HasSwitch(kGTestListTestsFlag)
        ...
        || CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestChildProcess) || force_single_process) {
        return std::move(run_test_suite).Run();
    }
...
#endif
    use_job_objects = use_job_objects && !CommandLine::ForCurrentProcess()->HasSwitch(kDontUseJobObjectFlag);
...
void MergeTestFilterSwitchHandler::ResolveDuplicate(base::StringPiece key, CommandLine::StringPieceType new_value, CommandLine::StringType& out_value)
{
