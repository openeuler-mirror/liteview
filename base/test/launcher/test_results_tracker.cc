...
std::string FormatTimeAsIso8601(Time time)
{
    ...
    return StringPrintf("%04d-%02d-%02dT%02d:%02d:%02d", exploded.year, exploded.month, exploded.day_of_month, exploded.hour, exploded.minute, exploded.second);
}
        ...
        for (const auto& j : per_iteration_data_[i].results) {
            ...
            for (size_t k = 0; k < j.second.test_results.size(); k++) {
                ...
                if (test_result.timestamp) {
                    ...
                    test_result_value.Set("timestamp", TimeToISO8601(*test_result.timestamp).c_str());
                }
