    ...
    TimeDelta timeout_per_test, size_t num_tests, TestLauncherDelegate* delegate, bool* was_timeout)
{
    ...
    if (flags & TestLauncher::USE_JOB_OBJECTS) {
        ...
        if (win::GetVersion() < win::Version::WIN8 && flags & TestLauncher::ALLOW_BREAKAWAY_FROM_JOB) {
...
class TestRunner {
public:
    explicit TestRunner(TestLauncher* launcher, size_t runner_count = 1u, size_t batch_size = 1u)
        : launcher_(launcher)
...
private:
    ...
    static bool ShouldReuseStateFromLastBatch(const std::vector<std::string>& test_names)
    {
    ...
    void LaunchNextTask(scoped_refptr<TaskRunner> task_runner, const FilePath& last_task_temp_dir);

    // Forwards |last_task_temp_dir| and launches the next task on main thread.
    // The method is called on |task_runner|.
    void ClearAndLaunchNext(scoped_refptr<TaskRunner> main_thread_runner, scoped_refptr<TaskRunner> task_runner, const FilePath& last_task_temp_dir)
    {
        main_thread_runner->PostTask(FROM_HERE, BindOnce(&TestRunner::LaunchNextTask, weak_ptr_factory_.GetWeakPtr(), task_runner, last_task_temp_dir));
    }
...
void TestRunner::Run(const std::vector<std::string>& test_names)
{
    ...
    for (size_t i = 0; i < runner_count_; i++) {
        task_runners_.push_back(ThreadPool::CreateSequencedTaskRunner({ MayBlock(), TaskShutdownBehavior::BLOCK_SHUTDOWN }));
        ThreadTaskRunnerHandle::Get()->PostTask(
            FROM_HERE, BindOnce(&TestRunner::LaunchNextTask, weak_ptr_factory_.GetWeakPtr(), task_runners_.back(), FilePath()));
    }
    run_loop_.Run();
}

void TestRunner::LaunchNextTask(scoped_refptr<TaskRunner> task_runner, const FilePath& last_task_temp_dir)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    // delete previous temporary directory
    if (!last_task_temp_dir.empty() && !DeletePathRecursively(last_task_temp_dir)) {
    ...
    while (post_to_current_runner && !tests_to_run_.empty()) {
        ...
        task_runner->PostTask(FROM_HERE,
            BindOnce(&TestLauncher::LaunchChildGTestProcess, Unretained(launcher_), ThreadTaskRunnerHandle::Get(), batch, task_temp_dir,
                CreateChildTempDirIfSupported(task_temp_dir, child_index++)));
        post_to_current_runner = ShouldReuseStateFromLastBatch(batch);
    }
    task_runner->PostTask(FROM_HERE, BindOnce(&TestRunner::ClearAndLaunchNext, Unretained(this), ThreadTaskRunnerHandle::Get(), task_runner, task_temp_dir));
}
...
// be at least 30.
std::string TruncateSnippet(const base::StringPiece snippet, size_t byte_limit)
{
...
void TestLauncher::OnTestFinished(const TestResult& original_result)
{
    ...
    if (print_snippet) {
        std::vector<base::StringPiece> snippet_lines = SplitStringPiece(result.output_snippet, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
...
std::vector<std::string> TestLauncher::CollectTests()
{
    ...
    for (const TestInfo& test_info : tests_) {
        ...
        if (PersistentHash(prefix_stripped_name) % total_shards_ != static_cast<uint32_t>(shard_index_)) {
            continue;
        }
...
std::string TruncateSnippetFocused(const base::StringPiece snippet, size_t byte_limit)
{
