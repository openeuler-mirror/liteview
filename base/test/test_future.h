...
public:
    ...
    //
    template <std::size_t I> const typename std::tuple_element<I, TupleType>::type& Get()
    {
    ...
    //
    template <typename... CallbackArgumentsTypes> base::OnceCallback<void(CallbackArgumentsTypes...)> GetCallback()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    void SetValue(Types... values)
    {
        ...
        DCHECK(!values_.has_value()) << "The value of a TestFuture can only be set once. If you need to "
    ...
    //
    // Will DCHECK if a timeout happens.
    template <typename T = TupleType, internal::EnableIfSingleValue<T> = true> [[nodiscard]] const FirstType& Get()
    {
    ...
    //
    // Will DCHECK if a timeout happens.
    template <typename T = TupleType, internal::EnableIfSingleValue<T> = true> [[nodiscard]] FirstType Take()
    {
    ...
    //
    // Will DCHECK if a timeout happens.
    template <typename T = TupleType, internal::EnableIfMultiValue<T> = true> [[nodiscard]] const TupleType& Get()
    {
        return GetTuple();
    }
    ...
    //
    // Will DCHECK if a timeout happens.
    template <typename T = TupleType, internal::EnableIfMultiValue<T> = true> [[nodiscard]] TupleType Take()
    {
        return TakeTuple();
    }
