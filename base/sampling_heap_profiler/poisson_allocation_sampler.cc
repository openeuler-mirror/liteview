...
SimpleTlsBoolXp g_tls_internal_reentry_guard;
SimpleTlsIntptrXp g_tls_accumulated_bytes;
SimpleTlsIntptrXp g_tls_accumulated_bytes_snapshot;
const intptr_t kAccumulatedBytesOffset = 1 << 29;
SimpleTlsBoolXp g_tls_sampling_interval_initialized;
#endif
...
{
    DCHECK(!g_tls_internal_reentry_guard.Get());
...
{
    return g_tls_internal_reentry_guard.Get();
}
...
PoissonAllocationSampler::ScopedMuteHookedSamplesForTesting::ScopedMuteHookedSamplesForTesting()
{
    ...
    accumulated_bytes_snapshot_ = g_tls_accumulated_bytes.Get();
    g_tls_accumulated_bytes = 0;
}
...
// static
void PoissonAllocationSampler::SetHooksInstallCallback(void (*hooks_install_callback)())
{
    // `g_hooks_install_callback` can't be used with
    // ScopedMuteHookedSamplesForTesting because there's no way to remove it.
    DCHECK(!g_mute_hooked_samples);
...
// static
void PoissonAllocationSampler::RecordAlloc(void* address, size_t size, AllocatorType type, const char* context)
{
    g_tls_accumulated_bytes += (intptr_t)size;
    intptr_t accumulated_bytes = g_tls_accumulated_bytes.Get();
...
void PoissonAllocationSampler::DoRecordAlloc(intptr_t accumulated_bytes, size_t size, void* address, AllocatorType type, const char* context)
{
    ...
    if (UNLIKELY(!g_tls_sampling_interval_initialized.Get())) {
