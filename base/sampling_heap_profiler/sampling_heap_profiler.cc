...
#if BUILDFLAG(IS_ANDROID) && BUILDFLAG(CAN_UNWIND_WITH_CFI_TABLE) && defined(OFFICIAL_BUILD)
...
#pragma GCC diagnostic push // #ifndef SUPPORT_XP_CODE
#pragma GCC diagnostic ignored "-Wexit-time-destructors"
#pragma GCC diagnostic ignored "-Wglobal-constructors"
    static SimpleTlsPointerXp<const char> thread_name;
...
[[maybe_unused]] StackUnwinder CheckForDefaultUnwindTables()
{
    ...
    size_t frame_count = base::debug::CollectStackTrace(const_cast<void**>(stack), kMaxStackEntries);
...
uint32_t SamplingHeapProfiler::Start()
{
    ...
    base::UmaHistogramCounts10M("HeapProfiling.SamplingIntervalKB", static_cast<int>(poisson_allocation_sampler->SamplingInterval() / 1024));
...
void** SamplingHeapProfiler::CaptureStackTrace(void** frames, size_t max_entries, size_t* count)
{
    ...
    case StackUnwinder::kCFIBacktrace:
        frame_count = base::trace_event::CFIBacktraceAndroid::GetInitializedInstance()->Unwind(const_cast<const void**>(frames), max_entries);
        break;
#endif
#if BUILDFLAG(CAN_UNWIND_WITH_FRAME_POINTERS)
    case StackUnwinder::kFramePointers:
        frame_count = base::debug::TraceStackFramePointers(const_cast<const void**>(frames), max_entries, skip_frames);
        skip_frames = 0;
        break;
#endif
...
void SamplingHeapProfiler::SampleAdded(void* address, size_t size, size_t total, PoissonAllocationSampler::AllocatorType type, const char* context)
{
    ...
    if (UNLIKELY(PoissonAllocationSampler::AreHookedSamplesMuted() && type != PoissonAllocationSampler::kManualForTesting)) {
