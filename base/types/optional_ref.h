...
public:
    ...
    template <typename U, typename = std::enable_if_t<std::is_const_v<T> && IsCompatibleV<U>>>
    // NOLINTNEXTLINE(google-explicit-constructor)
    constexpr optional_ref(const absl::optional<U>& o ABSL_ATTRIBUTE_LIFETIME_BOUND)
        : ptr_(o ? &*o : nullptr)
    {
    }
    ...
    template <typename U, typename = std::enable_if_t<IsCompatibleV<U>>>
    // NOLINTNEXTLINE(google-explicit-constructor)
    constexpr optional_ref(U* p ABSL_ATTRIBUTE_LIFETIME_BOUND)
        : ptr_(p)
    {
    }
    ...
    template <typename U = std::decay_t<T>, typename = std::enable_if_t<std::is_constructible_v<U, T>>> constexpr absl::optional<U> CopyAsOptional() const
    {
...
template <typename T> optional_ref(const absl::optional<T>&) -> optional_ref<const T>;
template <typename T> optional_ref(absl::optional<T>&) -> optional_ref<T>;
