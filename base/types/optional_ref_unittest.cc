...
static_assert(!std::is_constructible_v<optional_ref<int>, const absl::optional<int>&>);
...
{
    [](optional_ref<const TestClass> r) { EXPECT_CHECK_DEATH(r->ConstMethod()); }(absl::nullopt);
...
{
    [](optional_ref<const TestClass> r) { EXPECT_CHECK_DEATH(r.value()); }(absl::nullopt);

    [](optional_ref<TestClass> r) { EXPECT_CHECK_DEATH(r.value()); }(absl::nullopt);
}
...
TEST(OptionalRefTest, ClassTemplateArgumentDeduction)
{
    ...
    static_assert(std::is_same_v<decltype(optional_ref(absl::optional<int>())), optional_ref<const int>>);
