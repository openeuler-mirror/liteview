...
public:
    ...
    constexpr explicit ok(absl::in_place_t, std::initializer_list<U> il, Args&&... args) noexcept
...
public:
    ...
    template <typename Err = E, internal::EnableIfUnexpectedValueConstruction<E, Err> = 0>
    ...
    template <typename U, typename... Args>
    constexpr explicit unexpected(absl::in_place_t, std::initializer_list<U> il, Args&&... args) noexcept
...
template <typename T, typename E> class expected<T, E, /* is_void_v<T> = */ false> {
...
public:
    ...
    template <typename U, typename G, bool IsVoid> friend class expected;
    ...
    // corresponding type.
    template <typename U, typename G, internal::EnableIfExplicitConversion<T, E, const U&, const G&> = 0>
    explicit constexpr expected(const expected<U, G>& rhs) noexcept
        : impl_(rhs.impl_)
    {
    }

    template <typename U, typename G, internal::EnableIfImplicitConversion<T, E, const U&, const G&> = 0>
    ...
    template <typename U, typename G, internal::EnableIfExplicitConversion<T, E, U, G> = 0>
    explicit constexpr expected(expected<U, G>&& rhs) noexcept
        : impl_(std::move(rhs.impl_))
    {
    }

    template <typename U, typename G, internal::EnableIfImplicitConversion<T, E, U, G> = 0>
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename U = T, internal::EnableIfExplicitValueConstruction<T, E, U> = 0>
    explicit constexpr expected(U&& v) noexcept
        : impl_(kValTag, std::forward<U>(v))
    {
    }

    template <typename U = T, internal::EnableIfImplicitValueConstruction<T, E, U> = 0>
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename U, typename... Args>
    constexpr explicit expected(absl::in_place_t, std::initializer_list<U> il, Args&&... args) noexcept
    ...
    template <typename U = T, internal::EnableIfValueAssignment<T, E, U> = 0> constexpr expected& operator=(U&& v) noexcept
    {
    ...
    {
        static_assert(std::is_copy_constructible_v<T>, "expected<T, E>::value_or: T must be copy constructible");
        static_assert(std::is_convertible_v<U&&, T>, "expected<T, E>::value_or: U must be convertible to T");
        return has_value() ? value() : static_cast<T>(std::forward<U>(v));
    }

    template <typename U> constexpr T value_or(U&& v) && noexcept
    {
        static_assert(std::is_move_constructible_v<T>, "expected<T, E>::value_or: T must be move constructible");
        static_assert(std::is_convertible_v<U&&, T>, "expected<T, E>::value_or: U must be convertible to T");
...
template <typename T, typename E> class expected<T, E, /* is_void_v<T> = */ true> {
...
public:
    ...
    template <typename U, typename G, internal::EnableIfExplicitVoidConversion<E, U, const G&> = 0>
    constexpr explicit expected(const expected<U, G>& rhs) noexcept
        : impl_(rhs.impl_)
    {
    }

    template <typename U, typename G, internal::EnableIfImplicitVoidConversion<E, U, const G&> = 0>
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename U, typename G, internal::EnableIfExplicitVoidConversion<E, U, G> = 0>
    constexpr explicit expected(expected<U, G>&& rhs) noexcept
        : impl_(std::move(rhs.impl_))
    {
    }

    template <typename U, typename G, internal::EnableIfImplicitVoidConversion<E, U, G> = 0>
    // NOLINTNEXTLINE(google-explicit-constructor)
...
constexpr bool operator==(const expected<T, E, IsVoid>& x, const expected<U, G, IsVoid>& y) noexcept
{
    ...
    return x.has_value() == y.has_value() && (x.has_value() ? equal_values(x, y) : x.error() == y.error());
}

template <typename T, typename E, typename U, internal::EnableIfNotVoid<T> = 0> constexpr bool operator==(const expected<T, E>& x, const U& v) noexcept
{
    return x.has_value() && x.value() == v;
}

template <typename T, typename E, typename U, internal::EnableIfNotVoid<T> = 0> constexpr bool operator==(const U& v, const expected<T, E>& x) noexcept
{
    return x == v;
}

template <typename T, typename E, typename U, internal::EnableIfNotVoid<T> = 0> constexpr bool operator==(const expected<T, E>& x, const ok<U>& o) noexcept
{
    return x.has_value() && x.value() == o.value();
}

template <typename T, typename E, typename U, internal::EnableIfNotVoid<T> = 0> constexpr bool operator==(const ok<U>& o, const expected<T, E>& x) noexcept
{
    return x == o;
}
...
template <typename T, typename E, typename G> constexpr bool operator==(const unexpected<G>& e, const expected<T, E>& x) noexcept
{
    return x == e;
}
