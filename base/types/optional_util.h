...
template <class T> const T* OptionalToPtr(const absl::optional<T>& optional)
{
    return optional.has_value() ? &optional.value() : nullptr;
}

template <class T> T* OptionalToPtr(absl::optional<T>& optional)
{
    return optional.has_value() ? &optional.value() : nullptr;
}

// Helper for creating an `absl::optional<T>` from a `T*` which may be null.
template <class T> absl::optional<T> OptionalFromPtr(const T* value)
{
