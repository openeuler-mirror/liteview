...
TYPED_TEST(StrongAliasTest, ValueAccessesUnderlyingValue)
{
    ...
    static_assert(
        std::is_const<typename std::remove_reference<decltype(const_alias.value())>::type>::value, "Reference returned by const value getter should be const.");
}
...
TYPED_TEST(StrongAliasTest, CanBeMoveConstructed)
{
    ...
    static_assert(std::is_nothrow_move_constructible<FooAlias>::value, "Error: Alias is not nothow move constructible");
}
...
{
    using FooAlias = StrongAlias<class FooTag, TypeParam>;
    static_assert(std::is_default_constructible<FooAlias>::value, "Should be possible to default-construct a StrongAlias.");
    static_assert(std::is_trivially_default_constructible<FooAlias>::value == std::is_trivially_default_constructible<TypeParam>::value,
...
TEST(StrongAliasTest, TrivialTypeAliasIsStandardLayout)
{
    ...
    static_assert(std::is_standard_layout<FooAlias>::value, "int-based alias should have standard layout. ");
    static_assert(std::is_trivially_copyable<FooAlias>::value, "int-based alias should be trivially copyable. ");
}
...
TYPED_TEST(StrongAliasTest, CannotBeCreatedFromDifferentAlias)
{
    ...
    static_assert(!std::is_constructible<FooAlias, BarAlias>::value, "Should be impossible to construct FooAlias from a BarAlias.");
    static_assert(!std::is_convertible<BarAlias, FooAlias>::value, "Should be impossible to convert a BarAlias into FooAlias.");
}
...
void StreamOperatorExists()
{
    ...
    static_assert(!base::internal::SupportsOstreamOperator<NonStreamableAlias>::value, "");
}
