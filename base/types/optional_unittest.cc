...
static_assert(std::is_trivially_destructible<absl::optional<int>>::value, "OptionalIsTriviallyDestructible");

static_assert(!std::is_trivially_destructible<absl::optional<NonTriviallyDestructible>>::value, "OptionalIsTriviallyDestructible");
...
TEST(OptionalTest, MoveConstructor)
{
    ...
    {
        absl::optional<NonTriviallyDestructibleDeletedCopyConstructor> first(absl::in_place, 42);
        absl::optional<NonTriviallyDestructibleDeletedCopyConstructor> second(std::move(first));
...
TEST(OptionalTest, ConstructorForwardInitListAndArguments)
{
    ...
    {
        absl::optional<std::vector<int>> opt(absl::in_place, { 3, 1 }, std::allocator<int>());
        EXPECT_TRUE(opt);
        EXPECT_THAT(*opt, ElementsAre(3, 1));
        EXPECT_EQ(2u, opt->size());
    }
...
TEST(OptionalTest, ForwardConstructor)
{
    ...
    // absl::optional<T>(absl::optional<U>) constructor.
    {
        ...
        struct Test {
            ...
            explicit Test(absl::in_place_t param)
...

    {
        ...
        static_assert(std::is_convertible<int, Test>::value, "Int should be convertible to Test.");
...
TEST(OptionalTest, OperatorStar_rvalue)
{
    ...
    EXPECT_TRUE(TestObject(3, 0.1) == *absl::optional<TestObject>(TestObject(3, 0.1)));
}
...
TEST(OptionalTest, Value_rvalue)
{
    ...
    EXPECT_TRUE(TestObject(3, 0.1) == absl::optional<TestObject>(TestObject(3, 0.1)).value());
}
...
TEST(OptionalTest, MakeOptional)
{
...

    {
        ...
        EXPECT_EQ(TestObject::State::MOVE_CONSTRUCTED, absl::make_optional(std::move(value))->state());
    }
...

    {
        ...
        auto str2 = absl::make_optional<std::string>({ 'a', 'b', 'c' }, std::allocator<char>());
        EXPECT_EQ("abc", *str2);
    }
}
...
TEST(OptionalTest, Hash_OptionalReflectsInternal)
{
...

    {
        ...
        EXPECT_EQ(str_hash(std::string("foobar")), opt_str_hash(absl::optional<std::string>(std::string("foobar"))));
    }
}
...
TEST(OptionalTest, Hash_NullOptEqualsNullOpt)
{
    ...
    EXPECT_EQ(opt_str_hash(absl::optional<std::string>()), opt_int_hash(absl::optional<int>()));
}
...
TEST(OptionalTest, Noexcept)
{
    ...
    static_assert(noexcept(absl::optional<int>(std::declval<absl::optional<int>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(trivial copy, trivial move)");
    static_assert(!noexcept(absl::optional<Test1>(std::declval<absl::optional<Test1>>())),
        "move constructor for non-noexcept move-constructible T must not be "
        "noexcept (trivial copy)");
    static_assert(noexcept(absl::optional<Test2>(std::declval<absl::optional<Test2>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(non-trivial copy, trivial move)");
    static_assert(noexcept(absl::optional<Test3>(std::declval<absl::optional<Test3>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(trivial copy, non-trivial move)");
    static_assert(noexcept(absl::optional<Test4>(std::declval<absl::optional<Test4>>())),
        "move constructor for noexcept move-constructible T must be noexcept "
        "(non-trivial copy, non-trivial move)");
    static_assert(!noexcept(absl::optional<Test5>(std::declval<absl::optional<Test5>>())),
        "move constructor for non-noexcept move-constructible T must not be "
        "noexcept (non-trivial copy)");

    static_assert(noexcept(std::declval<absl::optional<int>>() = std::declval<absl::optional<int>>()),
        "move assign for noexcept move-constructible/move-assignable T "
        "must be noexcept");
    static_assert(!noexcept(std::declval<absl::optional<Test1>>() = std::declval<absl::optional<Test1>>()),
        "move assign for non-noexcept move-constructible T must not be noexcept");
    static_assert(!noexcept(std::declval<absl::optional<Test2>>() = std::declval<absl::optional<Test2>>()),
...
TEST(OptionalTest, OverrideAddressOf)
{
    ...
    static_assert(std::is_trivially_destructible<TriviallyDestructibleOverloadAddressOf>::value, "Trivially...AddressOf must be trivially destructible.");
    ...
    static_assert(
        !std::is_trivially_destructible<NonTriviallyDestructibleOverloadAddressOf>::value, "NotTrivially...AddressOf must not be trivially destructible.");
