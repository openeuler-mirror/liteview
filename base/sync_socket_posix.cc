...
size_t SendHelper(SyncSocket::Handle handle, const void* buffer, size_t length)
{
    DCHECK_GT(length, 0u);
    DCHECK_LE(length, kMaxMessageLength);
    DCHECK_NE(handle, SyncSocket::kInvalidHandle);
    return WriteFileDescriptor(handle, make_span(static_cast<const uint8_t*>(buffer), length)) ? length : 0;
}
