...
extern "C" long _InterlockedExchange(long volatile* _Target, long _Value);
#endif
...
public:
    ...
    {
        do {
            volatile long old_var = _InterlockedCompareExchange((long volatile*)&(run_once_), 2, 0);
            if (0 == old_var) {
                new (storage_) T(std::forward<Args>(args)...);
                _InterlockedExchange((long volatile*)&(run_once_), 2);
                break;
            } else if (2 == old_var) {
                continue;
            } else {
