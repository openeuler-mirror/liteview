...
{
    UnguessableToken token = UnguessableToken::Deserialize(0x1234567890ABCDEF, 0xFEDCBA0987654321);
...
{
    const UnguessableToken token1 = UnguessableToken::Deserialize(0x0000000012345678, 0x0000000123456789);
    const UnguessableToken token2 = UnguessableToken::Deserialize(0x0000000123456781, 0x0000000023456789);
...
TEST(UnguessableTokenTest, VerifySmallerThanOperator)
{
    ...
    // Use UnguessableToken::Create() in production code instead.
    {
        ...
        TestSmallerThanOperator(UnguessableToken::Deserialize(0, 1), UnguessableToken::Deserialize(0, 5));
    }
    {
        SCOPED_TRACE("a.low == b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 0), UnguessableToken::Deserialize(5, 0));
    }
    {
        SCOPED_TRACE("a.low < b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 1), UnguessableToken::Deserialize(5, 5));
    }
    {
        SCOPED_TRACE("a.low > b.low and a.high < b.high.");
        TestSmallerThanOperator(UnguessableToken::Deserialize(1, 10), UnguessableToken::Deserialize(10, 1));
    }
}
