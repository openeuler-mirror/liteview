...
{
    return [&pred, &proj](auto&& arg) -> bool { return base::invoke(pred, base::invoke(proj, std::forward<decltype(arg)>(arg))); };
}
...
private:
    ...
    using InvokeResult = std::invoke_result_t<Pred&, std::invoke_result_t<ProjT&, T&&>, std::invoke_result_t<ProjU&, U&&>>;

    template <typename T, typename U, typename = InvokeResult<Proj1, Proj2, T, U>> constexpr std::pair<Proj1&, Proj2&> GetProjs(priority_tag<3>) const
    {
        return { proj1_, proj2_ };
    }

    template <typename T, typename U, bool LazyPermute = kPermute, typename = std::enable_if_t<LazyPermute>, typename = InvokeResult<Proj2, Proj1, T, U>>
    constexpr std::pair<Proj2&, Proj1&> GetProjs(priority_tag<2>) const
    {
        return { proj2_, proj1_ };
    }

    template <typename T, typename U, bool LazyPermute = kPermute, typename = std::enable_if_t<LazyPermute>, typename = InvokeResult<Proj1, Proj1, T, U>>
    constexpr std::pair<Proj1&, Proj1&> GetProjs(priority_tag<1>) const
    {
        return { proj1_, proj1_ };
    }

    template <typename T, typename U, bool LazyPermute = kPermute, typename = std::enable_if_t<LazyPermute>, typename = InvokeResult<Proj2, Proj2, T, U>>
...
public:
    template <typename T, typename U> constexpr bool operator()(T&& lhs, U&& rhs) const
    {
        ...
        return base::invoke(pred_, base::invoke(projs.first, std::forward<T>(lhs)), base::invoke(projs.second, std::forward<U>(rhs)));
    }
...
template <typename I, typename F> struct in_fun_result {
    NO_UNIQUE_ADDRESS I in;
    NO_UNIQUE_ADDRESS F fun;

    template <typename I2, typename F2, std::enable_if_t<std::is_convertible<const I&, I2> {} && std::is_convertible<const F&, F2> {}>>
    constexpr operator in_fun_result<I2, F2>() const&
    {
        return { in, fun };
    }

    template <typename I2, typename F2, std::enable_if_t<std::is_convertible<I, I2> {} && std::is_convertible<F, F2> {}>>
...
template <typename InputIterator, typename Pred, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>>
...
template <typename Range, typename Pred, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr bool all_of(Range&& range, Pred pred, Proj proj = {})
{
    return ranges::all_of(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
{
    return ranges::any_of(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
{
    return ranges::none_of(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename I, typename F> using for_each_result = in_fun_result<I, F>;
...
template <typename InputIterator, typename Fun, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>>
...
template <typename Range, typename Fun, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto for_each(Range&& range, Fun f, Proj proj = {})
{
    return ranges::for_each(ranges::begin(range), ranges::end(range), std::move(f), std::move(proj));
}

// Reference: https://wg21.link/algorithm.syn#:~:text=for_each_n_result
template <typename I, typename F> using for_each_n_result = in_fun_result<I, F>;
...
template <typename InputIterator, typename Size, typename Fun, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>>
...
template <typename InputIterator, typename T, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>>
...
template <typename Range, typename T, typename Proj = identity, typename = internal::range_category_t<Range>>
...
{
    return ranges::find_if(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
{
    return ranges::find_if_not(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename Pred = ranges::equal_to, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = indirect_result_t<Pred&, projected<ForwardIterator1, Proj1>, projected<ForwardIterator2, Proj2>>>
...
template <typename Range1, typename Range2, typename Pred = ranges::equal_to, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = indirect_result_t<Pred&, projected<iterator_t<Range1>, Proj1>, projected<iterator_t<Range2>, Proj2>>>
constexpr auto find_end(Range1&& range1, Range2&& range2, Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::find_end(
        ranges::begin(range1), ranges::end(range1), ranges::begin(range2), ranges::end(range2), std::move(pred), std::move(proj1), std::move(proj2));
}
...
template <typename ForwardIterator, typename Pred = ranges::equal_to, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>>
...
template <typename Range, typename Pred = ranges::equal_to, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto adjacent_find(Range&& range, Pred pred = {}, Proj proj = {})
{
    return ranges::adjacent_find(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
constexpr auto count(InputIterator first, InputIterator last, const T& value, Proj proj = {})
{
    ...
    return std::count_if(first, last, [&proj, &value](auto&& lhs) { return base::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
{
    return ranges::count_if(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator, typename Size, typename T, typename Pred = ranges::equal_to, typename Proj = identity,
...
template <typename Range, typename Size, typename T, typename Pred = ranges::equal_to, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto search_n(Range&& range, Size count, const T& value, Pred pred = {}, Proj proj = {})
{
    return ranges::search_n(ranges::begin(range), ranges::end(range), count, value, std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename Pred, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
...
template <typename Range, typename OutputIterator, typename Pred, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto copy_if(Range&& range, OutputIterator result, Pred pred, Proj proj = {})
{
    return ranges::copy_if(ranges::begin(range), ranges::end(range), result, std::move(pred), std::move(proj));
}
...
{
    return ranges::swap_ranges(ranges::begin(range1), ranges::end(range1), ranges::begin(range2), ranges::end(range2));
}
...
template <typename InputIterator, typename OutputIterator, typename UnaryOperation, typename Proj = identity,
...
{
    return std::transform(first1, last1, result, [&op, &proj](auto&& arg) { return base::invoke(op, base::invoke(proj, std::forward<decltype(arg)>(arg))); });
}
...
template <typename Range, typename OutputIterator, typename UnaryOperation, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>, typename = indirect_result_t<UnaryOperation&, projected<iterator_t<Range>, Proj>>>
constexpr auto transform(Range&& range, OutputIterator result, UnaryOperation op, Proj proj = {})
{
    return ranges::transform(ranges::begin(range), ranges::end(range), result, std::move(op), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename OutputIterator, typename BinaryOperation, typename Proj1 = identity,
    typename Proj2 = identity, typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = internal::iterator_category_t<OutputIterator>,
    typename = indirect_result_t<BinaryOperation&, projected<ForwardIterator1, Proj1>, projected<ForwardIterator2, Proj2>>>
    ...
    BinaryOperation binary_op, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    ...
    return std::transform(first1, last1, first2, result, [&binary_op, &proj1, &proj2](auto&& lhs, auto&& rhs) {
        return base::invoke(binary_op, base::invoke(proj1, std::forward<decltype(lhs)>(lhs)), base::invoke(proj2, std::forward<decltype(rhs)>(rhs)));
    });
}
...
template <typename Range1, typename Range2, typename OutputIterator, typename BinaryOperation, typename Proj1 = identity, typename Proj2 = identity,
    ...
    typename = indirect_result_t<BinaryOperation&, projected<iterator_t<Range1>, Proj1>, projected<iterator_t<Range2>, Proj2>>>
...
{
    return ranges::transform(ranges::begin(range1), ranges::end(range1), ranges::begin(range2), ranges::end(range2), result, std::move(binary_op),
...
template <typename ForwardIterator, typename T, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>>
constexpr auto replace(ForwardIterator first, ForwardIterator last, const T& old_value, const T& new_value, Proj proj = {})
{
    ...
    std::replace_if(
        first, last, [&proj, &old_value](auto&& lhs) { return base::invoke(proj, std::forward<decltype(lhs)>(lhs)) == old_value; }, new_value);
    return last;
}
...
{
    return ranges::replace(ranges::begin(range), ranges::end(range), old_value, new_value, std::move(proj));
}
...
template <typename ForwardIterator, typename Predicate, typename T, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>>
...
template <typename Range, typename Predicate, typename T, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto replace_if(Range&& range, Predicate pred, const T& new_value, Proj proj = {})
{
    return ranges::replace_if(ranges::begin(range), ranges::end(range), std::move(pred), new_value, std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename T, typename Proj = identity, typename = internal::iterator_category_t<InputIterator>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy(InputIterator first, InputIterator last, OutputIterator result, const T& old_value, const T& new_value, Proj proj = {})
{
    ...
    std::replace_copy_if(
        first, last, result, [&proj, &old_value](auto&& lhs) { return base::invoke(proj, std::forward<decltype(lhs)>(lhs)) == old_value; }, new_value);
    return last;
}
...
template <typename Range, typename OutputIterator, typename T, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy(Range&& range, OutputIterator result, const T& old_value, const T& new_value, Proj proj = {})
{
    return ranges::replace_copy(ranges::begin(range), ranges::end(range), result, old_value, new_value, std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator, typename Predicate, typename T, typename Proj = identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<OutputIterator>>
...
template <typename Range, typename OutputIterator, typename Predicate, typename T, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto replace_copy_if(Range&& range, OutputIterator result, Predicate pred, const T& new_value, Proj proj = {})
{
    return ranges::replace_copy_if(ranges::begin(range), ranges::end(range), result, pred, new_value, std::move(proj));
}
...
constexpr auto remove(ForwardIterator first, ForwardIterator last, const T& value, Proj proj = {})
{
    ...
    return std::remove_if(first, last, [&proj, &value](auto&& lhs) { return base::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
template <typename ForwardIterator, typename Predicate, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>>
...
template <typename Range, typename Predicate, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto remove_if(Range&& range, Predicate pred, Proj proj = {})
{
    return ranges::remove_if(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
constexpr auto remove_copy(InputIterator first, InputIterator last, OutputIterator result, const T& value, Proj proj = {})
{
    ...
    return std::remove_copy_if(first, last, result, [&proj, &value](auto&& lhs) { return base::invoke(proj, std::forward<decltype(lhs)>(lhs)) == value; });
}
...
{
    return ranges::remove_copy(ranges::begin(range), ranges::end(range), result, value, std::move(proj));
}
...
{
    return ranges::remove_copy_if(ranges::begin(range), ranges::end(range), result, std::move(pred), std::move(proj));
}
...
template <typename ForwardIterator, typename Comp = ranges::equal_to, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = indirect_result_t<Comp&, projected<ForwardIterator, Proj>, projected<ForwardIterator, Proj>>>
...
template <typename Range, typename Comp = ranges::equal_to, typename Proj = identity, typename = internal::range_category_t<Range>,
...
{
    return ranges::unique(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename OutputIterator, typename Comp = ranges::equal_to, typename Proj = identity,
...
template <typename Range, typename OutputIterator, typename Comp = ranges::equal_to, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = internal::iterator_category_t<OutputIterator>>
constexpr auto unique_copy(Range&& range, OutputIterator result, Comp comp = {}, Proj proj = {})
{
    return ranges::unique_copy(ranges::begin(range), ranges::end(range), result, std::move(comp), std::move(proj));
}
...
{
    return ranges::shuffle(ranges::begin(range), ranges::end(range), std::forward<UniformRandomBitGenerator>(g));
}
...
template <typename RandomAccessIterator, typename Comp = ranges::less, typename Proj = identity, typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = indirect_result_t<Comp&, projected<RandomAccessIterator, Proj>, projected<RandomAccessIterator, Proj>>>
...
template <typename Range, typename Comp = ranges::less, typename Proj = identity, typename = internal::range_category_t<Range>,
    typename = indirect_result_t<Comp&, projected<iterator_t<Range>, Proj>, projected<iterator_t<Range>, Proj>>>
...
{
    return ranges::stable_sort(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::partial_sort(ranges::begin(range), middle, ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename InputIterator, typename RandomAccessIterator, typename Comp = ranges::less, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::iterator_category_t<InputIterator>, typename = internal::iterator_category_t<RandomAccessIterator>,
    typename = indirect_result_t<Comp&, projected<InputIterator, Proj1>, projected<RandomAccessIterator, Proj2>>,
    typename = indirect_result_t<Comp&, projected<RandomAccessIterator, Proj2>, projected<InputIterator, Proj1>>>
...
template <typename Range1, typename Range2, typename Comp = ranges::less, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>,
    typename = indirect_result_t<Comp&, projected<iterator_t<Range1>, Proj1>, projected<iterator_t<Range2>, Proj2>>,
    typename = indirect_result_t<Comp&, projected<iterator_t<Range2>, Proj2>, projected<iterator_t<Range1>, Proj1>>>
constexpr auto partial_sort_copy(Range1&& range, Range2&& result_range, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::partial_sort_copy(
        ranges::begin(range), ranges::end(range), ranges::begin(result_range), ranges::end(result_range), std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename ForwardIterator, typename Comp = ranges::less, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>,
    typename = indirect_result_t<Comp&, projected<ForwardIterator, Proj>, projected<ForwardIterator, Proj>>>
...
{
    return ranges::is_sorted_until(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::is_sorted(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::nth_element(ranges::begin(range), nth, ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename T, typename Comp = ranges::less, typename Proj = identity,
    typename = internal::iterator_category_t<ForwardIterator>>
...
template <typename Range, typename T, typename Comp = ranges::less, typename Proj = identity, typename = internal::range_category_t<Range>>
constexpr auto lower_bound(Range&& range, const T& value, Comp comp = {}, Proj proj = {})
{
    return ranges::lower_bound(ranges::begin(range), ranges::end(range), value, std::move(comp), std::move(proj));
}
...
{
    return ranges::upper_bound(ranges::begin(range), ranges::end(range), value, std::move(comp), std::move(proj));
}
...
{
    return ranges::equal_range(ranges::begin(range), ranges::end(range), value, std::move(comp), std::move(proj));
}
...
{
    return ranges::binary_search(ranges::begin(range), ranges::end(range), value, std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator, typename Pred, typename Proj = identity, typename = internal::iterator_category_t<ForwardIterator>>
...
{
    return ranges::is_partitioned(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
{
    return ranges::partition(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename BidirectionalIterator, typename Pred, typename Proj = identity, typename = internal::iterator_category_t<BidirectionalIterator>>
...
{
    return ranges::stable_partition(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator, typename OutputIterator1, typename OutputIterator2, typename Pred, typename Proj = identity,
...
template <typename Range, typename OutputIterator1, typename OutputIterator2, typename Pred, typename Proj = identity,
...
{
    return ranges::partition_copy(ranges::begin(range), ranges::end(range), out_true, out_false, std::move(pred), std::move(proj));
}
...
{
    return ranges::partition_point(ranges::begin(range), ranges::end(range), std::move(pred), std::move(proj));
}
...
template <typename InputIterator1, typename InputIterator2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = identity,
    typename Proj2 = identity, typename = internal::iterator_category_t<InputIterator1>, typename = internal::iterator_category_t<InputIterator2>,
    typename = internal::iterator_category_t<OutputIterator>,
    typename = indirect_result_t<Comp&, projected<InputIterator1, Proj1>, projected<InputIterator2, Proj2>>,
    typename = indirect_result_t<Comp&, projected<InputIterator2, Proj2>, projected<InputIterator1, Proj1>>>
...
template <typename Range1, typename Range2, typename OutputIterator, typename Comp = ranges::less, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::range_category_t<Range1>, typename = internal::range_category_t<Range2>, typename = internal::iterator_category_t<OutputIterator>,
    typename = indirect_result_t<Comp&, projected<iterator_t<Range1>, Proj1>, projected<iterator_t<Range2>, Proj2>>,
    typename = indirect_result_t<Comp&, projected<iterator_t<Range2>, Proj2>, projected<iterator_t<Range1>, Proj1>>>
...
{
    return ranges::merge(
        ranges::begin(range1), ranges::end(range1), ranges::begin(range2), ranges::end(range2), result, std::move(comp), std::move(proj1), std::move(proj2));
}
...
template <typename BidirectionalIterator, typename Comp = ranges::less, typename Proj = identity,
    typename = internal::iterator_category_t<BidirectionalIterator>>
...
{
    return ranges::inplace_merge(ranges::begin(range), middle, ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename InputIterator1, typename InputIterator2, typename Comp = ranges::less, typename Proj1 = identity, typename Proj2 = identity,
...
constexpr auto includes(Range1&& range1, Range2&& range2, Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {})
{
    return ranges::includes(
        ranges::begin(range1), ranges::end(range1), ranges::begin(range2), ranges::end(range2), std::move(comp), std::move(proj1), std::move(proj2));
}
...
{
    return ranges::push_heap(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::pop_heap(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::make_heap(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::sort_heap(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::is_heap(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::is_heap_until(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = identity> constexpr const T& min(const T& a, const T& b, Comp comp = {}, Proj proj = {})
{
...
template <typename T, typename Comp = ranges::less, typename Proj = identity> constexpr T min(std::initializer_list<T> ilist, Comp comp = {}, Proj proj = {})
{
...
{
    return *std::min_element(ranges::begin(range), ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = identity> constexpr const T& max(const T& a, const T& b, Comp comp = {}, Proj proj = {})
{
...
template <typename T, typename Comp = ranges::less, typename Proj = identity> constexpr T max(std::initializer_list<T> ilist, Comp comp = {}, Proj proj = {})
{
...
{
    return *std::max_element(ranges::begin(range), ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
}
...
template <typename T, typename Comp = ranges::less, typename Proj = identity> constexpr auto minmax(const T& a, const T& b, Comp comp = {}, Proj proj = {})
{
...
{
    auto it = std::minmax_element(ranges::begin(ilist), ranges::end(ilist), internal::ProjectedBinaryPredicate(comp, proj, proj));
...
constexpr auto minmax(Range&& range, Comp comp = {}, Proj proj = {})
{
    ...
    auto it = std::minmax_element(ranges::begin(range), ranges::end(range), internal::ProjectedBinaryPredicate(comp, proj, proj));
    return std::pair<T, T> { *it.first, *it.second };
}
...
{
    return ranges::min_element(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::max_element(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::minmax_element(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
template <typename ForwardIterator1, typename ForwardIterator2, typename Comp = ranges::less, typename Proj1 = identity, typename Proj2 = identity,
    typename = internal::iterator_category_t<ForwardIterator1>, typename = internal::iterator_category_t<ForwardIterator2>,
    typename = indirect_result_t<Comp&, projected<ForwardIterator1, Proj1>, projected<ForwardIterator2, Proj2>>,
    typename = indirect_result_t<Comp&, projected<ForwardIterator2, Proj2>, projected<ForwardIterator1, Proj1>>>
...
template <typename BidirectionalIterator, typename Comp = ranges::less, typename Proj = identity,
    typename = internal::iterator_category_t<BidirectionalIterator>,
    typename = indirect_result_t<Comp&, projected<BidirectionalIterator, Proj>, projected<BidirectionalIterator, Proj>>>
...
{
    return ranges::next_permutation(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
...
{
    return ranges::prev_permutation(ranges::begin(range), ranges::end(range), std::move(comp), std::move(proj));
}
