...
TEST(SHA1Test, Test1)
{
    ...
    static constexpr int kExpected[]
...
TEST(SHA1Test, Test1BytesAndSpan)
{
    ...
    base::SHA1HashBytes(reinterpret_cast<const unsigned char*>(input.c_str()), input.size(), output);
    for (size_t i = 0; i < base::kSHA1Length; i++)
        EXPECT_EQ(kExpected[i], output[i]);

    base::SHA1Digest output_array = base::SHA1HashSpan(base::as_bytes(base::make_span(input)));
    for (size_t i = 0; i < base::kSHA1Length; i++)
