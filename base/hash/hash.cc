...
size_t FastHashImpl(base::span<const uint8_t> data)
{
    ...
    if constexpr (sizeof(size_t) > 4) {
        return base::internal::cityhash_v111::CityHash64(reinterpret_cast<const char*>(data.data()), data.size());
    } else {
        return base::internal::cityhash_v111::CityHash32(reinterpret_cast<const char*>(data.data()), data.size());
    }
}
...
uint32_t PersistentHash(span<const uint8_t> data)
{
    ...
    return ::SuperFastHash(reinterpret_cast<const char*>(data.data()), static_cast<int>(data.size()));
}
