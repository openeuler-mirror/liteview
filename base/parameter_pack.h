...
template <typename... Ts> struct ParameterPack {
    ...
    template <typename Type> using HasType = std::bool_constant<any_of({ std::is_same<Type, Ts>::value... })>;

    // Checks if the parameter pack only contains |Type|.
    template <typename Type> using OnlyHasType = std::bool_constant<all_of({ std::is_same<Type, Ts>::value... })>;

    // Checks if |Type| occurs only once in the parameter pack.
    template <typename Type> using IsUniqueInPack = std::bool_constant<count({ std::is_same<Type, Ts>::value... }, true) == 1>;
    ...
    using IsAllSameType = std::bool_constant<all_of({ std::is_same<NthType<0>, Ts>::value... })>;
};
