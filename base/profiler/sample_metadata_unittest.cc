...
TEST(SampleMetadataTest, ScopedSampleMetadata)
{
    ...
    ASSERT_EQ(0u, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items));

    {
        ScopedSampleMetadata m("myname", 100, SampleMetadataScope::kProcess);

        ASSERT_EQ(1u, MetadataRecorder::MetadataProvider(GetSampleMetadataRecorder(), PlatformThread::CurrentId()).GetItems(&items));
