...
public:
    StackCopierDelegate(const base::circular_deque<std::unique_ptr<Unwinder>>* unwinders, ProfileBuilder* profile_builder,
...
StackSamplerImpl::StackSamplerImpl(std::unique_ptr<StackCopier> stack_copier, UnwindersFactory core_unwinders_factory, ModuleCache* module_cache,
...
{
    std::vector<std::unique_ptr<Unwinder>> unwinders = std::move(unwinders_factory_).Run();
...
void StackSamplerImpl::RecordStackFrames(StackBuffer* stack_buffer, ProfileBuilder* profile_builder, PlatformThreadId thread_id)
{
    DCHECK(stack_buffer);
    ...
    bool copy_stack_succeeded;
    {
        ...
        StackCopierDelegate delegate(&unwinders_, profile_builder, &metadata_provider);
    ...
    profile_builder->OnSampleCompleted(WalkStack(module_cache_, &thread_context, stack_top, unwinders_), timestamp);
    ...
    if ((++stack_size_histogram_sampling_counter_ % kUMAHistogramDownsampleAmount) == 0) {
        ...
        UmaHistogramMemoryKB("Memory.StackSamplingProfiler.StackSampleSize", saturated_cast<int>(stack_size / kBytesPerKilobyte));
    }
...
std::vector<Frame> StackSamplerImpl::WalkStackForTesting(
    ModuleCache* module_cache, RegisterContext* thread_context, uintptr_t stack_top, const base::circular_deque<std::unique_ptr<Unwinder>>& unwinders)
{
    ...
    ModuleCache* module_cache, RegisterContext* thread_context, uintptr_t stack_top, const base::circular_deque<std::unique_ptr<Unwinder>>& unwinders)
{
    ...
    do {
        ...
        auto unwinder = ranges::find_if(unwinders, [&stack](const std::unique_ptr<Unwinder>& unwinder) { return unwinder->CanUnwindFrom(stack.back()); });
        ...
        DCHECK(result != UnwindResult::kCompleted || unwinder->get() == unwinders.back().get());
