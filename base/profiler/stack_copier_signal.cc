...
public:
    ...
    {
        futex_.store(0, std::memory_order_release);
    }
    ~AsyncSafeWaitableEvent()
    {
    }
    ...
    bool Wait()
    {
        ...
        while (true) {
            long res = syscall(SYS_futex, futex_int_ptr(), FUTEX_WAIT | FUTEX_PRIVATE_FLAG, 0, nullptr, nullptr, 0);
    ...
    void Signal()
    {
        ...
        syscall(SYS_futex, futex_int_ptr(), FUTEX_WAKE | FUTEX_PRIVATE_FLAG, 1, nullptr, nullptr, 0);
    }
...
private:
    ...
    int* futex_int_ptr()
    {
        static_assert(sizeof(futex_) == sizeof(int), "Expected std::atomic_int to be the same size as int");
