...
public:
    ...
    void ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash, absl::optional<int64_t> key, int64_t value,
...
private:
    ...
    void ApplyMetadataToPastSamplesTask(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash, absl::optional<int64_t> key,
        int64_t value, absl::optional<PlatformThreadId> thread_id);
...
void StackSamplingProfiler::SamplingThread::ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash,
    absl::optional<int64_t> key, int64_t value, absl::optional<PlatformThreadId> thread_id)
{
    ThreadExecutionState state;
    scoped_refptr<SingleThreadTaskRunner> task_runner = GetTaskRunner(&state);
    if (state != RUNNING)
...
void StackSamplingProfiler::SamplingThread::Remove(int collection_id)
{
    ...
    task_runner->PostTask(FROM_HERE, BindOnce(&SamplingThread::RemoveCollectionTask, Unretained(this), collection_id));
}
...
void StackSamplingProfiler::SamplingThread::FinishCollection(std::unique_ptr<CollectionContext> collection)
{
    ...
    collection->profile_builder->OnProfileCompleted(profile_duration, collection->params.sampling_interval);
    ...
    absl::optional<int64_t> key, int64_t value, absl::optional<PlatformThreadId> thread_id)
{
    ...
    for (auto& id_collection_pair : active_collections_) {
        if (thread_id && id_collection_pair.second->thread_id != thread_id)
            continue;
        id_collection_pair.second->profile_builder->ApplyMetadataRetrospectively(period_start, period_end, item);
    }
}
...
void StackSamplingProfiler::SamplingThread::RecordSampleTask(int collection_id)
{
    ...
    collection->sampler->RecordStackFrames(stack_buffer_.get(), collection->profile_builder.get(), collection->thread_id);
    ...
    std::unique_ptr<CollectionContext> owned_collection = std::move(found->second);
    size_t count = active_collections_.erase(collection_id);
    DCHECK_EQ(1U, count);
...
StackSamplingProfiler::StackSamplingProfiler(SamplingProfilerThreadToken thread_token, const SamplingParams& params,
    ...
    : StackSamplingProfiler(thread_token, params, std::move(profile_builder), std::unique_ptr<StackSampler>())
{
    sampler_ = StackSampler::Create(
        thread_token, profile_builder_->GetModuleCache(), std::move(core_unwinders_factory), std::move(record_sample_callback), test_delegate);
}
...
void StackSamplingProfiler::Start()
{
    ...
    profiler_id_ = SamplingThread::GetInstance()->Add(
        std::make_unique<SamplingThread::CollectionContext>(thread_token_.id, params_, &profiling_inactive_, std::move(sampler_), std::move(profile_builder_)));
...
// static
void StackSamplingProfiler::ApplyMetadataToPastSamples(base::TimeTicks period_start, base::TimeTicks period_end, uint64_t name_hash,
    absl::optional<int64_t> key, int64_t value, absl::optional<PlatformThreadId> thread_id)
{
