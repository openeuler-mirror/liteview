...
bool operator==(const ArmCFITable::FrameEntry& a, const ArmCFITable::FrameEntry& b)
{
...
{
    auto parse_cfi = [](std::vector<uint16_t> data) { return ArmCFITable::Parse({ reinterpret_cast<const uint8_t*>(data.data()), data.size() * 2 }); };
...
TEST(ArmCFITableTest, FindEntryForAddress)
{
    ...
    auto reader = ArmCFITable::Parse({ reinterpret_cast<const uint8_t*>(input_data), sizeof(input_data) * 2 });
...
{
    auto parse_cfi_and_find = [](std::vector<uint16_t> data, uintptr_t address) -> absl::optional<ArmCFITable::FrameEntry> {
        auto reader = ArmCFITable::Parse({ reinterpret_cast<const uint8_t*>(data.data()), data.size() * 2 });
        if (!reader)
            return absl::nullopt;
