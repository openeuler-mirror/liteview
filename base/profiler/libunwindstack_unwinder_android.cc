...
{
#if defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_32_BITS)
    return absl::WrapUnique<unwindstack::Regs>(unwindstack::RegsArm::Read(reinterpret_cast<void*>(&thread_context->arm_r0)));
#elif defined(ARCH_CPU_ARM_FAMILY) && defined(ARCH_CPU_64_BITS)
    return absl::WrapUnique<unwindstack::Regs>(unwindstack::RegsArm64::Read(reinterpret_cast<void*>(&thread_context->regs[0])));
...
LibunwindstackUnwinderAndroid::LibunwindstackUnwinderAndroid()
    ...
    , process_memory_(std::shared_ptr<unwindstack::Memory>(NativeUnwinderAndroid::CreateProcessMemory().release()))
{
}
...
{
    if (!jit_debug_) {
        jit_debug_ = unwindstack::CreateJitDebug(arch, process_memory_, search_libs_);
    }
...
{
    if (!dex_files_) {
        dex_files_ = unwindstack::CreateDexFiles(arch, process_memory_, search_libs_);
    }
    return dex_files_.get();
}

UnwindResult LibunwindstackUnwinderAndroid::TryUnwind(RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* stack)
{
    ...
    auto attempt_unwind = [&]() {
        ...
        DCHECK(regs);
        unwindstack::Unwinder unwinder(kMaxFrames, memory_regions_map_.get(), regs.get(), process_memory_);
        ...
        return UnwindValues { unwinder.LastErrorCode(), /*unwinder.warnings()*/ 0, unwinder.ConsumeFrames() };
    };
    ...
    bool should_retry = values.error_code == unwindstack::ERROR_INVALID_MAP && samples_since_last_maps_parse_ > kMinSamplesBeforeNextMapsParse;
    if (should_retry) {
        TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "TryUnwind Reparsing Maps");
    ...
    if (values.error_code == unwindstack::ERROR_NONE) {
        for (const unwindstack::FrameData& frame : values.frames) {
            stack->emplace_back(frame.pc, module_cache()->GetModuleForAddress(frame.pc), frame.function_name);
        }
        return UnwindResult::kCompleted;
    }
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("cpu_profiler.debug"), "TryUnwind Failure", "error", values.error_code, "warning", values.warnings,
        "num_frames", values.frames.size());
    return UnwindResult::kAborted;
}

std::unique_ptr<const ModuleCache::Module> LibunwindstackUnwinderAndroid::TryCreateModuleForAddress(uintptr_t address)
{
