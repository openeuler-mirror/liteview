...
struct CFIDataRow {
    ...
    {
        return static_cast<uint16_t>((cfi_data & kReturnAddressMask) << kReturnAddressShift);
    }
...
std::unique_ptr<ArmCFITable> ArmCFITable::Parse(span<const uint8_t> cfi_data)
{
    ...
    if (function_addresses.size() != *unw_index_count || entry_data_indices.size() != *unw_index_count)
        return nullptr;

    // The UNW_DATA table data is right after the end of UNW_INDEX table.
    auto entry_data = cfi_iterator.Span<uint8_t>((cfi_iterator.total_size() - cfi_iterator.position()) / sizeof(uint8_t));
    return std::make_unique<ArmCFITable>(function_addresses, entry_data_indices, entry_data);
}

ArmCFITable::ArmCFITable(span<const uint32_t> function_addresses, span<const uint16_t> entry_data_indices, span<const uint8_t> entry_data)
...
absl::optional<ArmCFITable::FrameEntry> ArmCFITable::FindEntryForAddress(uintptr_t address) const
{
