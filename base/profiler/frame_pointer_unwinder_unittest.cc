...
TEST_F(FramePointerUnwinderTest, FPPointsOutsideOfStack)
{
    ...
    EXPECT_EQ(UnwindResult::kAborted, unwinder()->TryUnwind(&context, input.top(), &stack));
...
TEST_F(FramePointerUnwinderTest, RegularUnwind)
{
    ...
    EXPECT_EQ(UnwindResult::kCompleted, unwinder()->TryUnwind(&context, input.top(), &stack));
    EXPECT_EQ(std::vector<Frame>({
                  { kModuleStart, module() },
...
TEST_F(FramePointerUnwinderTest, NonNativeFrame)
{
    ...
    EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder()->TryUnwind(&context, input.top(), &stack));
    EXPECT_EQ(std::vector<Frame>({
                  { kModuleStart, module() },
                  { kModuleStart + 0x20, module() },
