...
public:
    bool CopyStack(StackBuffer* stack_buffer, uintptr_t* stack_top, TimeTicks* timestamp, RegisterContext* thread_context, Delegate* delegate) override
    {
        *stack_top = reinterpret_cast<uintptr_t>(stack_buffer->buffer()) + 10; // Make msan happy.
...
public:
    ...
    UnwindResult TryUnwind(RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* stack) override
    {
...
base::circular_deque<std::unique_ptr<Unwinder>> MakeUnwinderCircularDeque(std::unique_ptr<Unwinder> native_unwinder, std::unique_ptr<Unwinder> aux_unwinder)
{
...
TEST(StackSamplerImplTest, CopyStack)
{
    ...
    StackSamplerImpl stack_sampler_impl(
        std::make_unique<TestStackCopier>(stack), MakeUnwindersFactory(std::make_unique<TestUnwinder>(&stack_copy)), &module_cache);
    ...
    stack_sampler_impl.RecordStackFrames(stack_buffer.get(), &profile_builder, PlatformThread::CurrentId());
...
TEST(StackSamplerImplTest, RecordStackFramesUMAMetric)
{
    ...
    for (uint32_t i = 0; i < StackSamplerImpl::kUMAHistogramDownsampleAmount - 1; i++) {
        stack_sampler_impl.RecordStackFrames(stack_buffer.get(), &profile_builder, PlatformThread::CurrentId());

        // Should have no new samples in the
        // Memory.StackSamplingProfiler.StackSampleSize histogram.
        histogram_tester.ExpectUniqueSample("Memory.StackSamplingProfiler.StackSampleSize", kExpectedSizeKB, 0);
    }

    stack_sampler_impl.RecordStackFrames(stack_buffer.get(), &profile_builder, PlatformThread::CurrentId());

    histogram_tester.ExpectUniqueSample("Memory.StackSamplingProfiler.StackSampleSize", kExpectedSizeKB, 1);
}
...
TEST(StackSamplerImplTest, CopyStackTimestamp)
{
    ...
    StackSamplerImpl stack_sampler_impl(
        std::make_unique<TestStackCopier>(stack, timestamp), MakeUnwindersFactory(std::make_unique<TestUnwinder>(&stack_copy)), &module_cache);
...
TEST(StackSamplerImplTest, UnwinderInvokedWhileRecordingStackFrames)
{
    ...
    ModuleCache module_cache;
    TestProfileBuilder profile_builder(&module_cache);
    StackSamplerImpl stack_sampler_impl(std::make_unique<DelegateInvokingStackCopier>(), MakeUnwindersFactory(std::move(owned_unwinder)), &module_cache);
...
TEST(StackSamplerImplTest, AuxUnwinderInvokedWhileRecordingStackFrames)
{
    ...
    TestProfileBuilder profile_builder(&module_cache);
    StackSamplerImpl stack_sampler_impl(
        std::make_unique<DelegateInvokingStackCopier>(), MakeUnwindersFactory(std::make_unique<CallRecordingUnwinder>()), &module_cache);
...
TEST(StackSamplerImplTest, WalkStack_Completed)
{
    ...
    std::vector<Frame> stack
        = StackSamplerImpl::WalkStackForTesting(&module_cache, &thread_context, 0u, MakeUnwinderCircularDeque(std::move(native_unwinder), nullptr));
...
TEST(StackSamplerImplTest, WalkStack_AuxUnwind)
{
    ...
    // non-native module.
    module_cache.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(GetTestInstructionPointer(), 1u, false)));

    auto aux_unwinder = WrapUnique(new FakeTestUnwinder({ { UnwindResult::kAborted, { 1u } } }));
    aux_unwinder->Initialize(&module_cache);
    std::vector<Frame> stack
        = StackSamplerImpl::WalkStackForTesting(&module_cache, &thread_context, 0u, MakeUnwinderCircularDeque(nullptr, std::move(aux_unwinder)));
...
TEST(StackSamplerImplTest, WalkStack_AuxThenNative)
{
    ...
    // Treat the context instruction pointer as being in the aux unwinder's
    // non-native module.
    module_cache.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(0u, 1u, false)));
    ...
    std::vector<Frame> stack = StackSamplerImpl::WalkStackForTesting(
        &module_cache, &thread_context, 0u, MakeUnwinderCircularDeque(std::move(native_unwinder), std::move(aux_unwinder)));
...
TEST(StackSamplerImplTest, WalkStack_NativeThenAux)
{
    ...
    // module.
    module_cache.UpdateNonNativeModules({}, ToModuleVector(std::make_unique<TestModule>(1u, 1u, false)));
