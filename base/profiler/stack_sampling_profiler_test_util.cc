...
std::unique_ptr<NativeUnwinderAndroid> CreateNativeUnwinderAndroidForTesting(uintptr_t exclude_module_with_base_address)
{
    ...
    public:
        explicit NativeUnwinderAndroidForTesting(std::unique_ptr<unwindstack::Maps> memory_regions_map, std::unique_ptr<unwindstack::Memory> process_memory,
            uintptr_t exclude_module_with_base_address)
            : NativeUnwinderAndroid(memory_regions_map.get(), process_memory.get(), exclude_module_with_base_address)
    ...
    return std::make_unique<NativeUnwinderAndroidForTesting>(std::move(maps), std::move(memory), exclude_module_with_base_address);
}
...
std::unique_ptr<Unwinder> CreateChromeUnwinderAndroidForTesting(uintptr_t chrome_module_base_address)
{
    ...
    public:
        ChromeUnwinderAndroidForTesting(std::unique_ptr<MemoryMappedFile> cfi_file, const ChromeUnwindInfoAndroid& unwind_info,
            ...
            : ChromeUnwinderAndroidV2(unwind_info, chrome_module_base_address, text_section_start_address)
...
std::unique_ptr<Unwinder> CreateChromeUnwinderAndroidForTesting(uintptr_t chrome_module_base_address)
{
    ...
    public:
        ChromeUnwinderAndroidForTesting(
            std::unique_ptr<MemoryMappedFile> cfi_file, std::unique_ptr<ArmCFITable> cfi_table, uintptr_t chrome_module_base_address)
    ...
    bool ok = cfi_file->Initialize(base::File(fd), cfi_region);
    DCHECK(ok);
    std::unique_ptr<ArmCFITable> cfi_table = ArmCFITable::Parse({ cfi_file->data(), cfi_file->length() });
    DCHECK(cfi_table);
    return std::make_unique<ChromeUnwinderAndroidForTesting>(std::move(cfi_file), std::move(cfi_table), chrome_module_base_address);
}
...
void ExpectStackContains(const std::vector<Frame>& stack, const std::vector<FunctionAddressRange>& functions)
{
    ...
    for (; frame_it != stack.end() && function_it != functions.end(); ++frame_it) {
        if (frame_it->instruction_pointer >= reinterpret_cast<uintptr_t>(function_it->start)
            && frame_it->instruction_pointer <= reinterpret_cast<uintptr_t>(function_it->end)) {
            ++function_it;
        }
    }
...
void ExpectStackContainsNames(const std::vector<Frame>& stack, const std::vector<std::string>& function_names)
{
    auto frame_it = stack.begin();
    auto names_it = function_names.begin();
    for (; frame_it != stack.end() && names_it != function_names.end(); ++frame_it) {
        if (frame_it->function_name == *names_it) {
            ++names_it;
        }
    }

    EXPECT_EQ(names_it, function_names.end()) << "Function name in position " << names_it - function_names.begin() << " - {" << *names_it
                                              << "} was not found in stack "
                                              << "(or did not appear in the expected order):\n"
                                              << FormatSampleForDiagnosticOutput(stack);
}
...
void ExpectStackDoesNotContain(const std::vector<Frame>& stack, const std::vector<FunctionAddressRange>& functions)
{
    ...
    for (const auto& frame : stack) {
        for (const auto& function : functions) {
            if (frame.instruction_pointer >= reinterpret_cast<uintptr_t>(function.start)
                && frame.instruction_pointer <= reinterpret_cast<uintptr_t>(function.end)) {
...
NativeLibrary LoadOtherLibrary()
{
    ...
    const auto load = [](NativeLibrary* library) {
        ...
        other_library_path = other_library_path.AppendASCII(GetLoadableModuleName("base_profiler_test_support_library"));
        NativeLibraryLoadError load_error;
        *library = LoadNativeLibrary(other_library_path, &load_error);
        ASSERT_TRUE(*library) << "error loading " << other_library_path.value() << ": " << load_error.ToString();
    };
