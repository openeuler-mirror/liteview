...
public:
    ...
    struct BASE_EXPORT Item {
        Item(uint64_t name_hash, absl::optional<int64_t> key, absl::optional<PlatformThreadId> thread_id, int64_t value);
        Item();
    ...
    void Set(uint64_t name_hash, absl::optional<int64_t> key, absl::optional<PlatformThreadId> thread_id, int64_t value);

    // Removes the item with the specified name hash and optional key. Has no
    // effect if such an item does not exist.
    void Remove(uint64_t name_hash, absl::optional<int64_t> key, absl::optional<PlatformThreadId> thread_id);
