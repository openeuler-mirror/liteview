...
ArmCFITable::FrameEntry MakeFrameEntry(uint16_t cfa_offset, uint16_t ra_offset)
{
    return ArmCFITable::FrameEntry { static_cast<uint16_t>(cfa_offset * sizeof(uintptr_t)), static_cast<uint16_t>(ra_offset * sizeof(uintptr_t)) };
}
...
TEST(ChromeUnwinderAndroidTest, Step)
{
    ...
    const uintptr_t stack_top = reinterpret_cast<uintptr_t>(stack_buffer.data() + stack_buffer.size());
    RegisterContext context;
    RegisterContextInstructionPointer(&context) = 0xBEEF;
    RegisterContextStackPointer(&context) = reinterpret_cast<uintptr_t>(stack_buffer.data());

    EXPECT_TRUE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(4, 1)));
    EXPECT_EQ(RegisterContextInstructionPointer(&context), 0x1111U);
    EXPECT_EQ(RegisterContextStackPointer(&context), reinterpret_cast<uintptr_t>(stack_buffer.data() + 4));

    EXPECT_TRUE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(1, 0)));
    EXPECT_EQ(RegisterContextInstructionPointer(&context), 0x2222U);
    EXPECT_EQ(RegisterContextStackPointer(&context), reinterpret_cast<uintptr_t>(stack_buffer.data() + 5));
}
...
TEST(ChromeUnwinderAndroidTest, StepImmediate)
{
    ...
    EXPECT_TRUE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(0, 0)));
    EXPECT_EQ(RegisterContextInstructionPointer(&context), 0x4444U);
    EXPECT_EQ(RegisterContextStackPointer(&context), reinterpret_cast<uintptr_t>(stack_buffer.data()));
}
...
TEST(ChromeUnwinderAndroidTest, StepImmediateFail)
{
    ...
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(0, 0)));
}
...
TEST(ChromeUnwinderAndroidTest, StepInvalidStack)
{
    ...
    EXPECT_CHECK_DEATH({
        ...
        ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(1, 0));
    });

    EXPECT_CHECK_DEATH({
        RegisterContextStackPointer(&context) = reinterpret_cast<uintptr_t>(stack_buffer.data() + stack_buffer.size() + 1);
...
TEST(ChromeUnwinderAndroidTest, StepOutOfBounds)
{
    ...
    const uintptr_t stack_top = RegisterContextStackPointer(&context) + kStackSize;

    // ra_offset exceeds cfa_offset.
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(1, 2)));
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(1, kOverflowOffset)));

    // cfa_offset exceeds |stack_top|.
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(kStackSize, 0)));

    // sp + cfa_offset overflows.
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(kOverflowOffset, 0)));
}
...
TEST(ChromeUnwinderAndroidTest, StepUnderflows)
{
    ...
    EXPECT_FALSE(ChromeUnwinderAndroid::StepForTesting(&context, stack_top, MakeFrameEntry(1, 4)));
}

TEST(ChromeUnwinderAndroidTest, CanUnwindFrom)
{
    auto cfi_table = ArmCFITable::Parse({ reinterpret_cast<const uint8_t*>(cfi_data), sizeof(cfi_data) });
    ...
    ChromeUnwinderAndroid unwinder(cfi_table.get(), chrome_module->GetBaseAddress());
...
TEST(ChromeUnwinderAndroidTest, TryUnwind)
{
    ...
    ModuleCache module_cache;
    const ModuleCache::Module* chrome_module = AddNativeModule(&module_cache, std::make_unique<TestModule>(0x1000, 0x500));
    ...
    uintptr_t stack_top = reinterpret_cast<uintptr_t>(stack_buffer.data() + stack_buffer.size());
    ...
    EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder.TryUnwind(&context, stack_top, &stack));
    EXPECT_EQ(std::vector<Frame>({ { 0x1100, chrome_module }, { 0x11AA, chrome_module }, { 0x2000, nullptr } }), stack);
}
...
TEST(ChromeUnwinderAndroidTest, TryUnwindNoData)
{
    ...
    EXPECT_EQ(UnwindResult::kAborted, unwinder.TryUnwind(&context, stack_top, &stack));
    EXPECT_EQ(std::vector<Frame>({ { 0x1200, chrome_module }, { 0x12AA, chrome_module } }), stack);
}
