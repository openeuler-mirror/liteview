...
std::vector<Frame> CaptureScenario(
    UnwindScenario* scenario, ModuleCache* module_cache, OnceCallback<void(RegisterContext*, uintptr_t, std::vector<Frame>*)> unwind_callback)
{
...
TEST(NativeUnwinderAndroidTest, DISABLED_PlainFunction)
{
    ...
    std::unique_ptr<unwindstack::Memory> memory = NativeUnwinderAndroid::CreateProcessMemory();

    ModuleCache module_cache;
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(maps.get(), memory.get(), 0);

    unwinder->Initialize(&module_cache);
    std::vector<Frame> sample
        = CaptureScenario(&scenario, &module_cache, BindLambdaForTesting([&](RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
              ASSERT_TRUE(unwinder->CanUnwindFrom(sample->back()));
              UnwindResult result = unwinder->TryUnwind(thread_context, stack_top, sample);
...
TEST(NativeUnwinderAndroidTest, ExcludeOtherLibrary)
{
    ...
    std::unique_ptr<unwindstack::Memory> memory = NativeUnwinderAndroid::CreateProcessMemory();
    ModuleCache module_cache;
    unwindstack::MapInfo* other_library_map = maps->Find(GetAddressInOtherLibrary(other_library)).get();
    ASSERT_NE(nullptr, other_library_map);
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(maps.get(), memory.get(), other_library_map->start());
    unwinder->Initialize(&module_cache);

    std::vector<Frame> sample
        = CaptureScenario(&scenario, &module_cache, BindLambdaForTesting([&](RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
              ASSERT_TRUE(unwinder->CanUnwindFrom(sample->back()));
              EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder->TryUnwind(thread_context, stack_top, sample));
...
TEST(NativeUnwinderAndroidTest, MAYBE_ResumeUnwinding)
{
    ...
    auto unwinder_for_all = std::make_unique<NativeUnwinderAndroid>(maps.get(), memory.get(), 0);
    unwinder_for_all->Initialize(&module_cache_for_all);

    ModuleCache module_cache_for_native;
    auto unwinder_for_native = std::make_unique<NativeUnwinderAndroid>(maps.get(), memory.get(), reinterpret_cast<uintptr_t>(&__executable_start));
    unwinder_for_native->Initialize(&module_cache_for_native);

    ModuleCache module_cache_for_chrome;
    unwindstack::MapInfo* other_library_map = maps->Find(GetAddressInOtherLibrary(other_library)).get();
    ASSERT_NE(nullptr, other_library_map);
    auto unwinder_for_chrome = std::make_unique<NativeUnwinderAndroid>(maps.get(), memory.get(), other_library_map->start());
    unwinder_for_chrome->Initialize(&module_cache_for_chrome);

    std::vector<Frame> sample = CaptureScenario(
        &scenario, &module_cache_for_native, BindLambdaForTesting([&](RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* sample) {
            ...
            EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder_for_native->TryUnwind(thread_context, stack_top, sample));
            ...
            EXPECT_EQ(UnwindResult::kUnrecognizedFrame, unwinder_for_chrome->TryUnwind(thread_context, stack_top, sample));
            ...
            EXPECT_EQ(UnwindResult::kCompleted, unwinder_for_all->TryUnwind(thread_context, stack_top, sample));
        }));
...
TEST(NativeUnwinderAndroidTest, DISABLED_JavaFunction)
{
    ...
    bool can_always_unwind = build_info->sdk_int() > base::android::SDK_VERSION_MARSHMALLOW;
...
TEST(NativeUnwinderAndroidTest, ModuleDebugBasenameForNonElf)
{
    ...
    std::unique_ptr<unwindstack::Memory> memory = NativeUnwinderAndroid::CreateProcessMemory();
    auto unwinder = std::make_unique<NativeUnwinderAndroid>(&maps, memory.get(), 0);
    unwinder->Initialize(&module_cache);
