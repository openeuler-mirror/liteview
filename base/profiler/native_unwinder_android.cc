...
// static
std::unique_ptr<unwindstack::Memory> NativeUnwinderAndroid::CreateProcessMemory()
{
...
UnwindResult NativeUnwinderAndroid::TryUnwind(RegisterContext* thread_context, uintptr_t stack_top, std::vector<Frame>* stack)
{
    ...
    do {
        ...
        unwindstack::Elf* elf = map_info->GetElf({ process_memory_.get(), [](unwindstack::Memory*) {} }, arch);
        if (!elf->valid())
            break;
        ...
        bool finished = false;
        bool stepped = elf->StepIfSignalHandler(rel_pc, regs.get(), &stack_memory) || elf->Step(rel_pc, regs.get(), &stack_memory, &finished, &is_signal_frame);
        if (stepped && finished)
...
void NativeUnwinderAndroid::EmitDexFrame(uintptr_t dex_pc, std::vector<Frame>* stack) const
{
