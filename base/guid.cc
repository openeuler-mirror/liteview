...
template <typename StringPieceType> std::string GetCanonicalGUIDInternal(StringPieceType input, bool strict)
{
...
{
    return StringPrintf("%08x-%04x-%04x-%04x-%012llx", static_cast<uint32_t>(bytes[0] >> 32), static_cast<uint32_t>((bytes[0] >> 16) & 0x0000ffff),
        static_cast<uint32_t>(bytes[0] & 0x0000ffff), static_cast<uint32_t>(bytes[1] >> 48), bytes[1] & 0x0000ffff'ffffffffULL);
}
...
// static
GUID GUID::FormatRandomDataAsV4(base::span<const uint8_t, 16> input, base::PassKey<content::FileSystemAccessManagerImpl> /*pass_key*/)
{
