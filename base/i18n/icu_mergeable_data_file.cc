...
BASE_FEATURE(kLacrosMergeIcuDataFile, "LacrosMergeIcuDataFile", base::FEATURE_ENABLED_BY_DEFAULT);
...
public:
    ...
    size_t length() const
    {
        return memory_mapped_file_.length();
    }
...
std::unique_ptr<AshMemoryMappedFile> MmapAshFile(const FilePath& ash_file_path)
{
...
IcuMergeableDataFile::Hashes::Hashes(HashToOffsetMap ash, std::vector<HashType> lacros)
    : ash(std::move(ash))
    , lacros(std::move(lacros))
{
}
IcuMergeableDataFile::Hashes::Hashes(Hashes&& other) = default;
IcuMergeableDataFile::Hashes& IcuMergeableDataFile::Hashes::operator=(Hashes&& other) = default;
IcuMergeableDataFile::Hashes::~Hashes() = default;

bool IcuMergeableDataFile::Initialize(File lacros_file, MemoryMappedFile::Region region)
{
...
bool IcuMergeableDataFile::MmapLacrosFile(bool remap)
{
    ...
    if (remap) {
        ...
        lacros_data_ = static_cast<uint8_t*>(mmap(lacros_data_, lacros_length_, PROT_READ, MAP_FIXED | MAP_PRIVATE, lacros_file_.GetPlatformFile(), 0));
    } else {
        // Otherwise, simply map the file.
        lacros_data_ = static_cast<uint8_t*>(mmap(nullptr, lacros_length_, PROT_READ, MAP_PRIVATE, lacros_file_.GetPlatformFile(), 0));
    }
...
IcuMergeableDataFile::Slice IcuMergeableDataFile::FindOverlap(const AshMemoryMappedFile& ash_file, const Hashes& hashes, size_t lacros_offset) const
{
    ...
    size_t overlap_length = kPageSize * CountEqualPages(ash_file, ash_file.data() + ash_offset, lacros_data_ + lacros_offset);

    return { ash_offset, overlap_length };
}

bool IcuMergeableDataFile::MergeArea(const AshMemoryMappedFile& ash_file, const Slice& ash_overlap, size_t lacros_offset)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
    ...
    uint8_t* map_result
        = static_cast<uint8_t*>(mmap(lacros_data_ + lacros_offset, ash_overlap.length, PROT_READ, MAP_FIXED | MAP_PRIVATE, ash_file.fd(), ash_overlap.offset));
...
size_t IcuMergeableDataFile::CountEqualPages(const AshMemoryMappedFile& ash_file, const uint8_t* ash_page, const uint8_t* lacros_page) const
{
    size_t pages = 0;
    const uint8_t* ash_end = ash_file.data() + ash_file.length();
    const uint8_t* lacros_end = lacros_data_ + lacros_length_;

    while (ash_page < ash_end && lacros_page < lacros_end && memcmp(ash_page, lacros_page, kPageSize) == 0) {
        ash_page += kPageSize;
...
IcuMergeableDataFile::Hashes IcuMergeableDataFile::CalculateHashes(const AshMemoryMappedFile& ash_file, const FilePath& ash_file_path)
{
...
bool IcuMergeableDataFile::MaybeLoadCachedHashes(const AshMemoryMappedFile& ash_file, const FilePath& ash_file_path, Hashes& hashes)
{
    FilePath ash_hash_path = ash_file_path.AddExtensionASCII(kIcuDataFileHashExtension);
    FilePath lacros_hash_path = GetLacrosFilePath().AddExtensionASCII(kIcuDataFileHashExtension);
    ...
    if (!result || (ash_hash_file.length() % kHashBytes) || ((ash_hash_file.length() / kHashBytes) != ash_pages)) {
        return false;
    }
    ...
    if (!result || (lacros_hash_file.length() % kHashBytes) || ((lacros_hash_file.length() / kHashBytes) != lacros_pages)) {
        return false;
    }
...
FilePath IcuMergeableDataFile::GetLacrosFilePath()
{
    ...
    char path[PATH_MAX];
    FilePath proc_path = FilePath("/proc/self/fd/").AppendASCII(base::NumberToString(lacros_file_.GetPlatformFile()));
