...
public:
    ...
    bool used_cached_hashes() const
    {
        return used_cached_hashes_;
    }
...
private:
    ...
    Slice FindOverlap(const AshMemoryMappedFile& ash_file, const Hashes& hashes, size_t lacros_offset) const;

    bool MergeArea(const AshMemoryMappedFile& ash_file, const Slice& ash_overlap, size_t lacros_offset);
    ...
    size_t CountEqualPages(const AshMemoryMappedFile& ash_file, const uint8_t* ash_page, const uint8_t* lacros_page) const;

    Hashes CalculateHashes(const AshMemoryMappedFile& ash_file, const FilePath& ash_file_path);
    ...
    bool MaybeLoadCachedHashes(const AshMemoryMappedFile& ash_file, const FilePath& ash_file_path, Hashes& hashes);
