...
bool BreakIterator::Init()
{
    ...
    case BREAK_NEWLINE:
        ...
        break;
    case RULE_BASED:
        iter_ = ubrk_openRules(rules_.c_str(), static_cast<int32_t>(rules_.length()), nullptr, 0, &parse_error, &status);
        if (U_FAILURE(status)) {
    ...
    if (string_.data() != nullptr) {
        ubrk_setText(static_cast<UBreakIterator*>(iter_), string_.data(), static_cast<int32_t>(string_.size()), &status);
        if (U_FAILURE(status)) {
            return false;
        }
    }
