...
TEST_F(IcuMergeableDataFileTest, IcuDataFileMergesCommonPages)
{
    ...
    uint32_t flags = base::File::FLAG_CREATE | base::File::FLAG_READ | base::File::FLAG_WRITE;
    ...
    int pg0_sz = pg0.size(), pg1_sz = pg1.size(), pg2_sz = pg2.size(), pg3_sz = pg3.size(), pg4_sz = pg4.size();
    ...
    ASSERT_TRUE(icu_data_file.Initialize(std::move(lacros_file), MemoryMappedFile::Region::kWholeFile));
...
TEST_F(IcuMergeableDataFileTest, IcuDataFileMergesCommonPagesWithCachedHashes)
{
    ...
    FilePath ash_hash_path = ash_path.AddExtensionASCII(IcuMergeableDataFile::kIcuDataFileHashExtension);
    FilePath lacros_hash_path = lacros_path.AddExtensionASCII(IcuMergeableDataFile::kIcuDataFileHashExtension);
    ...
    ASSERT_EQ(8, ash_hash_file.WriteAtCurrentPos("\x00\x00\x00\x00\x00\x00\x00\x00", 8));
    ASSERT_EQ(8, ash_hash_file.WriteAtCurrentPos("\x11\x11\x11\x11\x11\x11\x11\x11", 8));
    ASSERT_EQ(8, ash_hash_file.WriteAtCurrentPos("\x22\x22\x22\x22\x22\x22\x22\x22", 8));
    ASSERT_EQ(8, ash_hash_file.WriteAtCurrentPos("\x44\x44\x44\x44\x44\x44\x44\x44", 8));
    ...
    ASSERT_EQ(8, lacros_hash_file.WriteAtCurrentPos("\x00\x00\x00\x00\x00\x00\x00\x00", 8));
    ASSERT_EQ(8, lacros_hash_file.WriteAtCurrentPos("\x11\x11\x11\x11\x11\x11\x11\x11", 8));
    // NOTE: Simulate hash collision.
    ASSERT_EQ(8, lacros_hash_file.WriteAtCurrentPos("\x22\x22\x22\x22\x22\x22\x22\x22", 8));
    ASSERT_EQ(8, lacros_hash_file.WriteAtCurrentPos("\x44\x44\x44\x44\x44\x44\x44\x44", 8));
