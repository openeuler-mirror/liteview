...
bool TimeDurationFormat(const TimeDelta time, const DurationFormatWidth width, std::u16string* out)
{
    DCHECK(out);
    UErrorCode status = U_ZERO_ERROR;
    ...
    if (U_FAILURE(status)) {
        LOG(ERROR) << "Creating MeasureUnit or Measure for " << hours << "h" << minutes << "m failed: " << u_errorName(status);
        return false;
    }

    icu::MeasureFormat measure_format(icu::Locale::getDefault(), u_width, status);
    if (U_FAILURE(status)) {
        LOG(ERROR) << "Creating MeasureFormat for " << icu::Locale::getDefault().getName() << " failed: " << u_errorName(status);
        return false;
    }
...
bool TimeDurationFormatWithSeconds(const TimeDelta time, const DurationFormatWidth width, std::u16string* out)
{
    DCHECK(out);
    UErrorCode status = U_ZERO_ERROR;
