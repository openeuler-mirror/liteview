...
void LazyInitIcuDataFile()
{
    ...
    ScopedCFTypeRef<CFStringRef> data_file_name(SysUTF8ToCFStringRef(kIcuDataFileName));
...
void InitializeExternalTimeZoneData()
{
    ...
    } else if (g_icu_time_zone_data_dir == kIcuTimeZoneDataDir && base::DirectoryExists(base::FilePath((kLegacyIcuTimeZoneDataDir)))) {
...
int LoadIcuData(
    PlatformFile data_fd, const MemoryMappedFile::Region& data_region, std::unique_ptr<IcuDataFile>* out_mapped_data_file, UErrorCode* out_error_code)
{
...
static void U_CALLCONV TraceICUEntry(const void*, int32_t fn_number)
{
    ...
    case UTRACE_UBRK_CREATE_CHARACTER:
        base::UmaHistogramEnumeration(kICUCreateInstance, ICUCreateInstance::kCharacterBreakIterator);
        break;
    case UTRACE_UBRK_CREATE_SENTENCE:
        base::UmaHistogramEnumeration(kICUCreateInstance, ICUCreateInstance::kSentenceBreakIterator);
        break;
    case UTRACE_UBRK_CREATE_TITLE:
        base::UmaHistogramEnumeration(kICUCreateInstance, ICUCreateInstance::kTitleBreakIterator);
        break;
    case UTRACE_UBRK_CREATE_WORD:
        base::UmaHistogramEnumeration(kICUCreateInstance, ICUCreateInstance::kWordBreakIterator);
        break;
    default:
        return;
    }
...
static void U_CALLCONV TraceICUData(const void* context, int32_t fn_number, int32_t level, const char* fmt, va_list args)
{
    switch (fn_number) {
