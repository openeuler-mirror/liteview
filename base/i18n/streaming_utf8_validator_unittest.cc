...
protected:
    ...
    void TestNumber(uint32_t n) const
    {
        ...
        EXPECT_EQ(IsStringUtf8(test, sizeof n), validator.AddBytes(test, sizeof n) == VALID_ENDPOINT)
            << "Difference of opinion for \""
            << base::StringPrintf("\\x%02X\\x%02X\\x%02X\\x%02X", test[0] & 0xFF, test[1] & 0xFF, test[2] & 0xFF, test[3] & 0xFF) << "\"";
    }
...
protected:
    ...
    template <typename Iterator> void CheckRange(Iterator begin, Iterator end, StreamingUtf8Validator::State expected)
    {
        for (Iterator it = begin; it != end; ++it) {
            ...
            EXPECT_EQ(expected, validator.AddBytes(sequence.data(), sequence.size())) << "Failed for \"" << sequence << "\"";
        }
    }
    ...
    void CheckCombinations(Iterator1 begin1, Iterator1 end1, Iterator2 begin2, Iterator2 end2, StreamingUtf8Validator::State expected)
    {
        ...
        for (Iterator1 it1 = begin1; it1 != end1; ++it1) {
            ...
            for (Iterator2 it2 = begin2; it2 != end2; ++it2) {
                ...
                EXPECT_EQ(expected, validator.AddBytes(c2.data(), c2.size())) << "Failed for \"" << c1 << c2 << "\"";
                validator.Reset();
            }
        }
    }
...
TEST(StreamingUtf8ValidatorTest, HelloWorld)
{
    ...
    EXPECT_EQ(VALID_ENDPOINT, StreamingUtf8Validator().AddBytes(kHelloWorld, strlen(kHelloWorld)));
}
