...
TEST(TaskRunnerHelpersTest, PostTaskAndReplyWithResult)
{
    ...
    PostTaskAndReplyWithResult(ThreadTaskRunnerHandle::Get().get(), FROM_HERE, BindOnce(&ReturnFourtyTwo), BindOnce(&StoreValue, &result));
...
TEST(TaskRunnerHelpersTest, PostTaskAndReplyWithResultImplicitConvert)
{
    ...
    test::TaskEnvironment task_environment;
    PostTaskAndReplyWithResult(ThreadTaskRunnerHandle::Get().get(), FROM_HERE, BindOnce(&ReturnFourtyTwo), BindOnce(&StoreDoubleValue, &result));
...
TEST(TaskRunnerHelpersTest, PostTaskAndReplyWithResultPassed)
{
    ...
    test::TaskEnvironment task_environment;
    PostTaskAndReplyWithResult(ThreadTaskRunnerHandle::Get().get(), FROM_HERE, BindOnce(&CreateFoo), BindOnce(&ExpectFoo));
...
TEST(TaskRunnerHelpersTest, PostTaskAndReplyWithResultPassedFreeProc)
{
    ...
    test::TaskEnvironment task_environment;
    PostTaskAndReplyWithResult(ThreadTaskRunnerHandle::Get().get(), FROM_HERE, BindOnce(&CreateScopedFoo), BindOnce(&ExpectScopedFoo));
...
TEST(TaskRunnerHelpersTest, PostTaskAndReplyWithResultWithoutDefaultConstructor)
{
    const int kSomeVal = 17;

    test::TaskEnvironment task_environment;
    int actual = 0;
    PostTaskAndReplyWithResult(ThreadTaskRunnerHandle::Get().get(), FROM_HERE, BindOnce(&CreateFooWithoutDefaultConstructor, kSomeVal),
