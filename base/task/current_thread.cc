...
void CurrentThread::EnableMessagePumpTimeKeeperMetrics(const char* thread_name)
{
...
void CurrentThread::RegisterOnNextIdleCallback(OnceClosure on_next_idle_callback)
{
...
#if BUILDFLAG(IS_WIN)
HRESULT CurrentIOThread::RegisterIOHandler(HANDLE file, MessagePumpForIO::IOHandler* handler)
{
    DCHECK(current_->IsBoundToCurrentThread());
...
bool CurrentIOThread::WaitForIOCompletionWithHandler(DWORD timeout, MessagePumpForIO::IOHandler* handler)
{
    DCHECK(current_->IsBoundToCurrentThread());
    return GetMessagePumpForIO()->WaitForIOCompletionWithHandler(timeout, handler);
}
...
void WaitIOThreadForIOCompletion()
{
