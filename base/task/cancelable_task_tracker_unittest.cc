...
void ExpectIsCanceled(const CancelableTaskTracker::IsCanceledCallback& is_canceled, bool expected_is_canceled)
{
...
TEST_F(CancelableTaskTrackerTest, NewTrackedTaskIdDifferentThread)
{
    ...
    other_thread.task_runner()->PostTask(FROM_HERE, BindOnce(&ExpectIsCanceled, is_canceled, false));
    other_thread.Stop();

    task_tracker_.TryCancel(task_id);

    ASSERT_TRUE(other_thread.Start());
    other_thread.task_runner()->PostTask(FROM_HERE, BindOnce(&ExpectIsCanceled, is_canceled, true));
    other_thread.Stop();
}
...
TEST_F(CancelableTaskTrackerDeathTest, PostFromDifferentThread)
{
    ...
    bad_thread.task_runner()->PostTask(FROM_HERE, BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_), BindOnce(&PostDoNothingTask)));
}

void TryCancel(CancelableTaskTracker::TaskId task_id, CancelableTaskTracker* task_tracker)
{
...
TEST_F(CancelableTaskTrackerDeathTest, CancelOnDifferentThread)
{
    ...
    bad_thread.task_runner()->PostTask(
        FROM_HERE, BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_), BindOnce(&TryCancel, task_id)));
...
TEST_F(CancelableTaskTrackerDeathTest, CancelAllOnDifferentThread)
{
    ...
    bad_thread.task_runner()->PostTask(
        FROM_HERE, BindOnce(&MaybeRunDeadlyTaskTrackerMemberFunction, Unretained(&task_tracker_), BindOnce(&CancelableTaskTracker::TryCancelAll)));
