...
TEST(TaskTraitsExtensionTest, NoExtension)
{
    ...
    EXPECT_EQ(traits.extension_id(), TaskTraitsExtensionStorage::kInvalidExtensionId);
}
...
TEST(TaskTraitsExtensionTest, CreateWithOneExtensionTrait)
{
    ...
    EXPECT_EQ(traits.GetExtension<TestTaskTraitsExtension>().enum_trait(), TestExtensionEnumTrait::kB);
...
{
    constexpr TaskTraits traits = { TestExtensionEnumTrait::kB, TestExtensionBoolTrait() };
...
{
    constexpr TaskTraits traits = { TaskPriority::USER_BLOCKING, TestExtensionEnumTrait::kC, TestExtensionBoolTrait() };

    EXPECT_EQ(traits.priority(), TaskPriority::USER_BLOCKING);
    EXPECT_EQ(traits.GetExtension<TestTaskTraitsExtension>().enum_trait(), TestExtensionEnumTrait::kC);
    EXPECT_TRUE(traits.GetExtension<TestTaskTraitsExtension>().bool_trait());
}
