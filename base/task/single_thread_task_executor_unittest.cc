...
TEST(SingleThreadTaskExecutorTest, GetTaskExecutorForCurrentThreadInPostedTask)
{
    SingleThreadTaskExecutor single_thread_task_executor;
    ...
    RunLoop run_loop;
    single_thread_task_executor.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&]() {
...
void RecursiveFunc(TaskList* order, int cookie, int depth)
{
    ...
    if (depth > 0) {
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, order, cookie, depth - 1));
    }
...
void MessageBoxFunc(TaskList* order, int cookie, bool is_reentrant)
{
    ...
    absl::optional<CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop> maybe_allow_nesting;
    if (is_reentrant)
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostTask)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test0, foo));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test1ConstRef, foo, a));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test1Ptr, foo, &b));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test1Int, foo, 100));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test2Ptr, foo, &a, &c));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&Foo::Test2Mixed, foo, a, &d));
    // After all tests, post a message that will shut down the message loop
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RunLoop::QuitCurrentWhenIdleDeprecated));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_Basic)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks), kDelay);
    RunLoop().Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InDelayOrder)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks), Milliseconds(200));
    // If we get a large pause in execution (due to a context switch) here, this
    // test could fail.
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks), Milliseconds(10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks), kDelay);
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks), kDelay);
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder_2)
{
    ...
    executor.task_runner()->PostTask(FROM_HERE, BindOnce(&SlowFunc, kPause, &num_tasks));
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks), Milliseconds(10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_InPostOrder_3)
{
    ...
    for (int i = 1; i < num_tasks; ++i)
        executor.task_runner()->PostTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks));

    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks), Milliseconds(1));
...
TEST_P(SingleThreadTaskExecutorTypedTest, PostDelayedTask_SharedTimer)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time1, &num_tasks), Seconds(1000));
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time2, &num_tasks), Milliseconds(10));
    ...
    ~RecordDeletionProbe()
    {
        *was_deleted_ = true;
        if (post_on_delete_.get())
            ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RecordDeletionProbe::Run, post_on_delete_));
    }
...
void NestingFunc(int* depth)
{
    if (*depth > 0) {
        *depth -= 1;
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&NestingFunc, depth));
...
TEST_P(SingleThreadTaskExecutorTypedTest, Recursive)
{
    ...
    TaskList order;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, &order, 1, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RecursiveFunc, &order, 2, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitFunc, &order, 3));
...
TEST_P(SingleThreadTaskExecutorTypedTest, NonNestableWithNoNesting)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostNonNestableTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitFunc, &order, 3));
    RunLoop().Run();
...
TEST_P(SingleThreadTaskExecutorTypedTest, NonNestableDelayedInNestedLoop)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatPumps, &order, 1));
    ThreadTaskRunnerHandle::Get()->PostNonNestableTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&SleepFunc, &order, 4, Milliseconds(50)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 5));
    ThreadTaskRunnerHandle::Get()->PostNonNestableTask(FROM_HERE, BindOnce(&QuitFunc, &order, 6));
...
TEST_P(SingleThreadTaskExecutorTypedTest, QuitNow)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 4)); // never runs
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitTop)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopNestedAfterQuit)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitAndRunNestedLoop, &order, 1, &outer_run_loop, &nested_run_loop));
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitBogus)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_run_loop)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, bogus_run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_run_loop.QuitClosure());
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitDeep)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 1, Unretained(&nested_loop1)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 2, Unretained(&nested_loop2)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 3, Unretained(&nested_loop3)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatRuns, &order, 4, Unretained(&nested_loop4)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 5));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, outer_run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 6));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_loop1.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 7));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_loop2.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 8));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_loop3.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 9));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_loop4.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 10));
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderBefore)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1)); // never runs
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow)); // never runs
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderDuring)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 1));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, run_loop.QuitClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 2)); // never runs
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow)); // never runs
...
TEST_P(SingleThreadTaskExecutorTypedTest, RunLoopQuitOrderAfter)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 3));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, nested_run_loop.QuitClosure()); // has no affect
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&OrderedFunc, &order, 4));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&FuncThatQuitsNow));
...
TEST_P(SingleThreadTaskExecutorTypedTest, NestableTasksAllowedExplicitlyInScope)
{
    SingleThreadTaskExecutor executor(GetParam());
    RunLoop run_loop;
    executor.task_runner()->PostTask(FROM_HERE,
...
TEST(SingleThreadTaskExecutorTest, PostDelayedTask_SharedTimer_SubPump)
{
    ...
    executor.task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordRunTimeFunc, &run_time, &num_tasks), Seconds(1000));
...
bool QuitOnSystemTimer(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result)
{
    if (message == static_cast<UINT>(WM_TIMER)) {
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&::PostQuitMessage, 0));
    }
    *result = 0;
    return true;
}
...
{
    if (message == static_cast<UINT>(WM_TIMER)) {
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce(&::PostQuitMessage, 0), Milliseconds(10));
    }
    *result = 0;
    return true;
}
...
void RunTest_NestingDenial2(MessagePumpType message_pump_type)
{
    ...
    worker.task_runner()->PostTask(FROM_HERE, BindOnce(&RecursiveFuncWin, ThreadTaskRunnerHandle::Get(), event.get(), true, &order, false));
...
TEST(SingleThreadTaskExecutorTest, NestingSupport2)
{
    ...
    TaskList order;
    win::ScopedHandle event(CreateEvent(NULL, FALSE, FALSE, NULL));
    worker.task_runner()->PostTask(FROM_HERE, BindOnce(&RecursiveFuncWin, ThreadTaskRunnerHandle::Get(), event.get(), false, &order, true));
    // Let the other thread execute.
    WaitForSingleObject(event.get(), INFINITE);
    RunLoop().Run();
...
{
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&EmptyFunction));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&EmptyFunction));
}
...
LRESULT CALLBACK TestWndProcThunk(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case 1:
        ...
        // test.
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&PostMultipleTasks));
        // Next, we post a task that posts a windows message to trigger the second
        // stage of the test.
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&PostWindowsMessage, hwnd));
        break;
    case 2:
        ...
        CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow_nestable_tasks;
        bool did_run = false;
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&EndTest, &did_run, hwnd));
...
TEST(SingleThreadTaskExecutorTest, SequenceLocalStorageSetGet)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { slot.emplace(11); }));

    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { EXPECT_EQ(*slot, 11); }));
...
TEST(SingleThreadTaskExecutorTest, SequenceLocalStorageDifferentMessageLoops)
{
    ...
    SingleThreadTaskExecutor executor;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindLambdaForTesting([&]() { EXPECT_FALSE(slot); }));
    ...
    static void PostTaskWithPostingDestructor(int times)
    {
        if (times > 0) {
            ThreadTaskRunnerHandle::Get()->PostTask(
                FROM_HERE, BindOnce([](std::unique_ptr<PostTaskOnDestroy>) {}, std::make_unique<PostTaskOnDestroy>(times - 1)));
        }
    }
