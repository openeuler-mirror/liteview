...
base::ThreadLocalPointer<internal::SequenceManagerImpl>* GetTLSSequenceManagerImpl()
{
...
perfetto::protos::pbzero::SequenceManagerTask::Priority TaskPriorityToProto(TaskQueue::QueuePriority priority)
{
...
{
    std::unique_ptr<SequenceManager> sequence_manager = internal::SequenceManagerImpl::CreateUnbound(std::move(settings));
...
public:
    NativeWorkHandleImpl(SequenceManagerImpl* sequence_manager, TaskQueue::QueuePriority priority)
        : sequence_manager_(sequence_manager->GetWeakPtr())
        , priority_(priority)
    {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("sequence_manager", "NativeWork", this, "priority", TaskQueue::PriorityToString(priority_));
...
void SequenceManagerImpl::ApplyNoWakeUpsForCanceledTasks()
{
    ...
    DCHECK_EQ(g_no_wake_ups_for_canceled_tasks.load(std::memory_order_relaxed), kNoWakeUpsForCanceledTasks.default_state == FEATURE_ENABLED_BY_DEFAULT);
    g_no_wake_ups_for_canceled_tasks.store(FeatureList::IsEnabled(kNoWakeUpsForCanceledTasks), std::memory_order_relaxed);
}

// static
void SequenceManagerImpl::ResetNoWakeUpsForCanceledTasksForTesting()
{
    g_no_wake_ups_for_canceled_tasks.store(kNoWakeUpsForCanceledTasks.default_state == FEATURE_ENABLED_BY_DEFAULT, std::memory_order_relaxed);
}
...
void SequenceManagerImpl::CompleteInitializationOnBoundThread()
{
    ...
    if (GetMessagePump()) {
        DCHECK(!GetTLSSequenceManagerImpl()->Get()) << "Can't register a second SequenceManagerImpl on the same thread.";
...
void SequenceManagerImpl::ShutdownTaskQueueGracefully(std::unique_ptr<internal::TaskQueueImpl> task_queue)
{
...
void SequenceManagerImpl::SetNextWakeUp(LazyNow* lazy_now, absl::optional<WakeUp> wake_up)
{
...
void SequenceManagerImpl::MaybeEmitTaskDetails(perfetto::EventContext& ctx, const SequencedTaskSource::SelectedTask& selected_task)
{
#if BUILDFLAG(ENABLE_BASE_TRACING)
    ...
    sequence_manager_task->set_priority(TaskPriorityToProto(selected_task.priority));
    sequence_manager_task->set_queue_name(selected_task.task_queue_name);

#endif //  BUILDFLAG(ENABLE_BASE_TRACING)
}

absl::optional<SequenceManagerImpl::SelectedTask> SequenceManagerImpl::SelectNextTask(LazyNow& lazy_now, SelectTaskOption option)
{
    absl::optional<SelectedTask> selected_task = SelectNextTaskImpl(lazy_now, option);
...
void SequenceManagerImpl::LogTaskDebugInfo(const WorkQueue* selected_work_queue) const
{
    ...
    case Settings::TaskLogging::kEnabledWithBacktrace: {
        ...
        if (length == 0)
            break;
        LOG(INFO) << "#" << static_cast<uint64_t>(task->enqueue_order()) << " " << selected_work_queue->task_queue()->GetName()
                  << (task->cross_thread_ ? " Run crossthread " : " Run ") << debug::StackTrace(task_trace.data(), length);
        break;
    }
...
absl::optional<SequenceManagerImpl::SelectedTask> SequenceManagerImpl::SelectNextTaskImpl(LazyNow& lazy_now, SelectTaskOption option)
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    ...
    while (true) {
        ...
        if (UNLIKELY(work_queue->GetFrontTask()->nestable == Nestable::kNonNestable && main_thread_only().nesting_depth > 0)) {
        ...
        if (UNLIKELY(!ShouldRunTaskOfPriority(work_queue->task_queue()->GetQueuePriority()))) {
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "SequenceManager.YieldToNative");
            return absl::nullopt;
        }
...
bool SequenceManagerImpl::ShouldRunTaskOfPriority(TaskQueue::QueuePriority priority) const
{
...
absl::optional<WakeUp> SequenceManagerImpl::GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option) const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
absl::optional<WakeUp> SequenceManagerImpl::GetNextDelayedWakeUpWithOption(SelectTaskOption option) const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
...
absl::optional<WakeUp> SequenceManagerImpl::AdjustWakeUp(absl::optional<WakeUp> wake_up, LazyNow* lazy_now) const
{
    DCHECK_CALLED_ON_VALID_THREAD(associated_thread_->thread_checker);
    if (!wake_up)
        return absl::nullopt;
...
Value::Dict SequenceManagerImpl::AsValueWithSelectorResult(internal::WorkQueue* selected_work_queue, bool force_verbose) const
{
    ...
    state.Set("native_work_priority", TaskQueue::PriorityToString(*main_thread_only().pending_native_work.begin()));
...
void SequenceManagerImpl::ReclaimMemory()
{
    ...
    for (auto it = main_thread_only().queues_to_gracefully_shutdown.begin(); it != main_thread_only().queues_to_gracefully_shutdown.end();) {
...
void SequenceManagerImpl::EnableMessagePumpTimeKeeperMetrics(const char* thread_name)
{
...
scoped_refptr<TaskQueue> SequenceManagerImpl::CreateTaskQueue(const TaskQueue::Spec& spec)
{
...
std::unique_ptr<NativeWorkHandle> SequenceManagerImpl::OnNativeWorkPending(TaskQueue::QueuePriority priority)
{
...
void SequenceManagerImpl::EnablePeriodicYieldingToNative(base::TimeDelta interval)
{
    TRACE_EVENT(
        TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "SequenceManagerImpl::EnablePeriodicYieldingToNative", "yield_interval_ms", interval.InMilliseconds());
...
void SequenceManagerImpl::RecordCrashKeys(const PendingTask& pending_task)
{
    ...
    *(--pos) = ' ';
    ...
    debug::SetCrashKeyString(main_thread_only().async_stack_crash_key, StringPiece(pos, static_cast<size_t>(buffer_end - pos)));
#endif // BUILDFLAG(IS_NACL)
}
