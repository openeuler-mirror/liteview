...
BASE_FEATURE(kDifferentWorkQueueCapacities, "DifferentWorkQueueCapacities", FEATURE_DISABLED_BY_DEFAULT);
...
{
    g_different_work_queue_capacities_feature_enabled.store(FeatureList::IsEnabled(kDifferentWorkQueueCapacities), std::memory_order_relaxed);
}

// static
bool WorkQueue::IsDifferentWorkQueueCapacitiesEnabled()
{
    return g_different_work_queue_capacities_feature_enabled.load(std::memory_order_relaxed);
}

// static
size_t WorkQueue::GetStackCapacityChoice()
{
    static const size_t kStackCapacityChoice = IsDifferentWorkQueueCapacitiesEnabled()
        ? static_cast<size_t>(GetFieldTrialParamByFeatureAsInt(kDifferentWorkQueueCapacities, "StackCapacity", StackCapacity::kDefault))
...
template <size_t stack_capacity> bool WorkQueue::RemoveAllCancelledTasksFromFrontImpl()
{
...
template bool WorkQueue::RemoveAllCancelledTasksFromFrontImpl<WorkQueue::StackCapacity::kSmall>();
template bool WorkQueue::RemoveAllCancelledTasksFromFrontImpl<WorkQueue::StackCapacity::kMedium>();
template bool WorkQueue::RemoveAllCancelledTasksFromFrontImpl<WorkQueue::StackCapacity::kLarge>();
template bool WorkQueue::RemoveAllCancelledTasksFromFrontImpl<WorkQueue::StackCapacity::kDefault>();
