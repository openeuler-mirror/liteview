...
template <typename T> struct CustomCompare {
    bool operator()(const T& lhs, const T& rhs) const
    {
        return std::tie(lhs.delayed_run_time, lhs.name) > std::tie(rhs.delayed_run_time, rhs.name);
    }
};
...
public:
    ...
    HierarchicalTimingWheelHandle* handle() const
    {
        return handle_.get();
    }
...
TEST(HierarchicalTimingWheelTest, SimpleTest)
{
    ...
    HierarchicalTimingWheel<Task, 4, 100, 500> hierarchical_timing_wheel { baseline };

    EXPECT_EQ(hierarchical_timing_wheel.Size(), 0u);

    auto* handle = hierarchical_timing_wheel.Insert(Task { baseline + Microseconds(100) })->handle();
...
TEST(HierarchicalTimingWheelTest, InsertAllDistinctElements)
{
    ...
    const TimeDelta kDelay[kHierarchyCount] = { Microseconds(100), Microseconds(500), Milliseconds(50), Seconds(5), Seconds(500), Seconds(50000) };

    HierarchicalTimingWheelHandle* handles[kHierarchyCount];
    for (size_t i = 0; i < kHierarchyCount; i++) {
        handles[i] = hierarchical_timing_wheel.Insert(Task { baseline + kDelay[i] })->handle();
    }
...
TEST(HierarchicalTimingWheelTest, InsertSimilarElements)
{
    ...
    const TimeDelta kDelay[kTotalElements] = { Microseconds(500), Milliseconds(21), Milliseconds(49) };
...
TEST(HierarchicalTimingWheelTest, UpdateOneElement)
{
    ...
    // element.
    const TimeDelta kTimeDelta[] = { Seconds(50000) - Seconds(500), Seconds(500) - Seconds(5), Seconds(5) - Milliseconds(50),
        Milliseconds(50) - Microseconds(500), Microseconds(500) - Microseconds(100), Microseconds(100) };

    // Create time delta to insert a task at the end of the hierarchy.
    const TimeTicks delayed_run_time = baseline + Seconds(50000);

    HierarchicalTimingWheelHandle* handle = hierarchical_timing_wheel.Insert(Task { delayed_run_time })->handle();
...
TEST(HierarchicalTimingWheelTest, RemoveElements)
{
    ...
    std::vector<Task> expired_tasks = hierarchical_timing_wheel.Update(baseline + Seconds(50000));
...
TEST(HierarchicalTimingWheelTest, TopSimilarElements)
{
    ...
    const TimeDelta kDelay[kTotalElements] = { Microseconds(100), Microseconds(200), Microseconds(300) };
...
TEST(HierarchicalTimingWheelTest, CustomComparator)
{
    ...
    const TimeTicks baseline = TimeTicks::Now();
    HierarchicalTimingWheel<Task, 4, 100, 500, DefaultHierarchicalTimingWheelHandleAccessor<Task>, GetDelayedRunTime<Task>, CustomCompare<Task>>
