...
public:
    void SetUp() override
    {
        work_queue_sets_ = std::make_unique<WorkQueueSets>("test", &mock_observer_, SequenceManager::Settings());
    }
...
protected:
    ...
    Task FakeTaskWithTaskOrder(TaskOrder task_order)
    {
        Task fake_task(PostedTask(nullptr, BindOnce([] {}), FROM_HERE, task_order.delayed_run_time(), subtle::DelayPolicy::kFlexibleNoSooner),
            EnqueueOrder::FromIntForTesting(task_order.sequence_num()), task_order.enqueue_order());
        return fake_task;
    }
...
TEST_F(WorkQueueSetsTest, GetOldestQueueAndTaskOrderInSet_TaskOrder)
{
    ...
    absl::optional<WorkQueueAndTaskOrder> work_queue_and_task_order = work_queue_sets_->GetOldestQueueAndTaskOrderInSet(set);
...
TEST_F(WorkQueueSetsTest, GetOldestQueueAndTaskOrderInSet_MultipleAgesInSet)
{
    ...
    absl::optional<WorkQueueAndTaskOrder> queue_and_order = work_queue_sets_->GetOldestQueueAndTaskOrderInSet(set);
