...
public:
    ...
    absl::optional<SelectedTask> SelectNextTask(LazyNow& lazy_now, SelectTaskOption option) override
    {
        if (tasks_.empty())
            return absl::nullopt;
        ...
        tasks_.pop();
        return SelectedTask(running_stack_.back(), TaskExecutionTraceLogger(), TaskQueue::QueuePriority::kNormalPriority, QueueName::TEST_TQ);
    }

    void DidRunTask(LazyNow& lazy_now) override
    {
        running_stack_.pop_back();
    }
    ...
    absl::optional<WakeUp> GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option) const override
    {
        if (tasks_.empty())
            return absl::nullopt;
        if (option == SequencedTaskSource::SelectTaskOption::kSkipDelayedTask && !tasks_.front().delayed_run_time.is_null()) {
...
public:
    ThreadControllerWithMessagePumpTest()
        ...
        , settings_(SequenceManager::Settings::Builder().SetTickClock(&clock_).Build())
        , thread_controller_(std::unique_ptr<MessagePump>(message_pump_), settings_)
        , task_source_(&clock_)
    {
...
TEST_F(ThreadControllerWithMessagePumpTest, PeriodicYieldingToNative)
{
    ...
    EXPECT_CALL(*message_pump_, Run(_)).WillOnce(Invoke([&](MessagePump::Delegate* delegate) {
        ...
        // B:
        thread_controller_.EnablePeriodicYieldingToNative(base::Milliseconds(3));
        ...
        // G:
        thread_controller_.EnablePeriodicYieldingToNative(base::TimeDelta::Max());
...
#endif // BUILDFLAG(IS_ANDROID)
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveAdvancedNesting)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveNestedNativeLoop)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveUnusedNativeLoop)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveNestedNativeLoopWithoutAllowance)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveMultipleNativeLoopsUnderOneApplicationTask)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveNativeLoopsReachingIdle)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
...
TEST_F(ThreadControllerWithMessagePumpTest, ThreadControllerActiveQuitNestedWhileApplicationIdle)
{
    ThreadTaskRunnerHandle handle(MakeRefCounted<FakeTaskRunner>());
