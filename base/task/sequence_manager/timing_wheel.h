...
private:
    template <typename T, size_t WheelSize, typename TimingWheelHandleAccessor, typename GetDelayedRunTime> friend class TimingWheel;
...
// due at.
template <typename T, size_t WheelSize, typename TimingWheelHandleAccessor, typename GetDelayedRunTime> class TimingWheel {
public:
    // Constructs a TimingWheel instance where each bucket corresponds to a
    // TimeDelta of |time_delta_per_bucket_|.
    explicit TimingWheel(TimeDelta time_delta_per_bucket, const GetDelayedRunTime& get_delayed_run_time = GetDelayedRunTime())
    ...
    typename std::vector<T>::const_iterator Insert(T element, const TimeDelta delay)
    {
        ...
        timing_wheel_handle_accessor.SetTimingWheelHandle(&buckets_[bucket_index][element_index], TimingWheelHandle(bucket_index, element_index));
    ...
    void Remove(TimingWheelHandle handle)
    {
        ...
        if (element_index != last_index_of_bucket) {
            timing_wheel_handle_accessor.SetTimingWheelHandle(&bucket[last_index_of_bucket], TimingWheelHandle(bucket_index, element_index));
    ...
    void AdvanceTimeAndRemoveExpiredElements(const TimeDelta time_delta, std::vector<T>& expired_elements)
    {
        const size_t nb_buckets_passed = (time_passed_ + time_delta) / time_delta_per_bucket_ + 1;
        const size_t new_bucket_index = (last_updated_bucket_index_ + nb_buckets_passed) % WheelSize;
        const TimeDelta new_time_passed = (time_passed_ + time_delta) % time_delta_per_bucket_;

        // Ensures each bucket is iterated over at most once if |nb_buckets_passed|
        // is bigger than the total number of buckets.
        const size_t nb_buckets_to_traverse = std::min(nb_buckets_passed, WheelSize);
    ...
    typename std::vector<T>::const_reference Top()
    {
        ...
        for (size_t i = 0; i < WheelSize; i++) {
            ...
            auto it = std::min_element(
                bucket.begin(), bucket.end(), [this](const T& a, const T& b) { return get_delayed_run_time_(a) > get_delayed_run_time_(b); });
            return *it;
        }
...
private:
    ...
    {
        return bucket_index < WheelSize && element_index < buckets_[bucket_index].size();
    }
    ...
    {
        const size_t nb_buckets_passed = (delay + time_passed_) / time_delta_per_bucket_;
    ...
    void ExtractElementsFromBucket(const size_t bucket_index, std::vector<T>& expired_elements)
    {
