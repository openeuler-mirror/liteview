...
void ThreadControllerImpl::SetNextDelayedDoWork(LazyNow* lazy_now, absl::optional<WakeUp> wake_up)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(associated_thread_->sequence_checker);
    DCHECK(sequence_);
...
void ThreadControllerImpl::DoWork(WorkType work_type)
{
    ...
    for (int i = 0; i < main_sequence_only().work_batch_size_; i++) {
        ...
        absl::optional<SequencedTaskSource::SelectedTask> selected_task = sequence_->SelectNextTask(lazy_now_select_task);
...

        {
            ...
            if (lazy_now_after_run_task.has_value()) {
                recent_time = absl::optional<base::TimeTicks>(lazy_now_after_run_task.Now());
            } else {
                recent_time.reset();
            }
        }
    ...
    absl::optional<WakeUp> next_wake_up = sequence_->GetPendingWakeUp(&lazy_now_after_work);
    // The OnSystemIdle callback allows the TimeDomains to advance virtual time
    // in which case we now have immediate work to do.
    if ((next_wake_up && next_wake_up->is_immediate()) || sequence_->OnSystemIdle()) {
...
void ThreadControllerImpl::EnablePeriodicYieldingToNative(base::TimeDelta delta)
{
    NOTREACHED();
}
