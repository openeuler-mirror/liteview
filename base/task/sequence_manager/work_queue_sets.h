...
public:
    ...
    // O(1)
    absl::optional<WorkQueueAndTaskOrder> GetOldestQueueAndTaskOrderInSet(size_t set_index) const;

#if DCHECK_IS_ON()
    // O(1)
    absl::optional<WorkQueueAndTaskOrder> GetRandomQueueAndTaskOrderInSet(size_t set_index) const;
#endif
...
private:
    ...
    std::array<IntrusiveHeap<OldestTaskOrder, std::greater<>>, TaskQueue::kQueuePriorityCount> work_queue_heaps_;
