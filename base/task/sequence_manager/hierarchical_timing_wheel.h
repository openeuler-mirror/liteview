...
public:
    ...
    HierarchicalTimingWheelHandle(const HierarchicalTimingWheelHandle& other) = default;
    HierarchicalTimingWheelHandle(HierarchicalTimingWheelHandle&& other) noexcept;

    HierarchicalTimingWheelHandle& operator=(const HierarchicalTimingWheelHandle& other) = default;
    HierarchicalTimingWheelHandle& operator=(HierarchicalTimingWheelHandle&& other) noexcept;
...
template <typename T> struct DefaultHierarchicalTimingWheelHandleAccessor {
    void SetTimingWheelHandle(T* element, internal::TimingWheelHandle handle) const
    {
...
template <typename T> struct GetDelayedRunTime {
...
template <typename T, size_t TotalWheels, size_t WheelSize, size_t SmallestBucketDeltaInMicroseconds,
    typename HierarchicalTimingWheelHandleAccessor = DefaultHierarchicalTimingWheelHandleAccessor<T>, typename GetDelayedRunTime = GetDelayedRunTime<T>,
...
public:
    ...
    explicit HierarchicalTimingWheel(TimeTicks last_wakeup,
        const HierarchicalTimingWheelHandleAccessor& hierarchical_timing_wheel_handle_accessor = HierarchicalTimingWheelHandleAccessor(),
        const GetDelayedRunTime& get_delayed_run_time = GetDelayedRunTime(), const Compare compare = Compare())
        ...
        , hierarchical_timing_wheel_handle_accessor_(hierarchical_timing_wheel_handle_accessor)
    ...
    size_t Size()
    {
        return small_delay_heap_.size() + large_delay_heap_.size()
            + std::accumulate(std::begin(wheels_), std::end(wheels_), 0, [](size_t i, auto& wheel) { return wheel.total_elements() + i; });
    }
    ...
    typename std::vector<T>::const_iterator Insert(T element)
    {
        ...
        if (IsHeap(hierarchy_index)) {
            ...
            hierarchical_timing_wheel_handle_accessor_.SetHierarchyIndex(&element, hierarchy_index);
    ...
    std::vector<T> Update(TimeTicks now)
    {
        ...
        while (!small_delay_heap_.empty() && get_delayed_run_time_(small_delay_heap_.top()) <= now) {
        ...
        const size_t timing_wheels_delay_upperbound = SmallestBucketDeltaInMicroseconds * Pow(WheelSize, TotalWheels);
        const TimeTicks timing_wheels_maximum_delayed_run_time = now + Milliseconds(timing_wheels_delay_upperbound);
        last_wakeup_ = now;

        for (size_t wheel_index = 0; wheel_index < TotalWheels; wheel_index++) {
            wheels_[wheel_index].AdvanceTimeAndRemoveExpiredElements(time_delta, elements);
        }
        ...
        while (!large_delay_heap_.empty() && get_delayed_run_time_(large_delay_heap_.top()) < timing_wheels_maximum_delayed_run_time) {
...
private:
    ...
    using Wheel = typename internal::TimingWheel<T, WheelSize, HierarchicalTimingWheelHandleAccessor, GetDelayedRunTime>;

    // Generates |wheels_| at compile time.
    template <size_t... I> static std::array<Wheel, TotalWheels> MakeWheels(std::index_sequence<I...>)
    {
        return { (Wheel(Microseconds(SmallestBucketDeltaInMicroseconds * Pow(WheelSize, I))))... };
    }

    // The timing wheels where the elements are added according to their delay.
    std::array<Wheel, TotalWheels> wheels_ = MakeWheels(std::make_index_sequence<TotalWheels> {});
    ...
    // wheel.
    IntrusiveHeap<T, Compare, HierarchicalTimingWheelHandleAccessor> small_delay_heap_;
    IntrusiveHeap<T, Compare, HierarchicalTimingWheelHandleAccessor> large_delay_heap_;

    // The last time when the timing wheels were updated.
    TimeTicks last_wakeup_;

    HierarchicalTimingWheelHandleAccessor hierarchical_timing_wheel_handle_accessor_;
