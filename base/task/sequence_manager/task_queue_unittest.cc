...
public:
    ...
    {
        scoped_feature_list_.InitWithFeatureState(kRemoveCanceledTasksInTaskQueue, feature_enabled);
...
TEST(TaskQueueTest, CanceledTaskRemovedIfFeatureEnabled)
{
    for (bool feature_enabled : { false, true }) {
        ScopedNoWakeUpsForCanceledTasks scoped_no_wake_ups_for_canceled_tasks(feature_enabled);
