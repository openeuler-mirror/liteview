...
public:
    ...
    TimingWheelHandle* handle() const
    {
        return handle_.get();
    }
...
// underlying type.
template <typename T> struct TimingWheelHandleAccessor {
...
struct GetDelayedRunTime {
    template <typename T> TimeTicks operator()(const T& element)
    {
...
{
    TimingWheel<Element, kWheelSize, TimingWheelHandleAccessor<Element>, GetDelayedRunTime> timing_wheel(kBucketTimeDelta);
...
TEST(TimingWheelTest, AdvancingFullWheel)
{
    ...
    timing_wheel.AdvanceTimeAndRemoveExpiredElements(kTimePassed, expired_elements);
...
TEST(TimingWheelTest, AdvancingMoreThanFullWheel)
{
    TimingWheel<Element, kWheelSize, TimingWheelHandleAccessor<Element>, GetDelayedRunTime> timing_wheel(kBucketTimeDelta);
    std::vector<Element> expired_elements;
    const TimeDelta kTimePassed = kBucketTimeDelta * kWheelSize + kBucketTimeDelta;
...
TEST(TimingWheelTest, InsertMultipleElementsInDifferentBuckets)
{
    ...
    timing_wheel.AdvanceTimeAndRemoveExpiredElements(kTestDelay1, expired_elements);
    ...
    // last elements.
    timing_wheel.AdvanceTimeAndRemoveExpiredElements(kTestDelay2 - kTestDelay1, expired_elements);
...
TEST(TimingWheelTest, InsertMultipleSimilarElementsInSameBucket)
{
    ...
    std::vector<Element> expired_elements;
    timing_wheel.AdvanceTimeAndRemoveExpiredElements(kTestDelay, expired_elements);
...
TEST(TimingWheelTest, RemoveElement)
{
    ...
    const TimeTicks baseline = TimeTicks::Now();
    const TimeDelta kLastBucketMinDelay = kLastBucketIndex * kBucketTimeDelta;
    const TimeDelta kLastBucketMaxDelay = (kLastBucketIndex + 1) * kBucketTimeDelta - Microseconds(1u);
    const TimeDelta kTimePassed = kBucketTimeDelta * kLastBucketIndex;

    timing_wheel.Insert({ baseline + kLastBucketMinDelay }, kLastBucketMinDelay);
    const TimingWheelHandle* handle = timing_wheel.Insert({ baseline + kLastBucketMaxDelay }, kLastBucketMaxDelay)->handle();
