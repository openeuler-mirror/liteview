...
public:
    ...
    static std::unique_ptr<SequenceManagerImpl> CreateOnCurrentThread(SequenceManager::Settings settings = SequenceManager::Settings());
    ...
    static void MaybeEmitTaskDetails(perfetto::EventContext& ctx, const SequencedTaskSource::SelectedTask& selected_task);
    ...
    scoped_refptr<TaskQueue> CreateTaskQueue(const TaskQueue::Spec& spec) override;
    std::string DescribeAllPendingTasks() const override;
    std::unique_ptr<NativeWorkHandle> OnNativeWorkPending(TaskQueue::QueuePriority priority) override;
    ...
    absl::optional<SelectedTask> SelectNextTask(LazyNow& lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) override;
    void DidRunTask(LazyNow& lazy_now) override;
    void RemoveAllCanceledDelayedTasksFromFront(LazyNow* lazy_now) override;
    absl::optional<WakeUp> GetPendingWakeUp(LazyNow* lazy_now, SelectTaskOption option = SelectTaskOption::kDefault) const override;
    ...
    void ShutdownTaskQueueGracefully(std::unique_ptr<internal::TaskQueueImpl> task_queue);
...
private:
    ...
    struct MainThreadOnly {
        ...
        std::map<internal::TaskQueueImpl*, std::unique_ptr<internal::TaskQueueImpl>> queues_to_gracefully_shutdown;
    ...
    std::unique_ptr<internal::TaskQueueImpl> CreateTaskQueueImpl(const TaskQueue::Spec& spec) override;
    ...
    // in SelectNextTask().
    absl::optional<SelectedTask> SelectNextTaskImpl(LazyNow& lazy_now, SelectTaskOption option);
    ...
    absl::optional<WakeUp> GetNextDelayedWakeUpWithOption(SelectTaskOption option) const;
    ...
    absl::optional<WakeUp> AdjustWakeUp(absl::optional<WakeUp> wake_up, LazyNow* lazy_now) const;
