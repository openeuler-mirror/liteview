...
BASE_FEATURE(kSweepCancelledTasks, "SweepCancelledTasks", FEATURE_ENABLED_BY_DEFAULT);
...
bool g_is_sweep_cancelled_tasks_enabled = kSweepCancelledTasks.default_state == FEATURE_ENABLED_BY_DEFAULT;
#if BUILDFLAG(IS_WIN)
...
TaskQueueImpl::TaskRunner::TaskRunner(
    scoped_refptr<GuardedTaskPoster> task_poster, scoped_refptr<const AssociatedThreadId> associated_thread, TaskType task_type)
    ...
    subtle::PostDelayedTaskPassKey pass_key, const Location& location, OnceClosure callback, TimeTicks delayed_run_time, base::subtle::DelayPolicy delay_policy)
{
    if (!g_is_remove_canceled_tasks_in_task_queue_enabled) {
        ...
        return SequencedTaskRunner::PostCancelableDelayedTaskAt(pass_key, location, std::move(callback), delayed_run_time, delay_policy);
    }
...
{
    if (!g_is_remove_canceled_tasks_in_task_queue_enabled) {
        return SequencedTaskRunner::PostCancelableDelayedTask(pass_key, location, std::move(callback), delay);
    }
...
void TaskQueueImpl::InitializeFeatures()
{
    ...
    g_is_sweep_cancelled_tasks_enabled = FeatureList::IsEnabled(kSweepCancelledTasks);
#if BUILDFLAG(IS_WIN)
...
void TaskQueueImpl::ApplyRemoveCanceledTasksInTaskQueue()
{
    ...
    // feature here.
    DCHECK_EQ(g_is_remove_canceled_tasks_in_task_queue_enabled, kRemoveCanceledTasksInTaskQueue.default_state == FEATURE_ENABLED_BY_DEFAULT);
    g_is_remove_canceled_tasks_in_task_queue_enabled = FeatureList::IsEnabled(kRemoveCanceledTasksInTaskQueue);
}

// static
void TaskQueueImpl::ResetRemoveCanceledTasksInTaskQueueForTesting()
{
    g_is_remove_canceled_tasks_in_task_queue_enabled = kRemoveCanceledTasksInTaskQueue.default_state == FEATURE_ENABLED_BY_DEFAULT;
}
...
const char* TaskQueueImpl::GetName() const
{
    ...
    return sequence_manager::GetQueueName_Name(name_);
}
...
TimeDelta TaskQueueImpl::GetTaskDelayAdjustment(CurrentThread current_thread)
{
    ...
    if (current_thread == TaskQueueImpl::CurrentThread::kNotMainThread) {
        ...
        return sequence_manager_->settings().per_priority_cross_thread_task_delay[any_thread_.queue_set_index];
    } else {
        return sequence_manager_->settings().per_priority_same_thread_task_delay[main_thread_only().immediate_work_queue->work_queue_set_index()];
    }
#else
    // No delay adjustment.
    return TimeDelta();
...
void TaskQueueImpl::RecordQueuingDelayedTaskMetrics(const Task& pending_task, LazyNow* lazy_now)
{
    ...
    if (((lazy_now->Now() - TimeTicks::UnixEpoch()).InMicroseconds() ^ pending_task.sequence_num) % kSampleRate == 0) {
        ...
        UMA_HISTOGRAM_LONG_TIMES("Scheduler.TaskQueueImpl.PostDelayedTaskDelay", delay);
        UMA_HISTOGRAM_COUNTS_1000("Scheduler.TaskQueueImpl.DelayedIncomingQueueSize", static_cast<int>(main_thread_only().delayed_incoming_queue.size()));
    }
}
...
absl::optional<WakeUp> TaskQueueImpl::GetNextDesiredWakeUp()
{
    ...
    WakeUpResolution resolution = has_pending_high_resolution_tasks() && GetQueuePriority() <= TaskQueue::QueuePriority::kNormalPriority
        ? WakeUpResolution::kHigh
        : WakeUpResolution::kLow;

    const auto& top_task = main_thread_only().delayed_incoming_queue.top();
    return WakeUp { top_task.delayed_run_time, top_task.leeway, resolution, top_task.delay_policy };
}
...
void TaskQueueImpl::SetQueuePriority(TaskQueue::QueuePriority priority)
{
    ...
    static_assert(TaskQueue::QueuePriority::kLowPriority > TaskQueue::QueuePriority::kNormalPriority, "Priorities are not ordered as expected");
...
Task TaskQueueImpl::MakeDelayedTask(PostedTask delayed_task, LazyNow* lazy_now) const
{
    ...
    if (explicit_high_resolution_timer_win) {
        resolution = delayed_task.delay_policy == base::subtle::DelayPolicy::kPrecise ? WakeUpResolution::kHigh : WakeUpResolution::kLow;
    } else if (delay < (2 * base::Milliseconds(Time::kMinLowResolutionThresholdMs))) {
...
void TaskQueueImpl::SetNextWakeUp(LazyNow* lazy_now, absl::optional<WakeUp> wake_up)
{
...
bool TaskQueueImpl::ShouldReportIpcTaskQueuedFromAnyThreadLocked(base::TimeDelta* time_since_disabled)
{
    ...
    if (any_thread_.tracing_only.is_enabled || any_thread_.tracing_only.should_report_posted_tasks_when_disabled) {
        return false;
    }
