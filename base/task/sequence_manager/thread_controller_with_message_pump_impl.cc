...
BASE_FEATURE(kRunTasksByBatches, "RunTasksByBatches", base::FEATURE_DISABLED_BY_DEFAULT);
...
TimeTicks WakeUpRunTime(const WakeUp& wake_up)
{
    if (g_align_wake_ups.load(std::memory_order_relaxed)) {
        TimeTicks aligned_run_time = wake_up.earliest_time().SnappedToNextTick(TimeTicks(), base::GetTaskLeeway());
...
void ThreadControllerWithMessagePumpImpl::InitializeFeatures()
{
    ...
    g_run_tasks_by_batches.store(FeatureList::IsEnabled(kRunTasksByBatches), std::memory_order_relaxed);
}

// static
void ThreadControllerWithMessagePumpImpl::ResetFeatures()
{
    g_align_wake_ups.store(kAlignWakeUps.default_state == FEATURE_ENABLED_BY_DEFAULT, std::memory_order_relaxed);
...
void ThreadControllerWithMessagePumpImpl::SetTimerSlack(TimerSlack timer_slack)
{
...
void ThreadControllerWithMessagePumpImpl::SetNextDelayedDoWork(LazyNow* lazy_now, absl::optional<WakeUp> wake_up)
{
    DCHECK(!wake_up || !wake_up->is_immediate());
    TimeTicks run_time = wake_up.has_value() ? WakeUpRunTime(*wake_up) : TimeTicks::Max();
...
void ThreadControllerWithMessagePumpImpl::InitializeThreadTaskRunnerHandle()
{
    ...
    main_thread_only().thread_task_runner_handle = std::make_unique<ThreadTaskRunnerHandle>(task_runner_);
...
MessagePump::Delegate::NextWorkInfo ThreadControllerWithMessagePumpImpl::DoWork()
{
    ...
    // for this experiment.
    bool yield_after_every_batch_of_8_ms = g_run_tasks_by_batches.load(std::memory_order_relaxed);
    ...
    // the allowed time
    bool yield_with_delay_after_batch_of_period = !periodic_yielding_to_native_interval_.is_max() && work_interval >= periodic_yielding_to_native_interval_;
    ...
    if (yield_after_every_batch_of_one || yield_after_every_batch_of_8_ms || yield_with_delay_after_batch_of_period) {
        next_work_info.yield_to_native = true;
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("sequence_manager"), "ThreadControllerImpl_YieldToNative");
    }
    ...
    next_work_info.delayed_run_time = CapAtOneDay(main_thread_only().next_delayed_do_work, &continuation_lazy_now);
    next_work_info.recent_now = continuation_lazy_now.Now();
    return next_work_info;
}

WorkDetails ThreadControllerWithMessagePumpImpl::DoWorkImpl(LazyNow* continuation_lazy_now)
{
    ...
    if (!main_thread_only().task_execution_allowed) {
        ...
        return WorkDetails { WakeUp { main_thread_only().quit_runloop_after }, Nanoseconds(0) };
    }
    ...
    const absl::optional<base::TimeTicks> start_time = batch_duration.is_zero() ? absl::nullopt : absl::optional<base::TimeTicks>(time_source_->NowTicks());
    ...
    for (int num_tasks_executed = 0;
         (!batch_duration.is_zero() && work_executed < batch_duration) || (batch_duration.is_zero() && num_tasks_executed < main_thread_only().work_batch_size);
         ++num_tasks_executed) {
        ...
        task_annotator_.RunTask("ThreadControllerImpl::RunTask", selected_task->task, [&selected_task](perfetto::EventContext& ctx) {
            if (selected_task->task_execution_trace_logger)
                selected_task->task_execution_trace_logger.Run(ctx, selected_task->task);
            SequenceManagerImpl::MaybeEmitTaskDetails(ctx, selected_task.value());
        });
    ...
    const SequencedTaskSource::SelectTaskOption select_task_option = power_monitor_.IsProcessInPowerSuspendState()
        ? SequencedTaskSource::SelectTaskOption::kSkipDelayedTask
        : SequencedTaskSource::SelectTaskOption::kDefault;
    main_thread_only().task_source->RemoveAllCanceledDelayedTasksFromFront(continuation_lazy_now);
    return { main_thread_only().task_source->GetPendingWakeUp(continuation_lazy_now, select_task_option), work_executed };
}
...
void ThreadControllerWithMessagePumpImpl::SetTaskExecutionAllowed(bool allowed)
{
    if (allowed) {
...
void ThreadControllerWithMessagePumpImpl::EnablePeriodicYieldingToNative(base::TimeDelta delta)
{
...
base::TimeDelta ThreadControllerWithMessagePumpImpl::GetAlternationInterval()
{
    ...
    return periodic_yielding_to_native_interval_.is_max() ? base::Milliseconds(0) : periodic_yielding_to_native_interval_;
#else
    return g_run_tasks_by_batches.load(std::memory_order_relaxed) ? base::Milliseconds(8) : base::Milliseconds(0);
#endif
}
