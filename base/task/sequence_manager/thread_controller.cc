...
void ThreadController::EnableMessagePumpTimeKeeperMetrics(const char* thread_name)
{
...
void ThreadController::RunLevelTracker::EnableTimeKeeperMetrics(const char* thread_name)
{
    time_keeper_.EnableRecording(thread_name);
}

void ThreadController::RunLevelTracker::TimeKeeper::EnableRecording(const char* thread_name)
{
    DCHECK(!histogram_);
    histogram_ = LinearHistogram::FactoryGet(JoinString({ "Scheduling.MessagePumpTimeKeeper", thread_name }, "."), 1, Phase::kMaxValue, Phase::kMaxValue + 1,
        base::HistogramBase::kUmaTargetedHistogramFlag);

#if BUILDFLAG(ENABLE_BASE_TRACING)
    perfetto_track_.emplace(reinterpret_cast<uint64_t>(this), perfetto::ThreadTrack::Current());
