...
protected:
    void SetUp() final
    {
        ...
        for (size_t i = 0; i < kTaskQueueCount; i++) {
            EXPECT_EQ(TaskQueue::kNormalPriority, task_queues_[i]->GetQueuePriority()) << i;
    ...
    const size_t kTaskQueueCount = static_cast<size_t>(TaskQueue::QueuePriority::kQueuePriorityCount);
...
TEST_F(TaskQueueSelectorTest, TestHighestPriority)
{
    ...
    PushTasks(queue_order, 5);
    selector_.SetQueuePriority(task_queues_[2].get(), TaskQueue::kHighestPriority);
...
TEST_F(TaskQueueSelectorTest, TestBestEffortPriority)
{
    size_t queue_order[] = { 0, 1, 2, 3, 4 };
    PushTasks(queue_order, 5);
    selector_.SetQueuePriority(task_queues_[0].get(), TaskQueue::kBestEffortPriority);
    selector_.SetQueuePriority(task_queues_[2].get(), TaskQueue::kLowPriority);
    selector_.SetQueuePriority(task_queues_[3].get(), TaskQueue::kHighestPriority);
...
TEST_F(TaskQueueSelectorTest, TestControlPriority)
{
    size_t queue_order[] = { 0, 1, 2, 3, 4 };
    PushTasks(queue_order, 5);
    selector_.SetQueuePriority(task_queues_[4].get(), TaskQueue::kControlPriority);
...
{
    selector_.SetQueuePriority(task_queues_[1].get(), TaskQueue::kHighestPriority);
    MockObserver mock_observer;
    selector_.SetTaskQueueSelectorObserver(&mock_observer);
...
TEST_F(TaskQueueSelectorTest, TestDisableEnable)
{
    ...
    selector_.SetQueuePriority(task_queues_[2].get(), TaskQueue::kBestEffortPriority);
...
{
    EXPECT_EQ(nullptr, selector_.ChooseWithPriority<TaskQueueSelectorForTest::SetOperationOldest>(TaskQueue::kNormalPriority));
}
...
TEST_F(TaskQueueSelectorTest, ChooseWithPriority_OnlyDelayed)
{
    ...
    EXPECT_EQ(task_queues_[0]->delayed_work_queue(), selector_.ChooseWithPriority<TaskQueueSelectorForTest::SetOperationOldest>(TaskQueue::kNormalPriority));
}
...
TEST_F(TaskQueueSelectorTest, ChooseWithPriority_OnlyImmediate)
{
    ...
    EXPECT_EQ(task_queues_[0]->immediate_work_queue(), selector_.ChooseWithPriority<TaskQueueSelectorForTest::SetOperationOldest>(TaskQueue::kNormalPriority));
}
...
TEST_P(ChooseWithPriorityTest, RoundRobinTest)
{
    ...
    WorkQueue* chosen_work_queue = selector_.ChooseWithPriority<TaskQueueSelectorForTest::SetOperationOldest>(TaskQueue::kNormalPriority);
...
TEST_F(ActivePriorityTrackerTest, SetPriorityActiveAndInactive)
{
    ...
    EXPECT_FALSE(active_priority_tracker_.IsActive(TaskQueue::QueuePriority::kNormalPriority));

    active_priority_tracker_.SetActive(TaskQueue::QueuePriority::kNormalPriority, true);

    EXPECT_TRUE(active_priority_tracker_.HasActivePriority());
    EXPECT_TRUE(active_priority_tracker_.IsActive(TaskQueue::QueuePriority::kNormalPriority));

    active_priority_tracker_.SetActive(TaskQueue::QueuePriority::kNormalPriority, false);
