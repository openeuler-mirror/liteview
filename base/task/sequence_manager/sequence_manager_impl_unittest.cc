...
std::string GetTestNameSuffix(const testing::TestParamInfo<std::tuple<RunnerType, WakeUpType>>& info)
{
    return StrCat({ "With", ToString(std::get<0>(info.param)).substr(1), ToString(std::get<1>(info.param)) });
}
...
public:
    FixtureWithMockTaskRunner()
        ...
        , sequence_manager_(SequenceManagerForTest::Create(nullptr, ThreadTaskRunnerHandle::Get(), mock_tick_clock(),
...
public:
    ...
    {
        if (wake_up_type_ == WakeUpType::kAlign) {
            return (start_time_ + delta).SnappedToNextTick(TimeTicks(), Milliseconds(8));
        }
        return start_time_ + delta;
    }
...
class SequenceManagerTest : public testing::TestWithParam<std::tuple<RunnerType, WakeUpType>>, public Fixture {
public:
    SequenceManagerTest()
    {
    ...
    scoped_refptr<TestTaskQueue> CreateTaskQueue(TaskQueue::Spec spec = TaskQueue::Spec(QueueName::TEST_TQ))
    {
        return sequence_manager()->CreateTaskQueueWithType<TestTaskQueue>(spec);
    }

    std::vector<scoped_refptr<TestTaskQueue>> CreateTaskQueues(size_t num_queues)
    {
...
auto GetTestTypes()
{
    return testing::Values(std::make_tuple(RunnerType::kMessagePump, WakeUpType::kDefault), std::make_tuple(RunnerType::kMessagePump, WakeUpType::kAlign),
...
void PostFromNestedRunloop(scoped_refptr<TestTaskQueue> runner, std::vector<std::pair<OnceClosure, bool>>* tasks)
{
    for (std::pair<OnceClosure, bool>& pair : *tasks) {
        if (pair.second) {
            runner->task_runner()->PostTask(FROM_HERE, std::move(pair.first));
        } else {
            runner->task_runner()->PostNonNestableTask(FROM_HERE, std::move(pair.first));
        }
    }
...
TEST_P(SequenceManagerTest, NonNestableTasksDoesntExecuteInNestedLoop)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostFromNestedRunloop, queue, Unretained(&tasks_to_post_from_nested_loop)));
...
void InsertFenceAndPostTestTask(int id, std::vector<EnqueueOrder>* run_order, scoped_refptr<TestTaskQueue> task_queue, SequenceManagerForTest* manager)
{
...
TEST_P(SequenceManagerTest, TaskQueueDisabledFromNestedLoop)
{
    ...
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&InsertFenceAndPostTestTask, 2, &run_order, queue, sequence_manager()), true));
...
void RecordTimeAndQueueTask(
    std::vector<std::pair<scoped_refptr<TestTaskQueue>, TimeTicks>>* run_times, scoped_refptr<TestTaskQueue> task_queue, const TickClock* clock)
{
...
{
    scoped_refptr<TestTaskQueue> queue = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetDelayedFencesAllowed(true));
...
TEST_P(SequenceManagerTest, DelayedFence_TakeIncomingImmediateQueue)
{
    ...
    const auto kStartTime = mock_tick_clock()->NowTicks();
    scoped_refptr<TestTaskQueue> queue1 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetDelayedFencesAllowed(true));
    scoped_refptr<TestTaskQueue> queue2 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST2_TQ).SetDelayedFencesAllowed(true));
    ...
    queue1->task_runner()->PostTask(FROM_HERE, BindOnce(&RecordTimeAndQueueTask, &run_times, queue1, mock_tick_clock()));
    // Force reload of immediate work queue. In real life the same effect can be
    // achieved with cross-thread posting.
    ...
    queue2->task_runner()->PostTask(FROM_HERE, BindOnce(&RecordTimeAndQueueTask, &run_times, queue2, mock_tick_clock()));

    FastForwardUntilNoTasksRemain();

    EXPECT_THAT(run_times, ElementsAre(std::make_pair(queue1, kStartTime + Milliseconds(300)), std::make_pair(queue2, kStartTime + Milliseconds(300))));
}

namespace {

void ReentrantTestTask(scoped_refptr<TestTaskQueue> runner, int countdown, std::vector<EnqueueOrder>* out_result)
{
    out_result->push_back(EnqueueOrder::FromIntForTesting(countdown));
    if (--countdown) {
        runner->task_runner()->PostTask(FROM_HERE, BindOnce(&ReentrantTestTask, runner, countdown, out_result));
    }
}
...
TEST_P(SequenceManagerTest, ReentrantPosting)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&ReentrantTestTask, queue, 3, &run_order));
...
void PostTaskToRunner(scoped_refptr<TestTaskQueue> runner, std::vector<EnqueueOrder>* run_order)
{
...
TEST_P(SequenceManagerTest, PostFromThread)
{
    ...
    Thread thread("TestThread");
    thread.Start();
    thread.task_runner()->PostTask(FROM_HERE, BindOnce(&PostTaskToRunner, queue, &run_order));
    thread.Stop();
...
{
    (*run_count)++;
    runner->task_runner()->PostTask(FROM_HERE, BindOnce(&RePostingTestTask, Unretained(runner.get()), run_count));
}
...
TEST_P(SequenceManagerTest, DoWorkCantPostItselfMultipleTimes)
{
    ...
    int run_count = 0;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RePostingTestTask, queue, &run_count));
...
void RemoveQueueObserverTask(scoped_refptr<TestTaskQueue> queue, TaskObserver* observer)
{
    queue->RemoveTaskObserver(observer);
}
...
TEST_P(SequenceManagerTest, QueueTaskObserverRemovingInsideTask)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&RemoveQueueObserverTask, queue, &observer));
...
TEST_P(SequenceManagerTest, GetAndClearSystemIsQuiescentBit)
{
    ...
    scoped_refptr<TestTaskQueue> queue0 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ).SetShouldMonitorQuiescence(true));
    scoped_refptr<TestTaskQueue> queue1 = CreateTaskQueue(TaskQueue::Spec(QueueName::TEST2_TQ).SetShouldMonitorQuiescence(true));
...
void PostAndQuitFromNestedRunloop(RunLoop* run_loop, scoped_refptr<TestTaskQueue> runner, bool* was_nested)
{
...
TEST_P(SequenceManagerTest, QuitWhileNested)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostAndQuitFromNestedRunloop, Unretained(&run_loop), queue, Unretained(&was_nested)));
...
TEST_P(SequenceManagerTest, ShutdownTaskQueue_InTasks)
{
    ...
    std::vector<EnqueueOrder> run_order;
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&ShutdownQueue, queues[1]));
    queues[0]->task_runner()->PostTask(FROM_HERE, BindOnce(&ShutdownQueue, queues[2]));
    queues[1]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order));
    queues[2]->task_runner()->PostTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order));
...
TEST_P(SequenceManagerTest, ShutdownTaskQueueInNestedLoop)
{
    ...
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&TaskQueue::ShutdownTaskQueue, Unretained(task_queue.get())), true));
    tasks_to_post_from_nested_loop.push_back(std::make_pair(BindOnce(&NopTask), true));
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&PostFromNestedRunloop, queue, Unretained(&tasks_to_post_from_nested_loop)));
    RunLoop().RunUntilIdle();
...
TEST_P(SequenceManagerTest, PostDelayedTasksReverseOrderAlternatingTimeDomains)
{
    ...
    sequence_manager()->SetTimeDomain(domain.get());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 1, &run_order), Milliseconds(40));

    sequence_manager()->ResetTimeDomain();
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 2, &run_order), Milliseconds(30));

    sequence_manager()->SetTimeDomain(domain.get());
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 3, &run_order), Milliseconds(20));

    sequence_manager()->ResetTimeDomain();
    queue->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&TestTask, 4, &run_order), Milliseconds(10));
...
public:
    ...
    absl::optional<WakeUp> GetNextAllowedWakeUp(LazyNow* lazy_now, absl::optional<WakeUp> next_desired_wake_up, bool has_immediate_work) override
    {
...
TEST_P(SequenceManagerTest, CurrentlyExecutingTaskQueue_TaskRunning)
{
    ...
    EXPECT_THAT(task_sources, ElementsAre(queue0->GetTaskQueueImpl(), queue1->GetTaskQueueImpl()));
    EXPECT_EQ(nullptr, sequence_manager()->currently_executing_task_queue());
}

namespace {
void RunloopCurrentlyExecutingTaskQueueTestTask(
    SequenceManagerImpl* sequence_manager, std::vector<internal::TaskQueueImpl*>* task_sources, std::vector<std::pair<OnceClosure, TestTaskQueue*>>* tasks)
{
    task_sources->push_back(sequence_manager->currently_executing_task_queue());
...
TEST_P(SequenceManagerTest, CurrentlyExecutingTaskQueue_NestedLoop)
{
    ...
    std::vector<internal::TaskQueueImpl*> task_sources;
    std::vector<std::pair<OnceClosure, TestTaskQueue*>> tasks_to_post_from_nested_loop;
    ...
    RunLoop().RunUntilIdle();
    EXPECT_THAT(
        task_sources, UnorderedElementsAre(queue0->GetTaskQueueImpl(), queue1->GetTaskQueueImpl(), queue2->GetTaskQueueImpl(), queue0->GetTaskQueueImpl()));
    EXPECT_EQ(nullptr, sequence_manager()->currently_executing_task_queue());
}
...
TEST_P(SequenceManagerTest, DelayedTasksNotSelected)
{
    ...
    EXPECT_EQ(absl::nullopt, sequence_manager()->GetPendingWakeUp(&lazy_now, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));
    ...
    EXPECT_EQ(absl::nullopt, sequence_manager()->GetPendingWakeUp(&lazy_now2, SequencedTaskSource::SelectTaskOption::kSkipDelayedTask));
...
namespace {
void MessageLoopTaskWithDelayedQuit(Fixture* fixture, scoped_refptr<TestTaskQueue> task_queue)
{
    RunLoop run_loop(RunLoop::Type::kNestableTasksAllowed);
...
TEST_P(SequenceManagerTest, DelayedTaskRunsInNestedMessageLoop)
{
    ...
    auto queue = CreateTaskQueue();
    RunLoop run_loop;
    queue->task_runner()->PostTask(FROM_HERE, BindOnce(&MessageLoopTaskWithDelayedQuit, this, RetainedRef(queue)));
    run_loop.RunUntilIdle();
}

namespace {
void MessageLoopTaskWithImmediateQuit(OnceClosure non_nested_quit_closure, scoped_refptr<TestTaskQueue> task_queue)
{
    RunLoop run_loop(RunLoop::Type::kNestableTasksAllowed);
...
TEST_P(SequenceManagerTest, DelayedNestedMessageLoopDoesntPreventTasksRunning)
{
    ...
    RunLoop run_loop;
    queue->task_runner()->PostDelayedTask(
        FROM_HERE, BindOnce(&MessageLoopTaskWithImmediateQuit, run_loop.QuitClosure(), RetainedRef(queue)), Milliseconds(100));
...
namespace {
void SetOnTaskHandlers(scoped_refptr<TestTaskQueue> task_queue, int* start_counter, int* complete_counter)
{
    task_queue->GetTaskQueueImpl()->SetOnTaskStartedHandler(
        BindRepeating([](int* counter, const Task& task, const TaskQueue::TaskTiming& task_timing) { ++(*counter); }, start_counter));
...
{
    task_queue->GetTaskQueueImpl()->SetOnTaskStartedHandler(internal::TaskQueueImpl::OnTaskStartedHandler());
    task_queue->GetTaskQueueImpl()->SetOnTaskCompletedHandler(internal::TaskQueueImpl::OnTaskCompletedHandler());
}
} // namespace
...
TEST_P(SequenceManagerTest, OnQueueNextWakeUpChangedNotFiredForDisabledQueuePostTask)
{
    ...
    std::unique_ptr<TaskQueue::QueueEnabledVoter> voter = main_tq->CreateQueueEnabledVoter();
    voter->SetVoteToEnable(false);
...
TEST_P(SequenceManagerTest, GracefulShutdown)
{
    ...
    for (int i = 1; i <= 5; ++i) {
        main_tq->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(i * 100));
    }
    ...
    EXPECT_THAT(run_times,
        ElementsAre(FromStartAligned(Milliseconds(100)), FromStartAligned(Milliseconds(200)), FromStartAligned(Milliseconds(300)),
            FromStartAligned(Milliseconds(400)), FromStartAligned(Milliseconds(500))));
...
TEST_P(SequenceManagerTest, GracefulShutdown_ManagerDeletedWithQueuesToShutdown)
{
    std::vector<TimeTicks> run_times;
    scoped_refptr<TestTaskQueue> main_tq = CreateTaskQueue();
    WeakPtr<TestTaskQueue> main_tq_weak_ptr = main_tq->GetWeakPtr();
...
TEST(SequenceManagerBasicTest, DefaultTaskRunnerSupport)
{
    ...
    scoped_refptr<SingleThreadTaskRunner> original_task_runner = ThreadTaskRunnerHandle::Get();
...
TEST_P(SequenceManagerTest, TaskRunnerDeletedOnAnotherThread)
{
    ...
    for (int i = 1; i <= 5; ++i) {
        task_runner->PostDelayedTask(FROM_HERE, BindOnce(&RecordTimeTask, &run_times, mock_tick_clock()), Milliseconds(i * 100));
    }
    ...
    WaitableEvent task_queue_deleted(WaitableEvent::ResetPolicy::MANUAL, WaitableEvent::InitialState::NOT_SIGNALED);
    std::unique_ptr<Thread> thread = std::make_unique<Thread>("test thread");
    thread->StartAndWaitForTesting();

    thread->task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](scoped_refptr<TaskRunner> task_runner, WaitableEvent* task_queue_deleted) {
                task_runner = nullptr;
...
base::OnceClosure PostOnDestruction(scoped_refptr<TestTaskQueue> task_queue, base::OnceClosure task)
{
    return RunOnDestruction(
        base::BindOnce([](base::OnceClosure task, scoped_refptr<TestTaskQueue> task_queue) { task_queue->task_runner()->PostTask(FROM_HERE, std::move(task)); },
...
TEST_P(SequenceManagerTest, TaskQueueUsedInTaskDestructorAfterShutdown)
{
    ...
    thread->task_runner()->PostTask(FROM_HERE,
        BindOnce(
            [](scoped_refptr<TestTaskQueue> task_queue, WaitableEvent* test_executed) {
...
TEST_P(SequenceManagerTest, DestructorPostChainDuringShutdown)
{
    ...
    task_queue->task_runner()->PostTask(
        FROM_HERE, PostOnDestruction(task_queue, PostOnDestruction(task_queue, RunOnDestruction(base::BindOnce([](bool* run) { *run = true; }, &run)))));
...
TEST_P(SequenceManagerTest, DestructorPostsViaTaskRunnerHandleDuringShutdown)
{
    ...
    task_queue->task_runner()->PostTask(FROM_HERE, RunOnDestruction(BindLambdaForTesting([&]() {
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&NopTask));
        run = true;
    })));
...
TEST_P(SequenceManagerTest, HasPendingHighResolutionTasksLowAndNormalPriorityQueues)
{
    ...
    // No task should be considered high resolution in a low priority queue.
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
    queueLow->task_runner()->PostDelayedTask(FROM_HERE, BindOnce(&NopTask), Milliseconds(10));
    EXPECT_FALSE(sequence_manager()->HasPendingHighResolutionTasks());
...
public:
    ...
    // TimeDomain:
    bool MaybeFastForwardToWakeUp(absl::optional<WakeUp> wakeup, bool quit_when_idle_requested) override
    {
...
{
    scoped_refptr<TaskQueue> task_queue = sequence_manager()->CreateTaskQueue(TaskQueue::Spec(QueueName::TEST_TQ));
...
TEST_P(SequenceManagerTest, TaskPriortyInterleaving)
{
    ...
    for (uint8_t priority = 0; priority < TaskQueue::QueuePriority::kQueuePriorityCount; priority++) {
...
TEST_P(SequenceManagerTest, CrashKeys)
{
    ...
    auto expected_stack1 = StringPrintf("0x%zX 0x0", reinterpret_cast<uintptr_t>(parent_location.program_counter()));
    ...
    auto expected_stack2
        = StringPrintf("0x%zX 0x%zX", reinterpret_cast<uintptr_t>(location.program_counter()), reinterpret_cast<uintptr_t>(parent_location.program_counter()));
    ...
    runner->PostTask(parent_location, BindLambdaForTesting([&]() { runner->PostTask(location, run_loop.QuitClosure()); }));
    run_loop.Run();
...
TEST_P(SequenceManagerTest, CrossQueueTaskPostingWhenQueueDeleted)
{
    ...
    queue_1->task_runner()->PostDelayedTask(FROM_HERE, PostOnDestruction(queue_2, task.Get()), Minutes(1));
...
TEST_P(SequenceManagerTest, OnNativeWorkPending)
{
    ...
    // deferred.
    auto native_work = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kHighPriority);
    queue->task_runner()->PostTask(FROM_HERE, task.Get());
    ...
    native_work = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kLowPriority);
    queue->task_runner()->PostTask(FROM_HERE, task.Get());
    CheckPostedTaskRan(true);

    // Equal priority native work doesn't preempt posted tasks.
    native_work = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kNormalPriority);
    queue->task_runner()->PostTask(FROM_HERE, task.Get());
    CheckPostedTaskRan(true);
    ...
    // priority matters.
    native_work = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kNormalPriority);
    auto native_work_high = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kHighPriority);
    auto native_work_low = sequence_manager()->OnNativeWorkPending(TaskQueue::QueuePriority::kLowPriority);
    queue->task_runner()->PostTask(FROM_HERE, task.Get());
    CheckPostedTaskRan(false);
    native_work.reset();
    CheckPostedTaskRan(false);
...
TEST_P(SequenceManagerTest, TaskObserverBlockedOrLowPriority_FenceBeginningOfTime)
{
    ...
    queue->task_runner()->PostTask(FROM_HERE, DoNothing());
    queue->GetTaskQueueImpl()->InsertFence(TaskQueue::InsertFencePosition::kBeginningOfTime);
...
TEST(SequenceManagerTest, CanAccessThreadTaskRunnerHandleDuringSequenceLocalStorageSlotDestruction)
{
    ...
    scoped_refptr<SingleThreadTaskRunner> expected_task_runner = ThreadTaskRunnerHandle::Get();

    StrictMock<MockCallback<base::OnceCallback<void()>>> cb;
    EXPECT_CALL(cb, Run).WillOnce(testing::Invoke([expected_task_runner]() { EXPECT_EQ(ThreadTaskRunnerHandle::Get(), expected_task_runner); }));
