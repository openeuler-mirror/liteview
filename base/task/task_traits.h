...
public:
    ...
    template <class... ArgTypes,
        class CheckArgumentsAreValid
        = std::enable_if_t<trait_helpers::AreValidTraits<ValidTrait, ArgTypes...>::value || trait_helpers::AreValidTraitsForExtension<ArgTypes...>::value>>
    constexpr TaskTraits(ArgTypes... args)
        : extension_(trait_helpers::GetTaskTraitsExtension(trait_helpers::AreValidTraits<ValidTrait, ArgTypes...> {}, args...))
    ...
    constexpr TaskTraits()
        ...
        , shutdown_behavior_((uint8_t)(TaskShutdownBehavior::SKIP_ON_SHUTDOWN))
        , thread_policy_(kIsExplicitFlag)
        , may_block_(false)
    ...
    {
        static_assert(sizeof(TaskTraits) == 14, "Update comparison operator when TaskTraits change");
        return extension_ == other.extension_ && priority_ == other.priority_ && shutdown_behavior_ == other.shutdown_behavior_
            && thread_policy_ == other.thread_policy_ && may_block_ == other.may_block_ && with_base_sync_primitives_ == other.with_base_sync_primitives_;
    }
    ...
    template <class TaskTraitsExtension> const TaskTraitsExtension GetExtension() const
    {
...
private:
    ...
    TaskTraits(TaskPriority priority, bool may_block, TaskTraitsExtensionStorage extension)
