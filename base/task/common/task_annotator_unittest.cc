...
TEST_F(TaskAnnotatorBacktraceIntegrationTest, SingleThreadedSimple)
{
    ...
    OnceClosure task5 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location5,
    ...
    OnceClosure task4 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location4,
        ...
        std::move(task5));
    OnceClosure task3 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location3,
        location4, ExpectedTrace({ location2.program_counter(), location1.program_counter(), location0.program_counter() }), 0, std::move(task4));
    OnceClosure task2 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location2,
        location3, ExpectedTrace({ location1.program_counter(), location0.program_counter() }), dummy_ipc_hash, std::move(task3));
    OnceClosure task1 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this), ThreadTaskRunnerHandle::Get(),
        location1, location2, ExpectedTrace({ location0.program_counter() }), 0, std::move(task2), dummy_ipc_hash);
    OnceClosure task0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location0,
...
TEST_F(TaskAnnotatorBacktraceIntegrationTest, SingleThreadedNested)
{
    ...
    OnceClosure task2 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPost, Unretained(this), ThreadTaskRunnerHandle::Get(), location2,
        location3, ExpectedTrace({ location1.program_counter(), location0.program_counter() }), 0, std::move(run_task_3_then_quit_nested_loop1));
    ...
    OnceClosure task0 = BindOnce(&TaskAnnotatorBacktraceIntegrationTest::VerifyTraceAndPostWithIpcContext, Unretained(this), ThreadTaskRunnerHandle::Get(),
        location0, location1, ExpectedTrace({}), 0, std::move(task1), dummy_ipc_hash);

    ThreadTaskRunnerHandle::Get()->PostTask(location0, std::move(task0));

    {
        TaskAnnotator::ScopedSetIpcHash scoped_ipc_hash(dummy_ipc_hash2);
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RunLoop::Run, Unretained(&nested_run_loop1), FROM_HERE));
    }
