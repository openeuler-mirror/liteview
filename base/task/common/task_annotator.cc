...
ThreadLocalPointer<TaskAnnotator::ScopedSetIpcHash>* GetTLSForCurrentScopedIpcHash()
{
...
void TaskAnnotator::WillQueueTask(perfetto::StaticString trace_event_name, PendingTask* pending_task)
{
    DCHECK(pending_task);
...
// EventContext.
void TaskAnnotator::EmitTaskLocation(perfetto::EventContext& ctx, const PendingTask& task) const
{
...
void TaskAnnotator::MaybeEmitIPCHashAndDelay(perfetto::EventContext& ctx, const PendingTask& task) const
{
...
// Static
uint32_t TaskAnnotator::ScopedSetIpcHash::MD5HashMetricName(base::StringPiece name)
{
