...
OnceCallback<void(Args...)> BindPostTask(
    scoped_refptr<TaskRunner> task_runner, OnceCallback<ReturnType(Args...)> callback, const Location& location = FROM_CURRENT_HERE)
{
...
RepeatingCallback<void(Args...)> BindPostTask(
    scoped_refptr<TaskRunner> task_runner, RepeatingCallback<ReturnType(Args...)> callback, const Location& location = FROM_CURRENT_HERE)
{
    static_assert(std::is_same<ReturnType, void>::value,
