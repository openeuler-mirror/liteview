...
base::TimeDelta GetSleepTimeBeforePurge(base::TimeDelta min_sleep_time)
{
    ...
    const base::TimeTicks snapped_wake = (now + min_sleep_time).SnappedToNextTick(base::TimeTicks(), kPurgeThreadCacheIdleDelay);
...
void WorkerThread::Delegate::WaitForWork(WaitableEvent* wake_up_event)
{
    ...
    static BASE_FEATURE(kDelayFirstWorkerWake, "DelayFirstWorkerWake", base::FEATURE_DISABLED_BY_DEFAULT);
    ...
    static const bool is_delay_first_worker_sleep_enabled = FeatureList::IsEnabled(kDelayFirstWorkerWake);
    ...
    if (!was_signaled) {
        ...
        if (sleep_time > min_sleep_time) {
            wake_up_event->TimedWait(sleep_time.is_max() ? base::TimeDelta::Max() : sleep_time - min_sleep_time);
        }
    }
#else
...
WorkerThread::WorkerThread(
    ThreadType thread_type_hint, std::unique_ptr<Delegate> delegate, TrackedRef<TaskTracker> task_tracker, const CheckedLock* predecessor_lock)
