...
public:
    ...
    {
        if (task_source) {
            auto task_source_with_transaction = TransactionWithRegisteredTaskSource::FromTaskSource(std::move(task_source));
    ...
    bool PostTaskNow(scoped_refptr<Sequence> sequence, Task task)
    {
        ...
        if (task_source) {
            bool should_wakeup = EnqueueTaskSource({ std::move(task_source), std::move(transaction) });
            if (should_wakeup)
                worker_->WakeUp();
        }
        return true;
...
protected:
    ...
    const TrackedRef<TaskTracker>& task_tracker()
    {
...
private:
    ...
    bool EnqueueTaskSource(TransactionWithRegisteredTaskSource transaction_with_task_source)
    {
        CheckedAutoLock auto_lock(lock_);
...
public:
    ...
    bool PostDelayedTask(const Location& from_here, OnceClosure closure, TimeDelta delay) override
    {
        ...
        Task task(from_here, std::move(closure), TimeTicks::Now(), delay, base::GetTaskLeeway());
        ...
        subtle::PostDelayedTaskPassKey, const Location& from_here, OnceClosure closure, TimeTicks delayed_run_time, subtle::DelayPolicy delay_policy) override
    {
        ...
        Task task(from_here, std::move(closure), TimeTicks::Now(), delayed_run_time, base::GetTaskLeeway(), delay_policy);
        return PostTask(std::move(task));
    }
...
private:
    ...
    bool PostTask(Task task)
    {
        ...
        outer_->delayed_task_manager_->AddDelayedTask(
            std::move(task), BindOnce(IgnoreResult(&WorkerThreadDelegate::PostTaskNow), Unretained(GetDelegate()), sequence_), this);
        return true;
    }
    ...
    bool started;
    {
        ...
        if (!worker) {
            ...
            worker = CreateAndRegisterWorkerThread<DelegateType>(
                worker_name, thread_mode, CanUseBackgroundThreadTypeForWorkerThread() ? environment_params.thread_type_hint : ThreadType::kDefault);
            new_worker = true;
        }
        started = started_;
    }
    ...
    const std::string& name, SingleThreadTaskRunnerThreadMode thread_mode, ThreadType thread_type_hint)
{
    ...
    scoped_refptr<WorkerThread> worker = MakeRefCounted<WorkerThread>(thread_type_hint, std::move(delegate), task_tracker_);
