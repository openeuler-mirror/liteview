...
TEST_P(ThreadPoolTaskTrackerTest, IOAllowed)
{
    ...
    Task task_without_may_block(FROM_HERE,
        BindOnce([]() { EXPECT_DCHECK_DEATH({ ScopedBlockingCall scope_blocking_call(FROM_HERE, BlockingType::WILL_BLOCK); }); }), TimeTicks::Now(),
        TimeDelta());
...
static void RunTaskRunnerHandleVerificationTask(
    TaskTracker* tracker, Task verify_task, TaskTraits traits, scoped_refptr<TaskRunner> task_runner, TaskSourceExecutionMode execution_mode)
{
...
TEST_P(ThreadPoolTaskTrackerTest, TaskRunnerHandleIsNotSetOnParallel)
{
    ...
    Task verify_task(FROM_HERE, BindOnce(&VerifyNoTaskRunnerHandle), TimeTicks::Now(), TimeDelta());

    RunTaskRunnerHandleVerificationTask(&tracker_, std::move(verify_task), TaskTraits(GetParam()), nullptr, TaskSourceExecutionMode::kParallel);
}

static void VerifySequencedTaskRunnerHandle(const SequencedTaskRunner* expected_task_runner)
{
    EXPECT_FALSE(ThreadTaskRunnerHandle::IsSet());
...
TEST_P(ThreadPoolTaskTrackerTest, SequencedTaskRunnerHandleIsSetOnSequenced)
{
    ...
    // being set to it.
    Task verify_task(FROM_HERE, BindOnce(&VerifySequencedTaskRunnerHandle, Unretained(test_task_runner.get())), TimeTicks::Now(), TimeDelta());
...
static void VerifyThreadTaskRunnerHandle(const SingleThreadTaskRunner* expected_task_runner)
{
...
TEST_P(ThreadPoolTaskTrackerTest, ThreadTaskRunnerHandleIsSetOnSingleThreaded)
{
    ...
    // being set on it.
    Task verify_task(FROM_HERE, BindOnce(&VerifyThreadTaskRunnerHandle, Unretained(test_task_runner.get())), TimeTicks::Now(), TimeDelta());
