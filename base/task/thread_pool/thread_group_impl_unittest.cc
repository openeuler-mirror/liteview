...
protected:
    ...
    void StartThreadGroup(TimeDelta suggested_reclaim_time, size_t max_tasks, absl::optional<int> max_best_effort_tasks = absl::nullopt,
        WorkerThreadObserver* worker_observer = nullptr, absl::optional<TimeDelta> may_block_threshold = absl::nullopt)
    {
        ASSERT_TRUE(thread_group_);
    ...
    void CreateAndStartThreadGroup(TimeDelta suggested_reclaim_time = TimeDelta::Max(), size_t max_tasks = kMaxTasks,
        absl::optional<int> max_best_effort_tasks = absl::nullopt, WorkerThreadObserver* worker_observer = nullptr,
...
TEST_F(ThreadGroupImplImplTest, ShouldYieldFloodedUserVisible)
{
    ...
    static_cast<ThreadGroup*>(thread_group_.get())
        ->PushTaskSourceAndWakeUpWorkers(TransactionWithRegisteredTaskSource::FromTaskSource(std::move(registered_task_source)));
...
public:
    ...
    ThreadGroupImplCheckTlsReuse& operator=(const ThreadGroupImplCheckTlsReuse&) = delete;
...
TEST_F(ThreadGroupImplCheckTlsReuse, CheckCleanupWorkers)
{
    ...
    for (size_t i = 0; i < kMaxTasks; ++i) {
        factories.push_back(std::make_unique<test::TestTaskFactory>(
            test::CreatePooledTaskRunner({ WithBaseSyncPrimitives() }, &mock_pooled_task_runner_delegate_), TaskSourceExecutionMode::kParallel));
        ASSERT_TRUE(factories.back()->PostTask(PostNestedTask::NO, BindOnce(&ThreadGroupImplCheckTlsReuse::SetTlsValueAndWait, Unretained(this))));
        factories.back()->WaitForAllTasksToRun();
    }
    ...
    for (auto& factory : factories) {
        ...
        ASSERT_TRUE(factory->PostTask(
            PostNestedTask::NO, BindOnce(&ThreadGroupImplCheckTlsReuse::CountZeroTlsValuesAndWait, Unretained(this), count_waiters.back().get())));
...
{
    auto task_runner = test::CreatePooledTaskRunner({ WithBaseSyncPrimitives() }, &mock_pooled_task_runner_delegate_);
...
{
    auto sequenced_task_runner = test::CreatePooledSequencedTaskRunner({}, &mock_pooled_task_runner_delegate_);
    ...
    sequenced_task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&]() {
        recurring_task.Start(FROM_HERE, kReclaimTimeForCleanupTests / 2, DoNothing());
    ...
    sequenced_task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&]() { recurring_task.AbandonAndStop(); }));
...
TEST_F(ThreadGroupImplStandbyPolicyTest, OnlyKeepActiveStandbyThreads)
{
    ...
    RepeatingTimer recurring_task;
    sequenced_task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&]() { recurring_task.Start(FROM_HERE, kReclaimTimeForCleanupTests / 2, DoNothing()); }));
...
class ThreadGroupImplOverCapacityTest : public ThreadGroupImplImplTestBase, public testing::TestWithParam<ReclaimType> {
public:
...
INSTANTIATE_TEST_SUITE_P(ReclaimType, ThreadGroupImplOverCapacityTest, ::testing::Values(ReclaimType::DELAYED_RECLAIM, ReclaimType::NO_RECLAIM));
...
public:
    ...
    void SetUp() override
    {
        CreateThreadGroup();
        thread_group_->Start(
            kMaxTasks, kMaxBestEffortTasks, base::TimeDelta::Max(), service_thread_.task_runner(), nullptr, ThreadGroup::WorkerEnvironment::NONE);
    }
...
TEST_F(ThreadGroupImplImplStartInBodyTest, RacyCleanup)
{
    ...
    thread_group_->Start(
        kLocalMaxTasks, kLocalMaxTasks, kReclaimTimeForRacyCleanupTest, service_thread_.task_runner(), nullptr, ThreadGroup::WorkerEnvironment::NONE);
