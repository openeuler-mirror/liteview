...
bool ContainsWorker(const std::vector<scoped_refptr<WorkerThread>>& workers, const WorkerThread* worker)
{
    auto it = ranges::find_if(workers, [worker](const scoped_refptr<WorkerThread>& i) { return i.get() == worker; });
...
private:
    ...
    public:
        ...
        template <typename Action> void ForEachWorker(Action action)
        {
            if (first_worker_) {
...
class ThreadGroupImpl::WorkerThreadDelegateImpl : public WorkerThread::Delegate, public BlockingObserver {
public:
    ...
    // MAY_BLOCK.
    void OnShutdownStartedLockRequired(ScopedCommandsExecutor* executor) EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);

    // Returns true iff the worker can get work. Cleans up the worker or puts it
    // on the idle stack if it can't get work.
    bool CanGetWorkLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);
...
private:
    ...
    bool CanCleanupLockRequired(const WorkerThread* worker) const EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);
    ...
    // true.
    void CleanupLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);

    // Called in GetWork() when a worker becomes idle.
    void OnWorkerBecomesIdleLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker) EXCLUSIVE_LOCKS_REQUIRED(outer_->lock_);
...
ThreadGroupImpl::ThreadGroupImpl(StringPiece histogram_label, StringPiece thread_group_label, ThreadType thread_type_hint, TrackedRef<TaskTracker> task_tracker,
...
void ThreadGroupImpl::Start(size_t max_tasks, size_t max_best_effort_tasks, TimeDelta suggested_reclaim_time,
    ...
    bool synchronous_thread_start_for_testing, absl::optional<TimeDelta> may_block_threshold)
{
    ThreadGroup::Start();
    ...
    in_start().may_block_threshold = may_block_threshold
        ...
        : (thread_type_hint_ == ThreadType::kDefault ? kForegroundMayBlockThreshold : kBackgroundMayBlockThreshold);
    in_start().blocked_workers_poll_period = thread_type_hint_ == ThreadType::kDefault ? kForegroundBlockedWorkersPoll : kBackgroundBlockedWorkersPoll;
...
void ThreadGroupImpl::PushTaskSourceAndWakeUpWorkers(TransactionWithRegisteredTaskSource transaction_with_task_source)
{
    ScopedCommandsExecutor executor(this);
...
ThreadGroupImpl::WorkerThreadDelegateImpl::WorkerThreadDelegateImpl(TrackedRef<ThreadGroupImpl> outer)
...
WorkerThread::ThreadLabel ThreadGroupImpl::WorkerThreadDelegateImpl::GetThreadLabel() const
{
    return WorkerThread::ThreadLabel::POOLED;
}

void ThreadGroupImpl::WorkerThreadDelegateImpl::OnMainEntry(WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
RegisteredTaskSource ThreadGroupImpl::WorkerThreadDelegateImpl::GetWork(WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::DidProcessTask(RegisteredTaskSource task_source)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(read_worker().current_task_priority);
    DCHECK(read_worker().current_shutdown_behavior);
    ...
    absl::optional<TransactionWithRegisteredTaskSource> transaction_with_task_source;
    if (task_source) {
        transaction_with_task_source.emplace(TransactionWithRegisteredTaskSource::FromTaskSource(std::move(task_source)));
    }
...
bool ThreadGroupImpl::WorkerThreadDelegateImpl::CanCleanupLockRequired(const WorkerThread* worker) const
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    if (outer_->after_start().no_worker_reclaim)
        return false;
    ...
    return !last_used_time.is_null() && subtle::TimeTicksNowIgnoringOverride() - last_used_time >= outer_->after_start().suggested_reclaim_time
        && LIKELY(!outer_->worker_cleanup_disallowed_for_testing_);
}

void ThreadGroupImpl::WorkerThreadDelegateImpl::CleanupLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker)
{
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::OnWorkerBecomesIdleLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(!outer_->idle_workers_stack_.Contains(worker));
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::OnMainExit(WorkerThread* worker)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::BlockingStarted(BlockingType blocking_type)
{
    DCHECK_CALLED_ON_VALID_THREAD(worker_thread_checker_);
    DCHECK(read_worker().current_task_priority);
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::OnShutdownStartedLockRequired(ScopedCommandsExecutor* executor)
{
...
bool ThreadGroupImpl::WorkerThreadDelegateImpl::CanGetWorkLockRequired(ScopedCommandsExecutor* executor, WorkerThread* worker)
{
    const bool is_on_idle_workers_stack = outer_->IsOnIdleStackLockRequired(worker);
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::MaybeIncrementMaxTasksLockRequired()
{
...
void ThreadGroupImpl::WorkerThreadDelegateImpl::IncrementMaxTasksLockRequired()
{
    if (!incremented_max_tasks_since_blocked_) {
        outer_->IncrementMaxTasksLockRequired();
...
scoped_refptr<WorkerThread> ThreadGroupImpl::CreateAndRegisterWorkerLockRequired(ScopedCommandsExecutor* executor)
{
    ...
    scoped_refptr<WorkerThread> worker = MakeRefCounted<WorkerThread>(
        thread_type_hint_, std::make_unique<WorkerThreadDelegateImpl>(tracked_ref_factory_.GetTrackedRef()), task_tracker_, &lock_);
...
void ThreadGroupImpl::OnShutdownStarted()
{
    ...
    for (scoped_refptr<WorkerThread>& worker : workers_) {
        ...
        WorkerThreadDelegateImpl* delegate = static_cast<WorkerThreadDelegateImpl*>(worker->delegate());
...
void ThreadGroupImpl::MaybeScheduleAdjustMaxTasksLockRequired(ScopedCommandsExecutor* executor)
{
...
bool ThreadGroupImpl::IsOnIdleStackLockRequired(WorkerThread* worker) const
{
    ...
    // the idle stack.
    return idle_workers_stack_.Peek() == worker || !worker->GetLastUsedTime().is_null();
}
