...
TEST(ThreadPoolSequenceTest, PushTakeRemoveTasksWithLocationSetting)
{
    ...
    EXPECT_EQ(sequence->GetCurrentLocationForTesting(), Sequence::SequenceLocation::kNone);
    ...
    EXPECT_EQ(sequence->GetCurrentLocationForTesting(), Sequence::SequenceLocation::kImmediateQueue);
    ...
    // to kInWorker.
    EXPECT_EQ(sequence->GetCurrentLocationForTesting(), Sequence::SequenceLocation::kInWorker);
...
TEST(ThreadPoolSequenceTest, CheckSequenceLocationInWorker)
{
    ...
    // The next task we get when we call Sequence::TakeTask should be Task A.
    absl::optional<Task> task_a = registered_task_source.TakeTask(&sequence_transaction);
    ...
    absl::optional<Task> task_b = registered_task_source.TakeTask(&sequence_transaction);
...
TEST(ThreadPoolSequenceTest, PushTakeRemoveDelayedTasks)
{
    ...
    // queue.
    EXPECT_EQ(sequence->GetCurrentLocationForTesting(), Sequence::SequenceLocation::kDelayedQueue);
...
TEST(ThreadPoolSequenceTest, GetDelayedSortKeyMixedtasks)
{
    ...
    sequence_transaction.PushDelayedTask(CreateDelayedTask(&mock_task_a, Milliseconds(10), now));
    ...
    auto registered_task_source = RegisteredTaskSource::CreateForTesting(sequence);
    registered_task_source.WillRunTask();
    absl::optional<Task> take_delayed_task = registered_task_source.TakeTask(&sequence_transaction);
    ...
    // is available for the check below.
    registered_task_source.WillRunTask();
    absl::optional<Task> take_immediate_task = registered_task_source.TakeTask(&sequence_transaction);
