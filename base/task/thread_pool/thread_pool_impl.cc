...
ThreadPoolImpl::ThreadPoolImpl(StringPiece histogram_label, std::unique_ptr<TaskTrackerImpl> task_tracker)
    ...
    , tracked_ref_factory_(this)
{
    ...
    if (CanUseBackgroundThreadTypeForWorkerThread()) {
        background_thread_group_ = std::make_unique<ThreadGroupImpl>(
            ...
            kBackgroundPoolEnvironmentParams.name_suffix, kBackgroundPoolEnvironmentParams.thread_type_hint, task_tracker_->GetTrackedRef(),
            tracked_ref_factory_.GetTrackedRef());
    }
}
...
void ThreadPoolImpl::Start(const ThreadPoolInstance::InitParams& init_params, WorkerThreadObserver* worker_thread_observer)
{
    ...
    const size_t max_best_effort_tasks = std::min(kMaxBestEffortTasks, init_params.max_num_foreground_threads);
...
#endif
            task_tracker_->GetTrackedRef(), tracked_ref_factory_.GetTrackedRef(), old_group.get());
        old_group->InvalidateAndHandoffAllTaskSourcesToOtherThreadGroup(foreground_thread_group_.get());
    }
...
#endif
            task_tracker_->GetTrackedRef(), tracked_ref_factory_.GetTrackedRef(), old_group.get());
        old_group->InvalidateAndHandoffAllTaskSourcesToOtherThreadGroup(background_thread_group_.get());
    }
#endif
...
#endif
    {
        ...
        static_cast<ThreadGroupImpl*>(foreground_thread_group_.get())
            ->Start(init_params.max_num_foreground_threads, max_best_effort_tasks, init_params.suggested_reclaim_time, service_thread_task_runner,
...
#if HAS_NATIVE_THREAD_POOL()
        if (FeatureList::IsEnabled(kUseBackgroundNativeThreadPool)) {
            static_cast<ThreadGroupNative*>(background_thread_group_.get())->Start(worker_environment);
        } else
#endif
        {
            static_cast<ThreadGroupImpl*>(background_thread_group_.get())
                ->Start(max_best_effort_tasks, max_best_effort_tasks, init_params.suggested_reclaim_time, service_thread_task_runner, worker_thread_observer,
...
bool ThreadPoolImpl::PostDelayedTask(const Location& from_here, const TaskTraits& traits, OnceClosure task, TimeDelta delay)
{
    ...
    return PostTaskWithSequence(Task(from_here, std::move(task), TimeTicks::Now(), delay, base::GetTaskLeeway()),
...
absl::optional<TimeTicks> ThreadPoolImpl::NextScheduledRunTimeForTesting() const
{
...
bool ThreadPoolImpl::PostTaskWithSequence(Task task, scoped_refptr<Sequence> sequence)
{
    ...
    if (task.delayed_run_time.is_null()) {
        ...
        delayed_task_manager_.AddDelayedTask(std::move(task),
            BindOnce([](scoped_refptr<Sequence> sequence, ThreadPoolImpl* thread_pool_impl,
