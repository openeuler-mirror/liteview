...
bool Sequence::Transaction::ShouldBeQueued() const
{
    ...
    // file.
    auto current_location = sequence()->current_location_.load(std::memory_order_relaxed);
    if (current_location == Sequence::SequenceLocation::kImmediateQueue || current_location == Sequence::SequenceLocation::kInWorker) {
        return false;
    }
...
bool Sequence::Transaction::TopDelayedTaskWillChange(Task& delayed_task) const
{
    if (sequence()->IsEmpty())
        return true;
    return delayed_task.latest_delayed_run_time() < sequence()->delayed_queue_.top().latest_delayed_run_time();
}
...
void Sequence::Transaction::PushImmediateTask(Task task)
{
    ...
    auto current_location = sequence()->current_location_.load(std::memory_order_relaxed);
    bool was_unretained = sequence()->IsEmpty() && current_location != Sequence::SequenceLocation::kInWorker;
    bool queue_was_empty = sequence()->queue_.empty();

    task.task = sequence()->traits_.shutdown_behavior() == TaskShutdownBehavior::BLOCK_SHUTDOWN ? MakeCriticalClosure(task.posted_from, std::move(task.task),
    ...
    if (queue_was_empty) {
        sequence()->ready_time_.store(sequence()->GetNextReadyTime(), std::memory_order_relaxed);
    }

    if (current_location == Sequence::SequenceLocation::kDelayedQueue || current_location == Sequence::SequenceLocation::kNone) {
        sequence()->current_location_.store(Sequence::SequenceLocation::kImmediateQueue, std::memory_order_relaxed);
    }
...
void Sequence::Transaction::PushDelayedTask(Task task)
{
    ...
    sequence()->current_location_.compare_exchange_strong(expected_location, Sequence::SequenceLocation::kDelayedQueue, std::memory_order_relaxed);
...
TaskSource::RunStatus Sequence::WillRunTask()
{
    ...
    DCHECK_EQ(current_location_.load(std::memory_order_relaxed), Sequence::SequenceLocation::kImmediateQueue);
    ...
    current_location_.store(Sequence::SequenceLocation::kInWorker, std::memory_order_relaxed);
...
void Sequence::OnBecomeReady()
{
    ...
    DCHECK(current_location_.load(std::memory_order_relaxed) == Sequence::SequenceLocation::kDelayedQueue);
...
TimeTicks Sequence::GetNextReadyTime()
{
    ...
    return std::min(queue_.front().queue_time, delayed_queue_.top().latest_delayed_run_time());
}
...
Task Sequence::TakeTask(TaskSource::Transaction* transaction)
{
    ...
    DCHECK(current_location_.load(std::memory_order_relaxed) == Sequence::SequenceLocation::kInWorker);
...
bool Sequence::DidProcessTask(TaskSource::Transaction* transaction)
{
    ...
    if (IsEmpty()) {
        ReleaseTaskRunner();
        current_location_.store(Sequence::SequenceLocation::kNone, std::memory_order_relaxed);
        return false;
    }
...
bool Sequence::WillReEnqueue(TimeTicks now, TaskSource::Transaction* transaction)
{
    ...
    if (has_ready_tasks) {
        ...
        current_location_.store(Sequence::SequenceLocation::kDelayedQueue, std::memory_order_relaxed);
    }
...
Task Sequence::Clear(TaskSource::Transaction* transaction)
{
    CheckedAutoLockMaybe auto_lock(transaction ? nullptr : &lock_);
    // See comment on TaskSource::task_runner_ for lifetime management details.
    if (!IsEmpty() && current_location_.load(std::memory_order_relaxed) != Sequence::SequenceLocation::kInWorker) {
        ReleaseTaskRunner();
    }
...
Sequence::Sequence(const TaskTraits& traits, TaskRunner* task_runner, TaskSourceExecutionMode execution_mode)
