...
public:
    ...
    ThreadGroupImpl(StringPiece histogram_label, StringPiece thread_group_label, ThreadType thread_type_hint, TrackedRef<TaskTracker> task_tracker,
    ...
    void Start(size_t max_tasks, size_t max_best_effort_tasks, TimeDelta suggested_reclaim_time,
        ...
        bool synchronous_thread_start_for_testing = false, absl::optional<TimeDelta> may_block_threshold = absl::optional<TimeDelta>());
...
private:
    ...
    void PushTaskSourceAndWakeUpWorkers(TransactionWithRegisteredTaskSource transaction_with_task_source) override;
    ...
    void MaybeScheduleAdjustMaxTasksLockRequired(ScopedCommandsExecutor* executor) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    bool IsOnIdleStackLockRequired(WorkerThread* worker) const EXCLUSIVE_LOCKS_REQUIRED(lock_);
