...
bool TaskTracker::WillPostTask(Task* task, TaskShutdownBehavior shutdown_behavior)
{
    ...
    if (state_->HasShutdownStarted()) {
        ...
        if (shutdown_behavior != TaskShutdownBehavior::BLOCK_SHUTDOWN || !task->delayed_run_time.is_null()) {
            return false;
        }
...
RegisteredTaskSource TaskTracker::RunAndPopNextTask(RegisteredTaskSource task_source)
{
    ...
    TaskTraits traits = { base::TaskPriority::USER_BLOCKING };
    {
...
void TaskTracker::RunTask(Task task, TaskSource* task_source, const TaskTraits& traits)
{
...

    {
        ...
        ScopedSetSequenceTokenForCurrentThread scoped_set_sequence_token_for_current_thread(environment.token);
        ...
        ScopedSetSequenceLocalStorageMapForCurrentThread scoped_set_sequence_local_storage_map_for_current_thread(
            environment.sequence_local_storage ? environment.sequence_local_storage.get() : &local_storage_map.value());
        ...
        case TaskSourceExecutionMode::kSequenced:
            ...
            sequenced_task_runner_handle.emplace(static_cast<SequencedTaskRunner*>(task_source->task_runner()));
            break;
        case TaskSourceExecutionMode::kSingleThread:
            DCHECK(task_source->task_runner());
            single_thread_task_runner_handle.emplace(static_cast<SingleThreadTaskRunner*>(task_source->task_runner()));
            break;
        }
