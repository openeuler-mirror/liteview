...
#if BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && defined(PA_THREAD_CACHE_SUPPORTED)
...
protected:
    ...
    void SetUp() override
    {
        worker_ = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::make_unique<TestWorkerThreadDelegate>(this), task_tracker_.GetTrackedRef());
        ASSERT_TRUE(worker_);
    ...
    public:
        ...
        RegisteredTaskSource GetWork(WorkerThread* worker) override
        {
            ...
            for (int i = 0; i < outer_->TasksPerSequence(); ++i) {
                Task task(FROM_HERE, BindOnce(&ThreadPoolWorkerTest::RunTaskCallback, Unretained(outer_)), TimeTicks::Now(), TimeDelta());
...
INSTANTIATE_TEST_SUITE_P(OneTaskPerSequence, ThreadPoolWorkerTest, ::testing::Values(1));
INSTANTIATE_TEST_SUITE_P(TwoTasksPerSequence, ThreadPoolWorkerTest, ::testing::Values(2));
...
TEST(ThreadPoolWorkerTest, WorkerCleanupFromGetWork)
{
    ...
    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kDefault, WrapUnique(delegate), task_tracker.GetTrackedRef());
...
TEST(ThreadPoolWorkerTest, WorkerCleanupDuringWork)
{
    ...
    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kDefault, std::move(delegate), task_tracker.GetTrackedRef());
    worker->Start(service_thread.task_runner());
    worker->WakeUp();
...
TEST(ThreadPoolWorkerTest, BumpThreadTypeOfAliveThreadDuringShutdown)
{
    ...
    auto worker = MakeRefCounted<WorkerThread>(ThreadType::kBackground, std::move(delegate), task_tracker.GetTrackedRef());
    worker->Start(service_thread.task_runner());
...
TEST(ThreadPoolWorkerThreadCachePurgeTest, Purge)
{
    ...
    PlatformThread::Sleep(1.1 * WorkerThread::Delegate::kPurgeThreadCacheIdleDelay);
    worker->JoinForTesting();
}
