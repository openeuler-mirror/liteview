...
bool ShouldRunInNativeThreadGroup(const TaskTraits& traits, GroupTypes group_types)
{
    if (traits.priority() == TaskPriority::BEST_EFFORT && traits.thread_policy() == ThreadPolicy::PREFER_BACKGROUND) {
...
void VerifyTaskEnvironment(const TaskTraits& traits, GroupTypes group_types)
{
    ...
    const bool expect_background_thread_type = TraitsSupportBackgroundThreadType(traits)
        ...
        && (group_types.background_type == test::GroupType::GENERIC || is_single_threaded)
#endif
        ;

    EXPECT_EQ(expect_background_thread_type ? ThreadType::kBackground : ThreadType::kDefault, PlatformThread::GetCurrentThreadType());
    ...
    EXPECT_THAT(thread_name, ::testing::HasSubstr(expect_background_thread_type ? "Background" : "Foreground"));
...
void VerifyTaskEnvironmentAndSignalEvent(const TaskTraits& traits, GroupTypes group_types, TestWaitableEvent* event)
{
    DCHECK(event);
...
void VerifyTimeAndTaskEnvironmentAndSignalEvent(const TaskTraits& traits, GroupTypes group_types, TimeTicks expected_time, TestWaitableEvent* event)
{
    DCHECK(event);
...
void VerifyOrderAndTaskEnvironmentAndSignalEvent(
    const TaskTraits& traits, GroupTypes group_types, TestWaitableEvent* expected_previous_event, TestWaitableEvent* event)
{
    DCHECK(event);
...
public:
    ...
    // |execution_mode|.
    ThreadPostingTasks(ThreadPoolImpl* thread_pool, const TaskTraits& traits, GroupTypes group_types, TaskSourceExecutionMode execution_mode)
...
private:
    void Run() override
    {
        ...
        for (size_t i = 0; i < kNumTasksPerThread; ++i) {
            factory_.PostTask(test::TestTaskFactory::PostNestedTask::NO, BindOnce(&VerifyTaskEnvironment, traits_, group_types_));
        }
    }
...
public:
    ...
    void set_worker_thread_observer(WorkerThreadObserver* worker_thread_observer)
    {
        worker_thread_observer_ = worker_thread_observer;
    }

    void StartThreadPool(size_t max_num_foreground_threads = kMaxNumForegroundThreads, TimeDelta reclaim_time = Seconds(30))
    {
        SetupFeatures();
...
class ThreadPoolImplTest : public ThreadPoolImplTestBase, public testing::WithParamInterface<GroupTypes> {
public:
    ...
    GroupTypes GetGroupTypes() const override
    {
        return GetParam();
    }
};
...
// single-thread) and whether the task is affected by a BEST_EFFORT fence.
class ThreadPoolImplTest_CoverAllSchedulingOptions : public ThreadPoolImplTestBase,
                                                     public testing::WithParamInterface<std::tuple<GroupTypes, TraitsExecutionModePair>> {
public:
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostDelayedTaskNoDelay)
{
    ...
    thread_pool_->PostDelayedTask(
        FROM_HERE, GetTraits(), BindOnce(&VerifyTaskEnvironmentAndSignalEvent, GetTraits(), GetGroupTypes(), Unretained(&task_ran)), TimeDelta());
    task_ran.Wait();
}
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostDelayedTaskWithDelay)
{
    ...
    thread_pool_->PostDelayedTask(FROM_HERE, GetTraits(),
        BindOnce(
            &VerifyTimeAndTaskEnvironmentAndSignalEvent, GetTraits(), GetGroupTypes(), TimeTicks::Now() + TestTimeouts::tiny_timeout(), Unretained(&task_ran)),
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostDelayedTaskAtViaTaskRunner)
{
    ...
    auto handle = CreateSequencedTaskRunnerAndExecutionMode(thread_pool_.get(), GetTraits(), GetExecutionMode())
                      ->PostCancelableDelayedTaskAt(subtle::PostDelayedTaskPassKeyForTesting(), FROM_HERE,
                          BindOnce(&VerifyTimeAndTaskEnvironmentAndSignalEvent, GetTraits(), GetGroupTypes(), TimeTicks::Now() + TestTimeouts::tiny_timeout(),
                              Unretained(&task_ran)),
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostTasksViaTaskRunner)
{
    ...
    for (size_t i = 0; i < kNumTasksPerTest; ++i) {
        factory.PostTask(test::TestTaskFactory::PostNestedTask::NO, BindOnce(&VerifyTaskEnvironment, GetTraits(), GetGroupTypes()));
    }
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostDelayedTaskNoDelayBeforeStart)
{
    ...
    thread_pool_->PostDelayedTask(
        FROM_HERE, GetTraits(), BindOnce(&VerifyTaskEnvironmentAndSignalEvent, GetTraits(), GetGroupTypes(), Unretained(&task_running)), TimeDelta());
...
TEST_P(ThreadPoolImplTest_CoverAllSchedulingOptions, PostTaskViaTaskRunnerBeforeStart)
{
    ...
    CreateTaskRunnerAndExecutionMode(thread_pool_.get(), GetTraits(), GetExecutionMode())
        ->PostTask(FROM_HERE, BindOnce(&VerifyTaskEnvironmentAndSignalEvent, GetTraits(), GetGroupTypes(), Unretained(&task_running)));
...
TEST_P(ThreadPoolImplTest, MultipleTraitsExecutionModePair)
{
    ...
    for (const auto& test_params : GetTraitsExecutionModePairs()) {
        threads_posting_tasks.push_back(
            std::make_unique<ThreadPostingTasks>(thread_pool_.get(), test_params.traits, GetGroupTypes(), test_params.execution_mode));
...
TEST_P(ThreadPoolImplTest, GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated)
{
    ...
    const size_t expected_max = GetGroupTypes().foreground_type == test::GroupType::GENERIC
        ? kMaxNumForegroundThreads
        : static_cast<size_t>(std::max(3, SysInfo::NumberOfProcessors() - 1));

    EXPECT_EQ(expected_max, thread_pool_->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ TaskPriority::USER_VISIBLE }));
    EXPECT_EQ(expected_max, thread_pool_->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ MayBlock(), TaskPriority::USER_VISIBLE }));
    EXPECT_EQ(expected_max, thread_pool_->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ TaskPriority::USER_BLOCKING }));
    EXPECT_EQ(expected_max, thread_pool_->GetMaxConcurrentNonBlockedTasksWithTraitsDeprecated({ MayBlock(), TaskPriority::USER_BLOCKING }));
}
...
TEST_P(ThreadPoolImplTest, WorkerThreadObserver)
{
    ...
    const int kExpectedNumForegroundPoolWorkers = (GetGroupTypes().foreground_type == test::GroupType::GENERIC) ? 1 : 0;
    const int kExpectedNumBackgroundPoolWorkers
        = (GetGroupTypes().background_type == test::GroupType::GENERIC && CanUseBackgroundThreadTypeForWorkerThread()) ? 1 : 0;
    const int kExpectedNumPoolWorkers = kExpectedNumForegroundPoolWorkers + kExpectedNumBackgroundPoolWorkers;
    const int kExpectedNumSharedSingleThreadedWorkers = CanUseBackgroundThreadTypeForWorkerThread() ? 4 : 2;
...
std::vector<std::unique_ptr<TaskRunnerAndEvents>> CreateTaskRunnersAndEvents(ThreadPoolImpl* thread_pool, ThreadPolicy thread_policy)
{
    ...
    task_runners_and_events.push_back(
        std::make_unique<TaskRunnerAndEvents>(thread_pool->CreateUpdateableSequencedTaskRunner({ TaskPriority::BEST_EFFORT, thread_policy }),
    ...
    TestWaitableEvent* expected_previous_event = CanUseBackgroundThreadTypeForWorkerThread() ? nullptr : &task_runners_and_events.back()->task_ran;
...
void TestUpdatePrioritySequenceNotScheduled(ThreadPoolImplTest* test, ThreadPolicy thread_policy)
{
    ...
    auto task_runners_and_events = CreateTaskRunnersAndEvents(test->thread_pool_.get(), thread_policy);
    ...
    for (auto& task_runner_and_events : task_runners_and_events) {
#if HAS_NATIVE_THREAD_POOL()
        const bool runs_in_background_group = (task_runner_and_events->updated_priority == TaskPriority::BEST_EFFORT
        ...
        task_runner_and_events->task_runner->PostTask(FROM_HERE,
            BindOnce(&VerifyOrderAndTaskEnvironmentAndSignalEvent, TaskTraits { task_runner_and_events->updated_priority, thread_policy },
                ...
                Unretained(
#if HAS_NATIVE_THREAD_POOL()
                    ((runs_in_background_group && test->GetGroupTypes().background_type == test::GroupType::NATIVE)
                        || (!runs_in_background_group && test->GetGroupTypes().foreground_type == test::GroupType::NATIVE))
                        ? nullptr
                        :
#endif
...
void TestUpdatePrioritySequenceScheduled(ThreadPoolImplTest* test, ThreadPolicy thread_policy)
{
    ...
    for (auto& task_runner_and_events : task_runners_and_events) {
        task_runner_and_events->task_runner->PostTask(FROM_HERE,
            BindOnce(&VerifyOrderAndTaskEnvironmentAndSignalEvent, TaskTraits { task_runner_and_events->updated_priority, thread_policy },
                test->GetGroupTypes(), Unretained(task_runner_and_events->expected_previous_event), Unretained(&task_runner_and_events->task_ran)));
    }
...
auto GetGroupTypes()
{
    return ::testing::Values(GroupTypes { test::GroupType::GENERIC, test::GroupType::GENERIC }
#if HAS_NATIVE_THREAD_POOL()
        ,
...
INSTANTIATE_TEST_SUITE_P(All, ThreadPoolImplTest_CoverAllSchedulingOptions,
    ::testing::Combine(GetGroupTypes(), ::testing::ValuesIn(GetTraitsExecutionModePairsToCoverAllSchedulingOptions())));
