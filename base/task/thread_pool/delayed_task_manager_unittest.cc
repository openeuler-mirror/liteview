...
protected:
    ...
    Task task_ { ConstructMockedTask(mock_callback_, service_thread_task_runner_->NowTicks(), kLongDelay) };
};
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskDoesNotRunBeforeStart)
{
    ...
    delayed_task_manager_.AddDelayedTask(std::move(task_), BindOnce(&PostTaskNow), nullptr);
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskRunsAtTime_FlexiblePreferEarly)
{
    ...
    // Send |task| to the DelayedTaskManager.
    delayed_task_manager_.AddDelayedTask(std::move(task), BindOnce(&PostTaskNow), nullptr);
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTaskRunsAfterCancelled)
{
    ...
    delayed_task_manager_.AddDelayedTask(std::move(cancelable_task), post_cancelable_task_now, nullptr);
...
TEST_F(ThreadPoolDelayedTaskManagerTest, DelayedTasksRunAfterDelay)
{
    ...
    delayed_task_manager_.AddDelayedTask(std::move(task_a), BindOnce(&PostTaskNow), nullptr);
    delayed_task_manager_.AddDelayedTask(std::move(task_b), BindOnce(&PostTaskNow), nullptr);
    delayed_task_manager_.AddDelayedTask(std::move(task_c), BindOnce(&PostTaskNow), nullptr);
...
TEST_F(ThreadPoolDelayedTaskManagerTest, PostTaskDuringStart)
{
    ...
    other_thread.task_runner()->PostTask(FROM_HERE, BindLambdaForTesting([&]() {
        delayed_task_manager_.AddDelayedTask(std::move(task_), BindOnce(&PostTaskNow), other_thread.task_runner());
        task_posted.Signal();
    }));
