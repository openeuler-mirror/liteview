...
protected:
    ...
    static LazyInstance<ThreadLocalPointer<const NotificationDataBase>>::Leaky tls_current_notification_;
...
template <class ObserverType> class ObserverListThreadSafe : public internal::ObserverListThreadSafeBase {
public:
    ...
    {
        DCHECK(SequencedTaskRunnerHandle::IsSet()) << "An observer can only be registered when SequencedTaskRunnerHandle "
        ...
        if (policy_ == ObserverListPolicy::ALL) {
            const NotificationDataBase* current_notification = tls_current_notification_.Get().Get();
            if (current_notification && current_notification->observer_list == this) {
                ...
                task_runner->PostTask(current_notification->from_here,
                    BindOnce(&ObserverListThreadSafe<ObserverType>::NotifyWrapper, this, UnsafeDanglingUntriaged(observer),
                        NotificationData(this, observer_id, current_notification->from_here, notification_data->method)));
            }
        }
    ...
    template <typename Method, typename... Params> void Notify(const Location& from_here, Method m, Params&&... params)
    {
        ...
        for (const auto& observer : observers_) {
            observer.second.task_runner->PostTask(from_here,
                BindOnce(&ObserverListThreadSafe<ObserverType>::NotifyWrapper, this, base::UnsafeDanglingUntriaged(observer.first),
                    NotificationData(this, observer.second.observer_id, from_here, method)));
        }
    }
...
private:
    ...
    void NotifyWrapper(ObserverType* observer, const NotificationData& notification)
    {
        {
            AutoLock auto_lock(lock_);
