...
int VlogInfo::GetVlogLevel(base::StringPiece file) const
{
    if (!vmodule_levels_.empty()) {
        ...
        for (const auto& it : vmodule_levels_) {
            base::StringPiece target((it.match_target == VmodulePattern::MATCH_FILE) ? file : module);
...
bool MatchVlogPattern(base::StringPiece string, base::StringPiece vlog_pattern)
{
