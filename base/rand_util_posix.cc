...
// // both are enabled.
// BASE_FEATURE(kUseGetrandomForRandBytes, "UseGetrandomForRandBytes", FEATURE_DISABLED_BY_DEFAULT);
// 
// bool UseGetrandom()
// {
...
// {
//     g_use_getrandom.store(FeatureList::IsEnabled(kUseGetrandomForRandBytes), std::memory_order_relaxed);
// }
// #endif
// 
namespace {
...
void RandBytes(void* output, size_t output_length)
{
    ...
    if (0 == randseed)
        randseed = (uint32_t)output;

    unsigned char* byte_output = (unsigned char*)output;
    for (size_t i = 0; i < output_length; ++i) {
        r = randseed = randseed * 1103515245 + 12345;
        uint32_t ret = (r << 16) | ((r >> 16) & 0xFFFF);

        byte_output[i] = ret % 256;
    }
}
