...
template <typename T>
inline typename std::enable_if<base::internal::SupportsOstreamOperator<const T&>::value && !std::is_function<typename std::remove_pointer<T>::type>::value,
    char*>::type
...
template <typename T>
inline typename std::enable_if<!base::internal::SupportsOstreamOperator<const T&>::value && base::internal::SupportsToString<const T&>::value, char*>::type
CheckOpValueStr(const T& v)
{
...
template <typename T> inline typename std::enable_if<std::is_function<typename std::remove_pointer<T>::type>::value, char*>::type CheckOpValueStr(const T& v)
{
...
template <typename T>
inline typename std::enable_if<!base::internal::SupportsOstreamOperator<const T&>::value && std::is_enum<T>::value, char*>::type CheckOpValueStr(const T& v)
{
...
#define CHECK_OP(name, op, val1, val2)                                                                                                                         \
    ...
    case 0:                                                                                                                                                    \
    default:                                                                                                                                                   \
        if (::logging::CheckOpResult true_if_passed = ::logging::Check##name##Impl((val1), (val2), #val1 " " #op " " #val2))                                   \
            ;                                                                                                                                                  \
        else                                                                                                                                                   \
...
#define DEFINE_CHECK_OP_IMPL(name, op)                                                                                                                         \
    template <typename T, typename U, std::enable_if_t<!std::is_fundamental<T>::value || !std::is_fundamental<U>::value, int> = 0>                             \
    constexpr ::logging::CheckOpResult Check##name##Impl(const T& v1, const U& v2, const char* expr_str)                                                       \
    {                                                                                                                                                          \
