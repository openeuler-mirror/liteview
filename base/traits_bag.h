...
template <typename... TraitsToExclude> struct Exclude {
    template <typename T, std::enable_if_t<ParameterPack<TraitsToExclude...>::template HasType<T>::value>* = nullptr> static constexpr EmptyTrait Filter(T t)
    {
        return EmptyTrait();
    }

    template <typename T, std::enable_if_t<!ParameterPack<TraitsToExclude...>::template HasType<T>::value>* = nullptr> static constexpr T Filter(T t)
    {
        return t;
    }
};
...
template <class TraitFilterType, class ArgType, class CheckArgumentIsCompatible = std::enable_if_t<std::is_constructible<TraitFilterType, ArgType>::value>>
...
template <class TraitFilterType, class... ArgTypes,
    class TestCompatibleArgument = std::enable_if_t<any_of({ std::is_constructible<TraitFilterType, ArgTypes>::value... })>>
...
{
    static_assert(std::is_constructible<TraitFilterType>::value, "The traits bag is missing a required trait.");
...
{
    static_assert(
        count({ std::is_constructible<TraitFilterType, ArgTypes>::value... }, true) <= 1, "The traits bag contains multiple traits of the same type.");
...
template <typename ArgType> struct OptionalEnumTraitFilter : public BasicTraitFilter<ArgType, absl::optional<ArgType>> {
...
template <class ValidTraits, class T> using IsValidTrait = std::disjunction<std::is_constructible<ValidTraits, T>, std::is_same<T, EmptyTrait>>;
...
//   ...
// };
template <class ValidTraits, class... ArgTypes> using AreValidTraits = std::bool_constant<all_of({ IsValidTrait<ValidTraits, ArgTypes>::value... })>;
...
// readable.
template <typename Enum, typename... Args> static constexpr absl::optional<Enum> GetOptionalEnum(Args... args)
{
...
template <typename Trait, typename... Args> struct HasTrait : ParameterPack<Args...>::template HasType<Trait> {
    static_assert(count({ std::is_constructible<Trait, Args>::value... }, true) <= 1, "The traits bag contains multiple traits of the same type.");
};
