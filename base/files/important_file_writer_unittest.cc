...
void WriteCallbacksObserver::ObserveNextWriteCallbacks(ImportantFileWriter* writer)
{
...
TEST_F(ImportantFileWriterTest, FailedWriteWithObserver)
{
    ...
    ImportantFileWriter writer(FilePath().AppendASCII("bad/../path"), ThreadTaskRunnerHandle::Get());
    EXPECT_FALSE(PathExists(writer.path()));
    EXPECT_EQ(NOT_CALLED, write_callback_observer_.GetAndResetObservationState());
    write_callback_observer_.ObserveNextWriteCallbacks(&writer);
    writer.WriteNow(std::make_unique<std::string>("foo"));
...
TEST_F(ImportantFileWriterTest, ScheduleWrite)
{
    ...
    ImportantFileWriter writer(file_, ThreadTaskRunnerHandle::Get(), kCommitInterval);
...
TEST_F(ImportantFileWriterTest, ScheduleWriteWithBackgroundDataSerializer)
{
    ...
    BackgroundDataSerializer serializer(base::BindLambdaForTesting([&](std::string* data) {
