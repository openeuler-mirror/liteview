...
public:
    ...
    void DidRead(base::RepeatingClosure continuation, File::Error error, const char* data, int bytes_read)
    {
        error_ = error;
...
{
    {
        ...
        {
            RunLoop run_loop;
            proxy.Write(0, "test", 4, BindOnce(&FileProxyTest::DidWrite, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
            run_loop.Run();
        }
        EXPECT_EQ(File::FILE_OK, error_);
...
TEST_F(FileProxyTest, Read)
{
    ...
    ASSERT_EQ(expected_bytes, base::WriteFile(TestPath(), expected_data, expected_bytes));
...
TEST_F(FileProxyTest, WriteAndFlush)
{
    ...
    {
        RunLoop run_loop;
        proxy.Write(0, data, data_bytes, BindOnce(&FileProxyTest::DidWrite, weak_factory_.GetWeakPtr(), run_loop.QuitWhenIdleClosure()));
        run_loop.Run();
    }
    EXPECT_EQ(File::FILE_OK, error_);
...
TEST_F(FileProxyTest, MAYBE_SetTimes)
{
    ...
    EXPECT_EQ(static_cast<int>(last_modified_time.ToDoubleT()), static_cast<int>(info.last_modified.ToDoubleT()));

#if !BUILDFLAG(IS_FUCHSIA)
    // On Fuchsia, /tmp is noatime
    EXPECT_EQ(static_cast<int>(last_accessed_time.ToDoubleT()), static_cast<int>(info.last_accessed.ToDoubleT()));
