...
OnceClosure GetDeleteFileCallback(const FilePath& path, OnceCallback<void(bool)> reply_callback)
{
    return BindOnce(&RunAndReply, BindOnce(&DeleteFile, path),
        reply_callback.is_null() ? std::move(reply_callback) : BindPostTask(SequencedTaskRunnerHandle::Get(), std::move(reply_callback)));
}
...
bool PreReadFileSlow(const FilePath& file_path, int64_t max_bytes)
{
    ...
    File file(file_path, File::FLAG_OPEN | File::FLAG_READ | File::FLAG_WIN_SEQUENTIAL_SCAN | File::FLAG_WIN_SHARE_DELETE);
    if (!file.IsValid())
        return false;
