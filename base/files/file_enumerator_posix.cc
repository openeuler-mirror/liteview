...
{
    DCHECK(st);
    const int res = show_links ? File::Lstat(path.value().c_str(), st) : File::Stat(path.value().c_str(), st);
    if (res < 0) {
...
FilePath FileEnumerator::Next()
{
    ...
    while (current_directory_entry_ >= directory_entries_.size()) {
        ...
        while (errno = 0, dent = readdir(dir)) {
            ...
            if (recursive_ && is_dir && (!ShouldTrackVisitedDirectories(file_type_) || visited_directories_.insert(info.stat_.st_ino).second)) {
