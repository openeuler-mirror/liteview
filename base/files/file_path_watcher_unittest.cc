...
class NotificationCollector : public base::RefCountedThreadSafe<NotificationCollector> {
public:
    ...
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&NotificationCollector::RecordChange, this, base::Unretained(delegate)));
    }
...
public:
    ...
    void OnFileChanged(const FilePath& path, bool error) override
    {
        if (error != expect_error_) {
            ADD_FAILURE() << "Unexpected change for \"" << path << "\" with |error| = " << (error ? "true" : "false");
        } else {
...
protected:
    ...
    [[nodiscard]] bool SetupWatch(const FilePath& target, FilePathWatcher* watcher, TestDelegateBase* delegate, FilePathWatcher::Type watch_type);
    ...
    [[nodiscard]] bool WaitForEventsWithTimeout(TimeDelta timeout)
    {
        ...
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), timeout);
        run_loop.Run();
...
{
    return watcher->Watch(target, watch_type, base::BindRepeating(&TestDelegateBase::OnFileChanged, delegate->AsWeakPtr()));
}
...
TEST_F(FilePathWatcherTest, NewFile)
{
    ...
    ASSERT_TRUE(SetupWatch(test_file(), &watcher, delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_MultipleWatchersSingleFile)
{
    ...
    ASSERT_TRUE(SetupWatch(test_file(), &watcher1, delegate1.get(), FilePathWatcher::Type::kNonRecursive));
    ASSERT_TRUE(SetupWatch(test_file(), &watcher2, delegate2.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_NonExistentDirectory)
{
    ...
    std::unique_ptr<TestDelegate> delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(file, &watcher, delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_DirectoryChain)
{
    ...
    for (std::vector<std::string>::const_iterator d(dir_names.begin()); d != dir_names.end(); ++d) {
...
TEST_F(FilePathWatcherTest, MAYBE_WatchDirectory)
{
    ...
    std::unique_ptr<TestDelegate> delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(dir, &watcher, delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_MoveParent)
{
    ...
    ASSERT_TRUE(SetupWatch(file, &file_watcher, file_delegate.get(), FilePathWatcher::Type::kNonRecursive));
    std::unique_ptr<TestDelegate> subdir_delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(subdir, &subdir_watcher, subdir_delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_RecursiveWatch)
{
    ...
    FilePath dir(temp_dir_.GetPath().AppendASCII("dir"));
    std::unique_ptr<TestDelegate> delegate(new TestDelegate(collector()));
    bool setup_result = SetupWatch(dir, &watcher, delegate.get(), FilePathWatcher::Type::kRecursive);
...
TEST_F(FilePathWatcherTest, RecursiveWithSymLink)
{
    ...
    std::unique_ptr<TestDelegate> delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(symlink, &watcher, delegate.get(), FilePathWatcher::Type::kRecursive));
...
TEST_F(FilePathWatcherTest, MAYBE_MoveChild)
{
    ...
    std::unique_ptr<TestDelegate> file_delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(dest_file, &file_watcher, file_delegate.get(), FilePathWatcher::Type::kNonRecursive));
    std::unique_ptr<TestDelegate> subdir_delegate(new TestDelegate(collector()));
    ASSERT_TRUE(SetupWatch(dest_subdir, &subdir_watcher, subdir_delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, CreateLink)
{
    ...
    ASSERT_TRUE(SetupWatch(test_link(), &watcher, delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, LinkedDirectoryPart1)
{
    ...
    ASSERT_TRUE(SetupWatch(linkfile, &watcher, delegate.get(), FilePathWatcher::Type::kNonRecursive));
...
TEST_F(FilePathWatcherTest, RacyRecursiveWatch)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto watcher_callback = base::BindLambdaForTesting([&](const FilePath& path, bool error) {
...
TEST_F(FilePathWatcherTest, InotifyLimitInWatch)
{
    ...
    ASSERT_FALSE(watcher->Watch(
        test_file(), FilePathWatcher::Type::kNonRecursive, base::BindLambdaForTesting([&](const FilePath& path, bool error) { ADD_FAILURE(); })));
...
TEST_F(FilePathWatcherTest, InotifyLimitInUpdateRecursive)
{
    ...
    for (auto callback_type : { kDestroyWatcher, kDoNothing }) {
        ...
        for (size_t i = 0; i < kMaxLimit; ++i) {
            base::FilePath subdir = dir.AppendASCII(base::StringPrintf("subdir_%" PRIuS, i));
            ASSERT_TRUE(CreateDirectory(subdir));
        }
...
TEST_F(FilePathWatcherTest, TrivialNoDir)
{
    ...
    ASSERT_FALSE(SetupWatch(non_existent, &watcher, delegate.get(), FilePathWatcher::Type::kTrivial));
}
...
TEST_F(FilePathWatcherTest, TrivialDirStart)
{
    ...
    FilePathWatcher watcher;
    auto delegate = std::make_unique<TestDelegate>(collector());
    ASSERT_TRUE(SetupWatch(tmp_dir, &watcher, delegate.get(), FilePathWatcher::Type::kTrivial));
}
...
TEST_F(FilePathWatcherTest, TrivialParentDirChange)
{
    ...
    FilePathWatcher watcher;
    auto delegate = std::make_unique<TestDelegate>(collector());
    ASSERT_TRUE(SetupWatch(sub_dir2, &watcher, delegate.get(), FilePathWatcher::Type::kTrivial));
...
TEST_F(FilePathWatcherTest, TrivialDirMove)
{
    ...
    ASSERT_TRUE(SetupWatch(sub_dir, &watcher, delegate.get(), FilePathWatcher::Type::kTrivial));
