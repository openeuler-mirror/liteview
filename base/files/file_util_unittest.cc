...
TEST_F(FileUtilTest, NormalizeFilePathReparsePoints)
{
    ...
    ASSERT_EQ(static_cast<size_t>(MAX_PATH - kCreateDirLimit), deep_file.value().length());
    ...
    FilePath normalized_path;
    {
        ...
        ASSERT_FALSE(NormalizeFilePath(to_sub_long.Append(deep_txt), &normalized_path));
...
TEST_F(FileUtilTest, NormalizeFilePathSymlinks)
{
    ...
    EXPECT_FALSE(NormalizeFilePath(link_from, &normalized_path)) << "Links to directories should return false.";
...
TEST_F(FileUtilTest, GetTempDirTest)
{
    ...
    static const TCHAR* kTmpValues[] = { _T(""), _T("C:"), _T("C:\\"), _T("C:\\tmp"), _T("C:\\tmp\\") };
...
TEST_F(FileUtilTest, ReadFile)
{
    ...
    int bytes_read_small = ReadFile(file_path, &small_buffer[0], static_cast<int>(small_buffer.size()));
    EXPECT_EQ(static_cast<int>(small_buffer.size()), bytes_read_small);
    EXPECT_EQ(std::string(kTestData.begin(), kTestData.begin() + small_buffer.size()), std::string(small_buffer.begin(), small_buffer.end()));

    // Read the file with buffer which have exactly same size.
    int bytes_read_exact = ReadFile(file_path, &exact_buffer[0], static_cast<int>(exact_buffer.size()));
    ...
    int bytes_read_large = ReadFile(file_path, &large_buffer[0], static_cast<int>(large_buffer.size()));
    ...
    EXPECT_EQ(-1, ReadFile(file_path_not_exist, &exact_buffer[0], static_cast<int>(exact_buffer.size())));
}
...
TEST_F(FileUtilTest, GetUniquePathNumberFilesExist)
{
    ...
    const FilePath some_file = temp_dir_.GetPath().Append(FPL("SomeFile.txt"));
    ASSERT_TRUE(File(some_file, File::FLAG_CREATE | File::FLAG_WRITE).IsValid());
    const FilePath some_file_one = temp_dir_.GetPath().Append(FPL("SomeFile (1).txt"));
    ASSERT_TRUE(File(some_file_one, File::FLAG_CREATE | File::FLAG_WRITE).IsValid());
...
TEST_F(FileUtilTest, GetUniquePathNumberTooManyFiles)
{
    ...
    for (int i = 1; i <= kMaxUniqueFiles; ++i) {
        ...
        ASSERT_TRUE(File(temp_dir_.GetPath().AppendASCII(StringPrintf("SomeFile (%d).txt", i)), File::FLAG_CREATE | File::FLAG_WRITE).IsValid());
    }
...
TEST_F(FileUtilTest, PreReadFile_ExistingFile_ExactSize)
{
    ...
    EXPECT_TRUE(PreReadFile(text_file, /*is_executable=*/false, std::size(bogus_content)));
}
...
TEST_F(FileUtilTest, PreReadFile_ExistingFile_OverSized)
{
    ...
    EXPECT_TRUE(PreReadFile(text_file, /*is_executable=*/false, std::size(bogus_content) * 2));
}
