...
OnceClosure GetDeleteFileCallbackInternal(const FilePath& path, bool recursive, OnceCallback<void(bool)> reply_callback)
{
    ...
    if (!reply_callback.is_null()) {
        bound_callback = BindPostTask(SequencedTaskRunnerHandle::Get(), std::move(reply_callback));
    }
...
bool ReplaceFile(const FilePath& from_path, const FilePath& to_path, File::Error* error)
{
    ...
    base::wcslcpy(from_path_str, from_path.value().c_str(), std::size(from_path_str));
...
bool CreateTemporaryDirInDir(const FilePath& base_dir, const FilePath::StringType& prefix, FilePath* new_dir)
{
    ScopedBlockingCall scoped_blocking_call(FROM_HERE, BlockingType::MAY_BLOCK);
...
bool NormalizeToNativeFilePath(const FilePath& path, FilePath* nt_path)
{
    ...
    win::ScopedHandle file_handle(::CreateFile(path.value().c_str(), GENERIC_READ, kFileShareAll, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL));
    ...
    win::ScopedHandle file_map_handle(::CreateFileMapping(file_handle.Get(), NULL, PAGE_READONLY, 0,
    ...
    void* file_view = MapViewOfFile(file_map_handle.Get(), FILE_MAP_READ, 0, 0, 1);
    if (!file_view)
        return false;
    ...
    if (::GetMappedFileNameWXp(cp, file_view, mapped_file_path, kMaxPathLength)) {
...
bool NormalizeFilePath(const FilePath& path, FilePath* real_path)
{
    ...
    DWORD used_wchars = ::GetFinalPathNameByHandleWXp(file.GetPlatformFile(), native_file_path, kMaxPathLength - 1, FILE_NAME_NORMALIZED | VOLUME_NAME_NT);
    ...
    return DevicePathToDriveLetterPath(FilePath(FilePath::StringPieceType(native_file_path, used_wchars)), real_path);
}
...
int WriteFile(const FilePath& filename, const char* data, int size)
{
    ...
    DWORD written;
    BOOL result = ::WriteFile(file.get(), data, static_cast<DWORD>(size), &written, NULL);
...
PrefetchVirtualMemoryPtr GetPrefetchVirtualMemoryPtr()
{
    ...
    return reinterpret_cast<PrefetchVirtualMemoryPtr>(GetProcAddress(kernel32_dll, "PrefetchVirtualMemory"));
}

} // namespace

bool PreReadFile(const FilePath& file_path, bool is_executable, int64_t max_bytes)
{
