...
void UmaHistogramTimesWithSuffix(const char* histogram_name, StringPiece histogram_suffix, base::TimeDelta sample)
{
...
void DeleteTmpFileWithRetry(File tmp_file, const FilePath& tmp_file_path, int attempt = 0)
{
    ...
    if (!DeleteFile(tmp_file_path) && ++attempt < kMaxDeleteAttempts && SequencedTaskRunnerHandle::IsSet()) {
...
// static
bool ImportantFileWriter::WriteFileAtomically(const FilePath& path, StringPiece data, StringPiece histogram_suffix)
{
...
void ImportantFileWriter::ProduceAndWriteStringToFileAtomically(const FilePath& path, BackgroundDataProducerCallback data_producer_for_background_sequence,
    OnceClosure before_write_callback, OnceCallback<void(bool success)> after_write_callback, const std::string& histogram_suffix)
{
...
// static
bool ImportantFileWriter::WriteFileAtomicallyImpl(const FilePath& path, StringPiece data, StringPiece histogram_suffix, bool from_instance)
{
...
ImportantFileWriter::ImportantFileWriter(const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, StringPiece histogram_suffix)
    : ImportantFileWriter(path, std::move(task_runner), kDefaultCommitInterval, histogram_suffix)
{
}

ImportantFileWriter::ImportantFileWriter(const FilePath& path, scoped_refptr<SequencedTaskRunner> task_runner, TimeDelta interval, StringPiece histogram_suffix)
    : path_(path)
...
void ImportantFileWriter::WriteNowWithBackgroundDataProducer(BackgroundDataProducerCallback background_data_producer)
{
    ...
    auto split_task = SplitOnceCallback(BindOnce(&ProduceAndWriteStringToFileAtomically, path_, std::move(background_data_producer),
        std::move(before_next_write_callback_), std::move(after_next_write_callback_), histogram_suffix_));
