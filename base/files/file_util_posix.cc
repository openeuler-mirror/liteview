...
bool DoDeleteFile(const FilePath& path, bool recursive)
{
    ...
    FileEnumerator traversal(path, true, FileEnumerator::FILES | FileEnumerator::DIRECTORIES | FileEnumerator::SHOW_SYM_LINKS);
...
bool ExecutableExistsInPath(Environment* env, const FilePath::StringType& executable)
{
    ...
    for (const StringPiece& cur_path : SplitStringPiece(path, ":", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
...
bool ReadFileToStringNonBlocking(const base::FilePath& file, std::string* ret)
{
    ...
    base::ScopedFD fd(HANDLE_EINTR(open(file.MaybeAsASCII().c_str(), O_CLOEXEC | O_NONBLOCK | O_RDONLY)));
    if (!fd.is_valid()) {
        return false;
    }
...
int WriteFile(const FilePath& filename, const char* data, int size)
{
    ...
    int bytes_written = WriteFileDescriptor(fd, StringPiece(data, static_cast<size_t>(size))) ? size : -1;
    if (IGNORE_EINTR(close(fd)) < 0)
        return -1;
...
bool WriteFileDescriptor(int fd, span<const uint8_t> data)
{
    ...
    for (ssize_t bytes_written_partial = 0; bytes_written_total < size; bytes_written_total += bytes_written_partial) {
        bytes_written_partial = HANDLE_EINTR(write(fd, data.data() + bytes_written_total, static_cast<size_t>(size - bytes_written_total)));
...
bool AllocateFileRegion(File* file, int64_t offset, size_t size)
{
    ...
    if (File::Fstat(file->GetPlatformFile(), &statbuf) == 0 && statbuf.st_blksize > 0 && base::bits::IsPowerOfTwo(statbuf.st_blksize)) {
