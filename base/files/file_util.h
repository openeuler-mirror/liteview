...
BASE_EXPORT absl::optional<std::vector<uint8_t>> ReadFileToBytes(const FilePath& path);
...
BASE_EXPORT bool CreateTemporaryDirInDir(const FilePath& base_dir, const FilePath::StringType& prefix, FilePath* new_dir);
...
BASE_EXPORT bool PreReadFile(const FilePath& file_path, bool is_executable, int64_t max_bytes = std::numeric_limits<int64_t>::max());
