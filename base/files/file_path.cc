...
StringType FilePath::GetHFSDecomposedForm(StringPieceType string)
{
    ...
    ScopedCFTypeRef<CFStringRef> cfstring(CFStringCreateWithBytesNoCopy(
        NULL, reinterpret_cast<const UInt8*>(string.data()), checked_cast<CFIndex>(string.length()), kCFStringEncodingUTF8, false, kCFAllocatorNull));
    if (cfstring) {
...
int FilePath::CompareIgnoreCase(StringPieceType string1, StringPieceType string2)
{
    ...
    if (hfs1.empty() || hfs2.empty()) {
        ...
        return static_cast<int>(CFStringCompare(cfstring1, cfstring2, kCFCompareCaseInsensitive));
    }
