...
// condition is false.
#define LAZY_CHECK_STREAM(stream, condition) !(condition) ? (void)0 : ::logging::VoidifyStream() & (stream)
...
#define CHECK_FUNCTION_IMPL(check_function, condition)                                                                                                         \
    LAZY_CHECK_STREAM(check_function(__FILE__, __LINE__, #condition).stream(), !ANALYZER_ASSUME_TRUE(condition))

#if defined(OFFICIAL_BUILD) && defined(NDEBUG) && !BUILDFLAG(DCHECK_IS_CONFIGURABLE)
...
#define PCHECK(condition) LAZY_CHECK_STREAM(::logging::CheckError::PCheck(__FILE__, __LINE__).stream(), UNLIKELY(!(condition)))

#else

#define CHECK_WILL_STREAM() true

#define CHECK(condition) CHECK_FUNCTION_IMPL(::logging::CheckError::Check, condition)

#define PCHECK(condition) CHECK_FUNCTION_IMPL(::logging::CheckError::PCheck, condition)

#endif

#if DCHECK_IS_ON()

#define DCHECK(condition) CHECK_FUNCTION_IMPL(::logging::CheckError::DCheck, condition)
#define DPCHECK(condition) CHECK_FUNCTION_IMPL(::logging::CheckError::DPCheck, condition)
