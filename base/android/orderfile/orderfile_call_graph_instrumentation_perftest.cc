...
void CallRecordAddress(int iterations, size_t addr_count)
{
    for (int i = 0; i < iterations; i++) {
        for (size_t caller_addr = kStartOfTextForTesting + kStep; caller_addr < addr_count; caller_addr += kStep) {
            for (size_t callee_addr = caller_addr + kStep; callee_addr < addr_count; callee_addr += kStep) {
...
{
    ResetForTesting();
    auto iterate = [iterations, addresses_count]() { CallRecordAddress(iterations, addresses_count); };
    if (threads != 1) {
    ...
    auto modifier = base::StringPrintf("_%zu_%zu_%d", iterations, addresses_count, threads);
...
void CheckValid(size_t iterations, size_t addr_count)
{
    ...
    for (size_t i = 0; i < reached.size(); i += buckets_per_callee) {
        ...
        for (size_t j = 0; j < callers_per_callee; j++) {
            EXPECT_EQ(reached[i + j * 2 + 1], j > callee_index ? 0UL : (j + 1) * kStep);
