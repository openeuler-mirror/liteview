...
#define INLINE_AND_NO_INSTRUMENT_FUNCTION __attribute__((always_inline, no_instrument_function))
...
class OrderfileMemoryDumpHook : public base::trace_event::MemoryDumpProvider {
    NO_INSTRUMENT_FUNCTION bool OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd) override
    {
        ...
        std::string process_type = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kProcessTypeSwitch);
...
INLINE_AND_NO_INSTRUMENT_FUNCTION uint32_t AtomicIncrement(std::atomic<uint32_t>* element)
{
...
INLINE_AND_NO_INSTRUMENT_FUNCTION bool RecordCaller(size_t index, size_t caller_offset)
{
    ...
    for (size_t i = index; i < index + kCallerBuckets; i++) {
        ...
        if (!offset) {
            ...
            if (g_caller_offset[i].compare_exchange_strong(expected, caller_offset, std::memory_order_relaxed, std::memory_order_relaxed)) {
                AtomicIncrement(g_caller_count + i);
                return true;
            }
        }
...
template <bool for_testing> __attribute__((always_inline, no_instrument_function)) void RecordAddress(size_t callee_address, size_t caller_address)
{
    ...
    if (callers_index == 0) {
        ...
        element->compare_exchange_strong(expected, insertion_index, std::memory_order_relaxed, std::memory_order_relaxed);
    ...
    if (caller_address <= start || caller_address > end || !RecordCaller(callers_index, caller_address - start)) {
...
NO_INSTRUMENT_FUNCTION bool DumpToFile(const base::FilePath& path)
{
    ...
    root.SetStringKey("total_calls_count", base::StringPrintf("%" PRIu32, total_calls_count));
    Value call_graph(Value::Type::LIST);
    for (size_t i = 0; i < kMaxElements; i++) {
        auto caller_index = callee_map[i].load(std::memory_order_relaxed) * kTotalBuckets;
        if (!caller_index)
        ...
        callee_element.SetStringKey("index", base::StringPrintf("%" PRIuS, caller_index));
        callee_element.SetStringKey("callee_offset", base::StringPrintf("%" PRIu32, callee_offset));
        std::string offset_str;
        Value callers_list(Value::Type::LIST);
        for (size_t j = 0; j < kTotalBuckets; j++) {
            uint32_t caller_offset = g_caller_offset[caller_index + j].load(std::memory_order_relaxed);
            ...
            uint32_t count = g_caller_count[caller_index + j].load(std::memory_order_relaxed);
            ...
            caller_count.SetStringKey("caller_offset", base::StringPrintf("%" PRIu32, caller_offset));
    ...
    if (file.WriteAtCurrentPos(output_js.c_str(), static_cast<int>(output_js.size())) < 0) {
...
NO_INSTRUMENT_FUNCTION void StopAndDumpToFile(int pid, uint64_t start_ns_since_epoch, const std::string& tag)
{
    ...
    auto path = base::StringPrintf("/data/local/tmp/chrome/orderfile/profile-hitmap-%s%d-%" PRIu64 ".txt", tag_str.c_str(), pid, start_ns_since_epoch);
...
{
    Disable();
    memset(reinterpret_cast<uint32_t*>(callee_map), 0, sizeof(uint32_t) * kMaxElements);
    memset(reinterpret_cast<uint32_t*>(g_caller_offset), 0, sizeof(uint32_t) * kMaxReachedSymbols * kTotalBuckets);
    memset(reinterpret_cast<uint32_t*>(g_caller_count), 0, sizeof(uint32_t) * kMaxReachedSymbols * kTotalBuckets);
    g_callers_index = 0;
    g_disabled = false;
}

NO_INSTRUMENT_FUNCTION void RecordAddressForTesting(size_t callee_address, size_t caller_address)
{
...
NO_INSTRUMENT_FUNCTION void __cyg_profile_func_enter_bare()
{
    base::android::orderfile::RecordAddress<false>(
        reinterpret_cast<size_t>(__builtin_return_address(0)), reinterpret_cast<size_t>(__builtin_return_address(1)));
}
