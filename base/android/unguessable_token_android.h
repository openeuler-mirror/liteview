...
public:
    ...
    static ScopedJavaLocalRef<jobject> Create(JNIEnv* env, const base::UnguessableToken& token);

    // Create a native UnguessableToken from Java UnguessableToken |token|.
    static base::UnguessableToken FromJavaUnguessableToken(JNIEnv* env, const JavaRef<jobject>& token);
    ...
    // in a clone of |token|.
    static ScopedJavaLocalRef<jobject> ParcelAndUnparcelForTesting(JNIEnv* env, const JavaRef<jobject>& token);
