...
public:
    ...
    void PostDelayedTask(JNIEnv* env, const base::android::JavaRef<jobject>& task, jlong delay, jstring runnable_class_name);

    bool BelongsToCurrentThread(JNIEnv* env);

    static std::unique_ptr<TaskRunnerAndroid> Create(JNIEnv* env, jint task_runner_type, jint priority, jboolean may_block, jboolean use_thread_pool,
