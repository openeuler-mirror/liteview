...
public:
    ...
    static TaskTraits Create(JNIEnv* env, jint priority, jboolean may_block, jbyte extension_id, const base::android::JavaParamRef<jbyteArray>& extension_data)
    {
        return TaskTraits(static_cast<TaskPriority>(priority), may_block,
            TaskTraitsExtensionStorage(static_cast<uint8_t>(extension_id), GetExtensionData(env, extension_data)));
    }

private:
    static std::array<uint8_t, TaskTraitsExtensionStorage::kStorageSize> GetExtensionData(
        JNIEnv* env, const base::android::JavaParamRef<jbyteArray>& array_object)
    {
        ...
        DCHECK_EQ(env->GetArrayLength(array), static_cast<jsize>(TaskTraitsExtensionStorage::kStorageSize));
...
{
    const bool has_extension = traits.extension_id() != TaskTraitsExtensionStorage::kInvalidExtensionId;
    DCHECK(has_extension ^ use_thread_pool) << "A destination (e.g. ThreadPool or UiThreadTaskTraits) is required.";
...
void RunJavaTask(base::android::ScopedJavaGlobalRef<jobject> task, const std::string& runnable_class_name)
{
    ...
    TRACE_EVENT("toplevel", nullptr, [&](::perfetto::EventContext& ctx) { ctx.event()->set_name(event_name.c_str()); });
    JNI_Runnable::Java_Runnable_run(base::android::AttachCurrentThread(), task);
}

} // namespace

jlong JNI_TaskRunnerImpl_Init(JNIEnv* env, jint task_runner_type, jint priority, jboolean may_block, jboolean use_thread_pool, jbyte extension_id,
    const base::android::JavaParamRef<jbyteArray>& extension_data)
{
    TaskRunnerAndroid* task_runner
        = TaskRunnerAndroid::Create(env, task_runner_type, priority, may_block, use_thread_pool, extension_id, extension_data).release();
...
void TaskRunnerAndroid::PostDelayedTask(JNIEnv* env, const base::android::JavaRef<jobject>& task, jlong delay, jstring runnable_class_name)
{
    // This could be run on any java thread, so we can't cache |env| in the
    // BindOnce because JNIEnv is thread specific.
    task_runner_->PostDelayedTask(FROM_HERE,
        base::BindOnce(&RunJavaTask, base::android::ScopedJavaGlobalRef<jobject>(task), android::ConvertJavaStringToUTF8(env, runnable_class_name)),
        Milliseconds(delay));
}
...
bool TaskRunnerAndroid::BelongsToCurrentThread(JNIEnv* env)
{
    ...
    if (type_ == TaskRunnerType::BASE)
        return false;
    return static_cast<SequencedTaskRunner*>(task_runner_.get())->RunsTasksInCurrentSequence();
}

// static
std::unique_ptr<TaskRunnerAndroid> TaskRunnerAndroid::Create(JNIEnv* env, jint task_runner_type, jint priority, jboolean may_block, jboolean use_thread_pool,
    jbyte extension_id, const base::android::JavaParamRef<jbyteArray>& extension_data)
{
    const TaskTraits task_traits = AndroidTaskTraits::Create(env, priority, may_block, extension_id, extension_data);
    TaskExecutor* const task_executor = GetTaskExecutor(use_thread_pool, task_traits);
    ...
    case TaskRunnerType::SINGLE_THREAD:
        task_runner = task_executor->CreateSingleThreadTaskRunner(task_traits, SingleThreadTaskRunnerThreadMode::SHARED);
        break;
    }
