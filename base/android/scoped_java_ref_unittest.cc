...
TEST_F(ScopedJavaRefTest, Conversions)
{
    ...
    // but not implicit conversions.
    {
        ...
        new_global = str;
        EXPECT_SAME_OBJECT(new_local, str);
        static_assert(!std::is_convertible<ScopedJavaLocalRef<jobject>, ScopedJavaGlobalRef<jobject>>::value, "");
        static_assert(!std::is_convertible<ScopedJavaGlobalRef<jobject>, ScopedJavaLocalRef<jobject>>::value, "");
    }
...
TEST_F(JavaObjectArrayReaderTest, InputIteratorRequirements)
{
    ...
    bool same_type = std::is_same<std::iterator_traits<It>::iterator_category, std::input_iterator_tag>::value;
