...
{
#if BUILDFLAG(SUPPORTS_CODE_ORDERING) && !defined(OFFICIAL_BUILD)
    static ReachedAddressesBitset text_bitset(kStartOfText, kEndOfText, reinterpret_cast<std::atomic<uint32_t>*>(g_text_bitfield), kTextBitfieldSize);
    return &text_bitset;
#else
    return nullptr;
#endif
...
ReachedAddressesBitset::ReachedAddressesBitset(uintptr_t start_address, uintptr_t end_address, std::atomic<uint32_t>* storage_ptr, size_t storage_size)
...
{
    size_t reachable_bits = (end_address_ + kBytesGranularity - 1) / kBytesGranularity - start_address_ / kBytesGranularity;
