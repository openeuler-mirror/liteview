...
JavaVM* g_jvm = NULL;
base::LazyInstance<ScopedJavaGlobalRef<jobject>>::Leaky g_class_loader = LAZY_INSTANCE_INITIALIZER;
jmethodID g_class_loader_load_class_method_id = 0;

#if BUILDFLAG(CAN_UNWIND_WITH_FRAME_POINTERS)
base::LazyInstance<base::ThreadLocalPointer<void>>::Leaky g_stack_frame_pointer = LAZY_INSTANCE_INITIALIZER;
#endif
...
jobject GetCachedClassLoader(JNIEnv* env, const std::string& split_name)
{
    ...
    static base::NoDestructor<base::flat_map<std::string, ScopedJavaGlobalRef<jobject>>> split_class_loader_map;
    ...
    ScopedJavaGlobalRef<jobject> class_loader(GetSplitClassLoader(env, split_name));
...
ScopedJavaLocalRef<jclass> GetClassInternal(JNIEnv* env, const char* class_name, jobject class_loader)
{
    jclass clazz;
    if (class_loader != nullptr) {
        ...
        clazz = static_cast<jclass>(env->CallObjectMethod(class_loader, g_class_loader_load_class_method_id, ConvertUTF8ToJavaString(env, dotted_name).obj()));
    } else {
...
void InitReplacementClassLoader(JNIEnv* env, const JavaRef<jobject>& class_loader)
{
...
ScopedJavaLocalRef<jclass> GetClass(JNIEnv* env, const char* class_name, const std::string& split_name)
{
    return GetClassInternal(env, class_name, GetCachedClassLoader(env, split_name));
}
...
// sensitive.
jclass LazyGetClass(JNIEnv* env, const char* class_name, const std::string& split_name, std::atomic<jclass>* atomic_class_id)
{
    ...
    if (atomic_class_id->compare_exchange_strong(cas_result, clazz.obj(), std::memory_order_acq_rel)) {
...
template <MethodID::Type type> jmethodID MethodID::Get(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature)
{
...
jmethodID MethodID::LazyGet(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature, std::atomic<jmethodID>* atomic_method_id)
{
...
template jmethodID MethodID::Get<MethodID::TYPE_STATIC>(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature);

template jmethodID MethodID::Get<MethodID::TYPE_INSTANCE>(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature);

template jmethodID MethodID::LazyGet<MethodID::TYPE_STATIC>(
    JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature, std::atomic<jmethodID>* atomic_method_id);
...
void CheckException(JNIEnv* env)
{
    ...
    if (java_throwable) {
        ...
        if (g_fatal_exception_occurred) {
            ...
            base::android::SetJavaException("Java OOM'ed in exception handling, check logcat");
        } else {
            g_fatal_exception_occurred = true;
            // RVO should avoid any extra copies of the exception string.
            base::android::SetJavaException(GetJavaExceptionInfo(env, java_throwable).c_str());
        }
    }
...
std::string GetJavaExceptionInfo(JNIEnv* env, jthrowable java_throwable)
{
    ...
    jmethodID log_getstacktracestring
        = MethodID::Get<MethodID::TYPE_STATIC>(env, log_clazz.obj(), "getStackTraceString", "(Ljava/lang/Throwable;)Ljava/lang/String;");

    // Call Log.getStackTraceString()
    ScopedJavaLocalRef<jstring> exception_string(
        env, static_cast<jstring>(env->CallStaticObjectMethod(log_clazz.obj(), log_getstacktracestring, java_throwable)));
    CheckException(env);

    ScopedJavaLocalRef<jclass> piielider_clazz = GetClass(env, "org/chromium/base/PiiElider");
    jmethodID piielider_sanitize_stacktrace
        = MethodID::Get<MethodID::TYPE_STATIC>(env, piielider_clazz.obj(), "sanitizeStacktrace", "(Ljava/lang/String;)Ljava/lang/String;");
    ScopedJavaLocalRef<jstring> sanitized_exception_string(
        env, static_cast<jstring>(env->CallStaticObjectMethod(piielider_clazz.obj(), piielider_sanitize_stacktrace, exception_string.obj())));
    CheckException(env);
