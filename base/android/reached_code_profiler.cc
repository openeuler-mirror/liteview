...
struct ScopedTimerCloseTraits {
    static absl::optional<timer_t> InvalidValue()
    {
        return absl::nullopt;
    }

    static void Free(absl::optional<timer_t> x)
    {
        timer_delete(*x);
    }
};
...
using ScopedTimer = base::ScopedGeneric<absl::optional<timer_t>, ScopedTimerCloseTraits>;

void DumpToFile(const base::FilePath& path, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    ...
    std::vector<uint32_t> reached_offsets = ReachedAddressesBitset::GetTextBitset()->GetReachedOffsets();
    base::StringPiece contents(reinterpret_cast<const char*>(reached_offsets.data()), reached_offsets.size());
    if (!base::ImportantFileWriter::WriteFileAtomically(path, contents, "ReachedDump")) {
        LOG(ERROR) << "Could not write reached dump into " << path;
    }

    task_runner->PostDelayedTask(FROM_HERE, base::BindOnce(&DumpToFile, path, task_runner), kDumpInterval);
}
...
public:
    ...
    void Start(LibraryProcessType library_process_type, base::TimeDelta sampling_interval)
    {
        if (is_enabled_)
            return;
        ...
        if (ret) {
            PLOG(ERROR) << "timer_create() failed. The reached code profiler is disabled";
            return;
        }
        ...
        its.it_interval.tv_nsec = checked_cast<long>(sampling_interval.InNanoseconds());
        its.it_value = its.it_interval;
        ret = timer_settime(timerid, 0, &its, nullptr);
        if (ret) {
            PLOG(ERROR) << "timer_settime() failed. The reached code profiler is disabled";
            return;
        }
...
private:
    ...
    void SendSignalToAllThreads()
    {
        ...
        if (iteration_number_ <= kIterationsBeforeSkipping || iteration_number_ % kIterationsBetweenUpdates == 0) {
            tids_.clear();
            if (!base::GetThreadsForProcess(current_pid_, &tids_)) {
                LOG(WARNING) << "Failed to get a list of threads for process " << current_pid_;
                return;
            }
        }
    ...
    void StartDumpingReachedCode()
    {
        ...
        auto file_path = dir_path.Append(base::StringPrintf("reached-code-%d.txt", getpid()));

        dumping_thread_ = std::make_unique<base::Thread>("ReachedCodeProfilerDumpingThread");
        dumping_thread_->StartWithOptions(base::Thread::Options(base::ThreadType::kBackground));
        dumping_thread_->task_runner()->PostDelayedTask(FROM_HERE, base::BindOnce(&DumpToFile, file_path, dumping_thread_->task_runner()), kDumpInterval);
    }
...
void InitReachedCodeProfilerAtStartup(LibraryProcessType library_process_type)
{
    ...
    CHECK(library_process_type == PROCESS_BROWSER || library_process_type == PROCESS_CHILD);
    ...
    if (base::StringToInt(base::CommandLine::ForCurrentProcess()->GetSwitchValueNative(switches::kReachedCodeSamplingIntervalUs), &interval_us)
        && interval_us > 0) {
        sampling_interval = base::Microseconds(interval_us);
    }
    ReachedCodeProfiler::GetInstance()->Start(library_process_type, sampling_interval);
}
