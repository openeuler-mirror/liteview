...
void* android_dlopen_ext(const char*, int, const android_dlextinfo*) __attribute__((weak_import));
...
public:
    LoadTimeReporterJni(JNIEnv* env, jclass modern_linker_jni_class)
        : env_(env)
        , class_(modern_linker_jni_class)
    {
    }

    void reportDlopenExtTime(int64_t millis) const override
    {
        env_->CallStaticVoidMethod(class_, env_->GetStaticMethodID(class_, "reportDlopenExtTime", "(J)V"), millis);
    }

    void reportIteratePhdrTime(int64_t millis) const override
    {
        env_->CallStaticVoidMethod(class_, env_->GetStaticMethodID(class_, "reportIteratePhdrTime", "(J)V"), millis);
    }
...
bool NativeLibInfo::LoadWithDlopenExt(const String& path, const LoadTimeReporter& reporter, void** handle)
{
    LOG_INFO("Entering");
...
bool NativeLibInfo::LoadLibrary(const String& library_path, bool spawn_relro_region, const LoadTimeReporter& reporter)
{
    // Load the library.
    void* handle = nullptr;
...
JNI_GENERATOR_EXPORT jboolean Java_org_chromium_base_library_1loader_ModernLinkerJni_nativeLoadLibrary(
...
JNI_GENERATOR_EXPORT jboolean Java_org_chromium_base_library_1loader_ModernLinkerJni_nativeUseRelros(
...
JNI_GENERATOR_EXPORT jint Java_org_chromium_base_library_1loader_ModernLinkerJni_nativeGetRelroSharingResult(JNIEnv* env, jclass clazz)
{
