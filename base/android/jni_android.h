...
#define JNI_SAVE_FRAME_POINTER base::android::JNIStackFrameSaver jni_frame_saver(__builtin_frame_address(0))
...
#define JNI_LINK_SAVED_FRAME_POINTER                                                                                                                           \
    base::debug::ScopedStackFrameLinker jni_frame_linker(__builtin_frame_address(0), base::android::JNIStackFrameSaver::SavedFrame())
...
// classes.
BASE_EXPORT void InitReplacementClassLoader(JNIEnv* env, const JavaRef<jobject>& class_loader);
...
BASE_EXPORT ScopedJavaLocalRef<jclass> GetClass(JNIEnv* env, const char* class_name, const std::string& split_name);
BASE_EXPORT ScopedJavaLocalRef<jclass> GetClass(JNIEnv* env, const char* class_name);
...
BASE_EXPORT jclass LazyGetClass(JNIEnv* env, const char* class_name, const std::string& split_name, std::atomic<jclass>* atomic_class_id);
BASE_EXPORT jclass LazyGetClass(JNIEnv* env, const char* class_name, std::atomic<jclass>* atomic_class_id);
...
public:
    ...
    template <Type type> static jmethodID Get(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature);

    // The caller is responsible to zero-initialize |atomic_method_id|.
    // It's fine to simultaneously call this on multiple threads referencing the
    // same |atomic_method_id|.
    template <Type type>
    static jmethodID LazyGet(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature, std::atomic<jmethodID>* atomic_method_id);
};
...
BASE_EXPORT std::string GetJavaExceptionInfo(JNIEnv* env, jthrowable java_throwable);
