...
#define CHECK_NATIVE_PTR(env, jcaller, native_ptr, method_name, ...) DCHECK(native_ptr) << method_name;
...
inline void HandleRegistrationError(JNIEnv* env, jclass clazz, const char* filename)
{
...
// TODO(ssid): Implement for other architectures.
    ...
    NOINLINE void Init(JNIEnv* env, jclass clazz, const char* method_name, const char* jni_signature, std::atomic<jmethodID>* atomic_method_id)
    {
        env1 = env;
        ...
        method_id = base::android::MethodID::LazyGet<type>(env, clazz, method_name, jni_signature, atomic_method_id);
    }
...
static_assert(sizeof(JniJavaCallContextChecked) == sizeof(JniJavaCallContextUnchecked), "Stack unwinder cannot work with structs of different sizes.");
