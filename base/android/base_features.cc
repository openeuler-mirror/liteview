...
BASE_FEATURE(kBrowserProcessMemoryPurge, "BrowserProcessMemoryPurge", FEATURE_DISABLED_BY_DEFAULT);
...
BASE_FEATURE(kCrashBrowserOnChildMismatchIfBrowserChanged, "CrashBrowserOnChildMismatchIfBrowserChanged", FEATURE_DISABLED_BY_DEFAULT);

// Crash the browser process if a child process is created which does not match
// the browser process regardless of whether the browser package appears to have
// changed.
BASE_FEATURE(kCrashBrowserOnAnyChildMismatch, "CrashBrowserOnAnyChildMismatch", FEATURE_DISABLED_BY_DEFAULT);
