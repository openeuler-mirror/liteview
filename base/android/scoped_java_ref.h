...
template <> class BASE_EXPORT JavaRef<jobject> {
public:
...
template <typename T> class JavaObjectArrayReader;
...
public:
    ...
    // template parameter.
    template <typename ElementType, typename T_ = T, typename = std::enable_if_t<std::is_same<T_, jobjectArray>::value>>
...
// instances manually.
template <typename T> class JavaParamRef : public JavaRef<T> {
public:
...
template <typename T> class ScopedJavaLocalRef : public JavaRef<T> {
public:
    ...
    ScopedJavaLocalRef(const ScopedJavaLocalRef<U>& other)
        : env_(other.env_)
    {
        JavaRef<T>::SetNewLocalRef(env_, other.obj());
    }
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>>
    ScopedJavaLocalRef(ScopedJavaLocalRef<U>&& other)
        : env_(other.env_)
    {
        JavaRef<T>::steal(std::move(other));
    }
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaLocalRef& operator=(const ScopedJavaLocalRef<U>& other)
    {
        Reset(other);
        return *this;
    }
    ...
    // Move assignment.
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaLocalRef& operator=(ScopedJavaLocalRef<U>&& other)
    {
        env_ = other.env_;
        Reset();
        JavaRef<T>::steal(std::move(other));
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> void Reset(const ScopedJavaLocalRef<U>& other)
    {
...
private:
    ...
    template <typename U> friend class ScopedJavaLocalRef;

    // Avoids JavaObjectArrayReader having to accept and store its own env.
    template <typename U> friend class JavaObjectArrayReader;
};
...
template <typename T> class ScopedJavaGlobalRef : public JavaRef<T> {
public:
    ...
    // Copy conversion constructor.
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaGlobalRef(const ScopedJavaGlobalRef<U>& other)
    {
        Reset(other);
    }
    ...
    // Move conversion constructor.
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaGlobalRef(ScopedJavaGlobalRef<U>&& other)
    {
        JavaRef<T>::steal(std::move(other));
    }
    ...
    // Copy conversion assignment.
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaGlobalRef& operator=(const ScopedJavaGlobalRef<U>& other)
    {
        Reset(other);
        return *this;
    }
    ...
    // Move assignment.
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> ScopedJavaGlobalRef& operator=(ScopedJavaGlobalRef<U>&& other)
    {
        Reset();
        JavaRef<T>::steal(std::move(other));
        return *this;
    ...
    template <typename U, typename = std::enable_if_t<std::is_convertible<U, T>::value>> void Reset(const ScopedJavaGlobalRef<U>& other)
    {
...
public:
    ...
    public:
        ...
        reference operator*() const
        {
            ...
            return value_type::Adopt(reader_->array_.env_, static_cast<T>(reader_->array_.env_->GetObjectArrayElement(reader_->array_.obj(), i_)));
        }
    ...
    JavaObjectArrayReader(const JavaRef<jobjectArray>& array)
        : array_(array)
    {
