...
TEST(JniArray, ByteArrayStringConversions)
{
    ...
    ScopedJavaLocalRef<jbyteArray> bytesFromString = ToJavaByteArray(env, inputString);
...
void CheckBoolConversion(JNIEnv* env, const bool* bool_array, const size_t len, const ScopedJavaLocalRef<jbooleanArray>& booleans)
{
...
void CheckIntConversion(JNIEnv* env, const int* int_array, const size_t len, const ScopedJavaLocalRef<jintArray>& ints)
{
    ASSERT_TRUE(ints);
...
void CheckLongConversion(JNIEnv* env, const int64_t* long_array, const size_t len, const ScopedJavaLocalRef<jlongArray>& longs)
{
    ASSERT_TRUE(longs);
...
void CheckIntArrayConversion(JNIEnv* env, ScopedJavaLocalRef<jintArray> jints, std::vector<int> int_vector, const size_t len)
{
    jint value;
    for (size_t i = 0; i < len; ++i) {
...
void CheckBoolArrayConversion(JNIEnv* env, ScopedJavaLocalRef<jbooleanArray> jbooleans, std::vector<bool> bool_vector, const size_t len)
{
    jboolean value;
    for (size_t i = 0; i < len; ++i) {
...
void CheckFloatConversion(JNIEnv* env, const float* float_array, const size_t len, const ScopedJavaLocalRef<jfloatArray>& floats)
{
    ASSERT_TRUE(floats);
...
TEST(JniArray, FloatConversions)
{
    ...
    JNIEnv* env = AttachCurrentThread();
    CheckFloatConversion(env, kFloats, kLen, ToJavaFloatArray(env, kFloats, kLen));
...
TEST(JniArray, JavaBooleanArrayToBoolVector)
{
    ...
    ASSERT_EQ(static_cast<jsize>(bools.size()), env->GetArrayLength(jbooleans.obj()));
...
TEST(JniArray, JavaLongArrayToInt64Vector)
{
    ...
    ASSERT_EQ(static_cast<jsize>(int64s.size()), env->GetArrayLength(jlongs.obj()));
...
TEST(JniArray, JavaLongArrayToLongVector)
{
    ...
    ASSERT_EQ(static_cast<jsize>(jlongs_vector.size()), env->GetArrayLength(jlongs.obj()));
...
TEST(JniArray, JavaFloatArrayToFloatVector)
{
    ...
    ASSERT_EQ(static_cast<jsize>(floats.size()), env->GetArrayLength(jfloats.obj()));
...
{
    const std::vector<double> kDoubles = { 0.0, 0.5, -0.5, std::numeric_limits<double>::min() };
    JNIEnv* env = AttachCurrentThread();
...
TEST(JniArray, JavaArrayOfByteArrayToStringVector)
{
    ...
    for (int i = 0; i < kMaxItems; ++i) {
        ...
        ScopedJavaLocalRef<jbyteArray> byte_array = ToJavaByteArray(env, reinterpret_cast<uint8_t*>(text), static_cast<size_t>(strlen(text)));
...
TEST(JniArray, JavaArrayOfByteArrayToBytesVector)
{
    ...
    for (size_t i = 0; i < kMaxItems; ++i) {
        ...
        ScopedJavaLocalRef<jbyteArray> byte_array = ToJavaByteArray(env, cur_bytes.data(), cur_bytes.size());
        ASSERT_TRUE(byte_array);
...
TEST(JniArray, JavaArrayOfIntArrayToIntVector)
{
    ...
    // Populate int[][] object.
    const int kInts0[] = { 0, 1, -1, std::numeric_limits<int32_t>::min(), std::numeric_limits<int32_t>::max() };
...
TEST(JniArray, ToJavaArrayOfObjectsOfClass)
{
    ...
    ScopedJavaLocalRef<jobjectArray> j_array = ToJavaArrayOfObjects(env, GetClass(env, "java/lang/String"), objects);
    ASSERT_TRUE(j_array);
