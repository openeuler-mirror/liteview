...
void DumpResidency(size_t start, size_t end, std::unique_ptr<std::vector<TimestampAndResidency>> data)
{
    ...
    auto path = base::FilePath(base::StringPrintf("/data/local/tmp/chrome/residency-%d.txt", getpid()));
    ...
    for (const auto& data_point : *data) {
        auto timestamp = base::StringPrintf("%" PRIu64 " ", data_point.timestamp_nanos);
        file.WriteAtCurrentPos(timestamp.c_str(), static_cast<int>(timestamp.size()));
...
int NativeLibraryPrefetcher::PercentageOfResidentCode(size_t start, size_t end)
{
    ...
    resident_pages += static_cast<size_t>(std::count_if(residency.begin(), residency.end(), [](unsigned char x) { return x & 1; }));
    if (total_pages == 0)
        return -1;
