...
public:
    ScopedCheckExpectation(const char* file, int line, std::string msg)
        ...
        , line_(line)
        , msg_(msg)
        , assert_handler_(base::BindRepeating(&ScopedCheckExpectation::Check, base::Unretained(this)))
        , fired_(false)
    {
    }
    ~ScopedCheckExpectation()
    {
        EXPECT_TRUE(fired_) << "CHECK at " << file_ << ":" << line_ << " never fired!";
    }

private:
    void Check(const char* file, int line, const base::StringPiece msg, const base::StringPiece stack)
    {
        fired_ = true;
...
TEST_F(CheckTest, Basics)
{
    ...
    EXPECT_CHECK("Check failed: a < b (2.000000 vs. 1.000000)foo", CHECK_LT(a, b) << "foo");
}
...
TEST_F(CheckTest, NotReached)
{
    ...
    EXPECT_LOG_ERROR(__LINE__, NOTREACHED() << "foo", "Check failed: false. foo\n");
#else
...
{
    static const std::string expected_msg = std::string("Not implemented reached in ") + __PRETTY_FUNCTION__;
...
{
    static const std::string expected_msg = "Not implemented reached in void (anonymous namespace)::NiLogOnce()\n";
