...
using LogAssertHandlerFunction
    = base::RepeatingCallback<void(const char* file, int line, const base::StringPiece message, const base::StringPiece stack_trace)>;
...
#define COMPACT_GOOGLE_LOG_EX_FATAL(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_FATAL, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_DFATAL(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_DFATAL, ##__VA_ARGS__)
#define COMPACT_GOOGLE_LOG_EX_DCHECK(ClassName, ...) ::logging::ClassName(__FILE__, __LINE__, ::logging::LOGGING_DCHECK, ##__VA_ARGS__)
...
#if BUILDFLAG(IS_CHROMEOS)
    void InitWithSyslogPrefix(base::StringPiece filename, int line, uint64_t tick_count, const char* log_severity_name_c_str, const char* log_prefix,
