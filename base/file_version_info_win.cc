...
std::unique_ptr<FileVersionInfo> FileVersionInfo::CreateFileVersionInfoForModule(HMODULE module)
{
    ...
    if (base::win::GetVersion() >= base::win::Version::VISTA) {
        ...
        if (!has_version_resource)
            return nullptr;
    } else { // SUPPORT_XP_CODE
        DWORD dummy = 0;

        std::vector<wchar_t> path_buff;
        path_buff.resize(MAX_PATH);
        ::GetModuleFileNameW(module, path_buff.data(), path_buff.size());
        const wchar_t* path = path_buff.data();
