...
constexpr ThreadType kAllThreadTypes[] = { ThreadType::kRealtimeAudio, ThreadType::kDisplayCritical, ThreadType::kCompositing, ThreadType::kDefault,
...
ThreadPriorityForTest GetCurrentThreadPriorityIfStartWithThreadType(ThreadType thread_type, MessagePumpType message_pump_type)
{
    ...
    thread.task_runner()->PostTask(
        FROM_HERE, BindOnce([](ThreadPriorityForTest* priority) { *priority = PlatformThread::GetCurrentThreadPriorityForTest(); }, &priority));
    thread.Stop();

    return priority;
}

ThreadPriorityForTest GetCurrentThreadPriorityIfSetThreadTypeLater(ThreadType thread_type, MessagePumpType message_pump_type)
{
    Thread::Options options;
    options.message_pump_type = message_pump_type;
...
void TestPriorityResultingFromThreadType(ThreadType thread_type, MessagePumpType message_pump_type, ThreadPriorityForTest priority)
{
    testing::Message message;
    message << "thread_type: " << static_cast<int>(thread_type) << ", message_pump_type: " << static_cast<int>(message_pump_type);
    SCOPED_TRACE(message);

    if (PlatformThread::CanChangeThreadType(ThreadType::kDefault, thread_type)) {
        EXPECT_EQ(GetCurrentThreadPriorityIfStartWithThreadType(thread_type, message_pump_type), priority);
        EXPECT_EQ(GetCurrentThreadPriorityIfSetThreadTypeLater(thread_type, message_pump_type), priority);
    }
}
...
TEST(PlatformThreadTest, CanChangeThreadType)
{
    ...
    EXPECT_FALSE(PlatformThread::CanChangeThreadType(ThreadType::kBackground, ThreadType::kCompositing));
    EXPECT_FALSE(PlatformThread::CanChangeThreadType(ThreadType::kDefault, ThreadType::kBackground));
    EXPECT_FALSE(PlatformThread::CanChangeThreadType(ThreadType::kCompositing, ThreadType::kBackground));
#else
    ...
    EXPECT_EQ(PlatformThread::CanChangeThreadType(ThreadType::kBackground, ThreadType::kCompositing), kCanIncreasePriority);
    EXPECT_TRUE(PlatformThread::CanChangeThreadType(ThreadType::kDefault, ThreadType::kBackground));
    EXPECT_TRUE(PlatformThread::CanChangeThreadType(ThreadType::kCompositing, ThreadType::kBackground));
#endif
...
TEST(PlatformThreadTest, SetCurrentThreadTypeTest)
{
    ...
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, ThreadPriorityForTest::kDisplay);
#if BUILDFLAG(IS_WIN)
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, MessagePumpType::UI, ThreadPriorityForTest::kNormal);
#else
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, MessagePumpType::UI, ThreadPriorityForTest::kDisplay);
#endif // BUILDFLAG(IS_WIN)
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, MessagePumpType::IO, ThreadPriorityForTest::kDisplay);
#else // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_FUCHSIA)
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, ThreadPriorityForTest::kNormal);
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, MessagePumpType::UI, ThreadPriorityForTest::kNormal);
    TestPriorityResultingFromThreadType(ThreadType::kCompositing, MessagePumpType::IO, ThreadPriorityForTest::kNormal);
#endif
...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_IOS) || BUILDFLAG(IS_FUCHSIA) || ((BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)) && !defined(THREAD_SANITIZER))         \
