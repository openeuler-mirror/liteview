...
protected:
    ...
    static void VerifyCurrentSequencedTaskRunner()
    {
        ...
        std::unique_ptr<SequenceCheckerImpl> sequence_checker(new SequenceCheckerImpl);
        task_runner->PostTask(FROM_HERE, base::BindOnce(&SequencedTaskRunnerHandleTest::CheckValidSequence, std::move(sequence_checker)));
    }
...
{
    base::ThreadPool::CreateSequencedTaskRunner({})->PostTask(FROM_HERE, base::BindOnce(&SequencedTaskRunnerHandleTest::VerifyCurrentSequencedTaskRunner));
    task_environment_.RunUntilIdle();
}

TEST_F(SequencedTaskRunnerHandleTest, NoHandleFromUnsequencedTask)
{
    base::ThreadPool::PostTask(base::BindOnce([]() { EXPECT_FALSE(SequencedTaskRunnerHandle::IsSet()); }));
    task_environment_.RunUntilIdle();
}
