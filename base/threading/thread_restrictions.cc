...
public:
    ...
    friend std::ostream& operator<<(std::ostream& out, const BooleanWithStack& bws)
    {
        out << bws.value_;
        if (kCaptureStackTraces) {
...
{
    DCHECK(!*GetBlockingDisallowedTls()) << "Function marked as blocking was called from a scope that disallows "
...
{
    DCHECK(*GetBlockingDisallowedTls()) << "g_blocking_disallowed " << *GetBlockingDisallowedTls();
}
...
ScopedDisallowBlocking::ScopedDisallowBlocking()
    : was_disallowed_(GetBlockingDisallowedTls().Set(std::make_unique<BooleanWithStack>(true)))
{
}

ScopedDisallowBlocking::~ScopedDisallowBlocking()
{
    DCHECK(*GetBlockingDisallowedTls()) << "~ScopedDisallowBlocking() running while surprisingly already no "
                                           "longer disallowed.\n"
                                        << "g_blocking_disallowed " << *GetBlockingDisallowedTls();
...
{
    GetBaseSyncPrimitivesDisallowedTls().Set(std::make_unique<BooleanWithStack>(true));
}

ScopedDisallowBaseSyncPrimitives::ScopedDisallowBaseSyncPrimitives()
    : was_disallowed_(GetBaseSyncPrimitivesDisallowedTls().Set(std::make_unique<BooleanWithStack>(true)))
{
}

ScopedDisallowBaseSyncPrimitives::~ScopedDisallowBaseSyncPrimitives()
{
    DCHECK(*GetBaseSyncPrimitivesDisallowedTls()) << "~ScopedDisallowBaseSyncPrimitives() running while surprisingly "
                                                     "already no longer disallowed.\n"
                                                  << "g_base_sync_primitives_disallowed " << *GetBaseSyncPrimitivesDisallowedTls();
...
ScopedAllowBaseSyncPrimitives::ScopedAllowBaseSyncPrimitives()
    : was_disallowed_(GetBaseSyncPrimitivesDisallowedTls().Set(std::make_unique<BooleanWithStack>(false)))
{
    DCHECK(!*GetBlockingDisallowedTls()) << "To allow //base sync primitives in a scope where blocking is "
...
ScopedAllowUnresponsiveTasksForTesting::ScopedAllowUnresponsiveTasksForTesting()
    : was_disallowed_base_sync_(GetBaseSyncPrimitivesDisallowedTls().Set(std::make_unique<BooleanWithStack>(false)))
    , was_disallowed_blocking_(GetBlockingDisallowedTls().Set(std::make_unique<BooleanWithStack>(false)))
    , was_disallowed_cpu_(GetCPUIntensiveWorkDisallowedTls().Set(std::make_unique<BooleanWithStack>(false)))
{
}
...
ScopedAllowUnresponsiveTasksForTesting::~ScopedAllowUnresponsiveTasksForTesting()
{
    ...
    GetBaseSyncPrimitivesDisallowedTls().Set(std::move(was_disallowed_base_sync_));
...
{
    DCHECK(!*GetBaseSyncPrimitivesDisallowedTls()) << "Waiting on a //base sync primitive is not allowed on this thread to "
                                                   ...
                                                   << "g_base_sync_primitives_disallowed " << *GetBaseSyncPrimitivesDisallowedTls()
                                                   << "It can be useful to know that g_blocking_disallowed is " << *GetBlockingDisallowedTls();
}
...
void ResetThreadRestrictionsForTesting()
{
    ...
    GetBaseSyncPrimitivesDisallowedTls().Set(std::make_unique<BooleanWithStack>(false));
    GetCPUIntensiveWorkDisallowedTls().Set(std::make_unique<BooleanWithStack>(false));
}

void AssertSingletonAllowed()
{
    DCHECK(!*GetSingletonDisallowedTls()) << "LazyInstance/Singleton is not allowed to be used on this thread. "
...
ScopedDisallowSingleton::ScopedDisallowSingleton()
    : was_disallowed_(GetSingletonDisallowedTls().Set(std::make_unique<BooleanWithStack>(true)))
{
}

ScopedDisallowSingleton::~ScopedDisallowSingleton()
{
    DCHECK(*GetSingletonDisallowedTls()) << "~ScopedDisallowSingleton() running while surprisingly already no "
                                            "longer disallowed.\n"
                                         << "g_singleton_disallowed " << *GetSingletonDisallowedTls();
...
{
    DCHECK(!*GetCPUIntensiveWorkDisallowedTls()) << "Function marked as CPU intensive was called from a scope that "
                                                    "disallows this kind of work! Consider making this work "
                                                    "asynchronous.\n"
                                                 << "g_cpu_intensive_work_disallowed " << *GetCPUIntensiveWorkDisallowedTls();
}
...
void DisallowUnresponsiveTasks()
{
    ...
    GetCPUIntensiveWorkDisallowedTls().Set(std::make_unique<BooleanWithStack>(true));
}
...
#if DCHECK_IS_ON()
    : was_disallowed_(GetBlockingDisallowedTls().Set(std::make_unique<BooleanWithStack>(false)))
#endif
{
    //   TRACE_EVENT_BEGIN(
