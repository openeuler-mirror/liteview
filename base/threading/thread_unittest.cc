...
public:
    ...
    {
        init_called_ = false;
        ANNOTATE_BENIGN_RACE(this, "Benign test-only data race on vptr - http://crbug.com/98219");
    }
...
// destroyed.
class CapturingDestructionObserver : public base::CurrentThread::DestructionObserver {
public:
...
void RegisterDestructionObserver(base::CurrentThread::DestructionObserver* observer)
{
...
// signal |event|.
void ReturnThreadId(base::Thread* thread, base::PlatformThreadId* id, base::WaitableEvent* event)
{
...
TEST_F(ThreadTest, StartWithOptions_StackSize)
{
    ...
    a.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));
    event.Wait();
}
...
TEST_F(ThreadTest, StartWithOptions_NonJoinable)
{
    ...
    base::WaitableEvent block_event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
    a->task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Wait, base::Unretained(&block_event)));
...
TEST_F(ThreadTest, TwoTasksOnJoinableThread)
{
    bool was_invoked = false;
    {
        ...
        a.task_runner()->PostTask(FROM_HERE, base::BindOnce(static_cast<void (*)(base::TimeDelta)>(&base::PlatformThread::Sleep), base::Milliseconds(20)));
        a.task_runner()->PostTask(FROM_HERE, base::BindOnce(&ToggleValue, &was_invoked));
    }
...
TEST_F(ThreadTest, DISABLED_StopOnNonOwningThreadIsDeath)
{
    ...
    EXPECT_DCHECK_DEATH({
        ...
        b.task_runner()->PostTask(FROM_HERE, base::BindOnce(&Thread::Stop, base::Unretained(&a)));
...
TEST_F(ThreadTest, TransferOwnershipAndStop)
{
    ...
    b.task_runner()->PostTask(FROM_HERE,
        base::BindOnce(
            [](std::unique_ptr<Thread> thread_to_stop, base::WaitableEvent* event_to_signal) -> void {
                thread_to_stop->Stop();
...
TEST_F(ThreadTest, StartTwiceNonJoinableNotAllowed)
{
    ...
    base::WaitableEvent last_task_event(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
    a->task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&last_task_event)));
...
TEST_F(ThreadTest, ThreadId)
{
    ...
    a.task_runner()->PostTask(FROM_HERE, base::BindOnce(ReturnThreadId, &a, &id_from_new_thread, &event));
...
TEST_F(ThreadTest, CleanUp)
{
...

    {
        ...
        t.task_runner()->PostTask(FROM_HERE, base::BindOnce(&RegisterDestructionObserver, base::Unretained(&loop_destruction_observer)));
...
TEST_F(ThreadTest, FlushForTesting)
{
    ...
    for (size_t i = 0; i < kNumSleepTasks; ++i) {
        a.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::PlatformThread::Sleep, kSleepPerTestTask));
    }

    // All tasks should have executed, as reflected by the elapsed time.
    a.FlushForTesting();
    EXPECT_GE(base::TimeTicks::Now() - ticks_before_post, kNumSleepTasks * kSleepPerTestTask);
...
TEST_F(ThreadTest, GetTaskExecutorForCurrentThread)
{
    ...
    a.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
...
public:
    SequenceManagerThreadDelegate()
        ...
        , task_queue_(sequence_manager_->CreateTaskQueueWithType<TaskQueue>(TaskQueue::Spec(base::sequence_manager::QueueName::DEFAULT_TQ)))
    {
    ...
    {
        sequence_manager_->BindToMessagePump(base::MessagePump::Create(base::MessagePumpType::DEFAULT));
...
TEST_F(ThreadTest, ProvidedThreadDelegate)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = options.delegate->GetDefaultTaskRunner();
    thread.StartWithOptions(std::move(options));

    base::WaitableEvent event;
    task_runner->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));
    event.Wait();
