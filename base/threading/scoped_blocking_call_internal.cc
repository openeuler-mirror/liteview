...
LazyInstance<ThreadLocalPointer<BlockingObserver>>::Leaky tls_blocking_observer = LAZY_INSTANCE_INITIALIZER;

// Last ScopedBlockingCall instantiated on this thread.
LazyInstance<ThreadLocalPointer<UncheckedScopedBlockingCall>>::Leaky tls_last_scoped_blocking_call = LAZY_INSTANCE_INITIALIZER;
...
UncheckedScopedBlockingCall::UncheckedScopedBlockingCall(const Location& from_here, BlockingType blocking_type, BlockingCallType blocking_call_type)
    ...
    , scoped_activity_(from_here, 0, kActivityTrackerId, 0)
{
    ...
    if (scoped_activity_.IsRecorded()) {
        ...
        user_data.SetUint("timestamp_us", static_cast<uint64_t>(now.since_origin().InMicroseconds()));
