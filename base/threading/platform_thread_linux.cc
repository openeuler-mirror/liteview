...
void SetThreadLatencySensitivity(ProcessId process_id, PlatformThreadId thread_id, ThreadType thread_type)
{
    ...
    if (is_urgent && latency_sensitive_urgent) {
        PLOG_IF(ERROR, !WriteFile(latency_sensitive_file, "1", 1)) << "Failed to write latency file.\n";
    } else {
        PLOG_IF(ERROR, !WriteFile(latency_sensitive_file, "0", 1)) << "Failed to write latency file.\n";
    }
...
void SetThreadCgroupsForThreadType(PlatformThreadId thread_id, ThreadType thread_type)
{
...
const ThreadPriorityToNiceValuePairForTest kThreadPriorityToNiceValueMapForTest[4] = {
...
bool SetCurrentThreadTypeForPlatform(ThreadType thread_type, MessagePumpType pump_type_hint)
{
    ...
    return thread_type == ThreadType::kRealtimeAudio && pthread_setschedparam(pthread_self(), SCHED_RR, &kRealTimePrio) == 0;
#else
    return false;
#endif
}

absl::optional<ThreadPriorityForTest> GetCurrentThreadPriorityForPlatformForTest()
{
#if !BUILDFLAG(IS_NACL)
...
#if !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_AIX)
// static
void PlatformThread::SetThreadType(ProcessId process_id, PlatformThreadId thread_id, ThreadType thread_type)
{
