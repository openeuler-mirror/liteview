...
struct DefaultCrossThreadBindTraits {
    ...
    template <typename Signature> static inline bool PostTask(SequencedTaskRunner& task_runner, const Location& location, CrossThreadTask<Signature>&& task)
    {
    ...
    template <template <typename> class CallbackType> using EnableIfIsCrossThreadTask = EnableIfIsBaseCallback<CallbackType>;
};
...
template <typename T, class CrossThreadBindTraits = internal::DefaultCrossThreadBindTraits> class SequenceBound {
public:
    template <typename Signature> using CrossThreadTask = typename CrossThreadBindTraits::template CrossThreadTask<Signature>;
    ...
    template <typename From> SequenceBound& operator=(SequenceBound<From, CrossThreadBindTraits>&& other)
    {
        Reset();
        MoveRecordFrom(other);
        return *this;
    ...
    template <typename R, typename C, typename... Args, typename = std::enable_if_t<std::is_base_of_v<C, T>>>
    ...
    void PostTaskWithThisObject(ConstPostTaskCallback callback, const Location& location = Location::Current()) const
    {
        ...
        CrossThreadBindTraits::PostTask(*impl_task_runner_, location, CrossThreadBindTraits::BindOnce(std::move(callback), std::cref(*t_)));
    }
    ...
    void PostTaskWithThisObject(PostTaskCallback callback, const Location& location = Location::Current()) const
    {
        ...
        CrossThreadBindTraits::PostTask(
            *impl_task_runner_, location, CrossThreadBindTraits::BindOnce(std::move(callback), CrossThreadBindTraits::Unretained(t_)));
    }
    ...
    void FlushPostedTasksForTesting() const
    {
        DCHECK(!is_null());
        RunLoop run_loop;
        CrossThreadBindTraits::PostTask(*impl_task_runner_, FROM_HERE, OnceClosure(run_loop.QuitClosure()));
        run_loop.Run();
    }
    ...
    void Reset()
    {
        ...
        CrossThreadBindTraits::PostTask(*impl_task_runner_, FROM_HERE,
            CrossThreadBindTraits::BindOnce(&DeleteOwnerRecord, CrossThreadBindTraits::Unretained(t_), CrossThreadBindTraits::Unretained(storage_)));
    ...
    void SynchronouslyResetForTest()
    {
        ...
        RunLoop run_loop;
        CrossThreadBindTraits::PostTask(*impl_task_runner_, FROM_HERE,
            CrossThreadBindTraits::BindOnce([](T* t, void* storage) { DeleteOwnerRecord(t, storage); }, t_, storage_).Then(run_loop.QuitClosure()));
        run_loop.Run();
...
private:
    // For move conversion.
    template <typename U, class Binder> friend class SequenceBound;

    template <template <typename> class CallbackType>
    using EnableIfIsCrossThreadTask = typename CrossThreadBindTraits::template EnableIfIsCrossThreadTask<CallbackType>;
    ...
    template <typename... Args> NO_SANITIZE("cfi-unrelated-cast") void AsyncConstruct(Args&&... args)
    {
        DCHECK(!t_);
        ...
        CrossThreadBindTraits::PostTask(*impl_task_runner_, FROM_HERE,
            CrossThreadBindTraits::BindOnce(&ConstructOwnerRecord<Args...>, CrossThreadBindTraits::Unretained(t_), std::forward<Args>(args)...));
    }
    ...
    protected:
        ...
        raw_ptr<const SequenceBound<T, CrossThreadBindTraits>, DanglingUntriaged> sequence_bound_;
    ...
    public:
        ...
        ~AsyncCallBuilderImpl()
        {
            if (this->sequence_bound_) {
                CrossThreadBindTraits::PostTask(*this->sequence_bound_->impl_task_runner_, *this->location_,
                    CrossThreadBindTraits::BindOnce(this->method_, CrossThreadBindTraits::Unretained(this->sequence_bound_->t_)));
            }
        }
        ...
        void Then(OnceClosure then_callback) &&
        {
            this->sequence_bound_->PostTaskAndThenHelper(*this->location_,
                CrossThreadBindTraits::BindOnce(this->method_, CrossThreadBindTraits::Unretained(this->sequence_bound_->t_)), std::move(then_callback));
    ...
    public:
        ...
        template <template <typename> class CallbackType, typename ThenArg, typename = EnableIfIsCrossThreadTask<CallbackType>>
    ...
    public:
        ...
        template <typename... BoundArgs> auto WithArgs(BoundArgs&&... bound_args)
        {
            ...
            return AsyncCallWithBoundArgsBuilder<ReturnType>(sequence_bound, this->location_,
                CrossThreadBindTraits::BindOnce(this->method_, CrossThreadBindTraits::Unretained(sequence_bound->t_), std::forward<BoundArgs>(bound_args)...));
        }
    ...
    public:
        ...
        {
            if (this->sequence_bound_) {
                CrossThreadBindTraits::PostTask(*this->sequence_bound_->impl_task_runner_, *this->location_, std::move(this->callback_));
            }
        }
    ...
    using AsyncCallWithBoundArgsBuilder =
        typename std::conditional<std::is_void<ReturnType>::value, AsyncCallWithBoundArgsBuilderVoid, AsyncCallWithBoundArgsBuilderDefault<ReturnType>>::type;
    ...
    {
        CrossThreadBindTraits::PostTaskAndReply(*impl_task_runner_, location, std::move(callback), std::move(then_callback));
    }

    template <typename ReturnType, template <typename> class CallbackType, typename ThenArg, typename = EnableIfIsCrossThreadTask<CallbackType>>
    void PostTaskAndThenHelper(const Location& location, CrossThreadTask<ReturnType()> callback, CallbackType<void(ThenArg)> then_callback) const
    {
        ...
        CrossThreadBindTraits::PostTaskAndReplyWithResult(*impl_task_runner_, location, std::move(callback), std::move(once_then_callback));
    }
    ...
    template <typename From> void NO_SANITIZE("cfi-unrelated-cast") MoveRecordFrom(From&& other)
    {
    ...
    template <typename... Args> static void ConstructOwnerRecord(T* t, std::decay_t<Args>&&... args)
    {
