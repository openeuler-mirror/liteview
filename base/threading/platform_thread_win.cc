...
#if BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && BUILDFLAG(STARSCAN)
    partition_alloc::internal::PCScan::NotifyThreadCreated(partition_alloc::internal::GetStackPointer());
#endif
...
void PlatformThread::Join(PlatformThreadHandle thread_handle)
{
    ...
    DWORD thread_id = 0;
    thread_id = ::GetThreadIdXp(thread_handle.platform_handle());
    DWORD last_error = 0;
    if (!thread_id)
...
{
    if (thread_type == ThreadType::kCompositing && pump_type_hint == MessagePumpType::UI) {
    ...
    [[maybe_unused]] const BOOL success = ::SetThreadPriority(thread_handle, desired_priority);
...
void SetCurrentThreadQualityOfService(ThreadType thread_type)
{
    ...
    static const auto set_thread_information_fn
        = reinterpret_cast<decltype(&::SetThreadInformation)>(::GetProcAddress(::GetModuleHandle(L"kernel32.dll"), "SetThreadInformation"));
    ...
    thread_power_throttling_state.Version = THREAD_POWER_THROTTLING_CURRENT_VERSION;
    thread_power_throttling_state.ControlMask = desire_ecoqos ? THREAD_POWER_THROTTLING_EXECUTION_SPEED : 0ul;
    thread_power_throttling_state.StateMask = desire_ecoqos ? THREAD_POWER_THROTTLING_EXECUTION_SPEED : 0ul;

    [[maybe_unused]] const BOOL success
        = set_thread_information_fn(::GetCurrentThread(), ::ThreadPowerThrottling, &thread_power_throttling_state, sizeof(thread_power_throttling_state));
    DPLOG_IF(ERROR, !success) << "Failed to set EcoQoS to " << std::boolalpha << desire_ecoqos;
}
