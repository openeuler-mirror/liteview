...
void LogHungThreadCountHistogram(HangWatcher::ThreadType thread_type, int count)
{
    ...
    case HangWatcher::ProcessType::kRendererProcess:
        switch (thread_type) {
        case HangWatcher::ThreadType::kIOThread:
            UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.RendererProcess."
                                  "IOThread",
                any_thread_hung);
            break;
        case HangWatcher::ThreadType::kMainThread:
    ...
    case HangWatcher::ProcessType::kUtilityProcess:
        switch (thread_type) {
        case HangWatcher::ThreadType::kIOThread:
            UMA_HISTOGRAM_BOOLEAN("HangWatcher.IsThreadHung.UtilityProcess."
                                  "IOThread",
                any_thread_hung);
            break;
        case HangWatcher::ThreadType::kMainThread:
...
WatchHangsInScope::WatchHangsInScope(TimeDelta timeout)
{
    internal::HangWatchState* current_hang_watch_state
        = HangWatcher::IsEnabled() ? internal::HangWatchState::GetHangWatchStateForCurrentThread()->Get() : nullptr;
...
WatchHangsInScope::~WatchHangsInScope()
{
    ...
    internal::HangWatchState* current_hang_watch_state = internal::HangWatchState::GetHangWatchStateForCurrentThread()->Get();
...
std::string HangWatcher::GetTimeSinceLastSystemPowerResumeCrashKeyValue() const
{
    ...
    const TimeTicks last_system_power_resume_time = PowerMonitor::GetLastSystemResumeTime();
...
void HangWatcher::WatchStateSnapShot::Init(const HangWatchStates& watch_states, base::TimeTicks deadline_ignore_threshold)
{
    DCHECK(!initialized_);
...
void HangWatcher::UnregisterThread()
{
    ...
    auto it = ranges::find_if(
        watch_states_, [current_hang_watch_state](const std::unique_ptr<internal::HangWatchState>& state) { return state.get() == current_hang_watch_state; });
...
// static
ThreadLocalPointer<HangWatchState>* HangWatchState::GetHangWatchStateForCurrentThread()
{
