...
{
    scoped_refptr<SingleThreadTaskRunner> task_runner(new TestSimpleTaskRunner);
    scoped_refptr<SingleThreadTaskRunner> overidding_task_runner(new TestSimpleTaskRunner);

    ThreadTaskRunnerHandle ttrh(task_runner);
    EXPECT_DCHECK_DEATH({ ThreadTaskRunnerHandle overriding_ttrh(overidding_task_runner); });
}
