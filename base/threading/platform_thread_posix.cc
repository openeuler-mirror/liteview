...
#if !BUILDFLAG(IS_NACL) && BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC) && BUILDFLAG(STARSCAN)
...
SimpleTlsIntptrXp g_thread_id(-1);
...
SimpleTlsBoolXp g_is_main_thread(true);
...
// 
//     if (g_thread_id.Get() == -1 || (g_is_main_thread.Get() && !g_main_thread_tid_cache_valid.load(std::memory_order_relaxed))) {
...
//         if (g_thread_id.Get() == getpid()) {
//             g_main_thread_tid_cache_valid.store(true, std::memory_order_relaxed);
//         } else {
//             g_is_main_thread.Set(false);
//         }
//     } else {
// #if DCHECK_IS_ON()
//         if (g_thread_id.Get() != syscall(__NR_gettid)) {
//             RAW_LOG(FATAL,
...
// #endif
//     }
//     return g_thread_id.Get();
    return syscall(__NR_gettid); // weolar: 不管了，用老方法，可能就是性能慢了点
