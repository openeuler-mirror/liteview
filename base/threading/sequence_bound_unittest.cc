...
TEST_F(SequenceBoundTest, SequenceValidation)
{
    ...
    SequenceBound<Validator> validator(background_task_runner_, background_task_runner_);
    ...
    validator.PostTaskWithThisObject(BindLambdaForTesting([](const Validator& v) { v.ReturnsVoid(); }));
    validator.PostTaskWithThisObject(BindLambdaForTesting([](Validator* v) { v->ReturnsVoidMutable(); }));
    validator.Reset();
    FlushPostedTasks();
}
...
TEST_F(SequenceBoundTest, Basic)
{
    ...
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0"));

    value.AsyncCall(&BoxedValue::set_value).WithArgs(66);
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("updated BoxedValue from 0 to 66"));
    ...
    EXPECT_FALSE(value);
    FlushPostedTasks();
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("destroyed BoxedValue = 66"));
}
...
TEST_F(SequenceBoundTest, ConstructAndImmediateAsyncCall)
{
    ...
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0", "updated BoxedValue from 0 to 8"));
}
...
TEST_F(SequenceBoundTest, MoveConstruction)
{
    ...
    SequenceBound<Derived> derived_old(background_task_runner_, std::ref(logger_));
    SequenceBound<Derived> derived_new = std::move(derived_old);
    EXPECT_TRUE(derived_old.is_null());
    EXPECT_FALSE(derived_new.is_null());
    derived_new.Reset();
    FlushPostedTasks();
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("constructed Base", "constructed Derived", "destroyed Derived", "destroyed Base"));
}
...
{
    SequenceBound<MultiplyDerived> multiply_derived(background_task_runner_, std::ref(logger_));
...
TEST_F(SequenceBoundTest, ConstructWithLvalue)
{
    ...
    value.FlushPostedTasksForTesting();
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 99"));
}
...
TEST_F(SequenceBoundTest, FlushPostedTasksForTesting)
{
    ...
    EXPECT_THAT(logger_.TakeEvents(), ::testing::ElementsAre("constructed BoxedValue = 0", "updated BoxedValue from 0 to 42"));
}
...
TEST_F(SequenceBoundTest, EmplaceOverExistingWithTaskRunnerSwap)
{
    ...
    value.emplace(background_task_runner_, 9);
    {
        value.PostTaskWithThisObject(
            BindLambdaForTesting([another_task_runner, background_task_runner = background_task_runner_](const BoxedValue& boxed_value) {
...
TEST_F(SequenceBoundTest, AsyncCallIntArgNoThen)
{
    ...
    SequenceBound<IntArgVoidReturn> s(background_task_runner_, &method_called_with, &const_method_called_with);
...
TEST_F(SequenceBoundTest, AsyncCallIgnoreResultNoArgs)
{
    {
        RunLoop loop;
        SequenceBound<IgnoreResultTestHelperWithNoArgs> s(background_task_runner_, &loop, nullptr);
...
{
    {
        RunLoop loop;
        bool called = false;
        SequenceBound<IgnoreResultTestHelperWithNoArgs> s(background_task_runner_, nullptr, &called);
...
{
    {
        RunLoop loop;
        int result = 0;
        SequenceBound<IgnoreResultTestHelperWithArgs> s(background_task_runner_, &loop, std::ref(result));
...
{
    {
        RunLoop loop;
        int result = 0;
        SequenceBound<IgnoreResultTestHelperWithArgs> s(background_task_runner_, nullptr, std::ref(result));
...
protected:
    ...
    scoped_refptr<SequencedTaskRunner> task_runner_ = SequencedTaskRunnerHandle::Get();
};
