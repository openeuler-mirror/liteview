...
TEST(PlatformThreadWinTest, SetBackgroundThreadModeFailsInIdlePriorityProcess)
{
    ...
    EXPECT_EQ(Process::Current().GetPriority(), static_cast<int>(IDLE_PRIORITY_CLASS));
    ...
    if (win::GetVersion() == win::Version::WIN7) {
        const ThreadPriorityForTest priority = PlatformThread::GetCurrentThreadPriorityForTest();
        EXPECT_TRUE(priority == ThreadPriorityForTest::kNormal || priority == ThreadPriorityForTest::kBackground);
    } else {
