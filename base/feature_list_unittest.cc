...
CONSTINIT Feature kFeatureOnByDefault(kFeatureOnByDefaultName, FEATURE_ENABLED_BY_DEFAULT);

constexpr char kFeatureOffByDefaultName[] = "OffByDefault";
CONSTINIT Feature kFeatureOffByDefault(kFeatureOffByDefaultName, FEATURE_DISABLED_BY_DEFAULT);

std::string SortFeatureListString(const std::string& feature_list)
{
    std::vector<base::StringPiece> features = FeatureList::SplitFeatureListString(feature_list);
...
TEST_F(FeatureListTest, InitializeFromCommandLine)
{
    ...
    for (size_t i = 0; i < std::size(test_cases); ++i) {
        ...
        feature_list->InitializeFromCommandLine(test_case.enable_features, test_case.disable_features);
...
{
    auto feature_list = std::make_unique<FeatureList>();
    feature_list->InitializeFromCommandLine(kFeatureOnByDefaultName, kFeatureOffByDefaultName);
    std::vector<FeatureList::FeatureOverrideInfo> overrides;
    overrides.push_back({ std::cref(kFeatureOnByDefault), FeatureList::OverrideState::OVERRIDE_DISABLE_FEATURE });
    overrides.push_back({ std::cref(kFeatureOffByDefault), FeatureList::OverrideState::OVERRIDE_ENABLE_FEATURE });
    feature_list->RegisterExtraFeatureOverrides(std::move(overrides));
...
TEST_F(FeatureListTest, InitializeFromCommandLine_UseDefault)
{
    ...
    auto feature_list = std::make_unique<FeatureList>();
    feature_list->InitializeFromCommandLine("A,*OffByDefault<T1,*OnByDefault<T2,X", "D");
    test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeatureList(std::move(feature_list));
...
{
    EXPECT_DEATH(Feature(StrCat({ BUILDFLAG(BANNED_BASE_FEATURE_PREFIX), "MyFeature" }).c_str(), FEATURE_DISABLED_BY_DEFAULT),
