...
bool IsSwitchWithKey(CommandLine::StringPieceType string, CommandLine::StringPieceType switch_key_without_prefix)
{
    size_t prefix_length = GetSwitchPrefixLength(string);
    if (prefix_length == 0 || prefix_length == string.length())
        return false;
...
// Windows*.
std::wstring QuoteForCommandLineToArgvW(const std::wstring& arg, bool allow_unsafe_insert_sequences)
{
...
CommandLine::StringType CommandLine::GetSwitchValueNative(StringPiece switch_string) const
{
    DCHECK_EQ(ToLowerASCII(switch_string), switch_string);
...
void CommandLine::AppendSwitchPath(StringPiece switch_string, const FilePath& path)
{
    AppendSwitchNative(switch_string, path.value());
}

void CommandLine::AppendSwitchNative(StringPiece switch_string, CommandLine::StringPieceType value)
{
#if BUILDFLAG(IS_WIN)
...
void CommandLine::AppendSwitchASCII(StringPiece switch_string, StringPiece value_string)
{
#if BUILDFLAG(IS_WIN)
...
void CommandLine::CopySwitchesFrom(const CommandLine& source, const char* const switches[], size_t count)
{
...
void CommandLine::AppendSwitchesAndArguments(const CommandLine::StringVector& argv)
{
...
#if BUILDFLAG(IS_WIN)
                switch_value = QuoteForCommandLineToArgvW(switch_value, allow_unsafe_insert_sequences);
#endif
