...
void MaybeWarnVmodule()
{
    if (base::CommandLine::InitializedForCurrentProcess()) {
        base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
        if (command_line->HasSwitch(switches::kV) || command_line->HasSwitch(switches::kVModule)) {
            LOG(WARNING) << "--" << switches::kV << " and --" << switches::kVModule
...
#if BUILDFLAG(IS_FUCHSIA)
inline FuchsiaLogSeverity LogSeverityToFuchsiaLogSeverity(LogSeverity severity)
{
    switch (severity) {
    case LOGGING_INFO:
...
LogMessage::~LogMessage()
{
    ...
    TRACE_LOG_MESSAGE(file_, base::StringPiece(str_newline).substr(message_start_), line_);
    ...
    if (true /*(g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0*/) { // weolar: Open log
#if BUILDFLAG(IS_WIN)
        ...
        GetScopedFxLogger().LogMessage(file_, static_cast<uint32_t>(line_), message.substr(0, message.size() - 1), LogSeverityToFuchsiaLogSeverity(severity_));
    ...
    if (severity_ == LOGGING_FATAL) {
        ...
        if (!GetLogAssertHandlerStack().empty()) {
            ...
            if (log_assert_handler) {
                log_assert_handler.Run(file_, line_, base::StringPiece(str_newline.c_str() + message_start_, stack_start - message_start_),
...
BASE_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    if (len) {
        ...
        return base::CollapseWhitespaceASCII(msgbuf, true) + base::StringPrintf(" (0x%lX)", error_code);
    }
