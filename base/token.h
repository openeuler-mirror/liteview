...
public:
    ...
    constexpr bool operator<(const Token& other) const
    {
        return std::tie(words_[0], words_[1]) < std::tie(other.words_[0], other.words_[1]);
    }
...
BASE_EXPORT absl::optional<Token> ReadTokenFromPickle(PickleIterator* pickle_iterator);
