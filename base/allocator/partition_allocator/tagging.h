...
extern PA_COMPONENT_EXPORT(PARTITION_ALLOC) TagMemoryRangeRandomlyInternalFn* global_tag_memory_range_randomly_fn;
extern PA_COMPONENT_EXPORT(PARTITION_ALLOC) TagMemoryRangeIncrementInternalFn* global_tag_memory_range_increment_fn;
extern PA_COMPONENT_EXPORT(PARTITION_ALLOC) RemaskPtrInternalFn* global_remask_void_ptr_fn;
...
template <typename T> PA_ALWAYS_INLINE T* TagMemoryRangeIncrement(T* ptr, size_t size)
{
#if defined(PA_HAS_MEMORY_TAGGING)
...
// range is set to the same tag.
// TODO(bartekn): Remove the T* variant.
template <typename T> PA_ALWAYS_INLINE T* TagMemoryRangeRandomly(T* ptr, size_t size, uint64_t mask = 0u)
{
#if defined(PA_HAS_MEMORY_TAGGING)
    return reinterpret_cast<T*>(global_tag_memory_range_randomly_fn(ptr, size, mask));
#else
    return ptr;
#endif
}
PA_ALWAYS_INLINE void* TagMemoryRangeRandomly(uintptr_t ptr, size_t size, uint64_t mask = 0u)
{
