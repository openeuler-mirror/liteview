...
#if !defined(MEMORY_TOOL_REPLACES_ALLOCATOR) && defined(PA_THREAD_CACHE_SUPPORTED)
...
ThreadSafePartitionRoot* CreatePartitionRoot()
{
    ThreadSafePartitionRoot* root = new ThreadSafePartitionRoot({
        PartitionOptions::AlignedAlloc::kAllowed,
            ...
            PartitionOptions::Quarantine::kAllowed, PartitionOptions::Cookie::kDisallowed, PartitionOptions::BackupRefPtr::kDisabled,
            PartitionOptions::BackupRefPtrZapping::kDisabled, PartitionOptions::UseConfigurablePool::kNo,
    });
...
class PartitionAllocThreadCacheTest : public ::testing::TestWithParam<PartitionRoot<internal::ThreadSafe>::BucketDistribution> {
public:
...
protected:
    ...
    void TearDown() override
    {
        ...
        ASSERT_EQ(root_->get_total_size_of_allocated_bytes(), GetBucketSizeForThreadCache());
    }
    ...
    {
        size_t tc_bucket_index = root_->SizeToBucketIndex(sizeof(ThreadCache), ThreadSafePartitionRoot::BucketDistribution::kCoarser);
    ...
    {
        return PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(size, GetParam());
    }
...
INSTANTIATE_TEST_SUITE_P(AlternateBucketDistribution, PartitionAllocThreadCacheTest,
...
#if defined(                                                                                                                                                   \
    PA_ENABLE_THREAD_CACHE_STATISTICS) // Required to record hits and                                                                                  \
    // misses.
TEST_P(PartitionAllocThreadCacheTest, LargeAllocationsAreNotCached)
{
    ...
    DeltaCounter alloc_miss_too_large_counter { tcache->stats_.alloc_miss_too_large };
...
TEST_P(PartitionAllocThreadCacheTest, DirectMappedReallocMetrics)
{
    ...
    EXPECT_EQ(expected_allocated_size, root_->get_total_size_of_allocated_bytes());
    EXPECT_EQ(expected_allocated_size, root_->get_max_size_of_allocated_bytes());

    void* ptr = root_->Alloc(10 * internal::kMaxBucketed, "");

    EXPECT_EQ(expected_allocated_size + 10 * internal::kMaxBucketed, root_->get_total_size_of_allocated_bytes());

    void* ptr2 = root_->Realloc(ptr, 9 * internal::kMaxBucketed, "");

    ASSERT_EQ(ptr, ptr2);
    EXPECT_EQ(expected_allocated_size + 9 * internal::kMaxBucketed, root_->get_total_size_of_allocated_bytes());
...
size_t FillThreadCacheAndReturnIndex(ThreadSafePartitionRoot* root, size_t size, BucketDistribution bucket_distribution, size_t count = 1)
{
    uint16_t bucket_index = PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(size, bucket_distribution);
    std::vector<void*> allocated_data;
...
public:
    ThreadDelegateForMultipleThreadCaches(ThreadCache* parent_thread_cache, ThreadSafePartitionRoot* root, BucketDistribution bucket_distribution)
...
TEST_P(PartitionAllocThreadCacheTest, MultipleThreadCaches)
{
    ...
    ThreadDelegateForMultipleThreadCaches delegate(parent_thread_tcache, root_, GetParam());
...
public:
    ThreadDelegateForThreadCacheReclaimedWhenThreadExits(ThreadSafePartitionRoot* root, void*& other_thread_ptr)
        : root_(root)
...
TEST_P(PartitionAllocThreadCacheTest, ThreadCacheReclaimedWhenThreadExits)
{
    ...
    ThreadDelegateForThreadCacheReclaimedWhenThreadExits delegate(root_, other_thread_ptr);
...
public:
    ThreadDelegateForThreadCacheRegistry(ThreadCache* parent_thread_cache, ThreadSafePartitionRoot* root, BucketDistribution bucket_distribution)
        : parent_thread_tcache_(parent_thread_cache)
        , root_(root)
        , bucket_distribution_(bucket_distribution)
    {
...
TEST_P(PartitionAllocThreadCacheTest, ThreadCacheRegistry)
{
    ...
    ThreadDelegateForThreadCacheRegistry delegate(parent_thread_tcache, root_, GetParam());
...
TEST_P(PartitionAllocThreadCacheTest, RecordStats)
{
    ...
    EXPECT_EQ(root_->buckets[bucket_index].slot_size * expected_count, stats.bucket_total_memory);
...
public:
    ThreadDelegateForMultipleThreadCachesAccounting(ThreadSafePartitionRoot* root, int alloc_ount, BucketDistribution bucket_distribution)
        : root_(root)
    ...
    void ThreadMain() override
    {
        ...
        EXPECT_EQ(2 * root_->buckets[bucket_index].slot_size * kFillCountForMediumBucket, stats.bucket_total_memory);
        EXPECT_EQ(2 * sizeof(ThreadCache), stats.metadata_overhead);

        uint64_t this_thread_alloc_count = root_->thread_cache_for_testing()->stats_.alloc_count;
...
TEST_P(PartitionAllocThreadCacheTest, MultipleThreadCachesAccounting)
{
    ...
    ThreadDelegateForMultipleThreadCachesAccounting delegate(root_, alloc_count, GetParam());
...
public:
    ThreadDelegateForPurgeAll(ThreadSafePartitionRoot* root, ThreadCache*& other_thread_tcache, std::atomic<bool>& other_thread_started,
        std::atomic<bool>& purge_called, int bucket_index, BucketDistribution bucket_distribution)
        : root_(root)
...
PA_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    ThreadDelegateForPurgeAll delegate(root_, other_thread_tcache, other_thread_started, purge_called, bucket_index, GetParam());
    internal::base::PlatformThreadHandle thread_handle;
    internal::base::PlatformThreadForTesting::Create(0, &delegate, &thread_handle);
...
TEST_P(PartitionAllocThreadCacheTest, PeriodicPurge)
{
    ...
    ASSERT_LT(tcache->CachedMemory(), ThreadCacheRegistry::kMinCachedMemoryForPurging);
    ...
    FillThreadCacheWithMemory(ThreadCacheRegistry::kMinCachedMemoryForPurging + 1);
    registry.RunPeriodicPurge();
    EXPECT_EQ(NextInterval(), ThreadCacheRegistry::kMaxPurgeInterval);

    FillThreadCacheWithMemory(2 * ThreadCacheRegistry::kMinCachedMemoryForPurging + 1);
    registry.RunPeriodicPurge();
...
void FillThreadCacheWithMemory(ThreadSafePartitionRoot* root, size_t target_cached_memory, BucketDistribution bucket_distribution)
{
    for (int batch : { 1, 2, 4, 8, 16 }) {
        for (size_t allocation_size = 1; allocation_size <= ThreadCache::kLargeSizeThreshold; allocation_size++) {
            FillThreadCacheAndReturnIndex(root, allocation_size, bucket_distribution, batch);
...
public:
    ThreadDelegateForPeriodicPurgeSumsOverAllThreads(
        ThreadSafePartitionRoot* root, std::atomic<int>& allocations_done, std::atomic<bool>& can_finish, BucketDistribution bucket_distribution)
        : root_(root)
    ...
    void ThreadMain() override
    {
        FillThreadCacheWithMemory(root_, 5 * ThreadCacheRegistry::kMinCachedMemoryForPurging, bucket_distribution_);
...
TEST_P(PartitionAllocThreadCacheTest, DISABLED_PeriodicPurgeSumsOverAllThreads)
{
    ...
    FillThreadCacheWithMemory(ThreadCacheRegistry::kMinCachedMemoryForPurging / 2);
    registry.RunPeriodicPurge();
    EXPECT_EQ(NextInterval(), ThreadCacheRegistry::kMaxPurgeInterval);

    std::atomic<int> allocations_done { 0 };
    std::atomic<bool> can_finish { false };
    ThreadDelegateForPeriodicPurgeSumsOverAllThreads delegate(root_, allocations_done, can_finish, GetParam());
...
TEST_P(PartitionAllocThreadCacheTest, MAYBE_DynamicCountPerBucket)
{
    ...
    EXPECT_EQ(kDefaultCountForMediumBucket - 1, tcache->buckets_[bucket_index].count);

    // Free() triggers the purge within limits.
    root_->Free(data);
    EXPECT_LE(tcache->buckets_[bucket_index].count, kDefaultCountForMediumBucket / 2);
    ...
    ThreadCacheRegistry::Instance().SetThreadCacheMultiplier(ThreadCache::kDefaultMultiplier * 2);
    FillThreadCacheAndReturnIndex(kMediumSize, 1000);
    EXPECT_GT(tcache->buckets_[bucket_index].count, kDefaultCountForMediumBucket / 2);
}
...
public:
    ThreadDelegateForDynamicCountPerBucketMultipleThreads(ThreadSafePartitionRoot* root, std::atomic<bool>& other_thread_started,
        std::atomic<bool>& threshold_changed, int bucket_index, BucketDistribution bucket_distribution)
        : root_(root)
        , other_thread_started_(other_thread_started)
...
TEST_P(PartitionAllocThreadCacheTest, MAYBE_DynamicCountPerBucketMultipleThreads)
{
    ...
    ThreadDelegateForDynamicCountPerBucketMultipleThreads delegate(root_, other_thread_started, threshold_changed, bucket_index, GetParam());
...
TEST_P(PartitionAllocThreadCacheTest, ClearFromTail)
{
    auto count_items = [](ThreadCache* tcache, size_t index) {
        ...
        while (head) {
            head = head->GetNextForThreadCache<true>(tcache->buckets_[index].slot_size);
            count++;
        }
        return count;
    };
...
TEST_P(PartitionAllocThreadCacheTest, MAYBE_Bookkeeping)
{
    ...
    root_->PurgeMemory(PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages);
    root_->ResetBookkeepingForTesting();
    ...
    size_t tc_bucket_index = root_->SizeToBucketIndex(sizeof(ThreadCache), ThreadSafePartitionRoot::BucketDistribution::kCoarser);
    auto* tc_bucket = &root_->buckets[tc_bucket_index];
    size_t expected_allocated_size = tc_bucket->slot_size; // For the ThreadCache itself.
    size_t expected_committed_size = kUseLazyCommit ? internal::SystemPageSize() : tc_bucket->get_bytes_per_span();
    ...
    EXPECT_EQ(root_->get_total_size_of_allocated_bytes(), expected_allocated_size);
    tcache->Purge();
    EXPECT_EQ(root_->get_total_size_of_allocated_bytes(), GetBucketSizeForThreadCache());
}
...
TEST(AlternateBucketDistributionTest, SizeToIndex)
{
    ...
    for (size_t i = 1 << 0; i < 1 << 8; i <<= 1) {
        for (size_t offset = 0; offset < 4; offset++) {
            ...
            EXPECT_EQ(BucketIndexLookup::GetIndex(n), BucketIndexLookup::GetIndexForDenserBuckets(n));
        }
    }
    ...
    for (size_t i = 1 << 8; i < internal::kHighThresholdForAlternateDistribution; i <<= 1) {
        ...
        for (size_t offset = 2; offset < 4; offset++) {
            ...
            EXPECT_GT(BucketIndexLookup::GetIndex(n), BucketIndexLookup::GetIndexForDenserBuckets(n));
...
TEST(AlternateBucketDistributionTest, SwitchBeforeAlloc)
{
    ...
    EXPECT_NE(internal::BucketIndexLookup::GetIndex(n), internal::BucketIndexLookup::GetIndexForDenserBuckets(n));
