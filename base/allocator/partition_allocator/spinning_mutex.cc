...
void SpinningMutex::FutexWait()
{
    ...
    int err = syscall(SYS_futex, &state_, /*FUTEX_WAIT*/0 | /*FUTEX_PRIVATE_FLAG*/128, kLockedContended, nullptr, nullptr, 0);
...
{
    int saved_errno = errno;
    long retval = syscall(SYS_futex, &state_, /*FUTEX_WAKE*/1 | /*FUTEX_PRIVATE_FLAG*/128, 1 /* wake up a single waiter */, nullptr, nullptr, 0);
    PA_CHECK(retval != -1);
    errno = saved_errno;
}
...
void SpinningMutex::LockSlow()
{
    ::AcquireSRWLockExclusiveXp(reinterpret_cast<PSRWLOCK>(&lock_));
}
