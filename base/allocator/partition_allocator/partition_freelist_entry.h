...
private:
    explicit PA_ALWAYS_INLINE constexpr EncodedPartitionFreelistEntryPtr(std::nullptr_t)
    ...
    explicit PA_ALWAYS_INLINE constexpr operator bool() const
    {
        return encoded_;
    }
...
public:
    ...
    static PA_ALWAYS_INLINE PartitionFreelistEntry* EmplaceAndInitNull(void* slot_start_tagged)
    {
        // |slot_start_tagged| is MTE-tagged.
        auto* entry = new (slot_start_tagged) PartitionFreelistEntry(nullptr);
        return entry;
    }
    static PA_ALWAYS_INLINE PartitionFreelistEntry* EmplaceAndInitNull(uintptr_t slot_start)
    {
    ...
    static PA_ALWAYS_INLINE PartitionFreelistEntry* EmplaceAndInitForThreadCache(uintptr_t slot_start, PartitionFreelistEntry* next)
    {
    ...
    static PA_ALWAYS_INLINE void EmplaceAndInitForTest(uintptr_t slot_start, void* next, bool make_shadow_match)
    {
        new (SlotStartAddr2Ptr(slot_start)) PartitionFreelistEntry(next, make_shadow_match);
    }
    ...
    template <bool crash_on_corruption> PA_ALWAYS_INLINE PartitionFreelistEntry* GetNextForThreadCache(size_t extra) const;
    ...
    {
        for (auto* entry = this; entry; entry = entry->GetNextForThreadCache<true>(extra)) {
    ...
    PA_ALWAYS_INLINE void SetNext(PartitionFreelistEntry* entry)
    {
        ...
        if (PA_UNLIKELY(entry && (SlotStartPtr2Addr(this) & kSuperPageBaseMask) != (SlotStartPtr2Addr(entry) & kSuperPageBaseMask))) {
...
private:
    template <bool crash_on_corruption> PA_ALWAYS_INLINE PartitionFreelistEntry* GetNextInternal(size_t extra, bool for_thread_cache) const;

    static PA_ALWAYS_INLINE bool IsSane(const PartitionFreelistEntry* here, const PartitionFreelistEntry* next, bool for_thread_cache)
    {
        ...
        bool same_superpage = (here_address & kSuperPageBaseMask) == (next_address & kSuperPageBaseMask);
#if BUILDFLAG(USE_FREESLOT_BITMAP)
        bool marked_as_free_in_bitmap = for_thread_cache ? true : !FreeSlotBitmapSlotIsUsed(reinterpret_cast<uintptr_t>(next));
#else
        ...
        bool not_in_metadata = (next_address & kSuperPageOffsetMask) >= PartitionPageSize();

        if (for_thread_cache)
            return shadow_ptr_ok & not_in_metadata;
        else
            return shadow_ptr_ok & same_superpage & marked_as_free_in_bitmap & not_in_metadata;
    }
...
static_assert(kSmallestBucket >= sizeof(PartitionFreelistEntry), "Need enough space for freelist entries in the smallest slot");
...
namespace {
constexpr size_t kSmallestUsedBucket = base::bits::AlignUp(1 + sizeof(PartitionRefCount), kSmallestBucket);
}
static_assert(kSmallestUsedBucket >= sizeof(PartitionFreelistEntry) + sizeof(PartitionRefCount),
    "Need enough space for freelist entries and the ref-count in the "
    "smallest *used* slot");
#endif // BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT)

template <bool crash_on_corruption> PA_ALWAYS_INLINE PartitionFreelistEntry* PartitionFreelistEntry::GetNextInternal(size_t extra, bool for_thread_cache) const
{
...
template <bool crash_on_corruption> PA_ALWAYS_INLINE PartitionFreelistEntry* PartitionFreelistEntry::GetNextForThreadCache(size_t extra) const
{
    return GetNextInternal<crash_on_corruption>(extra, true);
}

PA_ALWAYS_INLINE PartitionFreelistEntry* PartitionFreelistEntry::GetNext(size_t extra) const
{
