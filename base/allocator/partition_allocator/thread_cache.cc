...
void OnDllProcessDetach()
{
    ...
    g_thread_cache_root.load(std::memory_order_relaxed)->flags.with_thread_cache = false;
}
#endif
...
void ThreadCacheRegistry::SetLargestActiveBucketIndex(uint8_t largest_active_bucket_index)
{
...
void ThreadCacheRegistry::RunPeriodicPurge()
{
    ...
    if (cached_memory_approx > 10 * kMinCachedMemoryForPurging) {
        periodic_purge_next_interval_ = std::min(kDefaultPurgeInterval, periodic_purge_next_interval_ / 2);
    } else if (cached_memory_approx > 2 * kMinCachedMemoryForPurging) {
        periodic_purge_next_interval_ = std::max(kMinPurgeInterval, periodic_purge_next_interval_ / 2);
    } else if (cached_memory_approx < kMinCachedMemoryForPurging) {
        periodic_purge_next_interval_ = std::min(kMaxPurgeInterval, periodic_purge_next_interval_ * 2);
    }
    ...
    // allocation).
    periodic_purge_next_interval_ = internal::base::clamp(periodic_purge_next_interval_, kMinPurgeInterval, kMaxPurgeInterval);
...
void ThreadCache::EnsureThreadSpecificDataInitialized()
{
    ...
    internal::ScopedGuard scoped_locker(ThreadCacheRegistry::Instance().GetLock());
...
void ThreadCache::SetGlobalLimits(PartitionRoot<>* root, float multiplier)
{
    ...
    for (int index = 0; index < kBucketCount; index++) {
        ...
        global_limits_[index] = static_cast<uint8_t>(internal::base::clamp(value, kMinLimit, kMaxLimit));
...
void ThreadCache::SetLargestCachedSize(size_t size)
{
    ...
    largest_active_bucket_index_
        = PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(size, PartitionRoot<internal::ThreadSafe>::BucketDistribution::kCoarser);
...
ThreadCache* ThreadCache::Create(PartitionRoot<internal::ThreadSafe>* root)
{
    ...
    auto* bucket = root->buckets + PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(raw_size, root->GetBucketDistribution());
    uintptr_t buffer = root->RawAlloc(bucket, AllocFlags::kZeroFill, raw_size, internal::PartitionPageSize(), &usable_size, &already_zeroed);
    ThreadCache* tcache = new (internal::SlotStartAddr2Ptr(buffer)) ThreadCache(root);
...
void ThreadCache::FillBucket(size_t bucket_index)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        uintptr_t slot_start = root_->AllocFromBucket(&root_->buckets[bucket_index], AllocFlags::kFastPathOrReturnNull | AllocFlags::kReturnNull,
            root_->buckets[bucket_index].slot_size /* raw_size */, internal::PartitionPageSize(), &usable_size, &is_already_zeroed);
...
template <bool crash_on_corruption> void ThreadCache::ClearBucketHelper(Bucket& bucket, size_t limit)
{
    ...
    if (limit == 0) {
        ...
        FreeAfter<crash_on_corruption>(head->GetNextForThreadCache<crash_on_corruption>(bucket.slot_size), bucket.slot_size);
