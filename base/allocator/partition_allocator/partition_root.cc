...
template <bool thread_safe> internal::Lock& PartitionRoot<thread_safe>::GetEnumeratorLock()
{
...
public:
    using EnumerateCallback = void (*)(ThreadSafePartitionRoot* root, bool in_child);
    enum EnumerateOrder {
        kNormal,
        kReverse,
    };
    ...
    void Enumerate(EnumerateCallback callback, bool in_child, EnumerateOrder order) PA_NO_THREAD_SAFETY_ANALYSIS
    {
        if (order == kNormal) {
...
private:
    ...
    ThreadSafePartitionRoot* Tail(ThreadSafePartitionRoot* roots) PA_NO_THREAD_SAFETY_ANALYSIS
    {
        if (!roots)
            return nullptr;
    ...
    ThreadSafePartitionRoot* partition_roots_ PA_GUARDED_BY(ThreadSafePartitionRoot::GetEnumeratorLock()) = nullptr;
};
...
void LockRoot(PartitionRoot<internal::ThreadSafe>* root, bool) PA_NO_THREAD_SAFETY_ANALYSIS
{
    PA_DCHECK(root);
    root->lock_.Acquire();
}
...
void UnlockOrReinitRoot(PartitionRoot<internal::ThreadSafe>* root, bool in_child) PA_NO_THREAD_SAFETY_ANALYSIS
{
...
PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR PA_ALWAYS_INLINE size_t MinPurgeableSlotSize()
{
    return SystemPageSize() / kMaxPurgeableSlotsPerSystemPage;
}
} // namespace

template <bool thread_safe> static size_t PartitionPurgeSlotSpan(internal::SlotSpanMetadata<thread_safe>* slot_span, bool discard)
{
    ...
    if (slot_span->CanStoreRawSize()) {
        ...
        if (discardable_bytes && discard) {
            uintptr_t slot_span_start = internal::SlotSpanMetadata<thread_safe>::ToSlotSpanStart(slot_span);
    ...
    if (truncated_slots) {
        ...
        if (unprovisioned_bytes && discard) {
            ...
            for (size_t slot_index = 0; slot_index < num_slots; ++slot_index) {
                ...
                auto* entry = PartitionFreelistEntry::EmplaceAndInitNull(slot_span_start + (slot_size * slot_index));
                if (!head) {
                    head = entry;
                    back = entry;
                } else {
            ...
            FreeSlotBitmapReset(slot_span_start + (slot_size * num_slots), end_addr, slot_size);
#endif
...
template <bool thread_safe> static void PartitionPurgeBucket(internal::PartitionBucket<thread_safe>* bucket)
{
    if (bucket->active_slot_spans_head != internal::SlotSpanMetadata<thread_safe>::get_sentinel_slot_span()) {
        for (internal::SlotSpanMetadata<thread_safe>* slot_span = bucket->active_slot_spans_head; slot_span; slot_span = slot_span->next_slot_span) {
            PA_DCHECK(slot_span != internal::SlotSpanMetadata<thread_safe>::get_sentinel_slot_span());
            PartitionPurgeSlotSpan(slot_span, true);
        }
    }
}

template <bool thread_safe> static void PartitionDumpSlotSpanStats(PartitionBucketMemoryStats* stats_out, internal::SlotSpanMetadata<thread_safe>* slot_span)
{
...
template <bool thread_safe> static void PartitionDumpBucketStats(PartitionBucketMemoryStats* stats_out, const internal::PartitionBucket<thread_safe>* bucket)
{
    ...
    // slot spans.
    if (bucket->active_slot_spans_head == internal::SlotSpanMetadata<thread_safe>::get_sentinel_slot_span() && !bucket->empty_slot_spans_head
        && !bucket->decommitted_slot_spans_head && !bucket->num_full_slot_spans)
        return;
    ...
    for (internal::SlotSpanMetadata<thread_safe>* slot_span = bucket->empty_slot_spans_head; slot_span; slot_span = slot_span->next_slot_span) {
        PA_DCHECK(slot_span->is_empty() || slot_span->is_decommitted());
        PartitionDumpSlotSpanStats(stats_out, slot_span);
    }
    for (internal::SlotSpanMetadata<thread_safe>* slot_span = bucket->decommitted_slot_spans_head; slot_span; slot_span = slot_span->next_slot_span) {
...
template <bool thread_safe> [[noreturn]] PA_NOINLINE void PartitionRoot<thread_safe>::OutOfMemory(size_t size)
{
...
template <bool thread_safe> void PartitionRoot<thread_safe>::DecommitEmptySlotSpans()
{
...
template <bool thread_safe> void PartitionRoot<thread_safe>::DestructForTesting()
{
    ...
    while (curr != nullptr) {
        ...
        internal::AddressPoolManager::GetInstance().UnreserveAndDecommit(
            pool_handle, SuperPagesBeginFromExtent(curr), internal::kSuperPageSize * curr->number_of_consecutive_super_pages);
        curr = next;
    }
}

#if defined(PA_ENABLE_MAC11_MALLOC_SIZE_HACK)
template <bool thread_safe> void PartitionRoot<thread_safe>::EnableMac11MallocSizeHackForTesting()
{
    flags.mac11_malloc_size_hack_enabled_ = true;
}
#endif // defined(PA_ENABLE_MAC11_MALLOC_SIZE_HACK)

template <bool thread_safe> void PartitionRoot<thread_safe>::Init(PartitionOptions opts)
{
    {
#if BUILDFLAG(IS_APPLE)
        ...
        flags.allow_aligned_alloc = opts.aligned_alloc == PartitionOptions::AlignedAlloc::kAllowed;
        flags.allow_cookie = opts.cookie == PartitionOptions::Cookie::kAllowed;
#if BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        flags.brp_enabled_ = opts.backup_ref_ptr == PartitionOptions::BackupRefPtr::kEnabled;
        flags.brp_zapping_enabled_ = opts.backup_ref_ptr_zapping == PartitionOptions::BackupRefPtrZapping::kEnabled;
        PA_CHECK(!flags.brp_zapping_enabled_ || flags.brp_enabled_);
#if defined(PA_ENABLE_MAC11_MALLOC_SIZE_HACK) && BUILDFLAG(IS_APPLE)
        flags.mac11_malloc_size_hack_enabled_ = flags.brp_enabled_ && internal::base::mac::IsOS11();
#endif // defined(PA_ENABLE_MAC11_MALLOC_SIZE_HACK) && BUILDFLAG(IS_APPLE)
#else // BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        PA_CHECK(opts.backup_ref_ptr == PartitionOptions::BackupRefPtr::kDisabled);
#endif // BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        flags.use_configurable_pool = (opts.use_configurable_pool == PartitionOptions::UseConfigurablePool::kIfAvailable) && IsConfigurablePoolAvailable();
...
#if BUILDFLAG(STARSCAN)
            (opts.quarantine == PartitionOptions::Quarantine::kDisallowed ? QuarantineMode::kAlwaysDisabled : QuarantineMode::kDisabledByDefault);
#else
        ...
        sentinel_bucket.active_slot_spans_head = SlotSpan::get_sentinel_slot_span_non_const();
        ...
        flags.with_thread_cache = (opts.thread_cache == PartitionOptions::ThreadCache::kEnabled);
...
template <bool thread_safe> PartitionRoot<thread_safe>::~PartitionRoot()
{
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT)
    PA_CHECK(!flags.with_thread_cache) << "Must not destroy a partition with a thread cache";
#endif // BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT)
...
template <bool thread_safe> void PartitionRoot<thread_safe>::EnableThreadCacheIfSupported()
{
#if defined(PA_THREAD_CACHE_SUPPORTED)
    ::partition_alloc::internal::ScopedGuard guard { lock_ };
    PA_CHECK(!flags.with_thread_cache);
...
bool PartitionRoot<thread_safe>::TryReallocInPlaceForDirectMap(internal::SlotSpanMetadata<thread_safe>* slot_span, size_t requested_size)
{
    ...
    size_t available_reservation_size
        = current_reservation_size - extent->padding_for_alignment - PartitionRoot<thread_safe>::GetDirectMapMetadataAndGuardPagesSize();
#if BUILDFLAG(PA_DCHECK_IS_ON)
    ...
    if (new_slot_size == current_slot_size) {
        ...
        RecommitSystemPagesForData(slot_start + current_slot_size, recommit_slot_size_growth, PageAccessibilityDisposition::kRequireUpdate);
    ...
    if (flags.allow_cookie) {
        ...
        internal::PartitionCookieWriteValue(object + slot_span->GetUsableSize(this));
    }
#endif
...
template <bool thread_safe> bool PartitionRoot<thread_safe>::TryReallocInPlaceForNormalBuckets(void* object, SlotSpan* slot_span, size_t new_size)
{
...
#if BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT) && BUILDFLAG(PA_DCHECK_IS_ON)
        if (brp_enabled()) {
            internal::PartitionRefCount* new_ref_count = internal::PartitionRefCountPointer(slot_start);
    ...
    // BUILDFLAG(PA_DCHECK_IS_ON)
        ...
        if (flags.allow_cookie) {
            internal::PartitionCookieWriteValue(static_cast<unsigned char*>(object) + slot_span->GetUsableSize(this));
        }
...
template <bool thread_safe> void* PartitionRoot<thread_safe>::ReallocWithFlags(unsigned int flags, void* ptr, size_t new_size, const char* type_name)
{
    ...
    if (PA_UNLIKELY(!ptr)) {
        return no_hooks ? AllocWithFlagsNoHooks(flags, new_size, internal::PartitionPageSize())
                        : AllocWithFlagsInternal(flags, new_size, internal::PartitionPageSize(), type_name);
    }
    ...
    if (PA_LIKELY(!overridden)) {
        ...
        if (success) {
            if (PA_UNLIKELY(!no_hooks && hooks_enabled)) {
                PartitionAllocHooks::ReallocObserverHookIfEnabled(ptr, ptr, new_size, type_name);
            }
            return ptr;
        }
    ...
    void* ret = no_hooks ? AllocWithFlagsNoHooks(flags, new_size, internal::PartitionPageSize())
                         : AllocWithFlagsInternal(flags, new_size, internal::PartitionPageSize(), type_name);
    if (!ret) {
        if (flags & AllocFlags::kReturnNull)
            return nullptr;
...
template <bool thread_safe> void PartitionRoot<thread_safe>::PurgeMemory(int flags)
{
    {
        ::partition_alloc::internal::ScopedGuard guard { lock_ };
#if BUILDFLAG(STARSCAN)
...
template <bool thread_safe> void PartitionRoot<thread_safe>::ShrinkEmptySlotSpansRing(size_t limit)
{
...
template <bool thread_safe> void PartitionRoot<thread_safe>::DumpStats(const char* partition_name, bool is_light_dump, PartitionStatsDumper* dumper)
{
    ...
    // below.
    {
        ...
        for (DirectMapExtent* extent = direct_map_list; extent && num_direct_mapped_allocations < kMaxReportableDirectMaps;
...
// static
template <bool thread_safe> void PartitionRoot<thread_safe>::DeleteForTesting(PartitionRoot* partition_root)
{
...
template <bool thread_safe> void PartitionRoot<thread_safe>::ResetBookkeepingForTesting()
{
    ::partition_alloc::internal::ScopedGuard guard { lock_ };
...
template <> uintptr_t PartitionRoot<internal::ThreadSafe>::MaybeInitThreadCacheAndAlloc(uint16_t bucket_index, size_t* slot_size)
{
...
template <> void PartitionRoot<internal::ThreadSafe>::EnableSortActiveSlotSpans()
{
    sort_active_slot_spans_ = true;
}

template struct PA_COMPONENT_EXPORT(PARTITION_ALLOC) PartitionRoot<internal::ThreadSafe>;

static_assert(offsetof(PartitionRoot<internal::ThreadSafe>, sentinel_bucket)
        == offsetof(PartitionRoot<internal::ThreadSafe>, buckets) + internal::kNumBuckets * sizeof(PartitionRoot<internal::ThreadSafe>::Bucket),
    "sentinel_bucket must be just after the regular buckets.");

static_assert(offsetof(PartitionRoot<internal::ThreadSafe>, lock_) >= 64, "The lock should not be on the same cacheline as the read-mostly flags");
} // namespace partition_alloc
