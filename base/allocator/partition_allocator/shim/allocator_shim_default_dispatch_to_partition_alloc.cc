...
LeakySingleton<partition_alloc::ThreadSafePartitionRoot, MainPartitionConstructor> g_root PA_CONSTINIT = {};
...
LeakySingleton<partition_alloc::ThreadSafePartitionRoot, AlignedPartitionConstructor> g_aligned_root PA_CONSTINIT = {};
...
void* AllocateAlignedMemory(size_t alignment, size_t size)
{
    ...
    if (alignment <= partition_alloc::internal::kAlignment) {
        ...
        return Allocator()->AllocWithFlagsNoHooks(0, size, partition_alloc::PartitionPageSize());
    }

    return AlignedAllocator()->AlignedAllocWithFlags(partition_alloc::AllocFlags::kNoHooks, alignment, size);
}
...
void* PartitionMalloc(const AllocatorDispatch*, size_t size, void* context)
{
    ...
    return Allocator()->AllocWithFlagsNoHooks(g_alloc_flags, MaybeAdjustSize(size), partition_alloc::PartitionPageSize());
}

void* PartitionMallocUnchecked(const AllocatorDispatch*, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return Allocator()->AllocWithFlagsNoHooks(
        partition_alloc::AllocFlags::kReturnNull | g_alloc_flags, MaybeAdjustSize(size), partition_alloc::PartitionPageSize());
}

void* PartitionCalloc(const AllocatorDispatch*, size_t n, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    const size_t total = partition_alloc::internal::base::CheckMul(n, MaybeAdjustSize(size)).ValueOrDie();
    return Allocator()->AllocWithFlagsNoHooks(partition_alloc::AllocFlags::kZeroFill | g_alloc_flags, total, partition_alloc::PartitionPageSize());
}

void* PartitionMemalign(const AllocatorDispatch*, size_t alignment, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return AllocateAlignedMemory(alignment, size);
}

void* PartitionAlignedAlloc(const AllocatorDispatch* dispatch, size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    return AllocateAlignedMemory(alignment, size);
}
...
// and use it.
void* PartitionAlignedRealloc(const AllocatorDispatch* dispatch, void* address, size_t size, size_t alignment, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
    ...
    if (address) {
        size_t usage = partition_alloc::ThreadSafePartitionRoot::GetUsableSize(address);
...
void* PartitionRealloc(const AllocatorDispatch*, void* address, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
#if BUILDFLAG(IS_APPLE)
    if (PA_UNLIKELY(!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(address)) && address)) {
    ...
    return Allocator()->ReallocWithFlags(partition_alloc::AllocFlags::kNoHooks | g_alloc_flags, address, MaybeAdjustSize(size), "");
}
...
void PartitionFree(const AllocatorDispatch*, void* object, void* context)
{
    ...
    if (PA_UNLIKELY(!partition_alloc::IsManagedByPartitionAlloc(reinterpret_cast<uintptr_t>(object)) && object)) {
        // A memory region allocated by the system allocator is passed in this
...
// can use the size.
void PartitionFreeDefiniteSize(const AllocatorDispatch*, void* address, size_t size, void* context)
{
    partition_alloc::ScopedDisallowAllocations guard {};
...
size_t PartitionGetSizeEstimate(const AllocatorDispatch*, void* address, void* context)
{
    ...
    const size_t size = partition_alloc::ThreadSafePartitionRoot::GetUsableSizeWithMac11MallocSizeHack(address);
#if BUILDFLAG(IS_APPLE)
...
unsigned PartitionBatchMalloc(const AllocatorDispatch*, size_t size, void** results, unsigned num_requested, void* context)
{
...
void PartitionBatchFree(const AllocatorDispatch*, void** to_be_freed, unsigned num_to_be_freed, void* context)
{
    // No real batching: we could only acquire the lock once for instance, keep it
    // simple for now.
...
// static
partition_alloc::ThreadSafePartitionRoot* PartitionAllocMalloc::OriginalAllocator()
{
    return ::OriginalAllocator();
}

// static
partition_alloc::ThreadSafePartitionRoot* PartitionAllocMalloc::AlignedAllocator()
{
...
void EnablePartitionAllocMemoryReclaimer()
{
    ...
    if (original_root)
        ::partition_alloc::MemoryReclaimer::Instance()->RegisterPartition(original_root);
    if (AlignedAllocator() != Allocator()) {
        ::partition_alloc::MemoryReclaimer::Instance()->RegisterPartition(AlignedAllocator());
    }
}

void ConfigurePartitions(EnableBrp enable_brp, EnableBrpZapping enable_brp_zapping, SplitMainPartition split_main_partition,
    UseDedicatedAlignedPartition use_dedicated_aligned_partition, AlternateBucketDistribution use_alternate_bucket_distribution)
{
    ...
    static partition_alloc::internal::base::NoDestructor<partition_alloc::ThreadSafePartitionRoot> new_main_partition(partition_alloc::PartitionOptions {
        !use_dedicated_aligned_partition ? partition_alloc::PartitionOptions::AlignedAlloc::kAllowed
                                         : partition_alloc::PartitionOptions::AlignedAlloc::kDisallowed,
        partition_alloc::PartitionOptions::ThreadCache::kDisabled,
        partition_alloc::PartitionOptions::Quarantine::kAllowed,
        partition_alloc::PartitionOptions::Cookie::kAllowed,
        enable_brp ? partition_alloc::PartitionOptions::BackupRefPtr::kEnabled : partition_alloc::PartitionOptions::BackupRefPtr::kDisabled,
        enable_brp_zapping ? partition_alloc::PartitionOptions::BackupRefPtrZapping::kEnabled
                           : partition_alloc::PartitionOptions::BackupRefPtrZapping::kDisabled,
        partition_alloc::PartitionOptions::UseConfigurablePool::kNo,
    });
    ...
    if (use_dedicated_aligned_partition) {
        ...
        static partition_alloc::internal::base::NoDestructor<partition_alloc::ThreadSafePartitionRoot> new_aligned_partition(partition_alloc::PartitionOptions {
            partition_alloc::PartitionOptions::AlignedAlloc::kAllowed,
            partition_alloc::PartitionOptions::ThreadCache::kDisabled,
            partition_alloc::PartitionOptions::Quarantine::kAllowed,
            partition_alloc::PartitionOptions::Cookie::kAllowed,
...
{
    partition_alloc::internal::base::PlatformThread::SetThreadNameHook(&::base::PlatformThread::SetName);
    ...
    if (OriginalAllocator() != nullptr)
        partition_alloc::internal::PCScan::RegisterScannableRoot(OriginalAllocator());
    if (Allocator() != AlignedAllocator())
        partition_alloc::internal::PCScan::RegisterScannableRoot(AlignedAllocator());
...
const AllocatorDispatch AllocatorDispatch::default_dispatch = {
    ...
    &allocator_shim::internal::PartitionMallocUnchecked, // alloc_unchecked_function
    &allocator_shim::internal::PartitionCalloc, // alloc_zero_initialized_function
    ...
    &allocator_shim::internal::PartitionGetSizeEstimate, // get_size_estimate_function
...
#endif
    &allocator_shim::internal::PartitionAlignedAlloc, // aligned_malloc_function
    &allocator_shim::internal::PartitionAlignedRealloc, // aligned_realloc_function
...
SHIM_ALWAYS_EXPORT struct mallinfo mallinfo(void) __THROW
{
    ...
    if (AlignedAllocator() != Allocator()) {
        AlignedAllocator()->DumpStats("posix_memalign", true, &aligned_allocator_dumper);
    }

    // Dump stats for nonscannable and nonquarantinable allocators.
    auto& nonscannable_allocator = base::internal::NonScannableAllocator::Instance();
    ...
    if (auto* nonscannable_root = nonscannable_allocator.root())
        nonscannable_root->DumpStats("malloc", true, &nonscannable_allocator_dumper);
    auto& nonquarantinable_allocator = base::internal::NonQuarantinableAllocator::Instance();
    ...
    if (auto* nonquarantinable_root = nonquarantinable_allocator.root())
        nonquarantinable_root->DumpStats("malloc", true, &nonquarantinable_allocator_dumper);
    ...
    info.hblks = partition_alloc::internal::base::checked_cast<decltype(info.hblks)>(allocator_dumper.stats().total_mmapped_bytes
        + aligned_allocator_dumper.stats().total_mmapped_bytes + nonscannable_allocator_dumper.stats().total_mmapped_bytes
    ...
    info.hblkhd = partition_alloc::internal::base::checked_cast<decltype(info.hblkhd)>(allocator_dumper.stats().total_resident_bytes
        + aligned_allocator_dumper.stats().total_resident_bytes + nonscannable_allocator_dumper.stats().total_resident_bytes
    ...
    info.uordblks = partition_alloc::internal::base::checked_cast<decltype(info.uordblks)>(allocator_dumper.stats().total_active_bytes
        + aligned_allocator_dumper.stats().total_active_bytes + nonscannable_allocator_dumper.stats().total_active_bytes
