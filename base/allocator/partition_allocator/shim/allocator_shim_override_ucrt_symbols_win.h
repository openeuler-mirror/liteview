...
// SHIM_ALWAYS_EXPORT void* operator new(size_t size, const std::nothrow_t&) noexcept { return ShimCppNewNoThrow(size); }
//
// SHIM_ALWAYS_EXPORT void* operator new[](size_t size, const std::nothrow_t&) noexcept { return ShimCppNewNoThrow(size); }
...
void (*free_unchecked)(void*) = &allocator_shim::UncheckedFree;
void* (*realloc_unchecked)(void*, size_t) = &allocator_shim::UncheckedRealloc;
...
//
// void free(void* ptr) { ShimFree(ptr, nullptr); }
//
// __declspec(restrict) void* realloc(void* ptr, size_t size) { return ShimRealloc(ptr, size, nullptr); }
//
...
//
// __declspec(restrict) void* _aligned_malloc(size_t size, size_t alignment) { return ShimAlignedMalloc(size, alignment, nullptr); }
//
// __declspec(restrict) void* _aligned_realloc(void* address, size_t size, size_t alignment) { return ShimAlignedRealloc(address, size, alignment, nullptr); }
//
...
//
// __declspec(restrict) void* _recalloc(void* block, size_t count, size_t size) { return _recalloc_base(block, count, size); }
//
