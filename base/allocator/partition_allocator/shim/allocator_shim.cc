...
#if BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
    allocator_shim::internal::PartitionAllocSetCallNewHandlerOnMallocFailure(value);
#endif
}
...
{
    const AllocatorDispatch* const chain_head = GetChainHead();
    return chain_head->realloc_function(chain_head, ptr, size, nullptr);
}
...
void InsertAllocatorDispatch(AllocatorDispatch* dispatch)
{
    ...
    for (size_t i = 0; i < kMaxRetries; ++i) {
        ...
        // retry.
        if (g_chain_head.compare_exchange_strong(chain_head, dispatch, std::memory_order_relaxed, std::memory_order_relaxed)) {
            // Success.
            return;
        }
    }
...
ALWAYS_INLINE int ShimPosixMemalign(void** res, size_t alignment, size_t size)
{
    ...
    // in tc_malloc.cc.
    if (((alignment % sizeof(void*)) != 0) || !partition_alloc::internal::base::bits::IsPowerOfTwo(alignment)) {
        return EINVAL;
    }
...
ALWAYS_INLINE void* ShimPvalloc(size_t size)
{
    ...
    if (size == 0) {
        ...
        size = partition_alloc::internal::base::bits::AlignUp(size, GetCachedPageSize());
    }
...
#if !BUILDFLAG(IS_WIN) && !(BUILDFLAG(IS_APPLE) && !BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC))
