...
TEST(PartitionAllocAsMalloc, AlignedRealloc)
{
    for (size_t alloc_size : { 100, 100000, 10000000 }) {
        for (size_t alignment = 1; alignment <= partition_alloc::kMaxSupportedAlignment; alignment <<= 1) {
            ...
            void* data2 = PartitionAlignedRealloc(nullptr, data, alloc_size, alignment, nullptr);
            EXPECT_TRUE(data2);
...
{
    EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(PartitionAllocMalloc::Allocator()) % alignof(partition_alloc::ThreadSafePartitionRoot));
    // This works fine even if nullptr is returned.
    EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(PartitionAllocMalloc::OriginalAllocator()) % alignof(partition_alloc::ThreadSafePartitionRoot));
    EXPECT_EQ(0u, reinterpret_cast<uintptr_t>(PartitionAllocMalloc::AlignedAllocator()) % alignof(partition_alloc::ThreadSafePartitionRoot));
}
...
TEST(PartitionAllocAsMalloc, DisableCrashOnOom)
{
    ...
    void* ptr = PartitionMalloc(nullptr, std::numeric_limits<size_t>::max() - 10 * base::GetPageSize(), nullptr);
    // Should not crash.
    EXPECT_FALSE(ptr);
