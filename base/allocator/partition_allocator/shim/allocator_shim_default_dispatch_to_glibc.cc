...
void* GlibcUncheckedMalloc(const AllocatorDispatch*, size_t size, void* context)
{
    if (PA_UNLIKELY(size >= kMaxAllowedSize))
        return nullptr;
...
void* GlibcCalloc(const AllocatorDispatch*, size_t n, size_t size, void* context)
{
...
void* GlibcRealloc(const AllocatorDispatch*, void* address, size_t size, void* context)
{
    if (PA_UNLIKELY(size >= kMaxAllowedSize))
        base::TerminateBecauseOutOfMemory(size);
...
void* GlibcMemalign(const AllocatorDispatch*, size_t alignment, size_t size, void* context)
{
    if (PA_UNLIKELY(size >= kMaxAllowedSize))
        base::TerminateBecauseOutOfMemory(size);
...
size_t GlibcGetSizeEstimate(const AllocatorDispatch*, void* address, void* context)
{
