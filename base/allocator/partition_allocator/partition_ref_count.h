...
public:
    ...
    static constexpr CountType kNeedsMac11MallocSizeHackBit = 0x0000'0002'0000'0000;
    ...
    PA_ALWAYS_INLINE void AcquireFromUnprotectedPtr()
    {
        ...
        // Check overflow.
        PA_CHECK((old_count & kUnprotectedPtrCountMask) != kUnprotectedPtrCountMask);
#else
        Acquire();
#endif
    }
    ...
    PA_ALWAYS_INLINE bool Release()
    {
        ...
        if (PA_UNLIKELY((old_count & kDanglingRawPtrDetectedBit) == kDanglingRawPtrDetectedBit)) {
    ...
    PA_ALWAYS_INLINE bool ReleaseFromAllocator()
    {
        ...
        if (PA_LIKELY((old_count & ~kNeedsMac11MallocSizeHackBit) == kMemoryHeldByAllocatorBit)) {
    ...
    {
        CheckCookieIfSupported();
        return (count_.load(std::memory_order_acquire) & ~kNeedsMac11MallocSizeHackBit) == kMemoryHeldByAllocatorBit;
    }
    ...
    {
        return (count_.load(std::memory_order_acquire) & ~kNeedsMac11MallocSizeHackBit) == (kPtrInc | kMemoryHeldByAllocatorBit);
    }
...
private:
    ...
    PA_ALWAYS_INLINE bool ReleaseCommon(CountType count)
    {
        ...
        if (PA_LIKELY((count & (kMemoryHeldByAllocatorBit | kPtrCountMask | kUnprotectedPtrCountMask)))) {
...
PA_ALWAYS_INLINE PartitionRefCount::PartitionRefCount(bool use_mac11_hack)
    : count_(kMemoryHeldByAllocatorBit | (use_mac11_hack ? kNeedsMac11MallocSizeHackBit : 0))
#if defined(PA_REF_COUNT_CHECK_COOKIE)
    , brp_cookie_(CalculateCookie())
#endif
{
...
static_assert(kAlignment % alignof(PartitionRefCount) == 0, "kAlignment must be multiples of alignof(PartitionRefCount).");
...
#if defined(PA_REF_COUNT_CHECK_COOKIE) || defined(PA_REF_COUNT_STORE_REQUESTED_SIZE)
...
#if defined(PA_REF_COUNT_CHECK_COOKIE) && defined(PA_REF_COUNT_STORE_REQUESTED_SIZE)
static constexpr size_t kPartitionRefCountSizeShift = 4;
#elif defined(PA_REF_COUNT_CHECK_COOKIE) || defined(PA_REF_COUNT_STORE_REQUESTED_SIZE)
static constexpr size_t kPartitionRefCountSizeShift = 3;
#else
...
static PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR PA_ALWAYS_INLINE size_t GetPartitionRefCountIndexMultiplierShift()
{
...
PA_ALWAYS_INLINE PartitionRefCount* PartitionRefCountPointer(uintptr_t slot_start)
{
    ...
    if (PA_LIKELY(slot_start & SystemPageOffsetMask())) {
        ...
        PartitionRefCount* bitmap_base = reinterpret_cast<PartitionRefCount*>((slot_start & kSuperPageBaseMask) + SystemPageSize() * 2);
        size_t index = ((slot_start & kSuperPageOffsetMask) >> SystemPageShift()) << GetPartitionRefCountIndexMultiplierShift();
#if BUILDFLAG(PA_DCHECK_IS_ON) || BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)
...
static_assert(sizeof(PartitionRefCount) <= kInSlotRefCountBufferSize, "PartitionRefCount should fit into the in-slot buffer.");
