...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionDirectUnmap(SlotSpanMetadata<thread_safe>* slot_span)
{
    ...
    uintptr_t reservation_start = SlotSpanMetadata<thread_safe>::ToSlotSpanStart(slot_span);
...
template <bool thread_safe> PA_ALWAYS_INLINE void SlotSpanMetadata<thread_safe>::RegisterEmpty()
{
    PA_DCHECK(is_empty());
    ...
    SlotSpanMetadata<thread_safe>* slot_span_to_decommit = root->global_empty_slot_span_ring[current_index];
...
// static
template <bool thread_safe> const SlotSpanMetadata<thread_safe> SlotSpanMetadata<thread_safe>::sentinel_slot_span_;

// static
template <bool thread_safe> const SlotSpanMetadata<thread_safe>* SlotSpanMetadata<thread_safe>::get_sentinel_slot_span()
{
    return &sentinel_slot_span_;
}

// static
template <bool thread_safe> SlotSpanMetadata<thread_safe>* SlotSpanMetadata<thread_safe>::get_sentinel_slot_span_non_const()
{
    return const_cast<SlotSpanMetadata<thread_safe>*>(&sentinel_slot_span_);
}

template <bool thread_safe>
SlotSpanMetadata<thread_safe>::SlotSpanMetadata(PartitionBucket<thread_safe>* bucket)
    : bucket(bucket)
    , can_store_raw_size_(bucket->CanStoreRawSize())
{
}

template <bool thread_safe> void SlotSpanMetadata<thread_safe>::FreeSlowPath(size_t number_of_freed)
{
...
template <bool thread_safe> void SlotSpanMetadata<thread_safe>::Decommit(PartitionRoot<thread_safe>* root)
{
    root->lock_.AssertAcquired();
    PA_DCHECK(is_empty());
    PA_DCHECK(!bucket->is_direct_mapped());
...
template <bool thread_safe> void SlotSpanMetadata<thread_safe>::DecommitIfPossible(PartitionRoot<thread_safe>* root)
{
    root->lock_.AssertAcquired();
...
template <bool thread_safe> void SlotSpanMetadata<thread_safe>::SortFreelist()
{
    ...
    for (PartitionFreelistEntry* head = freelist_head; head; head = head->GetNext(slot_size)) {
        ++num_free_slots;
...
void UnmapNow(uintptr_t reservation_start, size_t reservation_size, pool_handle pool)
{
    ...
    {
        PA_DCHECK(pool == kRegularPoolHandle || (IsConfigurablePoolAvailable() && pool == kConfigurablePoolHandle));
