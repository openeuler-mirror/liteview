...
#if defined(PA_HAS_64_BITS_POINTERS) && (defined(__ARM_NEON) || defined(__ARM_NEON__)) && defined(__ARM_FP)
...
#if defined(PA_HAS_64_BITS_POINTERS) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_ANDROID))
...
#if defined(PA_HAS_LINUX_KERNEL) || BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
...
// realistic profiles.
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && !defined(OFFICIAL_BUILD)
...
#if !BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT) && defined(ARCH_CPU_LITTLE_ENDIAN)
...
#if 0 //weolar: !(BUILDFLAG(IS_WIN) && defined(COMPONENT_BUILD)) && !BUILDFLAG(IS_APPLE)
...
#if BUILDFLAG(PA_DCHECK_IS_ON) && BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && defined(PA_THREAD_LOCAL_TLS) && !BUILDFLAG(IS_ANDROID)
#define PA_HAS_ALLOCATION_GUARD
#endif

#if defined(ARCH_CPU_ARM64) && defined(__clang__) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_ANDROID))
static_assert(sizeof(void*) == 8);
//#define PA_HAS_MEMORY_TAGGING // weolar: clang do not impl
#endif
...
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && defined(PA_HAS_ATFORK_HANDLER)
...
#if !(BUILDFLAG(ENABLE_DANGLING_RAW_PTR_CHECKS) && BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT))                                                                  \
    && (BUILDFLAG(PA_DCHECK_IS_ON) || BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS))
...
#if defined(PA_REF_COUNT_STORE_REQUESTED_SIZE) && defined(PA_REF_COUNT_CHECK_COOKIE)
...
// hardware MTE.
#if BUILDFLAG(ENABLE_MTE_CHECKED_PTR_SUPPORT) && defined(PA_HAS_64_BITS_POINTERS) && !defined(PA_HAS_MEMORY_TAGGING)
...
#if BUILDFLAG(ENABLE_SHADOW_METADATA_FOR_64_BITS_POINTERS) && defined(PA_HAS_64_BITS_POINTERS)
