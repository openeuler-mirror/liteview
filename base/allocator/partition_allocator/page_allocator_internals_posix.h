...
bool UseMapJit()
{
    ...
    base::ScopedCFTypeRef<SecTaskRef> task(SecTaskCreateFromSelf(kCFAllocatorDefault));
    if (!task) {
        return true;
    }
    ...
    base::ScopedCFTypeRef<CFTypeRef> jit_entitlement(SecTaskCopyValueForEntitlement(task.get(), CFSTR("com.apple.security.cs.allow-jit"), nullptr));
    if (!jit_entitlement)
        return false;

    return base::mac::CFCast<CFBooleanRef>(jit_entitlement.get()) == kCFBooleanTrue;
}
#endif // BUILDFLAG(IS_MAC)
    ...
    uintptr_t hint, size_t length, PageAccessibilityConfiguration accessibility, PageTag page_tag, int file_descriptor_for_shared_alloc)
{
    ...
    if (ret) {
        ...
        prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, ret, length, PageTagToName(page_tag));
    }
#endif
...
{
    return 0 == PA_HANDLE_EINTR(mprotect(reinterpret_cast<void*>(address), length, GetAccessFlags(accessibility)));
}
...
void SetSystemPagesAccessInternal(uintptr_t address, size_t length, PageAccessibilityConfiguration accessibility)
{
    ...
    const int ret = PA_HANDLE_EINTR(mprotect(reinterpret_cast<void*>(address), length, access_flags));
...
void DecommitAndZeroSystemPagesInternal(uintptr_t address, size_t length)
{
    ...
    void* ptr = reinterpret_cast<void*>(address);
    void* ret = mmap(ptr, length, PROT_NONE, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    PA_CHECK(ptr == ret);
}
