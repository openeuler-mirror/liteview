...
#if defined(OFFICIAL_BUILD) && defined(NDEBUG) && !BUILDFLAG(PA_DCHECK_IS_CONFIGURABLE)
...
#define PA_BASE_CHECK(condition) PA_UNLIKELY(!(condition)) ? PA_IMMEDIATE_CRASH() : PA_EAT_CHECK_STREAM_PARAMS()
...
#define PA_BASE_PCHECK(condition)                                                                                                                              \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::CheckError::PCheck(__FILE__, __LINE__).stream(), PA_UNLIKELY(!(condition)))

#else

#define PA_BASE_CHECK(condition)                                                                                                                               \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::CheckError::Check(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))

#define CHECK_WILL_STREAM() true

#define PA_BASE_PCHECK(condition)                                                                                                                              \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::CheckError::PCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))
...
#define PA_BASE_DCHECK(condition)                                                                                                                              \
    PA_LAZY_CHECK_STREAM(::partition_alloc::internal::logging::CheckError::DCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))

#define PA_BASE_DPCHECK(condition)                                                                                                                             \
    PA_LAZY_CHECK_STREAM(                                                                                                                                      \
        ::partition_alloc::internal::logging::CheckError::DPCheck(__FILE__, __LINE__, #condition).stream(), !PA_ANALYZER_ASSUME_TRUE(condition))
...
#define PA_RAW_CHECK(condition)                                                                                                                                \
    do {                                                                                                                                                       \
        if (!(condition))                                                                                                                                      \
            ::partition_alloc::internal::logging::RawCheck("Check failed: " #condition "\n");                                                                  \
    } while (0)
