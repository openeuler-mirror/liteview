...
#if defined(BASE_ALLOCATOR_PARTITION_ALLOCATOR_PARTITION_ALLOC_BASE_CHECK_H_) || defined(BASE_CHECK_H_)                                                        \
...
void WriteToStderr(const char* data, size_t length)
{
    ...
    while (bytes_written < length) {
        rv = PA_HANDLE_EINTR(write(STDERR_FILENO, data + bytes_written, length - bytes_written));
        if (rv < 0) {
...
std::string SystemErrorCodeToString(SystemErrorCode error_code)
{
    ...
    return base::TruncatingStringPrintf("Error (0x%lX) while retrieving error. (0x%lX)", GetLastError(), error_code);
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
    return base::safe_strerror(error_code) + base::TruncatingStringPrintf(" (%d)", error_code);
#endif // BUILDFLAG(IS_WIN)
}
