...
#if defined(COMPILER_GCC) || defined(__clang__)
#define PA_PRINTF_FORMAT(format_param, dots_param) __attribute__((format(printf, format_param, dots_param)))
#else
...
#define PA_MSAN_CHECK_MEM_IS_INITIALIZED(p, size) __msan_check_mem_is_initialized(p, size)
...
#if defined(__arm__)
#if !defined(__ARMEB__) && !defined(__ARM_EABI__) && !defined(__EABI__) && !defined(__VFP_FP__) && !defined(_WIN32_WCE) && !defined(ANDROID)
...
#else
#define PA_NO_STACK_PROTECTOR __attribute__((__optimize__("-fno-stack-protector")))
#endif
#else
