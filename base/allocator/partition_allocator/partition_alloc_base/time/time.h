...
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) Time : public time_internal::TimeBase<Time> {
public:
...
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) TimeTicks : public time_internal::TimeBase<TimeTicks> {
public:
...
// thread is running.
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) ThreadTicks : public time_internal::TimeBase<ThreadTicks> {
public:
    ...
    {
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FUCHSIA)
        return true;
