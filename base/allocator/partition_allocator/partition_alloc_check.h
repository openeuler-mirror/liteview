...
#define PA_CHECK(condition) PA_UNLIKELY(!(condition)) ? PA_IMMEDIATE_CRASH() : PA_EAT_CHECK_STREAM_PARAMS(condition)
#else
...
#define PA_CHECK(condition)                                                                                                                                    \
    ...
    ? ::partition_alloc::internal::logging::RawCheck(__FILE__ "(" PA_STRINGIFY(__LINE__) ") Check failed: " #condition) : PA_EAT_CHECK_STREAM_PARAMS(condition)
...
#if BUILDFLAG(IS_ANDROID) && defined(ARCH_CPU_X86_FAMILY) && defined(ARCH_CPU_32_BITS)
