...
#if !BUILDFLAG(IS_ANDROID) && defined(GTEST_HAS_DEATH_TEST) && defined(PA_HAS_FREELIST_SHADOW_ENTRY)
...
TEST(HardeningTest, PartialCorruption)
{
    ...
    // encoded_next_.
    PartitionFreelistEntry::EmplaceAndInitForTest(root.ObjectToSlotStart(data), to_corrupt, false);
...
TEST(HardeningTest, OffHeapPointerCrashing)
{
    ...
    // consistent.
    PartitionFreelistEntry::EmplaceAndInitForTest(root.ObjectToSlotStart(data), to_corrupt, true);
...
TEST(HardeningTest, SuccessfulCorruption)
{
    ...
    uintptr_t* zero_vector = reinterpret_cast<uintptr_t*>(root.AllocWithFlags(AllocFlags::kZeroFill, 100 * sizeof(uintptr_t), ""));
