...
template <typename T> class RacefulWorklist {
    struct Node {
        ...
        Node(const Node& other)
            : value(other.value)
            , is_being_visited(other.is_being_visited.load(std::memory_order_relaxed))
...
public:
    ...
    public:
            ...
            , offset_(0)
        {
            if (worklist.data_.size() > 0)
                offset_ = static_cast<size_t>(internal::base::RandGenerator(worklist.data_.size()));
        }

        RandomizedView(const RandomizedView&) = delete;
        const RandomizedView& operator=(const RandomizedView&) = delete;

        template <typename Function> void Visit(Function f);
    ...
    void Push(const T& t)
    {
        data_.push_back(Node(t));
    }

    template <typename It> void Push(It begin, It end)
    {
        std::transform(begin, end, std::back_inserter(data_), [](const T& t) { return Node(t); });
    }

    template <typename Function> void VisitNonConcurrently(Function) const;
...
template <typename T> template <typename Function> void RacefulWorklist<T>::VisitNonConcurrently(Function f) const
{
    for (const auto& t : data_)
        f(t.value);
}

template <typename T> template <typename Function> void RacefulWorklist<T>::RandomizedView::Visit(Function f)
{
    auto& data = worklist_.data_;
    std::vector<typename Underlying::iterator, MetadataAllocator<typename Underlying::iterator>> to_revisit;
