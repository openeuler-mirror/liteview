...
public:
    ...
    template <typename U> MetadataAllocator(const MetadataAllocator<U>&)
    {
    } // NOLINT

    template <typename U> MetadataAllocator& operator=(const MetadataAllocator<U>&)
    {
        return *this;
    }

    template <typename U> bool operator==(const MetadataAllocator<U>&)
    {
        return true;
    }

    template <typename U> bool operator!=(const MetadataAllocator<U>& o)
    {
        return !operator==(o);
    }
    ...
    {
        return static_cast<value_type*>(PCScanMetadataAllocator().AllocWithFlagsNoHooks(0, size * sizeof(value_type), PartitionPageSize()));
    }
...
struct AllocatedOnPCScanMetadataPartition {
    ...
    {
        return PCScanMetadataAllocator().AllocWithFlagsNoHooks(0, size, PartitionPageSize());
    }
...
template <typename T, typename... Args> T* MakePCScanMetadata(Args&&... args)
{
    auto* memory = static_cast<T*>(PCScanMetadataAllocator().AllocWithFlagsNoHooks(0, sizeof(T), PartitionPageSize()));
