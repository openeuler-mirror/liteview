...
// static dispatch.
template <typename Derived> class ScanLoop {
public:
    explicit ScanLoop(SimdSupport simd_type)
        : simd_type_(simd_type)
    {
    }
...
template <typename Derived> void ScanLoop<Derived>::Run(uintptr_t begin, uintptr_t end)
{
...
template <typename Derived> void ScanLoop<Derived>::RunUnvectorized(uintptr_t begin, uintptr_t end)
{
...
void ScanLoop<Derived>::RunAVX2(uintptr_t begin, uintptr_t end)
{
    ...
    const __m256i regular_pool_mask = _mm256_set1_epi64x(derived().RegularPoolMask() & kPtrUntagMask);

    static_assert(sizeof(__m256i) == kBytesInVector);
    for (; begin <= (end - kBytesInVector); begin += kBytesInVector) {
        // Keep it MTE-untagged. See DisableMTEScope for details.
        const __m256i maybe_ptrs = _mm256_load_si256(reinterpret_cast<__m256i*>(begin));
...
__attribute__((target("sse4.1")))
#endif
void ScanLoop<Derived>::RunSSE4(uintptr_t begin, uintptr_t end)
{
    ...
    // If the read value is a pointer into the PA region, it's likely
    // MTE-tagged. Piggyback on |regular_pool_mask| to untag, for efficiency.
    const __m128i regular_pool_mask = _mm_set1_epi64x(derived().RegularPoolMask() & kPtrUntagMask);

    static_assert(sizeof(__m128i) == kBytesInVector);
    for (; begin <= (end - kBytesInVector); begin += kBytesInVector) {
        // Keep it MTE-untagged. See DisableMTEScope for details.
        const __m128i maybe_ptrs = _mm_loadu_si128(reinterpret_cast<__m128i*>(begin));
...
#if defined(PA_STARSCAN_NEON_SUPPORTED)
template <typename Derived> void ScanLoop<Derived>::RunNEON(uintptr_t begin, uintptr_t end)
{
    static constexpr size_t kAlignmentRequirement = 16;
    static constexpr size_t kWordsInVector = 2;
    static constexpr size_t kBytesInVector = kWordsInVector * sizeof(uintptr_t);
    ...
    // If the read value is a pointer into the PA region, it's likely
    // MTE-tagged. Piggyback on |regular_pool_mask| to untag, for efficiency.
    const uint64x2_t regular_pool_mask = vdupq_n_u64(derived().RegularPoolMask() & kPtrUntagMask);
