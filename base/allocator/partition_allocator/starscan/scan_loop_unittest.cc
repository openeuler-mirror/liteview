...
public:
    explicit TestScanLoop(SimdSupport ss)
        : ScanLoop(ss)
    {
    }
...
private:
    ...
    static uintptr_t RegularPoolBase()
    {
        return kBasePtr;
    }
    static uintptr_t RegularPoolMask()
    {
        return kRegularPoolMask;
    }

    void CheckPointer(uintptr_t maybe_ptr)
    {
        ++visited_;
    }
...
template <size_t Alignment, typename... Args> void TestOnRangeWithAlignment(TestScanLoop& sl, size_t expected_visited, Args... args)
{
    alignas(Alignment) uintptr_t range[] = { args... };
    std::sort(std::begin(range), std::end(range));
    do {
        sl.Run(reinterpret_cast<uintptr_t>(std::begin(range)), reinterpret_cast<uintptr_t>(std::end(range)));
...
TEST(PartitionAllocScanLoopTest, VectorizedAVX2)
{
        ...
        return;
    {
        ...
        TestOnRangeWithAlignment<32>(sl, 0u, kInvalidPtr, kInvalidPtr, kInvalidPtr, kInvalidPtr, kInvalidPtr);
    }
    {
        TestScanLoop sl(SimdSupport::kAVX2);
        TestOnRangeWithAlignment<32>(sl, 1u, kValidPtr, kInvalidPtr, kInvalidPtr, kInvalidPtr, kInvalidPtr);
    }
    {
        TestScanLoop sl(SimdSupport::kAVX2);
        TestOnRangeWithAlignment<32>(sl, 2u, kValidPtr, kValidPtr, kInvalidPtr, kInvalidPtr, kInvalidPtr);
    }
    {
        TestScanLoop sl(SimdSupport::kAVX2);
        TestOnRangeWithAlignment<32>(sl, 3u, kValidPtr, kValidPtr, kValidPtr, kInvalidPtr, kInvalidPtr);
    }
    {
        TestScanLoop sl(SimdSupport::kAVX2);
        TestOnRangeWithAlignment<32>(sl, 4u, kValidPtr, kValidPtr, kValidPtr, kValidPtr, kInvalidPtr);
    }
    {
        // Check that the residual pointer is also visited.
        TestScanLoop sl(SimdSupport::kAVX2);
        TestOnRangeWithAlignment<32>(sl, 5u, kValidPtr, kValidPtr, kValidPtr, kValidPtr, kValidPtr);
    }
}
