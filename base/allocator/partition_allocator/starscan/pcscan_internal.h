...
public:
    ...
    using RootsMap = std::unordered_map<Root*, SuperPages, std::hash<Root*>, std::equal_to<>, MetadataAllocator<std::pair<Root* const, SuperPages>>>;
    ...
    RootsMap& scannable_roots()
    {
        return scannable_roots_;
    }
    const RootsMap& scannable_roots() const
    {
        return scannable_roots_;
    }
    ...
    {
        return nonscannable_roots_;
    }
    const RootsMap& nonscannable_roots() const
    {
        return nonscannable_roots_;
    }
    ...
    SimdSupport simd_support() const
    {
        return simd_support_;
    }
    ...
    void EnableImmediateFreeing()
    {
        immediate_freeing_enabled_ = true;
    }
    bool IsImmediateFreeingEnabled() const
    {
        return immediate_freeing_enabled_;
    }
...
private:
    ...
    using StackTops = std::unordered_map<internal::base::PlatformThreadId, void*, std::hash<internal::base::PlatformThreadId>, std::equal_to<>,
        MetadataAllocator<std::pair<const internal::base::PlatformThreadId, void*>>>;
