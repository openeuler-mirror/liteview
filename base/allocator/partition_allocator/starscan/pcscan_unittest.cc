...
public:
    PartitionAllocPCScanTestBase()
    {
        ...
        PCScan::ReinitForTesting({ PCScan::InitConfig::WantedWriteProtectionMode::kDisabled, PCScan::InitConfig::SafepointMode::kEnabled });
        allocator_.init({
    ...
    {
        allocator_.root()->PurgeMemory(PurgeFlags::kDecommitEmptySlotSpans | PurgeFlags::kDiscardUnusedSystemPages);
    ...
    void SchedulePCScan()
    {
        PCScan::Instance().PerformScan(PCScan::InvocationMode::kScheduleOnlyForTesting);
    }
    ...
    void FinishPCScanAsScanner()
    {
        PCScan::FinishScanForTesting();
    }
    ...
    const ThreadSafePartitionRoot& root() const
    {
        return *allocator_.root();
    }
...
public:
    ...
    {
        root().SetQuarantineAlwaysForTesting(true);
    }
...
FullSlotSpanAllocation GetFullSlotSpan(ThreadSafePartitionRoot& root, size_t object_size)
{
    PA_CHECK(0u == root.get_total_size_of_committed_pages());

    const size_t raw_size = root.AdjustSizeForExtrasAdd(object_size);
    const size_t bucket_index = root.SizeToBucketIndex(raw_size, root.GetBucketDistribution());
    ...
    return { bucket.active_slot_spans_head, root.SlotStartToObject(first), root.SlotStartToObject(last) };
}
...
bool IsInFreeList(uintptr_t slot_start)
{
    ...
    for (auto* entry = slot_span->get_freelist_head(); entry; entry = entry->GetNext(slot_span->bucket->slot_size)) {
...
template <size_t Size, size_t Alignment = 0> struct List final : ListBase {
    char buffer[Size];
    ...
    static List* Create(ThreadSafePartitionRoot& root, ListBase* next = nullptr)
    {
        List* list;
        if (Alignment) {
            list = static_cast<List*>(root.AlignedAllocWithFlags(0, Alignment, sizeof(List)));
        } else {
...
TEST_F(PartitionAllocPCScanTest, FirstObjectInQuarantine)
{
    ...
    FullSlotSpanAllocation full_slot_span = GetFullSlotSpan(root(), kAllocationSize);
...
void TestDanglingReference(PartitionAllocPCScanTest& test, SourceList* source, ValueList* value, ThreadSafePartitionRoot& value_root)
{
    {
...
void TestDanglingReferenceNotVisited(PartitionAllocPCScanTest& test, void* value, ThreadSafePartitionRoot& value_root)
{
...
TEST_F(PartitionAllocPCScanTest, DanglingReferenceDifferentBucketsAligned)
{
    ...
    // allocated, within the same super page, with a gap in between.
    {
        ...
        IterateSlotSpans<ThreadSafe>(super_page, true, [&](SlotSpan* slot_span) -> bool {
            if (i == 0) {
                first_slot_span_end = SlotSpan::ToSlotSpanStart(slot_span) + slot_span->bucket->get_pages_per_slot_span() * PartitionPageSize();
            } else {
...
TEST_F(PartitionAllocPCScanTest, DanglingReferenceSameSlotSpanButDifferentPages)
{
    using SourceList = List<8>;
    using ValueList = SourceList;

    static const size_t kObjectSizeForSlotSpanConsistingOfMultiplePartitionPages = static_cast<size_t>(PartitionPageSize() * 0.75);

    FullSlotSpanAllocation full_slot_span
        = GetFullSlotSpan(root(), root().AdjustSizeForExtrasSubtract(kObjectSizeForSlotSpanConsistingOfMultiplePartitionPages));
    ...
    ASSERT_EQ(SlotSpan::FromObject(full_slot_span.first), SlotSpan::FromObject(full_slot_span.last));
    ...
    ASSERT_NE(first_slot_start & PartitionPageBaseMask(), last_slot_start & PartitionPageBaseMask());
...
TEST_F(PartitionAllocPCScanTest, DanglingReferenceFromFullPage)
{
    ...
    FullSlotSpanAllocation full_slot_span = GetFullSlotSpan(root(), sizeof(SourceList));
    ...
    void* value_buffer = root().AllocWithFlagsNoHooks(0, sizeof(ValueList), PartitionPageSize());

    // Assert that the first and the last objects are in different slot spans but
    // in the same bucket.
    SlotSpan* source_slot_span = ThreadSafePartitionRoot::SlotSpan::FromObject(source_buffer);
    SlotSpan* value_slot_span = ThreadSafePartitionRoot::SlotSpan::FromObject(value_buffer);
...
template <size_t Size> struct ListWithInnerReference {
    char buffer1[Size];
    // Volatile to prevent the compiler from doing dead store elimination.
    ...
    {
        auto* list = static_cast<ListWithInnerReference*>(root.Alloc(sizeof(ListWithInnerReference), nullptr));
        return list;
    }

    static void Destroy(ThreadSafePartitionRoot& root, ListWithInnerReference* list)
    {
        root.Free(list);
    }
};
...
namespace {
template <typename SourceList, typename ValueList>
void TestDanglingReferenceWithSafepoint(PartitionAllocPCScanTest& test, SourceList* source, ValueList* value, ThreadSafePartitionRoot& value_root)
{
    {
        // Free |value| and leave the dangling reference in |source|.
        ValueList::Destroy(value_root, value);
...
TEST_F(PartitionAllocPCScanTest, DontScanUnusedRawSize)
{
    ...
    ASSERT_TRUE(big_size + sizeof(void*) <= root().AllocationCapacityFromSlotStart(slot_start));
    ASSERT_TRUE(slot_span->CanStoreRawSize());
...
TEST_F(PartitionAllocPCScanTest, PointersToGuardPages)
{
    ...
    auto* const pointers = static_cast<Pointers*>(root().AllocWithFlagsNoHooks(0, sizeof(Pointers), PartitionPageSize()));

    // Converting to slot start strips MTE tag.
    const uintptr_t super_page = root().ObjectToSlotStart(pointers) & kSuperPageBaseMask;
    ...
    pointers->metadata_page = PartitionSuperPageToMetadataArea<ThreadSafe>(super_page);
    pointers->guard_page1 = static_cast<char*>(pointers->metadata_page) + SystemPageSize();
    pointers->scan_bitmap = SuperPageStateBitmap(super_page);
    pointers->guard_page2 = reinterpret_cast<void*>(super_page + kSuperPageSize - PartitionPageSize());
...
TEST_F(PartitionAllocPCScanTest, TwoDanglingPointersToSameObject)
{
    ...
    const auto& quarantine = PCScan::scheduler().scheduling_backend().GetQuarantineData();
...
{
    static const size_t kObjectSizeForSlotSpanConsistingOfMultiplePartitionPages = static_cast<size_t>(PartitionPageSize() * 1.25);
    ...
    ASSERT_LE(bucket->get_bytes_per_span(), bucket->get_pages_per_slot_span() * PartitionPageSize());
    ...
    *reinterpret_cast<uint8_t**>(full_slot_span.first)
        = reinterpret_cast<uint8_t*>(full_slot_span.last) + kObjectSizeForSlotSpanConsistingOfMultiplePartitionPages + kOffsetPastEnd;
...
TEST_F(PartitionAllocPCScanTest, DanglingPointerOutsideUsablePart)
{
    ...
    // It should fall within the same slot, so no need for MTE-retagging.
    static constexpr size_t kOffsetPastEnd = 7;
    source->next = reinterpret_cast<ListBase*>(reinterpret_cast<uint8_t*>(value + 1) + kOffsetPastEnd);
