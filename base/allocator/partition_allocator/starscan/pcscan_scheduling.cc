...
void PCScanScheduler::SetNewSchedulingBackend(PCScanSchedulingBackend& backend)
{
    backend_ = &backend;
}
...
void LimitBackend::UpdateScheduleAfterScan(size_t survived_bytes, base::TimeDelta, size_t heap_size)
{
    ...
    data.size_limit.store(
        std::max(QuarantineData::kQuarantineSizeMinLimit, static_cast<size_t>(kQuarantineSizeFraction * heap_size)), std::memory_order_relaxed);
}
...
MUAwareTaskBasedBackend::MUAwareTaskBasedBackend(PCScanScheduler& scheduler, ScheduleDelayedScanFunc schedule_delayed_scan)
...
bool MUAwareTaskBasedBackend::LimitReached()
{
    ...
    base::TimeDelta reschedule_delay;
    {
        ...
        if (hard_limit_) {
            ...
            if (PA_UNLIKELY(data.current_size.load(std::memory_order_relaxed) > data.size_limit.load(std::memory_order_relaxed))) {
                return true;
            }
            ...
            PA_PCSCAN_VLOG(3) << "Rescheduling scan with delay: " << reschedule_delay.InMillisecondsF() << " ms";
...
void MUAwareTaskBasedBackend::UpdateScheduleAfterScan(size_t survived_bytes, base::TimeDelta time_spent_in_scan, size_t heap_size)
{
    scheduler_.AccountFreed(survived_bytes);
    ...
    auto& data = GetQuarantineData();
    data.size_limit.store(
        std::max(QuarantineData::kQuarantineSizeMinLimit, static_cast<size_t>(kSoftLimitQuarantineSizePercent * heap_size)), std::memory_order_relaxed);
    hard_limit_ = std::max(QuarantineData::kQuarantineSizeMinLimit, static_cast<size_t>(kHardLimitQuarantineSizePercent * heap_size));
    ...
    const auto time_required_on_mutator = time_spent_in_scan * kTargetMutatorUtilizationPercent / (1.0 - kTargetMutatorUtilizationPercent);
