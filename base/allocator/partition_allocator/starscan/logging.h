...
struct LoggerWithAllowedAllocations : ScopedAllowAllocations, logging::LogMessage {
    using logging::LogMessage::LogMessage;
};

#define PA_PCSCAN_VLOG_STREAM(verbose_level) ::partition_alloc::internal::LoggerWithAllowedAllocations(__FILE__, __LINE__, -(verbose_level)).stream()
...
#define PA_PCSCAN_VLOG(verbose_level) PA_LAZY_STREAM(PA_PCSCAN_VLOG_STREAM(verbose_level), PA_VLOG_IS_ON(verbose_level))
