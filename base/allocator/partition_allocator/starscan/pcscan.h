...
[[noreturn]] PA_COMPONENT_EXPORT(PARTITION_ALLOC) PA_NOINLINE PA_NOT_TAIL_CALLED void DoubleFreeAttempt();
...
public:
    ...
    PA_ALWAYS_INLINE static void MoveToQuarantine(void* object, size_t usable_size, uintptr_t slot_start, size_t slot_size);
...
PA_ALWAYS_INLINE void PCScan::MoveToQuarantine(void* object, size_t usable_size, uintptr_t slot_start, size_t slot_size)
{
    PCScan& instance = Instance();
    ...
    [[maybe_unused]] const bool succeeded = state_bitmap->Quarantine(slot_start, instance.epoch());
