...
public:
    PageWithBitmap()
        : base_(AllocPages(kSuperPageSize, kSuperPageAlignment, PageAccessibilityConfiguration::kReadWrite, PageTag::kPartitionAlloc))
    ...
    ~PageWithBitmap()
    {
        FreePages(base_, kSuperPageSize);
    }

    TestBitmap& bitmap() const
    {
        return *bitmap_;
    }
...
protected:
    ...
    {
        return page.bitmap().MarkQuarantinedAsReachable(ObjectAddress(object_position), epoch);
    }
    ...
    bool IsQuarantineEmpty() const
    {
        return !CountQuarantined();
    }

    uintptr_t ObjectAddress(size_t pos) const
    {
        return reinterpret_cast<uintptr_t>(page.base()) + sizeof(TestBitmap) + pos * kAlignment;
    }
    ...
    static constexpr uintptr_t MiddleIndex()
    {
        return LastIndex() / 2;
    }
...
TEST_F(PartitionAllocStateBitmapTest, AdjacentQuarantinedObjectsAtBegin)
{
    ...
    // Now mark only the first object.
    {
        ...
        size_t count = 0;
        this->bitmap().IterateUnmarkedQuarantined(kTestEpoch, [&count, this](uintptr_t current) {
            if (count == 0)
                EXPECT_EQ(ObjectAddress(1), current);
            count++;
        });
