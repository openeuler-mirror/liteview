...
public:
    ...
    class ViewBase {
    public:
        template <typename Function> void VisitConcurrently(Function);

        template <typename Function> void VisitNonConcurrently(Function);

    protected:
        explicit ViewBase(SuperPagesWorklist& worklist)
            : worklist_(worklist)
        {
        }
...
template <typename Function> void StarScanSnapshot::ViewBase::VisitConcurrently(Function f)
{
    SuperPagesWorklist::RandomizedView view(worklist_);
    view.Visit(std::move(f));
}

template <typename Function> void StarScanSnapshot::ViewBase::VisitNonConcurrently(Function f)
{
