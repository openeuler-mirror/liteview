...
struct DisableMTEScope final {
    DisableMTEScope()
    {
        ::partition_alloc::ChangeMemoryTaggingModeForCurrentThread(::partition_alloc::TagViolationReportingMode::kDisabled);
    }
    ~DisableMTEScope()
    {
...
private:
    ::partition_alloc::TagViolationReportingMode parent_tagging_mode = ::partition_alloc::internal::GetMemoryTaggingModeForCurrentThread();
};
...
public:
    ...
    PA_ALWAYS_INLINE static QuarantineCardTable& GetFrom(uintptr_t address)
    {
        ...
        return *reinterpret_cast<QuarantineCardTable*>(address & PartitionAddressSpace::RegularPoolBaseMask());
    }
...
private:
    ...
    {
        return (address & ~PartitionAddressSpace::RegularPoolBaseMask()) / kCardSize;
    }
...
template <typename T> using MetadataVector = std::vector<T, MetadataAllocator<T>>;
template <typename T> using MetadataSet = std::set<T, std::less<>, MetadataAllocator<T>>;
template <typename K, typename V> using MetadataHashMap = std::unordered_map<K, V, std::hash<K>, std::equal_to<>, MetadataAllocator<std::pair<const K, V>>>;
...
PA_SCAN_INLINE GetSlotStartResult GetSlotStartInSuperPage(uintptr_t maybe_inner_address)
{
    ...
    const uintptr_t partition_page_index = (maybe_inner_address & kSuperPageOffsetMask) >> PartitionPageShift();
    auto* page = PartitionSuperPageToMetadataArea<ThreadSafe>(super_page) + partition_page_index;
    ...
    const uintptr_t slot_span_start = SlotSpanMetadata<ThreadSafe>::ToSlotSpanStart(slot_span);
    const ptrdiff_t ptr_offset = maybe_inner_address - slot_span_start;
    PA_SCAN_DCHECK(0 <= ptr_offset && ptr_offset < static_cast<ptrdiff_t>(slot_span->bucket->get_pages_per_slot_span() * PartitionPageSize()));
    ...
    PA_SCAN_DCHECK(slot_start <= maybe_inner_address && maybe_inner_address < slot_start + slot_size);

    GetSlotStartResult ret;
    ret.slot_start = slot_start;
...
{
#if PA_STARSCAN_USE_CARD_TABLE
    RecommitSystemPages(PartitionAddressSpace::RegularPoolBase(), sizeof(QuarantineCardTable), PageAccessibilityConfiguration::kReadWrite,
        PageAccessibilityDisposition::kRequireUpdate);
#endif
}

template <class Function> void IterateNonEmptySlotSpans(uintptr_t super_page, size_t nonempty_slot_spans, Function function)
{
    ...
    IterateSlotSpans<ThreadSafe>(super_page, true /*with_quarantine*/,
...
class SuperPageSnapshot final {
    ...
    static constexpr size_t kMinPartitionPageSize = __builtin_constant_p(PartitionPageSize()) ? PartitionPageSize() : 1 << 14;
    static constexpr size_t kStateBitmapMinReservedSize = __builtin_constant_p(ReservedStateBitmapSize())
        ? ReservedStateBitmapSize()
        : partition_alloc::internal::base::bits::AlignUp(sizeof(AllocationStateMap), kMinPartitionPageSize);
#else
    static constexpr size_t kMinPartitionPageSize = PartitionPageSize();
    static constexpr size_t kStateBitmapMinReservedSize = ReservedStateBitmapSize();
#endif
    ...
    static constexpr size_t kPayloadMaxSize = kSuperPageSize - kStateBitmapMinReservedSize - kGuardPagesSize;
    static_assert(kPayloadMaxSize % kMinPartitionPageSize == 0, "kPayloadMaxSize must be multiple of kMinPartitionPageSize");

    static constexpr size_t kMaxSlotSpansInSuperPage = kPayloadMaxSize / kMinPartitionPageSize;
...
public:
    ...
    const ScanAreas& scan_areas() const
    {
        return scan_areas_;
    }
...
static_assert(sizeof(SuperPageSnapshot) <= 2048, "SuperPageSnapshot must stay relatively small to be allocated on stack");
...
SuperPageSnapshot::SuperPageSnapshot(uintptr_t super_page)
{
    ...
    const size_t nonempty_slot_spans = extent_entry->number_of_nonempty_slot_spans;
    ...
    IterateNonEmptySlotSpans(super_page, nonempty_slot_spans, [this, &current](SlotSpan* slot_span) {
        const uintptr_t payload_begin = SlotSpan::ToSlotSpanStart(slot_span);
        // For single-slot slot-spans, scan only utilized slot part.
        const size_t provisioned_size = PA_UNLIKELY(slot_span->CanStoreRawSize()) ? slot_span->GetRawSize() : slot_span->GetProvisionedSize();
        ...
        const size_t offset_in_words = (payload_begin & kSuperPageOffsetMask) / sizeof(uintptr_t);
        const size_t size_in_words = (payload_end - payload_begin) / sizeof(uintptr_t);
        const size_t slot_size_in_words = slot_span->bucket->slot_size / sizeof(uintptr_t);

#if PA_SCAN_DCHECK_IS_ON()
        PA_DCHECK(offset_in_words <= std::numeric_limits<decltype(area.offset_within_page_in_words)>::max());
        PA_DCHECK(size_in_words <= std::numeric_limits<decltype(area.size_in_words)>::max());
        PA_DCHECK(slot_size_in_words <= std::numeric_limits<decltype(area.slot_size_in_words)>::max());
#endif
...
class PCScanTask final : public base::RefCountedThreadSafe<PCScanTask>, public AllocatedOnPCScanMetadataPartition {
public:
    ...
    PCScanScheduler& scheduler() const
    {
        return pcscan_.scheduler();
    }
...
private:
    ...
    template <Context context> class SyncScope final {
    public:
        explicit SyncScope(PCScanTask& task)
            : task_(task)
        {
            task_.number_of_scanning_threads_.fetch_add(1, std::memory_order_relaxed);
            if (context == Context::kScanner) {
                task_.pcscan_.state_.store(PCScan::State::kScanning, std::memory_order_relaxed);
        ...
        ~SyncScope()
        {
            ...
            if (context == Context::kScanner) {
                ...
                // descheduled mutator.
                WaitForOtherThreads();
                task_.pcscan_.state_.store(PCScan::State::kSweepingAndFinishing, std::memory_order_relaxed);
            }
        }
    ...
    private:
        ...
        {
            {
                ...
                task_.number_of_scanning_threads_.fetch_sub(1, std::memory_order_relaxed);
                {
        ...
        void WaitForOtherThreads()
        {
            ...
            task_.condvar_.wait(lock, [this] { return !task_.number_of_scanning_threads_.load(std::memory_order_relaxed); });
        }
    ...
    PA_SCAN_INLINE AllocationStateMap* TryFindScannerBitmapForPointer(uintptr_t maybe_ptr) const;
    ...
    void ScanNormalArea(PCScanInternal& pcscan, PCScanScanLoop& scan_loop, uintptr_t begin, uintptr_t end);
    void ScanLargeArea(PCScanInternal& pcscan, PCScanScanLoop& scan_loop, uintptr_t begin, uintptr_t end, size_t slot_size);
...
PA_SCAN_INLINE AllocationStateMap* PCScanTask::TryFindScannerBitmapForPointer(uintptr_t maybe_ptr) const
{
    ...
    if (PA_LIKELY(!QuarantineCardTable::GetFrom(maybe_ptr).IsQuarantined(maybe_ptr)))
        return nullptr;
#else
    ...
    const uintptr_t offset = maybe_ptr & ~PartitionAddressSpace::RegularPoolBaseMask();
    if (PA_LIKELY(*ReservationOffsetPointer(kRegularPoolHandle, offset) != kOffsetTagNormalBuckets))
        return nullptr;
#endif
...
PA_SCAN_INLINE size_t PCScanTask::TryMarkSlotInNormalBuckets(uintptr_t maybe_ptr) const
{
    ...
    const GetSlotStartResult slot_start_result = GetSlotStartInSuperPage(maybe_ptr);
    ...
    PA_SCAN_DCHECK((maybe_ptr & kSuperPageBaseMask) == (slot_start & kSuperPageBaseMask));
    ...
    // operation for it.
    if (PA_LIKELY(state_map->MarkQuarantinedAsReachable(slot_start, pcscan_epoch_)))
...
void PCScanTask::UnprotectPartitions()
{
    ...
    unprotect_view.VisitConcurrently([&pcscan](uintptr_t super_page) {
        ...
        for (const auto& scan_area : super_page_snapshot.scan_areas()) {
            const uintptr_t begin = super_page | (scan_area.offset_within_page_in_words * sizeof(uintptr_t));
...
private:
    ...
    PA_SCAN_INLINE void CheckPointer(uintptr_t maybe_ptr_maybe_tagged)
    {
        ...
        quarantine_size_ += task_.TryMarkSlotInNormalBuckets(UntagAddr(maybe_ptr_maybe_tagged));
    }
...
public:
    explicit StackVisitor(const PCScanTask& task)
        : task_(task)
    {
    }
    ...
    void VisitStack(uintptr_t* stack_ptr, uintptr_t* stack_top) override
    {
        ...
        uintptr_t begin = reinterpret_cast<uintptr_t>(stack_ptr) & ~(kMinimalAlignment - 1);
        uintptr_t end = (reinterpret_cast<uintptr_t>(stack_top) + kMinimalAlignment - 1) & ~(kMinimalAlignment - 1);
        PA_CHECK(begin < end);
...
PCScanTask::PCScanTask(PCScan& pcscan, size_t quarantine_last_size)
    ...
    , immediatelly_free_slots_(PCScanInternal::Instance().IsImmediateFreeingEnabled())
    , pcscan_(pcscan)
{
}
...
void PCScanTask::ScanNormalArea(PCScanInternal& pcscan, PCScanScanLoop& scan_loop, uintptr_t begin, uintptr_t end)
{
...
void PCScanTask::ScanLargeArea(PCScanInternal& pcscan, PCScanScanLoop& scan_loop, uintptr_t begin, uintptr_t end, size_t slot_size)
{
    ...
    for (uintptr_t current_slot = begin; current_slot < end; current_slot += slot_size) {
...
void PCScanTask::ScanPartitions()
{
    ...
    static constexpr size_t kLargeScanAreaThresholdInWords = 1024 / sizeof(uintptr_t);

    PCScanScanLoop scan_loop(*this);
    auto& pcscan = PCScanInternal::Instance();

    StarScanSnapshot::ScanningView snapshot_view(*snapshot_);
    snapshot_view.VisitConcurrently([this, &pcscan, &scan_loop](uintptr_t super_page) {
        SuperPageSnapshot super_page_snapshot(super_page);

        for (const auto& scan_area : super_page_snapshot.scan_areas()) {
            const uintptr_t begin = super_page | (scan_area.offset_within_page_in_words * sizeof(uintptr_t));
            PA_SCAN_DCHECK(begin == super_page + (scan_area.offset_within_page_in_words * sizeof(uintptr_t)));
            const uintptr_t end = begin + scan_area.size_in_words * sizeof(uintptr_t);

            if (PA_UNLIKELY(scan_area.slot_size_in_words >= kLargeScanAreaThresholdInWords)) {
                ScanLargeArea(pcscan, scan_loop, begin, end, scan_area.slot_size_in_words * sizeof(uintptr_t));
            } else {
...
void UnmarkInCardTable(uintptr_t slot_start, SlotSpanMetadata<ThreadSafe>* slot_span)
{
#if PA_STARSCAN_USE_CARD_TABLE
    ...
    // next PCScan cycle.
    QuarantineCardTable::GetFrom(slot_start).Unquarantine(slot_start, slot_span->GetUtilizedSlotSize());
#endif
}

[[maybe_unused]] size_t FreeAndUnmarkInCardTable(PartitionRoot<ThreadSafe>* root, SlotSpanMetadata<ThreadSafe>* slot_span, uintptr_t slot_start)
{
    void* object = root->SlotStartToObject(slot_start);
...
[[maybe_unused]] void SweepSuperPage(ThreadSafePartitionRoot* root, uintptr_t super_page, size_t epoch, SweepStat& stat)
{
    auto* bitmap = StateBitmapFromAddr(super_page);
    ThreadSafePartitionRoot::FromFirstSuperPage(super_page);
    bitmap->IterateUnmarkedQuarantined(epoch, [root, &stat](uintptr_t slot_start) {
...
[[maybe_unused]] void SweepSuperPageAndDiscardMarkedQuarantine(ThreadSafePartitionRoot* root, uintptr_t super_page, size_t epoch, SweepStat& stat)
{
    auto* bitmap = StateBitmapFromAddr(super_page);
    bitmap->IterateQuarantined(epoch, [root, &stat](uintptr_t slot_start, bool is_marked) {
        auto* slot_span = SlotSpanMetadata<ThreadSafe>::FromSlotStart(slot_start);
        ...
        if (slot_size >= SystemPageSize()) {
            const uintptr_t discard_end = base::bits::AlignDown(slot_start + slot_size, SystemPageSize());
            const uintptr_t discard_begin = base::bits::AlignUp(slot_start, SystemPageSize());
...
[[maybe_unused]] void SweepSuperPageWithBatchedFree(ThreadSafePartitionRoot* root, uintptr_t super_page, size_t epoch, SweepStat& stat)
{
    using SlotSpan = SlotSpanMetadata<ThreadSafe>;
    ...
    const auto bitmap_iterator = [&](uintptr_t slot_start) {
        ...
        if (current_slot_span != previous_slot_span) {
            ...
            if (previous_slot_span && freelist_entries) {
                root->RawFreeBatch(freelist_head, freelist_tail, freelist_entries, previous_slot_span);
            }
            freelist_head = entry;
...
void PCScanTask::SweepQuarantine()
{
    ...
    const bool should_discard = (pcscan_epoch_ % kDiscardMarkedQuarantineFrequency == 0) && (pcscan_.clear_type_ == PCScan::ClearType::kEager);

    SweepStat stat;
    StarScanSnapshot::SweepingView sweeping_view(*snapshot_);
    sweeping_view.VisitNonConcurrently([this, &stat, should_discard](uintptr_t super_page) {
        ...
        if (PA_UNLIKELY(should_discard && !root->flags.allow_cookie))
            SweepSuperPageAndDiscardMarkedQuarantine(root, super_page, pcscan_epoch_, stat);
        else
...
void PCScanTask::FinishScanner()
{
    ...
    pcscan_.scheduler_.scheduling_backend().UpdateScheduleAfterScan(
        stats_.survived_quarantine_size(), stats_.GetOverallTime(), PCScanInternal::Instance().CalculateTotalHeapSize());

    PCScanInternal::Instance().ResetCurrentPCScanTask();
    // Change the state and check that concurrent task can't be scheduled twice.
    PA_CHECK(pcscan_.state_.exchange(PCScan::State::kNotRunning, std::memory_order_acq_rel) == PCScan::State::kSweepingAndFinishing);
}

void PCScanTask::RunFromMutator()
{
    ReentrantScannerGuard reentrancy_guard;
    StatsCollector::MutatorScope overall_scope(stats_, StatsCollector::MutatorId::kOverall);
    {
            ...
            return;
        {
            ...
            StatsCollector::MutatorScope clear_scope(stats_, StatsCollector::MutatorId::kClear);
            ClearQuarantinedSlotsAndPrepareCardTable();
        }
        {
            // Scan the thread's stack to find dangling references.
            StatsCollector::MutatorScope scan_scope(stats_, StatsCollector::MutatorId::kScanStack);
            ScanStack();
        }
        {
        ...
        }
        {
            ...
            StatsCollector::MutatorScope scan_scope(stats_, StatsCollector::MutatorId::kScan);
            ScanPartitions();
        }
    }
}
...
void PCScanTask::RunFromScanner()
{
    ReentrantScannerGuard reentrancy_guard;
    {
        StatsCollector::ScannerScope overall_scope(stats_, StatsCollector::ScannerId::kOverall);
        {
            SyncScope<Context::kScanner> sync_scope(*this);
            {
                // Clear all quarantined slots and prepare the card table.
                StatsCollector::ScannerScope clear_scope(stats_, StatsCollector::ScannerId::kClear);
                ClearQuarantinedSlotsAndPrepareCardTable();
            }
            {
                // Scan heap for dangling references.
                StatsCollector::ScannerScope scan_scope(stats_, StatsCollector::ScannerId::kScan);
                ScanPartitions();
            }
            {
                // Unprotect all scanned pages, if needed.
        ...
        }
        {
            ...
            StatsCollector::ScannerScope sweep_scope(stats_, StatsCollector::ScannerId::kSweep);
            SweepQuarantine();
        }
    }
    FinishScanner();
...
private:
    ...
    bool Wait(std::unique_lock<std::mutex>& lock)
    {
        ...
        condvar_.wait_for(lock, std::chrono::microseconds(wanted_delay_.InMicroseconds()));
        ...
        while (true) {
            TaskHandle current_task;
            {
                ...
                while (!posted_task_.get()) {
                    if (Wait(lock)) {
                        wanted_delay_ = scheduler().scheduling_backend().UpdateDelayedSchedule();
                        if (wanted_delay_.is_zero()) {
                            break;
                        }
                    }
    ...
    PCScanScheduler& scheduler() const
    {
        return PCScan::Instance().scheduler();
    }
...
PCScanInternal::PCScanInternal()
    : simd_support_(DetectSimdSupport())
{
}
...
void PCScanInternal::Initialize(PCScan::InitConfig config)
{
    ...
    if (config.write_protection == PCScan::InitConfig::WantedWriteProtectionMode::kEnabled)
...
void PCScanInternal::PerformScan(PCScan::InvocationMode invocation_mode)
{
    ...
    PA_DCHECK(std::all_of(scannable_roots().begin(), scannable_roots().end(), [](const auto& pair) { return pair.first->IsScanEnabled(); }));
    PA_DCHECK(std::all_of(nonscannable_roots().begin(), nonscannable_roots().end(), [](const auto& pair) { return pair.first->IsQuarantineEnabled(); }));
#endif
    ...
    PCScan& frontend = PCScan::Instance();
    {
        ...
        if (!frontend.state_.compare_exchange_strong(expected, PCScan::State::kScheduled, std::memory_order_acq_rel, std::memory_order_relaxed))
            return;
    }

    const size_t last_quarantine_size = frontend.scheduler_.scheduling_backend().ScanStarted();
    ...
    if (PA_UNLIKELY(invocation_mode == PCScan::InvocationMode::kScheduleOnlyForTesting)) {
    ...
    if (PA_LIKELY(invocation_mode == PCScan::InvocationMode::kNonBlocking)) {
        ...
        PA_SCAN_DCHECK(PCScan::InvocationMode::kBlocking == invocation_mode || PCScan::InvocationMode::kForcedBlocking == invocation_mode);
        std::move(*task).RunFromScanner();
    }
}

void PCScanInternal::PerformScanIfNeeded(PCScan::InvocationMode invocation_mode)
{
    if (!scannable_roots().size())
        return;
    PCScan& frontend = PCScan::Instance();
    if (invocation_mode == PCScan::InvocationMode::kForcedBlocking
        || frontend.scheduler_.scheduling_backend().GetQuarantineData().MinimumScanningThresholdReached())
...
namespace {
PCScanInternal::SuperPages GetSuperPagesAndCommitStateBitmaps(PCScan::Root& root)
{
    const size_t state_bitmap_size_to_commit = CommittedStateBitmapSize();
    PCScanInternal::SuperPages super_pages;
    for (auto* super_page_extent = root.first_extent; super_page_extent; super_page_extent = super_page_extent->next) {
        for (uintptr_t super_page = SuperPagesBeginFromExtent(super_page_extent), super_page_end = SuperPagesEndFromExtent(super_page_extent);
            ...
            const volatile char* metadata = reinterpret_cast<char*>(PartitionSuperPageToMetadataArea<ThreadSafe>(super_page));
            *metadata;
            RecommitSystemPages(SuperPageStateBitmapAddr(super_page), state_bitmap_size_to_commit, PageAccessibilityConfiguration::kReadWrite,
                PageAccessibilityDisposition::kRequireUpdate);
...
void PCScanInternal::RegisterScannableRoot(Root* root)
{
    ...
    root_super_pages.insert(root_super_pages.end(), super_pages.begin(), super_pages.end());
}
...
void PCScanInternal::RegisterNewSuperPage(Root* root, uintptr_t super_page_base)
{
    PA_DCHECK(is_initialized());
    PA_DCHECK(root);
    PA_CHECK(root->IsQuarantineAllowed());
    ...
    // Make sure the metadata is committed.
    // TODO(bikineev): Remove once this is known to work.
    const volatile char* metadata = reinterpret_cast<char*>(PartitionSuperPageToMetadataArea<ThreadSafe>(super_page_base));
    *metadata;
    ...
    if (root->IsScanEnabled()) {
        ...
        PA_DCHECK(std::find(super_pages.begin(), super_pages.end(), super_page_base) == super_pages.end());
...
size_t PCScanInternal::CalculateTotalHeapSize() const
{
    PA_DCHECK(is_initialized());
    std::lock_guard<std::mutex> lock(roots_mutex_);
    const auto acc = [](size_t size, const auto& pair) { return size + pair.first->get_total_size_of_committed_pages(); };
    return std::accumulate(scannable_roots_.begin(), scannable_roots_.end(), 0u, acc)
        + std::accumulate(nonscannable_roots_.begin(), nonscannable_roots_.end(), 0u, acc);
}
...
void PCScanInternal::ProtectPages(uintptr_t begin, size_t size)
{
    ...
    write_protector_->ProtectPages(begin, partition_alloc::internal::base::bits::AlignUp(size, SystemPageSize()));
}

void PCScanInternal::UnprotectPages(uintptr_t begin, size_t size)
{
    PA_SCAN_DCHECK(write_protector_.get());
    write_protector_->UnprotectPages(begin, partition_alloc::internal::base::bits::AlignUp(size, SystemPageSize()));
}
...
void PCScanInternal::RegisterStatsReporter(partition_alloc::StatsReporter* reporter)
{
    PA_DCHECK(reporter);
