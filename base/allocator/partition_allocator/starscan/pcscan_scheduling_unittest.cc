...
TEST(PartitionAllocPCScanSchedulerLimitBackendTest, NoScanBelowMinimumScanningThreshold)
{
    PCScanScheduler scheduler;
    LimitBackend limit_backend(scheduler);
    scheduler.SetNewSchedulingBackend(limit_backend);
    constexpr size_t kMinimumScanningThreshold = QuarantineData::kQuarantineSizeMinLimit;
    EXPECT_FALSE(scheduler.AccountFreed(kMinimumScanningThreshold / 2));
    EXPECT_FALSE(scheduler.AccountFreed(kMinimumScanningThreshold - kMinimumScanningThreshold / 2));
    EXPECT_TRUE(scheduler.AccountFreed(1));
}

TEST(PartitionAllocPCScanSchedulerLimitBackendTest, ScanAtQuarantineSizeFraction)
{
    PCScanScheduler scheduler;
    LimitBackend limit_backend(scheduler);
    scheduler.SetNewSchedulingBackend(limit_backend);
    ...
    limit_backend.UpdateScheduleAfterScan(kNoSurvivedBytes, base::TimeDelta(), kHeapSize);
    constexpr size_t kExpectedTriggerSize = static_cast<size_t>(static_cast<double>(kHeapSize) * LimitBackend::kQuarantineSizeFraction);
    EXPECT_FALSE(scheduler.AccountFreed(kExpectedTriggerSize / 2));
    EXPECT_FALSE(scheduler.AccountFreed(kExpectedTriggerSize - kExpectedTriggerSize / 2));
    EXPECT_TRUE(scheduler.AccountFreed(1));
}
...
public:
    ...
    {
        return static_cast<size_t>(static_cast<double>(heap_size) * MUAwareTaskBasedBackend::kHardLimitQuarantineSizePercent) + 1;
    }

    static constexpr size_t SoftLimitSize(size_t heap_size)
    {
        return static_cast<size_t>(static_cast<double>(heap_size) * MUAwareTaskBasedBackend::kSoftLimitQuarantineSizePercent) + 1;
    }
        ...
        : backend_(scheduler_, &IncrementDelayedScanScheduledCount)
    {
        ...
        backend_.UpdateScheduleAfterScan(kNoSurvivedBytes, kZeroTimeForScan, kHeapSize);
    }
    ...
    MUAwareTaskBasedBackend& backend()
    {
        return backend_;
    }
...
private:
    static void IncrementDelayedScanScheduledCount(int64_t delay_in_microseconds)
    {
...
size_t PartitionAllocPCScanMUAwareTaskBasedBackendTest::delayed_scan_scheduled_count_ = 0;
...
public:
    ...
    void AddTicksToNow(base::TimeDelta ticks)
    {
        now_ticks_ += ticks;
    }
...
private:
    ...
    explicit ScopedTimeTicksOverride(base::TimeTicksNowFunction time_ticks_function)
...
TEST_F(PartitionAllocPCScanMUAwareTaskBasedBackendTest, SoftLimitSchedulesScanIfMUNotSatisfied)
{
    // Stop the time.
...
TEST_F(PartitionAllocPCScanMUAwareTaskBasedBackendTest, SoftLimitInvokesScanIfMUSatisfied)
{
    // Stop the time.
    ScopedTimeTicksOverride now_ticks_override;
...
TEST_F(PartitionAllocPCScanMUAwareTaskBasedBackendTest, HardLimitSchedulesScanImmediately)
{
    // Stop the time.
    ScopedTimeTicksOverride now_ticks_override;
    // Simulate PCScan that processed kHeapSize in 1s. Since time is stopped that
