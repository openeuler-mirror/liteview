...
struct QuarantineData final {
    ...
    {
        return current_size.load(std::memory_order_relaxed) > kQuarantineSizeMinLimit;
    }
...
public:
    ...
    virtual void UpdateScheduleAfterScan(size_t survived_bytes, base::TimeDelta time_spent_in_scan, size_t heap_size) = 0;
...
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) LimitBackend final : public PCScanSchedulingBackend {
public:
...
class PA_COMPONENT_EXPORT(PARTITION_ALLOC) MUAwareTaskBasedBackend final : public PCScanSchedulingBackend {
public:
...
public:
    ...
    PCScanSchedulingBackend& scheduling_backend()
    {
        return *backend_;
    }
...
constexpr PCScanSchedulingBackend::PCScanSchedulingBackend(PCScanScheduler& scheduler)
...
{
    const size_t size_before = quarantine_data_.current_size.fetch_add(size, std::memory_order_relaxed);
    return (size_before + size > quarantine_data_.size_limit.load(std::memory_order_relaxed)) && backend_->LimitReached();
}
