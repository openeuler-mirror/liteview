...
#undef DECLARE_ENUM
    ...
    template <Context context> using IdType = std::conditional_t<context == Context::kMutator, MutatorId, ScannerId>;
    ...
    template <Context context> class DeferredTraceEventMap final {
    public:
        using IdType = StatsCollector::IdType<context>;
        using PerThreadEvents = std::array<DeferredTraceEvent, static_cast<size_t>(IdType::kNumIds)>;
        using UnderlyingMap = std::unordered_map<internal::base::PlatformThreadId, PerThreadEvents, std::hash<internal::base::PlatformThreadId>,
            std::equal_to<>, MetadataAllocator<std::pair<const internal::base::PlatformThreadId, PerThreadEvents>>>;

        inline void RegisterBeginEventFromCurrentThread(IdType id);
        inline void RegisterEndEventFromCurrentThread(IdType id);

        const UnderlyingMap& get_underlying_map_unsafe() const
        {
            return events_;
        }
    ...
    template <Context context> class Scope final {
    public:
        ...
        ~Scope()
        {
            stats_.RegisterEndEventFromCurrentThread(type_);
        }
    ...
    void IncreaseSweptSize(size_t size)
    {
        swept_size_ += size;
    }
    size_t swept_size() const
    {
        return swept_size_;
    }
...
private:
    using MetadataString = std::basic_string<char, std::char_traits<char>, MetadataAllocator<char>>;
    ...
    template <Context context> base::TimeDelta GetTimeImpl(const DeferredTraceEventMap<context>& event_map, IdType<context> id) const;

    template <Context context> void ReportTracesAndHistsImpl(partition_alloc::StatsReporter& reporter, const DeferredTraceEventMap<context>& event_map) const;
...
template <Context context> inline void StatsCollector::DeferredTraceEventMap<context>::RegisterBeginEventFromCurrentThread(IdType id)
{
...
template <Context context> inline void StatsCollector::DeferredTraceEventMap<context>::RegisterEndEventFromCurrentThread(IdType id)
{
    std::lock_guard<std::mutex> lock(mutex_);
    const auto tid = base::PlatformThread::CurrentId();
    const auto now = base::TimeTicks::Now();
...
inline StatsCollector::MetadataString StatsCollector::ToUMAString(ScannerId id) const
{
...
inline StatsCollector::MetadataString StatsCollector::ToUMAString(MutatorId id) const
{
    PA_DCHECK(process_name_);
    const MetadataString process_name = process_name_;
    switch (id) {
