...
PA_ALWAYS_INLINE uintptr_t ShadowMetadataStart(uintptr_t super_page, pool_handle pool)
{
    uintptr_t shadow_metadata_start = super_page + SystemPageSize() + ShadowPoolOffset(pool);
...
[[noreturn]] PA_NOINLINE void PartitionOutOfMemoryMappingFailure(PartitionRoot<thread_safe>* root, size_t size) PA_LOCKS_EXCLUDED(root->lock_)
{
    PA_NO_CODE_FOLDING();
...
template <bool thread_safe>
[[noreturn]] PA_NOINLINE void PartitionOutOfMemoryCommitFailure(PartitionRoot<thread_safe>* root, size_t size) PA_LOCKS_EXCLUDED(root->lock_)
{
    PA_NO_CODE_FOLDING();
    root->OutOfMemory(size);
    PA_IMMEDIATE_CRASH(); // Not required, kept as documentation.
...
#if !defined(PA_HAS_64_BITS_POINTERS) && BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
...
template <bool thread_safe>
SlotSpanMetadata<thread_safe>* PartitionDirectMap(PartitionRoot<thread_safe>* root, unsigned int flags, size_t raw_size, size_t slot_span_alignment)
{
    PA_DCHECK((slot_span_alignment >= PartitionPageSize()) && base::bits::IsPowerOfTwo(slot_span_alignment));
...

    {
        ...
        const size_t slot_size = PartitionRoot<thread_safe>::GetDirectMapSlotSize(raw_size);
        ...
        const size_t reservation_size = PartitionRoot<thread_safe>::GetDirectMapReservationSize(raw_size + padding_for_alignment);
#if BUILDFLAG(PA_DCHECK_IS_ON)
        const size_t available_reservation_size
            = reservation_size - padding_for_alignment - PartitionRoot<thread_safe>::GetDirectMapMetadataAndGuardPagesSize();
        ...
        if (pool == kBRPPoolHandle) {
            ScopedSyscallTimer timer { root };
            RecommitSystemPages(reservation_start + SystemPageSize() * 2, SystemPageSize(), PageAccessibilityConfiguration::kReadWrite,
                PageAccessibilityDisposition::kRequireUpdate);
        }
#endif
        ...
        {
            ScopedSyscallTimer timer { root };
            RecommitSystemPages(ShadowMetadataStart(reservation_start, pool), SystemPageSize(), PageAccessibilityConfiguration::kReadWrite,
                PageAccessibilityDisposition::kRequireUpdate);
        }
#endif
        ...
        auto* super_page_extent = PartitionSuperPageToExtent<thread_safe>(reservation_start);
        ...
        PartitionPage<thread_safe>* first_page = reinterpret_cast<PartitionPage<thread_safe>*>(super_page_extent) + 1;
        ...
        if (page != first_page) {
            ...
            PA_DCHECK(page - first_page <= PartitionPage<thread_safe>::kMaxSlotSpanMetadataOffset);
        ...
        auto* metadata = reinterpret_cast<PartitionDirectMapMetadata<thread_safe>*>(page);
        ...
        PA_DCHECK(base::bits::AlignDown(reinterpret_cast<uintptr_t>(metadata), SystemPageSize())
            == base::bits::AlignDown(reinterpret_cast<uintptr_t>(metadata) + sizeof(PartitionDirectMapMetadata<thread_safe>) - 1, SystemPageSize()));
        ...
        new (&page->slot_span_metadata) SlotSpanMetadata<thread_safe>(&metadata->bucket);
        ...
        // soon.
        const bool ok = root->TryRecommitSystemPagesForData(slot_start, slot_size, PageAccessibilityDisposition::kRequireUpdate);
        if (!ok) {
            if (!return_null) {
...
template <bool thread_safe> void PartitionBucket<thread_safe>::Init(uint32_t new_slot_size)
{
    slot_size = new_slot_size;
    slot_size_reciprocal = kReciprocalMask / new_slot_size + 1;
    active_slot_spans_head = SlotSpanMetadata<thread_safe>::get_sentinel_slot_span_non_const();
...
template <bool thread_safe>
PA_ALWAYS_INLINE SlotSpanMetadata<thread_safe>* PartitionBucket<thread_safe>::AllocNewSlotSpan(
    PartitionRoot<thread_safe>* root, unsigned int flags, size_t slot_span_alignment)
{
    ...
    if (PA_UNLIKELY(adjusted_next_partition_page + slot_span_reservation_size > root->next_partition_page_end)) {
    ...
    auto* gap_start_page = PartitionPage<thread_safe>::FromAddr(root->next_partition_page);
    auto* gap_end_page = PartitionPage<thread_safe>::FromAddr(adjusted_next_partition_page);
    ...
    PA_DCHECK(slot_span == SlotSpanMetadata<thread_safe>::FromSlotStart(slot_span_start));
    ...
    if (!kUseLazyCommit) {
        ...
        root->RecommitSystemPagesForData(slot_span_start, slot_span_committed_size, PageAccessibilityDisposition::kRequireUpdate);
    }

    PA_CHECK(get_slots_per_span() <= SlotSpanMetadata<ThreadSafe>::kMaxSlotsPerSlotSpan);
    ...
    uintptr_t next_tag_bitmap_page = base::bits::AlignUp(reinterpret_cast<uintptr_t>(PartitionTagPointer(root->next_partition_page)), SystemPageSize());
    if (root->next_tag_bitmap_page < next_tag_bitmap_page) {
#if BUILDFLAG(PA_DCHECK_IS_ON)
        uintptr_t super_page = reinterpret_cast<uintptr_t>(slot_span) & kSuperPageBaseMask;
...
#endif
        SetSystemPagesAccess(root->next_tag_bitmap_page, next_tag_bitmap_page - root->next_tag_bitmap_page, PageAccessibilityConfiguration::kReadWrite);
...
template <bool thread_safe>
uintptr_t PartitionBucket<thread_safe>::AllocNewSuperPageSpan(PartitionRoot<thread_safe>* root, size_t super_page_count, unsigned int flags)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE uintptr_t PartitionBucket<thread_safe>::AllocNewSuperPage(PartitionRoot<thread_safe>* root, unsigned int flags)
{
...
PA_ALWAYS_INLINE uintptr_t PartitionBucket<thread_safe>::InitializeSuperPage(
    PartitionRoot<thread_safe>* root, uintptr_t super_page, uintptr_t requested_address)
{
    *ReservationOffsetPointer(super_page) = kOffsetTagNormalBuckets;
    ...
    uintptr_t state_bitmap = super_page + PartitionPageSize() + (is_direct_mapped() ? 0 : ReservedTagBitmapSize() + ReservedFreeSlotBitmapSize());
#if BUILDFLAG(STARSCAN)
    PA_DCHECK(SuperPageStateBitmapAddr(super_page) == state_bitmap);
    const size_t state_bitmap_reservation_size = root->IsQuarantineAllowed() ? ReservedStateBitmapSize() : 0;
    const size_t state_bitmap_size_to_commit = root->IsQuarantineAllowed() ? CommittedStateBitmapSize() : 0;
    ...
    PA_DCHECK(payload == SuperPagePayloadBegin(super_page, root->IsQuarantineAllowed()));
    ...
    if (root->ChoosePool() == kBRPPoolHandle) {
        ScopedSyscallTimer timer { root };
        RecommitSystemPages(
            super_page + SystemPageSize() * 2, SystemPageSize(), PageAccessibilityConfiguration::kReadWrite, PageAccessibilityDisposition::kRequireUpdate);
    }
#endif
    ...
    {
        ScopedSyscallTimer timer { root };
        RecommitSystemPages(ShadowMetadataStart(super_page, root->ChoosePool()), SystemPageSize(), PageAccessibilityConfiguration::kReadWrite,
            PageAccessibilityDisposition::kRequireUpdate);
    }
#endif
    ...
    PartitionSuperPageExtentEntry<thread_safe>* current_extent = root->current_extent;
    ...
    root->next_tag_bitmap_page = base::bits::AlignDown(reinterpret_cast<uintptr_t>(PartitionTagPointer(root->next_partition_page)), SystemPageSize());
    PA_DCHECK(root->next_tag_bitmap_page >= super_page + PartitionPageSize()) << "tag bitmap can never intrude on metadata partition page";
#endif // defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
    ...
    if (root->IsQuarantineEnabled()) {
        {
            ScopedSyscallTimer timer { root };
            RecommitSystemPages(
                state_bitmap, state_bitmap_size_to_commit, PageAccessibilityConfiguration::kReadWrite, PageAccessibilityDisposition::kRequireUpdate);
        }
    ...
    if (!is_direct_mapped()) {
        uintptr_t freeslot_bitmap_addr = super_page + PartitionPageSize() + ReservedTagBitmapSize();
        PA_DCHECK(SuperPageFreeSlotBitmapAddr(super_page) == freeslot_bitmap_addr);
        ScopedSyscallTimer timer { root };
        RecommitSystemPages(
            freeslot_bitmap_addr, CommittedFreeSlotBitmapSize(), PageAccessibilityConfiguration::kReadWrite, PageAccessibilityDisposition::kRequireUpdate);
    }
#endif
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionBucket<thread_safe>::InitializeSlotSpan(SlotSpanMetadata<thread_safe>* slot_span)
{
...
template <bool thread_safe>
PA_ALWAYS_INLINE uintptr_t PartitionBucket<thread_safe>::ProvisionMoreSlotsAndAllocOne(
    PartitionRoot<thread_safe>* root, SlotSpanMetadata<thread_safe>* slot_span)
{
    PA_DCHECK(slot_span != SlotSpanMetadata<thread_safe>::get_sentinel_slot_span());
    ...
    if (kUseLazyCommit) {
        ...
        root->RecommitSystemPagesForData(commit_start, commit_end - commit_start, PageAccessibilityDisposition::kRequireUpdate);
    }

    if (PA_LIKELY(slot_size <= kMaxMemoryTaggingSize && root->IsMemoryTaggingEnabled())) {
        // Ensure the MTE-tag of the memory pointed by |return_slot| is unguessable.
        TagMemoryRangeRandomly(return_slot, slot_size);
    }
#if defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
    NormalBucketPartitionTagSetValue(return_slot, slot_size, root->GetNewPartitionTag());
#endif // defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
...
#if defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
        NormalBucketPartitionTagSetValue(next_slot, slot_size, root->GetNewPartitionTag());
#endif // defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
...
template <bool thread_safe> bool PartitionBucket<thread_safe>::SetNewActiveSlotSpan()
{
...
template <bool thread_safe> void PartitionBucket<thread_safe>::MaintainActiveList()
{
    SlotSpanMetadata<thread_safe>* slot_span = active_slot_spans_head;
    if (slot_span == SlotSpanMetadata<thread_safe>::get_sentinel_slot_span())
        return;
    ...
    if (!new_active_slot_spans_head) {
        new_active_slot_spans_head = SlotSpanMetadata<thread_safe>::get_sentinel_slot_span_non_const();
    }
    active_slot_spans_head = new_active_slot_spans_head;
}

template <bool thread_safe> void PartitionBucket<thread_safe>::SortSlotSpanFreelists()
{
...
bool CompareSlotSpans(SlotSpanMetadata<ThreadSafe>* a, SlotSpanMetadata<ThreadSafe>* b)
{
...
template <bool thread_safe> void PartitionBucket<thread_safe>::SortActiveSlotSpans()
{
    ...
    for (int i = index - 1; i >= 0; i--) {
        if (active_spans_array[i] == SlotSpanMetadata<thread_safe>::get_sentinel_slot_span()) {
...
uintptr_t PartitionBucket<thread_safe>::SlowPathAlloc(
    PartitionRoot<thread_safe>* root, unsigned int flags, size_t raw_size, size_t slot_span_alignment, bool* is_already_zeroed)
{
    PA_DCHECK((slot_span_alignment >= PartitionPageSize()) && base::bits::IsPowerOfTwo(slot_span_alignment));
    ...
    *is_already_zeroed = false;
    ...
    if (PA_UNLIKELY(is_direct_mapped())) {
        ...
        PA_DCHECK(active_slot_spans_head == SlotSpanMetadata<thread_safe>::get_sentinel_slot_span());
        ...
        *is_already_zeroed = true;
    ...
    } else if (PA_LIKELY(!allocate_aligned_slot_span && (empty_slot_spans_head != nullptr || decommitted_slot_spans_head != nullptr))) {
        ...
        while (PA_LIKELY((new_slot_span = empty_slot_spans_head) != nullptr)) {
            ...
            if (new_slot_span->get_freelist_head()) {
                ...
                new_slot_span->ToSuperPageExtent()->IncrementNumberOfNonemptySlotSpans();
        ...
        if (PA_UNLIKELY(!new_slot_span) && PA_LIKELY(decommitted_slot_spans_head != nullptr)) {
            ...
            if (!kUseLazyCommit) {
                uintptr_t slot_span_start = SlotSpanMetadata<thread_safe>::ToSlotSpanStart(new_slot_span);
                ...
                // TODO(lizeb): Handle commit failure.
                root->RecommitSystemPagesForData(slot_span_start, new_slot_span->bucket->get_bytes_per_span(), PageAccessibilityDisposition::kAllowKeepForPerf);
            }
    ...
    if (PA_LIKELY(new_slot_span->get_freelist_head() != nullptr)) {
        PartitionFreelistEntry* entry = new_slot_span->PopForAlloc(new_bucket->slot_size);
...
template <bool thread_safe>
uintptr_t PartitionBucket<thread_safe>::AllocNewSuperPageSpanForGwpAsan(PartitionRoot<thread_safe>* root, size_t super_page_count, unsigned int flags)
{
    return AllocNewSuperPageSpan(root, super_page_count, flags);
}

template <bool thread_safe> void PartitionBucket<thread_safe>::InitializeSlotSpanForGwpAsan(SlotSpanMetadata<thread_safe>* slot_span)
{
    InitializeSlotSpan(slot_span);
}
