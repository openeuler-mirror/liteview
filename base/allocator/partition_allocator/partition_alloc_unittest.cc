...
const size_t kTestSizes[] = {
    ...
    partition_alloc::PartitionRoot<partition_alloc::internal::ThreadSafe>::GetDirectMapSlotSize(100),
    1 << 20,
    1 << 21,
};
constexpr size_t kTestSizesCount = std::size(kTestSizes);

void AllocateRandomly(partition_alloc::PartitionRoot<partition_alloc::internal::ThreadSafe>* root, size_t count, unsigned int flags)
{
...
#endif
const size_t kRealAllocSize = partition_alloc::internal::base::bits::AlignUp(kTestAllocSize + kExtraAllocSize, kAlignment);

const char* type_name = nullptr;

void SetDistributionForPartitionRoot(ThreadSafePartitionRoot* root, BucketDistribution distribution)
{
    switch (distribution) {
...
public:
    ScopedPageAllocation(PartitionAllocator<internal::ThreadSafe>& allocator, base::CheckedNumeric<size_t> npages)
        : allocator_(allocator)
        , npages_(npages)
        , ptr_(static_cast<char*>(allocator_.root()->Alloc((npages * SystemPageSize() - kExtraAllocSize).ValueOrDie(), type_name)))
    {
    }
    ...
    void TouchAllPages()
    {
        memset(ptr_, 'A', ((npages_ * SystemPageSize()) - kExtraAllocSize).ValueOrDie());
    }
...
protected:
    ...
    void SetUp() override
    {
        ...
        allocator.init({
#if !BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) || BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT)
...
#endif
                PartitionOptions::ThreadCache::kDisabled, PartitionOptions::Quarantine::kDisallowed, PartitionOptions::Cookie::kAllowed,
#if BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
                PartitionOptions::BackupRefPtr::kEnabled, PartitionOptions::BackupRefPtrZapping::kEnabled,
#else
                PartitionOptions::BackupRefPtr::kDisabled, PartitionOptions::BackupRefPtrZapping::kDisabled,
#endif
    ...
    size_t SizeToIndex(size_t size)
    {
        ...
        return PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(size, distribution_to_use);
    }
    ...
    size_t GetNumPagesPerSlotSpan(size_t size)
    {
        ...
        PartitionRoot<ThreadSafe>::Bucket* bucket = &allocator.root()->buckets[bucket_index];
    ...
    void CycleFreeCache(size_t size)
    {
        for (size_t i = 0; i < kMaxFreeableSpans; ++i) {
            ...
            EXPECT_TRUE(
                bucket->active_slot_spans_head->in_empty_cache() || bucket->active_slot_spans_head == SlotSpanMetadata<ThreadSafe>::get_sentinel_slot_span());
        }
    }
    ...
    void DoReturnNullTest(size_t alloc_size, ReturnNullTestMode mode)
    {
        ...
        for (i = 0; i < num_allocations; ++i) {
            ...
            case kPartitionAllocWithFlags: {
                ptrs[i] = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, alloc_size, type_name);
                break;
            }
            case kPartitionReallocWithFlags: {
                ptrs[i] = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, 1, type_name);
                ptrs[i] = allocator.root()->ReallocWithFlags(AllocFlags::kReturnNull, ptrs[i], alloc_size, type_name);
                break;
            }
            case kPartitionRootTryRealloc: {
                ptrs[i] = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, 1, type_name);
                ptrs[i] = allocator.root()->TryRealloc(ptrs[i], alloc_size, type_name);
            }
            }
...
INSTANTIATE_TEST_SUITE_P(AlternateBucketDistribution, PartitionAllocDeathTest,
    testing::Values(BucketDistribution::kDefault, BucketDistribution::kCoarser, BucketDistribution::kDenser));

#endif

namespace {

void FreeFullSlotSpan(PartitionRoot<internal::ThreadSafe>* root, SlotSpan* slot_span)
{
...
INSTANTIATE_TEST_SUITE_P(
    AlternateBucketDistribution, PartitionAllocTest, testing::Values(BucketDistribution::kDefault, BucketDistribution::kCoarser, BucketDistribution::kDenser));

// Check that the most basic of allocate / free pairs work.
TEST_P(PartitionAllocTest, Basic)
{
    PartitionRoot<ThreadSafe>::Bucket* bucket = &allocator.root()->buckets[test_bucket_index_];
    ...
    EXPECT_EQ(
        PartitionPageSize() + partition_alloc::internal::ReservedTagBitmapSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize() + kPointerOffset,
...
TEST_P(PartitionAllocTest, MultiSlotSpans)
{
    ...
    EXPECT_EQ(0u, slot_span->num_allocated_slots);
    EXPECT_TRUE(bucket->empty_slot_spans_head);
    EXPECT_EQ(SlotSpanMetadata<ThreadSafe>::get_sentinel_slot_span(), bucket->active_slot_spans_head);
...
TEST_P(PartitionAllocTest, PreferSlotSpansWithProvisionedEntries)
{
    ...
    size_t real_size = size + kExtraAllocSize;
    size_t bucket_index = allocator.root()->SizeToBucketIndex(real_size, GetParam());
    PartitionRoot<ThreadSafe>::Bucket* bucket = &allocator.root()->buckets[bucket_index];
...
TEST_P(PartitionAllocTest, FreeSlotSpanListSlotSpanTransitions)
{
    ...
    size_t num_to_fill_free_list_slot_span = PartitionPageSize() / (sizeof(SlotSpan) + kExtraAllocSize);
    ...
    auto slot_spans = std::make_unique<SlotSpan*[]>(num_to_fill_free_list_slot_span);
...
TEST_P(PartitionAllocTest, MultiPageAllocs)
{
    ...
    size_t num_slot_spans_needed = (NumPartitionPagesPerSuperPage() - 2 - partition_alloc::internal::NumPartitionPagesPerTagBitmap()
    ...
    for (i = 0; i < num_slot_spans_needed; ++i) {
        ...
        if (i == num_slot_spans_needed - 1) {
            ...
            // the second page.
            EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedTagBitmapSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize(),
...
TEST_P(PartitionAllocTest, Alloc)
{
    ...
    size_t base_size = partition_alloc::internal::base::bits::AlignUp(kExtraAllocSize, kAlignment) - kExtraAllocSize;
...
TEST_P(PartitionAllocTest, AllocSizes)
{
...

    {
        ...
        auto* slot_span = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
...

    {
        ...
        *(char_ptr + (size - 1)) = 'A';
        ...
        EXPECT_EQ(nullptr, allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, 3u * 1024 * 1024 * 1024, type_name));
    }
}
...
TEST_P(PartitionAllocTest, AllocGetSizeAndStart)
{
    ...
    for (size_t offset = 0; offset < requested_size; ++offset) {
        EXPECT_EQ(PartitionAllocGetSlotStartInBRPPool(address + offset), slot_start);
    }
...
{
    const size_t kMinReasonableTestSize = partition_alloc::internal::base::bits::AlignUp(kExtraAllocSize + 1, kAlignment);
    ...
    for (size_t size : kSizes) {
        ...
        for (void*& ptr : ptrs) {
            ...
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address, requested_size + kSuperPageSize));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address, requested_size + kFarFarAwayDelta));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, kFarFarAwayDelta));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, kSuperPageSize));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, 1));
            EXPECT_TRUE(PartitionAllocIsValidPtrDelta(address + requested_size, 0));
            EXPECT_TRUE(PartitionAllocIsValidPtrDelta(address + requested_size, -(requested_size / 2)));
            EXPECT_TRUE(PartitionAllocIsValidPtrDelta(address + requested_size, -requested_size));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, -requested_size - 1));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, -requested_size - kSuperPageSize));
            EXPECT_FALSE(PartitionAllocIsValidPtrDelta(address + requested_size, -requested_size - kFarFarAwayDelta));
        }
...
TEST_P(PartitionAllocTest, ReallocDirectMapAligned)
{
    ...
    for (size_t alignment : alignments) {
        ...
        ASSERT_GT(size, kMaxBucketed);
        void* ptr = allocator.root()->AllocWithFlagsInternal(0, size, alignment, type_name);
        uintptr_t slot_start = allocator.root()->ObjectToSlotStart(ptr);
        size_t actual_capacity = allocator.root()->AllocationCapacityFromSlotStart(slot_start);
...
TEST_P(PartitionAllocTest, ReallocDirectMapAlignedRelocate)
{
    ...
    ASSERT_GT(size, kMaxBucketed);
    void* ptr = allocator.root()->AllocWithFlagsInternal(0, size, kMaxSupportedAlignment, type_name);
    ...
    ASSERT_GT(size, kMaxBucketed);
    ptr = allocator.root()->AllocWithFlagsInternal(0, size, kMaxSupportedAlignment, type_name);
...
TEST_P(PartitionAllocTest, PartialPageFreelists)
{
    ...
    auto* slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    size_t total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (big_size + kExtraAllocSize);
    ...
    slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (medium_size + kExtraAllocSize);
    size_t first_slot_span_slots = SystemPageSize() / (medium_size + kExtraAllocSize);
    ...
    slot_span = SlotSpan::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (small_size + kExtraAllocSize);
    ...
    size_t very_small_size = (kExtraAllocSize <= 32) ? (32 - kExtraAllocSize) : (64 - kExtraAllocSize);
    ...
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (very_small_actual_size + kExtraAllocSize);
    first_slot_span_slots = SystemPageSize() / (very_small_actual_size + kExtraAllocSize);
    EXPECT_EQ(total_slots - first_slot_span_slots, slot_span->num_unprovisioned_slots);
    ...
    size_t page_and_a_half_size = (SystemPageSize() + (SystemPageSize() / 2)) - kExtraAllocSize;
    ...
    EXPECT_TRUE(!slot_span->get_freelist_head());
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (page_and_a_half_size + kExtraAllocSize);
    EXPECT_EQ(total_slots - 1, slot_span->num_unprovisioned_slots);
    ...
    EXPECT_EQ(1u, slot_span->num_allocated_slots);
    EXPECT_TRUE(slot_span->get_freelist_head());
    total_slots = (slot_span->bucket->num_system_pages_per_slot_span * SystemPageSize()) / (page_size + kExtraAllocSize);
    EXPECT_EQ(total_slots - 2, slot_span->num_unprovisioned_slots);
    allocator.root()->Free(ptr);
}
...
TEST_P(PartitionAllocTest, MappingCollision)
{
    ...
    size_t num_slot_span_needed = (NumPartitionPagesPerSuperPage() - 2 - partition_alloc::internal::NumPartitionPagesPerTagBitmap()
                                      - partition_alloc::internal::NumPartitionPagesPerFreeSlotBitmap())
        / num_pages_per_slot_span;
    size_t num_partition_pages_needed = num_slot_span_needed * num_pages_per_slot_span;

    auto first_super_page_pages = std::make_unique<SlotSpan*[]>(num_partition_pages_needed);
    auto second_super_page_pages = std::make_unique<SlotSpan*[]>(num_partition_pages_needed);

    size_t i;
    for (i = 0; i < num_partition_pages_needed; ++i)
        first_super_page_pages[i] = GetFullSlotSpan(kTestAllocSize);

    uintptr_t slot_spart_start = SlotSpan::ToSlotSpanStart(first_super_page_pages[0]);
    EXPECT_EQ(PartitionPageSize() + partition_alloc::internal::ReservedTagBitmapSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize(),
        slot_spart_start & kSuperPageOffsetMask);
    uintptr_t super_page
        = slot_spart_start - PartitionPageSize() - partition_alloc::internal::ReservedTagBitmapSize() - partition_alloc::internal::ReservedFreeSlotBitmapSize();
    ...
    super_page -= PartitionPageSize() + partition_alloc::internal::ReservedTagBitmapSize() + partition_alloc::internal::ReservedFreeSlotBitmapSize();
    // Map a single system page either side of the mapping for our allocations,
    // with the goal of tripping up alignment of the next mapping.
    map1 = AllocPages(super_page - PageAllocationGranularity(), PageAllocationGranularity(), PageAllocationGranularity(),
        PageAccessibilityConfiguration::kReadWriteTagged, PageTag::kPartitionAlloc);
    EXPECT_TRUE(map1);
    map2 = AllocPages(super_page + kSuperPageSize, PageAllocationGranularity(), PageAllocationGranularity(), PageAccessibilityConfiguration::kReadWriteTagged,
        PageTag::kPartitionAlloc);
    EXPECT_TRUE(map2);
    EXPECT_TRUE(TrySetSystemPagesAccess(map1, PageAllocationGranularity(), PageAccessibilityConfiguration::kInaccessible));
    EXPECT_TRUE(TrySetSystemPagesAccess(map2, PageAllocationGranularity(), PageAccessibilityConfiguration::kInaccessible));
...
TEST_P(PartitionAllocTest, FreeCache)
{
    ...
    size_t bucket_index = SizeToIndex(big_size + kExtraAllocSize);
    PartitionBucket<internal::ThreadSafe>* bucket = &allocator.root()->buckets[bucket_index];
    ...
    EXPECT_EQ(0u, slot_span->num_allocated_slots);
    size_t num_system_pages_per_slot_span = allocator.root()->buckets[test_bucket_index_].num_system_pages_per_slot_span;
    size_t expected_size = kUseLazyCommit ? SystemPageSize() : num_system_pages_per_slot_span * SystemPageSize();
    EXPECT_EQ(expected_size, allocator.root()->get_total_size_of_committed_pages());
...
TEST_P(PartitionAllocTest, LostFreeSlotSpansBug)
{
    ...
    SlotSpanMetadata<internal::ThreadSafe>* slot_span = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr));
    SlotSpanMetadata<internal::ThreadSafe>* slot_span2 = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr2));
    ...
    EXPECT_TRUE(bucket->empty_slot_spans_head);
    EXPECT_TRUE(bucket->empty_slot_spans_head->next_slot_span);
    EXPECT_EQ(SlotSpanMetadata<internal::ThreadSafe>::get_sentinel_slot_span(), bucket->active_slot_spans_head);
...
// cause flake.
#if !BUILDFLAG(IS_WIN) && (!defined(ARCH_CPU_64_BITS) || (BUILDFLAG(IS_POSIX) && !(BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_ANDROID))))
...
TEST_P(PartitionAllocDeathTest, RepeatedAllocReturnNullDirect)
{
    ...
    EXPECT_DEATH(DoReturnNullTest(direct_map_size, kPartitionAllocWithFlags), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, RepeatedReallocReturnNullDirect)
{
    size_t direct_map_size = 32 * 1024 * 1024;
    ASSERT_GT(direct_map_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(direct_map_size, kPartitionReallocWithFlags), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, RepeatedTryReallocReturnNullDirect)
{
    size_t direct_map_size = 32 * 1024 * 1024;
    ASSERT_GT(direct_map_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(direct_map_size, kPartitionRootTryRealloc), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, DISABLED_RepeatedAllocReturnNull)
{
    ...
    EXPECT_DEATH(DoReturnNullTest(single_slot_size, kPartitionAllocWithFlags), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, DISABLED_RepeatedReallocReturnNull)
{
    ...
    ASSERT_GT(single_slot_size, MaxRegularSlotSpanSize());
    ASSERT_LE(single_slot_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(single_slot_size, kPartitionReallocWithFlags), "Passed DoReturnNullTest");
}
...
TEST_P(PartitionAllocDeathTest, DISABLED_RepeatedTryReallocReturnNull)
{
    ...
    ASSERT_GT(single_slot_size, MaxRegularSlotSpanSize());
    ASSERT_LE(single_slot_size, kMaxBucketed);
    EXPECT_DEATH(DoReturnNullTest(single_slot_size, kPartitionRootTryRealloc), "Passed DoReturnNullTest");
}
...
#if !BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) || (defined(PA_HAS_64_BITS_POINTERS) && defined(ARCH_CPU_LITTLE_ENDIAN))
...
{
    const size_t kSizes[] = { kMaxBucketed + kExtraAllocSize + 1, kMaxBucketed + SystemPageSize(), kMaxBucketed + PartitionPageSize(),
...
#if !BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT) && defined(PA_HAS_FREELIST_SHADOW_ENTRY)
...
TEST_P(PartitionAllocTest, DumpMemoryStats)
{
    ...
    // This test checks large-but-not-quite-direct allocations.
    {
...

        {
            ...
            EXPECT_EQ(0u, stats->decommittable_bytes);
            EXPECT_EQ((slot_size - (requested_size + 1)) / SystemPageSize() * SystemPageSize(), stats->discardable_bytes);
            EXPECT_EQ(1u, stats->num_full_slot_spans);
            EXPECT_EQ(0u, stats->num_active_slot_spans);
            EXPECT_EQ(0u, stats->num_empty_slot_spans);
            EXPECT_EQ(0u, stats->num_decommitted_slot_spans);
        ...
        void* ptr2 = allocator.root()->Alloc(requested_size + SystemPageSize() + 1, type_name);
        EXPECT_EQ(ptr, ptr2);

        {
            ...
            size_t slot_size = SizeToBucketSize(requested_size + SystemPageSize() + 1);
            const PartitionBucketMemoryStats* stats = dumper.GetBucketStats(slot_size);
            EXPECT_TRUE(stats);
            EXPECT_TRUE(stats->is_valid);
            EXPECT_FALSE(stats->is_direct_map);
            EXPECT_EQ(slot_size, stats->bucket_slot_size);
            EXPECT_EQ(requested_size + SystemPageSize() + 1 + kExtraAllocSize, stats->active_bytes);
            EXPECT_EQ(1u, stats->active_count);
            EXPECT_EQ(slot_size, stats->resident_bytes);
            EXPECT_EQ(0u, stats->decommittable_bytes);
            EXPECT_EQ((slot_size - (requested_size + SystemPageSize() + 1)) / SystemPageSize() * SystemPageSize(), stats->discardable_bytes);
            EXPECT_EQ(1u, stats->num_full_slot_spans);
            EXPECT_EQ(0u, stats->num_active_slot_spans);
            EXPECT_EQ(0u, stats->num_empty_slot_spans);
            EXPECT_EQ(0u, stats->num_decommitted_slot_spans);
...
{
    char* ptr = static_cast<char*>(allocator.root()->Alloc(2048 - kExtraAllocSize, type_name));
    allocator.root()->Free(ptr);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
...
TEST_P(PartitionAllocTest, PreferActiveOverEmpty)
{
    ...
    SlotSpanMetadata<internal::ThreadSafe>* slot_span1 = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr1));
    SlotSpanMetadata<internal::ThreadSafe>* slot_span2 = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr3));
    SlotSpanMetadata<internal::ThreadSafe>* slot_span3 = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr6));
...
TEST_P(PartitionAllocTest, PurgeDiscardableSecondPage)
{
    ...
    void* ptr1 = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    char* ptr2 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name));
    allocator.root()->Free(ptr2);
    SlotSpanMetadata<internal::ThreadSafe>* slot_span = SlotSpanMetadata<internal::ThreadSafe>::FromSlotStart(allocator.root()->ObjectToSlotStart(ptr1));
    EXPECT_EQ(2u, slot_span->num_unprovisioned_slots);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
...
TEST_P(PartitionAllocTest, PurgeDiscardableFirstPage)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name));
    void* ptr2 = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    allocator.root()->Free(ptr1);
    {
        MockPartitionStatsDumper dumper;
        allocator.root()->DumpStats("mock_allocator", false /* detailed dump */, &dumper);
...
TEST_P(PartitionAllocTest, PurgeDiscardableNonPageSizedAlloc)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(requested_size - kExtraAllocSize, type_name));
    void* ptr2 = allocator.root()->Alloc(requested_size - kExtraAllocSize, type_name);
    void* ptr3 = allocator.root()->Alloc(requested_size - kExtraAllocSize, type_name);
    void* ptr4 = allocator.root()->Alloc(requested_size - kExtraAllocSize, type_name);
...
TEST_P(PartitionAllocTest, PurgeDiscardableWithFreeListRewrite)
{
    ...
    char* ptr1 = static_cast<char*>(allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name));
    void* ptr2 = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    void* ptr3 = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    void* ptr4 = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    ptr1[0] = 'A';
    ...
    void* ptr1b = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
    PA_EXPECT_PTR_EQ(ptr1, ptr1b);
    void* ptr2b = allocator.root()->Alloc(SystemPageSize() - kExtraAllocSize, type_name);
...
TEST_P(PartitionAllocTest, ActiveListMaintenance)
{
    ...
    ASSERT_NE(bucket->active_slot_spans_head, SlotSpanMetadata<ThreadSafe>::get_sentinel_slot_span());
    EXPECT_FALSE(bucket->active_slot_spans_head->next_slot_span);

    // The empty list has 1 entry.
    ASSERT_NE(bucket->empty_slot_spans_head, SlotSpanMetadata<ThreadSafe>::get_sentinel_slot_span());
...
{
    constexpr static size_t kAllZerosSentinel = std::numeric_limits<size_t>::max();
    for (size_t size : kTestSizes) {
        char* p = static_cast<char*>(allocator.root()->AllocWithFlags(AllocFlags::kZeroFill, size, nullptr));
...
TEST_P(PartitionAllocTest, Bug_897585)
{
    ...
    void* ptr = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, kInitialSize, nullptr);
    ASSERT_NE(nullptr, ptr);
    ptr = allocator.root()->ReallocWithFlags(AllocFlags::kReturnNull, ptr, kDesiredSize, nullptr);
    ASSERT_NE(nullptr, ptr);
...
TEST_P(PartitionAllocTest, OverrideHooks)
{
    ...
    PartitionAllocHooks::SetOverrideHooks(
        [](void** out, unsigned int flags, size_t size, const char* type_name) -> bool {
    ...
    void* ptr = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, kOverriddenSize, kOverriddenType);
    ...
    // realloc it.
    free_called = false;
    ptr = allocator.root()->ReallocWithFlags(AllocFlags::kReturnNull, ptr, 1, nullptr);
...
TEST_P(PartitionAllocTest, FundamentalAlignment)
{
    ...
    for (size_t size = 0; size < SystemPageSize(); size++) {
        ...
        EXPECT_EQ(allocator.root()->AllocationCapacityFromSlotStart(slot_start) % fundamental_alignment, fundamental_alignment - kInSlotRefCountBufferSize);
#else
        EXPECT_EQ(allocator.root()->AllocationCapacityFromSlotStart(slot_start) % fundamental_alignment, 0u);
#endif
...
void VerifyAlignment(PartitionRoot<ThreadSafe>* root, size_t size, size_t alignment)
{
    std::vector<void*> allocated_ptrs;
...
TEST_P(PartitionAllocTest, GetUsableSize)
{
    ...
    for (size_t size = 1; size <= kMinDirectMappedDownsize; size += delta) {
        ...
        size_t usable_size_with_hack = PartitionRoot<ThreadSafe>::GetUsableSizeWithMac11MallocSizeHack(ptr);
...
TEST_P(PartitionAllocTest, Bookkeeping)
{
    ...
    for (size_t huge_size : huge_sizes) {
        ...
        for (size_t alignment : alignments) {
            ...
            ptr = root.AllocWithFlagsInternal(0, huge_size - kExtraAllocSize, alignment, type_name);
            ...
            size_t surrounding_pages_size = PartitionRoot<ThreadSafe>::GetDirectMapMetadataAndGuardPagesSize() + alignment - PartitionPageSize();
...
TEST_P(PartitionAllocTest, RefCountBasic)
{
    ...
    *ptr1 = kCookie;
    ...
    auto* ref_count = PartitionRefCountPointer(allocator.root()->ObjectToSlotStart(ptr1));
...
void PartitionAllocTest::RunRefCountReallocSubtest(size_t orig_size, size_t new_size)
{
    ...
    auto* ref_count1 = PartitionRefCountPointer(allocator.root()->ObjectToSlotStart(ptr1));
    ...
    auto* ref_count2 = PartitionRefCountPointer(allocator.root()->ObjectToSlotStart(ptr2));
...
INSTANTIATE_TEST_SUITE_P(AlternateBucketDistribution, UnretainedDanglingRawPtrTest, testing::Values(false, true));

TEST_P(UnretainedDanglingRawPtrTest, UnretainedDanglingPtrNoReport)
{
    void* ptr = allocator.root()->Alloc(kTestAllocSize, type_name);
    EXPECT_TRUE(ptr);
    auto* ref_count = PartitionRefCountPointer(allocator.root()->ObjectToSlotStart(ptr));
    ref_count->Acquire();
...
TEST_P(PartitionAllocTest, DanglingPtr)
{
    ...
    uint64_t* ptr = static_cast<uint64_t*>(allocator.root()->Alloc(64 - kExtraAllocSize, type_name));
    auto* ref_count = PartitionRefCountPointer(allocator.root()->ObjectToSlotStart(ptr));
    ref_count->Acquire();
    ref_count->Acquire();
    ref_count->Acquire();
...
TEST_P(PartitionAllocTest, CrossPartitionRootRealloc)
{
    ...
    void* ptr = allocator.root()->AllocWithFlags(AllocFlags::kReturnNull, test_size, nullptr);
    EXPECT_TRUE(ptr);
    ...
    void* ptr2 = new_root->ReallocWithFlags(AllocFlags::kReturnNull, ptr, test_size + 1024, nullptr);
    EXPECT_TRUE(ptr2);
    PA_EXPECT_PTR_NE(ptr, ptr2);
}
...
TEST_P(PartitionAllocTest, FastPathOrReturnNull)
{
    ...
    EXPECT_FALSE(allocator.root()->AllocWithFlags(AllocFlags::kFastPathOrReturnNull, allocation_size, ""));
    void* ptr = allocator.root()->AllocWithFlags(0, allocation_size, "");
    ASSERT_TRUE(ptr);

    // Next one is, since the partition page has been activated.
    void* ptr2 = allocator.root()->AllocWithFlags(AllocFlags::kFastPathOrReturnNull, allocation_size, "");
    EXPECT_TRUE(ptr2);

    // First allocation of a different bucket is slow.
    EXPECT_FALSE(allocator.root()->AllocWithFlags(AllocFlags::kFastPathOrReturnNull, 2 * allocation_size, ""));

    size_t allocated_size = 2 * allocation_size;
    std::vector<void*> ptrs;
    while (void* new_ptr = allocator.root()->AllocWithFlags(AllocFlags::kFastPathOrReturnNull, allocation_size, "")) {
...
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && defined(PA_HAS_DEATH_TESTS) && !BUILDFLAG(PA_IS_CASTOS)
...
TEST_P(PartitionAllocTest, ConfigurablePool)
{
    ...
    for (size_t pool_size = max_pool_size; pool_size >= min_pool_size; pool_size /= 2) {
        ...
        EXPECT_FALSE(IsConfigurablePoolAvailable());
        uintptr_t pool_base = AllocPages(pool_size, pool_size, PageAccessibilityConfiguration::kInaccessible, PageTag::kPartitionAlloc);
...
TEST_P(PartitionAllocTest, IncreaseEmptySlotSpanRingSize)
{
    ...
    EXPECT_EQ(PA_TS_UNCHECKED_READ(root.empty_slot_spans_dirty_bytes), kMaxFreeableSpans * bucket_size);
}
...
TEST_P(PartitionAllocTest, PartitionTagBasic)
{
    ...
    constexpr partition_alloc::PartitionTag kTag1 = static_cast<partition_alloc::PartitionTag>(0xBADA);
    constexpr partition_alloc::PartitionTag kTag2 = static_cast<partition_alloc::PartitionTag>(0xDB8A);
    constexpr partition_alloc::PartitionTag kTag3 = static_cast<partition_alloc::PartitionTag>(0xA3C4);

    partition_alloc::internal::NormalBucketPartitionTagSetValue(allocator.root()->ObjectToSlotStart(ptr1), slot_span->bucket->slot_size, kTag1);
    partition_alloc::internal::NormalBucketPartitionTagSetValue(allocator.root()->ObjectToSlotStart(ptr2), slot_span->bucket->slot_size, kTag2);
    partition_alloc::internal::NormalBucketPartitionTagSetValue(allocator.root()->ObjectToSlotStart(ptr3), slot_span->bucket->slot_size, kTag3);
...
TEST_P(PartitionAllocTest, PartitionTagDirectMapBasic)
{
    ...
    void* object = allocator.root()->AllocWithFlags(AllocFlags::kZeroFill, kAllocSize, type_name);
    ASSERT_TRUE(object);
    ASSERT_TRUE(IsManagedByDirectMap(UntagPtr(object)));

    constexpr partition_alloc::PartitionTag kTag = static_cast<partition_alloc::PartitionTag>(0xBADA);
    partition_alloc::internal::DirectMapPartitionTagSetValue(allocator.root()->ObjectToSlotStart(object), kTag);
    ...
    EXPECT_EQ(kTag, partition_alloc::internal::PartitionTagGetValue(static_cast<char*>(object) + kSuperPageSize));
    EXPECT_EQ(kTag, partition_alloc::internal::PartitionTagGetValue(static_cast<char*>(object) + (2 * kSuperPageSize)));
    EXPECT_EQ(kTag, partition_alloc::internal::PartitionTagGetValue(static_cast<char*>(object) + (3 * kSuperPageSize) - 1));
...
#if BUILDFLAG(PA_IS_CAST_ANDROID) && BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT)
extern "C" {
...
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && BUILDFLAG(IS_LINUX) && defined(ARCH_CPU_64_BITS)
...
TEST_P(PartitionAllocTest, SortActiveSlotSpans)
{
    auto run_test = [](size_t count) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            slot_span.num_unprovisioned_slots = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() / 2);
            slot_span.num_allocated_slots = partition_alloc::internal::base::RandGenerator(bucket.get_slots_per_span() - slot_span.num_unprovisioned_slots);
        ...
        size_t sorted_spans_count = std::min(PartitionBucket<ThreadSafe>::kMaxSlotSpansToSort, count);
        ...
        auto has_empty_freelist = [](SlotSpanMetadata<ThreadSafe>* a) { return a->GetFreelistLength() == 0; };
