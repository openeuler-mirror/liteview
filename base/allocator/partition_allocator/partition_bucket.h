...
template <bool thread_safe> struct PartitionBucket {
    ...
    PA_COMPONENT_EXPORT(PARTITION_ALLOC)
    PA_NOINLINE uintptr_t SlowPathAlloc(PartitionRoot<thread_safe>* root, unsigned int flags, size_t raw_size, size_t slot_span_alignment,
    ...
    // compilation unit.
    uintptr_t AllocNewSuperPageSpanForGwpAsan(PartitionRoot<thread_safe>* root, size_t super_page_count, unsigned int flags)
...
private:
    ...
    // super page.
    PA_ALWAYS_INLINE uintptr_t AllocNewSuperPageSpan(PartitionRoot<thread_safe>* root, size_t super_page_count, unsigned int flags)
        PA_EXCLUSIVE_LOCKS_REQUIRED(root->lock_);

    // Allocates a new slot span with size |num_partition_pages| from the
    // current extent. Metadata within this slot span will be initialized.
    // Returns nullptr on error.
    PA_ALWAYS_INLINE SlotSpanMetadata<thread_safe>* AllocNewSlotSpan(PartitionRoot<thread_safe>* root, unsigned int flags, size_t slot_span_alignment)
        PA_EXCLUSIVE_LOCKS_REQUIRED(root->lock_);
    ...
    PA_ALWAYS_INLINE uintptr_t AllocNewSuperPage(PartitionRoot<thread_safe>* root, unsigned int flags) PA_EXCLUSIVE_LOCKS_REQUIRED(root->lock_);
    ...
    // freelist.
    PA_ALWAYS_INLINE void InitializeSlotSpan(SlotSpanMetadata<thread_safe>* slot_span);

    // Initializes a super page. Returns the address of the super page's payload.
    PA_ALWAYS_INLINE uintptr_t InitializeSuperPage(PartitionRoot<thread_safe>* root, uintptr_t super_page, uintptr_t requested_address)
        PA_EXCLUSIVE_LOCKS_REQUIRED(root->lock_);
    ...
    PA_ALWAYS_INLINE uintptr_t ProvisionMoreSlotsAndAllocOne(PartitionRoot<thread_safe>* root, SlotSpanMetadata<thread_safe>* slot_span)
        PA_EXCLUSIVE_LOCKS_REQUIRED(root->lock_);
};
