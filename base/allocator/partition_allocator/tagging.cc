...
void ChangeMemoryTaggingModeForCurrentThread(TagViolationReportingMode m)
{
    ...
    if (m == TagViolationReportingMode::kSynchronous) {
        ChangeMemoryTaggingModeInternal(PR_TAGGED_ADDR_ENABLE | PR_MTE_TCF_SYNC | (0xfffe << PR_MTE_TAG_SHIFT));
    } else if (m == TagViolationReportingMode::kAsynchronous) {
        ChangeMemoryTaggingModeInternal(PR_TAGGED_ADDR_ENABLE | PR_MTE_TCF_ASYNC | (0xfffe << PR_MTE_TAG_SHIFT));
    } else {
...
#if BUILDFLAG(IS_ANDROID)
void ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode m)
{
#if defined(PA_HAS_MEMORY_TAGGING)
...
//extern "C" void* __arm_mte_create_random_tag(void* ptr, uint64_t mask);
...
TagMemoryRangeIncrementInternalFn* global_tag_memory_range_increment_fn = TagRegionIncrementNoOp;
TagMemoryRangeRandomlyInternalFn* global_tag_memory_range_randomly_fn = TagRegionRandomlyNoOp;
