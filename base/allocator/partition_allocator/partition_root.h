...
template <typename Z> static constexpr bool offset_type = std::is_integral_v<Z> && sizeof(Z) <= sizeof(ptrdiff_t);

// We want this size to be big enough that we have time to start up other
// scripts _before_ we wrap around.
static constexpr size_t kAllocInfoSize = /*1 << 24*/ 1 << 12;
...
struct PartitionOptions {
    ...
    constexpr PartitionOptions(AlignedAlloc aligned_alloc, ThreadCache thread_cache, Quarantine quarantine, Cookie cookie, BackupRefPtr backup_ref_ptr,
        BackupRefPtrZapping backup_ref_ptr_zapping, UseConfigurablePool use_configurable_pool)
...
// PartitionAllocator.
template <bool thread_safe> struct PA_ALIGNAS(64) PA_COMPONENT_EXPORT(PARTITION_ALLOC) PartitionRoot {
    ...
    using SuperPageExtentEntry = internal::PartitionSuperPageExtentEntry<thread_safe>;
    ...
    PA_ALWAYS_INLINE void DecreaseTotalSizeOfAllocatedBytes(SlotSpan* slot_span) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    PA_ALWAYS_INLINE void IncreaseTotalSizeOfAllocatedBytes(SlotSpan* slot_span, size_t raw_size) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    PA_ALWAYS_INLINE void DecreaseTotalSizeOfAllocatedBytes(uintptr_t addr, size_t len) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    PA_ALWAYS_INLINE void IncreaseTotalSizeOfAllocatedBytes(uintptr_t addr, size_t len, size_t raw_size) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    PA_ALWAYS_INLINE void RecommitSystemPagesForData(uintptr_t address, size_t length, PageAccessibilityDisposition accessibility_disposition)
        PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    PA_ALWAYS_INLINE bool TryRecommitSystemPagesForData(uintptr_t address, size_t length, PageAccessibilityDisposition accessibility_disposition)
    ...
    PA_ALWAYS_INLINE void* AlignedAllocWithFlags(unsigned int flags, size_t alignment, size_t requested_size);
    ...
    PA_NOINLINE PA_MALLOC_FN void* Alloc(size_t requested_size, const char* type_name) PA_MALLOC_ALIGNED;
    PA_ALWAYS_INLINE PA_MALLOC_FN void* AllocWithFlags(unsigned int flags, size_t requested_size, const char* type_name) PA_MALLOC_ALIGNED;
    ...
    PA_ALWAYS_INLINE PA_MALLOC_FN void* AllocWithFlagsInternal(
        unsigned int flags, size_t requested_size, size_t slot_span_alignment, const char* type_name) PA_MALLOC_ALIGNED;
    ...
    PA_ALWAYS_INLINE PA_MALLOC_FN void* AllocWithFlagsNoHooks(unsigned int flags, size_t requested_size, size_t slot_span_alignment) PA_MALLOC_ALIGNED;

    PA_NOINLINE void* Realloc(void* ptr, size_t newize, const char* type_name) PA_MALLOC_ALIGNED;
    // Overload that may return nullptr if reallocation isn't possible. In this
    // case, |ptr| remains valid.
    PA_NOINLINE void* TryRealloc(void* ptr, size_t new_size, const char* type_name) PA_MALLOC_ALIGNED;
    PA_NOINLINE void* ReallocWithFlags(unsigned int flags, void* ptr, size_t new_size, const char* type_name) PA_MALLOC_ALIGNED;
    ...
    PA_ALWAYS_INLINE void FreeNoHooksImmediate(void* object, SlotSpan* slot_span, uintptr_t slot_start);
    ...
    // |limit|.
    void ShrinkEmptySlotSpansRing(size_t limit) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    PA_ALWAYS_INLINE void FreeInSlotSpan(uintptr_t slot_start, SlotSpan* slot_span) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Frees memory, with |slot_start| as returned by |RawAlloc()|.
    PA_ALWAYS_INLINE void RawFree(uintptr_t slot_start);
    PA_ALWAYS_INLINE void RawFree(uintptr_t slot_start, SlotSpan* slot_span) PA_LOCKS_EXCLUDED(lock_);

    PA_ALWAYS_INLINE void RawFreeBatch(FreeListEntry* head, FreeListEntry* tail, size_t size, SlotSpan* slot_span) PA_LOCKS_EXCLUDED(lock_);

    PA_ALWAYS_INLINE void RawFreeWithThreadCache(uintptr_t slot_start, SlotSpan* slot_span);
...
#if BUILDFLAG(ENABLE_BACKUP_REF_PTR_SUPPORT)
        return brp_enabled() ? internal::kBRPPoolHandle : internal::kRegularPoolHandle;
#else
    ...
    static PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR PA_ALWAYS_INLINE size_t GetDirectMapMetadataAndGuardPagesSize()
    {
    ...
    static PAGE_ALLOCATOR_CONSTANTS_DECLARE_CONSTEXPR PA_ALWAYS_INLINE size_t GetDirectMapSlotSize(size_t raw_size)
    {
    ...
    static PA_ALWAYS_INLINE size_t GetDirectMapReservationSize(size_t padded_raw_size)
    {
        // Caller must check that the size is not above the MaxDirectMapped()
        // limit before calling. This also guards against integer overflow in the
        // calculation here.
...
private:
    ...
    // this.
    PA_ALWAYS_INLINE const Bucket& PA_NO_SANITIZE("undefined") bucket_at(size_t i) const
    {
    ...
    PA_ALWAYS_INLINE uintptr_t AllocFromBucket(Bucket* bucket, unsigned int flags, size_t raw_size, size_t slot_span_alignment, size_t* usable_size,
        bool* is_already_zeroed) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);

    bool TryReallocInPlaceForNormalBuckets(void* object, SlotSpan* slot_span, size_t new_size);
    bool TryReallocInPlaceForDirectMap(internal::SlotSpanMetadata<thread_safe>* slot_span, size_t requested_size) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void DecommitEmptySlotSpans() PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    PA_ALWAYS_INLINE void RawFreeLocked(uintptr_t slot_start) PA_EXCLUSIVE_LOCKS_REQUIRED(lock_);
    uintptr_t MaybeInitThreadCacheAndAlloc(uint16_t bucket_index, size_t* slot_size);
...
public:
    ...
    ~ScopedSyscallTimer()
    {
        ...
        root_->syscall_total_time_ns.fetch_add(elapsed_nanos, std::memory_order_relaxed);
    }
...
PA_ALWAYS_INLINE uintptr_t PartitionAllocGetDirectMapSlotStartInBRPPool(uintptr_t address)
{
    ...
    auto* first_page = PartitionPage<ThreadSafe>::FromAddr(reservation_start + PartitionPageSize());
    auto* page = first_page + first_page->slot_span_metadata_offset;
    PA_DCHECK(page->is_valid);
    PA_DCHECK(!page->slot_span_metadata_offset);
    auto* slot_span = &page->slot_span_metadata;
    uintptr_t slot_start = SlotSpanMetadata<ThreadSafe>::ToSlotSpanStart(slot_span);
#if BUILDFLAG(PA_DCHECK_IS_ON)
    auto* metadata = PartitionDirectMapMetadata<ThreadSafe>::FromSlotSpan(slot_span);
    size_t padding_for_alignment = metadata->direct_map_extent.padding_for_alignment;
    PA_DCHECK(padding_for_alignment == static_cast<size_t>(page - first_page) * PartitionPageSize());
...
PA_ALWAYS_INLINE uintptr_t PartitionAllocGetSlotStartInBRPPool(uintptr_t address)
{
    ...
    uintptr_t directmap_slot_start = PartitionAllocGetDirectMapSlotStartInBRPPool(address);
    ...
    uintptr_t slot_span_start = SlotSpanMetadata<ThreadSafe>::ToSlotSpanStart(slot_span);
    size_t offset_in_slot_span = address - slot_span_start;

    auto* bucket = slot_span->bucket;
    return slot_span_start + bucket->slot_size * bucket->GetSlotNumber(offset_in_slot_span);
}
...
PA_ALWAYS_INLINE bool PartitionAllocIsValidPtrDelta(uintptr_t address, Z delta_in_bytes)
{
...
PA_ALWAYS_INLINE uintptr_t PartitionRoot<thread_safe>::AllocFromBucket(
    Bucket* bucket, unsigned int flags, size_t raw_size, size_t slot_span_alignment, size_t* usable_size, bool* is_already_zeroed)
{
    PA_DCHECK((slot_span_alignment >= internal::PartitionPageSize()) && internal::base::bits::IsPowerOfTwo(slot_span_alignment));
    ...
    if (PA_LIKELY(slot_span_alignment <= internal::PartitionPageSize() && slot_start)) {
        ...
        *usable_size = AdjustSizeForExtrasSubtract(bucket->slot_size);
        ...
        slot_start = bucket->SlowPathAlloc(this, flags, raw_size, slot_span_alignment, is_already_zeroed);
...
// static
template <bool thread_safe> PA_NOINLINE void PartitionRoot<thread_safe>::Free(void* object)
{
    return FreeWithFlags(0, object);
}

// static
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::FreeWithFlags(unsigned int flags, void* object)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE bool PartitionRoot<thread_safe>::IsMemoryTaggingEnabled() const
{
#if defined(PA_HAS_MEMORY_TAGGING)
    return !flags.use_configurable_pool;
#else
...
// static
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::FreeNoHooks(void* object)
{
    if (PA_UNLIKELY(!object))
        return;
    ...
    // in the shim.
#if BUILDFLAG(ENABLE_PARTITION_ALLOC_AS_MALLOC_SUPPORT) && (BUILDFLAG(IS_ANDROID) && !BUILDFLAG(PA_IS_CAST_ANDROID))
    ...
    if (!root->IsDirectMappedBucket(slot_span->bucket)) {
        partition_alloc::internal::PartitionTagIncrementValue(slot_start, slot_span->bucket->slot_size);
    }
#endif // defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
    ...
    if (PA_UNLIKELY(root->ShouldQuarantine(object))) {
        ...
        if (PA_LIKELY(internal::IsManagedByNormalBuckets(slot_start))) {
            PCScan::MoveToQuarantine(object, slot_span->GetUsableSize(root), slot_start, slot_span->bucket->slot_size);
            return;
        }
    }
#endif // BUILDFLAG(STARSCAN)
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::FreeNoHooksImmediate(void* object, SlotSpan* slot_span, uintptr_t slot_start)
{
    ...
    if (flags.allow_cookie) {
        ...
        internal::PartitionCookieCheckValue(static_cast<unsigned char*>(object) + slot_span->GetUsableSize(this));
    }
#endif
    ...
    if (brp_enabled()) {
        ...
        if (PA_UNLIKELY(!ref_count->IsAliveWithNoKnownRefs() && brp_zapping_enabled()))
            internal::SecureMemset(object, internal::kQuarantinedByte, slot_span->GetUsableSize(this));
    ...
    // memset() can be really expensive.
#if BUILDFLAG(PA_EXPENSIVE_DCHECKS_ARE_ON)
    internal::DebugMemset(internal::SlotStartAddr2Ptr(slot_start), internal::kFreedByte,
        slot_span->GetUtilizedSlotSize()
#if BUILDFLAG(PUT_REF_COUNT_IN_PREVIOUS_SLOT)
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::FreeInSlotSpan(uintptr_t slot_start, SlotSpan* slot_span)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::RawFree(uintptr_t slot_start)
{
...
#pragma optimize("", off)
#endif
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::RawFree(uintptr_t slot_start, SlotSpan* slot_span)
{
    ...
    *static_cast<volatile uintptr_t*>(internal::SlotStartAddr2Ptr(slot_start)) = 0;
...
template <bool thread_safe>
PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::RawFreeBatch(FreeListEntry* head, FreeListEntry* tail, size_t size, SlotSpan* slot_span)
{
    PA_DCHECK(head);
    PA_DCHECK(tail);
    PA_DCHECK(size > 0);
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::RawFreeWithThreadCache(uintptr_t slot_start, SlotSpan* slot_span)
{
    ...
    if (PA_LIKELY(flags.with_thread_cache && !IsDirectMappedBucket(slot_span->bucket))) {
        size_t bucket_index = static_cast<size_t>(slot_span->bucket - this->buckets);
        auto* thread_cache = ThreadCache::Get();
        if (PA_LIKELY(ThreadCache::IsValid(thread_cache) && thread_cache->MaybePutInCache(slot_start, bucket_index))) {
            return;
        }
    }
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::RawFreeLocked(uintptr_t slot_start)
{
    SlotSpan* slot_span = SlotSpan::FromSlotStart(slot_start);
...
// static
template <bool thread_safe> PA_ALWAYS_INLINE bool PartitionRoot<thread_safe>::IsValidSlotSpan(SlotSpan* slot_span)
{
    PartitionRoot* root = FromSlotSpan(slot_span);
    return root->inverted_self == ~reinterpret_cast<uintptr_t>(root);
}

template <bool thread_safe> PA_ALWAYS_INLINE PartitionRoot<thread_safe>* PartitionRoot<thread_safe>::FromSlotSpan(SlotSpan* slot_span)
{
    auto* extent_entry = reinterpret_cast<SuperPageExtentEntry*>(reinterpret_cast<uintptr_t>(slot_span) & internal::SystemPageBaseMask());
    return extent_entry->root;
}

template <bool thread_safe> PA_ALWAYS_INLINE PartitionRoot<thread_safe>* PartitionRoot<thread_safe>::FromFirstSuperPage(uintptr_t super_page)
{
    PA_DCHECK(internal::IsReservationStart(super_page));
    auto* extent_entry = internal::PartitionSuperPageToExtent<thread_safe>(super_page);
    PartitionRoot* root = extent_entry->root;
    PA_DCHECK(root->inverted_self == ~reinterpret_cast<uintptr_t>(root));
    return root;
}

template <bool thread_safe> PA_ALWAYS_INLINE PartitionRoot<thread_safe>* PartitionRoot<thread_safe>::FromAddrInFirstSuperpage(uintptr_t address)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::IncreaseTotalSizeOfAllocatedBytes(SlotSpan* slot_span, size_t raw_size)
{
    IncreaseTotalSizeOfAllocatedBytes(reinterpret_cast<uintptr_t>(slot_span), slot_span->GetSlotSizeForBookkeeping(), raw_size);
}

template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::DecreaseTotalSizeOfAllocatedBytes(SlotSpan* slot_span)
{
    DecreaseTotalSizeOfAllocatedBytes(reinterpret_cast<uintptr_t>(slot_span), slot_span->GetSlotSizeForBookkeeping());
}

template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::IncreaseTotalSizeOfAllocatedBytes(uintptr_t addr, size_t len, size_t raw_size)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::DecreaseTotalSizeOfAllocatedBytes(uintptr_t addr, size_t len)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::IncreaseCommittedPages(size_t len)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::DecreaseCommittedPages(size_t len)
{
...
PA_ALWAYS_INLINE void PartitionRoot<thread_safe>::DecommitSystemPagesForData(
    uintptr_t address, size_t length, PageAccessibilityDisposition accessibility_disposition)
{
    ...
    uintptr_t address, size_t length, PageAccessibilityDisposition accessibility_disposition)
{
    ...
    bool ok = TryRecommitSystemPages(address, length, GetPageAccessibility(), accessibility_disposition);
    if (PA_UNLIKELY(!ok)) {
        // Decommit some memory and retry. The alternative is crashing.
        DecommitEmptySlotSpans();
        RecommitSystemPages(address, length, GetPageAccessibility(), accessibility_disposition);
    }
    ...
    uintptr_t address, size_t length, PageAccessibilityDisposition accessibility_disposition)
{
    ...
    if (PA_UNLIKELY(!ok)) {
        ...
        ok = TryRecommitSystemPages(address, length, GetPageAccessibility(), accessibility_disposition);
    }
...
template <bool thread_safe> PA_ALWAYS_INLINE size_t PartitionRoot<thread_safe>::GetUsableSize(void* ptr)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE size_t PartitionRoot<thread_safe>::GetUsableSizeWithMac11MallocSizeHack(void* ptr)
{
    // malloc_usable_size() is expected to handle NULL gracefully and return 0.
    if (!ptr)
        return 0;
    ...
    if (PA_UNLIKELY(root->flags.mac11_malloc_size_hack_enabled_ && usable_size == internal::kMac11MallocSizeHackUsableSize)) {
        uintptr_t slot_start = internal::PartitionAllocGetSlotStartInBRPPool(UntagPtr(ptr));
        auto* ref_count = internal::PartitionRefCountPointer(slot_start);
...
template <bool thread_safe> PA_ALWAYS_INLINE PageAccessibilityConfiguration PartitionRoot<thread_safe>::GetPageAccessibility() const
{
#if defined(PA_HAS_MEMORY_TAGGING)
...
template <bool thread_safe> PA_ALWAYS_INLINE size_t PartitionRoot<thread_safe>::AllocationCapacityFromSlotStart(uintptr_t slot_start) const
{
...
// static
template <bool thread_safe> PA_ALWAYS_INLINE uint16_t PartitionRoot<thread_safe>::SizeToBucketIndex(size_t size, BucketDistribution bucket_distribution)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE void* PartitionRoot<thread_safe>::AllocWithFlags(unsigned int flags, size_t requested_size, const char* type_name)
{
    return AllocWithFlagsInternal(flags, requested_size, internal::PartitionPageSize(), type_name);
}

template <bool thread_safe>
PA_ALWAYS_INLINE void* PartitionRoot<thread_safe>::AllocWithFlagsInternal(
    unsigned int flags, size_t requested_size, size_t slot_span_alignment, const char* type_name)
{
    PA_DCHECK((slot_span_alignment >= internal::PartitionPageSize()) && partition_alloc::internal::base::bits::IsPowerOfTwo(slot_span_alignment));
...
template <bool thread_safe>
PA_ALWAYS_INLINE void* PartitionRoot<thread_safe>::AllocWithFlagsNoHooks(unsigned int flags, size_t requested_size, size_t slot_span_alignment)
{
    PA_DCHECK((slot_span_alignment >= internal::PartitionPageSize()) && partition_alloc::internal::base::bits::IsPowerOfTwo(slot_span_alignment));
    ...
    if (PA_LIKELY(this->flags.with_thread_cache && slot_span_alignment <= internal::PartitionPageSize())) {
        ...
        if (PA_LIKELY(slot_start)) {
            ...
            slot_start = RawAlloc(buckets + bucket_index, flags, raw_size, slot_span_alignment, &usable_size, &is_already_zeroed);
        }
    } else {
        slot_start = RawAlloc(buckets + bucket_index, flags, raw_size, slot_span_alignment, &usable_size, &is_already_zeroed);
    }
    ...
    if (brp_enabled()) {
        ...
        if (PA_UNLIKELY(this->flags.mac11_malloc_size_hack_enabled_ && requested_size == internal::kMac11MallocSizeHackRequestedSize)) {
            needs_mac11_malloc_size_hack = true;
        }
#endif // defined(PA_ENABLE_MAC11_MALLOC_SIZE_HACK)
        auto* ref_count = new (internal::PartitionRefCountPointer(slot_start)) internal::PartitionRefCount(needs_mac11_malloc_size_hack);
...
template <bool thread_safe>
PA_ALWAYS_INLINE uintptr_t PartitionRoot<thread_safe>::RawAlloc(
    Bucket* bucket, unsigned int flags, size_t raw_size, size_t slot_span_alignment, size_t* usable_size, bool* is_already_zeroed)
{
    ::partition_alloc::internal::ScopedGuard guard { lock_ };
    return AllocFromBucket(bucket, flags, raw_size, slot_span_alignment, usable_size, is_already_zeroed);
}

template <bool thread_safe>
PA_ALWAYS_INLINE void* PartitionRoot<thread_safe>::AlignedAllocWithFlags(unsigned int flags, size_t alignment, size_t requested_size)
{
    ...
    if (alignment <= internal::PartitionPageSize()) {
        ...
        if (PA_UNLIKELY(raw_size < alignment)) {
            ...
            // size.
            raw_size = static_cast<size_t>(1) << (int { sizeof(size_t) * 8 } - partition_alloc::internal::base::bits::CountLeadingZeroBits(raw_size - 1));
        }
    ...
    void* object = no_hooks ? AllocWithFlagsNoHooks(0, adjusted_size, slot_span_alignment) : AllocWithFlagsInternal(0, adjusted_size, slot_span_alignment, "");
...
template <bool thread_safe> PA_NOINLINE void* PartitionRoot<thread_safe>::Alloc(size_t requested_size, const char* type_name)
{
    return AllocWithFlags(0, requested_size, type_name);
}

template <bool thread_safe> PA_NOINLINE void* PartitionRoot<thread_safe>::Realloc(void* ptr, size_t new_size, const char* type_name)
{
    return ReallocWithFlags(0, ptr, new_size, type_name);
}

template <bool thread_safe> PA_NOINLINE void* PartitionRoot<thread_safe>::TryRealloc(void* ptr, size_t new_size, const char* type_name)
{
...
template <bool thread_safe> PA_ALWAYS_INLINE size_t PartitionRoot<thread_safe>::AllocationCapacityFromRequestedSize(size_t size) const
{
#if defined(MEMORY_TOOL_REPLACES_ALLOCATOR)
    return size;
#else
...
static_assert(offsetof(ThreadSafePartitionRoot, lock_) == internal::kPartitionCachelineSize, "Padding is incorrect");
