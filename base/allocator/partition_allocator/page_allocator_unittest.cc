...
TEST(PartitionAllocPageAllocatorTest, AllocFailure)
{
    ...
    uintptr_t result = AllocPages(size, PageAllocationGranularity(), PageAccessibilityConfiguration::kInaccessible, PageTag::kChromium);
    if (!result) {
...
{
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(), PageAccessibilityConfiguration::kReadWrite, PageTag::kChromium);
    EXPECT_TRUE(buffer);
...
TEST(PartitionAllocPageAllocatorTest, AllocPagesAligned)
{
    ...
    for (size_t size : sizes) {
        for (size_t offset : offsets) {
            uintptr_t buffer = AllocPagesWithAlignOffset(0, size, alignment, offset, PageAccessibilityConfiguration::kReadWrite, PageTag::kChromium);
            EXPECT_TRUE(buffer);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadWriteTagged)
{
    ...
    uintptr_t buffer
        = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(), PageAccessibilityConfiguration::kReadWriteTagged, PageTag::kChromium);
    EXPECT_TRUE(buffer);
    int* buffer0 = reinterpret_cast<int*>(buffer);
    *buffer0 = 42;
    EXPECT_EQ(42, *buffer0);
...
TEST(PartitionAllocPageAllocatorTest, AllocAndFreePagesWithPageReadExecuteConfirmCFI)
{
    ...
    SetSystemPagesAccess(buffer, PageAllocationGranularity(), PageAccessibilityConfiguration::kReadExecuteProtected);
...
#if BUILDFLAG(IS_ANDROID)
            ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode::kSynchronous);
#else
...
#if BUILDFLAG(IS_ANDROID)
            ChangeMemoryTaggingModeForAllThreadsPerProcess(TagViolationReportingMode::kAsynchronous);
#else
...
{
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(), PageAccessibilityConfiguration::kInaccessible, PageTag::kChromium);
    EXPECT_TRUE(buffer);
...
{
    uintptr_t buffer = AllocPages(PageAllocationGranularity(), PageAllocationGranularity(), PageAccessibilityConfiguration::kReadExecute, PageTag::kChromium);
    EXPECT_TRUE(buffer);
    int* buffer0 = reinterpret_cast<int*>(buffer);
...
TEST(PartitionAllocPageAllocatorTest, DecommitErasesMemory)
{
    ...
    uintptr_t buffer = AllocPages(size, PageAllocationGranularity(), PageAccessibilityConfiguration::kReadWrite, PageTag::kChromium);
    ASSERT_TRUE(buffer);
    ...
    RecommitSystemPages(buffer, size, PageAccessibilityConfiguration::kReadWrite, PageAccessibilityDisposition::kAllowKeepForPerf);
...
// Test permission setting on POSIX, where we can set a trap handler.
    ...
    SetSystemPagesAccess(buffer, size, PageAccessibilityConfiguration::kReadWrite);
...
TEST(PartitionAllocPageAllocatorTest, MappedPagesAccounting)
{
    ...
    for (size_t offset : offsets) {
        uintptr_t data = AllocPagesWithAlignOffset(0, size, alignment, offset, PageAccessibilityConfiguration::kInaccessible, PageTag::kChromium);
        ASSERT_TRUE(data);
