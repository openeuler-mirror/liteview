...
{
    if (PA_UNLIKELY(state_.exchange(kUnlocked, std::memory_order_release) == kLockedContended)) {
...
PA_ALWAYS_INLINE bool SpinningMutex::Try()
{
    return !!::TryAcquireSRWLockExclusiveXp(reinterpret_cast<PSRWLOCK>(&lock_));
}

PA_ALWAYS_INLINE void SpinningMutex::Release()
{
    ::ReleaseSRWLockExclusiveXp(reinterpret_cast<PSRWLOCK>(&lock_));
}
