...
void* GwpAsanSupport::MapRegion(size_t slot_count, std::vector<uint16_t>& free_list)
{
    ...
    uint16_t bucket_index = PartitionRoot<internal::ThreadSafe>::SizeToBucketIndex(kSlotSize, root->GetBucketDistribution());
    ...
    uintptr_t super_page_span_start;
    {
        ...
        super_page_span_start = bucket->AllocNewSuperPageSpanForGwpAsan(root.get(), super_page_count, 0);
        ...
        for (uintptr_t super_page = super_page_span_start; super_page < super_page_span_end; super_page += kSuperPageSize) {
            auto* page_metadata = internal::PartitionSuperPageToMetadataArea<internal::ThreadSafe>(super_page);
            ...
            for (size_t partition_page_idx = 1 + internal::NumPartitionPagesPerFreeSlotBitmap();
                ...
                auto slot_span_start = internal::SlotSpanMetadata<internal::ThreadSafe>::ToSlotSpanStart(slot_span_metadata);
