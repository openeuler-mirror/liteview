...
void MemoryReclaimer::UnregisterPartition(PartitionRoot<internal::ThreadSafe>* partition)
{
    internal::ScopedGuard lock(lock_);
    PA_DCHECK(partition);
...
void MemoryReclaimer::Reclaim(int flags)
{
...
#if PA_STARSCAN_ENABLE_STARSCAN_ON_RECLAIM && BUILDFLAG(STARSCAN)
    {
        ...
        const auto invocation_mode = flags & PurgeFlags::kAggressiveReclaim ? PCScan::InvocationMode::kForcedBlocking : PCScan::InvocationMode::kBlocking;
