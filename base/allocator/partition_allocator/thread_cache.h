...
extern PA_COMPONENT_EXPORT(PARTITION_ALLOC) thread_local g_thread_cache;
#endif
...
public:
    ...
    static constexpr internal::base::TimeDelta kMinPurgeInterval = internal::base::Seconds(1);
    static constexpr internal::base::TimeDelta kMaxPurgeInterval = internal::base::Minutes(1);
    static constexpr internal::base::TimeDelta kDefaultPurgeInterval = 2 * kMinPurgeInterval;
...
private:
    ...
    internal::base::TimeDelta periodic_purge_next_interval_ = kDefaultPurgeInterval;
...
#else
    uint8_t largest_active_bucket_index_ = internal::BucketIndexLookup::GetIndex(ThreadCacheLimits::kDefaultSizeThreshold);
#endif
};
...
public:
    ...
    // large.
    PA_ALWAYS_INLINE bool MaybePutInCache(uintptr_t slot_start, size_t bucket_index);
    ...
    static constexpr size_t kDefaultSizeThreshold = ThreadCacheLimits::kDefaultSizeThreshold;
    static constexpr size_t kLargeSizeThreshold = ThreadCacheLimits::kLargeSizeThreshold;
...
private:
...
#else
    static constexpr uint16_t kBucketCount = internal::BucketIndexLookup::GetIndex(ThreadCache::kLargeSizeThreshold) + 1;
#endif
    ...
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, MultipleObjectsCachedPerBucket);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, LargeAllocationsAreNotCached);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, MultipleThreadCaches);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, RecordStats);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, ThreadCacheRegistry);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, MultipleThreadCachesAccounting);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, DynamicCountPerBucket);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, DynamicCountPerBucketClamping);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, DynamicCountPerBucketMultipleThreads);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, DynamicSizeThreshold);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, DynamicSizeThresholdPurge);
    PA_FRIEND_TEST_ALL_PREFIXES(PartitionAllocThreadCacheTest, ClearFromTail);
};

PA_ALWAYS_INLINE bool ThreadCache::MaybePutInCache(uintptr_t slot_start, size_t bucket_index)
{
...
PA_ALWAYS_INLINE uintptr_t ThreadCache::GetFromCache(size_t bucket_index, size_t* slot_size)
{
    ...
    internal::PartitionFreelistEntry* next = entry->GetNextForThreadCache<true>(bucket.slot_size);
...
{
#if defined(PA_HAS_FREELIST_SHADOW_ENTRY) && defined(ARCH_CPU_X86_64) && defined(PA_HAS_64_BITS_POINTERS)
    ...
    auto* entry = internal::PartitionFreelistEntry::EmplaceAndInitForThreadCache(slot_start, bucket.freelist_head);
