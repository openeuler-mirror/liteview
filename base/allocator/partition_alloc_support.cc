...
constexpr const char* ScannerIdToTracingString(partition_alloc::internal::StatsCollector::ScannerId id)
{
    switch (id) {
...
constexpr const char* MutatorIdToTracingString(partition_alloc::internal::StatsCollector::MutatorId id)
{
    switch (id) {
...
public:
    void ReportTraceEvent(partition_alloc::internal::StatsCollector::ScannerId id, [[maybe_unused]] partition_alloc::internal::base::PlatformThreadId tid,
        int64_t start_time_ticks_internal_value, int64_t end_time_ticks_internal_value) override
    {
#if BUILDFLAG(ENABLE_BASE_TRACING)
        // TRACE_EVENT_* macros below drop most parameters when tracing is
        // disabled at compile time.
        const char* tracing_id = ScannerIdToTracingString(id);
        const TimeTicks start_time = TimeTicks::FromInternalValue(start_time_ticks_internal_value);
        const TimeTicks end_time = TimeTicks::FromInternalValue(end_time_ticks_internal_value);
        TRACE_EVENT_BEGIN(kTraceCategory, perfetto::StaticString(tracing_id), perfetto::ThreadTrack::ForThread(tid), start_time);
        TRACE_EVENT_END(kTraceCategory, perfetto::ThreadTrack::ForThread(tid), end_time);
#endif // BUILDFLAG(ENABLE_BASE_TRACING)
    }

    void ReportTraceEvent(partition_alloc::internal::StatsCollector::MutatorId id, [[maybe_unused]] partition_alloc::internal::base::PlatformThreadId tid,
        int64_t start_time_ticks_internal_value, int64_t end_time_ticks_internal_value) override
    {
#if BUILDFLAG(ENABLE_BASE_TRACING)
        // TRACE_EVENT_* macros below drop most parameters when tracing is
    ...
    {
        TRACE_COUNTER1(kTraceCategory, "PCScan.SurvivedQuarantineSize", survived_size);
    }
    ...
    void ReportSurvivedQuarantinePercent(double survived_rate) override
    {
        ...
        TRACE_COUNTER1(kTraceCategory, "PCScan.SurvivedQuarantinePercent", 1000 * survived_rate);
    }
...
void RunThreadCachePeriodicPurge()
{
    ...
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce(RunThreadCachePeriodicPurge), delay);
}
...
void RunMemoryReclaimer(scoped_refptr<SequencedTaskRunner> task_runner)
{
    ...
    task_runner->PostDelayedTask(FROM_HERE, BindOnce(RunMemoryReclaimer, task_runner), delay);
}
...
std::map<std::string, std::string> ProposeSyntheticFinchTrials()
{
    ...
    if (brp_finch_enabled && features::kBackupRefPtrModeParam.Get() != features::BackupRefPtrMode::kDisabled)
        brp_nondefault_behavior = true;
    if (brp_finch_enabled && features::kBackupRefPtrModeParam.Get() == features::BackupRefPtrMode::kEnabled)
    ...
    if (pcscan_enabled) {
        ...
        if (features::kBackupRefPtrModeParam.Get() != features::BackupRefPtrMode::kDisabled) {
...
{
    std::vector<StringPiece> lines = SplitStringPiece(stacktrace, "\r\n", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
...
void DanglingRawPtrReleasedLogSignature(uintptr_t id)
{
    ...
    if (stack_trace_free) {
        LOG(ERROR) << StringPrintf("[DanglingSignature]\t%s\t%s", ExtractDanglingPtrSignature(stack_trace_release.ToString()).c_str(),
            ExtractDanglingPtrSignature(stack_trace_free->ToString()).c_str());
    } else {
        LOG(ERROR) << StringPrintf("[DanglingSignature]\t%s\tmissing-stacktrace", ExtractDanglingPtrSignature(stack_trace_release.ToString()).c_str());
    }
}
...
void InstallDanglingRawPtrChecks()
{
    ...
    case features::DanglingPtrMode::kLogSignature:
        partition_alloc::SetDanglingRawPtrDetectedFn(DanglingRawPtrDetected);
        partition_alloc::SetDanglingRawPtrReleasedFn(DanglingRawPtrReleasedLogSignature);
        break;
    }
}
...
void UnretainedDanglingRawPtrDetectedCrash(uintptr_t id)
{
    ...
    debug::StackTrace stack_trace;
    LOG(ERROR) << "Detected dangling raw_ptr in unretained with id=" << StringPrintf("0x%016" PRIxPTR, id) << ":\n\n" << task_trace << stack_trace;
    IMMEDIATE_CRASH();
}
