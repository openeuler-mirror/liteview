...
template <typename T> inline void ReadBigEndian(const uint8_t buf[], T* out)
{
...
// Note: this loop is unrolled with -O1 and above.
template <typename T> inline void WriteBigEndian(char buf[], T val)
{
    static_assert(std::is_integral<T>::value, "T has to be an integral type.");
...
template <> inline void ReadBigEndian<uint8_t>(const uint8_t buf[], uint8_t* out)
{
    *out = buf[0];
}

template <> inline void WriteBigEndian<uint8_t>(char buf[], uint8_t val)
{
    buf[0] = static_cast<char>(val);
}

template <> inline void ReadBigEndian<int8_t>(const uint8_t buf[], int8_t* out)
{
    *out = static_cast<int8_t>(buf[0]);
}

template <> inline void WriteBigEndian<int8_t>(char buf[], int8_t val)
{
    buf[0] = static_cast<char>(val);
}
...
private:
    ...
    template <typename T> bool ReadLengthPrefixed(base::StringPiece* out);
...
private:
    // Hidden to promote type safety.
    template <typename T> bool Write(T v);
