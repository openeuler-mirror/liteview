...
BigEndianReader BigEndianReader::FromStringPiece(base::StringPiece string_piece)
{
...
template <typename T> bool BigEndianReader::Read(T* value)
{
...
template <typename T> bool BigEndianReader::ReadLengthPrefixed(base::StringPiece* out)
{
    T t_len;
    if (!Read(&t_len))
        return false;
...
template <typename T> bool BigEndianWriter::Write(T value)
{
    if (sizeof(T) > remaining())
        return false;
