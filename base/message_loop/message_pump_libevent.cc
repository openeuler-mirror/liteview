...
BASE_FEATURE(kMessagePumpEpoll, "MessagePumpEpoll", FEATURE_DISABLED_BY_DEFAULT);
#endif
...
void MessagePumpLibevent::FdWatchController::OnFileCanReadWithoutBlocking(int fd, MessagePumpLibevent* pump)
{
...
void MessagePumpLibevent::FdWatchController::OnFileCanWriteWithoutBlocking(int fd, MessagePumpLibevent* pump)
{
    DCHECK(watcher_);
    watcher_->OnFileCanWriteWithoutBlocking(fd);
}

const scoped_refptr<MessagePumpLibevent::EpollInterest>& MessagePumpLibevent::FdWatchController::AssignEpollInterest(const EpollInterestParams& params)
{
...
bool MessagePumpLibevent::WatchFileDescriptor(int fd, bool persistent, int mode, FdWatchController* controller, FdWatcher* delegate)
{
#if BUILDFLAG(ENABLE_MESSAGE_PUMP_EPOLL)
    if (epoll_pump_) {
        return epoll_pump_->WatchFileDescriptor(fd, persistent, mode, controller, delegate);
    }
#endif

    TRACE_EVENT("base", "MessagePumpLibevent::WatchFileDescriptor", "fd", fd, "persistent", persistent, "watch_read", mode & WATCH_READ, "watch_write",
        mode & WATCH_WRITE);
    DCHECK_GE(fd, 0);
    DCHECK(controller);
    DCHECK(delegate);
...
bool MessagePumpLibevent::Init()
{
    ...
    event_set(wakeup_event_.get(), wakeup_pipe_out_, EV_READ | EV_PERSIST, OnWakeup, this);
...
void MessagePumpLibevent::OnLibeventNotification(int fd, short flags, void* context)
{
    ...
    TRACE_EVENT("toplevel", "OnLibevent", "controller_created_from", controller->created_from_location(), "fd", fd, "flags", flags, "context", context);
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpLibevent::OnWakeup", "socket", socket, "flags", flags, "context", context);
...
MessagePumpLibevent::EpollInterest::EpollInterest(FdWatchController* controller, const EpollInterestParams& params)
