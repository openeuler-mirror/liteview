...
// See sys/timerfd.h
long timerfd_settime(int ufc, int flags, const struct itimerspec* utmr, struct itimerspec* otmr)
{
    ...
    : env_(base::android::AttachCurrentThread())
{
    ...
    resume_after_yielding_non_delayed_fd_ = checked_cast<int>(timerfd_create(CLOCK_MONOTONIC, O_NONBLOCK | O_CLOEXEC));
    ...
    // fine.
    delayed_fd_ = checked_cast<int>(timerfd_create(CLOCK_MONOTONIC, O_NONBLOCK | O_CLOEXEC));
    ...
    ALooper_addFd(looper_, resume_after_yielding_non_delayed_fd_, 0, ALOOPER_EVENT_INPUT, &ResumeNonDelayedLooperCallback, reinterpret_cast<void*>(this));
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::OnDelayedLooperCallback");
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::OnResumeNonDelayedLooperCallback");
    // There may be non-Chromium callbacks on the same ALooper which may have left
    // a pending exception set, and ALooper does not check for this between
    // callbacks. Check here, and if there's already an exception, just skip this
    // iteration without clearing the fd. If the exception ends up being non-fatal
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::OnNonDelayedLooperCallback");
    // There may be non-Chromium callbacks on the same ALooper which may have left
    // a pending exception set, and ALooper does not check for this between
    // callbacks. Check here, and if there's already an exception, just skip this
    // iteration without clearing the fd. If the exception ends up being non-fatal
...
void MessagePumpForUI::ScheduleWorkInternal(bool do_idle_work)
{
    ...
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::ScheduleWorkInternal", "do_idle_work", do_idle_work);
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::ScheduleWorkWithDelay");
    ...
    struct itimerspec ts = GetTimerSpecDelay(timer.since_origin() + yield_duration_.value());
    long ret = timerfd_settime(resume_after_yielding_non_delayed_fd_, TFD_TIMER_ABSTIME, &ts, nullptr);
    DPCHECK(ret >= 0);
}
...
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("base"), "MessagePumpForUI::QuitWhenIdle");
