...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    const int epoll_timeout = timeout.is_max() ? -1 : saturated_cast<int>(timeout.InMilliseconds());
    epoll_event event;
    const int epoll_result = epoll_wait(epoll_.get(), &event, /*maxevents=*/1, epoll_timeout);
    if (epoll_result < 0) {
        DPCHECK(errno == EINTR);
        return false;
    }
