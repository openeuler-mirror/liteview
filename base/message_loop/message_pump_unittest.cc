...
TEST_P(MessagePumpTest, YieldToNativeRequestedSmokeTest)
{
    ...
    EXPECT_CALL(delegate, DoWork).WillOnce(Invoke([this] {
        ...
        return MessagePump::Delegate::NextWorkInfo { now + Milliseconds(1), now, true };
    }));
...
TEST_P(MessagePumpTest, TimerSlackWithLongDelays)
{
    ...
    thread.task_runner()->PostDelayedTask(FROM_HERE, BindLambdaForTesting([&delegate] { delegate.WakeUpFromOtherThread(); }), Seconds(2));
