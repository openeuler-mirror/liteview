...
GSourceFuncs EventInjector::SourceFuncs = { EventInjector::Prepare, EventInjector::Check, EventInjector::Dispatch, nullptr };
...
TEST_F(MessagePumpGLibTest, TestWorkWhileWaitingForEvents)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&IncrementInt, &task_count));
    }
    // After all the previous tasks have executed, enqueue an event that will
    // quit.
    {
        RunLoop run_loop;
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&EventInjector::AddEvent, Unretained(injector()), 0, run_loop.QuitClosure()));
        run_loop.Run();
    }
    ...
    task_count = 0;
    for (int i = 0; i < 10; ++i) {
        ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce(&IncrementInt, &task_count), Milliseconds(10 * i));
    }
    // After all the previous tasks have executed, enqueue an event that will
    // quit.
...
public:
    ...
    void FromTask()
    {
        ...
        if (task_count_ == 0 && event_count_ == 0) {
            ...
            ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, this));
        }
    }
...
TEST_F(MessagePumpGLibTest, TestConcurrentEventPostedTask)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, helper));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&ConcurrentHelper::FromTask, helper));
...
TEST_F(MessagePumpGLibTest, TestDrainingGLib)
{
    ...
    RunLoop run_loop;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&AddEventsAndDrainGLib, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
...
void TestGLibLoopInternal(EventInjector* injector, OnceClosure done)
{
    ...
    injector->AddDummyEvent(10);
    // Delayed work
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce(&IncrementInt, &task_count), Milliseconds(30));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, BindOnce(&GLibLoopRunner::Quit, runner), Milliseconds(40));
...
TEST_F(MessagePumpGLibTest, TestGLibLoop)
{
    ...
    RunLoop run_loop;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&TestGLibLoopInternal, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
}
...
TEST_F(MessagePumpGLibTest, TestGtkLoop)
{
    ...
    // between events and posted tasks.
    RunLoop run_loop;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&TestGtkLoopInternal, Unretained(injector()), run_loop.QuitClosure()));
    run_loop.Run();
}
...
public:
    ...
    {
        RunLoop runloop;
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitMessageLoopAndStart, runloop.QuitClosure()));
        runloop.Run();
    }
...
public:
    QuitWatcher(MessagePumpGlib::FdWatchController* controller, base::OnceClosure quit_closure)
        : BaseWatcher(controller)
...
TEST_F(MessagePumpGLibFdWatchTest, QuitWatcher)
{
    ...
    const char buf = 0;
    WaitableEventWatcher::EventCallback write_fd_task = BindOnce(&WriteFDWrapper, pipefds_[1], &buf, 1);
    io_runner()->PostTask(
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&WaitableEvent::Signal, Unretained(&event)));
