...
void MessagePumpForUI::ScheduleWork()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Chrome.MessageLoopProblem", MESSAGE_POST_ERROR, MESSAGE_LOOP_PROBLEM_MAX);
...
void MessagePumpForUI::ScheduleNativeTimer(Delegate::NextWorkInfo next_work_info)
{
    ...
    if (delay_msec == 0) {
        ...
        delay_msec = clamp(delay_msec, UINT(USER_TIMER_MINIMUM), UINT(USER_TIMER_MAXIMUM));
        ...
        UMA_HISTOGRAM_ENUMERATION("Chrome.MessageLoopProblem", SET_TIMER_ERROR, MESSAGE_LOOP_PROBLEM_MAX);
...
bool MessagePumpForUI::ProcessMessageHelper(const MSG& msg)
{
    ...
    if (msg.message == WM_QUIT) {
        ...
        // unexpected.
        UMA_HISTOGRAM_ENUMERATION("Chrome.MessageLoopProblem", RECEIVED_WM_QUIT_ERROR, MESSAGE_LOOP_PROBLEM_MAX);
        return true;
    }
...
void MessagePumpForIO::ScheduleWork()
{
    ...
    UMA_HISTOGRAM_ENUMERATION("Chrome.MessageLoopProblem", COMPLETION_POST_ERROR, MESSAGE_LOOP_PROBLEM_MAX);
...
HRESULT MessagePumpForIO::RegisterIOHandler(HANDLE file_handle, IOHandler* handler)
{
    DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
...
//--
bool MessagePumpForIO::WaitForIOCompletionWithHandler(DWORD timeout, MessagePumpForIO::IOHandler* filter)
{
    DCHECK_CALLED_ON_VALID_THREAD(bound_thread_);
    ...
    MessagePump::Delegate::ScopedDoWorkItem scoped_do_work_item = run_state_->delegate->BeginWorkItem();
    ...
    if (/*!item.has_valid_io_context || item.context->handler*/true) {
