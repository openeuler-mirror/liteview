...
public:
    ...
    static void CreatePortPair(mac::ScopedMachReceiveRight* receive, mac::ScopedMachSendRight* send)
    {
        mach_port_options_t options {};
        options.flags = MPO_INSERT_SEND_RIGHT;
        mac::ScopedMachReceiveRight port;
        kern_return_t kr = mach_port_construct(mach_task_self(), &options, 0, mac::ScopedMachReceiveRight::Receiver(*receive).get());
...
TEST_F(MessagePumpKqueueTest, MachPortStopWatching)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([](mach_port_t port) { EXPECT_EQ(KERN_SUCCESS, SendEmptyMessage(port, 100)); }, port.get()));
