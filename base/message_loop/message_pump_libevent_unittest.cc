...
class MessagePumpLibeventTest : public testing::Test, public testing::WithParamInterface<PumpType> {
protected:
...
#if BUILDFLAG(ENABLE_MESSAGE_PUMP_EPOLL)
        if (GetParam() == kEpoll) {
            return std::make_unique<MessagePumpLibevent>(MessagePumpLibevent::kUseEpoll);
        }
#endif
    ...
    void SimulateIOEvent(MessagePumpLibevent* pump, MessagePumpLibevent::FdWatchController* controller)
    {
#if BUILDFLAG(ENABLE_MESSAGE_PUMP_EPOLL)
        if (GetParam() == kEpoll) {
            pump->epoll_pump_->HandleEvent(0, /*can_read=*/true, /*can_write=*/true, controller);
            return;
        }
#endif
...
{
    std::unique_ptr<MessagePumpLibevent> pump = CreateMessagePump();
    MessagePumpLibevent::FdWatchController* watcher = new MessagePumpLibevent::FdWatchController(FROM_HERE);
    DeleteWatcher delegate(watcher);
    pump->WatchFileDescriptor(pipefds_[1], false, MessagePumpLibevent::WATCH_READ_WRITE, watcher, &delegate);
...
TEST_P(MessagePumpLibeventTest, NestedPumpWatcher)
{
    ...
    pump->WatchFileDescriptor(pipefds_[1], false, MessagePumpLibevent::WATCH_READ, &watcher, &delegate);
    SimulateIOEvent(pump.get(), &watcher);
}
...
public:
    QuitWatcher(MessagePumpLibevent::FdWatchController* controller, base::OnceClosure quit_closure)
        : BaseWatcher(controller)
...
TEST_P(MessagePumpLibeventTest, QuitWatcher)
{
    ...
    pump->WatchFileDescriptor(pipefds_[0], false, MessagePumpLibevent::WATCH_READ, &controller, &delegate);
...
INSTANTIATE_TEST_SUITE_P(, MessagePumpLibeventTest, ::testing::Values(TEST_PARAM_VALUES));
