...
void PowerMonitor::Initialize(std::unique_ptr<PowerMonitorSource> source)
{
    ...
    PowerMonitor::PowerMonitor::NotifyThermalStateChange(PowerMonitor::Source()->GetCurrentThermalState());

    PowerMonitor::PowerMonitor::NotifySpeedLimitChange(PowerMonitor::Source()->GetInitialSpeedLimit());
}
...
void PowerMonitor::NotifyPowerStateChange(bool on_battery_power)
{
    ...
    if (power_monitor->on_battery_power_ != on_battery_power) {
        ...
        GetInstance()->power_state_observers_->Notify(FROM_HERE, &PowerStateObserver::OnPowerStateChange, on_battery_power);
    }
}
...
{
    DCHECK(IsInitialized());
    TRACE_EVENT_INSTANT0("base", "PowerMonitor::NotifySuspend", TRACE_EVENT_SCOPE_PROCESS);
    ...
    if (!power_monitor->is_system_suspended_) {
        ...
        GetInstance()->power_suspend_observers_->Notify(FROM_HERE, &PowerSuspendObserver::OnSuspend);
    }
}
...
{
    DCHECK(IsInitialized());
    TRACE_EVENT_INSTANT0("base", "PowerMonitor::NotifyResume", TRACE_EVENT_SCOPE_PROCESS);
...
void PowerMonitor::NotifyThermalStateChange(PowerThermalObserver::DeviceThermalState new_state)
{
    ...
    if (power_monitor->power_thermal_state_ != new_state) {
        ...
        GetInstance()->thermal_state_observers_->Notify(FROM_HERE, &PowerThermalObserver::OnThermalStateChange, new_state);
    }
}
...
void PowerMonitor::NotifySpeedLimitChange(int speed_limit)
{
    ...
    if (power_monitor->speed_limit_ != speed_limit) {
        ...
        GetInstance()->thermal_state_observers_->Notify(FROM_HERE, &PowerThermalObserver::OnSpeedLimitChange, speed_limit);
    }
}
