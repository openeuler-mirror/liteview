...
TEST_F(PowerMonitorTest, SuspendStateReturnedFromAddObserver)
{
    ...
    EXPECT_FALSE(PowerMonitor::AddPowerSuspendObserverAndReturnSuspendedState(&observer1));

    source().GenerateSuspendEvent();

    EXPECT_TRUE(PowerMonitor::AddPowerSuspendObserverAndReturnSuspendedState(&observer2));
...
TEST_F(PowerMonitorTest, PowerStateReturnedFromAddObserver)
{
    ...
    EXPECT_FALSE(PowerMonitor::AddPowerStateObserverAndReturnOnBatteryState(&observer1));

    source().GeneratePowerStateEvent(true);

    // An observer is added after the on-battery notification.
    EXPECT_TRUE(PowerMonitor::AddPowerStateObserverAndReturnOnBatteryState(&observer2));
