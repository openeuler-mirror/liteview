...
{
    const auto register_suspend_resume_notification_ptr = reinterpret_cast<decltype(&::RegisterSuspendResumeNotification)>(
        ::GetProcAddress(::GetModuleHandle(L"user32.dll"), "RegisterSuspendResumeNotification"));
...
{
    const auto unregister_suspend_resume_notification_ptr = reinterpret_cast<decltype(&::UnregisterSuspendResumeNotification)>(
        ::GetProcAddress(::GetModuleHandle(L"user32.dll"), "UnregisterSuspendResumeNotification"));
    ...
    , message_hwnd_(NULL)
{
    ...
    base::win::InitializeWindowClass(kWindowClassName, &base::win::WrappedWindowProc<PowerMonitorDeviceSource::PowerMessageWindow::WndProcThunk>, 0, 0, 0, NULL,
        NULL, NULL, NULL, NULL, &window_class);
    instance_ = window_class.hInstance;
    ATOM clazz = RegisterClassEx(&window_class);
    DCHECK(clazz);

    message_hwnd_ = CreateWindowEx(WS_EX_NOACTIVATE, kWindowClassName, NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, instance_, NULL);
    if (message_hwnd_) {
        ...
        // fired.
        power_notify_handle_ = base::RegisterSuspendResumeNotification(message_hwnd_, DEVICE_NOTIFY_WINDOW_HANDLE);
    }
}
