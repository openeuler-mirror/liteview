...
#else
    static NtQuerySectionType nt_query_section_func = nullptr;
    static long run_once = 0;
    do {
        volatile long old_var = _InterlockedCompareExchange((long volatile*)&(run_once), 2, 0);
        if (0 == old_var) {
            HMODULE mod_handle = ::GetModuleHandle(L"ntdll.dll");
            nt_query_section_func = reinterpret_cast<NtQuerySectionType>(::GetProcAddress(mod_handle, "NtQuerySection"));
