...
public:
    ...
    // weolar
    //PlatformSharedMemoryRegion(uintptr_t mem_handle);
    // ====
    ...
    absl::optional<span<uint8_t>> MapAt(uint64_t offset, size_t size, SharedMemoryMapper* mapper) const;
...
private:
    ...
    PlatformSharedMemoryRegion(uintptr_t handle, Mode mode, size_t size, const UnguessableToken& guid);

    ScopedPlatformSharedMemoryHandle handle_;
    uintptr_t mem_handle_ = 0; // weolar: not shared memory, but direct memory.
