...
{
    return other.get() && size() == other->size() && (size() == 0 || (memcmp(front(), other->front(), size()) == 0));
}
...
scoped_refptr<RefCountedBytes> RefCountedBytes::TakeVector(std::vector<unsigned char>* to_destroy)
{
...
// static
scoped_refptr<RefCountedString> RefCountedString::TakeString(std::string* to_destroy)
{
    return TakeString(std::move(*to_destroy));
}

// static
scoped_refptr<RefCountedString> RefCountedString::TakeString(std::string&& str)
{
...
// static
scoped_refptr<RefCountedString16> RefCountedString16::TakeString(std::u16string* to_destroy)
{
    return TakeString(std::move(*to_destroy));
}

// static
scoped_refptr<RefCountedString16> RefCountedString16::TakeString(std::u16string&& str)
{
