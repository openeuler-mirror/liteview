...
public:
    ...
    {
        static_assert(std::is_base_of<internal::SupportsWeakPtrBase, Derived>::value, "AsWeakPtr argument must inherit from SupportsWeakPtr");
...
private:
    ...
    template <typename Derived, typename Base> static WeakPtr<Derived> AsWeakPtrImpl(SupportsWeakPtr<Base>* t)
    {
...
template <typename T> SafeRef<T> MakeSafeRefFromWeakPtrInternals(const internal::WeakReference& ref, T* ptr);
...
//     foo->method();
//
template <typename T> class TRIVIAL_ABI WeakPtr : public internal::WeakPtrBase {
public:
    WeakPtr() = default;
    ...
    /*implicit*/ WeakPtr(const WeakPtr<U>& other)
        : WeakPtrBase(other)
    {
...
private:
    ...
    template <typename U> friend SafeRef<U> internal::MakeSafeRefFromWeakPtrInternals(const internal::WeakReference& ref, U* ptr);
...
template <class T> class SupportsWeakPtr : public internal::SupportsWeakPtrBase {
public:
