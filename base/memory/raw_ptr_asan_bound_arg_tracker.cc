...
// static
ThreadLocalPointer<RawPtrAsanBoundArgTracker::ProtectedArgsVector>& RawPtrAsanBoundArgTracker::CurrentProtectedArgs()
{
    // We use thread-local storage instead of sequence-local storage for
    // consistency with PendingReport in RawPtrAsanService.
    static NoDestructor<ThreadLocalPointer<RawPtrAsanBoundArgTracker::ProtectedArgsVector>> protected_args;
