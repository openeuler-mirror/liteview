...
DiscardableSharedMemory::LockResult DiscardableSharedMemory::Lock(size_t offset, size_t length)
{
    ...
    madvise(static_cast<char*>(shared_memory_mapping_.memory()) + AlignToPageSize(sizeof(SharedState)), AlignToPageSize(mapped_size_), MADV_FREE_REUSE);
...
{
    return static_cast<uint8_t*>(shared_memory_mapping_.memory()) + AlignToPageSize(sizeof(SharedState));
}
...
#define MADV_PURGE_ARGUMENT MADV_FREE
    ...
    if (madvise(
            static_cast<char*>(shared_memory_mapping_.memory()) + AlignToPageSize(sizeof(SharedState)), AlignToPageSize(mapped_size_), MADV_PURGE_ARGUMENT)) {
    ...
    static DiscardVirtualMemoryFunction discard_virtual_memory
        = reinterpret_cast<DiscardVirtualMemoryFunction>(GetProcAddress(GetModuleHandle(L"Kernel32.dll"), "DiscardVirtualMemory"));

    char* address = static_cast<char*>(shared_memory_mapping_.memory()) + AlignToPageSize(sizeof(SharedState));
    ...
    uint64_t address_int = reinterpret_cast<uint64_t>(static_cast<char*>(shared_memory_mapping_.memory()) + AlignToPageSize(sizeof(SharedState)));
    zx_status_t status = zx::vmar::root_self()->op_range(ZX_VMO_OP_DECOMMIT, address_int, AlignToPageSize(mapped_size_), nullptr, 0);
...
void DiscardableSharedMemory::ReleaseMemoryIfPossible(size_t offset, size_t length)
{
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL)
// Linux and Android provide MADV_REMOVE which is preferred as it has a
// behavior that can be verified in tests. Other POSIX flavors (MacOSX, BSDs),
...
#define MADV_PURGE_ARGUMENT MADV_FREE
    ...
    // Subsequent accesses of memory pages will succeed, but might result in
    // zero-fill-on-demand pages.
    if (madvise(static_cast<char*>(shared_memory_mapping_.memory()) + offset, length, MADV_PURGE_ARGUMENT)) {
        DPLOG(ERROR) << "madvise() failed";
    }
#else // BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL)
    partition_alloc::DiscardSystemPages(static_cast<char*>(shared_memory_mapping_.memory()) + offset, length);
