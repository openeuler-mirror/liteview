...
TYPED_TEST(SharedMemoryRegionTest, MoveRegion)
{
    ...
    EXPECT_EQ(memcmp(this->rw_mapping_.memory(), mapping.memory(), kRegionSize), 0);
...
TYPED_TEST(SharedMemoryRegionTest, MappingValidAfterClose)
{
    ...
    EXPECT_TRUE(IsMemoryFilledWithByte(this->rw_mapping_.memory(), kRegionSize, 'G'));
}
...
{
    EXPECT_EQ(0U, reinterpret_cast<uintptr_t>(this->rw_mapping_.memory()) & (subtle::PlatformSharedMemoryRegion::kMapMinimumAlignment - 1));
}
