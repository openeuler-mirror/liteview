...
public:
    ...
    template <typename T> const T* GetMemoryAs() const
    {
    ...
    // page-aligned.
    template <typename T> span<const T> GetMemoryAsSpan() const
    {
        static_assert(std::is_trivially_copyable<T>::value,
            "Copying non-trivially-copyable object across memory spaces "
            "is dangerous");
    ...
    template <typename T> span<const T> GetMemoryAsSpan(size_t count) const
    {
        static_assert(std::is_trivially_copyable<T>::value,
            "Copying non-trivially-copyable object across memory spaces "
            "is dangerous");
...
public:
    ...
    template <typename T> T* GetMemoryAs() const
    {
        static_assert(std::is_trivially_copyable<T>::value, "Copying non-trivially-copyable object across memory spaces is dangerous");
        if (!IsValid())
            return nullptr;
        if (sizeof(T) > size())
            return nullptr;
    ...
    template <typename T> span<T> GetMemoryAsSpan() const
    {
        static_assert(std::is_trivially_copyable<T>::value,
            "Copying non-trivially-copyable object across memory spaces "
            "is dangerous");
    ...
    template <typename T> span<T> GetMemoryAsSpan(size_t count) const
    {
        static_assert(std::is_trivially_copyable<T>::value,
            "Copying non-trivially-copyable object across memory spaces "
            "is dangerous");
