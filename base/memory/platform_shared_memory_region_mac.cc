...
// static
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(mac::ScopedMachSendRight handle, Mode mode, size_t size, const UnguessableToken& guid)
{
    if (!handle.is_valid())
        return {};
...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Duplicate() const
{
    ...
    return PlatformSharedMemoryRegion(mac::ScopedMachSendRight(handle_.get()), mode_, size_, guid_);
}
...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Create(Mode mode, size_t size)
{
    ...
    kern_return_t kr = mach_make_memory_entry_64(mach_task_self(), &vm_size,
        0, // Address.
        MAP_MEM_NAMED_CREATE | VM_PROT_READ | VM_PROT_WRITE, mac::ScopedMachSendRight::Receiver(named_right).get(),
...
PlatformSharedMemoryRegion::PlatformSharedMemoryRegion(mac::ScopedMachSendRight handle, Mode mode, size_t size, const UnguessableToken& guid)
