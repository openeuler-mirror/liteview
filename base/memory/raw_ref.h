...
template <class T, class RawPtrType> class raw_ref;

namespace internal {

template <class T> struct is_raw_ref : std::false_type { };

template <class T, class I> struct is_raw_ref<::base::raw_ref<T, I>> : std::true_type { };
...
template <class T, class RawPtrType = DefaultRawPtrType> class TRIVIAL_ABI GSL_POINTER raw_ref {
    ...
    static constexpr bool need_clear_after_move = std::is_same_v<Impl, internal::RawPtrNoOpImpl> ||
#if defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
        std::is_same_v<Impl, internal::MTECheckedPtrImpl<internal::MTECheckedPtrImplPartitionAllocSupport>> ||
#endif // defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
        std::is_same_v<Impl, internal::AsanBackupRefPtrImpl>;

public:
    ALWAYS_INLINE explicit raw_ref(T& p) noexcept
    ...
    ALWAYS_INLINE raw_ref(const raw_ref& p) noexcept
        : inner_(p.inner_)
    {
        CHECK(inner_.get()); // Catch use-after-move.
    }

    ALWAYS_INLINE raw_ref(raw_ref&& p) noexcept
    ...
    // Upcast assignment
    template <class U, class = std::enable_if_t<std::is_convertible_v<U&, T&>>> ALWAYS_INLINE raw_ref& operator=(const raw_ref<U, RawPtrType>& p) noexcept
    {
        CHECK(p.inner_.get()); // Catch use-after-move.
        inner_.operator=(p.inner_);
        return *this;
    }
    template <class U, class = std::enable_if_t<std::is_convertible_v<U&, T&>>> ALWAYS_INLINE raw_ref& operator=(raw_ref<U, RawPtrType>&& p) noexcept
    {
        CHECK(p.inner_.get()); // Catch use-after-move.
        inner_.operator=(std::move(p.inner_));
        if constexpr (need_clear_after_move)
    ...
    template <class U> friend ALWAYS_INLINE bool operator==(const raw_ref& lhs, const raw_ref<U, RawPtrType>& rhs)
    {
        CHECK(lhs.inner_.get()); // Catch use-after-move.
        CHECK(rhs.inner_.get()); // Catch use-after-move.
    ...
    template <class U, class = std::enable_if_t<!internal::is_raw_ref_v<U>, void>> friend ALWAYS_INLINE bool operator==(const raw_ref& lhs, const U& rhs)
    {
        CHECK(lhs.inner_.get()); // Catch use-after-move.
    ...
    template <class U, class = std::enable_if_t<!internal::is_raw_ref_v<U>, void>> friend ALWAYS_INLINE bool operator==(const U& lhs, const raw_ref& rhs)
    {
        CHECK(rhs.inner_.get()); // Catch use-after-move.
...
private:
    template <class U, class J> friend class raw_ref;
...
template <typename T, typename I> struct IsRawRef<raw_ref<T, I>> : std::true_type { };
...
template <typename T, typename I> struct RemoveRawRef<raw_ref<T, I>> {
    using type = T;
};
...
template <typename T, typename RawPtrType> struct less<raw_ref<T, RawPtrType>> {
    using Impl = typename base::raw_ptr_traits::RawPtrTypeToImpl<RawPtrType>::Impl;
    using is_transparent = void;

    bool operator()(const raw_ref<T, RawPtrType>& lhs, const raw_ref<T, RawPtrType>& rhs) const
    {
