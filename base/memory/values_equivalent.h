...
// pointers.
template <typename T> bool ValuesEquivalent(const T* a, const T* b)
{
    if (a == b)
        return true;
    if (!a || !b)
...
//     }
//   };
template <typename T, std::enable_if_t<std::is_pointer_v<decltype(std::declval<T>().get())>>* = nullptr> bool ValuesEquivalent(const T& x, const T& y)
{
