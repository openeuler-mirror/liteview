...
#if BUILDFLAG(USE_BACKUP_REF_PTR) || defined(PA_ENABLE_MTE_CHECKED_PTR_SUPPORT_WITH_64_BITS_POINTERS)
...
template <typename T> struct RawPtrTypeToImpl;
}
...
struct RawPtrNoOpImpl {
    // Wraps a pointer.
    template <typename T> static PA_ALWAYS_INLINE T* WrapRawPtr(T* ptr)
    {
        return ptr;
    }

    // Notifies the allocator when a wrapped pointer is being removed or replaced.
    template <typename T> static PA_ALWAYS_INLINE void ReleaseWrappedPtr(T*)
    {
    }

    // Unwraps the pointer, while asserting that memory hasn't been freed. The
    // function is allowed to crash on nullptr.
    template <typename T> static PA_ALWAYS_INLINE T* SafelyUnwrapPtrForDereference(T* wrapped_ptr)
    {
        return wrapped_ptr;
    }
    ...
    template <typename T> static PA_ALWAYS_INLINE T* SafelyUnwrapPtrForExtraction(T* wrapped_ptr)
    {
        return wrapped_ptr;
    }
    ...
    // freed or not.
    template <typename T> static PA_ALWAYS_INLINE T* UnsafelyUnwrapPtrForComparison(T* wrapped_ptr)
    {
        return wrapped_ptr;
    }

    // Upcasts the wrapped pointer.
    template <typename To, typename From> static PA_ALWAYS_INLINE constexpr To* Upcast(From* wrapped_ptr)
    {
    ...
    template <typename T, typename Z, typename = std::enable_if_t<offset_type<Z>, void>> static PA_ALWAYS_INLINE T* Advance(T* wrapped_ptr, Z delta_elems)
    {
        return wrapped_ptr + delta_elems;
    }

    template <typename T> static PA_ALWAYS_INLINE ptrdiff_t GetDeltaElems(T* wrapped_ptr1, T* wrapped_ptr2)
    {
    ...
    template <typename T> static PA_ALWAYS_INLINE T* Duplicate(T* wrapped_ptr)
    {
        return wrapped_ptr;
    }
...
constexpr uintptr_t kTagMask = ~kAddressMask & partition_alloc::internal::kPtrUntagMask;
...
static_assert((kTopBit & kTagMask) > 0, "kTopBit bit must be inside the tag region");
...
struct MTECheckedPtrImplPartitionAllocSupport {
    ...
    template <typename T> static PA_ALWAYS_INLINE bool EnabledForPtr(T* ptr)
    {
...
template <typename PartitionAllocSupport> struct MTECheckedPtrImpl {
    ...
    template <typename T> static PA_ALWAYS_INLINE T* WrapRawPtr(T* ptr)
    {
        ...
        uintptr_t tag = *(static_cast<volatile partition_alloc::PartitionTag*>(PartitionAllocSupport::TagPointer(addr)));
        DCHECK(tag);
    ...
    template <typename T> static PA_ALWAYS_INLINE T* SafelyUnwrapPtrForDereference(T* wrapped_ptr)
    {
        ...
        if (tag > 0) {
            ...
            uintptr_t read_tag = *static_cast<volatile partition_alloc::PartitionTag*>(PartitionAllocSupport::TagPointer(ExtractAddress(wrapped_addr)));
            if (PA_UNLIKELY(tag != read_tag))
                IMMEDIATE_CRASH();
            // See the disambiguation comment above.
            // TODO(kdlee): Ensure that ptr's hardware MTE tag is preserved.
            // TODO(kdlee): Ensure that hardware and software MTE tags don't conflict.
            return static_cast<T*>(partition_alloc::internal::TagAddr(ExtractAddress(wrapped_addr)));
        }
        return wrapped_ptr;
    }
    ...
    template <typename T> static PA_ALWAYS_INLINE ptrdiff_t GetDeltaElems(T* wrapped_ptr1, T* wrapped_ptr2)
    {
        ...
        return reinterpret_cast<T*>(ExtractAddress(partition_alloc::UntagPtr(wrapped_ptr1)))
            - reinterpret_cast<T*>(ExtractAddress(partition_alloc::UntagPtr(wrapped_ptr2)));
    }
...
private:
    ...
    template <typename T> static PA_ALWAYS_INLINE T* ExtractPtr(T* wrapped_ptr)
    {
        ...
        // TODO(kdlee): Ensure that hardware and software MTE tags don't conflict.
        return static_cast<T*>(partition_alloc::internal::TagAddr(ExtractAddress(partition_alloc::UntagPtr(wrapped_ptr))));
    }
...
BASE_EXPORT void CheckThatAddressIsntWithinFirstPartitionPage(uintptr_t address);
#endif
...
template <bool AllowDangling = false> struct BackupRefPtrImpl {
    ...
    // BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)
        ...
        // PartitionAlloc pool.
        bool is_in_brp_pool = partition_alloc::IsManagedByPartitionAllocBRPPool(address);
    ...
    // Notifies the allocator when a wrapped pointer is being removed or replaced.
    template <typename T> static PA_ALWAYS_INLINE void ReleaseWrappedPtr(T* wrapped_ptr)
    {
    ...
    template <typename T> static PA_ALWAYS_INLINE void ReportIfDangling(T* wrapped_ptr)
    {
...
private:
    ...
    static BASE_EXPORT PA_NOINLINE void ReportIfDanglingInternal(uintptr_t address);
    template <typename Z, typename = std::enable_if_t<offset_type<Z>, void>> static PA_ALWAYS_INLINE bool IsValidDelta(uintptr_t address, Z delta_in_bytes)
    {
    ...
    static BASE_EXPORT PA_NOINLINE bool IsValidSignedDelta(uintptr_t address, ptrdiff_t delta_in_bytes);
    static BASE_EXPORT PA_NOINLINE bool IsValidUnsignedDelta(uintptr_t address, size_t delta_in_bytes);
};
...
private:
    static BASE_EXPORT PA_NOINLINE void AsanCheckIfValidInstantiation(void const volatile* ptr);
    static BASE_EXPORT PA_NOINLINE void AsanCheckIfValidDereference(void const volatile* ptr);
    static BASE_EXPORT PA_NOINLINE void AsanCheckIfValidExtraction(void const volatile* ptr);
};

template <class Super> struct RawPtrCountingImplWrapperForTest : public raw_ptr_traits::RawPtrTypeToImpl<Super>::Impl {
    ...
    template <typename T> static PA_ALWAYS_INLINE void ReleaseWrappedPtr(T* ptr)
    {
...
template <typename T> struct IsSupportedType<T, std::enable_if_t<std::is_function<T>::value>> {
...
// purposes.
template <typename T> struct IsRawPtrCountingImpl : std::false_type { };

template <typename T> struct IsRawPtrCountingImpl<internal::RawPtrCountingImplWrapperForTest<T>> : std::true_type { };
...
template <typename T> struct IsSupportedType<T, std::enable_if_t<std::is_convertible<T*, id>::value>> {
    static constexpr bool value = false;
};
#endif // __OBJC__
...
template <typename T> struct RawPtrTypeToImpl<internal::RawPtrCountingImplWrapperForTest<T>> {
    using Impl = internal::RawPtrCountingImplWrapperForTest<T>;
};

template <> struct RawPtrTypeToImpl<RawPtrMayDangle> {
#if BUILDFLAG(USE_BACKUP_REF_PTR)
    ...
    using Impl = internal::MTECheckedPtrImpl<internal::MTECheckedPtrImplPartitionAllocSupport>;
#else
    using Impl = internal::RawPtrNoOpImpl;
#endif
};

template <> struct RawPtrTypeToImpl<RawPtrBanDanglingIfSupported> {
#if BUILDFLAG(USE_BACKUP_REF_PTR)
...
template <> struct RawPtrTypeToImpl<RawPtrNoOp> {
    using Impl = internal::RawPtrNoOpImpl;
};
...
template <typename T, typename RawPtrType = DefaultRawPtrType> class PA_TRIVIAL_ABI PA_GSL_POINTER raw_ptr {
    using Impl = typename raw_ptr_traits::RawPtrTypeToImpl<RawPtrType>::Impl;
    using DanglingRawPtr = std::conditional_t<raw_ptr_traits::IsRawPtrCountingImpl<Impl>::value,
        raw_ptr<T, internal::RawPtrCountingImplWrapperForTest<RawPtrMayDangle>>, raw_ptr<T, RawPtrMayDangle>>;
...
public:
    ...
    constexpr PA_ALWAYS_INLINE raw_ptr() noexcept
    ...
    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    // NOLINTNEXTLINE(google-explicit-constructor)
    PA_ALWAYS_INLINE raw_ptr(T* p) noexcept
        : wrapped_ptr_(Impl::WrapRawPtr(p))
    {
    }

    // Deliberately implicit in order to support implicit upcast.
    template <typename U, typename Unused = std::enable_if_t<std::is_convertible<U*, T*>::value && !std::is_void<typename std::remove_cv<T>::type>::value>>
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    // Upcast assignment
    template <typename U, typename Unused = std::enable_if_t<std::is_convertible<U*, T*>::value && !std::is_void<typename std::remove_cv<T>::type>::value>>
    PA_ALWAYS_INLINE raw_ptr& operator=(const raw_ptr<U, RawPtrType>& ptr) noexcept
    {
        // Make sure that pointer isn't assigned to itself (look at pointer address,
        // not its value).
#if BUILDFLAG(PA_DCHECK_IS_ON) || BUILDFLAG(ENABLE_BACKUP_REF_PTR_SLOW_CHECKS)
        CHECK(reinterpret_cast<uintptr_t>(this) != reinterpret_cast<uintptr_t>(&ptr));
#endif
        Impl::ReleaseWrappedPtr(wrapped_ptr_);
        wrapped_ptr_ = Impl::Duplicate(Impl::template Upcast<T, U>(ptr.wrapped_ptr_));
        return *this;
    ...
    explicit PA_ALWAYS_INLINE operator bool() const
    {
        return !!wrapped_ptr_;
    }

    template <typename U = T, typename Unused = std::enable_if_t<!std::is_void<typename std::remove_cv<U>::type>::value>> PA_ALWAYS_INLINE U& operator*() const
    {
    ...
    template <typename PMF> void operator->*(PMF) const = delete;

    // Deliberately implicit, because raw_ptr is supposed to resemble raw ptr.
    // NOLINTNEXTLINE(runtime/explicit)
    PA_ALWAYS_INLINE operator T*() const
    {
        return GetForExtraction();
    }
    template <typename U> explicit PA_ALWAYS_INLINE operator U*() const
    {
    ...
    template <typename Z, typename = std::enable_if_t<internal::offset_type<Z>>> PA_ALWAYS_INLINE raw_ptr& operator+=(Z delta_elems)
    {
    ...
    template <typename Z, typename = std::enable_if_t<internal::offset_type<Z>>> friend PA_ALWAYS_INLINE raw_ptr operator+(const raw_ptr& p, Z delta_elems)
    {
        raw_ptr result = p;
    ...
    friend PA_ALWAYS_INLINE ptrdiff_t operator-(const raw_ptr& p1, const raw_ptr& p2)
    {
    ...
    {
        if constexpr (std::is_same_v<typename std::remove_reference<decltype(*this)>::type, DanglingRawPtr>) {
    ...
    template <typename U, typename V, typename I> friend PA_ALWAYS_INLINE bool operator==(const raw_ptr<U, I>& lhs, const raw_ptr<V, I>& rhs);
    template <typename U> friend PA_ALWAYS_INLINE bool operator!=(const raw_ptr& lhs, const raw_ptr<U, Impl>& rhs)
    {
        return !(lhs == rhs);
    }
    template <typename U, typename V, typename I> friend PA_ALWAYS_INLINE bool operator<(const raw_ptr<U, I>& lhs, const raw_ptr<V, I>& rhs);
    ...
    // T*.
    template <typename U> friend PA_ALWAYS_INLINE bool operator==(const raw_ptr& lhs, U* rhs)
    {
    ...
    template <typename U> friend PA_ALWAYS_INLINE bool operator==(U* lhs, const raw_ptr& rhs)
    {
    ...
    // serialisable.
    template <class U = T> typename perfetto::check_traced_value_support<U>::type WriteIntoTrace(perfetto::TracedValue&& context) const
    {
...
private:
    ...
    T* wrapped_ptr_ = nullptr; // NO_RBI

    template <typename U, typename V> friend class raw_ptr;
};

template <typename U, typename V, typename I> PA_ALWAYS_INLINE bool operator==(const raw_ptr<U, I>& lhs, const raw_ptr<V, I>& rhs)
{
...
template <typename T, typename I> struct IsRawPtr<raw_ptr<T, I>> : std::true_type { };
...
template <typename T, typename I> struct IsPointer<raw_ptr<T, I>> : std::true_type { };
...
template <typename T, typename I> struct RemovePointer<raw_ptr<T, I>> {
    using type = T;
};
...
using DisableDanglingPtrDetectionDegradeToNoOpWhenMTE = DisableDanglingPtrDetection;
...
template <typename T, typename RawPtrType> struct less<raw_ptr<T, RawPtrType>> {
    using Impl = typename base::raw_ptr_traits::RawPtrTypeToImpl<RawPtrType>::Impl;
    using is_transparent = void;

    bool operator()(const raw_ptr<T, RawPtrType>& lhs, const raw_ptr<T, RawPtrType>& rhs) const
    {
...
template <typename T, typename Impl> struct iterator_traits<raw_ptr<T, Impl>> {
