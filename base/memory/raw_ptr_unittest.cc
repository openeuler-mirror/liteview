...
// be copied by memcpy.
static_assert(std::is_trivially_copyable<raw_ptr<void>>::value, "raw_ptr should be trivially copyable");
static_assert(std::is_trivially_copyable<raw_ptr<int>>::value, "raw_ptr should be trivially copyable");
static_assert(std::is_trivially_copyable<raw_ptr<std::string>>::value, "raw_ptr should be trivially copyable");
...
static_assert(std::is_trivially_default_constructible<raw_ptr<void>>::value, "raw_ptr should be trivially default constructible");
static_assert(std::is_trivially_default_constructible<raw_ptr<int>>::value, "raw_ptr should be trivially default constructible");
static_assert(std::is_trivially_default_constructible<raw_ptr<std::string>>::value, "raw_ptr should be trivially default constructible");
...
using RawPtrCountingImpl = base::internal::RawPtrCountingImplWrapperForTest<base::DefaultRawPtrType>;
using RawPtrCountingMayDangleImpl = base::internal::RawPtrCountingImplWrapperForTest<base::RawPtrMayDangle>;

template <typename T> using CountingRawPtr = raw_ptr<T, RawPtrCountingImpl>;

template <typename T> using CountingRawPtrMayDangle = raw_ptr<T, RawPtrCountingMayDangleImpl>;
...
#define REPORT_UNEQUAL_RAW_PTR_COUNTER(member_name, CounterClassImpl)                                                                                          \
    {                                                                                                                                                          \
        if (arg.member_name.has_value() && arg.member_name.value() != CounterClassImpl::member_name) {                                                         \
            *result_listener << "Expected `" #member_name "` to be " << arg.member_name.value() << " but got " << CounterClassImpl::member_name << "; ";       \
            result = false;                                                                                                                                    \
        }                                                                                                                                                      \
    }
#define REPORT_UNEQUAL_RAW_PTR_COUNTERS(result, CounterClassImpl)                                                                                              \
    {                                                                                                                                                          \
        ...
        REPORT_UNEQUAL_RAW_PTR_COUNTER(pointer_to_member_operator_cnt, CounterClassImpl)                                                                       \
    }
...
// Implicit `arg` has type `CountingRawPtrExpectations`.
MATCHER(MayDangleCountingRawPtrHasCounts, "`MayDangleCountingRawPtr` has specified counters")
{
    bool result = true;
...
TEST_F(RawPtrTest, NullExtractNoDereference)
{
    ...
    std::ignore = raw;
    EXPECT_THAT(
        (CountingRawPtrExpectations { .get_for_dereference_cnt = 0, .get_for_extraction_cnt = 1, .get_for_comparison_cnt = 0 }), CountingRawPtrHasCounts());
}
...
TEST_F(RawPtrTest, ExtractAsDangling)
{
    ...
    if constexpr (std::is_same_v<RawPtrCountingImpl, RawPtrCountingMayDangleImpl>) {
...
TEST_F(RawPtrTest, UpcastNotConvertible)
{
    ...
    EXPECT_FALSE((std::is_convertible<raw_ptr<int64_t>, raw_ptr<int32_t>>::value));
    EXPECT_FALSE((std::is_convertible<raw_ptr<int16_t>, raw_ptr<int32_t>>::value));
}
...
TEST_F(RawPtrTest, TracedValueSupport)
{
...

    {
        ...
        EXPECT_EQ(perfetto::TracedValueToString(raw_ptr<WithTraceSupport>(&value)), "result");
    }
}
#endif // BUILDFLAG(ENABLE_BASE_TRACING) && BUILDFLAG(PA_USE_BASE_TRACING)
...
void RunBackupRefPtrImplAdvanceTest(partition_alloc::PartitionAllocator& allocator, size_t requested_size)
{
    ...
    protected_ptr = protected_ptr + requested_size / 2; // end-of-allocation address is ok
...
TEST_F(BackupRefPtrTest, Advance)
{
    ...
    ASSERT_EQ(requested_size, allocator_.root()->AllocationCapacityFromRequestedSize(requested_size));
...
const char kAsanBrpNotProtected_Instantiation[] = ASAN_BRP_NOT_PROTECTED("pointer to an already freed region was assigned to a raw_ptr<T>");
const char kAsanBrpNotProtected_EarlyAllocation[] = ASAN_BRP_NOT_PROTECTED("region was allocated before MiraclePtr was activated");
...
TEST_F(AsanBackupRefPtrTest, Instantiation)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED({ [[maybe_unused]] raw_ptr<AsanStruct> protected_ptr2 = ptr; }, kAsanBrpNotProtected_Instantiation);
}
...
TEST_F(AsanBackupRefPtrTest, BoundArgumentsInstantiation)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(std::move(callback).Run(), kAsanBrpNotProtected_Instantiation);
}
...
struct MTECheckedPtrImplPartitionAllocSupportForTest {
    static bool EnabledForPtr(void* ptr)
    {
        return !!ptr;
    }
...
using MTECheckedPtrImplForTest = MTECheckedPtrImpl<MTECheckedPtrImplPartitionAllocSupportForTest>;
...
TEST(MTECheckedPtrImpl, WrapAndSafelyUnwrap)
{
    ...
    uintptr_t wrapped = reinterpret_cast<uintptr_t>(MTECheckedPtrImplForTest::WrapRawPtr(ptr));
    ...
    ASSERT_EQ(MTECheckedPtrImplForTest::SafelyUnwrapPtrForDereference(reinterpret_cast<void*>(wrapped)), ptr);

    // Modify the tag in the fake allocation.
    bytes[0] |= 0x40;
    wrapped = reinterpret_cast<uintptr_t>(MTECheckedPtrImplForTest::WrapRawPtr(ptr));
...
TEST(MTECheckedPtrImpl, CrashOnTagMismatch)
{
    ...
    // pointer and points to the tag appropriately.
    unsigned char bytes[] = { 0xBA, 0x42, 0x78, 0x89 };
    unsigned char* ptr = MTECheckedPtrImplForTest::WrapRawPtr(bytes + kTagOffsetForTest);
    EXPECT_EQ(*MTECheckedPtrImplForTest::SafelyUnwrapPtrForDereference(ptr), 0x78);
    // Clobber the tag associated with the fake allocation.
    bytes[0] = 0;
    EXPECT_DEATH_IF_SUPPORTED(if (*MTECheckedPtrImplForTest::SafelyUnwrapPtrForDereference(ptr) == 0x78) return, "");
}

#if !defined(MEMORY_TOOL_REPLACES_ALLOCATOR) && BUILDFLAG(USE_PARTITION_ALLOC_AS_MALLOC)
...
TEST(MTECheckedPtrImpl, DirectMapCrashOnUseAfterFree)
{
    ...
    constexpr size_t kIntsPerSuperPage = partition_alloc::kSuperPageSize / sizeof(int);
    constexpr size_t kAllocAmount = kIntsPerSuperPage * 2;
    int* unwrapped_ptr = new int[kAllocAmount];
    ASSERT_TRUE(partition_alloc::internal::IsManagedByDirectMap(partition_alloc::UntagPtr(unwrapped_ptr)));
...
TEST(MTECheckedPtrImpl, AdvancedPointerShiftedAppropriately)
{
    ...
    ptr += 5;
    EXPECT_EQ(reinterpret_cast<uintptr_t>(ptr.get()) - original_addr, 5 * sizeof(uint64_t));
    EXPECT_THAT((CountingRawPtrExpectations {
                    .get_for_dereference_cnt = 0,
