...
public:
    ...
    SafeRef(const SafeRef<U>& p)
        : w_(p.w_)
    {
        // Avoid use-after-move.
        CHECK(w_);
    }
    template <typename U> SafeRef& operator=(const SafeRef<U>& p)
    {
        w_ = p.w_;
        // Avoid use-after-move.
        CHECK(w_);
    ...
    template <typename U> SafeRef& operator=(SafeRef<U>&& p)
    {
        w_ = std::move(p.w_);
        // Avoid use-after-move.
        CHECK(w_);
...
private:
    template <typename U> friend class SafeRef;
