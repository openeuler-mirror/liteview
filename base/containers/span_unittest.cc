...
constexpr bool constexpr_equal(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2)
{
...
TEST(SpanTest, AllowedConversionsFromStdArray)
{
    ...
    static_assert(std::is_convertible<std::array<int, 3>&, base::span<int>>::value,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<int> with dynamic extent.");
    static_assert(std::is_convertible<std::array<int, 3>&, base::span<int, 3>>::value,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<int> with the same static extent.");
    static_assert(std::is_convertible<std::array<int, 3>&, base::span<const int>>::value,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible<std::array<int, 3>&, base::span<const int, 3>>::value,
        "Error: l-value reference to std::array<int> should be convertible to "
        "base::span<const int> with the same static extent.");
    static_assert(std::is_convertible<const std::array<int, 3>&, base::span<const int>>::value,
        "Error: const l-value reference to std::array<int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible<const std::array<int, 3>&, base::span<const int, 3>>::value,
        "Error: const l-value reference to std::array<int> should be convertible "
        "to base::span<const int> with the same static extent.");
    static_assert(std::is_convertible<std::array<const int, 3>&, base::span<const int>>::value,
        "Error: l-value reference to std::array<const int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible<std::array<const int, 3>&, base::span<const int, 3>>::value,
        "Error: l-value reference to std::array<const int> should be convertible "
        "to base::span<const int> with the same static extent.");
    static_assert(std::is_convertible<const std::array<const int, 3>&, base::span<const int>>::value,
        "Error: const l-value reference to std::array<const int> should be "
        "convertible to base::span<const int> with dynamic extent.");
    static_assert(std::is_convertible<const std::array<const int, 3>&, base::span<const int, 3>>::value,
        "Error: const l-value reference to std::array<const int> should be "
...
TEST(SpanTest, DisallowedConstructionsFromStdArray)
{
    ...
    // as well.
    static_assert(!std::is_constructible<base::span<int>, const std::array<int, 3>&>::value,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from const l-value reference to std::array<int>");

    static_assert(!std::is_constructible<base::span<int>, std::array<const int, 3>&>::value,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from l-value reference to std::array<const int>");

    static_assert(!std::is_constructible<base::span<int>, const std::array<const int, 3>&>::value,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "const from l-value reference to std::array<const int>");

    static_assert(!std::is_constructible<base::span<int, 2>, std::array<int, 3>&>::value,
    ...
    static_assert(!std::is_constructible<base::span<int, 4>, std::array<int, 3>&>::value,
        "Error: base::span<int> with dynamic extent should not be constructible "
        "from l-value reference to std::array<int> with different extent");

    static_assert(!std::is_constructible<base::span<int>, std::array<bool, 3>&>::value,
        "Error: base::span<int> with dynamic extent should not be constructible "
...
TEST(SpanTest, ConstexprIterator)
{
    ...
    static_assert(constexpr_equal(std::begin(kArray), std::end(kArray), span.begin(), span.end()), "");
...
TEST(SpanTest, ReverseIterator)
{
    ...
    EXPECT_TRUE(std::equal(std::rbegin(kArray), std::rend(kArray), span.rbegin(), span.rend()));
    EXPECT_TRUE(std::equal(std::crbegin(kArray), std::crend(kArray), std::crbegin(span), std::crend(span)));
}
...
{
    {
        ...
        span<const uint8_t, sizeof(kArray)> bytes_span = as_bytes(make_span(kArray));
...
TEST(SpanTest, MakeSpanFromDataAndSize)
{
    ...
    static_assert(std::is_same<decltype(expected_span), decltype(made_span)>::value, "the type of made_span differs from expected_span!");
}
...
TEST(SpanTest, MakeStaticSpanFromConstexprContainer)
{
    ...
    static_assert(std::is_same<decltype(str)::value_type, decltype(made_span)::value_type>::value, "Error: value_type does not match");
    static_assert(str.size() == decltype(made_span)::extent, "Error: extent does not match");
}
...
TEST(SpanTest, MakeSpanFromDynamicSpan)
{
    ...
    static_assert(std::is_same<decltype(expected_span)::element_type, decltype(made_span)::element_type>::value,
...
TEST(SpanTest, IteratorIsRangeMoveSafe)
{
    ...
    for (const int dest_start_index : kOverlappingStartIndexes) {
        EXPECT_FALSE(CheckedContiguousIterator<const int>::IsRangeMoveSafe(
            span.begin(), span.end(), CheckedContiguousIterator<const int>(span.data() + dest_start_index, span.data() + dest_start_index + kNumElements)));
    }
    ...
    EXPECT_TRUE(
        CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.begin(), span.begin(), CheckedContiguousIterator<const int>(span.data(), span.data())));

    // IsRangeMoveSafe is false if end < begin.
    EXPECT_FALSE(
        CheckedContiguousIterator<const int>::IsRangeMoveSafe(span.end(), span.begin(), CheckedContiguousIterator<const int>(span.data(), span.data())));
}
...
TEST(SpanTest, SpanExtentConversions)
{
    ...
    static_assert(!std::is_constructible<span<int, 0>, span<int>>::value, "Error: static span should not be constructible from dynamic span");
    ...
    static_assert(std::is_convertible<span<int, 0>, span<int>>::value, "Error: static span should be convertible to dynamic span");

    static_assert(std::is_convertible<span<int>, span<int>>::value, "Error: dynamic span should be convertible to dynamic span");

    static_assert(std::is_convertible<span<int, 2>, span<int, 2>>::value, "Error: static span should be convertible to static span");
}

TEST(SpanTest, IteratorConversions)
{
    static_assert(std::is_convertible<span<int>::iterator, span<const int>::iterator>::value, "Error: iterator should be convertible to const iterator");

    static_assert(!std::is_convertible<span<const int>::iterator, span<int>::iterator>::value, "Error: const iterator should not be convertible to iterator");
}
