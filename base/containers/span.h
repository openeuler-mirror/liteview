...
template <typename T> using Extent = ExtentImpl<remove_cvref_t<T>>;
...
template <typename Iter, typename T> using IteratorHasConvertibleReferenceType = IsLegalDataConversion<std::remove_reference_t<iter_reference_t<Iter>>, T>;
...
template <typename T, size_t Extent> class GSL_POINTER span : public internal::ExtentStorage<Extent> {
private:
...
template <class T, size_t Extent> constexpr size_t span<T, Extent>::extent;
