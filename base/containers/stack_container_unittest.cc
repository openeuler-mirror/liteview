...
TEST(StackContainer, Vector)
{
    ...
    std::vector<int, StackAllocator<int, stack_size, std::allocator<int>>> other(vect.container());
...
template <typename T, size_t size> void CheckStackVectorElements(const StackVector<T, size>& vec, std::initializer_list<T> expected)
{
