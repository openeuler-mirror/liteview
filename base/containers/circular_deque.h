...
protected:
    ...
    void Add(difference_type delta)
    {
        ...
        DCHECK(new_offset >= 0 && new_offset <= static_cast<difference_type>(parent_deque_->size()));
    ...
    void CheckUnstableUsage() const
    {
        ...
        DCHECK(created_generation_ == parent_deque_->generation_) << "circular_deque iterator dereferenced after mutation.";
    }
...
public:
    ...
    // Range constructor.
    template <class InputIterator> circular_deque(InputIterator first, InputIterator last)
    {
        assign(first, last);
    }
    ...
    typename std::enable_if<::base::internal::is_iterator<InputIterator>::value, void>::type assign(InputIterator first, InputIterator last)
    {
    ...
    typename std::enable_if<::base::internal::is_iterator<InputIterator>::value, void>::type insert(const_iterator pos, InputIterator first, InputIterator last)
    {
        ValidateIterator(pos);
        ...
        const size_type inserted_items = static_cast<size_type>(inserted_items_signed);
    ...
    iterator erase(const_iterator first, const_iterator last)
    {
        ...
        for (; move_src < move_src_end; move_src++, move_dest++) {
            buffer_.MoveRange(&buffer_[move_src.index_], &buffer_[move_src.index_ + 1], &buffer_[move_dest.index_]);
        }
...
private:
    ...
    static void MoveBuffer(VectorBuffer& from_buf, size_t from_begin, size_t from_end, VectorBuffer* to_buf, size_t* to_begin, size_t* to_end)
    {
