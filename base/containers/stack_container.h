...
// buffer.
template <typename T, size_t stack_capacity, typename FallbackAllocator> class StackAllocator : public /*FallbackAllocator*/ std::allocator<T> {
public:
    ...
    template <typename U> struct rebind {
        typedef StackAllocator<U, stack_capacity, std::allocator<T>> other;
    };

    // For the straight up copy c-tor, we can share storage.
    StackAllocator(const StackAllocator<T, stack_capacity, std::allocator<T>>& rhs)
        : source_(rhs.source_)
    {
    }
...
auto begin(const StackContainer<TContainerType, stack_capacity>& stack_container) -> decltype(begin(stack_container.container()))
{
    return begin(stack_container.container());
}

template <typename TContainerType, int stack_capacity>
auto begin(StackContainer<TContainerType, stack_capacity>& stack_container) -> decltype(begin(stack_container.container()))
{
    return begin(stack_container.container());
}

template <typename TContainerType, int stack_capacity>
auto end(StackContainer<TContainerType, stack_capacity>& stack_container) -> decltype(end(stack_container.container()))
{
    return end(stack_container.container());
}

template <typename TContainerType, int stack_capacity>
auto end(const StackContainer<TContainerType, stack_capacity>& stack_container) -> decltype(end(stack_container.container()))
{
    return end(stack_container.container());
}
...
class StackVector : public StackContainer<std::vector<T, StackAllocator<T, stack_capacity, FallbackAllocator>>, stack_capacity> {
public:
    StackVector()
        : StackContainer<std::vector<T, StackAllocator<T, stack_capacity, FallbackAllocator>>, stack_capacity>()
    {
    }
    ...
    StackVector<T, stack_capacity, FallbackAllocator>& operator=(const StackVector<T, stack_capacity, FallbackAllocator>& other)
    {
        this->container().assign(other->begin(), other->end());
        return *this;
    }
