...
template <typename T, typename SFINAE = void> struct HasKeyType : std::false_type { };

template <typename T> struct HasKeyType<T, std::void_t<typename T::key_type>> : std::true_type { };
...
constexpr auto ContainsImpl(const Container& container, const Value& value, priority_tag<2>) -> decltype(container.contains(value))
{
...
template <typename Container, typename Value>
constexpr auto ContainsImpl(const Container& container, const Value& value, priority_tag<1>) -> decltype(container.find(value) != container.end())
{
...
template <typename Container, typename Value>
constexpr auto ContainsImpl(const Container& container, const Value& value, priority_tag<1>) -> decltype(container.find(value) != Container::npos)
{
...
template <typename Container, typename Value> constexpr bool ContainsImpl(const Container& container, const Value& value, priority_tag<0>)
{
