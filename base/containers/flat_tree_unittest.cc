...
// Common test trees.
template <typename ContainerT> using TypedTree = flat_tree<typename ContainerT::value_type, base::identity, std::less<>, ContainerT>;
using IntTree = TypedTree<std::vector<int>>;
using IntPair = std::pair<int, int>;
using IntPairTree = flat_tree<IntPair, base::identity, LessByFirst<IntPair>, std::vector<IntPair>>;
using MoveOnlyTree = flat_tree<MoveOnlyInt, base::identity, std::less<>, std::vector<MoveOnlyInt>>;
using EmplaceableTree = flat_tree<Emplaceable, base::identity, std::less<>, std::vector<Emplaceable>>;
using ReversedTree = flat_tree<int, base::identity, std::greater<int>, std::vector<int>>;

using TreeWithStrangeCompare = flat_tree<int, base::identity, NonDefaultConstructibleCompare, std::vector<int>>;
...
{
    static_assert(!is_multipass<std::istream_iterator<int>>(), "InputIterator is not multipass");
    static_assert(!is_multipass<std::ostream_iterator<int>>(), "OutputIterator is not multipass");

    static_assert(is_multipass<std::forward_list<int>::iterator>(), "ForwardIterator is multipass");
    static_assert(is_multipass<std::list<int>::iterator>(), "BidirectionalIterator is multipass");
    static_assert(is_multipass<std::vector<int>::iterator>(), "RandomAccessIterator is multipass");
}
...
TEST(FlatTree, NoExcept)
{
    ...
    using MoveThrowsTree = flat_tree<MoveThrows, base::identity, std::less<>, std::array<MoveThrows, 1>>;

    static_assert(std::is_nothrow_move_constructible<IntTree>::value, "Error: IntTree is not nothrow move constructible");
    static_assert(std::is_nothrow_move_assignable<IntTree>::value, "Error: IntTree is not nothrow move assignable");

    static_assert(!std::is_nothrow_move_constructible<MoveThrowsTree>::value, "Error: MoveThrowsTree is nothrow move constructible");
    static_assert(!std::is_nothrow_move_assignable<MoveThrowsTree>::value, "Error: MoveThrowsTree is nothrow move assignable");
}
...
TEST(FlatTree, Stability)
{
    ...
    using Tree = flat_tree<Pair, base::identity, LessByFirst<Pair>, std::vector<Pair>>;
...
TEST(FlatTree, KeyComp)
{
    ...
    std::copy(std::begin(new_elements), std::end(new_elements), std::inserter(cont, cont.end()));
    EXPECT_TRUE(ranges::is_sorted(cont, cont.key_comp()));
}
