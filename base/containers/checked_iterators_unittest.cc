...
// from the base class.
namespace std {
template <typename Iter> struct __is_cpp17_contiguous_iterator<DisableDerefAndIncr<Iter>> : __is_cpp17_contiguous_iterator<Iter> { };

template <typename Iter> struct pointer_traits<DisableDerefAndIncr<Iter>> : pointer_traits<Iter> { };
} // namespace std
...
TEST(CheckedContiguousIterator, OptimizedCopy)
{
    ...
    EXPECT_TRUE(std::equal(std::begin(arr_in), std::end(arr_in), std::begin(arr_out), std::end(arr_out)));
}
