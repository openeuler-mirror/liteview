...
void ExpectCalledOnValidSequenceWithSequenceToken(SequenceCheckerImpl* sequence_checker, SequenceToken sequence_token)
{
    ScopedSetSequenceTokenForCurrentThread scoped_set_sequence_token_for_current_thread(sequence_token);
...
{
    ScopedSetSequenceTokenForCurrentThread scoped_set_sequence_token_for_current_thread(SequenceToken::Create());
    SequenceCheckerImpl sequence_checker;
    EXPECT_TRUE(sequence_checker.CalledOnValidSequence());
}
...
TEST(SequenceCheckerTest, CallsAllowedOnDifferentThreadsSameSequenceToken)
{
    ...
    RunCallbackThread thread(BindOnce(&ExpectCalledOnValidSequenceWithSequenceToken, Unretained(&sequence_checker), sequence_token));
}
...
{
    auto scope = std::make_unique<ScopedSetSequenceTokenForCurrentThread>(SequenceToken::Create());
...
TEST(SequenceCheckerTest, CalledOnValidSequenceFromThreadDestruction)
{
    ...
    ThreadLocalOwnedPointer<SequenceCheckerOwner> thread_local_owner;
    {
        ...
        task_runner->PostTask(FROM_HERE, BindLambdaForTesting([&]() { thread_local_owner.Set(std::make_unique<SequenceCheckerOwner>()); }));
        task_runner = nullptr;
