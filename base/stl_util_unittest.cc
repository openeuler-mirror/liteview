...
template <typename Container> void RunConstCastIteratorTest()
{
    using std::begin;
    using std::cbegin;
    ...
    static_assert(std::is_same<decltype(cbegin(std::declval<Container&>())), decltype(c_it)>::value, "c_it is not a constant iterator.");
    static_assert(std::is_same<decltype(begin(std::declval<Container&>())), decltype(it)>::value, "it is not a iterator.");
    EXPECT_EQ(c_it, it);
...
TEST(STLUtilTest, ToUnderlying)
{
    ...
    static_assert(std::is_same<decltype(to_underlying(ScopedEnum::kOne)), char>::value, "");
    static_assert(std::is_same<decltype(to_underlying(ScopedEnum::kTwo)), char>::value, "");
...
{
    {
        ...
        static_assert(
            std::is_same<decltype(GetUnderlyingContainer(queue)), const std::deque<int>&>::value, "GetUnderlyingContainer(queue) should be of type deque");
...

    {
        ...
        static_assert(std::is_same<decltype(GetUnderlyingContainer(queue)), const base::circular_deque<int>&>::value,
...

    {
        ...
        static_assert(
            std::is_same<decltype(GetUnderlyingContainer(queue)), const std::vector<int>&>::value, "GetUnderlyingContainer(queue) should be of type vector");
...

    {
        ...
        static_assert(
            std::is_same<decltype(GetUnderlyingContainer(stack)), const std::deque<int>&>::value, "GetUnderlyingContainer(stack) should be of type deque");
