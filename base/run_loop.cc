...
void RunLoop::RegisterDelegateForCurrentThread(Delegate* delegate)
{
    ...
    DCHECK(!GetTlsDelegate().Get()) << "Error: Multiple RunLoop::Delegates registered on the same thread.\n\n"
...
void RunLoop::QuitCurrentDeprecated()
{
    ...
    Delegate* delegate = GetTlsDelegate().Get();
    DCHECK(delegate->active_run_loops_.top()->allow_quit_current_deprecated_) << "Please migrate off QuitCurrentDeprecated(), e.g. to QuitClosure().";
...
void RunLoop::QuitCurrentWhenIdleDeprecated()
{
    DCHECK(IsRunningOnCurrentThread());
    Delegate* delegate = GetTlsDelegate().Get();
    DCHECK(delegate->active_run_loops_.top()->allow_quit_current_deprecated_) << "Please migrate off QuitCurrentWhenIdleDeprecated(), e.g. to "
                                                                                 "QuitWhenIdleClosure().";
...
ScopedDisallowRunningRunLoop::ScopedDisallowRunningRunLoop()
    ...
    , previous_run_allowance_(current_delegate_ ? current_delegate_->allow_running_for_testing_ : false)
{
