...
base::StringPiece MakeNotNullTerminatedInput(const char* input, std::unique_ptr<char[]>* owner)
{
...
{
    absl::optional<Value> root = JSONReader::Read("\" \\\"\\\\\\/\\b\\f\\n\\r\\t\\v\"");
    ASSERT_TRUE(root);
    ASSERT_TRUE(root->is_string());
    EXPECT_EQ(" \"\\/\b\f\n\r\t\v", root->GetString());
}
...
TEST(JSONReaderTest, UnicodeEscapes)
{
    ...
    absl::optional<Value> root = JSONReader::Read("\"\\x41\\xFF\\x00\\u1234\\u0000\"");
    ASSERT_TRUE(root);
    ASSERT_TRUE(root->is_string());
...
TEST(JSONReaderTest, BasicArray)
{
    ...
    absl::optional<Value> root2 = JSONReader::Read("[true, false, null, ]", JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(root2);
...
{
    absl::optional<Value> value = JSONReader::Read("[[true], [], {\"smell\": \"nice\",\"taste\": \"yummy\" }, [false, [], "
                                                   "[null]], null]");
    ASSERT_TRUE(value);
    Value::List* list = value->GetIfList();
    ASSERT_TRUE(list);
    ...
    absl::optional<Value> root2 = JSONReader::Read("[[true], [], {\"smell\": \"nice\",\"taste\": \"yummy\" }, [false, [], "
                                                   "[null, ]  , ], null,]",
...
TEST(JSONReaderTest, ArrayTrailingComma)
{
    ...
    absl::optional<Value> value = JSONReader::Read("[true,]", JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(value);
    Value::List* list = value->GetIfList();
    ASSERT_TRUE(list);
...
{
    absl::optional<Value> root1 = JSONReader::Read("{\"number\":9.87654321, \"null\":null , \"\\x53\" : \"str\", \"bool\": "
                                                   "false, \"more\": {} }");
    ASSERT_TRUE(root1);
    ...
    absl::optional<Value> root2 = JSONReader::Read("{\"number\":9.87654321, \"null\":null , \"\\x53\" : \"str\", \"bool\": "
                                                   "false, \"more\": {},}",
...
{
    absl::optional<Value> root1 = JSONReader::Read("{\"inner\":{\"array\":[true, 3, 4.56, null]},\"false\":false,\"d\":{}}");
    ASSERT_TRUE(root1);
    ...
    absl::optional<Value> root2 = JSONReader::Read("{\"inner\": {\"array\":[true, 3, 4.56, null] , "
                                                   "},\"false\":false,\"d\":{},}",
        JSON_ALLOW_TRAILING_COMMAS);
    ASSERT_TRUE(root2);
...
{
    absl::optional<Value> root = JSONReader::Read("{\"a.b\":3,\"c\":2,\"d.e.f\":{\"g.h.i.j\":1}}");
    ASSERT_TRUE(root);
...
TEST(JSONReaderTest, ASCIIControlCodes)
{
    ...
    absl::optional<Value> root = JSONReader::Read(std::string(json, sizeof(json)));
    ASSERT_TRUE(root);
    ASSERT_TRUE(root->is_string());
...
TEST(JSONReaderTest, ReplaceInvalidCharacters)
{
    ...
    absl::optional<Value> value = JSONReader::Read(invalid_high, JSON_REPLACE_INVALID_CHARACTERS);
    ASSERT_TRUE(value);
...
TEST(JSONReaderTest, ParseNumberErrors)
{
    ...
    for (unsigned int i = 0; i < std::size(kCases); ++i) {
        ...
        StringPiece input = MakeNotNullTerminatedInput(test_case.input, &input_owner);
...
TEST(JSONReaderTest, UnterminatedInputs)
{
    const char* const kCases[] = {
        // clang-format on
        "/", "//", "/*", "\"xxxxxx", "\"", "{   ", "[\t", "tru", "fals", "nul", "\"\\x", "\"\\x2", "\"\\u123", "\"\\uD803\\u", "\"\\", "\"\\/",
        // clang-format on
    };
...
TEST(JSONReaderTest, LineColumnCounting)
{
    ...
    for (unsigned int i = 0; i < std::size(kCases); ++i) {
        ...
        auto root = JSONReader::ReadAndReturnValueWithError(test_case.input, JSON_PARSE_RFC | JSON_ALLOW_CONTROL_CHARS);
        EXPECT_FALSE(root.has_value());
