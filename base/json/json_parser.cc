...
JSONParser::StringBuilder& JSONParser::StringBuilder::operator=(StringBuilder&& other) = default;
...
bool JSONParser::ConsumeStringRaw(StringBuilder* out)
{
    ...
    while (PeekChar()) {
        ...
        if (!ReadUnicodeCharacter(input_.data(), input_.length(), &index_, &next_char) || !IsValidCodepoint(next_char)) {
        ...
        if (next_char != '\\') {
            ...
            case 'x': { // UTF-8 sequence.
                ...
                int hex_digit = 0;
                if (!UnprefixedHexStringToInt(*escape_sequence, &hex_digit) || !IsValidCharacter(hex_digit)) {
                    ReportError(JSON_INVALID_ESCAPE, -3);
                    return false;
                }
