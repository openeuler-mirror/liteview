...
#define JSON_NEWLINE "\n"
    ...
    EXPECT_TRUE(JSONWriter::WriteWithOptions(root_dict, JSONWriter::OPTIONS_PRETTY_PRINT, &output_js));
    EXPECT_EQ("{" JSON_NEWLINE "   \"list\": [ {" JSON_NEWLINE "      \"inner int\": 10" JSON_NEWLINE "   }, {" JSON_NEWLINE "   }, [  ], true ]" JSON_NEWLINE
              "}" JSON_NEWLINE,
        output_js);
...
TEST(JSONWriterTest, BinaryValues)
{
    ...
    const auto kBufferSpan = base::make_span(reinterpret_cast<const uint8_t*>("asdf"), 4);
    Value root(kBufferSpan);
    EXPECT_FALSE(JSONWriter::Write(root, &output_js));
    EXPECT_TRUE(JSONWriter::WriteWithOptions(root, JSONWriter::OPTIONS_OMIT_BINARY_VALUES, &output_js));
    ...
    EXPECT_TRUE(JSONWriter::WriteWithOptions(binary_list, JSONWriter::OPTIONS_OMIT_BINARY_VALUES, &output_js));
    ...
    EXPECT_TRUE(JSONWriter::WriteWithOptions(binary_dict, JSONWriter::OPTIONS_OMIT_BINARY_VALUES, &output_js));
...
TEST(JSONWriterTest, DoublesAsInts)
{
    ...
    EXPECT_TRUE(JSONWriter::WriteWithOptions(double_value, JSONWriter::OPTIONS_OMIT_DOUBLE_TYPE_PRESERVATION, &output_js));
...
TEST(JSONWriterTest, StackOverflow)
{
    ...
    EXPECT_FALSE(JSONWriter::WriteWithOptions(deep_list_value, JSONWriter::OPTIONS_PRETTY_PRINT, &output_js));
...
TEST(JSONWriterTest, TestMaxDepthWithValidNodes)
{
    ...
    auto json_val = JSONReader::ReadAndReturnValueWithError(nested_json, JSON_ALLOW_TRAILING_COMMAS);
