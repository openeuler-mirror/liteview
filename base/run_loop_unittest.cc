...
void RunNestedLoopTask(int* counter)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&nested_run_loop), Unretained(counter)));

    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
public:
    ...
    {
        thread_task_runner_handle_ = std::make_unique<ThreadTaskRunnerHandle>(simple_task_runner_);
...
{
    int counter = 0;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&run_loop_), Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
{
    int counter = 0;
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&RunNestedLoopTask, Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce(&QuitWhenIdleTask, Unretained(&run_loop_), Unretained(&counter)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
{
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, run_loop_.QuitWhenIdleClosure());
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, MakeExpectedRunClosure(FROM_HERE));
    ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE), Days(1));
...
TEST_P(RunLoopTest, QuitFromOtherSequence)
{
    ...
    loop_was_quit.Wait();
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, MakeExpectedNotRunClosure(FROM_HERE));
...
TEST_P(RunLoopTest, IsRunningOnCurrentThread)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([]() { EXPECT_TRUE(RunLoop::IsRunningOnCurrentThread()); }));
...
TEST_P(RunLoopTest, IsNestedOnCurrentThread)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([]() {
        ...
        ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([]() { EXPECT_TRUE(RunLoop::IsNestedOnCurrentThread()); }));
...
TEST_P(RunLoopTest, NestingObservers)
{
    ...
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MockTask::Task, base::Unretained(&mock_task_a)));
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, run_nested_loop);
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MockTask::Task, base::Unretained(&mock_task_b)));
...
TEST(RunLoopDelegateTest, NestableTasksDontRunInDefaultNestedLoops)
{
    ...
    // it's quit.
    ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, BindOnce([](RunLoop* nested_run_loop) { nested_run_loop->Run(); }, Unretained(&nested_run_loop)));
