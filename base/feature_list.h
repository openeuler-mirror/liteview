...
// header files.
#define BASE_FEATURE(feature, name, default_state) CONSTINIT const base::Feature feature(name, default_state)
...
public:
    ...
    public:
        ...
        FeatureList::OverrideState GetOverrideStateByFeatureName(StringPiece feature_name);

        // Look up the feature, and, if present, populate |params|.
        // See GetFieldTrialParams in field_trial_params.h for more documentation.
        bool GetParamsByFeatureName(StringPiece feature_name, std::map<std::string, std::string>* params);
    ...
    void InitializeFromCommandLine(const std::string& enable_features, const std::string& disable_features);
    ...
    base::FieldTrial* GetAssociatedFieldTrialByFeatureName(StringPiece name) const;
    ...
    static std::vector<base::StringPiece> SplitFeatureListString(base::StringPiece input);
    ...
    static bool InitializeInstance(const std::string& enable_features, const std::string& disable_features);
...
private:
    ...
    const base::FeatureList::OverrideEntry* GetOverrideEntryByFeatureName(StringPiece name) const;
    ...
    absl::optional<bool> IsFeatureEnabledIfOverridden(const Feature& feature) const;
    ...
    // will not be changed.
    void RegisterOverride(StringPiece feature_name, OverrideState overridden_state, FieldTrial* field_trial);
