...
TEST_P(WaitableEventWatcherTest, BasicSignalManual)
{
    ...
    watcher.StartWatching(&event, BindOnce(&QuitWhenSignaled), SequencedTaskRunnerHandle::Get());
...
TEST_P(WaitableEventWatcherTest, CancelAfterSet)
{
    ...
    watcher.StartWatching(&event, std::move(callback), SequencedTaskRunnerHandle::Get());
...
TEST_P(WaitableEventWatcherTest, MultipleWatchersManual)
{
    ...
    watcher1.StartWatching(&event, BindOnce(BindLambdaForTesting(callback), Unretained(&watcher1_counter)), SequencedTaskRunnerHandle::Get());

    WaitableEventWatcher watcher2;
    watcher2.StartWatching(&event, BindOnce(BindLambdaForTesting(callback), Unretained(&watcher2_counter)), SequencedTaskRunnerHandle::Get());
...
TEST_P(WaitableEventWatcherTest, MultipleWatchersAutomatic)
{
    ...
    WaitableEventWatcher watcher1;
    watcher1.StartWatching(&event, BindOnce(callback, Unretained(&current_run_loop), Unretained(&counter1)), SequencedTaskRunnerHandle::Get());

    WaitableEventWatcher watcher2;
    watcher2.StartWatching(&event, BindOnce(callback, Unretained(&current_run_loop), Unretained(&counter2)), SequencedTaskRunnerHandle::Get());
...
TEST_P(WaitableEventWatcherDeletionTest, SignalAndDelete)
{
...

    {
        ...
        watcher.StartWatching(event.get(), BindOnce(&QuitWhenSignaled), SequencedTaskRunnerHandle::Get());
        event->Signal();
        event.reset();
...
TEST_P(WaitableEventWatcherDeletionTest, DeleteWatcherBeforeCallback)
{
    ...
    test::TaskEnvironment task_environment(main_thread_type);
    scoped_refptr<SingleThreadTaskRunner> task_runner = ThreadTaskRunnerHandle::Get();
