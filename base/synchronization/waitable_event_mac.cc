...
void WaitableEvent::Signal()
{
    ...
    // has a length of one.
    kern_return_t kr = mach_msg(&msg.header, MACH_SEND_MSG | MACH_SEND_TIMEOUT, sizeof(msg), 0, MACH_PORT_NULL, 0, MACH_PORT_NULL);
...
bool WaitableEvent::TimedWait(const TimeDelta& wait_delta)
{
    ...
    absl::optional<internal::ScopedBlockingCallWithBaseSyncPrimitives> scoped_blocking_call;
    ...
    for (mach_msg_timeout_t timeout = wait_delta.is_max() ? MACH_MSG_TIMEOUT_NONE : saturated_cast<mach_msg_timeout_t>(wait_delta.InMillisecondsRoundedUp());
        ...
        kr = mach_msg(&msg.header, options, 0, rcv_size, receive_right_->Name(), timeout, MACH_PORT_NULL);
    }
...
size_t WaitableEvent::WaitMany(WaitableEvent** raw_waitables, size_t count)
{
    ...
    if (kPrimitive == KQUEUE) {
        ...
        int rv = HANDLE_EINTR(kevent64(wait_many.get(), events.data(), count_int, out_events.data(), count_int, 0, nullptr));
        ...
        kr = mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_LARGE | MACH_RCV_LARGE_IDENTITY, 0, sizeof(msg.header), port_set.get(), 0, MACH_PORT_NULL);
...
bool WaitableEvent::PeekPort(mach_port_t port, bool dequeue)
{
    if (dequeue) {
        ...
        kern_return_t kr = mach_msg(&msg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0, sizeof(msg), port, 0, MACH_PORT_NULL);
        if (kr == KERN_SUCCESS) {
            return true;
        } else {
            MACH_CHECK(kr == MACH_RCV_TIMED_OUT, kr) << "mach_msg";
