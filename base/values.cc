...
const char* const kTypeNames[] = { "null", "boolean", "integer", "double", "string", "binary", "dictionary", "list" };
static_assert(std::size(kTypeNames) == static_cast<size_t>(Value::Type::LIST) + 1, "kTypeNames Has Wrong Size");
...
class PathSplitter {
public:
    explicit PathSplitter(StringPiece path)
        : path_(path)
    {
    }
...
Value::Value(StringPiece value)
    : Value(std::string(value))
{
}

Value::Value(StringPiece16 value)
...
{
    return (is_int() || is_double()) ? absl::make_optional(GetDouble()) : absl::nullopt;
}
...
absl::optional<double> Value::Dict::FindDoubleByDottedPath(StringPiece path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfDouble() : absl::nullopt;
}
...
const Value::BlobStorage* Value::Dict::FindBlobByDottedPath(StringPiece path) const
{
    const Value* v = FindByDottedPath(path);
    return v ? v->GetIfBlob() : nullptr;
}
...
absl::optional<Value> Value::Dict::ExtractByDottedPath(StringPiece path)
{
    ...
    absl::optional<Value> extracted = next_dict->ExtractByDottedPath(path.substr(dot_index + 1));
...
{
    return iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()));
}

Value::List::const_iterator Value::List::begin() const
{
    return const_iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()));
}

Value::List::const_iterator Value::List::cbegin() const
{
    return const_iterator(base::to_address(storage_.cbegin()), base::to_address(storage_.cend()));
}

Value::List::iterator Value::List::end()
{
    return iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()), base::to_address(storage_.end()));
}

Value::List::const_iterator Value::List::end() const
{
    return const_iterator(base::to_address(storage_.begin()), base::to_address(storage_.end()), base::to_address(storage_.end()));
}

Value::List::const_iterator Value::List::cend() const
{
    return const_iterator(base::to_address(storage_.cbegin()), base::to_address(storage_.cend()), base::to_address(storage_.cend()));
}
...
Value::List::iterator Value::List::erase(iterator pos)
{
    ...
    return iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end()));
}

Value::List::const_iterator Value::List::erase(const_iterator pos)
{
    auto next_it = storage_.erase(storage_.begin() + (pos - begin()));
    return const_iterator(base::to_address(storage_.begin()), base::to_address(next_it), base::to_address(storage_.end()));
}
...
Value::List::iterator Value::List::Insert(const_iterator pos, Value&& value)
{
    ...
    return iterator(base::to_address(storage_.begin()), base::to_address(inserted_it), base::to_address(storage_.end()));
}
...
Value* Value::FindPathOfType(std::initializer_list<StringPiece> path, Type type)
{
...
const Value* Value::FindPathOfType(std::initializer_list<StringPiece> path, Type type) const
{
    DCHECK_GE(path.size(), 2u) << "Use FindKeyOfType() for a path of length 1.";
    return FindPathOfType(make_span(path.begin(), path.size()), type);
}

const Value* Value::FindPathOfType(span<const StringPiece> path, Type type) const
{
...
Value* Value::SetPath(span<const StringPiece> path, Value&& value)
{
    ...
    for (; (cur_path + 1) < path.end(); ++cur_path) {
        ...
        if (found == cur->dict().end() || found->first != path_component) {
            ...
            auto inserted = cur->dict().try_emplace(found, path_component, std::make_unique<Value>(Type::DICTIONARY));
...
DictAdapterForMigration::DictAdapterForMigration(const Value::Dict& dict) noexcept
    : dict_(dict)
{
}

DictAdapterForMigration::DictAdapterForMigration(const DictionaryValue& dict) noexcept
    : dict_(dict.GetDict())
{
}
...
DictAdapterForMigration::const_iterator DictAdapterForMigration::cbegin() const
{
    return dict_.cbegin();
}
...
absl::optional<double> DictAdapterForMigration::FindDouble(StringPiece key) const
{
...
const Value::BlobStorage* DictAdapterForMigration::FindBlob(StringPiece key) const
{
...
absl::optional<bool> DictAdapterForMigration::FindBoolByDottedPath(StringPiece path) const
{
    return dict_.FindBoolByDottedPath(path);
}

absl::optional<int> DictAdapterForMigration::FindIntByDottedPath(StringPiece path) const
{
    return dict_.FindIntByDottedPath(path);
}

absl::optional<double> DictAdapterForMigration::FindDoubleByDottedPath(StringPiece path) const
{
    return dict_.FindDoubleByDottedPath(path);
}

const std::string* DictAdapterForMigration::FindStringByDottedPath(StringPiece path) const
{
    return dict_.FindStringByDottedPath(path);
}

const Value::BlobStorage* DictAdapterForMigration::FindBlobByDottedPath(StringPiece path) const
{
    return dict_.FindBlobByDottedPath(path);
}

const Value::Dict* DictAdapterForMigration::FindDictByDottedPath(StringPiece path) const
{
    return dict_.FindDictByDottedPath(path);
}

const Value::List* DictAdapterForMigration::FindListByDottedPath(StringPiece path) const
{
...
#if BUILDFLAG(ENABLE_BASE_TRACING)
void DictAdapterForMigration::WriteIntoTrace(perfetto::TracedValue context) const
{
...
Value* DictionaryValue::Set(StringPiece path, std::unique_ptr<Value> in_value)
{
    ...
    for (size_t delimiter_position = current_path.find('.'); delimiter_position != StringPiece::npos; delimiter_position = current_path.find('.')) {
    ...
    auto result = static_cast<DictionaryValue*>(current_dictionary)->dict().try_emplace(current_path, std::move(in_value));
    if (!result.second) {
...
Value* DictionaryValue::SetString(StringPiece path, const std::u16string& in_value)
{
    return Set(path, std::make_unique<Value>(in_value));
}

ListValue* DictionaryValue::SetList(StringPiece path, std::unique_ptr<ListValue> in_value)
{
...
bool DictionaryValue::GetString(StringPiece path, std::string* out_value) const
{
    const Value* value;
    if (!Get(path, &value))
        return false;
...
bool DictionaryValue::GetDictionary(StringPiece path, const DictionaryValue** out_value) const
{
    const Value* value;
...
bool DictionaryValue::GetDictionary(StringPiece path, DictionaryValue** out_value)
{
    return std::as_const(*this).GetDictionary(path, const_cast<const DictionaryValue**>(out_value));
}

bool DictionaryValue::GetList(StringPiece path, const ListValue** out_value) const
{
    const Value* value;
    bool result = Get(path, &value);
...
{
    return std::as_const(*this).GetList(path, const_cast<const ListValue**>(out_value));
}
