...
public:
    ...
    const RepeatingClosure& user_task() const
    {
        return user_task_;
    }
...
public:
    ...
    // already running, it will be replaced to call the given |user_task|.
    void Start(const Location& posted_from, TimeTicks deadline, OnceClosure user_task, ExactDeadline exact = ExactDeadline(false));
    ...
    // |receiver->*method|.
    template <class Receiver>
    void Start(const Location& posted_from, TimeTicks deadline, Receiver* receiver, void (Receiver::*method)(), ExactDeadline exact = ExactDeadline(false))
    {
